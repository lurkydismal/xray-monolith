...
public:
    bool _dx10_msaa_alphatest_atoc() {
        return ( RImplementation.o.dx10_msaa_alphatest ==
...
public:
    adopt_dx10sampler( CBlender_Compile* C, u32 SamplerIndex )
        : m_pC( C ), m_SI( SamplerIndex ) {
        if ( u32( -1 ) == m_SI )
            m_pC = 0;
    }

    adopt_dx10sampler( const adopt_dx10sampler& _C )
        : m_pC( _C.m_pC ), m_SI( _C.m_SI ) {
        if ( u32( -1 ) == m_SI )
            m_pC = 0;
    }
    ...
    //{ if (C) C->i_Filter
    //(stage,D3DTEXF_LINEAR,D3DTEXF_LINEAR,D3DTEXF_LINEAR);		return
    //*this;	} 	adopt_sampler&			_f_bilinear
    //()						{ if (C) C->i_Filter
    //(stage,D3DTEXF_LINEAR,D3DTEXF_POINT, D3DTEXF_LINEAR);		return
    //*this;	} 	adopt_sampler&			_f_linear
    //()						{ if (C) C->i_Filter
...
class	adopt_dx10texture
{
        CBlender_Compile*		m_pC;
        u32						m_TI;	//	Sampler
index public: adopt_dx10texture	(CBlender_Compile*	C, u32 TextureIndex)
: m_pC(C), m_TI(TextureIndex)		{ if (u32(-1)==m_TI) m_pC=0;}
        adopt_dx10texture	(const adopt_dx10texture&	_C)
: m_pC(_C.m_pC), m_TI(_C.m_TI)	{ if (u32(-1)==m_TI) m_pC=0;}
};
*/
...
public:
    adopt_compiler( CBlender_Compile* _C, bool& bFirstPass )
        : C( _C ), m_bFirstPass( bFirstPass ) {
        m_bFirstPass = true;
    }

    adopt_compiler( const adopt_compiler& _C )
        : C( _C.C ), m_bFirstPass( _C.m_bFirstPass ) {}
    ...
    //	DX10 specific
    adopt_compiler& _dx10color_write_enable( bool cR,
                                             bool cG,
                                             bool cB,
                                             bool cA ) {
    ...
    adopt_compiler& _dx10Stencil( bool Enable,
                                  u32 Func,
                                  u32 Mask,
                                  u32 WriteMask,
                                  u32 Fail,
    ...
    // adopt_dx10texture		_dx10texture	(LPCSTR _name)
    // {	u32 s = C->r_dx10Texture(_name,0);			return
...
static void* lua_alloc( void* ud, void* ptr, size_t osize, size_t nsize ) {
    ...
    memory_monitor::monitor_free( ptr );
    void* const result =
        g_render_lua_allocator.realloc_impl( ptr, ( u32 )nsize );
    memory_monitor::monitor_alloc( result, nsize, "render:LUA" );
    return result;
...
void CResourceManager::LS_Load() {
    ...
    module( LSVM )
        [ class_< adopt_dx10options >( "_dx10options" )
              .def( "dx10_msaa_alphatest_atoc",
                    &adopt_dx10options::_dx10_msaa_alphatest_atoc )
          ...
          class_< adopt_dx10sampler >( "_dx10sampler" )
              // TODO: why are these commented out?
              //.def("texture",
              //&adopt_sampler::_texture
              //,return_reference_to<1>()) .def("project",
              //&adopt_sampler::_projective
              //,return_reference_to<1>())
              .def( "clamp", &adopt_dx10sampler::_clamp,
                    return_reference_to< 1 >() )
          //.def("wrap",
          //&adopt_sampler::_wrap ,return_reference_to<1>()) .def("mirror",
          //&adopt_sampler::_mirror ,return_reference_to<1>())
          //.def("f_anisotropic",
          //&adopt_sampler::_f_anisotropic	,return_reference_to<1>())
          //.def("f_trilinear",
          //&adopt_sampler::_f_trilinear	,return_reference_to<1>())
          //.def("f_bilinear",
          //&adopt_sampler::_f_bilinear		,return_reference_to<1>())
          //.def("f_linear",
          //&adopt_sampler::_f_linear		,return_reference_to<1>())
          //.def("f_none",
          //&adopt_sampler::_f_none ,return_reference_to<1>()) .def("fmin_none",
          //&adopt_sampler::_fmin_none		,return_reference_to<1>())
          //.def("fmin_point",
          //&adopt_sampler::_fmin_point		,return_reference_to<1>())
          //.def("fmin_linear",
          //&adopt_sampler::_fmin_linear	,return_reference_to<1>())
          //.def("fmin_aniso",
          //&adopt_sampler::_fmin_aniso		,return_reference_to<1>())
          //.def("fmip_none",
          //&adopt_sampler::_fmip_none		,return_reference_to<1>())
          //.def("fmip_point",
          //&adopt_sampler::_fmip_point		,return_reference_to<1>())
          //.def("fmip_linear",
          //&adopt_sampler::_fmip_linear	,return_reference_to<1>())
          //.def("fmag_none",
          //&adopt_sampler::_fmag_none		,return_reference_to<1>())
          //.def("fmag_point",
          //&adopt_sampler::_fmag_point		,return_reference_to<1>())
          //.def("fmag_linear",
          //&adopt_sampler::_fmag_linear	,return_reference_to<1>())
          ,
          ...
          class_< adopt_compiler >( "_compiler" )
              ...
              .def( "begin", &adopt_compiler::_pass,
                    return_reference_to< 1 >() )
              .def( "begin", &adopt_compiler::_passgs,
                    return_reference_to< 1 >() )
              .def( "sorting", &adopt_compiler::_options,
                    return_reference_to< 1 >() )
              .def( "emissive", &adopt_compiler::_o_emissive,
                    return_reference_to< 1 >() )
              .def( "distort", &adopt_compiler::_o_distort,
                    return_reference_to< 1 >() )
              .def( "wmark", &adopt_compiler::_o_wmark,
                    return_reference_to< 1 >() )
              ...
              .def( "scopelense", &adopt_compiler::_o_scopelense,
                    return_reference_to< 1 >() ) //  Redotix99: for 3D Shader
                                                 //  Based Scopes

              //	For compatibility only
              .def( "dx10color_write_enable",
                    &adopt_compiler::_dx10color_write_enable,
                    return_reference_to< 1 >() )
              .def( "color_write_enable",
                    &adopt_compiler::_dx10color_write_enable,
                    return_reference_to< 1 >() )
              .def( "dx10texture", &adopt_compiler::_dx10texture,
                    return_reference_to< 1 >() )
              .def( "dx10stencil", &adopt_compiler::_dx10Stencil,
                    return_reference_to< 1 >() )
              .def( "dx10stencil_ref", &adopt_compiler::_dx10StencilRef,
                    return_reference_to< 1 >() )
              .def( "dx10cullmode", &adopt_compiler::_dx10CullMode,
                    return_reference_to< 1 >() )
              .def( "dx10atoc", &adopt_compiler::_dx10ATOC,
                    return_reference_to< 1 >() )
              .def( "dx10zfunc", &adopt_compiler::_dx10ZFunc,
                    return_reference_to< 1 >() )

              .def( "dx10sampler",
                    &adopt_compiler::_dx10sampler ) // returns sampler-object
              .def( "dx10Options",
                    &adopt_compiler::_dx10Options ), // returns options-object

          class_< adopt_blend >( "blend" ).enum_(
              "blend" )[ value( "zero", int( D3DBLEND_ZERO ) ),
    ...
    xr_vector< char* >* folder =
        FS.file_list_open( "$game_shaders$", ::Render->getShaderPath(),
    ...
    for ( u32 it = 0; it < folder->size(); it++ ) {
        ...
        *strext( namesp ) = 0;
        if ( 0 == namesp[ 0 ] )
            xr_strcpy( namesp, "_G" );
        strconcat( sizeof( fn ), fn, ::Render->getShaderPath(),
                   ( *folder )[ it ] );
...
#ifdef _EDITOR
    return Script::bfIsObjectPresent( LSVM, undercorated, "editor",
                                      LUA_TFUNCTION );
#else
    return Script::bfIsObjectPresent( LSVM, undercorated, "normal",
                                      LUA_TFUNCTION ) ||
           Script::bfIsObjectPresent( LSVM, undercorated, "l_special",
                                      LUA_TFUNCTION );
#endif
}
...
Shader* CResourceManager::_lua_Create( LPCSTR d_shader, LPCSTR s_textures ) {
    ...
    if ( Script::bfIsObjectPresent( LSVM, s_shader, "normal_hq",
                                    LUA_TFUNCTION ) ) {
        ...
        C.bDetail =
            dxRenderDeviceRender::Instance()
                .Resources->m_textures_description.GetDetailTexture(
        ...
        if ( Script::bfIsObjectPresent( LSVM, s_shader, "normal",
                                        LUA_TFUNCTION ) ) {
            C.iElement = 0;
            //.			C.bDetail			=
            // dxRenderDeviceRender::Instance().Resources->_GetDetailTexture(*C.L_textures[0],C.detail_texture,C.detail_scaler);
...
/// SSS fix water for DX10
    ...
    // Compile element
    if ( Script::bfIsObjectPresent( LSVM, s_shader, "l_point",
                                    LUA_TFUNCTION ) ) {
        C.iElement = 2;
        C.bDetail = FALSE;
    ...
    // Compile element
    if ( Script::bfIsObjectPresent( LSVM, s_shader, "l_spot",
                                    LUA_TFUNCTION ) ) {
        C.iElement = 3;
        C.bDetail = FALSE;
    ...
    // Compile element
    if ( Script::bfIsObjectPresent( LSVM, s_shader, "l_special",
                                    LUA_TFUNCTION ) ) {
        C.iElement = 4;
        C.bDetail = FALSE;
...
ShaderElement* CBlender_Compile::_lua_Compile( LPCSTR namesp, LPCSTR name ) {
    ...
    r_End();
    ShaderElement* _r =
        dxRenderDeviceRender::Instance().Resources->_CreateElement( E );
    return _r;
}
