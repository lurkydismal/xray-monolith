...
HRESULT dx10State::Apply() {
    ...
    //	VERIFY(m_pDepthStencilState);
    //	HW.pDevice->OMSetDepthStencilState(m_pDepthStencilState,
    //m_uiStencilRef); 	VERIFY(m_pBlendState);
...
void dx10State::InitSamplers( tSamplerHArray& SamplerArray,
        ...
        for ( int i = 0; i <= iMaxSampler; ++i ) {
            if ( SamplerUsed[ i ] )
                ...
                SamplerArray.push_back(
                    u32( dx10SamplerStateCache::hInvalidHandle ) );
        }
    }
}
