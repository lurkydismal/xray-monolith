...
dx103DFluidEmitters::dx103DFluidEmitters( int gridWidth,
                                          int gridHeight,
                                          int gridDepth,
...
void dx103DFluidEmitters::ApplyDensity( const CEmitter& Emitter ) {
    ...
    switch ( Emitter.m_eType ) {
        case ET_SimpleDraught:
            // fRadius += (0.1f - fRadius) * (1.0f + 0.5f * _sin( (1.0f/30.0f) *
            // t * (2.0f * float(PI))) ); float fFactor = 1.0f + 0.5f * _sin(t *
            // (2.0f * float(PI)) / 30 ); FlowVelocity.mul( fFactor );
            break;
    }

    // Color is the density of the smoke. We use a sinusoidal function of 't' to
    ...
    FLOAT density =
        1.5f *
        ( ( ( _sin( t * 1.5f + 2.0f * float( PI ) / 3.0f ) * 0.5f + 0.5f ) ) *
              saturation +
    ...
    Fvector4 center;
    center.set( Emitter.m_vPosition.x, Emitter.m_vPosition.y,
                Emitter.m_vPosition.z, 0 );
...
void dx103DFluidEmitters::ApplyVelocity( const CEmitter& Emitter ) {
    ...
    switch ( Emitter.m_eType ) {
        case ET_SimpleDraught:
            ...
            if ( fPeriod < 0.0001f )
                fPeriod = 0.0001f;
            float fFactor =
                1.0f + Emitter.m_DraughtParams.m_fAmp *
                           _sin( ( t + Emitter.m_DraughtParams.m_fPhase ) *
                                 ( 2.0f * float( PI ) ) / fPeriod );
    ...
    Fvector4 center;
    center.set( Emitter.m_vPosition.x + lilrand(),
                Emitter.m_vPosition.y + lilrand(),
                Emitter.m_vPosition.z + lilrand(), 0 );
    RCache.set_c( strImpulseCenter, center );
