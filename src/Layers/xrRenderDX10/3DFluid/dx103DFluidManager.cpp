...
namespace {
// For render call
// DrawTextureShaderVariable = pEffect->GetVariableByName(
// "textureNumber")->AsScalar();
...
// ImpulseCenterShaderVariable = pEffect->GetVariableByName(
// "center")->AsVector(); shared_str	strImpulseCenter("center");
// SplatColorShaderVariable = pEffect->GetVariableByName(
// "splatColor")->AsVector(); shared_str	strSplatColor("splatColor");
// For confinement
...
// HalfVolumeDimShaderVariable = pEffect->GetVariableByName(
// "halfVolumeDim")->AsVector();
...
dx103DFluidManager::dx103DFluidManager()
    ...
    // RenderTargetFormats [RENDER_TARGET_VELOCITY0]	=
    // DXGI_FORMAT_R16G16B16A16_FLOAT;
    RenderTargetFormats[ RENDER_TARGET_VELOCITY1 ] =
        DXGI_FORMAT_R16G16B16A16_FLOAT;
    ...
    RenderTargetFormats[ RENDER_TARGET_OBSTVELOCITY ] =
        DXGI_FORMAT_R16G16B16A16_FLOAT;
    // RENDER_TARGET_TEMPSCALAR: for AdvectBFECC and for Jacobi (for pressure
    // projection)
    ...
    RenderTargetFormats[ RENDER_TARGET_TEMPVECTOR ] =
        DXGI_FORMAT_R16G16B16A16_FLOAT;
}
...
void dx103DFluidManager::Initialize( int width, int height, int depth ) {
    ...
    m_pRenderer->Initialize( m_iTextureWidth, m_iTextureHeight,
                             m_iTextureDepth );

    m_pObstaclesHandler = xr_new< dx103DFluidObstacles >(
        m_iTextureWidth, m_iTextureHeight, m_iTextureDepth, m_pGrid );

    m_pEmittersHandler = xr_new< dx103DFluidEmitters >(
        m_iTextureWidth, m_iTextureHeight, m_iTextureDepth, m_pGrid );
...
void dx103DFluidManager::PrepareTexture( int rtIndex ) {
    pRTTextures[ rtIndex ] =
        dxRenderDeviceRender::Instance().Resources->_CreateTexture(
            m_pEngineTextureNames[ rtIndex ] );
}

void dx103DFluidManager::CreateRTTextureAndViews( int rtIndex,
    ...
    CHK_DX( HW.pDevice->CreateRenderTargetView(
        pRT, &DescRT, &pRenderTargetViews[ rtIndex ] ) );
...
void dx103DFluidManager::Reset() {
    ...
    for ( int rtIndex = 0; rtIndex < NUM_OWN_RENDER_TARGETS; rtIndex++ ) {
        HW.pContext->ClearRenderTargetView( pRenderTargetViews[ rtIndex ],
                                            color );
    }
}
...
void dx103DFluidManager::Update( dx103DFluidData& FluidData, float timestep ) {
    ...
    const bool bSimulateFire =
        ( VolumeSettings.m_SimulationType == dx103DFluidData::ST_FIRE );

    AttachFluidData( FluidData );

    // All drawing will take place to a viewport with the dimensions of a 3D
    // texture slice
    ...
    // Update the obstacle velocity based on its movement
    {
            ...
            obstVelocity.x *= grid->dim[0]; obstVelocity.y *= grid->dim[1];
    obstVelocity.z *= grid->dim[2];
    ...
    */
    ...
    if ( !RImplementation.o.dx10_msaa )
        ...
        pTarget->u_setrt( pTarget->rt_Generic_0_r, 0, 0,
...
void dx103DFluidManager::AttachFluidData( dx103DFluidData& FluidData ) {
    ...
    for ( int i = 0; i < dx103DFluidData::VP_NUM_TARGETS; ++i ) {
        ID3DTexture3D* pT =
            FluidData.GetTexture( ( dx103DFluidData::eVolumePrivateRT )i );
        pRTTextures[ RENDER_TARGET_VELOCITY0 + i ]->surface_set( pT );
        _RELEASE( pT );

        VERIFY( !pRenderTargetViews[ RENDER_TARGET_VELOCITY0 + i ] );
        pRenderTargetViews[ RENDER_TARGET_VELOCITY0 + i ] =
            FluidData.GetView( ( dx103DFluidData::eVolumePrivateRT )i );
    }
}
...
void dx103DFluidManager::DetachAndSwapFluidData( dx103DFluidData& FluidData ) {
    ...
    ID3DTexture3D* pTTarg =
        ( ID3DTexture3D* )pRTTextures[ RENDER_TARGET_COLOR ]->surface_get();
    ...
    FluidData.SetView( dx103DFluidData::VP_COLOR,
                       pRenderTargetViews[ RENDER_TARGET_COLOR ] );
...
void dx103DFluidManager::AdvectColorBFECC( float timestep, bool bTeperature ) {
    ...
    HW.pContext->ClearRenderTargetView(
        pRenderTargetViews[ RENDER_TARGET_TEMPVECTOR ], color );
    HW.pContext->ClearRenderTargetView(
        pRenderTargetViews[ RENDER_TARGET_TEMPSCALAR ], color );
    ...
    // D3DXVECTOR3 halfVol( grid->dim[0]/2.0f, grid->dim[1]/2.0f,
    // grid->dim[2]/2.0f ); HalfVolumeDimShaderVariable->SetFloatVector(
    // (float*)&halfVol);
    Fvector4 halfVol;
    halfVol.set( ( float )m_iTextureWidth / 2.0f,
                 ( float )m_iTextureHeight / 2.0f,
                 ( float )m_iTextureDepth / 2.0f, 0.0f );
    ...
    // m_pD3DDevice->OMSetRenderTargets(0, NULL, NULL);
    // pShaderResourceVariables[RENDER_TARGET_TEMPSCALAR]->SetResource( NULL );
    //  Apply the technique again so that the RENDER_TARGET_TEMPSCALAR shader
    //  resource is unbound
    // TechniqueAdvectBFECC->GetPassByIndex(0)->Apply(0);*/
}
...
void dx103DFluidManager::AdvectVelocity( float timestep, float fGravity ) {
    ...
    m_pGrid->DrawSlices();
    // m_pD3DDevice->OMSetRenderTargets(0, NULL, NULL);
    // pShaderResourceVariables[RENDER_TARGET_VELOCITY1]->SetResource(
    // pRenderTargetShaderViews[RENDER_TARGET_VELOCITY1] );
}
...
void dx103DFluidManager::ComputePressure( float timestep ) {
    ...
    // ID3DxxTexture3D	*pTemp = (ID3DxxTexture3D*)
    // pRTTextures[RENDER_TARGET_TEMPSCALAR]->surface_get(); ID3DxxTexture3D
    // *pPressure = (ID3DxxTexture3D*)
    // pRTTextures[RENDER_TARGET_PRESSURE]->surface_get();
    ...
    static shared_str strPressureName(
        m_pEngineTextureNames[ RENDER_TARGET_PRESSURE ] );
...
void dx103DFluidManager::ProjectVelocity( float timestep ) {
    ...
    m_pGrid->DrawSlices();
    // m_pD3DDevice->OMSetRenderTargets(0, NULL, NULL);
    // pShaderResourceVariables[RENDER_TARGET_VELOCITY0]->SetResource(
    // pRenderTargetShaderViews[RENDER_TARGET_VELOCITY0] );
}
...
void dx103DFluidManager::UpdateObstacles( const dx103DFluidData& FluidData,
                                          float timestep ) {
    PIX_EVENT( Fluid_update_obstacles );
    //	Reset data
    float color[ 4 ] = { 0, 0, 0, 0 };
    HW.pContext->ClearRenderTargetView(
        pRenderTargetViews[ RENDER_TARGET_OBSTACLES ], color );
    HW.pContext->ClearRenderTargetView(
        pRenderTargetViews[ RENDER_TARGET_OBSTVELOCITY ], color );
...
#ifdef DEBUG
void dx103DFluidManager::RegisterFluidData( dx103DFluidData* pData,
                                            const xr_string& SectionName ) {
