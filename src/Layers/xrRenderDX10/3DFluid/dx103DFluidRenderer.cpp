...
shared_str strZFar;
// pGridScaleFactorVar = pEffect->GetVariableByName(
// "gridScaleFactor")->AsScalar();
shared_str strGridScaleFactor;
// pEyeOnGridVar = pEffect->GetVariableByName("eyeOnGrid")->AsVector();
shared_str strEyeOnGrid;
// pWorldViewProjectionVar =
// pEffect->GetVariableByName("WorldViewProjection")->AsMatrix();
shared_str strWorldViewProjection;
// pInvWorldViewProjectionVar =
// pEffect->GetVariableByName("InvWorldViewProjection")->AsMatrix();
...
LPCSTR dx103DFluidRenderer::m_pRTNames[ RRT_NumRT ] = {
    "$user$rayDataTex", "$user$rayDataTexSmall", "$user$rayCastTex",
    "$user$edgeTex" };

LPCSTR dx103DFluidRenderer::m_pResourceRTNames[ RRT_NumRT ] = {
    "rayDataTex", "rayDataTexSmall", "rayCastTex", "edgeTex" };
...
void dx103DFluidRenderer::Initialize( int gridWidth,
                                      int gridHeight,
                                      int gridDepth ) {
    Destroy();
    ...
    m_fMaxDim =
        _max( _max( m_vGridDim[ 0 ], m_vGridDim[ 1 ] ), m_vGridDim[ 2 ] );

    // Initialize the grid offset matrix
    {
        // Make a scale matrix to scale the unit-sided box to be unit-length on
        // the
        //  side/s with maximum dimension
        D3DXMATRIX scaleM;
        D3DXMatrixIdentity( &scaleM );
        D3DXMatrixScaling( &scaleM, m_vGridDim[ 0 ] / m_fMaxDim,
                           m_vGridDim[ 1 ] / m_fMaxDim,
                           m_vGridDim[ 2 ] / m_fMaxDim );
        ...
        // m_gridMatrix.scale(m_vGridDim[0] / m_fMaxDim, m_vGridDim[1] /
        // m_fMaxDim, m_vGridDim[2] / m_fMaxDim);
...
void dx103DFluidRenderer::CreateGridBox() {
    VsInput vertices[] = {
        ...
        { D3DXVECTOR3( 0, 1, 0 ) }, { D3DXVECTOR3( 0, 1, 1 ) },
        { D3DXVECTOR3( 1, 0, 0 ) }, { D3DXVECTOR3( 1, 0, 1 ) },
        { D3DXVECTOR3( 1, 1, 0 ) }, { D3DXVECTOR3( 1, 1, 1 ) },
    };
    m_iGridBoxVertNum = sizeof( vertices ) / sizeof( vertices[ 0 ] );

    CHK_DX( dx10BufferUtils::CreateVertexBuffer(
        &m_pGridBoxVertexBuffer, vertices, sizeof( vertices ) ) );
    ...
    CHK_DX( dx10BufferUtils::CreateIndexBuffer( &m_pGridBoxIndexBuffer, indices,
                                                sizeof( indices ) ) );
    HW.stats_manager.increment_stats(
        sizeof( indices ), enum_stats_buffer_type_index, D3DPOOL_MANAGED );
    ...
    m_GeomGridBox.create( layout, m_pGridBoxVertexBuffer,
                          m_pGridBoxIndexBuffer );
}
...
void dx103DFluidRenderer::CreateScreenQuad() {
    ...
    CHK_DX( dx10BufferUtils::CreateVertexBuffer( &m_pQuadVertexBuffer, svQuad,
                                                 sizeof( svQuad ) ) );
...
float bsW2( float a ) {
    return (
        1.0f / 6.0f *
        ( -( 3.0f * a * a * a ) + ( 3.0f * a * a ) + ( 3.0f * a ) + 1.0f ) );
}
...
void dx103DFluidRenderer::CreateHHGGTexture() {
    ...
    m_HHGGTexture = dxRenderDeviceRender::Instance().Resources->_CreateTexture(
        "$user$NVHHGGTex" );
...
void dx103DFluidRenderer::CalculateRenderTextureSize( int screenWidth,
                                                      int screenHeight ) {
    ...
    if ( maxScreenDim > maxProjectedSide ) {
        if ( screenHeight > screenWidth ) {
            ...
            m_iRenderTextureWidth =
                ( int )( screenAspectRatio * maxProjectedSide );
        } else {
            m_iRenderTextureWidth = maxProjectedSide;
            m_iRenderTextureHeight =
                ( int )( ( 1.0f / screenAspectRatio ) * maxProjectedSide );
        }
    } else {
...
void dx103DFluidRenderer::CreateRayDataResources( int width, int height ) {
    ...
    for ( int i = 1; i < RRT_NumRT; ++i ) {
        RT[ i ] = nullptr;
        RT[ i ].create( m_pRTNames[ i ], m_iRenderTextureWidth,
                        m_iRenderTextureHeight, RTFormats[ i ] );
    }
}
...
void dx103DFluidRenderer::Draw( const dx103DFluidData& FluidData ) {
    ...
    const bool bRenderFire =
        ( VolumeSettings.m_SimulationType == dx103DFluidData::ST_FIRE );
    ...
    pTarget->u_setrt( RT[ RRT_RayCastTex ], nullptr, nullptr,
                      nullptr ); // LDR RT
    ...
    // Render to the back buffer sampling from the raycast texture that we just
    // created
    //  If and edge was detected at the current pixel we will raycast again to
    //  avoid smoke aliasing artifacts at scene edges

    //	Restore render state
    if ( !RImplementation.o.dx10_msaa )
        pTarget->u_setrt( pTarget->rt_Generic_0, nullptr, nullptr,
                          HW.pBaseZB ); // LDR RT
    else
        pTarget->u_setrt( pTarget->rt_Generic_0_r, nullptr, nullptr,
                          pTarget->rt_MSAADepth->pZRT ); // LDR RT
    ...
    RCache.set_c( strDiffuseLight, LightData.m_vLightIntencity.x,
                  LightData.m_vLightIntencity.y, LightData.m_vLightIntencity.z,
                  1.0f );
...
void dx103DFluidRenderer::ComputeRayData( const dx103DFluidData& FluidData ) {
    ...
    CRenderTarget* pTarget = RImplementation.Target;
    pTarget->u_setrt( RT[ RRT_RayDataTex ], nullptr, nullptr,
                      nullptr ); // LDR RT
    RCache.set_Element( m_RendererTechnique[ RS_CompRayData_Back ] );
    ...
    //  unless the pixel is occluded by the scene, in which case we output
    //  xyzw=(1,0,0,0)
...
void dx103DFluidRenderer::ComputeEdgeTexture(
    ...
    CRenderTarget* pTarget = RImplementation.Target;
    pTarget->u_setrt( RT[ RRT_RayDataTexSmall ], nullptr, nullptr,
                      nullptr ); // LDR RT
    RCache.set_Element(
        m_RendererTechnique[ RS_QuadDownSampleRayDataTexture ] );
    ...
    // Downsample the rayDataTexture to a new small texture, simply using point
    // sample (no filtering)
...
void dx103DFluidRenderer::DrawBox() {
    ...
    RCache.Render( D3DPT_TRIANGLELIST, 0, 0, m_iGridBoxVertNum, 0,
                   m_iGridBoxFaceNum );
}

void dx103DFluidRenderer::CalculateLighting( const dx103DFluidData& FluidData,
                                             FogLighting& LightData ) {
    ...
    Fvector4 hemi_color =
        g_pGamePersistent->Environment().CurrentEnv->hemi_color;
    // hemi_color.mul(0.2f);
    hemi_color.mul( VolumeSettings.m_fHemi );
    LightData.m_vLightIntencity.set( hemi_color.x, hemi_color.y, hemi_color.z );
    LightData.m_vLightIntencity.add(
        g_pGamePersistent->Environment().CurrentEnv->ambient );
...
void dx103DFluidRenderer::PrepareCBuffer( const dx103DFluidData& FluidData,
                                          u32 RTWidth,
                                          u32 RTHeight,
                                          bool SizeOnly ) {
    ...
    // The length of one of the axis of the worldView matrix is the length of
    // longest side of the box
    //  in view space. This is used to convert the length of a ray from view
    //  space to grid space.
    D3DXVECTOR3 worldXaxis =
        D3DXVECTOR3( WorldView._11, WorldView._12, WorldView._13 );
    float worldScale = D3DXVec3Length( &worldXaxis );
    RCache.set_c( strGridScaleFactor, worldScale );

    // We prepend the current world matrix with this other matrix which adds an
    // offset (-0.5, -0.5, -0.5)
    //  and scale factors to account for unequal number of voxels on different
    //  sides of the volume box.
    // This is because we want to preserve the aspect ratio of the original
    // simulation grid when
    ...
    // invWorldViewProjection is used to transform positions in the "near" plane
    // into grid space
    D3DXMATRIX InvWorldViewProjection;
    D3DXMatrixInverse( ( D3DXMATRIX* )&InvWorldViewProjection, nullptr,
                       ( D3DXMATRIX* )&WorldViewProjection );
    RCache.set_c( strInvWorldViewProjection,
                  *( Fmatrix* )&InvWorldViewProjection );

    // Compute the inverse of the worldView matrix
    D3DXMATRIX WorldViewInv;
    D3DXMatrixInverse( ( D3DXMATRIX* )&WorldViewInv, nullptr,
                       ( D3DXMATRIX* )&WorldView );
    // Compute the eye's position in "grid space" (the 0-1 texture coordinate
    // cube)
    D3DXVECTOR4 EyeInGridSpace;
    D3DXVECTOR3 Origin( 0, 0, 0 );
    D3DXVec3Transform( ( D3DXVECTOR4* )&EyeInGridSpace, ( D3DXVECTOR3* )&Origin,
                       ( D3DXMATRIX* )&WorldViewInv );
