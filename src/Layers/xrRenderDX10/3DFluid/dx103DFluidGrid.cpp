...
namespace {
//	namespace start
inline void ComputeRowColsForFlat3DTexture( int depth,
                                            int* outCols,
                                            int* outRows ) {
    // Compute # of m_iRows and m_iCols for a "flat 3D-texture" configuration
    // (in this configuration all the slices in the volume are spread in a
    // single 2D texture)
...
void dx103DFluidGrid::Initialize( int gridWidth,
                                  int gridHeight,
                                  int gridDepth ) {
    m_vDim[ 0 ] = gridWidth;
    m_vDim[ 1 ] = gridHeight;
...
void dx103DFluidGrid::CreateVertexBuffers() {
    ...
    //D3Dxx_INPUT_PER_VERTEX_DATA, 0 }, 	{ "TEXCOORD", 0,
    //DXGI_FORMAT_R32G32B32_FLOAT,       0,12, D3Dxx_INPUT_PER_VERTEX_DATA, 0 },
    //};
    ...
    renderQuad =
        xr_alloc< VS_INPUT_FLUIDSIM_STRUCT >( m_iNumVerticesRenderQuad );
    ...
    boundarySlices =
        xr_alloc< VS_INPUT_FLUIDSIM_STRUCT >( m_iNumVerticesBoundarySlices );

    m_iNumVerticesBoundaryLines =
        VERTICES_PER_LINE * LINES_PER_SLICE * ( m_vDim[ 2 ] );
    boundaryLines =
        xr_alloc< VS_INPUT_FLUIDSIM_STRUCT >( m_iNumVerticesBoundaryLines );

    VERIFY( renderQuad && m_iNumVerticesSlices &&
            m_iNumVerticesBoundarySlices && m_iNumVerticesBoundaryLines );

    // Vertex buffer for "m_vDim[2]" quads to draw all the slices of the
    // 3D-texture as a flat 3D-texture (used to draw all the individual slices
    // at once to the screen buffer)
    index = 0;
    ...
    //	D3Dxx_BIND_VERTEX_BUFFER, &renderQuadBuffer, renderQuad,
    //numVerticesRenderQuad));
    CHK_DX( dx10BufferUtils::CreateVertexBuffer(
        &m_pRenderQuadBuffer, renderQuad, vSize * m_iNumVerticesRenderQuad ) );
    m_GeomRenderQuad.create( layoutDesc, m_pRenderQuadBuffer, 0 );

    // Vertex buffer for "m_vDim[2]" quads to draw all the slices to a 3D
    // texture (a Geometry Shader is used to send each quad to the appropriate
    // slice)
    index = 0;
    ...
    CHK_DX( dx10BufferUtils::CreateVertexBuffer(
        &m_pSlicesBuffer, slices, vSize * m_iNumVerticesSlices ) );
    ...
    //	D3Dxx_BIND_VERTEX_BUFFER, &boundarySlicesBuffer, boundarySlices,
    //numVerticesBoundarySlices));
    CHK_DX( dx10BufferUtils::CreateVertexBuffer(
        &m_pBoundarySlicesBuffer, boundarySlices,
        vSize * m_iNumVerticesBoundarySlices ) );
    ...
    //	D3Dxx_BIND_VERTEX_BUFFER, &boundaryLinesBuffer, boundaryLines,
    //numVerticesBoundaryLines));
    CHK_DX( dx10BufferUtils::CreateVertexBuffer(
        &m_pBoundaryLinesBuffer, boundaryLines,
        vSize * m_iNumVerticesBoundaryLines ) );
...
void dx103DFluidGrid::InitScreenSlice( VS_INPUT_FLUIDSIM_STRUCT** vertices,
                                       int z,
                                       int& index ) {
    ...
    // compute the offset (px, py) in the "flat 3D-texture" space for the slice
    // with given 'z' coordinate
    ...
    tempVertex1.Pos = D3DXVECTOR3( px * 2.0f / Width - 1.0f,
                                   -( py * 2.0f / Height ) + 1.0f, 0.0f );
    tempVertex1.Tex = D3DXVECTOR3( 0, 0, float( z ) );

    tempVertex2.Pos = D3DXVECTOR3( ( px + w ) * 2.0f / Width - 1.0f,
                                   -( ( py ) * 2.0f / Height ) + 1.0f, 0.0f );
    tempVertex2.Tex = D3DXVECTOR3( w, 0, float( z ) );

    tempVertex3.Pos =
        D3DXVECTOR3( ( px + w ) * 2.0f / Width - 1.0f,
                     -( ( py + h ) * 2.0f / Height ) + 1.0f, 0.0f );
    tempVertex3.Tex = D3DXVECTOR3( w, h, float( z ) );

    tempVertex4.Pos =
        D3DXVECTOR3( ( px ) * 2.0f / Width - 1.0f,
                     -( ( py + h ) * 2.0f / Height ) + 1.0f, 0.0f );
...
void dx103DFluidGrid::InitSlice( int z,
    ...
    tempVertex1.Pos =
        D3DXVECTOR3( 1 * 2.0f / w - 1.0f, -1 * 2.0f / h + 1.0f, 0.0f );
    tempVertex1.Tex = D3DXVECTOR3( 1.0f, 1.0f, float( z ) );

    tempVertex2.Pos = D3DXVECTOR3( ( w - 1.0f ) * 2.0f / w - 1.0f,
                                   -1 * 2.0f / h + 1.0f, 0.0f );
    tempVertex2.Tex = D3DXVECTOR3( ( w - 1.0f ), 1.0f, float( z ) );

    tempVertex3.Pos = D3DXVECTOR3( ( w - 1.0f ) * 2.0f / w - 1.0f,
                                   -( h - 1 ) * 2.0f / h + 1.0f, 0.0f );
    tempVertex3.Tex = D3DXVECTOR3( ( w - 1.0f ), ( h - 1.0f ), float( z ) );

    tempVertex4.Pos = D3DXVECTOR3( 1 * 2.0f / w - 1.0f,
                                   -( h - 1.0f ) * 2.0f / h + 1.0f, 0.0f );
...
void dx103DFluidGrid::InitLine( float x1,
                                float y1,
                                float x2,
                                float y2,
                                int z,
    ...
    tempVertex.Pos =
        D3DXVECTOR3( x1 * 2.0f / w - 1.0f, -y1 * 2.0f / h + 1.0f, 0.5f );
    tempVertex.Tex = D3DXVECTOR3( 0.0f, 0.0f, float( z ) );
    ( *vertices )[ index++ ] = tempVertex;

    tempVertex.Pos =
        D3DXVECTOR3( x2 * 2.0f / w - 1.0f, -y2 * 2.0f / h + 1.0f, 0.5f );
    tempVertex.Tex = D3DXVECTOR3( 0.0f, 0.0f, float( z ) );
    ( *vertices )[ index++ ] = tempVertex;
}

void dx103DFluidGrid::InitBoundaryQuads( VS_INPUT_FLUIDSIM_STRUCT** vertices,
                                         int& index ) {
    InitSlice( 0, vertices, index );
    InitSlice( m_vDim[ 2 ] - 1, vertices, index );
}

void dx103DFluidGrid::InitBoundaryLines( VS_INPUT_FLUIDSIM_STRUCT** vertices,
                                         int& index ) {
    int w = m_vDim[ 0 ];
    int h = m_vDim[ 1 ];
    ...
    for ( int z = 0; z < m_vDim[ 2 ]; z++ ) {
        ...
        // top
        InitLine( 0.0f, float( h ), float( w ), float( h ), z, vertices,
                  index );
        // left
        InitLine( 1.0f, 0.0f, 1.0f, float( h ), z, vertices, index );
        // right
        InitLine( float( w ), 0.0f, float( w ), float( h ), z, vertices,
                  index );
    }
}
...
void dx103DFluidGrid::DrawSlices( void ) {
    ...
    // UINT offset[1] = { 0 };
    // DrawPrimitive( D3Dxx_PRIMITIVE_TOPOLOGY_TRIANGLELIST, layout,
    // &slicesBuffer, 	stride, offset, 0, numVerticesSlices );
...
void dx103DFluidGrid::DrawSlicesToScreen( void ) {
    ...
    // UINT offset[1] = { 0 };
    // DrawPrimitive( D3Dxx_PRIMITIVE_TOPOLOGY_TRIANGLELIST, layout,
    // &renderQuadBuffer, 	stride, offset, 0, numVerticesRenderQuad );
...
void dx103DFluidGrid::DrawBoundaryQuads( void ) {
    ...
    // UINT offset[1] = { 0 };
    // DrawPrimitive( D3Dxx_PRIMITIVE_TOPOLOGY_TRIANGLELIST, layout,
    // &boundarySlicesBuffer, 	stride, offset, 0, numVerticesBoundarySlices );
...
void dx103DFluidGrid::DrawBoundaryLines( void ) {
    ...
    //&boundaryLinesBuffer, 		stride, offset, 0, numVerticesBoundaryLines  );
