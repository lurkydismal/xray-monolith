...
namespace {
const xr_token simulation_type_token[] = { { "Fog", dx103DFluidData::ST_FOG },
...
void dx103DFluidData::CreateRTTextureAndViews( int rtIndex,
                                               D3D_TEXTURE3D_DESC TexDesc ) {
    // Create the texture
    CHK_DX( HW.pDevice->CreateTexture3D( &TexDesc, NULL,
                                         &m_pRTTextures[ rtIndex ] ) );
    ...
    CHK_DX( HW.pDevice->CreateRenderTargetView(
        m_pRTTextures[ rtIndex ], &DescRT, &m_pRenderTargetViews[ rtIndex ] ) );

    float color[ 4 ] = { 0, 0, 0, 0 };

    HW.pContext->ClearRenderTargetView( m_pRenderTargetViews[ rtIndex ],
                                        color );
}
...
void dx103DFluidData::ParseProfile( const xr_string& Profile,
    ...
    Fmatrix WorldToFluid;
    {
        ...
        // bit
        Fvector vGridDim;
        vGridDim.set( ( float )FluidManager.GetTextureWidth(),
                      ( float )FluidManager.GetTextureHeight(),
    ...
    if ( ini.line_exist( "volume", "Type" ) )
        prepared.settings.m_SimulationType = ( SimulationType )ini.r_token(
            "volume", "Type", simulation_type_token );

    if ( ini.line_exist( "volume", "Hemi" ) )
        prepared.settings.m_fHemi = ini.r_float( "volume", "Hemi" );

    if ( ini.line_exist( "volume", "ConfinementScale" ) )
        prepared.settings.m_fConfinementScale =
            ini.r_float( "volume", "ConfinementScale" );
    ...
    if ( ini.line_exist( "volume", "GravityBuoyancy" ) )
        prepared.settings.m_fGravityBuoyancy =
            ini.r_float( "volume", "GravityBuoyancy" );
    ...
    for ( u32 i = 0; i < iEmittersNum; ++i ) {
        ...
        Emitter.m_eType = ( dx103DFluidEmitters::EmitterType )ini.r_token(
            EmitterSectionName, "Type", emitter_type_token );

        if ( ini.line_exist( EmitterSectionName, "Position" ) )
            Emitter.m_vPosition =
                ini.r_fvector3( EmitterSectionName, "Position" );
        else {
            Emitter.m_vPosition =
                ini.r_fvector3( EmitterSectionName, "WorldPosition" );
        ...
        Emitter.m_vFlowVelocity =
            ini.r_fvector3( EmitterSectionName, "FlowDirection" );
        ...
        Emitter.m_bApplyDensity =
            ini.r_bool( EmitterSectionName, "ApplyDensity" ) ? true : false;
        Emitter.m_bApplyImpulse =
            ini.r_bool( EmitterSectionName, "ApplyImpulse" ) ? true : false;

        switch ( Emitter.m_eType ) {
            case dx103DFluidEmitters::ET_SimpleDraught:
                Emitter.m_DraughtParams.m_fPeriod =
                    ini.r_float( EmitterSectionName, "DraughtPeriod" );
                Emitter.m_DraughtParams.m_fPhase =
                    ini.r_float( EmitterSectionName, "DraughtPhase" );
                Emitter.m_DraughtParams.m_fAmp =
                    ini.r_float( EmitterSectionName, "DraughtAmp" );
