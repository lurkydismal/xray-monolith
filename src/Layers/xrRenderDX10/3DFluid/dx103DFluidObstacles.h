...
public:
    dx103DFluidObstacles( int gridWidth,
                          int gridHeight,
                          int gridDepth,
...
private:
    enum ObstacleShader { OS_OOBB = 0, OS_DynamicOOBB, OS_NumShaders };

private:
    void InitShaders();
    void DestroyShaders();

    void ProcessStaticObstacles( const dx103DFluidData& FluidData,
                                 const Fmatrix& WorldToFluid );
    void ProcessDynamicObstacles( const dx103DFluidData& FluidData,
                                  const Fmatrix& WorldToFluid,
                                  float timestep );

    //	This technique renders several objects.
    void RenderPhysicsShell( const IPhysicsShell* pShell,
                             const Fmatrix& WorldToFluid,
                             float timestep );
    void RenderPhysicsElement( const IPhysicsElement& Element,
                               const Fmatrix& WorldToFluid,
                               float timestep );

    void RenderStaticOOBB( const Fmatrix& Transform );
    // void	RenderDynamicOOBB( const IPhysicsElement &Element, const Fmatrix
    // &WorldToFluid, float timestep);
    void RenderDynamicOOBB( const IPhysicsGeometry& Geometry,
                            const Fmatrix& WorldToFluid,
                            float timestep );
