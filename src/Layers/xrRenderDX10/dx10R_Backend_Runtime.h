...
ICF void CBackend::set_Vertices( ID3DVertexBuffer* _vb, u32 _vb_stride ) {
    if ( ( vb != _vb ) || ( vb_stride != _vb_stride ) ) {
        ...
        // CHK_DX
        // (HW.pDevice->SetStreamSource(0,vb,0,vb_stride)); UINT StreamNumber,
...
#ifdef USE_DX11
IC void CBackend::Compute( UINT ThreadGroupCountX,
    ...
    constants.flush();
    HW.pContext->Dispatch( ThreadGroupCountX, ThreadGroupCountY,
                           ThreadGroupCountZ );
}
#endif

IC void CBackend::Render( D3DPRIMITIVETYPE T,
                          u32 baseV,
                          u32 startV,
                          u32 countV,
                          u32 startI,
    ...
    // CHK_DX(HW.pDevice->DrawIndexedPrimitive(T,baseV, startV,
    // countV,startI,PC)); D3DPRIMITIVETYPE Type, INT BaseVertexIndex, UINT
    // MinIndex, UINT NumVertices, UINT StartIndex, UINT PriResmitiveCount
...
IC void CBackend::Render( D3DPRIMITIVETYPE T, u32 startV, u32 PC ) {
    ...
    // CHK_DX				(HW.pDevice->DrawPrimitive(T, startV,
    // PC));
...
IC void CBackend::set_Scissor( Irect* R ) {
    if ( R ) {
        // CHK_DX
        // (HW.pDevice->SetRenderState(D3DRS_SCISSORTESTENABLE,TRUE));
        StateManager.EnableScissoring();
        RECT* clip = ( RECT* )R;
        HW.pContext->RSSetScissorRects( 1, clip );
    } else {
        // CHK_DX
        // (HW.pDevice->SetRenderState(D3DRS_SCISSORTESTENABLE,FALSE));
        StateManager.EnableScissoring( FALSE );
        HW.pContext->RSSetScissorRects( 0, 0 );
    }
}

IC void CBackend::set_Stencil( u32 _enable,
                               u32 _func,
                               u32 _ref,
                               u32 _mask,
                               u32 _writemask,
                               ...
                               u32 _pass,
                               u32 _zfail ) {
    StateManager.SetStencil( _enable, _func, _ref, _mask, _writemask, _fail,
                             _pass, _zfail );
    // Simple filter
    // if (stencil_enable		!= _enable)		{
    // stencil_enable=_enable;		CHK_DX(HW.pDevice->SetRenderState
    // ( D3DRS_STENCILENABLE,		_enable				)); } if
    // (!stencil_enable)					return; if
    // (stencil_func		!= _func)		{ stencil_func=_func;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILFUNC,
    // _func				)); } if (stencil_ref
    // != _ref)		{ stencil_ref=_ref;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILREF,
    // _ref				)); } if (stencil_mask		!=
    // _mask)		{ stencil_mask=_mask;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILMASK,
    // _mask				)); } if (stencil_writemask	!=
    // _writemask)	{ stencil_writemask=_writemask;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILWRITEMASK,
    // _writemask			)); } if (stencil_fail		!=
    // _fail)		{ stencil_fail=_fail;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILFAIL,
    // _fail				)); } if (stencil_pass		!=
    // _pass)		{ stencil_pass=_pass;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILPASS,
    // _pass				)); } if (stencil_zfail		!=
    // _zfail)		{ stencil_zfail=_zfail;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILZFAIL,
    // _zfail				)); }
}
...
IC void CBackend::set_ColorWriteEnable( u32 _mask ) {
    ...
    // D3DRS_COLORWRITEENABLE2,	_mask	)); CHK_DX(HW.pDevice->SetRenderState ( D3DRS_COLORWRITEENABLE3, _mask	));
    // }
}
...
ICF void CBackend::set_CullMode( u32 _mode ) {
    ...
    // if (cull_mode		!= _mode)		{ cull_mode = _mode;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_CULLMODE,
    // _mode				)); }
}
...
IC void CBackend::ApplyVertexLayout() {
    ...
    if ( it == decl->vs_to_layout.end() ) {
        ...
        CHK_DX( HW.pDevice->CreateInputLayout(
            &decl->dx10_dcl_code[ 0 ], decl->dx10_dcl_code.size() - 1,
            m_pInputSignature->GetBufferPointer(),
            m_pInputSignature->GetBufferSize(), &pLayout ) );

        it = decl->vs_to_layout
                 .insert( std::pair< ID3DBlob*, ID3DInputLayout* >(
                     m_pInputSignature, pLayout ) )
                 .first;
    }
...
IC bool CBackend::CBuffersNeedUpdate( ref_cbuffer buf1[ MaxCBuffers ],
                                      ref_cbuffer buf2[ MaxCBuffers ],
                                      u32& uiMin,
                                      u32& uiMax ) {
    bool bRes = false;
    int i = 0;
...
IC void CBackend::set_Constants( R_constant_table* C ) {
    ...
    //	Setup constant tables
    {
        ...
        for ( ; it != end; ++it ) {
            ...
            if ( ( uiBufferIndex & CB_BufferTypeMask ) ==
                 CB_BufferPixelShader ) {
                VERIFY( ( uiBufferIndex & CB_BufferIndexMask ) < MaxCBuffers );
                m_aPixelConstants[ uiBufferIndex & CB_BufferIndexMask ] =
                    it->second;
            } else if ( ( uiBufferIndex & CB_BufferTypeMask ) ==
                        CB_BufferVertexShader ) {
                VERIFY( ( uiBufferIndex & CB_BufferIndexMask ) < MaxCBuffers );
                m_aVertexConstants[ uiBufferIndex & CB_BufferIndexMask ] =
                    it->second;
            } else if ( ( uiBufferIndex & CB_BufferTypeMask ) ==
                        CB_BufferGeometryShader ) {
                VERIFY( ( uiBufferIndex & CB_BufferIndexMask ) < MaxCBuffers );
                m_aGeometryConstants[ uiBufferIndex & CB_BufferIndexMask ] =
                    it->second;
            }
#ifdef USE_DX11
            else if ( ( uiBufferIndex & CB_BufferTypeMask ) ==
                      CB_BufferHullShader ) {
                VERIFY( ( uiBufferIndex & CB_BufferIndexMask ) < MaxCBuffers );
                m_aHullConstants[ uiBufferIndex & CB_BufferIndexMask ] =
                    it->second;
            } else if ( ( uiBufferIndex & CB_BufferTypeMask ) ==
                        CB_BufferDomainShader ) {
                VERIFY( ( uiBufferIndex & CB_BufferIndexMask ) < MaxCBuffers );
                m_aDomainConstants[ uiBufferIndex & CB_BufferIndexMask ] =
                    it->second;
            } else if ( ( uiBufferIndex & CB_BufferTypeMask ) ==
                        CB_BufferComputeShader ) {
                VERIFY( ( uiBufferIndex & CB_BufferIndexMask ) < MaxCBuffers );
                m_aComputeConstants[ uiBufferIndex & CB_BufferIndexMask ] =
                    it->second;
            }
#endif
            else
        ...
        if ( CBuffersNeedUpdate( m_aPixelConstants, aPixelConstants, uiMin,
                                 uiMax ) ) {
            ++uiMax;
            ...
            HW.pContext->PSSetConstantBuffers( uiMin, uiMax - uiMin,
                                               &tempBuffer[ uiMin ] );
        }

        if ( CBuffersNeedUpdate( m_aVertexConstants, aVertexConstants, uiMin,
                                 uiMax ) ) {
            ++uiMax;
            ...
            HW.pContext->VSSetConstantBuffers( uiMin, uiMax - uiMin,
                                               &tempBuffer[ uiMin ] );
        }

        if ( CBuffersNeedUpdate( m_aGeometryConstants, aGeometryConstants,
                                 uiMin, uiMax ) ) {
            ++uiMax;
            ...
            HW.pContext->GSSetConstantBuffers( uiMin, uiMax - uiMin,
                                               &tempBuffer[ uiMin ] );
        }

#ifdef USE_DX11
        if ( CBuffersNeedUpdate( m_aHullConstants, aHullConstants, uiMin,
                                 uiMax ) ) {
            ++uiMax;
            ...
            HW.pContext->HSSetConstantBuffers( uiMin, uiMax - uiMin,
                                               &tempBuffer[ uiMin ] );
        }

        if ( CBuffersNeedUpdate( m_aDomainConstants, aDomainConstants, uiMin,
                                 uiMax ) ) {
            ++uiMax;
            ...
            HW.pContext->DSSetConstantBuffers( uiMin, uiMax - uiMin,
                                               &tempBuffer[ uiMin ] );
        }

        if ( CBuffersNeedUpdate( m_aComputeConstants, aComputeConstants, uiMin,
                                 uiMax ) ) {
            ++uiMax;
            ...
            HW.pContext->CSSetConstantBuffers( uiMin, uiMax - uiMin,
                                               &tempBuffer[ uiMin ] );
        }
#endif
        /*
...
ICF void CBackend::ApplyRTandZB() {
    if ( m_bChangedRTorZB ) {
        ...
        HW.pContext->OMSetRenderTargets( sizeof( pRT ) / sizeof( pRT[ 0 ] ),
                                         pRT, pZB );
    }
}

IC void CBackend::get_ConstantDirect( shared_str& n,
                                      u32 DataSize,
                                      void** pVData,
                                      void** pGData,
                                      void** pPData ) {
