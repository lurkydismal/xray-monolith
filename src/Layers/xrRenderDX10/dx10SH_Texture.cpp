...
// Defined in dx10ResourceManager_Resources.cpp;
// CResourceManager::_CreateTexture dispatches brand-new textures' initial
// Load() through it whenever the device is ready. apply_load() below reuses the
// same queue for evicted textures instead of calling Load() inline.
        ...
        if ( D3D_RESOURCE_DIMENSION_TEXTURE2D == type ) {
            ...
            if ( ( desc.SampleDesc.Count <= 1 ) ||
                ...
                CHK_DX( HW.pDevice->CreateShaderResourceView(
                    pSurface, &ViewDesc, &m_pSRView ) );
            else
                m_pSRView = 0;
        } else
            CHK_DX( HW.pDevice->CreateShaderResourceView( pSurface, NULL,
                                                          &m_pSRView ) );
    }
}
...
void CTexture::PostLoad() {
    if ( pTheora )
        bind = xr_make_delegate( this, &CTexture::apply_theora );
    else if ( pAVI )
        bind = xr_make_delegate( this, &CTexture::apply_avi );
    else if ( !seqDATA.empty() )
        bind = xr_make_delegate( this, &CTexture::apply_seq );
    else if ( gifPlayer )
        bind = xr_make_delegate( this, &CTexture::apply_gif );
    else
        bind = xr_make_delegate( this, &CTexture::apply_normal );
}
...
void CTexture::apply_load( u32 dwStage ) {
    ...
    if ( bind == xr_make_delegate( this, &CTexture::apply_load ) ) {
        // This should not happen - if bind is still apply_load, fall back to
        // apply_normal which will just apply the (potentially unloaded) surface
...
void CTexture::Apply( u32 dwStage ) {
    ...
    //    VERIFY( !((!pSurface)^(!m_pSRView)) );	//	Both present or
    //    both missing
    // else
    //{
...
void CTexture::apply_theora( u32 dwStage ) {
    wait_for_loading();
    if ( pTheora->Update( m_play_time != 0xFFFFFFFF
                              ? m_play_time
...
#endif
        // R_ASSERT			(R.Pitch ==
        // int(pTheora->Width(false)*4));
        R_ASSERT( mapData.RowPitch == int( pTheora->Width( false ) * 4 ) );
        int _pos = 0;
        pTheora->DecompressFrame( ( u32* )mapData.pData, _w - rect.right,
                                  _pos );
...
void CTexture::apply_avi( u32 dwStage ) {
    ...
    if ( pAVI->NeedUpdate() ) {
        ...
        BYTE* ptr;
        pAVI->GetFrame( &ptr );
        CopyMemory( mapData.pData, ptr,
                    pAVI->m_dwWidth * pAVI->m_dwHeight * 4 );
        // R_CHK	(T2D->UnlockRect(0));
#ifdef USE_DX11
        HW.pContext->Unmap( T2D, 0 );
#else
...
void CTexture::apply_gif( u32 dwStage ) {
    ...
    if ( gifPlayer->UpdateFrame() ) {
        const CGIFAnimationPlayer::Frame* const gifFrame =
            gifPlayer->GetActiveFrame();
        R_ASSERT( gifFrame );
...
void CTexture::SetLoadSource( LPCSTR logical_name,
                              LPCSTR resolved_path,
                              ELoadKind kind ) {
...
bool CTexture::TryQueueLoad() {
    ...
    return loadState.compare_exchange_strong( expected, LoadStateQueued,
...
void CTexture::CancelQueuedLoad() {
    ...
    loadState.compare_exchange_strong( expected, LoadStateUnloaded,
                                       std::memory_order_acq_rel,
                                       std::memory_order_acquire );
}
...
void CTexture::wait_for_loading() const {
    for ( ;; ) {
        ...
        if ( state != LoadStateQueued && state != LoadStateLoading &&
             state != LoadStateUnloading )
            return;
...
bool CTexture::BeginLoad( bool queued ) {
    for ( ;; ) {
        ...
        if ( loadState.compare_exchange_strong( expected, LoadStateLoading,
                                                std::memory_order_acq_rel,
                                                std::memory_order_acquire ) ) {
            return true;
        }
        if ( !queued && expected == LoadStateQueued ) {
            ...
            if ( loadState.compare_exchange_strong(
                     expected, LoadStateLoading, std::memory_order_acq_rel,
                     std::memory_order_acquire ) ) {
                return true;
            }
        }

        if ( expected == LoadStateLoaded || expected == LoadStateFailed ||
             ( queued && expected == LoadStateUnloaded ) )
            return false;
...
void CTexture::Load( bool queued ) {
    ...
    const LPCSTR resolvedSource =
        m_resolvedSourcePath.size() ? m_resolvedSourcePath.c_str() : nullptr;

    // Check for OGM
    string_path fn;
    if ( kind == LoadKindOgm ||
         ( kind == LoadKindUnknown &&
           FS.exist( fn, "$game_textures$", name.c_str(), ".ogm" ) ) ) {
        if ( kind == LoadKindOgm )
        ...
        if ( !pTheora->Load( fn ) ) {
            ...
            flags.MemoryUsage =
                pTheora->Width( true ) * pTheora->Height( true ) * 4;
            ...
            if ( FAILED( hrr ) ) {
                ...
                CHK_DX( HW.pDevice->CreateShaderResourceView( pSurface, 0,
                                                              &m_pSRView ) );
            }
        }
    } else if ( kind == LoadKindAvi ||
                ( kind == LoadKindUnknown &&
                  FS.exist( fn, "$game_textures$", name.c_str(), ".avi" ) ) ) {
        if ( kind == LoadKindAvi )
            xr_strcpy( fn, resolvedSource );
        // AVI
    ...
    } else if ( kind == LoadKindSequence ||
                ( kind == LoadKindUnknown &&
                  FS.exist( fn, "$game_textures$", name.c_str(), ".seq" ) ) ) {
        ...
        while ( !_fs->eof() ) {
            ...
            if ( buffer[ 0 ] ) {
                ...
                if ( pSurface ) {
                    ...
                    HW.pDevice->CreateShaderResourceView( seqDATA.back(), NULL,
                                                          &m_seqSRView.back() );
        ...
        pSurface = 0;
        FS.r_close( _fs );
    } else if ( kind == LoadKindGif ||
                ( kind == LoadKindUnknown &&
                  FS.exist( fn, "$game_textures$", name.c_str(), ".gif" ) ) ) {
        if ( kind == LoadKindGif )
            xr_strcpy( fn, resolvedSource );
        ...
        // Normal texture
        u32 mem = 0;
        pSurface = ::RImplementation.texture_load(
            name.c_str(), mem, false,
...
void CTexture::Unload() {
    for ( ;; ) {
        ...
        if ( loadState.compare_exchange_strong( state, LoadStateUnloading,
                                                std::memory_order_acq_rel,
                                                std::memory_order_acquire ) ) {
            break;
        }
...
void CTexture::ReleaseLoadedData() {
    ...
    xr_sprintf( msg_buff, sizeof( msg_buff ),
                "* Unloading texture [%s] pSurface RefCount=", cName.c_str() );
#endif // DEBUG

    //.	if (flags.bLoaded)		Msg		("* Unloaded:
    //%s",cName.c_str());
...
void CTexture::video_Play( BOOL looped, u32 _time ) {
    wait_for_loading();
    if ( pTheora )
        pTheora->Play( looped, ( _time != 0xFFFFFFFF )
                                   ? ( m_play_time = _time )
                                   : Device.dwTimeContinual );
}
