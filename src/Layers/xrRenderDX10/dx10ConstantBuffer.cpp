...
dx10ConstantBuffer::dx10ConstantBuffer(
    ...
    m_uiMembersCRC = crc32( &m_MembersList[ 0 ],
                            Desc.Variables * sizeof( m_MembersList[ 0 ] ) );
...
bool dx10ConstantBuffer::Similar( dx10ConstantBuffer& _in ) {
    ...
    if ( memcmp( &m_MembersList[ 0 ], &_in.m_MembersList[ 0 ],
                 m_MembersList.size() * sizeof( m_MembersList[ 0 ] ) ) )
        return false;
...
void dx10ConstantBuffer::Flush() {
    if ( m_bChanged ) {
        ...
        CHK_DX( HW.pContext->Map( m_pBuffer, 0, D3D_MAP_WRITE_DISCARD, 0,
                                  &pSubRes ) );
        pData = pSubRes.pData;
#else
