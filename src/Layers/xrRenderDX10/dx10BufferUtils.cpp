...
HRESULT IC CreateBuffer( ID3DBuffer** ppBuffer,
                         const void* pData,
                         UINT DataSize,
                         bool bImmutable,
                         bool bIndexBuffer ) {
    ...
    desc.BindFlags =
        bIndexBuffer ? D3D_BIND_INDEX_BUFFER : D3D_BIND_VERTEX_BUFFER;
...
HRESULT CreateVertexBuffer( ID3DVertexBuffer** ppBuffer,
                            const void* pData,
                            UINT DataSize,
                            bool bImmutable ) {
...
HRESULT CreateIndexBuffer( ID3DIndexBuffer** ppBuffer,
                           const void* pData,
                           UINT DataSize,
                           bool bImmutable ) {
...
VertexFormatPairs VertexFormatList[] = {
    ...
    // Warning. Explicit RGB component swizzling is nesessary	//	Not
    // available
    { D3DDECLTYPE_UBYTE4, DXGI_FORMAT_R8G8B8A8_UINT },
    // Note: Shader gets UINT values, but if Direct3D 9 style integral floats
    // are needed (0.0f, 1.0f... 255.f), UINT can just be converted to float32
    // in shader.
    { D3DDECLTYPE_SHORT2, DXGI_FORMAT_R16G16_SINT },
    // Note: Shader gets SINT values, but if Direct3D 9 style integral floats
...
DXGI_FORMAT ConvertVertexFormat( D3DDECLTYPE dx9FMT ) {
    int arrayLength =
        sizeof( VertexFormatList ) / sizeof( VertexFormatList[ 0 ] );
    ...
    VERIFY(
        !"ConvertVertexFormat didn't find appropriate dx10 vertex format!" );
...
LPCSTR ConvertSemantic( D3DDECLUSAGE Semantic ) {
    int arrayLength =
        sizeof( VertexSemanticList ) / sizeof( VertexSemanticList[ 0 ] );
    for ( int i = 0; i < arrayLength; ++i ) {
