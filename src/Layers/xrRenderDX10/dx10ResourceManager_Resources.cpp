...
SVS* CResourceManager::_CreateVS( LPCSTR _name ) {
    ...
    if ( I != m_vs.end() )
        ...
        if ( !file ) {
            string1024 tmp;
            xr_sprintf( tmp,
                        "DX10: %s is missing. Replace with stub_default.vs",
                        cname );
            Msg( tmp );
            strconcat( sizeof( cname ), cname, ::Render->getShaderPath(),
                       "stub_default", ".vs" );
...
SPS* CResourceManager::_CreatePS( LPCSTR _name ) {
    ...
    if ( I != m_ps.end() )
        ...
        if ( !file ) {
            ...
            // Memory.mem_compact();
            xr_sprintf( tmp,
                        "DX10: %s is missing. Replace with stub_default.ps",
                        cname );
            Msg( tmp );
            strconcat( sizeof( cname ), cname, ::Render->getShaderPath(),
                       "stub_default", ".ps" );
...
SGS* CResourceManager::_CreateGS( LPCSTR name ) {
    ...
    if ( I != m_gs.end() )
        ...
        // Open file
        string_path cname;
        strconcat( sizeof( cname ), cname, ::Render->getShaderPath(), name,
                   ".gs" );
        FS.update_path( cname, "$game_shaders$", cname );
        ...
        if ( !file ) {
            ...
            // Memory.mem_compact();
            xr_sprintf( tmp,
                        "DX10: %s is missing. Replace with stub_default.gs",
                        cname );
            Msg( tmp );
            strconcat( sizeof( cname ), cname, ::Render->getShaderPath(),
                       "stub_default", ".gs" );
