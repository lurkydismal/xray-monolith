...
template < typename T >
void remove_indexed( xr_unordered_flat_map< u64, xr_vector< T* > >& index,
                     u64 hash,
                     const T* value ) {
...
SVS* CResourceManager::_CreateVS( LPCSTR _name, ref_vs* keep_alive ) {
    ...
    string_path cname;
    strconcat( sizeof( cname ), cname, ::Render->getShaderPath(),
               /*_name*/ shName, ".vs" );
    ...
    if ( !file ) {
        string1024 tmp;
        xr_sprintf( tmp, "DX10: %s is missing. Replace with stub_default.vs",
                    cname );
        Msg( tmp );
        strconcat( sizeof( cname ), cname, ::Render->getShaderPath(),
                   "stub_default", ".vs" );
    ...
    HRESULT const _hr = ::Render->shader_compile(
        name, ( DWORD const* )data, size, c_entry, c_target,
        D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, ( void*& )_vs );

    VERIFY( SUCCEEDED( _hr ) );

    CHECK_OR_EXIT( !FAILED( _hr ),
                   make_string( "Shader compilation failed, check your log "
...
void CResourceManager::_DeleteVS( const SVS* vs ) {
    ...
    if ( I != m_vs.end() ) {
        ...
        for ( iDecl = v_declarations.begin(); iDecl != v_declarations.end();
              ++iDecl ) {
...
SPS* CResourceManager::_CreatePS( LPCSTR _name, ref_ps* keep_alive ) {
    ...
    if ( !file ) {
        ...
        // Memory.mem_compact();
        xr_sprintf( tmp, "DX10: %s is missing. Replace with stub_default.ps",
                    cname );
        Msg( tmp );
        strconcat( sizeof( cname ), cname, ::Render->getShaderPath(),
                   "stub_default", ".ps" );
...
SGS* CResourceManager::_CreateGS( LPCSTR name, ref_gs* keep_alive ) {
    ...
    if ( !file ) {
        string1024 tmp;
        xr_sprintf( tmp, "DX10: %s is missing. Replace with stub_default.gs",
                    cname );
        Msg( tmp );
        strconcat( sizeof( cname ), cname, ::Render->getShaderPath(),
                   "stub_default", ".gs" );
    ...
    HRESULT const _hr = ::Render->shader_compile(
        name, ( DWORD const* )file->pointer(), file->length(), c_entry,
...
SDeclaration* CResourceManager::_CreateDecl( D3DVERTEXELEMENT9* dcl ) {
    ...
    // CHK_DX
    // (HW.pDevice->CreateVertexDeclaration(dcl,&D->dcl));
...
//--------------------------------------------------------------------------------------------------------------
R_constant_table* CResourceManager::_CreateConstantTable(
    R_constant_table& C,
    ref_ctable* keep_alive ) {
    if ( C.empty() )
        return NULL;
...
//--------------------------------------------------------------------------------------------------------------
#ifdef USE_DX11
CRT* CResourceManager::_CreateRT( LPCSTR Name,
                                  u32 w,
                                  u32 h,
                                  D3DFORMAT f,
                                  u32 SampleCount,
...
void CResourceManager::DBG_VerifyGeoms() {
    ...
    u32 vb_stride					= D3DXGetDeclVertexSize
    (test,0); u32 vb_stride_cached			= G->vb_stride; R_ASSERT
    (vb_stride == vb_stride_cached);
    }
    */
}

SGeometry* CResourceManager::CreateGeom( D3DVERTEXELEMENT9* decl,
                                         ID3DVertexBuffer* vb,
                                         ID3DIndexBuffer* ib,
                                         ref_geom* keep_alive ) {
    xrCriticalSectionGuard guard( creationGuard );
    ...
    for ( u32 it = 0; it < v_geoms.size(); it++ ) {
        SGeometry& G = *( v_geoms[ it ] );
        if ( ( G.dcl == dcl ) && ( G.vb == vb ) && ( G.ib == ib ) &&
             ( G.vb_stride == vb_stride ) ) {
            if ( keep_alive )
...
SGeometry* CResourceManager::CreateGeom( u32 FVF,
                                         ID3DVertexBuffer* vb,
                                         ID3DIndexBuffer* ib,
                                         ref_geom* keep_alive ) {
...
//--------------------------------------------------------------------------------------------------------------
void CResourceManager::ResolveTextureSource( LPCSTR name,
                                             LPCSTR canonical_level_path,
                                             TextureSourceInfo& result ) {
    xr_string key = name;
    key += '\n';
    ...
    if ( producer ) {
        ...
        if ( sourceInfo.loadKind != CTexture::LoadKindDds )
            ...
            if ( !source &&
                 ( !canonical_level_path || !canonical_level_path[ 0 ] ) ) {
...
//--------------------------------------------------------------------------------------------------------------
ref_texture CResourceManager::_CreateTexture( LPCSTR _Name,
                                              bool prefetch,
                                              LPCSTR canonical_level_path ) {
    ...
    if ( ( !canonical_level_path || !canonical_level_path[ 0 ] ) &&
         !g_resource_level_path_override.empty() )
    ...
    if ( source.levelLocal ) {
        string128 identity;
        xr_sprintf( identity, "\n@level:%08x:%08x:%08x:%08x:", source.crc,
                    source.sizeReal, source.sizeCompressed, source.modified );
    ...
    bool created = false;
    {
        ...
        if ( I != m_textures.end() ) {
            ...
            m_textures.insert(
                mk_pair( T->set_name( registryName.c_str() ), T ) );
            T->SetLoadSource(
                Name,
                source.resolvedPath.empty() ? nullptr
...
//--------------------------------------------------------------------------------------------------------------
bool cmp_tl( const std::pair< u32, ref_texture >& _1,
             const std::pair< u32, ref_texture >& _2 ) {
    return _1.first < _2.first;
}

STextureList* CResourceManager::_CreateTextureList(
    STextureList& L,
    ref_texture_list* keep_alive ) {
    xrCriticalSectionGuard guard( creationGuard );
...
//--------------------------------------------------------------------------------------------------------------
dx10ConstantBuffer* CResourceManager::_CreateConstantBuffer(
...
//--------------------------------------------------------------------------------------------------------------
void CResourceManager::_DeleteConstantBuffer(
    const dx10ConstantBuffer* pBuffer ) {
...
//--------------------------------------------------------------------------------------------------------------
SInputSignature* CResourceManager::_CreateInputSignature(
    ID3DBlob* pBlob,
    ref_input_sign* keep_alive ) {
    VERIFY( pBlob );
    xrCriticalSectionGuard guard( creationGuard );
    ...
    for ( u32 it = 0; it < v_input_signature.size(); it++ ) {
        ...
        if ( ( pBlob->GetBufferSize() == sign->signature->GetBufferSize() ) &&
             ( !( memcmp( pBlob->GetBufferPointer(),
                          sign->signature->GetBufferPointer(),
                          pBlob->GetBufferSize() ) ) ) ) {
            if ( keep_alive )
                *keep_alive = sign;
            return sign;
...
//--------------------------------------------------------------------------------------------------------------
void CResourceManager::_DeleteInputSignature(
    const SInputSignature* pSignature ) {
