...
void CHW::SelectAdapterAndOutput( HMONITOR hTargetMonitor ) {
    ...
    // Iterate adapters x outputs; pick the pair whose output owns
    // hTargetMonitor.
    for ( UINT ai = 0;; ++ai ) {
    ...
    Msg( "!HW: selected monitor not found on any adapter, falling back to "
         "default" );
...
void CHW::CreateD3D() {
    ...
    typedef HRESULT _CreateDXGIFactory( REFIID riid,	void **ppFactory);
        ...
        _CreateDXGIFactory  *CreateFactory =
    (_CreateDXGIFactory*)GetProcAddress(hD3D,"CreateDXGIFactory");
    ...
    */
    ...
    IDXGIFactory* pFactory;
    R_CHK( CreateDXGIFactory( __uuidof( IDXGIFactory ),
                              ( void** )( &pFactory ) ) );
#endif
    ...
    UINT i = 0;
#if defined( USE_DX11 )
    while ( m_pFactory->EnumAdapters1( i, &m_pAdapter ) !=
    ...
    if ( !m_pAdapter ) {
#if defined( USE_DX11 )
        SelectAdapterAndOutput(
            MonitorFromWindow( m_hWnd, MONITOR_DEFAULTTOPRIMARY ) );
#elif defined( USE_DX10 )
        const HMONITOR hTarget =
            MonitorFromWindow( m_hWnd, MONITOR_DEFAULTTOPRIMARY );
        bool found = false;
        for ( UINT ai = 0; !found; ++ai ) {
            IDXGIAdapter* adapter = nullptr;
            if ( pFactory->EnumAdapters( ai, &adapter ) ==
                 DXGI_ERROR_NOT_FOUND )
                break;

            for ( UINT oi = 0; !found; ++oi ) {
                IDXGIOutput* output = nullptr;
                if ( adapter->EnumOutputs( oi, &output ) ==
                     DXGI_ERROR_NOT_FOUND )
                    break;

                DXGI_OUTPUT_DESC desc;
                if ( SUCCEEDED( output->GetDesc( &desc ) ) &&
                     desc.Monitor == hTarget ) {
                    m_pAdapter = adapter;
                    m_pOutput = output;
                    found = true;
                    break;
        ...
        if ( !found ) {
            Msg( "!HW: selected monitor not found on any adapter, falling back "
                 "to default (DX10)" );
...
#if defined( USE_DX11 )
    // when using FLIP_* present modes, to disable DWM vsync we have to use
    // DXGI_PRESENT_ALLOW_TEARING with ->Present() when vsync is off
    // (PresentInterval = 0) and only when in window mode store whether we can
    // use the flag for later use (swapchain creation, buffer resize, present
    // call)

    // TODO: On some PC configurations (versions of Windows, graphics drivers,
    // currently unknown what exactly) this isn't sufficient to disable the DWM
    // vsync when the game is launched in a windowed mode, however if the user
    // switches from borderless/windowed -> exclusive fullscreen ->
    // borderless/windowed then it seems to work correctly. Worth investigating
    ...
    // - NVIDIA driver version 560.94
    {
        ...
        if ( SUCCEEDED( hr ) && factory5 ) {
            ...
            hr = factory5->CheckFeatureSupport(
                DXGI_FEATURE_PRESENT_ALLOW_TEARING, &supports_vrr,
                sizeof( supports_vrr ) );
    ...
    /*
        R_ASSERT2	           	 	(hD3D,"Can't find
       'd3d10.dll'\nPlease install latest version of DirectX before running this
       program"); typedef IDirect3D9 * WINAPI _Direct3DCreate9(UINT SDKVersion);
        _Direct3DCreate9* createD3D	=
       (_Direct3DCreate9*)GetProcAddress(hD3D,"Direct3DCreate9");
       R_ASSERT(createD3D); this->pD3D 					=
       createD3D( D3D_SDK_VERSION ); R_ASSERT2
       (this->pD3D,"Please install DirectX 9.0c");
    */
}
...
void CHW::CreateDevice( HWND hwnd, bool move_window ) {
    ...
    */
    ...
    m_DriverType = Caps.bForceGPU_REF ? D3D_DRIVER_TYPE_REFERENCE
    ...
    for (UINT Adapter=0;Adapter<pD3D->GetAdapterCount();Adapter++)	{
        ...
        if (SUCCEEDED(Res) && (xr_strcmp(Identifier.Description,"NVIDIA
    PerfHUD")==0))
        {
            DevAdapter	=Adapter;
            DevT		=D3DDEVTYPE_REF;
    ...
    */
    ...
    Msg( "* GPU [vendor:%X]-[device:%X]: %S", Desc.VendorId, Desc.DeviceId,
         Desc.Description );
    /*
    // Display the name of video board
    D3DADAPTER_IDENTIFIER9	adapterID;
    R_CHK	(pD3D->GetAdapterIdentifier(DevAdapter,0,&adapterID));
        Msg		("* GPU [vendor:%X]-[device:%X]:
    %s",adapterID.VendorId,adapterID.DeviceId,adapterID.Description);
    ...
    u16	drv_Build		= LOWORD(adapterID.DriverVersion.LowPart);
        Msg		("* GPU driver:
    %d.%d.%d.%d",u32(drv_Product),u32(drv_Version),u32(drv_SubVersion),
    u32(drv_Build));
    */
    ...
    */
    ...
    if (bWindowed)
    {
        ...
        case 32:
            ...
            if
    (SUCCEEDED(pD3D->CheckDeviceType(DevAdapter,DevT,fTarget,fTarget,FALSE)))
                break;
    ...
    graphics hardware.\nPlease try to restart the game."); xrLogger::FlushLog();
                MessageBox			(NULL,"Failed to initialize
    graphics hardware.\nPlease try to restart the
    game.","Error!",MB_OK|MB_ICONERROR); TerminateProcess
    (GetCurrentProcess(),0);
    }
    ...
    */
    ...
    sd.Format = ps_r4_hdr10_on ? DXGI_FORMAT_R10G10B10A2_UNORM
    ...
    // Windoze
    // P.SwapEffect			=
    // bWindowed?D3DSWAPEFFECT_COPY:D3DSWAPEFFECT_DISCARD; P.hDeviceWindow
    // = m_hWnd; P.Windowed				= bWindowed;
    ...
    // if( !bWindowed )		P.FullScreen_RefreshRateInHz	= selectRefresh
    // (P.BackBufferWidth, P.BackBufferHeight,fTarget); else
...
#if defined( USE_DX11 )
        sd_fullscreen.RefreshRate =
            selectRefresh( sd.Width, sd.Height, sd.Format );
#elif defined( USE_DX10 )
        sd.BufferDesc.RefreshRate = selectRefresh(
    ...
    if ( Core.ParamsData.test( ECoreParams::dxgi_dbg ) ) {
        ...
        // viewable in VS debugger `Output > Debug` view or using a tool like
    ...
    R_CHK( D3D11CreateDevice( nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr,
                              create_device_flags, pFeatureLevels, 1,
    ...
    R_CHK( m_pFactory->CreateSwapChainForHwnd(
        pDevice, m_hWnd, &sd, &sd_fullscreen, NULL, &m_pSwapChain ) );
    ...
    if ( ps_r4_hdr10_on ) {
        ...
        if ( color_space_supported &
            ...
            R_CHK( swapchain3->SetColorSpace1(
                DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020 ) );
        } else {
            Log( "HDR10 color space unsupported, failed to enable HDR10 "
                 "output" );
            R_CHK( swapchain3->SetColorSpace1(
    ...
    R_CHK( pContext->QueryInterface( __uuidof( ID3DUserDefinedAnnotation ),
                                     ( void** )&pAnnotation ) );
    const u32 setupMs =
        startupTimer.GetElapsed_ms() - factoryMs - deviceMs - swapchainMs;

#else
    R = D3DX10CreateDeviceAndSwapChain( m_pAdapter, m_DriverType, NULL,
                                        createDeviceFlags, &sd, &m_pSwapChain,
                                        &pDevice );
    ...
    */
    ...
    //	DX10: Don't neeed this?
    // #ifdef DEBUG
    //	R_CHK	(pDevice->CreateStateBlock
    //(D3DSBT_ALL,&dwDebugSB)); #endif
    //	Create render target and depth-stencil views here

    // NOTE: this seems required to get the default render target to match the
    // swap chain resolution probably the sequence ResizeTarget, ResizeBuffers,
    // and UpdateViews is important

    // u32	memory
    // = pDevice->GetAvailableTextureMem	();
...
#if defined( USE_DX11 )
    const u32 viewsMs = startupTimer.GetElapsed_ms() - factoryMs - deviceMs -
                        swapchainMs - setupMs;
    Msg( "* [STARTUP/RENDER HW] factory=%u device=%u swapchain=%u setup=%u "
         "views=%u total=%u ms",
         factoryMs, deviceMs, swapchainMs, setupMs, viewsMs,
...
void CHW::DestroyDevice() {
    ...
    _RELEASE( pBaseRT );
    // #ifdef DEBUG
    //	_SHOW_REF ("refCount:dwDebugSB",dwDebugSB); 	_RELEASE
    //(dwDebugSB); #endif
    ...
    if ( !is_windowed ) {
        ...
        if ( Core.ParamsData.test( ECoreParams::dxgi_old ) ) {
            ...
            CHK_DX( m_pSwapChain->ResizeBuffers(
                cd.BufferCount, cd.Width, cd.Height, cd.Format,
...
void CHW::Reset( HWND hwnd ) {
    ...
    m_pSwapChain->SetFullscreenState( !bWindowed,
                                      bWindowed ? NULL : m_pOutput );
...
#elif defined( USE_DX10 )
        desc.RefreshRate =
            selectRefresh( desc.Width, desc.Height, desc.Format );
#endif
    }
    ...
    CHK_DX( m_pSwapChain->ResizeBuffers( cd.BufferCount, cd.Width, cd.Height,
                                         cd.Format, flags ) );
#elif defined( USE_DX10 )
    CHK_DX( m_pSwapChain->ResizeBuffers(
        cd.BufferCount, desc.Width, desc.Height, desc.Format,
        DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH ) );
#endif
    ...
    /*
        // Windoze
        DevPP.SwapEffect			=
       bWindowed?D3DSWAPEFFECT_COPY:D3DSWAPEFFECT_DISCARD; DevPP.Windowed
       = bWindowed; DevPP.PresentationInterval	= D3DPRESENT_INTERVAL_IMMEDIATE;
                if( !bWindowed )		DevPP.FullScreen_RefreshRateInHz
       = selectRefresh
       (DevPP.BackBufferWidth,DevPP.BackBufferHeight,Caps.fTarget); else
       DevPP.FullScreen_RefreshRateInHz	= D3DPRESENT_RATE_DEFAULT; #endif

        while	(TRUE)	{
            HRESULT _hr							=
       HW.pDevice->Reset	(&DevPP); if (SUCCEEDED(_hr))
       break; Msg		("! ERROR: [%dx%d]:
       %s",DevPP.BackBufferWidth,DevPP.BackBufferHeight,Debug.error2string(_hr));
                        Sleep	(100);
        }
        R_CHK				(pDevice->GetRenderTarget
       (0,&pBaseRT)); R_CHK
       (pDevice->GetDepthStencilSurface	(&pBaseZB));
    */
...
DevPP.SwapEffect			=
bWindowed?D3DSWAPEFFECT_COPY:D3DSWAPEFFECT_DISCARD; DevPP.Windowed
= bWindowed; DevPP.PresentationInterval	= D3DPRESENT_INTERVAL_IMMEDIATE; if(
!bWindowed )		DevPP.FullScreen_RefreshRateInHz	= selectRefresh
(DevPP.BackBufferWidth,DevPP.BackBufferHeight,Caps.fTarget); else
DevPP.FullScreen_RefreshRateInHz	= D3DPRESENT_RATE_DEFAULT; #endif
...
HW.pDevice->Reset	(&DevPP); if (SUCCEEDED(_hr))
break; Msg		("! ERROR: [%dx%d]:
%s",DevPP.BackBufferWidth,DevPP.BackBufferHeight,Debug.error2string(_hr)); Sleep
(100);
}
R_CHK				(pDevice->GetRenderTarget
(0,&pBaseRT)); R_CHK				(pDevice->GetDepthStencilSurface
(&pBaseZB)); #ifdef DEBUG R_CHK (pDevice->CreateStateBlock
(D3DSBT_ALL,&dwDebugSB)); #endif #ifndef _EDITOR updateWindowProps	(hwnd);
#endif
*/
}
...
void CHW::selectResolution( u32& dwWidth, u32& dwHeight, BOOL bWindowed ) {
    ...
    if ( g_screenmode == 0 ) {
        ...
        string64 buff;
        xr_sprintf( buff, sizeof( buff ), "%dx%d", psCurrentVidMode[ 0 ],
                    psCurrentVidMode[ 1 ] );

        if ( _ParseItem( buff, vid_mode_token ) == u32( -1 ) ) // not found
        {
            // select safe
            xr_sprintf( buff, sizeof( buff ), "vid_mode %s",
                        vid_mode_token[ 0 ].name );
...
u32 CHW::selectGPU ()
{
    ...
    {
        if (Caps.bForceGPU_NonPure)	return
D3DCREATE_HARDWARE_VERTEXPROCESSING; else { if
(caps.DevCaps&D3DDEVCAPS_PUREDEVICE) return
D3DCREATE_HARDWARE_VERTEXPROCESSING|D3DCREATE_PUREDEVICE; else return
...
DXGI_RATIONAL CHW::selectRefresh( u32 dwWidth, u32 dwHeight, DXGI_FORMAT fmt ) {
    ...
    if ( psDeviceFlags.is( rsRefresh60hz ) ||
         Core.ParamsData.test( ECoreParams::_60hz ) ) {
    ...
    for ( u32 i = 0; i < num; ++i ) {
        ...
        if ( ( desc.Width == dwWidth ) && ( desc.Height == dwHeight ) ) {
            ...
            float TempFreq = float( desc.RefreshRate.Numerator ) /
                             float( desc.RefreshRate.Denominator );
...
void CHW::OnAppActivate() {
    ...
    if ( m_pSwapChain && !is_windowed ) {
        ...
        m_pSwapChain->ResizeBuffers( cd.BufferCount, cd.Width, cd.Height,
                                     cd.Format, flags );
...
BOOL CHW::support( D3DFORMAT fmt, DWORD type, DWORD usage ) {
    ...
    /*
        HRESULT hr		=
       pD3D->CheckDeviceFormat(DevAdapter,DevT,Caps.fTarget,usage,(D3DRESOURCETYPE)type,fmt);
...
void CHW::updateWindowProps( HWND m_hWnd ) {
    ...
    if ( bWindowed ) {
        if ( m_move_window ) {
            ...
            if ( psCurrentVidMode[ 0 ] == 0 || psCurrentVidMode[ 1 ] == 0 )
                GetMonitorResolution( psCurrentVidMode[ 0 ],
                                      psCurrentVidMode[ 1 ] );
            ...
            SetRect( &m_rcWindowBounds, ( LONG( monW ) - res_width ) / 2,
                     ( LONG( monH ) - res_height ) / 2,
                     ( monW + res_width ) / 2, ( monH + res_height ) / 2 );

            SetWindowPos( m_hWnd, HWND_NOTOPMOST, monX + m_rcWindowBounds.left,
    ...
    MapWindowPoints( m_hWnd, nullptr, reinterpret_cast< LPPOINT >( &winRect ),
                     2 );
    ClipCursor( &winRect );
}
...
u32 _cnt								=
_tmp.size()+1; vid_quality_token = xr_alloc<xr_token>(_cnt);
...
void fill_vid_mode_list( CHW* _hw ) {
    ...
    for ( u32 i = 0; i < num; ++i ) {
        ...
        if ( _tmp.end() !=
             std::find_if( _tmp.begin(), _tmp.end(), _uniq_mode( str ) ) )
            continue;
    ...
    xr_vector<LPCSTR>	_tmp;
    u32 cnt = _hw->pD3D->GetAdapterModeCount	(_hw->DevAdapter,
_hw->Caps.fTarget);
    ...
    for(i=0; i<cnt;++i)
    {
        ...
        _hw->pD3D->EnumAdapterModes(_hw->DevAdapter, _hw->Caps.fTarget, i,
&Mode); if(Mode.Width < 800)		continue;

        xr_sprintf
(str,sizeof(str),"%dx%d", Mode.Width, Mode.Height);

        if(_tmp.end() != std::find_if(_tmp.begin(), _tmp.end(),
_uniq_mode(str))) continue;
...
void CHW::UpdateViews() {
    ...
    R = m_pSwapChain->GetBuffer( 0, __uuidof( ID3DTexture2D ),
                                 ( LPVOID* )&pBuffer );
    R_CHK( R );
