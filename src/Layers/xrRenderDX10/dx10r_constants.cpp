...
BOOL R_constant_table::parseConstants(
    ID3DShaderReflectionConstantBuffer* pTable,
    u32 destination ) {
    // VERIFY(_desc);
    // ID3D10ShaderReflectionConstantBuffer *pTable =
    // (ID3D10ShaderReflectionConstantBuffer *)_desc;
    VERIFY( pTable );
    ...
    // D3DXSHADER_CONSTANTINFO* it		= (D3DXSHADER_CONSTANTINFO*)
    // (LPBYTE(desc)+desc->ConstantInfo); LPBYTE
    // ptr	= LPBYTE(desc); for (u32 dwCount = desc->Constants; dwCount;
    // dwCount--,it++)
    for ( u32 i = 0; i < TableDesc.Variables; ++i ) {
        ...
        switch ( TypeDesc.Type ) {
            ...
            default:
                fatal(
                    "R_constant_table::parse: unexpected shader variable "
                    "type." );
        }
        ...
        // TypeInfo + class
        // D3DXSHADER_TYPEINFO*	T	=
        // (D3DXSHADER_TYPEINFO*)(ptr+it->TypeInfo);
        BOOL bSkip = FALSE;
        // switch (T->Class)
        switch ( TypeDesc.Class ) {
            ...
            case D3D10_SVC_VECTOR: {
                switch ( TypeDesc.Columns ) {
                    ...
                    default:
                        fatal(
                            "Vector: 1 components is scalar - there is special "
                            "case for this!!!!!" );
                        break;
                }
            } break;
                    ...
                    case 4:
                        switch ( TypeDesc.Rows ) {
                            ...
                            case 3:
                                ...
                                default:
                                fatal		("MATRIX_ROWS: unsupported
                                number of RegisterCount"); break;
                                }
                                break;
                                */
                            case 4:
                            ...
                            default:
                                fatal(
                                    "MATRIX_ROWS: unsupported number of Rows" );
                                break;
                        }
                        break;
                    default:
            ...
            case D3D10_SVC_OBJECT: {
                ...
                {
                // ***Register sampler***
                // We have determined all valuable info, search if constant
                already created ref_constant	C		=	get
                (name); if (!C)	{ C					=
                xr_new<R_constant>();//.g_constant_allocator.create(); C->name
                =	name; C->destination		=	RC_dest_sampler;
                C->type				=	RC_sampler;
                R_constant_load& L	=	C->samp;
                L.index				=	u16(r_index	+ (
                (destination&1)? 0 : D3DVERTEXTEXTURESAMPLER0 )); L.cls
                =	RC_sampler	; table.push_back		(C); }
                else { R_ASSERT			(C->destination	==
                RC_dest_sampler); R_ASSERT			(C->type
                ==	RC_sampler); R_constant_load& L	=	C->samp;
                R_ASSERT			(L.index		==
                r_index); R_ASSERT			(L.cls ==
                RC_sampler);
                }
                }
                break;
                default:
                fatal		("Pclass D3DXPC_OBJECT - object isn't of
                'sampler' type"); break;
                }
                */
            }
        ...
        // We have determined all valuable info, search if constant already
        // created
...
BOOL R_constant_table::parseResources( ID3DShaderReflection* pReflection,
                                       int ResNum,
                                       u32 destination ) {
    for ( int i = 0; i < ResNum; ++i ) {
        ...
        // u16	r_index = u16( ResDesc.BindPoint + ((destination&1)? 0 :
        // CTexture::rstVertex) );
    ...
    if ( ShaderDesc.ConstantBuffers ) {
        ...
        for ( u16 iBuf = 0; iBuf < ShaderDesc.ConstantBuffers; ++iBuf ) {
            ...
            if ( pTable ) {
                ...
                updatedDest |= iBuf << dest_to_shift_value(
                                   destination ); /*((destination&RC_dest_pixel)
...
? CB_BufferVertexShader : CB_BufferGeometryShader;*/
                ...
                dxRenderDeviceRender::Instance()
                    .Resources->_CreateConstantBuffer( pTable, &tempBuffer );
                m_CBTable.push_back(
                    cb_table_record( uiBufferIndex, tempBuffer ) );
            }
        }
    }
