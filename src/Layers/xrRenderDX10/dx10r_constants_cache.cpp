...
dx10ConstantBuffer& R_constants::GetCBuffer( R_constant* C, BufferType BType ) {
    ...
    if ( BType == BT_PixelBuffer ) {
        //	Decode index
        int iBufferIndex = ( C->destination & RC_dest_pixel_cb_index_mask ) >>
        ...
        //	Decode index
        int iBufferIndex = ( C->destination & RC_dest_vertex_cb_index_mask ) >>
        ...
        //	Decode index
        int iBufferIndex =
            ( C->destination & RC_dest_geometry_cb_index_mask ) >>
        ...
        //	Decode index
        int iBufferIndex = ( C->destination & RC_dest_hull_cb_index_mask ) >>
        ...
        //	Decode index
        int iBufferIndex = ( C->destination & RC_dest_domain_cb_index_mask ) >>
        ...
        //	Decode index
        int iBufferIndex = ( C->destination & RC_dest_compute_cb_index_mask ) >>
        ...
        if (a_pixel.b_dirty)
        {
                ...
                R_constant_array::t_f&	F	= a_pixel.c_f;
                {
                        //if (F.r_lo() <= 32) //. hack
                        {
                                ...
                                RCache.m_pPixelConstants->Map(D3Dxx_MAP_WRITE_DISCARD,
0, &pBuffer); CopyMemory(pBuffer, F.access(0),
iVectorNumber*iVectorElements*sizeof(float)); RCache.m_pPixelConstants->Unmap();
                        }
                }
        ...
        if (a_vertex.b_dirty)
        {
                ...
                R_constant_array::t_f&	F	= a_vertex.c_f;
                {
                        ...
                        if (count)			{
                                ...
                                {
                                        Debug.fatal(DEBUG_INFO,"Internal error
setting VS-constants: overflow\nregs[%d],hi[%d]",
...
#endif
                                {
                                        ...
                                        const int iVectorElements = 4;
                                        const int iVectorNumber = 256;
                                        RCache.m_pVertexConstants->Map(D3Dxx_MAP_WRITE_DISCARD,
0, &pBuffer); CopyMemory(pBuffer, F.access(0),
iVectorNumber*iVectorElements*sizeof(float));
