...
void CBlender_Compile::r_Stencil( BOOL Enable,
                                  u32 Func,
                                  u32 Mask,
                                  u32 WriteMask,
                                  u32 Fail,
...
void CBlender_Compile::r_dx10Texture( LPCSTR ResourceName, LPCSTR texture ) {
    ...
    passTextures.push_back(
        mk_pair( stage, ref_texture( DEV->_CreateTexture( TexName ) ) ) );
}
...
#ifdef USE_DX11
void CBlender_Compile::r_TessPass( LPCSTR vs,
                                   LPCSTR hs,
                                   LPCSTR ds,
                                   LPCSTR gs,
                                   LPCSTR ps,
                                   ...
                                   BOOL aTest,
                                   u32 aRef ) {
    r_Pass( vs, gs, ps, bFog, bZtest, bZwrite, bABlend, abSRC, abDST, aTest,
            aRef );
...
void CBlender_Compile::r_End() {
    ...
    // SH->passes.push_back
    // (DEV->_CreatePass(dest.state,dest.ps,dest.vs,dest.gs,dest.constants,dest.T,temp,dest.C));
}
