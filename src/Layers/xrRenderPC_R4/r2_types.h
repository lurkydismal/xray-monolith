...
#define r2_RT_accum_temp \
    "$user$accum_temp" // ---	16 bit fp - only for HW which doesn't feature
                       // fp16 blend
...
#define r2_RT_generic2 \
    "$user$generic2"                  // ---	//	Igor: for volumetric lights
...
#define r2_RT_secondVP \
    "$user$viewport2" // --#SM+#-- +SecondVP+ Рендер-таргет для второго вьюпорта
...
// #define		r2_RT_ssfx_hud			"$user$ssfx_hud" // HUD
// & Velocity Buffer
...
const u32 TEX_material_LdotN =
    128;                            // diffuse,		X, almost linear = small res
...
IC float u_diffuse2s( float x, float y, float z ) {
    if ( ps_ssfx_gloss_method == 0 ) {
        ...
        return ps_r2_gloss_min +
               ps_r2_gloss_factor * ( ( v < 1 ) ? powf( v, 2.f / 3.f ) : v );
    } else {
        // Remove sun from the equation and clamp value.
        return ps_ssfx_gloss_minmax.x +
               clampr( ps_ssfx_gloss_minmax.y - ps_ssfx_gloss_minmax.x, 0.0f,
                       1.0f ) *
                   ps_ssfx_gloss_factor;
    }
}
