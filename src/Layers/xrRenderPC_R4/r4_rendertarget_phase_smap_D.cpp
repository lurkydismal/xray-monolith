...
void CRenderTarget::phase_smap_direct( light* L, u32 sub_phase ) {
    ...
    if ( sub_phase != SE_SUN_RAIN_SMAP )
        ...
        D3D_VIEWPORT VP = { ( float )L->X.D.minX,
                            ( float )L->X.D.minY,
    ...
    /*
    if (SE_SUN_NEAR==sub_phase)			RCache.set_CullMode
    ( CULL_CCW	);	// near else
    { if (RImplementation.o.HW_smap)	RCache.set_CullMode
    ( CULL_CW	);	// far, reversed else
    RCache.set_CullMode			( CULL_CCW	);	// far,
    front-faces
    }
    if (RImplementation.o.HW_smap)		RCache.set_ColorWriteEnable
    ( FALSE		); else
...
void CRenderTarget::phase_smap_direct_tsh( light* L, u32 sub_phase ) {
    ...
    // u32		_clr						=
    // 0xffffffff;	//color_rgba(127,127,12,12);
    ...
    // CHK_DX
    // (HW.pDevice->Clear( 0L, NULL, D3DCLEAR_TARGET,	_clr,	1.0f, 0L));
}
