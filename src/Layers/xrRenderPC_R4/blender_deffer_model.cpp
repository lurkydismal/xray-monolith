...
void CBlender_deffer_model::Compile( CBlender_Compile& C ) {
    ...
    if ( bForward ) {
        ...
        bool bUseATOC = ( bAref && ( RImplementation.o.dx10_msaa_alphatest ==
        ...
        switch ( C.iElement ) {
            case SE_R2_NORMAL_HQ: // deffer
                if ( bUseATOC ) {
                    uber_deffer( C, true, "model", "base_atoc", bAref, 0,
                                 true );
                ...
                if ( C.HudElement ) {
                    uber_deffer( C, true, "model_hud", "base_hud", bAref, 0,
                                 true );
            ...
            case SE_R2_NORMAL_LQ: // deffer
                if ( bUseATOC ) {
                    uber_deffer( C, false, "model", "base_atoc", bAref, 0,
                                 true );
                    C.r_Stencil( TRUE, D3DCMP_ALWAYS, 0xff, 0x7f,
                                 D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE,
                                 D3DSTENCILOP_KEEP );
            ...
            case SE_R2_SHADOW: // smap
                if ( bAref ) {
                    ...
                    // ("shadow_direct_model_aref","shadow_direct_base_aref",
                    // FALSE); C.r_Sampler
                    // ("s_base",C.L_textures[0]);
                    C.r_Pass( "shadow_direct_model_aref",
                              "shadow_direct_base_aref", FALSE, TRUE, TRUE,
                              FALSE, D3DBLEND_ZERO, D3DBLEND_ONE, TRUE, 220 );
                    C.r_dx10Texture( "s_base", C.L_textures[ 0 ] );
                    C.r_dx10Sampler( "smp_base" );
