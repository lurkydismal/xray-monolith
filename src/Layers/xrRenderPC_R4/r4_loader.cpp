...
u64 LevelIdentity( LPCSTR canonical_level_path ) {
    static LPCSTR files[] = { "level",         "level.geom", "level.geomx",
                              "level.details", "level.hom",  "build.lights",
                              "level.fog_vol" };
    ...
    // o.distortion is a per-frame phase flag, so hashing the struct causes
    // false misses for otherwise identical static packages.
    const CRender::_options& o = RImplementation.o;
    const u32 option_values[] = { o.ssfx_branches,
                                  o.ssfx_blood,
                                  o.ssfx_rain,
                                  o.ssfx_hud_raindrops,
                                  o.ssfx_ssr,
                                  o.ssfx_terrain,
                                  o.ssfx_volumetric,
                                  o.ssfx_water,
                                  ...
                                  o.dx10_msaa_opt,
                                  o.dx10_sm4_1,
                                  o.dx10_msaa_alphatest,
                                  o.dx10_msaa_samples,
                                  o.dx10_minmax_sm,
                                  o.dx10_minmax_sm_screenarea_threshold,
    ...
    MixLevelIdentity( identity,
                      crc32( &o.forcegloss_v, sizeof( o.forcegloss_v ) ) );
    MixLevelIdentity( identity, dm_current_size );
    MixLevelIdentity( identity, crc32( &ps_current_detail_density,
                                       sizeof( ps_current_detail_density ) ) );
    MixLevelIdentity( identity, crc32( &ps_current_detail_height,
                                       sizeof( ps_current_detail_height ) ) );
    MixLevelIdentity(
        identity, g_level_asset_generation.load( std::memory_order_acquire ) );
    return identity;
}
...
void PrepareLevelLights( const xr_string& level_path,
                         PreparedLights& prepared ) {
...
void PrepareLevelChunk( const xr_string& level_path,
                        u32 chunk_id,
                        xr_vector< u8 >& data ) {
    xr_string level_name = level_path + "level";
    IReader* level = FS.r_open( level_name.c_str() );
...
void PrepareLevelSectors( const xr_string& level_path,
                          xr_vector< u8 >& portals,
                          xr_vector< xr_vector< u8 > >& sectors,
                          CDB::MODEL*& portal_model ) {
    xr_string level_name = level_path + "level";
    IReader* level = FS.r_open( level_name.c_str() );
    R_ASSERT2( level, level_name.c_str() );
    ...
    const b_portal* source =
        reinterpret_cast< const b_portal* >( portals.data() );
    const u32 portal_count =
        static_cast< u32 >( portals.size() / sizeof( b_portal ) );
    if ( !portal_count )
        return;

    CDB::Collector collector;
    for ( u32 index = 0; index < portal_count; ++index )
        for ( u32 vertex = 2; vertex < source[ index ].vertices.size();
              ++vertex )
            collector.add_face_packed_D( source[ index ].vertices[ 0 ],
                                         source[ index ].vertices[ vertex - 1 ],
                                         source[ index ].vertices[ vertex ],
                                         index );
    ...
    portal_model->build( collector.getV(), int( collector.getVS() ),
                         collector.getT(), int( collector.getTS() ) );
}

void PrepareLevelFluids(
    const xr_string& level_path,
    xr_vector< dx103DFluidData::PreparedData >& prepared ) {
    prepared.clear();
...
void CommitVisualShaderTree( IRenderVisual* visual ) {
    ...
    if ( xr_vector< IRenderVisual* >* children =
             visual->get_children_invisible() )
        for ( IRenderVisual* child : *children )
            CommitVisualShaderTree( child );
}
...
bool IsWorkerSafeLevelVisual( u32 type ) {
    return type == MT_NORMAL || type == MT_PROGRESSIVE || type == MT_TREE_ST ||
           type == MT_TREE_PM;
}
...
bool CRender::level_StaticCacheReady( LPCSTR canonical_level_path ) {
    ...
    for ( const LevelStaticPackage* package : m_level_cache )
        if ( package->key.equal( path.c_str() ) &&
...
void CRender::level_Prepare( LPCSTR canonical_level_path ) {
    ...
    if ( m_prepared_level_geometry )
        dxRenderDeviceRender::Instance().Resources->ReleaseLevelShaderCache(
            m_prepared_level_geometry->key.c_str(),
            m_prepared_level_geometry->identity );
    ...
    auto submit = [ this, &executor ]( NativeLoadPriority priority,
                                       auto&& work ) {
        if ( m_level_prepare_batch.Valid() )
            executor.Submit( m_level_prepare_batch, priority,
                             std::forward< decltype( work ) >( work ) );
        else
            m_level_prepare_tasks.run(
                std::forward< decltype( work ) >( work ) );
    };
    submit( NativeLoadPriority::Geometry,
            [ path ]() { CObjectSpace::PrepareStatic( path.c_str() ); } );
    if ( static_cache_hit ) {
    ...
    const bool detail_recipe_changed =
    ...
    if ( !g_dedicated_server && ( !b_loaded || !detail_recipe_changed ) ) {
        ...
        submit( NativeLoadPriority::Environment,
                [ path, geometry, normal_swing, fast_swing ]() {
                    const CDetailManager::SSwingValue prepared_swing[ 2 ] = {
                        normal_swing, fast_swing };
                    geometry->details = xr_new< CDetailManager >();
                    geometry->details->Load( false, false, path.c_str(),
                                             prepared_swing );
    ...
    submit( NativeLoadPriority::Geometry, [ this, path, geometry ]() {
        ...
        auto load_normal = [ this, path, geometry ]() {
            ...
            R_ASSERT2( reader, file_name.c_str() );
            LoadBuffers( reader, geometry->normal_declarations,
                         geometry->normal_vertex_buffers,
        ...
        auto load_fast = [ this, path, geometry ]() {
            ...
            CStreamReader* reader = FS.rs_open( nullptr, file_name.c_str() );
            R_ASSERT2( reader, file_name.c_str() );
            LoadBuffers( reader, geometry->fast_declarations,
                         geometry->fast_vertex_buffers,
        ...
        NativeLoadExecutor::Batch batch =
            executor.BeginBatch( executor.CurrentGeneration() );
        if ( batch.Valid() ) {
        ...
        VisualGeometrySource source = { &geometry->normal_declarations,
                                        ...
                                        &geometry->fast_index_buffers,
                                        &geometry->swis };
        IReader visuals( geometry->visual_data.data(),
                         static_cast< int >( geometry->visual_data.size() ) );
        PrepareVisuals( &visuals, geometry->visuals, &source );
        geometry->visual_leaves_ready = true;
    } );
    submit( NativeLoadPriority::Environment, [ path, geometry ]() {
        PrepareLevelSectors( path, geometry->prepared_portals,
                             geometry->prepared_sectors,
    ...
    submit( NativeLoadPriority::ShaderTexture, [ path, geometry, identity ]() {
        ...
        for ( u32 i = 0; i < count; ++i ) {
            ...
            *delimiter = 0;
            ...
            geometry->shader_indices[ i ] =
                static_cast< u32 >( geometry->shader_descriptions.size() );
            unique_indices.emplace( std::move( key ),
                                    geometry->shader_indices[ i ] );
            geometry->shader_descriptions.push_back(
                { shader_name, textures } );
        }
        shaders->close();
        FS.r_close( level );

        geometry->cpp_shader_results.resize(
            geometry->shader_descriptions.size() );
        NativeLoadExecutor& shader_executor = NativeLoadExecutor::Instance();
        NativeLoadExecutor::Batch shader_batch =
            shader_executor.BeginBatch( shader_executor.CurrentGeneration() );
        auto compile_shader = [ path, geometry, identity ]( u32 index ) {
            const LevelShaderDescription& description =
                geometry->shader_descriptions[ index ];
            geometry->cpp_shader_results[ index ] =
                dxRenderDeviceRender::Instance()
                    .Resources->CreateLevelCppShader(
                        description.shader.c_str(),
                        description.textures.c_str(), nullptr, nullptr,
                        identity, path.c_str() );
        };
        if ( shader_batch.Valid() ) {
            for ( u32 index = 0; index < geometry->shader_descriptions.size();
                  ++index )
                shader_executor.Submit(
                    shader_batch, NativeLoadPriority::ShaderTexture,
                    [ compile_shader, index ]() { compile_shader( index ); } );
            shader_executor.Wait( shader_batch );
        } else
            xr_parallel_for(
                0u, static_cast< u32 >( geometry->shader_descriptions.size() ),
                compile_shader );
        geometry->shader_recipes_ready = true;
    } );
    const bool prepare_fluids = RImplementation.o.volumetricfog;
    submit( NativeLoadPriority::Environment,
            [ path, geometry, prepare_fluids ]() {
                if ( prepare_fluids )
...
void CRender::CommitLevelShaderComponentsOwner() {
    ...
    if ( !m_level_shader_owner_required ) {
        if ( m_level_cache_attach_pending ) {
            ...
            xr_delete( m_level_prepared_lights );
            Msg( "* [LEVEL CACHE] R4 owner lights commit: %d ms",
                 cache_lights_timer.GetElapsed_ms() );
            m_active_level_lights_dynamic.swap(
                m_level_prepared_lights_dynamic );
            m_active_level_lights_hemi.swap( m_level_prepared_lights_hemi );
            if ( Details )
                Details->Resume();
            Commit3DFluid();
            m_level_cache_attach_pending = false;
            Msg( "* [LEVEL CACHE] R4 owner attach: %d ms",
                 cache_timer.GetElapsed_ms() );
        }
        return;
    }
    ...
    for ( u32 i = 0; i < m_level_shader_descriptions.size(); ++i ) {
        const LevelShaderDescription& description =
            m_level_shader_descriptions[ i ];
        if ( resources->_lua_HasShader( description.shader.c_str() ) ) {
            ++lua_count;
            m_level_shader_owner_results[ i ] = resources->CreateLevelShader(
                description.shader.c_str(), description.textures.c_str(),
                m_level_shader_identity );
        } else {
            m_level_shader_owner_results[ i ] = m_level_shader_cpp_results[ i ];
            if ( !m_level_shader_owner_results[ i ] )
                m_level_shader_owner_results[ i ] =
                    resources->CreateLevelShader( description.shader.c_str(),
                                                  description.textures.c_str(),
                                                  m_level_shader_identity );
        }
    }
    ...
    Lights.CommitPrepared();
    xr_delete( m_level_prepared_lights );
    Msg( "* [LEVEL LOAD] R4 owner lights commit: %d ms",
         lights_timer.GetElapsed_ms() );
...
void CRender::CommitLevelEnvironmentOwner() {
    ...
    if ( m_level_prepared_visuals_ready ) {
        ...
        IReader visuals(
            m_level_prepared_visual_data.data(),
    ...
    g_pGameLevel->ObjectSpace.GetStaticModel()->syncronize();
    Commit3DFluid();
    Msg( "* [LEVEL LOAD] R4 owner sectors/fluid commit: %d ms",
         environment_timer.GetElapsed_ms() );
}
...
void CRender::level_Load( IReader* fs ) {
    ...
    if ( m_prepared_level_path.size() ) {
        ...
        if ( m_prepared_level_path.equal( level_key ) )
            Msg( "* [LEVEL PREPARE] barrier: %u ms total (%s)",
                 Device.TimerAsync() - m_level_prepare_started_at,
                 level_key.c_str() );
    ...
    const bool use_prepared_fluids =
        prepared_geometry && prepared_geometry->key.equal( level_key ) &&
        prepared_geometry->identity == level_identity &&
        prepared_geometry->fluid_descriptors_ready;
    ...
    if ( RestoreLevelStaticPackage( level_key, level_identity ) ) {
        if ( prepared_geometry &&
             ( !prepared_geometry->key.equal( level_key ) ||
               prepared_geometry->identity != level_identity ) ) {
            dxRenderDeviceRender::Instance().Resources->ReleaseLevelShaderCache(
        ...
        for ( u32 i = 0; i < m_active_level_shader_indices.size(); ++i )
            if ( m_active_level_shader_indices[ i ] != u32( -1 ) )
                Shaders[ i ] = m_level_shader_owner_results
                    [ m_active_level_shader_indices[ i ] ];
        for ( dxRender_Visual* visual : Visuals )
            CommitVisualShaderTree( visual );
        ...
        Msg( "* [LEVEL CACHE] R4 attach: %d ms (%s)",
             level_timer.GetElapsed_ms(), level_key.c_str() );
        return;
    }
    R_ASSERT( !b_loaded );
    const bool use_prepared_geometry =
        prepared_geometry && prepared_geometry->key.equal( level_key ) &&
        prepared_geometry->identity == level_identity &&
        prepared_geometry->normal_geometry_ready &&
        prepared_geometry->fast_geometry_ready;
    const bool use_prepared_shaders =
        prepared_geometry && prepared_geometry->key.equal( level_key ) &&
        prepared_geometry->identity == level_identity &&
        prepared_geometry->shader_recipes_ready;
    const bool use_prepared_visuals =
        use_prepared_geometry && prepared_geometry->visual_leaves_ready;
    const bool use_prepared_sectors =
        prepared_geometry && prepared_geometry->key.equal( level_key ) &&
        prepared_geometry->identity == level_identity &&
        prepared_geometry->sectors_ready;
    const bool use_prepared_details =
        prepared_geometry && prepared_geometry->key.equal( level_key ) &&
        prepared_geometry->identity == level_identity &&
        prepared_geometry->details_ready;
    const bool use_prepared_hom =
        prepared_geometry && prepared_geometry->key.equal( level_key ) &&
        prepared_geometry->identity == level_identity &&
        prepared_geometry->hom_ready;
    const bool use_prepared_lights =
        prepared_geometry && prepared_geometry->key.equal( level_key ) &&
        prepared_geometry->identity == level_identity &&
        prepared_geometry->lights_ready && prepared_geometry->prepared_lights;
    ...
    if ( use_prepared_shaders ) {
        ...
        m_level_shader_cpp_results.swap(
            prepared_geometry->cpp_shader_results );
        Msg( "* [LEVEL PREPARE] C++ shader recipes committed: %u",
    ...
    if ( use_prepared_sectors ) {
        ...
        m_level_prepared_portals_model =
            prepared_geometry->prepared_portals_model;
    ...
    if ( use_prepared_lights ) {
        m_level_prepared_lights_dynamic.swap(
            prepared_geometry->prepared_lights_dynamic );
        m_level_prepared_lights_hemi.swap(
            prepared_geometry->prepared_lights_hemi );
    ...
    NativeLoadExecutor::Batch component_batch =
        load_executor.BeginBatch( load_executor.CurrentGeneration() );
    xr_task_group fallback_component_tasks;
    auto submit_component = [ &load_executor, &component_batch,
                              &fallback_component_tasks ](
                                NativeLoadPriority priority, auto&& work ) {
        if ( component_batch.Valid() )
            load_executor.Submit( component_batch, priority,
                                  std::forward< decltype( work ) >( work ) );
        else
            fallback_component_tasks.run(
                std::forward< decltype( work ) >( work ) );
    };
    CTimer shader_timer;
    shader_timer.Start();
    if ( !use_prepared_shaders )
        for ( u32 index = 0; index < unique_shaders.size(); ++index )
            submit_component(
                NativeLoadPriority::ShaderTexture,
                [ this, &unique_shaders, level_identity, level_key, index ]() {
                    const LevelShaderDescription& description =
                        unique_shaders[ index ];
                    m_level_shader_cpp_results[ index ] =
                        dxRenderDeviceRender::Instance()
                            .Resources->CreateLevelCppShader(
                                description.shader.c_str(),
                                description.textures.c_str(), nullptr, nullptr,
                                level_identity, level_key.c_str() );
                } );
    ...
    if ( !use_prepared_lights )
        submit_component( NativeLoadPriority::Environment, [ this,
                                                             level_key ]() {
            CTimer timer;
            timer.Start();
            ...
            m_level_prepared_lights->Prepare( m_level_prepared_lights_dynamic,
                                              m_level_prepared_lights_hemi );
            Msg( "* [LEVEL LOAD] R4 lights prepare: %d ms",
                 timer.GetElapsed_ms() );
        } );
    if ( !use_prepared_hom )
        submit_component(
            NativeLoadPriority::Environment, [ this, level_key ]() {
                CTimer timer;
                timer.Start();
    ...
    if ( !g_dedicated_server && !use_prepared_details ) {
        ...
        submit_component( NativeLoadPriority::Environment, [ this, level_key,
            ...
            Msg( "* [LEVEL LOAD] R4 details prepare: %d ms",
                 timer.GetElapsed_ms() );
        } );
    }
    ...
    auto commit_prepared_components = [ & ]() {
        ...
        if ( !use_prepared_shaders )
            Msg( "* [LEVEL LOAD] R4 C++ shader prepare: %d ms (%u speculative)",
                 shader_timer.GetElapsed_ms(),
                 static_cast< u32 >( unique_shaders.size() ) );
        CommitLevelShaderComponentsOwner();
        R_ASSERT2( !m_level_async_failed.load( std::memory_order_acquire ),
                   "R4 owner shader/environment commit failed" );
        for ( u32 i = 0; i < shader_indices.size(); ++i )
            if ( shader_indices[ i ] != u32( -1 ) )
                Shaders[ i ] =
                    m_level_shader_owner_results[ shader_indices[ i ] ];
        m_active_level_shader_descriptions = m_level_shader_descriptions;
        m_active_level_shader_cpp_results = m_level_shader_cpp_results;
    ...
    if ( !g_dedicated_server ) {
        ...
        NativeLoadExecutor::Batch geometry_batch =
            load_executor.BeginBatch( load_executor.CurrentGeneration() );
        xr_task_group fallback_geometry_tasks;
        auto submit_geometry = [ &load_executor, &geometry_batch,
                                 &fallback_geometry_tasks ]( auto&& work ) {
            if ( geometry_batch.Valid() )
                load_executor.Submit(
                    geometry_batch, NativeLoadPriority::Geometry,
                    std::forward< decltype( work ) >( work ) );
            else
                fallback_geometry_tasks.run(
                    std::forward< decltype( work ) >( work ) );
        };
        if ( !use_prepared_geometry )
        ...
        if ( !use_prepared_geometry )
            submit_geometry( [ this, level_key ]() {
                ...
                timer.Start();
                xr_string file_name =
                    xr_string( level_key.c_str() ) + "level.geomx";
                CStreamReader* geom = FS.rs_open( nullptr, file_name.c_str() );
                R_ASSERT2( geom, file_name.c_str() );
                LoadBuffers( geom, xDC, xVB, xIB );
                FS.r_close( geom );
                Msg( "* [LEVEL LOAD] R4 level.geomx: %d ms",
                     timer.GetElapsed_ms() );
            } );
        ...
        Msg( "* [LEVEL LOAD] R4 geometry barrier: %d ms",
             geometry_timer.GetElapsed_ms() );
        ...
        Msg( "* [LEVEL LOAD] R4 visual leaves prepare: %d ms",
             visuals_timer.GetElapsed_ms() );
    } else {
        commit_prepared_components();
    }
    ...
    R_ASSERT2( !m_level_async_failed.load( std::memory_order_acquire ),
               "R4 owner sectors/fluid commit failed" );
    m_level_owner_reader = nullptr;
...
void CRender::level_Unload() {
    ...
    const bool clear_models =
        psDeviceFlags2.test( rsClearModels ) || clear_resources;
    if ( !clear_models ) {
    ...
    if ( clear_resources ) {
        dxRenderDeviceRender::Instance()
            .Resources->UnloadAllTexturesOnLevelUnload();
...
void CRender::EvictLevelCacheUnderPressure() {
    ...
    while ( !m_level_cache.empty() && GlobalMemoryStatusEx( &memory ) ) {
        const u64 minimum_available =
            _max( 2ull * 1024 * 1024 * 1024, memory.ullTotalPhys * 15 / 100 );
        if ( memory.ullAvailPhys >= minimum_available )
            break;
        LevelStaticPackage* package = m_level_cache.front();
        Msg( "* [LEVEL CACHE] R4 evicted under memory pressure: %s",
             package->key.c_str() );
...
void CRender::LoadBuffers( CStreamReader* base_fs,
    ...
    // Vertex buffers
    {
        ...
        R_ASSERT2( fs,
                   "Could not load geometry. File 'level.geom?' corrupted." );
        u32 count = fs->r_u32();
        _DC.resize( count );
        _VB.resize( count );
        ...
        for ( u32 i = 0; i < count; i++ ) {
            // decl
            //			D3DVERTEXELEMENT9*	dcl		=
            //(D3DVERTEXELEMENT9*) fs().pointer();
            fs->r( dcl, bufferSize );
            ...
            Msg( "* [Loading VB] %d verts, %d Kb", vCount,
                 ( vCount * vSize ) / 1024 );
            ...
            // R_CHK
            // (_VB[i]->Lock(0,0,(void**)&pData,0)); 			CopyMemory
            //(pData,fs().pointer(),vCount*vSize); fs->r
            // (pData,vCount*vSize); _VB[i]->Unlock		();
        ...
        for ( u32 i = 0; i < count; ++i )
            submit( [ &, i ]() {
                dx10BufferUtils::CreateVertexBuffer(
                    &_VB[ i ], vertex_data[ i ].data(),
                    static_cast< UINT >( vertex_data[ i ].size() ) );
            } );
    }
    ...
    // Index buffers
    {
        ...
        for ( u32 i = 0; i < count; i++ ) {
            u32 iCount = fs->r_u32();
            Msg( "* [Loading IB] %d indices, %d Kb", iCount,
                 ( iCount * 2 ) / 1024 );
            ...
            // R_CHK
            // (_IB[i]->Lock(0,0,(void**)&pData,0)); 			CopyMemory
            //(pData,fs().pointer(),iCount*2); fs->r
            // (pData,iCount*2); _IB[i]->Unlock		();
        ...
        for ( u32 i = 0; i < count; ++i )
            submit( [ &, i ]() {
                dx10BufferUtils::CreateIndexBuffer(
                    &_IB[ i ], index_data[ i ].data(),
                    static_cast< UINT >( index_data[ i ].size() ) );
            } );
    }
...
void CRender::LoadVisualLeaf( dxRender_Visual* visual,
                              IReader* chunk,
...
void CRender::PrepareVisuals( IReader* fs,
                              xr_vector< dxRender_Visual* >& visuals,
                              const VisualGeometrySource* geometry ) {
    R_ASSERT( visuals.empty() );
    ...
    NativeLoadExecutor& executor = NativeLoadExecutor::Instance();
    NativeLoadExecutor::Batch visual_batch =
        executor.BeginBatch( executor.CurrentGeneration() );
#if 0
	try
	{
...
void CRender::LinkPreparedVisuals( IReader* fs,
                                   xr_vector< dxRender_Visual* >& visuals ) {
    const xr_vector< dxRender_Visual* >* previous_visuals =
...
void CRender::LoadSectors( IReader* fs ) {
    ...
    if ( count ) {
        ...
        for ( u32 i = 0; i < count; i++ ) {
            ...
            for ( u32 j = 2; j < P.vertices.size(); j++ )
                CL.add_face_packed_D( P.vertices[ 0 ], P.vertices[ j - 1 ],
                                      P.vertices[ j ], u32( i ) );
        }
        if ( CL.getTS() < 2 ) {
            Fvector v1, v2, v3;
        ...
        rmPortals->build( CL.getV(), int( CL.getVS() ), CL.getT(),
                          int( CL.getTS() ) );
    } else {
        rmPortals = 0;
    }
    ...
    // Search for default sector - assume "default" or "outdoor" sector is the
    // largest one
...
void CRender::LoadPreparedSectors() {
    ...
    const u32 portal_count = static_cast< u32 >(
        m_level_prepared_portals.size() / sizeof( b_portal ) );
    ...
    for ( u32 index = 0; index < portal_count; ++index ) {
        ...
        CopyMemory(
            &portal,
            m_level_prepared_portals.data() + index * sizeof( b_portal ),
            sizeof( portal ) );
        CPortal* target = static_cast< CPortal* >( Portals[ index ] );
        target->Setup(
            portal.vertices.begin(), portal.vertices.size(),
...
void CRender::Remove3DFluid() {
    for ( dxRender_Visual* visual : Visuals ) {
        if ( visual->getType() != MT_HIERRARHY )
            continue;
        FHierrarhyVisual* hierarchy =
            static_cast< FHierrarhyVisual* >( visual );
        for ( auto child = hierarchy->children.begin();
              child != hierarchy->children.end(); ) {
...
void CRender::Commit3DFluid() {
    ...
    for ( const dx103DFluidData::PreparedData& prepared :
          m_level_fluid_descriptors ) {
        dx103DFluidVolume* pVolume = xr_new< dx103DFluidVolume >();
        pVolume->LoadPrepared( prepared );

        //	Attach to sector's static geometry
        CSector* pSector =
            ( CSector* )detectSector( pVolume->getVisData().sphere.P );
