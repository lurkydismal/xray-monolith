...
u32 GetCB( R_constant* C ) {
    return ( C->destination & RC_dest_pixel_cb_index_mask ) >>
...
ComputeShader& ComputeShader::set_c( shared_str name,
                                     float x,
                                     float y,
                                     float z,
                                     float w ) {
...
void ComputeShader::Dispatch( u32 dimx, u32 dimy, u32 dimz ) {
    ...
    if ( !m_Textures.empty() )
        HW.pContext->CSSetShaderResources( 0, m_Textures.size(),
                                           &m_Textures[ 0 ] );
    ...
    if ( !m_Outputs.empty() ) {
        UINT num = 0;
        HW.pContext->CSSetUnorderedAccessViews( 0, m_Outputs.size(),
                                                &m_Outputs[ 0 ], &num );
    }
