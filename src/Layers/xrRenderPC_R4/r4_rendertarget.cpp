...
CRenderTarget::CRenderTarget() {
    ...
    s_hdr10_bloom_downsample.create( b_hdr10_bloom_downsample,
                                     "hdr10_bloom_downsample" );
    s_hdr10_bloom_blur.create( b_hdr10_bloom_blur, "hdr10_bloom_blur" );
    s_hdr10_bloom_upsample.create( b_hdr10_bloom_upsample,
                                   "hdr10_bloom_upsample" );

    s_hdr10_lens_flare_downsample.create( b_hdr10_lens_flare_downsample,
                                          "hdr10_lens_flare_downsample" );
    s_hdr10_lens_flare_fgen.create( b_hdr10_lens_flare_fgen,
                                    "hdr10_lens_flare_fgen" );
    s_hdr10_lens_flare_blur.create( b_hdr10_lens_flare_blur,
                                    "hdr10_lens_flare_blur" );
    s_hdr10_lens_flare_upsample.create( b_hdr10_lens_flare_upsample,
                                        "hdr10_lens_flare_upsample" );
    ...
    // Screen Space Shaders Stuff
    s_ssfx_fog_scattering.create( b_ssfx_fog_scattering,
                                  "ssfx_fog_scattering" ); // SSS Fog Scattering
    s_ssfx_motion_blur.create( b_ssfx_motion_blur,
                               "ssfx_motion_blur" );   // SSS Motion Blur
    ...
    s_ssfx_bloom_lens.create( b_ssfx_bloom_lens,
                              "ssfx_bloom_flares" ); // SSS Bloom Lens flare
    s_ssfx_bloom_downsample.create( b_ssfx_bloom_downsample,
                                    "ssfx_bloom_downsample" ); // SSS Bloom
    s_ssfx_bloom_upsample.create( b_ssfx_bloom_upsample,
                                  "ssfx_bloom_upsample" );   // SSS Bloom
    ...
    s_ssfx_volumetric_blur.create( b_ssfx_volumetric_blur,
                                   "ssfx_volumetric_blur" ); // Volumetric Blur
    ...
    if ( RImplementation.o.HW_smap ) {
        ...
        if ( RImplementation.o.dx10_msaa ) {
            ...
            for ( int i = 0; i < bound; ++i ) {
                s_accum_direct_msaa[ i ].create( b_accum_direct_msaa[ i ],
                                                 "r3\\accum_direct" );
                s_accum_mask_msaa[ i ].create( b_accum_mask_msaa[ i ],
                                               "r3\\accum_direct" );
            }
        }
        if ( RImplementation.o.advancedpp ) {
            ...
            if ( RImplementation.o.dx10_msaa ) {
                ...
                for ( int i = 0; i < bound; ++i ) {
                    // s_accum_direct_volumetric_msaa[i].create
                    // (b_accum_direct_volumetric_sun_msaa[i],
                    // "r3\\accum_direct");
    ...
    //	Or make DX10 rain switch dynamic?
    {
        ...
        if ( RImplementation.o.dx10_msaa ) {
            ...
            for ( int i = 0; i < bound; ++i ) {
                ...
                s_accum_spot_msaa[ i ].create( b_accum_spot_msaa[ i ],
                                               "r2\\accum_spot_s",
                                               "lights\\lights_spot01" );
                s_accum_point_msaa[ i ].create( b_accum_point_msaa[ i ],
                                                "r2\\accum_point_s" );
                // s_accum_volume_msaa[i].create(b_accum_direct_volumetric_msaa[i],
                // "lights\\lights_spot01");
                s_accum_volume_msaa[ i ].create( b_accum_volumetric_msaa[ i ],
                                                 "lights\\lights_spot01" );
                s_combine_msaa[ i ].create( b_combine_msaa[ i ],
                                            "r2\\combine" );
            }
        }
    }
    ...
    // SPOT
    {
        s_accum_spot.create( b_accum_spot, "r2\\accum_spot_s",
                             "lights\\lights_spot01" );
    ...
    // REFLECTED
    {
        ...
        if ( RImplementation.o.dx10_msaa ) {
            ...
            for ( int i = 0; i < bound; ++i ) {
                s_accum_reflected_msaa[ i ].create( b_accum_reflected_msaa[ i ],
                                                    "null" );
            }
        }
    }
