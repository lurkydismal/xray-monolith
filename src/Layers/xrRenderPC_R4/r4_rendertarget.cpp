...
void CRenderTarget::set_viewport_size( ID3DDeviceContext* dev,
                                       float w,
                                       float h ) {
...
void CRenderTarget::u_setrt( const ref_rt& _1,
                             const ref_rt& _2,
                             const ref_rt& _3,
                             const ref_rt& _4,
...
void CRenderTarget::u_setrt( u32 W,
                             u32 H,
...
void CRenderTarget::u_stencil_optimize( eStencilOptimizeMode eSOM ) {
    ...
    float _dh = 0.5f;
    FVF::TL* pv =
        ( FVF::TL* )RCache.Vertex.Lock( 4, g_combine->vb_stride, Offset );
...
void CRenderTarget::u_compute_texgen_screen( Fmatrix& m_Texgen ) {
    // float	_w						=
    // float(Device.dwWidth); float	_h
    // = float(Device.dwHeight); float	o_w
    // = (.5f / _w); float	o_h
    // = (.5f / _h);
...
void generate_jitter( DWORD* dest, u32 elem_count ) {
    ...
    while ( samples.size() < elem_count * 2 ) {
        ...
        for ( u32 t = 0; t < samples.size(); t++ ) {
            int dist = _abs( test.x - samples[ t ].x ) +
                       _abs( test.y - samples[ t ].y );
            if ( dist < 32 ) {
                valid = FALSE;
                break;
            }
    ...
    for ( u32 it = 0; it < elem_count; it++, dest++ )
        *dest = color_rgba( samples[ 2 * it ].x, samples[ 2 * it ].y,
                            samples[ 2 * it + 1 ].y, samples[ 2 * it + 1 ].x );
}
...
CRenderTarget::CRenderTarget() {
    ...
    b_hdr10_lens_flare_downsample =
        xr_new< CBlender_hdr10_lens_flare_downsample >();
    ...
    b_hdr10_lens_flare_upsample =
        xr_new< CBlender_hdr10_lens_flare_upsample >();
    ...
    b_ssfx_volumetric_blur =
        xr_new< CBlender_ssfx_volumetric_blur >(); // Volumetric Blur
    ...
    if ( RImplementation.o.dx10_msaa ) {
        ...
        for ( int i = 0; i < bound; ++i ) {
            static LPCSTR SampleDefs[] = { "0", "1", "2", "3",
                                           "4", "5", "6", "7" };
            b_combine_msaa[ i ] = xr_new< CBlender_combine_msaa >();
            b_accum_mask_msaa[ i ] =
                xr_new< CBlender_accum_direct_mask_msaa >();
            b_accum_direct_msaa[ i ] = xr_new< CBlender_accum_direct_msaa >();
            b_accum_direct_volumetric_msaa[ i ] =
                xr_new< CBlender_accum_direct_volumetric_msaa >();
            // b_accum_direct_volumetric_sun_msaa[i]	=
            // xr_new<CBlender_accum_direct_volumetric_sun_msaa>
            // ();
            b_accum_spot_msaa[ i ] = xr_new< CBlender_accum_spot_msaa >();
            b_accum_volumetric_msaa[ i ] =
                xr_new< CBlender_accum_volumetric_msaa >();
            b_accum_point_msaa[ i ] = xr_new< CBlender_accum_point_msaa >();
            b_accum_reflected_msaa[ i ] =
                xr_new< CBlender_accum_reflected_msaa >();
            b_ssao_msaa[ i ] = xr_new< CBlender_SSAO_MSAA >();
            static_cast< CBlender_accum_direct_mask_msaa* >(
                b_accum_mask_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            static_cast< CBlender_accum_direct_volumetric_msaa* >(
                b_accum_direct_volumetric_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            // static_cast<CBlender_accum_direct_volumetric_sun_msaa*>(b_accum_direct_volumetric_sun_msaa[i])->SetDefine(
            // "ISAMPLE", SampleDefs[i]);
            static_cast< CBlender_accum_direct_msaa* >(
                b_accum_direct_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            static_cast< CBlender_accum_volumetric_msaa* >(
                b_accum_volumetric_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            static_cast< CBlender_accum_spot_msaa* >( b_accum_spot_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            static_cast< CBlender_accum_point_msaa* >( b_accum_point_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            static_cast< CBlender_accum_reflected_msaa* >(
                b_accum_reflected_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            static_cast< CBlender_combine_msaa* >( b_combine_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
            static_cast< CBlender_SSAO_MSAA* >( b_ssao_msaa[ i ] )
                ->SetDefine( "ISAMPLE", SampleDefs[ i ] );
        }
    }
    //	NORMAL
    {
        ...
        if ( RImplementation.o.dx10_msaa ) {
            rt_MSAADepth.create( r2_RT_MSAAdepth, w, h, D3DFMT_D24S8,
                                 SampleCount );
        }

        rt_tempzb.create(
            "$user$temp_zb", w, h,
            D3DFMT_D24S8 ); // Redotix99: for 3D Shader Based Scopes

        // select albedo & accum
        if ( RImplementation.o.mrtmixdepth ) {
            // NV50
            if ( RImplementation.o.dx11_hdr10 ) {
                rt_Color.create( r2_RT_albedo, w, h, D3DFMT_A16B16G16R16F,
                                 SampleCount );
            } else {
            ...
            if ( RImplementation.o.fp16_blend ) {
                ...
                rt_Accumulator.create( r2_RT_accum, w, h, D3DFMT_A16B16G16R16F,
                                       SampleCount );
                rt_Accumulator_temp.create( r2_RT_accum_temp, w, h,
                                            D3DFMT_A16B16G16R16F, SampleCount );
            }
        }

        // generic(LDR) RTs
        // LV - we should change their formats into D3DFMT_A16B16G16R16F for
        // better HDR support.
        if ( RImplementation.o.dx11_hdr10 ) {
        ...
        rt_fakescope.create( r2_RT_scopert, w, h, D3DFMT_A8R8G8B8,
                             1 ); // crookr fakescope
        ...
        if ( RImplementation.o.dx11_hdr10 ) {
            rt_Generic_temp.create(
                "$user$generic_temp", w, h, D3DFMT_A16B16G16R16F,
                RImplementation.o.dx10_msaa ? SampleCount : 1 );
        } else {
            rt_Generic_temp.create(
                "$user$generic_temp", w, h, D3DFMT_A8R8G8B8,
                RImplementation.o.dx10_msaa ? SampleCount : 1 );
        }

        rt_dof.create( r2_RT_dof, w, h,
                       RImplementation.o.dx11_hdr10 ? D3DFMT_A16B16G16R16F
                                                    : D3DFMT_A8R8G8B8 );
        ...
        // TODO: R11G11B10F? needs another horrible hack + cast + update to
        // converter function
        if ( RImplementation.o.dx11_hdr10 ) {
            rt_HDR10_HalfRes[ 0 ].create( r4_RT_HDR10_halfres0, w / 2, h / 2,
                                          D3DFMT_A16B16G16R16F );
            rt_HDR10_HalfRes[ 1 ].create( r4_RT_HDR10_halfres1, w / 2, h / 2,
                                          D3DFMT_A16B16G16R16F );
        }
        ...
        // RT Blur
        rt_blur_h_2.create( r2_RT_blur_h_2, u32( w / 2 ), u32( h / 2 ),
                            D3DFMT_A8R8G8B8 );
        rt_blur_2.create( r2_RT_blur_2, u32( w / 2 ), u32( h / 2 ),
                          D3DFMT_A8R8G8B8 );

        rt_blur_h_4.create( r2_RT_blur_h_4, u32( w / 4 ), u32( h / 4 ),
                            D3DFMT_A8R8G8B8 );
        rt_blur_4.create( r2_RT_blur_4, u32( w / 4 ), u32( h / 4 ),
                          D3DFMT_A8R8G8B8 );

        rt_blur_h_8.create( r2_RT_blur_h_8, u32( w / 8 ), u32( h / 8 ),
                            D3DFMT_A8R8G8B8 );
        rt_blur_8.create( r2_RT_blur_8, u32( w / 8 ), u32( h / 8 ),
                          D3DFMT_A8R8G8B8 );

        rt_pp_bloom.create( r2_RT_pp_bloom, w, h, D3DFMT_A8R8G8B8 );

        // Screen Space Shaders Stuff
        rt_ssfx_taa.create( r2_RT_ssfx_taa, w, h, D3DFMT_A16B16G16R16F,
                            SampleCount ); // Temp RT

        if ( RImplementation.o.dx11_hdr10 )
            rt_ssfx_prev_frame.create( r2_RT_ssfx_prev_frame, w, h,
                                       D3DFMT_A16B16G16R16F ); // Temp RT
        else
            rt_ssfx_prev_frame.create( r2_RT_ssfx_prev_frame, w, h,
                                       D3DFMT_A8R8G8B8 ); // Temp RT

        rt_ssfx_motion_vectors.create(
            r2_RT_ssfx_motion_vectors, w, h, D3DFMT_A16B16G16R16F,
            SampleCount ); // HUD mask & Velocity buffer

        rt_ssfx.create( r2_RT_ssfx, w, h, D3DFMT_A8R8G8B8 ); // Temp RT
        rt_ssfx_temp.create( r2_RT_ssfx_temp, w, h,
                             D3DFMT_A8R8G8B8 ); // Temp RT
        rt_ssfx_temp2.create( r2_RT_ssfx_temp2, w, h,
                              D3DFMT_A8R8G8B8 ); // Temp RT
        rt_ssfx_temp3.create( r2_RT_ssfx_temp3, w, h,
                              D3DFMT_A8R8G8B8 ); // Temp RT

        rt_ssfx_accum.create( r2_RT_ssfx_accum, w, h, D3DFMT_A16B16G16R16F,
                              SampleCount ); // Volumetric Acc
        rt_ssfx_ssr.create( r2_RT_ssfx_ssr, w, h, D3DFMT_A8R8G8B8 ); // SSR Acc
        rt_ssfx_water.create( r2_RT_ssfx_water, w, h,
                              D3DFMT_A8R8G8B8 );                   // Water Acc
        ...
        if ( RImplementation.o.ssfx_sss ) {
            rt_ssfx_sss.create( r2_RT_ssfx_sss, w, h,
                                D3DFMT_A8R8G8B8 ); // SSS Acc
            rt_ssfx_sss_ext.create( r2_RT_ssfx_sss_ext, w, h,
                                    D3DFMT_A8R8G8B8 ); // SSS EXT Acc
            rt_ssfx_sss_ext2.create( r2_RT_ssfx_sss_ext2, w, h,
                                     D3DFMT_A8R8G8B8 ); // SSS EXT Acc
            rt_ssfx_sss_tmp.create( r2_RT_ssfx_sss_tmp, w, h,
                                    D3DFMT_A8R8G8B8 ); // SSS EXT Acc
        }

        if ( RImplementation.o.ssfx_bloom ) {
            rt_ssfx_bloom1.create( r2_RT_ssfx_bloom1, w / 2.0f, h / 2.0f,
                                   D3DFMT_A16B16G16R16F ); // Bloom
            rt_ssfx_bloom_emissive.create( r2_RT_ssfx_bloom_emissive, w, h,
                                           D3DFMT_A8R8G8B8,
                                           SampleCount ); // Emissive
            rt_ssfx_bloom_lens.create( r2_RT_ssfx_bloom_lens, w / 4.0f,
                                       h / 4.0f, D3DFMT_A8R8G8B8 ); // Lens

            rt_ssfx_bloom_tmp2.create( r2_RT_ssfx_bloom_tmp2, w / 2.0f,
                                       h / 2.0f,
                                       D3DFMT_A16B16G16R16F ); // Bloom / 2
            rt_ssfx_bloom_tmp4.create( r2_RT_ssfx_bloom_tmp4, w / 4.0f,
                                       h / 4.0f,
                                       D3DFMT_A16B16G16R16F ); // Bloom / 4
            rt_ssfx_bloom_tmp8.create( r2_RT_ssfx_bloom_tmp8, w / 8.0f,
                                       h / 8.0f,
                                       D3DFMT_A16B16G16R16F ); // Bloom / 8
            rt_ssfx_bloom_tmp16.create( r2_RT_ssfx_bloom_tmp16, w / 16.0f,
                                        h / 16.0f,
                                        D3DFMT_A16B16G16R16F ); // Bloom / 16
            rt_ssfx_bloom_tmp32.create( r2_RT_ssfx_bloom_tmp32, w / 32.0f,
                                        h / 32.0f,
                                        D3DFMT_A16B16G16R16F ); // Bloom / 32
            rt_ssfx_bloom_tmp64.create( r2_RT_ssfx_bloom_tmp64, w / 64.0f,
                                        h / 64.0f,
                                        D3DFMT_A16B16G16R16F ); // Bloom / 64

            rt_ssfx_bloom_tmp32_2.create( r2_RT_ssfx_bloom_tmp32_2, w / 32.0f,
                                          h / 32.0f,
                                          D3DFMT_A16B16G16R16F ); // Bloom / 32
            rt_ssfx_bloom_tmp16_2.create( r2_RT_ssfx_bloom_tmp16_2, w / 16.0f,
                                          h / 16.0f,
                                          D3DFMT_A16B16G16R16F ); // Bloom / 16
            rt_ssfx_bloom_tmp8_2.create( r2_RT_ssfx_bloom_tmp8_2, w / 8.0f,
                                         h / 8.0f,
                                         D3DFMT_A16B16G16R16F ); // Bloom / 8
            rt_ssfx_bloom_tmp4_2.create( r2_RT_ssfx_bloom_tmp4_2, w / 4.0f,
                                         h / 4.0f,
                                         D3DFMT_A16B16G16R16F ); // Bloom / 4
        }

        rt_ssfx_volumetric.create( r2_RT_ssfx_volumetric, w / 8.0f, h / 8.0f,
                                   D3DFMT_A16B16G16R16F ); // Volumetric
        rt_ssfx_volumetric_tmp.create( r2_RT_ssfx_volumetric_tmp, w / 8.0f,
                                       h / 8.0f,
                                       D3DFMT_A16B16G16R16F ); // Volumetric
        rt_ssfx_rain.create( r2_RT_ssfx_rain, w / 8.0f, h / 8.0f,
                             D3DFMT_A8R8G8B8 ); // Rain refraction buffer
        rt_ssfx_water_waves.create( r2_RT_ssfx_water_waves, 512, 512,
                                    D3DFMT_A8R8G8B8 ); // Water Waves

        rt_ssfx_prevPos.create( r2_RT_ssfx_prevPos, w, h, D3DFMT_A16B16G16R16F,
                                SampleCount );

        // rt_ssfx_hud.create(r2_RT_ssfx_hud, w, h, D3DFMT_A16B16G16R16F); //
        // Deprecated

        if ( RImplementation.o.dx10_msaa ) {
            rt_Generic_0_r.create(
                r2_RT_generic0_r, w, h,
                ps_r4_hdr10_on ? D3DFMT_A16B16G16R16F : D3DFMT_A8R8G8B8,
                SampleCount );
            rt_Generic_1_r.create(
                r2_RT_generic1_r, w, h,
                ps_r4_hdr10_on ? D3DFMT_A16B16G16R16F : D3DFMT_A8R8G8B8,
                SampleCount );
            // rt_Generic.create		      (r2_RT_generic,w,h,
            // D3DFMT_A8R8G8B8, 1		);
        }
    ...
    startup_shader_tasks.run( [ this ]() {
        s_hdr10_bloom_downsample.create_parallel( b_hdr10_bloom_downsample,
                                                  "hdr10_bloom_downsample" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_hdr10_bloom_blur.create_parallel( b_hdr10_bloom_blur,
                                            "hdr10_bloom_blur" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_hdr10_bloom_upsample.create_parallel( b_hdr10_bloom_upsample,
                                                "hdr10_bloom_upsample" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_hdr10_lens_flare_downsample.create_parallel(
            b_hdr10_lens_flare_downsample, "hdr10_lens_flare_downsample" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_hdr10_lens_flare_fgen.create_parallel( b_hdr10_lens_flare_fgen,
                                                 "hdr10_lens_flare_fgen" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_hdr10_lens_flare_blur.create_parallel( b_hdr10_lens_flare_blur,
                                                 "hdr10_lens_flare_blur" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_hdr10_lens_flare_upsample.create_parallel(
            b_hdr10_lens_flare_upsample, "hdr10_lens_flare_upsample" );
    } );

    startup_shader_tasks.run( [ this ]() {
        s_sunshafts.create_parallel( b_sunshafts, "r2\\sunshafts" );
    } );
    startup_shader_tasks.run(
        [ this ]() { s_blur.create_parallel( b_blur, "r2\\blur" ); } );
    startup_shader_tasks.run( [ this ]() {
        s_pp_bloom.create_parallel( b_pp_bloom, "r2\\pp_bloom" );
    } );
    startup_shader_tasks.run(
        [ this ]() { s_dof.create_parallel( b_dof, "r2\\dof" ); } );
    startup_shader_tasks.run( [ this ]() {
    ...
    startup_shader_tasks.run( [ this ]() {
        s_nightvision.create_parallel( b_nightvision, "r2\\nightvision" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_fakescope.create_parallel( b_fakescope, "r2\\fakescope" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_heatvision.create_parallel( b_heatvision, "r2\\heatvision" );
    } );
    startup_shader_tasks.run(
        [ this ]() { s_lut.create_parallel( b_lut, "r2\\lut" ); } );
    startup_shader_tasks.run(
        [ this ]() { s_occq.create_parallel( b_occq, "r2\\occq" ); } );

    startup_shader_tasks.run( [ this ]() {
        s_ssfx_fog_scattering.create_parallel( b_ssfx_fog_scattering,
                                               "ssfx_fog_scattering" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_motion_blur.create_parallel( b_ssfx_motion_blur,
                                            "ssfx_motion_blur" );
    } );
    startup_shader_tasks.run(
        [ this ]() { s_ssfx_taa.create_parallel( b_ssfx_taa, "ssfx_taa" ); } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_rain.create_parallel( b_ssfx_rain, "ssfx_rain" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_bloom.create_parallel( b_ssfx_bloom, "ssfx_bloom" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_bloom_lens.create_parallel( b_ssfx_bloom_lens,
                                           "ssfx_bloom_flares" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_bloom_downsample.create_parallel( b_ssfx_bloom_downsample,
                                                 "ssfx_bloom_downsample" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_bloom_upsample.create_parallel( b_ssfx_bloom_upsample,
                                               "ssfx_bloom_upsample" );
    } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_sss_ext.create_parallel( b_ssfx_sss_ext, "ssfx_sss_ext" );
    } );
    startup_shader_tasks.run(
        [ this ]() { s_ssfx_sss.create_parallel( b_ssfx_sss, "ssfx_sss" ); } );
    startup_shader_tasks.run(
        [ this ]() { s_ssfx_ssr.create_parallel( b_ssfx_ssr, "ssfx_ssr" ); } );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_volumetric_blur.create_parallel( b_ssfx_volumetric_blur,
                                                "ssfx_volumetric_blur" );
    } );

    s_ssfx_water_ssr.create( "ssfx_water_ssr" );
    s_ssfx_water.create( "ssfx_water" );
    startup_shader_tasks.run( [ this ]() {
        s_ssfx_water_blur.create_parallel( b_ssfx_water_blur,
                                           "ssfx_water_blur" );
    } );
    startup_shader_tasks.run(
        [ this ]() { s_ssfx_ao.create_parallel( b_ssfx_ao, "ssfx_ao" ); } );
    // SSS 23: Deprecated
    ...
    if ( RImplementation.o.HW_smap ) {
        ...
        if ( RImplementation.o.dx10_minmax_sm ) {
            rt_smap_depth_minmax.create( r2_RT_smap_depth_minmax, size / 4,
                                         size / 4, D3DFMT_R32F );
            startup_shader_tasks.run( [ this ]() {
        ...
        // rt_smap_surf.create			(r2_RT_smap_surf,
        // size,size,nullrt		); rt_smap_ZB
        // = NULL;
        startup_shader_tasks.run( [ this ]() {
            s_accum_mask.create_parallel( b_accum_mask, "r3\\accum_mask" );
        } );
        startup_shader_tasks.run( [ this ]() {
            s_accum_direct.create_parallel( b_accum_direct,
                                            "r3\\accum_direct" );
        } );
        if ( RImplementation.o.dx10_msaa ) {
            const int bound = RImplementation.o.dx10_msaa_opt
                                  ? 1
                                  : RImplementation.o.dx10_msaa_samples;
            for ( int i = 0; i < bound; ++i ) {
                startup_shader_tasks.run( [ this, i ]() {
                    s_accum_direct_msaa[ i ].create_parallel(
                        b_accum_direct_msaa[ i ], "r3\\accum_direct" );
                } );
                startup_shader_tasks.run( [ this, i ]() {
                    s_accum_mask_msaa[ i ].create_parallel(
                        b_accum_mask_msaa[ i ], "r3\\accum_direct" );
                } );
            }
        }
        if ( RImplementation.o.advancedpp ) {
            ...
            if ( RImplementation.o.dx10_minmax_sm )
                s_accum_direct_volumetric_minmax.create(
                    "accum_volumetric_sun_nomsaa_minmax" );
            if ( RImplementation.o.dx10_msaa ) {
                const int bound = RImplementation.o.dx10_msaa_opt
                                      ? 1
                                      : RImplementation.o.dx10_msaa_samples;
                for ( int i = 0; i < bound; ++i ) {
                    static LPCSTR names[] = { "accum_volumetric_sun_msaa0",
        ...
        // u32	size =RImplementation.o.smapsize	; rt_smap_surf.create
        // (r2_RT_smap_surf,			size,size,D3DFMT_R32F);
        // rt_smap_depth				= NULL;
        // R_CHK
        // (HW.pDevice->CreateDepthStencilSurface
        // (size,size,D3DFMT_D24X8,D3DMULTISAMPLE_NONE,0,TRUE,&rt_smap_ZB,NULL));
        // s_accum_mask.create			(b_accum_mask,
        // "r2\\accum_mask"); s_accum_direct.create		(b_accum_direct,
        // "r2\\accum_direct"); if (RImplementation.o.advancedpp)
    ...
    if ( RImplementation.o.dx10_msaa ) {
        ...
        for ( int i = 0; i < bound; ++i ) {
            startup_shader_tasks.run( [ this, i ]() {
                static LPCSTR sampleDefinitions[] = { "0", "1", "2", "3",
                                                      "4", "5", "6", "7" };
                ...
                s_accum_spot_msaa[ i ].create_parallel(
                    b_accum_spot_msaa[ i ], "r2\\accum_spot_s",
                    "lights\\lights_spot01" );
                s_accum_point_msaa[ i ].create_parallel(
                    b_accum_point_msaa[ i ], "r2\\accum_point_s" );
                s_accum_volume_msaa[ i ].create_parallel(
                    b_accum_volumetric_msaa[ i ], "lights\\lights_spot01" );
                s_combine_msaa[ i ].create_parallel( b_combine_msaa[ i ],
                                                     "r2\\combine" );
            } );
        }
    }
    ...
    {
        startup_shader_tasks.run( [ this ]() {
            s_accum_point.create_parallel( b_accum_point, "r2\\accum_point_s" );
        } );
        accum_point_geom_create();
        g_accum_point.create( D3DFVF_XYZ, g_accum_point_vb, g_accum_point_ib );
        accum_omnip_geom_create();
        g_accum_omnipart.create( D3DFVF_XYZ, g_accum_omnip_vb,
                                 g_accum_omnip_ib );
    }
    ...
    {
        startup_shader_tasks.run( [ this ]() {
            s_accum_spot.create_parallel( b_accum_spot, "r2\\accum_spot_s",
                                          "lights\\lights_spot01" );
        } );
...

    {
        ...
        g_accum_volumetric.create( D3DFVF_XYZ, g_accum_volumetric_vb,
                                   g_accum_volumetric_ib );
    }

    // REFLECTED
    {
        startup_shader_tasks.run( [ this ]() {
            s_accum_reflected.create_parallel( b_accum_reflected,
                                               "r2\\accum_refl" );
        } );
        if ( RImplementation.o.dx10_msaa ) {
            const int bound = RImplementation.o.dx10_msaa_opt
            ...
            for ( int i = 0; i < bound; ++i )
                startup_shader_tasks.run( [ this, i ]() {
                    s_accum_reflected_msaa[ i ].create_parallel(
                        b_accum_reflected_msaa[ i ], "null" );
                } );
        }
    }
    ...
    // BLOOM
    {
        ...
        g_bloom_build.create( fvf_build, RCache.Vertex.Buffer(),
                              RCache.QuadIB );
        g_bloom_filter.create( fvf_filter, RCache.Vertex.Buffer(),
                               RCache.QuadIB );
        s_bloom_dbg_1.create( "effects\\screen_set", r2_RT_bloom1 );
        s_bloom_dbg_2.create( "effects\\screen_set", r2_RT_bloom2 );
        startup_shader_tasks.run(
            [ this ]() { s_bloom.create_parallel( b_bloom, "r2\\bloom" ); } );
        if ( RImplementation.o.dx10_msaa ) {
            startup_shader_tasks.run( [ this ]() {
                s_bloom_msaa.create_parallel( b_bloom_msaa, "r2\\bloom" );
            } );
            startup_shader_tasks.run( [ this ]() {
                s_postprocess_msaa.create_parallel( b_postprocess_msaa,
                                                    "r2\\post" );
            } );
        }
        f_bloom_factor = 0.5f;
    ...
    // SMAA
    {
        ...
        startup_shader_tasks.run(
            [ this ]() { s_smaa.create_parallel( b_smaa, "r3\\smaa" ); } );
    }
    ...
    // TONEMAP
    {
        ...
        startup_shader_tasks.run( [ this ]() {
            s_luminance.create_parallel( b_luminance, "r2\\luminance" );
        } );
        ...
        for ( u32 it = 0; it < 2; it++ ) {
            shared_str name;
            name.printf( "%s_%d", r2_RT_luminance_pool, it );
            rt_LUM_pool[ it ].create( name.c_str(), 1, 1, D3DFMT_R32F );

            FLOAT ColorRGBA[ 4 ] = { 127.0f / 255.0f, 127.0f / 255.0f,
                                     127.0f / 255.0f, 127.0f / 255.0f };
            HW.pContext->ClearRenderTargetView( rt_LUM_pool[ it ]->pRT,
                                                ColorRGBA );
        }
    ...
    if ( RImplementation.o.ssao_opt_data ) {
        ...
        startup_shader_tasks.run(
            [ this ]() { s_ssao.create_parallel( b_ssao, "r2\\ssao" ); } );
    }
    ...
    // COMBINE
    {
        static D3DVERTEXELEMENT9 dwDecl[] = {
            { 0, 0, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT,
              D3DDECLUSAGE_POSITION, 0 }, // pos+uv
            D3DDECL_END() };
        startup_shader_tasks.run( [ this ]() {
            s_combine.create_parallel( b_combine, "r2\\combine" );
        } );
        ...
        g_combine_2UV.create( FVF::F_TL2uv, RCache.Vertex.Buffer(),
                              RCache.QuadIB );
        g_combine_cuboid.create( dwDecl, RCache.Vertex.Buffer(),
                                 RCache.Index.Buffer() );
        ...
        u32 fvf_aa_AA = D3DFVF_XYZRHW | D3DFVF_TEX7 |
                        D3DFVF_TEXCOORDSIZE2( 0 ) | D3DFVF_TEXCOORDSIZE2( 1 ) |
                        D3DFVF_TEXCOORDSIZE2( 2 ) | D3DFVF_TEXCOORDSIZE2( 3 ) |
    ...
    // Build textures
    {
        ...
        // Build material(s)
        {
            ...
            // t_material					=
            // dxRenderDeviceRender::Instance().Resources->_CreateTexture(r2_material);
            // t_material->surface_set		(t_material_surf);
            //	Use DXGI_FORMAT_R8G8_UNORM

            u16 tempData[ TEX_material_LdotN * TEX_material_LdotH *
                          TEX_material_Count ];
                    ...
                    for ( u32 x = 0; x < TEX_material_LdotN; x++ ) {
                        ...
                        switch ( slice ) {
                            ...
                            case 3: {
                                // looks like Metal
                                float s0 =
                                    _abs( 1 - _abs( 0.05f * _sin( 33.f * ld ) +
                                                    ld - ls ) );
                                float s1 = _abs(
                                    1 - _abs( 0.05f * _cos( 33.f * ld * ls ) +
                                              ld - ls ) );
                        ...
                        if ( ( y == ( TEX_material_LdotH - 1 ) ) &&
                             ( x == ( TEX_material_LdotN - 1 ) ) ) {
                            _d = 255;
                            _s = 255;
                        }
                        *p = u16( _s * 256 + _d );
            ...
            R_CHK( HW.pDevice->CreateTexture3D( &desc, &subData,
                                                &t_material_surf ) );
            t_material =
            ...
            // #ifdef DEBUG
            // R_CHK	(D3DXSaveTextureToFile
            // ("x:\\r2_material.dds",D3DXIFF_DDS,t_material_surf,0)); #endif
        }
        ...
        if ( 1 ) {
            ...
            for ( u32 y = 0; y < TEX_jitter; y++ ) {
                for ( u32 x = 0; x < TEX_jitter; x++ ) {
                    ...
                    for ( u32 it = 0; it < TEX_jitter_count - 1; it++ ) {
                        u32* p =
                            ( u32* )( LPBYTE( subData[ it ].pSysMem ) +
                                      y * subData[ it ].SysMemPitch + x * 4 );
            ...
            for ( int it = 0; it < TEX_jitter_count - 1; it++ ) {
                ...
                // (HW.pDevice,TEX_jitter,TEX_jitter,1,0,D3DFMT_Q8W8V8U8,D3DPOOL_MANAGED,&t_noise_surf[it]));
                R_CHK( HW.pDevice->CreateTexture2D( &desc, &subData[ it ],
                                                    &t_noise_surf[ it ] ) );
                t_noise[ it ] =
                    dxRenderDeviceRender::Instance().Resources->_CreateTexture(
                        name );
            ...
            subData[ it ].SysMemPitch =
                descHBAO.Width * sampleSize * sizeof( float );
            ...
            for ( u32 y = 0; y < TEX_jitter; y++ ) {
                for ( u32 x = 0; x < TEX_jitter; x++ ) {
                    ...
                    float angle =
                        2 * PI * ::Random.randF( 0.0f, 1.0f ) / numDir;
                    float dist = ::Random.randF( 0.0f, 1.0f );

                    float* p = ( float* )( LPBYTE( subData[ it ].pSysMem ) +
            ...
            // R_CHK	(D3DXCreateTexture
            // (HW.pDevice,TEX_jitter,TEX_jitter,1,0,D3DFMT_Q8W8V8U8,D3DPOOL_MANAGED,&t_noise_surf[it]));
            R_CHK( HW.pDevice->CreateTexture2D( &descHBAO, &subData[ it ],
                                                &t_noise_surf[ it ] ) );
            t_noise[ it ] =
                dxRenderDeviceRender::Instance().Resources->_CreateTexture(
                    name );
            ...
            //	Create noise mipped
            {
                //	Autogen mipmaps
                desc.MipLevels = 0;
                R_CHK( HW.pDevice->CreateTexture2D( &desc, 0,
                                                    &t_noise_surf_mipped ) );
                t_noise_mipped =
                    dxRenderDeviceRender::Instance().Resources->_CreateTexture(
                        r2_jitter_mipped );
                ...
                HW.pContext->CopySubresourceRegion(
                    t_noise_surf_mipped, 0, 0, 0, 0, t_noise_surf[ 0 ], 0, 0 );

                D3DX11FilterTexture( HW.pContext, t_noise_surf_mipped, 0,
                                     D3DX10_FILTER_POINT );
            }
        }
    }
    ...
    const u32 overlappedWorkMs =
        startupTimer.GetElapsed_ms() - initialTargetsMs;
    startup_shader_tasks.wait();
    const u32 shaderWaitMs =
        startupTimer.GetElapsed_ms() - initialTargetsMs - overlappedWorkMs;

    //
    dwWidth = Device.dwWidth;
    dwHeight = Device.dwHeight;
    Msg( "* [STARTUP/RENDER TARGET] initial=%u overlap=%u shader-wait=%u "
         "total=%u ms",
         initialTargetsMs, overlappedWorkMs, shaderWaitMs,
         startupTimer.GetElapsed_ms() );
}
...
bool CRenderTarget::use_minmax_sm_this_frame() {
    switch ( RImplementation.o.dx10_minmax_sm ) {
        ...
        case CRender::MMSM_AUTODETECT: {
            u32 dwScreenArea = HW.m_ChainDesc.Width * HW.m_ChainDesc.Height;

            if ( ( dwScreenArea >=
                   RImplementation.o.dx10_minmax_sm_screenarea_threshold ) )
                return need_to_render_sunshafts();
            else
                return false;
        }
