...
void CRenderTarget::phase_smap_spot_clear() {
    ...
    */
    ...
    HW.pContext->ClearDepthStencilView( rt_smap_depth->pZRT, D3D_CLEAR_DEPTH,
                                        1.0f, 0L );
}
...
void CRenderTarget::phase_smap_spot( light* L ) {
    ...
    D3D_VIEWPORT VP = { ( float )L->X.S.posX,
                        ( float )L->X.S.posY,
                        ( float )L->X.S.size,
                        ( float )L->X.S.size,
                        0,
...
#pragma todo( "can optimize for multi-lights covering more than say 50%..." )
    ...
    // HW.pContext->ClearDepthStencilView( rt_smap_depth->pZRT,
    // D3D_CLEAR_DEPTH, 1.0f, 0L);
}
