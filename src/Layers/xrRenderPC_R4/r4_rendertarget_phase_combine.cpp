...
void CRenderTarget::DoAsyncScreenshot() {
    ...
    if ( RImplementation.m_bMakeAsyncSS ) {
        ...
        hr = HW.m_pSwapChain->GetBuffer( 0, __uuidof( ID3D10Texture2D ),
                                         ( LPVOID* )&pBuffer );
...
void CRenderTarget::phase_combine() {
    ...
    //*** exposure-pipeline
    if ( Device.m_SecondViewport
             .IsSVPActive() ) //--#SM+#-- +SecondVP+ Fix for screen flickering
    {
    ...
    if ( !RImplementation.o.dx10_msaa ) {
        ...
        u_setrt( rt_Generic_0, rt_Generic_1, rt_Heat,
                 HW.pBaseZB ); //--DSR-- HeatVision
    } else {
        ...
        u_setrt(
            rt_Generic_0_r, rt_Generic_1_r, rt_Heat,
            RImplementation.Target->rt_MSAADepth->pZRT ); //--DSR-- HeatVision
    }
    ...
    RCache.set_Stencil( TRUE, D3DCMP_LESSEQUAL, 0x01, 0xff,
                        0x00 ); // stencil should be >= 1
    ...
    if ( !_menu_pp ) {
        PIX_EVENT( combine_1 );
        // Compute params
        CEnvDescriptorMixer& envdesc =
            *g_pGamePersistent->Environment().CurrentEnv;
        const float minamb = 0.001f;
        Fvector4 ambclr = { _max( envdesc.ambient.x * 2, minamb ),
                            _max( envdesc.ambient.y * 2, minamb ),
        ...
        Fvector4 fogclr = { envdesc.fog_color.x, envdesc.fog_color.y,
                            envdesc.fog_color.z, 0 };
        ...
        {
            light* fuckingsun =
                ( light* )RImplementation.Lights.sun_adapted._get();
            Fvector L_dir, L_clr;
            float L_spec;
            L_clr.set( fuckingsun->color.r, fuckingsun->color.g,
                       fuckingsun->color.b );
        ...
        RCache.Vertex.Lock	(4,g_combine_VP->vb_stride,Offset);
        //pv->set						(hclip(EPS,
        _w),	hclip(_h+EPS,	_h),	p0.x, p1.y);	pv++;
        //pv->set						(hclip(EPS,
        _w),	hclip(EPS,		_h),	p0.x, p0.y);	pv++;
        //pv->set						(hclip(_w+EPS,
        _w),	hclip(_h+EPS,	_h),	p1.x, p1.y);	pv++;
        //pv->set						(hclip(_w+EPS,
        _w),	hclip(EPS,		_h),	p1.x, p0.y);	pv++; pv->set
        (-1,	1,	0, 1);	pv++; pv->set
        (-1,	-1,	0, 0);	pv++; pv->set
        (1,		1,	1, 1);	pv++; pv->set
        (1,		-1,	1, 0);	pv++; RCache.Vertex.Unlock
        (4,g_combine_VP->vb_stride);
        */
        ...
        dxEnvDescriptorMixerRender& envdescren =
            *( dxEnvDescriptorMixerRender* )( &*envdesc.m_pDescriptorMixer );

        // Setup textures
        ID3DBaseTexture* e0 =
            _menu_pp ? 0
                     : envdescren.sky_r_textures_env[ 0 ].second->surface_get();
        ID3DBaseTexture* e1 =
            _menu_pp ? 0
                     : envdescren.sky_r_textures_env[ 1 ].second->surface_get();
        ...
        if ( !RImplementation.o.dx10_msaa )
            ...
            if ( RImplementation.o.dx10_msaa_opt ) {
                ...
                for ( u32 i = 0; i < RImplementation.o.dx10_msaa_samples;
                      ++i ) {
    ...
    // Copy previous rt
    if ( !RImplementation.o.dx10_msaa )
        HW.pContext->CopyResource( rt_Generic_temp->pTexture->surface_get(),
                                   rt_Generic_0->pTexture->surface_get() );
    else
        HW.pContext->CopyResource( rt_Generic_temp->pTexture->surface_get(),
                                   rt_Generic_0_r->pTexture->surface_get() );
    ...
    if ( RImplementation.o.ssfx_water &&
        ...
        // Render Scale
        set_viewport_size( HW.pContext, w / ps_ssfx_water.x,
                           h / ps_ssfx_water.x );
        ...
        // Save Frame
        HW.pContext->CopyResource( rt_ssfx_water->pTexture->surface_get(),
                                   rt_ssfx_temp->pTexture->surface_get() );
    ...
    // Final water rendering ( All the code above can be omitted if the Water
...

    {
        if ( RImplementation.o.ssfx_rain ) {
            phase_ssfx_rain(); // Render a small color buffer to do the
                               // refraction and more

            if ( !RImplementation.o.dx10_msaa )
                u_setrt( rt_Generic_0, 0, rt_ssfx_motion_vectors, HW.pBaseZB );
            else
                u_setrt( rt_Generic_0_r, 0, rt_ssfx_motion_vectors,
                         rt_MSAADepth->pZRT );
        }
    ...
    /*if (ssfx_PrevPos_Requiered)
            HW.pContext->CopyResource(rt_ssfx_prevPos->pTexture->surface_get(),
    ...
    if ( RImplementation.o.ssfx_glass ) {
        if ( !RImplementation.o.dx10_msaa )
            ...
            HW.pContext->CopyResource(
                rt_Generic_temp->pTexture->surface_get(),
                rt_Generic_0_r->pTexture->surface_get() );
    }
    ...
    // Forward rendering
    {
        ...
        if ( !RImplementation.o.dx10_msaa )
            u_setrt( rt_Generic_0, rt_Heat, rt_ssfx_motion_vectors,
                     HW.pBaseZB ); // LDR RT
        else
            u_setrt( rt_Generic_0_r, rt_Heat, rt_ssfx_motion_vectors,
                     RImplementation.Target->rt_MSAADepth->pZRT ); // LDR RT
        //--DSR-- HeatVision_end
    ...
    if ( RImplementation.o.dx10_msaa ) {
        ...
        HW.pContext->ResolveSubresource(
            rt_Generic_1->pTexture->surface_get(), 0,
            rt_Generic_1_r->pTexture->surface_get(), 0,
            DXGI_FORMAT_R8G8B8A8_UNORM );
        HW.pContext->ResolveSubresource(
            rt_Generic_0->pTexture->surface_get(), 0,
            rt_Generic_0_r->pTexture->surface_get(), 0,
            DXGI_FORMAT_R8G8B8A8_UNORM );
    }
    ...
    // Distortion filter
    BOOL bDistort =
        RImplementation.o.distortion_enabled; // This can be modified
    {
        if ( ( 0 == RImplementation.GMBase.RGraph.mapStaticSorted.Distort
                        .size() &&
               0 == RImplementation.GMBase.RGraph.mapDynamicSorted.Distort
                        .size() &&
               0 == RImplementation.GMBase.RGraph.mapHUDSorted.Distort
                        .size() ) &&
             !_menu_pp )
            bDistort = FALSE;
        if ( bDistort ) {
            PIX_EVENT( render_distort_objects );
            FLOAT ColorRGBA[ 4 ] = { 127.0f / 255.0f, 127.0f / 255.0f, 0.0f,
                                     127.0f / 255.0f };
            if ( !RImplementation.o.dx10_msaa ) {
                u_setrt( rt_Generic_1, 0, 0,
                         HW.pBaseZB ); // Now RT is a distortion mask
                HW.pContext->ClearRenderTargetView( rt_Generic_1->pRT,
                                                    ColorRGBA );
            } else {
                u_setrt( rt_Generic_1_r, 0, 0,
                         RImplementation.Target->rt_MSAADepth->pZRT );
                // Now RT is a distortion mask
                HW.pContext->ClearRenderTargetView( rt_Generic_1_r->pRT,
                                                    ColorRGBA );
            }
            RCache.set_CullMode( CULL_CCW );
            RCache.set_Stencil( FALSE );
            RCache.set_ColorWriteEnable();
            // CHK_DX(HW.pDevice->Clear	( 0L, NULL, D3DCLEAR_TARGET,
            // color_rgba(127,127,0,127), 1.0f, 0L));
    ...
    if ( !_menu_pp ) {
        if ( ps_sunshafts_mode == R2SS_SCREEN_SPACE ||
             ps_sunshafts_mode == R2SS_COMBINE_SUNSHAFTS )
            phase_sunshafts();
    }
    ...
    if ( !Device.m_SecondViewport
              .IsSVPFrame() ) // Temp fix for blur buffer and SVP
        phase_blur();
    ...
    if ( RImplementation.o.ssfx_bloom ) {
        if ( !Device.m_SecondViewport.IsSVPFrame() )
            ...
            HW.pContext->ClearRenderTargetView( rt_ssfx_bloom1->pRT,
                                                ColorRGBA );
    } else {
        phase_pp_bloom();
    }
    ...
    if ( 1 ) {
        ...
        if ( !RImplementation.o.dx10_msaa ) {
            ...
            // look at blender_combine.cpp
                ...
                RCache.set_Element(
                    s_combine->E[ bDistort ? 4 : 2 ] ); // look at
                                                        // blender_combine.cpp
        } else {
            if ( ps_r2_ls_flags.test( R2FLAG_AA ) )
                RCache.set_Element(
                    s_combine_msaa[ 0 ]->E[ bDistort ? 3 : 1 ] );
            // look at blender_combine.cpp
            else
                RCache.set_Element(
                    s_combine_msaa[ 0 ]
                        ->E[ bDistort ? 4
                                      : 2 ] ); // look at blender_combine.cpp
        }
        RCache.set_c( "e_barrier", ps_r2_aa_barier.x, ps_r2_aa_barier.y,
                      ps_r2_aa_barier.z, 0 );
        RCache.set_c( "e_weights", ps_r2_aa_weight.x, ps_r2_aa_weight.y,
                      ps_r2_aa_weight.z, 0 );
        RCache.set_c( "e_kernel", ps_r2_aa_kernel, ps_r2_aa_kernel,
                      ps_r2_aa_kernel, 0 );
        RCache.set_c( "m_current", Matrix_current );
        RCache.set_c( "m_previous", Matrix_previous );
        RCache.set_c( "m_blur", m_blur_scale.x, m_blur_scale.y, 0, 0 );
        /////lvutner
        RCache.set_c( "mask_control", ps_r2_mask_control.x,
                      ps_r2_mask_control.y, ps_r2_mask_control.z,
                      ps_r2_mask_control.w );
    ...
    if ( RImplementation.o.dx11_hdr10 ) {
        ...
        if ( ps_r4_hdr10_bloom_on ) {
            HW.pContext->CopyResource( rt_Generic_0->pTexture->surface_get(),
                                       rt_Color->pTexture->surface_get() );
            phase_hdr10_bloom(); // samples from rt_Generic_0, writes to
                                 // rt_Color
        }
        if ( ps_r4_hdr10_flare_on ) {
            HW.pContext->CopyResource( rt_Generic_0->pTexture->surface_get(),
                                       rt_Color->pTexture->surface_get() );
            phase_hdr10_lens_flare(); // samples from rt_Generic_0, writes to
                                      // rt_Color
        }
    }
    ...
    if ( 1 )
        for ( u32 it = 0; it < dbg_lines.size(); it++ ) {
            RCache.dbg_DrawLINE( Fidentity, dbg_lines[ it ].P0,
                                 dbg_lines[ it ].P1, dbg_lines[ it ].color );
        }
#endif
    ...
    if (0)		{
            u32		C					= color_rgba
(255,255,255,255); float	_w					=
float(Device.dwWidth)/3; float	_h					=
float(Device.dwHeight)/3;
            ...
            {
                    Fsphere				S	=
dbg_spheres[it].first; Fmatrix				M; u32
ccc		= dbg_spheres[it].second.get(); M.scale
(S.R,S.R,S.R); M.translate_over		(S.P); RCache.dbg_DrawEllipse	(M,ccc);
            ...
            if (1)
            {
                    u32 IX=0,IY=1; p0.set
(.5f/_w, .5f/_h); p1.set
((_w+.5f)/_w, (_h+.5f)/_h );

                    // Fill vertex buffer
                    FVF::TL* pv					= (FVF::TL*)
RCache.Vertex.Lock	(4,g_combine->vb_stride,Offset); pv->set
((IX+0)*_w+EPS,	(IY+1)*_h+EPS,	EPS,	1.f, C, p0.x, p1.y);	pv++; pv->set
((IX+0)*_w+EPS,	(IY+0)*_h+EPS,	EPS,	1.f, C, p0.x, p0.y);	pv++; pv->set
((IX+1)*_w+EPS,	(IY+1)*_h+EPS,	EPS,	1.f, C, p1.x, p1.y);	pv++; pv->set
((IX+1)*_w+EPS,	(IY+0)*_h+EPS,	EPS,	1.f, C, p1.x, p0.y);	pv++;
                    RCache.Vertex.Unlock (4,g_combine->vb_stride);
...
void CRenderTarget::phase_combine_volumetric() {
    ...
    if ( !RImplementation.o.dx10_msaa )
        ...
        u_setrt( rt_Generic_0_r, rt_Generic_1_r, 0,
                 RImplementation.Target->rt_MSAADepth->pZRT );
