...
static void PushFSQ( FVF::TL* pv, float w, float h ) {
    ...
    pv->set( 0, h, d_Z, d_W, C, 0.0f, 1.0f );
    pv++;
    pv->set( 0, 0, d_Z, d_W, C, 0.0f, 0.0f );
    pv++;
    pv->set( w, h, d_Z, d_W, C, 1.0f, 1.0f );
    pv++;
    pv->set( w, 0, d_Z, d_W, C, 1.0f, 0.0f );
    pv++;
}
...
void CRenderTarget::phase_hdr10_lens_flare() {
    ...
    // blur
    {
        // ping pong between buffers, sampling the src target, applying blur at
        // different scales (Kawase) viewport already set to correct resolution
        for ( int pass = 0; pass < ps_r4_hdr10_flare_blur_passes; pass++ ) {
            ...
            for ( int dir = 0; dir < 2; dir++ ) {
                ...
                float dx = ps_r4_hdr10_flare_blur_scale * float( pass + 1 ) *
                           1.0f / flare_w;
                float dy = ps_r4_hdr10_flare_blur_scale * float( pass + 1 ) *
                           1.0f / flare_h;
                RCache.set_c( "hdr10_flare_sparams", dx, dy, float( dir % 2 ),
                              0 );
