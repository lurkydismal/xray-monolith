...
void CRenderTarget::accum_spot( light* L ) {
    ...
    BOOL bIntersect = FALSE; // enable_scissor(L);
    {
        ...
        // *** similar to "Carmack's reverse", but assumes convex, non
        // intersecting objects,
        // *** thus can cope without stencil clear with 127 lights
        // *** in practice, 'cause we "clear" it back to 0x1 it usually allows
        // us to > 200 lights :)
        //	Done in blender!
        // RCache.set_ColorWriteEnable		(FALSE);
        RCache.set_Element( s_accum_mask->E[ SE_MASK_SPOT ] ); // masker

        // backfaces: if (stencil>=1 && zfail)			stencil =
        // light_id
        RCache.set_CullMode( CULL_CW );
        if ( !RImplementation.o.dx10_msaa )
            RCache.set_Stencil( TRUE, D3DCMP_LESSEQUAL, dwLightMarkerID, 0x01,
                                0xff, D3DSTENCILOP_KEEP, D3DSTENCILOP_KEEP,
                                D3DSTENCILOP_REPLACE );
        else
            RCache.set_Stencil( TRUE, D3DCMP_LESSEQUAL, dwLightMarkerID, 0x01,
                                0x7f, D3DSTENCILOP_KEEP, D3DSTENCILOP_KEEP,
                                D3DSTENCILOP_REPLACE );
        draw_volume( L );
    ...
    // *****************************	Minimize overdraw
    // ************************************* Select shader (front or
    // back-faces), *** back, if intersect near plane
    ...
    // Draw volume with projective texgen
    {
        ...
        RCache.set_ca( "m_lmap", 0, m_Lmap._11, m_Lmap._21, m_Lmap._31,
                       m_Lmap._41 );
        RCache.set_ca( "m_lmap", 1, m_Lmap._12, m_Lmap._22, m_Lmap._32,
                       m_Lmap._42 );
    ...
    if ( !RImplementation.o.fp16_blend ) {
        ...
        } else // checked Holger
        {
            ...
            if ( RImplementation.o.dx10_msaa_opt ) {
                RCache.set_Element(
                    s_accum_mask_msaa[ 0 ]->E[ SE_MASK_ACCUM_VOL ] );
                RCache.set_Stencil( TRUE, D3DCMP_EQUAL, dwLightMarkerID | 0x80,
                                    0xff, 0x00 );
                draw_volume( L );
            } else // checked Holger
            {
                for ( u32 i = 0; i < RImplementation.o.dx10_msaa_samples;
                      ++i ) {
                    RCache.set_Element(
                        s_accum_mask_msaa[ i ]->E[ SE_MASK_ACCUM_VOL ] );
                    StateManager.SetSampleMask( u32( 1 ) << i );
                    RCache.set_Stencil( TRUE, D3DCMP_EQUAL,
                                        dwLightMarkerID | 0x80, 0xff, 0x00 );
                    draw_volume( L );
    ...
    // dwLightMarkerID					+=	2;	// keep
    // lowest bit always setted up
...
void CRenderTarget::accum_volumetric_lv( light* L ) {
    ...
    if ( ps_ssfx_volumetric.x > 0 ) {
        float Falloff =
            ...
            std::min( std::max( ( L->vis.distance - 20 ) * 0.01f, 0.0f ),
                      1.0f ) *
                ps_ssfx_volumetric.y;
    ...
    if ( !RImplementation.o.ssfx_volumetric ) {
        ...
        if ( !m_bHasActiveVolumetric_spot ) {
            ...
            HW.pContext->ClearRenderTargetView( rt_ssfx_volumetric->pRT,
                                                ColorRGBA );
        }
    ...
    // Set constants
    RCache.set_c( "Ldynamic_pos", L_pos.x, L_pos.y, L_pos.z, att_factor );
    RCache.set_c( "Ldynamic_dir", L_dir.x, L_dir.y, L_dir.z,
                  -cos( L->cone * 0.5 ) );
...
void CRenderTarget::accum_volumetric( light* L ) {
    ...
    */
    ...
    CFrustum ClipFrustum;
    {
        ...
        ClipFrustum.planes[ 4 ].d -=
            ( ClipFrustum.planes[ 4 ].d + ClipFrustum.planes[ 5 ].d ) *
            ( 1 - L->m_volumetric_distance );
    }
    ...
    Fbox	BB;
    Fvector	rr;
    rr.set(L->spatial.sphere.R,L->spatial.sphere.R,L->spatial.sphere.R); BB.setb
    (L->spatial.sphere.P, rr);
    ...
    */
    ...
    // (1+L->m_volumetric_distance)*0.5f;
    float scaledRadius =
        L->SpatialComponent->spatial.sphere.R * L->m_volumetric_distance;
    ...
    aabb.setb( pt, rr );
    /*
            //	Calculate presise AABB assuming we are drawing for the spot
       light
            {
                    aabb.invalidate();
                    Fmatrix	transform;
    ...
    */
    ...
    // Draw volume with projective texgen
    {
        ...
        u32		_id					= 0;
        if (L->flags.bShadow)		{
                bool	bFullSize			= (L->X.S.size ==
        RImplementation.o.smapsize); if (L->X.S.transluent)	_id	=
        SE_L_TRANSLUENT; else if		(bFullSize)	_id	=
        SE_L_FULLSIZE; else					_id	=
        SE_L_NORMAL; } else { _id = SE_L_UNSHADOWED; m_Shadow
        = m_Lmap;
        }
        RCache.set_Element			(shader->E[ _id ]	);
        */
        ...
        //	Set up user clip planes
        {
            ...
            // RCache.set_ClipPlanes
            // (TRUE,ClipFrustum.planes,ClipFrustum.p_count);
            ...
            for ( int i = 0; i < 6; ++i ) {
                Fvector4& ClipPlane =
                    *( Fvector4* )&ClipFrustum.planes[ i ].n.x;
            ...
            {
                    Fvector4	TransformedPlane;
                    ClipTransform.transform(TransformedPlane,
            UnitClipPlanes[i]); TransformedPlane.normalize_as_plane();
            ...
            */
            ...
            {
                    Fvector4	&ClipPlane =
            *(Fvector4*)&ClipFrustum.planes[i].n.x; Fvector4
            TransformedPlane; PlaneTransform.transform(TransformedPlane,
            ClipPlane); TransformedPlane.mul(-1.0f); HW.pDevice->SetClipPlane(
            i, &TransformedPlane.x);
            }
            */
        }
        ...
        ++i )
                        {
                                ...
                                StateManager.SetSampleMask ( u32(1) << i );
                                RCache.set_Stencil
        (TRUE,D3DCMP_EQUAL,dwLightMarkerID|0x80,0xff,0x00);
                                RCache.Render(D3DPT_TRIANGLELIST,0,0,iNumSlises*4,0,iNumSlises*2);
                        }
                        StateManager.SetSampleMask( 0xffffffff );
                }
    ...
    /*
            ...
            if (!RImplementation.o.fp16_blend)	{
                    u_setrt
       (rt_Accumulator,NULL,NULL,HW.pBaseZB); RCache.set_Element
       (s_accum_mask->E[SE_MASK_ACCUM_VOL]	); RCache.set_c
       ("m_texgen",		m_Texgen); RCache.set_c
       ("m_texgen_J",		m_Texgen_J	); draw_volume
       (L);
            }
    */
    // CHK_DX
