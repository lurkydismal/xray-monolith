...
void CRenderTarget::accum_point( light* L ) {
    ...
    // *****************************	Mask by stencil
    // *************************************
    // *** similar to "Carmack's reverse", but assumes convex, non intersecting
    // objects,
    // *** thus can cope without stencil clear with 127 lights
    // *** in practice, 'cause we "clear" it back to 0x1 it usually allows us to
    // > 200 lights :)
    ...
    /*
    if (bIntersect)	RCache.set_CullMode		(CULL_CW);
    // back else			RCache.set_CullMode
    (CULL_CCW);		// front
    */
    ...
    // Draw volume with projective texgen
    {
        ...
        if ( L->flags.bShadow ) {
            bool bFullSize =
                ( L->X.S.size == u32( RImplementation.o.smapsize ) );
            if ( L->X.S.transluent )
                _id = SE_L_TRANSLUENT;
            else if ( bFullSize )
                _id = SE_L_FULLSIZE;
        ...
        // Constants
        RCache.set_c( "Ldynamic_pos", L_pos.x, L_pos.y, L_pos.z,
                      1 / ( L_R * L_R ) );
