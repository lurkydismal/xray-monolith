...
void CRenderTarget::phase_ssao() {
    ...
    if ( !RImplementation.o.dx10_msaa ) {
        ...
        u_setrt( rt_ssao_temp, 0, 0,
                 0 /*RImplementation.Target->rt_MSAADepth->pZRT*/ );
    }

    RCache.set_Stencil( FALSE );

    /*RCache.set_Stencil
    (TRUE,D3DCMP_LESSEQUAL,0x01,0xff,0x00);	// stencil should be >= 1 if
    (RImplementation.o.nvstencil)	{ u_stencil_optimize
    ...
    if ( !RImplementation.o.dx10_msaa )
        ...
        else
        {
                ...
                {
                        RCache.set_Element			(
        s_ssao_msaa[i]->E[0]	); StateManager.SetSampleMask	( u32(1) << i );
                        RCache.set_Stencil			( TRUE,
        D3DCMP_EQUAL, 0x81, 0x81, 0 ); RCache.Render (
    ...
    set_viewport( HW.pContext, float( Device.dwWidth ),
...
void CRenderTarget::phase_downsamp() {
    ...
    if ( RImplementation.o.ssao_half_data ) {
        set_viewport( HW.pContext, float( Device.dwWidth ) * 0.5f,
                      float( Device.dwHeight ) * 0.5f );
        w /= 2;
        h /= 2;
    }
...
void CRenderTarget::phase_ssfx_ao() {
    ...
    // Save AO frame
    HW.pContext->CopyResource( rt_ssfx_ao->pTexture->surface_get(),
...
void CRenderTarget::phase_ssfx_il() {
    ...
    // Save AO frame
    HW.pContext->CopyResource( rt_ssfx_il->pTexture->surface_get(),
