...
void CRenderTarget::phase_scene_prepare() {
    ...
    if ( !RImplementation.o.dx10_msaa )
        u_setrt( Device.dwWidth, Device.dwHeight, rt_Position->pRT, NULL, NULL,
                 HW.pBaseZB );
    else
        u_setrt( Device.dwWidth, Device.dwHeight, rt_Position->pRT, NULL, NULL,
                 rt_MSAADepth->pZRT );
    ...
    if ( !RImplementation.o.dx10_msaa )
        HW.pContext->ClearDepthStencilView(
            HW.pBaseZB, D3D_CLEAR_DEPTH | D3D_CLEAR_STENCIL, 1.0f, 0 );
    else {
        HW.pContext->ClearDepthStencilView(
            rt_MSAADepth->pZRT, D3D_CLEAR_DEPTH | D3D_CLEAR_STENCIL, 1.0f, 0 );
        HW.pContext->ClearDepthStencilView(
            HW.pBaseZB, D3D_CLEAR_DEPTH | D3D_CLEAR_STENCIL, 1.0f, 0 );
    }
...
void CRenderTarget::phase_scene_begin() {
    ...
    // Targets, use accumulator for temporary storage
    {
        ...
        if ( RImplementation.o.albedo_wo )
            u_setrt( rt_Position, rt_Accumulator, rt_Heat,
                     rt_ssfx_motion_vectors, pZB );
        else
            u_setrt( rt_Position, rt_Color, rt_Heat, rt_ssfx_motion_vectors,
                     pZB );
        //--DSR-- HeatVision_end
    }
