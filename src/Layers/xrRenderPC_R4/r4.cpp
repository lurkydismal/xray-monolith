        ...
        if ( SUCCEEDED( _result ) && pReflection ) {
            ...
            svs_result->signature =
                dxRenderDeviceRender::Instance()
                    .Resources->_CreateInputSignature( pSignatureBlob );
...
HRESULT CRender::shader_compile( LPCSTR name,
    ...
    bool const useGeneratedShaderCache =
        psDeviceFlags2.test( rsPrecompiledShaders ) ||
    ...
    u32 source_crc = 0;
    if ( useGeneratedShaderCache )
        source_crc = getShaderSourceCrc32( pSrcData, SrcDataLen,
                                           ::Render->getShaderPath() );
        ...
        if ( useGeneratedShaderCache ) {
            if ( file->length() > 8 ) {
                ...
                if ( saved_source_crc == source_crc ) {
                    ...
                    Msg( "! Shader cache source CRC mismatch for '%s' (%s): "
...
static inline bool match_shader_id( LPCSTR const debug_shader_id,
    ...
    for ( ; i != e; ++i ) {
        if ( match_shader( debug_shader_id, full_shader_id, ( *i ).name.c_str(),
                           ( *i ).name.size() ) ) {
            VERIFY( !found );
            found = true;
            temp = ( *i ).name.c_str();
