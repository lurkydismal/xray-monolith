...
void CRenderTarget::phase_bloom() {
    ...
    // CHK_DX		(HW.pDevice->SetRenderState	( D3DRS_ZENABLE,
    // FALSE				));
    RCache.set_Z( FALSE );
    ...
    // Transfer into Bloom1
    {
        ...
        // Fill vertex buffer
        v_build* pv = ( v_build* )RCache.Vertex.Lock(
            4, g_bloom_build->vb_stride, Offset );
        ...
        // Perform combine (all scalers must account for 4 samples + final
        // diffuse multiply);
        float s = ps_r2_ls_bloom_threshold; // scale
        f_bloom_factor = .9f * f_bloom_factor + .1f * ps_r2_ls_bloom_speed *
                                                    Device.fTimeDelta; // speed
    ...
    if ( ps_r2_ls_flags.test( R2FLAG_FASTBLOOM ) ) {
        ...
        // Transfer into Bloom2, horizontal filter
        {
            ...
            Fvector4 a_1 = { a_0.x - one.x - half.x, half.y, half.y,
                             a_0.w + one.x + half.x }; // -1,+1i
            Fvector4 a_2 = { a_1.x - two.x, half.y, half.y,
                             a_1.w + two.x }; // -2,+2i
            Fvector4 a_3 = { a_2.x - two.x, half.y, half.y,
                             a_2.w + two.x }; // -3,+3i
            Fvector4 a_4 = { a_3.x - two.x, half.y, half.y,
                             a_3.w + two.x }; // -4,+4i
            Fvector4 a_5 = { a_4.x - two.x, half.y, half.y,
                             a_4.w + two.x }; // -5,+5i
            Fvector4 a_6 = { a_5.x - two.x, half.y, half.y,
                             a_5.w + two.x }; // -6,+6i
            Fvector4 a_7 = { a_6.x - two.x, half.y, half.y,
                             a_6.w + two.x }; // -7,+7i

            // Fill vertex buffer
            v_filter* pv = ( v_filter* )RCache.Vertex.Lock(
                4, g_bloom_filter->vb_stride, Offset );
            ...
            CalcGauss_wave( w0, w1, kernel, kernel / 3.f,
                            ps_r2_ls_bloom_kernel_scale );
            u_setrt( rt_Bloom_2, NULL, NULL,
                     NULL ); // No need for ZBuffer at all
        ...
        // Transfer into Bloom1, vertical filter
        {
            ...
            Fvector2 half = { .5f / _w, .5f / _h };
            Fvector4 a_0 = { half.x, half.y, half.y, half.x }; // center
            Fvector4 a_1 = { half.x, a_0.y - one.y - half.y,
                             half.y + one.y + a_0.z, half.x }; // -1,+1i
            Fvector4 a_2 = { half.x, a_1.y - two.y, two.y + a_1.z,
                             half.x }; // -2,+2i
            Fvector4 a_3 = { half.x, a_2.y - two.y, two.y + a_2.z,
                             half.x }; // -3,+3i
            Fvector4 a_4 = { half.x, a_3.y - two.y, two.y + a_3.z,
                             half.x }; // -4,+4i
            Fvector4 a_5 = { half.x, a_4.y - two.y, two.y + a_4.z,
                             half.x }; // -5,+5i
            Fvector4 a_6 = { half.x, a_5.y - two.y, two.y + a_5.z,
                             half.x }; // -6,+6i
            Fvector4 a_7 = { half.x, a_6.y - two.y, two.y + a_6.z,
                             half.x }; // -7,+7i
            ...
            // Perform filtering
            Fvector4 w0, w1;
            float kernel = ps_r2_ls_bloom_kernel_g * float( Device.dwHeight ) /
                           float( Device.dwWidth );
            CalcGauss_wave( w0, w1, kernel, kernel / 3.f,
                            ps_r2_ls_bloom_kernel_scale );
            u_setrt( rt_Bloom_1, NULL, NULL,
                     NULL ); // No need for ZBuffer at all
    ...
    // for FP16-BLEND capable HW we can blend flares into smaller target,
    ...
    if ( _menu_pp ) {
        // CHK_DX				(HW.pDevice->Clear( 0L, NULL,
        // D3DCLEAR_TARGET,	0,	1.0f, 0L));
...
void CRenderTarget::phase_ssfx_bloom() {
    ...
    if ( ps_r2_mask_control.x > 0 ) {
        ...
        // Lens 2 Phase blur
        ref_rt* rt_LensBlur[ 2 ] = { &rt_ssfx_bloom_tmp4_2,
                                     &rt_ssfx_bloom_lens };
        ...
        for ( int lensblur = 0; lensblur < 2; lensblur++ ) {
            ...
            // Fill vertex buffer
            pv = ( FVF::TL* )RCache.Vertex.Lock( 4, g_combine->vb_stride,
                                                 Offset );
            pv->set( 0, h, EPS_S, 1.0f, C, 0.0f, 1.0f );
            pv++;
            pv->set( 0, 0, EPS_S, 1.0f, C, 0.0f, 0.0f );
            ...
            RCache.set_c( "blur_setup", w / 4, h / 4, 0,
                          2.0f + ( 3.0f * lensblur ) );
            RCache.set_Geometry( g_combine );
            RCache.Render( D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2 );
        }
    ...
    ref_rt* rt_Down[ 6 ] = { &rt_ssfx_bloom_tmp2,  &rt_ssfx_bloom_tmp4,
                             &rt_ssfx_bloom_tmp8,  &rt_ssfx_bloom_tmp16,
                             &rt_ssfx_bloom_tmp32, &rt_ssfx_bloom_tmp64 };
    ...
    for ( int downsample = 0; downsample < 6; downsample++ ) {
        ...
        RCache.set_c( "blur_setup", w / SampleScale, h / SampleScale, 0,
                      ps_ssfx_bloom_2.x );
        RCache.set_Geometry( g_combine );
        RCache.Render( D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2 );
    }
    ...
    ref_rt* rt_Up[ 5 ] = { &rt_ssfx_bloom_tmp32_2, &rt_ssfx_bloom_tmp16_2,
                           &rt_ssfx_bloom_tmp8_2, &rt_ssfx_bloom_tmp4_2,
                           &rt_ssfx_bloom_tmp2 };
    ...
    RCache.set_c( "mask_control", ps_r2_mask_control.x, ps_r2_mask_control.y,
                  ps_r2_mask_control.z, ps_r2_mask_control.w );
    RCache.set_Geometry( g_combine );
    RCache.Render( D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2 );
