...
/*
        ...
        agressive culling - Remember, we are raytracing lights, not emmisive
   surfaces like in 90% of actual PT/SSPT implementations. Keeping this comment
...
void light::gi_generate() {
    ...
    for ( int it = 0; it < int( indirect_photons * 8 ); it++ ) {
        ...
        CDB::TRI& T = tris[ R->id ];
        Fvector Tv[ 3 ] = { verts[ T.verts[ 0 ] ], verts[ T.verts[ 1 ] ],
                            verts[ T.verts[ 2 ] ] };
        Fvector TN;
        ...
        LI.E = dot *
               ( 1 - R->range /
                         range ); // Looks to be light attenuation or something
