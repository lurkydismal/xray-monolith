...
void CBlender_Screen_GRAY::Compile( CBlender_Compile& C ) {
    ...
    C.PassBegin();
    {
        ...
        C.R().SetRS( D3DRS_TEXTUREFACTOR,
                     color_rgba( 76 + 105, 150 + 105, 29 + 105, 0 ) );
        ...
        {
            C.StageSET_Address( D3DTADDRESS_CLAMP );
            C.StageSET_Color( D3DTA_CURRENT, D3DTOP_DOTPRODUCT3,
                              D3DTA_TFACTOR );
            C.StageSET_Alpha( D3DTA_CURRENT, D3DTOP_DOTPRODUCT3,
                              D3DTA_TFACTOR );
            C.Stage_Texture( oT_Name );
            C.Stage_Matrix( oT_xform, 0 );
            C.Stage_Constant( "$null" );
