...
void CRender::create() {
    ...
    o.distortion =
        v_dev >= v_need && !Core.ParamsData.test( ECoreParams::nodistort );
    Msg( "* distortion: %s, dev(%d),need(%d)",
         o.distortion ? "used" : "unavailable", v_dev, v_need );

    //	Color mapping
    o.color_mapping =
        v_dev >= v_need && !Core.ParamsData.test( ECoreParams::nocolormap );
    Msg( "* color_mapping: %s, dev(%d),need(%d)",
         o.color_mapping ? "used" : "unavailable", v_dev, v_need );
...
void CRender::apply_object( IRenderable* O ) {
    ...
    if ( O->renderable_ROS() ) {
        ...
        // shadowing
        if ( ( LT.shadow_recv_frame == Device.dwFrame ) &&
             O->renderable_ShadowReceive() )
...
void CRender::Calculate() {
    ...
    // Main process
    GMBase.traverse( pLastSector, ViewBase, Device.vCameraPosition,
...
void CRender::Render() {
    ...
    GMBase.r_dsgraph_render_dynamic( 1,
                                     true ); // normal level, secondary priority
    ...
    for ( int _priority = 0; _priority < 2; ++_priority ) {
        for ( u32 iPass = 0; iPass < SHADER_PASSES_MAX; ++iPass ) {
            R_ASSERT(
                GMBase.RGraph.mapStaticPasses[ _priority ][ iPass ].size() ==
                0 );
            R_ASSERT(
                GMBase.RGraph.mapDynamicPasses[ _priority ][ iPass ].size() ==
                0 );
        }
    }
...
HRESULT CRender::shader_compile( LPCSTR name,
    ...
    if ( FAILED( _result ) ) {
        ...
        _result = D3DXCompileShader( ( LPCSTR )pSrcData, SrcDataLen, defines,
                                     pInclude, pFunctionName, pTarget,
...
void CRender::RenderToTarget( RRT target ) {
    ...
    switch ( target ) {
        ...
        case rtSVP:
            ...
            break;
        default:
            Debug.fatal( DEBUG_INFO, "None or wrong Target specified: %i",
                         target );
            break;
    }
    ...
    D3DXLoadSurfaceFromSurface( ( *RT )->pRT, 0, 0, pBackBuffer, 0, 0,
                                D3DX_DEFAULT, 0 );
    pBackBuffer->Release();
}
