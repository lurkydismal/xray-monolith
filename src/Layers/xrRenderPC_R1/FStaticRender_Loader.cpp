    ...
    g_pGamePersistent->LoadTitle();
    {
        ...
        for ( u32 i = 0; i < count; i++ ) // skip first shader as "reserved" one
        {
            ...
            *delim = 0;
            ...
            // Alundaio: Fix Static Renderer issue with 'default' shader and
            // only a single texture (we know this because string won't have
            // comma present! Force change to 'def_shaders\def_vertex'
            if ( xr_strcmp( n_sh, "default" ) == 0 && !strstr( n_tlist, "," ) )
                Shaders[ i ] =
                    dxRenderDeviceRender::Instance().Resources->Create(
                        "def_shaders\\def_vertex", n_tlist );
            else
                Shaders[ i ] =
                    dxRenderDeviceRender::Instance().Resources->Create(
                        n_sh, n_tlist );
...
void CRender::LoadBuffers( CStreamReader* base_fs ) {
    ...
    u32 dwUsage =
        D3DUSAGE_WRITEONLY |
        ( HW.Caps.geometry.bSoftware ? D3DUSAGE_SOFTWAREPROCESSING : 0 );
    ...
    if ( base_fs->find_chunk( fsL_VB ) ) {
        ...
        u32 buffer_size =
            ( MAXD3DDECLLENGTH + 1 ) * sizeof( D3DVERTEXELEMENT9 );
        D3DVERTEXELEMENT9* dcl = ( D3DVERTEXELEMENT9* )_alloca(
            buffer_size ); // Alundaio: moved out of loop!
        ...
        for ( u32 i = 0; i < count; i++ ) {
            ...
            R_CHK( HW.pDevice->CreateVertexBuffer(
                vCount * vSize, dwUsage, 0, D3DPOOL_MANAGED, &VB[ i ], 0 ) );
            HW.stats_manager.increment_stats( vCount * vSize,
    ...
    if ( base_fs->find_chunk( fsL_IB ) ) {
        ...
        for ( u32 i = 0; i < count; i++ ) {
            ...
            R_CHK( HW.pDevice->CreateIndexBuffer(
                iCount * 2, dwUsage, D3DFMT_INDEX16, D3DPOOL_MANAGED, &IB[ i ],
                0 ) );
            HW.stats_manager.increment_stats(
                iCount * 2, enum_stats_buffer_type_index, D3DPOOL_MANAGED );
            R_CHK( IB[ i ]->Lock( 0, 0, ( void** )&pData, 0 ) );
            //			CopyMemory
            //(pData,fs->pointer(),iCount*2);
            fs->r( pData, iCount * 2 );
            IB[ i ]->Unlock();
