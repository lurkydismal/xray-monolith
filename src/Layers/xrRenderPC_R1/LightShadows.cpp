...
void CLightShadows::set_object( IRenderable* O, IDSGraphManager& DM ) {
    ...
    if ( 0 == O )
        ...
        if ( !O->renderable_ShadowGenerate() ||
             DM.i_mask[ IDSGraphManager::fl_hud ] ||
             ( ( CROS_impl* )O->renderable_ROS() )->shadow_gen_frame ==
                 Device.dwFrame ) {
            current = 0;
            return;
        }
...
void CLightShadows::add_element( NODE& N ) {
    ...
    if ( 0 == current )
        return;
    VERIFY2( casters.back()->nodes.size() < 24,
             "Object exceeds limit of 24 renderable parts/materials" );
    if ( 0 == N.val.pVisual->shader->E[ SE_R1_LMODELS ]._get() )
        return;
...
void CLightShadows::calculate() {
    ...
    for ( u32 o_it = 0; o_it < casters.size(); o_it++ ) {
        ...
        // iterate on lights
        for ( u32 l_it = 0; ( l_it < lights.size() ) && ( slot_id < slot_max );
              l_it++ ) {
            ...
            if ( !bRTS ) {
                ...
                HW.pDevice->Clear( 0, 0, D3DCLEAR_TARGET,
            ...
            if ( L.source->flags.type == IRender_Light::DIRECT ) {
                ...
                while ( true ) {
                    ...
                    if ( _dist > EPS_L )
                        break;
                    Lpos.y +=
                        .01f; //. hack to avoid light-in-the-center-of-object
                }
            ...
            // float		p_nearR	=
            // C.C.distance_to(L.source->position) + p_R*0.85f + eps; 			p_nearR =
            //p_near;
            float p_far = _min( Lrange, _max( p_dist + S_fade, p_dist + p_R ) );
            // if (p_near<eps)			continue;
            // if (p_far<(p_near+eps))	continue;
            //	Igor: make check here instead of assertion in
            //buil_projection_hat if (!(_abs(p_far-p_near) > eps)) continue; if
            // (p_hat>0.9f)			continue; if (p_hat<0.01f)
            // continue;

            // Msg			("* near(%f),
            // near-x(%f)",p_near,p_nearR);
            ...
            D3DVIEWPORT9 VP = {
                s_x * S_size, s_y * S_size, S_size, S_size, 0, 1 };
    ...
    if ( bRTS ) {
        ...
        u32 Offset;
        FVF::TL4uv* pv =
            ( FVF::TL4uv* )RCache.Vertex.Lock( 4, geom_Blur.stride(), Offset );
...
IC bool cache_search( const CLightShadows::cache_item& A,
                      const CLightShadows::cache_item& B ) {
    if ( A.O < B.O )
        return true;
    if ( A.O > B.O )
        return false;
...
IC int PLC_calc( Fvector& P, Fvector& N, light* L, float energy, Fvector& O ) {
    ...
    float C1 = clampr(
        Device.vCameraPosition.distance_to_sqr( P ) / S_distance2, 0.f, 1.f );
...
void CLightShadows::render() {
    ...
    //	lerp
    Device.mProject._43 -=
        fMinNearBias + ( fMaxNearBias - fMinNearBias ) * fLerpCoeff;
    ...
    int batch = 0;
    u32 Offset = 0;
    FVF::LIT* pv = ( FVF::LIT* )RCache.Vertex.Lock(
        batch_size * 3, geom_World->vb_stride, Offset );
    for ( u32 s_it = 0; s_it < shadows.size(); s_it++ ) {
        ...
        t_scale.set( float( S_size ) / float( S_rt_size ),
                     float( S_size ) / float( S_rt_size ) );
        t_scale.mul( .5f );
        t_offset.set( float( s_x ) / float( slot_line ),
                      float( s_y ) / float( slot_line ) );
        ...
        CI_what.L = S.L;
        CI_what.tris = 0;
        xr_vector< cache_item >::iterator CI_ptr = std::lower_bound(
            cache.begin(), cache.end(), CI_what, cache_search );
        if ( CI_ptr == cache.end() ) {
            // empty ?
            ...
            if ( CI_ptr->O != CI_what.O || CI_ptr->L != CI_what.L ) {
                ...
                CI = &*CI_ptr;
                bValid = TRUE;
                if ( !CI->Op.similar( CI->O->renderable.xform.c ) )
                    bValid = FALSE;
        ...
        if ( !bValid ) {
            ...
            for ( CDB::RESULT* p = xrc.r_begin(); p != xrc.r_end(); p++ ) {
                ...
                // Calc plane, throw away degenerate tris and invisible to light
                // polygons
                Fplane P;
                float mag = 0;
                Fvector t1, t2, n;
            ...
            if ( tess.size() ) {
                ...
                // Msg					("---alloc---
                // %x",u32(CI->tris));
                CopyMemory( CI->tris, &*tess.begin(),
                            CI->tcnt * sizeof( tess_tri ) );
            }
        }
        ...
        for ( u32 tid = 0; tid < CI->tcnt; tid++ ) {
            ...
            */
            ...
            S.M.transform( T, v[ 0 ] );
            pv->set( v[ 0 ], CLS( c0 ), ( T.x + 1 ) * t_scale.x + t_offset.x,
                     ( 1 - T.y ) * t_scale.y + t_offset.y );
            pv++;
            S.M.transform( T, v[ 1 ] );
            pv->set( v[ 1 ], CLS( c1 ), ( T.x + 1 ) * t_scale.x + t_offset.x,
                     ( 1 - T.y ) * t_scale.y + t_offset.y );
            pv++;
            S.M.transform( T, v[ 2 ] );
            pv->set( v[ 2 ], CLS( c2 ), ( T.x + 1 ) * t_scale.x + t_offset.x,
                     ( 1 - T.y ) * t_scale.y + t_offset.y );
            pv++;
