...
BOOL CRenderTarget::Create() {
    ...
    if ( RImplementation.o.color_mapping ) {
        // RT_color_map.create	(RTname_color_map,
        // rtWidth,rtHeight,HW.Caps.fTarget);
        RT_color_map.create( RTname_color_map, curWidth, curHeight,
                             HW.Caps.fTarget );
    }
    // RImplementation.o.color_mapping = RT_color_map->valid();

    // We need to use Device.resolution in order to maintain performance while
    // rendering image with second viewport + supersampling. Please do not
    // change this. It's just working properly.
    rt_secondVP.create( RTname_SecondVP, Device.dwWidth, Device.dwHeight,
                        HW.Caps.fTarget ); //--#SM+#-- +SecondVP+
    rt_ui_pda.create( RTname_pda_ui, Device.dwWidth, Device.dwHeight,
                      HW.Caps.fTarget );

    if ( ( rtHeight != Device.dwHeight ) || ( rtWidth != Device.dwWidth ) ) {
        R_CHK( HW.pDevice->CreateDepthStencilSurface(
            rtWidth, rtHeight, HW.Caps.fDepth, D3DMULTISAMPLE_NONE, 0, TRUE,
            &ZB, NULL ) );
    } else {
        ZB = HW.pBaseZB;
        ZB->AddRef();
    ...
    R_CHK( HW.pDevice->CreateDepthStencilSurface( 512, 512, HW.Caps.fDepth,
    ...
    HW.pDevice->CreateOffscreenPlainSurface( rtWidth, rtHeight, HW.Caps.fTarget,
...
void CRenderTarget::calc_tc_noise( Fvector2& p0, Fvector2& p1 ) {
    //.	CTexture*	T					=
    // RCache.get_ActiveTexture	(2); .	VERIFY2		(T, "Texture #3 in noise
    // shader should be setted up"); .	u32			tw
    //= iCeil(float(T->get_Width	())*param_noise_scale+EPS_S); .	u32
    // th					= iCeil(float(T->get_Height
    //())*param_noise_scale+EPS_S);
...
void CRenderTarget::calc_tc_duality_ss( Fvector2& r0,
                                        Fvector2& r1,
                                        Fvector2& l0,
                                        Fvector2& l1 ) {
...
bool CRenderTarget::NeedColorMapping() {
    return RImplementation.o.color_mapping &&
           ( param_color_map_influence > 0.001f );
}
...
BOOL CRenderTarget::NeedPostProcess() {
    ...
    return _blur || _gray || _noise || _dual || _cbase || _cadd || _cmap ||
           _menu_pp;
}

BOOL CRenderTarget::Perform() {
    return Available() && ( ( ( BOOL )RImplementation.m_bMakeAsyncSS ) ||
                            NeedPostProcess() || ( ps_r__Supersample > 1 ) ||
...
void CRenderTarget::Begin() {
    ...
    {
            Msg
    ("[%5d]------------------------",Device.dwFrame); SHOW
    (param_blur) SHOW				(param_gray) SHOW
    (param_duality_h) SHOW				(param_duality_v) SHOW
    (param_noise) SHOW				(param_noise_scale) SHOW
    (param_noise_fps)
...
void CRenderTarget::End() {
    ...
    bool _menu_pp =
        g_pGamePersistent ? g_pGamePersistent->OnRenderPPUI_query() : false;
    if ( ( 0 == RImplementation.GMBase.RGraph.mapStaticSorted.Distort.size() &&
    ...
    // Msg
    // ("param_gray:%f(%d),param_noise:%f(%d)",param_gray,gblend,param_noise,nblend);
    // Msg				("base: %d,%d,%d",
    // color_get_R(p_color),		color_get_G(p_color),
    // color_get_B(p_color)); Msg				("gray:
    // %d,%d,%d",	color_get_R(p_gray),		color_get_G(p_gray),
    // color_get_B(p_gray)); Msg				("add:
    // %d,%d,%d",	color_get_R(p_brightness),
    ...
    TL_2c3uv* pv =
        ( TL_2c3uv* )RCache.Vertex.Lock( 4, g_postprocess.stride(), Offset );
    pv->set( du + 0, dv + float( _h ), p_color, p_gray, r0.x, r1.y, l0.x, l1.y,
             n0.x, n1.y );
    pv++;
    pv->set( du + 0, dv + 0, p_color, p_gray, r0.x, r0.y, l0.x, l0.y, n0.x,
             n0.y );
    pv++;
    pv->set( du + float( _w ), dv + float( _h ), p_color, p_gray, r1.x, r1.y,
             l1.x, l1.y, n1.x, n1.y );
    pv++;
    pv->set( du + float( _w ), dv + 0, p_color, p_gray, r1.x, r0.y, l1.x, l0.y,
             n1.x, n0.y );
    pv++;
    ...
    if ( bCMap ) {
        ...
        RCache.set_Element( bDistort ? s_postprocess_D[ 1 ]->E[ 4 ]
                                     : s_postprocess[ 1 ]->E[ 4 ] );
        RCache.set_Geometry( g_postprocess );
        RCache.set_c( s_colormap, param_color_map_influence,
                      param_color_map_interpolate, 0, 0 );
    ...
    // RCache.set_Element	(bDistort ? s_postprocess_D->E[iShaderElement] :
    // s_postprocess->E[iShaderElement]);

    RCache.set_Shader( bDistort ? s_postprocess_D[ bCMap ]
                                : s_postprocess[ bCMap ] );
    ...
    RCache.set_c( s_brightness, p_brightness.x, p_brightness.y, p_brightness.z,
                  0 );
    RCache.set_c( s_colormap, param_color_map_influence,
                  param_color_map_interpolate, 0, 0 );
    RCache.set_Geometry( g_postprocess );
...
void CRenderTarget::phase_distortion() {
    ...
    CHK_DX( HW.pDevice->Clear( 0L, NULL, D3DCLEAR_TARGET,
                               color_rgba( 127, 127, 127, 127 ), 1.0f, 0L ) );
