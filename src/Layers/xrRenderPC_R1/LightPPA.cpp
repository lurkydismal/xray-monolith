...
//////////////////////////////////////////////////////////////////////////
/*
IC void mk_vertex					(CLightR_Vertex& D,
Fvector& P, Fvector& N, Fvector& C, float r2)
{
        D.P.set	(P);
        D.N.set	(P);
        D.u0	= (P.x-C.x)/r2+.5f;
...
void CLightR_Manager::render_point	()
{
        ...
        for (xr_vector<light*>::iterator it=selected_point.begin();
it!=selected_point.end(); it++)
        {
                ...
                // Culling
                if (PPL.range<0.05f)
continue; if (PPL.color.magnitude_sqr_rgb()<EPS)
continue; float	alpha		=
Device.vCameraPosition.distance_to(PPL.position)/MAX_DISTANCE; if (alpha>=1)
continue; if (!RImplementation.ViewBase.testSphere_dirty
(PPL.position,PPL.range))	continue;
                ...
                RCache.set_c (hPPAcolor,factor.r,factor.g,factor.b,1);
                {
                        // Build bbox
                        float				size_f	=
PPL.range+EPS_L; Fvector				size; size.set
(size_f,size_f,size_f);

                        // Query collision DB (Select polygons)
                        xrc.box_options		(0);
                        xrc.box_query
(g_pGameLevel->ObjectSpace.GetStaticModel(),PPL.position,size); u32	triCount
= xrc.r_count	(); if	(0==triCount)	return; CDB::TRI* tris		=
g_pGameLevel->ObjectSpace.GetStaticTris(); Fvector* verts		=
g_pGameLevel->ObjectSpace.GetStaticVerts();

                        // Lock
                        RCache.set_Geometry		(hGeom);
                        u32 triLock				=
_min(256u,triCount); u32	vOffset; CLightR_Vertex* VB		=
...
[xrc.r_begin()[t].id];
                                ...
                                // Test for poly facing away from light or
camera if (Poly.classify(PPL.position)<0)	continue; if
(Poly.classify(cam)<0)			continue;

                                // Triangulation and UV0/UV1
                                mk_vertex(*VB,V1,Poly.n,PPL.position,r2);
VB++; mk_vertex(*VB,V2,Poly.n,PPL.position,r2);	VB++;
                                mk_vertex(*VB,V3,Poly.n,PPL.position,r2);
VB++; actual++;

                                if (actual>=triLock)
                                {
                                        RCache.Vertex.Unlock
(actual*3,hGeom->vb_stride); if (actual) RCache.Render
(D3DPT_TRIANGLELIST,vOffset,actual); actual
= 0; triLock						= _min(256u,triCount-t);
                                        VB
= (CLightR_Vertex*)RCache.Vertex.Lock(triLock*3,hGeom->vb_stride,vOffset);
                                }
                        }
                        ...
                        RCache.Vertex.Unlock
(actual*3,hGeom->vb_stride); if (actual) RCache.Render
(D3DPT_TRIANGLELIST,vOffset,actual);
                }
...
void CLightR_Manager::render_point( u32 _priority ) {
    ...
    for ( xr_vector< light* >::iterator it = selected_point.begin();
        ...
        //		0. Dimm & Clip
        float lc_dist =
            lc_COP.distance_to( L->SpatialComponent->spatial.sphere.P ) -
            L->SpatialComponent->spatial.sphere.R;
        ...
        L_project.build_projection( deg2rad( 2.f ), 1.f, _camrange - L->range,
                                    _camrange + L->range );
        ...
        // if( _priority == 1)
        //	RImplementation.r_pmask
        //(false,true);

        CFrustum F;
        F.CreateFromMatrix( L_combine, FRUSTUM_P_LRTB | FRUSTUM_P_FAR );

        L->GMLight.traverse( ( CSector* )L->SpatialComponent->spatial.sector, F,
                             L_pos, L_combine );
...
void CLightR_Manager::render_spot( u32 _priority ) {
    ...
    for ( xr_vector< light* >::iterator it = selected_spot.begin();
          it != selected_spot.end(); it++ ) {
        light* L = *it;
        ...
        L_pos.set( L->position );
        L_view.build_camera_dir( L_pos, L_dir, L_up );
        L_project.build_projection( L->cone, 1.f, SSM_near_plane,
                                    L->range + EPS_S );
        L_combine.mul( L_project, L_view );
        ...
        if ( bHUD && _priority == 0 )
            L->GMLight.r_dsgraph_render_hud();
        //	RCache.set_ClipPlanes					(false,
        //&L_combine);
    }
    //		??? grass ???l
}

IC void hud_light_apply(
