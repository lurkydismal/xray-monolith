...
#define MAX_GlowsDist1 \
    float( g_pGamePersistent->Environment().CurrentEnv->far_plane )
...
void CGlowManager::render_sw() {
    ...
    for ( int i = 0; i < ps_r1_GlowsPerFrame; i++, dwTestID++ ) {
        ...
        if ( range > EPS_S ) {
            dir.div( range );
            G.bTestResult = g_pGameLevel->ObjectSpace.RayTest(
                start, dir, range, collide::rqtBoth, &G.RayCache, o_main );
        }
    }
...
void CGlowManager::render_hw() {
    ...
    // 1. Sort into two parts - 1(selected-to-test)[to-test],
    // 2(selected)[just-draw] Fvector &start	= Device.vCameraPosition;
    for ( int i = 0; ( i < ps_r1_GlowsPerFrame ) && Selected.size();
          i++, dwTestID++ ) {
        u32 ID = dwTestID % Selected.size();
...
void CGlowManager::render_selected() {
    ...
    for ( ; pos < Selected.size(); ) {
        ...
        count = 0;
        while ( ( pos + count < Selected.size() ) &&
                ( ( ( CGlow* )Selected[ pos + count ]._get() )->shader == T ) )
            count++;

        u32 vOffset;
        u32 end = pos + count;
        FVF::LIT* pvs = pv = ( FVF::LIT* )RCache.Vertex.Lock(
            count * 4, hGeom->vb_stride, vOffset );
        for ( ; pos < end; pos++ ) {
            // Cull invisible
        ...
        if ( vCount ) {
            ...
            RCache.Render( D3DPT_TRIANGLELIST, vOffset, 0, vCount, 0,
                           vCount / 2 );
        }
    }
