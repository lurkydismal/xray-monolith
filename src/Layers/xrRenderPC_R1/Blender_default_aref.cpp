    ...
    if ( C.bEditor ) {
        C.PassBegin();
        {
            ...
            if ( oBlend.value )
                ...
                C.PassSET_Blend( TRUE, D3DBLEND_ONE, D3DBLEND_ZERO, TRUE,
                                 oAREF.value );
        ...
        switch ( C.iElement ) {
            case SE_R1_NORMAL_HQ: {
                ...
                if ( C.bDetail_Diffuse )
                    sname = "lmap_dt";
                if ( oBlend.value )
                    C.r_Pass( sname, sname, TRUE, TRUE, TRUE, TRUE,
                              D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA, TRUE,
                              oAREF.value );
                else
                    C.r_Pass( sname, sname, TRUE, TRUE, TRUE, TRUE,
                              D3DBLEND_ONE, D3DBLEND_ZERO, TRUE, oAREF.value );
            ...
            case SE_R1_LPOINT:
                if ( !oBlend.value ) {
                    C.r_Pass( "lmap_point", "add_point", FALSE, TRUE, FALSE,
                              TRUE, D3DBLEND_ONE, D3DBLEND_ONE, TRUE,
                              oAREF.value );
                    C.r_Sampler( "s_base", C.L_textures[ 0 ] );
            ...
            case SE_R1_LSPOT:
                if ( !oBlend.value ) {
                    C.r_Pass( "lmap_spot", "add_spot", FALSE, TRUE, FALSE, TRUE,
                              D3DBLEND_ONE, D3DBLEND_ONE, TRUE, oAREF.value );
                    C.r_Sampler( "s_base", C.L_textures[ 0 ] );
