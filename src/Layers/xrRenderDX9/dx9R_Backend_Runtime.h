...
IC void CBackend::set_xform( u32 ID, const Fmatrix& M ) {
    stat.xforms++;
    CHK_DX( HW.pDevice->SetTransform( ( D3DTRANSFORMSTATETYPE )ID,
                                      ( D3DMATRIX* )&M ) );
}
...
ICF void CBackend::Render( D3DPRIMITIVETYPE T,
                           u32 baseV,
                           u32 startV,
                           u32 countV,
                           u32 startI,
    ...
    stat.polys += PC;
    constants.flush();
    CHK_DX( HW.pDevice->DrawIndexedPrimitive( T, baseV, startV, countV, startI,
                                              PC ) );
...
IC void CBackend::set_Stencil( u32 _enable,
    ...
    if ( stencil_writemask != _writemask ) {
        ...
        CHK_DX(
            HW.pDevice->SetRenderState( D3DRS_STENCILWRITEMASK, _writemask ) );
    }
