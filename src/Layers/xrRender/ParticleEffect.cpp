...
void PS::OnEffectParticleBirth( void* owner, u32, PAPI::Particle& m, u32 ) {
    ...
    if ( PED ) {
        if ( PED->m_Flags.is( CPEDef::dfRandomFrame ) )
            m.frame = ( u16 )iFloor(
                Random.randI( PED->m_Frame.m_iFrameCount ) * 255.f );
        if ( PED->m_Flags.is( CPEDef::dfAnimated ) &&
             PED->m_Flags.is( CPEDef::dfRandomPlayback ) && Random.randI( 2 ) )
...
void CParticleEffect::Stop( BOOL bDefferedStop ) {
    ParticleManager()->StopEffect( m_HandleEffect, m_HandleActionList,
                                   bDefferedStop );
    if ( bDefferedStop ) {
...
void CParticleEffect::UpdateParent( const Fmatrix& m,
...
void CParticleEffect::OnFrame( u32 frame_dt ) {
    if ( 0 == m_RT_Flags.is( flRT_LiveUpdate ) ) {
        if ( m_Def && m_RT_Flags.is( flRT_Playing ) ) {
            ...
            if ( m_MemDT >= uDT_STEP ) {
                // allow maximum of three steps (99ms) to avoid slowdown after
                // loading it will really skip updates at less than 10fps, which
                // is unplayable
            ...
            for ( ; StepCount; StepCount-- ) {
                ...
                ParticleManager()->Update( m_HandleEffect, m_HandleActionList,
                                           fDT_STEP );

                PAPI::Particle* particles;
                u32 p_cnt;
                ParticleManager()->GetParticles( m_HandleEffect, particles,
                                                 p_cnt );
                ...
                if ( p_cnt ) {
                    ...
                    for ( u32 i = 0; i < p_cnt; i++ ) {
                        ...
                        if ( !RImplementation.ViewBase.testSphere_dirty(
                                 vis.sphere.P, vis.sphere.R ) ) {
                            m.posI.set( m.pos );
                            m.rotI.set( m.rot );
                            m.velI.set( m.vel );
                            m.sizeI.set( m.size );
        ...
        if ( m_Def && m_RT_Flags.is( flRT_Playing ) ) {
            ...
            if ( m_Def->m_Flags.is( CPEDef::dfCollision ) )
                m_Def->ExecuteCollision( particles, p_cnt, Device.fTimeDelta,
                                         this, m_CollisionCallback );
            if ( p_cnt ) {
                vis.box.invalidate();
                float p_size = 0.f;
                for ( u32 i = 0; i < p_cnt; i++ ) {
            ...
            if ( deffered_stop && m_RT_Flags.is( flRT_DefferedStop ) &&
                 ( 0 == p_cnt ) )
                m_RT_Flags.set( flRT_Playing | flRT_DefferedStop, FALSE );
        } else {
            vis.box.set( m_InitialPosition, m_InitialPosition );
...
BOOL CParticleEffect::Compile( CPEDef* def ) {
    ...
    if ( m_Def ) {
        ...
        ParticleManager()->SetMaxParticles( m_HandleEffect,
                                            m_Def->m_MaxParticles );
        ParticleManager()->SetCallback( m_HandleEffect, OnEffectParticleBirth,
                                        OnEffectParticleDead, this, 0 );
        // time limit
        if ( m_Def->m_Flags.is( CPEDef::dfTimeLimit ) )
            m_fElapsedLimit = m_Def->m_fTimeLimit;
    }
...
void CParticleEffect::SetBirthDeadCB( PAPI::OnBirthParticleCB bc,
...
#ifndef _EDITOR
//----------------------------------------------------
IC void FillSprite_fpu( FVF::LIT*& pv,
                        const Fvector& T,
                        const Fvector& R,
                        const Fvector& pos,
                        const Fvector2& lt,
...
IC void FillSprite( FVF::LIT*& pv,
    ...
    Vr = _mm_mul_ps( _mm_set1_ps( r1 ), _mm_add_ps( _mm_mul_ps( _T, _sa ),
                                                    _mm_mul_ps( _R, _ca ) ) );
    Vt = _mm_mul_ps( _mm_set1_ps( r2 ), _mm_sub_ps( _mm_mul_ps( _T, _ca ),
                                                    _mm_mul_ps( _R, _sa ) ) );
...
IC void FillSprite( FVF::LIT*& pv,
    ...
    _r1 = _mm_shuffle_ps( _r1, _r1,
                          _MM_SHUFFLE( 1, 1, 1, 1 ) ); // zz | zz | zz | zz
...
bool ParticleRenderStream( FVF::LIT* pv,
    ...
    for ( u32 i = 0; i != count; ++i ) {
        ...
        if ( Device.vCameraPosition.distance_to_sqr( wp_eff ) >
             _sqr(
        ...
        if ( pPE->m_Def->m_Flags.is( CPEDef::dfFramed ) )
            pPE->m_Def->m_Frame.CalculateTC( iFloor( float( m.frame ) / 255.f ),
                                             lt, rb );
        ...
        if ( pPE->m_Def->m_Flags.is( CPEDef::dfAlignToPath ) ) {
            if ( !speed_calculated )
                magnitude_sse( m.velI, speed );
            if ( ( speed < EPS_S ) &&
                 pPE->m_Def->m_Flags.is( CPEDef::dfWorldAlign ) ) {
                Fmatrix M;
                M.setXYZ( pPE->m_Def->m_APDefaultRotation );
                if ( pPE->m_RT_Flags.is( CParticleEffect::flRT_XFORM ) ) {
                    M.mulA_43( pPE->m_XFORM );
                    FillSprite(
                        pv, M.k, M.i, wp_eff, lt, rb, r_x, r_y,
                        color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                        sina, cosa );
                } else {
                    FillSprite(
                        pv, M.k, M.i, m.posI, lt, rb, r_x, r_y,
                        color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                        sina, cosa );
                }
            } else if ( ( speed >= EPS_S ) &&
                        pPE->m_Def->m_Flags.is( CPEDef::dfFaceAlign ) ) {
                Fmatrix M;
                M.identity();
                ...
                if ( pPE->m_RT_Flags.is( CParticleEffect::flRT_XFORM ) ) {
                    M.mulA_43( pPE->m_XFORM );
                    FillSprite(
                        pv, M.j, M.i, wp_eff, lt, rb, r_x, r_y,
                        color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                        sina, cosa );
                } else {
                    FillSprite(
                        pv, M.j, M.i, m.posI, lt, rb, r_x, r_y,
                        color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                        sina, cosa );
                }
            } else {
                ...
                if ( speed >= EPS_S )
                    ...
                    dir.setHP( -pPE->m_Def->m_APDefaultRotation.y,
                               -pPE->m_Def->m_APDefaultRotation.x );
                if ( pPE->m_RT_Flags.is( CParticleEffect::flRT_XFORM ) ) {
                    Fvector d;
                    pPE->m_XFORM.transform_dir( d, dir );
                    FillSprite(
                        pv, wp_eff, d, lt, rb, r_x, r_y,
                        color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                        sina, cosa );
                } else {
                    FillSprite(
                        pv, m.posI, dir, lt, rb, r_x, r_y,
                        color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                        sina, cosa );
                }
            }
            ...
            if ( pPE->m_RT_Flags.is( CParticleEffect::flRT_XFORM ) ) {
                FillSprite(
                    pv, RDEVICE.vCameraTop, RDEVICE.vCameraRight, wp_eff, lt,
                    rb, r_x, r_y,
                    color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                    sina, cosa );
            } else {
                FillSprite(
                    pv, RDEVICE.vCameraTop, RDEVICE.vCameraRight, m.posI, lt,
                    rb, r_x, r_y,
                    color_rgba_f( m.colorR, m.colorG, m.colorB, m.colorA ),
                    sina, cosa );
            }
...
void CParticleEffect::Render( float ) {
    ...
    if ( p_cnt > 0 ) {
        if ( m_Def && m_Def->m_Flags.is( CPEDef::dfSprite ) ) {
            FVF::LIT* pv_start = ( FVF::LIT* )RCache.Vertex.Lock(
                p_cnt * 4 * 4, geom->vb_stride, dwOffset );
            bool NeedExit =
            ...
            if ( dwCount ) {
                ...
                RCache.set_CullMode(
                    m_Def->m_Flags.is( CPEDef::dfCulling )
                        ? ( m_Def->m_Flags.is( CPEDef::dfCullCCW ) ? CULL_CCW
                                                                   : CULL_CW )
                        : CULL_NONE );
                RCache.Render( D3DPT_TRIANGLELIST, dwOffset, 0, dwCount, 0,
                               dwCount / 2 );
    ...
    if ( p_cnt > 0 ) {
        if ( m_Def && m_Def->m_Flags.is( CPEDef::dfSprite ) ) {
            ...
            for ( u32 i = 0; i < p_cnt; i++ ) {
                ...
                rb.set( 1.f, 1.f );
                if ( m_Def->m_Flags.is( CPEDef::dfFramed ) )
                    m_Def->m_Frame.CalculateTC(
                        iFloor( float( m.frame ) / 255.f ), lt, rb );
                ...
                if ( m_Def->m_Flags.is( CPEDef::dfAlignToPath ) ) {
                    float speed = m.vel.magnitude();
                    if ( ( speed < EPS_S ) &&
                         m_Def->m_Flags.is( CPEDef::dfWorldAlign ) ) {
                        Fmatrix M;
                        ...
                        if ( m_RT_Flags.is( flRT_XFORM ) ) {
                            ...
                            M.mulA_43( m_XFORM );
                            FillSprite( pv, M.k, M.i, p, lt, rb, r_x, r_y,
                                        m.color, m.rot.x );
                        } else {
                            FillSprite( pv, M.k, M.i, m.pos, lt, rb, r_x, r_y,
                                        m.color, m.rot.x );
                        }
                    } else if ( ( speed >= EPS_S ) &&
                                m_Def->m_Flags.is( CPEDef::dfFaceAlign ) ) {
                        Fmatrix M;
                        M.identity();
                        M.k.div( m.vel, speed );
                        M.j.set( 0, 1, 0 );
                        ...
                        if ( m_RT_Flags.is( flRT_XFORM ) ) {
                            ...
                            m_XFORM.transform_tiny( p, m.pos );
                            M.mulA_43( m_XFORM );
                            FillSprite( pv, M.j, M.i, p, lt, rb, r_x, r_y,
                                        m.color, m.rot.x );
                        } else {
                            FillSprite( pv, M.j, M.i, m.pos, lt, rb, r_x, r_y,
                                        m.color, m.rot.x );
                        }
                    } else {
                        Fvector dir;
                        if ( speed >= EPS_S )
                            ...
                            dir.setHP( -m_Def->m_APDefaultRotation.y,
                                       -m_Def->m_APDefaultRotation.x );
                        if ( m_RT_Flags.is( flRT_XFORM ) ) {
                            Fvector p, d;
                            m_XFORM.transform_tiny( p, m.pos );
                            m_XFORM.transform_dir( d, dir );
                            FillSprite( pv, p, d, lt, rb, r_x, r_y, m.color,
                                        m.rot.x );
                        } else {
                            FillSprite( pv, m.pos, dir, lt, rb, r_x, r_y,
                                        m.color, m.rot.x );
                        }
                    }
                } else {
                    if ( m_RT_Flags.is( flRT_XFORM ) ) {
                        Fvector p;
                        m_XFORM.transform_tiny( p, m.pos );
                        FillSprite( pv, RDEVICE.vCameraTop,
                                    RDEVICE.vCameraRight, p, lt, rb, r_x, r_y,
                                    m.color, m.rot.x );
                    } else {
                        FillSprite( pv, RDEVICE.vCameraTop,
                                    RDEVICE.vCameraRight, m.pos, lt, rb, r_x,
                                    r_y, m.color, m.rot.x );
                    }
                }
            }
