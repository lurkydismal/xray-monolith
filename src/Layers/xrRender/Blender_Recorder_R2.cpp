...
void CBlender_Compile::r_ColorWriteEnable( bool cR,
                                           bool cG,
                                           bool cB,
                                           bool cA ) {
    BYTE Mask = 0;
...
u32 CBlender_Compile::i_Sampler( LPCSTR _name ) {
    ...
    // Create texture
    // while (stage>=passTextures.size())	passTextures.push_back
    // (NULL);
    return stage;
}
...
void CBlender_Compile::i_Texture( u32 s, LPCSTR name ) {
    if ( name )
        passTextures.push_back(
            mk_pair( s, ref_texture( DEV->_CreateTexture( name ) ) ) );
}

void CBlender_Compile::i_Projective( u32 s, bool b ) {
    if ( b )
        RS.SetTSS( s, D3DTSS_TEXTURETRANSFORMFLAGS,
                   D3DTTFF_DISABLE | D3DTTFF_PROJECTED );
    else
...
u32 CBlender_Compile::r_Sampler( LPCSTR _name,
                                 LPCSTR texture,
                                 bool b_ps1x_ProjectiveDivide,
                                 u32 address,
                                 u32 fmin,
                                 u32 fmip,
                                 u32 fmag ) {
    ...
    if ( u32( -1 ) != dwStage ) {
        ...
        if ( 0 == xr_strcmp( _name, "s_base_hud" ) ) {
            ...
            // D3DTEXF_PYRAMIDALQUAD; //D3DTEXF_ANISOTROPIC; //D3DTEXF_LINEAR;
        ...
        //.i_Filter
        //(dwStage,D3DTEXF_POINT,D3DTEXF_POINT,D3DTEXF_POINT); // show pixels
        if ( dwStage < 4 )
...
void CBlender_Compile::r_Sampler_rtf( LPCSTR name,
                                      LPCSTR texture,
                                      bool b_ps1x_ProjectiveDivide ) {
    r_Sampler( name, texture, b_ps1x_ProjectiveDivide, D3DTADDRESS_CLAMP,
               D3DTEXF_POINT, D3DTEXF_NONE, D3DTEXF_POINT );
}

void CBlender_Compile::r_Sampler_clf( LPCSTR name,
                                      LPCSTR texture,
                                      bool b_ps1x_ProjectiveDivide ) {
    r_Sampler( name, texture, b_ps1x_ProjectiveDivide, D3DTADDRESS_CLAMP,
...
void CBlender_Compile::r_Sampler_clw( LPCSTR name,
                                      LPCSTR texture,
                                      bool b_ps1x_ProjectiveDivide ) {
    u32 s =
        r_Sampler( name, texture, b_ps1x_ProjectiveDivide, D3DTADDRESS_CLAMP,
