...
IC void MouseRayFromPoint( Fvector& direction,
                           int x,
                           int y,
                           Fmatrix& m_CamMat ) {
...
#if defined( USE_DX10 ) || defined( USE_DX11 )
void CRender::ScreenshotImpl( ScreenshotMode mode,
                              LPCSTR name,
    ...
    switch ( mode ) {
        case IRender_interface::SM_FOR_GAMESAVE: {
            ...
            CHK_DX(
                HW.pDevice->CreateTexture2D( &desc, NULL, &pSrcSmallTexture ) );
...
#else
            CHK_DX( D3DX10LoadTextureFromTexture( pSrcTexture, NULL,
                                                  pSrcSmallTexture ) );
#endif
            ...
            HRESULT hr = D3DX11SaveTextureToMemory(
                HW.pContext, pSrcSmallTexture, D3DX11_IFF_DDS, &saved, 0 );
#else
            HRESULT hr = D3DX10SaveTextureToMemory( pSrcSmallTexture,
                                                    D3DX10_IFF_DDS, &saved, 0 );
            // HRESULT hr					=
            // D3DXSaveTextureToFileInMemory (&saved,D3DXIFF_DDS,texture,0);
#endif
            if ( hr == D3D_OK ) {
                IWriter* fs = FS.w_open( name );
                if ( fs ) {
                    fs->w( saved->GetBufferPointer(),
                           ( u32 )saved->GetBufferSize() );
                    FS.w_close( fs );
                }
            }
            _RELEASE( saved );
        ...
        case IRender_interface::SM_FOR_MPSENDING: {
            ...
            if ( hr == D3D_OK ) {
                if ( !memory_writer ) {
                    ...
                    memory_writer->w( saved->GetBufferPointer(),
                                      ( u32 )saved->GetBufferSize() );
                }
            }
            _RELEASE( saved );
        ...
        case IRender_interface::SM_NORMAL: {
            ...
            string_path buf;
            xr_sprintf(
                buf, sizeof( buf ), "ss_%s_%s_(%s).%s", Core.UserName,
                timestamp( t_stemp ),
                ( g_pGameLevel ) ? g_pGameLevel->name().c_str() : "mainmenu",
                ps_r_screenshot_token == 0
                    ? "jpg"
                    : ( ps_r_screenshot_token == 1 ? "png" : "tga" ) );
            ID3DBlob* saved = 0;
#ifdef USE_DX11
            CHK_DX( D3DX11SaveTextureToMemory(
                HW.pContext, pSrcTexture,
                ps_r_screenshot_token == 0
                    ? D3DX11_IFF_JPG
                    : ( ps_r_screenshot_token == 1 ? D3DX11_IFF_PNG
                                                   : D3DX11_IFF_BMP ),
                &saved, 0 ) );
#else
            CHK_DX( D3DX10SaveTextureToMemory(
                pSrcTexture,
                ps_r_screenshot_token == 0
                    ? D3DX10_IFF_JPG
                    : ( ps_r_screenshot_token == 1 ? D3DX10_IFF_PNG
                                                   : D3DX10_IFF_BMP ),
                &saved, 0 ) );
#endif
        ...
        case IRender_interface::SM_FOR_CUBEMAP: {
            ...
            // Swap r and b, but don't kill alpha
            {
                ...
                for ( ; pPixel != pEnd; pPixel++ ) {
                    u32 p = *pPixel;
                    *pPixel = color_argb( color_get_A( p ), color_get_B( p ),
                                          color_get_G( p ), color_get_R( p ) );
                }
            }

            u32* data =
                ( u32* )xr_malloc( Device.dwHeight * Device.dwHeight * 4 );
            imf_Process( data, Device.dwHeight, Device.dwHeight,
                         ( u32* )MappedData.pData, Device.dwWidth,
...
void CRender::ScreenshotImpl( ScreenshotMode mode,
    ...
    HRESULT hr = HW.pDevice->CreateOffscreenPlainSurface(
        Device.dwWidth, Device.dwHeight, HW.DevPP.BackBufferFormat,
    ...
    for ( ; pPixel != pEnd; pPixel++ ) {
        u32 p = *pPixel;
        *pPixel =
            color_xrgb( color_get_R( p ), color_get_G( p ), color_get_B( p ) );
    }
    ...
    switch ( mode ) {
        case IRender_interface::SM_FOR_GAMESAVE: {
            ...
            hr = D3DXCreateTexture( HW.pDevice, GAMESAVE_SIZE, GAMESAVE_SIZE, 1,
                                    0, D3DFMT_DXT1, D3DPOOL_SCRATCH, &texture );
            if ( hr != D3D_OK )
                goto _end_;
            if ( NULL == texture )
                goto _end_;
            ...
            if ( hr != D3D_OK )
                goto _end_;
            VERIFY( surface );
            hr = D3DXLoadSurfaceFromSurface( surface, 0, 0, pFB, 0, 0,
                                             D3DX_DEFAULT, 0 );
            _RELEASE( surface );
            if ( hr != D3D_OK )
                goto _end_;
            ...
            // save (logical & physical)
            ID3DBlob* saved = 0;
            hr = D3DXSaveTextureToFileInMemory( &saved, D3DXIFF_DDS, texture,
                                                0 );
            if ( hr != D3D_OK )
                goto _end_;
        ...
        case IRender_interface::SM_FOR_MPSENDING: {
            // texture
            ID3DTexture2D* texture = NULL;
            hr = D3DXCreateTexture( HW.pDevice, SM_FOR_SEND_WIDTH,
                                    SM_FOR_SEND_HEIGHT, 1, 0, D3DFMT_R8G8B8,
        ...
        case IRender_interface::SM_NORMAL: {
            ...
            xr_sprintf(
                ...
                ps_r_screenshot_token == 0
                    ...
                    : ( ps_r_screenshot_token == 1 ? "png" : "tga" ) );
            ID3DBlob* saved = 0;
            CHK_DX( D3DXSaveSurfaceToFileInMemory(
                &saved,
                ps_r_screenshot_token == 0
                    ? D3DXIFF_JPG
                    : ( ps_r_screenshot_token == 1 ? D3DXIFF_PNG
                                                   : D3DXIFF_TGA ),
                pFB, 0, 0 ) );
            IWriter* fs = FS.w_open( "$screenshots$", buf );
            R_ASSERT( fs );
        ...
        case IRender_interface::SM_FOR_CUBEMAP: {
            ...
            u32* data =
                ( u32* )xr_malloc( Device.dwHeight * Device.dwHeight * 4 );
            imf_Process( data, Device.dwHeight, Device.dwHeight,
                         ( u32* )D.pBits, Device.dwWidth, Device.dwHeight,
                         imf_lanczos3 );
            p.scanlenght = Device.dwHeight * 4;
            p.width = Device.dwHeight;
            p.height = Device.dwHeight;
...
void CRender::ScreenshotAsyncEnd( CMemoryWriter& memory_writer ) {
...

    {
        ...
        for ( ; pPixel != pEnd; pPixel++ ) {
            u32 p = *pPixel;
            *pPixel = color_xrgb( color_get_B( p ), color_get_G( p ),
                                  color_get_R( p ) );
        }

        memory_writer.w( &Device.dwWidth, sizeof( Device.dwWidth ) );
        memory_writer.w( &Device.dwHeight, sizeof( Device.dwHeight ) );
        memory_writer.w( MappedData.pData,
                         ( Device.dwWidth * Device.dwHeight ) * 4 );
    }
...
void CRender::ScreenshotAsyncEnd( CMemoryWriter& memory_writer ) {
    ...
    if ( Target->rt_Color->fmt == D3DFMT_A16B16G16R16F ) {
        ...
        while ( pPixel != pEnd ) {
            const int iProcessPixels =
                _min( iMaxPixelsInARow, ( s32 )( pEnd - pPixel ) );

            D3DXFloat16To32Array( tmpArray, pPixelElement16,
                                  iProcessPixels * 4 );

            for ( int i = 0; i < iProcessPixels; ++i ) {
                *pPixel = color_argb_f( 1.0f, tmpArray[ i * 4 ],
                                        tmpArray[ i * 4 + 1 ],
                                        tmpArray[ i * 4 + 2 ] );
...
#endif //	RENDER != R_R1
    {
        for ( ; pPixel != pEnd; pPixel++ ) {
            u32 p = *pPixel;
            *pPixel = color_xrgb( color_get_R( p ), color_get_G( p ),
                                  color_get_B( p ) );
        }
    }
...
#if defined( USE_DX10 ) || defined( USE_DX11 )
void CRender::TakeScreenshot( LPCSTR path,
                              Fvector2 dimensions,
                              DxEncoding encoding ) {
    // demonized: Disable screenshots if HDR is enabled
    if ( ps_r4_hdr10_on ) {
    ...
    string_path fname;
    if ( 0 == strext( path ) )
        strconcat( sizeof( fname ), fname, path, ".dds" );
    else
        xr_strcpy( fname, sizeof( fname ), path );
...
#ifdef USE_DX11
    CHK_DX( D3DX11LoadTextureFromTexture( HW.pContext, pSrcTexture, NULL,
                                          pSrcSmallTexture ) );
#else
    CHK_DX(
        D3DX10LoadTextureFromTexture( pSrcTexture, NULL, pSrcSmallTexture ) );
#endif
...
#else
    HRESULT hr = D3DX10SaveTextureToMemory( pSrcSmallTexture, D3DX10_IFF_DDS,
                                            &saved, 0 );
    // HRESULT hr					=
    // D3DXSaveTextureToFileInMemory (&saved,D3DXIFF_DDS,texture,0);
#endif
...
void CRender::TakeScreenshot( LPCSTR path,
    ...
    ID3DTexture2D* texture = NULL;
    hr = D3DXCreateTexture( HW.pDevice, u32( width ), u32( height ), 1, 0,
                            dx_encoding, D3DPOOL_SCRATCH, &texture );
    if ( hr != D3D_OK )
        goto _end_;
    if ( NULL == texture )
        goto _end_;
    ...
    hr =
        D3DXLoadSurfaceFromSurface( surface, 0, 0, pFB, 0, 0, D3DX_DEFAULT, 0 );
    _RELEASE( surface );
    if ( hr != D3D_OK )
        goto _end_;
    // save (logical & physical)
