...
void dxThunderboltRender::Render( CEffect_Thunderbolt& owner ) {
    ...
    dxThunderboltDescRender* pThRen =
        ( dxThunderboltDescRender* )&*owner.current->m_pRender;
    ...
    IRender_DetailModel::fvfVertexOut* v_ptr =
        ( IRender_DetailModel::fvfVertexOut* )RCache.Vertex.Lock(
            vCount_Lock, hGeom_model->vb_stride, v_offset );
    u16* i_ptr = RCache.Index.Lock( iCount_Lock, i_offset );
    // XForm verts
    pThRen->l_model->transfer( owner.current_xform, v_ptr, 0xffffffff, i_ptr, 0,
                               0.f, dv );
    // Flush if needed
    ...
    Fvector vecSx, vecSy;
    u32 VS_Offset;
    FVF::LIT* pv = ( FVF::LIT* )RCache.Vertex.Lock( 8, hGeom_gradient.stride(),
                                                    VS_Offset );
    // top
    {
        u32 c_val = iFloor( owner.current->m_GradientTop->fOpacity *
                            owner.lightning_phase * 255.f );
        u32 c = color_rgba( c_val, c_val, c_val, c_val );
        vecSx.mul(
            Device.vCameraRight,
            owner.current->m_GradientTop->fRadius.x * owner.lightning_size );
        vecSy.mul( Device.vCameraTop, -owner.current->m_GradientTop->fRadius.y *
                                          owner.lightning_size );
        pv->set( owner.current_xform.c.x + vecSx.x - vecSy.x,
                 owner.current_xform.c.y + vecSx.y - vecSy.y,
    ...
    // center
    {
        ...
        vecSx.mul(
            Device.vCameraRight,
            owner.current->m_GradientCenter->fRadius.x * owner.lightning_size );
        vecSy.mul( Device.vCameraTop,
                   -owner.current->m_GradientCenter->fRadius.y *
                       owner.lightning_size );
        pv->set( owner.lightning_center.x + vecSx.x - vecSy.x,
                 owner.lightning_center.y + vecSx.y - vecSy.y,
    ...
    RCache.set_Shader(
        ( ( dxFlareRender* )&*owner.current->m_GradientTop->m_pFlare )
            ->hShader );

#if defined( USE_DX10 ) || defined( USE_DX11 )
    //	Hack. Since lightning gradient uses sun shader override z write settings
    // manually
    RCache.set_Z( TRUE );
    ...
    RCache.set_Shader(
        ( ( dxFlareRender* )&*owner.current->m_GradientCenter->m_pFlare )
            ->hShader );
