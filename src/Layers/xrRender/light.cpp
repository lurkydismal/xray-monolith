...
light::light() {
    ISpatialOwner::spatial_create( g_SpatialSpaceLights, this,
                                   STYPE_LIGHTSOURCE );
...
void light::destroy( bool deffered ) {
    set_active( false );
    if ( deffered ) {
        if ( std::find( RImplementation.v_all_lights_dque.begin(),
                        RImplementation.v_all_lights_dque.end(),
                        this ) == RImplementation.v_all_lights_dque.end() )
    ...
    if ( RImplementation.o.dx10_msaa ) {
        ...
        for ( int i = 0; i < bound; ++i ) {
            s_spot_msaa[ i ].create(
                RImplementation.Target->b_accum_spot_msaa[ i ],
                strconcat( sizeof( temp ), temp, "r2\\accum_spot_", name ),
                name );
            s_volumetric_msaa[ i ].create(
                RImplementation.Target->b_accum_volumetric_msaa[ i ],
                strconcat( sizeof( temp ), temp, "r2\\accum_volumetric_",
                           name ),
                name );
        }
    }
...
void light::set_rotation( const Fvector& D,
                          const Fvector& R,
                          const float eps ) {
...
//-X,				+Y,				-Y,
//+Z,				-Z
static Fvector cmNorm[ 6 ] = { { 0.f, 1.f, 0.f },  { 0.f, 1.f, 0.f },
                               { 0.f, 0.f, -1.f }, { 0.f, 0.f, 1.f },
                               { 0.f, 1.f, 0.f },  { 0.f, 1.f, 0.f } };
static Fvector cmDir[ 6 ] = { { 1.f, 0.f, 0.f }, { -1.f, 0.f, 0.f },
                              { 0.f, 1.f, 0.f }, { 0.f, -1.f, 0.f },
                              { 0.f, 0.f, 1.f }, { 0.f, 0.f, -1.f } };
...
void light::get_sectors() {
    ...
    if ( flags.type ==
         IRender_Light::SPOT /* || flags.type == IRender_Light::OMNIPART*/ ) {
        RImplementation.detectSectors_frustum( sector, m_sectors,
                                               &X.S.frustum );
    }
    if ( flags.type == IRender_Light::POINT ) {
        RImplementation.detectSectors_sphere(
            sector, m_sectors, position, Fvector().set( range, range, range ) );
    }
}
...
void light::spatial_move() {
    switch ( flags.type ) {
        ...
        case IRender_Light::SPOT: {
            ...
            VERIFY2( cone < deg2rad( 121.f ),
                     "Too large light-cone angle. Maybe you have passed it in "
                     "'degrees'?" );
            if ( cone >= PI_DIV_2 ) {
                // obtused-angled
                SpatialComponent->spatial.sphere.P.mad( position, direction,
                                                        range );
                SpatialComponent->spatial.sphere.R = range * tanf( cone / 2.f );
            } else {
                // acute-angled
                SpatialComponent->spatial.sphere.R =
                    range / ( 2.f * _sqr( _cos( cone / 2.f ) ) );
                SpatialComponent->spatial.sphere.P.mad(
                    position, direction, SpatialComponent->spatial.sphere.R );
            }
        } break;
...
vis_data& light::get_homdata() {
    // commit vis-data
    hom.sphere.set( SpatialComponent->spatial.sphere.P,
                    SpatialComponent->spatial.sphere.R );
    hom.box.set( SpatialComponent->spatial.sphere.P,
...
void light::xform_calc() {
    if ( Device.dwFrame == m_parent_u_frame )
        return;
    ...
    if ( flags.type == IRender_Light::SPOT ||
         flags.type == IRender_Light::OMNIPART ) {
        // make N pixel border
        X.S.view.build_camera_dir( position, L_dir, L_up );

        // _min(L->cone + deg2rad(4.5f), PI*0.98f) - Here, it is needed to
        // enlarge the shadow map frustum to include also displaced pixels and
        // the pixels neighbor to the examining one.
        /* Ray Twitty */
        float tan_shift;
#if RENDER == R_R4
        if ( flags.type ==
             IRender_Light::OMNIPART ) // [ SSS ] 0.3f fix almost all frustum
                                       // problems... 0.5f was the old value (
                                       // SSS 19 ) but was causing issues?
            tan_shift = 0.3f;
...
#endif
        /* Ray Twitty end */
        X.S.project.build_projection( cone + tan_shift, 1.f, virtual_size,
                                      range + EPS_S );

        X.S.combine.mul( X.S.project, X.S.view );

        X.S.frustum.CreateFromMatrix( X.S.combine,
...
void light::optimize_smap_size() {
    ...
    // Compute approximate screen area (treating it as an point light) -
    // R*R/dist_sq Note: we clamp screen space area to ONE, although it is not
    // correct at all
    //
    // float	dist =
    // Device.vCameraPosition.distance_to(SpatialComponent->spatial.sphere.P) -
    // SpatialComponent->spatial.sphere.R; if (dist < 0)	dist = 0; float
    // ssa = clampr(range * range / (1.f + dist * dist), 0.f, 1.f);

    float dist_sqr = Device.vCameraPosition.distance_to_sqr(
        SpatialComponent->spatial.sphere.P );
    float R = SpatialComponent->spatial.sphere.R;
    float R_sqr = _sqr( R );
    float ssa = dist_sqr < R_sqr
                    ? 1.f
                    : clampr( _sqr( range ) / ( 1.f + ( dist_sqr - R_sqr ) ),
                              0.f, 1.f );

    // compute intensity
    // float	intensity0			= (L->color.r + L->color.g +
    // L->color.b)/3.f; float	intensity1			= (L->color.r *
    // 0.2125f + L->color.g * 0.7154f + L->color.b * 0.0721f); float
    // intensity			= (intensity0+intensity1)/2.f;
    // // intensity1 tends to underestimate...

    // compute how much duelling frusta occurs	[-1..1]-> 1 + [-0.5 .. +0.5]
    float duel_dot =
        1.f - 0.5f * Device.vCameraDirection.dotproduct( direction );

    // compute how large the light is - give more texels to larger lights,
    // assume 8m as being optimal radius
    ...
    // float	factor1				= powf	(intensity, 0.0625f);
    // // less perceptually important?
    float factor2 =
        powf( duel_dot, 0.25f ); // difficult to fast-change this -> visible
    float factor3 =
        powf( sizefactor, 0.25f ); // this shouldn't make much difference
    float factor4 = powf( widefactor, 0.5f ); // make it linear ???
    float factor =
        ps_r2_ls_squality * factor0 /** factor1*/ * factor2 * factor3 * factor4;
            ...
            case IRender_Light::POINT: {
                ...
                for ( int f = 0; f < 6; f++ ) {
                    ...
                    L->SpatialComponent->spatial.sector =
                        SpatialComponent->spatial.sector;
                    L->set_range( range );
                    L->set_color( color );
                    L->omnipart_num = f;
                    ...
                    for ( int f = 0; f < 6; f++ )
                        L->set_decor_object( decor_object[ f ], f );
...
float light::get_LOD() {
    if ( !flags.bShadow )
        return 1;
    float distSQ = Device.vCameraPosition.distance_to_sqr(
                       SpatialComponent->spatial.sphere.P ) +
                   EPS;
    distance = distSQ;
    float ssa = ps_r2_slight_fade * SpatialComponent->spatial.sphere.R / distSQ;
    float lod = _sqrt(
        clampr( ( ssa - r_ssaGLOD_end ) / ( r_ssaGLOD_start - r_ssaGLOD_end ),
                0.f, 1.f ) );
    return lod;
}
