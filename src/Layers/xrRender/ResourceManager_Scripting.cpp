...
public:
    adopt_sampler( CBlender_Compile* _C, u32 _stage )
        : C( _C ), stage( _stage ) {
        if ( u32( -1 ) == stage )
            C = 0;
    }
    ...
    adopt_sampler& _f_anisotropic() {
        if ( C )
            C->i_Filter( stage, D3DTEXF_ANISOTROPIC, D3DTEXF_LINEAR,
                         D3DTEXF_ANISOTROPIC );
        return *this;
    }
    ...
    adopt_sampler& _f_trilinear() {
        if ( C )
            C->i_Filter( stage, D3DTEXF_LINEAR, D3DTEXF_LINEAR,
                         D3DTEXF_LINEAR );
        return *this;
    }
    ...
    module( LSVM )
        [ class_< adopt_sampler >( "_sampler" )
          ...
          class_< adopt_compiler >( "_compiler" )
              ...
              .def( "color_write_enable", &adopt_compiler::_color_write_enable,
                    return_reference_to< 1 >() )
              .def( "sampler",
                    &adopt_compiler::_sampler ), // returns sampler-object
