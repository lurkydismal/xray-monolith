...
void _VertexStream::Create() {
    ...
    R_CHK( HW.pDevice->CreateVertexBuffer(
        mSize, D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0, D3DPOOL_DEFAULT, &pVB,
        NULL ) );
    HW.stats_manager.increment_stats_vb( pVB );
#endif //	USE_DX10
...
void* _VertexStream::Lock( u32 vl_Count, u32 Stride, u32& vOffset ) {
    ...
    R_ASSERT2( ( bytes_need <= mSize ) && vl_Count,
               make_string( "bytes_need = %d, mSize = %d, vl_Count = %d",
    ...
    if ( ( vl_Count + vl_mPosition ) >= vl_mSize ) {
        ...
        pData += vOffset;
#else //	USE_DX10
        HRESULT res = pVB->Lock( mPosition, bytes_need, ( void** )&pData,
                                 LOCKFLAGS_FLUSH );

        if ( res != D3D_OK )
            Msg( " pVB->Lock - failed: res = %d,mPosition = %d, bytes_need = "
                 "%d, &pData = %x, LOCKFLAGS_FLUSH",
                 res, mPosition, bytes_need, ( void** )&pData );
...
#if defined( USE_DX11 )
        HW.pContext->Map( pVB, 0, D3D_MAP_WRITE_NO_OVERWRITE, 0,
                          &MappedSubRes );
        pData = ( BYTE* )MappedSubRes.pData;
...
void _IndexStream::Create() {
    ...
    R_CHK( HW.pDevice->CreateIndexBuffer(
        mSize, D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, D3DFMT_INDEX16,
...
u16* _IndexStream::Lock( u32 Count, u32& vOffset ) {
    ...
    D3D_MAP MapMode = ( dwFlags == LOCKFLAGS_APPEND )
                          ? D3D_MAP_WRITE_NO_OVERWRITE
                          : D3D_MAP_WRITE_DISCARD;
