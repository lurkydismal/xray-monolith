...
void i_scan( int curY,
    ...
    float Z =
        startZ + ( minT - startR ) / lenR * Zlen; // interpolate Z to the start
    float Zend =
        startZ + ( maxT - startR ) / lenR * Zlen; // interpolate Z to the end
    float dZ = ( Zend - Z ) / ( maxT - minT ); // increment in Z / pixel wrt dX

    // Move to far my dz/5 to place the pixel at the center of face that it
    // covers. This will make sure that objects will not be clipped for just
    // standing next to the home from outside.
    Z += 0.5f * _abs( dZ );
...
/*
Rasterises 1 section of the triangle a 'section' of a triangle is the portion of
a triangle between 2 horizontal scan lines corresponding to 2 vertices of a
triangle p2.y >= p1.y, p1, p2 are start/end vertices E1 E2 are the triangle edge
differences of the 2 bounding edges for this section
*/
...
u32 occRasterizer::rasterize( occTri* T ) {
    ...
    currentTri = T;
    dwPixels = 0;
    i_order( &( T->raster[ 0 ].x ), &( T->raster[ 1 ].x ),
             &( T->raster[ 2 ].x ) );
