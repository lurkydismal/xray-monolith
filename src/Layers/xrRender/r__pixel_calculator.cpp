...
void r_pixel_calculator::begin() {
    ...
    R_CHK( HW.pDevice->CreateDepthStencilSurface(
        rt_dimensions, rt_dimensions, HW.Caps.fDepth, D3DMULTISAMPLE_NONE, 0,
        TRUE, &zb, NULL ) );
...
r_aabb_ssa r_pixel_calculator::calculate( dxRender_Visual* V ) {
    ...
    for ( u32 face = 0; face < 6; face++ ) {
        ...
        mView.build_camera_dir( vFrom.invert( cmDir[ face ] ).mul( 100.f ),
                                cmDir[ face ], cmNorm[ face ] );
        aabb.xform( V->vis.box, mView );
        D3DXMatrixOrthoOffCenterLH( ( D3DXMATRIX* )&mProject, aabb.min.x,
                                    aabb.max.x, aabb.min.y, aabb.max.y,
    ...
    for ( u32 it = 0; it < 6; it++ ) {
        ...
        result.ssa[ it ] = ( u8 )clampr( iFloor( coeff * 255.f + 0.5f ),
                                         int( 0 ), int( 255 ) );
    }
...
void r_pixel_calculator::run() {
    ...
    for ( u32 it = 0; it < RImplementation.Visuals.size(); it++ ) {
        if ( 0 == fast_dynamic_cast< IRender_Mesh* >(
                      RImplementation.Visuals[ it ] ) )
            continue;
        Msg( "*%d*", it );
