...
struct ECORE_API R_constant : public xr_resource {
    ...
    IC BOOL equal( R_constant& C ) {
        return ( 0 == xr_strcmp( name, C.name ) ) && ( type == C.type ) &&
               ( destination == C.destination ) && ps.equal( C.ps ) &&
...
#if defined( USE_DX10 ) || defined( USE_DX11 )
    BOOL parseConstants( ID3DShaderReflectionConstantBuffer* pTable,
                         u32 destination );
    BOOL parseResources( ID3DShaderReflection* pReflection,
                         int ResNum,
                         u32 destination );
#endif //	USE_DX10
...
typedef resptr_core< R_constant_table, resptr_base< R_constant_table > >
    ref_ctable;
...
inline std::string rc_destination_name( u32 dest ) {
    ...
    if ( dest & RC_dest_vertex )
        append( "vertex" );
    if ( dest & RC_dest_sampler )
        append( "sampler" );
    if ( dest & RC_dest_geometry )
        append( "geometry" );
#ifdef USE_DX11
    if ( dest & RC_dest_hull )
        append( "hull" );
    if ( dest & RC_dest_domain )
        append( "domain" );
    if ( dest & RC_dest_compute )
        append( "compute" );
#endif
