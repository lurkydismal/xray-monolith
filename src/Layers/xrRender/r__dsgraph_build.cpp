...
void CDSGraphManager::r_dsgraph_insert_dynamic( dxRender_Visual* pVisual,
                                                Fmatrix* xform ) {
    Fvector Center;
    ...
    if ( sh_d && sh_d->flags.bDistort && i_mask[ sh_d->flags.iPriority / 2 ] ) {
        if ( i_mask[ CDSGraphManager::fl_hud ] )
            RGraph.mapHUDSorted.Distort.emplace_back(
                distSQ, SSA, val_pObject, pVisual, xform, sh_d,
                i_mask[ CDSGraphManager::fl_hud ] );
        else
            RGraph.mapDynamicSorted.Distort.emplace_back(
                distSQ, SSA, val_pObject, pVisual, xform, sh_d,
                i_mask[ CDSGraphManager::fl_hud ] );
    }
    ...
    // Select shader
    ShaderElement* sh =
        RImplementation.rimp_select_sh_dynamic( pVisual, distSQ );
    ...
    switch ( sh->flags.iScopeLense ) {
        ...
        case 1: {
            RGraph.mapHUD.emplace_back( EPS, SSA, val_pObject, pVisual, xform,
                                        sh, i_mask[ CDSGraphManager::fl_hud ] );
        ...
        case 2: {
            RGraph.mapScopeHUD.emplace_back(
                distSQ, SSA, val_pObject, pVisual, xform, sh,
                i_mask[ CDSGraphManager::fl_hud ] );
            return;
        }

        case 3: {
            RGraph.mapScopeHUDSorted.emplace_back(
                distSQ, SSA, val_pObject, pVisual, xform, sh,
                i_mask[ CDSGraphManager::fl_hud ] );
            return;
        }
        ...
        if ( sh->flags.bStrictB2F ) {
            ...
            if ( sh->flags.bEmissive ) {
                if ( i_mask[ CDSGraphManager::fl_cam ] )
                    RGraph.mapCamAttachedSorted.Emissive.emplace_back(
                        distSQ, SSA, val_pObject, pVisual, xform, sh_d,
                        i_mask[ CDSGraphManager::fl_cam ] );
                else
                    RGraph.mapHUDSorted.Emissive.emplace_back(
                        distSQ, SSA, val_pObject, pVisual, xform, sh_d,
                        i_mask[ CDSGraphManager::fl_hud ] );
            }
#endif // RENDER!=R_R1
            if ( i_mask[ CDSGraphManager::fl_cam ] )
                RGraph.mapCamAttachedSorted.Sorted.emplace_back(
                    distSQ, SSA, val_pObject, pVisual, xform, sh,
                    i_mask[ CDSGraphManager::fl_cam ] );
            else
                RGraph.mapHUDSorted.Sorted.emplace_back(
                    distSQ, SSA, val_pObject, pVisual, xform, sh,
                    i_mask[ CDSGraphManager::fl_hud ] );
            return;
        } else {
            if ( i_mask[ CDSGraphManager::fl_cam ] )
                RGraph.mapCamAttached.emplace_back(
                    distSQ, SSA, val_pObject, pVisual, xform, sh,
                    i_mask[ CDSGraphManager::fl_cam ] );
            else
                RGraph.mapHUD.emplace_back( distSQ, SSA, val_pObject, pVisual,
                                            xform, sh,
                                            i_mask[ CDSGraphManager::fl_hud ] );
            ...
            {
                    HUDMask_Node* N2 = val_bCamAttached ?
HUDMaskCamAttached.insertInAnyWay(distSQ) : HUDMask.insertInAnyWay(distSQ);
                    N2->val.ssa = SSA;
                    N2->val.pObject = RI.val_pObject;
                    N2->val.pVisual = pVisual;
                    N2->val.Matrix = *RI.val_pTransform;
    ...
    if ( sh->flags.bStrictB2F && !pVisual->dcast_ParticleCustom() ) {
        RGraph.mapDynamicSorted.Sorted.emplace_back(
            distSQ, SSA, val_pObject, pVisual, xform, sh,
            i_mask[ CDSGraphManager::fl_hud ] );
        return;
    }
    ...
    if ( sh->flags.bEmissive )
        RGraph.mapDynamicSorted.Emissive.emplace_back(
            distSQ, SSA, val_pObject, pVisual, xform, sh_d,
            i_mask[ CDSGraphManager::fl_hud ] );

    if ( sh->flags.bWmark && i_mask[ CDSGraphManager::fl_wmarks ] ) {
        if ( i_mask[ CDSGraphManager::fl_hud ] )
            RGraph.mapHUDSorted.Wmark.emplace_back(
                distSQ, SSA, val_pObject, pVisual, xform, sh,
                i_mask[ CDSGraphManager::fl_hud ] );
        else
            RGraph.mapDynamicSorted.Wmark.emplace_back(
                distSQ, SSA, val_pObject, pVisual, xform, sh,
                i_mask[ CDSGraphManager::fl_hud ] );
        return;
    }
...
#else
        AddToRenderQueue( RGraph.mapDynamicPasses[ shader_priority ][ iPass ],
                          { 0, SSA, val_pObject, pVisual, xform, nullptr,
                            i_mask[ CDSGraphManager::fl_hud ] },
                          pass );
#endif
    }
...
void CDSGraphManager::r_dsgraph_insert_static( dxRender_Visual* pVisual ) {
    ...
    if ( !( flags.test( IRenderVisualFlags::eIgnoreOptimization ) ||
        ...
        // Smaller objects that fail the SSA test will still render depending on
        // how much smaller they are than the discard limit. Reduces the
        // "rendering radius" effect and makes pop-in less noticeable Allows to
        // increase the discard limit for better performance without making
        // pop-in much worse Define where the "thinning" begins. E.g., objects
        // 4x the size of the discard limit start fading.
        ...
        if ( SSA < fade_start ) {
            ...
            float survival_chance =
                ( SSA - r_ssaDISCARD ) / ( fade_start - r_ssaDISCARD );
            ...
            // If the object's hash value is higher than its survival chance,
            // cull it
    ...
    VERIFY( pVisual->shader._get() );
    if ( sh_d && sh_d->flags.bDistort && i_mask[ sh_d->flags.iPriority / 2 ] )
        RGraph.mapStaticSorted.Distort.emplace_back(
            distSQ, SSA, nullptr, pVisual, &Fidentity, sh_d, false );

    // Select shader
    ShaderElement* sh =
        RImplementation.rimp_select_sh_static( pVisual, distSQ );
    ...
    if ( sh->flags.isWater && RImplementation.o.ssfx_water ) {
        RGraph.mapWater.emplace_back( distSQ, SSA, nullptr, pVisual, &Fidentity,
                                      sh, false );
        return;
    }
#endif
    ...
    // strict-sorting selection
    if ( sh->flags.bStrictB2F ) {
        RGraph.mapStaticSorted.Sorted.emplace_back(
            distSQ, SSA, nullptr, pVisual, &Fidentity, sh, false );
        return;
    }
    ...
    // d) Should be rendered to accumulation buffer in the second pass
    if ( sh->flags.bEmissive )
        RGraph.mapStaticSorted.Emissive.emplace_back(
            distSQ, SSA, nullptr, pVisual, &Fidentity, sh_d, false );

    if ( sh->flags.bWmark && i_mask[ CDSGraphManager::fl_wmarks ] ) {
        RGraph.mapStaticSorted.Wmark.emplace_back(
            distSQ, SSA, nullptr, pVisual, &Fidentity, sh, false );
        return;
    }
#endif
    ...
    for ( u32 iPass = 0; iPass < sh->passes.size(); ++iPass ) {
        ...
        AddToRenderQueue( RGraph.mapStaticPasses[ shader_priority ][ iPass ],
                          { 0, SSA, nullptr, pVisual, nullptr, nullptr, false },
                          pass );
    }
}

void CDSGraphManager::AddToRenderQueue(
    R_dsgraph::RenderQueue& queue,
    const R_dsgraph::DSGraphItem< u32, false >& item,
    const SPass& pass ) {
    if ( PortalTraverseDbg_Enabled() ) {
        PortalTraverseDebugStats& dbg = PortalTraverseDbg_Get();
        const bool opt_bucket = PortalTraverseDbg_IsOptions( i_options );
...
void CDSGraphManager::add_Dynamic( IRenderVisual* piVisual, Fmatrix* xform ) {
    ...
    if ( !i_mask[ CDSGraphManager::fl_normal ] &&
         !!flags.test( IRenderVisualFlags::eNoShadow ) )
        return;
    ...
    switch ( pVisual->Type ) {
        case MT_PARTICLE_GROUP: {
            ...
            PS::CParticleGroup* pG =
                ( PS::CParticleGroup* )pVisual->dcast_ParticleCustom();
        ...
        case MT_SKELETON_RIGID: {
            ...
            if ( pV->m_lod ) {
                ...
                float ssa = CalcSSA(
                    D, Tpos,
                    pV->vis.sphere.R *
                        0.5f ); // assume dynamics never consume full sphere
                if ( ssa < r_ssaLOD_A )
                    _use_lod = TRUE;
            }
            if ( _use_lod )
...
void CDSGraphManager::add_leafs_Dynamic(
...
void CDSGraphManager::add_leafs_Dynamic( xr_vector< IRenderVisual* >& children,
                                         Fmatrix* xform ) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
void CDSGraphManager::add_Static( IRenderVisual* piVisual,
                                  CFrustum& frustum,
                                  u32 planes ) {
    dxRender_Visual* pVisual = ( dxRender_Visual* )piVisual;
    if ( !pVisual )
    ...
    EFC_Visible VIS =
        frustum.testSAABB( vis.sphere.P, vis.sphere.R, vis.box.data(), planes );
    ...
    switch ( pVisual->Type ) {
        ...
        case MT_LOD: {
            ...
            if ( ssa < r_ssaLOD_A ) {
                ...
                RGraph.mapLOD.emplace_back( D, ssa, nullptr, pVisual, nullptr,
                                            nullptr, false );
            }

#if RENDER != R_R1
            if ( ssa > r_ssaLOD_B ||
                 i_mask[ CDSGraphManager::fl_shmap ] ) // phase shmap
#else
            if ( ssa > r_ssaLOD_B )
#endif
            {
...
void CDSGraphManager::add_Static_MultiFrustum(
    IRenderVisual* piVisual,
    const xr_vector< CFrustum >& frustums,
    const xr_vector< u32 >& masks ) {
    ...
    // Check visibility against all active frustums and propagate per-frustum
    // masks.
    vis_data& vis = pVisual->vis;
    ...
    for ( u32 i = 0; i < masks.size(); ++i ) {
        ...
        EFC_Visible VIS = frustums[ i ].testSAABB( vis.sphere.P, vis.sphere.R,
                                                   vis.box.data(), planeMask );
        if ( VIS == fcvNone ) {
            continue;
        }
        ...
        anyVisible = true;
        if ( VIS == fcvFully ) {
            // In original per-frustum traversal this path immediately adds
            // leafs. Preserve union semantics by marking this frustum as fully
            // visible.
            hasFullyVisibleFrustum = true;
            childMasks[ i ] = FULLY_VISIBLE_MASK;
        } else {
...
void CDSGraphManager::add_leafs_Static(
    xr_vector< dxRender_Visual* >& children ) {
    for ( dxRender_Visual* pVisual : children ) {
