...
u16 CKinematics::LL_BoneID( LPCSTR B ) {
    accel::iterator I =
        std::lower_bound( bone_map_N->begin(), bone_map_N->end(), B, pred_N );
    if ( I == bone_map_N->end() )
        return BI_NONE;
...
u16 CKinematics::LL_BoneID( const shared_str& B ) {
    accel::iterator I =
        std::lower_bound( bone_map_P->begin(), bone_map_P->end(), B, pred_P );
    if ( I == bone_map_P->end() )
        return BI_NONE;
...
CKinematics::~CKinematics() {
    ...
    if ( m_lod ) {
        if ( CKinematics* lod_kinematics =
                 fast_dynamic_cast< CKinematics* >( m_lod ) ) {
...
void CKinematics::IBoneInstances_Create() {
    // VERIFY2				(bones->size() < 64, "More than 64 bones
    // is a crazy thing!");
    u32 size = bones->size();
...
bool pred_sort_N( const std::pair< shared_str, u32 >& A,
                  const std::pair< shared_str, u32 >& B ) {
    return xr_strcmp( A.first, B.first ) < 0;
}

bool pred_sort_P( const std::pair< shared_str, u32 >& A,
                  const std::pair< shared_str, u32 >& B ) {
...
void CKinematics::Load( const char* N, IReader* data, u32 dwFlags ) {
    ...
    if ( LD ) {
        ...
        // From stream
        {
            ...
            /*
               ...
               ":lod:1"); m_lod 			=
               ::Render->model_CreateChild(name_load,LD); VERIFY
               (m_lod->Type==MT_SKELETON_GEOMDEF_PM ||
               m_lod->Type==MT_SKELETON_GEOMDEF_ST);
            */
        }
        LD->close();
    }
    ...
    pUserData =
        UD ? xr_new< CInifile >( UD, FS.get_path( "$game_config$" )->m_Path )
           : 0;
    if ( UD )
        UD->close();
#endif
...
#pragma todo( "container is created in stack!" )
    ...
    // Msg				("!!! %d bones",dwCount);
    // if (dwCount >= 64)	Msg			("!!! More than 64 bones
    // is a crazy thing! (%d), %s",dwCount,N);
    ...
    // after load process
    {
        for ( u16 child_idx = 0; child_idx < ( u16 )children.size();
              child_idx++ )
    ...
    // unique bone faces
    {
        for ( u32 bone_idx = 0; bone_idx < bones->size(); bone_idx++ ) {
            ...
            for ( u32 child_idx = 0; child_idx < children.size();
                  child_idx++ ) {
                CBoneData::FacesVec faces = B->child_faces[ child_idx ];
                std::sort( faces.begin(), faces.end() );
                CBoneData::FacesVecIt new_end =
                    std::unique( faces.begin(), faces.end() );
...
void CKinematics::LL_Validate() {
    ...
    for ( u16 k = 0; k < LL_BoneCount(); k++ ) {
        if ( LL_GetData( k ).IK_data.ik_flags.is( SJointIKData::flBreakable ) &&
             ( LL_GetData( k ).IK_data.type != jtNone ) ) {
...
void CKinematics::Copy( dxRender_Visual* P ) {
    ...
    m_lod = ( pFrom->m_lod )
                ? ( dxRender_Visual* )::Render->model_Duplicate( pFrom->m_lod )
                : 0;
}
...
void CKinematics::Depart() {
    ...
    if ( bones ) {
        ...
        for ( u32 b = 0; b < count; b++ ) {
            ...
            hidden_bones.set( ( u64( 1 ) << b ), TRUE );
        }
    }
    // visibility
    children.insert( children.end(), children_invisible.begin(),
                     children_invisible.end() );
...
void CKinematics::LL_SetBoneVisible( u16 bone_id, BOOL val, BOOL bRecursive ) {
    ...
    if ( !visimask.is( mask ) ) {
        bone_instances[ bone_id ].mTransformHidden =
            bone_instances[ bone_id ].mTransform;
        ...
        if ( parent_id < LL_BoneCount() && parent_id != BI_NONE )
            bone_instances[ bone_id ].mTransform.c =
                LL_GetBoneInstance( parent_id ).mTransform.c;
    ...
    if ( bRecursive ) {
        for ( xr_vector< CBoneData* >::iterator C =
                  ( *bones )[ bone_id ]->children.begin();
...
void CKinematics::Visibility_Update() {
    ...
    for ( u32 c_it = 0; c_it < children.size(); c_it++ ) {
        ...
        if ( !_c->has_visible_bones() ) {
            ...
            children_invisible.push_back(
                ( dxRender_Visual* )children[ c_it ] );
...
IC static void RecursiveBindTransform( CKinematics* K,
...
void BuildMatrix( Fmatrix& mView,
                  float invsz,
                  const Fvector norm,
                  const Fvector& from ) {
    // build projection
...
bool CKinematics::PickBone( const Fmatrix& parent_xform,
...
void CKinematics::AddWallmark( const Fmatrix* parent_xform,
    ...
    for ( u16 k = 0; k < LL_BoneCount(); k++ ) {
        ...
        if ( LL_GetBoneVisible( k ) &&
             !BD.shape.flags.is( SBoneShape::sfNoPickable ) ) {
    ...
    intrusive_ptr< CSkeletonWallmark > wm = xr_new< CSkeletonWallmark >(
        this, parent_xform, shader, cp, RDEVICE.fTimeGlobal,
        ttl ? ttl : ps_r__WallmarkTTL );
    ...
    for ( u32 i = 0; i < children.size(); i++ ) {
        ...
        for ( U16It b_it = test_bones.begin(); b_it != test_bones.end();
              b_it++ )
...
struct zero_wm_pred {
    bool operator()( const intrusive_ptr< CSkeletonWallmark > x ) const {
        return x == 0;
    }
};
    ...
    if ( !wallmarks.empty() && ( wm_frame != RDEVICE.dwFrame ) ) {
        ...
        for ( SkeletonWMVecIt it = wallmarks.begin(); it != wallmarks.end(); ) {
            ...
            float w =
                wm->TimeEnd() == -1.f
                    ? 0.f
                    : ( RDEVICE.fTimeGlobal - wm->TimeStart() ) / wm->TimeEnd();
            if ( w < 1.f ) {
                // append wm to WallmarkEngine
                if ( ::Render->ViewBase.testSphere_dirty( wm->m_Bounds.P,
                                                          wm->m_Bounds.R ) ) {
...
void CKinematics::RenderWallmark( intrusive_ptr< CSkeletonWallmark > wm,
                                  FVF::LIT*& V ) {
    PROF_EVENT( "CKinematics::RenderWallmark" );
    VERIFY( wm );
    VERIFY( V );
    VERIFY2( bones, "Invalid visual. Bones already released." );
    VERIFY2( bone_instances,
             "Invalid visual. bone_instances already deleted." );
    ...
    for ( u32 f_idx = 0; f_idx < wm->m_Faces.size(); f_idx++ ) {
        ...
        for ( u32 k = 0; k < 3; k++ ) {
            ...
            if ( F.bone_id[ k ][ 0 ] == F.bone_id[ k ][ 1 ] ) {
                // 1-link
                Fmatrix& xform0 =
                    LL_GetBoneInstance( F.bone_id[ k ][ 0 ] ).mRenderTransform;
                ...
                Fmatrix& xform0 =
                    LL_GetBoneInstance( F.bone_id[ k ][ 0 ] ).mRenderTransform;
                Fmatrix& xform1 =
                    LL_GetBoneInstance( F.bone_id[ k ][ 1 ] ).mRenderTransform;
                ...
                Fmatrix& xform1 =
                    LL_GetBoneInstance( F.bone_id[ k ][ 1 ] ).mRenderTransform;
                Fmatrix& xform2 =
                    LL_GetBoneInstance( F.bone_id[ k ][ 2 ] ).mRenderTransform;
                xform0.transform_tiny( P0, F.vert[ k ] );
                xform1.transform_tiny( P1, F.vert[ k ] );
                ...
                Fvector PB[ 4 ];
                for ( int i = 0; i < 4; ++i ) {
                    Fmatrix& xform = LL_GetBoneInstance( F.bone_id[ k ][ i ] )
                                         .mRenderTransform;
