...
IRender_Sector* CRender::detectLastSector( const Fvector& P ) {
    ...
    static auto detectSector = []( const Fvector& P,
                                   Fvector& dir ) -> IRender_Sector* {
        sectors_detect_xrc.ray_options( CDB::OPT_ONLYNEAREST );
        // Portals model
        if ( RImplementation.rmPortals ) {
            sectors_detect_xrc.ray_query( RImplementation.rmPortals, P, dir,
                                          1000.f );
            if ( sectors_detect_xrc.r_count() ) {
                ...
                CPortal* pPortal =
                    ( CPortal* )RImplementation.Portals[ pTri->dummy ];
        ...
        // Geometry model
        sectors_detect_xrc.ray_query(
            g_pGameLevel->ObjectSpace.GetStaticModel(), P, dir, 1000.f );
        if ( sectors_detect_xrc.r_count() ) {
            CDB::RESULT* RP = sectors_detect_xrc.r_begin();
...
IRender_Sector* CRender::detectSector( const Fvector& P, Fvector& dir ) {
    ...
    int id2 = -1;
    float range2 = range1;
    sectors_detect_xrc.ray_query( g_pGameLevel->ObjectSpace.GetStaticModel(), P,
                                  dir, range2 );
    if ( sectors_detect_xrc.r_count() ) {
...
void CRender::detectSectors_sphere( CSector* sector,
    ...
    if ( rmPortals ) {
        ...
        for ( int K = 0; K < sectors_detect_xrc.r_count(); K++ ) {
            CPortal* pPortal = ( CPortal* )Portals
                [ rmPortals->get_tris()[ sectors_detect_xrc.r_begin()[ K ].id ]
                      .dummy ];
...
void CRender::detectSectors_frustum( CSector* sector,
                                     FixedSet< IRender_Sector* >& m_sectors,
                                     CFrustum* _frustum ) {
    m_sectors.clear();
    m_sectors.insert( sector );
