...
void CBackend::set_ClipPlanes( u32 _enable,
                               Fplane* _planes /*=NULL */,
                               u32 count /* =0*/ ) {
#if defined( USE_DX10 ) || defined( USE_DX11 )
    ...
    D3DXMatrixInverse( &worldToClipMatrixIT, NULL,
                       ( D3DXMATRIX* )&RDEVICE.mFullTransform );
...
void CBackend::set_Textures( STextureList* _T ) {
    ...
    for ( ; _it != _end; _it++ ) {
...
#endif
                if ( load_surf ) {
                    PGO( Msg( "PGO:tex%d:%s", load_id,
                              load_surf->cName.c_str() ) );
...
#endif //	UDE_DX10
        {
            ...
            //	Set up pixel shader resources
            VERIFY( load_id <
                    CTexture::rstGeometry + mtMaxGeometryShaderTextures );
            ...
            //	Set up pixel shader resources
            VERIFY( load_id <
                    CTexture::rstCompute + mtMaxComputeShaderTextures );
...
#else  //	USE_DX10
        CHK_DX(
            HW.pDevice->SetTexture( _last_vs + CTexture::rstVertex, NULL ) );
#endif //	USE_DX10
    }
