...
void dxRender_Visual::Load( const char* N, IReader* data, u32 ) {
    ...
    // Static level visuals are prepared on workers and are always world
    // geometry. Avoid reading the owner-only HUD mode concurrently with
    // HUD/model creation.
...
void dxRender_Visual::ResetShaderTexture() {
    if ( !dbg_shader.equal( dbg_shader_def ) ||
         !dbg_texture.equal( dbg_texture_def ) )
