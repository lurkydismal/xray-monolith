...
ColorMapManager::ColorMapManager() {
    m_CMap[ 0 ] = dxRenderDeviceRender::Instance().Resources->_CreateTexture(
        "$user$cmap0" );
    m_CMap[ 1 ] = dxRenderDeviceRender::Instance().Resources->_CreateTexture(
        "$user$cmap1" );
}

void ColorMapManager::SetTextures( const shared_str& tex0,
