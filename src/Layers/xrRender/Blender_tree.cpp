        ...
        switch ( C.iElement ) {
            case SE_R1_NORMAL_HQ:
                if ( oNotAnTree.value ) {
                    ...
                    if ( oBlend.value )
                        C.r_Pass( tsv, tsp, TRUE, TRUE, TRUE, TRUE,
                                  D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA, TRUE,
                                  tree_aref );
                    else
                        C.r_Pass( tsv, tsp, TRUE, TRUE, TRUE, TRUE,
                                  D3DBLEND_ONE, D3DBLEND_ZERO, TRUE,
                                  tree_aref );
                    ...
                    if ( C.bDetail_Diffuse ) {
                        if ( oBlend.value )
                            C.r_Pass( "tree_w_dt", "vert_dt", TRUE, TRUE, TRUE,
                                      TRUE, D3DBLEND_SRCALPHA,
                                      D3DBLEND_INVSRCALPHA, TRUE, tree_aref );
                        else
                            C.r_Pass( "tree_w_dt", "vert_dt", TRUE, TRUE, TRUE,
                                      TRUE, D3DBLEND_ONE, D3DBLEND_ZERO, TRUE,
                                      tree_aref );
                        C.r_Sampler( "s_base", C.L_textures[ 0 ] );
                        C.r_Sampler( "s_detail", C.detail_texture );
                        C.r_End();
                        ...
                        if ( oBlend.value )
                            C.r_Pass( "tree_w", "vert", TRUE, TRUE, TRUE, TRUE,
            ...
            case SE_R1_NORMAL_LQ:
                // Level view
                if ( oBlend.value )
                    C.r_Pass( "tree_s", "vert", TRUE, TRUE, TRUE, TRUE,
                              D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA, TRUE,
                              tree_aref );
                else
                    C.r_Pass( "tree_s", "vert", TRUE, TRUE, TRUE, TRUE,
            ...
            case SE_R1_LPOINT:
                C.r_Pass(
                    ( oNotAnTree.value ) ? "tree_s_point" : "tree_w_point",
                    "add_point", FALSE, TRUE, FALSE, TRUE, D3DBLEND_ONE,
                    D3DBLEND_ONE, TRUE, 0 );
                C.r_Sampler( "s_base", C.L_textures[ 0 ] );
                ...
                break;
            case SE_R1_LSPOT:
                C.r_Pass( ( oNotAnTree.value ) ? "tree_s_spot" : "tree_w_spot",
                          "add_spot", FALSE, TRUE, FALSE, TRUE, D3DBLEND_ONE,
                          D3DBLEND_ONE, TRUE, 0 );
                C.r_Sampler( "s_base", C.L_textures[ 0 ] );
...
void CBlender_Tree::Compile( CBlender_Compile& C ) {
    ...
    switch ( C.iElement ) {
        ...
        case SE_R2_SHADOW: // smap-spot
            //	TODO: DX10: Use dumb shader for shadowmap since shadows are
            // drawn using hardware PCF
            if ( oBlend.value )
                C.r_Pass( tvs_s, "shadow_direct_base_aref", FALSE, TRUE, TRUE,
                          TRUE, D3DBLEND_ZERO, D3DBLEND_ONE, TRUE, 200 );
            else
...
void CBlender_Tree::Compile( CBlender_Compile& C ) {
    ...
    bool bUseATOC = ( oBlend.value && ( RImplementation.o.dx10_msaa_alphatest ==
    ...
    switch ( C.iElement ) {
        ...
        case SE_R2_NORMAL_LQ: // deffer
            if ( bUseATOC ) {
                uber_deffer( C, false, tvs, "base_atoc", oBlend.value, 0,
                             true );
                C.r_Stencil( TRUE, D3DCMP_ALWAYS, 0xff, 0x7f, D3DSTENCILOP_KEEP,
                             D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP );
                C.r_StencilRef( 0x01 );
