...
xr_token ttype_token[] = { { "2D Texture", STextureParams::ttImage },
...
void STextureParams::OnTypeChange( PropValue* prop ) {
    switch ( type ) {
        ...
        case ttBumpMap:
            ...
            break;
        case ttNormalMap:
            flags.set( flImplicitLighted | flBinaryAlpha | flAlphaBorder |
                           flColorBorder | flFadeToColor | flFadeToAlpha |
            ...
            break;
        case ttTerrain:
            flags.set( flGenerateMipMaps | flBinaryAlpha | flAlphaBorder |
                           flColorBorder | flFadeToColor | flFadeToAlpha |
                           flDitherColor | flDitherEachMIPLevel | flBumpDetail,
                       FALSE );
...
void STextureParams::FillProp( LPCSTR base_name,
                               PropItemVec& items,
                               PropValue::TOnChange on_type_change ) {
    OnTypeChangeEvent = on_type_change;
    PropValue* P =
        PHelper().CreateToken32( items, "Type", ( u32* )&type, ttype_token );
    P->OnChangeEvent.bind( this, &STextureParams::OnTypeChange );
    PHelper().CreateCaption( items, "Source\\Width",
                             shared_str().printf( "%d", width ) );
    PHelper().CreateCaption( items, "Source\\Height",
                             shared_str().printf( "%d", height ) );
    PHelper().CreateCaption( items, "Source\\Alpha",
                             HasAlpha() ? "present" : "absent" );
    switch ( type ) {
        case ttImage:
        case ttCubeMap:
            PHelper().CreateToken32( items, "Format", ( u32* )&fmt,
                                     tfmt_token );

            PHelper().CreateFlag32( items, "MipMaps\\Enabled", &flags,
                                    flGenerateMipMaps );
            PHelper().CreateToken32( items, "MipMaps\\Filter",
                                     ( u32* )&mip_filter, tparam_token );

            P = PHelper().CreateToken32( items, "Bump\\Mode",
                                         ( u32* )&bump_mode, tbmode_token );
            P->OnChangeEvent.bind( this, &STextureParams::OnTypeChange );
            if ( tbmUse == bump_mode || tbmUseParallax == bump_mode ) {
                AnsiString path;
                path = base_name;
                PHelper().CreateChoose( items, "Bump\\Texture", &bump_name,
                                        smTexture, path.c_str() );
            }

            PHelper().CreateFlag32( items, "Details\\Use As Diffuse", &flags,
                                    flDiffuseDetail );
            PHelper().CreateFlag32( items, "Details\\Use As Bump (R2)", &flags,
                                    flBumpDetail );
            PHelper().CreateChoose( items, "Details\\Texture", &detail_name,
                                    smTexture );
            PHelper().CreateFloat( items, "Details\\Scale", &detail_scale, 0.1f,
                                   10000.f, 0.1f, 2 );

            PHelper().CreateToken32( items, "Material\\Base", ( u32* )&material,
                                     tmtl_token );
            PHelper().CreateFloat( items, "Material\\Weight",
                                   &material_weight );
            ...
            PHelper().CreateFlag32( items, "Flags\\Implicit Lighted", &flags,
                                    flImplicitLighted );

            PHelper().CreateFlag32( items, "Fade\\Enable Color", &flags,
                                    flFadeToColor );
            PHelper().CreateFlag32( items, "Fade\\Enabled Alpha", &flags,
                                    flFadeToAlpha );
            PHelper().CreateU8( items, "Fade\\Delay 'n' MIP", &fade_delay, 0,
                                255 );
            PHelper().CreateU32( items, "Fade\\% of color to fade in",
                                 &fade_amount, 0, 100, 0 );
            PHelper().CreateColor( items, "Fade\\Color", &fade_color );
            PHelper().CreateU8( items, "Fade\\Alpha",
                                ( ( u8* )&fade_color ) + 3, 0, 255 );

            PHelper().CreateFlag32( items, "Border\\Enabled Color", &flags,
                                    flColorBorder );
            PHelper().CreateFlag32( items, "Border\\Enabled Alpha", &flags,
                                    flAlphaBorder );
            PHelper().CreateColor( items, "Border\\Color", &border_color );
            break;
        case ttBumpMap:
            PHelper().CreateChoose( items, "Bump\\Special NormalMap",
                                    &ext_normal_map_name, smTexture,
                                    base_name );
            PHelper().CreateFloat( items, "Bump\\Virtual Height (m)",
                                   &bump_virtual_height, 0.f, 0.1f, 0.001f, 3 );
            break;
        case ttNormalMap:
            P = PHelper().CreateToken32( items, "Format", ( u32* )&fmt,
                                         tfmt_token );
        ...
        case ttTerrain:
            ...
            P = PHelper().CreateFlag32( items, "Flags\\Implicit Lighted",
                                        &flags, flImplicitLighted );
            P->Owner()->Enable( false );
            break;
    }
}

BOOL STextureParams::similar( STextureParams& tp1,
                              xr_vector< AnsiString >& sel_params ) {
    BOOL res = TRUE;
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( par_name == "Type" ) {
            ...
            res = ( flags.test( flGenerateMipMaps ) ==
                    tp1.flags.test( flGenerateMipMaps ) );
            ...
            res = ( flags.test( flDiffuseDetail ) ==
                    tp1.flags.test( flDiffuseDetail ) );
        } else if ( par_name == "Details\\Use As Bump (R2)" ) {
            res = ( flags.test( flBumpDetail ) ==
            ...
            res = ( flags.test( flBinaryAlpha ) ==
                    tp1.flags.test( flBinaryAlpha ) );
        } else if ( par_name == "Flags\\Dither" ) {
            res = ( flags.test( flDitherColor ) ==
                    tp1.flags.test( flDitherColor ) );
        } else if ( par_name == "Flags\\Dither Each MIP" ) {
            res = ( flags.test( flDitherEachMIPLevel ) ==
                    tp1.flags.test( flDitherEachMIPLevel ) );
        } else if ( par_name == "Flags\\Implicit Lighted" ) {
            res = ( flags.test( flImplicitLighted ) ==
                    tp1.flags.test( flImplicitLighted ) );
        } else if ( par_name == "Fade\\Enable Color" ) {
            res = ( flags.test( flFadeToColor ) ==
                    tp1.flags.test( flFadeToColor ) );
        } else if ( par_name == "Fade\\Enabled Alpha" ) {
            res = ( flags.test( flFadeToAlpha ) ==
                    tp1.flags.test( flFadeToAlpha ) );
            ...
            res =
                ( color_get_A( fade_color ) == color_get_A( tp1.fade_color ) );
        } else if ( par_name == "Border\\Enabled Color" ) {
            res = ( flags.test( flColorBorder ) ==
                    tp1.flags.test( flColorBorder ) );
        } else if ( par_name == "Border\\Enabled Alpha" ) {
            res = ( flags.test( flAlphaBorder ) ==
                    tp1.flags.test( flAlphaBorder ) );
...
u32 STextureParams::MemoryUsage( LPCSTR base_name ) {
    ...
    string_path fn;
    FS.update_path( fn, _game_textures_,
                    EFS.ChangeFileExt( base_name, ".seq" ).c_str() );
    if ( FS.exist( fn ) ) {
        string128 buffer;
