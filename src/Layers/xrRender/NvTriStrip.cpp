...
//
void GenerateStrips( const u16* in_indices,
                     const s32 in_numIndices,
                     xr_vector< PrimitiveGroup >& primGroups ) {
    ...
    stripifier.Stripify( tempIndices, cacheSize, minStripSize, tempStrips,
                         tempFaces );
    ...
    if ( bListsOnly ) {
        ...
        for ( u32 i = 0; i < tempStrips.size(); i++ ) {
            for ( int j = 0; j < tempStrips[ i ]->m_faces.size(); j++ ) {
                primGroups[ 0 ].indices[ indexCtr++ ] =
                    u16( tempStrips[ i ]->m_faces[ j ]->m_v0 );
                primGroups[ 0 ].indices[ indexCtr++ ] =
                    u16( tempStrips[ i ]->m_faces[ j ]->m_v1 );
                primGroups[ 0 ].indices[ indexCtr++ ] =
                    u16( tempStrips[ i ]->m_faces[ j ]->m_v2 );
            }
        }
        ...
        stripifier.CreateStrips( tempStrips, stripIndices, bStitchStrips,
                                 numSeparateStrips );

        // if we're stitching strips together, we better get back only one strip
        // from CreateStrips()
        assert( ( bStitchStrips && ( numSeparateStrips == 1 ) ) ||
                !bStitchStrips );
        ...
        for ( stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++ ) {
            ...
            for ( int i = startingLoc; i < stripLength + startingLoc; i++ )
                primGroups[ stripCtr ].indices[ indexCtr++ ] =
                    u16( stripIndices[ i ] );

            startingLoc +=
                stripLength +
                1; // we add 1 to account for the -1 separating strips
        }
        ...
        if ( tempFaces.size() != 0 ) {
            ...
            primGroups[ faceGroupLoc ].indices =
                xr_alloc< u16 >( tempFaces.size() * 3 );
            primGroups[ faceGroupLoc ].numIndices = tempFaces.size() * 3;
            int indexCtr = 0;
            for ( int i = 0; i < tempFaces.size(); i++ ) {
                primGroups[ faceGroupLoc ].indices[ indexCtr++ ] =
                    u16( tempFaces[ i ]->m_v0 );
                primGroups[ faceGroupLoc ].indices[ indexCtr++ ] =
                    u16( tempFaces[ i ]->m_v1 );
                primGroups[ faceGroupLoc ].indices[ indexCtr++ ] =
                    u16( tempFaces[ i ]->m_v2 );
            }
        }
    }
...
//  vertex buffer.
//
void RemapIndices( const xr_vector< PrimitiveGroup >& in_primGroups,
                   const u16 numVerts,
