...
template < typename T >
static void remove_shader_indexed(
    xr_unordered_flat_map< u64, xr_vector< T* > >& index,
    u64 hash,
    const T* value ) {
...
static u64 element_hash( const ShaderElement& element ) {
    ...
    hash ^= element.flags.iPriority | ( element.flags.bStrictB2F << 2 ) |
            ( element.flags.bEmissive << 3 ) | ( element.flags.bDistort << 4 ) |
            ( element.flags.bWmark << 5 ) | ( element.flags.bLandscape << 6 ) |
...
void CResourceManager::ED_UpdateBlender( LPCSTR Name, IBlender* data ) {
    ...
    map_Blender::iterator I = m_blenders.find( N );
    if ( I != m_blenders.end() ) {
        R_ASSERT( data->getDescription().CLS ==
                  I->second->getDescription().CLS );
        xr_delete( I->second );
        I->second = data;
    } else {
...
Shader* CResourceManager::_cpp_Create( IBlender* B,
                                       LPCSTR s_shader,
                                       LPCSTR s_textures,
                                       LPCSTR s_constants,
                                       LPCSTR s_matrices,
                                       bool hud_loading,
                                       ref_shader* keep_alive ) {
    CBlender_Compile C;
    Shader S;
    ...
    {
        C.iElement = 0;
        C.bDetail = m_textures_description.GetDetailTexture(
            C.L_textures[ 0 ], C.detail_texture, C.detail_scaler );
        //.		C.bDetail			=
        //_GetDetailTexture(*C.L_textures[0],C.detail_texture,C.detail_scaler);
        ShaderElement E;
        C._cpp_Compile( &E );
    ...
    // Hacky way to remove from the HUD mask transparent stuff. ( Let's try
...
Shader* CResourceManager::_cpp_Create( LPCSTR s_shader,
                                       LPCSTR s_textures,
                                       LPCSTR s_constants,
                                       LPCSTR s_matrices ) {
...
#endif
    {
        ...
        if ( !pBlender )
            return NULL;
        return _cpp_Create( pBlender, s_shader, s_textures, s_constants,
                            s_matrices, ::Render->hud_loading );
#else  //	USE_DX10
        return _cpp_Create( _GetBlender( s_shader ? s_shader : "null" ),
                            s_shader, s_textures, s_constants, s_matrices,
                            ::Render->hud_loading );
#endif //	USE_DX10
       // #else
    }
...
Shader* CResourceManager::Create( IBlender* B,
                                  LPCSTR s_shader,
                                  LPCSTR s_textures,
                                  LPCSTR s_constants,
                                  LPCSTR s_matrices ) {
...
Shader* CResourceManager::Create( LPCSTR s_shader,
                                  LPCSTR s_textures,
                                  LPCSTR s_constants,
                                  LPCSTR s_matrices ) {
    // #ifndef DEDICATED_SERVER
...
#endif
    {
        ...
        if ( _lua_HasShader( s_shader ) )
            ...
            Shader* pShader =
                _cpp_Create( s_shader, s_textures, s_constants, s_matrices );
            if ( pShader )
                return pShader;
            else {
...
Shader* CResourceManager::CreateLevelShader( LPCSTR s_shader,
                                             LPCSTR s_textures,
                                             u64 recipe_identity ) {
...
ref_shader CResourceManager::CreateLevelCppShader(
    LPCSTR s_shader,
    LPCSTR s_textures,
    LPCSTR s_constants,
    LPCSTR s_matrices,
    ...
    if ( have_blender ) {
        ...
        if ( blender ) {
            ...
            _cpp_Create( blender, s_shader, s_textures, s_constants, s_matrices,
                         false, &shader );
...
void CResourceManager::CompleteTextureLoad(
    const ResourceLoadGenerationPtr& generation ) {
...
void CResourceManager::RecordTextureLoadFailure(
    const ResourceLoadGenerationPtr& generation,
...
void CResourceManager::DrainOwnerTextureLoads(
    const ResourceLoadGenerationPtr& generation ) {
...
std::exception_ptr CResourceManager::WaitForTextureLoadGeneration(
    const ResourceLoadGenerationPtr& generation ) {
    for ( ;; ) {
        u32 serial;
        {
        ...
        if ( generation->native_generation &&
             NativeLoadExecutor::Instance().HelpGeneration(
                 generation->native_generation ) ) {
            continue;
        }
        const HANDLE events[] = { generation->completed,
                                  generation->ownerWorkAvailable };
        WaitForMultipleObjects( static_cast< DWORD >( std::size( events ) ),
                                events, FALSE, INFINITE );

        xrCriticalSectionGuard guard( textureLoadGuard );
        if ( serial == generation->serial && generation->pending == 0 &&
             generation->ownerTextureLoads.empty() )
...
void CResourceManager::QueueTextureLoad( const ref_texture& texture ) {
    ...
    if ( ( generation && ( activeResourceLoadGeneration != generation ||
                           generation->closed || generation->aborted ) ) ||
         ( !generation && activeResourceLoadGeneration ) )
        return;
    ...
    const bool submitted = executor.Submit(
        batch, NativeLoadPriority::ShaderTexture,
...
void CResourceManager::WaitForTextureLoads() {
    ...
    for ( ;; ) {
        ...
        // Startup fallback: video, sequence and GIF decoders stay on the
        // render/owner thread.
...
u64 CResourceManager::BeginLoadGeneration() {
    {
        xrCriticalSectionGuard guard( textureLoadGuard );
        R_ASSERT2(
            !activeResourceLoadGeneration && !resourceLoadGenerationStarting,
    ...
    u64 result;
    {
		...
		{
#endif
        const u64 nativeGeneration =
            NativeLoadExecutor::Instance().CurrentGeneration();
        R_ASSERT2( nativeGeneration,
                   "Native load generation must begin before texture load "
                   "generation" );
        startedGeneration = xr_make_shared< ResourceLoadGeneration >(
            nextResourceLoadGeneration, nativeGeneration );
...
void CResourceManager::PrefetchTexture( LPCSTR name,
                                        LPCSTR canonical_level_path ) {
...
int CResourceManager::GetTextureLoadLod( LPCSTR name ) const {
    ...
    static const bool enough_address_space_available =
...
void CResourceManager::DeferredUnload() {
    ...
    xr_parallel_foreach( textures.begin(), textures.end(),
                         []( ref_texture& texture ) { texture->Unload(); } );
}
...
void CResourceManager::UnloadAllTexturesOnLevelUnload() {
    ...
    xr_vector< ref_texture > textures_to_unload;
    {
        ...
        for ( const auto& pair : m_textures ) {
            ...
            // Keep $ textures alive since they are bound to runtime render
            // targets or other important parts
...
void CResourceManager::_GetMemoryUsage( u32& m_base,
                                        u32& c_base,
                                        u32& m_lmaps,
                                        u32& c_lmaps ) {
    xrCriticalSectionGuard guard( creationGuard );
...
void CResourceManager::_DumpMemoryUsage() {
    ...
    // sort
    {
        ...
        for ( ; I != E; I++ ) {
            ...
            mtex.insert( mk_pair( m, mk_pair( I->second->dwReference.load(
    ...
    // dump
    {
        xr_multimap< u32, std::pair< u32, shared_str > >::iterator I =
            mtex.begin();
        xr_multimap< u32, std::pair< u32, shared_str > >::iterator E =
            mtex.end();
        for ( ; I != E; I++ )
            Msg( "* %4.1f : [%4d] %s", float( I->first ) / 1024.f,
                 I->second.first, I->second.second.c_str() );
    }
}
...
void CResourceManager::EvictStalledTextures() {
    ...
    s_evict_cursor =
        ( I != m_textures.end() ) ? xr_string( I->first ) : xr_string();

    creationGuard.Leave();

#ifdef DEBUG
    Msg(
        "* [TexEvict] frame=%u total=%u scanned=%u evicted=%u skip_user=%u "
        "skip_name=%u skip_unloaded=%u skip_refs=%u skip_recent=%u freed_kb=%u",
        cur_frame, ( u32 )m_textures.size(), scanned, evicted, skip_user,
        skip_name, skip_unloaded, skip_refs, skip_recent, evicted_kb );
#endif // DEBUG
#endif
}

/*
BOOL	CResourceManager::_GetDetailTexture(LPCSTR Name,LPCSTR& T,
R_constant_setup* &CS)
{
        LPSTR N = LPSTR(Name);
