...
void dxRainRender::Render( CEffect_Rain& owner ) {
    ...
    u32 render_items = _min( desired_items, ( u32 )owner.items.size() );
    if ( !render_items )
        return;
    ...
    Fvector3 f_rain_color =
        g_pGamePersistent->Environment().CurrentEnv->rain_color;
    u32 u_rain_color = color_rgba_f( f_rain_color.x, f_rain_color.y,
                                     f_rain_color.z, factor_visual );

    // perform update
    u32 vOffset;
    FVF::LIT* verts = ( FVF::LIT* )RCache.Vertex.Lock(
        render_items * 4, hGeom_Rain->vb_stride, vOffset );
    FVF::LIT* start = verts;
    ...
    for ( CEffect_Rain::Item& one : owner.items ) {
        ...
        if ( ++rendered_items >= render_items )
            break;
    }
    ...
    if ( vCount ) {
        ...
        RCache.set_c(
            s_shader_setup,
            ps_ssfx_rain_2 ); // Alpha, Brigthness, Refraction, Reflection
    }
...

    {
        ...
        while ( P ) {
            ...
            // Render
            if ( ::Render->ViewBase.testSphere_dirty( P->bounds.P,
                                                      P->bounds.R ) ) {
                // Build matrix
                ...
                // XForm verts
                DM_Drop->transfer( mXform, v_ptr, u_rain_color, i_ptr,
                                   pcount * DM_Drop->number_vertices );
                ...
                if ( pcount >= particles_cache ) {
                    ...
                    RCache.Render( D3DPT_TRIANGLELIST, v_offset, 0, vCount_Lock,
                                   i_offset, dwNumPrimitives );

                    v_ptr =
                        ( IRender_DetailModel::fvfVertexOut* )
                            RCache.Vertex.Lock(
                                vCount_Lock, hGeom_Drops->vb_stride, v_offset );
