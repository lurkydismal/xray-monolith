...
void CPGDef::Clone( CPGDef* source ) {
    ...
    for ( EffectIt d_it = m_Effects.begin(), s_it = source->m_Effects.begin();
          s_it != source->m_Effects.end(); s_it++, d_it++ )
...
BOOL CPGDef::Load2( CInifile& ini ) {
    ...
    u32 counter = 0;
    string256 buff;
    for ( EffectIt it = m_Effects.begin(); it != m_Effects.end();
          ++it, ++counter ) {
        *it = xr_new< SEffect >();
...
void CPGDef::Save2( CInifile& ini ) {
    ...
    for ( EffectIt it = m_Effects.begin(); it != m_Effects.end();
        ...
        ini.w_string( buff, "on_play_child",
                      ( *it )->m_Flags.test( SEffect::flOnPlayChild )
                          ? ( *it )->m_OnPlayChildName.c_str()
                          : "" );
        ini.w_string( buff, "on_birth_child",
                      ( *it )->m_Flags.test( SEffect::flOnBirthChild )
                          ? ( *it )->m_OnBirthChildName.c_str()
                          : "" );
        ini.w_string( buff, "on_death_child",
                      ( *it )->m_Flags.test( SEffect::flOnDeadChild )
                          ? ( *it )->m_OnDeadChildName.c_str()
                          : "" );
...
PS::CParticleGroup::SItem::~SItem() {
    ...
    auto Callback =
        xr_make_delegate( this, &PS::CParticleGroup::SItem::DelayDeleteChilds );
    auto Iter = std::find( Device.seqParallelBeforRender.begin(),
...
void CParticleGroup::SItem::Clear() {
    ...
    for ( VisualVecIt it = visuals.begin(); it != visuals.end(); it++ ) {
        auto Iter = std::find( _children_destroy.begin(),
                               _children_destroy.end(), *it );
...
void CParticleGroup::SItem::StartRelatedChild( CParticleEffect* emitter,
                                               LPCSTR eff_name,
                                               PAPI::Particle& m ) {
    CParticleEffect* C = static_cast< CParticleEffect* >(
        RImplementation.model_CreatePE( eff_name ) );
    ...
    vel.div( C->m_RT_Flags.is( CParticleEffect::flRT_LiveUpdate )
                 ? Device.fTimeDelta
                 : C->m_Def->GetFStep() );
...
void CParticleGroup::SItem::StartFreeChild( CParticleEffect* emitter,
                                            LPCSTR nm,
                                            PAPI::Particle& m ) {
    CParticleEffect* C =
        static_cast< CParticleEffect* >( RImplementation.model_CreatePE( nm ) );
    C->SetHudMode( emitter->GetHudMode() );
    if ( !C->IsLooped() ) {
        Fmatrix M;
        M.identity();
...
#ifdef _EDITOR
        Msg( "!Can't use looped effect '%s' as 'On Birth' child for group.",
             nm );
#else
        Debug.fatal(
            DEBUG_INFO,
            "Can't use looped effect '%s' as 'On Birth' child for group.", nm );
#endif
    }
}
...
void CParticleGroup::SItem::UpdateParent( const Fmatrix& m,
...
//------------------------------------------------------------------------------
void OnGroupParticleBirth( void* owner,
                           u32 param,
                           PAPI::Particle& m,
                           u32 idx ) {
    ...
    VERIFY( PG );
    CParticleEffect* PE =
        static_cast< CParticleEffect* >( PG->items[ param ]._effect );
...
void CParticleGroup::SItem::OnFrame( u32 u_dt,
                                     const CPGDef::SEffect& def,
                                     Fbox& box,
                                     bool& bPlaying ) {
    CParticleEffect* E = static_cast< CParticleEffect* >( _effect );
    if ( E ) {
        ...
        if ( E->IsPlaying() ) {
            ...
            if ( E->vis.box.is_valid() )
                box.merge( E->vis.box );
            if ( def.m_Flags.is( CPGDef::SEffect::flOnPlayChild ) &&
                 def.m_OnPlayChildName.size() ) {
                PAPI::Particle* particles;
                u32 p_cnt;
                PAPI::ParticleManager()->GetParticles( E->GetHandleEffect(),
                                                       particles, p_cnt );
                ...
                if ( p_cnt ) {
                    for ( u32 i = 0; i < p_cnt; i++ ) {
                        ...
                        vel.sub( m.pos, m.posB );
                        vel.div(
                            C->m_RT_Flags.is( CParticleEffect::flRT_LiveUpdate )
                                ? Device.fTimeDelta
                                : C->m_Def->GetFStep() );
                        C->UpdateParent( M, vel, FALSE );
                    }
    ...
    if ( !_children_related.empty() ) {
        for ( it = _children_related.begin(); it != _children_related.end();
              it++ ) {
            ...
            if ( E ) {
                ...
                if ( E->IsPlaying() ) {
                    ...
                    if ( def.m_Flags.is(
                             CPGDef::SEffect::flOnPlayChildRewind ) ) {
                        E->Play();
                    }
                }
            }
    ...
    if ( !_children_free.empty() ) {
        ...
        if ( !_children_destroy.empty() ) {
            auto Callback = xr_make_delegate(
                this, &PS::CParticleGroup::SItem::DelayDeleteChilds );
            if ( std::find( Device.seqParallelBeforRender.begin(),
                            Device.seqParallelBeforRender.end(), Callback ) ==
                 Device.seqParallelBeforRender.end() ) {
                Device.seqParallelBeforRender.emplace_back(
                    std::move( Callback ) );
            }
        }
    }
...
void PS::CParticleGroup::SItem::DelayDeleteChilds() {
    for ( dxRender_Visual* Vis : _children_destroy ) {
        auto Iter =
            std::find( _children_free.begin(), _children_free.end(), Vis );
        if ( Iter != _children_free.end() ) {
            _children_free.erase( Iter );
        } else {
            Iter = std::find( _children_related.begin(),
                              _children_related.end(), Vis );
...
void CParticleGroup::OnFrame( u32 u_dt ) {
    ...
    if ( m_Def && m_RT_Flags.is( flRT_Playing ) ) {
        ...
        for ( PS::CPGDef::SEffect* pEffect : m_Def->m_Effects ) {
            if ( pEffect->m_Flags.is( CPGDef::SEffect::flEnabled ) ) {
                ...
                if ( particleRenderItem.IsPlaying() ) {
                    if ( ( m_CurrentTime <= pEffect->m_Time1 ) &&
                         ( m_CurrentTime + fdeltaTime >= pEffect->m_Time1 ) ) {
                        particleRenderItem.Stop( pEffect->m_Flags.is(
                            CPGDef::SEffect::flDefferedStop ) );
                    }
                } else {
                    if ( !m_RT_Flags.is( flRT_DefferedStop ) ) {
                        if ( ( m_CurrentTime <= pEffect->m_Time0 ) &&
        ...
        m_CurrentTime += fdeltaTime;
        if ( ( m_Def->m_fTimeLimit > 0.f ) &&
             ( m_CurrentTime > m_Def->m_fTimeLimit ) ) {
            if ( !m_RT_Flags.is( flRT_DefferedStop ) ) {
                Stop( true );
            }
        }
        ...
        for ( auto i_it = items.begin(); i_it != items.end(); i_it++ ) {
            i_it->OnFrame( u_dt, *m_Def->m_Effects[ i_it - items.begin() ], box,
                           bPlaying );
        }
...
void CParticleGroup::UpdateParent( const Fmatrix& m,
                                   const Fvector& velocity,
                                   BOOL bXFORM ) {
    xrCriticalSectionGuard guard( &onframe_lock );
...
BOOL CParticleGroup::Compile( CPGDef* def ) {
    ...
    if ( m_Def ) {
        ...
        for ( CPGDef::EffectVec::const_iterator e_it = m_Def->m_Effects.begin();
            ...
            CParticleEffect* eff =
                ( CParticleEffect* )RImplementation.model_CreatePE(
                    *( *e_it )->m_EffectName );
            eff->SetBirthDeadCB( OnGroupParticleBirth, OnGroupParticleDead,
                                 this, u32( e_it - m_Def->m_Effects.begin() ) );
...
BOOL CParticleGroup::GetHudMode() {
    ...
    if ( items.size() ) {
        CParticleEffect* E =
            static_cast< CParticleEffect* >( items[ 0 ]._effect );
        return E->GetHudMode();
    }
