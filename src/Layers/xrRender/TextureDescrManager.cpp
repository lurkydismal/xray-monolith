    ...
    for ( const FS_File& fs_iter : flist ) {
        ...
        if ( STextureParams::ttImage == tp.type ||
            ...
            if ( tp.detail_name.size() &&
                 tp.flags.is_any( STextureParams::flDiffuseDetail |
                ...
                if ( dts )
                    ...
                    /*desc.m_assoc->cs*/ dts =
                        xr_new< cl_dt_scaler >( tp.detail_scale );
            ...
            desc.m_spec->m_material =
                tp.material + ( tp.material < 4 ? tp.material_weight : 0 );
...
void CTextureDescrMngr::Load() {
    TH_LoadTHM* gtex = new TH_LoadTHM(
        { "$game_textures$", m_texture_details, m_detail_scalers } );
    TH_LoadTHM* lvl =
        new TH_LoadTHM( { "$level$", m_texture_details, m_detail_scalers } );
