...
void fix_texture_thm_name( LPSTR fn ) {
    ...
    if ( _ext &&
         ( 0 == stricmp( _ext, ".tga" ) || 0 == stricmp( _ext, ".thm" ) ||
           0 == stricmp( _ext, ".dds" ) || 0 == stricmp( _ext, ".bmp" ) ||
           0 == stricmp( _ext, ".ogm" ) || 0 == stricmp( _ext, ".gif" ) ) )
        *_ext = 0;
}
...
void CTextureDescrMngr::LoadTHM( LPCSTR initial,
    ...
    struct PreparedThm {
        ...
        xr_string bump_name;
        u32 flags =
            STextureParams::flGenerateMipMaps | STextureParams::flDitherColor;
    ...
    xr_parallel_for(
        0u, static_cast< u32 >( prepared.size() ), [ & ]( u32 index ) {
            ...
            if ( F->find_chunk( THM_CHUNK_TEXTURE_TYPE ) )
                result.type =
                    static_cast< STextureParams::ETType >( F->r_u32() );
            ...
            if ( F->find_chunk( THM_CHUNK_BUMP ) ) {
                F->r_float();
                result.bump_mode =
                    static_cast< STextureParams::ETBumpMode >( F->r_u32() );
    ...
    for ( const PreparedThm& result : prepared ) {
        if ( STextureParams::ttImage == result.type ||
             STextureParams::ttTerrain == result.type ||
            ...
            if ( !result.detail_name.empty() &&
                 ( result.flags & ( STextureParams::flDiffuseDetail |
                                    STextureParams::flBumpDetail ) ) ) {
                if ( desc.m_assoc )
                ...
                if ( dts )
                    ...
                    /*desc.m_assoc->cs*/ dts =
                        xr_new< cl_dt_scaler >( result.detail_scale );
            ...
            desc.m_spec->m_material =
                result.material +
                ( result.material < 4 ? result.material_weight : 0 );
    ...
    Msg( "* [STARTUP/THM] source=%s files=%u prepare=%u ms commit=%u ms "
         "total=%u ms",
         initial, static_cast< u32 >( prepared.size() ), prepareMs,
         timer.GetElapsed_ms() - prepareMs, timer.GetElapsed_ms() );
}
...
void CTextureDescrMngr::Load() {
    ...
    // Level THMs override game THMs deterministically, as intended by the old
    // two-source load.
...
void CTextureDescrMngr::GetTextureUsage( const shared_str& tex_name,
                                         BOOL& bDiffuse,
                                         BOOL& bBump ) const {
    xrSRWLockGuard dataGuard( m_data_lock, true );
    map_TD::const_iterator I = m_texture_details.find( tex_name );
...
BOOL CTextureDescrMngr::GetDetailTexture( const shared_str& tex_name,
                                          LPCSTR& res,
