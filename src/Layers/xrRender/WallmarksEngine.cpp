...
IC bool operator==( const CWallmarksEngine::wm_slot* slot,
                    const ref_shader& shader ) {
...
void CWallmarksEngine::clear() {
    {
        for ( auto p_it = marks.begin(); p_it != marks.end(); p_it++ ) {
            for ( auto& [ sector, static_vec ] : ( *p_it )->static_items ) {
                for ( auto m_it = static_vec.begin(); m_it != static_vec.end();
                      m_it++ )
...
// destroy
void CWallmarksEngine::static_wm_destroy(
    CWallmarksEngine::static_wallmark* W ) {
    static_pool.push_back( W );
}

// render
void CWallmarksEngine::static_wm_render( CWallmarksEngine::static_wallmark* W,
                                         FVF::LIT*& V ) {
    float a = W->TimeEnd() == -1.f
                  ? 0.f
                  : ( RDEVICE.fTimeGlobal - W->TimeStart() ) / W->TimeEnd();
...
//--------------------------------------------------------------------------------
void CWallmarksEngine::RecurseTri( u32 t,
                                   Fmatrix& mView,
                                   CWallmarksEngine::static_wallmark& W ) {
    ...
    if ( P ) {
        ...
        for ( u32 i = 0; i < 3; i++ ) {
            ...
            Fvector test_normal;
            test_normal.mknormal( v_data[ v_ids[ 0 ] ], v_data[ v_ids[ 1 ] ],
                                  v_data[ v_ids[ 2 ] ] );
...
void CWallmarksEngine::BuildMatrix( Fmatrix& mView,
                                    float invsz,
                                    const Fvector& from ) {
    // build projection
    Fmatrix mScale;
...
void CWallmarksEngine::AddWallmark_internal( CDB::TRI* pTri,
    ...
    sml_collector.add_face_packed_D( pVerts[ pTri->verts[ 0 ] ],
                                     pVerts[ pTri->verts[ 1 ] ],
                                     pVerts[ pTri->verts[ 2 ] ], 0 );
    for ( u32 t = 0; t < triCount; t++ ) {
        CDB::TRI* T = tris + xrc.r_begin()[ t ].id;
        if ( T == pTri )
            continue;
        sml_collector.add_face_packed_D( pVerts[ T->verts[ 0 ] ],
                                         pVerts[ T->verts[ 1 ] ],
                                         pVerts[ T->verts[ 2 ] ], 0 );
    }
    ...
    // calc face normal
    Fvector N;
    N.mknormal( pVerts[ pTri->verts[ 0 ] ], pVerts[ pTri->verts[ 1 ] ],
                pVerts[ pTri->verts[ 2 ] ] );
    sml_normal.set( N );
...
void CWallmarksEngine::AddWallmark_internal( CDB::TRI* pTri,
    ...
    AddWallmark_internal(
        pTri, pVerts, contact_point, hShader, sz, ttl,
...
void CWallmarksEngine::AddStaticWallmark( CDB::TRI* pTri,
                                          ...
                                          bool ignore_opt,
                                          bool random_rotation ) {
    AddStaticWallmark( pTri, pVerts, contact_point, hShader, sz, ttl,
                       ignore_opt,
                       random_rotation ? ::Random.randF( -20.f, 20.f ) : 0.f );
}
...
void CWallmarksEngine::AddSkeletonWallmark( const Fmatrix* xf,
                                            CKinematics* obj,
                                            ref_shader& sh,
                                            const Fvector& start,
                                            const Fvector& dir,
...
ICF void BeginStream( ref_geom hGeom,
    ...
    w_offset = 0;
    w_verts = ( FVF::LIT* )RCache.Vertex.Lock( MAX_TRIS * 3, hGeom->vb_stride,
                                               w_offset );
    w_start = w_verts;
}

ICF void FlushStream( ref_geom hGeom,
                      ref_shader shader,
                      u32& w_offset,
                      FVF::LIT*& w_verts,
                      FVF::LIT*& w_start,
...
void CWallmarksEngine::RemoveSkeletonWallmarksFromObject( CKinematics* obj ) {
    ...
    for ( wm_slot* slot : marks ) {
        slot->skeleton_items.erase(
            std::remove_if(
                slot->skeleton_items.begin(), slot->skeleton_items.end(),
                [ obj ]( const intrusive_ptr< CSkeletonWallmark >& wm ) {
                    return wm && wm->Parent() == obj;
                } ),
...
void CWallmarksEngine::UpdateWallmarks() {
    ...
    for ( wm_slot* slot : marks ) {
        // static wallmarks
        for ( auto it_sect = slot->static_items.begin();
              it_sect != slot->static_items.end(); ) {
        ...
        // dynamic wallmarks
        slot->skeleton_items.erase(
            std::remove_if( slot->skeleton_items.begin(),
                            slot->skeleton_items.end(),
                            []( const intrusive_ptr< CSkeletonWallmark >& wm ) {
                                return wm->TimeEnd() != -1.f &&
                                       RDEVICE.fTimeGlobal >
                                           ( wm->TimeStart() + wm->TimeEnd() );
                            } ),
            slot->skeleton_items.end() );
    }
}
...
void CWallmarksEngine::Render() {
    ...
    Fvector mViewPos;
    mViewPos.mad( Device.vCameraPosition, Device.vCameraDirection,
                  ps_r__WallmarkSHIFT_V );
    Device.mView.build_camera_dir( mViewPos, Device.vCameraDirection,
                                   Device.vCameraTop );
    ...
    if ( ( r_wallmarks_static || r_wallmarks_dynamic ) && !marks.empty() ) {
        ...
        xrCriticalSectionGuard g(
            lock ); // Physics may add wallmarks in parallel with rendering

        for ( int i = 0; i < marks.size(); i++ ) {
            wm_slot* slot = marks[ i ];
            bool static_empty =
                !r_wallmarks_static || slot->static_items.empty();
            bool skeleton_empty =
                !r_wallmarks_dynamic || slot->skeleton_items.empty();
            ...
            if ( !static_empty ) {
                ...
                if ( !skeleton_empty ) {
                    // Flush stream
                    FlushStream(
                        hGeom, slot->shader, w_offset, w_verts, w_start,
                        FALSE ); //. remove line if !(suppress cull needed)
                    BeginStream( hGeom, w_offset, w_verts, w_start );
                }
            }
            ...
            if ( !skeleton_empty ) {
                ...
                for ( int j = 0; j < slot->skeleton_items.size(); j++ ) {
                    intrusive_ptr< CSkeletonWallmark > W =
                        slot->skeleton_items[ j ];
                    if ( !W )
                        continue;
                    ...
                    if ( !RImplementation.ViewBase.testSphere_dirty(
                             W->m_Bounds.P, W->m_Bounds.R ) )
                        continue;

                    Device.Statistic->RenderDUMP_WMD_Count++;
                    float dst =
                        Device.vCameraPosition.distance_to_sqr( W->m_Bounds.P );
                    ...
                    if ( ( w_count + W->VCount() ) >= max_verts ) {
                        FlushStream( hGeom, slot->shader, w_offset, w_verts,
                                     w_start, TRUE );
                        BeginStream( hGeom, w_offset, w_verts, w_start );
                    }
