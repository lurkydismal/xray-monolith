...
void CDetailManager::soft_Load() {
    ...
    // Vertex Stream
    soft_Geom.create( D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1,
...
void CDetailManager::soft_Render() {
    ...
    // float	fPhaseX		= _sin(RDEVICE.fTimeGlobal*0.1f)
    // *fPhaseRange; float	fPhaseZ		=
    // _sin(RDEVICE.fTimeGlobal*0.11f)*fPhaseRange;
    ...
    for ( u32 O = 0; O < objects.size(); O++ ) {
        ...
        for ( ; _vI != _vE; _vI++ ) {
            ...
            for ( u32 L_ID = 0; L_ID < lock_count; L_ID++ ) {
                    ...
                    VERIFY( iOffset < 65535 );
                    {
                        ...
                        if ( Object.number_indices & 1 )
                            iDest[ Object.number_indices - 1 ] =
                                ( u16 )( Object.indices[ Object.number_indices -
                                                         1 ] +
                                         u16( iOffset ) );
                    }
                ...
                RCache.Render( D3DPT_TRIANGLELIST, vBase, 0, vCount_Lock, iBase,
                               dwNumPrimitives );
            }
        }
        // Clean up
...
//.
                ...
                for	(u32 k=0; k<vCount_Lock; k++)
                {
                                        ...
                                        {
                                                u32
C = 0xffffffff; CDetail::fvfVertexIn	*srcIt = Object.vertices, *srcEnd =
Object.vertices+Object.number_vertices; CDetail::fvfVertexOut	*dstIt = vDest;

                                                for	(; srcIt!=srcEnd;
srcIt++, dstIt++)
                                                {
                                                        mXform.transform_tiny
(dstIt->P,srcIt->P); dstIt->C	= C; dstIt->u	= srcIt->u; dstIt->v	=
srcIt->v;
                                                }
                                        }
                }
