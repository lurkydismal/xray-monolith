...
void CKinematics::CalculateBones( BOOL bForceExact ) {
    ...
    // skip all the computations - assume nothing changes in a small period of
    // time :)
    if ( RDEVICE.dwTimeGlobal == UCalc_Time )
        return; // early out for "fast" update

    // demonized: reduce calculate bones updates when the object is far away and
    // not in frustum Available only if can get parent xform Refactor later for
    // per object basis
    ...
    if ( g_bootComplete ) {
        if ( spatialParent ) {
            ...
            if ( !visibleCheck ) {
                ...
                {
                        Msg("CKinematics::CalculateBones, object out of frustum,
                dist %.2f, update_rate_k %.2f", dist / dist_k, update_rate_k);
                }*/
            }
            ...
            if ( r_optimize_calculate_bones && canBeOptimized() &&
                ...
                /*if (RDEVICE.dwTimeGlobal % 100 < 10)
                {
                        Msg("CKinematics::CalculateBones, object canBeOptimized
                and dist > IK_CALC_DIST * dist_k, dist %.2f, update_rate_k %.2f,
                ssa %.4f", dist / dist_k, update_rate_k, ssa);
                }*/
            }
        }
    }
    ...
    OnCalculateBones();
    if ( !bForceExact && ( RDEVICE.dwTimeGlobal <
                           ( UCalc_Time + UCalc_Interval * update_rate_k ) ) )
    ...
    if ( UCalc_Visibox >= psSkeletonUpdate ) {
...
#ifdef DEBUG
        // Validate
        VERIFY3( _valid( vis.box.min ) && _valid( vis.box.max ),
                 "Invalid bones-xform in model", dbg_name.c_str() );
        ...
        VERIFY3( vis.sphere.R < 1000.f, "Invalid bones-xform in model",
                 dbg_name.c_str() );
#endif
    } else
...
void check_kinematics( CKinematics* _k, LPCSTR s ) {
    CKinematics* K = _k;
    Fmatrix& MrootBone =
        K->LL_GetBoneInstance( K->LL_GetBoneRoot() ).mTransform;
...
void CKinematics::BuildBoneMatrix( const CBoneData* bd,
                                   CBoneInstance& bi,
...
void CKinematics::CLBone( const CBoneData* bd,
                          CBoneInstance& bi,
                          const Fmatrix* parent,
                          u8 channel_mask /*= (1<<0)*/ ) {
...
void CKinematics::Bone_GetAnimPos( Fmatrix& pos,
                                   u16 id,
                                   u8 mask_channel,
    ...
    BoneChain_Calculate( &LL_GetData( id ), bi, mask_channel,
                         ignore_callbacks );
#ifndef MASTER_GOLD
...
void CKinematics::BoneChain_Calculate( const CBoneData* bd,
                                       CBoneInstance& bi,
                                       u8 mask_channel,
                                       bool ignore_callbacks ) {
    u16 SelfID = bd->GetSelfID();
    ...
    BoneChain_Calculate( ParrentDT, parrent_bi, mask_channel,
                         ignore_callbacks );
