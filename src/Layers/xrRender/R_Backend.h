...
public:
    IC CTexture* get_ActiveTexture( u32 stage ) {
        ...
        if ( stage < CTexture::rstVertex )
            ...
            T = textures_vs[ stage - CTexture::rstVertex ];
#ifdef USE_DX10
        else
            T = textures_gs[ stage - CTexture::rstGeometry ];
#elif USE_DX11
        else if ( stage < CTexture::rstHull )
            T = textures_gs[ stage - CTexture::rstGeometry ];
        else if ( stage < CTexture::rstDomain )
            T = textures_hs[ stage - CTexture::rstHull ];
        else if ( stage < CTexture::rstCompute )
            T = textures_ds[ stage - CTexture::rstDomain ];
        else if ( stage < CTexture::rstInvalid )
            T = textures_cs[ stage - CTexture::rstCompute ];
        else {
...
#if defined( USE_DX10 ) || defined( USE_DX11 )
    IC void get_ConstantDirect( shared_str& n,
                                u32 DataSize,
                                void** pVData,
                                void** pGData,
                                void** pPData );
...
public:
    ...
    IC void set_Stencil( u32 _enable,
    ...
    IC void set_ColorWriteEnable( u32 _mask = D3DCOLORWRITEENABLE_RED |
    ...
    void set_ClipPlanes( u32 _enable,
                         Fmatrix* _xform = NULL,
                         u32 fmask = 0xff );
    ...
    ICF void Render( D3DPRIMITIVETYPE T,
                     u32 baseV,
                     u32 startV,
                     u32 countV,
                     u32 startI,
...
#ifdef USE_DX11
    ICF void Compute( UINT ThreadGroupCountX,
                      UINT ThreadGroupCountY,
                      UINT ThreadGroupCountZ );
#endif

    // Device create / destroy / frame signaling
    void RestoreQuadIBData(); // Igor: is used to test bug with rain, particles
                              // corruption
    void CreateQuadIB();
    void OnFrameBegin();
    void OnFrameEnd();
    ...
    void dbg_DIP( D3DPRIMITIVETYPE pt,
                  ref_geom geom,
                  u32 baseV,
                  u32 startV,
                  u32 countV,
    ...
    void dbg_Draw( D3DPRIMITIVETYPE T,
                   FVF::L* pVerts,
                   int vcnt,
                   u16* pIdx,
                   int pcnt );
...
private:
    ...
    bool CBuffersNeedUpdate( ref_cbuffer buf1[ MaxCBuffers ],
