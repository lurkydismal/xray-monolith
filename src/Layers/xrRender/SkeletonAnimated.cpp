...
void CKinematicsAnimated::Bone_Motion_Start_IM( CBoneData* bd,
                                                CBlend* handle ) {
    LL_GetBlendInstance( bd->GetSelfID() ).blend_add( handle );
}
...
std::pair< LPCSTR, LPCSTR > CKinematicsAnimated::LL_MotionDefName_dbg(
    MotionID ID ) {
...
static void dump_blend( CKinematicsAnimated* K, CBlend& B, u32 index ) {
    ...
    Msg( "time total: %f, speed: %f , power: %f ", B.timeTotal, B.speed,
         B.blendPower );
    Msg( "ammount: %f, time current: %f, frame %d ", B.blendAmount,
         B.timeCurrent, B.dwFrame );
    ...
    Msg( "state: %s, playing: %s, stop_at_end_callback: %s ",
    ...
    if ( B.blend_state() != CBlend::eFREE_SLOT ) {
        Msg( "motion : name %s, set: %s ",
             K->LL_MotionDefName_dbg( B.motionID ).first,
...
MotionID CKinematicsAnimated::ID_Cycle( shared_str N ) {
    ...
    R_ASSERT2( motion_ID.valid(),
               make_string( "! MODEL [%s]: can't find cycle: [%s]",
...
void CKinematicsAnimated::LL_FadeCycle( u16 part,
                                        float falloff,
                                        u8 mask_channel /*= (1<<0)*/ ) {
...
void CKinematicsAnimated::IBlendSetup( CBlend& B,
                                       u16 part,
                                       u8 channel,
                                       MotionID motion_ID,
                                       BOOL bMixing,
    ...
    B.timeCurrent = 0;
    B.timeTotal = m_Motions[ B.motionID.slot ]
                      .bone_motions[ LL_GetBoneRoot() ]
                      ->at( motion_ID.idx )
                      .GetLength();
    B.bone_or_part = part;
...
void CKinematicsAnimated::IFXBlendSetup( CBlend& B,
                                         MotionID motion_ID,
                                         float blendAccrue,
                                         float blendFalloff,
                                         float Power,
...
CBlend* CKinematicsAnimated::LL_PlayCycle( u16 part,
                                           MotionID motion_ID,
                                           BOOL bMixing,
                                           float blendAccrue,
                                           float blendFalloff,
    ...
    if ( BI_NONE == part ) {
        for ( u16 i = 0; i < MAX_PARTS; i++ )
            LL_PlayCycle( i, motion_ID, bMixing, blendAccrue, blendFalloff,
                          Speed, noloop, Callback, CallbackParam, channel );
        return 0;
    }
    if ( part >= MAX_PARTS )
        return 0;
    if ( 0 == m_Partition->part( part ) )
        return 0;
    ...
    _DBG_SINGLE_USE_MARKER;
    IBlendSetup( *B, part, channel, motion_ID, bMixing, blendAccrue,
                 blendFalloff, Speed, noloop, Callback, CallbackParam );
    for ( u32 i = 0; i < P->bones.size(); i++ ) {
        if ( !( *bones )[ P->bones[ i ] ] )
            Debug.fatal( DEBUG_INFO,
                         "! MODEL: missing bone/wrong armature? : %s",
                         *getDebugName() );
...
CBlend* CKinematicsAnimated::LL_PlayCycle( u16 part,
    ...
    CMotionDef* m_def =
        m_Motions[ motion_ID.slot ].motions.motion_def( motion_ID.idx );
    VERIFY( m_def );
    if ( !m_def )
        return NULL;
    return LL_PlayCycle( part, motion_ID, bMixIn, m_def->Accrue(),
                         m_def->Falloff(), m_def->Speed(), m_def->StopAtEnd(),
                         Callback, CallbackParam, channel );
}

CBlend* CKinematicsAnimated::PlayCycle( LPCSTR N,
                                        BOOL bMixIn,
                                        PlayCallback Callback,
                                        LPVOID CallbackParam,
                                        u8 channel /*= 0*/ ) {
    ...
    if ( motion_ID.valid() )
        ...
        Debug.fatal( DEBUG_INFO, "! MODEL [%s]: can't find cycle: [%s]",
                     dbg_name.c_str(), N );
        return 0;
    }
}

CBlend* CKinematicsAnimated::PlayCycle( MotionID motion_ID,
                                        BOOL bMixIn,
                                        PlayCallback Callback,
                                        LPVOID CallbackParam,
                                        u8 channel /*= 0*/ ) {
    ...
    return LL_PlayCycle( m_def->bone_or_part, motion_ID, bMixIn,
                         m_def->Accrue(), m_def->Falloff(), m_def->Speed(),
                         m_def->StopAtEnd(), Callback, CallbackParam, channel );
}

CBlend* CKinematicsAnimated::PlayCycle( u16 partition,
                                        MotionID motion_ID,
                                        BOOL bMixIn,
                                        PlayCallback Callback,
                                        LPVOID CallbackParam,
    ...
    if ( !m_def )
        return NULL;
    return LL_PlayCycle( partition, motion_ID, bMixIn, m_def->Accrue(),
                         m_def->Falloff(), m_def->Speed() * speed,
                         m_def->StopAtEnd(), Callback, CallbackParam, channel );
}
...
CBlend* CKinematicsAnimated::PlayFX( MotionID motion_ID, float power_scale ) {
    ...
    if ( !m_def )
        return NULL;
    return LL_PlayFX( m_def->bone_or_part, motion_ID, m_def->Accrue(),
                      m_def->Falloff(), m_def->Speed(),
...
// u16 part,u8 channel, MotionID motion_ID, BOOL  bMixing, float blendAccrue,
// float blendFalloff, float Speed, BOOL noloop, PlayCallback callback(), LPVOID
// CallbackParam)

CBlend* CKinematicsAnimated::LL_PlayFX( u16 bone,
                                        MotionID motion_ID,
                                        float blendAccrue,
                                        float blendFalloff,
                                        float Speed,
    ...
    _DBG_SINGLE_USE_MARKER;
    IFXBlendSetup( *B, motion_ID, blendAccrue, blendFalloff, Power, Speed,
                   bone );
...
void CKinematicsAnimated::LL_UpdateTracks( float dt,
                                           bool b_force,
                                           bool leave_blends ) {
    BlendSVecIt I, E;
    // Cycles
...
void CKinematicsAnimated::UpdateTracks() {
    ...
    if ( GetUpdateTracksCalback() ) {
        if ( ( *GetUpdateTracksCalback() )(
                 float( RDEVICE.dwTimeGlobal - Update_LastTime ) / 1000.f,
                 *this ) )
...
void CKinematicsAnimated::Load( const char* N, IReader* data, u32 dwFlags ) {
    ...
    const auto loadOMF = [ & ]( LPCSTR _path ) {
        ...
        if ( !FS.exist( fn, "$level$", _path ) ) {
            if ( !FS.exist( fn, "$game_meshes$", _path ) )
                Debug.fatal(
                    DEBUG_INFO,
                    "Can't find motion file '%s'\nsection '%s'\nmodel '%s'",
                    _path, current_player_hud_sect.c_str(), N );
        }
        ...
        if ( create_res )
            ...
            m_Motions.pop_back();
            Msg( "! error in model [%s]. Unable to load motion file '%s', "
                 "section '%s'.",
                 N, _path, current_player_hud_sect.c_str() );
        }
    };
    ...
    if ( data->find_chunk( OGF_S_MOTION_REFS ) ) {
        ...
        R_ASSERT2( set_cnt < MAX_ANIM_SLOT,
                   make_string( "section '%s'\nmodel '%s'",
                                current_player_hud_sect.c_str(), N )
                       .c_str() );
        ...
        for ( u32 k = 0; k < set_cnt; ++k ) {
            ...
            if ( strstr( nm, "\\*.omf" ) ) {
                ...
                for ( FS_FileSet::iterator it = fset.begin(); it != fset.end();
                      it++ )
            ...
            if ( strstr( nm, "stalker_animation.omf" ) ) {
                ...
                FS.file_list( extra_anims, "$game_meshes$", FS_ListFiles,
                              "actors\\modded_stalker_animations\\*.omf" );

                if ( !extra_anims.empty() ) {
                    for ( auto it = extra_anims.begin();
                          it != extra_anims.end(); ++it ) {
                        loadOMF( it->name.c_str() );
                    }
                }
            }
    ...
    // initialize motions
    for ( MotionsSlotVecIt m_it = m_Motions.begin(); m_it != m_Motions.end();
          m_it++ ) {
...
void CKinematicsAnimated::LL_BuldBoneMatrixDequatize( const CBoneData* bd,
                                                      u8 channel_mask,
                                                      SKeyTable& keys ) {
    ...
    for ( u16 j = 0; MAX_CHANNELS > j; ++j )
        if ( channels.rule( j ).extern_ == animation::add )
            keys_substruct( keys.keys[ j ], BK[ j ],
                            keys.chanel_blend_conts[ j ] );
}

// calculate single bone with key blending
void CKinematicsAnimated::LL_BoneMatrixBuild( u16 bone_id,
                                              CBoneInstance& bi,
                                              const Fmatrix* parent,
                                              const SKeyTable& keys ) {
    // Blend them together
    ...
    for ( u16 j = 0; MAX_CHANNELS > j; ++j ) {
        ...
        process_single_channel( channel_keys[ ch_count ], BC[ ch_count ],
                                keys.keys[ j ], keys.blends[ j ],
                                keys.chanel_blend_conts[ j ] );
        ++ch_count;
    }
    CKey Result;
    ...
    dbg_box.set( -box_size, -box_size, -box_size, box_size, box_size,
                 box_size );
    // VERIFY(dbg_box.contains(bi.mTransform.c));
    VERIFY2( dbg_box.contains( bi.mTransform.c ),
             ( make_string( "model: %s has strange bone position, matrix : ",
                            getDebugName().c_str() ) +
...
void CKinematicsAnimated::BuildBoneMatrix( const CBoneData* bd,
                                           CBoneInstance& bi,
                                           const Fmatrix* parent,
                                           u8 channel_mask /*= (1<<0)*/ ) {
    // CKey				R
    // [MAX_CHANNELS][MAX_BLENDED];	//all keys float
    // BA
    // [MAX_CHANNELS][MAX_BLENDED];	//all factors int
    // b_counts				[MAX_CHANNELS]	= {0,0,0,0}; //channel
    // counts
    SKeyTable keys;
    ...
    CBlend*			B		=	*BI;
    CMotion&		M		=
    *LL_GetMotion(B->motionID,SelfID); float			time	=
    B->timeCurrent*float(SAMPLE_FPS); u32				frame
    =	iFloor(time); u32				count	=
    M.get_count(); float			delta	=
    time-float(frame);
...
void CKinematicsAnimated::SetUpdateTracksCalback(
    IUpdateTracksCallback* callback ) {
