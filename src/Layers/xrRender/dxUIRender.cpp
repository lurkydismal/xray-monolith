...
void dxUIRender::StartTriList(u32 iMaxVerts)
{
        ...
        start_pv	= (FVF::LIT*)RCache.Vertex.Lock
(m_iMaxVerts,hGeom_fan.stride(),vOffset); pv			= start_pv;
...
void dxUIRender::FlushTriFan()
{
        ...
        RCache.set_Geometry	 		(hGeom_fan);
        if (p_cnt>2) RCache.Render
(D3DPT_TRIANGLEFAN,vOffset,u32(p_cnt-2));
...
void dxUIRender::StartTriStrip(u32 iMaxVerts)
{
        VERIFY(PrimitiveType==ptNone);
        m_iMaxVerts = iMaxVerts;
        start_pv	= (FVF::TL*)RCache.Vertex.Lock
(m_iMaxVerts,hGeom_fan.stride(),vOffset); pv			= start_pv;
...
void dxUIRender::PushPoint( float x,
                            float y,
                            float z,
                            u32 C,
                            float u,
...
void dxUIRender::StartPrimitive( u32 iMaxVerts,
    ...
    switch ( m_PointType ) {
        case pttLIT:
            LIT_start_pv = ( FVF::LIT* )RCache.Vertex.Lock(
                m_iMaxVerts, hGeom_LIT.stride(), vOffset );
            LIT_pv = LIT_start_pv;
            break;
        case pttTL:
            TL_start_pv = ( FVF::TL* )RCache.Vertex.Lock(
                m_iMaxVerts, hGeom_TL.stride(), vOffset );
            TL_pv = TL_start_pv;
            break;
    }
}
