...
void AppendTextureList( LPCSTR source, xr_vector< xr_string >& textures ) {
    for ( int index = 0, count = _GetItemCount( source, ',' ); index < count;
          ++index ) {
        string_path texture;
        ...
        for ( LPSTR cursor = texture; *cursor; ++cursor )
            if ( *cursor == '/' )
                *cursor = '\\';
        if ( texture[ 0 ] && xr_strcmp( texture, "null" ) &&
             xr_strcmp( texture, "$null" ) )
            textures.emplace_back( texture );
    }
}

bool ResolveModelFile( LPCSTR source,
...
void CollectModelTextures( LPCSTR source,
                           LPCSTR canonical_level_path,
                           xr_vector< xr_string >& textures,
                           xr_set< xr_string >& visited );

void CollectModelReaderTextures( IReader& data,
                                 LPCSTR canonical_level_path,
                                 xr_vector< xr_string >& textures,
                                 xr_set< xr_string >& visited ) {
    ...
    if ( IReader* lod = data.open_chunk( OGF_S_LODS ) ) {
        ...
        lod->close();
        CollectModelTextures( lod_name, canonical_level_path, textures,
                              visited );
    }
    ...
    for ( u32 index = 0;; ++index ) {
        ...
        if ( !child )
            break;
        CollectModelReaderTextures( *child, canonical_level_path, textures,
                                    visited );
        child->close();
    }
    children->close();
...
void CollectModelTextures( LPCSTR source,
    ...
    if ( !reader )
        return;
    CollectModelReaderTextures( *reader, canonical_level_path, textures,
                                visited );
    FS.r_close( reader );
}
} // namespace
...
CModelPool::ModelBlueprint::ModelBlueprint()
    : completed( CreateEvent( nullptr, TRUE, FALSE, nullptr ) ),
      preparedVisual( nullptr ),
      found( false ) {
    R_ASSERT( completed );
}
...
dxRender_Visual* CModelPool::Instance_Duplicate( dxRender_Visual* V ) {
    ...
    N->Spawn();
    {
        ...
        auto it =
            std::lower_bound( Models.begin(), Models.end(), V, std::less<>{} );
...
dxRender_Visual* CModelPool::Instance_Load( const char* N,
                                            BOOL allow_register,
                                            bool assert ) {
    dxRender_Visual* V;
    string_path fn;
...
#else
                if ( assert )
                    Debug.fatal( DEBUG_INFO, "Can't find model file '%s'.",
                                 name );
                else
                    return nullptr;
#endif
...
dxRender_Visual* CModelPool::Instance_Load( LPCSTR name,
                                            IReader* data,
                                            BOOL allow_register ) {
    dxRender_Visual* V;
...
dxRender_Visual* CModelPool::Create( const char* name,
                                     IReader* data,
                                     bool assert ) {
#ifdef _EDITOR
    if ( !name || !name[ 0 ] )
...
void CModelPool::DeleteInternal( dxRender_Visual*& V, BOOL bDiscard ) {
    ...
    if ( bDiscard || bForceDiscard ) {
        ...
        if ( it != Registry.end() ) {
            // Registry entry found - move it to pool and reset changed
            // shader/texture if necessary
            ...
            // Registry entry not-found - just special type of visual /
            // particles / etc.
            xr_delete( V );
        }
    }
...
void CModelPool::DeleteDeffered( dxRender_Visual*& V ) {
    ...
    if ( std::find( ModelsToDeleteDeffer.begin(), ModelsToDeleteDeffer.end(),
                    V ) == ModelsToDeleteDeffer.end() )
...
void CModelPool::Discard( dxRender_Visual*& V, BOOL b_complete ) {
    ...
    if ( it != Registry.end() ) {
        ...
        // Registry entry not-found - just special type of visual / particles /
        // etc.
        xr_delete( V );
    }
    V = NULL;
...
xr_shared_ptr< CModelPool::ModelBlueprint > CModelPool::PrepareBlueprint(
    LPCSTR name,
    LPCSTR canonical_level_path ) {
    const xr_string normalized = NormalizeModelName( name );
    xr_string resolved_path;
    const bool resolved =
        ResolveModelFile( name, canonical_level_path, resolved_path );
    xr_string key = normalized;
    key += '\n';
    key += resolved ? resolved_path
    ...
    if ( resolved )
        if ( const CLocatorAPI::file* file =
                 FS.exist( resolved_path.c_str() ) ) {
            string128 identity;
            xr_sprintf( identity, "\n%08x:%08x:%08x:%08x", file->crc,
                        file->size_real, file->size_compressed, file->modif );
            key += identity;
        }
    ...
    if ( producer ) {
        ...
        if ( resolved ) {
            ...
            if ( source ) {
                ...
                CollectModelReaderTextures( texture_reader,
                                            canonical_level_path,
                                            blueprint->textures, visited );

#if RENDER == R_R4
                IReader header_reader( blueprint->data.data(), size );
                ogf_header header;
                R_ASSERT( header_reader.r_chunk_safe( OGF_HEADER, &header,
                                                      sizeof( header ) ) );
                IReader container_reader( blueprint->data.data(), size );
                const bool references_level_geometry =
                    container_reader.find_chunk( OGF_GCONTAINER ) ||
                    container_reader.find_chunk( OGF_VCONTAINER ) ||
                    container_reader.find_chunk( OGF_ICONTAINER ) ||
                    container_reader.find_chunk( OGF_FASTPATH );
                IReader local_geometry_reader( blueprint->data.data(), size );
                const bool has_local_geometry =
                    local_geometry_reader.find_chunk( OGF_VERTICES ) &&
                    local_geometry_reader.find_chunk( OGF_INDICES );
                if ( !references_level_geometry && has_local_geometry &&
                     ( header.type == MT_NORMAL ||
                       header.type == MT_PROGRESSIVE ) ) {
                    ...
                    blueprint->preparedVisual->Load( normalized.c_str(),
                                                     &visual_reader, 0 );
#if 0
						}
						catch (...)
        ...
        blueprint->textures.erase( std::unique( blueprint->textures.begin(),
...
void CModelPool::CollectTextures( LPCSTR name,
                                  LPCSTR canonical_level_path,
                                  xr_vector< xr_string >& textures ) {
    xr_shared_ptr< ModelBlueprint > blueprint =
        PrepareBlueprint( name, canonical_level_path );
    textures.insert( textures.end(), blueprint->textures.begin(),
                     blueprint->textures.end() );
...
bool CModelPool::PrefetchPrepared( LPCSTR name,
                                   LPCSTR canonical_level_path,
                                   bool assert ) {
    xr_shared_ptr< ModelBlueprint > blueprint =
        PrepareBlueprint( name, canonical_level_path );
    ...
    if ( Instance_Find( normalized.c_str() ) ) {
        ...
        if ( base )
            ...
            IReader data( blueprint->data.data(),
                          static_cast< int >( blueprint->data.size() ) );
            const BOOL previous_allow_children_duplicate =
                bAllowChildrenDuplicate;
            bAllowChildrenDuplicate = FALSE;
#if 0
				try
				{
...
bool CModelPool::Exists( LPCSTR N ) {
    ...
    // Search for already loaded model (reference, base model) and return early
    // if exists
    dxRender_Visual* Base = Instance_Find( low_name );
    if ( Base )
        return true;
...
void CModelPool::dump() {
    ...
    u32 k = 0;
    xrSRWLockGuard g( ModelsLock, true );
    for ( xr_vector< ModelDef >::iterator I = Models.begin(); I != Models.end();
          I++ ) {
        CKinematics* K = PCKinematics( I->model );
        if ( K ) {
            u32 cur = K->mem_usage( false );
            sz += cur;
            Msg( "#%3d: [%3d/%5d Kb] - %s", k++, I->refs, cur / 1024,
                 I->name.c_str() );
        }
    }
    ...
    for ( REGISTRY_IT it = Registry.begin(); it != Registry.end(); it++ ) {
        ...
        if ( K ) {
            ...
            if ( b_free )
                ++free_cnt;
            Msg( "#%3d: [%s] [%5d Kb] - %s", k++, ( b_free ) ? "free" : "used",
                 cur / 1024, it->second.c_str() );
        }
    }
    Msg( "--- instances: %d, free %d, mem usage: %d Kb ", k, free_cnt,
         sz / 1024 );
    Log( "--- model pool --- end." );
}

void CModelPool::memory_stats( u32& vb_mem_video,
                               u32& vb_mem_system,
                               u32& ib_mem_video,
                               u32& ib_mem_system ) {
    vb_mem_video = 0;
...
void CModelPool::Render( dxRender_Visual* m_pVisual,
    ...
    switch ( m_pVisual->Type ) {
        ...
        case MT_SKELETON_RIGID: {
            if ( _IsBoxVisible( m_pVisual, mTransform ) ) {
                CKinematics* pV =
                    fast_dynamic_cast< CKinematics* >( m_pVisual );
                VERIFY( pV );
                if ( fis_zero( m_fLOD, EPS ) && pV->m_lod ) {
                    if ( _IsValidShader( pV->m_lod, priority, strictB2F ) ) {
                        RCache.set_Shader( pV->m_lod->shader
                                               ? pV->m_lod->shader
                                               : EDevice.m_WireShader );
        ...
        case MT_HIERRARHY: {
            if ( _IsBoxVisible( m_pVisual, mTransform ) ) {
                FHierrarhyVisual* pV =
                    fast_dynamic_cast< FHierrarhyVisual* >( m_pVisual );
                VERIFY( pV );
                I = pV->children.begin();
                E = pV->children.end();
                for ( ; I != E; I++ ) {
        ...
        case MT_PARTICLE_GROUP: {
            PS::CParticleGroup* pG =
                fast_dynamic_cast< PS::CParticleGroup* >( m_pVisual );
            VERIFY( pG );
            //		if (_IsBoxVisible(m_pVisual,mTransform))
            {
                RCache.set_xform_world( mTransform );
                for ( PS::CParticleGroup::SItemVecIt i_it = pG->items.begin();
                      i_it != pG->items.end(); i_it++ ) {
                    xr_vector< dxRender_Visual* > visuals;
                    i_it->GetVisuals( visuals );
                    for ( xr_vector< dxRender_Visual* >::iterator it =
                              visuals.begin();
                          it != visuals.end(); it++ )
        ...
        case MT_PARTICLE_EFFECT: {
            //		if (_IsBoxVisible(m_pVisual,mTransform))
            {
                if ( _IsValidShader( m_pVisual, priority, strictB2F ) ) {
                    RCache.set_Shader( m_pVisual->shader
                                           ? m_pVisual->shader
                                           : EDevice.m_WireShader );
                    RCache.set_xform_world( mTransform );
...
void CModelPool::RenderSingle( dxRender_Visual* m_pVisual,
                               const Fmatrix& mTransform,
                               float m_fLOD ) {
    for ( int p = 0; p < 4; p++ ) {
