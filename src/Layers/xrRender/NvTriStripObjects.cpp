...
//
NvEdgeInfo* NvStripifier::FindEdgeInfo( NvEdgeInfoVec& edgeInfos,
                                        int v0,
                                        int v1 ) {
...
//
NvFaceInfo* NvStripifier::FindOtherFace( NvEdgeInfoVec& edgeInfos,
                                         int v0,
                                         int v1,
                                         NvFaceInfo* faceInfo ) {
...
bool NvStripifier::AlreadyExists( NvFaceInfo* faceInfo,
                                  NvFaceInfoVec& faceInfos ) {
...
//
void NvStripifier::BuildStripifyInfo( NvFaceInfoVec& faceInfos,
    ...
    for ( u32 i = 0; i < numTriangles; i++ ) {
        ...
        // create the face info and add it to the list of faces, but only if
        // this exact face doesn't already
        //  exist in the list
        ...
        if ( !AlreadyExists( faceInfo, faceInfos ) ) {
            ...
            if ( edgeInfo01 == NULL ) {
                ...
                if ( edgeInfo01->m_face1 != NULL )
                    ;
                // Msg("! WARNING: BuildStripifyInfo: > 2 triangles on an
                // edge... uncertain consequences");
                else
...
//
int NvStripifier::FindStartPoint( NvFaceInfoVec& faceInfos,
                                  NvEdgeInfoVec& edgeInfos ) {
    for ( int i = 0; i < faceInfos.size(); i++ ) {
        int ctr = 0;

        if ( FindOtherFace( edgeInfos, faceInfos[ i ]->m_v0,
                            faceInfos[ i ]->m_v1, faceInfos[ i ] ) == NULL )
            ctr++;
        if ( FindOtherFace( edgeInfos, faceInfos[ i ]->m_v1,
                            faceInfos[ i ]->m_v2, faceInfos[ i ] ) == NULL )
            ctr++;
        if ( FindOtherFace( edgeInfos, faceInfos[ i ]->m_v2,
                            faceInfos[ i ]->m_v0, faceInfos[ i ] ) == NULL )
            ctr++;
        if ( ctr > 1 )
            return i;
    }
...
//
NvFaceInfo* NvStripifier::FindGoodResetPoint( NvFaceInfoVec& faceInfos,
                                              NvEdgeInfoVec& edgeInfos ) {
    ...
    if ( result == NULL ) {
        ...
        if ( bFirstTimeResetPoint ) {
            // first time, find a face with few neighbors (look for an edge of
            // the mesh)
...
IC int NvStripifier::GetNextIndex( const WordVec& indices, NvFaceInfo* face ) {
    ...
    if ( fv0 != v0 && fv0 != v1 ) {
        if ( ( fv1 != v0 && fv1 != v1 ) || ( fv2 != v0 && fv2 != v1 ) ) {
            Msg( "! WARNING: GetNextIndex: Triangle doesn't have all of its "
                 "vertices" );
            Msg( "! WARNING: GetNextIndex: Duplicate triangle probably got us "
                 "derailed" );
        }
        return fv0;
    }
...
IC bool NvStripInfo::IsMarked( NvFaceInfo* faceInfo ) {
    return ( faceInfo->m_stripId >= 0 ) ||
           ( IsExperiment() && faceInfo->m_experimentId == m_experimentId );
}
...
bool NvStripInfo::Unique( NvFaceInfoVec& faceVec, NvFaceInfo* face ) {
    bool bv0, bv1,
        bv2; // bools to indicate whether a vertex is in the faceVec or not
    bv0 = bv1 = bv2 = false;
...
// Build()
//
// Builds a strip forward as far as we can go, then builds backwards, and joins
// the two lists
//
void NvStripInfo::Build( NvEdgeInfoVec& edgeInfos, NvFaceInfoVec& faceInfos ) {
    ...
    int v0 = ( m_startInfo.m_toV1 ? m_startInfo.m_startEdge->m_v0
                                  : m_startInfo.m_startEdge->m_v1 );
    int v1 = ( m_startInfo.m_toV1 ? m_startInfo.m_startEdge->m_v1
                                  : m_startInfo.m_startEdge->m_v0 );
    ...
    int v2 =
        NvStripifier::GetNextIndex( scratchIndices, m_startInfo.m_startFace );
    ...
    NvFaceInfo* nextFace = NvStripifier::FindOtherFace(
        edgeInfos, nv0, nv1, m_startInfo.m_startFace );
    while ( nextFace != NULL && !IsMarked( nextFace ) ) {
        // this tests to see if a face is "unique", meaning that its vertices
    ...
    nv0 = v1;
    nv1 = v0;
    nextFace = NvStripifier::FindOtherFace( edgeInfos, nv0, nv1,
                                            m_startInfo.m_startFace );
    while ( nextFace != NULL && !IsMarked( nextFace ) ) {
        // this tests to see if a face is "unique", meaning that its vertices
        // aren't already in the list
        //  so, strips which "wrap-around" are not allowed
    ...
    // Combine the forward and backwards stripification lists and put into our
    // own face vector3
...
//
void NvStripInfo::Combine( const NvFaceInfoVec& forward,
                           const NvFaceInfoVec& backward ) {
    // add backward faces
...
//
bool NvStripInfo::SharesEdge( const NvFaceInfo* faceInfo,
                              NvEdgeInfoVec& edgeInfos ) {
    // check v0->v1 edge
    NvEdgeInfo* currEdge =
        NvStripifier::FindEdgeInfo( edgeInfos, faceInfo->m_v0, faceInfo->m_v1 );
    ...
    // check v1->v2 edge
    currEdge =
        NvStripifier::FindEdgeInfo( edgeInfos, faceInfo->m_v1, faceInfo->m_v2 );
    ...
    // check v2->v0 edge
    currEdge =
        NvStripifier::FindEdgeInfo( edgeInfos, faceInfo->m_v2, faceInfo->m_v0 );
...
// CommitStrips()
//
// "Commits" the input strips by setting their m_experimentId to -1 and adding
// to the allStrips
//  vector3
//
...
bool NvStripifier::FindTraversal( NvFaceInfoVec& faceInfos,
    ...
    // if the strip was v0->v1 on the edge, then v1 will be a vertex in the next
    // edge.
    int v =
        ( strip->m_startInfo.m_toV1 ? strip->m_startInfo.m_startEdge->m_v1
    ...
    while ( edgeIter != NULL ) {
        ...
        edgeIter =
            ( edgeIter->m_v0 == v ? edgeIter->m_nextV0 : edgeIter->m_nextV1 );
    }
...
// RemoveSmallStrips()
//
// allStrips is the whole strip _vector_...all small strips will be deleted from
// this list, to avoid leaking mem allBigStrips is an out parameter which will
// contain all strips above minStripLength faceList is an out parameter which
// will contain all faces which were removed from the striplist
//
void NvStripifier::RemoveSmallStrips( NvStripInfoVec& allStrips,
                                      NvStripInfoVec& allBigStrips,
...
//
void NvStripifier::CreateStrips( const NvStripInfoVec& allStrips,
                                 IntVec& stripIndices,
    ...
    // strips this is to account for that
    ...
    for ( int i = 0; i < nStripCount; i++ ) {
        ...
        {
            NvFaceInfo tFirstFace( strip->m_faces[ 0 ]->m_v0,
                                   strip->m_faces[ 0 ]->m_v1,
                                   strip->m_faces[ 0 ]->m_v2 );

            // If there is a second face, reorder vertices such that the
            // unique vertex is first
            if ( nStripFaceCount > 1 ) {
                int nUnique = NvStripifier::GetUniqueVertexInB(
                    strip->m_faces[ 1 ], &tFirstFace );
                ...
                if ( nStripFaceCount > 2 ) {
                    int nShared =
                        GetSharedVertex( strip->m_faces[ 2 ], &tFirstFace );
            ...
            if ( ( i == 0 ) || !bStitchStrips ) {
                if ( !IsCW( strip->m_faces[ 0 ], tFirstFace.m_v0,
                            tFirstFace.m_v1 ) )
                ...
                if ( NextIsCW( stripIndices.size() - accountForNegatives ) !=
                     IsCW( strip->m_faces[ 0 ], tFirstFace.m_v0,
                           tFirstFace.m_v1 ) ) {
                    stripIndices.push_back( tFirstFace.m_v0 );
                }
            }
...
//
void NvStripifier::Stripify( const WordVec& in_indices,
    ...
    minStripLength =
        in_minStripLength; // this is the strip size threshold below which we
                           // dump the strip into a list
    ...
    // split up the strips into cache friendly pieces, optimize them, then dump
    // these into outStrips
    ...
    for ( u32 i = 0; i < allEdgeInfos.size(); i++ ) {
        ...
        while ( info != NULL ) {
            NvEdgeInfo* next =
                ( info->m_v0 == int( i ) ? info->m_nextV0 : info->m_nextV1 );
            info->Unref();
            info = next;
        }
    }
...
//
// Splits the input _vector_ of strips (allBigStrips) into smaller, cache
// friendly pieces, then
//  reorders these pieces to maximize cache hits
// The final strips are output through outStrips
//
void NvStripifier::SplitUpStripsAndOptimize( NvStripInfoVec& allStrips,
                                             NvStripInfoVec& outStrips,
                                             NvEdgeInfoVec& edgeInfos,
                                             NvFaceInfoVec& outFaceList ) {
    ...
    for ( i = 0; i < allStrips.size(); i++ ) {
        ...
        if ( allStrips[ i ]->m_faces.size() > threshold ) {
            ...
            for ( j = 0; j < numTimes; j++ ) {
                ...
                for ( int faceCtr = j * threshold;
                      faceCtr < threshold + ( j * threshold ); faceCtr++ ) {
                    currentStrip->m_faces.push_back(
                        allStrips[ i ]->m_faces[ faceCtr ] );
                }
            ...
            if ( numLeftover != 0 ) {
                ...
                for ( int k = 0; k < numLeftover; k++ ) {
                    currentStrip->m_faces.push_back(
                        allStrips[ i ]->m_faces[ leftOff++ ] );
                }

                tempStrips.push_back( currentStrip );
            }
        } else {
            // we're not just doing a tempStrips.push_back(allBigStrips[i])
            // because
    ...
    if ( tempStrips2.size() != 0 ) {
        ...
        for ( i = 0; i < tempStrips2.size(); i++ ) {
            ...
            for ( int j = 0; j < tempStrips2[ i ]->m_faces.size(); j++ ) {
                numNeighbors +=
                    NumNeighbors( tempStrips2[ i ]->m_faces[ j ], edgeInfos );
            }

            float currCost = ( float )numNeighbors /
                             ( float )tempStrips2[ i ]->m_faces.size();
            if ( currCost < minCost ) {
                minCost = currCost;
                firstIndex = i;
            }
...
//
float NvStripifier::CalcNumHitsStrip( VertexCache* vcache,
                                      NvStripInfo* strip ) {
    int numHits = 0;
    int numFaces = 0;
...
//
// Works by setting runnning a number of experiments in different areas of the
// mesh, and
//  accepting the one which results in the longest strips.  It then accepts
//  this, and moves on to a different area of the mesh.  We try to jump around
//  the mesh some, to ensure that large open spans of strips get generated.
//
void NvStripifier::FindAllStrips( NvStripInfoVec& allStrips,
    ...
    while ( !done ) {
        ...
        for ( i = 0; i < numSamples; i++ ) {
            ...
            NvFaceInfo* nextFace =
                FindGoodResetPoint( allFaceInfos, allEdgeInfos );
            if ( nextFace == NULL ) {
                done = true;
                break;
            }
            ...
            NvEdgeInfo* edge01 =
                FindEdgeInfo( allEdgeInfos, nextFace->m_v0, nextFace->m_v1 );
            NvStripInfo* strip01 = xr_new< NvStripInfo >(
                NvStripStartInfo( nextFace, edge01, true ), stripId++,
                experimentId++ );
            ...
            NvEdgeInfo* edge10 =
                FindEdgeInfo( allEdgeInfos, nextFace->m_v0, nextFace->m_v1 );
            NvStripInfo* strip10 = xr_new< NvStripInfo >(
                NvStripStartInfo( nextFace, edge10, false ), stripId++,
                experimentId++ );
            experiments[ experimentIndex++ ].push_back( strip10 );

            // build the strip off of this face's 1-2 edge
            NvEdgeInfo* edge12 =
                FindEdgeInfo( allEdgeInfos, nextFace->m_v1, nextFace->m_v2 );
            NvStripInfo* strip12 = xr_new< NvStripInfo >(
                NvStripStartInfo( nextFace, edge12, true ), stripId++,
                experimentId++ );
            ...
            NvEdgeInfo* edge21 =
                FindEdgeInfo( allEdgeInfos, nextFace->m_v1, nextFace->m_v2 );
            NvStripInfo* strip21 = xr_new< NvStripInfo >(
                NvStripStartInfo( nextFace, edge21, false ), stripId++,
                experimentId++ );
            experiments[ experimentIndex++ ].push_back( strip21 );

            // build the strip off of this face's 2-0 edge
            NvEdgeInfo* edge20 =
                FindEdgeInfo( allEdgeInfos, nextFace->m_v2, nextFace->m_v0 );
            NvStripInfo* strip20 = xr_new< NvStripInfo >(
                NvStripStartInfo( nextFace, edge20, true ), stripId++,
                experimentId++ );
            ...
            NvEdgeInfo* edge02 =
                FindEdgeInfo( allEdgeInfos, nextFace->m_v2, nextFace->m_v0 );
            NvStripInfo* strip02 = xr_new< NvStripInfo >(
                NvStripStartInfo( nextFace, edge02, false ), stripId++,
                experimentId++ );
        ...
        for ( i = 0; i < numExperiments; i++ ) {
            ...
            while ( FindTraversal( allFaceInfos, allEdgeInfos, stripIter,
                ...
                stripIter =
                    xr_new< NvStripInfo >( startInfo, stripId++, experimentId );
        ...
        for ( i = 0; i < numExperiments; i++ ) {
            ...
            float value = avgStripSize * avgStripSizeWeight +
                          ( avgStripSize * numStrips * numTrisWeight );
