...
void CDSGraphManager::r_dsgraph_render_graph_sorted(
    R_dsgraph::mapDSGraphItems< T, Reverse >& graph,
    bool _clear ) {
    if ( graph.empty() )
        return;
...
void CDSGraphManager::r_dsgraph_render_graph( RenderQueueArray& queues,
                                              u32 _priority,
                                              bool _clear,
    ...
    for ( u32 iPass = 0; iPass < SHADER_PASSES_MAX; ++iPass ) {
        ...
        // 1. Sort by generated sort key to replicate previous fixed map
        // behaviour
        if ( queue.size() < 4096 )
...
void CDSGraphManager::r_dsgraph_render_hud() {
    ...
    if ( scope_3D_fake_enabled ) {
        RCache.set_RT( RImplementation.Target->rt_ssfx_temp->pRT,
                       3 ); // Render scope_3D to any buffer
        ...
        if ( !RImplementation.o.ssfx_motionvectors )
            ...
            RCache.set_RT( RImplementation.Target->rt_ssfx_motion_vectors->pRT,
                           3 );
    }
#endif
...
//////////////////////////////////////////////////////////////////////////
// strict-sorted render
void CDSGraphManager::r_dsgraph_render_ScopeSorted() //  Redotix99: for 3D
                                                     //  Shader Based Scopes
{
    // Change projection
    CHudInitializer initializer( true );
...
#if RENDER == R_R4
    HW.pContext->CopyResource( RImplementation.Target->rt_Accumulator->pSurface,
                               RImplementation.Target->rt_Generic_0->pSurface );
#endif
    CHudInitializer initializer( true );
...
void CDSGraphManager::r_dsgraph_render_water_ssr() {
    ...
    for ( auto& N : RGraph.mapWater ) {
        ...
        RCache.set_c( "cam_pos", RImplementation.Target->Position_previous.x,
                      RImplementation.Target->Position_previous.y,
...
void CDSGraphManager::r_dsgraph_capture_static() {
    ...
    PortalTraverseDebugStats* dbg =
        dbg_enabled ? &PortalTraverseDbg_Get() : nullptr;
        ...
        if ( psDeviceFlags.test( rsDrawStatic ) ) {
            ...
            for ( auto& pair : m_sector_frustums ) {
                ...
                if ( frustums.size() == 1 ) {
                    ...
                    add_Static( ( IRenderVisual* )pair.key->root(),
                                frustums.front(), frustums.front().getMask() );
                    continue;
                }
                ...
                add_Static_MultiFrustum( ( IRenderVisual* )pair.key->root(),
                                         frustums, masks );
            }
        }
    }
...
void CDSGraphManager::r_dsgraph_capture_lights() {
    ...
    g_SpatialSpaceLights->q_frustum( lstLights, ISpatial_DB::O_ORDERED,
                                     STYPE_LIGHTSOURCE, i_frustum );

#if RENDER == R_R1
    std::sort(
        lstLights.begin(), lstLights.end(),
        []( const ISpatialShared& _1, const ISpatialShared& _2 ) noexcept {
            if ( !_1.get() || !_2.get() )
                return false;

            return _1->spatial.sphere.P.distance_to_sqr(
                       Device.vCameraPosition_saved ) <
                   _2->spatial.sphere.P.distance_to_sqr(
                       Device.vCameraPosition_saved );
        } );
#endif
        ...
        if ( ( spatial->spatial.type & STYPE_LIGHTSOURCE ) ) {
            ...
            if ( light* L = ( light* )( spatial->dcast_Light() ) ) {
                ...
                if ( L->get_LOD() > ps_r2_shadow_lod_min &&
                     L->has_light_visible_from_sectors( *this ) ) {
...
void CDSGraphManager::r_dsgraph_capture_dynamic( CObject* O ) {
    ...
    if ( i_start ) {
        if ( psDeviceFlags.test( rsDrawDynamic ) ) {
            ...
            g_SpatialSpace->q_frustum( lstRenderables, ISpatial_DB::O_ORDERED,
                                       i_doptions, i_frustum );
            if ( dbg ) {
            ...
            {
                std::sort( lstRenderables.begin(), lstRenderables.end(),
                           []( const ISpatialShared& _1,
                               const ISpatialShared& _2 ) noexcept {
                               if ( !_1.get() || !_2.get() )
                                   return false;
            ...
            if ( i_mask[ CDSGraphManager::fl_normal ] ) // normal phase
            {
                ...
                if ( !lstRenderables.empty() ) {
                    uID_LTRACK =
                        RImplementation.uLastLTRACK % lstRenderables.size();
#if RENDER != R_R1
                    // update light-vis for selected entity
                    // track lighting environment
                    if ( IRenderable* renderable =
                             ( IRenderable* )lstRenderables[ uID_LTRACK ]
                                 ->dcast_Renderable() ) {
                        if ( CROS_impl* T =
                                 ( CROS_impl* )renderable->renderable_ROS() )
                            T->update( renderable );
                    }
#endif
                }
            ...
            for ( u32 o_it = 0; o_it < lstRenderables.size(); o_it++ ) {
                ...
                if ( i_mask[ CDSGraphManager::fl_normal ] &&
                     !RImplementation.HOM.visible( spatial->spatial.sphere ) )
                    continue;
                ...
                //					// lightsource
                //					if (light* L =
                //(light*)(spatial->dcast_Light()))
                //					{
                // #if	RENDER==R_R1
                //						RImplementation.L_DB->add_light(L);
                // #else
                //						if (L->get_LOD()
                //> EPS_L && L->has_light_visible_from_sectors(PT))
                //						{
                //							RImplementation.Lights.add_light(L);
                //						}
                // #endif
                ...
                if ( !( spatial->spatial.type & STYPE_RENDERABLE ) &&
                     ...
                     !( spatial->spatial.type & STYPE_RENDERABLESHADOW ) )
                    continue;
                if ( !is_sector_visible( sector ) )
                    continue;

                for ( CFrustum& frustum :
                      m_sector_frustums.find( sector )->val.first ) {
                    if ( dbg ) {
                    ...
                    if ( frustum.testSphere_dirty(
                        ...
                        if ( O && O->dcast_Renderable() == renderable )
                            break;
                        ...
                        if ( i_mask[ CDSGraphManager::fl_normal ] &&
                             o_it == uID_LTRACK &&
                             renderable->renderable_ROS() ) {
                            // track lighting environment
                            if ( CROS_impl* T = ( CROS_impl* )renderable
                                                    ->renderable_ROS() )
                                T->update( renderable );
                        }
#endif
...
void CDSGraphManager::r_dsgraph_capture( bool lights,
                                         bool dynamic,
                                         CObject* O ) {
