...
void uber_deffer( CBlender_Compile& C,
                  bool hq,
                  LPCSTR _vspec,
                  LPCSTR _pspec,
                  BOOL _aref,
                  LPCSTR _detail_replace,
                  bool DO_NOT_FINISH,
                  bool DO_NOT_WRITE ) {
    // Uber-parse
    ...
    xr_strcpy(
        fname,
        *C.L_textures[ 0 ] ); //. andy if (strext(fname)) *strext(fname)=0;
    ...
    xr_strcpy( dt, sizeof( dt ),
               _detail_replace ? _detail_replace
                               : ( C.detail_texture ? C.detail_texture : "" ) );
    ...
    if ( C.bDetail_Bump ) {
        LPCSTR detail_bump_texture =
            DEV->m_textures_description.GetBumpName( dt ).c_str();
        //	Detect and use detail bump
        if ( detail_bump_texture ) {
            bHasDetailBump = true;
            xr_strcpy( texDetailBump, sizeof( texDetailBump ),
                       detail_bump_texture );
            xr_strcpy( texDetailBumpX, sizeof( texDetailBumpX ),
                       detail_bump_texture );
...
#ifdef USE_DX11
    if ( bump && hq && RImplementation.o.dx11_enable_tessellation &&
         C.TessMethod != 0 ) {
        char hs[ 256 ], ds[ 256 ]; // = "DX11\\tess", ds[256] = "DX11\\tess";
        char params[ 256 ] = "(";

        if ( C.TessMethod == CBlender_Compile::TESS_PN ||
             C.TessMethod == CBlender_Compile::TESS_PN_HM ) {
        ...
        strconcat( sizeof( ps ), ps, "deffer_", _pspec, _aref ? "_aref" : "",
                   "_bump", params );
    ...
    // false,	D3DTADDRESS_WRAP,	D3DTEXF_ANISOTROPIC,D3DTEXF_LINEAR,
    // D3DTEXF_ANISOTROPIC); C.r_Sampler		("s_bumpX",
    // fnameB,				false,	D3DTADDRESS_WRAP,
    // D3DTEXF_ANISOTROPIC,D3DTEXF_LINEAR,	D3DTEXF_ANISOTROPIC);	//
    // should be before base bump C.r_Sampler		("s_bump",
    // fnameA,				false,	D3DTADDRESS_WRAP,
    // D3DTEXF_ANISOTROPIC,D3DTEXF_LINEAR,	D3DTEXF_ANISOTROPIC);
    // C.r_Sampler		("s_bumpD",		dt,
    // false,	D3DTADDRESS_WRAP,	D3DTEXF_ANISOTROPIC,D3DTEXF_LINEAR,
    // D3DTEXF_ANISOTROPIC); C.r_Sampler		("s_detail",	dt,
    // false,	D3DTADDRESS_WRAP,	D3DTEXF_ANISOTROPIC,D3DTEXF_LINEAR,
    // D3DTEXF_ANISOTROPIC);
    ...
    if ( lmap ) {
        // C.r_Sampler("s_hemi",	C.L_textures[2],	false,
        // D3DTADDRESS_CLAMP,	D3DTEXF_LINEAR,		D3DTEXF_NONE,
        // D3DTEXF_LINEAR);
    ...
    C.r_Sampler( "s_bump", fnameA, false, D3DTADDRESS_WRAP, D3DTEXF_ANISOTROPIC,
                 D3DTEXF_LINEAR, D3DTEXF_ANISOTROPIC );
    C.r_Sampler( "s_bumpD", dt, false, D3DTADDRESS_WRAP, D3DTEXF_ANISOTROPIC,
                 D3DTEXF_LINEAR, D3DTEXF_ANISOTROPIC );
    C.r_Sampler( "s_detail", dt, false, D3DTADDRESS_WRAP, D3DTEXF_ANISOTROPIC,
                 D3DTEXF_LINEAR, D3DTEXF_ANISOTROPIC );
    if ( bHasDetailBump ) {
        C.r_Sampler( "s_detailBump", texDetailBump, false, D3DTADDRESS_WRAP,
                     D3DTEXF_ANISOTROPIC, D3DTEXF_LINEAR, D3DTEXF_ANISOTROPIC );
        C.r_Sampler( "s_detailBumpX", texDetailBumpX, false, D3DTADDRESS_WRAP,
                     D3DTEXF_ANISOTROPIC, D3DTEXF_LINEAR, D3DTEXF_ANISOTROPIC );
    }
    if ( lmap )
        C.r_Sampler( "s_hemi", C.L_textures[ 2 ], false, D3DTADDRESS_CLAMP,
                     D3DTEXF_LINEAR, D3DTEXF_NONE, D3DTEXF_LINEAR );
#endif //	USE_DX10
...
void uber_shadow( CBlender_Compile& C, LPCSTR _vspec ) {
    ...
    if ( bump && RImplementation.o.dx11_enable_tessellation &&
         C.TessMethod != 0 ) {
        char hs[ 256 ], ds[ 256 ]; // = "DX11\\tess", ds[256] = "DX11\\tess";
        char params[ 256 ] = "(";
