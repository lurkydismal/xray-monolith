...
bool CGIFAnimationPlayer::Load( const char* fname ) {
    ...
    for ( u32 i = 0; i < imageCount; ++i ) {
        ...
        const HRESULT result =
            HW.pDevice->CreateTexture2D( &desc_10_11, &subRes_10_11, &texture );
        if ( FAILED( result ) ) {
            m_Frames.pop_back();
        ...
        CHK_DX(
            HW.pDevice->CreateShaderResourceView( texture, nullptr, &f.srv ) );
#else
        HRESULT result =
            HW.pDevice->CreateTexture( width, height, 1, 0, D3DFMT_A8R8G8B8,
