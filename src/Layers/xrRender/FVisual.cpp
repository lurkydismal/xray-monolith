...
void Fvisual::Load( const char* N, IReader* data, u32 dwFlags ) {
    ...
    if ( data->find_chunk( OGF_GCONTAINER ) ) {
        ...
        if ( data->find_chunk( OGF_FASTPATH ) ) {
            ...
            if ( g_defer_visual_shader_creation )
                ...
                m_fast->rm_geom.create( fmt, m_fast->p_rm_Vertices,
                                        m_fast->p_rm_Indices );
        }
    ...
    if ( !loaded_v && ( dwFlags & VLOAD_NOVERTICES ) == 0 ) {
        if ( data->find_chunk( OGF_VCONTAINER ) ) {
            ...
            R_CHK( dx10BufferUtils::CreateVertexBuffer(
                &p_rm_Vertices, data->pointer(), vCount * vStride ) );
            HW.stats_manager.increment_stats_vb( p_rm_Vertices );
#else  //	USE_DX10
            BOOL bSoft = HW.Caps.geometry.bSoftware;
            u32 dwUsage = D3DUSAGE_WRITEONLY |
                          ( bSoft ? D3DUSAGE_SOFTWAREPROCESSING : 0 );
            BYTE* bytes = 0;
            VERIFY( NULL == p_rm_Vertices );
            R_CHK( HW.pDevice->CreateVertexBuffer( vCount * vStride, dwUsage, 0,
                                                   D3DPOOL_MANAGED,
                                                   &p_rm_Vertices, 0 ) );
            HW.stats_manager.increment_stats_vb( p_rm_Vertices );
    ...
    if ( !loaded_v ) {
        ...
        if ( data->find_chunk( OGF_ICONTAINER ) ) {
...
#if defined( USE_DX10 ) || defined( USE_DX11 )
            // BOOL	bSoft		= HW.Caps.geometry.bSoftware ||
            // (dwFlags&VLOAD_FORCESOFTWARE); u32		dwUsage =
            // /*D3DUSAGE_WRITEONLY |*/ (bSoft?D3DUSAGE_SOFTWAREPROCESSING:0);
            // // indices are read in model-wallmarks code BYTE*	bytes
            // = 0;
            ...
            // R_CHK
            // (p_rm_Indices->Lock(0,0,(void**)&bytes,0)); CopyMemory
            // (bytes, data->pointer(), iCount*2);

            VERIFY( NULL == p_rm_Indices );
            R_CHK( dx10BufferUtils::CreateIndexBuffer(
                &p_rm_Indices, data->pointer(), iCount * 2 ) );
            ...
            R_CHK( HW.pDevice->CreateIndexBuffer(
                iCount * 2, dwUsage, D3DFMT_INDEX16, D3DPOOL_MANAGED,
                &p_rm_Indices, 0 ) );
            HW.stats_manager.increment_stats_ib( p_rm_Indices );
...
void Fvisual::Render( float ) {
    ...
    if ( m_fast && RImplementation.phase == CRender::PHASE_SMAP &&
        ...
        RCache.Render( D3DPT_TRIANGLELIST, m_fast->vBase, 0, m_fast->vCount,
                       m_fast->iBase, m_fast->dwPrimitives );
        RCache.stat.r.s_static.add( m_fast->vCount );
    } else {
        RCache.set_Geometry( rm_geom );
        RCache.Render( D3DPT_TRIANGLELIST, vBase, 0, vCount, iBase,
                       dwPrimitives );
