...
void CRender::render_lights( light_Package& LP ) {
    ...
    stats.ls_shadowed_peak_in =
        _max( stats.ls_shadowed_peak_in, ( u32 )LP.v_shadowed.size() );
    //////////////////////////////////////////////////////////////////////////
...

    {
...
#endif
        {
                    ...
                    []( light* L ) {
                        if ( L->m_parent ) {
                            if ( ps_r2_shadow_omnipart_vischeck ) {
                                ...
                                for ( int f = 0; f < 6; f++ ) {
                                    L->m_parent->omnipart[ f ]->vis.pending =
                                        L->m_parent->vis.pending;
                                    L->m_parent->omnipart[ f ]->vis.visible =
                                        L->m_parent->vis.visible;
                                }
                            }
                        } else
                            L->vis_update();
                        if ( !L->vis.visible ) {
                            RImplementation.stats
                                .ls_shadowed_invisible_skipped++;
                            return true;
                        }
            ...
            stats.ls_shadowed_peak_after_vis =
                _max( stats.ls_shadowed_peak_after_vis, ( u32 )source.size() );
        }

        {
            ...
            for ( u16 smap_ID = 0; refactored.size() != total; smap_ID++ ) {
                ...
                std::sort( source.begin(), source.end(),
                           []( light* _1, light* _2 ) {
                               return _1->X.S.size > _2->X.S.size;
                           } );
                source.erase(
                    std::remove_if( source.begin(), source.end(),
                                    [ smap_ID ]( light* L ) {
                                        SMAP_Rect R;
                                        if ( RImplementation.LP_smap_pool.push(
                                                 R, L->X.S.size ) ) {
                                            L->X.S.posX = R.min.x;
                                            L->X.S.posY = R.min.y;
        ...
        while ( !LP.v_shadowed.empty() ) {
            ...
            static xr_vector< light* > L_spot_s;
            {
                ...
                while ( !source.empty() ) {
                    ...
                    {
                        if ( ( L->decor_object[ 0 ] &&
                               !L->decor_object[ 0 ]->getDestroy() ) ||
                             ( L->decor_object[ 1 ] &&
                               !L->decor_object[ 1 ]->getDestroy() ) ||
                             ( L->decor_object[ 2 ] &&
                               !L->decor_object[ 2 ]->getDestroy() ) ||
                             ( L->decor_object[ 3 ] &&
                               !L->decor_object[ 3 ]->getDestroy() ) ||
                             ( L->decor_object[ 4 ] &&
                               !L->decor_object[ 4 ]->getDestroy() ) ||
                             ( L->decor_object[ 5 ] &&
                               !L->decor_object[ 5 ]->getDestroy() ) ) {
                            for ( int f = 0; f < 6; f++ ) {
                                if ( L->decor_object[ f ] &&
                                     !L->decor_object[ f ]->getDestroy() ) {
                            ...
                            if ( L->m_moving_frames < 32u ) {
                                ...
                                L->GMLight.traverse(
                                    ...
                                    L->X.S.frustum, L->position,
                                    L->X.S.combine );
                                L->GMLight.r_dsgraph_capture_static();
                                L->m_moving_frames++;
                            }
                            L->GMLight.r_dsgraph_capture_dynamic(
                                L->ignore_object );
                        }
                    }

                    bool bDeffered_Shadows =
                        L->GMLight.RGraph.mapStaticPasses[ 0 ][ 0 ].size() ||
                        L->GMLight.RGraph.mapDynamicPasses[ 0 ][ 0 ].size();
                    bool bForward_Shadows =
                        L->GMLight.RGraph.mapStaticPasses[ 1 ][ 0 ].size() ||
                        L->GMLight.RGraph.mapDynamicPasses[ 1 ][ 0 ].size() ||
                        L->GMLight.RGraph.mapStaticSorted.Sorted.size() ||
                        L->GMLight.RGraph.mapDynamicSorted.Sorted.size();
                    if ( bDeffered_Shadows || bForward_Shadows ) {
                        ...
                        if ( Details && Details->dtFS &&
                             check_grass_shadow( L, ViewBase ) &&
                             L->flags.bShadow && !decorative_light ) {
                            Details->fade_distance =
                                -1; // Use light position to calc "fade"
            ...
            if ( !L_spot_s.empty() ) {
                ...
                for ( light* L : L_spot_s ) {
                    Target->accum_spot( L );
                    render_indirect( L );
                    if ( L->flags.bVolumetric && RImplementation.o.advancedpp &&
                        ...
                        if ( RImplementation.o.ssfx_volumetric )
                            Target->set_viewport_size(
                                HW.pContext, w / RImplementation.o.volsize,
