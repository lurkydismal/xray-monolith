...
void dxDebugRender::Render() {
    ...
    if ( !m_line_vertices_hud.empty() ) {
        ...
        for ( auto& m_vert : m_line_vertices_hud ) {
            ...
            RCache.set_c( "tfactor", float( color_get_R( color ) ) / 255.f,
                          float( color_get_G( color ) ) / 255.f,
                          float( color_get_B( color ) ) / 255.f,
                          float( color_get_A( color ) ) / 255.f );
#endif
            RCache.dbg_Draw( D3DPT_LINELIST, &vert_vec.front(), vert_vec.size(),
                             &ind_vec.front(), ind_vec.size() / 2 );
        }
...
void dxDebugRender::add_lines( Fvector const* vertices,
    ...
    if ( ( ( all_verts_count + vertex_count ) >= u16( -1 ) ) ||
         ( ( all_inds_count + 2 * pair_count ) >= u16( -1 ) ) )
        Render();

    auto& vert_vec =
        bHud ? m_line_vertices_hud[ color ] : m_line_vertices[ color ];
    auto& ind_vec =
        bHud ? m_line_indices_hud[ color ] : m_line_indices[ color ];
...
void dxDebugRender::SetDebugShader( dbgShaderHandle shdHandle ) {
    ...
    if ( !m_dbgShaders[ shdHandle ] )
        m_dbgShaders[ shdHandle ].create( dbgShaderParams[ shdHandle ][ 0 ],
                                          dbgShaderParams[ shdHandle ][ 1 ] );
...
void dxDebugRender::dbg_DrawTRI( Fmatrix& T,
                                 Fvector& p1,
                                 Fvector& p2,
                                 Fvector& p3,
                                 u32 C ) {
