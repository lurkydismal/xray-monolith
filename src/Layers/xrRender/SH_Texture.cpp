...
bool CTexture::CanLoadAsync() const {
    ...
    if ( !kind ) {
        ...
        kind = FS.exist( path, "$game_textures$", *cName, ".ogm" ) ||
                       ...
                       FS.exist( path, "$game_textures$", *cName, ".gif" )
                   ? 2u
                   : 1u;
...
#endif
    {
        ...
        if ( FS.exist( fn, "$game_textures$", *cName, ".ogm" ) ) {
            ...
            if ( !pTheora->Load( fn ) ) {
                ...
                BOOL bstop_at_end =
                    ( 0 != strstr( cName.c_str(), "intro\\" ) ) ||
                    ( 0 != strstr( cName.c_str(), "outro\\" ) );
                ...
                HRESULT hrr = HW.pDevice->CreateTexture(
                    _w, _h, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pTexture,
                    NULL );
            ...
            if ( !pAVI->Load( fn ) ) {
                ...
                ID3DTexture2D* pTexture = 0;
                HRESULT hrr = HW.pDevice->CreateTexture(
                    pAVI->m_dwWidth, pAVI->m_dwHeight, 1, 0, D3DFMT_A8R8G8B8,
