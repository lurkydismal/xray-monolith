...
SVS* CResourceManager::_CreateVS( LPCSTR _name, ref_vs* keep_alive ) {
    ...
    if ( I != m_vs.end() )
        ...
        string_path cname;
        strconcat( sizeof( cname ), cname, ::Render->getShaderPath(), _name,
                   ".vs" );
...
SVS* CResourceManager::_CreateVS( LPCSTR _name, ref_vs* keep_alive ) {
    ...
    if ( I != m_vs.end() )
        ...
        LPCSTR c_target = "vs_2_0";
        LPCSTR c_entry = "main";
        /*if (HW.Caps.geometry.dwVersion>=CAP_VERSION(3,0))
        target="vs_3_0"; else*/
        if ( HW.Caps.geometry_major >= 2 )
            c_target = "vs_2_0";
        else
        ...
        // vertex
        R_ASSERT2( fs, cname );
        _hr = ::Render->shader_compile(
            name, LPCSTR( fs->pointer() ), fs->length(), NULL, &Includer,
            c_entry, c_target,
            D3DXSHADER_DEBUG |
        ...
        //		_hr = D3DXCompileShader
        //(LPCSTR(fs->pointer()),fs->length(), NULL, &Includer, "main", target,
        //D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR, &pShaderBuf,
        //&pErrorBuf, NULL);
        FS.r_close( fs );

        if ( SUCCEEDED( _hr ) ) {
            if ( pShaderBuf ) {
                _hr = HW.pDevice->CreateVertexShader(
                    ( DWORD* )pShaderBuf->GetBufferPointer(), &_vs->vs );
                if ( SUCCEEDED( _hr ) ) {
                    LPCVOID data = NULL;
                    _hr = D3DXFindShaderComment(
                        ( DWORD* )pShaderBuf->GetBufferPointer(),
                        MAKEFOURCC( 'C', 'T', 'A', 'B' ), &data, NULL );
...
SPS* CResourceManager::_CreatePS( LPCSTR name, ref_ps* keep_alive ) {
    ...
    if ( I != m_ps.end() )
        ...
        HRESULT _hr = S_OK;
        _hr = ::Render->shader_compile(
            name, data, size, NULL, &Includer, c_entry, c_target,
            D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR, &pShaderBuf,
            &pErrorBuf, NULL );
        //_hr = D3DXCompileShader		(text,text_size, NULL,
        //&Includer, c_entry, c_target, D3DXSHADER_DEBUG |
        //D3DXSHADER_PACKMATRIX_ROWMAJOR, &pShaderBuf, &pErrorBuf, NULL);
        xr_free( data );

        if ( SUCCEEDED( _hr ) ) {
            if ( pShaderBuf ) {
                _hr = HW.pDevice->CreatePixelShader(
                    ( DWORD* )pShaderBuf->GetBufferPointer(), &_ps->ps );
                if ( SUCCEEDED( _hr ) ) {
                    LPCVOID data = NULL;
                    _hr = D3DXFindShaderComment(
                        ( DWORD* )pShaderBuf->GetBufferPointer(),
