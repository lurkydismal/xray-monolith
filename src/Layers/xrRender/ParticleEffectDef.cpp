...
void CPEDef::pCollision(float friction, float resilience, float cutoff, BOOL
destroy_on_contact)
{
        ...
        m_Flags.set
(dfCollisionDel,destroy_on_contact);
}
...
void CPEDef::pFrame(BOOL random_frame, u32 frame_count, u32 tex_width, u32
tex_height, u32 frame_width, u32 frame_height)
{
        m_Flags.set			(dfFramed,TRUE);
        m_Flags.set			(dfRandomFrame,random_frame);
        m_Frame.Set			(frame_count, (float)tex_width,
(float)tex_height, (float)frame_width, (float)frame_height);
}
...
void CPEDef::ExecuteAnimate( Particle* particles, u32 p_cnt, float dt ) {
    ...
    for ( u32 i = 0; i < p_cnt; i++ ) {
        ...
        float f = ( float( m.frame ) / 255.f +
                    ( ( m.flags.is( Particle::ANIMATE_CCW ) ) ? -1.f : 1.f ) *
                        speedFac );
...
void CPEDef::ExecuteCollision( PAPI::Particle* particles,
                               u32 p_cnt,
                               float dt,
    ...
    for ( int i = p_cnt - 1; i >= 0; i-- ) {
        ...
        do {
            ...
            if ( dist >= EPS ) {
                ...
                collide::rq_target RT = m_Flags.is( dfCollisionDyn )
                                            ? collide::rqtBoth
                                            : collide::rqtStatic;
                if ( g_pGameLevel->ObjectSpace.RayPick( m.posB, dir, dist, RT,
                                                        RQ, NULL ) ) {
                    ...
                    if ( RQ.O ) {
                        ...
                        CDB::TRI* T =
                            g_pGameLevel->ObjectSpace.GetStaticTris() +
                            RQ.element;
                        Fvector* verts =
                            g_pGameLevel->ObjectSpace.GetStaticVerts();
                        n.mknormal( verts[ T->verts[ 0 ] ],
                                    verts[ T->verts[ 1 ] ],
                                    verts[ T->verts[ 2 ] ] );
                    }
#endif
                    ...
                    if ( m_Flags.is( dfCollisionDel ) ) {
                        ...
                        if ( vt.length2() <= m_fCollideSqrCutoff ) {
                            ...
                            m.vel = vt * m_fCollideOneMinusFriction -
                                    vn * m_fCollideResilience;
                        }
...
BOOL CPEDef::Load( IReader& F ) {
    ...
    if ( pCreateEAction && F.find_chunk( PED_CHUNK_EDATA ) ) {
        ...
        bool valid = false;
        for ( EPAVecIt it = m_EActionList.begin(); it != m_EActionList.end();
              ++it ) {
...
BOOL CPEDef::Load2( CInifile& ini ) {
    ...
    if ( m_Flags.is( dfCollision ) ) {
        m_fCollideOneMinusFriction =
            ini.r_float( "collision", "one_minus_friction" );
    ...
    if ( m_Flags.is( dfAlignToPath ) ) {
        m_APDefaultRotation =
            ini.r_fvector3( "align_to_path", "default_rotation" );
    }
    // #ifdef _EDITOR
    if ( pCreateEAction ) {
        ...
        for ( EPAVecIt it = m_EActionList.begin(); it != m_EActionList.end();
            ...
            PAPI::PActionEnum type =
                ( PAPI::PActionEnum )( ini.r_u32( sect, "action_type" ) );
            ( *it ) = pCreateEAction( type );
            ( *it )->Load2( ini, sect );
        }
        Compile( m_EActionList );
...
void CPEDef::Save2( CInifile& ini ) {
    ...
    if ( m_Flags.is( dfCollision ) ) {
        ini.w_float( "collision", "one_minus_friction",
                     m_fCollideOneMinusFriction );
    ...
    if ( m_Flags.is( dfAlignToPath ) ) {
        ini.w_fvector3( "align_to_path", "default_rotation",
                        m_APDefaultRotation );
    }
    // #ifdef _EDITOR
