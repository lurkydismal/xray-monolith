...
void CBlender_Model_EbB::Compile( CBlender_Compile& C ) {
    ...
    if ( C.bEditor ) {
        C.PassBegin();
        {
            ...
            // Stage2 - Base texture
            C.StageBegin();
            C.StageSET_Color( D3DTA_TEXTURE, D3DTOP_BLENDTEXTUREALPHA,
                              D3DTA_CURRENT );
        ...
        switch ( C.iElement ) {
            case SE_R1_NORMAL_HQ:
                ...
                if ( oBlend.value )
                    C.r_Pass( vsname, psname, TRUE, TRUE, FALSE, TRUE,
            ...
            case SE_R1_LPOINT:
                ...
                psname = "add_point";
                C.r_Pass( vsname, psname, FALSE, TRUE, FALSE, TRUE,
            ...
            case SE_R1_LMODELS:
                ...
                psname = "model_shadow";
                C.r_Pass( vsname, psname, FALSE, FALSE, FALSE, TRUE,
                          D3DBLEND_ZERO, D3DBLEND_SRCCOLOR, FALSE, 0 );
                C.r_End();
                break;
        }
    }
        ...
        switch ( C.iElement ) {
            ...
            case SE_R2_SHADOW: // smap
                if ( RImplementation.o.HW_smap )
                    C.r_Pass( "shadow_direct_model", "dumb", FALSE, TRUE, TRUE,
                              FALSE );
                else
                    C.r_Pass( "shadow_direct_model", "shadow_direct_base",
                              FALSE );
                C.r_Sampler( "s_base", C.L_textures[ 0 ] );
                C.r_End();
                break;
    ...
    if ( oBlend.value ) {
        ...
        switch ( C.iElement ) {
            ...
            case 1:
                ...
                C.r_Pass( vsname, psname, TRUE, TRUE, FALSE, TRUE,
                          D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA, TRUE, 0 );
                // C.r_Sampler			("s_base",
                // C.L_textures[0]); C.r_Sampler ("s_env",
                // oT2_Name,false,D3DTADDRESS_CLAMP);
                ...
                if ( RImplementation.o.ssfx_glass ) {
                    ...
                    C.r_dx10Texture( "s_screen",
                                     "$user$generic_temp" ); // r2_RT_ssfx_temp3
        ...
        switch ( C.iElement ) {
            case SE_R2_NORMAL_HQ: // deffer
                ...
                if ( C.HudElement ) {
                    uber_deffer( C, true, "model_hud", "base_hud", false, 0,
                                 true );
            ...
            case SE_R2_SHADOW: // smap
                ...
                // ("shadow_direct_model","dumb",	FALSE,TRUE,TRUE,FALSE);
                // else							C.r_Pass
                // ("shadow_direct_model","shadow_direct_base",FALSE);
                C.r_Pass( "shadow_direct_model", "dumb", FALSE, TRUE, TRUE,
                          FALSE );
                // C.r_Sampler		("s_base",C.L_textures[0]);
                C.r_dx10Texture( "s_base", C.L_textures[ 0 ] );
