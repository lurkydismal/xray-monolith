...
template < typename T, bool Reverse >
using mapDSGraphItems = xr_vector< DSGraphItem< T, Reverse >,
                                   typename render_allocator::template helper<
                                       DSGraphItem< T, Reverse > >::result >;

template < typename T, bool Reverse >
using mapDSGraphItemsMap =
    FixedMAP< T, DSGraphItem< T, Reverse >, render_allocator >;
...
struct RenderPacket {
    ...
    RenderPacket( const DSGraphItem< u32, false >& _item, const SPass& pass )
        : item( _item ) {
        // Extract resource pointers from shader pass (previously used as map
        // keys)
#if defined( USE_DX10 ) || defined( USE_DX11 )
        pVS = &*pass.vs;
        pGS = pass.gs->gs;
        ...
        // States:4, GS:0, HS:0, DS:0 they are pretty much unused and/or
        // unchanged VS:13, PS:28, CS:93, Tex:179. Grouping based on increasing
        // change of state Low key is used just for sorting
        u64 keyHigh = 0;
        u64 keyLow = 0;
...
using RenderQueue =
    xr_vector< RenderPacket, render_allocator::helper< RenderPacket >::result >;
using RenderQueueArray =
    xr_array< xr_array< RenderQueue, SHADER_PASSES_MAX >, 2 >;
