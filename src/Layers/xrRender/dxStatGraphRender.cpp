...
void dxStatGraphRender::OnDeviceCreate() {
    hGeomLine.create( FVF::F_TL0uv, RCache.Vertex.Buffer(),
                      RCache.Index.Buffer() );
...
void dxStatGraphRender::OnRender( CStatGraph& owner ) {
    ...
    u32 TriElem = 0;
    u32 LineElem = 0;
    for ( CStatGraph::SubGraphVecIt it = owner.subgraphs.begin();
          it != owner.subgraphs.end(); it++ ) {
        switch ( it->style ) {
    ...
    if ( TriElem ) {
        pv_Tri_start = ( FVF::TL0uv* )RCache.Vertex.Lock(
            TriElem, hGeomTri->vb_stride, dwOffsetTri );
        pv_Tri = pv_Tri_start;
        ...
        RCache.Render( D3DPT_TRIANGLELIST, dwOffsetTri, 0, dwCount, 0,
                       dwCount / 2 );
    };

    if ( LineElem ) {
        pv_Line_start = ( FVF::TL0uv* )RCache.Vertex.Lock(
            LineElem, hGeomLine->vb_stride, dwOffsetLine );
...
void dxStatGraphRender::RenderBack( CStatGraph& owner ) {
    ...
    FVF::TL0uv* pv_start =
        ( FVF::TL0uv* )RCache.Vertex.Lock( 4, hGeomTri->vb_stride, dwOffset );
    ...
    // draw rect
    pv_start =
        ( FVF::TL0uv* )RCache.Vertex.Lock( 5, hGeomLine->vb_stride, dwOffset );
    pv = pv_start;
    // base rect
    ...
    // draw owner.grid
    float elem_factor =
        float( owner.rb.y - owner.lt.y ) / float( owner.mx - owner.mn );
    float base_y = float( owner.rb.y ) + ( owner.mn * elem_factor );

    int PNum_H_LinesUp = int( ( base_y - float( owner.lt.y ) ) /
                              ( owner.grid_step.y * elem_factor ) );
    int PNum_H_LinesDwn = u32( ( float( owner.rb.y ) - base_y ) /
                               ( owner.grid_step.y * elem_factor ) );
    int Num_H_LinesUp =
        ( owner.grid.y < PNum_H_LinesUp ) ? owner.grid.y : PNum_H_LinesUp;
    int Num_H_LinesDwn =
        ( owner.grid.y < PNum_H_LinesUp ) ? owner.grid.y : PNum_H_LinesDwn;

    pv_start = ( FVF::TL0uv* )RCache.Vertex.Lock(
        2 + 2 * owner.grid.x + Num_H_LinesUp * 2 + Num_H_LinesDwn * 2,
    ...
    // owner.grid
    //    float grid_offs_x		=
    //    float(owner.rb.x-owner.lt.x)/float(owner.grid.x+1); float grid_offs_y
    //    = float(owner.rb.y-owner.lt.y)/float(owner.grid.y+1);
    for ( int g_x = 1; g_x <= owner.grid.x; g_x++ ) {
        pv->set( int( owner.lt.x + g_x * owner.grid_step.x * elem_factor ),
                 owner.lt.y, owner.grid_color );
        pv++;
        pv->set( int( owner.lt.x + g_x * owner.grid_step.x * elem_factor ),
                 owner.rb.y, owner.grid_color );
        pv++;
    }
    for ( int g_y = 1; g_y <= Num_H_LinesDwn; g_y++ ) {
        pv->set( owner.lt.x,
                 int( base_y + g_y * owner.grid_step.y * elem_factor ),
                 owner.grid_color );
        pv++;
        pv->set( owner.rb.x,
                 int( base_y + g_y * owner.grid_step.y * elem_factor ),
    ...
    //	    pv->set
    //(owner.lt.x,iFloor(g_y*grid_offs_y+owner.lt.y),owner.grid_color); pv++;
    //	    pv->set
    //(owner.rb.x,iFloor(g_y*grid_offs_y+owner.lt.y),owner.grid_color); pv++;
    //	}
...
void dxStatGraphRender::RenderBars( CStatGraph& owner,
                                    FVF::TL0uv** ppv,
                                    CStatGraph::ElementsDeq* pelements ) {
    ...
    if ( column_width > 1 )
        column_width--;
    for ( CStatGraph::ElementsDeqIt it = pelements->begin();
          it != pelements->end(); it++ ) {
...
void dxStatGraphRender::RenderLines( CStatGraph& owner,
                                     FVF::TL0uv** ppv,
                                     CStatGraph::ElementsDeq* pelements ) {
    float elem_offs = float( owner.rb.x - owner.lt.x ) / owner.max_item_count;
    float elem_factor =
    ...
    for ( CStatGraph::ElementsDeqIt it = pelements->begin() + 1;
          it != pelements->end() && it != pelements->end() + 1; it++ ) {
        CStatGraph::ElementsDeqIt it_prev = it - 1;
        float X0 =
            float( it_prev - pelements->begin() ) * elem_offs + owner.lt.x;
...
void dxStatGraphRender::RenderBarLines( CStatGraph& owner,
                                        FVF::TL0uv** ppv,
                                        CStatGraph::ElementsDeq* pelements ) {
    float elem_offs = float( owner.rb.x - owner.lt.x ) / owner.max_item_count;
    float elem_factor =
    ...
    for ( CStatGraph::ElementsDeqIt it = pelements->begin() + 1;
          it != pelements->end() && it != pelements->end() + 1; it++ ) {
        CStatGraph::ElementsDeqIt it_prev = it - 1;
        float X0 = float( it_prev - pelements->begin() ) * elem_offs +
                   owner.lt.x + elem_offs;
        float Y0 = base_y - it_prev->data * elem_factor;
        ( *ppv )->set( X0, Y0, it->color );
        ( *ppv )++;
        float X1 = float( it - pelements->begin() ) * elem_offs + owner.lt.x;
...
void dxStatGraphRender::RenderMarkers( CStatGraph& owner,
                                       FVF::TL0uv** ppv,
                                       CStatGraph::MarkersDeq* pmarkers ) {
    float elem_offs = float( owner.rb.x - owner.lt.x ) / owner.max_item_count;
    float elem_factor =
        float( owner.rb.y - owner.lt.y ) / float( owner.mx - owner.mn );
    float base_y = float( owner.rb.y ) + ( owner.mn * elem_factor );

    for ( CStatGraph::MarkersDeqIt it = pmarkers->begin();
          it != pmarkers->end() && it != pmarkers->end() + 1; it++ ) {
