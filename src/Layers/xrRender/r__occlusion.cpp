...
void R_occlusion::occq_stats() {
    if ( occq_debug )
        Msg( "R_occlusion::occq_stats: pool: %d fids: %d used: %d", pool.size(),
             fids.size(), used.size() );
}
...
u32 R_occlusion::occq_begin( u32& ID ) {
    ...
    if ( pool.empty() ) {
        if ( occq_debug && Device.dwFrame % 40 == 0 )
            Msg( " RENDER [Warning]: Too many occlusion queries were "
                 "issued(>1536)!!!" );
        ID = iInvalidHandle;
    ...
    pool.pop_back();
    // CHK_DX					(used[ID].Q->Issue
    // (D3DISSUE_BEGIN));
    CHK_DX( BeginQuery( used[ ID ].Q ) );

    // Msg				("begin: [%2d] - %d", used[ID].order,
    // ID);
...
R_occlusion::occq_result R_occlusion::occq_get( u32& ID ) {
    ...
    HRESULT hr;
    // CHK_DX
    // (used[ID].Q->GetData(&fragments,sizeof(fragments),D3DGETDATA_FLUSH)); Msg
    // ("get  : [%2d] - %d => %d", used[ID].order, ID, fragments);
    CTimer T;
    T.Start();
    Device.Statistic->RenderDUMP_Wait.Begin();
    // while
    // ((hr=used[ID].Q->GetData(&fragments,sizeof(fragments),D3DGETDATA_FLUSH))==S_FALSE)
    // {
    VERIFY2( ID < used.size(),
             make_string( "_Pos = %d, size() = %d ", ID, used.size() ) );
    while ( ( hr = GetData( used[ ID ].Q, &fragments, sizeof( fragments ) ) ) ==
            S_FALSE ) {
