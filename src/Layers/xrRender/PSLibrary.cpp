...
#else
    PS::PEDIt I =
        std::lower_bound( m_PEDs.begin(), m_PEDs.end(), Name, ped_find_pred );
...
#else
    PS::PGDIt I =
        std::lower_bound( m_PGDs.begin(), m_PGDs.end(), Name, pgd_find_pred );
...
bool CPSLibrary::LoadDefinitions( const char* nm ) {
    ...
    xr_parallel_for(
        0u, static_cast< u32 >( prepared.size() ), [ & ]( u32 index ) {
            ...
            if ( result.effect ) {
                xr_unique_ptr< PS::CPEDef > definition =
                    xr_make_unique< PS::CPEDef >();
                definition->m_Name = result.name;
                if ( definition->Load2( ini ) )
                    result.effectDefinition = std::move( definition );
            } else {
                xr_unique_ptr< PS::CPGDef > definition =
                    xr_make_unique< PS::CPGDef >();
                definition->m_Name = result.name;
                if ( definition->Load2( ini ) )
    ...
    std::sort( m_PEDs.begin(), m_PEDs.end(), ped_sort_pred );
    std::sort( m_PGDs.begin(), m_PGDs.end(), pgd_sort_pred );
    m_prepare_sort_ms = startupTimer.GetElapsed_ms() - m_prepare_loose_ms -
                        m_prepare_library_ms;
...
void CPSLibrary::FinalizeLoad() {
    ...
    for ( PS::PEDIt e_it = m_PEDs.begin(); e_it != m_PEDs.end(); ++e_it )
        ( *e_it )->CreateShader();
    Msg( "* [STARTUP/RENDER PARTICLES] loose=%u library=%u sort=%u shaders=%u "
         "effects=%u groups=%u total=%u ms",
         m_prepare_loose_ms, m_prepare_library_ms, m_prepare_sort_ms,
         shaderTimer.GetElapsed_ms(), static_cast< u32 >( m_PEDs.size() ),
         static_cast< u32 >( m_PGDs.size() ),
         m_prepare_loose_ms + m_prepare_library_ms + m_prepare_sort_ms +
             shaderTimer.GetElapsed_ms() );
}
...
void CPSLibrary::particles_group_next(
...
shared_str const& CPSLibrary::particles_group_id(
    CPGDef const& particles_group ) const {
...
bool CPSLibrary::Save( const char* nm ) {
    ...
    u32 chunk_id = 0;
    for ( PS::PEDIt it = m_PEDs.begin(); it != m_PEDs.end();
          ++it, ++chunk_id ) {
    ...
    chunk_id = 0;
    for ( PS::PGDIt g_it = m_PGDs.begin(); g_it != m_PGDs.end();
          ++g_it, ++chunk_id ) {
        F.open_chunk( chunk_id );
        ( *g_it )->Save( F );
        F.close_chunk();
