...
void stats_manager::increment_stats( u32 size,
...
void stats_manager::increment_stats_vb( ID3DVertexBuffer* buff ) {
    ...
    buff->GetDesc( &desc );
    increment_stats( desc.ByteWidth, enum_stats_buffer_type_vertex,
                     D3DPOOL_MANAGED, buff );
#else
    D3DVERTEXBUFFER_DESC desc;
    buff->GetDesc( &desc );
    increment_stats( desc.Size, enum_stats_buffer_type_vertex, desc.Pool,
                     buff );
#endif
}
...
void stats_manager::increment_stats_ib( ID3DIndexBuffer* buff ) {
    ...
    D3D_BUFFER_DESC desc;
    buff->GetDesc( &desc );
    increment_stats( desc.ByteWidth, enum_stats_buffer_type_index,
                     D3DPOOL_MANAGED, buff );
#else
...
void stats_manager::decrement_stats_vb( ID3DVertexBuffer* buff ) {
    ...
    D3D_BUFFER_DESC desc;
    buff->GetDesc( &desc );
    decrement_stats( desc.ByteWidth, enum_stats_buffer_type_vertex,
                     D3DPOOL_MANAGED, buff );
#else
    D3DVERTEXBUFFER_DESC desc;
    buff->GetDesc( &desc );
    decrement_stats( desc.Size, enum_stats_buffer_type_vertex, desc.Pool,
                     buff );
#endif
}
...
void stats_manager::decrement_stats_ib( ID3DIndexBuffer* buff ) {
    ...
    D3D_BUFFER_DESC desc;
    buff->GetDesc( &desc );
    decrement_stats( desc.ByteWidth, enum_stats_buffer_type_index,
                     D3DPOOL_MANAGED, buff );
#else
    D3DINDEXBUFFER_DESC desc;
    buff->GetDesc( &desc );
...
void stats_manager::decrement_stats( u32 size,
                                     enum_stats_buffer_type type,
                                     _D3DPOOL location ) {
    if ( g_dedicated_server )
        return;
...
void stats_manager::decrement_stats( u32 size,
    ...
    for ( ; it != en; ++it ) {
        if ( it->buff_ptr == buff_ptr ) {
            // The pointers may coincide so this assertion may some times fail
            // normally.
            // R_ASSERT ( it->type == type && it->location == location &&
            // it->size == size );
...
u32 get_format_pixel_size( DXGI_FORMAT format ) {
    if ( format >= DXGI_FORMAT_R32G32B32A32_TYPELESS &&
        ...
        return 16;
    else if ( format >= DXGI_FORMAT_R32G32B32_TYPELESS &&
              format <= DXGI_FORMAT_R32G32B32_SINT )
        return 12;
    else if ( format >= DXGI_FORMAT_R16G16B16A16_TYPELESS &&
              format <= DXGI_FORMAT_X32_TYPELESS_G8X24_UINT )
        return 8;
    else if ( format >= DXGI_FORMAT_R10G10B10A2_TYPELESS &&
              format <= DXGI_FORMAT_X24_TYPELESS_G8_UINT )
        return 4;
    else if ( format >= DXGI_FORMAT_R8G8_TYPELESS &&
              format <= DXGI_FORMAT_R16_SINT )
        return 2;
    else if ( format >= DXGI_FORMAT_R8_TYPELESS &&
