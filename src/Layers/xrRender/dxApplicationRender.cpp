...
void dxApplicationRender::LoadBegin() {
    ...
    hLevelLogo_Add.create( "hud\\default",
                           "ui\\ui_actor_widescreen_sidepanels.dds" );
...
void dxApplicationRender::load_draw_internal( CApplication& owner ) {
    ...
    FVF::TL* pv = NULL;
    u32 v_cnt = 40;
    pv = ( FVF::TL* )RCache.Vertex.Lock( 2 * ( v_cnt + 1 ), ll_hGeom2.stride(),
                                         Offset );
    FVF::TL* _pv = pv;
    ...
    for ( u32 idx = 0; idx < v_cnt + 1; ++idx ) {
        clr = calc_progress_color( idx, v_cnt, owner.load_stage,
                                   owner.max_load_stage );
        pv->set( back_coords.lt.x + pos_delta * idx + offs,
                 back_coords.rb.y + offs, 0 + EPS_S, 1, clr,
                 back_tex_coords.lt.x + tc_delta * idx, back_tex_coords.rb.y );
        pv++;
        pv->set( back_coords.lt.x + pos_delta * idx + offs,
                 back_coords.lt.y + offs, 0 + EPS_S, 1, clr,
...
void dxApplicationRender::draw_face( ref_shader& sh,
                                     Frect& coords,
                                     Frect& tex_coords,
                                     const Fvector2& tsz ) {
    u32 Offset;
...
void draw_multiline_text( CGameFont* F, float fTargetWidth, LPCSTR pszText ) {
    ...
    while ( *ch ) {
        ...
        if ( curr_len + curr_word_len > fTargetWidth ) {
            ...
            strncpy_s( buff + xr_strlen( buff ),
                       sizeof( buff ) - xr_strlen( buff ), ch, next_word - ch );
            ch = next_word;
        }
