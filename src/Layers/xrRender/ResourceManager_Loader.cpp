...
void CResourceManager::OnDeviceDestroy( BOOL ) {
    ...
    // Matrices
    for ( map_Matrix::iterator m = m_matrices.begin(); m != m_matrices.end();
          m++ ) {
        R_ASSERT( 1 ==
                  m->second->dwReference.load( std::memory_order_relaxed ) );
        xr_delete( m->second );
    }
    m_matrices.clear();

    // Constants
    for ( map_Constant::iterator c = m_constants.begin();
          c != m_constants.end(); c++ ) {
        R_ASSERT( 1 ==
                  c->second->dwReference.load( std::memory_order_relaxed ) );
        xr_delete( c->second );
    }
    m_constants.clear();
...
void CResourceManager::OnDeviceCreate( IReader* F ) {
    ...
    if ( fs ) {
        ...
        while ( ( chunk = fs->open_chunk( chunk_id ) ) != NULL ) {
            ...
            if ( 0 == B ) {
                ...
                std::pair< map_BlenderIt, bool > I =
                    m_blenders.insert( mk_pair( xr_strdup( desc.cName ), B ) );
    ...
    const u32 texturesMs =
        startupTimer.GetElapsed_ms() - scriptingMs - libraryMs;
    m_reduceLodTextureList.clear();
    if ( pSettings && pSettings->section_exist( "reduce_lod_texture_list" ) ) {
        const CInifile::Sect& section =
            pSettings->r_section( "reduce_lod_texture_list" );
    ...
    const u32 settingsMs =
        startupTimer.GetElapsed_ms() - scriptingMs - libraryMs - texturesMs;
    Msg( "* [STARTUP/RENDER RESOURCES] scripting=%u library=%u textures=%u "
         "settings=%u total=%u ms",
         scriptingMs, libraryMs, texturesMs, settingsMs,
