...
// predicates
IC bool p_sort_constants( const ref_constant& C1,
                          const ref_constant& C2 ) noexcept {
...
R_constant* R_constant_table::get( LPCSTR S ) {
    ...
    // demonized: make shared_str and use override, str most likely already
    // exists, faster
    shared_str s( S );
    return get( s );
}
...
R_constant* R_constant_table::get( shared_str& S ) {
    ...
    static auto sortFunc = []( const ref_constant& C,
                               const shared_str& S ) noexcept {
        return C->name < S;
    };
...
BOOL R_constant_table::parse( void* _desc, u32 destination ) {
    ...
    D3DXSHADER_CONSTANTINFO* it =
        ( D3DXSHADER_CONSTANTINFO* )( LPBYTE( desc ) + desc->ConstantInfo );
                            ...
                            case 3:
                                switch ( it->RegisterCount ) {
                                    ...
                                    case 3:
                                        ...
                                        break;
                                    default:
                                        Msg( "Invalid matrix dimension:%dx%d "
                                             "in constant %s",
                                             it->RegisterCount, T->Columns,
                                             name );
                                        fatal(
                                            "MATRIX_ROWS: unsupported number "
                                            "of RegisterCount" );
                                        break;
                                }
                                break;
            ...
            case D3DXPC_OBJECT: {
                switch ( T->Type ) {
                    ...
                    case D3DXPT_SAMPLERCUBE: {
                        // ***Register sampler***
                        // We have determined all valuable info, search if
                        // constant already created
                        ref_constant C = get( name );
                        if ( !C ) {
                            C = xr_new<
                                R_constant >(); //.g_constant_allocator.create();
                            C->name = name;
                    ...
                    default:
                        fatal(
                            "Pclass D3DXPC_OBJECT - object isn't of 'sampler' "
                            "type" );
                        break;
                }
            }
...
void R_constant_table::merge( R_constant_table* T ) {
    ...
    for ( u32 it = 0; it < T->table.size(); it++ ) {
        ...
        if ( !C ) {
            ...
            VERIFY2( !( C->destination & src->destination & RC_dest_sampler ),
                     "Can't have samplers or textures with the same name for "
                     "PS, VS and GS." );
            C->destination |= src->destination;
    ...
    if ( !table_tmp.empty() ) {
        // Append
        std::move( table_tmp.begin(), table_tmp.end(),
