...
bool PortalTraverseDbg_IsOptions( u32 options ) {
    return !!( options & ( CDSGraphManager::VQ_HOM | CDSGraphManager::VQ_SSA |
...
void CDSGraphManager::traverse( CSector* start,
                                CFrustum& F,
                                Fvector& vBase,
                                Fmatrix& mXFORM ) {
    if ( !start )
...
#if RENDER == R_R1
    if ( val_pObject ) {
        VERIFY( smart_cast< CObject* >( O ) ||
                smart_cast< CPS_Instance* >( O ) );
        if ( O->renderable.pROS ) {
            VERIFY( smart_cast< CROS_impl* >( O->renderable.pROS ) );
        }
    }
...
void CDSGraphManager::fade_render() {
    ...
    for ( auto& fp : f_portals )
        _pcount += fp.key->getPoly().size() - 2;

    // fill buffers
    u32 _offset = 0;
    FVF::L* _v =
        ( FVF::L* )RCache.Vertex.Lock( _pcount * 3, f_geom.stride(), _offset );
    float ssaRange = r_ssaLOD_A - r_ssaLOD_B;
    Fvector _ambient_f = g_pGamePersistent->Environment().CurrentEnv->ambient;
    u32 _ambient =
        psDeviceFlags.test( rsDrawPortals )
            ? u32( -1 )
            : color_rgba_f( _ambient_f.x, _ambient_f.y, _ambient_f.z, 0.f );
    for ( auto& fp : f_portals ) {
        CPortal* _P = fp.key;
        u32 _clr = u32( -1 );
        if ( psDeviceFlags.test( rsDrawPortals ) )
