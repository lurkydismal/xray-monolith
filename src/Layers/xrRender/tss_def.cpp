...
IDirect3DStateBlock9* SimulatorStates::record() {
    ...
    for ( u32 it = 0; it < States.size(); it++ ) {
        ...
        switch ( S.type ) {
            case 0:
                CHK_DX( HW.pDevice->SetRenderState( ( D3DRENDERSTATETYPE )S.v1,
                                                    S.v2 ) );
                break;
            case 1:
            ...
            case 2: {
                CHK_DX( HW.pDevice->SetSamplerState(
                    ...
                    ( ( D3DSAMPLERSTATETYPE )S.v2 == D3DSAMP_MAGFILTER &&
                      S.v3 == D3DTEXF_ANISOTROPIC )
                        ? D3DTEXF_LINEAR
                        : S.v3 ) );
            } break;
        }
...
BOOL SimulatorStates::equal( SimulatorStates& S ) {
    if ( States.size() != S.States.size() )
        return FALSE;
    if ( 0 != memcmp( &*States.begin(), &*S.States.begin(),
    ...
    for ( u32 it = 0; it < States.size(); it++ ) {
        ...
        if ( S.type == 0 ) {
            ...
            switch ( S.v1 ) {
                ...
                case D3DRS_CULLMODE:
                    desc.CullMode =
                        dx10StateUtils::ConvertCullMode( ( D3DCULL )S.v2 );
                    break;
                    /*
                    switch (S.v2)
                    {
        ...
        if ( S.type == 0 ) {
            switch ( S.v1 ) {
                ...
                case D3DRS_ZWRITEENABLE:
                    desc.DepthWriteMask = S.v2 ? D3D_DEPTH_WRITE_MASK_ALL
                                               : D3D_DEPTH_WRITE_MASK_ZERO;
                    break;

                case D3DRS_ZFUNC:
                    desc.DepthFunc = dx10StateUtils::ConvertCmpFunction(
                        ( D3DCMPFUNC )S.v2 );
                    break;
                ...
                case D3DRS_STENCILFAIL:
                    desc.FrontFace.StencilFailOp =
                        dx10StateUtils::ConvertStencilOp(
                            ( D3DSTENCILOP )S.v2 );
                    break;
                ...
                case D3DRS_STENCILFUNC:
                    desc.FrontFace.StencilFunc =
                        dx10StateUtils::ConvertCmpFunction(
                            ( D3DCMPFUNC )S.v2 );
                    break;
                ...
                case D3DRS_SRCBLEND:
                    for ( int i = 0; i < 8; ++i )
                        desc.RenderTarget[ i ].SrcBlend =
                            dx10StateUtils::ConvertBlendArg( ( D3DBLEND )S.v2 );
                    break;
    ...
    for ( u32 it = 0; it < States.size(); it++ ) {
        ...
        if ( S.type == 0 ) {
            switch ( S.v1 ) {
                ...
                case D3DRS_BLENDOP:
                    desc.BlendOp =
                        dx10StateUtils::ConvertBlendOp( ( D3DBLENDOP )S.v2 );
                    break;
...
void SimulatorStates::UpdateDesc(
    ...
    for ( u32 it = 0; it < States.size(); it++ ) {
        ...
        if ( S.type == 2 ) {
            ...
            switch ( S.v2 ) {
                    // D3D_FILTER Filter;
                case D3DSAMP_MAGFILTER: /* D3DTEXTUREFILTER filter to use for
                                           magnification */
                    switch ( S.v3 ) {
                        case D3DTEXF_NONE:
                        case D3DTEXF_POINT:
                            desc.Filter =
                                ( D3D_FILTER )( desc.Filter &
                                                ( ~MagfilterLinear ) );
                            break;
                        case D3DTEXF_LINEAR:
                            desc.Filter =
                ...
                case D3DSAMP_MINFILTER: /* D3DTEXTUREFILTER filter to use for
                                           minification */
                    switch ( S.v3 ) {
                        case D3DTEXF_NONE:
                        case D3DTEXF_POINT:
                        ...
                        case D3DTEXF_LINEAR:
                            desc.Filter =
                                ( D3D_FILTER )( desc.Filter | MinfilterLinear );
                            // desc.Filter |= MinfilterLinear;
                            break;
                        default:
                            NODEFAULT;
                ...
                case D3DSAMP_MIPFILTER: /* D3DTEXTUREFILTER filter to use
                                           between mipmaps during minification
                                         */
                    switch ( S.v3 ) {
                        case D3DTEXF_NONE:
                        case D3DTEXF_POINT:
                        ...
                        case D3DTEXF_LINEAR:
                            desc.Filter =
                                ( D3D_FILTER )( desc.Filter | MipfilterLinear );
                            // desc.Filter |= MipfilterLinear;
                            break;
                        default:
                            NODEFAULT;
                ...
                case XRDX10SAMP_ANISOTROPICFILTER:
                    if ( S.v3 )
                        desc.Filter =
                            ( D3D_FILTER )( desc.Filter | FilterAnisotropic );
                    // desc.Filter |= FilterAnisotropic;
                    else
                        desc.Filter = ( D3D_FILTER )( desc.Filter &
                                                      ( ~FilterAnisotropic ) );
                    // desc.Filter &= ~FilterAnisotropic;
                    break;
                ...
                case XRDX10SAMP_COMPARISONFILTER:
                    if ( S.v3 )
                        desc.Filter =
                            ( D3D_FILTER )( desc.Filter | FilterComparison );
                ...
                case D3DSAMP_ADDRESSU: /* D3DTEXTUREADDRESS for U coordinate */
                    desc.AddressU = dx10StateUtils::ConvertTextureAddressMode(
                        D3DTEXTUREADDRESS( S.v3 ) );
                    break;

                case D3DSAMP_ADDRESSV: /* D3DTEXTUREADDRESS for V coordinate */
                    desc.AddressV = dx10StateUtils::ConvertTextureAddressMode(
                        D3DTEXTUREADDRESS( S.v3 ) );
                    break;

                case D3DSAMP_ADDRESSW: /* D3DTEXTUREADDRESS for W coordinate */
                    desc.AddressW = dx10StateUtils::ConvertTextureAddressMode(
                        D3DTEXTUREADDRESS( S.v3 ) );
                    break;
