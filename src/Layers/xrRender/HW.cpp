...
void CHW::Reset( HWND hwnd ) {
    ...
    // Windoze
    DevPP.SwapEffect =
        D3DSWAPEFFECT_FLIP; // bWindowed?D3DSWAPEFFECT_COPY:D3DSWAPEFFECT_DISCARD;
    ...
    if ( !bWindowed )                                     // Refresh rate
        DevPP.FullScreen_RefreshRateInHz = selectRefresh(
            DevPP.BackBufferWidth, DevPP.BackBufferHeight, Caps.fTarget );
    else
    ...
    while ( TRUE ) {
        ...
        if ( SUCCEEDED( _hr ) )
            break;
        Msg( "! ERROR: [%dx%d]: %s", DevPP.BackBufferWidth,
             DevPP.BackBufferHeight, Debug.error2string( _hr ) );
        Sleep( 100 );
    }
...
void CHW::CreateD3D() {
    ...
    R_ASSERT2( hD3D,
               "Can't find 'd3d9.dll'\nPlease install latest version of "
               "DirectX before running this program" );
...
#pragma todo( "R2 need to specify depth format" )
    ...
    // R1 usual
    static D3DFORMAT fDS_Try1[ 6 ] = { D3DFMT_D24S8, D3DFMT_D24X4S4,
                                       D3DFMT_D32,   D3DFMT_D24X8,
    ...
    for ( int it = 0; it < fDS_Cnt; it++ ) {
        if ( SUCCEEDED( pD3D->CheckDeviceFormat(
                 DevAdapter, DevT, fTarget, D3DUSAGE_DEPTHSTENCIL,
                 D3DRTYPE_SURFACE, fDS_Try[ it ] ) ) ) {
            if ( SUCCEEDED( pD3D->CheckDepthStencilMatch(
                     DevAdapter, DevT, fTarget, fTarget, fDS_Try[ it ] ) ) ) {
                return fDS_Try[ it ];
            }
        }
    }
...
void CHW::CreateDevice( HWND m_hWnd, bool move_window ) {
    ...
    for ( UINT Adapter = 0; Adapter < pD3D->GetAdapterCount(); Adapter++ ) {
        ...
        if ( SUCCEEDED( Res ) &&
             ( xr_strcmp( Identifier.Description, "NVIDIA PerfHUD" ) == 0 ) ) {
            DevAdapter = Adapter;
            DevT = D3DDEVTYPE_REF;
            break;
        }
    ...
    Msg( "* GPU [vendor:%X]-[device:%X]: %s", adapterID.VendorId,
         adapterID.DeviceId, adapterID.Description );
    ...
    Msg( "* GPU driver: %d.%d.%d.%d", u32( drv_Product ), u32( drv_Version ),
         u32( drv_SubVersion ), u32( drv_Build ) );
    ...
    if ( bWindowed ) {
        ...
        R_CHK(
            pD3D->CheckDeviceType( DevAdapter, DevT, fTarget, fTarget, TRUE ) );
        fDepth = selectDepthStencil( fTarget );
    } else {
        switch ( psCurrentBPP ) {
            case 32:
                fTarget = D3DFMT_X8R8G8B8;
                if ( SUCCEEDED( pD3D->CheckDeviceType(
                         DevAdapter, DevT, fTarget, fTarget, FALSE ) ) )
                    break;
    ...
    if ( !bWindowed )
        P.FullScreen_RefreshRateInHz =
            selectRefresh( P.BackBufferWidth, P.BackBufferHeight, fTarget );
#else  //! VSYNC_FIX
    // Refresh rate
    ...
    Msg( "*          DDI-level: %2.1f",
         float( D3DXGetDriverLevel( pDevice ) ) / 100.f );
#ifndef _EDITOR
...
#define GMA_SL_SIZE 43
        ...
        DWORD IntelGMA_SoftList[ GMA_SL_SIZE ] = {
            0x2782, 0x2582, 0x2792, 0x2592, 0x2772, 0x2776, 0x27A2, 0x27A6,
            0x27AE, 0x2982, 0x2983, 0x2992, 0x2993, 0x29A2, 0x29A3, 0x2972,
            0x2973, 0x2A02, 0x2A03, 0x2A12, 0x2A13, 0x29C2, 0x29C3, 0x29B2,
            0x29B3, 0x29D2, 0x29D3,

            0x2A42, 0x2A43, 0x2E02, 0x2E03, 0x2E12, 0x2E13, 0x2E22, 0x2E23,
            0x2E32, 0x2E33, 0x2E42, 0x2E43, 0x2E92, 0x2E93, 0x0042, 0x0046 };
    ...
    if ( isIntelGMA )
        switch ( ps_r1_SoftwareSkinning ) {
            ...
            case 2:
                ...
                Msg( "*   setting 'r1_software_skinning' to '1' may improve "
                     "performance" );
                break;
        }
    else if ( ps_r1_SoftwareSkinning == 1 ) {
        Msg( "* WARNING: Using software skinning" );
        Msg( "*   setting 'r1_software_skinning' to '0' should improve "
             "performance" );
    }
...
BOOL CHW::support( D3DFORMAT fmt, DWORD type, DWORD usage ) {
    HRESULT hr = pD3D->CheckDeviceFormat( DevAdapter, DevT, Caps.fTarget, usage,
...
void	fill_render_mode_list()
{
    ...
    CHW							_HW;
    _HW.CreateD3D				();
    _HW.pD3D->GetDeviceCaps
(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,&caps); _HW.DestroyD3D
...
void fill_vid_mode_list( CHW* _hw ) {
    ...
    xr_vector< LPCSTR > _tmp;
    u32 cnt =
        _hw->pD3D->GetAdapterModeCount( _hw->DevAdapter, _hw->Caps.fTarget );
