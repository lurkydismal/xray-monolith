...
struct ECORE_API resptrcode_geom : public resptr_base< SGeometry > {
    void create( D3DVERTEXELEMENT9* decl,
                 ID3DVertexBuffer* vb,
                 ID3DIndexBuffer* ib );
...
struct ECORE_API SPass : public xr_resource_flagged {
    ...
    ref_vs vs;
    // may be NULL = FFP, in that case "state" must contain RS setup, *and*
    // FVF-compatible declaration must be used
...
public:
    ref_selement
        E[ SHADER_ELEMENTS_MAX ]; // R1 - 0=norm_lod0(det),
                                  // 1=norm_lod1(normal),	2=L_point,
                                  // 3=L_spot,	4=L_for_models,
    // R2 - 0=deffer,			1=norm_lod1(normal),	2=psm,
    // 3=ssm,		4=dsm
    // Shader();
...
struct ECORE_API resptrcode_shader : public resptr_base< Shader > {
    void create( LPCSTR s_shader = 0,
                 LPCSTR s_textures = 0,
                 LPCSTR s_constants = 0,
                 LPCSTR s_matrices = 0 );
    void create( IBlender* B,
                 LPCSTR s_shader = 0,
                 LPCSTR s_textures = 0,
                 LPCSTR s_constants = 0,
                 LPCSTR s_matrices = 0 );
    void create_parallel( IBlender* B,
                          LPCSTR s_shader = 0,
                          LPCSTR s_textures = 0,
                          LPCSTR s_constants = 0,
                          LPCSTR s_matrices = 0 );
