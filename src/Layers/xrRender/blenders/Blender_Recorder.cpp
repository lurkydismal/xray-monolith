...
void CBlender_Compile::_cpp_Compile( ShaderElement* _SH ) {
    ...
    if ( bDetail && BT->canBeDetailed() ) {
        ...
        if ( id >= 0 ) {
            if ( id >= int( lst.size() ) )
                Debug.fatal(
                    DEBUG_INFO,
                    "Not enought textures for shader. Base texture: '%s'.",
                    *lst[ 0 ] );
            base = *lst[ id ];
        }
        //.		if
    ...
    if ( bDetail ) {
        DEV->m_textures_description.GetTextureUsage( base, bDetail_Diffuse,
                                                     bDetail_Bump );
        ...
        if ( !( RImplementation.o.advancedpp &&
                ps_r2_ls_flags.test( R2FLAG_DETAIL_BUMP ) ) ) {
...
void CBlender_Compile::PassSET_ZB( BOOL bZTest,
                                   BOOL bZWrite,
                                   BOOL bInvertZTest ) {
    if ( Pass() )
        bZWrite = FALSE;
    RS.SetRS( D3DRS_ZFUNC,
              bZTest ? ( bInvertZTest ? D3DCMP_GREATER : D3DCMP_LESSEQUAL )
                     : D3DCMP_ALWAYS );
    RS.SetRS( D3DRS_ZWRITEENABLE, BC( bZWrite ) );
    /*
    if (bZWrite || bZTest)				RS.SetRS
    (D3DRS_ZENABLE,	D3DZB_TRUE); else
    RS.SetRS	(D3DRS_ZENABLE,	D3DZB_FALSE);
    */
}

void CBlender_Compile::PassSET_ablend_mode( BOOL bABlend,
                                            u32 abSRC,
                                            u32 abDST ) {
...
void CBlender_Compile::PassSET_Blend( BOOL bABlend,
                                      u32 abSRC,
                                      u32 abDST,
                                      BOOL bATest,
                                      u32 aRef ) {
...
#if !defined( USE_DX10 ) && !defined( USE_DX11 )
void CBlender_Compile::StageSET_TMC( LPCSTR T,
                                     LPCSTR M,
                                     LPCSTR C,
                                     int UVW_channel ) {
    Stage_Texture( T );
...
void CBlender_Compile::Stage_Texture( LPCSTR name,
    ...
    passTextures.push_back(
        mk_pair( Stage(), ref_texture( DEV->_CreateTexture( N ) ) ) );
    ...
    if ( M ) {
        switch ( M->dwMode ) {
            case CMatrix::modeProgrammable:
                StageSET_XForm( D3DTTFF_COUNT3,
                                D3DTSS_TCI_CAMERASPACEPOSITION | ID );
                break;
            case CMatrix::modeTCM:
                StageSET_XForm( D3DTTFF_COUNT2,
                                D3DTSS_TCI_PASSTHRU | iChannel );
                break;
            case CMatrix::modeC_refl:
                StageSET_XForm( D3DTTFF_COUNT3,
                                D3DTSS_TCI_CAMERASPACEREFLECTIONVECTOR | ID );
                break;
            case CMatrix::modeS_refl:
                StageSET_XForm( D3DTTFF_COUNT2,
                                D3DTSS_TCI_CAMERASPACENORMAL | ID );
                break;
            default:
                StageSET_XForm( D3DTTFF_DISABLE,
                                D3DTSS_TCI_PASSTHRU | iChannel );
                break;
        }
    } else {
...
void CBlender_Compile::Stage_Constant( LPCSTR name ) {
    ...
    passConstants.push_back(
        DEV->_CreateConstant( ( id >= 0 ) ? *lst[ id ] : name ) );
}
