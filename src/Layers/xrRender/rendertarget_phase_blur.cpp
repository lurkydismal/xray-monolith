...
void CRenderTarget::phase_ssfx_ssr() {
    ...
    RCache.set_c( "cam_pos", ::Random.randF( -1.0, 1.0 ),
                  ::Random.randF( -1.0, 1.0 ), 0.0f, 0.0f );
    ...
    // COPY SSR RESULT ( ACC ) ////////////////////////////////////////////
    HW.pContext->CopyResource( rt_ssfx_ssr->pTexture->surface_get(),
                               rt_ssfx->pTexture->surface_get() );
...
void CRenderTarget::phase_ssfx_volumetric_blur() {
    ...
    // Be careful and clear the buffer ( rt_Generic_2 contain unspeakable stuff
    // if no volumetric is written )
    ...
    set_viewport_size( HW.pContext, w / RImplementation.o.volsize,
                       h / RImplementation.o.volsize );
    ...
    for ( int b = 0; b < 4; b++ ) {
        ...
        RCache.set_c( "blur_setup", w / RImplementation.o.volsize,
                      h / RImplementation.o.volsize, pixelsize[ b ],
                      pixelscale[ b ] );
        RCache.set_Geometry( g_combine );
        RCache.Render( D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2 );
    }
    ...
    HW.pContext->CopyResource( rt_Generic_2->pTexture->surface_get(),
                               rt_ssfx_accum->pTexture->surface_get() );
};
...
void CRenderTarget::phase_ssfx_water_blur() {
    ...
    if ( ps_ssfx_water.y > 0 ) {
        ...
        HW.pContext->CopyResource( rt_ssfx_temp2->pTexture->surface_get(),
...
void CRenderTarget::phase_ssfx_water_waves() {
    ...
    RCache.set_c( "wind_setup", g_pGamePersistent->Environment().wind_anim.w,
                  g_pGamePersistent->Environment().CurrentEnv->wind_velocity, 0,
                  0 );
    RCache.set_Geometry( g_combine );
    RCache.Render( D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2 );
...
void CRenderTarget::phase_ssfx_sss() {
    ...
    HW.pContext->CopyResource( rt_ssfx_sss->pTexture->surface_get(),
                               rt_ssfx_temp2->pTexture->surface_get() );
};
...
void CRenderTarget::phase_ssfx_sss_ext( light_Package& LP ) {
    ...
    {
        RCache.get_ConstantDirect( strLights, 4 * sizeof( Fvector4 ) * 2, 0, 0,
                                   &LightData );
    ...
    if ( Lights_Array ) {
        ...
        if ( Device.dwFrame > sss_currentframe ) {
            ...
            // normally it should be omnipart_num 0, but with omnipart_vischeck
            // its not guaranteed, hope its stable
            ...
            for ( u32 it = 0; it < source.size(); it++ ) {
                ...
                if ( ( L->omnipart_num == 0 ||
                       ( parent && omni_parents.find( parent ) ==
                                       omni_parents.end() ) ) &&
                     L->range > 1.5f ) {
                    ...
                    if ( L->distance < 800 && L->flags.bActive ) {
                        L->distance_lpos =
                            Device.vCameraPosition.distance_to( L->position );
        ...
        for ( int slot = 0; slot < 8; slot++ ) {
            if ( LightSlot[ slot ] ) {
                ...
                if ( CheckPackage ) {
                    ...
                    LightSlot[ slot ]->distance_lpos =
                        Device.vCameraPosition.distance_to(
                            LightSlot[ slot ]->position );
                    Remove = false;
                }
                ...
                if ( !LightSlot[ slot ]->flags.bActive || Remove ) {
                    // demonized: keep the light pointer for some frames to
                    // eliminate flicker, but also check if its actually been
                    // disabled
                    LightSlot[ slot ]->sss_remove_latency++;
                    if ( !LightSlot[ slot ]->flags.bActive ||
                         LightSlot[ slot ]->sss_remove_latency > 5 ) {
                        if ( LightSlot[ slot ]->flags.type ==
                             IRender_Light::OMNIPART )
                    ...
                    Device.mView.transform_tiny( L_pos,
                                                 LightSlot[ slot ]->position );

                    // Distance Atte ( Use MaxAtte if the light range is bigger
                    // than the max sort range )
                    float MaxAtte =
                        1.0f -
                        ( clampr( ( LightSlot[ slot ]->distance - 780 ) / -100,
                                  0.f, 1.f ) );
                    float Atte =
                        1.0f -
                        ( clampr( ( Dist - LightSlot[ slot ]->range * 1.9f ) /
                                      -( LightSlot[ slot ]->range / 2.0f ),
                                  0.f, 1.f ) );

                    // ( Reminder ) The value is inverted ( 1.0 = Fadeout ~ 0.0
                    // = Full Visible )
                    Lights_Array[ slot ].set( L_pos.x, L_pos.y, L_pos.z,
                                              std::max( MaxAtte, Atte ) );
                }
            }
        }
    }
    ...
    HW.pContext->CopyResource( rt_ssfx_sss_ext->pTexture->surface_get(),
                               rt_ssfx_sss_tmp->pTexture->surface_get() );
    ...
    HW.pContext->CopyResource( rt_ssfx_sss_ext2->pTexture->surface_get(),
                               rt_ssfx_sss_tmp->pTexture->surface_get() );
