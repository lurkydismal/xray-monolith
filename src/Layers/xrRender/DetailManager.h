...
// //! dm_size*2 must be div dm_cache1_count
const int dm_max_objects =
    16383; // v4 14-bit id range (0x3FFF reserved for empty)
...
extern u32 dm_current_size; //				=
                            // iFloor((float)ps_r__detail_radius/4)*2;
                            ////!
extern u32 dm_current_cache1_line;
//		= dm_current_size*2/dm_cache1_count;		//!
// dm_current_size*2 must be div dm_cache1_count
extern u32
    dm_current_cache_line; //		= dm_current_size+1+dm_current_size;
extern u32
    dm_current_cache_size; //		=
                           // dm_current_cache_line*dm_current_cache_line;
extern float
...
#else
const int dm_size = 24; //!
const int dm_cache1_line =
    dm_size * 2 / dm_cache1_count; //! dm_size*2 must be div dm_cache1_count
const int dm_cache_line = dm_size + 1 + dm_size;
const int dm_cache_size = dm_cache_line * dm_cache_line;
const float dm_fade = float( 2 * dm_size ) - .5f;
#endif
...
public:
    ...
    struct SlotItem {
        ...
        u32 vis_ID; // индекс в visibility списке он же тип [не качается,
                    // качается1, качается2]
        float c_hemi;
        float c_sun;
        float distance;
    ...
    typedef xr_vector< CDetail* >
        DetailVec; // dynamic; 14-bit id range enforced at Load()
...
public:
    ...
    // // non-unpacked slots Slot
    // cache_pool	[dm_cache_size];				// just
    // memory for slots
...
public:
    ...
    void hw_Render_dump( const Fvector4& consts,
                         const Fvector4& wave,
                         const Fvector4& wind,
                         ...
                         light* L = nullptr );
#else  //	USE_DX10
    void hw_Render_dump( ref_constant array,
                         u32 var_id,
                         u32 lod_id,
                         u32 c_base,
                         light* L = nullptr );
...
public:
    ...
    void Load( bool publish = true,
               bool create_shaders = true,
