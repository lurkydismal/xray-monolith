...
public:
    CSkeletonWallmark( CKinematics* p,
                       const Fmatrix* m,
                       ref_shader s,
                       const Fvector& cp,
                       float ts,
...
DEFINE_VECTOR( intrusive_ptr< CSkeletonWallmark >,
               SkeletonWMVec,
               SkeletonWMVecIt );
...
public:
    ...
    void CLBone( const CBoneData* bd,
                 CBoneInstance& bi,
                 const Fmatrix* parent,
                 u8 mask_channel = ( 1 << 0 ) );

    void BoneChain_Calculate( const CBoneData* bd,
                              CBoneInstance& bi,
                              u8 channel_mask,
                              bool ignore_callbacks );
    void Bone_GetAnimPos( Fmatrix& pos,
                          u16 id,
                          u8 channel_mask,
                          bool ignore_callbacks );
...
protected:
    ...
    accel* bone_map_P; // bones  associations	(shared)	- sorted by
                       // name-pointer
...
public:
    // wallmarks
    void AddWallmark( const Fmatrix* parent,
    ...
    void RenderWallmark( intrusive_ptr< CSkeletonWallmark > wm,
                         FVF::LIT*& verts );
    void ClearWallmarks();

public:
    bool PickBone( const Fmatrix& parent_xform,
...
public:
    ...
    ICF Fmatrix& _BCL LL_GetTransform( u16 bone_id ) {
    ...
    // Main functionality
    virtual void CalculateBones(
        BOOL bForceExact = FALSE ); // Recalculate skeleton
...
public:
    ...
    virtual u32 mem_usage( bool bInstance ) {
        ...
        if ( !bInstance ) {
            ...
            for ( vecBonesIt b_it = bones->begin(); b_it != bones->end();
                  ++b_it )
