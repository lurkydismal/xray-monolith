...
//*** The visual itself
//***************************************************************************
class ECORE_API CKinematicsAnimated : public CKinematics,
...
public:
    // Calculation
private:
    void LL_BuldBoneMatrixDequatize( const CBoneData* bd,
                                     u8 channel_mask,
                                     SKeyTable& keys );
    void LL_BoneMatrixBuild( u16 bone_id,
                             CBoneInstance& bi,
                             const Fmatrix* parent,
                             const SKeyTable& keys );
    virtual void BuildBoneMatrix( const CBoneData* bd,
                                  CBoneInstance& bi,
                                  const Fmatrix* parent,
                                  u8 mask_channel = ( 1 << 0 ) );
...
public:
    ...
    CBlend* LL_PlayFX( u16 bone,
                       MotionID motion,
                       float blendAccrue,
                       float blendFalloff,
                       float Speed,
                       float Power );
    CBlend* LL_PlayCycle( u16 partition,
                          MotionID motion,
                          BOOL bMixing,
                          float blendAccrue,
                          float blendFalloff,
    ...
    CBlend* LL_PlayCycle( u16 partition,
                          ...
                          LPVOID CallbackParam,
                          u8 channel = 0 );
    void LL_FadeCycle( u16 partition,
                       float falloff,
                       u8 mask_channel = ( 1 << 0 ) );
    ...
    void LL_UpdateTracks( float dt,
                          bool b_force,
                          bool leave_blends ); // Update motions
    ...
    CBlend* PlayCycle( LPCSTR N,
                       ...
                       u8 channel = 0 );
    CBlend* PlayCycle( MotionID M,
                       BOOL bMixIn = TRUE,
                       PlayCallback Callback = 0,
                       LPVOID CallbackParam = 0,
                       u8 channel = 0 );
    CBlend* PlayCycle( u16 partition,
                       MotionID M,
                       BOOL bMixIn = TRUE,
                       PlayCallback Callback = 0,
                       LPVOID CallbackParam = 0,
    ...
    virtual u32 mem_usage( bool bInstance ) {
        u32 sz =
            CKinematics::mem_usage( bInstance ) + sizeof( *this ) +
...
// IC CKinematicsAnimated* PKinematicsAnimated(IRender_Visual* V) { return
// V?V->dcast_PKinematicsAnimated():0; }
