...
//	int s_x	= iFloor			(EYE.x/slot_size+.5f)+offs_x;
//// [0...size_x) 	int s_z	= iFloor
//(EYE.z/slot_size+.5f)+offs_z;		// [0...size_z)
...
struct DetailSlot {
    ...
    // --- u32 #2 : 12 + 8 + 4 + 4 + 4 = 32 ---
    u32 y_base
        : 12; // 1 unit = 20 cm, low = -200m, high = 4096*20cm - 200 = 619.2m
...
static_assert( sizeof( DetailSlot_v3 ) == 16,
               "DetailSlot_v3 must be 16 bytes" );
...
inline u16 _detail_id_v3_to_v4( u32 v ) {
...
inline void expand_v3( DetailSlot& d,
                       const DetailSlot_v3& s ) // read path: v3 -> working
{
    d.y_base = s.y_base;
    d.y_height = s.y_height;
    d.id0 = _detail_id_v3_to_v4( s.id0 );
    d.id1 = _detail_id_v3_to_v4( s.id1 );
    d.id2 = _detail_id_v3_to_v4( s.id2 );
    d.id3 = _detail_id_v3_to_v4( s.id3 );
    d.c_dir = s.c_dir;
    d.c_hemi = s.c_hemi;
    d.c_r = s.c_r;
    d.c_g = s.c_g;
    d.c_b = s.c_b;
    for ( int i = 0; i < 4; ++i )
        d.palette[ i ] = s.palette[ i ];
}

inline void pack_v3( DetailSlot_v3& d,
                     const DetailSlot& s ) // write path: working -> v3
{
    // only valid when all ids <= 62 (a <=63-object level); caller guarantees
    // this
    d.y_base = s.y_base;
    d.y_height = s.y_height;
    d.id0 = _detail_id_v4_to_v3( s.id0 );
    d.id1 = _detail_id_v4_to_v3( s.id1 );
    d.id2 = _detail_id_v4_to_v3( s.id2 );
    d.id3 = _detail_id_v4_to_v3( s.id3 );
    d.c_dir = s.c_dir;
    d.c_hemi = s.c_hemi;
    d.c_r = s.c_r;
    d.c_g = s.c_g;
