    ...
    for ( int y = 0; y < occ_dim_0; y++ ) {
        for ( int x = 0; x < occ_dim_0; x++ ) {
            ...
            if ( Tu1 ) {
                ...
                } else if ( pFrame[ pos_down2 ] &&
                            shared( Tu1, pFrame[ pos_down2 ] ) ) {
    ...
    for ( int i = 0; i < occ_dim_0; ++i ) {
        for ( int j = 0; j < occ_dim_0; ++j ) {
            if ( bDebug ) {
                ...
                float z = -Device.mProject._43 /
                          ( float )( Device.mProject._33 - quad.z );
                left_top.set(
                    quad.x * z / Device.mProject._11 / ( occ_dim_0 / 2.f ),
                    quad.y * z / Device.mProject._22 / ( occ_dim_0 / 2.f ), z );
                right_bottom.set( ( quad.x + 1 ) * z / Device.mProject._11 /
                                      ( occ_dim_0 / 2.f ),
                                  ( quad.y + 1 ) * z / Device.mProject._22 /
                                      ( occ_dim_0 / 2.f ),
                                  z );

                box_center.set( ( right_bottom.x + left_top.x ) / 2,
                                ( right_bottom.y + left_top.y ) / 2, z );
                box_r = right_bottom;
            ...
            RCache.dbg_DrawOBB(
                ...
                D3DCOLOR_XRGB( u32( 255 * pow( tmp.z, 20.f ) ),
                               u32( 255 * ( 1 - pow( tmp.z, 20.f ) ) ), 0 ) );
...
BOOL occRasterizer::test( float _x0,
                          float _y0,
                          float _x1,
                          float _y1,
                          float _z ) {
