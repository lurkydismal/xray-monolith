...
private:
    ...
    xr_unordered_flat_map< u64, xr_vector< R_constant_table* > >
        m_constant_table_index;
    xr_unordered_flat_map< u64, xr_vector< STextureList* > >
        m_texture_list_index;
    ...
    struct ResourceLoadGeneration {
        ...
        ResourceLoadGeneration( u64 value, u64 native_value )
            : id( value ),
              native_generation( native_value ),
...
public:
    ...
    void _GetMemoryUsage( u32& m_base,
                          u32& c_base,
                          u32& m_lmaps,
                          u32& c_lmaps );
    ...
    ref_texture _CreateTexture( LPCSTR Name,
                                bool prefetch = false,
                                LPCSTR canonical_level_path = nullptr );
    ...
    void ReleaseLevelShaderCache( LPCSTR canonical_level_path,
                                  u64 recipe_identity );
    ...
    //	DX10 cut CRTC*
    //_CreateRTC			(LPCSTR Name, u32 size,	D3DFORMAT f);
    //	DX10 cut void
    //_DeleteRTC			(const CRTC*	RT	);
#if defined( USE_DX10 ) || defined( USE_DX11 )
    ...
    STextureList* _CreateTextureList( STextureList& L,
                                      ref_texture_list* keep_alive = nullptr );
    ...
    ShaderElement* _CreateElement( ShaderElement& L,
                                   ref_selement* keep_alive = nullptr );
    void _DeleteElement( const ShaderElement* L );

    Shader* _cpp_Create( LPCSTR s_shader,
                         LPCSTR s_textures = 0,
                         LPCSTR s_constants = 0,
                         LPCSTR s_matrices = 0 );
    Shader* _cpp_Create( IBlender* B,
                         LPCSTR s_shader = 0,
                         LPCSTR s_textures = 0,
                         LPCSTR s_constants = 0,
                         LPCSTR s_matrices = 0,
                         bool hud_loading = false,
                         ref_shader* keep_alive = nullptr );
    ...
    // Creation/Destroying
    Shader* Create( LPCSTR s_shader = 0,
                    LPCSTR s_textures = 0,
                    LPCSTR s_constants = 0,
                    LPCSTR s_matrices = 0 );
    Shader* CreateLevelShader( LPCSTR s_shader,
                               LPCSTR s_textures,
                               u64 recipe_identity = 0 );
    ref_shader CreateLevelCppShader( LPCSTR s_shader,
                                     LPCSTR s_textures,
                                     LPCSTR s_constants = nullptr,
                                     LPCSTR s_matrices = nullptr,
                                     u64 recipe_identity = 0,
                                     LPCSTR canonical_level_path = nullptr );
    Shader* Create( IBlender* B,
                    LPCSTR s_shader = 0,
                    LPCSTR s_textures = 0,
                    LPCSTR s_constants = 0,
                    LPCSTR s_matrices = 0 );
    ...
    SGeometry* CreateGeom( D3DVERTEXELEMENT9* decl,
                           ID3DVertexBuffer* vb,
                           ID3DIndexBuffer* ib,
                           ref_geom* keep_alive = nullptr );
    SGeometry* CreateGeom( u32 FVF,
                           ID3DVertexBuffer* vb,
                           ID3DIndexBuffer* ib,
                           ref_geom* keep_alive = nullptr );
    ...
    bool IsTextureOwnerThread() const {
        return textureOwnerThread == GetCurrentThreadId();
    }
