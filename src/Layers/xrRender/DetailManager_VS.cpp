...
void CDetailManager::hw_Load_Geom() {
    ...
    Msg( "* [DETAILS] VertexConsts(%d), Batch(%d)",
         u32( HW.Caps.geometry.dwRegisters ), hw_BatchSize );
    ...
    // Create VB/IB
    R_CHK( HW.pDevice->CreateVertexBuffer( dwVerts * vSize, dwUsage, 0,
                                           D3DPOOL_MANAGED, &hw_VB, 0 ) );
    HW.stats_manager.increment_stats_vb( hw_VB );
    R_CHK( HW.pDevice->CreateIndexBuffer(
        dwIndices * 2, dwUsage, D3DFMT_INDEX16, D3DPOOL_MANAGED, &hw_IB, 0 ) );
    HW.stats_manager.increment_stats_ib( hw_IB );

#endif //	USE_DX10
    Msg( "* [DETAILS] Batch(%d), VB(%dK), IB(%dK)", hw_BatchSize,
         ( dwVerts * vSize ) / 1024, ( dwIndices * 2 ) / 1024 );
...
#if defined( USE_DX10 ) || defined( USE_DX11 )
        R_CHK( dx10BufferUtils::CreateVertexBuffer( &hw_VB, pVOriginal,
                                                    dwVerts * vSize ) );
        HW.stats_manager.increment_stats_vb( hw_VB );
        xr_free( pVOriginal );
#else  //	USE_DX10
...
#if defined( USE_DX10 ) || defined( USE_DX11 )
        R_CHK( dx10BufferUtils::CreateIndexBuffer( &hw_IB, pIOriginal,
                                                   dwIndices * 2 ) );
        HW.stats_manager.increment_stats_ib( hw_IB );
        xr_free( pIOriginal );
#else  //	USE_DX10
...
void CDetailManager::hw_Render( light* L ) {
    ...
    // float		tm_rot1		=
    // (PI_MUL_2*RDEVICE.fTimeGlobal/swing_current.rot1); float		tm_rot2
    // = (PI_MUL_2*RDEVICE.fTimeGlobal/swing_current.rot2);
    ...
    // RDEVICE.fTimeGlobal*swing_current.speed);
    wave.set( 1.f / 5.f, 1.f / 7.f, 1.f / 3.f, m_time_pos );
    RCache.set_c( &*hwc_consts, scale, scale, ps_r__Detail_l_aniso,
                  ps_r__Detail_l_ambient );           // consts
...
void CDetailManager::hw_Render_dump( ref_constant x_array,
                                     u32 var_id,
                                     u32 lod_id,
                                     u32 c_offset,
                                     light* L ) {
    ...
    for ( u32 O = 0; O < objects.size(); O++ ) {
        ...
        if ( !vis.empty() ) {
            ...
            Fvector4* c_storage =
                RCache.get_ConstantCache_Vertex().get_array_f().access(
                    c_base );
            ...
            for ( ; _vI != _vE; _vI++ ) {
                ...
                for ( ; _iI != _iE; _iI++ ) {
                    ...
                    if ( RImplementation.phase == CRender::PHASE_SMAP && L ) {
                        ...
                        if ( L->position.distance_to_sqr( Instance.position ) >=
                             _sqr( L->range ) )
                            continue;
                    }
#endif
