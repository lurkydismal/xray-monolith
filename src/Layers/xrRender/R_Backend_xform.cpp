...
void R_xforms::set_W_prev( const Fmatrix& m ) {
    ...
    if ( c_w_prev )
        RCache.set_c( c_w_prev, m_w_prev );
    if ( c_wv_prev )
        RCache.set_c( c_wv_prev, m_wv_prev );
    if ( c_wvp_prev )
...
void R_xforms::set_V_prev( const Fmatrix& m ) {
    ...
    if ( c_v_prev )
        RCache.set_c( c_v_prev, m_v_prev );
    if ( c_vp_prev )
        RCache.set_c( c_vp_prev, m_vp_prev );
    if ( c_wv_prev )
        RCache.set_c( c_wv_prev, m_wv_prev );
    if ( c_wvp_prev )
        RCache.set_c( c_wvp_prev, m_wvp_prev );
...
void R_xforms::set_P_prev( const Fmatrix& m ) {
    ...
    m_wvp_prev.mul( m_p_prev, m_wv_prev );
    if ( c_p_prev )
        RCache.set_c( c_p_prev, m_p_prev );
    if ( c_vp_prev )
        RCache.set_c( c_vp_prev, m_vp_prev );
    if ( c_wvp_prev )
        RCache.set_c( c_wvp_prev, m_wvp_prev );
