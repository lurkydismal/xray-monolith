...
void dxImGuiRender::SetState( ImDrawData* data ) {
    ...
    D3D_VIEWPORT VP = { 0, 0,  data->DisplaySize.x, data->DisplaySize.y,
                        0, 1.f };
#if defined( USE_DX11 )
...
#endif
#else
    D3DVIEWPORT9 VP = { 0, 0,  data->DisplaySize.x, data->DisplaySize.y,
                        0, 1.f };
    ...
    ctx->IASetIndexBuffer(bd->pIB, sizeof(ImDrawIdx) == 2 ? DXGI_FORMAT_R16_UINT
    ...
    ctx->HSSetShader(nullptr, nullptr, 0); // In theory we should backup and
    restore this as well.. very infrequently used.. ctx->DSSetShader(nullptr,
    nullptr, 0); // In theory we should backup and restore this as well.. very
    infrequently used.. ctx->CSSetShader(nullptr, nullptr, 0); // In theory we
    should backup and restore this as well.. very infrequently used..
...
void dxImGuiRender::OnDeviceCreate( ImGuiContext* context ) {
    ImGui::SetAllocatorFunctions(
        []( size_t size, void* /*user_data*/ ) { return xr_malloc( size ); },
