...
void CGammaControl::GenLUT( const DXGI_GAMMA_CONTROL_CAPABILITIES& GC,
                            DXGI_GAMMA_CONTROL& G ) {
    ...
    for ( u32 i = 0; i < GC.NumGammaControlPoints; i++ ) {
        ...
        clamp( G.GammaCurve[ i ].Red, GC.MinConvertedValue,
               GC.MaxConvertedValue );
        clamp( G.GammaCurve[ i ].Green, GC.MinConvertedValue,
               GC.MaxConvertedValue );
        clamp( G.GammaCurve[ i ].Blue, GC.MinConvertedValue,
               GC.MaxConvertedValue );
    }
}
...
void CGammaControl::GenLUT( D3DGAMMARAMP& G ) {
    ...
    float C = fContrast / 2.f;
    for ( int i = 0; i < 256; i++ ) {
        float c = ( C + .5f ) * powf( i / 255.f, og ) * 65535.f +
                  ( B - 0.5f ) * 32768.f - C * 32768.f + 16384.f;
