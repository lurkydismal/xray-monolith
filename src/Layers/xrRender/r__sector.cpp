...
void CPortal::Setup( Fvector* V, int vcnt, CSector* face, CSector* back ) {
    ...
    Debug.fatal		(DEBUG_INFO,"Degenerated portal found at
    {%3.2f,%3.2f,%3.2f}.",VPUSH(poly[0]));
    */
}
...
void CSector::traverse( CFrustum&& F, CDSGraphManager& DM ) {
    ...
    // If the map reallocates, SNODE becomes invalid, but the Index stays
    // correct.
    ...
    for ( CPortal* PORTAL : m_portals ) {
        ...
        // Refresh pointer before use (in case previous loop iteration
        // realloc'd)
        ...
        if ( PORTAL->S.P.distance_to( cam_pos ) < PORTAL->S.R &&
             PORTAL->distance( cam_pos ) <= test_sphere_r ) // bDualRender
        ...
        if ( DM.i_options & CDSGraphManager::VQ_FADE |
                 CDSGraphManager::VQ_SSA &&
             psDeviceFlags.test( rsDrawPortals ) )
        ...
        if ( pSector ) {
            ...
            pSector->traverse(
                CreateFrustumFromPortal( P, DM.i_vBase, DM.i_mXFORM ), DM );
        }
    }
}
