...
IC bool InterpolateAndDither( float* alpha255,
                              u32 x,
                              u32 y,
                              u32 sx,
                              u32 sy,
...
static void draw_obb( const Fmatrix& matrix, const u32& color ) {
    ...
    rdebug_render->add_lines(
        aabb, sizeof( aabb ) / sizeof( Fvector ), &aabb_id[ 0 ],
    ...
    for ( u32 z = 0; z <= d_size; z++ ) {
        for ( u32 x = 0; x <= d_size; x++ ) {
            ...
            if ( ( DS.id0 != DetailSlot::ID_Empty ) &&
                 InterpolateAndDither( alpha255[ 0 ], x, z, shift_x, shift_z,
                                       d_size, dither ) )
                selected.push_back( 0 );
            if ( ( DS.id1 != DetailSlot::ID_Empty ) &&
                 InterpolateAndDither( alpha255[ 1 ], x, z, shift_x, shift_z,
                                       d_size, dither ) )
                selected.push_back( 1 );
            if ( ( DS.id2 != DetailSlot::ID_Empty ) &&
                 InterpolateAndDither( alpha255[ 2 ], x, z, shift_x, shift_z,
                                       d_size, dither ) )
                selected.push_back( 2 );
            if ( ( DS.id3 != DetailSlot::ID_Empty ) &&
                 InterpolateAndDither( alpha255[ 3 ], x, z, shift_x, shift_z,
                                       d_size, dither ) )
            ...
            // Position (XZ)
            float rx = ( float( x ) / float( d_size ) ) * dm_slot_size +
                       D.vis.box.min.x;
            float rz = ( float( z ) / float( d_size ) ) * dm_slot_size +
                       D.vis.box.min.z;
            Fvector Item_P;

#ifndef DBG_SWITCHOFF_RANDOMIZE
            Item_P.set( rx + r_jitter.randFs( jitter ), D.vis.box.max.y,
                        rz + r_jitter.randFs( jitter ) );
#else
            ...
            for ( u32 tid = 0; tid < triCount; tid++ ) {
                ...
                for ( int k = 0; k < ( int )I.inf.size(); k++ ) {
                    ...
                    I.e_obj->GetFaceWorld( I.s_obj->_Transform(), I.e_mesh,
                                           I.inf[ k ].id, verts );
                    RDEVICE.Statistic->TEST0.End();
                    if ( CDB::TestRayTri( Item_P, dir, verts, r_u, r_v, r_range,
                                          TRUE ) ) {
                        if ( r_range >= 0 ) {
                ...
                if ( RImplementation.pOutdoorSector ) {
                    CSector* sector =
                        ( CSector* )RImplementation.getSector( T.sector );
                ...
                Fvector Tv[ 3 ] = { verts[ T.verts[ 0 ] ],
                                    verts[ T.verts[ 1 ] ],
                                    verts[ T.verts[ 2 ] ] };
                if ( CDB::TestRayTri( Item_P, dir, Tv, r_u, r_v, r_range,
                                      TRUE ) ) {
                    if ( r_range >= 0 ) {
                        float y_test = Item_P.y - r_range;
                        if ( y_test > y )
            ...
            if ( DotP >
                 -( 1.0f - r_scale.randF( ps_ssfx_terrain_grass_slope * 0.8f,
                                          ps_ssfx_terrain_grass_slope ) ) )
                continue;
            ...
            if ( y < D.vis.box.min.y )
                continue;
            Item_P.y = y;
            Item.normal = terrain_normal; // Save terrain normal here to feed
                                          // the grass shader later.

            // Angles and scale
#ifndef DBG_SWITCHOFF_RANDOMIZE
            Item.scale = r_scale.randF( Dobj->m_fMinScale * 0.5f,
                                        Dobj->m_fMaxScale * 0.9f );
#else
            Item.scale =
                ( Dobj->m_fMinScale * 0.5f + Dobj->m_fMaxScale * 0.9f ) / 2;
            // Item.scale	= 0.1f;
#endif
            ...
            if ( ps_ssfx_terrain_grass_align > 0 ) {
                ...
                Fvector::generate_orthonormal_basis( Item.mRotY.j, Item.mRotY.i,
                                                     Item.mRotY.k );
            ...
            if ( det_render_debug )
                draw_obb(
                    mXform,
                    color_rgba( 255, 0, 0, 255 ) ); // Fmatrix().mul_43( mXform,
                                                    // Fmatrix().scale(5,5,5) )
#endif
#endif
            ...
            /*
            DetailPalette*	c_pal			=
            (DetailPalette*)&DS.color; float gray255	[4]; gray255[0]
            =	255.f*float(c_pal->a0)/15.f; gray255[1]
            =	255.f*float(c_pal->a1)/15.f; gray255[2]
            =	255.f*float(c_pal->a2)/15.f; gray255[3]
            =	255.f*float(c_pal->a3)/15.f;
            */
            // float c_f
            // =	1.f;	//Interpolate (gray255,x,z,d_size)+.5f; int c_dw
            // =	255;	//iFloor			(c_f); clamp
            // (c_dw,0,255); Item.C_dw
            // =	color_rgba		(c_dw,c_dw,c_dw,255);
#if RENDER == R_R1
            ...
            //? hack: RGB = hemi
            //? Item.c_rgb.add
            //(ps_r__Detail_rainbow_hemi*Item.c_hemi);
