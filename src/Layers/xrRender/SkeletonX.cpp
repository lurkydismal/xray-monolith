...
//////////////////////////////////////////////////////////////////////
void CSkeletonX::_Render( ref_geom& hGeom,
                          u32 vCount,
                          u32 iOffset,
                          u32 pCount ) {
    ...
    if ( RImplementation.o.ssfx_motionvectors ) {
        ...
        if ( RenderMode == 1 ) {
            // RM_SINGLE
            Fmatrix Bone_Prev;
            Bone_Prev.mul_43( Parent->Matrix_Prev,
                              Parent->LL_GetBoneInstance( u16( RMS_boneid ) )
    ...
    switch ( RenderMode ) {
        ...
        case RM_SINGLE: {
            ...
            Fmatrix W;
            W.mul_43( RCache.xforms.m_w,
                      Parent->LL_GetTransform_R( u16( RMS_boneid ) ) );
        ...
        case RM_SKINNING_4B: {
            ...
            R_constant* array_prev =
                RImplementation.phase == RImplementation.PHASE_NORMAL
                    ? RCache.get_c( s_bones_array_prev_const )
                    : array;

            {
                ...
                for ( u32 mid = 0; mid < count; mid++ ) {
                    ...
                    u32 id = mid * 3;
                    RCache.set_ca( &*array, id + 0, M._11, M._21, M._31,
                                   M._41 );
                    RCache.set_ca( &*array, id + 1, M._12, M._22, M._32,
                                   M._42 );
                    RCache.set_ca( &*array, id + 2, M._13, M._23, M._33,
                                   M._43 );

#ifdef USE_DX11
                    if ( RImplementation.phase ==
                         RImplementation.PHASE_NORMAL ) {
                        if ( RImplementation.o.ssfx_motionvectors ) {
                            // Save previous transform
                            Fmatrix& Mprev =
                                Parent->LL_GetBoneInstance( u16( mid ) )
                                    .mRenderTransform_prev;
                            RCache.set_ca( &*array_prev, id + 0, Mprev._11,
                                           Mprev._21, Mprev._31, Mprev._41 );
                            RCache.set_ca( &*array_prev, id + 1, Mprev._12,
                                           Mprev._22, Mprev._32, Mprev._42 );
                            RCache.set_ca( &*array_prev, id + 2, Mprev._13,
                                           Mprev._23, Mprev._33, Mprev._43 );
                        }
                    }
#endif
                }
...
void CSkeletonX::_Render_soft( ref_geom& hGeom,
                               u32 vCount,
                               u32 iOffset,
                               u32 pCount ) {
    ...
    if ( cache_DiscardID != _VS.DiscardID() || vCount != cache_vCount ) {
        vertRender* Dest =
            ( vertRender* )_VS.Lock( vCount, hGeom->vb_stride, vOffset );
        ...
        if ( *Vertices1W ) {
            ...
            PSGP.skin3W( Dest,                  // dest
                         *Vertices3W,           // source
                         vCount,                // count
                         Parent->bone_instances // bones
            );
        } else if ( *Vertices4W ) {
            PSGP.skin4W( Dest,                  // dest
                         *Vertices4W,           // source
                         vCount,                // count
                         Parent->bone_instances // bones
            );
...
void CSkeletonX::_Load( const char* N, IReader* data, u32& dwVertCount ) {
    ...
    switch ( dwVertType ) {
        ...
        case 1: {
            ...
            Vertices1W.create( crc, dwVertCount,
                               ( vertBoned1W* )data->pointer() );
#else
            if ( 1 == bids.size() ) {
                // HW- single bone
        ...
        case 2: {
            ...
            for ( it = 0; it < dwVertCount; ++it ) {
                ...
                if ( bids.end() ==
                     std::find( bids.begin(), bids.end(), VB.matrix0 ) )
                    bids.push_back( VB.matrix0 );

                if ( bids.end() ==
                     std::find( bids.begin(), bids.end(), VB.matrix1 ) )
            ...
            if ( sw_bones_cnt <= hw_bones_cnt ) {
                ...
                // software
                crc = crc32( data->pointer(), size );
                Vertices2W.create( crc, dwVertCount,
                                   ( vertBoned2W* )data->pointer() );
                Engine.External.SetSkinningMode();
            }
        } break;
        ...
        case 3: {
            ...
            for ( it = 0; it < dwVertCount; ++it ) {
                ...
                for ( int i = 0; i < 3; ++i ) {
                    ...
                    if ( bids.end() ==
                         std::find( bids.begin(), bids.end(), VB.m[ i ] ) )
            ...
            if ( ( sw_bones_cnt <= hw_bones_cnt ) ) {
                ...
                crc = crc32( data->pointer(), size );
                Vertices3W.create( crc, dwVertCount,
                                   ( vertBoned3W* )data->pointer() );
                Engine.External.SetSkinningMode();
            }
        } break;
        ...
        case 4: {
            ...
            if ( sw_bones_cnt <= hw_bones_cnt ) {
                ...
                crc = crc32( data->pointer(), size );
                Vertices4W.create( crc, dwVertCount,
                                   ( vertBoned4W* )data->pointer() );
                Engine.External.SetSkinningMode();
            }
        } break;
        default:
            Debug.fatal( DEBUG_INFO,
                         "Invalid vertex type in skinned model '%s'", N );
            break;
    }
#ifdef _EDITOR
    if ( bids.size() > 0 )
...
void get_pos_bones( const vertBoned1W& v, Fvector& p, CKinematics* Parent ) {
    const Fmatrix& xform =
        Parent->LL_GetBoneInstance( ( u16 )v.matrix ).mRenderTransform;
...
void get_pos_bones( const vertBoned2W& vert, Fvector& p, CKinematics* Parent ) {
    ...
    Fmatrix& xform0 =
        Parent->LL_GetBoneInstance( vert.matrix0 ).mRenderTransform;
    Fmatrix& xform1 =
        Parent->LL_GetBoneInstance( vert.matrix1 ).mRenderTransform;
...
BOOL CSkeletonX::_PickBoneSoft1W( IKinematics::pick_result& r,
                                  float dist,
                                  const Fvector& S,
                                  const Fvector& D,
                                  u16* indices,
                                  CBoneData::FacesVec& faces ) {
    return pick_bone< vertBoned1W >( Vertices1W, Parent, r, dist, S, D, indices,
                                     faces );
}

BOOL CSkeletonX::_PickBoneSoft2W( IKinematics::pick_result& r,
                                  float dist,
                                  const Fvector& S,
                                  const Fvector& D,
                                  u16* indices,
                                  CBoneData::FacesVec& faces ) {
    return pick_bone< vertBoned2W >( Vertices2W, Parent, r, dist, S, D, indices,
                                     faces );
}

BOOL CSkeletonX::_PickBoneSoft3W( IKinematics::pick_result& r,
                                  float dist,
                                  const Fvector& S,
                                  const Fvector& D,
                                  u16* indices,
                                  CBoneData::FacesVec& faces ) {
    return pick_bone< vertBoned3W >( Vertices3W, Parent, r, dist, S, D, indices,
                                     faces );
}

BOOL CSkeletonX::_PickBoneSoft4W( IKinematics::pick_result& r,
                                  float dist,
                                  const Fvector& S,
                                  const Fvector& D,
                                  u16* indices,
                                  CBoneData::FacesVec& faces ) {
    return pick_bone< vertBoned4W >( Vertices4W, Parent, r, dist, S, D, indices,
                                     faces );
}

/*
BOOL	CSkeletonX::_PickBoneSoft1W	(Fvector& normal, float& dist, const
Fvector& S, const Fvector& D, u16* indices, CBoneData::FacesVec& faces)
{
        VERIFY				(*Vertices1W);
        ...
        for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end(); it++){
                ...
                for (u32 k=0; k<3; k++){
                        vertBoned1W& vert		=
Vertices1W[indices[idx+k]]; get_pos_bones(vert, p[k], Parent);
                }
...
BOOL CSkeletonX::_PickBoneSoft2W	(Fvector& normal, float& dist, const
Fvector& S, const Fvector& D, u16* indices, CBoneData::FacesVec& faces)
{
        VERIFY				(*Vertices2W);
        bool intersect		= FALSE;
        for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end(); it++){
                ...
                for (u32 k=0; k<3; k++){
                        vertBoned2W& vert		=
Vertices2W[indices[idx+k]]; get_pos_bones(vert, p[k], Parent);
                }
                float u,v,range	= flt_max;
                if (CDB::TestRayTri(S,D,p,u,v,range,true)&&(range<dist)){
                        normal.mknormal(p[0],p[1],p[2]);
...
*/
// Fill Vertices
void CSkeletonX::_FillVerticesSoft1W( const Fmatrix& view,
...
void CSkeletonX::_FillVerticesSoft2W( const Fmatrix& view,
                                      CSkeletonWallmark& wm,
                                      const Fvector& normal,
                                      float size,
                                      u16* indices,
    ...
    for ( CBoneData::FacesVecIt it = faces.begin(); it != faces.end(); it++ ) {
        ...
        for ( u32 k = 0; k < 3; k++ ) {
            ...
            Fmatrix& xform0 = Parent->LL_GetBoneInstance( F.bone_id[ k ][ 0 ] )
                                  .mRenderTransform;
            Fmatrix& xform1 = Parent->LL_GetBoneInstance( F.bone_id[ k ][ 1 ] )
                                  .mRenderTransform;
            F.vert[ k ].set( vert.P );
...
void CSkeletonX::_FillVerticesSoft3W( const Fmatrix& view,
                                      CSkeletonWallmark& wm,
                                      const Fvector& normal,
                                      float size,
                                      u16* indices,
...
void CSkeletonX::_FillVerticesSoft4W( const Fmatrix& view,
                                      CSkeletonWallmark& wm,
                                      const Fvector& normal,
                                      float size,
                                      u16* indices,
