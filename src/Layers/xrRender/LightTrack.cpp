...
const float hdir[ lt_hemisamples ][ 3 ] = {
    { -0.26287, 0.52573, 0.80902 },  { 0.27639, 0.44721, 0.85065 },
    { -0.95106, 0.00000, 0.30902 },  { -0.95106, 0.00000, -0.30902 },
    { 0.58779, 0.00000, -0.80902 },  { 0.58779, 0.00000, 0.80902 },

    { -0.00000, 0.00000, 1.00000 },  { 0.52573, 0.85065, 0.00000 },
    { -0.26287, 0.52573, -0.80902 }, { -0.42533, 0.85065, 0.30902 },
    { 0.95106, 0.00000, 0.30902 },   { 0.95106, 0.00000, -0.30902 },

    { 0.00000, 1.00000, 0.00000 },   { -0.58779, 0.00000, 0.80902 },
    { -0.72361, 0.44721, 0.52573 },  { -0.72361, 0.44721, -0.52573 },
    { -0.58779, 0.00000, -0.80902 }, { 0.16246, 0.85065, -0.50000 },

    { 0.89443, 0.44721, 0.00000 },   { -0.85065, 0.52573, -0.00000 },
    { 0.16246, 0.85065, 0.50000 },   { 0.68819, 0.52573, -0.50000 },
    { 0.27639, 0.44721, -0.85065 },  { 0.00000, 0.00000, -1.00000 },
...
inline void CROS_impl::accum_hemi( float* hemi_cube,
                                   Fvector3& dir,
                                   float scale ) {
    if ( dir.x > 0 )
...
void CROS_impl::update( IRenderable* O ) {
    ...
    if ( MODE & IRender_ObjectSpecific::TRACE_LIGHTS ) {
        ...
        for ( u32 lit = 0; lit < lights.size(); lit++ ) {
...
#if RENDER != R_R1
            float a = ( 1 / ( L->attenuation0 + L->attenuation1 * d +
                              L->attenuation2 * d * d ) -
                        d * L->falloff ) *
            ...
            // multiply intensity on attenuation and accumulate result in hemi
            // cube face
            float koef = ( lights[ lit ].color.r + lights[ lit ].color.g +
                           lights[ lit ].color.b ) /
                         3.0f * a * ps_r2_dhemi_light_scale;
        ...
        float hemi_light =
            ( lacc.x + lacc.y + lacc.z ) / 3.0f * ps_r2_dhemi_light_scale;
        ...
        for ( size_t i = 0; i < NUM_FACES; ++i ) {
            hemi_cube[ i ] +=
                hemi_cube_light[ i ] * ( 1 - ps_r2_dhemi_light_flow ) +
                ps_r2_dhemi_light_flow *
                    hemi_cube_light[ ( i + NUM_FACES / 2 ) % NUM_FACES ];
...
void CROS_impl::smart_update( IRenderable* O ) {
    ...
    if ( ticks_to_update <= 0 ) {
        ...
        if ( result_count < lt_hemisamples )
            ticks_to_update =
                ::Random.randI( s_iUTFirstTimeMin, s_iUTFirstTimeMax + 1 );
        else if ( sky_rays_uptodate < lt_hemisamples )
            ticks_to_update =
                ::Random.randI( s_iUTPosChangedMin, s_iUTPosChangedMax + 1 );
        else
        ...
        if ( !last_position.similar( position, 0.15 ) ) {
            ...
            if ( result_count < lt_hemisamples )
                ...
                ticks_to_update = ::Random.randI( s_iUTPosChangedMin,
                                                  s_iUTPosChangedMax + 1 );
        }
    }
}
...
void CROS_impl::update_smooth( IRenderable* O ) {
    ...
    for ( size_t i = 0; i < NUM_FACES; ++i ) {
        hemi_cube_smooth[ i ] =
            hemi_cube[ i ] * l_f + hemi_cube_smooth[ i ] * l_i;
    }
}
...
void CROS_impl::calc_sun_value( Fvector& position, CObject* _object ) {
    ...
    if ( MODE & IRender_ObjectSpecific::TRACE_SUN ) {
        if ( --result_sun < 0 ) {
            result_sun +=
                ::Random.randI( lt_hemisamples / 4, lt_hemisamples / 2 );
            Fvector direction;
            direction.set( sun->direction ).invert().normalize();
            sun_value = !( g_pGameLevel->ObjectSpace.RayTest(
                            position, direction, 500.f, collide::rqtBoth,
                            &cache_sun, _object ) )
                            ? 1.f
                            : 0.f;
        }
...
void CROS_impl::calc_sky_hemi_value( Fvector& position, CObject* _object ) {
    ...
    if ( MODE & IRender_ObjectSpecific::TRACE_HEMI ) {
        ...
        for ( u32 it = 0; it < ( u32 )ps_r2_dhemi_count; it++ ) {
            ...
            Fvector direction;
            direction
                .set( hdir[ sample ][ 0 ], hdir[ sample ][ 1 ],
                      hdir[ sample ][ 2 ] )
                .normalize();
            //.			result[sample]	=
            ...
            result[ sample ] = !g_pGameLevel->ObjectSpace.RayTest(
                ...
                _object );
            //	Msg				("%d:--
            //%s",sample,result[sample]?"true":"false");
        }
    }
    // hemi & sun: update and smooth
    ...
    for ( int it = 0; it < result_count; it++ ) {
        if ( result[ it ] ) {
            accum_hemi( hemi_cube,
                        Fvector3().set( hdir[ it ][ 0 ], hdir[ it ][ 1 ],
                                        hdir[ it ][ 2 ] ),
                        ps_r2_dhemi_sky_scale );
        }
    }
}
...
void CROS_impl::prepare_lights( Fvector& position, IRenderable* O ) {
    ...
    if ( ( !O->renderable_ShadowGenerate() ) &&
    ...
    if ( bTraceLights ) {
...
#if RENDER != R_R1
        g_SpatialSpaceLights->q_box( lstSpatial, 0, STYPE_LIGHTSOURCEHEMI,
                                     position, bb_size );
#else
        g_SpatialSpaceLights->q_box( lstSpatial, 0, STYPE_LIGHTSOURCE, position,
                                     bb_size );
#endif
        ...
        for ( s32 id = 0; id < s32( track.size() ); id++ ) {
...
#if RENDER == R_R1
            P.mad( position, P.random_dir(),
                   traceR ); // Random point inside range
#else
            P = position;
#endif
            ...
            if ( g_pGameLevel->ObjectSpace.RayTest( LP, D.div( f ), f,
                                                    collide::rqtStatic,
                                                    &I->cache, _object ) )
                amount -= lt_dec;
            else
        ...
        std::sort(
            ...
            []( const CROS_impl::Light& L1, const CROS_impl::Light& L2 ) {
