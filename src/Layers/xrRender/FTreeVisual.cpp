...
void FTreeVisual::Load( const char* N, IReader* data, u32 dwFlags ) {
    ...
    R_ASSERT( data->find_chunk( OGF_TREEDEF2 ) );
    {
        ...
        c_bias.sun *= .5f;
        // Msg				("hemi[%f / %f], sun[%f /
        // %f]",c_scale.hemi,c_bias.hemi,c_scale.sun,c_bias.sun);
    }
...
void FTreeVisual::Render( float LOD ) {
...
#if RENDER != R_R1
    s *= 1.3333f;
    RCache.tree.set_c_scale( s * c_scale.rgb.x, s * c_scale.rgb.y,
                             s * c_scale.rgb.z, s * c_scale.hemi ); // scale
    RCache.tree.set_c_bias( s * c_bias.rgb.x, s * c_bias.rgb.y,
                            s * c_bias.rgb.z, s * c_bias.hemi ); // bias
#else
    CEnvDescriptor& desc = *g_pGamePersistent->Environment().CurrentEnv;
    RCache.tree.set_c_scale( s * c_scale.rgb.x, s * c_scale.rgb.y,
                             s * c_scale.rgb.z, s * c_scale.hemi ); // scale
    RCache.tree.set_c_bias(
        s * c_bias.rgb.x + desc.ambient.x, s * c_bias.rgb.y + desc.ambient.y,
    ...
    if ( ps_ssfx_grass_interactive.y > 0 ) {
        ...
        {
            void* GrassData;
            RCache.get_ConstantDirect( c_c_BendersPos,
                                       BendersQty * sizeof( Fvector4 ) * 2,
                                       &GrassData, 0, 0 );
        ...
        if ( c_grass ) {
            ...
            for ( int Bend = 1; Bend < BendersQty; Bend++ ) {
                c_grass[ Bend ].set( GData.pos[ Bend ].x, GData.pos[ Bend ].y,
                                     GData.pos[ Bend ].z,
        ...
        {
            void* prevGrassData;
            RCache.get_ConstantDirect( c_c_PrevBendersPos,
                                       BendersQty * sizeof( Fvector4 ) * 2,
                                       &prevGrassData, 0, 0 );
...
void FTreeVisual_PM::Render( float LOD ) {
    ...
    RCache.set_Geometry( rm_geom );
    RCache.Render( D3DPT_TRIANGLELIST, vBase, 0, SW.num_verts,
                   iBase + SW.offset, SW.num_tris );
