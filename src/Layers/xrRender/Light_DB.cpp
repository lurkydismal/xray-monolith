...
CLight_DB::CLight_DB() : rain_light( nullptr ), m_prepared( false ) {}

CLight_DB::~CLight_DB() {
    if ( m_prepared || rain_light || sun_original || sun_adapted ||
         !v_static.empty() || !v_hemi.empty() )
        Unload();
}
...
void CLight_DB::LoadDynamic( IReader& reader, bool publish ) {
    ...
    if (0)
    {
            Fvector	P;			P.set(-5.58f,	-0.00f + 2,
    -3.63f); Fvector	D;			D.set(0,-1,0); light*	fake
    = Create(); fake->set_type		(IRender_Light::SPOT); fake->set_color
    (1,1,1); fake->set_cone		(deg2rad(60.f)); fake->set_direction
    (D); fake->set_position	(P); fake->set_range		(3.f);
...
void CLight_DB::LoadHemi( IReader& reader, bool publish ) {
    ...
    for ( u32 i = 0; i < count; i++ ) {
        ...
        if ( publish )
            L->set_active( true );
        L->set_attenuation_params( Ldata.attenuation0, Ldata.attenuation1,
                                   Ldata.attenuation2, Ldata.falloff );
        L->SpatialComponent->spatial.type = STYPE_LIGHTSOURCEHEMI;
    }
}
#endif

void CLight_DB::LoadPrepared( const xr_vector< u8 >& dynamic,
                              const xr_vector< u8 >& hemi ) {
    Prepare( dynamic, hemi );
    CommitPrepared();
}

void CLight_DB::Prepare( const xr_vector< u8 >& dynamic,
                         const xr_vector< u8 >& hemi ) {
    R_ASSERT( !dynamic.empty() );
    R_ASSERT( !m_prepared && v_static.empty() && v_hemi.empty() &&
              !sun_original && !sun_adapted && !rain_light );
    m_prepared = true;
    IReader dynamic_reader( const_cast< u8* >( dynamic.data() ),
                            static_cast< int >( dynamic.size() ) );
    LoadDynamic( dynamic_reader, false );
#if RENDER != R_R1
    if ( !hemi.empty() ) {
        IReader hemi_reader( const_cast< u8* >( hemi.data() ),
                             static_cast< int >( hemi.size() ) );
...
void CLight_DB::add_light( light* L ) {
    ...
    if ( Device.vCameraPosition.distance_to_sqr(
             L->SpatialComponent->spatial.sphere.P ) >
         _sqr( g_pGamePersistent->Environment().CurrentEnv->fog_distance ) )
        return;
...
void CLight_DB::Update() {
    ...
    if ( sun_original && sun_adapted ) {
        ...
        VERIFY2( E.sun_dir.y < 0,
                 "Invalid sun direction settings in evironment-config" );
        Fvector OD, OP, AD, AP;
        ...
        sun_adapted->set_color(
            E.sun_color.x * ps_r2_sun_lumscale * ps_r2_sun_lumscale_color.x,
