...
static u16 hbox_faces[ 20 * 3 ] = {
    0,  2, 3,  3,  1, 0, 4,  5,  7, 7, 6, 4,  0,  1, 9,  9, 8, 0, 8,  9,
...
const u32 v_skybox_fvf = D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX2 |
...
void dxEnvDescriptorMixerRender::Copy( IEnvDescriptorMixerRender& _in ) {
    ...
    clouds_r_textures.assign( other->clouds_r_textures.begin(),
                              other->clouds_r_textures.end() );
    clouds_r_textures.dwReference++;

    sky_r_textures.assign( other->sky_r_textures.begin(),
                           other->sky_r_textures.end() );
    sky_r_textures.dwReference++;

    sky_r_textures_env.assign( other->sky_r_textures_env.begin(),
                               other->sky_r_textures_env.end() );
...
dxEnvironmentRender::particles_systems_library() {
...
void dxEnvDescriptorMixerRender::lerp( IEnvDescriptorRender* inA,
...
void dxEnvDescriptorRender::OnDeviceCreate( CEnvDescriptor& owner ) {
    dxEnvDescriptorMixerRender& mixRen =
        *( dxEnvDescriptorMixerRender* )&*g_pGamePersistent->Environment()
             .CurrentEnv->m_pDescriptorMixer;
    ...
    if ( owner.sky_texture_env_name.size() ) {
        ...
        if ( g_pGamePersistent->Environment().m_paused ) {
            ...
            mixRen.sky_r_textures_env.push_back(
                mk_pair( 0, sky_texture_env ) );
            mixRen.sky_r_textures_env.push_back(
                mk_pair( 1, sky_texture_env ) );
        }
    }
...
void dxEnvironmentRender::OnFrame( CEnvironment& env ) {
    ...
    dxEnvDescriptorMixerRender& mixRen =
        *( dxEnvDescriptorMixerRender* )&*env.CurrentEnv->m_pDescriptorMixer;

    if ( ::Render->get_generation() == IRender_interface::GENERATION_R2 ) {
        //. very very ugly hack
        if ( HW.Caps.raster_major >= 3 && HW.Caps.geometry.bVTF ) {
            // tonemapping in VS
            mixRen.sky_r_textures.push_back(
                mk_pair( u32( D3DVERTEXTEXTURESAMPLER0 ), tonemap ) ); //. hack
            mixRen.sky_r_textures_env.push_back(
                mk_pair( u32( D3DVERTEXTEXTURESAMPLER0 ), tonemap ) ); //. hack
            mixRen.clouds_r_textures.push_back(
                mk_pair( u32( D3DVERTEXTEXTURESAMPLER0 ), tonemap ) ); //. hack
        } else {
            // tonemapping in PS
            mixRen.sky_r_textures.push_back( mk_pair( 2, tonemap ) ); //. hack
            mixRen.sky_r_textures_env.push_back(
                mk_pair( 2, tonemap ) ); //. hack
            mixRen.clouds_r_textures.push_back(
                mk_pair( 2, tonemap ) ); //. hack
        }
    }
    ...
    CHK_DX( HW.pDevice->SetRenderState(
        D3DRS_FOGCOLOR,
        color_rgba_f( fog_color.x, fog_color.y, fog_color.z, 0 ) ) );
    CHK_DX( HW.pDevice->SetRenderState(
        D3DRS_FOGSTART, *( u32* )( &env.CurrentEnv->fog_near ) ) );
    CHK_DX( HW.pDevice->SetRenderState(
        D3DRS_FOGEND, *( u32* )( &env.CurrentEnv->fog_far ) ) );
#endif //	USE_DX10
}
...
void dxEnvironmentRender::RenderSky( CEnvironment& env, bool OnlyMV ) {
    ...
    if ( env.bNeed_re_create_env ) {
        ...
        sh_2geom.create( v_skybox_fvf, RCache.Vertex.Buffer(),
                         RCache.Index.Buffer() );
        clouds_sh.create( "clouds", "null" );
        clouds_geom.create( v_clouds_fvf, RCache.Vertex.Buffer(),
                            RCache.Index.Buffer() );
    ...
    u32 C = color_rgba( iFloor( env.CurrentEnv->sky_color.x * 255.f ),
                        iFloor( env.CurrentEnv->sky_color.y * 255.f ),
                        iFloor( env.CurrentEnv->sky_color.z * 255.f ),
    ...
    // Fill vertex buffer
    v_skybox* pv =
        ( v_skybox* )RCache.Vertex.Lock( 12, sh_2geom.stride(), v_offset );
    for ( u32 v = 0; v < 12; v++ )
    ...
    if ( !OnlyMV ) {
        ...
        // The state may be not set by RCache if the state is changed using API
        // SetRenderState() function before and the RCache flag will remain
        // unchanged to it's old value.
        //
        RCache.set_Z( FALSE );
        RCache.set_Z( TRUE );
...
void dxEnvironmentRender::RenderClouds( CEnvironment& env ) {
    ...
    wind_dir.set( wd0.x, wd0.z, wd1.x, wd1.z )
        .mul( 0.5f )
        .add( 0.5f )
        .mul( 255.f );
    u32 i_offset, v_offset;
    u32 C0 = color_rgba( iFloor( wind_dir.x ), iFloor( wind_dir.y ),
                         iFloor( wind_dir.w ), iFloor( wind_dir.z ) );
    u32 C1 = color_rgba( iFloor( env.CurrentEnv->clouds_color.x * 255.f ),
                         iFloor( env.CurrentEnv->clouds_color.y * 255.f ),
                         iFloor( env.CurrentEnv->clouds_color.z * 255.f ),
                         iFloor( env.CurrentEnv->clouds_color.w * 255.f ) );

    // Fill index buffer
    u16* pib = RCache.Index.Lock( env.CloudsIndices.size(), i_offset );
    CopyMemory( pib, &env.CloudsIndices.front(),
                env.CloudsIndices.size() * sizeof( u16 ) );
    RCache.Index.Unlock( env.CloudsIndices.size() );

    // Fill vertex buffer
    v_clouds* pv = ( v_clouds* )RCache.Vertex.Lock(
        env.CloudsVerts.size(), clouds_geom.stride(), v_offset );
    for ( FvectorIt it = env.CloudsVerts.begin(); it != env.CloudsVerts.end();
          it++, pv++ )
        pv->set( *it, C0, C1 );
    ...
    RCache.Render( D3DPT_TRIANGLELIST, v_offset, 0, env.CloudsVerts.size(),
                   i_offset, env.CloudsIndices.size() / 3 );
