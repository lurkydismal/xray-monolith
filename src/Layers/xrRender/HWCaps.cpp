...
u32 GetNVGpuNum() {
    ...
    Msg( "* NVidia MGPU: Logical(%d), Physical(%d)", physicalGPUCount,
         logicalGPUCount );
...
void CHWCaps::Update() {
    ...
    // ***************** GEOMETRY
    geometry_major =
        u16( ( u32( caps.VertexShaderVersion ) & ( 0xf << 8ul ) ) >> 8 );
    ...
    geometry.bVTF =
        ( geometry_major >= 3 ) && HW.support( D3DFMT_R32F, D3DRTYPE_TEXTURE,
                                               D3DUSAGE_QUERY_VERTEXTEXTURE );

    // ***************** PIXEL processing
    raster_major =
        u16( u32( u32( caps.PixelShaderVersion ) & u32( 0xf << 8ul ) ) >> 8 );
    ...
    Msg( "* GPU shading: vs(%x/%d.%d/%d), ps(%x/%d.%d/%d)",
         ...
         CAP_VERSION( geometry_major, geometry_minor ), caps.PixelShaderVersion,
    ...
    Msg( "* GPU vertex cache: %s, %d",
         ( 1 == vc.OptMethod ) ? "recognized" : "unrecognized",
         u32( geometry.dwVertexCache ) );
    ...
    if ( ( !( dwStencilCaps & D3DSTENCILCAPS_INCR ) &&
        ...
        // Prefer sat ops that cap at 0/max, but can use other ones as long as
        // enough stencil bits
        soInc = ( dwStencilCaps & D3DSTENCILCAPS_INCRSAT )
                    ? D3DSTENCILOP_INCRSAT
                    : D3DSTENCILOP_INCR;
        soDec = ( dwStencilCaps & D3DSTENCILCAPS_DECRSAT )
...
void CHWCaps::Update() {
    ...
    // ***************** Info
    Msg( "* GPU shading: vs(%x/%d.%d/%d), ps(%x/%d.%d/%d)", 0, geometry_major,
         geometry_minor, CAP_VERSION( geometry_major, geometry_minor ), 0,
         raster_major, raster_minor,
         CAP_VERSION( raster_major, raster_minor ) );
    ...
    Msg( "* GPU vertex cache: %s, %d", "unrecognized",
         u32( geometry.dwVertexCache ) );
