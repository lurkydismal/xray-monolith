...
public:
    void compute_planes() {
        for ( u32 it = 0; it < LIGHT_CUBOIDSIDEPOLYS_COUNT; it++ ) {
            ...
            if ( _debug ) {
                ...
                Fplane p301;
                p301.build( p3, p0, p1 );
                VERIFY( p012.n.similar( p123.n ) && p012.n.similar( p230.n ) &&
                        p012.n.similar( p301.n ) );
            }
        }
    }
    ...
    void compute_caster_model_fixed( xr_vector< Fplane >& dest,
        ...
        if ( fis_zero( 1 - abs( view_ray.D.dotproduct( light_ray.D ) ),
                       EPS_S ) )
            return;
        ...
        for ( u32 i = 0; i < LIGHT_CUBOIDSIDEPOLYS_COUNT; i++ ) {
            float tmp_dot =
                view_ray.D.dotproduct( light_cuboid_polys[ i ].plane.n );
            if ( tmp_dot <= EPS_L )
                continue;
        ...
        for ( int p = 0; p < align_planes_count; ++p ) {
            ...
            for ( u32 i = 0; i < view_frustum_rays.size(); ++i ) {
                ...
                tmp_dist =
                    light_cuboid_polys[ align_planes[ p ] ].plane.classify(
                        tmp_point );
        ...
        // Here we can skip this stage us in the next pass we need only normals
        // of planes. in the next translate_light_model call will contain this
        // shift as well. translate_light_model	( align_vector );
        ...
        for ( int p = 0; p < align_planes_count; ++p ) {
            float max_mag = 0;
            for ( u32 i = 0; i < view_frustum_rays.size(); ++i ) {
                float plane_dot_ray = view_frustum_rays[ i ].D.dotproduct(
                    light_cuboid_polys[ align_planes[ p ] ].plane.n );
                if ( plane_dot_ray < 0 ) {
                    Fvector per_plane_view;
                    per_plane_view.crossproduct(
                        light_cuboid_polys[ align_planes[ p ] ].plane.n,
                        view_ray.D );
                    Fvector per_view_to_plane;
                    per_view_to_plane.crossproduct( per_plane_view,
                                                    view_ray.D );

                    float tmp_mag =
                        -plane_dot_ray / view_frustum_rays[ i ].D.dotproduct(
                                             per_view_to_plane );
            ...
            float dist =
                -light_cuboid_polys[ align_planes[ p ] ].plane.n.dotproduct(
                    translation );
            align_vector.mad( light_cuboid_polys[ align_planes[ p ] ].plane.n,
                              dist * max_mag );
        }
        ...
        if ( clip_by_view_near &&
             abs( view_ray.D.dotproduct( light_ray.D ) ) < 0.8 ) {
            ...
            float max_dist = -1000;
            for ( u32 i = 0; i < view_frustum_rays.size(); ++i )
                max_dist = _max( plane.classify( view_frustum_rays[ i ].P ),
                                 max_dist );
        ...
        // Compute ray intersection with light model, this is needed to next
        // cascade to start it's placement.
        for ( u32 i = 0; i < view_frustum_rays.size(); ++i ) {
            float min_dist = 2 * map_size;
            for ( int p = 0; p < 4; ++p ) {
                float dist;
                if ( ( light_cuboid_polys[ p ].plane.n.dotproduct(
                         view_frustum_rays[ i ].D ) ) > -0.1 )
                    dist = map_size;
                else
                    light_cuboid_polys[ p ].plane.intersectRayDist(
                        view_frustum_rays[ i ].P, view_frustum_rays[ i ].D,
                        dist );
    ...
    bool check_cull_plane_valid( Fplane const& plane,
                                 float& sign,
                                 float mad_factor = 0.f ) {
        bool valid = false;
        bool oriented = false;
        float orient = 0;
    ...
    void translate_light_model( Fvector translate ) {
        ...
        for ( int i = 0; i < LIGHT_CUBOIDSIDEPOLYS_COUNT; ++i )
            light_cuboid_polys[ i ].plane.d -=
                translate.dotproduct( light_cuboid_polys[ i ].plane.n );
    }
};
    ...
    void compute_caster_model( xr_vector< Fplane >& dest, Fvector3 direction ) {
        ...
        for ( int it = 0; it < int( polys.size() ); it++ ) {
            ...
            for ( int p = 0; p < int( plist.size() ); p++ ) {
                _edge E( plist[ p ], plist[ ( p + 1 ) % plist.size() ],
                         marker );
                bool found = false;
                ...
                if ( !found ) {
                    edges.push_back( E );
                    if ( _debug )
                        T.dbg_addline( points[ E.p0 ], points[ E.p1 ],
        ...
        // Extend model to infinity, the volume is not capped, so this is indeed
        // up to infinity
        for ( int e = 0; e < int( edges.size() ); e++ ) {
            ...
            if ( _debug )
                T.dbg_addline( points[ E.p0 ],
                               point.mad( points[ E.p0 ], direction, -1000 ),
                               color_rgba( 0, 255, 0, 255 ) );
            if ( _debug )
                T.dbg_addline( points[ E.p1 ],
                               point.mad( points[ E.p1 ], direction, -1000 ),
                               color_rgba( 0, 255, 0, 255 ) );
        }
