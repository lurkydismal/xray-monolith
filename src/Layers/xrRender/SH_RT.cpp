...
void CRT::create( LPCSTR Name, u32 w, u32 h, D3DFORMAT f, u32 SampleCount ) {
    ...
    _hr = HW.pD3D->CheckDeviceFormat( HW.DevAdapter, HW.DevT, HW.Caps.fTarget,
                                      usage, D3DRTYPE_TEXTURE, f );
    if ( FAILED( _hr ) )
        return;

    // Try to create texture/surface
    DEV->Evict();
    _hr = HW.pDevice->CreateTexture( w, h, 1, usage, f, D3DPOOL_DEFAULT,
                                     &pSurface, NULL );
...
//RDEVICE.GetTimerGlobal()->GetElapsed_clk();
