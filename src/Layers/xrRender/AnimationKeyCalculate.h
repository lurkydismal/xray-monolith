...
IC bool operator<( const ConsistantKey& A,
                   const ConsistantKey& B ) // note: inverse operator
{
    return A.w > B.w;
}
...
IC void Dequantize( CKey& K, const CBlend& BD, const CMotion& M ) {
    ...
    if ( M.test_flag( flTKeyPresent ) ) {
        ...
        */
        ...
        /*
        if ((_abs(D->T.y)>10000) || (_abs(D->T.x)>10000) ||
        (_abs(D->T.z)>10000))
        {
        Log("xxx");
        Log("Blend--------");
...
IC void MixInterlerp( CKey& Result,
                      const CKey* R,
    ...
    switch ( b_count ) {
        ...
        case 1:
            ...
            Log("Bone",LL_BoneName_dbg(SelfID));
            Msg("Result.Q
            %f,%f,%f,%f",Result.Q.x,Result.Q.y,Result.Q.z,Result.Q.w);
...
IC void q_scale_vs_basem( Fmatrix& m_res,
                          const Fquaternion& q,
                          const Fquaternion& base,
                          float scale_factor ) {
    Fmatrix mb, imb;
    mb.rotation( base );
    imb.invert( mb );
...
IC void q_add_scaled_basem( Fquaternion& q,
                            const Fquaternion& base,
                            const Fquaternion& q0,
                            const Fquaternion& q1,
                            float v1 ) {
    // VERIFY(0.f =< v && 1.f >= v );
    Fmatrix m0;
    m0.rotation( q0 );
...
IC void process_single_channel( CKey& Result,
...
IC void MixChannels( CKey& Result,
                     const CKey* R,
                     const animation::channel_def* BA,
                     int b_count ) {
    VERIFY( b_count > 0 );
    Result = R[ 0 ];
    ...
    for ( int i = 1; i < b_count; i++ )
        switch ( BA[ i ].rule.extern_ ) {
            ...
            case animation::lerp:
                ...
                KEY_Interp( Result, CKey( Result ), R[ i ],
                            BA[ i ].factor / lerp_factor_sum );
                break;
            default:
                NODEFAULT;
        }
