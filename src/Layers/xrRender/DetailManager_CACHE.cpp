            ...
            for ( int _z = 0; _z < dm_cache1_count; _z++ )
                for ( int _x = 0; _x < dm_cache1_count; _x++ )
                    MS.slots[ _z * dm_cache1_count + _x ] =
                        &cache[ _mz1 * dm_cache1_count + _z ]
                              [ _mx1 * dm_cache1_count + _x ];
        }
    }
}
...
void CDetailManager::cache_Update( int v_x,
                                   int v_z,
                                   Fvector& view,
                                   int limit ) {
    ...
    if ( ps_r2_ls_flags.test( R2FLAG_FAST_DETAILS_UPDATE ) ) {
        ...
        for ( int iteration = 0; cache_task.size() && ( iteration < limit );
              iteration++ ) {
            u32 best_id = 0;
    ...
    if ( bNeedMegaUpdate ) {
        for ( u32 _mz1 = 0; _mz1 < dm_cache1_line; _mz1++ ) {
            for ( u32 _mx1 = 0; _mx1 < dm_cache1_line; _mx1++ ) {
                ...
                MS.empty = TRUE;
                MS.vis.clear();
                for ( int _i = 0; _i < dm_cache1_count * dm_cache1_count;
                      _i++ ) {
                    Slot* PS = *MS.slots[ _i ];
                    Slot& S = *PS;
...
DetailSlot& CDetailManager::QueryDB( int sx, int sz ) {
    ...
    if ( ( db_x >= 0 ) && ( db_x < int( dtH.size_x ) ) && ( db_z >= 0 ) &&
         ( db_z < int( dtH.size_z ) ) ) {
