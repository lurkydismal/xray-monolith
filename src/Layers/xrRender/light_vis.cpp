...
void light::vis_prepare() {
    ...
    // Msg	("sc[%f,%f,%f]/c[%f,%f,%f] -
    // sr[%f]/r[%f]",VPUSH(spatial.center),VPUSH(position),spatial.radius,range);
    // Msg	("dist:%f,
    // sa:%f",Device.vCameraPosition.distance_to(spatial.center),safe_area);
    bool skiptest = false;
    if ( ps_r2_ls_flags.test( R2FLAG_EXP_DONT_TEST_UNSHADOWED ) &&
         !flags.bShadow )
        skiptest = true;
    if ( ps_r2_ls_flags.test( R2FLAG_EXP_DONT_TEST_SHADOWED ) && flags.bShadow )
        skiptest = true;
#ifdef USE_DX11
    // if (ps_ssfx_volumetric.x > 0 && flags.bShadow) skiptest = true; // Temp
    // Fix
#endif

    vis.distance = Device.vCameraPosition.distance_to(
        SpatialComponent->spatial.sphere.P );

    if ( skiptest ||
         vis.distance <=
             ( SpatialComponent->spatial.sphere.R * 1.01f + safe_area +
               ( SpatialComponent->spatial.sphere.R * 0.1f ) ) ) // small error
    {
        // small error
        vis.visible = true;
        vis.pending = false;
        vis.frame2test =
            frame + ::Random.randI( delay_small_min, delay_small_max );
        return;
    }
...
void light::vis_update() {
    ...
    HRESULT hr = GetData( vis.Q, &fragments, sizeof( fragments ),
                          0x1 /*D3D11_ASYNC_GETDATA_DONOTFLUSH*/ );
    ...
    vis.frame2test =
        vis.visible
            ? ( frame + ::Random.randI( delay_large_min, delay_large_max ) )
            : ( frame + 1 );
}
