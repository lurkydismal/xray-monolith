...
float errN( Fvector3 v, u8* qv ) {
    Fvector3 uv;
    uv.set( float( qv[ 0 ] ), float( qv[ 1 ] ), float( qv[ 2 ] ) )
        .div( 255.f )
        .mul( 2.f )
        .sub( 1.f );
    uv.normalize();
...
static D3DVERTEXELEMENT9 dwDecl_01W[] = // 36bytes
    { { 0, 0, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION,
        0 }, // P : 2 : -12..+12
      { 0, 16, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_NORMAL,
        0 }, // N, w=index(RC, 0..1) : 1 : -1..+1
      { 0, 20, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT,
        D3DDECLUSAGE_TANGENT, 0 }, // T : 1 : -1..+1
      { 0, 24, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT,
        D3DDECLUSAGE_BINORMAL, 0 }, // B : 1 : -1..+1
      { 0, 28, D3DDECLTYPE_FLOAT2, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD,
        0 }, // tc : 1 : -16..+16
      D3DDECL_END() };
...
struct vertHW_1W {
    ...
    void get_pos_bones( Fvector& p, CKinematics* Parent ) const {
        const Fmatrix& xform =
            Parent->LL_GetBoneInstance( get_bone() ).mRenderTransform;
        get_pos( p );
...
static D3DVERTEXELEMENT9 dwDecl_2W[] = // 44bytes
    { { 0, 0, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION,
      ...
      { 0, 24, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT,
        D3DDECLUSAGE_BINORMAL, 0 }, // B : 1 : -1..+1
      { 0, 28, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD,
        0 },
...
struct vertHW_2W {
    ...
    void set( Fvector3& P,
        ...
        _P[ 2 ] = P.z;
        _P[ 3 ] = 1.f;
        _N_w = color_rgba( q_N( N.x ), q_N( N.y ), q_N( N.z ),
                           u8( clampr( iFloor( w * 255.f + .5f ), 0, 255 ) ) );
        _T = color_rgba( q_N( T.x ), q_N( T.y ), q_N( T.z ), 0 );
        _B = color_rgba( q_N( B.x ), q_N( B.y ), q_N( B.z ), 0 );
        _tc_i[ 0 ] = tc.x;
        _tc_i[ 1 ] = tc.y;
    ...
    void get_pos_bones( Fvector& p, CKinematics* Parent ) const {
        Fvector P0, P1;
        Fmatrix& xform0 =
            Parent->LL_GetBoneInstance( get_bone( 0 ) ).mRenderTransform;
        Fmatrix& xform1 =
            Parent->LL_GetBoneInstance( get_bone( 1 ) ).mRenderTransform;
        get_pos( P0 );
...
struct vertHW_3W {
    ...
    void set( Fvector3& P,
        ...
        _P[ 3 ] = 1.f;
        _N_w = color_rgba( q_N( N.x ), q_N( N.y ), q_N( N.z ),
                           u8( clampr( iFloor( w0 * 255.f + .5f ), 0, 255 ) ) );
        _T_w = color_rgba( q_N( T.x ), q_N( T.y ), q_N( T.z ),
                           u8( clampr( iFloor( w1 * 255.f + .5f ), 0, 255 ) ) );
    ...
    void get_pos_bones( Fvector& p, CKinematics* Parent ) const {
        ...
        Fmatrix& xform1 =
            Parent->LL_GetBoneInstance( get_bone( 1 ) ).mRenderTransform;
        Fmatrix& xform2 =
            Parent->LL_GetBoneInstance( get_bone( 2 ) ).mRenderTransform;
        get_pos( P0 );
        xform0.transform_tiny( P0 );
        get_pos( P1 );
        xform1.transform_tiny( P1 );
...
struct vertHW_4W {
    ...
    void set( Fvector3& P,
        ...
        _T_w = color_rgba( q_N( T.x ), q_N( T.y ), q_N( T.z ),
                           u8( clampr( iFloor( w1 * 255.f + .5f ), 0, 255 ) ) );
        _B_w = color_rgba( q_N( B.x ), q_N( B.y ), q_N( B.z ),
                           u8( clampr( iFloor( w2 * 255.f + .5f ), 0, 255 ) ) );
        _tc[ 0 ] = tc.x;
        _tc[ 1 ] = tc.y;
        _i = color_rgba( u8( index0 ), u8( index1 ), u8( index2 ),
                         u8( index3 ) );
    }
    ...
    void get_pos_bones( Fvector& p, CKinematics* Parent ) const {
        ...
        for ( u16 i = 0; i < 4; ++i ) {
            Fmatrix& xform =
                Parent->LL_GetBoneInstance( get_bone( i ) ).mRenderTransform;
            get_pos( P[ i ] );
...
void CSkeletonX_ext::_Load_hw( Fvisual& V, void* _verts_ ) {
    ...
    switch ( RenderMode ) {
        case RM_SKINNING_SOFT:
            ...
            V.rm_geom.create( vertRenderFVF, RCache.Vertex.Buffer(),
                              V.p_rm_Indices );
            break;
        case RM_SINGLE:
        case RM_SKINNING_1B: {
            ...
            // R_CHK
            // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_1W*
            // dst	= (vertHW_1W*)bytes; vertBoned1W*	src =
            // (vertBoned1W*)_verts_; for (u32 it=0; it<V.vCount; it++)	{
            ...
            R_CHK( dx10BufferUtils::CreateVertexBuffer(
                &V.p_rm_Vertices, dstOriginal, V.vCount * vStride ) );
        ...
        case RM_SKINNING_2B: {
            ...
            // (HW.pDevice->CreateVertexBuffer(V.vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&V.p_rm_Vertices,0));
            // R_CHK
            // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_2W* dst
            // = (vertHW_2W*)bytes; vertBoned2W* src	= (vertBoned2W*)_verts_;

            // for (u32 it=0; it<V.vCount; ++it)
            //{
            //	Fvector2	uv; uv.set(src->u,src->v);
            //	dst->set
            //(src->P,src->N,src->T,src->B,uv,int(src->matrix0)*3,int(src->matrix1)*3,src->w);
            //	dst++;		src++;
            // }
            // V.p_rm_Vertices->Unlock	();
            ...
            for ( u32 it = 0; it < V.vCount; it++ ) {
                ...
                dst->set( src->P, src->N, src->T, src->B, uv,
                          int( src->matrix0 ) * 3, int( src->matrix1 ) * 3,
                          src->w );
                dst++;
                src++;
            }
        ...
        case RM_SKINNING_3B: {
            ...
            // (HW.pDevice->CreateVertexBuffer(V.vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&V.p_rm_Vertices,0));
            // R_CHK
            // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_3W* dst
            // = (vertHW_3W*)bytes; vertBoned3W* src		=
            // (vertBoned3W*)_verts_;
            ...
            //	Fvector2	uv; uv.set(src->u,src->v);
            //	dst->set
            //(src->P,src->N,src->T,src->B,uv,int(src->m[0])*3,int(src->m[1])*3,int(src->m[2])*3,src->w[0],src->w[1]);
            //	dst++;
            //	src++;
            // }
            // V.p_rm_Vertices->Unlock	();
            ...
            for ( u32 it = 0; it < V.vCount; it++ ) {
                ...
                uv.set( src->u, src->v );
                dst->set( src->P, src->N, src->T, src->B, uv,
                          int( src->m[ 0 ] ) * 3, int( src->m[ 1 ] ) * 3,
                          int( src->m[ 2 ] ) * 3, src->w[ 0 ], src->w[ 1 ] );
                dst++;
                src++;
            }
            R_CHK( dx10BufferUtils::CreateVertexBuffer(
        ...
        case RM_SKINNING_4B: {
            ...
            // (HW.pDevice->CreateVertexBuffer(V.vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&V.p_rm_Vertices,0));
            // R_CHK
            // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_4W* dst
            // = (vertHW_4W*)bytes; vertBoned4W* src		=
            // (vertBoned4W*)_verts_;
            ...
            //	Fvector2	uv; uv.set(src->u,src->v);
            //	dst->set
            //(src->P,src->N,src->T,src->B,uv,int(src->m[0])*3,int(src->m[1])*3,int(src->m[2])*3,int(src->m[3])*3,src->w[0],src->w[1],src->w[2]);
            //	dst++;
            //	src++;
            // }
            // V.p_rm_Vertices->Unlock	();
...
void CSkeletonX_ext::_Load_hw( Fvisual& V, void* _verts_ ) {
    ...
    BOOL bSoft = HW.Caps.geometry.bSoftware;
    u32 dwUsage =
        /*D3DUSAGE_WRITEONLY |*/ ( bSoft ? D3DUSAGE_SOFTWAREPROCESSING : 0 );
    ...
    switch ( RenderMode ) {
        ...
        case RM_SKINNING_1B: {
            ...
            BYTE* bytes = 0;
            VERIFY( NULL == V.p_rm_Vertices );
            R_CHK( HW.pDevice->CreateVertexBuffer( V.vCount * vStride, dwUsage,
                                                   0, D3DPOOL_MANAGED,
                                                   &V.p_rm_Vertices, 0 ) );
            HW.stats_manager.increment_stats_vb( V.p_rm_Vertices );
...
static void verify_vertex( const vertex_type& v,
                           const Fvisual* V,
    ...
    for ( u8 i = 0; i < vertex_type::bones_count; ++i )
        if ( v.get_bone_id( i ) >= Parent->LL_BoneCount() ) {
            Msg( "v.get_bone_id(i): %d, Parent->LL_BoneCount() %d ",
                 v.get_bone_id( i ), Parent->LL_BoneCount() );
            Msg( "&v: %p, &V: %p, indices: %p", &v, V, indices );
            Msg( " iBase: %d, iCount: %d, V->iBase %d, V->iCount %d, V->vBase: "
                 "%d,  V->vCount  %d, vertex_idx: %d, idx: %d",
                 iBase, iCount, V->iBase, V->iCount, V->vBase, V->vCount,
                 vertex_idx, idx );
            Msg( " v.P: %s , v.N: %s, v.T: %s, v.B: %s",
                 get_string( v.P ).c_str(), get_string( v.N ).c_str(),
...
void CSkeletonX_ext::_CollectBoneFaces( Fvisual* V, u32 iBase, u32 iCount ) {
    ...
    indices = *m_Indices;
#else  //	USE_DX10
    R_CHK( V->p_rm_Indices->Lock( 0, V->dwPrimitives * 3, ( void** )&indices,
                                  D3DLOCK_READONLY ) );
#endif //	USE_DX10
...
#ifdef DEBUG
                    verify_vertex( v, V, Parent, iBase, iCount, indices,
                                   V->vBase + indices[ idx ], idx );
#endif
    ...
    !defined( USE_DX11 ) //	Don't use hardware buffers in DX10 since we
        ...
        case RM_SKINNING_1B: {
            ...
            R_CHK( V->p_rm_Vertices->Lock(
                V->vBase, V->vCount, ( void** )&vertices, D3DLOCK_READONLY ) );
            for ( u32 idx = 0; idx < iCount; idx++ ) {
...
#endif // USE_DX10	//	Don't use hardware buffers in DX10 since we
       // can't read them
}
...
BOOL CSkeletonX_ext::_PickBoneHW1W( IKinematics::pick_result& r,
                                    float dist,
                                    const Fvector& S,
                                    const Fvector& D,
                                    Fvisual* V,
...
BOOL CSkeletonX_ext::_PickBoneHW2W( IKinematics::pick_result& r,
                                    float dist,
                                    const Fvector& S,
                                    const Fvector& D,
                                    Fvisual* V,
...
BOOL CSkeletonX_ext::_PickBoneHW3W( IKinematics::pick_result& r,
                                    float dist,
                                    const Fvector& S,
                                    const Fvector& D,
                                    Fvisual* V,
...
BOOL CSkeletonX_ext::_PickBoneHW4W( IKinematics::pick_result& r,
                                    float dist,
                                    const Fvector& S,
                                    const Fvector& D,
                                    Fvisual* V,
...
/*
BOOL CSkeletonX_ext::_PickBoneHW1W		(Fvector& normal, float& dist,
const Fvector& S, const Fvector& D, Fvisual* V, u16* indices,
CBoneData::FacesVec& faces)
{
        vertHW_1W* vertices;
        CHK_DX
(V->p_rm_Vertices->Lock(V->vBase,V->vCount,(void**)&vertices,D3DLOCK_READONLY));
        ...
        for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end(); it++){
                ...
                for (u32 k=0; k<3; k++){
                        vertHW_1W& vert			=
vertices[indices[idx+k]]; vert.get_pos_bones(p[k],Parent);
                }
...
BOOL CSkeletonX_ext::_PickBoneHW2W		(Fvector& normal, float& dist,
const Fvector& S, const Fvector& D, Fvisual* V, u16* indices,
CBoneData::FacesVec& faces)
{
        vertHW_2W*			vertices;
...
BOOL CSkeletonX_ext::_PickBoneHW3W(Fvector& normal, float& dist, const Fvector&
S, const Fvector& D, Fvisual* V, u16* indices, CBoneData::FacesVec& faces)
{
        vertHW_3W*			vertices;
        CHK_DX
(V->p_rm_Vertices->Lock(V->vBase,V->vCount,(void**)&vertices,D3DLOCK_READONLY));
...
BOOL CSkeletonX_ext::_PickBoneHW4W(Fvector& normal, float& dist, const Fvector&
S, const Fvector& D, Fvisual* V, u16* indices, CBoneData::FacesVec& faces)
{
        vertHW_4W*			vertices;
        CHK_DX
...
BOOL CSkeletonX_ext::_PickBone( IKinematics::pick_result& r,
                                float dist,
                                const Fvector& start,
                                const Fvector& dir,
                                Fvisual* V,
    ...
    indices = *m_Indices;
#else  //	USE_DX10
    CHK_DX( V->p_rm_Indices->Lock( 0, V->dwPrimitives * 3, ( void** )&indices,
                                   D3DLOCK_READONLY ) );
    // fill vertices
    switch ( RenderMode ) {
        case RM_SKINNING_SOFT:
    ...
    if ( *Vertices1W )
        result =
            _PickBoneSoft1W( r, dist, start, dir, indices + iBase, *faces );
    else if ( *Vertices2W )
        result =
            _PickBoneSoft2W( r, dist, start, dir, indices + iBase, *faces );
    else if ( *Vertices3W )
        result =
            _PickBoneSoft3W( r, dist, start, dir, indices + iBase, *faces );
    else {
        VERIFY( !!( *Vertices4W ) );
        result =
            _PickBoneSoft4W( r, dist, start, dir, indices + iBase, *faces );
    }
    ...
    case RM_SKINNING_1B:
        result =
            _PickBoneHW1W( r, dist, start, dir, V, indices + iBase, *faces );
        break;
    case RM_SKINNING_2B:
        result =
            _PickBoneHW2W( r, dist, start, dir, V, indices + iBase, *faces );
        break;
    case RM_SKINNING_3B:
        result =
            _PickBoneHW3W( r, dist, start, dir, V, indices + iBase, *faces );
        break;
    case RM_SKINNING_4B:
        result =
            _PickBoneHW4W( r, dist, start, dir, V, indices + iBase, *faces );
        break;
    default:
        NODEFAULT;
}
...
BOOL CSkeletonX_ST::PickBone( IKinematics::pick_result& r,
                              float dist,
                              const Fvector& start,
                              const Fvector& dir,
                              u16 bone_id ) {
    return inherited2::_PickBone( r, dist, start, dir, this, bone_id, iBase,
                                  iCount );
}

BOOL CSkeletonX_PM::PickBone( IKinematics::pick_result& r,
                              float dist,
                              const Fvector& start,
                              const Fvector& dir,
                              u16 bone_id ) {
    FSlideWindow& SW = nSWI.sw[ 0 ];
    return inherited2::_PickBone( r, dist, start, dir, this, bone_id,
                                  iBase + SW.offset, SW.num_tris * 3 );
}
...
void CSkeletonX_PM::EnumBoneVertices( SEnumVerticesCallback& C, u16 bone_id ) {
    FSlideWindow& SW = nSWI.sw[ 0 ];
    inherited2::_EnumBoneVertices( C, this, bone_id, iBase + SW.offset,
                                   SW.num_tris * 3 );
}

#if defined( USE_DX10 ) || defined( USE_DX11 )

void CSkeletonX_ext::_FillVerticesHW1W( const Fmatrix& view,
...
void CSkeletonX_ext::_FillVerticesHW2W( const Fmatrix& view,
                                        CSkeletonWallmark& wm,
                                        const Fvector& normal,
                                        float size,
                                        Fvisual* V,
...
void CSkeletonX_ext::_FillVerticesHW3W( const Fmatrix& view,
                                        CSkeletonWallmark& wm,
                                        const Fvector& normal,
                                        float size,
                                        Fvisual* V,
...
void CSkeletonX_ext::_FillVerticesHW4W( const Fmatrix& view,
                                        CSkeletonWallmark& wm,
                                        const Fvector& normal,
                                        float size,
                                        Fvisual* V,
    ...
    void CSkeletonX_ext::_FillVerticesHW1W(
        const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal,
        float size, Fvisual* V, u16* indices, CBoneData::FacesVec& faces ) {
        vertHW_1W* vertices;
        CHK_DX( V->p_rm_Vertices->Lock(
            V->vBase, V->vCount, ( void** )&vertices, D3DLOCK_READONLY ) );
        for ( CBoneData::FacesVecIt it = faces.begin(); it != faces.end();
              it++ ) {
            Fvector p[ 3 ];
            u32 idx = ( *it ) * 3;
            ...
            for ( u32 k = 0; k < 3; k++ ) {
                ...
                const Fmatrix& xform =
                    Parent->LL_GetBoneInstance( F.bone_id[ k ][ 0 ] )
                        .mRenderTransform;
            ...
            for ( u32 k = 0; k < 3; k++ ) {
                ...
                Fmatrix& xform0 =
                    Parent->LL_GetBoneInstance( F.bone_id[ k ][ 0 ] )
                        .mRenderTransform;
                Fmatrix& xform1 =
                    Parent->LL_GetBoneInstance( F.bone_id[ k ][ 1 ] )
                        .mRenderTransform;
                vert.get_pos( F.vert[ k ] );
...
void CSkeletonX_ext::_FillVertices( const Fmatrix& view,
                                    CSkeletonWallmark& wm,
                                    const Fvector& normal,
                                    float size,
                                    Fvisual* V,
    ...
    indices = *m_Indices;
#else  //    USE_DX10
        CHK_DX( V->p_rm_Indices->Lock( 0, V->dwPrimitives * 3,
                                       ( void** )&indices, D3DLOCK_READONLY ) );
        // fill vertices
        switch ( RenderMode ) {
            case RM_SKINNING_SOFT:
#endif //    USE_DX10
...
void CSkeletonX_ST::FillVertices( const Fmatrix& view,
                                  CSkeletonWallmark& wm,
                                  const Fvector& normal,
                                  float size,
                                  u16 bone_id ) {
    inherited2::_FillVertices( view, wm, normal, size, this, bone_id, iBase,
                               iCount );
}

void CSkeletonX_PM::FillVertices( const Fmatrix& view,
                                  CSkeletonWallmark& wm,
                                  const Fvector& normal,
                                  float size,
                                  u16 bone_id ) {
    FSlideWindow& SW = nSWI.sw[ 0 ];
    inherited2::_FillVertices( view, wm, normal, size, this, bone_id,
                               iBase + SW.offset, SW.num_tris * 3 );
}
...
void CSkeletonX_ext::TEnumBoneVertices	( Vertices1W &verteses, u16 bone_id,
u16* indices, CBoneData::FacesVec& faces, SEnumVerticesCallback &C ) const
{
                for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end();
it++){ Fvector			p[3]; u32 idx			= (*it)*3; for
(u32 k=0; k<3; k++){ vertBoned1W& vert		= Vertices1W[indices[idx+k]];
                }}

}
void CSkeletonX_ext::TEnumBoneVertices	( Vertices2W &verteses, u16 bone_id,
u16* indices, CBoneData::FacesVec& faces, SEnumVerticesCallback &C ) const
{
                for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end();
it++){ Fvector			p[3]; u32 idx			= (*it)*3; for
(u32 k=0; k<3; k++){ Fvector		P0,P1; vertBoned2W& vert =
Vertices2W[indices[idx+k]];
                }}

}
void CSkeletonX_ext::TEnumBoneVertices	( vertHW_1W &verteses, u16 bone_id, u16*
indices, CBoneData::FacesVec& faces, SEnumVerticesCallback &C ) const
{
                for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end();
it++){ Fvector			p[3]; u32 idx			= (*it)*3;
                CSkeletonWallmark::WMFace F;
                for (u32 k=0; k<3; k++){
                        vertHW_1W& vert			=
vertices[indices[idx+k]];

}
void CSkeletonX_ext::TEnumBoneVertices	( vertHW_2W &verteses, u16 bone_id, u16*
indices, CBoneData::FacesVec& faces, SEnumVerticesCallback &C ) const
{
        for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end(); it++){
                Fvector			p[3];
...
IC void TEnumBoneVertices( vertex_buffer_type vertices,
                           u16* indices,
                           CBoneData::FacesVec& faces,
...
void CSkeletonX_ext::_EnumBoneVertices( SEnumVerticesCallback& C,
                                        Fvisual* V,
                                        u16 bone_id,
                                        u32 iBase,
                                        u32 iCount ) const {
    ...
    CBoneData::FacesVec* faces = &BD.child_faces[ ChildIDX ];
    u16* indices = 0;
    //.	R_CHK				(V->pIndices->Lock(iBase,iCount,
    //(void**)&indices,	D3DLOCK_READONLY));
    ...
    indices = *m_Indices;
            ...
            if ( RenderMode != RM_SKINNING_SOFT )
                CHK_DX( V->p_rm_Vertices->Lock( V->vBase, V->vCount,
                                                ( void** )&vertices,
                                                D3DLOCK_READONLY ) );
            switch ( RenderMode ) {
                case RM_SKINNING_SOFT:
