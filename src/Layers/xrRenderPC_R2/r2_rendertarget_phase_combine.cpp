...
void CRenderTarget::phase_combine() {
    ...
    if ( !_menu_pp ) {
        ...
        // RCache.Vertex.Lock	(4,g_combine_VP->vb_stride,Offset); pv->set
        // (hclip(EPS,		_w),	hclip(_h+EPS,	_h),	p0.x, p1.y);
        // pv++; pv->set
        // (hclip(EPS,		_w),	hclip(EPS,		_h),	p0.x,
        // p0.y);	pv++; pv->set
        // (hclip(_w+EPS,	_w),	hclip(_h+EPS,	_h),	p1.x, p1.y);
        // pv++; pv->set
        // (hclip(_w+EPS,	_w),	hclip(EPS,		_h),	p1.x,
        // p0.y);	pv++; RCache.Vertex.Unlock
        ...
        FVF::TL* pv = ( FVF::TL* )RCache.Vertex.Lock(
            4, g_combine_VP->vb_stride, Offset );
        pv->set( hclip( EPS, _w ), hclip( _h + EPS, _h ), p0.x, p1.y, 0, 0,
                 scale_Y );
        pv++;
        pv->set( hclip( EPS, _w ), hclip( EPS, _h ), p0.x, p0.y, 0, 0, 0 );
        pv++;
        pv->set( hclip( _w + EPS, _w ), hclip( _h + EPS, _h ), p1.x, p1.y, 0,
                 scale_X, scale_Y );
        pv++;
        pv->set( hclip( _w + EPS, _w ), hclip( EPS, _h ), p1.x, p0.y, 0,
                 scale_X, 0 );
        pv++;
    ...
    if ( 1 ) {
        ...
        // look at blender_combine.cpp
            ...
            RCache.set_Element(
                s_combine
                    ->E[ bDistort ? 4 : 2 ] ); // look at blender_combine.cpp
