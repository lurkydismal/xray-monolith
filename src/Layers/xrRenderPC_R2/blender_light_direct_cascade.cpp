...
void CBlender_accum_direct_cascade::Compile( CBlender_Compile& C ) {
    ...
    switch ( C.iElement ) {
        ...
        case SE_SUN_MIDDLE:
            C.r_Pass( "accum_volume", "accum_sun_cascade", false, TRUE, FALSE,
            ...
            // 			C.i_BorderColor	(s, D3DCOLOR_ARGB(255, 255, 255,
            // 255));
            // 		}
            C.r_End();
            break;
        case SE_SUN_FAR: // far pass, only stencil clipping performed
            C.r_Pass( "accum_volume", "accum_sun_cascade_far", false, TRUE,
