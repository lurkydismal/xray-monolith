...
BOOL tri_vs_sphere_intersect( Fvector& SC,
                              float R,
                              Fvector& v0,
                              Fvector& v1,
                              Fvector& v2 ) {
...
void CRenderTarget::enable_dbt_bounds( light* L ) {
    ...
    u32 mask = 0xffffffff;
    EFC_Visible vis = RImplementation.ViewBase.testSphere(
        L->SpatialComponent->spatial.sphere.P,
        L->SpatialComponent->spatial.sphere.R * 1.01f, mask );
    if ( vis != fcvFully )
        return;
    ...
    Fbox BB;
    Fvector rr;
    rr.set( L->SpatialComponent->spatial.sphere.R,
...
BOOL CRenderTarget::u_DBT_enable( float zMin, float zMax ) {
    ...
    // enable cheat
    HW.pDevice->SetRenderState( D3DRS_ADAPTIVETESS_X,
                                MAKEFOURCC( 'N', 'V', 'D', 'B' ) );
...
{
    // Msg	("%d: %x type(%d),
    // pos(%f,%f,%f)",Device.dwFrame,u32(L),u32(L->flags.type),VPUSH(L->position));
    ...
    BOOL near_intersect = FALSE;
    {
        ...
        P.d = plane.w;
        float p_dist = P.classify( L->SpatialComponent->spatial.sphere.P ) -
                       L->SpatialComponent->spatial.sphere.R;
    ...
    if ( 1 ) {
        Fsphere S;
        S.set( L->SpatialComponent->spatial.sphere.P,
               L->SpatialComponent->spatial.sphere.R );
    ...
    // Scissor
    //. disable scissor because some bugs prevent it to work through
    // multi-portals . if (!HW.Caps.bScissor)	return		near_intersect;
...
{
        Fmatrix& M						=
RCache.xforms.m_wvp; BOOL	bIntersect				= FALSE;
