...
void CRender::LoadBuffers( CStreamReader* base_fs, BOOL _alternative ) {
    ...
    // Vertex buffers
    {
        ...
        for ( u32 i = 0; i < count; i++ ) {
            ...
            D3DVERTEXELEMENT9* dcl =
                ( D3DVERTEXELEMENT9* )_alloca( buffer_size );
            ...
            R_CHK( HW.pDevice->CreateVertexBuffer(
                vCount * vSize, dwUsage, 0, D3DPOOL_MANAGED, &_VB[ i ], 0 ) );
            HW.stats_manager.increment_stats_vb( _VB[ i ] );
            R_CHK( _VB[ i ]->Lock( 0, 0, ( void** )&pData, 0 ) );
            //			CopyMemory
            //(pData,fs().pointer(),vCount*vSize);
    ...
    // Index buffers
    {
        ...
        for ( u32 i = 0; i < count; i++ ) {
            ...
            R_CHK( HW.pDevice->CreateIndexBuffer(
                iCount * 2, dwUsage, D3DFMT_INDEX16, D3DPOOL_MANAGED, &_IB[ i ],
                0 ) );
