...
CRenderTarget::CRenderTarget() {
    ...
    //	NORMAL
    {
        ...
        if ( RImplementation.o.mrtmixdepth ) {
            ...
            if ( RImplementation.o.fp16_blend ) {
                // NV40
                rt_Color.create( r2_RT_albedo, w, h,
                                 D3DFMT_A16B16G16R16F ); // expand to full
        ...
        rt_secondVP.create( r2_RT_secondVP, w, h,
                            D3DFMT_A8R8G8B8 ); //--#SM+#-- +SecondVP+
        ...
        R_CHK( HW.pDevice->CreateDepthStencilSurface(
            u32( w / 2 ), u32( h / 2 ), D3DFMT_D24S8, D3DMULTISAMPLE_NONE, 0,
            TRUE, &rt_blur_2_zb, NULL ) );
        ...
        rt_blur_4.create( r2_RT_blur_4, u32( w / 4 ), u32( h / 4 ),
                          D3DFMT_A8R8G8B8 );
        R_CHK( HW.pDevice->CreateDepthStencilSurface(
            u32( w / 4 ), u32( h / 4 ), D3DFMT_D24S8, D3DMULTISAMPLE_NONE, 0,
            TRUE, &rt_blur_4_zb, NULL ) );
        ...
        rt_blur_8.create( r2_RT_blur_8, u32( w / 8 ), u32( h / 8 ),
                          D3DFMT_A8R8G8B8 );
        R_CHK( HW.pDevice->CreateDepthStencilSurface(
            u32( w / 8 ), u32( h / 8 ), D3DFMT_D24S8, D3DMULTISAMPLE_NONE, 0,
            TRUE, &rt_blur_8_zb, NULL ) );
        ...
        rt_Generic_PingPong.create( "$user$generic_pingpong", w, h,
                                    D3DFMT_A8R8G8B8 );
    }
    ...
    if ( RImplementation.o.HW_smap ) {
        ...
        s_accum_direct_cascade.create( b_accum_direct_cascade,
                                       "r2\\accum_direct_cascade" );
        if ( RImplementation.o.advancedpp ) {
            s_accum_direct_volumetric.create( "accum_volumetric_sun" );
            s_accum_direct_volumetric_cascade.create(
                "accum_volumetric_sun_cascade" );
        }
    } else {
        u32 size = RImplementation.o.smapsize;
        ...
        rt_smap_depth = NULL;
        R_CHK( HW.pDevice->CreateDepthStencilSurface(
            size, size, D3DFMT_D24X8, D3DMULTISAMPLE_NONE, 0, TRUE, &rt_smap_ZB,
            NULL ) );
        s_accum_mask.create( b_accum_mask, "r2\\accum_mask" );
        s_accum_direct.create( b_accum_direct, "r2\\accum_direct" );
        s_accum_direct_cascade.create( b_accum_direct_cascade,
    ...
    // TONEMAP
    {
        ...
        for ( u32 it = 0; it < 2; it++ ) {
            ...
            CHK_DX( HW.pDevice->Clear( 0L, nullptr, D3DCLEAR_TARGET, 0x7f7f7f7f,
                                       1.0f, 0L ) );
        }
        u_setrt( Device.dwWidth, Device.dwHeight, HW.pBaseRT, nullptr, nullptr,
                 HW.pBaseZB );
    }
    ...
    // Build textures
    {
        // Build material(s)
        {
            ...
            R_CHK( D3DXCreateVolumeTexture(
                HW.pDevice, TEX_material_LdotN, TEX_material_LdotH, 4, 1, 0,
            ...
            for ( u32 slice = 0; slice < 4; slice++ ) {
                for ( u32 y = 0; y < TEX_material_LdotH; y++ ) {
                    for ( u32 x = 0; x < TEX_material_LdotN; x++ ) {
                        u16* p =
                            ( u16* )( LPBYTE( R.pBits ) + slice * R.SlicePitch +
                                      y * R.RowPitch + x * 2 );
        ...
        if ( 1 ) {
            ...
            for ( int it1 = 0; it1 < TEX_jitter_count - 1; it1++ ) {
                ...
                R_CHK( D3DXCreateTexture( HW.pDevice, TEX_jitter, TEX_jitter, 1,
                                          0, D3DFMT_Q8W8V8U8, D3DPOOL_MANAGED,
                                          &t_noise_surf[ it1 ] ) );
                t_noise[ it1 ] =
                    dxRenderDeviceRender::Instance().Resources->_CreateTexture(
                        name );
            ...
            R_CHK( D3DXCreateTexture( HW.pDevice, TEX_jitter, TEX_jitter, 1, 0,
            ...
            for ( u32 y = 0; y < TEX_jitter; y++ ) {
                for ( u32 x = 0; x < TEX_jitter; x++ ) {
                    ...
                    float* p = ( float* )( LPBYTE( R[ it ].pBits ) +
                                           y * R[ it ].Pitch +
                                           x * 4 * sizeof( float ) );
                    *p = ( float )( _cos( angle ) );
    ...
    HW.pDevice->CreateOffscreenPlainSurface( Device.dwWidth, Device.dwHeight,
...
void CRenderTarget::reset_light_marker( bool bResetStencil ) {
    ...
    if ( bResetStencil ) {
        ...
        RCache.set_Stencil( TRUE, D3DCMP_ALWAYS, dwLightMarkerID, 0x00, 0xFE,
                            D3DSTENCILOP_ZERO, D3DSTENCILOP_ZERO,
                            D3DSTENCILOP_ZERO );
        // RCache.set_Stencil	(TRUE,D3DCMP_ALWAYS,dwLightMarkerID,0x00,0xFF,
        ...
        /*
                        u32		Offset;
                        float	_w					=
           float(Device.dwWidth); float	_h = float(Device.dwHeight); u32
           C					= color_rgba
           (255,255,255,255); float	eps = 0; float	_dw
           = 0.5f; float	_dh					= 0.5f;
                        FVF::TL* pv					=
           (FVF::TL*) RCache.Vertex.Lock	(4,g_combine->vb_stride,Offset);
                        pv->set						(-_dw,
           _h-_dh,		eps,	1.f, C, 0, 0);	pv++; pv->set
           (-_dw,		-_dh,		eps,	1.f, C, 0, 0);	pv++;
                        pv->set						(_w-_dw,
           _h-_dh,		eps,	1.f, C, 0, 0);	pv++; pv->set
           (_w-_dw,	-_dh,		eps,	1.f, C, 0, 0);	pv++;
                        RCache.Vertex.Unlock (4,g_combine->vb_stride);
