...
void CRender::create() {
    ...
    */
    ...
    o.HW_smap =
        HW.support( D3DFMT_D24X8, D3DRTYPE_TEXTURE, D3DUSAGE_DEPTHSTENCIL );
    ...
    o.fp16_filter = HW.support( D3DFMT_A16B16G16R16F, D3DRTYPE_TEXTURE,
                                D3DUSAGE_QUERY_FILTER );
    o.fp16_blend = HW.support( D3DFMT_A16B16G16R16F, D3DRTYPE_TEXTURE,
    ...
    if ( ( HW.Caps.id_vendor == 0x10DE ) && ( HW.Caps.id_device >= 0x40 ) ) {
        // o.nvstencil = HW.support	((D3DFORMAT)MAKEFOURCC('R','A','W','Z'),
        // D3DRTYPE_SURFACE, 0); o.nvstencil = TRUE;
        o.nvstencil =
            ( S_OK == HW.pD3D->CheckDeviceFormat(
    ...
    // nv-dbt
    o.nvdbt = HW.support( ( D3DFORMAT )MAKEFOURCC( 'N', 'V', 'D', 'B' ),
                          D3DRTYPE_SURFACE, 0 );
    if ( o.nvdbt )
...
static HRESULT create_shader( LPCSTR const pTarget,
    ...
    if ( pTarget[ 0 ] == 'p' ) {
        ...
        LPCVOID data = NULL;
        _result = D3DXFindShaderComment(
            buffer, MAKEFOURCC( 'C', 'T', 'A', 'B' ), &data, NULL );
        if ( SUCCEEDED( _result ) && data ) {
            LPD3DXSHADER_CONSTANTTABLE pConstants =
                LPD3DXSHADER_CONSTANTTABLE( data );
...
HRESULT CRender::shader_compile( LPCSTR name,
    ...
    if ( FAILED( _result ) ) {
        ...
        if ( 0 == xr_strcmp( pFunctionName, "main" ) ) {
            if ( 'v' == pTarget[ 0 ] )
                pTarget = D3DXGetVertexShaderProfile(
                    HW.pDevice ); // vertex	"vs_2_a"; //
            else
                pTarget = D3DXGetPixelShaderProfile(
                    HW.pDevice ); // pixel	"ps_2_a"; //
        }
        ...
        _result = D3DXCompileShader( ( LPCSTR )pSrcData, SrcDataLen, defines,
                                     pInclude, pFunctionName, pTarget, Flags,
        ...
        if ( SUCCEEDED( _result ) ) {
            ...
            _result = create_shader(
                ...
                pShaderBuf->GetBufferSize(), file_name, result, o.disasm );
        } else {
            //			Msg
