...
void CRender::create() {
    ...
    */
    ...
    o.dx10_msaa_hybrid &=
        !o.dx10_msaa_opt && o.dx10_msaa && ( HW.pDevice1 != 0 );
...
#ifdef USE_DX11
        _result = D3DReflect( buffer, buffer_size, IID_ID3DShaderReflection,
                              ( void** )&pReflection );
#else
...
HRESULT CRender::shader_compile( LPCSTR name,
    ...
    sh_name[ len ] =
        '0' + char( o.ssao_opt_data ? ( o.ssao_half_data ? 2 : 1 ) : 0 );
    ++len;
    ...
    if ( 0 == xr_strcmp( pFunctionName, "main" ) ) {
        if ( 'v' == pTarget[ 0 ] ) {
            if ( HW.pDevice1 == 0 )
                pTarget = D3D10GetVertexShaderProfile(
                    HW.pDevice ); // vertex	"vs_4_0"; //
            else
                pTarget = "vs_4_1"; // pixel	"ps_4_0"; //
        } else if ( 'p' == pTarget[ 0 ] ) {
            if ( HW.pDevice1 == 0 )
                pTarget = D3D10GetPixelShaderProfile(
                    HW.pDevice ); // pixel	"ps_4_0"; //
            else
                pTarget = "ps_4_1"; // pixel	"ps_4_0"; //
        } else if ( 'g' == pTarget[ 0 ] ) {
            if ( HW.pDevice1 == 0 )
                pTarget = D3D10GetGeometryShaderProfile(
                    HW.pDevice ); // geometry	"gs_4_0"; //
            else
