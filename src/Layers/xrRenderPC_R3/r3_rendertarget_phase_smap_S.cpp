...
void CRenderTarget::phase_smap_spot_clear() {
    /*
    if (RImplementation.b_HW_smap)		u_setrt	(rt_smap_surf, NULL,
    NULL, rt_smap_d_depth->pRT); else
    u_setrt	(rt_smap_surf, NULL, NULL, rt_smap_d_ZB); CHK_DX
    (HW.pDevice->Clear( 0L, NULL, D3DCLEAR_ZBUFFER,	0xffffffff,	1.0f,
    0L));
    */

    HW.pDevice->ClearDepthStencilView( rt_smap_depth->pZRT, D3D10_CLEAR_DEPTH,
                                       1.0f, 0L );
}
...
void CRenderTarget::phase_smap_spot( light* L ) {
    ...
    // else								u_setrt
    // (rt_smap_surf, NULL, NULL, rt_smap_ZB);
    else
        VERIFY( !"Use HW SMap only for DX10!" );
    D3D_VIEWPORT VP = { L->X.S.posX, L->X.S.posY, L->X.S.size,
                        L->X.S.size, 0,           1 };
    // CHK_DX
...
#pragma todo( "can optimize for multi-lights covering more than say 50%..." )
    ...
    // CHK_DX
    // (HW.pDevice->Clear( 0L, NULL, D3DCLEAR_ZBUFFER,	0xffffffff,	1.0f,
    // 0L)); 	Do it once per smap generation pass in phase_smap_spot_clear
    // HW.pDevice->ClearDepthStencilView( rt_smap_depth->pZRT,
    // D3D10_CLEAR_DEPTH, 1.0f, 0L);
}
...
void CRenderTarget::phase_smap_spot_tsh( light* L ) {
    ...
    if ( IRender_Light::OMNIPART == L->flags.type ) {
        // omni-part
        // CHK_DX
        // (HW.pDevice->Clear( 0L, NULL, D3DCLEAR_TARGET,
        // 0xffffffff,	1.0f, 0L));
