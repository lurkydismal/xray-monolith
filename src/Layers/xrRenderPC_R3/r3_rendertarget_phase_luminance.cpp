...
void CRenderTarget::phase_luminance() {
    ...
    RCache.set_Z( FALSE );
    // CHK_DX
    // (HW.pDevice->SetRenderState	(D3DRS_ZENABLE,FALSE));
    ...
    // RImplementation.rmNormal();
    {
        ...
        for ( int t = 0; t < 8; t++ )
            pv->uv[ t ].set( a[ t ].x, b[ t ].y, b[ t + 8 ].y,
                             a[ t + 8 ].x ); // xy/yx	- left+down
        pv++;
        pv->p.set( eps, eps, eps, 1.f );
        for ( int t = 0; t < 8; t++ )
            pv->uv[ t ].set( a[ t ].x, a[ t ].y, a[ t + 8 ].y,
                             a[ t + 8 ].x ); // xy/yx	- left+up
        pv++;
        pv->p.set( float( _ts + eps ), float( _ts + eps ), eps, 1.f );
        for ( int t = 0; t < 8; t++ )
            pv->uv[ t ].set( b[ t ].x, b[ t ].y, b[ t + 8 ].y,
                             b[ t + 8 ].x ); // xy/yx	- right+down
        pv++;
        pv->p.set( float( _ts + eps ), eps, eps, 1.f );
        for ( int t = 0; t < 8; t++ )
            pv->uv[ t ].set( b[ t ].x, a[ t ].y, a[ t + 8 ].y,
                             b[ t + 8 ].x ); // xy/yx	- right+up
        pv++;
    ...
    // RImplementation.rmNormal();
    {
        ...
        f_luminance_adapt = .9f * f_luminance_adapt +
                            .1f * Device.fTimeDelta * ps_r2_tonemap_adaptation;
        float amount =
            ps_r2_ls_flags.test( R2FLAG_TONEMAP ) ? ps_r2_tonemap_amount : 0;
        ...
        RCache.set_Geometry( g_bloom_filter );
        RCache.set_c( "MiddleGray", _result.x, _result.y, _result.z,
                      f_luminance_adapt );
        RCache.Render( D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2 );
    }
