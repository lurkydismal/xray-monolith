...
void CBlender_accum_direct::Compile( CBlender_Compile& C ) {
    ...
    switch ( C.iElement ) {
        ...
        case SE_SUN_MIDDLE: // middle pass - enable Z-test to perform
                            // depth-clipping
            //	FVF::TL2uv
            C.r_Pass( "accum_sun", "accum_sun_near_nomsaa_nominmax", false,
                      TRUE, FALSE, blend, D3DBLEND_ONE, dest );
        ...
        case SE_SUN_FAR: // far pass, only stencil clipping performed
            //	FVF::TL2uv
            // C.r_Pass			("null",
            // "accum_sun_far",	false,	TRUE,	FALSE,blend,D3DBLEND_ONE,dest);
            C.r_Pass( "accum_sun", "accum_sun_far_nomsaa", false, TRUE, FALSE,
                      blend, D3DBLEND_ONE, dest );
            C.r_CullMode( D3DCULL_NONE );
        ...
        case SE_SUN_LUMINANCE: // luminance pass
            // C.r_Pass			("null",
            // "accum_sun",		false,	FALSE,	FALSE);
            C.r_Pass( "stub_notransform_aa_AA", "accum_sun_nomsaa", false,
                      FALSE, FALSE );
            C.r_CullMode( D3DCULL_NONE );
        ...
        case SE_SUN_NEAR_MINMAX: // near pass - enable Z-test to perform
                                 // depth-clipping
            //	FVF::TL2uv
            C.r_Pass( "accum_sun", "accum_sun_near_nomsaa_minmax", false, TRUE,
...
void CBlender_accum_direct_msaa::Compile( CBlender_Compile& C ) {
    ...
    switch ( C.iElement ) {
        ...
        case SE_SUN_MIDDLE: // middle pass - enable Z-test to perform
            ...
            //	FVF::TL2uv
            // C.r_Pass			("null",
            // "accum_sun_near",	false,	TRUE,
            // FALSE,blend,D3DBLEND_ONE,dest);
            C.r_Pass( "accum_sun", "accum_sun_near_msaa_nominmax", false, TRUE,
                      FALSE, blend, D3DBLEND_ONE, dest );
            C.r_CullMode( D3DCULL_NONE );
            C.PassSET_ZB( TRUE, FALSE, TRUE ); // force inverted Z-Buffer
        ...
        case SE_SUN_FAR: // far pass, only stencil clipping performed
            ...
            // C.r_Pass			("null",
            // "accum_sun_far",	false,	TRUE,	FALSE,blend,D3DBLEND_ONE,dest);
            C.r_Pass( "accum_sun", "accum_sun_far_msaa", false, TRUE, FALSE,
                      blend, D3DBLEND_ONE, dest );
            C.r_CullMode( D3DCULL_NONE );
        ...
        case SE_SUN_LUMINANCE: // luminance pass
            // C.r_Pass			("null",
            // "accum_sun",		false,	FALSE,	FALSE);
            C.r_Pass( "stub_notransform_aa_AA", "accum_sun_msaa", false, FALSE,
                      FALSE );
            C.r_CullMode( D3DCULL_NONE );
            // C.r_Sampler_rtf		("s_position",		r2_RT_P
            // ); C.r_Sampler_rtf		("s_normal",		r2_RT_N
            // ); C.r_Sampler_clw		("s_material",
            // r2_material		); C.r_Sampler_clf
            // ("s_smap",			r2_RT_generic0	); jitter
            // (C);
        ...
        case SE_SUN_NEAR_MINMAX: // near pass - enable Z-test to perform
                                 // depth-clipping
            //	FVF::TL2uv
            C.r_Pass( "accum_sun", "accum_sun_near_msaa_minmax", false, TRUE,
                      FALSE, blend, D3DBLEND_ONE, dest );
            C.r_CullMode( D3DCULL_NONE );
            C.PassSET_ZB( TRUE, FALSE, TRUE ); // force inverted Z-Buffer
...
void CBlender_accum_direct_volumetric_msaa::Compile( CBlender_Compile& C ) {
    ...
    switch ( C.iElement ) {
        case 0: // near pass - enable Z-test to perform depth-clipping
            C.r_Pass( "accum_sun", "accum_volumetric_sun_msaa", false, TRUE,
                      FALSE, blend, D3DBLEND_ONE, dest );
...
void CBlender_accum_direct_volumetric_sun_msaa::Compile( CBlender_Compile& C ) {
    ...
    switch ( C.iElement ) {
        case 0: // near pass - enable Z-test to perform depth-clipping
            C.r_Pass( "accum_sun", "accum_volumetric_sun_msaa", false, false,
                      false, true, D3DBLEND_ONE, D3DBLEND_ONE, false, 0 );
            C.r_dx10Texture( "s_smap", r2_RT_smap_depth );
            C.r_dx10Texture( "s_position", r2_RT_P );
            jitter( C );
