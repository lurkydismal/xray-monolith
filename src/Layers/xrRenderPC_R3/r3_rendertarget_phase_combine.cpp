...
void CRenderTarget::phase_combine() {
    ...
    // Forward rendering
    {
        ...
        if ( !RImplementation.o.dx10_msaa )
            ...
            u_setrt( rt_Generic_0_r, rt_Heat, 0,
    ...
    if ( RImplementation.o.dx10_msaa ) {
        ...
        HW.pDevice->ResolveSubresource( rt_Generic_1->pTexture->surface_get(),
                                        0,
                                        rt_Generic_1_r->pTexture->surface_get(),
                                        0, DXGI_FORMAT_R8G8B8A8_UNORM );
        HW.pDevice->ResolveSubresource( rt_Generic_0->pTexture->surface_get(),
                                        0,
                                        rt_Generic_0_r->pTexture->surface_get(),
                                        0, DXGI_FORMAT_R8G8B8A8_UNORM );
    }
        ...
        RImplementation.o.distortion_enabled; // This can be modified
    {
        ...
        if ( bDistort ) {
            ...
            if ( !RImplementation.o.dx10_msaa ) {
                ...
                HW.pDevice->ClearRenderTargetView( rt_Generic_1->pRT,
                                                   ColorRGBA );
            } else {
                u_setrt( rt_Generic_1_r, 0, 0,
                         RImplementation.Target->rt_MSAADepth->pZRT );
                // Now RT is a distortion mask
                HW.pDevice->ClearRenderTargetView( rt_Generic_1_r->pRT,
                                                   ColorRGBA );
            }
            RCache.set_CullMode( CULL_CCW );
            RCache.set_Stencil( FALSE );
    ...
    if ( 1 ) {
        ...
        RCache.set_c( "tnmp_exposure", ps_r2_tnmp_exposure, ps_r2_tnmp_exposure,
                      ps_r2_tnmp_exposure, 0 );
        RCache.set_c( "tnmp_gamma", ps_r2_tnmp_gamma, ps_r2_tnmp_gamma,
                      ps_r2_tnmp_gamma, 0 );
        RCache.set_c( "tnmp_onoff", ps_r2_tnmp_onoff, ps_r2_tnmp_onoff,
                      ps_r2_tnmp_onoff, 0 );
