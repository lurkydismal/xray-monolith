...
void CRenderTarget::phase_scene_prepare() {
    ...
    if ( !RImplementation.o.dx10_msaa )
        HW.pDevice->ClearDepthStencilView(
            HW.pBaseZB, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL, 1.0f, 0 );
    else {
        HW.pDevice->ClearDepthStencilView(
            rt_MSAADepth->pZRT, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL, 1.0f,
            0 );
        HW.pDevice->ClearDepthStencilView(
            HW.pBaseZB, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL, 1.0f, 0 );
    }
...
void CRenderTarget::phase_scene_begin() {
    ...
    // Targets, use accumulator for temporary storage
    {
        ...
        if ( RImplementation.o.albedo_wo )
            u_setrt( rt_Position, rt_Accumulator, rt_Heat, pZB );
        else
            u_setrt( rt_Position, rt_Color, rt_Heat, pZB );
        //--DSR-- HeatVision_end
    }
