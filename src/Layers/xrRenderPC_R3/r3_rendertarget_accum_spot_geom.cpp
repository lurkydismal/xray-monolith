...
void CRenderTarget::accum_spot_geom_create() {
    ...
    // vertices
    {
        ...
        // BYTE*	pData				= 0;
        // R_CHK
        // (g_accum_spot_vb->Lock(0,0,(void**)&pData,0)); CopyMemory
        // (pData,du_cone_vertices,vCount*vSize); g_accum_spot_vb->Unlock
        // ();

        R_CHK( dx10BufferUtils::CreateVertexBuffer(
            &g_accum_spot_vb, du_cone_vertices, vCount * vSize ) );
    ...
    // Indices
    {
        ...
        // R_CHK
        // (g_accum_spot_ib->Lock(0,0,(void**)&pData,0)); CopyMemory
        // (pData,du_cone_faces,iCount*2); g_accum_spot_ib->Unlock	();

        R_CHK( dx10BufferUtils::CreateIndexBuffer(
            &g_accum_spot_ib, du_cone_faces, iCount * 2 ) );
...
void CRenderTarget::accum_volumetric_geom_create() {
    ...
    // vertices
    {
        ...
        // BYTE*	pData				= 0;
        // R_CHK
        // (g_accum_volumetric_vb->Lock(0,0,(void**)&pData,0)); Slice	*pSlice
        // = (Slice*)pData; float t=0; float dt = 1.0f/(VOLUMETRIC_SLICES-1);
        ...
        R_CHK( dx10BufferUtils::CreateVertexBuffer( &g_accum_volumetric_vb,
                                                    &pSlice, vCount * vSize ) );
    ...
    // Indices
    {
        ...
        // R_CHK
        // (g_accum_volumetric_ib->Lock(0,0,(void**)&pData,0)); u16 *pInd =
        // (u16*) pData; for ( u16 i=0; i<VOLUMETRIC_SLICES; ++i, pInd+=6)
        //{
        //	u16 basevert = i*4;
        //	pInd[0] = basevert;
        //	pInd[1] = basevert+1;
        ...
        R_CHK( dx10BufferUtils::CreateIndexBuffer( &g_accum_volumetric_ib,
                                                   &Datap, iCount * 2 ) );
