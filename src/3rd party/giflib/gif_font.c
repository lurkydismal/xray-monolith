...
static void GifPutPixelClipped( SavedImage* Image,
                                int x,
                                int y,
                                GifPixelType color ) {
    size_t width, height;
...
void GifDrawText8x8( SavedImage* Image,
    ...
    for ( i = 0; i < GIF_FONT_HEIGHT; i++ ) {
        ...
        for ( cp = ( const unsigned char* )legend; *cp != '\0'; cp++ ) {
            size_t char_index =
                ( size_t )( cp - ( const unsigned char* )legend );
            size_t j;
            int glyph_x;
            unsigned int glyph;
...
void GifDrawBoxedText8x8( SavedImage* Image,
    ...
    if ( dup != NULL ) {
        ...
        if ( TextWidth <=
             ( SIZE_MAX - ( size_t )( 2 * border ) ) / GIF_FONT_WIDTH ) {
        ...
        if ( LineCount <=
             ( SIZE_MAX - ( size_t )( 2 * border ) ) / GIF_FONT_HEIGHT ) {
        ...
        for ( line = dup; line != NULL; line = next ) {
            ...
            if ( ( size_t )i <= ( size_t )( INT_MAX / GIF_FONT_HEIGHT ) ) {
                ...
                if ( leadspace <= ( size_t )( INT_MAX / GIF_FONT_WIDTH ) &&
                     text_x <=
                         INT_MAX - ( int )( leadspace * GIF_FONT_WIDTH ) ) {
