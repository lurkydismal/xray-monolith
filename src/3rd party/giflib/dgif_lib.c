                ...
                if ( Prefix[ CrntCode ] == NO_SUCH_CODE ) {
                    ...
                    if ( CrntCode == Private->RunningCode - 2 ) {
                        Suffix[ Private->RunningCode - 2 ] =
                            Stack[ StackPtr++ ] = DGifGetPrefixChar(
                                Prefix, LastCode, ClearCode );
                    } else {
                        Suffix[ Private->RunningCode - 2 ] =
                            Stack[ StackPtr++ ] = DGifGetPrefixChar(
                                Prefix, CrntCode, ClearCode );
                    }
                } else {
                    CrntPrefix = CrntCode;
                }
...
void DGifDecreaseImageCounter( GifFileType* GifFile ) {
    ...
    if ( GifFile->SavedImages[ GifFile->ImageCount ].ImageDesc.ColorMap !=
         NULL ) {
        GifFreeMapObject(
            GifFile->SavedImages[ GifFile->ImageCount ].ImageDesc.ColorMap );
    }
