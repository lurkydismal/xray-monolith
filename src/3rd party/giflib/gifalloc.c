...
*******************************************************************************/
void GifApplyTranslation( SavedImage* Image,
        ...
        if ( CopyFrom != NULL ) {
            ...
            if ( CopyFrom->ImageDesc.ColorMap != NULL ) {
                sp->ImageDesc.ColorMap =
                    GifMakeMapObject( CopyFrom->ImageDesc.ColorMap->ColorCount,
            ...
            if ( CopyFrom->ExtensionBlocks != NULL ) {
                int k;
                sp->ExtensionBlocks = ( ExtensionBlock* )calloc(
                    CopyFrom->ExtensionBlockCount, sizeof( ExtensionBlock ) );
                if ( sp->ExtensionBlocks == NULL ) {
                    FreeLastSavedImage( GifFile );
                    return ( SavedImage* )( NULL );
                }
                for ( k = 0; k < CopyFrom->ExtensionBlockCount; k++ ) {
                    ExtensionBlock* dst = &sp->ExtensionBlocks[ k ];
                    ExtensionBlock* src = &CopyFrom->ExtensionBlocks[ k ];
                    dst->Function = src->Function;
                    dst->ByteCount = src->ByteCount;
                    if ( src->ByteCount > 0 ) {
                        dst->Bytes = ( GifByteType* )malloc( src->ByteCount );
                        if ( dst->Bytes == NULL ) {
                            FreeLastSavedImage( GifFile );
                            return ( SavedImage* )( NULL );
                        }
                        memcpy( dst->Bytes, src->Bytes, src->ByteCount );
                    }
                }
            }
        } else {
