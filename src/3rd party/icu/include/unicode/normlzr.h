 ...
 * for backward compatibility. This class simply delegates to the Normalizer2
 * class. There is one exception: The new API does not provide a replacement for
 * Normalizer::compare().
 *
 ...
 *
 * The iterator API with the Normalizer constructors and the non-static
 * functions use a CharacterIterator as input. It is possible to pass a string
 * which is then internally wrapped in a CharacterIterator. The input text is
 * not normalized all at once, but incrementally where needed (providing
 * efficient random access). This allows to pass in a large text but spend only
 * a small amount of time normalizing a small part of that text. However, if the
 * entire text is normalized, then the iterator will be slower than normalizing
 * the entire text at once and iterating over the result. A possible use of the
 * Normalizer iterator is also to report an index into the original text that is
 * close to where the normalized characters come from.
 *
 * <em>Important:</em> The iterator API was cleaned up significantly for
 * ICU 2.0. The earlier implementation reported the getIndex() inconsistently,
 * and previous() could not be used after setIndex(), next(), first(), and
 * current().
 *
 * Normalizer allows to start normalizing from anywhere in the input text by
 * calling setIndexOnly(), first(), or last().
 * Without calling any of these, the iterator will start at the beginning of the
 * text.
 *
 ...
 * normalize.
 *
 * getIndex() always refers to the position in the input text where the
 * normalized code points are returned from. It does not always change with each
 * returned code point. The code point that is returned from any of the
 * functions corresponds to text at or after getIndex(), according to the
 ...
 * It is for this reason that Normalizer does not implement the
...
public:
...
#ifndef U_HIDE_DEPRECATED_API
    /**
     * Normalizes a <code>UnicodeString</code> according to the specified
     * normalization mode. This is a wrapper for unorm_normalize(), using
     * UnicodeString's.
     *
     ...
     * the results.
     * @param source       string for determining if it is in a normalized
     * format
     ...
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
    static inline UNormalizationCheckResult quickCheck(
        const UnicodeString& source,
        UNormalizationMode mode,
        UErrorCode& status );

    /**
     * Performing quick check on a string; same as the other version of
     * quickCheck but takes an extra options parameter like most normalization
     * functions.
     *
     * @param source       string for determining if it is in a normalized
     * format
     * @param mode         normalization format
     * @param options      the optional features to be enabled (0 for no
     * options)
     * @param status A reference to a UErrorCode to receive any errors
     * @return UNORM_YES, UNORM_NO or UNORM_MAYBE
     *
     ...
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
    static UNormalizationCheckResult quickCheck( const UnicodeString& source,
                                                 UNormalizationMode mode,
                                                 int32_t options,
                                                 UErrorCode& status );
     ...
     *
     * @param src        String that is to be tested if it is in a normalization
     * format.
     ...
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
    static inline UBool isNormalized( const UnicodeString& src,
                                      UNormalizationMode mode,
                                      UErrorCode& errorCode );

    /**
     * Test if a string is in a given normalization form; same as the other
     * version of isNormalized but takes an extra options parameter like most
     ...
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
    static UBool isNormalized( const UnicodeString& src,
                               UNormalizationMode mode,
                               int32_t options,
                               UErrorCode& errorCode );
     ...
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
    static UnicodeString& U_EXPORT2 concatenate( const UnicodeString& left,
     ...
     *
     * UAX 21 2.5 Caseless Matching specifies that for a canonical caseless
     * match the case folding must be performed first, then the normalization.
     *
     ...
     * conditions. If not set, the function will quickCheck for FCD and
     ...
     * @stable ICU 2.2
     */
    static inline int32_t compare( const UnicodeString& s1,
     ...
     *
     * The C API unorm_next() is more efficient and does not have this
     * ambiguity.
     *
     ...
     * the iterator state!)
     *
     * The C API unorm_previous() is more efficient and does not have this
     * ambiguity.
     *
     ...
     * index of the <code>CharacterIterator</code> or the start (i.e. index 0)
     * of the string over which this <code>Normalizer</code> is iterating.
     *
     * @return the smallest index in the input text where the Normalizer
     * operates
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
     ...
     * This end index is exclusive, i.e., the Normalizer operates only on
     * characters before this index.
     *
     * @return the first index in the input text where the Normalizer does not
     * operate
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
     ...
     * @param   value   the new setting for the option.  Use <code>TRUE</code>
     * to turn the option(s) on and <code>FALSE</code> to turn it/them off.
     *
     * @see #getOption
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
     ...
     * The iteration position is set to the beginning.
     *
     * @param newText a CharacterIterator object that replaces the current input
     * text
     * @param status a UErrorCode
     * @deprecated ICU 56 Use Normalizer2 instead.
     */
...
private:
    ...
    Normalizer& operator=(
        const Normalizer& that ); // assignment operator not implemented
...
#ifndef U_HIDE_DEPRECATED_API
inline UBool Normalizer::operator!=( const Normalizer& other ) const {
    return !operator==( other );
}

inline UNormalizationCheckResult Normalizer::quickCheck(
    const UnicodeString& source,
    UNormalizationMode mode,
    UErrorCode& status ) {
...
inline UBool Normalizer::isNormalized( const UnicodeString& source,
                                       UNormalizationMode mode,
                                       UErrorCode& status ) {
...
inline int32_t Normalizer::compare( const UnicodeString& s1,
                                    const UnicodeString& s2,
                                    uint32_t options,
                                    UErrorCode& errorCode ) {
    // all argument checking is done in unorm_compare
    return unorm_compare( toUCharPtr( s1.getBuffer() ), s1.length(),
                          toUCharPtr( s2.getBuffer() ), s2.length(), options,
                          &errorCode );
}
