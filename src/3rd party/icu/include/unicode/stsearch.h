 ...
 * desired. Using a <tt>BreakIterator</tt> will only give you results that
 * exactly matches the boundaries given by the breakiterator. For instance the
 * pattern "e" will not be found in the string "\u00e9" if a character break
 * iterator is used. <p> Options are provided to handle overlapping matches.
...
public:
     ...
     * <p>
     * Caller may modify but <b>must not</b> delete the
     * <tt>RuleBasedCollator</tt>! Modifications to this collator will affect
     * the original collator passed in to the <tt>StringSearch></tt> constructor
     * or to setCollator, if any.
     ...
     * with the user. The iterator's position will not be changed by this
     * method.
     * @param coll    collator
