 ...
 * <p>Typical use of CollationKeys are in databases, where you store a
 CollationKey
 ...
 * You can then cache the collation keys and compare them using
...
public:
     ...
     * collation keys, the result is Collator::EQUAL.  Comparing empty collation
     * key with non-empty collation key is always Collator::LESS.
     * @stable ICU 2.0
     */
    CollationKey();
     ...
     * @param source the collation key to compare to.
     * @return Returns TRUE if two collation keys are different, FALSE
     * otherwise.
     * @stable ICU 2.0
     */
    UBool operator!=( const CollationKey& source ) const;
     ...
     * @param count the output parameter of number of collation key values,
     * including any trailing nulls.
     * @obsolete ICU 2.6. Use getByteArray instead since this API will be
     ...
     * @stable ICU 2.6
     */
    UCollationResult compareTo( const CollationKey& target,
     ...
     * status); .    if (U_FAILURE(status)) return; .    CollationKey key1,
     * key2; .    UErrorCode status1 = U_ZERO_ERROR, status2 = U_ZERO_ERROR; .
     * myCollation->getCollationKey("abc", key1, status1); .    if
     * (U_FAILURE(status1)) { delete myCollation; return; } .
     * myCollation->getCollationKey("ABC", key2, status2); .    if
     * (U_FAILURE(status2)) { delete myCollation; return; } .    //
...
inline UBool CollationKey::operator!=( const CollationKey& other ) const {
    return !( *this == other );
}

inline UBool CollationKey::isBogus() const {
