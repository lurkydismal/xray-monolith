 ...
 *   was impractical.
 * - Switch on UTF_SAFE etc. (selection of unsafe/safe/strict default string
 * processing) was of little use and impractical.
 ...
 * - Change in Unicode standard: "irregular" sequences (3.0) became illegal
 * (3.2).
 * - Change of language in Unicode standard:
 *   Growing distinction between internal x-bit Unicode strings and external
 * UTF-x forms, with the former more lenient. Suggests renaming of UTF16_ macros
 * to U16_.
 ...
 * by defining UTF_SIZE to either 8, 16, or 32. utf.h would then define the
 * UChar type accordingly. UTF-16 was the default.</p>
 *
 ...
 * The UTF-8 and UTF-32 macros remain for now for completeness and backward
 * compatibility.</p>
 *
 * <p>Accordingly, utf.h defines UChar to be an unsigned 16-bit integer. If this
 * matches wchar_t, then UChar is defined to be exactly wchar_t, otherwise
 * uint16_t.</p>
 *
 * <p>UChar32 is defined to be a signed 32-bit integer (int32_t), large enough
 * for a 21-bit Unicode code point (Unicode scalar value, 0..0x10ffff). Before
 * ICU 2.4, the definition of UChar32 was similarly platform-dependent as the
 * definition of UChar. For details see the documentation for UChar32
 * itself.</p>
 *
 * <p>utf.h also defines a number of C macros for handling single Unicode code
 * points and for using UTF Unicode strings. It includes utf8.h, utf16.h, and
 * utf32.h for the actual implementations of those macros and then aliases one
 * set of them (for UTF-16) for general use. The UTF-specific macros have the
 * UTF size in the macro name prefixes (UTF16_...), while the general alias
 * macros always begin with UTF_...</p>
 *
 * <p>Many string operations can be done with or without error checking.
 * Where such a distinction is useful, there are two versions of the macros,
 * "unsafe" and "safe" ones with ..._UNSAFE and ..._SAFE suffixes. The unsafe
 * macros are fast but may cause program failures if the strings are not
 * well-formed. The safe macros have an additional, boolean parameter "strict".
 * If strict is FALSE, then only illegal sequences are detected. Otherwise,
 * irregular sequences and non-characters are detected as well (like single
 * surrogates). Safe macros return special error code points for
 * illegal/irregular sequences: Typically, U+ffff, or values that would result
 * in a code unit sequence of the same length as the erroneous input
 * sequence.<br> Note that _UNSAFE macros have fewer parameters: They do not
 * have the strictness parameter, and they do not have start/length parameters
 * for boundary checking.</p>
 *
 * <p>Here, the macros are aliased in two steps:
 * In the first step, the UTF-specific macros with UTF16_ prefix and _UNSAFE and
 * _SAFE suffixes are aliased according to the UTF_SIZE to macros with UTF_
 * prefix and the same suffixes and signatures. Then, in a second step, the
 * default, general alias macros are set to use either the unsafe or the
 * safe/not strict (default) or the safe/strict macro; these general macros do
 * not have a strictness parameter.</p>
 *
 * <p>It is possible to change the default choice for the general alias macros
 * to be unsafe, safe/not strict or safe/strict. The default is safe/not strict.
 * It is not recommended to select the unsafe macros as the basis for Unicode
 * string handling in ICU! To select this, define UTF_SAFE, UTF_STRICT, or
 * UTF_UNSAFE.</p>
 *
 * <p>For general use, one should use the default, general macros with UTF_
 * prefix and no _SAFE/_UNSAFE suffix. Only in some cases it may be necessary to
 * control the choice of macro directly and use a less generic alias. For
 * example, if it can be assumed that a string is well-formed and the index will
 * stay within the bounds, then the _UNSAFE version may be used. If a UTF-8
 * string is to be processed, then the macros with UTF8_ prefixes need to be
 * used.</p>
 *
 * <hr>
 *
 ...
 *
 * @obsolete ICU 2.6. Use int32_t directly instead since this API will be
...
/**
 * The default choice for general Unicode string macros is to use the ..._SAFE
 * macro implementations with strict=FALSE.
 *
 * @deprecated ICU 2.4. Obsolete, see utf_old.h.
 */
#define UTF_SAFE
...
/**
 * Error value for all UTFs. This code point value will be set by macros with
 * error checking if an error is detected.
 *
 * @deprecated ICU 2.4. Obsolete, see utf_old.h.
 */
...
#define UTF_IS_ERROR( c )                                            \
    ( ( ( c ) & 0xfffe ) == 0xfffe || ( c ) == UTF8_ERROR_VALUE_1 || \
...
#define UTF_IS_VALID( c )                                        \
    ( UTF_IS_UNICODE_CHAR( c ) && ( c ) != UTF8_ERROR_VALUE_1 && \
      ( c ) != UTF8_ERROR_VALUE_2 )

/**
 * Is this code unit or code point a surrogate (U+d800..U+dfff)?
 * @deprecated ICU 2.4. Renamed to U_IS_SURROGATE and U16_IS_SURROGATE, see
 * utf_old.h.
 */
 ...
 * remain stable, and should not be hidden when other internal functions are
 * hidden (otherwise public macros would fail to compile).
 * @internal
 */
#ifdef U_UTF8_IMPL
// No forward declaration if compiling utf_impl.cpp, which defines
    ...
    ( ( uint32_t )( c ) <= 0x7f                                               \
          ...
          : ( ( uint32_t )( c ) <= 0x7ff                                      \
                  ...
                  : ( ( uint32_t )( c ) <= 0xffff                             \
                          ...
                          : ( ( uint32_t )( c ) <= 0x10ffff                   \
                                  ? 4                                         \
                                  : ( ( uint32_t )( c ) <= 0x3ffffff          \
                                          ? 5                                 \
                                          : ( ( uint32_t )( c ) <= 0x7fffffff \
                                                  ? 6                         \
                                                  : 3 ) ) ) ) ) )
#endif

/** The maximum number of bytes per code point. @deprecated ICU 2.4. Renamed to
 * U8_MAX_LENGTH, see utf_old.h. */
#define UTF8_MAX_CHAR_LENGTH 4

/** Average number of code units compared to UTF-16. @deprecated ICU 2.4.
...
#define UTF8_GET_CHAR_SAFE( s, start, i, length, c, strict )             \
    UPRV_BLOCK_MACRO_BEGIN {                                             \
        ...
        UTF8_NEXT_CHAR_SAFE( s, _utf8_get_char_safe_index, length, c,    \
                             strict );                                   \
    }                                                                    \
    UPRV_BLOCK_MACRO_END
...
#define UTF8_APPEND_CHAR_SAFE( s, i, length, c )                               \
    UPRV_BLOCK_MACRO_BEGIN {                                                   \
        if ( ( uint32_t )( c ) <= 0x7f ) {                                     \
            ...
            ( i ) = utf8_appendCharSafeBody( s, ( int32_t )( i ),              \
                                             ( int32_t )( length ), c, NULL ); \
        }                                                                      \
    }                                                                          \
    UPRV_BLOCK_MACRO_END
...
/** Is uchar a first/lead surrogate? @deprecated ICU 2.4. Renamed to U_IS_LEAD
 * and U16_IS_LEAD, see utf_old.h. */
#define UTF_IS_FIRST_SURROGATE( uchar ) ( ( ( uchar ) & 0xfffffc00 ) == 0xd800 )

/** Is uchar a second/trail surrogate? @deprecated ICU 2.4. Renamed to
 * U_IS_TRAIL and U16_IS_TRAIL, see utf_old.h. */
#define UTF_IS_SECOND_SURROGATE( uchar ) \
    ( ( ( uchar ) & 0xfffffc00 ) == 0xdc00 )

/** Assuming c is a surrogate, is it a first/lead surrogate? @deprecated
 * ICU 2.4. Renamed to U_IS_SURROGATE_LEAD and U16_IS_SURROGATE_LEAD, see
 * utf_old.h. */
#define UTF_IS_SURROGATE_FIRST( c ) ( ( ( c ) & 0x400 ) == 0 )

/** Helper constant for UTF16_GET_PAIR_VALUE. @deprecated ICU 2.4. Renamed to
 * U16_SURROGATE_OFFSET, see utf_old.h. */
#define UTF_SURROGATE_OFFSET ( ( 0xd800 << 10UL ) + 0xdc00 - 0x10000 )

/** Get the UTF-32 value from the surrogate code units. @deprecated ICU 2.4.
 * Renamed to U16_GET_SUPPLEMENTARY, see utf_old.h. */
#define UTF16_GET_PAIR_VALUE( first, second ) \
    ( ( ( first ) << 10UL ) + ( second ) - UTF_SURROGATE_OFFSET )

/** @deprecated ICU 2.4. Renamed to U16_LEAD, see utf_old.h. */
#define UTF_FIRST_SURROGATE( supplementary ) \
    ( UChar )( ( ( supplementary ) >> 10 ) + 0xd7c0 )

/** @deprecated ICU 2.4. Renamed to U16_TRAIL, see utf_old.h. */
#define UTF_SECOND_SURROGATE( supplementary ) \
...
#define UTF16_GET_CHAR_SAFE( s, start, i, length, c, strict )                 \
    UPRV_BLOCK_MACRO_BEGIN {                                                  \
        ...
        if ( UTF_IS_SURROGATE( c ) ) {                                        \
            ...
            if ( UTF_IS_SURROGATE_FIRST( c ) ) {                              \
                if ( ( i ) + 1 < ( length ) &&                                \
                     UTF_IS_SECOND_SURROGATE( __c2 = ( s )[ ( i ) + 1 ] ) ) { \
                    ( c ) = UTF16_GET_PAIR_VALUE( ( c ), __c2 );              \
                    /* strict: ((c)&0xfffe)==0xfffe is caught by              \
                     * UTF_IS_ERROR() and UTF_IS_UNICODE_CHAR() */            \
                } else if ( strict ) {                                        \
                ...
                if ( ( i ) - 1 >= ( start ) &&                                \
                     UTF_IS_FIRST_SURROGATE( __c2 = ( s )[ ( i ) - 1 ] ) ) {  \
...
#define UTF16_NEXT_CHAR_SAFE( s, i, length, c, strict )                     \
    UPRV_BLOCK_MACRO_BEGIN {                                                \
        ...
        if ( UTF_IS_FIRST_SURROGATE( c ) ) {                                \
            uint16_t __c2;                                                  \
            if ( ( i ) < ( length ) &&                                      \
                 UTF_IS_SECOND_SURROGATE( __c2 = ( s )[ ( i ) ] ) ) {       \
                ++( i );                                                    \
                ( c ) = UTF16_GET_PAIR_VALUE( ( c ), __c2 );                \
                /* strict: ((c)&0xfffe)==0xfffe is caught by UTF_IS_ERROR() \
                 * and UTF_IS_UNICODE_CHAR() */                             \
            } else if ( strict ) {                                          \
                /* unmatched first surrogate */                             \
                ( c ) = UTF_ERROR_VALUE;                                    \
            }                                                               \
 ...
 *   <p><b>Usage:</b>  ICU coding guidelines for if() statements should be
 * followed when using these macros. Compound statements (curly braces {}) must
 * be used  for if-else-while... bodies and all macro statements should be
 * terminated with semicolon.</p>
 */
...
/**
 * Estimate the number of code units for a string based on the number of UTF-16
 * code units.
 *
 * @deprecated ICU 2.4. Obsolete, see utf_old.h.
 */
...
/**
 * Does this code point require multiple code units (is it a supplementary code
 * point)? Same as UTF16_NEED_MULTIPLE_UCHAR.
 * @deprecated ICU 2.4. Use U16_LENGTH or test ((uint32_t)(c)>0xffff) instead.
 */
...
/**
 * Advance i to beyond the code units of the n code points where the first one
 * begins at i. I.e., advance i by n code points. Same as UT16_FWD_N.
 * \pre 0<=i<length
 * \post 0<i<=length
 *
 * @deprecated ICU 2.4. Renamed to U16_FWD_N, see utf_old.h.
...
/**
 * Take the random-access index i and adjust it so that it points to the
 * beginning of a code point. The input index points to any code unit of a code
 * point and is moved to point to the first code unit of the same code point. i
 * is never incremented. In other words, if i points to a trail surrogate that
 * is preceded by a matching lead surrogate, then i is decremented. Otherwise it
 * is not modified. This can be used to start an iteration with UTF_NEXT_CHAR()
 * from a random index. Same as UTF16_SET_CHAR_START.
 * \pre start<=i<length
 * \post start<=i<length
 *
 * @deprecated ICU 2.4. Renamed to U16_SET_CP_START, see utf_old.h.
 ...
 * i must point to the first code unit after the last unit of a code point
 * (i==length is allowed). Same as UTF16_PREV_CHAR.
 * \pre start<i<=length
 * \post start<=i<length
 *
 ...
 * i must point to the first code unit after the last unit of a code point
 * (i==length is allowed). Same as UTF16_BACK_1.
 * \pre start<i<=length
 * \post start<=i<length
 *
 * @deprecated ICU 2.4. Renamed to U16_BACK_1, see utf_old.h.
 ...
 * i must point to the first code unit after the last unit of a code point
 * (i==length is allowed). Same as UTF16_BACK_N.
 * \pre start<i<=length
 * \post start<=i<length
 *
 * @deprecated ICU 2.4. Renamed to U16_BACK_N, see utf_old.h.
 ...
 * In other words, if i points to a trail surrogate that is preceded by a
 * matching lead surrogate, then i is incremented. Otherwise it is not modified.
 * This can be used to start an iteration with UTF_PREV_CHAR() from a random
 * index. Same as UTF16_SET_CHAR_LIMIT.
 * \pre start<i<=length
 * \post start<i<=length
 *
 * @deprecated ICU 2.4. Renamed to U16_SET_CP_LIMIT, see utf_old.h.
