...
#define U8_COUNT_TRAIL_BYTES( leadByte )                                   \
    ( U8_IS_LEAD( leadByte ) ? ( ( uint8_t )( leadByte ) >= 0xe0 ) +       \
 ...
 *
 * This is internal since it is not meant to be called directly by external
 * clients; however it is U_STABLE (not U_INTERNAL) since it is called by public
 * macros in this file and thus must remain stable, and should not be hidden
 * when other internal functions are hidden (otherwise public macros would fail
 * to compile).
 * @internal
 */
 ...
 * @internal
 */
U_STABLE int32_t U_EXPORT2 utf8_back1SafeBody( const uint8_t* s,
                                               int32_t start,
                                               int32_t i );
 ...
 * If the offset points to an illegal UTF-8 byte sequence, then
 * c is set to U+FFFD.
 * Iteration through a string is more efficient with U8_NEXT_UNSAFE or
 * U8_NEXT_OR_FFFD.
 *
...
#define U8_INTERNAL_NEXT_OR_SUB( s, i, length, c, sub )                        \
    UPRV_BLOCK_MACRO_BEGIN {                                                   \
        ...
        if ( !U8_IS_SINGLE( c ) ) {                                            \
            uint8_t __t = 0;                                                   \
            if ( ( i ) != ( length ) && /* fetch/validate/assemble all but     \
                                           last trail byte */                  \
                 ( ( c ) >= 0xe0                                               \
                       ? ( ( c ) < 0xf0                                        \
                               ? /* U+0800..U+FFFF except surrogates */        \
                               U8_LEAD3_T1_BITS[ ( c ) &= 0xf ] &              \
                                       ( 1                                     \
                                         << ( ( __t = ( s )[ i ] ) >> 5 ) ) && \
                                   ( __t &= 0x3f, 1 )                          \
                               : /* U+10000..U+10FFFF */                       \
                               ( ( c ) -= 0xf0 ) <= 4 &&                       \
                                   U8_LEAD4_T1_BITS[ ( __t = ( s )[ i ] ) >>   \
                                                     4 ] &                     \
                                       ( 1 << ( c ) ) &&                       \
                                   ( ( c ) = ( ( c ) << 6 ) | ( __t & 0x3f ),  \
                                     ++( i ) != ( length ) ) &&                \
                                   ( __t = ( s )[ i ] - 0x80 ) <=              \
                                       0x3f ) && /* valid second-to-last trail \
                                                    byte */                    \
                             ( ( c ) = ( ( c ) << 6 ) | __t,                   \
                               ++( i ) != ( length ) )                         \
                       : /* U+0080..U+07FF */                                  \
                       ( c ) >= 0xc2 &&                                        \
                           ( ( c ) &= 0x1f, 1 ) ) && /* last trail byte */     \
                 ( __t = ( s )[ i ] - 0x80 ) <= 0x3f &&                        \
                 ( ( c ) = ( ( c ) << 6 ) | __t, ++( i ), 1 ) ) {              \
            } else {                                                           \
                ( c ) = ( sub ); /* ill-formed*/                               \
        ...
        if ( __uc <= 0x7f ) {                                                \
            ...
            if ( __uc <= 0x7ff ) {                                           \
                ...
                if ( __uc <= 0xffff ) {                                      \
                    ...
                    ( s )[ ( i )++ ] =                                       \
                        ( uint8_t )( ( ( __uc >> 12 ) & 0x3f ) | 0x80 );     \
                }                                                            \
                ( s )[ ( i )++ ] =                                           \
                    ( uint8_t )( ( ( __uc >> 6 ) & 0x3f ) | 0x80 );          \
            }                                                                \
 ...
 * If a non-ASCII code point is written, checks for sufficient space in the
 * string. If the code point is not valid or trail bytes do not fit, then
 ...
 * @param isError output UBool set to TRUE if an error occurs, otherwise not
 * modified
    ...
    UPRV_BLOCK_MACRO_BEGIN {                                                   \
        ...
        if ( __uc <= 0x7f ) {                                                  \
            ...
            ( s )[ ( i )++ ] = ( uint8_t )( ( __uc & 0x3f ) | 0x80 );          \
                      ...
                      ( 0xe000 <= __uc && __uc <= 0xffff ) ) &&                \
                    ( i ) + 2 < ( capacity ) ) {                               \
            ( s )[ ( i )++ ] = ( uint8_t )( ( __uc >> 12 ) | 0xe0 );           \
            ( s )[ ( i )++ ] = ( uint8_t )( ( ( __uc >> 6 ) & 0x3f ) | 0x80 ); \
            ( s )[ ( i )++ ] = ( uint8_t )( ( __uc & 0x3f ) | 0x80 );          \
        } else if ( 0xffff < __uc && __uc <= 0x10ffff &&                       \
                    ( i ) + 3 < ( capacity ) ) {                               \
            ( s )[ ( i )++ ] = ( uint8_t )( ( __uc >> 18 ) | 0xf0 );           \
            ( s )[ ( i )++ ] =                                                 \
                ( uint8_t )( ( ( __uc >> 12 ) & 0x3f ) | 0x80 );               \
    ...
    UPRV_BLOCK_MACRO_BEGIN {                                              \
        if ( ( length ) > ( start ) ) {                                   \
            ...
            if ( U8_IS_SINGLE( __b1 ) ) {                                 \
                ...
                --( length );                                             \
            } else if ( U8_IS_TRAIL( __b1 ) &&                            \
                        ( ( length ) - 2 ) >= ( start ) ) {               \
                uint8_t __b2 = s[ ( length ) - 2 ];                       \
                if ( 0xe0 <= __b2 && __b2 <= 0xf4 ) {                     \
                    if ( __b2 < 0xf0                                      \
                             ? U8_IS_VALID_LEAD3_AND_T1( __b2, __b1 )     \
                             : U8_IS_VALID_LEAD4_AND_T1( __b2, __b1 ) ) { \
                        ( length ) -= 2;                                  \
                    }                                                     \
                } else if ( U8_IS_TRAIL( __b2 ) &&                        \
                            ( ( length ) - 3 ) >= ( start ) ) {           \
...
#define U8_PREV( s, start, i, c )                                              \
    UPRV_BLOCK_MACRO_BEGIN {                                                   \
        ( c ) = ( uint8_t )( s )[ --( i ) ];                                   \
        if ( !U8_IS_SINGLE( c ) ) {                                            \
            ( c ) = utf8_prevCharSafeBody( ( const uint8_t* )s, start, &( i ), \
                                           c, -1 );                            \
        }                                                                      \
    }                                                                          \
    UPRV_BLOCK_MACRO_END
...
/**
 * Move the string offset from one code point boundary to the n-th one before
 * it, i.e., move backward by n code points. (Pre-decrementing backward
 * iteration.) The input offset may be the same as the string length. "Unsafe"
 * macro, assumes well-formed UTF-8.
 *
 * @param s const uint8_t * string
 * @param i string offset
 * @param n number of code points to skip
 ...
 * Move the string offset from one code point boundary to the n-th one before
 * it, i.e., move backward by n code points. (Pre-decrementing backward
 * iteration.) The input offset may be the same as the string length. "Safe"
