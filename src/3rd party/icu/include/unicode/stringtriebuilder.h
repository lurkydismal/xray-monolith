...
protected:
    // Do not enclose the protected default constructor with #ifndef
    // U_HIDE_INTERNAL_API or else the compiler will create a public default
    // constructor.
    /** @internal */
    StringTrieBuilder();
    /** @internal */
    ...
    /** @internal */
    void build( UStringTrieBuildOption buildOption,
    ...
    /** @internal */
    int32_t writeBranchSubNode( int32_t start,
                                int32_t limit,
                                int32_t unitIndex,
                                int32_t length );
#endif /* U_HIDE_INTERNAL_API */
...
#ifndef U_HIDE_INTERNAL_API
    // max(BytesTrie::kMaxBranchLinearSubNodeLength,
    // UCharsTrie::kMaxBranchLinearSubNodeLength).
    /** @internal */
    static const int32_t kMaxBranchLinearSubNodeLength = 5;

    // Maximum number of nested split-branch levels for a branch on all 2^16
    // possible char16_t units. log2(2^16/kMaxBranchLinearSubNodeLength) rounded
    // up.
    /** @internal */
     ...
     * @param errorCode ICU in/out UErrorCode.
                        Set to U_MEMORY_ALLOCATION_ERROR if it was success but
     newNode==NULL.
    ...
    /*
     * C++ note:
     * registerNode() and registerFinalValue() take ownership of their input
     * nodes, and only return owned nodes. If they see a failure UErrorCode,
     * they will delete the input node. If they get a NULL pointer, they will
     * record a U_MEMORY_ALLOCATION_ERROR. If there is a failure, they return
     * NULL.
     *
     * NULL Node pointers can be safely passed into other Nodes because
     * they call the static Node::hashCode() which checks for a NULL pointer
     * first.
     *
    ...
    public:
         ...
         * Most branch edges "jump" to other nodes but the rightmost branch
         * edges just continue without a jump. Therefore, write() must write the
         * rightmost branch edge last (trie units are written backwards), and
         * must write it at that point even if it is a duplicate of a node
         * previously written elsewhere.
         *
         * This function visits and marks right branch edges first.
         * Edges are numbered with increasingly negative values because we share
         * the offset field which gets positive values when nodes are written.
         * A branch edge also remembers the first number for any of its edges.
         *
         * When a further-left branch edge has a number in the range of the
         * rightmost edge's numbers, then it will be written as part of the
         * required right edge and we can avoid writing it first.
         *
         * After root.markRightEdgesFirst(-1) the offsets of all nodes are
         * negative edge numbers.
         *
         * @param edgeNumber The first edge number for this node and its
         * sub-nodes.
         * @return An edge number that is at least the maximum-negative
         *         of the input edge number and the numbers of this node and all
         * of its sub-nodes.
         */
        ...
        inline void writeUnlessInsideRightEdge( int32_t firstRight,
            ...
            // If offset>0 then this node and its sub-nodes have been written
            // already and we need not write them again. If this node is part of
            // the unwritten right branch edge, then we wait until that is
            // written.
    ...
    // (stack-allocated so that we don't unnecessarily create lots of duplicate
    // nodes) with the input node, and the !Node::operator==(other) used inside
    // FinalValueNode::operator==(other) will be false if the typeid's are
    // different.
    /** @internal */
    ...
    public:
        IntermediateValueNode( int32_t v, Node* nextNode )
            : ValueNode( 0x222222u * 37u + hashCode( nextNode ) ),
              next( nextNode ) {
            setValue( v );
        }
    ...
    public:
        LinearMatchNode( int32_t len, Node* nextNode )
            : ValueNode( ( 0x333333u * 37u + len ) * 37u +
                         hashCode( nextNode ) ),
              length( len ),
              next( nextNode ) {}
    ...
    public:
        BranchHeadNode( int32_t len, Node* subNode )
            : ValueNode( ( 0x666666u * 37u + len ) * 37u +
                         hashCode( subNode ) ),
              length( len ),
              next( subNode ) {}
