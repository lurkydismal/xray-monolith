...
/**
 * Useful constant for the maximum total size of keywords and their values in a
 * locale
 * @stable ICU 2.8
 */
 ...
 *    By default (without calling this function), ICU's default locale will be
 * based on information obtained from the underlying system environment. <p>
 ...
 * @stable ICU 2.0
 */
U_STABLE void U_EXPORT2 uloc_setDefault( const char* localeID,
                                         UErrorCode* status );
 ...
 * @stable ICU 2.0
 */
U_STABLE int32_t U_EXPORT2 uloc_getVariant( const char* localeID,
                                            char* variant,
 ...
 * @stable ICU 2.0
 */
U_STABLE int32_t U_EXPORT2 uloc_getName( const char* localeID,
                                         char* name,
 ...
 * @stable ICU 2.0
 */
U_STABLE const char* U_EXPORT2 uloc_getISO3Language( const char* localeID );
 ...
 * @stable ICU 2.0
 */
U_STABLE uint32_t U_EXPORT2 uloc_getLCID( const char* localeID );
 ...
 * Locale::getFrench() for inLocale would result in "Anglais", while passing
 * Locale::getGerman() for inLocale would result in "Englisch".
 ...
 * it's greater than languageCapacity, the returned language code will be
 * truncated.
 * @stable ICU 2.0
 */
U_STABLE int32_t U_EXPORT2 uloc_getDisplayLanguage( const char* locale,
 ...
 * @param displayLocale Specifies the locale to be used to display the name.  In
 * other words, if the locale's language code is "en", passing
 * Locale::getFrench() for inLocale would result in "", while passing
 * Locale::getGerman() for inLocale would result in "". NULL may be used to
 * specify the default.
 ...
 * it's greater than scriptCapacity, the returned displayable script code will
 * be truncated.
 * @stable ICU 2.8
 */
U_STABLE int32_t U_EXPORT2 uloc_getDisplayScript( const char* locale,
                                                  const char* displayLocale,
                                                  UChar* script,
                                                  int32_t scriptCapacity,
                                                  UErrorCode* status );
 ...
 * other words, if the locale's language code is "en", passing
 * Locale::getFrench() for inLocale would result in "Anglais", while passing
 * Locale::getGerman() for inLocale would result in "Englisch". NULL may be used
 ...
 * it's greater than countryCapacity, the returned displayable country code will
 * be truncated.
 * @stable ICU 2.0
 */
U_STABLE int32_t U_EXPORT2 uloc_getDisplayCountry( const char* locale,
                                                   const char* displayLocale,
                                                   UChar* country,
                                                   int32_t countryCapacity,
                                                   UErrorCode* status );
 ...
 * it's greater than variantCapacity, the returned displayable variant code will
 * be truncated.
 * @stable ICU 2.0
 */
 ...
 * E.g: for the locale string de_DE\@collation=PHONEBOOK, this API gets the
 * display string for the keyword collation. Usage: <code> UErrorCode status =
 * U_ZERO_ERROR; const char* keyword =NULL; int32_t keywordLen = 0; int32_t
 * keywordCount = 0; UChar displayKeyword[256]; int32_t displayKeywordLen = 0;
 *    UEnumeration* keywordEnum =
 * uloc_openKeywords("de_DE@collation=PHONEBOOK;calendar=TRADITIONAL", &status);
 *    for(keywordCount = uenum_count(keywordEnum, &status); keywordCount > 0 ;
 * keywordCount--){ if(U_FAILURE(status)){
 *              ...something went wrong so handle the error...
 *              break;
 *          }
 *          // the uenum_next returns NUL terminated string
 *          keyword = uenum_next(keywordEnum, &keywordLen, &status);
 *          displayKeywordLen = uloc_getDisplayKeyword(keyword, "en_US",
 * displayKeyword, 256);
 *          ... do something interesting .....
 *    }
 *    uenum_close(keywordEnum);
 * </code>
 * @param keyword           The keyword whose display string needs to be
 * returned.
 * @param displayLocale     Specifies the locale to be used to display the name.
 * In other words, if the locale's language code is "en", passing
 * Locale::getFrench() for inLocale would result in "Anglais", while passing
 * Locale::getGerman() for inLocale would result in "Englisch". NULL may be used
 * to specify the default.
 * @param dest              the buffer to which the displayable keyword should
 * be written.
 ...
 * (pre-flighting).
 * @param status            error information if retrieving the displayable
 * string failed. Should not be NULL and should not indicate failure on entry.
 * @return the actual buffer size needed for the displayable variant code.
 * @see #uloc_openKeywords
 * @stable ICU 2.8
 */
U_STABLE int32_t U_EXPORT2 uloc_getDisplayKeyword( const char* keyword,
                                                   const char* displayLocale,
                                                   UChar* dest,
 ...
 * E.g: for the locale string de_DE\@collation=PHONEBOOK, this API gets the
 * display string for PHONEBOOK, in the display locale, when "collation" is
 ...
 * (pre-flighting).
 * @param status            error information if retrieving the displayable
 * string failed. Should not be NULL and must not indicate failure on entry.
 * @return the actual buffer size needed for the displayable variant code.
 * @stable ICU 2.8
 */
 ...
 *
 * @param localeID the locale to get the displayable name with. NULL may be used
 * to specify the default.
 * @param inLocaleID Specifies the locale to be used to display the name.  In
 * other words, if the locale's language code is "en", passing
 * Locale::getFrench() for inLocale would result in "Anglais", while passing
 * Locale::getGerman() for inLocale would result in "Englisch". NULL may be used
 * to specify the default.
 ...
 * @return the actual buffer size needed for the displayable name.  If it's
 * greater than maxResultSize, the returned displayable name will be truncated.
 * @stable ICU 2.0
 */
 ...
 * @stable ICU 2.0
 */
U_STABLE const char* U_EXPORT2 uloc_getAvailable( int32_t n );
 ...
 * plus additional 3-letter codes determined to be useful for locale generation
 * as defined by Unicode CLDR. This is a pointer to an array of pointers to
 * arrays of char.  All of these pointers are owned by ICU-- do not delete them,
 * and do not write through them.  The array is terminated with a null pointer.
 * @return a list of all available language codes
 * @stable ICU 2.0
 */
U_STABLE const char* const* U_EXPORT2 uloc_getISOLanguages( void );

/**
 *
 * Gets a list of all available 2-letter country codes defined in ISO 639.  This
 * is a pointer to an array of pointers to arrays of char.  All of these
 ...
 * @stable ICU 2.0
 */
U_STABLE const char* const* U_EXPORT2 uloc_getISOCountries( void );
 ...
 * @stable ICU 2.0
 */
U_STABLE int32_t U_EXPORT2 uloc_getParent( const char* localeID,
                                           char* parent,
 ...
 * @stable ICU 2.8
 */
U_STABLE int32_t U_EXPORT2 uloc_getBaseName( const char* localeID,
                                             char* name,
                                             int32_t nameCapacity,
                                             UErrorCode* err );
 ...
 * @param keywordName name of the keyword for which we want the value; must not
 * be NULL or empty, and must consist only of [A-Za-z0-9]. Case insensitive.
 * @param buffer receiving buffer
 * @param bufferCapacity capacity of receiving buffer
 * @param status containing error code: e.g. buffer not big enough or ill-formed
 * localeID or keywordName parameters.
 * @return the length of keyword value
 * @stable ICU 2.8
 */
U_STABLE int32_t U_EXPORT2 uloc_getKeywordValue( const char* localeID,
 ...
 * @stable ICU 3.2
 */
U_STABLE int32_t U_EXPORT2 uloc_setKeywordValue( const char* keywordName,
 ...
 * @stable ICU 54
 */
U_STABLE UBool U_EXPORT2 uloc_isRightToLeft( const char* locale );
 ...
 * @stable ICU 4.0
 */
U_STABLE ULayoutType U_EXPORT2 uloc_getLineOrientation( const char* localeId,
                                                        UErrorCode* status );
 ...
 */
U_STABLE int32_t U_EXPORT2
uloc_acceptLanguageFromHTTP( char* result,
 ...
 * @stable ICU 3.2
 */
U_STABLE int32_t U_EXPORT2 uloc_acceptLanguage( char* result,
                                                int32_t resultAvailable,
                                                UAcceptResult* outResult,
 ...
 * @param locale the output buffer for the ICU locale ID, which will be
 * NUL-terminated if there is room.
 * @param localeCapacity the size of the output buffer
 * @param status an error is returned if the LCID is unrecognized or the output
 * buffer is too small
 * @return actual the actual size of the locale ID, not including
 * NUL-termination
 * @stable ICU 3.8
 */
U_STABLE int32_t U_EXPORT2 uloc_getLocaleForLCID( uint32_t hostID,
                                                  char* locale,
 ...
 * of the localeID and the null-terminator is greater than the maximum allowed
 * size, or the localeId is not well-formed, the error code is
 ...
 * @stable ICU 4.2
 */
U_STABLE int32_t U_EXPORT2 uloc_forLanguageTag( const char* langtag,
                                                char* localeID,
 ...
 * @stable ICU 4.2
 */
U_STABLE int32_t U_EXPORT2 uloc_toLanguageTag( const char* localeID,
                                               char* langtag,
 ...
 * @stable ICU 54
 */
U_STABLE const char* U_EXPORT2 uloc_toUnicodeLocaleKey( const char* keyword );
 ...
 * @stable ICU 54
 */
U_STABLE const char* U_EXPORT2 uloc_toUnicodeLocaleType( const char* keyword,
                                                         const char* value );
 ...
 * @stable ICU 54
 */
U_STABLE const char* U_EXPORT2 uloc_toLegacyKey( const char* keyword );
 ...
 * @stable ICU 54
 */
U_STABLE const char* U_EXPORT2 uloc_toLegacyType( const char* keyword,
                                                  const char* value );
