 ...
 * completeness of the information to show. If the range smaller than the
 * resolution specified by the skeleton, a single date format will be produced.
 * If the range is larger than the format specified by the skeleton, a
 * locale-specific fallback will be used to format the items missing from the
 * skeleton.
 *
 ...
 *   for en_US, "Mar 4"
 *   for en_GB, "4 Mar"
 * If the range is 2010-03-04 07:56 - 2010-03-08 16:11 (4 days, 8 hours, 15
 * minutes)
 * - The skeleton jm will produce
 ...
 * skeletons from this set, or those formed by combining these standard
 * skeletons (note that for these skeletons, the length of digit field such as
 * d, y, or M vs MM is irrelevant (but for non-digit fields such as MMM vs MMMM
 * it is relevant). Note that a skeleton involving h or H generally explicitly
 * requests that time style (12- or 24-hour time respectively). For a skeleton
 * that requests the locale's default time style (h or H), use 'j' instead of h
 * or H. h, H, hm, Hm, hv, Hv, hmv, Hmv, d, M, MMM, MMMM, Md, MMMd, MEd, MMMEd,
 *   y,
 *   yM, yMMM, yMMMM,
 *   yMd, yMMMd,
 *   yMEd, yMMMEd
 ...
 * include:
 *   af, am, ar, be, bg, bn, ca, cs, da, de (_AT), el, en
 * (_AU,_CA,_GB,_IE,_IN...), eo, es (_AR,_CL,_CO,...,_US) et, fa, fi, fo, fr
 * (_BE,_CH,_CA), fur, gsw, he, hr, hu, hy, is, it (_CH), ja, kk, km, ko, lt,
 * lv, mk, ml, mt, nb, nl )_BE), nn, pl, pt (_PT), rm, ro, ru (_UA), sk, sl, so,
 * sq, sr, sr_Latn, sv, th, to, tr, uk, ur, vi, zh (_SG), zh_Hant (_HK,_MO)
 */
...
typedef struct UDateIntervalFormat
    UDateIntervalFormat; /**< C typedef for struct UDateIntervalFormat. @stable
                            ICU 4.8 */
 ...
 *            The length of skeleton; may be -1 if the skeleton is
 * zero-terminated.
 * @param tzID
 *            A timezone ID specifying the timezone to use. If 0, use the
 * default timezone.
 * @param tzIDLength
 *            The length of tzID, or -1 if null-terminated. If 0, use the
 * default timezone.
 * @param status
 *            A pointer to a UErrorCode to receive any errors.
 * @return
 *            A pointer to a UDateIntervalFormat object for the specified
 * locale, or NULL if an error occurred.
 * @stable ICU 4.8
 */
U_STABLE UDateIntervalFormat* U_EXPORT2 udtitvfmt_open( const char* locale,
 ...
 * @stable ICU 4.8
 */
U_STABLE void U_EXPORT2 udtitvfmt_close( UDateIntervalFormat* formatter );
 ...
 * only as long as the UFormattedDateInterval is present and unchanged in
 * memory.
 *
...
udtitvfmt_resultAsValue( const UFormattedDateInterval* uresult,
                         UErrorCode* ec );
 ...
 * @stable ICU 4.8
 */
U_DEFINE_LOCAL_OPEN_POINTER( LocalUDateIntervalFormatPointer,
                             UDateIntervalFormat,
                             udtitvfmt_close );
...
udtitvfmt_formatToResult( const UDateIntervalFormat* formatter,
