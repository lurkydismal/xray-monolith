 ...
 * Instances returned by getInstance() are singletons that must not be deleted
 * by the caller. The Normalizer2 class is not intended for public subclassing.
 *
 ...
 * forms are those defined in http://www.unicode.org/unicode/reports/tr15/
 * However, this API supports additional normalization forms for specialized
 * purposes. For example, NFKC_Casefold is provided via getInstance("nfkc_cf",
 ...
 * before it and starting from it do not interact and can be handled
 * independently.
 *
 ...
 * For example, in ICU, iterative normalization is used by the
 * NormalizationTransliterator (to avoid replacing already-normalized text) and
 * ucol_nextSortKeyPart() (to process only the substring for which sort key
 * bytes are computed).
 *
...
public:
     ...
     * ures_open()/ResourceBundle) and which composes or decomposes text
     * according to the specified mode. Returns an unmodifiable singleton
     * instance. Do not delete it.
     *
     ...
     * @stable ICU 4.4
     */
    static const Normalizer2* getInstance( const char* packageName,
                                           const char* name,
     ...
     * and U_EDITS_NO_RESET.
     * @param src       Source UTF-8 string.
     * @param sink      A ByteSink to which the normalized UTF-8 result string
     * is written. sink.Flush() is called at the end.
     * @param edits     Records edits for index mapping, working with styled
     * text, and getting only changes (if any). The Edits contents is undefined
     * if any error occurs. This function calls edits->reset() first unless
     ...
     * @stable ICU 60
     */
    virtual void normalizeUTF8( uint32_t options,
                                StringPiece src,
                                ByteSink& sink,
                                Edits* edits,
                                UErrorCode& errorCode ) const;
     ...
     * @stable ICU 4.4
     */
    virtual UnicodeString& normalizeSecondAndAppend(
        UnicodeString& first,
     ...
     * on a UNORM2_DECOMPOSE Normalizer2 instance, but much faster, and except
     * that this function returns FALSE and does not write a string if c does
     * not have a decomposition mapping in this instance's data. This function
     * is independent of the mode of the Normalizer2.
     * @param c code point
     ...
     * @stable ICU 4.6
     */
    virtual UBool getDecomposition( UChar32 c,
                                    UnicodeString& decomposition ) const = 0;
     ...
     * is Canonical (Can); in this case, the result contains either one or two
     ...
     * @stable ICU 49
     */
    virtual UBool getRawDecomposition( UChar32 c,
                                       UnicodeString& decomposition ) const;

    /**
     * Performs pairwise composition of a & b and returns the composite if there
     * is one.
     *
     ...
     * but all standard implementations return the Unicode
     ...
     * @stable ICU 4.4
     */
    virtual UBool isNormalized( const UnicodeString& s,
                                UErrorCode& errorCode ) const = 0;
    /**
     ...
     * @stable ICU 60
     */
    virtual UBool isNormalizedUTF8( StringPiece s,
                                    UErrorCode& errorCode ) const;
     ...
     * @stable ICU 4.4
     */
    virtual UNormalizationCheckResult quickCheck(
     ...
     * @stable ICU 4.4
     */
    virtual int32_t spanQuickCheckYes( const UnicodeString& s,
                                       UErrorCode& errorCode ) const = 0;
     ...
     * This is used for iterative normalization. See the class documentation for
     * details.
     ...
     * character independently.
     * This is used for iterative normalization. See the class documentation for
     * details. Note that this operation may be significantly slower than
     * hasBoundaryBefore().
     * @param c character to test
     ...
     * If true, then the character does not change, nor normalization-interact
     * with preceding or following characters. In other words, a string
     * containing this character can be normalized by processing portions before
     * this character and after this character independently. This is used for
     * iterative normalization. See the class documentation for details. Note
     * that this operation may be significantly slower than hasBoundaryBefore().
     * @param c character to test
...
public:
     ...
     * @param filterSet UnicodeSet which determines the characters to be
     * normalized
     * @stable ICU 4.4
     */
     ...
     * @stable ICU 4.6
     */
    virtual UBool getDecomposition( UChar32 c, UnicodeString& decomposition )
        const U_OVERRIDE;
     ...
     * @stable ICU 49
     */
    virtual UBool getRawDecomposition( UChar32 c, UnicodeString& decomposition )
        const U_OVERRIDE;

    /**
     * Performs pairwise composition of a & b and returns the composite if there
     * is one. For details see the base class documentation.
     *
     * This function is independent of the mode of the Normalizer2.
     * @param a A (normalization starter) code point.
     * @param b Another code point.
     ...
     * @stable ICU 4.4
     */
    virtual UNormalizationCheckResult quickCheck( const UnicodeString& s,
                                                  UErrorCode& errorCode ) const
        U_OVERRIDE;
    /**
     * Returns the end of the normalized substring of the input string.
...
private:
    ...
    void normalizeUTF8( uint32_t options,
                        const char* src,
                        int32_t length,
                        ByteSink& sink,
                        Edits* edits,
