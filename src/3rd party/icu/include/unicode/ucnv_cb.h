 ...
 * Writes out the specified byte output bytes to the target byte buffer or to
 ...
 * @param err error status. If <TT>U_BUFFER_OVERFLOW</TT> is returned, then
 * U_BUFFER_OVERFLOW <STRONG>must</STRONG> be returned to the user, because it
 * means that not all data could be written into the target buffer, and some is
 * in the converter error buffer.
 * @see ucnv_cbFromUWriteSub
 * @stable ICU 2.0
 */
U_STABLE void U_EXPORT2 ucnv_cbFromUWriteBytes( UConverterFromUnicodeArgs* args,
                                                const char* source,
                                                int32_t length,
                                                int32_t offsetIndex,
                                                UErrorCode* err );
 ...
 *
 * @param args callback fromUnicode arguments
 * @param offsetIndex the relative offset index from the current source pointer
 * to be used
 * @param err error status. If <TT>U_BUFFER_OVERFLOW</TT> is returned, then
 * U_BUFFER_OVERFLOW <STRONG>must</STRONG> be returned to the user, because it
 * means that not all data could be written into the target buffer, and some is
 ...
 * @stable ICU 2.0
 */
U_STABLE void U_EXPORT2 ucnv_cbFromUWriteSub( UConverterFromUnicodeArgs* args,
                                              int32_t offsetIndex,
                                              UErrorCode* err );

/**
 * ONLY used by fromU callback functions.
 * This function will write out the error character(s) to the target UChar
 * buffer.
 *
 * @param args callback fromUnicode arguments
 * @param source pointer to pointer to first UChar to write [on exit: 1 after
 * last UChar processed]
