 ...
 * nature, and the results can not be guaranteed to always be correct. <p> For
 * best accuracy in charset detection, the input data should be primarily in a
 * single language, and a minimum of a few hundred bytes worth of plain text in
 * the language are needed.  The detection process will attempt to ignore html
 * or xml style markup that could otherwise obscure the content. <p> An
 * alternative to the ICU Charset Detector is the Compact Encoding Detector,
 * https://github.com/google/compact_enc_det. It often gives more accurate
 * results, especially with short input samples.
 */
 ...
 *  @stable ICU 3.6
 */
U_STABLE UCharsetDetector* U_EXPORT2 ucsdet_open( UErrorCode* status );
 ...
 * @stable ICU 3.6
 */
U_STABLE void U_EXPORT2 ucsdet_setDeclaredEncoding( UCharsetDetector* ucsd,
                                                    const char* encoding,
                                                    int32_t length,
                                                    UErrorCode* status );
 ...
 * @stable ICU 3.6
 */
U_STABLE const UCharsetMatch* U_EXPORT2 ucsdet_detect( UCharsetDetector* ucsd,
                                                       UErrorCode* status );
 ...
 * @param status        any error conditions are reported back in this variable.
 * @return              A pointer to an array of pointers to UCharSetMatch
 * objects. This array, and the UCharSetMatch instances to which it refers, are
 * owned by the UCharsetDetector, and will remain valid until the detector is
 * closed or modified.
 * @stable ICU 3.6
 */
 ...
 *  @stable ICU 3.6
 */
U_STABLE const char* U_EXPORT2 ucsdet_getName( const UCharsetMatch* ucsm,
                                               UErrorCode* status );
 ...
 *  @stable ICU 3.6
 */
U_STABLE int32_t U_EXPORT2 ucsdet_getConfidence( const UCharsetMatch* ucsm,
                                                 UErrorCode* status );
 ...
 *  @stable ICU 3.6
 */
U_STABLE const char* U_EXPORT2 ucsdet_getLanguage( const UCharsetMatch* ucsm,
                                                   UErrorCode* status );
 ...
 * @stable ICU 3.6
 */
U_STABLE int32_t U_EXPORT2 ucsdet_getUChars( const UCharsetMatch* ucsm,
                                             UChar* buf,
                                             int32_t cap,
                                             UErrorCode* status );
 ...
 * returns an array including "ISO-8859-1" (ISO Latin 1), but not including
 * "windows-1252" (Windows Latin 1). However, actual detection result could be
 * "windows-1252" when the input data matches Latin 1 code points with any
 * points only available in "windows-1252".
 *
...
ucsdet_getAllDetectableCharsets( const UCharsetDetector* ucsd,
                                 UErrorCode* status );
 ...
 * @stable ICU 3.6
 */
U_STABLE UBool U_EXPORT2 ucsdet_enableInputFilter( UCharsetDetector* ucsd,
                                                   UBool filter );
...
ucsdet_getDetectableCharsets( const UCharsetDetector* ucsd,
                              UErrorCode* status );
 ...
 * @internal
 */
U_INTERNAL void U_EXPORT2 ucsdet_setDetectableCharset( UCharsetDetector* ucsd,
                                                       const char* encoding,
                                                       UBool enabled,
                                                       UErrorCode* status );
