...
public:
     ...
     *
     * The trieUChars must contain a copy of a char16_t sequence from the
     * UCharsTrieBuilder, starting with the first char16_t of that sequence. The
     * UCharsTrie object will not read more char16_ts than the UCharsTrieBuilder
     ...
     * @param uchar Input char value. Values below 0 and above 0xffff will never
     * match.
     * @return The match/value Result.
     * @stable ICU 4.8
     */
    ...
    inline int32_t getValue() const {
        ...
        return leadUnit & kValueIsFinal ? readValue( pos, leadUnit & 0x7fff )
     ...
     * That is, each char16_t c for which it would be
     * next(c)!=USTRINGTRIE_NO_MATCH now.
    ...
    public:
         ...
         *                        Otherwise, the iterator returns strings with
         * this maximum length.
         ...
         * @stable ICU 4.8
         */
        Iterator( ConstChar16Ptr trieUChars,
         ...
         * @stable ICU 4.8
         */
        Iterator( const UCharsTrie& trie,
                  int32_t maxStringLength,
                  UErrorCode& errorCode );
    ...
    private:
        ...
        const char16_t* branchNext( const char16_t* pos,
                                    int32_t length,
                                    UErrorCode& errorCode );
        ...
        UBool
            skipValue_; // Skip intermediate value which was already delivered.
        ...
        // bits 15..0, and the remaining branch length in bits 31..16. (We could
        // store the remaining branch length minus 1 in bits 30..16 and not use
        // the sign bit, but the code looks more confusing that way.)
        UVector32* stack_;
    };
...
private:
    ...
    static inline int32_t readNodeValue( const char16_t* pos,
                                         int32_t leadUnit ) {
        ...
        if ( leadUnit < kMinTwoUnitNodeValueLead ) {
            ...
            value =
                ( ( ( leadUnit & 0x7fc0 ) - kMinTwoUnitNodeValueLead ) << 10 ) |
                *pos;
        } else {
            value = ( pos[ 0 ] << 16 ) | pos[ 1 ];
        }
    ...
    UStringTrieResult branchNext( const char16_t* pos,
                                  int32_t length,
                                  int32_t uchar );
    ...
    // from a branch.
    static const char16_t* findUniqueValueFromBranch( const char16_t* pos,
                                                      int32_t length,
                                                      UBool haveUniqueValue,
                                                      int32_t& uniqueValue );
    // Recursively finds a unique value (or whether there is not a unique one)
    // starting from a position on a node lead unit.
    static UBool findUniqueValue( const char16_t* pos,
                                  UBool haveUniqueValue,
                                  int32_t& uniqueValue );

    // Helper functions for getNextUChars().
    // getNextUChars() when pos is on a branch node.
    static void getNextBranchUChars( const char16_t* pos,
                                     int32_t length,
                                     Appendable& out );

    // UCharsTrie data structure
    //
    // The trie consists of a series of char16_t-serialized nodes for
    // incremental Unicode string/char16_t sequence matching. (char16_t=16-bit
    // unsigned integer) The root node is at the beginning of the trie data.
    //
    ...
    //
    // Node types:
    //  - Final-value node: Stores a 32-bit integer in a compact,
    //  variable-length format.
    //    The value is for the string/char16_t sequence so far.
    //  - Match node, optionally with an intermediate value in a different
    //  compact format.
    ...
    //  unit.
    //    The node unit is the length of the branch (number of units to select
    //    from) minus 1. It is followed by a sub-node:
    //    - If the length is at most kMaxBranchLinearSubNodeLength, then
    //      there are length-1 (key, value) pairs and then one more comparison
    //      unit. If one of the key units matches, then the value is either a
    //      final value for the string so far, or a "jump" delta to the next
    //      node. If the last unit matches, then matching continues with the
    //      next node. (Values have the same encoding as final-value nodes.)
    //    - If the length is greater than kMaxBranchLinearSubNodeLength, then
    //      there is one unit and one "jump" delta.
    //      If the input unit is less than the sub-node unit, then "jump" by
    ...
    // 0030..003f: Linear-match node, match 1..16 units and continue reading the
    // next node.
    ...
    static const int32_t kMinValueLead =
        kMinLinearMatch + kMaxLinearMatchLength;            // 0x0040
    ...
    // Compact value: After testing and masking off bit 15, use the following
    // thresholds.
    ...
    static const int32_t kMaxTwoUnitValue =
        ( ( kThreeUnitValueLead - kMinTwoUnitValueLead ) << 16 ) -
        1; // 0x3ffeffff

    // Compact intermediate-value integer, lead unit shared with a branch or
    // linear-match node.
    static const int32_t kMaxOneUnitNodeValue = 0xff;
    static const int32_t kMinTwoUnitNodeValueLead =
    ...
    static const int32_t kMaxTwoUnitNodeValue =
        ( ( kThreeUnitNodeValueLead - kMinTwoUnitNodeValueLead ) << 10 ) -
        1; // 0xfdffff
    ...
    static const int32_t kMaxTwoUnitDelta =
        ( ( kThreeUnitDeltaLead - kMinTwoUnitDeltaLead ) << 16 ) -
        1; // 0x03feffff
