...
#define U16_GET_SUPPLEMENTARY( lead, trail )                   \
    ( ( ( UChar32 )( lead ) << 10UL ) + ( UChar32 )( trail ) - \
...
#define U16_GET( s, start, i, length, c )                          \
    UPRV_BLOCK_MACRO_BEGIN {                                       \
        ...
        if ( U16_IS_SURROGATE( c ) ) {                             \
            ...
            if ( U16_IS_SURROGATE_LEAD( c ) ) {                    \
                if ( ( i ) + 1 != ( length ) &&                    \
                     U16_IS_TRAIL( __c2 = ( s )[ ( i ) + 1 ] ) ) { \
                    ( c ) = U16_GET_SUPPLEMENTARY( ( c ), __c2 );  \
                }                                                  \
            } else {                                               \
                if ( ( i ) > ( start ) &&                          \
                     U16_IS_LEAD( __c2 = ( s )[ ( i ) - 1 ] ) ) {  \
...
#define U16_NEXT_OR_FFFD( s, i, length, c )                           \
    UPRV_BLOCK_MACRO_BEGIN {                                          \
        ...
        if ( U16_IS_SURROGATE( c ) ) {                                \
            uint16_t __c2;                                            \
            if ( U16_IS_SURROGATE_LEAD( c ) && ( i ) != ( length ) && \
                 U16_IS_TRAIL( __c2 = ( s )[ ( i ) ] ) ) {            \
                ++( i );                                              \
                ( c ) = U16_GET_SUPPLEMENTARY( ( c ), __c2 );         \
            } else {                                                  \
...
#define U16_PREV_OR_FFFD( s, start, i, c )                           \
    UPRV_BLOCK_MACRO_BEGIN {                                         \
        ...
        if ( U16_IS_SURROGATE( c ) ) {                               \
            uint16_t __c2;                                           \
            if ( U16_IS_SURROGATE_TRAIL( c ) && ( i ) > ( start ) && \
                 U16_IS_LEAD( __c2 = ( s )[ ( i ) - 1 ] ) ) {        \
                --( i );                                             \
                ( c ) = U16_GET_SUPPLEMENTARY( __c2, ( c ) );        \
            } else {                                                 \
 ...
 * macro, assumes well-formed UTF-16.
 *
 * @param s const UChar * string
 * @param i string offset
 * @param n number of code points to skip
