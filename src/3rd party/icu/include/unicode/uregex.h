 ...
 *
 * <p>This is a C wrapper around the C++ RegexPattern and RegexMatcher
 * classes.</p>
 */
...
typedef enum URegexpFlag {
    ...
    /**  If set, '.' matches line terminators,  otherwise '.' matching stops at
     * line end.
     *  @stable ICU 2.4 */
    UREGEX_DOTALL = 32,
 ...
 * in string form into an internal representation using the specified match mode
 * flags. The resulting regular expression handle can then be used to perform
 ...
 *                       example.  For default behavior, set this parameter to
 * zero. See <code>enum URegexpFlag</code>.  All desired flags are bitwise-ORed
 * together.
 * @param pe             Receives the position (line and column numbers) of any
 * syntax error within the source regular expression string.  If this
 *                       information is not wanted, pass NULL for this
 * parameter.
 ...
 * in string form into an internal representation using the specified match mode
 * flags. The resulting regular expression handle can then be used to perform
 * various matching operations. <p> The contents of the pattern UText will be
 * extracted and saved. Ownership of the UText struct itself remains with the
 * caller. This is to match the behavior of uregex_open().
 *
 * @param pattern        The Regular Expression pattern to be compiled.
 * @param flags          Flags that alter the default matching behavior for
 *                       the regular expression, UREGEX_CASE_INSENSITIVE, for
 ...
 * @stable ICU 4.6
 */
U_STABLE URegularExpression* U_EXPORT2 uregex_openUText( UText* pattern,
                                                         uint32_t flags,
                                                         UParseError* pe,
                                                         UErrorCode* status );
 ...
 * @stable ICU 3.0
 */
U_STABLE void U_EXPORT2 uregex_close( URegularExpression* regexp );
 ...
 * expression. This function will work even if the pattern was originally
 * specified as a UText.
 *
 ...
 * expression object, and must not be altered or deleted.
 *
 * @stable ICU 4.6
 */
U_STABLE UText* U_EXPORT2 uregex_patternUText( const URegularExpression* regexp,
                                               UErrorCode* status );

/**
 * Get the match mode flags that were specified when compiling this regular
 * expression.
 * @param status   Receives errors detected by this function.
 * @param regexp   The compiled regular expression.
 ...
 * @stable ICU 3.0
 */
U_STABLE int32_t U_EXPORT2 uregex_flags( const URegularExpression* regexp,
                                         UErrorCode* status );

/**
 *  Set the subject text string upon which the regular expression will look for
 * matches. This function may be called any number of times, allowing the
 * regular expression pattern to be applied to different strings. <p> Regular
 * expression matching operations work directly on the application's string
 * data.  No copy is made.  The subject string data must not be altered after
 * calling this function until after all regular expression operations involving
 * this string data are completed. <p> Zero length strings are permitted.  In
 * this case, no subsequent match operation will dereference the text string
 * pointer.
 *
 * @param regexp     The compiled regular expression.
 ...
 * @stable ICU 3.0
 */
U_STABLE void U_EXPORT2 uregex_setText( URegularExpression* regexp,
                                        const UChar* text,
                                        int32_t textLength,
                                        UErrorCode* status );
 ...
 * regular expression pattern to be applied to different strings. <p> Regular
 * expression matching operations work directly on the application's string
 * data; only a shallow clone is made.  The subject string data must not be
 ...
 * @stable ICU 4.6
 */
U_STABLE void U_EXPORT2 uregex_setUText( URegularExpression* regexp,
                                         UText* text,
                                         UErrorCode* status );
 ...
 * uregex_setText(), that pointer will be returned.  Otherwise, the characters
 * in the input will be extracted to a buffer and returned.  In either case,
 * ownership remains with the regular expression object.
 *
 ...
 * parameter. A NULL pointer may be used here if the text length is not needed,
 * as would be the case if the text is known in advance to be a NUL terminated
 *                    string.
 * @param status      Receives errors detected by this function.
 * @return            Pointer to the subject text string currently associated
 * with this regular expression.
 * @stable ICU 3.0
 */
U_STABLE const UChar* U_EXPORT2 uregex_getText( URegularExpression* regexp,
                                                int32_t* textLength,
                                                UErrorCode* status );
 ...
 * shallow clone of the actual input string.
 * @param status      Receives errors detected by this function.
 * @return            The subject text currently associated with this regular
 * expression. If a pre-allocated UText was provided, it will always be used and
 * returned.
 *
 * @stable ICU 4.6
 */
U_STABLE UText* U_EXPORT2 uregex_getUText( URegularExpression* regexp,
                                           UText* dest,
                                           UErrorCode* status );

/**
 *  Set the subject text string upon which the regular expression is looking for
 * matches without changing any other aspect of the matching state. The new and
 * previous text strings must have the same content.
 *
 *  This function is intended for use in environments where ICU is operating on
 *  strings that may move around in memory.  It provides a mechanism for
 * notifying ICU that the string has been relocated, and providing a new UText
 * to access the string in its new position.
 *
 *  Note that the regular expression implementation never copies the underlying
 * text of a string being matched, but always operates directly on the original
 * text provided by the user. Refreshing simply drops the references to the old
 * text and replaces them with references to the new.
 *
 *  Caution:  this function is normally used only by very specialized
 *            system-level code.   One example use case is with garbage
 * collection that moves the text in memory.
 *
 * @param regexp     The compiled regular expression.
 ...
 * @stable ICU 4.8
 */
U_STABLE void U_EXPORT2 uregex_refreshUText( URegularExpression* regexp,
                                             UText* text,
                                             UErrorCode* status );
 ...
 * matching, or -1 to match the input Region.
 *    @param  status      Receives errors detected by this function.
 *    @return             TRUE if there is a match
 *    @stable ICU 3.0
 */
U_STABLE UBool U_EXPORT2 uregex_matches( URegularExpression* regexp,
                                         int32_t startIndex,
                                         UErrorCode* status );
 ...
 *   @stable ICU 4.6
 */
U_STABLE UBool U_EXPORT2 uregex_matches64( URegularExpression* regexp,
                                           int64_t startIndex,
                                           UErrorCode* status );

/**
 *   Attempts to match the input string, starting from the specified index,
 * against the pattern. The match may be of any length, and is not required to
 * extend to the end of the input string.  Contrast with uregex_matches().
 *
 *   <p>If startIndex is >= 0 any input region that was set for this
 *   URegularExpression is reset before the operation begins.
 *
 *   <p>If the specified starting index == -1 the match begins at the start of
 * the input region, or at the start of the full string if no region has been
 * specified. This corresponds directly with Matcher.lookingAt() in Java.
 *
 ...
 *    @stable ICU 4.6
 */
U_STABLE UBool U_EXPORT2 uregex_lookingAt64( URegularExpression* regexp,
                                             int64_t startIndex,
                                             UErrorCode* status );

/**
 *   Find the first matching substring of the input string that matches the
 * pattern. If startIndex is >= zero the search for a match begins at the
 * specified index, and any match region is reset.  This corresponds directly
 * with Matcher.find(startIndex) in Java.
 *
 ...
 *   <code>uregex_group()</code> will provide more information regarding the
 * match.
 *
 *   @param   regexp      The compiled regular expression.
 *   @param   startIndex  The position (native) in the input string to begin the
 * search, or -1 to search within the Input Region.
 *   @param   status      A reference to a UErrorCode to receive any errors.
 ...
 *   @stable ICU 4.6
 */
U_STABLE UBool U_EXPORT2 uregex_find64( URegularExpression* regexp,
                                        int64_t startIndex,
                                        UErrorCode* status );
 ...
 *  previous match.  If a match is found, <code>uregex_start(),
 * uregex_end()</code>, and <code>uregex_group()</code> will provide more
 ...
 *  @stable ICU 3.0
 */
U_STABLE UBool U_EXPORT2 uregex_findNext( URegularExpression* regexp,
                                          UErrorCode* status );
 ...
 *   @stable ICU 3.0
 */
U_STABLE int32_t U_EXPORT2 uregex_groupCount( URegularExpression* regexp,
                                              UErrorCode* status );
 ...
 *
 *   @param   regexp       The compiled regular expression.
 *   @param   groupNum     The capture group to extract.  Group 0 is the
 * complete match.  The value of this parameter must be less than or equal to
...
/** Returns a shallow immutable clone of the entire input string with the
  current index set
  *   to the beginning of the requested capture group.  The capture group length
  is also
  ...
  *                         less than or equal to the number of capture groups
  in
  *                         the pattern.
  *   @param   dest         A mutable UText in which to store the current input.
  *                         If NULL, a new UText will be created as an immutable
  ...
  *   @stable ICU 4.6
  */
U_STABLE UText* U_EXPORT2 uregex_groupUText( URegularExpression* regexp,
                                             int32_t groupNum,
                                             UText* dest,
                                             int64_t* groupLength,
                                             UErrorCode* status );
...
/**
 *   Returns the index in the input string of the start of the text matched by
 * the specified capture group during the previous match operation.  Return -1
 * if the capture group was not part of the last match. Group #0 refers to the
 * complete range of matched text. Group #1 refers to the text matched by the
 * first set of capturing parentheses.
 *
 *    @param   regexp      The compiled regular expression.
 *    @param   groupNum    The capture group number
 *    @param   status      A reference to a UErrorCode to receive any errors.
 *    @return              the starting (native) position in the input of the
 * text matched by the specified group.
 *    @stable ICU 3.0
 */
 ...
 *   @stable ICU 4.6
 */
U_STABLE int64_t U_EXPORT2 uregex_start64( URegularExpression* regexp,
                                           int32_t groupNum,
                                           UErrorCode* status );
 ...
 *   Group #1 refers to the text matched by the first set of capturing
 * parentheses.
 *
 *    @param   regexp      The compiled regular expression.
 *    @param   groupNum    The capture group number
 *    @param   status      A reference to a UErrorCode to receive any errors.
 *    @return              the (native) index of the position following the last
 * matched character.
 *    @stable ICU 3.0
 */
U_STABLE int32_t U_EXPORT2 uregex_end( URegularExpression* regexp,
                                       int32_t groupNum,
                                       UErrorCode* status );
 ...
 *   @stable ICU 4.6
 */
U_STABLE int64_t U_EXPORT2 uregex_end64( URegularExpression* regexp,
                                         int32_t groupNum,
                                         UErrorCode* status );
 ...
 *    @stable ICU 3.0
 */
U_STABLE void U_EXPORT2 uregex_reset( URegularExpression* regexp,
                                      int32_t index,
                                      UErrorCode* status );
 ...
 *    @stable ICU 4.6
 */
U_STABLE void U_EXPORT2 uregex_reset64( URegularExpression* regexp,
                                        int64_t index,
                                        UErrorCode* status );
 ...
 * matching. Invoking this method resets any saved state from the previous
 * match, then sets the region to start at the index specified by the start
 ...
 * @stable ICU 4.0
 */
U_STABLE void U_EXPORT2 uregex_setRegion( URegularExpression* regexp,
                                          int32_t regionStart,
                                          int32_t regionLimit,
                                          UErrorCode* status );
 ...
 * @stable ICU 4.6
 */
U_STABLE void U_EXPORT2 uregex_setRegion64( URegularExpression* regexp,
                                            int64_t regionStart,
                                            int64_t regionLimit,
                                            UErrorCode* status );
 ...
 * @stable ICU 4.6
 */
U_STABLE void U_EXPORT2 uregex_setRegionAndStart( URegularExpression* regexp,
                                                  int64_t regionStart,
                                                  int64_t regionLimit,
                                                  int64_t startIndex,
                                                  UErrorCode* status );
...
/**
 * Reports the start index of the matching region. Any matches found are limited
 * to to the region bounded by regionStart (inclusive) and regionEnd
 * (exclusive).
 *
 * @param regexp The compiled regular expression.
 * @param status A pointer to a UErrorCode to receive any errors.
 ...
 * @stable ICU 4.0
 */
U_STABLE int32_t U_EXPORT2 uregex_regionStart( const URegularExpression* regexp,
                                               UErrorCode* status );
...
uregex_regionStart64( const URegularExpression* regexp, UErrorCode* status );

/**
 * Reports the end index (exclusive) of the matching region for this
 * URegularExpression. Any matches found are limited to to the region bounded by
 ...
 * @stable ICU 4.0
 */
U_STABLE int32_t U_EXPORT2 uregex_regionEnd( const URegularExpression* regexp,
                                             UErrorCode* status );
 ...
 * @stable ICU 4.6
 */
U_STABLE int64_t U_EXPORT2 uregex_regionEnd64( const URegularExpression* regexp,
                                               UErrorCode* status );
 ...
 * be used.
 *
 * Using transparent bounds, the boundaries of the matching region are
 * transparent to lookahead, lookbehind, and boundary matching constructs. Those
 * constructs can see text beyond the boundaries of the region while checking
 * for a match.
 *
 * With opaque bounds, no text outside of the matching region is visible to
 ...
 * @stable ICU 4.0
 **/
U_STABLE void U_EXPORT2 uregex_useTransparentBounds( URegularExpression* regexp,
                                                     UBool b,
                                                     UErrorCode* status );
 ...
 * @stable ICU 4.0
 */
U_STABLE void U_EXPORT2 uregex_useAnchoringBounds( URegularExpression* regexp,
                                                   UBool b,
                                                   UErrorCode* status );
 ...
 *  @stable ICU 4.0
 */
U_STABLE UBool U_EXPORT2 uregex_hitEnd( const URegularExpression* regexp,
                                        UErrorCode* status );

/**
 * Return TRUE the most recent match succeeded and additional input could cause
 * it to fail. If this function returns false and a match was found, then more
 ...
 * @stable ICU 4.0
 */
U_STABLE UBool U_EXPORT2 uregex_requireEnd( const URegularExpression* regexp,
                                            UErrorCode* status );
 ...
 * errors.
 *    @return                     The length of the string resulting from the
 * find and replace operation.  In the event that the destination capacity is
 * inadequate, the return value is still the full length of the untruncated
 * string.
 *    @stable ICU 3.0
 */
 ...
 * not be mutable).
 *    @param   status         A reference to a UErrorCode to receive any errors.
 *    @return                 A UText containing the results of the find and
 * replace. If a pre-allocated UText was provided, it will always be used and
 * returned.
 *
 *    @stable ICU 4.6
 */
U_STABLE UText* U_EXPORT2 uregex_replaceAllUText( URegularExpression* regexp,
                                                  UText* replacement,
                                                  UText* dest,
                                                  UErrorCode* status );
 ...
 *    @stable ICU 3.0
 */
U_STABLE int32_t U_EXPORT2 uregex_replaceFirst( URegularExpression* regexp,
                                                const UChar* replacementText,
                                                int32_t replacementLength,
                                                UChar* destBuf,
                                                int32_t destCapacity,
 ...
 *    @stable ICU 4.6
 */
U_STABLE UText* U_EXPORT2 uregex_replaceFirstUText( URegularExpression* regexp,
                                                    UText* replacement,
                                                    UText* dest,
                                                    UErrorCode* status );
 ...
 * at the start of the current match, is appended to the destination string.
 * Then the replacement string is appended to the output string, including
 * handling any substitutions of captured text.</p>
 *
 ...
 *   to for the next.  If the destination buffer is exhausted partway through
 * such a sequence, a U_BUFFER_OVERFLOW_ERROR status will be returned.  Normal
 *   ICU conventions are for a function to perform no action if it is
 *   called with an error status, but for this one case,
 * uregex_appendRepacement() will operate normally so that buffer size
 * computations will complete correctly.
 *
 ...
 *   @param   replacementText The string that will replace the matched portion
 * of the input string as it is copied to the destination buffer. The
 * replacement text may contain references ($1, for example) to capture groups
 * from the match.
 ...
 *                        parameter will be updated to reflect the space
 * remaining unused in the output buffer.
 *   @param   status      A reference to a UErrorCode to receive any errors.
 ...
 * replacement text may contain references ($1, for example) to capture groups
 * from the match.
 *   @param   dest        A mutable UText that will receive the result. Must not
 * be NULL.
 *   @param   status      A reference to a UErrorCode to receive any errors.
 *
 *   @stable ICU 4.6
 ...
 *   @stable ICU 3.0
 */
U_STABLE int32_t U_EXPORT2 uregex_appendTail( URegularExpression* regexp,
                                              UChar** destBuf,
                                              int32_t* destCapacity,
                                              UErrorCode* status );
 ...
 *   @stable ICU 4.6
 */
U_STABLE UText* U_EXPORT2 uregex_appendTailUText( URegularExpression* regexp,
                                                  UText* dest,
                                                  UErrorCode* status );
 ...
 *                         If destCapacity is too small, requiredCapacity will
 * return the total capacity required to hold all of the output, and a
 ...
 *    @param   destFieldsCapacity  The number of elements in the destFields
 * array. If the number of fields found is less than destFieldsCapacity, the
 * extra destFields elements are set to zero. If destFieldsCapacity is too
 * small, the trailing part of the input, including any field delimiters, is
 * treated as if it were the last field - it is copied to the destBuf, and its
 * position is in the destBuf is stored in the last element of destFields.  This
 * behavior mimics that of Perl.  It is not an error condition, and no error
 * status is returned when all destField positions are used.
 * @param status  A reference to a UErrorCode to receive any errors.
 ...
 * uregex_split(); instead, it is based on (and implemented directly on top of)
 * the C++ split method.
 *
 * @param regexp  The compiled regular expression.
 * @param destFields    An array of mutable UText structs to receive the results
 ...
 *                of fields, the trailing part of the input string, including
 * any field delimiters, is placed in the last destination string. This behavior
 * mimics that of Perl.  It is not  an error condition, and no error status is
 * returned when all destField positions are used.
 * @param status  A reference to a UErrorCode to receive any errors.
 * @return        The number of fields into which the input string was split.
 *
 * @stable ICU 4.6
 */
U_STABLE int32_t U_EXPORT2 uregex_splitUText( URegularExpression* regexp,
                                              UText* destFields[],
                                              int32_t destFieldsCapacity,
                                              UErrorCode* status );

/**
 * Set a processing time limit for match operations with this
 * URegularExpression.
 *
 ...
 * @stable ICU 4.0
 */
U_STABLE void U_EXPORT2 uregex_setTimeLimit( URegularExpression* regexp,
                                             int32_t limit,
                                             UErrorCode* status );
 ...
 */
U_STABLE int32_t U_EXPORT2
uregex_getTimeLimit( const URegularExpression* regexp, UErrorCode* status );

/**
 * Set the amount of heap storage available for use by the match backtracking
 * stack. <p> ICU uses a backtracking regular expression engine, with the
 * backtrack stack maintained on the heap.  This function sets the limit to the
 * amount of memory that can be used  for this purpose.  A backtracking stack
 * overflow will result in an error from the match operation that caused it. <p>
 ...
 * stack. A value of zero means no limit. The limit must be greater than or
 * equal to zero.
 * @param   status      A reference to a UErrorCode to receive any errors.
 *
 * @stable ICU 4.0
 ...
 */
U_STABLE int32_t U_EXPORT2
uregex_getStackLimit( const URegularExpression* regexp, UErrorCode* status );
 ...
 */
U_CDECL_BEGIN
typedef UBool U_CALLCONV URegexMatchCallback( const void* context,
                                              int32_t steps );
U_CDECL_END
 ...
 * @stable ICU 4.0
 */
U_STABLE void U_EXPORT2 uregex_setMatchCallback( URegularExpression* regexp,
 ...
 * and for operations that depend on find, such as findNext, split and some
 * replace operations like replaceFirst. The callback will usually be called
 * after each attempt at a match, but this is not a guarantee that the callback
 * will be invoked at each character.  For finds where the match engine is
 * invoked at each character, this may be close to true, but less likely for
 * more optimized loops where the pattern is known to only start, and the match
 * engine invoked, at certain characters.
 * When invoked, this callback will specify the index at which a match operation
 * is about to be attempted, giving the application the opportunity to terminate
 * a long-running find operation.
 *
 * If the call back function returns FALSE, the find operation will be
 * terminated early.
 *
 * Note:  the callback function must not call other functions on this
 *        URegularExpression
 ...
 * @param matchIndex  the next index at which a match attempt will be attempted
 * for this find operation.  If this callback interrupts the search, this is the
 ...
 */
U_CDECL_BEGIN
typedef UBool U_CALLCONV URegexFindProgressCallback( const void* context,
                                                     int64_t matchIndex );
U_CDECL_END
 ...
 *                      callback function.
 * @param   context     Out parameter, receives the user context pointer that
 *                      was set when uregex_setFindProgressCallback() was
 * called.
 * @param   status      A reference to a UErrorCode to receive any errors.
 * @stable ICU 4.6
 */
