...
public:
     ...
     * to *result_capacity. Guarantees *result_capacity>=min_capacity. May
     * return a pointer to the caller-owned scratch buffer which must have
     ...
     *
     * If the ByteSink allocates or reallocates an internal buffer, it should
     * use the desired_capacity_hint if appropriate. If a caller cannot provide
     * a reasonable guess at the desired capacity, it should pass
...
public:
    ...
    /**
     * Returns a writable buffer for appending and writes the buffer's capacity
     * to *result_capacity. For details see the base class documentation.
     * @param min_capacity required minimum capacity of the returned buffer;
     *                     must be non-negative
     * @param desired_capacity_hint desired capacity of the returned buffer;
     *                              must be non-negative
...
public:
     ...
     *
     * @param dest pointer to string object to append to
     * @param initialAppendCapacity capacity beyond dest->length() to be
     * reserve()d
     * @stable ICU 60
     */
    StringByteSink( StringClass* dest, int32_t initialAppendCapacity )
        ...
        if ( initialAppendCapacity > 0 &&
             ( uint32_t )initialAppendCapacity >
