...
#define U_USRDATA_NAME \
    "usrdt" U_ICU_VERSION_SHORT U_ICUDATA_TYPE_LETTER /**< @internal */
#define U_USE_USRDATA 0                               /**< @internal */
#endif                                                /* U_HIDE_INTERNAL_API */

/**
 *  U_ICU_ENTRY_POINT is the name of the DLL entry point to the ICU data
 * library. Defined as a literal, not a string. Tricky Preprocessor use - ##
 * operator replaces macro parameters with the literal string from the
 * corresponding macro invocation, _before_ other macro substitutions. Need a
 * nested \#defines to get the actual version numbers rather than the literal
 * text U_ICU_VERSION_MAJOR_NUM into the name. The net result will be something
 * of the form
 *                                  \#define U_ICU_ENTRY_POINT icudt19_dat
 * @stable ICU 2.4
 */
#define U_ICUDATA_ENTRY_POINT \
    U_DEF2_ICUDATA_ENTRY_POINT( U_ICU_VERSION_MAJOR_NUM, U_LIB_SUFFIX_C_NAME )
 ...
 * @internal
 */
#define U_DEF2_ICUDATA_ENTRY_POINT( major, suff ) \
    U_DEF_ICUDATA_ENTRY_POINT( major, suff )
...
typedef enum UErrorCode {
     ...
     * weird and is that way because VC++ debugger displays first encountered
     * constant, which is not the what the code is used for
     */

    U_USING_FALLBACK_WARNING = -128, /**< A resource bundle lookup returned a
                                        fallback result (not an error) */
    ...
    U_SAFECLONE_ALLOCATED_WARNING =
        -126, /**< A SafeClone operation required allocating memory
                 (informational only) */

    U_STATE_OLD_WARNING =
        -125, /**< ICU has to use compatibility layer to construct the service.
                 Expect performance/memory usage degradation. Consider upgrading
               */

    U_STRING_NOT_TERMINATED_WARNING =
        -124, /**< An output string could not be NUL-terminated because output
                 length==destCapacity. */
    ...
    U_PLUGIN_CHANGED_LEVEL_WARNING =
        -120, /**< A plugin caused a level change. May not be an error, but
                 later plugins may not load. */
    ...
    U_INVALID_CHAR_FOUND = 10, /**< Character conversion: Unmappable input
                                  sequence. In other APIs: Invalid character. */
    ...
    U_NO_SPACE_AVAILABLE = 20, /**< No space available for in-buffer expansion
                                  for Arabic shaping */
    U_CE_NOT_FOUND_ERROR = 21, /**< Currently used only while setting variable
                                  top, but can be used generally */
    U_PRIMARY_TOO_LONG_ERROR = 22, /**< User tried to set variable top to a
                                      primary that is longer than two bytes */
    U_STATE_TOO_OLD_ERROR = 23,    /**< ICU cannot construct a service from this
                                      state, as it is no longer supported */
    U_TOO_MANY_ALIASES_ERROR =
        24, /**< There are too many aliases in the path to the requested
               resource. It is very possible that a circular alias definition
               has occurred */
    U_ENUM_OUT_OF_SYNC_ERROR =
        25, /**< UEnumeration out of sync with underlying collection */
    U_INVARIANT_CONVERSION_ERROR = 26, /**< Unable to convert a UChar* string to
                                          char* with the invariant converter. */
    U_INVALID_STATE_ERROR = 27, /**< Requested operation can not be completed
                                   with ICU in its current state */
    ...
    U_UNDEFINED_SEGMENT_REFERENCE, /**< A segment reference does not correspond
    ...
    U_VARIABLE_RANGE_EXHAUSTED,  /**< Too many stand-ins generated for the given
                                    variable range */
    U_VARIABLE_RANGE_OVERLAP, /**< The variable range overlaps characters used
                                 in rules */
    ...
    /*
     * Error codes in the range 0x10100 0x10200 are reserved for the formatting
     * API.
     */
    ...
    U_MULTIPLE_EXPONENTIAL_SYMBOLS, /**< More than one exponent symbol in number
                                       pattern */
    ...
    U_ARGUMENT_TYPE_MISMATCH,  /**< Argument name and argument index mismatch in
    ...
    U_NUMBER_ARG_OUTOFBOUNDS_ERROR, /**< The argument to a NumberFormatter
                                       ...
                                       bounds are usually 0 to 999. @stable ICU
                                       61 */
    U_NUMBER_SKELETON_SYNTAX_ERROR, /**< The number skeleton passed to C++
                                       NumberFormatter or C UNumberFormatter was
                                       invalid or contained a syntax error.
                                       @stable ICU 62 */
#ifndef U_HIDE_DEPRECATED_API
    /**
    ...
    U_REGEX_INVALID_STATE, /**< RegexMatcher in invalid state for requested
                              operation */
    U_REGEX_BAD_ESCAPE_SEQUENCE, /**< Unrecognized backslash escape sequence in
                                    pattern  */
    ...
    U_REGEX_STOPPED_BY_CALLER, /**< Matching operation aborted by user callback
                                  fn.    */
    U_REGEX_PATTERN_TOO_BIG, /**< Pattern exceeds limits on size or complexity.
                                @stable ICU 55 */
    U_REGEX_INVALID_CAPTURE_GROUP_NAME, /**< Invalid capture group name. @stable
                                           ICU 55 */
#ifndef U_HIDE_DEPRECATED_API
    /**
    ...
    /*
     * Error codes in the range 0x10400-0x104ff are reserved for IDNA related
     * error codes.
     */
    ...
    /*
     * Error codes in the range 0x10500-0x105ff are reserved for Plugin related
     * error codes.
     */
    U_PLUGIN_ERROR_START =
        0x10500, /**< Start of codes indicating plugin failures */
    U_PLUGIN_TOO_HIGH =
        0x10500, /**< The plugin's level is too high to be loaded right now. */
    U_PLUGIN_DIDNT_SET_LEVEL, /**< The plugin didn't call uplug_setPlugLevel in
                                 response to a QUERY */
#ifndef U_HIDE_DEPRECATED_API
    /**
