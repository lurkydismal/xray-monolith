 ...
 * Fast map from Unicode code points (U+0000..U+10FFFF) to 32-bit integer
 * values. For details see http://site.icu-project.org/design/struct/utrie
 *
...
/**
 * Creates a mutable trie that initially maps each Unicode code point to the
 * same value. It uses 32-bit data values until umutablecptrie_buildImmutable()
 * is called. umutablecptrie_buildImmutable() takes a valueWidth parameter which
 ...
 * @param errorValue the value for out-of-range code points and ill-formed
 * UTF-8/16
 * @param pErrorCode an in/out ICU UErrorCode
 * @return the trie
 * @stable ICU 63
 */
U_CAPI UMutableCPTrie* U_EXPORT2 umutablecptrie_open( uint32_t initialValue,
                                                      uint32_t errorValue,
 ...
 * @stable ICU 63
 */
U_DEFINE_LOCAL_OPEN_POINTER( LocalUMutableCPTriePointer,
                             UMutableCPTrie,
 ...
 * @stable ICU 63
 */
U_CAPI uint32_t U_EXPORT2 umutablecptrie_get( const UMutableCPTrie* trie,
                                              UChar32 c );
 ...
 * @stable ICU 63
 */
U_CAPI UChar32 U_EXPORT2 umutablecptrie_getRange( const UMutableCPTrie* trie,
                                                  UChar32 start,
 ...
 * @stable ICU 63
 */
U_CAPI void U_EXPORT2 umutablecptrie_set( UMutableCPTrie* trie,
                                          UChar32 c,
                                          uint32_t value,
                                          UErrorCode* pErrorCode );
 ...
 * Faster and more space-efficient than setting the value for each code point
 * separately.
 *
 * @param trie the trie
 ...
 * @stable ICU 63
 */
U_CAPI void U_EXPORT2 umutablecptrie_setRange( UMutableCPTrie* trie,
                                               UChar32 start,
                                               UChar32 end,
                                               uint32_t value,
                                               UErrorCode* pErrorCode );
...
/**
 * Compacts the data and builds an immutable UCPTrie according to the
 * parameters. After this, the mutable trie will be empty.
 *
 ...
 * (Some implementations may want to make productive temporary use of the upper
 ...
 * structure limits. In such a case this function will fail with a
 * U_INDEX_OUTOFBOUNDS_ERROR.
 *
 * @param trie the trie trie
 * @param type selects the trie type
 * @param valueWidth selects the number of bits in a trie data value; if smaller
 * than 32 bits, then the values stored in the trie will be truncated first
 * @param pErrorCode an in/out ICU UErrorCode
 *
...
umutablecptrie_buildImmutable( UMutableCPTrie* trie,
                               UCPTrieType type,
