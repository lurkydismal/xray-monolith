 ...
 * The public constructor and the copy constructor only alias the data (only
 * copy the pointer). There is no assignment operator.
 *
...
public:
     ...
     *
     * The trieBytes must contain a copy of a byte sequence from the
     * BytesTrieBuilder, starting with the first byte of that sequence. The
     * BytesTrie object will not read more bytes than the BytesTrieBuilder
     * generated in the corresponding build() call.
     *
    ...
    uint64_t getState64() const {
        return ( static_cast< uint64_t >( remainingMatchLength_ + 2 )
    ...
    BytesTrie& resetToState64( uint64_t state ) {
        remainingMatchLength_ =
            static_cast< int32_t >( state >> kState64RemainingShift ) - 2;
    ...
    /**
     * Determines whether the byte sequence so far matches, whether it has a
     * value, and whether another input byte can continue a matching byte
     * sequence.
     ...
     * @param inByte Input byte value. Values -0x100..-1 are treated like
     * 0..0xff. Values below -0x100 and above 0xff will never match.
     * @return The match/value Result.
     * @stable ICU 4.8
     */
     ...
     * @param length The length of the byte sequence. Can be -1 if
     * NUL-terminated.
     * @return The match/value Result.
     * @stable ICU 4.8
     */
     ...
     * USTRINGTRIE_FINAL_VALUE. getValue() can be called multiple times.
     *
     * Do not call getValue() after USTRINGTRIE_NO_MATCH or
     ...
     * @param uniqueValue Receives the unique value, if this function returns
     * TRUE. (output-only)
    ...
    inline UBool hasUniqueValue( int32_t& uniqueValue ) const {
        ...
        return pos != NULL && findUniqueValue( pos + remainingMatchLength_ + 1,
                                               FALSE, uniqueValue );
    }

    /**
     * Finds each byte which continues the byte sequence from the current state.
     * That is, each byte b for which it would be next(b)!=USTRINGTRIE_NO_MATCH
     * now.
    ...
    public:
         ...
         * @param maxStringLength If 0, the iterator returns full strings/byte
         * sequences. Otherwise, the iterator returns strings with this maximum
         * length.
         * @param errorCode Standard ICU error code. Its input value must
         *                  pass the U_SUCCESS() test, or else the function
         * returns immediately. Check for U_FAILURE() on output or use with
         *                  function chaining. (See User Guide for details.)
         * @stable ICU 4.8
         */
        Iterator( const void* trieBytes,
        ...
        /**
         * @return The NUL-terminated byte sequence for the last successful
         * next().
         * @stable ICU 4.8
         */
    ...
    private:
        ...
        const uint8_t* branchNext( const uint8_t* pos,
                                   int32_t length,
                                   UErrorCode& errorCode );
        ...
        // The second integer has the str_->length() from before the node in
        // bits 15..0, and the remaining branch length in bits 24..16.
        // (Bits 31..25 are unused.) (We could store the remaining branch length
        // minus 1 in bits 23..16 and not use bits 31..24, but the code looks
...
private:
    ...
    static inline UStringTrieResult valueResult( int32_t node ) {
        return ( UStringTrieResult )( USTRINGTRIE_INTERMEDIATE_VALUE -
                                      ( node & kValueIsFinal ) );
    }

    // Handles a branch node for both next(byte) and next(string).
    UStringTrieResult branchNext( const uint8_t* pos,
                                  int32_t length,
                                  int32_t inByte );
    ...
    // from a branch.
    static const uint8_t* findUniqueValueFromBranch( const uint8_t* pos,
                                                     int32_t length,
                                                     UBool haveUniqueValue,
                                                     int32_t& uniqueValue );
    // Recursively finds a unique value (or whether there is not a unique one)
    // starting from a position on a node lead byte.
    static UBool findUniqueValue( const uint8_t* pos,
                                  UBool haveUniqueValue,
                                  int32_t& uniqueValue );

    // Helper functions for getNextBytes().
    // getNextBytes() when pos is on a branch node.
    static void getNextBranchBytes( const uint8_t* pos,
                                    int32_t length,
                                    ByteSink& out );
    ...
    // string/byte sequence matching. The root node is at the beginning of the
    // trie data.
    //
    ...
    //
    // Node types:
    //  - Value node: Stores a 32-bit integer in a compact, variable-length
    //  format.
    ...
    //  - Branch node: Branches to other nodes according to the current input
    //  byte.
    //    The node byte is the length of the branch (number of bytes to select
    //    from) minus 1. It is followed by a sub-node:
    //    - If the length is at most kMaxBranchLinearSubNodeLength, then
    //      there are length-1 (key, value) pairs and then one more comparison
    //      byte. If one of the key bytes matches, then the value is either a
    //      final value for the string/byte sequence so far, or a "jump" delta
    //      to the next node. If the last byte matches, then matching continues
    //      with the next node. (Values have the same encoding as value nodes.)
    //    - If the length is greater than kMaxBranchLinearSubNodeLength, then
    //      there is one byte and one "jump" delta.
    //      If the input byte is less than the sub-node byte, then "jump" by
    //      delta to the next sub-node which will have a length of length/2.
    ...
    // 10..1f: Linear-match node, match 1..16 bytes and continue reading the
    // next node.
    static const int32_t kMinLinearMatch = 0x10;
    static const int32_t kMaxLinearMatchLength = 0x10;

    // 20..ff: Variable-length value node.
    // If odd, the value is final. (Otherwise, intermediate value or jump
    // delta.) Then shift-right by 1 bit. The remaining lead byte value
    // indicates the number of following bytes (0..4) and contains the value's
    // top bits.
    static const int32_t kMinValueLead =
        kMinLinearMatch + kMaxLinearMatchLength; // 0x20
    // It is a final value if bit 0 is set.
    static const int32_t kValueIsFinal = 1;

    // Compact value: After testing bit 0, shift right by 1 and then use the
    // following thresholds.
    static const int32_t kMinOneByteValueLead = kMinValueLead / 2; // 0x10
    static const int32_t kMaxOneByteValue =
    ...
    static const int32_t kMaxTwoByteDelta =
        ( ( kMinThreeByteDeltaLead - kMinTwoByteDeltaLead ) << 8 ) -
        1; // 0x2fff
    static const int32_t kMaxThreeByteDelta =
        ( ( kFourByteDeltaLead - kMinThreeByteDeltaLead ) << 16 ) -
        1; // 0xdffff
    ...
    static constexpr uint64_t kState64PosMask =
    ...
    const uint8_t* pos_;
    // Remaining length of a linear-match node, minus 1. Negative if not in such
    // a node.
    int32_t remainingMatchLength_;
};
