 ...
 * (milliseconds since midnight, January 1, 1970) to text is known as
 * "formatting," and converting from text to millis is known as "parsing."  We
 * currently define only one concrete structure UDateFormat, which can handle
 * pretty much all normal date formatting and parsing actions. <P> Date Format
 * helps you to format and parse dates for any locale. Your code can be
 * completely independent of the locale conventions for months, days of the
 ...
 *  UChar *myString;
 *  int32_t myStrlen = 0;
 *  UDateFormat* dfmt = udat_open(UDAT_DEFAULT, UDAT_DEFAULT, NULL, NULL, -1,
 * NULL, -1, &status); myStrlen = udat_format(dfmt, myDate, NULL, myStrlen,
 * NULL, &status); if (status==U_BUFFER_OVERFLOW_ERROR){ status=U_ZERO_ERROR;
 ...
 *  UDateFormat* df = udat_open(UDAT_DEFAULT, UDAT_DEFAULT, NULL, NULL, -1,
 * NULL, 0, &status); for (i = 0; i < 3; i++) { myStrlen = udat_format(df,
 * myDateArr[i], NULL, myStrlen, NULL, &status); if(status ==
 * U_BUFFER_OVERFLOW_ERROR){ status = U_ZERO_ERROR; myString =
 * (UChar*)malloc(sizeof(UChar) * (myStrlen+1) ); udat_format(df, myDateArr[i],
 * myString, myStrlen+1, NULL, &status); printf("%s\n", u_austrcpy(buffer,
 * myString) ); free(myString);
 *      }
 *  }
 * \endcode
 * </pre>
 ...
 *  UDateFormat* dfmt = udat_open(UDAT_DEFAULT, UDAT_DEFAULT, NULL, -1, NULL, 0,
 * &status); myStrlen = udat_format(dfmt, myDate, NULL, myStrlen, &pos,
 * &status); if (status==U_BUFFER_OVERFLOW_ERROR){ status=U_ZERO_ERROR;
 *      myString=(UChar*)malloc(sizeof(UChar) * (myStrlen+1) );
 ...
 * <pre>
 * \code
 *        UDateFormat* df = udat_open(UDAT_SHORT, UDAT_SHORT, "fr_FR", NULL, -1,
 * NULL, 0, &status);
 * \endcode
 * </pre>
 ...
 * <P>
 * You can also use forms of the parse and format methods with Parse Position
 * and UFieldPosition to allow you to <ul type=round> <li>   Progressively parse
 * through pieces of a string. <li>   Align any particular field, or find out
 * where it is for selection on the screen.
 * </ul>
 ...
 * <code>UDateFormat</code> supports the date and time formatting algorithm and
 * pattern letters defined by <a
...
/**
 * Constant for date skeleton with hour, with the locale's preferred hour format
 * (12 or 24).
 * @stable ICU 4.0
 */
#define UDAT_HOUR "j"
...
/**
 * Constant for date skeleton with hour and minute, with the locale's preferred
 * hour format (12 or 24). Used in combinations date + time, date + time + zone,
 * or time + zone.
 * @stable ICU 4.0
 */
 ...
 * @see <a href="http://unicode.org/reports/tr35/#Date_Format_Patterns">LDML
 * Date Format Patterns</a>
 * @see <a href="http://unicode.org/reports/tr35/#Time_Zone_Fallback">LDML Time
 * Zone Fallback</a>
 * @stable ICU 51
 */
 ...
 * such as PT; used in combinations date + time + zone, or time + zone.
 * @see <a href="http://unicode.org/reports/tr35/#Date_Format_Patterns">LDML
 * Date Format Patterns</a>
 * @see <a href="http://unicode.org/reports/tr35/#Time_Zone_Fallback">LDML Time
 * Zone Fallback</a>
...
/**
 * Constant for <i>specific non-location format</i>, such as Pacific Daylight
 * Time; used in combinations date + time + zone, or time + zone.
 * @see <a href="http://unicode.org/reports/tr35/#Date_Format_Patterns">LDML
 * Date Format Patterns</a>
 * @see <a href="http://unicode.org/reports/tr35/#Time_Zone_Fallback">LDML Time
 * Zone Fallback</a>
...
/**
 * Constant for <i>specific non-location format</i>, abbreviated if possible,
 * such as PDT; used in combinations date + time + zone, or time + zone.
 * @see <a href="http://unicode.org/reports/tr35/#Date_Format_Patterns">LDML
 * Date Format Patterns</a>
 * @see <a href="http://unicode.org/reports/tr35/#Time_Zone_Fallback">LDML Time
 * Zone Fallback</a>
 ...
 * @deprecated ICU 50 Use instead UDAT_HOUR_MINUTE UDAT_ABBR_GENERIC_TZ or some
 * other timezone presentation.
 */
#define UDAT_HOUR_MINUTE_GENERIC_TZ "jmv"
/**
 * Constant for date skeleton with hour, minute, and timezone.
 * @deprecated ICU 50 Use instead UDAT_HOUR_MINUTE UDAT_ABBR_SPECIFIC_TZ or some
 * other timezone presentation.
 */
#define UDAT_HOUR_MINUTE_TZ "jmz"
/**
 * Constant for date skeleton with hour and generic timezone.
 * @deprecated ICU 50 Use instead UDAT_HOUR UDAT_ABBR_GENERIC_TZ or some other
 * timezone presentation.
 */
#define UDAT_HOUR_GENERIC_TZ "jv"
/**
 * Constant for date skeleton with hour and timezone.
 * @deprecated ICU 50 Use instead UDAT_HOUR UDAT_ABBR_SPECIFIC_TZ or some other
 * timezone presentation.
 */
 ...
 * When the pattern parameter is used, pass in UDAT_PATTERN for both timeStyle
 * and dateStyle.
 * @param dateStyle The style used to format dates; one of UDAT_FULL, UDAT_LONG,
 * UDAT_MEDIUM, UDAT_SHORT, UDAT_DEFAULT, UDAT_FULL_RELATIVE,
 * UDAT_LONG_RELATIVE, UDAT_MEDIUM_RELATIVE, UDAT_SHORT_RELATIVE, or UDAT_NONE.
 * When the pattern parameter is used, pass in UDAT_PATTERN for both timeStyle
 * and dateStyle. As currently implemented, relative date formatting only
 * affects a limited range of calendar days before or after the current date,
 * based on the CLDR &lt;field type="day"&gt;/&lt;relative&gt; data: For
 * example, in English, "Yesterday", "Today", and "Tomorrow". Outside of this
 * range, dates are formatted using the corresponding non-relative style.
 ...
 * @param patternLength The number of characters in the pattern, or -1 if
 * null-terminated.
 * @param status A pointer to an UErrorCode to receive any errors
 * @return A pointer to a UDateFormat to use for formatting dates and times, or
...
typedef enum UDateFormatBooleanAttribute {
    ...
    /**
     * indicates tolerance of numeric data when String data may be assumed. eg:
     * UDAT_YEAR_NAME_FIELD, UDAT_STANDALONE_MONTH_FIELD, UDAT_DAY_OF_WEEK_FIELD
     * @stable ICU 53
     */
    UDAT_PARSE_ALLOW_NUMERIC = 1,
    /**
     * indicates tolerance of a partial literal match
     * e.g. accepting "--mon-02-march-2011" for a pattern of "'--:
     * 'EEE-WW-MMMM-yyyy"
     * @stable ICU 56
     */
    UDAT_PARSE_PARTIAL_LITERAL_MATCH = 2,
    /**
     * indicates tolerance of pattern mismatch between input data and specified
     * format pattern. e.g. accepting "September" for a month pattern of MMM
     * ("Sep")
     * @stable ICU 56
     */
 ...
 * An example would be a true value for a key of UDAT_PARSE_ALLOW_WHITESPACE
 * indicating allowing whitespace leniency. If the formatter does not understand
 * the attribute, -1 is returned.
 ...
 * An example of a boolean attribute is parse leniency control.  If the
 * formatter does not understand the attribute, the call is ignored.
 ...
 * @stable ICU 2.0
 */
U_CAPI UDateFormat* U_EXPORT2 udat_clone( const UDateFormat* fmt,
                                          UErrorCode* status );

/**
 * Format a date using a UDateFormat.
 * The date will be formatted using the conventions specified in {@link
 * #udat_open }
 ...
 * #ufieldpositer_open} (may be NULL if field position information is not
 * needed). Any iteration information already present in the
 * UFieldPositionIterator will be deleted, and the iterator will be reset to
 * apply to the fields in the formatted string created by this function call;
 * the field values provided by {@link #ufieldpositer_next} will be from the
 ...
 *          The formatter to use
 * @param calendar
 *          The calendar to format. The calendar instance might be mutated if
 * fields are not yet fully calculated, though the function won't change the
 * logical date and time held by the instance.
 * @param result
 *          A pointer to a buffer to receive the formatted number.
 * @param capacity
 *          The maximum size of result.
 ...
 * }. <P> Note that the normal date formats associated with some calendars -
 * such as the Chinese lunar calendar - do not specify enough fields to enable
 ...
 * offset at which to begin parsing.  If not 0, on output the offset at which
 * parsing ended.
 * @param status A pointer to an UErrorCode to receive any errors
 ...
 * do not precisely match the pattern. With strict parsing, inputs must match
 * the pattern.
 * @param fmt The formatter to query
 ...
 * @stable ICU 2.0
 */
U_CAPI void U_EXPORT2 udat_setCalendar( UDateFormat* fmt,
 ...
 * "yy", "yyy") If the field is not numeric, then override has no effect (like
 * "MMM" will use abbreviation, not numerical field)
 *
 * @param fields the fields to set
 * @param fmt The formatter to set.
 * @param numberFormatToSet A pointer to the UNumberFormat to be used by fmt to
 * format numbers.
 ...
 * for example the day number.
 * @param fmt The formatter to set.
 * @param numberFormatToAdopt A pointer to the UNumberFormat to be used by fmt
 * to format numbers.
 * @see udat_getNumberFormat
 * @stable ICU 54
 */
U_CAPI void U_EXPORT2
udat_adoptNumberFormat( UDateFormat* fmt, UNumberFormat* numberFormatToAdopt );
/**
 * Get a locale for which date/time formatting patterns are available.
 * A UDateFormat in a locale returned by this function will perform the correct
 * formatting and parsing for the locale.
 * @param localeIndex The index of the desired locale.
 * @return A locale for which date/time formatting patterns are available, or 0
 * if none.
 ...
 * @return The number of locales for which date/time formatting patterns are
 * available.
...
typedef enum UDateFormatSymbolType {
    ...
    UDAT_STANDALONE_SHORTER_WEEKDAYS,
    /**
     * Cyclic year names (only supported for some calendars, and only for FORMAT
     * usage; udat_setSymbols not supported for UDAT_CYCLIC_YEARS_WIDE)
     * @stable ICU 54
     */
    ...
    /**
     * Cyclic year names (only supported for some calendars, and only for FORMAT
     * usage; udat_setSymbols not supported for UDAT_CYCLIC_YEARS_NARROW)
     * @stable ICU 54
     */
    UDAT_CYCLIC_YEARS_NARROW,
    /**
     * Calendar zodiac  names (only supported for some calendars, and only for
     * FORMAT usage; udat_setSymbols not supported for UDAT_ZODIAC_NAMES_WIDE)
     * @stable ICU 54
     */
    ...
    /**
     * Calendar zodiac  names (only supported for some calendars, and only for
     * FORMAT usage; udat_setSymbols not supported for UDAT_ZODIAC_NAMES_NARROW)
     * @stable ICU 54
     */
 ...
 * @param fmt The formatter to query.
 * @param type The type of symbols to get.  One of UDAT_ERAS, UDAT_MONTHS,
 * UDAT_SHORT_MONTHS, UDAT_WEEKDAYS, UDAT_SHORT_WEEKDAYS, UDAT_AM_PMS, or
 * UDAT_LOCALIZED_CHARS
 ...
 * @param fmt The formatter to query.
 * @param type The type of symbols to count.  One of UDAT_ERAS, UDAT_MONTHS,
 * UDAT_SHORT_MONTHS, UDAT_WEEKDAYS, UDAT_SHORT_WEEKDAYS, UDAT_AM_PMS, or
 * UDAT_LOCALIZED_CHARS
 ...
 * for example month or day names.
 * @param format The formatter to set
 * @param type The type of symbols to set.  One of UDAT_ERAS, UDAT_MONTHS,
 * UDAT_SHORT_MONTHS, UDAT_WEEKDAYS, UDAT_SHORT_WEEKDAYS, UDAT_AM_PMS, or
 * UDAT_LOCALIZED_CHARS
 ...
 * @stable ICU 2.8
 */
U_CAPI const char* U_EXPORT2 udat_getLocaleByType( const UDateFormat* fmt,
 ...
 * @stable ICU 51
 */
U_CAPI void U_EXPORT2 udat_setContext( UDateFormat* fmt,
                                       UDisplayContext value,
                                       UErrorCode* status );

/**
 * Get the formatter's UDisplayContext value for the specified
 * UDisplayContextType, such as UDISPCTX_TYPE_CAPITALIZATION.
 * @param fmt The formatter to query.
 ...
 * @stable ICU 53
 */
U_CAPI UDisplayContext U_EXPORT2 udat_getContext( const UDateFormat* fmt,
 ...
 * formatting. The patterns should follow the pattern syntax rules.
 * @param format The formatter to set.
 ...
 * @see udat_open
 */
typedef UDateFormat*( U_EXPORT2* UDateFormatOpener )(
