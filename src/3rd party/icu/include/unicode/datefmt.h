 ...
 *   11/15/99    weiv        Added support for week of year/day of week
 * formatting
 ********************************************************************************
 */
...
/**
 * \cond
 * Export an explicit template instantiation. (See digitlst.h, datefmt.h, and
 * others.) (When building DLLs for Windows this is required.)
 */
#if U_PF_WINDOWS <= U_PLATFORM && U_PLATFORM <= U_PF_CYGWIN && \
    !defined( U_IN_DOXYGEN )
template class U_I18N_API
    EnumSet< UDateFormatBooleanAttribute, 0, UDAT_BOOLEAN_ATTRIBUTE_COUNT >;
#endif
/** \endcond */

/**
 * DateFormat is an abstract class for a family of classes that convert dates
 * and times from their internal representations to textual form and back again
 * in a language-independent manner. Converting from the internal representation
 ...
 * currently define only one concrete subclass of DateFormat: SimpleDateFormat,
 * which can handle pretty much all normal date formatting and parsing actions.
 * <P>
 ...
 * to produce the normal time format for that country. Use
 * createDateTimeInstance() to produce a DateFormat that formats both date and
 * time. You can pass in different options to these factory methods to control
 * the length of the result; from SHORT to MEDIUM to LONG to FULL. The exact
 * result depends on the locale, but generally: <ul type=round> <li>   SHORT is
 * completely numeric, such as 12/13/52 or 3:30pm <li>   MEDIUM is longer, such
 * as Jan 12, 1952 <li>   LONG is longer, such as January 12, 1952 or 3:30:32pm
...
public:
     ...
     * the DateTimePatterns resource. There are 4 time patterns, 4 date
     * patterns, the default date-time pattern, and 4 date-time patterns. Each
     * block of 4 values in the resource occurs in the order full, long, medium,
     * short.
     * @stable ICU 2.4
     */
    enum EStyle {
    ...
    /**
     * Equality operator.  Returns true if the two formats have the same
     * behavior.
     * @stable ICU 2.0
     */
     ...
     * Date, then it returns a failing UErrorCode.
     *
     ...
     * <P>
     * On input, the FieldPosition parameter may have its "field" member filled
     * with an enum value specifying a field.  On output, the FieldPosition will
     * be filled in with the text offsets for that field. <P> For example, given
     * a time text "1996.07.10 AD at 15:08:56 PDT", if the given
     * fieldPosition.field is UDAT_YEAR_FIELD, the offsets
     * fieldPosition.beginIndex and statfieldPositionus.getEndIndex will be set
     * to 0 and 4, respectively. <P> Notice that if the same time field appears
     * more than once in a pattern, the status will be set for the first
     * occurence of that time field. For instance, formatting a UDate to the
     * time string "1 PM PDT (Pacific Daylight Time)" using the pattern "h a z
     * (zzzz)" and the alignment field DateFormat::TIMEZONE_FIELD, the offsets
     * fieldPosition.beginIndex and fieldPosition.getEndIndex will be set to 5
     * and 8, respectively, for the first occurence of the timezone pattern
     * character 'z'.
     *
     ...
     * @param appendTo      Output parameter to receive result.
     *                      Result is appended to existing contents.
     * @param fieldPosition On input: an alignment field, if desired (see
     * examples above) On output: the offsets of the alignment field (see
     * examples above)
     * @return              Reference to 'appendTo' parameter.
     * @stable ICU 2.1
     */
    ...
    /**
     * Formats a date into a date/time string. Subclasses should implement this
     * method.
     *
     * @param cal       Calendar set to the date and time to be formatted
     *                  into a date/time string.  When the calendar type is
     ...
     * @stable ICU 2.0
     */
    static DateFormat* U_EXPORT2
    createTimeInstance( EStyle style = kDefault,
     ...
     * const locale.
     *
     * @param style     The given formatting style. For example, SHORT for
     * "M/d/yy" in the US locale. As currently implemented, relative date
     * formatting only affects a limited range of calendar days before or after
     * the current date, based on the CLDR &lt;field
     * type="day"&gt;/&lt;relative&gt; data: For example, in English,
     * "Yesterday", "Today", and "Tomorrow". Outside of this range, dates are
     ...
     */
    static DateFormat* U_EXPORT2
    createDateInstance( EStyle style = kDefault,
                        const Locale& aLocale = Locale::getDefault() );
     ...
     * given locale.
     *
     * @param dateStyle The given formatting style for the date portion of the
     * result. For example, SHORT for "M/d/yy" in the US locale. As currently
     *                  implemented, relative date formatting only affects a
     * limited range of calendar days before or after the current date, based on
     * the CLDR &lt;field type="day"&gt;/&lt;relative&gt; data: For example, in
     * English, "Yesterday", "Today", and "Tomorrow". Outside of this range,
     * dates are formatted using the corresponding non-relative style.
     * @param timeStyle The given formatting style for the time portion of the
     * result. For example, SHORT for "h:mm a" in the US locale. Relative time
     ...
     */
    static UnicodeString getBestPattern( const Locale& locale,
     ...
     */
    static DateFormat* U_EXPORT2
    createInstanceForSkeleton( const UnicodeString& skeleton,
                               UErrorCode& status );
     ...
     */
    static DateFormat* U_EXPORT2
    createInstanceForSkeleton( Calendar* calendarToAdopt,
                               const UnicodeString& skeleton,
                               const Locale& locale,
                               UErrorCode& status );
     ...
     * @param count Filled in with the number of locales in the list that is
     * returned.
     * @return the set of locales for which DateFormats are installed.  The
     * caller does NOT own this list and must not delete it.
     * @stable ICU 2.0
     */
    static const Locale* U_EXPORT2 getAvailableLocales( int32_t& count );

    /**
     * Returns whether both date/time parsing in the encapsulated Calendar
     * object and DateFormat whitespace & numeric processing is lenient.
     * @stable ICU 2.0
     */
     ...
     * future leniency control & behavior of DateFormat. For control of pre 53
     * leniency, Calendar and DateFormat whitespace & numeric tolerance, this
     * method is safe to use. However, mixing leniency control via this method
     * and modification of the newer attributes via setBooleanAttribute() may
    ...
    /**
     * Returns whether date/time parsing in the encapsulated Calendar object
     * processing is lenient.
     * @stable ICU 53
     */
    virtual UBool isCalendarLenient( void ) const;

    /**
     * Specifies whether encapsulated Calendar date/time parsing is to be
     * lenient.  With lenient parsing, the parser may use heuristics to
     * interpret inputs that do not precisely match this object's format.
     * Without lenient parsing, inputs must match this object's format more
     * closely.
     ...
     * To parse to a calendar, use {@link #parse(const UnicodeString&, Calendar&
     * cal, ParsePosition&) const parse(const UnicodeString&, Calendar& cal,
     * ParsePosition&)}
     ...
     * no longer owns the TimeZone object and should not delete it after this
     * call.
     ...
     * @stable ICU 53
     */
    virtual UDisplayContext getContext( UDisplayContextType type,
                                        UErrorCode& status ) const;
...
protected:
    /**
     * Default constructor.  Creates a DateFormat with no Calendar or
     * NumberFormat associated with it.  This constructor depends on the
     * subclasses to fill in the calendar and numberFormat fields.
     * @stable ICU 2.0
     */
    DateFormat();
...
private:
     ...
     */
    static DateFormat* U_EXPORT2 create( EStyle timeStyle,
                                         EStyle dateStyle,
