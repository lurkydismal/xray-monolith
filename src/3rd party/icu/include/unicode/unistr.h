...
// Not #ifndef U_HIDE_INTERNAL_API because UnicodeString needs the
// UStringCaseMapper.
/**
 ...
 * @internal
 */
typedef int32_t U_CALLCONV UStringCaseMapper( int32_t caseLocale,
                                              uint32_t options,
#if !UCONFIG_NO_BREAK_ITERATION
 ...
 * char * string. About invariant characters see utypes.h. This constructor has
 * no runtime dependency on conversion code and is therefore recommended over
 * ones taking a charset name string (where the empty string "" indicates
 * invariant-character conversion).
 *
 * @stable ICU 3.2
 */
...
#define UNICODE_STRING( cs, _length ) \
    icu::UnicodeString( TRUE, ( const char16_t* )u##cs, _length )
#endif
...
#else
                               // Empty by default for source code
                               // compatibility.
 ...
 * This can be defined to be empty or "explicit".
 * If explicit, then the UnicodeString(const char *) and UnicodeString(const
 * char16_t *) constructors are marked as explicit, preventing their inadvertent
 * use.
 *
 * In particular, this helps prevent accidentally depending on ICU conversion
 * code by passing a string literal into an API with a const UnicodeString &
 * parameter.
 * @stable ICU 49
 */
 ...
 * It should be a multiple of sizeof(pointer) to avoid unusable space for
 * padding. The object size may want to be a multiple of 16 bytes, which is a
 * common granularity for heap allocation.
 *
 * Any space inside the object beyond sizeof(vtable pointer) + 2
 * is available for storing short strings inside the object.
 * The bigger the object, the longer a string that can be stored inside the
 * object, without additional heap allocation.
 *
 ...
 * to hold at least 2 char16_ts, for one supplementary code point
 * (U16_MAX_LENGTH).
 *
...
/**
 * UnicodeString is a string class that stores Unicode characters directly and
 * provides similar functionality as the Java String and
 * StringBuffer/StringBuilder classes. It is a concrete implementation of the
 * abstract class Replaceable (for transliteration).
 *
 ...
 * For details see the description of storage models at the end of the class API
 * docs and in the User Guide chapter linked from there.
 *
 * The UnicodeString class is not suitable for subclassing.
 *
 * For an overview of Unicode strings in C and C++ see the
 * [User Guide Strings
 * chapter](http://userguide.icu-project.org/strings#TOC-Strings-in-C-C-).
 *
 ...
 *
 * Indexes and offsets into and lengths of strings always count code units, not
 * code points. This is the same as with multi-byte char* strings in traditional
 * string handling. Operations on partial strings typically do not test for code
 * point boundaries. If necessary, the user needs to take care of such
 * boundaries by testing for the code unit values or by using functions like
 * UnicodeString::getChar32Start() and UnicodeString::getChar32Limit()
 * (or, in C, the equivalent macros U16_SET_CP_START() and U16_SET_CP_LIMIT(),
 * see utf.h).
 *
 ...
 *   of memory, error codes in low-level C++ string methods would be
 * inconvenient, and the error code as the last parameter (ICU convention) would
 * prevent the use of default parameter values. Instead, such methods set the
 * UnicodeString into a "bogus" state (see isBogus()) if an error occurs.
 *
 ...
 * One object may be passed in as such a parameter concurrently in multiple
 * threads. This includes the const UnicodeString & parameters for copy
 * construction, assignment, and cloning.
 *
 * UnicodeString uses several storage methods.
 * String contents can be stored inside the UnicodeString object itself,
 * in an allocated and shared buffer, or in an outside buffer that is "aliased".
 * Most of this is done transparently, but careful aliasing in particular
 * provides significant performance improvements. Also, the internal buffer is
 * accessible via special functions. For details see the [User Guide Strings
 * chapter](http://userguide.icu-project.org/strings#TOC-Maximizing-Performance-with-the-UnicodeString-Storage-Model).
 *
 * @see utf.h
...
public:
     ...
     * Constant to be used in the UnicodeString(char *, int32_t, EInvariant)
     * constructor which constructs a Unicode string from an invariant-character
     * char * string. Use the macro US_INV instead of the full qualification for
     * this value.
     *
     * @see US_INV
     * @stable ICU 3.2
     ...
     * etc. if supplementary characters are present:
     *
     * In UTF-16, supplementary characters (with code points U+10000 and above)
     * are stored with pairs of surrogate code units. These have values from
     * 0xd800 to 0xdfff, which means that they compare as less than some other
     * BMP characters like U+feff. This function compares Unicode strings in
     * code point order. If either of the UTF-16 strings is malformed (i.e., it
     * contains unpaired surrogates), then the result is not defined.
     *
     ...
     * contains unpaired surrogates), then the result is not defined.
     *
     * @param start The start offset in this string at which the compare
     * operation begins.
     ...
     * @param length The number of code units from this string to compare.
     * @param srcText Another string to compare this one to.
     * @param srcStart The start offset in that string at which the compare
     * operation begins.
     ...
     * @param start The start offset in this string at which the compare
     * operation begins.
     * @param limit The offset after the last code unit from this string to
     * compare.
     * @param srcText Another string to compare this one to.
     * @param srcStart The start offset in that string at which the compare
     * operation begins.
     * @param srcLimit The offset after the last code unit from that string to
     * compare.
     * @return a negative/zero/positive integer corresponding to whether
     * this string is less than/equal to/greater than the second one
     * in code point order
     ...
     * this->foldCase(options).compare(text.foldCase(options)).
     *
     * @param text Another string to compare this one to.
     ...
     *
     * @return A negative, zero, or positive integer indicating the comparison
     * result.
     * @stable ICU 2.0
     */
    inline int8_t caseCompare( const UnicodeString& text,
                               uint32_t options ) const;

    /**
     * Compare two strings case-insensitively using full case folding.
     * This is equivalent to
     * this->foldCase(options).compare(srcText.foldCase(options)).
     *
     * @param start The start offset in this string at which the compare
     * operation begins.
     * @param length The number of code units from this string to compare.
     ...
     * Compare two strings case-insensitively using full case folding.
     * This is equivalent to
     * this->foldCase(options).compare(srcChars.foldCase(options)).
     *
     * @param srcChars A pointer to another string to compare this one to.
     * @param srcLength The number of code units from that string to compare.
     * @param options A bit set of options:
     ...
     * Compare two strings case-insensitively using full case folding.
     * This is equivalent to
     * this->foldCase(options).compareBetween(text.foldCase(options)).
     *
     * @param start The start offset in this string at which the compare
     * operation begins.
     * @param limit The offset after the last code unit from this string to
     ...
     * @return TRUE if this ends with the characters in `srcChars`, FALSE
     * otherwise
     * @stable ICU 2.0
     */
     ...
     * @param offset a valid offset after any code unit of a code point of the
     * text
     ...
     * If delta<0 then the index is moved backward (toward the start of the
     * string). If delta>0 then the index is moved forward (toward the end of
     * the string).
     *
     ...
     * forward or backward,
     * but no further backward than to 0 and no further forward than to
     * length(). The resulting index return value will be in between 0 and
     * length(), inclusively.
     *
     * Examples:
     * \code
     ...
     *     int32_t index=1;
     *
     *     // the following examples will all result in index==4, position of
     * U+10ffff
     *
     ...
     *     index=s.moveIndex32(s.length(), -2); // backward-skips U+2029 and
     * U+10ffff
     * \endcode
     *
     * @param index input code unit index
     * @param delta (signed) code point count to move the index forward or
     * backward in the string
     ...
     *
     * @param start offset of first character which will be copied into the
     * array
     ...
     * (length()==destCapacity) then the error code is set to
     * U_STRING_NOT_TERMINATED_WARNING. If the string itself does not fit into
     * dest (length()>destCapacity) then the error code is set to
     ...
     * extract(int32_t start, int32_t length, char *target, int32_t
     * targetCapacity, enum EInvariant inv) const because it avoids object code
     * dependencies of UnicodeString on the conversion code.
     *
     * @param start offset of first character which will be copied
     * @param startLength the number of characters to extract
     * @param target the target buffer for extraction
    ...
    /**
     * Convert the UnicodeString into a codepage string using an existing
     * UConverter. The output string is NUL-terminated if possible.
     *
     ...
     * @param cnv the converter object to be used (ucnv_resetFromUnicode() will
     * be called), or NULL for the default converter
     * @param errorCode normal ICU error code
     * @return the length of the output string, not counting the terminating
     * NUL; if the length is greater than destCapacity, then the string will not
     * fit and a buffer of the indicated length would need to be passed in
     * @stable ICU 2.0
     */
     ...
     * @stable ICU 4.4
     */
    UnicodeString tempSubString( int32_t start = 0,
                                 int32_t length = INT32_MAX ) const;
     ...
     * @param start offset of the first character visible in the substring
     * @param limit offset immediately following the last character visible in
     * the substring
     * @return a read-only alias UnicodeString object for the substring
     * @stable ICU 4.4
     */
    inline UnicodeString tempSubStringBetween(
        int32_t start,
        int32_t limit = INT32_MAX ) const;
     ...
     *
     * @param sink A ByteSink to which the UTF-8 version of the string is
     * written. sink.Flush() is called at the end.
     * @stable ICU 4.2
     * @see toUTF8String
     */
    ...
    /**
     * Count Unicode code points in the length char16_t code units of the
     * string. A code point may occupy either one or two char16_t code units.
     ...
     * This is more efficient than counting all code points in this part of the
     * string and comparing that number with a threshold. This function may not
     * need to scan the string at all if the length falls within a certain
     ...
     *
     * @param start the index of the first code unit to check (0 for the entire
     * string)
     * @param length the number of char16_t code units to check
     *               (use INT32_MAX for the entire string; remember that
     * start/length values are pinned)
     * @param number The number of code points in the (sub)string is compared
     * against the 'number' parameter.
     * @return Boolean value for whether the string contains more Unicode code
     * points than 'number'. Same as (u_countChar32(s, length)>number).
     * @see countChar32
     * @see u_strHasMoreChar32Than
     * @stable ICU 2.4
     */
    UBool hasMoreChar32Than( int32_t start,
                             int32_t length,
     ...
     * setToBogus() and isBogus() can be used to indicate that no string value
     * is available. For a bogus string, getBuffer() and getTerminatedBuffer()
     * return NULL, and length() returns 0.
     *
     ...
     * allocate a new buffer and copy the buffer contents even for readonly
     * aliases. By contrast, the fastCopyFrom() function implements the old,
     ...
     * Starting with ICU 2.4, the assignment operator and the copy constructor
     * allocate a new buffer and copy the buffer contents even for readonly
     * aliases. This function implements the old, more efficient but less safe
     * behavior of making this string also a readonly alias to the same buffer.
     *
     * The fastCopyFrom function must be used only if it is known that the
     * lifetime of this UnicodeString does not exceed the lifetime of the
     * aliased buffer including its contents, for example for strings from
     * resource bundles or aliases to string constants.
     *
     * If the source object has an "open" buffer from getBuffer(minCapacity),
     * then the copy is an empty string.
     *
     ...
     * This string will have the same contents and state that the source string
     * had. The behavior is undefined if *this and src are the same object.
     ...
     * @stable ICU 56
     */
    friend inline void U_EXPORT2 swap( UnicodeString& s1,
                                       UnicodeString& s2 ) U_NOEXCEPT {
        s1.swap( s2 );
    }
    ...
    /**
     * Aliasing setTo() function, analogous to the readonly-aliasing char16_t*
     * constructor. The text will be used for the UnicodeString object, but it
     * will not be released when the UnicodeString is destroyed. This has
     * copy-on-write semantics: When the string is modified, then the buffer is
     * first copied into newly allocated memory. The aliased buffer is never
     * modified.
     *
     * In an assignment to another UnicodeString, when using the copy
     * constructor or the assignment operator, the text will be copied. When
     * using fastCopyFrom(), the text will be aliased again, so that both
     * strings then alias the same readonly-text.
     *
    ...
    /**
     * Aliasing setTo() function, analogous to the writable-aliasing char16_t*
     * constructor. The text will be used for the UnicodeString object, but it
     * will not be released when the UnicodeString is destroyed. This has
     * write-through semantics: For as long as the capacity of the buffer is
     * sufficient, write operations will directly affect the buffer. When more
     * capacity is necessary, then a new buffer will be allocated and the
     * contents copied as with regularly constructed strings. In an assignment
     * to another UnicodeString, the buffer will be copied. The
     * extract(Char16Ptr dst) function detects whether the dst pointer is the
     * same as the string buffer itself and will in this case not copy the
     * contents.
     *
     ...
     *
     * The following methods, and no others, will clear a string object's bogus
     * flag:
     * - remove()
     * - remove(0, INT32_MAX)
     * - truncate(0)
     ...
     * Examples for other functions that are equivalent to "set to empty
     * string":
     * \code
     * if(s.isBogus()) {
     *   s.remove();           // set to an empty string (remove all), or
     *   s.remove(0, INT32_MAX); // set to an empty string (remove all), or
     *   s.truncate(0);        // set to an empty string (complete truncation),
     * or s=UnicodeString();    // assign an empty string, or
     *   s.setTo((UChar32)-1); // set to a pseudo code point that is out of
     * range, or s.setTo(u"", 0);      // set to an empty C Unicode string
     * }
     * \endcode
     *
     * @see isBogus()
     ...
     *                  the append string; can be -1 if `srcChars` is
     * NUL-terminated
     * @return a reference to this
     * @stable ICU 2.0
     */
    ...
    /**
     * Removes all characters from the UnicodeString object and clears the bogus
     * flag. This is the UnicodeString equivalent of std::string’s clear().
     *
     * @return a reference to this
     * @see setToBogus
     * @stable ICU 2.0
     ...
     * @stable ICU 4.4
     */
    inline UnicodeString& retainBetween( int32_t start,
     ...
     *                  Otherwise the provided iterator is set to the string's
     * text.
     * @return A reference to this.
     * @stable ICU 2.1
     */
     ...
     * text.
     * @param locale    The locale to consider.
     * @param options   Options bit set, usually 0. See
     * U_TITLECASE_NO_LOWERCASE, U_TITLECASE_NO_BREAK_ADJUSTMENT,
     * U_TITLECASE_ADJUST_TO_CASED, U_TITLECASE_WHOLE_STRING,
     ...
     * dotted I and dotless i that are marked with 'T' in CaseFolding.txt.
     *
     * The result may be longer or shorter than the original.
     *
     * @param options Either U_FOLD_CASE_DEFAULT or
     * U_FOLD_CASE_EXCLUDE_SPECIAL_I
     * @return A reference to this.
     * @stable ICU 2.0
     */
     ...
     * The buffer is guaranteed to be large enough for at least minCapacity
     * char16_ts, writable, and is still owned by the UnicodeString object.
     ...
     * For many operations, the full capacity should be used to avoid
     * reallocations.
     *
     * While the buffer is "open" between getBuffer(minCapacity)
     * and releaseBuffer(newLength), the following applies:
     * - The string length is set to 0.
     * - Any read API call on the UnicodeString object will behave like on a
     * 0-length string.
     * - Any write API call on the UnicodeString object is disallowed and will
     * have no effect.
     * - You can read from and write to the returned buffer.
     * - The previous string contents will still be in the buffer;
     *   if you want to use it, then you need to call length() before
     * getBuffer(minCapacity). If the length() was greater than minCapacity,
     * then any contents after minCapacity may be lost. The buffer contents is
     * not NUL-terminated by getBuffer(). If length() < getCapacity() then you
     * can terminate it by writing a NUL at index length().
     * - You must call releaseBuffer(newLength) before and in order to
     *   return to normal UnicodeString operation.
     *
     * @param minCapacity the minimum number of char16_ts that are to be
     * available in the buffer, starting at the returned pointer; default to the
     * current string capacity if minCapacity==-1
     ...
     * This function must be called in a matched pair with
     * getBuffer(minCapacity). releaseBuffer(newLength) must be called if and
     * only if a getBuffer(minCapacity) is "open".
     *
     * It will set the string length to newLength, at most to the current
     * capacity. If newLength==-1 then it will set the length according to the
     * first NUL in the buffer, or to the capacity if there is no NUL.
     *
     * After calling releaseBuffer(newLength) the UnicodeString is back to
     * normal operation.
     *
     * @param newLength the new length of the UnicodeString object;
     *        defaults to the current capacity if newLength is greater than
     * that; if newLength==-1, it defaults to u_strlen(buffer) but not more than
     ...
     * The pointer that it returns will remain valid until the UnicodeString
     ...
     * Note that a UnicodeString may also contain NUL characters as part of its
     * contents.
     *
     * The buffer may reside in read-only memory. Its contents must not
     * be modified.
     ...
     * @stable ICU 59
     */
    UNISTR_FROM_STRING_EXPLICIT inline UnicodeString(
     ...
     * The extract(Char16Ptr dst) function detects whether the dst pointer is
     ...
     *
     * For ASCII (really "invariant character") strings it is more efficient to
     * use the constructor that takes a US_INV (for its enum EInvariant). For
     * ASCII (invariant-character) string literals, see UNICODE_STRING and
     ...
     * @param srcLength length of the input string, can be -1 for NUL-terminated
     * strings
     ...
     * Illegal input is replaced with U+FFFD. Otherwise, errors result in a
     * bogus string. Calls u_strFromUTF8WithSub().
     *
     ...
     * Illegal input is replaced with U+FFFD. Otherwise, errors result in a
     * bogus string. Calls u_strFromUTF32WithSub().
     *
...
private:
    ...
    // this version: New API will be cleaner if we make callers create
    // substrings rather than having start+length on every method, and it should
    // take a UErrorCode&.
    int32_t toUTF8( int32_t start,
                    int32_t len,
                    char* target,
    ...
    // these do not check for kOpenGetBuffer, unlike the public getBuffer()
    // function
    ...
    // is writable unless it isBogus() or it has an "open"
    ...
    inline void setArray( char16_t* array,
                          int32_t len,
                          int32_t capacity ); // sets length but not flags
    ...
    int32_t doExtract( int32_t start,
                       int32_t length,
                       char* dest,
                       int32_t destCapacity,
                       UConverter* cnv,
    ...
    enum {
         ...
         * Must be at least U16_MAX_LENGTH for the single-code point constructor
         * to work.
         * @see UNISTR_OBJECT_SIZE
         */
        US_STACKBUF_SIZE =
            ( int32_t )( UNISTR_OBJECT_SIZE - sizeof( void* ) - 2 ) /
            U_SIZEOF_UCHAR,
        ...
        kLengthShift = 5, // remaining 11 bits for non-negative short length, or
                          // negative if long
        kLength1 = 1 << kLengthShift,
        kMaxShortLength =
            0x3ff, // max non-negative short length (leaves top bit 0)
        kLengthIsLarge = 0xffe0, // short length < 0, real length is in
                                 // fUnion.fFields.fLength
    ...
    union StackBufferOrFields; // forward declaration necessary before friend
                               // declaration
     ...
     * a multiple of sizeof(pointer) [the largest field base unit here is a data
     * pointer], rounded up with additional padding if the fields do not already
     ...
     * Both fStackFields and fFields must begin with the same fields for flags
     * and short length, that is, those must have the same memory offsets inside
     * the object, because the flags must be inspected in order to decide which
     * half of fUnion is being used. We assume that the compiler does not
     * reorder the fields.
     *
    ...
    union StackBufferOrFields {
        // fStackFields is used iff (fLengthAndFlags&kUsingStackBuffer) else
        // fFields is used. Each struct of the union must begin with
        // fLengthAndFlags.
        struct {
        ...
        struct {
            ...
            // array pointer last to minimize padding for machines with P128
            // data model or pointer sizes that are not a power of 2
...
inline void UnicodeString::pinIndices( int32_t& start,
...
inline UnicodeString::UnicodeString( std::nullptr_t /*buffer*/,
                                     int32_t /*buffLength*/,
                                     int32_t /*buffCapacity*/ ) {
    fUnion.fStackFields.fLengthAndFlags = kShortString;
}
...
inline UBool UnicodeString::isWritable() const {
    return ( UBool ) !( fUnion.fFields.fLengthAndFlags &
                        ( kOpenGetBuffer | kIsBogus ) );
}

inline UBool UnicodeString::isBufferWritable() const {
    return ( UBool )( !( fUnion.fFields.fLengthAndFlags &
                         ( kOpenGetBuffer | kIsBogus | kBufferIsReadonly ) ) &&
                      ( !( fUnion.fFields.fLengthAndFlags & kRefCounted ) ||
                        refCount() == 1 ) );
}
...
//========================================
inline int8_t UnicodeString::doCompare( int32_t start,
                                        int32_t thisLength,
                                        const UnicodeString& srcText,
                                        int32_t srcStart,
                                        int32_t srcLength ) const {
    if ( srcText.isBogus() ) {
        ...
        return doCompare( start, thisLength, srcText.getArrayStart(), srcStart,
                          srcLength );
    }
}
...
inline int8_t UnicodeString::compare( int32_t start,
                                      int32_t _length,
                                      const UnicodeString& srcText ) const {
    return doCompare( start, _length, srcText, 0, srcText.length() );
}

inline int8_t UnicodeString::compare( ConstChar16Ptr srcChars,
                                      int32_t srcLength ) const {
...
inline int8_t UnicodeString::compareBetween( int32_t start,
                                             int32_t limit,
                                             const UnicodeString& srcText,
                                             int32_t srcStart,
                                             int32_t srcLimit ) const {
    return doCompare( start, limit - start, srcText, srcStart,
                      srcLimit - srcStart );
}

inline int8_t UnicodeString::doCompareCodePointOrder(
    int32_t start,
    int32_t thisLength,
    const UnicodeString& srcText,
    int32_t srcStart,
    ...
    if ( srcText.isBogus() ) {
        ...
        srcText.pinIndices( srcStart, srcLength );
        return doCompareCodePointOrder(
            start, thisLength, srcText.getArrayStart(), srcStart, srcLength );
    }
}

inline int8_t UnicodeString::compareCodePointOrder(
    const UnicodeString& text ) const {
...
inline int8_t UnicodeString::compareCodePointOrder(
    ...
    int32_t _length,
    const UnicodeString& srcText ) const {
    return doCompareCodePointOrder( start, _length, srcText, 0,
                                    srcText.length() );
}

inline int8_t UnicodeString::compareCodePointOrder( ConstChar16Ptr srcChars,
                                                    int32_t srcLength ) const {
...
inline int8_t UnicodeString::compareCodePointOrder(
    ...
    int32_t srcStart,
    int32_t srcLength ) const {
    return doCompareCodePointOrder( start, _length, srcText, srcStart,
                                    srcLength );
}
...
inline int8_t UnicodeString::compareCodePointOrder( int32_t start,
                                                    int32_t _length,
                                                    const char16_t* srcChars,
                                                    int32_t srcStart,
                                                    int32_t srcLength ) const {
    return doCompareCodePointOrder( start, _length, srcChars, srcStart,
                                    srcLength );
}

inline int8_t UnicodeString::compareCodePointOrderBetween(
    int32_t start,
    int32_t limit,
    const UnicodeString& srcText,
    int32_t srcStart,
    int32_t srcLimit ) const {
    return doCompareCodePointOrder( start, limit - start, srcText, srcStart,
                                    srcLimit - srcStart );
}

inline int8_t UnicodeString::doCaseCompare( int32_t start,
                                            int32_t thisLength,
                                            const UnicodeString& srcText,
                                            int32_t srcStart,
                                            int32_t srcLength,
    ...
    if ( srcText.isBogus() ) {
        ...
        srcText.pinIndices( srcStart, srcLength );
        return doCaseCompare( start, thisLength, srcText.getArrayStart(),
                              srcStart, srcLength, options );
    }
}

inline int8_t UnicodeString::caseCompare( const UnicodeString& text,
                                          uint32_t options ) const {
    return doCaseCompare( 0, length(), text, 0, text.length(), options );
}

inline int8_t UnicodeString::caseCompare( int32_t start,
                                          int32_t _length,
                                          const UnicodeString& srcText,
                                          uint32_t options ) const {
    return doCaseCompare( start, _length, srcText, 0, srcText.length(),
                          options );
}

inline int8_t UnicodeString::caseCompare( ConstChar16Ptr srcChars,
                                          int32_t srcLength,
                                          uint32_t options ) const {
...
inline int8_t UnicodeString::caseCompare( int32_t start,
                                          ...
                                          int32_t srcLength,
                                          uint32_t options ) const {
    return doCaseCompare( start, _length, srcText, srcStart, srcLength,
                          options );
}
...
inline int8_t UnicodeString::caseCompare( int32_t start,
                                          ...
                                          int32_t srcLength,
                                          uint32_t options ) const {
    return doCaseCompare( start, _length, srcChars, srcStart, srcLength,
                          options );
}

inline int8_t UnicodeString::caseCompareBetween( int32_t start,
                                                 int32_t limit,
                                                 const UnicodeString& srcText,
                                                 int32_t srcStart,
                                                 int32_t srcLimit,
                                                 uint32_t options ) const {
    return doCaseCompare( start, limit - start, srcText, srcStart,
                          srcLimit - srcStart, options );
}

inline int32_t UnicodeString::indexOf( const UnicodeString& srcText,
                                       int32_t srcStart,
                                       int32_t srcLength,
                                       int32_t start,
                                       int32_t _length ) const {
    if ( !srcText.isBogus() ) {
        srcText.pinIndices( srcStart, srcLength );
        if ( srcLength > 0 ) {
            return indexOf( srcText.getArrayStart(), srcStart, srcLength, start,
                            _length );
        }
    }
    return -1;
...
inline int32_t UnicodeString::indexOf( const UnicodeString& text,
                                       int32_t start ) const {
    pinIndex( start );
...
inline int32_t UnicodeString::indexOf( const char16_t* srcChars,
                                       int32_t srcLength,
                                       int32_t start ) const {
    pinIndex( start );
...
inline int32_t UnicodeString::indexOf( ConstChar16Ptr srcChars,
                                       int32_t srcLength,
                                       int32_t start,
                                       int32_t _length ) const {
...
inline int32_t UnicodeString::indexOf( char16_t c,
                                       int32_t start,
                                       int32_t _length ) const {
    return doIndexOf( c, start, _length );
}

inline int32_t UnicodeString::indexOf( UChar32 c,
                                       int32_t start,
                                       int32_t _length ) const {
    return doIndexOf( c, start, _length );
}
...
inline int32_t UnicodeString::lastIndexOf( ConstChar16Ptr srcChars,
                                           int32_t srcLength,
                                           int32_t start,
                                           int32_t _length ) const {
...
inline int32_t UnicodeString::lastIndexOf( const char16_t* srcChars,
                                           int32_t srcLength,
                                           int32_t start ) const {
    pinIndex( start );
...
inline int32_t UnicodeString::lastIndexOf( const UnicodeString& srcText,
                                           int32_t srcStart,
                                           int32_t srcLength,
                                           int32_t start,
                                           int32_t _length ) const {
    if ( !srcText.isBogus() ) {
        srcText.pinIndices( srcStart, srcLength );
        if ( srcLength > 0 ) {
            return lastIndexOf( srcText.getArrayStart(), srcStart, srcLength,
                                start, _length );
        }
    }
    return -1;
...
inline int32_t UnicodeString::lastIndexOf( const UnicodeString& text,
                                           int32_t start,
                                           int32_t _length ) const {
...
inline int32_t UnicodeString::lastIndexOf( char16_t c,
                                           int32_t start,
                                           int32_t _length ) const {
    return doLastIndexOf( c, start, _length );
}

inline int32_t UnicodeString::lastIndexOf( UChar32 c,
                                           int32_t start,
                                           int32_t _length ) const {
    return doLastIndexOf( c, start, _length );
}
...
inline UBool UnicodeString::startsWith( const UnicodeString& srcText,
                                        int32_t srcStart,
                                        int32_t srcLength ) const {
    return doCompare( 0, srcLength, srcText, srcStart, srcLength ) == 0;
}

inline UBool UnicodeString::startsWith( ConstChar16Ptr srcChars,
                                        int32_t srcLength ) const {
    if ( srcLength < 0 ) {
...
inline UBool UnicodeString::startsWith( const char16_t* srcChars,
                                        int32_t srcStart,
                                        int32_t srcLength ) const {
    if ( srcLength < 0 ) {
        srcLength = u_strlen( toUCharPtr( srcChars ) );
...
inline UBool UnicodeString::endsWith( const UnicodeString& text ) const {
    return doCompare( length() - text.length(), text.length(), text, 0,
                      text.length() ) == 0;
}

inline UBool UnicodeString::endsWith( const UnicodeString& srcText,
                                      int32_t srcStart,
                                      int32_t srcLength ) const {
    srcText.pinIndices( srcStart, srcLength );
    return doCompare( length() - srcLength, srcLength, srcText, srcStart,
                      srcLength ) == 0;
}

inline UBool UnicodeString::endsWith( ConstChar16Ptr srcChars,
                                      int32_t srcLength ) const {
    if ( srcLength < 0 ) {
        srcLength = u_strlen( toUCharPtr( srcChars ) );
    }
    return doCompare( length() - srcLength, srcLength, srcChars, 0,
                      srcLength ) == 0;
}

inline UBool UnicodeString::endsWith( const char16_t* srcChars,
                                      int32_t srcStart,
                                      int32_t srcLength ) const {
    if ( srcLength < 0 ) {
        srcLength = u_strlen( toUCharPtr( srcChars + srcStart ) );
    }
    return doCompare( length() - srcLength, srcLength, srcChars, srcStart,
                      srcLength ) == 0;
}
...
// replace
//========================================
inline UnicodeString& UnicodeString::replace( int32_t start,
                                              int32_t _length,
                                              const UnicodeString& srcText ) {
...
inline UnicodeString& UnicodeString::replaceBetween(
    int32_t start,
    int32_t limit,
    const UnicodeString& srcText ) {
...
inline UnicodeString& UnicodeString::replaceBetween(
    ...
    int32_t srcStart,
    int32_t srcLimit ) {
    return doReplace( start, limit - start, srcText, srcStart,
                      srcLimit - srcStart );
}

inline UnicodeString& UnicodeString::findAndReplace(
    const UnicodeString& oldText,
    const UnicodeString& newText ) {
    return findAndReplace( 0, length(), oldText, 0, oldText.length(), newText,
                           0, newText.length() );
}
...
// extract
// ============================
inline void UnicodeString::doExtract( int32_t start,
                                      int32_t _length,
                                      UnicodeString& target ) const {
    target.replace( 0, target.length(), *this, start, _length );
}

inline void UnicodeString::extract( int32_t start,
                                    int32_t _length,
                                    Char16Ptr target,
...
inline int32_t UnicodeString::extract( int32_t start,
                                       int32_t _length,
                                       char* dst,
                                       const char* codepage ) const
...
inline UnicodeString UnicodeString::tempSubStringBetween(
    int32_t start,
    int32_t limit ) const {
...
inline void UnicodeString::setShortLength( int32_t len ) {
    ...
    fUnion.fFields.fLengthAndFlags =
        ( int16_t )( ( fUnion.fFields.fLengthAndFlags & kAllStorageFlags ) |
                     ( len << kLengthShift ) );
}
...
inline void UnicodeString::setArray( char16_t* array,
                                     int32_t len,
                                     int32_t capacity ) {
    setLength( len );
...
inline UnicodeString& UnicodeString::operator=( char16_t ch ) {
    return doReplace( 0, length(), &ch, 0, 1 );
}

inline UnicodeString& UnicodeString::operator=( UChar32 ch ) {
...
inline UnicodeString& UnicodeString::setTo( const UnicodeString& srcText,
    ...
    return doReplace( 0, length(), srcText, srcStart,
...
inline UnicodeString& UnicodeString::setTo( char16_t srcChar ) {
    unBogus();
    return doReplace( 0, length(), &srcChar, 0, 1 );
}

inline UnicodeString& UnicodeString::setTo( UChar32 srcChar ) {
    unBogus();
    return replace( 0, length(), srcChar );
}

inline UnicodeString& UnicodeString::append( const UnicodeString& srcText,
                                             int32_t srcStart,
                                             int32_t srcLength ) {
...
inline UnicodeString& UnicodeString::append( const char16_t* srcChars,
                                             int32_t srcStart,
                                             int32_t srcLength ) {
    return doAppend( srcChars, srcStart, srcLength );
}

inline UnicodeString& UnicodeString::append( ConstChar16Ptr srcChars,
                                             int32_t srcLength ) {
    return doAppend( srcChars, 0, srcLength );
}

inline UnicodeString& UnicodeString::append( char16_t srcChar ) {
...
inline UnicodeString& UnicodeString::operator+=(
    const UnicodeString& srcText ) {
    return doAppend( srcText, 0, srcText.length() );
}

inline UnicodeString& UnicodeString::insert( int32_t start,
                                             const UnicodeString& srcText,
                                             int32_t srcStart,
                                             int32_t srcLength ) {
...
inline UnicodeString& UnicodeString::insert( int32_t start, char16_t srcChar ) {
    return doReplace( start, 0, &srcChar, 0, 1 );
}

inline UnicodeString& UnicodeString::insert( int32_t start, UChar32 srcChar ) {
...
inline UnicodeString& UnicodeString::removeBetween( int32_t start,
                                                    int32_t limit ) {
    return doReplace( start, limit - start, NULL, 0, 0 );
}

inline UnicodeString& UnicodeString::retainBetween( int32_t start,
                                                    int32_t limit ) {
    truncate( limit );
...
inline UnicodeString& UnicodeString::reverse() {
    return doReverse( 0, length() );
}

inline UnicodeString& UnicodeString::reverse( int32_t start, int32_t _length ) {
