 ...
 * 02/12/97     aliu        Modified to create objects from RuleBasedCollator
 * cache. Moved cache out of Collation class. 02/13/97     aliu        Moved
 * several methods out of this class and into RuleBasedCollator, with
 * modifications.  Modified createDefault() to call new
 * RuleBasedCollator(Locale&) constructor.  General clean up and documentation.
 ...
 *                          UnicodeString construction and special case for
 * NO_OP. 11/23/99     srl         More performance enhancements. Inlining of
 *                          critical accessors.
...
public:
     ...
     *
     * The base class implementation returns TRUE if "other" has the same
     * type/class as "this": `typeid(*this) == typeid(other)`.
     *
     * Subclass implementations should do something like the following:
     *
     *     if (this == &other) { return TRUE; }
     *     if (!Collator::operator==(other)) { return FALSE; }  // not the same
     * class
     *
     ...
     * The UErrorCode& err parameter is used to return status information to the
     * user. To check whether the construction succeeded or not, you should
     * check the value of U_SUCCESS(err).  If you wish more detailed
     * information, you can check for informational error results which still
     * indicate success. U_USING_FALLBACK_ERROR indicates that a fall back
     * locale was used. For example, 'de_CH' was requested, but nothing was
     * found there, so 'de' was used. U_USING_DEFAULT_ERROR indicates that the
     * default locale data was used; neither the requested locale nor any of its
     * fall back locales could be found. The caller owns the returned object and
     * is responsible for deleting it.
     *
     * @param err    the error code status.
     * @return       the collation object of the default locale.(for example,
     * en_US)
     ...
     * ("el@colCaseFirst=upper") or in language tag syntax ("el-u-kf-upper").
     * See <a href="http://userguide.icu-project.org/collation/api">User Guide:
     * Collation API</a>.
     *
     * The UErrorCode& err parameter is used to return status information to the
     * user. To check whether the construction succeeded or not, you should
     ...
     * @param err    the error code status.
     * @return       the created table-based collation object based on the
     * desired locale.
     * @see Locale
     * @see ResourceLoader
     * @stable ICU 2.2
     */
    static Collator* U_EXPORT2 createInstance( const Locale& loc,
                                               UErrorCode& err );
     ...
     * than target; UCOL_EQUAL if source is equal to target; UCOL_LESS if source
     * is less than target
     * @stable ICU 2.6
     */
     ...
     * @param length the length the comparison is limited to
     * @param status possible error code
     * @return Returns an enum value. UCOL_GREATER if source (up to the
     * specified length) is greater than target; UCOL_EQUAL if source (up to
     * specified length) is equal to target; UCOL_LESS if source (up to the
     * specified length) is less  than target.
     * @stable ICU 2.6
     */
    virtual UCollationResult compare( const UnicodeString& source,
                                      const UnicodeString& target,
     ...
     * .                         Collator::createInstance(Locale::getUS(),
     * status); .       if (U_FAILURE(status)) return; .
     * myCollation->setStrength(Collator::PRIMARY); .       // result would be
     * Collator::EQUAL ("abc" == "ABC") .       // (no primary difference
     * between "abc" and "ABC") .       Collator::EComparisonResult result = .
     * myCollation->compare(abc, 3, ABC, 3); .
     * myCollation->setStrength(Collator::TERTIARY); .       // result would be
     * Collator::LESS ("abc" &lt;&lt;&lt; "ABC") .       // (with tertiary
     * difference between "abc" and "ABC") .       result =
     * myCollation->compare(abc, 3, ABC, 3);
     * </pre>
     ...
     * @deprecated ICU 2.6 use the overload with UErrorCode &
     */
    virtual EComparisonResult compare( const char16_t* source,
     ...
     *
     * Note that sort keys are often less efficient than simply doing
     * comparison. For more details, see the ICU User Guide.
     *
     ...
     * in ICU 3.0.
     */
    virtual Locale getLocale( ULocDataLocaleType type,
                              UErrorCode& status ) const = 0;
#endif // U_FORCE_HIDE_DEPRECATED_API
     ...
     *  \code
     *  UErrorCode status = U_ZERO_ERROR;
     *  Collator*myCollation = Collator::createInstance(Locale::getUS(),
     * status); if (U_FAILURE(status)) return;
     ...
     * @param destCapacity The length of dest. If it is 0, then dest may be NULL
     * and the function will only return the length of the result without
     ...
     *
     * <p>The reordering codes are a combination of script codes and reorder
     * codes.
     * @param reorderCodes An array of script codes in the new order. This can
     * be NULL if the length is also set to 0. An empty array will clear any
     * reordering codes on the collator.
     ...
     * Some reorder codes will be grouped and must reorder together. Beginning
     * with ICU 55, scripts only reorder together if they are primary-equal, for
     * example Hiragana and Katakana.
     *
     * @param reorderCode The reorder code to determine equivalence for.
     * @param dest The array to fill with the script equivalence reordering
     * codes.
     * @param destCapacity The length of dest. If it is 0, then dest may be NULL
     * and the function will only return the length of the result without
     * writing any codes (pre-flighting).
     ...
     * If collators might have been registered, use the overload of
     * getAvailableLocales that returns a StringEnumeration.</p>
     *
     ...
     * call, including registered locales.  If a severe error occurs (such as
     * out of memory condition) this will return null. If there is no locale
     * data, an empty enumeration will be returned.
     * @return a StringEnumeration over the locales available at the time of the
     * call
     * @stable ICU 2.6
     */
     ...
     * @stable ICU 3.0
     */
    static StringEnumeration* U_EXPORT2 getKeywordValues( const char* keyword,
                                                          UErrorCode& status );
     ...
     * @param keyword        one of the keys supported by this service.  For
     * now, only "collation" is supported.
     ...
     * @stable ICU 3.0
     */
    static Locale U_EXPORT2 getFunctionalEquivalent( const char* keyword,
                                                     const Locale& locale,
                                                     UBool& isAvailable,
                                                     UErrorCode& status );
     ...
     * @stable ICU 2.6
     */
    static URegistryKey U_EXPORT2 registerInstance( Collator* toAdopt,
                                                    const Locale& locale,
                                                    UErrorCode& status );
     ...
     * @stable ICU 2.6
     */
    static URegistryKey U_EXPORT2 registerFactory( CollatorFactory* toAdopt,
                                                   UErrorCode& status );
     ...
     * @stable ICU 2.2
     */
    virtual void setAttribute( UColAttribute attr,
     ...
     * If that attribute is set to UCOL_NON_IGNORABLE, then the variable top has
     * no effect.
     *
     * The base class implementation sets U_UNSUPPORTED_ERROR.
     * @param group one of UCOL_REORDER_CODE_SPACE,
     * UCOL_REORDER_CODE_PUNCTUATION, UCOL_REORDER_CODE_SYMBOL,
     * UCOL_REORDER_CODE_CURRENCY; or UCOL_REORDER_CODE_DEFAULT to restore the
     * default max variable group
     ...
     * See setMaxVariable().
     * @param varTop one or more (if contraction) char16_ts to which the
     * variable top should be set
     * @param len length of variable top string. If -1 it is considered to be
     * zero terminated.
     * @param status error code. If error code is set, the return value is
     * undefined. Errors set by this function are: <br> U_CE_NOT_FOUND_ERROR if
     * more than one character was passed and there is no such contraction<br>
     *    U_ILLEGAL_ARGUMENT_ERROR if the variable top is beyond
     *    the last reordering group supported by setMaxVariable()
     * @return variable top primary weight
     * @deprecated ICU 53 Call setMaxVariable() instead.
     */
    virtual uint32_t setVariableTop( const char16_t* varTop,
                                     int32_t len,
                                     UErrorCode& status ) = 0;
     ...
     * char16_ts to which the variable top should be set
     * @param status error code. If error code is set, the return value is
     * undefined. Errors set by this function are: <br> U_CE_NOT_FOUND_ERROR if
     * more than one character was passed and there is no such contraction<br>
     *    U_ILLEGAL_ARGUMENT_ERROR if the variable top is beyond
     ...
     * and it must not be beyond the last of those groups.
     * See setMaxVariable().
     * @param varTop primary weight, as returned by setVariableTop or
     * ucol_getVariableTop
     * @param status error code
     * @deprecated ICU 53 Call setMaxVariable() instead.
     */
     ...
     *         in the root collator. The object must be disposed of by using
     * delete
     * @stable ICU 2.4
     */
     ...
     *        If -1, the string is 0 terminated and length will be decided by
     * the function.
     * @param result buffer to store result in. If NULL, number of bytes needed
     *        will be returned.
     * @param resultLength length of the result buffer. If if not enough the
     ...
     * @stable ICU 2.2
     */
    virtual int32_t getSortKey( const char16_t* source,
                                int32_t sourceLength,
                                uint8_t* result,
                                int32_t resultLength ) const = 0;
     ...
     *                     (If an unmodified sortkey is passed, it is always
     * null terminated).
     * @param boundType Type of bound required. It can be UCOL_BOUND_LOWER,
     * which produces a lower inclusive bound, UCOL_BOUND_UPPER, that produces
     * upper bound that matches strings of the same length or
     * UCOL_BOUND_UPPER_LONG that matches strings that have the same starting
     * substring as the source string.
     * @param noOfLevels  Number of levels required in the resulting bound (for
     * most uses, the recommended value is 1). See users guide for explanation
     * on number of levels a sortkey can have.
     * @param result A pointer to a buffer to receive the resulting sortkey.
     * @param resultLength The maximum size of result.
     * @param status Used for returning error code if something went wrong. If
     * the number of levels requested is higher than the number of levels in the
     * source key, a warning (U_SORT_KEY_TOO_SHORT_WARNING) is issued.
...
public:
    /**
     * Used internally by registration to define the requested and valid
     * locales.
     ...
     * @internal
     */
    virtual void setLocales( const Locale& requestedLocale,
                             const Locale& validLocale,
                             const Locale& actualLocale );

    /** Get the short definition string for a collator. This internal API
     * harvests the collator's locale and the attribute set and produces a
     * string that can be used for opening a collator with the same attributes
     * using the ucol_openFromShortString API. This string will be normalized.
     ...
     * resulting short string definition. If NULL, the locale will be harvested
     *                from the collator.
     ...
     * @internal
     */
    virtual UCollationResult internalCompareUTF8( const char* left,
                                                  int32_t leftLength,
                                                  const char* right,
                                                  int32_t rightLength,
     ...
     * @internal
     */
    virtual int32_t internalNextSortKeyPart( UCharIterator* iter,
                                             uint32_t state[ 2 ],
                                             uint8_t* dest,
                                             int32_t count,
                                             UErrorCode& errorCode ) const;
...
#if !UCONFIG_NO_SERVICE
/**
 * A factory, used with registerFactory, the creates multiple collators and
 * provides display names for them.  A factory supports some number of locales--
 * these are the locales for which it can create collators.  The factory can be
 * visible, in which case the supported locales will be enumerated by
 * getAvailableLocales, or invisible, in which they are not.  Invisible locales
 * are still supported, they are just not listed by getAvailableLocales. <p> If
 * standard locale display names are sufficient, Collator instances can be
 * registered using registerInstance instead.</p> <p> Note: if the collators are
 * to be used from C APIs, they must be instances of RuleBasedCollator.</p>
 *
 * @stable ICU 2.6
 */
...
public:
    ...
    /**
     * Return the name of the collator for the objectLocale, localized for the
     * displayLocale. If objectLocale is not supported, or the factory is not
     * visible, set the result string to bogus.
     * @param objectLocale the locale identifying the collator
     * @param displayLocale the locale for which the display name of the
     * collator should be localized
     * @param result an output parameter for the display name, set to bogus if
     * not supported.
    ...
    /**
     * Return an array of all the locale names directly supported by this
     * factory. The number of names is returned in count.  This array is owned
     * by the factory. Its contents must never change.
     * @param count output parameter for the number of locales supported by the
     * factory
