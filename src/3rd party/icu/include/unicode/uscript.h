 ...
 * Hans or Hant. All Han ideographs have the Hani script property value in
 * Unicode.
 *
 * Private-use codes Qaaa..Qabx are not included, except as used in the UCD or
 * in CLDR.
 *
 ...
 * to ensure that their script names are stable and match the names of the
 * constants. Script codes like Latf and Aran that are not subject to separate
...
typedef enum UScriptCode {
     ...
     *     USCRIPT_<Unicode Script value name> = <integer>,  / * <ISO script
     * code> * /
     */
    ...
    /** @stable ICU 2.2 */
    USCRIPT_INHERITED = 1,
    /* Zinh */ /* "Code for inherited script", for non-spacing combining marks;
                  also Qaai */
    /** @stable ICU 2.2 */
     ...
     * u_getIntPropertyMaxValue(UCHAR_SCRIPT).
     *
...
/**
 * Gets the script codes associated with the given locale or ISO 15924
 * abbreviation or name. Fills in USCRIPT_MALAYALAM given "Malayam" OR "Mlym".
 * Fills in USCRIPT_LATIN given "en" OR "en_US"
 * If the required capacity is greater than the capacity of the destination
 * buffer, then the error code is set to U_BUFFER_OVERFLOW_ERROR and the
 * required capacity is returned.
 *
 ...
 * @return long script name as given in PropertyValueAliases.txt, or the
 * 4-letter code, or NULL if scriptCode is invalid
 * @stable ICU 2.4
 */
 ...
 * which is the same as the short Unicode script name if Unicode has names for
 * the script. Returns "Mlym" given USCRIPT_MALAYALAM.
 *
 * @param scriptCode UScriptCode enum
 ...
 * @stable ICU 49
 */
U_STABLE UBool U_EXPORT2 uscript_hasScript( UChar32 c, UScriptCode sc );
 ...
 * - If c does not have Script_Extensions, then the one Script code is written
 * to the output array.
 * - If c is not a valid code point, then the one USCRIPT_UNKNOWN code is
 * written. In other words, if the return value is 1, then the output array
 * contains exactly c's single Script code. If the return value is n>=2, then
 * the output array contains c's n Script_Extensions script codes.
 *
 * Some characters are commonly used in multiple scripts.
 * For more information, see UAX #24: http://www.unicode.org/reports/tr24/.
 *
 * If there are more than capacity script codes to be written, then
 * U_BUFFER_OVERFLOW_ERROR is set and the number of Script_Extensions is
 * returned. (Usual ICU buffer handling behavior.)
 *
 * @param c code point
 ...
 * @return number of script codes in c's Script_Extensions, or 1 for the single
 * Script value, written to scripts unless U_BUFFER_OVERFLOW_ERROR indicates
 * insufficient capacity
 * @stable ICU 49
 */
U_STABLE int32_t U_EXPORT2 uscript_getScriptExtensions( UChar32 c,
                                                        UScriptCode* scripts,
                                                        int32_t capacity,
 ...
 * @param pErrorCode standard ICU in/out error code, must pass U_SUCCESS() on
 * input
 * @return the string length, even if U_BUFFER_OVERFLOW_ERROR
 * @stable ICU 51
 */
U_STABLE int32_t U_EXPORT2 uscript_getSampleString( UScriptCode script,
                                                    UChar* dest,
                                                    int32_t capacity,
...
/**
 * Returns the script usage according to UAX #31 Unicode Identifier and Pattern
 * Syntax. Returns USCRIPT_USAGE_NOT_ENCODED if the script is not encoded in
 * Unicode.
 *
 * @param script script code
 * @return script usage
 ...
 * @stable ICU 51
 */
U_STABLE UScriptUsage U_EXPORT2 uscript_getUsage( UScriptCode script );
 ...
 * @stable ICU 51
 */
U_STABLE UBool U_EXPORT2 uscript_isRightToLeft( UScriptCode script );

/**
 * Returns TRUE if the script allows line breaks between letters (excluding
 * hyphenation). Such a script typically requires dictionary-based line
 * breaking. For example, Hani and Thai.
 *
 * @param script script code
 * @return TRUE if the script allows line breaks between letters
 * @stable ICU 51
 */
U_STABLE UBool U_EXPORT2 uscript_breaksBetweenLetters( UScriptCode script );

/**
 * Returns TRUE if in modern (or most recent) usage of the script case
 * distinctions are customary. For example, Latn and Cyrl.
 *
 * @param script script code
 * @return TRUE if the script is cased
 * @stable ICU 51
 */
U_STABLE UBool U_EXPORT2 uscript_isCased( UScriptCode script );
