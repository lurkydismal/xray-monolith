...
typedef enum UTimeZoneFormatStyle {
    /**
     * Generic location format, such as "United States Time (New York)", "Italy
     * Time"
     * @stable ICU 50
     */
     ...
     * indicator. For example, "-0500", "+0530", and "Z"(UTC). This style is
     ...
     * field, or the UTC indicator. For example, "-0500", "+052538", and
     * "Z"(UTC). This style is equivalent to the LDML date format pattern
     * "XXXX".
     * @stable ICU 51
     */
    ...
    /**
     * ISO 8601 local time difference (basic format) with optional seconds
     * field. For example, "-0500" and "+052538". This style is equivalent to
     * the LDML date format pattern "xxxx".
     * @stable ICU 51
     */
    UTZFMT_STYLE_ISO_BASIC_LOCAL_FULL,
    /**
     * Fixed width ISO 8601 local time difference (extended format) or the UTC
     * indicator. For example, "-05:00", "+05:30", and "Z"(UTC). This style is
    ...
    /**
     * ISO 8601 local time difference (extended format) with optional seconds
     * field, or the UTC indicator. For example, "-05:00", "+05:25:38", and
     * "Z"(UTC). This style is equivalent to the LDML date format pattern
     * "XXXXX".
     * @stable ICU 51
     */
    ...
    /**
     * ISO 8601 local time difference (extended format) with optional seconds
     * field. For example, "-05:00" and "+05:25:38". This style is equivalent to
    ...
    /**
     * Short Time Zone ID (BCP 47 Unicode location extension, time zone type
     * value), such as "uslax".
     * @stable ICU 51
     */
...
typedef enum UTimeZoneFormatGMTOffsetPatternType {
    ...
    /* The following cannot be #ifndef U_HIDE_INTERNAL_API, needed for other .h
       declarations */
    /**
...
/**
 * Constants for parse option flags, used for specifying optional parse
 * behavior.
 * @stable ICU 50
 */
typedef enum UTimeZoneFormatParseOption {
    ...
    /**
     * When parsing a time zone display name in \link
     * UTZFMT_STYLE_SPECIFIC_SHORT \endlink, look for the IANA tz database
     * compatible zone abbreviations in addition to the localized names coming
     * from the icu::TimeZoneNames currently used by the icu::TimeZoneFormat.
     * @stable ICU 54
     */
...
/**
 * <code>TimeZoneFormat</code> supports time zone display name formatting and
 * parsing. An instance of TimeZoneFormat works as a subformatter of {@link
 * SimpleDateFormat}, but you can also directly get a new instance of
 * <code>TimeZoneFormat</code> and formatting/parsing time zone display names.
 * <p>
 * ICU implements the time zone display names defined by <a
 * href="http://www.unicode.org/reports/tr35/">UTS#35 Unicode Locale Data Markup
 * Language (LDML)</a>. {@link TimeZoneNames} represents the time zone display
 * name data model and this class implements the algorithm for actual formatting
 * and parsing.
 *
...
public:
     ...
     * @stable ICU 50
     */
    static TimeZoneFormat* U_EXPORT2 createInstance( const Locale& locale,
                                                     UErrorCode& status );
     ...
     * @param pattern The localized GMT format pattern to be used by this
     * object.
     ...
     * @param type The offset pattern type enum.
     * @param pattern The offset pattern used for localized GMT format for the
     * type.
     * @param status Receives the status.
     * @see #getGMTOffsetPattern
     * @stable ICU 50
     */
    void setGMTOffsetPattern( UTimeZoneFormatGMTOffsetPatternType type,
     ...
     * digit 0 to digit 9 in the ascending order. When the input
     * <code>digits</code> does not satisfy the condition,
     * <code>U_ILLEGAL_ARGUMENT_ERROR</code> will be set to the return status.
    ...
    /**
     * Returns the localized GMT format string for GMT(UTC) itself (GMT offset
     * is 0).
     * @param gmtZeroFormat Receives the localized GMT string string for
     * GMT(UTC) itself.
    ...
    /**
     * Sets the localized GMT format string for GMT(UTC) itself (GMT offset is
     * 0).
    ...
    /**
     * Returns the bitwise flags of UTimeZoneFormatParseOption representing the
     * default parse options used by this object.
     ...
     * (UTZFMT_PARSE_OPTION_NONE). To specify multipe options, use bitwise flags
     * of UTimeZoneFormatParseOption.
     ...
     * @param useUtcIndicator true if ISO 8601 UTC indicator "Z" is used when
     * the offset is 0.
     ...
     * @stable ICU 51
     */
    UnicodeString& formatOffsetISO8601Basic( int32_t offset,
     ...
     * @stable ICU 51
     */
    UnicodeString& formatOffsetISO8601Extended( int32_t offset,
                                                UBool useUtcIndicator,
                                                UBool isShort,
                                                UBool ignoreSeconds,
                                                UnicodeString& result,
     ...
     * #getGMTOffsetDigits}) <li>GMT zero format (e.g. "GMT" - see {@link
     * #getGMTZeroFormat})
     * </ul>
     * This format always uses 2 digit hours and minutes. When the given offset
     * has non-zero seconds, 2 digit seconds field will be appended. For
     * example, GMT+05:00 and GMT+05:28:06.
     * @param offset the offset from GMT(UTC) in milliseconds.
     * @param status Receives the status
     ...
     * #getGMTZeroFormat})
     * </ul>
     * This format uses the shortest representation of offset. The hours field
     * does not have leading zero and lower fields with zero will be truncated.
     * For example, GMT+5 and GMT+530.
     * @param offset the offset from GMT(UTC) in milliseconds.
     * @param status Receives the status
     ...
     * @stable ICU 51
     */
    UnicodeString& formatOffsetShortLocalizedGMT( int32_t offset,
                                                  UnicodeString& result,
                                                  UErrorCode& status ) const;
     ...
     * @param timeType the output argument for receiving the time type
     * (standard/daylight/unknown) used for the display name, or NULL if the
     ...
     * style time zone string. When the given string is not an ISO 8601 time
     * zone string, this method sets the current position as the error index to
     * <code>ParsePosition pos</code> and returns 0.
     * @param text The text contains ISO8601 style time zone string (e.g.
     * "-08:00", "Z") at the position.
     * @param pos The ParsePosition object.
     * @return The offset from GMT(UTC) in milliseconds for the given ISO 8601
     * style time zone string.
     * @see #formatOffsetISO8601Basic
     * @see #formatOffsetISO8601Extended
     * @stable ICU 50
     */
    int32_t parseOffsetISO8601( const UnicodeString& text,
     ...
     * pos</code> and returns 0.
     * @param text The text contains a localized GMT offset string at the
     * position.
     * @param pos The ParsePosition object.
     * @return The offset from GMT(UTC) in milliseconds for the given localized
     * GMT offset format string.
     * @see #formatOffsetLocalizedGMT
     * @stable ICU 50
     */
    int32_t parseOffsetLocalizedGMT( const UnicodeString& text,
                                     ParsePosition& pos ) const;

    /**
     * Returns offset from GMT(UTC) in milliseconds for the given short
     * localized GMT offset format string. When the given string cannot be
     * parsed, this method sets the current position as the error index to
     * <code>ParsePosition pos</code> and returns 0.
     * @param text The text contains a short localized GMT offset string at the
     * position.
     * @param pos The ParsePosition object.
     * @return The offset from GMT(UTC) in milliseconds for the given short
     * localized GMT offset format string.
     * @see #formatOffsetShortLocalizedGMT
     * @stable ICU 51
     */
    int32_t parseOffsetShortLocalizedGMT( const UnicodeString& text,
                                          ParsePosition& pos ) const;

    /**
     * Returns a <code>TimeZone</code> by parsing the time zone string according
     * to the given parse position, the specified format style and parse
     * options.
     *
     ...
     * UTimeZoneFormatParseOption.
     * @param timeType The output argument for receiving the time type
     * (standard/daylight/unknown), or NULL if the information is not necessary.
     * @return A <code>TimeZone</code>, or null if the input could not be
     * parsed.
     * @see UTimeZoneFormatStyle
     * @see UTimeZoneFormatParseOption
     * @see UTimeZoneFormatTimeType
     * @stable ICU 50
     */
    virtual TimeZone* parse( UTimeZoneFormatStyle style,
    ...
    /**
     * Returns a <code>TimeZone</code> by parsing the time zone string according
     * to the given parse position, the specified format style and the default
     * parse options.
     *
     * @param text The text contains a time zone string at the position.
     * @param style The format style
     ...
     * @stable ICU 50
     */
    TimeZone* parse( UTimeZoneFormatStyle style,
                     const UnicodeString& text,
                     ParsePosition& pos,
                     UTimeZoneFormatTimeType* timeType = NULL ) const;
    ...
    /**
     * Format an object to produce a time zone display string using localized
     * GMT offset format. This method handles Formattable objects with a
     * <code>TimeZone</code>. If a the Formattable object type is not a
     * <code>TimeZone</code>, then it returns a failing UErrorCode.
     * @param obj The object to format. Must be a <code>TimeZone</code>.
     * @param appendTo Output parameter to receive result. Result is appended to
     * existing contents.
     ...
     * time zone display strings into Formattable objects with
     * <code>TimeZone</code>.
     * @param source The string to be parsed into an object.
     * @param result Formattable to be set to the parse result. If parse fails,
     * return contents are undefined.
     * @param parse_pos The position to start parsing at. Upon return this param
     * is set to the position after the last character successfully parsed. If
     * the source is not parsed successfully, this param will remain unchanged.
     * @return A newly created Formattable* object, or NULL on failure.  The
     * caller owns this and should delete it when done.
     * @stable ICU 50
     */
...
private:
    ...
    /* Constant parts of GMT format pattern, populated from localized GMT format
     * pattern*/
     ...
     */
    const TimeZoneGenericNames* getTimeZoneGenericNames(
        UErrorCode& status ) const;
     ...
     */
    enum OffsetFields { FIELDS_H, FIELDS_HM, FIELDS_HMS };
     ...
     * @param status U_ILLEGAL_ARGUMENT_ERROR is set when the specified pattern
     * does not contain an argument "{0}".
     */
     ...
     * @param status U_ILLEGAL_ARGUMENT is set when the specified pattern does
     * not contain pattern letters for the required fields.
     ...
     * Note: This code will be obsoleted once we add hour-minute-second pattern
     * data in CLDR.
     * @param offsetHM the offset pattern including hours and minutes fields
     * @param result the output offset pattern including hour, minute and
     * seconds fields
     * @param status receives the status
     ...
     * @return TRUE when the specified code array is fully filled with code
     * points (no under/overflow).
     */
    static UBool toCodePoints( const UnicodeString& str,
                               UChar32* codeArray,
                               int32_t capacity );
     ...
     * (extended format) time zone string. When the given string is not an ISO
     * 8601 time zone string, this method sets the current position as the error
     * index to <code>ParsePosition pos</code> and returns 0.
     * @param text the text contains ISO 8601 style time zone string (e.g.
     * "-08:00", "Z") at the position.
     * @param pos the position, non-negative error index will be set on failure.
     * @param extendedOnly TRUE if parsing the text as ISO 8601 extended offset
     * format (e.g. "-08:00"), or FALSE to evaluate the text as basic format.
     * @param hasDigitOffset receiving if the parsed zone string contains offset
     * digits.
     * @return the offset from GMT(UTC) in milliseconds for the given ISO 8601
     * style time zone string.
     */
     ...
     */
    void appendOffsetDigits( UnicodeString& buf,
                             int32_t n,
                             uint8_t minDigits ) const;
    ...
    /**
     * Parse localized GMT format generated by the patter used by this
     * formatter, except GMT Zero format.
     ...
     */
    int32_t parseOffsetLocalizedGMTPattern( const UnicodeString& text,
                                            int32_t start,
                                            UBool isShort,
     ...
     * @return the parsed offset in milliseconds.
     */
    int32_t parseOffsetFields( const UnicodeString& text,
                               int32_t start,
                               UBool isShort,
                               int32_t& parsedLen ) const;
     ...
     * @param forceSingleHourDigit true if hours field is parsed as a single
     * digit
     ...
     */
    int32_t parseOffsetFieldsWithPattern( const UnicodeString& text,
                                          int32_t start,
                                          UVector* patternItems,
     ...
     * @return the parsed offset in milliseconds.
     */
    int32_t parseAbuttingOffsetFields( const UnicodeString& text,
                                       int32_t start,
                                       int32_t& parsedLen ) const;
     ...
     * @return the parsed offset in milliseconds.
     */
    int32_t parseOffsetDefaultLocalizedGMT( const UnicodeString& text,
                                            int start,
                                            int32_t& parsedLen ) const;
     ...
     * @return the parsed offset in milliseconds.
     */
    int32_t parseDefaultOffsetFields( const UnicodeString& text,
                                      int32_t start,
                                      char16_t separator,
                                      int32_t& parsedLen ) const;
     ...
     */
    int32_t parseOffsetFieldWithLocalizedDigits( const UnicodeString& text,
                                                 int32_t start,
                                                 uint8_t minDigits,
                                                 uint8_t maxDigits,
                                                 uint16_t minVal,
     ...
     */
    int32_t parseSingleLocalizedDigit( const UnicodeString& text,
                                       int32_t start,
                                       int32_t& len ) const;
     ...
     */
    static int32_t parseAbuttingAsciiOffsetFields( const UnicodeString& text,
                                                   ParsePosition& pos,
                                                   OffsetFields minFields,
                                                   OffsetFields maxFields,
     ...
     * @return Parsed offset, 0 or positive number.
     */
    static int32_t parseAsciiOffsetFields( const UnicodeString& text,
                                           ParsePosition& pos,
                                           char16_t sep,
                                           OffsetFields minFields,
                                           OffsetFields maxFields );
     ...
     */
    UnicodeString& getTimeZoneID(
        const TimeZoneNames::MatchInfoCollection* matches,
        int32_t idx,
     ...
     * @param text the text contains a short time zone ID string at the
     * position.
     * @param pos the position
     ...
     * @param text the text contains an exemplar location string at the
     * position.
     * @param pos the position.
