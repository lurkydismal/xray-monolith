 ...
 *
 * <p>This function performs basic operations for "shaping" Arabic text. It is
 * most useful for use with legacy data formats and legacy display technology
 ...
 * @stable ICU 2.0
 */
U_STABLE int32_t U_EXPORT2 u_shapeArabic( const UChar* source,
 ...
 * Memory option: the result must have the same length as the source.
 * If more room is necessary, then try to consume spaces next to modified
 * characters. Affects: LamAlef options This option is an alias to
...
/**
 * Memory option: the result must have the same length as the source.
 * If more room is necessary, then try to consume spaces at the beginning of the
 * text.
 * @stable ICU 2.0
 */
 ...
 * Memory option: the result must have the same length as the source.
 * If more room is necessary, then try to consume spaces at the beginning of the
 * text. Affects: LamAlef options This option is an alias to
...
/**
 * Memory option: the result must have the same length as the source.
 * Shaping Mode: For each LAMALEF character found, expand LAMALEF using space at
 * end. If there is no space at end, use spaces at beginning of the buffer. If
 * there is no space at beginning of the buffer, use spaces at the near (i.e.
 * the space after the LAMALEF character). If there are no spaces found, an
 * error U_NO_SPACE_AVAILABLE (as defined in utypes.h) will be set in pErrorCode
 *
 * Deshaping Mode: Perform the same function as the flag equals
 * U_SHAPE_LAMALEF_END. Affects: LamAlef options
 * @stable ICU 4.2
 */
 ...
 * (<code>u_charDirection()</code> result <code>U_RIGHT_TO_LEFT_ARABIC</code>
 * [AL]).<br> The direction of "preceding" depends on the direction indicator
 * option. For the first characters, the preceding strongly directional
 * character (initial state) is assumed to be not an Arabic letter (it is
 * <code>U_LEFT_TO_RIGHT</code> [L] or <code>U_RIGHT_TO_LEFT</code> [R]).
 * @stable ICU 2.0
 */
...
#define U_SHAPE_DIGIT_TYPE_MASK \
    0x300 /* I need to change this from 0x3f00 to 0x300 */
...
/**
 * Memory option: the result must have the same length as the source.
 * Shaping mode: The SEEN family character will expand into two characters using
 * space near the SEEN family character(i.e. the space after the character). If
 * there are no spaces found, an error U_NO_SPACE_AVAILABLE (as defined in
 * utypes.h) will be set in pErrorCode
 *
...
/**
 * Memory option: the result must have the same length as the source.
 * Shaping mode: The YEHHAMZA character will expand into two characters using
 * space near it (i.e. the space after the character If there are no spaces
 * found, an error U_NO_SPACE_AVAILABLE (as defined in utypes.h) will be set in
 * pErrorCode
 *
 * De-shaping mode: Any Yeh (final or isolated) character followed by Hamza
 * character will be replaced by one cell YehHamza and space will replace the
 * Hamza. Affects: YehHamza options
 * @stable ICU 4.2
 */
...
/**
 * Memory option: the result must have the same length as the source.
 * Shaping mode: Tashkeel characters will be replaced by Tatweel if it is
 * connected to adjacent characters (i.e. shaped on Tatweel) or replaced by
 * space if it is not connected.
 *
 * De-shaping mode: N/A
...
/**
 * This option affect the meaning of BEGIN and END options. if this option is
 * not used the default for BEGIN and END will be as following: The Default (for
 * both Visual LTR, Visual RTL and Logical Text)
 *           1. BEGIN always refers to the start address of physical memory.
 *           2. END always refers to the end address of physical memory.
 *
 * If this option is used it will swap the meaning of BEGIN and END only for
 * Visual LTR text.
 *
 * The effect on BEGIN and END Memory Options will be as following:
 *    A. BEGIN For Visual LTR text: This will be the beginning (right side) of
 * the visual text( corresponding to the physical memory address end for Visual
 * LTR text, Same as END in default behavior) B. BEGIN For Logical text: Same as
 * BEGIN in default behavior. C. END For Visual LTR text: This will be the end
 * (left side) of the visual text (corresponding to the physical memory address
 * beginning for Visual LTR text, Same as BEGIN in default behavior. D. END For
 * Logical text: Same as END in default behavior). Affects: All LamAlef BEGIN,
 * END and AUTO options.
 * @stable ICU 4.2
 */
...
/**
 * If this option is used, shaping will use the new Unicode code point for TAIL
 * (i.e. 0xFE73). If this option is not specified (Default), old unofficial
 * Unicode TAIL code point is used (i.e. 0x200B) De-shaping will not use this
 * option as it will always search for both the new Unicode code point for the
 * TAIL (i.e. 0xFE73) or the old unofficial Unicode TAIL code point (i.e.
 * 0x200B) and de-shape the Seen-Family letter accordingly.
 *
