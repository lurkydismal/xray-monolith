 ...
 * Fast, reasonably compact, map from Unicode code points (U+0000..U+10FFFF) to
 * integer values. For details see
 ...
 *
 * The macros will return bogus values, or may crash, if used on the wrong type
 * or value width.
 *
 * @see UMutableCPTrie
 * @stable ICU 63
...
struct UCPTrie {
     ...
     * Set to an impossibly high value (e.g., 0xffff) if there is no dedicated
     * index-3 null block.
     * @internal
     */
     ...
     * Set to an impossibly high value (e.g., 0xfffff) if there is no dedicated
     * data null block.
     * @internal
     */
 ...
 * @param data a pointer to 32-bit-aligned memory containing the binary data of
 * a UCPTrie
 * @param length the number of bytes available at data;
 *               can be more than necessary
 * @param pActualLength receives the actual number of bytes at data taken up by
 * the trie data; can be NULL
 ...
 * @stable ICU 63
 */
U_CAPI UCPTrie* U_EXPORT2 ucptrie_openFromBinary( UCPTrieType type,
 ...
 * checking. Returns the trie error value if c is not in the range 0..U+10FFFF.
 *
...
/**
 * Returns the last code point such that all those from start to there have the
 * same value. Can be used to efficiently iterate over all same-value ranges in
 * a trie. (This is normally faster than iterating over code points and
 * get()ting each value, but much slower than a data structure that stores
 * ranges directly.)
 *
 * If the UCPMapValueFilter function pointer is not NULL, then
 * the value to be delivered is passed through that function, and the return
 * value is the end of the range where all values are modified to the same
 * actual value. The value is unchanged if that function pointer is NULL.
 *
 * Example:
 * \code
 * UChar32 start = 0, end;
 ...
 * @param pValue if not NULL, receives the value that every code point
 * start..end has; may have been modified by filter(context, trie value) if that
 * function pointer is not NULL
 * @return the range end code point, or -1 if start is not a valid code point
 * @stable ICU 63
 */
U_CAPI UChar32 U_EXPORT2 ucptrie_getRange( const UCPTrie* trie,
                                           UChar32 start,
 ...
 *
 * @param trie the trie
 * @param data a pointer to 32-bit-aligned memory to be filled with the trie
 * data; can be NULL if capacity==0
 * @param capacity the number of bytes available at data, or 0 for pure
 * preflighting
 * @param pErrorCode an in/out ICU UErrorCode;
 *                   U_BUFFER_OVERFLOW_ERROR if the capacity is too small
 * @return the number of bytes written or (if buffer overflow) needed for the
 * trie
 *
 * @see ucptrie_openFromBinary()
 * @stable ICU 63
 */
U_CAPI int32_t U_EXPORT2 ucptrie_toBinary( const UCPTrie* trie,
                                           void* data,
                                           int32_t capacity,
                                           UErrorCode* pErrorCode );
 ...
 * @stable ICU 63
 */
#define UCPTRIE_FAST_GET( trie, dataAccess, c ) \
 ...
 * @param limit (const UChar *, in) the limit pointer for the text, or NULL if
 * NUL-terminated
    ...
    UPRV_BLOCK_MACRO_BEGIN {                                             \
        ...
        if ( !U16_IS_SURROGATE( c ) ) {                                  \
            ...
            if ( U16_IS_SURROGATE_LEAD( c ) && ( src ) != ( limit ) &&   \
                 U16_IS_TRAIL( __c2 = *( src ) ) ) {                     \
                ++( src );                                               \
                ( c ) = U16_GET_SUPPLEMENTARY( ( c ), __c2 );            \
                __index = _UCPTRIE_SMALL_INDEX( trie, c );               \
            } else {                                                     \
                __index = ( trie )->dataLength -                         \
 ...
 * point because it would be more work to do so and is often not needed. If the
 * trie value differs from the error value, then the byte sequence is
 ...
 * @param limit (const char *, in) the limit pointer for the text (must not be
 * NULL)
 * @param result (out) variable for the trie lookup result
 * @stable ICU 63
 */
            ...
            if (                                                                   \
                ...
                ( __lead >= 0xe0                                                   \
                            ...
                            U8_LEAD3_T1_BITS[ __lead &= 0xf ] &                    \
                                ...
                                ( __lead =                                         \
                                  ...
                                  1 )                                              \
                                      : /* U+10000..U+10FFFF */                    \
                            ( __lead -= 0xf0 ) <= 4 &&                             \
                                U8_LEAD4_T1_BITS[ ( __t1 = *( src ) ) >> 4 ] &     \
                                    ( 1 << __lead ) &&                             \
                                ( __lead = ( __lead << 6 ) | ( __t1 & 0x3f ),      \
                                  ++( src ) != ( limit ) ) &&                      \
                                ( __t2 = *( src ) - 0x80 ) <= 0x3f &&              \
                                ++( src ) != ( limit ) &&                          \
                                ( __t3 = *( src ) - 0x80 ) <= 0x3f &&              \
                                ( __lead =                                         \
                                      __lead >= ( trie )->shifted12HighStart       \
                                          ? ( trie )->dataLength -                 \
                      ...
                      __lead >= 0xc2 && ( __t1 = *( src ) - 0x80 ) <= 0x3f &&      \
                          ( __lead =                                               \
                                ( int32_t )( trie )->index[ __lead & 0x1f ] +      \
                                __t1,                                              \
                            1 ) ) ) {                                              \
                ++( src );                                                         \
            } else {                                                               \
                __lead = ( trie )->dataLength -                                    \
...
#define UCPTRIE_FAST_U8_PREV( trie, dataAccess, start, src, result ) \
    UPRV_BLOCK_MACRO_BEGIN {                                         \
        ...
        if ( !U8_IS_SINGLE( __index ) ) {                            \
            __index = ucptrie_internalU8PrevIndex(                   \
                ( trie ), __index, ( const uint8_t* )( start ),      \
                ( const uint8_t* )( src ) );                         \
            ( src ) -= __index & 7;                                  \
            __index >>= 3;                                           \
        }                                                            \
...
/** @internal */
U_INTERNAL int32_t U_EXPORT2 ucptrie_internalSmallIndex( const UCPTrie* trie,
                                                         UChar32 c );

/** @internal */
U_INTERNAL int32_t U_EXPORT2 ucptrie_internalSmallU8Index( const UCPTrie* trie,
                                                           int32_t lt1,
                                                           uint8_t t2,
                                                           uint8_t t3 );
 ...
 * @internal
 */
U_INTERNAL int32_t U_EXPORT2 ucptrie_internalU8PrevIndex( const UCPTrie* trie,
                                                          UChar32 c,
                                                          const uint8_t* start,
                                                          const uint8_t* src );
