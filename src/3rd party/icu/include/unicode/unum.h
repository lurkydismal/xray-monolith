...
* others. All Rights Reserved. Modification History:
*
 ...
 * &status); unum_formatDouble(nf, myNumber, myString, 20, NULL, &status);
 *    printf(" Example 1: %s\n", austrdup(myString) ); //austrdup( a function
 * used to convert UChar* to char*)
 * \endcode
 * </pre>
 ...
 * <pre>
 * \code
 *     UNumberFormat* nf = unum_open(UNUM_DEFAULT, NULL, -1, "fr_FR", NULL,
 * &success)
 * \endcode
 * </pre>
...
typedef enum UNumberFormatStyle {
    ...
    UNUM_SCIENTIFIC = 4,
    /**
     * Spellout rule-based format. The default ruleset can be specified/changed
     * using unum_setTextAttribute with UNUM_DEFAULT_RULESET; the available
     * public rulesets can be listed using unum_getTextAttribute with
     * UNUM_PUBLIC_RULESETS.
     * @stable ICU 2.0
     */
    UNUM_SPELLOUT = 5,
    /**
     * Ordinal rule-based format . The default ruleset can be specified/changed
     * using unum_setTextAttribute with UNUM_DEFAULT_RULESET; the available
     * public rulesets can be listed using unum_getTextAttribute with
     * UNUM_PUBLIC_RULESETS.
     * @stable ICU 3.0
 ...
 * @param result A pointer to a buffer to receive the NULL-terminated formatted
 * number. If the formatted number fits into dest but cannot be NULL-terminated
 * (length == resultLength) then the error code is set to
 * U_STRING_NOT_TERMINATED_WARNING. If the formatted number doesn't fit into
 * result then the error code is set to U_BUFFER_OVERFLOW_ERROR.
...
/**
 * Format a double using a UNumberFormat according to the UNumberFormat's
 * locale, and initialize a UFieldPositionIterator that enumerates the
 * subcomponents of the resulting string.
 *
 * @param format
 ...
 * needed, but in this case it's preferable to use {@link #unum_formatDouble}).
 * Iteration information already present in the UFieldPositionIterator is
 * deleted, and the iterator is reset to apply to the fields in the formatted
 ...
 *          the UNumberFormatFields enum. Fields are not returned in a
 ...
 * @param fmt The formatter to use.
 * @param number The number to format.
 * @param length The length of the input number, or -1 if the input is
 * nul-terminated.
 * @param result A pointer to a buffer to receive the NULL-terminated formatted
 * number. If the formatted number fits into dest but cannot be NULL-terminated
 * (length == resultLength) then the error code is set to
 ...
 * @param resultLength The maximum size of result.
 * @param pos    A pointer to a UFieldPosition.  On input, position->field
 *               is read.  On output, position->beginIndex and
 * position->endIndex indicate the beginning and ending indices of field number
 * position->field, if such a field exists.  This parameter may be NULL, in
 ...
 * @stable ICU 52
 */
U_STABLE int32_t U_EXPORT2 unum_formatUFormattable( const UNumberFormat* fmt,
 ...
 * @param parsePos If not NULL, on input a pointer to an integer specifying the
 * offset at which to begin parsing.  If not NULL, on output the offset at which
 * parsing ended.
 * @param status A pointer to an UErrorCode to receive any errors
...
/**
 * Parse a number from a string into an unformatted numeric string using a
 * UNumberFormat. The input string will be parsed according to the
 * UNumberFormat's locale. The syntax of the output is a "numeric string" as
 * defined in the Decimal Arithmetic Specification, available at
 * http://speleotrove.com/decimal
 * Note: parsing is not supported for styles UNUM_DECIMAL_COMPACT_SHORT
 * and UNUM_DECIMAL_COMPACT_LONG.
 * @param fmt The formatter to use.
 ...
 * offset at which to begin parsing.  If not NULL, on output the offset at which
 * parsing ended.
 * @param outBuf A (char *) buffer to receive the parsed number as a string. The
 * output string will be nul-terminated if there is sufficient space.
 * @param outBufLength The size of the output buffer.  May be zero, in which
 * case the outBuf pointer may be NULL, and the function will return the size of
 * the output string.
 * @param status A pointer to an UErrorCode to receive any errors
 ...
 * and UNUM_DECIMAL_COMPACT_LONG.
 * @param fmt the formatter to use
 * @param result the UFormattable to hold the result. If NULL, a new
 * UFormattable will be allocated (which the caller must close with ufmt_close).
 * @param text the text to parse
 * @param textLength the length of text, or -1 if null-terminated
 * @param parsePos a pointer to an offset index into text at which to
 * begin parsing. On output, *parsePos will point after the last
 ...
 * begins at offset 0.
 * @param status a pointer to an input-output UErrorCode
 * @return the UFormattable.  Will be ==result unless NULL was passed in for
 * result, in which case it will be the newly opened UFormattable.
 * @see ufmt_getType
 * @see ufmt_close
 * @stable ICU 52
 */
 ...
 * A UNumberFormat in a locale returned by this function will perform the
 * correct formatting and parsing for the locale.  The results of this call are
 * not valid for rule-based number formats.
 * @param localeIndex The index of the desired locale.
 * @return A locale for which number formatting patterns are available, or 0 if
 * none.
 ...
 * @return The number of locales for which decimal formatting patterns are
 * available.
...
/* The UNumberFormatAttributeValue type cannot be #ifndef U_HIDE_INTERNAL_API,
 * needed for .h variable declaration */
/**
 * @internal
 */
...
typedef enum UNumberFormatAttribute {
    ...
    UNUM_LENIENT_PARSE,
#if UCONFIG_HAVE_PARSEALLINPUT
    /** Consume all input. (may use fastpath). Set to UNUM_YES (require
     * fastpath), UNUM_NO (skip fastpath), or UNUM_MAYBE (heuristic). This is an
     * internal ICU API. Do not use.
     * @internal
     */
     ...
     * decimal point when formatting.  Amounts will be multiplied by 10 ^
     * (scale) before they are formatted.  The default value for the scale is 0
     * ( no adjustment ).
     *
     * <p>Example: setting the scale to 3, 123 formats as "123,000"
     * <p>Example: setting the scale to -4, 123 formats as "0.0123"
     *
     * This setting is analogous to getMultiplierScale() and
     * setMultiplierScale() in decimfmt.h.
     *
     * @stable ICU 51 */
    UNUM_SCALE = 21,
...
#ifndef U_HIDE_DRAFT_API
    /**
     * Minimum grouping digits; most commonly set to 2 to print "1000" instead
     * of "1,000". See DecimalFormat::getMinimumGroupingDigits().
     *
     ...
     * purpose, otherwise it is UNUM_CURRENCY_CASH purpose Default: 0
    ...
    /** If 1, specifies that if setting the "max integer digits" attribute would
     * truncate a value, set an error status rather than silently truncating.
     * For example,  formatting the value 1234 with 4 max int digits would
     * succeed, but formatting 12345 would fail. There is no effect on parsing.
     * Default: 0 (not set)
     * @stable ICU 50
     */
     ...
     * contain an exponent, the exponent will not be parsed. If the pattern does
     * contain an exponent, this attribute has no effect. Has no effect on
     * formatting. Default: 0 (unset)
     * @stable ICU 50
     */
     ...
     * decimal mark the input is required to have one. If this attribute is set
     * to 0, specifies that input does not have to contain a decimal mark. Has
     * no effect on formatting. Default: 0 (unset)
     * @stable ICU 54
     */
    ...
    /**
     * Formatting: if set to 1, whether to show the plus sign on non-negative
     * numbers.
     *
 ...
 * An example of a numeric attribute is the number of integer digits a formatter
 * will produce.
 * @param fmt The formatter to query.
 * @param attr The attribute to query; one of UNUM_PARSE_INT_ONLY,
 * UNUM_GROUPING_USED, UNUM_DECIMAL_ALWAYS_SHOWN, UNUM_MAX_INTEGER_DIGITS,
 * UNUM_MIN_INTEGER_DIGITS, UNUM_INTEGER_DIGITS, UNUM_MAX_FRACTION_DIGITS,
 * UNUM_MIN_FRACTION_DIGITS, UNUM_FRACTION_DIGITS, UNUM_MULTIPLIER,
 * UNUM_GROUPING_SIZE, UNUM_ROUNDING_MODE, UNUM_FORMAT_WIDTH,
 * UNUM_PADDING_POSITION, UNUM_SECONDARY_GROUPING_SIZE, UNUM_SCALE,
 ...
 * An example of a numeric attribute is the number of integer digits a formatter
 * will produce.  If the formatter does not understand the attribute, the call
 * is ignored.  Rule-based formatters only understand the lenient-parse
 * attribute.
 * @param fmt The formatter to set.
 * @param attr The attribute to set; one of UNUM_PARSE_INT_ONLY,
 * UNUM_GROUPING_USED, UNUM_DECIMAL_ALWAYS_SHOWN, UNUM_MAX_INTEGER_DIGITS,
 * UNUM_MIN_INTEGER_DIGITS, UNUM_INTEGER_DIGITS, UNUM_MAX_FRACTION_DIGITS,
 * UNUM_MIN_FRACTION_DIGITS, UNUM_FRACTION_DIGITS, UNUM_MULTIPLIER,
 * UNUM_GROUPING_SIZE, UNUM_ROUNDING_MODE, UNUM_FORMAT_WIDTH,
 * UNUM_PADDING_POSITION, UNUM_SECONDARY_GROUPING_SIZE, UNUM_LENIENT_PARSE,
 ...
 * Get a numeric attribute associated with a UNumberFormat.
 * An example of a numeric attribute is the number of integer digits a formatter
 * will produce. If the formatter does not understand the attribute, -1 is
 * returned.
 * @param fmt The formatter to query.
 ...
 * Set a numeric attribute associated with a UNumberFormat.
 * An example of a numeric attribute is the number of integer digits a formatter
 * will produce. If the formatter does not understand the attribute, this call
 * is ignored.
 * @param fmt The formatter to set.
...
typedef enum UNumberFormatTextAttribute {
     ...
     * "%spellout-ordinal-neuter:". The available public rulesets can be listed
     * using unum_getTextAttribute with UNUM_PUBLIC_RULESETS. This is only
     * available with rule-based formatters.
     * @stable ICU 3.0
     */
     ...
     * single string, with each ruleset name delimited by ';' (semicolon). See
 ...
 * An example of a text attribute is the suffix for positive numbers.  If the
 * formatter does not understand the attribute, U_UNSUPPORTED_ERROR is returned
 * as the status. Rule-based formatters only understand UNUM_DEFAULT_RULESET and
 * UNUM_PUBLIC_RULESETS.
 * @param fmt The formatter to query.
 * @param tag The attribute to query; one of UNUM_POSITIVE_PREFIX,
 * UNUM_POSITIVE_SUFFIX, UNUM_NEGATIVE_PREFIX, UNUM_NEGATIVE_SUFFIX,
 * UNUM_PADDING_CHARACTER, UNUM_CURRENCY_CODE, UNUM_DEFAULT_RULESET, or
 * UNUM_PUBLIC_RULESETS.
 ...
 * An example of a text attribute is the suffix for positive numbers. Rule-based
 * formatters only understand UNUM_DEFAULT_RULESET.
 * @param fmt The formatter to set.
 * @param tag The attribute to set; one of UNUM_POSITIVE_PREFIX,
 * UNUM_POSITIVE_SUFFIX, UNUM_NEGATIVE_PREFIX, UNUM_NEGATIVE_SUFFIX,
 * UNUM_PADDING_CHARACTER, UNUM_CURRENCY_CODE, or UNUM_DEFAULT_RULESET.
 * @param newValue The new value of attr.
 ...
 * @stable ICU 53
 */
U_STABLE void U_EXPORT2 unum_setContext( UNumberFormat* fmt,
 ...
 * @stable ICU 53
 */
U_STABLE UDisplayContext U_EXPORT2 unum_getContext( const UNumberFormat* fmt,
