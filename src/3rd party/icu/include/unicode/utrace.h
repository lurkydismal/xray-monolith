...
/**
 * Trace severity levels.  Higher levels increase the verbosity of the trace
 * output.
 * @see utrace_setLevel
 * @stable ICU 2.8
 */
...
/**
 *  Type signature for the trace function to be called when exiting from a
 * function.
 *  @param context value supplied at the time the trace functions are set.
 ...
 *  @stable ICU 2.8
 */
typedef void U_CALLCONV UTraceExit( const void* context,
                                    int32_t fnNumber,
                                    const char* fmt,
                                    va_list args );

/**
 *  Type signature for the trace function to be called from within an ICU
 * function to display data or messages.
 *  @param context  value supplied at the time the trace functions are set.
 *  @param fnNumber Enum value indicating the ICU function being exited.
 ...
 *  @stable ICU 2.8
 */
typedef void U_CALLCONV UTraceData( const void* context,
                                    int32_t fnNumber,
                                    int32_t level,
                                    const char* fmt,
                                    va_list args );
 ...
 *  @stable ICU 2.8
 */
U_STABLE void U_EXPORT2 utrace_setFunctions( const void* context,
                                             UTraceEntry* e,
                                             UTraceExit* x,
                                             UTraceData* d );

/**
 * Get the currently installed ICU tracing functions.   Note that a null
 * function pointer will be returned if no trace function has been set.
 *
 ...
 * @stable ICU 2.8
 */
U_STABLE void U_EXPORT2 utrace_getFunctions( const void** context,
                                             UTraceEntry** e,
                                             UTraceExit** x,
                                             UTraceData** d );
 ...
 * - S A UChar * string.  Requires two params, (ptr, length).  Length=-1 for nul
 * term.
...
/**
 *  Trace output Formatter.  An application's UTraceData tracing functions may
 * call back to this function to format the trace output in a human readable
 * form.  Note that a UTraceData function may choose to not format the data;  it
 * could, for example, save it in in the raw form it was received (more
 * compact), leaving formatting for a later trace analysis tool.
 *  @param outBuf  pointer to a buffer to receive the formatted output.  Output
 *                 will be nul terminated if there is space in the buffer -
 *                 if the length of the requested output < the output buffer
 * size.
 *  @param capacity  Length of the output buffer.
 *  @param indent  Number of spaces to indent the output.  Intended to allow
 *                 data displayed from nested functions to be indented for
 * readability.
 ...
 *                 If buffer capacity is insufficient, the required capacity is
 * returned.
 *  @stable ICU 2.8
 */
U_STABLE int32_t U_EXPORT2 utrace_vformat( char* outBuf,
                                           int32_t capacity,
                                           int32_t indent,
                                           const char* fmt,
                                           va_list args );
...
/**
 *  Trace output Formatter.  An application's UTraceData tracing functions may
 * call this function to format any additional trace data, beyond that provided
 * by default, in human readable form with the same formatting conventions used
 * by utrace_vformat().
 *  @param outBuf  pointer to a buffer to receive the formatted output.  Output
 *                 will be nul terminated if there is space in the buffer -
 *                 if the length of the requested output < the output buffer
