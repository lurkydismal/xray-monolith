...
public:
     ...
     * @stable ICU 4.8
     */
    UCharsTrie* build( UStringTrieBuildOption buildOption,
                       UErrorCode& errorCode );
     ...
     * If build() is called after buildUnicodeString(), the trie object will
     ...
     * char16_t-serialized UCharsTrie for the add()ed data.
     * @param errorCode Standard ICU error code. Its input value must
     *                  pass the U_SUCCESS() test, or else the function returns
     *                  immediately. Check for U_FAILURE() on output or use with
     *                  function chaining. (See User Guide for details.)
...
private:
    ...
    UCharsTrieBuilder& operator=(
        const UCharsTrieBuilder& other ); // no assignment operator

    void buildUChars( UStringTrieBuildOption buildOption,
                      UErrorCode& errorCode );
    ...
    public:
        UCTLinearMatchNode( const char16_t* units,
                            int32_t len,
                            Node* nextNode );
