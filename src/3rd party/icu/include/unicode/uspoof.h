 ...
 *
 * <p>
 * This class, based on <a href="http://unicode.org/reports/tr36">Unicode
 * Technical Report #36</a> and <a
 * href="http://unicode.org/reports/tr39">Unicode Technical Standard #39</a>,
 * has two main functions:
 *
 * <ol>
 * <li>Checking whether two strings are visually <em>confusable</em> with each
 * other, such as "Harvest" and &quot;&Eta;arvest&quot;, where the second string
 * starts with the Greek capital letter Eta.</li> <li>Checking whether an
 * individual string is likely to be an attempt at confusing the reader
 * (<em>spoof detection</em>), such as "paypal" with some Latin characters
 * substituted with Cyrillic look-alikes.</li>
 * </ol>
 *
 * <p>
 * Although originally designed as a method for flagging suspicious identifier
 * strings such as URLs, <code>USpoofChecker</code> has a number of other
 * practical use cases, such as preventing attempts to evade bad-word content
 * filters.
 *
 * <p>
 * The functions of this class are exposed as C API, with a handful of
 * syntactical conveniences for C++.
 *
 * <h2>Confusables</h2>
 *
 * <p>
 * The following example shows how to use <code>USpoofChecker</code> to check
 * for confusability between two strings:
 *
 * \code{.c}
 * UErrorCode status = U_ZERO_ERROR;
 * UChar* str1 = (UChar*) u"Harvest";
 * UChar* str2 = (UChar*) u"\u0397arvest";  // with U+0397 GREEK CAPITAL LETTER
 * ETA
 *
 ...
 *
 * <p>
 * The call to {@link uspoof_open} creates a <code>USpoofChecker</code> object;
 * the call to {@link uspoof_setChecks} enables confusable checking and disables
 * all other checks; the call to {@link uspoof_areConfusable} performs the
 * confusability test; and the following line extracts the result out of the
 * return value. For best performance, the instance should be created once
 * (e.g., upon application startup), and the efficient
 * {@link uspoof_areConfusable} method can be used at runtime.
 *
 * <p>
 * The type {@link LocalUSpoofCheckerPointer} is exposed for C++ programmers. It
 ...
 * \endcode
 *
 * UTS 39 defines two strings to be <em>confusable</em> if they map to the same
 * <em>skeleton string</em>. A skeleton can be thought of as a "hash code".
 * {@link uspoof_getSkeleton} computes the skeleton for a particular string, so
 ...
 * \endcode
 *
 * If you need to check if a string is confusable with any string in a
 * dictionary of many strings, rather than calling
 * {@link uspoof_areConfusable} many times in a loop, {@link uspoof_getSkeleton}
 ...
 * }; UChar* skeletons[DICTIONARY_LENGTH]; UChar* str = (UChar*) u"1orern";
 *
 * // Setup:
 * USpoofChecker* sc = uspoof_open(&status);
 * uspoof_setChecks(sc, USPOOF_CONFUSABLE, &status);
 ...
 * \endcode
 *
 * <b>Note:</b> Since the Unicode confusables mapping table is frequently
 * updated, confusable skeletons are <em>not</em> guaranteed to be the same
 * between ICU releases. We therefore recommend that you always compute
 * confusable skeletons at runtime and do not rely on creating a permanent, or
 * difficult to update, database of skeletons.
 *
 * <h2>Spoof Detection</h2>
 *
 * The following snippet shows a minimal example of using
 * <code>USpoofChecker</code> to perform spoof detection on a string:
 *
 * \code{.c}
 * UErrorCode status = U_ZERO_ERROR;
 ...
 * <code>USpoofChecker</code> instance at startup, and call the cheaper {@link
 * uspoof_check} online. We specify the set of allowed characters to be those
 * with type RECOMMENDED or INCLUSION, according to the recommendation in
 * UTS 39.
 *
 * In addition to {@link uspoof_check}, the function {@link uspoof_checkUTF8} is
 * exposed for UTF8-encoded char* strings, and {@link uspoof_checkUnicodeString}
 * is exposed for C++ programmers.
 *
 * If the {@link USPOOF_AUX_INFO} check is enabled, a limited amount of
 * information on why a string failed the checks is available in the returned
 * bitmask.  For complete information, use the {@link uspoof_check2} class of
 * functions with a {@link USpoofCheckResult} parameter:
 *
 * \code{.c}
 * UErrorCode status = U_ZERO_ERROR;
 * UChar* str = (UChar*) u"p\u0430ypal";  // with U+0430 CYRILLIC SMALL LETTER A
 ...
 * \endcode
 *
 * C++ users can take advantage of a few syntactical conveniences.  The
 * following snippet is functionally equivalent to the one above:
 *
 * \code{.cpp}
 * UErrorCode status = U_ZERO_ERROR;
 * UnicodeString str((UChar*) u"p\u0430ypal");  // with U+0430 CYRILLIC SMALL
 * LETTER A
 *
 * // Get the default set of allowable characters:
 * UnicodeSet allowed;
 ...
 * int32_t bitmask = uspoof_check2UnicodeString(sc.getAlias(), str,
 * checkResult.getAlias(), &status);
 *
 * int32_t failures1 = bitmask;
 * int32_t failures2 = uspoof_getCheckResultChecks(checkResult.getAlias(),
 * &status); assert(failures1 == failures2);
 * // checks that failed: 0x00000010 (status: U_ZERO_ERROR)
 * printf("checks that failed: %#010x (status: %s)\n", failures1,
 * u_errorName(status));
 *
 ...
 * \endcode
 *
 * The return value is a bitmask of the checks that failed. In this case, there
 * was one check that failed:
 * {@link USPOOF_RESTRICTION_LEVEL}, corresponding to the fifth bit (16). The
 * possible checks are:
 *
 * <ul>
 * <li><code>RESTRICTION_LEVEL</code>: flags strings that violate the
 * <a
 * href="http://unicode.org/reports/tr39/#Restriction_Level_Detection">Restriction
 * Level</a> test as specified in UTS 39; in most cases, this means flagging
 * strings that contain characters from multiple different scripts.</li>
 * <li><code>INVISIBLE</code>: flags strings that contain invisible characters,
 * such as zero-width spaces, or character sequences that are likely not to
 * display, such as multiple occurrences of the same non-spacing mark.</li>
 * <li><code>CHAR_LIMIT</code>: flags strings that contain characters outside of
 * a specified set of acceptable characters. See {@link uspoof_setAllowedChars}
 * and {@link uspoof_setAllowedLocales}.</li> <li><code>MIXED_NUMBERS</code>:
 * flags strings that contain digits from multiple different numbering
 * systems.</li>
 * </ul>
 *
 * <p>
 * These checks can be enabled independently of each other. For example, if you
 * were interested in checking for only the INVISIBLE and MIXED_NUMBERS
 ...
 * \endcode
 *
 * Here is an example in C++ showing how to compute the restriction level of a
 * string:
 *
 * \code{.cpp}
 * UErrorCode status = U_ZERO_ERROR;
 ...
 * uspoof_setAllowedChars(sc.getAlias(), allowed.toUSet(), &status);
 * uspoof_setRestrictionLevel(sc.getAlias(), USPOOF_MODERATELY_RESTRICTIVE);
 * uspoof_setChecks(sc.getAlias(), USPOOF_RESTRICTION_LEVEL | USPOOF_AUX_INFO,
 * &status);
 *
 * LocalUSpoofCheckResultPointer checkResult(uspoof_openCheckResult(&status));
 * int32_t bitmask = uspoof_check2UnicodeString(sc.getAlias(), str,
 ...
 * uspoof_getCheckResultRestrictionLevel(checkResult.getAlias(), &status);
 * // Since USPOOF_AUX_INFO was enabled, the restriction level is also available
 * in the upper bits of the bitmask: assert((restrictionLevel & bitmask) ==
 * restrictionLevel);
 ...
 * \endcode
 *
 * The code '0x50000000' corresponds to the restriction level
 * USPOOF_MINIMALLY_RESTRICTIVE.  Since USPOOF_MINIMALLY_RESTRICTIVE is weaker
 * than USPOOF_MODERATELY_RESTRICTIVE, the string fails the check.
 *
 * <b>Note:</b> The Restriction Level is the most powerful of the checks. The
 * full logic is documented in <a
 * href="http://unicode.org/reports/tr39/#Restriction_Level_Detection">UTS
 * 39</a>, but the basic idea is that strings are restricted to contain
 * characters from only a single script, <em>except</em> that most scripts are
 * allowed to have Latin characters interspersed. Although the default
 * restriction level is <code>HIGHLY_RESTRICTIVE</code>, it is recommended that
 * users set their restriction level to <code>MODERATELY_RESTRICTIVE</code>,
 * which allows Latin mixed with all other scripts except Cyrillic, Greek, and
 * Cherokee, with which it is often confusable. For more details on the levels,
 * see UTS 39 or {@link URestrictionLevel}. The Restriction Level test is aware
 * of the set of allowed characters set in {@link uspoof_setAllowedChars}. Note
 * that characters which have script code COMMON or INHERITED, such as numbers
 * and punctuation, are ignored when computing whether a string has multiple
 * scripts.
 *
 * <h2>Additional Information</h2>
 *
 * A <code>USpoofChecker</code> instance may be used repeatedly to perform
 * checks on any number of identifiers.
 *
 * <b>Thread Safety:</b> The test functions for checking a single identifier, or
 * for testing whether two identifiers are possible confusable, are thread safe.
 * They may called concurrently, from multiple threads, using the same
 * USpoofChecker instance.
 *
 * More generally, the standard ICU thread safety rules apply: functions that
 * take a const USpoofChecker parameter are thread safe. Those that take a
 * non-const USpoofChecker are not thread safe..
 *
 * @stable ICU 4.6
 */
 ...
 * @stable ICU 4.2
 */
typedef struct USpoofChecker
    USpoofChecker; /**< typedef for C of USpoofChecker */
...
typedef enum USpoofChecks {
    /**
     * When performing the two-string {@link uspoof_areConfusable} test, this
     * flag in the return value indicates that the two strings are visually
     * confusable and that they are from the same script, according to UTS 39
     * section 4.
     *
     ...
     * When performing the two-string {@link uspoof_areConfusable} test, this
     * flag in the return value indicates that the two strings are visually
     * confusable and that they are <b>not</b> from the same script, according
     * to UTS 39 section 4.
     *
     * @see uspoof_areConfusable
     * @stable ICU 4.2
     ...
     * When performing the two-string {@link uspoof_areConfusable} test, this
     * flag in the return value indicates that the two strings are visually
     * confusable and that they are not from the same script but both of them
    ...
    /**
     * Enable this flag in {@link uspoof_setChecks} to turn on all types of
     * confusables.  You may set the checks to some subset of
     * SINGLE_SCRIPT_CONFUSABLE, MIXED_SCRIPT_CONFUSABLE, or
     * WHOLE_SCRIPT_CONFUSABLE to make {@link uspoof_areConfusable} return only
     * those types of confusables.
     *
     * @see uspoof_areConfusable
     ...
     * SpoofChecker.
     *
     * @deprecated ICU 58  Any case confusable mappings were removed from UTS
     * 39; the corresponding ICU API was deprecated.
     */
    USPOOF_ANY_CASE = 8,
     ...
     * RestrictionLevel. The default if {@link uspoof_setRestrictionLevel} is
     * not called is HIGHLY_RESTRICTIVE.
     *
     ...
     * {@link uspoof_setAllowedLocales}.  Note that a string that fails this
     * check will also fail the {@link USPOOF_RESTRICTION_LEVEL} check.
     */
    USPOOF_CHAR_LIMIT = 64,

    /**
     * Check that an identifier does not mix numbers from different numbering
     * systems. For more information, see UTS 39 section 5.3.
     *
     * @stable ICU 51
     */
    ...
    /**
     * Check that an identifier does not have a combining character following a
     * character in which that combining character would be hidden; for example
     * 'i' followed by a U+0307 combining dot.
     *
     * More specifically, the following characters are forbidden from preceding
     * a U+0307: <ul> <li>Those with the Soft_Dotted Unicode property (which
     * includes 'i' and 'j')</li> <li>Latin lowercase letter 'l'</li>
     ...
     * characters and U+0307 except those with combining class 0 or combining
     * class "Above" (230, same class as U+0307).
     *
     * This list and the number of combing characters considered by this check
     * may grow over time.
     *
     * @stable ICU 62
     */
     ...
     *
     * If this "check" is not enabled, the results of {@link uspoof_check} will
     * be zero when an identifier passes all of the enabled checks.
     *
     * If this "check" is enabled, (uspoof_check() & {@link USPOOF_ALL_CHECKS})
     * will be zero when an identifier passes all checks.
     *
     * @stable ICU 51
     */
...
typedef enum URestrictionLevel {
    /**
     * All characters in the string are in the identifier profile and all
     * characters in the string are in the ASCII range.
     *
     * @stable ICU 51
     */
    USPOOF_ASCII = 0x10000000,
    /**
     * The string classifies as ASCII-Only, or all characters in the string are
     * in the identifier profile and the string is single-script, according to
     * the definition in UTS 39 section 5.1.
     *
     * @stable ICU 53
     */
    USPOOF_SINGLE_SCRIPT_RESTRICTIVE = 0x20000000,
    /**
     * The string classifies as Single Script, or all characters in the string
     * are in the identifier profile and the string is covered by any of the
     * following sets of scripts, according to the definition in UTS 39
     * section 5.1:
     * <ul>
     *   <li>Latin + Han + Bopomofo (or equivalently: Latn + Hanb)</li>
     *   <li>Latin + Han + Hiragana + Katakana (or equivalently: Latn +
     * Jpan)</li> <li>Latin + Han + Hangul (or equivalently: Latn +Kore)</li>
     * </ul>
    ...
    /**
     * The string classifies as Highly Restrictive, or all characters in the
     * string are in the identifier profile and the string is covered by Latin
     * and any one other Recommended or Aspirational script, except Cyrillic,
     * Greek, and Cherokee.
     *
     * @stable ICU 51
     */
    ...
    /**
     * All characters in the string are in the identifier profile.  Allow
     * arbitrary mixtures of scripts.
     *
     * @stable ICU 51
     */
    ...
    /**
     * Mask for selecting the Restriction Level bits from the return value of
     * {@link uspoof_check}.
     *
     * @stable ICU 53
     */
...
/**
 * Open a Spoof checker from its serialized form, stored in 32-bit-aligned
 * memory. Inverse of uspoof_serialize(). The memory containing the serialized
 * data must remain valid and unchanged as long as the spoof checker, or any
 * cloned copies of the spoof checker, are in use.  Ownership of the memory
 * remains with the caller. The spoof checker (and any clones) must be closed
 * prior to deleting the serialized data.
 *
 * @param data a pointer to 32-bit-aligned memory containing the serialized form
 * of spoof data
 * @param length the number of bytes available at data;
 *               can be more than necessary
 * @param pActualLength receives the actual number of bytes at data taken up by
...
uspoof_openFromSerialized( const void* data,
                           int32_t length,
 ...
 * position in the input text (line, offset) of the error.
 ...
 * @stable ICU 4.2
 */
U_STABLE USpoofChecker* U_EXPORT2 uspoof_clone( const USpoofChecker* sc,
                                                UErrorCode* status );

/**
 * Specify the bitmask of checks that will be performed by {@link uspoof_check}.
 * Calling this method overwrites any checks that may have already been enabled.
 * By default, all checks are enabled.
 *
 * To enable specific checks and disable all others, the "whitelisted" checks
 * should be ORed together. For example, to fail strings containing characters
 * outside of the set specified by {@link uspoof_setAllowedChars} and also
 * strings that contain digits from mixed numbering systems:
 *
 * <pre>
 * {@code
 ...
 * </pre>
 *
 * To disable specific checks and enable all others, the "blacklisted" checks
 * should be ANDed away from ALL_CHECKS. For example, if you are not planning to
 * use the {@link uspoof_areConfusable} functionality, it is good practice to
 * disable the CONFUSABLE check:
 *
 * <pre>
 * {@code
 ...
 * </pre>
 *
 * Note that methods such as {@link uspoof_setAllowedChars}, {@link
 * uspoof_setAllowedLocales}, and
 * {@link uspoof_setRestrictionLevel} will enable certain checks when called.
 * Those methods will OR the check they enable onto the existing bitmask
 * specified by this method. For more details, see the documentation of those
 * methods.
 *
...
/**
 * Set the loosest restriction level allowed for strings. The default if this is
 * not called is
 * {@link USPOOF_HIGHLY_RESTRICTIVE}. Calling this method enables the {@link
 * USPOOF_RESTRICTION_LEVEL} and
 * {@link USPOOF_MIXED_NUMBERS} checks, corresponding to Sections 5.1 and 5.2 of
 * UTS 39. To customize which checks are to be performed by {@link
 * uspoof_check}, see {@link uspoof_setChecks}.
 *
 * @param sc       The USpoofChecker
...
uspoof_setRestrictionLevel( USpoofChecker* sc,
                            URestrictionLevel restrictionLevel );

/**
 * Get the Restriction Level that will be tested if the checks include {@link
 ...
 * placed on the allowed characters.
 *
 * @param status       The error code, set if this function encounters a
 * problem.
 * @stable ICU 4.2
 */
 ...
 *
 * @param sc      The USpoofChecker
 * @param id      A identifier to be checked for possible security issues, in
 * UTF8 format.
 ...
 * functions to return information about the identifier.  Information includes:
 * <ul>
 ...
 * </ul>
 * The data held in a USpoofCheckResult is cleared whenever it is passed into a
 * new call of {@link uspoof_check2}.
 *
 * @param status  The error code, set if this function encounters a problem.
...
/**
 * Indicates which of the spoof check(s) have failed. The value is a bitwise OR
 * of the constants for the tests in question: USPOOF_RESTRICTION_LEVEL,
 * USPOOF_CHAR_LIMIT, and so on.
 *
 * @param checkResult  The instance of USpoofCheckResult created by {@link
...
uspoof_getCheckResultChecks( const USpoofCheckResult* checkResult,
                             UErrorCode* status );

/**
 * Gets the restriction level that the text meets, if the
 * USPOOF_RESTRICTION_LEVEL check was enabled; otherwise, undefined.
 *
 * @param checkResult  The instance of USpoofCheckResult created by {@link
 * uspoof_openCheckResult}
 * @param status       The error code, set if an error occurred.
 ...
 */
U_STABLE URestrictionLevel U_EXPORT2
uspoof_getCheckResultRestrictionLevel( const USpoofCheckResult* checkResult,
                                       UErrorCode* status );

/**
 * Gets the set of numerics found in the string, if the USPOOF_MIXED_NUMBERS
 * check was enabled; otherwise, undefined.  The set will contain the zero digit
 * from each decimal number system found in the input string.  Ownership of the
 * returned USet remains with the USpoofCheckResult. The USet will be free'd
 * when {@link uspoof_closeCheckResult} is called.
 *
 * @param checkResult  The instance of USpoofCheckResult created by {@link
 * uspoof_openCheckResult}
...
uspoof_getCheckResultNumerics( const USpoofCheckResult* checkResult,
                               UErrorCode* status );
 ...
 *                return the actual length of the skeleton.
 * @param status  The error code, set if an error occurred while attempting to
 *                perform the check.  Possible Errors include
 * U_INVALID_CHAR_FOUND for invalid UTF-8 sequences, and U_BUFFER_OVERFLOW_ERROR
 * if the destination buffer is too small to hold the complete skeleton.
 * @return        The length of the skeleton string, in bytes.  The returned
 * length is always that of the complete skeleton, even when the supplied buffer
 * is too small (or of zero length)
 *
 * @stable ICU 4.2
 */
 ...
 * and documented in http://www.unicode.org/reports/tr39/, Unicode Security
 * Mechanisms.
 *
 * The returned set is frozen. Ownership of the set remains with the ICU
 * library; it must not be deleted by the caller.
 *
...
/**
 * Get the set of characters from Recommended Scripts for Inclusion in
 * Identifiers, as defined in
 * http://unicode.org/Public/security/latest/xidmodifications.txt and documented
 * in http://www.unicode.org/reports/tr39/, Unicode Security Mechanisms.
 *
 * The returned set is frozen. Ownership of the set remains with the ICU
 * library; it must not be deleted by the caller.
 *
 ...
 */
U_DEFINE_LOCAL_OPEN_POINTER( LocalUSpoofCheckerPointer,
                             USpoofChecker,
                             uspoof_close );
/** \endcond */
 ...
 * `uspoof_closeCheckResult()`. For most methods see the LocalPointerBase base
 * class.
 *
 * @see LocalPointerBase
 * @see LocalPointer
 ...
 *       For now, suppress with a Doxygen cond
 */
U_DEFINE_LOCAL_OPEN_POINTER( LocalUSpoofCheckResultPointer,
                             USpoofCheckResult,
 ...
 */
U_STABLE void U_EXPORT2
uspoof_setAllowedUnicodeSet( USpoofChecker* sc,
