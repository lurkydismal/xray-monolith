 ...
 * According to the Linux stdint.h, the ISO C99 standard specifies that in C++
 * implementations macros like INT32_MIN and UINTPTR_MAX should only be defined
 * if explicitly requested. We need to define __STDC_LIMIT_MACROS before
 * including stdint.h in C++ code that uses such limit macros.
 * @internal
 */
...
/*
 * We mostly need <stdint.h> (which defines the standard integer types) but not
 * <inttypes.h>. <inttypes.h> includes <stdint.h> and adds the printf/scanf
 * helpers PRId32, SCNx16 etc. which we almost never use, plus stuff like
 * imaxabs() which we never use.
 */
#include <stdint.h>
