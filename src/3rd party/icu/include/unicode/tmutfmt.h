...
typedef enum UTimeUnitFormatStyle
    UTimeUnitFormatStyle; /**< @deprecated ICU 53 */
 ...
 *   TimeUnitAmount* source = new TimeUnitAmount(2, TimeUnit::UTIMEUNIT_YEAR,
 * status);
...
public:
     ...
     * @see Format#parseObject(const UnicodeString&, Formattable&,
     * ParsePosition&) const;
     * @deprecated ICU 53
     */
...
private:
    ...
    void readFromCurrentLocale( UTimeUnitFormatStyle style,
                                const char* key,
    ...
    void checkConsistency( UTimeUnitFormatStyle style,
                           const char* key,
                           UErrorCode& status );

    // fill in fTimeUnitToCountToPatterns from locale fall-back chain
    void searchInLocaleChain( UTimeUnitFormatStyle style,
                              const char* key,
    ...
    // UTIMEUNIT_YEAR.
    static const char* getTimeUnitName( TimeUnit::UTimeUnitFields field,
                                        UErrorCode& status );

    friend struct TimeUnitFormatReadSink;
};

inline UBool TimeUnitFormat::operator!=( const Format& other ) const {
