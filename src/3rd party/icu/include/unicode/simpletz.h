 ...
 * <P>
 * When specifying daylight-savings-time begin and end dates, use a negative
 * value for <code>dayOfWeekInMonth</code> to indicate that
 * <code>SimpleTimeZone</code> should count from the end of the month backwards.
 * For example, if Daylight Savings Time starts or ends at the last Sunday a
 * month, use <code>dayOfWeekInMonth = -1</code> along with <code>dayOfWeek =
 * UCAL_SUNDAY</code> to specify the rule.
 *
 * @see      Calendar
...
public:
     ...
     * @return      True if the given time zone is equal to this time zone;
     * false otherwise.
     * @stable ICU 2.0
     */
    ...
    /**
     * Constructs a SimpleTimeZone with the given raw GMT offset and time zone
     * ID, and which doesn't observe daylight savings time.  Normally you should
     * use TimeZone::createInstance() to create a TimeZone instead of creating a
     ...
     * and times to start and end daylight savings time. To create a TimeZone
     * that doesn't observe daylight savings time, don't use this constructor;
     * use SimpleTimeZone(rawOffset, ID) instead. Normally, you should use
     ...
     * @param savingsStartTime  The daylight savings starting time, expressed as
     * the number of milliseconds after midnight.
     * @param savingsEndMonth   The daylight savings ending month. Month is
     *                          0-based. eg, 0 for January.
     * @param savingsEndDayOfWeekInMonth     The daylight savings ending
     * day-of-week-in-month. See setStartRule() for a complete explanation.
     * @param savingsEndDayOfWeek The daylight savings ending day-of-week.
     *                          See setStartRule() for a complete explanation.
     * @param savingsEndTime    The daylight savings ending time, expressed as
     * the number of milliseconds after midnight.
     ...
     * @param savingsEndTime    The daylight savings ending time, expressed as
     * the number of milliseconds after midnight.
     * @param savingsDST        The number of milliseconds added to standard
     * time to get DST time. Default is one hour.
     * @param status            An UErrorCode to receive the status.
     * @stable ICU 2.0
     */
    SimpleTimeZone( int32_t rawOffsetGMT,
     ...
     * @param savingsStartTime  The daylight savings starting time, expressed as
     * the number of milliseconds after midnight.
     * @param savingsStartTimeMode Whether the start time is local wall time,
     * local standard time, or UTC time. Default is local wall time.
     * @param savingsEndMonth   The daylight savings ending month. Month is
     *                          0-based. eg, 0 for January.
     * @param savingsEndDayOfWeekInMonth     The daylight savings ending
     * day-of-week-in-month. See setStartRule() for a complete explanation.
     ...
     *                          standard time, or UTC time. Default is local
     * wall time.
     * @param savingsDST        The number of milliseconds added to standard
     * time to get DST time. Default is one hour.
     * @param status            An UErrorCode to receive the status.
     ...
     * Daylight Savings Time starts at the second Sunday in March, at 2 AM in
     ...
     * follows: <ul> <li>If both dayOfWeekInMonth and dayOfWeek are positive,
     * they specify the day of week in the month (e.g., (2, WEDNESDAY) is the
     * second Wednesday of the month).</li> <li>If dayOfWeek is positive and
     * dayOfWeekInMonth is negative, they specify the day of week in the month
     * counting backward from the end of the month. (e.g., (-1, MONDAY) is the
     * last Monday in the month)</li> <li>If dayOfWeek is zero and
     * dayOfWeekInMonth is positive, dayOfWeekInMonth specifies the day of the
     * month, regardless of what day of the week it is. (e.g., (10, 0) is the
     * tenth day of the month)</li> <li>If dayOfWeek is zero and
     * dayOfWeekInMonth is negative, dayOfWeekInMonth specifies the day of the
     * month counting backward from the end of the month, regardless of what day
     * of the week it is (e.g., (-2, 0) is the next-to-last day of the
     * month).</li> <li>If dayOfWeek is negative and dayOfWeekInMonth is
     * positive, they specify the first specified day of the week on or after
     * the specfied day of the month. (e.g., (15, -SUNDAY) is the first Sunday
     * after the 15th of the month [or the 15th itself if the 15th is a
     * Sunday].)</li> <li>If dayOfWeek and DayOfWeekInMonth are both negative,
     * they specify the last specified day of the week on or before the
     * specified day of the month. (e.g., (-20, -TUESDAY) is the last Tuesday
     * before the 20th of the month [or the 20th itself if the 20th is a
     * Tuesday].)</li>
     * </ul>
     ...
     * @stable ICU 2.0
     */
    void setStartRule( int32_t month,
     ...
     * @param time          The time of that day (number of millis after
     * midnight) when DST takes effect in local wall time, which is standard
     * time in this case.
     * @param status An UErrorCode
     * @stable ICU 2.0
     */
     ...
     * time. Therefore, you can set the end rule by calling: <pre>
     ...
     * day-of-week-in-month. See setStartRule() for a complete explanation.
     * @param dayOfWeek the daylight savings ending day-of-week. See
     ...
     * @param dayOfMonth    The date in that month (1-based).
     * @param time          The time of that day (number of millis after
     * midnight) when DST ends in local wall time, which is daylight time in
     * this case.
     * @param status An UErrorCode
     * @stable ICU 2.0
     */
     ...
     * Gregorian calendar, and the date and time fields are in GMT, NOT local
     * time.
     *
     ...
     * @param millis     The reference date's milliseconds in day, UTT (NOT
     * local time).
     * @param status     An UErrorCode to receive the status.
     ...
     * @internal
     */
    virtual void getOffsetFromLocal( UDate date,
     ...
     * are in effect. Typically one hour (+3600000). The amount could be
     * negative, but not 0.
     * @param status  An UErrorCode to receive the status.
     * @stable ICU 2.0
     */
     ...
     *
     * @return   True if this TimeZone uses Daylight Savings Time; false
     * otherwise.
     * @stable ICU 2.0
     */
    ...
    /**
     * Returns true if the given date is within the period when daylight savings
     * time is in effect; false otherwise.  If the TimeZone doesn't observe
     * daylight savings time, this functions always returns false. This method
     * is wasteful since it creates a new GregorianCalendar and deletes it each
     * time it is called. This is a deprecated method and provided only for Java
     * compatibility.
     *
     ...
     * @stable ICU 3.8
     */
    virtual UBool getNextTransition( UDate base,
                                     UBool inclusive,
     ...
     * @param base      The base time.
     * @param inclusive Whether the base time is inclusive or not.
     * @param result    Receives the most recent transition before the base
     * time.
     * @return  TRUE if the transition is found.
     * @stable ICU 3.8
     */
    virtual UBool getPreviousTransition( UDate base,
                                         UBool inclusive,
                                         TimeZoneTransition& result ) const;

    /**
     * Returns the number of <code>TimeZoneRule</code>s which represents time
     * transitions, for this time zone, that is, all <code>TimeZoneRule</code>s
     * for this time zone except <code>InitialTimeZoneRule</code>.  The return
     * value range is 0 or any positive value.
     * @param status    Receives error status code.
     * @return The number of <code>TimeZoneRule</code>s representing time
     * transitions.
     * @stable ICU 3.8
     */
    ...
    /**
     * Gets the <code>InitialTimeZoneRule</code> and the set of
     * <code>TimeZoneRule</code> which represent time transitions for this time
     * zone.  On successful return, the argument initial points to non-NULL
     * <code>InitialTimeZoneRule</code> and the array trsrules is filled with 0
     * or multiple <code>TimeZoneRule</code> instances up to the size specified
     * by trscount.  The results are referencing the rule instance held by this
     * time zone instance.  Therefore, after this time zone is destructed, they
     * are no longer available.
     * @param initial       Receives the initial timezone rule
     * @param trsrules      Receives the timezone transition rules
     * @param trscount      On input, specify the size of the array
     * 'transitions' receiving the timezone transition rules.  On output, actual
     * number of rules filled in the array will be set.
     * @param status        Receives error status code.
     * @stable ICU 3.8
     */
...
public:
     ...
     * override. This method is to implement a simple version of RTTI, since not
     * all C++ compilers support genuine RTTI. Polymorphic operator==() and
     ...
     * a return value from getDynamicClassID(). For example: <pre> .   Base*
     * polymorphic_pointer = createPolymorphicObject(); .   if
     * (polymorphic_pointer->getDynamicClassID() == .
...
private:
     ...
     *                        standard time, or UTC time. Default is local wall
     * time.
     ...
     */
    static int32_t compareToRule( int8_t month,
                                  int8_t monthLen,
                                  int8_t prevMonthLen,
                                  int8_t dayOfMonth,
                                  int8_t dayOfWeek,
    ...
    int8_t startMonth, startDay,
        startDayOfWeek; // the month, day, DOW, and time DST starts
    int32_t startTime;
    TimeMode startTimeMode,
        endTimeMode; // Mode for startTime, endTime; see TimeMode
    int8_t endMonth, endDay,
        endDayOfWeek; // the month, day, DOW, and time DST ends
    int32_t endTime;
    ...
    EMode startMode,
        endMode; // flags indicating what kind of rules the DST rules are
...
inline void SimpleTimeZone::setStartRule( int32_t month,
                                          int32_t dayOfWeekInMonth,
                                          int32_t dayOfWeek,
                                          int32_t time,
                                          UErrorCode& status ) {
...
inline void SimpleTimeZone::setStartRule( int32_t month,
                                          ...
                                          UBool after,
                                          UErrorCode& status ) {
    setStartRule( month, dayOfMonth, dayOfWeek, time, WALL_TIME, after,
                  status );
}

inline void SimpleTimeZone::setEndRule( int32_t month,
                                        int32_t dayOfWeekInMonth,
                                        int32_t dayOfWeek,
                                        int32_t time,
                                        UErrorCode& status ) {
...
inline void SimpleTimeZone::getOffset( UDate date,
                                       UBool local,
