 ...
 *  regular expression.  They are created from a regular expression pattern
 * string, and can be used to create `RegexMatcher` objects for the pattern.
 *
 ...
 * expression pattern strings application code can be simplified and the
 * explicit need for `RegexPattern` objects can usually be eliminated.
 *
 */
 ...
 * form of a regular expression, methods for creating RegexMatchers that allow
 * the pattern to be applied to input text, and a few convenience methods for
 * simple common uses of regular expressions.
 *
...
public:
    /**
     * default constructor.  Create a RegexPattern object that refers to no
     * actual pattern.  Not normally needed; RegexPattern objects are usually
    ...
    /**
     * Comparison operator.  Two RegexPattern objects are considered equal if
     * they were constructed from identical source patterns using the same
     * #URegexpFlag settings.
    ...
    /**
     * Assignment operator.  After assignment, this RegexPattern will behave
     * identically to the source object.
     * @stable ICU 2.4
     */
    RegexPattern& operator=( const RegexPattern& source );

    /**
     * Create an exact copy of this RegexPattern object.  Since RegexPattern is
     * not intended to be subclassed, <code>clone()</code> and the copy
     * construction are equivalent operations.
     ...
     * usual way that RegexPattern objects are created.
     *
     * Note that RegexPattern objects must not be deleted while RegexMatcher
     * objects created from the pattern are active.  RegexMatchers keep a
     * pointer back to their pattern, so premature deletion of the pattern is a
     * catastrophic error.
     *
     * All #URegexpFlag pattern match mode flags are set to their default
     * values.
     *
     * Note that it is often more convenient to construct a RegexMatcher
     * directly from a pattern string rather than separately compiling the
     * pattern and then creating a RegexMatcher object from the pattern.
     *
     ...
     * pattern and then creating a RegexMatcher object from the pattern.
     *
     * @param regex The regular expression to be compiled. Note, the text
     * referred to by this UText must not be deleted during the lifetime of the
     *              RegexPattern object or any RegexMatcher object created from
     * it.
     * @param pe    Receives the position (line and column nubers) of any error
     *              within the regular expression.)
     * @param status A reference to a UErrorCode to receive any errors.
    ...
    /**
     * Compiles the regular expression in string form into a RegexPattern
     * object using the specified #URegexpFlag match mode flags.  These compile
     * methods, rather than the constructors, are the usual way that
     * RegexPattern objects are created.
     *
     * Note that RegexPattern objects must not be deleted while RegexMatcher
     * objects created from the pattern are active.  RegexMatchers keep a
     * pointer back to their pattern, so premature deletion of the pattern is a
     ...
     *
     * Note that it is often more convenient to construct a RegexMatcher
     * directly from a pattern string instead of than separately compiling the
     * pattern and then creating a RegexMatcher object from the pattern.
     *
     * @param regex The regular expression to be compiled.
     * @param flags The #URegexpFlag match mode flags to be used, e.g.
    ...
    /**
     * Get the #URegexpFlag match mode flags that were used when compiling this
     * pattern.
     ...
     * pattern.  The RegexMatcher can then be used to perform match, find or
     * replace operations on the input.  Note that a RegexPattern object must
     * not be deleted while RegexMatchers created from it still exist and might
     * possibly be used again.
     *
     * The matcher will retain a reference to the supplied input string, and all
     * regexp pattern matching operations happen directly on this original
     * string.  It is critical that the string not be altered or deleted before
     * use by the regular expression operations is complete.
     *
     * @param input    The input string to which the regular expression will be
     * applied.
     * @param status   A reference to a UErrorCode to receive any errors.
...
private:
     ...
     *
     * To efficiently work with char16_t *strings, wrap the data in a
     * UnicodeString using one of the aliasing constructors, such as
     * `UnicodeString(UBool isTerminated, const char16_t *text, int32_t
     * textLength);` or in a UText, using `utext_openUChars(UText *ut, const
     * char16_t *text, int64_t textLength, UErrorCode *status);`
     *
     */
    RegexMatcher* matcher( const char16_t* input, UErrorCode& status ) const;
...
public:
     ...
     * function both compiles the regular expression and applies it in a single
     * operation. Note that if the same pattern needs to be applied repeatedly,
     * this method will be less efficient than creating and reusing a
     * RegexMatcher object.
     *
     ...
     * method will work even if the pattern was compiled from a UText.
     *
     * Note: If the pattern was originally compiled from a UText, and that UText
     * was modified, the returned string may no longer reflect the RegexPattern
     * object.
     * @stable ICU 2.4
     */
    ...
    /**
     * Returns the regular expression from which this pattern was compiled. This
     * method will work even if the pattern was compiled from a UnicodeString.
     *
     * Note: This is the original input, not a clone. If the pattern was
     * originally compiled from a UText, and that UText was modified, the
     * returned UText may no longer reflect the RegexPattern object.
     *
     * @stable ICU 4.6
     */
     ...
     * @stable ICU 55
     */
    virtual int32_t groupNumberFromName( const char* groupName,
                                         int32_t nameLength,
                                         UErrorCode& status ) const;
     ...
     * This count includes the strings from capture groups in the delimiter
     * pattern. This behavior differs from Java, which ignores capture groups.
     *
     ...
     * @param dest    An array of UnicodeStrings to receive the results of the
     * split. This is an array of actual UnicodeString objects, not an array of
     * pointers to strings.  Local (stack based) arrays can work well here.
     * @param destCapacity  The number of elements in the destination array.
     *                If the number of fields found is less than destCapacity,
     * the extra strings in the destination array are not altered. If the number
     * of destination strings is less than the number of fields, the trailing
     * part of the input string, including any field delimiters, is placed in
     * the last destination string.
     * @param status  A reference to a UErrorCode to receive any errors.
     ...
     * @param input   The string to be split into fields.  The field delimiters
     *                match the pattern (in the "this" object)
     * @param dest    An array of mutable UText structs to receive the results
     * of the split. If a field is NULL, a new UText is allocated to contain the
     * results for that field. This new UText is not guaranteed to be mutable.
     * @param destCapacity  The number of elements in the destination array.
     *                If the number of fields found is less than destCapacity,
     * the extra strings in the destination array are not altered. If the number
     * of destination strings is less than the number of fields, the trailing
...
private:
    ...
    UnicodeString*
        fPatternString;      // The original pattern UncodeString if relevant
    uint32_t fFlags;         // The flags used when compiling the pattern.
                             //
    UVector64* fCompiledPat; // The compiled pattern p-code.
    UnicodeString
        fLiteralText; // Any literal string data from the pattern,
    ...
    int32_t fMinMatchLen; // Minimum Match Length.  All matches will have length
                          //   >= this value.  For some patterns, this
                          //   calculated value may be less than the true
...
public:
     ...
     *  @param status Any errors are reported by setting this UErrorCode
     * variable.
     *  @stable ICU 2.6
     */
    RegexMatcher( const UnicodeString& regexp,
                  uint32_t flags,
                  UErrorCode& status );
     ...
     *
     * The matcher will retain a reference to the supplied input string, and all
     * regexp pattern matching operations happen directly on the original
     * string.  It is critical that the string not be altered or deleted before
     * use by the regular expression operations is complete.
     *
     *  @param regexp The Regular Expression to be compiled.
     *  @param input  The string to match.  The matcher retains a reference to
     * the caller's string; mo copy is made.
     *  @param flags  #URegexpFlag options, such as #UREGEX_CASE_INSENSITIVE.
     *  @param status Any errors are reported by setting this UErrorCode
     * variable.
     *  @stable ICU 2.6
     ...
     * regexp pattern matching operations happen on this clone.  While read-only
     * operations on the supplied text are permitted, it is critical that the
     * underlying string not be altered or deleted before use by the regular
...
public:
     ...
     *    @param   startIndex The input string (native) index at which to begin
     * matching.
     *    @param   status     A reference to a UErrorCode to receive any errors.
     *    @return TRUE if there is a match
     *    @stable ICU 2.8
    ...
    /**
     *   Attempts to match the input string, starting from the beginning of the
     * region, against the pattern.  Like the matches() method, this function
     *   always starts at the beginning of the input region;
     *   unlike that function, it does not require that the entire region be
     * matched.
     *
     *   If the match succeeds then more information can be obtained via the
     * start(), end(), and group() functions.
     *
     *    @param   status     A reference to a UErrorCode to receive any errors.
     ...
     * against the pattern. The match may be of any length, and is not required
     * to extend to the end of the input string.  Contrast with match().
     *
     *   If the match succeeds then more information can be obtained via the
     * start(), end(), and group() functions.
     *
     ...
     *  the previous match, or at the start of the string if there is no
     * previous match. If a match is found, `start()`, `end()` and `group()`
     *  will provide more information regarding the match.
     *  Note that if the input string is changed by the application,
     *     use find(startPos, status) instead of find(), because the saved
     * starting position may not be valid with the altered input string.
     ...
     * of the input string that matches the pattern, starting at the specified
     * index.
     *
     *   @param   start     The (native) index in the input string to begin the
     * search.
     *   @param   status    A reference to a UErrorCode to receive any errors.
     *   @return  TRUE if a match is found.
     *   @stable ICU 2.4
     ...
     *   If the pattern can match an empty string, an empty string may be
     * returned.
     *   @param   status      A reference to a UErrorCode to receive any errors.
     *                        Possible errors are  U_REGEX_INVALID_STATE if no
     * match has been attempted or the last match failed.
     ...
     *    @param   status     A reference to a UErrorCode to receive any errors.
     *                        Possible errors are  U_REGEX_INVALID_STATE if no
     * match has been attempted or the last match failed and
     *                        U_INDEX_OUTOFBOUNDS_ERROR for a bad capture group
     * number.
    ...
    /**
     *   Returns a shallow clone of the entire live input string with the UText
     * current native index set to the beginning of the requested group.
     *
     *   @param   dest        The UText into which the input should be cloned,
     * or NULL to create a new UText
     *   @param   group_len   A reference to receive the length of the desired
     * capture group
     *   @param   status      A reference to a UErrorCode to receive any errors.
     *                        Possible errors are  U_REGEX_INVALID_STATE if no
     * match has been attempted or the last match failed and
     ...
     *   @stable ICU 4.6
     */
    virtual UText* group( UText* dest,
                          int64_t& group_len,
                          UErrorCode& status ) const;
     ...
     *   @stable ICU 4.6
     */
    virtual UText* group( int32_t groupNum,
                          UText* dest,
                          int64_t& group_len,
                          UErrorCode& status ) const;
     ...
     * errors.
     *    @return              The (native) position in the input string of the
     ...
     * Return -1 if the capture group exists in the pattern, but was not part of
     * the last match.
     *
     *    @param  group       the capture group number
     *    @param  status      A reference to a UErrorCode to receive any errors.
     * Possible errors are  U_REGEX_INVALID_STATE if no match has been attempted
     * or the last match failed, and U_INDEX_OUTOFBOUNDS_ERROR for a bad capture
     * group number
    ...
    /**
     *    Returns the index in the input string of the first character following
     * the text matched during the previous match operation.
     *
     *   @param   status      A reference to a UErrorCode to receive any errors.
     * Possible errors are  U_REGEX_INVALID_STATE if no match has been attempted
     * or the last match failed.
     *    @return the index of the last character matched, plus one.
     *                        The index value returned is a native index,
     * corresponding to code units for the underlying encoding type, for
     * example, a byte index for UTF-8.
     *   @stable ICU 2.4
     */
     ...
     *    text matched by the specified capture group during the previous match
     * operation.
     *
     *    @param group  the capture group number
     *    @param   status      A reference to a UErrorCode to receive any
     * errors.  Possible errors are  U_REGEX_INVALID_STATE if no match has been
     *                        attempted or the last match failed and
     *                        U_INDEX_OUTOFBOUNDS_ERROR for a bad capture group
     * number
     *    @return  the index of the first character following the text
     *              captured by the specified group during the previous match
     * operation. Return -1 if the capture group exists in the pattern but was
     * not part of the match. The index value returned is a native index,
     * corresponding to code units for the underlying encoding type, for
     * example, a byte index for UTF8.
     *    @stable ICU 2.4
     */
     ...
     * matches, and to cause subsequent find() operations to begin at the
     ...
     * RegexMatcher to be reused, which is more efficient than creating a new
     * RegexMatcher for each input string to be processed.
     *   @param input The new string on which subsequent pattern matches will
     * operate. The matcher retains a reference to the callers string, and
     * operates directly on that.  Ownership of the string remains with the
     * caller. Because no copy of the string is made, it is essential that the
     *                caller not delete the string until after regexp operations
     * on it are done. Note that while a reset on the matcher with an input
     * string that is then modified across/during matcher operations may be
     * supported currently for UnicodeString, this was not originally intended
     * behavior, and support for this is not guaranteed in upcoming versions of
     * ICU.
     *   @return this RegexMatcher.
     *   @stable ICU 2.4
     */
     ...
     * RegexMatcher for each input string to be processed.
     *   @param input The new string on which subsequent pattern matches will
     * operate. The matcher makes a shallow clone of the given text; ownership
     * of the original string remains with the caller. Because no deep copy of
     * the text is made, it is essential that the caller not modify the string
    ...
    /**
     *  Set the subject text string upon which the regular expression is looking
     * for matches without changing any other aspect of the matching state. The
     * new and previous text strings must have the same content.
     *
     *  This function is intended for use in environments where ICU is operating
     * on strings that may move around in memory.  It provides a mechanism for
     * notifying ICU that the string has been relocated, and providing a new
     * UText to access the string in its new position.
     *
     *  Note that the regular expression implementation never copies the
     * underlying text of a string being matched, but always operates directly
     * on the original text provided by the user. Refreshing simply drops the
     * references to the old text and replaces them with references to the new.
     *
     *  Caution:  this function is normally used only by very specialized,
     *  system-level code.  One example use case is with garbage collection that
     * moves the text in memory.
     *
...
public:
    /**
     *   Returns the input string being matched.  Ownership of the string
     * belongs to the matcher; it should not be altered or deleted. This method
     * will work even if the input was originally supplied as a UText.
     ...
     * it should not be altered or deleted. This method will work even if the
     * input was originally supplied as a UnicodeString.
     ...
     * provided UText parameter or by returning a shallow clone of the live
     * input. Note that copying the entire input may cause significant
     ...
     * The region is the part of the input string that will be searched to find
     * a match. Invoking this method resets the matcher, and then sets the
     * region to start at the index specified by the start parameter and end at
     * the index specified by the end parameter.
     *
     * Depending on the transparency and anchoring being used (see
     * useTransparentBounds and useAnchoringBounds), certain constructs such as
     * anchors may behave differently at or around the boundaries of the region
     *
     * The function will fail if start is greater than limit, or if either index
     *  is less than zero or greater than the length of the string being
     * matched.
     *
     ...
     * @stable ICU 4.0
     */
    virtual RegexMatcher& region( int64_t start,
                                  int64_t limit,
                                  UErrorCode& status );

    /**
     * Identical to region(start, limit, status) but also allows a start
     * position without resetting the region state.
     * @param regionStart The region start
     * @param regionLimit the limit of the region
     * @param startIndex  The (native) index within the region bounds at which
     * to begin searches.
     * @param status A reference to a UErrorCode to receive any errors.
     ...
     * @stable ICU 4.6
     */
    virtual RegexMatcher& region( int64_t regionStart,
                                  int64_t regionLimit,
                                  int64_t startIndex,
                                  UErrorCode& status );
    ...
    /**
     * Reports the end (limit) index (exclusive) of this matcher's region. The
     * searches this matcher conducts are limited to finding matches within
     * regionStart (inclusive) and regionEnd (exclusive).
     *
     ...
     * See useTransparentBounds for a description of transparent and opaque
     * bounds. By default, a matcher uses opaque region boundaries.
     *
     ...
     * Invoking this function with an argument of true will set this matcher to
     * use transparent bounds. If the boolean argument is false, then opaque
     * bounds will be used.
     *
     ...
     * Those constructs can see text beyond the boundaries of the region while
     * checking for a match.
     *
     * With opaque bounds, no text outside of the matcher's region is visible to
     * lookahead, lookbehind, and boundary matching constructs.
     *
     ...
     * With anchoring bounds, pattern anchors such as ^ and $ will match at the
     * start and end of the region.  Without Anchoring Bounds, anchors will only
     * match at the positions they would in the complete text.
     *
     ...
     *  In this case, additional input text could change the results of the
     * match.
     *
     ...
     * cause it to fail. If this method returns false and a match was found,
     * then more input might change the match but the match won't be lost. If a
     * match was not found, then requireEnd has no meaning.
     *
     * @return TRUE if more input could cause the most recent match to no longer
     * match.
     * @stable ICU 4.0
     */
     ...
     * that provides a complete find-and-replace-all operation.
     *
     ...
     * errors.
     *    @return              a string containing the results of the find and
     * replace. If a pre-allocated UText was provided, it will always be used
     * and returned.
     *
     *    @stable ICU 4.6
     */
    virtual UText* replaceAll( UText* replacement,
                               UText* dest,
                               UErrorCode& status );
     ...
     *
     * This function first resets this RegexMatcher. It then scans the input
     * string looking for a match of the pattern. Input that is not part of the
     * match is appended directly to the result string; the match is replaced in
     * the result by the replacement string. The replacement string may contain
     ...
     *    RegexMatcher should be reset before doing additional find()
     * operations.
     *
     *    @param   replacement a string containing the replacement text.
     *    @param   status      a reference to a UErrorCode to receive any
     ...
     *    @stable ICU 4.6
     */
    virtual UText* replaceFirst( UText* replacement,
                                 UText* dest,
                                 UErrorCode& status );
     ...
     * ending at the start of the current match, is appended to the destination
     * string.  Then the replacement string is appended to the output string,
     ...
     *
     *   @param   dest        A UnicodeString to which the results of the
     * find-and-replace are appended.
     *   @param   replacement A UnicodeString that provides the text to be
     * substituted for the input text that matched the regexp pattern.  The
     * replacement text may contain references to captured text from the input.
     *   @param   status      A reference to a UErrorCode to receive any errors.
     * Possible errors are  U_REGEX_INVALID_STATE if no match has been attempted
     * or the last match failed, and U_INDEX_OUTOFBOUNDS_ERROR if the
     * replacement text specifies a capture group that does not exist in the
     * pattern.
     *
     *   @return  this  RegexMatcher
     *   @stable ICU 2.4
     ...
     *   operations, see replaceFirst() or replaceAll().
     *
     *   @param   dest        A mutable UText to which the results of the
     * find-and-replace are appended. Must not be NULL.
     *   @param   replacement A UText that provides the text to be substituted
     * for the input text that matched the regexp pattern.  The replacement text
     * may contain references to captured text from the input.
     *   @param   status      A reference to a UErrorCode to receive any errors.
     * Possible errors are  U_REGEX_INVALID_STATE if no match has been attempted
     * or the last match failed, and U_INDEX_OUTOFBOUNDS_ERROR if the
     * replacement text specifies a capture group that does not exist in the
     ...
     * appendReplacement(), to the destination string. `appendTail()` is
     * intended to be invoked after one or more invocations of the
     * `RegexMatcher::appendReplacement()`.
     *
     *  @param dest A UnicodeString to which the results of the find-and-replace
     * are appended.
     ...
     * `RegexMatcher::appendReplacement()`.
     *
     *  @param dest A mutable UText to which the results of the find-and-replace
     * are appended. Must not be NULL.
     ...
     * time. For practical purposes, the match operation may appear to be in an
     *   infinite loop.
     ...
     * backtracking stack. The matcher is also reset, discarding any results
     * from previous matches.
     *
     *  ICU uses a backtracking regular expression engine, with the backtrack
     * stack maintained on the heap.  This function sets the limit to the amount
     * of memory that can be used for this purpose.  A backtracking stack
     * overflow will result in an error from the match operation that caused it.
     *
     *  A limit is desirable because a malicious or poorly designed pattern can
     * use excessive memory, potentially crashing the process.  A limit is
     * enabled by default.
     *
     *  @param limit  The maximum size, in bytes, of the matching backtrack
     * stack. A value of zero means no limit. The limit must be greater or equal
     * to zero.
     *
     *  @param status   A reference to a UErrorCode to receive any errors.
     *
     ...
     *                         and passed to the callback each time that it is
     * called.
     *    @param   status      A reference to a UErrorCode to receive any
     * errors.
     *  @stable ICU 4.0
     ...
     *
     *    @param   callback    Out parameter, receives a pointer to the
     * user-supplied callback function.
     *    @param   context     Out parameter, receives the user context pointer
     * that was set when uregex_setMatchCallback() was called.
     *    @param   status      A reference to a UErrorCode to receive any
     * errors.
     *    @stable ICU 4.0
     */
     ...
     * Matcher. During find operations, the callback will be invoked after each
     * return from a match attempt, giving the application the opportunity to
     * terminate a long-running find operation.
     *
     *    @param   callback    A pointer to the user-supplied callback function.
     *    @param   context     User context pointer.  The value supplied at the
     *                         time the callback function is set will be saved
     ...
     * user-supplied callback function.
     *    @param   context     Out parameter, receives the user context pointer
     * that was set when uregex_setFindProgressCallback() was called.
     *    @param   status      A reference to a UErrorCode to receive any
     * errors.
     *    @stable ICU 4.6
     */
     ...
     * engine. For internal ICU development use only.  DO NO USE!!!!
     *   @internal
     */
...
private:
    ...
    // Call user find callback function, if set. Return TRUE if operation should
    // be interrupted.
    inline UBool findProgressInterrupt( int64_t matchIndex,
                                        UErrorCode& status );
    ...
    RegexPattern*
        fPatternOwned; // Non-NULL if this matcher owns the pattern, and
                       //   should delete it when through.

    const UnicodeString*
        fInput;        // The string being matched. Only used for input()
    UText* fInputText; // The text being matched. Is never NULL.
    UText*
        fAltInputText; // A shallow copy of the text being matched.
    ...
    int64_t fMatchEnd; // First position after the end of the most recent match
                       ...
                       //   is active.
    int64_t
        fLastMatchEnd; // First position after the end of the previous match,
    ...
    URegexMatchCallback*
        fCallbackFn;              // Pointer to match progress callback funct.
                                  //   NULL if there is no callback.
    const void* fCallbackContext; // User Context ptr for callback function.

    URegexFindProgressCallback*
        fFindProgressCallbackFn; // Pointer to match progress callback funct.
                                 //   NULL if there is no callback.
    const void*
        fFindProgressCallbackContext; // User Context ptr for callback function.

    UBool fInputUniStrMaybeMutable; // Set when fInputText wraps a UnicodeString
                                    // that may be mutable - compatibility.
    ...
    UErrorCode fDeferredStatus; // Save error state that cannot be immediately
                                //   reported, or that permanently disables this
                                //   matcher.
