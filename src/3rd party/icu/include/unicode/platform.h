...
#ifdef U_IN_DOXYGEN
/*
 * Problem: "platform.h:335: warning: documentation for unknown define
 * U_HAVE_STD_STRING found." means that U_HAVE_STD_STRING is not documented.
 * Solution: #define any defines for non @internal API here, so that they are
 * visible in the docs.  If you just set PREDEFINED in Doxyfile.in,  they won't
 * be documented.
 */
 ...
 * AT&T and Sun collaborated on a project to merge the most popular Unix
 * variants on the market at that time: BSD, System V, and Xenix. This became
...
/**
 * Darwin is a POSIX-compliant operating system, composed of code developed by
 * Apple, as well as code derived from NeXTSTEP, BSD, and other projects, built
 * around the Mach kernel. Darwin forms the core set of components upon which
 * Mac OS X, Apple TV, and iOS are based. (Original description modified from
 * WikiPedia.)
 * @internal
 */
...
#include <TargetConditionals.h>
    ...
    TARGET_OS_IPHONE /* variant of TARGET_OS_MAC */
...
#if defined( __GNUC__ )
/* Solaris/GCC needs this header file to get the proper endianness. Normally,
 * this header file is included with stddef.h but on Solairs/GCC, the GCC
 * version of stddef.h is included which does not include this header file.
 */
 ...
 * We used to include inttypes.h which includes stdint.h but we usually do not
 * need the additional definitions from inttypes.h.
 * @internal
 */
#ifdef U_HAVE_STDINT_H
/* Use the predefined value. */
#elif U_PLATFORM_USES_ONLY_WIN32_API
#if defined( __BORLANDC__ ) || U_PLATFORM == U_PF_MINGW || \
    ( defined( _MSC_VER ) && _MSC_VER >= 1600 )
/* Windows Visual Studio 9 and below do not have stdint.h & inttypes.h, but VS
 * 2010 adds them. */
 ...
 *
 * For example, for testing for whether we have gcc, and whether it's 4.6 or
 * higher, use "#if U_GCC_MAJOR_MINOR >= 406".
 * @internal
 */
#ifdef __GNUC__
 ...
 * Attribute to specify the size of the allocated buffer for malloc-like
 * functions
 * @internal
 */
#if ( defined( __GNUC__ ) &&                                            \
 ...
 * Support for specific features cannot always be determined by the C++ version
 * alone.
 * @internal
 */
...
#define U_CPLUSPLUS_VERSION 1
    ...
    defined( __cplusplus ) && ( U_CPLUSPLUS_VERSION < 11 )
...
/**
 * U_CHARSET_FAMILY is equal to this value when the platform is an ASCII based
 * platform.
 * @stable ICU 2.0
 */
...
/**
 * U_CHARSET_FAMILY is equal to this value when the platform is an EBCDIC based
 * platform.
 * @stable ICU 2.0
 */
...
/* Use the predefined value. */
    ...
    ( !defined( __CHARSET_LIB ) || !__CHARSET_LIB )
...
/*
 * Android before Gingerbread (Android 2.3, API level 9) did not support
 * wchar_t. The type and header existed, but the library functions did not work
 * as expected. The size of wchar_t was 1 but L"xyz" string literals had 32-bit
 * units anyway.
 */
...
#elif ( U_PLATFORM == U_PF_ANDROID && __ANDROID_API__ < 9 )
/*
 * Classic Mac OS and Mac OS X before 10.3 (Panther) did not support wchar_t or
 * wstring. Newer Mac OS X has size 4.
 */
 ...
 * "In 31-bit mode, the size of long and pointers is 4 bytes and the size of
 * wchar_t is 2 bytes. Under LP64, the size of long and pointer is 8 bytes and
...
/* Use the predefined value. */
    ...
    ( defined( __xlC__ ) && defined( __IBM_UTF_LITERAL ) && \
      U_SIZEOF_WCHAR_T != 2 ) ||                            \
    ( defined( __HP_aCC ) && __HP_aCC >= 035000 ) ||        \
    ( defined( __HP_cc ) && __HP_cc >= 111106 ) || ( defined( U_IN_DOXYGEN ) )
#define U_DECLARE_UTF16( string ) u##string
#elif U_SIZEOF_WCHAR_T == 2 &&                                  \
    ( U_CHARSET_FAMILY == 0 ||                                  \
      ( U_PF_OS390 <= U_PLATFORM && U_PLATFORM <= U_PF_OS400 && \
        defined( __UCS2__ ) ) )
