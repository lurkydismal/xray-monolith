 ...
 * handled, e.g. for the German collator, characters &szlig; and SS will be
 * matched if case is chosen to be ignored. See the <a
 * href="http://source.icu-project.org/repos/icu/icuhtml/trunk/design/collation/ICU_collation_design.htm">
 * "ICU Collation Design Document"</a> for more information.
 * <p>
 * The implementation may use a linear search or a modified form of the
 * Boyer-Moore search; for more information on the latter see <a
 * href="http://icu-project.org/docs/papers/efficient_text_searching_in_java.html">
...
typedef enum {
     ...
     * Algorithm](http://www.unicode.org/reports/tr10/#Asymmetric_Search), but
     * also allows unmarked characters in the searched text to match marked or
     * unmarked versions of that character in the pattern.
     *
     * @stable ICU 4.4
     */
 ...
 * @stable ICU 2.4
 */
U_STABLE UStringSearch* U_EXPORT2
usearch_openFromCollator( const UChar* pattern,
                          int32_t patternlength,
                          const UChar* text,
                          int32_t textlength,
 ...
 * @stable ICU 2.4
 */
U_STABLE USearchAttributeValue U_EXPORT2
usearch_getAttribute( const UStringSearch* strsrch,
                      USearchAttribute attribute );
...
usearch_getMatchedStart( const UStringSearch* strsrch );
...
usearch_getPattern( const UStringSearch* strsrch, int32_t* length );
 ...
 *      ICU API.
 *
 *  The collator options, such as UCOL_STRENGTH and UCOL_NORMALIZTION, are
 * honored.
 *
 ...
 *  @param matchLimit Out parameter, the index of the first position following
 * the matched text. The matchLimit will be at a suitable position for beginning
 * a subsequent search in the input text. This parameter may be NULL. A value of
 * -1 will be returned if no match was found.
 *
 *  @param status     Report any errors.  Note that no match found is not an
 * error.
