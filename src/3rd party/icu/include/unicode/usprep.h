 ...
 *     <li> Mapping Table: Contains code points that are deleted from the output
 * or case mapped. </li>
 * </ul>
 *
 ...
 *                  e.g. "/usr/resource/my_app/profiles/mydata" on a Unix
 * system. if NULL, ICU default data files will be used.
 ...
 * @stable ICU 2.8
 */
U_STABLE UStringPrepProfile* U_EXPORT2 usprep_open( const char* path,
                                                    const char* fileName,
                                                    UErrorCode* status );
 ...
 * closed by calling usprep_close()
 * @see usprep_close()
 * @stable ICU 4.2
 */
 ...
 * operation maps, normalizes(NFKC), checks for prohibited and BiDi characters
 * in the order defined by RFC 3454 depending on the options specified in the
 * profile.
 *
 ...
 *
 *  - USPREP_DEFAULT            Prohibit processing of unassigned code points in
 * the input
 *
 *  - USPREP_ALLOW_UNASSIGNED   Treat the unassigned code points are in the
 * input as normal Unicode code points.
 *
...
U_STABLE int32_t U_EXPORT2 usprep_prepare( const UStringPrepProfile* prep,
                                           const UChar* src,
                                           int32_t srcLength,
                                           UChar* dest,
