...
#ifdef NDEBUG
static_assert( false,
               "Do not define NDEBUG macros in DEBUG configuration since "
               "luabind classes are sensisitve to it" )
#endif // #ifdef NDEBUG
#endif // #ifdef DEBUG
    ...
    typedef void*( __cdecl * memory_allocation_function_pointer )(
        memory_allocation_function_parameter parameter, void const*, size_t );
...
namespace luabind {
template < typename T >
class internal_vector : public std::vector< T, memory_allocator< T > > {
public:
...
class internal_multimap
    : public std::
          multimap< K, V, P, memory_allocator< std::pair< const K, V > > > {
public:
};

typedef std::
    basic_string< char, std::char_traits< char >, memory_allocator< char > >
        internal_string;
} // namespace luabind
