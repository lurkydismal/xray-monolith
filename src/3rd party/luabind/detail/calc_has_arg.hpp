...
struct calc_has_args {
    ...
    static constexpr int value() noexcept {
        typename find_conversion_policy< Index + Offset, Policies... >::type
            FindConversionsPolicyHack;
        using converter_policy = decltype( FindConversionsPolicyHack );
        return calc_has_args< ResInit, Offset, Until, Index + 1,
                              Result + toInt( converter_policy::has_arg ) >::
            template value< Policies... >();
    }
};
...
constexpr int calcHasArg() noexcept {
    return private_detail::calc_has_args<
        ResInit, Offset, Until >::template value< Policies... >();
}
