...
public:
    ...
    SimpleRand( time_e ) { seed( randomSeed() ); }

    /// Constructor that uses the given seed.
    SimpleRand( uint s = 0 ) { seed( s ); }

    /// Set the given seed.
    virtual void seed( uint s ) { current = s; }

    /// Get a random number.
    virtual uint get() { return current = current * 1103515245 + 12345; }
...
public:
    ...
    /// Constructor that uses the current time as the seed.
    MTRand( time_e ) { seed( randomSeed() ); }

    /// Constructor that uses the given seed.
    MTRand( uint s = 0 ) { seed( s ); }
...
public:
    ...
    GMRand( time_e ) { seed( randomSeed() ); }

    GMRand( uint s = 987654321 ) { seed( s ); }
...
public:
    Rand48( time_e ) { seed( randomSeed() ); }

    Rand48( uint s = 0x1234ABCD ) { seed( s ); }
