...
inline static float K( int l, int m ) {
    nvDebugCheck( m >= 0 );
    return sqrtf( ( ( 2 * l + 1 ) * factorial( l - m ) ) /
                  ( 4 * PI * factorial( l + m ) ) );
}
...
static float legendre( int l, int m, float x ) {
    ...
    if ( l == m ) {
        return powf( -1.0f, float( m ) ) * doubleFactorial( 2 * m - 1 ) *
               powf( 1 - x * x, 0.5f * m );
    }
    ...
    return ( x * ( 2 * l - 1 ) * legendrePolynomial( l - 1, m, x ) -
             ( l + m - 1 ) * legendrePolynomial( l - 2, m, x ) ) /
           ( l - m );
}
...
float nv::y( int l, int m, float theta, float phi ) {
    if ( m == 0 ) {
        ...
        return sqrtf( ( 2 * l + 1 ) / ( 4 * PI ) ) *
               legendrePolynomial( l, 0, cosf( theta ) );
    } else if ( m > 0 ) {
        return sqrtf( 2.0f ) * K( l, m ) * cosf( m * phi ) *
               legendrePolynomial( l, m, cosf( theta ) );
    } else {
        return sqrtf( 2.0f ) * K( l, -m ) * sinf( -m * phi ) *
               legendrePolynomial( l, -m, cosf( theta ) );
    }
}
...
float nv::hy( int l, int m, float theta, float phi ) {
    if ( m == 0 ) {
        ...
        return sqrtf( ( 2 * l + 1 ) / ( 2 * PI ) ) *
               legendrePolynomial( l, 0, 2 * cosf( theta ) - 1 );
    } else if ( m > 0 ) {
        return sqrtf( 2.0f ) * HK( l, m ) * cosf( m * phi ) *
               legendrePolynomial( l, m, 2 * cosf( theta ) - 1 );
    } else {
        return sqrtf( 2.0f ) * HK( l, -m ) * sinf( -m * phi ) *
               legendrePolynomial( l, -m, 2 * cosf( theta ) - 1 );
    }
}
