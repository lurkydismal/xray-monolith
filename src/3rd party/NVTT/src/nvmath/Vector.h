...
inline bool equal( Vector2::Arg v1,
                   Vector2::Arg v2,
                   float epsilon = NV_EPSILON ) {
...
inline Vector3 lerp( Vector3::Arg v1, Vector3::Arg v2, scalar t ) {
    ...
    return Vector3( v1.x() * s + t * v2.x(), v1.y() * s + t * v2.y(),
                    v1.z() * s + t * v2.z() );
}
...
inline Vector3 normalizeSafe( Vector3::Arg v,
                              Vector3::Arg fallback,
                              float epsilon = NV_EPSILON ) {
    float l = length( v );
    if ( isZero( l, epsilon ) ) {
        return fallback;
...
inline bool equal( Vector3::Arg v1,
                   Vector3::Arg v2,
                   float epsilon = NV_EPSILON ) {
    return equal( v1.x(), v2.x(), epsilon ) &&
           equal( v1.y(), v2.y(), epsilon ) && equal( v1.z(), v2.z(), epsilon );
}
...
inline Vector3 clamp( Vector3::Arg v, float min, float max ) {
    return Vector3( clamp( v.x(), min, max ), clamp( v.y(), min, max ),
                    clamp( v.z(), min, max ) );
}
...
inline Vector4 scale( Vector4::Arg v, Vector4::Arg s ) {
    return Vector4( v.x() * s.x(), v.y() * s.y(), v.z() * s.z(),
                    v.w() * s.w() );
}
...
inline Vector4 add_scaled( Vector4::Arg a, Vector4::Arg b, scalar s ) {
    return Vector4( a.x() + b.x() * s, a.y() + b.y() * s, a.z() + b.z() * s,
                    a.w() + b.w() * s );
}
...
inline Vector4 normalizeSafe( Vector4::Arg v,
                              Vector4::Arg fallback,
                              float epsilon = NV_EPSILON ) {
    float l = length( v );
    if ( isZero( l, epsilon ) ) {
        return fallback;
...
inline bool equal( Vector4::Arg v1,
                   Vector4::Arg v2,
                   float epsilon = NV_EPSILON ) {
    return equal( v1.x(), v2.x(), epsilon ) &&
           equal( v1.y(), v2.y(), epsilon ) &&
           equal( v1.z(), v2.z(), epsilon ) && equal( v1.w(), v2.w(), epsilon );
}

inline Vector4 min( Vector4::Arg a, Vector4::Arg b ) {
    return Vector4( min( a.x(), b.x() ), min( a.y(), b.y() ),
                    min( a.z(), b.z() ), min( a.w(), b.w() ) );
}

inline Vector4 max( Vector4::Arg a, Vector4::Arg b ) {
    return Vector4( max( a.x(), b.x() ), max( a.y(), b.y() ),
