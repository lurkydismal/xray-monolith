...
bool nv::ImageIO::saveFloat( const char* fileName,
                             const FloatImage* fimage,
                             uint base_component,
                             uint num_components ) {
    const char* extension = Path::extension( fileName );
...
#if defined( HAVE_OPENEXR )
    if ( strCaseCmp( extension, ".exr" ) == 0 ) {
        return ImageIO::saveFloatEXR( fileName, fimage, base_component,
                                      num_components );
    }
#endif
    ...
    if ( strCaseCmp( extension, ".tif" ) == 0 ||
         strCaseCmp( extension, ".tiff" ) == 0 ) {
        return ImageIO::saveFloatTIFF( fileName, fimage, base_component,
                                       num_components );
    }
#endif
    ...
    /* // @@ Disable Temporarily
            if (strCaseCmp(extension, ".pfm") == 0)
            {
    //		return ImageIO::saveFloatPFM(fileName, fimage, base_component,
    num_components);
            }
    */
    ...
    if ( num_components == 3 || num_components == 4 ) {
        AutoPtr< Image > image(
            fimage->createImage( base_component, num_components ) );
        nvCheck( image != NULL );
    ...
    if ( pal ) {
        for ( int y = 0; y < tga.height; y++ ) {
            for ( int x = 0; x < tga.width; x++ ) {
                uint8 idx = *src++;
                dst[ x ].setBGRA( palette[ 3 * idx + 0 ],
                                  palette[ 3 * idx + 1 ],
                                  palette[ 3 * idx + 2 ], 0xFF );
            }
            dst += lstep;
        }
    } else if ( grey ) {
...
Image* nv::ImageIO::loadPSD( Stream& s ) {
    ...
    if ( compression ) {
        s.seek( s.tell() +
                header.height * header.channel_count * sizeof( uint16 ) );
        ...
        for ( uint channel = 0; channel < channel_num; channel++ ) {
            ...
            while ( count < pixel_count ) {
                ...
                if ( len < 128 ) {
                    ...
                    // source byte. (Interpret len as a negative 8-bit int.)
                    len ^= 0xFF;
                    len += 2;
                    count += len;
...
Image* nv::ImageIO::loadJPG( Stream& s ) {
    ...
    cinfo.dct_method = JDCT_FLOAT;			// Choose floating point
    DCT method.
    */

    uint8* tmp_buffer = new uint8[ cinfo.output_width * cinfo.output_height *
...
FloatImage* nv::ImageIO::loadFloatTIFF( const char* fileName, Stream& s ) {
    ...
    // TIFF * tif = TIFFClientOpen(fileName, "r", &s, tiffReadWriteProc,
    ...
    for ( uint y = 0; y < height; y++ ) {
        ...
        for ( uint c = 0; c < spp; c++ ) {
            ...
            for ( uint x = 0; x < width; x++ ) {
                if ( bpp == 8 ) {
                    dst[ x ] = float( ( ( ::uint8* )buf )[ x * spp + c ] ) /
                               float( 0xFF );
                } else if ( bpp == 16 ) {
                    dst[ x ] = float( ( ( ::uint16* )buf )[ x * spp + c ] ) /
                               float( 0xFFFF );
                } else if ( bpp == 32 ) {
...
bool nv::ImageIO::saveFloatTIFF( const char* fileName,
                                 const FloatImage* fimage,
                                 uint base_component,
                                 uint num_components ) {
...
public:
    ...
    virtual Imf::Int64 tellg() { return m_stream.tell(); }

    virtual void seekg( Imf::Int64 pos ) { m_stream.seek( pos ); }

    virtual void clear() { m_stream.clearError(); }
...
FloatImage* nv::ImageIO::loadFloatEXR( const char* fileName, Stream& s ) {
    ...
    for ( Imf::ChannelList::ConstIterator it = channels.begin();
        ...
        frameBuffer.insert(
            it.name(), Imf::Slice( Imf::FLOAT, ( char* )fimage->channel( i ),
...
bool nv::ImageIO::saveFloatEXR( const char* fileName,
                                const FloatImage* fimage,
                                uint base_component,
                                uint num_components ) {
    nvCheck( fileName != NULL );
    ...
    for ( uint c = 0; c < num_components; c++ ) {
        header.channels().insert( channelNames[ c ],
    ...
    for ( uint c = 0; c < num_components; c++ ) {
        ...
        frameBuffer.insert( channelNames[ c ],
                            Imf::Slice( Imf::FLOAT, channel, sizeof( float ),
                                        sizeof( float ) * w ) );
    }
