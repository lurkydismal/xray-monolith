 ...
 * Schumacher http://tog.acm.org/GraphicsGems/gemsiii/filter.c
 *
...
float KaiserFilter::evaluate( float x ) const {
    ...
    if ( ( 1 - t * t ) >= 0 )
        return sinc_value * bessel0( alpha * sqrtf( 1 - t * t ) ) /
               bessel0( alpha );
    else
        return 0;
}
...
void Kernel2::transpose() {
    for ( uint i = 0; i < m_windowSize; i++ ) {
        for ( uint j = i + 1; j < m_windowSize; j++ ) {
            swap( m_data[ i * m_windowSize + j ],
                  m_data[ j * m_windowSize + i ] );
        }
    }
}
...
void Kernel2::initSobel() {
    if ( m_windowSize == 3 ) {
        ...
        float elements[] = { -1, -2, 0,  2,  1, -2, -3, 0,  3,  2, -3, -4, 0,
                             4,  3,  -2, -3, 0, 3,  2,  -1, -2, 0, 2,  1 };

        for ( int i = 0; i < 5 * 5; i++ ) {
            m_data[ i ] = elements[ i ];
        }
    } else if ( m_windowSize == 7 ) {
        float elements[] = { -1, -2, -3, 0,  3,  2,  1, -2, -3, -4, 0,  4,  3,
                             2,  -3, -4, -5, 0,  5,  4, 3,  -4, -5, -6, 0,  6,
                             5,  4,  -3, -4, -5, 0,  5, 4,  3,  -2, -3, -4, 0,
                             4,  3,  2,  -1, -2, -3, 0, 3,  2,  1 };
        ...
        float elements[] = {
            -1, -2, -3, -4, 0,  4,  3,  2,  1, -2, -3, -4, -5, 0,  5,  4,  3,
            2,  -3, -4, -5, -6, 0,  6,  5,  4, 3,  -4, -5, -6, -7, 0,  7,  6,
            5,  4,  -5, -6, -7, -8, 0,  8,  7, 6,  5,  -4, -5, -6, -7, 0,  7,
            6,  5,  4,  -3, -4, -5, -6, 0,  6, 5,  4,  3,  -2, -3, -4, -5, 0,
            5,  4,  3,  2,  -1, -2, -3, -4, 0, 4,  3,  2,  1 };
...
void Kernel2::initPrewitt() {
    if ( m_windowSize == 3 ) {
        ...
        // @@ Is this correct?
        float elements[] = { -2, -1, 0,  1,  2, -2, -1, 0,  1,  2, -2, -1, 0,
...
void Kernel2::initBlendedSobel( const Vector4& scale ) {
    ...
    {
        const float elements[] = {
            -1, -2, -3, 0,  3,  2,  1,  -2, -3, -4, 0,  4,  3,  2,  -3, -4, -5,
            0,  5,  4,  3,  -4, -5, -6, 0,  6,  5,  4,  -3, -4, -5, 0,  5,  4,
            3,  -2, -3, -4, 0,  4,  3,  2,  -1, -2, -3, 0,  3,  2,  1,
        };

        for ( int i = 0; i < 7; i++ ) {
            for ( int e = 0; e < 7; e++ ) {
                m_data[ ( i + 1 ) * 9 + e + 1 ] +=
                    elements[ i * 7 + e ] * scale.z();
            }
        }
    }
    {
        const float elements[] = { -1, -2, 0,  2,  1, -2, -3, 0,  3,
                                   2,  -3, -4, 0,  4, 3,  -2, -3, 0,
                                   3,  2,  -1, -2, 0, 2,  1 };
    ...
    }
    {
        ...
        for ( int i = 0; i < 3; i++ ) {
            for ( int e = 0; e < 3; e++ ) {
                m_data[ ( i + 3 ) * 9 + e + 3 ] +=
                    elements[ i * 3 + e ] * scale.x();
            }
        }
    }
}

PolyphaseKernel::PolyphaseKernel( const Filter& f,
                                  uint srcLength,
                                  uint dstLength,
                                  int samples /*= 32*/ ) {
    nvDebugCheck( samples > 0 );
    ...
    for ( uint i = 0; i < m_length; i++ ) {
        ...
        for ( int j = 0; j < m_windowSize; j++ ) {
            const float sample =
                f.sampleBox( left + j - center, scale, samples );
