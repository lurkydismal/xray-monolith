        ...
        for ( int c = 0; c < count; c++ ) {
            ...
            for ( int y = 0; y < h; y++ ) {
                for ( int x = 0; x < w; x++ ) {
                    ...
                    const bool west =
                        bmap->bitAt( img->indexClamp( x - 1, y ) );
                    const bool east =
                        bmap->bitAt( img->indexClamp( x + 1, y ) );
                    const bool north =
                        bmap->bitAt( img->indexClamp( x, y - 1 ) );
                    const bool south =
                        bmap->bitAt( img->indexClamp( x, y + 1 ) );
                    const bool northwest =
                        bmap->bitAt( img->indexClamp( x - 1, y - 1 ) );
                    ...
                    int num = west + east + north + south + northwest +
                    ...
                    if ( num != 0 ) {
                        ...
                        if ( num == 3 && west && northwest && southwest ) {
                        ...
                        } else if ( num == 3 && east && northeast &&
                                    southeast ) {
                            average = channel[ img->indexClamp( x + 1, y ) ];
                        } else if ( num == 3 && north && northwest &&
                                    northeast ) {
                            average = channel[ img->indexClamp( x, y - 1 ) ];
                        } else if ( num == 3 && south && southwest &&
                                    southeast ) {
                            average = channel[ img->indexClamp( x, y + 1 ) ];
                        } else {
                            float total = 0.0f;
                            if ( west ) {
                                average +=
                                    1 * channel[ img->indexClamp( x - 1, y ) ];
                                total += 1;
                            }
                            if ( east ) {
                                average +=
                            ...
                            if ( north ) {
                                average +=
                                    1 * channel[ img->indexClamp( x, y - 1 ) ];
                                total += 1;
                            }
                            if ( south ) {
                                average +=
                            ...
                            if ( northwest ) {
                                average +=
                                    channel[ img->indexClamp( x - 1, y - 1 ) ];
                                ++total;
                            }
                            if ( northeast ) {
                                average +=
...
static bool downsample( const FloatImage* src,
                        const BitMap* srcMask,
                        const FloatImage** _dst,
                        const BitMap** _dstMask ) {
...
void nv::fillPullPush( FloatImage* img, const BitMap* bmap ) {
    ...
    while ( downsample( mipmaps.back(), mipmapMasks.back(), &current,
                        &currentMask ) ) {
        ...
        for ( uint x = 0; x < w; x++ ) {
            ...
            for ( uint l = 0; l < levelCount; l++ ) {
                ...
                if ( mipmapMasks[ l ]->bitAt( sx, sy ) ) {
                    ...
                    for ( uint c = 0; c < count; c++ ) {
                        // img->setPixel(mipmaps[l]->linear_clamp(fx, fy, c), x,
                        // y, c);
                        img->setPixel( mipmaps[ l ]->pixel( sx, sy, c ), x, y,
                                       c );
                    }
                    break;
                }
...
Note that I'm working on normals, but I treat them just as 3 scalars and
normalize at the end.  To be more correct, I would work on the surface of a
sphere, but that just seems like way too much work.
...
struct LocalPixels {
    ...
    bool tryPlanar() const {
        // four cases :
        const int indices[] = { 2, 1, 1, 2, 1, 1, 2, 1, 3, 2, 3, 1,
                                2, 3, 1, 2, 1, 3, 2, 3, 3, 2, 3, 3 };
        bool res = false;
        for ( int i = 0; i < 4; i++ ) {
            ...
            result += data[ I[ 0 ] ][ I[ 1 ] ] + data[ I[ 2 ] ][ I[ 3 ] ] -
                      data[ I[ 4 ] ][ I[ 5 ] ];
            weight += 1.0f;
            res = true;
        }
    ...
    bool tryTwos() const {
        ...
        // four side-rotates :
        const int indices[] = {
            2, 1, 2, 0, 2, 3, 2, 4, 1, 2, 0, 2, 3, 2, 4, 2,
        };
        for ( int i = 0; i < 4; i++ ) {
            ...
            result +=
                data[ I[ 0 ] ][ I[ 1 ] ] * 2.0f - data[ I[ 2 ] ][ I[ 3 ] ];
            weight += 1.0f;
            res = true;
        }
...
// This is a quadratic extrapolation filter from Charles Bloom (DoPixelSeamFix).
// Used with his permission.
void nv::fillQuadraticExtrapolate( int passCount,
                                   FloatImage* img,
                                   BitMap* bmap,
                                   int coverageIndex /*= -1*/ ) {
    nvCheck( passCount > 0 );
            ...
            for ( int yb = 0; yb < h; yb++ ) {
                for ( int xb = 0; xb < w; xb++ ) {
                    ...
                    if ( lp.doLocalPixelFill() ) {
                        ...
                        if ( c == firstChannel ) {
                            // coverageChannel[idx] /= lp.weight;	// @@
                            // Not sure what this was for, coverageChannel[idx]
                            // is always zero.
                            newbmap->setBitAt( xb, yb );
                        }
                    }
