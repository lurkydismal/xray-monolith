...
Image* FloatImage::createImage( uint base_component /*= 0*/,
                                uint num /*= 4*/ ) const {
    nvCheck( num <= 4 );
...
/// Convert the floating point image to a regular image. Correct gamma of rgb,
/// but not alpha.
Image* FloatImage::createImageGammaCorrect( float gamma /*= 2.2f*/ ) const {
    ...
    const uint size = m_width * m_height;
    for ( uint i = 0; i < size; i++ ) {
        const uint8 r = nv::clamp(
            int( 255.0f * pow( rChannel[ i ], 1.0f / gamma ) ), 0, 255 );
        const uint8 g = nv::clamp(
            int( 255.0f * pow( gChannel[ i ], 1.0f / gamma ) ), 0, 255 );
        const uint8 b = nv::clamp(
            int( 255.0f * pow( bChannel[ i ], 1.0f / gamma ) ), 0, 255 );
...
void FloatImage::allocate( uint c, uint w, uint h ) {
    ...
    m_componentNum = c;
    m_count = w * h * c;
    m_mem = reinterpret_cast< float* >(
        nv::mem::malloc( m_count * sizeof( float ) ) );
}
...
void FloatImage::scaleBias( uint base_component,
                            uint num,
                            float scale,
                            float bias ) {
    const uint size = m_width * m_height;
...
void FloatImage::toLinear( uint base_component,
                           uint num,
                           float gamma /*= 2.2f*/ ) {
...
void FloatImage::toGamma( uint base_component,
                          uint num,
                          float gamma /*= 2.2f*/ ) {
...
float FloatImage::sampleNearest( const float x,
                                 const float y,
                                 const int c,
                                 const WrapMode wm ) const {
...
float FloatImage::sampleLinear( const float x,
                                const float y,
                                const int c,
                                const WrapMode wm ) const {
    if ( wm == WrapMode_Clamp )
...
float FloatImage::sampleNearestClamp( const float x,
                                      const float y,
                                      const int c ) const {
...
float FloatImage::sampleNearestRepeat( const float x,
                                       const float y,
                                       const int c ) const {
...
float FloatImage::sampleNearestMirror( const float x,
                                       const float y,
                                       const int c ) const {
...
///
/// When the size of the image is odd, this uses a polyphase box filter as
/// explained in: http://developer.nvidia.com/object/np2_mipmapping.html
///
FloatImage* FloatImage::fastDownSample() const {
    ...
    // Regular box filter.
        ...
        for ( uint c = 0; c < m_componentNum; c++ ) {
            ...
            for ( uint y = 0; y < h; y++ ) {
                for ( uint x = 0; x < w; x++ ) {
                    *dst = 0.25f * ( src[ 0 ] + src[ 1 ] + src[ m_width ] +
                                     src[ m_width + 1 ] );
                    dst++;
                    src += 2;
                }
    ...
    // Polyphase filters.
        ...
        for ( uint c = 0; c < m_componentNum; c++ ) {
            ...
            for ( uint y = 0; y < h; y++ ) {
                ...
                for ( uint x = 0; x < w; x++ ) {
                    ...
                    float f = 0.0f;
                    f += v0 * ( w0 * src[ 0 * m_width + 2 * x ] +
                                w1 * src[ 0 * m_width + 2 * x + 1 ] +
                                w2 * src[ 0 * m_width + 2 * x + 2 ] );
                    f += v1 * ( w0 * src[ 1 * m_width + 2 * x ] +
                                w1 * src[ 1 * m_width + 2 * x + 1 ] +
                                w2 * src[ 0 * m_width + 2 * x + 2 ] );
                    f += v2 * ( w0 * src[ 2 * m_width + 2 * x ] +
                                w1 * src[ 2 * m_width + 2 * x + 1 ] +
                                w2 * src[ 0 * m_width + 2 * x + 2 ] );
        ...
        for ( uint c = 0; c < m_componentNum; c++ ) {
            ...
            for ( uint y = 0; y < h; y++ ) {
                ...
                for ( uint x = 0; x < w; x++ ) {
                    float f = 0.0f;
                    f += v0 * ( src[ 0 * m_width + 2 * x ] +
                                src[ 0 * m_width + 2 * x + 1 ] );
                    f += v1 * ( src[ 1 * m_width + 2 * x ] +
                                src[ 1 * m_width + 2 * x + 1 ] );
                    f += v2 * ( src[ 2 * m_width + 2 * x ] +
                                src[ 2 * m_width + 2 * x + 1 ] );
...
/// Downsample applying a 1D kernel separately in each dimension.
FloatImage* FloatImage::downSample( const Filter& filter,
                                    WrapMode wm,
                                    uint alpha ) const {
    const uint w = max( 1, m_width / 2 );
    const uint h = max( 1, m_height / 2 );
...
/// Downsample applying a 1D kernel separately in each dimension.
FloatImage* FloatImage::resize( const Filter& filter,
                                uint w,
                                uint h,
                                WrapMode wm ) const {
    ...
    // if (w * m_height <= h * m_width)
    {
        ...
        for ( uint c = 0; c < m_componentNum; c++ ) {
            ...
            for ( uint y = 0; y < m_height; y++ ) {
                this->applyKernelHorizontal( xkernel, y, c, wm,
                                             tmp_channel + y * w );
            }
            ...
            for ( uint x = 0; x < w; x++ ) {
                tmp_image->applyKernelVertical( ykernel, x, c, wm,
                                                tmp_column.unsecureBuffer() );
...
FloatImage* FloatImage::resize( const Filter& filter,
...

    {
        ...
        for ( uint c = 0; c < m_componentNum; c++ ) {
            ...
            for ( uint y = 0; y < m_height; y++ ) {
                this->applyKernelHorizontal( xkernel, y, c, alpha, wm,
                                             tmp_channel + y * w );
            }
        }

        // Process all channels before applying vertical kernel to make sure
        // alpha has been computed.

        for ( uint c = 0; c < m_componentNum; c++ ) {
            float* dst_channel = dst_image->channel( c );

            for ( uint x = 0; x < w; x++ ) {
                tmp_image->applyKernelVertical( ykernel, x, c, alpha, wm,
                                                tmp_column.unsecureBuffer() );
...
float FloatImage::applyKernel( const Kernel2* k,
                               int x,
                               int y,
                               uint c,
                               WrapMode wm ) const {
...
float FloatImage::applyKernelVertical( const Kernel1* k,
                                       int x,
                                       int y,
                                       uint c,
                                       WrapMode wm ) const {
...
float FloatImage::applyKernelHorizontal( const Kernel1* k,
                                         int x,
                                         int y,
                                         uint c,
                                         WrapMode wm ) const {
...
/// Apply 1D vertical kernel at the given coordinates and return result.
void FloatImage::applyKernelVertical( const PolyphaseKernel& k,
                                      int x,
                                      uint c,
                                      WrapMode wm,
...
/// Apply 1D horizontal kernel at the given coordinates and return result.
void FloatImage::applyKernelHorizontal( const PolyphaseKernel& k,
                                        int y,
                                        uint c,
                                        WrapMode wm,
                                        float* __restrict output ) const {
