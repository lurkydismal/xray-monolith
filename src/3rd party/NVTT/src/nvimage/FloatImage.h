...
public:
    ...
    NVIMAGE_API Image* createImage( uint base_component = 0,
                                    uint num = 4 ) const;
    ...
    NVIMAGE_API void scaleBias( uint base_component,
                                uint num,
                                float scale,
                                float add );
    ...
    NVIMAGE_API void toLinear( uint base_component,
                               uint num,
                               float gamma = 2.2f );
    NVIMAGE_API void toGamma( uint base_component,
                              uint num,
                              float gamma = 2.2f );
    NVIMAGE_API void exponentiate( uint base_component, uint num, float power );

    NVIMAGE_API FloatImage* fastDownSample() const;
    NVIMAGE_API FloatImage* downSample( const Filter& filter,
                                        WrapMode wm ) const;
    NVIMAGE_API FloatImage* downSample( const Filter& filter,
                                        WrapMode wm,
                                        uint alpha ) const;
    NVIMAGE_API FloatImage* resize( const Filter& filter,
                                    uint w,
                                    uint h,
                                    WrapMode wm ) const;
    ...
    NVIMAGE_API float applyKernel( const Kernel2* k,
                                   int x,
                                   int y,
                                   uint c,
                                   WrapMode wm ) const;
    NVIMAGE_API float applyKernelVertical( const Kernel1* k,
                                           int x,
                                           int y,
                                           uint c,
                                           WrapMode wm ) const;
    NVIMAGE_API float applyKernelHorizontal( const Kernel1* k,
                                             int x,
                                             int y,
                                             uint c,
                                             WrapMode wm ) const;
    NVIMAGE_API void applyKernelVertical( const PolyphaseKernel& k,
                                          int x,
                                          uint c,
                                          WrapMode wm,
                                          float* output ) const;
    NVIMAGE_API void applyKernelHorizontal( const PolyphaseKernel& k,
                                            int y,
                                            uint c,
                                            WrapMode wm,
                                            float* output ) const;
...
inline uint FloatImage::indexClamp( int x, int y ) const {
    return nv::clamp( y, int( 0 ), int( m_height - 1 ) ) * m_width +
           nv::clamp( x, int( 0 ), int( m_width - 1 ) );
}
...
inline uint FloatImage::indexRepeat( int x, int y ) const {
    return repeat_remainder( y, m_height ) * m_width +
           repeat_remainder( x, m_width );
}
