...
void ColorBlock::init( const Image* img, uint x, uint y ) {
    ...
    static int remainder[] = {
        0, 0, 0, 0, 0, 1, 0, 1, 0, 1, 2, 0, 0, 1, 2, 3,
    };
...
Color32 ColorBlock::averageColor() const {
    ...
    return Color32( uint8( r / 16 ), uint8( g / 16 ), uint8( b / 16 ),
                    uint8( a / 16 ) );
}
...
void ColorBlock::computeRange( Vector3::Arg axis,
                               Color32* start,
                               Color32* end ) const {
    nvDebugCheck( start != NULL );
    nvDebugCheck( end != NULL );
    ...
    float min, max;
    min = max =
        dot( Vector3( m_color[ 0 ].r, m_color[ 0 ].g, m_color[ 0 ].b ), axis );
