...
#define NV_FOREACH( i, container )                                             \
    ...
    for ( NV_STRING_JOIN2( cont,                                               \
                           __LINE__ )::PseudoIndex i( ( container ).start() ); \
          !( container ).isDone( i ); ( container ).advance( i ) )
/*
#define NV_FOREACH(i, container) \
        for(typename typeof(container)::PseudoIndex i((container).start());
!(container).isDone(i); (container).advance(i))
*/
...
#define NV_FOREACH( i, container )                      \
    for ( PseudoIndexWrapper i( container );            \
          !( container ).isDone( i( &( container ) ) ); \
          ( container ).advance( i( &( container ) ) ) )
...
void deleteAll( T& container ) {
    for ( typename T::PseudoIndex i = container.start(); !container.isDone( i );
          container.advance( i ) ) {
        delete container[ i ];
    }
}
...
public:
    ...
    void push_back( const T& val ) {
        ...
        if ( new_size > m_buffer_size ) {
            const T copy(
                val ); // create a copy in case value is inside of this array.
            resize( new_size );
    ...
    /// Insert the given element at the given index shifting all the elements
    /// up.
    ...
    void append( const T other[], uint count ) {
        if ( count > 0 ) {
            ...
            resize( m_size + count );
            // Must use operator=() to copy elements, in case of side effects
            // (e.g. ref-counting).
    ...
    void resize( uint new_size ) {
        ...
        if ( m_size == 0 ) {
            // Allocate(0);	// Don't shrink automatically.
                    ...
                    m_buffer_size /* && m_size > m_buffer_size >> 1*/ ) {
            // don't compact yet.
    ...
    /// Assignment operator.
    void operator=( const Array< T >& a ) { copy( a.m_buffer, a.m_size ); }
...
private:
    ...
    void allocate( uint rsize ) {
        ...
        // realloc the buffer
            ...
            if ( m_buffer )
                m_buffer =
                    ( T* )mem::realloc( m_buffer, sizeof( T ) * m_buffer_size );
            else
...
public:
    ...
    explicit HashMap( int size_hint )
        : entry_count( 0 ), size_mask( -1 ), table( NULL ) {
    ...
    void add( const T& key, const U& value ) {
        ...
        if ( natural_entry->isEmpty() ) {
            ...
            if ( int( natural_entry->hash_value & size_mask ) == index ) {
                ...
                new ( blank_entry )
                    Entry( *natural_entry ); // placement new, copy ctor
                ...
                // Find natural location of collided element (i.e. root of
                // chain)
                ...
                for ( ;; ) {
                    ...
                    nvCheck( collided_index >= 0 &&
                             collided_index <= size_mask );
                }
    ...
    bool isEmpty() const { return table == NULL || entry_count == 0; }
    ...
    bool contains( const T& key ) const { return get( key ); }
...
private:
    ...
    int findIndex( const T& key ) const {
        ...
        for ( ;; ) {
            nvCheck( ( e->hash_value & size_mask ) ==
                     ( hash_value & size_mask ) );
            ...
            nvDebugCheck(
                !( e->key == key ) ); // keys are equal, but hash differs!
