...
//			include
//"M:\\dev7\\vs\\devtools\\common\\win32sdk\\include\\dbghelp.h"
#else
...
static LONG WINAPI
nvTopLevelFilter( struct _EXCEPTION_POINTERS* pExceptionInfo ) {
    NV_UNUSED( pExceptionInfo );
    /*	BOOL (WINAPI * Dump) (HANDLE, DWORD, HANDLE, MINIDUMP_TYPE,
       ...
       EXCEPTION_CONTINUE_SEARCH;
            ...
            // create the file
            HANDLE hFile = ::CreateFile( "nv.dmp", GENERIC_WRITE,
       FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL ); if(
       hFile == INVALID_HANDLE_VALUE ) { nvDebug("*** Failed to create dump
       file.\n"); return EXCEPTION_CONTINUE_SEARCH;
            }
            ...
            // write the dump
            bool ok = Dump( GetCurrentProcess(), GetCurrentProcessId(), hFile,
       MiniDumpNormal, &ExInfo, NULL, NULL )!=0;
            ::CloseHandle(hFile);
...
static void* callerAddress( void* secret ) {
    ...
    // How to obtain the instruction pointers in different platforms, from
    // mlton's source code. http://mlton.org/ OpenBSD && NetBSD ucp->sc_eip
    // FreeBSD:
...
struct Win32AssertHandler : public AssertHandler {
    ...
    static bool isDebuggerPresent() {
        ...
        if ( kern_lib ) {
            FARPROC lIsDebuggerPresent =
                GetProcAddress( kern_lib, "IsDebuggerPresent" );
    ...
    // Flush the message queue. This is necessary for the message box to show
    // up.
    ...
    virtual int assert( const char* exp,
                        const char* file,
                        int line,
        ...
        flushMessageQueue();
        int action = MessageBoxA( NULL, error_string, "Assertion failed",
