...
public:
    ...
    StringBuilder& operator=( const StringBuilder& s ) { return copy( s ); }

    /// Implement value semantics.
    StringBuilder& operator=( const char* s ) { return copy( s ); }
...
public:
    ...
    /// Constructs a shared string from a standard string.
    String( const char* str, int length ) { setString( str, length ); }

    /// Constructs a shared string from a StringBuilder.
    String( const StringBuilder& str ) { setString( str ); }
...
private:
    ...
    void setRefCount( uint16 count ) {
        ...
        *reinterpret_cast< uint16* >( const_cast< char* >( data - 2 ) ) =
            uint16( count );
    }

    void setData( const char* str ) { data = str + 2; }
    ...
    void allocString( const char* str, int len ) {
        const char* ptr =
            static_cast< const char* >( mem::malloc( 2 + len + 1 ) );
    ...
    ///	Swap strings.
    friend void swap( String& a, String& b ) { swap( a.data, b.data ); }
