...
#define nvAssertMacro( exp )                                      \
    do {                                                          \
        if ( !( exp ) ) {                                         \
            if ( nvAbort( #exp, __FILE__, __LINE__, __FUNC__ ) == \
                 NV_ABORT_DEBUG ) {                               \
                nvDebugBreak();                                   \
            }                                                     \
        }                                                         \
...
// Use nvAssume for very simple expresions only: nvAssume(0), nvAssume(value ==
// true), etc.
#if defined( _DEBUG )
#if NV_CC_MSVC
...
#define nvWarning( x ) \
    nvDebug( "*** Warning %s/%d: %s\n", __FILE__, __LINE__, ( x ) )

#if PI_CC_MSVC
// @@ I'm not sure it's a good idea to use the default static assert.
#define nvStaticCheck( x ) _STATIC_ASSERT( x )
#else
#define nvStaticCheck( x ) \
    typedef char NV_DO_STRING_JOIN2( __static_assert_, __LINE__ )[ ( x ) ]
// 	define nvStaticCheck(x) switch(0) { case 0: case x:; }
#endif

NVCORE_API int nvAbort( const char* exp,
                        const char* file,
                        int line,
                        const char* func = 0 );
NVCORE_API void NV_CDECL nvDebug( const char* msg, ... )
