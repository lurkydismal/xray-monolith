...
public:
    ...
    virtual bool isError() const { return m_fp == NULL || ferror( m_fp ) != 0; }
...
public:
    ...
    virtual bool isAtEnd() const { return m_ptr == m_mem + m_size; }
...
public:
    ...
    virtual uint tell() const { return m_s->tell(); }

    virtual uint size() const { return m_s->size(); }

    virtual bool isError() const { return m_s->isError(); }

    virtual void clearError() { m_s->clearError(); }

    virtual bool isAtEnd() const { return m_s->isAtEnd(); }

    virtual bool isSeekable() const { return m_s->isSeekable(); }
