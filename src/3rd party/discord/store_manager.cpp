...
public:
    static void OnEntitlementCreate( void* callbackData,
    ...
    static void OnEntitlementDelete( void* callbackData,
                                     DiscordEntitlement* entitlement ) {
        auto* core = reinterpret_cast< Core* >( callbackData );
        if ( !core ) {
            return;
...
Result StoreManager::GetSku( Snowflake skuId, Sku* sku ) {
    ...
    auto result = internal_->get_sku( internal_, skuId,
                                      reinterpret_cast< DiscordSku* >( sku ) );
...
void StoreManager::FetchEntitlements(
...
void StoreManager::CountEntitlements( std::int32_t* count ) {
    ...
    internal_->count_entitlements( internal_,
                                   reinterpret_cast< int32_t* >( count ) );
}

Result StoreManager::GetEntitlement( Snowflake entitlementId,
...
Result StoreManager::GetEntitlementAt( std::int32_t index,
                                       Entitlement* entitlement ) {
    if ( !entitlement ) {
        return Result::InternalError;
    }
...
Result StoreManager::HasSkuEntitlement( Snowflake skuId,
                                        bool* hasEntitlement ) {
    if ( !hasEntitlement ) {
        return Result::InternalError;
    }
...
void StoreManager::StartPurchase( Snowflake skuId,
                                  std::function< void( Result ) > callback ) {
    static auto wrapper = []( void* callbackData,
                              EDiscordResult result ) -> void {
        std::unique_ptr< std::function< void( Result ) > > cb(
