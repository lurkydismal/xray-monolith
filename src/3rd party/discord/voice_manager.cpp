...
void VoiceManager::SetInputMode( InputMode inputMode,
...
Result VoiceManager::IsLocalMute( Snowflake userId, bool* mute ) {
    ...
    auto result = internal_->is_local_mute( internal_, userId,
                                            reinterpret_cast< bool* >( mute ) );
    return static_cast< Result >( result );
}

Result VoiceManager::SetLocalMute( Snowflake userId, bool mute ) {
    auto result =
        internal_->set_local_mute( internal_, userId, ( mute ? 1 : 0 ) );
    return static_cast< Result >( result );
}
