...
Result Core::Create( ClientId clientId, std::uint64_t flags, Core** instance ) {
    ...
    auto result = DiscordCreate( DISCORD_VERSION, &params,
                                 &( ( *instance )->internal_ ) );
...
void Core::SetLogHook( LogLevel minLevel,
    ...
    static auto wrapper = []( void* callbackData, EDiscordLogLevel level,
        ...
        ( *cb )( static_cast< LogLevel >( level ),
                 static_cast< const char* >( message ) );
    };

    internal_->set_log_hook( internal_,
                             static_cast< EDiscordLogLevel >( minLevel ),
                             &setLogHook_, wrapper );
}
...
discord::ActivityManager& Core::ActivityManager() {
    if ( !activityManager_.internal_ ) {
        activityManager_.internal_ =
            internal_->get_activity_manager( internal_ );
    }
