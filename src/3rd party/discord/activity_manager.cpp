...
Result ActivityManager::RegisterCommand( char const* command ) {
    auto result = internal_->register_command( internal_,
...
void ActivityManager::UpdateActivity(
    ...
    internal_->update_activity( internal_,
                                reinterpret_cast< DiscordActivity* >(
                                    const_cast< Activity* >( &activity ) ),
                                cb.release(), wrapper );
}
...
void ActivityManager::SendRequestReply(
    ...
    internal_->send_request_reply(
        internal_, userId,
        static_cast< EDiscordActivityJoinRequestReply >( reply ), cb.release(),
        wrapper );
}
...
void ActivityManager::AcceptInvite( UserId userId,
                                    std::function< void( Result ) > callback ) {
    static auto wrapper = []( void* callbackData,
                              EDiscordResult result ) -> void {
        std::unique_ptr< std::function< void( Result ) > > cb(
