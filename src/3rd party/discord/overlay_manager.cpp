...
void OverlayManager::SetLocked( bool locked,
    ...
    internal_->set_locked( internal_, ( locked ? 1 : 0 ), cb.release(),
                           wrapper );
}
...
void OverlayManager::OpenActivityInvite(
    ...
    internal_->open_activity_invite(
        internal_, static_cast< EDiscordActivityActionType >( type ),
        cb.release(), wrapper );
}
...
void OverlayManager::OpenGuildInvite(
    ...
    std::unique_ptr< std::function< void( Result ) > > cb{};
    cb.reset( new std::function< void( Result ) >( std::move( callback ) ) );
    internal_->open_guild_invite( internal_, const_cast< char* >( code ),
                                  cb.release(), wrapper );
}

void OverlayManager::OpenVoiceSettings(
    std::function< void( Result ) > callback ) {
    static auto wrapper = []( void* callbackData,
                              EDiscordResult result ) -> void {
        std::unique_ptr< std::function< void( Result ) > > cb(
...
Result OverlayManager::InitDrawingDxgi( IDXGISwapChain* swapchain,
                                        bool useMessageForwarding ) {
    auto result = internal_->init_drawing_dxgi(
        internal_, swapchain, ( useMessageForwarding ? 1 : 0 ) );
...
void OverlayManager::KeyEvent( bool down,
                               char const* keyCode,
                               KeyVariant variant ) {
    internal_->key_event( internal_, ( down ? 1 : 0 ),
...
void OverlayManager::MouseButtonEvent( std::uint8_t down,
                                       ...
                                       std::int32_t x,
                                       std::int32_t y ) {
    internal_->mouse_button_event( internal_, down, clickCount,
                                   static_cast< EDiscordMouseButton >( which ),
                                   x, y );
}
...
void OverlayManager::ImeSetComposition( char const* text,
                                        ...
                                        std::int32_t from,
                                        std::int32_t to ) {
    internal_->ime_set_composition(
        internal_, const_cast< char* >( text ),
        reinterpret_cast< DiscordImeUnderline* >( underlines ),
        underlinesLength, from, to );
}
...
void OverlayManager::SetImeCompositionRangeCallback(
    ...
    static auto wrapper = []( void* callbackData, int32_t from, int32_t to,
                              DiscordRect* bounds,
                              uint32_t boundsLength ) -> void {
        std::unique_ptr< std::function< void( std::int32_t, std::int32_t, Rect*,
                                              std::uint32_t ) > >
            cb( reinterpret_cast< std::function< void(
                    std::int32_t, std::int32_t, Rect*, std::uint32_t ) >* >(
                callbackData ) );
        if ( !cb || !( *cb ) ) {
            return;
        }
    ...
    std::unique_ptr< std::function< void( std::int32_t, std::int32_t, Rect*,
                                          std::uint32_t ) > >
        cb{};
    cb.reset( new std::function< void( std::int32_t, std::int32_t, Rect*,
                                       std::uint32_t ) >(
        std::move( onImeCompositionRangeChanged ) ) );
    internal_->set_ime_composition_range_callback( internal_, cb.release(),
                                                   wrapper );
}

void OverlayManager::SetImeSelectionBoundsCallback(
    std::function< void( Rect, Rect, bool ) > onImeSelectionBoundsChanged ) {
    static auto wrapper = []( void* callbackData, DiscordRect anchor,
                              DiscordRect focus, bool isAnchorFirst ) -> void {
        std::unique_ptr< std::function< void( Rect, Rect, bool ) > > cb(
            reinterpret_cast< std::function< void( Rect, Rect, bool ) >* >(
                callbackData ) );
        if ( !cb || !( *cb ) ) {
            return;
        }
    ...
    std::unique_ptr< std::function< void( Rect, Rect, bool ) > > cb{};
    cb.reset( new std::function< void( Rect, Rect, bool ) >(
        std::move( onImeSelectionBoundsChanged ) ) );
    internal_->set_ime_selection_bounds_callback( internal_, cb.release(),
                                                  wrapper );
}
