...
public:
    ...
    static void OnRelationshipUpdate( void* callbackData,
...
void RelationshipManager::Filter(
    ...
    static auto wrapper = []( void* callbackData,
                              DiscordRelationship* relationship ) -> bool {
        auto cb(
            reinterpret_cast< std::function< bool( Relationship const& ) >* >(
                callbackData ) );
        if ( !cb || !( *cb ) ) {
            return {};
        }
    ...
    cb.reset( new std::function< bool( Relationship const& ) >(
        std::move( filter ) ) );
...
Result RelationshipManager::GetAt( std::uint32_t index,
