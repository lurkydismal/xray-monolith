...
#elif GLM_COMPILER & \
    ( GLM_COMPILER_GCC | GLM_COMPILER_APPLE_CLANG | GLM_COMPILER_LLVM )
...
inline vec< 4, T, P > make_vec4( vec< 1, T, P > const& v ) {
    return vec< 4, T, P >( v.x, static_cast< T >( 0 ), static_cast< T >( 0 ),
                           static_cast< T >( 1 ) );
}

template < typename T, precision P >
inline vec< 4, T, P > make_vec4( vec< 2, T, P > const& v ) {
    return vec< 4, T, P >( v.x, v.y, static_cast< T >( 0 ),
                           static_cast< T >( 1 ) );
}
