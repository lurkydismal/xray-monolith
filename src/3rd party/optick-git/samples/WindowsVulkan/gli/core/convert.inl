...
inline texture_type convert( texture_type const& Texture, format Format ) {
    ...
    typedef typename detail::convert< texture_type, T, defaultp >::fetchFunc
        fetch_type;
    typedef typename detail::convert< texture_type, T, defaultp >::writeFunc
        write_type;
    ...
    fetch_type Fetch =
        detail::convert< texture_type, T, defaultp >::call( Texture.format() )
            .Fetch;
    write_type Write =
        detail::convert< texture_type, T, defaultp >::call( Format ).Write;

    texture Storage( Texture.target(), Format, Texture.texture::extent(),
    ...
    for ( size_type Layer = 0; Layer < Texture.layers(); ++Layer )
        for ( size_type Face = 0; Face < Texture.faces(); ++Face )
            for ( size_type Level = 0; Level < Texture.levels(); ++Level ) {
                extent_type const& Dimensions =
                    Texture.texture::extent( Level );

                for ( component_type k = 0; k < Dimensions.z; ++k )
                    for ( component_type j = 0; j < Dimensions.y; ++j )
                        for ( component_type i = 0; i < Dimensions.x; ++i ) {
                            typename texture_type::extent_type const Texelcoord(
                                extent_type( i, j, k ) );
                            Write( Copy, Texelcoord, Layer, Face, Level,
                                   Fetch( Texture, Texelcoord, Layer, Face,
                                          Level ) );
                        }
            }
