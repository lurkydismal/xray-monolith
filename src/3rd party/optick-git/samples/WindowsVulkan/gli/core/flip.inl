...
inline void flip( image ImageDst, image ImageSrc, size_t BlockSize ) {
    ...
    for ( int y = 0; y < ImageDst.extent().y; ++y ) {
        ...
        memcpy( ImageDst.data< glm::byte >() + OffsetDst,
                ImageSrc.data< glm::byte >() + OffsetSrc, LineSize );
    }
}

inline void flip_block_s3tc( uint8_t* BlockDst,
                             uint8_t* BlockSrc,
                             format Format,
                             bool HeightTwo ) {
    ...
    // Moreover, in DXT1 (which does not contain an alpha channel), transparency
    // can be emulated using Color0 and Color1 on a per-compression-block basis.
    // There is no difference in how textures with and without transparency are
    // laid out in the file, so they can be flipped using the same method.
    if ( Format == FORMAT_RGB_DXT1_UNORM_BLOCK8 ||
         Format == FORMAT_RGB_DXT1_SRGB_BLOCK8 ||
         Format == FORMAT_RGBA_DXT1_UNORM_BLOCK8 ||
    ...
    // DXT3
    if ( Format == FORMAT_RGBA_DXT3_UNORM_BLOCK16 ||
         Format == FORMAT_RGBA_DXT3_SRGB_BLOCK16 ) {
    ...
    // DXT5
    if ( Format == FORMAT_RGBA_DXT5_UNORM_BLOCK16 ||
         Format == FORMAT_RGBA_DXT5_SRGB_BLOCK16 ) {
        ...
        if ( HeightTwo ) {
            ...
            // operator+ has precedence over operator>> and operator<<, hence
            // the parentheses. very important! the values below are bitmasks
            // used to retrieve alpha values according to the DXT specification
            // 0xF0 == 0b11110000 and 0xF == 0b1111
            Dst->AlphaBitmap[ 0 ] = ( ( Src->AlphaBitmap[ 1 ] & 0xF0 ) >> 4 ) +
                                    ( ( Src->AlphaBitmap[ 2 ] & 0xF ) << 4 );
            Dst->AlphaBitmap[ 1 ] = ( ( Src->AlphaBitmap[ 2 ] & 0xF0 ) >> 4 ) +
                                    ( ( Src->AlphaBitmap[ 0 ] & 0xF ) << 4 );
            Dst->AlphaBitmap[ 2 ] = ( ( Src->AlphaBitmap[ 0 ] & 0xF0 ) >> 4 ) +
                                    ( ( Src->AlphaBitmap[ 1 ] & 0xF ) << 4 );
        ...
        Dst->Alpha[ 0 ] = Src->Alpha[ 0 ];
        Dst->Alpha[ 1 ] = Src->Alpha[ 1 ];
        // operator+ has precedence over operator>> and operator<<, hence the
        // parentheses. very important! the values below are bitmasks used to
        // retrieve alpha values according to the DXT specification 0xF0 ==
        // 0b11110000 and 0xF == 0b1111
        Dst->AlphaBitmap[ 0 ] = ( ( Src->AlphaBitmap[ 4 ] & 0xF0 ) >> 4 ) +
                                ( ( Src->AlphaBitmap[ 5 ] & 0xF ) << 4 );
        Dst->AlphaBitmap[ 1 ] = ( ( Src->AlphaBitmap[ 5 ] & 0xF0 ) >> 4 ) +
                                ( ( Src->AlphaBitmap[ 3 ] & 0xF ) << 4 );
        Dst->AlphaBitmap[ 2 ] = ( ( Src->AlphaBitmap[ 3 ] & 0xF0 ) >> 4 ) +
                                ( ( Src->AlphaBitmap[ 4 ] & 0xF ) << 4 );
        Dst->AlphaBitmap[ 3 ] = ( ( Src->AlphaBitmap[ 1 ] & 0xF0 ) >> 4 ) +
                                ( ( Src->AlphaBitmap[ 2 ] & 0xF ) << 4 );
        Dst->AlphaBitmap[ 4 ] = ( ( Src->AlphaBitmap[ 2 ] & 0xF0 ) >> 4 ) +
                                ( ( Src->AlphaBitmap[ 0 ] & 0xF ) << 4 );
        Dst->AlphaBitmap[ 5 ] = ( ( Src->AlphaBitmap[ 0 ] & 0xF0 ) >> 4 ) +
                                ( ( Src->AlphaBitmap[ 1 ] & 0xF ) << 4 );
        Dst->Color0 = Src->Color0;
        Dst->Color1 = Src->Color1;
        Dst->Row[ 0 ] = Src->Row[ 3 ];
...
inline void flip_s3tc( image ImageDst, image ImageSrc, format Format ) {
    if ( ImageSrc.extent().y == 1 ) {
        memcpy( ImageDst.data(), ImageSrc.data(), ImageSrc.size() );
        return;
    }

    std::size_t const XBlocks =
        ImageSrc.extent().x <= 4 ? 1 : ImageSrc.extent().x / 4;
    if ( ImageSrc.extent().y == 2 ) {
        for ( std::size_t i_block = 0; i_block < XBlocks; ++i_block )
            flip_block_s3tc(
                ImageDst.data< uint8_t >() + i_block * block_size( Format ),
                ImageSrc.data< uint8_t >() + i_block * block_size( Format ),
                Format, true );
    ...
    for ( std::size_t i_row = 0; i_row <= MaxYBlock; ++i_row )
        for ( std::size_t i_block = 0; i_block < XBlocks; ++i_block )
            flip_block_s3tc(
                ImageDst.data< uint8_t >() +
                    ( MaxYBlock - i_row ) * block_size( Format ) * XBlocks +
                    i_block * block_size( Format ),
                ImageSrc.data< uint8_t >() +
                    i_row * block_size( Format ) * XBlocks +
                    i_block * block_size( Format ),
                Format, false );
}
...
inline texture2d flip( texture2d const& Texture ) {
    ...
    if ( !is_compressed( Texture.format() ) ) {
        ...
        for ( texture2d::size_type Level = 0; Level < Flip.levels(); ++Level )
            detail::flip_s3tc( Flip[ Level ], Texture[ Level ],
                               Texture.format() );
...
inline texture2d_array flip( texture2d_array const& Texture ) {
    ...
    texture2d_array Flip( Texture.format(), Texture.extent(), Texture.layers(),
                          Texture.levels() );
    ...
    if ( !gli::is_compressed( Texture.format() ) ) {
        ...
        for ( texture2d_array::size_type Layer = 0; Layer < Flip.layers();
              ++Layer )
            for ( texture2d_array::size_type Level = 0; Level < Flip.levels();
                  ++Level )
                detail::flip( Flip[ Layer ][ Level ], Texture[ Layer ][ Level ],
                              BlockSize );
    } else
        for ( texture2d_array::size_type Layer = 0; Layer < Flip.layers();
              ++Layer )
            for ( texture2d_array::size_type Level = 0; Level < Flip.levels();
                  ++Level )
                detail::flip_s3tc( Flip[ Layer ][ Level ],
                                   Texture[ Layer ][ Level ],
                                   Texture.format() );
...
inline texture_cube flip( texture_cube const& Texture ) {
    ...
    if ( !gli::is_compressed( Texture.format() ) ) {
        ...
        for ( texture_cube::size_type Face = 0; Face < Flip.faces(); ++Face )
            for ( texture_cube::size_type Level = 0; Level < Flip.levels();
                  ++Level )
                detail::flip( Flip[ Face ][ Level ], Texture[ Face ][ Level ],
                              BlockSize );
    } else
        for ( texture_cube::size_type Face = 0; Face < Flip.faces(); ++Face )
            for ( texture_cube::size_type Level = 0; Level < Flip.levels();
                  ++Level )
                detail::flip_s3tc( Flip[ Face ][ Level ],
                                   Texture[ Face ][ Level ], Texture.format() );
...
inline texture_cube_array flip( texture_cube_array const& Texture ) {
    ...
    texture_cube_array Flip( Texture.format(), Texture.extent(),
                             Texture.layers(), Texture.levels() );
        ...
        for ( std::size_t Layer = 0; Layer < Flip.layers(); ++Layer )
            for ( std::size_t Face = 0; Face < Flip.faces(); ++Face )
                for ( std::size_t Level = 0; Level < Flip.levels(); ++Level )
                    detail::flip( Flip[ Layer ][ Face ][ Level ],
                                  Texture[ Layer ][ Face ][ Level ],
                                  BlockSize );
    } else
        for ( std::size_t Layer = 0; Layer < Flip.layers(); ++Layer )
            for ( std::size_t Face = 0; Face < Flip.faces(); ++Face )
                for ( std::size_t Level = 0; Level < Flip.levels(); ++Level )
                    detail::flip_s3tc( Flip[ Layer ][ Face ][ Level ],
                                       Texture[ Layer ][ Face ][ Level ],
                                       Texture.format() );
