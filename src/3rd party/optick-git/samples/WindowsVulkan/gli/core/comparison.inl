...
inline bool equalData( texture const& TextureA, texture const& TextureB ) {
    ...
    for ( texture::size_type LayerIndex = 0, LayerCount = TextureA.layers();
          LayerIndex < LayerCount; ++LayerIndex )
        for ( texture::size_type FaceIndex = 0, FaceCount = TextureA.faces();
              FaceIndex < FaceCount; ++FaceIndex )
            for ( texture::size_type LevelIndex = 0,
                                     LevelCount = TextureA.levels();
                  LevelIndex < LevelCount; ++LevelIndex ) {
                void const* PointerA =
                    TextureA.data( LayerIndex, FaceIndex, LevelIndex );
                void const* PointerB =
                    TextureB.data( LayerIndex, FaceIndex, LevelIndex );
                if ( std::memcmp( PointerA, PointerB,
                                  TextureA.size( LevelIndex ) ) != 0 )
                    return false;
            }
