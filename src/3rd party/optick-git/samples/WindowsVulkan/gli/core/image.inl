namespace gli {
namespace detail {
inline size_t texel_linear_aAdressing( extent1d const& Extent,
...
inline size_t texel_linear_adressing( extent2d const& Extent,
...
inline size_t texel_linear_adressing( extent3d const& Extent,
    ...
    return static_cast< size_t >(
        TexelCoord.x + Extent.x * ( TexelCoord.y + Extent.y * TexelCoord.z ) );
}

inline size_t texel_morton_adressing( extent1d const& Extent,
                                      extent1d const& TexelCoord ) {
    GLI_ASSERT( TexelCoord.x < Extent.x );
...
inline size_t texel_morton_adressing( extent2d const& Extent,
                                      extent2d const& TexelCoord ) {
    GLI_ASSERT( TexelCoord.x < Extent.x && TexelCoord.x >= 0 &&
                TexelCoord.x <
                    std::numeric_limits< extent2d::value_type >::max() );
    GLI_ASSERT( TexelCoord.y < Extent.y && TexelCoord.y >= 0 &&
                TexelCoord.y <
                    std::numeric_limits< extent2d::value_type >::max() );
...
inline size_t texel_morton_adressing( extent3d const& Extent,
                                      extent3d const& TexelCoord ) {
    GLI_ASSERT( TexelCoord.x < Extent.x );
    GLI_ASSERT( TexelCoord.y < Extent.y );
    GLI_ASSERT( TexelCoord.z < Extent.z );
    ...
    return static_cast< size_t >(
        glm::bitfieldInterleave( Input.x, Input.y, Input.z ) );
}
} // namespace detail
...
inline image::image( format_type Format, extent_type const& Extent )
...
inline image::image( std::shared_ptr< storage_linear > Storage,
                     format_type Format,
                     size_type BaseLayer,
                     size_type BaseFace,
                     size_type BaseLevel )
...
inline image::image( image const& Image, format_type Format )
...
inline image::extent_type image::extent() const {
    ...
    storage_linear::extent_type const& SrcExtent =
        this->Storage->extent( this->BaseLevel );
    storage_linear::extent_type const& DstExtent =
        SrcExtent * block_extent( this->format() ) /
        this->Storage->block_extent();
...
inline void image::clear( genType const& Texel ) {
    ...
    for ( size_type TexelIndex = 0; TexelIndex < this->size< genType >();
          ++TexelIndex )
        *( this->data< genType >() + TexelIndex ) = Texel;
}

inline image::data_type* image::compute_data( size_type BaseLayer,
                                              size_type BaseFace,
                                              size_type BaseLevel ) {
    size_type const BaseOffset =
        this->Storage->base_offset( BaseLayer, BaseFace, BaseLevel );
...
genType image::load( extent_type const& TexelCoord ) {
    ...
    return *( this->data< genType >() +
              detail::texel_linear_adressing( this->extent(), TexelCoord ) );
}
...
void image::store( extent_type const& TexelCoord, genType const& Data ) {
    ...
    *( this->data< genType >() +
       detail::texel_linear_adressing( this->extent(), TexelCoord ) ) = Data;
}
} // namespace gli
