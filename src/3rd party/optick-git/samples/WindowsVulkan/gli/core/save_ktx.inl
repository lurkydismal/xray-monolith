...
inline texture::size_type compute_ktx_storage_size( texture const& Texture ) {
    ...
    texture::size_type TotalSize =
        sizeof( detail::FOURCC_KTX10 ) + sizeof( detail::ktx_header10 );

    for ( texture::size_type Level = 0, Levels = Texture.levels();
          Level < Levels; ++Level ) {
        TotalSize += sizeof( std::uint32_t );

        for ( texture::size_type Layer = 0, Layers = Texture.layers();
              Layer < Layers; ++Layer )
            for ( texture::size_type Face = 0, Faces = Texture.faces();
                  Face < Faces; ++Face ) {
                texture::size_type const FaceSize = Texture.size( Level );
                texture::size_type const PaddedSize = std::max(
                    BlockSize,
                    glm::ceilMultiple(
                        FaceSize, static_cast< texture::size_type >( 4 ) ) );
...
inline bool save_ktx( texture const& Texture, std::vector< char >& Memory ) {
    ...
    std::memcpy( &Memory[ 0 ], detail::FOURCC_KTX10,
                 sizeof( detail::FOURCC_KTX10 ) );

    std::size_t Offset = sizeof( detail::FOURCC_KTX10 );

    detail::ktx_header10& Header =
        *reinterpret_cast< detail::ktx_header10* >( &Memory[ 0 ] + Offset );
    ...
    Header.PixelHeight =
        !is_target_1d( Target )
            ? static_cast< std::uint32_t >( Texture.extent().y )
            : 0;
    Header.PixelDepth = Target == TARGET_3D
                            ? static_cast< std::uint32_t >( Texture.extent().z )
                            : 0;
    Header.NumberOfArrayElements =
        is_target_array( Target )
            ? static_cast< std::uint32_t >( Texture.layers() )
            : 0;
    Header.NumberOfFaces = is_target_cube( Target )
                               ? static_cast< std::uint32_t >( Texture.faces() )
                               : 1;
    Header.NumberOfMipmapLevels =
        static_cast< std::uint32_t >( Texture.levels() );
    ...
    for ( texture::size_type Level = 0, Levels = Texture.levels();
        ...
        std::uint32_t& ImageSize =
            *reinterpret_cast< std::uint32_t* >( &Memory[ 0 ] + Offset );
        ...
        for ( texture::size_type Layer = 0, Layers = Texture.layers();
            ...
            for ( texture::size_type Face = 0, Faces = Texture.faces();
                ...
                std::memcpy( &Memory[ 0 ] + Offset,
                             Texture.data( Layer, Face, Level ), FaceSize );

                texture::size_type const PaddedSize = glm::ceilMultiple(
                    FaceSize, static_cast< texture::size_type >( 4 ) );
        ...
        ImageSize =
            glm::ceilMultiple( ImageSize, static_cast< std::uint32_t >( 4 ) );
    }
