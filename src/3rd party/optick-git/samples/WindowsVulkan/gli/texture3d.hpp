...
public:
    ...
    /// Create a texture3d and allocate a new storage_linear with a complete
    /// mipmap chain
    texture3d( format_type Format,
               extent_type const& Extent,
               swizzles_type const& Swizzles = swizzles_type( SWIZZLE_RED,
    ...
    explicit texture3d( texture const& Texture );
    ...
    /// Create a texture3d view, reference a subset of an existing texture3d
    /// instance
    texture3d( texture3d const& Texture,
               size_type BaseLevel,
               size_type MaxLevel );

    /// Create a view of the image identified by Level in the mipmap chain of
    /// the texture
