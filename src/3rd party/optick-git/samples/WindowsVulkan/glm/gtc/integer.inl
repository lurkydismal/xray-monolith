...
struct compute_log2< L, T, P, vecType, false, Aligned > {
    GLM_FUNC_QUALIFIER static vecType< L, T, P > call(
        ...
        // Equivalent to return findMSB(vec); but save one function call in ASM
        // with VC return findMSB(vec);
        return vecType< L, T, P >(
            detail::compute_findMSB_vec< L, T, P, vecType,
                                         sizeof( T ) * 8 >::call( v ) );
    }
};
...
template < glm::length_t L,
           typename T,
           precision P,
           template < glm::length_t, typename, precision > class vecType >
