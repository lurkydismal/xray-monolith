...
GLM_FUNC_QUALIFIER float nextafterf( float x, float y ) {
    ...
    if ( ix == 0 ) { // x == 0
        GLM_SET_FLOAT_WORD( x,
                            ( hy & 0x80000000 ) | 1 ); // return +-minsubnormal
        t = x * x;
        if ( t == x )
            return t;
        else
...
GLM_FUNC_QUALIFIER vecType< L, T, P > next_float(
...
template < length_t L,
           ...
           precision P,
           template < length_t, typename, precision > class vecType >
GLM_FUNC_QUALIFIER vecType< L, T, P > prev_float(
    vecType< L, T, P > const& x ) {
    vecType< L, T, P > Result( uninitialize );
    for ( length_t i = 0, n = Result.length(); i < n; ++i )
...
template < length_t L,
           ...
           precision P,
           template < length_t, typename, precision > class vecType >
GLM_FUNC_QUALIFIER vecType< L, uint, P > float_distance(
    vecType< L, T, P > const& x,
