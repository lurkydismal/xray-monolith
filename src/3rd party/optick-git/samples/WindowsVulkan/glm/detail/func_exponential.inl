...
genType log2( genType Value ) {
    return std::log( Value ) *
           static_cast< genType >( 1.4426950408889634073599246810019 );
}
#endif
...
struct compute_inversesqrt< L, float, lowp, Aligned > {
    GLM_FUNC_QUALIFIER static vec< L, float, lowp > call(
        vec< L, float, lowp > const& x ) {
        vec< L, float, lowp > tmp( x );
        vec< L, float, lowp > xhalf( tmp * 0.5f );
        vec< L, uint, lowp >* p = reinterpret_cast< vec< L, uint, lowp >* >(
            const_cast< vec< L, float, lowp >* >( &x ) );
        vec< L, uint, lowp > i = vec< L, uint, lowp >( 0x5f375a86 ) -
                                 ( *p >> vec< L, uint, lowp >( 1 ) );
        vec< L, float, lowp >* ptmp =
            reinterpret_cast< vec< L, float, lowp >* >( &i );
        tmp = *ptmp;
...
GLM_FUNC_QUALIFIER vecType< L, T, P > pow(
    vecType< L, T, P > const& base,
    vecType< L, T, P > const& exponent ) {
...
GLM_FUNC_QUALIFIER genType exp2( genType x ) {
    ...
    return std::exp(
        static_cast< genType >( 0.69314718055994530941723212145818 ) * x );
}
...
GLM_FUNC_QUALIFIER vecType< L, T, P > log2( vecType< L, T, P > const& x ) {
    return detail::compute_log2< L, T, P, vecType,
...
GLM_FUNC_QUALIFIER vecType< L, T, P > sqrt( vecType< L, T, P > const& x ) {
    ...
    return detail::compute_sqrt< L, T, P,
                                 detail::is_aligned< P >::value >::call( x );
}
