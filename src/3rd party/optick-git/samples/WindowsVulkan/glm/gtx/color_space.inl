...
GLM_FUNC_QUALIFIER vec< 3, T, P > hsvColor( const vec< 3, T, P >& rgbColor ) {
    ...
    if ( Max != static_cast< T >( 0 ) ) {
        ...
        if ( rgbColor.r == Max )
            ...
            h = static_cast< T >( 0 ) +
                T( 60 ) * ( rgbColor.g - rgbColor.b ) / Delta;
        else if ( rgbColor.g == Max )
            // between cyan & yellow
            h = static_cast< T >( 120 ) +
                T( 60 ) * ( rgbColor.b - rgbColor.r ) / Delta;
        else
            // between magenta & cyan
            h = static_cast< T >( 240 ) +
                T( 60 ) * ( rgbColor.r - rgbColor.g ) / Delta;
...
GLM_FUNC_QUALIFIER mat< 4, 4, T, defaultp > saturation( T const s ) {
    vec< 3, T, defaultp > rgbw =
        vec< 3, T, defaultp >( T( 0.2126 ), T( 0.7152 ), T( 0.0722 ) );
...
template < typename T, precision P >
GLM_FUNC_QUALIFIER vec< 3, T, P > saturation( const T s,
                                              const vec< 3, T, P >& color ) {
    return vec< 3, T, P >( saturation( s ) * vec< 4, T, P >( color, T( 0 ) ) );
}

template < typename T, precision P >
GLM_FUNC_QUALIFIER vec< 4, T, P > saturation( const T s,
