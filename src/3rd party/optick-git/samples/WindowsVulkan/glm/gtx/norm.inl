...
GLM_FUNC_QUALIFIER T length2( vecType< L, T, P > const& v ) {
    GLM_STATIC_ASSERT( std::numeric_limits< T >::is_iec559,
                       "'length2' accepts only floating-point inputs" );
    return detail::compute_length2< vecType, L, T, P,
...
template < length_t L,
           ...
           precision P,
           template < length_t, typename, precision > class vecType >
GLM_FUNC_QUALIFIER T distance2( vecType< L, T, P > const& p0,
...
GLM_FUNC_QUALIFIER T l1Norm( vec< 3, T, P > const& a,
                             vec< 3, T, P > const& b ) {
...
template < typename T, precision P >
GLM_FUNC_QUALIFIER T l2Norm( vec< 3, T, P > const& a,
                             vec< 3, T, P > const& b ) {
    return length( b - a );
}
...
template < typename T, precision P >
GLM_FUNC_QUALIFIER T lxNorm( vec< 3, T, P > const& x,
                             vec< 3, T, P > const& y,
                             unsigned int Depth ) {
...
GLM_FUNC_QUALIFIER T lxNorm( vec< 3, T, P > const& v, unsigned int Depth ) {
    return pow( pow( v.x, T( Depth ) ) + pow( v.y, T( Depth ) ) +
                    pow( v.z, T( Depth ) ),
                T( 1 ) / T( Depth ) );
}
