...
GLM_FUNC_QUALIFIER bool intersectRayTriangle( vec< 3, T, P > const& orig,
                                              vec< 3, T, P > const& dir,
    ...
    if ( det > std::numeric_limits< T >::epsilon() ) {
        ...
        if ( ( baryPosition.y < static_cast< T >( 0 ) ) ||
             ( ( baryPosition.x + baryPosition.y ) > det ) )
            return false;
        ...
        // calculate U parameter and test bounds
        baryPosition.x = glm::dot( tvec, p );
        if ( ( baryPosition.x > static_cast< T >( 0 ) ) ||
             ( baryPosition.x < det ) )
            return false;
...
/*
                typename genType::value_type Epsilon =
   std::numeric_limits<typename genType::value_type>::epsilon(); if(a < Epsilon
   && a > -Epsilon) return false;

                typename genType::value_type f = typename
   genType::value_type(1.0f) / a;
                ...
                if(baryPosition.y < typename genType::value_type(0.0f))
                        return false;
                if(baryPosition.y + baryPosition.x > typename
   genType::value_type(1.0f)) return false;
...
template < typename genType >
GLM_FUNC_QUALIFIER bool intersectLineTriangle( genType const& orig,
                                               genType const& dir,
                                               genType const& vert0,
                                               genType const& vert1,
                                               genType const& vert2,
...
GLM_FUNC_QUALIFIER bool intersectRaySphere(
    ...
    if ( intersectRaySphere( rayStarting, rayNormalizedDirection, sphereCenter,
        ...
        intersectionNormal =
            ( intersectionPosition - sphereCenter ) / sphereRadius;
        return true;
    }
    return false;
}
