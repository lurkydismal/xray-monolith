...
struct VulkanDevice {
    ...
    VulkanDevice( VkPhysicalDevice physicalDevice ) {
        ...
        // device for later use Device properties also contain limits and sparse
        // properties
        ...
        vkGetPhysicalDeviceMemoryProperties( physicalDevice,
                                             &memoryProperties );
        // Queue family properties, used for setting up requested queues upon
        // device creation
        ...
        if ( extCount > 0 ) {
            ...
            if ( vkEnumerateDeviceExtensionProperties(
                     physicalDevice, nullptr, &extCount,
    ...
    VkResult createLogicalDevice(
        ...
        // implementations this can be a bit tricky, especially if the
        ...
        if ( requestedQueueTypes & VK_QUEUE_COMPUTE_BIT ) {
            ...
            if ( queueFamilyIndices.compute != queueFamilyIndices.graphics ) {
                // If compute family index differs, we need an additional queue
                // create info for the compute queue
                VkDeviceQueueCreateInfo queueInfo{};
                queueInfo.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
     ...
     */
    void copyBuffer( vks::Buffer* src,
                     vks::Buffer* dst,
                     VkQueue queue,
    ...
    VkCommandPool createCommandPool(
        ...
        VK_CHECK_RESULT( vkCreateCommandPool( logicalDevice, &cmdPoolInfo,
                                              nullptr, &cmdPool ) );
        return cmdPool;
    }
    ...
    VkCommandBuffer createCommandBuffer( VkCommandBufferLevel level,
        ...
        VkCommandBufferAllocateInfo cmdBufAllocateInfo =
            vks::initializers::commandBufferAllocateInfo( commandPool, level,
                                                          1 );

        VkCommandBuffer cmdBuffer;
        VK_CHECK_RESULT( vkAllocateCommandBuffers(
            logicalDevice, &cmdBufAllocateInfo, &cmdBuffer ) );
