...
private:
     ...
     * buffers containing camera matrices need to be updated */
...
public:
    ...
    /** @brief Product model and manufacturer of the Android device (via
     * android.Product*) */
...
#elif defined( VK_USE_PLATFORM_ANDROID_KHR )
    static int32_t handleAppInput( struct android_app* app,
                                   AInputEvent* event );
...
//
    ...
    /** @brief (Virtual) Called after a key was pressed, can be used to do
     * custom key handling */
    virtual void keyPressed( uint32_t );
    /** @brief (Virtual) Called after th mouse cursor moved and before internal
     * events (like camera rotation) is handled */
    virtual void mouseMoved( double x, double y, bool& handled );
    // Called when the window has been resized
    // Can be overriden in derived class to recreate or rebuild resources
    // attached to the frame buffer / swapchain
    virtual void windowResized();
    // Pure virtual function to be overriden by the dervice class
    // Called in case of an event where e.g. the framebuffer has to be rebuild
    // and thus all command buffers that may reference this
     ...
     * read, can be used to set features to enable on the device */
