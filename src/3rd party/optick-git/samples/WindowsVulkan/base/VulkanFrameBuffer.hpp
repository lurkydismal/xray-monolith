...
struct FramebufferAttachment {
    ...
    /**
     * @brief Returns true if the attachment is a depth and/or stencil
     * attachment
     */
...
/**
 * @brief Encapsulates a complete Vulkan framebuffer with an arbitrary number
 * and combination of attachments
 */
struct Framebuffer {
private:
...
public:
    ...
    /**
     * Destroy and free Vulkan resources used for the framebuffer and all of
     * it's attachments
     */
    ~Framebuffer() {
        assert( vulkanDevice );
        for ( auto attachment : attachments ) {
            vkDestroyImage( vulkanDevice->logicalDevice, attachment.image,
                            nullptr );
            vkDestroyImageView( vulkanDevice->logicalDevice, attachment.view,
                                nullptr );
            vkFreeMemory( vulkanDevice->logicalDevice, attachment.memory,
                          nullptr );
        }
        vkDestroySampler( vulkanDevice->logicalDevice, sampler, nullptr );
        vkDestroyRenderPass( vulkanDevice->logicalDevice, renderPass, nullptr );
        vkDestroyFramebuffer( vulkanDevice->logicalDevice, framebuffer,
                              nullptr );
    }

    /**
     * Add a new attachment described by createinfo to the framebuffer's
     * attachment list
     *
     * @param createinfo Structure that specifices the framebuffer to be
     * constructed
     *
    ...
    uint32_t addAttachment( vks::AttachmentCreateInfo createinfo ) {
        ...
        VK_CHECK_RESULT( vkCreateImage( vulkanDevice->logicalDevice, &image,
                                        nullptr, &attachment.image ) );
        vkGetImageMemoryRequirements( vulkanDevice->logicalDevice,
                                      attachment.image, &memReqs );
        memAlloc.allocationSize = memReqs.size;
        memAlloc.memoryTypeIndex = vulkanDevice->getMemoryType(
            memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT );
        VK_CHECK_RESULT( vkAllocateMemory( vulkanDevice->logicalDevice,
                                           &memAlloc, nullptr,
                                           &attachment.memory ) );
        VK_CHECK_RESULT( vkBindImageMemory( vulkanDevice->logicalDevice,
        ...
        imageView.viewType = ( createinfo.layerCount == 1 )
        ...
        attachment.description.stencilStoreOp =
            VK_ATTACHMENT_STORE_OP_DONT_CARE;
    ...
    /**
     * Creates a default sampler for sampling from any of the framebuffer
     * attachments Applications are free to create their own samplers for
     * different use cases
     *
     ...
     */
    VkResult createSampler( VkFilter magFilter,
                            VkFilter minFilter,
        ...
        return vkCreateSampler( vulkanDevice->logicalDevice, &samplerInfo,
                                nullptr, &sampler );
    }
    ...
    VkResult createRenderPass() {
        ...
        VK_CHECK_RESULT( vkCreateRenderPass( vulkanDevice->logicalDevice,
        ...
        VK_CHECK_RESULT( vkCreateFramebuffer( vulkanDevice->logicalDevice,
