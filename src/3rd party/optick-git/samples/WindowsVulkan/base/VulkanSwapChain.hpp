...
#define GET_INSTANCE_PROC_ADDR( inst, entrypoint )               \
    {                                                            \
        fp##entrypoint = reinterpret_cast< PFN_vk##entrypoint >( \
...
#define GET_DEVICE_PROC_ADDR( dev, entrypoint )                  \
    {                                                            \
        fp##entrypoint = reinterpret_cast< PFN_vk##entrypoint >( \
            vkGetDeviceProcAddr( dev, "vk" #entrypoint ) );      \
        if ( fp##entrypoint == NULL ) {                          \
            exit( 1 );                                           \
        }                                                        \
    }
...
public:
...
#endif
    {
        ...
        err = vkCreateWin32SurfaceKHR( instance, &surfaceCreateInfo, nullptr,
                                       &surface );
#elif defined( VK_USE_PLATFORM_ANDROID_KHR )
        ...
        err = vkCreateAndroidSurfaceKHR( instance, &surfaceCreateInfo, NULL,
                                         &surface );
#elif defined( VK_USE_PLATFORM_IOS_MVK )
        ...
        err = vkCreateIOSSurfaceMVK( instance, &surfaceCreateInfo, nullptr,
                                     &surface );
        ...
        surfaceCreateInfo.flags = 0;
        surfaceCreateInfo.pView = view;
        err = vkCreateMacOSSurfaceMVK( instance, &surfaceCreateInfo, NULL,
                                       &surface );
#elif defined( _DIRECT2DISPLAY )
        ...
        surfaceCreateInfo.window = window;
        err = vkCreateXcbSurfaceKHR( instance, &surfaceCreateInfo, nullptr,
                                     &surface );
#endif
        ...
        if ( graphicsQueueNodeIndex != presentQueueNodeIndex ) {
            vks::tools::exitFatal(
                "Separate graphics and presenting queues are not supported "
                "yet!",
                -1 );
        }
        ...
        VK_CHECK_RESULT( fpGetPhysicalDeviceSurfaceFormatsKHR(
            physicalDevice, surface, &formatCount, NULL ) );
    ...
    /**
     * Set instance, physical and logical device to use for the swapchain and
     * get all required function pointers
     *
     * @param instance Vulkan instance to use
     * @param physicalDevice Physical device used to query properties and
     * formats relevant to the swapchain
     * @param device Logical representation of the device to create the
     * swapchain for
     *
     */
     ...
     *
     * @param width Pointer to the width of the swapchain (may be adjusted to
    ...
    void create( uint32_t* width, uint32_t* height, bool vsync = false ) {
        ...
        // of the surface will be set by the swapchain
        ...
        swapchainCI.imageExtent = { swapchainExtent.width,
        ...
        VK_CHECK_RESULT( fpGetSwapchainImagesKHR(
            device, swapChain, &imageCount, images.data() ) );
        ...
        for ( uint32_t i = 0; i < imageCount; i++ ) {
            ...
            VK_CHECK_RESULT( vkCreateImageView( device, &colorAttachmentView,
     ...
     *
     * @param presentCompleteSemaphore (Optional) Semaphore that is signaled
     * when the image is ready for use
     * @param imageIndex Pointer to the image index that will be increased if
     * the next image could be acquired
     *
     * @note The function will always wait until the next image has been
     * acquired by setting timeout to UINT64_MAX
     *
    ...
    VkResult acquireNextImage( VkSemaphore presentCompleteSemaphore,
        ...
        // image has been acquired or an actual error is thrown With that we
        // don't have to handle VK_NOT_READY
        return fpAcquireNextImageKHR( device, swapChain, UINT64_MAX,
     ...
     * @param waitSemaphore (Optional) Semaphore that is waited on before the
    ...
    void createDirect2DisplaySurface( uint32_t width, uint32_t height ) {
        ...
        vkGetPhysicalDeviceDisplayPlanePropertiesKHR(
            physicalDevice, &planePropertyCount, NULL );
        ...
        if ( !foundMode ) {
            vks::tools::exitFatal( "Can't find a display and a display mode!",
                                   -1 );
            return;
        }
        ...
        vkGetDisplayPlaneCapabilitiesKHR( physicalDevice, displayMode,
                                          bestPlaneIndex, &planeCap );
        VkDisplayPlaneAlphaFlagBitsKHR alphaMode =
            ( VkDisplayPlaneAlphaFlagBitsKHR )0;
