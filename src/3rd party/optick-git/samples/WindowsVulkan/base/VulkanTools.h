    ...
    {                                                               \
        ...
        if ( res != VK_SUCCESS ) {                                  \
            LOGE( "Fatal : VkResult is \" %s \" in %s at line %d",  \
                  vks::tools::errorString( res ).c_str(), __FILE__, \
                  __LINE__ );                                       \
...
#if defined( __ANDROID__ )
VkShaderModule loadShader( AAssetManager* assetManager,
                           const char* fileName,
                           VkDevice device );
#else
...
// Note: GLSL support requires vendor-specific extensions to be enabled and is
// not a core-feature of Vulkan
