...
VkResult VulkanExampleBase::createInstance( bool enableValidation ) {
    ...
    if ( settings.validation ) {
        ...
        instanceCreateInfo.ppEnabledLayerNames =
            vks::debug::validationLayerNames;
    }
...
// iOS & macOS: VulkanExampleBase::getAssetPath() implemented externally to
// allow access to Objective-C components
...
VkCommandBuffer VulkanExampleBase::createCommandBuffer(
...
void VulkanExampleBase::renderFrame() {
    ...
    if ( fpsTimer > 1000.0f ) {
        lastFPS = static_cast< uint32_t >( ( float )frameCounter *
                                           ( 1000.0f / fpsTimer ) );
#if defined( _WIN32 )
        if ( !settings.overlay ) {
...
void VulkanExampleBase::renderLoop() {
    ...
    while ( 1 ) {
        ...
        if ( prepared ) {
            ...
            // TODO: Cap UI overlay update rates/only issue when update
            // requested
            updateOverlay();
            ...
            if ( camera.type != Camera::CameraType::firstperson ) {
                ...
                if ( std::abs( gamePadState.axisLeft.x ) > deadZone ) {
                    rotation.y +=
                        gamePadState.axisLeft.x * 0.5f * rotationSpeed;
                    camera.rotate( glm::vec3(
                        0.0f, gamePadState.axisLeft.x * 0.5f, 0.0f ) );
                    updateView = true;
                }
                if ( std::abs( gamePadState.axisLeft.y ) > deadZone ) {
                    rotation.x -=
                        gamePadState.axisLeft.y * 0.5f * rotationSpeed;
                    camera.rotate( glm::vec3( gamePadState.axisLeft.y * 0.5f,
                                              0.0f, 0.0f ) );
                    updateView = true;
                }
                // Zoom
                ...
                updateView = camera.updatePad(
                    gamePadState.axisLeft, gamePadState.axisRight, frameTimer );
                if ( updateView ) {
                    viewChanged();
                }
            }
    ...
    while ( !quit ) {
        ...
        if ( fpsTimer > 1000.0f ) {
            if ( !settings.overlay ) {
                std::string windowTitle = getWindowTitle();
                wl_shell_surface_set_title( shell_surface,
                                            windowTitle.c_str() );
            }
            lastFPS = ( float )frameCounter * ( 1000.0f / fpsTimer );
            fpsTimer = 0.0f;
...
void VulkanExampleBase::prepareFrame() {
    ...
    VkResult err = swapChain.acquireNextImage( semaphores.presentComplete,
                                               &currentBuffer );
...
void VulkanExampleBase::submitFrame() {
    OPTICK_EVENT();
    VkResult res = swapChain.queuePresent( queue, currentBuffer,
                                           semaphores.renderComplete );
    if ( !( ( res == VK_SUCCESS ) || ( res == VK_SUBOPTIMAL_KHR ) ) ) {
        if ( res == VK_ERROR_OUT_OF_DATE_KHR ) {
            // Swap chain is no longer compatible with the surface and needs to
            // be recreated
            windowResize();
            return;
        } else {
...
VulkanExampleBase::VulkanExampleBase( bool enableValidation ) {
    ...
    for ( size_t i = 0; i < args.size(); i++ ) {
        ...
        if ( ( args[ i ] == std::string( "-f" ) ) ||
             ( args[ i ] == std::string( "--fullscreen" ) ) ) {
            settings.fullscreen = true;
        }
        if ( ( args[ i ] == std::string( "-w" ) ) ||
             ( args[ i ] == std::string( "-width" ) ) ) {
            uint32_t w = strtol( args[ i + 1 ], &numConvPtr, 10 );
            if ( numConvPtr != args[ i + 1 ] ) {
                width = w;
            };
        }
        if ( ( args[ i ] == std::string( "-h" ) ) ||
             ( args[ i ] == std::string( "-height" ) ) ) {
        ...
        // Benchmark
        if ( ( args[ i ] == std::string( "-b" ) ) ||
             ( args[ i ] == std::string( "--benchmark" ) ) ) {
            benchmark.active = true;
            vks::tools::errorModeSilent = true;
        }
        // Warmup time (in seconds)
        if ( ( args[ i ] == std::string( "-bw" ) ) ||
             ( args[ i ] == std::string( "--benchwarmup" ) ) ) {
            if ( args.size() > i + 1 ) {
                uint32_t num = strtol( args[ i + 1 ], &numConvPtr, 10 );
                if ( numConvPtr != args[ i + 1 ] ) {
                    benchmark.warmup = num;
                } else {
                    std::cerr << "Warmup time for benchmark mode must be "
        ...
        if ( ( args[ i ] == std::string( "-br" ) ) ||
             ( args[ i ] == std::string( "--benchruntime" ) ) ) {
            if ( args.size() > i + 1 ) {
                uint32_t num = strtol( args[ i + 1 ], &numConvPtr, 10 );
                if ( numConvPtr != args[ i + 1 ] ) {
                    benchmark.duration = num;
                } else {
                    std::cerr << "Benchmark run duration must be specified as "
                                 "a number!"
                              << std::endl;
                }
            }
        ...
        if ( ( args[ i ] == std::string( "-bf" ) ) ||
             ( args[ i ] == std::string( "--benchfilename" ) ) ) {
            if ( args.size() > i + 1 ) {
                if ( args[ i + 1 ][ 0 ] == '-' ) {
                    std::cerr << "Filename for benchmark results must not "
                                 "start with a hyphen!"
                              << std::endl;
                } else {
        ...
        if ( ( args[ i ] == std::string( "-bt" ) ) ||
             ( args[ i ] == std::string( "--benchframetimes" ) ) ) {
...
bool VulkanExampleBase::initVulkan() {
    ...
    if ( settings.validation ) {
        ...
        // be displayed For validating (debugging) an appplication the error and
    ...
    for ( size_t i = 0; i < args.size(); i++ ) {
        // Select GPU
        if ( ( args[ i ] == std::string( "-g" ) ) ||
             ( args[ i ] == std::string( "-gpu" ) ) ) {
            char* endptr;
            uint32_t index = strtol( args[ i + 1 ], &endptr, 10 );
            if ( endptr != args[ i + 1 ] ) {
                if ( index > gpuCount - 1 ) {
                    std::cerr << "Selected device index " << index
                              << " is out of range, reverting to device 0 (use "
                                 "-listgpus to show available Vulkan devices)"
                              << std::endl;
                } else {
        ...
        if ( args[ i ] == std::string( "-listgpus" ) ) {
            ...
            if ( gpuCount == 0 ) {
                ...
                VK_CHECK_RESULT( vkEnumeratePhysicalDevices(
                    instance, &gpuCount, devices.data() ) );
                for ( uint32_t i = 0; i < gpuCount; i++ ) {
                    ...
                    std::cout << " Type: "
                              ...
                              << std::endl;
                    std::cout
                        << " API: " << ( deviceProperties.apiVersion >> 22 )
                        << "."
    ...
    OPTICK_GPU_INIT_VULKAN( &device, &physicalDevice, &queue,
                            &vulkanDevice->queueFamilyIndices.graphics, 1,
                            nullptr );
...
HWND VulkanExampleBase::setupWindow( HINSTANCE hinstance, WNDPROC wndproc ) {
    ...
    std::wstring className( name.begin(), name.end() );
    ...
    std::string windowTitle = getWindowTitle();
    window = CreateWindowExA( 0, name.c_str(), windowTitle.c_str(),
...
void VulkanExampleBase::handleMessages( HWND hWnd,
                                        UINT uMsg,
                                        WPARAM wParam,
                                        LPARAM lParam ) {
    switch ( uMsg ) {
        ...
        case WM_KEYUP:
            ...
            break;
        case WM_LBUTTONDOWN:
            mousePos = glm::vec2( ( float )LOWORD( lParam ),
                                  ( float )HIWORD( lParam ) );
        ...
        case WM_MOUSEWHEEL: {
            ...
            camera.translate( glm::vec3(
                0.0f, 0.0f, ( float )wheelDelta * 0.005f * zoomSpeed ) );
            viewUpdated = true;
            break;
        }
        case WM_MOUSEMOVE: {
        ...
        case WM_SIZE:
            if ( ( prepared ) && ( wParam != SIZE_MINIMIZED ) ) {
                if ( ( resizing ) || ( ( wParam == SIZE_MAXIMIZED ) ||
...
#elif defined( VK_USE_PLATFORM_ANDROID_KHR )
int32_t VulkanExampleBase::handleAppInput( struct android_app* app,
                                           AInputEvent* event ) {
    VulkanExampleBase* vulkanExample =
        reinterpret_cast< VulkanExampleBase* >( app->userData );
    if ( AInputEvent_getType( event ) == AINPUT_EVENT_TYPE_MOTION ) {
        int32_t eventSource = AInputEvent_getSource( event );
        switch ( eventSource ) {
            case AINPUT_SOURCE_JOYSTICK: {
                // Left thumbstick
                vulkanExample->gamePadState.axisLeft.x =
                    AMotionEvent_getAxisValue( event, AMOTION_EVENT_AXIS_X, 0 );
                vulkanExample->gamePadState.axisLeft.y =
                    AMotionEvent_getAxisValue( event, AMOTION_EVENT_AXIS_Y, 0 );
                // Right thumbstick
                vulkanExample->gamePadState.axisRight.x =
                    AMotionEvent_getAxisValue( event, AMOTION_EVENT_AXIS_Z, 0 );
                vulkanExample->gamePadState.axisRight.y =
                    AMotionEvent_getAxisValue( event, AMOTION_EVENT_AXIS_RZ,
                                               0 );
                break;
            }
            ...
            case AINPUT_SOURCE_TOUCHSCREEN: {
                ...
                switch ( action ) {
                    case AMOTION_EVENT_ACTION_UP: {
                        vulkanExample->lastTapTime =
                            AMotionEvent_getEventTime( event );
                        vulkanExample->touchPos.x =
                            AMotionEvent_getX( event, 0 );
                        vulkanExample->touchPos.y =
                        ...
                        if ( eventTime - downTime <=
                             vks::android::TAP_TIMEOUT ) {
                            float deadZone =
                                ( 160.f / vks::android::screenDensity ) *
                                vks::android::TAP_SLOP * vks::android::TAP_SLOP;
                    ...
                    case AMOTION_EVENT_ACTION_DOWN: {
                        // Detect double tap
                        int64_t eventTime = AMotionEvent_getEventTime( event );
                        if ( eventTime - vulkanExample->lastTapTime <=
                        ...
                        vulkanExample->touchPos.y =
                            AMotionEvent_getY( event, 0 );
                        vulkanExample->mousePos.x =
                            AMotionEvent_getX( event, 0 );
                        vulkanExample->mousePos.y =
                            AMotionEvent_getY( event, 0 );
                        break;
                    }
                    case AMOTION_EVENT_ACTION_MOVE: {
                        ...
                        if ( !handled ) {
                            ...
                            float deltaX =
                                ( float )( vulkanExample->touchPos.y -
                                           eventY ) *
                                vulkanExample->rotationSpeed * 0.5f;
                            float deltaY =
                                ( float )( vulkanExample->touchPos.x -
                                           eventX ) *
                                vulkanExample->rotationSpeed * 0.5f;

                            vulkanExample->camera.rotate(
                                glm::vec3( deltaX, 0.0f, 0.0f ) );
                            vulkanExample->camera.rotate(
                                glm::vec3( 0.0f, -deltaY, 0.0f ) );
...
void VulkanExampleBase::handleAppCommand( android_app* app, int32_t cmd ) {
    ...
    switch ( cmd ) {
        case APP_CMD_SAVE_STATE:
            ...
            *((struct saved_state*)vulkanExample->app->savedState) =
            vulkanExample->state; vulkanExample->app->savedStateSize =
...
/*static*/ void VulkanExampleBase::seatCapabilitiesCb( void* data,
                                                       wl_seat* seat,
                                                       uint32_t caps ) {
    VulkanExampleBase* self = reinterpret_cast< VulkanExampleBase* >( data );
...
void VulkanExampleBase::pointerAxis( wl_pointer* pointer,
                                     uint32_t time,
                                     uint32_t axis,
                                     wl_fixed_t value ) {
...
/*static*/ void VulkanExampleBase::keyboardKeymapCb(
    void* data,
...
/*static*/ void VulkanExampleBase::keyboardEnterCb(
    void* data,
    struct wl_keyboard* keyboard,
    uint32_t serial,
...
/*static*/ void VulkanExampleBase::keyboardLeaveCb(
    void* data,
    struct wl_keyboard* keyboard,
    uint32_t serial,
    struct wl_surface* surface ) {}
...
void VulkanExampleBase::seatCapabilities( wl_seat* seat, uint32_t caps ) {
    if ( ( caps & WL_SEAT_CAPABILITY_POINTER ) && !pointer ) {
        ...
        static const struct wl_pointer_listener pointer_listener = {
            pointerEnterCb,  pointerLeaveCb, pointerMotionCb,
            pointerButtonCb, pointerAxisCb,
        };
    ...
    if ( ( caps & WL_SEAT_CAPABILITY_KEYBOARD ) && !keyboard ) {
        ...
        static const struct wl_keyboard_listener keyboard_listener = {
            keyboardKeymapCb, keyboardEnterCb,     keyboardLeaveCb,
            keyboardKeyCb,    keyboardModifiersCb,
        };
...
void VulkanExampleBase::registryGlobal( wl_registry* registry,
                                        uint32_t name,
                                        const char* interface,
                                        uint32_t version ) {
...
/*static*/ void VulkanExampleBase::registryGlobalRemoveCb(
    void* data,
...
static void PingCb( void* data,
                    struct wl_shell_surface* shell_surface,
                    uint32_t serial ) {
    wl_shell_surface_pong( shell_surface, serial );
}

static void ConfigureCb( void* data,
                         struct wl_shell_surface* shell_surface,
                         uint32_t edges,
                         int32_t width,
                         int32_t height ) {}
...
static inline xcb_intern_atom_reply_t* intern_atom_helper(
...
void VulkanExampleBase::handleEvent( const xcb_generic_event_t* event ) {
    switch ( event->response_type & 0x7f ) {
        ...
        case XCB_MOTION_NOTIFY: {
            ...
            handleMouseMove( ( int32_t )motion->event_x,
                             ( int32_t )motion->event_y );
            break;
        } break;
        ...
        case XCB_CONFIGURE_NOTIFY: {
            ...
            if ( ( prepared ) && ( ( cfgEvent->width != width ) ||
                                   ( cfgEvent->height != height ) ) ) {
...
void VulkanExampleBase::setupDepthStencil() {
    ...
    mem_alloc.memoryTypeIndex = vulkanDevice->getMemoryType(
