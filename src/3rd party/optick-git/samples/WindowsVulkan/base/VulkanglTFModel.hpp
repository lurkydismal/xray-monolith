...
struct Texture {
    ...
    /*
            Load a texture from a glTF image (stored as vector of chars loaded
       via stb_image) Also generates the mip chain as glTF images are stored as
       jpg or png without any mips
    */
    void fromglTfImage( tinygltf::Image& gltfimage,
        ...
        mipLevels = static_cast< uint32_t >(
            floor( log2( std::max( width, height ) ) ) + 1.0 );
        ...
        VK_CHECK_RESULT( vkCreateImageView( device->logicalDevice, &viewInfo,
                                            nullptr, &view ) );
...
struct Mesh {
    ...
    Mesh( vks::VulkanDevice* device, glm::mat4 matrix ) {
        ...
        uniformBuffer.descriptor = { uniformBuffer.buffer, 0,
                                     sizeof( uniformBlock ) };
    };
...
struct Model {
        ...
        if ( node.children.size() > 0 ) {
            for ( auto i = 0; i < node.children.size(); i++ ) {
                loadNode( newNode, model.nodes[ node.children[ i ] ],
                          node.children[ i ], model, indexBuffer, vertexBuffer,
                          globalscale );
            }
        }
        ...
        if ( node.mesh > -1 ) {
            ...
            for ( size_t j = 0; j < mesh.primitives.size(); j++ ) {
                ...
                // Vertices
                {
                    ...
                    if ( primitive.attributes.find( "NORMAL" ) !=
                        ...
                        const tinygltf::Accessor& normAccessor =
                            model.accessors[ primitive.attributes
                                                 .find( "NORMAL" )
                                                 ->second ];
                    ...
                    for ( size_t v = 0; v < posAccessor.count; v++ ) {
                        Vertex vert{};
                        vert.pos = glm::vec4(
                            glm::make_vec3( &bufferPos[ v * 3 ] ), 1.0f );
                        ...
                        vert.uv =
                            bufferTexCoords
                                ? glm::make_vec2( &bufferTexCoords[ v * 2 ] )
                                : glm::vec3( 0.0f );

                        vert.joint0 = hasSkin ? glm::vec4( glm::make_vec4(
                                                    &bufferJoints[ v * 4 ] ) )
                                              : glm::vec4( 0.0f );
                        vert.weight0 =
                            hasSkin ? glm::make_vec4( &bufferWeights[ v * 4 ] )
                                    : glm::vec4( 0.0f );
                ...
                Primitive* newPrimitive = new Primitive(
                    indexStart, indexCount, materials[ primitive.material ] );
        ...
        for ( tinygltf::Material& mat : gltfModel.materials ) {
            ...
            if ( mat.values.find( "baseColorTexture" ) != mat.values.end() ) {
                material.baseColorTexture =
                    &textures[ gltfModel
                                   .textures[ mat.values[ "baseColorTexture" ]
                                                  .TextureIndex() ]
                                   .source ];
            }
            // Metallic roughness workflow
            if ( mat.values.find( "metallicRoughnessTexture" ) !=
                 mat.values.end() ) {
            ...
            if ( mat.values.find( "roughnessFactor" ) != mat.values.end() ) {
                material.roughnessFactor = static_cast< float >(
                    mat.values[ "roughnessFactor" ].Factor() );
            }
            if ( mat.values.find( "metallicFactor" ) != mat.values.end() ) {
                material.metallicFactor = static_cast< float >(
                    mat.values[ "metallicFactor" ].Factor() );
            }
            ...
            if ( mat.extPBRValues.size() > 0 ) {
                if ( mat.extPBRValues.find( "specularGlossinessTexture" ) !=
                     mat.extPBRValues.end() ) {
                    material.specularGlossinessTexture =
                        &textures
                            [ gltfModel
                                  .textures[ mat.extPBRValues
                                                 [ "specularGlossinessTexture" ]
                                                     .TextureIndex() ]
                                  .source ];
                }
                if ( mat.extPBRValues.find( "diffuseTexture" ) !=
                     mat.extPBRValues.end() ) {
                ...
                // static_cast<float>(mat.values["glossinessFactor"].Factor());
                // }
                // if (mat.values.find("specularFactor") != mat.values.end()) {
                //	// TODO: Rename or explicit param
                //	material.metallicFactor =
                // static_cast<float>(mat.values["specularFactor"].Factor());
                // }
            }
    ...
    void loadFromFile( std::string filename,
        ...
        AAsset_close( asset );
        std::string baseDir;
        bool fileLoaded = gltfContext.LoadASCIIFromString(
            &gltfModel, &error, fileData, size, baseDir );
        free( fileData );
#else
        bool fileLoaded = gltfContext.LoadASCIIFromFile( &gltfModel, &error,
                                                         filename.c_str() );
#endif
        std::vector< uint32_t > indexBuffer;
        std::vector< Vertex > vertexBuffer;
        ...
        VkDescriptorPoolCreateInfo descriptorPoolCI =
            vks::initializers::descriptorPoolCreateInfo(
                poolSizes.size(), poolSizes.data(), uboCount );
        VK_CHECK_RESULT( vkCreateDescriptorPool( device->logicalDevice,
        ...
        VK_CHECK_RESULT( vkCreateDescriptorSetLayout(
            device->logicalDevice, &descriptorLayoutCI, nullptr,
            &descriptorSetLayout ) );
        for ( auto node : nodes ) {
    ...
    void getNodeDimensions( Node* node, glm::vec3& min, glm::vec3& max ) {
        if ( node->mesh ) {
            for ( Primitive* primitive : node->mesh->primitives ) {
                glm::vec4 locMin =
                    glm::vec4( primitive->dimensions.min, 1.0f ) *
                    node->getMatrix();
                glm::vec4 locMax =
                    glm::vec4( primitive->dimensions.max, 1.0f ) *
                    node->getMatrix();
                if ( locMin.x < min.x ) {
                    min.x = locMin.x;
                }
    ...
    void getSceneDimensions() {
        ...
        dimensions.radius =
            glm::distance( dimensions.min, dimensions.max ) / 2.0f;
    }
        ...
        for ( auto& channel : animation.channels ) {
            ...
            for ( auto i = 0; i < sampler.inputs.size() - 1; i++ ) {
                if ( ( time >= sampler.inputs[ i ] ) &&
                    ...
                    if ( u <= 1.0f ) {
                        switch ( channel.path ) {
                            case vkglTF::AnimationChannel::PathType::
                                TRANSLATION: {
                                glm::vec4 trans =
    ...
    */
    ...
    void prepareNodeDescriptor( vkglTF::Node* node,
                                VkDescriptorSetLayout descriptorSetLayout ) {
        if ( node->mesh ) {
            ...
            VK_CHECK_RESULT( vkAllocateDescriptorSets(
                device->logicalDevice, &descriptorSetAllocInfo,
            ...
            vkUpdateDescriptorSets( device->logicalDevice, 1,
