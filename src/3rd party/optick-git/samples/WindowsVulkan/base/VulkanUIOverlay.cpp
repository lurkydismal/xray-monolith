...
void UIOverlay::prepareResources() {
    ...
    VK_CHECK_RESULT( vkCreateImage( device->logicalDevice, &imageInfo, nullptr,
                                    &fontImage ) );
    ...
    VK_CHECK_RESULT( vkAllocateDescriptorSets( device->logicalDevice,
 ...
 * main application */
void UIOverlay::preparePipeline( const VkPipelineCache pipelineCache,
...
bool UIOverlay::inputFloat( const char* caption,
                            float* value,
                            float step,
                            uint32_t precision ) {
    bool res =
        ImGui::InputFloat( caption, value, step, step * 10.0f, precision );
    if ( res ) {
        updated = true;
    };
    return res;
...
bool UIOverlay::sliderFloat( const char* caption,
                             float* value,
                             float min,
                             float max ) {
...
bool UIOverlay::sliderInt( const char* caption,
                           int32_t* value,
                           int32_t min,
                           int32_t max ) {
...
bool UIOverlay::comboBox( const char* caption,
                          int32_t* itemindex,
    ...
    bool res = ImGui::Combo( caption, itemindex, &charitems[ 0 ], itemCount,
                             itemCount );
    if ( res ) {
        updated = true;
    };
