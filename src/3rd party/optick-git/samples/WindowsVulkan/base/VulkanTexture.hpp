...
public:
     ...
     * @param copyQueue Queue used for the texture staging copy commands (must
     * support transfer)
     ...
     * @param (Optional) forceLinear Force linear tiling (not advised, defaults
     * to false)
     *
     */
    void loadFromFile(
        ...
        if ( !asset ) {
            vks::tools::exitFatal(
                "Could not load texture from " + filename +
                    "\n\nThe file may be part of the additional asset "
                    "pack.\n\nRun \"download_assets.py\" in the repository "
                    "root to download the latest version.",
                -1 );
        }
        ...
        // limited amount of formats and features (mip maps, cubemaps, arrays,
        // etc.)
        ...
        if ( useStaging ) {
            // Create a host-visible staging buffer that contains the raw image
            // data
            ...
            VK_CHECK_RESULT( vkCreateBuffer( device->logicalDevice,
            ...
            VK_CHECK_RESULT( vkAllocateMemory( device->logicalDevice,
                                               &memAllocInfo, nullptr,
                                               &stagingMemory ) );
            VK_CHECK_RESULT( vkBindBufferMemory(
                device->logicalDevice, stagingBuffer, stagingMemory, 0 ) );

            // Copy texture data into staging buffer
            uint8_t* data;
            VK_CHECK_RESULT( vkMapMemory( device->logicalDevice, stagingMemory,
                                          0, memReqs.size, 0,
                                          ( void** )&data ) );
            ...
            if ( !( imageCreateInfo.usage &
                    VK_IMAGE_USAGE_TRANSFER_DST_BIT ) ) {
                imageCreateInfo.usage |= VK_IMAGE_USAGE_TRANSFER_DST_BIT;
            }
            VK_CHECK_RESULT( vkCreateImage(
                device->logicalDevice, &imageCreateInfo, nullptr, &image ) );
            ...
            VK_CHECK_RESULT( vkAllocateMemory( device->logicalDevice,
                                               &memAllocInfo, nullptr,
                                               &deviceMemory ) );
            VK_CHECK_RESULT( vkBindImageMemory( device->logicalDevice, image,
                                                deviceMemory, 0 ) );
            ...
            // depending on implementation (e.g. no mip maps, only one layer,
            // etc.)
            ...
            VK_CHECK_RESULT( vkCreateImage( device->logicalDevice,
            ...
            VK_CHECK_RESULT( vkBindImageMemory(
                device->logicalDevice, mappableImage, mappableMemory, 0 ) );
            ...
            vkGetImageSubresourceLayout( device->logicalDevice, mappableImage,
                                         &subRes, &subResLayout );

            // Map image memory
            VK_CHECK_RESULT( vkMapMemory( device->logicalDevice, mappableMemory,
                                          0, memReqs.size, 0, &data ) );

            // Copy image data into memory
            memcpy( data, tex2D[ subRes.mipLevel ].data(),
                    tex2D[ subRes.mipLevel ].size() );
            ...
            vks::tools::setImageLayout(
                copyCmd, image, VK_IMAGE_ASPECT_COLOR_BIT,
        ...
        VK_CHECK_RESULT( vkCreateSampler(
            device->logicalDevice, &samplerCreateInfo, nullptr, &sampler ) );
        ...
        viewCreateInfo.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0,
                                            1 };
        // Linear tiling usually won't support mip maps
        // Only set mip map count if optimal tiling is used
        viewCreateInfo.subresourceRange.levelCount =
            ( useStaging ) ? mipLevels : 1;
        viewCreateInfo.image = image;
        VK_CHECK_RESULT( vkCreateImageView( device->logicalDevice,
                                            &viewCreateInfo, nullptr, &view ) );
...
public:
    ...
    void loadFromFile(
        ...
        gli::texture2d_array tex2DArray(
            gli::load( ( const char* )textureData, size ) );
        ...
        VK_CHECK_RESULT( vkMapMemory( device->logicalDevice, stagingMemory, 0,
                                      memReqs.size, 0, ( void** )&data ) );
        memcpy( data, tex2DArray.data(),
                static_cast< size_t >( tex2DArray.size() ) );
        vkUnmapMemory( device->logicalDevice, stagingMemory );

        // Setup buffer copy regions for each layer including all of it's
        // miplevels
        std::vector< VkBufferImageCopy > bufferCopyRegions;
        size_t offset = 0;
        ...
        for ( uint32_t layer = 0; layer < layerCount; layer++ ) {
            for ( uint32_t level = 0; level < mipLevels; level++ ) {
                ...
                bufferCopyRegion.imageSubresource.layerCount = 1;
                bufferCopyRegion.imageExtent.width = static_cast< uint32_t >(
                    tex2DArray[ layer ][ level ].extent().x );
                bufferCopyRegion.imageExtent.height = static_cast< uint32_t >(
                    tex2DArray[ layer ][ level ].extent().y );
                bufferCopyRegion.imageExtent.depth = 1;
                bufferCopyRegion.bufferOffset = offset;
        ...
        // Image barrier for optimal image (target)
        // Set initial layout for all array layers (faces) of the optimal
        // (target) tiled texture
        VkImageSubresourceRange subresourceRange = {};
        subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        subresourceRange.baseMipLevel = 0;
        subresourceRange.levelCount = mipLevels;
...
public:
    ...
    void loadFromFile(
        ...
        // Setup buffer copy regions for each face including all of it's
        // miplevels
        std::vector< VkBufferImageCopy > bufferCopyRegions;
        size_t offset = 0;
        ...
        for ( uint32_t face = 0; face < 6; face++ ) {
            for ( uint32_t level = 0; level < mipLevels; level++ ) {
                ...
                bufferCopyRegion.imageSubresource.layerCount = 1;
                bufferCopyRegion.imageExtent.width = static_cast< uint32_t >(
                    texCube[ face ][ level ].extent().x );
                bufferCopyRegion.imageExtent.height = static_cast< uint32_t >(
                    texCube[ face ][ level ].extent().y );
                bufferCopyRegion.imageExtent.depth = 1;
                bufferCopyRegion.bufferOffset = offset;
