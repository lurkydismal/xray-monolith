        ...
        for ( auto& component : components ) {
            switch ( component ) {
                ...
                case VERTEX_COMPONENT_DUMMY_VEC4:
                    ...
                    break;
                default:
                    // All components except the ones listed above are made up
                    // of 3 floats
                    res += 3 * sizeof( float );
            }
        }
...
struct Model {
     ...
     *
     * @param device Pointer to the Vulkan device used to generated the vertex
     * and index buffers on
     * @param filename File to load (must be a model format supported by ASSIMP)
     * @param layout Vertex layout components (position, normals, tangents,
     * etc.)
    ...
    bool loadFromFile( const std::string& filename,
        ...
        if ( pScene ) {
            ...
            // Use staging buffer to move vertex and index buffer to device
            // local memory Create staging buffers
            ...
            vkDestroyBuffer( device->logicalDevice, vertexStaging.buffer,
                             nullptr );
            vkFreeMemory( device->logicalDevice, vertexStaging.memory,
                          nullptr );
            vkDestroyBuffer( device->logicalDevice, indexStaging.buffer,
                             nullptr );
