...
public:
    ...
    void run( std::function< void() > renderFunc,
        ...
        // Benchmark phase
        {
            ...
            std::cout << "device : " << deviceProps.deviceName
    ...
    void saveResults() {
        ...
        if ( result.is_open() ) {
            ...
            result << "device,driverversion,duration (ms),frames,fps"
                   << std::endl;
            result << deviceProps.deviceName << "," << deviceProps.driverVersion
                   << "," << runtime << "," << frameCount << ","
            ...
            if ( outputFrameTimes ) {
                ...
                double tAvg = std::accumulate( frameTimes.begin(),
                                               frameTimes.end(), 0.0 ) /
                              ( double )frameTimes.size();
                std::cout << "best   : " << ( 1000.0 / tMin ) << " fps ("
                          << tMin << " ms)" << std::endl;
                std::cout << "worst  : " << ( 1000.0 / tMax ) << " fps ("
                          << tMax << " ms)" << std::endl;
                std::cout << "avg    : " << ( 1000.0 / tAvg ) << " fps ("
                          << tAvg << " ms)" << std::endl;
                std::cout << std::endl;
            }
