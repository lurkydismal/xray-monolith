...
bool OnOptickStateChanged( Optick::State::Type state ) {
    ...
    if ( state == Optick::State::DUMP_CAPTURE ) {
        ...
        // Attach screenshot
        Optick::AttachFile( Optick::File::OPTICK_IMAGE, "Screenshot.bmp",
                            g_ScreenshotRequest.c_str() );
...
public:
    ...
    // Secondary scene command buffers used to store backgdrop and user
    // interface
    ...
    ~VulkanExample() {
        ...
        for ( auto& thread : threadData ) {
            vkFreeCommandBuffers( device, thread.commandPool,
    ...
    void prepareMultiThreadedRenderer() {
        ...
        VK_CHECK_RESULT(
            vkAllocateCommandBuffers( device, &cmdBufAllocateInfo,
                                      &secondaryCommandBuffers.background ) );
        VK_CHECK_RESULT( vkAllocateCommandBuffers(
            device, &cmdBufAllocateInfo, &secondaryCommandBuffers.ui ) );

        threadData.resize( numThreads );

        float maxX =
            std::floor( std::sqrt( numThreads * numObjectsPerThread ) );
        uint32_t posX = 0;
        uint32_t posZ = 0;
        ...
        for ( uint32_t i = 0; i < numThreads; i++ ) {
            ...
            // One secondary command buffer per object that is updated by this
            // thread
            thread->commandBuffer.resize( numObjectsPerThread );
            // Generate secondary command buffers for each thread
            VkCommandBufferAllocateInfo secondaryCmdBufAllocateInfo =
                vks::initializers::commandBufferAllocateInfo(
                    thread->commandPool, VK_COMMAND_BUFFER_LEVEL_SECONDARY,
                    thread->commandBuffer.size() );
            VK_CHECK_RESULT(
                vkAllocateCommandBuffers( device, &secondaryCmdBufAllocateInfo,
                                          thread->commandBuffer.data() ) );
            ...
            for ( uint32_t j = 0; j < numObjectsPerThread; j++ ) {
                ...
                thread->objectData[ j ].pos =
                    glm::vec3( sin( phi ) * cos( theta ), 0.0f, cos( phi ) ) *
                    35.0f;

                thread->objectData[ j ].rotation =
                    glm::vec3( 0.0f, rnd( 360.0f ), 0.0f );
                thread->objectData[ j ].deltaT = rnd( 1.0f );
                thread->objectData[ j ].rotationDir =
                    ( rnd( 100.0f ) < 50.0f ) ? 1.0f : -1.0f;
                thread->objectData[ j ].rotationSpeed =
                    ( 2.0f + rnd( 4.0f ) ) *
                    thread->objectData[ j ].rotationDir;
                thread->objectData[ j ].scale = 0.75f + rnd( 0.5f );

                thread->pushConstBlock[ j ].color =
                    glm::vec3( rnd( 1.0f ), rnd( 1.0f ), rnd( 1.0f ) );
            }
        }
    }
    ...
    void threadRenderCode( uint32_t threadIndex,
        ...
        objectData->visible =
            frustum.checkSphere( objectData->pos, objectSphereDim * 0.5f );
            ...
            vkBeginCommandBuffer( cmdBuffer, &commandBufferBeginInfo ) );
        {
            ...
            if ( !paused ) {
                ...
                objectData->rotation.y +=
                    2.5f * objectData->rotationSpeed * frameTimer;
                ...
                objectData->pos.y =
                    sin( glm::radians( objectData->deltaT * 360.0f ) ) * 2.5f;
            }

            {
                OPTICK_EVENT( "UpdateMVP" );
                objectData->model =
                    glm::translate( glm::mat4( 1.0f ), objectData->pos );
                objectData->model = glm::rotate(
                    objectData->model,
                    -sinf( glm::radians( objectData->deltaT * 360.0f ) ) *
                        0.25f,
                    glm::vec3( objectData->rotationDir, 0.0f, 0.0f ) );
                objectData->model = glm::rotate(
                    objectData->model, glm::radians( objectData->rotation.y ),
                    glm::vec3( 0.0f, objectData->rotationDir, 0.0f ) );
                objectData->model = glm::rotate(
                    objectData->model,
                    glm::radians( objectData->deltaT * 360.0f ),
                    glm::vec3( 0.0f, objectData->rotationDir, 0.0f ) );
                objectData->model = glm::scale(
                    objectData->model, glm::vec3( objectData->scale ) );

                thread->pushConstBlock[ cmdBufferIndex ].mvp =
                    matrices.projection * matrices.view * objectData->model;
            }
            ...
            VkDeviceSize offsets[ 1 ] = { 0 };
            {
                ...
                vkCmdBindVertexBuffers( cmdBuffer, 0, 1,
                                        &models.ufo.vertices.buffer, offsets );
            }
            {
                OPTICK_EVENT( "vkCmdBindIndexBuffer" );
                vkCmdBindIndexBuffer( cmdBuffer, models.ufo.indices.buffer, 0,
                                      VK_INDEX_TYPE_UINT32 );
            }
            {
                OPTICK_EVENT( "vkCmdDrawIndexed" );
                vkCmdDrawIndexed( cmdBuffer, models.ufo.indexCount, 1, 0, 0,
                                  0 );
            }
        }
    ...
    void updateSecondaryCommandBuffers(
        ...
        */
        ...
        VK_CHECK_RESULT( vkBeginCommandBuffer(
            secondaryCommandBuffers.background, &commandBufferBeginInfo ) );
        ...
        view = glm::rotate( view, glm::radians( rotation.x ),
                            glm::vec3( 1.0f, 0.0f, 0.0f ) );
        view = glm::rotate( view, glm::radians( rotation.y ),
                            glm::vec3( 0.0f, 1.0f, 0.0f ) );
        view = glm::rotate( view, glm::radians( rotation.z ),
                            glm::vec3( 0.0f, 0.0f, 1.0f ) );

        glm::mat4 mvp = matrices.projection * view;

        vkCmdPushConstants( secondaryCommandBuffers.background, pipelineLayout,
                            VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof( mvp ),
                            &mvp );

        VkDeviceSize offsets[ 1 ] = { 0 };
        vkCmdBindVertexBuffers( secondaryCommandBuffers.background, 0, 1,
                                &models.skysphere.vertices.buffer, offsets );
        vkCmdBindIndexBuffer( secondaryCommandBuffers.background,
                              models.skysphere.indices.buffer, 0,
                              VK_INDEX_TYPE_UINT32 );
        vkCmdDrawIndexed( secondaryCommandBuffers.background,
                          models.skysphere.indexCount, 1, 0, 0, 0 );

        VK_CHECK_RESULT(
            vkEndCommandBuffer( secondaryCommandBuffers.background ) );

        /*
                User interface

                With VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS, the primary
           command buffer's content has to be defined by secondary command
           buffers, which also applies to the UI overlay command buffer
        */

        VK_CHECK_RESULT( vkBeginCommandBuffer( secondaryCommandBuffers.ui,
                                               &commandBufferBeginInfo ) );

        vkCmdSetViewport( secondaryCommandBuffers.ui, 0, 1, &viewport );
        vkCmdSetScissor( secondaryCommandBuffers.ui, 0, 1, &scissor );

        vkCmdBindPipeline( secondaryCommandBuffers.ui,
                           VK_PIPELINE_BIND_POINT_GRAPHICS,
                           pipelines.starsphere );
    ...
    void updateCommandBuffers( VkFramebuffer frameBuffer ) {
        ...
        vkCmdBeginRenderPass( primaryCommandBuffer, &renderPassBeginInfo,
        ...
        // Execute render commands from the secondary command buffer
        {
            ...
            vkCmdExecuteCommands( primaryCommandBuffer, commandBuffers.size(),
                                  commandBuffers.data() );
        }
    ...
    void loadAssets() {
        models.ufo.loadFromFile( getAssetPath() + "models/retroufo_red.dae",
                                 vertexLayout, 0.12f, vulkanDevice, queue );
        models.skysphere.loadFromFile( getAssetPath() + "models/sphere.obj",
                                       vertexLayout, 1.0f, vulkanDevice,
                                       queue );
        objectSphereDim =
            std::max( std::max( models.ufo.dim.size.x, models.ufo.dim.size.y ),
                      models.ufo.dim.size.z );
    }
    ...
    void setupPipelineLayout() {
        ...
        VkPushConstantRange pushConstantRange =
            vks::initializers::pushConstantRange(
                VK_SHADER_STAGE_VERTEX_BIT, sizeof( ThreadPushConstantBlock ),
                0 );
        ...
        VK_CHECK_RESULT( vkCreatePipelineLayout(
            device, &pPipelineLayoutCreateInfo, nullptr, &pipelineLayout ) );
    }
    ...
    void preparePipelines() {
        ...
        const std::vector< VkVertexInputAttributeDescription >
            vertexInputAttributes = {
                ...
                vks::initializers::vertexInputAttributeDescription(
                    ...
                    sizeof( float ) * 6 ), // Location 2: Color
            };
        ...
        shaderStages[ 0 ] = loadShader(
            getAssetPath() + "shaders/multithreading/phong.vert.spv",
            VK_SHADER_STAGE_VERTEX_BIT );
        shaderStages[ 1 ] = loadShader(
            getAssetPath() + "shaders/multithreading/phong.frag.spv",
        ...
        shaderStages[ 0 ] = loadShader(
            getAssetPath() + "shaders/multithreading/starsphere.vert.spv",
            VK_SHADER_STAGE_VERTEX_BIT );
        shaderStages[ 1 ] = loadShader(
            getAssetPath() + "shaders/multithreading/starsphere.frag.spv",
            VK_SHADER_STAGE_FRAGMENT_BIT );
        VK_CHECK_RESULT( vkCreateGraphicsPipelines( device, pipelineCache, 1,
                                                    &pipelineCI, nullptr,
                                                    &pipelines.starsphere ) );
    ...
    void updateMatrices() {
        matrices.projection =
            glm::perspective( glm::radians( 60.0f ),
                              ( float )width / ( float )height, 0.1f, 256.0f );
        matrices.view =
            glm::translate( glm::mat4( 1.0f ), glm::vec3( 0.0f, 0.0f, zoom ) );
        matrices.view = glm::rotate( matrices.view, glm::radians( rotation.x ),
                                     glm::vec3( 1.0f, 0.0f, 0.0f ) );
        matrices.view = glm::rotate( matrices.view, glm::radians( rotation.y ),
                                     glm::vec3( 0.0f, 1.0f, 0.0f ) );
        matrices.view = glm::rotate( matrices.view, glm::radians( rotation.z ),
                                     glm::vec3( 0.0f, 0.0f, 1.0f ) );
    ...
    void draw() {
        ...
        do {
            ...
            fenceRes =
                vkWaitForFences( device, 1, &renderFence, VK_TRUE, 100000000 );
    ...
    // szPathName : Specifies the pathname        -> the file path to save the
    // image lpBits    : Specifies the bitmap bits      -> the buffer (content
    // of the) image w    : Specifies the image width h    : Specifies the image
    // height
    bool saveBitmap( std::ofstream& stream, void* lpBits, int w, int h ) {
        ...
        // Write the bitmap file header               // Saving the first header
        // to file
        stream.write( ( const char* )&bmfh, sizeof( BITMAPFILEHEADER ) );

        // And then the bitmap info header            // Saving the second
        // header to file
        stream.write( ( const char* )&BMIH, sizeof( BITMAPINFOHEADER ) );

        // Finally, write the image data itself
        //-- the data represents our drawing          // Saving the file content
        // in lpBits to file
    ...
    // This is done using a blit from the swapchain image to a linear image
    // whose memory content is then saved as a ppm image Getting the image date
    // directly from a swapchain image wouldn't work as they're usually stored
    // in an implementation dependant optimal tiling format Note: This requires
    // the swapchain images to be created with the
    ...
    void saveScreenshot( const wchar_t* filename ) {
        ...
        // Check if the device supports blitting from optimal images (the
        // swapchain images are in optimal format)
        vkGetPhysicalDeviceFormatProperties(
            physicalDevice, swapChain.colorFormat, &formatProps );
        ...
        if ( !( formatProps.linearTilingFeatures &
            ...
            std::cerr << "Device does not support blitting to linear tiled "
                         "images, using copy instead of blit!"
                      << std::endl;
            supportsBlit = false;
        }
        ...
        // Note that vkCmdBlitImage (if supported) will also do format
        // conversions if the swapchain color format would differ
        ...
        if ( supportsBlit ) {
            // Define the region to blit (we will blit the whole swapchain
            // image)
            VkOffset3D blitSize;
            blitSize.x = width;
            blitSize.y = height;
            ...
            vkCmdCopyImage( copyCmd, srcImage,
                            VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstImage,
                            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                            &imageCopyRegion );
        }
        ...
        if ( !supportsBlit ) {
            ...
            isBGR = ( std::find( formatsBGR.begin(), formatsBGR.end(),
