...
// Newcomers, read 'Programmer guide' below for notes on how to setup Dear ImGui
// in your codebase. Get latest version at https://github.com/ocornut/imgui
// Releases change-log at https://github.com/ocornut/imgui/releases
// Technical Support for Getting Started
// https://discourse.dearimgui.org/c/getting-started Gallery (please post your
// screenshots/video there!): https://github.com/ocornut/imgui/issues/1269
// Developed by Omar Cornut and every direct or indirect contributors to the
// GitHub. This library is free but I need your support to sustain development
// and maintenance. If you work for a company, please consider financial
// support, see README. For individuals: https://www.patreon.com/imgui
...
- FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
  - How can I tell whether to dispatch mouse/keyboard to imgui or to my
application?
  - How can I display an image? What is ImTextureID, how does it works?
  - How can I have multiple widgets with the same label or without a label? A
primer on labels and the ID Stack.
...
ImDrawList API)
  ...
  - I integrated Dear ImGui in my engine and some elements are clipping or
...
/*
 ...
 - Efficient runtime and memory consumption (NB- we do allocate when "growing"
 content e.g. creating a window, opening a tree node for the first time, etc.
 but a typical frame should not allocate anything)
 ...
 be highly dynamic, there are no construction or destruction steps, less data
 retention on your side, less state duplication, less state synchronization,
 less bugs.
 - Call and read ImGui::ShowDemoWindow() for demo code demonstrating most
 features.
 - You can learn about immediate-mode gui principles at
 http://www.johno.se/book/imgui.html or watch http://mollyrocket.com/861
 ...
 - Add the Dear ImGui source files to your projects or using your preferred
 build system. It is recommended you build the .cpp files as part of your
 project and not as a library.
 - You can later customize the imconfig.h file to tweak some compilation time
 behavior, such as integrating imgui types with your own maths types.
 ...
 All rendering informatioe are stored into command-lists that you will retrieve
 after calling ImGui::Render().
 - Refer to the bindings and demo applications in the examples/ folder for
 instruction on how to setup your code.

 THIS IS HOW A SIMPLE APPLICATION MAY LOOK LIKE
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the
 examples/ folder)
 ...
 ImGuiConfigFlags_NavEnableKeyboard' to enable keyboard controls
     ...
     // Initialize helper Platform and Renderer bindings (here we are using
 imgui_impl_win32 and imgui_impl_dx11) ImGui_ImplWin32_Init(hwnd);
 ...
 == void*. Read FAQ below for details about ImTextureID. MyTexture* texture =
 ...
 write to those fields from your Windows message handlers, etc.
        // (In the examples/ app this is usually done within the
 ImGui_ImplXXX_NewFrame() function from one of the demo Platform bindings)
 ...
 mainloop to be able to use imgui everywhere) ImGui::NewFrame();
 ...
 able to use imgui in your own game rendering code) ImGui::EndFrame();
        ImGui::Render();
 ...
 buffer generated by ImGui const ImDrawIdx* idx_buffer =
 cmd_list->IdxBuffer.Data;   // index buffer generated by ImGui for (int cmd_i =
 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
 ...
 pcmd->TextureId.
                 // The vast majority of draw calls with use the imgui texture
 atlas, which value you have set yourself during initialization.
 ...
 fix that once everywhere else works!
 ...
 applications in the future, always subtract draw_data->DisplayPos from
 ...
 (int)(pcmd->ClipRect.w - pos.y));
                 ...
                 // By default the indices ImDrawIdx are 16-bits, you can change
 them to 32-bits if your engine doesn't support 16-bits indices.
 ...
 - The examples/ folders contains many functional implementation of the
 pseudo-code above.
 - When calling NewFrame(), the 'io.WantCaptureMouse', 'io.WantCaptureKeyboard'
 and 'io.WantTextInput' flags are updated. They tell you if ImGui intends to use
 your inputs. When a flag is set you want to hide the corresponding inputs from
 the rest of your application. In both cases you need to pass on the inputs to
 imgui. Read the FAQ below for more information about those flags.
 - Please read the FAQ above. Amusingly, it is called a FAQ because people
 frequently have the same issues!
 ...
 the mouse cursor to be moved. When that happens your back-end NEEDS to move the
 OS or underlying mouse cursor on the next frame. Some of the binding in
 ...
 beta flag in favor of a global io.ConfigResizeWindowsFromEdges to enable the
 feature.
 ...
 and only honored by some binding ahead of merging the Nav branch).
 ...
 now accept negative mouse coordinates. In your binding if you need to support
 ...
 obsolete. prefer using the more explicit ImGuiOnceUponAFrame.
 ...
 - 2017/08/13 (1.51) - renamed ImGuiCol_Columns*** to ImGuiCol_Separator***.
 Kept redirection enums (will obsolete).
 - 2017/08/11 (1.51) - renamed ImGuiSetCond_*** types and flags to
 ImGuiCond_***. Kept redirection enums (will obsolete).
 ...
 default theme it will not affect you. If your TitleBg/TitleBgActive alpha was
 <1.0f you need to tweak your custom theme to readjust for the fact that we
 don't draw a WindowBg background behind the title bar. This helper function
 will convert an old TitleBg/TitleBgActive color into a new one with the same
 visual output, given the OLD color and the OLD WindowBg color. ImVec4
 ConvertTitleBgCol(const ImVec4& win_bg_col, const ImVec4& title_bg_col)
                           {
                               float new_a = 1.0f - ((1.0f - win_bg_col.w) *
 (1.0f - title_bg_col.w)), k = title_bg_col.w / new_a; return
 ImVec4((win_bg_col.x * win_bg_col.w + title_bg_col.x) * k, (win_bg_col.y *
 win_bg_col.w + title_bg_col.y) * k, (win_bg_col.z * win_bg_col.w +
 title_bg_col.z) * k, new_a);
                           }
 ...
 cmd_lists_count) new: ImGui_XXXX_RenderDrawLists(ImDrawData* draw_data).
                         argument:   'cmd_lists' becomes 'draw_data->CmdLists',
 ...
 multiple fonts. no need for a PNG loader. (1.30) - removed
 GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve
 uncompressed pixels. font init:  const void* png_data; unsigned int png_size;
 ImGui::GetDefaultFontData(NULL, NULL, &png_data, &png_size); <..Upload texture
 to GPU..> became:     unsigned char* pixels; int width, height;
 io.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height); <..Upload texture to
 GPU>; io.Fonts->TexId = YourTextureIdentifier; you now more flexibility to load
 multiple TTF fonts and manage the texture buffer for internal needs. it is now
 recommended that you sample the font texture with bilinear interpolation.
 ...
 in user projection matrix) (1.30) - removed ImGui::IsItemFocused() in favor of
 ...
 Q: How can I tell whether to dispatch mouse/keyboard to imgui or to my
 application? A: You can read the 'io.WantCaptureMouse',
 'io.WantCaptureKeyboard' and 'io.WantTextInput' flags from the ImGuiIO
 structure (e.g. if (ImGui::GetIO().WantCaptureMouse) { ... } )
    - When 'io.WantCaptureMouse' is set, imgui wants to use your mouse state,
 and you may want to discard/hide the inputs from the rest of your application.
    - When 'io.WantCaptureKeyboard' is set, imgui wants to use your keyboard
 state, and you may want to discard/hide the inputs from the rest of your
 application.
    - When 'io.WantTextInput' is set to may want to notify your OS to popup an
 on-screen keyboard, if available (e.g. on a mobile phone, or console OS). Note:
 you should always pass your mouse/keyboard inputs to imgui, even when the
 io.WantCaptureXXX flag are set false. This is because imgui needs to detect
 that you clicked in the void to unfocus its windows. Note: The
 'io.WantCaptureMouse' is more accurate that any attempt to "check if the mouse
 is hovering a window" (don't do that!). It handle mouse dragging correctly
 (both dragging that started over your application or over an imgui window) and
 handle e.g. modal windows blocking inputs. Those flags are updated by
 ImGui::NewFrame(). Preferably read the flags after calling NewFrame() if you
 can afford it, but reading them before is also perfectly fine, as the bool
 toggle fairly rarely. If you have on a touch device, you might find use for an
 early call to UpdateHoveredWindowAndCaptureFlags(). Note: Text input widget
 releases focus on "Return KeyDown", so the subsequent "Return KeyUp" event that
 your application receive will typically have 'io.WantCaptureKeyboard=false'.
 Depending on your application logic it may or not be inconvenient. You might
 want to track which key-downs were targeted for Dear ImGui, e.g. with an array
 of bool, and filter out the corresponding key-ups.)

 Q: How can I display an image? What is ImTextureID, how does it works?
 A: Short explanation:
    - You may use functions such as ImGui::Image(), ImGui::ImageButton() or
 lower-level ImDrawList::AddImage() to emit draw calls that will use your own
 textures.
    - Actual textures are identified in a way that is up to the user/engine.
    - Loading image files from the disk and turning them into a texture is not
 within the scope of Dear ImGui (for a good reason). Please read documentations
 or tutorials on your graphics API to understand how to display textures on the
 screen before moving onward.

    Long explanation:
    - Dear ImGui's job is to create "meshes", defined in a renderer-agnostic
 format made of draw commands and vertices. At the end of the frame those meshes
 (ImDrawList) will be displayed by your rendering function. They are made up of
 textured polygons and the code to render them is generally fairly short (a few
 dozen lines). In the examples/ folder we provide functions for popular graphics
 API (OpenGL, DirectX, etc.).
    - Each rendering function decides on a data type to represent "textures".
 The concept of what is a "texture" is entirely tied to your underlying
 engine/graphics API. We carry the information to identify a "texture" in the
 ImTextureID type. ImTextureID is nothing more that a void*, aka 4/8 bytes worth
 of data: just enough to store 1 pointer or 1 integer of your choice. Dear ImGui
 doesn't know or understand what you are storing in ImTextureID, it merely pass
 ImTextureID values until they reach your rendering function.
    - In the examples/ bindings, for each graphics API binding we decided on a
 type that is likely to be a good representation for specifying an image from
 the end-user perspective. This is what the _examples_ rendering functions are
 using:

         OpenGL:     ImTextureID = GLuint                       (see
 ImGui_ImplGlfwGL3_RenderDrawData() function in imgui_impl_glfw_gl3.cpp)
         DirectX9:   ImTextureID = LPDIRECT3DTEXTURE9           (see
 ImGui_ImplDX9_RenderDrawData()     function in imgui_impl_dx9.cpp) DirectX11:
 ImTextureID = ID3D11ShaderResourceView*    (see ImGui_ImplDX11_RenderDrawData()
 function in imgui_impl_dx11.cpp) DirectX12:  ImTextureID =
 D3D12_GPU_DESCRIPTOR_HANDLE  (see ImGui_ImplDX12_RenderDrawData()    function
 in imgui_impl_dx12.cpp)

      For example, in the OpenGL example binding we store raw OpenGL texture
 identifier (GLuint) inside ImTextureID. Whereas in the DirectX11 example
 binding we store a pointer to ID3D11ShaderResourceView inside ImTextureID,
 which is a higher-level structure tying together both the texture and
 information about its format and how to read it.
    - If you have a custom engine built over e.g. OpenGL, instead of passing
 GLuint around you may decide to use a high-level data type to carry information
 about the texture as well as how to display it (shaders, etc.). The decision of
 what to use as ImTextureID can always be made better knowing how your codebase
      is designed. If your engine has high-level data types for "textures" and
 "material" then you may want to use them. If you are starting with OpenGL or
 DirectX or Vulkan and haven't built much of a rendering engine over them,
 keeping the default ImTextureID representation suggested by the example
 bindings is probably the best choice. (Advanced users may also decide to keep a
 low-level type in ImTextureID, and use ImDrawList callback and pass information
 to their renderer)
    ...
    The renderer function called after ImGui::Render() will receive that same
 value that the user code passed:

        // Cast ImTextureID / void* stored in the draw command as our texture
 type MyTexture* texture = (MyTexture*)pcmd->TextureId;
        MyEngineBindTexture2D(texture);

    Once you understand this design you will understand that loading image files
 and turning them into displayable textures is not within the scope of Dear
 ImGui. This is by design and is actually a good thing, because it means your
 code has full control over your data types and how you display them. If you
 want to display an image file (e.g. PNG file) into the screen, please refer to
 documentation and tutorials for the graphics API you are using.

    Here's a simplified OpenGL example using stb_image.h:

        // Use stb_image.h to load a PNG from disk and turn it into raw RGBA
 pixel data: #define STB_IMAGE_IMPLEMENTATION #include <stb_image.h>
        [...]
        int my_image_width, my_image_height;
        unsigned char* my_image_data = stbi_load("my_image.png",
 &my_image_width, &my_image_height, NULL, 4);
 ...
 GL_RGBA, GL_UNSIGNED_BYTE, image_data);
        ...
        // Now that we have an OpenGL texture, assuming our imgui rendering
 function (imgui_impl_xxx.cpp file) takes GLuint as ImTextureID, we can display
 it: ImGui::Image((void*)(intptr_t)my_opengl_texture, ImVec2(my_image_width,
 my_image_height));

    C/C++ tip: a void* is pointer-sized storage. You may safely store any
 pointer or integer into it by casting your value to ImTexture / void*, and
 vice-versa. Because both end-points (user code and rendering function) are
 under your control, you know exactly what is stored inside the ImTexture /
 void*. Examples:

        GLuint my_tex = XXX;
        void* my_void_ptr;
        my_void_ptr = (void*)(intptr_t)my_tex;                  // cast a GLuint
 into a void* (we don't take its address! we literally store the value inside
 the pointer) my_tex = (GLuint)(intptr_t)my_void_ptr;                 // cast a
 void* into a GLuint

        ID3D11ShaderResourceView* my_dx11_srv = XXX;
        void* my_void_ptr;
        my_void_ptr = (void*)my_dx11_srv;                       // cast a
 ID3D11ShaderResourceView* into an opaque void* my_dx11_srv =
 (ID3D11ShaderResourceView*)my_void_ptr;   // cast a void* into a
 ID3D11ShaderResourceView*

    Finally, you may call ImGui::ShowMetricsWindow() to
 explore/visualize/understand how the ImDrawList are generated.

 Q: How can I have multiple widgets with the same label or without a label?
 Q: I have multiple widgets with the same label, and only the first one works.
 Why is that? A: A primer on labels and the ID Stack...

    Dear ImGui internally need to uniquely identify UI elements.
    Elements that are typically not clickable (such as calls to the Text
 functions) don't need an ID. Interactive widgets (such as calls to Button
 buttons) need a unique ID. Unique ID are used internally to track active
 widgets and occasionally associate state to widgets. Unique ID are implicitly
 built from the hash of multiple elements that identify the "path" to the UI
 element.
   ...
   - ID are uniquely scoped within windows, tree nodes, etc. which all pushes to
 the ID stack. Having two buttons labeled "OK" in different windows or different
 tree locations is fine. We used "..." above to signify whatever was already
 pushed to the ID stack previously:

       Begin("MyWindow");
       Button("OK");          // Label = "OK",     ID = hash of ("MyWindow",
 "OK") End();

   - If you have a same ID twice in the same location, you'll have a conflict:

       Button("OK");
       Button("OK");          // ID collision! Interacting with either button
 will trigger the first one.

     Fear not! this is easy to solve and there are many ways to solve it!

   - Solving ID conflict in a simple/local context:
     When passing a label you can optionally specify extra ID information within
 string itself. Use "##" to pass a complement to the ID that won't be visible to
 the end-user. This helps solving the simple collision cases when you know e.g.
 at compilation time which items are going to be created:

       Begin("MyWindow");
       Button("Play");        // Label = "Play",   ID = hash of ("MyWindow",
 "Play") Button("Play##foo1");  // Label = "Play",   ID = hash of ("MyWindow",
 "Play##foo1")  // Different from above Button("Play##foo2");  // Label =
 "Play",   ID = hash of ("MyWindow", "Play##foo2")  // Different from above
       End();
 ...
 constant ID. This allows you to animate labels. For example you may want to
 include varying information in a window title bar, but windows are uniquely
 identified by their ID. Use "###" to pass a label that isn't part of ID:

       Button("Hello###ID";   // Label = "Hello",  ID = hash of (..., "ID")
       Button("World###ID";   // Label = "World",  ID = hash of (..., "ID") //
 Same as above, even though the label looks different

       sprintf(buf, "My game (%f FPS)###MyGame", fps);
       Begin(buf);            // Variable label,   ID = hash of "MyGame"

   - Solving ID conflict in a more general manner:
     Use PushID() / PopID() to create scopes and manipulate the ID stack, as to
 avoid ID conflicts within the same window. This is the most convenient way of
 distinguishing ID when iterating and creating many UI elements
 programmatically. You can push a pointer, a string or an integer value into the
 ID stack. Remember that ID are formed from the concatenation of _everything_ in
 the ID stack!
       ...
       for (int i = 0; i < 100; i++)
       {
         ...
         Button("Click");   // Label = "Click",  ID = Hash of ("Window", i,
 "Click") PopID();
       }
       for (int i = 0; i < 100; i++)
       {
         ...
         PushID(obj);
         Button("Click");   // Label = "Click",  ID = Hash of ("Window", obj
 pointer, "Click") PopID();
       }
       for (int i = 0; i < 100; i++)
       {
         MyObject* obj = Objects[i];
         PushID(obj->Name);
         Button("Click");   // Label = "Click",  ID = Hash of ("Window",
 obj->Name, "Click") PopID();
       }
       End();
 ...
 stack:
       ...
       PushID("node");
       Button("Click");     // Label = "Click",  ID = hash of (..., "node",
 "Click") PushID(my_ptr); Button("Click"); // Label = "Click",  ID = hash of
 (..., "node", my_ptr, "Click") PopID(); PopID();
 ...
 each tree node. Depending on your use cases you may want to use strings,
 indices or pointers as ID. e.g. when following a single pointer that may change
 over time, using a static string as ID will preserve your node open/closed
 state when the targeted object change. e.g. when displaying a list of objects,
 using indices or pointers as ID will preserve the node open/closed state
 differently. See what makes more sense in your situation!

 Q: How can I use my own math types instead of ImVec2/ImVec4?
 A: You can edit imconfig.h and setup the
 IM_VEC2_CLASS_EXTRA/IM_VEC4_CLASS_EXTRA macros to add implicit type
 conversions. This way you'll be able to use your own types everywhere, e.g.
 passsing glm::vec2 to ImGui functions instead of ImVec2.
 ...
 A: Use the font atlas to load the TTF/OTF file you want:
    ...
    (default is ProggyClean.ttf, rendered at size 13, embedded in dear imgui's
 source code)

    New programmers: remember that in C/C++ and most programming languages if
 you want to use a backslash \ within a string literal, you need to write it
 double backslash "\\":
      io.Fonts->AddFontFromFileTTF("MyDataFolder\MyFontFile.ttf",
 size_in_pixels);   // WRONG (you are escape the M here!)
      io.Fonts->AddFontFromFileTTF("MyDataFolder\\MyFontFile.ttf",
 size_in_pixels);  // CORRECT
      io.Fonts->AddFontFromFileTTF("MyDataFolder/MyFontFile.ttf",
 ...
 strings. Read 'How can I load multiple fonts?' and the file
 'misc/fonts/README.txt' for instructions and useful header files.
 ...
 size_in_pixels); ImFont* font2 =
 io.Fonts->AddFontFromFileTTF("myfontfile2.ttf", size_in_pixels);
      io.Fonts->GetTexDataAsRGBA32() or GetTexDataAsAlpha8()
 ...
 ranges); // Merge icon font io.Fonts->LoadFromFileTTF("myfontfile.ttf",
 size_pixels, NULL, &config, io.Fonts->GetGlyphRangesJapanese()); // Merge
 japanese glyphs

 Q: How can I display and input non-Latin characters such as Chinese, Japanese,
 Korean, Cyrillic? A: When loading a font, pass custom Unicode ranges to specify
 the glyphs to load.
 ...
 entire game script and only build the characters the game need)
      ...
      builder.AddText("Hello world");                        // Add a string
 (here "Hello world" contains 7 unique characters) builder.AddChar(0x7262); //
 Add a specific character builder.AddRanges(io.Fonts->GetGlyphRangesJapanese());
 // Add one of the default ranges builder.BuildRanges(&ranges); // Build the
 final result (ordered ranges with all the unique characters submitted)
      io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels, NULL,
 ranges.Data);

    All your strings needs to use UTF-8 encoding. In C++11 you can encode a
 string literal in UTF-8 by using the u8"hello" syntax. Specifying literal in
 your source code using a local code page (such as CP-923 for Japanese or
 CP-1251 for Cyrillic) will NOT work! Otherwise you can convert yourself to
 UTF-8 or load text data from file already saved as UTF-8.

    Text input: it is up to your application to pass the right character code by
 calling io.AddInputCharacter(). The applications in examples/ are doing that.
    Windows: you can use the WM_CHAR or WM_UNICHAR or WM_IME_CHAR message
 (depending if your app is built using Unicode or MultiByte mode). You may also
 use MultiByteToWideChar() or ToUnicode() to retrieve Unicode codepoints from
 MultiByte characters or keyboard state. Windows: if your language is relying on
 an Input Method Editor (IME), you copy the HWND of your window to
 io.ImeWindowHandle in order for the default implementation of
 io.ImeSetInputScreenPosFn() to set your Microsoft IME position correctly.

 Q: How can I use the drawing facilities without an ImGui window? (using
 ImDrawList API) A: - You can create a dummy window. Call
 SetNextWindowBgAlpha(0.0f), call Begin() with
 NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flags. Then you
 can retrieve the ImDrawList* via GetWindowDrawList() and draw to it in any way
 you like.
    - You can call ImGui::GetOverlayDrawList() and use this draw list to display
 contents over every other imgui windows.
    - You can create your own ImDrawList instance. You'll need to initialize
 them ImGui::GetDrawListSharedData(), or create your own ImDrawListSharedData.

 Q: I integrated Dear ImGui in my engine and the text or lines are blurry..
 A: In your Render function, try translating your projection matrix by
 (0.5f,0.5f) or (0.375f,0.375f). Also make sure your orthographic projection
 matrix and io.DisplaySize matches your actual framebuffer dimension.

 Q: I integrated Dear ImGui in my engine and some elements are clipping or
 disappearing when I move windows around.. A: You are probably mishandling the
 clipping rectangles in your render function. Rectangles provided by ImGui are
 defined as (x1=left,y1=top,x2=right,y2=bottom) and NOT as (x1,y1,width,height).

 Q: How can I help?
 A: - If you are experienced with Dear ImGui and C++, look at the github issues,
 or docs/TODO.txt and see how you want/can help!
    - Convince your company to fund development time! Individual users: you can
 also become a Patron (patreon.com/imgui) or donate on PayPal! See README.
 ...
 gallery threads (github.com/ocornut/imgui/issues/1269). Visuals are ideal as
 ...
 - tip: the ImGuiOnceUponAFrame helper will allow run the block of code only
 once a frame. You can use it to quickly add custom UI in the middle of a deep
 nested inner loop in your code.
 - tip: you can call Render() multiple times (e.g for VR renders).
 - tip: call and read the ShowDemoWindow() code in imgui_demo.cpp for more
...
#pragma clang diagnostic ignored \
    "-Wunknown-pragmas" // warning : unknown warning group '-Wformat-pedantic *'
                        // // not all warnings are known by all clang versions..
                        // so ignoring warnings triggers new warnings on some
                        // configuration. great!
#pragma clang diagnostic ignored \
...
#if __GNUC__ >= 8
#pragma GCC diagnostic ignored \
    "-Wclass-memaccess" // warning: 'memset/memcpy' clearing/writing an object
                        // of type 'xxxx' with no trivial copy-assignment; use
...
static void AddWindowToDrawData( ImVector< ImDrawList* >* out_list,
                                 ImGuiWindow* window );
static void AddWindowToSortedBuffer(
    ImVector< ImGuiWindow* >* out_sorted_windows,
    ImGuiWindow* window );
...
static void NavProcessItem( ImGuiWindow* window,
                            const ImRect& nav_bb,
                            const ImGuiID id );
...
static void UpdateManualResize( ImGuiWindow* window,
...
// Current context pointer. Implicitly used by all ImGui functions. Always
// assumed to be != NULL. CreateContext() will automatically set this pointer if
// it is NULL. Change to a different context by calling
// ImGui::SetCurrentContext(). If you use DLL hotreloading you might need to
// call SetCurrentContext() after reloading code from this file. ImGui functions
// are not thread-safe because of this pointer. If you want thread-safety to
// allow N threads to access N different contexts, you can:
// - Change this variable to use thread local storage. You may #define GImGui in
// imconfig.h for that purpose. Future development aim to make this context
...
static void ( *GImAllocatorFreeFunc )( void* ptr,
...
ImGuiStyle::ImGuiStyle() {
    ...
    WindowRounding = 7.0f; // Radius of window corners rounding. Set to 0.0f to
    ...
    ItemInnerSpacing = ImVec2(
        4, 4 ); // Horizontal and vertical spacing between within elements of a
    ...
    DisplayWindowPadding = ImVec2(
        20,
        20 ); // Window position are clamped to be visible within the display
    ...
    AntiAliasedLines = true; // Enable anti-aliasing on lines/borders. Disable
                             // if you are really short on CPU/GPU.
    AntiAliasedFill = true;  // Enable anti-aliasing on filled shapes (rounded
...
void ImGuiIO::AddInputCharactersUTF8( const char* utf8_chars ) {
    // We can't pass more wchars than ImGuiIO::InputCharacters[] can hold so
    // don't convert more
    const int wchars_buf_len =
        sizeof( ImGuiIO::InputCharacters ) / sizeof( ImWchar );
...
bool ImTriangleContainsPoint( const ImVec2& a,
                              ...
                              const ImVec2& c,
                              const ImVec2& p ) {
    bool b1 = ( ( p.x - b.x ) * ( a.y - b.y ) -
                ( p.y - b.y ) * ( a.x - b.x ) ) < 0.0f;
    bool b2 = ( ( p.x - c.x ) * ( b.y - c.y ) -
                ( p.y - c.y ) * ( b.x - c.x ) ) < 0.0f;
    bool b3 = ( ( p.x - a.x ) * ( c.y - a.y ) -
                ( p.y - a.y ) * ( c.x - a.x ) ) < 0.0f;
    return ( ( b1 == b2 ) && ( b2 == b3 ) );
}
...
ImU32 ImHash( const void* data, int data_size, ImU32 seed ) {
    ...
    if ( data_size > 0 ) {
        ...
        while ( unsigned char c = *current++ ) {
            // We support a syntax of "label###id" where only "###id" is
            // included in the hash, and only "label" gets displayed. Because
            // this syntax is rarely used we are optimizing for the common case.
            // - If we reach ### in the string we discard the hash so far and
            // reset to the seed.
            // - We don't do 'current += 2; continue;' after handling ### to
...
FILE* ImFileOpen( const char* filename, const char* mode ) {
    ...
    // filenames. Converting both strings from UTF-8 to wchar format (using a
    // single allocation, because we can)
...
void* ImFileLoadToMemory( const char* filename,
    ...
    if ( fseek( f, 0, SEEK_END ) || ( file_size_signed = ftell( f ) ) == -1 ||
         fseek( f, 0, SEEK_SET ) ) {
        fclose( f );
        return NULL;
    }
    ...
    while ( buf_out < buf_end - 1 &&
        ...
        if ( c < 0x80 )
            *buf_out++ = ( char )c;
            ...
            buf_out += ImTextCharToUtf8( buf_out,
...
ImU32 ImGui::GetColorU32( ImU32 col ) {
    ...
    a = ( ImU32 )( a * style_alpha ); // We don't need to clamp 0..255 because
...
static ImVector< ImGuiStorage::Pair >::iterator LowerBound(
...
void ImGuiStorage::BuildSortByKey() {
    struct StaticFunc {
        static int IMGUI_CDECL PairCompareByID( const void* lhs,
                                                const void* rhs ) {
            // We can't just do a subtraction because qsort uses signed integers
            // and subtracting our ID doesn't play well with that.
...
int ImGuiStorage::GetInt( ImGuiID key, int default_val ) const {
    ImVector< Pair >::iterator it = LowerBound(
        const_cast< ImVector< ImGuiStorage::Pair >& >( Data ), key );
...
void ImGuiTextBuffer::appendfv( const char* fmt, va_list args ) {
    ...
    if ( write_off + len >= Buf.Capacity ) {
        ...
        Buf.reserve( needed_sz > double_capacity ? needed_sz
                                                 : double_capacity );
    }
...
//-----------------------------------------------------------------------------
// [SECTION] ImGuiListClipper
// This is currently not as flexible/powerful as it should be, needs some rework
// (see TODO)
//-----------------------------------------------------------------------------
...
static void SetCursorPosYAndSetupDummyPrevLine( float pos_y,
    ...
    window->DC.CursorPosPrevLine.y =
        ...
        line_height; // Setting those fields so that SetScrollHere() can
                     // properly function after the end of our clipper usage.
    window->DC.PrevLineSize.y =
        ( line_height -
          GImGui->Style.ItemSpacing
...
// again from Sync() in StepNo 1 Use case B: Begin() called from constructor
// with items_height>0
...
void ImGuiListClipper::End() {
    ...
    // + DisplayEnd * ItemsHeight, but it feels saner to just seek at the end
    // and not assert/crash the user.
    if ( ItemsCount < INT_MAX )
        SetCursorPosYAndSetupDummyPrevLine(
            StartPosY + ItemsCount * ItemsHeight,
            ItemsHeight ); // advance cursor
    ItemsCount = -1;
    StepNo = 3;
}
...
bool ImGuiListClipper::Step() {
    ...
    if ( StepNo == 0 ) // Step 0: the clipper let you process the first element,
                       // regardless of it being visible or not, so we can
                       ...
                       // position the cursor before the first element.
    {
        ...
        IM_ASSERT( items_height > 0.0f ); // If this triggers, it means Item 0
                                          // hasn't moved the cursor vertically
    ...
    if ( StepNo ==
         2 ) // Step 2: dummy step only required if an explicit items_height was
             // passed to constructor or Begin() and user still call Step().
    ...
    if ( StepNo == 3 ) // Step 3: the clipper validate that we have reached the
                       // expected Y position (corresponding to element
                       // DisplayEnd), advance the cursor to the end of the list
                       // and then returns 'false' to end the loop.
        End();
    return false;
}
...
//-----------------------------------------------------------------------------
// [SECTION] RENDER HELPERS
// Those (internal) functions are currently quite a legacy mess - their
// signature and behavior will change. Also see imgui_draw.cpp for some more
// which have been reworked to not rely on ImGui:: state.
//-----------------------------------------------------------------------------
...
void ImGui::RenderArrow( ImVec2 p_min, ImGuiDir dir, float scale ) {
    ...
    g.CurrentWindow->DrawList->AddTriangleFilled(
        center + a, center + b, center + c, GetColorU32( ImGuiCol_Text ) );
}

void ImGui::RenderBullet( ImVec2 pos ) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    window->DrawList->AddCircleFilled( pos, g.FontSize * 0.20f,
...
void ImGui::SetFocusID( ImGuiID id, ImGuiWindow* window ) {
    ...
    // Assume that SetFocusID() is called in the context where its NavLayer is
    // the current layer, which is the case everywhere we call it.
    ...
    if ( g.NavWindow )
        if ( ImGuiWindow* focused_root_window = g.NavWindow->RootWindow )
            if ( focused_root_window->WasActive &&
                ...
                // popup" from "modal popup" NB: The order of those two tests is
                // important because Modal windows are also Popups.
...
void ImGui::ItemSize( const ImVec2& size, float text_offset_y ) {
    ...
    window->DC.CursorPos =
        ImVec2( ( float )( int )( window->Pos.x + window->DC.Indent.x +
...
// size requirement to ItemSize() and then use a larger region for
...
bool ImGui::ItemAdd( const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg ) {
    ...
    if ( id != 0 ) {
        ...
        //      it may not scale very well for windows with ten of thousands of
        //      item, but at least NavMoveRequest is only set on user
        //      interaction, aka maximum once a frame. We could early out with
        //      "if (is_clipped && !g.NavInitRequest) return false;" but when we
        //      wouldn't be able to reach unclipped widgets. This would work if
        //      user had explicit scrolling control (e.g. mapped on a stick)
...
bool ImGui::IsItemHovered( ImGuiHoveredFlags flags ) {
    ...
    // RootWindow instead. I believe it is correct to NOT test for RootWindow
    // but this leaves us unable to use IsItemHovered() after EndChild() itself.
    // Until a solution is found I believe reverting to the test from 2017/09/27
    ...
    // Special handling for the 1st item after Begin() which represent the title
    // bar. When the window is collapsed (SkipItems==true) that last item will
    // never be overwritten so we need to detect tht case.
...
bool ImGui::FocusableItemRegister( ImGuiWindow* window,
    ...
    const bool allow_keyboard_focus =
        ( window->DC.ItemFlags &
          ( ImGuiItemFlags_AllowKeyboardFocus | ImGuiItemFlags_Disabled ) ) ==
        ImGuiItemFlags_AllowKeyboardFocus;
    window->FocusIdxAllCounter++;
    if ( allow_keyboard_focus )
        window->FocusIdxTabCounter++;

    // Process keyboard input at this point: TAB/Shift-TAB to tab out of the
    // currently focused item. Note that we can always TAB out of a widget that
    // doesn't allow tabbing in.
    if ( tab_stop && ( g.ActiveId == id ) &&
         window->FocusIdxAllRequestNext == INT_MAX &&
         window->FocusIdxTabRequestNext == INT_MAX && !g.IO.KeyCtrl &&
         IsKeyPressedMap( ImGuiKey_Tab ) )
        window->FocusIdxTabRequestNext =
            window->FocusIdxTabCounter +
            ( g.IO.KeyShift
                  ? ( allow_keyboard_focus ? -1 : 0 )
                  : +1 ); // Modulo on index will be applied at the end of frame
                          // once we've got the total counter of items.

    if ( window->FocusIdxAllCounter == window->FocusIdxAllRequestCurrent )
        return true;
    if ( allow_keyboard_focus &&
         window->FocusIdxTabCounter == window->FocusIdxTabRequestCurrent ) {
...
ImVec2 ImGui::CalcItemSize( ImVec2 size, float default_x, float default_y ) {
    ...
    if ( size.x <= 0.0f )
        size.x = ( size.x == 0.0f )
                     ? default_x
                     : ImMax( content_max.x - g.CurrentWindow->DC.CursorPos.x,
                              4.0f ) +
                           size.x;
    if ( size.y <= 0.0f )
        size.y = ( size.y == 0.0f )
                     ? default_y
                     : ImMax( content_max.y - g.CurrentWindow->DC.CursorPos.y,
                              4.0f ) +
                           size.y;
    return size;
}
...
void ImGui::SetCurrentContext( ImGuiContext* ctx ) {
    ...
    IMGUI_SET_CURRENT_CONTEXT_FUNC(
        ctx ); // For custom thread-based hackery you may want to have control
               // over this.
#else
    GImGui = ctx;
#endif
...
// Helper function to verify that the type sizes are matching between the
...
// pragma, etc. you may see different structures from what imgui.cpp sees which
...
// Same value as passed to the old io.RenderDrawListsFn function. Valid after
...
void ImGui::StartMouseMovingWindow( ImGuiWindow* window ) {
    ...
    // from a window with _NoMove would activate hover on other windows.
    ImGuiContext& g = *GImGui;
    FocusWindow( window );
...
static void ImGui::UpdateMouseInputs() {
    ...
    for ( int i = 0; i < IM_ARRAYSIZE( g.IO.MouseDown ); i++ ) {
        ...
        if ( g.IO.MouseClicked[ i ] ) {
            if ( ( float )( g.Time - g.IO.MouseClickedTime[ i ] ) <
                ...
                g.IO.MouseClickedTime[ i ] =
                    -FLT_MAX; // so the third click isn't turned into a
                              // double-click
            } else {
        ...
        if ( g.IO.MouseClicked[ i ] ) // Clicking any mouse button reactivate
                                      // mouse hovering which may have been
                                      // deactivated by gamepad/keyboard
                                      // navigation
            g.NavDisableMouseHover = false;
    }
}
...
void ImGui::UpdateMouseWheel() {
    ...
    // give a chance to scroll its parent (unless either
    // ImGuiWindowFlags_NoInputs or ImGuiWindowFlags_NoScrollbar are also set).
    ImGuiWindow* window = g.HoveredWindow;
    ImGuiWindow* scroll_window = window;
    while ( ( scroll_window->Flags & ImGuiWindowFlags_ChildWindow ) &&
            ( scroll_window->Flags & ImGuiWindowFlags_NoScrollWithMouse ) &&
            !( scroll_window->Flags & ImGuiWindowFlags_NoScrollbar ) &&
            !( scroll_window->Flags & ImGuiWindowFlags_NoInputs ) &&
    ...
    if ( g.IO.MouseWheel != 0.0f ) {
        if ( g.IO.KeyCtrl && g.IO.FontAllowUserScaling ) {
            ...
            scroll_amount = ( float )( int )ImMin(
                scroll_amount, ( scroll_window->ContentsRegionRect.GetHeight() +
                                 scroll_window->WindowPadding.y * 2.0f ) *
                                   0.67f );
            SetWindowScrollY(
                scroll_window,
                scroll_window->Scroll.y - g.IO.MouseWheel * scroll_amount );
        }
    }
    if ( g.IO.MouseWheelH != 0.0f && scroll_allowed && !g.IO.KeyCtrl ) {
        // Mouse wheel horizontal scrolling (for hardware that supports it)
        float scroll_amount = scroll_window->CalcFontSize();
        SetWindowScrollX( scroll_window, scroll_window->Scroll.x -
...
void ImGui::UpdateHoveredWindowAndCaptureFlags() {
    ...
    // keyboard info to imgui, false = dispatch keyboard info to imgui + app)
...
void ImGui::NewFrame() {
    ...
    IM_ASSERT( g.IO.DeltaTime >= 0.0f &&
               "Need a positive DeltaTime (zero is tolerated but will cause "
               "some timing issues)" );
    ...
    IM_ASSERT( g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f &&
               "Invalid style setting. Alpha cannot be negative (allows us to "
               "avoid a few clamps in color computations)" );
    ...
    for ( int n = 0; n < ImGuiKey_COUNT; n++ )
        IM_ASSERT( g.IO.KeyMap[ n ] >= -1 &&
                   ...
                   "0..512, or -1 for unmapped key)" );

    // Perform simple check: required key mapping (we intentionally do NOT check
    // all keys to not pressure user into setting up everything, but Space is
    // required and was only recently added in 1.60 WIP)
    if ( g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard )
        IM_ASSERT(
            g.IO.KeyMap[ ImGuiKey_Space ] != -1 &&
            "ImGuiKey_Space is not mapped, required for keyboard navigation." );

    // Perform simple check: the beta io.ConfigResizeWindowsFromEdges option
    // requires back-end to honor mouse cursor changes and set the
    // ImGuiBackendFlags_HasMouseCursors flag accordingly.
    if ( g.IO.ConfigResizeWindowsFromEdges &&
    ...
    g.OverlayDrawList.Flags =
        ( g.Style.AntiAliasedLines ? ImDrawListFlags_AntiAliasedLines : 0 ) |
        ( g.Style.AntiAliasedFill ? ImDrawListFlags_AntiAliasedFill : 0 );
    ...
    if ( g.ActiveId == 0 && g.NavWindow != NULL && g.NavWindow->Active &&
         !( g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs ) &&
         !g.IO.KeyCtrl && IsKeyPressedMap( ImGuiKey_Tab, false ) ) {
        if ( g.NavId != 0 && g.NavIdTabCounter != INT_MAX )
            g.NavWindow->FocusIdxTabRequestNext =
    ...
    // something to it. We don't use "Debug" to avoid colliding with user trying
    // to create a "Debug" window with custom flags.
...
static void AddWindowToSortedBuffer(
    ...
    if ( window->Active ) {
        ...
        if ( count > 1 )
            ImQsort( window->DC.ChildWindows.begin(), ( size_t )count,
...
static void AddDrawListToDrawData( ImVector< ImDrawList* >* out_list,
    ...
    // manually: A) Make sure you are coarse clipping, because ImDrawList let
    // all your vertices pass. You can use the Metrics window to inspect draw
    // list contents. B) If you need/want meshes with more than 64K vertices,
    // uncomment the '#define ImDrawIdx unsigned int' line in imconfig.h to set
    // the index size to 4 bytes.
    //    You'll need to handle the 4-bytes indices to your renderer. For
    //    example, the OpenGL example code detect index size at compile-time by
    //    doing:
    //      glDrawElements(GL_TRIANGLES, (GLsizei)pcmd->ElemCount,
    //      sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT,
    //      idx_buffer_offset);
    //    Your own engine or render API may use different parameters or function
    //    calls to specify index sizes. 2 and 4 bytes indices are generally
    //    supported by most API.
    // C) If for some reason you cannot use 4 bytes indices or don't want to, a
    // workaround is to call BeginChild()/EndChild() before reaching the 64K
    // limit to split your draw commands in multiple draw lists.
...
void ImGui::EndFrame() {
    ImGuiContext& g = *GImGui;
    IM_ASSERT( g.Initialized );
    if ( g.FrameCountEnded ==
         g.FrameCount ) // Don't process EndFrame() multiple times.
        return;
    ...
    IM_ASSERT( g.CurrentWindowStack.Size ==
               1 ); // Mismatched Begin()/End() calls, did you forget to call
                    // end on g.CurrentWindow->Name?
    ...
    if ( g.ActiveId == 0 && g.HoveredId == 0 ) {
        if ( !g.NavWindow ||
             !g.NavWindow
                  ->Appearing ) // Unless we just made a window/popup appear
        {
            // Click to focus window and start moving (after we're done with all
            // our widgets)
            if ( g.IO.MouseClicked[ 0 ] ) {
                if ( g.HoveredRootWindow != NULL )
                    StartMouseMovingWindow( g.HoveredWindow );
                else if ( g.NavWindow != NULL &&
                          GetFrontMostPopupModal() == NULL )
                    FocusWindow( NULL ); // Clicking on void disable focus
            }

            // With right mouse button we close popups without changing focus
            // (The left mouse button path calls FocusWindow which will lead
            // NewFrame->ClosePopupsOverWindow to trigger)
            if ( g.IO.MouseClicked[ 1 ] ) {
                // Find the top-most window between HoveredWindow and the front
                // most Modal Window. This is where we can trim the popup stack.
                ImGuiWindow* modal = GetFrontMostPopupModal();
                bool hovered_window_above_modal = false;
                if ( modal == NULL )
                    hovered_window_above_modal = true;
                for ( int i = g.Windows.Size - 1;
                      i >= 0 && hovered_window_above_modal == false; i-- ) {
                    ImGuiWindow* window = g.Windows[ i ];
                    if ( window == modal )
                        break;
...
// ## marker. CalcTextSize("") should return ImVec2(0.0f, GImGui->FontSize)
ImVec2 ImGui::CalcTextSize( const char* text,
    ...
    // Cancel out character spacing for the last character of a line (it is
    // baked into glyph->AdvanceX field)
...
void ImGui::CalcListClipping( int items_count,
    ...
    // We create the union of the ClipRect and the NavScoringRect which at worst
    // should be 1 page away from ClipRect
    ...
    // When performing a navigation request, ensure we have one item extra in
    // the direction we are moving to
...
// updated in Begin(), so windows moved programatically with SetWindowPos() and
...
static void FindHoveredWindow() {
    ...
    if ( g.MovingWindow &&
         !( g.MovingWindow->Flags & ImGuiWindowFlags_NoInputs ) )
    ...
    for ( int i = g.Windows.Size - 1; i >= 0 && hovered_window == NULL; i-- ) {
        ...
        ImRect bb( window->OuterRectClipped.Min - g.Style.TouchExtraPadding,
                   window->OuterRectClipped.Max + g.Style.TouchExtraPadding );
...
// Note that imgui doesn't know the semantic of each entry of io.KeysDown[]. Use
// your own indices/enums according to how your back-end/engine stored them into
// io.KeysDown[]!
...
int ImGui::CalcTypematicPressedRepeatAmount( float t,
    ...
    if ( t <= repeat_delay || repeat_rate <= 0.0f )
        return 0;
    const int count = ( int )( ( t - repeat_delay ) / repeat_rate ) -
...
int ImGui::GetKeyPressedAmount( int key_index,
    ...
    return CalcTypematicPressedRepeatAmount( t, t - g.IO.DeltaTime,
...
bool ImGui::IsMouseClicked( int button, bool repeat ) {
    ...
    if ( repeat && t > g.IO.KeyRepeatDelay ) {
        ...
        if ( ( ImFmod( t - delay, rate ) > rate * 0.5f ) !=
             ( ImFmod( t - delay - g.IO.DeltaTime, rate ) > rate * 0.5f ) )
            return true;
    }
...
// NB: This is only valid if IsMousePosValid(). Back-ends in theory should
// always keep mouse position valid when dragging even outside the client
// window.
ImVec2 ImGui::GetMouseDragDelta( int button, float lock_threshold ) {
    ...
    if ( g.IO.MouseDown[ button ] )
        if ( g.IO.MouseDragMaxDistanceSqr[ button ] >=
             lock_threshold * lock_threshold )
            return g.IO.MousePos -
                   g.IO.MouseClickedPos[ button ]; // Assume we can only get
                                                   // active with left-mouse
...
bool ImGui::IsItemFocused() {
    ...
    return g.NavId && !g.NavDisableHighlight &&
           g.NavId == g.CurrentWindow->DC.LastItemId;
}
...
static bool ImGui::BeginChildEx( const char* name,
    ...
    if ( size.x <= 0.0f )
        size.x = ImMax( content_avail.x + size.x,
                        4.0f ); // Arbitrary minimum child size (0.0f causing
                                // too much issues)
    if ( size.y <= 0.0f )
    ...
    char title[ 256 ];
    if ( name )
        ImFormatString( title, IM_ARRAYSIZE( title ), "%s/%s",
                        parent_window->Name, name );
    else
    ...
    if ( g.NavActivateId == id && !( flags & ImGuiWindowFlags_NavFlattened ) &&
        ...
        SetActiveID( id + 1,
                     child_window ); // Steal ActiveId with a dummy id so that
...
void ImGui::EndChild() {
    ...
    IM_ASSERT( window->Flags &
               ImGuiWindowFlags_ChildWindow ); // Mismatched
                                               // BeginChild()/EndChild() callss
    if ( window->BeginCount > 1 ) {
        End();
    } else {
        ImVec2 sz = window->Size;
        if ( window->AutoFitChildAxises &
             ( 1 << ImGuiAxis_X ) ) // Arbitrary minimum zero-ish child size
        ...
        if ( ( window->DC.NavLayerActiveMask != 0 ||
            ...
            // we keep a highlight on the child
...
static void CheckStacksSize( ImGuiWindow* window, bool write ) {
    ...
    // DC.TextWrapPos (per window) to allow user to conveniently push once and
    // not pop (they are cleared on Begin)
    ImGuiContext& g = *GImGui;
    ...
    IM_ASSERT( p_backup == window->DC.StackSizesBackup +
...
static ImGuiWindow* CreateNewWindow( const char* name,
    ...
    if ( !( flags & ImGuiWindowFlags_NoSavedSettings ) )
        if ( ImGuiWindowSettings* settings =
            ...
            window->SettingsIdx =
                g.SettingsWindows.index_from_pointer( settings );
            SetWindowConditionAllowFlags( window, ImGuiCond_FirstUseEver,
                                          false );
    ...
    window->DC.CursorMaxPos =
        window->Pos; // So first call to CalcSizeContents() doesn't return crazy
                     // values
...
static ImVec2 CalcSizeContents( ImGuiWindow* window ) {
    ImVec2 sz;
    sz.x = ( float )( int )( ( window->SizeContentsExplicit.x != 0.0f )
                                 ? window->SizeContentsExplicit.x
                                 : ( window->DC.CursorMaxPos.x - window->Pos.x +
                                     window->Scroll.x ) );
    sz.y = ( float )( int )( ( window->SizeContentsExplicit.y != 0.0f )
                                 ? window->SizeContentsExplicit.y
                                 : ( window->DC.CursorMaxPos.y - window->Pos.y +
                                     window->Scroll.y ) );
...
static ImVec2 CalcSizeAutoFit( ImGuiWindow* window,
    ...
    if ( window->Flags & ImGuiWindowFlags_Tooltip ) {
        ...
        // Might turn bigger than DisplaySize-WindowPadding.
        ...
        if ( is_popup ||
             is_menu ) // Popups and menus bypass style.WindowMinSize by
                       // default, but we give then a non-zero minimum size to
                       // facilitate understanding problematic cases (e.g. empty
                       // popups)
            size_min = ImMin( size_min, ImVec2( 4.0f, 4.0f ) );
        ImVec2 size_auto_fit = ImClamp(
            size_contents, size_min,
            ImMax( size_min,
                   g.IO.DisplaySize - style.DisplaySafeAreaPadding * 2.0f ) );
        ImVec2 size_auto_fit_after_constraint =
            CalcSizeAfterConstraint( window, size_auto_fit );
        if ( size_auto_fit_after_constraint.x < size_contents.x &&
             !( window->Flags & ImGuiWindowFlags_NoScrollbar ) &&
             ( window->Flags & ImGuiWindowFlags_HorizontalScrollbar ) )
            size_auto_fit.y += style.ScrollbarSize;
        if ( size_auto_fit_after_constraint.y < size_contents.y &&
             !( window->Flags & ImGuiWindowFlags_NoScrollbar ) )
...
ImVec2 ImGui::CalcWindowExpectedSize( ImGuiWindow* window ) {
    ...
    return CalcSizeAfterConstraint( window,
                                    CalcSizeAutoFit( window, size_contents ) );
}

static float GetScrollMaxX( ImGuiWindow* window ) {
    return ImMax( 0.0f, window->SizeContents.x -
                            ( window->SizeFull.x - window->ScrollbarSizes.x ) );
}

static float GetScrollMaxY( ImGuiWindow* window ) {
    return ImMax( 0.0f, window->SizeContents.y -
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp( ImGuiWindow* window,
    ...
    if ( window->ScrollTarget.x < FLT_MAX ) {
        ...
        scroll.x = window->ScrollTarget.x -
                   cr_x * ( window->SizeFull.x - window->ScrollbarSizes.x );
    }
    if ( window->ScrollTarget.y < FLT_MAX ) {
        // 'snap_on_edges' allows for a discontinuity at the edge of scrolling
        // limits to take account of WindowPadding so that scrolling to make the
        // last item visible scroll far enough to see the padding.
        float cr_y = window->ScrollTargetCenterRatio.y;
        float target_y = window->ScrollTarget.y;
        if ( snap_on_edges && cr_y <= 0.0f &&
             target_y <= window->WindowPadding.y )
            target_y = 0.0f;
        if ( snap_on_edges && cr_y >= 1.0f &&
             target_y >= window->SizeContents.y - window->WindowPadding.y +
                             g.Style.ItemSpacing.y )
            target_y = window->SizeContents.y;
        scroll.y = target_y -
                   ( 1.0f - cr_y ) *
                       ( window->TitleBarHeight() + window->MenuBarHeight() ) -
                   cr_y * ( window->SizeFull.y - window->ScrollbarSizes.y );
    }
...
static ImRect GetResizeBorderRect( ImGuiWindow* window,
    ...
    if ( thickness == 0.0f )
        rect.Max -= ImVec2( 1, 1 );
    if ( border_n == 0 )
        return ImRect( rect.Min.x + perp_padding, rect.Min.y,
                       rect.Max.x - perp_padding, rect.Min.y + thickness );
    if ( border_n == 1 )
        return ImRect( rect.Max.x - thickness, rect.Min.y + perp_padding,
                       rect.Max.x, rect.Max.y - perp_padding );
    if ( border_n == 2 )
        return ImRect( rect.Min.x + perp_padding, rect.Max.y - thickness,
                       rect.Max.x - perp_padding, rect.Max.y );
    if ( border_n == 3 )
        return ImRect( rect.Min.x, rect.Min.y + perp_padding,
...
static void ImGui::UpdateManualResize( ImGuiWindow* window,
    ...
    const float grip_draw_size = ( float )( int )ImMax(
    ...
    for ( int resize_grip_n = 0; resize_grip_n < resize_grip_count;
        ...
        const ImVec2 corner =
            ImLerp( window->Pos, window->Pos + window->Size, grip.CornerPos );
        ...
        bool hovered, held;
        ButtonBehavior(
            resize_rect, window->GetID( ( void* )( intptr_t )resize_grip_n ),
            &hovered, &held,
        ...
        if ( held && g.IO.MouseDoubleClicked[ 0 ] && resize_grip_n == 0 ) {
            ...
            ImVec2 corner_target =
                ...
                resize_rect.GetSize() *
                    grip.CornerPos; // Corner of the window corresponding to our
                                    // corner grip
            CalcResizePosSizeFromAnyCorner( window, corner_target,
                                            grip.CornerPos, &pos_target,
                                            &size_target );
        }
    ...
    for ( int border_n = 0; border_n < resize_border_count; border_n++ ) {
        const float BORDER_SIZE = 5.0f; // FIXME: Only works _inside_ window
                                        // because of HoveredWindow check.
        const float BORDER_APPEAR_TIMER = 0.05f; // Reduce visual noise
        bool hovered, held;
        ImRect border_rect = GetResizeBorderRect(
            window, border_n, grip_hover_size, BORDER_SIZE );
        ButtonBehavior( border_rect,
                        window->GetID( ( void* )( intptr_t )( border_n + 4 ) ),
                        &hovered, &held, ImGuiButtonFlags_FlattenChildren );
        if ( ( hovered && g.HoveredIdTimer > BORDER_APPEAR_TIMER ) || held ) {
            g.MouseCursor = ( border_n & 1 ) ? ImGuiMouseCursor_ResizeEW
        ...
        if ( held ) {
            ...
            if ( border_n == 1 ) {
                ...
                border_target.x =
                    ( g.IO.MousePos.x - g.ActiveIdClickOffset.x + BORDER_SIZE );
            }
            if ( border_n == 2 ) {
                border_posn = ImVec2( 0, 1 );
                border_target.y =
                    ( g.IO.MousePos.y - g.ActiveIdClickOffset.y + BORDER_SIZE );
            }
            if ( border_n == 3 ) {
                border_posn = ImVec2( 0, 0 );
                border_target.x = ( g.IO.MousePos.x - g.ActiveIdClickOffset.x );
            }
            CalcResizePosSizeFromAnyCorner( window, border_target, border_posn,
    ...
    if ( g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window ) {
        ...
        if ( g.NavInputSource == ImGuiInputSource_NavKeyboard && g.IO.KeyShift )
            nav_resize_delta = GetNavInputAmount2d(
                ImGuiNavDirSourceFlags_Keyboard, ImGuiInputReadMode_Down );
        if ( g.NavInputSource == ImGuiInputSource_NavGamepad )
            nav_resize_delta = GetNavInputAmount2d(
                ImGuiNavDirSourceFlags_PadDPad, ImGuiInputReadMode_Down );
        if ( nav_resize_delta.x != 0.0f || nav_resize_delta.y != 0.0f ) {
            ...
            size_target = CalcSizeAfterConstraint(
...
bool ImGui::Begin( const char* name, bool* p_open, ImGuiWindowFlags flags ) {
    ...
    IM_ASSERT( g.FrameCountEnded !=
               g.FrameCount ); // Called ImGui::Render() or ImGui::EndFrame()
    ...
    if ( window_just_created ) {
        ImVec2 size_on_first_use =
            ( g.NextWindowData.SizeCond != 0 )
                ...
                : ImVec2( 0.0f, 0.0f ); // Any condition flag will do since we
                                        // are creating a new window here.
    ...
    const bool window_just_appearing_after_hidden_for_resize =
        ( window->HiddenFramesForResize > 0 );
    ...
    if ( first_begin_of_the_frame ) {
        ...
        if ( window_just_activated_by_user &&
             ( flags &
               ( ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip ) ) != 0 ) {
            window->HiddenFramesForResize = 1;
        ...
        window->WindowBorderSize =
            ( flags & ImGuiWindowFlags_ChildWindow ) ? style.ChildBorderSize
            : ( ( flags &
        ...
        const ImVec2 size_auto_fit =
            CalcSizeAutoFit( window, window->SizeContents );
        ...
        // Update scrollbar status (based on the Size that was effective during
        // last frame or the auto-resized Size).
        if ( !window->Collapsed ) {
            // When reading the current size we need to read it after size
            // constraints have been applied
            float size_x_for_scrollbars = size_full_modified.x != FLT_MAX
                                              ? window->SizeFull.x
                                              : window->SizeFullAtLastBegin.x;
            float size_y_for_scrollbars = size_full_modified.y != FLT_MAX
                                              ? window->SizeFull.y
                                              : window->SizeFullAtLastBegin.y;
            window->ScrollbarY =
                ( flags & ImGuiWindowFlags_AlwaysVerticalScrollbar ) ||
                ( ( window->SizeContents.y > size_y_for_scrollbars ) &&
            ...
            SetWindowPos(
                ...
                ImMax( style.DisplaySafeAreaPadding,
                       window->SetWindowPosVal -
                           window->SizeFull * window->SetWindowPosPivot ),
        ...
        if ( !( flags & ImGuiWindowFlags_ChildWindow ) ) {
            if ( !window_pos_set_by_api && window->AutoFitFramesX <= 0 &&
                 ...
                 g.IO.DisplaySize.y >
                     0.0f ) // Ignore zero-sized display explicitly to avoid
                            // losing positions if a window manager reports
                            // zero-sized window when initializing or
                            // minimizing.
            {
                ImVec2 padding = ImMax( style.DisplayWindowPadding,
                                        style.DisplaySafeAreaPadding );
                window->Pos =
                    ImMax( window->Pos + window->Size, padding ) - window->Size;
        ...
        window->FocusIdxAllRequestNext = window->FocusIdxTabRequestNext =
            INT_MAX;
        ...
        const int resize_grip_count =
            g.IO.ConfigResizeWindowsFromEdges ? 2 : 1; // 4
        const float grip_draw_size = ( float )( int )ImMax(
            g.FontSize * 1.35f,
            window->WindowRounding + 1.0f + g.FontSize * 0.2f );
        if ( !window->Collapsed )
            UpdateManualResize( window, size_auto_fit, &border_held,
        ...
        window->DrawList->Flags =
            ( g.Style.AntiAliasedLines ? ImDrawListFlags_AntiAliasedLines
                                       : 0 ) |
            ( g.Style.AntiAliasedFill ? ImDrawListFlags_AntiAliasedFill : 0 );
        ...
        if ( ( flags & ImGuiWindowFlags_ChildWindow ) &&
            ...
            PushClipRect( parent_window->ClipRect.Min,
                          parent_window->ClipRect.Max, true );
        else
            PushClipRect( viewport_rect.Min, viewport_rect.Max, true );

        // Draw modal window background (darkens what is behind them, all
        // viewports)
        const bool dim_bg_for_modal = ( flags & ImGuiWindowFlags_Modal ) &&
                                      window == GetFrontMostPopupModal() &&
                                      window->HiddenFramesForResize <= 0;
        ...
        if ( dim_bg_for_modal || dim_bg_for_window_list ) {
            const ImU32 dim_bg_col =
                GetColorU32( dim_bg_for_modal ? ImGuiCol_ModalWindowDimBg
                                              : ImGuiCol_NavWindowingDimBg,
                             g.DimBgRatio );
            window->DrawList->AddRectFilled( viewport_rect.Min,
                                             viewport_rect.Max, dim_bg_col );
        }
        ...
        if ( dim_bg_for_window_list && window == g.NavWindowingTargetAnim ) {
            ...
            if ( !bb.Contains(
                     viewport_rect ) ) // Avoid drawing if the window covers all
                                       // the viewport anyway
        ...
        if ( window->Collapsed ) {
            ...
            RenderFrame( title_bar_rect.Min, title_bar_rect.Max, title_bar_col,
            ...
            if ( g.NextWindowData.BgAlphaCond != 0 ) {
                bg_col = ( bg_col & ~IM_COL32_A_MASK ) |
                         ( IM_F32_TO_INT8_SAT( g.NextWindowData.BgAlphaVal )
                           << IM_COL32_A_SHIFT );
            ...
            // Title bar
            ImU32 title_bar_col =
                GetColorU32( window->Collapsed ? ImGuiCol_TitleBgCollapsed
            ...
            if ( flags & ImGuiWindowFlags_MenuBar ) {
                ...
                menu_bar_rect.ClipWith(
                    window
                        ->Rect() ); // Soft clipping, in particular child window
                ...
                window->DrawList->AddRectFilled(
                    ...
                    ( flags & ImGuiWindowFlags_NoTitleBar ) ? window_rounding
                                                            : 0.0f,
            ...
            // Render resize grips (after their input handling so we don't have
            // a frame of latency)
            if ( !( flags & ImGuiWindowFlags_NoResize ) ) {
                for ( int resize_grip_n = 0; resize_grip_n < resize_grip_count;
                      resize_grip_n++ ) {
                    ...
                    const ImVec2 corner =
                        ImLerp( window->Pos, window->Pos + window->Size,
                                grip.CornerPos );
                    window->DrawList->PathLineTo(
                        corner +
                        grip.InnerDir *
                            ( ( resize_grip_n & 1 )
                                  ? ImVec2( window_border_size, grip_draw_size )
                    ...
                    window->DrawList->PathLineTo(
                        ...
                        grip.InnerDir *
                            ( ( resize_grip_n & 1 )
                                  ? ImVec2( grip_draw_size, window_border_size )
                                  : ImVec2( window_border_size,
                                            grip_draw_size ) ) );
                    window->DrawList->PathArcToFast(
                        ImVec2(
                            corner.x + grip.InnerDir.x * ( window_rounding +
                                                           window_border_size ),
                            corner.y +
                                grip.InnerDir.y *
                                    ( window_rounding + window_border_size ) ),
            ...
            if ( window_border_size > 0.0f )
                window->DrawList->AddRect(
                    ...
                    ImDrawCornerFlags_All, window_border_size );
            if ( border_held != -1 ) {
                ImRect border = GetResizeBorderRect( window, border_held,
                                                     grip_draw_size, 0.0f );
                window->DrawList->AddLine(
            ...
            if ( style.FrameBorderSize > 0 &&
                 !( flags & ImGuiWindowFlags_NoTitleBar ) )
                window->DrawList->AddLine(
                    title_bar_rect.GetBL() +
                        ImVec2( style.WindowBorderSize, -1 ),
        ...
        if ( g.NavWindowingTargetAnim == window ) {
            float rounding =
                ImMax( window->WindowRounding, g.Style.WindowRounding );
            ImRect bb = window->Rect();
            bb.Expand( g.FontSize );
            if ( bb.Contains(
                     viewport_rect ) ) // If a window fits the entire viewport,
        ...
        // Store a backup of SizeFull which we will use next frame to decide if
        // we need scrollbars.
        window->SizeFullAtLastBegin = window->SizeFull;

        // Update various regions. Variables they depends on are set above in
        // this function.
        // FIXME: window->ContentsRegionRect.Max is currently very misleading /
        // partly faulty, but some BeginChild() patterns relies on it.
        ...
        window->DC.ItemFlags = parent_window ? parent_window->DC.ItemFlags
        ...
        window->MenuColumns.Update( 3, style.ItemSpacing.x,
                                    window_just_activated_by_user );
        ...
        if ( !( flags & ImGuiWindowFlags_NoTitleBar ) ) {
            ...
            if ( !( flags & ImGuiWindowFlags_NoCollapse ) )
                if ( CollapseButton( window->GetID( "#COLLAPSE" ),
                    ...
                    window->WantCollapseToggle =
                        true; // Defer collapsing to next frame as we are too
                              // far in the Begin() function
            ...
            // Title text (FIXME: refactor text alignment facilities along with
            // RenderText helpers, this is too much code for what it does.)
            ImVec2 text_size = CalcTextSize( name, NULL, true );
            ImRect text_r = title_bar_rect;
            float pad_left = ( flags & ImGuiWindowFlags_NoCollapse )
                                 ? style.FramePadding.x
                                 : ( style.FramePadding.x + g.FontSize +
                                     style.ItemInnerSpacing.x );
            float pad_right = ( p_open == NULL )
                                  ? style.FramePadding.x
                                  : ( style.FramePadding.x + g.FontSize +
                                      style.ItemInnerSpacing.x );
            if ( style.WindowTitleAlign.x > 0.0f )
                pad_right =
                    ImLerp( pad_right, pad_left, style.WindowTitleAlign.x );
            text_r.Min.x += pad_left;
            text_r.Max.x -= pad_right;
            ImRect clip_rect = text_r;
            clip_rect.Max.x =
                window->Pos.x + window->Size.x -
                ( p_open ? title_bar_rect.GetHeight() - 3
                         : style.FramePadding
                               .x ); // Match the size of CloseButton()
            RenderTextClipped( text_r.Min, text_r.Max, name, NULL, &text_size,
        ...
        */
        ...
        // rectangle doesn't lag by a frame Note that if our window is collapsed
        // we will end up with an inverted (~null) clipping rectangle which is
        // the correct behavior.
        window->InnerMainRect.Min.x =
            title_bar_rect.Min.x + window->WindowBorderSize;
        window->InnerMainRect.Min.y =
            title_bar_rect.Max.y + window->MenuBarHeight() +
            ( ( ( flags & ImGuiWindowFlags_MenuBar ) ||
                !( flags & ImGuiWindowFlags_NoTitleBar ) )
                  ? style.FrameBorderSize
                  : window->WindowBorderSize );
        window->InnerMainRect.Max.x = window->Pos.x + window->Size.x -
                                      window->ScrollbarSizes.x -
                                      window->WindowBorderSize;
        window->InnerMainRect.Max.y = window->Pos.y + window->Size.y -
                                      window->ScrollbarSizes.y -
                                      window->WindowBorderSize;
        // window->DrawList->AddRect(window->InnerRect.Min,
        // window->InnerRect.Max, IM_COL32_WHITE);
        ...
        window->InnerClipRect.Min.x =
            ImFloor( 0.5f + window->InnerMainRect.Min.x +
                     ImMax( 0.0f, ImFloor( window->WindowPadding.x * 0.5f -
                                           window->WindowBorderSize ) ) );
        window->InnerClipRect.Min.y =
            ImFloor( 0.5f + window->InnerMainRect.Min.y );
        window->InnerClipRect.Max.x =
            ImFloor( 0.5f + window->InnerMainRect.Max.x -
                     ImMax( 0.0f, ImFloor( window->WindowPadding.x * 0.5f -
                                           window->WindowBorderSize ) ) );
        window->InnerClipRect.Max.y =
            ImFloor( 0.5f + window->InnerMainRect.Max.y );

        // After Begin() we fill the last item / hovered data based on title bar
        // data. It is a standard behavior (to allow creation of context menus
    ...
    if ( flags & ImGuiWindowFlags_ChildWindow ) {
        ...
        if ( parent_window &&
             ( parent_window->Collapsed || parent_window->Hidden ) )
            window->HiddenFramesRegular = 1;
    }
...
bool ImGui::Begin( const char* name,
                   ...
                   ImGuiWindowFlags flags ) {
    // Old API feature: we could pass the initial window size as a parameter.
    // This was misleading because it only had an effect if the window didn't
    // have data in the .ini file.
...
void ImGui::BringWindowToFront( ImGuiWindow* window ) {
    ...
    if ( current_front_window == window ||
         current_front_window->RootWindow == window )
        return;
    for ( int i = g.Windows.Size - 2; i >= 0;
          i-- ) // We can ignore the front most window
...
void ImGui::FocusWindow( ImGuiWindow* window ) {
    ...
    if ( window->Flags &
         ImGuiWindowFlags_Popup ) // FIXME: This statement should be
                                  // unnecessary. Need further testing before
                                  // removing it..
...
void ImGui::FocusFrontMostActiveWindowIgnoringOne(
    ImGuiWindow* ignore_window ) {
    ImGuiContext& g = *GImGui;
    for ( int i = g.Windows.Size - 1; i >= 0; i-- )
        if ( g.Windows[ i ] != ignore_window && g.Windows[ i ]->WasActive &&
             !( g.Windows[ i ]->Flags & ImGuiWindowFlags_ChildWindow ) ) {
            ImGuiWindow* focus_window =
                NavRestoreLastChildNavWindow( g.Windows[ i ] );
...
void ImGui::PushMultiItemsWidths( int components, float w_full ) {
    ...
    const float w_item_one =
        ImMax( 1.0f, ( float )( int )( ( w_full - ( style.ItemInnerSpacing.x ) *
                                                      ( components - 1 ) ) /
                                       ( float )components ) );
    const float w_item_last = ImMax(
        1.0f,
        ( float )( int )( w_full - ( w_item_one + style.ItemInnerSpacing.x ) *
                                       ( components - 1 ) ) );
...
float ImGui::CalcItemWidth() {
    ...
    if ( w < 0.0f ) {
        ...
        // calculation because this is how the width is always used (we add 2
        // frame padding to it), but we could move that responsibility to the
        // widget as well.
...
void ImGui::SetCurrentFont( ImFont* font ) {
    ImGuiContext& g = *GImGui;
    IM_ASSERT( font && font->IsLoaded() ); // Font Atlas not created. Did you
                                           // call io.Fonts->GetTexDataAsRGBA32
                                           // / GetTexDataAsAlpha8 ?
...
void ImGui::PopItemFlag() {
    ...
    window->DC.ItemFlags = window->DC.ItemFlagsStack.empty()
                               ? ImGuiItemFlags_Default_
...
void ImGui::PushStyleVar( ImGuiStyleVar idx, const ImVec2& val ) {
    ...
    IM_ASSERT(
        0 ); // Called function with wrong-type? Variable is not a ImVec2.
}
...
void ImGui::SetWindowScrollX( ImGuiWindow* window, float new_scroll_x ) {
    window->DC.CursorMaxPos.x +=
        window->Scroll.x; // SizeContents is generally computed based on
                          // CursorMaxPos which is affected by scroll position,
                          // so we need to apply our change to it.
...
void ImGui::SetWindowScrollY( ImGuiWindow* window, float new_scroll_y ) {
    window->DC.CursorMaxPos.y +=
        window->Scroll.y; // SizeContents is generally computed based on
                          // CursorMaxPos which is affected by scroll position,
                          // so we need to apply our change to it.
...
static void SetWindowPos( ImGuiWindow* window,
    ...
    window->DC.CursorPos +=
        ( window->Pos -
          old_pos ); // As we happen to move the window while it is being
                     // appended to (which is a bad idea - will smear) let's at
                     // least offset the cursor
    window->DC.CursorMaxPos +=
        ( window->Pos -
          old_pos ); // And more importantly we need to adjust this so size
...
void ImGui::SetNextWindowContentSize( const ImVec2& size ) {
    ...
    g.NextWindowData.ContentSizeVal =
        size; // In Begin() we will add the size of window decorations (title
              // bar, menu etc.) to that to form a SizeContents value.
...
void ImGui::SetNextWindowFocus() {
    ...
    g.NextWindowData.FocusCond =
        ImGuiCond_Always; // Using a Cond member for consistency (may transition
                          // all of them to single flag set for fast Clear() op)
}

void ImGui::SetNextWindowBgAlpha( float alpha ) {
    ImGuiContext& g = *GImGui;
    g.NextWindowData.BgAlphaVal = alpha;
    g.NextWindowData.BgAlphaCond =
        ImGuiCond_Always; // Using a Cond member for consistency (may transition
                          // all of them to single flag set for fast Clear() op)
}
...
ImVec2 ImGui::GetContentRegionMax() {
    ...
    if ( window->DC.ColumnsSet )
        mx.x = GetColumnOffset( window->DC.ColumnsSet->Current + 1 ) -
...
void ImGui::SetScrollY( float scroll_y ) {
    ...
    window->ScrollTarget.y =
        scroll_y + window->TitleBarHeight() +
        window->MenuBarHeight(); // title bar height canceled out when using
                                 // ScrollTargetRelY
    window->ScrollTargetCenterRatio.y = 0.0f;
}

void ImGui::SetScrollFromPosY( float pos_y, float center_y_ratio ) {
    // We store a target position so centering can occur on the next frame when
    // we are guaranteed to have a known window size
    ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::SetScrollHere( float center_y_ratio ) {
    ImGuiWindow* window = GetCurrentWindow();
    float target_y = window->DC.CursorPosPrevLine.y -
                     window->Pos.y; // Top of last item, in window space
    target_y +=
        ( window->DC.PrevLineSize.y * center_y_ratio ) +
...
void ImGui::PushID( const char* str_id_begin, const char* str_id_end ) {
    ...
    window->IDStack.push_back(
        window->GetIDNoKeepAlive( str_id_begin, str_id_end ) );
}
...
void ImGui::BeginGroup() {
    ...
    window->DC.LogLinePosY =
        window->DC.CursorPos.y - 9999.0f; // To enforce Log carriage return
}
...
void ImGui::EndGroup() {
    ...
    if ( group_data.AdvanceCursor ) {
        window->DC.CurrentLineTextBaseOffset = ImMax(
            window->DC.PrevLineTextBaseOffset,
            group_data
                .BackupCurrentLineTextBaseOffset ); // FIXME: Incorrect, we
                                                    // should grab the base
                                                    // offset from the *first
                                                    // line* of the group but it
                                                    // is hard to obtain now.
    ...
    // functional on the entire group. It would be be neater if we replaced
    // window.DC.LastItemId by e.g. 'bool LastItemIsActive', but would put a
    // little more burden on individual widgets. (and if you grep for LastItemId
    // you'll notice it is only used in that context.
    if ( ( group_data.BackupActiveIdIsAlive != g.ActiveId ) &&
         ( g.ActiveIdIsAlive == g.ActiveId ) &&
         g.ActiveId ) // && g.ActiveIdWindow->RootWindow == window->RootWindow)
        window->DC.LastItemId = g.ActiveId;
    else if (
        !group_data.BackupActiveIdPreviousFrameIsAlive &&
        g.ActiveIdPreviousFrameIsAlive ) // &&
                                         // g.ActiveIdPreviousFrameWindow->RootWindow
                                         // == window->RootWindow)
...
//      window/group left) spacing_w < 0   : use default spacing if pos_x == 0,
//      no spacing if pos_x != 0 spacing_w >= 0  : enforce spacing amount
void ImGui::SameLine( float pos_x, float spacing_w ) {
    ...
    if ( pos_x != 0.0f ) {
        ...
        window->DC.CursorPos.x = window->Pos.x - window->Scroll.x + pos_x +
                                 spacing_w + window->DC.GroupOffset.x +
                                 window->DC.ColumnsOffset.x;
...
void ImGui::BeginTooltip() {
    ...
    if ( g.DragDropWithinSourceOrTarget ) {
        // The default tooltip position is a little offset to give space to see
        // the context menu (it's also clamped within the current
        // viewport/monitor) In the context of a dragging tooltip we try to
        // reduce that offset and we enforce following the cursor. Whatever we
        // do we want to call SetNextWindowPos() to enforce a tooltip position
        // and disable clipping the tooltip without our display area, like
        // regular tooltip do.
...
// Not exposed publicly as BeginTooltip() because bool parameters are evil.
// Let's see if other needs arise first.
...
bool ImGui::IsPopupOpen( const char* str_id ) {
    ImGuiContext& g = *GImGui;
    return g.OpenPopupStack.Size > g.CurrentPopupStack.Size &&
           g.OpenPopupStack[ g.CurrentPopupStack.Size ].PopupId ==
...
void ImGui::OpenPopupEx( ImGuiID id ) {
    ...
    ImGuiPopupRef popup_ref; // Tagged as new ref as Window will be set back to
    ...
    if ( g.OpenPopupStack.Size < current_stack_size + 1 ) {
        ...
        // Gently handle the user mistakenly calling OpenPopup() every frame. It
        // is a programming mistake! However, if we were to run the regular code
        // path, the ui would become completely unusable because the popup will
        // always be in hidden-while-calculating-size state _while_ claiming
...
void ImGui::ClosePopupsOverWindow( ImGuiWindow* ref_window ) {
    ...
    // to it and close popups above it. Don't close our own child popup windows.
    int n = 0;
    if ( ref_window ) {
    ...
    if ( n < g.OpenPopupStack
                 .Size ) // This test is not required but it allows to set a
                         // convenient breakpoint on the block below
        ClosePopupToLevel( n );
}
...
void ImGui::ClosePopupToLevel( int remaining ) {
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* focus_window = ( remaining > 0 )
                                    ? g.OpenPopupStack[ remaining - 1 ].Window
                                    : g.OpenPopupStack[ 0 ].ParentWindow;
    if ( g.NavLayer == 0 )
...
bool ImGui::BeginPopup( const char* str_id, ImGuiWindowFlags flags ) {
    ...
    if ( g.OpenPopupStack.Size <=
         g.CurrentPopupStack.Size ) // Early out for performance
    {
        g.NextWindowData
            .Clear(); // We behave like Begin() and need to consume those values
        return false;
    }
    return BeginPopupEx( g.CurrentWindow->GetID( str_id ),
                         flags | ImGuiWindowFlags_AlwaysAutoResize |
...
bool ImGui::BeginPopupModal( const char* name,
    ...
    if ( g.NextWindowData.PosCond == 0 )
        SetNextWindowPos( g.IO.DisplaySize * 0.5f, ImGuiCond_Appearing,
                          ImVec2( 0.5f, 0.5f ) );
...
// to one given widget. You may want to handle this on user side if you have
// specific needs (e.g. tweaking IsItemHovered() parameters). You can pass a
// NULL str_id to use the identifier of the last item.
    ...
    ImGuiID id =
        str_id ? window->GetID( str_id )
               : window->DC.LastItemId; // If user hasn't passed an ID, we can
                                        // use the LastItemID. Using LastItemID
                                        // as a Popup ID won't conflict!
    IM_ASSERT( id != 0 ); // You cannot pass a NULL str_id if the last item has
...
ImRect ImGui::GetWindowAllowedExtentRect( ImGuiWindow* ) {
    ...
    r_screen.Expand( ImVec2(
        ( r_screen.GetWidth() > padding.x * 2 ) ? -padding.x : 0.0f,
...
ImVec2 ImGui::FindBestWindowPosForPopupEx( const ImVec2& ref_pos,
    ...
    // GImGui->OverlayDrawList.AddRect(r_avoid.Min, r_avoid.Max,
    // IM_COL32(255,0,0,255)); GImGui->OverlayDrawList.AddRect(r_outer.Min,
...
ImVec2 ImGui::FindBestWindowPosForPopup( ImGuiWindow* window ) {
    ...
    if ( window->Flags & ImGuiWindowFlags_ChildMenu ) {
        ...
        // item, and then our FindBestWindowPosForPopup() function will move the
        // new menu outside the parent bounds. This is how we end up with child
        // menus appearing (most-commonly) on the right of the parent menu.
        ...
        if ( parent_window->DC.MenuBarAppending )
            r_avoid = ImRect(
                -FLT_MAX,
                parent_window->Pos.y + parent_window->TitleBarHeight(), FLT_MAX,
    ...
    if ( window->Flags & ImGuiWindowFlags_Tooltip ) {
        ...
        if ( !g.NavDisableHighlight && g.NavDisableMouseHover &&
            ...
            r_avoid = ImRect(
                ...
                ref_pos.y + 24 * sc ); // FIXME: Hard-coded based on mouse
        ...
        if ( window->AutoPosLastDirection == ImGuiDir_None )
            pos =
                ref_pos +
                ImVec2(
                    2, 2 ); // If there's not enough room, for tooltip we prefer
                            // avoiding the cursor at all cost even if it means
...
static float inline NavScoreItemDistInterval( float a0,
                                              float a1,
                                              float b0,
                                              float b1 ) {
    if ( a1 < b0 )
...
static bool NavScoreItem( ImGuiNavMoveResult* result, ImRect cand ) {
    ...
    const ImRect& curr =
        g.NavScoringRectScreen; // Current modified source rect (NB: we've
                                // applied Max.x = Min.x in NavUpdate() to
                                // inhibit the effect of having varied item
                                // width)
    g.NavScoringCount++;
    ...
    // We perform scoring on items bounding box clipped by the current clipping
    // rectangle on the other axis (clipping on our movement axis would give us
    // equal scores for all clipped items) For example, this ensure that items
    // in one column are not reached when moving vertically from items in
    // another column.
...
static void ImGui::NavProcessItem( ImGuiWindow* window,
    ...
    // test but I am not sure of all the side-effects yet. Some of the feature
    // here will need to work regardless of using a _NoNavInputs flag.
    //     return;
    ...
    // NavScoringRectScreen + scoring from a rect wrapped according to current
    // wrapping policy)
    if ( ( g.NavId != id ||
           ( g.NavMoveRequestFlags & ImGuiNavMoveFlags_AllowCurrentNavId ) ) &&
         !( item_flags & ImGuiItemFlags_NoNav ) ) {
    ...
    if ( g.NavId == id ) {
        g.NavWindow =
            window; // Always refresh g.NavWindow, because some operations such
                    // as FocusItem() don't have a window.
...
void ImGui::NavMoveRequestTryWrapping( ImGuiWindow* window,
    ...
    if ( g.NavMoveDir == ImGuiDir_Left &&
        ...
        bb_rel.Min.x = bb_rel.Max.x =
            ImMax( window->SizeFull.x, window->SizeContents.x ) -
            window->Scroll.x;
    ...
    if ( g.NavMoveDir == ImGuiDir_Up &&
        ...
        bb_rel.Min.y = bb_rel.Max.y =
            ImMax( window->SizeFull.y, window->SizeContents.y ) -
            window->Scroll.y;
...
static void ImGui::NavSaveLastChildNavWindow( ImGuiWindow* nav_window ) {
    ...
    while ( parent_window &&
            ( parent_window->Flags & ImGuiWindowFlags_ChildWindow ) != 0 &&
            ( parent_window->Flags &
...
static ImGuiWindow* ImGui::NavRestoreLastChildNavWindow( ImGuiWindow* window ) {
    return window->NavLastChildNavWindow ? window->NavLastChildNavWindow
                                         : window;
}
...
static void NavRestoreLayer( int layer ) {
    ...
    if ( layer == 0 && g.NavWindow->NavLastIds[ 0 ] != 0 )
        ImGui::SetNavIDWithRectRel( g.NavWindow->NavLastIds[ 0 ], layer,
                                    g.NavWindow->NavRectRel[ 0 ] );
    else
...
static ImVec2 ImGui::NavCalcPreferredRefPos() {
    ...
    // When navigation is active and mouse is disabled, decide on an arbitrary
    // position around the bottom left of the currently navigated item
    ...
    return ImFloor( ImClamp(
        ...
        visible_rect.Max ) ); // ImFloor() is important because non-integer
                              // mouse position application in back-end might be
...
float ImGui::GetNavInputAmount( ImGuiNavInput n, ImGuiInputReadMode mode ) {
    ...
    if ( mode == ImGuiInputReadMode_Down )
        return g.IO.NavInputs[ n ]; // Instant, read analog input (0.0f..1.0f,
                                    // as provided by user)

    const float t = g.IO.NavInputsDownDuration[ n ];
    if ( t < 0.0f &&
         mode ==
             ImGuiInputReadMode_Released ) // Return 1.0f when just released, no
                                           // repeat, ignore analog input.
        return ( g.IO.NavInputsDownDurationPrev[ n ] >= 0.0f ? 1.0f : 0.0f );
    if ( t < 0.0f )
        return 0.0f;
    if ( mode == ImGuiInputReadMode_Pressed ) // Return 1.0f when just pressed,
                                              // no repeat, ignore analog input.
        return ( t == 0.0f ) ? 1.0f : 0.0f;
    if ( mode == ImGuiInputReadMode_Repeat )
        return ( float )CalcTypematicPressedRepeatAmount(
            t, t - g.IO.DeltaTime, g.IO.KeyRepeatDelay * 0.80f,
            g.IO.KeyRepeatRate * 0.80f );
    if ( mode == ImGuiInputReadMode_RepeatSlow )
        return ( float )CalcTypematicPressedRepeatAmount(
            t, t - g.IO.DeltaTime, g.IO.KeyRepeatDelay * 1.00f,
...
ImVec2 ImGui::GetNavInputAmount2d( ImGuiNavDirSourceFlags dir_sources,
    ...
    if ( dir_sources & ImGuiNavDirSourceFlags_PadLStick )
        delta +=
            ImVec2( GetNavInputAmount( ImGuiNavInput_LStickRight, mode ) -
...
// NB: We modify rect_rel by the amount we scrolled for, so it is immediately
// updated.
static void NavScrollToBringItemIntoView( ImGuiWindow* window,
                                          const ImRect& item_rect ) {
    ImRect window_rect( window->InnerMainRect.Min - ImVec2( 1, 1 ),
                        window->InnerMainRect.Max + ImVec2( 1, 1 ) );
    // g.OverlayDrawList.AddRect(window_rect.Min, window_rect.Max,
    ...
    if ( window->ScrollbarX && item_rect.Min.x < window_rect.Min.x ) {
        window->ScrollTarget.x = item_rect.Min.x - window->Pos.x +
                                 window->Scroll.x - g.Style.ItemSpacing.x;
        window->ScrollTargetCenterRatio.x = 0.0f;
    } else if ( window->ScrollbarX && item_rect.Max.x >= window_rect.Max.x ) {
        window->ScrollTarget.x = item_rect.Max.x - window->Pos.x +
                                 window->Scroll.x + g.Style.ItemSpacing.x;
        window->ScrollTargetCenterRatio.x = 1.0f;
    }
    if ( item_rect.Min.y < window_rect.Min.y ) {
        window->ScrollTarget.y = item_rect.Min.y - window->Pos.y +
                                 window->Scroll.y - g.Style.ItemSpacing.y;
        window->ScrollTargetCenterRatio.y = 0.0f;
    } else if ( item_rect.Max.y >= window_rect.Max.y ) {
        window->ScrollTarget.y = item_rect.Max.y - window->Pos.y +
                                 window->Scroll.y + g.Style.ItemSpacing.y;
...
#undef NAV_MAP_KEY
    ...
    if ( g.NavInitResultId != 0 &&
        ...
        if ( g.NavInitRequestFromMove )
            SetNavIDWithRectRel( g.NavInitResultId, g.NavLayer,
                                 g.NavInitResultRectRel );
        else
    ...
    if ( g.NavMoveRequest &&
         ( g.NavMoveResultLocal.ID != 0 || g.NavMoveResultOther.ID != 0 ) )
        NavUpdateMoveResult();

    // When a forwarded move request failed, we restore the highlight that we
    // disabled during the forward frame
    ...
    if ( g.NavMousePosDirty && g.NavIdIsAlive ) {
        // Set mouse position given our knowledge of the navigated item position
        // from last frame
    ...
    // Store our return window (for returning from Layer 1 to Layer 0) and clear
    // it as soon as we step back in our own Layer 0
    if ( g.NavWindow )
    ...
    g.IO.NavActive = ( nav_keyboard_active || nav_gamepad_active ) &&
                     ...
                     !( g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs );
    g.IO.NavVisible =
        ( g.IO.NavActive && g.NavId != 0 && !g.NavDisableHighlight ) ||
    ...
    if ( IsNavInputPressed( ImGuiNavInput_Cancel,
        ...
        if ( g.ActiveId != 0 ) {
            ...
            // Clear NavLastId for popups but keep it for regular child window
            // so we can leave one and come back where we were
            if ( g.NavWindow &&
                 ( ( g.NavWindow->Flags & ImGuiWindowFlags_Popup ) ||
                   !( g.NavWindow->Flags & ImGuiWindowFlags_ChildWindow ) ) )
    ...
    if ( g.NavId != 0 && !g.NavDisableHighlight && !g.NavWindowingTarget &&
        ...
        bool activate_pressed =
            activate_down && IsNavInputPressed( ImGuiNavInput_Activate,
                                                ImGuiInputReadMode_Pressed );
    ...
    if ( g.NavMoveRequestForward == ImGuiNavForward_None ) {
        ...
        // other side of the window) (Preserve g.NavMoveRequestFlags,
        // g.NavMoveClipDir which were set by the NavMoveRequestForward()
        // function)
    ...
    if ( nav_keyboard_active )
        nav_scoring_rect_offset_y =
            NavUpdatePageUpPageDown( allowed_dir_flags );

    // If we initiate a movement request and have no current NavId, we initiate
    // a InitDefautRequest that will be used as a fallback if the direction
    ...
    if ( g.NavWindow &&
        ...
        const float scroll_speed =
            ImFloor( window->CalcFontSize() * 100 * g.IO.DeltaTime +
    ...
    // When we have manually scrolled (without using navigation) and NavId
    // becomes out of bounds, we project its bounding box to the visible area to
    // restart navigation within visible items
    if ( g.NavMoveRequest && g.NavMoveFromClampedRefRect && g.NavLayer == 0 ) {
        ImGuiWindow* window = g.NavWindow;
        ImRect window_rect_rel(
            window->InnerMainRect.Min - window->Pos - ImVec2( 1, 1 ),
            window->InnerMainRect.Max - window->Pos + ImVec2( 1, 1 ) );
    ...
    // items)
    ImRect nav_rect_rel =
        ( g.NavWindow && !g.NavWindow->NavRectRel[ g.NavLayer ].IsInverted() )
            ? g.NavWindow->NavRectRel[ g.NavLayer ]
            : ImRect( 0, 0, 0, 0 );
    g.NavScoringRectScreen = g.NavWindow
                                 ? ImRect( g.NavWindow->Pos + nav_rect_rel.Min,
                                           g.NavWindow->Pos + nav_rect_rel.Max )
                                 : GetViewportRect();
    g.NavScoringRectScreen.TranslateY( nav_scoring_rect_offset_y );
    g.NavScoringRectScreen.Min.x = ImMin( g.NavScoringRectScreen.Min.x + 1.0f,
    ...
    // g.OverlayDrawList.AddRect(g.NavScoringRectScreen.Min,
    // g.NavScoringRectScreen.Max, IM_COL32(255,200,0,255)); // [DEBUG]
    g.NavScoringCount = 0;
#if IMGUI_DEBUG_NAV_RECTS
    if ( g.NavWindow ) {
        for ( int layer = 0; layer < 2; layer++ )
            GetOverlayDrawList()->AddRect(
                g.NavWindow->Pos + g.NavWindow->NavRectRel[ layer ].Min,
                g.NavWindow->Pos + g.NavWindow->NavRectRel[ layer ].Max,
                IM_COL32( 255, 200, 0, 255 ) );
    } // [DEBUG]
    if ( g.NavWindow ) {
        ImU32 col = ( g.NavWindow->HiddenFrames == 0 )
                        ? IM_COL32( 255, 0, 255, 255 )
                        : IM_COL32( 255, 0, 0, 255 );
        ImVec2 p = NavCalcPreferredRefPos();
        char buf[ 32 ];
        ImFormatString( buf, 32, "%d", g.NavLayer );
        g.OverlayDrawList.AddCircleFilled( p, 3.0f, col );
        g.OverlayDrawList.AddText( NULL, 13.0f, p + ImVec2( 8, -4 ), col, buf );
    }
#endif
}
...
static void ImGui::NavUpdateMoveResult() {
    ...
    if ( g.NavLayer == 0 ) {
        ...
        // Estimate upcoming scroll so we can offset our result position so
        // mouse position can be applied immediately after in NavUpdate()
        ImVec2 next_scroll =
            CalcNextScrollFromScrollTargetAndClamp( result->Window, false );
        ImVec2 delta_scroll = result->Window->Scroll - next_scroll;
        result->RectRel.Translate( delta_scroll );

        // Also scroll parent window to keep us into view if necessary (we
        // could/should technically recurse back the whole the parent
        // hierarchy).
        if ( result->Window->Flags & ImGuiWindowFlags_ChildWindow )
            NavScrollToBringItemIntoView(
                result->Window->ParentWindow,
                ImRect( rect_abs.Min + delta_scroll,
                        rect_abs.Max + delta_scroll ) );
    }
...
static float ImGui::NavUpdatePageUpPageDown( int allowed_dir_flags ) {
    ...
    if ( g.NavMoveDir == ImGuiDir_None && g.NavWindow &&
        ...
        ImGuiWindow* window = g.NavWindow;
        bool page_up_held = IsKeyDown( g.IO.KeyMap[ ImGuiKey_PageUp ] ) &&
                            ( allowed_dir_flags & ( 1 << ImGuiDir_Up ) );
        bool page_down_held = IsKeyDown( g.IO.KeyMap[ ImGuiKey_PageDown ] ) &&
                              ( allowed_dir_flags & ( 1 << ImGuiDir_Down ) );
        if ( ( page_up_held && !page_down_held ) ||
             ( page_down_held && !page_up_held ) ) {
            if ( window->DC.NavLayerActiveMask == 0x00 &&
                ...
                if ( IsKeyPressed( g.IO.KeyMap[ ImGuiKey_PageUp ], true ) )
                    SetWindowScrollY(
                        window,
                        window->Scroll.y - window->InnerClipRect.GetHeight() );
                else if ( IsKeyPressed( g.IO.KeyMap[ ImGuiKey_PageDown ],
                                        true ) )
                    SetWindowScrollY(
                        window,
                        window->Scroll.y + window->InnerClipRect.GetHeight() );
                ...
                const float page_offset_y =
                    ImMax( 0.0f, window->InnerClipRect.GetHeight() -
                ...
                if ( IsKeyPressed( g.IO.KeyMap[ ImGuiKey_PageUp ], true ) ) {
                    ...
                    g.NavMoveDir =
                        ImGuiDir_Down; // Because our scoring rect is offset, we
                                       // intentionally request the opposite
                                       // direction (so we can always land on
                                       // the last item)
                    ...
                    nav_scoring_rect_offset_y = +page_offset_y;
                    g.NavMoveDir =
                        ImGuiDir_Up; // Because our scoring rect is offset, we
                                     // intentionally request the opposite
                                     // direction (so we can always land on the
                                     // last item)
...
{
    ImGuiContext& g = *GImGui;
    for ( int i = i_start; i >= 0 && i < g.Windows.Size && i != i_stop;
          i += dir )
...
static void NavUpdateWindowingHighlightWindow( int focus_change_dir ) {
    ...
    if ( !window_target )
        window_target = FindWindowNavFocusable(
            ( focus_change_dir < 0 ) ? ( g.Windows.Size - 1 ) : 0, i_current,
            focus_change_dir );
...
// Window management mode (hold to: change focus/move/resize, tap to: toggle
// menu layer)
static void ImGui::NavUpdateWindowing() {
    ...
    if ( g.NavWindowingTargetAnim && g.NavWindowingTarget == NULL ) {
        g.NavWindowingHighlightAlpha = ImMax(
            g.NavWindowingHighlightAlpha - g.IO.DeltaTime * 10.0f, 0.0f );
    ...
    bool start_windowing_with_gamepad =
        !g.NavWindowingTarget &&
        IsNavInputPressed( ImGuiNavInput_Menu, ImGuiInputReadMode_Pressed );
    bool start_windowing_with_keyboard =
        !g.NavWindowingTarget && g.IO.KeyCtrl &&
        IsKeyPressedMap( ImGuiKey_Tab ) &&
        ( g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard );
    if ( start_windowing_with_gamepad || start_windowing_with_keyboard )
        if ( ImGuiWindow* window =
                 g.NavWindow ? g.NavWindow
                             : FindWindowNavFocusable( g.Windows.Size - 1,
                                                       -INT_MAX, -1 ) ) {
    ...
    if ( g.NavWindowingTarget &&
        ...
        g.NavWindowingHighlightAlpha =
            ImMax( g.NavWindowingHighlightAlpha,
                   ImSaturate(
                       ( g.NavWindowingTimer - NAV_WINDOWING_HIGHLIGHT_DELAY ) /
                       0.05f ) );
        ...
        // on release (until then the window was merely rendered front-most)
    ...
    // so compare mouse validity to detect the common case of back-end clearing
    // releases all keys on ALT-TAB
    if ( ( g.ActiveId == 0 || g.ActiveIdAllowOverlap ) &&
    ...
    if ( g.NavWindowingTarget &&
        ...
        if ( g.NavInputSource == ImGuiInputSource_NavKeyboard &&
             !g.IO.KeyShift )
            move_delta = GetNavInputAmount2d( ImGuiNavDirSourceFlags_Keyboard,
                                              ImGuiInputReadMode_Down );
        if ( g.NavInputSource == ImGuiInputSource_NavGamepad )
            move_delta = GetNavInputAmount2d( ImGuiNavDirSourceFlags_PadLStick,
                                              ImGuiInputReadMode_Down );
    ...
    if ( apply_toggle_layer && g.NavWindow ) {
        ...
        g.NavDisableHighlight = false;
        g.NavDisableMouseHover = true;
        NavRestoreLayer( ( g.NavWindow->DC.NavLayerActiveMask & ( 1 << 1 ) )
                             ? ( g.NavLayer ^ 1 )
                             : 0 );
    }
}
...
static const char* GetFallbackWindowNameForWindowingList(
    ImGuiWindow* window ) {
    if ( window->Flags & ImGuiWindowFlags_Popup )
        return "(Popup)";
...
void ImGui::NextColumn() {
    ...
    window->DC.CursorPos.x =
        ( float )( int )( window->Pos.x + window->DC.Indent.x +
                          window->DC.ColumnsOffset.x );
...
static inline float GetColumnsRectHalfWidth() {
    return 4.0f;
}
...
static float GetDraggedColumnOffset( ImGuiColumnsSet* columns,
    ...
    float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x +
              GetColumnsRectHalfWidth() - window->Pos.x;
...
void ImGui::SetColumnOffset( int column_index, float offset ) {
    ...
    const bool preserve_width =
        !( columns->Flags & ImGuiColumnsFlags_NoPreserveWidths ) &&
    ...
    if ( !( columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow ) )
        offset = ImMin( offset,
                        columns->MaxX - g.Style.ColumnsMinSpacing *
                                            ( columns->Count - column_index ) );
    columns->Columns[ column_index ].OffsetNorm =
        PixelsToOffsetNorm( columns, offset - columns->MinX );
...
void ImGui::PushColumnClipRect( int column_index ) {
    ...
    PushClipRect( columns->Columns[ column_index ].ClipRect.Min,
                  columns->Columns[ column_index ].ClipRect.Max, false );
}
...
void ImGui::BeginColumns( const char* str_id,
    ...
    IM_ASSERT( window->DC.ColumnsSet ==
    ...
    const float content_region_width =
        ( window->SizeContentsExplicit.x != 0.0f )
            ? ( window->SizeContentsExplicit.x )
            : ( window->InnerClipRect.Max.x - window->Pos.x );
    columns->MinX = window->DC.Indent.x -
                    g.Style.ItemSpacing.x; // Lock our horizontal range
    columns->MaxX =
        ImMax( content_region_width - window->Scroll.x, columns->MinX + 1.0f );
    ...
    for ( int n = 0; n < columns_count; n++ ) {
        ...
        float clip_x1 =
            ImFloor( 0.5f + window->Pos.x + GetColumnOffset( n ) - 1.0f );
        float clip_x2 =
            ImFloor( 0.5f + window->Pos.x + GetColumnOffset( n + 1 ) - 1.0f );
...
void ImGui::EndColumns() {
    ...
    if ( !( columns->Flags & ImGuiColumnsFlags_GrowParentContentsSize ) )
        window->DC.CursorMaxPos.x =
            columns->StartMaxPosX; // Restore cursor max pos, as columns don't
                                   // grow parent

    // Draw columns borders and handle resize
    bool is_being_resized = false;
    if ( !( columns->Flags & ImGuiColumnsFlags_NoBorder ) &&
         !window->SkipItems ) {
        ...
        for ( int n = 1; n < columns->Count; n++ ) {
            ...
            const ImRect column_rect( ImVec2( x - column_hw, y1 ),
                                      ImVec2( x + column_hw, y2 ) );
            ...
            if ( !( columns->Flags & ImGuiColumnsFlags_NoResize ) ) {
                ...
                if ( held && !( columns->Columns[ n ].Flags &
                                ImGuiColumnsFlags_NoResize ) )
                    dragging_column = n;
            }

            // Draw column (we clip the Y boundaries CPU side because very long
            // triangles are mishandled by some GPU drivers.)
            const ImU32 col = GetColorU32( held      ? ImGuiCol_SeparatorActive
                                           : hovered ? ImGuiCol_SeparatorHovered
                                                     : ImGuiCol_Separator );
            const float xi = ( float )( int )x;
            window->DrawList->AddLine(
                ImVec2( xi, ImMax( y1 + 1.0f, window->ClipRect.Min.y ) ),
                ImVec2( xi, ImMin( y2, window->ClipRect.Max.y ) ), col );
        }
...
// [2018-03: This is currently the only public API, while we are working on
// making BeginColumns/EndColumns user-facing]
void ImGui::Columns( int columns_count, const char* id, bool border ) {
    ...
    if ( window->DC.ColumnsSet != NULL &&
         window->DC.ColumnsSet->Count == columns_count &&
         window->DC.ColumnsSet->Flags == flags )
        return;
...
bool ImGui::BeginDragDropSource( ImGuiDragDropFlags flags ) {
    ...
    if ( !( flags & ImGuiDragDropFlags_SourceExtern ) ) {
        ...
        if ( source_id == 0 ) {
            ...
            // ImGuiDragDropFlags_SourceAllowNullID flag, C) Swallow your
            // programmer pride.
            ...
            // on current ID stack + relative AABB of items in window. THE
            // IDENTIFIER WON'T SURVIVE ANY REPOSITIONING OF THE WIDGET, so if
            // your widget moves your dragging operation will be canceled. We
            // don't need to maintain/call ClearActiveID() as releasing the
            // button will early out this function and trigger !ActiveIdIsAlive.
            bool is_hovered = ( window->DC.LastItemStatusFlags &
    ...
    if ( source_drag_active ) {
        ...
        if ( !( flags & ImGuiDragDropFlags_SourceNoPreviewTooltip ) ) {
            // Target can request the Source to not display its tooltip (we use
            // a dedicated flag to make this request explicit) We unfortunately
            // can't just modify the source flags and skip the call to
            // BeginTooltip, as caller may be emitting contents.
            BeginTooltip();
            if ( g.DragDropActive && g.DragDropAcceptIdPrev &&
...
bool ImGui::BeginDragDropTargetCustom( const ImRect& bb, ImGuiID id ) {
    ...
    ImGuiWindow* window = g.CurrentWindow;
    if ( g.HoveredWindow == NULL ||
         window->RootWindow != g.HoveredWindow->RootWindow )
        return false;
    IM_ASSERT( id != 0 );
    if ( !IsMouseHoveringRect( bb.Min, bb.Max ) ||
...
const ImGuiPayload* ImGui::AcceptDragDropPayload( const char* type,
    ...
    if ( !( flags & ImGuiDragDropFlags_AcceptNoDrawDefaultRect ) &&
        ...
        window->DrawList->AddRect( r.Min, r.Max,
                                   GetColorU32( ImGuiCol_DragDropTarget ), 0.0f,
                                   ~0, 2.0f );
        if ( push_clip_rect )
            window->DrawList->PopClipRect();
    }
    ...
    payload.Delivery =
        ...
        !IsMouseDown(
            g.DragDropMouseButton ); // For extern drag sources affecting os
                                     // window focus, it's easier to just test
                                     // !IsMouseDown() instead of
                                     // IsMouseReleased()
    if ( !payload.Delivery &&
...
void ImGui::LogRenderedText( const ImVec2* ref_pos,
    ...
    const bool log_new_line =
        ref_pos && ( ref_pos->y > window->DC.LogLinePosY + 1 );
    if ( ref_pos )
    ...
    for ( ;; ) {
        ...
        if ( line_end != NULL &&
            ...
            if ( log_new_line || !is_first_line )
                LogText( IM_NEWLINE "%*s%.*s", tree_depth * 4, "", char_count,
                         text_remaining );
            else
...
void ImGui::LoadIniSettingsFromMemory( const char* ini_data, size_t ini_size ) {
    ...
    for ( char* line = buf; line < buf_end; line = line_end + 1 ) {
        ...
        if ( line[ 0 ] == '[' && line_end > line && line_end[ -1 ] == ']' ) {
            ...
            char* type_end =
                ( char* )( intptr_t )ImStrchrRange( type_start, name_end, ']' );
            const char* name_start =
                type_end ? ImStrchrRange( type_end + 1, name_end, '[' ) : NULL;
            if ( !type_end || !name_start ) {
                name_start =
                    type_start; // Import legacy entries that have no type
                type_start = "Window";
            } else {
            ...
            entry_data = entry_handler ? entry_handler->ReadOpenFn(
...
static void* SettingsHandlerWindow_ReadOpen( ImGuiContext*,
    ...
    ImGuiWindowSettings* settings =
        ImGui::FindWindowSettings( ImHash( name, 0 ) );
    if ( !settings )
...
static void SettingsHandlerWindow_ReadLine( ImGuiContext*,
    ...
    if ( sscanf( line, "Pos=%f,%f", &x, &y ) == 2 )
        ...
        settings->Size = ImMax( ImVec2( x, y ), GImGui->Style.WindowMinSize );
...
static void SettingsHandlerWindow_WriteAll( ImGuiContext* imgui_ctx,
    ...
    buf->reserve( buf->size() +
                  g.SettingsWindows.Size * 96 ); // ballpark reserve
    for ( int i = 0; i != g.SettingsWindows.Size; i++ ) {
        const ImGuiWindowSettings* settings = &g.SettingsWindows[ i ];
        if ( settings->Pos.x == FLT_MAX )
            continue;
        const char* name = settings->Name;
        if ( const char* p = strstr(
                 name, "###" ) ) // Skip to the "###" marker if any. We don't
                                 // skip past to match the behavior of GetID()
            name = p;
...
static void SetClipboardTextFn_DefaultImpl( void*, const char* text ) {
    ...
    HGLOBAL wbuf_handle = ::GlobalAlloc(
        GMEM_MOVEABLE, ( SIZE_T )wbuf_length * sizeof( ImWchar ) );
    if ( wbuf_handle == NULL ) {
        ::CloseClipboard();
        return;
    }
...
void ImGui::ShowMetricsWindow( bool* p_open ) {
    ...
    ImGui::Text( "%d active windows (%d visible)", io.MetricsActiveWindows,
                 io.MetricsRenderWindows );
    ImGui::Text( "%d allocations", io.MetricsActiveAllocations );
    ImGui::Checkbox( "Show clipping rectangles when hovering draw commands",
                     &show_draw_cmd_clip_rects );
    ImGui::Checkbox( "Ctrl shows window begin order",
                     &show_window_begin_order );

    ImGui::Separator();

    struct Funcs {
        static void NodeDrawList( ImGuiWindow* window,
                                  ImDrawList* draw_list,
                                  const char* label ) {
            ...
            ImDrawList* overlay_draw_list =
                GetOverlayDrawList(); // Render additional visuals into the
                                      // top-most draw list
            if ( window && IsItemHovered() )
                overlay_draw_list->AddRect( window->Pos,
            ...
            for ( const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin();
                ...
                if ( pcmd->UserCallback == NULL && pcmd->ElemCount == 0 )
                    continue;
                if ( pcmd->UserCallback ) {
                    ImGui::BulletText( "Callback %p, user_data %p",
                                       pcmd->UserCallback,
                ...
                bool pcmd_node_open = ImGui::TreeNode(
                    ...
                    "Draw %4d %s vtx, tex 0x%p, clip_rect "
                    "(%4.0f,%4.0f)-(%4.0f,%4.0f)",
                    pcmd->ElemCount,
                ...
                if ( show_draw_cmd_clip_rects && ImGui::IsItemHovered() ) {
                    ...
                    ImRect vtxs_rect;
                    for ( int i = elem_offset;
                          i < elem_offset + ( int )pcmd->ElemCount; i++ )
                        vtxs_rect.Add(
                            draw_list
                                ->VtxBuffer[ idx_buffer ? idx_buffer[ i ] : i ]
                                .pos );
                    clip_rect.Floor();
                ...
                ImGuiListClipper clipper(
                    ...
                    3 ); // Manually coarse clip our print out of individual
                         // vertices to save CPU, only items that may be
                         // visible.
                while ( clipper.Step() )
                    for ( int prim = clipper.DisplayStart,
                        ...
                        for ( int n = 0; n < 3; n++, vtx_i++ ) {
                            ImDrawVert& v =
                                draw_list->VtxBuffer[ idx_buffer
                                                          ? idx_buffer[ vtx_i ]
                                                          : vtx_i ];
                            triangles_pos[ n ] = v.pos;
                            buf_p += ImFormatString(
                                buf_p, ( int )( buf_end - buf_p ),
                                "%s %04d: pos (%8.2f,%8.2f), uv (%.6f,%.6f), "
                                "col %08X\n",
                        ...
                        if ( ImGui::IsItemHovered() ) {
                            ImDrawListFlags backup_flags =
                                overlay_draw_list->Flags;
                            overlay_draw_list->Flags &=
                                ~ImDrawListFlags_AntiAliasedLines; // Disable AA
                                                                   // on
                                                                   // triangle
                                                                   // outlines
                                                                   // at is more
                            ...
                            overlay_draw_list->AddPolyline(
                                triangles_pos, 3, IM_COL32( 255, 255, 0, 255 ),
                                true, 1.0f );
        ...
        static void NodeWindow( ImGuiWindow* window, const char* label ) {
            if ( !ImGui::TreeNode(
                     window, "%s '%s', %d @ 0x%p", label, window->Name,
            ...
            ImGui::BulletText(
                "Flags: 0x%08X (%s%s%s%s%s%s%s%s..)", flags,
                ...
                ( flags & ImGuiWindowFlags_AlwaysAutoResize )
                    ? "AlwaysAutoResize"
                    : "" );
            ImGui::BulletText( "Scroll: (%.2f/%.2f,%.2f/%.2f)",
                               window->Scroll.x, GetScrollMaxX( window ),
                               window->Scroll.y, GetScrollMaxY( window ) );
            ImGui::BulletText(
                ...
                ( window->Active || window->WasActive )
                    ...
                    : -1 );
            ImGui::BulletText(
                "Appearing: %d, Hidden: %d (Reg %d Resize %d), SkipItems: %d",
                window->Appearing, window->Hidden, window->HiddenFramesRegular,
                window->HiddenFramesForResize, window->SkipItems );
            ImGui::BulletText(
            ...
            if ( window->ColumnsStorage.Size > 0 &&
                 ImGui::TreeNode( "Columns", "Columns sets (%d)",
                                  window->ColumnsStorage.Size ) ) {
                for ( int n = 0; n < window->ColumnsStorage.Size; n++ ) {
                    const ImGuiColumnsSet* columns =
                        &window->ColumnsStorage[ n ];
                    if ( ImGui::TreeNode(
                        ...
                        ImGui::BulletText(
                            ...
                            columns->MaxX - columns->MinX, columns->MinX,
                            columns->MaxX );
                        for ( int column_n = 0;
                              column_n < columns->Columns.Size; column_n++ )
                            ImGui::BulletText(
    ...
    // Access private state, we are going to display the draw lists from last
    // frame
    ImGuiContext& g = *GImGui;
    ...
    if ( ImGui::TreeNode( "DrawList", "Active DrawLists (%d)",
        ...
        for ( int i = 0; i < g.DrawDataBuilder.Layers[ 0 ].Size; i++ )
            Funcs::NodeDrawList( NULL, g.DrawDataBuilder.Layers[ 0 ][ i ],
                                 "DrawList" );
        ImGui::TreePop();
    }
    if ( ImGui::TreeNode( "Popups", "Popups (%d)", g.OpenPopupStack.Size ) ) {
        for ( int i = 0; i < g.OpenPopupStack.Size; i++ ) {
            ...
            ImGui::BulletText(
                ...
                g.OpenPopupStack[ i ].PopupId, window ? window->Name : "NULL",
    ...
    if ( ImGui::TreeNode( "Internal state" ) ) {
        const char* input_source_names[] = { "None", "Mouse", "Nav",
                                             "NavKeyboard", "NavGamepad" };
        ...
        ImGui::Text(
            ...
            g.HoveredIdAllowOverlap ); // Data is "in-flight" so depending on
                                       // when the Metrics window is called we
                                       // may see current frame information or
                                       // not
        ImGui::Text(
            "ActiveId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d, Source: %s",
            g.ActiveId, g.ActiveIdPreviousFrame, g.ActiveIdTimer,
            g.ActiveIdAllowOverlap, input_source_names[ g.ActiveIdSource ] );
        ImGui::Text( "ActiveIdWindow: '%s'",
                     g.ActiveIdWindow ? g.ActiveIdWindow->Name : "NULL" );
        ImGui::Text( "MovingWindow: '%s'",
                     g.MovingWindow ? g.MovingWindow->Name : "NULL" );
        ImGui::Text( "NavWindow: '%s'",
                     g.NavWindow ? g.NavWindow->Name : "NULL" );
        ImGui::Text( "NavId: 0x%08X, NavLayer: %d", g.NavId, g.NavLayer );
        ImGui::Text( "NavInputSource: %s",
                     input_source_names[ g.NavInputSource ] );
        ImGui::Text( "NavActive: %d, NavVisible: %d", g.IO.NavActive,
                     g.IO.NavVisible );
        ImGui::Text( "NavActivateId: 0x%08X, NavInputId: 0x%08X",
                     g.NavActivateId, g.NavInputId );
        ImGui::Text( "NavDisableHighlight: %d, NavDisableMouseHover: %d",
    ...
    if ( g.IO.KeyCtrl && show_window_begin_order ) {
        for ( int n = 0; n < g.Windows.Size; n++ ) {
            ...
            overlay_draw_list->AddText( NULL, font_size, window->Pos,
                                        IM_COL32( 255, 255, 255, 255 ), buf );
        }
    }
    ImGui::End();
