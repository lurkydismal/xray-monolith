...
// Data Type helpers
static inline int DataTypeFormatString( char* buf,
                                        int buf_size,
...
void ImGui::TextUnformatted( const char* text, const char* text_end ) {
    ...
    if ( text_end - text > 2000 && !wrap_enabled ) {
        ...
        // which is unlikely to matter because we are likely the biggest and
            ...
            if ( line < text_end ) {
                ...
                while ( line < text_end ) {
                    ...
                    const ImVec2 line_size =
                        CalcTextSize( line, line_end, false );
...
void ImGui::TextWrappedV( const char* fmt, va_list args ) {
    bool need_wrap =
        ( GImGui->CurrentWindow->DC.TextWrapPos <
...
void ImGui::BulletTextV( const char* fmt, va_list args ) {
    ...
    const float text_base_offset_y = ImMax(
        0.0f,
        window->DC
            .CurrentLineTextBaseOffset ); // Latch before ItemSize changes it
    const float line_height =
        ImMax( ImMin( window->DC.CurrentLineSize.y,
...
bool ImGui::ButtonBehavior( const ImRect& bb,
    ...
    if ( ( flags & ImGuiButtonFlags_FlattenChildren ) &&
    ...
    if ( g.DragDropActive &&
        ...
        if ( IsItemHovered( ImGuiHoveredFlags_AllowWhenBlockedByActiveItem ) ) {
            ...
            if ( CalcTypematicPressedRepeatAmount(
                     ...
                     g.HoveredIdTimer + 0.0001f - g.IO.DeltaTime, 0.01f,
                     0.70f ) ) // FIXME: Our formula for
                               // CalcTypematicPressedRepeatAmount() is fishy
            {
                pressed = true;
                FocusWindow( window );
            }
    ...
    if ( hovered ) {
        if ( !( flags & ImGuiButtonFlags_NoKeyModifiers ) ||
            ...
            // PressedOnClickRelease  |  <on release>*  |  <on repeat> <on
            // repeat> .. (NOT on release)  <-- MOST COMMON! (*) only if both
            // click/release were over bounds PressedOnClick         |  <on
            // click>     |  <on click> <on repeat> <on repeat> ..
            // PressedOnRelease       |  <on release>   |  <on repeat> <on
            // repeat> .. (NOT on release) PressedOnDoubleClick   |  <on dclick>
            // |  <on dclick> <on repeat> <on repeat> ..
...
bool ImGui::ButtonEx( const char* label,
    ...
    if ( ( flags & ImGuiButtonFlags_AlignTextBaseLine ) &&
         style.FramePadding.y <
             window->DC
                 .CurrentLineTextBaseOffset ) // Try to vertically align buttons
                                              // that are smaller/have no
                                              // padding so that text baseline
                                              // matches (bit hacky, since it
                                              // shouldn't be a flag)
...
bool ImGui::ArrowButtonEx( const char* str_id,
    ...
    RenderArrow(
        bb.Min +
            ImVec2(
                ImMax( 0.0f, size.x - g.FontSize - g.Style.FramePadding.x ),
                ImMax( 0.0f, size.y - g.FontSize - g.Style.FramePadding.y ) ),
        dir );
...
bool ImGui::CloseButton( ImGuiID id, const ImVec2& pos, float radius ) {
    ...
    const ImRect bb( pos - ImVec2( radius, radius ),
                     pos + ImVec2( radius, radius ) );
...
bool ImGui::CollapseButton( ImGuiID id, const ImVec2& pos ) {
    ...
    ImU32 col = GetColorU32( ( held && hovered ) ? ImGuiCol_ButtonActive
                             : hovered           ? ImGuiCol_ButtonHovered
                                                 : ImGuiCol_Button );
    if ( hovered || held )
        window->DrawList->AddCircleFilled(
            bb.GetCenter() + ImVec2( 0.0f, -0.5f ), g.FontSize * 0.5f + 1.0f,
            col, 9 );
    RenderArrow( bb.Min + g.Style.FramePadding,
...
void ImGui::Scrollbar( ImGuiLayoutType direction ) {
    ...
    bool other_scrollbar =
        ( horizontal ? window->ScrollbarY : window->ScrollbarX );
    ...
    if ( horizontal )
        window_rounding_corners =
            ...
            ( other_scrollbar ? 0 : ImDrawCornerFlags_BotRight );
    else
        window_rounding_corners =
    ...
    window->DrawList->AddRectFilled(
        bb.Min, bb.Max, GetColorU32( ImGuiCol_ScrollbarBg ),
        window->WindowRounding, window_rounding_corners );
    bb.Expand( ImVec2(
        -ImClamp( ( float )( int )( ( bb.Max.x - bb.Min.x - 2.0f ) * 0.5f ),
                  0.0f, 3.0f ),
        -ImClamp( ( float )( int )( ( bb.Max.y - bb.Min.y - 2.0f ) * 0.5f ),
                  0.0f, 3.0f ) ) );
    ...
    float win_size_avail_v =
        ( horizontal ? window->SizeFull.x : window->SizeFull.y ) -
    ...
    IM_ASSERT( ImMax( win_size_contents_v, win_size_avail_v ) >
    ...
    const float win_size_v =
        ImMax( ImMax( win_size_contents_v, win_size_avail_v ), 1.0f );
    const float grab_h_pixels =
        ImClamp( scrollbar_size_v * ( win_size_avail_v / win_size_v ),
    ...
    if ( held && grab_h_norm < 1.0f ) {
        ...
        // after the calculation of SizeContents and before setting up our
        // starting position
        const float scroll_v_norm =
            ImSaturate( ( clicked_v_norm - *click_delta_to_grab_center_v -
                          grab_h_norm * 0.5f ) /
                        ( 1.0f - grab_h_norm ) );
        scroll_v =
            ( float )( int )( 0.5f + scroll_v_norm *
                                         scroll_max ); //(win_size_contents_v
                                                       //- win_size_v));
        if ( horizontal )
    ...
    if ( horizontal )
        grab_rect = ImRect(
            ...
            ImMin( ImLerp( bb.Min.x, bb.Max.x, grab_v_norm ) + grab_h_pixels,
                   window_rect.Max.x ),
            bb.Max.y );
    else
        grab_rect = ImRect(
            bb.Min.x, ImLerp( bb.Min.y, bb.Max.y, grab_v_norm ), bb.Max.x,
            ImMin( ImLerp( bb.Min.y, bb.Max.y, grab_v_norm ) + grab_h_pixels,
                   window_rect.Max.y ) );
...
void ImGui::Image( ImTextureID user_texture_id,
    ...
    if ( border_col.w > 0.0f ) {
        window->DrawList->AddRect( bb.Min, bb.Max, GetColorU32( border_col ),
                                   0.0f );
...
bool ImGui::ImageButton( ImTextureID user_texture_id,
    ...
    // prefixes. We could hash the size/uv to create a unique ID but that would
    // prevent the user from animating UV.
    ...
    if ( bg_col.w > 0.0f )
        window->DrawList->AddRectFilled( image_bb.Min, image_bb.Max,
                                         GetColorU32( bg_col ) );
    window->DrawList->AddImage( user_texture_id, image_bb.Min, image_bb.Max,
...
bool ImGui::Checkbox( const char* label, bool* v ) {
    ...
    if ( *v ) {
        ...
        RenderCheckMark( check_bb.Min + ImVec2( pad, pad ),
                         GetColorU32( ImGuiCol_CheckMark ),
                         check_bb.GetWidth() - pad * 2.0f );
    }
...
void ImGui::ProgressBar( float fraction,
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImRect bb( pos,
               pos + CalcItemSize( size_arg, CalcItemWidth(),
    ...
    if ( overlay_size.x > 0.0f )
        RenderTextClipped(
            ImVec2(
                ImClamp( fill_br.x + style.ItemSpacing.x, bb.Min.x,
...
void ImGui::NewLine() {
    ...
    if ( window->DC.CurrentLineSize.y >
...
void ImGui::AlignTextToFramePadding() {
    ...
    ImGuiContext& g = *GImGui;
    window->DC.CurrentLineSize.y = ImMax(
        window->DC.CurrentLineSize.y, g.FontSize + g.Style.FramePadding.y * 2 );
...
void ImGui::Separator() {
    ...
    ItemSize(
        ImVec2( 0.0f, 0.0f ) ); // NB: we don't provide our width so that it
...
void ImGui::VerticalSeparator() {
    ...
    const ImRect bb( ImVec2( window->DC.CursorPos.x, y1 ),
                     ImVec2( window->DC.CursorPos.x + 1.0f, y2 ) );
...
bool ImGui::BeginCombo( const char* label,
    ...
    // Always consume the SetNextWindowSizeConstraint() call in our early return
    // paths
    ImGuiContext& g = *GImGui;
    ...
    const ImRect value_bb( frame_bb.Min,
                           frame_bb.Max - ImVec2( arrow_size, 0.0f ) );
    const ImU32 frame_col =
        GetColorU32( hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg );
    RenderNavHighlight( frame_bb, id );
    if ( !( flags & ImGuiComboFlags_NoPreview ) )
        window->DrawList->AddRectFilled(
            frame_bb.Min, ImVec2( frame_bb.Max.x - arrow_size, frame_bb.Max.y ),
            frame_col, style.FrameRounding, ImDrawCornerFlags_Left );
    if ( !( flags & ImGuiComboFlags_NoArrowButton ) ) {
        window->DrawList->AddRectFilled(
            ImVec2( frame_bb.Max.x - arrow_size, frame_bb.Min.y ), frame_bb.Max,
            GetColorU32( ( popup_open || hovered ) ? ImGuiCol_ButtonHovered
                                                   : ImGuiCol_Button ),
            style.FrameRounding,
            ( w <= arrow_size ) ? ImDrawCornerFlags_All
                                : ImDrawCornerFlags_Right );
        RenderArrow( ImVec2( frame_bb.Max.x - arrow_size + style.FramePadding.y,
...
bool ImGui::Combo( const char* label,
    ...
    // Display items
    // FIXME-OPT: Use clipper (but we need to disable it on the appearing frame
    // to make sure our call to SetItemDefaultFocus() is processed)
...
#else
    { sizeof( ImS64 ), "%lld", "%lld" }, { sizeof( ImU64 ), "%llu", "%llu" },
#endif
...
static const char* PatchFormatStringFloatToInt( const char* fmt ) {
    ...
    const char* fmt_end = ImParseFormatFindEnd(
        fmt_start ); // Find end of format specifier, which itself is an
                     // exercise of confidence/recklessness (because snprintf is
                     // dependent on libc or user).
    if ( fmt_end > fmt_start && fmt_end[ -1 ] == 'f' ) {
        ...
        ImFormatString( g.TempBuffer, IM_ARRAYSIZE( g.TempBuffer ), "%.*s%%d%s",
                        ...
                        fmt_end ); // Honor leading and trailing decorations,
                                   // but lose alignment/precision.
        return g.TempBuffer;
#else
        IM_ASSERT(
            0 &&
...
static inline int DataTypeFormatString( char* buf,
    ...
    if ( data_type == ImGuiDataType_S32 ||
         data_type == ImGuiDataType_U32 ) // Signedness doesn't matter when
                                          // pushing the argument
        return ImFormatString( buf, buf_size, format,
                               *( const ImU32* )data_ptr );
    if ( data_type == ImGuiDataType_S64 ||
         data_type == ImGuiDataType_U64 ) // Signedness doesn't matter when
                                          // pushing the argument
        return ImFormatString( buf, buf_size, format,
                               *( const ImU64* )data_ptr );
...
// FIXME: Adding support for clamping on boundaries of the data type would be
// nice.
static void DataTypeApplyOp( ImGuiDataType data_type,
                             int op,
                             void* output,
    ...
    switch ( data_type ) {
        case ImGuiDataType_S32:
            if ( op == '+' )
                *( int* )output = *( const int* )arg1 + *( const int* )arg2;
            else if ( op == '-' )
                *( int* )output = *( const int* )arg1 - *( const int* )arg2;
            return;
        case ImGuiDataType_U32:
            if ( op == '+' )
                *( unsigned int* )output =
                    *( const unsigned int* )arg1 + *( const ImU32* )arg2;
            else if ( op == '-' )
                *( unsigned int* )output =
                    *( const unsigned int* )arg1 - *( const ImU32* )arg2;
            return;
...
static bool DataTypeApplyOpFromText( const char* buf,
    ...
    // value. Instead you can use +-100 to subtract from an existing value
    char op = buf[ 0 ];
    if ( op == '+' || op == '*' || op == '/' ) {
        buf++;
        while ( ImCharIsBlankA( *buf ) )
    ...
    if ( data_type == ImGuiDataType_S32 ) {
        ...
        // multipliers (*1.1), but constant always parsed as integer so we can
        // fit big integers (e.g. 2000000003) past float precision
        if ( op == '+' ) {
        ...
        // Assign constant
        // FIXME: We don't bother handling support for legacy operators since
        // they are a little too crappy. Instead we may implement a proper
        // expression evaluator in the future.
        sscanf( buf, format, data_ptr );
    } else if ( data_type == ImGuiDataType_Float ) {
        // For floats we have to ignore format with precision (e.g. "%.2f")
        // because sscanf doesn't take them in
        format = "%f";
    ...
    return memcmp( data_backup, data_ptr, GDataTypeInfo[ data_type ].Size ) !=
           0;
}
...
static float GetMinimumStepAtDecimalPrecision( int decimal_precision ) {
    ...
    if ( decimal_precision < 0 )
        return FLT_MIN;
    return ( decimal_precision >= 0 && decimal_precision < 10 )
...
TYPE ImGui::RoundScalarWithFormatT( const char* format,
                                    ImGuiDataType data_type,
                                    TYPE v ) {
...
bool ImGui::DragBehaviorT( ImGuiDataType data_type,
    ...
    if ( g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
        ...
        int decimal_precision = ( data_type == ImGuiDataType_Float ||
                                  data_type == ImGuiDataType_Double )
                                    ? ImParseFormatPrecision( format, 3 )
                                    : 0;
        adjust_delta = GetNavInputAmount2d( ImGuiNavDirSourceFlags_Keyboard |
                                                ImGuiNavDirSourceFlags_PadDPad,
                                            ImGuiInputReadMode_RepeatFast,
                                            1.0f / 10.0f, 10.0f )
                           .x;
        v_speed = ImMax(
            v_speed, GetMinimumStepAtDecimalPrecision( decimal_precision ) );
    }
    ...
    bool is_already_past_limits_and_pushing_outward =
        has_min_max && ( ( *v >= v_max && adjust_delta > 0.0f ) ||
    ...
    if ( is_power ) {
        ...
        FLOATTYPE v_old_norm_curved = ImPow(
            ( FLOATTYPE )( v_cur - v_min ) / ( FLOATTYPE )( v_max - v_min ),
            ( FLOATTYPE )1.0f / power );
        FLOATTYPE v_new_norm_curved =
            v_old_norm_curved + ( g.DragCurrentAccum / ( v_max - v_min ) );
        v_cur = v_min + ( TYPE )ImPow( ImSaturate( ( float )v_new_norm_curved ),
                                       power ) *
                            ( v_max - v_min );
    ...
    v_cur =
        RoundScalarWithFormatT< TYPE, SIGNEDTYPE >( format, data_type, v_cur );

    // Preserve remainder after rounding has been applied. This also allow slow
    // tweaking of values.
    g.DragCurrentAccumDirty = false;
    if ( is_power ) {
        FLOATTYPE v_cur_norm_curved = ImPow(
            ( FLOATTYPE )( v_cur - v_min ) / ( FLOATTYPE )( v_max - v_min ),
            ( FLOATTYPE )1.0f / power );
        g.DragCurrentAccum -=
            ( float )( v_cur_norm_curved - v_old_ref_for_accum_remainder );
    } else {
        g.DragCurrentAccum -=
    ...
    switch ( data_type ) {
        case ImGuiDataType_S32:
            return DragBehaviorT< ImS32, ImS32, float >(
                data_type, ( ImS32* )v, v_speed,
                v_min ? *( const ImS32* )v_min : IM_S32_MIN,
                v_max ? *( const ImS32* )v_max : IM_S32_MAX, format, power );
        case ImGuiDataType_U32:
            return DragBehaviorT< ImU32, ImS32, float >(
                data_type, ( ImU32* )v, v_speed,
                v_min ? *( const ImU32* )v_min : IM_U32_MIN,
                v_max ? *( const ImU32* )v_max : IM_U32_MAX, format, power );
        case ImGuiDataType_S64:
            return DragBehaviorT< ImS64, ImS64, double >(
                data_type, ( ImS64* )v, v_speed,
                v_min ? *( const ImS64* )v_min : IM_S64_MIN,
                v_max ? *( const ImS64* )v_max : IM_S64_MAX, format, power );
        case ImGuiDataType_U64:
            return DragBehaviorT< ImU64, ImS64, double >(
                data_type, ( ImU64* )v, v_speed,
                v_min ? *( const ImU64* )v_min : IM_U64_MIN,
                v_max ? *( const ImU64* )v_max : IM_U64_MAX, format, power );
        case ImGuiDataType_Float:
            return DragBehaviorT< float, float, float >(
                data_type, ( float* )v, v_speed,
                v_min ? *( const float* )v_min : -FLT_MAX,
                v_max ? *( const float* )v_max : FLT_MAX, format, power );
        case ImGuiDataType_Double:
            return DragBehaviorT< double, double, double >(
                data_type, ( double* )v, v_speed,
                v_min ? *( const double* )v_min : -DBL_MAX,
                v_max ? *( const double* )v_max : DBL_MAX, format, power );
        case ImGuiDataType_COUNT:
            break;
    }
    IM_ASSERT( 0 );
...
bool ImGui::DragScalar( const char* label,
    ...
    if ( power != 1.0f )
        IM_ASSERT( v_min != NULL &&
                   v_max != NULL ); // When using a power curve the drag needs
                                    // to have known bounds
    ...
    if ( tab_focus_requested ||
         ( hovered &&
           ( g.IO.MouseClicked[ 0 ] || g.IO.MouseDoubleClicked[ 0 ] ) ) ||
         g.NavActivateId == id ||
    ...
    if ( start_text_input ||
         ( g.ActiveId == id && g.ScalarAsInputTextId == id ) )
        return InputScalarAsWidgetReplacement( frame_bb, id, label, data_type,
                                               v, format );
...
bool ImGui::DragScalarN( const char* label,
    ...
    for ( int i = 0; i < components; i++ ) {
        PushID( i );
        value_changed |= DragScalar( "##v", data_type, v, v_speed, v_min, v_max,
                                     format, power );
...
bool ImGui::DragFloatRange2( const char* label,
    ...
    value_changed |= DragFloat(
        ...
        ( v_min >= v_max ) ? FLT_MAX : v_max, format_max ? format_max : format,
        power );
    PopItemWidth();
    SameLine( 0, g.Style.ItemInnerSpacing.x );
...
float ImGui::SliderCalcRatioFromValueT( ImGuiDataType data_type,
    ...
    const bool is_power =
        ( power != 1.0f ) && ( data_type == ImGuiDataType_Float ||
                               data_type == ImGuiDataType_Double );
    ...
    if ( is_power ) {
        if ( v_clamped < 0.0f ) {
            const float f =
                1.0f - ( float )( ( v_clamped - v_min ) /
                                  ( ImMin( ( TYPE )0, v_max ) - v_min ) );
            return ( 1.0f - ImPow( f, 1.0f / power ) ) * linear_zero_pos;
        } else {
            const float f =
                ( float )( ( v_clamped - ImMax( ( TYPE )0, v_min ) ) /
                           ( v_max - ImMax( ( TYPE )0, v_min ) ) );
            return linear_zero_pos +
                   ImPow( f, 1.0f / power ) * ( 1.0f - linear_zero_pos );
        }
    }

    // Linear slider
    return ( float )( ( FLOATTYPE )( v_clamped - v_min ) /
                      ( FLOATTYPE )( v_max - v_min ) );
}

// FIXME: Move some of the code into SliderBehavior(). Current responsability is
// larger than what the equivalent DragBehaviorT<> does, we also do some
// rendering, etc.
template < typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE >
bool ImGui::SliderBehaviorT( const ImRect& bb,
    ...
    if ( !is_decimal &&
        ...
        grab_sz =
            ImMax( ( float )( slider_sz / ( v_range + 1 ) ),
    ...
    const float slider_usable_pos_min =
        ( is_horizontal ? bb.Min.x : bb.Min.y ) + grab_padding + grab_sz * 0.5f;
    const float slider_usable_pos_max =
        ( is_horizontal ? bb.Max.x : bb.Max.y ) - grab_padding - grab_sz * 0.5f;

    // For power curve sliders that cross over sign boundary we want the curve
    // to be symmetric around 0.0f
    float linear_zero_pos; // 0.0->1.0f
    if ( is_power && v_min * v_max < 0.0f ) {
        // Different sign
        const FLOATTYPE linear_dist_min_to_0 =
            ImPow( v_min >= 0 ? ( FLOATTYPE )v_min : -( FLOATTYPE )v_min,
                   ( FLOATTYPE )1.0f / power );
        const FLOATTYPE linear_dist_max_to_0 =
            ImPow( v_max >= 0 ? ( FLOATTYPE )v_max : -( FLOATTYPE )v_max,
                   ( FLOATTYPE )1.0f / power );
        linear_zero_pos =
    ...
    if ( g.ActiveId == id ) {
        ...
        if ( g.ActiveIdSource == ImGuiInputSource_Mouse ) {
            ...
            if ( g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated ) {
                ...
                clicked_t = SliderCalcRatioFromValueT< TYPE, FLOATTYPE >(
                ...
                if ( ( decimal_precision > 0 ) || is_power ) {
                    delta /= 100.0f; // Gamepad/keyboard tweak speeds in % of
                                     // slider bounds
                    ...
                    if ( ( v_range >= -100.0f && v_range <= 100.0f ) ||
                        ...
                        delta = ( ( delta < 0.0f ) ? -1.0f : +1.0f ) /
                                ( float )v_range; // Gamepad/keyboard tweak
                ...
                if ( ( clicked_t >= 1.0f && delta > 0.0f ) ||
                     ( clicked_t <= 0.0f &&
                       delta <
                           0.0f ) ) // This is to avoid applying the saturation
        ...
        if ( set_new_value ) {
            ...
            if ( is_power ) {
                ...
                if ( is_decimal ) {
                    ...
                    // For integer values we want the clicking position to match
                    // the grab box so we round above This code is carefully
                    // tuned to work with large values (e.g. high ranges of U64)
            ...
            // Round to user desired precision based on format string
            v_new = RoundScalarWithFormatT< TYPE, SIGNEDTYPE >(
    ...
    float grab_t = SliderCalcRatioFromValueT< TYPE, FLOATTYPE >(
        data_type, *v, v_min, v_max, power, linear_zero_pos );
    if ( !is_horizontal )
        grab_t = 1.0f - grab_t;
...
// For 32-bits and larger types, slider bounds are limited to half the natural
    ...
    switch ( data_type ) {
        ...
        case ImGuiDataType_Float:
            ...
            return SliderBehaviorT< float, float, float >(
                bb, id, data_type, ( float* )v, *( const float* )v_min,
        ...
        case ImGuiDataType_Double:
            ...
            return SliderBehaviorT< double, double, double >(
                bb, id, data_type, ( double* )v, *( const double* )v_min,
...
bool ImGui::SliderScalar( const char* label,
    ...
    ImRect grab_bb;
    const bool value_changed =
        SliderBehavior( frame_bb, id, data_type, v, v_min, v_max, format, power,
...
bool ImGui::SliderScalarN( const char* label,
    ...
    for ( int i = 0; i < components; i++ ) {
        PushID( i );
        value_changed |=
            SliderScalar( "##v", data_type, v, v_min, v_max, format, power );
        SameLine( 0, g.Style.ItemInnerSpacing.x );
        PopID();
        PopItemWidth();
        v = ( void* )( ( char* )v + type_size );
...
const char* ImParseFormatTrimDecorations( const char* fmt,
    ...
    ImStrncpy( buf, fmt_start,
               ImMin( ( int )( fmt_end + 1 - fmt_start ), buf_size ) );
    return buf;
}
...
bool ImGui::InputScalarAsWidgetReplacement( const ImRect& bb,
    ...
    // Our replacement widget will override the focus ID (registered previously
    // to allow for a TAB focus to happen) On the first frame,
    // g.ScalarAsInputTextId == 0, then on subsequent frames it becomes == id
    ...
    bool value_changed = InputTextEx( label, data_buf, IM_ARRAYSIZE( data_buf ),
                                      bb.GetSize(), flags );
    if ( g.ScalarAsInputTextId ==
         0 ) // First frame we started displaying the InputText widget
    {
        IM_ASSERT( g.ActiveId ==
                   id ); // InputText ID expected to match the Slider ID
        g.ScalarAsInputTextId = g.ActiveId;
        SetHoveredID( id );
    }
    if ( value_changed )
        return DataTypeApplyOpFromText( data_buf,
                                        g.InputTextState.InitialText.Data,
                                        data_type, data_ptr, NULL );
    return false;
}

// NB: format here must be a simple "%xx" format string with no prefix/suffix
// (unlike the Drag/Slider functions "format" argument)
bool ImGui::InputScalar( const char* label,
    ...
    char buf[ 64 ];
    DataTypeFormatString( buf, IM_ARRAYSIZE( buf ), data_type, data_ptr,
                          format );
    ...
    if ( step != NULL ) {
        ...
        if ( InputText(
                 ...
                 extra_flags ) ) // PushId(label) + "" gives us the expected ID
                                 // from outside point of view
            value_changed =
                DataTypeApplyOpFromText( buf, g.InputTextState.InitialText.Data,
...
bool ImGui::InputScalarN( const char* label,
    ...
    for ( int i = 0; i < components; i++ ) {
        PushID( i );
        value_changed |= InputScalar( "##v", data_type, v, step, step_fast,
                                      format, extra_flags );
        SameLine( 0, g.Style.ItemInnerSpacing.x );
        PopID();
        PopItemWidth();
...
// Prefer using "const char* format" directly, which is more flexible and
// consistent with other API.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::InputFloat( const char* label,
                        float* v,
                        float step,
    ...
    char format[ 16 ] = "%f";
    if ( decimal_precision >= 0 )
        ImFormatString( format, IM_ARRAYSIZE( format ), "%%.%df",
                        decimal_precision );
...
bool ImGui::InputText( const char* label,
    ...
    return InputTextEx( label, buf, ( int )buf_size, ImVec2( 0, 0 ), flags,
                        callback, user_data );
}
...
static ImVec2 InputTextCalcTextSizeW( const ImWchar* text_begin,
    ...
    while ( s < text_end ) {
        ...
        const float char_width =
            font->GetCharAdvance( ( unsigned short )c ) * scale;
...
void ImGuiInputTextCallbackData::InsertChars( int pos,
    ...
    if ( new_text_len + BufTextLen >= BufSize ) {
        ...
        // buffer, hence the midly similar code (until we remove the U16 buffer
        // alltogether!)
        ImGuiContext& g = *GImGui;
...
static bool InputTextFilterCharacter( unsigned int* p_char,
    ...
    if ( c >= 0xE000 &&
         c <= 0xF8FF ) // Filter private Unicode range. I don't imagine anybody
                       // would want to input them. GLFW on OSX seems to send
...
// misc/stl/imgui_stl.h (FIXME: Rather messy function partly because we are
// doing UTF8 > u16 > UTF8 conversions on the go to more easily handle
// stb_textedit calls. Ideally we should stay in UTF-8 all the time. See
...
bool ImGui::InputTextEx( const char* label,
    ...
    if ( is_multiline ) {
        ...
        draw_window->DC.NavLayerActiveMaskNext |=
            draw_window->DC
                .NavLayerCurrentMask; // This is to ensure that EndChild() will
                                      // display a navigation highlight
    ...
    // NB: we are only allowed to access 'edit_state' if we are the active
    // widget.
    ImGuiInputTextState& edit_state = g.InputTextState;
    ...
    if ( focus_requested || user_clicked || user_scrolled ||
        ...
        if ( g.ActiveId != id ) {
            // Start edition
            // Take a copy of the initial buffer value (both in original UTF-8
            // format and converted to wchar) From the moment we focused we are
            // ignoring the content of 'buf' (unless we are in read-only mode)
            const int prev_len_w = edit_state.CurLenW;
            const int init_buf_len = ( int )strlen( buf );
            edit_state.TextW.resize(
                buf_size +
                1 ); // wchar count <= UTF-8 count. we use +1 to make sure that
                     // .Data isn't NULL so it doesn't crash.
            edit_state.InitialText.resize(
                init_buf_len + 1 ); // UTF-8. we use +1 to make sure that .Data
            ...
            edit_state.CurLenW = ImTextStrFromUtf8(
                edit_state.TextW.Data, buf_size, buf, NULL, &buf_end );
            edit_state.CurLenA =
                ( int )( buf_end -
            ...
            // same widget
            // FIXME: We should probably compare the whole buffer to be on the
            // safety side. Comparing buf (utf8) and edit_state.Text (wchar).
            const bool recycle_state =
                ( edit_state.ID == id ) && ( prev_len_w == edit_state.CurLenW );
            if ( recycle_state ) {
                // Recycle existing cursor/selection/undo stack but clamp
                // position Note a single mouse click will override the
                // cursor/position immediately by calling stb_textedit_click
                // handler.
                edit_state.CursorClamp();
            } else {
                edit_state.ID = id;
                edit_state.ScrollX = 0.0f;
                stb_textedit_initialize_state( &edit_state.StbState,
                                               !is_multiline );
    ...
    if ( g.ActiveId == id ) {
        if ( !is_editable && !g.ActiveIdIsJustActivated ) {
            ...
            const char* buf_end = NULL;
            edit_state.CurLenW =
                ImTextStrFromUtf8( edit_state.TextW.Data, edit_state.TextW.Size,
                                   buf, NULL, &buf_end );
        ...
        if ( select_all ||
            ...
            // Double-click select a word only, OS X style (by simulating
            // keystrokes)
            edit_state.OnKeyPressed( STB_TEXTEDIT_K_WORDLEFT );
            edit_state.OnKeyPressed( STB_TEXTEDIT_K_WORDRIGHT |
                                     STB_TEXTEDIT_K_SHIFT );
        } else if ( io.MouseClicked[ 0 ] && !edit_state.SelectedAllMouseLock ) {
            if ( hovered ) {
                stb_textedit_click( &edit_state, &edit_state.StbState, mouse_x,
                                    mouse_y );
                edit_state.CursorAnimReset();
            }
        } else if ( io.MouseDown[ 0 ] && !edit_state.SelectedAllMouseLock &&
                    ( io.MouseDelta.x != 0.0f || io.MouseDelta.y != 0.0f ) ) {
            stb_textedit_drag( &edit_state, &edit_state.StbState, mouse_x,
                               mouse_y );
            edit_state.CursorAnimReset();
        ...
        if ( io.InputCharacters[ 0 ] ) {
            // Process text input (before we check for Return because using some
            // IME will effectively send a Return?) We ignore CTRL inputs, but
            ...
            if ( !ignore_inputs && is_editable && !user_nav_input_start )
                for ( int n = 0; n < IM_ARRAYSIZE( io.InputCharacters ) &&
                                 io.InputCharacters[ n ];
                      n++ ) {
                    // Insert character if they pass filtering
                    unsigned int c = ( unsigned int )io.InputCharacters[ n ];
                    if ( InputTextFilterCharacter( &c, flags, callback,
                                                   callback_user_data ) )
    ...
    if ( g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id ) {
        ...
        const bool is_shortcut_key =
            ...
            !io.KeyAlt && !io.KeyShift; // OS X style: Shortcuts using Cmd/Super
                                        // instead of Ctrl
        ...
        const bool is_shift_key_only =
            io.KeyShift && !io.KeyCtrl && !io.KeyAlt && !io.KeySuper;
        ...
        const bool is_copy =
            ...
            !is_password && ( !is_multiline || edit_state.HasSelection() );
        const bool is_paste =
        ...
        if ( IsKeyPressedMap( ImGuiKey_LeftArrow ) ) {
            ...
            edit_state.OnKeyPressed( io.KeyCtrl
                                         ? STB_TEXTEDIT_K_TEXTSTART | k_mask
                                         : STB_TEXTEDIT_K_LINESTART | k_mask );
        } else if ( IsKeyPressedMap( ImGuiKey_End ) ) {
            edit_state.OnKeyPressed( io.KeyCtrl
                                         ? STB_TEXTEDIT_K_TEXTEND | k_mask
            ...
            if ( !is_multiline || ( ctrl_enter_for_new_line && !io.KeyCtrl ) ||
                ...
                if ( InputTextFilterCharacter( &c, flags, callback,
                    ...
                    IsKeyPressedMap( ImGuiKey_Tab ) && !io.KeyCtrl &&
            ...
            edit_state.OnKeyPressed( is_undo ? STB_TEXTEDIT_K_UNDO
                                             : STB_TEXTEDIT_K_REDO );
            ...
            if ( io.SetClipboardTextFn ) {
                ...
                ImTextStrToUtf8(
                    edit_state.TempBuffer.Data, edit_state.TempBuffer.Size,
                    edit_state.TextW.Data + ib, edit_state.TextW.Data + ie );
            ...
            if ( const char* clipboard = GetClipboardText() ) {
                ...
                ImWchar* clipboard_filtered = ( ImWchar* )ImGui::MemAlloc(
                    ( clipboard_len + 1 ) * sizeof( ImWchar ) );
                ...
                for ( const char* s = clipboard; *s; ) {
                    ...
                    if ( c == 0 )
                        break;
                    if ( c >= 0x10000 ||
                         !InputTextFilterCharacter( &c, flags, callback,
                                                    callback_user_data ) )
                        continue;
                ...
                if ( clipboard_filtered_len >
                     0 ) // If everything was filtered, ignore the pasting
                         // operation
                {
    ...
    if ( g.ActiveId == id ) {
        ...
        if ( cancel_edit ) {
            ...
            if ( is_editable &&
                 strcmp( buf, edit_state.InitialText.Data ) != 0 ) {
        ...
        // ImGuiInputTextFlags_EnterReturnsTrue would fail. Also this allows the
        // user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue
        // without maintaining any user-side storage.
        ...
        if ( apply_edit_back_to_user_buffer ) {
            ...
            if ( is_editable ) {
                ...
                ImTextStrToUtf8( edit_state.TempBuffer.Data,
                                 edit_state.TempBuffer.Size,
                                 edit_state.TextW.Data, NULL );
            }
            ...
            if ( ( flags & ( ImGuiInputTextFlags_CallbackCompletion |
                ...
                } else if ( ( flags & ImGuiInputTextFlags_CallbackHistory ) !=
                                0 &&
                ...
                if ( event_flag ) {
                    ...
                    const int utf8_cursor_pos = callback_data.CursorPos =
                        ImTextCountUtf8BytesFromStr(
                            text, text + edit_state.StbState.cursor );
                    ...
                    IM_ASSERT( callback_data.Buf ==
                               edit_state.TempBuffer
                                   .Data ); // Invalid to modify those fields
                    IM_ASSERT( callback_data.BufSize ==
                               edit_state.BufCapacityA );
                    ...
                    if ( callback_data.SelectionStart !=
                        ...
                        edit_state.StbState.select_start =
                    ...
                    if ( callback_data.BufDirty ) {
                        ...
                        if ( callback_data.BufTextLen >
                             ...
                             is_resizable )
                            edit_state.TextW.resize(
                                edit_state.TextW.Size +
                                ( callback_data.BufTextLen -
                                  backup_current_text_length ) );
                        edit_state.CurLenW = ImTextStrFromUtf8(
                            edit_state.TextW.Data, edit_state.TextW.Size,
            ...
            if ( is_editable &&
                 strcmp( edit_state.TempBuffer.Data, buf ) != 0 ) {
        ...
        if ( apply_new_text ) {
            ...
            // If the underlying buffer resize was denied or not carried to the
            // next frame, apply_new_text_length+1 may be >= buf_size.
            ImStrncpy( buf, edit_state.TempBuffer.Data,
    ...
    const char* buf_display =
        ( g.ActiveId == id && is_editable ) ? edit_state.TempBuffer.Data : buf;
    buf = NULL;
    ...
    if ( g.ActiveId == id || is_currently_scrolling ) {
        ...
        {
            // Count lines + find lines numbers straddling 'cursor' and
            ...
            if ( edit_state.StbState.select_start !=
                 edit_state.StbState.select_end ) {
            ...
            if ( searches_result_line_number[ 1 ] >= 0 ) {
                ...
                select_start_offset.y =
                    searches_result_line_number[ 1 ] * g.FontSize;
            }
        ...
        if ( edit_state.CursorFollow ) {
            ...
            if ( !( flags & ImGuiInputTextFlags_NoHorizontalScroll ) ) {
                ...
                if ( cursor_offset.x < edit_state.ScrollX )
                    edit_state.ScrollX = ( float )( int )ImMax(
                        0.0f, cursor_offset.x - scroll_increment_x );
                else if ( cursor_offset.x - size.x >= edit_state.ScrollX )
                    edit_state.ScrollX =
                        ( float )( int )( cursor_offset.x - size.x +
                                          scroll_increment_x );
            } else {
                edit_state.ScrollX = 0.0f;
            }
            ...
            if ( is_multiline ) {
                ...
                draw_window->DC.CursorPos.y +=
                    ( draw_window->Scroll.y -
                      scroll_y ); // To avoid a frame of lag
            ...
            for ( const ImWchar* p = text_selected_begin;
                ...
                if ( rect_pos.y < clip_rect.y ) {
                    ...
                    ImVec2 rect_size = InputTextCalcTextSizeW(
                        p, text_selected_end, &p, NULL, true );
                    if ( rect_size.x <= 0.0f )
                        rect_size.x =
                            ( float )( int )( g.Font->GetCharAdvance(
                                                  ( unsigned short )' ' ) *
                                              0.50f ); // So we can see selected
                                                       // empty lines
                    ImRect rect(
                        rect_pos + ImVec2( 0.0f, bg_offy_up - g.FontSize ),
                        rect_pos + ImVec2( rect_size.x, bg_offy_dn ) );
                    rect.ClipWith( clip_rect );
                    if ( rect.Overlaps( clip_rect ) )
                        draw_window->DrawList->AddRectFilled(
                            rect.Min, rect.Max, bg_color );
                }
        ...
        if ( is_multiline || buf_display_len < buf_display_max_length )
            draw_window->DrawList->AddText(
                g.Font, g.FontSize, render_pos - render_scroll,
                GetColorU32( ImGuiCol_Text ), buf_display,
                buf_display + buf_display_len, 0.0f,
                is_multiline ? NULL : &clip_rect );

        // Draw blinking cursor
        bool cursor_is_visible =
            ( !g.IO.ConfigInputTextCursorBlink ) ||
            ( g.InputTextState.CursorAnim <= 0.0f ) ||
            ImFmod( g.InputTextState.CursorAnim, 1.20f ) <= 0.80f;
        ...
        if ( is_editable )
            g.PlatformImePos = ImVec2( cursor_screen_pos.x - 1,
                                       cursor_screen_pos.y - g.FontSize );
    } else {
        // Render text only
        const char* buf_end = NULL;
        if ( is_multiline )
            text_size = ImVec2( size.x, InputTextCalcTextLenAndLineCount(
                                            buf_display, &buf_end ) *
                                            g.FontSize ); // We don't need width
        else
            buf_end = buf_display + strlen( buf_display );
        if ( is_multiline ||
             ( buf_end - buf_display ) < buf_display_max_length )
            draw_window->DrawList->AddText(
                g.Font, g.FontSize, render_pos, GetColorU32( ImGuiCol_Text ),
                buf_display, buf_end, 0.0f, is_multiline ? NULL : &clip_rect );
    }
    ...
    if ( is_multiline ) {
        Dummy(
            text_size +
            ImVec2( 0.0f,
                    g.FontSize ) ); // Always add room to scroll an extra line
        EndChildFrame();
        EndGroup();
    }
...
// Left-click on colored square to open color picker. Right-click to open option
...
bool ImGui::ColorEdit4( const char* label,
    ...
    if ( flags & ImGuiColorEditFlags_NoInputs )
        flags = ( flags & ( ~ImGuiColorEditFlags__InputsMask ) ) |
                ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_NoOptions;
    ...
    int i[ 4 ] = {
        IM_F32_TO_INT8_UNBOUND( f[ 0 ] ), IM_F32_TO_INT8_UNBOUND( f[ 1 ] ),
    ...
    if ( ( flags & ( ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV ) ) !=
        ...
        const float w_item_one = ImMax(
            1.0f,
            ( float )( int )( ( w_items_all - ( style.ItemInnerSpacing.x ) *
                                                  ( components - 1 ) ) /
                              ( float )components ) );
        ...
        const bool hide_prefix =
            ( w_item_one <= CalcTextSize( ( flags & ImGuiColorEditFlags_Float )
                                              ? "M:0.000"
                                              : "M:000" )
                                .x );
        ...
        for ( int n = 0; n < components; n++ ) {
            ...
            if ( flags & ImGuiColorEditFlags_Float )
                ...
                value_changed |=
                    DragInt( ids[ n ], &i[ n ], 1.0f, 0, hdr ? 0 : 255,
        ...
        PopItemWidth();
        PopItemWidth();
    } else if ( ( flags & ImGuiColorEditFlags_HEX ) != 0 &&
                ( flags & ImGuiColorEditFlags_NoInputs ) == 0 ) {
        ...
        if ( InputText( "##Text", buf, IM_ARRAYSIZE( buf ),
            ...
            if ( alpha )
                sscanf( p, "%02X%02X%02X%02X", ( unsigned int* )&i[ 0 ],
                        ...
                        ( unsigned int* )&i[ 3 ] ); // Treat at unsigned (%X is
                                                    // unsigned)
            else
    ...
    if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) ) {
        ...
        if ( ColorButton( "##ColorButton", col_v4, flags ) ) {
            if ( !( flags & ImGuiColorEditFlags_NoPicker ) ) {
                ...
                OpenPopup( "picker" );
                SetNextWindowPos( window->DC.LastItemRect.GetBL() +
        ...
        if ( BeginPopup( "picker" ) ) {
            ...
            ImGuiColorEditFlags picker_flags_to_forward =
                ImGuiColorEditFlags__DataTypeMask |
                ImGuiColorEditFlags__PickerMask | ImGuiColorEditFlags_HDR |
            ...
            ImGuiColorEditFlags picker_flags =
                ...
                ImGuiColorEditFlags__InputsMask | ImGuiColorEditFlags_NoLabel |
...
// avoiding that. I spent a non reasonable amount of time trying to getting this
...
void ImGui::RenderColorRectWithAlphaCheckerboard( ImVec2 p_min,
    ...
    if ( ( ( col & IM_COL32_A_MASK ) >> IM_COL32_A_SHIFT ) < 0xFF ) {
        ImU32 col_bg1 = GetColorU32(
            ImAlphaBlendColor( IM_COL32( 204, 204, 204, 255 ), col ) );
        ImU32 col_bg2 = GetColorU32(
            ImAlphaBlendColor( IM_COL32( 128, 128, 128, 255 ), col ) );
...
bool ImGui::ColorPicker4( const char* label,
    ...
    if ( !( flags & ImGuiColorEditFlags__PickerMask ) )
        flags |= ( ( g.ColorEditOptions & ImGuiColorEditFlags__PickerMask )
                 ...
                 ImGuiColorEditFlags__PickerMask;
    IM_ASSERT( ImIsPowerOfTwo( (
        int )( flags & ImGuiColorEditFlags__PickerMask ) ) ); // Check that only
                                                              // 1 is selected
    if ( !( flags & ImGuiColorEditFlags_NoOptions ) )
    ...
    if ( flags & ImGuiColorEditFlags_PickerHueWheel ) {
        ...
        InvisibleButton(
            "hsv",
            ImVec2( sv_picker_size + style.ItemInnerSpacing.x + bars_width,
                    sv_picker_size ) );
        if ( IsItemActive() ) {
    ...
    if ( value_changed_h || value_changed_sv )
        ColorConvertHSVtoRGB( H >= 1.0f ? H - 10 * 1e-6f : H,
                              S > 0.0f ? S : 10 * 1e-6f, V > 0.0f ? V : 1e-6f,
                              col[ 0 ], col[ 1 ], col[ 2 ] );
    ...
    if ( ( flags & ImGuiColorEditFlags_NoInputs ) == 0 ) {
        ...
        ImGuiColorEditFlags sub_flags_to_forward =
            ImGuiColorEditFlags__DataTypeMask | ImGuiColorEditFlags_HDR |
        ...
        if ( flags & ImGuiColorEditFlags_RGB ||
             ( flags & ImGuiColorEditFlags__InputsMask ) == 0 )
            if ( ColorEdit4( "##rgb", col,
                             sub_flags | ImGuiColorEditFlags_RGB ) ) {
                // FIXME: Hackily differenciating using the DragInt (ActiveId !=
        ...
        if ( flags & ImGuiColorEditFlags_HSV ||
             ( flags & ImGuiColorEditFlags__InputsMask ) == 0 )
            value_changed |=
                ColorEdit4( "##hsv", col, sub_flags | ImGuiColorEditFlags_HSV );
        if ( flags & ImGuiColorEditFlags_HEX ||
             ( flags & ImGuiColorEditFlags__InputsMask ) == 0 )
            value_changed |=
                ColorEdit4( "##hex", col, sub_flags | ImGuiColorEditFlags_HEX );
        PopItemWidth();
    }
    ...
    ImU32 col32_no_alpha =
        ColorConvertFloat4ToU32( ImVec4( col[ 0 ], col[ 1 ], col[ 2 ], 1.0f ) );
    ...
    if ( flags & ImGuiColorEditFlags_PickerHueWheel ) {
        // Render Hue Wheel
        const float aeps = 1.5f / wheel_r_outer; // Half a pixel arc length in
        ...
        for ( int n = 0; n < 6; n++ ) {
            ...
            draw_list->PathArcTo( wheel_center,
                                  ( wheel_r_inner + wheel_r_outer ) * 0.5f, a0,
                                  a1, segment_per_arc );
            ...
            ShadeVertsLinearColorGradientKeepAlpha(
                ...
                gradient_p1, hue_colors[ n ], hue_colors[ n + 1 ] );
        }
        ...
        draw_list->AddCircle( hue_cursor_pos, hue_cursor_rad + 1,
                              ...
                              hue_cursor_segments );
        draw_list->AddCircle( hue_cursor_pos, hue_cursor_rad, IM_COL32_WHITE,
                              hue_cursor_segments );

        // Render SV triangle (rotated according to hue)
        ImVec2 tra = wheel_center +
                     ImRotate( triangle_pa, cos_hue_angle, sin_hue_angle );
        ImVec2 trb = wheel_center +
                     ImRotate( triangle_pb, cos_hue_angle, sin_hue_angle );
        ImVec2 trc = wheel_center +
        ...
        draw_list->AddTriangle( tra, trb, trc, IM_COL32( 128, 128, 128, 255 ),
                                1.5f );
        ...
        draw_list->AddRectFilledMultiColor(
            picker_pos, picker_pos + ImVec2( sv_picker_size, sv_picker_size ),
            IM_COL32_BLACK_TRANS, IM_COL32_BLACK_TRANS, IM_COL32_BLACK,
            IM_COL32_BLACK );
        RenderFrameBorder(
            picker_pos, picker_pos + ImVec2( sv_picker_size, sv_picker_size ),
            0.0f );
        sv_cursor_pos.x = ImClamp(
            ( float )( int )( picker_pos.x + ImSaturate( S ) * sv_picker_size +
                              0.5f ),
            picker_pos.x + 2,
            picker_pos.x + sv_picker_size -
                2 ); // Sneakily prevent the circle to stick out too much
        sv_cursor_pos.y = ImClamp(
            ( float )( int )( picker_pos.y +
                              ImSaturate( 1 - V ) * sv_picker_size + 0.5f ),
        ...
        for ( int i = 0; i < 6; ++i )
            draw_list->AddRectFilledMultiColor(
                ...
                hue_colors[ i ], hue_colors[ i ], hue_colors[ i + 1 ],
                hue_colors[ i + 1 ] );
        float bar0_line_y =
            ( float )( int )( picker_pos.y + H * sv_picker_size + 0.5f );
        RenderFrameBorder(
    ...
    draw_list->AddCircleFilled( sv_cursor_pos, sv_cursor_rad, col32_no_alpha,
                                12 );
    draw_list->AddCircle( sv_cursor_pos, sv_cursor_rad + 1,
...
// display it next to the button, but only in the tooltip.
bool ImGui::ColorButton( const char* desc_id,
    ...
    if ( g.ActiveId == id && !( flags & ImGuiColorEditFlags_NoDragDrop ) &&
         BeginDragDropSource() ) {
        if ( flags & ImGuiColorEditFlags_NoAlpha )
            SetDragDropPayload( IMGUI_PAYLOAD_TYPE_COLOR_3F, &col,
                                sizeof( float ) * 3, ImGuiCond_Once );
        else
            SetDragDropPayload( IMGUI_PAYLOAD_TYPE_COLOR_4F, &col,
...
void ImGui::SetColorEditOptions( ImGuiColorEditFlags flags ) {
    ...
    IM_ASSERT( ImIsPowerOfTwo(
        ( int )( flags &
                 ImGuiColorEditFlags__InputsMask ) ) ); // Check only 1 option
                                                        // is selected
    IM_ASSERT( ImIsPowerOfTwo(
        ( int )( flags &
                 ImGuiColorEditFlags__DataTypeMask ) ) ); // Check only 1 option
                                                          // is selected
    IM_ASSERT( ImIsPowerOfTwo(
        ( int )( flags &
                 ImGuiColorEditFlags__PickerMask ) ) ); // Check only 1 option
                                                        // is selected
...
void ImGui::ColorEditOptionsPopup( const float* col,
    ...
    if ( allow_opt_inputs ) {
        if ( RadioButton( "RGB", ( opts & ImGuiColorEditFlags_RGB ) != 0 ) )
            opts = ( opts & ~ImGuiColorEditFlags__InputsMask ) |
                   ImGuiColorEditFlags_RGB;
        if ( RadioButton( "HSV", ( opts & ImGuiColorEditFlags_HSV ) != 0 ) )
            opts = ( opts & ~ImGuiColorEditFlags__InputsMask ) |
                   ImGuiColorEditFlags_HSV;
        if ( RadioButton( "HEX", ( opts & ImGuiColorEditFlags_HEX ) != 0 ) )
            opts = ( opts & ~ImGuiColorEditFlags__InputsMask ) |
    ...
    if ( BeginPopup( "Copy" ) ) {
        ...
        char buf[ 64 ];
        ImFormatString(
            buf, IM_ARRAYSIZE( buf ), "(%.3ff, %.3ff, %.3ff, %.3ff)", col[ 0 ],
            col[ 1 ], col[ 2 ],
            ( flags & ImGuiColorEditFlags_NoAlpha ) ? 1.0f : col[ 3 ] );
        if ( Selectable( buf ) )
            SetClipboardText( buf );
        ImFormatString( buf, IM_ARRAYSIZE( buf ), "(%d,%d,%d,%d)", cr, cg, cb,
                        ca );
        if ( Selectable( buf ) )
            SetClipboardText( buf );
        if ( flags & ImGuiColorEditFlags_NoAlpha )
            ImFormatString( buf, IM_ARRAYSIZE( buf ), "0x%02X%02X%02X", cr, cg,
                            cb );
        else
            ImFormatString( buf, IM_ARRAYSIZE( buf ), "0x%02X%02X%02X%02X", cr,
                            cg, cb, ca );
        if ( Selectable( buf ) )
            SetClipboardText( buf );
        EndPopup();
...
void ImGui::ColorPickerOptionsPopup( const float* ref_col,
    ...
    if ( allow_opt_picker ) {
        ImVec2 picker_size(
            ...
            ImMax(
                g.FontSize * 8 -
                    ( ImGui::GetFrameHeight() + g.Style.ItemInnerSpacing.x ),
        ...
        for ( int picker_type = 0; picker_type < 2; picker_type++ ) {
            ...
            if ( ImGui::Selectable( "##selectable", false, 0,
                                    picker_size ) ) // By default, Selectable()
                                                    // is closing popup
                g.ColorEditOptions =
                    ( g.ColorEditOptions & ~ImGuiColorEditFlags__PickerMask ) |
                    ( picker_flags & ImGuiColorEditFlags__PickerMask );
            ImGui::SetCursorScreenPos( backup_pos );
            ImVec4 dummy_ref_col;
            memcpy(
                &dummy_ref_col.x, ref_col,
                sizeof( float ) *
                    ( picker_flags & ImGuiColorEditFlags_NoAlpha ? 3 : 4 ) );
            ImGui::ColorPicker4( "##dummypicker", &dummy_ref_col.x,
                                 picker_flags );
            ImGui::PopID();
        }
        ImGui::PopItemWidth();
    ...
    if ( allow_opt_alpha_bar ) {
        if ( allow_opt_picker )
            ImGui::Separator();
        ImGui::CheckboxFlags( "Alpha Bar", ( unsigned int* )&g.ColorEditOptions,
...
bool ImGui::TreeNodeBehavior( ImGuiID id,
    ...
    if ( display_frame ) {
        ...
        frame_bb.Min.x -=
            ( float )( int )( window->WindowPadding.x * 0.5f ) - 1;
        frame_bb.Max.x +=
            ( float )( int )( window->WindowPadding.x * 0.5f ) - 1;
    }
    ...
    // ItemSpacing (Ideally we'd want to add a flag for the user to specify if
    // we want the hit test to be done up to the right side of the content or
    // not)
    ...
    // Store a flag for the current depth to tell if we will allow closing this
    // node when navigating one of its child. For this purpose we essentially
    // compare if g.NavIdIsAlive went from 0 to 1 between TreeNode() and
    // TreePop(). This is currently only support 32 level deep and we are fine
    // with (1 << Depth) overflowing into a zero.
    if ( is_open && !g.NavIdIsAlive &&
         ( flags & ImGuiTreeNodeFlags_NavLeftJumpsBackHere ) &&
         !( flags & ImGuiTreeNodeFlags_NoTreePushOnOpen ) )
        window->DC.TreeDepthMayJumpToParentOnPop |=
    ...
    if ( !( flags & ImGuiTreeNodeFlags_Leaf ) ) {
        bool toggled = false;
        if ( pressed ) {
            toggled = !( flags & ( ImGuiTreeNodeFlags_OpenOnArrow |
                                   ImGuiTreeNodeFlags_OpenOnDoubleClick ) ) ||
                      ( g.NavActivateId == id );
            if ( flags & ImGuiTreeNodeFlags_OpenOnArrow )
                toggled |= IsMouseHoveringRect(
                               interact_bb.Min,
                               ImVec2( interact_bb.Min.x + text_offset_x,
                                       interact_bb.Max.y ) ) &&
                           ( !g.NavDisableMouseHover );
            if ( flags & ImGuiTreeNodeFlags_OpenOnDoubleClick )
                toggled |= g.IO.MouseDoubleClicked[ 0 ];
            if ( g.DragDropActive &&
                 is_open ) // When using Drag and Drop "hold to open" we keep
                           // the node highlighted after opening, but never
                           // close it again.
                toggled = false;
        }
    ...
    const ImU32 col = GetColorU32( ( held && hovered ) ? ImGuiCol_HeaderActive
                                   : hovered           ? ImGuiCol_HeaderHovered
                                                       : ImGuiCol_Header );
    const ImVec2 text_pos =
        frame_bb.Min + ImVec2( text_offset_x, text_base_offset_y );
    if ( display_frame ) {
        // Framed type
        RenderFrame( frame_bb.Min, frame_bb.Max, col, true,
                     style.FrameRounding );
        RenderNavHighlight( frame_bb, id, ImGuiNavHighlightFlags_TypeThin );
        RenderArrow( frame_bb.Min + ImVec2( padding.x, text_base_offset_y ),
                     is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f );
        if ( g.LogEnabled ) {
            // NB: '##' is normally used to hide text (as a library-wide
            // feature), so we need to specify the text range to make sure the
        ...
        if ( flags & ImGuiTreeNodeFlags_Bullet )
            RenderBullet( frame_bb.Min +
                          ImVec2( text_offset_x * 0.5f,
                                  g.FontSize * 0.50f + text_base_offset_y ) );
        else if ( !( flags & ImGuiTreeNodeFlags_Leaf ) )
            RenderArrow(
                frame_bb.Min + ImVec2( padding.x, g.FontSize * 0.15f +
                                                      text_base_offset_y ),
...
void ImGui::TreePop() {
    ...
    if ( g.NavMoveDir == ImGuiDir_Left && g.NavWindow == window &&
        ...
        if ( g.NavIdIsAlive && ( window->DC.TreeDepthMayJumpToParentOnPop &
                                 ( 1 << window->DC.TreeDepth ) ) ) {
...
bool ImGui::CollapsingHeader( const char* label,
    ...
    if ( p_open ) {
        ...
        // into user accessible helpers to add extra buttons on title bars,
        // headers, etc.
        ImGuiContext& g = *GImGui;
        ImGuiItemHoveredDataBackup last_item_backup;
        float button_radius = g.FontSize * 0.5f;
        ImVec2 button_center = ImVec2(
            ImMin( window->DC.LastItemRect.Max.x, window->ClipRect.Max.x ) -
                g.Style.FramePadding.x - button_radius,
            window->DC.LastItemRect.GetCenter().y );
        if ( CloseButton( window->GetID( ( void* )( intptr_t )( id + 1 ) ),
                          button_center, button_radius ) )
            *p_open = false;
...
// other text or image. But you need to make sure the ID is unique, e.g. enclose
// calls in PushID/PopID or use ##unique_id.
bool ImGui::Selectable( const char* label,
    ...
    if ( ( flags & ImGuiSelectableFlags_SpanAllColumns ) &&
         window->DC.ColumnsSet ) // FIXME-OPT: Avoid if vertically clipped.
        PopClipRect();
    ...
    float w_draw =
        ImMax( label_size.x, window->Pos.x + max_x - window_padding.x -
                                 window->DC.CursorPos.x );
    ImVec2 size_draw( ( size_arg.x != 0 &&
                        !( flags & ImGuiSelectableFlags_DrawFillAvailWidth ) )
                          ? size_arg.x
                          : w_draw,
    ...
    // Selectables are tightly packed together, we extend the box to cover
    ...
    // Hovering selectable with mouse updates NavId accordingly so navigation
    // can be resumed with gamepad/keyboard (this doesn't happen on most
    // widgets)
    if ( pressed || hovered )
    ...
    RenderTextClipped( bb_inner.Min, bb.Max, label, NULL, &label_size,
                       ImVec2( 0.0f, 0.0f ) );
    if ( flags & ImGuiSelectableFlags_Disabled )
        PopStyleColor();
...
bool ImGui::ListBoxHeader( const char* label, const ImVec2& size_arg ) {
    ...
    // Size default to hold ~7 items. Fractional number of items helps seeing
    // that we can scroll down/up without looking at scrollbar.
    ImVec2 size = CalcItemSize(
        size_arg, CalcItemWidth(),
...
bool ImGui::ListBoxHeader( const char* label,
                           ...
                           int height_in_items ) {
    // Size default to hold ~7 items. Fractional number of items helps seeing
    // that we can scroll down/up without looking at scrollbar. We don't add
    // +0.40f if items_count <= height_in_items. It is slightly dodgy, because
    // it means a dynamic list of items will make the widget resize occasionally
    // when it crosses that size. I am expecting that someone will come and
    // complain about this behavior in a remote future, then we can advise on a
    // better solution.
...
void ImGui::ListBoxFooter() {
    ...
    // full size in LastItemRect) We call SameLine() to restore DC.CurrentLine*
    // data
    SameLine();
...
bool ImGui::ListBox( const char* label,
    ...
    // of different or variable sizes you can create a custom version of
    // ListBox() in your code without using the clipper.
    ImGuiContext& g = *GImGui;
    bool value_changed = false;
    ...
    if ( values_count > 0 ) {
        ...
        for ( int n = 0; n < res_w; n++ ) {
            ...
            const ImVec2 tp1 = ImVec2(
                t1, 1.0f - ImSaturate( ( v1 - scale_min ) * inv_scale ) );
...
bool ImGui::BeginMenuBar() {
    ...
    ImRect clip_rect(
        ImFloor( bar_rect.Min.x + 0.5f ),
        ImFloor( bar_rect.Min.y + window->WindowBorderSize + 0.5f ),
        ImFloor(
            ImMax( bar_rect.Min.x, bar_rect.Max.x - window->WindowRounding ) +
            0.5f ),
    ...
    if ( NavMoveRequestButNoResultYet() &&
        ...
        if ( nav_earliest_child->ParentWindow == window &&
            ...
            IM_ASSERT( window->DC.NavLayerActiveMaskNext &
                       0x02 ); // Sanity check
            FocusWindow( window );
            SetNavIDWithRectRel( window->NavLastIds[ 1 ], 1,
                                 window->NavRectRel[ 1 ] );
            g.NavLayer = 1;
...
bool ImGui::BeginMenu( const char* label, bool enabled ) {
    ...
    if ( menuset_is_open )
        g.NavWindow =
            window; // Odd hack to allow hovering across menus of a same
                    // menu-set (otherwise we wouldn't be able to hover parent)

    // The reference position stored in popup_pos will be used by Begin() to
    // find a suitable position for the child menu (using
    ...
    if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
        ...
        popup_pos =
            ImVec2( pos.x - window->WindowPadding.x,
                    pos.y - style.FramePadding.y + window->MenuBarHeight() );
        window->DC.CursorPos.x +=
            ( float )( int )( style.ItemSpacing.x * 0.5f );
        ...
        window->DC.CursorPos.x +=
            ( float )( int )( style.ItemSpacing.x *
                              ( -1.0f +
                                0.5f ) ); // -1 spacing to compensate the
                                          // spacing added when Selectable() did
                                          // a SameLine(). It would also work to
                                          // call SameLine() ourselves after the
                                          // PopStyleVar().
    } else {
        // Menu inside a menu
        ...
        if ( !enabled )
            PushStyleColor( ImGuiCol_Text,
                            g.Style.Colors[ ImGuiCol_TextDisabled ] );
        RenderArrow( pos + ImVec2( window->MenuColumns.Pos[ 2 ] + extra_w +
                                       g.FontSize * 0.30f,
                                   0.0f ),
                     ImGuiDir_Right );
        if ( !enabled )
                                   ...
                                   // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if ( g.HoveredWindow == window &&
             g.OpenPopupStack.Size > g.CurrentPopupStack.Size &&
             g.OpenPopupStack[ g.CurrentPopupStack.Size ].ParentWindow ==
                 window &&
             !( window->Flags & ImGuiWindowFlags_MenuBar ) ) {
            if ( ImGuiWindow* next_window =
                ...
                float extra = ImClamp( ImFabs( ta.x - tb.x ) * 0.30f, 5.0f,
                ...
                tb.y =
                    ...
                    ImMax(
                        ( tb.y - extra ) - ta.y,
                        -100.0f ); // triangle is maximum 200 high to limit the
                ...
                // IM_COL32(128,0,0,128)); window->DrawList->PopClipRect(); //
                // Debug
            }
        }
    ...
    if ( !menu_is_open && want_open &&
         g.OpenPopupStack.Size > g.CurrentPopupStack.Size ) {
        // Don't recycle same menu level in the same frame, first close the
...
void ImGui::EndMenu() {
    // Nav: When a left move request _within our child menu_ failed, close the
    // menu. A menu doesn't close itself because EndMenuBar() wants the catch
    // the last Left<>Right inputs. However, it means that with the current
    // code, a BeginMenu() from outside another menu or a menu-bar won't be
...
bool ImGui::MenuItem( const char* label,
    ...
    if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
        ...
        // Note that in this situation we render neither the shortcut neither
        ...
        float extra_w = ImMax( 0.0f, GetContentRegionAvail().x - w );
        pressed = Selectable( label, false,
                              flags | ImGuiSelectableFlags_DrawFillAvailWidth,
                              ImVec2( w, 0.0f ) );
        if ( shortcut_size.x > 0.0f ) {
            PushStyleColor( ImGuiCol_Text,
                            g.Style.Colors[ ImGuiCol_TextDisabled ] );
            RenderText(
                pos + ImVec2( window->MenuColumns.Pos[ 1 ] + extra_w, 0.0f ),
                shortcut, NULL, false );
            PopStyleColor();
        }
        if ( selected )
            RenderCheckMark(
                pos + ImVec2( window->MenuColumns.Pos[ 2 ] + extra_w +
                                  g.FontSize * 0.40f,
