...
struct ImGuiInputTextState; // Internal state of the currently focused/edited
                            // text input box
struct ImGuiItemHoveredDataBackup; // Backup and restore IsItemHovered()
                                   // internal data
struct ImGuiMenuColumns;     // Simple column measurement, currently used for
                             // MenuItem() only
struct ImGuiNavMoveResult;   // Result of a directional navigation move query
                             // result
...
struct ImGuiWindowTempData; // Temporary storage for one window (that's the data
                            ...
                            // frame)
struct ImGuiWindowSettings; // Storage for window settings stored in .ini file
                            // (we keep one of those even if the actual window
...
typedef int ImGuiNavHighlightFlags; // -> enum ImGuiNavHighlightFlags_ // Flags:
                                    // for RenderNavHighlight()
typedef int ImGuiNavDirSourceFlags; // -> enum ImGuiNavDirSourceFlags_ // Flags:
                                    // for GetNavInputAmount2d()
...
typedef int ImGuiSeparatorFlags; // -> enum ImGuiSeparatorFlags_    // Flags:
                                 // for Separator() - internal
typedef int ImGuiSliderFlags; // -> enum ImGuiSliderFlags_       // Flags: for
                              // SliderBehavior()
...
#ifdef _WIN32
#define IM_NEWLINE \
    "\r\n" // Play it nice with Windows users (2018/05 news: Microsoft announced
           // that Notepad will finally display Unix-style carriage returns!)
#else
#define IM_NEWLINE "\n"
#endif
...
ImHash( const void* data,
        int data_size,
        ImU32 seed = 0 ); // Pass data_size==0 for zero-terminated strings
...
IMGUI_API const ImWchar* ImStrbolW(
...
#endif
// - ImMin/ImMax/ImClamp/ImLerp/ImSwap are used by widgets which support for
// variety of types: signed/unsigned int/long long float/double, using templates
...
// 1D vector (this odd construct is used to facilitate the transition between 1D
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_PressedOnClickRelease =
        1 << 1, // return true on click + release on same item [DEFAULT if no
                // PressedOn* flag is set]
    ...
    ImGuiButtonFlags_AllowItemOverlap =
        1
        << 6, // require previous frame HoveredId to either match id or be null
              // before being usable, use along with SetItemAllowOverlap()
    ImGuiButtonFlags_DontClosePopups =
        1
        << 7, // disable automatically closing parent popup on press // [UNUSED]
    ImGuiButtonFlags_Disabled = 1 << 8, // disable interactions
    ImGuiButtonFlags_AlignTextBaseLine =
        1
        << 9, // vertically align button to match text baseline - ButtonEx()
    ...
    ImGuiButtonFlags_NoHoldingActiveID =
        1 << 11, // don't set ActiveId while holding the mouse
                 // (ImGuiButtonFlags_PressedOnClick only)
    ImGuiButtonFlags_PressedOnDragDropHold =
        1 << 12, // press when held into while we are drag and dropping another
...
enum ImGuiColumnsFlags_ {
    ...
    ImGuiColumnsFlags_GrowParentContentsSize =
        1 << 4 // (WIP) Restore pre-1.51 behavior of extending the parent window
...
enum ImGuiNavMoveFlags_ {
    ...
    ImGuiNavMoveFlags_AlsoScoreVisibleSet =
        1 << 5 // Store alternate result in NavMoveResultLocalVisibleSet that
...
struct IMGUI_API ImGuiInputTextState {
    ...
    ImVector< char > InitialText; // backup of end-user buffer at the time of
                                  // focus (in UTF-8, unaltered)
    ImVector< char > TempBuffer;  // temporary buffer for callback and other
                                  // other operations. size=capacity.
    int CurLenA, CurLenW; // we need to maintain our buffer length in both UTF-8
                          // and wchar format.
...
struct ImGuiPopupRef {
    ...
    ImGuiID OpenParentId; // Set on OpenPopup(), we need this to differenciate
...
struct ImGuiNextWindowData {
    ...
    float BgAlphaVal;
    ImVec2 MenuBarOffsetMinVal; // This is not exposed publicly, so we don't
                                // clear it.

    ImGuiNextWindowData() {
        PosCond = SizeCond = ContentSizeCond = CollapsedCond =
...
struct ImGuiContext {
    ...
    ImGuiWindow*
        HoveredRootWindow; // Will catch mouse inputs (for focus/move only)
    ...
    bool ActiveIdAllowOverlap;  // Active widget allows another widget to steal
                                ...
                                // but not always)
    bool ActiveIdHasBeenEdited; // Was the value associated to the widget Edited
                                // over the course of the Active state.
    bool ActiveIdPreviousFrameIsAlive;
    bool ActiveIdPreviousFrameHasBeenEdited;
    int ActiveIdAllowNavDirFlags; // Active widget allows using directional
                                  // navigation (e.g. can activate a button and
                                  // move away from it)
    ...
    ImGuiWindow* MovingWindow; // Track the window we clicked on (in order to
                               // preserve focus). The actually window that is
    ...
    ImVector< ImGuiStyleMod >
        StyleModifiers;            // Stack for PushStyleVar()/PopStyleVar()
    ImVector< ImFont* > FontStack; // Stack for PushFont()/PopFont()
    ImVector< ImGuiPopupRef >
        OpenPopupStack; // Which popups are open (persistent)
    ImVector< ImGuiPopupRef >
        CurrentPopupStack; // Which level of BeginPopup() we are in (reset every
                           // frame)
    ...
    ImGuiID NavActivateId;  // ~~ (g.ActiveId == 0) &&
    ...
    ImGuiID NavActivateDownId; // ~~ IsNavInputDown(ImGuiNavInput_Activate) ?
                               // NavId : 0
    ImGuiID
        NavActivatePressedId; // ~~ IsNavInputPressed(ImGuiNavInput_Activate) ?
                              // NavId : 0
    ...
    ImRect NavScoringRectScreen; // Rectangle used for scoring, in screen space.
                                 // Based of window->DC.NavRefRectRel[],
                                 // modified for directional navigation scoring.
    int NavScoringCount;         // Metrics for debugging
    ImGuiWindow*
        NavWindowingTarget; // When selecting a window (holding
                            // Menu+FocusPrev/Next, or equivalent of CTRL-TAB)
                            // this window is temporarily displayed front-most.
    ImGuiWindow*
    ...
    int NavLayer; // Layer we are navigating on. For now the system is
                  // hard-coded for 0=main contents and 1=menu/title bar, may
                  // expose layers later.
    int NavIdTabCounter; // == NavWindow->DC.FocusIdxTabCounter at time of NavId
                         // processing
    bool NavIdIsAlive;   // Nav widget has been seen this frame ~~ NavRefRectRel
                         // is valid
    ...
    bool NavDisableHighlight;  // When user starts using mouse, we hide
                               ...
                               // still available, which is why
                               // NavDisableHighlight isn't always !=
                               // NavDisableMouseHover)
    bool NavDisableMouseHover; // When user starts using gamepad/keyboard, we
                               // hide mouse hovering highlight until mouse is
                               // touched again.
    ...
    bool NavMoveFromClampedRefRect; // Set by manual scrolling, if we scroll to
                                    // a point where NavId isn't visible we
                                    // reset navigation from visible items
    bool NavMoveRequest;            // Move request for this frame
    ImGuiNavMoveFlags NavMoveRequestFlags;
    ImGuiNavForward
        NavMoveRequestForward; // None / ForwardQueued / ForwardActive (this is
                               // used to navigate sibling parent menus from a
                               // child menu)
    ImGuiDir NavMoveDir,
        NavMoveDirLast; // Direction of the move request (left/right/up/down),
    ...
    ImGuiNavMoveResult
        NavMoveResultLocalVisibleSet; // Best move request candidate within
                                      // NavWindow that are mostly visible (when
                                      // using
                                      // ImGuiNavMoveFlags_AlsoScoreVisibleSet
                                      // flag)
    ImGuiNavMoveResult
    ...
    // Render
    ImDrawData DrawData; // Main ImDrawData instance to pass render information
                         // to the user
    ImDrawDataBuilder DrawDataBuilder;
    float DimBgRatio; // 0.0..1.0 animation when fading in a dimming background
                      // (for modal window and CTRL+TAB list)
    ImDrawList
        OverlayDrawList; // Optional software render of mouse cursors, if
                         // io.MouseDrawCursor is set + a few debug overlays
    ...
    unsigned char
        DragDropPayloadBufLocal[ 8 ]; // Local buffer for small payloads

    // Widget state
    ImGuiInputTextState InputTextState;
    ImFont InputTextPasswordFont;
    ImGuiID ScalarAsInputTextId; // Temporary text input when CTRL+clicking on a
                                 // slider, etc.
    ImGuiColorEditFlags
    ...
    ImVec2 ScrollbarClickDeltaToGrabCenter; // Distance between mouse and center
                                            // of grab box, normalized in parent
                                            // space. Use storage?
    int TooltipOverrideCount;
    ImVector< char >
        PrivateClipboard; // If no custom clipboard handler is defined
    ImVec2 PlatformImePos,
        PlatformImeLastPos; // Cursor position request & last passed to the OS
                            // Input Method Editor
    ...
    ImVector< ImGuiWindowSettings >
        SettingsWindows; // ImGuiWindow .ini settings entries (parsed from the
                         // last loaded .ini file and maintained on saving)

    // Logging
    bool LogEnabled;
    FILE* LogFile;                // If != NULL log to stdout/ file
    ImGuiTextBuffer LogClipboard; // Accumulation buffer when log to clipboard.
    ...
    ImGuiContext( ImFontAtlas* shared_font_atlas ) : OverlayDrawList( NULL ) {
        ...
        IO.Fonts =
            shared_font_atlas ? shared_font_atlas : IM_NEW( ImFontAtlas )();
        ...
        NavWindow = NULL;
        NavId = NavActivateId = NavActivateDownId = NavActivatePressedId =
            NavInputId = 0;
...
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_Disabled =
        1 << 2, // false    // [BETA] Disable interactions but doesn't affect
...
// FIXME: That's theory, in practice the delimitation between ImGuiWindow and
// ImGuiWindowTempData is quite tenuous and could be reconsidered.
struct IMGUI_API ImGuiWindowTempData {
    ImVec2 CursorPos;
    ImVec2 CursorPosPrevLine;
    ImVec2 CursorStartPos; // Initial position in client area with padding
    ImVec2
        CursorMaxPos; // Used to implicitly calculate the size of our contents,
                      // always growing during the frame. Turned into
                      // window->SizeContents at the beginning of next frame
    ...
    float LogLinePosY;
    int TreeDepth;
    ImU32 TreeDepthMayJumpToParentOnPop; // Store a copy of !g.NavIdIsAlive for
                                         // TreeDepth 0..31
    ImGuiID LastItemId;
    ImGuiItemStatusFlags LastItemStatusFlags;
    ImRect LastItemRect;        // Interaction rect
    ImRect LastItemDisplayRect; // End-user display rect (only valid if
                                // LastItemStatusFlags &
                                // ImGuiItemStatusFlags_HasDisplayRect)
    bool NavHideHighlightOneFrame;
    bool NavHasScroll;   // Set when scrolling can be used (ScrollMax > 0.0f)
    int NavLayerCurrent; // Current layer, 0..31 (we currently only use 0..1)
    int NavLayerCurrentMask; // = (1 << NavLayerCurrent) used by ItemAdd prior
                             // to clipping.
    int NavLayerActiveMask;  // Which layer have been written to (result from
                             // previous frame)
    int NavLayerActiveMaskNext; // Which layer have been written to (buffer for
                                // current frame)
    ...
    ImGuiItemFlags
        ItemFlags; // == ItemFlagsStack.back() [empty == ImGuiItemFlags_Default]
    float ItemWidth;   // == ItemWidthStack.back(). 0.0: default, >0.0: width in
    ...
    ImGuiWindowTempData() {
        CursorPos = CursorPosPrevLine = CursorStartPos = CursorMaxPos =
            ImVec2( 0.0f, 0.0f );
...
struct IMGUI_API ImGuiWindow {
    ...
    ImVec2 SizeFullAtLastBegin; // Copy of SizeFull at the end of Begin. This is
                                // the reference value we'll use on the next
                                // frame to decide if we need scrollbars.
    ImVec2 SizeContents; // Size of contents (== extents reach of the drawing
                         // cursor) from previous frame. Include decoration,
                         // window title, border, menu, etc.
    ImVec2 SizeContentsExplicit; // Size of contents explicitly set by the user
    ...
    bool HasCloseButton; // Set when the window has a close button (p_open !=
                         // NULL)
    int BeginCount; // Number of Begin() during the current frame (generally 0
                    // or 1, 1+ if appending via multiple Begin/End pairs)
    int BeginOrderWithinParent;  // Order within immediate parent window, if we
                                 // are a child window. Otherwise 0.
    int BeginOrderWithinContext; // Order within entire imgui context. This is
                                 // mostly used for debugging submission order
                                 // related issues.
    ImGuiID
    ...
    int HiddenFramesForResize; // Hide the window for N frames while allowing
                               // items to be submitted so we can measure their
                               // size
    ImGuiCond SetWindowPosAllowFlags;  // store acceptable condition flags for
                                       // SetNextWindowPos() use.
    ImGuiCond SetWindowSizeAllowFlags; // store acceptable condition flags for
                                       // SetNextWindowSize() use.
    ImGuiCond SetWindowCollapsedAllowFlags; // store acceptable condition flags
    ...
    ImVec2 SetWindowPosVal; // store window position when using a non-zero Pivot
                            ...
                            // the window size)
    ImVec2 SetWindowPosPivot; // store window pivot for positioning. ImVec2(0,0)
                              // when positioning from top-left corner;
    ...
    ImRect ClipRect;             // Current clipping rectangle. =
                     // DrawList->clip_rect_stack.back(). Scissoring / clipping
                     // rectangle. x1, y1, x2, y2.
    ImRect OuterRectClipped; // = WindowRect just after setup in Begin(). ==
                             // window->Rect() for root window.
    ImRect InnerMainRect, InnerClipRect;
    ImRect
        ContentsRegionRect; // FIXME: This is currently confusing/misleading.
                            // Maximum visible content position ~~ Pos +
                            // (SizeContentsExplicit ? SizeContentsExplicit :
                            // Size - ScrollbarSizes) - CursorStartPos, per axis
    ...
    int SettingsIdx; // Index into SettingsWindow[] (indices are always valid as
                     // we only grow the array from the back)

    ImDrawList*
        DrawList; // == &DrawListInst (for backward compatibility reason with
                  // code using imgui_internal.h we keep this a pointer)
    ImDrawList DrawListInst;
    ImGuiWindow* ParentWindow; // If we are a child _or_ popup window, this is
                               // pointing to our parent. Otherwise NULL.
    ImGuiWindow* RootWindow; // Point to ourself or first ancestor that is not a
                             // child window.
    ImGuiWindow*
    ...
    ImGuiWindow*
        NavLastChildNavWindow; // When going to the menu bar, we remember the
                               ...
                               // window.)
    ImGuiID
        NavLastIds[ 2 ];    // Last known NavId for this window, per layer (0/1)
    ...
    // variables should be moved to ImGuiContext
    int FocusIdxAllCounter; // Start at -1 and increase as assigned via
                            // FocusItemRegister()
    int FocusIdxTabCounter; // (same, but only count widgets which you can Tab
                            // through)
    int FocusIdxAllRequestCurrent; // Item being requested for focus
    int FocusIdxTabRequestCurrent; // Tab-able item being requested for focus
    int FocusIdxAllRequestNext;    // Item being requested for focus, for next
                                // update (relies on layout to be stable between
                                // the frame pressing TAB and the next frame)
...
IMGUI_API void Shutdown(
    ImGuiContext* context ); // Since 1.60 this is a _private_ function. You can
...
IMGUI_API bool IsClippedEx( const ImRect& bb,
                            ImGuiID id,
                            bool clip_even_when_logged );
IMGUI_API bool FocusableItemRegister(
    ImGuiWindow* window,
    ImGuiID id,
    bool tab_stop = true ); // Return true if focus is requested
...
IMGUI_API ImVec2 FindBestWindowPosForPopupEx(
...
IMGUI_API void SetNavIDWithRectRel( ImGuiID id,
                                    int nav_layer,
...
inline bool IsNavInputPressedAnyOfTwo( ImGuiNavInput n1,
                                       ImGuiNavInput n2,
                                       ImGuiInputReadMode mode ) {
    return ( GetNavInputAmount( n1, mode ) + GetNavInputAmount( n2, mode ) ) >
           0.0f;
}
...
IMGUI_API void
VerticalSeparator(); // Vertical separator, for menu bars (use current line
                     // height). Not exposed because it is misleading and it
                     // doesn't have an effect on regular layout.
...
IMGUI_API bool TreeNodeBehaviorIsOpen(
    ImGuiID id,
    ImGuiTreeNodeFlags flags =
        0 ); // Consume previous SetNextTreeNodeOpened() data, if any. May
             // return true when logging
...
IMGUI_API bool InputTextEx( const char* label,
                            ...
                            void* user_data = NULL );
IMGUI_API bool InputScalarAsWidgetReplacement( const ImRect& bb,
                                               ImGuiID id,
                                               const char* label,
                                               ImGuiDataType data_type,
                                               void* data_ptr,
