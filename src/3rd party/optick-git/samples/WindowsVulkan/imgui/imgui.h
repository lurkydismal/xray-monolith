...
#define IM_ARRAYSIZE( _ARR )    \
    ( ( int )( sizeof( _ARR ) / \
               sizeof( *_ARR ) ) ) // Size of a static C-style array. Don't use
                                   // on pointers!
#define IM_OFFSETOF( _TYPE, _MEMBER ) \
    ( ( size_t )&( ( ( _TYPE* )0 )->_MEMBER ) ) // Offset of _MEMBER within
                                                // _TYPE. Standardized as
...
struct ImDrawListSharedData; // Data shared among multiple draw lists (typically
                             ...
                             // create one yourself)
struct ImDrawVert; // A single vertex (20 bytes by default, override layout with
...
typedef int
    ImFontAtlasFlags; // -> enum ImFontAtlasFlags_     // Flags: for ImFontAtlas
...
EndFrame(); // ends the ImGui frame. automatically called by Render(), you
            ...
            // better to not create any imgui windows and not call NewFrame() at
            // all!
IMGUI_API void
Render(); // ends the ImGui frame, finalize the draw data. (Obsolete: optionally
          // call io.RenderDrawListsFn if set. Nowadays, prefer calling your
...
IMGUI_API bool ImageButton(
    ...
    const ImVec4& tint_col =
        ImVec4( 1, 1, 1, 1 ) ); // <0 frame_padding uses default frame padding
...
IMGUI_API bool ListBoxHeader(
    ...
    const ImVec2& size = ImVec2(
        0,
        0 ) ); // use if you want to reimplement ListBox() will custom data or
...
IMGUI_API ImDrawList*
GetOverlayDrawList(); // this draw list will be the last rendered one, useful to
                      // quickly draw overlays shapes/text
...
IMGUI_API void SetAllocatorFunctions( void* ( *alloc_func )( size_t sz,
                                                             void* user_data ),
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_NoInputs =
        1 << 9, // Disable catching mouse or keyboard inputs, hovering test with
                // pass through.
    ...
    ImGuiWindowFlags_NavFlattened =
        1 << 23, // [BETA] Allow gamepad/keyboard navigation to cross over
                 // parent border to this child (only use on child that have no
                 // scrolling!)
    ...
    // 1 << 17,  // --> Set io.ConfigResizeWindowsFromEdges and make sure mouse
    // cursors are supported by back-end (io.BackendFlags &
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_CallbackResize =
        1
        << 18, // Allow buffer capacity resize + notify when the string wants to
               // be resized (for string types which hold a cache of their Size)
               // (see misc/stl/imgui_stl.h for an example of using this)
    // [Internal]
...
enum ImGuiHoveredFlags_ {
    ...
    ImGuiHoveredFlags_AllowWhenOverlapped =
        1 << 6, // Return true even if the position is overlapped by another
                // window
...
enum ImGuiDragDropFlags_ {
    ...
    ImGuiDragDropFlags_SourceExtern =
        1 << 4, // External source (from outside of imgui), won't attempt to
...
enum ImGuiNavInput_ {
    ...
    ImGuiNavInput_Input, // text input / on-screen keyboard              // e.g.
                         // Triang.(PS4), Y (Xbox), X (Switch), Return
                         // (Keyboard)
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableSetMousePos =
        1 << 2, // Instruct navigation to move the mouse cursor. May be useful
                // on TV/console systems where moving a virtual mouse is
                // awkward. Will update io.MousePos and set
                // io.WantSetMousePos=true. If enabled you MUST honor
                // io.WantSetMousePos requests in your binding, otherwise ImGui
                // will react as if the mouse is jumping around back and forth.
    ...
    ImGuiConfigFlags_NoMouseCursorChange =
        1
        << 5, // Instruct back-end to not alter mouse cursor shape and
    ...
    // User storage (to allow your back-end/engine to communicate to code that
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasSetMousePos =
        1 << 2 // Back-end supports io.WantSetMousePos requests to reposition
               // the OS mouse position (only used if
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_NoAlpha =
        1 << 1, //              // ColorEdit, ColorPicker, ColorButton: ignore
                //              Alpha component (read 3 components from the
                //              input pointer).
    ...
    ImGuiColorEditFlags_NoSmallPreview =
        1 << 4, //              // ColorEdit, ColorPicker: disable colored
                //              square preview next to the inputs. (e.g. to show
                //              only the inputs)
...
struct ImGuiIO {
    ...
    ImGuiConfigFlags
        ConfigFlags; // = 0                  // See ImGuiConfigFlags_ enum. Set
                     ...
                     // options, etc.
    ImGuiBackendFlags
        BackendFlags;   // = 0                  // Set ImGuiBackendFlags_ enum.
                        // Set by imgui_impl_xxx files or custom back-end to
    ...
    const char*
        IniFilename; // = "imgui.ini"        // Path to .ini file. Set NULL to
                     // disable automatic .ini loading/saving, if e.g. you want
                     // to manually load/save from memory.
    const char* LogFilename; // = "imgui_log.txt"    // Path to .log file
                             // (default parameter to ImGui::LogToFile when no
                             // file is specified).
    ...
    ImVec2 DisplayFramebufferScale; // = (1.0f,1.0f)        // For retina
                                    // display or other situations where window
                                    // coordinates are different from
                                    // framebuffer coordinates. User storage
                                    // only, presently not used by ImGui.
    ImVec2 DisplayVisibleMin; // <unset> (0.0f,0.0f)  // [obsolete] If you use
                              // DisplaySize as a virtual space larger than your
                              // screen, set DisplayVisibleMin/Max to the
                              // visible area.
    ...
    bool ConfigMacOSXBehaviors; // = defined(__APPLE__) // OS X style: Text
                                ...
                                // Ctrl, Line/Text Start and End using
                                // Cmd+Arrows instead of Home/End, Double click
                                // selects by word instead of selecting whole
    ...
    bool ConfigResizeWindowsFromEdges; // = false          // [BETA] Enable
                                       // resizing of windows from their edges
                                       // and from the lower-left corner. This
                                       // requires (io.BackendFlags &
                                       // ImGuiBackendFlags_HasMouseCursors)
                                       // because it needs mouse cursor
                                       // feedback. (This used to be the
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // [OBSOLETE since 1.60+] Rendering function, will be automatically called
    // in Render(). Please call your rendering function yourself now! You can
    // obtain the ImDrawData* by calling ImGui::GetDrawData() after Render().
    ...
    bool MouseDown[ 5 ]; // Mouse buttons: 0=left, 1=right, 2=middle + extras.
                         // ImGui itself mostly only uses left button
                         // (BeginPopupContext** are using right button). Others
                         // buttons allows us to track if the mouse is being
                         // used by your application + available to user as a
                         // convenience via IsMouse** API.
    float
    ...
    bool KeysDown[ 512 ]; // Keyboard keys that are pressed (ideally left in the
                          ...
                          // keys, so you can use your own defines/enums for
                          // keys).
    ImWchar InputCharacters[ 16 + 1 ]; // List of characters input (translated
                                       // by user from keypress+keyboard state).
                                       // Fill using AddInputCharacter() helper.
    float NavInputs[ ImGuiNavInput_COUNT ]; // Gamepad inputs (keyboard keys
                                            // will be auto-mapped and be
                                            // written here by ImGui::NewFrame,
                                            // all values will be cleared back
                                            // to zero in ImGui::EndFrame)
    ...
    bool
        WantCaptureMouse; // When io.WantCaptureMouse is true, imgui will use
    ...
    bool WantCaptureKeyboard; // When io.WantCaptureKeyboard is true, imgui will
                              // use the keyboard inputs, do not dispatch them
                              // to your main game/application (in both cases,
                              // always pass keyboard inputs to imgui). (e.g.
                              // InputText active, or an imgui window is focused
                              // and navigation is enabled, etc.).
    bool WantTextInput; // Mobile/console: when io.WantTextInput is true, you
                        // may display an on-screen keyboard. This is set by
                        // ImGui when it wants textual keyboard input to happen
    ...
    bool
        MouseDownOwned[ 5 ]; // Track if button was clicked inside a window. We
    ...
    float KeysDownDuration[ 512 ]; // Duration the keyboard key has been down
                                   // (0.0f == just pressed)
    float
        KeysDownDurationPrev[ 512 ]; // Previous duration the key has been down
...
public:
    ...
    inline void reserve( int new_capacity ) {
        if ( new_capacity <= Capacity )
            return;
        value_type* new_data = ( value_type* )ImGui::MemAlloc(
            ( size_t )new_capacity * sizeof( value_type ) );
        if ( Data ) {
...
struct ImGuiStorage {
    ...
    // For quicker full rebuild of a storage (instead of an incremental one),
    // you may add all your contents and then sort once.
...
struct ImGuiInputTextCallbackData {
    ...
    ImWchar EventChar; // Character input                      // Read-write //
                       // [CharFilter] Replace character or set to zero. return
                       // 1 is equivalent to setting EventChar=0;
...
struct ImGuiListClipper {
    ...
    // GetFrameHeightWithSpacing(). If you don't specify an items_height, you
    // NEED to call Step(). If you specify items_height you may call the old
...
struct ImDrawCmd {
    ...
    ImDrawCallback UserCallback; // If != NULL, call the function instead of
                                 // rendering the vertices. clip_rect and
...
enum ImDrawCornerFlags_ {
    ...
    ImDrawCornerFlags_All =
        0xF // In your function calls you may use ~0 (= all bits sets) instead
...
struct ImDrawList {
    ...
    const ImDrawListSharedData*
        _Data; // Pointer to shared draw data (you can use
               // ImGui::GetDrawListSharedData() to get the one from current
               // ImGui context)
    ...
    // Primitives
    IMGUI_API void AddLine( const ImVec2& a,
                            const ImVec2& b,
                            ImU32 col,
    ...
    IMGUI_API void AddTriangle( const ImVec2& a,
                                ...
                                ImU32 col,
                                float thickness = 1.0f );
    IMGUI_API void AddTriangleFilled( const ImVec2& a,
                                      const ImVec2& b,
                                      const ImVec2& c,
                                      ImU32 col );
...
struct ImDrawData {
    ...
    ImVec2
        DisplaySize; // Size of the viewport to render (== io.DisplaySize for
                     // the main viewport) (DisplayPos + DisplaySize ==
    ...
    IMGUI_API void ScaleClipRects(
        const ImVec2&
            sc ); // Helper to scale the ClipRect field of each ImDrawCmd. Use
                  // if your final output buffer is at a different scale than
                  // ImGui expects, or if there is a difference between your
...
struct ImFontConfig {
    ...
    int OversampleV;  // 1        // Rasterize at higher quality for sub-pixel
                      ...
                      // axis.
    bool
        PixelSnapH; // false    // Align every glyph to pixel boundary. Useful
    ...
    const ImWchar*
        GlyphRanges; // NULL     // Pointer to a user-provided list of Unicode
                     ...
                     // zero-terminated list). THE ARRAY DATA NEEDS TO PERSIST
...
struct ImFontAtlas {
    ...
    // create UTF-8 string literal using the u8"Hello world" syntax. See FAQ for
    // details. NB: Consider using GlyphRangesBuilder to build glyph ranges from
    // textual data.
    ...
    // position and render your pixels. You can also request your rectangles to
    // be mapped as font glyph (given a font + Unicode point), so you can render
    // e.g. custom colorful icons and use them as regular glyphs.
    struct CustomRect {
        unsigned int
    ...
    IMGUI_API int AddCustomRectRegular(
        unsigned int id,
        int width,
        int height ); // Id needs to be >= 0x10000. Id >= 0x80000000 are
                      // reserved for ImGui and ImDrawList
    ...
    // [Internal]
    IMGUI_API void CalcCustomRectUV( const CustomRect* rect,
                                     ImVec2* out_uv_min,
                                     ImVec2* out_uv_max );
...
struct ImFont {
    ...
    float
        Scale; // = 1.f        // Base font scale, multiplied by the per-window
    ...
    ImVector< unsigned short >
        IndexLookup; //              // Sparse. Index glyphs by Unicode
                     //              code-point.
    ...
    float Ascent, Descent;       //              // Ascent: distance from top to
                           //              bottom of e.g. 'A' [0..FontSize]
    bool DirtyLookupTables;
    int MetricsTotalSurface; //              // Total surface in pixels to get
                             //              an idea of the font
