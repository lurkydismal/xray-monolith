...
#if STATS
static FName NAME_STATGROUP_CPUStalls(
    FStatGroup_STATGROUP_CPUStalls::GetGroupName() );
...
class FOptickPlugin : public IOptickPlugin {
    ...
    bool UpdateCalibrationTimestamp( FRealtimeGPUProfilerFrameImpl* Frame,
                                     int GPUIndex );
...
bool FOptickPlugin::Tick( float DeltaTime ) {
    ...
    // GEngine->AddOnScreenDebugMessage(31313, 5.f, FColor::Yellow,
    // OptickMessage);
...
void FOptickPlugin::OnOpenGUI() {
    ...
    const FString OptickPath =
        IPluginManager::Get().FindPlugin( "OptickPlugin" )->GetBaseDir() /
        TEXT( "GUI/Optick.exe" );
    const FString OptickFullPath =
        FPaths::ConvertRelativePathToFull( OptickPath );
    const FString OptickArgs;
    FPlatformProcess::CreateProc( *OptickFullPath, *OptickArgs, true, false,
...
void FOptickPlugin::RegisterMenus() {
    ...
    if ( UToolMenu* ProfileMenu =
             UToolMenus::Get()->ExtendMenu( "MainFrame.MainMenu.Tools" ) ) {
        FToolMenuSection& Section = ProfileMenu->AddSection(
            "Optick Profiler", FText::FromString( TEXT( "Optick Profiler" ) ) );
        Section.AddMenuEntry(
            "OpenOptickProfiler",
            LOCTEXT( "OpenOptickProfiler_Label", "Open Optick Profiler" ),
            LOCTEXT( "OpenOptickProfiler_Desc", "Open Optick Profiler" ),
            FSlateIcon( FOptickStyle::GetStyleSetName(),
                        "Optick.PluginAction" ),
            FUIAction(
                FExecuteAction::CreateRaw( this, &FOptickPlugin::OnOpenGUI ),
                FCanExecuteAction() ) );
    } else {
        UE_LOG( OptickLog, Error,
                TEXT( "Can't find 'MainFrame.MainMenu.Tools' menu section" ) )
    }
}
            ...
            if ( plugin.IsScreenshotReady() ) {
                ...
                if ( PlatformFile.FileExists( *FullName ) ) {
                    ...
                    IFileHandle* FileHandle =
                        PlatformFile.OpenRead( *FullName );
                    if ( FileHandle ) {
                        FileHandle->Read( Image.GetData(), FileSize );
                        delete FileHandle;
                    }

                    Optick::AttachFile( Optick::File::OPTICK_IMAGE,
                                        "Screenshot.png", Image.GetData(),
                                        ( uint32_t )FileSize );
                }
            }
...
void FOptickPlugin::StartupModule() {
    ...
    GPUThreadStorage.EventStorage = Optick::RegisterStorage(
        TCHAR_TO_ANSI( *FPlatformMisc::GetPrimaryGPUBrand() ), ( uint64_t )-1,
        Optick::ThreadMask::GPU );

    // Subscribing for Ticker
    TickDelegateHandle = FTSTicker::GetCoreTicker().AddTicker(
        FTickerDelegate::CreateRaw( this, &FOptickPlugin::Tick ) );
    ...
    if ( GIsEditor ) {
        ...
        // ExtensionManager =
        // LevelEditorModule.GetToolBarExtensibilityManager(); ToolbarExtender =
        // MakeShareable(new FExtender); ToolbarExtension =
        // ToolbarExtender->AddToolBarExtension("Game", EExtensionHook::After,
        // PluginCommands,
        // FToolBarExtensionDelegate::CreateLambda([this](FToolBarBuilder&
        // ToolbarBuilder) { AddToolbarExtension(ToolbarBuilder); })	);
        ...
        //{
        //	FToolMenuSection& Section = ToolbarMenu->AddSection("Optick
        // Profiler", FText::FromString(TEXT("Optick Profiler")));
        //	FToolMenuEntry& Entry =
        // Section.AddEntry(FToolMenuEntry::InitToolBarButton(FOptickCommands::Get().PluginAction));
        //	Entry.SetCommandList(PluginCommands);
        // }
        // else
        //{
        //	UE_LOG(OptickLog, Error, TEXT("Can't find
        //'LevelEditor.LevelEditorToolBar' menu section"))
        // }
    }
#endif

#ifdef OPTICK_UE4_GPU
    EndFrameRTDelegateHandle = FCoreDelegates::OnEndFrameRT.AddRaw(
        this, &FOptickPlugin::OnEndFrameRT );
#endif
}
...
void FOptickPlugin::StartCapture() {
    ...
    if ( !IsCapturing ) {
...
#if WITH_EDITOR
        UEditorPerformanceSettings* Settings =
            GetMutableDefault< UEditorPerformanceSettings >();
        check( Settings );
        BaseSettings.bCPUThrottleEnabled =
            Settings->bThrottleCPUWhenNotForeground;
        ...
        // Set up our delegate to gather data from the stats thread for safe
        // consumption on game thread.
        StatFrameDelegateHandle =
            Stats.NewFrameDelegate.Add( FStatFrameDelegate::CreateRaw(
                this, &FOptickPlugin::GetDataFromStatsThread ) );
#endif
    }
}
...
void FOptickPlugin::RequestScreenshot() {
    ...
    FScreenshotRequest::OnScreenshotRequestProcessed().AddRaw(
        this, &FOptickPlugin::OnScreenshotProcessed );
    FScreenshotRequest::RequestScreenshot( Optick_SCREENSHOT_NAME, true,
                                           false );
}
...
bool FOptickPlugin::IsReadyToDumpCapture() const {
    return IsScreenshotReady() ||
           ( GFrameNumber - WaitingForScreenshotFrameNumber ) >
               WaitForScreenshotMaxFrameCount;
}
...
bool FOptickPlugin::UpdateCalibrationTimestamp(
    FRealtimeGPUProfilerFrameImpl* Frame,
    int GPUIndex ) {
    FGPUTimingCalibrationTimestamp& CalibrationTimestamp =
        CalibrationTimestamps[ GPUIndex ];
    CalibrationTimestamp = FGPUTimingCalibrationTimestamp{ 0, 0 };

    if ( Frame->TimestampCalibrationQuery.IsValid() ) {
        CalibrationTimestamp.GPUMicroseconds =
            Frame->TimestampCalibrationQuery->GPUMicroseconds[ GPUIndex ];
        CalibrationTimestamp.CPUMicroseconds =
            Frame->TimestampCalibrationQuery->CPUMicroseconds[ GPUIndex ];
    }

    if ( CalibrationTimestamp.GPUMicroseconds == 0 ||
         CalibrationTimestamp.CPUMicroseconds ==
             0 ) // Unimplemented platforms, or invalid on the first frame
    {
        if ( Frame->GpuProfilerEvents.Num() > 1 ) {
            // Align CPU and GPU frames
            CalibrationTimestamp.GPUMicroseconds =
                Frame->GpuProfilerEvents[ 1 ].GetStartResultMicroseconds( 0 );
            CalibrationTimestamp.CPUMicroseconds =
                FPlatformTime::ToSeconds64( Frame->CPUFrameStartTimestamp ) *
                1000 * 1000;
        } else {
            // Fallback to legacy
    ...
    return CalibrationTimestamp.GPUMicroseconds != 0 &&
           CalibrationTimestamp.CPUMicroseconds != 0;
}
...
void FOptickPlugin::OnEndFrameRT() {
    ...
    if ( FRealtimeGPUProfilerImpl* gpuProfiler =
             reinterpret_cast< FRealtimeGPUProfilerImpl* >(
                 FRealtimeGPUProfiler::Get() ) ) {
        if ( FRealtimeGPUProfilerFrameImpl* Frame =
                 gpuProfiler->Frames[ gpuProfiler->ReadBufferIndex ] ) {
            ...
            for ( int32 i = Frame->NextResultPendingEventIdx;
                  i < NumEventsThisFramePlusOne; ++i ) {
                FRealtimeGPUProfilerEventImpl& Event =
                    Frame->GpuProfilerEvents[ i ];

                if ( !Event.GatherQueryResults( RHICmdList ) ) {
#if !( UE_BUILD_SHIPPING || UE_BUILD_TEST )
                    UE_LOG( OptickLog, Warning, TEXT( "Query is not ready." ) );
#endif
                    // The frame isn't ready yet. Don't update stats - we'll try
                    // again next frame.
                    return;
                }
            }
            ...
            // Can't collect GPU data without valid calibration between CPU and
            // GPU timestamps
            if ( !UpdateCalibrationTimestamp( Frame, GPUIndex ) )
                return;

            uint64 lastTimeStamp =
                FMath::Max( CalibrationTimestamps[ GPUIndex ].CPUMicroseconds,
                            GPUThreadStorage.LastTimestamp );

            const FRealtimeGPUProfilerEventImpl& FirstEvent =
                Frame->GpuProfilerEvents[ 1 ];
            uint64 frameStartTimestamp = FMath::Max(
                ConvertGPUTimestamp(
                    FirstEvent.GetStartResultMicroseconds( GPUIndex ),
                    GPUIndex ),
                lastTimeStamp );

            OPTICK_FRAME_FLIP( Optick::FrameType::GPU, frameStartTimestamp );
            OPTICK_STORAGE_PUSH(
                GPUThreadStorage.EventStorage,
                Optick::GetFrameDescription( Optick::FrameType::GPU ),
                frameStartTimestamp )

            for ( int32 Idx = 1; Idx < NumEventsThisFramePlusOne; ++Idx ) {
                const FRealtimeGPUProfilerEventImpl& Event =
                    Frame->GpuProfilerEvents[ Idx ];
                ...
                if ( Event.GetGPUMask().Contains( GPUIndex ) ) {
                    ...
                    if ( Optick::EventDescription** ppDescription =
                             GPUDescriptionMap.Find( Name ) ) {
                        Description = *ppDescription;
                    } else {
                        Description = Optick::EventDescription::CreateShared(
                            TCHAR_TO_ANSI( *Name.ToString() ) );
                        GPUDescriptionMap.Add( Name, Description );
                    }

                    uint64 startTimestamp = ConvertGPUTimestamp(
                        Event.GetStartResultMicroseconds( GPUIndex ),
                        GPUIndex );
                    uint64 endTimestamp = ConvertGPUTimestamp(
                        Event.GetEndResultMicroseconds( GPUIndex ), GPUIndex );

                    // Fixing potential errors
                    startTimestamp =
                        FMath::Max( startTimestamp, lastTimeStamp );
                    endTimestamp = FMath::Max( endTimestamp, startTimestamp );

                    // Ensuring correct hierarchy
                    while ( EventStack.Num() &&
                            ( EventStack.Last().Finish <= startTimestamp ) )
                        EventStack.Pop();

                    // Discovered broken hierarchy, skipping event
                    if ( EventStack.Num() &&
                         ( endTimestamp < EventStack.Last().Start ) )
                        continue;

                    // Clamp range against the parent counter
                    if ( EventStack.Num() ) {
                        TimeRange parent = EventStack.Last();
                        startTimestamp = FMath::Clamp(
                            startTimestamp, parent.Start, parent.Finish );
                        endTimestamp = FMath::Clamp( endTimestamp, parent.Start,
                                                     parent.Finish );
                    }
                    ...
                    // if (Name == NAME_GPU_Unaccounted)
                    //{
                    //	OPTICK_FRAME_FLIP(Optick::FrameType::GPU,
                    // startTimestamp);

                    //	OPTICK_STORAGE_PUSH(GPUThreadStorage.EventStorage,
                    // Optick::GetFrameDescription(Optick::FrameType::GPU),
                    // startTimestamp)
                    //}
                    // else
                    {
                        ...
                        OPTICK_STORAGE_EVENT( GPUThreadStorage.EventStorage,
                                              Description, startTimestamp,
                                              endTimestamp );
                    }
                    lastTimeStamp =
                        FMath::Max< uint64 >( lastTimeStamp, endTimestamp );
                }
            }
...
uint64 FOptickPlugin::ConvertGPUTimestamp( uint64 timestamp, int GPUIndex ) {
    if ( CalibrationTimestamps[ GPUIndex ].CPUMicroseconds == 0 ||
         CalibrationTimestamps[ GPUIndex ].GPUMicroseconds == 0 ) {
        return ( uint64 )-1;
    }

    const uint64 cpuTimestampUs =
        timestamp - CalibrationTimestamps[ GPUIndex ].GPUMicroseconds +
        CalibrationTimestamps[ GPUIndex ].CPUMicroseconds;
    const uint64 cpuTimestamp =
        cpuTimestampUs * 1e-6 / FPlatformTime::GetSecondsPerCycle64();
    return cpuTimestamp;
}
#endif
...
void FOptickPlugin::GetDataFromStatsThread( int64 CurrentFrame ) {
    ...
    for ( int32 PacketIndex = 0; PacketIndex < Frame.Packets.Num();
          PacketIndex++ ) {
        ...
        if ( ThreadStorage** ppStorage = StorageMap.Find( Packet.ThreadId ) ) {
            ...
            Storage = new ThreadStorage( Optick::RegisterStorage(
                TCHAR_TO_ANSI( *ThreadName.ToString() ), Packet.ThreadId ) );
            StorageMap.Add( Packet.ThreadId, Storage );
        }

        for ( FStatMessage const& Item : Packet.StatMessages ) {
            EStatOperation::Type Op =
                Item.NameAndInfo.GetField< EStatOperation >();
            ...
            if ( Op == EStatOperation::CycleScopeStart ||
                ...
                uint64 Timestamp =
                    Convert32bitCPUTimestamp( Item.GetValue_int64() );

                if ( Op == EStatOperation::CycleScopeStart ) {
                    Optick::EventDescription* Description = nullptr;

                    if ( Optick::EventDescription** ppDescription =
                             DescriptionMap.Find( name ) ) {
                        Description = *ppDescription;
                    } else {
                        ...
                        for ( int i = 0; i < sizeof( NAME_Wait ) /
                                                 sizeof( NAME_Wait[ 0 ] );
                              ++i ) {
                        ...
                        Description = Optick::EventDescription::CreateShared(
                            TCHAR_TO_ANSI( *shortName.ToString() ), nullptr, 0,
                            color );

                        DescriptionMap.Add( name, Description );
                    }

                    OPTICK_STORAGE_PUSH( Storage->EventStorage, Description,
                                         Timestamp );
                } else {
                    Storage->LastTimestamp = FMath::Max< uint64 >(
                        Storage->LastTimestamp, Timestamp );
                ...
                if ( Storage->LastTimestamp != 0 ) {
                    const Optick::EventDescription* cpuFrameDescription =
                        Optick::GetFrameDescription( Optick::FrameType::CPU );
                    // Pop the previous frame event
                    OPTICK_STORAGE_POP( Storage->EventStorage,
                                        Storage->LastTimestamp );
                    // Push the new one
                    OPTICK_STORAGE_PUSH( Storage->EventStorage,
                                         cpuFrameDescription,
                                         Storage->LastTimestamp );

                    OPTICK_FRAME_FLIP( Optick::FrameType::CPU,
                                       Storage->LastTimestamp,
                                       Packet.ThreadId );
                }
            } else if ( Op == EStatOperation::AdvanceFrameEventRenderThread ) {
                if ( Storage->LastTimestamp != 0 ) {
                    const Optick::EventDescription* renderFrameDescription =
                        Optick::GetFrameDescription(
                    ...
                    OPTICK_FRAME_FLIP( Optick::FrameType::Render,
                                       Storage->LastTimestamp,
                                       Packet.ThreadId );
                }
            }
