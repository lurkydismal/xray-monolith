...
#define REALTIME_GPU_PROFILER_EVENT_TRACK_FRAME_NUMBER \
    ( TRACING_PROFILER || DO_CHECK )
...
public:
        ...
        for ( uint32 GPUIndex : GPUMask ) {
            if ( StartResultMicroseconds[ GPUIndex ] == InvalidQueryResult ) {
                if ( !RHICmdList.GetRenderQueryResult(
                         StartQuery.GetQuery(),
                         StartResultMicroseconds[ GPUIndex ], false,
                         GPUIndex ) ) {
            ...
            if ( EndResultMicroseconds[ GPUIndex ] == InvalidQueryResult ) {
                if ( !RHICmdList.GetRenderQueryResult(
                         EndQuery.GetQuery(), EndResultMicroseconds[ GPUIndex ],
                         false, GPUIndex ) ) {
    ...
    uint64 GetResultUs( uint32 GPUIndex ) const {
        ...
        if ( StartResultMicroseconds[ GPUIndex ] >
             EndResultMicroseconds[ GPUIndex ] ) {
            return 0llu;
        }

        return EndResultMicroseconds[ GPUIndex ] -
               StartResultMicroseconds[ GPUIndex ];
    }

    bool HasValidResult( uint32 GPUIndex ) const {
        return StartResultMicroseconds[ GPUIndex ] != InvalidQueryResult &&
               EndResultMicroseconds[ GPUIndex ] != InvalidQueryResult;
    }
...
#if STATS
    const FName& GetStatName() const { return StatName; }
#endif
...
public:
    ...
    TArray< FRealtimeGPUProfilerEventImpl,
            TInlineAllocator< GPredictedMaxNumEvents > >
        GpuProfilerEvents;
    TArray< int32, TInlineAllocator< GPredictedMaxNumEvents > >
        GpuProfilerEventParentIndices;
    TArray< int32, TInlineAllocator< GPredictedMaxStackDepth > > EventStack;
    TArray< FGPUEventTimeAggregate, TInlineAllocator< GPredictedMaxNumEvents > >
        EventAggregates;
};
...
static_assert( sizeof( FRealtimeGPUProfilerImpl ) ==
                   sizeof( FRealtimeGPUProfiler ),
               "Size mismatch" );
