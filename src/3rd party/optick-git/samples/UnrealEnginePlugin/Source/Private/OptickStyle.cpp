...
#define BOX_BRUSH( RelativePath, ... )                                       \
    FSlateBoxBrush( Style->RootToContentDir( RelativePath, TEXT( ".png" ) ), \
                    __VA_ARGS__ )
#define BORDER_BRUSH( RelativePath, ... ) \
    FSlateBorderBrush(                    \
        Style->RootToContentDir( RelativePath, TEXT( ".png" ) ), __VA_ARGS__ )
#define TTF_FONT( RelativePath, ... )                                        \
    FSlateFontInfo( Style->RootToContentDir( RelativePath, TEXT( ".ttf" ) ), \
                    __VA_ARGS__ )
#define OTF_FONT( RelativePath, ... )                                        \
    FSlateFontInfo( Style->RootToContentDir( RelativePath, TEXT( ".otf" ) ), \
                    __VA_ARGS__ )
...
TSharedRef< FSlateStyleSet > FOptickStyle::Create() {
    TSharedRef< FSlateStyleSet > Style =
        MakeShareable( new FSlateStyleSet( "OptickStyle" ) );
    Style->SetContentRoot(
        IPluginManager::Get().FindPlugin( "OptickPlugin" )->GetBaseDir() /
        TEXT( "Resources" ) );

    Style->Set( "Optick.PluginAction",
                new IMAGE_BRUSH( TEXT( "Icon128" ), Icon40x40 ) );
