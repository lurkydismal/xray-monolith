...
bool OnOptickStateChanged( Optick::State::Type state ) {
    if ( state == Optick::State::DUMP_CAPTURE ) {
        ...
        // Attach screenshot
        Optick::AttachFile( Optick::File::OPTICK_IMAGE, Optick::TestImage::Name,
                            Optick::TestImage::Data, Optick::TestImage::Size );

        // Attach text file
        const char* textFile =
            "You could attach custom text files!\nFor example you could add "
            "dxdiag.txt or current game settings.";
        Optick::AttachFile( Optick::File::OPTICK_TEXT, "Test.txt",
                            ( uint8_t* )textFile,
                            ( uint32_t )strlen( textFile ) );
    }
    return true;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
struct RootTask {
    ...
    void Do( MT::FiberContext& context ) {
        ...
        context.RunSubtasksAndYield( MT::TaskGroup::Default(), children,
                                     CHILDREN_COUNT );
...
struct PriorityTask {
    MT_DECLARE_TASK( PriorityTask,
                     MT::StackRequirements::STANDARD,
                     MT::TaskPriority::HIGH,
                     MT::Color::Orange );
...
void Engine::UpdateTasks() {
    ...
    scheduler.RunAsync( MT::TaskGroup::Default(), &priorityTasks[ 0 ],
                        MT_ARRAY_SIZE( priorityTasks ) );
...
void Engine::Draw() {
    ...
    static Optick::EventDescription* GPUFrame =
        Optick::EventDescription::CreateShared( "GPU Frame" );

    // Adding GPUFrame event to the GPUStorage with specified start\stop
    // timestamps
    OPTICK_STORAGE_EVENT( GPUStorage, GPUFrame, cpuTimestampStart,
                          cpuTimestampFinish );
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
class Profiler : public MT::IProfilerEventListener {
    Optick::EventStorage*
        fiberEventStorages[ MT::MT_MAX_STANDART_FIBERS_COUNT +
                            MT::MT_MAX_EXTENDED_FIBERS_COUNT ];
...
public:
    Profiler() : totalFibersCount( 0 ) {}

    virtual void OnFibersCreated( uint32 fibersCount ) override {
        totalFibersCount = fibersCount;
        MT_ASSERT( fibersCount <= MT_ARRAY_SIZE( fiberEventStorages ),
                   "Too many fibers!" );
        for ( uint32 fiberIndex = 0; fiberIndex < fibersCount; fiberIndex++ ) {
            Optick::RegisterFiber( fiberIndex,
                                   &fiberEventStorages[ fiberIndex ] );
        }
    }
    ...
    virtual void OnTemporaryWorkerThreadLeave() override {
        Optick::EventStorage** currentThreadStorageSlot =
            Optick::GetEventStorageSlotForCurrentThread();
    ...
    virtual void OnTaskExecuteStateChanged( MT::Color::Type debugColor,
                                            const mt_char* debugID,
                                            MT::TaskExecuteState::Type type,
                                            int32 fiberIndex ) override {
        MT_ASSERT( fiberIndex < ( int32 )totalFibersCount,
                   "Sanity check failed" );
        ...
        // if actual fiber is scheduler internal fiber (don't have event storage
        // for internal scheduler fibers)
        if ( fiberIndex < 0 ) {
            return;
        }
        ...
        switch ( type ) {
            ...
            case MT::TaskExecuteState::RESUME: {
                MT_ASSERT( originalThreadStorage == nullptr,
                           "Sanity check failed" );

                originalThreadStorage = *currentThreadStorageSlot;

                MT_ASSERT( IsFiberStorage( originalThreadStorage ) == false,
                           "Sanity check failed" );
                ...
                *currentThreadStorageSlot = currentFiberStorage;
                ...
                Optick::FiberSyncData::AttachToThread(
                    currentFiberStorage, MT::ThreadId::Self().AsUInt64() );
            } break;

            case MT::TaskExecuteState::STOP:
            case MT::TaskExecuteState::SUSPEND: {
                Optick::EventStorage* currentFiberStorage =
                    *currentThreadStorageSlot;
                ...
                MT_ASSERT( checkFiberStorage == currentFiberStorage,
                           "Sanity check failed" );

                MT_ASSERT( activeThreadStorage == currentFiberStorage,
                           "Sanity check failed" );

                //////////////////////////////////////////////////////////////////////////

                MT_ASSERT( IsFiberStorage( currentFiberStorage ) == true,
                           "Sanity check failed" );
