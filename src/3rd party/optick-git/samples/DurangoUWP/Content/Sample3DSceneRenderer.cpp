...
void Sample3DSceneRenderer::CreateWindowSizeDependentResources() {
    ...
    // Eye is at (0,0.7,1.5), looking at point (0,-0.1,0) with the up-vector
    // along the y-axis.
...
// Called once per frame, rotates the cube and calculates the model and view
// matrices.
...
// When tracking, the 3D cube can be rotated around its Y axis by tracking
// pointer position relative to the output screen width.
void Sample3DSceneRenderer::TrackingUpdate( float positionX ) {
    if ( m_tracking ) {
        float radians = XM_2PI * 2.0f * positionX /
...
void Sample3DSceneRenderer::Render() {
    ...
    context->UpdateSubresource1( m_constantBuffer.Get(), 0, NULL,
                                 &m_constantBufferData, 0, 0, 0 );
    ...
    context->IASetIndexBuffer(
        m_indexBuffer.Get(),
        DXGI_FORMAT_R16_UINT, // Each index is one 16-bit unsigned integer
                              // (short).
        0 );
    ...
    context->VSSetConstantBuffers1( 0, 1, m_constantBuffer.GetAddressOf(),
                                    nullptr, nullptr );
...
void Sample3DSceneRenderer::CreateDeviceDependentResources() {
    ...
    // After the vertex shader file is loaded, create the shader and input
    // layout.
    auto createVSTask =
        loadVSTask.then( [ this ]( const std::vector< byte >& fileData ) {
            DX::ThrowIfFailed(
                m_deviceResources->GetD3DDevice()->CreateVertexShader(
                    &fileData[ 0 ], fileData.size(), nullptr,
                    &m_vertexShader ) );
            ...
            DX::ThrowIfFailed(
                m_deviceResources->GetD3DDevice()->CreateInputLayout(
                    vertexDesc, ARRAYSIZE( vertexDesc ), &fileData[ 0 ],
                    fileData.size(), &m_inputLayout ) );
        } );
