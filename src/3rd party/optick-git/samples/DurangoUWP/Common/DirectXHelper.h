...
inline Concurrency::task< std::vector< byte > > ReadDataAsync(
    ...
    return create_task( folder->GetFileAsync(
        ...
        .then( []( Streams::IBuffer ^ fileBuffer ) -> std::vector< byte > {
            ...
            Streams::DataReader::FromBuffer( fileBuffer )
                ->ReadBytes( Platform::ArrayReference< byte >(
                    returnBuffer.data(), fileBuffer->Length ) );
            return returnBuffer;
        } );
}
...
inline float ConvertDipsToPixels( float dips, float dpi ) {
    ...
    return floorf( dips * dpi / dipsPerInch +
