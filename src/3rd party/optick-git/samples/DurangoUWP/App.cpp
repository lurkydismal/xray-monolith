...
void App::Initialize( CoreApplicationView ^ applicationView ) {
    ...
    // Activated, so that we can make the CoreWindow active and start rendering
    // on the window.
...
void App::SetWindow( CoreWindow ^ window ) {
    ...
    DisplayInformation::DisplayContentsInvalidated +=
        ref new TypedEventHandler< DisplayInformation ^, Object ^ >(
            this, &App::OnDisplayContentsInvalidated );
...
void App::Load( Platform::String ^ entryPoint ) {
    if ( m_main == nullptr ) {
        m_main = std::unique_ptr< BrofilerDurangoTestMain >(
            new BrofilerDurangoTestMain( m_deviceResources ) );
    }
}
...
void App::OnSuspending( Platform::Object ^ sender,
...
void App::OnWindowSizeChanged( CoreWindow ^ sender,
                               WindowSizeChangedEventArgs ^ args ) {
    m_deviceResources->SetLogicalSize(
        Size( sender->Bounds.Width, sender->Bounds.Height ) );
    m_main->CreateWindowSizeDependentResources();
}

void App::OnVisibilityChanged( CoreWindow ^ sender,
...
void App::OnDpiChanged( DisplayInformation ^ sender, Object ^ args ) {
    // Note: The value for LogicalDpi retrieved here may not match the effective
    // DPI of the app if it is being scaled for high resolution devices. Once
    // the DPI is set on DeviceResources, you should always retrieve it using
    // the GetDpi method. See DeviceResources.cpp for more details.
...
void App::OnDisplayContentsInvalidated( DisplayInformation ^ sender,
                                        Object ^ args ) {
