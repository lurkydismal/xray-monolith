...
//
// Function for capturing a 2D texture and saving it to a file (aka a
// 'screenshot' when used on a Direct3D 12 Render Target).
//
...
// For 2D array textures and cubemaps, it captures only the first image in the
// array
...
const DDS_PIXELFORMAT DDSPF_V8U8 = { sizeof( DDS_PIXELFORMAT ),
                                     DDS_BUMPDUDV,
                                     0,
                                     16,
                                     0x00ff,
...
const DDS_PIXELFORMAT DDSPF_Q8W8V8U8 = { sizeof( DDS_PIXELFORMAT ),
                                         DDS_BUMPDUDV,
                                         0,
                                         32,
                                         0x000000ff,
...
const DDS_PIXELFORMAT DDSPF_V16U16 = { sizeof( DDS_PIXELFORMAT ),
                                       DDS_BUMPDUDV,
                                       0,
                                       32,
                                       0x0000ffff,
...
// DXGI_FORMAT_R10G10B10A2_UNORM should be written using DX10 extension to avoid
// D3DX 10:10:10:2 reversal issue

// This indicates the DDS_HEADER_DXT10 extension is present (the format is in
// dxgiFormat)
...
public:
    ...
    ~auto_delete_file() {
        if ( m_handle ) {
            ...
            ( void )SetFileInformationByHandle( m_handle, FileDispositionInfo,
                                                &info, sizeof( info ) );
        }
    }
...
HRESULT GetSurfaceInfo( _In_ size_t width,
    ...
    if ( bc ) {
        ...
        if ( width > 0 ) {
            numBlocksWide =
                std::max< uint64_t >( 1u, ( uint64_t( width ) + 3u ) / 4u );
        }
        uint64_t numBlocksHigh = 0;
        if ( height > 0 ) {
            numBlocksHigh =
                std::max< uint64_t >( 1u, ( uint64_t( height ) + 3u ) / 4u );
        }
        ...
        numRows = uint64_t( height ) *
                  2u; // Direct3D makes this simplifying assumption, although it
                      // is larger than the 4:1:1 data
        numBytes = rowBytes * numRows;
    } else if ( planar ) {
        rowBytes = ( ( uint64_t( width ) + 1u ) >> 1 ) * bpe;
        numBytes = ( rowBytes * uint64_t( height ) ) +
                   ( ( rowBytes * uint64_t( height ) + 1u ) >> 1 );
        ...
        rowBytes =
            ( uint64_t( width ) * bpp + 7u ) / 8u; // round up to nearest byte
        numRows = uint64_t( height );
        numBytes = rowBytes * height;
    }
...
HRESULT CaptureTexture( _In_ ID3D12Device* device,
    ...
    HRESULT hr =
        pSource->GetHeapProperties( &sourceHeapProperties, &sourceHeapFlags );
    if ( FAILED( hr ) )
        return hr;
    ...
    if ( desc.SampleDesc.Count > 1 ) {
        // MSAA content must be resolved before being copied to a staging
        // texture
        auto descCopy = desc;
        ...
        hr = device->CreateCommittedResource(
            &defaultHeapProperties, D3D12_HEAP_FLAG_NONE, &descCopy,
        ...
        D3D12_FEATURE_DATA_FORMAT_SUPPORT formatInfo = {
            fmt, D3D12_FORMAT_SUPPORT1_NONE, D3D12_FORMAT_SUPPORT2_NONE };
        ...
        for ( UINT item = 0; item < desc.DepthOrArraySize; ++item ) {
            for ( UINT level = 0; level < desc.MipLevels; ++level ) {
                UINT index = D3D12CalcSubresource(
                    level, item, 0, desc.MipLevels, desc.DepthOrArraySize );
                commandList->ResolveSubresource( pTemp.Get(), index, pSource,
                                                 index, fmt );
            }
        }
    ...
    TransitionResource( commandList.Get(), pSource, beforeState,
    ...
    TransitionResource( commandList.Get(), pSource,
    ...
    pCommandQ->ExecuteCommandLists(
        1, CommandListCast( commandList.GetAddressOf() ) );
...
DirectX::SaveDDSTextureToFile( ID3D12CommandQueue* pCommandQ,
    ...
    device->GetCopyableFootprints( &desc, 0, 1, 0, nullptr, &fpRowCount,
                                   &fpRowPitch, &totalResourceSize );
    ...
    ComPtr< ID3D12Resource > pStaging;
    HRESULT hr = CaptureTexture( device.Get(), pCommandQ, pSource, dstRowPitch,
                                 desc, pStaging, beforeState, afterState );
    if ( FAILED( hr ) )
        return hr;

    // Create file
    ScopedHandle hFile( safe_handle(
        CreateFile2( fileName, GENERIC_WRITE, 0, CREATE_ALWAYS, nullptr ) ) );
    if ( !hFile )
    ...
    *reinterpret_cast< uint32_t* >( &fileHeader[ 0 ] ) = DDS_MAGIC;
    ...
    auto header = reinterpret_cast< DDS_HEADER* >( &fileHeader[ 0 ] +
                                                   sizeof( uint32_t ) );
    ...
    switch ( desc.Format ) {
        ...
        case DXGI_FORMAT_B5G5R5A1_UNORM:
            ...
            break;
        case DXGI_FORMAT_R8G8_SNORM:
            memcpy_s( &header->ddspf, sizeof( header->ddspf ), &DDSPF_V8U8,
                      sizeof( DDS_PIXELFORMAT ) );
            break;
        case DXGI_FORMAT_R8G8B8A8_SNORM:
            memcpy_s( &header->ddspf, sizeof( header->ddspf ), &DDSPF_Q8W8V8U8,
                      sizeof( DDS_PIXELFORMAT ) );
            break;
        case DXGI_FORMAT_R16G16_SNORM:
            memcpy_s( &header->ddspf, sizeof( header->ddspf ), &DDSPF_V16U16,
                      sizeof( DDS_PIXELFORMAT ) );
            break;
        case DXGI_FORMAT_B8G8R8A8_UNORM:
        ...
        default:
            ...
            extHeader = reinterpret_cast< DDS_HEADER_DXT10* >(
                fileHeader + sizeof( uint32_t ) + sizeof( DDS_HEADER ) );
    ...
    hr = GetSurfaceInfo( static_cast< size_t >( desc.Width ), desc.Height,
                         desc.Format, &slicePitch, &rowPitch, &rowCount );
    if ( FAILED( hr ) )
        return hr;
    ...
    // Write header & pixels
    DWORD bytesWritten;
    if ( !WriteFile( hFile.get(), fileHeader,
                     static_cast< DWORD >( headerSize ), &bytesWritten,
                     nullptr ) )
        return HRESULT_FROM_WIN32( GetLastError() );
    ...
    if ( !WriteFile( hFile.get(), pixels.get(),
                     static_cast< DWORD >( slicePitch ), &bytesWritten,
                     nullptr ) )
        return HRESULT_FROM_WIN32( GetLastError() );
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::SaveWICTextureToFile(
    ...
    hr = pWIC->CreateEncoder( guidContainerFormat, nullptr,
    ...
    if ( targetFormat && memcmp( &guidContainerFormat, &GUID_ContainerFormatBmp,
                                 sizeof( WICPixelFormatGUID ) ) == 0 ) {
        // Opt-in to the WIC2 support for writing 32-bit Windows BMP files with
        // an alpha channel
        PROPBAG2 option = {};
    ...
    if ( targetFormat ) {
        ...
        // Screenshots don't typically include the alpha channel of the render
        // target
        switch ( desc.Format ) {
            case DXGI_FORMAT_R32G32B32A32_FLOAT:
            case DXGI_FORMAT_R16G16B16A16_FLOAT:
    ...
    if ( targetFormat && memcmp( targetFormat, &targetGuid,
                                 sizeof( WICPixelFormatGUID ) ) != 0 ) {
    ...
    if ( SUCCEEDED(
             frame->GetMetadataQueryWriter( metawriter.GetAddressOf() ) ) ) {
        PROPVARIANT value;
        ...
        if ( memcmp( &guidContainerFormat, &GUID_ContainerFormatPng,
                     sizeof( GUID ) ) == 0 ) {
            // Set Software name
            ( void )metawriter->SetMetadataByName( L"/tEXt/{str=Software}",
                                                   &value );
            ...
            if ( sRGB ) {
                value.vt = VT_UI1;
                value.bVal = 0;
                ( void )metawriter->SetMetadataByName( L"/sRGB/RenderingIntent",
                                                       &value );
            } else {
                // add gAMA chunk with gamma 1.0
                value.vt = VT_UI4;
                value.uintVal =
                    100000; // gama value * 100,000 -- i.e. gamma 1.0
                ( void )metawriter->SetMetadataByName( L"/gAMA/ImageGamma",
                                                       &value );

                // remove sRGB chunk which is added by default.
                ( void )metawriter->RemoveMetadataByName(
                    L"/sRGB/RenderingIntent" );
            }
        } else {
            // Set Software name
            ( void )metawriter->SetMetadataByName( L"System.ApplicationName",
                                                   &value );
            ...
            if ( sRGB ) {
                ...
                value.vt = VT_UI2;
                value.uiVal = 1;
                ( void )metawriter->SetMetadataByName(
                    L"System.Image.ColorSpace", &value );
            }
        }
    }
    ...
    if ( memcmp( &targetGuid, &pfGuid, sizeof( WICPixelFormatGUID ) ) != 0 ) {
        ...
        hr = pWIC->CreateBitmapFromMemory(
            static_cast< UINT >( desc.Width ), desc.Height, pfGuid,
        ...
        hr = FC->Initialize( source.Get(), targetGuid, WICBitmapDitherTypeNone,
        ...
        WICRect rect = { 0, 0, static_cast< INT >( desc.Width ),
        ...
        hr =
            frame->WritePixels( desc.Height, static_cast< UINT >( dstRowPitch ),
                                static_cast< UINT >( imageSize ),
