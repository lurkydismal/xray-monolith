...
FrameResource::FrameResource( ID3D12Device* pDevice,
                              ...
                              ID3D12PipelineState* pShadowMapPso,
    ...
    for ( UINT i = 0; i < CommandListCount; i++ ) {
        ...
        ThrowIfFailed( pDevice->CreateCommandList(
            0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocators[ i ].Get(),
            m_pipelineState.Get(), IID_PPV_ARGS( &m_commandLists[ i ] ) ) );
    ...
    CD3DX12_RESOURCE_DESC shadowTexDesc(
        ...
        static_cast< UINT >( pViewport->Height ), 1, 1,
        DXGI_FORMAT_R32_TYPELESS, 1, 0, D3D12_TEXTURE_LAYOUT_UNKNOWN,
        D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL );

    D3D12_CLEAR_VALUE clearValue; // Performance tip: Tell the runtime at
    ...
    ThrowIfFailed( pDevice->CreateCommittedResource(
        ...
        D3D12_HEAP_FLAG_NONE, &shadowTexDesc, D3D12_RESOURCE_STATE_DEPTH_WRITE,
        &clearValue, IID_PPV_ARGS( &m_shadowTexture ) ) );
    ...
    const UINT dsvDescriptorSize = pDevice->GetDescriptorHandleIncrementSize(
        D3D12_DESCRIPTOR_HEAP_TYPE_DSV );
    CD3DX12_CPU_DESCRIPTOR_HANDLE depthHandle(
        pDsvHeap->GetCPUDescriptorHandleForHeapStart(), 1 + frameResourceIndex,
        dsvDescriptorSize ); // + 1 for the shadow map.
    ...
    pDevice->CreateDepthStencilView( m_shadowTexture.Get(),
                                     &depthStencilViewDesc, depthHandle );
    ...
    const UINT textureCount = _countof(
        SampleAssets::Textures ); // Diffuse + normal textures near the start of
                                  // the heap.  Ideally, track descriptor heap
                                  // contents/offsets at a higher level.
    const UINT cbvSrvDescriptorSize = pDevice->GetDescriptorHandleIncrementSize(
    ...
    cbvSrvCpuHandle.Offset(
        nullSrvCount + textureCount + ( frameResourceIndex * FrameCount ),
    ...
    shadowSrvDesc.Shader4ComponentMapping =
        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
    pDevice->CreateShaderResourceView( m_shadowTexture.Get(), &shadowSrvDesc,
                                       cbvSrvCpuHandle );
    ...
    ThrowIfFailed( m_sceneConstantBuffer->Map(
        0, &readRange,
    ...
    const UINT batchSize =
        _countof( m_sceneCommandLists ) + _countof( m_shadowCommandLists ) + 3;
    ...
    m_batchSubmit[ _countof( m_shadowCommandLists ) + 1 ] =
        m_commandLists[ CommandListMid ].Get();
    memcpy( m_batchSubmit + _countof( m_shadowCommandLists ) + 2,
            m_sceneCommandLists,
...
// this frame resource.
void FrameResource::WriteConstantBuffers( D3D12_VIEWPORT* pViewport,
                                          Camera* pSceneCamera,
    ...
    ::XMStoreFloat4x4( &shadowConsts.model,
                       XMMatrixScaling( 0.1f, 0.1f, 0.1f ) );
    ...
    shadowConsts.ambientColor =
        sceneConsts.ambientColor = { 0.1f, 0.2f, 0.3f, 1.0f };

    {
        OPTICK_EVENT( "MemCpy_sceneConsts" );
        memcpy( mp_sceneConstantBufferWO, &sceneConsts,
                sizeof( SceneConstantBuffer ) );
    }

    {
        OPTICK_EVENT( "MemCpy_sceneConsts" );
        memcpy( mp_shadowConstantBufferWO, &shadowConsts,
                sizeof( SceneConstantBuffer ) );
    }
}
...
void FrameResource::Init() {
    ...
    for ( int i = 0; i < CommandListCount; i++ ) {
        ...
        ThrowIfFailed( m_commandLists[ i ]->Reset(
            m_commandAllocators[ i ].Get(), m_pipelineState.Get() ) );
    }

    // Clear the depth stencil buffer in preparation for rendering the shadow
    // map.
    {
        OPTICK_EVENT( "ClearDepthStencilView" );
        m_commandLists[ CommandListPre ]->ClearDepthStencilView(
            m_shadowDepthView, D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, nullptr );
    }
...
void FrameResource::SwapBarriers() {
    ...
    m_commandLists[ CommandListMid ]->ResourceBarrier(
        1, &CD3DX12_RESOURCE_BARRIER::Transition(
               m_shadowTexture.Get(), D3D12_RESOURCE_STATE_DEPTH_WRITE,
...
void FrameResource::Finish() {
    m_commandLists[ CommandListPost ]->ResourceBarrier(
        ...
        &CD3DX12_RESOURCE_BARRIER::Transition(
            m_shadowTexture.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
            D3D12_RESOURCE_STATE_DEPTH_WRITE ) );
}

// Sets up the descriptor tables for the worker command list to use
// resources provided by frame resource.
void FrameResource::Bind( ID3D12GraphicsCommandList* pCommandList,
                          BOOL scenePass,
    ...
    if ( scenePass ) {
        ...
        pCommandList->SetGraphicsRootDescriptorTable(
            2, m_shadowDepthHandle ); // Set the shadow texture as an SRV.
        ...
        pCommandList->SetGraphicsRootDescriptorTable(
            2, m_nullSrvHandle ); // Set a null SRV for the shadow texture.
        pCommandList->SetGraphicsRootDescriptorTable( 1, m_shadowCbvHandle );

        pCommandList->OMSetRenderTargets(
            0, nullptr, FALSE, &m_shadowDepthView ); // No render target needed
                                                     // for the shadow pass.
    }
}
