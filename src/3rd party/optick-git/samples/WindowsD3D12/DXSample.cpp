...
DXSample::DXSample( UINT width, UINT height, std::wstring name )
    : m_width( width ),
      ...
      m_prefferedAdapter( DefaultAdapter ) {
...
_Use_decl_annotations_ void DXSample::GetHardwareAdapter(
    ...
    *ppAdapter = nullptr;
    ...
    for ( UINT adapterIndex = 0;
        ...
        if ( m_prefferedAdapter == NVidiaAdapter &&
             !wcsstr( desc.Description, L"NVIDIA" ) )
            continue;

        if ( m_prefferedAdapter == IntelAdapter &&
             !wcsstr( desc.Description, L"Intel" ) )
            continue;
...
_Use_decl_annotations_ void DXSample::ParseCommandLineArgs( WCHAR* argv[],
                                                            int argc ) {
    for ( int i = 1; i < argc; ++i ) {
        ...
        if ( _wcsnicmp( argv[ i ], L"-nvidia", wcslen( argv[ i ] ) ) == 0 ||
             _wcsnicmp( argv[ i ], L"/nvidia", wcslen( argv[ i ] ) ) == 0 ) {
            m_prefferedAdapter = NVidiaAdapter;
        }

        if ( _wcsnicmp( argv[ i ], L"-intel", wcslen( argv[ i ] ) ) == 0 ||
             _wcsnicmp( argv[ i ], L"/intel", wcslen( argv[ i ] ) ) == 0 ) {
            m_prefferedAdapter = IntelAdapter;
        }
    }
}
