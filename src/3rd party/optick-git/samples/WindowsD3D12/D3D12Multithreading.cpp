...
bool OnOptickStateChanged( Optick::State::Type state ) {
    ...
    if ( state == Optick::State::DUMP_CAPTURE ) {
        ...
        // Attach screenshot
        Optick::AttachFile( Optick::File::OPTICK_IMAGE, "Screenshot.jpg",
...
D3D12Multithreading::D3D12Multithreading( UINT width,
                                          UINT height,
                                          std::wstring name )
...
void D3D12Multithreading::LoadPipeline() {
    ...
    // Create descriptor heaps.
    {
        ...
        const UINT nullSrvCount =
            2; // Null descriptors are needed for out of bounds behavior reads.
        const UINT cbvCount = FrameCount * 2;
        const UINT srvCount =
            _countof( SampleAssets::Textures ) + ( FrameCount * 1 );
        D3D12_DESCRIPTOR_HEAP_DESC cbvSrvHeapDesc = {};
        cbvSrvHeapDesc.NumDescriptors = nullSrvCount + cbvCount + srvCount;
        cbvSrvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
        cbvSrvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
        ThrowIfFailed( m_device->CreateDescriptorHeap(
            &cbvSrvHeapDesc, IID_PPV_ARGS( &m_cbvSrvHeap ) ) );
...
void D3D12Multithreading::LoadAssets() {
    // Create the root signature.
    {
        ...
        ranges[ 4 ]; // Perfomance TIP: Order from most frequent to least
                     // frequent.
        ranges[ 0 ].Init(
            D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 1, 0,
            D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC ); // 2 frequently changed
                                                       // diffuse + normal
                                                       // textures - using
                                                       // registers t1 and t2.
        ranges[ 1 ].Init(
            D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 0, 0,
            D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC ); // 1 frequently changed
                                                       // constant buffer.
        ranges[ 2 ].Init( D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1,
                          0 ); // 1 infrequently changed shadow texture -
                               // starting in register t0.
        ranges[ 3 ].Init( D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, 2,
                          0 ); // 2 static samplers.
    ...
    // Create the pipeline state, which includes loading shaders.
    {
        ...
        ThrowIfFailed( D3DCompileFromFile(
            GetAssetFullPath( L"shaders.hlsl" ).c_str(), nullptr, nullptr,
        ...
        inputLayoutDesc.NumElements =
            _countof( SampleAssets::StandardVertexDescription );
        ...
        ThrowIfFailed( m_device->CreateGraphicsPipelineState(
            &psoDesc, IID_PPV_ARGS( &m_pipelineStateShadowMap ) ) );
    ...
    ThrowIfFailed( m_device->CreateCommandList(
        ...
        m_pipelineState.Get(), IID_PPV_ARGS( &commandList ) ) );
    ...
    // Create the depth stencil.
    {
        ...
        D3D12_CLEAR_VALUE
        clearValue; // Performance tip: Tell the runtime at resource
                    // creation the desired clear value.
        ...
        ThrowIfFailed( m_device->CreateCommittedResource(
            ...
            D3D12_HEAP_FLAG_NONE, &shadowTextureDesc,
    ...
    ThrowIfFailed( ReadDataFromFile(
        GetAssetFullPath( SampleAssets::DataFileName ).c_str(), &pAssetData,
        &fileSize ) );
    ...
    // Create the vertex buffer.
    {
...

        {
            ...
            // PIXBeginEvent(commandList.Get(), 0, L"Copy vertex buffer data to
            // default resource...");

            UpdateSubresources< 1 >( commandList.Get(), m_vertexBuffer.Get(),
                                     m_vertexBufferUpload.Get(), 0, 0, 1,
                                     &vertexData );
    ...
    // Create the index buffer.
    {
...

        {
            ...
            // PIXBeginEvent(commandList.Get(), 0, L"Copy index buffer data to
            // default resource...");

            UpdateSubresources< 1 >( commandList.Get(), m_indexBuffer.Get(),
                                     m_indexBufferUpload.Get(), 0, 0, 1,
                                     &indexData );
            commandList->ResourceBarrier(
                1, &CD3DX12_RESOURCE_BARRIER::Transition(
    ...
    // Create shader resources.
    {
        ...
        {
            // Describe and create 2 null SRVs. Null descriptors are needed in
        ...
        // PIXBeginEvent(commandList.Get(), 0, L"Copy diffuse and normal texture
        // data to default resources...");
        for ( UINT i = 0; i < srvCount; i++ ) {
            // Describe and create a Texture2D.
            const SampleAssets::TextureResource& tex =
                SampleAssets::Textures[ i ];
            CD3DX12_RESOURCE_DESC texDesc(
                D3D12_RESOURCE_DIMENSION_TEXTURE2D, 0, tex.Width, tex.Height, 1,
                static_cast< UINT16 >( tex.MipLevels ), tex.Format, 1, 0,
                D3D12_TEXTURE_LAYOUT_UNKNOWN, D3D12_RESOURCE_FLAG_NONE );

            ThrowIfFailed( m_device->CreateCommittedResource(
                &CD3DX12_HEAP_PROPERTIES( D3D12_HEAP_TYPE_DEFAULT ),
                D3D12_HEAP_FLAG_NONE, &texDesc, D3D12_RESOURCE_STATE_COPY_DEST,
                nullptr, IID_PPV_ARGS( &m_textures[ i ] ) ) );

            NAME_D3D12_OBJECT_INDEXED( m_textures, i );

            {
                const UINT subresourceCount =
                    texDesc.DepthOrArraySize * texDesc.MipLevels;
                UINT64 uploadBufferSize = GetRequiredIntermediateSize(
                    m_textures[ i ].Get(), 0, subresourceCount );
                ThrowIfFailed( m_device->CreateCommittedResource(
                    &CD3DX12_HEAP_PROPERTIES( D3D12_HEAP_TYPE_UPLOAD ),
                    D3D12_HEAP_FLAG_NONE,
                ...
                // Copy data to the intermediate upload heap and then schedule a
                // copy from the upload heap to the Texture2D.
                D3D12_SUBRESOURCE_DATA textureData = {};
                textureData.pData = pAssetData + tex.Data->Offset;
                textureData.RowPitch = tex.Data->Pitch;
                textureData.SlicePitch = tex.Data->Size;

                UpdateSubresources( commandList.Get(), m_textures[ i ].Get(),
                                    m_textureUploads[ i ].Get(), 0, 0,
                                    subresourceCount, &textureData );
                commandList->ResourceBarrier(
                    1,
                    &CD3DX12_RESOURCE_BARRIER::Transition(
                        m_textures[ i ].Get(), D3D12_RESOURCE_STATE_COPY_DEST,
            ...
            m_device->CreateShaderResourceView( m_textures[ i ].Get(), &srvDesc,
                                                cbvSrvHandle );
    ...
    // Create the samplers.
    {
        ...
        const UINT samplerDescriptorSize =
            m_device->GetDescriptorHandleIncrementSize(
    ...
    for ( int i = 0; i < NumLights; i++ ) {
        ...
        XMVECTOR at =
            XMVectorAdd( eye, XMLoadFloat4( &m_lights[ i ].direction ) );
        XMVECTOR up = { 0, 1, 0 };
    ...
    for ( int i = 0; i < FrameCount; i++ ) {
        m_frameResources[ i ] =
            new FrameResource( m_device.Get(), m_pipelineState.Get(),
                               m_pipelineStateShadowMap.Get(), m_dsvHeap.Get(),
                               m_cbvSrvHeap.Get(), &m_viewport, i );
        m_frameResources[ i ]->WriteConstantBuffers( &m_viewport, &m_camera,
                                                     m_lightCameras, m_lights );
    }
    m_currentFrameResourceIndex = 0;
...
void D3D12Multithreading::LoadContexts() {
    ...
    struct threadwrapper {
        static unsigned int WINAPI thunk( LPVOID lpParameter ) {
            ThreadParameter* parameter =
                reinterpret_cast< ThreadParameter* >( lpParameter );
    ...
    for ( int i = 0; i < NumContexts; i++ ) {
        ...
        m_threadHandles[ i ] = reinterpret_cast< HANDLE >( _beginthreadex(
            nullptr, 0, threadwrapper::thunk,
            reinterpret_cast< LPVOID >( &m_threadParameters[ i ] ), 0,
            nullptr ) );
...
void D3D12Multithreading::OnUpdate() {
    OPTICK_CATEGORY( "OnUpdate", Optick::Category::Scene );

    m_timer.Tick( NULL );

    // PIXSetMarker(m_commandQueue.Get(), 0, L"Getting last completed fence.");

    // Get current GPU progress against submitted workload. Resources still
    // scheduled for GPU execution cannot be modified or else undefined behavior
    // will result.
    const UINT64 lastCompletedFence = m_fence->GetCompletedValue();

    // Move to the next frame resource.
    m_currentFrameResourceIndex =
        ( m_currentFrameResourceIndex + 1 ) % FrameCount;
    m_pCurrentFrameResource = m_frameResources[ m_currentFrameResourceIndex ];
    ...
    if ( m_pCurrentFrameResource->m_fenceValue > lastCompletedFence ) {
        ...
        ThrowIfFailed( m_fence->SetEventOnCompletion(
            m_pCurrentFrameResource->m_fenceValue, eventHandle ) );
    ...
    if ( m_keyboardInput.animate ) {
        for ( int i = 0; i < NumLights; i++ ) {
            ...
            XMStoreFloat4(
                &m_lights[ i ].position,
                XMVector4Transform( XMLoadFloat4( &m_lights[ i ].position ),
                                    XMMatrixRotationY( direction ) ) );

            XMVECTOR eye = XMLoadFloat4( &m_lights[ i ].position );
            XMVECTOR at = { 0.0f, 8.0f, 0.0f };
            XMStoreFloat4( &m_lights[ i ].direction,
                           XMVector3Normalize( XMVectorSubtract( at, eye ) ) );
            XMVECTOR up = { 0.0f, 1.0f, 0.0f };
            m_lightCameras[ i ].Set( eye, at, up );

            m_lightCameras[ i ].Get3DViewProjMatrices(
                &m_lights[ i ].view, &m_lights[ i ].projection, 90.0f,
                static_cast< float >( m_width ),
                static_cast< float >( m_height ) );
        }
    }

    m_pCurrentFrameResource->WriteConstantBuffers( &m_viewport, &m_camera,
                                                   m_lightCameras, m_lights );
}
...
void D3D12Multithreading::OnRender() {
    ...
    m_commandQueue->ExecuteCommandLists(
        _countof( m_pCurrentFrameResource->m_batchSubmit ),
        m_pCurrentFrameResource->m_batchSubmit );
#else
    for ( int i = 0; i < NumContexts; i++ ) {
        OPTICK_EVENT( "SetEvent" );
        SetEvent( m_workerBeginRenderFrame[ i ] ); // Tell each worker to start
                                                   // drawing.
    }
...

    {
        ...
        WaitForMultipleObjects( NumContexts, m_workerFinishShadowPass, TRUE,
                                INFINITE );
    }
    ...
    // Submit remaining command lists.
    {
        OPTICK_EVENT( "ExecuteCommandLists" );
        m_commandQueue->ExecuteCommandLists(
            _countof( m_pCurrentFrameResource->m_batchSubmit ) - NumContexts -
                2,
    ...
    if ( g_TakingScreenshot ) {
        ThrowIfFailed( SaveWICTextureToFile(
            m_commandQueue.Get(), m_renderTargets[ m_frameIndex ].Get(),
...
void D3D12Multithreading::MidFrame() {
    ...
    // Transition our shadow map from the shadow pass to readable in the scene
    // pass.
...
void D3D12Multithreading::WorkerThread( int threadIndex ) {
    ...
    while ( threadIndex >= 0 && threadIndex < NumContexts ) {
        ...
        WaitForSingleObject( m_workerBeginRenderFrame[ threadIndex ],
                             INFINITE );

        OPTICK_CATEGORY( "Render", Optick::Category::Rendering );

#endif
        ID3D12GraphicsCommandList* pShadowCommandList =
            m_pCurrentFrameResource->m_shadowCommandLists[ threadIndex ].Get();
        ID3D12GraphicsCommandList* pSceneCommandList =
            m_pCurrentFrameResource->m_sceneCommandLists[ threadIndex ].Get();
        ...
        m_pCurrentFrameResource->Bind(
            pShadowCommandList, FALSE, nullptr,
            nullptr ); // No need to pass RTV or DSV descriptor heap.

        // Set null SRVs for the diffuse/normal textures.
        {
            OPTICK_EVENT( "SetGraphicsRootDescriptorTable" );
            pShadowCommandList->SetGraphicsRootDescriptorTable(
        ...
        // PIXBeginEvent(pShadowCommandList, 0, L"Worker drawing shadow
        // pass...");

        {
            ...
            OPTICK_TAG( "Count",
                        _countof( SampleAssets::Draws ) / NumContexts );
            for ( int j = threadIndex; j < _countof( SampleAssets::Draws );
                  j += NumContexts ) {
                // OPTICK_GPU_EVENT("DrawIndexedInstanced");
                SampleAssets::DrawParameters drawArgs =
                    SampleAssets::Draws[ j ];

                OPTICK_EVENT( "DrawIndexedInstanced" );
                OPTICK_TAG( "Name", SampleAssets::DebugNames[ j ] );
                pShadowCommandList->DrawIndexedInstanced(
                    drawArgs.IndexCount, 1, drawArgs.IndexStart,
                    drawArgs.VertexBase, 0 );
            }
        }
        ...
        CD3DX12_CPU_DESCRIPTOR_HANDLE dsvHandle(
            m_dsvHeap->GetCPUDescriptorHandleForHeapStart() );
        m_pCurrentFrameResource->Bind( pSceneCommandList, TRUE, &rtvHandle,
                                       &dsvHandle );
...

        {
            ...
            for ( int j = threadIndex; j < _countof( SampleAssets::Draws );
                ...
                CD3DX12_GPU_DESCRIPTOR_HANDLE cbvSrvHandle(
                    cbvSrvHeapStart,
                    nullSrvCount + drawArgs.DiffuseTextureIndex,
                    cbvSrvDescriptorSize );
                {
                    OPTICK_EVENT( "SetGraphicsRootDescriptorTable" );
                    pSceneCommandList->SetGraphicsRootDescriptorTable(
                        0, cbvSrvHandle );
                }
                ...
                // OPTICK_GPU_EVENT("DrawIndexedInstanced");
                {
                    OPTICK_EVENT( "DrawIndexedInstanced" );
                    OPTICK_TAG( "Name", SampleAssets::DebugNames[ j ] );
                    pSceneCommandList->DrawIndexedInstanced(
                        drawArgs.IndexCount, 1, drawArgs.IndexStart,
                        drawArgs.VertexBase, 0 );
                }
            }
...
void D3D12Multithreading::SetCommonPipelineState(
