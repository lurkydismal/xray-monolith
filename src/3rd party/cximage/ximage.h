...
/*
  ...
  --------------------------------------------------------------------------------
        ...
        Other information about CxImage, and the latest version, can be found at
  the CxImage home page: http://www.xdp.it/cximage/
...
enum ENUM_CXIMAGE_FORMATS {
    ...
    CMAX_IMAGE_FORMATS =
        ...
        CXIMAGE_SUPPORT_WBMP + CXIMAGE_SUPPORT_WMF + CXIMAGE_SUPPORT_JBG +
        CXIMAGE_SUPPORT_JP2 + CXIMAGE_SUPPORT_JPC + CXIMAGE_SUPPORT_PGX +
        CXIMAGE_SUPPORT_PNM + CXIMAGE_SUPPORT_RAS + CXIMAGE_SUPPORT_SKA +
        CXIMAGE_SUPPORT_RAW + 1
};
...
class DLL_EXP CxImage {
    ...
    typedef struct tagCxImageInfo {
        ...
        long yOffset;
        DWORD dwCodecOpt
            [ CMAX_IMAGE_FORMATS ]; ///< for GIF, TIF :
                                    ///< 0=def.1=unc,2=fax3,3=fax4,4=pack,5=jpg
        RGBQUAD last_c;             ///< for GetNearestIndex optimization
        BYTE last_c_index;
        bool last_c_isvalid;
        long nNumLayers;
        DWORD dwFlags; ///< 0x??00000 = reserved, 0x00??0000 = blend mode,
                       ///< 0x0000???? = layer id - user flags
        BYTE dispmeth;
        bool bGetAllFrames;
...
public:
    ...
    typedef struct tagCxTextInfo {
        ...
        long
            align; ///< DT_CENTER, DT_RIGHT, DT_LEFT aligment for multiline text
        ...
        float b_opacity; ///< opacity value for background between 0.0-1.0
                         ///< Default is 0. (opaque)
        BYTE b_outline;  ///< outline width for background (zero: no outline)
        BYTE b_round;    ///< rounding radius for background rectangle. % of the
                      ///< height, between 0-50. Default is 10. (backgr. always
                      ///< has a frame: width = 3 pixel + 10% of height by
                      ///< default.)
    } CXTEXTINFO;
#endif
...
public:
    ...
    CxImage( const CxImage& src,
             ...
             bool copyalpha = true );
    CxImage( const TCHAR* filename,
             DWORD imagetype ); // For UNICODE support: char -> TCHAR
    ...
    bool CreateFromArray( BYTE* pArray,
                          DWORD dwWidth,
                          DWORD dwHeight,
                          DWORD dwBitsperpixel,
                          DWORD dwBytesperline,
                          bool bFlipImage );
    bool CreateFromMatrix( BYTE** ppMatrix,
                           DWORD dwWidth,
                           DWORD dwHeight,
                           DWORD dwBitsperpixel,
                           DWORD dwBytesperline,
    ...
    /** \addtogroup Palette
     * These functions have no effects on RGB images and in this case the
...
protected:
    ...
    RGBQUAD BlindGetPixelColor( const long x,
                                const long y,
                                bool bGetAlpha = true );
    void* BlindGetPixelPointer( const long x, const long y );
    void BlindSetPixelColor( long x,
                             long y,
                             RGBQUAD c,
                             bool bSetAlpha = false );
...
protected:
    /** \addtogroup Protected */ //@{
    void AddAveragingCont( RGBQUAD const& color,
                           float const surf,
                           float& rr,
                           float& gg,
                           float& bb,
...
/** \addtogroup Painting */ //@{
    ...
    bool CreateFromHBITMAP( HBITMAP hbmp,
                            HPALETTE hpal = 0 ); // Windows resource
    ...
    // long	DrawString(HDC hdc, long x, long y, const char* text, RGBQUAD
    // color, const char* font, long lSize=0, long lWeight=400, BYTE bItalic=0,
    // BYTE bUnderline=0, bool bSetAlpha=false);
    long DrawString( HDC hdc,
                     long x,
                     long y,
                     const TCHAR* text,
                     RGBQUAD color,
...
//@}
    ...
    bool CropRotatedRectangle( long topx,
                               long topy,
                               long width,
                               long height,
                               float angle,
                               CxImage* iDst = NULL );
    bool Skew( float xgain,
               float ygain,
               long xpivot = 0,
               long ypivot = 0,
               bool bEnableInterpolation = false );
    bool Expand( long left,
                 long top,
                 long right,
                 long bottom,
                 RGBQUAD canvascolor,
...
//@}
    ...
    bool HistogramRoot();
    bool HistogramLog();
    long Histogram( long* red,
                    long* green = 0,
                    long* blue = 0,
                    long* gray = 0,
                    long colorspace = 0 );
    bool Jitter( long radius = 2 );
    bool Repair( float radius = 0.25f,
                 long niterations = 1,
                 long colorspace = 0 );
    bool Combine( CxImage* r,
                  CxImage* g,
                  CxImage* b,
                  CxImage* a,
                  long colorspace = 0 );
    bool FFT2( CxImage* srcReal,
               CxImage* srcImag,
               CxImage* dstReal,
               CxImage* dstImag,
               long direction = 1,
    ...
    bool ConvertColorSpace( const long dstColorSpace,
                            const long srcColorSpace );
    int OptimalThreshold( long method = 0,
                          RECT* pBox = 0,
                          CxImage* pContrastMask = 0 );
    bool AdaptiveThreshold( long method = 0,
                            long nBoxSize = 64,
                            CxImage* pContrastMask = 0,
                            long nBias = 0,
