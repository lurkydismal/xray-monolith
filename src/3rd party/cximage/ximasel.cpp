...
bool CxImage::SelectionAddRect( RECT r, BYTE level ) {
    ...
    if ( info.rSelectionBox.top <= r2.top )
        info.rSelectionBox.top = max( 0L, min( head.biHeight, r2.top + 1 ) );
    if ( info.rSelectionBox.left > r2.left )
        info.rSelectionBox.left = max( 0L, min( head.biWidth, r2.left ) );
    if ( info.rSelectionBox.right <= r2.right )
        info.rSelectionBox.right = max( 0L, min( head.biWidth, r2.right + 1 ) );
    if ( info.rSelectionBox.bottom > r2.bottom )
        info.rSelectionBox.bottom = max( 0L, min( head.biHeight, r2.bottom ) );
...
bool CxImage::SelectionAddEllipse( RECT r, BYTE level ) {
    ...
    if ( info.rSelectionBox.left > ( xcenter - xradius ) )
        info.rSelectionBox.left =
            max( 0L, min( head.biWidth, ( xcenter - xradius ) ) );
    if ( info.rSelectionBox.right <= ( xcenter + xradius ) )
        info.rSelectionBox.right =
            max( 0L, min( head.biWidth, ( xcenter + xradius + 1 ) ) );
    if ( info.rSelectionBox.bottom > ( ycenter - yradius ) )
        info.rSelectionBox.bottom =
            max( 0L, min( head.biHeight, ( ycenter - yradius ) ) );
    if ( info.rSelectionBox.top <= ( ycenter + yradius ) )
        info.rSelectionBox.top =
            max( 0L, min( head.biHeight, ( ycenter + yradius + 1 ) ) );
    ...
    for ( y = ymin; y < min( ycenter, ymax ); y++ ) {
        for ( long x = xmin; x < xmax; x++ ) {
            yo = ( long )( ycenter -
                           yradius * sqrt( 1 - pow( ( float )( x - xcenter ) /
                                                        ( float )xradius,
                                                    2 ) ) );
            if ( yo < y )
 ...
 * Note: the SelectionBox is set to "full image", call SelectionGetBox before
 * (if necessary)
 */
...
bool CxImage::SelectionCopy( CxImage& from ) {
    if ( from.pSelection == NULL || head.biWidth != from.head.biWidth ||
        ...
        return false;
    if ( pSelection == NULL )
        pSelection = ( BYTE* )cxalloc(
            head.biWidth *
            head.biHeight ); // malloc(head.biWidth * head.biHeight);
    if ( pSelection == NULL )
        return false;
 ...
 * of POINT structures. Each structure specifies the x-coordinate and
 * y-coordinate of one vertex of the polygon. npoints specifies the number of
 * POINT structures in the array pointed to by points.
 */
bool CxImage::SelectionAddPolygon( POINT* points, long npoints, BYTE level ) {
    ...
    while ( i < npoints ) {
        ...
        if ( localbox.top < r2.top )
            localbox.top = max( 0L, min( head.biHeight - 1, r2.top + 1 ) );
        if ( localbox.left > r2.left )
            localbox.left = max( 0L, min( head.biWidth - 1, r2.left - 1 ) );
        if ( localbox.right < r2.right )
            localbox.right = max( 0L, min( head.biWidth - 1, r2.right + 1 ) );
        if ( localbox.bottom > r2.bottom )
            localbox.bottom =
    ...
    long npix =
        ( localbox.right - localbox.left ) * ( localbox.top - localbox.bottom );
    ...
    for ( int side = 0; side < 4; side++ ) {
        switch ( side ) {
            case 0:
                xmin = localbox.left;
                xmax = localbox.right + 1;
                ymin = localbox.bottom;
                ymax = localbox.bottom + 1;
                break;
            case 1:
                xmin = localbox.right;
                xmax = localbox.right + 1;
        ...
        for ( y = ymin; y < ymax; y++ ) {
            for ( x = xmin; x < xmax; x++ ) {
                if ( plocal[ x + y * head.biWidth ] == 0 ) {
                    // Subject: FLOOD FILL ROUTINE              Date: 12-23-97
                    // (00:57) Author:  Petter Holmberg                 Code:
                    // QB, QBasic, PDS Origin:  petter.holmberg@usa.net Packet:
                    // GRAPHICS.ABC
                    first = 0;
                    last = 1;
                    while ( first != last ) {
                        ...
                        fyy = fy + y;
                        for ( ;; ) {
                            if ( ( plocal[ fxx + fyy * head.biWidth ] ==
                                   back ) &&
                                 fxx >= localbox.left &&
                                 fxx <= localbox.right &&
                                 fyy >= localbox.bottom &&
    ...
    for ( y = localbox.bottom; y <= localbox.top; y++ ) {
        ...
        for ( x = localbox.left; x <= localbox.right; x++ )
            if ( plocal[ x + yoffset ] != 1 )
                pSelection[ x + yoffset ] = level;
    }
    if ( info.rSelectionBox.top <= localbox.top )
        info.rSelectionBox.top = min( head.biHeight, localbox.top + 1 );
    if ( info.rSelectionBox.left > localbox.left )
        info.rSelectionBox.left = min( head.biWidth, localbox.left );
...
bool CxImage::SelectionAddColor( RGBQUAD c, BYTE level ) {
    ...
    for ( long y = 0; y < head.biHeight; y++ ) {
        for ( long x = 0; x < head.biWidth; x++ ) {
            ...
            if ( color.rgbRed == c.rgbRed && color.rgbGreen == c.rgbGreen &&
    ...
    if ( info.rSelectionBox.top <= localbox.top )
        info.rSelectionBox.top = localbox.top + 1;
    if ( info.rSelectionBox.left > localbox.left )
        info.rSelectionBox.left = localbox.left;
    if ( info.rSelectionBox.right <= localbox.right )
        info.rSelectionBox.right = localbox.right + 1;
    if ( info.rSelectionBox.bottom > localbox.bottom )
        info.rSelectionBox.bottom = localbox.bottom;
...
bool CxImage::SelectionAddPixel( long x, long y, BYTE level ) {
    ...
    if ( IsInside( x, y ) ) {
        ...
        if ( info.rSelectionBox.top <= y )
            info.rSelectionBox.top = y + 1;
        if ( info.rSelectionBox.left > x )
            info.rSelectionBox.left = x;
        if ( info.rSelectionBox.right <= x )
            info.rSelectionBox.right = x + 1;
        if ( info.rSelectionBox.bottom > y )
...
bool CxImage::SelectionSet( CxImage& from ) {
    if ( !from.IsGrayScale() || head.biWidth != from.head.biWidth ||
         head.biHeight != from.head.biHeight ) {
        strcpy( info.szLastError,
                "CxImage::SelectionSet: wrong width or height, or image is not "
                "gray scale" );
        return false;
    }
...
bool CxImage::SelectionMirror() {
    if ( !pSelection )
        return false;
    BYTE* pSelection2 = ( BYTE* )cxalloc(
        head.biWidth * head.biHeight ); // malloc(head.biWidth * head.biHeight);
    if ( !pSelection2 )
        return false;
