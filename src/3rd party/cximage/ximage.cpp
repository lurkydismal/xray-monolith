...
void CxImage::Copy( const CxImage& src,
    ...
    memcpy( &head, &src.head,
            sizeof( BITMAPINFOHEADER ) ); // [andy] - fix for bitmap header DPI
    // rebuild the image
...
void* CxImage::Create( DWORD dwWidth,
    ...
    pDib = cxalloc( GetSize() ); // malloc(GetSize()); // alloc memory block to
                                 // store our bitmap
    if ( !pDib ) {
 ...
 * \param dwBytesperline: line alignment, in bytes, for a single row stored in
 * pArray
...
bool CxImage::CreateFromArray( BYTE* pArray,
    ...
    if ( pArray == NULL )
        return false;
    if ( !( ( dwBitsperpixel == 1 ) || ( dwBitsperpixel == 4 ) ||
            ( dwBitsperpixel == 8 ) || ( dwBitsperpixel == 24 ) ||
            ( dwBitsperpixel == 32 ) ) )
        return false;

    if ( !Create( dwWidth, dwHeight, dwBitsperpixel ) )
        return false;
    ...
    for ( DWORD y = 0; y < dwHeight; y++ ) {
        dst = info.pImage +
              ( bFlipImage ? ( dwHeight - 1 - y ) : y ) * info.dwEffWidth;
        ...
        if ( dwBitsperpixel == 32 ) {
            for ( DWORD x = 0; x < dwWidth; x++ ) {
                ...
                *dst++ = src[ 2 ];
#if CXIMAGE_SUPPORT_ALPHA
                AlphaSet( x, ( bFlipImage ? ( dwHeight - 1 - y ) : y ),
                          src[ 3 ] );
#endif // CXIMAGE_SUPPORT_ALPHA
                src += 4;
            }
