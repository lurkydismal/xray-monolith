...
bool CxImageJPG::Decode( CxFile* hFile ) {
     ...
     * else, so that the setjmp() error recovery below can assume the file is
     * open. VERY IMPORTANT: use "b" option to fopen() if you are on a machine
     * that requires it in order to read binary files.
     */
...
#pragma warning( pop )
    ...
    Create( cinfo.output_width, cinfo.output_height,
            8 * cinfo.output_components, CXIMAGE_FORMAT_JPG );

    if ( !pDib )
        longjmp( jerr.setjmp_buffer,
                 1 ); //<DP> check if the image has been created

    if ( is_exif ) {
#if CXIMAGEJPG_SUPPORT_EXIF
        if ( ( m_exifinfo.Xresolution != 0.0 ) &&
             ( m_exifinfo.ResolutionUnit != 0 ) )
            SetXDPI( ( long )( m_exifinfo.Xresolution /
                               m_exifinfo.ResolutionUnit ) );
        if ( ( m_exifinfo.Yresolution != 0.0 ) &&
             ( m_exifinfo.ResolutionUnit != 0 ) )
            SetYDPI( ( long )( m_exifinfo.Yresolution /
                               m_exifinfo.ResolutionUnit ) );
#endif
    } else {
        switch ( cinfo.density_unit ) {
            case 0: // [andy] fix for aspect ratio...
                if ( ( cinfo.Y_density > 0 ) && ( cinfo.X_density > 0 ) ) {
                    SetYDPI(
                        ( long )( GetXDPI() * ( float( cinfo.Y_density ) /
                                                float( cinfo.X_density ) ) ) );
                }
                break;
    ...
    if ( cinfo.out_color_space == JCS_GRAYSCALE ) {
        ...
        if ( cinfo.quantize_colors ) {
            SetPalette( cinfo.actual_number_of_colors, cinfo.colormap[ 0 ],
                        cinfo.colormap[ 1 ], cinfo.colormap[ 2 ] );
    ...
    while ( cinfo.output_scanline < cinfo.output_height ) {
        ...
        // info.nProgress =
        // (long)(100*cinfo.output_scanline/cinfo.output_height);
        //<DP> Step 6a: CMYK->RGB */
        if ( ( cinfo.num_components == 4 ) &&
            ...
            src = buffer[ 0 ];
            for ( long x3 = 0, x4 = 0;
                  x3 < ( long )info.dwEffWidth && x4 < row_stride;
                  x3 += 3, x4 += 4 ) {
                k = src[ x4 + 3 ];
    ...
    // not necessary if swapped red and blue definition in jmorecfg.h;ln322 <W.
    // Morrison>
...
bool CxImageJPG::Encode( CxFile* hFile ) {
    ...
    if ( head.biClrUsed != 0 && !IsGrayScale() ) {
        strcpy( info.szLastError,
                "JPEG can save only RGB or GreyScale images" );
        return false;
    }
...
#pragma warning( pop )
    ...
    jpeg_set_quality(
        &cinfo, GetJpegQuality(),
        ( GetCodecOption( CXIMAGE_FORMAT_JPG ) & ENCODE_BASELINE ) != 0 );
    ...
    // SetCodecOption(ENCODE_SUBSAMPLE_444 |
    // GetCodecOption(CXIMAGE_FORMAT_JPG),CXIMAGE_FORMAT_JPG);
    ...
    if ( ( GetCodecOption( CXIMAGE_FORMAT_JPG ) & ENCODE_SUBSAMPLE_422 ) !=
         0 ) {
    ...
    if ( ( GetCodecOption( CXIMAGE_FORMAT_JPG ) & ENCODE_SUBSAMPLE_444 ) !=
         0 ) {
    ...
    while ( cinfo.next_scanline < cinfo.image_height ) {
        ...
        if ( head.biClrUsed == 0 ) { // swap R & B for RGB images
            RGBtoBGR( buffer[ 0 ],
                      row_stride ); // Lance : 1998/09/01 : Bug ID: EXP-2.1.1-9
        }
        iter.PrevRow();
