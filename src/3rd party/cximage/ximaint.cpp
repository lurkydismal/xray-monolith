 ...
 *  \author ***bd*** 2.2004
 */
void CxImage::OverflowCoordinates( long& x,
                                   long& y,
                                   OverflowMethod const ofMethod ) {
    if ( IsInside( x, y ) )
        return; // if pixel is within bounds, no change
    switch ( ofMethod ) {
        case OM_REPEAT:
            // clip coordinates
            x = max( x, 0 );
        ...
        case OM_MIRROR:
            ...
            if ( x < 0 )
                ...
                x = head.biWidth - ( x % head.biWidth + 1 );
            if ( y < 0 )
                y = ( ( -y ) % head.biHeight );
            else if ( y >= head.biHeight )
                y = head.biHeight - ( y % head.biHeight + 1 );
            break;
        default:
            return;
    } // switch
...
void CxImage::OverflowCoordinates( float& x,
                                   float& y,
                                   OverflowMethod const ofMethod ) {
    if ( x >= 0 && x < head.biWidth && y >= 0 && y < head.biHeight )
        return; // if pixel is within bounds, no change
    switch ( ofMethod ) {
        case OM_REPEAT:
            // clip coordinates
        ...
        case OM_MIRROR:
            ...
            if ( x < 0 )
                ...
                x = head.biWidth -
                    ( ( float )fmod( x, ( float )head.biWidth ) + 1 );
            if ( y < 0 )
                y = ( float )fmod( -y, ( float )head.biHeight );
            else if ( y >= head.biHeight )
                y = head.biHeight -
                    ( ( float )fmod( y, ( float )head.biHeight ) + 1 );
            break;
        default:
            return;
    } // switch
...
////////////////////////////////////////////////////////////////////////////////
/**
 * Method return pixel color. Different methods are implemented for out of
 * bounds pixels. If an image has alpha channel, alpha value is returned in
 * .RGBReserved.
 *
 ...
 *
 *  \param rplColor : input color (returned for out-of-bound coordinates in
 * OF_COLOR mode and if other mode is not applicable)
 *
 * \return color : color of pixel
 * \author ***bd*** 2.2004
 */
RGBQUAD CxImage::GetPixelColorWithOverflow( long x,
                                            long y,
                                            OverflowMethod const ofMethod,
    ...
    if ( ( !IsInside( x, y ) ) || pDib == NULL ) { // is pixel within bouns?:
        ...
        if ( rplColor != NULL )
            ...
            color.rgbReserved =
                0; // default replacement colour: white transparent
        } // if
        if ( pDib == NULL )
            return color;
       ...
       // no alpha transparency
                    if ( GetTransIndex() >= 0 ) {
                        color = GetTransColor(); // single color transparency
            ...
            case OM_BACKGROUND:
                ...
                if ( info.nBkgndIndex >= 0 ) {
                    if ( head.biBitCount < 24 )
                        color = GetPaletteColor( ( BYTE )info.nBkgndIndex );
                    else
...
////////////////////////////////////////////////////////////////////////////////
/**
 * This method reconstructs image according to chosen interpolation method and
 then returns pixel (x,y).
 * (x,y) can lie between actual image pixels. If (x,y) lies outside of image,
 method returns value
 * according to overflow method.
 * This method is very useful for geometrical image transformations, where
 destination pixel
 * can often assume color value lying between source pixels.
 *
 *  \param (x,y) - coordinates of pixel to return
 *           GPCI method recreates "analogue" image back from digital data, so x
 and y
 *           are float values and color value of point (1.1,1) will generally
 not be same
 *           as (1,1). Center of first pixel is at (0,0) and center of pixel
 right to it is (1,0).
 *           (0.5,0) is half way between these two pixels.
 *  \param inMethod - interpolation (reconstruction) method (kernel) to use:
 *    - IM_NEAREST_NEIGHBOUR - returns colour of nearest lying pixel (causes
 stairy look of
 *                            processed images)
 *    - IM_BILINEAR - interpolates colour from four neighbouring pixels (softens
 image a bit)
 *    - IM_BICUBIC - interpolates from 16 neighbouring pixels (can produce
 "halo" artifacts)
 *    - IM_BICUBIC2 - interpolates from 16 neighbouring pixels (perhaps a bit
 less halo artifacts than IM_BICUBIC)
 *    - IM_BSPLINE - interpolates from 16 neighbouring pixels (softens image,
 washes colours)
 *                  (As far as I know, image should be prefiltered for this
 method to give
 *                   good results... some other time :) )
 *                  This method uses bicubic interpolation kernel from
 CXImage 5.99a and older
 *                  versions.
 *    - IM_LANCZOS - interpolates from 12*12 pixels (slow, ringing artifacts)
 *
 *  \param ofMethod - overflow method (see comments at
 GetPixelColorWithOverflow)
 *  \param rplColor - pointer to color used for out of borders pixels in
 OM_COLOR mode
 *              (and other modes if colour can't calculated in a specified way)
 *
 *  \return interpolated color value (including interpolated alpha value, if
 image has alpha layer)
 *
 *  \author ***bd*** 2.2004
 */
RGBQUAD CxImage::GetPixelColorInterpolated( float x,
    ...
    int xi = ( int )( x );
    if ( x < 0 )
        xi--; // these replace (incredibly slow) floor (Visual c++ 2003, AMD
              // Athlon)
    int yi = ( int )( y );
    if ( y < 0 )
        yi--;
    ...
    switch ( inMethod ) {
        case IM_NEAREST_NEIGHBOUR:
            return GetPixelColorWithOverflow( ( long )( x + 0.5f ),
                                              ( long )( y + 0.5f ), ofMethod,
                                              rplColor );
        default: {
            // IM_BILINEAR: bilinear interpolation
            if ( xi < -1 || xi >= head.biWidth || yi < -1 ||
                 yi >= head.biHeight ) { // all 4 points are outside bounds?:
                switch ( ofMethod ) {
                    case OM_COLOR:
                    case OM_TRANSPARENT:
                    case OM_BACKGROUND:
                        // we don't need to interpolate anything with all points
                        // outside in this case
                        return GetPixelColorWithOverflow( -999, -999, ofMethod,
                                                          rplColor );
                    default:
                        // recalculate coordinates and use faster method later
                        // on
                        OverflowCoordinates( x, y, ofMethod );
                        xi = ( int )( x );
                        if ( x < 0 )
                            xi--; // x and/or y have changed ... recalculate xi
                                  // and yi
                        yi = ( int )( y );
                        if ( y < 0 )
                            yi--;
            ...
            if ( ( xi + 1 ) < head.biWidth && xi >= 0 &&
                 ( yi + 1 ) < head.biHeight && yi >= 0 &&
                 head.biClrUsed == 0 ) {
                // all pixels are inside RGB24 image... optimize reading (and
                // use fixed point arithmetic)
                WORD wt1 = ( WORD )( ( x - xi ) * 256.0f ),
                     wt2 = ( WORD )( ( y - yi ) * 256.0f );
                WORD wd = wt1 * wt2 >> 8;
                WORD wb = wt1 - wd;
                WORD wc = wt2 - wd;
                WORD wa = 256 - wt1 - wc;
                WORD wrr, wgg, wbb;
                BYTE* pxptr =
                    ( BYTE* )info.pImage + yi * info.dwEffWidth + xi * 3;
                wbb = wa * ( *pxptr++ );
                wgg = wa * ( *pxptr++ );
                wrr = wa * ( *pxptr++ );
                wbb += wb * ( *pxptr++ );
                ...
                if ( pAlpha ) {
                    WORD waa;
                    // image has alpha layer... we have to do the same for alpha
                    // data
                    pxptr = AlphaGetPointer( xi, yi ); // pointer to first byte
                    waa = wa * ( *pxptr++ );
                    waa += wb * ( *pxptr );        // first two pixels
                ...
                // default (slower) way to get pixels (not RGB24 or some pixels
                // out of borders)
                ...
                rgb21 =
                    GetPixelColorWithOverflow( xi + 1, yi, ofMethod, rplColor );
                rgb12 =
                    GetPixelColorWithOverflow( xi, yi + 1, ofMethod, rplColor );
                rgb22 = GetPixelColorWithOverflow( xi + 1, yi + 1, ofMethod,
                                                   rplColor );
                // calculate linear interpolation
                color.rgbRed = ( BYTE )( a * rgb11.rgbRed + b * rgb21.rgbRed +
                                         c * rgb12.rgbRed + d * rgb22.rgbRed );
                color.rgbGreen =
                    ( BYTE )( a * rgb11.rgbGreen + b * rgb21.rgbGreen +
                              c * rgb12.rgbGreen + d * rgb22.rgbGreen );
                color.rgbBlue =
                    ( BYTE )( a * rgb11.rgbBlue + b * rgb21.rgbBlue +
                              c * rgb12.rgbBlue + d * rgb22.rgbBlue );
#if CXIMAGE_SUPPORT_ALPHA
                if ( AlphaIsValid() )
                    color.rgbReserved = ( BYTE )( a * rgb11.rgbReserved +
                                                  b * rgb21.rgbReserved +
                                                  c * rgb12.rgbReserved +
                                                  d * rgb22.rgbReserved );
                else
        ...
        case IM_POWER:
            ...
            if ( ( ( xi + 2 ) < 0 ) || ( ( xi - 1 ) >= head.biWidth ) ||
                 ( ( yi + 2 ) < 0 ) ||
                 ( ( yi - 1 ) >=
                   head.biHeight ) ) { // all points are outside bounds?:
                switch ( ofMethod ) {
                    case OM_COLOR:
                    case OM_TRANSPARENT:
                    case OM_BACKGROUND:
            ...
            switch ( inMethod ) {
                case IM_BICUBIC:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelCubic( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelCubic( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_BICUBIC2:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] = KernelGeneralizedCubic(
                            ( float )( xi + i - 1 - x ), -0.5 );
                        kernely[ i ] = KernelGeneralizedCubic(
                            ( float )( yi + i - 1 - y ), -0.5 );
                    } // for i
                    break;
                case IM_BSPLINE:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelBSpline( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelBSpline( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_BOX:
                    for ( i = 0; i < 4; i++ ) {
                ...
                case IM_HERMITE:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelHermite( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelHermite( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_HAMMING:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelHamming( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelHamming( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_SINC:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelSinc( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelSinc( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_BLACKMAN:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelBlackman( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelBlackman( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_BESSEL:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelBessel( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelBessel( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_GAUSSIAN:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelGaussian( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelGaussian( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_QUADRATIC:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelQuadratic( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelQuadratic( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_MITCHELL:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelMitchell( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelMitchell( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_CATROM:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelCatrom( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelCatrom( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_HANNING:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelHanning( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelHanning( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
                case IM_POWER:
                    for ( i = 0; i < 4; i++ ) {
                        kernelx[ i ] =
                            KernelPower( ( float )( xi + i - 1 - x ) );
                        kernely[ i ] =
                            KernelPower( ( float )( yi + i - 1 - y ) );
                    } // for i
                    break;
            } // switch
            rr = gg = bb = aa = 0;
            if ( ( ( xi + 2 ) < head.biWidth ) && xi >= 1 &&
                 ( ( yi + 2 ) < head.biHeight ) && ( yi >= 1 ) &&
                 !IsIndexed() ) {
                // optimized interpolation (faster pixel reads) for RGB24 images
                // with all pixels inside bounds
                BYTE *pxptr, *pxptra;
                for ( yii = yi - 1; yii < yi + 3; yii++ ) {
                    pxptr = ( BYTE* )BlindGetPixelPointer(
                        xi - 1, yii ); // calculate pointer to first byte in row
                    kernelyc = kernely[ yii - ( yi - 1 ) ];
#if CXIMAGE_SUPPORT_ALPHA
                    if ( AlphaIsValid() ) {
                        // alpha is supported and valid (optimized bicubic int.
...
#endif
                    // alpha not supported or valid (optimized bicubic int. for
                    // no alpha channel)
                    {
                        kernel = kernelyc * kernelx[ 0 ];
                        bb += kernel * ( *pxptr++ );
                ...
                // slower more flexible interpolation for border pixels and
                // paletted images
                RGBQUAD rgbs;
                for ( yii = yi - 1; yii < yi + 3; yii++ ) {
                    kernelyc = kernely[ yii - ( yi - 1 ) ];
                    for ( xii = xi - 1; xii < xi + 3; xii++ ) {
                        ...
                        rgbs = GetPixelColorWithOverflow( xii, yii, ofMethod,
                                                          rplColor );
            ...
            if ( rr < 0 )
                rr = 0;
            color.rgbRed = ( BYTE )rr;
            if ( gg > 255 )
                gg = 255;
            if ( gg < 0 )
                gg = 0;
            color.rgbGreen = ( BYTE )gg;
            if ( bb > 255 )
                bb = 255;
            if ( bb < 0 )
                bb = 0;
            color.rgbBlue = ( BYTE )bb;
#if CXIMAGE_SUPPORT_ALPHA
            if ( AlphaIsValid() ) {
                if ( aa > 255 )
                    aa = 255;
        ...
        case IM_LANCZOS:
            ...
            if ( ( ( xi + 6 ) < 0 ) || ( ( xi - 5 ) >= head.biWidth ) ||
                 ( ( yi + 6 ) < 0 ) || ( ( yi - 5 ) >= head.biHeight ) ) {
            ...
            for ( xii = xi - 5; xii < xi + 7; xii++ )
                kernelx[ xii - ( xi - 5 ) ] =
                    KernelLanczosSinc( ( float )( xii - x ), 6.0f );
            rr = gg = bb = aa = 0;

            if ( ( ( xi + 6 ) < head.biWidth ) && ( ( xi - 5 ) >= 0 ) &&
                 ( ( yi + 6 ) < head.biHeight ) && ( ( yi - 5 ) >= 0 ) &&
                 !IsIndexed() ) {
                // optimized interpolation (faster pixel reads) for RGB24 images
                // with all pixels inside bounds
                BYTE *pxptr, *pxptra;
                for ( yii = yi - 5; yii < yi + 7; yii++ ) {
                    pxptr = ( BYTE* )BlindGetPixelPointer(
                        xi - 5, yii ); // calculate pointer to first byte in row
 ...
 * properly handles subsampling. If you need to sample original image with
 * interval of more than 1 pixel (as when shrinking an image), you should use
 * this method instead of GetPixelColorInterpolated or aliasing will occur. When
 * area width and height are both less than pixel, this method gets pixel color
 * by interpolating color of frame center with selected (inMethod) interpolation
 * by calling GetPixelColorInterpolated. If width and height are more than 1,
 * method calculates color by averaging color of pixels within area.
 * Interpolation method is not used in this case. Pixel color is interpolated by
 * averaging instead. If only one of both is more than 1, method uses
 * combination of interpolation and averaging. Chosen interpolation method is
 * used, but since it is averaged later on, there is little difference between
 * IM_BILINEAR (perhaps best for this case) and better methods.
 * IM_NEAREST_NEIGHBOUR again leads to aliasing artifacts. This method is a bit
 * slower than GetPixelColorInterpolated and when aliasing is not a problem, you
 * should simply use the later.
 *
 * \param  xc, yc - center of (rectangular) area
 * \param  w, h - width and height of area
 * \param  inMethod - interpolation method that is used, when interpolation is
 * used (see above)
 * \param  ofMethod - overflow method used when retrieving individual pixel
 * colors
...
RGBQUAD CxImage::GetAreaColorInterpolated( float const xc,
    ...
    if ( h <= 1 && w <= 1 ) {
        // both width and height are less than one... we will use interpolation
        // of center point
        return GetPixelColorInterpolated( xc, yc, inMethod, ofMethod,
                                          rplColor );
    } else {
        // area is wider and/or taller than one pixel:
        CxRect2 area( xc - w / 2.0f, yc - h / 2.0f, xc + w / 2.0f,
                      yc + h / 2.0f );                     // area
        ...
        if ( h > 1 && w > 1 ) {
            // width and height of area are greater than one pixel, so we can
            // employ "ordinary" averaging
            CxRect2 intBL, intTR; // bottom left and top right intersection
            intBL = area.CrossSection(
                CxRect2( ( ( float )xi1 ) - 0.5f, ( ( float )yi1 ) - 0.5f,
                         ( ( float )xi1 ) + 0.5f, ( ( float )yi1 ) + 0.5f ) );
            intTR = area.CrossSection(
                CxRect2( ( ( float )xi2 ) - 0.5f, ( ( float )yi2 ) - 0.5f,
                         ( ( float )xi2 ) + 0.5f, ( ( float )yi2 ) + 0.5f ) );
            ...
            AddAveragingCont(
                GetPixelColorWithOverflow( xi1, yi1, ofMethod, rplColor ),
                wBL * hBL, rr, gg, bb, aa ); // bottom left pixel
            AddAveragingCont(
                GetPixelColorWithOverflow( xi2, yi1, ofMethod, rplColor ),
                wTR * hBL, rr, gg, bb, aa ); // bottom right pixel
            AddAveragingCont(
                GetPixelColorWithOverflow( xi1, yi2, ofMethod, rplColor ),
                wBL * hTR, rr, gg, bb, aa ); // top left pixel
            AddAveragingCont(
                GetPixelColorWithOverflow( xi2, yi2, ofMethod, rplColor ),
                wTR * hTR, rr, gg, bb, aa ); // top right pixel
            // bottom and top row
            for ( x = xi1 + 1; x < xi2; x++ ) {
                AddAveragingCont(
                    GetPixelColorWithOverflow( x, yi1, ofMethod, rplColor ),
                    hBL, rr, gg, bb, aa ); // bottom row
                AddAveragingCont(
                    GetPixelColorWithOverflow( x, yi2, ofMethod, rplColor ),
                    hTR, rr, gg, bb, aa ); // top row
            }
            // leftmost and rightmost column
            for ( y = yi1 + 1; y < yi2; y++ ) {
                AddAveragingCont(
                    GetPixelColorWithOverflow( xi1, y, ofMethod, rplColor ),
                    wBL, rr, gg, bb, aa ); // left column
                AddAveragingCont(
                    GetPixelColorWithOverflow( xi2, y, ofMethod, rplColor ),
                    wTR, rr, gg, bb, aa ); // right column
            }
            for ( y = yi1 + 1; y < yi2; y++ ) {
                for ( x = xi1 + 1; x < xi2; x++ ) {
                    color =
                        GetPixelColorWithOverflow( x, y, ofMethod, rplColor );
                    rr += color.rgbRed;
                    gg += color.rgbGreen;
                    bb += color.rgbBlue;
#if CXIMAGE_SUPPORT_ALPHA
            ...
            for ( y = yi1; y <= yi2; y++ ) {
                for ( x = xi1; x <= xi2; x++ ) {
                    intersect = area.CrossSection( CxRect2(
                        ( ( float )x ) - 0.5f, ( ( float )y ) - 0.5f,
                        ( ( float )x ) + 0.5f, ( ( float )y ) + 0.5f ) );
                    center = intersect.Center();
                    color = GetPixelColorInterpolated(
                        center.x, center.y, inMethod, ofMethod, rplColor );
...
float CxImage::KernelBSpline( const float x ) {
    ...
    return ( 0.16666666666666666667f *
...
float CxImage::KernelBessel_Order1( float x ) {
    ...
    q = ( float )sqrt( 2.0f / ( PI * x ) ) *
        ( float )( KernelBessel_P1( x ) *
