...
CxImageJPG::CxExifInfo::~CxExifInfo() {
    for ( int i = 0; i < MAX_SECTIONS; i++ )
        if ( Sections[ i ].Data )
            cxfree( Sections[ i ].Data ); // free(Sections[i].Data);
    if ( freeinfo )
        delete m_exifinfo;
}
////////////////////////////////////////////////////////////////////////////////
bool CxImageJPG::CxExifInfo::DecodeExif( CxFile* hFile, int nReadMode ) {
    ...
    for ( ;; ) {
        ...
        if ( marker == 0xff ) {
            // 0xff is legal padding, but if we get that many, something's
            // wrong.
        ...
        got =
            hFile->Read( Data + 2, 1, itemlen - 2 ); // Read the whole section.
        if ( got != itemlen - 2 ) {
        ...
        switch ( marker ) {
            case M_SOS: // stop before hitting compressed data
                ...
                if ( nReadMode & EXIF_READ_IMAGE ) {
                    ...
                    if ( Data == NULL ) {
                        strcpy( m_szLastError,
                                "could not allocate data for entire image" );
                        return false;
                    }
                    ...
                    if ( got != size ) {
                        strcpy( m_szLastError,
                                "could not read the rest of the image" );
                        return false;
                    }
            ...
            case M_COM: // Comment section
                if ( HaveCom || ( ( nReadMode & EXIF_READ_EXIF ) == 0 ) ) {
                    ...
                    cxfree( Sections[ --SectionsRead ]
                                .Data ); // free(Sections[--SectionsRead].Data);
            ...
            case M_JFIF:
                ...
                // marker instead, althogh ACDsee will write images with both
                // markers. this program will re-create this marker on absence
                // of exif marker. hence no need to keep the copy from the file.
                cxfree( Sections[ --SectionsRead ]
                            .Data ); // free(Sections[--SectionsRead].Data);
                Sections[ SectionsRead ].Data = 0;
                break;
            ...
            case M_EXIF:
                ...
                if ( ( nReadMode & EXIF_READ_EXIF ) &&
                     memcmp( Data + 2, "Exif", 4 ) == 0 ) {
                    m_exifinfo->IsExif =
                        process_EXIF( ( BYTE* )Data + 2, itemlen );
                } else {
                    // Discard this section.
                    cxfree( Sections[ --SectionsRead ]
                                .Data ); // free(Sections[--SectionsRead].Data);
            ...
            default:
                ...
                // if (ShowTags) printf("Jpeg section marker 0x%02x size
                // %d\n",marker, itemlen);
                break;
        }
    }
...
--------------------------------------------------------------------------*/
bool CxImageJPG::CxExifInfo::process_EXIF( unsigned char* CharBuf,
                                           unsigned int length ) {
    ...
    (Sept 11 2002) strcpy(m_szLastError,"Suspicious offset of first IFD value");
                return false;
    }*/
    ...
    if ( !ProcessExifDir( CharBuf + 14, CharBuf + 6, length - 6, m_exifinfo,
                          &LastExifRefd ) )
        return false;

    /* <Richard Collins> give a chance for a second directory */
    if ( FirstOffset > 8 ) {
        if ( !ProcessExifDir( CharBuf + 14 + FirstOffset - 8, CharBuf + 6,
                              length - 6, m_exifinfo, &LastExifRefd ) )
            return false;
    }
    ...
    if ( m_exifinfo->FocalplaneXRes != 0 ) {
        m_exifinfo->CCDWidth =
            ( float )( ExifImageWidth * m_exifinfo->FocalplaneUnits /
...
--------------------------------------------------------------------------*/
bool CxImageJPG::CxExifInfo::ProcessExifDir(
    ...
    if ( NestingLevel > 4 ) {
        strcpy( m_szLastError,
                "Maximum directory nesting exceeded (corrupt exif header)" );
        return false;
    }
        ...
        */
        ...
        if ( ByteCount > 4 ) {
            ...
            if ( OffsetVal + ByteCount > ExifLength ) {
                ...
                strcpy( m_szLastError,
                        "Illegal pointer offset value in EXIF." );
                return false;
            }
        ...
        switch ( Tag ) {
            ...
            case TAG_USERCOMMENT:
                // Olympus has this padded with trailing spaces. Remove these
                // first.
                for ( a = ByteCount;; ) {
                    a--;
                    if ( ( ( char* )ValuePtr )[ a ] == ' ' ) {
            ...
            case TAG_MAXAPERTURE:
                ...
                */
                if ( m_exifinfo->ApertureFNumber == 0 ) {
                    m_exifinfo->ApertureFNumber =
                        ( float )exp( ConvertAnyFormat( ValuePtr, Format ) *
                                      log( 2.0f ) * 0.5 );
                }
                break;

            case TAG_BRIGHTNESS:
                m_exifinfo->Brightness =
                    ( float )ConvertAnyFormat( ValuePtr, Format );
                break;
            ...
            case TAG_SHUTTERSPEED:
                ...
                */
                if ( m_exifinfo->ExposureTime == 0 ) {
                    m_exifinfo->ExposureTime =
                        ( float )( 1 /
                                   exp( ConvertAnyFormat( ValuePtr, Format ) *
                                        log( 2.0f ) ) );
                }
                break;
            ...
            case TAG_RESOLUTIONUNIT:
                switch ( ( int )ConvertAnyFormat( ValuePtr, Format ) ) {
                    ...
                    case 4:
                        ...
                        break; /* 1 millimeter*/
                    case 5:
                        m_exifinfo->ResolutionUnit =
                            0.00003937007874f; /* 1 micrometer*/
                }
                break;
            ...
            case TAG_FOCALPLANEUNITS:
                switch ( ( int )ConvertAnyFormat( ValuePtr, Format ) ) {
                    ...
                    case 4:
                        ...
                        break; /* 1 millimeter*/
                    case 5:
                        m_exifinfo->FocalplaneUnits =
                            0.00003937007874f; /* 1 micrometer*/
                }
                break;

                // Remaining cases contributed by: Volker C. Schoech
                // <schoech(at)gmx(dot)de>

            case TAG_EXPOSURE_BIAS:
                m_exifinfo->ExposureBias =
                    ( float )ConvertAnyFormat( ValuePtr, Format );
                break;

            case TAG_WHITEBALANCE:
                m_exifinfo->Whitebalance =
                    ( int )ConvertAnyFormat( ValuePtr, Format );
                break;

            case TAG_METERING_MODE:
                m_exifinfo->MeteringMode =
                    ( int )ConvertAnyFormat( ValuePtr, Format );
                break;

            case TAG_EXPOSURE_PROGRAM:
                m_exifinfo->ExposureProgram =
                    ( int )ConvertAnyFormat( ValuePtr, Format );
                break;

            case TAG_ISO_EQUIVALENT:
                m_exifinfo->ISOequivalent =
                    ( int )ConvertAnyFormat( ValuePtr, Format );
            ...
            case TAG_COMPRESSION_LEVEL:
                m_exifinfo->CompressionLevel =
                    ( int )ConvertAnyFormat( ValuePtr, Format );
                break;

            case TAG_XRESOLUTION:
                m_exifinfo->Xresolution =
                    ( float )ConvertAnyFormat( ValuePtr, Format );
                break;
            case TAG_YRESOLUTION:
                m_exifinfo->Yresolution =
                    ( float )ConvertAnyFormat( ValuePtr, Format );
                break;
...
void CxImageJPG::CxExifInfo::process_SOFn( const BYTE* Data, int marker ) {
    ...
    // if (ShowTags) printf("JPEG image is %uw * %uh, %d color components, %d
    // bits per sample\n",
...
bool CxImageJPG::CxExifInfo::EncodeExif( CxFile* hFile ) {
    ...
    if ( Sections[ 0 ].Type != M_EXIF && Sections[ 0 ].Type != M_JFIF ) {
        // The image must start with an exif or jfif marker.  If we threw those
        // away, create one.
        static BYTE JfifHead[ 18 ] = { 0xff, M_JFIF, 0x00, 0x10, 'J',  'F',
                                       'I',  'F',    0x00, 0x01, 0x01, 0x01,
