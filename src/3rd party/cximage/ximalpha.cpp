 ...
 * a 24 bit image. The background color can be selected using SetTransColor().
 */
        ...
        for ( long y = 0; y < head.biHeight; y++ ) {
            for ( long x = 0; x < head.biWidth; x++ ) {
                ...
                if ( bAlphaIsValid )
                    ...
                    a = info.nAlphaMax;
                a1 = 256 - a;
                c.rgbBlue = ( BYTE )( ( c.rgbBlue * a +
                                        a1 * info.nBkgndColor.rgbBlue ) >>
                                      8 );
                c.rgbGreen = ( BYTE )( ( c.rgbGreen * a +
                                         a1 * info.nBkgndColor.rgbGreen ) >>
                                       8 );
                c.rgbRed =
                    ( BYTE )( ( c.rgbRed * a + a1 * info.nBkgndColor.rgbRed ) >>
                              8 );
...
bool CxImage::AlphaPaletteSplit( CxImage* dest ) {
    ...
    for ( long y = 0; y < head.biHeight; y++ ) {
        for ( long x = 0; x < head.biWidth; x++ ) {
            tmp.BlindSetPixelIndex( x, y,
                                    BlindGetPixelColor( x, y ).rgbReserved );
        }
    }
