...
// #define FAST_FACTOR	//use a factorization approximation to the LCP solver
// (fast, theoretically less accurate)
#define SLOW_LCP   // use the old LCP solver
#define NO_ISLANDS // does not perform island creation code (3~4% of simulation
                   // time), body disabling doesn't work
// #define TIMING
...
// little bit of math.... the _sym_ functions assume the return matrix will be
// symmetric
static void Multiply2_sym_p8p( dReal* A,
                               dReal* B,
                               dReal* C,
                               int p,
                               int Askip ) {
...
static void MultiplyAdd2_sym_p8p( dReal* A,
                                  dReal* B,
                                  dReal* C,
                                  int p,
                                  int Askip ) {
...
void dInternalStepFast( dxWorld* world,
                        dxBody* body[ 2 ],
                        dReal* GI[ 2 ],
                        dReal* GinvI[ 2 ],
                        dxJoint* joint,
    ...
    for ( i = 0; i < 2; i++ ) {
        if ( !body[ i ] )
            continue;
        for ( j = 0; j < 3; j++ )
            tmp1[ i * 8 + j ] = body[ i ]->facc[ j ] * body[ i ]->invMass +
...
void dInternalStepIslandFast( dxWorld* world,
                              dxBody* const* bodies,
                              int nb,
                              dxJoint** joints,
                              int nj,
    ...
    for ( iter = 0; iter < maxiterations; iter++ ) {
        ...
        for ( b = 0; b < nb; b++ ) {
            ...
            // global frame, and compute the rotational force and add it to the
            // torque accumulator. I and invI are vertically stacked 3x4
            // matrices, one per body.
        ...
        for ( j = 0; j < nj; j++ ) {
            ...
            // dInternalStepIslandFast is an exact copy of the old routine with
            // one modification: the calculated forces are added back to the
            // facc and tacc vectors instead of applying them to the bodies and
            // moving them.
            if ( info[ j ].m > 0 ) {
                switch ( joints[ j ]->vtable->typenum ) {
                    case dJointTypeContact:
                        if ( info[ j ].m == 3 ) {
                            dInternalStepJointContact(
                                world, bodyPair, GIPair, GinvIPair, joint,
                                info[ j ], Jinfo[ j ], ministep );
                            // dInternalStepFast (world, bodyPair, GIPair,
                            // GinvIPair, joint, info[j], Jinfo[j], ministep);

                            break;
                        };

                    default:
                        dInternalStepFast( world, bodyPair, GIPair, GinvIPair,
                                           joint, info[ j ], Jinfo[ j ],
                                           ministep );
                }
            }
        }
...
// It's downfall is it's inability to handle disabled bodies as well as the old
// one.
static void processIslandsFast( dxWorld* world,
    ...
    if ( nj > 3 )
        dInternalStepIslandFast( world, bodies, nb, joints, nj, stepsize,
                                 maxiterations );
    else
        dInternalStepIsland( world, bodies, nb, joints, nj, stepsize );
#ifdef TIMING
    dTimerEnd();
...
static void processIslandsFast( dxWorld* world,
    ...
    for ( j = world->firstjoint; j; j = ( dxJoint* )j->next ) {
        if ( ( j->node[ 0 ].body &&
               ( j->node[ 0 ].body->flags & dxBodyDisabled ) == 0 ) ||
             ( j->node[ 1 ].body &&
               ( j->node[ 1 ].body->flags & dxBodyDisabled ) == 0 ) ) {
            // if (!j->tag)
