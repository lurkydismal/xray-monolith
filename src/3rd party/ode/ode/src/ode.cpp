...
dxAutoDisable dxWorld::adis = { REAL( 0.001 ) * REAL( 0.001 ),
                                REAL( 0.001 ) * REAL( 0.001 ), 10, 0 };
    ...
    */
    ...
    for ( j = w->firstjoint; j; j = ( dxJoint* )j->next ) {
        for ( int i = 0; i < 2; i++ ) {
            if ( j->node[ i ].body ) {
                int ok = 0;
                for ( dxJointNode* n = j->node[ i ].body->firstjoint; n;
                      n = n->next ) {
                    if ( n->joint == j )
                        ok = 1;
                }
        ...
        for ( dxJointNode* n = b->firstjoint; n; n = n->next ) {
            if ( &n->joint->node[ 0 ] == n ) {
                if ( n->joint->node[ 1 ].body != b )
                    dDebug( 0,
                            "bad body pointer in joint node of body list (1)" );
            } else {
                if ( n->joint->node[ 0 ].body != b )
                    dDebug( 0,
                            "bad body pointer in joint node of body list (2)" );
            }
...
void dBodyAddForceAtPos( dBodyID b,
                         dReal fx,
                         dReal fy,
                         dReal fz,
                         dReal px,
...
void dBodyAddForceAtRelPos( dBodyID b,
                            dReal fx,
                            dReal fy,
                            dReal fz,
                            dReal px,
...
void dBodyAddRelForceAtPos( dBodyID b,
                            dReal fx,
                            dReal fy,
                            dReal fz,
                            dReal px,
...
void dBodyAddRelForceAtRelPos( dBodyID b,
                               dReal fx,
                               dReal fy,
                               dReal fz,
                               dReal px,
...
void dBodyGetRelPointPos( dBodyID b,
                          dReal px,
                          dReal py,
                          dReal pz,
                          dVector3 result ) {
...
void dBodyGetRelPointVel( dBodyID b,
                          dReal px,
                          dReal py,
                          dReal pz,
                          dVector3 result ) {
...
void dBodyGetPointVel( dBodyID b,
                       dReal px,
                       dReal py,
                       dReal pz,
                       dVector3 result ) {
...
void dBodyGetPosRelPoint( dBodyID b,
                          dReal px,
                          dReal py,
                          dReal pz,
                          dVector3 result ) {
...
void dBodyVectorToWorld( dBodyID b,
                         dReal px,
                         dReal py,
                         dReal pz,
                         dVector3 result ) {
...
void dBodyVectorFromWorld( dBodyID b,
                           dReal px,
                           dReal py,
                           dReal pz,
                           dVector3 result ) {
...
static dxJoint* createJoint( dWorldID w,
                             dJointGroupID group,
...
dxJoint* dJointCreateContact( dWorldID w,
                              dJointGroupID group,
                              const dContact* c ) {
    dAASSERT( c );
    dxJointContact* j =
        ( dxJointContact* )createJoint( w, group, &__dcontact_vtable );
    j->contact = *c;
    return j;
}
dxJoint* dJointCreateContactSpecial( dWorldID w,
    ...
    dAASSERT( c );
    dxJointContact* j =
        ( dxJointContact* )createJoint( w, group, &__dcontact_special_vtable );
    j->contact = *c;
    return j;
}
...
#error dSINGLE or dDOUBLE must be defined
    ...
    w->adis.linear_threshold =
        REAL( 0.001 ) * REAL( 0.001 ); // (magnitude squared)
    w->adis.angular_threshold =
        REAL( 0.001 ) * REAL( 0.001 ); // (magnitude squared)
...
void dWorldImpulseToForce( dWorldID w,
                           dReal stepsize,
                           dReal ix,
                           dReal iy,
                           dReal iz,
...
void dWorldSetAutoDisableAngularThreshold( dWorldID w,
