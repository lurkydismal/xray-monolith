...
#define dRealAllocaArray( name, n ) \
    dReal* name = ( dReal* )ALLOCA( ( n ) * sizeof( dReal ) );
...
static void compute_invM_JT( int m,
                             dRealMutablePtr J,
                             dRealMutablePtr iMJ,
                             int* jb,
                             dxBody* const* body,
...
static void multiply_invM_JT( int m,
                              int nb,
                              dRealMutablePtr iMJ,
                              int* jb,
                              dRealMutablePtr in,
...
/*
static void multiply_J_invM_JT (int m, int nb, dRealMutablePtr J,
dRealMutablePtr iMJ, int *jb, dRealPtr cfm, dRealMutablePtr z, dRealMutablePtr
in, dRealMutablePtr out)
{
...
// note: fc is returned as inv(M)*J'*lambda, the constraint force is actually
// J'*lambda
//
...
static void SOR_LCP( int m,
    ...
    for ( int iteration = 0; iteration < num_iterations; iteration++ ) {
        ...
        //@@@ potential optimization: swap lambda and last_lambda pointers
        // rather
        ...
        for ( int i = 0; i < m; i++ ) {
            ...
            if ( findex[ index ] >= 0 ) {
                hi[ index ] =
                    dFabs( hicopy[ index ] * lambda[ findex[ index ] ] );
                lo[ index ] = -hi[ index ];
            }
            ...
            // @@@ potential optimization: handle 1-body constraints in a
            // separate
...
dRealMutablePtr b, dRealMutablePtr lo, dRealMutablePtr hi, dRealPtr cfm, int
*findex, dxQuickStepParameters *qs)
{
        const int num_iterations = qs->num_iterations;
        const dReal sor_w = qs->w;		// SOR over-relaxation parameter
        ...
        for (int iteration=0; iteration < num_iterations; iteration++) {
                ...
                // constraints with findex < 0 always come first.
                if (iteration < 2) {
                        // for the first two iterations, solve the constraints
in
                        // the given order
                        for (i=0; i<m; i++) {
                                order[i].error = i;
                        ...
                        // sort the constraints so that the ones converging
slowest
                        // get solved last. use the absolute (not relative)
error. for (i=0; i<m; i++) { dReal v1 = dFabs (lambda[i]); dReal v2 = dFabs
(last_lambda[i]); dReal max = (v1 > v2) ? v1 : v2; if (max > 0) {
                                        //@@@ relative error: order[i].error =
dFabs(lambda[i]-last_lambda[i])/max; order[i].error =
dFabs(lambda[i]-last_lambda[i]);
                                }
                                else {
                                        order[i].error = dInfinity;
                                }
                ...
                //@@@ potential optimization: swap lambda and last_lambda
pointers rather
                //    than copying the data. we must make sure lambda is
properly
                //    returned to the caller
                memcpy (last_lambda,lambda,m*sizeof(dReal));

                for (int i=0; i<m; i++) {
                        // @@@ potential optimization: we could pre-sort J and
iMJ, thereby
                        //     linearizing access to those arrays. hmmm, this
does not seem
                        //     like a win, but we should think carefully about
our memory
                        //     access pattern.
                        ...
                        // set the limits for this constraint. note that
'hicopy' is used.
                        // this is the place where the QuickStep method differs
from the
                        // direct LCP solving method, since that method only
performs this
                        // limit adjustment once per time step, whereas this
method performs
                        // once per iteration per constraint row.
                        // the constraints are ordered so that all lambda[]
values needed have
                        ...
                        // @@@ potential optimization: SIMD-ize this and the b2
>= 0 case delta -=fc_ptr[0] * J_ptr[0] + fc_ptr[1] * J_ptr[1] + fc_ptr[2] *
J_ptr[2] + fc_ptr[3] * J_ptr[3] + fc_ptr[4] * J_ptr[4] + fc_ptr[5] * J_ptr[5];
                        // @@@ potential optimization: handle 1-body constraints
in a separate
                        //     loop to avoid the cost of test & jump?
                        if (b2 >= 0) {
                                fc_ptr = fc + 6*b2;
                                delta -=fc_ptr[0] * J_ptr[6] + fc_ptr[1] *
J_ptr[7] + fc_ptr[2] * J_ptr[8] + fc_ptr[3] * J_ptr[9] + fc_ptr[4] * J_ptr[10] +
fc_ptr[5] * J_ptr[11];
                        }

                        // compute lambda and clamp it to [lo,hi].
                        // @@@ potential optimization: does SSE have clamping
instructions
                        //     to save test+jump penalties here?
                        ...
                        //@@@ a trick that may or may not help
                        //dReal ramp =
(1-((dReal)(iteration+1)/(dReal)num_iterations));
                        //delta *= ramp;

                        // update fc.
                        // @@@ potential optimization: SIMD for this and the b2
>= 0 case
...
lambda[findex[index]]); lo[index] = -hi[index];
                        }
                        ...
                        iMJ_ptr = iMJ + index*12;
                        dRealMutablePtr fc_ptr = fc + 6*b1;
                        fc_ptr[0] += delta_lambda[index] *
iMJ_ptr[0];//_lambda[index] fc_ptr[1] += delta_lambda[index] *
iMJ_ptr[1];//_lambda[index] fc_ptr[2] += delta_lambda[index] *
iMJ_ptr[2];//_lambda[index] fc_ptr[3] += delta_lambda[index] *
iMJ_ptr[3];//_lambda[index] fc_ptr[4] += delta_lambda[index] *
iMJ_ptr[4];//_lambda[index] fc_ptr[5] += delta_lambda[index] *
iMJ_ptr[5];//_lambda[index]
                        // @@@ potential optimization: handle 1-body constraints
in a separate
                        //     loop to avoid the cost of test & jump?
                        if (b2 >= 0) {
                                fc_ptr = fc + 6*b2;
                                fc_ptr[0] += delta_lambda[index] *
iMJ_ptr[6];//_lambda[index] fc_ptr[1] += delta_lambda[index] *
iMJ_ptr[7];//_lambda[index] fc_ptr[2] += delta_lambda[index] *
iMJ_ptr[8];//_lambda[index] fc_ptr[3] += delta_lambda[index] *
iMJ_ptr[9];//_lambda[index] fc_ptr[4] += delta_lambda[index] *
iMJ_ptr[10];//_lambda[index] fc_ptr[5] += delta_lambda[index] *
iMJ_ptr[11];//_lambda[index]
                        }
                }
        }
}
...
void dxQuickStepper( dxWorld* world,
    ...
    // joints but not the joint array, because the caller might need it
    // unchanged).
    //@@@ do we really need to do this? we'll be sorting constraint rows
    // individually, not joints dxJoint **joint = (dxJoint**) alloca (nj *
    // sizeof(dxJoint*));//slipch@ - no need  to copy with sipmle island
    // processing memcpy (joint,_joint,nj * sizeof(dxJoint*));

    // for all bodies, compute the inertia tensor and its inverse in the global
    // frame, and compute the rotational force and add it to the torque
    // accumulator. I and invI are a vertical stack of 3x4 matrices, one per
    // body.
    dRealAllocaArray(
        I, 3 * 4 * nb ); // need to remember all I's for feedback purposes only
    dRealAllocaArray( invI, 3 * 4 * nb );
    for ( i = 0; i < nb; i++ ) {
        dMatrix3 tmp;
        dxBody* b = body[ i ];
#ifdef DEBUG_VALID
        dIASSERT( dValid( b->tacc[ 0 ] ) && dValid( b->tacc[ 1 ] ) &&
                  dValid( b->tacc[ 2 ] ) );
#endif
...
#ifndef dNODEBUG
        if ( !( dValid( b->tacc[ 0 ] ) && dValid( b->tacc[ 1 ] ) &&
                dValid( b->tacc[ 2 ] ) ) ) {
            // char s[64];
            //_snprintf (s,sizeof(s),"tmp %f,%f,%f \n avel
            //%f,%f,%f",tmp[0],tmp[1],tmp[2],b->avel[0],b->avel[1],b->avel[2]);
            // dUASSERT(0,"tmp %f,%f,%f \n avel
            // %f,%f,%f",tmp[0],tmp[1],tmp[2],b->avel[0],b->avel[1],b->avel[2]);

            dDebug( d_ERR_UASSERT, " (%s:%d \n tmp %f,%f,%f \n avel %f,%f,%f)",
                    __FILE__, __LINE__, tmp[ 0 ], tmp[ 1 ], tmp[ 2 ],
    ...
    // get joint information (m = total constraint dimension, nub = number of
    // unbounded variables). joints with m=0 are inactive and are removed from
    // the joints array entirely, so that the code that follows does not
    // consider them.
    //@@@ do we really need to save all the info1's
    dxJoint::Info1* info =
        ( dxJoint::Info1* )alloca( nj * sizeof( dxJoint::Info1 ) );
    for ( i = 0, j = 0; j < nj; j++ ) { // i=dest, j=src
        joint[ j ]->vtable->getInfo1( joint[ j ], info + i );
        dIASSERT( info[ i ].m >= 0 && info[ i ].m <= 6 && info[ i ].nub >= 0 &&
                  info[ i ].nub <= info[ i ].m );
        if ( info[ i ].m > 0 ) {
            joint[ i ] = joint[ j ];
            i++;
    ...
    if ( m > 0 ) {
        ...
        //   l1 l1 l1 a1 a1 a1 l2 l2 l2 a2 a2 a2  }-- jacobian for joint 0, body
        //   1 and body 2 (3 rows) l1 l1 l1 a1 a1 a1 l2 l2 l2 a2 a2 a2 / l1 l1
        //   l1 a1 a1 a1 l2 l2 l2 a2 a2 a2 }--- jacobian for joint 1, body 1 and
        //   body 2 (3 rows) etc...
        //
        ...
        int* jb_ptr = jb;
        for ( i = 0; i < nj; i++ ) {
            int b1 = ( joint[ i ]->node[ 0 ].body )
                         ? ( joint[ i ]->node[ 0 ].body->tag )
                         : -1;
            int b2 = ( joint[ i ]->node[ 1 ].body )
                         ? ( joint[ i ]->node[ 1 ].body->tag )
                         : -1;
            for ( j = 0; j < info[ i ].m; j++ ) {
                jb_ptr[ 0 ] = b1;
                jb_ptr[ 1 ] = b2;
        ...
        for ( i = 0; i < nb; i++ ) {
            ...
            for ( j = 0; j < 3; j++ )
                tmp1[ i * 6 + j ] = body[ i ]->facc[ j ] * body_invMass +
        ...
        for ( i = 0; i < nj; i++ ) {
            memcpy( lambda + ofs[ i ], joint[ i ]->lambda,
                    info[ i ].m * sizeof( dReal ) );
        }
#endif
        ...
        SOR_LCP( m, nb, J, jb, body, invI, lambda, cforce, rhs, lo, hi, cfm,
                 findex, &world->qs );
        ...
        for ( i = 0; i < nj; i++ ) {
            memcpy( joint[ i ]->lambda, lambda + ofs[ i ],
                    info[ i ].m * sizeof( dReal ) ); // valid?
        }
#endif
            ...
            if ( joint[ j ]->feedback ) {
                ...
                for ( ; j < nj; j++ ) {
                    ...
                    if ( fb ) {
                        ...
                        memcpy( fb->t1, cforce + b1 * 6 + 3,
                                3 * sizeof( dReal ) );
                        if ( joint[ j ]->node[ 1 ].body ) {
                            int b2 = joint[ j ]->node[ 1 ].body->tag;
                            memcpy( fb->f2, cforce + b2 * 6,
                                    3 * sizeof( dReal ) );
                            memcpy( fb->t2, cforce + b2 * 6 + 3,
                                    3 * sizeof( dReal ) );
                        }
                    }
                }
