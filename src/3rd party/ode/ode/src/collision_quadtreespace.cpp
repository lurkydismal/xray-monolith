...
public:
    ...
    void Create( const dVector3 Center,
    ...
    void Collide( dGeomID Object,
                  dGeomID g,
                  void* UserData,
                  dNearCallback* Callback );

    void CollideLocal( dGeomID Object,
                       void* UserData,
                       dNearCallback* Callback );
...
void Block::Create( const dVector3 Center,
                    const dVector3 Extents,
                    Block* Parent,
                    int Depth,
                    Block*& Blocks ) {
    ...
    if ( Depth > 0 ) {
        ...
        for ( int i = 0; i < SPLITAXIS; i++ ) {
            for ( int j = 0; j < SPLITAXIS; j++ ) {
                ...
                ChildCenter[ AXIS0 ] = Center[ AXIS0 ] - Extents[ AXIS0 ] +
                                       ChildExtents[ AXIS0 ] +
                                       i * ( ChildExtents[ AXIS0 ] * 2 );
                ChildCenter[ AXIS1 ] = Center[ AXIS1 ] - Extents[ AXIS1 ] +
                                       ChildExtents[ AXIS1 ] +
                                       j * ( ChildExtents[ AXIS1 ] * 2 );
                ChildCenter[ UP ] = Center[ UP ];

                Children[ Index ].Create( ChildCenter, ChildExtents, this,
                                          Depth - 1, Blocks );
            }
        }
    } else
        ...
        for ( int i = 0; i < SPLITS; i++ ) {
            ...
            if ( Children[ i ].GeomCount == 1 && Children[ i ].First ) {
                ...
                if ( g1->aabb[ AXIS0 * 2 + 0 ] > Children[ i ].MaxX ||
                     ...
                     g1->aabb[ AXIS1 * 2 + 1 ] < Children[ i ].MinZ )
                    continue;
            }
            Children[ i ].Collide( g1, Children[ i ].First, UserData,
                                   Callback );
        }
    }
}

void Block::CollideLocal( dxGeom* g1,
                          void* UserData,
                          dNearCallback* Callback ) {
    // Collide against local list
    dxGeom* g2 = First;
...
bool Block::Inside( const dReal* AABB ) {
    return AABB[ AXIS0 * 2 + 0 ] >= MinX && AABB[ AXIS0 * 2 + 1 ] <= MaxX &&
           AABB[ AXIS1 * 2 + 0 ] >= MinZ && AABB[ AXIS1 * 2 + 1 ] <= MaxZ;
}
...
Block* Block::GetBlock( const dReal* AABB ) {
    if ( Inside( AABB ) ) {
        ...
        return this; // We are at the root, so we have little choice
}
    ...
    if ( Children ) {
        for ( int i = 0; i < SPLITS; i++ ) {
            if ( Children[ i ].Inside( AABB ) ) {
                return Children[ i ].GetBlockChild(
                    AABB ); // Child will have good block
            }
        }
    }
...
struct dxQuadTreeSpace : public dxSpace {
    ...
    dxQuadTreeSpace( dSpaceID _space,
                     dVector3 Center,
                     dVector3 Extents,
                     int Depth );
    ~dxQuadTreeSpace();
...
dxQuadTreeSpace::dxQuadTreeSpace( dSpaceID _space,
                                  dVector3 Center,
                                  dVector3 Extents,
                                  int Depth )
    : dxSpace( _space ) {
    ...
    // Init AABB. We initialize to infinity because it is not illegal for an
    // object to be outside of the tree. Its simply inserted in the root block
    aabb[ 0 ] = -dInfinity;
    aabb[ 1 ] = dInfinity;
    aabb[ 2 ] = -dInfinity;
    aabb[ 3 ] = dInfinity;
...
PARENTRECURSE:
                            for (int& i = CurrentChild[CurrentLevel]; i <
SPLITS; i++){ if (CurrentBlock->Children[i].GeomCount == 0){ continue;
                                    }
            ...
            // Now lets go back to the parent so it can continue processing its
other children. if (CurrentBlock->Parent){ CurrentBlock = CurrentBlock->Parent;
                    CurrentLevel--;
                    goto PARENTRECURSE;
            }
    }
...
void dxQuadTreeSpace::add( dxGeom* g ) {
    ...
    dAASSERT( g );
    dUASSERT( g->parent_space == 0 && g->next == 0,
...
void dxQuadTreeSpace::collide2( void* UserData,
                                dxGeom* g1,
                                dNearCallback* Callback ) {
    ...
    if ( g1->parent_space == this ) {
        ...
        Blocks[ 0 ].Collide( g1, Blocks[ 0 ].First, UserData, Callback );

    lock_count--;
}

dSpaceID dQuadTreeSpaceCreate( dxSpace* space,
                               dVector3 Center,
                               dVector3 Extents,
                               int Depth ) {
