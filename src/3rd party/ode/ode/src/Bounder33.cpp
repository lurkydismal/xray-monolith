...
void Lcp33::SolveW() {
    switch ( NBn ) {
        case 1: {
            ...
            w[ idx0 ] = ( x[ idx0 ] - EinRowI( row0, 1 ) * w[ index[ 1 ] ] -
                          EinRowI( row0, 2 ) * w[ index[ 2 ] ] ) /
                        row0[ idx0 ];
        ...
        case 2: {
            ...
            dReal D = EinRowA( row0, idx0 ) * EinRowA( row1, idx1 ) -
                      EinRowA( row0, idx1 ) * EinRowA( row1, idx0 );
            dReal rhs0 = x[ idx0 ] - EinRowI( row0, 2 ) * w[ index[ 2 ] ];
            dReal rhs1 = x[ idx1 ] - EinRowI( row1, 2 ) * w[ index[ 2 ] ];
            w[ idx0 ] = ( rhs0 * EinRowA( row1, idx1 ) -
                          rhs1 * EinRowA( row0, idx1 ) ) /
                        D;
            w[ idx1 ] = ( rhs1 * EinRowA( row0, idx0 ) -
                          rhs0 * EinRowA( row1, idx0 ) ) /
                        D;
...
void Lcp33::FillA() {
    dReal fDetInv =
        ( EsA( 0, 0 ) *
              ( EsA( 1, 1 ) * EsA( 2, 2 ) - EsA( 1, 2 ) * EsA( 2, 1 ) ) -
          EsA( 0, 1 ) *
              ( EsA( 1, 0 ) * EsA( 2, 2 ) - EsA( 1, 2 ) * EsA( 2, 0 ) ) +
          EsA( 0, 2 ) *
              ( EsA( 1, 0 ) * EsA( 2, 1 ) - EsA( 1, 1 ) * EsA( 2, 0 ) ) );

    // VERIFY(_abs(fDetInv)>flt_zero);
    fDetInv = 1.0f / fDetInv;

    EA( 0, 0 ) =
        fDetInv * ( EsA( 1, 1 ) * EsA( 2, 2 ) - EsA( 1, 2 ) * EsA( 2, 1 ) );
    EA( 0, 1 ) =
        -fDetInv * ( EsA( 0, 1 ) * EsA( 2, 2 ) - EsA( 0, 2 ) * EsA( 2, 1 ) );
    EA( 0, 2 ) =
        fDetInv * ( EsA( 0, 1 ) * EsA( 1, 2 ) - EsA( 0, 2 ) * EsA( 1, 1 ) );

    EA( 1, 0 ) = EA(
        0, 1 ); //-fDetInv * ( EsA(1,0) * EsA(2,2) - EsA(1,2) * EsA(2,0) );////
    EA( 1, 1 ) =
        fDetInv * ( EsA( 0, 0 ) * EsA( 2, 2 ) - EsA( 0, 2 ) * EsA( 2, 0 ) );
    EA( 1, 2 ) =
        -fDetInv * ( EsA( 0, 0 ) * EsA( 1, 2 ) - EsA( 0, 2 ) * EsA( 1, 0 ) );

    EA( 2, 0 ) = EA(
        0, 2 ); // fDetInv * ( EsA(1,0) * EsA(2,1) - EsA(1,1) * EsA(2,0) );//
    EA( 2, 1 ) = EA(
        1, 2 ); // -fDetInv * ( EsA(0,0) * EsA(2,1) - EsA(0,1) * EsA(2,0) );//
    EA( 2, 2 ) =
        fDetInv * ( EsA( 0, 0 ) * EsA( 1, 1 ) - EsA( 0, 1 ) * EsA( 1, 0 ) );
}
