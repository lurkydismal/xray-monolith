...
struct dStack {
    ...
    // memory until it is referenced - so it is safe to set this to a high
    // value.
    ...
    // allocate `size' bytes from the stack and return a pointer to the
    // allocated memory. `size' must be >= 0. the returned pointer will be
    // aligned to the size of a long int.
    ...
    char* alloc( int size ) {
        ...
        if ( ( pointer - base ) > committed ) {
            committed = ( ( pointer - base - 1 ) | ( pagesize - 1 ) ) +
                        1; // round up to pgsize
