...
static void Multiply2_p8r( dReal* A,
                           dReal* B,
                           dReal* C,
                           int p,
                           int r,
...
static void MultiplyAdd2_p8r( dReal* A,
                              dReal* B,
                              dReal* C,
                              int p,
                              int r,
...
void dInternalStepIsland_x1( dxWorld* world,
                             dxBody* const* body,
                             int nb,
    ...
    for ( i = 0; i < nb; i++ ) {
        for ( j = 0; j < 3; j++ )
            fe[ i * 6 + j ] = body[ i ]->facc[ j ];
        for ( j = 0; j < 3; j++ )
            fe[ i * 6 + 3 + j ] = body[ i ]->tacc[ j ];
        for ( j = 0; j < 3; j++ )
            v[ i * 6 + j ] = body[ i ]->lvel[ j ];
        for ( j = 0; j < 3; j++ )
            v[ i * 6 + 3 + j ] = body[ i ]->avel[ j ];
    }
    ...
    if ( m > 0 ) {
        ...
        for ( i = 0; i < nj; i++ ) {
            Jinfo.J1l =
                J + nskip * ofs[ i ] + 6 * joint[ i ]->node[ 0 ].body->tag;
            Jinfo.J1a = Jinfo.J1l + 3;
            if ( joint[ i ]->node[ 1 ].body ) {
                Jinfo.J2l =
                    J + nskip * ofs[ i ] + 6 * joint[ i ]->node[ 1 ].body->tag;
        ...
        //    if (dFactorCholesky (L,m)==0) dDebug (0,"A is not positive
        //    definite");
        //
        //    // compute lambda
        // #   ifdef TIMING
        ...
        // equals `c' (to within a certain tolerance).
#ifdef TIMING
    ...
    for ( i = 0; i < nb; i++ ) {
        for ( j = 0; j < 3; j++ )
            body[ i ]->lvel[ j ] = vnew[ i * 6 + j ];
        for ( j = 0; j < 3; j++ )
            body[ i ]->avel[ j ] = vnew[ i * 6 + 3 + j ];
    }
...
void dInternalStepIsland_x2( dxWorld* world,
                             dxBody* const* body,
                             int nb,
                             dxJoint* const* _joint,
                             int nj,
    ...
    // for all bodies, compute the inertia tensor and its inverse in the global
    // frame, and compute the rotational force and add it to the torque
    // accumulator. I and invI are vertically stacked 3x4 matrices, one per
    // body.
    // @@@ check computation of rotational force.
    dReal* I = ( dReal* )ALLOCA( 3 * nb * 4 * sizeof( dReal ) );
    dReal* invI = ( dReal* )ALLOCA( 3 * nb * 4 * sizeof( dReal ) );
    ...
    if ( m > 0 ) {
        ...
        // compute A = J*invM*J'. first compute JinvM = J*invM. this has the
        // same format as J so we just go through the constraints in J
        // multiplying by the appropriate scalars and matrices.
#ifdef TIMING
        dTimerNow( "compute A" );
#endif
        ...
        // now compute A = JinvM * J'. A's rows and columns are grouped by
        // joint, i.e. in the same way as the rows of J. block (i,j) of A is
        // only nonzero if joints i and j have at least one body in common. this
        // fact suggests the algorithm used to fill A:
        //
        //    for b = all bodies
        ...
        // body in common. it does not compute the diagonal blocks of A however
        ...
        for ( i = 0; i < nb; i++ ) {
            for ( dxJointNode* n1 = body[ i ]->firstjoint; n1; n1 = n1->next ) {
                for ( dxJointNode* n2 = n1->next; n2; n2 = n2->next ) {
                    ...
                    // if either joint was tagged as -1 then it is an inactive
                    // (m=0) joint that should not be considered
                    if ( j1 == -1 || j2 == -1 )
                        continue;
                    ...
                    // determine if body i is the 1st or 2nd body of joints j1
                    // and j2
        ...
        for ( i = 0; i < nb; i++ ) {
            ...
            for ( j = 0; j < 3; j++ )
                tmp1[ i * 8 + j ] = body[ i ]->facc[ j ] * body_invMass +
        ...
        for ( i = 0; i < nj; i++ ) {
            ...
            if ( joint[ i ]->node[ 0 ].body ) {
                Multiply0_p81( rhs + ofs[ i ], JJ,
                               tmp1 + 8 * joint[ i ]->node[ 0 ].body->tag,
                               info[ i ].m );
            }
            if ( joint[ i ]->node[ 1 ].body ) {
                MultiplyAdd0_p81( rhs + ofs[ i ], JJ + 8 * info[ i ].m,
                                  tmp1 + 8 * joint[ i ]->node[ 1 ].body->tag,
                                  info[ i ].m );
            }
        }
        // complete rhs
        ...
        for ( i = 0; i < nj; i++ ) {
            ...
            if ( fb ) {
                // the user has requested feedback on the amount of force that
                // this joint is applying to the bodies. we use a slightly
                // slower computation that splits out the force components and
                // puts them in the feedback structure.
                ...
                if ( b2 ) {
                    Multiply1_8q1( data2, JJ + 8 * info[ i ].m,
                ...
                MultiplyAdd1_8q1( cforce + 8 * b1->tag, JJ, lambda + ofs[ i ],
                                  info[ i ].m );
                if ( b2 ) {
                    MultiplyAdd1_8q1( cforce + 8 * b2->tag,
                                      JJ + 8 * info[ i ].m, lambda + ofs[ i ],
                                      info[ i ].m );
                }
            }
        }
    ...
    for ( i = 0; i < nb; i++ ) {
        for ( j = 0; j < 3; j++ )
            cforce[ i * 8 + j ] += body[ i ]->facc[ j ];
        for ( j = 0; j < 3; j++ )
            cforce[ i * 8 + 4 + j ] += body[ i ]->tacc[ j ];
    }
    ...
    for ( i = 0; i < nb; i++ ) {
        for ( j = 0; j < 3; j++ )
            tmp_vnew[ i * 6 + j ] = body[ i ]->lvel[ j ];
        for ( j = 0; j < 3; j++ )
            tmp_vnew[ i * 6 + 3 + j ] = body[ i ]->avel[ j ];
    }
...
void dInternalStepIsland( dxWorld* world,
    ...
    for ( i = 0; i < nb; i++ )
        memcpy( state + i, body[ i ], sizeof( dxBody ) );
    ...
    // restore state
    for ( i = 0; i < nb; i++ )
        memcpy( body[ i ], state + i, sizeof( dxBody ) );
