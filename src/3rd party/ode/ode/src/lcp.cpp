...
static void swapRowsAndCols( ATYPE A,
                             int n,
                             int i1,
                             int i2,
                             int nskip,
...
static void swapProblem( ATYPE A,
                         dReal* x,
                         dReal* b,
                         dReal* w,
                         dReal* lo,
...
static void checkFactorization( ATYPE A,
                                dReal* _L,
                                dReal* _d,
                                int nC,
                                int* C,
...
struct dLCP {
    ...
    // space for other working data (which the caller may allocate on the
    // stack). some of this data is specific to the fast dLCP implementation.
...
dLCP::dLCP( int _n,
    ...
    if ( nub > 0 ) {
        for ( i = 0; i < nub; i++ )
            memcpy( _L + i * nskip, AROW( i ), ( i + 1 ) * sizeof( dReal ) );
...
void dLCP::transfer_i_to_C( int i ) {
    if ( i < nub )
        dDebug( 0, "bad i" );
    if ( C[ i ] )
        dDebug( 0, "i already in C" );
    if ( N[ i ] )
        dDebug( 0, "i already in N" );
    for ( int k = 0; k < i; k++ ) {
        if ( !( C[ k ] ^ N[ k ] ) )
            dDebug( 0, "assumptions for C and N violated" );
    }
    for ( int k = i; k < n; k++ )
        if ( C[ k ] || N[ k ] )
            dDebug( 0, "assumptions for C and N violated" );
    if ( i != last_i_for_solve1 )
        dDebug( 0, "assumptions for i violated" );
    last_i_for_solve1 = -1;
    C[ i ] = 1;
}
...
void dLCP::pN_plusequals_ANi( dReal* p, int i, int sign ) {
    ...
    for ( k = 0; k < n; k++ )
        if ( N[ k ] && k >= i )
            dDebug( 0, "N assumption violated" );
    if ( sign > 0 ) {
        for ( k = 0; k < n; k++ )
            if ( N[ k ] )
                p[ k ] += AROW( i )[ k ];
...
dLCP::dLCP( int _n,
    ...
    */
    ...
    // potentially push up `nub' this way and get a bigger initial
    // factorization. note that when we swap rows/cols here we must not just
    // swap row pointers, as the initial factorization relies on the data being
    // all in one chunk. variables that have findex >= 0 are *not* considered to
    // be unbounded even if lo=-inf and hi=inf - this is because these limits
    // may change during the solution process.

    for ( k = nub; k < n; k++ ) {
        if ( findex && findex[ k ] >= 0 )
            continue;
        if ( lo[ k ] == -dInfinity && hi[ k ] == dInfinity ) {
            swapProblem( A, x, b, w, lo, hi, p, state, findex, n, nub, k, nskip,
                         0 );
            nub++;
        }
    }
    ...
    if ( nub > 0 ) {
        for ( k = 0; k < nub; k++ )
            memcpy( L + k * nskip, AROW( k ), ( k + 1 ) * sizeof( dReal ) );
    ...
    if ( findex ) {
        ...
        for ( k = n - 1; k >= nub; k-- ) {
            if ( findex[ k ] >= 0 ) {
                swapProblem( A, x, b, w, lo, hi, p, state, findex, n, k,
                             n - 1 - num_at_end, nskip, 1 );
                num_at_end++;
            }
        }
    }
...
void dLCP::transfer_i_to_C( int i ) {
    int j;
    if ( nC > 0 ) {
        // ell,Dell were computed by solve1(). note, ell = D \ L1solve
        // (L,A(i,C))
        for ( j = 0; j < nC; j++ )
            L[ nC * nskip + j ] = ell[ j ];
...
void dSolveLCPBasic( int n,
    ...
    dLCP lcp( n, 0, A, x, b, w, tmp, tmp, L, d, Dell, ell, tmp, dummy, dummy, p,
              C, Arows );
    nub = lcp.getNub();
            ...
            for ( ;; ) {
                ...
                for ( k = 0; k < lcp.numN(); k++ ) {
                    if ( delta_w[ lcp.indexN( k ) ] < 0 ) {
                        dReal s2 =
                            -w[ lcp.indexN( k ) ] / delta_w[ lcp.indexN( k ) ];
                        if ( s2 < s ) {
                            s = s2;
                            si = lcp.indexN( k );
                            si_in_N = 1;
                ...
                for ( k = 0; k < lcp.numC(); k++ ) {
                    if ( delta_x[ lcp.indexC( k ) ] < 0 ) {
                        dReal s2 =
                            -x[ lcp.indexC( k ) ] / delta_x[ lcp.indexC( k ) ];
                        if ( s2 < s ) {
                            s = s2;
                            si = lcp.indexC( k );
                            si_in_N = 0;
...
void dSolveLCP( int n,
    ...
    dLCP lcp( n, nub, A, x, b, w, lo, hi, L, d, Dell, ell, delta_w, state,
              findex, p, C, Arows );
    nub = lcp.getNub();
    ...
    for ( i = nub; i < n; i++ ) {
        // the index i is the driving index and indexes i+1..n-1 are "dont
        // care", i.e. when we make changes to the system those x's will be zero
        // and we don't care what happens to those w's. in other words, we only
        // consider an (i+1)*(i+1) sub-problem of A*x=b+w.
        ...
        // prevent this from happening, we use the rule that indexes with
        // lo=hi=0 will never be checked for set changes. this means that the
        // state for these indexes may be incorrect, but that doesn't matter.
        ...
        if ( lo[ i ] == 0 && w[ i ] >= 0 ) {
            ...
            // this is a degenerate case. by the time we get to this test we
            // know that lo != 0, which means that lo < 0 as lo is not allowed
            // to be +ve, and similarly that hi > 0. this means that the line
            // segment corresponding to set C is at least finite in extent, and
            // we are on it. NOTE: we must call lcp.solve1() before
            // lcp.transfer_i_to_C()
            ...
            for ( ;; ) {
                ...
                delta_w[ i ] =
                    lcp.AiC_times_qC( i, delta_x ) + lcp.Aii( i ) * dirf;

                // find largest step we can take (size=s), either to drive
                // x(i),w(i) to the valid LCP region or to drive an
                // already-valid variable outside the valid region.
                ...
                if ( dir > 0 ) {
                    if ( hi[ i ] < dInfinity ) {
                        dReal s2 =
                            ( hi[ i ] - x[ i ] ) / dirf; // step to x(i)=hi(i)
                        if ( s2 < s ) {
                            s = s2;
                            cmd = 3;
                        }
                    ...
                    if ( lo[ i ] > -dInfinity ) {
                        dReal s2 =
                            ( lo[ i ] - x[ i ] ) / dirf; // step to x(i)=lo(i)
                        if ( s2 < s ) {
                            s = s2;
                            cmd = 2;
                        }
                ...
                for ( k = 0; k < lcp.numN(); k++ ) {
                    if ( ( state[ lcp.indexN( k ) ] == 0 &&
                           delta_w[ lcp.indexN( k ) ] < 0 ) ||
                         ( state[ lcp.indexN( k ) ] != 0 &&
                           delta_w[ lcp.indexN( k ) ] > 0 ) ) {
                        // don't bother checking if lo=hi=0
                        if ( lo[ lcp.indexN( k ) ] == 0 &&
                             hi[ lcp.indexN( k ) ] == 0 )
                            continue;
                        dReal s2 =
                            -w[ lcp.indexN( k ) ] / delta_w[ lcp.indexN( k ) ];
                ...
                for ( k = nub; k < lcp.numC(); k++ ) {
                    if ( delta_x[ lcp.indexC( k ) ] < 0 &&
                         lo[ lcp.indexC( k ) ] > -dInfinity ) {
                        dReal s2 =
                            ( lo[ lcp.indexC( k ) ] - x[ lcp.indexC( k ) ] ) /
                    ...
                    if ( delta_x[ lcp.indexC( k ) ] > 0 &&
                         hi[ lcp.indexC( k ) ] < dInfinity ) {
                        dReal s2 =
                            ( hi[ lcp.indexC( k ) ] - x[ lcp.indexC( k ) ] ) /
                            delta_x[ lcp.indexC( k ) ];
                        if ( s2 < s ) {
                            s = s2;
                            cmd = 6;
                ...
                //			     "C->NL","C->NH"};
                // printf ("cmd=%d (%s), si=%d\n",cmd,cmdstring[cmd],(cmd>3) ?
                // si : i);

                // if s <= 0 then we've got a problem. if we just keep going
                // then we're going to get stuck in an infinite loop. instead,
                // just cross our fingers and exit with the current solution.
                if ( s <= 0 ) {
                    dMessage( d_ERR_LCP, "LCP internal error, s <= 0 (s=%.4e)",
                              s );
                    if ( i < ( n - 1 ) ) {
