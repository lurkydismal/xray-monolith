...
void dxSimpleSpace::collide2( void* data,
                              dxGeom* geom,
...
static long int prime[ NUM_PRIMES ] = {
    1L,         2L,        3L,        7L,         13L,        31L,
    61L,        127L,      251L,      509L,       1021L,      2039L,
    4093L,      8191L,     16381L,    32749L,     65521L,     131071L,
    262139L,    524287L,   1048573L,  2097143L,   4194301L,   8388593L,
    16777213L,  33554393L, 67108859L, 134217689L, 268435399L, 536870909L,
    1073741789L };
...
void dxHashSpace::getLevels( int* minlevel, int* maxlevel ) {
    if ( minlevel )
        *minlevel = global_minlevel;
    if ( maxlevel )
        *maxlevel = global_maxlevel;
}
...
void dxHashSpace::collide( void* data, dNearCallback* callback ) {
    ...
    for ( geom = first; geom; geom = geom->next ) {
        ...
        if ( level < global_minlevel )
            level = global_minlevel;
        ...
        for ( int xi = dbounds[ 0 ]; xi <= dbounds[ 1 ]; xi++ ) {
            for ( int yi = dbounds[ 2 ]; yi <= dbounds[ 3 ]; yi++ ) {
                for ( int zi = dbounds[ 4 ]; zi <= dbounds[ 5 ]; zi++ ) {
                    // get the hash index
                    unsigned long hi =
                        getVirtualAddress( aabb->level, xi, yi, zi ) % sz;
            ...
            for ( int xi = db[ 0 ]; xi <= db[ 1 ]; xi++ ) {
                for ( int yi = db[ 2 ]; yi <= db[ 3 ]; yi++ ) {
                    for ( int zi = db[ 4 ]; zi <= db[ 5 ]; zi++ ) {
                        // get the hash index
                        unsigned long hi =
                            getVirtualAddress( level, xi, yi, zi ) % sz;
                        ...
                        for ( node = table[ hi ]; node; node = node->next ) {
                            ...
                            if ( node->aabb == aabb )
                                continue;
                            if ( node->aabb->level == level && node->x == xi &&
                                 node->y == yi && node->z == zi ) {
                                // see if aabb and node->aabb have already been
                                // tested against each other
                                unsigned char mask;
                                if ( aabb->index <= node->aabb->index ) {
                                    i = ( aabb->index * tested_rowsize ) +
                                        ( node->aabb->index >> 3 );
                                    mask = 1 << ( node->aabb->index & 7 );
                                } else {
                                    i = ( node->aabb->index * tested_rowsize ) +
                                        ( aabb->index >> 3 );
                                    mask = 1 << ( aabb->index & 7 );
                                }
                                dIASSERT( i >= 0 &&
                                          i < ( tested_rowsize * n ) );
                                if ( ( tested[ i ] & mask ) == 0 ) {
                                    collideAABBs( aabb->geom, node->aabb->geom,
                                                  data, callback );
                                }
                                tested[ i ] |= mask;
                            }
...
void dxHashSpace::collide2( void* data,
                            dxGeom* geom,
                            dNearCallback* callback ) {
    dAASSERT( geom && callback );
