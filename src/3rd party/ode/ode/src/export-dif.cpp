...
// @@@ TODO:
//	* export all spaces, and geoms in spaces, not just ones attached to
// bodies 	  (separate export function?)
...
static void printLimot( PrintingContext& c,
...
static void printContact( PrintingContext& c, dxJoint* j ) {
    ...
    //@@@ may want to write the geoms g1 and g2 that are involved, for
    // debugging.
    ...
    if ( mode & dContactMu2 )
        c.print( "mu2", ct->contact.surface.mu2 );
    if ( mode & dContactBounce )
        c.print( "bounce", ct->contact.surface.bounce );
    if ( mode & dContactBounce )
        c.print( "bounce_vel", ct->contact.surface.bounce_vel );
    if ( mode & dContactSoftERP )
        c.print( "soft_ERP", ct->contact.surface.soft_erp );
    if ( mode & dContactSoftCFM )
        c.print( "soft_CFM", ct->contact.surface.soft_cfm );
    if ( mode & dContactMotion1 )
        c.print( "motion1", ct->contact.surface.motion1 );
    if ( mode & dContactMotion2 )
        c.print( "motion2", ct->contact.surface.motion2 );
    if ( mode & dContactSlip1 )
        c.print( "slip1", ct->contact.surface.slip1 );
    if ( mode & dContactSlip2 )
        c.print( "slip2", ct->contact.surface.slip2 );
    int fa = 0; // friction approximation code
    if ( mode & dContactApprox1_1 )
        fa |= 1;
    if ( mode & dContactApprox1_2 )
        fa |= 2;
    if ( fa )
        c.print( "friction_approximation", fa );
    if ( mode & dContactFDir1 )
        c.print( "fdir1", ct->contact.fdir1 );
}
...
static void printHinge2( PrintingContext& c, dxJoint* j ) {
    ...
    c.print(
        "v1",
        h->v1 ); //@@@ much better to write out 'qrel' here, if it's available
    c.print( "v2", h->v2 );
...
void dWorldExportDIF( dWorldID w, FILE* file, const char* prefix ) {
    ...
    for ( dxBody* b = w->firstbody; b; b = ( dxBody* )b->next ) {
        ...
        if ( b->flags & dxBodyFlagFiniteRotation )
            c.print( "finite_rotation", 1 );
        if ( b->flags & dxBodyDisabled )
            c.print( "disabled", 1 );
        if ( b->flags & dxBodyNoGravity )
            c.print( "no_gravity", 1 );
        if ( b->flags & dxBodyAutoDisable ) {
            //			c.print ("auto_disable = {");
            //			c.indent++;
            //			c.print
            //("linear_threshold",b->adis.linear_threshold);
            //c.print
            //("angular_threshold",b->adis.angular_threshold);
            //c.print
            //("idle_time",b->adis.idle_time); 			c.print
            //("idle_steps",b->adis.idle_steps); 			c.print
            //("time_left",b->adis_timeleft); 			c.print
            //("steps_left",b->adis_stepsleft); c.indent--;
            //c.print ("},");
        }
