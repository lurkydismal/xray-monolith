...
static inline void setBall( dxJoint* joint,
    ...
    if ( joint->node[ 1 ].body ) {
        for ( int j = 0; j < 3; j++ ) {
            dReal err = a2[ j ] + joint->node[ 1 ].body->pos[ j ] - a1[ j ] -
...
static inline void setBall2( dxJoint* joint,
                             dxJoint::Info2* info,
                             dVector3 anchor1,
                             dVector3 anchor2,
                             dVector3 axis,
    ...
    for ( i = 0; i < 3; i++ )
        a1[ i ] += joint->node[ 0 ].body->pos[ i ];
    if ( joint->node[ 1 ].body ) {
        for ( i = 0; i < 3; i++ )
            a2[ i ] += joint->node[ 1 ].body->pos[ i ];
...
static void setFixedOrientation( dxJoint* joint,
                                 dxJoint::Info2* info,
                                 dQuaternion qrel,
                                 int start_row ) {
    int s = info->rowskip;
    ...
    dMULTIPLY0_331( e, joint->node[ 0 ].body->R,
                    qerr + 1 ); // @@@ bad SIMD padding!
    dReal k = info->fps * info->erp;
...
static void setAnchors( dxJoint* j,
                        dReal x,
                        dReal y,
                        dReal z,
                        dVector3 anchor1,
...
static dReal getHingeAngleFromRelativeQuat( dQuaternion qrel, dVector3 axis ) {
    ...
    // sint2 = |sin(theta/2)|
    dReal cost2 = qrel[ 0 ];
    dReal sint2 = dSqrt( qrel[ 1 ] * qrel[ 1 ] + qrel[ 2 ] * qrel[ 2 ] +
                         qrel[ 3 ] * qrel[ 3 ] );
    dReal theta =
        ( dDOT( qrel + 1, axis ) >= 0 ) ? // @@@ padding assumptions
            ( 2 * dAtan2( sint2, cost2 ) )
                                        : // if u points in direction of axis
            ( 2 *
              dAtan2( sint2, -cost2 ) ); // if u points in opposite direction
...
static dReal getHingeAngle( dxBody* body1,
                            dxBody* body2,
                            dVector3 axis,
...
int dxJointLimitMotor::addLimot( dxJoint* joint,
    ...
    if ( powered || limit ) {
        ...
        if ( !rotational && joint->node[ 1 ].body ) {
            dVector3 c;
            c[ 0 ] = REAL( 0.5 ) * ( joint->node[ 1 ].body->pos[ 0 ] -
                                     joint->node[ 0 ].body->pos[ 0 ] );
            c[ 1 ] = REAL( 0.5 ) * ( joint->node[ 1 ].body->pos[ 1 ] -
                                     joint->node[ 0 ].body->pos[ 1 ] );
            c[ 2 ] = REAL( 0.5 ) * ( joint->node[ 1 ].body->pos[ 2 ] -
                                     joint->node[ 0 ].body->pos[ 2 ] );
            dCROSS( ltd, =, c, ax1 );
        ...
        if ( powered ) {
            ...
            if ( !limit ) {
                ...
                // the joint is at a limit, AND is being powered. if the joint
                // is being powered into the limit then we apply the maximum
                // motor force in that direction, because the motor is working
                // against the immovable limit. if the joint is being powered
                // away from the limit then we have problems because actually we
                // need *two* lcp constraints to handle this case. so we fake it
                // and apply some fraction of the maximum force. the fraction to
                // use can be set as a fudge factor.
                ...
                if ( rotational ) {
                    dBodyAddTorque( joint->node[ 0 ].body, -fm * ax1[ 0 ],
                                    -fm * ax1[ 1 ], -fm * ax1[ 2 ] );
                    if ( joint->node[ 1 ].body )
                        dBodyAddTorque( joint->node[ 1 ].body, fm * ax1[ 0 ],
                                        fm * ax1[ 1 ], fm * ax1[ 2 ] );
                } else {
                    dBodyAddForce( joint->node[ 0 ].body, -fm * ax1[ 0 ],
                                   -fm * ax1[ 1 ], -fm * ax1[ 2 ] );
                    if ( joint->node[ 1 ].body ) {
                        dBodyAddForce( joint->node[ 1 ].body, fm * ax1[ 0 ],
                                       fm * ax1[ 1 ], fm * ax1[ 2 ] );

                        // linear limot torque decoupling step: refer to above
                        // discussion
                        dBodyAddTorque( joint->node[ 0 ].body, -fm * ltd[ 0 ],
                                        -fm * ltd[ 1 ], -fm * ltd[ 2 ] );
                        dBodyAddTorque( joint->node[ 1 ].body, -fm * ltd[ 0 ],
                                        -fm * ltd[ 1 ], -fm * ltd[ 2 ] );
                    }
                }
            }
...
static void hingeGetInfo1( dxJointHinge* j, dxJoint::Info1* info ) {
    ...
    if ( ( j->limot.lostop >= -M_PI || j->limot.histop <= M_PI ) &&
        ...
        dReal angle = getHingeAngle( j->node[ 0 ].body, j->node[ 1 ].body,
                                     j->axis1, j->qrel );
        if ( j->limot.testRotationalLimit( angle ) )
            info->m = 6;
    }
}
...
static void hingeComputeInitialRelativeRotation( dxJointHinge* joint ) {
    if ( joint->node[ 0 ].body ) {
        if ( joint->node[ 1 ].body ) {
            dQMultiply1( joint->qrel, joint->node[ 0 ].body->q,
                         joint->node[ 1 ].body->q );
        } else {
...
extern "C" dReal dJointGetHingeAngle( dxJointHinge* joint ) {
    ...
    dUASSERT( joint->vtable == &__dhinge_vtable, "joint is not a hinge" );
    if ( joint->node[ 0 ].body ) {
        dReal ang = getHingeAngle( joint->node[ 0 ].body, joint->node[ 1 ].body,
...
extern "C" void dJointAddHingeTorque( dxJointHinge* joint, dReal torque ) {
    ...
    if ( joint->node[ 0 ].body != 0 )
        dBodyAddTorque( joint->node[ 0 ].body, axis[ 0 ], axis[ 1 ],
                        axis[ 2 ] );
    if ( joint->node[ 1 ].body != 0 )
        dBodyAddTorque( joint->node[ 1 ].body, -axis[ 0 ], -axis[ 1 ],
                        -axis[ 2 ] );
}
...
extern "C" dReal dJointGetSliderPosition( dxJointSlider* joint ) {
    ...
    if ( joint->node[ 1 ].body ) {
        ...
        for ( int i = 0; i < 3; i++ )
            q[ i ] = joint->node[ 0 ].body->pos[ i ] - q[ i ] -
                     joint->node[ 1 ].body->pos[ i ];
    } else {
        for ( int i = 0; i < 3; i++ )
            q[ i ] = joint->node[ 0 ].body->pos[ i ] - joint->offset[ i ];
    }
    return dDOT( ax1, q );
}
...
extern "C" void dJointSetSliderAxis( dxJointSlider* joint,
    ...
    if ( joint->node[ 1 ].body ) {
        ...
        for ( i = 1; i < 4; i++ )
            joint->qrel[ i ] = -joint->node[ 0 ].body->q[ i ];
        for ( i = 0; i < 3; i++ )
            joint->offset[ i ] = joint->node[ 0 ].body->pos[ i ];
    }
}
...
extern "C" void dJointAddSliderForce( dxJointSlider* joint, dReal force ) {
    ...
    if ( joint->node[ 1 ].body != 0 )
        dBodyAddForce( joint->node[ 1 ].body, -axis[ 0 ], -axis[ 1 ],
                       -axis[ 2 ] );
}
...
static void hinge2GetInfo1( dxJointHinge2* j, dxJoint::Info1* info ) {
    ...
    if ( ( j->limot1.lostop >= -M_PI || j->limot1.histop <= M_PI ) &&
        ...
        if ( j->limot1.testRotationalLimit( angle ) )
            atlimit = 1;
    }
    if ( atlimit || j->limot1.fmax > 0 )
        info->m++;
...
#define HINGE2_GET_AXIS_INFO( axis, sin_angle, cos_angle )             \
    ...
    dMULTIPLY0_331( ax2, joint->node[ 1 ].body->R, joint->axis2 );     \
    dCROSS( axis, =, ax1, ax2 );                                       \
    sin_angle = dSqrt( axis[ 0 ] * axis[ 0 ] + axis[ 1 ] * axis[ 1 ] + \
                       axis[ 2 ] * axis[ 2 ] );                        \
...
static void hinge2GetInfo2( dxJointHinge2* joint, dxJoint::Info2* info ) {
    ...
    setBall2( joint, info, joint->anchor1, joint->anchor2, ax1,
              joint->susp_erp );
...
extern "C" void dJointSetHinge2Param( dxJointHinge2* joint,
    ...
    if ( ( parameter & 0xff00 ) == 0x100 ) {
        ...
        if ( parameter == dParamSuspensionERP )
            joint->susp_erp = value;
        else if ( parameter == dParamSuspensionCFM )
...
extern "C" void dJointGetHinge2Anchor2( dxJointHinge2* joint,
                                        dVector3 result ) {
    dUASSERT( joint, "bad joint argument" );
    dUASSERT( result, "bad result argument" );
    dUASSERT( joint->vtable == &__dhinge2_vtable, "joint is not a hinge2" );
...
extern "C" void dJointAddHinge2Torques( dxJointHinge2* joint,
                                        dReal torque1,
                                        dReal torque2 ) {
    ...
    if ( joint->node[ 0 ].body && joint->node[ 1 ].body ) {
        ...
        dBodyAddTorque( joint->node[ 0 ].body, axis1[ 0 ], axis1[ 1 ],
                        axis1[ 2 ] );
        dBodyAddTorque( joint->node[ 1 ].body, -axis1[ 0 ], -axis1[ 1 ],
                        -axis1[ 2 ] );
    }
}
...
// I just realized that the universal joint is equivalent to a hinge 2 joint
// with perfectly stiff suspension.  By comparing the hinge 2 implementation to
...
static void getUniversalAxes( dxJointUniversal* joint,
                              dVector3 ax1,
                              dVector3 ax2 ) {
...
static dReal getUniversalAngle1( dxJointUniversal* joint ) {
    if ( joint->node[ 0 ].body ) {
        ...
        dRFrom2Axes( R, ax1[ 0 ], ax1[ 1 ], ax1[ 2 ], ax2[ 0 ], ax2[ 1 ],
                     ax2[ 2 ] );
        dRtoQ( R, qcross );
...
static dReal getUniversalAngle2( dxJointUniversal* joint ) {
    if ( joint->node[ 0 ].body ) {
        ...
        dRFrom2Axes( R, ax2[ 0 ], ax2[ 1 ], ax2[ 2 ], ax1[ 0 ], ax1[ 1 ],
                     ax1[ 2 ] );
        dRtoQ( R, qcross );
...
static void universalGetInfo1( dxJointUniversal* j, dxJoint::Info1* info ) {
    ...
    bool limiting1 =
        ( j->limot1.lostop >= -M_PI || j->limot1.histop <= M_PI ) &&
        j->limot1.lostop <= j->limot1.histop;
    bool limiting2 =
        ( j->limot2.lostop >= -M_PI || j->limot2.histop <= M_PI ) &&
    ...
    if ( limiting1 || limiting2 ) {
        ...
        if ( limiting1 && j->limot1.testRotationalLimit( angle1 ) )
            constraint1 = true;
        if ( limiting2 && j->limot2.testRotationalLimit( angle2 ) )
            constraint2 = true;
    }
    if ( constraint1 )
        info->m++;
...
static void universalComputeInitialRelativeRotations(
...
extern "C" void dJointSetUniversalAnchor( dxJointUniversal* joint,
                                          ...
                                          dReal z ) {
    dUASSERT( joint, "bad joint argument" );
    dUASSERT( joint->vtable == &__duniversal_vtable,
...
extern "C" dReal dJointGetUniversalParam( dxJointUniversal* joint,
                                          int parameter ) {
    dUASSERT( joint, "bad joint argument" );
    dUASSERT( joint->vtable == &__duniversal_vtable,
              "joint is not a universal" );
...
extern "C" void dJointAddUniversalTorques( dxJointUniversal* joint,
                                           dReal torque1,
                                           dReal torque2 ) {
    dVector3 axis1, axis2;
    dAASSERT( joint );
...
static void amotorComputeGlobalAxes( dxJointAMotor* joint, dVector3 ax[ 3 ] ) {
    if ( joint->mode == dAMotorEuler ) {
        ...
        if ( joint->node[ 1 ].body ) {
            dMULTIPLY0_331( ax[ 2 ], joint->node[ 1 ].body->R,
                            joint->axis[ 2 ] );
        } else {
        ...
        for ( int i = 0; i < joint->num; i++ ) {
            if ( joint->rel[ i ] == 1 ) {
                // relative to b1
                dMULTIPLY0_331( ax[ i ], joint->node[ 0 ].body->R,
                                joint->axis[ i ] );
            }
            if ( joint->rel[ i ] == 2 ) {
                // relative to b2
                dIASSERT( joint->node[ 1 ].body );
                dMULTIPLY0_331( ax[ i ], joint->node[ 1 ].body->R,
                                joint->axis[ i ] );
            } else {
                // global - just copy it
...
static void amotorGetInfo2( dxJointAMotor* joint, dxJoint::Info2* info ) {
    ...
    for ( i = 0; i < joint->num; i++ ) {
        row +=
            joint->limot[ i ].addLimot( joint, info, row, *( axptr[ i ] ), 1 );
    }
}
...
extern "C" void dJointSetAMotorAxis( dxJointAMotor* joint,
                                     int anum,
                                     int rel,
                                     dReal x,
                                     dReal y,
    ...
    dUASSERT( joint->vtable == &__damotor_vtable, "joint is not an amotor" );
    dUASSERT( !( !joint->node[ 1 ].body && ( joint->flags & dJOINT_REVERSE ) &&
                 rel == 1 ),
...
extern "C" void dJointSetAMotorAngle( dxJointAMotor* joint,
                                      int anum,
                                      dReal angle ) {
...
extern "C" void dJointSetAMotorParam( dxJointAMotor* joint,
                                      int parameter,
                                      dReal value ) {
    dAASSERT( joint );
    dUASSERT( joint->vtable == &__damotor_vtable, "joint is not an amotor" );
...
extern "C" void dJointGetAMotorAxis( dxJointAMotor* joint,
                                     int anum,
                                     dVector3 result ) {
    dAASSERT( joint && anum >= 0 && anum < 3 );
    dUASSERT( joint->vtable == &__damotor_vtable, "joint is not an amotor" );
    ...
    if ( joint->rel[ anum ] > 0 ) {
        if ( joint->rel[ anum ] == 1 ) {
            dMULTIPLY0_331( result, joint->node[ 0 ].body->R,
                            joint->axis[ anum ] );
        } else {
            dMULTIPLY0_331( result, joint->node[ 1 ].body->R,
                            joint->axis[ anum ] );
        }
    } else {
...
extern "C" void dJointAddAMotorTorques( dxJointAMotor* joint,
                                        dReal torque1,
                                        dReal torque2,
                                        dReal torque3 ) {
    dVector3 axes[ 3 ];
    ...
    if ( joint->num == 0 )
        return;
    dUASSERT( ( joint->flags & dJOINT_REVERSE ) == 0,
              "dJointAddAMotorTorques not yet implemented for reverse AMotor "
              "joints" );
    ...
    if ( joint->node[ 0 ].body != 0 )
        dBodyAddTorque( joint->node[ 0 ].body, axes[ 0 ][ 0 ], axes[ 0 ][ 1 ],
                        axes[ 0 ][ 2 ] );
    if ( joint->node[ 1 ].body != 0 )
        dBodyAddTorque( joint->node[ 1 ].body, -axes[ 0 ][ 0 ], -axes[ 0 ][ 1 ],
                        -axes[ 0 ][ 2 ] );
}
...
static void fixedGetInfo2( dxJointFixed* joint, dxJoint::Info2* info ) {
    ...
    if ( joint->node[ 1 ].body ) {
        ...
        for ( int j = 0; j < 3; j++ )
            info->c[ j ] =
                k * ( joint->offset[ j ] - joint->node[ 0 ].body->pos[ j ] );
    }
}
...
extern "C" void dJointSetFixed( dxJointFixed* joint ) {
    ...
    if ( joint->node[ 0 ].body ) {
        if ( joint->node[ 1 ].body ) {
            ...
            dReal ofs[ 4 ];
            for ( i = 0; i < 4; i++ )
                ofs[ i ] = joint->node[ 0 ].body->pos[ i ];
            for ( i = 0; i < 4; i++ )
                ofs[ i ] -= joint->node[ 1 ].body->pos[ i ];
...
extern "C" void dJointSetFixedQuaternionPos( dxJointFixed* joint,
    ...
    if ( joint->node[ 0 ].body ) {
        if ( joint->node[ 1 ].body ) {
            /* dQMultiply1
             (joint->qrel,joint->node[0].body->q,joint->node[1].body->q); dReal
             ofs[4]; for (i=0; i<4; i++) ofs[i] = joint->node[0].body->pos[i];
            ...
            for ( i = 1; i < 4; i++ )
                joint->qrel[ i ] =
                    -quaternion[ i ]; //-joint->node[0].body->q[i];
            for ( i = 0; i < 4; i++ )
                joint->offset[ i ] = pos[ i ]; // joint->node[0].body->pos[i];
        }
    }
}
