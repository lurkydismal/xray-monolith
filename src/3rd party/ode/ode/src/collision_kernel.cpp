...
static int dCollideSpaceGeom( dxGeom* o1,
                              dxGeom* o2,
                              int flags,
                              dContactGeom* contact,
                              int skip ) {
...
static void initColliders() {
    ...
    setCollider( dCCylinderClass, dCCylinderClass,
...
int dCollide( dxGeom* o1,
    ...
    dUASSERT( o1->type >= 0 && o1->type < dGeomNumClasses,
              "bad o1 class number" );
    dUASSERT( o2->type >= 0 && o2->type < dGeomNumClasses,
              "bad o2 class number" );
...
void dGeomSetBody( dxGeom* g, dxBody* b ) {
    ...
    if ( b ) {
        ...
        // the new position of the geom is set to the old position of the body,
        // so the effective position of the geom remains unchanged.
    }
}
...
static int dCollideUserGeomWithGeom( dxGeom* o1,
                                     dxGeom* o2,
                                     int flags,
                                     dContactGeom* contact,
                                     int skip ) {
    ...
    // find the collider function to use. if o1 does not know how to collide
    // with o2, then o2 might know how to collide with o1 (provided that it is a
    // user geom).
    ...
    // found, which means that dCollide() will always return 0 for this case.
...
int dCreateGeomClass( const dGeomClass* c ) {
    ...
    if ( num_user_classes >= dMaxUserClasses ) {
        dDebug( 0,
                "too many user classes, you must increase the limit and "
                "recompile ODE" );
    }
...
void* dGeomGetClassData( dxGeom* g ) {
    dUASSERT( g && g->type >= dFirstUserClass && g->type <= dLastUserClass,
              "not a custom class" );
...
dGeomID dCreateGeom( int classnum ) {
    dUASSERT( classnum >= dFirstUserClass && classnum <= dLastUserClass,
              "not a custom class" );
