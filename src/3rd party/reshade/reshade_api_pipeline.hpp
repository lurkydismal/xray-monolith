...
enum class shader_stage : uint32_t {
    ...
    all_compute = compute,
    all_graphics =
        vertex | hull | domain | geometry | pixel | amplification | mesh,
    all_ray_tracing =
...
enum class pipeline_stage : uint32_t {
    ...
    all_graphics = vertex_shader | hull_shader | domain_shader |
                   geometry_shader | pixel_shader | input_assembler |
                   stream_output | rasterizer | depth_stencil | output_merger,
    all_ray_tracing = ray_tracing_shader,
    all_shader_stages = vertex_shader | hull_shader | domain_shader |
...
struct descriptor_range {
    ...
    /// In D3D this is equivalent to the offset (in descriptors) of this range
    /// in the descriptor table (since each binding can only have an array size
    /// of 1).
    /// </summary>
    uint32_t binding = 0;
    /// <summary>
    /// D3D9/D3D10/D3D11/D3D12 shader register index (<c>register(xX)</c> in
    /// HLSL).
    /// </summary>
    uint32_t dx_register_index = 0;
    /// <summary>
    ...
    uint32_t count = 0;
    /// <summary>
    /// Shader pipeline stages that can make use of the descriptors in this
    /// range.
    /// </summary>
    shader_stage visibility = shader_stage::all;
    /// <summary>
    /// Size of the array in case this is an array binding.
    /// Only meaningful in Vulkan, in OpenGL and other APIs this has to be 1
    /// (since each array element is a separate binding there). If this is less
    /// than the total number of descriptors specified in <see cref="count"/>,
    /// then the remaining descriptors are assigned a separate binding (with an
    /// array size of 1), with the binding index incrementing with each
...
struct descriptor_range_with_static_samplers : public descriptor_range {
    ...
    /// descriptor table when the descriptor type is <see
    /// cref="descriptor_type::sampler"/> or <see
    /// cref="descriptor_type::sampler_with_resource_view"/>.
    /// </summary>
...
struct pipeline_layout_param {
    ...
    constexpr pipeline_layout_param(
        ...
        : type( pipeline_layout_param_type::
          ...
          descriptor_table_with_static_samplers( { count, ranges } ) {}

    /// <summary>
    /// Type of the parameter.
    /// </summary>
    pipeline_layout_param_type type =
        pipeline_layout_param_type::push_descriptors;

    union {
        /// <summary>
        /// Used when parameter type is <see
        /// cref="pipeline_layout_param_type::push_constants"/>.
        /// </summary>
        constant_range push_constants;

        /// <summary>
        /// Used when parameter type is <see
        /// cref="pipeline_layout_param_type::push_descriptors"/>.
        /// </summary>
        descriptor_range push_descriptors;

        /// <summary>
        /// Used when parameter type is <see
        /// cref="pipeline_layout_param_type::descriptor_table"/> or <see
        /// cref="pipeline_layout_param_type::push_descriptors_with_ranges"/>.
        /// </summary>
        struct {
            uint32_t count;
            const descriptor_range* ranges;
        ...
        /// <summary>
        /// Used when parameter type is <see
        /// cref="pipeline_layout_param_type::descriptor_table_with_static_samplers"/>
        /// or <see
        /// cref="pipeline_layout_param_type::push_descriptors_with_static_samplers"/>.
        /// </summary>
        struct {
            uint32_t count;
            const descriptor_range_with_static_samplers* ranges;
...
/// </summary>
enum class fill_mode : uint32_t { solid = 0, wireframe = 1, point = 2 };
...
/// <summary>
/// Blend factors in color or alpha blending operations, which modulate values
/// between the pixel shader output and render target.
/// </summary>
...
/// <summary>
/// Specifies how the pipeline interprets vertex data that is bound to the
/// vertex input stage and subsequently renders it.
/// </summary>
...
struct shader_desc {
    ...
    size_t code_size = 0;
    /// <summary>
    /// Optional entry point name if the shader source <see cref="code"/> or
    /// binary contains multiple entry points. Can be <see langword="nullptr"/>
    /// if it does not, or to use the default "main" entry point.
    /// </summary>
    const char* entry_point = nullptr;

    /// <summary>
    /// Number of entries in the <see cref="spec_constant_ids"/> and <see
    /// cref="spec_constant_values"/> arrays. This is meaningful only when the
    /// shader binary is a SPIR-V module and is ignored otherwise.
    /// </summary>
    ...
    /// <summary>
    /// Pointer to an array of constant values, one for each specialization
    /// constant index in <see cref="spec_constant_ids"/>.
    /// </summary>
...
struct shader_group {
    ...
    shader_group( shader_group_type type,
        ...
        : type( type ),
          hit_group( { closest_hit_shader_index, any_hit_shader_index,
    ...
    union {
        ...
        struct {
            /// <summary>
            /// Index of the shader in the ray generation shader pipeline
            /// subobject.
            /// </summary>
        ...
        /// <summary>
        /// Used when type is <see
        /// cref="shader_group_type::hit_group_triangles"/> or <see
        /// cref="shader_group_type::hit_group_aabbs"/>.
        /// </summary>
        struct {
            /// <summary>
            /// Index of the shader in the closest-hit shader pipeline
            /// subobject. Set to -1 (UINT32_MAX) to indicate that this shader
            /// type is not used in the hit group.
            /// </summary>
            ...
            /// Set to -1 (UINT32_MAX) to indicate that this shader type is not
            /// used in the hit group.
            /// </summary>
            /// <seealso cref="pipeline_subobject_type::any_hit_shader"/>
            uint32_t any_hit_shader_index = UINT32_MAX;
            /// <summary>
            /// Index of the shader in the intersection shader pipeline
            /// subobject. Set to -1 (UINT32_MAX) to indicate that this shader
            /// type is not used in the hit group.
            /// </summary>
...
/// <summary>
/// Describes a single element in the vertex layout for the input-assembler
/// stage.
/// </summary>
struct input_element {
    /// <summary>
    /// GLSL attribute location associated with this element (<c>layout(location
    /// = X)</c>).
    /// </summary>
    uint32_t location = 0;
    /// <summary>
    ...
    /// <summary>
    /// Optional index for the HLSL semantic (e.g. for "TEXCOORD1" set <see
    /// cref="semantic"/> to "TEXCOORD" and <see cref="semantic_index"/> to 1).
    /// </summary>
    ...
    uint32_t offset = 0;
    /// <summary>
    /// Stride of the entire vertex (this has to be consistent for all elements
    /// per vertex buffer binding). Set to zero in case this is unknown.
    /// </summary>
    uint32_t stride = 0;
    /// <summary>
    /// Number of instances to draw using the same per-instance data before
    /// advancing by one element. This has to be consistent for all elements per
    /// vertex buffer binding. Set to zero to indicate that this element is
    /// per-vertex rather than per-instance.
    /// </summary>
...
struct blend_desc {
    /// <summary>
    /// Use alpha-to-coverage as a multisampling technique when setting a pixel
    /// to a render target.
    /// </summary>
    ...
    blend_factor source_color_blend_factor[ 8 ] = {
        blend_factor::one, blend_factor::one, blend_factor::one,
        blend_factor::one, blend_factor::one, blend_factor::one,
        blend_factor::one, blend_factor::one };
    /// <summary>
    /// Destination to use for the current RGB value in the render target.
    /// </summary>
    /// <seealso cref="device_caps::independent_blend"/>
    blend_factor dest_color_blend_factor[ 8 ] = {
        blend_factor::zero, blend_factor::zero, blend_factor::zero,
        blend_factor::zero, blend_factor::zero, blend_factor::zero,
        blend_factor::zero, blend_factor::zero };
    /// <summary>
    /// Operation to use to combine <see cref="source_color_blend_factor"/> and
    /// <see cref="dest_color_blend_factor"/>.
    /// </summary>
    /// <seealso cref="device_caps::independent_blend"/>
    blend_op color_blend_op[ 8 ] = {
        blend_op::add, blend_op::add, blend_op::add, blend_op::add,
        blend_op::add, blend_op::add, blend_op::add, blend_op::add };
    /// <summary>
    ...
    blend_factor dest_alpha_blend_factor[ 8 ] = {
        ...
        blend_factor::zero, blend_factor::zero };
    /// <summary>
    /// Operation to use to combine <see cref="source_alpha_blend_factor"/> and
    /// <see cref="dest_alpha_blend_factor"/>.
    /// </summary>
    /// <seealso cref="device_caps::independent_blend"/>
    blend_op alpha_blend_op[ 8 ] = {
        ...
        blend_op::add, blend_op::add, blend_op::add, blend_op::add };
    /// <summary>
    /// Constant RGBA value to use when <see cref="source_color_blend_factor"/>
    /// or <see cref="dest_color_blend_factor"/> is <see
    /// cref="blend_factor::constant_color"/>.
    /// </summary>
    float blend_constant[ 4 ] = { 1.0f, 1.0f, 1.0f, 1.0f };
    /// <summary>
    /// Logical operation for each render target. Ignored if <see
    /// cref="logic_op_enable"/> is <see langword="false"/>.
    /// </summary>
    /// <seealso cref="device_caps::logic_op"/>
    logic_op logic_op[ 8 ] = { logic_op::noop, logic_op::noop, logic_op::noop,
                               logic_op::noop, logic_op::noop, logic_op::noop,
                               logic_op::noop, logic_op::noop };
    /// <summary>
    /// A write mask specifying which color components are written to each
    /// render target. Bitwise combination of <c>0x1</c> for red, <c>0x2</c> for
    /// green, <c>0x4</c> for blue and <c>0x8</c> for alpha.
    /// </summary>
    uint8_t render_target_write_mask[ 8 ] = { 0xF, 0xF, 0xF, 0xF,
                                              0xF, 0xF, 0xF, 0xF };
};
...
struct rasterizer_desc {
    ...
    /// <summary>
    /// Use the quadrilateral or alpha line anti-aliasing algorithm on
    /// multisample antialiasing render targets.
    /// </summary>
    bool multisample_enable = false;
    /// <summary>
    /// Enable or disable line antialiasing. Only applies if doing line drawing
    /// and <see cref="multisample_enable"/> is <see langword="false"/>.
    /// </summary>
...
struct depth_stencil_desc {
    ...
    /// <summary>
    /// Comparison function to use to compare new depth value from a fragment
    /// against current depth value in the depth-stencil buffer.
    /// </summary>
    ...
    /// <summary>
    /// Mask applied to stencil values read from the depth-stencil buffer for
    /// pixels whose surface normal is towards the camera.
    /// </summary>
    uint8_t front_stencil_read_mask = 0xFF;
    /// <summary>
    /// Mask applied to stencil values written to the depth-stencil buffer for
    /// pixels whose surface normal is towards the camera.
    /// </summary>
    uint8_t front_stencil_write_mask = 0xFF;
    /// <summary>
    /// Reference value to perform against when stencil testing pixels whose
    /// surface normal is towards the camera.
    /// </summary>
    uint8_t front_stencil_reference_value = 0;
    /// <summary>
    /// Comparison function to use to compare new stencil value from a fragment
    /// against current stencil value for pixels whose surface normal is facing
    /// towards the camera.
    /// </summary>
    compare_op front_stencil_func = compare_op::always;
    /// <summary>
    /// Stencil operation to perform when stencil testing and depth testing both
    /// pass for pixels whose surface normal is facing towards the camera.
    /// </summary>
    stencil_op front_stencil_pass_op = stencil_op::keep;
    /// <summary>
    /// Stencil operation to perform when stencil testing fails for pixels whose
    /// surface normal is towards the camera.
    /// </summary>
    stencil_op front_stencil_fail_op = stencil_op::keep;
    /// <summary>
    /// Stencil operation to perform when stencil testing passes and depth
    /// testing fails for pixels whose surface normal is facing towards the
    /// camera.
    /// </summary>
    ...
    /// <summary>
    /// Reference value to perform against when stencil testing pixels whose
    /// surface normal is facing away from the camera.
    /// </summary>
    ...
    /// <summary>
    /// Stencil operation to perform when stencil testing and depth testing both
    /// pass for pixels whose surface normal is facing away from the camera.
    /// </summary>
    ...
    /// <summary>
    /// Stencil operation to perform when stencil testing passes and depth
    /// testing fails for pixels whose surface normal is facing away from the
    /// camera.
    /// </summary>
...
enum class pipeline_subobject_type : uint32_t {
    ...
    /// Sub-object data is a pointer to a <see cref="primitive_topology"/>
    /// value.
    /// </summary>
    primitive_topology,
    /// <summary>
    ...
    sample_count,
    /// <summary>
    /// Maximum number of viewports that may be bound via <see
    /// cref="command_list::bind_viewports"/> with this pipeline. Sub-object
    /// data is a pointer to a 32-bit unsigned integer value.
    /// </summary>
    viewport_count,
    /// <summary>
    /// States that may be dynamically updated via <see
    /// cref="command_list::bind_pipeline_states"/> after binding this pipeline.
    /// Sub-object data is a pointer to an array of <see cref="dynamic_state"/>
    /// values.
    /// </summary>
    ...
    /// Sub-object data is a pointer to an array of <see cref="pipeline"/>
    /// handles.
    /// </summary>
    libraries,
    /// <summary>
    /// Ray tracing shader groups to use.
    /// Sub-object data is a pointer to an array of <see cref="shader_group"/>
    /// values.
    /// </summary>
    shader_groups,
    /// <summary>
...
struct pipeline_subobject {
    ...
    /// This should usually be 1, except for array sub-objects like <see
    /// cref="pipeline_subobject_type::render_target_formats"/> (where this
    /// specifies the number of render targets) or <see
    /// cref="pipeline_subobject_type::dynamic_pipeline_states"/>.
    /// </summary>
    uint32_t count = 0;
    /// <summary>
    /// Pointer to an array of sub-object descriptions (which should be as large
    /// as the specified <see cref="count"/>). Depending on the sub-object <see
    /// cref="type"/> this should be a pointer to a <see cref="shader_desc"/>,
    /// <see cref="blend_desc"/>, <see cref="rasterizer_desc"/>, <see
    /// cref="depth_stencil_desc"/> or ...
    /// </summary>
    void* data = nullptr;
};
...
struct descriptor_table_update {
    ...
    /// In D3D this is equivalent to the offset (in descriptors) from the start
    /// of the table.
    /// </summary>
    uint32_t binding = 0;
    /// <summary>
    /// Array index in the specified <see cref="binding"/> to begin updating at.
    /// Only meaningful in Vulkan, in OpenGL and other APIs this has to be 0
    /// (since each array element gets a separate binding).
    /// </summary>
    uint32_t array_offset = 0;
    /// <summary>
    /// Number of descriptors to update, starting at the specified <see
    /// cref="binding"/>. If the specified <see cref="binding"/> has fewer than
    /// <see cref="count"/> array elements starting from <see
    /// cref="array_offset"/>, then the remainder will be used to update the
    /// subsequent binding starting at array element zero, recursively.
    /// </summary>
    uint32_t count = 0;
    /// <summary>
    ...
    descriptor_type type = descriptor_type::sampler;
    /// <summary>
    /// Pointer to an array of descriptors to update in the descriptor table
    /// (which should be as large as the specified <see cref="count"/>).
    /// Depending on the descriptor <see cref="type"/> this should be pointer to
    /// an array of <see cref="buffer_range"/>, <see cref="resource_view"/>,
    /// <see cref="sampler"/>, <see cref="sampler_with_resource_view"/> or <see
    /// cref="acceleration_structure"/>.
    /// </summary>
...
enum class query_type {
    /// <summary>
    /// Number of samples that passed the depth and stencil tests between
    /// beginning and end of the query. Data is a 64-bit unsigned integer value.
    /// </summary>
    occlusion = 0,
    /// <summary>
    /// Zero if no samples passed, one if at least one sample passed the depth
    /// and stencil tests between beginning and end of the query. Data is a
    /// 64-bit unsigned integer value.
    /// </summary>
    binary_occlusion = 1,
    /// <summary>
    /// GPU timestamp at the frequency returned by <see
    /// cref="command_queue::get_timestamp_frequency"/>. Data is a 64-bit
    /// unsigned integer value.
    /// </summary>
    timestamp = 2,
    /// <summary>
    /// Pipeline statistics (such as the number of shader invocations) between
    /// beginning and end of the query. Data is a structure of type <c>{
    /// uint64_t ia_vertices; uint64_t ia_primitives; uint64_t vs_invocations;
    /// uint64_t gs_invocations; uint64_t gs_primitives; uint64_t invocations;
    /// uint64_t primitives; uint64_t ps_invocations; uint64_t hs_invocations;
    /// uint64_t ds_invocations; uint64_t cs_invocations; }</c>.
    /// </summary>
    pipeline_statistics = 3,
    /// <summary>
    /// Streaming output statistics for stream 0 between beginning and end of
    /// the query. Data is a structure of type <c>{ uint64_t primitives_written;
    /// uint64_t primitives_storage_needed; }</c>.
    /// </summary>
    ...
    /// <summary>
    /// Number of bottom-level acceleration structure pointers in the
    /// acceleration structure. Data is a 64-bit unsigned integer value.
    /// </summary>
    /// <seealso cref="command_list::query_acceleration_structures"/>
...
/// <summary>
/// A list of all possible render pipeline states that can be set independent of
/// pipeline state objects. <para>Support for these varies between graphics APIs
/// (e.g. modern APIs like D3D12 and Vulkan support much less dynamic states
/// than D3D9).</para>
/// </summary>
