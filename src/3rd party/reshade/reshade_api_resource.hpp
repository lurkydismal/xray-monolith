...
#define RESHADE_DEFINE_HANDLE( name )                     \
    ...
    constexpr bool operator<( name lhs, name rhs ) {      \
        return lhs.handle < rhs.handle;                   \
    }                                                     \
    constexpr bool operator!=( name lhs, name rhs ) {     \
        return lhs.handle != rhs.handle;                  \
    }                                                     \
    constexpr bool operator!=( name lhs, uint64_t rhs ) { \
...
#define RESHADE_DEFINE_ENUM_FLAG_OPERATORS( type )                   \
    ...
    constexpr bool operator==( type lhs, uint32_t rhs ) {            \
...
/// <summary>
/// Sampling behavior at texture coordinates outside the bounds of a texture
/// resource.
/// </summary>
...
struct sampler_desc {
    ...
    /// <summary>
    /// Clamping value to use when filtering mode is <see
    /// cref="filter_mode::anisotropic"/>.
    /// </summary>
    float max_anisotropy = 1.0f;
    /// <summary>
    /// Comparison function to use to compare sampled data against existing
    /// sampled data.
    /// </summary>
    compare_op compare_op = compare_op::never;
    /// <summary>
    /// RGBA value to return for texture coordinates outside 0 to 1 range when
    /// addressing mode is <see cref="texture_address_mode::border"/>.
    /// </summary>
...
/// <item>Direct3D 12: A 'D3D12_CPU_DESCRIPTOR_HANDLE' (to a sampler
/// descriptor).</item> <item>OpenGL: An OpenGL sampler object name.</item>
...
/// </summary>
enum class map_access { read_only = 1, write_only, read_write, write_discard };

/// <summary>
/// Memory heap types, which give a hint as to where to place the allocation for
/// a resource.
/// </summary>
enum class memory_heap : uint32_t {
    unknown, // Usually indicates a resource that is reserved, but not yet bound
             // to any memory.
    gpu_only,
    // Upload heap
    cpu_to_gpu,
...
/// Various operations may have special requirements on the type of resources
/// they operate on (e.g. copies can only happen between resources of the same
/// type, ...).
/// </summary>
enum class resource_type : uint32_t {
    ...
    texture_2d,
    texture_3d,
    surface // Special type for resources that are implicitly both resource and
            // render target view.
};
...
enum class resource_flags : uint32_t {
    none = 0,
    /// <summary>
    /// Dynamic resources can be frequently updated during a frame, with
    /// previous contents automatically being shadowed so to no affect already
    /// executing operations on the GPU. Required for <see
    /// cref="map_access::write_discard"/>. The flag is not supported in D3D12
    /// or Vulkan.
    /// </summary>
    dynamic = ( 1 << 3 ),
    /// <summary>
    /// Required to create <see cref="resource_view_type::texture_cube"/> or
    /// <see cref="resource_view_type::texture_cube_array"/> views of the
    /// resource.
    /// </summary>
    cube_compatible = ( 1 << 2 ),
    /// <summary>
    /// Required to use the resource with <see
    /// cref="command_list::generate_mipmaps"/>.
    /// </summary>
    generate_mipmaps = ( 1 << 0 ),
    /// <summary>
    /// Shared resources can be imported/exported from/to different graphics
    /// APIs and/or processes. Required to use the "shared_handle" parameter of
    /// <see cref="device::create_resource"/>.
    /// </summary>
    shared = ( 1 << 1 ),
...
/// This needs to be specified during creation and is also used to transition
/// between different resource states within a command list.
/// </summary>
enum class resource_usage : uint32_t {
    ...
    // The following are special resource states and may only be used in
    // barriers:

    general = 0x80000000,
    present = 0x80000000 | render_target | copy_source,
    cpu_access = vertex_buffer | index_buffer | shader_resource |
...
struct [[nodiscard]] resource_desc {
    ...
    constexpr resource_desc( uint32_t width,
        ...
        : type( resource_type::texture_2d ),
          texture( { width, height, layers, levels, format, samples } ),
          heap( heap ),
          usage( usage ),
          flags( flags ) {}
    ...
    union {
        ...
        struct {
            ...
            uint64_t size = 0;
            /// <summary>
            /// Structure stride for structured buffers (in bytes), otherwise
            /// zero.
            /// </summary>
            uint32_t stride = 0;
        } buffer;
        ...
        struct {
            ...
            uint32_t width = 1;
            /// <summary>
            /// If this is a 2D or 3D texture, height of the texture (in
            /// texels), otherwise 1.
            /// </summary>
            uint32_t height = 1;
            /// <summary>
            /// If this is a 3D texture, depth of the texture (in texels),
            /// otherwise number of array layers.
            /// </summary>
            uint16_t depth_or_layers = 1;
            /// <summary>
            /// Maximum number of mipmap levels in the texture, including the
            /// base level, so at least 1. Can also be zero in case the exact
            /// number of mipmap levels is unknown.
            /// </summary>
            uint16_t levels = 1;
            /// <summary>
            ...
            /// <summary>
            /// The number of samples per texel. Set to a value higher than 1
            /// for multisampling.
            /// </summary>
            uint16_t samples = 1;
        } texture;
    ...
    /// This should contain all resource states the resource will ever be
    /// transitioned to (including the initial state specified for resource
    /// creation).
    /// </summary>
...
/// <para>Resources created by the application are only guaranteed to be valid
/// during event callbacks.</para> <para> Depending on the graphics API this can
/// be: <list type="bullet"> <item>Direct3D 9: A pointer to a
/// 'IDirect3DResource9' object.</item> <item>Direct3D 10: A pointer to a
/// 'ID3D10Resource' object.</item> <item>Direct3D 11: A pointer to a
/// 'ID3D11Resource' object.</item> <item>Direct3D 12: A pointer to a
/// 'ID3D12Resource' object.</item> <item>OpenGL: The upper 24-bit contain the
/// OpenGL object type (like GL_BUFFER, GL_TEXTURE_2D, GL_RENDERBUFFER, ...),
/// the lower 32-bit contain the OpenGL object name. So the object type can be
/// extracted with <c>(handle >> 40)</c>, the object with <c>(handle &
/// 0xFFFFFFFF)</c>.</item> <item>Vulkan: A 'VkImage' handle.</item>
/// </list>
/// </para>
/// </summary>
...
/// <summary>
/// Type of a resource view. This identifies how a resource view interprets the
/// data of its resource.
/// </summary>
...
/// <summary>
/// Describes a resource view, which specifies how to interpret the data of a
/// resource.
/// </summary>
struct [[nodiscard]] resource_view_desc {
    ...
    /// <summary>
    /// Format the view should reinterpret the resource data to (can be
    /// different than the format of the resource as long as they are
    /// compatible).
    /// </summary>
    format format = format::unknown;
    ...
    union {
        ...
        struct {
            ...
            uint64_t offset = 0;
            /// <summary>
            /// Number of elements this view covers in the buffer resource (in
            /// bytes). Set to -1 (UINT64_MAX) to indicate that the entire
            /// buffer resource should be used.
            /// </summary>
        ...
        struct {
            /// <summary>
            /// Index of the most detailed mipmap level to use. This number has
            /// to be between zero and the maximum number of mipmap levels in
            /// the texture minus 1.
            /// </summary>
            uint32_t first_level = 0;
            /// <summary>
            /// Maximum number of mipmap levels for the view of the texture.
            /// Set to -1 (UINT32_MAX) to indicate that all mipmap levels down
            /// to the least detailed should be used.
            /// </summary>
            uint32_t level_count = UINT32_MAX;
            /// <summary>
            /// Index of the first array layer of the texture array to use. This
            /// value is ignored if the texture is not layered.
            /// </summary>
            uint32_t first_layer = 0;
            /// <summary>
            /// Maximum number of array layers for the view of the texture
            /// array. This value is ignored if the texture is not layered. Set
            /// to -1 (UINT32_MAX) to indicate that all array layers should be
            /// used.
            /// </summary>
...
/// <summary>
/// An opaque handle to a resource view object (depth-stencil, render target,
/// shader resource view, ...). <para>Resource views created by the application
/// are only guaranteed to be valid during event callbacks.</para> <para>
/// Depending on the graphics API this can be:
/// <list type="bullet">
...
/// <item>Direct3D 12: A 'D3D12_CPU_DESCRIPTOR_HANDLE' (to a view descriptor) or
/// 'D3D12_GPU_VIRTUAL_ADDRESS' (to an acceleration structrue).</item>
/// <item>OpenGL: The upper 24-bit contain the OpenGL object type (like
/// GL_TEXTURE_BUFFER, GL_TEXTURE_2D, GL_TEXTURE_CUBE_MAP_POSITIVE_X, ...), the
/// lower 32-bit contain the OpenGL object ID. So the object type can be
/// extracted with <c>(handle >> 40)</c>, the object with <c>(handle &
/// 0xFFFFFFFF)</c>.</item> <item>Vulkan: A 'VkImageView' or
/// 'VkAccelerationStructureKHR' handle.</item>
/// </list>
/// </para>
/// </summary>
...
struct subresource_data {
    ...
    void* data = nullptr;
    /// <summary>
    /// Row pitch of the data (added to the data pointer to move between texture
    /// rows, unused for buffers and 1D textures).
    /// </summary>
    /// <seealso cref="format_row_pitch"/>
    uint32_t row_pitch = 0;
    /// <summary>
    /// Depth pitch of the data (added to the data pointer to move between
    /// texture depth/array slices, unused for buffers and 1D/2D textures).
    /// </summary>
...
/// <summary>
/// Specifies how the contents of a render target or depth-stencil view are
/// treated at the start of a render pass.
/// </summary>
enum class render_pass_load_op : uint32_t { load, clear, discard, no_access };

/// <summary>
/// Specifies how the contents of a render target or depth-stencil view are
/// treated at the end of a render pass.
/// </summary>
enum class render_pass_store_op : uint32_t { store, discard, no_access };

/// <summary>
/// Describes a depth-stencil view and how it is treated at the start and end of
/// a render pass.
/// </summary>
struct render_pass_depth_stencil_desc {
    ...
    /// <summary>
    /// Specifies how the depth contents of the depth-stencil view are treated
    ...
    /// <summary>
    /// Specifies how the stencil contents of the depth-stencil view are treated
    /// at the start of the render pass.
    /// </summary>
    ...
    /// <summary>
    /// Value the depth contents of the depth-stencil resource is cleared to
    /// when <see cref="depth_load_op"/> is <see
    /// cref="render_pass_load_op::clear"/>.
    /// </summary>
    float clear_depth = 0.0f;
    /// <summary>
    /// Value the stencil contents of the depth-stencil resource is cleared to
    /// when <see cref="stencil_load_op"/> is <see
    /// cref="render_pass_load_op::clear"/>.
    /// </summary>
...
/// <summary>
/// Describes a render target view and how it is treated at the start and end of
/// a render pass.
/// </summary>
struct render_pass_render_target_desc {
    ...
    resource_view view = { 0 };
    /// <summary>
    /// Specifies how the contents of the render target view are treated at the
    ...
    /// <summary>
    /// Value the render target resource is cleared to when <see
    /// cref="load_op"/> is <see cref="render_pass_load_op::clear"/>.
    /// </summary>
...
/// </summary>
enum class acceleration_structure_build_mode { build = 0, update = 1 };

/// <summary>
/// Flags that specify additional parameters to an acceleration structure build
/// operation.
/// </summary>
...
/// <summary>
/// Flags that specify additional parameters of an acceleration structure build
/// input.
/// </summary>
...
/// The data in <see
/// cref="acceleration_structure_build_input::instances::buffer"/> should be an
...
struct acceleration_structure_build_input {
    ...
    constexpr acceleration_structure_build_input(
        ...
        : type( acceleration_structure_build_input_type::triangles ),
    ...
    constexpr acceleration_structure_build_input(
        ...
        : type( acceleration_structure_build_input_type::instances ),
          instances( { instance_buffer, instance_offset, instance_count,
                       array_of_pointers } ) {}
    ...
    acceleration_structure_build_input_type type =
        acceleration_structure_build_input_type::triangles;

    union {
        /// <summary>
        /// Used when build input type is <see
        /// cref="acceleration_structure_build_input_type::triangles"/>.
        /// </summary>
        struct {
            api::resource vertex_buffer = {};
        ...
        /// <summary>
        /// Used when build input type is <see
        /// cref="acceleration_structure_build_input_type::aabbs"/>.
        /// </summary>
        struct {
            api::resource buffer = {};
            uint64_t offset = 0;
        ...
        /// <summary>
        /// Used when build input type is <see
        /// cref="acceleration_structure_build_input_type::instances"/>.
        /// </summary>
        struct {
            api::resource buffer = {};
            uint64_t offset = 0;
    ...
    acceleration_structure_build_input_flags flags =
        acceleration_structure_build_input_flags::none;
};
} // namespace api
} // namespace reshade
