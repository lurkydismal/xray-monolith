...
//                * Added hash method that makes use of new unchecked function
//                unsafe_horrible_cast
//                * Removed VC6 code
//                * Use variadic templates (C++11)
//                * Added MakeDelegate for plain function pointers
//                * Use static_assert for compile-time checks (C++11)
// 21-Jan-14 2.0.1* Fixed 2 typos (line 393 & 429) where a static_cast should
// have been a static_assert. 21-Jun-14 2.0.2* Fixed incorrect union member name
// in the SimplifyMemFunc struct.
...
//  'this' pointer. According to the standard, this can be done legally with
//  reinterpret_cast<>. For (non-standard) compilers which use member function
//  pointers which vary in size depending on the class, we need to use knowledge
//  of the internal structure of a member function pointer, as used by the
//  compiler. Template specialization is used to distinguish between the sizes.
//  Because some compilers don't support partial template specialisation, I use
//  full specialisation of a wrapper struct.
...
public:
    ...
    inline void bindstaticfunc( DerivedClass* pParent,
        ...
        m_pStaticFunction =
            reinterpret_cast< GenericFuncPtr >( function_to_bind );
    }
    ...
    //
    // For compilers where data pointers are at least as big as code pointers,
    // it is possible to store the function pointer in the this pointer, using
    ...
    // identical to this. Note that the Sun C++ and MSVC documentation
    // explicitly state that they support static_cast between void * and
    // function pointers.

    template < class DerivedClass >
    inline void CopyFrom( DerivedClass* /*pParent*/,
    ...
    inline bool IsEqualToStaticFuncPtr( StaticFuncPtr funcptr ) {
        ...
        // function, this will return an arbitrary value that is not equal to
        // any valid function pointer.
        else
            return funcptr ==
...
private:
    ...
    typedef detail::
        ClosurePtr< GenericMemFn, StaticFunctionPtr, UnvoidStaticFunctionPtr >
            ClosureType;
...
public:
    ...
    FastDelegateImpl(
        Y* pthis,
        DesiredRetType ( X::*function_to_bind )( Args... args ) ) {
    ...
    template < typename X, typename Y >
    FastDelegateImpl( const Y* pthis,
                      DesiredRetType ( X::*function_to_bind )( Args... args )
                          const ) {
    ...
    RetType operator()( Args... args ) const {
        return (
            m_Closure.GetClosureThis()->*( m_Closure.GetClosureMemPtr() ) )(
            args... );
    }
...
public:
    ...
    template < typename X, typename Y >
    FastDelegate( Y* pthis, RetType ( X::*function_to_bind )( Args... args ) )
...
template < typename RetType, typename... Args >
FastDelegate< RetType( Args... ) > MakeDelegate(
    RetType ( *func )( Args... ) ) {
...
template < typename RetType, typename X, typename Y, typename... Args >
FastDelegate< RetType( Args... ) > MakeDelegate( Y* x,
                                                 RetType ( X::*func )( Args... )
                                                     const ) {
    return FastDelegate< RetType( Args... ) >( x, func );
}
