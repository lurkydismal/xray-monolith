...
EffectStateFactory* getFactoryByType( EffectSlotType type ) {
    auto iter =
        std::find_if( std::begin( FactoryList ), std::end( FactoryList ),
...
inline ALeffect* LookupEffect( ALCdevice* device, ALuint id ) noexcept {
    ...
    if ( lidx >= device->EffectList.size() )
        UNLIKELY
    return nullptr;
...
void AddActiveEffectSlots( const al::span< ALeffectslot* > auxslots,
                           ALCcontext* context ) {
    if ( auxslots.empty() )
        return;
    EffectSlotArray* curarray{
        context->mActiveAuxSlots.load( std::memory_order_acquire ) };
    ...
    auto slotiter = std::transform(
        auxslots.begin(), auxslots.end(), newarray->begin(),
    ...
    newcount =
        static_cast< size_t >( std::distance( newarray->begin(), last ) );

    /* Reallocate newarray if the new size ended up smaller from duplicate
     * removal.
     */
    if ( newcount < newarray->size() )
        UNLIKELY {
            curarray = newarray;
    ...
    curarray = context->mActiveAuxSlots.exchange( newarray,
...
void RemoveActiveEffectSlots( const al::span< ALeffectslot* > auxslots,
                              ALCcontext* context ) {
    if ( auxslots.empty() )
        return;
    EffectSlotArray* curarray{
    ...
    auto new_end =
        std::copy( curarray->begin(), curarray->end(), newarray->begin() );
    ...
    auto newsize =
        static_cast< size_t >( std::distance( newarray->begin(), new_end ) );
...
EffectSlotType EffectSlotTypeFromEnum( ALenum type ) {
    switch ( type ) {
        ...
        case AL_EFFECT_CONVOLUTION_REVERB_SOFT:
...
bool EnsureEffectSlots( ALCcontext* context, size_t needed ) {
    size_t count{ std::accumulate(
        context->mEffectSlotList.cbegin(), context->mEffectSlotList.cend(),
        size_t{ 0 },
    ...
    while ( needed > count ) {
        if ( context->mEffectSlotList.size() >= 1 << 25 )
            UNLIKELY
        return false;
...
ALeffectslot* AllocEffectSlot( ALCcontext* context ) {
    auto sublist = std::find_if(
        context->mEffectSlotList.begin(), context->mEffectSlotList.end(),
        []( const EffectSlotSubList& entry ) noexcept -> bool {
            return entry.FreeMask != 0;
        } );
    auto lidx = static_cast< ALuint >(
        std::distance( context->mEffectSlotList.begin(), sublist ) );
    auto slidx = static_cast< ALuint >( al::countr_zero( sublist->FreeMask ) );
    ASSUME( slidx < 64 );

    ALeffectslot* slot{
        al::construct_at( sublist->EffectSlots + slidx, context ) };
...
alGenAuxiliaryEffectSlots( ALsizei n, ALuint* effectslots ) START_API_FUNC {
    ...
    if ( static_cast< ALuint >( n ) >
         device->AuxiliaryEffectSlotMax - context->mNumEffectSlots ) {
        context->setError(
            AL_OUT_OF_MEMORY, "Exceeding %u effect slot limit (%u + %d)",
...
alDeleteAuxiliaryEffectSlots( ALsizei n,
                              const ALuint* effectslots ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
    ...
    if ( n == 1 ) {
        ALeffectslot* slot{
            LookupEffectSlot( context.get(), effectslots[ 0 ] ) };
        if ( !slot )
            UNLIKELY {
                context->setError( AL_INVALID_NAME, "Invalid effect slot ID %u",
                                   effectslots[ 0 ] );
                return;
            }
        ...
        for ( size_t i{ 0 }; i < slots.size(); ++i ) {
            ALeffectslot* slot{
                LookupEffectSlot( context.get(), effectslots[ i ] ) };
            if ( !slot )
                UNLIKELY {
                    context->setError( AL_INVALID_NAME,
...
AL_API void AL_APIENTRY alAuxiliaryEffectSloti( ALuint effectslot,
    ...
    if ( !slot )
        UNLIKELY
    return context->setError( AL_INVALID_NAME, "Invalid effect slot ID %u",
                              effectslot );
    ...
    switch ( param ) {
        case AL_EFFECTSLOT_EFFECT:
...

            {
                ...
                ALeffect* effect{
                    value
                        ? LookupEffect( device, static_cast< ALuint >( value ) )
                        : nullptr };
                if ( effect )
                    err = slot->initEffect( effect->type, effect->Props,
                                            context.get() );
                else {
                    if ( value != 0 )
                        return context->setError(
                            AL_INVALID_VALUE, "Invalid effect ID %u", value );
                    err = slot->initEffect( AL_EFFECT_NULL, EffectProps{},
                                            context.get() );
                }
            }
            if ( err != AL_NO_ERROR )
        ...
        case AL_EFFECTSLOT_TARGET_SOFT:
            target = LookupEffectSlot( context.get(),
                                       static_cast< ALuint >( value ) );
            if ( value && !target )
            ...
            if ( target ) {
                ...
                if ( checker )
                    return context->setError(
                        ...
                        "Setting target of effect slot ID %u to %u creates "
                        "circular chain",
                        slot->id, target->id );
            }
        ...
        case AL_BUFFER:
...

            {
                ...
                if ( value ) {
                    buffer =
                        LookupBuffer( device, static_cast< ALuint >( value ) );
                    if ( !buffer )
                        return context->setError( AL_INVALID_VALUE,
                                                  "Invalid buffer ID" );
                    if ( buffer->mCallback )
        ...
        default:
            return context->setError(
                AL_INVALID_ENUM, "Invalid effect slot integer property 0x%04x",
                param );
    }
...
AL_API void AL_APIENTRY alAuxiliaryEffectSlotf( ALuint effectslot,
    ...
    switch ( param ) {
        ...
        default:
            return context->setError(
                AL_INVALID_ENUM, "Invalid effect slot float property 0x%04x",
                param );
    }
    UpdateProps( slot, context.get() );
}
...
ALeffectslot::ALeffectslot( ALCcontext* context ) {
    ...
    if ( !factory )
        throw std::runtime_error{ "Failed to get null effect factory" };
...
ALenum ALeffectslot::initEffect( ALenum effectType,
...
void ALeffectslot::updateProps( ALCcontext* context ) {
    ...
    EffectSlotProps* props{
        context->mFreeEffectslotProps.load( std::memory_order_relaxed ) };
    if ( !props )
        ...
        } while ( context->mFreeEffectslotProps.compare_exchange_weak(
                      props, next, std::memory_order_seq_cst,
    ...
    for ( auto& sublist : context->mEffectSlotList ) {
        ...
        while ( usemask ) {
            ...
            if ( slot->mState != SlotState::Stopped &&
                 std::exchange( slot->mPropsDirty, false ) )
                slot->updateProps( context );
        }
    }
}
...
#ifdef ALSOFT_EAX
void ALeffectslot::eax_initialize( ALCcontext& al_context,
    ...
    if ( index == 0 )
        eax_effect_->init< EaxReverbCommitter >();
    else if ( index == 1 )
        eax_effect_->init< EaxChorusCommitter >();
    else
        eax_effect_->init< EaxNullCommitter >();
}
...
void ALeffectslot::eax4_fx_slot_get( const EaxCall& call,
                                     const Eax4Props& props ) const {
...
void ALeffectslot::eax5_fx_slot_get( const EaxCall& call,
                                     const Eax5Props& props ) const {
    switch ( call.get_property_id() ) {
        case EAXFXSLOT_ALLPARAMETERS:
            call.set_value< Exception >( props );
            break;
...
void ALeffectslot::eax_fx_slot_get( const EaxCall& call ) const {
    switch ( call.get_version() ) {
        case 4:
            eax4_fx_slot_get( call, eax4_.i );
            break;
        case 5:
            eax5_fx_slot_get( call, eax5_.i );
            break;
        default:
...
void ALeffectslot::eax_fx_slot_set_volume() {
    const auto volume =
        clamp( eax_.lVolume, EAXFXSLOT_MINVOLUME, EAXFXSLOT_MAXVOLUME );
...
void ALeffectslot::eax_fx_slot_set_environment_flag() {
    eax_set_efx_slot_send_auto( ( eax_.ulFlags & EAXFXSLOTFLAGS_ENVIRONMENT ) !=
                                0u );
}
...
void ALeffectslot::eax4_fx_slot_set_all( const EaxCall& call ) {
    ...
    const auto& src =
        call.get_value< Exception, const EAX40FXSLOTPROPERTIES >();
    Eax4AllValidator{}( src );
    auto& dst = eax4_.i;
    eax_df_ |= eax_load_effect_dirty_bit; // Always reset the effect.
    eax_df_ |=
        ( dst.lVolume != src.lVolume ? eax_volume_dirty_bit : EaxDirtyFlags{} );
    eax_df_ |=
        ( dst.lLock != src.lLock ? eax_lock_dirty_bit : EaxDirtyFlags{} );
    eax_df_ |=
        ( dst.ulFlags != src.ulFlags ? eax_flags_dirty_bit : EaxDirtyFlags{} );
    dst = src;
}

void ALeffectslot::eax5_fx_slot_set_all( const EaxCall& call ) {
    const auto& src =
        call.get_value< Exception, const EAX50FXSLOTPROPERTIES >();
    ...
    eax_df_ |=
        ( dst.ulFlags != src.ulFlags ? eax_flags_dirty_bit : EaxDirtyFlags{} );
    eax_df_ |= ( dst.lOcclusion != src.lOcclusion ? eax_flags_dirty_bit
                                                  : EaxDirtyFlags{} );
    eax_df_ |=
        ( dst.flOcclusionLFRatio != src.flOcclusionLFRatio ? eax_flags_dirty_bit
                                                           : EaxDirtyFlags{} );
    dst = src;
}

bool ALeffectslot::eax_fx_slot_should_update_sources() const noexcept {
    const auto dirty_bits = eax_occlusion_dirty_bit |
        ...
        case EAXFXSLOT_ALLPARAMETERS:
            ...
            if ( ( eax_df_ & eax_load_effect_dirty_bit ) )
                eax_fx_slot_load_effect(
                    4, eax_get_efx_effect_type( dst.guidLoadEffect ) );
            break;
        case EAXFXSLOT_LOADEFFECT:
            eax4_fx_slot_ensure_unlocked();
            eax_fx_slot_set_dirty< Eax4GuidLoadEffectValidator,
                                   eax_load_effect_dirty_bit >(
                call, dst.guidLoadEffect, eax_df_ );
            if ( ( eax_df_ & eax_load_effect_dirty_bit ) )
                eax_fx_slot_load_effect(
                    4, eax_get_efx_effect_type( dst.guidLoadEffect ) );
            break;
        case EAXFXSLOT_VOLUME:
            eax_fx_slot_set< Eax4VolumeValidator, eax_volume_dirty_bit >(
                call, dst.lVolume, eax_df_ );
            break;
        case EAXFXSLOT_LOCK:
            eax4_fx_slot_ensure_unlocked();
            eax_fx_slot_set< Eax4LockValidator, eax_lock_dirty_bit >(
                call, dst.lLock, eax_df_ );
            break;
        case EAXFXSLOT_FLAGS:
            eax_fx_slot_set< Eax4FlagsValidator, eax_flags_dirty_bit >(
                call, dst.ulFlags, eax_df_ );
            break;
        default:
            eax_fail_unknown_property_id();
    }
...
bool ALeffectslot::eax5_fx_slot_set( const EaxCall& call ) {
    ...
    switch ( call.get_property_id() ) {
        ...
        case EAXFXSLOT_ALLPARAMETERS:
            ...
            if ( ( eax_df_ & eax_load_effect_dirty_bit ) )
                eax_fx_slot_load_effect(
                    5, eax_get_efx_effect_type( dst.guidLoadEffect ) );
            break;
        case EAXFXSLOT_LOADEFFECT:
            eax_fx_slot_set_dirty< Eax4GuidLoadEffectValidator,
                                   eax_load_effect_dirty_bit >(
        ...
        case EAXFXSLOT_LOCK:
            ...
            break;
        case EAXFXSLOT_FLAGS:
            eax_fx_slot_set< Eax5FlagsValidator, eax_flags_dirty_bit >(
                call, dst.ulFlags, eax_df_ );
            break;
        case EAXFXSLOT_OCCLUSION:
            eax_fx_slot_set< Eax5OcclusionValidator, eax_occlusion_dirty_bit >(
                call, dst.lOcclusion, eax_df_ );
            break;
        case EAXFXSLOT_OCCLUSIONLFRATIO:
            eax_fx_slot_set< Eax5OcclusionLfRatioValidator,
                             eax_occlusion_lf_ratio_dirty_bit >(
                call, dst.flOcclusionLFRatio, eax_df_ );
            break;
        default:
            eax_fail_unknown_property_id();
    }
...
bool ALeffectslot::eax_fx_slot_set( const EaxCall& call ) {
    switch ( call.get_version() ) {
        case 4:
            return eax4_fx_slot_set( call );
        case 5:
            return eax5_fx_slot_set( call );
        default:
            eax_fail_unknown_version();
    }
}
...
bool ALeffectslot::eax_set( const EaxCall& call ) {
    ...
    switch ( call.get_property_set_id() ) {
        case EaxCallPropertySetId::fx_slot:
            ret = eax_fx_slot_set( call );
            break;
        case EaxCallPropertySetId::fx_slot_effect:
            eax_effect_->set( call );
            break;
        default:
            eax_fail_unknown_property_id();
    }
...
void ALeffectslot::eax4_fx_slot_commit( EaxDirtyFlags& dst_df ) {
    eax_fx_slot_commit_property< eax_load_effect_dirty_bit >(
        eax4_, dst_df, &EAX40FXSLOTPROPERTIES::guidLoadEffect );
    eax_fx_slot_commit_property< eax_volume_dirty_bit >(
        eax4_, dst_df, &EAX40FXSLOTPROPERTIES::lVolume );
    eax_fx_slot_commit_property< eax_lock_dirty_bit >(
        eax4_, dst_df, &EAX40FXSLOTPROPERTIES::lLock );
    eax_fx_slot_commit_property< eax_flags_dirty_bit >(
        eax4_, dst_df, &EAX40FXSLOTPROPERTIES::ulFlags );
...
void ALeffectslot::eax5_fx_slot_commit( Eax5State& state,
                                        EaxDirtyFlags& dst_df ) {
    eax_fx_slot_commit_property< eax_load_effect_dirty_bit >(
        state, dst_df, &EAX50FXSLOTPROPERTIES::guidLoadEffect );
    eax_fx_slot_commit_property< eax_volume_dirty_bit >(
        state, dst_df, &EAX50FXSLOTPROPERTIES::lVolume );
    eax_fx_slot_commit_property< eax_lock_dirty_bit >(
        state, dst_df, &EAX50FXSLOTPROPERTIES::lLock );
    eax_fx_slot_commit_property< eax_flags_dirty_bit >(
        state, dst_df, &EAX50FXSLOTPROPERTIES::ulFlags );
    eax_fx_slot_commit_property< eax_occlusion_dirty_bit >(
        state, dst_df, &EAX50FXSLOTPROPERTIES::lOcclusion );
    eax_fx_slot_commit_property< eax_occlusion_lf_ratio_dirty_bit >(
...
#define EAX_PREFIX "[EAX_SET_EFFECT_SLOT_EFFECT] "
    ...
    const auto error = initEffect( effect.al_effect_type_,
                                   effect.al_effect_props_, eax_al_context_ );
...
void eax_delete_al_effect_slot( ALCcontext& context,
