...
#define HANDLE_PHENOME( x )            \
    case AL_VOCAL_MORPHER_PHONEME_##x: \
...
#define HANDLE_PHENOME( x )  \
    case VMorpherPhenome::x: \
        return AL_VOCAL_MORPHER_PHONEME_##x
    switch ( phenome ) {
        HANDLE_PHENOME( A );
        HANDLE_PHENOME( E );
        HANDLE_PHENOME( I );
    ...
    throw std::runtime_error{ "Invalid phenome: " +
                              std::to_string( static_cast< int >( phenome ) ) };
#undef HANDLE_PHENOME
}
...
al::optional< VMorpherWaveform > WaveformFromEmum( ALenum value ) {
    switch ( value ) {
        ...
        case AL_VOCAL_MORPHER_WAVEFORM_SAWTOOTH:
            return VMorpherWaveform::Sawtooth;
    }
    return al::nullopt;
}
...
ALenum EnumFromWaveform( VMorpherWaveform type ) {
    switch ( type ) {
        ...
        case VMorpherWaveform::Sawtooth:
...
void Vmorpher_setParami( EffectProps* props, ALenum param, int val ) {
    switch ( param ) {
        ...
        case AL_VOCAL_MORPHER_PHONEMEA_COARSE_TUNING:
            if ( !( val >= AL_VOCAL_MORPHER_MIN_PHONEMEA_COARSE_TUNING &&
                    val <= AL_VOCAL_MORPHER_MAX_PHONEMEA_COARSE_TUNING ) )
                throw effect_exception{
                    AL_INVALID_VALUE,
        ...
        case AL_VOCAL_MORPHER_PHONEMEB_COARSE_TUNING:
            if ( !( val >= AL_VOCAL_MORPHER_MIN_PHONEMEB_COARSE_TUNING &&
                    val <= AL_VOCAL_MORPHER_MAX_PHONEMEB_COARSE_TUNING ) )
                throw effect_exception{
                    AL_INVALID_VALUE,
...
void Vmorpher_setParamf( EffectProps* props, ALenum param, float val ) {
    switch ( param ) {
        case AL_VOCAL_MORPHER_RATE:
            if ( !( val >= AL_VOCAL_MORPHER_MIN_RATE &&
                    val <= AL_VOCAL_MORPHER_MAX_RATE ) )
        ...
        default:
            throw effect_exception{
                AL_INVALID_ENUM, "Invalid vocal morpher float property 0x%04x",
                param };
    }
}
...
struct PhonemeAValidator {
    void operator()( unsigned long ulPhonemeA ) const {
        eax_validate_range< VocalMorpherCommitter::Exception >(
            "Phoneme A", ulPhonemeA, EAXVOCALMORPHER_MINPHONEMEA,
...
struct PhonemeACoarseTuningValidator {
    void operator()( long lPhonemeACoarseTuning ) const {
        eax_validate_range< VocalMorpherCommitter::Exception >(
            "Phoneme A Coarse Tuning", lPhonemeACoarseTuning,
...
struct PhonemeBCoarseTuningValidator {
    void operator()( long lPhonemeBCoarseTuning ) const {
        eax_validate_range< VocalMorpherCommitter::Exception >(
            "Phoneme B Coarse Tuning", lPhonemeBCoarseTuning,
...
struct WaveformValidator {
    void operator()( unsigned long ulWaveform ) const {
        eax_validate_range< VocalMorpherCommitter::Exception >(
            "Waveform", ulWaveform, EAXVOCALMORPHER_MINWAVEFORM,
...
struct RateValidator {
    void operator()( float flRate ) const {
        eax_validate_range< VocalMorpherCommitter::Exception >(
            "Rate", flRate, EAXVOCALMORPHER_MINRATE, EAXVOCALMORPHER_MAXRATE );
    }
}; // RateValidator
...
bool VocalMorpherCommitter::commit( const EaxEffectProps& props ) {
    if ( props.mType == mEaxProps.mType &&
         ...
         mEaxProps.mVocalMorpher.lPhonemeACoarseTuning ==
             props.mVocalMorpher.lPhonemeACoarseTuning &&
         mEaxProps.mVocalMorpher.ulPhonemeB == props.mVocalMorpher.ulPhonemeB &&
         mEaxProps.mVocalMorpher.lPhonemeBCoarseTuning ==
             props.mVocalMorpher.lPhonemeBCoarseTuning &&
...
#undef HANDLE_PHENOME
    ...
    auto get_waveform = []( unsigned long form ) noexcept {
        if ( form == EAX_VOCALMORPHER_SINUSOID )
            return VMorpherWaveform::Sinusoid;
        if ( form == EAX_VOCALMORPHER_TRIANGLE )
            return VMorpherWaveform::Triangle;
        if ( form == EAX_VOCALMORPHER_SAWTOOTH )
            return VMorpherWaveform::Sawtooth;
        return VMorpherWaveform::Sinusoid;
    };

    mAlProps.Vmorpher.PhonemeA = get_phoneme( props.mVocalMorpher.ulPhonemeA );
    mAlProps.Vmorpher.PhonemeACoarseTuning =
        static_cast< int >( props.mVocalMorpher.lPhonemeACoarseTuning );
    mAlProps.Vmorpher.PhonemeB = get_phoneme( props.mVocalMorpher.ulPhonemeB );
    mAlProps.Vmorpher.PhonemeBCoarseTuning =
        static_cast< int >( props.mVocalMorpher.lPhonemeBCoarseTuning );
...
template <>
void VocalMorpherCommitter::Get( const EaxCall& call,
...
void VocalMorpherCommitter::Set( const EaxCall& call, EaxEffectProps& props ) {
    switch ( call.get_property_id() ) {
        ...
        case EAXVOCALMORPHER_PHONEMEACOARSETUNING:
            defer< PhonemeACoarseTuningValidator >(
                call, props.mVocalMorpher.lPhonemeACoarseTuning );
            break;
        ...
        case EAXVOCALMORPHER_PHONEMEBCOARSETUNING:
            defer< PhonemeBCoarseTuningValidator >(
                call, props.mVocalMorpher.lPhonemeBCoarseTuning );
            break;
