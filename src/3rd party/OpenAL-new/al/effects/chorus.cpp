...
static_assert( ChorusMaxDelay >= AL_CHORUS_MAX_DELAY,
               "Chorus max delay too small" );
static_assert( FlangerMaxDelay >= AL_FLANGER_MAX_DELAY,
               "Flanger max delay too small" );

static_assert( AL_CHORUS_WAVEFORM_SINUSOID == AL_FLANGER_WAVEFORM_SINUSOID,
               "Chorus/Flanger waveform value mismatch" );
static_assert( AL_CHORUS_WAVEFORM_TRIANGLE == AL_FLANGER_WAVEFORM_TRIANGLE,
               "Chorus/Flanger waveform value mismatch" );
...
inline al::optional< ChorusWaveform > WaveformFromEnum( ALenum type ) {
    switch ( type ) {
        ...
        case AL_CHORUS_WAVEFORM_TRIANGLE:
            return ChorusWaveform::Triangle;
    }
    return al::nullopt;
}
inline ALenum EnumFromWaveform( ChorusWaveform type ) {
    ...
    throw std::runtime_error{ "Invalid chorus waveform: " +
    ...
    switch ( param ) {
        case AL_CHORUS_WAVEFORM:
            if ( auto formopt = WaveformFromEnum( val ) )
                ...
                throw effect_exception{
                    AL_INVALID_VALUE, "Invalid chorus waveform: 0x%04x", val };
            break;
        ...
        default:
            throw effect_exception{ AL_INVALID_ENUM,
                                    "Invalid chorus integer property 0x%04x",
                                    param };
    }
}
...
void Chorus_setParamf( EffectProps* props, ALenum param, float val ) {
    switch ( param ) {
        ...
        case AL_CHORUS_FEEDBACK:
            if ( !( val >= AL_CHORUS_MIN_FEEDBACK &&
                    val <= AL_CHORUS_MAX_FEEDBACK ) )
                throw effect_exception{
                    AL_INVALID_VALUE, "Chorus feedback out of range: %f", val };
        ...
        default:
            throw effect_exception{ AL_INVALID_ENUM,
                                    "Invalid chorus float property 0x%04x",
                                    param };
    }
}
    ...
    switch ( param ) {
        case AL_FLANGER_WAVEFORM:
            if ( auto formopt = WaveformFromEnum( val ) )
                ...
                throw effect_exception{
                    AL_INVALID_VALUE, "Invalid flanger waveform: 0x%04x", val };
            break;

        case AL_FLANGER_PHASE:
            if ( !( val >= AL_FLANGER_MIN_PHASE &&
                    val <= AL_FLANGER_MAX_PHASE ) )
                throw effect_exception{ AL_INVALID_VALUE,
        ...
        default:
            throw effect_exception{ AL_INVALID_ENUM,
                                    "Invalid flanger integer property 0x%04x",
                                    param };
    }
}
...
void Flanger_setParamf( EffectProps* props, ALenum param, float val ) {
    switch ( param ) {
        ...
        case AL_FLANGER_DEPTH:
            if ( !( val >= AL_FLANGER_MIN_DEPTH &&
                    val <= AL_FLANGER_MAX_DEPTH ) )
                throw effect_exception{ AL_INVALID_VALUE,
        ...
        case AL_FLANGER_FEEDBACK:
            if ( !( val >= AL_FLANGER_MIN_FEEDBACK &&
                    val <= AL_FLANGER_MAX_FEEDBACK ) )
                throw effect_exception{ AL_INVALID_VALUE,
                                        "Flanger feedback out of range: %f",
                                        val };
            props->Chorus.Feedback = val;
            break;

        case AL_FLANGER_DELAY:
            if ( !( val >= AL_FLANGER_MIN_DELAY &&
                    val <= AL_FLANGER_MAX_DELAY ) )
                throw effect_exception{ AL_INVALID_VALUE,
        ...
        default:
            throw effect_exception{ AL_INVALID_ENUM,
                                    "Invalid flanger float property 0x%04x",
                                    param };
    }
}
...
struct EaxChorusTraits {
    ...
    static ChorusWaveform eax_waveform( unsigned long type ) {
        if ( type == EAX_CHORUS_SINUSOID )
            return ChorusWaveform::Sinusoid;
        if ( type == EAX_CHORUS_TRIANGLE )
            return ChorusWaveform::Triangle;
        return ChorusWaveform::Sinusoid;
    }
}; // EaxChorusTraits
...
struct EaxFlangerTraits {
    ...
    static ChorusWaveform eax_waveform( unsigned long type ) {
        if ( type == EAX_FLANGER_SINUSOID )
            return ChorusWaveform::Sinusoid;
        if ( type == EAX_FLANGER_TRIANGLE )
            return ChorusWaveform::Triangle;
        return ChorusWaveform::Sinusoid;
    }
}; // EaxFlangerTraits
...
struct ChorusFlangerEffect {
    ...
    struct WaveformValidator {
        void operator()( unsigned long ulWaveform ) const {
            eax_validate_range< Exception >( "Waveform", ulWaveform,
    ...
    struct PhaseValidator {
        void operator()( long lPhase ) const {
            eax_validate_range< Exception >( "Phase", lPhase,
    ...
    struct RateValidator {
        void operator()( float flRate ) const {
            eax_validate_range< Exception >( "Rate", flRate,
                                             Traits::eax_min_rate(),
                                             Traits::eax_max_rate() );
        }
    }; // RateValidator
    ...
    struct DepthValidator {
        void operator()( float flDepth ) const {
            eax_validate_range< Exception >( "Depth", flDepth,
    ...
    struct FeedbackValidator {
        void operator()( float flFeedback ) const {
            eax_validate_range< Exception >( "Feedback", flFeedback,
    ...
    struct DelayValidator {
        void operator()( float flDelay ) const {
            eax_validate_range< Exception >( "Delay", flDelay,
...
public:
    ...
    static void Set( const EaxCall& call, EaxEffectProps& props ) {
        ...
        switch ( call.get_property_id() ) {
            ...
            case Traits::eax_waveform_param_id():
                Committer::template defer< WaveformValidator >(
                    call, all.ulWaveform );
                break;
            ...
            case Traits::eax_depth_param_id():
                Committer::template defer< DepthValidator >( call,
                                                             all.flDepth );
                break;

            case Traits::eax_feedback_param_id():
                Committer::template defer< FeedbackValidator >(
                    call, all.flFeedback );
                break;

            case Traits::eax_delay_param_id():
                Committer::template defer< DelayValidator >( call,
                                                             all.flDelay );
                break;
    ...
    static bool Commit( const EaxEffectProps& props,
                        EaxEffectProps& props_,
                        EffectProps& al_props_ ) {
        if ( props.mType == props_.mType ) {
            ...
            if ( dst.ulWaveform == src.ulWaveform && dst.lPhase == src.lPhase &&
                 ...
                 dst.flFeedback == src.flFeedback &&
                 dst.flDelay == src.flDelay )
                return false;
        }
