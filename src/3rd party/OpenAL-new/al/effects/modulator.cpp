...
ALenum EnumFromWaveform( ModulatorWaveform type ) {
    switch ( type ) {
        ...
        case ModulatorWaveform::Sawtooth:
    ...
    switch ( param ) {
        case AL_RING_MODULATOR_FREQUENCY:
            if ( !( val >= AL_RING_MODULATOR_MIN_FREQUENCY &&
                ...
                throw effect_exception{ AL_INVALID_VALUE,
                                        "Modulator frequency out of range: %f",
                                        val };
        ...
        default:
            throw effect_exception{ AL_INVALID_ENUM,
                                    "Invalid modulator float property 0x%04x",
                                    param };
    }
}
    ...
    switch ( param ) {
        ...
        case AL_RING_MODULATOR_WAVEFORM:
            if ( auto formopt = WaveformFromEmum( val ) )
                ...
                throw effect_exception{ AL_INVALID_VALUE,
                                        "Invalid modulator waveform: 0x%04x",
                                        val };
            break;
...
struct FrequencyValidator {
    void operator()( float flFrequency ) const {
        eax_validate_range< ModulatorCommitter::Exception >(
            "Frequency", flFrequency, EAXRINGMODULATOR_MINFREQUENCY,
...
struct HighPassCutOffValidator {
    void operator()( float flHighPassCutOff ) const {
        eax_validate_range< ModulatorCommitter::Exception >(
            "High-Pass Cutoff", flHighPassCutOff,
...
struct WaveformValidator {
    void operator()( unsigned long ulWaveform ) const {
        eax_validate_range< ModulatorCommitter::Exception >(
            "Waveform", ulWaveform, EAXRINGMODULATOR_MINWAVEFORM,
...
bool ModulatorCommitter::commit( const EaxEffectProps& props ) {
    if ( props.mType == mEaxProps.mType &&
         ...
         mEaxProps.mModulator.flHighPassCutOff ==
             props.mModulator.flHighPassCutOff &&
...
template <>
void ModulatorCommitter::Get( const EaxCall& call,
                              const EaxEffectProps& props ) {
    switch ( call.get_property_id() ) {
        case EAXRINGMODULATOR_NONE:
            break;
        case EAXRINGMODULATOR_ALLPARAMETERS:
            call.set_value< Exception >( props.mModulator );
            break;
        case EAXRINGMODULATOR_FREQUENCY:
            call.set_value< Exception >( props.mModulator.flFrequency );
            break;
        case EAXRINGMODULATOR_HIGHPASSCUTOFF:
            call.set_value< Exception >( props.mModulator.flHighPassCutOff );
            break;
        case EAXRINGMODULATOR_WAVEFORM:
            call.set_value< Exception >( props.mModulator.ulWaveform );
            break;
        default:
...
void ModulatorCommitter::Set( const EaxCall& call, EaxEffectProps& props ) {
    switch ( call.get_property_id() ) {
        case EAXRINGMODULATOR_NONE:
            break;
        case EAXRINGMODULATOR_ALLPARAMETERS:
            defer< AllValidator >( call, props.mModulator );
            break;
        case EAXRINGMODULATOR_FREQUENCY:
            defer< FrequencyValidator >( call, props.mModulator.flFrequency );
            break;
        case EAXRINGMODULATOR_HIGHPASSCUTOFF:
            defer< HighPassCutOffValidator >(
                call, props.mModulator.flHighPassCutOff );
            break;
        case EAXRINGMODULATOR_WAVEFORM:
            defer< WaveformValidator >( call, props.mModulator.ulWaveform );
            break;
        default:
            fail_unknown_property_id();
    }
