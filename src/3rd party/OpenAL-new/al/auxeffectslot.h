...
struct ALeffectslot {
    ...
    ALenum initEffect( ALenum effectType,
...
private:
    ...
    static constexpr auto eax_occlusion_lf_ratio_dirty_bit = EaxDirtyFlags{ 1 }
                                                             << 5;
    ...
    struct EaxRangeValidator {
        ...
        void operator()( const char* name,
            ...
            eax_validate_range< Exception >( name, value, min_value,
                                             max_value );
        }
    };
    ...
    struct Eax4VolumeValidator {
        void operator()( long lVolume ) const {
            EaxRangeValidator{}( "Volume", lVolume, EAXFXSLOT_MINVOLUME,
                                 EAXFXSLOT_MAXVOLUME );
        }
    };
    ...
    struct Eax4LockValidator {
        void operator()( long lLock ) const {
            EaxRangeValidator{}( "Lock", lLock, EAXFXSLOT_MINLOCK,
                                 EAXFXSLOT_MAXLOCK );
        }
    };
    ...
    struct Eax4FlagsValidator {
        void operator()( unsigned long ulFlags ) const {
            EaxRangeValidator{}( "Flags", ulFlags, 0UL,
    ...
    struct Eax5OcclusionValidator {
        void operator()( long lOcclusion ) const {
            EaxRangeValidator{}( "Occlusion", lOcclusion,
    ...
    struct Eax5OcclusionLfRatioValidator {
        void operator()( float flOcclusionLFRatio ) const {
            EaxRangeValidator{}( "Occlusion LF Ratio", flOcclusionLFRatio,
    ...
    struct Eax5AllValidator {
        void operator()( const EAX50FXSLOTPROPERTIES& all ) const {
            Eax4AllValidator{}(
                static_cast< const EAX40FXSLOTPROPERTIES& >( all ) );
    ...
    static void eax_fx_slot_set( const EaxCall& call,
                                 TProperties& dst,
    ...
    template < typename TValidator,
               EaxDirtyFlags TDirtyBit,
               typename TProperties >
    static void eax_fx_slot_set_dirty( const EaxCall& call,
                                       TProperties& dst,
                                       EaxDirtyFlags& dirty_flags ) {
        const auto& src = call.get_value< Exception, const TProperties >();
        TValidator{}( src );
    ...
    // `alAuxiliaryEffectSloti(effect_slot, AL_EFFECTSLOT_AUXILIARY_SEND_AUTO,
    // value)`
...
#ifdef ALSOFT_EAX
using EaxAlEffectSlotUPtr =
    std::unique_ptr< ALeffectslot, ALeffectslot::EaxDeleter >;
