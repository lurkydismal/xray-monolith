...
void UpdateSourceProps( const ALsource* source,
                        Voice* voice,
                        ALCcontext* context ) {
    /* Get an unused property container, or allocate a new one as needed. */
    VoicePropsItem* props{
        context->mFreeVoiceProps.load( std::memory_order_acquire ) };
    if ( !props ) {
    ...
    } while ( context->mFreeVoiceProps.compare_exchange_weak(
                  props, next, std::memory_order_acq_rel,
    ...
    auto copy_send = []( const ALsource::SendData& srcsend ) noexcept
        -> VoiceProps::SendData {
    ...
    std::transform( source->Send.cbegin(), source->Send.cend(), props->Send,
                    copy_send );
 ...
 * the current buffer).
 */
int64_t GetSourceSampleOffset( ALsource* Source,
                               ALCcontext* context,
                               nanoseconds* clocktime ) {
    ...
    do {
        ...
        *clocktime = GetDeviceClockTime( device );
        ...
        if ( voice ) {
            ...
            readPos =
                int64_t{ voice->mPosition.load( std::memory_order_relaxed ) }
                << MixerFracBits;
    ...
    if ( readPos > std::numeric_limits< int64_t >::max() >>
         ( 32 - MixerFracBits ) )
 ...
 */
double GetSourceSecOffset( ALsource* Source,
                           ALCcontext* context,
                           nanoseconds* clocktime ) {
    ALCdevice* device{ context->mALDevice.get() };
    const VoiceBufferItem* Current{};
    ...
    while ( BufferList != Source->mQueue.cend() &&
            al::to_address( BufferList ) != Current ) {
        if ( !BufferFmt )
    ...
    return static_cast< double >( readPos ) / double{ MixerFracOne } /
...
double GetSourceOffset( ALsource* Source, ALenum name, ALCcontext* context ) {
    ...
    switch ( name ) {
        case AL_SEC_OFFSET:
            offset = static_cast< double >( readPos ) +
                     readPosFrac / double{ MixerFracOne };
        ...
        case AL_BYTE_OFFSET:
            ...
            offset =
                static_cast< double >( readPos / BlockSamples ) * BlockSize;
            break;
    }
    return offset;
}
 ...
 */
al::optional< VoicePos > GetSampleOffset(
    al::deque< ALbufferQueueItem >& BufferList,
    ALenum OffsetType,
    double Offset ) {
    ...
    switch ( OffsetType ) {
        case AL_SEC_OFFSET:
            ...
            if ( dblfrac < 0.0 ) {
                 ...
                 * value (e.g. -2.75 -> -3 + 0.25).
                 */
                dbloff -= 1.0;
                dblfrac += 1.0;
            }
            offset = static_cast< int64_t >( dbloff );
            frac = static_cast< uint >(
                mind( dblfrac * MixerFracOne, MixerFracOne - 1.0 ) );
            break;
    ...
    if ( offset < 0 ) {
        if ( offset < std::numeric_limits< int >::min() )
            return al::nullopt;
        return VoicePos{ static_cast< int >( offset ), frac,
                         &BufferList.front() };
    }
    ...
    for ( auto& item : BufferList ) {
        ...
        if ( item.mSampleLen > offset - totalBufferLen ) {
            ...
            return VoicePos{ static_cast< int >( offset - totalBufferLen ),
                             frac, &item };
        }
...
void InitVoice( Voice* voice,
                ...
                ALCcontext* context,
                ALCdevice* device ) {
    voice->mLoopBuffer.store(
        source->Looping ? &source->mQueue.front() : nullptr,
        std::memory_order_relaxed );

    ALbuffer* buffer{ BufferList->mBuffer };
    voice->mFrequency = buffer->mSampleRate;
    voice->mFmtChannels = ( buffer->mChannels == FmtStereo &&
                            source->mStereoMode == SourceStereo::Enhanced )
                              ? FmtSuperStereo
                              : buffer->mChannels;
    ...
    voice->mAmbiLayout =
        IsUHJ( voice->mFmtChannels ) ? AmbiLayout::FuMa : buffer->mAmbiLayout;
    voice->mAmbiScaling =
        IsUHJ( voice->mFmtChannels ) ? AmbiScaling::UHJ : buffer->mAmbiScaling;
    voice->mAmbiOrder =
        ( voice->mFmtChannels == FmtSuperStereo ) ? 1 : buffer->mAmbiOrder;

    if ( buffer->mCallback )
        voice->mFlags.set( VoiceIsCallback );
    else if ( source->SourceType == AL_STATIC )
        voice->mFlags.set( VoiceIsStatic );
...
VoiceChange* GetVoiceChanger( ALCcontext* ctx ) {
    ...
    if ( vchg == ctx->mCurrentVoiceChange.load( std::memory_order_acquire ) )
        UNLIKELY {
            ctx->allocVoiceChanges();
            vchg = ctx->mVoiceChangeTail;
        }

    ctx->mVoiceChangeTail =
        vchg->mNext.exchange( nullptr, std::memory_order_relaxed );
...
void SendVoiceChanges( ALCcontext* ctx, VoiceChange* tail ) {
    ...
    VoiceChange* oldhead{
        ctx->mCurrentVoiceChange.load( std::memory_order_acquire ) };
    while ( VoiceChange *
            next{ oldhead->mNext.load( std::memory_order_relaxed ) } )
        oldhead = next;
    ...
    if ( !connected )
        UNLIKELY {
            if ( ctx->mStopVoicesOnDisconnect.load(
                 ...
                 */
                VoiceChange* cur{ ctx->mCurrentVoiceChange.load(
                    std::memory_order_acquire ) };
                while ( VoiceChange *
                ...
                ctx->mCurrentVoiceChange.store( cur,
                                                std::memory_order_release );
            }
        }
}

bool SetVoiceOffset( Voice* oldvoice,
                     const VoicePos& vpos,
                     ALsource* source,
                     ALCcontext* context,
                     ALCdevice* device ) {
    ...
    for ( Voice* voice : voicelist ) {
        if ( voice->mPlayState.load( std::memory_order_acquire ) ==
                 Voice::Stopped &&
             voice->mSourceID.load( std::memory_order_relaxed ) == 0u &&
             voice->mPendingChange.load( std::memory_order_relaxed ) ==
                 false ) {
            newvoice = voice;
            break;
        }
    ...
    if ( !newvoice )
        UNLIKELY {
            auto& allvoices =
                *context->mVoices.load( std::memory_order_relaxed );
            if ( allvoices.size() == voicelist.size() )
                context->allocVoices( 1 );
            context->mActiveVoiceCount.fetch_add( 1,
                                                  std::memory_order_release );
    ...
    newvoice->mCurrentBuffer.store( vpos.bufferitem,
                                    std::memory_order_relaxed );
     ...
     */
    if ( oldvoice->mSourceID.load( std::memory_order_acquire ) != 0u )
        LIKELY
    return true;
     ...
     * already happened.
     */
    if ( newvoice->mPlayState.load( std::memory_order_acquire ) !=
         Voice::Pending )
        return true;
...
bool EnsureSources( ALCcontext* context, size_t needed ) {
    size_t count{ std::accumulate(
        context->mSourceList.cbegin(), context->mSourceList.cend(), size_t{ 0 },
        []( size_t cur, const SourceSubList& sublist ) noexcept -> size_t {
            return cur +
                   static_cast< ALuint >( al::popcount( sublist.FreeMask ) );
        } ) };

    while ( needed > count ) {
        if ( context->mSourceList.size() >= 1 << 25 )
            UNLIKELY
        return false;
        ...
        sublist->Sources = static_cast< ALsource* >(
            al_calloc( alignof( ALsource ), sizeof( ALsource ) * 64 ) );
        if ( !sublist->Sources )
            UNLIKELY {
...
ALsource* AllocSource( ALCcontext* context ) {
    auto sublist =
        std::find_if( context->mSourceList.begin(), context->mSourceList.end(),
                      []( const SourceSubList& entry ) noexcept -> bool {
                          return entry.FreeMask != 0;
                      } );
    auto lidx = static_cast< ALuint >(
        std::distance( context->mSourceList.begin(), sublist ) );
...
inline ALsource* LookupSource( ALCcontext* context, ALuint id ) noexcept {
    ...
    if ( lidx >= context->mSourceList.size() )
        UNLIKELY
    return nullptr;
    SourceSubList& sublist{ context->mSourceList[ lidx ] };
    if ( sublist.FreeMask & ( 1_u64 << slidx ) )
        UNLIKELY
    return nullptr;
...
inline ALbuffer* LookupBuffer( ALCdevice* device, ALuint id ) noexcept {
    ...
    if ( lidx >= device->BufferList.size() )
        UNLIKELY
    return nullptr;
...
inline ALfilter* LookupFilter( ALCdevice* device, ALuint id ) noexcept {
    ...
    if ( lidx >= device->FilterList.size() )
        UNLIKELY
    return nullptr;
...
inline ALeffectslot* LookupEffectSlot( ALCcontext* context,
                                       ALuint id ) noexcept {
    const size_t lidx{ ( id - 1 ) >> 6 };
    const ALuint slidx{ ( id - 1 ) & 0x3f };

    if ( lidx >= context->mEffectSlotList.size() )
        UNLIKELY
    return nullptr;
...
ALenum EnumFromStereoMode( SourceStereo mode ) {
    switch ( mode ) {
        ...
        case SourceStereo::Enhanced:
            return AL_SUPER_STEREO_SOFT;
    }
    throw std::runtime_error{ "Invalid SourceStereo: " +
...
ALenum EnumFromSpatializeMode( SpatializeMode mode ) {
    ...
    throw std::runtime_error{ "Invalid SpatializeMode: " +
                              std::to_string( int( mode ) ) };
}
...
al::optional< DirectMode > DirectModeFromEnum( ALenum mode ) {
    switch ( mode ) {
        ...
        case AL_DROP_UNMATCHED_SOFT:
            return DirectMode::DropMismatch;
        case AL_REMIX_UNMATCHED_SOFT:
...
ALenum EnumFromDirectMode( DirectMode mode ) {
    switch ( mode ) {
        case DirectMode::Off:
            return AL_FALSE;
        case DirectMode::DropMismatch:
            return AL_DROP_UNMATCHED_SOFT;
        case DirectMode::RemixMismatch:
            return AL_REMIX_UNMATCHED_SOFT;
    }
    throw std::runtime_error{ "Invalid DirectMode: " +
                              std::to_string( int( mode ) ) };
}
...
void SetSourcefv( ALsource* const Source,
                  ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< const float > values );
void SetSourceiv( ALsource* const Source,
                  ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< const int > values );
void SetSourcei64v( ALsource* const Source,
                    ALCcontext* const Context,
                    const SourceProp prop,
...
inline void CommitAndUpdateSourceProps( ALsource* source,
                                        ALCcontext* context ) {
...
auto GetCheckers( ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< T, N > values ) {
...
void SetSourcefv( ALsource* const Source,
                  ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< const float > values ) try {
    ...
    switch ( prop ) {
        ...
        case AL_BYTE_OFFSET:
            ...
            if ( Voice * voice{ GetSourceVoice( Source, Context ) } ) {
                auto vpos =
                    GetSampleOffset( Source->mQueue, prop, values[ 0 ] );
                if ( !vpos )
                    return Context->setError( AL_INVALID_VALUE,
                                              "Invalid offset" );

                if ( SetVoiceOffset( voice, *vpos, Source, Context,
                                     Context->mALDevice.get() ) )
                    return;
            }
        ...
        case AL_ORIENTATION:
            ...
            CheckValue(
                ...
                std::isfinite( values[ 4 ] ) && std::isfinite( values[ 5 ] ) );
    ...
    Context->setError( AL_INVALID_ENUM, "Invalid source float property 0x%04x",
                       prop );
...
void SetSourceiv( ALsource* const Source,
                  ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< const int > values ) try {
    auto Checkers = GetCheckers( Context, prop, values );
    auto& CheckSize = Checkers.first;
    auto& CheckValue = Checkers.second;
    ALCdevice* device{ Context->mALDevice.get() };
    ...
    switch ( prop ) {
        ...
        case AL_LOOPING:
            ...
            if ( Voice * voice{ GetSourceVoice( Source, Context ) } ) {
                if ( Source->Looping )
                    voice->mLoopBuffer.store( &Source->mQueue.front(),
                                              std::memory_order_release );
                else
                    voice->mLoopBuffer.store( nullptr,
                                              std::memory_order_release );
        ...
        case AL_BUFFER:
            CheckSize( 1 );
            {
                const ALenum state{ GetSourceState(
                    Source, GetSourceVoice( Source, Context ) ) };
                if ( state == AL_PLAYING || state == AL_PAUSED )
                    return Context->setError(
                        AL_INVALID_OPERATION,
                        "Setting buffer on playing or paused source %u",
                        Source->id );
            }
            if ( values[ 0 ] ) {
                std::lock_guard< std::mutex > _{ device->BufferLock };
                ALbuffer* buffer{ LookupBuffer(
                    device, static_cast< ALuint >( values[ 0 ] ) ) };
                if ( !buffer )
                    return Context->setError(
                        AL_INVALID_VALUE, "Invalid buffer ID %u",
                        static_cast< ALuint >( values[ 0 ] ) );
                if ( buffer->MappedAccess &&
                     !( buffer->MappedAccess & AL_MAP_PERSISTENT_BIT_SOFT ) )
                    return Context->setError(
                        AL_INVALID_OPERATION,
                        "Setting non-persistently mapped buffer %u",
                        buffer->id );
        ...
        case AL_DIRECT_FILTER:
            ...
            if ( values[ 0 ] ) {
                ...
                ALfilter* filter{ LookupFilter(
                    device, static_cast< ALuint >( values[ 0 ] ) ) };
                if ( !filter )
                    return Context->setError(
                        AL_INVALID_VALUE, "Invalid filter ID %u",
                        static_cast< ALuint >( values[ 0 ] ) );
        ...
        case AL_DISTANCE_MODEL:
            ...
            Context->setError( AL_INVALID_VALUE,
                               "Distance model out of range: 0x%04x",
                               values[ 0 ] );
            return;

        case AL_SOURCE_RESAMPLER_SOFT:
            CheckSize( 1 );
            CheckValue( values[ 0 ] >= 0 &&
                        values[ 0 ] <= static_cast< int >( Resampler::Max ) );
        ...
        case AL_STEREO_MODE_SOFT:
            CheckSize( 1 );
            {
                ...
                if ( state == AL_PLAYING || state == AL_PAUSED )
                    return Context->setError(
                        AL_INVALID_OPERATION,
                        "Modifying stereo mode on playing or paused source %u",
                        Source->id );
            }
        ...
        case AL_AUXILIARY_SEND_FILTER:
            CheckSize( 3 );
            slotlock =
                std::unique_lock< std::mutex >{ Context->mEffectSlotLock };
            if ( values[ 0 ] &&
                 ( slot = LookupEffectSlot(
                       Context, static_cast< ALuint >( values[ 0 ] ) ) ) ==
                     nullptr )
                return Context->setError( AL_INVALID_VALUE,
                                          "Invalid effect ID %u", values[ 0 ] );
            if ( static_cast< ALuint >( values[ 1 ] ) >= device->NumAuxSends )
                return Context->setError( AL_INVALID_VALUE, "Invalid send %u",
                                          values[ 1 ] );

            if ( values[ 2 ] ) {
                std::lock_guard< std::mutex > _{ device->FilterLock };
                ALfilter* filter{ LookupFilter(
                    device, static_cast< ALuint >( values[ 2 ] ) ) };
                if ( !filter )
                    return Context->setError(
                        AL_INVALID_VALUE, "Invalid filter ID %u", values[ 2 ] );

                auto& send =
                    Source->Send[ static_cast< ALuint >( values[ 1 ] ) ];
             ...
             * and about to be changed on an active source, in case the old slot
             * is about to be deleted.
             */
            if ( Source->Send[ static_cast< ALuint >( values[ 1 ] ) ].Slot &&
                 slot != Source->Send[ static_cast< ALuint >( values[ 1 ] ) ]
                             .Slot &&
                ...
                Source->Send[ static_cast< ALuint >( values[ 1 ] ) ].Slot =
                    slot;
...
void SetSourcei64v( ALsource* const Source,
                    ALCcontext* const Context,
                    const SourceProp prop,
                    const al::span< const int64_t > values ) try {
    auto Checkers = GetCheckers( Context, prop, values );
    auto& CheckSize = Checkers.first;
    auto& CheckValue = Checkers.second;
...
template < typename T, size_t N >
auto GetSizeChecker( ALCcontext* const Context,
                     const SourceProp prop,
                     const al::span< T, N > values ) {
...
bool GetSourcedv( ALsource* const Source,
                  ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< double > values );
bool GetSourceiv( ALsource* const Source,
                  ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< int > values );
bool GetSourcei64v( ALsource* const Source,
                    ALCcontext* const Context,
                    const SourceProp prop,
...
bool GetSourcedv( ALsource* const Source,
                  ALCcontext* const Context,
                  const SourceProp prop,
                  const al::span< double > values ) try {
    ...
    switch ( prop ) {
        ...
        case AL_SEC_OFFSET_LATENCY_SOFT:
            ...
            if ( srcclock == clocktime.ClockTime )
                values[ 1 ] =
                    static_cast< double >( clocktime.Latency.count() ) /
                    1000000000.0;
            else {
                ...
                const nanoseconds latency{
                    clocktime.Latency - std::min( clocktime.Latency, diff ) };
                values[ 1 ] =
                    static_cast< double >( latency.count() ) / 1000000000.0;
            }
            return true;
        ...
        case AL_SEC_OFFSET_CLOCK_SOFT:
            ...
            values[ 0 ] = GetSourceSecOffset( Source, Context, &srcclock );
            values[ 1 ] =
                static_cast< double >( srcclock.count() ) / 1000000000.0;
            return true;
        ...
        case AL_STEREO_MODE_SOFT:
            CheckSize( 1 );
            if ( ( err = GetSourceiv( Source, Context, prop,
                                      { ivals, 1u } ) ) != false )
    ...
    ERR( "Unexpected property: 0x%04x\n", prop );
    Context->setError( AL_INVALID_ENUM, "Invalid source double property 0x%04x",
                       prop );
    return false;
} catch ( check_exception& ) {
    return false;
...
bool GetSourceiv( ALsource* const Source,
    ...
    switch ( prop ) {
        ...
        case AL_BUFFER:
            ...
            {
                ALbufferQueueItem* BufferList{};
                /* HACK: This query should technically only return the buffer
                 * set on a static source. However, some apps had used it to
                 * detect when a streaming source changed buffers, so report the
                 * current buffer's ID when playing.
                 */
                if ( Source->SourceType == AL_STATIC ||
                    ...
                    if ( !Source->mQueue.empty() )
                        BufferList = &Source->mQueue.front();
                            ...
                            voice{ GetSourceVoice( Source, Context ) } ) {
                    VoiceBufferItem* Current{ voice->mCurrentBuffer.load(
                        std::memory_order_relaxed ) };
        ...
        case AL_SOURCE_STATE:
            CheckSize( 1 );
            values[ 0 ] =
                GetSourceState( Source, GetSourceVoice( Source, Context ) );
            return true;
        ...
        case AL_SEC_LENGTH_SOFT:
            CheckSize( 1 );
            values[ 0 ] =
                static_cast< int >( mind( GetSourceLength( Source, prop ),
        ...
        case AL_SOURCE_RADIUS: /*AL_BYTE_RW_OFFSETS_SOFT:*/
            if ( sBufferSubDataCompat ) {
                CheckSize( 2 );
                const auto offset =
                    GetSourceOffset( Source, AL_BYTE_OFFSET, Context );
                /* FIXME: values[1] should be ahead of values[0] by the device
                 * update time. It needs to clamp or wrap the length of the
                 * buffer queue.
                 */
        ...
        case AL_SUPER_STEREO_WIDTH_SOFT:
            CheckSize( 1 );
            if ( ( err = GetSourcedv( Source, Context, prop,
                                      { dvals, 1u } ) ) != false )
...
bool GetSourcei64v( ALsource* const Source,
                    ALCcontext* const Context,
                    const SourceProp prop,
                    const al::span< int64_t > values ) try {
    auto CheckSize = GetSizeChecker( Context, prop, values );
    ALCdevice* device{ Context->mALDevice.get() };
    ClockLatency clocktime;
    nanoseconds srcclock;
    ...
    switch ( prop ) {
        ...
        case AL_SEC_LENGTH_SOFT:
            CheckSize( 1 );
            values[ 0 ] =
                static_cast< int64_t >( GetSourceLength( Source, prop ) );
            return true;
        ...
        case AL_SAMPLE_OFFSET_LATENCY_SOFT:
            ...
            if ( srcclock == clocktime.ClockTime )
                 ...
                 */
                const nanoseconds diff{ clocktime.ClockTime - srcclock };
                values[ 1 ] = nanoseconds{ clocktime.Latency -
                                           std::min( clocktime.Latency, diff ) }
                                  .count();
            }
            return true;
...
void StartSources( ALCcontext* const context,
                   const al::span< ALsource* > srchandles,
    ...
    if ( !device->Connected.load( std::memory_order_acquire ) )
        UNLIKELY {
            if ( context->mStopVoicesOnDisconnect.load(
                     std::memory_order_acquire ) ) {
    ...
    for ( const Voice* voice : voicelist ) {
        free_voices +=
            ( voice->mPlayState.load( std::memory_order_acquire ) ==
                  Voice::Stopped &&
              voice->mSourceID.load( std::memory_order_relaxed ) == 0u &&
              voice->mPendingChange.load( std::memory_order_relaxed ) ==
                  false );
    ...
    if ( srchandles.size() != free_voices )
        UNLIKELY {
            ...
            if ( inc_amount > allvoices.size() - voicelist.size() ) {
                ...
                context->allocVoices( inc_amount -
                                      ( allvoices.size() - voicelist.size() ) );
            }
            context->mActiveVoiceCount.fetch_add( inc_amount,
                                                  std::memory_order_release );
            voicelist = context->getVoicesSpan();
        }
    ...
    for ( ALsource* source : srchandles ) {
        ...
        auto BufferList = std::find_if( source->mQueue.begin(),
                                        source->mQueue.end(), find_buffer );

        /* If there's nothing to play, go right to stopped. */
        if ( BufferList == source->mQueue.end() )
            UNLIKELY {
        ...
        if ( !cur )
            ...
            cur->mNext.store( GetVoiceChanger( context ),
                              std::memory_order_relaxed );
        ...
        switch ( GetSourceState( source, voice ) ) {
            case AL_PAUSED:
                /* A source that's paused simply resumes. If there's no voice,
                 * it was lost from a disconnect, so just start over with a new
                 * one.
                 */
            ...
            case AL_PLAYING:
                /* A source that's already playing is restarted from the
                 * beginning. Stop the current voice and start a new one so it
                 * properly cross- fades back to the beginning.
                 */
                if ( voice )
                    voice->mPendingChange.store( true,
                                                 std::memory_order_relaxed );
                cur->mOldVoice = voice;
                voice = nullptr;
                break;
        ...
        for ( ; voiceiter != voicelist.end(); ++voiceiter, ++vidx ) {
            Voice* v{ *voiceiter };
            if ( v->mPlayState.load( std::memory_order_acquire ) ==
                     Voice::Stopped &&
                 v->mSourceID.load( std::memory_order_relaxed ) == 0u &&
                 v->mPendingChange.load( std::memory_order_relaxed ) ==
                     false ) {
                voice = v;
                break;
            }
        ...
        voice->mCurrentBuffer.store( &source->mQueue.front(),
                                     std::memory_order_relaxed );
        ...
        if ( const ALenum offsettype{ source->OffsetType } ) {
            ...
            source->Offset = 0.0;
            if ( auto vpos =
                     GetSampleOffset( source->mQueue, offsettype, offset ) ) {
                voice->mPosition.store( vpos->pos, std::memory_order_relaxed );
                voice->mPositionFrac.store( vpos->frac,
                                            std::memory_order_relaxed );
                voice->mCurrentBuffer.store( vpos->bufferitem,
                                             std::memory_order_relaxed );
                if ( vpos->pos > 0 || ( vpos->pos == 0 && vpos->frac > 0 ) ||
                     vpos->bufferitem != &source->mQueue.front() )
        ...
        InitVoice( voice, source, al::to_address( BufferList ), context,
                   device );
...
AL_API void AL_APIENTRY alGenSources( ALsizei n,
    ...
    ALCdevice* device{ context->mALDevice.get() };
    if ( static_cast< ALuint >( n ) >
         device->SourcesMax - context->mNumSources ) {
    ...
    if ( !EnsureSources( context.get(), static_cast< ALuint >( n ) ) ) {
        context->setError( AL_OUT_OF_MEMORY, "Failed to allocate %d source%s",
                           n, ( n == 1 ) ? "" : "s" );
        return;
    }
...
AL_API void AL_APIENTRY alDeleteSources( ALsizei n, const ALuint* sources )
    ...
    if ( invsrc != sources_end )
        UNLIKELY
    return context->setError( AL_INVALID_NAME, "Invalid source ID %u",
                              *invsrc );
...
AL_API void AL_APIENTRY alSourcefv( ALuint source,
                                    ALenum param,
                                    const ALfloat* values ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
...
AL_API void AL_APIENTRY alSource3dSOFT( ALuint source,
                                        ...
                                        ALdouble value1,
                                        ALdouble value2,
                                        ALdouble value3 ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
    ...
    context->setError( AL_INVALID_NAME, "Invalid source ID %u", source );
        ...
        const float fvals[ 3 ]{ static_cast< float >( value1 ),
...
AL_API void AL_APIENTRY alSourceiv( ALuint source,
                                    ALenum param,
                                    const ALint* values ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
...
AL_API void AL_APIENTRY alSourcei64SOFT( ALuint source,
                                         ALenum param,
                                         ALint64SOFT value ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
...
AL_API void AL_APIENTRY alSource3i64SOFT( ALuint source,
                                          ...
                                          ALint64SOFT value1,
                                          ALint64SOFT value2,
                                          ALint64SOFT value3 ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
    ...
    context->setError( AL_INVALID_NAME, "Invalid source ID %u", source );
        ...
        SetSourcei64v( Source, context.get(),
...
AL_API void AL_APIENTRY alGetSourcef( ALuint source,
    ...
    context->setError( AL_INVALID_NAME, "Invalid source ID %u", source );
        ...
        UNLIKELY context->setError( AL_INVALID_VALUE, "NULL pointer" );
    else {
        double dval[ 1 ];
        if ( GetSourcedv( Source, context.get(),
                          static_cast< SourceProp >( param ), dval ) )
...
AL_API void AL_APIENTRY alGetSource3f( ALuint source,
    ...
    context->setError( AL_INVALID_NAME, "Invalid source ID %u", source );
        ...
        double dvals[ 3 ];
        if ( GetSourcedv( Source, context.get(),
                          static_cast< SourceProp >( param ), dvals ) ) {
...
AL_API void AL_APIENTRY alGetSourcefv( ALuint source,
    ...
    const ALuint count{ FloatValsByProp( param ) };
    double dvals[ MaxValues ];
    if ( GetSourcedv( Source, context.get(), static_cast< SourceProp >( param ),
                      { dvals, count } ) )
...
AL_API void AL_APIENTRY alGetSourcedvSOFT( ALuint source,
                                           ALenum param,
                                           ALdouble* values ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
...
AL_API void AL_APIENTRY alGetSource3i( ALuint source,
    ...
    context->setError( AL_INVALID_NAME, "Invalid source ID %u", source );
        ...
        int ivals[ 3 ];
        if ( GetSourceiv( Source, context.get(),
                          static_cast< SourceProp >( param ), ivals ) ) {
            *value1 = ivals[ 0 ];
            *value2 = ivals[ 1 ];
            *value3 = ivals[ 2 ];
        }
...
AL_API void AL_APIENTRY alGetSource3i64SOFT( ALuint source,
    ...
    context->setError( AL_INVALID_NAME, "Invalid source ID %u", source );
        ...
        int64_t i64vals[ 3 ];
        if ( GetSourcei64v( Source, context.get(),
                            static_cast< SourceProp >( param ), i64vals ) ) {
            *value1 = i64vals[ 0 ];
            *value2 = i64vals[ 1 ];
            *value3 = i64vals[ 2 ];
...
void AL_APIENTRY alSourcePlayAtTimeSOFT( ALuint source, ALint64SOFT start_time )
    ...
    if ( start_time < 0 )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE, "Invalid time point %" PRId64,
                              start_time );
...
AL_API void AL_APIENTRY alSourcePlayv( ALsizei n,
                                       const ALuint* sources ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
    ...
    if ( static_cast< ALuint >( n ) <= source_storage.size() )
        LIKELY
    ...
    for ( ALsource* source : srchandles ) {
        ...
        if ( GetSourceState( source, voice ) == AL_PLAYING ) {
            if ( !cur )
                ...
                cur->mNext.store( GetVoiceChanger( context.get() ),
                                  std::memory_order_relaxed );
                cur = cur->mNext.load( std::memory_order_relaxed );
            }
            cur->mVoice = voice;
...
AL_API void AL_APIENTRY alSourceQueueBuffers( ALuint src,
    ...
    for ( ALsizei i{ 0 }; i < nb; i++ ) {
        ...
        if ( buffers[ i ] &&
             ( buffer = LookupBuffer( device, buffers[ i ] ) ) == nullptr ) {
            context->setError( AL_INVALID_NAME, "Queueing invalid buffer ID %u",
                               buffers[ i ] );
            goto buffer_error;
        }
        if ( buffer ) {
            ...
            if ( buffer->mCallback ) {
                context->setError( AL_INVALID_OPERATION,
                                   "Queueing callback buffer %u", buffer->id );
                goto buffer_error;
            }
        ...
        if ( fmt_mismatch )
            UNLIKELY {
                context->setError(
                    ...
                    NameFromFormat( BufferFmt->mChannels ), buffer->mSampleRate,
                    NameFromFormat( buffer->mType ),
...
AL_API void AL_APIENTRY alSourceUnqueueBuffers( ALuint src,
    ...
    if ( source->SourceType != AL_STREAMING )
        UNLIKELY
    return context->setError(
        AL_INVALID_VALUE, "Unqueueing from a non-streaming source %u", src );
    if ( source->Looping )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE,
    ...
    if ( processed < static_cast< ALuint >( nb ) )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE,
                              "Unqueueing %d buffer%s (only %u processed)", nb,
                              ( nb == 1 ) ? "" : "s", processed );
...
alSourceQueueBufferLayersSOFT( ALuint, ALsizei, const ALuint* ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;
...
ALsource* ALsource::EaxLookupSource( ALCcontext& al_context,
...
void ALsource::eax_set_sends_defaults( EaxSends& sends,
                                       const EaxFxSlotIds& ids ) noexcept {
...
void ALsource::eax4_set_active_fx_slots_defaults(
    EAX40ACTIVEFXSLOTS& slots ) noexcept {
...
void ALsource::eax5_set_source_defaults(
    EAX50SOURCEPROPERTIES& props ) noexcept {
...
void ALsource::eax5_set_active_fx_slots_defaults(
    EAX50ACTIVEFXSLOTS& slots ) noexcept {
    slots = EAX50SOURCE_3DDEFAULTACTIVEFXSLOTID;
}

void ALsource::eax5_set_speaker_levels_defaults(
    EaxSpeakerLevels& speaker_levels ) noexcept {
    for ( auto i = size_t{}; i < eax_max_speakers; ++i ) {
        auto& speaker_level = speaker_levels[ i ];
        speaker_level.lSpeakerID =
            static_cast< long >( EAXSPEAKER_FRONT_LEFT + i );
...
void ALsource::eax1_translate( const Eax1Props& src, Eax5Props& dst ) noexcept {
    ...
    if ( src.fMix == EAX_REVERBMIX_USEDISTANCE ) {
        ...
        dst.source.ulFlags &= ~EAXSOURCEFLAGS_ROOMAUTO;
        dst.sends[ 0 ].lSend =
            clamp( static_cast< long >( gain_to_level_mb( src.fMix ) ),
...
float ALsource::eax_calculate_dst_occlusion_mb( long src_occlusion_mb,
                                                float path_ratio,
    ...
    const auto dst_occlustion_mb =
        static_cast< float >( src_occlusion_mb ) * ratio;
...
EaxAlLowPassParam ALsource::eax_create_direct_filter_param() const noexcept {
    auto gain_mb =
        ...
        ( static_cast< float >( mEax.source.lObstruction ) *
          mEax.source.flObstructionLFRatio ) +
    ...
    auto gain_hf_mb = static_cast< float >( mEax.source.lDirectHF ) +
                      static_cast< float >( mEax.source.lObstruction );
    ...
    for ( auto i = std::size_t{}; i < EAX_MAX_FXSLOTS; ++i ) {
        ...
        if ( has_source_occlusion ) {
            ...
            const auto is_environmental_fx =
                ( ( fx_slot_eax.ulFlags & EAXFXSLOTFLAGS_ENVIRONMENT ) != 0 );
            const auto is_primary = ( mEaxPrimaryFxSlotId.value_or( -1 ) ==
                                      fx_slot.eax_get_index() );
            const auto is_listener_environment =
                ( is_environmental_fx && is_primary );

            if ( is_listener_environment ) {
                gain_mb += eax_calculate_dst_occlusion_mb(
                    mEax.source.lOcclusion, mEax.source.flOcclusionDirectRatio,
                    mEax.source.flOcclusionLFRatio );

                gain_hf_mb += static_cast< float >( mEax.source.lOcclusion ) *
                              mEax.source.flOcclusionDirectRatio;
            }
        }
        ...
        if ( send.lOcclusion != 0 ) {
            gain_mb += eax_calculate_dst_occlusion_mb(
                send.lOcclusion, send.flOcclusionDirectRatio,
                send.flOcclusionLFRatio );

            gain_hf_mb += static_cast< float >( send.lOcclusion ) *
                          send.flOcclusionDirectRatio;
        }
    }

    const auto al_low_pass_param =
        EaxAlLowPassParam{ level_mb_to_gain( gain_mb ),
...
EaxAlLowPassParam ALsource::eax_create_room_filter_param(
    ...
    const auto is_environmental_fx =
        ( ( fx_slot_eax.ulFlags & EAXFXSLOTFLAGS_ENVIRONMENT ) != 0 );
    const auto is_primary =
        ( mEaxPrimaryFxSlotId.value_or( -1 ) == fx_slot.eax_get_index() );
    const auto is_listener_environment = ( is_environmental_fx && is_primary );

    const auto gain_mb =
        ( static_cast< float >( fx_slot_eax.lOcclusion ) *
          fx_slot_eax.flOcclusionLFRatio ) +
        static_cast< float >( ( is_environmental_fx ? mEax.source.lRoom : 0 ) +
                              send.lSend ) +
        ( is_listener_environment
              ? eax_calculate_dst_occlusion_mb(
                    mEax.source.lOcclusion, mEax.source.flOcclusionRoomRatio,
                    mEax.source.flOcclusionLFRatio )
              : 0.0f ) +
        eax_calculate_dst_occlusion_mb( send.lOcclusion,
                                        send.flOcclusionRoomRatio,
                                        send.flOcclusionLFRatio ) +
        ( is_listener_environment
              ? ( static_cast< float >( mEax.source.lExclusion ) *
                  mEax.source.flExclusionLFRatio )
              : 0.0f ) +
    ...
    const auto gain_hf_mb =
        static_cast< float >( fx_slot_eax.lOcclusion ) +
        static_cast< float >(
            ( is_environmental_fx ? mEax.source.lRoomHF : 0 ) + send.lSendHF ) +
        ( is_listener_environment
              ? ( ( static_cast< float >( mEax.source.lOcclusion ) *
                    mEax.source.flOcclusionRoomRatio ) )
              : 0.0f ) +
        ( static_cast< float >( send.lOcclusion ) *
          send.flOcclusionRoomRatio ) +
        ( is_listener_environment
...
void ALsource::eax_set_efx_outer_gain_hf() {
    OuterGainHF = clamp( level_mb_to_gain( static_cast< float >(
                             mEax.source.lOutsideVolumeHF ) ),
                         AL_MIN_CONE_OUTER_GAINHF, AL_MAX_CONE_OUTER_GAINHF );
}
...
void ALsource::eax_set_efx_dry_gain_hf_auto() {
    DryGainHFAuto =
        ( ( mEax.source.ulFlags & EAXSOURCEFLAGS_DIRECTHFAUTO ) != 0 );
}
...
void ALsource::eax_set_efx_wet_gain_hf_auto() {
    WetGainHFAuto =
        ( ( mEax.source.ulFlags & EAXSOURCEFLAGS_ROOMHFAUTO ) != 0 );
}
...
void ALsource::eax2_set( const EaxCall& call, Eax2Props& props ) {
    switch ( call.get_property_id() ) {
        ...
        case DSPROPERTY_EAX20BUFFER_ROOMROLLOFFFACTOR:
            eax_defer< Eax2SourceRoomRolloffFactorValidator >(
                call, props.flRoomRolloffFactor );
            break;

        case DSPROPERTY_EAX20BUFFER_OBSTRUCTION:
            eax_defer< Eax2SourceObstructionValidator >( call,
                                                         props.lObstruction );
            break;

        case DSPROPERTY_EAX20BUFFER_OBSTRUCTIONLFRATIO:
            eax_defer< Eax2SourceObstructionLfRatioValidator >(
                call, props.flObstructionLFRatio );
            break;
        ...
        case DSPROPERTY_EAX20BUFFER_OCCLUSIONLFRATIO:
            eax_defer< Eax2SourceOcclusionLfRatioValidator >(
                call, props.flOcclusionLFRatio );
            break;

        case DSPROPERTY_EAX20BUFFER_OCCLUSIONROOMRATIO:
            eax_defer< Eax2SourceOcclusionRoomRatioValidator >(
                call, props.flOcclusionRoomRatio );
            break;

        case DSPROPERTY_EAX20BUFFER_OUTSIDEVOLUMEHF:
            eax_defer< Eax2SourceOutsideVolumeHfValidator >(
                call, props.lOutsideVolumeHF );
            break;

        case DSPROPERTY_EAX20BUFFER_AIRABSORPTIONFACTOR:
            eax_defer< Eax2SourceAirAbsorptionFactorValidator >(
                call, props.flAirAbsorptionFactor );
            break;
...
void ALsource::eax3_set( const EaxCall& call, Eax3Props& props ) {
    switch ( call.get_property_id() ) {
        ...
        case EAXSOURCE_OBSTRUCTIONPARAMETERS:
            eax_defer_sub< Eax4ObstructionValidator, EAXOBSTRUCTIONPROPERTIES >(
                call, props.lObstruction );
            break;

        case EAXSOURCE_OCCLUSIONPARAMETERS:
            eax_defer_sub< Eax4OcclusionValidator, EAXOCCLUSIONPROPERTIES >(
                call, props.lOcclusion );
            break;

        case EAXSOURCE_EXCLUSIONPARAMETERS:
            eax_defer_sub< Eax4ExclusionValidator, EAXEXCLUSIONPROPERTIES >(
                call, props.lExclusion );
            break;
        ...
        case EAXSOURCE_OCCLUSIONDIRECTRATIO:
            eax_defer< Eax3SourceOcclusionDirectRatioValidator >(
                call, props.flOcclusionDirectRatio );
            break;
        ...
        case EAXSOURCE_EXCLUSIONLFRATIO:
            eax_defer< Eax3SourceExclusionLfRatioValidator >(
                call, props.flExclusionLFRatio );
            break;
        ...
        case EAXSOURCE_DOPPLERFACTOR:
            eax_defer< Eax3SourceDopplerFactorValidator >(
                call, props.flDopplerFactor );
            break;

        case EAXSOURCE_ROLLOFFFACTOR:
            eax_defer< Eax3SourceRolloffFactorValidator >(
                call, props.flRolloffFactor );
            break;
...
void ALsource::eax4_set( const EaxCall& call, Eax4Props& props ) {
    switch ( call.get_property_id() ) {
        ...
        case EAXSOURCE_ALLSENDPARAMETERS:
            eax4_defer_sends< Eax4AllSendValidator,
                              EAXSOURCEALLSENDPROPERTIES >( call, props.sends );
            break;

        case EAXSOURCE_OCCLUSIONSENDPARAMETERS:
            eax4_defer_sends< Eax4OcclusionSendValidator,
                              EAXSOURCEOCCLUSIONSENDPROPERTIES >( call,
                                                                  props.sends );
            break;

        case EAXSOURCE_EXCLUSIONSENDPARAMETERS:
            eax4_defer_sends< Eax4ExclusionSendValidator,
                              EAXSOURCEEXCLUSIONSENDPROPERTIES >( call,
                                                                  props.sends );
            break;

        case EAXSOURCE_ACTIVEFXSLOTID:
            eax4_defer_active_fx_slot_id(
                call, props.active_fx_slots.guidActiveFXSlots );
            break;
...
void ALsource::eax5_defer_all_2d( const EaxCall& call,
                                  EAX50SOURCEPROPERTIES& props ) {
    const auto& src_props =
        call.get_value< Exception, const EAXSOURCE2DPROPERTIES >();
...
void ALsource::eax5_defer_speaker_levels( const EaxCall& call,
                                          EaxSpeakerLevels& props ) {
    const auto values =
        call.get_values< const EAXSPEAKERLEVELPROPERTIES >( eax_max_speakers );
    std::for_each( values.cbegin(), values.cend(), Eax5SpeakerAllValidator{} );

    for ( const auto& value : values ) {
        const auto index =
            static_cast< size_t >( value.lSpeakerID - EAXSPEAKER_FRONT_LEFT );
...
void ALsource::eax5_set( const EaxCall& call, Eax5Props& props ) {
    switch ( call.get_property_id() ) {
        ...
        case EAXSOURCE_SENDPARAMETERS:
            eax5_defer_sends< Eax5SendValidator, EAXSOURCESENDPROPERTIES >(
                call, props.sends );
            break;

        case EAXSOURCE_ALLSENDPARAMETERS:
            eax5_defer_sends< Eax5AllSendValidator,
                              EAXSOURCEALLSENDPROPERTIES >( call, props.sends );
            break;

        case EAXSOURCE_OCCLUSIONSENDPARAMETERS:
            eax5_defer_sends< Eax5OcclusionSendValidator,
                              EAXSOURCEOCCLUSIONSENDPROPERTIES >( call,
                                                                  props.sends );
            break;

        case EAXSOURCE_EXCLUSIONSENDPARAMETERS:
            eax5_defer_sends< Eax5ExclusionSendValidator,
                              EAXSOURCEEXCLUSIONSENDPROPERTIES >( call,
                                                                  props.sends );
            break;

        case EAXSOURCE_ACTIVEFXSLOTID:
            eax5_defer_active_fx_slot_id(
                call, props.active_fx_slots.guidActiveFXSlots );
            break;

        case EAXSOURCE_MACROFXFACTOR:
            eax_defer< Eax5SourceMacroFXFactorValidator >(
                call, props.source.flMacroFXFactor );
            break;
...
void ALsource::eax_set( const EaxCall& call ) {
    ...
    switch ( eax_version ) {
        case 1:
            eax1_set( call, mEax1.d );
            break;
        case 2:
            eax2_set( call, mEax2.d );
            break;
        case 3:
            eax3_set( call, mEax3.d );
            break;
        case 4:
            eax4_set( call, mEax4.d );
            break;
        case 5:
            eax5_set( call, mEax5.d );
            break;
        default:
            eax_fail_unknown_property_id();
    }
...
void ALsource::eax_get_active_fx_slot_id( const EaxCall& call,
                                          const GUID* ids,
                                          size_t max_count ) {
    assert( ids != nullptr );
    assert( max_count == EAX40_MAX_ACTIVE_FXSLOTS ||
            max_count == EAX50_MAX_ACTIVE_FXSLOTS );
...
void ALsource::eax3_get_obstruction( const EaxCall& call,
                                     const Eax3Props& props ) {
    const auto& subprops = reinterpret_cast< const EAXOBSTRUCTIONPROPERTIES& >(
        props.lObstruction );
    call.set_value< Exception >( subprops );
}

void ALsource::eax3_get_occlusion( const EaxCall& call,
                                   const Eax3Props& props ) {
    const auto& subprops =
        reinterpret_cast< const EAXOCCLUSIONPROPERTIES& >( props.lOcclusion );
    call.set_value< Exception >( subprops );
}

void ALsource::eax3_get_exclusion( const EaxCall& call,
                                   const Eax3Props& props ) {
    const auto& subprops =
        reinterpret_cast< const EAXEXCLUSIONPROPERTIES& >( props.lExclusion );
    call.set_value< Exception >( subprops );
}
...
void ALsource::eax4_get( const EaxCall& call, const Eax4Props& props ) {
    switch ( call.get_property_id() ) {
        ...
        case EAXSOURCE_OCCLUSIONSENDPARAMETERS:
            eax_get_sends< EAXSOURCEOCCLUSIONSENDPROPERTIES >( call,
                                                               props.sends );
            break;

        case EAXSOURCE_EXCLUSIONSENDPARAMETERS:
            eax_get_sends< EAXSOURCEEXCLUSIONSENDPROPERTIES >( call,
                                                               props.sends );
            break;

        case EAXSOURCE_ACTIVEFXSLOTID:
            eax_get_active_fx_slot_id( call,
                                       props.active_fx_slots.guidActiveFXSlots,
...
void ALsource::eax5_get_all_2d( const EaxCall& call,
                                const EAX50SOURCEPROPERTIES& props ) {
...
void ALsource::eax5_get_speaker_levels( const EaxCall& call,
                                        const EaxSpeakerLevels& props ) {
    const auto subprops =
        call.get_values< EAXSPEAKERLEVELPROPERTIES >( eax_max_speakers );
    std::uninitialized_copy_n( props.cbegin(), subprops.size(),
                               subprops.begin() );
}
...
void ALsource::eax_get( const EaxCall& call ) {
    switch ( call.get_version() ) {
        case 1:
            eax1_get( call, mEax1.i );
            break;
        case 2:
            eax2_get( call, mEax2.i );
            break;
        case 3:
            eax3_get( call, mEax3.i );
            break;
        case 4:
            eax4_get( call, mEax4.i );
            break;
        case 5:
            eax5_get( call, mEax5.i );
            break;
        default:
...
void ALsource::eax_set_al_source_send( ALeffectslot* slot,
                                       size_t sendidx,
...
void ALsource::eax_commit_active_fx_slots() {
    ...
    for ( auto i = size_t{}; i < EAX_MAX_FXSLOTS; ++i ) {
        if ( !mEaxActiveFxSlots[ i ] )
            eax_set_al_source_send( nullptr, i,
                                    EaxAlLowPassParam{ 1.0f, 1.0f } );
    }
}
...
void ALsource::eaxCommit() {
    ...
    const auto is_primary_fx_slot_id_changed =
