...
ALenum EnumFromAmbiLayout( AmbiLayout layout ) {
    ...
    throw std::runtime_error{ "Invalid AmbiLayout: " +
...
ALenum EnumFromAmbiScaling( AmbiScaling scale ) {
    ...
    throw std::runtime_error{ "Invalid AmbiScaling: " +
                              std::to_string( int( scale ) ) };
}
...
ALenum EnumFromEaxStorage( EaxStorage storage ) {
    switch ( storage ) {
        ...
        case EaxStorage::Accessible:
            return AL_STORAGE_ACCESSIBLE;
        case EaxStorage::Hardware:
            return AL_STORAGE_HARDWARE;
    }
    throw std::runtime_error{ "Invalid EaxStorage: " +
                              std::to_string( int( storage ) ) };
}

bool eax_x_ram_check_availability( const ALCdevice& device,
...
constexpr ALbitfieldSOFT INVALID_STORAGE_MASK{
    ~unsigned( AL_MAP_READ_BIT_SOFT | AL_MAP_WRITE_BIT_SOFT |
               AL_MAP_PERSISTENT_BIT_SOFT | AL_PRESERVE_DATA_BIT_SOFT ) };
constexpr ALbitfieldSOFT MAP_READ_WRITE_FLAGS{ AL_MAP_READ_BIT_SOFT |
                                               AL_MAP_WRITE_BIT_SOFT };
constexpr ALbitfieldSOFT INVALID_MAP_FLAGS{
    ~unsigned( AL_MAP_READ_BIT_SOFT | AL_MAP_WRITE_BIT_SOFT |
               AL_MAP_PERSISTENT_BIT_SOFT ) };

bool EnsureBuffers( ALCdevice* device, size_t needed ) {
    size_t count{ std::accumulate(
        device->BufferList.cbegin(), device->BufferList.cend(), size_t{ 0 },
    ...
    while ( needed > count ) {
        if ( device->BufferList.size() >= 1 << 25 )
            UNLIKELY
        return false;
        ...
        sublist->Buffers = static_cast< ALbuffer* >(
            al_calloc( alignof( ALbuffer ), sizeof( ALbuffer ) * 64 ) );
        if ( !sublist->Buffers )
            UNLIKELY {
...
ALbuffer* AllocBuffer( ALCdevice* device ) {
    auto sublist =
        std::find_if( device->BufferList.begin(), device->BufferList.end(),
                      []( const BufferSubList& entry ) noexcept -> bool {
                          return entry.FreeMask != 0;
                      } );
    auto lidx = static_cast< ALuint >(
        std::distance( device->BufferList.begin(), sublist ) );
...
ALuint SanitizeAlignment( FmtType type, ALuint align ) {
    if ( align == 0 ) {
        if ( type == FmtIMA4 ) {
             ...
             * nVidia and Apple use 64+1 sample frames per block ->
             * block_size=36 bytes per channel Most PC sound software uses
             * 2040+1 sample frames per block -> block_size=1024 bytes per
             * channel
             */
            return 65;
        }
...
void LoadData( ALCcontext* context,
               ALbuffer* ALBuf,
               ALsizei freq,
               ALuint size,
               const FmtChannels DstChannels,
               const FmtType DstType,
               const al::byte* SrcData,
    ...
    const ALuint ambiorder{ IsBFormat( DstChannels )
    ...
    return context->setError(
        AL_OUT_OF_MEMORY,
        "Buffer size overflow, %d blocks x %d samples per block", blocks,
        align );
    if ( blocks > std::numeric_limits< size_t >::max() / BlockSize )
        UNLIKELY
    return context->setError(
        AL_OUT_OF_MEMORY,
        "Buffer size overflow, %d frames x %d bytes per frame", blocks,
        BlockSize );
    ...
    if ( ALBuf->eax_x_ram_mode == EaxStorage::Hardware ) {
        ...
        if ( !eax_x_ram_check_availability( device, *ALBuf, size ) )
            return context->setError(
                AL_OUT_OF_MEMORY, "Out of X-RAM memory (avail: %u, needed: %u)",
    ...
    if ( newsize != ALBuf->mDataStorage.size() ) {
        ...
        if ( ( access & AL_PRESERVE_DATA_BIT_SOFT ) ) {
            const size_t tocopy{
                minz( newdata.size(), ALBuf->mDataStorage.size() ) };
    ...
    ALBuf->mBlockAlign =
        ( DstType == FmtIMA4 || DstType == FmtMSADPCM ) ? align : 1;
...
/** Prepares the buffer to use the specified callback, using the specified
 * format. */
void PrepareCallback( ALCcontext* context,
                      ALbuffer* ALBuf,
                      ALsizei freq,
                      const FmtChannels DstChannels,
                      const FmtType DstType,
     ...
     */
    static constexpr size_t line_size{ DeviceBase::MixerLineSize * MaxPitch +
                                       MaxResamplerEdge };
...
void PrepareUserPtr( ALCcontext* context,
                     ALbuffer* ALBuf,
                     ALsizei freq,
                     const FmtChannels DstChannels,
                     const FmtType DstType,
    ...
    if ( ( reinterpret_cast< uintptr_t >( sdata ) & ( typealign - 1 ) ) != 0 )
        return context->setError(
            AL_INVALID_VALUE, "Pointer %p is misaligned for %s samples (%u)",
            static_cast< void* >( sdata ), NameFromFormat( DstType ),
            typealign );
...
al::optional< DecompResult > DecomposeUserFormat( ALenum format ) {
    ...
    for ( const auto& fmt : UserFmtList ) {
        if ( fmt.format == format )
            return al::make_optional< DecompResult >(
                { fmt.channels, fmt.type } );
    }
    return al::nullopt;
}
...
AL_API void AL_APIENTRY alGenBuffers( ALsizei n,
    ...
    if ( !EnsureBuffers( device, static_cast< ALuint >( n ) ) ) {
        context->setError( AL_OUT_OF_MEMORY, "Failed to allocate %d buffer%s",
                           n, ( n == 1 ) ? "" : "s" );
        return;
    }
...
AL_API void AL_APIENTRY alDeleteBuffers( ALsizei n, const ALuint* buffers )
    ...
    auto validate_buffer = [ device, &context ]( const ALuint bid ) -> bool {
        ...
        if ( !ALBuf )
            UNLIKELY {
                context->setError( AL_INVALID_NAME, "Invalid buffer ID %u",
                                   bid );
                return false;
            }
...
AL_API void AL_APIENTRY alBufferStorageSOFT( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
    else if ( size < 0 ) UNLIKELY context->setError(
        AL_INVALID_VALUE, "Negative storage size %d", size );
    else if ( freq < 1 ) UNLIKELY context->setError(
        AL_INVALID_VALUE, "Invalid sample rate %d", freq );
    else if ( ( flags & INVALID_STORAGE_MASK ) != 0 )
        UNLIKELY context->setError( AL_INVALID_VALUE,
                                    "Invalid storage flags 0x%x",
                                    flags & INVALID_STORAGE_MASK );
    else if ( ( flags & AL_MAP_PERSISTENT_BIT_SOFT ) &&
              !( flags & MAP_READ_WRITE_FLAGS ) )
        UNLIKELY context->setError( AL_INVALID_VALUE,
                                    "Declaring persistently mapped storage "
                                    "without read or write access" );
    else {
        ...
        context->setError( AL_INVALID_ENUM, "Invalid format 0x%04x", format );
            ...
            LoadData( context.get(), albuf, freq, static_cast< ALuint >( size ),
                      usrfmt->channels, usrfmt->type,
...
void AL_APIENTRY alBufferDataStatic( const ALuint buffer,
    ...
    if ( size < 0 )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE, "Negative storage size %d",
                              size );
    if ( freq < 1 )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE, "Invalid sample rate %d",
                              freq );

    auto usrfmt = DecomposeUserFormat( format );
    if ( !usrfmt )
        UNLIKELY
    return context->setError( AL_INVALID_ENUM, "Invalid format 0x%04x",
                              format );
...
AL_API void* AL_APIENTRY alMapBufferSOFT( ALuint buffer,
    ...
    if ( !albuf )
        UNLIKELY
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
    else if ( ( access & INVALID_MAP_FLAGS ) != 0 )
        UNLIKELY context->setError( AL_INVALID_VALUE, "Invalid map flags 0x%x",
                                    access & INVALID_MAP_FLAGS );
    else if ( !( access & MAP_READ_WRITE_FLAGS ) ) UNLIKELY context->setError(
        AL_INVALID_VALUE, "Mapping buffer %u without read or write access",
        buffer );
    else {
        ALbitfieldSOFT unavailable = ( albuf->Access ^ access ) & access;
        if ( ReadRef( albuf->ref ) != 0 &&
             !( access & AL_MAP_PERSISTENT_BIT_SOFT ) )
            UNLIKELY
        context->setError(
            AL_INVALID_OPERATION,
            "Mapping in-use buffer %u without persistent mapping", buffer );
        else if ( albuf->MappedAccess != 0 ) UNLIKELY context->setError(
            AL_INVALID_OPERATION, "Mapping already-mapped buffer %u", buffer );
        else if ( ( unavailable & AL_MAP_READ_BIT_SOFT ) )
            UNLIKELY context->setError(
                AL_INVALID_VALUE,
                "Mapping buffer %u for reading without read access", buffer );
        else if ( ( unavailable & AL_MAP_WRITE_BIT_SOFT ) )
            UNLIKELY context->setError(
                AL_INVALID_VALUE,
                "Mapping buffer %u for writing without write access", buffer );
        else if ( ( unavailable & AL_MAP_PERSISTENT_BIT_SOFT ) )
            UNLIKELY context->setError(
                AL_INVALID_VALUE,
                "Mapping buffer %u persistently without persistent access",
                buffer );
        else if ( offset < 0 || length <= 0 ||
                  static_cast< ALuint >( offset ) >= albuf->OriginalSize ||
                  static_cast< ALuint >( length ) >
                      albuf->OriginalSize - static_cast< ALuint >( offset ) )
            UNLIKELY context->setError(
                AL_INVALID_VALUE, "Mapping invalid range %d+%d for buffer %u",
...
AL_API void AL_APIENTRY alUnmapBufferSOFT( ALuint buffer ) START_API_FUNC {
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        AL_INVALID_OPERATION, "Unmapping unmapped buffer %u", buffer );
    else {
...
AL_API void AL_APIENTRY alFlushMappedBufferSOFT( ALuint buffer,
    ...
    if ( !albuf )
        UNLIKELY
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
    else if ( !( albuf->MappedAccess & AL_MAP_WRITE_BIT_SOFT ) )
        UNLIKELY context->setError(
            AL_INVALID_OPERATION,
...
AL_API void AL_APIENTRY alBufferSubDataSOFT( ALuint buffer,
    ...
    if ( align < 1 )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE, "Invalid unpack alignment %u",
                              unpack_align );
    ...
    return context->setError(
        ...
        align, albuf->mBlockAlign );
    if ( albuf->isBFormat() && albuf->UnpackAmbiOrder != albuf->mAmbiOrder )
        UNLIKELY
    return context->setError(
        AL_INVALID_VALUE, "Unpacking data with mismatched ambisonic order" );
    ...
    const ALuint byte_align{
        ...
        : ( albuf->mType == FmtMSADPCM )
            ? ( ( align - 2 ) / 2 + 7 ) * num_chans
    ...
    if ( ( static_cast< ALuint >( offset ) % byte_align ) != 0 )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE,
                              "Sub-range offset %d is not a multiple of frame "
                              "size %d (%d unpack alignment)",
                              offset, byte_align, align );
    if ( ( static_cast< ALuint >( length ) % byte_align ) != 0 )
        UNLIKELY
    return context->setError( AL_INVALID_VALUE,
                              "Sub-range length %d is not a multiple of frame "
                              "size %d (%d unpack alignment)",
                              length, byte_align, align );

    assert( al::to_underlying( usrfmt->type ) ==
            al::to_underlying( albuf->mType ) );
...
AL_API void AL_APIENTRY alBuffer3f( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        default:
            context->setError( AL_INVALID_ENUM,
                               "Invalid buffer 3-float property 0x%04x",
                               param );
    }
}
...
AL_API void AL_APIENTRY alBufferfv( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        default:
            context->setError( AL_INVALID_ENUM,
                               "Invalid buffer float-vector property 0x%04x",
                               param );
    }
}
...
AL_API void AL_APIENTRY alBufferi( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        case AL_UNPACK_BLOCK_ALIGNMENT_SOFT:
            ...
            else albuf->UnpackAlign = static_cast< ALuint >( value );
            break;
        ...
        case AL_PACK_BLOCK_ALIGNMENT_SOFT:
            ...
            else albuf->PackAlign = static_cast< ALuint >( value );
            break;
        ...
        case AL_AMBISONIC_LAYOUT_SOFT:
            ...
            else UNLIKELY context->setError(
                AL_INVALID_VALUE, "Invalid unpack ambisonic layout %04x",
                value );
            break;
        ...
        case AL_AMBISONIC_SCALING_SOFT:
            ...
            context->setError( AL_INVALID_OPERATION,
                ...
                AL_INVALID_VALUE, "Invalid unpack ambisonic scaling %04x",
                value );
            break;
        ...
        case AL_UNPACK_AMBISONIC_ORDER_SOFT:
            ...
            else albuf->UnpackAmbiOrder = static_cast< ALuint >( value );
            break;

        default:
            context->setError( AL_INVALID_ENUM,
                               "Invalid buffer integer property 0x%04x",
                               param );
    }
}
...
AL_API void AL_APIENTRY alBuffer3i( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        default:
            context->setError( AL_INVALID_ENUM,
                               "Invalid buffer 3-integer property 0x%04x",
                               param );
    }
}
...
AL_API void AL_APIENTRY alGetBufferf( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        case AL_SEC_LENGTH_SOFT:
            *value = ( albuf->mSampleRate < 1 )
                         ? 0.0f
                         : ( static_cast< float >( albuf->mSampleLen ) /
                             static_cast< float >( albuf->mSampleRate ) );
            break;
...
AL_API void AL_APIENTRY alGetBufferi( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        case AL_BITS:
            *value = ( albuf->mType == FmtIMA4 || albuf->mType == FmtMSADPCM )
                         ? 4
        ...
        case AL_SIZE:
            *value = albuf->mCallback
                         ? 0
                         : static_cast< ALint >( albuf->mData.size() );
            break;

        case AL_BYTE_LENGTH_SOFT:
            *value =
                static_cast< ALint >( albuf->mSampleLen / albuf->mBlockAlign *
...
AL_API void AL_APIENTRY alBufferCallbackSOFT( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        UNLIKELY context->setError( AL_INVALID_VALUE, "NULL callback" );
    else {
        auto usrfmt = DecomposeUserFormat( format );
        if ( !usrfmt )
            UNLIKELY
        context->setError( AL_INVALID_ENUM, "Invalid format 0x%04x", format );
        else PrepareCallback( context.get(), albuf, freq, usrfmt->channels,
...
AL_API void AL_APIENTRY alGetBufferPtrSOFT( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        default:
            context->setError( AL_INVALID_ENUM,
                               "Invalid buffer pointer property 0x%04x",
                               param );
    }
}
...
AL_API void AL_APIENTRY alGetBuffer3PtrSOFT( ALuint buffer,
    ...
    context->setError( AL_INVALID_NAME, "Invalid buffer ID %u", buffer );
        ...
        default:
            context->setError( AL_INVALID_ENUM,
                               "Invalid buffer 3-pointer property 0x%04x",
                               param );
    }
}
...
#ifdef ALSOFT_EAX
FORCE_ALIGN ALboolean AL_APIENTRY EAXSetBufferMode( ALsizei n,
...
#define EAX_PREFIX "[EAXSetBufferMode] "
    ...
    if ( !eax_g_is_enabled ) {
        context->setError( AL_INVALID_OPERATION, EAX_PREFIX "%s",
                           "EAX not enabled." );
        return ALC_FALSE;
    }
    ...
    if ( !storage ) {
        context->setError( AL_INVALID_ENUM,
                           EAX_PREFIX "Unsupported X-RAM mode 0x%x", value );
        return ALC_FALSE;
    }
    ...
    if ( n < 0 ) {
        context->setError( AL_INVALID_VALUE,
                           EAX_PREFIX "Buffer count %d out of range", n );
        return ALC_FALSE;
    }

    if ( !buffers ) {
        context->setError( AL_INVALID_VALUE, EAX_PREFIX "%s",
                           "Null AL buffers" );
        return ALC_FALSE;
    }
    ...
    for ( auto i = 0; i < n; ++i ) {
        ...
        if ( *storage == EaxStorage::Hardware &&
             ...
             */
            if ( std::numeric_limits< size_t >::max() - buffer->OriginalSize <
                 total_needed )
                UNLIKELY {
                    context->setError( AL_OUT_OF_MEMORY,
                                       EAX_PREFIX "Size overflow (%u + %zu)\n",
...
EAXGetBufferMode( ALuint buffer, ALint* pReserved ) START_API_FUNC {
#define EAX_PREFIX "[EAXGetBufferMode] "
    ...
    if ( !al_buffer ) {
        context->setError( AL_INVALID_NAME, EAX_PREFIX "Invalid buffer ID %u",
                           buffer );
        return AL_NONE;
    }
