...
void eax_validate_range( const char* value_name,
                         const TValue& value,
    ...
    const auto message = std::string{ value_name } +
                         " out of range (value: " + std::to_string( value ) +
...
private:
    ...
    static auto test( int )
        -> decltype( std::declval< typename U::EaxIsBitFieldStruct >(), yes{} );
...
inline bool eax_bit_fields_are_equal( const T& lhs, const T& rhs ) noexcept {
    ...
    return reinterpret_cast< const TValue& >( lhs ) ==
           reinterpret_cast< const TValue& >( rhs );
}
...
inline bool operator==( const T& lhs, const T& rhs ) noexcept {
    using Value = std::conditional_t<
        ...
        std::conditional_t<
            ...
            std::conditional_t< sizeof( T ) == 4, std::uint32_t, void > > >;
