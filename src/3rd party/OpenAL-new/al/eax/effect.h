...
constexpr ALenum EnumFromEaxEffectType( const EaxEffectProps& props ) {
    switch ( props.mType ) {
        case EaxEffectType::None:
            break;
        case EaxEffectType::Reverb:
            return AL_EFFECT_EAXREVERB;
        case EaxEffectType::Chorus:
            return AL_EFFECT_CHORUS;
        case EaxEffectType::Autowah:
            return AL_EFFECT_AUTOWAH;
        case EaxEffectType::Compressor:
            return AL_EFFECT_COMPRESSOR;
        case EaxEffectType::Distortion:
            return AL_EFFECT_DISTORTION;
        case EaxEffectType::Echo:
            return AL_EFFECT_ECHO;
        case EaxEffectType::Equalizer:
            return AL_EFFECT_EQUALIZER;
        case EaxEffectType::Flanger:
            return AL_EFFECT_FLANGER;
        case EaxEffectType::FrequencyShifter:
            return AL_EFFECT_FREQUENCY_SHIFTER;
        case EaxEffectType::Modulator:
            return AL_EFFECT_RING_MODULATOR;
        case EaxEffectType::PitchShifter:
            return AL_EFFECT_PITCH_SHIFTER;
        case EaxEffectType::VocalMorpher:
...
struct EaxReverbCommitter {
    ...
    template < typename TValidator,
               ...
               typename TProperty >
    static void defer( const EaxCall& call,
    ...
    template < typename TValidator, typename TProperty >
    static void defer3( const EaxCall& call,
                        EAXREVERBPROPERTIES& properties,
                        TProperty& property ) {
        const auto& value = call.get_value< Exception, const TProperty >();
        TValidator{}( value );
        if ( value == property )
    ...
    static void Get( const EaxCall& call,
    ...
    static void translate( const EAX_REVERBPROPERTIES& src,
                           EaxEffectProps& dst ) noexcept;
    static void translate( const EAX20LISTENERPROPERTIES& src,
                           EaxEffectProps& dst ) noexcept;
    static void translate( const EAXREVERBPROPERTIES& src,
                           EaxEffectProps& dst ) noexcept;
};
    ...
    void set_defaults( int eax_version, ALenum altype ) {
        switch ( eax_version ) {
            ...
            case 3:
                ...
                break;
            case 4:
                call_set_defaults( altype, state4_.d );
                break;
            case 5:
                call_set_defaults( altype, state5_.d );
                break;
        }
        changed_ = true;
    }
...
#define EAXCALL( T, Callable, ... )                                     \
    ...
    void set( const EaxCall& call ) {
        switch ( call.get_version() ) {
            case 1:
                call_set< EaxReverbCommitter >( call, state1_.d );
                break;
            case 2:
                call_set< EaxReverbCommitter >( call, state2_.d );
                break;
            case 3:
                call_set< EaxReverbCommitter >( call, state3_.d );
                break;
            case 4:
                call_set( call, state4_.d );
                break;
            case 5:
                call_set( call, state5_.d );
                break;
        }
        changed_ = true;
    }
    ...
    void get( const EaxCall& call ) {
        switch ( call.get_version() ) {
            case 1:
                call_get< EaxReverbCommitter >( call, state1_.d );
                break;
            case 2:
                call_get< EaxReverbCommitter >( call, state2_.d );
                break;
            case 3:
                call_get< EaxReverbCommitter >( call, state3_.d );
                break;
            case 4:
                call_get( call, state4_.d );
                break;
            case 5:
                call_get( call, state5_.d );
                break;
        }
    }
    ...
    bool call_commit( Args&&... args ) {
        return T{ props_, al_effect_props_ }.commit(
