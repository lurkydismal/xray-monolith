...
std::string EaxException::make_message( const char* context,
    ...
    const auto context_size =
        ( context ? std::string::traits_type::length( context ) : 0 );
    const auto has_contex = ( context_size > 0 );

    const auto message_size =
        ( message ? std::string::traits_type::length( message ) : 0 );
