...
EaxCall::EaxCall( EaxCallType type,
    ...
    : mCallType{ type },
      mVersion{ 0 },
      mPropertySetId{ EaxCallPropertySetId::none },
      mIsDeferred{ ( property_id & deferred_flag ) != 0 },
      mPropertyId{ property_id & ~deferred_flag },
      mPropertySourceId{ property_source_id },
      mPropertyBuffer{ property_buffer },
      mPropertyBufferSize{ property_size } {
    switch ( mCallType ) {
    ...
    switch ( mPropertyId ) {
        ...
        case EAXFXSLOT_OCCLUSIONLFRATIO:
            ...
            // If we don't clear our flag then "applyAllUpdates" in EAX context
            // won't be called.
            mIsDeferred = false;
            break;
    }
    ...
    if ( mPropertySetId == EaxCallPropertySetId::source &&
         mPropertySourceId == 0 )
...
EaxCall create_eax_call( EaxCallType type,
