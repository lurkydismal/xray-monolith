...
effect_exception::effect_exception( ALenum code, const char* msg, ... )
    : mErrorCode{ code } {
    std::va_list args;
    va_start( args, msg );
    setMessage( msg, args );
...
constexpr EffectPropsItem EffectPropsList[] = {
    ...
    { AL_EFFECT_DEDICATED_DIALOGUE, DedicatedEffectProps,
      DedicatedEffectVtable },
    { AL_EFFECT_DEDICATED_LOW_FREQUENCY_EFFECT, DedicatedEffectProps,
      DedicatedEffectVtable },
    { AL_EFFECT_CONVOLUTION_REVERB_SOFT, ConvolutionEffectProps,
      ConvolutionEffectVtable },
};
...
const EffectPropsItem* getEffectPropsItemByType( ALenum type ) {
    auto iter = std::find_if(
        std::begin( EffectPropsList ), std::end( EffectPropsList ),
        [ type ]( const EffectPropsItem& item ) noexcept -> bool {
            return item.Type == type;
        } );
    return ( iter != std::end( EffectPropsList ) ) ? al::to_address( iter )
                                                   : nullptr;
}
...
bool EnsureEffects( ALCdevice* device, size_t needed ) {
    size_t count{ std::accumulate(
        device->EffectList.cbegin(), device->EffectList.cend(), size_t{ 0 },
    ...
    while ( needed > count ) {
        if ( device->EffectList.size() >= 1 << 25 )
            UNLIKELY
        return false;
        ...
        sublist->Effects = static_cast< ALeffect* >(
            al_calloc( alignof( ALeffect ), sizeof( ALeffect ) * 64 ) );
        if ( !sublist->Effects )
            UNLIKELY {
...
ALeffect* AllocEffect( ALCdevice* device ) {
    auto sublist =
        std::find_if( device->EffectList.begin(), device->EffectList.end(),
                      []( const EffectSubList& entry ) noexcept -> bool {
                          return entry.FreeMask != 0;
                      } );
    auto lidx = static_cast< ALuint >(
        std::distance( device->EffectList.begin(), sublist ) );
...
AL_API void AL_APIENTRY alGenEffects( ALsizei n,
                                      ALuint* effects ) START_API_FUNC {
    ...
    if ( !EnsureEffects( device, static_cast< ALuint >( n ) ) ) {
        context->setError( AL_OUT_OF_MEMORY, "Failed to allocate %d effect%s",
                           n, ( n == 1 ) ? "" : "s" );
        return;
    }
...
AL_API void AL_APIENTRY alDeleteEffects( ALsizei n, const ALuint* effects )
    ...
    if ( inveffect != effects_end )
        UNLIKELY {
            context->setError( AL_INVALID_NAME, "Invalid effect ID %u",
                               *inveffect );
            return;
        }
    ...
    context->setError( AL_INVALID_NAME, "Invalid effect ID %u", effect );
        ...
        if ( !isOk ) {
            for ( const EffectList& effectitem : gEffectList ) {
                if ( value == effectitem.val &&
                     !DisabledEffects[ effectitem.type ] ) {
                    isOk = true;
                    break;
                }
            }
...
AL_API void AL_APIENTRY alGetEffecti( ALuint effect,
    ...
    if ( !aleffect )
        UNLIKELY
    context->setError( AL_INVALID_NAME, "Invalid effect ID %u", effect );
    else if ( param == AL_EFFECT_TYPE )* value = aleffect->type;
    else try {
        /* Call the appropriate handler */
...
void LoadReverbPreset( const char* name, ALeffect* effect ) {
    ...
    for ( const auto& reverbitem : reverblist ) {
        ...
        effect->Props.Reverb.DecayHFLimit =
            props->iDecayHFLimit ? AL_TRUE : AL_FALSE;
        return;
    }
