...
#define HANDLE_RESAMPLER( r ) \
    case r:                   \
        return ResamplerName< r >::Get()
    switch ( rtype ) {
 ...
 * alcFunctions list.
 */
AL_API const ALchar* AL_APIENTRY alsoft_get_version( void ) START_API_FUNC {
...
AL_API void AL_APIENTRY alEnable( ALenum capability ) START_API_FUNC {
    ...
    switch ( capability ) {
        ...
        case AL_STOP_SOURCES_ON_DISCONNECT_SOFT:
            context->setError( AL_INVALID_OPERATION,
                               "Re-enabling AL_STOP_SOURCES_ON_DISCONNECT_SOFT "
                               "not yet supported" );
            break;
...
AL_API ALboolean AL_APIENTRY alIsEnabled( ALenum capability ) START_API_FUNC {
    ...
    switch ( capability ) {
        ...
        default:
            context->setError( AL_INVALID_VALUE,
                               "Invalid is enabled property 0x%04x",
                               capability );
    }
...
AL_API ALdouble AL_APIENTRY alGetDouble( ALenum pname ) START_API_FUNC {
    ...
    switch ( pname ) {
        ...
        case AL_DISTANCE_MODEL:
            value = static_cast< ALdouble >(
                ALenumFromDistanceModel( context->mDistanceModel ) );
            break;
...
AL_API ALint64SOFT AL_APIENTRY alGetInteger64SOFT( ALenum pname )
    ...
    switch ( pname ) {
        ...
        case AL_GAIN_LIMIT_SOFT:
            value =
                static_cast< ALint64SOFT >( GainMixMax / context->mGainBoost );
            break;
...
AL_API void AL_APIENTRY alGetBooleanv( ALenum pname,
                                       ALboolean* values ) START_API_FUNC {
    if ( values ) {
        switch ( pname ) {
            case AL_DOPPLER_FACTOR:
            case AL_DOPPLER_VELOCITY:
    ...
    if ( !values )
        ...
        switch ( pname ) {
            default:
                context->setError( AL_INVALID_VALUE,
                                   "Invalid boolean-vector property 0x%04x",
                                   pname );
        }
}
...
AL_API void AL_APIENTRY alGetDoublev( ALenum pname,
    ...
    if ( !values )
        ...
        switch ( pname ) {
            default:
                context->setError( AL_INVALID_VALUE,
                                   "Invalid double-vector property 0x%04x",
                                   pname );
        }
}
...
AL_API void AL_APIENTRY alGetFloatv( ALenum pname,
    ...
    if ( !values )
        ...
        switch ( pname ) {
            default:
                context->setError( AL_INVALID_VALUE,
                                   "Invalid float-vector property 0x%04x",
                                   pname );
        }
}
...
AL_API void AL_APIENTRY alGetIntegerv( ALenum pname,
    ...
    if ( !values )
        ...
        switch ( pname ) {
            default:
                context->setError( AL_INVALID_VALUE,
                                   "Invalid integer-vector property 0x%04x",
                                   pname );
        }
}
...
AL_API void AL_APIENTRY alGetInteger64vSOFT( ALenum pname, ALint64SOFT* values )
    ...
    if ( !values )
        ...
        switch ( pname ) {
            default:
                context->setError( AL_INVALID_VALUE,
                                   "Invalid integer64-vector property 0x%04x",
                                   pname );
        }
}
...
AL_API void AL_APIENTRY alGetPointervSOFT( ALenum pname,
    ...
    if ( !values )
        ...
        switch ( pname ) {
            default:
                context->setError( AL_INVALID_VALUE,
                                   "Invalid pointer-vector property 0x%04x",
                                   pname );
        }
}
...
AL_API void AL_APIENTRY alDopplerFactor( ALfloat value ) START_API_FUNC {
    ...
    if ( !( value >= 0.0f && std::isfinite( value ) ) )
        context->setError( AL_INVALID_VALUE, "Doppler factor %f out of range",
                           value );
    else {
        std::lock_guard< std::mutex > _{ context->mPropLock };
...
AL_API void AL_APIENTRY alDopplerVelocity( ALfloat value ) START_API_FUNC {
    ...
    if ( !( value >= 0.0f && std::isfinite( value ) ) )
        context->setError( AL_INVALID_VALUE, "Doppler velocity %f out of range",
                           value );
    else {
        std::lock_guard< std::mutex > _{ context->mPropLock };
...
AL_API void AL_APIENTRY alSpeedOfSound( ALfloat value ) START_API_FUNC {
    ...
    if ( !( value > 0.0f && std::isfinite( value ) ) )
        context->setError( AL_INVALID_VALUE, "Speed of sound %f out of range",
                           value );
    else {
        std::lock_guard< std::mutex > _{ context->mPropLock };
...
void UpdateContextProps( ALCcontext* context ) {
    ...
    ContextProps* props{
        context->mFreeContextProps.load( std::memory_order_acquire ) };
    if ( !props )
    ...
    props = context->mParams.ContextUpdate.exchange(
        props, std::memory_order_acq_rel );
    if ( props ) {
