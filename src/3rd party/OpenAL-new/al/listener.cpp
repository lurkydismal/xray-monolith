...
AL_API void AL_APIENTRY alListenerf( ALenum param,
    ...
    switch ( param ) {
        ...
        case AL_METERS_PER_UNIT:
            if ( !( value >= AL_MIN_METERS_PER_UNIT &&
                    value <= AL_MAX_METERS_PER_UNIT ) )
                return context->setError(
                    AL_INVALID_VALUE, "Listener meters per unit out of range" );
    ...
    switch ( param ) {
        case AL_ORIENTATION:
            if ( !( std::isfinite( values[ 0 ] ) &&
                    ...
                    std::isfinite( values[ 4 ] ) &&
                    std::isfinite( values[ 5 ] ) ) )
                return context->setError( AL_INVALID_VALUE,
...
AL_API void AL_APIENTRY alListener3i( ALenum param,
    ...
    switch ( param ) {
        case AL_POSITION:
        case AL_VELOCITY:
            alListener3f( param, static_cast< ALfloat >( value1 ),
...
AL_API void AL_APIENTRY alListeneriv( ALenum param,
    ...
    if ( values ) {
        ...
        switch ( param ) {
            case AL_POSITION:
            case AL_VELOCITY:
                alListener3f( param, static_cast< ALfloat >( values[ 0 ] ),
