    ...
    while ( !quitnow ) {
        ...
        do {
            ...
            if ( evt.EnumType == AsyncEvent::SourceStateChange ) {
                ALuint state{};
                std::string msg{ "Source ID " +
                                 std::to_string( evt.u.srcstate.id ) };
                ...
                context->mEventCb( AL_EVENT_TYPE_SOURCE_STATE_CHANGED_SOFT,
                                   evt.u.srcstate.id, state,
                                   static_cast< ALsizei >( msg.length() ),
                ...
                if ( evt.u.bufcomp.count == 1 )
                    msg += " buffer completed";
                else
                    msg += " buffers completed";
                context->mEventCb( AL_EVENT_TYPE_BUFFER_COMPLETED_SOFT,
                                   evt.u.bufcomp.id, evt.u.bufcomp.count,
                                   static_cast< ALsizei >( msg.length() ),
                                   msg.c_str(), context->mEventParam );
            } else if ( evt.EnumType == AsyncEvent::Disconnected ) {
                context->mEventCb(
                    AL_EVENT_TYPE_DISCONNECTED_SOFT, 0, 0,
                    static_cast< ALsizei >( strlen( evt.u.disconnect.msg ) ),
                    evt.u.disconnect.msg, context->mEventParam );
            }
...
void StopEventThrd( ALCcontext* ctx ) {
    ...
    al::construct_at( reinterpret_cast< AsyncEvent* >( evt_data.buf ),
...
AL_API void AL_APIENTRY alEventControlSOFT( ALsizei count,
                                            const ALenum* types,
                                            ALboolean enable ) START_API_FUNC {
    ContextRef context{ GetContextRef() };
    if ( !context )
        UNLIKELY return;

    if ( count < 0 )
        context->setError( AL_INVALID_VALUE, "Controlling %d events", count );
    if ( count <= 0 )
        return;
    if ( !types )
    ...
    auto bad_type = std::find_if_not(
        types, types_end, [ &flags ]( ALenum type ) noexcept -> bool {
    ...
    if ( bad_type != types_end )
        return context->setError( AL_INVALID_ENUM, "Invalid event type 0x%04x",
                                  *bad_type );

    if ( enable ) {
        auto enabledevts =
            context->mEnabledEvts.load( std::memory_order_relaxed );
        while ( context->mEnabledEvts.compare_exchange_weak(
                    enabledevts, enabledevts | flags, std::memory_order_acq_rel,
