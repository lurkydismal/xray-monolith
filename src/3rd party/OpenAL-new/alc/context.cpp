...
ALCcontext::~ALCcontext() {
    ...
    size_t count{ std::accumulate(
        mSourceList.cbegin(), mSourceList.cend(), size_t{ 0u },
        []( size_t cur, const SourceSubList& sublist ) noexcept -> size_t {
            return cur +
                   static_cast< uint >( al::popcount( ~sublist.FreeMask ) );
        } ) };
    if ( count > 0 )
    ...
    count = std::accumulate(
        mEffectSlotList.cbegin(), mEffectSlotList.cend(), size_t{ 0u },
...
void ALCcontext::init() {
    if ( sDefaultEffect.type != AL_EFFECT_NULL &&
         mDevice->Type == DeviceType::Playback ) {
    ...
    allocVoiceChanges();
    {
        ...
        while ( VoiceChange *
                next{ cur->mNext.load( std::memory_order_relaxed ) } )
            cur = next;
...
bool ALCcontext::deinit() {
    ...
    if ( auto toremove = static_cast< size_t >(
             std::count( oldarray->begin(), oldarray->end(), this ) ) ) {
        using ContextArray = al::FlexArray< ContextBase* >;
        auto alloc_ctx_array = []( const size_t count ) -> ContextArray* {
            if ( count == 0 )
                return &DeviceBase::sEmptyContextArray;
...
ALenum ALCcontext::eax_eax_set( const GUID* property_set_id,
                                ALuint property_id,
                                ALuint property_source_id,
                                ALvoid* property_value,
                                ALuint property_value_size ) {
    const auto call = create_eax_call( EaxCallType::set, property_set_id,
                                       property_id, property_source_id,
                                       property_value, property_value_size );
...
ALenum ALCcontext::eax_eax_get( const GUID* property_set_id,
                                ALuint property_id,
                                ALuint property_source_id,
                                ALvoid* property_value,
                                ALuint property_value_size ) {
    const auto call = create_eax_call( EaxCallType::get, property_set_id,
                                       property_id, property_source_id,
                                       property_value, property_value_size );
...
void ALCcontext::eax_initialize_extensions() {
    ...
    const auto string_max_capacity =
        std::strlen( mExtensionList ) + 1 + std::strlen( eax1_ext_name ) + 1 +
        std::strlen( eax2_ext_name ) + 1 + std::strlen( eax3_ext_name ) + 1 +
        std::strlen( eax4_ext_name ) + 1 + std::strlen( eax5_ext_name ) + 1 +
...
#define EAX_PREFIX "[EAX_DETECT_SPEAKER_CONFIG]"
    ...
    switch ( mDevice->FmtChans ) {
        ...
        case DevFmtX714:
            return SPEAKERS_7;
        /* 3D7.1 is only compatible with 5.1. This could instead be HEADPHONES
         * to suggest full-sphere surround sound (like HRTF).
         */
        case DevFmtX3D71:
            return SPEAKERS_5;
        /* This could also be HEADPHONES, since headphones-based HRTF and Ambi3D
         * provide full-sphere surround sound. Depends if apps are more likely
         * to consider headphones or 7.1 for surround sound support.
         */
        case DevFmtAmbi3D:
            return SPEAKERS_7;
    }
...
void ALCcontext::eax_update_sources() {
    ...
    auto update_source = []( ALsource& source ) { source.eaxCommit(); };
...
void ALCcontext::eax4_defer_all( const EaxCall& call, Eax4State& state ) {
    const auto& src =
        call.get_value< ContextException, const EAX40CONTEXTPROPERTIES >();
...
void ALCcontext::eax5_defer_all( const EaxCall& call, Eax5State& state ) {
    const auto& src =
        call.get_value< ContextException, const EAX50CONTEXTPROPERTIES >();
    Eax4AllValidator{}( src );
    const auto& dst_i = state.i;
    auto& dst_d = state.d;
    dst_d = src;
...
void ALCcontext::eax5_defer( const EaxCall& call, Eax5State& state ) {
    switch ( call.get_property_id() ) {
        case EAXCONTEXT_ALLPARAMETERS:
            ...
            break;
        case EAXCONTEXT_PRIMARYFXSLOTID:
            eax_defer< Eax5PrimaryFxSlotIdValidator,
                       eax_primary_fx_slot_id_dirty_bit >(
        ...
        case EAXCONTEXT_HFREFERENCE:
            ...
            break;
        case EAXCONTEXT_MACROFXFACTOR:
            eax_defer< Eax5MacroFxFactorValidator,
...
void ALCcontext::eax_set( const EaxCall& call ) {
    ...
    switch ( version ) {
        case 4:
            eax4_defer( call, mEax4 );
            break;
        case 5:
            eax5_defer( call, mEax5 );
            break;
        default:
            eax_fail_unknown_version();
    }
...
void ALCcontext::eax4_context_commit( Eax4State& state,
...
void ALCcontext::eax5_context_commit( Eax5State& state,
                                      EaxDirtyFlags& dst_df ) {
    if ( mEaxDf == EaxDirtyFlags{} )
        return;
...
FORCE_ALIGN ALenum AL_APIENTRY EAXSet( const GUID* property_set_id,
                                       ALuint property_id,
                                       ALuint property_source_id,
                                       ALvoid* property_value,
    ...
    return context->eax_eax_set( property_set_id, property_id,
                                 property_source_id, property_value,
                                 property_value_size );
} catch ( ... ) {
...
FORCE_ALIGN ALenum AL_APIENTRY EAXGet( const GUID* property_set_id,
                                       ALuint property_id,
                                       ALuint property_source_id,
                                       ALvoid* property_value,
                                       ALuint property_value_size ) noexcept
    ...
    return context->eax_eax_get( property_set_id, property_id,
                                 property_source_id, property_value,
                                 property_value_size );
} catch ( ... ) {
    eax_log_exception( __func__ );
