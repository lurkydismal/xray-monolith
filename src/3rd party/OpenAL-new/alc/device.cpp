...
ALCdevice::~ALCdevice() {
    ...
    size_t count{ std::accumulate(
        BufferList.cbegin(), BufferList.cend(), size_t{ 0u },
    ...
    count = std::accumulate(
        EffectList.cbegin(), EffectList.cend(), size_t{ 0u },
    ...
    count = std::accumulate(
        FilterList.cbegin(), FilterList.cend(), size_t{ 0u },
...
void ALCdevice::enumerateHrtfs() {
    mHrtfList =
        EnumerateHrtf( configValue< std::string >( nullptr, "hrtf-paths" ) );
    if ( auto defhrtfopt =
             configValue< std::string >( nullptr, "default-hrtf" ) ) {
        auto iter =
            std::find( mHrtfList.begin(), mHrtfList.end(), *defhrtfopt );
...
auto ALCdevice::getOutputMode1() const noexcept -> OutputMode1 {
    ...
    switch ( FmtChans ) {
        ...
        case DevFmtQuad:
            return OutputMode1::Quad;
        case DevFmtX51:
            return OutputMode1::X51;
        case DevFmtX61:
            return OutputMode1::X61;
        case DevFmtX71:
            return OutputMode1::X71;
        case DevFmtX714:
        case DevFmtX3D71:
        case DevFmtAmbi3D:
            break;
