...
void ALCossListAppend( al::vector< DevMap >& list,
    ...
    if ( std::find_if( list.cbegin(), list.cend(), match_devname ) !=
         list.cend() )
        return;
...
void ALCossListPopulate( al::vector< DevMap >& devlist, int type_flag ) {
    ...
    for ( int i{ 0 }; i < si.numaudios; i++ ) {
        ...
        al::span< const char > devnode{
            ai.devnode, strnlen( ai.devnode, sizeof( ai.devnode ) ) };
    ...
    const char* defdev{
        ( ( type_flag == DSP_CAP_INPUT ) ? DefaultCapture : DefaultPlayback )
            .c_str() };
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        if ( pret < 0 ) {
            if ( errno == EINTR || errno == EAGAIN )
                continue;
            ERR( "poll failed: %s\n", strerror( errno ) );
            mDevice->handleDisconnect( "Failed waiting for playback buffer: %s",
                                       strerror( errno ) );
            break;
        } else if ( pret == 0 ) {
            WARN( "poll timeout\n" );
        ...
        mDevice->renderSamples( write_ptr,
                                static_cast< uint >( to_write / frame_size ),
                                frame_step );
        while ( to_write > 0 && !mKillNow.load( std::memory_order_acquire ) ) {
            ...
            if ( wrote < 0 ) {
                ...
                mDevice->handleDisconnect(
                    "Failed writing playback samples: %s", strerror( errno ) );
                break;
            }
...
bool OSSPlayback::reset() {
    ...
    uint log2FragmentSize{
        maxu( log2i( mDevice->UpdateSize * frameSize ), 4 ) };
...
#undef CHECKERR
    ...
    mDevice->BufferSize =
        static_cast< uint >( info.fragments ) * mDevice->UpdateSize;
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) ) {
        ...
        if ( sret < 0 ) {
            if ( errno == EINTR || errno == EAGAIN )
                continue;
            ERR( "poll failed: %s\n", strerror( errno ) );
            mDevice->handleDisconnect( "Failed to check capture samples: %s",
                                       strerror( errno ) );
            break;
        } else if ( sret == 0 ) {
            WARN( "poll timeout\n" );
        ...
        if ( vec.first.len > 0 ) {
            ssize_t amt{
                read( mFd, vec.first.buf, vec.first.len * frame_size ) };
            if ( amt < 0 ) {
                ERR( "read failed: %s\n", strerror( errno ) );
                mDevice->handleDisconnect( "Failed reading capture samples: %s",
                                           strerror( errno ) );
                break;
            }
...
void OSScapture::open( const char* name ) {
    ...
    uint log2FragmentSize{
        maxu( log2i( mDevice->BufferSize * frameSize / periods ), 4 ) };
    uint numFragmentsLogSize{ ( periods << 16 ) | log2FragmentSize };
...
#undef CHECKERR
    ...
    if ( mDevice->channelsFromFmt() != numChannels )
        throw al::backend_exception{
            ...
            "Failed to set %s, got %d channels instead",
            DevFmtChannelsString( mDevice->FmtChans ), numChannels };
    ...
    if ( !( ( ossFormat == AFMT_S8 && mDevice->FmtType == DevFmtByte ) ||
        ...
        throw al::backend_exception{
            al::backend_error::DeviceError,
            "Failed to set %s samples, got OSS format %#x",
            DevFmtTypeString( mDevice->FmtType ), ossFormat };
...
BackendPtr OSSBackendFactory::createBackend( DeviceBase* device,
                                             BackendType type ) {
