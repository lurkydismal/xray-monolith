...
using ReferenceTime =
    std::chrono::duration< REFERENCE_TIME, std::ratio< 1, 10000000 > >;

inline constexpr ReferenceTime operator"" _reftime(
    unsigned long long int n ) noexcept {
...
#define X6DOT1                                                          \
    ( SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT | SPEAKER_FRONT_CENTER | \
      SPEAKER_LOW_FREQUENCY | SPEAKER_BACK_CENTER | SPEAKER_SIDE_LEFT | \
      SPEAKER_SIDE_RIGHT )
#define X7DOT1                                                          \
    ( SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT | SPEAKER_FRONT_CENTER | \
      SPEAKER_LOW_FREQUENCY | SPEAKER_BACK_LEFT | SPEAKER_BACK_RIGHT |  \
...
#define X7DOT1DOT4                                                      \
    ( SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT | SPEAKER_FRONT_CENTER | \
      ...
      SPEAKER_TOP_FRONT_RIGHT | SPEAKER_TOP_BACK_LEFT |                 \
...
inline uint RefTime2Samples( const ReferenceTime& val, uint srate ) {
    const auto retval =
        ( val * srate + ReferenceTime{ seconds{ 1 } } / 2 ) / seconds{ 1 };
    return static_cast< uint >(
        mini64( retval, std::numeric_limits< uint >::max() ) );
}
...
struct DevMap {
    std::string name;
    std::string endpoint_guid; // obtained from PKEY_AudioEndpoint_GUID , set to
                               // "Unknown device GUID" if absent.
    std::wstring devid;
...
bool checkName( const al::vector< DevMap >& list, const std::string& name ) {
    ...
    return std::find_if( list.cbegin(), list.cend(), match_name ) !=
           list.cend();
}
...
NameGUIDPair get_device_name_and_guid( IMMDevice* device ) {
    ...
    PropVariant pvprop;
    hr = ps->GetValue(
        reinterpret_cast< const PROPERTYKEY& >( DEVPKEY_Device_FriendlyName ),
        pvprop.get() );
    if ( FAILED( hr ) ) {
    ...
    pvprop.clear();
    hr = ps->GetValue(
        reinterpret_cast< const PROPERTYKEY& >( PKEY_AudioEndpoint_GUID ),
        pvprop.get() );
    if ( FAILED( hr ) ) {
...
void add_device( IMMDevice* device,
                 const WCHAR* devid,
                 al::vector< DevMap >& list ) {
    for ( auto& entry : list ) {
    ...
    list.emplace_back( std::move( newname ), std::move( name_guid.second ),
                       devid );
...
void probe_devices( IMMDeviceEnumerator* devenum,
                    EDataFlow flowdir,
                    al::vector< DevMap >& list ) {
    al::vector< DevMap >{}.swap( list );

    ComPtr< IMMDeviceCollection > coll;
    HRESULT hr{ devenum->EnumAudioEndpoints( flowdir, DEVICE_STATE_ACTIVE,
                                             coll.getPtr() ) };
    if ( FAILED( hr ) ) {
    ...
    hr = devenum->GetDefaultAudioEndpoint( flowdir, eMultimedia,
                                           device.getPtr() );
    if ( SUCCEEDED( hr ) ) {
...
void TraceFormat( const char* msg, const WAVEFORMATEX* format ) {
    constexpr size_t fmtex_extra_size{ sizeof( WAVEFORMATEXTENSIBLE ) -
                                       sizeof( WAVEFORMATEX ) };
    if ( format->wFormatTag == WAVE_FORMAT_EXTENSIBLE &&
         format->cbSize >= fmtex_extra_size ) {
        ...
        TRACE(
            ...
            "    ChannelMask    = 0x%lx\n"
            "    SubFormat      = %s\n",
            msg, fmtex->Format.wFormatTag, fmtex->Format.nChannels,
            fmtex->Format.nSamplesPerSec, fmtex->Format.nAvgBytesPerSec,
            fmtex->Format.nBlockAlign, fmtex->Format.wBitsPerSample,
            fmtex->Format.cbSize, fmtex->Samples.wReserved,
            fmtex->dwChannelMask, GuidPrinter{ fmtex->SubFormat }.c_str() );
    } else
        TRACE(
            "%s:\n"
            "    FormatTag      = 0x%04x\n"
...
constexpr char MessageStr[ static_cast< size_t >( MsgType::Count ) ][ 20 ]{
    "Open Device",  "Reset Device",       "Start Device",     "Stop Device",
    "Close Device", "Enumerate Playback", "Enumerate Capture" };
...
struct WasapiProxy {
    ...
    std::future< HRESULT > pushMessage( MsgType type,
        ...
        std::future< HRESULT > future{ promise.get_future() };
        {
            ...
            mMsgQueue.emplace_back(
                Msg{ type, this, param, std::move( promise ) } );
        }
    ...
    static std::future< HRESULT > pushMessageStatic( MsgType type ) {
        ...
        std::future< HRESULT > future{ promise.get_future() };
        {
            std::lock_guard< std::mutex > _{ mMsgQueueLock };
            mMsgQueue.emplace_back(
                Msg{ type, nullptr, nullptr, std::move( promise ) } );
        }
        mMsgQueueCond.notify_one();
        return future;
    }
...
int WasapiProxy::messageHandler( std::promise< HRESULT >* promise ) {
    ...
    while ( Msg msg{ popMessage() } ) {
        TRACE( "Got message \"%s\" (0x%04x, this=%p, param=%p)\n",
               MessageStr[ static_cast< size_t >( msg.mType ) ],
               static_cast< uint >( msg.mType ),
               static_cast< void* >( msg.mProxy ),
               static_cast< const void* >( msg.mParam ) );
            ...
            case MsgType::EnumerateCapture: {
                ...
                if ( FAILED( hr ) )
                    ...
                    ComPtr< IMMDeviceEnumerator > devenum{
                        static_cast< IMMDeviceEnumerator* >( ptr ) };

                    if ( msg.mType == MsgType::EnumeratePlayback )
                        probe_devices( devenum.get(), eRender,
                                       PlaybackDevices );
                    else if ( msg.mType == MsgType::EnumerateCapture )
                        probe_devices( devenum.get(), eCapture,
                                       CaptureDevices );
                    msg.mPromise.set_value( S_OK );
                }
                continue;
...
FORCE_ALIGN int WasapiPlayback::mixerProc() {
    ...
    const uint frame_size{ mFormat.Format.nChannels *
                           mFormat.Format.wBitsPerSample / 8u };
        ...
        if ( SUCCEEDED( hr ) ) {
            if ( mResampler ) {
                ...
                for ( UINT32 done{ 0 }; done < len; ) {
                    if ( mBufferFilled == 0 ) {
                        mDevice->renderSamples( mResampleBuffer.get(),
                                                mDevice->UpdateSize,
                    ...
                    uint got{ mResampler->convert( &src, &srclen, buffer,
                                                   len - done ) };
                    ...
                    if ( srclen ) {
                        ...
                        std::copy( bsrc, bsrc + srclen * frame_size,
                                   mResampleBuffer.get() );
                    }
...
HRESULT WasapiPlayback::openProxy( const char* name ) {
    ...
    if ( name ) {
        auto iter = std::find_if(
            PlaybackDevices.cbegin(), PlaybackDevices.cend(),
        ...
        if ( iter == PlaybackDevices.cend() ) {
            ...
            iter =
                std::find_if( PlaybackDevices.cbegin(), PlaybackDevices.cend(),
    ...
    if ( name )
        ...
        mDevice->DeviceName =
            DevNameHead + get_device_name_and_guid( mMMDev.get() ).first;
...
bool WasapiPlayback::reset() {
    ...
    if ( FAILED( hr ) )
        throw al::backend_exception{ al::backend_error::DeviceError, "0x%08lx",
                                     hr };
    return true;
}
...
HRESULT WasapiPlayback::resetProxy() {
    ...
    void* ptr;
    HRESULT hr{ mMMDev->Activate( IID_IAudioClient, CLSCTX_INPROC_SERVER,
                                  nullptr, &ptr ) };
    if ( FAILED( hr ) ) {
    ...
    const ReferenceTime per_time{
        ReferenceTime{ seconds{ mDevice->UpdateSize } } / mDevice->Frequency };
    const ReferenceTime buf_time{
        ReferenceTime{ seconds{ mDevice->BufferSize } } / mDevice->Frequency };
    bool isRear51{ false };
    ...
    if ( !mDevice->Flags.test( ChannelsRequest ) ) {
        ...
        if ( chancount >= 12 && ( chanmask & X714Mask ) == X7DOT1DOT4 )
            ...
            mDevice->FmtChans = DevFmtQuad;
                  ...
                  ( ( chanmask & StereoMask ) == STEREO || !chanmask ) )
            mDevice->FmtChans = DevFmtStereo;
        else if ( chancount >= 1 &&
                  ( ( chanmask & MonoMask ) == MONO || !chanmask ) )
        ...
        const DWORD chanmask{ OutputType.dwChannelMask };
        isRear51 =
            ( chancount == 6 && ( chanmask & X51RearMask ) == X5DOT1REAR );
    }
    ...
    OutputType.Format.nBlockAlign = static_cast< WORD >(
    ...
    hr = mClient->IsFormatSupported( AUDCLNT_SHAREMODE_SHARED,
                                     &OutputType.Format, &wfx );
    if ( FAILED( hr ) ) {
    ...
    if ( wfx != nullptr ) {
        ...
        if ( !GetConfigValueBool( mDevice->DeviceName.c_str(), "wasapi",
                                  "allow-resampler", true ) )
            mDevice->Frequency = OutputType.Format.nSamplesPerSec;
        else
            mDevice->Frequency =
                minu( mDevice->Frequency, OutputType.Format.nSamplesPerSec );
        ...
        if ( mDevice->Flags.test( ChannelsRequest ) ) {
            ...
            switch ( mDevice->FmtChans ) {
                ...
                case DevFmtStereo:
                    chansok = ( chancount >= 2 &&
                                ( ( chanmask & StereoMask ) == STEREO ||
                                  !chanmask ) );
                    break;
                case DevFmtQuad:
                    chansok =
                        ( chancount >= 4 &&
                          ( ( chanmask & QuadMask ) == QUAD || !chanmask ) );
                    break;
                case DevFmtX51:
                    chansok = ( chancount >= 6 &&
                                ( ( chanmask & X51Mask ) == X5DOT1 ||
                ...
                case DevFmtX61:
                    chansok =
                        ( chancount >= 7 &&
                          ( ( chanmask & X61Mask ) == X6DOT1 || !chanmask ) );
                    break;
                case DevFmtX71:
                case DevFmtX3D71:
                    chansok =
                        ( chancount >= 8 &&
                          ( ( chanmask & X71Mask ) == X7DOT1 || !chanmask ) );
                    break;
                case DevFmtX714:
                    chansok = ( chancount >= 12 &&
                                ( ( chanmask & X714Mask ) == X7DOT1DOT4 ||
                                  !chanmask ) );
                case DevFmtAmbi3D:
                    break;
            }
    ...
    const EndpointFormFactor formfactor{
        get_device_formfactor( mMMDev.get() ) };
    mDevice->Flags.set( DirectEar,
                        ( formfactor == Headphones || formfactor == Headset ) );

    setDefaultWFXChannelOrder();

    hr = mClient->Initialize(
    ...
    hr = mClient->GetDevicePeriod(
        &reinterpret_cast< REFERENCE_TIME& >( min_per ), nullptr );
    if ( SUCCEEDED( hr ) )
    ...
    mOrigUpdateSize =
        minu( RefTime2Samples( min_per, mFormat.Format.nSamplesPerSec ),
              buffer_len / 2 );

    mDevice->BufferSize =
        static_cast< uint >( uint64_t{ buffer_len } * mDevice->Frequency /
    ...
    if ( mDevice->Frequency != mFormat.Format.nSamplesPerSec ) {
        mResampler = SampleConverter::Create(
            mDevice->FmtType, mDevice->FmtType, mFormat.Format.nChannels,
            mDevice->Frequency, mFormat.Format.nSamplesPerSec,
            Resampler::FastBSinc24 );
        mResampleBuffer = std::make_unique< char[] >(
            size_t{ mDevice->UpdateSize } * mFormat.Format.nChannels *
            mFormat.Format.wBitsPerSample / 8 );

        TRACE(
            "Created converter for %s/%s format, dst: %luhz (%u), src: %uhz "
            "(%u)\n",
            DevFmtChannelsString( mDevice->FmtChans ),
            DevFmtTypeString( mDevice->FmtType ), mFormat.Format.nSamplesPerSec,
            mOrigUpdateSize, mDevice->Frequency, mDevice->UpdateSize );
    }
...
HRESULT WasapiPlayback::startProxy() {
    ...
    if ( SUCCEEDED( hr ) ) {
        mRender = ComPtr< IAudioRenderClient >{
            static_cast< IAudioRenderClient* >( ptr ) };
        try {
            mKillNow.store( false, std::memory_order_release );
            mThread =
                std::thread{ std::mem_fn( &WasapiPlayback::mixerProc ), this };
        } catch ( ... ) {
            mRender = nullptr;
...
ClockLatency WasapiPlayback::getClockLatency() {
    ...
    if ( mResampler ) {
        ...
        ret.Latency += std::chrono::duration_cast< nanoseconds >( extra ) /
                       mDevice->Frequency;
        ret.Latency +=
            nanoseconds{ seconds{ mBufferFilled } } / mDevice->Frequency;
    }
...
FORCE_ALIGN int WasapiCapture::recordProc() {
    ...
    while ( !mKillNow.load( std::memory_order_relaxed ) ) {
        ...
        if ( FAILED( hr ) )
            ...
            hr = mCapture->GetBuffer( &rdata, &numsamples, &flags, nullptr,
                                      nullptr );
            if ( FAILED( hr ) )
                ...
                if ( mSampleConv ) {
                    ...
                    dstframes = mSampleConv->convert(
                        &srcdata, &srcframes, data.first.buf,
                        static_cast< uint >(
                            minz( data.first.len, INT_MAX ) ) );
                    if ( srcframes > 0 && dstframes == data.first.len &&
                         data.second.len > 0 ) {
                    ...
                    if ( len2 > 0 )
                        memcpy( data.second.buf, rdata + len1 * framesize,
                                len2 * framesize );
                    dstframes = len1 + len2;
                }
        ...
        if ( FAILED( hr ) ) {
            mDevice->handleDisconnect( "Failed to capture samples: 0x%08lx",
                                       hr );
            break;
        }
...
HRESULT WasapiCapture::openProxy( const char* name ) {
    ...
    if ( name ) {
        auto iter = std::find_if(
            CaptureDevices.cbegin(), CaptureDevices.cend(),
            [ name ]( const DevMap& entry ) -> bool {
                return entry.name == name || entry.endpoint_guid == name;
            } );
    ...
    if ( SUCCEEDED( hr ) ) {
        ComPtr< IMMDeviceEnumerator > enumerator{
            static_cast< IMMDeviceEnumerator* >( ptr ) };
        if ( !devid )
            hr = enumerator->GetDefaultAudioEndpoint( eCapture, eMultimedia,
                                                      mMMDev.getPtr() );
        else
...
HRESULT WasapiCapture::resetProxy() {
    ...
    const bool isRear51{ InputType.Format.nChannels == 6 &&
                         ( InputType.dwChannelMask & X51RearMask ) ==
                             X5DOT1REAR };

    // Make sure buffer is at least 100ms in size
    ReferenceTime buf_time{ ReferenceTime{ seconds{ mDevice->BufferSize } } /
                            mDevice->Frequency };
    ...
    InputType.Format.nBlockAlign = static_cast< WORD >(
        InputType.Format.nChannels * InputType.Format.wBitsPerSample / 8 );
    InputType.Format.nAvgBytesPerSec =
        InputType.Format.nSamplesPerSec * InputType.Format.nBlockAlign;
    InputType.Format.cbSize = sizeof( InputType ) - sizeof( InputType.Format );

    TraceFormat( "Requesting capture format", &InputType.Format );
    hr = mClient->IsFormatSupported( AUDCLNT_SHAREMODE_SHARED,
                                     &InputType.Format, &wfx );
    if ( FAILED( hr ) ) {
    ...
    if ( wfx != nullptr ) {
        ...
        auto validate_fmt = []( DeviceBase* device, uint32_t chancount,
                                DWORD chanmask ) noexcept -> bool {
            switch ( device->FmtChans ) {
                /* If the device wants mono, we can handle any input. */
                case DevFmtMono:
                    return true;
                /* If the device wants stereo, we can handle mono or stereo
                 * input. */
                case DevFmtStereo:
                    return ( chancount == 2 &&
                             ( chanmask == 0 ||
                               ( chanmask & StereoMask ) == STEREO ) ) ||
                           ( chancount == 1 &&
                             ( chanmask & MonoMask ) == MONO );
                /* Otherwise, the device must match the input type. */
                case DevFmtQuad:
                    return (
                        chancount == 4 &&
                        ( chanmask == 0 || ( chanmask & QuadMask ) == QUAD ) );
                ...
                case DevFmtX61:
                    return (
                        chancount == 7 &&
                        ( chanmask == 0 || ( chanmask & X61Mask ) == X6DOT1 ) );
                case DevFmtX71:
                case DevFmtX3D71:
                    return (
                        chancount == 8 &&
                        ( chanmask == 0 || ( chanmask & X71Mask ) == X7DOT1 ) );
                case DevFmtX714:
                    return ( chancount == 12 &&
                             ( chanmask == 0 ||
                               ( chanmask & X714Mask ) == X7DOT1DOT4 ) );
                case DevFmtAmbi3D:
                    return ( chanmask == 0 &&
                             chancount == device->channelsFromFmt() );
            }
            return false;
        };
        if ( !validate_fmt( mDevice, InputType.Format.nChannels,
                            InputType.dwChannelMask ) ) {
            ERR( "Failed to match format, wanted: %s %s %uhz, got: 0x%08lx "
                 "mask %d channel%s %d-bit %luhz\n",
                 DevFmtChannelsString( mDevice->FmtChans ),
                 DevFmtTypeString( mDevice->FmtType ), mDevice->Frequency,
                 InputType.dwChannelMask, InputType.Format.nChannels,
                 ( InputType.Format.nChannels == 1 ) ? "" : "s",
    ...
    } else if ( IsEqualGUID( InputType.SubFormat,
                             KSDATAFORMAT_SUBTYPE_IEEE_FLOAT ) ) {
    ...
    if ( mDevice->FmtChans == DevFmtMono && InputType.Format.nChannels != 1 ) {
        ...
        if ( ( InputType.dwChannelMask & SPEAKER_LOW_FREQUENCY ) ) {
            ...
            const int lfeidx{
                al::popcount( InputType.dwChannelMask & lfemask ) - 1 };
            chanmask &= ~( 1u << lfeidx );
        }

        mChannelConv = ChannelConverter{ srcType, InputType.Format.nChannels,
                                         chanmask, mDevice->FmtChans };
    ...
    if ( mDevice->Frequency != InputType.Format.nSamplesPerSec ||
         mDevice->FmtType != srcType ) {
        mSampleConv = SampleConverter::Create(
            srcType, mDevice->FmtType, mDevice->channelsFromFmt(),
            InputType.Format.nSamplesPerSec, mDevice->Frequency,
            Resampler::FastBSinc24 );
        if ( !mSampleConv ) {
            ERR( "Failed to create converter for %s format, dst: %s %uhz, src: "
                 "%s %luhz\n",
                 DevFmtChannelsString( mDevice->FmtChans ),
                 DevFmtTypeString( mDevice->FmtType ), mDevice->Frequency,
                 DevFmtTypeString( srcType ), InputType.Format.nSamplesPerSec );
            return E_FAIL;
        }
    ...
    mRing =
        RingBuffer::Create( buffer_len, mDevice->frameSizeFromFmt(), false );
...
HRESULT WasapiCapture::startProxy() {
    ...
    if ( SUCCEEDED( hr ) ) {
        mCapture = ComPtr< IAudioCaptureClient >{
            static_cast< IAudioCaptureClient* >( ptr ) };
        try {
            mKillNow.store( false, std::memory_order_release );
            mThread =
                std::thread{ std::mem_fn( &WasapiCapture::recordProc ), this };
        } catch ( ... ) {
            mCapture = nullptr;
            ERR( "Failed to start thread\n" );
            hr = E_FAIL;
    ...
    if ( FAILED( InitResult ) )
        try {
            auto res = std::async( std::launch::async, []() -> HRESULT {
                ...
                if ( FAILED( hr ) ) {
                    WARN(
                        "Failed to create IMMDeviceEnumerator instance: "
                        "0x%08lx\n",
                        hr );
                    CoUninitialize();
                    return hr;
    ...
    switch ( type ) {
        case BackendType::Playback:
            ...
            for ( const DevMap& entry : PlaybackDevices ) {
                 ...
                 */
                outnames.append( DevNameHead )
                    .append( entry.name.c_str(), entry.name.length() + 1 );
            }
            break;
...
BackendPtr WasapiBackendFactory::createBackend( DeviceBase* device,
                                                BackendType type ) {
