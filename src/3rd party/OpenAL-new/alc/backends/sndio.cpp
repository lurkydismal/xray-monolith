...
int SndioPlayback::mixerProc() {
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        mDevice->renderSamples(
            buffer.data(), static_cast< uint >( buffer.size() / frameSize ),
            frameStep );
        while ( !buffer.empty() &&
                !mKillNow.load( std::memory_order_acquire ) ) {
            size_t wrote{
                sio_write( mSndHandle, buffer.data(), buffer.size() ) };
...
retry_params:
    ...
    if ( !par.appbufsz )
        par.appbufsz = mDevice->UpdateSize;
    ...
    try {
        ...
        if ( par.bps > 1 && par.le != SIO_LE_NATIVE )
            throw al::backend_exception{ al::backend_error::DeviceError,
                                         "%s-endian samples not supported",
                                         par.le ? "Little" : "Big" };
        if ( par.bits < par.bps * 8 && !par.msb )
            throw al::backend_exception{
                al::backend_error::DeviceError,
                "MSB-padded samples not supported (%u of %u bits)", par.bits,
                par.bps * 8 };
        if ( par.pchan < 1 )
            throw al::backend_exception{ al::backend_error::DeviceError,
...
int SndioCapture::recordProc() {
    ...
    if ( nfds_pre <= 0 ) {
        mDevice->handleDisconnect( "Incorrect return value from sio_nfds(): %d",
                                   nfds_pre );
        return 1;
    }
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        al::span< al::byte > buffer{ data.first.buf,
        ...
        if ( buffer.empty() ) {
            ...
            sio_read( mSndHandle, junk,
                      sizeof( junk ) - ( sizeof( junk ) % frameSize ) );
        }
    }
...
void SndioCapture::open( const char* name ) {
    ...
    if ( !match_fmt( mDevice->FmtType, par ) ||
         mDevice->channelsFromFmt() != par.rchan ||
         mDevice->Frequency != par.rate )
        throw al::backend_exception{
            al::backend_error::DeviceError,
    ...
    mRing =
        RingBuffer::Create( mDevice->BufferSize, par.bps * par.rchan, false );
...
void SndioCapture::start() {
    ...
    try {
        ...
        sio_stop( mSndHandle );
        throw al::backend_exception{ al::backend_error::DeviceError,
                                     "Failed to start capture thread: %s",
                                     e.what() };
    }
}
...
BackendPtr SndIOBackendFactory::createBackend( DeviceBase* device,
                                               BackendType type ) {
