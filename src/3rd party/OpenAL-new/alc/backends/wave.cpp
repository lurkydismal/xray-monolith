...
constexpr ubyte SUBTYPE_PCM[]{ 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00,
                               0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71 };
constexpr ubyte SUBTYPE_FLOAT[]{ 0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x10, 0x00, 0x80, 0x00, 0x00, 0xaa,
                                 0x00, 0x38, 0x9b, 0x71 };

constexpr ubyte SUBTYPE_BFORMAT_PCM[]{ 0x01, 0x00, 0x00, 0x00, 0x21, 0x07,
                                       0xd3, 0x11, 0x86, 0x44, 0xc8, 0xc1,
                                       0xca, 0x00, 0x00, 0x00 };

constexpr ubyte SUBTYPE_BFORMAT_FLOAT[]{ 0x03, 0x00, 0x00, 0x00, 0x21, 0x07,
                                         0xd3, 0x11, 0x86, 0x44, 0xc8, 0xc1,
                                         0xca, 0x00, 0x00, 0x00 };

void fwrite16le( ushort val, FILE* f ) {
    ubyte data[ 2 ]{ static_cast< ubyte >( val & 0xff ),
                     static_cast< ubyte >( ( val >> 8 ) & 0xff ) };
    fwrite( data, 1, 2, f );
}

void fwrite32le( uint val, FILE* f ) {
    ubyte data[ 4 ]{ static_cast< ubyte >( val & 0xff ),
                     static_cast< ubyte >( ( val >> 8 ) & 0xff ),
                     static_cast< ubyte >( ( val >> 16 ) & 0xff ),
                     static_cast< ubyte >( ( val >> 24 ) & 0xff ) };
    fwrite( data, 1, 4, f );
}
...
int WaveBackend::mixerProc() {
    const milliseconds restTime{ mDevice->UpdateSize * 1000 /
                                 mDevice->Frequency / 2 };
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        int64_t avail{ std::chrono::duration_cast< seconds >(
                           ( now - start ) * mDevice->Frequency )
                           .count() };
        ...
        while ( avail - done >= mDevice->UpdateSize ) {
            mDevice->renderSamples( mBuffer.data(), mDevice->UpdateSize,
                                    frameStep );
            ...
            const size_t fs{ fwrite( mBuffer.data(), frameSize,
                                     mDevice->UpdateSize, mFile ) };
...
bool WaveBackend::reset() {
    ...
    switch ( mDevice->FmtChans ) {
        ...
        case DevFmtX71:
            chanmask =
                0x01 | 0x02 | 0x04 | 0x08 | 0x010 | 0x020 | 0x200 | 0x400;
            break;
        case DevFmtX714:
            chanmask = 0x01 | 0x02 | 0x04 | 0x08 | 0x010 | 0x020 | 0x200 |
                       0x400 | 0x1000 | 0x4000 | 0x8000 | 0x20000;
            break;
        /* NOTE: Same as 7.1. */
        case DevFmtX3D71:
            chanmask =
    ...
    val = fwrite( ( mDevice->FmtType == DevFmtFloat )
                      ...
                      : ( isbformat ? SUBTYPE_BFORMAT_PCM : SUBTYPE_PCM ),
                  1, 16, mFile );
    ( void )val;
    ...
    if ( mDataStart > 0 ) {
        ...
        if ( size > 0 ) {
            ...
            if ( fseek( mFile, 4, SEEK_SET ) == 0 )
                fwrite32le( static_cast< uint >( size - 8 ),
                            mFile ); // 'WAVE' header len
            if ( fseek( mFile, mDataStart - 4, SEEK_SET ) == 0 )
                fwrite32le( static_cast< uint >( dataLen ),
                            mFile ); // 'data' header len
        }
    }
}
...
BackendPtr WaveBackendFactory::createBackend( DeviceBase* device,
                                              BackendType type ) {
