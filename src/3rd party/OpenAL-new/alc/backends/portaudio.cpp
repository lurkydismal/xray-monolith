...
struct PortPlayback final : public BackendBase {
    ...
    static int writeCallbackC( const void* inputBuffer,
        ...
        return static_cast< PortPlayback* >( userData )
            ->writeCallback( inputBuffer, outputBuffer, framesPerBuffer,
...
int PortPlayback::writeCallback( const void*,
                                 void* outputBuffer,
                                 unsigned long framesPerBuffer,
                                 const PaStreamCallbackTimeInfo*,
                                 const PaStreamCallbackFlags ) noexcept {
    mDevice->renderSamples( outputBuffer,
                            static_cast< uint >( framesPerBuffer ),
...
void PortPlayback::open( const char* name ) {
    ...
    if ( devidopt && *devidopt >= 0 )
        params.device = *devidopt;
    else
        params.device = Pa_GetDefaultOutputDevice();
    params.suggestedLatency =
        mDevice->BufferSize / static_cast< double >( mDevice->Frequency );
...
retry_open:
    PaStream* stream{};
    PaError err{ Pa_OpenStream( &stream, nullptr, &params, mDevice->Frequency,
                                mDevice->UpdateSize, paNoFlag,
                                &PortPlayback::writeCallbackC, this ) };
    if ( err != paNoError ) {
...
struct PortCapture final : public BackendBase {
    ...
    static int readCallbackC( const void* inputBuffer,
                              ...
                              const PaStreamCallbackFlags statusFlags,
                              void* userData ) noexcept {
        return static_cast< PortCapture* >( userData )
            ->readCallback( inputBuffer, outputBuffer, framesPerBuffer,
                            timeInfo, statusFlags );
    }
...
int PortCapture::readCallback( const void* inputBuffer,
                               void*,
                               unsigned long framesPerBuffer,
                               const PaStreamCallbackTimeInfo*,
                               const PaStreamCallbackFlags ) noexcept {
...
void PortCapture::open( const char* name ) {
    ...
    PaError err{ Pa_OpenStream( &mStream, &mParams, nullptr, mDevice->Frequency,
                                ...
                                &PortCapture::readCallbackC, this ) };
    if ( err != paNoError )
        throw al::backend_exception{ al::backend_error::NoDevice,
                                     "Failed to open stream: %s",
                                     Pa_GetErrorText( err ) };
...
BackendPtr PortBackendFactory::createBackend( DeviceBase* device,
                                              BackendType type ) {
