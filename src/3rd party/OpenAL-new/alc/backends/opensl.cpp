...
constexpr SLuint32 GetChannelMask( DevFmtChannels chans ) noexcept {
    switch ( chans ) {
        ...
        case DevFmtX61:
            return SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT |
                   SL_SPEAKER_FRONT_CENTER | SL_SPEAKER_LOW_FREQUENCY |
                   SL_SPEAKER_BACK_CENTER | SL_SPEAKER_SIDE_LEFT |
        ...
        case DevFmtX714:
            return SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT |
                   ...
                   SL_SPEAKER_BACK_LEFT | SL_SPEAKER_BACK_RIGHT |
                   SL_SPEAKER_SIDE_LEFT | SL_SPEAKER_SIDE_RIGHT |
                   SL_SPEAKER_TOP_FRONT_LEFT | SL_SPEAKER_TOP_FRONT_RIGHT |
                   SL_SPEAKER_TOP_BACK_LEFT | SL_SPEAKER_TOP_BACK_RIGHT;
        case DevFmtAmbi3D:
            break;
    }
    return 0;
...
int OpenSLPlayback::mixerProc() {
    ...
    SLresult result{ VCALL( mBufferQueueObj, GetInterface )(
    ...
    if ( SL_RESULT_SUCCESS != result )
        mDevice->handleDisconnect( "Failed to get playback buffer: 0x%08x",
                                   result );

    while ( SL_RESULT_SUCCESS == result &&
            !mKillNow.load( std::memory_order_acquire ) &&
        ...
        if ( mRing->writeSpace() == 0 ) {
            ...
            if ( SL_RESULT_SUCCESS != result ) {
                mDevice->handleDisconnect( "Failed to start playback: 0x%08x",
                                           result );
                break;
            }
        ...
        mDevice->renderSamples(
            data.first.buf,
            static_cast< uint >( data.first.len ) * mDevice->UpdateSize,
            frame_step );
        if ( data.second.len > 0 )
            mDevice->renderSamples(
                data.second.buf,
                static_cast< uint >( data.second.len ) * mDevice->UpdateSize,
                frame_step );
        ...
        for ( size_t i{ 0 }; i < todo; i++ ) {
            ...
            result = VCALL( bufferQueue, Enqueue )(
                data.first.buf, mDevice->UpdateSize * mFrameSize );
            PrintErr( result, "bufferQueue->Enqueue" );
            if ( SL_RESULT_SUCCESS != result ) {
                mDevice->handleDisconnect( "Failed to queue audio: 0x%08x",
                                           result );
                break;
            }
...
void OpenSLPlayback::open( const char* name ) {
    ...
    if ( SL_RESULT_SUCCESS == result ) {
        result = VCALL( mEngine, CreateOutputMix )( &mOutputMix, 0, nullptr,
                                                    nullptr );
...
bool OpenSLPlayback::reset() {
    ...
    const std::array< SLInterfaceID, 2 > ids{
        { SL_IID_ANDROIDSIMPLEBUFFERQUEUE, SL_IID_ANDROIDCONFIGURATION } };
    const std::array< SLboolean, 2 > reqs{
    ...
    result = VCALL( mEngine, CreateAudioPlayer )( &mBufferQueueObj, &audioSrc,
                                                  &audioSnk, ids.size(),
                                                  ids.data(), reqs.data() );
    if ( SL_RESULT_SUCCESS != result )
#endif
    {
        ...
        result = VCALL( mEngine, CreateAudioPlayer )(
            &mBufferQueueObj, &audioSrc, &audioSnk, ids.size(), ids.data(),
            reqs.data() );
    ...
    if ( SL_RESULT_SUCCESS == result ) {
        ...
        result = VCALL( mBufferQueueObj, GetInterface )(
            SL_IID_ANDROIDCONFIGURATION, &config );
        PrintErr( result,
                  "bufferQueue->GetInterface SL_IID_ANDROIDCONFIGURATION" );
        if ( SL_RESULT_SUCCESS == result ) {
            SLint32 streamType = SL_ANDROID_STREAM_MEDIA;
            result = VCALL( config, SetConfiguration )(
                SL_ANDROID_KEY_STREAM_TYPE, &streamType, sizeof( streamType ) );
    ...
    if ( SL_RESULT_SUCCESS == result ) {
        ...
        mRing = RingBuffer::Create( num_updates,
                                    mFrameSize * mDevice->UpdateSize, true );
    }
...
void OpenSLPlayback::start() {
    ...
    if ( SL_RESULT_SUCCESS == result ) {
        result = VCALL( bufferQueue, RegisterCallback )(
            &OpenSLPlayback::processC, this );
        PrintErr( result, "bufferQueue->RegisterCallback" );
    }
    if ( SL_RESULT_SUCCESS != result )
        throw al::backend_exception{ al::backend_error::DeviceError,
                                     "Failed to register callback: 0x%08x",
                                     result };

    try {
        mKillNow.store( false, std::memory_order_release );
        mThread =
            std::thread( std::mem_fn( &OpenSLPlayback::mixerProc ), this );
...
void OpenSLPlayback::stop() {
    ...
    SLPlayItf player;
    SLresult result{
        VCALL( mBufferQueueObj, GetInterface )( SL_IID_PLAY, &player ) };
    PrintErr( result, "bufferQueue->GetInterface" );
    if ( SL_RESULT_SUCCESS == result ) {
...
ClockLatency OpenSLPlayback::getClockLatency() {
    ...
    ret.Latency =
        std::chrono::seconds{ mRing->readSpace() * mDevice->UpdateSize };
...
void OpenSLCapture::open( const char* name ) {
    ...
    if ( SL_RESULT_SUCCESS == result ) {
         ...
         */
        uint update_len{ clampu( mDevice->BufferSize / 3,
                                 mDevice->Frequency / 100,
                                 mDevice->Frequency / 100 * 5 ) };
        uint num_updates{ ( length + update_len - 1 ) / update_len };

        mRing =
            RingBuffer::Create( num_updates, update_len * mFrameSize, false );

        mDevice->UpdateSize = update_len;
        mDevice->BufferSize =
            static_cast< uint >( mRing->writeSpace() * update_len );
    }
    if ( SL_RESULT_SUCCESS == result ) {
        const std::array< SLInterfaceID, 2 > ids{
            { SL_IID_ANDROIDSIMPLEBUFFERQUEUE, SL_IID_ANDROIDCONFIGURATION } };
        ...
        format_pcm_ex.channelMask = GetChannelMask( mDevice->FmtChans );
        format_pcm_ex.endianness = GetByteOrderEndianness();
        format_pcm_ex.representation =
            GetTypeRepresentation( mDevice->FmtType );

        audioSnk.pLocator = &loc_bq;
        audioSnk.pFormat = &format_pcm_ex;
        result = VCALL( mEngine, CreateAudioRecorder )(
            &mRecordObj, &audioSrc, &audioSnk, ids.size(), ids.data(),
            reqs.data() );
        if ( SL_RESULT_SUCCESS != result )
#endif
        {
             ...
             * sample type.
             */
            if ( mDevice->FmtType == DevFmtUByte ||
                 mDevice->FmtType == DevFmtShort ||
                 mDevice->FmtType == DevFmtInt ) {
                SLDataFormat_PCM format_pcm{};
                format_pcm.formatType = SL_DATAFORMAT_PCM;
    ...
    SLAndroidSimpleBufferQueueItf bufferQueue;
    if ( SL_RESULT_SUCCESS == result ) {
        result = VCALL( mRecordObj, GetInterface )(
            SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &bufferQueue );
        PrintErr( result, "recordObj->GetInterface" );
    }
    if ( SL_RESULT_SUCCESS == result ) {
        result = VCALL( bufferQueue, RegisterCallback )(
            &OpenSLCapture::processC, this );
        PrintErr( result, "bufferQueue->RegisterCallback" );
    }
    if ( SL_RESULT_SUCCESS == result ) {
        const uint chunk_size{ mDevice->UpdateSize * mFrameSize };
        const auto silence = ( mDevice->FmtType == DevFmtUByte )
                                 ? al::byte{ 0x80 }
                                 : al::byte{ 0 };
        ...
        for ( size_t i{ 0u }; i < data.first.len && SL_RESULT_SUCCESS == result;
              i++ ) {
            result = VCALL( bufferQueue, Enqueue )(
                data.first.buf + chunk_size * i, chunk_size );
            PrintErr( result, "bufferQueue->Enqueue" );
        }
        for ( size_t i{ 0u };
              i < data.second.len && SL_RESULT_SUCCESS == result; i++ ) {
            result = VCALL( bufferQueue, Enqueue )(
                data.second.buf + chunk_size * i, chunk_size );
            PrintErr( result, "bufferQueue->Enqueue" );
        }
    }
...
void OpenSLCapture::start() {
    SLRecordItf record;
    SLresult result{
        VCALL( mRecordObj, GetInterface )( SL_IID_RECORD, &record ) };
    PrintErr( result, "recordObj->GetInterface" );
    ...
    if ( SL_RESULT_SUCCESS != result )
        throw al::backend_exception{ al::backend_error::DeviceError,
                                     "Failed to start capture: 0x%08x",
                                     result };
}
...
void OpenSLCapture::captureSamples( al::byte* buffer, uint samples ) {
    ...
    for ( uint i{ 0 }; i < samples; ) {
        ...
        std::copy_n( rdata.first.buf + mSplOffset * size_t{ mFrameSize },
                     rem * size_t{ mFrameSize },
    ...
    if ( mDevice->Connected.load( std::memory_order_acquire ) )
        LIKELY {
            const SLresult result{ VCALL( mRecordObj, GetInterface )(
                SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &bufferQueue ) };
            PrintErr( result, "recordObj->GetInterface" );
            if ( SL_RESULT_SUCCESS != result )
                UNLIKELY {
                    mDevice->handleDisconnect(
                        "Failed to get capture buffer queue: 0x%08x", result );
                    bufferQueue = nullptr;
                }
        }
    ...
    if ( adv_count > wdata.second.len )
        LIKELY {
            auto len1 =
                std::min( wdata.first.len, adv_count - wdata.second.len );
            auto buf1 =
                wdata.first.buf + chunk_size * ( wdata.first.len - len1 );
            for ( size_t i{ 0u }; i < len1 && SL_RESULT_SUCCESS == result;
                  i++ ) {
                result = VCALL( bufferQueue, Enqueue )( buf1 + chunk_size * i,
                                                        chunk_size );
                PrintErr( result, "bufferQueue->Enqueue" );
            }
        }
    if ( wdata.second.len > 0 ) {
        ...
        for ( size_t i{ 0u }; i < len2 && SL_RESULT_SUCCESS == result; i++ ) {
            result = VCALL( bufferQueue, Enqueue )( buf2 + chunk_size * i,
                                                    chunk_size );
            PrintErr( result, "bufferQueue->Enqueue" );
        }
    }
...
uint OpenSLCapture::availableSamples() {
    return static_cast< uint >( mRing->readSpace() * mDevice->UpdateSize -
                                mSplOffset );
}
...
BackendPtr OSLBackendFactory::createBackend( DeviceBase* device,
                                             BackendType type ) {
