...
#ifdef HAVE_DYNLOAD
void* ds_handle;
HRESULT( WINAPI* pDirectSoundCreate )( const GUID* pcGuidDevice,
                                       IDirectSound** ppDS,
                                       IUnknown* pUnkOuter );
HRESULT( WINAPI* pDirectSoundEnumerateW )( LPDSENUMCALLBACKW pDSEnumCallback,
                                           void* pContext );
HRESULT( WINAPI* pDirectSoundCaptureCreate )( const GUID* pcGuidDevice,
                                              IDirectSoundCapture** ppDSC,
                                              IUnknown* pUnkOuter );
HRESULT( WINAPI* pDirectSoundCaptureEnumerateW )(
...
BOOL CALLBACK DSoundEnumDevices( GUID* guid,
                                 const WCHAR* desc,
                                 const WCHAR*,
                                 void* data ) noexcept {
    if ( !guid )
...
FORCE_ALIGN int DSoundPlayback::mixerProc() {
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        DWORD avail = ( PlayCursor - LastCursor + DSBCaps.dwBufferBytes ) %
        ...
        err = mBuffer->Lock( LastCursor, avail, &WritePtr1, &WriteCnt1,
        ...
        if ( err == DSERR_BUFFERLOST ) {
            ...
            if ( SUCCEEDED( err ) ) {
                Playing = false;
                LastCursor = 0;
                err = mBuffer->Lock( 0, DSBCaps.dwBufferBytes, &WritePtr1,
                                     &WriteCnt1, &WritePtr2, &WriteCnt2, 0 );
            }
        }

        if ( SUCCEEDED( err ) ) {
            mDevice->renderSamples( WritePtr1, WriteCnt1 / FrameSize,
                                    FrameStep );
            if ( WriteCnt2 > 0 )
                mDevice->renderSamples( WritePtr2, WriteCnt2 / FrameSize,
                                        FrameStep );

            mBuffer->Unlock( WritePtr1, WriteCnt1, WritePtr2, WriteCnt2 );
        } else {
            ERR( "Buffer lock error: %#lx\n", err );
            mDevice->handleDisconnect( "Failed to lock output buffer: 0x%lx",
                                       err );
            return 1;
        }
...
void DSoundPlayback::open( const char* name ) {
    ...
    if ( !name && !PlaybackDevices.empty() ) {
        ...
        if ( iter == PlaybackDevices.cend() ) {
            ...
            if ( SUCCEEDED( hr ) )
                iter = std::find_if( PlaybackDevices.cbegin(),
                                     PlaybackDevices.cend(),
                                     [ &id ]( const DevMap& entry ) -> bool {
...
bool DSoundPlayback::reset() {
    ...
    if ( FAILED( hr ) )
        throw al::backend_exception{ al::backend_error::DeviceError,
                                     "Failed to get speaker config: 0x%08lx",
                                     hr };
    ...
    if ( !mDevice->Flags.test( ChannelsRequest ) ) {
        if ( speakers == DSSPEAKER_MONO )
            ...
            mDevice->FmtChans = DevFmtQuad;
                  ...
                  speakers == DSSPEAKER_5POINT1_BACK )
            mDevice->FmtChans = DevFmtX51;
        else if ( speakers == DSSPEAKER_7POINT1 ||
                  speakers == DSSPEAKER_7POINT1_SURROUND )
...
retry_open:
    ...
    OutputType.Format.nChannels =
        static_cast< WORD >( mDevice->channelsFromFmt() );
    OutputType.Format.wBitsPerSample =
        static_cast< WORD >( mDevice->bytesFromFmt() * 8 );
    ...
    if ( OutputType.Format.nChannels > 2 || mDevice->FmtType == DevFmtFloat ) {
        ...
        if ( SUCCEEDED( hr ) && !mPrimaryBuffer ) {
            ...
            hr = mDS->CreateSoundBuffer( &DSBDescription,
                                         mPrimaryBuffer.getPtr(), nullptr );
        }
        if ( SUCCEEDED( hr ) )
    ...
    if ( SUCCEEDED( hr ) ) {
        ...
        DSBUFFERDESC DSBDescription{};
        DSBDescription.dwSize = sizeof( DSBDescription );
        DSBDescription.dwFlags = DSBCAPS_CTRLPOSITIONNOTIFY |
                                 DSBCAPS_GETCURRENTPOSITION2 |
                                 DSBCAPS_GLOBALFOCUS;
        DSBDescription.dwBufferBytes =
            mDevice->BufferSize * OutputType.Format.nBlockAlign;
        DSBDescription.lpwfxFormat = &OutputType.Format;

        hr = mDS->CreateSoundBuffer( &DSBDescription, mBuffer.getPtr(),
                                     nullptr );
    ...
    if ( SUCCEEDED( hr ) ) {
        ...
        if ( SUCCEEDED( hr ) ) {
            mNotifies = ComPtr< IDirectSoundNotify >{
                static_cast< IDirectSoundNotify* >( ptr ) };
            ...
            for ( uint i{ 0 }; i < num_updates; ++i ) {
                nots[ i ].dwOffset =
                    i * mDevice->UpdateSize * OutputType.Format.nBlockAlign;
                nots[ i ].hEventNotify = mNotifyEvent;
            }
            if ( mNotifies->SetNotificationPositions( num_updates,
                                                      nots.data() ) != DS_OK )
                hr = E_FAIL;
        }
    }
...
void DSoundPlayback::start() {
    try {
        ...
        mThread =
            std::thread{ std::mem_fn( &DSoundPlayback::mixerProc ), this };
...
void DSoundCapture::open( const char* name ) {
    ...
    if ( !name && !CaptureDevices.empty() ) {
        ...
        if ( iter == CaptureDevices.cend() ) {
            ...
            hr = CLSIDFromString( utf8_to_wstr( name ).c_str(), &id );
            if ( SUCCEEDED( hr ) )
                iter = std::find_if( CaptureDevices.cbegin(),
                                     CaptureDevices.cend(),
                                     [ &id ]( const DevMap& entry ) -> bool {
                                         return entry.guid == id;
                                     } );
    ...
    if ( SUCCEEDED( hr ) )
        mDSC->CreateCaptureBuffer( &DSCBDescription, mDSCbuffer.getPtr(),
                                   nullptr );
    if ( SUCCEEDED( hr ) )
...
uint DSoundCapture::availableSamples() {
    ...
    if ( SUCCEEDED( hr ) ) {
        const DWORD NumBytes{ ( BufferBytes + ReadCursor - LastCursor ) %
                              BufferBytes };
        if ( !NumBytes )
            return static_cast< uint >( mRing->readSpace() );
        hr = mDSCbuffer->Lock( LastCursor, NumBytes, &ReadPtr1, &ReadCnt1,
                               &ReadPtr2, &ReadCnt2, 0 );
    }
    if ( SUCCEEDED( hr ) ) {
    ...
    if ( FAILED( hr ) ) {
        ...
        mDevice->handleDisconnect( "Failure retrieving capture data: 0x%lx",
                                   hr );
    }
...
std::string DSoundBackendFactory::probe( BackendType type ) {
    ...
    switch ( type ) {
        case BackendType::Playback:
            ...
            if ( FAILED( hr ) )
                ERR(
                    "Error enumerating DirectSound playback devices (0x%lx)!\n",
                    hr );
        ...
        case BackendType::Capture:
            ...
            hr = DirectSoundCaptureEnumerateW( DSoundEnumDevices,
                                               &CaptureDevices );
            if ( FAILED( hr ) )
...
BackendPtr DSoundBackendFactory::createBackend( DeviceBase* device,
                                                BackendType type ) {
