    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        if ( pret < 0 ) {
            if ( errno == EINTR || errno == EAGAIN )
                continue;
            ERR( "poll failed: %s\n", strerror( errno ) );
            mDevice->handleDisconnect( "Failed to wait for playback buffer: %s",
                                       strerror( errno ) );
            break;
        } else if ( pret == 0 ) {
            WARN( "poll timeout\n" );
        ...
        while ( to_write > 0 && !mKillNow.load( std::memory_order_acquire ) ) {
            ...
            if ( wrote < 0 ) {
                ...
                mDevice->handleDisconnect(
                    "Failed to write playback samples: %s", strerror( errno ) );
                break;
            }
...
void SolarisBackend::open( const char* name ) {
    ...
    if ( fd == -1 )
        throw al::backend_exception{
            al::backend_error::NoDevice, "Could not open %s: %s",
...
bool SolarisBackend::reset() {
    ...
    else if ( info.play.precision == 8 &&
              info.play.encoding == AUDIO_ENCODING_LINEAR )
        mDevice->FmtType = DevFmtByte;
    else if ( info.play.precision == 16 &&
              info.play.encoding == AUDIO_ENCODING_LINEAR )
        mDevice->FmtType = DevFmtShort;
    else if ( info.play.precision == 32 &&
              info.play.encoding == AUDIO_ENCODING_LINEAR )
...
void SolarisBackend::start() {
    try {
        ...
        mThread =
            std::thread{ std::mem_fn( &SolarisBackend::mixerProc ), this };
...
BackendPtr SolarisBackendFactory::createBackend( DeviceBase* device,
                                                 BackendType type ) {
