...
constexpr pa_channel_map MonoChanMap{ 1, { PA_CHANNEL_POSITION_MONO } },
    StereoChanMap{
        2,
    ...
    X61ChanMap{
        ...
        { PA_CHANNEL_POSITION_FRONT_LEFT, PA_CHANNEL_POSITION_FRONT_RIGHT,
          PA_CHANNEL_POSITION_FRONT_CENTER, PA_CHANNEL_POSITION_LFE,
          PA_CHANNEL_POSITION_REAR_CENTER, PA_CHANNEL_POSITION_SIDE_LEFT,
...
public:
    ...
    auto signal( bool wait = false ) const {
        return pa_threaded_mainloop_signal( mLoop, wait );
    }
    ...
    static void streamSuccessCallbackC( pa_stream* stream,
                                        int success,
                                        void* pdata ) noexcept {
        static_cast< PulseMainloop* >( pdata )->streamSuccessCallback(
            stream, success );
    }

    void close( pa_context* context, pa_stream* stream = nullptr );

    void deviceSinkCallback( pa_context*,
        ...
        if ( std::find_if( PlaybackDevices.cbegin(), PlaybackDevices.cend(),
                           match_devname ) != PlaybackDevices.cend() )
            return;
        ...
        PlaybackDevices.emplace_back(
            DevMap{ std::move( newname ), info->name } );
        DevMap& newentry = PlaybackDevices.back();

        TRACE( "Got device \"%s\", \"%s\"\n", newentry.name.c_str(),
               newentry.device_name.c_str() );
    }

    void deviceSourceCallback( pa_context*,
        ...
        if ( std::find_if( CaptureDevices.cbegin(), CaptureDevices.cend(),
                           match_devname ) != CaptureDevices.cend() )
            return;
...
struct MainloopUniqueLock : public std::unique_lock< PulseMainloop > {
    ...
    auto wait() const -> void { pa_threaded_mainloop_wait( mutex()->mLoop ); }
    ...
    void waitForOperation( pa_operation* op ) {
        if ( op ) {
            wait( [ op ] {
                return pa_operation_get_state( op ) != PA_OPERATION_RUNNING;
            } );
    ...
    pa_stream* connectStream( const char* device_name,
                              pa_context* context,
...
pa_context* MainloopUniqueLock::connectContext() {
    ...
    pa_context_set_state_callback(
        context,
        []( pa_context* ctx, void* pdata ) noexcept {
            return static_cast< MainloopUniqueLock* >( pdata )
                ->contextStateCallback( ctx );
        },
        this );

    int err;
    if ( ( err = pa_context_connect( context, nullptr, pulse_ctx_flags,
                                     nullptr ) ) >= 0 ) {
        pa_context_state_t state;
        while ( ( state = pa_context_get_state( context ) ) !=
                PA_CONTEXT_READY ) {
...
pa_stream* MainloopUniqueLock::connectStream( const char* device_name,
                                              pa_context* context,
                                              pa_stream_flags_t flags,
                                              pa_buffer_attr* attr,
                                              pa_sample_spec* spec,
                                              pa_channel_map* chanmap,
                                              BackendType type ) {
    const char* stream_id{ ( type == BackendType::Playback )
                               ? "Playback Stream"
                               : "Capture Stream" };
    pa_stream* stream{ pa_stream_new( context, stream_id, spec, chanmap ) };
    if ( !stream )
        throw al::backend_exception{
            al::backend_error::OutOfMemory, "pa_stream_new() failed (%s)",
            pa_strerror( pa_context_errno( context ) ) };

    pa_stream_set_state_callback(
        stream,
        []( pa_stream* strm, void* pdata ) noexcept {
            return static_cast< MainloopUniqueLock* >( pdata )
    ...
    int err{
        ( type == BackendType::Playback )
            ? pa_stream_connect_playback( stream, device_name, attr, flags,
                                          nullptr, nullptr )
...
void PulseMainloop::probePlaybackDevices() {
    ...
    try {
        ...
        auto sink_callback = []( pa_context* ctx, const pa_sink_info* info,
                                 int eol, void* pdata ) noexcept {
            return static_cast< PulseMainloop* >( pdata )->deviceSinkCallback(
                ctx, info, eol );
        };

        context = plock.connectContext();
        pa_operation* op{ pa_context_get_sink_info_by_name(
            context, nullptr, sink_callback, this ) };
...
void PulseMainloop::probeCaptureDevices() {
    ...
    try {
        MainloopUniqueLock plock{ *this };
        auto src_callback = []( pa_context* ctx, const pa_source_info* info,
                                int eol, void* pdata ) noexcept {
            return static_cast< PulseMainloop* >( pdata )->deviceSourceCallback(
                ctx, info, eol );
        };

        context = plock.connectContext();
        pa_operation* op{ pa_context_get_source_info_by_name(
            context, nullptr, src_callback, this ) };
        plock.waitForOperation( op );
...
struct PulsePlayback final : public BackendBase {
    ...
    void sinkInfoCallback( pa_context* context,
                           const pa_sink_info* info,
                           int eol ) noexcept;
    void sinkNameCallback( pa_context* context,
                           const pa_sink_info* info,
                           int eol ) noexcept;
...
void PulsePlayback::streamWriteCallback( pa_stream* stream,
                                         size_t nbytes ) noexcept {
    do {
        pa_free_cb_t free_func{ nullptr };
        auto buflen = static_cast< size_t >( -1 );
        void* buf{};
        if ( pa_stream_begin_write( stream, &buf, &buflen ) || !buf )
            UNLIKELY {
                buflen = nbytes;
        ...
        mDevice->renderSamples( buf, static_cast< uint >( buflen / mFrameSize ),
                                mSpec.channels );

        int ret{ pa_stream_write( stream, buf, buflen, free_func, 0,
                                  PA_SEEK_RELATIVE ) };
        if ( ret != PA_OK )
            UNLIKELY
...
void PulsePlayback::sinkInfoCallback( pa_context*,
                                      const pa_sink_info* info,
                                      int eol ) noexcept {
    struct ChannelMap {
        DevFmtChannels fmt;
    ...
    auto chaniter = std::find_if(
        chanmaps.cbegin(), chanmaps.cend(),
    ...
    if ( chaniter != chanmaps.cend() ) {
        ...
        pa_channel_map_snprint( chanmap_str, sizeof( chanmap_str ),
                                &info->channel_map );
    ...
    mDevice->Flags.set(
        DirectEar,
        ( info->active_port && strcmp( info->active_port->name,
                                       "analog-output-headphones" ) == 0 ) );
}

void PulsePlayback::sinkNameCallback( pa_context*,
                                      const pa_sink_info* info,
                                      int eol ) noexcept {
    if ( eol ) {
        mMainloop.signal();
...
void PulsePlayback::open( const char* name ) {
    ...
    pa_stream_flags_t flags{ PA_STREAM_START_CORKED | PA_STREAM_FIX_FORMAT |
                             PA_STREAM_FIX_RATE | PA_STREAM_FIX_CHANNELS };
    ...
    if ( !pulse_name ) {
        ...
        if ( defname )
            pulse_name = defname->c_str();
    }
    TRACE( "Connecting to \"%s\"\n", pulse_name ? pulse_name : "(default)" );
    mStream = plock.connectStream( pulse_name, mContext, flags, nullptr, &spec,
                                   nullptr, BackendType::Playback );

    pa_stream_set_moved_callback(
        mStream,
        []( pa_stream* stream, void* pdata ) noexcept {
            return static_cast< PulsePlayback* >( pdata )->streamMovedCallback(
                stream );
        },
        this );
    mFrameSize = static_cast< uint >(
        pa_frame_size( pa_stream_get_sample_spec( mStream ) ) );

    if ( pulse_name )
        mDeviceName.emplace( pulse_name );
    else
        mDeviceName.reset();
    if ( !dev_name ) {
        auto name_callback = []( pa_context* context, const pa_sink_info* info,
                                 int eol, void* pdata ) noexcept {
            return static_cast< PulsePlayback* >( pdata )->sinkNameCallback(
                context, info, eol );
        };
        pa_operation* op{ pa_context_get_sink_info_by_name(
            mContext, pa_stream_get_device_name( mStream ), name_callback,
            this ) };
        plock.waitForOperation( op );
    } else
...
bool PulsePlayback::reset() {
    ...
    auto info_callback = []( pa_context* context, const pa_sink_info* info,
                             int eol, void* pdata ) noexcept {
        return static_cast< PulsePlayback* >( pdata )->sinkInfoCallback(
            context, info, eol );
    };
    ...
    if ( !GetConfigValueBool( nullptr, "pulse", "allow-moves", true ) )
        flags |= PA_STREAM_DONT_MOVE;
    if ( GetConfigValueBool( mDevice->DeviceName.c_str(), "pulse",
    ...
    if ( GetConfigValueBool( mDevice->DeviceName.c_str(), "pulse", "fix-rate",
                             false ) ||
    ...
    mStream = plock.connectStream( deviceName, mContext, flags, &mAttr, &mSpec,
                                   &chanmap, BackendType::Playback );

    pa_stream_set_state_callback(
        mStream,
        []( pa_stream* stream, void* pdata ) noexcept {
            return static_cast< PulsePlayback* >( pdata )->streamStateCallback(
                stream );
        },
        this );
    pa_stream_set_moved_callback(
    ...
    if ( mDevice->Frequency != mSpec.rate ) {
         ...
         */
        const auto scale =
            static_cast< double >( mSpec.rate ) / mDevice->Frequency;
        const auto perlen = static_cast< uint >(
            clampd( scale * mDevice->UpdateSize + 0.5, 64.0, 8192.0 ) );
        const auto buflen = static_cast< uint >(
            clampd( scale * mDevice->BufferSize + 0.5, perlen * 2,
        ...
        op = pa_stream_set_buffer_attr( mStream, &mAttr,
    ...
    auto attr_callback = []( pa_stream* stream, void* pdata ) noexcept {
        return static_cast< PulsePlayback* >( pdata )->bufferAttrCallback(
            stream );
    };
...
void PulsePlayback::start() {
    ...
    if ( size_t todo{ pa_stream_writable_size( mStream ) } ) {
        ...
        mDevice->renderSamples( buf, static_cast< uint >( todo / mFrameSize ),
                                mSpec.channels );
    ...
    pa_stream_set_write_callback(
        mStream,
        []( pa_stream* stream, size_t nbytes, void* pdata ) noexcept {
            return static_cast< PulsePlayback* >( pdata )->streamWriteCallback(
                stream, nbytes );
        },
        this );
    pa_operation* op{ pa_stream_cork(
        mStream, 0, &PulseMainloop::streamSuccessCallbackC, &mMainloop ) };
...
void PulsePlayback::stop() {
    ...
    pa_operation* op{ pa_stream_cork(
        mStream, 1, &PulseMainloop::streamSuccessCallbackC, &mMainloop ) };
    plock.waitForOperation( op );
    pa_stream_set_write_callback( mStream, nullptr, nullptr );
}
...
ClockLatency PulsePlayback::getClockLatency() {
    ...
    if ( err != 0 )
        UNLIKELY {
             ...
             * server yet. Give a generic value since nothing better is
             * available.
             */
...
struct PulseCapture final : public BackendBase {
    ...
    void streamStateCallback( pa_stream* stream ) noexcept;
    void sourceNameCallback( pa_context* context,
                             const pa_source_info* info,
                             int eol ) noexcept;
    void streamMovedCallback( pa_stream* stream ) noexcept;
...
void PulseCapture::sourceNameCallback( pa_context*,
                                       const pa_source_info* info,
                                       int eol ) noexcept {
    if ( eol ) {
        mMainloop.signal();
...
void PulseCapture::open( const char* name ) {
    ...
    if ( name ) {
        ...
        auto iter =
            std::find_if( CaptureDevices.cbegin(), CaptureDevices.cend(),
                          [ name ]( const DevMap& entry ) -> bool {
                              return entry.name == name;
                          } );
    ...
    switch ( mDevice->FmtChans ) {
        ...
        case DevFmtAmbi3D:
            throw al::backend_exception{
                al::backend_error::DeviceError, "%s capture not supported",
    ...
    const auto frame_size = static_cast< uint >( pa_frame_size( &mSpec ) );
    const uint samples{
        maxu( mDevice->BufferSize, 100 * mDevice->Frequency / 1000 ) };
    mAttr.minreq = ~0u;
    mAttr.prebuf = ~0u;
    mAttr.maxlength = samples * frame_size;
    mAttr.tlength = ~0u;
    mAttr.fragsize =
        minu( samples, 50 * mDevice->Frequency / 1000 ) * frame_size;
    ...
    TRACE( "Connecting to \"%s\"\n", pulse_name ? pulse_name : "(default)" );
    mStream = plock.connectStream( pulse_name, mContext, flags, &mAttr, &mSpec,
                                   &chanmap, BackendType::Capture );

    pa_stream_set_moved_callback(
        mStream,
        []( pa_stream* stream, void* pdata ) noexcept {
            return static_cast< PulseCapture* >( pdata )->streamMovedCallback(
                stream );
        },
        this );
    pa_stream_set_state_callback(
        mStream,
        []( pa_stream* stream, void* pdata ) noexcept {
            return static_cast< PulseCapture* >( pdata )->streamStateCallback(
                stream );
        },
        this );
    ...
    if ( mDevice->DeviceName.empty() ) {
        auto name_callback = []( pa_context* context,
                                 const pa_source_info* info, int eol,
                                 void* pdata ) noexcept {
            return static_cast< PulseCapture* >( pdata )->sourceNameCallback(
                context, info, eol );
        };
        pa_operation* op{ pa_context_get_source_info_by_name(
            mContext, pa_stream_get_device_name( mStream ), name_callback,
...
void PulseCapture::captureSamples( al::byte* buffer, uint samples ) {
    ...
    while ( !dstbuf.empty() ) {
        ...
        const void* capbuf;
        size_t caplen;
        if ( pa_stream_peek( mStream, &capbuf, &caplen ) < 0 )
            UNLIKELY {
        ...
        if ( !capbuf )
            UNLIKELY
        mHoleLength = caplen;
        else mCapBuffer = { static_cast< const al::byte* >( capbuf ), caplen };
...
uint PulseCapture::availableSamples() {
    ...
    if ( mDevice->Connected.load( std::memory_order_acquire ) ) {
        ...
        if ( static_cast< ssize_t >( got ) < 0 )
            UNLIKELY {
                ...
                mDevice->handleDisconnect( "Failed getting readable size: %s",
                                           err );
            }
        else {
    ...
    readable =
        std::min< size_t >( readable, std::numeric_limits< uint >::max() );
...
#define LOAD_FUNC( x )                               \
    do {                                             \
        p##x = reinterpret_cast< decltype( p##x ) >( \
            GetSymbol( pulse_handle, #x ) );         \
        if ( !( p##x ) ) {                           \
            ret = false;                             \
            missing_funcs += "\n" #x;                \
        }                                            \
...
std::string PulseBackendFactory::probe( BackendType type ) {
    ...
    switch ( type ) {
        case BackendType::Playback:
            ...
            std::for_each( PlaybackDevices.cbegin(), PlaybackDevices.cend(),
                           add_device );
            break;

        case BackendType::Capture:
            gGlobalMainloop.probeCaptureDevices();
            std::for_each( CaptureDevices.cbegin(), CaptureDevices.cend(),
                           add_device );
            break;
    }
...
BackendPtr PulseBackendFactory::createBackend( DeviceBase* device,
                                               BackendType type ) {
