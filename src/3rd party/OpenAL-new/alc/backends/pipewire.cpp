...
_Pragma( "GCC diagnostic push" )
    _Pragma( "GCC diagnostic ignored \"-Wpedantic\"" ) _Pragma(
        ...
        _Pragma( "GCC diagnostic ignored \"-Wfloat-conversion\"" ) _Pragma(
...
#include "spa/utils/json.h"
    ...
    auto ppw_node_subscribe_params( pw_node * node, Args && ... args ) {
        return pw_node_subscribe_params( node,
                                         std::forward< Args >( args )... );
    }
    template < typename... Args >
    auto ppw_metadata_add_listener( pw_metadata * mdata, Args && ... args ) {
        return pw_metadata_add_listener( mdata,
                                         std::forward< Args >( args )... );
    }
...
#define PW_KEY_NODE_RATE "node.rate"
    ...
    bool check_version( const char* version ) {
        /* There doesn't seem to be a function to get the version as an integer,
         * so instead we have to parse the string, which hopefully won't break
         * in the future.
         */
        int major{ 0 }, minor{ 0 }, revision{ 0 };
        int ret{ sscanf( version, "%d.%d.%d", &major, &minor, &revision ) };
        if ( ret == 3 &&
             ( major > PW_MAJOR || ( major == PW_MAJOR && minor > PW_MINOR ) ||
...
#define PWIRE_FUNCS2( MAGIC ) MAGIC( pw_stream_get_time_n )
#else
#define PWIRE_FUNCS2( MAGIC ) MAGIC( pw_stream_get_time )
#endif
...
#define LOAD_FUNC( f )                               \
    do {                                             \
        p##f = reinterpret_cast< decltype( p##f ) >( \
            GetSymbol( pwire_handle, #f ) );         \
        if ( p##f == nullptr )                       \
            missing_funcs += "\n" #f;                \
    } while ( 0 );
...
#define pw_stream_get_time_n ppw_stream_get_time_n
#else
    inline auto pw_stream_get_time_n( pw_stream * stream, pw_time * ptime,
                                      size_t /*size*/ ) {
     ...
     * from the API and the caller is expected to C-style cast to inherited
     * types as needed. It's also not made very clear what types a given type
     * can be casted to. To make it a bit safer, this as() method allows casting
     * pw_* types to known inherited types, generating a compile-time error for
     ...
     */
    template <>
    pw_proxy* as( pw_registry * reg ) noexcept {
        return reinterpret_cast< pw_proxy* >( reg );
    }
    template <>
    pw_proxy* as( pw_node * node ) noexcept {
        return reinterpret_cast< pw_proxy* >( node );
    }
    template <>
    pw_proxy* as( pw_metadata * mdata ) noexcept {
        return reinterpret_cast< pw_proxy* >( mdata );
    }
    ...
    struct PwRegistryDeleter {
        void operator()( pw_registry* reg ) const {
    ...
    struct PwNodeDeleter {
        void operator()( pw_node* node ) const {
    ...
    struct PwMetadataDeleter {
        void operator()( pw_metadata* mdata ) const {
    ...
    constexpr pw_stream_flags operator|( pw_stream_flags lhs,
                                         pw_stream_flags rhs ) noexcept {
    ...
    public:
        ...
        auto signal( bool wait ) const {
            return pw_thread_loop_signal( mLoop, wait );
        }

        auto newContext( pw_properties* props = nullptr,
                         size_t user_data_size = 0 ) {
            return PwContextPtr{
                pw_context_new( getLoop(), props, user_data_size ) };
        }
    ...
    struct MainloopUniqueLock : public std::unique_lock< ThreadMainloop > {
        ...
        auto wait() const -> void { pw_thread_loop_wait( mutex()->mLoop ); }
     ...
     * their default formats, so playback devices can be configured to match.
     * The device list is updated asynchronously, so it will have the latest
     * list of devices provided by the server.
     */

    struct NodeProxy;
    struct MetadataProxy;

    /* The global thread watching for global events. This particular class
     * responds to objects being added to or removed from the registry.
     */
    struct EventManager {
        ...
        /* A list of proxy objects watching for events about changes to objects
         * in the registry.
         */
         ...
         * SequenceID that marks the end of populating the registry. As objects
         * of interest are found, events to parse them are generated and
         * mInitSeq is updated with a newer ID. When mInitSeq stops being
         * updated and the event corresponding to it is reached, mInitDone will
         * be set to true.
         */
         ...
         */
        void waitForInit() {
            if ( !mInitDone.load( std::memory_order_acquire ) )
                UNLIKELY {
                    MainloopUniqueLock plock{ mLoop };
                    plock.wait( [ this ]() {
                        return mInitDone.load( std::memory_order_acquire );
                    } );
                }
        }
        ...
        static void addCallbackC( void* object,
                                  ...
                                  uint32_t version,
                                  const spa_dict* props ) {
            static_cast< EventManager* >( object )->addCallback(
    ...
    /* Enumerated devices. This is updated asynchronously as the app runs, and
     * the gEventHandler thread loop must be locked when accessing the list.
     */
    ...
    const spa_audio_channel MonoMap[]{ SPA_AUDIO_CHANNEL_MONO },
        StereoMap[]{ SPA_AUDIO_CHANNEL_FL, SPA_AUDIO_CHANNEL_FR },
        QuadMap[]{ SPA_AUDIO_CHANNEL_FL, SPA_AUDIO_CHANNEL_FR,
                   SPA_AUDIO_CHANNEL_RL, SPA_AUDIO_CHANNEL_RR },
        X51Map[]{ SPA_AUDIO_CHANNEL_FL, SPA_AUDIO_CHANNEL_FR,
                  SPA_AUDIO_CHANNEL_FC, SPA_AUDIO_CHANNEL_LFE,
                  SPA_AUDIO_CHANNEL_SL, SPA_AUDIO_CHANNEL_SR },
        X51RearMap[]{ SPA_AUDIO_CHANNEL_FL, SPA_AUDIO_CHANNEL_FR,
                      SPA_AUDIO_CHANNEL_FC, SPA_AUDIO_CHANNEL_LFE,
                      SPA_AUDIO_CHANNEL_RL, SPA_AUDIO_CHANNEL_RR },
        X61Map[]{ SPA_AUDIO_CHANNEL_FL, SPA_AUDIO_CHANNEL_FR,
                  SPA_AUDIO_CHANNEL_FC, SPA_AUDIO_CHANNEL_LFE,
                  SPA_AUDIO_CHANNEL_RC, SPA_AUDIO_CHANNEL_SL,
                  SPA_AUDIO_CHANNEL_SR },
        X71Map[]{ SPA_AUDIO_CHANNEL_FL, SPA_AUDIO_CHANNEL_FR,
                  SPA_AUDIO_CHANNEL_FC, SPA_AUDIO_CHANNEL_LFE,
                  SPA_AUDIO_CHANNEL_RL, SPA_AUDIO_CHANNEL_RR,
                  SPA_AUDIO_CHANNEL_SL, SPA_AUDIO_CHANNEL_SR },
        X714Map[]{ SPA_AUDIO_CHANNEL_FL,  SPA_AUDIO_CHANNEL_FR,
                   SPA_AUDIO_CHANNEL_FC,  SPA_AUDIO_CHANNEL_LFE,
                   SPA_AUDIO_CHANNEL_RL,  SPA_AUDIO_CHANNEL_RR,
                   SPA_AUDIO_CHANNEL_SL,  SPA_AUDIO_CHANNEL_SR,
    ...
    /**
     * Checks if every channel in 'map1' exists in 'map0' (that is, map0 is
     * equal to or a superset of map1).
     */
    template < size_t N >
    bool MatchChannelMap( const al::span< const uint32_t > map0,
                          const spa_audio_channel( &map1 )[ N ] ) {
        if ( map0.size() < N )
            return false;
    ...
    void DeviceNode::parseSampleRate( const spa_pod* value ) noexcept {
        ...
        if ( choiceType == SPA_CHOICE_Range ) {
            ...
            TRACE( "Device ID %" PRIu64 " sample rate: %d (range: %d -> %d)\n",
                   mSerial, srates[ 0 ], srates[ 1 ], srates[ 2 ] );
            mSampleRate = static_cast< uint >(
                clampi( srates[ 0 ], MIN_OUTPUT_RATE, MAX_OUTPUT_RATE ) );
            return;
        }
        ...
        if ( choiceType == SPA_CHOICE_Enum ) {
            ...
            std::string others{ ( srates.size() > 1 )
                                    ? std::to_string( srates[ 1 ] )
                                    : std::string{} };
    ...
    struct NodeProxy {
        ...
        static void paramCallbackC( void* object,
                                    ...
                                    uint32_t next,
                                    const spa_pod* param ) {
            static_cast< NodeProxy* >( object )->paramCallback( seq, id, index,
                                                                next, param );
        }
    };
    ...
    void NodeProxy::infoCallback( const pw_node_info* info ) {
         ...
         *
         * TODO: Can the media class or name/desc change without being removed
         * and readded?
         */
        if ( ( info->change_mask & PW_NODE_CHANGE_MASK_PROPS ) ) {
            ...
            if ( al::strcasecmp( media_class, AudioSinkClass ) == 0 )
                ntype = NodeType::Sink;
                      ...
                      al::strcasecmp( media_class, AudioSourceVirtualClass ) ==
                          0 )
            ...
            const char* nodeName{
                spa_dict_lookup( info->props, PW_KEY_NODE_DESCRIPTION ) };
            if ( !nodeName || !*nodeName )
                nodeName = spa_dict_lookup( info->props, PW_KEY_NODE_NICK );
            if ( !nodeName || !*nodeName )
                nodeName = devName;

            uint64_t serial_id{ info->id };
#ifdef PW_KEY_OBJECT_SERIAL
            if ( const char* serial_str{
                     spa_dict_lookup( info->props, PW_KEY_OBJECT_SERIAL ) } ) {
                char* serial_end{};
            ...
            const char* form_factor{
                spa_dict_lookup( info->props, PW_KEY_DEVICE_FORM_FACTOR ) };
            TRACE( "Got %s device \"%s\"%s%s%s\n", AsString( ntype ),
                   devName ? devName : "(nil)", form_factor ? " (" : "",
            ...
            if ( nodeName && *nodeName )
                ...
                node.mName = "PipeWire node #" + std::to_string( info->id );
            node.mDevName = devName ? devName : "";
            node.mType = ntype;
            node.mIsHeadphones =
                form_factor &&
                ( al::strcasecmp( form_factor, "headphones" ) == 0 ||
                  al::strcasecmp( form_factor, "headset" ) == 0 );
        }
    }

    void NodeProxy::paramCallback( int, uint32_t id, uint32_t, uint32_t,
                                   const spa_pod* param ) {
        if ( id == SPA_PARAM_EnumFormat ) {
            ...
            if ( const spa_pod_prop* prop{ spa_pod_find_prop(
                     param, nullptr, SPA_FORMAT_AUDIO_rate ) } )
                node->parseSampleRate( &prop->value );

            if ( const spa_pod_prop* prop{ spa_pod_find_prop(
                     param, nullptr, SPA_FORMAT_AUDIO_position ) } )
                node->parsePositions( &prop->value );
            else if ( ( prop = spa_pod_find_prop(
                            param, nullptr, SPA_FORMAT_AUDIO_channels ) ) !=
                      nullptr )
    ...
    struct MetadataProxy {
        ...
        MetadataProxy( uint32_t id, PwMetadataPtr mdata )
            ...
            static constexpr pw_metadata_events metadataEvents{
        ...
        static int propertyCallbackC( void* object,
                                      ...
                                      const char* type,
                                      const char* value ) {
            return static_cast< MetadataProxy* >( object )->propertyCallback(
                id, key, type, value );
        }
    };

    int MetadataProxy::propertyCallback( uint32_t id, const char* key,
        ...
        if ( !type ) {
            ...
            if ( !isCapture )
                DefaultSinkDevice.clear();
            else
                DefaultSourceDevice.clear();
            return 0;
        }
        ...
        while ( auto propKey = get_json_string( &it[ 1 ] ) ) {
            if ( *propKey == "name" ) {
                ...
                TRACE( "Got default %s device \"%s\"\n",
                       isCapture ? "capture" : "playback", propValue->c_str() );
                if ( !isCapture )
    ...
    bool EventManager::init() {
        ...
        mContext = mLoop.newContext( pw_properties_new(
            PW_KEY_CONFIG_NAME, "client-rt.conf", nullptr ) );
        if ( !mContext ) {
        ...
        mRegistry = PwRegistryPtr{
            pw_core_get_registry( mCore.get(), PW_VERSION_REGISTRY, 0 ) };
        if ( !mRegistry ) {
        ...
        ppw_registry_add_listener( mRegistry.get(), &mRegistryListener,
                                   &registryEvents, this );
    ...
    void EventManager::addCallback( uint32_t id, uint32_t, const char* type,
        ...
        if ( std::strcmp( type, PW_TYPE_INTERFACE_Node ) == 0 ) {
            ...
            const bool isGood{
                al::strcasecmp( media_class, AudioSinkClass ) == 0 ||
            ...
            if ( !isGood ) {
                if ( std::strstr( media_class, "/Video" ) == nullptr &&
                     std::strncmp( media_class, StreamClass,
                                   sizeof( StreamClass ) - 1 ) != 0 )
                    TRACE( "Ignoring node class %s\n", media_class );
                return;
            }

            /* Create the proxy object. */
            auto node = PwNodePtr{ static_cast< pw_node* >( pw_registry_bind(
                mRegistry.get(), id, type, version, sizeof( NodeProxy ) ) ) };
            if ( !node ) {
             ...
             */
            auto* proxy = static_cast< NodeProxy* >(
                pw_proxy_get_user_data( as< pw_proxy* >( node.get() ) ) );
            mNodeList.emplace_back(
                al::construct_at( proxy, id, std::move( node ) ) );
            syncInit();
            ...
            auto mdata = PwMetadataPtr{ static_cast< pw_metadata* >(
                pw_registry_bind( mRegistry.get(), id, type, version,
            ...
            mDefaultMetadata =
                al::construct_at( proxy, id, std::move( mdata ) );
            syncInit();
        }
    }
    ...
    void EventManager::removeCallback( uint32_t id ) {
        ...
        auto node_end =
            std::remove_if( mNodeList.begin(), mNodeList.end(), clear_node );
    ...
    void EventManager::coreCallback( uint32_t id, int seq ) {
        if ( id == PW_ID_CORE && seq == mInitSeq ) {
            /* Initialization done. Remove this callback and signal anyone that
             * may be waiting.
             */
        ...
        if ( use_f32p ) {
            ...
            switch ( device->FmtType ) {
                ...
                case DevFmtShort:
                    ...
                    break;
                case DevFmtUShort:
                    info.format = SPA_AUDIO_FORMAT_U16;
                    break;
                case DevFmtInt:
                    info.format = SPA_AUDIO_FORMAT_S32;
                    break;
                case DevFmtUInt:
                    info.format = SPA_AUDIO_FORMAT_U32;
                    break;
                case DevFmtFloat:
    ...
    class PipeWirePlayback final : public BackendBase {
        ...
        static void stateChangedCallbackC( void* data,
                                           ...
                                           pw_stream_state state,
                                           const char* error ) {
            static_cast< PipeWirePlayback* >( data )->stateChangedCallback(
                old, state, error );
        }
        ...
        static void ioChangedCallbackC( void* data,
                                        ...
                                        void* area,
                                        uint32_t size ) {
            static_cast< PipeWirePlayback* >( data )->ioChangedCallback(
                id, area, size );
        }
    ...
    void PipeWirePlayback::stateChangedCallback(
        pw_stream_state, pw_stream_state, const char* ) {
        mLoop.signal( false );
    }

    void PipeWirePlayback::ioChangedCallback( uint32_t id, void* area,
                                              uint32_t size ) {
        switch ( id ) {
    ...
    void PipeWirePlayback::outputCallback() {
        ...
        const al::span< spa_data > datas{
            pw_buf->buffer->datas,
            minu( mNumChannels, pw_buf->buffer->n_datas ) };
#if PW_CHECK_VERSION( 0, 3, 49 )
        /* In 0.3.49, pw_buffer::requested specifies the number of samples
         * needed by the resampler/graph for this audio update.
         */
        uint length{ static_cast< uint >( pw_buf->requested ) };
#else
        /* In 0.3.48 and earlier, spa_io_rate_match::size apparently has the
         * number of samples per update.
         */
        uint length{ mRateMatch ? mRateMatch->size : 0u };
#endif
        /* If no length is specified, use the device's update size as a
         * fallback. */
        if ( !length )
            UNLIKELY length = mDevice->UpdateSize;

        /* For planar formats, each datas[] seems to contain one channel, so
         * store the pointers in an array. Limit the render length in case the
         * available buffer length in any one channel is smaller than we wanted
    ...
    void PipeWirePlayback::open( const char* name ) {
        ...
        if ( !name ) {
            ...
            if ( !DefaultSinkDevice.empty() ) {
                ...
                match = std::find_if( devlist.cbegin(), devlist.cend(),
                                      match_default );
            }
            ...
            auto match =
                std::find_if( devlist.cbegin(), devlist.cend(), match_name );
            if ( match == devlist.cend() )
        ...
        if ( !mLoop ) {
            ...
            const std::string thread_name{ "ALSoftP" +
                                           std::to_string( count ) };
        ...
        if ( !mContext ) {
            pw_properties* cprops{ pw_properties_new(
                PW_KEY_CONFIG_NAME, "client-rt.conf", nullptr ) };
        ...
        if ( !mCore ) {
            mCore =
                PwCorePtr{ pw_context_connect( mContext.get(), nullptr, 0 ) };
            if ( !mCore )
                throw al::backend_exception{
                    al::backend_error::DeviceError,
    ...
    bool PipeWirePlayback::reset() {
        ...
        /* If connecting to a specific device, update various device parameters
         * to match its format.
         */
        bool is51rear{ false };
        ...
        if ( mTargetId != PwIdAny ) {
            ...
            auto match_id = [ targetid =
                                  mTargetId ]( const DeviceNode& n ) -> bool {
                return targetid == n.mSerial;
            };
            auto match =
                std::find_if( devlist.cbegin(), devlist.cend(), match_id );
            if ( match != devlist.cend() ) {
                if ( !mDevice->Flags.test( FrequencyRequest ) &&
                     match->mSampleRate > 0 ) {
                    /* Scale the update size if the sample rate changes. */
                    const double scale{
                        static_cast< double >( match->mSampleRate ) /
                        mDevice->Frequency };
                    const double numbufs{
                        static_cast< double >( mDevice->BufferSize ) /
                        mDevice->UpdateSize };
                    mDevice->Frequency = match->mSampleRate;
                    mDevice->UpdateSize = static_cast< uint >( clampd(
                        mDevice->UpdateSize * scale + 0.5, 64.0, 8192.0 ) );
                    mDevice->BufferSize = static_cast< uint >(
                        numbufs * mDevice->UpdateSize + 0.5 );
                }
         ...
         */
        spa_audio_info_raw info{
            make_spa_info( mDevice, is51rear, ForceF32Planar ) };
        ...
        spa_pod_builder b{
            make_pod_builder( pod_buffer.get(), pod_buffer_size ) };
        ...
        /* TODO: Which properties are actually needed here? Any others that
         * could be useful?
         */
        auto&& binary = GetProcBinary();
        const char* appname{ binary.fname.length() ? binary.fname.c_str()
                                                   : "OpenAL Soft" };
        pw_properties* props{ pw_properties_new(
            PW_KEY_NODE_NAME, appname, PW_KEY_NODE_DESCRIPTION, appname,
            PW_KEY_MEDIA_TYPE, "Audio", PW_KEY_MEDIA_CATEGORY, "Playback",
            PW_KEY_MEDIA_ROLE, "Game", PW_KEY_NODE_ALWAYS_PROCESS, "true",
            nullptr ) };
        if ( !props )
            throw al::backend_exception{
                al::backend_error::DeviceError,
                "Failed to create PipeWire stream properties (errno: %d)",
                errno };

        pw_properties_setf( props, PW_KEY_NODE_LATENCY, "%u/%u",
                            mDevice->UpdateSize, mDevice->Frequency );
        pw_properties_setf( props, PW_KEY_NODE_RATE, "1/%u",
                            mDevice->Frequency );
#ifdef PW_KEY_TARGET_OBJECT
        pw_properties_setf( props, PW_KEY_TARGET_OBJECT, "%" PRIu64,
                            mTargetId );
#else
        ...
        mStream = PwStreamPtr{
            pw_stream_new( mCore.get(), "Playback Stream", props ) };
        if ( !mStream )
            throw al::backend_exception{
                al::backend_error::NoDevice,
        ...
        if ( GetConfigValueBool( mDevice->DeviceName.c_str(), "pipewire",
                                 "rt-mix", true ) )
            flags |= PW_STREAM_FLAG_RT_PROCESS;
        if ( int res{ pw_stream_connect( mStream.get(), PW_DIRECTION_OUTPUT,
                                         PwIdAny, flags, &params, 1 ) } )
            throw al::backend_exception{
                al::backend_error::DeviceError,
         ...
         * mDevice->BufferSize with the total known buffering delay from the
         * head of this playback stream to the tail of the device output.
         *
    ...
    void PipeWirePlayback::start() {
        ...
        /* Wait for the stream to start playing (would be nice to not, but we
         * need the actual update size which is only available after starting).
         */
        plock.wait( [ stream = mStream.get() ]() {
            const char* error{};
            pw_stream_state state{ pw_stream_get_state( stream, &error ) };
            if ( state == PW_STREAM_STATE_ERROR )
                throw al::backend_exception{ al::backend_error::DeviceError,
                                             "PipeWire stream error: %s",
                                             error ? error : "(unknown)" };
            return state == PW_STREAM_STATE_STREAMING;
        } );

        /* HACK: Try to work out the update size and total buffering size.
         * There's no actual query for this, so we have to work it out from the
         * stream time info, and assume it stays accurate with future updates.
         * The stream time info may also not be available right away, so we have
         * to wait until it is (up to about 2 seconds).
         */
        int wait_count{ 100 };
        do {
            pw_time ptime{};
            if ( int res{ pw_stream_get_time_n( mStream.get(), &ptime,
                                                sizeof( ptime ) ) } ) {
...
#if PW_CHECK_VERSION( 0, 3, 50 )
            /* Assume ptime.avail_buffers+ptime.queued_buffers is the target
             * buffer queue size.
             */
            if ( ptime.rate.denom > 0 &&
                 ( ptime.avail_buffers || ptime.queued_buffers ) &&
                 updatesize > 0 ) {
                const uint totalbuffers{ ptime.avail_buffers +
                                         ptime.queued_buffers };

                /* Ensure the delay is in sample frames. */
                const uint64_t delay{ static_cast< uint64_t >( ptime.delay ) *
                                      mDevice->Frequency * ptime.rate.num /
                                      ptime.rate.denom };

                mDevice->UpdateSize = updatesize;
                mDevice->BufferSize = static_cast< uint >(
                    ptime.buffered + delay + totalbuffers * updatesize );
                break;
            }
#else
            /* Prior to 0.3.50, we can only measure the delay with the update
             * size, assuming one buffer and no resample buffering.
             */
            ...
            plock.lock();
        } while ( pw_stream_get_state( mStream.get(), nullptr ) ==
    ...
    void PipeWirePlayback::stop() {
        ...
        plock.wait( [ stream = mStream.get() ]() {
            return pw_stream_get_state( stream, nullptr ) !=
                   PW_STREAM_STATE_STREAMING;
        } );
    }

    ClockLatency PipeWirePlayback::getClockLatency() {
        /* Given a real-time low-latency output, this is rather complicated to
         * get accurate timing. So, here we go.
         */
        ...
        /* Now get the mixer time and the CLOCK_MONOTONIC time atomically (i.e.
         * the monotonic clock closest to 'now', and the last mixer time at
         * 'now').
         */
        nanoseconds mixtime{};
        timespec tspec{};
         ...
         * nanoseconds.
         */
        nanoseconds monoclock{ seconds{ tspec.tv_sec } +
                               nanoseconds{ tspec.tv_nsec } };
        nanoseconds curtic{}, delay{};
        if ( ptime.rate.denom < 1 )
            UNLIKELY {
                /* If there's no stream rate, the stream hasn't had a chance to
                 * get going and return time info yet. Just use dummy values.
                 */
                ptime.now = monoclock.count();
                curtic = mixtime;
                delay = nanoseconds{ seconds{ mDevice->BufferSize } } /
                        mDevice->Frequency;
            }
        else {
            /* The stream gets recreated with each reset, so include the time
             ...
             */
            curtic +=
                seconds{ ptime.ticks / ptime.rate.denom } * ptime.rate.num;
            curtic += nanoseconds{ seconds{ ptime.ticks % ptime.rate.denom } *
                                   ptime.rate.num } /
                      ptime.rate.denom;

            /* The delay should be small enough to not worry about overflow. */
            delay = nanoseconds{ seconds{ ptime.delay } * ptime.rate.num } /
                    ptime.rate.denom;
        }
        ...
        /* Reduce the delay according to how much time has passed since the
         * known stream time. This isn't 100% accurate since the system
         * monotonic clock doesn't tick at the exact same rate as the audio
         * device, but it should be good enough with ptime.now being constantly
         * updated every few milliseconds with ptime.ticks.
         */
    ...
    class PipeWireCapture final : public BackendBase {
        ...
        static void stateChangedCallbackC( void* data,
                                           ...
                                           pw_stream_state state,
                                           const char* error ) {
            static_cast< PipeWireCapture* >( data )->stateChangedCallback(
                old, state, error );
        }
    ...
    void PipeWireCapture::stateChangedCallback( pw_stream_state,
                                                pw_stream_state, const char* ) {
        mLoop.signal( false );
    }
    ...
    void PipeWireCapture::inputCallback() {
        ...
        const uint size{
            minu( bufdata->chunk->size, bufdata->maxsize - offset ) };

        mRing->write( static_cast< char* >( bufdata->data ) + offset,
    ...
    void PipeWireCapture::open( const char* name ) {
        ...
        if ( !name ) {
            ...
            targetid = match->mSerial;
            if ( match->mType != NodeType::Sink )
                devname = match->mName;
            else
                devname = MonitorPrefix + match->mName;
        } else {
            EventWatcherLockGuard _{ gEventHandler };
            auto&& devlist = DeviceNode::GetList();
            ...
            auto match =
                std::find_if( devlist.cbegin(), devlist.cend(), match_name );
            if ( match == devlist.cend() &&
                 std::strncmp( name, MonitorPrefix, MonitorPrefixLen ) == 0 ) {
                const char* sinkname{ name + MonitorPrefixLen };
                auto match_sinkname =
                    [ sinkname ]( const DeviceNode& n ) -> bool {
        ...
        if ( !mLoop ) {
            const uint count{
                OpenCount.fetch_add( 1, std::memory_order_relaxed ) };
            const std::string thread_name{ "ALSoftC" +
                                           std::to_string( count ) };
            mLoop = ThreadMainloop::Create( thread_name.c_str() );
            if ( !mLoop )
                throw al::backend_exception{
        ...
        spa_audio_info_raw info{
            make_spa_info( mDevice, is51rear, UseDevType ) };
        ...
        /* We don't actually care what the latency/update size is, as long as
         * it's reasonable. Unfortunately, when unspecified PipeWire seems to
         * default to around 40ms, which isn't great. So request 20ms instead.
         */
        pw_properties_setf( props, PW_KEY_NODE_LATENCY, "%u/%u",
                            ( mDevice->Frequency + 25 ) / 50,
                            mDevice->Frequency );
        pw_properties_setf( props, PW_KEY_NODE_RATE, "1/%u",
                            mDevice->Frequency );
#ifdef PW_KEY_TARGET_OBJECT
        ...
        MainloopUniqueLock plock{ mLoop };
        mStream = PwStreamPtr{
            pw_stream_new( mCore.get(), "Capture Stream", props ) };
        if ( !mStream )
            throw al::backend_exception{
                al::backend_error::NoDevice,
                "Failed to create PipeWire stream (errno: %d)", errno };
        ...
        constexpr pw_stream_flags Flags{
            PW_STREAM_FLAG_AUTOCONNECT | PW_STREAM_FLAG_INACTIVE |
            PW_STREAM_FLAG_MAP_BUFFERS | PW_STREAM_FLAG_RT_PROCESS };
        if ( int res{ pw_stream_connect( mStream.get(), PW_DIRECTION_INPUT,
                                         PwIdAny, Flags, params, 1 ) } )
            throw al::backend_exception{
                al::backend_error::DeviceError,
                "Error connecting PipeWire stream (res: %d)", res };
        ...
        mRing = RingBuffer::Create(
            maxu( mDevice->Frequency / 10, mDevice->BufferSize ),
...
bool PipeWireBackendFactory::init() {
    ...
    if ( !GetConfigValueBool( nullptr, "pipewire", "assume-audio", false ) &&
         ...
         */
        WARN(
            "No audio support detected in PipeWire. See the PipeWire options "
            "in alsoftrc.sample if this is wrong.\n" );
        return false;
    }
    return true;
}
...
std::string PipeWireBackendFactory::probe( BackendType type ) {
    ...
    auto sort_devnode = []( DeviceNode& lhs,
                            DeviceNode& rhs ) noexcept -> bool {
    ...
    switch ( type ) {
        case BackendType::Playback:
            ...
            if ( defmatch != devlist.cend() ) {
                /* Includes null char. */
                outnames.append( defmatch->mName.c_str(),
                                 defmatch->mName.length() + 1 );
            }
            for ( auto iter = devlist.cbegin(); iter != devlist.cend();
                  ++iter ) {
                if ( iter != defmatch && iter->mType != NodeType::Source )
                    outnames.append( iter->mName.c_str(),
                                     iter->mName.length() + 1 );
            }
            break;
        case BackendType::Capture:
            defmatch =
                std::find_if( defmatch, devlist.cend(), match_defsource );
            if ( defmatch != devlist.cend() ) {
...
BackendPtr PipeWireBackendFactory::createBackend( DeviceBase* device,
                                                  BackendType type ) {
