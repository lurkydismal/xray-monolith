...
struct OboePlayback final : public BackendBase,
...
bool OboePlayback::reset() {
    ...
    if ( mDevice->Flags.test( ChannelsRequest ) ) {
         ...
         */
        builder.setChannelCount( ( mDevice->FmtChans == DevFmtMono )
    ...
    if ( mDevice->Flags.test( SampleTypeRequest ) ) {
        ...
        switch ( mDevice->FmtType ) {
            ...
            case DevFmtInt:
            case DevFmtUInt:
#if OBOE_VERSION_MAJOR > 1 || \
    ( OBOE_VERSION_MAJOR == 1 && OBOE_VERSION_MINOR >= 6 )
    ...
    mStream->setBufferSizeInFrames(
        mini( static_cast< int32_t >( mDevice->BufferSize ),
              mStream->getBufferCapacityInFrames() ) );
    TRACE( "Got stream with properties:\n%s",
           oboe::convertToText( mStream.get() ) );

    if ( static_cast< uint >( mStream->getChannelCount() ) !=
    ...
    ( OBOE_VERSION_MAJOR == 1 && OBOE_VERSION_MINOR >= 6 )
        ...
        case oboe::AudioFormat::Invalid:
            throw al::backend_exception{
                al::backend_error::DeviceError, "Got unhandled sample type: %s",
                oboe::convertToText( mStream->getFormat() ) };
    }
    mDevice->Frequency = static_cast< uint32_t >( mStream->getSampleRate() );

    /* Ensure the period size is no less than 10ms. It's possible for
     * FramesPerCallback to be 0 indicating variable updates, but OpenAL should
     * have a reasonable minimum update size set. FramesPerBurst may not
     * necessarily be correct, but hopefully it can act as a minimum update
     * size.
     */
    mDevice->UpdateSize =
        maxu( mDevice->Frequency / 100,
              static_cast< uint32_t >( mStream->getFramesPerBurst() ) );
    mDevice->BufferSize =
        maxu( mDevice->UpdateSize * 2,
...
oboe::DataCallbackResult OboeCapture::onAudioReady( oboe::AudioStream*,
                                                    void* audioData,
                                                    int32_t numFrames ) {
...
void OboeCapture::open( const char* name ) {
    ...
    mRing = RingBuffer::Create(
        maxu( mDevice->BufferSize, mDevice->Frequency / 10 ),
...
BackendPtr OboeBackendFactory::createBackend( DeviceBase* device,
                                              BackendType type ) {
