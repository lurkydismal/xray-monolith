...
struct DeviceEntry {
    ...
    DeviceEntry( T&& name, U&& pattern )
        : mName{ std::forward< T >( name ) },
          mPattern{ std::forward< U >( pattern ) } {}
};

al::vector< DeviceEntry > PlaybackList;

void EnumerateDevices( jack_client_t* client,
                       al::vector< DeviceEntry >& list ) {
    std::remove_reference_t< decltype( list ) >{}.swap( list );

    if ( JackPortsPtr ports{ jack_get_ports(
             client, nullptr, JackDefaultAudioType, JackPortIsInput ) } ) {
        for ( size_t i{ 0 }; ports[ i ]; ++i ) {
            ...
            if ( std::find_if( list.cbegin(), list.cend(), check_name ) !=
                 list.cend() )
                continue;
        ...
        if ( ports[ 0 ] && list.empty() ) {
            WARN(
                "No device names found in available ports, adding a generic "
                "name.\n" );
        ...
        for ( size_t strpos{ 0 }; strpos < listopt->size(); ) {
            ...
            if ( seppos >= nextpos || seppos == strpos ) {
                const std::string entry{
                    listopt->substr( strpos, nextpos - strpos ) };
            ...
            const al::span< const char > name{ listopt->data() + strpos,
                                               seppos - strpos };
            const al::span< const char > pattern{
                listopt->data() + ( seppos + 1 ),
                std::min( nextpos, listopt->size() ) - ( seppos + 1 ) };

            /* Check if this custom pattern already exists in the list. */
            auto check_pattern =
                [ pattern ]( const DeviceEntry& entry ) -> bool {
                const size_t len{ pattern.size() };
                return entry.mPattern.length() == len &&
                       entry.mPattern.compare( 0, len, pattern.data(), len ) ==
                           0;
            };
            auto itemmatch =
                std::find_if( list.begin(), list.end(), check_pattern );
            if ( itemmatch != list.end() ) {
                ...
                TRACE( "Customized device name: %s = %s\n",
                       itemmatch->mName.c_str(), itemmatch->mPattern.c_str() );
            } else {
    ...
    if ( list.size() > 1 ) {
         ...
         * etc, as needed.
         */
        for ( auto curitem = list.begin() + 1; curitem != list.end();
              ++curitem ) {
            auto check_match = [ curitem ]( const DeviceEntry& entry ) -> bool {
                return entry.mName == curitem->mName;
            };
            if ( std::find_if( list.begin(), curitem, check_match ) !=
                 curitem ) {
                ...
                } while ( std::find_if( list.begin(), curitem, check_name ) !=
                          curitem );
...
int JackPlayback::processRt( jack_nframes_t numframes ) noexcept {
    ...
    for ( auto port : mPort ) {
        if ( !port || numchans == mDevice->RealOut.Buffer.size() )
            break;
        out[ numchans++ ] =
            static_cast< float* >( jack_port_get_buffer( port, numframes ) );
    }

    if ( mPlaying.load( std::memory_order_acquire ) )
        LIKELY
    mDevice->renderSamples( { out.data(), numchans },
...
int JackPlayback::process( jack_nframes_t numframes ) noexcept {
    ...
    if ( mPlaying.load( std::memory_order_acquire ) )
        LIKELY {
            ...
            jack_nframes_t todo{
                minu( numframes, static_cast< uint >( data.first.len ) ) };
            auto write_first = [ &data, numchans,
                                 todo ]( float* outbuf ) -> float* {
                const float* RESTRICT in =
                    reinterpret_cast< float* >( data.first.buf );
            ...
            std::transform( out.begin(), out.begin() + numchans, out.begin(),
                            write_first );
            total += todo;

            todo = minu( numframes - total,
                         static_cast< uint >( data.second.len ) );
            if ( todo > 0 ) {
                auto write_second = [ &data, numchans,
                                      todo ]( float* outbuf ) -> float* {
                    const float* RESTRICT in =
                        reinterpret_cast< float* >( data.second.buf );
                    auto deinterlace_input = [ &in,
                                               numchans ]() noexcept -> float {
                        float ret{ *in };
                        in += numchans;
                        return ret;
                    };
                ...
                std::transform( out.begin(), out.begin() + numchans,
                                out.begin(), write_second );
                total += todo;
            }
    ...
    if ( numframes > total ) {
        ...
        auto clear_buf = [ todo ]( float* outbuf ) -> void {
...
int JackPlayback::mixerProc() {
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        const auto len2 =
            static_cast< uint >( minz( data.second.len, todo - len1 ) );
...
void JackPlayback::open( const char* name ) {
    if ( !mClient ) {
        ...
        const char* client_name{
            binname.fname.empty() ? "alsoft" : binname.fname.c_str() };

        jack_status_t status;
        mClient =
            jack_client_open( client_name, ClientOptions, &status, nullptr );
        if ( mClient == nullptr )
    ...
    if ( !name && !PlaybackList.empty() ) {
        ...
        auto iter = std::find_if( PlaybackList.cbegin(), PlaybackList.cend(),
                                  check_name );
...
bool JackPlayback::reset() {
    ...
    while ( bad_port != ports_end ) {
        ...
        *bad_port =
            jack_port_register( mClient, name.c_str(), JackDefaultAudioType,
...
void JackPlayback::start() {
    ...
    const char* devname{ mDevice->DeviceName.c_str() };
    if ( ConfigValueBool( devname, "jack", "connect-ports" )
             .value_or( true ) ) {
        JackPortsPtr pnames{ jack_get_ports( mClient, mPortPattern.c_str(),
        ...
        for ( size_t i{ 0 }; i < al::size( mPort ) && mPort[ i ]; ++i ) {
            ...
            if ( jack_connect( mClient, jack_port_name( mPort[ i ] ),
                               pnames[ i ] ) )
                ERR( "Failed to connect output port \"%s\" to \"%s\"\n",
                     jack_port_name( mPort[ i ] ), pnames[ i ] );
        }
    }
    ...
    if ( mRTMixing )
        ...
        mRing =
            RingBuffer::Create( bufsize, mDevice->frameSizeFromFmt(), true );

        try {
            mPlaying.store( true, std::memory_order_release );
            mKillNow.store( false, std::memory_order_release );
            mThread =
                std::thread{ std::mem_fn( &JackPlayback::mixerProc ), this };
...
ClockLatency JackPlayback::getClockLatency() {
    ...
    ret.Latency = std::chrono::seconds{ mRing ? mRing->readSpace()
                                              : mDevice->UpdateSize };
...
bool JackBackendFactory::init() {
    ...
    if ( !GetConfigValueBool( nullptr, "jack", "spawn-server", false ) )
        ClientOptions =
            static_cast< jack_options_t >( ClientOptions | JackNoStartServer );

    const PathNamePair& binname = GetProcBinary();
    const char* client_name{ binname.fname.empty() ? "alsoft"
                                                   : binname.fname.c_str() };

    void ( *old_error_cb )( const char* ){
        &jack_error_callback ? jack_error_callback : nullptr };
...
std::string JackBackendFactory::probe( BackendType type ) {
    ...
    switch ( type ) {
        case BackendType::Playback:
            if ( jack_client_t *
                 client{ jack_client_open( client_name, ClientOptions, &status,
                                           nullptr ) } ) {
...
BackendPtr JackBackendFactory::createBackend( DeviceBase* device,
                                              BackendType type ) {
