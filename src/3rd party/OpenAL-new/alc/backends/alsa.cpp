...
#define snd_pcm_hw_params_set_buffer_time_near \
    psnd_pcm_hw_params_set_buffer_time_near
#define snd_pcm_hw_params_set_period_time_near \
    psnd_pcm_hw_params_set_period_time_near
...
#define snd_pcm_hw_params_set_buffer_size_min \
    psnd_pcm_hw_params_set_buffer_size_min
#define snd_pcm_hw_params_get_buffer_time_min \
    psnd_pcm_hw_params_get_buffer_time_min
#define snd_pcm_hw_params_get_buffer_time_max \
    psnd_pcm_hw_params_get_buffer_time_max
#define snd_pcm_hw_params_get_period_time_min \
    psnd_pcm_hw_params_get_period_time_min
#define snd_pcm_hw_params_get_period_time_max \
    psnd_pcm_hw_params_get_period_time_max
...
struct DevMap {
    ...
    DevMap( T&& name_, U&& devname )
        : name{ std::forward< T >( name_ ) },
          device_name{ std::forward< U >( devname ) } {}
};
...
const char* prefix_name( snd_pcm_stream_t stream ) {
    ...
    return ( stream == SND_PCM_STREAM_PLAYBACK ) ? "device-prefix"
                                                 : "capture-prefix";
}
...
al::vector< DevMap > probe_devices( snd_pcm_stream_t stream ) {
    ...
    if ( auto customdevs = ConfigValueStr( nullptr, "alsa",
        ...
        while ( ( curpos = nextpos ) < customdevs->length() ) {
            ...
            if ( seppos == curpos || seppos >= nextpos ) {
                std::string spec{
                    customdevs->substr( curpos, nextpos - curpos ) };
                ERR( "Invalid ALSA device specification \"%s\"\n",
                     spec.c_str() );
            } else {
                devlist.emplace_back(
                    customdevs->substr( curpos, seppos - curpos ),
    ...
    const std::string main_prefix{
        ConfigValueStr( nullptr, "alsa", prefix_name( stream ) )
            .value_or( "plughw:" ) };
    ...
    for ( ; err >= 0 && card >= 0; err = snd_card_next( &card ) ) {
        ...
        const std::string card_prefix{
            ConfigValueStr( nullptr, "alsa", name.c_str() )
...
int AlsaPlayback::mixerProc() {
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) ) {
        ...
        while ( avail > 0 ) {
            ...
            int err{
                snd_pcm_mmap_begin( mPcmHandle, &areas, &offset, &frames ) };
            if ( err < 0 ) {
                ERR( "mmap begin error: %s\n", snd_strerror( err ) );
                break;
            }

            char* WritePtr{ static_cast< char* >( areas->addr ) +
                            ( offset * areas->step / 8 ) };
            mDevice->renderSamples( WritePtr, static_cast< uint >( frames ),
                                    mFrameStep );

            snd_pcm_sframes_t commitres{
                snd_pcm_mmap_commit( mPcmHandle, offset, frames ) };
            if ( commitres < 0 ||
                 static_cast< snd_pcm_uframes_t >( commitres ) != frames ) {
                ERR( "mmap commit error: %s\n",
                     snd_strerror( commitres >= 0
                                       ? -EPIPE
...
int AlsaPlayback::mixerNoMMapProc() {
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) ) {
        ...
        avail = snd_pcm_bytes_to_frames(
            mPcmHandle, static_cast< ssize_t >( mBuffer.size() ) );
        std::lock_guard< std::mutex > _{ mMutex };
        mDevice->renderSamples( WritePtr, static_cast< uint >( avail ),
                                mFrameStep );
        while ( avail > 0 ) {
            snd_pcm_sframes_t ret{
                snd_pcm_writei( mPcmHandle, WritePtr,
            ...
            switch ( ret ) {
                ...
                case -EPIPE:
                case -EINTR:
                    ret = snd_pcm_recover( mPcmHandle,
...
void AlsaPlayback::open( const char* name ) {
    ...
    snd_pcm_t* pcmHandle{};
    int err{ snd_pcm_open( &pcmHandle, driver.c_str(), SND_PCM_STREAM_PLAYBACK,
                           SND_PCM_NONBLOCK ) };
    if ( err < 0 )
        throw al::backend_exception{ al::backend_error::NoDevice,
                                     "Could not open ALSA device \"%s\"",
                                     driver.c_str() };
    if ( mPcmHandle )
        snd_pcm_close( mPcmHandle );
    mPcmHandle = pcmHandle;

    /* Free alsa's global config tree. Otherwise valgrind reports a ton of
     * leaks. */
...
bool AlsaPlayback::reset() {
    ...
    bool allowmmap{ !!GetConfigValueBool( mDevice->DeviceName.c_str(), "alsa",
                                          "mmap", true ) };
    uint periodLen{ static_cast< uint >( mDevice->UpdateSize * 1000000_u64 /
                                         mDevice->Frequency ) };
    uint bufferLen{ static_cast< uint >( mDevice->BufferSize * 1000000_u64 /
                                         mDevice->Frequency ) };
...
#define CHECK( x )                                                       \
    ...
    if ( !allowmmap ||
         snd_pcm_hw_params_set_access( mPcmHandle, hp.get(),
                                       SND_PCM_ACCESS_MMAP_INTERLEAVED ) < 0 ) {
        /* No mmap */
        CHECK( snd_pcm_hw_params_set_access( mPcmHandle, hp.get(),
    ...
    if ( snd_pcm_hw_params_test_format( mPcmHandle, hp.get(), format ) < 0 ) {
        ...
        for ( const auto& fmt : formatlist ) {
            format = fmt.format;
            if ( snd_pcm_hw_params_test_format( mPcmHandle, hp.get(),
                                                format ) >= 0 ) {
    ...
    if ( snd_pcm_hw_params_set_channels( mPcmHandle, hp.get(),
                                         mDevice->channelsFromFmt() ) < 0 ) {
        uint numchans{ 2u };
        CHECK( snd_pcm_hw_params_set_channels_near( mPcmHandle, hp.get(),
                                                    &numchans ) );
        if ( numchans < 1 )
            throw al::backend_exception{ al::backend_error::DeviceError,
    ...
    if ( !GetConfigValueBool( mDevice->DeviceName.c_str(), "alsa",
                              "allow-resampler", false ) ||
         !mDevice->Flags.test( FrequencyRequest ) ) {
        if ( snd_pcm_hw_params_set_rate_resample( mPcmHandle, hp.get(), 0 ) <
             0 )
            WARN( "Failed to disable ALSA resampler\n" );
    } else if ( snd_pcm_hw_params_set_rate_resample( mPcmHandle, hp.get(), 1 ) <
                0 )
        WARN( "Failed to enable ALSA resampler\n" );
    CHECK( snd_pcm_hw_params_set_rate_near( mPcmHandle, hp.get(), &rate,
                                            nullptr ) );
    /* set period time (implicitly constrains period/buffer parameters) */
    if ( ( err = snd_pcm_hw_params_set_period_time_near(
               mPcmHandle, hp.get(), &periodLen, nullptr ) ) < 0 )
        ERR( "snd_pcm_hw_params_set_period_time_near failed: %s\n",
             snd_strerror( err ) );
    /* set buffer time (implicitly sets buffer size/bytes/time and period
     * size/bytes) */
    if ( ( err = snd_pcm_hw_params_set_buffer_time_near(
               mPcmHandle, hp.get(), &bufferLen, nullptr ) ) < 0 )
    ...
    CHECK( snd_pcm_hw_params_get_period_size( hp.get(), &periodSizeInFrames,
                                              nullptr ) );
    ...
    CHECK( snd_pcm_sw_params_set_avail_min( mPcmHandle, sp.get(),
                                            periodSizeInFrames ) );
    CHECK( snd_pcm_sw_params_set_stop_threshold( mPcmHandle, sp.get(),
                                                 bufferSizeInFrames ) );
...
#define CHECK( x )                                                       \
    ...
    if ( access == SND_PCM_ACCESS_RW_INTERLEAVED ) {
        auto datalen =
            snd_pcm_frames_to_bytes( mPcmHandle, mDevice->UpdateSize );
...
void AlsaPlayback::stop() {
    if ( mKillNow.exchange( true, std::memory_order_acq_rel ) ||
         !mThread.joinable() )
        return;
    mThread.join();
...
ClockLatency AlsaPlayback::getClockLatency() {
    ...
    ret.Latency =
        std::chrono::seconds{ std::max< snd_pcm_sframes_t >( 0, delay ) };
...
void AlsaCapture::open( const char* name ) {
    ...
    TRACE( "Opening device \"%s\"\n", driver.c_str() );
    int err{ snd_pcm_open( &mPcmHandle, driver.c_str(), SND_PCM_STREAM_CAPTURE,
                           SND_PCM_NONBLOCK ) };
    if ( err < 0 )
        throw al::backend_exception{ al::backend_error::NoDevice,
                                     "Could not open ALSA device \"%s\"",
    ...
    snd_pcm_uframes_t periodSizeInFrames{
        minu( mDevice->BufferSize, 25 * mDevice->Frequency / 1000 ) };
...
#define CHECK( x )                                                       \
    ...
    CHECK( snd_pcm_hw_params_set_format( mPcmHandle, hp.get(), format ) );
    /* set channels (implicitly sets frame bits) */
    CHECK( snd_pcm_hw_params_set_channels( mPcmHandle, hp.get(),
                                           mDevice->channelsFromFmt() ) );
    /* set rate (implicitly constrains period/buffer parameters) */
    CHECK( snd_pcm_hw_params_set_rate( mPcmHandle, hp.get(), mDevice->Frequency,
                                       0 ) );
    /* set buffer size in frame units (implicitly sets period size/bytes/time
     * and buffer time/bytes) */
    if ( snd_pcm_hw_params_set_buffer_size_min( mPcmHandle, hp.get(),
                                                &bufferSizeInFrames ) < 0 ) {
        TRACE( "Buffer too large, using intermediate ring buffer\n" );
        needring = true;
        CHECK( snd_pcm_hw_params_set_buffer_size_near( mPcmHandle, hp.get(),
                                                       &bufferSizeInFrames ) );
    }
    /* set buffer size in frame units (implicitly sets period size/bytes/time
     * and buffer time/bytes) */
    CHECK( snd_pcm_hw_params_set_period_size_near(
        mPcmHandle, hp.get(), &periodSizeInFrames, nullptr ) );
    /* install and prepare hardware configuration */
    CHECK( snd_pcm_hw_params( mPcmHandle, hp.get() ) );
    /* retrieve configuration info */
    CHECK( snd_pcm_hw_params_get_period_size( hp.get(), &periodSizeInFrames,
...
#undef CHECK
    ...
    if ( needring )
        mRing = RingBuffer::Create( mDevice->BufferSize,
...
void AlsaCapture::stop() {
    ...
    if ( !mRing && avail > 0 ) {
         ...
         */
        auto temp = al::vector< al::byte >( static_cast< size_t >(
            snd_pcm_frames_to_bytes( mPcmHandle, avail ) ) );
...
void AlsaCapture::captureSamples( al::byte* buffer, uint samples ) {
    ...
    mLastAvail -= samples;
    while ( mDevice->Connected.load( std::memory_order_acquire ) &&
            samples > 0 ) {
        snd_pcm_sframes_t amt{ 0 };

        if ( !mBuffer.empty() ) {
            /* First get any data stored from the last stop */
            amt = snd_pcm_bytes_to_frames(
                mPcmHandle, static_cast< ssize_t >( mBuffer.size() ) );
        ...
        if ( amt < 0 ) {
            ...
            if ( amt == -EAGAIN )
                continue;
            if ( ( amt = snd_pcm_recover( mPcmHandle, static_cast< int >( amt ),
                                          1 ) ) >= 0 ) {
            ...
            if ( amt < 0 ) {
                ...
                ERR( "restore error: %s\n", err );
                mDevice->handleDisconnect( "Capture recovery failure: %s",
                                           err );
                break;
            }
    ...
    if ( samples > 0 )
        std::fill_n(
            buffer, snd_pcm_frames_to_bytes( mPcmHandle, samples ),
...
uint AlsaCapture::availableSamples() {
    ...
    if ( avail < 0 ) {
        ...
        if ( ( avail = snd_pcm_recover( mPcmHandle, static_cast< int >( avail ),
                                        1 ) ) >= 0 ) {
            if ( mDoCapture )
    ...
    while ( avail > 0 ) {
        ...
        snd_pcm_sframes_t amt{ std::min(
            static_cast< snd_pcm_sframes_t >( vec.first.len ), avail ) };
        amt = snd_pcm_readi( mPcmHandle, vec.first.buf,
                             static_cast< snd_pcm_uframes_t >( amt ) );
        if ( amt < 0 ) {
            ERR( "read error: %s\n",
                 snd_strerror( static_cast< int >( amt ) ) );
...
BackendPtr AlsaBackendFactory::createBackend( DeviceBase* device,
                                              BackendType type ) {
