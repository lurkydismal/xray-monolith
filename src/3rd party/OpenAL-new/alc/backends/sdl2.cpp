...
#define DEVNAME_PREFIX ""
    ...
    void Sdl2Backend::audioCallback( Uint8 * stream, int len ) noexcept {
        ...
        mDevice->renderSamples( stream, ulen / mFrameSize,
    ...
    void Sdl2Backend::open( const char* name ) {
        ...
        want.samples =
            static_cast< Uint16 >( minu( mDevice->UpdateSize, 8192 ) );
        ...
        if ( !name || strcmp( name, defaultDeviceName ) == 0 )
            devid = SDL_OpenAudioDevice( nullptr, SDL_FALSE, &want, &have,
                                         SDL_AUDIO_ALLOW_ANY_CHANGE );
        else {
            const size_t prefix_len = strlen( DEVNAME_PREFIX );
            if ( strncmp( name, DEVNAME_PREFIX, prefix_len ) == 0 )
                devid =
                    SDL_OpenAudioDevice( name + prefix_len, SDL_FALSE, &want,
                                         &have, SDL_AUDIO_ALLOW_ANY_CHANGE );
            else
                devid = SDL_OpenAudioDevice( name, SDL_FALSE, &want, &have,
                                             SDL_AUDIO_ALLOW_ANY_CHANGE );
        }
        if ( !devid )
            throw al::backend_exception{ al::backend_error::NoDevice, "%s",
                                         SDL_GetError() };
    ...
    bool Sdl2Backend::reset() {
        ...
        mDevice->BufferSize =
            mUpdateSize * 2; /* SDL always (tries to) use two periods. */
...
BackendPtr SDL2BackendFactory::createBackend( DeviceBase* device,
                                              BackendType type ) {
    if ( type == BackendType::Playback )
