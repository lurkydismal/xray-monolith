...
OSStatus GetHwProperty( AudioHardwarePropertyID propId,
                        UInt32 dataSize,
                        void* propData ) {
    const AudioObjectPropertyAddress addr{ propId,
                                           kAudioObjectPropertyScopeGlobal,
                                           kAudioObjectPropertyElementMaster };
    return AudioObjectGetPropertyData( kAudioObjectSystemObject, &addr, 0,
                                       nullptr, &dataSize, propData );
}
...
OSStatus GetHwPropertySize( AudioHardwarePropertyID propId, UInt32* outSize ) {
    const AudioObjectPropertyAddress addr{ propId,
                                           kAudioObjectPropertyScopeGlobal,
                                           kAudioObjectPropertyElementMaster };
    return AudioObjectGetPropertyDataSize( kAudioObjectSystemObject, &addr, 0,
                                           nullptr, outSize );
}

OSStatus GetDevProperty( AudioDeviceID devId,
                         AudioDevicePropertyID propId,
                         bool isCapture,
                         UInt32 elem,
                         UInt32 dataSize,
                         void* propData ) {
    static const AudioObjectPropertyScope scopes[ 2 ]{
    ...
    return AudioObjectGetPropertyData( devId, &addr, 0, nullptr, &dataSize,
                                       propData );
}

OSStatus GetDevPropertySize( AudioDeviceID devId,
    ...
    static const AudioObjectPropertyScope scopes[ 2 ]{
        kAudioDevicePropertyScopeOutput, kAudioDevicePropertyScopeInput };
    const AudioObjectPropertyAddress addr{ inPropertyID, scopes[ isCapture ],
                                           elem };
...
std::string GetDeviceName( AudioDeviceID devId ) {
    ...
    OSStatus err{ GetDevProperty( devId, kAudioDevicePropertyDeviceNameCFString,
                                  false, 0, sizeof( nameRef ), &nameRef ) };
    if ( err == noErr ) {
        const CFIndex propSize{ CFStringGetMaximumSizeForEncoding(
            CFStringGetLength( nameRef ), kCFStringEncodingUTF8 ) };
        devname.resize( static_cast< size_t >( propSize ) + 1, '\0' );

        CFStringGetCString( nameRef, &devname[ 0 ], propSize + 1,
         ...
         */
        UInt32 propSize{};
        if ( GetDevPropertySize( devId, kAudioDevicePropertyDeviceName, false,
                                 0, &propSize ) )
            return devname;

        devname.resize( propSize + 1, '\0' );
        if ( GetDevProperty( devId, kAudioDevicePropertyDeviceName, false, 0,
                             propSize, &devname[ 0 ] ) ) {
            devname.clear();
            return devname;
        }
...
UInt32 GetDeviceChannelCount( AudioDeviceID devId, bool isCapture ) {
    UInt32 propSize{};
    auto err =
        GetDevPropertySize( devId, kAudioDevicePropertyStreamConfiguration,
                            isCapture, 0, &propSize );
    if ( err ) {
    ...
    err = GetDevProperty( devId, kAudioDevicePropertyStreamConfiguration,
                          isCapture, 0, propSize, buflist );
    if ( err ) {
...
void EnumerateDevices( std::vector< DeviceEntry >& list, bool isCapture ) {
    UInt32 propSize{};
    if ( auto err =
             GetHwPropertySize( kAudioHardwarePropertyDevices, &propSize ) ) {
        ERR( "Failed to get device list size: %u\n", err );
        return;
    }

    auto devIds = std::vector< AudioDeviceID >(
        propSize / sizeof( AudioDeviceID ), kAudioDeviceUnknown );
    if ( auto err = GetHwProperty( kAudioHardwarePropertyDevices, propSize,
                                   devIds.data() ) ) {
    ...
    GetHwProperty( isCapture ? kAudioHardwarePropertyDefaultInputDevice
                   ...
                   sizeof( defaultId ), &defaultId );

    if ( defaultId != kAudioDeviceUnknown ) {
        newdevs.emplace_back(
            DeviceEntry{ defaultId, GetDeviceName( defaultId ) } );
    ...
    for ( const AudioDeviceID devId : devIds ) {
        ...
        auto match_devid =
            [ devId ]( const DeviceEntry& entry ) noexcept -> bool {
            return entry.mId == devId;
        };
        auto match =
            std::find_if( newdevs.cbegin(), newdevs.cend(), match_devid );
        if ( match != newdevs.cend() )
            continue;

        auto numChannels = GetDeviceChannelCount( devId, isCapture );
        if ( numChannels > 0 ) {
            newdevs.emplace_back(
                DeviceEntry{ devId, GetDeviceName( devId ) } );
            const auto& entry = newdevs.back();
            TRACE( "Got device: %s = ID %u\n", entry.mName.c_str(), entry.mId );
        }
    }
    ...
    if ( newdevs.size() > 1 ) {
         ...
         * etc, as needed.
         */
        for ( auto curitem = newdevs.begin() + 1; curitem != newdevs.end();
              ++curitem ) {
            auto check_match = [ curitem ]( const DeviceEntry& entry ) -> bool {
                return entry.mName == curitem->mName;
            };
            if ( std::find_if( newdevs.begin(), curitem, check_match ) !=
                 curitem ) {
                std::string name{ curitem->mName };
                size_t count{ 1 };
                auto check_name =
                    [ &name ]( const DeviceEntry& entry ) -> bool {
                ...
                } while ( std::find_if( newdevs.begin(), curitem,
                                        check_name ) != curitem );
...
struct CoreAudioPlayback final : public BackendBase {
    ...
    static OSStatus MixerProcC( void* inRefCon,
                                ...
                                UInt32 inNumberFrames,
                                AudioBufferList* ioData ) noexcept {
        return static_cast< CoreAudioPlayback* >( inRefCon )
            ->MixerProc( ioActionFlags, inTimeStamp, inBusNumber,
    ...
    AudioStreamBasicDescription
        mFormat{}; // This is the OpenAL format as a CoreAudio ASBD
...
void CoreAudioPlayback::open( const char* name ) {
    ...
    if ( !name )
        GetHwProperty( kAudioHardwarePropertyDefaultOutputDevice,
                       sizeof( audioDevice ), &audioDevice );
    else {
        ...
        auto devmatch = std::find_if( PlaybackList.cbegin(),
                                      PlaybackList.cend(), find_name );
    ...
    if ( audioDevice != kAudioDeviceUnknown )
        AudioUnitSetProperty( audioUnit, kAudioOutputUnitProperty_CurrentDevice,
                              kAudioUnitScope_Global, OutputElement,
                              &audioDevice, sizeof( AudioDeviceID ) );
#endif

    err = AudioUnitInitialize( audioUnit );
    if ( err != noErr )
        throw al::backend_exception{ al::backend_error::DeviceError,
                                     "Could not initialize audio unit: %u",
                                     err };
...
bool CoreAudioPlayback::reset() {
    ...
    err = AudioUnitGetProperty( mAudioUnit, kAudioUnitProperty_StreamFormat,
                                kAudioUnitScope_Output, OutputElement,
                                &streamFormat, &size );
    ...
    if ( mDevice->Frequency != streamFormat.mSampleRate ) {
        mDevice->BufferSize = static_cast< uint >(
            mDevice->BufferSize * streamFormat.mSampleRate /
                mDevice->Frequency +
            0.5 );
    ...
    err = AudioUnitSetProperty( mAudioUnit, kAudioUnitProperty_StreamFormat,
                                kAudioUnitScope_Input, OutputElement,
    ...
    err = AudioUnitSetProperty(
        mAudioUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input,
        OutputElement, &input, sizeof( AURenderCallbackStruct ) );
    if ( err != noErr ) {
        ERR( "AudioUnitSetProperty failed\n" );
        return false;
    }
...
struct CoreAudioCapture final : public BackendBase {
    ...
    static OSStatus RecordProcC( void* inRefCon,
                                 ...
                                 UInt32 inNumberFrames,
                                 AudioBufferList* ioData ) noexcept {
        return static_cast< CoreAudioCapture* >( inRefCon )
            ->RecordProc( ioActionFlags, inTimeStamp, inBusNumber,
                          inNumberFrames, ioData );
    }
...
OSStatus CoreAudioCapture::RecordProc(
    AudioUnitRenderActionFlags* ioActionFlags,
    const AudioTimeStamp* inTimeStamp,
    UInt32 inBusNumber,
    UInt32 inNumberFrames,
    ...
    union {
        al::byte _[ maxz( sizeof( AudioBufferList ),
                          offsetof( AudioBufferList, mBuffers[ 1 ] ) ) ];
    ...
    audiobuf.list.mBuffers[ 0 ].mDataByteSize =
        static_cast< UInt32 >( mCaptureData.size() );

    OSStatus err{ AudioUnitRender( mAudioUnit, ioActionFlags, inTimeStamp,
...
void CoreAudioCapture::open( const char* name ) {
    ...
    AudioDeviceID audioDevice{ kAudioDeviceUnknown };
    if ( !name )
        GetHwProperty( kAudioHardwarePropertyDefaultInputDevice,
                       sizeof( audioDevice ), &audioDevice );
    else {
        ...
        auto devmatch =
            std::find_if( CaptureList.cbegin(), CaptureList.cend(), find_name );
    ...
    enableIO = 1;
    err = AudioUnitSetProperty( mAudioUnit, kAudioOutputUnitProperty_EnableIO,
                                kAudioUnitScope_Input, InputElement, &enableIO,
                                sizeof( enableIO ) );
    if ( err != noErr )
        throw al::backend_exception{
            al::backend_error::DeviceError,
    ...
    if ( audioDevice != kAudioDeviceUnknown )
        AudioUnitSetProperty( mAudioUnit,
                              ...
                              kAudioUnitScope_Global, InputElement,
                              &audioDevice, sizeof( AudioDeviceID ) );
#endif
    ...
    if ( err != noErr )
        throw al::backend_exception{ al::backend_error::DeviceError,
                                     "Could not set capture callback: %u",
                                     err };

    // Disable buffer allocation for capture
    UInt32 flag{ 0 };
    err = AudioUnitSetProperty(
        mAudioUnit, kAudioUnitProperty_ShouldAllocateBuffer,
    ...
    err = AudioUnitGetProperty( mAudioUnit, kAudioUnitProperty_StreamFormat,
                                kAudioUnitScope_Input, InputElement,
                                &hardwareFormat, &propertySize );
    ...
    switch ( mDevice->FmtType ) {
        ...
        case DevFmtFloat:
            requestedFormat.mBitsPerChannel = 32;
            requestedFormat.mFormatFlags = kLinearPCMFormatFlagIsFloat |
                                           kAudioFormatFlagsNativeEndian |
                                           kAudioFormatFlagIsPacked;
            break;
    }
    ...
    switch ( mDevice->FmtChans ) {
        ...
        case DevFmtAmbi3D:
            throw al::backend_exception{
                al::backend_error::DeviceError, "%s not supported",
                DevFmtChannelsString( mDevice->FmtChans ) };
    }

    requestedFormat.mBytesPerFrame =
        requestedFormat.mChannelsPerFrame * requestedFormat.mBitsPerChannel / 8;
    ...
    // The output format should be the requested format, but using the hardware
    // sample rate This is because the AudioUnit will automatically scale other
    // properties, except for sample rate
    err = AudioUnitSetProperty( mAudioUnit, kAudioUnitProperty_StreamFormat,
                                kAudioUnitScope_Output, InputElement,
    ...
    auto FrameCount64 =
        maxu64( static_cast< uint64_t >(
                    std::ceil( mDevice->BufferSize * srateScale ) ),
    ...
    err = AudioUnitGetProperty( mAudioUnit,
                                ...
                                kAudioUnitScope_Global, OutputElement,
                                &outputFrameCount, &propertySize );
    if ( err != noErr || propertySize != sizeof( outputFrameCount ) )
        throw al::backend_exception{ al::backend_error::DeviceError,
                                     "Could not get input frame count: %u",
                                     err };

    mCaptureData.resize( outputFrameCount * mFrameSize );

    outputFrameCount =
        static_cast< UInt32 >( maxu64( outputFrameCount, FrameCount64 ) );
    mRing = RingBuffer::Create( outputFrameCount, mFrameSize, false );

    /* Set up sample converter if needed */
    if ( outputFormat.mSampleRate != mDevice->Frequency )
        mConverter = SampleConverter::Create(
            mDevice->FmtType, mDevice->FmtType, mFormat.mChannelsPerFrame,
            static_cast< uint >( hardwareFormat.mSampleRate ),
    ...
    if ( name )
        ...
        AudioUnitGetProperty(
            mAudioUnit, kAudioOutputUnitProperty_CurrentDevice,
...
void CoreAudioCapture::captureSamples( al::byte* buffer, uint samples ) {
    ...
    if ( got < samples && !src0len && rec_vec.second.len > 0 ) {
        ...
        got += mConverter->convert( &src1, &src1len, buffer + got * mFrameSize,
                                    samples - got );
...
uint CoreAudioCapture::availableSamples() {
    ...
    return mConverter->availableOut(
        static_cast< uint >( mRing->readSpace() ) );
}
...
std::string CoreAudioBackendFactory::probe( BackendType type ) {
    ...
    switch ( type ) {
        case BackendType::Playback:
            EnumerateDevices( PlaybackList, false );
            std::for_each( PlaybackList.cbegin(), PlaybackList.cend(),
                           append_name );
            break;
        case BackendType::Capture:
            EnumerateDevices( CaptureList, true );
            std::for_each( CaptureList.cbegin(), CaptureList.cend(),
                           append_name );
            break;
    }
...
BackendPtr CoreAudioBackendFactory::createBackend( DeviceBase* device,
                                                   BackendType type ) {
