...
bool checkName( const al::vector< std::string >& list,
...
void ProbePlaybackDevices( void ) {
    ...
    for ( UINT i{ 0 }; i < numdevs; ++i ) {
        ...
        if ( waveOutGetDevCapsW( i, &WaveCaps, sizeof( WaveCaps ) ) ==
             MMSYSERR_NOERROR ) {
            const std::string basename{ DEVNAME_HEAD +
                                        wstr_to_utf8( WaveCaps.szPname ) };
...
void ProbeCaptureDevices( void ) {
    ...
    for ( UINT i{ 0 }; i < numdevs; ++i ) {
        ...
        if ( waveInGetDevCapsW( i, &WaveCaps, sizeof( WaveCaps ) ) ==
             MMSYSERR_NOERROR ) {
            const std::string basename{ DEVNAME_HEAD +
                                        wstr_to_utf8( WaveCaps.szPname ) };
...
struct WinMMPlayback final : public BackendBase {
    ...
    static void CALLBACK waveOutProcC( HWAVEOUT device,
                                       ...
                                       DWORD_PTR param1,
                                       DWORD_PTR param2 ) noexcept {
        reinterpret_cast< WinMMPlayback* >( instance )
            ->waveOutProc( device, msg, param1, param2 );
    }
 ...
 */
void CALLBACK WinMMPlayback::waveOutProc( HWAVEOUT,
                                          UINT msg,
                                          DWORD_PTR,
                                          DWORD_PTR ) noexcept {
    if ( msg != WOM_DONE )
...
FORCE_ALIGN int WinMMPlayback::mixerProc() {
    ...
    while ( !mKillNow.load( std::memory_order_acquire ) &&
        ...
        do {
            ...
            if ( ++widx == mWaveBuffer.size() )
                widx = 0;

            mDevice->renderSamples( waveHdr.lpData, mDevice->UpdateSize,
                                    mFormat.nChannels );
...
void WinMMPlayback::open( const char* name ) {
    ...
    auto iter = name ? std::find( PlaybackDevices.cbegin(),
                                  PlaybackDevices.cend(), name )
                     : PlaybackDevices.cbegin();
    if ( iter == PlaybackDevices.cend() )
        throw al::backend_exception{ al::backend_error::NoDevice,
                                     "Device name \"%s\" not found", name };
    auto DeviceID =
        static_cast< UINT >( std::distance( PlaybackDevices.cbegin(), iter ) );
...
retry_open:
    ...
    format.nBlockAlign =
        static_cast< WORD >( format.wBitsPerSample * format.nChannels / 8 );
...
bool WinMMPlayback::reset() {
    mDevice->BufferSize =
        static_cast< uint >( uint64_t{ mDevice->BufferSize } *
    ...
    uint BufferSize{ mDevice->UpdateSize * mFormat.nChannels *
                     mDevice->bytesFromFmt() };

    al_free( mWaveBuffer[ 0 ].lpData );
    mWaveBuffer[ 0 ] = WAVEHDR{};
    mWaveBuffer[ 0 ].lpData = static_cast< char* >(
        al_calloc( 16, BufferSize * mWaveBuffer.size() ) );
    mWaveBuffer[ 0 ].dwBufferLength = BufferSize;
    for ( size_t i{ 1 }; i < mWaveBuffer.size(); i++ ) {
        mWaveBuffer[ i ] = WAVEHDR{};
        mWaveBuffer[ i ].lpData =
            mWaveBuffer[ i - 1 ].lpData + mWaveBuffer[ i - 1 ].dwBufferLength;
...
void WinMMPlayback::start() {
    try {
        ...
        mWritable.store( static_cast< uint >( mWaveBuffer.size() ),
...
struct WinMMCapture final : public BackendBase {
    ...
    static void CALLBACK waveInProcC( HWAVEIN device,
                                      ...
                                      DWORD_PTR param1,
                                      DWORD_PTR param2 ) noexcept {
        reinterpret_cast< WinMMCapture* >( instance )
            ->waveInProc( device, msg, param1, param2 );
    }
 ...
 */
void CALLBACK WinMMCapture::waveInProc( HWAVEIN,
                                        UINT msg,
                                        DWORD_PTR,
                                        DWORD_PTR ) noexcept {
    if ( msg != WIM_DATA )
...
void WinMMCapture::open( const char* name ) {
    ...
    // Find the Device ID matching the deviceName if valid
    auto iter =
        name ? std::find( CaptureDevices.cbegin(), CaptureDevices.cend(), name )
             : CaptureDevices.cbegin();
    if ( iter == CaptureDevices.cend() )
        throw al::backend_exception{ al::backend_error::NoDevice,
                                     "Device name \"%s\" not found", name };
    auto DeviceID =
        static_cast< UINT >( std::distance( CaptureDevices.cbegin(), iter ) );
    ...
    mFormat.nBlockAlign =
        static_cast< WORD >( mFormat.wBitsPerSample * mFormat.nChannels / 8 );
    mFormat.nSamplesPerSec = mDevice->Frequency;
    mFormat.nAvgBytesPerSec = mFormat.nSamplesPerSec * mFormat.nBlockAlign;
    mFormat.cbSize = 0;

    MMRESULT res{ waveInOpen(
        &mInHdl, DeviceID, &mFormat,
    ...
    CapturedDataSize = static_cast< uint >(
        maxz( CapturedDataSize, BufferSize * mWaveBuffer.size() ) );
...
void WinMMCapture::start() {
    try {
        ...
        mKillNow.store( false, std::memory_order_release );
        mThread =
            std::thread{ std::mem_fn( &WinMMCapture::captureProc ), this };
...
BackendPtr WinMMBackendFactory::createBackend( DeviceBase* device,
                                               BackendType type ) {
