...
static_assert( HilSize % OversampleFactor == 0,
...
void FshifterState::update( const ContextBase* context,
                            const EffectSlot* slot,
                            const EffectProps* props,
                            const EffectTarget target ) {
    const DeviceBase* device{ context->mDevice };

    const float step{ props->Fshifter.Frequency /
                      static_cast< float >( device->Frequency ) };
    mPhaseStep[ 0 ] = mPhaseStep[ 1 ] =
        fastf2u( minf( step, 1.0f ) * MixerFracOne );
...
void FshifterState::process( const size_t samplesToDo,
    ...
    for ( size_t base{ 0u }; base < samplesToDo; ) {
        ...
        for ( size_t dst{ mPos }, k{ 0u }; dst < HilSize; ++dst, ++k )
            mOutputAccum[ dst ] +=
                2.0 / OversampleFactor * gWindow.mData[ k ] * mAnalytic[ k ];
    ...
    for ( size_t c{ 0 }; c < 2; ++c ) {
        ...
        for ( size_t k{ 0 }; k < samplesToDo; ++k ) {
            const double phase{ phase_idx *
                                ( al::numbers::pi * 2.0 / MixerFracOne ) };
            BufferOut[ k ] = static_cast< float >(
                mOutdata[ k ].real() * std::cos( phase ) +
        ...
        MixSamples( { BufferOut, samplesToDo }, samplesOut, mGains[ c ].Current,
                    mGains[ c ].Target, maxz( samplesToDo, 512 ), 0 );
    }
}
