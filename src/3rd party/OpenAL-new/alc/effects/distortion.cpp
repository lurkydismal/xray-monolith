...
void DistortionState::update( const ContextBase* context,
                              const EffectSlot* slot,
                              const EffectProps* props,
    ...
    const float edge{ minf(
        std::sin( al::numbers::pi_v< float > * 0.5f * props->Distortion.Edge ),
        0.99f ) };
    ...
    mLowpass.setParamsFromBandwidth(
        BiquadType::LowPass, cutoff / frequency / 4.0f, 1.0f, bandwidth );

    cutoff = props->Distortion.EQCenter;
    /* Convert bandwidth in Hz to octaves. */
    bandwidth = props->Distortion.EQBandwidth / ( cutoff * 0.67f );
    mBandpass.setParamsFromBandwidth(
        BiquadType::BandPass, cutoff / frequency / 4.0f, 1.0f, bandwidth );

    static constexpr auto coeffs = CalcDirectionCoeffs( { 0.0f, 0.0f, -1.0f } );

    mOutTarget = target.Main->Buffer;
    ComputePanGains( target.Main, coeffs.data(),
                     slot->Gain * props->Distortion.Gain, mGain );
}

void DistortionState::process(
    ...
    for ( size_t base{ 0u }; base < samplesToDo; ) {
        ...
        for ( size_t i{ 0u }; i < todo; i++ )
            mBuffer[ 0 ][ i ] =
                !( i & 3 ) ? samplesIn[ 0 ][ ( i >> 2 ) + base ] * 4.0f : 0.0f;
        ...
        std::transform( std::begin( mBuffer[ 1 ] ),
                        std::begin( mBuffer[ 1 ] ) + todo,
                        std::begin( mBuffer[ 0 ] ), proc_sample );
