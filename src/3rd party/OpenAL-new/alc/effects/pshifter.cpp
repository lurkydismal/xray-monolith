...
static_assert( StftSize % OversampleFactor == 0,
               "Factor must be a clean divisor of the size" );
...
struct Windower {
    ...
    Windower() {
        ...
        for ( size_t i{ 0 }; i < StftHalfSize; i++ ) {
            ...
            const double val{
                std::sin( ( static_cast< double >( i ) + 0.5 ) * scale ) };
            mData[ i ] = mData[ StftSize - 1 - i ] =
                static_cast< float >( val * val );
        }
    }
};
...
void PshifterState::update( const ContextBase*,
    ...
    const int tune{ props->Pshifter.CoarseTune * 100 +
                    props->Pshifter.FineTune };
    const float pitch{
        std::pow( 2.0f, static_cast< float >( tune ) / 1200.0f ) };
    mPitchShiftI = clampu( fastf2u( pitch * MixerFracOne ), MixerFracHalf,
                           MixerFracOne * 2 );
    mPitchShift =
        static_cast< float >( mPitchShiftI ) * float{ 1.0f / MixerFracOne };
...
void PshifterState::process( const size_t samplesToDo,
     ...
     */
    constexpr float expected_cycles{ al::numbers::pi_v< float > * 2.0f /
                                     OversampleFactor };
    ...
    for ( size_t base{ 0u }; base < samplesToDo; ) {
        ...
        for ( size_t k{ 0u }; k < StftHalfSize + 1; k++ ) {
            ...
            const auto bin_offset =
                static_cast< float >( k % OversampleFactor );
            float tmp{ ( phase - mLastPhase[ k ] ) -
                       bin_offset * expected_cycles };
         ...
         */
        std::fill( mSynthesisBuffer.begin(), mSynthesisBuffer.end(),
                   FrequencyBin{} );

        constexpr size_t bin_limit{ ( ( StftHalfSize + 1 ) << MixerFracBits ) -
                                    MixerFracHalf - 1 };
        const size_t bin_count{
            minz( StftHalfSize + 1, bin_limit / mPitchShiftI + 1 ) };
        for ( size_t k{ 0u }; k < bin_count; k++ ) {
            const size_t j{ ( k * mPitchShiftI + MixerFracHalf ) >>
                            MixerFracBits };
             ...
             */
            if ( mAnalysisBuffer[ k ].Magnitude >
                 mSynthesisBuffer[ j ].Magnitude )
                mSynthesisBuffer[ j ].FreqBin =
                    mAnalysisBuffer[ k ].FreqBin * mPitchShift;
        ...
        for ( size_t k{ 0u }; k < StftHalfSize + 1; k++ ) {
             ...
             */
            float tmp{ mSumPhase[ k ] +
                       mSynthesisBuffer[ k ].FreqBin * expected_cycles };
            ...
            mFftBuffer[ k ] =
                std::polar( mSynthesisBuffer[ k ].Magnitude, mSumPhase[ k ] );
        }
        ...
        for ( size_t dst{ mPos }, k{ 0u }; dst < StftSize; ++dst, ++k )
            mOutputAccum[ dst ] +=
                gWindow.mData[ k ] * mFftBuffer[ k ].real() * scale;
         ...
         */
        std::copy_n( mOutputAccum.begin() + mPos, StftStep,
                     mFIFO.begin() + mPos );
        std::fill_n( mOutputAccum.begin() + mPos, StftStep, 0.0f );
    }

    /* Now, mix the processed sound data to the output. */
    MixSamples( { mBufferOut.data(), samplesToDo }, samplesOut, mCurrentGains,
                mTargetGains, maxz( samplesToDo, 512 ), 0 );
}
