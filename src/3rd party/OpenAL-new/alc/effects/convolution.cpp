...
void LoadSamples( float* RESTRICT dst,
                  const al::byte* src,
...
#define HANDLE_FMT( T )                                         \
    case T:                                                     \
        al::LoadSampleArray< T >( dst, src, srcstep, samples ); \
        break
    switch ( srctype ) {
        HANDLE_FMT( FmtUByte );
        HANDLE_FMT( FmtShort );
...
void apply_fir( al::span< float > dst,
    ...
    for ( float& output : dst ) {
        ...
        r4 = _mm_add_ps( r4,
                         _mm_shuffle_ps( r4, r4, _MM_SHUFFLE( 0, 1, 2, 3 ) ) );
    ...
    for ( float& output : dst ) {
        ...
        for ( size_t j{ 0 }; j < ConvolveUpdateSamples; j += 4 )
            r4 = vmlaq_f32( r4, vld1q_f32( &src[ j ] ),
                            vld1q_f32( &filter[ j ] ) );
        r4 = vaddq_f32( r4, vrev64q_f32( r4 ) );
        output = vget_lane_f32(
            vadd_f32( vget_low_f32( r4 ), vget_high_f32( r4 ) ), 0 );
...
struct ConvolutionState final : public EffectState {
    ...
    void NormalMix( const al::span< FloatBufferLine > samplesOut,
                    const size_t samplesToDo );
    void UpsampleMix( const al::span< FloatBufferLine > samplesOut,
                      const size_t samplesToDo );
    void ( ConvolutionState::*mMix )( const al::span< FloatBufferLine >,
                                      const size_t ){
...
void ConvolutionState::NormalMix( const al::span< FloatBufferLine > samplesOut,
    ...
    for ( auto& chan : *mChans )
        MixSamples( { chan.mBuffer.data(), samplesToDo }, samplesOut,
                    chan.Current, chan.Target, samplesToDo, 0 );
}

void ConvolutionState::UpsampleMix(
    ...
    for ( auto& chan : *mChans ) {
        ...
        MixSamples( src, samplesOut, chan.Current, chan.Target, samplesToDo,
                    0 );
    }
}
...
void ConvolutionState::deviceUpdate( const DeviceBase* device,
    ...
    const auto numChannels = ( mChannels == FmtUHJ2 )
                                 ? 3u
                                 : ChannelsFromFmt( mChannels, mAmbiOrder );
    ...
    const auto resampledCount = static_cast< uint >(
        ( uint64_t{ buffer->mSampleLen } * device->Frequency +
          ( buffer->mSampleRate - 1 ) ) /
        buffer->mSampleRate );
     ...
     */
    mNumConvolveSegs = ( resampledCount + ( ConvolveUpdateSamples - 1 ) ) /
    ...
    const size_t srclinelength{
        RoundUp( buffer->mSampleLen + DecoderPadding, 16 ) };
    auto srcsamples =
        std::make_unique< float[] >( srclinelength * numChannels );
    std::fill_n( srcsamples.get(), srclinelength * numChannels, 0.0f );
    for ( size_t c{ 0 }; c < numChannels && c < realChannels; ++c )
        LoadSamples( srcsamples.get() + srclinelength * c,
                     buffer->mData.data() + bytesPerSample * c, realChannels,
    ...
    if ( IsUHJ( mChannels ) ) {
        ...
        decoder->decode( { samples.data(), numChannels }, buffer->mSampleLen,
                         buffer->mSampleLen );
    }

    auto ressamples = std::make_unique< double[] >(
        buffer->mSampleLen + ( resampler ? resampledCount : 0 ) );
    complex_f* filteriter = mComplexData.get() + mNumConvolveSegs * m;
    for ( size_t c{ 0 }; c < numChannels; ++c ) {
        /* Resample to match the device. */
        if ( resampler ) {
            std::copy_n( srcsamples.get() + srclinelength * c,
                         buffer->mSampleLen,
                         ressamples.get() + resampledCount );
            resampler.process( buffer->mSampleLen,
                               ressamples.get() + resampledCount,
                               resampledCount, ressamples.get() );
        } else
            std::copy_n( srcsamples.get() + srclinelength * c,
                         buffer->mSampleLen, ressamples.get() );

        /* Store the first segment's samples in reverse in the time-domain, to
         * apply as a FIR filter.
         */
        const size_t first_size{
            minz( resampledCount, ConvolveUpdateSamples ) };
        std::transform( ressamples.get(), ressamples.get() + first_size,
                        mFilter[ c ].rbegin(),
                        []( const double d ) noexcept -> float {
                            return static_cast< float >( d );
                        } );

        auto fftbuffer =
            std::vector< std::complex< double > >( ConvolveUpdateSize );
        size_t done{ first_size };
        for ( size_t s{ 0 }; s < mNumConvolveSegs; ++s ) {
            const size_t todo{
                minz( resampledCount - done, ConvolveUpdateSamples ) };

            auto iter =
                std::copy_n( &ressamples[ done ], todo, fftbuffer.begin() );
            done += todo;
...
void ConvolutionState::update( const ContextBase* context,
                               const EffectSlot* slot,
                               const EffectProps* /*props*/,
    ...
    static constexpr ChanMap MonoMap[ 1 ]{ { FrontCenter, 0.0f, 0.0f } },
        StereoMap[ 2 ]{ { FrontLeft, Deg2Rad( -45.0f ), Deg2Rad( 0.0f ) },
                        { FrontRight, Deg2Rad( 45.0f ), Deg2Rad( 0.0f ) } },
        RearMap[ 2 ]{ { BackLeft, Deg2Rad( -135.0f ), Deg2Rad( 0.0f ) },
    ...
    if ( IsAmbisonic( mChannels ) ) {
        ...
        for ( size_t c{ 0u }; c < mChans->size(); ++c ) {
            ...
            ComputePanGains( target.Main, coeffs.data(), gain,
                             ( *mChans )[ c ].Target );
            coeffs[ acn ] = 0.0f;
        }
    } else {
        ...
        if ( device->mRenderMode == RenderMode::Pairwise ) {
            auto ScaleAzimuthFront = []( float azimuth, float scale ) -> float {
                ...
                if ( !( abs_azi >= half_pi ) )
                    return std::copysign( minf( abs_azi * scale, half_pi ),
                                          azimuth );
                return azimuth;
            };

            for ( size_t i{ 0 }; i < chanmap.size(); ++i ) {
                if ( chanmap[ i ].channel == LFE )
                    continue;
                const auto coeffs = CalcAngleCoeffs(
                    ScaleAzimuthFront( chanmap[ i ].angle, 2.0f ),
            ...
            for ( size_t i{ 0 }; i < chanmap.size(); ++i ) {
                if ( chanmap[ i ].channel == LFE )
                    continue;
                const auto coeffs = CalcAngleCoeffs(
                    chanmap[ i ].angle, chanmap[ i ].elevation, 0.0f );
                ComputePanGains( target.Main, coeffs.data(), gain,
                                 ( *mChans )[ i ].Target );
            }
    }
...
void ConvolutionState::process(
    ...
    for ( size_t base{ 0u }; base < samplesToDo; ) {
        const size_t todo{
            minz( ConvolveUpdateSamples - mFifoPos, samplesToDo - base ) };
        ...
        for ( size_t c{ 0 }; c < chans.size(); ++c ) {
            ...
            apply_fir( { buf_iter, todo }, mInput.data() + 1 + mFifoPos,
                       mFilter[ c ].data() );

            auto fifo_iter = mOutput[ c ].begin() + mFifoPos;
            std::transform( fifo_iter, fifo_iter + todo, buf_iter, buf_iter,
                            std::plus<>{} );
        }
        ...
        const complex_f* RESTRICT filter{ mComplexData.get() +
                                          mNumConvolveSegs * m };
        for ( size_t c{ 0 }; c < chans.size(); ++c ) {
            ...
            for ( size_t i{ m }; i < ConvolveUpdateSize; ++i )
                mFftBuffer[ i ] =
                    std::conj( mFftBuffer[ ConvolveUpdateSize - i ] );
            ...
            for ( size_t i{ 0 }; i < ConvolveUpdateSamples; ++i )
                mOutput[ c ][ ConvolveUpdateSamples + i ] =
                    mFftBuffer[ ConvolveUpdateSamples + i ].real();
        }
