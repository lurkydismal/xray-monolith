...
struct CubicFilter {
    ...
    constexpr CubicFilter() {
        ...
        for ( size_t i{ 0 }; i < sTableSteps; ++i ) {
            const double mu{ static_cast< double >( i ) /
                             double{ sTableSteps } };
...
alignas( 16 ) constexpr std::array< std::array< float, NUM_LINES >,
                                    NUM_LINES > EarlyA2B{
    { { { 0.5f, 0.5f, 0.5f, 0.5f } },
      { { 0.5f, -0.5f, 0.5f, -0.5f } },
      { { 0.5f, -0.5f, -0.5f, 0.5f } },
...
struct DelayLineI {
     ...
     * offset.
     */
    void realizeLineOffset(
        std::array< float, NUM_LINES >* sampleBuffer ) noexcept {
        Line = sampleBuffer + LineOffset;
    }

    /* Calculate the length of a delay line and store its mask and offset. */
    uint calcLineLength( const float length,
...
struct VecAllpass {
    ...
    void process( const al::span< ReverbUpdateLine, NUM_LINES > samples,
                  size_t offset,
                  const float xCoeff,
                  const float yCoeff,
                  const size_t todo );
...
struct T60Filter {
    ...
    void calcCoeffs( const float length,
...
struct EarlyReflections {
    ...
    void updateLines( const float density_mult,
...
struct ReverbPipeline {
    ...
    void updateDelayLine( const float earlyDelay,
                          const float lateDelay,
                          const float density_mult,
                          const float decayTime,
                          const float frequency );
    void update3DPanning( const float* ReflectionsPan,
                          const float* LateReverbPan,
                          const float earlyGain,
                          const float lateGain,
                          const bool doUpmix,
                          const MixParams* mainMix );
    ...
    void processLate( size_t offset,
                      const size_t samplesToDo,
                      const al::span< ReverbUpdateLine, NUM_LINES > tempSamples,
                      const al::span< FloatBufferLine, NUM_LINES > outSamples );
...
struct ReverbState final : public EffectState {
    ...
    void MixOutPlain( ReverbPipeline& pipeline,
    ...
    void MixOutAmbiUp( ReverbPipeline& pipeline,
                       const al::span< FloatBufferLine > samplesOut,
                       const size_t todo ) {
        ASSUME( todo > 0 );

        auto DoMixRow = []( const al::span< float > OutBuffer,
                            const al::span< const float, 4 > Gains,
                            const float* InSamples, const size_t InStride ) {
            std::fill( OutBuffer.begin(), OutBuffer.end(), 0.0f );
            for ( const float gain : Gains ) {
                const float* RESTRICT input{
                    al::assume_aligned< 16 >( InSamples ) };
                InSamples += InStride;

                if ( !( std::fabs( gain ) > GainSilenceThreshold ) )
                    continue;

                auto mix_sample = [ gain ]( const float sample,
                                            const float in ) noexcept -> float {
                    return sample + in * gain;
                };
                std::transform( OutBuffer.begin(), OutBuffer.end(), input,
         ...
         */
        const al::span< float > tmpspan{
            al::assume_aligned< 16 >( mTempLine.data() ), todo };
        for ( size_t c{ 0u }; c < NUM_LINES; c++ ) {
            DoMixRow( tmpspan, EarlyA2B[ c ], mEarlySamples[ 0 ].data(),
                      mEarlySamples[ 0 ].size() );

            /* Apply scaling to the B-Format's HF response to "upsample" it to
             * higher-order output.
             */
            const float hfscale{ ( c == 0 ) ? mOrderScales[ 0 ]
                                            : mOrderScales[ 1 ] };
        ...
        for ( size_t c{ 0u }; c < NUM_LINES; c++ ) {
            DoMixRow( tmpspan, LateA2B[ c ], mLateSamples[ 0 ].data(),
                      mLateSamples[ 0 ].size() );
    ...
    void mixOut( ReverbPipeline& pipeline,
                 const al::span< FloatBufferLine > samplesOut,
                 const size_t todo ) {
        if ( mUpmixOutput )
...
void ReverbState::allocLines( const float frequency ) {
     ...
     * swing.
     */
    constexpr float max_mod_delay{ MaxModulationTime * MODULATION_DEPTH_COEFF /
                                   2.0f };
    ...
    for ( auto& pipeline : mPipelines ) {
         ...
         */
        float length{ ReverbMaxReflectionsDelay +
                      EARLY_TAP_LENGTHS.back() * multiplier };
        totalSamples += pipeline.mEarlyDelayIn.calcLineLength(
            length, totalSamples, frequency, BufferLineSize );

        constexpr float LateLineDiffAvg{
            ( LATE_LINE_LENGTHS.back() - LATE_LINE_LENGTHS.front() ) /
            float{ NUM_LINES } };
        length = ReverbMaxLateReverbDelay + LateLineDiffAvg * multiplier;
        totalSamples += pipeline.mLateDelayIn.calcLineLength(
            length, totalSamples, frequency, BufferLineSize );

        /* The early vector all-pass line. */
        length = EARLY_ALLPASS_LENGTHS.back() * multiplier;
        totalSamples += pipeline.mEarly.VecAp.Delay.calcLineLength(
            length, totalSamples, frequency, 0 );

        /* The early reflection line. */
        length = EARLY_LINE_LENGTHS.back() * multiplier;
        totalSamples += pipeline.mEarly.Delay.calcLineLength(
            length, totalSamples, frequency, MAX_UPDATE_SAMPLES );

        /* The late vector all-pass line. */
        length = LATE_ALLPASS_LENGTHS.back() * multiplier;
        totalSamples += pipeline.mLate.VecAp.Delay.calcLineLength(
            length, totalSamples, frequency, 0 );
        ...
        totalSamples += pipeline.mLate.Delay.calcLineLength(
            length, totalSamples, frequency, 4 );
    }
    ...
    std::fill( mSampleBuffer.begin(), mSampleBuffer.end(),
...
void ReverbState::deviceUpdate( const DeviceBase* device,
                                const BufferStorage* ) {
    ...
    for ( auto& pipeline : mPipelines ) {
        ...
        std::fill( std::begin( pipeline.mEarlyDelayCoeff ),
                   std::end( pipeline.mEarlyDelayCoeff ), 0.0f );
        std::fill( std::begin( pipeline.mEarlyDelayCoeff ),
                   std::end( pipeline.mEarlyDelayCoeff ), 0.0f );
    ...
    mPipelines[ 0 ].mAmbiSplitter[ 0 ][ 0 ].init( device->mXOverFreq /
                                                  frequency );
    for ( auto& pipeline : mPipelines ) {
        std::fill( pipeline.mAmbiSplitter[ 0 ].begin(),
                   pipeline.mAmbiSplitter[ 0 ].end(),
                   pipeline.mAmbiSplitter[ 0 ][ 0 ] );
        std::fill( pipeline.mAmbiSplitter[ 1 ].begin(),
                   pipeline.mAmbiSplitter[ 1 ].end(),
                   pipeline.mAmbiSplitter[ 0 ][ 0 ] );
    }
}
 ...
 * filters.
 */
float CalcLimitedHfRatio( const float hfRatio,
 ...
 */
void T60Filter::calcCoeffs( const float length,
                            const float lfDecayTime,
                            const float mfDecayTime,
                            const float hfDecayTime,
                            const float lf0norm,
...
void EarlyReflections::updateLines( const float density_mult,
                                    const float diffusion,
                                    const float decayTime,
                                    const float frequency ) {
 ...
 */
void Modulation::updateModulator( float modTime,
                                  float modDepth,
                                  float frequency ) {
    ...
    if ( modTime >= DefaultModulationTime ) {
         ...
         */
        Depth = MODULATION_DEPTH_COEFF / 4.0f * DefaultModulationTime *
                modDepth * frequency;
    } else
...
void LateReverb::updateLines( const float density_mult,
                              const float diffusion,
                              const float lfDecayTime,
                              const float mfDecayTime,
                              const float hfDecayTime,
    ...
    const float late_allpass_avg{ std::accumulate( LATE_ALLPASS_LENGTHS.begin(),
                                                   LATE_ALLPASS_LENGTHS.end(),
                                                   0.0f ) /
                                  float{ NUM_LINES } };
     ...
     */
    float length{ std::accumulate( LATE_LINE_LENGTHS.begin(),
                                   LATE_LINE_LENGTHS.end(), 0.0f ) /
                      float{ NUM_LINES } +
                  late_allpass_avg };
    length *= density_mult;
    ...
    DensityGain =
        CalcDensityGain( CalcDecayCoeff( length, decayTimeWeighted ) );
    ...
    for ( size_t i{ 0u }; i < NUM_LINES; i++ ) {
         ...
         */
        length +=
            lerpf( LATE_ALLPASS_LENGTHS[ i ], late_allpass_avg, diffusion ) *
                density_mult +
            Mod.Depth / frequency;

        /* Calculate the T60 damping coefficients for each line. */
        T60[ i ].calcCoeffs( length, lfDecayTime, mfDecayTime, hfDecayTime,
                             lf0norm, hf0norm );
    }
}

/* Update the offsets for the main effect delay line. */
void ReverbPipeline::updateDelayLine( const float earlyDelay,
                                      const float lateDelay,
                                      const float density_mult,
                                      const float decayTime,
                                      const float frequency ) {
    ...
    for ( size_t i{ 0u }; i < NUM_LINES; i++ ) {
        ...
        mEarlyDelayTap[ i ][ 1 ] =
            float2uint( ( earlyDelay + length ) * frequency );
        mEarlyDelayCoeff[ i ] = CalcDecayCoeff( length, decayTime );

        length = ( LATE_LINE_LENGTHS[ i ] - LATE_LINE_LENGTHS.front() ) /
                     float{ NUM_LINES } * density_mult +
                 lateDelay;
 ...
 */
std::array< std::array< float, 4 >, 4 > GetTransformFromVector(
    const float* vec ) {
     ...
     */
    float norm[ 3 ];
    float mag{ std::sqrt( vec[ 0 ] * vec[ 0 ] + vec[ 1 ] * vec[ 1 ] +
                          vec[ 2 ] * vec[ 2 ] ) };
    if ( mag > 1.0f ) {
...
void ReverbPipeline::update3DPanning( const float* ReflectionsPan,
                                      const float* LateReverbPan,
                                      const float earlyGain,
                                      const float lateGain,
                                      const bool doUpmix,
     ...
     * panning vectors.
     */
    const std::array< std::array< float, 4 >, 4 > earlymat{
        GetTransformFromVector( ReflectionsPan ) };
    const std::array< std::array< float, 4 >, 4 > latemat{
    ...
    if ( doUpmix ) {
         ...
         * upsampled.
         */
        auto mult_matrix = []( const al::span< const std::array< float, 4 >, 4 >
                                   mtx1 ) {
        ...
        for ( size_t i{ 0u }; i < NUM_LINES; i++ )
            ComputePanGains( mainMix, earlycoeffs[ i ].data(), earlyGain,
                             mEarly.TargetGains[ i ] );
        for ( size_t i{ 0u }; i < NUM_LINES; i++ )
            ComputePanGains( mainMix, latecoeffs[ i ].data(), lateGain,
                             mLate.TargetGains[ i ] );
    } else {
         ...
         */
        auto mult_matrix =
            []( const al::span< const std::array< float, NUM_LINES >, 4 > mtx1,
                const al::span< const std::array< float, 4 >, 4 > mtx2 ) {
                std::array< std::array< float, MaxAmbiChannels >, NUM_LINES >
                    res{};
...
void ReverbState::update( const ContextBase* Context,
                          const EffectSlot* Slot,
                          const EffectProps* props,
    ...
    if ( props->Reverb.DecayHFLimit &&
         props->Reverb.AirAbsorptionGainHF < 1.0f )
        hfRatio =
            CalcLimitedHfRatio( hfRatio, props->Reverb.AirAbsorptionGainHF,
    ...
    pipeline.update3DPanning(
        props->Reverb.ReflectionsPan, props->Reverb.LateReverbPan,
        props->Reverb.ReflectionsGain * gain,
        props->Reverb.LateReverbGain * gain, mUpmixOutput, target.Main );

    /* Calculate the master filters */
    float hf0norm{ minf( props->Reverb.HFReference / frequency, 0.49f ) };
    pipeline.mFilter[ 0 ].Lp.setParamsFromSlope( BiquadType::HighShelf, hf0norm,
                                                 props->Reverb.GainHF, 1.0f );
    float lf0norm{ minf( props->Reverb.LFReference / frequency, 0.49f ) };
    pipeline.mFilter[ 0 ].Hp.setParamsFromSlope( BiquadType::LowShelf, lf0norm,
    ...
    pipeline.updateDelayLine( props->Reverb.ReflectionsDelay,
                              props->Reverb.LateReverbDelay, density_mult,
                              props->Reverb.DecayTime, frequency );

    if ( fullUpdate ) {
        /* Update the early lines. */
        pipeline.mEarly.updateLines( density_mult, props->Reverb.Diffusion,
                                     props->Reverb.DecayTime, frequency );

        /* Get the mixing matrix coefficients. */
        CalcMatrixCoeffs( props->Reverb.Diffusion, &pipeline.mMixX,
                          &pipeline.mMixY );
        ...
        pipeline.mLate.updateLines( density_mult, props->Reverb.Diffusion,
                                    lfDecayTime, props->Reverb.DecayTime,
                                    hfDecayTime, lf0norm, hf0norm, frequency );
    }

    const float decaySamples{ ( props->Reverb.ReflectionsDelay +
                                props->Reverb.LateReverbDelay +
                                props->Reverb.DecayTime ) *
                              frequency };
    pipeline.mFadeSampleCount =
        static_cast< size_t >( minf( decaySamples, 1'000'000.0f ) );
}
 ...
 */
inline auto VectorPartialScatter(
    const std::array< float, NUM_LINES >& RESTRICT in,
    const float xCoeff,
    const float yCoeff ) -> std::array< float, NUM_LINES > {
...
void VectorScatterRevDelayIn(
    const DelayLineI delay,
    size_t offset,
    const float xCoeff,
    const float yCoeff,
    const al::span< const ReverbUpdateLine, NUM_LINES > in,
    const size_t count ) {
    ASSUME( count > 0 );
 ...
 */
void VecAllpass::process( const al::span< ReverbUpdateLine, NUM_LINES > samples,
                          size_t offset,
                          const float xCoeff,
                          const float yCoeff,
                          const size_t todo ) {
    ...
    for ( size_t i{ 0u }; i < todo; ) {
        ...
        do {
            ...
            for ( size_t j{ 0u }; j < NUM_LINES; j++ ) {
                ...
                const float out{ delay.Line[ vap_offset[ j ]++ ][ j ] -
                                 feedCoeff * input };
 ...
 */
void ReverbPipeline::processEarly(
    size_t offset,
    const size_t samplesToDo,
    const al::span< ReverbUpdateLine, NUM_LINES > tempSamples,
    const al::span< FloatBufferLine, NUM_LINES > outSamples ) {
    ...
    for ( size_t base{ 0 }; base < samplesToDo; ) {
        ...
        for ( size_t j{ 0u }; j < NUM_LINES; j++ ) {
            ...
            const float coeffStep{ early_delay_tap0 != early_delay_tap1
                                       ? coeff * fadeStep
                                       : 0.0f };
            ...
            for ( size_t i{ 0u }; i < todo; ) {
                ...
                const size_t max_tap{
                    maxz( early_delay_tap0, early_delay_tap1 ) };
                ...
                do {
                    ...
                    fadeCount += 1.0f;
                    tempSamples[ j ][ i++ ] =
                        in_delay.Line[ early_delay_tap0++ ][ j ] * fade0 +
         ...
         */
        for ( size_t j{ 0u }; j < NUM_LINES; j++ )
            early_delay.write( offset, NUM_LINES - 1 - j,
                               tempSamples[ j ].data(), todo );
        for ( size_t j{ 0u }; j < NUM_LINES; j++ ) {
            size_t feedb_tap{ offset - mEarly.Offset[ j ] };
            const float feedb_coeff{ mEarly.Coeff[ j ] };
            float* RESTRICT out{
                al::assume_aligned< 16 >( outSamples[ j ].data() + base ) };
            ...
            for ( size_t i{ 0u }; i < todo; ) {
                ...
                do {
                    tempSamples[ j ][ i ] +=
                        early_delay.Line[ feedb_tap++ ][ j ] * feedb_coeff;
         ...
         */
        VectorScatterRevDelayIn( mLateDelayIn, offset, mixX, mixY, tempSamples,
                                 todo );
...
void Modulation::calcDelays( size_t todo ) {
    constexpr float mod_scale{ al::numbers::pi_v< float > * 2.0f /
                               MOD_FRACONE };
    uint idx{ Index };
    ...
    for ( size_t i{ 0 }; i < todo; ++i ) {
        idx += step;
        const float lfo{ std::sin( static_cast< float >( idx & MOD_FRACMASK ) *
                                   mod_scale ) };
 ...
 */
void ReverbPipeline::processLate(
    size_t offset,
    const size_t samplesToDo,
    const al::span< ReverbUpdateLine, NUM_LINES > tempSamples,
    const al::span< FloatBufferLine, NUM_LINES > outSamples ) {
    ...
    for ( size_t base{ 0 }; base < samplesToDo; ) {
        const size_t todo{
            minz( samplesToDo - base,
                  minz( mLate.Offset[ 0 ], MAX_UPDATE_SAMPLES ) ) };
        ASSUME( todo > 0 );
        ...
        for ( size_t j{ 0u }; j < NUM_LINES; j++ ) {
            ...
            for ( size_t i{ 0u }; i < todo; ) {
                ...
                size_t td{ minz(
                    todo - i, in_delay.Mask + 1 -
                                  maxz( late_delay_tap0, late_delay_tap1 ) ) };
                do {
                    /* Calculate the read offset and offset between it and the
                     * next sample.
                     */
                    const float fdelay{ mLate.Mod.ModDelays[ i ] };
                    const size_t idelay{ float2uint(
                        fdelay * float{ gCubicTable.sTableSteps } ) };
                    const size_t delay{ late_feedb_tap -
                                        ( idelay >> gCubicTable.sTableBits ) };
                    const size_t delayoffset{ idelay & gCubicTable.sTableMask };
                    ++late_feedb_tap;

                    /* Get the samples around by the delayed offset. */
                    const float out0{
                        late_delay.Line[ ( delay )&late_delay.Mask ][ j ] };
                    const float out1{ late_delay.Line[ ( delay - 1 ) &
                                                       late_delay.Mask ][ j ] };
                    const float out2{ late_delay.Line[ ( delay - 2 ) &
                                                       late_delay.Mask ][ j ] };
                    const float out3{ late_delay.Line[ ( delay - 3 ) &
                                                       late_delay.Mask ][ j ] };
                     ...
                     */
                    const float out{
                        out0 * gCubicTable.getCoeff0( delayoffset ) +
        ...
        for ( size_t j{ 0u }; j < NUM_LINES; j++ )
            std::copy_n( tempSamples[ j ].begin(), todo,
                         outSamples[ j ].begin() + base );

        /* Finally, scatter and bounce the results to refeed the feedback
         * buffer. */
        VectorScatterRevDelayIn( late_delay, offset, mixX, mixY, tempSamples,
                                 todo );
...
void ReverbState::process( const size_t samplesToDo,
    ...
    if ( mPipelineState >= Fading ) {
        ...
        const al::span< float > tmpspan{
            al::assume_aligned< 16 >( mTempLine.data() ), samplesToDo };
        for ( size_t c{ 0u }; c < NUM_LINES; c++ ) {
            std::fill( tmpspan.begin(), tmpspan.end(), 0.0f );
            for ( size_t i{ 0 }; i < numInput; ++i ) {
                const float gain{ B2A[ c ][ i ] };
                const float* RESTRICT input{
                    al::assume_aligned< 16 >( samplesIn[ i ].data() ) };

                auto mix_sample = [ gain ]( const float sample,
                                            const float in ) noexcept -> float {
                    return sample + in * gain;
                };
                std::transform( tmpspan.begin(), tmpspan.end(), input,
                                tmpspan.begin(), mix_sample );
            }

            /* Band-pass the incoming samples and feed the initial delay line.
             */
            auto&& filter = DualBiquad{ pipeline.mFilter[ c ].Lp,
                                        pipeline.mFilter[ c ].Hp };
            filter.process( tmpspan, tmpspan.data() );
            pipeline.mEarlyDelayIn.write( offset, c, tmpspan.cbegin(),
                                          samplesToDo );
        }
        if ( mPipelineState == Fading ) {
            ...
            for ( size_t c{ 0u }; c < NUM_LINES; c++ ) {
                ...
                auto&& filter = DualBiquad{ oldpipeline.mFilter[ c ].Lp,
                                            oldpipeline.mFilter[ c ].Hp };
                filter.process( tmpspan, tmpspan.data() );
                oldpipeline.mEarlyDelayIn.write( offset, c, tmpspan.cbegin(),
                                                 samplesToDo );
            }
        }
    } else {
    ...
    if ( mPipelineState != Normal ) {
        if ( mPipelineState == Cleanup ) {
            ...
            oldpipeline.processEarly( offset, samplesToDo, mTempSamples,
                                      mEarlySamples );
            oldpipeline.processLate( offset, samplesToDo, mTempSamples,
                                     mLateSamples );
