...
void ChorusState::deviceUpdate( const DeviceBase* Device,
    ...
    const size_t maxlen{
        NextPowerOf2( float2uint( max_delay * 2.0f * frequency ) + 1u ) };
...
void ChorusState::update( const ContextBase* Context,
                          const EffectSlot* Slot,
                          const EffectProps* props,
    ...
    mDelay = maxi(
        float2int( props->Chorus.Delay * frequency * MixerFracOne + 0.5f ),
        mindelay );
    ...
    static constexpr auto inv_sqrt2 =
        static_cast< float >( 1.0 / al::numbers::sqrt2 );
    static constexpr auto lcoeffs_pw =
        CalcDirectionCoeffs( { -1.0f, 0.0f, 0.0f } );
    static constexpr auto rcoeffs_pw =
        CalcDirectionCoeffs( { 1.0f, 0.0f, 0.0f } );
    static constexpr auto lcoeffs_nrml =
        CalcDirectionCoeffs( { -inv_sqrt2, 0.0f, inv_sqrt2 } );
    static constexpr auto rcoeffs_nrml =
        CalcDirectionCoeffs( { inv_sqrt2, 0.0f, inv_sqrt2 } );
    auto& lcoeffs = ( device->mRenderMode != RenderMode::Pairwise )
                        ? lcoeffs_nrml
                        : lcoeffs_pw;
    auto& rcoeffs = ( device->mRenderMode != RenderMode::Pairwise )
                        ? rcoeffs_nrml
                        : rcoeffs_pw;

    mOutTarget = target.Main->Buffer;
    ComputePanGains( target.Main, lcoeffs.data(), Slot->Gain,
                     mGains[ 0 ].Target );
    ComputePanGains( target.Main, rcoeffs.data(), Slot->Gain,
                     mGains[ 1 ].Target );
    ...
    if ( !( rate > 0.0f ) ) {
         ...
         */
        uint lfo_range{ float2uint(
            minf( frequency / rate + 0.5f, float{ INT_MAX / 360 - 180 } ) ) };
        ...
        switch ( mWaveform ) {
            ...
            case ChorusWaveform::Sinusoid:
                mLfoScale = al::numbers::pi_v< float > * 2.0f /
...
void ChorusState::calcTriangleDelays( const size_t todo ) {
    ...
    auto gen_lfo = [ lfo_scale, depth, delay ]( const uint offset ) -> uint {
        ...
        return static_cast< uint >(
            fastf2i( ( 1.0f - std::abs( 2.0f - offset_norm ) ) * depth ) +
            delay );
    };
...
void ChorusState::calcSinusoidDelays( const size_t todo ) {
    ...
    auto gen_lfo = [ lfo_scale, depth, delay ]( const uint offset ) -> uint {
        const float offset_norm{ static_cast< float >( offset ) * lfo_scale };
        return static_cast< uint >( fastf2i( std::sin( offset_norm ) * depth ) +
                                    delay );
    };
...
void ChorusState::process( const size_t samplesToDo,
    ...
    const uint avgdelay{ ( static_cast< uint >( mDelay ) + MixerFracHalf ) >>
                         MixerFracBits };
    ...
    for ( size_t i{ 0u }; i < samplesToDo; ++i ) {
        ...
        float mu{ static_cast< float >( ldelays[ i ] & MixerFracMask ) *
                  ( 1.0f / MixerFracOne ) };
        lbuffer[ i ] = cubic( delaybuf[ ( delay + 1 ) & bufmask ],
                              delaybuf[ ( delay )&bufmask ],
                              delaybuf[ ( delay - 1 ) & bufmask ],
                              delaybuf[ ( delay - 2 ) & bufmask ], mu );

        // Tap for the right output.
        delay = offset - ( rdelays[ i ] >> MixerFracBits );
        mu = static_cast< float >( rdelays[ i ] & MixerFracMask ) *
             ( 1.0f / MixerFracOne );
        rbuffer[ i ] = cubic( delaybuf[ ( delay + 1 ) & bufmask ],
                              delaybuf[ ( delay )&bufmask ],
                              delaybuf[ ( delay - 1 ) & bufmask ],
                              delaybuf[ ( delay - 2 ) & bufmask ], mu );

        // Accumulate feedback from the average delay of the taps.
        delaybuf[ offset & bufmask ] +=
            delaybuf[ ( offset - avgdelay ) & bufmask ] * feedback;
        ++offset;
    }

    MixSamples( { lbuffer, samplesToDo }, samplesOut, mGains[ 0 ].Current,
                mGains[ 0 ].Target, samplesToDo, 0 );
    MixSamples( { rbuffer, samplesToDo }, samplesOut, mGains[ 1 ].Current,
                mGains[ 1 ].Target, samplesToDo, 0 );
