...
void EchoState::deviceUpdate( const DeviceBase* Device, const BufferStorage* ) {
    ...
    const uint maxlen{
        NextPowerOf2( float2uint( EchoMaxDelay * frequency + 0.5f ) +
...
void EchoState::update( const ContextBase* context,
                        const EffectSlot* slot,
                        const EffectProps* props,
    ...
    mTap[ 0 ].delay =
        maxu( float2uint( props->Echo.Delay * frequency + 0.5f ), 1 );
    mTap[ 1 ].delay =
        float2uint( props->Echo.LRDelay * frequency + 0.5f ) + mTap[ 0 ].delay;

    const float gainhf{
        maxf( 1.0f - props->Echo.Damping, 0.0625f ) }; /* Limit -24dB */
    mFilter.setParamsFromSlope( BiquadType::HighShelf,
                                LowpassFreqRef / frequency, gainhf, 1.0f );
    ...
    ComputePanGains( target.Main, coeffs0.data(), slot->Gain,
                     mGains[ 0 ].Target );
    ComputePanGains( target.Main, coeffs1.data(), slot->Gain,
                     mGains[ 1 ].Target );
}

void EchoState::process( const size_t samplesToDo,
                         const al::span< const FloatBufferLine > samplesIn,
    ...
    for ( size_t i{ 0u }; i < samplesToDo; ) {
        ...
        size_t td{ minz( mask + 1 - maxz( offset, maxz( tap1, tap2 ) ),
                         samplesToDo - i ) };
        do {
            ...
            delaybuf[ offset++ ] +=
                filter.processOne( feedb, z1, z2 ) * mFeedGain;
        } while ( --td );
    }
    ...
    for ( size_t c{ 0 }; c < 2; c++ )
        MixSamples( { mTempBuffer[ c ], samplesToDo }, samplesOut,
                    mGains[ c ].Current, mGains[ c ].Target, samplesToDo, 0 );
}
