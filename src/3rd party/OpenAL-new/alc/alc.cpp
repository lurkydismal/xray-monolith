...
constexpr struct {
  ...
  eaxEnumerations[] = {
      DECL( AL_EAX_RAM_SIZE ),       DECL( AL_EAX_RAM_FREE ),
      DECL( AL_STORAGE_AUTOMATIC ),  DECL( AL_STORAGE_HARDWARE ),
...
void alc_initconfig( void ) {
    ...
    if ( auto cpuopt =
        ...
        if ( al::strcasecmp( str, "all" ) == 0 )
            ...
            do {
                ...
                size_t len{ next ? static_cast< size_t >( next - str )
                                 : strlen( str ) };
                ...
                if ( len == 3 && al::strncasecmp( str, "sse", len ) == 0 )
                    ...
                    capfilter &= ~CPU_CAP_SSE3;
                          ...
                          al::strncasecmp( str, "sse4.1", len ) == 0 )
    ...
    if ( auto cpuopt = GetCPUInfo() ) {
        ...
        TRACE( "Extensions:%s%s%s%s%s%s\n",
               ( ( capfilter & CPU_CAP_SSE )
                     ? ( ( caps & CPU_CAP_SSE ) ? " +SSE" : " -SSE" )
                     : "" ),
               ( ( capfilter & CPU_CAP_SSE2 )
                     ? ( ( caps & CPU_CAP_SSE2 ) ? " +SSE2" : " -SSE2" )
                     : "" ),
               ( ( capfilter & CPU_CAP_SSE3 )
                     ? ( ( caps & CPU_CAP_SSE3 ) ? " +SSE3" : " -SSE3" )
                     : "" ),
               ( ( capfilter & CPU_CAP_SSE4_1 )
                     ? ( ( caps & CPU_CAP_SSE4_1 ) ? " +SSE4.1" : " -SSE4.1" )
                     : "" ),
               ( ( capfilter & CPU_CAP_NEON )
                     ? ( ( caps & CPU_CAP_NEON ) ? " +NEON" : " -NEON" )
                     : "" ),
               ( ( !capfilter ) ? " -none-" : "" ) );
...

    {
        ...
        auto checkflag = []( const char* envname,
                             const char* optname ) -> bool {
        ...
        sBufferSubDataCompat =
            checkflag( "__ALSOFT_ENABLE_SUB_DATA_EXT", "enable-sub-data-ext" );
        compatflags.set( CompatFlags::ReverseX,
                         checkflag( "__ALSOFT_REVERSE_X", "reverse-x" ) );
        compatflags.set( CompatFlags::ReverseY,
                         checkflag( "__ALSOFT_REVERSE_Y", "reverse-y" ) );
        compatflags.set( CompatFlags::ReverseZ,
                         checkflag( "__ALSOFT_REVERSE_Z", "reverse-z" ) );

        aluInit( compatflags,
                 ConfigValueFloat( nullptr, "game_compat", "nfc-scale" )
                     .value_or( 1.0f ) );
    }
    ...
    if ( traperr && ( al::strcasecmp( traperr->c_str(), "true" ) == 0 ||
        ...
        if ( traperr )
            ...
            TrapALError = !!GetConfigValueBool( nullptr, nullptr,
                                                "trap-al-error", false );
        ...
        if ( traperr )
            ...
            TrapALCError = !!GetConfigValueBool( nullptr, nullptr,
                                                 "trap-alc-error", false );
    }

    if ( auto boostopt = ConfigValueFloat( nullptr, "reverb", "boost" ) ) {
        const float valf{ std::isfinite( *boostopt )
                              ? clampf( *boostopt, -24.0f, 24.0f )
                              : 0.0f };
    ...
    if ( devopt ||
         ( devopt = ConfigValueStr( nullptr, nullptr, "drivers" ) ) ) {
        ...
        do {
            ...
            size_t len{ next ? ( static_cast< size_t >( next - devs ) )
                             : strlen( devs ) };
            ...
            auto find_backend = [ devs,
                                  len ]( const BackendInfo& backend ) -> bool {
                return len == strlen( backend.name ) &&
                       strncmp( backend.name, devs, len ) == 0;
            };
            auto this_backend = std::find_if( std::begin( BackendList ),
                                              BackendListEnd, find_backend );
            ...
            if ( delitem )
                BackendListEnd =
                    std::move( this_backend + 1, BackendListEnd, this_backend );
            else
                backendlist_cur = std::rotate( backendlist_cur, this_backend,
                                               this_backend + 1 );
        } while ( next++ );
    ...
    auto init_backend = []( BackendInfo& backend ) -> void {
        ...
        if ( !PlaybackFactory &&
             factory.querySupport( BackendType::Playback ) ) {
    ...
    if ( defrevopt ||
         ( defrevopt = ConfigValueStr( nullptr, nullptr, "default-reverb" ) ) )
...
#ifdef ALSOFT_EAX
    {
        ...
        if ( ( DisabledEffects[ EAXREVERB_EFFECT ] ||
               DisabledEffects[ CHORUS_EFFECT ] ) &&
             eax_g_is_enabled ) {
            eax_g_is_enabled = false;
            TRACE( "EAX disabled because %s disabled.\n",
                   ( DisabledEffects[ EAXREVERB_EFFECT ] &&
                     DisabledEffects[ CHORUS_EFFECT ] )
...
al::optional< DevFmtPair > DecomposeDevFormat( ALenum format ) {
    ...
    for ( const auto& item : list ) {
        if ( item.format == format )
            return al::make_optional< DevFmtPair >(
                { item.channels, item.type } );
    }
...
ALCenum EnumFromDevFmt( DevFmtType type ) {
    ...
    throw std::runtime_error{ "Invalid DevFmtType: " +
...
ALCenum EnumFromDevFmt( DevFmtChannels channels ) {
    ...
    throw std::runtime_error{ "Invalid DevFmtChannels: " +
...
ALCenum EnumFromDevAmbi( DevAmbiLayout layout ) {
    switch ( layout ) {
        ...
        case DevAmbiLayout::ACN:
            return ALC_ACN_SOFT;
    }
    throw std::runtime_error{ "Invalid DevAmbiLayout: " +
                              std::to_string( int( layout ) ) };
}
...
ALCenum EnumFromDevAmbi( DevAmbiScaling scaling ) {
    switch ( scaling ) {
        case DevAmbiScaling::FuMa:
            return ALC_FUMA_SOFT;
        case DevAmbiScaling::SN3D:
            return ALC_SN3D_SOFT;
        case DevAmbiScaling::N3D:
            return ALC_N3D_SOFT;
    }
    throw std::runtime_error{ "Invalid DevAmbiScaling: " +
                              std::to_string( int( scaling ) ) };
}
...
std::unique_ptr< Compressor > CreateDeviceLimiter( const ALCdevice* device,
    ...
    return Compressor::Create( device->RealOut.Buffer.size(),
                               ...
                               AutoKnee, AutoAttack, AutoRelease, AutoPostGain,
                               AutoDeclip, LookAheadTime, HoldTime, PreGainDb,
                               PostGainDb, threshold, Ratio, KneeDb, AttackTime,
                               ReleaseTime );
}
...
ALCenum UpdateDeviceParams( ALCdevice* device, const int* attrList ) {
    ...
    if ( device->Type != DeviceType::Loopback ) {
        ...
        if ( auto freqopt =
                 device->configValue< uint >( nullptr, "frequency" ) ) {
            optsrate = clampu( *freqopt, MIN_OUTPUT_RATE, MAX_OUTPUT_RATE );

            const double scale{ static_cast< double >( *optsrate ) /
                                DEFAULT_OUTPUT_RATE };
            period_size = static_cast< uint >( period_size * scale + 0.5 );
        }

        if ( auto persizeopt =
                 device->configValue< uint >( nullptr, "period_size" ) )
            period_size = clampu( *persizeopt, 64, 8192 );
        if ( auto numperopt =
                 device->configValue< uint >( nullptr, "periods" ) )
            buffer_size = clampu( *numperopt, 2, 16 ) * period_size;
        else
            buffer_size = period_size * DEFAULT_NUM_UPDATES;

        if ( auto typeopt = device->configValue< std::string >(
                 nullptr, "sample-type" ) ) {
            static constexpr struct TypeMap {
                const char name[ 8 ];
                DevFmtType type;
            } typelist[] = {
                { "int8", DevFmtByte },     { "uint8", DevFmtUByte },
                { "int16", DevFmtShort },   { "uint16", DevFmtUShort },
                { "int32", DevFmtInt },     { "uint32", DevFmtUInt },
                { "float32", DevFmtFloat },
            };

            const ALCchar* fmt{ typeopt->c_str() };
            auto iter =
                std::find_if( std::begin( typelist ), std::end( typelist ),
        ...
        if ( auto chanopt =
                 device->configValue< std::string >( nullptr, "channels" ) ) {
            ...
            auto iter =
                std::find_if( std::begin( chanlist ), std::end( chanlist ),
        ...
        if ( auto ambiopt = device->configValue< std::string >(
                 nullptr, "ambi-format" ) ) {
            ...
            if ( al::strcasecmp( fmt, "fuma" ) == 0 ) {
                ...
                optscale = DevAmbiScaling::FuMa;
            } else if ( al::strcasecmp( fmt, "ambix" ) == 0 ||
                        al::strcasecmp( fmt, "acn+sn3d" ) == 0 ) {
                optlayout = DevAmbiLayout::ACN;
        ...
        if ( auto hrtfopt =
                 device->configValue< std::string >( nullptr, "hrtf" ) ) {
            WARN(
                "general/hrtf is deprecated, please use stereo-encoding "
                "instead\n" );
    ...
    if ( auto encopt = device->configValue< std::string >(
             nullptr, "stereo-encoding" ) ) {
        const char* mode{ encopt->c_str() };
        if ( al::strcasecmp( mode, "basic" ) == 0 ||
             al::strcasecmp( mode, "panpot" ) == 0 )
    ...
    TRACE( "%s = %d\n", #a, attrList[ attrIdx + 1 ] );
        ...
        while ( attrList[ attrIdx ] ) {
            switch ( attrList[ attrIdx ] ) {
                case ATTRIBUTE(
                    ALC_FORMAT_CHANNELS_SOFT ) if ( device->Type ==
                                                    DeviceType::Loopback )
                    optchans =
                        DevFmtChannelsFromEnum( attrList[ attrIdx + 1 ] );
                    break;

                    case ATTRIBUTE(
                        ALC_FORMAT_TYPE_SOFT ) if ( device->Type ==
                                                    DeviceType::Loopback )
                        opttype = DevFmtTypeFromEnum( attrList[ attrIdx + 1 ] );
                    break;

                    case ATTRIBUTE( ALC_FREQUENCY ) freqAttr =
                        attrList[ attrIdx + 1 ];
                    break;

                    case ATTRIBUTE(
                        ALC_AMBISONIC_LAYOUT_SOFT ) if ( device->Type ==
                                                         DeviceType::Loopback )
                        optlayout =
                            DevAmbiLayoutFromEnum( attrList[ attrIdx + 1 ] );
                    break;

                    case ATTRIBUTE(
                        ALC_AMBISONIC_SCALING_SOFT ) if ( device->Type ==
                                                          DeviceType::Loopback )
                        optscale =
                            DevAmbiScalingFromEnum( attrList[ attrIdx + 1 ] );
                    break;

                    case ATTRIBUTE(
                        ALC_AMBISONIC_ORDER_SOFT ) if ( device->Type ==
                                                        DeviceType::Loopback )
                        aorder = static_cast< uint >( attrList[ attrIdx + 1 ] );
                    break;

                    case ATTRIBUTE( ALC_MONO_SOURCES ) numMono =
                        static_cast< uint >( attrList[ attrIdx + 1 ] );
                    if ( numMono > INT_MAX ) numMono = 0; break;

                    case ATTRIBUTE( ALC_STEREO_SOURCES ) numStereo =
                        static_cast< uint >( attrList[ attrIdx + 1 ] );
                    if ( numStereo > INT_MAX ) numStereo = 0; break;

                    case ATTRIBUTE( ALC_MAX_AUXILIARY_SENDS ) numSends =
                        static_cast< uint >( attrList[ attrIdx + 1 ] );
                    if ( numSends > INT_MAX ) numSends = 0;
                    else numSends = minu( numSends, MAX_SENDS ); break;

                    case ATTRIBUTE(
                        ALC_HRTF_SOFT ) if ( attrList[ attrIdx + 1 ] ==
                                             ALC_FALSE ) opthrtf = false;
                    else if ( attrList[ attrIdx + 1 ] == ALC_TRUE ) opthrtf =
                        true;
                    else if ( attrList[ attrIdx + 1 ] == ALC_DONT_CARE_SOFT )
                        opthrtf = al::nullopt;
                    break;

                    case ATTRIBUTE( ALC_HRTF_ID_SOFT ) hrtf_id =
                        attrList[ attrIdx + 1 ];
                    break;

                    case ATTRIBUTE(
                        ALC_OUTPUT_LIMITER_SOFT ) if ( attrList[ attrIdx +
                                                                 1 ] ==
                                                       ALC_FALSE ) optlimit =
                        false;
                    else if ( attrList[ attrIdx + 1 ] == ALC_TRUE ) optlimit =
                        true;
                    else if ( attrList[ attrIdx + 1 ] == ALC_DONT_CARE_SOFT )
                        optlimit = al::nullopt;
                    break;

                    case ATTRIBUTE( ALC_OUTPUT_MODE_SOFT ) outmode =
                        attrList[ attrIdx + 1 ];
                    break;
...
#undef ATTRIBUTE
        ...
        if ( device->Type == DeviceType::Loopback ) {
            ...
            if ( *optchans == DevFmtAmbi3D ) {
                ...
                if ( aorder < 1 || aorder > MaxAmbiOrder )
                    return ALC_INVALID_VALUE;
                if ( ( *optlayout == DevAmbiLayout::FuMa ||
                       *optscale == DevAmbiScaling::FuMa ) &&
                     aorder > 3 )
                    return ALC_INVALID_VALUE;
                ...
                if ( opthrtf ) {
                    if ( *opthrtf )
                        ...
                        if ( stereomode.value_or( StereoEncoding::Hrtf ) ==
                             StereoEncoding::Hrtf )
                            stereomode = StereoEncoding::Default;
                    }
                }
            ...
            if ( freqAttr ) {
                ...
                const double scale{ static_cast< double >( freqAttr ) /
                                    oldrate };
                period_size = static_cast< uint >( period_size * scale + 0.5 );
    ...
    std::fill( std::begin( device->HrtfAccumData ),
               std::end( device->HrtfAccumData ), float2{} );

    device->Dry.AmbiMap.fill( BFChannelConfig{} );
    device->Dry.Buffer = {};
    std::fill( std::begin( device->NumChannelsPerOrder ),
               std::end( device->NumChannelsPerOrder ), 0u );
    ...
    if ( device->Type == DeviceType::Loopback ) {
        ...
        if ( device->FmtChans == DevFmtAmbi3D ) {
            ...
            if ( device->mAmbiOrder > 3 &&
                ...
                ERR( "FuMa is incompatible with %d%s order ambisonics (up to "
                     "3rd order only)\n",
                     device->mAmbiOrder,
                     ( ( ( device->mAmbiOrder % 100 ) / 10 ) == 1 ) ? "th"
                     : ( ( device->mAmbiOrder % 10 ) == 1 )         ? "st"
                     : ( ( device->mAmbiOrder % 10 ) == 2 )         ? "nd"
                     : ( ( device->mAmbiOrder % 10 ) == 3 )         ? "rd"
                                                                    : "th" );
                device->mAmbiOrder = 3;
            }
        }
    ...
    TRACE( "Pre-reset: %s%s, %s%s, %s%uhz, %u / %u buffer\n",
           device->Flags.test( ChannelsRequest ) ? "*" : "",
           DevFmtChannelsString( device->FmtChans ),
           device->Flags.test( SampleTypeRequest ) ? "*" : "",
    ...
    if ( device->Frequency != oldFreq &&
         device->Flags.test( FrequencyRequest ) ) {
    ...
    if ( device->Type != DeviceType::Loopback ) {
        if ( auto modeopt = device->configValue< std::string >(
                 nullptr, "stereo-mode" ) ) {
    ...
    switch ( device->FmtChans ) {
        ...
        case DevFmtStereo:
            ...
            break;
        case DevFmtQuad:
            device->RealOut.RemixMap = QuadDownmix;
            break;
        case DevFmtX51:
            device->RealOut.RemixMap = X51Downmix;
            break;
        case DevFmtX61:
            device->RealOut.RemixMap = X61Downmix;
            break;
        case DevFmtX71:
            device->RealOut.RemixMap = X71Downmix;
            break;
        case DevFmtX714:
            device->RealOut.RemixMap = X71Downmix;
            break;
    ...
    if ( device->getConfigValueBool( nullptr, "dither", true ) ) {
        int depth{ device->configValue< int >( nullptr, "dither-depth" )
                       .value_or( 0 ) };
        if ( depth <= 0 ) {
        ...
        if ( depth > 0 ) {
            ...
            device->DitherDepth =
                std::pow( 2.0f, static_cast< float >( depth - 1 ) );
        }
    }
    ...
    device->FixedLatency +=
        nanoseconds{ seconds{ sample_delay } } / device->Frequency;
    ...
    for ( ContextBase* ctxbase : *device->mContexts.load() ) {
        ...
        auto slot_cluster_not_in_use =
            []( ContextBase::EffectSlotCluster& cluster ) {
                for ( size_t i{ 0 }; i < ContextBase::EffectSlotClusterSize;
                      ++i ) {
                    if ( cluster[ i ].InUse )
                        return false;
                }
        ...
        auto slotcluster_iter = std::remove_if(
            context->mEffectSlotClusters.begin(),
            context->mEffectSlotClusters.end(), slot_cluster_not_in_use );
        context->mEffectSlotClusters.erase(
            slotcluster_iter, context->mEffectSlotClusters.end() );
        ...
        if ( EffectSlotArray * curarray{ context->mActiveAuxSlots.load(
        ...
        for ( auto& sublist : context->mSourceList ) {
            ...
            while ( usemask ) {
                ...
                auto send_begin = source->Send.begin() +
                                  static_cast< ptrdiff_t >( num_sends );
        ...
        for ( Voice* voice : voicelist ) {
            ...
            std::fill( std::begin( voice->mProps.Send ) + num_sends,
                       std::end( voice->mProps.Send ), VoiceProps::SendData{} );

            std::fill( voice->mSend.begin() + num_sends, voice->mSend.end(),
                       Voice::TargetData{} );
            for ( auto& chandata : voice->mChans ) {
                std::fill( chandata.mWetParams.begin() + num_sends,
                           chandata.mWetParams.end(), SendParams{} );
            }

            if ( VoicePropsItem * props{ voice->mUpdate.exchange(
            ...
            voice->mPlayState.compare_exchange_strong(
                vstate, Voice::Stopped, std::memory_order_acquire,
    ...
    if ( !device->Connected.load( std::memory_order_relaxed ) )
        UNLIKELY {
            ...
            for ( ContextBase* ctxbase :
                ...
                /* Clear any pending voice changes and reallocate voices to get
                 * a clean restart.
                 */
                ...
                while ( auto* next =
                            vchg->mNext.load( std::memory_order_acquire ) )
                    vchg = next;
                ctx->mCurrentVoiceChange.store( vchg,
                                                std::memory_order_release );

                ctx->mVoicePropClusters.clear();
                ctx->mFreeVoiceProps.store( nullptr,
                                            std::memory_order_relaxed );

                ctx->mVoiceClusters.clear();
                ctx->allocVoices( std::max< size_t >(
                    256, ctx->mActiveVoiceCount.load(
                             std::memory_order_relaxed ) ) );
            }
...
DeviceRef VerifyDevice( ALCdevice* device ) {
    std::lock_guard< std::recursive_mutex > _{ ListLock };
    auto iter =
        std::lower_bound( DeviceList.begin(), DeviceList.end(), device );
...
ContextRef VerifyContext( ALCcontext* context ) {
    std::lock_guard< std::recursive_mutex > _{ ListLock };
    auto iter =
        std::lower_bound( ContextList.begin(), ContextList.end(), context );
...
ContextRef GetContextRef( void ) {
    ...
    if ( context )
        ...
        while ( ALCcontext::sGlobalContextLock.exchange(
        ...
        if ( context )
            LIKELY context->add_ref();
        ALCcontext::sGlobalContextLock.store( false,
                                              std::memory_order_release );
    }
...
ALC_API ALCenum ALC_APIENTRY alcGetError( ALCdevice* device ) START_API_FUNC {
...
alcGetString( ALCdevice* Device, ALCenum param ) START_API_FUNC {
...
static size_t GetIntegerv( ALCdevice* device,
    ...
    if ( device->Type == DeviceType::Capture ) {
        ...
        switch ( param ) {
            ...
            case ALC_ALL_ATTRIBUTES:
                ...
                if ( values.size() < MaxCaptureAttributes )
                    ...
                    values[ i++ ] = static_cast< int >(
                        device->Backend->availableSamples() );
            ...
            case ALC_CAPTURE_SAMPLES:
                values[ 0 ] =
                    static_cast< int >( device->Backend->availableSamples() );
                return 1;
    ...
    auto NumAttrsForDevice = []( ALCdevice* aldev ) noexcept {
        if ( aldev->Type == DeviceType::Loopback &&
             aldev->FmtChans == DevFmtAmbi3D )
            return 37;
        return 31;
    };
    switch ( param ) {
        ...
        case ALC_ALL_ATTRIBUTES:
            i = 0;
            if ( values.size() <
                 static_cast< size_t >( NumAttrsForDevice( device ) ) )
                alcSetError( device, ALC_INVALID_VALUE );
            else {
                values[ i++ ] = ALC_MAJOR_VERSION;
                values[ i++ ] = alcMajorVersion;
                ...
                if ( device->Type != DeviceType::Loopback ) {
                    ...
                    if ( device->FmtChans == DevFmtAmbi3D ) {
                        ...
                        values[ i++ ] =
                            static_cast< int >( device->mAmbiOrder );
                    }
                ...
                values[ i++ ] =
                    static_cast< ALCenum >( device->getOutputMode1() );
        ...
        case ALC_REFRESH:
            ...
            values[ 0 ] =
                static_cast< int >( device->Frequency / device->UpdateSize );
            return 1;
        ...
        case ALC_NUM_HRTF_SPECIFIERS_SOFT:
            ...
            values[ 0 ] = static_cast< int >( minz(
                device->mHrtfList.size(), std::numeric_limits< int >::max() ) );
            return 1;
    ...
    switch ( pname ) {
        ...
        case ALC_ALL_ATTRIBUTES:
            if ( size < NumAttrsForDevice( dev.get() ) )
                ...
                ClockLatency clock{
                    GetClockLatency( dev.get(), dev->Backend.get() ) };
        ...
        case ALC_DEVICE_LATENCY_SOFT:
            *values = GetClockLatency( dev.get(), dev->Backend.get() )
                          .Latency.count();
            break;
...
alcGetProcAddress( ALCdevice* device, const ALCchar* funcName ) START_API_FUNC {
    if ( !funcName ) {
        DeviceRef dev{ VerifyDevice( device ) };
        alcSetError( dev.get(), ALC_INVALID_VALUE );
        return nullptr;
...
alcGetEnumValue( ALCdevice* device, const ALCchar* enumName ) START_API_FUNC {
    if ( !enumName ) {
        DeviceRef dev{ VerifyDevice( device ) };
        alcSetError( dev.get(), ALC_INVALID_VALUE );
        return 0;
...
alcCreateContext( ALCdevice* device, const ALCint* attrList ) START_API_FUNC {
    ...
    DeviceRef dev{ VerifyDevice( device ) };
    if ( !dev || dev->Type == DeviceType::Capture ||
         !dev->Connected.load( std::memory_order_relaxed ) ) {
        listlock.unlock();
...

    {
        ...
        std::unique_ptr< ContextArray > newarray{
    ...
    {
        std::lock_guard< std::recursive_mutex > _{ ListLock };
        auto iter = std::lower_bound( ContextList.cbegin(), ContextList.cend(),
                                      context.get() );
    ...
    if ( ALeffectslot * slot{ context->mDefaultSlot.get() } ) {
        ALenum sloterr{ slot->initEffect( ALCcontext::sDefaultEffect.type,
                                          ALCcontext::sDefaultEffect.Props,
                                          context.get() ) };
...
ALC_API ALCdevice* ALC_APIENTRY alcOpenDevice( const ALCchar* deviceName )
    ...
    if ( deviceName ) {
        ...
        if ( !deviceName[ 0 ] ||
             al::strcasecmp( deviceName, alcDefaultName ) == 0
#ifdef _WIN32
    ...
    try {
        auto backend = PlaybackFactory->createBackend( device.get(),
        ...
        alcSetError( nullptr,
                     ( e.errorCode() == al::backend_error::OutOfMemory )
                         ? ALC_OUT_OF_MEMORY
                         : ALC_INVALID_VALUE );
        return nullptr;
    }
    ...
    {
        std::lock_guard< std::recursive_mutex > _{ ListLock };
        auto iter = std::lower_bound( DeviceList.cbegin(), DeviceList.cend(),
                                      device.get() );
...
ALC_API ALCboolean ALC_APIENTRY alcCloseDevice( ALCdevice* device )
    ...
    for ( ContextBase* ctx : *dev->mContexts.load() ) {
        auto ctxiter =
            std::lower_bound( ContextList.begin(), ContextList.end(), ctx );
...
ALC_API ALCdevice* ALC_APIENTRY alcCaptureOpenDevice( const ALCchar* deviceName,
    ...
    try {
        ...
        auto backend =
            CaptureFactory->createBackend( device.get(), BackendType::Capture );
        std::lock_guard< std::recursive_mutex > _{ ListLock };
        backend->open( deviceName );
        device->Backend = std::move( backend );
    } catch ( al::backend_exception& e ) {
...
ALC_API void ALC_APIENTRY alcCaptureSamples( ALCdevice* device,
                                             ALCvoid* buffer,
                                             ALCsizei samples ) START_API_FUNC {
    DeviceRef dev{ VerifyDevice( device ) };
    if ( !dev || dev->Type != DeviceType::Capture ) {
        alcSetError( dev.get(), ALC_INVALID_DEVICE );
        return;
...
alcLoopbackOpenDeviceSOFT( const ALCchar* deviceName ) START_API_FUNC {
    InitConfig();
    ...
    try {
        auto backend = LoopbackBackendFactory::getFactory().createBackend(
            device.get(), BackendType::Playback );
...
alcIsRenderFormatSupportedSOFT( ALCdevice* device,
    ...
    if ( !dev || dev->Type != DeviceType::Loopback )
        ...
        if ( DevFmtTypeFromEnum( type ).has_value() &&
             DevFmtChannelsFromEnum( channels ).has_value() &&
 ...
 */
FORCE_ALIGN ALC_API void ALC_APIENTRY alcRenderSamplesSOFT( ALCdevice* device,
                                                            ALCvoid* buffer,
                                                            ALCsizei samples )
    START_API_FUNC {
...
ALC_API const ALCchar* ALC_APIENTRY alcGetStringiSOFT( ALCdevice* device,
    ...
    if ( !dev || dev->Type == DeviceType::Capture )
        ...
        switch ( paramName ) {
            case ALC_HRTF_SPECIFIER_SOFT:
                if ( index >= 0 &&
                     static_cast< uint >( index ) < dev->mHrtfList.size() )
                    return dev->mHrtfList[ static_cast< uint >( index ) ]
                        .c_str();
                alcSetError( dev.get(), ALC_INVALID_VALUE );
                break;
...
ALC_API ALCboolean ALC_APIENTRY
alcResetDeviceSOFT( ALCdevice* device, const ALCint* attribs ) START_API_FUNC {
    std::unique_lock< std::recursive_mutex > listlock{ ListLock };
    DeviceRef dev{ VerifyDevice( device ) };
    if ( !dev || dev->Type == DeviceType::Capture ) {
        listlock.unlock();
...
/** Reopens the given device output, using the specified name and attribute
 * list. */
FORCE_ALIGN ALCboolean ALC_APIENTRY
alcReopenDeviceSOFT( ALCdevice* device,
                     const ALCchar* deviceName,
                     const ALCint* attribs ) START_API_FUNC {
    if ( deviceName ) {
        if ( !deviceName[ 0 ] ||
             al::strcasecmp( deviceName, alcDefaultName ) == 0 )
            deviceName = nullptr;
    ...
    try {
        newbackend =
            PlaybackFactory->createBackend( dev.get(), BackendType::Playback );
