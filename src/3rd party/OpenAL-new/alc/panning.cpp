...
std::unique_ptr< FrontStablizer > CreateStablizer( const size_t outchans,
                                                   const uint srate ) {
...
void AllocChannels( ALCdevice* device,
...
struct DecoderConfig< DualBand, 0 > {
    ...
    template < size_t N >
    DecoderConfig& operator=(
        const DecoderConfig< SingleBand, N >& rhs ) noexcept {
        mOrder = rhs.mOrder;
        mIs3D = rhs.mIs3D;
    ...
    template < size_t N >
    DecoderConfig& operator=(
        const DecoderConfig< DualBand, N >& rhs ) noexcept {
        mOrder = rhs.mOrder;
        mIs3D = rhs.mIs3D;
        mChannels = rhs.mChannels;
        mScaling = rhs.mScaling;
...
void InitNearFieldCtrl( ALCdevice* device,
    ...
    if ( !device->getConfigValueBool( "decoder", "nfc", false ) ||
         !( ctrl_dist > 0.0f ) )
        return;
    ...
    auto iter =
        std::copy_n( is3d ? chans_per_order3d : chans_per_order2d, order + 1u,
                     std::begin( device->NumChannelsPerOrder ) );
    std::fill( iter, std::end( device->NumChannelsPerOrder ), 0u );
}

void InitDistanceComp(
    ALCdevice* device,
    const al::span< const Channel > channels,
    const al::span< const float, MAX_OUTPUT_CHANNELS > dists ) {
    const float maxdist{
        std::accumulate( std::begin( dists ), std::end( dists ), 0.0f, maxf ) };

    if ( !device->getConfigValueBool( "decoder", "distance-comp", true ) ||
         !( maxdist > 0.0f ) )
        return;
    ...
    for ( size_t chidx{ 0 }; chidx < channels.size(); ++chidx ) {
         ...
         */
        float delay{
            std::floor( ( maxdist - distance ) * distSampleScale + 0.5f ) };
        if ( delay > float{ DistanceComp::MaxDelay - 1 } ) {
            ERR( "Delay for channel %u (%s) exceeds buffer length (%f > %d)\n",
                 idx, GetLabelFromChannel( ch ), delay,
                 DistanceComp::MaxDelay - 1 );
        ...
        TRACE( "Channel %s distance comp: %u samples, %f gain\n",
               GetLabelFromChannel( ch ), ChanDelay[ idx ].Length,
               ChanDelay[ idx ].Gain );
...
DecoderView MakeDecoderView(
    ...
    decoder.mOrder = ( conf->ChanMask > Ambi3OrderMask )   ? uint8_t{ 4 }
                     ...
                     : ( conf->ChanMask > Ambi1OrderMask ) ? uint8_t{ 2 }
                                                           : uint8_t{ 1 };
    decoder.mIs3D = ( conf->ChanMask & AmbiPeriphonicMask ) != 0;

    switch ( conf->CoeffScale ) {
        case AmbDecScale::Unset:
            ASSUME( false );
            break;
        case AmbDecScale::N3D:
            decoder.mScaling = DevAmbiScaling::N3D;
            break;
        case AmbDecScale::SN3D:
            decoder.mScaling = DevAmbiScaling::SN3D;
            break;
        case AmbDecScale::FuMa:
            decoder.mScaling = DevAmbiScaling::FuMa;
            break;
    }

    std::copy_n( std::begin( conf->HFOrderGain ),
                 std::min( al::size( conf->HFOrderGain ),
                           al::size( decoder.mOrderGain ) ),
                 std::begin( decoder.mOrderGain ) );
    std::copy_n( std::begin( conf->LFOrderGain ),
                 std::min( al::size( conf->LFOrderGain ),
                           al::size( decoder.mOrderGainLF ) ),
                 std::begin( decoder.mOrderGainLF ) );

    const auto num_coeffs = decoder.mIs3D
                                ? AmbiChannelsFromOrder( decoder.mOrder )
    ...
    for ( auto& speaker :
          const_speaker_span{ conf->Speakers.get(), conf->NumSpeakers } ) {
        ...
        if ( speaker.Name == "LF" )
            ...
            int idx{};
            char c{};
            if ( sscanf( speaker.Name.c_str(), "AUX%d%c", &idx, &c ) != 1 ||
                 idx < 0 || idx >= MaxChannels - Aux0 ) {
        ...
        for ( size_t dst{ 0 }; dst < num_coeffs; ++dst ) {
            ...
            decoder.mCoeffs[ chan_count ][ dst ] =
                hfmatrix[ chan_count ][ src ];
        }
        if ( conf->FreqBands > 1 ) {
            for ( size_t dst{ 0 }; dst < num_coeffs; ++dst ) {
                const size_t src{ idx_map[ dst ] };
                decoder.mCoeffsLF[ chan_count ][ dst ] =
                    lfmatrix[ chan_count ][ src ];
            }
        }
        ++chan_count;
    }
...
constexpr DecoderConfig< SingleBand, 1 > MonoConfig{ 0,
                                                     false,
                                                     { { FrontCenter } },
                                                     DevAmbiScaling::N3D,
                                                     { { 1.0f } },
...
constexpr DecoderConfig< DualBand, 4 > QuadConfig{
    1,
    false,
    { { BackLeft, FrontLeft, FrontRight, BackRight } },
    DevAmbiScaling::N3D,
...
constexpr DecoderConfig< DualBand, 5 > X51Config{
    2,
    false,
    { { SideLeft, FrontLeft, FrontCenter, FrontRight, SideRight } },
    DevAmbiScaling::FuMa,
    /*HF*/ { { 1.00000000e+0f, 1.00000000e+0f, 1.00000000e+0f } },
    { {
        { { 5.67316000e-1f, 4.22920000e-1f, -3.15495000e-1f, -6.34490000e-2f,
            -2.92380000e-2f } },
        { { 3.68584000e-1f, 2.72349000e-1f, 3.21616000e-1f, 1.92645000e-1f,
            4.82600000e-2f } },
        { { 1.83579000e-1f, 0.00000000e+0f, 1.99588000e-1f, 0.00000000e+0f,
            9.62820000e-2f } },
        { { 3.68584000e-1f, -2.72349000e-1f, 3.21616000e-1f, -1.92645000e-1f,
            4.82600000e-2f } },
        { { 5.67316000e-1f, -4.22920000e-1f, -3.15495000e-1f, 6.34490000e-2f,
    ...
    { {
        { { 4.90109850e-1f, 3.77305010e-1f, -3.73106990e-1f, -1.25914530e-1f,
            1.45133000e-2f } },
        { { 1.49085730e-1f, 3.03561680e-1f, 1.53290060e-1f, 2.45112480e-1f,
            -1.50753130e-1f } },
        { { 1.37654920e-1f, 0.00000000e+0f, 4.49417940e-1f, 0.00000000e+0f,
            2.57844070e-1f } },
        { { 1.49085730e-1f, -3.03561680e-1f, 1.53290060e-1f, -2.45112480e-1f,
            -1.50753130e-1f } },
        { { 4.90109850e-1f, -3.77305010e-1f, -3.73106990e-1f, 1.25914530e-1f,
...
constexpr DecoderConfig< SingleBand, 5 > X61Config{
    2,
    false,
    { { SideLeft, FrontLeft, FrontRight, SideRight, BackCenter } },
    DevAmbiScaling::N3D,
    { { 1.0f, 1.0f, 1.0f } },
    { {
        { { 2.04460341e-1f, 2.17177926e-1f, -4.39996780e-2f, -2.60790269e-2f,
            -6.87239792e-2f } },
        { { 1.58923161e-1f, 9.21772680e-2f, 1.59658796e-1f, 6.66278083e-2f,
            3.84686854e-2f } },
        { { 1.58923161e-1f, -9.21772680e-2f, 1.59658796e-1f, -6.66278083e-2f,
            3.84686854e-2f } },
        { { 2.04460341e-1f, -2.17177926e-1f, -4.39996780e-2f, 2.60790269e-2f,
            -6.87239792e-2f } },
        { { 2.50001688e-1f, 0.00000000e+0f, -2.50000094e-1f, 0.00000000e+0f,
            6.05133395e-2f } },
    } } };
constexpr DecoderConfig< DualBand, 6 > X71Config{
    2,
    false,
    { { BackLeft, SideLeft, FrontLeft, FrontRight, SideRight, BackRight } },
    DevAmbiScaling::N3D,
    /*HF*/ { { 1.41421356e+0f, 1.22474487e+0f, 7.07106781e-1f } },
    { {
        { { 1.66666667e-1f, 9.62250449e-2f, -1.66666667e-1f, -1.49071198e-1f,
            8.60662966e-2f } },
        { { 1.66666667e-1f, 1.92450090e-1f, 0.00000000e+0f, 0.00000000e+0f,
            -1.72132593e-1f } },
        { { 1.66666667e-1f, 9.62250449e-2f, 1.66666667e-1f, 1.49071198e-1f,
            8.60662966e-2f } },
        { { 1.66666667e-1f, -9.62250449e-2f, 1.66666667e-1f, -1.49071198e-1f,
...
constexpr DecoderConfig< DualBand, 6 > X3D71Config{
    1,
    true,
    { { Aux0, SideLeft, FrontLeft, FrontRight, SideRight, Aux1 } },
    DevAmbiScaling::N3D,
    /*HF*/ { { 1.73205081e+0f, 1.00000000e+0f } },
    { {
        { { 1.666666667e-01f, 0.000000000e+00f, 2.356640879e-01f,
            -1.667265410e-01f } },
        { { 1.666666667e-01f, 2.033043281e-01f, -1.175581508e-01f,
            -1.678904388e-01f } },
        { { 1.666666667e-01f, 2.033043281e-01f, 1.175581508e-01f,
            1.678904388e-01f } },
        { { 1.666666667e-01f, -2.033043281e-01f, 1.175581508e-01f,
...
constexpr DecoderConfig< SingleBand, 10 > X714Config{
    1,
    true,
    { { FrontLeft, FrontRight, SideLeft, SideRight, BackLeft, BackRight,
        TopFrontLeft, TopFrontRight, TopBackLeft, TopBackRight } },
    DevAmbiScaling::N3D,
    { { 1.00000000e+0f, 1.00000000e+0f, 1.00000000e+0f } },
    { {
        { { 1.27149251e-01f, 7.63047539e-02f, -3.64373750e-02f,
            1.59700680e-01f } },
        { { 1.07005418e-01f, -7.67638760e-02f, -4.92129762e-02f,
            1.29012797e-01f } },
        { { 1.26400196e-01f, 1.77494694e-01f, -3.71203389e-02f,
            0.00000000e+00f } },
        { { 1.26396516e-01f, -1.77488059e-01f, -3.71297878e-02f,
            0.00000000e+00f } },
        { { 1.06996956e-01f, 7.67615256e-02f, -4.92166307e-02f,
            -1.29001640e-01f } },
        { { 1.27145671e-01f, -7.63003471e-02f, -3.64353304e-02f,
            -1.59697510e-01f } },
        { { 8.80919747e-02f, 7.48940670e-02f, 9.08786244e-02f,
            6.22527183e-02f } },
        { { 1.57880745e-01f, -7.28755272e-02f, 1.82364187e-01f,
            8.74240284e-02f } },
        { { 1.57892225e-01f, 7.28944768e-02f, 1.82363474e-01f,
            -8.74301086e-02f } },
        { { 8.80892603e-02f, -7.48948724e-02f, 9.08779842e-02f,
            -6.22480443e-02f } },
    } } };

void InitPanning( ALCdevice* device,
                  const bool hqdec = false,
                  const bool stablize = false,
                  DecoderView decoder = {} ) {
    if ( !decoder ) {
        switch ( device->FmtChans ) {
            ...
            case DevFmtAmbi3D:
                ...
                const size_t count{
                    AmbiChannelsFromOrder( device->mAmbiOrder ) };
                std::transform(
                    acnmap.begin(), acnmap.begin() + count,
                ...
                float avg_dist{};
                if ( auto distopt = device->configValue< float >(
                         "decoder", "speaker-dist" ) )
                    avg_dist = *distopt;
                else if ( auto delayopt = device->configValue< float >(
                              "decoder", "nfc-ref-delay" ) ) {
                    WARN(
                        "nfc-ref-delay is deprecated, use speaker-dist "
                        "instead\n" );
    ...
    const size_t ambicount{ decoder.mIs3D
                                ? AmbiChannelsFromOrder( decoder.mOrder )
                                : Ambi2DChannelsFromOrder( decoder.mOrder ) };
    ...
    for ( size_t i{ 0u }; i < decoder.mChannels.size(); ++i ) {
        ...
        for ( size_t ambichan{ 0 }; ambichan < ambicount; ++ambichan )
            dst[ ambichan ] =
                src[ ambichan ] * decoder.mOrderGain[ ordermap[ ambichan ] ];

        if ( !dual_band )
            continue;

        chancoeffslf.resize( maxz( chancoeffslf.size(), idx + 1u ),
                             ChannelDec{} );
        src = decoder.mCoeffsLF[ i ];
        dst = chancoeffslf[ idx ];
        for ( size_t ambichan{ 0 }; ambichan < ambicount; ++ambichan )
            dst[ ambichan ] =
                src[ ambichan ] * decoder.mOrderGainLF[ ordermap[ ambichan ] ];
    }
    ...
    std::transform( acnmap.begin(), acnmap.end(),
                    std::begin( device->Dry.AmbiMap ),
    ...
    if ( stablize ) {
        ...
        if ( !hasfc ) {
            stablizer =
                CreateStablizer( device->channelsFromFmt(), device->Frequency );
    ...
    TRACE( "Enabling %s-band %s-order%s ambisonic decoder\n",
           ...
           : ( decoder.mOrder > 1 ) ? "second"
                                    : "first",
           decoder.mIs3D ? " periphonic" : "" );
    device->AmbiDecoder = BFormatDec::Create(
        ambicount, chancoeffs, chancoeffslf,
...
void InitHrtfPanning( ALCdevice* device ) {
    ...
    static const float AmbiMatrix1O[][ MaxAmbiChannels ]{
        { 1.250000000e-01f, 1.250000000e-01f, 1.250000000e-01f,
          1.250000000e-01f },
        { 1.250000000e-01f, 1.250000000e-01f, 1.250000000e-01f,
          -1.250000000e-01f },
        { 1.250000000e-01f, -1.250000000e-01f, 1.250000000e-01f,
    ...
    static const float AmbiOrderHFGain1O[ MaxAmbiOrder + 1 ]{
        ...
        AmbiOrderHFGain3O[ MaxAmbiOrder + 1 ]{
            ...
            /*AMP*/ 1.000000000e+00f, 8.611363116e-01f, 6.123336207e-01f,
            3.047469850e-01f
            /*RMS   8.340921354e-01f, 7.182670250e-01f, 5.107426573e-01f, 2.541870634e-01f*/
        };

    static_assert( al::size( AmbiPoints1O ) == al::size( AmbiMatrix1O ),
                   "First-Order Ambisonic HRTF mismatch" );
    static_assert( al::size( AmbiPoints2O ) == al::size( AmbiMatrix2O ),
                   "Second-Order Ambisonic HRTF mismatch" );
    static_assert( al::size( AmbiPoints3O ) == al::size( AmbiMatrix3O ),
    ...
    uint ambi_order{ 1 };
    if ( auto modeopt =
             device->configValue< std::string >( nullptr, "hrtf-mode" ) ) {
        ...
        auto iter = std::find_if( std::begin( hrtf_modes ),
                                  std::end( hrtf_modes ), match_entry );
    ...
    al::span< const float, MaxAmbiOrder + 1 > AmbiOrderHFGain{
        AmbiOrderHFGain1O };
    if ( ambi_order >= 3 ) {
        perHrirMin = true;
    ...
    std::transform(
        AmbiIndex::FromACN().begin(), AmbiIndex::FromACN().begin() + count,
        std::begin( device->Dry.AmbiMap ), []( const uint8_t& index ) noexcept {
            return BFChannelConfig{ 1.0f, index };
        } );
    AllocChannels( device, count, device->channelsFromFmt() );

    HrtfStore* Hrtf{ device->mHrtf.get() };
    auto hrtfstate = DirectHrtfState::Create( count );
    hrtfstate->build( Hrtf, device->mIrSize, perHrirMin, AmbiPoints, AmbiMatrix,
                      device->mXOverFreq, AmbiOrderHFGain );
...
void InitUhjPanning( ALCdevice* device ) {
    ...
    std::transform(
        acnmap_begin, acnmap_begin + count, std::begin( device->Dry.AmbiMap ),
        []( const uint8_t& acn ) noexcept -> BFChannelConfig {
...
void aluInitRenderer( ALCdevice* device,
    ...
    if ( device->FmtChans != DevFmtStereo ) {
        ...
        std::unique_ptr< DecoderConfig< DualBand, MAX_OUTPUT_CHANNELS > >
            decoder_store;
        DecoderView decoder{};
        float speakerdists[ MAX_OUTPUT_CHANNELS ]{};
        auto load_config = [ device, &decoder_store, &decoder,
                             &speakerdists ]( const char* config ) {
            AmbDecConf conf{};
        ...
        if ( layout ) {
            if ( auto decopt =
                     device->configValue< std::string >( "decoder", layout ) )
         ...
         */
        const bool stablize{
            device->RealOut.ChannelIndex[ FrontCenter ] !=
                InvalidChannelIndex &&
            device->RealOut.ChannelIndex[ FrontLeft ] != InvalidChannelIndex &&
            device->RealOut.ChannelIndex[ FrontRight ] != InvalidChannelIndex &&
            device->getConfigValueBool( nullptr, "front-stablizer", false ) !=
                0 };
        const bool hqdec{
            device->getConfigValueBool( "decoder", "hq-mode", true ) != 0 };
        ...
        if ( decoder ) {
            ...
            const float avg_dist{
                ( accum_dist > 0.0f && spkr_count > 0 )
                    ? accum_dist / spkr_count
                    : device->configValue< float >( "decoder", "speaker-dist" )
                          .value_or( 1.0f ) };
            InitNearFieldCtrl( device, avg_dist, decoder.mOrder,
                               decoder.mIs3D );
        ...
        if ( auto* ambidec{ device->AmbiDecoder.get() } ) {
            device->PostProcess = ambidec->hasStablizer()
     ...
     */
    if ( stereomode.value_or( StereoEncoding::Default ) ==
             StereoEncoding::Hrtf ||
        ...
        if ( hrtf_id >= 0 &&
             static_cast< uint >( hrtf_id ) < device->mHrtfList.size() ) {
            const std::string& hrtfname =
                device->mHrtfList[ static_cast< uint >( hrtf_id ) ];
            if ( HrtfStorePtr hrtf{
                     GetLoadedHrtf( hrtfname, device->Frequency ) } ) {
        ...
        if ( device->mHrtf ) {
            ...
            if ( auto hrtfsizeopt =
                     device->configValue< uint >( nullptr, "hrtf-size" ) ) {
    ...
    if ( stereomode.value_or( StereoEncoding::Default ) ==
        ...
        switch ( UhjEncodeQuality ) {
            ...
            case UhjQualityType::FIR256:
                device->mUhjEncoder =
                    std::make_unique< UhjEncoder< UhjLength256 > >();
                break;
            case UhjQualityType::FIR512:
                device->mUhjEncoder =
                    std::make_unique< UhjEncoder< UhjLength512 > >();
                break;
        }
    ...
    if ( device->Type != DeviceType::Loopback ) {
        if ( auto cflevopt =
                 device->configValue< int >( nullptr, "cf_level" ) ) {
...
void aluInitEffectPanning( EffectSlot* slot, ALCcontext* context ) {
    ...
    auto iter = std::transform(
        acnmap_begin, acnmap_begin + count, slot->Wet.AmbiMap.begin(),
        []( const uint8_t& acn ) noexcept -> BFChannelConfig {
