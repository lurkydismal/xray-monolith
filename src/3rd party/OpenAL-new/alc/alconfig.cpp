...
std::string expdup( const char* str ) {
    ...
    while ( *str != '\0' ) {
        ...
        if ( str[ 0 ] != '$' ) {
            ...
            addstr = str;
            addstrlen =
                next ? static_cast< size_t >( next - str ) : std::strlen( str );
            ...
            if ( *str == '$' ) {
                ...
                addstr = str;
                addstrlen = next ? static_cast< size_t >( next - str )
                                 : std::strlen( str );
                ...
                envval =
                    al::getenv( envname.c_str() ).value_or( std::string{} );
                addstr = envval.data();
...
void LoadConfigFromFile( std::istream& f ) {
    ...
    while ( readline( f, buffer ) ) {
        ...
        std::string value{ ( sep < buffer.size() ) ? buffer.substr( sep )
                                                   : std::string{} };
        if ( value.size() > 1 ) {
        ...
        if ( ent != ConfOpts.end() ) {
            ...
            ConfOpts.emplace_back(
                ConfigEntry{ std::move( fullKey ), expdup( value.c_str() ) } );
    }
    ConfOpts.shrink_to_fit();
}

const char* GetConfigValue( const char* devName,
...
void ReadALConfig() {
    ...
    if ( mainBundle ) {
        ...
        if ( ( configURL = CFBundleCopyResourceURL(
                   mainBundle, CFSTR( ".alsoftrc" ), CFSTR( "" ), nullptr ) ) &&
             CFURLGetFileSystemRepresentation( configURL, true, fileName,
                                               sizeof( fileName ) ) ) {
...
al::optional< std::string > ConfigValueStr( const char* devName,
                                            const char* blockName,
                                            const char* keyName ) {
...
al::optional< int > ConfigValueInt( const char* devName,
                                    const char* blockName,
                                    const char* keyName ) {
    if ( const char* val{ GetConfigValue( devName, blockName, keyName ) } )
...
al::optional< unsigned int > ConfigValueUInt( const char* devName,
                                              const char* blockName,
                                              const char* keyName ) {
    if ( const char* val{ GetConfigValue( devName, blockName, keyName ) } )
...
al::optional< float > ConfigValueFloat( const char* devName,
                                        const char* blockName,
                                        const char* keyName ) {
    if ( const char* val{ GetConfigValue( devName, blockName, keyName ) } )
...
al::optional< bool > ConfigValueBool( const char* devName,
                                      const char* blockName,
                                      const char* keyName ) {
    if ( const char* val{ GetConfigValue( devName, blockName, keyName ) } )
        return al::strcasecmp( val, "on" ) == 0 ||
               al::strcasecmp( val, "yes" ) == 0 ||
               al::strcasecmp( val, "true" ) == 0 || atoi( val ) != 0;
    return al::nullopt;
}

bool GetConfigValueBool( const char* devName,
                         const char* blockName,
                         const char* keyName,
                         bool def ) {
    if ( const char* val{ GetConfigValue( devName, blockName, keyName ) } )
