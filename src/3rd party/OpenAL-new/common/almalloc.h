...
#define DEF_PLACE_NEWDEL()                                        \
    ...
    void operator delete( void* block, void* ) noexcept {         \
        al_free( block );                                         \
    }                                                             \
    void operator delete( void* block ) noexcept {                \
        al_free( block );                                         \
...
#define DEF_FAM_NEWDEL( T, FamMem )                                       \
    ...
    void operator delete( void* block, FamCount ) {                       \
        al_free( block );                                                 \
    }                                                                     \
    void operator delete( void* block ) noexcept {                        \
        al_free( block );                                                 \
...
struct allocator {
    ...
    T* allocate( std::size_t n ) {
        if ( n > std::numeric_limits< std::size_t >::max() / sizeof( T ) )
            throw std::bad_alloc();
        if ( auto p = al_malloc( alignment, n * sizeof( T ) ) )
 ...
 */
DIAGNOSTIC_PUSH
msc_pragma( warning( disable : 4100 ) ) template < typename T >
constexpr std::enable_if_t< !std::is_array< T >::value > destroy_at(
...
destroy_at( T* ptr ) noexcept(
    std::is_nothrow_destructible< std::remove_all_extents_t< T > >::value ) {
    for ( auto& elem : *ptr )
        al::destroy_at( std::addressof( elem ) );
}

template < typename T >
constexpr void destroy( T first, T end ) noexcept(
    noexcept( al::destroy_at( std::addressof( *first ) ) ) ) {
    while ( first != end ) {
...
destroy_n( T first, N count ) noexcept(
    noexcept( al::destroy_at( std::addressof( *first ) ) ) ) {
    if ( count != 0 ) {
        do {
            al::destroy_at( std::addressof( *first ) );
...
struct FlexArrayStorage {
    ...
    static constexpr size_t Sizeof( size_t count, size_t base = 0u ) noexcept {
        ...
        return std::max( offsetof( FlexArrayStorage, mArray ) + len,
...
struct FlexArray {
    ...
    static constexpr index_type Sizeof( index_type count,
    ...
    static std::unique_ptr< FlexArray > Create( index_type count ) {
        ...
        return std::unique_ptr< FlexArray >{
            al::construct_at( static_cast< FlexArray* >( ptr ), count ) };
    }
