...
template < size_t N >
struct BitReverser {
    static_assert( N <= sizeof( ushort ) * 8,
                   "Too many bits for the bit-reversal table." );
...
std::enable_if_t< std::is_floating_point< Real >::value > complex_fft(
    ...
    if ( log2_size >= gBitReverses.size() )
        UNLIKELY {
            for ( size_t idx{ 1u }; idx < fftsize - 1; ++idx ) {
                size_t revidx{ 0u }, imask{ idx };
...
void complex_hilbert( const al::span< std::complex< double > > buffer ) {
    ...
    *bufiter *= inverse_size;
    ++bufiter;
    bufiter =
        std::transform( bufiter, halfiter, bufiter,
                        [ scale = inverse_size * 2.0 ](
                            std::complex< double > d ) { return d * scale; } );
    *bufiter *= inverse_size;
    ++bufiter;
...
template void complex_fft<>( const al::span< std::complex< float > > buffer,
                             const float sign );
template void complex_fft<>( const al::span< std::complex< double > > buffer,
                             const double sign );
