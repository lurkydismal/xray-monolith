...
RingBufferPtr RingBuffer::Create( size_t sz,
                                  size_t elem_sz,
                                  int limit_writes ) {
    ...
    ++power_of_two;
    if ( power_of_two <= sz ||
         power_of_two > std::numeric_limits< size_t >::max() / elem_sz )
...
size_t RingBuffer::read( void* dest, size_t cnt ) noexcept {
    ...
    auto outiter =
        std::copy_n( mBuffer.begin() + read_ptr * mElemSize, n1 * mElemSize,
...
size_t RingBuffer::write( const void* src, size_t cnt ) noexcept {
    ...
    std::copy_n( srcbytes, n1 * mElemSize,
                 mBuffer.begin() + write_ptr * mElemSize );
    write_ptr += n1;
    if ( n2 > 0 ) {
        std::copy_n( srcbytes + n1 * mElemSize, n2 * mElemSize,
                     mBuffer.begin() );
        write_ptr += n2;
    }
...
auto RingBuffer::getReadVector() const noexcept -> DataPair {
    ...
    if ( cnt2 > mSizeMask + 1 ) {
        ...
        ret.first.buf =
            const_cast< al::byte* >( mBuffer.data() + r * mElemSize );
...
auto RingBuffer::getWriteVector() const noexcept -> DataPair {
    ...
    size_t w{ mWritePtr.load( std::memory_order_acquire ) };
    size_t r{ mReadPtr.load( std::memory_order_acquire ) + mWriteSize -
              mSizeMask };
    w &= mSizeMask;
    r &= mSizeMask;
    ...
    if ( cnt2 > mSizeMask + 1 ) {
         ...
         * plus some from the start of the buffer. */
        ret.first.buf =
            const_cast< al::byte* >( mBuffer.data() + w * mElemSize );
