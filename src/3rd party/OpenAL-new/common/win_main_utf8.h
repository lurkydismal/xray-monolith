...
#ifdef __cplusplus
    auto strbuf = std::make_unique< wchar_t[] >(
        static_cast< size_t >( namelen ) + modelen );
    wname = strbuf.get();
#else
    wname =
        ( wchar_t* )calloc( sizeof( wchar_t ), ( size_t )namelen + modelen );
#endif
    wmode = wname + namelen;
...
extern "C"
    ...
    for ( i = 0; i < argc; i++ )
        total += STATIC_CAST( size_t )( WideCharToMultiByte(
    ...
    for ( i = 0; i < argc - 1; i++ ) {
        int len = WideCharToMultiByte( CP_UTF8, 0, wargv[ i ], -1, argv[ i ],
                                       65535, NULL, NULL );
        argv[ i + 1 ] = argv[ i ] + len;
    }
    WideCharToMultiByte( CP_UTF8, 0, wargv[ i ], -1, argv[ i ], 65535, NULL,
                         NULL );
