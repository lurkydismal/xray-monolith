...
template < typename T >
constexpr auto size( const T& cont ) noexcept( noexcept( cont.size() ) )
    -> decltype( cont.size() ) {
    return cont.size();
}
...
template < typename T >
constexpr auto data( T& cont ) noexcept( noexcept( cont.data() ) )
    -> decltype( cont.data() ) {
    return cont.data();
}

template < typename T >
constexpr auto data( const T& cont ) noexcept( noexcept( cont.data() ) )
    -> decltype( cont.data() ) {
    return cont.data();
}
...
constexpr bool
    has_size_and_data< T,
                       void_t< decltype( al::size( std::declval< T >() ) ),
                               decltype( al::data( std::declval< T >() ) ) > > =
        true;

template < typename T, typename U >
constexpr bool is_array_compatible =
    std::is_convertible< T ( * )[], U ( * )[] >::value;

template < typename C, typename T >
constexpr bool is_valid_container =
    !is_span_v< C > && !is_std_array_v< C > && !std::is_array< C >::value &&
    has_size_and_data< C > &&
    is_array_compatible<
        std::remove_pointer_t< decltype( al::data( std::declval< C& >() ) ) >,
        T >;
} // namespace detail_
...
public:
    ...
    template < typename U,
               REQUIRES( detail_::is_valid_container< U, element_type > ) >
    constexpr explicit span( U&& cont )
        : span{ al::data( cont ), al::size( cont ) } {}
    ...
    constexpr auto subspan() const
        -> std::enable_if_t< C != dynamic_extent, span< element_type, C > > {
    ...
    constexpr auto subspan() const
        -> std::enable_if_t< C == dynamic_extent,
                             span< element_type, E - O > > {
        static_assert( E >= O, "Offset exceeds extent" );
...
public:
    ...
    constexpr span( U first, V last )
        : span{ to_address( first ), static_cast< size_t >( last - first ) } {}
...
template < typename T, size_t E >
constexpr inline auto span< T, E >::first( size_t count ) const
...
template < typename T, size_t E >
constexpr inline auto span< T, E >::last( size_t count ) const
    -> span< element_type, dynamic_extent > {
    return ( count >= size() )
               ? span< element_type >{ mData, extent }
...
constexpr inline auto span< T, E >::subspan( size_t offset, size_t count ) const
    ...
    return ( offset > size() ) ? span< element_type >{}
           : ( count >= size() - offset )
               ? span< element_type >{ mData + offset, mData + extent }
