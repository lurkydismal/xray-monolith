...
struct PhaseShifterT {
    ...
    static_assert( ( FilterSize & ( FilterSize - 1 ) ) == 0,
                   "FilterSize needs to be power-of-two" );
    ...
    PhaseShifterT() {
        ...
        for ( size_t i{ 0 }; i < half_size + 1; ++i )
            fftBuffer[ i ] =
                complex_d{ -fftBuffer[ i ].imag(), fftBuffer[ i ].real() };
        ...
        for ( float& coeff : mCoeffs ) {
            coeff =
                static_cast< float >( fftiter->real() / double{ fft_size } );
            fftiter -= 2;
        }
    }
...
template < size_t S >
inline void PhaseShifterT< S >::process( al::span< float > dst,
                                         const float* RESTRICT src ) const {
#ifdef HAVE_SSE_INTRINSICS
    if ( size_t todo{ dst.size() >> 1 } ) {
        ...
        do {
            ...
            __m128 r4{ _mm_add_ps( _mm_unpackhi_ps( r04, r14 ),
                                   _mm_unpacklo_ps( r04, r14 ) ) };
    ...
    if ( ( dst.size() & 1 ) ) {
        ...
        for ( size_t j{ 0 }; j < mCoeffs.size(); j += 4 ) {
            const __m128 coeffs{ _mm_load_ps( &mCoeffs[ j ] ) };
            const __m128 s{ _mm_setr_ps( src[ j * 2 ], src[ j * 2 + 2 ],
                                         src[ j * 2 + 4 ], src[ j * 2 + 6 ] ) };
    ...
    if ( size_t todo{ dst.size() >> 1 } ) {
        do {
            ...
            float32x4_t r4{
                vaddq_f32( unpackhi( r04, r14 ), unpacklo( r04, r14 ) ) };
            float32x2_t r2{
                vadd_f32( vget_low_f32( r4 ), vget_high_f32( r4 ) ) };
    ...
    if ( ( dst.size() & 1 ) ) {
        ...
        for ( size_t j{ 0 }; j < mCoeffs.size(); j += 4 ) {
            const float32x4_t coeffs{ vld1q_f32( &mCoeffs[ j ] ) };
            const float32x4_t s{ load4( src[ j * 2 ], src[ j * 2 + 2 ],
                                        src[ j * 2 + 4 ], src[ j * 2 + 6 ] ) };
            r4 = vmlaq_f32( r4, s, coeffs );
        }
        r4 = vaddq_f32( r4, vrev64q_f32( r4 ) );
        dst[ pos ] = vget_lane_f32(
