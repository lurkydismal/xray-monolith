...
std::array< float, MaxAmbiChannels > CalcAmbiCoeffs( const float y,
    ...
    if ( spread > 0.0f ) {
         ...
         * ZH5 = -0.0625*sqrt(pi) * (-1+ca)*(ca+1)*(21*ca*ca*ca*ca - 14*ca*ca +
         * 1);
         *
...
void ComputePanGains( const MixParams* mix,
    ...
    auto iter = std::transform(
        ambimap, ambimap + mix->Buffer.size(), gains.begin(),
