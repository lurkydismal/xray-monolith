...
void BiquadFilterR< Real >::setParams( BiquadType type,
                                       Real f0norm,
                                       Real gain,
                                       Real rcpQ ) {
    ...
    switch ( type ) {
        case BiquadType::HighShelf:
            ...
            b[ 0 ] = gain * ( ( gain + 1.0f ) + ( gain - 1.0f ) * cos_w0 +
                              sqrtgain_alpha_2 );
            b[ 1 ] =
                -2.0f * gain * ( ( gain - 1.0f ) + ( gain + 1.0f ) * cos_w0 );
            b[ 2 ] = gain * ( ( gain + 1.0f ) + ( gain - 1.0f ) * cos_w0 -
                              sqrtgain_alpha_2 );
            a[ 0 ] =
                ( gain + 1.0f ) - ( gain - 1.0f ) * cos_w0 + sqrtgain_alpha_2;
...
template < typename Real >
void BiquadFilterR< Real >::process( const al::span< const Real > src,
                                     Real* dst ) {
    const Real b0{ mB0 };
    const Real b1{ mB1 };
    const Real b2{ mB2 };
     ...
     */
    auto proc_sample = [ b0, b1, b2, a1, a2, &z1,
                         &z2 ]( Real input ) noexcept -> Real {
...
template < typename Real >
void BiquadFilterR< Real >::dualProcess( BiquadFilterR& other,
                                         const al::span< const Real > src,
                                         Real* dst ) {
    const Real b00{ mB0 };
    const Real b01{ mB1 };
    const Real b02{ mB2 };
    ...
    auto proc_sample = [ b00, b01, b02, a01, a02, b10, b11, b12, a11, a12, &z01,
                         &z02, &z11, &z12 ]( Real input ) noexcept -> Real {
