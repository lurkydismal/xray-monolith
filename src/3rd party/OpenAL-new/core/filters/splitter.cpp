...
template < typename Real >
void BandSplitterR< Real >::process( const al::span< const Real > input,
                                     Real* hpout,
                                     Real* lpout ) {
    ...
    Real lp_z2{ mLpZ2 };
    Real ap_z1{ mApZ1 };
    auto proc_sample = [ ap_coeff, lp_coeff, &lp_z1, &lp_z2, &ap_z1,
                         &lpout ]( const Real in ) noexcept -> Real {
...
void BandSplitterR< Real >::processHfScale( const al::span< const Real > input,
    ...
    Real lp_z2{ mLpZ2 };
    Real ap_z1{ mApZ1 };
    auto proc_sample = [ hfscale, ap_coeff, lp_coeff, &lp_z1, &lp_z2,
                         &ap_z1 ]( const Real in ) noexcept -> Real {
        /* Low-pass sample processing. */
        Real d{ ( in - lp_z1 ) * lp_coeff };
        Real lp_y{ lp_z1 + d };
        lp_z1 = lp_y + d;
...
template < typename Real >
void BandSplitterR< Real >::processHfScale( const al::span< Real > samples,
                                            const Real hfscale ) {
    const Real ap_coeff{ mCoeff };
    const Real lp_coeff{ mCoeff * 0.5f + 0.5f };
    Real lp_z1{ mLpZ1 };
...
template < typename Real >
void BandSplitterR< Real >::processScale( const al::span< Real > samples,
                                          const Real hfscale,
                                          const Real lfscale ) {
    const Real ap_coeff{ mCoeff };
    const Real lp_coeff{ mCoeff * 0.5f + 0.5f };
    ...
    Real lp_z2{ mLpZ2 };
    Real ap_z1{ mApZ1 };
    auto proc_sample = [ hfscale, lfscale, ap_coeff, lp_coeff, &lp_z1, &lp_z2,
                         &ap_z1 ]( const Real in ) noexcept -> Real {
        Real d{ ( in - lp_z1 ) * lp_coeff };
        Real lp_y{ lp_z1 + d };
        lp_z1 = lp_y + d;
