...
constexpr std::array< float, 4 > Filter1Coeff{
    { square( 0.6923878f ), square( 0.9360654322959f ),
      square( 0.9882295226860f ), square( 0.9987488452737f ) } };
...
constexpr std::array< float, 4 > Filter2Coeff{
    { square( 0.4021921162426f ), square( 0.8561710882420f ),
      square( 0.9722909545651f ), square( 0.9952884791278f ) } };
...
template < size_t N >
void UhjEncoder< N >::encode( float* LeftOut,
                              float* RightOut,
                              const al::span< const float* const, 3 > InSamples,
    ...
    std::transform( winput, winput + SamplesToDo, xinput,
                    mWX.begin() + sWXInOffset,
    ...
    std::copy( mW.cbegin() + SamplesToDo,
               mW.cbegin() + SamplesToDo + sFilterDelay, mW.begin() );
    std::copy( mX.cbegin() + SamplesToDo,
               mX.cbegin() + SamplesToDo + sFilterDelay, mX.begin() );
    std::copy( mY.cbegin() + SamplesToDo,
               mY.cbegin() + SamplesToDo + sFilterDelay, mY.begin() );
    std::copy( mWX.cbegin() + SamplesToDo,
               mWX.cbegin() + SamplesToDo + sWXInOffset, mWX.begin() );
    ...
    for ( float* buffer : { LeftOut, RightOut } ) {
        ...
        if ( SamplesToDo >= sFilterDelay )
            LIKELY {
                auto delay_end =
                    std::rotate( inout, inout_end - sFilterDelay, inout_end );
 ...
 * inputs.
 */
void UhjEncoderIIR::encode( float* LeftOut,
                            float* RightOut,
                            const al::span< const float* const, 3 > InSamples,
                            const size_t SamplesToDo ) {
    ASSUME( SamplesToDo > 0 );
    ...
    mFilter1WX.process( Filter1Coeff, { mTemp.data(), SamplesToDo }, true,
                        mS.data() + 1 );
    mS[ 0 ] = mDelayWX;
    mDelayWX = mS[ SamplesToDo ];
    ...
    mFilter2WX.process( Filter2Coeff, { mTemp.data(), SamplesToDo }, true,
                        mWX.data() );

    /* Apply filter1 to Y and store in mD. */
    mFilter1Y.process( Filter1Coeff, { yinput, SamplesToDo }, SamplesToDo,
                       mD.data() + 1 );
    mD[ 0 ] = mDelayY;
    mDelayY = mD[ SamplesToDo ];
    ...
    mFilter1Direct[ 0 ].process( Filter1Coeff, { LeftOut, SamplesToDo }, true,
                                 mTemp.data() + 1 );
    mTemp[ 0 ] = mDirectDelay[ 0 ];
    mDirectDelay[ 0 ] = mTemp[ SamplesToDo ];
    ...
    mFilter1Direct[ 1 ].process( Filter1Coeff, { RightOut, SamplesToDo }, true,
                                 mTemp.data() + 1 );
    mTemp[ 0 ] = mDirectDelay[ 1 ];
    mDirectDelay[ 1 ] = mTemp[ SamplesToDo ];
 ...
 */
template < size_t N >
void UhjDecoder< N >::decode( const al::span< float* > samples,
                              const size_t samplesToDo,
                              const bool updateState ) {
    static_assert( sInputPadding <= sMaxPadding,
    ...
    std::transform( mD.cbegin(), mD.cbegin() + samplesToDo + sInputPadding,
                    mT.cbegin(), tmpiter,
    ...
    if ( updateState )
        LIKELY
    std::copy_n( mTemp.cbegin() + samplesToDo, mDTHistory.size(),
                 mDTHistory.begin() );
    ...
    if ( updateState )
        LIKELY
    std::copy_n( mTemp.cbegin() + samplesToDo, mSHistory.size(),
                 mSHistory.begin() );
    PShift.process( { youtput, samplesToDo }, mTemp.data() );

    /* Y = 0.795968*D - 0.676392*T + j(0.186633*S) */
    for ( size_t i{ 0 }; i < samplesToDo; ++i )
        youtput[ i ] = 0.795968f * mD[ i ] - 0.676392f * mT[ i ] +
                       0.186633f * youtput[ i ];
...
void UhjDecoderIIR::decode( const al::span< float* > samples,
                            const size_t samplesToDo,
                            const bool updateState ) {
    static_assert( sInputPadding <= sMaxPadding,
                   "Filter padding is too large" );
    ...
    /* Precompute j(0.828331*D + 0.767820*T) and store in xoutput. */
    std::transform( mD.cbegin(), mD.cbegin() + samplesToDo, youtput,
                    mTemp.begin(), []( const float d, const float t ) noexcept {
                        return 0.828331f * d + 0.767820f * t;
                    } );
    mFilter2DT.process( Filter2Coeff, { mTemp.data(), samplesToDo },
                        updateState, xoutput );

    /* Apply filter1 to S and store in mTemp. */
    mTemp[ 0 ] = mDelayS;
    mFilter1S.process( Filter1Coeff, { mS.data(), samplesToDo }, updateState,
                       mTemp.data() + 1 );
    if ( updateState )
        LIKELY mDelayS = mTemp[ samplesToDo ];
    ...
    std::transform( mD.cbegin(), mD.cbegin() + samplesToDo, youtput, youtput,
                    []( const float d, const float t ) noexcept {
                        return 0.795968f * d - 0.676392f * t;
                    } );
    mTemp[ 0 ] = mDelayDT;
    mFilter1DT.process( Filter1Coeff, { youtput, samplesToDo }, updateState,
                        mTemp.data() + 1 );
    if ( updateState )
        LIKELY mDelayDT = mTemp[ samplesToDo ];

    /* Precompute j*S and store in youtput. */
    mFilter2S.process( Filter2Coeff, { mS.data(), samplesToDo }, updateState,
                       youtput );
    ...
    if ( samples.size() > 3 ) {
        ...
        mTemp[ 0 ] = mDelayQ;
        mFilter1Q.process( Filter1Coeff, { zoutput, samplesToDo }, updateState,
                           mTemp.data() + 1 );
        if ( updateState )
            LIKELY mDelayQ = mTemp[ samplesToDo ];
 ...
 */
template < size_t N >
void UhjStereoDecoder< N >::decode( const al::span< float* > samples,
                                    const size_t samplesToDo,
                                    const bool updateState ) {
    static_assert( sInputPadding <= sMaxPadding,
                   "Filter padding is too large" );
...

    {
        ...
        const float wcurrent{ ( mCurrentWidth < 0.0f ) ? wtarget
                                                       : mCurrentWidth };
        if ( wtarget == wcurrent || !updateState ) {
            ...
            const float wstep{ ( wtarget - wcurrent ) /
                               static_cast< float >( samplesToDo ) };
            float fi{ 0.0f };
            for ( size_t i{ 0 }; i < samplesToDo; ++i ) {
                mD[ i ] =
                    ( left[ i ] - right[ i ] ) * ( wcurrent + wstep * fi );
                fi += 1.0f;
            }
            for ( size_t i{ samplesToDo }; i < samplesToDo + sInputPadding;
                  ++i )
...
void UhjStereoDecoderIIR::decode( const al::span< float* > samples,
                                  const size_t samplesToDo,
                                  const bool updateState ) {
    static_assert( sInputPadding <= sMaxPadding,
                   "Filter padding is too large" );
    ...
    /* Precompute j*D and store in xoutput. */
    mFilter2D.process( Filter2Coeff, { mD.data(), samplesToDo }, updateState,
                       xoutput );
    ...
    mTemp[ 0 ] = mDelayD;
    mFilter1D.process( Filter1Coeff, { mD.data(), samplesToDo }, updateState,
                       mTemp.data() + 1 );
    if ( updateState )
        LIKELY mDelayD = mTemp[ samplesToDo ];
