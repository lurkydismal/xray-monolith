...
static_assert( ( BufferLineSize & ( BufferLineSize - 1 ) ) == 0,
               "BufferLineSize is not a power of 2" );
...
void ShiftSlidingHold( SlidingHold* Hold, const uint n ) {
    ...
    std::transform( exp_begin, exp_last + 1, exp_begin,
                    [ n ]( uint e ) { return e - n; } );
}
 ...
 * channels.
 */
void LinkChannels( Compressor* Comp,
                   const uint SamplesToDo,
                   const FloatBufferLine* OutBuffer ) {
    ...
    auto fill_max = [ SamplesToDo,
                      side_begin ]( const FloatBufferLine& input ) -> void {
        const float* RESTRICT buffer{
            al::assume_aligned< 16 >( input.data() ) };
        auto max_abs = std::bind(
            maxf, _1,
            std::bind( static_cast< float ( & )( float ) >( std::fabs ), _2 ) );
        std::transform( side_begin, side_begin + SamplesToDo, buffer,
                        side_begin, max_abs );
    };
...
void CrestDetector( Compressor* Comp, const uint SamplesToDo ) {
    ...
    auto calc_crest = [ &y2_rms, &y2_peak,
                        a_crest ]( const float x_abs ) noexcept -> float {
    ...
    auto side_begin = std::begin( Comp->mSideChain ) + Comp->mLookAhead;
    std::transform( side_begin, side_begin + SamplesToDo,
                    std::begin( Comp->mCrestFactor ), calc_crest );
...
void PeakDetector( Compressor* Comp, const uint SamplesToDo ) {
    ...
    auto side_begin = std::begin( Comp->mSideChain ) + Comp->mLookAhead;
    std::transform(
        side_begin, side_begin + SamplesToDo, side_begin,
...
void GainCompressor( Compressor* Comp, const uint SamplesToDo ) {
    ...
    for ( float& sideChain :
          al::span< float >{ Comp->mSideChain, SamplesToDo } ) {
        if ( autoKnee )
         ...
         */
        const float x_over{ std::addressof( sideChain )[ lookAhead ] -
                            threshold };
        const float y_G{ ( x_over <= -knee_h ) ? 0.0f
                         : ( std::fabs( x_over ) < knee_h )
                             ? ( x_over + knee_h ) * ( x_over + knee_h ) /
                                   ( 2.0f * knee )
                             : x_over };
 ...
 * limiter.
 */
void SignalDelay( Compressor* Comp,
                  const uint SamplesToDo,
    ...
    for ( size_t c{ 0 }; c < numChans; c++ ) {
        ...
        if ( SamplesToDo >= lookAhead )
            LIKELY {
                auto delay_end =
                    std::rotate( inout, inout_end - lookAhead, inout_end );
...
std::unique_ptr< Compressor > Compressor::Create( const size_t NumChans,
    ...
    const auto lookAhead = static_cast< uint >( clampf(
        std::round( LookAheadTime * SampleRate ), 0.0f, BufferLineSize - 1 ) );
    const auto hold = static_cast< uint >( clampf(
        std::round( HoldTime * SampleRate ), 0.0f, BufferLineSize - 1 ) );
    ...
    auto Comp = CompressorPtr{ al::construct_at(
        static_cast< Compressor* >( al_calloc( 16, size ) ) ) };
    ...
    if ( lookAhead > 0 ) {
        if ( hold > 1 ) {
            Comp->mHold = al::construct_at(
                reinterpret_cast< SlidingHold* >( Comp.get() + 1 ) );
            Comp->mHold->mValues[ 0 ] =
                -std::numeric_limits< float >::infinity();
            Comp->mHold->mExpiries[ 0 ] = hold;
            Comp->mHold->mLength = hold;
            Comp->mDelay =
                reinterpret_cast< FloatBufferLine* >( Comp->mHold + 1 );
        } else
            Comp->mDelay =
                reinterpret_cast< FloatBufferLine* >( Comp.get() + 1 );
...
void Compressor::process( const uint SamplesToDo, FloatBufferLine* OutBuffer ) {
    ...
    if ( preGain != 1.0f ) {
        auto apply_gain = [ SamplesToDo, preGain ](
                              FloatBufferLine& input ) noexcept -> void {
    ...
    auto apply_comp =
        [ SamplesToDo, &sideChain ]( FloatBufferLine& input ) noexcept -> void {
        float* buffer{ al::assume_aligned< 16 >( input.data() ) };
