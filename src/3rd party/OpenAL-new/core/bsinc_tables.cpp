 ...
 */
constexpr double Kaiser( const double beta,
                         const double k,
 ...
 * Rejection is in dB.
 */
constexpr double CalcKaiserWidth( const double rejection,
                                  const uint order ) noexcept {
    if ( rejection > 21.19 )
...
struct BSincHeader {
    ...
    constexpr BSincHeader( uint Rejection, uint Order ) noexcept {
        ...
        for ( uint si{ 0 }; si < BSincScaleCount; ++si ) {
            const double scale{ scaleBase +
                                ( scaleRange * ( si + 1 ) / BSincScaleCount ) };
            const uint a_{ std::min(
                static_cast< uint >( num_points / 2.0 / scale ), num_points ) };
            const uint m{ 2 * a_ };
...
static_assert( BSincPointsMax <= MaxResamplerPadding,
...
struct BSincFilterArray {
    ...
    BSincFilterArray() {
        ...
        for ( uint si{ 0 }; si < BSincScaleCount; ++si ) {
            ...
            const double scale{ hdr.scaleBase + ( hdr.scaleRange * ( si + 1 ) /
                                                  BSincScaleCount ) };
            const double cutoff{
                scale - ( hdr.scaleBase * std::max( 1.0, scale * 2.0 ) ) };
            ...
            for ( uint pi{ 0 }; pi <= BSincPhaseCount; ++pi ) {
                const double phase{ std::floor( l ) +
                                    ( pi / double{ BSincPhaseCount } ) };

                for ( uint i{ 0 }; i < m; ++i ) {
                    const double x{ i - phase };
                    filter[ si ][ pi ][ o + i ] =
                        Kaiser( hdr.beta, x / l, hdr.besseli_0_beta ) * cutoff *
                        Sinc( cutoff * x );
                }
            }
        }
        ...
        for ( size_t si{ 0 }; si < BSincScaleCount; ++si ) {
            ...
            if ( si == BSincScaleCount - 1 ) {
                ...
                for ( size_t pi{ 0 }; pi < BSincPhaseCount; ++pi ) {
                     ...
                     *
                     * Given a difference in the number of points between
                     * scales, the destination points will be 0, thus: x = a + f
                     * (-a)
                     */
                    for ( size_t i{ 0 }; i < m; ++i ) {
                        const double scDelta{ filter[ si + 1 ][ pi ][ o + i ] -
                                              filter[ si ][ pi ][ o + i ] };
                     ...
                     */
                    for ( size_t i{ 0 }; i < m; ++i ) {
                        const double spDelta{
                            ( filter[ si + 1 ][ pi + 1 ][ o + i ] -
                              filter[ si + 1 ][ pi ][ o + i ] ) -
                            ( filter[ si ][ pi + 1 ][ o + i ] -
                              filter[ si ][ pi ][ o + i ] ) };
...
constexpr BSincTable GenerateBSincTable( const T& filter ) {
    ...
    for ( size_t i{ 1 }; i < BSincScaleCount; ++i )
        ret.filterOffset[ i ] =
            ret.filterOffset[ i - 1 ] + ret.m[ i - 1 ] * 4 * BSincPhaseCount;
