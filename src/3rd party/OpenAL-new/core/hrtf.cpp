...
static_assert( MaxHrirDelay * HrirDelayFracOne < 256,
               "MAX_HRIR_DELAY or DELAY_FRAC too large" );
...
public:
    databuf( const char_type* start_, const char_type* end_ ) noexcept {
        setg( const_cast< char_type* >( start_ ),
...
IdxBlend CalcEvIndex( uint evcount, float ev ) {
    ev = ( al::numbers::pi_v< float > * 0.5f + ev ) *
         static_cast< float >( evcount - 1 ) * al::numbers::inv_pi_v< float >;
    uint idx{ float2uint( ev ) };

    return IdxBlend{ minu( idx, evcount - 1 ),
                     ev - static_cast< float >( idx ) };
}
...
IdxBlend CalcAzIndex( uint azcount, float az ) {
    az = ( al::numbers::pi_v< float > * 2.0f + az ) *
...
void HrtfStore::getCoeffs( float elevation,
    ...
    const float dirfact{ 1.0f -
                         ( al::numbers::inv_pi_v< float > / 2.0f * spread ) };

    size_t ebase{ 0 };
    auto match_field = [ &ebase,
                         distance ]( const Field& field ) noexcept -> bool {
    ...
    auto field =
        std::find_if( mFields.begin(), mFields.end() - 1, match_field );
    ...
    float d{ mDelays[ idx[ 0 ] ][ 0 ] * blend[ 0 ] +
    ...
    for ( size_t c{ 0 }; c < 4; c++ ) {
        const float* srccoeffs{
            al::assume_aligned< 16 >( mCoeffs[ idx[ c ] ][ 0 ].data() ) };
        const float mult{ blend[ c ] };
        auto blend_coeffs = [ mult ]( const float src,
                                      const float coeff ) noexcept -> float {
            return src * mult + coeff;
        };
        std::transform( srccoeffs, srccoeffs + HrirLength * 2, coeffout,
                        coeffout, blend_coeffs );
    }
}

std::unique_ptr< DirectHrtfState > DirectHrtfState::Create( size_t num_chans ) {
    return std::unique_ptr< DirectHrtfState >{
        new ( FamCount( num_chans ) ) DirectHrtfState{ num_chans } };
}

void DirectHrtfState::build(
    const HrtfStore* Hrtf,
    const uint irSize,
    ...
    al::vector< ImpulseResponse > impres;
    impres.reserve( AmbiPoints.size() );
    auto calc_res = [ Hrtf, &max_delay, &min_delay ](
                        const AngularPoint& pt ) -> ImpulseResponse {
        ...
        const auto az0 =
            CalcAzIndex( Hrtf->mElev[ elev0.idx ].azCount, pt.Azim.value );
        const auto az1 =
            CalcAzIndex( Hrtf->mElev[ elev1_idx ].azCount, pt.Azim.value );
        ...
        const size_t idx[ 4 ]{
            ...
            ir1offset + az1.idx,
            ir1offset +
                ( ( az1.idx + 1 ) % Hrtf->mElev[ elev1_idx ].azCount ) };

        /* The largest blend factor serves as the closest HRIR. */
        const size_t irOffset{
            idx[ ( elev0.blend >= 0.5f ) * 2 + ( az1.blend >= 0.5f ) ] };
        ImpulseResponse res{ Hrtf->mCoeffs[ irOffset ],
                             Hrtf->mDelays[ irOffset ][ 0 ],
                             Hrtf->mDelays[ irOffset ][ 1 ] };
    ...
    std::transform( AmbiPoints.begin(), AmbiPoints.end(),
                    std::back_inserter( impres ), calc_res );
    ...
    TRACE( "Min delay: %.2f, max delay: %.2f, FIR length: %u\n",
           min_delay / double{ HrirDelayFracOne },
           max_delay / double{ HrirDelayFracOne }, irSize );

    auto tmpres =
        al::vector< std::array< double2, HrirLength > >( mChannels.size() );
    max_delay = 0;
    for ( size_t c{ 0u }; c < AmbiPoints.size(); ++c ) {
        const ConstHrirSpan hrir{ impres[ c ].hrir };
        const uint base_delay{
            perHrirMin ? minu( impres[ c ].ldelay, impres[ c ].rdelay )
                       : min_delay };
        const uint ldelay{
            hrir_delay_round( impres[ c ].ldelay - base_delay ) };
        const uint rdelay{
            hrir_delay_round( impres[ c ].rdelay - base_delay ) };
        max_delay =
            maxu( max_delay,
                  maxu( impres[ c ].ldelay, impres[ c ].rdelay ) - base_delay );
    ...
    for ( size_t i{ 0u }; i < mChannels.size(); ++i ) {
        auto copy_arr = []( const double2& in ) noexcept -> float2 {
            return float2{ { static_cast< float >( in[ 0 ] ),
                             static_cast< float >( in[ 1 ] ) } };
        };
        std::transform( tmpres[ i ].cbegin(), tmpres[ i ].cend(),
                        mChannels[ i ].mCoeffs.begin(), copy_arr );
    }
    tmpres.clear();

    const uint max_length{
        minu( hrir_delay_round( max_delay ) + irSize, HrirLength ) };
    TRACE( "New max delay: %.2f, FIR length: %u\n",
           max_delay / double{ HrirDelayFracOne }, max_length );
    mIrSize = max_length;
}

namespace {

std::unique_ptr< HrtfStore > CreateHrtfStore(
    uint rate,
    uint8_t irSize,
    const al::span< const HrtfStore::Field > fields,
    const al::span< const HrtfStore::Elevation > elevs,
    ...
    total = RoundUp( total,
                     alignof( HrtfStore::Field ) ); /* Align for field infos */
    total +=
        sizeof( std::declval< HrtfStore& >().mFields[ 0 ] ) * fields.size();
    total = RoundUp(
        total,
        alignof( HrtfStore::Elevation ) ); /* Align for elevation infos */
    ...
    if ( void* ptr{ al_calloc( 16, total ) } ) {
        ...
        offset = RoundUp(
            offset, alignof( HrtfStore::Field ) ); /* Align for field infos */
...
void MirrorLeftHrirs( const al::span< const HrtfStore::Elevation > elevs,
                      HrirArray* coeffs,
                      ubyte2* delays ) {
...
template < size_t num_bits, typename T >
    ...
    std::is_signed< T >::value&& num_bits< sizeof( T ) * 8, T > fixsign(
        T value ) noexcept {
...
template < size_t num_bits, typename T >
constexpr std::
    enable_if_t< !std::is_signed< T >::value || num_bits == sizeof( T ) * 8, T >
    fixsign( T value ) noexcept {
    return value;
}

template < typename T, size_t num_bits = sizeof( T ) * 8 >
inline std::enable_if_t< al::endian::native == al::endian::little, T > readle(
    std::istream& data ) {
    static_assert( ( num_bits & 7 ) == 0, "num_bits must be a multiple of 8" );
    static_assert( num_bits <= sizeof( T ) * 8,
                   "num_bits is too large for the type" );
...
template < typename T, size_t num_bits = sizeof( T ) * 8 >
inline std::enable_if_t< al::endian::native == al::endian::big, T > readle(
    std::istream& data ) {
    static_assert( ( num_bits & 7 ) == 0, "num_bits must be a multiple of 8" );
    static_assert( num_bits <= sizeof( T ) * 8,
                   "num_bits is too large for the type" );
    ...
    if ( !data.read( reinterpret_cast< char* >( b ), num_bits / 8 ) )
        return static_cast< T >( EOF );
    std::reverse_copy( std::begin( b ), std::end( b ),
                       reinterpret_cast< al::byte* >( &ret ) );
...
std::unique_ptr< HrtfStore > LoadHrtf00( std::istream& data,
    ...
    for ( size_t i{ 1 }; i < evCount; i++ ) {
        elevs[ i - 1 ].azCount = static_cast< ushort >(
            elevs[ i ].irOffset - elevs[ i - 1 ].irOffset );
        if ( elevs[ i - 1 ].azCount < MinAzCount ||
             elevs[ i - 1 ].azCount > MaxAzCount ) {
    ...
    elevs.back().azCount =
        static_cast< ushort >( irCount - elevs.back().irOffset );
    if ( elevs.back().azCount < MinAzCount ||
         elevs.back().azCount > MaxAzCount ) {
    ...
    MirrorLeftHrirs( { elevs.data(), elevs.size() }, coeffs.data(),
                     delays.data() );

    const HrtfStore::Field field[ 1 ]{ { 0.0f, evCount } };
    return CreateHrtfStore( rate, static_cast< uint8_t >( irSize ), field,
                            { elevs.data(), elevs.size() }, coeffs.data(),
...
std::unique_ptr< HrtfStore > LoadHrtf01( std::istream& data,
    ...
    for ( size_t i{ 0 }; i < evCount; ++i ) {
        if ( elevs[ i ].azCount < MinAzCount ||
             elevs[ i ].azCount > MaxAzCount ) {
            ERR( "Unsupported azimuth count: azCount[%zd]=%d (%d to %d)\n", i,
                 elevs[ i ].azCount, MinAzCount, MaxAzCount );
            return nullptr;
        }
    }
    ...
    elevs[ 0 ].irOffset = 0;
    for ( size_t i{ 1 }; i < evCount; i++ )
        elevs[ i ].irOffset = static_cast< ushort >( elevs[ i - 1 ].irOffset +
                                                     elevs[ i - 1 ].azCount );
    const ushort irCount{
        static_cast< ushort >( elevs.back().irOffset + elevs.back().azCount ) };
    ...
    const HrtfStore::Field field[ 1 ]{ { 0.0f, evCount } };
    return CreateHrtfStore( rate, irSize, field, { elevs.data(), elevs.size() },
...
std::unique_ptr< HrtfStore > LoadHrtf02( std::istream& data,
    ...
    for ( size_t f{ 0 }; f < fdCount; f++ ) {
        ...
        if ( distance < MinFdDistance || distance > MaxFdDistance ) {
            ERR( "Unsupported field distance[%zu]=%d (%d to %d millimeters)\n",
                 f, distance, MinFdDistance, MaxFdDistance );
            return nullptr;
        }
        if ( evCount < MinEvCount || evCount > MaxEvCount ) {
        ...
        for ( auto& elev : al::span< HrtfStore::Elevation >(
                  elevs.data() + ebase, evCount ) )
            elev.azCount = readle< uint8_t >( data );
        if ( !data || data.eof() ) {
            ERR( "Failed reading %s\n", filename );
            return nullptr;
        ...
        for ( size_t e{ 0 }; e < evCount; e++ ) {
            if ( elevs[ ebase + e ].azCount < MinAzCount ||
                 elevs[ ebase + e ].azCount > MaxAzCount ) {
                ERR( "Unsupported azimuth count: azCount[%zu][%zu]=%d (%d to "
                     "%d)\n",
                     f, e, elevs[ ebase + e ].azCount, MinAzCount, MaxAzCount );
                return nullptr;
            }
        }
    }
    ...
    elevs[ 0 ].irOffset = 0;
    std::partial_sum(
        elevs.cbegin(), elevs.cend(), elevs.begin(),
        []( const HrtfStore::Elevation& last,
            const HrtfStore::Elevation& cur ) -> HrtfStore::Elevation {
            return HrtfStore::Elevation{
                cur.azCount,
                static_cast< ushort >( last.azCount + last.irOffset ) };
        } );
    const auto irTotal =
        static_cast< ushort >( elevs.back().azCount + elevs.back().irOffset );
    ...
    if ( channelType == ChanType_LeftOnly ) {
        if ( sampleType == SampleType_S16 ) {
            ...
            for ( auto& hrir : coeffs ) {
                for ( auto& val : al::span< float2 >{ hrir.data(), irSize } )
                    val[ 0 ] =
                        static_cast< float >( readle< int, 24 >( data ) ) /
                        8388608.0f;
            }
        }
        for ( auto& val : delays )
    ...
    if ( fdCount > 1 ) {
        ...
        auto copy_azs = [ &elevs, &elevs__end ](
                            const ptrdiff_t ebase,
                            const HrtfStore::Field& field ) -> ptrdiff_t {
            auto elevs_src = elevs.begin() + ebase;
            elevs__end = std::copy_backward(
                elevs_src, elevs_src + field.evCount, elevs__end );
            return ebase + field.evCount;
        };
        ( void )std::accumulate( fields.cbegin(), fields.cend(), ptrdiff_t{ 0 },
                                 copy_azs );
        ...
        elevs_[ 0 ].irOffset = 0;
        std::partial_sum(
            elevs_.cbegin(), elevs_.cend(), elevs_.begin(),
            []( const HrtfStore::Elevation& last,
                const HrtfStore::Elevation& cur ) -> HrtfStore::Elevation {
                return HrtfStore::Elevation{
                    cur.azCount,
        ...
        auto copy_irs = [ &elevs, &coeffs, &delays, &coeffs_end, &delays_end ](
            ...
            auto accum_az =
                []( int count,
                    const HrtfStore::Elevation& elev ) noexcept -> int {
                return count + elev.azCount;
            };
            const auto elevs_mid = elevs.cbegin() + ebase;
            const auto elevs_end = elevs_mid + field.evCount;
            const int abase{
                std::accumulate( elevs.cbegin(), elevs_mid, 0, accum_az ) };
            const int num_azs{
                std::accumulate( elevs_mid, elevs_end, 0, accum_az ) };

            coeffs_end = std::copy_backward(
                coeffs.cbegin() + abase, coeffs.cbegin() + ( abase + num_azs ),
                coeffs_end );
            delays_end = std::copy_backward(
                delays.cbegin() + abase, delays.cbegin() + ( abase + num_azs ),
                delays_end );
...
std::unique_ptr< HrtfStore > LoadHrtf03( std::istream& data,
    ...
    if ( channelType == ChanType_LeftOnly ) {
        for ( auto& hrir : coeffs ) {
            for ( auto& val : al::span< float2 >{ hrir.data(), irSize } )
                val[ 0 ] = static_cast< float >( readle< int, 24 >( data ) ) /
                           8388608.0f;
        }
        for ( auto& val : delays )
            val[ 0 ] = readle< uint8_t >( data );
        ...
        for ( size_t i{ 0 }; i < irTotal; ++i ) {
            if ( delays[ i ][ 0 ] > MaxHrirDelay << HrirDelayFracBits ) {
                ERR( "Invalid delays[%zu][0]: %f (%d)\n", i,
                     delays[ i ][ 0 ] / float{ HrirDelayFracOne },
                     MaxHrirDelay );
                return nullptr;
            }
        }
        ...
        for ( auto& hrir : coeffs ) {
            for ( auto& val : al::span< float2 >{ hrir.data(), irSize } ) {
                val[ 0 ] = static_cast< float >( readle< int, 24 >( data ) ) /
                           8388608.0f;
                val[ 1 ] = static_cast< float >( readle< int, 24 >( data ) ) /
                           8388608.0f;
            }
        }
        for ( auto& val : delays ) {
        ...
        for ( size_t i{ 0 }; i < irTotal; ++i ) {
            ...
            if ( delays[ i ][ 1 ] > MaxHrirDelay << HrirDelayFracBits ) {
                ERR( "Invalid delays[%zu][1]: %f (%d)\n", i,
                     delays[ i ][ 1 ] / float{ HrirDelayFracOne },
                     MaxHrirDelay );
                return nullptr;
            }
        }
...
bool checkName( const std::string& name ) {
    auto match_name = [ &name ]( const HrtfEntry& entry ) -> bool {
        return name == entry.mDispName;
    };
    auto& enum_names = EnumeratedHrtfs;
    return std::find_if( enum_names.cbegin(), enum_names.cend(), match_name ) !=
           enum_names.cend();
}
...
void AddFileEntry( const std::string& filename ) {
    ...
    auto enum_iter =
        std::find_if( EnumeratedHrtfs.cbegin(), EnumeratedHrtfs.cend(),
    ...
    if ( !namepos )
        namepos = filename.find_last_of( '\\' ) + 1;
    ...
    const std::string basename{
        ( extpos == std::string::npos )
            ? filename.substr( namepos )
            : filename.substr( namepos, extpos - namepos ) };
...
void AddBuiltInEntry( const std::string& dispname, uint residx ) {
    const std::string filename{ '!' + std::to_string( residx ) + '_' +
                                dispname };
...
al::span< const char > GetResource( int name ) {
    if ( name == IDR_DEFAULT_HRTF_MHR )
        return { reinterpret_cast< const char* >( hrtf_default ),
                 sizeof( hrtf_default ) };
    return {};
}
#endif
        ...
        while ( pathlist && *pathlist ) {
            ...
            if ( end != pathlist ) {
                ...
                for ( const auto& fname :
                      SearchDataFiles( ".mhr", pname.c_str() ) )
                    AddFileEntry( fname );
            }
...
HrtfStorePtr GetLoadedHrtf( const std::string& name, const uint devrate ) {
    ...
    auto entry_iter =
        std::find_if( EnumeratedHrtfs.cbegin(), EnumeratedHrtfs.cend(),
                      [ &name ]( const HrtfEntry& entry ) -> bool {
                          return entry.mDispName == name;
                      } );
    ...
    auto hrtf_lt_fname = []( LoadedHrtf& hrtf,
                             const std::string& filename ) -> bool {
        return hrtf.mFilename < filename;
    };
    auto handle = std::lower_bound( LoadedHrtfs.begin(), LoadedHrtfs.end(),
                                    fname, hrtf_lt_fname );
    ...
    if ( sscanf( fname.c_str(), "!%d%c", &residx, &ch ) == 2 && ch == '_' ) {
        ...
        TRACE( "Loading %s...\n", fname.c_str() );
        auto fstr =
            std::make_unique< al::ifstream >( fname.c_str(), std::ios::binary );
        if ( !fstr->is_open() ) {
    ...
    if ( stream->gcount() <
         static_cast< std::streamsize >( sizeof( magicMarker03 ) ) )
    ...
    if ( hrtf->mSampleRate != devrate ) {
        ...
        const size_t lastEv{
            std::accumulate(
                hrtf->mFields.begin(), hrtf->mFields.end(), size_t{ 0 },
                []( const size_t curval,
                    const HrtfStore::Field& field ) noexcept -> size_t {
                    return curval + field.evCount;
                } ) -
            1 };
        const size_t irCount{ size_t{ hrtf->mElev[ lastEv ].irOffset } +
                              hrtf->mElev[ lastEv ].azCount };
        ...
        for ( size_t i{ 0 }; i < irCount; ++i ) {
            ...
            for ( size_t j{ 0 }; j < 2; ++j ) {
                std::transform( coeffs.cbegin(), coeffs.cend(),
                                inout[ 0 ].begin(),
                                [ j ]( const float2& in ) noexcept -> double {
                                    return in[ j ];
                                } );
                rs.process( HrirLength, inout[ 0 ].data(), HrirLength,
                            inout[ 1 ].data() );
        ...
        const float rate_scale{ static_cast< float >( devrate ) /
                                static_cast< float >( hrtf->mSampleRate ) };
        for ( size_t i{ 0 }; i < irCount; ++i ) {
            for ( size_t j{ 0 }; j < 2; ++j ) {
                const float new_delay{
                    std::round( hrtf->mDelays[ i ][ j ] * rate_scale ) /
        ...
        for ( size_t i{ 0 }; i < irCount; ++i ) {
            ...
            for ( size_t j{ 0 }; j < 2; ++j )
                delays[ j ] = static_cast< ubyte >(
                    float2int( new_delays[ i ][ j ] * delay_scale + 0.5f ) );
        }

        /* Scale the IR size for the new sample rate and update the stored
         * sample rate.
         */
        const float newIrSize{
            std::round( static_cast< float >( hrtf->mIrSize ) * rate_scale ) };
        hrtf->mIrSize = static_cast< uint8_t >( minf( HrirLength, newIrSize ) );
        hrtf->mSampleRate = devrate;
    }

    TRACE( "Loaded HRTF %s for sample rate %uhz, %u-sample filter\n",
           name.c_str(), hrtf->mSampleRate, hrtf->mIrSize );
...
void HrtfStore::dec_ref() {
    ...
    if ( ref == 0 ) {
        ...
        auto iter = std::remove_if( LoadedHrtfs.begin(), LoadedHrtfs.end(),
                                    remove_unused );
