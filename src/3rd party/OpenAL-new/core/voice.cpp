...
static_assert( !( MaxResamplerEdge & 3 ),
               "MaxResamplerEdge is not a multiple of 4" );

static_assert( ( BufferLineSize - 1 ) / MaxPitch > 0,
               "MaxPitch is too large for BufferLineSize!" );
static_assert(
    ( INT_MAX >> MixerFracBits ) / MaxPitch > BufferLineSize,
...
using HrtfMixerFunc = void ( * )( const float* InSamples,
                                  float2* AccumSamples,
                                  const uint IrSize,
                                  const MixHrtfFilter* hrtfparams,
                                  const size_t BufferSize );
using HrtfMixerBlendFunc = void ( * )( const float* InSamples,
                                       float2* AccumSamples,
                                       const uint IrSize,
                                       const HrtfFilter* oldparams,
                                       const MixHrtfFilter* newparams,
                                       const size_t BufferSize );
    ...
    if ( resampler ) {
        ...
        if ( al::strcasecmp( str, "bsinc" ) == 0 ) {
            WARN( "Resampler option \"%s\" is deprecated, using bsinc12\n",
                  str );
            str = "bsinc12";
        } else if ( al::strcasecmp( str, "sinc4" ) == 0 ||
                    al::strcasecmp( str, "sinc8" ) == 0 ) {
            WARN( "Resampler option \"%s\" is deprecated, using cubic\n", str );
            str = "cubic";
        }

        auto iter = std::find_if(
            std::begin( ResamplerList ), std::end( ResamplerList ),
...
constexpr int IMA4Index_adjust[ 16 ] = { -1, -1, -1, -1, 2, 4, 6, 8,
                                         -1, -1, -1, -1, 2, 4, 6, 8 };

/* MSADPCM Adaption table */
constexpr int MSADPCMAdaption[ 16 ] = { 230, 230, 230, 230, 307, 409,
...
const float* DoFilters( BiquadFilter& lpfilter,
                        BiquadFilter& hpfilter,
                        float* dst,
                        const al::span< const float > src,
                        int type ) {
    switch ( type ) {
        case AF_None:
            lpfilter.clear();
...
inline void LoadSamples( float* RESTRICT dstSamples,
                         const al::byte* src,
    ...
    constexpr size_t sampleSize{
        sizeof( typename al::FmtTypeTraits< Type >::Type ) };
...
template <>
inline void LoadSamples< FmtIMA4 >( float* RESTRICT dstSamples,
                                    const al::byte* src,
                                    const size_t srcChan,
                                    const size_t srcOffset,
                                    const size_t srcStep,
    ...
    do {
        ...
        index = clampi( ( index ^ 0x8000 ) - 32768, 0,
                        al::size( IMAStep_size ) - 1 );
        ...
        for ( ; skip; --skip ) {
            ...
            const size_t byteOffset{ wordOffset * srcStep +
                                     ( ( nibbleOffset >> 1 ) & 3u ) };
            ++nibbleOffset;

            std::ignore = decode_sample(
                ( nibbleData[ byteOffset ] >> byteShift ) & 15u );
        }
         ...
         */
        const size_t todo{
            minz( samplesPerBlock - startOffset, samplesToLoad - wrote ) };
        for ( size_t i{ 0 }; i < todo; ++i ) {
            ...
            const int result{ decode_sample(
                ( nibbleData[ byteOffset ] >> byteShift ) & 15u ) };
...
template <>
inline void LoadSamples< FmtMSADPCM >( float* RESTRICT dstSamples,
                                       const al::byte* src,
                                       const size_t srcChan,
                                       const size_t srcOffset,
                                       const size_t srcStep,
    ...
    do {
        ...
        input += srcStep;
        int delta{ input[ 2 * srcChan + 0 ] |
                   ( input[ 2 * srcChan + 1 ] << 8 ) };
        input += srcStep * 2;

        int sampleHistory[ 2 ]{};
        sampleHistory[ 0 ] =
            input[ 2 * srcChan + 0 ] | ( input[ 2 * srcChan + 1 ] << 8 );
        input += srcStep * 2;
        sampleHistory[ 1 ] =
            input[ 2 * srcChan + 0 ] | ( input[ 2 * srcChan + 1 ] << 8 );
        input += srcStep * 2;
        ...
        if ( skip == 0 ) {
            dstSamples[ wrote++ ] =
                static_cast< float >( sampleHistory[ 1 ] ) / 32768.0f;
            if ( wrote == samplesToLoad )
                return;
            dstSamples[ wrote++ ] =
                static_cast< float >( sampleHistory[ 0 ] ) / 32768.0f;
            if ( wrote == samplesToLoad )
                return;
        } else if ( skip == 1 ) {
            --skip;
        ...
        auto decode_sample = [ &sampleHistory, &delta,
                               coeffs ]( const int nibble ) {
            int pred{ ( sampleHistory[ 0 ] * coeffs[ 0 ] +
                        sampleHistory[ 1 ] * coeffs[ 1 ] ) /
                      256 };
        ...
        for ( ; skip; --skip ) {
            ...
            std::ignore =
                decode_sample( ( input[ byteOffset ] >> byteShift ) & 15 );
        }
...
void LoadSamples( float* dstSamples,
                  const al::byte* src,
                  const size_t srcChan,
                  const size_t srcOffset,
...
void LoadBufferStatic( VoiceBufferItem* buffer,
                       VoiceBufferItem* bufferLoopItem,
    ...
    if ( !bufferLoopItem ) {
        ...
        if ( buffer->mSampleLen > dataPosInt )
            LIKELY {
                const size_t buffer_remaining{ buffer->mSampleLen -
                                               dataPosInt };
                const size_t remaining{
                    minz( samplesToLoad - samplesLoaded, buffer_remaining ) };
                LoadSamples( voiceSamples + samplesLoaded, buffer->mSamples,
                             srcChannel, dataPosInt, sampleType, srcStep,
                             buffer->mBlockAlign, remaining );
        ...
        const size_t intPos{
            ( dataPosInt < loopEnd )
                ? dataPosInt
                : ( ( ( dataPosInt - loopStart ) % ( loopEnd - loopStart ) ) +
                    loopStart ) };

        /* Load what's left of this loop iteration */
        const size_t remaining{
            minz( samplesToLoad - samplesLoaded, loopEnd - dataPosInt ) };
        LoadSamples( voiceSamples + samplesLoaded, buffer->mSamples, srcChannel,
                     intPos, sampleType, srcStep, buffer->mBlockAlign,
                     remaining );
        samplesLoaded += remaining;

        /* Load repeats of the loop to fill the buffer. */
        const size_t loopSize{ loopEnd - loopStart };
        while ( const size_t toFill{
            minz( samplesToLoad - samplesLoaded, loopSize ) } ) {
            LoadSamples( voiceSamples + samplesLoaded, buffer->mSamples,
                         srcChannel, loopStart, sampleType, srcStep,
                         buffer->mBlockAlign, toFill );
            samplesLoaded += toFill;
        }
    }
}

void LoadBufferCallback( VoiceBufferItem* buffer,
                         const size_t dataPosInt,
                         ...
                         float* voiceSamples ) {
    /* Load what's left to play from the buffer */
    if ( numCallbackSamples > dataPosInt )
        LIKELY {
            const size_t remaining{ minz( samplesToLoad - samplesLoaded,
...
void LoadBufferQueue( VoiceBufferItem* buffer,
                      VoiceBufferItem* bufferLoopItem,
                      size_t dataPosInt,
                      const FmtType sampleType,
                      const size_t srcChannel,
    ...
    while ( buffer && samplesLoaded != samplesToLoad ) {
        ...
        LoadSamples( voiceSamples + samplesLoaded, buffer->mSamples, srcChannel,
                     dataPosInt, sampleType, srcStep, buffer->mBlockAlign,
                     remaining );
...
void DoHrtfMix( const float* samples,
    ...
    auto src_iter =
        std::copy( parms.Hrtf.History.begin(), parms.Hrtf.History.end(),
    ...
    std::copy_n( std::begin( HrtfSamples ) + DstBufferSize,
                 parms.Hrtf.History.size(), parms.Hrtf.History.begin() );
    ...
    if ( Counter && OutPos == 0 ) {
         ...
         */
        if ( Counter > fademix ) {
            const float a{ static_cast< float >( fademix ) /
                           static_cast< float >( Counter ) };
            gain = lerpf( parms.Hrtf.Old.Gain, TargetGain, a );
        }

        MixHrtfFilter hrtfparams{ parms.Hrtf.Target.Coeffs,
                                  parms.Hrtf.Target.Delay, 0.0f,
                                  gain / static_cast< float >( fademix ) };
        MixHrtfBlendSamples( HrtfSamples, AccumSamples + OutPos, IrSize,
    ...
    if ( fademix < DstBufferSize ) {
        ...
        if ( Counter > DstBufferSize ) {
            const float a{ static_cast< float >( todo ) /
                           static_cast< float >( Counter - fademix ) };
            gain = lerpf( parms.Hrtf.Old.Gain, TargetGain, a );
        }

        MixHrtfFilter hrtfparams{
            parms.Hrtf.Target.Coeffs, parms.Hrtf.Target.Delay,
            parms.Hrtf.Old.Gain,
            ( gain - parms.Hrtf.Old.Gain ) / static_cast< float >( todo ) };
        MixHrtfSamples( HrtfSamples + fademix, AccumSamples + OutPos, IrSize,
                        &hrtfparams, todo );
...
void DoNfcMix( const al::span< const float > samples,
               FloatBufferLine* OutBuffer,
               DirectParams& parms,
               ...
               DeviceBase* Device ) {
    using FilterProc =
        void ( NfcFilter::* )( const al::span< const float >, float* );
    static constexpr FilterProc NfcProcess[ MaxAmbiOrder + 1 ]{
        nullptr, &NfcFilter::process1, &NfcFilter::process2,
        &NfcFilter::process3 };

    float* CurrentGains{ parms.Gains.Current.data() };
    MixSamples( samples, { OutBuffer, 1u }, CurrentGains, TargetGains, Counter,
                OutPos );
    ++OutBuffer;
    ++CurrentGains;
    ++TargetGains;
    ...
    while ( const size_t chancount{ Device->NumChannelsPerOrder[ order ] } ) {
        ( parms.NFCtrlFilter.*NfcProcess[ order ] )( samples,
                                                     nfcsamples.data() );
        MixSamples( nfcsamples, { OutBuffer, chancount }, CurrentGains,
...
void Voice::mix( const State vstate,
                 ContextBase* Context,
    ...
    VoiceBufferItem* BufferListItem{
    ...
    if ( mFlags.test( VoiceIsStatic ) && BufferLoopItem ) {
        if ( DataPosInt >= 0 &&
             static_cast< uint >( DataPosInt ) >= BufferListItem->mLoopEnd )
    ...
    if ( mStartTime > deviceTime )
        UNLIKELY {
            /* If the voice is supposed to be stopping but hasn't actually
             * started yet, make sure its stopped.
             */
            if ( vstate == Stopping ) {
                mPlayState.store( Stopped, std::memory_order_release );
                return;
             ...
             * should start at. Skip this update if it's beyond the output
             * sample count.
             *
             * Round the start position to a multiple of 4, which some mixers
             * want. This makes the start time accurate to 4 samples. This could
             * be made sample-accurate by forcing non-SIMD functions on the
             * first run.
             */
            seconds::rep sampleOffset{
                duration_cast< seconds >( diff * Device->Frequency ).count() };
    ...
    const al::span< float* > MixingSamples{ SamplePointers.data(),
                                            mChans.size() };
    auto get_bufferline =
        []( DeviceBase::MixerBufferLine& bufline ) noexcept -> float* {
        return bufline.data();
    };
    std::transform( Device->mSampleData.end() - mChans.size(),
                    Device->mSampleData.end(), MixingSamples.begin(),
                    get_bufferline );

    /* If there's a matching sample step and no phase offset, use a simple copy
     * for resampling.
     */
    const ResamplerFunc Resample{
        ( increment == MixerFracOne && DataPosFrac == 0 )
            ? ResamplerFunc{ []( const InterpState*, const float* RESTRICT src,
                                 uint, const uint,
     ...
     */
    const size_t realChannels{
        ( mFmtChannels == FmtUHJ2 || mFmtChannels == FmtSuperStereo )
            ? 2u
            : MixingSamples.size() };
    for ( size_t chan{ 0 }; chan < realChannels; ++chan ) {
        using ResBufType = decltype( DeviceBase::mResampleData );
        static constexpr uint srcSizeMax{
            static_cast< uint >( ResBufType{}.size() - MaxResamplerEdge ) };

        const auto prevSamples = al::as_span( mPrevSamples[ chan ] );
        const auto resampleBuffer =
            std::copy( prevSamples.cbegin(), prevSamples.cend(),
        ...
        for ( uint samplesLoaded{ 0 }; samplesLoaded < samplesToLoad; ) {
             ...
             */
            auto calc_buffer_sizes = [ fracPos,
                                       increment ]( uint dstBufferSize ) {
                ...
                dataSize64 =
                    ( dataSize64 * increment + fracPos ) >> MixerFracBits;
                /* Also include resampler padding. */
                dataSize64 += ext + MaxResamplerEdge;

                if ( dataSize64 <= srcSizeMax )
                    return std::make_pair( dstBufferSize,
                                           static_cast< uint >( dataSize64 ) );
                ...
                dataSize64 =
                    ( ( dataSize64 << MixerFracBits ) - fracPos ) / increment;
            ...
            const auto bufferSizes =
                calc_buffer_sizes( samplesToLoad - samplesLoaded );
            ...
            if ( !BufferListItem ) {
                 ...
                 */
                auto abs_lt = []( const float lhs,
                                  const float rhs ) noexcept -> bool {
                    return std::abs( lhs ) < std::abs( rhs );
                };
                auto srciter = std::min_element(
                    resampleBuffer, resampleBuffer + avail, abs_lt );
                ...
                if ( intPos < 0 )
                    UNLIKELY {
                        /* If the current position is negative, there's that
                         * many silent samples to load before using the buffer.
                         */
                        srcSampleDelay = static_cast< uint >( -intPos );
                        if ( srcSampleDelay >= srcBufferSize ) {
                            /* If the number of silent source samples exceeds
                             * the number to load, the output will be silent.
                             */
                            std::fill_n( MixingSamples[ chan ] + samplesLoaded,
                                         dstBufferSize, 0.0f );
                ...
                if ( mFlags.test( VoiceIsStatic ) )
                    LoadBufferStatic( BufferListItem, BufferLoopItem, uintPos,
                                      mFmtType, chan, mFrameStep,
                                      srcSampleDelay, srcBufferSize,
                                      al::to_address( resampleBuffer ) );
                else if ( mFlags.test( VoiceIsCallback ) ) {
                    const uint callbackBase{ mCallbackBlockBase *
                                             mSamplesPerBlock };
                    const size_t bufferOffset{ uintPos - callbackBase };
                    const size_t needSamples{ bufferOffset + srcBufferSize -
                                              srcSampleDelay };
                    const size_t needBlocks{
                        ( needSamples + mSamplesPerBlock - 1 ) /
                        mSamplesPerBlock };
                    if ( !mFlags.test( VoiceCallbackStopped ) &&
                         needBlocks > mNumCallbackBlocks ) {
                        const size_t byteOffset{ mNumCallbackBlocks *
                                                 mBytesPerBlock };
                        const size_t needBytes{
                            ( needBlocks - mNumCallbackBlocks ) *
                            mBytesPerBlock };

                        const int gotBytes{ BufferListItem->mCallback(
                            BufferListItem->mUserData,
                            &BufferListItem->mSamples[ byteOffset ],
                            static_cast< int >( needBytes ) ) };
                        if ( gotBytes < 0 )
                            mFlags.set( VoiceCallbackStopped );
                        else if ( static_cast< uint >( gotBytes ) <
                                  needBytes ) {
                            mFlags.set( VoiceCallbackStopped );
                            mNumCallbackBlocks +=
                                static_cast< uint >( gotBytes ) /
                                mBytesPerBlock;
                        } else
                            mNumCallbackBlocks =
                                static_cast< uint >( needBlocks );
                    }
                    const size_t numSamples{ uint{ mNumCallbackBlocks } *
                                             mSamplesPerBlock };
                    LoadBufferCallback( BufferListItem, bufferOffset,
                                        numSamples, mFmtType, chan, mFrameStep,
                                        srcSampleDelay, srcBufferSize,
                                        al::to_address( resampleBuffer ) );
                } else
                    LoadBufferQueue( BufferListItem, BufferLoopItem, uintPos,
                                     mFmtType, chan, mFrameStep, srcSampleDelay,
                                     srcBufferSize,
                                     al::to_address( resampleBuffer ) );
            }

            Resample(
                &mResampleState, al::to_address( resampleBuffer ), fracPos,
                increment,
            ...
            if ( vstate == Playing )
                LIKELY {
                    ...
                    if ( samplesToMix > samplesLoaded &&
                         samplesToMix <= loadEnd )
                        LIKELY {
                            const size_t dstOffset{ samplesToMix -
                                                    samplesLoaded };
                            const size_t srcOffset{
                                ( dstOffset * increment + fracPos ) >>
                                MixerFracBits };
                            std::copy_n(
                                resampleBuffer - MaxResamplerEdge + srcOffset,
                                prevSamples.size(), prevSamples.begin() );
                        }
                }
        ...
        skip_resample:
            ...
            if ( samplesLoaded < samplesToLoad ) {
                ...
                std::copy( resampleBuffer - MaxResamplerEdge + srcOffset,
                           resampleBuffer + MaxResamplerEdge + srcOffset,
                           resampleBuffer - MaxResamplerEdge );
            }
        }
    }
    ...
    if ( mFlags.test( VoiceIsAmbisonic ) ) {
        ...
        for ( auto& chandata : mChans ) {
            chandata.mAmbiSplitter.processScale(
                { *voiceSamples, samplesToMix }, chandata.mAmbiHFScale,
                chandata.mAmbiLFScale );
            ++voiceSamples;
        }
    }

    const uint Counter{ mFlags.test( VoiceIsFading ) ? minu( samplesToMix, 64u )
                                                     : 0u };
    if ( !Counter ) {
    ...
    for ( auto& chandata : mChans ) {
        ...
        const al::span< float, BufferLineSize > FilterBuf{
            Device->FilteredData };
        {
            DirectParams& parms = chandata.mDryParams;
            const float* samples{ DoFilters(
                parms.LowPass, parms.HighPass, FilterBuf.data(),
                { *voiceSamples, samplesToMix }, mDirect.FilterType ) };

            if ( mFlags.test( VoiceHasHrtf ) ) {
                const float TargetGain{ parms.Hrtf.Target.Gain *
                                        ( vstate == Playing ) };
                DoHrtfMix( samples, samplesToMix, parms, TargetGain, Counter,
                           OutPos, ( vstate == Playing ), Device );
            } else {
                const float* TargetGains{ ( vstate == Playing )
                                              ? parms.Gains.Target.data()
                                              : SilentTarget.data() };
                if ( mFlags.test( VoiceHasNfc ) )
                    DoNfcMix( { samples, samplesToMix }, mDirect.Buffer.data(),
                              parms, TargetGains, Counter, OutPos, Device );
                else
                    MixSamples( { samples, samplesToMix }, mDirect.Buffer,
                                parms.Gains.Current.data(), TargetGains,
                                Counter, OutPos );
            }
        }
        ...
        for ( uint send{ 0 }; send < NumSends; ++send ) {
            ...
            MixSamples( { samples, samplesToMix }, mSend[ send ].Buffer,
                        parms.Gains.Current.data(), TargetGains, Counter,
                        OutPos );
        }
    ...
    if ( BufferListItem && DataPosInt >= 0 )
        LIKELY {
            if ( mFlags.test( VoiceIsStatic ) ) {
                if ( BufferLoopItem ) {
                    ...
                    if ( DataPosUInt >= LoopEnd ) {
                        ...
                        DataPosUInt = ( ( DataPosUInt - LoopStart ) %
                                        ( LoopEnd - LoopStart ) ) +
                                      LoopStart;
                    ...
                    if ( static_cast< uint >( DataPosInt ) >=
                         BufferListItem->mSampleLen )
                        BufferListItem = nullptr;
                }
            } else if ( mFlags.test( VoiceIsCallback ) ) {
                /* Handle callback buffer source */
                const uint currentBlock{ static_cast< uint >( DataPosInt ) /
                                         mSamplesPerBlock };
                ...
                do {
                    if ( BufferListItem->mSampleLen >
                         static_cast< uint >( DataPosInt ) )
                        break;
                    ...
                    ++buffers_done;
                    BufferListItem =
                        BufferListItem->mNext.load( std::memory_order_relaxed );
                    if ( !BufferListItem )
    ...
    const auto enabledevt =
        Context->mEnabledEvts.load( std::memory_order_acquire );
...
void Voice::prepare( DeviceBase* device ) {
    ...
    uint num_channels{
        ( mFmtChannels == FmtUHJ2 || mFmtChannels == FmtSuperStereo )
            ? 3
            : ChannelsFromFmt( mFmtChannels,
                               minu( mAmbiOrder, device->mAmbiOrder ) ) };
    if ( num_channels > device->mSampleData.size() )
        UNLIKELY {
            ERR( "Unexpected channel count: %u (limit: %zu, %d:%d)\n",
                 num_channels, device->mSampleData.size(), mFmtChannels,
                 mAmbiOrder );
    ...
    if ( mFmtChannels == FmtSuperStereo ) {
        switch ( UhjDecodeQuality ) {
            ...
            case UhjQualityType::FIR256:
                mDecoder =
                    std::make_unique< UhjStereoDecoder< UhjLength256 > >();
                mDecoderPadding =
                    UhjStereoDecoder< UhjLength256 >::sInputPadding;
                break;
            case UhjQualityType::FIR512:
                mDecoder =
                    std::make_unique< UhjStereoDecoder< UhjLength512 > >();
                mDecoderPadding =
                    UhjStereoDecoder< UhjLength512 >::sInputPadding;
                break;
        }
    ...
    if ( mFmtChannels == FmtUHJ2 && !device->mUhjEncoder ) {
         ...
         */
        const BandSplitter splitter{
            device->mXOverFreq / static_cast< float >( device->Frequency ) };
        for ( auto& chandata : mChans ) {
            ...
            std::fill_n( chandata.mWetParams.begin(), device->NumAuxSends,
                         SendParams{} );
        }
     ...
     */
        ...
        const uint8_t* OrderFromChan{
            Is2DAmbisonic( mFmtChannels )
                ? AmbiIndex::OrderFrom2DChannel().data()
                : AmbiIndex::OrderFromChannel().data() };
        const auto scales = AmbiScale::GetHFOrderScales(
            mAmbiOrder, device->mAmbiOrder, device->m2DMixing );
