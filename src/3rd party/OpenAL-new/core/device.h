...
enum class DeviceType : unsigned char { Playback, Capture, Loopback };

enum class RenderMode : unsigned char { Normal, Pairwise, Hrtf };
...
struct MixParams {
     ...
     */
    template < typename F >
    void setAmbiMixParams( const MixParams& inmix,
...
using AmbiRotateMatrix =
    std::array< std::array< float, MaxAmbiChannels >, MaxAmbiChannels >;
...
struct DeviceBase {
    ...
    static constexpr size_t MixerLineSize{ BufferLineSize +
    ...
    uint waitForMix() const noexcept {
        uint refcount;
        while ( ( refcount = MixCount.load( std::memory_order_acquire ) ) &
                1 ) {
        }
        return refcount;
    }
    ...
    inline void postProcess( const size_t SamplesToDo ) {
        if ( PostProcess )
            LIKELY( this->*PostProcess )( SamplesToDo );
    }

    void renderSamples( const al::span< float* > outBuffers,
                        const uint numSamples );
    void renderSamples( void* outBuffer,
                        const uint numSamples,
