...
template <>
inline float LoadSample< DevFmtByte >(
...
template <>
inline float LoadSample< DevFmtUByte >(
...
inline float LoadSample< DevFmtUInt >(
    ...
    return LoadSample< DevFmtInt >(
        static_cast< int32_t >( val - 2147483648u ) );
}

template < DevFmtType T >
inline void LoadSampleArray( float* RESTRICT dst,
                             const void* src,
                             const size_t srcstep,
                             const size_t samples ) noexcept {
    const DevFmtType_t< T >* ssrc =
        static_cast< const DevFmtType_t< T >* >( src );
    for ( size_t i{ 0u }; i < samples; i++ )
        dst[ i ] = LoadSample< T >( ssrc[ i * srcstep ] );
}

void LoadSamples( float* dst,
                  const void* src,
                  const size_t srcstep,
...
#define HANDLE_FMT( T )                                     \
    case T:                                                 \
        LoadSampleArray< T >( dst, src, srcstep, samples ); \
        break
    switch ( srctype ) {
...
template <>
inline uint32_t StoreSample< DevFmtUInt >( float val ) noexcept {
...
template < DevFmtType T >
inline void StoreSampleArray( void* dst,
...
#define HANDLE_FMT( T )                                      \
    case T:                                                  \
        StoreSampleArray< T >( dst, src, dststep, samples ); \
        break
    switch ( dsttype ) {
        HANDLE_FMT( DevFmtByte );
        HANDLE_FMT( DevFmtUByte );
...
template < DevFmtType T >
void Mono2Stereo( float* RESTRICT dst,
                  const void* src,
    ...
    for ( size_t i{ 0u }; i < frames; i++ )
        dst[ i * 2 + 1 ] = dst[ i * 2 + 0 ] =
            LoadSample< T >( ssrc[ i ] ) * 0.707106781187f;
}

template < DevFmtType T >
void Multi2Mono( uint chanmask,
                 const size_t step,
                 const float scale,
                 float* RESTRICT dst,
                 const void* src,
...
SampleConverterPtr SampleConverter::Create( DevFmtType srcType,
                                            DevFmtType dstType,
                                            size_t numchans,
                                            uint srcRate,
                                            uint dstRate,
    ...
    SampleConverterPtr converter{ new ( FamCount( numchans ) )
                                      SampleConverter{ numchans } };
    ...
    /* Have to set the mixer FPU mode since that's what the resampler code
     * expects. */
    FPUCtl mixer_mode{};
    auto step = static_cast< uint >(
        mind( srcRate * double{ MixerFracOne } / dstRate + 0.5,
              MaxPitch * MixerFracOne ) );
    converter->mIncrement = maxu( step, 1 );
    if ( converter->mIncrement == MixerFracOne )
        converter->mResample = []( const InterpState*,
                                   const float* RESTRICT src, uint, const uint,
                                   const al::span< float > dst ) {
            std::copy_n( src, dst.size(), dst.begin() );
        };
    else
        converter->mResample = PrepareResampler(
            resampler, converter->mIncrement, &converter->mState );
...
uint SampleConverter::availableOut( uint srcframes ) const {
    ...
    if ( prepcount < MaxResamplerPadding &&
         MaxResamplerPadding - prepcount >= srcframes ) {
    ...
    return static_cast< uint >(
        clampu64( ( DataSize64 + mIncrement - 1 ) / mIncrement, 1,
                  std::numeric_limits< int >::max() ) );
}

uint SampleConverter::convert( const void** src,
                               uint* srcframes,
                               void* dst,
                               uint dstframes ) {
    const uint SrcFrameSize{ static_cast< uint >( mChan.size() ) *
                             mSrcTypeSize };
    const uint DstFrameSize{ static_cast< uint >( mChan.size() ) *
                             mDstTypeSize };
    ...
    while ( pos < dstframes && NumSrcSamples > 0 ) {
        const uint prepcount{ mSrcPrepCount };
        const uint readable{
            minu( NumSrcSamples, BufferLineSize - prepcount ) };

        if ( prepcount < MaxResamplerPadding &&
             MaxResamplerPadding - prepcount >= readable ) {
            /* Not enough input samples to generate an output sample. Store
             * what we're given for later.
             */
            for ( size_t chan{ 0u }; chan < mChan.size(); chan++ )
                LoadSamples( &mChan[ chan ].PrevSamples[ prepcount ],
                             SamplesIn + mSrcTypeSize * chan, mChan.size(),
                             mSrcType, readable );
        ...
        /* If we have a full prep, we can generate at least one sample. */
        auto DstSize = static_cast< uint >( clampu64(
            ( DataSize64 + increment - 1 ) / increment, 1, BufferLineSize ) );
        ...
        const uint nextprep{
            minu( prepcount + readable - SrcDataEnd, MaxResamplerPadding ) };

        for ( size_t chan{ 0u }; chan < mChan.size(); chan++ ) {
            const al::byte* SrcSamples{ SamplesIn + mSrcTypeSize * chan };
            al::byte* DstSamples =
                static_cast< al::byte* >( dst ) + mDstTypeSize * chan;

            /* Load the previous samples into the source data first, then the
             * new samples from the input buffer.
             */
            std::copy_n( mChan[ chan ].PrevSamples, prepcount, SrcData );
            LoadSamples( SrcData + prepcount, SrcSamples, mChan.size(),
                         mSrcType, readable );

            /* Store as many prep samples for next time as possible, given the
             * number of output samples being generated.
             */
            std::copy_n( SrcData + SrcDataEnd, nextprep,
                         mChan[ chan ].PrevSamples );
            std::fill( std::begin( mChan[ chan ].PrevSamples ) + nextprep,
                       std::end( mChan[ chan ].PrevSamples ), 0.0f );

            /* Now resample, and store the result in the output buffer. */
            mResample( &mState, SrcData + MaxResamplerEdge, DataPosFrac,
                       increment, { DstData, DstSize } );

            StoreSamples( DstSamples, DstData, mChan.size(), mDstType,
                          DstSize );
        }
        ...
        const uint srcread{
            minu( NumSrcSamples, SrcDataEnd + mSrcPrepCount - prepcount ) };
...
void ChannelConverter::convert( const void* src,
                                float* dst,
                                uint frames ) const {
    if ( mDstChans == DevFmtMono ) {
        const float scale{ std::sqrt(
            1.0f / static_cast< float >( al::popcount( mChanMask ) ) ) };
        switch ( mSrcType ) {
#define HANDLE_FMT( T )                                                  \
    case T:                                                              \
        Multi2Mono< T >( mChanMask, mSrcStep, scale, dst, src, frames ); \
        break
            HANDLE_FMT( DevFmtByte );
            HANDLE_FMT( DevFmtUByte );
            HANDLE_FMT( DevFmtShort );
