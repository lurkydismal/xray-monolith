...
inline float do_point( const InterpState&,
...
inline float do_cubic( const InterpState& istate,
    ...
    const float* RESTRICT fil{
        al::assume_aligned< 16 >( istate.cubic.filter[ pi ].mCoeffs ) };
    const float* RESTRICT phd{
        al::assume_aligned< 16 >( istate.cubic.filter[ pi ].mDeltas ) };

    /* Apply the phase interpolated filter. */
    return ( fil[ 0 ] + pf * phd[ 0 ] ) * vals[ 0 ] +
           ( fil[ 1 ] + pf * phd[ 1 ] ) * vals[ 1 ] +
...
inline float do_bsinc( const InterpState& istate,
    ...
    for ( size_t j_f{ 0 }; j_f < m; j_f++ )
        r += ( fil[ j_f ] + istate.bsinc.sf * scd[ j_f ] +
               pf * ( phd[ j_f ] + istate.bsinc.sf * spd[ j_f ] ) ) *
             vals[ j_f ];
    return r;
}
...
template <>
void Resample_< PointTag, CTag >( const InterpState* state,
                                  const float* RESTRICT src,
                                  uint frac,
                                  const uint increment,
                                  const al::span< float > dst ) {
...
template <>
void Resample_< LerpTag, CTag >( const InterpState* state,
                                 const float* RESTRICT src,
                                 uint frac,
                                 const uint increment,
                                 const al::span< float > dst ) {
...
template <>
void Resample_< CubicTag, CTag >( const InterpState* state,
                                  const float* RESTRICT src,
                                  uint frac,
                                  const uint increment,
                                  const al::span< float > dst ) {
...
template <>
void Resample_< BSincTag, CTag >( const InterpState* state,
                                  const float* RESTRICT src,
                                  uint frac,
                                  const uint increment,
                                  const al::span< float > dst ) {
...
template <>
void Resample_< FastBSincTag, CTag >( const InterpState* state,
                                      const float* RESTRICT src,
                                      uint frac,
                                      const uint increment,
                                      const al::span< float > dst ) {
    DoResample< do_fastbsinc >( state, src - state->bsinc.l, frac, increment,
                                dst );
}

template <>
void MixHrtf_< CTag >( const float* InSamples,
                       float2* AccumSamples,
                       const uint IrSize,
...
template <>
void MixHrtfBlend_< CTag >( const float* InSamples,
                            float2* AccumSamples,
                            const uint IrSize,
...
template <>
void MixDirectHrtf_< CTag >( const FloatBufferSpan LeftOut,
...
template <>
void Mix_< CTag >( const al::span< const float > InSamples,
    ...
    for ( FloatBufferLine& output : OutBuffer )
        MixLine( InSamples, al::assume_aligned< 16 >( output.data() + OutPos ),
                 *CurrentGains++, *TargetGains++, delta, min_len, Counter );
}
