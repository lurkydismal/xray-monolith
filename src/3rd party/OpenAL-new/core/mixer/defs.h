...
using ResamplerFunc = void ( * )( const InterpState* state,
...
void Resample_( const InterpState* state,
                const float* RESTRICT src,
                uint frac,
                const uint increment,
                const al::span< float > dst );
...
void Mix_( const al::span< const float > InSamples,
...
template < typename InstTag >
void MixHrtf_( const float* InSamples,
               float2* AccumSamples,
               const uint IrSize,
               const MixHrtfFilter* hrtfparams,
               const size_t BufferSize );
template < typename InstTag >
void MixHrtfBlend_( const float* InSamples,
                    float2* AccumSamples,
                    const uint IrSize,
                    ...
                    const size_t BufferSize );
template < typename InstTag >
void MixDirectHrtf_( const FloatBufferSpan LeftOut,
...
template < size_t N >
inline void InitPosArrays( uint frac,
                           uint increment,
                           uint ( &frac_arr )[ N ],
                           uint ( &pos_arr )[ N ] ) {
    pos_arr[ 0 ] = 0;
