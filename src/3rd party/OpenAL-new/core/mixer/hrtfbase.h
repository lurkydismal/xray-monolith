...
using ApplyCoeffsT = void ( & )( float2* RESTRICT Values,
                                 const size_t irSize,
...
inline void MixHrtfBase( const float* InSamples,
                         float2* RESTRICT AccumSamples,
                         const size_t IrSize,
...
template < ApplyCoeffsT ApplyCoeffs >
inline void MixHrtfBlendBase( const float* InSamples,
                              float2* RESTRICT AccumSamples,
                              const size_t IrSize,
...
inline void MixDirectHrtfBase(
    ...
    for ( const FloatBufferLine& input : InSamples ) {
         ...
         */
        ChanState->mSplitter.processHfScale( { input.data(), BufferSize },
                                             TempBuf, ChanState->mHfScale );
     ...
     */
    auto accum_iter =
        std::copy_n( AccumSamples + BufferSize, HrirLength, AccumSamples );
