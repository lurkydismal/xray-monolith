...
inline void ApplyCoeffs( float2* RESTRICT Values,
                         const size_t IrSize,
...
force_inline void MixLine( const al::span< const float > InSamples,
                           float* RESTRICT dst,
                           float& CurrentGain,
    ...
    if ( !( std::abs( step ) > std::numeric_limits< float >::epsilon() ) )
        ...
        if ( size_t todo{ min_len >> 2 } ) {
            ...
            do {
                ...
                dry4 = vmlaq_f32( dry4, val4,
                                  vmlaq_f32( gain4, step4, step_count4 ) );
        ...
        /* Mix with applying left over gain steps that aren't aligned multiples
         * of 4. */
    ...
    for ( size_t leftover{ ( InSamples.size() - pos ) & 3 }; leftover;
          ++pos, --leftover )
        dst[ pos ] += InSamples[ pos ] * gain;
}
...
template <>
void Resample_< LerpTag, NEONTag >( const InterpState*,
    ...
    const int32x4_t increment4 =
        vdupq_n_s32( static_cast< int >( increment * 4 ) );
    ...
    for ( size_t todo{ dst.size() >> 2 }; todo; --todo ) {
        ...
        const float32x4_t val1{
            set_f4( src[ pos0 ], src[ pos1 ], src[ pos2 ], src[ pos3 ] ) };
        const float32x4_t val2{ set_f4( src[ pos0 + 1 ], src[ pos1 + 1 ],
                                        src[ pos2 + 1 ], src[ pos3 + 1 ] ) };
...
template <>
void Resample_< CubicTag, NEONTag >( const InterpState* state,
                                     const float* RESTRICT src,
                                     uint frac,
                                     const uint increment,
                                     const al::span< float > dst ) {
    ...
    const CubicCoefficients* RESTRICT filter =
        al::assume_aligned< 16 >( state->cubic.filter );

    src -= 1;
    for ( float& out_sample : dst ) {
        const uint pi{ frac >> CubicPhaseDiffBits };
        const float pf{ static_cast< float >( frac & CubicPhaseDiffMask ) *
...
template <>
void Resample_< BSincTag, NEONTag >( const InterpState* state,
                                     const float* RESTRICT src,
                                     uint frac,
                                     const uint increment,
                                     const al::span< float > dst ) {
    ...
    for ( float& out_sample : dst ) {
        ...
        const float pf{ static_cast< float >( frac & BSincPhaseDiffMask ) *
                        ( 1.0f / BSincPhaseDiffOne ) };
        ...
        float32x4_t r4{ vdupq_n_f32( 0.0f ) };
        {
            ...
            do {
                ...
                const float32x4_t f4 =
                    vmlaq_f32( vmlaq_f32( vld1q_f32( &fil[ j ] ), sf4,
                                          vld1q_f32( &scd[ j ] ) ),
                               pf4,
                               vmlaq_f32( vld1q_f32( &phd[ j ] ), sf4,
                                          vld1q_f32( &spd[ j ] ) ) );
                /* r += f*src */
...
template <>
void Resample_< FastBSincTag, NEONTag >( const InterpState* state,
                                         const float* RESTRICT src,
                                         uint frac,
                                         const uint increment,
                                         const al::span< float > dst ) {
    ...
    for ( float& out_sample : dst ) {
        ...
        float32x4_t r4{ vdupq_n_f32( 0.0f ) };
        {
            ...
            do {
                /* f = fil + pf*phd */
                const float32x4_t f4 = vmlaq_f32( vld1q_f32( &fil[ j ] ), pf4,
                                                  vld1q_f32( &phd[ j ] ) );
                /* r += f*src */
                r4 = vmlaq_f32( r4, f4, vld1q_f32( &src[ j ] ) );
                j += 4;
...
template <>
void MixHrtf_< NEONTag >( const float* InSamples,
                          float2* AccumSamples,
                          const uint IrSize,
                          const MixHrtfFilter* hrtfparams,
                          const size_t BufferSize ) {
    MixHrtfBase< ApplyCoeffs >( InSamples, AccumSamples, IrSize, hrtfparams,
                                BufferSize );
}

template <>
void MixHrtfBlend_< NEONTag >( const float* InSamples,
                               float2* AccumSamples,
                               const uint IrSize,
                               const HrtfFilter* oldparams,
                               const MixHrtfFilter* newparams,
                               const size_t BufferSize ) {
    MixHrtfBlendBase< ApplyCoeffs >( InSamples, AccumSamples, IrSize, oldparams,
                                     newparams, BufferSize );
}

template <>
void MixDirectHrtf_< NEONTag >(
    ...
    const size_t IrSize,
    const size_t BufferSize ) {
    MixDirectHrtfBase< ApplyCoeffs >( LeftOut, RightOut, InSamples,
                                      AccumSamples, TempBuf, ChanState, IrSize,
                                      BufferSize );
}

template <>
void Mix_< NEONTag >( const al::span< const float > InSamples,
                      const al::span< FloatBufferLine > OutBuffer,
                      float* CurrentGains,
                      const float* TargetGains,
                      const size_t Counter,
                      const size_t OutPos ) {
    const float delta{ ( Counter > 0 ) ? 1.0f / static_cast< float >( Counter )
                                       : 0.0f };
    const auto min_len = minz( Counter, InSamples.size() );
    const auto aligned_len =
        minz( ( min_len + 3 ) & ~size_t{ 3 }, InSamples.size() ) - min_len;

    for ( FloatBufferLine& output : OutBuffer )
        MixLine( InSamples, al::assume_aligned< 16 >( output.data() + OutPos ),
                 *CurrentGains++, *TargetGains++, delta, min_len, aligned_len,
                 Counter );
}
...
void Mix_< NEONTag >( const al::span< const float > InSamples,
    ...
    MixLine( InSamples, al::assume_aligned< 16 >( OutBuffer ), CurrentGain,
             TargetGain, delta, min_len, aligned_len, Counter );
}
