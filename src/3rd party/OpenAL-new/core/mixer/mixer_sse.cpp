    ...
    if ( !( reinterpret_cast< uintptr_t >( Values ) & 15 ) ) {
        ...
        __m128 vals{
            _mm_loadl_pi( _mm_setzero_ps(),
                          reinterpret_cast< __m64* >( Values[ 0 ].data() ) ) };
        ...
        vals = _mm_loadl_pi( vals,
                             reinterpret_cast< __m64* >( Values[ i ].data() ) );
...
template <>
void Resample_< CubicTag, SSETag >( const InterpState* state,
...
template <>
void Resample_< BSincTag, SSETag >( const InterpState* state,
                                    const float* RESTRICT src,
                                    uint frac,
                                    const uint increment,
                                    const al::span< float > dst ) {
    ...
    for ( float& out_sample : dst ) {
        ...
        __m128 r4{ _mm_setzero_ps() };
        {
            ...
            do {
                ...
                const __m128 f4 = MLA4( MLA4( _mm_load_ps( &fil[ j ] ), sf4,
                                              _mm_load_ps( &scd[ j ] ) ),
                                        pf4,
                                        MLA4( _mm_load_ps( &phd[ j ] ), sf4,
                                              _mm_load_ps( &spd[ j ] ) ) );
                /* r += f*src */
...
template <>
void Resample_< FastBSincTag, SSETag >( const InterpState* state,
                                        const float* RESTRICT src,
                                        uint frac,
                                        const uint increment,
                                        const al::span< float > dst ) {
    ...
    for ( float& out_sample : dst ) {
        ...
        __m128 r4{ _mm_setzero_ps() };
        {
            ...
            do {
                /* f = fil + pf*phd */
                const __m128 f4 = MLA4( _mm_load_ps( &fil[ j ] ), pf4,
                                        _mm_load_ps( &phd[ j ] ) );
                /* r += f*src */
                r4 = MLA4( r4, f4, _mm_loadu_ps( &src[ j ] ) );
                j += 4;
...
template <>
void MixHrtf_< SSETag >( const float* InSamples,
                         float2* AccumSamples,
                         const uint IrSize,
...
template <>
void MixHrtfBlend_< SSETag >( const float* InSamples,
                              float2* AccumSamples,
                              const uint IrSize,
...
template <>
void Mix_< SSETag >( const al::span< const float > InSamples,
