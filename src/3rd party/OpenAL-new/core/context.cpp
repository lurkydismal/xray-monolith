...
static_assert(
    std::atomic< ContextBase::AsyncEventBitset >::is_always_lock_free,
...
ContextBase::~ContextBase() {
    size_t count{ 0 };
    ContextProps* cprops{
        mParams.ContextUpdate.exchange( nullptr, std::memory_order_relaxed ) };
    if ( cprops ) {
        ++count;
        delete cprops;
    }
    ...
    count = 0;
    EffectSlotProps* eprops{
        mFreeEffectslotProps.exchange( nullptr, std::memory_order_acquire ) };
    while ( eprops ) {
    ...
    if ( EffectSlotArray * curarray{ mActiveAuxSlots.exchange(
    ...
    if ( mAsyncEvents ) {
        ...
        if ( evt_vec.first.len > 0 ) {
            al::destroy_n( reinterpret_cast< AsyncEvent* >( evt_vec.first.buf ),
                           evt_vec.first.len );
            count += evt_vec.first.len;
        }
        if ( evt_vec.second.len > 0 ) {
            al::destroy_n(
                reinterpret_cast< AsyncEvent* >( evt_vec.second.buf ),
                evt_vec.second.len );
...
void ContextBase::allocVoiceChanges() {
    ...
    VoiceChangeCluster cluster{
        std::make_unique< VoiceChange[] >( clustersize ) };
    for ( size_t i{ 1 }; i < clustersize; ++i )
        cluster[ i - 1 ].mNext.store( std::addressof( cluster[ i ] ),
...
void ContextBase::allocVoiceProps() {
    ...
    VoicePropsCluster cluster{
        std::make_unique< VoicePropsItem[] >( clustersize ) };
    for ( size_t i{ 1 }; i < clustersize; ++i )
        cluster[ i - 1 ].next.store( std::addressof( cluster[ i ] ),
                                     std::memory_order_relaxed );
    mVoicePropClusters.emplace_back( std::move( cluster ) );

    VoicePropsItem* oldhead{
        mFreeVoiceProps.load( std::memory_order_acquire ) };
    do {
        mVoicePropClusters.back()[ clustersize - 1 ].next.store(
            oldhead, std::memory_order_relaxed );
    } while ( mFreeVoiceProps.compare_exchange_weak(
                  oldhead, mVoicePropClusters.back().get(),
...
void ContextBase::allocVoices( size_t addcount ) {
    ...
    if ( addcount >= std::numeric_limits< int >::max() / clustersize -
                         mVoiceClusters.size() )
        throw std::runtime_error{ "Allocating too many voices" };
    const size_t totalcount{ ( mVoiceClusters.size() + addcount ) *
                             clustersize };
    TRACE( "Increasing allocated voices to %zu\n", totalcount );

    auto newarray = VoiceArray::Create( totalcount );
    while ( addcount ) {
        mVoiceClusters.emplace_back(
            std::make_unique< Voice[] >( clustersize ) );
        --addcount;
    }
    ...
    if ( auto* oldvoices = mVoices.exchange( newarray.release(),
                                             std::memory_order_acq_rel ) ) {
        mDevice->waitForMix();
        delete oldvoices;
    }
...
EffectSlot* ContextBase::getEffectSlot() {
    ...
    if ( 1 >= std::numeric_limits< int >::max() / EffectSlotClusterSize -
                  mEffectSlotClusters.size() )
        throw std::runtime_error{ "Allocating too many effect slots" };
    const size_t totalcount{ ( mEffectSlotClusters.size() + 1 ) *
                             EffectSlotClusterSize };
    TRACE( "Increasing allocated effect slots to %zu\n", totalcount );

    mEffectSlotClusters.emplace_back(
        std::make_unique< EffectSlot[] >( EffectSlotClusterSize ) );
