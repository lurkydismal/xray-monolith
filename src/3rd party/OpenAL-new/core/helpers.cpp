...
const PathNamePair& GetProcBinary() {
    ...
    DWORD len{ GetModuleFileNameW( nullptr, fullpath.data(),
                                   static_cast< DWORD >( fullpath.size() ) ) };
    while ( len == fullpath.size() ) {
        fullpath.resize( fullpath.size() << 1 );
        len = GetModuleFileNameW( nullptr, fullpath.data(),
                                  static_cast< DWORD >( fullpath.size() ) );
    }
    if ( len == 0 ) {
    ...
    if ( sep != fullpath.rend() ) {
        *sep = 0;
        procbin.emplace( wstr_to_utf8( fullpath.data() ),
                         wstr_to_utf8( al::to_address( sep.base() ) ) );
    } else
...
void DirectorySearch( const char* path,
                      const char* ext,
                      al::vector< std::string >* const results ) {
    ...
    const al::span< std::string > newlist{ results->data() + base,
                                           results->size() - base };
...
al::vector< std::string > SearchDataFiles( const char* ext,
                                           const char* subdir ) {
...
const PathNamePair& GetProcBinary() {
    ...
    if ( pathname.empty() ) {
        ...
        if ( proc_pidpath( pid, procpath, sizeof( procpath ) ) < 1 )
            ...
            pathname.insert( pathname.end(), procpath,
                             procpath + strlen( procpath ) );
    }
#endif
#ifdef __HAIKU__
    if ( pathname.empty() ) {
        char procpath[ PATH_MAX ];
        if ( find_path( B_APP_IMAGE_SYMBOL, B_FIND_PATH_IMAGE_PATH, NULL,
                        procpath, sizeof( procpath ) ) == B_OK )
            pathname.insert( pathname.end(), procpath,
                             procpath + strlen( procpath ) );
    }
#endif
    ...
    if ( pathname.empty() ) {
        static const char SelfLinkNames[][ 32 ]{
            "/proc/self/exe", "/proc/self/file", "/proc/curproc/exe",
            "/proc/curproc/file" };
...
al::vector< std::string > SearchDataFiles( const char* ext,
    ...
    std::string datadirs{ al::getenv( "XDG_DATA_DIRS" )
    ...
    while ( curpos < datadirs.size() ) {
        ...
        std::string path{ ( nextpos != std::string::npos )
                              ? datadirs.substr( curpos, nextpos++ - curpos )
                              : datadirs.substr( curpos ) };
        curpos = nextpos;
...
bool SetRTPriorityPthread( int prio ) {
    ...
    err = pthread_setschedparam( pthread_self(), SCHED_RR | SCHED_RESET_ON_FORK,
                                 &param );
    if ( err == EINVAL )
#endif
...
bool SetRTPriorityRTKit( int prio ) {
    ...
    auto limit_rttime = []( DBusConnection* c ) -> int {
        ...
        if ( maxrttime <= 0 )
            return static_cast< int >( std::abs( maxrttime ) );
        ...
        TRACE( "RTTime max: %llu (hard: %llu, soft: %llu)\n", umaxtime,
               static_cast< ulonglong >( rlim.rlim_max ),
               static_cast< ulonglong >( rlim.rlim_cur ) );
        if ( rlim.rlim_max > umaxtime ) {
            rlim.rlim_max = static_cast< rlim_t >( std::min< ulonglong >(
                umaxtime, std::numeric_limits< rlim_t >::max() ) );
