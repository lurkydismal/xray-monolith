...
al::optional< std::string > make_error( size_t linenum, const char* fmt, ... ) {
    ...
    str.resize( 256 );
    int printed{ std::snprintf( const_cast< char* >( str.data() ), str.length(),
                                "Line %zu: ", linenum ) };
    if ( printed < 0 )
        printed = 0;
    ...
    while ( f.good() && std::getline( f, buffer ) ) {
        ...
        if ( scope == ReaderScope::Speakers ) {
            if ( command == "add_spkr" ) {
                ...
                return make_error( linenum, "Unexpected speakers command: %s",
                                   command.c_str() );
        } else if ( scope == ReaderScope::LFMatrix ||
                    scope == ReaderScope::HFMatrix ) {
            auto& gains =
                ( scope == ReaderScope::LFMatrix ) ? LFOrderGain : HFOrderGain;
            auto* matrix =
                ( scope == ReaderScope::LFMatrix ) ? LFMatrix : HFMatrix;
            auto& pos = ( scope == ReaderScope::LFMatrix ) ? lfmatrix_pos
                                                           : hfmatrix_pos;
            ...
            if ( command == "order_gain" ) {
                ...
                return make_error( linenum, "Unexpected matrix command: %s",
                                   command.c_str() );
        }
        // Global scope commands
            ...
            istr >> Version;
            if ( Version != 3 )
                return make_error( linenum, "Unsupported version: %d",
                                   Version );
            ...
            istr >> NumSpeakers;
            if ( !NumSpeakers )
                return make_error( linenum, "Invalid speakers: %zu",
                                   NumSpeakers );
            ...
            if ( scale == "n3d" )
                ...
                return make_error( linenum, "Unexpected coeff_scale: %s",
                                   scale.c_str() );
            ...
            istr >> XOverRatio;
        } else if ( command == "/opt/input_scale" ||
                    command == "/opt/nfeff_comp" ||
                    command == "/opt/delay_comp" ||
                    command == "/opt/level_comp" ) {
            /* Unused */
            read_word( istr );
        ...
        } else if ( command == "/lfmatrix/{" || command == "/hfmatrix/{" ||
                    command == "/matrix/{" ) {
            if ( !NumSpeakers )
                return make_error( linenum,
            ...
            if ( !Matrix ) {
                Matrix =
                    std::make_unique< CoeffArray[] >( NumSpeakers * FreqBands );
                LFMatrix = Matrix.get();
            ...
            if ( FreqBands == 1 ) {
                if ( command != "/matrix/{" )
                    return make_error(
                        linenum, "Unexpected \"%s\" for a single-band decoder",
                        command.c_str() );
                scope = ReaderScope::HFMatrix;
            } else {
                if ( command == "/lfmatrix/{" )
                    ...
                    return make_error(
                        linenum, "Unexpected \"%s\" for a dual-band decoder",
                        command.c_str() );
            }
        } else if ( command == "/end" ) {
            const auto endpos = static_cast< std::size_t >( istr.tellg() );
            if ( !is_at_end( buffer, endpos ) )
                return make_error( linenum, "Extra junk on end: %s",
                                   buffer.substr( endpos ).c_str() );
            ...
            return make_error( linenum, "Unexpected command: %s",
                               command.c_str() );

        istr.clear();
        const auto endpos = static_cast< std::size_t >( istr.tellg() );
        if ( !is_at_end( buffer, endpos ) )
            return make_error( linenum, "Extra junk on line: %s",
                               buffer.substr( endpos ).c_str() );
        buffer.clear();
    }
