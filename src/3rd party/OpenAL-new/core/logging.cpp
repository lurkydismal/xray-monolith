...
void al_print( LogLevel level, FILE* logfile, const char* fmt, ... ) {
    ...
    switch ( level ) {
        case LogLevel::Disable:
            break;
        case LogLevel::Error:
            prefix = al::as_span( "[ALSOFT] (EE) " ).first< 14 >();
            break;
        case LogLevel::Warning:
            prefix = al::as_span( "[ALSOFT] (WW) " ).first< 14 >();
            break;
        case LogLevel::Trace:
            prefix = al::as_span( "[ALSOFT] (II) " ).first< 14 >();
            break;
    }
    ...
    if ( msglen >= 0 && static_cast< size_t >( msglen ) >= msg.size() )
        UNLIKELY {
            dynmsg.resize( static_cast< size_t >( msglen ) + prefix.size() +
                           1u );
