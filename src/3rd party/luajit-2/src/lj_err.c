...
#if defined( __GNUC__ ) && \
    ( LJ_TARGET_X64 || defined( LUAJIT_UNWIND_EXTERNAL ) )
        ...
        switch ( frame_typep( frame ) ) {
            ...
            case FRAME_CP:                     /* Protected C frame. */
                if ( cframe_canyield( cf ) ) { /* Resume? */
                    if ( errcode ) {
                        hook_leave( G( L ) ); /* Assumes nobody uses coroutines
                                                 inside hooks. */
                        L->cframe = NULL;
...
LJ_FUNCA int lj_err_unwind_dwarf( int version,
                                  int actions,
                                  uint64_t uexclass,
    ...
    if ( ( actions & _UA_CLEANUP_PHASE ) ) {
         ...
         * with
         ** it on non-x64 because the interpreter restores all callee-saved
         * regs.
         */
...
LJ_FUNCA int lj_err_unwind_arm( int state, void* ucb, _Unwind_Context* ctx ) {
    ...
    if ( ( state & ( _US_ACTION_MASK | _US_FORCE_UNWIND ) ) ==
...
VOID RtlUnwindEx_FIXED( PVOID, PVOID, PVOID, PVOID, PVOID, PVOID ) asm(
    "RtlUnwindEx" );
...
lj_err_unwind_win64( EXCEPTION_RECORD* rec,
                     void* cf,
                     CONTEXT* ctx,
                     UndocumentedDispatcherContext* dispatch ) {
    lua_State* L = cframe_L( cf );
    int errcode = LJ_EXCODE_CHECK( rec->ExceptionCode )
                      ? LJ_EXCODE_ERRCODE( rec->ExceptionCode )
                      : LUA_ERRRUN;
    if ( ( rec->ExceptionFlags & 6 ) ) { /* EH_UNWINDING|EH_EXIT_UNWIND */
        ...
        if ( cf2 ) { /* We catch it, so start unwinding the upper frames. */
            ...
            */
            RtlUnwindEx(
                cf,
                ( void* )( ( cframe_unwind_ff( cf2 ) && errcode != LUA_YIELD )
                               ? lj_vm_unwind_ff_eh
                               : lj_vm_unwind_c_eh ),
                rec, ( void* )( uintptr_t )errcode, ctx,
...
static void err_raise_ext( int errcode ) {
    RaiseException( LJ_EXCODE_MAKE( errcode ), 1 /* EH_NONCONTINUABLE */, 0,
                    NULL );
}
...
LJ_NOINLINE void lj_err_mem( lua_State* L ) {
    if ( L->status ==
         LUA_ERRERR + 1 ) /* Don't touch the stack during lua_open. */
...
static ptrdiff_t finderrfunc( lua_State* L ) {
    ...
    void* cf = L->cframe;
    while ( frame > bot && cf ) {
        while ( cframe_nres( cframe_raw( cf ) ) <
                0 ) { /* cframe without frame? */
            if ( frame >= restorestack( L, -cframe_nres( cf ) ) )
                break;
            if ( cframe_errfunc( cf ) >=
                 0 ) /* Error handler not inherited (-1)? */
                return cframe_errfunc( cf );
            cf = cframe_prev(
                cf ); /* Else unwind cframe and continue searching. */
            if ( cf == NULL )
                return 0;
        }
        switch ( frame_typep( frame ) ) {
            ...
            case FRAME_PCALLH:
                if ( frame_ftsz( frame ) >=
                     ( ptrdiff_t )( 2 * sizeof( TValue ) ) ) /* xpcall? */
                    return savestack(
                        L, frame - 1 ); /* Point to xpcall's errorfunc. */
                return 0;
            default:
                lua_assert( 0 );
                return 0;
...
/* Lexer error. */
LJ_NOINLINE void lj_err_lex( lua_State* L,
                             GCstr* src,
                             const char* tok,
                             BCLine line,
                             ErrMsg em,
...
LJ_NOINLINE void lj_err_optype( lua_State* L, cTValue* o, ErrMsg opm ) {
    ...
    if ( curr_funcisL( L ) ) {
        ...
        const char* kind =
            lj_debug_slotname( pt, pc, ( BCReg )( o - L->base ), &oname );
        if ( kind )
...
LJ_NORET LJ_NOINLINE static void err_argmsg( lua_State* L,
                                             int narg,
                                             const char* msg ) {
    const char* fname = "?";
