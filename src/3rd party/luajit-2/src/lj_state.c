...
static void resizestack( lua_State* L, MSize n ) {
    ...
    GCobj* up;
    lua_assert( ( MSize )( tvref( L->maxstack ) - oldst ) ==
                L->stacksize - LJ_STACK_EXTRA - 1 );
    st = ( TValue* )lj_mem_realloc(
        L, tvref( L->stack ), ( MSize )( L->stacksize * sizeof( TValue ) ),
    ...
    for ( up = gcref( L->openupval ); up != NULL; up = gcnext( up ) )
        setmref( gco2uv( up )->v,
                 ( TValue* )( ( char* )uvval( gco2uv( up ) ) + delta ) );
...
void lj_state_relimitstack( lua_State* L ) {
    if ( L->stacksize > LJ_STACK_MAXEX &&
         L->top - tvref( L->stack ) < LJ_STACK_MAX - 1 )
...
void lj_state_shrinkstack( lua_State* L, MSize used ) {
    ...
    if ( 4 * used < L->stacksize &&
         ...
         obj2gco( L ) !=
             gcref( G( L )->jit_L ) ) /* Don't shrink stack of live trace. */
...
static void stack_init( lua_State* L1, lua_State* L ) {
    TValue *stend,
        *st = lj_mem_newvec( L, LJ_STACK_START + LJ_STACK_EXTRA, TValue );
    setmref( L1->stack, st );
...
static TValue* cpluaopen( lua_State* L, lua_CFunction dummy, void* ud ) {
    ...
    lj_lex_init( L );
    fixstring(
        lj_err_str( L, LJ_ERR_ERRMEM ) ); /* Preallocate memory error msg. */
...
#if LJ_64 && \
    !( defined( LUAJIT_USE_VALGRIND ) && defined( LUAJIT_USE_SYSMALLOC ) )
...
#endif
{
    ...
    L->status =
        LUA_ERRERR + 1; /* Avoid touching the stack upon memory error. */
    ...
    for ( i = 0;; ) {
        ...
        if ( lj_vm_cpcall( L, NULL, NULL, cpfinalize ) == 0 ) {
            ...
            if ( gcref( g->gc.mmudata ) ==
                 NULL ) /* Until nothing is left to do. */
                break;
        }
    }
    close_state( L );
