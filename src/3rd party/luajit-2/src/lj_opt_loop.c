...
static void loop_emit_phi( jit_State* J,
                           IRRef1* subst,
                           IRRef1* phi,
                           IRRef nphi,
                           SnapNo onsnap ) {
    ...
    for ( i = 0, j = 0; i < nphi; i++ ) {
        ...
        if ( lref == rref ||
             rref == REF_DROP ) { /* Invariants are redundant. */
    ...
    if ( passx ) {
        ...
        for ( i = J->cur.nins - 1; i > invar; i-- ) {
            ...
            if ( !irref_isk( ir->op1 ) ) {
                ...
                if ( ir->op1 < invar && ir->o >= IR_CALLN &&
                     ir->o <= IR_CARG ) { /* ORDER IR */
                    ir = IR( ir->op1 );
                    while ( ir->o == IR_CARG ) {
                        if ( !irref_isk( ir->op2 ) )
                            irt_clearmark( IR( ir->op2 )->t );
    ...
    while ( passx ) {
        ...
        for ( i = 0; i < nphi; i++ ) {
            ...
            IRIns* ir = IR( lref );
            if ( !irt_ismarked(
                     ir->t ) ) { /* Propagate only from unmarked PHIs. */
                IRIns* irr = IR( subst[ lref ] );
                if ( irt_ismarked(
                         irr->t ) ) { /* Right ref points to other PHI? */
                    irt_clearmark(
                        irr->t ); /* Mark that PHI as non-redundant. */
                    passx = 1;    /* Retry. */
                }
            }
        }
...
static void loop_subst_snap( jit_State* J,
                             SnapShot* osnap,
                             SnapEntry* loopmap,
                             IRRef1* subst ) {
    ...
    while ( on < onent ) {
        ...
        if ( snap_slot( lsn ) <
             snap_slot( osn ) ) { /* Copy slot from loop map. */
            nmap[ nn++ ] = lsn;
            ln++;
    ...
    while ( snap_slot( loopmap[ ln ] ) <
            nslots ) /* Copy remaining loop slots. */
        nmap[ nn++ ] = loopmap[ ln++ ];
...
static void loop_unroll( jit_State* J ) {
    ...
    */
    invar = J->cur.nins;
    subst =
        ( IRRef1* )lj_str_needbuf( J->L, &G( J->L )->tmpbuf,
                                   ( invar - REF_BIAS ) * sizeof( IRRef1 ) ) -
        REF_BIAS;
    ...
    */
    ...
    lj_snap_grow_map( J, J->cur.nsnapmap * 2 +
                             ( onsnap - 2 ) * J->cur.snap[ onsnap - 1 ].nent );
    ...
    *psentinel = SNAP( 255, 0, 0 ); /* Replace PC with temporary sentinel. */
    ...
    for ( ins = REF_FIRST; ins < invar; ins++ ) {
        ...
        if ( ins >= osnap->ref ) /* Instruction belongs to next snapshot? */
            loop_subst_snap( J, osnap++, loopmap,
                             subst ); /* Copy-substitute it. */
        ...
        if ( !irref_isk( op2 ) )
            op2 = subst[ op2 ];
        if ( irm_kind( lj_ir_mode[ ir->o ] ) == IRM_N && op1 == ir->op1 &&
             op2 == ir->op2 ) {           /* Regular invariant ins? */
            subst[ ins ] = ( IRRef1 )ins; /* Shortcut. */
        } else {
            /* Re-emit substituted instruction to the FOLD/CSE/etc. pipeline. */
            IRType1 t =
                ir->t; /* Get this first, since emitir may invalidate ir. */
            ...
            if ( ref != ins ) {
                ...
                if ( ref < invar ) { /* Loop-carried dependency? */
                    /* Potential PHI? */
                    if ( !irref_isk( ref ) && !irt_isphi( irr->t ) &&
                         !irt_ispri( irr->t ) ) {
                        irt_setphi( irr->t );
                        if ( nphi >= LJ_MAX_PHI )
                            lj_trace_err( J, LJ_TRERR_PHIOV );
                    ...
                    if ( !irt_sametype( t, irr->t ) ) {
                        if ( irt_isinteger( t ) && irt_isinteger( irr->t ) )
                            continue;
                                  ...
                                  irt_isinteger( irr->t ) ) /* Fix int->num. */
                            ref = tref_ref( emitir( IRTN( IR_CONV ), ref,
                                                    IRCONV_NUM_INT ) );
                        else if ( irt_isnum( irr->t ) &&
                                  irt_isinteger( t ) ) /* Fix num->int. */
                            ref = tref_ref(
                                emitir( IRTGI( IR_CONV ), ref,
...
static void loop_undo( jit_State* J, IRRef ins, SnapNo nsnap, MSize nsnapmap ) {
    ...
    map[ snap->mapofs + snap->nent ] =
        map[ J->cur.snap[ 0 ].nent ]; /* Restore PC. */
