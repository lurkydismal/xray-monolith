...
static GCupval* func_finduv( lua_State* L, TValue* slot ) {
    ...
    while ( gcref( *pp ) != NULL &&
            uvval( ( p = gco2uv( gcref( *pp ) ) ) ) >= slot ) {
        lua_assert( !p->closed && uvval( p ) != &p->tv );
        if ( uvval( p ) ==
             slot ) { /* Found open upvalue pointing to same slot? */
    ...
    setgcrefr( uv->nextgc,
               *pp ); /* Insert into sorted list of open upvalues. */
...
static GCfunc* func_newL( lua_State* L, GCproto* pt, GCtab* env ) {
    uint32_t count;
    GCfunc* fn =
        ( GCfunc* )lj_mem_newgco( L, sizeLfunc( ( MSize )pt->sizeuv ) );
    fn->l.gct = ~LJ_TFUNC;
    fn->l.ffid = FF_LUA;
...
GCfunc* lj_func_newL_empty( lua_State* L, GCproto* pt, GCtab* env ) {
    ...
    for ( i = 0; i < nuv; i++ ) {
        ...
        uv->dhash = ( uint32_t )( uintptr_t )pt ^
                    ( ( uint32_t )proto_uv( pt )[ i ] << 24 );
...
GCfunc* lj_func_newL_gc( lua_State* L, GCproto* pt, GCfuncL* parent ) {
    ...
    for ( i = 0; i < nuv; i++ ) {
        ...
        if ( ( v & PROTO_UV_LOCAL ) ) {
            ...
            uv->dhash =
                ( uint32_t )( uintptr_t )mref( parent->pc, char ) ^ ( v << 24 );
        } else {
            uv = &gcref( puv[ v ] )->uv;
        }
        setgcref( fn->l.uvptr[ i ], obj2gco( uv ) );
