...
#else
    lua_pushstring( L,
                    getenv( luaL_checkstring( L, 1 ) ) ); /* if NULL push nil */
#endif
    return 1;
}
...
LJLIB_CF( os_clock ) {
    setnumV( L->top++, ( ( lua_Number )clock() ) *
                           ( 1.0 / ( lua_Number )CLOCKS_PER_SEC ) );
    return 1;
}
...
LJLIB_CF( os_difftime ) {
    lua_pushnumber(
        L, difftime( ( time_t )( luaL_checknumber( L, 1 ) ),
...
LJLIB_CF( os_setlocale ) {
    ...
    int opt = lj_lib_checkopt(
        L, 2, 6, "\5ctype\7numeric\4time\7collate\10monetary\1\377\3all" );
    if ( opt == 0 )
        opt = LC_CTYPE;
    else if ( opt == 1 )
        opt = LC_NUMERIC;
