...
void lj_snap_grow_map_( jit_State* J, MSize need ) {
    if ( need < 2 * J->sizesnapmap )
        ...
        need = 64;
    J->snapmapbuf = ( SnapEntry* )lj_mem_realloc(
        J->L, J->snapmapbuf, J->sizesnapmap * sizeof( SnapEntry ),
...
static MSize snapshot_slots( jit_State* J, SnapEntry* map, BCReg nslots ) {
    ...
    for ( s = 0; s < nslots; s++ ) {
        ...
        if ( ref ) {
            ...
            IRIns* ir = IR( ref );
            if ( !( sn & ( SNAP_CONT | SNAP_FRAME ) ) && ir->o == IR_SLOAD &&
                 ir->op1 == s && ref > retf ) {
                /* No need to snapshot unmodified non-inherited slots. */
                if ( !( ir->op2 & IRSLOAD_INHERIT ) )
                    continue;
                /* No need to restore readonly slots and unmodified non-parent
                 * slots. */
                if ( !( LJ_DUALNUM && ( ir->op2 & IRSLOAD_CONVERT ) ) &&
                     ( ir->op2 & ( IRSLOAD_READONLY | IRSLOAD_PARENT ) ) !=
                         IRSLOAD_PARENT )
                    sn |= SNAP_NORESTORE;
            }
...
static BCReg snapshot_framelinks( jit_State* J, SnapEntry* map ) {
    ...
    MSize f = 0;
    map[ f++ ] =
        SNAP_MKPC( J->pc ); /* The current PC is always the first entry. */
...
static BCReg snap_usedef( jit_State* J,
                          uint8_t* udf,
                          const BCIns* pc,
                          BCReg maxslot ) {
    BCReg s;
...
#define DEF_SLOT( s ) udf[ ( s ) ] *= 3
    ...
    lua_assert( pc >= proto_bc( J->pt ) &&
                pc < proto_bc( J->pt ) + J->pt->sizebc );
    for ( ;; ) {
        BCIns ins = *pc++;
        BCOp op = bc_op( ins );
        switch ( bcmode_b( op ) ) {
        ...
        switch ( bcmode_a( op ) ) {
            ...
            case BCMbase:
                if ( op >= BC_CALLM && op <= BC_VARG ) {
                    BCReg top = ( op == BC_CALLM || op == BC_CALLMT ||
                                  bc_c( ins ) == 0 )
                                    ? maxslot
                                    : ( bc_a( ins ) + bc_c( ins ) );
                    s = bc_a( ins ) -
                        ( ( op == BC_ITERC || op == BC_ITERN ) ? 3 : 0 );
                    for ( ; s < top; s++ )
                        USE_SLOT( s );
                    for ( ; s < maxslot; s++ )
                        DEF_SLOT( s );
...
void lj_snap_shrink( jit_State* J ) {
    ...
    J->cur.nsnapmap =
        ( uint16_t )( snap->mapofs + m ); /* Free up space in map. */
}
...
static TRef snap_replay_const( jit_State* J, IRIns* ir ) {
    ...
    switch ( ( IROp )ir->o ) {
        ...
        case IR_KNUM:
            return lj_ir_k64( J, IR_KNUM, ir_knum( ir ) );
        case IR_KINT64:
            return lj_ir_k64( J, IR_KINT64, ir_kint64( ir ) );
        case IR_KPTR:
    ...
    for ( j = 0; j < nmax; j++ )
        if ( snap_ref( map[ j ] ) == ref )
            return J->slot[ snap_slot( map[ j ] ) ] &
                   ~( SNAP_CONT | SNAP_FRAME );
    return 0;
}

/* Emit parent reference with de-duplication. */
static TRef snap_pref( jit_State* J,
                       GCtrace* T,
                       SnapEntry* map,
                       MSize nmax,
                       BloomFilter seen,
...
static int snap_sunk_store2( jit_State* J, IRIns* ira, IRIns* irs ) {
    if ( irs->o == IR_ASTORE || irs->o == IR_HSTORE || irs->o == IR_FSTORE ||
         irs->o == IR_XSTORE ) {
...
void lj_snap_replay( jit_State* J, GCtrace* T ) {
    ...
    setslot:
        J->slot[ s ] =
            tr |
            ( sn & ( SNAP_CONT | SNAP_FRAME ) ); /* Same as TREF_* flags. */
    ...
    if ( pass23 ) {
            ...
            if ( regsp_reg( ir->r ) == RID_SUNK ) {
                ...
                if ( LJ_HASFFI && ir->o == IR_CNEWI ) {
                    ...
                    for ( irs = ir + 1; irs < irlast; irs++ )
                        if ( irs->r == RID_SINK &&
                             snap_sunk_store( J, ir, irs ) ) {
                            if ( snap_pref( J, T, map, nent, seen, irs->op2 ) ==
                                 0 )
                                snap_pref( J, T, map, nent, seen,
                                           T->ir[ irs->op2 ].op1 );
        ...
        for ( n = 0; pass23 && n < nent; n++ ) {
            ...
            if ( regsp_reg( ir->r ) == RID_SUNK ) {
                TRef op1, op2;
                if ( J->slot[ snap_slot( sn ) ] !=
                     snap_slot( sn ) ) { /* De-dup allocs. */
                    J->slot[ snap_slot( sn ) ] =
                        J->slot[ J->slot[ snap_slot( sn ) ] ];
                    continue;
                }
                op1 = ir->op1;
                if ( op1 >= T->nk )
                    ...
                    for ( irs = ir + 1; irs < irlast; irs++ )
                        if ( irs->r == RID_SINK &&
                            ...
                            if ( val == 0 ) {
                                ...
                                lua_assert( irc->o == IR_CONV &&
                                            irc->op2 == IRCONV_NUM_INT );
                                val = snap_pref( J, T, map, nent, seen,
                                                 irc->op1 );
                                val = emitir( IRTN( IR_CONV ), val,
                                              IRCONV_NUM_INT );
                            } else if ( ( LJ_SOFTFP ||
                                          ( LJ_32 && LJ_HASFFI ) ) &&
                                        irs + 1 < irlast &&
                                ...
                                IRType t = IRT_I64;
                                if ( LJ_SOFTFP &&
                                     irt_type( ( irs + 1 )->t ) == IRT_SOFTFP )
                                    t = IRT_NUM;
                                lj_needsplit( J );
                                if ( irref_isk( irs->op2 ) &&
                                     irref_isk( ( irs + 1 )->op2 ) ) {
                                    uint64_t k =
                                        ( uint32_t )T->ir[ irs->op2 ].i +
                                        ( ( uint64_t )T->ir[ ( irs + 1 )->op2 ]
                                              .i
                                          << 32 );
                                    val = lj_ir_k64(
                                        J, t == IRT_I64 ? IR_KINT64 : IR_KNUM,
                                        lj_ir_k64_find( J, k ) );
                                } else {
                                    val = emitir_raw(
                                        IRT( IR_HIOP, t ), val,
                        ...
                        } else if ( LJ_HASFFI && irs->o == IR_XBAR &&
                                    ir->o == IR_CNEW ) {
...
static void snap_unsink( jit_State* J,
                         GCtrace* T,
                         ExitState* ex,
                         SnapNo snapno,
                         BloomFilter rfilt,
...
static void snap_restoreval( jit_State* J,
                             GCtrace* T,
                             ExitState* ex,
                             SnapNo snapno,
                             BloomFilter rfilt,
    ...
    if ( ra_hasspill( regsp_spill( rs ) ) ) { /* Restore from spill slot. */
        ...
        if ( irt_isinteger( t ) ) {
            ...
            lua_assert(
                !irt_ispri( t ) ); /* PRI refs never have a spill slot. */
            setgcrefi( o->gcr, *sps );
...
static void snap_restoredata( GCtrace* T,
                              ExitState* ex,
                              SnapNo snapno,
                              BloomFilter rfilt,
                              IRRef ref,
    ...
    if ( irref_isk( ref ) ) {
        ...
        if ( ra_hasspill( regsp_spill( rs ) ) ) {
            ...
            Reg r = regsp_reg( rs );
            if ( ra_noreg( r ) ) {
                /* Note: this assumes CNEWI is never used for SOFTFP split
                 * numbers. */
                lua_assert( sz == 8 && ir->o == IR_CONV &&
...
static void snap_unsink( jit_State* J,
    ...
    if ( ir->o == IR_CNEW || ir->o == IR_CNEWI ) {
        ...
        if ( ir->o == IR_CNEWI ) {
            ...
            if ( LJ_32 && sz == 8 && ir + 1 < T->ir + T->nins &&
                 ( ir + 1 )->o == IR_HIOP ) {
                snap_restoredata( T, ex, snapno, rfilt, ( ir + 1 )->op2,
                                  LJ_LE ? p + 4 : p, 4 );
                if ( LJ_BE )
                    p += 4;
                sz = 4;
            ...
            for ( irs = ir + 1; irs < irlast; irs++ )
                if ( irs->r == RID_SINK && snap_sunk_store( J, ir, irs ) ) {
                    ...
                    CTSize szs;
                    lua_assert( irs->o == IR_XSTORE &&
                                T->ir[ irs->op1 ].o == IR_ADD );
                    ...
                    if ( LJ_32 && irs + 1 < T->ir + T->nins &&
                         ( irs + 1 )->o == IR_HIOP ) {
                        lua_assert( szs == 4 );
                        snap_restoredata( T, ex, snapno, rfilt,
                                          ( irs + 1 )->op2, LJ_LE ? p + 4 : p,
                                          4 );
                        if ( LJ_BE )
                            p += 4;
                    }
    ...
    {
        IRIns *irs, *irlast;
        GCtab* t = ir->o == IR_TNEW
                       ? lj_tab_new( J->L, ir->op1, ir->op2 )
        ...
        for ( irs = ir + 1; irs < irlast; irs++ )
            if ( irs->r == RID_SINK && snap_sunk_store( J, ir, irs ) ) {
                ...
                if ( irk->o == IR_FREF ) {
                    ...
                    if ( LJ_SOFTFP && irs + 1 < T->ir + T->nins &&
                         ( irs + 1 )->o == IR_HIOP ) {
                        snap_restoreval( J, T, ex, snapno, rfilt,
                                         ( irs + 1 )->op2, &tmp );
...
const BCIns* lj_snap_restore( jit_State* J, void* exptr ) {
    ...
    frame = L->base - 1;
    ftsz0 =
        frame_ftsz( frame ); /* Preserve link to previous frame in slot #0. */
    for ( n = 0; n < nent; n++ ) {
        SnapEntry sn = map[ n ];
        if ( !( sn & SNAP_NORESTORE ) ) {
            ...
            if ( ir->r == RID_SUNK ) {
                ...
                for ( j = 0; j < n; j++ )
                    if ( snap_ref( map[ j ] ) ==
                         ref ) { /* De-duplicate sunk allocations. */
            ...
            if ( LJ_SOFTFP && ( sn & SNAP_SOFTFPNUM ) && tvisint( o ) ) {
                ...
                o->fr.tp.ftsz =
                    snap_slot( sn ) != 0 ? ( int32_t )*flinks-- : ftsz0;
                L->base = o + 1;
            }
        }
    }
