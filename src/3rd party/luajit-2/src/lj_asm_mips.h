...
static void asm_exitstub_setup( ASMState* as ) {
    ...
    *--mxp = MIPSI_J | ( ( ( ( uintptr_t )( void* )lj_vm_exit_handler ) >> 2 ) &
                         0x03ffffffu );
    lua_assert(
        ( ( uintptr_t )mxp ^ ( uintptr_t )( void* )lj_vm_exit_handler ) >> 28 ==
        0 );
...
static Reg asm_fuseahuref( ASMState* as,
                           IRRef ref,
                           int32_t* ofsp,
                           RegSet allow ) {
    IRIns* ir = IR( ref );
    if ( ra_noreg( ir->r ) ) {
        if ( ir->o == IR_AREF ) {
            ...
            if ( irref_isk( ir->op1 ) ) {
                ...
                int32_t ofs =
                    i32ptr( &gcref( fn->l.uvptr[ ( ir->op2 >> 8 ) ] )->uv.tv );
...
static void asm_fusexref( ASMState* as,
    ...
    if ( ra_noreg( ir->r ) && canfuse( as, ir ) ) {
        if ( ir->o == IR_ADD ) {
            int32_t ofs2;
            if ( irref_isk( ir->op2 ) &&
                 ( ofs2 = ofs + IR( ir->op2 )->i, checki16( ofs2 ) ) ) {
                ref = ir->op1;
                ofs = ofs2;
            }
    ...
    for ( n = 0; n < nargs; n++ ) { /* Setup args. */
        ...
        if ( ref ) {
            ...
            if ( irt_isfp( ir->t ) && fpr <= REGARG_LASTFPR &&
                 !( ci->flags & CCI_VARARG ) ) {
                lua_assert(
                    rset_test( as->freeset, fpr ) ); /* Already evicted. */
                ...
                if ( gpr <= REGARG_LASTGPR ) {
                    lua_assert(
                        rset_test( as->freeset, gpr ) ); /* Already evicted. */
                    if ( irt_isfp( ir->t ) ) {
                        RegSet of = as->freeset;
                        Reg r;
                        /* Workaround to protect argument GPRs from being used
                         * for remat. */
                        as->freeset &=
                            ~RSET_RANGE( REGARG_FIRSTGPR, REGARG_LASTGPR + 1 );
                        r = ra_alloc1( as, ref, RSET_FPR );
                        as->freeset |=
                            ( of & RSET_RANGE( REGARG_FIRSTGPR,
                                               REGARG_LASTGPR + 1 ) );
                        if ( irt_isnum( ir->t ) ) {
                            emit_tg( as, MIPSI_MFC1, gpr + ( LJ_BE ? 0 : 1 ),
                                     r + 1 );
                            emit_tg( as, MIPSI_MFC1, gpr + ( LJ_BE ? 1 : 0 ),
                                     r );
                            lua_assert( rset_test(
                                as->freeset, gpr + 1 ) ); /* Already evicted. */
                            gpr += 2;
                    ...
                    Reg r = ra_alloc1z(
                        as, ref, irt_isfp( ir->t ) ? RSET_FPR : RSET_GPR );
                    if ( irt_isnum( ir->t ) )
                        ofs = ( ofs + 4 ) & ~4;
    ...
    if ( ra_used( ir ) ) {
        ...
        if ( irt_isfp( ir->t ) ) {
            if ( ( ci->flags & CCI_CASTU64 ) ) {
                ...
                if ( ofs ) {
                    emit_tsi( as, MIPSI_SW, RID_RETLO, RID_SP,
                              ofs + ( LJ_BE ? 4 : 0 ) );
                    emit_tsi( as, MIPSI_SW, RID_RETHI, RID_SP,
                              ofs + ( LJ_BE ? 0 : 4 ) );
                }
            } else {
...
static void asm_callround( ASMState* as, IRIns* ir, IRCallID id ) {
    ...
    RegSet drop = RID2RSET( RID_R1 ) | RID2RSET( RID_R12 ) |
                  RID2RSET( RID_FPRET ) | RID2RSET( RID_F2 ) |
                  RID2RSET( RID_F4 ) | RID2RSET( REGARG_FIRSTFPR );
    if ( ra_hasreg( ir->r ) )
...
static void asm_conv( ASMState* as, IRIns* ir ) {
    ...
    if ( irt_isfp( ir->t ) ) {
        ...
        if ( stfp ) { /* FP to FP conversion. */
            emit_fg( as, st == IRT_NUM ? MIPSI_CVT_S_D : MIPSI_CVT_D_S, dest,
            ...
            if ( irt_isfloat( ir->t ) )
                emit_lsptr( as, MIPSI_LWC1, ( tmp & 31 ),
                            ( void* )lj_ir_k64_find(
                                as->J, U64x( 4f000000, 4f000000 ) ),
                            RSET_GPR );
            else
                emit_lsptr( as, MIPSI_LDC1, ( tmp & 31 ),
                            ( void* )lj_ir_k64_find(
                                as->J, U64x( 41e00000, 00000000 ) ),
                            RSET_GPR );
        ...
        if ( irt_isguard( ir->t ) ) {
            ...
            if ( irt_isu32( ir->t ) ) {
                ...
                emit_tg( as, MIPSI_MFC1, dest, tmp );
                emit_fg( as,
                         st == IRT_FLOAT ? MIPSI_FLOOR_W_S : MIPSI_FLOOR_W_D,
                         tmp, tmp );
                emit_fgh( as, st == IRT_FLOAT ? MIPSI_SUB_S : MIPSI_SUB_D, tmp,
                          left, tmp );
                if ( st == IRT_FLOAT )
                    emit_lsptr( as, MIPSI_LWC1, ( tmp & 31 ),
                                ( void* )lj_ir_k64_find(
                ...
                emit_tg( as, MIPSI_MFC1, dest, tmp );
                emit_fg( as,
                         st == IRT_FLOAT ? MIPSI_TRUNC_W_S : MIPSI_TRUNC_W_D,
                         tmp, left );
            }
        }
    } else {
        ...
        if ( st >= IRT_I8 && st <= IRT_U16 ) { /* Extend to 32 bit integer. */
            ...
            if ( ( ir->op2 & IRCONV_SEXT ) ) {
                if ( ( as->flags & JIT_F_MIPS32R2 ) ) {
                    emit_dst( as, st == IRT_I8 ? MIPSI_SEB : MIPSI_SEH, dest, 0,
                              left );
                } else {
...
static void asm_conv64( ASMState* as, IRIns* ir ) {
    ...
    if ( st == IRT_NUM || st == IRT_FLOAT ) {
        id = IRCALL_fp64_d2l + ( ( st == IRT_FLOAT ) ? 2 : 0 ) +
             ( dt - IRT_I64 );
        ir--;
    } else {
        id = IRCALL_fp64_l2d + ( ( dt == IRT_FLOAT ) ? 2 : 0 ) +
             ( st - IRT_I64 );
    }
...
static void asm_strto( ASMState* as, IRIns* ir ) {
    ...
    emit_tsi( as, MIPSI_ADDIU, ra_releasetmp( as, ASMREF_TMP1 ), RID_SP,
              sps_scale( ir->s ) );
}
...
static void asm_tvptr( ASMState* as, Reg dest, IRRef ref ) {
    ...
    if ( irt_isnum( ir->t ) ) {
        if ( irref_isk(
                 ref ) ) /* Use the number constant itself as a TValue. */
        ...
        Reg type;
        emit_tsi( as, MIPSI_ADDIU, dest, RID_JGL,
                  offsetof( global_State, tmptv ) - 32768 );
        if ( !irt_ispri( ir->t ) ) {
...
static void asm_href( ASMState* as, IRIns* ir ) {
    ...
    Reg key = RID_NONE, type = RID_NONE, tmpnum = RID_NONE, tmp1 = RID_TMP,
        tmp2;
    IRRef refkey = ir->op2;
    ...
    *l_loop =
    ...
    if ( khash == 0 ) {
        ...
        emit_tsi( as, MIPSI_LW, dest, tab, ( int32_t )offsetof( GCtab, node ) );
        emit_tsi( as, MIPSI_LW, tmp2, tab,
                  ( int32_t )offsetof( GCtab, hmask ) );
        if ( irref_isk( refkey ) ) {
            /* Nothing to do. */
        } else if ( irt_isstr( kt ) ) {
            emit_tsi( as, MIPSI_LW, tmp1, key,
                      ( int32_t )offsetof( GCstr, hash ) );
            ...
            if ( irt_isnum( kt ) ) {
                ...
                emit_dst( as, MIPSI_ADDU, tmp1, key,
                          ra_allock( as, HASH_BIAS, allow ) );
            }
        }
    }
}
...
static void asm_hrefk( ASMState* as, IRIns* ir ) {
    ...
    Reg dest = ( ra_used( ir ) || ofs > 32736 ) ? ra_dest( as, ir, RSET_GPR )
                                                : RID_NONE;
...
nolo:
    asm_guard( as, MIPSI_BNE, type,
               hi ? ra_allock( as, hi, allow ) : RID_ZERO );
    if ( ra_hasreg( key ) )
...
static void asm_uref( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ir->op1 ) ) {
        ...
        if ( ir->o == IR_UREFC ) {
            ...
            emit_tsi( as, MIPSI_ADDIU, dest, uv,
                      ( int32_t )offsetof( GCupval, tv ) );
            emit_tsi( as, MIPSI_LBU, RID_TMP, uv,
                      ( int32_t )offsetof( GCupval, closed ) );
        } else {
            emit_tsi( as, MIPSI_LW, dest, uv,
                      ( int32_t )offsetof( GCupval, v ) );
        }
...
static void asm_strref( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ref ) ) {
        ...
        if ( ra_hasreg( irr->r ) ) {
            ra_noweak( as, irr->r );
            right = irr->r;
        } else if ( mayfuse( as, irr->op2 ) && irr->o == IR_ADD &&
                    irref_isk( irr->op2 ) &&
...
static void asm_xstore( ASMState* as, IRIns* ir, int32_t ofs ) {
    if ( ir->r != RID_SINK ) {
        Reg src =
            ra_alloc1z( as, ir->op2, irt_isfp( ir->t ) ? RSET_FPR : RSET_GPR );
...
static void asm_ahuvload( ASMState* as, IRIns* ir ) {
    ...
    if ( irt_isnum( t ) ) {
        ...
        asm_guard( as, MIPSI_BNE, type,
                   ra_allock( as, irt_toitype( t ), allow ) );
        if ( ra_hasreg( dest ) )
...
static void asm_cnew( ASMState* as, IRIns* ir ) {
    ...
    emit_ti( as, MIPSI_LI, RID_TMP,
             ctypeid ); /* Lower 16 bit used. Sign-ext ok. */
    asm_gencall( as, ci, args );
...
static void asm_obar( ASMState* as, IRIns* ir ) {
    ...
    asm_gencall( as, ci, args );
    emit_tsi( as, MIPSI_ADDIU, ra_releasetmp( as, ASMREF_TMP1 ), RID_JGL,
              -32768 );
    obj = IR( ir->op1 )->r;
    ...
    emit_tsi( as, MIPSI_LBU, tmp, obj,
              ( int32_t )offsetof( GCupval, marked ) -
                  ( int32_t )offsetof( GCupval, tv ) );
    emit_tsi( as, MIPSI_LBU, RID_TMP, val,
...
static void asm_arithov( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ir->op2 ) ) {
        ...
        if ( checki16( k ) ) { /* (dest < left) == (k >= 0 ? 1 : 0) */
            ...
            emit_dst( as, MIPSI_SLT, RID_TMP, dest,
                      dest == left ? RID_TMP : left );
            emit_tsi( as, MIPSI_ADDIU, dest, left, k );
            if ( dest == left )
...
loarith:
    ...
    if ( dest == left && dest == right ) {
        Reg tmp = ra_scratch(
            as, rset_exclude( rset_exclude( RSET_GPR, left ), right ) );
        emit_move( as, dest, tmp );
        dest = tmp;
    }
...
static void asm_bitswap( ASMState* as, IRIns* ir ) {
    ...
    if ( ( as->flags & JIT_F_MIPS32R2 ) ) {
        ...
        Reg tmp = ra_scratch(
            as, rset_exclude( rset_exclude( RSET_GPR, left ), dest ) );
...
static void asm_bitror( ASMState* as, IRIns* ir ) {
    if ( ( as->flags & JIT_F_MIPS32R2 ) ) {
        ...
        if ( irref_isk( ir->op2 ) ) { /* Constant shifts. */
            ...
            emit_dst( as, MIPSI_SUBU, RID_TMP, ra_allock( as, 32, RSET_GPR ),
                      right );
        }
    }
}
...
static void asm_min_max( ASMState* as, IRIns* ir, int ismax ) {
    if ( irt_isnum( ir->t ) ) {
        ...
        emit_fgh( as, MIPSI_C_OLT_D, 0, ismax ? left : right,
                  ismax ? right : left );
    } else {
        Reg dest = ra_dest( as, ir, RSET_GPR );
        Reg right, left = ra_alloc2( as, ir, RSET_GPR );
        ...
        emit_dst( as, MIPSI_SLT, RID_TMP, ismax ? left : right,
                  ismax ? right : left );
    }
}
...
static void asm_comp( ASMState* as, IRIns* ir ) {
    ...
    if ( irt_isnum( ir->t ) ) {
        ...
        emit_fgh( as, MIPSI_C_OLT_D + ( ( op & 3 ) ^ ( ( op >> 2 ) & 1 ) ), 0,
                  left, right );
    } else {
        Reg right, left = ra_alloc1( as, ir->op1, RSET_GPR );
        if ( op == IR_ABC )
        ...
        if ( ( op & 4 ) == 0 && irref_isk( ir->op2 ) &&
            ...
            if ( irref_isk( ir->op2 ) ) {
                ...
                if ( ( op & 2 ) )
                    k++;
                if ( checki16( k ) ) {
                    asm_guard( as, ( op & 1 ) ? MIPSI_BNE : MIPSI_BEQ, RID_TMP,
                               RID_ZERO );
            ...
            right = ra_alloc1( as, ir->op2, rset_exclude( RSET_GPR, left ) );
            asm_guard( as, ( ( op ^ ( op >> 1 ) ) & 1 ) ? MIPSI_BNE : MIPSI_BEQ,
                       RID_TMP, RID_ZERO );
...
static void asm_compeq( ASMState* as, IRIns* ir ) {
    Reg right,
        left = ra_alloc2( as, ir, irt_isnum( ir->t ) ? RSET_FPR : RSET_GPR );
    right = ( left >> 8 );
    left &= 255;
    if ( irt_isnum( ir->t ) ) {
...
static void asm_comp64( ASMState* as, IRIns* ir ) {
    ...
    lefthi &= 255;
    leftlo = ra_alloc2(
        as, ir - 1, rset_exclude( rset_exclude( RSET_GPR, lefthi ), righthi ) );
    rightlo = ( leftlo >> 8 );
    leftlo &= 255;
    asm_guard( as, ( ( op ^ ( op >> 1 ) ) & 1 ) ? MIPSI_BNE : MIPSI_BEQ,
               RID_TMP, RID_ZERO );
    ...
    if ( lefthi != righthi )
        emit_dst( as, ( op & 4 ) ? MIPSI_SLTU : MIPSI_SLT, RID_TMP,
                  ( op & 2 ) ? righthi : lefthi,
                  ( op & 2 ) ? lefthi : righthi );
    emit_dst( as, MIPSI_SLTU, RID_TMP, ( op & 2 ) ? rightlo : leftlo,
              ( op & 2 ) ? leftlo : rightlo );
    if ( lefthi != righthi )
...
static void asm_comp64eq( ASMState* as, IRIns* ir ) {
    ...
    right = ( left >> 8 );
    left &= 255;
    asm_guard( as, ( ( ir - 1 )->o & 1 ) ? MIPSI_BEQ : MIPSI_BNE, RID_TMP,
               RID_ZERO );
    tmp =
        ra_scratch( as, rset_exclude( rset_exclude( RSET_GPR, left ), right ) );
...
static void asm_hiop( ASMState* as, IRIns* ir ) {
    ...
    if ( ( ir - 1 )->o == IR_CONV ) { /* Conversions to/from 64 bit. */
        ...
        return;
                ...
                IR_EQ ) { /* 64 bit integer comparisons. ORDER IR. */
...
static void asm_stack_check( ASMState* as,
    ...
    Reg tmp,
        pbase = irp ? ( ra_hasreg( irp->r ) ? irp->r : RID_TMP ) : RID_BASE;
    ...
    for ( n = 0; n < nent; n++ ) {
        ...
        if ( irt_isnum( ir->t ) ) {
            ...
            if ( !irt_ispri( ir->t ) ) {
                Reg src = ra_alloc1( as, ref, allow );
                rset_clear( allow, src );
                emit_tsi( as, MIPSI_SW, src, RID_BASE,
                          ofs + ( LJ_BE ? 4 : 0 ) );
            }
...
static void asm_tail_fixup( ASMState* as, TraceNo lnk ) {
    MCode* target =
        lnk ? traceref( as->J, lnk )->mcode : ( MCode* )lj_vm_exit_interp;
    int32_t spadj = as->T->spadjust;
    MCode* p = as->mctop - 1;
    *p = spadj ? ( MIPSI_ADDIU | MIPSF_T( RID_SP ) | MIPSF_S( RID_SP ) | spadj )
               : MIPSI_NOP;
...
static Reg asm_setup_call_slots( ASMState* as,
    ...
    asm_collectargs( as, ir, ci, args );
    for ( i = 0; i < nargs; i++ ) {
        if ( args[ i ] && irt_isfp( IR( args[ i ] )->t ) && nfpr > 0 &&
             !( ci->flags & CCI_VARARG ) ) {
            nfpr--;
            ...
            if ( ( ( p[ -1 ] ^ ( px - p ) ) & 0xffffu ) ==
                ...
                patchbranch:
                    ...
                    *p = MIPSI_NOP; /* Replace the load of the exit number. */
                    ...
                    /* Ignore jump slot overflow. Child trace is simply not
                     * attached. */
                }
            } else if ( p + 1 == pe ) {
                /* Patch NOP after code for inverted loop branch. Use of J is
                 * ok. */
