...
static void LJ_FASTCALL recff_setmetatable( jit_State* J, RecordFFData* rd ) {
    ...
    TRef mt = J->base[ 1 ];
    if ( tref_istab( tr ) &&
         ( tref_istab( mt ) || ( mt && tref_isnil( mt ) ) ) ) {
        TRef fref, mtref;
        RecordIndex ix;
        ix.tab = tr;
        copyTV( J->L, &ix.tabv, &rd->argv[ 0 ] );
        lj_record_mm_lookup( J, &ix,
                             MM_metatable ); /* Guard for no __metatable. */
...
static void LJ_FASTCALL recff_rawequal( jit_State* J, RecordFFData* rd ) {
    ...
    if ( tra && trb ) {
        int diff =
            lj_record_objcmp( J, tra, trb, &rd->argv[ 0 ], &rd->argv[ 1 ] );
...
int32_t lj_ffrecord_select_mode( jit_State* J, TRef tr, TValue* tv ) {
    if ( tref_isstr( tr ) && *strVdata( tv ) == '#' ) { /* select('#', ...) */
        if ( strV( tv )->len == 1 ) {
            ...
            TRef trptr =
                emitir( IRT( IR_STRREF, IRT_P32 ), tr, lj_ir_kint( J, 0 ) );
            TRef trchar =
                emitir( IRT( IR_XLOAD, IRT_U8 ), trptr, IRXLOAD_READONLY );
    ...
    if ( tref_isnumber_str( tr ) ) {
        if ( tref_isstr( tr ) ) {
            ...
            if ( !lj_strscan_num( strV( &rd->argv[ 0 ] ), &tmp ) )
                recff_nyiu(
                    J ); /* Would need an inverted STRTO for this case. */
...
static TValue* recff_metacall_cp( lua_State* L,
                                  lua_CFunction dummy,
                                  void* ud ) {
...
static void LJ_FASTCALL recff_tostring( jit_State* J, RecordFFData* rd ) {
    ...
    if ( tref_isstr( tr ) ) {
        ...
        if ( tref_isnumber( tr ) ) {
            ...
            J->base[ 0 ] =
                lj_ir_kstr( J, strV( &J->fn->c.upvalue[ tref_type( tr ) ] ) );
        } else {
            recff_nyiu( J );
        }
    }
...
static void LJ_FASTCALL recff_ipairs_aux( jit_State* J, RecordFFData* rd ) {
    ...
    if ( tref_istab( ix.tab ) ) {
        if ( !tvisnumber(
                 &rd->argv[ 1 ] ) ) /* No support for string coercion. */
            lj_trace_err( J, LJ_TRERR_BADTYPE );
        ...
        J->base[ 0 ] = ix.key =
            emitir( IRTI( IR_ADD ), ix.key, lj_ir_kint( J, 1 ) );
...
static void LJ_FASTCALL recff_math_round( jit_State* J, RecordFFData* rd ) {
    ...
    if ( !tref_isinteger( tr ) ) { /* Pass through integers unmodified. */
        ...
        if ( LJ_DUALNUM ) { /* Try to narrow using a guarded conversion to int.
                             */
            lua_Number n =
                lj_vm_foldfpm( numberVnum( &rd->argv[ 0 ] ), rd->data );
            if ( n == ( lua_Number )lj_num2int( n ) )
                tr = emitir( IRTGI( IR_CONV ), tr,
...
static void LJ_FASTCALL recff_math_unary( jit_State* J, RecordFFData* rd ) {
    J->base[ 0 ] =
        emitir( IRTN( IR_FPMATH ), lj_ir_tonum( J, J->base[ 0 ] ), rd->data );
}
...
static void LJ_FASTCALL recff_math_random( jit_State* J, RecordFFData* rd ) {
    ...
    tr = lj_ir_call( J, IRCALL_lj_math_random_step,
                     lj_ir_kptr( J, uddata( ud ) ) );
...
static void LJ_FASTCALL recff_bit_unary( jit_State* J, RecordFFData* rd ) {
    ...
    J->base[ 0 ] =
        ( rd->data == IR_TOBIT ) ? tr : emitir( IRTI( rd->data ), tr, 0 );
}
...
static void LJ_FASTCALL recff_string_len( jit_State* J, RecordFFData* rd ) {
    J->base[ 0 ] = emitir( IRTI( IR_FLOAD ), lj_ir_tostr( J, J->base[ 0 ] ),
                           IRFL_STR_LEN );
    UNUSED( rd );
}
...
static void LJ_FASTCALL recff_string_range( jit_State* J, RecordFFData* rd ) {
    ...
    if ( rd->data ) { /* Return string.sub result. */
        if ( end - start >= 0 ) {
            ...
            J->base[ 0 ] =
                lj_ir_kstr( J, lj_str_new( J->L, strdata( str ), 0 ) );
        }
        ...
        if ( len > 0 ) {
            ...
            rd->nres = len;
            for ( i = 0; i < len; i++ ) {
                TRef tmp = emitir( IRTI( IR_ADD ), trstart,
                                   lj_ir_kint( J, ( int32_t )i ) );
                tmp = emitir( IRT( IR_STRREF, IRT_P32 ), trstr, tmp );
                J->base[ i ] =
                    emitir( IRT( IR_XLOAD, IRT_U8 ), tmp, IRXLOAD_READONLY );
            }
        ...
        if ( tref_isnil( J->base[ 1 ] ) ) { /* Simple pop: t[#t] = nil */
            ...
            if ( len ) {
                ...
                if ( results_wanted( J ) !=
                    ...
                    ix.val = 0;
                    J->base[ 0 ] =
                        lj_record_idx( J, &ix ); /* Load previous value. */
                    rd->nres = 1;
                    /* Assumes ix.key/ix.tab is not modified for raw
                     * lj_record_idx(). */
                }
                ix.val = TREF_NIL;
