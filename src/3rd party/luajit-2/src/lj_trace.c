...
static void perftools_addtrace( GCtrace* T ) {
    ...
    fprintf( fp, "%lx %x TRACE_%d::%s:%u\n", ( long )T->mcode, T->szmcode,
             T->traceno, name, lineno );
}
#endif
...
static void trace_save( jit_State* J ) {
    ...
    size_t sz = sztr + szins + J->cur.nsnap * sizeof( SnapShot ) +
    ...
    if ( ( pt->flags & PROTO_ILOOP ) ) {
        ...
        for ( i = 1; i < sizebc; i++ ) {
            ...
            if ( op == BC_IFORL || op == BC_IITERL || op == BC_ILOOP )
                setbc_op( &bc[ i ],
                          ( int )op + ( int )BC_LOOP - ( int )BC_ILOOP );
        }
    }
}
...
void lj_trace_freestate( global_State* g ) {
    ...
    { /* This assumes all traces have already been freed. */
        ...
        for ( i = 1; i < ( ptrdiff_t )J->sizetrace; i++ )
            lua_assert( i == ( ptrdiff_t )J->cur.traceno ||
                        traceref( J, i ) == NULL );
    }
#endif
    lj_mcode_free( J );
    ...
    lj_mem_freevec( g, J->irbuf + J->irbotlim, J->irtoplim - J->irbotlim,
                    IRIns );
...
static void penalty_pc( jit_State* J, GCproto* pt, BCIns* pc, TraceError e ) {
    ...
    for ( i = 0; i < PENALTY_SLOTS; i++ )
        if ( mref( J->penalty[ i ].pc, const BCIns ) ==
             pc ) { /* Cache slot found? */
...
static void trace_start( jit_State* J ) {
    ...
    if ( ( J->pt->flags & PROTO_NOJIT ) ) { /* JIT disabled for this proto? */
        if ( J->parent == 0 ) {
            ...
            lua_assert(
                bc_op( *J->pc ) == BC_FORL || bc_op( *J->pc ) == BC_ITERL ||
                bc_op( *J->pc ) == BC_LOOP || bc_op( *J->pc ) == BC_FUNCF );
            setbc_op( J->pc, ( int )bc_op( *J->pc ) + ( int )BC_ILOOP -
                                 ( int )BC_LOOP );
    ...
    L = J->L;
    lj_vmevent_send(
        L, TRACE, setstrV( L, L->top++, lj_str_newlit( L, "start" ) );
        setintV( L->top++, traceno ); setfuncV( L, L->top++, J->fn );
        setintV( L->top++, proto_bcpos( J->pt, J->pc ) ); if ( J->parent ) {
...
static void trace_stop( jit_State* J ) {
    ...
    switch ( op ) {
        case BC_FORL:
            setbc_op( pc + bc_j( J->cur.startins ),
                      BC_JFORI ); /* Patch FORI, too. */
                                  /* fallthrough */
        case BC_LOOP:
        case BC_ITERL:
        case BC_FUNCF:
        ...
        case BC_JMP:
            ...
            lj_asm_patchexit( J, traceref( J, J->parent ), J->exitno,
                              J->cur.mcode );
            /* Avoid compiling a side trace twice (stack resizing uses parent
             * exit). */
...
static int trace_abort( jit_State* J ) {
    ...
    if ( J->parent == 0 && !bc_isret( bc_op( J->cur.startins ) ) )
        penalty_pc( J, &gcref( J->cur.startpt )->pt,
                    mref( J->cur.startpc, BCIns ), e );

    /* Is there anything to abort? */
    traceno = J->cur.traceno;
    if ( traceno ) {
        ptrdiff_t errobj =
            savestack( L, L->top - 1 ); /* Stack may be resized. */
        J->cur.link = 0;
        J->cur.linktype = LJ_TRLINK_NONE;
        lj_vmevent_send(
            L, TRACE, TValue * frame; const BCIns* pc; GCfunc * fn;
            setstrV( L, L->top++, lj_str_newlit( L, "abort" ) );
            setintV( L->top++, traceno );
            /* Find original Lua function call to generate a better error
               message. */
            frame = J->L->base - 1;
...
static TValue* trace_state( lua_State* L, lua_CFunction dummy, void* ud ) {
    ...
    retry:
        switch ( J->state ) {
            case LJ_TRACE_START:
                J->state =
                    LJ_TRACE_RECORD; /* trace_start() may change state. */
                trace_start( J );
            ...
            case LJ_TRACE_RECORD:
                ...
                lj_vmevent_send_(
                    ...
                    setfuncV( L, L->top++, J->fn ); setintV(
                        L->top++,
                        J->pt ? ( int32_t )proto_bcpos( J->pt, J->pc ) : -1 );
                    setintV( L->top++, J->framedepth );
                    , J2G( J )->tmptv = savetv; J2G( J )->tmptv2 = savetv2; );
                lj_record_ins( J );
                break;
            ...
            case LJ_TRACE_END:
                ...
                if ( ( J->flags & JIT_F_OPT_LOOP ) &&
                    ...
                    if ( lj_opt_loop( J ) ) { /* Loop optimization failed? */
                        ...
                        J->state =
                            LJ_TRACE_RECORD; /* Try to continue recording. */
                        break;
                    }
...
static TraceNo trace_exit_find( jit_State* J, MCode* pc ) {
    ...
    for ( traceno = 1; traceno < J->sizetrace; traceno++ ) {
        GCtrace* T = traceref( J, traceno );
        if ( T && pc >= T->mcode &&
             pc < ( MCode* )( ( char* )T->mcode + T->szmcode ) )
            return traceno;
    }
    lua_assert( 0 );
    return 0;
...
int LJ_FASTCALL lj_trace_exit( jit_State* J, void* exptr ) {
...
#ifdef EXITSTATE_PCREG
    J->parent =
        trace_exit_find( J, ( MCode* )( intptr_t )ex->gpr[ EXITSTATE_PCREG ] );
#endif
    ...
    lj_vmevent_send(
        ...
        setintV( L->top++, J->parent ); setintV( L->top++, J->exitno );
    ...
    switch ( bc_op( *pc ) ) {
        case BC_CALLM:
        case BC_CALLMT:
            return ( int )( ( BCReg )( L->top - L->base ) - bc_a( *pc ) -
                            bc_c( *pc ) );
        case BC_RETM:
