...
#define IRFPMDEF( _ )                                                       \
    ...
    _( TRUNC ) /* Must be first and in this order. */                       \
        _( SQRT ) _( EXP ) _( EXP2 ) _( LOG ) _( LOG2 ) _( LOG10 ) _( SIN ) \
            _( COS ) _( TAN ) _( OTHER )
...
#define IRMODE( name, m, m1, m2 ) \
    ( ( ( IRM##m1 ) | ( ( IRM##m2 ) << 2 ) | ( IRM_##m ) ) ^ IRM_W ),
...
#define IRTDEF( _ )                                                         \
    _( NIL, 4 )                                                             \
    _( FALSE, 4 )                                                           \
    _( TRUE, 4 ) _( LIGHTUD, LJ_64 ? 8 : 4 ) _( STR, 4 ) _( P32, 4 )        \
        _( THREAD, 4 ) _( PROTO, 4 ) _( FUNC, 4 ) _( P64, 8 ) _( CDATA, 4 ) \
            _( TAB, 4 ) _( UDATA, 4 ) _( FLOAT, 4 ) _( NUM, 8 ) _( I8, 1 )  \
                _( U8, 1 ) _( I16, 2 ) _( U16, 2 ) _( INT, 4 ) _( U32, 4 )  \
                    _( I64, 8 ) _( U64, 8 )                                 \
...
#define irt_sametype( t1, t2 ) \
    ( ( ( ( t1 ).irt ^ ( t2 ).irt ) & IRT_TYPE ) == 0 )
...
#define tref_typerange( tr, first, last )                    \
    ( ( ( ( tr ) >> 24 ) & IRT_TYPE ) - ( TRef )( first ) <= \
      ( TRef )( last - first ) )

#define tref_istype( tr, t ) \
    ( ( ( tr ) & ( IRT_TYPE << 24 ) ) == ( ( t ) << 24 ) )
...
#define tref_isnumber_str( tr ) \
    ( tref_isnumber( ( tr ) ) || tref_isstr( ( tr ) ) )
...
#define ir_knum( ir ) \
    check_exp( ( ir )->o == IR_KNUM, mref( ( ir )->ptr, cTValue ) )
#define ir_kint64( ir ) \
    check_exp( ( ir )->o == IR_KINT64, mref( ( ir )->ptr, cTValue ) )
#define ir_k64( ir )                                           \
