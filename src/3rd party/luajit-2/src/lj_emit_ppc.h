...
#define emit_asb( as, pi, ra, rs, rb ) \
    emit_tab( as, ( pi ), ( rs ), ( ra ), ( rb ) )
...
#define emit_asi( as, pi, ra, rs, i ) \
    emit_tai( as, ( pi ), ( rs ), ( ra ), ( i ) )

#define emit_fab( as, pi, rf, ra, rb ) \
    emit_tab( as, ( pi ), ( rf ) & 31, ( ra ) & 31, ( rb ) & 31 )
#define emit_fb( as, pi, rf, rb ) \
    emit_tab( as, ( pi ), ( rf ) & 31, 0, ( rb ) & 31 )
#define emit_fac( as, pi, rf, ra, rc ) \
    emit_tab( as, ( pi ) | PPCF_C( ( rc ) & 31 ), ( rf ) & 31, ( ra ) & 31, 0 )
#define emit_facb( as, pi, rf, ra, rc, rb )                                 \
    emit_tab( as, ( pi ) | PPCF_C( ( rc ) & 31 ), ( rf ) & 31, ( ra ) & 31, \
              ( rb ) & 31 )
#define emit_fai( as, pi, rf, ra, i ) \
    emit_tai( as, ( pi ), ( rf ) & 31, ( ra ), ( i ) )

static void emit_rot( ASMState* as,
                      PPCIns pi,
                      Reg ra,
                      Reg rs,
                      int32_t n,
                      int32_t b,
                      int32_t e ) {
    *--as->mcp = pi | PPCF_T( rs ) | PPCF_A( ra ) | PPCF_B( n ) | PPCF_MB( b ) |
                 PPCF_ME( e );
}
...
static void emit_lsptr( ASMState* as,
                        PPCIns pi,
                        Reg r,
                        void* p,
                        RegSet allow ) {
...
#define emit_getgl( as, r, field )     \
    emit_lsglptr( as, PPCI_LWZ, ( r ), \
                  ( int32_t )offsetof( global_State, field ) )
#define emit_setgl( as, r, field )     \
    emit_lsglptr( as, PPCI_STW, ( r ), \
                  ( int32_t )offsetof( global_State, field ) )
...
static void emit_condbranch( ASMState* as,
                             PPCIns pi,
                             PPCCC cc,
                             MCode* target ) {
    MCode* p = --as->mcp;
...
static void emit_spload( ASMState* as, IRIns* ir, Reg r, int32_t ofs ) {
    if ( r < RID_MAX_GPR )
        ...
        emit_fai( as, irt_isnum( ir->t ) ? PPCI_LFD : PPCI_LFS, r, RID_SP,
                  ofs );
}
