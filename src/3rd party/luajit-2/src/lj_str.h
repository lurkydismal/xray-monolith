...
LJ_FUNCA GCstr* LJ_FASTCALL lj_str_fromnum( lua_State* L,
                                            const lua_Number* np );
...
/* String formatting. */
LJ_FUNC const char* lj_str_pushvf( lua_State* L,
                                   const char* fmt,
                                   va_list argp );
...
#define lj_str_resizebuf( L, sb, size )                                    \
    ( ( sb )->buf =                                                        \
          ( char* )lj_mem_realloc( L, ( sb )->buf, ( sb )->sz, ( size ) ), \
      ( sb )->sz = ( size ) )
#define lj_str_freebuf( g, sb ) \
    lj_mem_free( g, ( void* )( sb )->buf, ( sb )->sz )
