...
static MCode* asm_exitstub_gen( ASMState* as, ExitNo group ) {
    ...
    *mxp++ = XI_JMP;
    mxp += 4;
    *( ( int32_t* )( mxp - 4 ) ) =
        jmprel( mxp, ( MCode* )( void* )lj_vm_exit_handler );
...
static void asm_exitstub_setup( ASMState* as, ExitNo nexits ) {
    ...
    for ( i = 0; i < ( nexits + EXITSTUBS_PER_GROUP - 1 ) / EXITSTUBS_PER_GROUP;
          i++ )
...
static IRRef asm_fuseabase( ASMState* as, IRRef ref ) {
    ...
    if ( irb->o == IR_FLOAD ) {
        ...
        if ( ira->o == IR_TNEW && ira->op1 <= LJ_MAX_COLOSIZE &&
            ...
            as->mrm.ofs =
                ( int32_t )sizeof( GCtab ); /* Ofs to colocated array. */
...
static void asm_fusearef( ASMState* as, IRIns* ir, RegSet allow ) {
    ...
    as->mrm.base =
        ( uint8_t )ra_alloc1( as, asm_fuseabase( as, ir->op1 ), allow );
    irx = IR( ir->op2 );
    if ( irref_isk( ir->op2 ) ) {
        as->mrm.ofs += 8 * irx->i;
        as->mrm.idx = RID_NONE;
        ...
        */
        if ( !LJ_64 && /* Has bad effects with negative index on x64. */
             mayfuse( as, ir->op2 ) && ra_noreg( irx->r ) && irx->o == IR_ADD &&
             irref_isk( irx->op2 ) ) {
        ...
        switch ( ( IROp )ir->o ) {
            ...
            case IR_HREFK:
                if ( mayfuse( as, ref ) ) {
                    ...
                    as->mrm.ofs =
                        ( int32_t )( IR( ir->op2 )->op2 * sizeof( Node ) );
                    as->mrm.idx = RID_NONE;
                    return;
                }
                break;
            case IR_UREFC:
                ...
                break;
            default:
                lua_assert( ir->o == IR_HREF || ir->o == IR_NEWREF ||
                            ir->o == IR_UREFO || ir->o == IR_KKPTR );
                break;
        }
    }
...
static void asm_fusestrref( ASMState* as, IRIns* ir, RegSet allow ) {
    ...
    if ( irref_isk( ir->op2 ) ) {
        ...
        if ( !LJ_64 && /* Has bad effects with negative index on x64. */
             mayfuse( as, ir->op2 ) && irr->o == IR_ADD &&
             irref_isk( irr->op2 ) ) {
...
static void asm_fusexref( ASMState* as, IRRef ref, RegSet allow ) {
    ...
    if ( ir->o == IR_KPTR || ir->o == IR_KKPTR ) {
        ...
        if ( canfuse( as, ir ) && ir->o == IR_ADD && ra_noreg( ir->r ) ) {
            ...
            IRRef idx;
            Reg r;
            if ( asm_isk32( as, ir->op2,
                            &as->mrm.ofs ) ) { /* Recognize x+ofs. */
                ref = ir->op1;
                ir = IR( ref );
                if ( !( ir->o == IR_ADD && canfuse( as, ir ) &&
                        ra_noreg( ir->r ) ) )
                    goto noadd;
            }
            as->mrm.scale = XM_SCALE1;
            ...
            irx = IR( idx );
            if ( !( irx->o == IR_BSHL ||
                    irx->o == IR_ADD ) ) { /* Try other operand. */
                idx = ir->op2;
                ref = ir->op1;
                irx = IR( idx );
            }
            if ( canfuse( as, irx ) && ra_noreg( irx->r ) ) {
                if ( irx->o == IR_BSHL && irref_isk( irx->op2 ) &&
                     IR( irx->op2 )->i <= 3 ) {
                    /* Recognize idx<<b with b = 0-3, corresponding to sz =
                     * (1),2,4,8. */
                    idx = irx->op1;
...
static Reg asm_fuseload( ASMState* as, IRRef ref, RegSet allow ) {
    ...
    if ( ir->o == IR_KNUM ) {
        ...
        if ( !( avail &
                ( avail - 1 ) ) ) { /* Fuse if less than two regs available. */
        ...
        } else if ( ir->o == IR_ALOAD || ir->o == IR_HLOAD ||
                    ir->o == IR_ULOAD ) {
    ...
    if ( !( as->freeset & allow ) && !irref_isk( ref ) &&
         ( allow == RSET_EMPTY || ra_hasspill( ir->s ) ||
           iscrossref( as, ref ) ) )
        goto fusespill;
...
#else
#define asm_fuseloadm( as, ref, allow, is64 ) \
    asm_fuseload( as, ( ref ), ( allow ) )
#endif

/* -- Calls --------------------------------------------------------------- */

/* Count the required number of stack slots for a call. */
static int asm_count_call_slots( ASMState* as,
                                 const CCallInfo* ci,
                                 IRRef* args ) {
    ...
    if ( LJ_ABI_WIN ) {
        nslots =
            ( int )( nargs * 2 ); /* Only matters for more than four args. */
    } else {
...
static void asm_gencall( ASMState* as, const CCallInfo* ci, IRRef* args ) {
    ...
    if ( ( ci->flags & CCI_VARARG ) ) { /* Special handling for vararg calls. */
        ...
        for ( n = 0; n < 4 && n < nargs; n++ ) {
            ...
            if ( irt_isfp( ir->t ) ) /* Duplicate FPRs in GPRs. */
                emit_rr(
                    as, XO_MOVDto,
                    ( irt_isnum( ir->t ) ? REX_64 : 0 ) | ( fpr + n ),
                    ( ( gprs >> ( n * 5 ) ) & 31 ) ); /* Either MOVD or MOVQ. */
        }
#else
        patchnfpr =
            --as->mcp; /* Indicate number of used FPRs in register al. */
    ...
    for ( n = 0; n < nargs; n++ ) { /* Setup args. */
        ...
        r = irt_isfp( ir->t ) ? ( fpr <= REGARG_LASTFPR ? fpr : 0 )
                              : ( gprs & 31 );
        fpr++;
        gprs >>= 5;
#elif LJ_64
        ...
        if ( r ) { /* Argument is in a register. */
            if ( r < RID_MAX_GPR && ref < ASMREF_TMP1 ) {
                ...
                lua_assert(
                    rset_test( as->freeset, r ) ); /* Must have been evicted. */
                if ( ra_hasreg( ir->r ) ) {
                    ra_noweak( as, ir->r );
            ...
            lua_assert( !( irt_isfloat( ir->t ) &&
                           irref_isk( ref ) ) ); /* No float k. */
            if ( LJ_32 && ( ofs & 4 ) && irref_isk( ref ) ) {
                /* Split stores for unaligned FP consts. */
                emit_movmroi( as, RID_ESP, ofs,
                              ( int32_t )ir_knum( ir )->u32.lo );
                emit_movmroi( as, RID_ESP, ofs + 4,
                              ( int32_t )ir_knum( ir )->u32.hi );
            } else {
                r = ra_alloc1( as, ref, RSET_FPR );
                emit_rmro( as, irt_isnum( ir->t ) ? XO_MOVSDto : XO_MOVSSto, r,
                           RID_ESP, ofs );
            }
...
static void asm_setupresult( ASMState* as, IRIns* ir, const CCallInfo* ci ) {
    ...
    if ( ra_used( ir ) ) {
        if ( irt_isfp( ir->t ) ) {
            int32_t ofs =
                sps_scale( ir->s ); /* Use spill slot or temp slots. */
#if LJ_64
            if ( ( ci->flags & CCI_CASTU64 ) ) {
                ...
                if ( ra_hasreg( dest ) ) {
                    ra_free( as, dest );
                    ra_modified( as, dest );
                    emit_rr( as, XO_MOVD, dest | REX_64,
                             RID_RET ); /* Really MOVQ. */
                }
                if ( ofs )
            ...
            if ( ( ci->flags & CCI_CASTU64 ) ) {
                ...
                emit_rmro( as, irt_isnum( ir->t ) ? XO_FSTPq : XO_FSTPd,
                           irt_isnum( ir->t ) ? XOg_FSTPq : XOg_FSTPd, RID_ESP,
                           ofs );
            }
#endif
#if LJ_32
...
static void asm_retf( ASMState* as, IRIns* ir ) {
    ...
    if ( ( int32_t )as->topslot < 0 )
        as->topslot = 0;
    irt_setmark(
        IR( REF_BASE )->t ); /* Children must not coalesce with BASE reg. */
...
static void asm_tobit( ASMState* as, IRIns* ir ) {
    Reg dest = ra_dest( as, ir, RSET_GPR );
    Reg tmp = ra_noreg( IR( ir->op1 )->r ) ? ra_alloc1( as, ir->op1, RSET_FPR )
...
static void asm_conv( ASMState* as, IRIns* ir ) {
    ...
    lua_assert( !( LJ_32 &&
                   ( irt_isint64( ir->t ) || st64 ) ) ); /* Handled by SPLIT. */
    if ( irt_isfp( ir->t ) ) {
        Reg dest = ra_dest( as, ir, RSET_FPR );
        if ( stfp ) { /* FP to FP conversion. */
            Reg left = asm_fuseload( as, lref, RSET_FPR );
            emit_mrm( as, st == IRT_NUM ? XO_CVTSD2SS : XO_CVTSS2SD, dest,
                      left );
            if ( left == dest )
                return; /* Avoid the XO_XORPS. */
        } else if ( LJ_32 &&
                    st == IRT_U32 ) { /* U32 to FP conversion on x86. */
            ...
            if ( LJ_64 && st == IRT_U64 ) {
                ...
                emit_rr( as, XO_TEST, left | REX_64,
                         left ); /* Check if u64 >= 2^63. */
            }
            emit_mrm(
                as, irt_isnum( ir->t ) ? XO_CVTSI2SD : XO_CVTSI2SS,
                dest | ( ( LJ_64 && ( st64 || st == IRT_U32 ) ) ? REX_64 : 0 ),
                left );
        }
        if ( !( as->flags & JIT_F_SPLIT_XMM ) )
            emit_rr( as, XO_XORPS, dest,
                     dest ); /* Avoid partial register stall. */
        ...
        if ( irt_isguard( ir->t ) ) {
            ...
            Reg dest = ra_dest( as, ir, RSET_GPR );
            x86Op op = st == IRT_NUM
                           ? ( ( ir->op2 & IRCONV_TRUNC ) ? XO_CVTTSD2SI
                                                          : XO_CVTSD2SI )
                           : ( ( ir->op2 & IRCONV_TRUNC ) ? XO_CVTTSS2SI
                                                          : XO_CVTSS2SI );
            if ( LJ_64 ? irt_isu64( ir->t ) : irt_isu32( ir->t ) ) {
                /* LJ_64: For inputs >= 2^63 add -2^64, convert again. */
                /* LJ_32: For inputs >= 2^31 add -2^31, convert again and add
                 * 2^31. */
                Reg tmp = ra_noreg( IR( lref )->r )
                              ? ra_alloc1( as, lref, RSET_FPR )
                              : ra_scratch( as, RSET_FPR );
                MCLabel l_end = emit_label( as );
                if ( LJ_32 )
                    emit_gri( as, XG_ARITHi( XOg_ADD ), dest,
                              ( int32_t )0x80000000 );
                emit_rr( as, op, dest | REX_64, tmp );
                if ( st == IRT_NUM )
                    emit_rma( as, XO_ADDSD, tmp,
                              lj_ir_k64_find(
                                  as->J, LJ_64 ? U64x( c3f00000, 00000000 )
                                               : U64x( c1e00000, 00000000 ) ) );
                else
                    emit_rma( as, XO_ADDSS, tmp,
                              lj_ir_k64_find(
                                  as->J, LJ_64 ? U64x( 00000000, df800000 )
                                               : U64x( 00000000, cf000000 ) ) );
                emit_sjcc( as, CC_NS, l_end );
                emit_rr( as, XO_TEST, dest | REX_64,
                         dest ); /* Check if dest negative. */
                emit_rr( as, op, dest | REX_64, tmp );
                ra_left( as, tmp, lref );
            } else {
                Reg left = asm_fuseload( as, lref, RSET_FPR );
                ...
                st <= IRT_U16 ) { /* Extend to 32 bit integer. */
        ...
        if ( LJ_32 ) { /* Only need to handle 32/32 bit no-op (cast) on x86. */
            Reg dest = ra_dest( as, ir, RSET_GPR );
            ra_left( as, dest,
                     lref ); /* Do nothing, but may need to move regs. */
...
static void asm_conv_fp_int64( ASMState* as, IRIns* ir ) {
    ...
    if ( ra_hasreg( dest ) ) {
        ra_free( as, dest );
        ra_modified( as, dest );
        emit_rmro( as, irt_isnum( ir->t ) ? XMM_MOVRM( as ) : XO_MOVSS, dest,
                   RID_ESP, ofs );
    }
    emit_rmro( as, irt_isnum( ir->t ) ? XO_FSTPq : XO_FSTPd,
...
static void asm_strto( ASMState* as, IRIns* ir ) {
    ...
    emit_rmro( as, XO_LEA, ra_releasetmp( as, ASMREF_TMP1 ) | REX_64, RID_ESP,
               sps_scale( ir->s ) );
}
...
static MCode* merge_href_niltv( ASMState* as, IRIns* ir ) {
    ...
    if ( ( ir[ 1 ].o == IR_NE || ir[ 1 ].o == IR_EQ ) &&
         ir[ 1 ].op1 == as->curins && ra_hasreg( ir->r ) ) {
        MCode* p = as->mcp;
        p += ( LJ_64 && *p != XI_ARITHi ) ? 7 + 6 : 6 + 6;
        /* Ensure no loop branch inversion happened. */
        if ( p[ -6 ] == 0x0f &&
             p[ -5 ] == XI_JCCn + ( CC_NE ^ ( ir[ 1 ].o & 1 ) ) ) {
...
static void asm_href( ASMState* as, IRIns* ir ) {
    MCode* nilexit =
        merge_href_niltv( as, ir ); /* Do this before any restores. */
    RegSet allow = RSET_GPR;
    ...
    if ( nilexit && ir[ 1 ].o == IR_NE ) {
        emit_jcc( as, CC_E,
                  nilexit ); /* XI_JMP is not found by lj_asm_patchexit. */
        nilexit = NULL;
    } else {
    ...
    if ( irt_isnum( kt ) ) {
        if ( isk ) {
            ...
            emit_gmroi( as, XG_ARITHi( XOg_CMP ), dest,
                        offsetof( Node, key.u32.lo ),
                        ( int32_t )ir_knum( irkey )->u32.lo );
            emit_sjcc( as, CC_NE, l_next );
            emit_gmroi( as, XG_ARITHi( XOg_CMP ), dest,
                        offsetof( Node, key.u32.hi ),
            ...
            emit_rmro( as, XO_ARITHi8, XOg_CMP, dest,
                       offsetof( Node, key.it ) );
#endif
        }
#if LJ_64
    ...
    if ( khash == 0 ) {
        ...
        emit_rmro( as, XO_ARITH( XOg_ADD ), dest, tab,
                   offsetof( GCtab, node ) );
        ...
        if ( isk ) {
            ...
            emit_rmro( as, XO_ARITH( XOg_AND ), dest, key,
                       offsetof( GCstr, hash ) );
            emit_rmro( as, XO_MOV, dest, tab, offsetof( GCtab, hmask ) );
        } else { /* Must match with hashrot() in lj_tab.c. */
            emit_rmro( as, XO_ARITH( XOg_AND ), dest, tab,
...
#else
                emit_rmro( as, XO_MOV, dest, RID_ESP,
                           ra_spill( as, irkey ) + 4 );
...
static void asm_newref( ASMState* as, IRIns* ir ) {
    ...
    if ( irt_isnum( irkey->t ) ) {
        ...
        if ( irref_isk( ir->op2 ) )
            ...
            emit_rmro( as, XO_LEA, tmp | REX_64, RID_ESP,
                       ra_spill( as, irkey ) );
    } else {
...
static void asm_uref( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ir->op1 ) ) {
        ...
        if ( ir->o == IR_UREFC ) {
            ...
            asm_guardcc( as, CC_NE );
            emit_i8( as, 1 );
            emit_rmro( as, XO_ARITHib, XOg_CMP, uv,
                       offsetof( GCupval, closed ) );
        } else {
    ...
    */
    if ( irt_isi16( ir->t ) || irt_isu16( ir->t ) || irt_isfp( ir->t ) ||
        ...
        RegSet allow8 = irt_isfp( ir->t )                            ? RSET_FPR
                        : ( irt_isi8( ir->t ) || irt_isu8( ir->t ) ) ? RSET_GPR8
                                                                     : RSET_GPR;
        src = osrc = ra_alloc1( as, ir->op2, allow8 );
        if ( !LJ_64 &&
             !rset_test( allow8, src ) ) { /* Already in wrong register. */
...
static void asm_ahuvload( ASMState* as, IRIns* ir ) {
    lua_assert( irt_isnum( ir->t ) || irt_ispri( ir->t ) ||
                irt_isaddr( ir->t ) || ( LJ_DUALNUM && irt_isint( ir->t ) ) );
#if LJ_64
    if ( irt_islightud( ir->t ) ) {
        ...
        Reg dest = ra_dest( as, ir, allow );
        asm_fuseahuref( as, ir->op1, RSET_GPR );
        emit_mrm( as, dest < RID_MAX_GPR ? XO_MOV : XMM_MOVRM( as ), dest,
                  RID_MRM );
    } else {
        asm_fuseahuref( as, ir->op1, RSET_GPR );
    }
...
static void asm_sload( ASMState* as, IRIns* ir ) {
    ...
    Reg base;
    lua_assert(
        !( ir->op2 & IRSLOAD_PARENT ) ); /* Handled by asm_head_side(). */
    lua_assert( irt_isguard( t ) || !( ir->op2 & IRSLOAD_TYPECHECK ) );
    lua_assert( LJ_DUALNUM || !irt_isint( t ) ||
                ( ir->op2 & ( IRSLOAD_CONVERT | IRSLOAD_FRAME ) ) );
    if ( ( ir->op2 & IRSLOAD_CONVERT ) && irt_isguard( t ) && irt_isint( t ) ) {
        Reg left = ra_scratch( as, RSET_FPR );
        asm_tointg( as, ir,
                    left ); /* Frees dest reg. Do this before base alloc. */
        ...
        Reg dest =
            asm_load_lightud64( as, ir, ( ir->op2 & IRSLOAD_TYPECHECK ) );
        if ( ra_hasreg( dest ) ) {
            base = ra_alloc1( as, REF_BASE, RSET_GPR );
        ...
        if ( ( ir->op2 & IRSLOAD_CONVERT ) ) {
            t.irt = irt_isint( t ) ? IRT_NUM
                                   : IRT_INT; /* Check for original type. */
            emit_rmro( as, irt_isint( t ) ? XO_CVTSI2SD : XO_CVTSD2SI, dest,
                       base, ofs );
        } else if ( irt_isnum( t ) ) {
...
static void asm_cnew( ASMState* as, IRIns* ir ) {
    ...
    asm_gencall( as, ci, args );
    emit_loadi( as, ra_releasetmp( as, ASMREF_TMP1 ),
...
static void asm_obar( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ir->op2 ) ) {
        ...
        Reg val = ra_alloc1( as, ir->op2,
                             rset_exclude( RSET_SCRATCH & RSET_GPR, obj ) );
        emit_rmro( as, XO_GROUP3b, XOg_TEST, val,
                   ( int32_t )offsetof( GChead, marked ) );
    }
    emit_sjcc( as, CC_Z, l_end );
    emit_i8( as, LJ_GC_BLACK );
    emit_rmro( as, XO_GROUP3b, XOg_TEST, obj,
               ( int32_t )offsetof( GCupval, marked ) -
...
static int fpmjoin_pow( ASMState* as, IRIns* ir ) {
    ...
    if ( irp == ir - 1 && irp->o == IR_MUL && !ra_used( irp ) ) {
        ...
        if ( irpp == ir - 2 && irpp->o == IR_FPMATH &&
            ...
            RegSet drop =
                RSET_RANGE( RID_XMM0, RID_XMM2 + 1 ) | RID2RSET( RID_EAX );
            IRIns* irx;
            if ( ra_hasreg( ir->r ) )
                rset_clear( drop, ir->r ); /* Dest reg handled below. */
            ra_evictset( as, drop );
...
static void asm_fpmath( ASMState* as, IRIns* ir ) {
    ...
    if ( fpm == IRFPM_SQRT ) {
        ...
        if ( as->flags &
             JIT_F_SSE4_1 ) { /* SSE4.1 has a rounding instruction. */
            Reg dest = ra_dest( as, ir, RSET_FPR );
            Reg left = asm_fuseload( as, ir->op1, RSET_FPR );
            ...
            *--as->mcp = 0x66; /* 1st byte of ROUNDSD opcode. */
            ...
            /* The modified regs must match with the *.dasc implementation. */
            RegSet drop =
                RSET_RANGE( RID_XMM0, RID_XMM3 + 1 ) | RID2RSET( RID_EAX );
            if ( ra_hasreg( ir->r ) )
                rset_clear( drop, ir->r ); /* Dest reg handled below. */
            ra_evictset( as, drop );
            ra_destreg( as, ir, RID_XMM0 );
            emit_call( as, fpm == IRFPM_FLOOR  ? lj_vm_floor_sse
                           : fpm == IRFPM_CEIL ? lj_vm_ceil_sse
                                               : lj_vm_trunc_sse );
...
static void asm_intarith( ASMState* as, IRIns* ir, x86Arith xa ) {
    ...
    if ( lref == rref ) {
        ...
        right = asm_fuseloadm( as, rref, rset_clear( allow, dest ),
                               irt_is64( ir->t ) );
    }
...
static int asm_lea( ASMState* as, IRIns* ir ) {
    ...
    } else if ( ir->op1 != ir->op2 && irl->o == IR_ADD &&
                mayfuse( as, ir->op1 ) &&
...
static void asm_bitswap( ASMState* as, IRIns* ir ) {
    Reg dest = ra_dest( as, ir, RSET_GPR );
    as->mcp = emit_op( XO_BSWAP + ( ( dest & 7 ) << 24 ), REX_64IR( ir, 0 ),
                       dest, 0, as->mcp, 1 );
    ra_left( as, dest, ir->op1 );
}
    ...
    if ( irt_isnum( ir->t ) ) {
        ...
        */
        ...
        if ( cc & VCC_P ) { /* Extra CC_P branch required? */
            if ( !( cc & VCC_U ) ) {
                asm_guardcc(
                    as, CC_P ); /* Branch to exit for ordered comparisons. */
            } else if ( l_around != as->invmcp ) {
                emit_sjcc( as, CC_P,
                           l_around ); /* Branch around for unordered. */
            } else {
        ...
        if ( irref_isk( lref ) ||
             ( !irref_isk( rref ) && opisfusableload( leftop ) ) ) {
            if ( ( cc & 0xc ) == 0xc )
                cc ^= 0x53; /* L <-> G, LE <-> GE */
            else if ( ( cc & 0xa ) == 0x2 )
                cc ^= 0x55; /* A <-> B, AE <-> BE */
        ...
        if ( asm_isk32( as, rref, &imm ) ) {
            ...
            if ( usetest && irl->o == IR_BAND && irl + 1 == ir &&
                 !ra_used( irl ) ) {
                ...
                if ( !asm_isk32( as, irl->op2, &imm ) ) {
                    ...
                    if ( opisfusableload( ( IROp )irll->o ) &&
                        ...
                        as->curins--; /* Skip to BAND to avoid failing in
                                         noconflict(). */
            ...
            test_nofuse:
                ...
                if ( opisfusableload( ( IROp )irl->o ) &&
                     ( ( irt_isu8( irl->t ) && checku8( imm ) ) ||
                       ( ( irt_isi8( irl->t ) || irt_isi16( irl->t ) ) &&
                         checki8( imm ) ) ||
                       ( irt_isu16( irl->t ) && checku16( imm ) &&
                         checki8( ( int16_t )imm ) ) ) ) {
                    ...
                    */
                    ...
                    left = asm_fuseloadm(
                        as, lref, irt_isu8( ir->t ) ? RSET_GPR8 : RSET_GPR,
                        r64 );
                }
                asm_guardcc( as, cc );
                if ( usetest && left != RID_MRM ) {
                    ...
                    if ( irt_isu8( ir->t ) ) {
                        ...
                        xo = XO_TESTb;
                        if ( !rset_test( RSET_RANGE( RID_EAX, RID_EBX + 1 ),
                                         left ) ) {
                            if ( LJ_64 ) {
                                left |= FORCE_REX;
                            } else {
                    ...
                    if ( irl + 1 == ir )       /* Referencing previous ins? */
                        as->flagmcp = as->mcp; /* Set flag to drop test r,r if
                                                  possible. */
                } else {
            ...
            Reg right =
                asm_fuseloadm( as, rref, rset_exclude( RSET_GPR, left ), r64 );
            asm_guardcc( as, cc );
...
static void asm_comp_int64( ASMState* as, IRIns* ir ) {
    ...
    if ( ( cc & 15 ) != CC_NE )
        emit_sjcc( as, CC_NE,
                   l_around ); /* Hiword unequal: skip loword compare. */
    if ( ( cc & 15 ) != CC_E )
...
static void asm_hiop( ASMState* as, IRIns* ir ) {
    ...
    int uselo = ra_used( ir - 1 ),
        usehi = ra_used( ir ); /* Loword/hiword used? */
    ...
    if ( ( ir - 1 )->o == IR_CONV ) { /* Conversions to/from 64 bit. */
        ...
        return;
                ...
                IR_NE ) { /* 64 bit integer comparisons. ORDER IR. */
    ...
    switch ( ( ir - 1 )->o ) {
        ...
        case IR_CALLXS:
            if ( !uselo )
                ra_allocref( as, ir->op1,
                             RID2RSET( RID_RETLO ) ); /* Mark lo op as used. */
            break;
        case IR_CNEWI:
...
static void asm_stack_restore( ASMState* as, SnapShot* snap ) {
    ...
    for ( n = 0; n < nent; n++ ) {
        ...
        if ( irt_isnum( ir->t ) ) {
            ...
            if ( !irref_isk( ref ) ) {
                Reg src =
                    ra_alloc1( as, ref, rset_exclude( RSET_GPR, RID_BASE ) );
            ...
            if ( ( sn & ( SNAP_CONT | SNAP_FRAME ) ) ) {
                if ( s != 0 ) /* Do not overwrite link to previous frame. */
                    emit_movmroi( as, RID_BASE, ofs + 4,
                                  ( int32_t )( *flinks-- ) );
            } else {
...
static void asm_loop_fixup( ASMState* as ) {
    ...
    if ( as->realign ) {    /* Realigned loops use short jumps. */
        ...
        if ( as->loopinv ) { /* Inverted loop branch? */
            ...
            if ( as->loopinv == 2 )
                p[ -3 ] =
                    ( MCode )( target - p + 2 ); /* Patch opt. short jp. */
        } else {
            lua_assert( target - p >= -128 );
        ...
        if ( as->loopinv ) { /* Inverted loop branch? */
            ...
            *( int32_t* )( p - 4 ) = ( int32_t )( target - p ); /* Patch jcc. */
            if ( as->loopinv == 2 ) {
                *( int32_t* )( p - 10 ) =
                    ( int32_t )( target - p + 6 ); /* Patch opt. jp. */
                newloop = target + 8;
            }
...
static RegSet asm_head_side_base( ASMState* as, IRIns* irp, RegSet allow ) {
    ...
    if ( ra_hasreg( r ) ) {
        ...
        if ( irp->r == r ) {
            ...
            emit_rr( as, XO_MOV, r,
                     irp->r ); /* Move from coalesced parent reg. */
        } else {
...
#endif
            p1[ -3 ] = ( MCode )XI_LEA;
            p1[ -2 ] = MODRM( checki8( spadj ) ? XM_OFS8 : XM_OFS32, RID_ESP,
                              RID_ESP );
...
static void asm_tail_prep( ASMState* as ) {
    ...
    if ( as->loopref ) {
        ...
        as->mcp = p - ( ( ( as->flags & JIT_F_LEA_AGU ) ? 7 : 6 ) +
                        ( LJ_64 ? 1 : 0 ) );
        as->invmcp = NULL;
    }
}
...
static Reg asm_setup_call_slots( ASMState* as,
                                 IRIns* ir,
                                 const CCallInfo* ci ) {
    IRRef args[ CCI_NARGS_MAX * 2 ];
    int nslots;
...
void lj_asm_patchexit( jit_State* J,
                       GCtrace* T,
                       ExitNo exitno,
                       MCode* target ) {
    MCode* p = T->mcode;
    ...
    if ( len > 5 && p[ len - 5 ] == XI_JMP &&
         p + len - 6 + *( int32_t* )( p + len - 4 ) == px )
        *( int32_t* )( p + len - 4 ) = jmprel( p + len, target );
    /* Do not patch parent exit for a stack check. Skip beyond vmstate update.
     */
    for ( ; p < pe; p++ )
        if ( *( uint32_t* )( p + ( LJ_64 ? 3 : 2 ) ) == stateaddr &&
             p[ 0 ] == XI_MOVmi ) {
            p += LJ_64 ? 11 : 10;
            break;
        }
