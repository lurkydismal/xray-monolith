...
#define CALLBACK_SLOT2OFS( slot )                                      \
    ( CALLBACK_MCODE_HEAD + CALLBACK_MCODE_GROUP * ( ( slot ) / 32 ) + \
      4 * ( slot ) )
...
#define CALLBACK_MAX_SLOT                               \
    ( ( ( CALLBACK_MCODE_SIZE - CALLBACK_MCODE_HEAD ) / \
        ( CALLBACK_MCODE_GROUP + 4 * 32 ) ) *           \
      32 )
...
static void callback_mcode_init( global_State* g, uint32_t* page ) {
    ...
    *p++ = ARMI_PUSH | ARMF_N( RID_SP ) | RSET_RANGE( RID_R4, RID_R11 + 1 ) |
           RID2RSET( RID_LR );
    *p++ = ARMI_SUB | ARMI_K12 | ARMF_D( RID_R12 ) | ARMF_N( RID_R12 ) |
           CALLBACK_MCODE_HEAD;
    *p++ = ARMI_STR | ARMI_LS_P | ARMI_LS_W | ARMF_D( RID_R12 ) |
           ARMF_N( RID_SP ) | ( CFRAME_SIZE - 4 * 9 );
    *p++ =
        ARMI_LDR | ARMI_LS_P | ARMI_LS_U | ARMF_D( RID_R12 ) | ARMF_N( RID_PC );
    *p++ =
        ARMI_LDR | ARMI_LS_P | ARMI_LS_U | ARMF_D( RID_PC ) | ARMF_N( RID_PC );
    *p++ = u32ptr( g );
    *p++ = u32ptr( target );
    for ( slot = 0; slot < CALLBACK_MAX_SLOT; slot++ ) {
...
static void callback_mcode_init( global_State* g, uint32_t* page ) {
    ...
    *p++ = PPCI_ORI | PPCF_A( RID_TMP ) | PPCF_T( RID_TMP ) |
           ( u32ptr( target ) & 0xffff );
    *p++ = PPCI_ORI | PPCF_A( RID_R12 ) | PPCF_T( RID_R12 ) |
           ( u32ptr( g ) & 0xffff );
...
static void callback_mcode_init( global_State* g, uint32_t* page ) {
    ...
    *p++ = MIPSI_ORI | MIPSF_T( RID_R3 ) | MIPSF_S( RID_R3 ) |
           ( u32ptr( target ) & 0xffff );
...
#define CALLBACK_HANDLE_REGARG                                                 \
    if ( isfp ) {                                                              \
        if ( nfpr + 1 <= CCALL_NARG_FPR ) {                                    \
            sp = &cts->cb.fpr[ nfpr++ ];                                       \
            cta =                                                              \
                ctype_get( cts, CTID_DOUBLE ); /* FPRs always hold doubles. */ \
            goto done;                                                         \
        }                                                                      \
    } else { /* Try to pass argument in GPRs. */                               \
        if ( n > 1 ) {                                                         \
            lua_assert( ctype_isinteger( cta->info ) &&                        \
                        n == 2 );       /* int64_t. */                         \
...
#define CALLBACK_HANDLE_REGARG                                                 \
    if ( isfp && nfpr < CCALL_NARG_FPR ) { /* Try to pass argument in FPRs. */ \
        sp = ( void* )( ( uint8_t* )&cts->cb.fpr[ nfpr ] +                     \
                        ( ( LJ_BE && n == 1 ) ? 4 : 0 ) );                     \
        nfpr++;                                                                \
        ngpr += n;                                                             \
        goto done;                                                             \
...
static void callback_conv_args( CTState* cts, lua_State* L ) {
    ...
    setframe_ftsz(
        o, ( int )( ( char* )( o + 1 ) - ( char* )L->base ) + FRAME_CONT );
    L->top = L->base = ++o;
    if ( !ct )
