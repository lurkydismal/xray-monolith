...
static void bcwrite_ktabk( BCWriteCtx* ctx, cTValue* o, int narrow ) {
    ...
    if ( tvisstr( o ) ) {
        ...
        if ( !LJ_DUALNUM &&
             narrow ) { /* Narrow number constants to integers. */
...
static void bcwrite_knum( BCWriteCtx* ctx, GCproto* pt ) {
    ...
    for ( i = 0; i < sizekn; i++, o++ ) {
        ...
        if ( tvisint( o ) ) {
            ...
            bcwrite_uleb128(
                ctx, 1 + ( 2 * o->u32.lo | ( o->u32.lo & 0x80000000u ) ) );
    ...
    if ( ( pt->flags & PROTO_ILOOP ) || pt->trace ) {
        ...
        for ( i = 0; i < nbc; i++, p += sizeof( BCIns ) ) {
            ...
            if ( op == BC_IFORL || op == BC_IITERL || op == BC_ILOOP ||
                 op == BC_JFORI ) {
                p[ LJ_ENDIAN_SELECT( 0, 3 ) ] =
                    ( uint8_t )( op - BC_IFORL + BC_FORL );
            } else if ( op == BC_JFORL || op == BC_JITERL || op == BC_JLOOP ) {
                BCReg rd = p[ LJ_ENDIAN_SELECT( 2, 1 ) ] +
                           ( p[ LJ_ENDIAN_SELECT( 3, 0 ) ] << 8 );
                BCIns ins = traceref( J, rd )->startins;
                p[ LJ_ENDIAN_SELECT( 0, 3 ) ] =
                    ( uint8_t )( op - BC_JFORL + BC_FORL );
...
static void bcwrite_proto( BCWriteCtx* ctx, GCproto* pt ) {
    ...
    bcwrite_need( ctx, 4 + 6 * 5 +
                           ( pt->sizebc - 1 ) * ( MSize )sizeof( BCIns ) +
                           pt->sizeuv * 2 );

    /* Write prototype header. */
    bcwrite_byte( ctx,
                  ( pt->flags & ( PROTO_CHILD | PROTO_VARARG | PROTO_FFI ) ) );
    ...
    if ( !ctx->strip ) {
        if ( proto_lineinfo( pt ) )
            sizedbg = pt->sizept -
                      ( MSize )( ( char* )proto_lineinfo( pt ) - ( char* )pt );
    ...
    if ( ctx->status == 0 ) {
        ...
        ctx->status =
            ctx->wfunc( ctx->L, ctx->sb.buf + 5 - nn, nn + n, ctx->wdata );
    }
}
...
static void bcwrite_header( BCWriteCtx* ctx ) {
    ...
    bcwrite_byte(
        ctx, ( ctx->strip ? BCDUMP_F_STRIP : 0 ) + ( LJ_BE ? BCDUMP_F_BE : 0 ) +
