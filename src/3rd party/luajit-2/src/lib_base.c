...
LJLIB_CF( getfenv ) {
    ...
    fn = &gcval( o )->fn;
    settabV( L, L->top++,
             isluafunc( fn ) ? tabref( fn->l.env ) : tabref( L->env ) );
    return 1;
}
        ...
        if ( tviscdata( o ) ) {
            ...
            if ( ctype_isnum( ct->info ) || ctype_iscomplex( ct->info ) ) {
                if ( LJ_DUALNUM && ctype_isinteger_or_bool( ct->info ) &&
                    ...
                    int32_t i;
                    lj_cconv_ct_tv( cts, ctype_get( cts, CTID_INT32 ),
                                    ( uint8_t* )&i, o, 0 );
                    setintV( L->base - 1, i );
                    return FFH_RES( 1 );
                }
    ...
    if ( !tvisnil( mo = lj_meta_lookup( L, o, MM_tostring ) ) ) {
        ...
        if ( tvisnumber( o ) ) {
            ...
            if ( tvisfunc( o ) && isffunc( funcV( o ) ) )
                ...
                lua_pushfstring( L, "%s: %p", lj_typename( o ),
                                 lua_topointer( L, 1 ) );
...
static const char* reader_func( lua_State* L, void* ud, size_t* size ) {
    ...
    if ( tvisnil( L->top ) ) {
        *size = 0;
        ...
        copyTV( L, L->base + 4,
                L->top ); /* Anchor string in reserved stack slot. */
...
LJLIB_CF( load ) {
    ...
    if ( L->base < L->top && ( tvisstr( L->base ) || tvisnumber( L->base ) ) ) {
        ...
        status = luaL_loadbufferx( L, strdata( s ), s->len,
                                   strdata( name ? name : s ),
                                   mode ? strdata( mode ) : NULL );
    } else {
        ...
        status =
            lua_loadx( L, reader_func, NULL, name ? strdata( name ) : "=(load)",
                       mode ? strdata( mode ) : NULL );
    }
...
LJLIB_CF( collectgarbage ) {
    int opt = lj_lib_checkopt(
        L, 1, LUA_GCCOLLECT, /* ORDER LUA_GC* */
...
LJLIB_CF( print ) {
    ...
    cTValue* tv =
        lj_tab_getstr( tabref( L->env ), strV( lj_lib_upvalue( L, 1 ) ) );
    int shortcut;
    if ( tv && !tvisnil( tv ) ) {
        copyTV( L, L->top++, tv );
    } else {
...
LJLIB_CF( coroutine_status ) {
    ...
    if ( co == L )
        ...
        s = "dead";
    else if ( co->base > tvref( co->stack ) + 1 )
        s = "normal";
