...
static CTypeID ffi_checkctype( lua_State* L, CTState* cts, TValue* param ) {
    ...
    if ( tvisstr( o ) ) { /* Parse an abstract C type declaration. */
        ...
        cd = cdataV( o );
        return cd->ctypeid == CTID_CTYPEID ? *( CTypeID* )cdataptr( cd )
                                           : cd->ctypeid;
    }
}
...
static void* ffi_checkptr( lua_State* L, int narg, CTypeID id ) {
    ...
    lj_cconv_ct_tv( cts, ctype_get( cts, id ), ( uint8_t* )&p, o,
                    CCF_ARG( narg ) );
    return p;
}
...
static int ffi_index_meta( lua_State* L, CTState* cts, CType* ct, MMS mm ) {
    ...
    err_index:
        ...
        if ( tvisstr( L->base + 1 ) ) {
            lj_err_callerv( L, LJ_ERR_FFI_BADMEMBER, s,
                            strVdata( L->base + 1 ) );
        } else {
            const char* key =
                tviscdata( L->base + 1 )
                    ? strdata( lj_ctype_repr( L, cdataV( L->base + 1 )->ctypeid,
                                              NULL ) )
...
LJLIB_CF( ffi_meta___index ) LJLIB_REC( cdata_index 0 ) {
    ...
    TValue* o = L->base;
    if ( !( o + 1 < L->top &&
            tviscdata( o ) ) ) /* Also checks for presence of key. */
...
LJLIB_CF( ffi_meta___newindex ) LJLIB_REC( cdata_index 1 ) {
    ...
    TValue* o = L->base;
    if ( !( o + 2 < L->top &&
            tviscdata( o ) ) ) /* Also checks for key and value. */
        lj_err_argt( L, 1, LUA_TCDATA );
    ct = lj_cdata_index( cts, cdataV( o ), o + 1, &p, &qual );
    if ( ( qual & 1 ) ) {
        if ( ( qual & CTF_CONST ) )
...
static int ffi_arith( lua_State* L ) {
    MMS mm = ( MMS )( curr_func( L )->c.ffid - ( int )FF_ffi_meta___eq +
                      ( int )MM_eq );
...
LJLIB_CF( ffi_meta___tostring ) {
    ...
    if ( id == CTID_CTYPEID ) {
        ...
        if ( ctype_iscomplex( ct->info ) ) {
            setstrV( L, L->top - 1,
                     lj_ctype_repr_complex( L, cdataptr( cd ), ct->size ) );
            goto checkgc;
        } else if ( ct->size == 8 && ctype_isinteger( ct->info ) ) {
            setstrV( L, L->top - 1,
                     lj_ctype_repr_int64( L, *( uint64_t* )cdataptr( cd ),
            ...
            if ( ctype_isstruct( ct->info ) || ctype_isvector( ct->info ) ) {
                ...
                cTValue* tv =
                    lj_ctype_meta( cts, ctype_typeid( cts, ct ), MM_tostring );
                if ( tv )
                    return lj_meta_tailcall( L, tv );
            }
        }
...
#define LJLIB_MODULE_ffi_clib
    ...
    if ( !( o < L->top && tvisudata( o ) &&
            udataV( o )->udtype == UDTYPE_FFI_CLIB ) )
...
LJLIB_CF( ffi_new ) LJLIB_REC(.) {
    ...
    lj_cconv_ct_init( cts, ct, sz, cdataptr( cd ), o,
                      ( MSize )( L->top - o ) ); /* Initialize cdata. */
    if ( ctype_isstruct( ct->info ) ) {
        /* Handle ctype __gc metamethod. Use the fast lookup here. */
        cTValue* tv = lj_tab_getinth( cts->miscmap, -( int32_t )id );
        if ( tv && tvistab( tv ) &&
             ( tv = lj_meta_fast( L, tabV( tv ), MM_gc ) ) ) {
...
LJLIB_CF( ffi_sizeof ) LJLIB_REC( ffi_xof FF_ffi_sizeof ) {
    ...
    CTypeID id = ffi_checkctype( L, cts, NULL );
    CTSize sz;
    if ( LJ_UNLIKELY( tviscdata( L->base ) &&
                      cdataisv( cdataV( L->base ) ) ) ) {
...
LJLIB_SET( !) /* Store reference to finalizer table. */
    ...
    if ( !( ctype_isptr( ct->info ) || ctype_isstruct( ct->info ) ||
        ...
        lj_err_arg( L, 1, LJ_ERR_FFI_INVTYPE );
    if ( gcref(
             t->metatable ) ) { /* Update finalizer table, if still enabled. */
...
static void ffi_register_module( lua_State* L ) {
    cTValue* tmp =
        lj_tab_getstr( tabV( registry( L ) ), lj_str_newlit( L, "_LOADED" ) );
    if ( tmp && tvistab( tmp ) ) {
        GCtab* t = tabV( tmp );
        copyTV( L, lj_tab_setstr( L, t, lj_str_newlit( L, LUA_FFILIBNAME ) ),
                L->top - 1 );
        lj_gc_anybarriert( L, t );
    }
}
...
LUALIB_API int luaopen_ffi( lua_State* L ) {
    ...
    settabV( L, lj_tab_setstr( L, cts->miscmap, &cts->g->strempty ),
             tabV( L->top - 1 ) );
    L->top--;
    lj_clib_default( L,
                     tabV( L->top - 1 ) ); /* Create ffi.C default namespace. */
