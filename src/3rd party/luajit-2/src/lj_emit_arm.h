...
static void emit_lso( ASMState* as, ARMIns ai, Reg rd, Reg rn, int32_t ofs ) {
    ...
    if ( *as->mcp == ( ai | ARMI_LS_P | ARMI_LS_U | ARMF_D( rd ^ 1 ) |
                       ARMF_N( rn ) | ( ofs ^ 4 ) ) &&
         ( ai & ~( ARMI_LDR ^ ARMI_STR ) ) == ARMI_STR && rd != rn &&
         ( uint32_t )ofs <= 252 && !( ofs & 3 ) &&
         !( ( rd ^ ( ofs >> 2 ) ) & 1 ) && as->mcp != as->mcloop ) {
        as->mcp++;
        emit_lsox( as, ai == ARMI_LDR ? ARMI_LDRD : ARMI_STRD, rd & ~1, rn,
                   ofs & ~4 );
        return;
    }
    if ( ofs < 0 )
...
static void emit_vlso( ASMState* as, ARMIns ai, Reg rd, Reg rn, int32_t ofs ) {
    ...
    if ( ofs < 0 )
        ...
        ai |= ARMI_LS_U;
    *--as->mcp =
        ai | ARMI_LS_P | ARMF_D( rd & 15 ) | ARMF_N( rn ) | ( ofs >> 2 );
}
#endif
...
static int emit_kdelta1( ASMState* as, Reg d, int32_t i ) {
    ...
    while ( work ) {
        ...
        if ( emit_canremat( ref ) ) {
            int32_t delta =
                i - ( ra_iskref( ref ) ? ra_krefk( as, ref ) : IR( ref )->i );
...
static int emit_kdelta2( ASMState* as, Reg d, int32_t i ) {
    ...
    while ( work ) {
        ...
        if ( emit_canremat( ref ) ) {
            int32_t other =
                ra_iskref( ref ) ? ra_krefk( as, ref ) : IR( ref )->i;
            if ( other ) {
...
static void emit_loadn( ASMState* as, Reg r, cTValue* tv ) {
    ...
    i = i32ptr( tv );
    emit_vlso( as, ARMI_VLDR_D, r, ra_allock( as, ( i & ~1020 ), RSET_GPR ),
               ( i & 1020 ) );
}
#endif
...
#else
    if ( r >= RID_MAX_GPR )
        emit_vlso( as, irt_isnum( ir->t ) ? ARMI_VLDR_D : ARMI_VLDR_S, r,
                   RID_SP, ofs );
    else
#endif
...
#else
    if ( r >= RID_MAX_GPR )
        emit_vlso( as, irt_isnum( ir->t ) ? ARMI_VSTR_D : ARMI_VSTR_S, r,
                   RID_SP, ofs );
    else
#endif
