...
static void emit_peobj_sym( BuildCtx* ctx,
                            const char* name,
                            uint32_t value,
                            int sect,
                            int type,
...
void emit_peobj( BuildCtx* ctx ) {
    ...
    sofs += ( pesect[ PEOBJ_SECT_TEXT ].nreloc = ( uint16_t )ctx->nreloc ) *
            PEOBJ_RELOC_SIZE;
    ...
    sofs += ( pesect[ PEOBJ_SECT_XDATA ].size =
                  8 * 2 + 4 + 6 * 2 ); /* See below. */
    ...
    memcpy( pesect[ PEOBJ_SECT_RDATA_Z ].name, ".rdata$Z",
            sizeof( ".rdata$Z" ) - 1 );
    pesect[ PEOBJ_SECT_RDATA_Z ].ofs = sofs;
    sofs += ( pesect[ PEOBJ_SECT_RDATA_Z ].size =
                  ( uint32_t )strlen( ctx->dasm_ident ) + 1 );
    /* Flags: 40 = read, 30 = align4, 40 = initialized data. */
    ...
    */
    ...
    for ( i = 0; i < ctx->nreloc; i++ ) {
        ...
        reloc.symidx =
            1 + 2 + ctx->reloc[ i ].sym; /* Reloc syms are after .text sym. */
        reloc.type =
            ctx->reloc[ i ].type ? PEOBJ_RELOC_REL32 : PEOBJ_RELOC_DIR32;
    ...
    { /* Write .xdata section. */
        ...
        PEreloc reloc;
        xdata[ 0 ] =
            0x01 | 0x08 | 0x10; /* Ver. 1, uhandler/ehandler, prolog size 0. */
        ...
        xdata[ 7 ] = 0;         /* Alignment. */
        xdata[ 8 ] = xdata[ 9 ] =
            0;              /* Relocated address of exception handler. */
        xdata[ 10 ] = 0x01; /* Ver. 1, no handler, prolog size 0. */
        xdata[ 11 ] =
            0x1504; /* Number of unwind codes, fp = rbp, fpofs = 16. */
    ...
    for ( ;; ) {
        ...
        emit_peobj_sym( ctx, "@feat.00", 1, PEOBJ_SECT_ABS, PEOBJ_TYPE_NULL,
                        PEOBJ_SCL_STATIC );

        emit_peobj_sym_sect( ctx, pesect, PEOBJ_SECT_TEXT );
        for ( i = 0; i < nrsym; i++ )
            emit_peobj_sym( ctx, ctx->relocsym[ i ], 0, PEOBJ_SECT_UNDEF,
                            PEOBJ_TYPE_FUNC, PEOBJ_SCL_EXTERN );

#if LJ_TARGET_X64
        emit_peobj_sym_sect( ctx, pesect, PEOBJ_SECT_PDATA );
        emit_peobj_sym_sect( ctx, pesect, PEOBJ_SECT_XDATA );
        emit_peobj_sym( ctx, "lj_err_unwind_win64", 0, PEOBJ_SECT_UNDEF,
                        PEOBJ_TYPE_FUNC, PEOBJ_SCL_EXTERN );
#endif

        emit_peobj_sym( ctx, ctx->beginsym, 0, PEOBJ_SECT_TEXT, PEOBJ_TYPE_NULL,
                        PEOBJ_SCL_EXTERN );
        for ( i = 0; i < ctx->nsym; i++ )
            emit_peobj_sym( ctx, ctx->sym[ i ].name,
                            ( uint32_t )ctx->sym[ i ].ofs, PEOBJ_SECT_TEXT,
                            PEOBJ_TYPE_FUNC, PEOBJ_SCL_EXTERN );
