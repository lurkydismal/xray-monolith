...
static void emit_asm_reloc_mach( BuildCtx* ctx,
                                 uint8_t* cp,
                                 int n,
                                 const char* sym ) {
    ...
    if ( cp[ n ] == 0xe8 ) {
        ...
        opname = "jmp";
    } else if ( cp[ n ] >= 0x80 && cp[ n ] <= 0x8f && n > 0 &&
                cp[ n - 1 ] == 0x0f ) {
...
static void emit_asm_wordreloc( BuildCtx* ctx,
                                uint8_t* p,
                                int n,
                                const char* sym ) {
    uint32_t ins;
...
static void emit_asm_label( BuildCtx* ctx,
                            const char* name,
                            int size,
                            int isfunc ) {
    switch ( ctx->mode ) {
        case BUILD_elfasm:
            ...
            if ( !strncmp( name, "lj_vm_", 6 ) &&
                 strcmp( name, ctx->beginsym ) && !strstr( name, "hook" ) ) {
                fprintf( ctx->fp,
                         "\n\t.globl %s\n"
                         "\t.section \".opd\",\"aw\"\n"
                         "%s:\n"
                         ...
                         "\t.size .%s, %d\n"
                         ".%s:\n",
                         name, name, name, name, name, name, name, name, size,
                         name );
                break;
            }
#endif
            fprintf( ctx->fp,
                     ...
                     "\t.size %s, %d\n"
                     "%s:\n",
                     name, name, name, isfunc ? "function" : "object", name,
                     size, name );
            break;
        case BUILD_coffasm:
            fprintf( ctx->fp, "\n\t.globl %s\n", name );
    ...
    for ( i = rel = 0; i < ctx->nsym; i++ ) {
        ...
        while ( rel < ctx->nreloc && ctx->reloc[ rel ].ofs <= next ) {
            ...
            if ( ctx->mode == BUILD_machasm && r->type != 0 ) {
                emit_asm_reloc_mach( ctx, ctx->code + ofs, n,
                                     ctx->relocsym[ r->sym ] );
            } else {
            ...
            ofs += n + 4;
#else
            emit_asm_wordreloc( ctx, ctx->code + ofs, n,
                                ctx->relocsym[ r->sym ] );
            ofs += n;
#endif
            rel++;
