...
#if LJ_TARGET_X86ORX64
        if ( !LJ_64 &&
             ( ctx->mode == BUILD_coffasm || ctx->mode == BUILD_peobj ) )
            name[ 0 ] = '@';
        else
            *p = '\0';
...
static int collect_reloc( BuildCtx* ctx, uint8_t* addr, int idx, int type ) {
    ...
    if ( relocmap[ idx ] < 0 ) {
        ...
        ctx->relocsym[ ctx->nrelocsym ] =
            sym_decorate( ctx, "", extnames[ idx ] );
        ctx->nrelocsym++;
    }
...
static void sym_insert( BuildCtx* ctx,
                        int32_t ofs,
                        const char* prefix,
                        const char* suffix ) {
...
static int build_code( BuildCtx* ctx ) {
    ...
    ctx->sym = ( BuildSym* )malloc( ( ctx->npc + ctx->nglob + 1 ) *
                                    sizeof( BuildSym ) );
    ctx->nsym = 0;
    ...
    for ( i = 0; i < ctx->npc; i++ ) {
        ...
        ctx->bc_ofs[ i ] = ofs;
        if ( ( LJ_HASJIT ||
               !( i == BC_JFORI || i == BC_JFORL || i == BC_JITERL ||
                  i == BC_JLOOP || i == BC_IFORL || i == BC_IITERL ||
                  i == BC_ILOOP ) ) &&
    ...
    for ( i = 0; i < ctx->nglob; i++ ) {
        ...
        if ( !( len >= 2 && gl[ len - 2 ] == '_' && gl[ len - 1 ] == 'Z' ) )
            sym_insert(
                ctx, ( int32_t )( ( uint8_t* )( ctx->glob[ i ] ) - ctx->code ),
