...
static LJ_NOINLINE void bcread_fill( LexState* ls, MSize len, int need ) {
    ...
    do {
        ...
        ls->sb.n = ls->n;
        buf = ls->rfunc( ls->L, ls->rdata,
                         &size );         /* Get more data from reader. */
...
#define bcread_consume( ls, len ) \
    check_exp( ls->n >= ( len ), ls->n -= ( len ) )
...
static GCproto* bcread_proto( LexState* ls ) {
    ...
    sizept = ( MSize )sizeof( GCproto ) + sizebc * ( MSize )sizeof( BCIns ) +
             sizekgc * ( MSize )sizeof( GCRef );
    sizept = ( sizept + ( MSize )sizeof( TValue ) - 1 ) &
             ~( ( MSize )sizeof( TValue ) - 1 );
    ofsk = sizept;
    ...
    *( uint32_t* )( ( char* )pt + ofsk - sizeof( GCRef ) * ( sizekgc + 1 ) ) =
        0;
    ...
    pt->numline = numline;
    if ( sizedbg ) {
        MSize sizeli = ( sizebc - 1 ) << ( numline < 256     ? 0
                                           : numline < 65536 ? 1
                                                             : 2 );
...
static int bcread_header( LexState* ls ) {
    ...
    if ( ( flags & BCDUMP_F_STRIP ) ) {
        ...
        bcread_need( ls, len );
        ls->chunkname =
            lj_str_new( ls->L, ( const char* )bcread_mem( ls, len ), len );
    }
    return 1; /* Ok. */
}
