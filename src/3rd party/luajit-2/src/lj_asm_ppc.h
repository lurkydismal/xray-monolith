...
static void asm_exitstub_setup( ASMState* as, ExitNo nexits ) {
    ...
    *--mxp = PPCI_LI | PPCF_T( RID_TMP ) |
             as->T->traceno; /* Read by exit handler. */
    mxp--;
    *mxp = PPCI_BL |
           ( ( ( ( MCode* )( void* )lj_vm_exit_handler - mxp ) & 0x00ffffffu )
             << 2 );
    ...
    if ( ra_noreg( ir->r ) && canfuse( as, ir ) ) {
        if ( ir->o == IR_ADD ) {
            ...
            if ( irref_isk( ir->op2 ) &&
                ...
                right = ( left >> 8 );
                left &= 255;
                emit_fab( as, PPCI_LWZX | ( ( pi >> 20 ) & 0x780 ), rt, left,
                          right );
                return;
            }
            ...
            if ( irref_isk( ir->op2 ) ) {
                ...
                left &= 255;
                tmp = ra_scratch(
                    as, rset_exclude( rset_exclude( allow, left ), right ) );
...
static void asm_fusexrefx( ASMState* as,
                           PPCIns pi,
                           Reg rt,
                           IRRef ref,
                           RegSet allow ) {
...
static void asm_gencall( ASMState* as, const CCallInfo* ci, IRRef* args ) {
    ...
    if ( ( ci->flags &
           CCI_VARARG ) ) /* Vararg calls need to know about FPR use. */
        emit_tab( as, fpr == REGARG_FIRSTFPR ? PPCI_CRXOR : PPCI_CREQV, 6, 6,
                  6 );
}
...
static void asm_conv( ASMState* as, IRIns* ir ) {
    ...
    if ( irt_isfp( ir->t ) ) {
        ...
        if ( stfp ) {            /* FP to FP conversion. */
            ...
            kbias = ( const float* )lj_ir_k64_find(
                as->J, U64x( 59800004, 59800000 ) );
            if ( st == IRT_U32 )
                kbias++;
            emit_lsptr( as, PPCI_LFS, ( fbias & 31 ), ( void* )kbias,
                        rset_clear( allow, hibias ) );
            emit_tai( as, PPCI_STW, st == IRT_U32 ? left : RID_TMP, RID_SP,
                      SPOFS_TMPLO );
            emit_tai( as, PPCI_STW, hibias, RID_SP, SPOFS_TMPHI );
            if ( st != IRT_U32 )
        ...
        if ( irt_isguard( ir->t ) ) {
            ...
            if ( irt_isu32( ir->t ) ) {
                ...
                emit_asb( as, PPCI_OR, dest, dest,
                          tmpi ); /* Select with mask idiom. */
                emit_asb( as, PPCI_AND, tmpi, tmpi, RID_TMP );
                emit_asb( as, PPCI_ANDC, dest, dest, RID_TMP );
                emit_tai( as, PPCI_LWZ, tmpi, RID_SP,
                          SPOFS_TMPLO ); /* tmp = (int)(x) */
                emit_tai( as, PPCI_ADDIS, dest, dest,
                          0x8000 ); /* dest += 2^31 */
                emit_asb( as, PPCI_SRAWI, RID_TMP, dest,
                          31 ); /* mask = -(dest < 0) */
                emit_fai( as, PPCI_STFD, tmp, RID_SP, SPOFS_TMP );
                emit_tai( as, PPCI_LWZ, dest, RID_SP,
                          SPOFS_TMPLO ); /* dest = (int)(x-2^31) */
                emit_fb( as, PPCI_FCTIWZ, tmp, left );
                emit_fai( as, PPCI_STFD, tmp, RID_SP, SPOFS_TMP );
        ...
        if ( st >= IRT_I8 && st <= IRT_U16 ) { /* Extend to 32 bit integer. */
            ...
            if ( ( ir->op2 & IRCONV_SEXT ) )
                emit_as( as, st == IRT_I8 ? PPCI_EXTSB : PPCI_EXTSH, dest,
                         left );
            else
                emit_rot( as, PPCI_RLWINM, dest, left, 0,
                          st == IRT_U8 ? 24 : 16, 31 );
...
static void asm_tvptr( ASMState* as, Reg dest, IRRef ref ) {
    ...
    if ( irt_isnum( ir->t ) ) {
        ...
        Reg type;
        emit_tai( as, PPCI_ADDI, dest, RID_JGL,
...
static void asm_href( ASMState* as, IRIns* ir, IROp merge ) {
    ...
    if ( irt_isnum( kt ) ) {
        ...
        emit_fai( as, PPCI_LFD, tmpnum, dest,
        ...
        if ( !irt_ispri( kt ) )
            emit_tai( as, PPCI_LWZ, tmp2, dest,
    ...
    *l_loop = PPCI_BC | PPCF_Y | PPCF_CC( CC_NE ) |
    ...
    if ( khash == 0 ) {
        ...
        emit_tai( as, PPCI_LWZ, dest, tab, ( int32_t )offsetof( GCtab, node ) );
        emit_tai( as, PPCI_LWZ, tmp2, tab,
                  ( int32_t )offsetof( GCtab, hmask ) );
        if ( irref_isk( refkey ) ) {
            /* Nothing to do. */
        } else if ( irt_isstr( kt ) ) {
            emit_tai( as, PPCI_LWZ, tmp1, key,
            ...
            if ( irt_isnum( kt ) ) {
                ...
                emit_tai( as, PPCI_ADDIS, tmp2, key,
                          ( HASH_BIAS + 32768 ) >> 16 );
            }
        }
    }
...
static void asm_uref( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ir->op1 ) ) {
        ...
        if ( ir->o == IR_UREFC ) {
            ...
            emit_tai( as, PPCI_ADDI, dest, uv,
                      ( int32_t )offsetof( GCupval, tv ) );
            emit_tai( as, PPCI_LBZ, RID_TMP, uv,
                      ( int32_t )offsetof( GCupval, closed ) );
        } else {
            emit_tai( as, PPCI_LWZ, dest, uv,
...
static void asm_xstore( ASMState* as, IRIns* ir, int32_t ofs ) {
    ...
    if ( ir->r == RID_SINK )
        return;
    if ( ofs == 0 && mayfuse( as, ir->op2 ) &&
         ( irb = IR( ir->op2 ) )->o == IR_BSWAP && ra_noreg( irb->r ) &&
         ( irt_isint( ir->t ) || irt_isu32( ir->t ) ) ) {
        /* Fuse BSWAP with XSTORE to stwbrx. */
        Reg src = ra_alloc1( as, irb->op1, RSET_GPR );
        asm_fusexrefx( as, PPCI_STWBRX, src, ir->op1,
                       rset_exclude( RSET_GPR, src ) );
    } else {
        Reg src =
            ra_alloc1( as, ir->op2, irt_isfp( ir->t ) ? RSET_FPR : RSET_GPR );
...
static void asm_ahuvload( ASMState* as, IRIns* ir ) {
    ...
    if ( irt_isnum( t ) ) {
        Reg tisnum =
            ra_allock( as, ( int32_t )LJ_TISNUM, rset_exclude( allow, idx ) );
        asm_guardcc( as, CC_GE );
        emit_ab( as, PPCI_CMPLW, type, tisnum );
        if ( ra_hasreg( dest ) ) {
            if ( ofs == AHUREF_LSX ) {
                tmp = ra_scratch(
                    as, rset_exclude( rset_exclude( RSET_GPR, ( idx & 255 ) ),
                                      ( idx >> 8 ) ) );
    ...
    if ( ( ir->op2 & IRSLOAD_CONVERT ) && irt_isguard( t ) && irt_isint( t ) ) {
        ...
        if ( ( ir->op2 & IRSLOAD_CONVERT ) ) {
            if ( irt_isint( t ) ) {
                ...
                Reg hibias =
                    ra_allock( as, 0x43300000, rset_clear( allow, tmp ) );
                Reg fbias = ra_scratch( as, rset_exclude( RSET_FPR, dest ) );
                emit_fab( as, PPCI_FSUB, dest, dest, fbias );
                emit_fai( as, PPCI_LFD, dest, RID_SP, SPOFS_TMP );
                emit_lsptr( as, PPCI_LFS, ( fbias & 31 ),
...
static void asm_obar( ASMState* as, IRIns* ir ) {
    ...
    asm_gencall( as, ci, args );
    emit_tai( as, PPCI_ADDI, ra_releasetmp( as, ASMREF_TMP1 ), RID_JGL,
              -32768 );
    obj = IR( ir->op1 )->r;
    ...
    emit_tai( as, PPCI_LBZ, tmp, obj,
              ( int32_t )offsetof( GCupval, marked ) -
                  ( int32_t )offsetof( GCupval, tv ) );
    emit_tai( as, PPCI_LBZ, RID_TMP, val,
...
static void asm_add( ASMState* as, IRIns* ir ) {
    if ( irt_isnum( ir->t ) ) {
        ...
        if ( irref_isk( ir->op2 ) ) {
            ...
            if ( checki16( k ) ) {
                pi = PPCI_ADDI;
                /* May fail due to spills/restores above, but simplifies the
                 * logic. */
...
static void asm_bitop( ASMState* as, IRIns* ir, PPCIns pi, PPCIns pik ) {
    ...
    if ( irref_isk( ir->op2 ) ) {
        int32_t k = IR( ir->op2 )->i;
        Reg tmp = left;
        if ( ( checku16( k ) || ( k & 0xffff ) == 0 ) ||
             ( tmp = dest, !as->sectref ) ) {
            if ( !checku16( k ) ) {
                emit_asi( as, pik ^ ( PPCI_ORI ^ PPCI_ORIS ), dest, tmp,
                          ( k >> 16 ) );
                if ( ( k & 0xffff ) == 0 )
                    return;
            }
...
static void asm_bitand( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ir->op2 ) ) {
        ...
        if ( k ) {
            ...
            if ( ( k1 & ( k1 + 1 ) ) == 0 ) {
                asm_fuseandsh( as,
                               PPCI_RLWINM | dot | PPCF_A( dest ) |
                                   PPCF_MB( 31 - lj_fls( ( uint32_t )k ) ) |
                                   PPCF_ME( 31 - s1 ),
                               k, lref );
                return;
            }
...
static void asm_bitshift( ASMState* as, IRIns* ir, PPCIns pi, PPCIns pik ) {
    ...
    if ( irref_isk( ir->op2 ) ) { /* Constant shifts. */
        ...
        if ( pik == 0 ) /* SLWI */
            ...
            emit_rot( as, PPCI_RLWINM | dot, dest, left, ( 32 - shift ) & 31,
                      shift, 31 );
        else
        ...
        if ( tmp == left || tmp == right )
            tmp = ra_scratch(
                as, rset_exclude(
                        ...
                        right ) );
        emit_facb( as, PPCI_FSEL, dest, tmp, ismax ? left : right,
                   ismax ? right : left );
...
static void asm_intcomp_( ASMState* as,
                          IRRef lref,
                          IRRef rref,
                          Reg cr,
                          PPCCC cc ) {
        ...
        if ( ( cc & CC_UNSIGNED ) ==
            ...
            if ( checki16( k ) ) {
                ...
                if ( k == 0 && lref == as->curins - 1 )
                    as->flagmcp =
                        as->mcp; /* Allow elimination of the compare. */
                return;
            } else if ( ( cc & 3 ) ==
                        ( CC_EQ & 3 ) ) { /* Use CMPLWI for EQ or NE. */
                if ( checku16( k ) ) {
    ...
    emit_tab( as, ( cc & CC_UNSIGNED ) ? PPCI_CMPLW : PPCI_CMPW, cr, left,
              right );
}
...
static void asm_comp( ASMState* as, IRIns* ir ) {
    ...
    if ( irt_isnum( ir->t ) ) {
        ...
        asm_guardcc( as, ( cc >> 4 ) );
        if ( ( cc & CC_TWO ) )
            emit_tab( as, PPCI_CROR, ( ( cc >> 4 ) & 3 ), ( ( cc >> 4 ) & 3 ),
                      ( CC_EQ & 3 ) );
...
static void asm_comp64( ASMState* as, IRIns* ir ) {
    ...
    if ( ( cc & 3 ) == ( CC_EQ & 3 ) ) {
        asm_guardcc( as, cc );
        emit_tab( as, ( cc & 4 ) ? PPCI_CRAND : PPCI_CROR, ( CC_EQ & 3 ),
                  ( CC_EQ & 3 ), 4 + ( CC_EQ & 3 ) );
    } else {
        asm_guardcc( as, CC_EQ );
        emit_tab( as, PPCI_CROR, ( CC_EQ & 3 ), ( CC_EQ & 3 ),
                  ( ( cc ^ ~( cc >> 2 ) ) & 1 ) );
        emit_tab( as, ( cc & 4 ) ? PPCI_CRAND : PPCI_CRANDC, ( CC_EQ & 3 ),
                  ( CC_EQ & 3 ), 4 + ( cc & 3 ) );
    }
...
static void asm_loop_fixup( ASMState* as ) {
    ...
    if ( as->loopinv ) { /* Inverted loop branch? */
         ...
         */
        p[ -2 ] = ( p[ -2 ] & ( 0xffff0000u & ~PPCF_Y ) ) |
                  ( ( ( target - p + 2 ) & 0x3fffu ) << 2 );
    } else {
...
static void asm_tail_fixup( ASMState* as, TraceNo lnk ) {
    ...
    if ( spadj == 0 ) {
        ...
        *--p = PPCI_NOP;
        ...
        p[ -3 ] = PPCI_ADDI | PPCF_T( RID_TMP ) | PPCF_A( RID_SP ) |
                  ( CFRAME_SIZE + spadj );
            ...
            if ( ( ( delta + 0x8000 ) >> 16 ) == 0 ) {
                *p = ( ins & 0xffdf0000u ) | ( ( uint32_t )delta & 0xffffu ) |
                ...
                if ( !cstart )
                    ...
                    ( ( ins ^ ( ( char* )px - ( char* )p ) ) & 0x03ffffffu ) ==
                        0 ) {
            ptrdiff_t delta = ( char* )target - ( char* )p;
