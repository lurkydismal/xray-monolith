...
LJ_NORET static void cp_errmsg( CPState* cp, CPToken tok, ErrMsg em, ... ) {
    ...
    if ( tok == 0 ) {
        tokstr = NULL;
    } else if ( tok == CTOK_IDENT || tok == CTOK_INTEGER ||
                tok == CTOK_STRING || tok >= CTOK_FIRSTDECL ) {
        if ( cp->sb.n == 0 )
            cp_save( cp, '$' );
        cp_save( cp, '\0' );
        tokstr = cp->sb.buf;
...
static CPToken cp_param( CPState* cp ) {
    CPChar c = cp_get( cp );
    TValue* o = cp->param;
    if ( lj_char_isident( c ) ||
         c == '$' ) /* Reserve $xyz for future extensions. */
    ...
    if ( tvisstr( o ) ) {
        ...
        GCcdata* cd;
        if ( !tviscdata( o ) )
            lj_err_argtype( cp->L, ( int )( o - cp->L->base ) + 1,
                            "type parameter" );
        cd = cdataV( o );
...
#define CPNS_DEFAULT                                               \
    ( ( 1u << CT_KW ) | ( 1u << CT_TYPEDEF ) | ( 1u << CT_FUNC ) | \
      ( 1u << CT_EXTERN ) | ( 1u << CT_CONSTVAL ) )
#define CPNS_STRUCT \
    ( ( 1u << CT_KW ) | ( 1u << CT_STRUCT ) | ( 1u << CT_ENUM ) )
    ...
    if ( wantsz ) {
        if ( sz != CTSIZE_INVALID )
            k->u32 = sz;
                  ...
                  CTID_A_CCHAR ) /* Special case for sizeof("string"). */
    ...
    for ( ;; ) {
        ...
        if ( cp_opt( cp, '[' ) ) { /* Array/pointer index. */
            ...
            k->u32 = 0;
                    ...
                    cp->tok == CTOK_DEREF ) { /* Struct deref. */
            CTSize ofs;
            CType* fct;
            ct = lj_ctype_rawref( cp->cts, k->id );
            ...
            if ( !ctype_isstruct( ct->info ) || ct->size == CTSIZE_INVALID ||
                ...
                GCstr* s = lj_ctype_repr( cp->cts->L,
                                          ctype_typeid( cp->cts, ct ), NULL );
                cp_errmsg( cp, 0, LJ_ERR_FFI_BADMEMBER, strdata( s ),
                           strdata( cp->str ) );
            }
            ct = fct;
            ...
            case 10:
                if ( cp_opt( cp, '*' ) ) {
                    ...
                    if ( k2.id > k->id )
                        k->id = k2.id; /* Trivial promotion to unsigned. */
                    if ( k2.u32 == 0 ||
                         ( k->id == CTID_INT32 && k->u32 == 0x80000000u &&
                           k2.i32 == -1 ) )
...
static void cp_push_attributes( CPDecl* decl ) {
    ...
    if ( ctype_isfunc( ct->info ) ) { /* Ok to modify in-place. */
        ...
        if ( ( decl->attr & CTFP_ALIGNED ) &&
             !( decl->mode & CPARSE_MODE_FIELD ) )
...
static void cp_push_type( CPDecl* decl, CTypeID id ) {
    ...
    switch ( ctype_type( info ) ) {
        case CT_STRUCT:
        case CT_ENUM:
            cp_push( decl, CTINFO( CT_TYPEDEF, id ),
...
static CTypeID cp_decl_intern( CPState* cp, CPDecl* decl ) {
    ...
    do {
        ...
        if ( ctype_istypedef( info ) ) {
            lua_assert( id == 0 );
            id = ctype_cid( info );
            /* Always refetch info/size, since struct/enum may have been
             * completed. */
            ...
            if ( id ) {
                ...
                if ( ctype_isfunc( refct->info ) ||
                     ctype_isrefarray( refct->info ) )
            ...
            if ( ctype_isnum(
                ...
                if ( !( info & CTF_BOOL ) ) {
                    ...
                    if ( vsize ) { /* Vector size set? */
                        ...
                        if ( vsize >= esize ) {
                            ...
                            /* Then create a vector (array) with vsize
                             * alignment. */
                            size = ( 1u << vsize );
                            if ( vsize > 4 )
                            ...
                            info = CTINFO( CT_ARRAY, ( info & CTF_QUAL ) +
                                                         CTF_VECTOR +
                                                         CTALIGN( vsize ) );
                        }
                    }
                ...
                if ( id && ctype_isref( ctype_raw( cp->cts, id )->info ) )
                    cp_err( cp, LJ_ERR_FFI_INVTYPE );
                if ( ctype_isref( info ) ) {
                    info &= ~CTF_VOLATILE; /* Refs are always const, never
                                              volatile. */
                    /* No intervening attributes allowed, skip forward. */
                    while ( idx ) {
                        CType* ctn = &decl->stack[ idx ];
            ...
            } else if ( ctype_isarray(
                            info ) ) { /* Check for valid array size etc. */
                if ( ct->sib ==
        ...
        if ( cp->tok == CTOK_IDENT ) {
            ...
            switch ( attrstr->hash ) {
                ...
                default: /* Skip all other attributes. */
                    ...
                    CTOK_FIRSTDECL ) { /* For __attribute((const)) etc. */
            cp_next( cp );
        skip_attr:
            if ( cp_opt( cp, '(' ) ) {
...
static CTypeID cp_struct_name( CPState* cp, CPDecl* sdecl, CTInfo info ) {
    ...
    if ( cp->tok != '{' ) {
        ...
        if ( cp->val.id ) { /* Name of existing struct/union/enum. */
            ...
            ct = cp->ct;
            if ( ( ct->info ^ info ) &
                 ( CTMASK_NUM | CTF_UNION ) ) /* Wrong type. */
                cp_errmsg( cp, 0, LJ_ERR_FFI_REDEF,
                           strdata( gco2str( gcref( ct->name ) ) ) );
...
static void cp_struct_layout( CPState* cp, CTypeID sid, CTInfo sattr ) {
    ...
    while ( fieldid ) {
        ...
        if ( ctype_isfield( ct->info ) ||
            ...
            CTSize align,
                amask; /* Alignment (pow2) and alignment mask (bits). */
            CTSize sz;
            CTInfo info = lj_ctype_info( cp->cts, ctype_cid( ct->info ), &sz );
            CTSize bsz,
                csz = 8 * sz; /* Field size and container size (in bits). */
            sinfo |= ( info &
                       ( CTF_QUAL | CTF_VLA ) ); /* Merge pseudo-qualifiers. */
                ...
                if ( bsz == csz && ( bofs & amask ) == 0 ) {
                    ...
                    ct->info = CTINFO(
                        ...
                        ( info & ( CTF_QUAL | CTF_UNSIGNED | CTF_BOOL ) ) +
                            ( csz << ( CTSHIFT_BITCSZ - 3 ) ) +
                            ( bsz << CTSHIFT_BITBSZ ) );
#if LJ_BE
                    ct->info += ( ( csz - ( bofs & ( csz - 1 ) ) - bsz )
                                  << CTSHIFT_BITPOS );
#else
                    ct->info += ( ( bofs & ( csz - 1 ) ) << CTSHIFT_BITPOS );
#endif
                    ct->size = ( ( bofs & ~( csz - 1 ) ) >>
                                 3 ); /* Store container offset. */
                }
            }
    ...
    if ( cp_opt( cp, '{' ) ) { /* Struct/union definition. */
        ...
        while ( cp->tok != '}' ) {
            ...
            decl.mode = scl ? CPARSE_MODE_DIRECT
                            : CPARSE_MODE_DIRECT | CPARSE_MODE_ABSTRACT |
                                  CPARSE_MODE_FIELD;
            ...
            for ( ;; ) {
                ...
                if ( ( scl & CDF_STATIC ) ) { /* Static constant in struct
                                                 namespace. */
                    CType* ct;
                    ...
                    CType* ct;
                    CTypeID fieldid =
                        lj_ctype_new( cp->cts, &ct ); /* Do this first. */
                    CType* tct = ctype_raw( cp->cts, ctypeid );

                    if ( decl.bits == CTSIZE_INVALID ) { /* Regular field. */
                        if ( ctype_isarray( tct->info ) &&
                             tct->size == CTSIZE_INVALID )
                            lastdecl = 1; /* a[] or a[?] must be the last
                                             declared field. */

                        /* Accept transparent struct/union/enum. */
                        if ( !decl.name ) {
                            if ( !( ( ctype_isstruct( tct->info ) &&
                                      !( tct->info & CTF_VLA ) ) ||
                                    ctype_isenum( tct->info ) ) )
                                cp_err_token( cp, CTOK_IDENT );
                            ct->info = CTINFO(
                                CT_ATTRIB, CTATTRIB( CTA_SUBTYPE ) + ctypeid );
                            ct->size = ctype_isstruct( tct->info )
                                           ? ( decl.attr | 0x80000000u )
                                           : 0; /* For layout phase. */
                            goto add_field;
                        }
                    } else { /* Bitfield. */
                    ...
                    ct->info =
                        CTINFO( CT_FIELD, ctypeid + ( bsz << CTSHIFT_BITCSZ ) );
                    ct->size = decl.attr;
                    if ( decl.name )
                        ctype_setname( ct, decl.name );
        ...
        cp_check( cp, '}' );
        ctype_get( cp->cts, lastid )->sib =
            0; /* Drop sib = 1 for empty structs. */
        cp_decl_attributes(
            cp, sdecl ); /* Layout phase needs postfix attributes. */
        ...
        do {
            ...
            if ( cp_opt( cp, '=' ) ) {
                ...
                if ( k.id == CTID_UINT32 ) {
                    ...
                    ** that the enum type itself is unsigned, unless there are
                    * any
                    ** negative constants.
                    */
                    k.id = CTID_INT32;
...
static CPscl cp_decl_spec( CPState* cp, CPDecl* decl, CPscl scl ) {
    ...
    for ( ;; ) { /* Parse basic types. */
        ...
        if ( sz || tdef ||
             ( cds & ( CDF_SHORT | CDF_LONG | CDF_SIGNED | CDF_UNSIGNED |
                       CDF_COMPLEX ) ) )
            break;
        switch ( cp->tok ) {
            case CTOK_STRUCT:
            ...
            case CTOK_UNION:
                tdef =
                    cp_decl_struct( cp, decl, CTINFO( CT_STRUCT, CTF_UNION ) );
                continue;
            case CTOK_ENUM:
...
end_decl:
    ...
    if ( tdef ) {
        ...
        cp_push( decl, CTINFO( CT_VOID, ( decl->attr & CTF_QUAL ) ),
                 CTSIZE_INVALID );
        decl->attr &= ~CTF_QUAL;
    } else {
        /* Determine type info and size. */
        CTInfo info =
            CTINFO( CT_NUM, ( cds & CDF_UNSIGNED ) ? CTF_UNSIGNED : 0 );
        if ( ( cds & CDF_BOOL ) ) {
            if ( ( cds & ~( CDF_SCL | CDF_BOOL | CDF_INT | CDF_SIGNED |
                            CDF_UNSIGNED ) ) )
            ...
            if ( ( cds & ( CDF_CHAR | CDF_SIGNED | CDF_UNSIGNED ) ) ==
                 CDF_CHAR )
                info |=
                    CTF_UCHAR; /* Handle platforms where char is unsigned. */
    ...
    if ( cp->tok != ')' ) {
        do {
            ...
            if ( ctype_isvoid( ct->info ) )
                ...
                ctypeid = lj_ctype_intern(
                    cp->cts,
                    CTINFO( CT_PTR, CTALIGN_PTR | ctype_cid( ct->info ) ),
                    CTSIZE_PTR );
            else if ( ctype_isfunc( ct->info ) )
                ctypeid = lj_ctype_intern(
                    cp->cts, CTINFO( CT_PTR, CTALIGN_PTR | ctypeid ),
                    CTSIZE_PTR );
            /* Add new parameter. */
...
static void cp_declarator( CPState* cp, CPDecl* decl ) {
    ...
    for ( ;; ) {                   /* Head of declarator. */
        if ( cp_opt( cp, '*' ) ) { /* Pointer. */
            ...
            cp_push( decl, info, sz );
                    ...
                    cp_opt( cp, CTOK_ANDAND ) ) { /* Reference. */
            decl->attr &= ~( CTF_QUAL | ( CTMASK_MSIZEP << CTSHIFT_MSIZEP ) );
    ...
    if ( cp_opt( cp, '(' ) ) { /* Inner declarator. */
        CPDeclIdx pos;
        cp_decl_attributes( cp, decl );
        /* Resolve ambiguity between inner declarator and 1st function
         * parameter. */
    ...
    if ( ( decl->mode & CPARSE_MODE_FIELD ) &&
         cp_opt( cp, ':' ) ) /* Field width. */
...
static void cp_pragma( CPState* cp, BCLine pragmaline ) {
    ...
    if ( cp->tok == CTOK_IDENT &&
        ...
        if ( cp->tok == CTOK_IDENT ) {
            if ( cp->str->hash == H_( 738e923c, a1b65954 ) ) { /* push */
                if ( cp->curpack < CPARSE_MAX_PACKSTACK ) {
                    cp->packstack[ cp->curpack + 1 ] =
                        cp->packstack[ cp->curpack ];
                    cp->curpack++;
                }
        ...
        if ( cp->tok == CTOK_INTEGER ) {
            cp->packstack[ cp->curpack ] =
                cp->val.u32 ? lj_fls( cp->val.u32 ) : 0;
            cp_next( cp );
        } else {
...
static void cp_decl_multi( CPState* cp ) {
    ...
    while ( cp->tok != CTOK_EOF ) {
        ...
        for ( ;; ) {
            ...
            ctypeid = cp_decl_intern( cp, &decl );
            if ( decl.name &&
                 !decl.nameid ) { /* NYI: redeclarations are ignored. */
                CType* ct;
                CTypeID id;
                if ( ( scl & CDF_TYPEDEF ) ) { /* Create new typedef. */
                    ...
                    goto noredir;
                } else if ( ctype_isfunc(
                                ctype_get( cp->cts, ctypeid )->info ) ) {
                    /* Treat both static and extern function declarations as
                     * extern. */
                    ct = ctype_get( cp->cts, ctypeid );
                    /* We always get new anonymous functions (typedefs are
                     * copied). */
                    lua_assert( gcref( ct->name ) == NULL );
                    id = ctypeid; /* Just name it. */
                } else if ( ( scl &
                              CDF_STATIC ) ) { /* Accept static constants. */
                ...
                if ( decl.redir ) { /* Add attribute for redirected symbol name.
                    ...
                    ct = ctype_get( cp->cts,
                                    id ); /* Table may have been reallocated. */
