...
#define JIT_F_OPT_3                                                 \
    ( JIT_F_OPT_2 | JIT_F_OPT_FWD | JIT_F_OPT_DSE | JIT_F_OPT_ABC | \
      JIT_F_OPT_SINK | JIT_F_OPT_FUSE )
...
#define SNAP( slot, flags, ref ) \
    ( ( ( SnapEntry )( slot ) << 24 ) + ( flags ) + ( ref ) )
#define SNAP_TR( slot, tr )             \
    ( ( ( SnapEntry )( slot ) << 24 ) + \
      ( ( tr ) & ( TREF_CONT | TREF_FRAME | TREF_REFMASK ) ) )
...
#define snap_setref( sn, ref ) \
    ( ( ( sn ) & ( 0xffff0000 & ~SNAP_NORESTORE ) ) | ( ref ) )
...
#define gco2trace( o ) \
    check_exp( ( o )->gch.gct == ~LJ_TTRACE, ( GCtrace* )( o ) )
#define traceref( J, n )                                   \
    check_exp( ( n ) > 0 && ( MSize )( n ) < J->sizetrace, \
               ( GCtrace* )gcref( J->trace[ ( n ) ] ) )
...
#define LJ_KSIMD( J, n )                                         \
    ( ( TValue* )( ( ( intptr_t )&J->ksimd[ 2 * ( n ) ] + 15 ) & \
                   ~( intptr_t )15 ) )
