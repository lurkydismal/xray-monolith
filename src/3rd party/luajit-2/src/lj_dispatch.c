...
void lj_dispatch_init( GG_State* GG ) {
    ...
    GG->g.bc_cfunc_ext = GG->g.bc_cfunc_int =
        BCINS_AD( BC_FUNCC, LUA_MINSTACK, 0 );
...
void lj_dispatch_update( global_State* g ) {
    ...
    mode |= G2J( g )->state != LJ_TRACE_IDLE
                ? ( DISPMODE_REC | DISPMODE_INS | DISPMODE_CALL )
                : 0;
#endif
    mode |=
        ( g->hookmask & ( LUA_MASKLINE | LUA_MASKCOUNT ) ) ? DISPMODE_INS : 0;
    ...
    if ( oldmode != mode ) { /* Mode changed? */
        ...
        /* Init static counting instruction dispatch first (may be copied
         * below). */
        ...
        if ( ( oldmode ^ mode ) & ( DISPMODE_REC | DISPMODE_INS ) ) {
            ...
            if ( !( mode &
                    ( DISPMODE_REC | DISPMODE_INS ) ) ) { /* No ins dispatch? */
        ...
        if ( ( oldmode ^ mode ) &
             DISPMODE_CALL ) { /* Update the whole table? */
            uint32_t i;
        ...
        case LUAJIT_MODE_ENGINE:
            if ( ( mode & LUAJIT_MODE_FLUSH ) ) {
                ...
                if ( !( mode & LUAJIT_MODE_ON ) )
                    ...
                    return 0; /* Don't turn on JIT compiler without SSE2
                                 support. */
#else
                else
                    G2J( g )->flags |= ( uint32_t )JIT_F_ON;
        ...
        case LUAJIT_MODE_ALLSUBFUNC: {
            cTValue* tv = idx == 0  ? frame_prev( L->base - 1 )
                          : idx > 0 ? L->base + ( idx - 1 )
                                    : L->top + idx;
            GCproto* pt;
            if ( ( idx == 0 || tvisfunc( tv ) ) &&
                 isluafunc( &gcval( tv )->fn ) )
                pt = funcproto(
                    &gcval( tv )->fn ); /* Cannot use funcV() for frame slot. */
...
void LJ_FASTCALL lj_dispatch_ins( lua_State* L, const BCIns* pc ) {
...
#if LJ_HASJIT
    {
        ...
        if ( J->state != LJ_TRACE_IDLE ) {
            ...
            J->L = L;
            lj_trace_ins(
                J, pc - 1 ); /* The interpreter bytecode PC is offset by 1. */
