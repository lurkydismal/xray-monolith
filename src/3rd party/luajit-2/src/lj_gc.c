...
static void gc_mark( global_State* g, GCobj* o ) {
    ...
    if ( LJ_UNLIKELY( gct == ~LJ_TUDATA ) ) {
        ...
        lua_assert( gct == ~LJ_TFUNC || gct == ~LJ_TTAB || gct == ~LJ_TTHREAD ||
                    gct == ~LJ_TPROTO );
...
static void gc_mark_uv( global_State* g ) {
    ...
    for ( uv = uvnext( &g->uvhead ); uv != &g->uvhead; uv = uvnext( uv ) ) {
        lua_assert( uvprev( uvnext( uv ) ) == uv &&
                    uvnext( uvprev( uv ) ) == uv );
...
size_t lj_gc_separateudata( global_State* g, int all ) {
    ...
    while ( ( o = gcref( *p ) ) != NULL ) {
        ...
        } else if ( !lj_meta_fastg( g, tabref( gco2ud( o )->metatable ),
                                    MM_gc ) ) {
...
static int gc_traverse_tab( global_State* g, GCtab* t ) {
    ...
    if ( weak ==
         LJ_GC_WEAK ) /* Nothing to mark if both keys/values are weak. */
        return 1;
...
static void gc_traverse_func( global_State* g, GCfunc* fn ) {
    ...
    if ( isluafunc( fn ) ) {
        ...
        for ( i = 0; i < fn->l.nupvalues;
              i++ ) /* Mark Lua function upvalues. */
...
static void gc_traverse_proto( global_State* g, GCproto* pt ) {
    ...
    for ( i = -( ptrdiff_t )pt->sizekgc; i < 0;
          i++ ) /* Mark collectable consts. */
...
static void gc_shrink( global_State* g, lua_State* L ) {
    if ( g->strnum <= ( g->strmask >> 2 ) &&
         g->strmask > LJ_MIN_STRTAB * 2 - 1 )
        lj_str_resize( L, g->strmask >> 1 ); /* Shrink string table. */
    if ( g->tmpbuf.sz > LJ_MIN_SBUF * 2 )
        lj_str_resizebuf( L, &g->tmpbuf,
                          g->tmpbuf.sz >> 1 ); /* Shrink temp buf. */
}
...
static const GCFreeFunc gc_freefunc[] = {
    ( GCFreeFunc )lj_str_free,   ( GCFreeFunc )lj_func_freeuv,
    ( GCFreeFunc )lj_state_free, ( GCFreeFunc )lj_func_freeproto,
...
static GCRef* gc_sweep( global_State* g, GCRef* p, uint32_t lim ) {
    ...
    while ( ( o = gcref( *p ) ) != NULL && lim-- > 0 ) {
        ...
        if ( ( ( o->gch.marked ^ LJ_GC_WHITES ) &
               ow ) ) { /* Black or current white? */
            lua_assert( !isdead( g, o ) || ( o->gch.marked & LJ_GC_FIXED ) );
            makewhite( g,
                       o ); /* Value is alive, change to the current white. */
            p = &o->gch.nextgc;
            ...
            if ( o == gcref( g->gc.root ) )
                setgcrefr( g->gc.root,
                           o->gch.nextgc ); /* Adjust list anchor. */
...
static int gc_mayclear( cTValue* o, int val ) {
    if ( tvisgcv( o ) ) { /* Only collectable objects can be weak references. */
        if ( tvisstr(
                 o ) ) { /* But strings cannot be used as weak references. */
...
static void gc_call_finalizer( global_State* g,
                               lua_State* L,
                               cTValue* mo,
                               GCobj* o ) {
...
static void atomic( global_State* g, lua_State* L ) {
    ...
    gc_mark_uv(
        g ); /* Need to remark open upvalues (the thread may be dead). */
    ...
    udsize =
        lj_gc_separateudata( g, 0 );  /* Separate userdata to be finalized. */
...
static size_t gc_onestep( lua_State* L ) {
    ...
    switch ( g->gc.state ) {
        case GCSpause:
            gc_mark_start(
                g ); /* Start a new GC cycle by marking all GC roots. */
            return 0;
        case GCSpropagate:
        ...
        case GCSsweepstring: {
            ...
            gc_fullsweep(
                g, &g->strhash[ g->gc.sweepstr++ ] ); /* Sweep one chain. */
        ...
        case GCSsweep: {
            MSize old = g->gc.total;
            setmref( g->gc.sweep,
                     gc_sweep( g, mref( g->gc.sweep, GCRef ), GCSWEEPMAX ) );
            lua_assert( old >= g->gc.total );
            g->gc.estimate -= old - g->gc.total;
...
void lj_gc_fullgc( lua_State* L ) {
    ...
    if ( g->gc.state <= GCSatomic ) { /* Caught somewhere in the middle. */
        setmref( g->gc.sweep,
                 &g->gc.root );     /* Sweep everything (preserving it). */
...
#define TV2MARKED( x )                                 \
    ( *( ( uint8_t* )( x ) - offsetof( GCupval, tv ) + \
         offsetof( GCupval, marked ) ) )
    if ( g->gc.state == GCSpropagate || g->gc.state == GCSatomic )
        gc_mark( g, gcV( tv ) );
    else
        TV2MARKED( tv ) =
            ( TV2MARKED( tv ) & ( uint8_t )~LJ_GC_COLORS ) | curwhite( g );
#undef TV2MARKED
}
