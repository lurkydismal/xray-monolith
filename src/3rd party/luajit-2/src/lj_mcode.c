...
static void* mcode_alloc_at( jit_State* J,
                             uintptr_t hint,
                             size_t sz,
                             DWORD prot ) {
...
static void* mcode_alloc_at( jit_State* J,
                             ...
                             int prot ) {
    void* p =
        mmap( ( void* )hint, sz, prot, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0 );
    if ( p == MAP_FAILED ) {
        if ( !hint )
            lj_trace_err( J, LJ_TRERR_MCODEAL );
        p = NULL;
...
static void* mcode_alloc( jit_State* J, size_t sz ) {
    ...
    */
    ...
    uintptr_t target =
        ( ( uintptr_t )( void* )lj_vm_exit_handler & 0xf0000000u ) +
        0x08000000u;
#else
    uintptr_t target =
        ( uintptr_t )( void* )lj_vm_exit_handler & ~( uintptr_t )0xffff;
#endif
    const uintptr_t range =
        ( 1u << ( LJ_TARGET_JUMPRANGE - 1 ) ) - ( 1u << 21 );
    ...
    lj_trace_err( J,
                  LJ_TRERR_MCODEAL ); /* Give up. OS probably ignores hints? */
    return NULL;
}
...
MCode* lj_mcode_patch( jit_State* J, MCode* ptr, int finish ) {
    ...
    if ( finish ) {
        if ( J->mcarea == ptr )
            mcode_protect( J, MCPROT_RUN );
        else if ( LJ_UNLIKELY( mcode_setprot( ptr, ( ( MCLink* )ptr )->size,
                                              MCPROT_RUN ) ) )
            mcode_protfail( J );
        return NULL;
    } else {
        ...
        for ( ;; ) {
            ...
            lua_assert( mc != NULL );
            if ( ptr >= mc &&
                 ptr < ( MCode* )( ( char* )mc + ( ( MCLink* )mc )->size ) ) {
                if ( LJ_UNLIKELY( mcode_setprot( mc, ( ( MCLink* )mc )->size,
                                                 MCPROT_GEN ) ) )
                    mcode_protfail( J );
                return mc;
            }
...
void lj_mcode_limiterr( jit_State* J, size_t need ) {
    ...
    sizemcode =
        ( sizemcode + LJ_PAGESIZE - 1 ) & ~( size_t )( LJ_PAGESIZE - 1 );
