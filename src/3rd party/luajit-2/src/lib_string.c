...
LJLIB_CF( string_dump ) {
    ...
    luaL_buffinit( L, &b );
    if ( !isluafunc( fn ) ||
         lj_bcwrite( L, funcproto( fn ), writer_buf, &b, strip ) )
...
static const unsigned char match_class_map[ 32 ] = { 0,
                                                     LJ_CHAR_ALPHA,
                                                     0,
                                                     LJ_CHAR_CNTRL,
                                                     LJ_CHAR_DIGIT,
    ...
    switch ( *p ) {
        ...
        case L_ESC:
            switch ( *( p + 1 ) ) {
                ...
                default:
                    if ( lj_char_isdigit( uchar(
                             *( p + 1 ) ) ) ) { /* capture results (%0-%9)? */
...
LJLIB_CF( string_gsub ) {
    ...
    MatchState ms;
    luaL_Buffer b;
    if ( !( tr == LUA_TNUMBER || tr == LUA_TSTRING || tr == LUA_TFUNCTION ||
            tr == LUA_TTABLE ) )
...
static GCstr* meta_tostring( lua_State* L, int arg ) {
    ...
    if ( tvisnumber( o ) ) {
        ...
        if ( tvisfunc( o ) && isffunc( funcV( o ) ) )
            ...
            lj_str_pushf( L, "%s: %p", lj_typename( o ),
                          lua_topointer( L, arg ) );
        L->top--;
        return strV( L->top );
    }
...
LUALIB_API int luaopen_string( lua_State* L ) {
    ...
    settabV( L, lj_tab_setstr( L, mt, mmname_str( g, MM_index ) ),
             tabV( L->top - 1 ) );
