...
static BCPos debug_framepc( lua_State* L, GCfunc* fn, cTValue* nextframe ) {
    ...
    if ( pos > pt->sizebc ) { /* Undo the effects of lj_trace_exit for JLOOP. */
        GCtrace* T = ( GCtrace* )( ( char* )( ins - 1 ) -
                                   offsetof( GCtrace, startins ) );
...
static TValue* debug_localname( lua_State* L,
                                const lua_Debug* ar,
                                const char** name,
                                BCReg slot1 ) {
    ...
    if ( ( int )slot1 < 0 ) { /* Negative slot number is for varargs. */
        if ( pc != NO_BCPOS ) {
            ...
            if ( ( pt->flags & PROTO_VARARG ) ) {
                ...
                if ( frame_isvarg(
                         frame ) ) { /* Vararg frame has been set up? (pc!=0) */
                    nextframe = frame;
                    frame = frame_prevd( frame );
                }
...
const char* lj_debug_slotname( GCproto* pt,
                               const BCIns* ip,
                               BCReg slot,
                               const char** name ) {
    const char* lname;
    ...
    while ( --ip > proto_bc( pt ) ) {
        ...
        if ( bcmode_a( op ) == BCMbase ) {
            ...
            switch ( bc_op( ins ) ) {
                ...
                case BC_GGET:
                    *name = strdata(
                        gco2str( proto_kgc( pt, ~( ptrdiff_t )bc_d( ins ) ) ) );
                    return "global";
                case BC_TGETS:
                    *name = strdata(
                        gco2str( proto_kgc( pt, ~( ptrdiff_t )bc_c( ins ) ) ) );
                    if ( ip > proto_bc( pt ) ) {
                        BCIns insp = ip[ -1 ];
                        if ( bc_op( insp ) == BC_MOV &&
                             bc_a( insp ) == ra + 1 &&
                             bc_d( insp ) == bc_b( ins ) )
                            return "method";
                    }
...
const char* lj_debug_funcname( lua_State* L,
                               TValue* frame,
                               const char** name ) {
    TValue* pframe;
    GCfunc* fn;
...
void lj_debug_addloc( lua_State* L,
                      const char* msg,
                      cTValue* frame,
                      cTValue* nextframe ) {
    if ( frame ) {
...
LUA_API const char* lua_getlocal( lua_State* L, const lua_Debug* ar, int n ) {
    ...
    if ( ar ) {
        ...
        name = debug_varname( funcproto( funcV( L->top - 1 ) ), 0,
                              ( BCReg )n - 1 );
    }
    return name;
}
...
int lj_debug_getinfo( lua_State* L, const char* what, lj_Debug* ar, int ext ) {
    ...
    for ( ; *what; what++ ) {
        if ( *what == 'S' ) {
            ...
            ar->namewhat =
                frame ? lj_debug_funcname( L, frame, &ar->name ) : NULL;
            if ( ar->namewhat == NULL ) {
                ar->namewhat = "";
                ar->name = NULL;
            }
            ...
            if ( lineinfo ) {
                ...
                for ( i = 0; i < szl; i++ ) {
                    BCLine line =
                        first +
                        ( sz == 1
                              ? ( BCLine )( ( const uint8_t* )lineinfo )[ i ]
                          : sz == 2
                              ? ( BCLine )( ( const uint16_t* )lineinfo )[ i ]
                              : ( BCLine )( (
                                    const uint32_t* )lineinfo )[ i ] );
