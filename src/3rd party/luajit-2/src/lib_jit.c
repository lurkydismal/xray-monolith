...
static int setjitmode( lua_State* L, int mode ) {
    int idx = 0;
    if ( L->base == L->top ||
         tvisnil( L->base ) ) { /* jit.on/off/flush([nil]) */
        ...
        if ( tvisfunc( L->base ) || tvisproto( L->base ) )
            idx = 1;
                      ...
                      L->base ) ) /* jit.on/off/flush(true, nil|true|false) */
            goto err;
        if ( L->base + 1 < L->top && tvisbool( L->base + 1 ) )
            mode |= boolV( L->base + 1 ) ? LUAJIT_MODE_ALLFUNC
...
static void flagbits_to_strings( lua_State* L,
                                 uint32_t flags,
                                 uint32_t base,
                                 const char* str ) {
...
LJLIB_CF( jit_attach ) {
    ...
    luaL_findtable( L, LUA_REGISTRYINDEX, LJ_VMEVENTS_REGKEY,
                    LJ_VMEVENTS_HSIZE );
...
LJLIB_PUSH( top - 5 )
LJLIB_SET( os ) LJLIB_PUSH( top - 4 ) LJLIB_SET( arch ) LJLIB_PUSH( top - 3 )
    LJLIB_SET( version_num ) LJLIB_PUSH( top - 2 ) LJLIB_SET( version )
#include "lj_libdef.h"
...
static void setintfield( lua_State* L,
                         GCtab* t,
                         const char* name,
                         int32_t val ) {
...
LJLIB_CF( jit_util_funcinfo ) {
    ...
    if ( pt ) {
        ...
        GCtab* t;
        lua_createtable( L, 0,
                         16 ); /* Increment hash size if fields are added. */
        t = tabV( L->top - 1 );
        ...
        GCtab* t;
        lua_createtable( L, 0,
                         4 ); /* Increment hash size if fields are added. */
        t = tabV( L->top - 1 );
        if ( !iscfunc( fn ) )
...
LJLIB_CF( jit_util_traceinfo ) {
    ...
    if ( T ) {
        GCtab* t;
        lua_createtable( L, 0,
                         8 ); /* Increment hash size if fields are added. */
        t = tabV( L->top - 1 );
...
LJLIB_CF( jit_util_traceir ) {
    ...
    if ( T && ref >= REF_BIAS && ref < T->nins ) {
        ...
        setintV( L->top++, ( int32_t )ir->op1 -
                               ( irm_op1( m ) == IRMref ? REF_BIAS : 0 ) );
        setintV( L->top++, ( int32_t )ir->op2 -
                               ( irm_op2( m ) == IRMref ? REF_BIAS : 0 ) );
...
LJLIB_CF( jit_util_tracesnap ) {
    ...
    if ( T && sn < T->nsnap ) {
        ...
        for ( n = 0; n < nent; n++ )
            setintV( lj_tab_setint( L, t, ( int32_t )( n + 2 ) ),
                     ( int32_t )map[ n ] );
        setintV( lj_tab_setint( L, t, ( int32_t )( nent + 2 ) ),
...
LJLIB_CF( jit_util_tracemc ) {
    ...
    if ( T && T->mcode != NULL ) {
        setstrV( L, L->top - 1,
                 lj_str_new( L, ( const char* )T->mcode, T->szmcode ) );
...
LJLIB_CF( jit_util_traceexitstub ) {
    ...
    if ( exitno < EXITSTUBS_PER_GROUP * LJ_MAX_EXITSTUBGR ) {
        setintptrV( L->top - 1,
                    ( intptr_t )( void* )exitstub_addr( J, exitno ) );
        return 1;
    }
#else
    if ( L->top > L->base + 1 ) { /* Don't throw for one-argument variant. */
        ...
        if ( T && T->mcode != NULL && exitno < maxexit ) {
            setintptrV( L->top - 1,
                        ( intptr_t )( void* )exitstub_trace_addr( T, exitno ) );
            return 1;
        }
    }
#endif
...
LJLIB_CF( jit_util_ircalladdr ) {
    ...
    if ( idx < IRCALL__MAX ) {
        setintptrV( L->top - 1,
                    ( intptr_t )( void* )lj_ir_callinfo[ idx ].func );
        return 1;
    }
    return 0;
}
...
LJLIB_CF( jit_opt_start ) {
    ...
    if ( nargs == 0 ) {
        ...
        for ( i = 1; i <= nargs; i++ ) {
            ...
            if ( !jitopt_level( J, str ) && !jitopt_flag( J, str ) &&
                 !jitopt_param( J, str ) )
...
#define JIT_F_SSE2 2
    ...
    if ( !( flags & JIT_F_MIPS32R2 ) ) {
        ...
        __asm__( "li $2, 1\n\t.long 0x00221042\n\tmove %0, $2"
                 ...
                 : "$2" );
        if ( x )
            flags |= JIT_F_MIPS32R2; /* Either 0x80000000 (R2) or 0 (R1). */
    }
#endif
#endif
#else
