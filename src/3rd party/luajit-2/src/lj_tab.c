...
#define hashgcref( t, r ) \
    hashlohi( ( t ), gcrefu( r ), gcrefu( r ) + HASH_BIAS )
...
GCtab* LJ_FASTCALL lj_tab_dup( lua_State* L, const GCtab* kt ) {
    ...
    if ( hmask > 0 ) {
        ...
        setmref( node->freetop,
                 ( Node* )( ( char* )noderef( knode->freetop ) + d ) );
        for ( i = 0; i <= hmask; i++ ) {
            Node* kn = &knode[ i ];
            Node* n = &node[ i ];
            ...
            n->key = kn->key;
            setmref( n->next,
                     next == NULL ? next : ( Node* )( ( char* )next + d ) );
        }
    }
    return t;
}
...
static void resizetab( lua_State* L,
                       GCtab* t,
                       uint32_t asize,
                       uint32_t hbits ) {
    ...
    if ( asize < oldasize ) { /* Array part shrinks? */
        ...
        if ( LJ_MAX_COLOSIZE != 0 && t->colo <= 0 )
            setmref( t->array,
                     lj_mem_realloc( L, array, oldasize * sizeof( TValue ),
                                     asize * sizeof( TValue ) ) );
    }
...
int lj_tab_next( lua_State* L, GCtab* t, TValue* key ) {
    ...
    for ( i -= t->asize; i <= t->hmask;
          i++ ) { /* Then traverse the hash keys. */
...
static MSize unbound_search( GCtab* t, MSize j ) {
    ...
    while ( ( tv = lj_tab_getint( t, ( int32_t )j ) ) && !tvisnil( tv ) ) {
        ...
        if ( j > ( MSize )( INT_MAX - 2 ) ) { /* overflow? */
            ...
            i = 1;
            while ( ( tv = lj_tab_getint( t, ( int32_t )i ) ) &&
                    !tvisnil( tv ) )
                i++;
            return i - 1;
        }
