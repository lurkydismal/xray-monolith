...
static Reg ra_scratchpair( ASMState* as, RegSet allow ) {
    ...
    if ( pick2 ) {
        ...
        if ( pick ) {
            ...
            if ( pick ) {
                r = ra_restore(
                    as, regcost_ref( as->cost[ rset_picktop( pick ) - 1 ] ) );
            } else {
...
static MCode* asm_exitstub_gen( ASMState* as, ExitNo group ) {
    ...
    *mxp++ =
        ARMI_STR | ARMI_LS_P | ARMI_LS_U | ARMF_D( RID_LR ) | ARMF_N( RID_SP );
    *mxp = ARMI_BL | ( ( ( ( MCode* )( void* )lj_vm_exit_handler - mxp ) - 2 ) &
                       0x00ffffffu );
    mxp++;
...
static uint32_t asm_fuseopm( ASMState* as,
                             ARMIns ai,
                             IRRef ref,
                             RegSet allow ) {
    IRIns* ir = IR( ref );
    ...
    if ( ra_noreg( ir->r ) && canfuse( as, ir ) ) {
        ...
        if ( ir->o == IR_ADD ) {
            ...
            if ( irref_isk( ir->op2 ) ) {
                ...
                uint32_t m =
                    asm_fuseopm( as, 0, ir->op2, rset_exclude( allow, rn ) );
                if ( ( ai & 0x04000000 ) )
...
static int asm_fusemadd( ASMState* as, IRIns* ir, ARMIns ai, ARMIns air ) {
    ...
    if ( lref != rref &&
        ...
        Reg right,
            left = ra_alloc2(
                as, irm, rset_exclude( rset_exclude( RSET_FPR, dest ), add ) );
        right = ( left >> 8 );
        left &= 255;
        ...
        if ( ref && irt_isfp( ir->t ) ) {
            ...
            if ( !LJ_ABI_SOFTFP && !( ci->flags & CCI_VARARG ) ) {
                if ( irt_isnum( ir->t ) ) {
                    ...
                    emit_dm( as, ARMI_VMOV_S, ( fprodd & 15 ),
                             ( src & 15 ) | 0x00400000 );
                    fprodd = 0;
                    continue;
                ...
                /* Workaround to protect argument GPRs from being used for
                 * remat. */
                as->freeset &=
                    ~RSET_RANGE( REGARG_FIRSTGPR, REGARG_LASTGPR + 1 );
                src = ra_alloc1( as, ref,
                                 RSET_FPR ); /* May alloc GPR to remat FPR. */
                as->freeset |=
                    ( of & RSET_RANGE( REGARG_FIRSTGPR, REGARG_LASTGPR + 1 ) );
                fprodd = 0;
                goto stackfp;
            }
            ...
            if ( gpr <= REGARG_LASTGPR ) {
                lua_assert( rset_test( as->freeset,
                                       gpr ) ); /* Must have been evicted. */
                if ( irt_isnum( ir->t ) ) {
                    lua_assert(
                        rset_test( as->freeset, gpr + 1 ) ); /* Ditto. */
...
static void asm_setupresult( ASMState* as, IRIns* ir, const CCallInfo* ci ) {
    ...
    if ( ra_used( ir ) ) {
        ...
        if ( !LJ_SOFTFP && irt_isfp( ir->t ) ) {
            if ( LJ_ABI_SOFTFP ||
                 ( ci->flags & ( CCI_CASTU64 | CCI_VARARG ) ) ) {
...
#endif
    {
        ...
        if ( st >= IRT_I8 && st <= IRT_U16 ) { /* Extend to 32 bit integer. */
            ...
            if ( ( as->flags & JIT_F_ARMV6 ) ) {
                ...
                emit_dm( as, ARMI_MOV | ARMF_SH( ARMSH_LSL, shift ), RID_TMP,
                         left );
            }
...
static void asm_tvptr( ASMState* as, Reg dest, IRRef ref ) {
    ...
    if ( irt_isnum( ir->t ) ) {
        if ( irref_isk( ref ) ) {
            ...
            emit_opk( as, ARMI_ADD, dest, RID_SP, ra_spill( as, ir ),
                      RSET_GPR );
#endif
        }
    } else {
...
static void asm_href( ASMState* as, IRIns* ir, IROp merge ) {
    ...
    if ( !irt_ispri( kt ) ) {
        ...
        emit_lso( as, ARMI_LDR, tmp, dest,
                  ( int32_t )offsetof( Node, key.it ) );
    }
    *l_loop =
        ARMF_CC( ARMI_B, CC_NE ) | ( ( as->mcp - l_loop - 2 ) & 0x00ffffffu );
    ...
    if ( khash == 0 ) {
        ...
        if ( irt_isstr(
                 kt ) ) { /* Fetch of str->hash is cheaper than ra_allock. */
            emit_dnm( as, ARMI_AND, tmp, tmp + 1, RID_TMP );
            emit_lso( as, ARMI_LDR, dest, tab,
                      ( int32_t )offsetof( GCtab, node ) );
            emit_lso( as, ARMI_LDR, tmp + 1, key,
                      ( int32_t )offsetof( GCstr, hash ) );
            emit_lso( as, ARMI_LDR, RID_TMP, tab,
            ...
            emit_dnm( as, ARMI_SUB | ARMF_SH( ARMSH_ROR, 32 - HASH_ROT3 ), tmp,
                      tmp, tmp + 1 );
            emit_lso( as, ARMI_LDR, dest, tab,
                      ( int32_t )offsetof( GCtab, node ) );
            emit_dnm(
                as,
                ARMI_EOR | ARMF_SH( ARMSH_ROR,
                                    32 - ( ( HASH_ROT2 + HASH_ROT1 ) & 31 ) ),
                tmp, tmp + 1, tmp );
            emit_lso( as, ARMI_LDR, RID_TMP, tab,
                      ( int32_t )offsetof( GCtab, hmask ) );
            emit_dnm( as, ARMI_SUB | ARMF_SH( ARMSH_ROR, 32 - HASH_ROT1 ),
                      tmp + 1, tmp + 1, tmp );
            if ( ra_hasreg( keynumhi ) ) {
                emit_dnm( as, ARMI_EOR, tmp + 1, tmp, key );
                emit_dnm( as, ARMI_ORR | ARMI_S, RID_TMP, tmp,
                          key ); /* Test for +-0.0. */
...
static void asm_hrefk( ASMState* as, IRIns* ir ) {
    ...
    Reg dest = ( ra_used( ir ) || ofs > 4095 ) ? ra_dest( as, ir, RSET_GPR )
                                               : RID_NONE;
    ...
    if ( irt_isnum( irkey->t ) ) {
        ...
        emit_opk( as, ARMI_CMP, 0, key, ( int32_t )ir_knum( irkey )->u32.lo,
                  allow );
    } else {
        if ( ra_hasreg( key ) )
...
static void asm_uref( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ir->op1 ) ) {
        ...
        if ( ir->o == IR_UREFC ) {
            ...
            emit_lso( as, ARMI_LDRB, RID_TMP, uv,
                      ( int32_t )offsetof( GCupval, closed ) );
        } else {
            emit_lso( as, ARMI_LDR, dest, uv,
...
static void asm_strref( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( ref ) ) {
        ...
        if ( ra_hasreg( irr->r ) ) {
            ...
            right = irr->r;
                    ...
                    irref_isk( irr->op2 ) &&
                    ( k = emit_isk12( ARMI_ADD, ( int32_t )sizeof( GCstr ) +
                                                    IR( irr->op2 )->i ) ) ) {
            m = k;
    ...
    emit_opk( as, ARMI_ADD, dest, r, sizeof( GCstr ) + IR( refk )->i,
...
static void asm_xload( ASMState* as, IRIns* ir ) {
    Reg dest = ra_dest(
        as, ir, ( !LJ_SOFTFP && irt_isfp( ir->t ) ) ? RSET_FPR : RSET_GPR );
...
static void asm_ahuvload( ASMState* as, IRIns* ir ) {
    ...
    if ( ra_used( ir ) ) {
        ...
        dest = ra_dest( as, ir,
                        ( !LJ_SOFTFP && t == IRT_NUM ) ? RSET_FPR : allow );
        rset_clear( allow, dest );
    }
...
static void asm_ahustore( ASMState* as, IRIns* ir ) {
    if ( ir->r != RID_SINK ) {
...
#endif
        {
            ...
            idx = asm_fuseahuref( as, ir->op1, &ofs,
                                  rset_exclude( allow, type ), 4096 );
            if ( ra_hasreg( src ) )
...
#if LJ_SOFTFP
    lua_assert(
        !( ir->op2 & IRSLOAD_CONVERT ) ); /* Handled by LJ_SOFTFP SPLIT. */
    if ( hiop && ra_used( ir + 1 ) ) {
        type = ra_dest( as, ir + 1, allow );
        rset_clear( allow, type );
    }
#else
    if ( ( ir->op2 & IRSLOAD_CONVERT ) && irt_isguard( ir->t ) &&
         t == IRT_INT ) {
...
static void asm_obar( ASMState* as, IRIns* ir ) {
    ...
    if ( ( l_end[ -1 ] >> 28 ) == CC_AL )
        ...
        emit_branch( as, ARMF_CC( ARMI_B, CC_EQ ), l_end );
    ra_allockreg( as, i32ptr( J2G( as->J ) ),
                  ra_releasetmp( as, ASMREF_TMP1 ) );
    obj = IR( ir->op1 )->r;
    ...
    emit_lso( as, ARMI_LDRB, RID_TMP, val,
...
static void asm_bitswap( ASMState* as, IRIns* ir ) {
    ...
    if ( ( as->flags & JIT_F_ARMV6 ) ) {
        ...
        if ( tmp2 == left )
            tmp2 = ra_scratch(
                as, rset_exclude( rset_exclude( RSET_GPR, dest ), left ) );
        ...
        emit_dnm( as, ARMI_EOR | ARMF_SH( ARMSH_ROR, 16 ), RID_TMP, left,
                  left );
    }
}
...
static void asm_intcomp( ASMState* as, IRIns* ir ) {
    ...
    if ( irref_isk( rref ) && IR( rref )->i == 0 ) {
        ...
        if ( cmpprev0 && irl->o == IR_BAND && !ra_used( irl ) ) {
            ...
            if ( !m2 )
                m2 = asm_fuseopm( as, 0, brref,
                                  rset_exclude( RSET_GPR, bleft ) );
            asm_guardcc( as, cc );
...
static void asm_hiop( ASMState* as, IRIns* ir ) {
    ...
    if ( LJ_UNLIKELY( !( as->flags & JIT_F_OPT_DCE ) ) )
        uselo = usehi = 1;
    if ( ( ir - 1 )->o <=
         IR_NE ) {    /* 64 bit integer or FP comparisons. ORDER IR. */
...
static void asm_stack_check( ASMState* as,
    ...
    if ( irp ) {
        if ( !ra_hasspill( irp->s ) ) {
            ...
            pbase = RID_RET;
            emit_lso( as, ARMI_LDR, RID_RET, RID_SP,
                      0 ); /* Restore temp. register. */
        }
    } else {
        pbase = RID_BASE;
    }
    emit_branch( as, ARMF_CC( ARMI_BL, CC_LS ),
                 exitstub_addr( as->J, exitno ) );
    ...
    if ( irp ) { /* Must not spill arbitrary registers in head of side trace. */
        ...
        if ( ra_hasspill( irp->s ) && !allow )
            emit_lso( as, ARMI_STR, RID_RET, RID_SP,
...
static void asm_stack_restore( ASMState* as, SnapShot* snap ) {
    ...
    for ( n = 0; n < nent; n++ ) {
        ...
        if ( irt_isnum( ir->t ) ) {
            ...
            Reg tmp;
            lua_assert(
                irref_isk( ref ) ); /* LJ_SOFTFP: must be a number constant. */
            ...
            if ( ( sn & ( SNAP_CONT | SNAP_FRAME ) ) ) {
                ...
                type = ra_alloc1( as, ref + 1,
                                  rset_exclude( RSET_GPRODD, RID_BASE ) );
#endif
            } else {
...
static void asm_head_root_base( ASMState* as ) {
    ...
    ir = IR( REF_BASE );
    if ( ra_hasreg( ir->r ) &&
         ( rset_test( as->modset, ir->r ) || irt_ismarked( ir->t ) ) )
        ra_spill( as, ir );
