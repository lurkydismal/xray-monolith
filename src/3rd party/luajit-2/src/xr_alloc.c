...
// Судя по комментарию к CALL_MUNMAP, луаджит может объединять выделенные ему
// чанки и освобождать их как один. Надеюсь он не слепит вместе чанки из разных
// пулов
...
// Находит подряд идущие свободные чанки количеством size, начиная с первого
// свободного Возвращает указатель на группу из heapMap или NULL, если найти не
// удалось
char* find_free( int size ) {
