...
static AliasRet aa_escape( jit_State* J, IRIns* ir, IRIns* stop ) {
    ...
    for ( ir++; ir < stop; ir++ )
        if ( ir->op2 == ref && ( ir->o == IR_ASTORE || ir->o == IR_HSTORE ||
...
static AliasRet aa_ahref( jit_State* J, IRIns* refa, IRIns* refb ) {
    ...
    if ( ka == kb ) {
        ...
        if ( ta == tb )
            ...
            return aa_table( J, ta,
                             tb ); /* Same key, possibly different table. */
    }
    ...
    if ( refa->o == IR_AREF ) {
        ...
        lua_assert( ( refa->o == IR_HREF || refa->o == IR_HREFK ||
                      refa->o == IR_NEWREF ) &&
                    ( refb->o == IR_HREF || refb->o == IR_HREFK ||
                      refb->o == IR_NEWREF ) );
...
static TRef fwd_ahload( jit_State* J, IRRef xref ) {
    ...
    {
        IRIns* ir =
            ( xr->o == IR_HREFK || xr->o == IR_AREF ) ? IR( xr->op1 ) : xr;
        IRRef tab = ir->op1;
        ir = IR( tab );
        if ( ir->o == IR_TNEW ||
             ( ir->o == IR_TDUP && irref_isk( xr->op2 ) ) ) {
            /* A NEWREF with a number key may end up pointing to the array part.
            ** But it's referenced from HSTORE and not found in the ASTORE
            * chain.
            ** For now simply consider this a conflict without forwarding
            * anything.
            */
            if ( xr->o == IR_AREF ) {
            ...
            } else if ( irt_isnum( fins->t ) ||
                        ( LJ_DUALNUM && irt_isint( fins->t ) ) ||
                        irt_isstr( fins->t ) ) {
                TValue keyv;
                cTValue* tv;
                IRIns* key = IR( xr->op2 );
    ...
    while ( ref > xref ) { /* Search for redundant or conflicting stores. */
        ...
        switch ( aa_ahref( J, xr, IR( store->op1 ) ) ) {
            ...
            case ALIAS_MAY: /* Store to MAYBE the same location. */
                if ( store->op2 !=
                     val ) /* Conflict if the value is different. */
                    goto doemit;
            ...
            case ALIAS_MUST:             /* Store to the same location. */
                ...
                if ( ref > J->chain[ IR_LOOP ] ) { /* Quick check to avoid
                                                      crossing LOOP. */
                    IRIns* ir;
                    /* Check for any intervening guards (includes conflicting
                     * loads). */
...
static AliasRet aa_uref( IRIns* refa, IRIns* refb ) {
    ...
    if ( refa->op1 == refb->op1 ) { /* Same function. */
        ...
        if ( ( ( refa->op2 ^ refb->op2 ) & 0xff ) )
            return ALIAS_NO; /* Upvalues with different hash values cannot
                                alias. */
        else
...
cselim:
    ...
    while ( ref > lim ) {
        IRIns* ir = IR( ref );
        if ( ir->op1 == uref || ( IR( ir->op1 )->op12 == IR( uref )->op12 &&
                                  IR( ir->op1 )->o == IR( uref )->o ) )
            return ref; /* Match for identical or equal UREFx (non-CSEable
                           UREFO). */
        ref = ir->prev;
    }
    return lj_ir_emit( J );
            ...
            case ALIAS_MUST:             /* Store to the same location. */
                ...
                if ( ref > J->chain[ IR_LOOP ] ) { /* Quick check to avoid
                    ...
                    *refp = store->prev;
                    ...
                    store->op1 = store->op2 = 0;
                    store->prev = 0;
                    if ( ref + 1 < J->cur.nins && store[ 1 ].o == IR_OBAR &&
                         store[ 1 ].op1 == xref ) {
                        IRRef1* bp = &J->chain[ IR_OBAR ];
                        IRIns* obar;
                        for ( obar = IR( *bp ); *bp > ref + 1;
                              obar = IR( *bp ) )
                            bp = &obar->prev;
                        /* Remove OBAR, too. */
                        *bp = obar->prev;
...
static AliasRet aa_xref( jit_State* J, IRIns* refa, IRIns* xa, IRIns* xb ) {
    ...
    if ( refa->o == IR_ADD && irref_isk( refa->op2 ) ) {
        ...
        basea = IR( refa->op1 );
        ofsa = ( LJ_64 && irk->o == IR_KINT64 )
                   ? ( ptrdiff_t )ir_k64( irk )->u64
                   : ( ptrdiff_t )irk->i;
    }
    if ( refb->o == IR_ADD && irref_isk( refb->op2 ) ) {
        IRIns* irk = IR( refb->op2 );
        baseb = IR( refb->op1 );
        ofsb = ( LJ_64 && irk->o == IR_KINT64 )
                   ? ( ptrdiff_t )ir_k64( irk )->u64
                   : ( ptrdiff_t )irk->i;
    }
    ...
    if ( basea == baseb ) {
        ...
        if ( ofsa == ofsb ) {
            if ( sza == szb && irt_isfp( xa->t ) == irt_isfp( xb->t ) )
                return ALIAS_MUST; /* Same-sized, same-kind. May need to
                                      convert. */
...
static IRRef reassoc_xref( jit_State* J, IRIns* ir ) {
    ...
    if ( ir->o == IR_ADD ) { /* Add of base + index. */
        ...
        if ( ir1 != ir && !( idxref = reassoc_trycse(
                                 J, ir1->o, idxref,
    ...
    while ( ref > lim ) {
        ...
        switch ( aa_xref( J, xr, fins, store ) ) {
            ...
            case ALIAS_MAY:
                ...
                goto cselim; /* Limit search for load. */
            case ALIAS_MUST:
                /* Emit conversion if the loaded type doesn't match the
                 * forwarded type. */
                if ( !irt_sametype( fins->t, IR( store->op2 )->t ) ) {
                    IRType dt = irt_type( fins->t ),
                           st = irt_type( IR( store->op2 )->t );
                    if ( dt == IRT_I8 ||
                         dt == IRT_I16 ) { /* Trunc + sign-extend. */
                        st = dt | IRCONV_SEXT;
                        dt = IRT_INT;
                    } else if ( dt == IRT_U8 ||
                                dt == IRT_U16 ) { /* Trunc + zero-extend. */
                        st = dt;
                        dt = IRT_INT;
                    }
        ...
        switch ( aa_xref( J, xr, fins, store ) ) {
            ...
            case ALIAS_MUST:
                ...
                if ( ref > J->chain[ IR_LOOP ] ) { /* Quick check to avoid
                                                      crossing LOOP. */
                    IRIns* ir;
                    /* Check for any intervening guards or any XLOADs (no AA
                     * performed). */
                    for ( ir = IR( J->cur.nins - 1 ); ir > store; ir-- )
...
int lj_opt_fwd_wasnonnil( jit_State* J, IROpT loadop, IRRef xref ) {
    ...
    while ( ref > xref ) {
        ...
        if ( store->op1 == xref ) { /* Same xREF. */
            ...
            /* Same key type MAY alias. Need ALOAD check due to multiple int
             * types. */
            if ( loadop == IR_ALOAD ||
                 irt_sametype( IR( skref )->t, IR( xkref )->t ) ) {
                if ( skref == xkref || !irref_isk( skref ) ||
                     !irref_isk( xkref ) )
                    return 0; /* A nil store with same const key or var key MAY
                                 alias. */
