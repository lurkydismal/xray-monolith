...
LJ_STATIC_ASSERT( ( int )BC_ISGE - ( int )BC_ISLT ==
                  ( int )OPR_GE - ( int )OPR_LT );
LJ_STATIC_ASSERT( ( int )BC_ISLE - ( int )BC_ISLT ==
                  ( int )OPR_LE - ( int )OPR_LT );
LJ_STATIC_ASSERT( ( int )BC_ISGT - ( int )BC_ISLT ==
                  ( int )OPR_GT - ( int )OPR_LT );
LJ_STATIC_ASSERT( ( int )BC_SUBVV - ( int )BC_ADDVV ==
                  ( int )OPR_SUB - ( int )OPR_ADD );
LJ_STATIC_ASSERT( ( int )BC_MULVV - ( int )BC_ADDVV ==
                  ( int )OPR_MUL - ( int )OPR_ADD );
LJ_STATIC_ASSERT( ( int )BC_DIVVV - ( int )BC_ADDVV ==
                  ( int )OPR_DIV - ( int )OPR_ADD );
LJ_STATIC_ASSERT( ( int )BC_MODVV - ( int )BC_ADDVV ==
                  ( int )OPR_MOD - ( int )OPR_ADD );
...
LJ_NORET static void err_limit( FuncState* fs,
                                uint32_t limit,
                                const char* what ) {
    if ( fs->linedefined == 0 )
...
static void jmp_append( FuncState* fs, BCPos* l1, BCPos l2 ) {
    if ( l2 == NO_JMP ) {
        ...
        *l1 = l2;
        ...
        BCPos list = *l1;
        BCPos next;
        while ( ( next = jmp_next( fs, list ) ) !=
                NO_JMP ) /* Find last element. */
            list = next;
...
static BCPos bcemit_INS( FuncState* fs, BCIns ins ) {
    ...
    if ( LJ_UNLIKELY( pc >= fs->bclim ) ) {
        ...
        checklimit( fs, ls->sizebcstack, LJ_MAX_BCINS,
                    "bytecode instructions" );
        lj_mem_growvec( fs->L, ls->bcstack, ls->sizebcstack, LJ_MAX_BCINS,
                        BCInsLine );
...
static void bcemit_nil( FuncState* fs, BCReg from, BCReg n ) {
    if ( fs->pc > fs->lasttarget ) { /* No jumps to current position? */
        ...
        switch (
            bc_op( *ip ) ) { /* Try to merge with the previous instruction. */
            case BC_KPRI:
                if ( bc_d( *ip ) != ~LJ_TNIL )
                    break;
                if ( from == pfrom ) {
                ...
                *ip =
                    BCINS_AD( BC_KNIL, from, from + n - 1 ); /* Replace KPRI. */
                return;
            case BC_KNIL:
                pto = bc_d( *ip );
                if ( pfrom <= from &&
                     from <= pto + 1 ) { /* Can we connect both ranges? */
                    if ( from + n - 1 > pto )
                        setbc_d(
                            ip, from + n -
...
static void expr_toreg( FuncState* fs, ExpDesc* e, BCReg reg ) {
    ...
    if ( e->k == VJMP )
        jmp_append( fs, &e->t,
                    e->u.s.info ); /* Add it to the true jump list. */
...
static void bcemit_store( FuncState* fs, ExpDesc* var, ExpDesc* e ) {
    ...
    if ( var->k == VLOCAL ) {
        ...
        if ( ( int32_t )rc < 0 ) {
            ...
            ins =
                BCINS_ABC( BC_TSETB, ra, var->u.s.info, rc - ( BCMAX_C + 1 ) );
        } else {
            /* Free late alloced key reg to avoid assert on free of value reg.
             */
            /* This can only happen when called from expr_table(). */
            lua_assert( e->k != VNONRELOC || ra < fs->nactvar || rc < ra ||
                        ( bcreg_free( fs, rc ), 1 ) );
...
static int foldarith( BinOpr opr, ExpDesc* e1, ExpDesc* e2 ) {
    ...
    if ( !expr_isnumk_nojump( e1 ) || !expr_isnumk_nojump( e2 ) )
        return 0;
    n = lj_vm_foldarith( expr_numberV( e1 ), expr_numberV( e2 ),
                         ( int )opr - OPR_ADD );
    setnumV( &o, n );
...
static void bcemit_arith( FuncState* fs,
                          BinOpr opr,
                          ExpDesc* e1,
                          ExpDesc* e2 ) {
    BCReg rb, rc, t;
    ...
    if ( op <= OPR_POW ) {
        ...
        *e1 = *e2;
        ...
        if ( e2->k == VRELOCABLE && bc_op( *bcptr( fs, e2 ) ) == BC_CAT ) {
            ...
            expr_free( fs, e1 );
            e1->u.s.info =
                bcemit_ABC( fs, BC_CAT, 0, e1->u.s.info, e2->u.s.info );
        }
        e1->k = VRELOCABLE;
    } else {
        lua_assert( op == OPR_NE || op == OPR_EQ || op == OPR_LT ||
...
static void bcemit_unop( FuncState* fs, BCOp op, ExpDesc* e ) {
    if ( op == BC_NOT ) {
        ...
        if ( op == BC_UNM &&
             !expr_hasjump( e ) ) { /* Constant-fold negations. */
#if LJ_HASFFI
            if ( e->k ==
                 VKCDATA ) { /* Fold in-place since cdata is not interned. */
...
#endif
                if ( expr_isnumk( e ) &&
                     !expr_numiszero( e ) ) { /* Avoid folding to -0. */
                TValue* o = expr_numtv( e );
                if ( tvisint( o ) ) {
                    int32_t k = intV( o );
                    if ( k == -k )
...
static void lex_match( LexState* ls,
                       LexToken what,
                       LexToken who,
                       BCLine line ) {
    if ( !lex_opt( ls, what ) ) {
...
static void var_new( LexState* ls, BCReg n, GCstr* name ) {
    ...
    if ( LJ_UNLIKELY( vtop >= ls->sizevstack ) ) {
        ...
        lj_mem_growvec( ls->L, ls->vstack, ls->sizevstack, LJ_MAX_VSTACK,
                        VarInfo );
    }
...
static MSize var_lookup_uv( FuncState* fs, MSize vidx, ExpDesc* e ) {
    ...
    fs->uvtmp[ n ] =
        ( uint16_t )( e->k == VLOCAL ? vidx : LJ_MAX_VSTACK + e->u.s.info );
    fs->nuv = n + 1;
    return n;
}
...
static MSize var_lookup_( FuncState* fs, GCstr* name, ExpDesc* e, int first ) {
    if ( fs ) {
        ...
        if ( ( int32_t )reg >= 0 ) { /* Local in this function? */
            ...
            MSize vidx =
                var_lookup_( fs->prev, name, e, 0 ); /* Var in outer func? */
    ...
    for ( ; v < ve; v++ ) {
        ...
        if ( name != NULL ) { /* Only consider remaining valid gotos/labels. */
            if ( gola_islabel( v ) ) {
                VarInfo* vg;
                setgcrefnull(
                    v->name ); /* Invalidate label that goes out of scope. */
                for ( vg = v + 1; vg < ve;
                      vg++ ) /* Resolve pending backward gotos. */
                ...
                if ( bl->prev ) { /* Propagate goto or break to outer scope. */
                    bl->prev->flags |=
                        name == NAME_BREAK ? FSCOPE_BREAK : FSCOPE_GOLA;
                    v->slot = bl->nactvar;
                    if ( ( bl->flags & FSCOPE_UPVAL ) )
                        gola_close( ls, v );
    ...
    if ( fs->flags & PROTO_FIXUP_RETURN ) {
        ...
        for ( pc = 1; pc < lastpc; pc++ ) {
            ...
            switch ( bc_op( ins ) ) {
                ...
                case BC_RET1:
                    offset =
                        bcemit_INS( fs, ins ); /* Copy original instruction. */
...
static GCproto* fs_finish( LexState* ls, BCLine line ) {
    ...
    *( uint32_t* )( ( char* )pt + ofsk - sizeof( GCRef ) * ( fs->nkgc + 1 ) ) =
        0;
...
static void expr_index( FuncState* fs, ExpDesc* t, ExpDesc* e ) {
    ...
    if ( expr_isnumk( e ) ) {
        ...
        if ( tvisint( expr_numtv( e ) ) ) {
            ...
            if ( checku8( k ) ) {
                t->u.s.aux =
                    BCMAX_C + 1 + ( uint32_t )k; /* 256..511: const byte key */
                return;
            }
        }
#else
    ...
    while ( ls->token != '}' ) {
        ...
        if ( expr_isk( &key ) && key.k != VKNIL &&
            ...
            if ( !t ) { /* Create template table on demand. */
                BCReg kidx;
                t = lj_tab_new( fs->L, needarr ? narr : 0,
                                hsize2hbits( nhash ) );
            ...
            if ( expr_isk_nojump(
                     &val ) ) { /* Add const key/value to template table. */
                expr_kvalue( v, &val );
            } else { /* Otherwise create dummy string key (avoids
                        lj_tab_newkey). */
                settabV( fs->L, v,
                         t ); /* Preserve key with table itself as value. */
...
static void expr_simple( LexState* ls, ExpDesc* v ) {
    switch ( ls->token ) {
        case TK_number:
            expr_init(
                v,
                ( LJ_HASFFI && tviscdata( &ls->tokenval ) ) ? VKCDATA : VKNUM,
                0 );
        ...
        case TK_dots: { /* Vararg. */
            ...
            base = fs->freereg - 1;
            expr_init( v, VCALL,
                       bcemit_ABC( fs, BC_VARG, base, 2, fs->numparams ) );
            v->u.s.aux = base;
            break;
        }
...
static void parse_assignment( LexState* ls, LHSVarList* lh, BCReg nvars ) {
    ...
    if ( lex_opt( ls, ',' ) ) { /* Collect LHS list and recurse upwards. */
        ...
        checklimit( ls->fs, ls->level + nvars, LJ_MAX_XLEVEL,
                    "variable names" );
        ...
        if ( nexps == nvars ) {
            if ( e.k == VCALL ) {
                if ( bc_op( *bcptr( ls->fs, &e ) ) ==
                     BC_VARG ) { /* Vararg assignment. */
                    ls->fs->freereg--;
                    e.k = VRELOCABLE;
                } else { /* Multiple call results. */
                    e.u.s.info =
                        e.u.s.aux; /* Base of call is not relocatable. */
                    e.k = VNONRELOC;
                }
            }
        ...
        if ( nret == 1 ) {        /* Return one result. */
            if ( e.k == VCALL ) { /* Check for tail call. */
                ...
                if ( bc_op( *ip ) == BC_VARG )
                    goto notailcall;
                fs->pc--;
                ins = BCINS_AD( bc_op( *ip ) - BC_CALL + BC_CALLT, bc_a( *ip ),
                                bc_c( *ip ) );
...
static void parse_repeat( LexState* ls, BCLine line ) {
    ...
    condexit =
        expr_cond( ls ); /* Parse condition (still inside inner scope). */
    if ( !( bl2.flags &
            FSCOPE_UPVAL ) ) { /* No upvalues? Just end inner scope. */
        fscope_end( fs );
...
static int predict_next( LexState* ls, FuncState* fs, BCPos pc ) {
    ...
    switch ( bc_op( ins ) ) {
        ...
        case BC_UGET:
            name =
                gco2str( gcref( ls->vstack[ fs->uvmap[ bc_d( ins ) ] ].name ) );
            break;
        case BC_GGET:
