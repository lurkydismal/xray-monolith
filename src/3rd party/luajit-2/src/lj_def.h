...
#define LJ_MAX_ASIZE \
    ( ( 1 << ( LJ_MAX_ABITS - 1 ) ) + 1 ) /* Max. array part size. */
...
#else
    uint32_t r;
#if __ARM_ARCH_6__ || __ARM_ARCH_6J__ || __ARM_ARCH_6T2__ || \
    __ARM_ARCH_6Z__ || __ARM_ARCH_6ZK__ || __ARM_ARCH_7__ || \
...
#define LJ_STATIC_ASSERT( cond )               \
    extern void LJ_ASSERT_NAME( __COUNTER__ )( \
        int STATIC_ASSERTION_FAILED[ ( cond ) ? 1 : -1 ] )
#else
#define LJ_STATIC_ASSERT( cond )            \
    extern void LJ_ASSERT_NAME( __LINE__ )( \
        int STATIC_ASSERTION_FAILED[ ( cond ) ? 1 : -1 ] )
#endif
