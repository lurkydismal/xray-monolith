...
#define emit_hsi( as, mi, rh, rs, i ) \
    emit_tsi( as, ( mi ), ( rh ) & 31, ( rs ), ( i ) )

static void emit_fgh( ASMState* as, MIPSIns mi, Reg rf, Reg rg, Reg rh ) {
    *--as->mcp =
        mi | MIPSF_F( rf & 31 ) | MIPSF_G( rg & 31 ) | MIPSF_H( rh & 31 );
}

#define emit_fg( as, mi, rf, rg ) emit_fgh( as, ( mi ), ( rf ), ( rg ), 0 )

static void emit_rotr( ASMState* as,
                       Reg dest,
                       Reg src,
                       Reg tmp,
                       uint32_t shift ) {
...
#define emit_getgl( as, r, field )     \
    emit_lsglptr( as, MIPSI_LW, ( r ), \
                  ( int32_t )offsetof( global_State, field ) )
#define emit_setgl( as, r, field )     \
    emit_lsglptr( as, MIPSI_SW, ( r ), \
                  ( int32_t )offsetof( global_State, field ) )
...
static void emit_branch( ASMState* as,
                         MIPSIns mi,
                         Reg rs,
                         Reg rt,
                         MCode* target ) {
...
static void emit_spload( ASMState* as, IRIns* ir, Reg r, int32_t ofs ) {
    if ( r < RID_MAX_GPR )
        ...
        emit_tsi( as, irt_isnum( ir->t ) ? MIPSI_LDC1 : MIPSI_LWC1, ( r & 31 ),
                  RID_SP, ofs );
}
...
static void emit_spstore( ASMState* as, IRIns* ir, Reg r, int32_t ofs ) {
    if ( r < RID_MAX_GPR )
        ...
        emit_tsi( as, irt_isnum( ir->t ) ? MIPSI_SDC1 : MIPSI_SWC1, ( r & 31 ),
                  RID_SP, ofs );
}
