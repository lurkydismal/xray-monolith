...
static void narrow_stripov_backprop( NarrowConv* nc, IRRef ref, int depth ) {
    ...
    if ( ir->o == IR_ADDOV || ir->o == IR_SUBOV ||
         ( ir->o == IR_MULOV &&
           ( nc->mode & IRCONV_CONVMASK ) == IRCONV_ANY ) ) {
        ...
        if ( bp ) {
            ...
            if ( nc->sp < nc->maxsp ) {
                ...
                if ( nc->sp < nc->maxsp ) {
                    *nc->sp++ = NARROWINS(
                        IRT( ir->o - IR_ADDOV + IR_ADD, IRT_INT ), ref );
                    return;
                }
            }
...
static int narrow_conv_backprop( NarrowConv* nc, IRRef ref, int depth ) {
    ...
    if ( ir->o == IR_CONV && ( ir->op2 & IRCONV_SRCMASK ) == IRT_INT ) {
        ...
        if ( ( nc->mode & IRCONV_CONVMASK ) == IRCONV_TOBIT ) {
            ...
            if ( n == ( lua_Number )
                          k64 ) { /* Only if const doesn't lose precision. */
                *nc->sp++ = NARROWINS( NARROW_INT, 0 );
                *nc->sp++ =
                    ( NarrowIns )k64; /* But always truncate to 32 bits. */
                return 0;
            }
        } else {
    ...
    while ( cref > ref ) {
        ...
        if ( cr->op1 == ref &&
             ( fins->o == IR_TOBIT ||
               ( ( cr->op2 & IRCONV_MODEMASK ) ==
                     ( nc->mode & IRCONV_MODEMASK ) &&
...
static IRRef narrow_conv_emit( jit_State* J, NarrowConv* nc ) {
    ...
    NarrowIns* next =
        nc->stack; /* List of instructions from backpropagation. */
    ...
    while ( next < last ) { /* Simple stack machine to process the ins. list. */
        ...
        if ( op == NARROW_REF ) {
            ...
            *sp++ = emitir_raw( convot, ref,
                                convop2 ); /* Raw emit avoids a loop. */
            ...
            *sp++ = nc->t == IRT_I64
            ...
            if ( ( mode & IRCONV_CONVMASK ) == IRCONV_INDEX ) {
                if ( next == last && irref_isk( narrow_ref( sp[ 0 ] ) ) &&
                     ( uint32_t )IR( narrow_ref( sp[ 0 ] ) )->i + 0x40000000u <
                         0x80000000u )
                    guardot = 0;
            ...
            /* Add to cache. */
            if ( narrow_ref( ref ) )
                narrow_bpc_set( J, narrow_ref( ref ), narrow_ref( sp[ -1 ] ),
                                mode );
        }
    }
    lua_assert( sp == nc->stack + 1 );
...
TRef LJ_FASTCALL lj_opt_narrow_convert( jit_State* J ) {
    if ( ( J->flags & JIT_F_OPT_NARROW ) ) {
        ...
        if ( fins->o == IR_TOBIT ) {
            nc.mode =
                IRCONV_TOBIT; /* Used only in the backpropagation cache. */
    ...
    if ( op >= IR_ADDOV && op <= lastop ) {
        ...
        if ( bp ) {
            ...
            tr = emitir( IRT( op - IR_ADDOV + IR_ADD,
                              ( ( mode & IRCONV_DSTMASK ) >> IRCONV_DSH ) ),
                         op1, op2 );
...
TRef LJ_FASTCALL lj_opt_narrow_toint( jit_State* J, TRef tr ) {
    ...
    */
    return narrow_stripov( J, tr, IR_MULOV,
                           ( IRT_INT << 5 ) | IRT_INT | IRCONV_TOBIT );
}
...
TRef LJ_FASTCALL lj_opt_narrow_tobit( jit_State* J, TRef tr ) {
    ...
    */
    return narrow_stripov( J, tr, IR_SUBOV,
                           ( IRT_INT << 5 ) | IRT_INT | IRCONV_TOBIT );
}
...
TRef LJ_FASTCALL lj_opt_narrow_cindex( jit_State* J, TRef tr ) {
    ...
    if ( tref_isnum( tr ) )
        return emitir(
            IRT( IR_CONV, IRT_INTP ), tr,
            ( IRT_INTP << 5 ) | IRT_NUM | IRCONV_TRUNC | IRCONV_ANY );
    /* Undefined overflow semantics allow stripping of ADDOV, SUBOV and MULOV.
     */
    return narrow_stripov(
        J, tr, IR_MULOV,
...
TRef lj_opt_narrow_arith( jit_State* J,
                          TRef rb,
                          TRef rc,
                          TValue* vb,
                          TValue* vc,
    ...
    if ( ( op >= IR_ADD && op <= ( LJ_DUALNUM ? IR_MUL : IR_SUB ) ) &&
         ...
         numisint( lj_vm_foldarith( numberVnum( vb ), numberVnum( vc ),
                                    ( int )op - ( int )IR_ADD ) ) )
        return emitir( IRTGI( ( int )op - ( int )IR_ADD + ( int )IR_ADDOV ), rb,
                       rc );
    if ( !tref_isnum( rb ) )
...
TRef lj_opt_narrow_pow( jit_State* J, TRef rb, TRef rc, TValue* vc ) {
    ...
    if ( tvisint( vc ) || numisint( numV( vc ) ) ) {
        int checkrange = 0;
        /* Split pow is faster for bigger exponents. But do this only for
         * (+k)^i. */
        if ( tref_isk( rb ) &&
             ( int32_t )ir_knum( IR( tref_ref( rb ) ) )->u32.hi >= 0 ) {
        ...
        if ( checkrange &&
             !tref_isk( rc ) ) { /* Range guard: -65536 <= i <= 65536 */
...
split_pow:
    ...
    if ( tref_isk( rc ) && ir_knum( IR( tref_ref( rc ) ) )->n == 0.5 )
        return emitir( IRTN( IR_FPMATH ), rb,
                       IRFPM_SQRT ); /* x ^ 0.5 ==> sqrt(x) */
