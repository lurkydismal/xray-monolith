...
#define emitir( ot, a, b ) \
    ( lj_ir_set( J, ( ot ), ( a ), ( b ) ), lj_opt_fold( J ) )

/* Emit raw IR without passing through optimizations. */
#define emitir_raw( ot, a, b ) \
    ( lj_ir_set( J, ( ot ), ( a ), ( b ) ), lj_ir_emit( J ) )
...
static void rec_check_slots( jit_State* J ) {
    ...
    lua_assert( J->baseslot == 1 ||
                ( J->slot[ J->baseslot - 1 ] & TREF_FRAME ) );
    ...
    for ( s = 0; s < nslots; s++ ) {
        ...
        if ( tr ) {
            ...
            if ( s == 0 ) {
                ...
                if ( tvisnumber( tv ) )
                    lua_assert(
                        tref_isnumber( tr ) ); /* Could be IRT_INT etc., too. */
                else
...
static TRef sloadt( jit_State* J, int32_t slot, IRType t, int mode ) {
    ...
    TRef ref =
        emitir_raw( IRT( IR_SLOAD, t ), ( int32_t )J->baseslot + slot, mode );
    J->base[ slot ] = ref;
    return ref;
}
...
#define getslot( J, s ) \
    ( J->base[ ( s ) ] ? J->base[ ( s ) ] : sload( J, ( int32_t )( s ) ) )
...
static void rec_stop( jit_State* J, TraceLink linktype, TraceNo lnk ) {
    ...
    if ( lnk == J->cur.traceno && J->framedepth + J->retdepth == 0 ) {
        if ( ( J->flags &
               JIT_F_OPT_LOOP ) ) /* Shall we try to create a loop? */
...
static TRef find_kinit( jit_State* J,
                        const BCIns* endpc,
                        BCReg slot,
                        IRType t ) {
    ...
    */
    ...
    for ( pc = endpc - 1; pc > startpc; pc-- ) {
        ...
        if ( bcmode_a( op ) == BCMbase && bc_a( ins ) <= slot ) {
            ...
            if ( op == BC_KSHORT ||
                 op == BC_KNUM ) { /* Found const. initializer. */
                ...
                if ( op == BC_KSHORT ) {
                    ...
                    return t == IRT_INT ? lj_ir_kint( J, k )
                                        : lj_ir_knum( J, ( lua_Number )k );
                } else {
...
static TRef fori_load( jit_State* J, BCReg slot, IRType t, int mode ) {
    int conv = ( tvisint( &J->L->base[ slot ] ) != ( t == IRT_INT ) )
                   ? IRSLOAD_CONVERT
                   : 0;
...
static TRef fori_arg( jit_State* J,
                      const BCIns* fori,
                      BCReg slot,
                      IRType t,
                      int mode ) {
...
static void rec_for_check( jit_State* J,
                           IRType t,
                           int dir,
                           TRef stop,
                           TRef step,
    ...
    if ( !tref_isk( step ) ) {
        ...
        TRef zero =
            ( t == IRT_INT ) ? lj_ir_kint( J, 0 ) : lj_ir_knum_zero( J );
        emitir( IRTG( dir ? IR_GE : IR_LT, t ), step, zero );
        /* Add hoistable overflow checks for a narrowed FORL index. */
        if ( init && t == IRT_INT ) {
            if ( tref_isk( stop ) ) {
                /* Constant stop: optimize check away or to a range check for
                 * step. */
                int32_t k = IR( tref_ref( stop ) )->i;
                if ( dir ) {
                    if ( k > 0 )
                        emitir( IRTGI( IR_LE ), step,
                                lj_ir_kint( J, ( int32_t )0x7fffffff - k ) );
                } else {
                    if ( k < 0 )
                        emitir( IRTGI( IR_GE ), step,
                                lj_ir_kint( J, ( int32_t )0x80000000 - k ) );
                }
            } else {
                /* Stop+step variable: need full overflow check. */
                TRef tr = emitir( IRTGI( IR_ADDOV ), step, stop );
                emitir( IRTI( IR_USE ), tr,
...
static void rec_for_loop( jit_State* J,
                          const BCIns* fori,
                          ScEvEntry* scev,
                          int init ) {
    BCReg ra = bc_a( *fori );
    ...
    IRType t = idx                      ? tref_type( idx )
               : ( init || LJ_DUALNUM ) ? lj_opt_narrow_forl( J, tv )
                                        : IRT_NUM;
    int mode =
        IRSLOAD_INHERIT + ( ( !LJ_DUALNUM || tvisint( tv ) == ( t == IRT_INT ) )
                                ? IRSLOAD_READONLY
                                : 0 );
    ...
    tc = ( LJ_DUALNUM && !( scev->start && irref_isk( scev->stop ) &&
                            irref_isk( scev->step ) &&
...
static LoopEvent rec_for( jit_State* J, const BCIns* fori, int isforl ) {
    ...
    if ( isforl ) { /* Handle FORL/JFORL opcodes. */
        ...
        BCReg i;
        lj_meta_for( J->L, tv );
        t = ( LJ_DUALNUM || tref_isint( tr[ FORL_IDX ] ) )
                ? lj_opt_narrow_forl( J, tv )
                : IRT_NUM;
        for ( i = FORL_IDX; i <= FORL_STEP; i++ ) {
            ...
            if ( t == IRT_INT ) {
                if ( !tref_isinteger( tr[ i ] ) )
                    tr[ i ] = emitir( IRTGI( IR_CONV ), tr[ i ],
                                      IRCONV_INT_NUM | IRCONV_CHECK );
            } else {
                if ( !tref_isnum( tr[ i ] ) )
                    tr[ i ] =
                        emitir( IRTN( IR_CONV ), tr[ i ], IRCONV_NUM_INT );
            }
        }
        tr[ FORL_EXT ] = tr[ FORL_IDX ];
        stop = tr[ FORL_STOP ];
        rec_for_check( J, t, rec_for_direction( &tv[ FORL_STEP ] ), stop,
                       tr[ FORL_STEP ], 1 );
    }
...
static LoopEvent rec_iterl( jit_State* J, const BCIns iterins ) {
    ...
    if ( !tref_isnil( J->base[ ra ] ) ) { /* Looping back? */
        J->base[ ra - 1 ] =
            J->base[ ra ]; /* Copy result of ITERC to control var. */
...
static void rec_loop_interp( jit_State* J, const BCIns* pc, LoopEvent ev ) {
    if ( J->parent == 0 ) {
        if ( pc == J->startpc && J->framedepth + J->retdepth == 0 ) {
            ...
            rec_stop( J, LJ_TRLINK_LOOP,
                      J->cur.traceno );    /* Looping root trace. */
            ...
            */
            if ( bc_j( *pc ) != -1 && !innerloopleft( J, pc ) )
                lj_trace_err(
                    J, LJ_TRERR_LINNER ); /* Root trace hit an inner loop. */
            if ( ( ev != LOOPEV_ENTERLO && J->loopref &&
                   J->cur.nins - J->loopref > 24 ) ||
                 --J->loopunroll < 0 )
...
static void rec_loop_jit( jit_State* J, TraceNo lnk, LoopEvent ev ) {
    if ( J->parent == 0 ) { /* Root trace hit an inner loop. */
        ...
        if ( J->pc == J->startpc && J->framedepth + J->retdepth == 0 )
            rec_stop( J, LJ_TRLINK_LOOP,
                      J->cur.traceno ); /* Form an extra loop. */
        else
...
static TRef rec_call_specialize( jit_State* J, GCfunc* fn, TRef tr ) {
    ...
    if ( isluafunc( fn ) ) {
        ...
        if ( pt->flags >=
             PROTO_CLC_POLY ) { /* Specialize to prototype instead. */
            TRef trpt = emitir( IRT( IR_FLOAD, IRT_P32 ), tr, IRFL_FUNC_PC );
            emitir( IRTG( IR_EQ, IRT_P32 ), trpt,
                    lj_ir_kptr( J, proto_bc( pt ) ) );
            ( void )lj_ir_kgc( J, obj2gco( pt ),
                               IRT_PROTO ); /* Prevent GC of proto. */
            return tr;
        }
    }
...
static void rec_call_setup( jit_State* J, BCReg func, ptrdiff_t nargs ) {
    ...
    for ( i = 0; i <= nargs; i++ )
        ( void )getslot(
            J, func + i ); /* Ensure func and all args have a reference. */
    if ( !tref_isfunc( fbase[ 0 ] ) ) { /* Resolve __call metamethod. */
        ix.tab = fbase[ 0 ];
        copyTV( J->L, &ix.tabv, functv );
        if ( !lj_record_mm_lookup( J, &ix, MM_call ) ||
             !tref_isfunc( ix.mobj ) )
    ...
    fbase[ 0 ] =
        TREF_FRAME | rec_call_specialize( J, funcV( functv ), fbase[ 0 ] );
...
void lj_record_tailcall( jit_State* J, BCReg func, ptrdiff_t nargs ) {
    ...
    /* Move func + args down. */
    memmove( &J->base[ -1 ], &J->base[ func ],
             sizeof( TRef ) * ( J->maxslot + 1 ) );
...
void lj_record_ret( jit_State* J, BCReg rbase, ptrdiff_t gotresults ) {
    ...
    for ( i = 0; i < gotresults; i++ )
        ( void )getslot( J,
                         rbase + i ); /* Ensure all results have a reference. */
    while (
        frame_ispcall( frame ) ) { /* Immediately resolve pcall() returns. */
    ...
    if ( frame_islua( frame ) ) { /* Return to Lua frame. */
        ...
        ptrdiff_t nresults =
            bc_b( callins ) ? ( ptrdiff_t )bc_b( callins ) - 1 : gotresults;
        ...
        if ( J->framedepth == 0 && J->pt && frame == J->L->base - 1 ) {
            if ( check_downrec_unroll( J, pt ) ) {
                ...
                lj_snap_purge( J );
                rec_stop( J, LJ_TRLINK_DOWNREC,
                          J->cur.traceno ); /* Down-recursion. */
                return;
            }
            lj_snap_add( J );
        }
        ...
        if ( J->framedepth >
            ...
            lj_trace_err(
                J, LJ_TRERR_NYIRETL ); /* No way to insert snapshot here. */
        ...
        lj_trace_err( J,
                      LJ_TRERR_NYIRETL ); /* NYI: handle return to C frame. */
    }
    lua_assert( J->baseslot >= 1 );
}
...
#if LJ_64
    trcont = lj_ir_kptr(
        J, ( void* )( ( int64_t )cont - ( int64_t )lj_vm_asm_begin ) );
#else
...
int lj_record_mm_lookup( jit_State* J, RecordIndex* ix, MMS mm ) {
    ...
    if ( tref_istab( ix->tab ) ) {
        ...
        if ( udtype != UDTYPE_USERDATA ) {
            ...
            if ( LJ_HASFFI && udtype == UDTYPE_FFI_CLIB ) {
                ...
                emitir( IRTG( IR_EQ, IRT_P32 ), ix->tab,
                        lj_ir_kptr( J, udataV( &ix->tabv ) ) );
            } else {
                /* Specialize to the type of userdata. */
                TRef tr = emitir( IRT( IR_FLOAD, IRT_U8 ), ix->tab,
                                  IRFL_UDATA_UDTYPE );
        ...
        immutable_mt:
            ...
            ix->mobj =
                lj_ir_kgc( J, gcV( mo ), tvisfunc( mo ) ? IRT_FUNC : IRT_TAB );
            ix->mtv = mt;
    ...
    emitir( IRTG( mt ? IR_NE : IR_EQ, IRT_TAB ), mix.tab,
            lj_ir_knull( J, IRT_TAB ) );
nocheck:
    if ( mt ) {
...
static TRef rec_mm_arith( jit_State* J, RecordIndex* ix, MMS mm ) {
    ...
    if ( !lj_record_mm_lookup( J, ix, mm ) ) { /* Lookup mm on 1st operand. */
        if ( mm != MM_unm ) {
            ...
            if ( lj_record_mm_lookup( J, ix,
                                      mm ) ) /* Lookup mm on 2nd operand. */
                goto ok;
        }
        lj_trace_err( J, LJ_TRERR_NOMM );
    }
...
static void rec_mm_equal( jit_State* J, RecordIndex* ix, int op ) {
    ...
    if ( lj_record_mm_lookup( J, ix, MM_eq ) ) { /* Lookup mm on 1st operand. */
        ...
        if ( tvistab( bv ) && tabref( tabV( bv )->metatable ) == ix->mtv ) {
            TRef mt2 =
                emitir( IRT( IR_FLOAD, IRT_TAB ), ix->key, IRFL_TAB_META );
            emitir( IRTG( IR_EQ, IRT_TAB ), mt2, ix->mt );
        } else if ( tvisudata( bv ) &&
                    tabref( udataV( bv )->metatable ) == ix->mtv ) {
            TRef mt2 =
                emitir( IRT( IR_FLOAD, IRT_TAB ), ix->key, IRFL_UDATA_META );
            emitir( IRTG( IR_EQ, IRT_TAB ), mt2, ix->mt );
        ...
        if ( lj_record_mm_lookup( J, ix,
            ...
            if ( tvistab( bv ) && tabref( tabV( bv )->metatable ) == ix->mtv ) {
                ...
                emitir( IRTG( IR_EQ, IRT_TAB ), mt2, ix->mt );
                        ...
                        tabref( udataV( bv )->metatable ) == ix->mtv ) {
                TRef mt2 = emitir( IRT( IR_FLOAD, IRT_TAB ), ix->key,
                                   IRFL_UDATA_META );
                emitir( IRTG( IR_EQ, IRT_TAB ), mt2, ix->mt );
            } else { /* Lookup metamethod on 2nd operand and compare both. */
                ix->tab = ix->key;
...
/* Record bounds-check. */
static void rec_idx_abc( jit_State* J,
                         TRef asizeref,
                         TRef ikey,
                         uint32_t asize ) {
    ...
    if ( ( J->flags & ( JIT_F_OPT_LOOP | JIT_F_OPT_ABC ) ) ==
        ...
        if ( ref == J->scev.idx ) {
            ...
            stop = numberVint(
                &( J->L->base - J->baseslot )[ ir->op1 + FORL_STOP ] );
            /* Runtime value for stop of loop is within bounds? */
            if ( ( uint64_t )stop + ofs < ( uint64_t )asize ) {
                /* Emit invariant bounds check for stop. */
                emitir( IRTG( IR_ABC, IRT_P32 ), asizeref,
                        ofs == 0
                            ? J->scev.stop
                            : emitir( IRTI( IR_ADD ), J->scev.stop, ofsref ) );
                /* Emit invariant bounds check for start, if not const or
                 * negative. */
...
static TRef rec_idx_key( jit_State* J, RecordIndex* ix ) {
    ...
    if ( tref_isnumber( key ) ) {
        ...
        if ( ( MSize )k < LJ_MAX_ASIZE ) { /* Potential array key? */
            ...
            if ( ( MSize )k < t->asize ) { /* Currently an array key? */
                ...
                arrayref =
                    emitir( IRT( IR_FLOAD, IRT_P32 ), ix->tab, IRFL_TAB_ARRAY );
                return emitir( IRT( IR_AREF, IRT_P32 ), arrayref, ikey );
            } else { /* Currently not in array (may be an array extension)? */
                emitir( IRTGI( IR_ULE ), asizeref,
                        ikey ); /* Inv. bounds check. */
                if ( k == 0 && tref_isk( key ) )
                    key = lj_ir_knum_zero(
                        J ); /* Canonicalize 0 or +-0.0 to +0.0. */
    ...
    if ( tref_isk( key ) ) {
        ...
        MSize hslot = ( MSize )( ( char* )ix->oldv -
                                 ( char* )&noderef( t->node )[ 0 ].val );
...
TRef lj_record_idx( jit_State* J, RecordIndex* ix ) {
    ...
    if ( tvisnil( &ix->keyv ) ||
         ( tvisnum( &ix->keyv ) && tvisnan( &ix->keyv ) ) ) {
    ...
    oldv = xrefop == IR_KKPTR ? ( cTValue* )ir_kptr( IR( tref_ref( xref ) ) )
                              : ix->oldv;
    ...
    if ( ix->val == 0 ) { /* Indexed load */
        ...
        if ( oldv == niltvg( J2G( J ) ) ) {
            emitir( IRTG( IR_EQ, IRT_P32 ), xref,
                    lj_ir_kkptr( J, niltvg( J2G( J ) ) ) );
            res = TREF_NIL;
        } else {
        ...
        if ( tvisnil( oldv ) ) { /* Previous value was nil? */
            ...
            if ( ix->idxchain && mt ) {
                cTValue* mo =
                    lj_tab_getstr( mt, mmname_str( J2G( J ), MM_newindex ) );
                hasmm = mo && !tvisnil( mo );
            }
            if ( hasmm )
                emitir( IRTG( loadop, IRT_NIL ), xref,
                        0 ); /* Guard for nil value. */
            else if ( xrefop == IR_HREF )
                emitir(
                    IRTG( oldv == niltvg( J2G( J ) ) ? IR_EQ : IR_NE, IRT_P32 ),
            ...
            if ( oldv == niltvg( J2G( J ) ) ) { /* Need to insert a new key. */
                TRef key = ix->key;
                if ( tref_isinteger(
                         key ) ) /* NEWREF needs a TValue as a key. */
                    key = emitir( IRTN( IR_CONV ), key, IRCONV_NUM_INT );
            ...
            /* Cannot derive that the previous value was non-nil, must do
             * checks. */
            if ( xrefop == IR_HREF ) /* Guard against store to niltv. */
                emitir( IRTG( IR_NE, IRT_P32 ), xref,
                        lj_ir_kkptr( J, niltvg( J2G( J ) ) ) );
            if ( ix->idxchain ) { /* Metamethod lookup required? */
                /* A check for NULL metatable is cheaper (hoistable) than a
                 * load. */
                if ( !mt ) {
                    TRef mtref = emitir( IRT( IR_FLOAD, IRT_TAB ), ix->tab,
                                         IRFL_TAB_META );
                    emitir( IRTG( IR_EQ, IRT_TAB ), mtref,
                            lj_ir_knull( J, IRT_TAB ) );
                } else {
                    IRType t = itype2irt( oldv );
                    emitir( IRTG( loadop, t ), xref,
        ...
        emitir( IRT( loadop + IRDELTA_L2S, tref_type( ix->val ) ), xref,
                ix->val );
...
static TRef rec_upvalue( jit_State* J, uint32_t uv, TRef val ) {
    ...
    IRRef uref;
    int needbarrier = 0;
    if ( rec_upvalue_constify(
             J, uvp ) ) { /* Try to constify immutable upvalue. */
        TRef tr, kfunc;
        lua_assert( val == 0 );
...
noconstify:
    ...
    if ( !uvp->closed ) {
        ...
        if ( uvval( uvp ) >= tvref( J->L->stack ) &&
            ...
            int32_t slot =
                ( int32_t )( uvval( uvp ) - ( J->L->base - J->baseslot ) );
            if ( slot >= 0 ) { /* Aliases an SSA slot? */
                slot -=
                    ( int32_t )
                        J->baseslot; /* Note: slot number may be negative! */
    ...
    if ( J->pc == J->startpc ) {
        if ( count + J->tailcalled > J->param[ JIT_P_recunroll ] ) {
            ...
            if ( J->framedepth + J->retdepth == 0 )
                rec_stop( J, LJ_TRLINK_TAILREC,
                          J->cur.traceno ); /* Tail-recursion. */
            else
                rec_stop( J, LJ_TRLINK_UPREC,
                          J->cur.traceno ); /* Up-recursion. */
        }
    } else {
        if ( count > J->param[ JIT_P_callunroll ] ) {
            if ( lnk ) {                  /* Possible tail- or up-recursion. */
                lj_trace_flush( J, lnk ); /* Flush trace that only returns. */
                /* Set a small, pseudo-random hotcount for a quick retry of
                 * JFUNC*. */
...
static void rec_func_jit( jit_State* J, TraceNo lnk ) {
    ...
    if ( J->pc == J->startpc && J->framedepth + J->retdepth == 0 )
        rec_stop( J, LJ_TRLINK_TAILREC,
                  J->cur.traceno ); /* Extra tail-recursion. */
    else
...
static void rec_varg( jit_State* J, BCReg dst, ptrdiff_t nresults ) {
    ...
    if ( J->framedepth > 0 ) { /* Simple case: varargs defined on-trace. */
        ...
        for ( i = 0; i < nresults; i++ )
            J->base[ dst + i ] =
                i < nvararg ? getslot( J, i - nvararg - 1 ) : TREF_NIL;
    } else { /* Unknown number of varargs passed to trace. */
        TRef fr =
            emitir( IRTI( IR_SLOAD ), 0, IRSLOAD_READONLY | IRSLOAD_FRAME );
        ...
        if ( nresults >= 0 ) { /* Known fixed number of results. */
            ...
            if ( nvararg > 0 ) {
                ...
                TRef vbase;
                if ( nvararg >= nresults )
                    emitir( IRTGI( IR_GE ), fr,
                            lj_ir_kint( J, frofs + 8 * ( int32_t )nresults ) );
                else
                    emitir( IRTGI( IR_EQ ), fr,
                            lj_ir_kint( J, frame_ftsz( J->L->base - 1 ) ) );
                vbase = emitir( IRTI( IR_SUB ), REF_BASE, fr );
                vbase = emitir( IRT( IR_ADD, IRT_P32 ), vbase,
                                lj_ir_kint( J, frofs - 8 ) );
                for ( i = 0; i < nload; i++ ) {
                    IRType t = itype2irt( &J->L->base[ i - 1 - nvararg ] );
                    TRef aref = emitir( IRT( IR_AREF, IRT_P32 ), vbase,
            ...
            TRef tr = TREF_NIL;
            ptrdiff_t idx =
                lj_ffrecord_select_mode( J, tridx, &J->L->base[ dst - 1 ] );
            if ( idx < 0 )
                goto nyivarg;
            if ( idx != 0 && !tref_isinteger( tridx ) )
                tridx = emitir( IRTGI( IR_CONV ), tridx,
                                IRCONV_INT_NUM | IRCONV_INDEX );
            if ( idx != 0 && tref_isk( tridx ) ) {
                emitir( IRTGI( idx <= nvararg ? IR_GE : IR_LT ), fr,
                        lj_ir_kint( J, frofs + 8 * ( int32_t )idx ) );
                frofs -= 8;                /* Bias for 1-based index. */
            } else if ( idx <= nvararg ) { /* Compute size. */
                TRef tmp =
                    emitir( IRTI( IR_ADD ), fr, lj_ir_kint( J, -frofs ) );
                if ( numparams )
                    emitir( IRTGI( IR_GE ), tmp, lj_ir_kint( J, 0 ) );
                tr = emitir( IRTI( IR_BSHR ), tmp, lj_ir_kint( J, 3 ) );
                if ( idx != 0 ) {
                    tridx =
                        emitir( IRTI( IR_ADD ), tridx, lj_ir_kint( J, -1 ) );
                ...
                if ( idx != 0 ) {
                    TRef tmp2 =
                        emitir( IRTI( IR_BSHL ), tridx, lj_ir_kint( J, 3 ) );
    ...
    if ( LJ_UNLIKELY( J->postproc != LJ_POST_NONE ) ) {
        switch ( J->postproc ) {
            case LJ_POST_FIXCOMP: /* Fixup comparison. */
                ...
                rec_comp_fixup( J, pc,
                                ( !tvistruecond( &J2G( J )->tmptv2 ) ^
                                  ( bc_op( *pc ) & 1 ) ) );
                /* fallthrough */
            case LJ_POST_FIXGUARD:     /* Fixup and emit pending guard. */
            case LJ_POST_FIXGUARDSNAP: /* Fixup and emit pending guard and
                                          snapshot. */
                if ( !tvistruecond( &J2G( J )->tmptv2 ) ) {
                    J->fold.ins.o ^= 1; /* Flip guard to opposite. */
                    if ( J->postproc == LJ_POST_FIXGUARDSNAP ) {
                        SnapShot* snap = &J->cur.snap[ J->cur.nsnap - 1 ];
                        J->cur.snapmap[ snap->mapofs + snap->nent -
                                        1 ]--; /* False -> true. */
                    }
                }
            ...
            case LJ_POST_FIXBOOL:
                if ( !tvistruecond( &J2G( J )->tmptv2 ) ) {
                    ...
                    TValue* tv = J->L->base;
                    for ( s = 0; s < J->maxslot;
                          s++ ) /* Fixup stack slot (if any). */
                        if ( J->base[ s ] == TREF_TRUE &&
                             tvisfalse( &tv[ s ] ) ) {
                            J->base[ s ] = TREF_FALSE;
                            break;
                        }
            ...
            case LJ_POST_FIXCONST: {
                ...
                TValue* tv = J->L->base;
                for ( s = 0; s < J->maxslot;
                      s++ ) /* Constify stack slots (if any). */
                    if ( J->base[ s ] == TREF_NIL && !tvisnil( &tv[ s ] ) )
                        J->base[ s ] = lj_record_constify( J, &tv[ s ] );
            } break;
            case LJ_POST_FFRETRY: /* Suppress recording of retried fast
                                     function. */
...
#define rcv ( &ix.keyv )
    ...
    switch ( bcmode_c( op ) ) {
        ...
        case BCMnum: {
            ...
            copyTV( J->L, rcv, tv );
            ix.key = rc = tvisint( tv ) ? lj_ir_kint( J, intV( tv ) )
    ...
    switch ( op ) {
        ...
        case BC_ISGT:
            ...
            if ( tref_iscdata( ra ) || tref_iscdata( rc ) ) {
                rec_mm_comp_cdata( J, &ix, op,
                                   ( ( int )op & 2 ) ? MM_le : MM_lt );
                break;
            }
#endif
            /* Emit nothing for two numeric or string consts. */
            if ( !( tref_isk2( ra, rc ) && tref_isnumber_str( ra ) &&
                    tref_isnumber_str( rc ) ) ) {
                IRType ta = tref_isinteger( ra ) ? IRT_INT : tref_type( ra );
                IRType tc = tref_isinteger( rc ) ? IRT_INT : tref_type( rc );
                int irop;
                if ( ta != tc ) {
                    /* Widen mixed number/int comparisons to number/number
                     * comparison. */
                    if ( ta == IRT_INT && tc == IRT_NUM ) {
                        ...
                        ta =
                            IRT_NIL; /* Force metamethod for different types. */
                    } else if ( !( ( ta == IRT_FALSE || ta == IRT_TRUE ) &&
                                   ( tc == IRT_FALSE || tc == IRT_TRUE ) ) ) {
                        break; /* Interpreter will throw for two different
                                  types. */
                    }
                }
                rec_comp_prep( J );
                ...
                if ( ta == IRT_NUM ) {
                    ...
                    if ( !lj_ir_numcmp( numberVnum( rav ), numberVnum( rcv ),
                                        ( IROp )irop ) )
                        irop ^= 5;
                } else if ( ta == IRT_INT ) {
                    if ( !lj_ir_numcmp( numberVnum( rav ), numberVnum( rcv ),
                    ...
                    if ( !lj_ir_strcmp( strV( rav ), strV( rcv ),
                                        ( IROp )irop ) )
                        irop ^= 1;
        ...
        case BC_KCDATA:
            rc =
                lj_ir_kgc( J, proto_kgc( J->pt, ~( ptrdiff_t )rc ), IRT_CDATA );
            break;
#endif
        ...
        case BC_GSET:
            ...
            ix.tab = emitir( IRT( IR_FLOAD, IRT_TAB ), getcurrf( J ),
                             IRFL_FUNC_ENV );
        ...
        case BC_TDUP:
            rc = emitir( IRTG( IR_TDUP, IRT_TAB ),
                         lj_ir_ktab( J, gco2tab( proto_kgc(
                                            J->pt, ~( ptrdiff_t )rc ) ) ),
                         0 );
            break;
        ...
        case BC_JFORI:
            ...
            if ( rec_for( J, pc, 0 ) !=
                 LOOPEV_LEAVE ) /* Link to existing loop. */
                rec_stop( J, LJ_TRLINK_ROOT,
                          bc_d( pc[ ( ptrdiff_t )rc - BCBIAS_J ] ) );
            /* Continue tracing if the loop is not entered. */
            break;

        case BC_FORL:
            rec_loop_interp(
                J, pc, rec_for( J, pc + ( ( ptrdiff_t )rc - BCBIAS_J ), 1 ) );
            break;
        case BC_ITERL:
        ...
        case BC_JFORL:
            rec_loop_jit(
                J, rc,
                rec_for( J, pc + bc_j( traceref( J, rc )->startins ), 1 ) );
            break;
        case BC_JITERL:
        ...
        case BC_JFUNCV:
            lua_assert(
                0 ); /* Cannot happen. No hotcall counting for varag funcs. */
            break;
...
static const BCIns* rec_setup_root( jit_State* J ) {
    ...
    switch ( bc_op( ins ) ) {
        ...
        case BC_ITERL:
            ...
            break;
        case BC_LOOP:
            /* Only check BC range for real loops, but not for "repeat until
             * true". */
            pcj = pc + bc_j( ins );
            ins = *pcj;
...
void lj_record_setup( jit_State* J ) {
    ...
    sidecheck:
        if ( traceref( J, J->cur.root )->nchild >= J->param[ JIT_P_maxside ] ||
             T->snap[ J->exitno ].count >=
                 J->param[ JIT_P_hotexit ] + J->param[ JIT_P_tryside ] ) {
        ...
        ** at the start! So snapshot #0 needs to point to the *next*
        * instruction.
        */
        lj_snap_add( J );
    ...
    */
    {
        TRef tr =
            emitir( IRT( IR_XLOAD, IRT_U8 ),
                    lj_ir_kptr( J, &J2G( J )->hookmask ), IRXLOAD_VOLATILE );
        tr = emitir( IRTI( IR_BAND ), tr,
                     lj_ir_kint( J, ( LUA_MASKLINE | LUA_MASKCOUNT ) ) );
