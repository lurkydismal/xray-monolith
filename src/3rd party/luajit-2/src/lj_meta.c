...
int lj_meta_tailcall( lua_State* L, cTValue* tv ) {
    ...
    setframe_ftsz(
        top + 1, ( int )( ( char* )( top + 2 ) - ( char* )base ) + FRAME_CONT );
    L->base = L->top = top + 2;
    /*
    ** before:   [old_mo|PC]    [... ...]
    **                         ^base     ^top
...
static TValue* mmcall( lua_State* L,
                       ASMFunction cont,
                       cTValue* mo,
                       cTValue* a,
                       cTValue* b ) {
    ...
    for ( loop = 0; loop < LJ_MAX_IDXCHAIN; loop++ ) {
        ...
        if ( LJ_LIKELY( tvistab( o ) ) ) {
            ...
            if ( LJ_LIKELY( !tvisnil( tv ) ) ) {
                ...
                return ( TValue* )tv;
            } else if ( !( mo = lj_meta_fast( L, tabref( t->metatable ),
                                              MM_newindex ) ) ) {
                t->nomm = 0; /* Invalidate negative metamethod cache. */
                lj_gc_anybarriert( L, t );
                if ( tv != niltv( L ) )
...
TValue* lj_meta_arith( lua_State* L,
                       TValue* ra,
                       cTValue* rb,
                       cTValue* rc,
                       BCReg op ) {
    ...
    if ( ( b = str2num( rb, &tempb ) ) != NULL &&
        ...
        setnumV( ra,
                 lj_vm_foldarith( numV( b ), numV( c ), ( int )mm - MM_add ) );
        return NULL;
    } else {
...
TValue* lj_meta_equal( lua_State* L, GCobj* o1, GCobj* o2, int ne ) {
    ...
    if ( mo ) {
        ...
        if ( tabref( o1->gch.metatable ) != tabref( o2->gch.metatable ) ) {
            cTValue* mo2 =
                lj_meta_fast( L, tabref( o2->gch.metatable ), MM_eq );
...
TValue* lj_meta_comp( lua_State* L, cTValue* o1, cTValue* o2, int op ) {
    if ( LJ_HASFFI && ( tviscdata( o1 ) || tviscdata( o2 ) ) ) {
        ...
        if ( tvisstr( o1 ) && tvisstr( o2 ) ) {
            ...
            return (
                TValue* )( intptr_t )( ( ( op & 2 ) ? res <= 0 : res < 0 ) ^
                                       ( op & 1 ) );
        } else {
        trymt:
            while ( 1 ) {
...
#if LJ_52
                if ( tvisnil( mo ) &&
                     tvisnil( ( mo = lj_meta_lookup( L, o2, mm ) ) ) )
#else
