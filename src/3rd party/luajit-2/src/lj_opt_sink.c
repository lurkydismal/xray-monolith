...
static IRIns* sink_checkalloc( jit_State* J, IRIns* irs ) {
    ...
    if ( ir->o == IR_HREFK || ir->o == IR_AREF )
        ir = IR( ir->op1 );
    else if ( !( ir->o == IR_HREF || ir->o == IR_NEWREF || ir->o == IR_FREF ||
                 ir->o == IR_ADD ) )
...
static int sink_checkphi( jit_State* J, IRIns* ira, IRRef ref ) {
    if ( ref >= REF_FIRST ) {
        IRIns* ir = IR( ref );
        if ( irt_isphi( ir->t ) ||
             ( ir->o == IR_CONV && ir->op2 == IRCONV_NUM_INT &&
...
static void sink_mark_ins( jit_State* J ) {
    ...
    for ( ir = irlast;; ir-- ) {
        switch ( ir->o ) {
            ...
            case IR_TBAR:
                irt_setmark(
                    IR( ir->op1 )->t ); /* Mark ref for remaining loads. */
                break;
            case IR_FLOAD:
                if ( irt_ismarked( ir->t ) || ir->op2 == IRFL_TAB_META )
                    irt_setmark(
                        IR( ir->op1 )
                            ->t ); /* Mark table for remaining loads. */
                break;
            case IR_ASTORE:
            case IR_HSTORE:
            case IR_FSTORE:
            case IR_XSTORE: {
                ...
                if ( !ira || ( irt_isphi( ira->t ) &&
                               !sink_checkphi( J, ira, ir->op2 ) ) )
            ...
            case IR_CALLS:
                irt_setmark(
                    IR( ir->op1 )->t ); /* Mark (potentially) stored values. */
                break;
            case IR_PHI: {
            ...
            default:
                if ( irt_ismarked( ir->t ) ||
                     irt_isguard( ir->t ) ) { /* Propagate mark. */
                    if ( ir->op1 >= REF_FIRST )
...
void lj_opt_sink( jit_State* J ) {
    const uint32_t need = ( JIT_F_OPT_SINK | JIT_F_OPT_FWD | JIT_F_OPT_DCE |
                            JIT_F_OPT_CSE | JIT_F_OPT_FOLD );
