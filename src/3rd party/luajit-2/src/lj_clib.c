...
static const char* clib_extname( lua_State* L, const char* name ) {
    if ( !strchr( name, '/' )
        ...
        if ( !( name[ 0 ] == CLIB_SOPREFIX[ 0 ] &&
                name[ 1 ] == CLIB_SOPREFIX[ 1 ] &&
...
static const char* clib_resolve_lds( lua_State* L, const char* name ) {
    ...
    if ( fp ) {
        ...
        if ( fgets( buf, sizeof( buf ), fp ) ) {
            if ( !strncmp( buf, "/* GNU ld script",
                           16 ) ) { /* ld script magic? */
                while (
                    fgets( buf, sizeof( buf ), fp ) ) { /* Check all lines. */
...
static void* clib_loadlib( lua_State* L, const char* name, int global ) {
    ...
    if ( !h ) {
        ...
        if ( *err == '/' && ( e = strchr( err, ':' ) ) &&
             ( name = clib_resolve_lds(
                   L, strdata( lj_str_new( L, err, e - err ) ) ) ) ) {
            h = dlopen( name,
                        RTLD_LAZY | ( global ? RTLD_GLOBAL : RTLD_LOCAL ) );
            if ( h )
                return h;
...
LJ_NORET LJ_NOINLINE static void clib_error( lua_State* L,
                                             ...
                                             const char* name ) {
    lj_err_callermsg( L,
                      lj_str_pushf( L, fmt, name, "no support for this OS" ) );
}
...
TValue* lj_clib_index( lua_State* L, CLibrary* cl, GCstr* name ) {
    ...
    if ( LJ_UNLIKELY( tvisnil( tv ) ) ) {
        ...
        if ( ctype_isconstval( ct->info ) ) {
            ...
            GCcdata* cd;
            lua_assert( ctype_isfunc( ct->info ) ||
            ...
            if ( !p && ctype_isfunc( ct->info ) ) {
                ...
                if ( cconv == CTCC_FASTCALL || cconv == CTCC_STDCALL ) {
                    ...
                    const char* symd = lj_str_pushf(
                        L, cconv == CTCC_FASTCALL ? "@%s@%d" : "_%s@%d", sym,
                        sz );
                    L->top--;
