...
/* Bad conversion. */
LJ_NORET static void cconv_err_conv( CTState* cts,
                                     CType* d,
                                     CType* s,
                                     CTInfo flags ) {
    const char* dst =
        strdata( lj_ctype_repr( cts->L, ctype_typeid( cts, d ), NULL ) );
    const char* src;
    if ( ( flags & CCF_FROMTV ) )
        src = lj_obj_typename[ 1 + ( ctype_isnum( s->info )     ? LUA_TNUMBER
                                     : ctype_isarray( s->info ) ? LUA_TSTRING
                                                                : LUA_TNIL ) ];
    else
        src = strdata( lj_ctype_repr( cts->L, ctype_typeid( cts, s ), NULL ) );
    if ( CCF_GETARG( flags ) )
        lj_err_argv( cts->L, CCF_GETARG( flags ), LJ_ERR_FFI_BADCONV, src,
                     dst );
    else
...
LJ_NORET static void cconv_err_convtv( CTState* cts,
                                       CType* d,
                                       TValue* o,
                                       CTInfo flags ) {
    const char* dst =
...
*/
void lj_cconv_ct_ct( CTState* cts,
                     CType* d,
                     CType* s,
                     uint8_t* dp,
                     uint8_t* sp,
    ...
    lua_assert( !ctype_isinteger( dinfo ) ||
                ( 1u << lj_fls( dsize ) ) == dsize );
    lua_assert( !ctype_isinteger( sinfo ) ||
                ( 1u << lj_fls( ssize ) ) == ssize );
    ...
    switch ( cconv_idx2( dinfo, sinfo ) ) {
...
#if LJ_LE
                uint8_t fill =
                    ( !( sinfo & CTF_UNSIGNED ) && ( sp[ ssize - 1 ] & 0x80 ) )
                        ? 0xff
                        : 0;
                memcpy( dp, sp, ssize );
...
#else
                uint8_t fill =
                    ( !( sinfo & CTF_UNSIGNED ) && ( sp[ 0 ] & 0x80 ) ) ? 0xff
                                                                        : 0;
        ...
        conv_I_F:
            ...
            /* The conversion must exactly match the semantics of JIT-compiled
             * code! */
        ...
        case CCX( C, C ):
            if ( dsize !=
                 ssize ) { /* Different types: convert re/im separately. */
                CType* dc = ctype_child( cts, d );
                CType* sc = ctype_child( cts, s );
                lj_cconv_ct_ct( cts, dc, sc, dp, sp, flags );
                lj_cconv_ct_ct( cts, dc, sc, dp + dc->size, sp + sc->size,
                                flags );
                return;
            }
        ...
        case CCX( V, V ):
            /* Copy same-sized vectors, even for different
        ...
        case CCX( P, F ):
             ...
             */
            dinfo =
                CTINFO( CT_NUM, ( LJ_64 && dsize == 8 ) ? 0 : CTF_UNSIGNED );
            goto conv_I_F;
        ...
        case CCX( A, A ):
            if ( ( flags & CCF_CAST ) || ( d->info & CTF_VLA ) ||
                 dsize != ssize || d->size == CTSIZE_INVALID ||
                 !lj_cconv_compatptr( cts, d, s, flags ) )
                goto err_conv;
            goto copyval;
...
int lj_cconv_tv_ct( CTState* cts,
                    CType* s,
                    CTypeID sid,
                    TValue* o,
                    uint8_t* sp ) {
        ...
        if ( !ctype_isbool( sinfo ) ) {
            ...
            if ( LJ_DUALNUM && ctype_isinteger( sinfo ) ) {
                ...
                lj_cconv_ct_ct( cts, ctype_get( cts, CTID_DOUBLE ), s,
                                ( uint8_t* )&o->n, sp, 0 );
                /* Numbers are NOT canonicalized here! Beware of uninitialized
                 * data. */
...
static void cconv_array_tab( CTState* cts,
                             CType* d,
                             uint8_t* dp,
                             GCtab* t,
                             CTInfo flags ) {
...
static void cconv_substruct_tab( CTState* cts,
                                 CType* d,
                                 uint8_t* dp,
                                 GCtab* t,
                                 int32_t* ip,
    ...
    while ( id ) {
        ...
        if ( ctype_isfield( df->info ) || ctype_isbitfield( df->info ) ) {
            ...
            tryname:
                tv =
                    ( TValue* )lj_tab_getstr( t, gco2str( gcref( df->name ) ) );
                if ( !tv || tvisnil( tv ) )
                    continue;
            }
            if ( ctype_isfield( df->info ) )
                lj_cconv_ct_tv( cts, ctype_rawchild( cts, df ), dp + df->size,
                                tv, flags );
            else
            ...
            cconv_substruct_tab( cts, ctype_rawchild( cts, df ), dp + df->size,
                                 t, ip, flags );
...
static void cconv_struct_tab( CTState* cts,
                              CType* d,
                              uint8_t* dp,
                              GCtab* t,
                              CTInfo flags ) {
...
void lj_cconv_ct_tv( CTState* cts,
                     CType* d,
                     uint8_t* dp,
                     TValue* o,
                     CTInfo flags ) {
    ...
    if ( LJ_LIKELY( tvisint( o ) ) ) {
        ...
        if ( ctype_isfunc( s->info ) ) {
            sid = lj_ctype_intern( cts, CTINFO( CT_PTR, CTALIGN_PTR | sid ),
                                   CTSIZE_PTR );
        } else {
...
static void cconv_array_init( CTState* cts,
                              CType* d,
                              CTSize sz,
                              uint8_t* dp,
                              TValue* o,
...
static void cconv_substruct_init( CTState* cts,
                                  CType* d,
                                  uint8_t* dp,
                                  TValue* o,
                                  MSize len,
    ...
    while ( id ) {
        ...
        if ( ctype_isfield( df->info ) || ctype_isbitfield( df->info ) ) {
            ...
            *ip = i + 1;
            ...
            cconv_substruct_init( cts, ctype_rawchild( cts, df ), dp + df->size,
                                  o, len, ip );
            if ( ( d->info & CTF_UNION ) )
                break;
        } /* Ignore all other entries in the chain. */
...
static void cconv_struct_init( CTState* cts,
                               CType* d,
                               CTSize sz,
                               uint8_t* dp,
                               TValue* o,
...
void lj_cconv_ct_init( CTState* cts,
                       CType* d,
                       CTSize sz,
                       uint8_t* dp,
                       TValue* o,
    ...
    if ( len == 0 )
        ...
        lj_cconv_ct_tv( cts, d, dp, o, 0 );
                  ...
                  d->info ) ) /* Also handles valarray init with len>1. */
