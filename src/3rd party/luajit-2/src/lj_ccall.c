...
#define CCALL_HANDLE_STRUCTRET                                         \
    cc->retref =                                                       \
        1; /* Return all structs by reference (in reg or on stack). */ \
    if ( ngpr < maxgpr )                                               \
        cc->gpr[ ngpr++ ] = ( GPRArg )dp;                              \
    else                                                               \
        cc->stack[ nsp++ ] = ( GPRArg )dp;
...
#define CCALL_HANDLE_COMPLEXRET2                                    \
    if ( ctr->size ==                                               \
         2 * sizeof( float ) ) { /* Copy complex float from FPR. */ \
...
#define CCALL_HANDLE_REGARG                                                   \
    ...
    if ( ngpr < maxgpr ) {                                                    \
        ...
        if ( ngpr + n > maxgpr ) {                                            \
            nsp +=                                                            \
                ngpr + n - maxgpr; /* Assumes contiguous gpr/stack fields. */ \
...
#define CCALL_HANDLE_REGARG                                                    \
    if ( n > 1 ) { /* Doesn't fit in a single GPR? */                          \
        lua_assert( n == 2 ||                                                  \
                    n == 4 ); /* int64_t, double or complex (float). */        \
        if ( n == 2 )                                                          \
            ngpr =                                                             \
                ( ngpr + 1u ) & ~1u; /* Only align 64 bit value to regpair. */ \
        else if ( ngpr + n > maxgpr )                                          \
            ngpr = maxgpr; /* Prevent reordering. */                           \
    }                                                                          \
    if ( ngpr + n <= maxgpr ) {                                                \
...
#define CCALL_HANDLE_COMPLEXRET2                                     \
    if ( ctr->size ==                                                \
         2 * sizeof( float ) ) { /* Copy complex float from FPRs. */ \
...
#define CCALL_HANDLE_REGARG                                               \
    if ( isfp && nfpr < CCALL_NARG_FPR && !( ct->info & CTF_VARARG ) ) {  \
        ...
        if ( ngpr < maxgpr ) {                                            \
            ...
            if ( ngpr + n > maxgpr ) {                                    \
                nsp += ngpr + n -                                         \
                       maxgpr; /* Assumes contiguous gpr/stack fields. */ \
                if ( nsp > CCALL_MAXSTACK )                               \
                    goto err_nyi; /* Too many arguments. */               \
                ngpr = maxgpr;                                            \
            } else {                                                      \
    ...
    while ( ct->sib ) {
        ...
        if ( ctype_isfield( ct->info ) ) {
            ...
            if ( ctype_isfp( sct->info ) ) {
                if ( sct->size == sz )
                    return ( sz >>
                             2 ); /* Return 1 for float or 2 for double. */
...
static int ccall_classify_struct( CTState* cts,
                                  CType* ct,
                                  int* rcl,
                                  CTSize ofs );
...
static int ccall_classify_struct( CTState* cts,
    ...
    while ( ct->sib ) {
        ...
        if ( ctype_isfield( ct->info ) )
            ...
            rcl[ ( fofs >= 8 ) ] |=
                CCALL_RCL_INT; /* NYI: unaligned bitfields? */
        else if ( ctype_isxattrib( ct->info, CTA_SUBTYPE ) )
...
static int ccall_struct_reg( CCallState* cc, GPRArg* dp, int* rcl ) {
    ...
    for ( i = 0; i < 2; i++ ) {
        ...
        if ( ( rcl[ i ] &
               CCALL_RCL_INT ) ) { /* Integer class takes precedence. */
...
static int ccall_struct_arg( CCallState* cc,
                             CTState* cts,
                             CType* d,
                             int* rcl,
                             TValue* o,
    ...
    if ( ccall_struct_reg( cc, dp,
                           rcl ) ) { /* Register overflow? Pass on stack. */
...
static void ccall_struct_ret( CCallState* cc,
                              int* rcl,
                              uint8_t* dp,
                              CTSize sz ) {
    GPRArg sp[ 2 ];
...
static unsigned int ccall_classify_struct( CTState* cts,
                                           CType* ct,
                                           CType* ctf ) {
    CTSize sz = ct->size;
...
CTypeID lj_ccall_ctid_vararg( CTState* cts, cTValue* o ) {
    if ( tvisnumber( o ) ) {
        ...
        if ( ctype_isrefarray( s->info ) ) {
            return lj_ctype_intern(
                cts, CTINFO( CT_PTR, CTALIGN_PTR | ctype_cid( s->info ) ),
                CTSIZE_PTR );
        } else if ( ctype_isstruct( s->info ) || ctype_isfunc( s->info ) ) {
            /* NYI: how to pass a struct by value in a vararg argument? */
            return lj_ctype_intern( cts, CTINFO( CT_PTR, CTALIGN_PTR | id ),
                                    CTSIZE_PTR );
...
static int ccall_set_args( lua_State* L,
                           CTState* cts,
                           CType* ct,
                           CCallState* cc ) {
    int gcsteps = 0;
    ...
    for ( o = L->base + 1, narg = 1; o < top; o++, narg++ ) {
        ...
        CCALL_HANDLE_REGARG /* Handle register arguments. */
            ...
            if ( CCALL_ALIGN_STACKARG && !rp &&
                 ( d->info & CTF_ALIGN ) > CTALIGN_PTR ) {
        ...
        if ( isfp == 2 && n == 2 &&
            ...
            cc->fpr[ nfpr - 1 ].d[ 0 ] =
                cc->fpr[ nfpr - 2 ].d[ 1 ]; /* Split complex double. */
            cc->fpr[ nfpr - 2 ].d[ 1 ] = 0;
        }
#else
        UNUSED( isfp );
...
static int ccall_get_results( lua_State* L,
                              CTState* cts,
                              CType* ct,
                              CCallState* cc,
                              int* ret ) {
    ...
    *ret = 1; /* One result. */
    if ( ctype_isstruct( ctr->info ) ) {
        ...
        if ( !cc->retref ) {
            void* dp =
                cdataptr( cdataV( L->top - 1 ) ); /* Use preallocated object. */
    ...
    if ( LJ_BE && ctype_isinteger_or_bool( ctr->info ) &&
         ctr->size < CTSIZE_PTR )
    ...
    lua_assert(
        !( ctype_isrefarray( ctr->info ) || ctype_isstruct( ctr->info ) ) );
...
int lj_ccall_func( lua_State* L, GCcdata* cd ) {
    ...
    if ( ctype_isfunc( ct->info ) ) {
        ...
        ct = ( CType* )( ( intptr_t )ct +
                         ( intptr_t )cts->tab ); /* May be reallocated. */
