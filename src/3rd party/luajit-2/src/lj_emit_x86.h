...
#define MODRM( mode, r1, r2 ) \
    ( ( MCode )( ( mode ) + ( ( ( r1 ) & 7 ) << 3 ) + ( ( r2 ) & 7 ) ) )
...
#define emit_u32( as, u ) \
    ( *( uint32_t* )( as->mcp - 4 ) = ( u ), as->mcp -= 4 )

#define emit_x87op( as, xo ) \
    ( *( uint16_t* )( as->mcp - 2 ) = ( uint16_t )( xo ), as->mcp -= 2 )

/* op */
static LJ_AINLINE MCode* emit_op( x86Op xo,
                                  Reg rr,
                                  Reg rb,
                                  Reg rx,
                                  MCode* p,
...
#if LJ_64
    {
        uint32_t rex = 0x40 + ( ( rr >> 1 ) & ( 4 + ( FORCE_REX >> 1 ) ) ) +
                       ( ( rx >> 2 ) & 2 ) + ( ( rb >> 3 ) & 1 );
        if ( rex != 0x40 ) {
            rex |= ( rr >> 16 );
            if ( n == -4 ) {
...
static void emit_rmrxo( ASMState* as,
                        x86Op xo,
                        Reg rr,
                        Reg rb,
                        Reg rx,
...
static void emit_gri( ASMState* as, x86Group xg, Reg rb, int32_t i ) {
    ...
    as->mcp =
        emit_opm( xo, XM_REG, ( Reg )( xg & 7 ) | ( rb & REX_64 ), rb, p, 0 );
}

/* op [base+ofs], i */
static void emit_gmroi( ASMState* as,
                        x86Group xg,
                        Reg rb,
                        int32_t ofs,
                        int32_t i ) {
    ...
    if ( rb == RID_MRM ) {
        ...
        if ( rb == RID_NONE ) {
            ...
            *--p = MODRM( XM_SCALE1, RID_ESP, RID_EBP );
            ...
            if ( as->mrm.idx != RID_NONE ) {
            mrmidx:
                as->mcp = emit_opmx( xo, mode, as->mrm.scale, rr, rb,
                                     as->mrm.idx, p );
                return;
            }
            if ( ( rb & 7 ) == RID_ESP )
...
#define XMM_MOVRR( as ) \
    ( ( as->flags & JIT_F_SPLIT_XMM ) ? XO_MOVSD : XO_MOVAPS )
#define XMM_MOVRM( as ) \
    ( ( as->flags & JIT_F_SPLIT_XMM ) ? XO_MOVLPD : XO_MOVSD )
...
static void emit_spload( ASMState* as, IRIns* ir, Reg r, int32_t ofs ) {
    if ( r < RID_MAX_GPR )
        ...
        emit_rmro( as, irt_isnum( ir->t ) ? XMM_MOVRM( as ) : XO_MOVSS, r,
                   RID_ESP, ofs );
}
...
static void emit_spstore( ASMState* as, IRIns* ir, Reg r, int32_t ofs ) {
    if ( r < RID_MAX_GPR )
        ...
        emit_rmro( as, irt_isnum( ir->t ) ? XO_MOVSDto : XO_MOVSSto, r, RID_ESP,
                   ofs );
}
