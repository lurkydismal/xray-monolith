...
static void LJ_FASTCALL gdbjit_ehframe( GDBJITctx* ctx ) {
    ...
    DSECT( CIE, DU32( 0 );                         /* Offset to CIE itself. */
           ...
           DB( 1 );
           DB( DW_EH_PE_textrel | DW_EH_PE_udata4 ); /* Augmentation data. */
...
#endif
        if ( ctx->spadjp !=
             ctx->spadj ) { /* Parent/interpreter stack frame size. */
                            DB( DW_CFA_def_cfa_offset );
                            DUV( ctx->spadjp );
                            DB( DW_CFA_advance_loc |
...
#define DLNE( op, s ) \
    ( DB( DW_LNS_extended_op ), DUV( 1 + ( s ) ), DB( ( op ) ) )
...
static void gdbjit_newentry( lua_State* L, GDBJITctx* ctx ) {
    ...
    MSize sz = ( MSize )( sizeof( GDBJITentryobj ) - sizeof( GDBJITobj ) +
                          ctx->objsize );
