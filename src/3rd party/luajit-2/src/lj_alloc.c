...
typedef long ( *PNTAVM )( HANDLE handle,
                          void** addr,
                          ULONG zbits,
                          size_t* size,
                          ULONG alloctype,
...
static LJ_AINLINE void* CALL_MMAP( size_t size ) {
    ...
    for ( ;; ) {
        void* p =
            mmap( ( void* )alloc_hint, size, MMAP_PROT, MMAP_FLAGS, -1, 0 );
...
static LJ_AINLINE void* CALL_MREMAP_( void* ptr,
                                      size_t osz,
                                      size_t nsz,
                                      int flags ) {
    int olderr = errno;
...
static size_t release_unused_segments( mstate m ) {
    ...
    /* Reset check counter */
    m->release_checks =
        nsegs > MAX_RELEASE_CHECK_RATE ? nsegs : MAX_RELEASE_CHECK_RATE;
    return released;
}
    ...
    if ( pad < MAX_REQUEST && is_initialized( m ) ) {
        ...
        if ( m->topsize > pad ) {
            ...
            if ( sp->size >= extra &&
                ...
                if ( ( CALL_MREMAP( sp->base, sp->size, newsize,
                                    CALL_MREMAP_NOMOVE ) != MFAIL ) ||
...
static LJ_NOINLINE void* lj_alloc_realloc( void* msp,
                                           void* ptr,
                                           size_t nsize ) {
