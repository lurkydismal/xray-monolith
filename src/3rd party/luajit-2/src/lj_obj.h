...
#define cdatav( cd ) \
    ( ( GCcdataVar* )( ( char* )( cd ) - sizeof( GCcdataVar ) ) )
...
#define round_nkgc( n ) \
    ( ( ( n ) + SCALE_NUM_GCO - 1 ) & ~( SCALE_NUM_GCO - 1 ) )
...
#define proto_kgc( pt, idx )                                  \
    check_exp( ( uintptr_t )( intptr_t )( idx ) >=            \
                   ( uintptr_t )-( intptr_t )( pt )->sizekgc, \
               gcref( mref( ( pt )->k, GCRef )[ ( idx ) ] ) )
#define proto_knumtv( pt, idx )                       \
    check_exp( ( uintptr_t )( idx ) < ( pt )->sizekn, \
               &mref( ( pt )->k, TValue )[ ( idx ) ] )
...
#define funcproto( fn )                                                    \
    check_exp( isluafunc( fn ), ( GCproto* )( mref( ( fn )->l.pc, char ) - \
                                              sizeof( GCproto ) ) )
#define sizeCfunc( n ) \
    ( sizeof( GCfuncC ) - sizeof( TValue ) + sizeof( TValue ) * ( n ) )
#define sizeLfunc( n ) \
    ( sizeof( GCfuncL ) - sizeof( GCRef ) + sizeof( GCRef ) * ( n ) )
...
#define MMDEF( _ )                                                           \
    ...
    _( newindex ) _( gc ) _( mode ) _( eq )                                  \
        _( len ) /* Only the above (fast) metamethods are negative cached    \
                    (max. 8). */                                             \
        _( lt ) _( le ) _( concat )                                          \
...
LJ_STATIC_ASSERT( offsetof( GChead, metatable ) ==
                  offsetof( GCtab, metatable ) );
LJ_STATIC_ASSERT( offsetof( GChead, metatable ) ==
                  offsetof( GCudata, metatable ) );
...
#if LJ_64
#define itypemap( o )                   \
    ( tvisnumber( o )    ? ~LJ_TNUMX    \
      : tvislightud( o ) ? ~LJ_TLIGHTUD \
                         : ~itype( o ) )
#else
...
#define checklightudptr( L, p )                                             \
    ( ( ( uint64_t )( p ) >> 47 ) ? ( lj_err_msg( L, LJ_ERR_BADLU ), NULL ) \
                                  : ( p ) )
#define setcont( o, f ) \
...
#define setminfV( o ) ( ( o )->u64 = U64x( fff00000, 00000000 ) )
                            ...
                            static LJ_AINLINE
    void setintV( TValue* o, int32_t i ) {
#if LJ_DUALNUM
    o->i = ( uint32_t )i;
