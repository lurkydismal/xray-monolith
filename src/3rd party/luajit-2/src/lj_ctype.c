...
#define CTTYINFODEF( id, sz, ct, info ) \
    CTINFO( ( ct ), ( ( ( sz ) & 0x3fu ) << 10 ) + ( info ) ),
#define CTTDINFODEF( name, id ) CTINFO( CT_TYPEDEF, CTID_##id ),
#define CTKWINFODEF( name, sz, kw ) \
    CTINFO( CT_KW, ( ( ( sz ) & 0x3fu ) << 10 ) + ( kw ) ),
...
CTypeID lj_ctype_getname( CTState* cts,
                          CType** ctp,
                          GCstr* name,
                          uint32_t tmask ) {
    ...
    *ctp =
        &cts->tab[ 0 ]; /* Simplify caller logic. ctype_get() would assert. */
    return 0;
}

/* Get a struct/union/enum/function field by name. */
CType* lj_ctype_getfieldq( CTState* cts,
                           CType* ct,
                           GCstr* name,
                           CTSize* ofs,
                           CTInfo* qual ) {
    ...
    if ( ctype_isstruct( ct->info ) ) {
        ...
        while ( fid ) {
            ...
            if ( ctype_type( ctf->info ) == CT_FIELD )
                arrid =
                    ctype_cid( ctf->info ); /* Remember last field of VLS. */
            fid = ctf->sib;
        }
...
cTValue* lj_ctype_meta( CTState* cts, CTypeID id, MMS mm ) {
    ...
    if ( tv && tvistab( tv ) &&
         ( tv = lj_tab_getstr( tabV( tv ), mmname_str( cts->g, mm ) ) ) &&
         !tvisnil( tv ) )
        return tv;
    return NULL;
}
...
#define ctype_preplit( ctr, str ) \
    ctype_prepstr( ( ctr ), "" str, sizeof( str ) - 1 )
...
/* Prepend named type. */
static void ctype_preptype( CTRepr* ctr,
                            CType* ct,
                            CTInfo qual,
                            const char* t ) {
    if ( gcref( ct->name ) ) {
    ...
    for ( ;; ) {
        ...
        switch ( ctype_type( info ) ) {
            ...
            case CT_VOID:
                ...
                return;
            case CT_STRUCT:
                ctype_preptype( ctr, ct, qual,
                                ( info & CTF_UNION ) ? "union" : "struct" );
                return;
            case CT_ENUM:
                if ( id == CTID_CTYPEID ) {
