...
typedef struct ASMState {
    RegCost
        cost[ RID_MAX ]; /* Reference and blended allocation cost for regs. */
...
#define opisfusableload( o )                                         \
    ( ( o ) == IR_ALOAD || ( o ) == IR_HLOAD || ( o ) == IR_ULOAD || \
      ( o ) == IR_FLOAD || ( o ) == IR_XLOAD || ( o ) == IR_SLOAD || \
      ( o ) == IR_VLOAD )
...
static LJ_NORET LJ_NOINLINE void asm_mclimit( ASMState* as ) {
    lj_mcode_limiterr( as->J,
                       ( size_t )( as->mctop - as->mcp + 4 * MCLIM_REDZONE ) );
}
...
static LJ_AINLINE void checkmclim( ASMState* as ) {
    ...
    if ( as->mcp + MCLIM_REDZONE < as->mcp_prev ) {
        ...
        fprintf( stderr, "RED ZONE OVERFLOW: %p IR %04d  %02d %04d %04d\n",
                 as->mcp, as->curins + 1 - REF_BIAS, ir->o, ir->op1 - REF_BIAS,
                 ir->op2 - REF_BIAS );
        lua_assert( 0 );
    }
#endif
...
static const char* const ra_regname[] = { GPRDEF( RIDNAME ) FPRDEF( RIDNAME )
                                              VRIDDEF( RIDNAME ) NULL };
#undef RIDNAME
...
static void ra_dprintf( ASMState* as, const char* fmt, ... ) {
    ...
    ra_dbg_mcp = NULL;
    p += sprintf( p, "%08x  \e[36m%04d ", ( uintptr_t )as->mcp,
                  as->curins - REF_BIAS );
    for ( ;; ) {
...
static Reg ra_rematk( ASMState* as, IRRef ref ) {
    ...
    if ( ir->o == IR_KNUM ) {
        ...
        lua_assert(
            irt_isnil( ir->t ) ); /* REF_NIL stores ASMREF_L register. */
        ...
        lua_assert( ir->o == IR_KINT || ir->o == IR_KGC || ir->o == IR_KPTR ||
                    ir->o == IR_KKPTR || ir->o == IR_KNULL );
...
static Reg ra_restore( ASMState* as, IRRef ref ) {
    if ( emit_canremat( ref ) ) {
        ...
        ra_free( as, r );
        if ( !rset_test( as->weakset,
                         r ) ) { /* Only restore non-weak references. */
            ra_modified( as, r );
...
#define MINCOST( name )                                 \
    if ( rset_test( RSET_ALL, RID_##name ) &&           \
         LJ_LIKELY( allow & RID2RSET( RID_##name ) ) && \
         as->cost[ RID_##name ] < cost )                \
...
static Reg ra_evict( ASMState* as, RegSet allow ) {
    ...
    if ( !irref_isk( ref ) && ( as->weakset & allow ) ) {
        ...
        if ( !rset_test( as->weakset, ir->r ) )
            ref = regcost_ref(
                as->cost[ rset_pickbot( ( as->weakset & allow ) ) ] );
    }
...
static Reg ra_allock( ASMState* as, int32_t k, RegSet allow ) {
    ...
    if ( pick ) {
        ...
        if ( ( pick & ~as->modset ) )
            pick &= ~as->modset;
        r = rset_pickbot(
            pick ); /* Reduce conflicts with inverse allocation. */
    } else {
        r = ra_evict( as, allow );
    }
...
static Reg ra_allocref( ASMState* as, IRRef ref, RegSet allow ) {
    ...
    if ( pick ) {
        ...
        if ( ra_hashint( ir->r ) ) {
            ...
            if ( rset_test( allow, r ) &&
                 emit_canremat( regcost_ref( as->cost[ r ] ) ) ) {
...
static void ra_rename( ASMState* as, Reg down, Reg up ) {
    ...
    lua_assert( !rset_test( as->freeset, down ) &&
                rset_test( as->freeset, up ) );
    ra_free( as, down ); /* 'down' is free ... */
    ra_modified( as, down );
    rset_clear( as->freeset, up ); /* ... and 'up' is now allocated. */
    ra_noweak( as, up );
    RA_DBGX(
        ( as, "rename    $f $r $r", regcost_ref( as->cost[ up ] ), down, up ) );
...
static Reg ra_dest( ASMState* as, IRIns* ir, RegSet allow ) {
    ...
    if ( ra_hasreg( dest ) ) {
        ...
        if ( ra_hashint( dest ) &&
             rset_test( ( as->freeset & allow ), ra_gethint( dest ) ) ) {
    ...
    if ( ra_noreg( left ) ) {
        if ( irref_isk( lref ) ) {
            if ( ir->o == IR_KNUM ) {
                ...
                /* FP remat needs a load except for +0. Still better than
                 * eviction. */
        ...
        left =
            ra_allocref( as, lref, dest < RID_MAX_GPR ? RSET_GPR : RSET_FPR );
    }
    ra_noweak( as, left );
...
static void ra_destpair( ASMState* as, IRIns* ir ) {
    ...
    if ( !rset_test( as->freeset, RID_RETLO ) && destlo != RID_RETLO &&
         desthi != RID_RETLO )
        ra_restore( as, regcost_ref( as->cost[ RID_RETLO ] ) );
    if ( !rset_test( as->freeset, RID_RETHI ) && destlo != RID_RETHI &&
         desthi != RID_RETHI )
    ...
    if ( !irref_isk( ref ) && ( !( ra_used( ir ) || ir->r == RID_SUNK ) ) ) {
            ...
            { /* Allocate stored values for TNEW, TDUP and CNEW. */
                ...
                for ( irs = IR( as->snapref - 1 ); irs > ir; irs-- )
                    if ( irs->r == RID_SINK && asm_sunk_store( as, ir, irs ) ) {
                        lua_assert(
                            irs->o == IR_ASTORE || irs->o == IR_HSTORE ||
                            irs->o == IR_FSTORE || irs->o == IR_XSTORE );
        ...
        nosink:
            ...
            if ( ( as->freeset & allow ) ||
                ...
                /* Get a weak register if we have a free one or can
                 * rematerialize. */
                Reg r =
...
static int asm_snap_checkrename( ASMState* as, IRRef ren ) {
    ...
    for ( n = 0; n < nent; n++ ) {
        ...
        IRRef ref = snap_ref( sn );
        if ( ref == ren ||
             ( LJ_SOFTFP && ( sn & SNAP_SOFTFPNUM ) && ++ref == ren ) ) {
            IRIns* ir = IR( ref );
...
static void asm_collectargs( ASMState* as,
                             IRIns* ir,
                             const CCallInfo* ci,
                             IRRef* args ) {
...
static void asm_tnew( ASMState* as, IRIns* ir ) {
    ...
    asm_gencall( as, ci, args );
    ra_allockreg( as, ir->op1 | ( ir->op2 << 24 ),
...
static void asm_phi_break( ASMState* as,
                           RegSet blocked,
                           RegSet blockedby,
                           RegSet allow ) {
    ...
    if ( candidates ) { /* If this register file has candidates. */
        ...
        */
        Reg down,
            up = ra_pick( as, ~blocked & allow ); /* Get a free register. */
...
static void asm_phi_shuffle( ASMState* as ) {
    ...
    for ( ;; ) {
        ...
        while ( phiset ) { /* Check all left PHI operand registers. */
            ...
            Reg left = irl->r;
            if ( r != left ) { /* Mismatch? */
                if ( !rset_test( as->freeset,
                                 r ) ) { /* PHI register blocked? */
                    IRRef ref = regcost_ref( as->cost[ r ] );
...
static void asm_phi_copyspill( ASMState* as ) {
    ...
    if ( ( need & 1 ) ) {                      /* Copy integer spill slots. */
        ...
        if ( !rset_test( as->freeset, r ) )
            emit_spstore( as, IR( regcost_ref( as->cost[ r ] ) ), r,
                          SPOFS_TMP );
#endif
    }
#if !LJ_SOFTFP
...
static void asm_phi_fixup( ASMState* as ) {
    ...
    while ( work ) {
        ...
        if ( irt_ismarked( ir->t ) ) {
            ...
            if ( ra_hasspill( ir->s ) ) {
                IRRef ren;
                lj_ir_set( as->J, IRT( IR_RENAME, IRT_NIL ), lref,
                           as->loopsnapno );
                ren = tref_ref( lj_ir_emit( as->J ) );
                as->ir = as->T->ir; /* The IR may have been reallocated. */
                IR( ren )->r = ( uint8_t )r;
...
static void asm_head_side( ASMState* as ) {
        ...
        for ( i = as->stopins; i > REF_BASE; i-- ) {
            ...
            if ( irt_ismarked( ir->t ) ) {
                ...
                if ( !ra_hasspill( regsp_spill( rs ) ) )
                    ra_sethint( ir->r,
                                rs ); /* Hint may be gone, set it again. */
                else if ( sps_scale( regsp_spill( rs ) ) + spdelta ==
                          sps_scale( ir->s ) )
                    continue; /* Same spill slot, do nothing. */
                mask = ( ( !LJ_SOFTFP && irt_isfp( ir->t ) ) ? RSET_FPR
                                                             : RSET_GPR ) &
                       allow;
                if ( mask == RSET_EMPTY )
    ...
    if ( ra_hasspill( irp->s ) )
        emit_spload( as, IR( REF_BASE ), IR( REF_BASE )->r,
                     sps_scale( irp->s ) );
#endif
    ...
    for ( ;; ) {
        ...
        if ( live & RSET_GPR ) {
            ...
            if ( tmpset == RSET_EMPTY )
                lj_trace_err( as->J, LJ_TRERR_NYICOAL );
            ra_rename( as, rset_pickbot( live & RSET_GPR ),
                       rset_pickbot( tmpset ) );
        }
        if ( !LJ_SOFTFP && ( live & RSET_FPR ) ) {
            ...
            if ( tmpset == RSET_EMPTY )
                lj_trace_err( as->J, LJ_TRERR_NYICOAL );
            ra_rename( as, rset_pickbot( live & RSET_FPR ),
                       rset_pickbot( tmpset ) );
        }
        checkmclim( as );
...
#endif
        as->T->topslot =
            ( uint8_t )as->topslot; /* Remember for child traces. */
...
static void asm_tail_link( ASMState* as ) {
    ...
    if ( as->T->link == 0 ) {
        ...
        const BCIns* pc =
            snap_pc( as->T->snapmap[ snap->mapofs + snap->nent ] );
        int32_t mres;
        if ( bc_op( *pc ) ==
             BC_JLOOP ) { /* NYI: find a better way to do this. */
        ...
        switch ( bc_op( *pc ) ) {
            case BC_CALLM:
            case BC_CALLMT:
                mres -= ( int32_t )( 1 + bc_a( *pc ) + bc_c( *pc ) );
                break;
            case BC_RETM:
    ...
    if ( !as->parent && gotframe )
        asm_stack_check( as, as->topslot, NULL, as->freeset & RSET_GPR,
                         snapno );
}
...
static void asm_setup_regsp( ASMState* as ) {
    ...
    if ( as->parent ) {
        ...
        for ( p = as->parentmap; ir < lastir; ir++ ) {
            RegSP rs = ir->prev;
            *p++ =
                ( uint16_t )rs; /* Copy original parent RegSP to parentmap. */
            if ( !ra_hasspill( regsp_spill( rs ) ) )
        ...
        switch ( ir->o ) {
            ...
            case IR_POW:
                if ( !LJ_SOFTFP && irt_isnum( ir->t ) ) {
                    ...
                    if ( inloop )
                        as->modset |= RSET_RANGE( RID_XMM0, RID_XMM1 + 1 ) |
                                      RID2RSET( RID_EAX );
#else
                    ir->prev = REGSP_HINT( RID_FPRET );
                    if ( inloop )
...
#if LJ_TARGET_X86ORX64
                if ( ir->op2 ==
                     IRFPM_EXP2 ) { /* May be joined to lj_vm_pow_sse. */
                    ir->prev = REGSP_HINT( RID_XMM0 );
#if !LJ_64
                    if ( as->evenspill <
...
#endif
                    if ( inloop )
                        as->modset |= RSET_RANGE( RID_XMM0, RID_XMM2 + 1 ) |
                                      RID2RSET( RID_EAX );
                    continue;
                } else if ( ir->op2 <= IRFPM_TRUNC &&
                            !( as->flags & JIT_F_SSE4_1 ) ) {
                    ir->prev = REGSP_HINT( RID_XMM0 );
                    if ( inloop )
                        as->modset |= RSET_RANGE( RID_XMM0, RID_XMM3 + 1 ) |
                                      RID2RSET( RID_EAX );
                    continue;
                }
                break;
...
void lj_asm_trace( jit_State* J, GCtrace* T ) {
    ...
    do {
        ...
        as->fuseref =
            ( as->flags & JIT_F_OPT_FUSE ) ? as->loopref : FUSE_DISABLED;
        asm_setup_regsp( as );
        if ( !as->loopref )
            asm_tail_link( as );
        ...
        for ( as->curins--; as->curins > as->stopins; as->curins-- ) {
            ...
            lua_assert(
                !( LJ_32 && irt_isint64( ir->t ) ) ); /* Handled by SPLIT. */
        ...
        as->realign ); /* Retry in case the MCode needs to be realigned. */
    ...
    T->mcode = as->mcp;
    T->mcloop =
        as->mcloop ? ( MSize )( ( char* )as->mcloop - ( char* )as->mcp ) : 0;
    if ( !as->loopref )
