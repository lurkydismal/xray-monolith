        ...
        if ( tviscdata( o ) ) {
            ...
            if ( ctype_isenum( ct->info ) ) {
                ...
                if ( cct && ctype_isconstval( cct->info ) ) {
                    ...
                    ca->p[ i ] =
                        ( uint8_t* )&cct->size; /* Assumes ct does not grow. */
                    ok = 1;
                } else {
                    ca->ct[ 1 - i ] =
                        ct; /* Use enum to improve error message. */
                    ca->p[ 1 - i ] = NULL;
                    break;
                }
            }
...
static int carith_ptr( lua_State* L, CTState* cts, CDArith* ca, MMS mm ) {
    ...
    if ( ctype_isptr( ctp->info ) || ctype_isrefarray( ctp->info ) ) {
        if ( ( mm == MM_sub || mm == MM_eq || mm == MM_lt || mm == MM_le ) &&
             ( ctype_isptr( ca->ct[ 1 ]->info ) ||
               ctype_isrefarray( ca->ct[ 1 ]->info ) ) ) {
            uint8_t* pp2 = ca->p[ 1 ];
            if ( mm ==
                 MM_eq ) { /* Pointer equality. Incompatible pointers are ok. */
            ...
            if ( mm == MM_sub ) { /* Pointer difference. */
                intptr_t diff;
                sz = lj_ctype_size(
                    cts, ctype_cid( ctp->info ) ); /* Element size. */
...
static int carith_int64( lua_State* L, CTState* cts, CDArith* ca, MMS mm ) {
    if ( ctype_isnum( ca->ct[ 0 ]->info ) && ca->ct[ 0 ]->size <= 8 &&
        ...
        CTypeID id = ( ( ( ca->ct[ 0 ]->info & CTF_UNSIGNED ) &&
                         ca->ct[ 0 ]->size == 8 ) ||
                       ( ( ca->ct[ 1 ]->info & CTF_UNSIGNED ) &&
                         ca->ct[ 1 ]->size == 8 ) )
                         ? CTID_UINT64
                         : CTID_INT64;
        ...
        if ( mm != MM_unm )
            lj_cconv_ct_ct( cts, ct, ca->ct[ 1 ], ( uint8_t* )&u1, ca->p[ 1 ],
                            0 );
        switch ( mm ) {
            case MM_eq:
                ...
                return 1;
            case MM_lt:
                setboolV( L->top - 1, id == CTID_INT64
        ...
        switch ( mm ) {
            ...
            case MM_div:
                if ( id == CTID_INT64 )
                    *up = ( uint64_t )lj_carith_divi64( ( int64_t )u0,
                                                        ( int64_t )u1 );
                else
            ...
            case MM_mod:
                if ( id == CTID_INT64 )
                    *up = ( uint64_t )lj_carith_modi64( ( int64_t )u0,
                                                        ( int64_t )u1 );
                else
            ...
            case MM_pow:
                if ( id == CTID_INT64 )
                    *up = ( uint64_t )lj_carith_powi64( ( int64_t )u0,
                                                        ( int64_t )u1 );
                else
...
static int lj_carith_meta( lua_State* L, CTState* cts, CDArith* ca, MMS mm ) {
    ...
    if ( !tv ) {
        ...
        for ( i = 0; i < 2; i++ ) {
            if ( ca->ct[ i ] && tviscdata( L->base + i ) ) {
                if ( ctype_isenum( ca->ct[ i ]->info ) )
                    isenum = i;
                repr[ i ] = strdata( lj_ctype_repr(
                    L, ctype_typeid( cts, ca->ct[ i ] ), NULL ) );
            } else {
                if ( tvisstr( &L->base[ i ] ) )
                    isstr = i;
        ...
        if ( ( isenum ^ isstr ) == 1 )
            lj_err_callerv( L, LJ_ERR_FFI_BADCONV, repr[ isstr ],
                            repr[ isenum ] );
        lj_err_callerv( L,
                        mm == MM_len      ? LJ_ERR_FFI_BADLEN
                        : mm == MM_concat ? LJ_ERR_FFI_BADCONCAT
                        : mm < MM_add     ? LJ_ERR_FFI_BADCOMP
                                          : LJ_ERR_FFI_BADARITH,
                        repr[ 0 ], repr[ 1 ] );
    }
...
int lj_carith_op( lua_State* L, MMS mm ) {
    ...
    if ( carith_checkarg( L, cts, &ca ) ) {
        if ( carith_int64( L, cts, &ca, mm ) ||
             carith_ptr( L, cts, &ca, mm ) ) {
            copyTV( L, &G( L )->tmptv2,
                    L->top - 1 ); /* Remember for trace recorder. */
            return 1;
        }
    }
