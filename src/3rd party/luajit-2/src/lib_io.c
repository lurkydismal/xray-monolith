...
static IOFileUD* io_file_open( lua_State* L, const char* mode ) {
    ...
    if ( iof->fp == NULL )
        luaL_argerror( L, 1,
                       lj_str_pushf( L, "%s: %s", fname, strerror( errno ) ) );
    return iof;
}
    ...
    if ( nargs == 0 ) {
        ...
        for ( n = start; nargs-- && ok; n++ ) {
            if ( tvisstr( L->base + n ) ) {
                ...
                ok = io_file_readlen( L, fp,
                                      ( MSize )lj_lib_checkint( L, n + 1 ) );
            } else {
                lj_err_arg( L, n + 1, LJ_ERR_INVOPT );
            }
        }
...
static int io_file_write( lua_State* L, FILE* fp, int start ) {
    ...
    for ( tv = L->base + start; tv < L->top; tv++ ) {
        if ( tvisstr( tv ) ) {
            ...
            status =
                status && ( fprintf( fp, LUA_NUMBER_FMT, numV( tv ) ) > 0 );
        } else {
...
LJLIB_CF( io_method_close ) {
    IOFileUD* iof =
        L->base < L->top ? io_tofile( L ) : IOSTDF_IOF( L, GCROOT_IO_OUTPUT );
...
LJLIB_CF( io_flush ) LJLIB_REC( io_flush GCROOT_IO_OUTPUT ) {
    return luaL_fileresult( L, fflush( io_stdfile( L, GCROOT_IO_OUTPUT ) ) == 0,
                            NULL );
}
...
LUALIB_API int luaopen_io( lua_State* L ) {
    ...
    setgcref( G( L )->gcroot[ GCROOT_IO_INPUT ],
              io_std_new( L, stdin, "stdin" ) );
    setgcref( G( L )->gcroot[ GCROOT_IO_OUTPUT ],
              io_std_new( L, stdout, "stdout" ) );
