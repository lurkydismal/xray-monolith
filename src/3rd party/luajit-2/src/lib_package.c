...
static const char* mksymname( lua_State* L,
                              const char* modname,
                              const char* prefix ) {
    const char* funcname;
...
static int ll_loadfunc( lua_State* L,
    ...
    if ( *reg == NULL ) {
        ...
        if ( !r ) {
            const char* bcdata =
                ll_bcsym( *reg, mksymname( L, name, SYMPREFIX_BC ) );
            lua_pop( L, 1 );
            if ( bcdata ) {
...
static int lj_cf_package_loadlib( lua_State* L ) {
    ...
    if ( st == 0 ) { /* no errors? */
        ...
        lua_insert( L, -2 );
        lua_pushstring( L,
                        ( st == PACKAGE_ERR_LIB ) ? PACKAGE_LIB_FAIL : "init" );
...
static const char* searchpath( lua_State* L,
    ...
    while ( ( path = pushnexttemplate( L, path ) ) != NULL ) {
        const char* filename =
            luaL_gsub( L, lua_tostring( L, -1 ), LUA_PATH_MARK, name );
...
static int lj_cf_package_searchpath( lua_State* L ) {
    const char* f = searchpath(
        L, luaL_checkstring( L, 1 ), luaL_checkstring( L, 2 ),
...
static int lj_cf_package_loader_preload( lua_State* L ) {
    ...
    if ( lua_isnil( L, -1 ) ) { /* Not found? */
        ...
        if ( bcdata == NULL ||
             luaL_loadbuffer( L, bcdata, ~( size_t )0, name ) != 0 )
...
static int lj_cf_package_require( lua_State* L ) {
    ...
    if ( lua_touserdata( L, -1 ) ==
         sentinel ) {               /* module did not set a value? */
...
static const luaL_Reg package_global[] = { { "module", lj_cf_package_module },
                                           { "require", lj_cf_package_require },
                                           { NULL, NULL } };

static const lua_CFunction package_loaders[] = {
    lj_cf_package_loader_preload, lj_cf_package_loader_lua,
    lj_cf_package_loader_c, lj_cf_package_loader_croot, NULL };
...
LUALIB_API int luaopen_package( lua_State* L ) {
    ...
    lua_createtable(
        L, sizeof( package_loaders ) / sizeof( package_loaders[ 0 ] ) - 1, 0 );
