    ...
    if ( idx >= 0 ) {
        ...
        if ( L->base + idx > L->top ) {
            if ( L->base + idx >= tvref( L->maxstack ) )
                lj_state_growstack(
                    L, ( MSize )idx - ( MSize )( L->top - L->base ) );
            do {
                setnilV( L->top++ );
...
#else
        int tt =
            ( int )( ( ( t < 8 ? 0x98042110u : 0x75a06u ) >> 4 * ( t & 7 ) ) &
                     15u );
#endif
...
LUA_API int lua_rawequal( lua_State* L, int idx1, int idx2 ) {
    ...
    return ( o1 == niltv( L ) || o2 == niltv( L ) ) ? 0
                                                    : lj_obj_equal( o1, o2 );
}
...
LUA_API void lua_createtable( lua_State* L, int narray, int nrec ) {
    GCtab* t;
    lj_gc_check( L );
    t = lj_tab_new( L, ( uint32_t )( narray > 0 ? narray + 1 : 0 ),
                    hsize2hbits( nrec ) );
    settabV( L, L->top, t );
    incr_top( L );
}
...
LUALIB_API int luaL_getmetafield( lua_State* L, int idx, const char* field ) {
    if ( lua_getmetatable( L, idx ) ) {
        cTValue* tv =
            lj_tab_getstr( tabV( L->top - 1 ), lj_str_newz( L, field ) );
        if ( tv && !tvisnil( tv ) ) {
            copyTV( L, L->top - 1, tv );
            return 1;
        }
...
LUA_API const char* lua_getupvalue( lua_State* L, int idx, int n ) {
    TValue* val;
    const char* name =
        lj_debug_uvnamev( index2adr( L, idx ), ( uint32_t )( n - 1 ), &val );
    if ( name ) {
        copyTV( L, L->top, val );
        incr_top( L );
    }
...
LUALIB_API void* luaL_checkudata( lua_State* L, int idx, const char* tname ) {
    ...
    if ( tvisudata( o ) ) {
        GCudata* ud = udataV( o );
        cTValue* tv =
            lj_tab_getstr( tabV( registry( L ) ), lj_str_newz( L, tname ) );
...
LUA_API int lua_yield( lua_State* L, int nresults ) {
    ...
    if ( cframe_canyield( cf ) ) {
        cf = cframe_raw( cf );
        if ( !hook_active(
                 g ) ) { /* Regular yield: move results down if needed. */
...
LUA_API int lua_gc( lua_State* L, int what, int data ) {
    ...
    switch ( what ) {
        ...
        case LUA_GCRESTART:
            g->gc.threshold =
                data == -1 ? ( g->gc.total / 100 ) * g->gc.pause : g->gc.total;
            break;
        case LUA_GCCOLLECT:
            lj_gc_fullgc( L );
            break;
