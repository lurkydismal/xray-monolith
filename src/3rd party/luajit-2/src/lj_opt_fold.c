...
#define gcstep_barrier( J, ref )                                              \
    ( ( ref ) < J->chain[ IR_LOOP ] &&                                        \
      ( J->chain[ IR_SNEW ] || J->chain[ IR_XSNEW ] || J->chain[ IR_TNEW ] || \
        J->chain[ IR_TDUP ] || J->chain[ IR_CNEW ] || J->chain[ IR_CNEWI ] || \
        J->chain[ IR_TOSTR ] ) )
...
LJFOLDF( kfold_int64arith2 ) {
    ...
    if ( irt_isi64( fins->t ) ) {
        k1 = fins->o == IR_DIV
                 ? lj_carith_divi64( ( int64_t )k1, ( int64_t )k2 )
             : fins->o == IR_MOD
                 ? lj_carith_modi64( ( int64_t )k1, ( int64_t )k2 )
...
LJFOLDF( kfold_int64comp ) {
    ...
    switch ( ( IROp )fins->o ) {
        ...
        case IR_GT:
            return CONDFOLD( a > b );
        case IR_ULT:
            return CONDFOLD( ( uint64_t )a < ( uint64_t )b );
        case IR_UGE:
            return CONDFOLD( ( uint64_t )a >= ( uint64_t )b );
        case IR_ULE:
            return CONDFOLD( ( uint64_t )a <= ( uint64_t )b );
...
LJFOLDF( kfold_strref_snew ) {
    ...
    if ( irref_isk( fins->op2 ) && fright->i == 0 ) {
        ...
        /* Reassociate: strref(snew(strref(str, a), len), b) ==> strref(str,
         * a+b) */
        IRIns* ir = IR( fleft->op1 );
        if ( ir->o == IR_STRREF ) {
            IRRef1 str = ir->op1; /* IRIns * is not valid across emitir. */
            PHIBARRIER( ir );
            fins->op2 = emitir( IRTI( IR_ADD ), ir->op2,
                                fins->op2 ); /* Clobbers fins! */
            fins->op1 = str;
...
LJFOLDF( simplify_conv_sext ) {
    ...
    PHIBARRIER( fleft );
    if ( fleft->o == IR_XLOAD &&
         ( irt_isu8( fleft->t ) || irt_isu16( fleft->t ) ) )
        goto ok_reduce;
...
LJFOLDF( narrow_convert ) {
    ...
    if ( J->chain[ IR_LOOP ] )
        return NEXTFOLD;
    lua_assert( fins->o != IR_CONV ||
                ( fins->op2 & IRCONV_CONVMASK ) != IRCONV_TOBIT );
...
LJFOLDF( simplify_intsub_k ) {
    ...
    fins->op2 =
        ( IRRef1 )lj_ir_kint( J, -fright->i ); /* Overflow for -2^31 ok. */
    return RETRYFOLD;
}
...
LJFOLDF( reassoc_shift ) {
    IRIns* irk = IR( fleft->op2 );
    PHIBARRIER(
        fleft ); /* The (shift any KINT) rule covers k2 == 0 and more. */
...
#define FOLD_SNEW_TYPE8 IRT_U8 /* Prefer unsigned loads. */
    ...
    if ( len <= FOLD_SNEW_MAX_LEN ) {
        ...
        if ( op == IR_EQ ) {
            ...
            if ( !irref_isk(
                     fleft->op2 ) ) /* Only handle the constant length case. */
                return NEXTFOLD;
            if ( IR( fleft->op2 )->i != len )
                return DROPFOLD;
        }
        if ( len > 0 ) {
            /* A 4 byte load for length 3 is ok -- all strings have an extra
             * NUL. */
            uint16_t ot =
                ( uint16_t )( len == 1   ? IRT( IR_XLOAD, FOLD_SNEW_TYPE8 )
            ...
            if ( len == 3 )
                tmp = emitir( IRTI( IR_BAND ), tmp,
                              lj_ir_kint( J, LJ_ENDIAN_SELECT( 0x00ffffff,
                                                               0xffffff00 ) ) );
            fins->op1 = ( IRRef1 )tmp;
            fins->op2 = ( IRRef1 )val;
    ...
    if ( LJ_LIKELY( J->flags & JIT_F_OPT_CSE ) ) {
        ...
        while ( ref > 0 ) {
            ...
            if ( irref_isk( ir->op1 ) ) {
                ...
                if ( gco2uv( gcref( fn2->l.uvptr[ ( ir->op2 >> 8 ) ] ) ) ==
                     uv ) {
...
LJFOLDF( fwd_href_tdup ) {
    ...
    if ( lj_tab_get( J->L, ir_ktab( IR( fleft->op1 ) ), &keyv ) ==
             niltvg( J2G( J ) ) &&
...
TRef LJ_FASTCALL lj_opt_fold( jit_State* J ) {
    ...
    if ( LJ_UNLIKELY( ( J->flags & JIT_F_OPT_MASK ) != JIT_F_OPT_DEFAULT ) ) {
        lua_assert( ( ( JIT_F_OPT_FOLD | JIT_F_OPT_FWD | JIT_F_OPT_CSE |
                        JIT_F_OPT_DSE ) |
                      JIT_F_OPT_DEFAULT ) == JIT_F_OPT_DEFAULT );
        /* Folding disabled? Chain to CSE, but not for loads/stores/allocs. */
        if ( !( J->flags & JIT_F_OPT_FOLD ) &&
             irm_kind( lj_ir_mode[ fins->o ] ) == IRM_N )
            return lj_opt_cse( J );
...
retry:
    ...
    if ( fins->op2 >= J->cur.nk ) {
        ...
        *fright = *IR( fins->op2 );
        ...
        key += ( fins->op2 &
                 0x3ffu ); /* Literal mask. Must include IRCONV_*MASK. */
    }
    ...
    for ( ;; ) {
        ...
        uint32_t fh =
            fold_hash[ h ]; /* Lookup key in semi-perfect hash table. */
        if ( ( fh & 0xffffff ) == k ||
             ( fh = fold_hash[ h + 1 ], ( fh & 0xffffff ) == k ) ) {
    ...
    if ( LJ_LIKELY( J->flags & JIT_F_OPT_CSE ) ) {
        ...
        while ( ref > lim ) {
            if ( IR( ref )->op12 == op12 )
                return TREF(
                    ref,
                    irt_t( IR( ref )->t ) ); /* Common subexpression found. */
            ref = IR( ref )->prev;
        }
    }
