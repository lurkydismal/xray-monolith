...
#define ctype_isrefarray( info )                                  \
    ( ( ( info ) & ( CTMASK_NUM | CTF_VECTOR | CTF_COMPLEX ) ) == \
      CTINFO( CT_ARRAY, 0 ) )
#define ctype_isvector( info )                      \
    ( ( ( info ) & ( CTMASK_NUM | CTF_VECTOR ) ) == \
      CTINFO( CT_ARRAY, CTF_VECTOR ) )
#define ctype_iscomplex( info )                      \
    ( ( ( info ) & ( CTMASK_NUM | CTF_COMPLEX ) ) == \
      CTINFO( CT_ARRAY, CTF_COMPLEX ) )
...
#define CTTYDEF( _ )                                                         \
    ...
    _( COMPLEX_DOUBLE, 16, CT_ARRAY,                                         \
       CTF_COMPLEX | CTALIGN( 3 ) | CTID_DOUBLE )                            \
...
#define CDF_SCL \
    ( CDF_TYPEDEF | CDF_EXTERN | CDF_STATIC | CDF_AUTO | CDF_REGISTER )
...
static LJ_AINLINE CType* ctype_child( CTState* cts, CType* ct ) {
    lua_assert(
        !( ctype_isvoid( ct->info ) || ctype_isstruct( ct->info ) ||
...
LJ_FUNC CTypeID lj_ctype_getname( CTState* cts,
                                  CType** ctp,
                                  GCstr* name,
                                  uint32_t tmask );
LJ_FUNC CType* lj_ctype_getfieldq( CTState* cts,
                                   CType* ct,
                                   GCstr* name,
                                   CTSize* ofs,
                                   CTInfo* qual );
