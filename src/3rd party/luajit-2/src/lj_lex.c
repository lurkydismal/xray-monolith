...
static void lex_number( LexState* ls, TValue* tv ) {
    ...
    if ( LJ_DUALNUM && fmt == STRSCAN_INT ) {
        ...
        lua_State* L = ls->L;
        GCcdata* cd;
        lua_assert( fmt == STRSCAN_I64 || fmt == STRSCAN_U64 ||
                    fmt == STRSCAN_IMAG );
        if ( !ctype_ctsG( G( L ) ) ) {
        ...
        if ( fmt == STRSCAN_IMAG ) {
            ...
            cd = lj_cdata_new_(
                L, fmt == STRSCAN_I64 ? CTID_INT64 : CTID_UINT64, 8 );
                ...
                switch ( c ) {
                    ...
                    case 'x': /* Hexadecimal escape '\xXX'. */
                        ...
                        if ( !lj_char_isdigit( ls->current ) ) {
                            if ( !lj_char_isxdigit( ls->current ) )
                                goto err_xesc;
                            c += 9 << 4;
                        }
                        c += ( next( ls ) & 15u );
        ...
        switch ( ls->current ) {
            ...
            case '-':
                ...
                if ( ls->current == '[' ) {
                    int sep = skip_sep( ls );
                    lj_str_resetbuf(
                        &ls->sb ); /* `skip_sep' may dirty the buffer */
                    if ( sep >= 0 ) {
...
int lj_lex_setup( lua_State* L, LexState* ls ) {
    ...
    if ( ls->current == LUA_SIGNATURE[ 0 ] ) { /* Bytecode dump. */
        if ( header ) {
            ...
            ** security violation no attempt is made to echo the chunkname
            * either.
            */
