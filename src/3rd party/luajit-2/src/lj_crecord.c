...
static GCcdata* argv2cdata( jit_State* J, TRef tr, cTValue* o ) {
    ...
    emitir( IRTG( IR_EQ, IRT_INT ), trtypeid,
            lj_ir_kint( J, ( int32_t )cd->ctypeid ) );
    return cd;
}
...
static CTypeID argv2ctype( jit_State* J, TRef tr, cTValue* o ) {
    if ( tref_isstr( tr ) ) {
        ...
        if ( lj_cparse( &cp ) ||
             cp.cts->top > oldtop ) /* Avoid new struct defs. */
            lj_trace_err( J, LJ_TRERR_BADTYPE );
        return cp.val.id;
    } else {
...
static MSize crec_copy_unroll( CRecMemList* ml,
                               CTSize len,
                               CTSize step,
                               IRType tp ) {
    CTSize ofs = 0;
...
*/
static void crec_copy_emit( jit_State* J,
                            CRecMemList* ml,
                            MSize mlp,
                            TRef trdst,
                            TRef trsrc ) {
    ...
    for ( i = 0, j = 0; i < mlp; ) {
        ...
        if ( rwin >= CREC_COPY_REGWIN ||
             i >= mlp ) { /* Flush buffered stores. */
            rwin = 0;
            for ( ; j < i; j++ ) {
                TRef trdptr =
                    emitir( IRT( IR_ADD, IRT_PTR ), trdst, ml[ j ].trofs );
...
static void crec_copy( jit_State* J,
                       TRef trdst,
                       TRef trsrc,
                       TRef trlen,
                       CType* ct ) {
    if ( tref_isk( trlen ) ) { /* Length must be constant. */
        ...
        if ( ct ) {
            ...
            lua_assert( ctype_isarray( ct->info ) ||
                        ctype_isstruct( ct->info ) );
...
*/
static void crec_fill_emit( jit_State* J,
                            CRecMemList* ml,
                            MSize mlp,
                            TRef trdst,
                            TRef trfill ) {
...
static void crec_fill( jit_State* J,
                       TRef trdst,
                       TRef trlen,
                       TRef trfill,
                       CTSize step ) {
    if ( tref_isk( trlen ) ) { /* Length must be constant. */
        ...
        if ( ml[ 0 ].tp != IRT_U8 ) { /* Scatter U8 to U16/U32/U64. */
            if ( CTSIZE_PTR == 8 && ml[ 0 ].tp == IRT_U64 ) {
                if ( tref_isk( trfill ) ) /* Pointless on x64 with zero-extended
                                             regs. */
                    trfill = emitconv( trfill, IRT_U64, IRT_U32, 0 );
                trfill =
                    emitir( IRT( IR_MUL, IRT_U64 ), trfill,
                            lj_ir_kint64( J, U64x( 01010101, 01010101 ) ) );
            } else {
                trfill = emitir(
                    IRTI( IR_MUL ), trfill,
                    lj_ir_kint( J,
                                ml[ 0 ].tp == IRT_U16 ? 0x0101 : 0x01010101 ) );
            }
        }
    ...
    fallback:
        ...
        lj_ir_call( J, IRCALL_memset, trdst, trfill,
                    trlen ); /* Note: arg order! */
    }
    emitir( IRT( IR_XBAR, IRT_NIL ), 0, 0 );
}
...
static TRef crec_ct_ct( jit_State* J,
                        CType* d,
                        CType* s,
                        TRef dp,
                        TRef sp,
    ...
    if ( ctype_type( dinfo ) > CT_MAYCONVERT ||
         ctype_type( sinfo ) > CT_MAYCONVERT )
        goto err_conv;
    ...
    switch ( cconv_idx2( dinfo, sinfo ) ) {
        ...
        case CCX( B, F ):
            if ( st != IRT_CDATA ) {
                ...
                TRef zero =
                    ( st == IRT_NUM || st == IRT_FLOAT ) ? lj_ir_knum( J, 0 )
        ...
        case CCX( I, F ):
            if ( dt == IRT_CDATA || st == IRT_CDATA )
                goto err_nyi;
            sp = emitconv( sp, dsize < 4 ? IRT_INT : dt, st,
                           IRCONV_TRUNC | IRCONV_ANY );
            goto xstore;
        case CCX( I, P ):
        case CCX( I, A ):
            ...
            st = IRT_UINTP;
            if ( ( ( dsize ^ ssize ) & 8 ) ==
                 0 ) { /* Must insert no-op type conversion. */
        ...
        case CCX( C, C ):
                ...
                goto err_nyi;
            {
                ...
                ptr = emitir( IRT( IR_ADD, IRT_PTR ), dp,
                              lj_ir_kintp( J, ( dsize >> 1 ) ) );
        ...
        case CCX( P, S ):
            ...
            ** Also ok on x64, since all 32 bit ops clear the upper part of the
            * reg.
            */
            goto xstore;
        case CCX( P, I ):
        ...
        case CCX( P, F ):
             ...
             */
            sp = emitconv( sp, ( LJ_64 && dsize == 8 ) ? IRT_I64 : IRT_U32, st,
                           IRCONV_TRUNC | IRCONV_ANY );
            goto xstore;
...
static TRef crec_tv_ct( jit_State* J, CType* s, CTypeID sid, TRef sp ) {
    ...
    if ( ctype_isnum( sinfo ) ) {
        ...
        if ( t == IRT_FLOAT ||
             t == IRT_U32 ) { /* Keep uint32_t/float as numbers. */
        ...
        cts->L = J->L;
        sid = lj_ctype_intern( cts, CTINFO_REF( sid ),
                               CTSIZE_PTR ); /* Create ref. */
    } else if ( ctype_iscomplex( sinfo ) ) { /* Unbox/box complex. */
        ptrdiff_t esz = ( ptrdiff_t )( s->size >> 1 );
        TRef ptr, tr1, tr2, dp;
        dp = emitir( IRTG( IR_CNEW, IRT_CDATA ), lj_ir_kint( J, sid ),
                     TREF_NIL );
        ...
        ptr = emitir( IRT( IR_ADD, IRT_PTR ), dp,
                      lj_ir_kintp( J, sizeof( GCcdata ) + esz ) );
...
static TRef crec_ct_tv( jit_State* J,
                        CType* d,
                        TRef dp,
                        TRef sp,
                        cTValue* sval ) {
    ...
    if ( LJ_LIKELY( tref_isinteger( sp ) ) ) {
        sid = CTID_INT32;
        svisnz = ( void* )( intptr_t )( tvisint( sval ) ? ( intV( sval ) != 0 )
                                                        : !tviszero( sval ) );
        ...
        if ( ud->udtype == UDTYPE_IO_FILE ) {
            ...
            sp = emitir( IRT( IR_ADD, IRT_PTR ), sp,
                         lj_ir_kintp( J, sizeof( GCudata ) ) );
        }
    } else if ( tref_isstr( sp ) ) {
        if ( ctype_isenum(
                 d->info ) ) { /* Match string against enum constant. */
            GCstr* str = strV( sval );
            CTSize ofs;
            ...
            /* Don't use STRREF. It folds with SNEW, which loses the trailing
             * NUL. */
            sp = emitir( IRT( IR_ADD, IRT_PTR ), sp,
                         lj_ir_kintp( J, sizeof( GCstr ) ) );
            sid = CTID_A_CCHAR;
        }
...
*/
static TRef crec_reassoc_ofs( jit_State* J,
                              TRef tr,
                              ptrdiff_t* ofsp,
                              MSize sz ) {
...
static void crec_index_meta( jit_State* J,
                             CTState* cts,
                             CType* ct,
                             RecordFFData* rd ) {
    ...
    if ( tvisfunc( tv ) ) {
        ...
        emitir( IRTG( IR_EQ, IRT_STR ), J->base[ 1 ],
                lj_ir_kstr( J, strV( &rd->argv[ 1 ] ) ) );
    } else {
    ...
    if ( tref_isnumber( idx ) ) {
        ...
        integer_key:
            if ( ( ct->info & CTF_COMPLEX ) )
                idx = emitir( IRT( IR_BAND, IRT_INTP ), idx,
                              lj_ir_kintp( J, 1 ) );
            ...
            if ( LJ_LIKELY( J->flags & JIT_F_OPT_LOOP ) && ofs
                ...
                ptr = emitir( IRT( IR_ADD, IRT_PTR ), ptr,
                              lj_ir_kintp( J, ofs ) );
                ofs = 0;
            }
#endif
        ...
        if ( ctype_isstruct( ct->info ) ) {
            ...
            if ( fct ) {
                ...
                if ( ctype_isconstval( fct->info ) ) {
                    if ( fct->size >= 0x80000000u &&
                        ...
                        J->base[ 0 ] = lj_ir_knum(
                            J, ( lua_Number )( uint32_t )fct->size );
                        return;
                    }
            ...
            if ( name->len == 2 && ( ( strdata( name )[ 0 ] == 'r' &&
                                       strdata( name )[ 1 ] == 'e' ) ||
                                     ( strdata( name )[ 0 ] == 'i' &&
                                       strdata( name )[ 1 ] == 'm' ) ) ) {
                /* Always specialize to the field name. */
...
static void crec_alloc( jit_State* J, RecordFFData* rd, CTypeID id ) {
    ...
    TRef trid;
    if ( !sz || sz > 128 || ( info & CTF_VLA ) ||
         ctype_align( info ) > CT_MEMALIGN )
        lj_trace_err( J,
                      LJ_TRERR_NYICONV ); /* NYI: large/special allocations. */
    trid = lj_ir_kint( J, id );
    /* Use special instruction to box pointer or 32/64 bit integer. */
    if ( ctype_isptr( info ) ||
         ( ctype_isinteger( info ) && ( sz == 4 || sz == 8 ) ) ) {
        TRef sp = J->base[ 1 ]
                      ? crec_ct_tv( J, d, 0, J->base[ 1 ], &rd->argv[ 1 ] )
        ...
        if ( J->base[ 1 ] && !J->base[ 2 ] &&
            ...
            if ( !( ctype_isnum( dc->info ) || ctype_isptr( dc->info ) ) )
                lj_trace_err(
                    J, LJ_TRERR_NYICONV ); /* NYI: init array of aggregates. */
            for ( i = 1, ofs = 0; ofs < sz; ofs += esize ) {
                ...
                if ( J->base[ i ] ) {
                    ...
                    sp =
                        ctype_isnum( dc->info ) ? lj_ir_kint( J, 0 ) : TREF_NIL;
                }
            ...
            while ( fid ) {
                ...
                if ( ctype_isfield( df->info ) ) {
                    ...
                    if ( !( ctype_isnum( dc->info ) ||
                        ...
                        lj_trace_err(
                            J, LJ_TRERR_NYICONV ); /* NYI: init aggregates. */
                    if ( J->base[ i ] ) {
                        sp = J->base[ i ];
                        sval = &rd->argv[ i ];
                        i++;
                    } else {
                        sp = ctype_isptr( dc->info ) ? TREF_NIL
                                                     : lj_ir_kint( J, 0 );
                    }
                    dp = emitir(
                        IRT( IR_ADD, IRT_PTR ), trcd,
...
static TRef crec_call_args( jit_State* J,
                            RecordFFData* rd,
                            CTState* cts,
                            CType* ct ) {
        ...
        if ( ctype_isinteger_or_bool( d->info ) ) {
            if ( d->size < 4 ) {
                if ( ( d->info & CTF_UNSIGNED ) )
                    tr = emitconv( tr, IRT_INT, d->size == 1 ? IRT_U8 : IRT_U16,
                                   0 );
                else
                    tr = emitconv( tr, IRT_INT, d->size == 1 ? IRT_I8 : IRT_I16,
                                   IRCONV_SEXT );
            }
...
static int crec_call( jit_State* J, RecordFFData* rd, GCcdata* cd ) {
    ...
    if ( ctype_isfunc( ct->info ) ) {
        ...
        setlightudV( &tv, cdata_getptr( cdataptr( cd ),
                                        ( LJ_64 && tp == IRT_P64 ) ? 8 : 4 ) );
        ...
        tr = emitir( IRT( IR_CALLXS, t ), crec_call_args( J, rd, cts, ct ),
                     func );
        if ( ctype_isbool( ctr->info ) ) {
            if ( frame_islua( J->L->base - 1 ) &&
                ...
                /* Note: only the x86/x64 backend supports U8 and only for
                 * EQ(tr, 0). */
        ...
        } else if ( t == IRT_PTR || ( LJ_64 && t == IRT_P32 ) || t == IRT_I64 ||
                    t == IRT_U64 || ctype_isenum( ctr->info ) ) {
...
void LJ_FASTCALL recff_cdata_call( jit_State* J, RecordFFData* rd ) {
    ...
    ct = ctype_raw( cts, id );
    tv = lj_ctype_meta(
        cts, ctype_isptr( ct->info ) ? ctype_cid( ct->info ) : id, mm );
    if ( tv ) {
        if ( tvisfunc( tv ) ) {
            J->base[ -1 ] = lj_ir_kfunc( J, funcV( tv ) ) | TREF_FRAME;
            rd->nres = -1; /* Pending tailcall. */
...
static TRef crec_arith_int64( jit_State* J, TRef* sp, CType** s, MMS mm ) {
    if ( ctype_isnum( s[ 0 ]->info ) && ctype_isnum( s[ 1 ]->info ) ) {
        ...
        if ( ( ( s[ 0 ]->info & CTF_UNSIGNED ) && s[ 0 ]->size == 8 ) ||
            ...
            dt = IRT_I64;
            id = CTID_INT64;
            if ( mm < MM_add && !( ( s[ 0 ]->info | s[ 1 ]->info ) & CTF_FP ) &&
                 s[ 0 ]->size == 4 &&
                 s[ 1 ]->size == 4 ) { /* Try to narrow comparison. */
                if ( !( ( s[ 0 ]->info ^ s[ 1 ]->info ) & CTF_UNSIGNED ) ||
                     ( tref_isk( sp[ 1 ] ) &&
                       IR( tref_ref( sp[ 1 ] ) )->i >= 0 ) ) {
                    dt = ( s[ 0 ]->info & CTF_UNSIGNED ) ? IRT_U32 : IRT_INT;
                    goto comp;
                } else if ( tref_isk( sp[ 0 ] ) &&
                            IR( tref_ref( sp[ 0 ] ) )->i >= 0 ) {
        ...
        for ( i = 0; i < 2; i++ ) {
            ...
            if ( st == IRT_NUM || st == IRT_FLOAT )
                sp[ i ] =
                    emitconv( sp[ i ], dt, st, IRCONV_TRUNC | IRCONV_ANY );
        ...
        comp:
            ...
            tr = emitir( IRT( mm + ( int )IR_ADD - ( int )MM_add, dt ), sp[ 0 ],
                         sp[ 1 ] );
        }
...
static TRef crec_arith_ptr( jit_State* J, TRef* sp, CType** s, MMS mm ) {
    ...
    if ( ctype_isptr( ctp->info ) || ctype_isrefarray( ctp->info ) ) {
        if ( ( mm == MM_sub || mm == MM_eq || mm == MM_lt || mm == MM_le ) &&
             ( ctype_isptr( s[ 1 ]->info ) ||
               ctype_isrefarray( s[ 1 ]->info ) ) ) {
            if ( mm == MM_sub ) { /* Pointer difference. */
                ...
                tr = emitir( IRT( IR_BSAR, IRT_INTP ), tr,
                             lj_ir_kint( J, lj_fls( sz ) ) );
#if LJ_64
    ...
    }
    {
        ...
        tr = emitir( IRT( mm + ( int )IR_ADD - ( int )MM_add, IRT_PTR ),
                     sp[ 0 ], tr );
        id = lj_ctype_intern(
            cts, CTINFO( CT_PTR, CTALIGN_PTR | ctype_cid( ctp->info ) ),
            CTSIZE_PTR );
        return emitir( IRTG( IR_CNEWI, IRT_CDATA ), lj_ir_kint( J, id ), tr );
    }
}
...
static TRef crec_arith_meta( jit_State* J,
                             TRef* sp,
                             CType** s,
                             CTState* cts,
                             RecordFFData* rd ) {
    ...
    if ( tv ) {
        if ( tvisfunc( tv ) ) {
            ...
            return 0;
                ...
                MM_eq ) { /* Fallback cdata pointer comparison. */
        if ( sp[ 0 ] && sp[ 1 ] &&
             ctype_isnum( s[ 0 ]->info ) == ctype_isnum( s[ 1 ]->info ) ) {
            /* Assume true comparison. Fixup and emit pending guard later. */
...
void LJ_FASTCALL recff_cdata_arith( jit_State* J, RecordFFData* rd ) {
    ...
    for ( i = 0; i < 2; i++ ) {
        ...
        if ( !tr ) {
            ...
            if ( ctype_isptr( ct->info ) ) { /* Resolve pointer or reference. */
                ...
                tr = emitir( IRT( IR_ADD, IRT_PTR ), tr,
                             lj_ir_kintp( J, sizeof( GCcdata ) ) );
            }
            if ( ctype_isenum( ct->info ) )
                ct = ctype_child( cts, ct );
            ...
            ct = ctype_raw( cts, id );
            if ( ctype_isenum(
                     ct->info ) ) { /* Match string against enum constant. */
    ...
    }
    {
        ...
        if ( J->postproc == LJ_POST_FIXGUARD &&
             frame_iscont( J->L->base - 1 ) && !irt_isguard( J->guardemit ) ) {
            const BCIns* pc = frame_contpc( J->L->base - 1 ) - 1;
            if ( bc_op( *pc ) <= BC_ISNEP ) {
                setframe_pc( &J2G( J )->tmptv, pc );
                J2G( J )->tmptv.u32.lo =
                    ( ( tref_istrue( tr ) ^ bc_op( *pc ) ) & 1 );
        ...
        if ( id && tv && !tvisnil( tv ) ) {
            ...
            if ( ctype_isconstval( ct->info ) ) {
                if ( ct->size >= 0x80000000u &&
                    ...
                    J->base[ 0 ] =
                        lj_ir_knum( J, ( lua_Number )( uint32_t )ct->size );
                else
...
void LJ_FASTCALL recff_ffi_string( jit_State* J, RecordFFData* rd ) {
    ...
    if ( tr ) {
        ...
        if ( !tref_isnil( trlen ) ) {
            ...
            tr = crec_ct_tv( J, ctype_get( cts, CTID_P_CVOID ), 0, tr,
                             &rd->argv[ 0 ] );
        } else {
            tr = crec_ct_tv( J, ctype_get( cts, CTID_P_CCHAR ), 0, tr,
                             &rd->argv[ 0 ] );
...
void LJ_FASTCALL recff_ffi_copy( jit_State* J, RecordFFData* rd ) {
    ...
    if ( trdst && trsrc && ( trlen || tref_isstr( trsrc ) ) ) {
        trdst = crec_ct_tv( J, ctype_get( cts, CTID_P_VOID ), 0, trdst,
                            &rd->argv[ 0 ] );
        trsrc = crec_ct_tv( J, ctype_get( cts, CTID_P_CVOID ), 0, trsrc,
                            &rd->argv[ 1 ] );
        if ( trlen ) {
...
void LJ_FASTCALL recff_ffi_fill( jit_State* J, RecordFFData* rd ) {
    ...
    if ( trdst && trlen ) {
        CTSize step = 1;
        if ( tviscdata( &rd->argv[ 0 ] ) ) { /* Get alignment of original
                                                destination. */
            CTSize sz;
            ...
            if ( ctype_isptr( ct->info ) )
                ct = ctype_rawchild( cts, ct );
            step = ( 1u << ctype_align(
                         lj_ctype_info( cts, ctype_typeid( cts, ct ), &sz ) ) );
        }
        trdst = crec_ct_tv( J, ctype_get( cts, CTID_P_VOID ), 0, trdst,
                            &rd->argv[ 0 ] );
        trlen = crec_toint( J, cts, trlen, &rd->argv[ 1 ] );
...
void LJ_FASTCALL recff_ffi_typeof( jit_State* J, RecordFFData* rd ) {
    if ( tref_iscdata( J->base[ 0 ] ) ) {
        TRef trid =
            lj_ir_kint( J, argv2ctype( J, J->base[ 0 ], &rd->argv[ 0 ] ) );
...
void LJ_FASTCALL recff_ffi_abi( jit_State* J, RecordFFData* rd ) {
    if ( tref_isstr( J->base[ 0 ] ) ) {
         ...
         */
        emitir( IRTG( IR_EQ, IRT_STR ), J->base[ 0 ],
                lj_ir_kstr( J, strV( &rd->argv[ 0 ] ) ) );
        J->postproc = LJ_POST_FIXBOOL;
        J->base[ 0 ] = TREF_TRUE;
    } else {
        lj_trace_err( J, LJ_TRERR_BADTYPE );
    ...
    if ( rd->data == FF_ffi_sizeof ) {
        ...
        if ( ctype_isvltype( ct->info ) )
            lj_trace_err( J, LJ_TRERR_BADTYPE );
                ...
                FF_ffi_offsetof ) { /* Specialize to the field name. */
