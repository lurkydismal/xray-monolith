...
static IRRef split_call_l( jit_State* J,
                           IRRef1* hisubst,
                           IRIns* oir,
                           IRIns* ir,
                           IRCallID id ) {
    ...
    J->cur.nins--;
#if LJ_LE
    tmp = split_emit( J, IRT( IR_CARG, IRT_NIL ), oir[ op1 ].prev,
                      hisubst[ op1 ] );
#else
    tmp = split_emit( J, IRT( IR_CARG, IRT_NIL ), hisubst[ op1 ],
                      oir[ op1 ].prev );
#endif
...
static IRRef split_call_li( jit_State* J,
                            IRRef1* hisubst,
                            IRIns* oir,
                            IRIns* ir,
                            IRCallID id ) {
...
static IRRef split_call_ll( jit_State* J,
                            IRRef1* hisubst,
                            IRIns* oir,
                            IRIns* ir,
                            IRCallID id ) {
    ...
    return split_emit(
        J,
        IRT( IR_HIOP,
             ( LJ_SOFTFP && irt_isnum( ir->t ) ) ? IRT_SOFTFP : IRT_INT ),
        tmp, tmp );
}
...
static void split_ir( jit_State* J ) {
    ...
    for ( ref = REF_FIRST; ref < nins; ref++ ) {
        ...
        if ( ref >= snref ) {
            ...
            snref =
                snap < &J->cur.snap[ J->cur.nsnap ] ? snap->ref : ~( IRRef )0;
        }
        ...
        if ( irt_isnum( ir->t ) ) {
            nir->t.irt =
                IRT_INT | ( nir->t.irt & IRT_GUARD ); /* Turn into INT op. */
            /* Note: hi ref = lo ref + 1! Required for SNAP_SOFTFPNUM logic. */
            switch ( ir->o ) {
                case IR_ADD:
                    hi =
                        split_call_ll( J, hisubst, oir, ir, IRCALL_softfp_add );
                    break;
                case IR_SUB:
                    hi =
                        split_call_ll( J, hisubst, oir, ir, IRCALL_softfp_sub );
                    break;
                case IR_MUL:
                    hi =
                        split_call_ll( J, hisubst, oir, ir, IRCALL_softfp_mul );
                    break;
                case IR_DIV:
                    hi =
                        split_call_ll( J, hisubst, oir, ir, IRCALL_softfp_div );
                    break;
                case IR_POW:
                    hi =
                        split_call_li( J, hisubst, oir, ir, IRCALL_lj_vm_powi );
                    break;
                case IR_FPMATH:
                    ...
                    if ( nir->op2 == IRFPM_EXP2 && nir->op1 > J->loopref ) {
                        ...
                        if ( irp->o == IR_CALLN &&
                             irp->op2 == IRCALL_softfp_mul ) {
                            IRIns* irm4 = IR( irp->op1 );
                            IRIns* irm3 = IR( irm4->op1 );
                            IRIns* irm12 = IR( irm3->op1 );
                            IRIns* irl1 = IR( irm12->op1 );
                            if ( irm12->op1 > J->loopref &&
                                 irl1->o == IR_CALLN &&
                                 irl1->op2 == IRCALL_lj_vm_log2 ) {
                                IRRef tmp = irl1->op1; /* Recycle first two args
                                                          from LOG2. */
                                IRRef arg3 = irm3->op2, arg4 = irm4->op2;
                                J->cur.nins--;
                                tmp = split_emit( J, IRT( IR_CARG, IRT_NIL ),
                                                  tmp, arg3 );
                                tmp = split_emit( J, IRT( IR_CARG, IRT_NIL ),
                                                  tmp, arg4 );
                                ir->prev = tmp = split_emit(
                                    J, IRTI( IR_CALLN ), tmp, IRCALL_pow );
                                hi = split_emit( J, IRT( IR_HIOP, IRT_SOFTFP ),
                                                 tmp, tmp );
                                break;
                            }
                        }
                ...
                case IR_ABS:
                    ...
                    hi = split_emit(
                        J,
                        IRT( ir->o == IR_NEG ? IR_BXOR : IR_BAND, IRT_SOFTFP ),
                        hisubst[ ir->op1 ], hisubst[ ir->op2 ] );
                    break;
                case IR_SLOAD:
                    if ( ( nir->op2 & IRSLOAD_CONVERT ) ) { /* Convert from int
                                                               to number. */
                        nir->op2 &= ~IRSLOAD_CONVERT;
                        ir->prev = nref = split_emit( J, IRTI( IR_CALLN ), nref,
                                                      IRCALL_softfp_i2d );
                        hi = split_emit( J, IRT( IR_HIOP, IRT_SOFTFP ), nref,
                                         nref );
                        break;
                    }
                    /* fallthrough */
                ...
                case IR_XLOAD: {
                    IRIns inslo =
                        *nir; /* Save/undo the emit of the lo XLOAD. */
                    J->cur.nins--;
                    hi = split_ptr( J, oir,
                                    ir->op1 ); /* Insert the hiref ADD. */
                    nref = lj_ir_nextins( J );
                    nir = IR( nref );
                    *nir = inslo; /* Re-emit lo XLOAD immediately before hi
                                     XLOAD. */
                    hi = split_emit( J, IRT( IR_XLOAD, IRT_SOFTFP ), hi,
                                     ir->op2 );
#if LJ_LE
                    ir->prev = nref;
#else
                ...
                case IR_USTORE:
                case IR_XSTORE:
                    split_emit( J, IRT( IR_HIOP, IRT_SOFTFP ), nir->op1,
                                hisubst[ ir->op2 ] );
                    break;
                ...
                case IR_HIOP:
                    ...
                    break;
                default:
                    lua_assert( ir->o <= IR_NE || ir->o == IR_MIN ||
                                ir->o == IR_MAX );
            ...
            switch ( ir->o ) {
                case IR_ADD:
                case IR_SUB:
                    /* Use plain op for hiword if loword cannot produce a
                     * carry/borrow. */
                    ...
                    /* fallthrough */
                case IR_NEG:
                    hi = split_emit( J, IRTI( IR_HIOP ), hiref,
                                     hisubst[ ir->op2 ] );
                    break;
                case IR_MUL:
                ...
                case IR_FLOAD:
                    ...
                    hi = split_emit( J, IRTI( IR_FLOAD ), nir->op1,
                                     IRFL_CDATA_INT64_4 );
#if LJ_BE
                    ir->prev = hi;
                    hi = nref;
                ...
                case IR_XLOAD:
                    ...
                    break;
                case IR_XSTORE:
                    split_emit( J, IRTI( IR_HIOP ), nir->op1,
                                hisubst[ ir->op2 ] );
                    break;
                case IR_CONV: { /* Conversion to 64 bit integer. Others handled
                                   below. */
                    IRType st = ( IRType )( ir->op2 & IRCONV_SRCMASK );
#if LJ_SOFTFP
                    if ( st == IRT_NUM ) { /* NUM to 64 bit int conv. */
                        hi = split_call_l( J, hisubst, oir, ir,
                                ...
                                IRT_FLOAT ) { /* FLOAT to 64 bit int conv. */
                        nir->o = IR_CALLN;
                        nir->op2 = irt_isi64( ir->t ) ? IRCALL_fp64_f2l
                                                      : IRCALL_fp64_f2ul;
                        hi = split_emit( J, IRTI( IR_HIOP ), nref, nref );
                    }
#else
                    if ( st == IRT_NUM ||
                         st == IRT_FLOAT ) { /* FP to 64 bit int conv. */
                        hi = split_emit( J, IRTI( IR_HIOP ), nir->op1, nref );
                    }
#endif
                    else if ( st == IRT_I64 ||
                              st == IRT_U64 ) { /* 64/64 bit cast. */
                        /* Drop cast, since assembler doesn't care. */
                        goto fwdlo;
                    } else if ( ( ir->op2 &
                                  IRCONV_SEXT ) ) { /* Sign-extend to 64 bit. */
                ...
                case IR_PHI: {
                    ...
                    if ( !( ( irref_isk( hiref ) && irref_isk( hiref2 ) ) ||
                            hiref == hiref2 ) )
                ...
                default:
                    ...
                    split_emit( J, IRTGI( IR_HIOP ), hiref,
                                hisubst[ ir->op2 ] );
                    break;
            }
        } else
            ...
            if ( ir->o == IR_SLOAD ) {
            if ( ( nir->op2 &
                   IRSLOAD_CONVERT ) ) { /* Convert from number to int. */
                nir->op2 &= ~IRSLOAD_CONVERT;
                if ( !( nir->op2 & IRSLOAD_TYPECHECK ) )
                    nir->t.irt = IRT_INT; /* Drop guard. */
                split_emit( J, IRT( IR_HIOP, IRT_SOFTFP ), nref, nref );
                ir->prev =
                    split_num2int( J, nref, nref + 1, irt_isguard( ir->t ) );
            }
...
#endif
            ir->prev =
                split_emit( J, IRTI( IR_CALLN ), tmp, IRCALL_lj_vm_tobit );
        } else if ( ir->o == IR_TOSTR ) {
            if ( hisubst[ ir->op1 ] ) {
                if ( irref_isk( ir->op1 ) )
                    nir->op1 = ir->op1;
                else
                    split_emit( J, IRT( IR_HIOP, IRT_NIL ), hisubst[ ir->op1 ],
                                nref );
            }
...
#if LJ_32 && LJ_HASFFI
            if ( st == IRT_I64 ||
                 st == IRT_U64 ) { /* Conversion from 64 bit int. */
#if LJ_SOFTFP
                if ( irt_isfloat( ir->t ) ) {
                    split_call_l(
                        J, hisubst, oir, ir,
...
#else
                if ( irt_isfp(
                         ir->t ) ) { /* 64 bit integer to FP conversion. */
...
#if LJ_SOFTFP
                if ( st == IRT_NUM ||
                     ( LJ_32 && LJ_HASFFI && st == IRT_FLOAT ) ) {
                if ( irt_isguard( ir->t ) ) {
                    lua_assert( st == IRT_NUM && irt_isint( ir->t ) );
                    J->cur.nins--;
                    ir->prev =
                        split_num2int( J, nir->op1, hisubst[ ir->op1 ], 1 );
                } else {
                    split_call_l(
                        J, hisubst, oir, ir,
#if LJ_32 && LJ_HASFFI
                        st == IRT_NUM
                            ? ( irt_isint( ir->t ) ? IRCALL_softfp_d2i
                                                   : IRCALL_softfp_d2ui )
                            : ( irt_isint( ir->t ) ? IRCALL_softfp_f2i
                                                   : IRCALL_softfp_f2ui )
#else
                        IRCALL_softfp_d2i
#endif
        ...
        split_call:
            ...
            if ( LJ_SOFTFP ? irt_is64( ir->t ) : irt_isint64( ir->t ) )
                hi = split_emit(
                    J,
                    IRT( IR_HIOP, ( LJ_SOFTFP && irt_isnum( ir->t ) )
                                      ? IRT_SOFTFP
                                      : IRT_INT ),
                    nref, nref );
...
#endif
                ir->prev = nref =
                    split_emit( J, IRT( IR_CARG, IRT_NIL ), nref, op2 );
                nir = IR( nref );
            }
            ...
            if ( hiref ) {
                ...
                int carg = 0;
                IRIns* cir;
                for ( cir = IR( nir->op1 ); cir->o == IR_CARG;
                      cir = IR( cir->op1 ) )
                    carg++;
...
#endif
                ir->prev =
                    split_emit( J, IRT( IR_CARG, IRT_NIL ), nref, hiref );
            }
        } else if ( ir->o == IR_CNEWI ) {
            if ( hisubst[ ir->op2 ] )
                split_emit( J, IRT( IR_HIOP, IRT_NIL ), nref,
                            hisubst[ ir->op2 ] );
