...
#define CKPL( kind, st )                                               \
    do {                                                               \
        if ( ( size_t )( ( char* )pl - ( char* )D->kind##labels ) >=   \
             D->kind##size ) {                                         \
        ...
        if ( action >= DASM__MAX ) {
            ...
            switch ( action ) {
                ...
                putrel:
                    ...
                    linkrel:
                        b[ pos ] =
                            n; /* Else link to rel chain, anchored at label. */
                        *pl = pos;
                    }
                    pos++;
                    break;
    ...
    *szp = 0;
    ...
    { /* Handle globals not defined in this translation unit. */
        ...
        for ( idx = 20; idx * sizeof( int ) < D->lgsize; idx++ ) {
            ...
            /* Undefined label: Collapse rel chain and replace with marker (<
             * 0). */
            while ( n > 0 ) {
                int* pb = DASM_POS2PTR( D, n );
                n = *pb;
    ...
    for ( secnum = 0; secnum < D->maxsection; secnum++ ) {
        ...
        while ( b != endb ) {
            ...
            while ( 1 ) {
                ...
                unsigned int action = ( ins >> 16 );
                int n =
                    ( action >= DASM_ALIGN && action < DASM__MAX ) ? *b++ : 0;
                switch ( action ) {
                    case DASM_STOP:
                    case DASM_SECTION:
                        goto stop;
                    case DASM_ESC:
                        *cp++ = *p++;
                        break;
                    case DASM_REL_EXT:
                        n = DASM_EXTERN( Dst, ( unsigned char* )cp,
                                         ( ins & 2047 ), !( ins & 2048 ) );
                        goto patchrel;
                    case DASM_ALIGN:
                        ins &= 255;
                    ...
                    patchrel:
                        if ( ( ins & 0x800 ) == 0 ) {
                            ...
                            CK( ( n & 3 ) == 0 && -256 <= n && n <= 256,
                                RANGE_REL );
                            goto patchimml8;
                        } else if ( ( ins & 0x2000 ) == 0 ) {
                            CK( ( n & 3 ) == 0 && -4096 <= n && n <= 4096,
                                RANGE_REL );
                            goto patchimml;
                        } else {
                            CK( ( n & 3 ) == 0 && -1020 <= n && n <= 1020,
                                RANGE_REL );
                            n >>= 2;
                            goto patchimml;
                        }
