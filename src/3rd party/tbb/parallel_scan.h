...
private:
    ...
    task* execute() __TBB_override {
        if ( my_body ) {
            ...
            sum_node& c = *this;
            task* b = c.create_child( Range( my_range, split() ), *my_left_sum,
                                      my_right, my_left_sum, my_stuff_last );
            task* a = my_left_is_final
                          ? NULL
                          : c.create_child( my_range, *my_body, my_left,
                                            my_incoming, NULL );
...
public:
    ...
    task* execute() __TBB_override {
        __TBB_ASSERT(
            my_result.ref_count() ==
                ( my_result.my_left != NULL ) + ( my_result.my_right != NULL ),
            NULL );
        if ( my_result.my_left )
            my_result.my_left_is_final = false;
        if ( my_right_zombie && my_sum )
            ( ( *my_sum )->my_body )
                .reverse_join( my_result.my_left_sum->my_body );
    ...
    finish_scan( sum_node_type*& return_slot_,
                 final_sum_type** sum_,
...
public:
    start_scan( sum_node_type*& return_slot_,
                start_scan& parent_,
    ...
    static void run( const Range& range_,
                     Body& body_,
                     const Partitioner& partitioner_ ) {
        if ( !range_.empty() ) {
            ...
            final_sum_type* temp_body =
                new ( task::allocate_root() ) final_sum_type( body_ );
            start_pass1_type& pass1 =
                *new ( task::allocate_root() ) start_pass1_type(
                    /*my_return_slot=*/root, range_, *temp_body, partitioner_ );
...
task* start_scan< Range, Body, Partitioner >::execute() {
    ...
    finish_pass1_type* p =
        my_parent_sum ? static_cast< finish_pass1_type* >( parent() ) : NULL;
    ...
    bool treat_as_stolen =
        my_is_right_child &&
        ( is_stolen_task() || my_body != p->my_result.my_left_sum );
    if ( treat_as_stolen ) {
        // Invocation is for right child that has been really stolen or needs to
        // be virtually stolen
        p->my_right_zombie = my_body =
            new ( allocate_root() ) final_sum_type( my_body->my_body );
        my_is_final = false;
    }
    task* next_task = NULL;
    if ( ( my_is_right_child && !treat_as_stolen ) ||
         !my_range.is_divisible() ||
         my_partition.should_execute_range( *this ) ) {
        if ( my_is_final )
        ...
        if ( my_parent_sum )
            result = new ( allocate_additional_child_of( *my_parent_sum ) )
                sum_node_type( my_range, /*my_left_is_final=*/my_is_final );
        else
            result = new ( task::allocate_root() )
                sum_node_type( my_range, /*my_left_is_final=*/my_is_final );
        finish_pass1_type& c =
            *new ( allocate_continuation() )
                finish_pass1_type( *my_return_slot, my_sum, *result );
        ...
        // Left child is recycling of *this.  Must recycle this before spawning
        // b, otherwise b might complete and decrement c.ref_count() to zero,
        // which would cause c.execute() to run prematurely.
...
public:
    lambda_scan_body( const Value& identity,
                      const Scan& scan,
                      const ReverseJoin& rev_join )
        : my_sum( identity ),
    ...
    void assign( lambda_scan_body& b ) { my_sum = b.my_sum; }
...
/** \page parallel_scan_body_req Requirements on parallel_scan body
    ...
    - \code Body::Body( Body&, split ); \endcode    Splitting constructor.
                                                    Split \c b so that \c this
and \c b can accumulate separately
    - \code Body::~Body(); \endcode                 Destructor
    - \code void Body::operator()( const Range& r, pre_scan_tag ); \endcode
                                                    Preprocess iterations for
range \c r
    - \code void Body::operator()( const Range& r, final_scan_tag ); \endcode
                                                    Do final processing for
iterations of range \c r
    - \code void Body::reverse_join( Body& a ); \endcode
                                                    Merge preprocessing state of
\c a into \c this, where \c a was created earlier from \c b by b's splitting
constructor
**/

/** \name parallel_scan
    See also requirements on \ref range_req "Range" and \ref
   parallel_scan_body_req "parallel_scan Body". **/
//@{
...
void parallel_scan( const Range& range, Body& body ) {
    internal::start_scan< Range, Body, __TBB_DEFAULT_PARTITIONER >::run(
...
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_scan( const Range& range,
                    Body& body,
                    const simple_partitioner& partitioner ) {
    internal::start_scan< Range, Body, simple_partitioner >::run( range, body,
                                                                  partitioner );
}
...
void parallel_scan( const Range& range,
    ...
    internal::start_scan< Range, Body, auto_partitioner >::run( range, body,
                                                                partitioner );
}
...
/** @ingroup algorithms **/
template < typename Range, typename Value, typename Scan, typename ReverseJoin >
Value parallel_scan( const Range& range,
                     const Value& identity,
                     const Scan& scan,
                     const ReverseJoin& reverse_join ) {
    internal::lambda_scan_body< Range, Value, Scan, ReverseJoin > body(
        identity, scan, reverse_join );
