...
class concurrent_queue : public internal::concurrent_queue_base_v3< T > {
    ...
    //! Allocator type
    typedef typename tbb::internal::allocator_rebind< A, char >::type
        page_allocator_type;
    ...
    static void move_construct_item( T* location, const void* src ) {
        new ( location )
            T( std::move( *static_cast< T* >( const_cast< void* >( src ) ) ) );
    }
...
public:
    ...
    concurrent_queue( InputIterator begin,
                      InputIterator end,
    ...
    concurrent_queue( concurrent_queue&& src )
        : internal::concurrent_queue_base_v3< T >(),
          my_allocator( std::move( src.my_allocator ) ) {
    ...
    bool try_pop( T& result ) { return this->internal_try_pop( &result ); }
    ...
    typedef internal::concurrent_queue_iterator< concurrent_queue, const T >
        const_iterator;

    //------------------------------------------------------------------------
    // The iterators are intended only for debugging.  They are slow and not
    // thread safe.
    //------------------------------------------------------------------------
...
/** This is the pre-PPL TBB concurrent queue which supports boundedness and
   blocking semantics. Note that method names agree with the PPL-style
   concurrent queue. Multiple threads may each push and pop concurrently.
    Assignment construction is not allowed.
    @ingroup containers */
...
class concurrent_bounded_queue : public internal::concurrent_queue_base_v8 {
    ...
    virtual void copy_item( page& dst,
                            size_t index,
                            const void* src ) __TBB_override {
        new ( &get_ref( dst, index ) ) T( *static_cast< const T* >( src ) );
    }

#if __TBB_CPP11_RVALUE_REF_PRESENT
    virtual void move_item( page& dst,
                            size_t index,
                            const void* src ) __TBB_override {
        new ( &get_ref( dst, index ) )
            T( std::move( *static_cast< T* >( const_cast< void* >( src ) ) ) );
    }
#else
    ...
    virtual void copy_page_item( page& dst,
                                 size_t dindex,
                                 const page& src,
                                 size_t sindex ) __TBB_override {
        new ( &get_ref( dst, dindex ) )
            T( get_ref( const_cast< page& >( src ), sindex ) );
    }
    ...
    virtual void move_page_item( page& dst,
                                 ...
                                 size_t sindex ) __TBB_override {
        new ( &get_ref( dst, dindex ) )
            T( std::move( get_ref( const_cast< page& >( src ), sindex ) ) );
    }
#else
    virtual void move_page_item( page&, size_t, const page&, size_t )
        __TBB_override {
        __TBB_ASSERT( false, "Unreachable code" );
    }
#endif
...
public:
    ...
    //! Construct empty queue
    explicit concurrent_bounded_queue(
        const allocator_type& a = allocator_type() )
    ...
    concurrent_bounded_queue( concurrent_bounded_queue&& src )
        : concurrent_queue_base_v8( sizeof( T ) ),
          my_allocator( std::move( src.my_allocator ) ) {
        internal_swap( src );
    }
    ...
    //! Enqueue an item at tail of queue.
    void push( const T& source ) { internal_push( &source ); }

#if __TBB_CPP11_RVALUE_REF_PRESENT
    //! Move an item at tail of queue.
    void push( T&& source ) { internal_push_move( &source ); }
    ...
    void pop( T& destination ) { internal_pop( &destination ); }

#if TBB_USE_EXCEPTIONS
    //! Abort all pending queue operations
    void abort() { internal_abort(); }
#endif
    ...
    size_type capacity() const { return my_capacity; }

    //! Set the capacity
    /** Setting the capacity to 0 causes subsequent try_push operations to
       always fail, and subsequent push operations to block forever. */
    ...
    typedef internal::concurrent_queue_iterator< concurrent_bounded_queue, T >
        iterator;
    typedef internal::concurrent_queue_iterator< concurrent_bounded_queue,
                                                 const T >
        const_iterator;
