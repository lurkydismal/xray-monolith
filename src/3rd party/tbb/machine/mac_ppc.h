...
// This file is for Power Architecture with compilers supporting GNU
// inline-assembler syntax (currently GNU g++ and IBM XL). Note that XL V9.0
// (sometimes?) has trouble dealing with empty input and/or clobber lists, so
...
// Little-endian could be just an address manipulation (compatibility with TBB
// not verified), or normal little-endian (on more recent systems). Embedded
// PowerPC systems may support page-specific endianness, but then one endianness
// must be hidden from TBB so that it still sees only one.
#if __BIG_ENDIAN__ || \
...
#if __TBB_WORDSIZE == 8
// Do not change the following definition, because TBB itself will use 64-bit
// atomics in 64-bit builds.
#define __TBB_64BIT_ATOMICS 1
#elif __bgp__
// Do not change the following definition, because this is known 32-bit
// hardware.
#define __TBB_64BIT_ATOMICS 0
#else
// To enable 64-bit atomics in 32-bit builds, set the value below to 1 instead
// of 0. You must make certain that the program will only use them on actual
// 64-bit hardware (which typically means that the entire program is only
// executed on such hardware), because their implementation involves machine
// instructions that are illegal elsewhere. The setting can be chosen
// independently per compilation unit, which also means that TBB itself does not
// need to be rebuilt. Alternatively (but only for the current architecture and
// TBB version), override the default as a predefined macro when invoking the
// compiler.
...
inline int32_t __TBB_machine_cmpswp4( volatile void* ptr,
                                      int32_t value,
                                      int32_t comparand ) {
    int32_t result;
    ...
    __asm__ __volatile__(
        ...
        : [ptr] "r"( ptr ), [val] "r"( value ), [cmp] "r"( comparand )
...
inline int64_t __TBB_machine_cmpswp8( volatile void* ptr,
                                      int64_t value,
                                      int64_t comparand ) {
    int64_t result;
    __asm__ __volatile__(
...
inline int64_t __TBB_machine_cmpswp8( volatile void* ptr,
                                      ...
                                      int64_t comparand ) {
    int64_t result;
    int64_t value_register, comparand_register,
        result_register; // dummy variables to allocate registers
    __asm__ __volatile__(
        "sync\n\t"
        "ld %[val],%[valm]\n\t"
        "ld %[cmp],%[cmpm]\n"
        ...
        "std %[res],%[resm]\n\t"
        "isync"
        : [resm] "=m"( result ), [res] "=&r"( result_register ),
          [val] "=&r"( value_register ), [cmp] "=&r"( comparand_register ),
          "+m"( *( int64_t* )ptr ) /* redundant with "memory" */
        : [ptr] "r"( ptr ), [valm] "m"( value ), [cmpm] "m"( comparand )
        : "memory" /* compiler full fence */
          ,
...
#define __TBB_MACHINE_DEFINE_LOAD_STORE( S, ldx, stx, cmpx )              \
    ...
    struct machine_load_store< T, S > {                                   \
        ...
        static inline void store_with_release( volatile T& location,      \
                                               T value ) {                \
            __asm__ __volatile__(                                         \
                "lwsync\n\t" stx " %[val],0(%[ptr])"                      \
                : "=m"( location )       /* redundant with "memory" */    \
...
struct machine_load_store_relaxed< T, 8 > {
    ...
    static inline void store( volatile T& location, T value ) {
        ...
        __asm__ __volatile__(
            "ld %[val],%[valm]\n\t"
            "std %[val],0(%[ptr])"
            : "=m"( location ), [val] "=&r"( value_register )
            : [ptr] "b"( &location ) /* cannot use register 0 here */
              ,
              [valm] "m"( value ) );
        /*(no compiler fence)*/ /*(cr0 not affected)*/
...
static inline intptr_t __TBB_machine_lg( uintptr_t x ) {
    ...
    // cntlzd/cntlzw starts counting at 2^63/2^31 (ignoring any higher-order
    // bits), and does not affect cr0
#if __TBB_WORDSIZE == 8
