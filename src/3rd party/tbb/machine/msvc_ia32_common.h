...
// TODO: consider moving this macro to tbb_config.h and using where MSVC asm is
// used
...
#define __TBB_NO_X86_MSVC_INLINE_ASM_MSG \
    "The compiler being used is not supported (outdated?)"
#endif
...
typedef long word;
  ...
  // S is the operand size in bytes, B is the suffix for intrinsics for that
  // size
#define __TBB_MACHINE_DEFINE_ATOMICS( S, B, T, U )                            \
    __pragma( intrinsic( _InterlockedCompareExchange##B ) ) static inline T   \
    __TBB_machine_cmpswp##S( volatile void* ptr, U value, U comparand ) {     \
        return _InterlockedCompareExchange##B( ( T* )ptr, value, comparand ); \
    }                                                                         \
    __pragma( intrinsic( _InterlockedExchangeAdd##B ) ) static inline T       \
    __TBB_machine_fetchadd##S( volatile void* ptr, U addend ) {               \
        return _InterlockedExchangeAdd##B( ( T* )ptr, addend );               \
    }                                                                         \
    __pragma( intrinsic( _InterlockedExchange##B ) ) static inline T          \
    __TBB_machine_fetchstore##S( volatile void* ptr, U value ) {              \
...
__TBB_set_cpu_ctl_env( const tbb::internal::cpu_ctl_env* );
}
#endif
...
// Intel(R) 64 and IA-32 Architectures Optimization Reference Manual 12.4.5 lock
// not free
