...
#define __TBB_MACHINE_DEFINE_ATOMICS( S, T, X, R )                             \
    static inline T __TBB_machine_cmpswp##S( volatile void* ptr, T value,      \
                                             T comparand ) {                   \
        T result;                                                              \
                                                                               \
        ...
        return result;                                                         \
                                                                               ...
                                                                               \
    static inline T __TBB_machine_fetchadd##S( volatile void* ptr,             \
                                               T addend ) {                    \
        T result;                                                              \
        __asm__ __volatile__( "lock\nxadd" X " %0,%1"                          \
                              : R( result ), "=m"( *( __TBB_VOLATILE T* )ptr ) \
                              : "0"( addend ),                                 \
                                "m"( *( __TBB_VOLATILE T* )ptr )               \
                              : "memory" );                                    \
        return result;                                                         \
    }                                                                          \
                                                                               \
    static inline T __TBB_machine_fetchstore##S( volatile void* ptr,           \
                                                 T value ) {                   \
        T result;                                                              \
        __asm__ __volatile__( "lock\nxchg" X " %0,%1"                          \
                              : R( result ), "=m"( *( __TBB_VOLATILE T* )ptr ) \
...
static inline __TBB_IA32_CAS8_NOINLINE int64_t
__TBB_machine_cmpswp8( volatile void* ptr, int64_t value, int64_t comparand ) {
// TODO: remove the extra part of condition once
// __TBB_GCC_BUILTIN_ATOMICS_PRESENT is lowered to gcc version 4.1.2
#if ( __TBB_GCC_BUILTIN_ATOMICS_PRESENT || ( __TBB_GCC_VERSION >= 40102 ) ) && \
    !__TBB_GCC_64BIT_ATOMIC_BUILTINS_BROKEN
    return __sync_val_compare_and_swap(
        reinterpret_cast< volatile int64_t* >( ptr ), comparand, value );
...
#endif
        "movl  %2,%%ebx"
        : "=A"( result ), "=m"( *( __TBB_VOLATILE int64_t* )ptr ), "=m"( tmp )
#if __GNUC__ == 3
    ...
    __asm__ __volatile__(
        ...
        : "m"( *( __TBB_VOLATILE int64_t* )ptr ), "0"( comparand ),
          "b"( i32[0] ), "c"( i32[1] )
        : "memory" );
#endif /* __PIC__ */
    return result;
...
// TODO: Check if it possible and profitable for IA-32 architecture on (Linux*
// and Windows*) to use of 64-bit load/store via floating point registers
// together with full fence for sequentially consistent load/store, instead of
// CAS.
...
static inline int64_t __TBB_machine_aligned_load8( const volatile void* ptr ) {
    __TBB_ASSERT( tbb::internal::is_aligned( ptr, 8 ),
                  "__TBB_machine_aligned_load8 should be used with 8 byte "
                  "aligned locations only \n" );
    int64_t result;
    __asm__ __volatile__( __TBB_fildq " %1\n\t" __TBB_fistpq " %0"
                          : "=m"( result )
                          : "m"( *( const __TBB_VOLATILE uint64_t* )ptr )
                          : "memory" );
    return result;
}

static inline void __TBB_machine_aligned_store8( volatile void* ptr,
                                                 int64_t value ) {
    __TBB_ASSERT( tbb::internal::is_aligned( ptr, 8 ),
                  "__TBB_machine_aligned_store8 should be used with 8 byte "
                  "aligned locations only \n" );
    // Aligned store
    __asm__ __volatile__( __TBB_fildq " %1\n\t" __TBB_fistpq " %0"
                          : "=m"( *( __TBB_VOLATILE int64_t* )ptr )
