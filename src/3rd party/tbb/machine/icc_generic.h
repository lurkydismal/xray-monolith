...
#error \
    "Intel(R) C++ Compiler of at least 12.0 version is needed to use ICC intrinsics port"
#endif
...
//__TBB_compiler_fence() defined just in case, as it seems not to be used on its
// own anywhere else
...
// instead of copy paste. however it seems unlikely that ICC will silently
// change exact enum values, as they are defined in the ISO exactly like this.
...
#define __TBB_MACHINE_DEFINE_ATOMICS( S, T, M )                              \
    inline T __TBB_machine_cmpswp##S##M( volatile void* ptr, T value,        \
                                         T comparand ) {                     \
        __atomic_compare_exchange_strong_explicit(                           \
            ( T* )ptr, &comparand, value,                                    \
    ...
    inline T __TBB_machine_fetchstore##S##M( volatile void* ptr, T value ) { \
        return __atomic_exchange_explicit(                                   \
            ( T* )ptr, value, tbb::internal::icc_intrinsics_port::M );       \
    }                                                                        \
                                                                             \
    inline T __TBB_machine_fetchadd##S##M( volatile void* ptr, T value ) {   \
        return __atomic_fetch_add_explicit(                                  \
            ( T* )ptr, value, tbb::internal::icc_intrinsics_port::M );       \
    }
    ...
    static void store_with_release( volatile T& location, T value ) {
        if ( tbb::internal::is_aligned( &location, 8 ) ) {
            __atomic_store_explicit(
                &location, icc_intrinsics_port::convert_argument( value ),
                memory_order_release );
        } else {
...
inline void __TBB_machine_OR( T* operand, T addend ) {
    __atomic_fetch_or_explicit( operand, addend,
                                tbb::internal::memory_order_seq_cst );
}

template < typename T >
inline void __TBB_machine_AND( T* operand, T addend ) {
    __atomic_fetch_and_explicit( operand, addend,
                                 tbb::internal::memory_order_seq_cst );
}
