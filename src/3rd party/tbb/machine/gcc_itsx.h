...
inline static uint8_t __TBB_machine_try_lock_elided( volatile uint8_t* lk ) {
    uint8_t value = 1;
    __asm__ volatile(
        ".byte " __TBB_STRINGIZE( __TBB_OP_XACQUIRE ) "; lock; xchgb %0, %1;"
        : __TBB_r_out( value ), "=m"( *lk )
        : "0"( value ), "m"( *lk )
        : "memory" );
...
inline static void __TBB_machine_unlock_elided( volatile uint8_t* lk ) {
    __asm__ volatile(
        ".byte " __TBB_STRINGIZE( __TBB_OP_XRELEASE ) "; movb $0, %0"
        : "=m"( *lk )
        : "m"( *lk )
        : "memory" );
}
...
inline static uint32_t __TBB_machine_begin_transaction() {
    ...
    __asm__ volatile(
        ...
        "   .long  2f-1b-6\n"  //  2f-1b == difference in addresses of start
                               ...
                               //  2f - 1b - 6 == that difference minus the size
                               //  of the XBEGIN instruction.  This is the abort
                               //  offset to 2: below.
...
inline static void __TBB_machine_end_transaction() {
    __asm__ volatile( ".byte 0x0F; .byte 0x01; .byte 0xD5" ::
                          : "memory" ); // XEND
}
