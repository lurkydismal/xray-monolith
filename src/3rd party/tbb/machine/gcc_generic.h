...
// __BYTE_ORDER__ is used in accordance with
// http://gcc.gnu.org/onlinedocs/cpp/Common-Predefined-Macros.html, but
// __BIG_ENDIAN__ or __LITTLE_ENDIAN__ may be more commonly found instead.
#if __BIG_ENDIAN__ || \
...
/** As this generic implementation has absolutely no information about
   underlying hardware, its performance most likely will be sub-optimal because
   of full memory fence usages where a more lightweight synchronization means
   (or none at all) could suffice. Thus if you use this header to enable TBB on
   a new platform, consider forking it and relaxing below helpers as
   appropriate. **/
...
#define __TBB_MACHINE_DEFINE_ATOMICS( S, T )                                 \
    inline T __TBB_machine_cmpswp##S( volatile void* ptr, T value,           \
                                      T comparand ) {                        \
        return __sync_val_compare_and_swap(                                  \
            reinterpret_cast< volatile T* >( ptr ), comparand, value );      \
    }                                                                        \
...
// Adding it at both sides of an intrinsic, as we do not know what reordering
// can be made.
...
#define __TBB_MACHINE_DEFINE_ATOMICS( S, T )                                  \
    inline T __TBB_machine_cmpswp##S( volatile void* ptr, T value,            \
        ...
        ( void )__atomic_compare_exchange_n(                                  \
            reinterpret_cast< volatile T* >( ptr ), &comparand, value, false, \
            __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST );                             \
        return comparand;                                                     \
    }                                                                         \
    inline T __TBB_machine_fetchadd##S( volatile void* ptr, T value ) {       \
        return __atomic_fetch_add( reinterpret_cast< volatile T* >( ptr ),    \
                                   value, __ATOMIC_SEQ_CST );                 \
    }                                                                         \
    inline T __TBB_machine_fetchstore##S( volatile void* ptr, T value ) {     \
        return __atomic_exchange_n( reinterpret_cast< volatile T* >( ptr ),   \
                                    value, __ATOMIC_SEQ_CST );                \
    }
...
static inline void __TBB_machine_or( volatile void* ptr, uintptr_t addend ) {
    __sync_fetch_and_or( reinterpret_cast< volatile uintptr_t* >( ptr ),
                         addend );
}

static inline void __TBB_machine_and( volatile void* ptr, uintptr_t addend ) {
    __sync_fetch_and_and( reinterpret_cast< volatile uintptr_t* >( ptr ),
                          addend );
}
...
/** GCC atomic operation intrinsics might miss compiler fence.
    ...
    on both sides of sequentially consistent operations is sufficient for
   correctness. **/
