...
/** The analogue of _msize/malloc_size/malloc_usable_size.
    Returns the usable size of a memory block previously allocated by
   scalable_*, or 0 (zero) if ptr does not point to such a block.
    @ingroup memory_allocation */
...
/* Setting TBB_MALLOC_USE_HUGE_PAGES environment variable to 1 enables huge
   pages. scalable_allocation_mode call has priority over environment variable.
 */
typedef enum {
...
typedef int ( *rawFreeType )( intptr_t pool_id,
                              void* raw_ptr,
                              size_t raw_bytes );
...
struct MemPoolPolicy {
    ...
    MemPoolPolicy( rawAllocType pAlloc_,
                   rawFreeType pFree_,
                   size_t granularity_ = 0,
                   bool fixedPool_ = false,
...
MemPoolError pool_create_v1( intptr_t pool_id,
...
void* pool_aligned_realloc( MemoryPool* mPool,
                            void* ptr,
                            size_t size,
                            size_t alignment );
...
template < typename E >
__TBB_NOINLINE( void throw_exception( const E& e ) );
#endif
...
public:
    ...
    pointer allocate( size_type n, const void* /*hint*/ = 0 ) {
        pointer p = static_cast< pointer >(
            scalable_malloc( n * sizeof( value_type ) ) );
        if ( !p )
    ...
    void deallocate( pointer p, size_type ) { scalable_free( p ); }

    //! Largest value for which method allocate might succeed.
    size_type max_size() const throw() {
        size_type absolutemax =
...
#if _MSC_VER
#if ( __TBB_BUILD || __TBBMALLOC_BUILD ) && \
    !defined( __TBBMALLOC_NO_IMPLICIT_LINKAGE )
