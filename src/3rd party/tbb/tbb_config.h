...
/** This header is supposed to contain macro definitions and C style comments
only. The macros defined here are intended to control such aspects of TBB build
as
 ...
 * considered for the future. See also for plain comment with TODO and FIXME
 * marks for small improvement opportunities.
 */
#define __TBB_TODO 0

/* Check which standard library we use. */
/* __TBB_SYMBOL is defined only while processing exported symbols list where C++
 * is not allowed. */
#if !defined( __TBB_SYMBOL ) && !__TBB_CONFIG_PREPROC_ONLY
#include <cstddef>
#endif

// Note that when ICC or Clang is in use, __TBB_GCC_VERSION might not fully
// match the actual GCC version on the system.
#define __TBB_GCC_VERSION \
...
// The version is explicitly specified in our public TBB_USE_GLIBCXX_VERSION
// macro. Its format should match the __TBB_GCC_VERSION above, e.g. 70301 for
// libstdc++ coming with GCC 7.3.1.
...
// The version macro is not defined or matches the GCC version; use
// __TBB_GCC_VERSION
...
#if __APPLE__
#if __INTEL_COMPILER &&                                     \
    __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ > 1099 && \
...
#define __TBB_TSX_AVAILABLE \
    ( ( __TBB_x86_32 || __TBB_x86_64 ) && !__TBB_DEFINE_MIC )
...
/**
    __TBB_CPP11_PRESENT macro indicates that the compiler supports vast majority
   of C++11 features. Depending on the compiler, some features might still be
   unsupported or work incorrectly. Use it when enabling C++11 features
   individually is not practical, and be aware that some "good enough" compilers
   might be excluded. **/
#define __TBB_CPP11_PRESENT ( __cplusplus >= 201103L || _MSC_VER >= 1900 )

#define __TBB_CPP17_FALLTHROUGH_PRESENT ( __cplusplus >= 201703L )
#define __TBB_FALLTHROUGH_PRESENT \
    ( __TBB_GCC_VERSION >= 70000 && !__INTEL_COMPILER )

/** C++11 mode detection macros for Intel(R) C++ Compiler (enabled by -std=c++XY
   option):
    __INTEL_CXX11_MODE__ for version >=13.0 (not available for ICC 15.0 if
   -std=c++14 is used),
...
#define __INTEL_CXX11_MODE__ \
    ( __GXX_EXPERIMENTAL_CXX0X__ || ( _MSC_VER && __STDC_HOSTED__ ) )
#endif

#if __INTEL_COMPILER && ( !_MSC_VER || __INTEL_CXX11_MODE__ )
//  On Windows, C++11 features supported by Visual Studio 2010 and higher are
//  enabled by default, so in absence of /Qstd= use MSVC branch for feature
//  detection. On other platforms, no -std= means C++03.

#define __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT \
    ( __INTEL_CXX11_MODE__ && __VARIADIC_TEMPLATES )
...
#define __TBB_CPP11_RVALUE_REF_PRESENT                                 \
    ( ( _MSC_VER >= 1700 ||                                            \
        __GXX_EXPERIMENTAL_CXX0X__ &&                                  \
            ( __TBB_GLIBCXX_VERSION >= 40500 || _LIBCPP_VERSION ) ) && \
      __INTEL_COMPILER >= 1400 )
#define __TBB_IMPLICIT_MOVE_PRESENT                       \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1400 && \
      ( _MSC_VER >= 1900 || __TBB_GCC_VERSION >= 40600 || __clang__ ) )
#if _MSC_VER >= 1600
#define __TBB_EXCEPTION_PTR_PRESENT                                     \
    ( __INTEL_COMPILER > 1300 /*ICC 12.1 Upd 10 and 13 beta Upd 2 fixed \
                                 exception_ptr linking  issue*/         \
      || ( __INTEL_COMPILER == 1300 &&                                  \
           __INTEL_COMPILER_BUILD_DATE >= 20120530 ) ||                 \
      ( __INTEL_COMPILER == 1210 &&                                     \
        __INTEL_COMPILER_BUILD_DATE >= 20120410 ) )
/** libstdc++ that comes with GCC 4.6 use C++11 features not supported by
 * ICC 12.1. Because of that ICC 12.1 does not support C++11 mode with gcc 4.6
 * (or higher), and therefore does not define __GXX_EXPERIMENTAL_CXX0X__ macro
 * **/
#elif __TBB_GLIBCXX_VERSION >= 40404 && __TBB_GLIBCXX_VERSION < 40600
#define __TBB_EXCEPTION_PTR_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __INTEL_COMPILER >= 1200 )
#elif __TBB_GLIBCXX_VERSION >= 40600
#define __TBB_EXCEPTION_PTR_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __INTEL_COMPILER >= 1300 )
...
#define __TBB_CPP11_TUPLE_PRESENT   \
    ( _MSC_VER >= 1600 ||           \
      __GXX_EXPERIMENTAL_CXX0X__ && \
          ( __TBB_GLIBCXX_VERSION >= 40300 || _LIBCPP_VERSION ) )
#define __TBB_INITIALIZER_LISTS_PRESENT                       \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1400 &&     \
      ( _MSC_VER >= 1800 || __TBB_GLIBCXX_VERSION >= 40400 || \
        _LIBCPP_VERSION ) )
#define __TBB_CONSTEXPR_PRESENT \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1400 )
#define __TBB_DEFAULTED_AND_DELETED_FUNC_PRESENT \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1200 )
/** ICC seems to disable support of noexcept event in c++11 when compiling in
 * compatibility mode for gcc <4.6 **/
#define __TBB_NOEXCEPT_PRESENT                            \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1300 && \
      ( __TBB_GLIBCXX_VERSION >= 40600 || _LIBCPP_VERSION || _MSC_VER ) )
#define __TBB_CPP11_STD_BEGIN_END_PRESENT                       \
    ( _MSC_VER >= 1700 ||                                       \
      __GXX_EXPERIMENTAL_CXX0X__ && __INTEL_COMPILER >= 1310 && \
          ( __TBB_GLIBCXX_VERSION >= 40600 || _LIBCPP_VERSION ) )
...
#define __TBB_CPP11_DEFAULT_FUNC_TEMPLATE_ARGS_PRESENT \
    ( _MSC_VER >= 1800 ||                              \
      __GXX_EXPERIMENTAL_CXX0X__ && __INTEL_COMPILER >= 1210 )
#define __TBB_OVERRIDE_PRESENT \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1400 )
#define __TBB_ALIGNAS_PRESENT \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1500 )
#define __TBB_CPP11_TEMPLATE_ALIASES_PRESENT \
    ( __INTEL_CXX11_MODE__ && __INTEL_COMPILER >= 1210 )
...
#define __TBB_CPP17_DEDUCTION_GUIDES_PRESENT \
    ( __INTEL_COMPILER > 1910 ) // a future version
#define __TBB_CPP17_INVOKE_RESULT_PRESENT ( __cplusplus >= 201703L )
#elif __clang__
/** TODO: these options need to be rechecked **/
#define __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT \
    __has_feature( __cxx_variadic_templates__ )
#define __TBB_CPP11_RVALUE_REF_PRESENT              \
    ( __has_feature( __cxx_rvalue_references__ ) && \
      ( _LIBCPP_VERSION || __TBB_GLIBCXX_VERSION >= 40500 ) )
...
#define __TBB_EXCEPTION_PTR_PRESENT \
    ( __cplusplus >= 201103L &&     \
      ( _LIBCPP_VERSION || __TBB_GLIBCXX_VERSION >= 40600 ) )
...
#define __TBB_DEFAULTED_AND_DELETED_FUNC_PRESENT      \
    ( __has_feature( __cxx_defaulted_functions__ ) && \
      __has_feature( __cxx_deleted_functions__ ) )
/**For some unknown reason  __has_feature(__cxx_noexcept) does not yield true
 * for all cases. Compiler bug ? **/
#define __TBB_NOEXCEPT_PRESENT ( __cplusplus >= 201103L )
#define __TBB_CPP11_STD_BEGIN_END_PRESENT   \
    ( __has_feature( __cxx_range_for__ ) && \
      ( _LIBCPP_VERSION || __TBB_GLIBCXX_VERSION >= 40600 ) )
...
#define __TBB_CPP17_DEDUCTION_GUIDES_PRESENT \
    ( __has_feature( __cpp_deduction_guides ) )
#define __TBB_CPP17_INVOKE_RESULT_PRESENT \
...
#define __TBB_CPP11_VARIADIC_FIXED_LENGTH_EXP_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40700 )
#define __TBB_CPP11_RVALUE_REF_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40500 )
#define __TBB_IMPLICIT_MOVE_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40600 )
/** __GCC_HAVE_SYNC_COMPARE_AND_SWAP_4 here is a substitution for
   _GLIBCXX_ATOMIC_BUILTINS_4, which is a prerequisite for exception_ptr but
   cannot be used in this file because it is defined in a header, not by the
   compiler. If the compiler has no atomic intrinsics, the C++ library should
   not expect those as well. **/
#define __TBB_EXCEPTION_PTR_PRESENT                               \
...
/** gcc seems have to support constexpr from 4.4 but tests in (test_atomic)
 * seeming reasonable fail to compile prior 4.6**/
#define __TBB_CONSTEXPR_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40400 )
#define __TBB_DEFAULTED_AND_DELETED_FUNC_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40400 )
...
#define __TBB_OVERRIDE_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40700 )
#define __TBB_ALIGNAS_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40800 )
#define __TBB_CPP11_TEMPLATE_ALIASES_PRESENT \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GCC_VERSION >= 40700 )
#define __TBB_CPP14_INTEGER_SEQUENCE_PRESENT \
    ( __cplusplus >= 201402L && __TBB_GCC_VERSION >= 50000 )
#define __TBB_CPP14_VARIABLE_TEMPLATES_PRESENT \
    ( __cplusplus >= 201402L && __TBB_GCC_VERSION >= 50000 )
#define __TBB_CPP17_DEDUCTION_GUIDES_PRESENT \
    ( __cpp_deduction_guides >= 201606L )
#define __TBB_CPP17_INVOKE_RESULT_PRESENT \
    ( __cplusplus >= 201703L && __TBB_GCC_VERSION >= 70000 )
#elif _MSC_VER
// These definitions are also used with Intel C++ Compiler in "default" mode
// (__INTEL_CXX11_MODE__ == 0); see a comment in "__INTEL_COMPILER" section
// above.

#define __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT ( _MSC_VER >= 1800 )
// Contains a workaround for ICC 13
#define __TBB_CPP11_RVALUE_REF_PRESENT \
    ( _MSC_VER >= 1700 && ( !__INTEL_COMPILER || __INTEL_COMPILER >= 1400 ) )
...
#define __TBB_CPP17_DEDUCTION_GUIDES_PRESENT \
    ( _MSVC_LANG >= 201703L && _MSC_VER >= 1914 )
#define __TBB_CPP17_INVOKE_RESULT_PRESENT \
    ( _MSVC_LANG >= 201703L && _MSC_VER >= 1911 )
#else
...
#define __TBB_CPP11_ARRAY_PRESENT            \
    ( _MSC_VER >= 1700 || _LIBCPP_VERSION || \
      __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GLIBCXX_VERSION >= 40300 )
...
#define __TBB_CPP11_IS_COPY_CONSTRUCTIBLE_PRESENT                     \
    ( __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GLIBCXX_VERSION >= 40700 || \
      __TBB_CPP11_TYPE_PROPERTIES_PRESENT )

// In GCC, std::move_if_noexcept appeared later than noexcept
#define __TBB_MOVE_IF_NOEXCEPT_PRESENT                              \
    ( __TBB_NOEXCEPT_PRESENT && ( __TBB_GLIBCXX_VERSION >= 40700 || \
                                  _MSC_VER >= 1900 || _LIBCPP_VERSION ) )
#define __TBB_ALLOCATOR_TRAITS_PRESENT                                 \
    ( __cplusplus >= 201103L && _LIBCPP_VERSION || _MSC_VER >= 1800 || \
      __GXX_EXPERIMENTAL_CXX0X__ && __TBB_GLIBCXX_VERSION >= 40700 &&  \
          !( __TBB_GLIBCXX_VERSION == 40700 && __TBB_DEFINE_MIC ) )
#define __TBB_MAKE_EXCEPTION_PTR_PRESENT                      \
    ( __TBB_EXCEPTION_PTR_PRESENT &&                          \
      ( _MSC_VER >= 1700 || __TBB_GLIBCXX_VERSION >= 40600 || \
        _LIBCPP_VERSION || __SUNPRO_CC ) )

// Due to libc++ limitations in C++03 mode, do not pass rvalues to
// std::make_shared()
#define __TBB_CPP11_SMART_POINTERS_PRESENT                          \
    ( _MSC_VER >= 1600 || _LIBCPP_VERSION ||                        \
      ( ( __cplusplus >= 201103L || __GXX_EXPERIMENTAL_CXX0X__ ) && \
        ( __TBB_GLIBCXX_VERSION >= 40500 ||                         \
          __TBB_GLIBCXX_VERSION >= 40400 && __TBB_USE_OPTIONAL_RTTI ) ) )

#define __TBB_CPP11_FUTURE_PRESENT                                    \
    ( _MSC_VER >= 1700 ||                                             \
      __TBB_GLIBCXX_VERSION >= 40600 && __GXX_EXPERIMENTAL_CXX0X__ || \
      _LIBCPP_VERSION )

#define __TBB_CPP11_GET_NEW_HANDLER_PRESENT                           \
    ( _MSC_VER >= 1900 ||                                             \
      __TBB_GLIBCXX_VERSION >= 40900 && __GXX_EXPERIMENTAL_CXX0X__ || \
      _LIBCPP_VERSION )

#define __TBB_CPP17_UNCAUGHT_EXCEPTIONS_PRESENT                           \
    ( _MSC_VER >= 1900 || __GLIBCXX__ && __cpp_lib_uncaught_exceptions || \
      _LIBCPP_VERSION >= 3700 && ( !__TBB_MACOS_TARGET_VERSION ||         \
                                   __TBB_MACOS_TARGET_VERSION >= 101200 ) )
// TODO: wait when memory_resource will be fully supported in clang and define
// the right macro Currently it is in experimental stage since 6 version.
...
// std::swap is in <utility> only since C++11, though MSVC had it at least since
// VS2005
...
//  On linux ICC fails to find existing std::exception_ptr in libstdc++ without
//  this define
#if __INTEL_COMPILER && __GNUC__ && __TBB_EXCEPTION_PTR_PRESENT && \
    !defined( __GCC_HAVE_SYNC_COMPARE_AND_SWAP_4 )
#define __GCC_HAVE_SYNC_COMPARE_AND_SWAP_4 1
#endif

// Work around a bug in MinGW32
#if __MINGW32__ && __TBB_EXCEPTION_PTR_PRESENT && \
...
// After GCC7 there was possible reordering problem in generic atomic load/store
// operations. So always using builtins.
...
#define __TBB_TSX_INTRINSICS_PRESENT                                 \
    ( ( __RTM__ || _MSC_VER >= 1700 || __INTEL_COMPILER >= 1300 ) && \
      !__TBB_DEFINE_MIC && !__ANDROID__ )

/** Macro helpers **/
#define __TBB_CONCAT_AUX( A, B ) A##B
// The additional level of indirection is needed to expand macros A and B (not
// to get the AB macro). See [cpp.subst] and [cpp.concat] for more details.
#define __TBB_CONCAT( A, B ) __TBB_CONCAT_AUX( A, B )
// The IGNORED argument and comma are needed to always have 2 arguments (even
// when A is empty).
#define __TBB_IS_MACRO_EMPTY( A, IGNORED ) \
...
There are four cases that are supported:
  ...
  2. "_DEBUG defined to something that is evaluated to 0" (including "garbage",
as per [cpp.cond]) means "no debug";
  3. "_DEBUG defined to something that is evaluated to a non-zero value" means
"debug";
...
#if __clang__
/* Old versions of Intel C++ Compiler do not have __has_include or cannot use it
 * in #define */
#if ( __INTEL_COMPILER &&          \
...
#define TBB_IMPLEMENT_CPP0X \
    ( __TBB_GCC_VERSION < 40400 || !__GXX_EXPERIMENTAL_CXX0X__ )
#elif _MSC_VER
#define TBB_IMPLEMENT_CPP0X ( _MSC_VER < 1700 )
#else
// TODO: Reconsider general approach to be more reliable, e.g. (!(__cplusplus >=
// 201103L && __ STDC_HOSTED__))
#define TBB_IMPLEMENT_CPP0X ( !__STDCPP_THREADS__ )
#endif
#endif /* TBB_IMPLEMENT_CPP0X */

/* TBB_USE_CAPTURED_EXCEPTION should be explicitly set to either 0 or 1, as it
 * is used as C++ const */
...
#define __TBB_CONCURRENT_ORDERED_CONTAINERS_PRESENT                            \
    ( __TBB_CPP11_RVALUE_REF_PRESENT &&                                        \
      __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT && __TBB_IMPLICIT_MOVE_PRESENT && \
      __TBB_CPP11_AUTO_PRESENT && __TBB_CPP11_LAMBDAS_PRESENT &&               \
      __TBB_CPP11_ARRAY_PRESENT && __TBB_INITIALIZER_LISTS_PRESENT )
#endif

/** __TBB_WEAK_SYMBOLS_PRESENT denotes that the system supports the weak symbol
 * mechanism **/
#ifndef __TBB_WEAK_SYMBOLS_PRESENT
#define __TBB_WEAK_SYMBOLS_PRESENT       \
    ( !_WIN32 && !__APPLE__ && !__sun && \
      ( __TBB_GCC_VERSION >= 40000 || __INTEL_COMPILER ) )
#endif

/** __TBB_DYNAMIC_LOAD_ENABLED describes the system possibility to load shared
...
**/
#if ( _WIN32 || _WIN64 ) && \
    ( __TBB_SOURCE_DIRECTLY_INCLUDED || TBB_USE_PREVIEW_BINARY )
...
// Intel C++ Compiler starts analyzing usages of the deprecated content at the
// template instantiation site, which is too late for suppression of the
// corresponding messages for internal stuff.
#if !defined( __INTEL_COMPILER ) &&                   \
    ( !defined( TBB_SUPPRESS_DEPRECATED_MESSAGES ) || \
...
#endif
#endif // !defined(TBB_SUPPRESS_DEPRECATED_MESSAGES) ||
       // (TBB_SUPPRESS_DEPRECATED_MESSAGES == 0)
...
/** __TBB_WIN8UI_SUPPORT enables support of Windows* Store Apps and limit a
   possibility to load shared libraries at run time only from application
   container **/
// TODO: Separate this single macro into two for Windows 8 Store* (win8ui mode)
// and UWP/UWD modes.
...
#if __SIZEOF_POINTER__ < 8 && __ANDROID__ && __TBB_GCC_VERSION <= 40403 && \
    !__GCC_HAVE_SYNC_COMPARE_AND_SWAP_8
/** Necessary because on Android 8-byte CAS and F&A are not available for some
   processor architectures, but no mandatory warning message appears from
   GCC 4.4.3. Instead, only a linkage error occurs when these atomic operations
   are used (such as in unit test test_atomic.exe). **/
#define __TBB_GCC_64BIT_ATOMIC_BUILTINS_BROKEN 1
#elif __TBB_x86_32 && __TBB_GCC_VERSION == 40102 && !__GNUC_RH_RELEASE__
/** GCC 4.1.2 erroneously emit call to external function for 64 bit sync_
   intrinsics. However these functions are not defined anywhere. It seems that
   this problem was fixed later on and RHEL got an updated version of gcc 4.1.2.
   **/
#define __TBB_GCC_64BIT_ATOMIC_BUILTINS_BROKEN 1
#endif
...
#define __TBB_ICC_12_0_INL_ASM_FSTCW_BROKEN 1
    ...
    ( __INTEL_COMPILER < 1110 ||    \
      __INTEL_COMPILER == 1110 && __INTEL_COMPILER_BUILD_DATE < 20091012 )
...
#if !__INTEL_COMPILER && \
    ( _MSC_VER && _MSC_VER < 1500 || __GNUC__ && __TBB_GCC_VERSION < 40102 )
/** gcc 3.4.6 (and earlier) and VS2005 (and earlier) do not allow declaring
   template class as a friend of classes defined in other namespaces. **/
...
/** That's a bug in Intel C++ Compiler 11.1.044/IA-32 architecture/Windows* OS,
 * that leads to a worker thread crash on the thread's startup. **/
#define __TBB_ICL_11_1_CODE_GEN_BROKEN 1
#endif

#if __clang__ || \
    ( __GNUC__ == 3 && __GNUC_MINOR__ == 3 && !defined( __INTEL_COMPILER ) )
...
/** The Intel C++ Compiler for IA-32 architecture (Linux* OS|macOS) crashes or
   generates incorrect code when __asm__ arguments have a cast to volatile. **/
#define __TBB_ICC_ASM_VOLATILE_BROKEN 1
#endif

#if !__INTEL_COMPILER && \
    ( _MSC_VER && _MSC_VER < 1700 || __GNUC__ == 3 && __GNUC_MINOR__ <= 2 )
...
#define __TBB_MAIN_THREAD_AFFINITY_BROKEN 1
    ...
    ( ( !__INTEL_COMPILER && !__clang__ &&                               \
        ( __TBB_GCC_VERSION >= 40400 && __TBB_GCC_VERSION < 40600 ) ) || \
      ( __INTEL_COMPILER <= 1400 && ( __TBB_GLIBCXX_VERSION >= 40400 &&  \
                                      __TBB_GLIBCXX_VERSION <= 40801 ) ) )
...
#if __INTEL_COMPILER == 1300 && __TBB_GLIBCXX_VERSION >= 40700 && \
    defined( __GXX_EXPERIMENTAL_CXX0X__ )
/* Some C++11 features used inside libstdc++ are not supported by Intel C++
 * Compiler. */
...
/* In a PIC mode some versions of GCC 4.1.2 generate incorrect inlined code for
 * 8 byte __sync_val_compare_and_swap intrinsic */
...
#define __TBB_GCC_CAS8_BUILTIN_INLINING_BROKEN 1
    ...
    ( __INTEL_COMPILER ||                         \
      ...
      ( __MINGW32__ && __GNUC__ == 4 && __GNUC_MINOR__ == 5 ) || __SUNPRO_CC )
// Some compilers for IA-32 architecture fail to provide 8-byte alignment of
// objects on the stack, even if the object specifies 8-byte alignment. On such
// platforms, the implementation of 64 bit atomics for IA-32 architecture (e.g.
// atomic<long long>) use different tactics depending upon whether the object is
// properly aligned or not.
#define __TBB_FORCE_64BIT_ALIGNMENT_BROKEN 1
#else
// Define to 0 explicitly because the macro is used in a compiled code of
// test_atomic
#define __TBB_FORCE_64BIT_ALIGNMENT_BROKEN 0
#endif

#if __GNUC__ && !__INTEL_COMPILER && !__clang__ && \
    __TBB_DEFAULTED_AND_DELETED_FUNC_PRESENT && __TBB_GCC_VERSION < 40700
...
// constness from the function parameter may get propagated to the template
// parameter.
#define __TBB_CONST_REF_TO_ARRAY_TEMPLATE_PARAM_BROKEN 1
#endif

// A compiler bug: a disabled copy constructor prevents use of the moving
// constructor
#define __TBB_IF_NO_COPY_CTOR_MOVE_SEMANTICS_BROKEN                           \
    ( _MSC_VER && ( __INTEL_COMPILER >= 1300 && __INTEL_COMPILER <= 1310 ) && \
      !__INTEL_CXX11_MODE__ )

#define __TBB_CPP11_DECLVAL_BROKEN \
    ( _MSC_VER == 1600 || ( __GNUC__ && __TBB_GCC_VERSION < 40500 ) )
// Intel C++ Compiler has difficulties with copying std::pair with VC11
// std::reference_wrapper being a const member
#define __TBB_COPY_FROM_NON_CONST_REF_BROKEN \
    ( _MSC_VER == 1700 && __INTEL_COMPILER && __INTEL_COMPILER < 1600 )

// The implicit upcasting of the tuple of a reference of a derived class to a
// base class fails on icc 13.X if the system's gcc environment is 4.8 Also in
// gcc 4.4 standard library the implementation of the tuple<&> conversion
// (tuple<A&> a = tuple<B&>, B is inherited from A) is broken.
#if __GXX_EXPERIMENTAL_CXX0X__ && __GLIBCXX__ &&                \
    ( ( __INTEL_COMPILER >= 1300 && __INTEL_COMPILER <= 1310 && \
        __TBB_GLIBCXX_VERSION >= 40700 ) ||                     \
      ( __TBB_GLIBCXX_VERSION < 40500 ) )
#define __TBB_UPCAST_OF_TUPLE_OF_REF_BROKEN 1
#endif

// In some cases decltype of a function adds a reference to a return type.
#define __TBB_CPP11_DECLTYPE_OF_FUNCTION_RETURN_TYPE_BROKEN \
    ( _MSC_VER == 1600 && !__INTEL_COMPILER )

// Visual Studio 2013 does not delete the copy constructor when a user-defined
...
// A macro to suppress erroneous or benign "unreachable code" MSVC warning
// (4702)
#define __TBB_MSVC_UNREACHABLE_CODE_IGNORED 1
#endif

#define __TBB_ATOMIC_CTORS                                                   \
    ( __TBB_CONSTEXPR_PRESENT && __TBB_DEFAULTED_AND_DELETED_FUNC_PRESENT && \
      ( !__TBB_ZERO_INIT_WITH_DEFAULTED_CTOR_BROKEN ) )

// Many OS versions (Android 4.0.[0-3] for example) need workaround for dlopen
// to avoid non-recursive loader lock hang Setting the workaround for all
// compile targets ($APP_PLATFORM) below Android 4.4 (android-19)
#if __ANDROID__
#include <android/api-level.h>
#define __TBB_USE_DLOPEN_REENTRANCY_WORKAROUND ( __ANDROID_API__ < 19 )
#endif

#define __TBB_ALLOCATOR_CONSTRUCT_VARIADIC \
    ( __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT && __TBB_CPP11_RVALUE_REF_PRESENT )

#define __TBB_VARIADIC_PARALLEL_INVOKE          \
    ( TBB_PREVIEW_VARIADIC_PARALLEL_INVOKE &&   \
      __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT && \
      __TBB_CPP11_RVALUE_REF_PRESENT )
#define __TBB_FLOW_GRAPH_CPP11_FEATURES                                       \
    ( __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT &&                               \
      __TBB_CPP11_SMART_POINTERS_PRESENT && __TBB_CPP11_RVALUE_REF_PRESENT && \
...
#define __TBB_PREVIEW_STREAMING_NODE                                     \
    ( __TBB_CPP11_VARIADIC_FIXED_LENGTH_EXP_PRESENT &&                   \
      __TBB_FLOW_GRAPH_CPP11_FEATURES && TBB_PREVIEW_FLOW_GRAPH_NODES && \
      !TBB_IMPLEMENT_CPP0X && !__TBB_UPCAST_OF_TUPLE_OF_REF_BROKEN )
#define __TBB_PREVIEW_OPENCL_NODE \
    ( __TBB_PREVIEW_STREAMING_NODE && __TBB_CPP11_TEMPLATE_ALIASES_PRESENT )
#define __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING \
    ( TBB_PREVIEW_FLOW_GRAPH_FEATURES || __TBB_PREVIEW_OPENCL_NODE )
#define __TBB_PREVIEW_ASYNC_MSG \
    ( TBB_PREVIEW_FLOW_GRAPH_FEATURES && __TBB_FLOW_GRAPH_CPP11_FEATURES )

#ifndef __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
#define __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES TBB_PREVIEW_FLOW_GRAPH_FEATURES
#endif

// This feature works only in combination with critical tasks
// (__TBB_PREVIEW_CRITICAL_TASKS)
#ifndef __TBB_PREVIEW_RESUMABLE_TASKS
#define __TBB_PREVIEW_RESUMABLE_TASKS                       \
    ( ( __TBB_CPF_BUILD || TBB_PREVIEW_RESUMABLE_TASKS ) && \
      !__TBB_WIN8UI_SUPPORT && !__ANDROID__ && !__TBB_ipf )
#endif

#ifndef __TBB_PREVIEW_CRITICAL_TASKS
#define __TBB_PREVIEW_CRITICAL_TASKS                            \
    ( __TBB_CPF_BUILD || __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES || \
      __TBB_PREVIEW_RESUMABLE_TASKS )
#endif

#ifndef __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
#define __TBB_PREVIEW_FLOW_GRAPH_NODE_SET                       \
    ( TBB_PREVIEW_FLOW_GRAPH_FEATURES && __TBB_CPP11_PRESENT && \
