...
#if !defined( __TBB_show_deprecation_message_tbb_thread_H ) && \
    defined( __TBB_show_deprecated_header_message )
#define __TBB_show_deprecation_message_tbb_thread_H
#pragma message( \
    "TBB Warning: tbb/tbb_thread.h is deprecated. For details, please see Deprecated Features appendix in the TBB reference manual." )
#endif
...
template < class F >
struct thread_closure_0 : thread_closure_base {
    F function;
...
public:
    ...
    tbb_thread_v3( F f, X x, Y y ) {
        ...
        internal_start( closure_type::start_routine,
...
private:
    ...
    /** Runs start_routine(closure) on another thread and sets my_handle to the
     * handle of the created thread. */
    void __TBB_EXPORTED_METHOD
    internal_start( __TBB_NATIVE_THREAD_ROUTINE_PTR( start_routine ),
                    void* closure );
...
public:
    ...
    friend inline size_t tbb_hasher( const tbb_thread_v3::id& id ) {
        __TBB_STATIC_ASSERT( sizeof( id.my_id ) <= sizeof( size_t ),
                             "Implementation assumes that thread_id_type fits "
                             "into machine word" );
        return tbb::tbb_hasher( id.my_id );
    }

    // A workaround for lack of tbb::atomic<id> (which would require id to be
    // POD in C++03).
    friend id atomic_compare_and_swap( id& location,
                                       const id& value,
                                       const id& comparand ) {
        return as_atomic( location.my_id )
            .compare_and_swap( value.my_id, comparand.my_id );
    }
}; // tbb_thread_v3::id
...
__TBB_DEPRECATED_VERBOSE_MSG( "tbb::thread is deprecated, use std::thread" )
