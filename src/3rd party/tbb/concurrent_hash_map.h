...
#include <stdio.h>
    ...
    __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT && __TBB_CPP11_TUPLE_PRESENT
// Definition of __TBB_CPP11_RVALUE_REF_PRESENT includes
// __TBB_CPP11_TUPLE_PRESENT for most of platforms, tuple present macro was
// added for logical correctness
#include <tuple>
#endif
...
public:
    ...
    atomic< size_type > my_size; // It must be in separate cache line from
    ...
    //! Initialize buckets
    static void init_buckets( segment_ptr_t ptr,
                              size_type sz,
                              bool is_initial ) {
        if ( is_initial )
    ...
    void enable_segment( segment_index_t k,
                         const Allocator& allocator,
                         bool is_initial = false ) {
        typedef
            typename tbb::internal::allocator_rebind< Allocator, bucket >::type
                bucket_allocator_type;
        typedef tbb::internal::allocator_traits< bucket_allocator_type >
        ...
        size_type sz;
        __TBB_ASSERT( !is_valid( my_table[ k ] ),
                      "Wrong concurrent assignment" );
        if ( k >= first_block ) {
            sz = segment_size( k );
            segment_ptr_t ptr =
                bucket_allocator_traits::allocate( bucket_allocator, sz );
            ...
            segment_ptr_t ptr = bucket_allocator_traits::allocate(
                bucket_allocator, sz - embedded_buckets );
            init_buckets( ptr, sz - embedded_buckets, is_initial );
            ptr -= segment_base( embedded_block );
            for ( segment_index_t i = embedded_block; i < first_block;
                  i++ ) // calc the offsets
    ...
    bucket* get_bucket( hashcode_t h ) const
        throw() { // TODO: add throw() everywhere?
        ...
        while ( segment_ptr_t seg = my_table[ ++s ] )
            if ( seg[ h ].node_list == rehash_req ) {
                ...
                mark_rehashed_levels(
                    h + ( ( hashcode_t )1 << s ) ); // optimized segment_base(s)
            }
    }
    ...
    bool check_rehashing_collision( const hashcode_t h,
                                    hashcode_t m_old,
                                    hashcode_t m ) const {
        __TBB_ASSERT(
            m_old != m,
            NULL ); // TODO?: m arg could be optimized out by passing h = h&m
        if ( ( h & m_old ) !=
             ( h & m ) ) { // mask changed for this hashcode, rare event
            // condition above proves that 'h' has some other bits set beside
            // 'm_old' find next applicable mask after m_old    //TODO: look at
            // bsl instruction
            for ( ++m_old; !( h & m_old );
                  m_old <<=
            ...
            if ( itt_load_word_with_acquire(
                     get_bucket( h & m_old )->node_list ) != rehash_req ) {
#if __TBB_STATISTICS
    ...
    //! Insert a node and check for load factor. @return segment index to
    //! enable.
    segment_index_t insert_new_node( bucket* b,
                                     node_base* n,
                                     hashcode_t mask ) {
        size_type sz = ++my_size; // prefix form is to enforce allocation after
                                  // the first item inserted
        add_to_bucket( b, n );
        // check load factor
        if ( sz >= mask ) { // TODO: add custom load_factor
            segment_index_t new_seg =
                __TBB_Log2( mask + 1 ); // optimized segment_index_of
            __TBB_ASSERT( is_valid( my_table[ new_seg - 1 ] ),
                          "new allocations must not publish new mask until "
                          "segment has allocated" );
            static const segment_ptr_t is_allocating = ( segment_ptr_t )2;
            if ( !itt_hide_load_word( my_table[ new_seg ] ) &&
                 as_atomic( my_table[ new_seg ] )
                         .compare_and_swap( is_allocating, NULL ) == NULL )
    ...
    void internal_swap( hash_map_base& table ) {
        ...
        for ( size_type i = 0; i < embedded_buckets; i++ )
            swap( this->my_embedded_segment[ i ].node_list,
    ...
    void internal_move( hash_map_base&& other ) {
        ...
        for ( size_type i = 0; i < embedded_buckets; ++i ) {
            my_embedded_segment[ i ].node_list =
                other.my_embedded_segment[ i ].node_list;
...
public: // workaround
    ...
    hash_map_iterator( const Container& map,
                       size_t index,
                       const bucket* b,
                       node_base* n );
...
template < typename Container, typename Value >
hash_map_iterator< Container, Value >::hash_map_iterator( const Container& map,
                                                          size_t index,
                                                          const bucket* b,
                                                          node_base* n )
    : my_map( &map ),
...
template < typename Container, typename Value >
hash_map_iterator< Container, Value >&
hash_map_iterator< Container, Value >::operator++() {
...
@par Compatibility
    The class meets all Container Requirements from C++ Standard (See ISO/IEC
14882:2003(E), clause 23.1).

@par Exception Safety
    - Hash function is not permitted to throw an exception. User-defined types
Key and T are forbidden from throwing an exception in destructors.
    - If exception happens during insert() operations, it has no effect (unless
exception raised by HashCompare::hash() function during grow_segment).
    - If exception happens during operator=() operation, the container can have
a part of source items, and methods size() and empty() can return wrong results.
...
public:
    ...
    typedef internal::hash_map_iterator< concurrent_hash_map, value_type >
        iterator;
    typedef internal::hash_map_iterator< concurrent_hash_map, const value_type >
        const_iterator;
...
protected:
    ...
    class node;
    typedef typename tbb::internal::allocator_rebind< Allocator, node >::type
        node_allocator_type;
    typedef tbb::internal::allocator_traits< node_allocator_type >
    ...
    void delete_node( node_base* n ) {
        node_allocator_traits::destroy( my_allocator,
                                        static_cast< node* >( n )->storage() );
        node_allocator_traits::destroy( my_allocator,
                                        static_cast< node* >( n ) );
        node_allocator_traits::deallocate( my_allocator,
                                           static_cast< node* >( n ), 1 );
    }
    ...
    static node* create_node( node_allocator_type& allocator,
                              __TBB_FORWARDING_REF( Arg1 ) arg1,
                              __TBB_FORWARDING_REF( Arg2 ) arg2 )
#endif
    {
...
#if __TBB_CPP11_RVALUE_REF_PRESENT && __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT
        node_allocator_traits::construct( allocator, node_ptr->storage(),
                                          std::forward< Args >( args )... );
#else
        node_allocator_traits::construct(
            allocator, node_ptr->storage(),
            tbb::internal::forward< Arg1 >( arg1 ),
            tbb::internal::forward< Arg2 >( arg2 ) );
#endif
        guard.dismiss();
        return node_ptr;
    }

    static node* allocate_node_copy_construct( node_allocator_type& allocator,
                                               const Key& key,
                                               const T* t ) {
...
#if __TBB_CPP11_RVALUE_REF_PRESENT
    static node* allocate_node_move_construct( node_allocator_type& allocator,
                                               const Key& key,
                                               const T* t ) {
        return create_node( allocator, key,
                            std::move( *const_cast< T* >( t ) ) );
    }
#endif

    static node* allocate_node_default_construct(
...
#else
        // Use of a temporary object is impossible, because create_node takes a
        // non-const reference. copy-initialization is possible because T is
        // already required to be CopyConstructible.
        T obj = T();
        return create_node( allocator, key, tbb::internal::move( obj ) );
#endif
    }

    static node* do_not_allocate_node( node_allocator_type&,
                                       const Key&,
                                       const T* ) {
    ...
    node* search_bucket( const key_type& key, bucket* b ) const {
        ...
        while ( is_valid( n ) &&
                !my_hash_compare.equal( key, n->value().first ) )
            n = static_cast< node* >( n->next );
        __TBB_ASSERT( n != internal::rehash_req,
    ...
    public:
        ...
        inline void acquire( concurrent_hash_map* base,
            ...
            // double-check
            if ( itt_load_word_with_acquire( my_b->node_list ) ==
                     internal::rehash_req &&
    ...
    void rehash_bucket( bucket* b_new, const hashcode_t h ) {
        __TBB_ASSERT( *( intptr_t* )( &b_new->mutex ),
                      "b_new must be locked (for write)" );
        __TBB_ASSERT( h > 1, "The lowermost buckets can't be rehashed" );
        __TBB_store_with_release( b_new->node_list,
                                  internal::empty_rehashed ); // mark rehashed
        hashcode_t mask = ( 1u << __TBB_Log2( h ) ) -
    ...
    restart:
        for ( node_base **p = &b_old()->node_list,
                        *n = __TBB_load_with_acquire( *p );
              is_valid( n ); n = *p ) {
            hashcode_t c = my_hash_compare.hash(
                static_cast< node* >( n )->value().first );
#if TBB_USE_ASSERT
            hashcode_t bmask = h & ( mask >> 1 );
            bmask = bmask == 0 ? 1
                               : ( 1u << ( __TBB_Log2( bmask ) + 1 ) ) -
                                     1; // minimal mask of parent bucket
            __TBB_ASSERT( ( c & bmask ) == ( h & bmask ),
                          "hash() function changed for key in table" );
#endif
            if ( ( c & mask ) == h ) {
                if ( !b_old.is_writer() )
                    if ( !b_old.upgrade_to_writer() ) {
                        goto restart; // node ptr can be invalid due to
                                      // concurrent erase
                    }
                *p = n->next; // exclude from b_old
                add_to_bucket( b_new, n );
            } else
                p = &n->next; // iterate to next item
        }
    }
...
public:
    ...
    public:
        ...
        ~const_accessor() {
            my_node =
                NULL; // scoped lock's release() is called in its destructor
        }
    ...
    //! Construct empty table with n preallocated buckets. This number serves
    //! also as initial concurrency level.
    ...
    concurrent_hash_map( concurrent_hash_map&& table )
        : internal::hash_map_base(),
          my_allocator( std::move( table.get_allocator() ) ) {
    ...
    concurrent_hash_map( concurrent_hash_map&& table, const allocator_type& a )
        ...
        if ( a == table.get_allocator() ) {
            ...
            call_clear_on_leave scope_guard( this );
            internal_copy( std::make_move_iterator( table.begin() ),
    ...
    concurrent_hash_map& operator=( const concurrent_hash_map& table ) {
        if ( this != &table ) {
            typedef typename node_allocator_traits::
                propagate_on_container_copy_assignment pocca_type;
            clear();
            tbb::internal::allocator_copy_assignment(
                my_allocator, table.my_allocator, pocca_type() );
            internal_copy( table );
        }
        return *this;
    }
    ...
    size_type count( const Key& key ) const {
        return const_cast< concurrent_hash_map* >( this )->lookup(
            /*insert*/ false, key, NULL, NULL, /*write=*/false,
    ...
    bool find( const_accessor& result, const Key& key ) const {
        result.release();
        return const_cast< concurrent_hash_map* >( this )->lookup(
            /*insert*/ false, key, NULL, &result, /*write=*/false,
            &do_not_allocate_node );
    }
    ...
    //! Insert item (if not already present) and acquire a read lock on the
    //! item.
    ...
    //! Insert item (if not already present) and acquire a write lock on the
    //! item.
    /** Returns true if item is new. */
    ...
    bool insert( const value_type& value ) {
        return lookup( /*insert*/ true, value.first, &value.second, NULL,
                       /*write=*/false, &allocate_node_copy_construct );
    }
    ...
    /** Return true if item was erased by particularly this call. */
    bool erase( accessor& item_accessor ) { return exclude( item_accessor ); }

protected:
    //! Insert or find item and optionally acquire a lock on the item.
    bool lookup( bool op_insert,
                 const Key& key,
                 const T* t,
    ...
    bool generic_emplace( Accessor&& result, Args&&... args ) {
        result.release();
        node* node_ptr =
            create_node( my_allocator, std::forward< Args >( args )... );
        return lookup( /*insert*/ true, node_ptr->value().first, NULL,
                       accessor_location( result ),
                       is_write_access_needed( result ), &do_not_allocate_node,
                       node_ptr );
    }
#endif //__TBB_CPP11_VARIADIC_TEMPLATES_PRESENT
#endif //__TBB_CPP11_RVALUE_REF_PRESENT
...
#if __TBB_CPP11_RVALUE_REF_PRESENT
    // A compile-time dispatch to allow move assignment of containers with
    // non-movable value_type if POCMA is true_type
    ...
    //! Fast find when no concurrent erasure is used. For internal use inside
    //! TBB only!
    ...
    restart:
        ...
        // TODO: actually, notification is unnecessary here, just hiding
        // double-check
        if ( itt_load_word_with_acquire( b->node_list ) ==
             internal::rehash_req ) {
...
using hash_map_t = Map<
    ...
    std::conditional_t< ( sizeof...( Args ) > 0 ) &&
                            !is_allocator_v< pack_element_t< 0, Args... > >,
                        pack_element_t< 0, Args... >,
                        tbb_hash_compare< Key > >,
    std::conditional_t<
        ( sizeof...( Args ) > 0 ) &&
            is_allocator_v< pack_element_t< sizeof...( Args ) - 1, Args... > >,
        pack_element_t< sizeof...( Args ) - 1, Args... >,
        tbb_allocator< std::pair< const Key, T > > > >;
} // namespace internal

// Deduction guide for the constructor from two iterators and hash_compare/
// allocator
template < typename I, typename... Args >
concurrent_hash_map( I, I, Args... )
    ->internal::hash_map_t< concurrent_hash_map,
                            internal::iterator_key_t< I >,
                            internal::iterator_mapped_t< I >,
                            Args... >;

// Deduction guide for the constructor from an initializer_list and
// hash_compare/ allocator Deduction guide for an initializer_list, hash_compare
// and allocator is implicit
...
bool concurrent_hash_map< Key, T, HashCompare, A >::lookup(
    ...
    const_accessor* result,
    bool write,
    node* ( *allocate_node )( node_allocator_type&, const Key&, const T* ),
    node* tmp_n ) {
    ...
    if ( op_insert ) {
        ...
        if ( !n ) {
            ...
            if ( !b.is_writer() &&
                ...
                // Rerun search_list, in case another thread inserted the item
                // during the upgrade.
                n = search_bucket( key, b() );
...
template < typename I >
std::pair< I, I >
concurrent_hash_map< Key, T, HashCompare, A >::internal_equal_range(
    const Key& key,
    I end_ ) const {
    hashcode_t h = my_hash_compare.hash( key );
...
template < typename Key, typename T, typename HashCompare, typename A >
bool concurrent_hash_map< Key, T, HashCompare, A >::exclude(
...
search:
    ...
    *p = n->next;
    ...
    delete_node(
        n ); // Only one thread can delete it due to write lock on the bucket
    return true;
}

template < typename Key, typename T, typename HashCompare, typename A >
void concurrent_hash_map< Key, T, HashCompare, A >::swap(
    concurrent_hash_map< Key, T, HashCompare, A >& table ) {
    typedef
        typename node_allocator_traits::propagate_on_container_swap pocs_type;
    if ( this != &table &&
         ( pocs_type::value || my_allocator == table.my_allocator ) ) {
        using std::swap;
        tbb::internal::allocator_swap( this->my_allocator, table.my_allocator,
                                       pocs_type() );
...
void concurrent_hash_map< Key, T, HashCompare, A >::rehash( size_type sz ) {
    reserve( sz,
             my_allocator ); // TODO: add reduction of number of buckets as well
    ...
    bucket* bp = get_bucket(
        b ); // only the last segment should be scanned for rehashing
    for ( ; b <= mask; b++, bp++ ) {
        node_base* n = bp->node_list;
        __TBB_ASSERT( is_valid( n ) || n == internal::empty_rehashed ||
                          n == internal::rehash_req,
        ...
        __TBB_ASSERT( *reinterpret_cast< intptr_t* >( &bp->mutex ) == 0,
                      ...
                      "rehash() execution" );
        if ( n == internal::rehash_req ) { // rehash bucket, conditional because
                                           // rehashing of a previous bucket may
                                           // affect this one
            hashcode_t h = b;
            bucket* b_old = bp;
            do {
                ...
                hashcode_t m = ( 1u << __TBB_Log2( h ) ) -
                               1; // get parent mask from the topmost bit
                b_old = get_bucket( h &= m );
            } while ( b_old->node_list == internal::rehash_req );
            // now h - is index of the root rehashed bucket b_old
            mark_rehashed_levels( h ); // mark all non-rehashed children
                                       // recursively across all segments
            for ( node_base **p = &b_old->node_list, *q = *p; is_valid( q );
                  q = *p ) {
                hashcode_t c = my_hash_compare.hash(
                    static_cast< node* >( q )->value().first );
                if ( ( c & mask ) != h ) { // should be rehashed
                    *p = q->next;          // exclude from b_old
                    ...
                    p = &q->next; // iterate to next item
            }
        }
    }
#if TBB_USE_PERFORMANCE_WARNINGS
    int current_size = int( my_size ), buckets = int( mask ) + 1,
        empty_buckets = 0, overpopulated_buckets = 0; // usage statistics
    static bool reported = false;
#endif
#if TBB_USE_ASSERT || TBB_USE_PERFORMANCE_WARNINGS
    for ( b = 0; b <= mask;
          b++ ) { // only last segment should be scanned for rehashing
        if ( b & ( b - 2 ) )
            ++bp; // not the beginning of a segment
        else
            bp = get_bucket( b );
        node_base* n = bp->node_list;
        __TBB_ASSERT( *reinterpret_cast< intptr_t* >( &bp->mutex ) == 0,
        ...
        __TBB_ASSERT( is_valid( n ) || n == internal::empty_rehashed,
                      "Broken internal structure" );
#if TBB_USE_PERFORMANCE_WARNINGS
        if ( n == internal::empty_rehashed )
            empty_buckets++;
        else if ( n->next )
            overpopulated_buckets++;
#endif
#if TBB_USE_ASSERT
        for ( ; is_valid( n ); n = n->next ) {
            hashcode_t h = my_hash_compare.hash(
                               static_cast< node* >( n )->value().first ) &
                           mask;
            __TBB_ASSERT(
                h == b,
    ...
    if ( buckets > current_size )
        empty_buckets -= buckets - current_size;
    else
        overpopulated_buckets -= current_size - buckets; // TODO: load_factor?
    if ( !reported && buckets >= 512 &&
         ( 2 * empty_buckets > current_size ||
           2 * overpopulated_buckets > current_size ) ) {
        tbb::internal::runtime_warning(
            "Performance is not optimal because the hash function produces bad "
            "randomness in lower bits in %s.\nSize: %d  Empties: %d  Overlaps: "
            "%d",
...
void concurrent_hash_map< Key, T, HashCompare, A >::clear() {
...
#if TBB_USE_PERFORMANCE_WARNINGS || __TBB_STATISTICS
    int current_size = int( my_size ), buckets = int( m ) + 1,
        empty_buckets = 0, overpopulated_buckets = 0; // usage statistics
    static bool reported = false;
#endif
    bucket* bp = 0;
    ...
    for ( segment_index_t b = 0; b <= m; b++ ) {
        ...
        for ( ; is_valid( n ); n = n->next ) {
            ...
            h &= m;
            __TBB_ASSERT(
                h == b || get_bucket( h )->node_list == internal::rehash_req,
                "hash() function changed for key in table or internal error" );
        }
#endif
    }
    ...
    printf(
        ...
        current_size, int( m + 1 ), buckets, empty_buckets,
        overpopulated_buckets, unsigned( my_info_resizes ),
        unsigned( my_info_rehashes ), unsigned( my_info_restarts ) );
    my_info_resizes = 0;  // concurrent ones
    my_info_restarts = 0; // race collisions
    my_info_rehashes = 0; // invocations of rehash_bucket
#endif
    ...
    __TBB_ASSERT( s + 1 == pointers_per_table || !my_table[ s + 1 ],
                  "wrong mask or concurrent grow" );
    do {
        __TBB_ASSERT( is_valid( my_table[ s ] ),
                      "wrong mask or concurrent grow" );
        segment_ptr_t buckets_ptr = my_table[ s ];
        size_type sz = segment_size( s ? s : 1 );
        for ( segment_index_t i = 0; i < sz; i++ )
            for ( node_base* n = buckets_ptr[ i ].node_list; is_valid( n );
                  n = buckets_ptr[ i ].node_list ) {
...
template < typename Key, typename T, typename HashCompare, typename A >
void concurrent_hash_map< Key, T, HashCompare, A >::internal_copy(
    ...
    if ( my_mask == mask ) {                     // optimized version
        ...
        internal_copy( source.begin(), source.end(), source.my_size );
}
...
void concurrent_hash_map< Key, T, HashCompare, A >::internal_copy(
    ...
    for ( ; first != last; ++first ) {
        ...
        bucket* b = get_bucket( h & m );
        __TBB_ASSERT( b->node_list != internal::rehash_req,
                      "Invalid bucket in destination table" );
        node* node_ptr =
            create_node( my_allocator, ( *first ).first, ( *first ).second );
