...
public:
    ...
    static void call( const Body& obj,
                      __TBB_FORWARDING_REF( A1 ) arg1,
                      A2& arg2 ) {
        internal_call( obj, tbb::internal::forward< A1 >( arg1 ), arg2,
                       &Body::operator() );
    }
};
...
class do_iteration_task : public task {
    ...
    task* execute() __TBB_override {
        parallel_do_operator_selector< Body, Item >::call(
            *my_feeder.my_body, tbb::internal::move( my_value ), my_feeder );
        return NULL;
    }
...
class do_iteration_task_iter : public task {
    ...
    task* execute() __TBB_override {
        parallel_do_operator_selector< Body, Item >::call(
            *my_feeder.my_body, *my_iter, my_feeder );
        return NULL;
    }
...
class parallel_do_feeder_impl : public parallel_do_feeder< Item > {
    ...
    void internal_add_copy_impl( std::true_type, const Item& item ) {
        ...
        iteration_type& t =
            *new ( task::allocate_additional_child_of( *my_barrier ) )
    ...
    void internal_add_copy( const Item& item ) __TBB_override {
        ...
        internal_add_copy_impl(
            typename std::is_copy_constructible< Item >::type(), item );
#else
    ...
    void internal_add_move( Item&& item ) __TBB_override {
        ...
        iteration_type& t =
            *new ( task::allocate_additional_child_of( *my_barrier ) )
                iteration_type( std::move( item ), *this );
        task::spawn( t );
    }
...
public:
    ...
    ~parallel_do_feeder_impl() { my_barrier->destroy( *my_barrier ); }
}; // class parallel_do_feeder_impl
...
class do_group_task_forward : public task {
    ...
    template < typename Iterator_, typename Body_, typename _Item >
    friend class do_task_iter;
...
class do_group_task_input : public task {
    ...
    task* execute() __TBB_override {
...
#endif
        typedef do_iteration_task_iter< Item_iterator, Body, Item >
            iteration_type;
        __TBB_ASSERT( my_size > 0, NULL );
        task_list list;
        task* t;
        size_t k = 0;
        for ( ;; ) {
            t = new ( allocate_child() ) iteration_type(
                Item_iterator( my_arg.begin() + k ), my_feeder );
            if ( ++k == my_size )
                break;
            list.push_back( *t );
        }
...
private:
    ...
    /* Do not merge run(xxx) and run_xxx() methods. They are separated in order
        ...
        An alternative could be using template functions, but explicit
       specialization of member function templates is not supported for non
       specialized class templates. Besides template functions would always fall
       back to the least efficient variant (the one for input iterators) in case
       of iterators having custom tags derived from basic ones. */
    task* execute() __TBB_override {
    ...
    task* run_for_input_iterator() {
        ...
        block_type& t =
            *new ( allocate_additional_child_of( *my_feeder.my_barrier ) )
                block_type( my_feeder );
        size_t k = 0;
        while ( !( my_first == my_last ) ) {
            // Move semantics are automatically used when supported by the
            // iterator
    ...
    inline task* run( std::forward_iterator_tag* ) {
    ...
    task* run_for_forward_iterator() {
        ...
        return k == 0 ? NULL
                      : new ( allocate_additional_child_of(
                            *my_feeder.my_barrier ) )
                            block_type( first, k, my_feeder );
    }

    inline task* run( std::random_access_iterator_tag* ) {
    ...
    task* run_for_random_access_iterator() {
        ...
        if ( k > block_type::max_arg_size ) {
            ...
            do_task_iter& b = *new ( c.allocate_child() )
                                  do_task_iter( middle, my_last, my_feeder );
            ...
            size_t k1 = 0;
            for ( ;; ) {
                t = new ( allocate_child() )
                    iteration_type( my_first, my_feeder );
                ++my_first;
                if ( ++k1 == k )
                    break;
                list.push_back( *t );
...
void run_parallel_do( Iterator first,
    ...
    root_iteration_task& t = *new ( feeder.my_barrier->allocate_child() )
                                 root_iteration_task( first, last, feeder );
...
void select_parallel_do( Iterator first,
    ...
    run_parallel_do< Iterator, Body,
                     typename ::tbb::internal::strip< Item >::type >( first,
                                                                      last, body
#if __TBB_TASK_GROUP_CONTEXT
                                                                      ,
                                                                      context
...
void select_parallel_do( Iterator first,
                         Iterator last,
                         const Body& body,
                         void ( Body::* )( Item, parallel_do_feeder< _Item >& )
                             const
#if __TBB_TASK_GROUP_CONTEXT
                         ,
                         task_group_context& context
...
#endif
    interface9::internal::select_parallel_do( first, last, body,
                                              &Body::operator()
#if __TBB_TASK_GROUP_CONTEXT
                                                  ,
                                              context
...
void parallel_do( Range& rng, const Body& body ) {
    parallel_do( tbb::internal::first( rng ), tbb::internal::last( rng ),
                 body );
}
...
template < typename Range, typename Body >
void parallel_do( const Range& rng,
                  const Body& body,
                  task_group_context& context ) {
    parallel_do( tbb::internal::first( rng ), tbb::internal::last( rng ), body,
                 context );
