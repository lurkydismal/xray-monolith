...
#if TBB_USE_THREADING_TOOLS && TBB_PREVIEW_FLOW_GRAPH_TRACE && \
    ( __linux__ || __APPLE__ )
#if __INTEL_COMPILER
...
//! An enumeration the provides the two most common concurrency levels:
//! unlimited and serial
...
// Holder of edges both for caches and for those nodes which do not have
// predecessor caches. C == receiver< ... > or sender< ... >, depending.
template < typename C >
class edge_container {
public:
    typedef std::list< C*, tbb::tbb_allocator< C* > > edge_list_type;

    void add_edge( C& s ) { built_edges.push_back( &s ); }

    void delete_edge( C& s ) {
        for ( typename edge_list_type::iterator i = built_edges.begin();
              i != built_edges.end(); ++i ) {
            if ( *i == &s ) {
    ...
    void copy_edges( edge_list_type& v ) { v = built_edges; }

    size_t edge_count() { return ( size_t )( built_edges.size() ); }

    void clear() { built_edges.clear(); }

    // methods remove the statement from all predecessors/successors liste in
    // the edge container.
    template < typename S >
...
// enqueue left task if necessary. Returns the non-enqueued task if there is
// one.
static inline tbb::task* combine_tasks( graph& g,
                                        tbb::task* left,
                                        tbb::task* right ) {
...
struct async_helpers {
    ...
    static void* to_void_ptr( T& t ) { return static_cast< void* >( &t ); }

    static const T& from_void_ptr( const void* p ) {
        return *static_cast< const T* >( p );
    }

    static T& from_void_ptr( void* p ) { return *static_cast< T* >( p ); }

    static task* try_put_task_wrapper_impl( receiver< T >* const this_recv,
                                            const void* p,
                                            bool is_async ) {
        if ( is_async ) {
            // This (T) is NOT async and incoming 'A<X> t' IS async
            // Get data from async_msg
            const async_msg< filtered_type >& msg =
                async_helpers< async_msg< filtered_type > >::from_void_ptr( p );
            task* const new_task = msg.my_storage->subscribe(
                *this_recv, this_recv->graph_reference() );
            // finalize() must be called after subscribe() because set() can be
            // called in finalize() and 'this_recv' client must be subscribed by
            // this moment
            msg.finalize();
            return new_task;
        } else {
...
template < typename T >
struct async_helpers< T,
                      typename std::enable_if< std::is_base_of<
                          async_msg< typename T::async_msg_data_type >,
                          T >::value >::type > {
    ...
    static const void* to_void_ptr( const T& t ) {
        return static_cast< const void* >(
            &static_cast< const async_msg< filtered_type >& >( t ) );
    }

    static void* to_void_ptr( T& t ) {
        return static_cast< void* >(
            &static_cast< async_msg< filtered_type >& >( t ) );
    }

    // Sender-classes use non-const interfaces
    static const T& from_void_ptr( const void* p ) {
        return *static_cast< const T* >(
            static_cast< const async_msg< filtered_type >* >( p ) );
    }

    static T& from_void_ptr( void* p ) {
        return *static_cast< T* >(
            static_cast< async_msg< filtered_type >* >( p ) );
    }
    ...
    static task* try_put_task_wrapper_impl( receiver< T >* const this_recv,
        ...
        if ( is_async ) {
            ...
            const filtered_type& t =
                async_helpers< filtered_type >::from_void_ptr( p );
            const T msg( t );
...
public:
    ...
    // NOTE: Following part of PUBLIC section is copy-paste from original
    // sender<T> class
...
protected:
    ...
    template < typename X >
    bool try_get( X& t ) {
        return try_get_wrapper( internal::async_helpers< X >::to_void_ptr( t ),
                                internal::async_helpers< X >::is_async_type );
    }

    //! Reserves an item in the sender
    template < typename X >
    bool try_reserve( X& t ) {
        return try_reserve_wrapper(
            internal::async_helpers< X >::to_void_ptr( t ),
            internal::async_helpers< X >::is_async_type );
    }
...
public:
    ...
    bool try_put( const X& t ) {
        ...
        if ( res != SUCCESSFULLY_ENQUEUED )
            internal::spawn_in_graph_arena( graph_reference(), *res );
        return true;
    }
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    typedef internal::edge_container< predecessor_type >
...
protected:
    ...
    // NOTE: Following part of PROTECTED and PRIVATE sections is copy-paste from
    // original receiver<T> class
...
public:
    ...
    __TBB_DEPRECATED typedef
        typename internal::async_helpers< T >::filtered_type filtered_type;
...
protected:
    virtual bool try_get_wrapper( void* p, bool is_async ) __TBB_override {
        ...
        __TBB_ASSERT( false,
                      "async_msg interface does not support 'pull' protocol in "
                      "try_get()" );
        return false;
    }
    ...
    virtual bool try_reserve_wrapper( void* p, bool is_async ) __TBB_override {
        ...
        if ( internal::async_helpers< T >::is_async_type == is_async ) {
            return try_reserve(
                internal::async_helpers< T >::from_void_ptr( p ) );
        }
        // Else: this (T) is async OR incoming 't' is async
        __TBB_ASSERT( false,
                      "async_msg interface does not support 'pull' protocol in "
...
protected:
    virtual task* try_put_task_wrapper( const void* p,
                                        bool is_async ) __TBB_override {
        return internal::async_helpers< T >::try_put_task_wrapper_impl(
            this, p, is_async );
    }
...
public:
    ...
    // NOTE: Following part of PUBLIC section is partly copy-pasted in sender<T>
    // under #if __TBB_PREVIEW_ASYNC_MSG
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    //! interface to record edges for traversal & deletion
    __TBB_DEPRECATED typedef typename internal::edge_container< successor_type >
        built_successors_type;
    __TBB_DEPRECATED typedef
        typename built_successors_type::edge_list_type successor_list_type;
    __TBB_DEPRECATED virtual built_successors_type& built_successors() = 0;
    __TBB_DEPRECATED virtual void internal_add_built_successor(
        successor_type& ) = 0;
    __TBB_DEPRECATED virtual void internal_delete_built_successor(
        successor_type& ) = 0;
...
public:
    // NOTE: Following part of PUBLIC and PROTECTED sections is copy-pasted in
    // receiver<T> under #if __TBB_PREVIEW_ASYNC_MSG

    //! Add a predecessor to the node
    __TBB_DEPRECATED virtual bool register_predecessor( predecessor_type& ) {
        return false;
    }

    //! Remove a predecessor from the node
    __TBB_DEPRECATED virtual bool remove_predecessor( predecessor_type& ) {
        return false;
    }

#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    __TBB_DEPRECATED typedef
        typename internal::edge_container< predecessor_type >
            built_predecessors_type;
    __TBB_DEPRECATED typedef
    ...
    __TBB_DEPRECATED virtual void internal_add_built_predecessor(
        predecessor_type& ) = 0;
    __TBB_DEPRECATED virtual void internal_delete_built_predecessor(
        predecessor_type& ) = 0;
    __TBB_DEPRECATED virtual void copy_predecessors(
        predecessor_list_type& ) = 0;
...
public:
    ...
    //! The predecessor type for this node
    __TBB_DEPRECATED typedef receiver< input_type >::predecessor_type
        predecessor_type;

    //! Constructor
    __TBB_DEPRECATED explicit continue_receiver(
        __TBB_FLOW_GRAPH_PRIORITY_ARG1( int number_of_predecessors,
    ...
    __TBB_DEPRECATED bool register_predecessor( predecessor_type& )
        __TBB_override {
    ...
    /** Does not check to see if the removal of the predecessor now makes the
       current count exceed the new threshold.  So removing a predecessor while
       the graph is active can cause unexpected results. */
    __TBB_DEPRECATED bool remove_predecessor( predecessor_type& )
        __TBB_override {
        spin_mutex::scoped_lock l( my_mutex );
        --my_predecessor_count;
        return true;
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    __TBB_DEPRECATED typedef internal::edge_container< predecessor_type >
        built_predecessors_type;
    ...
    __TBB_DEPRECATED void internal_add_built_predecessor( predecessor_type& s )
        __TBB_override {
        spin_mutex::scoped_lock l( my_mutex );
        my_built_predecessors.add_edge( s );
    }

    __TBB_DEPRECATED void internal_delete_built_predecessor(
        predecessor_type& s ) __TBB_override {
        spin_mutex::scoped_lock l( my_mutex );
        my_built_predecessors.delete_edge( s );
    }

    __TBB_DEPRECATED void copy_predecessors( predecessor_list_type& v )
        __TBB_override {
        spin_mutex::scoped_lock l( my_mutex );
...
template < typename C, typename N >
typename graph_iterator< C, N >::reference graph_iterator< C, N >::operator*()
    const {
...
template < typename C, typename N >
typename graph_iterator< C, N >::pointer graph_iterator< C, N >::operator->()
    const {
    return current_node;
}
...
inline graph::graph()
    : my_nodes( NULL ), my_nodes_last( NULL ), my_task_arena( NULL ) {
    prepare_task_arena();
    own_context = true;
    cancelled = false;
...
inline void graph::reset( tbb::flow::interface11::reset_flags f ) {
    ...
    for ( task_list_type::iterator rti = my_reset_task_list.begin();
          rti != my_reset_task_list.end(); ++rti ) {
        tbb::flow::interface11::internal::spawn_in_graph_arena( *this,
                                                                *( *rti ) );
    }
...
public:
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    typedef typename sender< output_type >::built_successors_type
        built_successors_type;
    typedef
        typename sender< output_type >::successor_list_type successor_list_type;
#endif
    ...
    __TBB_NOINLINE_SYM source_node( graph& g, Body body, bool is_active = true )
        : graph_node( g ),
          my_active( is_active ),
          init_my_active( is_active ),
          my_body(
              new internal::source_body_leaf< output_type, Body >( body ) ),
          my_init_body(
              new internal::source_body_leaf< output_type, Body >( body ) ),
          my_reserved( false ),
        ...
        tbb::internal::fgt_node_with_body(
            CODEPTR(), tbb::internal::FLOW_SOURCE_NODE, &this->my_graph,
    ...
    source_node(
        const node_set< internal::order::preceding, Successors... >& successors,
        Body body,
        bool is_active = true )
    ...
    __TBB_NOINLINE_SYM source_node( const source_node& src )
        : graph_node( src.my_graph ),
          ...
          init_my_active( src.init_my_active ),
    ...
    bool try_get( output_type& v ) __TBB_override {
        ...
        // we've been asked to provide an item, but we have none.  enqueue a
        // task to provide one.
        spawn_put();
        return false;
    }
    ...
    /** true = item has been released and so remains in sender, dest must
     * request or reserve future items */
    ...
    void extract() __TBB_override {
        my_successors.built_successors().sender_extract(
            *this ); // removes "my_owner" == this from each successor
...
protected:
    ...
    void reset_node( reset_flags f ) __TBB_override {
        ...
        if ( my_active )
            internal::add_task_to_graph_reset_list( this->my_graph,
                                                    create_put_task() );
    }
...
private:
    ...
    // when resetting, and if the source_node was created with my_active ==
    // true, then when we reset the node we must store a task to run the node,
    // and spawn it only after the reset is complete and is_active() is again
    // true.  This is why we don't test for is_active() here.
    task* create_put_task() {
        return ( new ( task::allocate_additional_child_of(
    ...
    void spawn_put() {
        if ( internal::is_graph_active( this->my_graph ) ) {
            internal::spawn_in_graph_arena( this->my_graph,
                                            *create_put_task() );
        }
    }
...
public:
    ...
    typedef Output output_type;
    typedef internal::
        function_input< input_type, output_type, Policy, Allocator >
            input_impl_type;
    typedef internal::function_input_queue< input_type, Allocator >
        input_queue_type;
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    typedef
        typename input_impl_type::predecessor_list_type predecessor_list_type;
    typedef typename fOutput_type::successor_list_type successor_list_type;
#endif
    using input_impl_type::my_predecessors;

    //! Constructor
    // input_queue_type is allocated here, but destroyed in the
    // function_input_base.
    // TODO: pass the graph_buffer_policy to the function_input_base so it can
    // all be done in one place.  This would be an interface-breaking change.
    template < typename Body >
    __TBB_NOINLINE_SYM function_node(
        graph& g,
        size_t concurrency,
        ...
        __TBB_FLOW_GRAPH_PRIORITY_ARG1(
            Policy = Policy(),
            node_priority_t priority = tbb::flow::internal::no_priority ) )
#else
        __TBB_FLOW_GRAPH_PRIORITY_ARG1(
            Body body,
            node_priority_t priority = tbb::flow::internal::no_priority ) )
        ...
        : graph_node( g ),
          input_impl_type( g,
                           concurrency,
                           __TBB_FLOW_GRAPH_PRIORITY_ARG1( body, priority ) ),
          fOutput_type( g ) {
        tbb::internal::fgt_node_with_body(
            CODEPTR(), tbb::internal::FLOW_FUNCTION_NODE, &this->my_graph,
    ...
    function_node(
        ...
        __TBB_FLOW_GRAPH_PRIORITY_ARG1(
            Policy p = Policy(),
            node_priority_t priority = tbb::flow::internal::no_priority ) )
        : function_node( nodes.graph_reference(),
                         concurrency,
                         body,
                         __TBB_FLOW_GRAPH_PRIORITY_ARG1( p, priority ) ) {
        make_edges_in_order( nodes, *this );
    }

#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
    template < typename Body, typename... Args >
    function_node( const node_set< Args... >& nodes,
                   size_t concurrency,
                   Body body,
                   node_priority_t priority )
...
public:
    ...
    typedef typename internal::
        wrap_tuple_elements< N, internal::multifunction_output, Output >::type
            output_ports_type;
    typedef internal::
        multifunction_input< input_type, output_ports_type, Policy, Allocator >
            input_impl_type;
    typedef internal::function_input_queue< input_type, Allocator >
        input_queue_type;
...
public:
    ...
    __TBB_NOINLINE_SYM multifunction_node(
        ...
        tbb::internal::fgt_multioutput_node_with_body< N >(
            CODEPTR(), tbb::internal::FLOW_MULTIFUNCTION_NODE, &this->my_graph,
            static_cast< receiver< input_type >* >( this ),
...
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES && __TBB_CPP11_PRESENT
    template < typename Body >
    __TBB_NOINLINE_SYM multifunction_node( graph& g,
                                           size_t concurrency,
                                           Body body,
                                           node_priority_t priority )
    ...
    __TBB_NOINLINE_SYM multifunction_node(
        ...
        __TBB_FLOW_GRAPH_PRIORITY_ARG1(
            Policy p = Policy(),
            node_priority_t priority = tbb::flow::internal::no_priority ) )
        : multifunction_node( nodes.graph_reference(),
                              concurrency,
                              body,
                              __TBB_FLOW_GRAPH_PRIORITY_ARG1( p, priority ) ) {
        make_edges_in_order( nodes, *this );
...
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
    template < typename Body, typename... Args >
    __TBB_NOINLINE_SYM multifunction_node( const node_set< Args... >& nodes,
                                           size_t concurrency,
                                           Body body,
                                           node_priority_t priority )
...
//! split_node: accepts a tuple as input, forwards each element of the tuple to
//! its
//  successors.  The node has unlimited concurrency, so it does not reject
//  inputs.
...
public:
    ...
    typedef internal::predecessor_cache< input_type, null_mutex >
        predecessor_cache_type;
    typedef typename predecessor_cache_type::built_predecessors_type
        built_predecessors_type;
#endif
    ...
    __TBB_NOINLINE_SYM explicit split_node( graph& g )
        : graph_node( g ),
          my_output_ports(
              internal::init_output_ports< output_ports_type >::call(
                  g,
                  my_output_ports ) ) {
        tbb::internal::fgt_multioutput_node< N >(
...
protected:
    task* try_put_task( const TupleType& t ) __TBB_override {
        // Sending split messages in parallel is not justified, as overheads
        // would prevail. Also, we do not have successors here. So we just tell
        // the task returned here is successful.
...
class continue_node : public graph_node,
                      public internal::continue_input< Output, Policy >,
                      public internal::function_output< Output > {
public:
    ...
    __TBB_NOINLINE_SYM continue_node(
        ...
        : graph_node( g ),
          ...
          fOutput_type( g ) {
        tbb::internal::fgt_node_with_body(
            CODEPTR(), tbb::internal::FLOW_CONTINUE_NODE, &this->my_graph,
    ...
    continue_node(
        ...
        __TBB_FLOW_GRAPH_PRIORITY_ARG1(
            Policy p = Policy(),
            node_priority_t priority = tbb::flow::internal::no_priority ) )
        : continue_node( nodes.graph_reference(),
                         body,
                         __TBB_FLOW_GRAPH_PRIORITY_ARG1( p, priority ) ) {
        make_edges_in_order( nodes, *this );
    }
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
    template < typename Body, typename... Args >
    continue_node( const node_set< Args... >& nodes,
                   Body body,
                   node_priority_t priority )
...
public:
    __TBB_NOINLINE_SYM explicit broadcast_node( graph& g ) : graph_node( g ) {
        ...
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_BROADCAST_NODE, &this->my_graph,
            static_cast< receiver< input_type >* >( this ),
    ...
    typedef
        typename receiver< T >::built_predecessors_type built_predecessors_type;
...
class buffer_node : public graph_node,
                    public internal::reservable_item_buffer< T, A >,
                    public receiver< T >,
                    public sender< T > {
public:
    typedef T input_type;
...
protected:
    ...
    bool forwarder_busy;
    typedef internal::aggregating_functor< class_type, buffer_operation >
        handler_type;
    ...
    void handle_operations_impl( buffer_operation* op_list,
        ...
        while ( op_list ) {
            ...
            switch ( tmp->type ) {
                ...
                // edge recording
                case add_blt_succ:
                    internal_add_built_succ( tmp );
                    break;
                case del_blt_succ:
                    internal_del_built_succ( tmp );
                    break;
                case add_blt_pred:
                    internal_add_built_pred( tmp );
                    break;
                case del_blt_pred:
                    internal_del_built_pred( tmp );
                    break;
                case blt_succ_cnt:
                    internal_succ_cnt( tmp );
                    break;
                case blt_pred_cnt:
                    internal_pred_cnt( tmp );
                    break;
                case blt_succ_cpy:
                    internal_copy_succs( tmp );
                    break;
                case blt_pred_cpy:
                    internal_copy_preds( tmp );
                    break;
#endif
            }
        }
        ...
        if ( try_forwarding && !forwarder_busy ) {
            if ( internal::is_graph_active( this->my_graph ) ) {
                forwarder_busy = true;
                task* new_task = new ( task::allocate_additional_child_of(
                    *( this->my_graph.root_task() ) ) )
                    internal::forward_task_bypass<
                        buffer_node< input_type, A > >( *this );
                // tmp should point to the last item handled by the aggregator.
                // This is the operation the handling thread enqueued.  So
                // modifying that record will be okay. workaround for icc bug
...
private:
    ...
    bool is_item_valid() { return this->my_item_valid( this->my_tail - 1 ); }
...
protected:
    ...
    template < typename derived_type >
    void internal_forward_task_impl( buffer_operation* op,
                                     derived_type* derived ) {
        __TBB_ASSERT( static_cast< class_type* >( derived ) == this,
                      "'this' is not a base class for derived" );
...
public:
    ...
    __TBB_NOINLINE_SYM explicit buffer_node( graph& g )
        : graph_node( g ),
          internal::reservable_item_buffer< T >(),
          forwarder_busy( false ) {
        my_successors.set_owner( this );
        my_aggregator.initialize_handler( handler_type( this ) );
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_BUFFER_NODE, &this->my_graph,
            static_cast< receiver< input_type >* >( this ),
            static_cast< sender< output_type >* >( this ) );
    }
    ...
    bool remove_successor( successor_type& r ) __TBB_override {
        ...
        // handler, and a forward is scheduled, we may be the first to reach
        // this point after the aggregator, and so should check for the task.
        ( void )enqueue_forwarding_task( op_data );
        return true;
    }
...
protected:
    ...
    task* try_put_task( const T& t ) __TBB_override {
        ...
        // inserted) We have to spawn the returned task if our own operation
        // fails.
        ...
        if ( ft && op_data.status == internal::FAILED ) {
            ...
            // forward this could happen.)  Queue the task and reset the
            // pointer.
            internal::spawn_in_graph_arena( graph_reference(), *ft );
            ft = NULL;
...
private:
    ...
    bool is_item_valid() { return this->my_item_valid( this->my_head ); }
...
public:
    ...
    __TBB_NOINLINE_SYM explicit queue_node( graph& g ) : base_type( g ) {
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_QUEUE_NODE, &( this->my_graph ),
            static_cast< receiver< input_type >* >( this ),
            static_cast< sender< output_type >* >( this ) );
    }
    ...
    queue_node( const node_set< Args... >& nodes )
        : queue_node( nodes.graph_reference() ) {
        make_edges_in_order( nodes, *this );
    }
#endif
...
public:
    ...
    __TBB_NOINLINE_SYM sequencer_node( graph& g, const Sequencer& s )
        : queue_node< T, A >( g ),
          my_sequencer(
              new internal::function_body_leaf< T, size_t, Sequencer >( s ) ) {
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_SEQUENCER_NODE, &( this->my_graph ),
            static_cast< receiver< input_type >* >( this ),
            static_cast< sender< output_type >* >( this ) );
    }
    ...
    __TBB_NOINLINE_SYM sequencer_node( const sequencer_node& src )
        : queue_node< T, A >( src ), my_sequencer( src.my_sequencer->clone() ) {
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_SEQUENCER_NODE, &( this->my_graph ),
            static_cast< receiver< input_type >* >( this ),
            static_cast< sender< output_type >* >( this ) );
...
private:
    bool internal_push( sequencer_operation* op ) __TBB_override {
        ...
        const internal::op_stat res = this->place_item( tag, *( op->elem ) )
                                          ? internal::SUCCEEDED
                                          : internal::FAILED;
...
public:
    ...
    //! Constructor
    __TBB_NOINLINE_SYM explicit priority_queue_node(
        graph& g,
        const Compare& comp = Compare() )
        : buffer_node< T, A >( g ), compare( comp ), mark( 0 ) {
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_PRIORITY_QUEUE_NODE,
...
private:
    ...
    bool is_item_valid() { return this->my_tail > 0; }
...
private:
    ...
    // in case a reheap has not been done after a push, check if the mark item
    // is higher than the 0'th item
    bool prio_use_tail() {
        __TBB_ASSERT( mark <= this->my_tail,
    ...
    // prio_push: checks that the item will fit, expand array if necessary, put
    // at end
    ...
    void reheap() {
        ...
        while ( child < mark ) {
            ...
            if ( child + 1 < mark && compare( this->get_my_item( child ),
...
public:
    ...
    typedef typename sender< output_type >::successor_type successor_type;
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    typedef typename receiver< input_type >::built_predecessors_type
        built_predecessors_type;
    typedef typename sender< output_type >::built_successors_type
        built_successors_type;
    typedef typename receiver< input_type >::predecessor_list_type
    ...
    typedef
        typename sender< output_type >::successor_list_type successor_list_type;
#endif
    // TODO: There is a lack of predefined types for its controlling
    // "decrementer" port. It should be fixed later.
...
private:
    ...
    friend class internal::forward_task_bypass<
        limiter_node< T, DecrementType > >;

    // Let decrementer call decrement_counter()
    friend class internal::decrementer< limiter_node< T, DecrementType >,
                                        DecrementType >;

    bool check_conditions() { // always called under lock
        return ( my_count + my_tries < my_threshold &&
                 !my_predecessors.empty() && !my_successors.empty() );
    }
                        ...
                        if ( internal::is_graph_active( this->my_graph ) ) {
                            task* rtask = new (
                                task::allocate_additional_child_of(
                                    *( this->my_graph.root_task() ) ) )
                                internal::forward_task_bypass<
                                    limiter_node< T, DecrementType > >( *this );
                            internal::spawn_in_graph_arena( graph_reference(),
                                                            *rtask );
                        }
                    }
                }
    ...
    task* decrement_counter( long long delta ) {
        {
            ...
            if ( delta > 0 && size_t( delta ) > my_count )
                ...
                my_count -= size_t(
                    delta ); // absolute value of delta is sufficiently small
        }
        return forward_task();
    }
...
public:
    ...
    __TBB_STATIC_ASSERT(
        ( tbb::internal::is_same_type< DecrementType, continue_msg >::value ),
        "Deprecated interface of the limiter node can be used only in "
        "conjunction "
    ...
    limiter_node(
        graph& g,
        __TBB_DEPRECATED_LIMITER_ARG2( size_t threshold,
    ...
    bool register_successor( successor_type& r ) __TBB_override {
        ...
        if ( was_empty && !my_predecessors.empty() &&
            ...
            if ( internal::is_graph_active( this->my_graph ) ) {
                task* task = new ( task::allocate_additional_child_of(
                    *( this->my_graph.root_task() ) ) )
                    internal::forward_task_bypass<
                        limiter_node< T, DecrementType > >( *this );
    ...
    void internal_delete_built_predecessor( predecessor_type& src )
        __TBB_override {
    ...
    bool register_predecessor( predecessor_type& src ) __TBB_override {
        ...
        if ( my_count + my_tries < my_threshold && !my_successors.empty() &&
             internal::is_graph_active( this->my_graph ) ) {
            task* task = new ( task::allocate_additional_child_of(
                *( this->my_graph.root_task() ) ) )
                internal::forward_task_bypass<
                    limiter_node< T, DecrementType > >( *this );
...
class join_node< OutputTuple, reserving >
    : public internal::unfolded_join_node<
          tbb::flow::tuple_size< OutputTuple >::value,
          reserving_port,
          OutputTuple,
          reserving > {
private:
    ...
    typedef typename internal::
        unfolded_join_node< N, reserving_port, OutputTuple, reserving >
            unfolded_type;
...
private:
    static const int N = tbb::flow::tuple_size< OutputTuple >::value;
    typedef typename internal::
        unfolded_join_node< N, queueing_port, OutputTuple, queueing >
            unfolded_type;
...
private:
    static const int N = tbb::flow::tuple_size< OutputTuple >::value;
    typedef typename internal::unfolded_join_node< N,
                                                   key_matching_port,
                                                   OutputTuple,
                                                   key_matching< K, KHash > >
        unfolded_type;
...
public:
...
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
    template < typename... Args >
    join_node( const node_set< Args... >& nodes,
               key_matching< K, KHash > = key_matching< K, KHash >() )
        : join_node( nodes.graph_reference() ) {
        make_edges_in_order( nodes, *this );
    }
#endif
    ...
    __TBB_NOINLINE_SYM join_node( graph& g, __TBB_B0 b0, __TBB_B1 b1 )
        ...
        tbb::internal::fgt_multiinput_node< N >(
            CODEPTR(), tbb::internal::FLOW_JOIN_NODE_TAG_MATCHING,
...
class indexer_node< T0 >
    : public internal::unfolded_indexer_node< tuple< T0 > > {
private:
...
class indexer_node< T0, T1 >
    : public internal::unfolded_indexer_node< tuple< T0, T1 > > {
private:
...
class indexer_node< T0, T1, T2 >
    : public internal::unfolded_indexer_node< tuple< T0, T1, T2 > > {
private:
...
class indexer_node< T0, T1, T2, T3 >
    : public internal::unfolded_indexer_node< tuple< T0, T1, T2, T3 > > {
private:
...
class indexer_node< T0, T1, T2, T3, T4 >
    : public internal::unfolded_indexer_node< tuple< T0, T1, T2, T3, T4 > > {
private:
    static const int N = 5;

public:
    typedef tuple< T0, T1, T2, T3, T4 > InputTuple;
    typedef typename internal::tagged_msg< size_t, T0, T1, T2, T3, T4 >
        output_type;
    typedef typename internal::unfolded_indexer_node< InputTuple >
        unfolded_type;
    __TBB_NOINLINE_SYM indexer_node( graph& g ) : unfolded_type( g ) {
...
class indexer_node< T0, T1, T2, T3, T4, T5 >
    : public internal::unfolded_indexer_node<
...
public:
    ...
    typedef typename internal::tagged_msg< size_t, T0, T1, T2, T3, T4, T5 >
        output_type;
    typedef typename internal::unfolded_indexer_node< InputTuple >
        unfolded_type;
    __TBB_NOINLINE_SYM indexer_node( graph& g ) : unfolded_type( g ) {
...
public:
    ...
    typedef typename internal::tagged_msg< size_t, T0, T1, T2, T3, T4, T5, T6 >
        output_type;
    typedef typename internal::unfolded_indexer_node< InputTuple >
        unfolded_type;
    __TBB_NOINLINE_SYM indexer_node( graph& g ) : unfolded_type( g ) {
...
public:
    ...
    typedef
        typename internal::tagged_msg< size_t, T0, T1, T2, T3, T4, T5, T6, T7 >
            output_type;
    typedef typename internal::unfolded_indexer_node< InputTuple >
        unfolded_type;
...
public:
    ...
    typedef typename internal::
        tagged_msg< size_t, T0, T1, T2, T3, T4, T5, T6, T7, T8 >
            output_type;
    typedef typename internal::unfolded_indexer_node< InputTuple >
        unfolded_type;
    __TBB_NOINLINE_SYM indexer_node( graph& g ) : unfolded_type( g ) {
...
public:
    ...
    typedef typename internal::
        tagged_msg< size_t, T0, T1, T2, T3, T4, T5, T6, T7, T8, T9 >
            output_type;
    typedef typename internal::unfolded_indexer_node< InputTuple >
        unfolded_type;
    __TBB_NOINLINE_SYM indexer_node( graph& g ) : unfolded_type( g ) {
...
#if __TBB_PREVIEW_ASYNC_MSG
inline void internal_make_edge( internal::untyped_sender& p,
...
template <
    typename TS,
    typename TR,
    typename = typename tbb::internal::enable_if<
        tbb::internal::is_same_type< TS, internal::untyped_sender >::value ||
        tbb::internal::is_same_type< TR, internal::untyped_receiver >::value >::
        type >
...
template < typename T >
inline void make_edge( sender< T >& p,
                       receiver< typename T::async_msg_data_type >& s ) {
    internal_make_edge( p, s );
}

template < typename T >
inline void make_edge( sender< typename T::async_msg_data_type >& p,
                       receiver< T >& s ) {
    internal_make_edge( p, s );
}
...
inline void make_edge( T& output, V& input ) {
    make_edge( get< 0 >( output.output_ports() ),
               get< 0 >( input.input_ports() ) );
}
...
#if __TBB_PREVIEW_ASYNC_MSG
inline void internal_remove_edge( internal::untyped_sender& p,
                                  internal::untyped_receiver& s ) {
#else
template < typename T >
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    // TODO: should we try to remove p from the predecessor list of s, in case
...
template < typename T >
inline void remove_edge( sender< T >& p,
                         receiver< typename T::async_msg_data_type >& s ) {
    internal_remove_edge( p, s );
}

template < typename T >
inline void remove_edge( sender< typename T::async_msg_data_type >& p,
                         receiver< T >& s ) {
    internal_remove_edge( p, s );
}
#endif // __TBB_PREVIEW_ASYNC_MSG
...
inline void remove_edge( T& output, V& input ) {
    remove_edge( get< 0 >( output.output_ports() ),
                 get< 0 >( input.input_ports() ) );
}
...
class composite_node< tbb::flow::tuple< InputTypes... >,
                      tbb::flow::tuple< OutputTypes... > > : public graph_node {
public:
...
public:
    ...
    composite_node( graph& g, const char* type_name = "composite_node" )
        ...
        tbb::internal::fgt_multiinput_multioutput_node(
            CODEPTR(), tbb::internal::FLOW_COMPOSITE_NODE, this,
            &this->my_graph );
    ...
    void set_external_ports( T1&& input_ports_tuple, T2&& output_ports_tuple ) {
        __TBB_STATIC_ASSERT(
            NUM_INPUTS == tbb::flow::tuple_size< input_ports_type >::value,
            "number of arguments does not match number of input ports" );
        __TBB_STATIC_ASSERT(
            NUM_OUTPUTS == tbb::flow::tuple_size< output_ports_type >::value,
            "number of arguments does not match number of output ports" );
        my_input_ports = tbb::internal::make_unique< input_ports_type >(
            std::forward< T1 >( input_ports_tuple ) );
        my_output_ports = tbb::internal::make_unique< output_ports_type >(
            std::forward< T2 >( output_ports_tuple ) );

        tbb::internal::fgt_internal_input_alias_helper<
            T1, NUM_INPUTS >::alias_port( this, input_ports_tuple );
        tbb::internal::fgt_internal_output_alias_helper<
            T2, NUM_OUTPUTS >::alias_port( this, output_ports_tuple );
    }

    template < typename... NodeTypes >
    void add_visible_nodes( const NodeTypes&... n ) {
        internal::add_nodes_impl( this, true, n... );
    }

    template < typename... NodeTypes >
    void add_nodes( const NodeTypes&... n ) {
        internal::add_nodes_impl( this, false, n... );
    }
    ...
    output_ports_type& output_ports() {
        __TBB_ASSERT( my_output_ports,
                      "output ports not set, call set_external_ports to set "
                      "output ports" );
...
public:
    ...
    void set_external_ports( T&& input_ports_tuple ) {
        ...
        my_input_ports = tbb::internal::make_unique< input_ports_type >(
            std::forward< T >( input_ports_tuple ) );

        tbb::internal::fgt_internal_input_alias_helper<
            T, NUM_INPUTS >::alias_port( this,
                                         std::forward< T >(
                                             input_ports_tuple ) );
    }
...
public:
#if TBB_PREVIEW_FLOW_GRAPH_TRACE
    __TBB_NOINLINE_SYM composite_node(
        graph& g,
        const char* type_name = "composite_node" )
        : graph_node( g ) {
        tbb::internal::fgt_composite( CODEPTR(), this, &g );
        tbb::internal::fgt_multiinput_multioutput_node_desc( this, type_name );
    }
    ...
    void set_external_ports( T&& output_ports_tuple ) {
        ...
        my_output_ports = tbb::internal::make_unique< output_ports_type >(
            std::forward< T >( output_ports_tuple ) );

        tbb::internal::fgt_internal_output_alias_helper<
            T, NUM_OUTPUTS >::alias_port( this,
                                          std::forward< T >(
                                              output_ports_tuple ) );
    }
...
class async_node
    ...
    typedef multifunction_node< Input, tuple< Output >, Policy, Allocator >
        base_type;
    typedef typename internal::multifunction_input<
        Input,
...
private:
    struct try_put_functor {
        ...
        // TODO: pass value by copy since we do not want to block asynchronous
        // thread.
        const Output* value;
        bool result;
    ...
    public:
        ...
        //! Implements gateway_type::try_put for an external activity to submit
        //! a message to FG
    ...
    bool try_put_impl( const Output& i ) {
        internal::multifunction_output< Output >& port_0 =
            internal::output_port< 0 >( *this );
        internal::broadcast_cache< output_type >& port_successors =
            port_0.successors();
        ...
        __TBB_ASSERT(
            is_at_least_one_put_successful || tasks.empty(),
            "Return status is inconsistent with the method operation." );

        while ( !tasks.empty() ) {
            internal::enqueue_in_graph_arena( this->my_graph,
                                              tasks.pop_front() );
        }
...
public:
    ...
    __TBB_NOINLINE_SYM async_node(
        ...
        : base_type(
              g,
              concurrency,
              internal::async_body< Input,
                                    typename base_type::output_ports_type,
                                    gateway_type,
                                    Body >( body, &my_gateway )
                  __TBB_FLOW_GRAPH_PRIORITY_ARG0( priority ) ),
          my_gateway( self() ) {
        tbb::internal::fgt_multioutput_node_with_body< 1 >(
            CODEPTR(), tbb::internal::FLOW_ASYNC_NODE, &this->my_graph,
            static_cast< receiver< input_type >* >( this ),
            this->output_ports(), this->my_body );
    }
...
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES && __TBB_CPP11_PRESENT
    template < typename Body, typename... Args >
    __TBB_NOINLINE_SYM async_node( graph& g,
                                   size_t concurrency,
                                   Body body,
                                   node_priority_t priority )
    ...
    __TBB_NOINLINE_SYM async_node(
        ...
        __TBB_FLOW_GRAPH_PRIORITY_ARG1(
            Policy = Policy(),
            node_priority_t priority = tbb::flow::internal::no_priority ) )
        : async_node( nodes.graph_reference(),
                      concurrency,
                      __TBB_FLOW_GRAPH_PRIORITY_ARG1( body, priority ) ) {
        make_edges_in_order( nodes, *this );
    }
...
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
    template < typename Body, typename... Args >
    __TBB_NOINLINE_SYM async_node( const node_set< Args... >& nodes,
                                   size_t concurrency,
                                   Body body,
                                   node_priority_t priority )
    ...
    __TBB_NOINLINE_SYM async_node( const async_node& other )
        ...
        static_cast< async_body_base_type* >(
            this->my_init_body->get_body_ptr() )
            ->set_gateway( &my_gateway );
    ...
    gateway_type& gateway() { return my_gateway; }
    ...
    template < typename Body >
    Body copy_function_object() {
        typedef internal::multifunction_body<
            input_type, typename base_type::output_ports_type >
            mfn_body_type;
        typedef internal::async_body<
            Input, typename base_type::output_ports_type, gateway_type, Body >
            async_body_type;
        mfn_body_type& body_ref = *this->my_body;
        async_body_type ab = *static_cast< async_body_type* >(
            dynamic_cast< internal::multifunction_body_leaf<
                input_type, typename base_type::output_ports_type,
    ...
    void internal_delete_built_successor( successor_type& r ) __TBB_override {
        internal::output_port< 0 >( *this ).internal_delete_built_successor(
            r );
    }
...
template < typename T >
class overwrite_node : public graph_node,
                       public receiver< T >,
                       public sender< T > {
public:
    typedef T input_type;
    ...
    __TBB_NOINLINE_SYM explicit overwrite_node( graph& g )
        : graph_node( g ), my_buffer_is_valid( false ) {
        my_successors.set_owner( this );
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_OVERWRITE_NODE, &this->my_graph,
            static_cast< receiver< input_type >* >( this ),
            static_cast< sender< output_type >* >( this ) );
    }
    ...
    //! Reserves an item
    bool try_reserve( T& v ) __TBB_override { return try_get( v ); }
...
protected:
    ...
    struct register_predecessor_task : public graph_task {
        register_predecessor_task( predecessor_type& owner,
                                   successor_type& succ )
            : o( owner ), s( succ ) {};
...
public:
    ...
    __TBB_NOINLINE_SYM explicit write_once_node( graph& g ) : base_type( g ) {
        tbb::internal::fgt_node(
            CODEPTR(), tbb::internal::FLOW_WRITE_ONCE_NODE, &( this->my_graph ),
            static_cast< receiver< input_type >* >( this ),
            static_cast< sender< output_type >* >( this ) );
    }
