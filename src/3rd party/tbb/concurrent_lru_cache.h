...
private:
    ...
    struct map_value_type {
        ...
        map_value_type( value_type const& a_value,
    ...
    typedef tbb::internal::aggregating_functor< self_type,
                                                aggregated_operation_type >
        aggregator_function_type;
    friend class tbb::internal::
        aggregating_functor< self_type, aggregated_operation_type >;
    typedef tbb::internal::aggregator< aggregator_function_type,
                                       aggregated_operation_type >
        aggregator_type;
...
public:
    concurrent_lru_cache( value_function_type f,
    ...
    handle_object operator[]( key_type k ) {
        ...
        if ( op.is_new_value_needed() ) {
            ...
            tbb::internal::spin_wait_while_eq( op.result().second.my_is_ready,
                                               false );
        }
...
private:
    ...
    struct handle_move_t : no_assign {
        ...
        handle_move_t( concurrent_lru_cache& cache_ref,
                       typename map_storage_type::reference value_ref )
            : my_cache_ref( cache_ref ), my_map_record_ref( value_ref ) {};
    };
#endif
    class handle_object {
    ...
    private:
        ...
        static handle_move_t move( handle_object& h ) {
            __TBB_ASSERT( ( h.my_cache_pointer && h.my_map_record_ptr ) ||
                              ( !h.my_cache_pointer && !h.my_map_record_ptr ),
                          "invalid state of moving object?" );
            concurrent_lru_cache* cache_pointer = h.my_cache_pointer;
            typename map_storage_type::pointer map_record_ptr =
                h.my_map_record_ptr;
...
private:
    // TODO: looks like aggregator_operation is a perfect match for statically
    // typed variant type
    struct aggregator_operation
        ...
        void cast_and_handle( self_type& container ) {
            if ( my_operation_type == op_retive ) {
                ...
                static_cast< signal_end_of_usage_aggregator_operation* >( this )
                    ->handle( container );
            }
        }
    };
...
private:
    void handle_operations( aggregator_operation* op_list ) {
        while ( op_list ) {
            ...
            tbb::internal::itt_store_word_with_release( tmp->status,
                                                        uintptr_t( 1 ) );
        }
    }

private:
    typename map_storage_type::reference retrieve_serial(
        key_type k,
        ...
        if ( it == my_map_storage.end() ) {
            it = my_map_storage.insert(
                it, std::make_pair(
                        k, map_value_type( value_type(), 0, my_lru_list.end(),
                                           false ) ) );
            is_new_value_needed = true;
        } else {
            typename lru_list_type::iterator list_it =
                it->second.my_lru_list_iterator;
            if ( list_it != my_lru_list.end() ) {
                ...
                // eviction so - remove it from LRU history.
    ...
    void signal_end_of_usage_serial(
        typename map_storage_type::reference map_record_ref ) {
        typename map_storage_type::iterator it =
            my_map_storage.find( map_record_ref.first );
        __TBB_ASSERT(
            it != my_map_storage.end(),
            "cache should not return past-end iterators to outer world" );
        __TBB_ASSERT( &( *it ) == &map_record_ref,
                      "dangling reference has been returned to outside world? "
                      "data race ?" );
        __TBB_ASSERT(
            my_lru_list.end() ==
                std::find( my_lru_list.begin(), my_lru_list.end(), it ),
        ...
        if ( !--( it->second.my_ref_counter ) ) {
            ...
            if ( my_lru_list.size() >= my_number_of_lru_history_items ) {
                ...
                for ( size_t i = 0; i < number_of_elements_to_evict; ++i ) {
                    typename map_storage_type::iterator it_to_evict =
                        my_lru_list.back();
                    __TBB_ASSERT( !it_to_evict->second.my_ref_counter,
                                  "item to be evicted should not have a live "
                                  "references" );
