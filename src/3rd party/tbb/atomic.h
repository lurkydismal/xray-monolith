...
#if !defined( __TBB_show_deprecation_message_atomic_H ) && \
...
#define __TBB_DECL_ATOMIC_FIELD( t, f, a ) \
    t f __attribute__( ( aligned( a ) ) );
...
#define __TBB_DECL_FENCED_ATOMIC_PRIMITIVES( S, M )                   \
    ...
    struct atomic_traits< S, M > {                                    \
        ...
        inline static word compare_and_swap( volatile void* location, \
                                             word new_value,          \
                                             word comparand ) {       \
            return __TBB_machine_cmpswp##S##M( location, new_value,   \
                                               comparand );           \
        }                                                             \
        inline static word fetch_and_add( volatile void* location,    \
                                          word addend ) {             \
            return __TBB_machine_fetchadd##S##M( location, addend );  \
        }                                                             \
        inline static word fetch_and_store( volatile void* location,  \
                                            word value ) {            \
...
#define __TBB_DECL_ATOMIC_LOAD_STORE_PRIMITIVES( M )                \
    template <>                                                     \
    struct atomic_load_store_traits< M > {                          \
        template < typename T >                                     \
...
/** Various compilers issue various warnings if -1 is used with various integer
   types. The baroque expression below avoids all the warnings (we hope). */
#define __TBB_MINUS_ONE( T ) ( T( T( 0 ) - T( 1 ) ) )

//! Base class that provides basic functionality for atomic<T> without
//! fetch_and_add.
/** Works for any type T that has the same size as an integral type, has a
   trivial constructor/destructor, and can be copied/compared by memcpy/memcmp.
 */
template < typename T >
struct atomic_impl {
protected:
...
private:
    // TODO: rechecks on recent versions of gcc if union is still the _only_ way
    // to do a conversion without warnings
    ...
    template < typename value_t >
    static typename converter< value_t >::bits_type& to_bits_ref(
        value_t& value ) {
// TODO: this #ifdef is temporary workaround, as union conversion seems to fail
// on suncc for 64 bit types for 32 bit target
#if !__SUNPRO_CC
        return *( typename converter< value_t >::bits_type* )
                    ptr_converter< value_t* >( &value )
                        .bits;
#else
...
public:
    ...
    value_type fetch_and_store( value_type value ) {
        return to_value< value_type >(
            internal::atomic_traits< sizeof( value_type ), M >::fetch_and_store(
                &my_storage.my_value, to_bits( value ) ) );
    }
    ...
    value_type compare_and_swap( value_type value, value_type comparand ) {
        return to_value< value_type >(
            internal::atomic_traits< sizeof( value_type ), M >::
                compare_and_swap( &my_storage.my_value, to_bits( value ),
                                  to_bits( comparand ) ) );
    }
    ...
    operator value_type() const
        volatile { // volatile qualifier here for backwards compatibility
        return to_value< value_type >(
    ...
    value_type load() const {
        return to_value< value_type >(
            internal::atomic_load_store_traits< M >::load(
                to_bits_ref( my_storage.my_value ) ) );
    }

    value_type load() const { return load< acquire >(); }

    template < memory_semantics M >
    void store( value_type value ) {
        internal::atomic_load_store_traits< M >::store(
            to_bits_ref( my_storage.my_value ), to_bits( value ) );
    }

    void store( value_type value ) { store< release >( value ); }
...
//! Base class that provides basic functionality for atomic<T> with
//! fetch_and_add.
...
public:
    ...
    value_type fetch_and_add( D addend ) {
        return value_type(
            internal::atomic_traits< sizeof( value_type ), M >::fetch_and_add(
                &this->my_storage.my_value, addend * sizeof( StepType ) ) );
    }
    ...
    value_type fetch_and_increment() { return fetch_and_add( 1 ); }
...
public:
    value_type operator+=( D value ) { return fetch_and_add( value ) + value; }
    ...
    value_type operator++( int ) { return fetch_and_add( 1 ); }
...
template < typename T >
struct __TBB_DEPRECATED_VERBOSE_MSG(
    "tbb::atomic is deprecated, use std::atomic" ) atomic
...
#endif
    T operator=( T rhs ) {
        // "this" required here in strict ISO C++ because store_with_release is
        // a dependent name
...
#define __TBB_DECL_ATOMIC( T )                                              \
    ...
    struct __TBB_DEPRECATED_VERBOSE_MSG(                                    \
        ...
        constexpr atomic( T arg )                                           \
            : internal::atomic_impl_with_arithmetic< T, T, char >( arg ) {} \
        constexpr atomic< T >( const atomic< T >& rhs )                     \
            : internal::atomic_impl_with_arithmetic< T, T, char >( rhs ) {} \
                                                                            \
        T operator=( T rhs ) { return store_with_release( rhs ); }          \
        atomic< T >& operator=( const atomic< T >& rhs ) {                  \
            store_with_release( rhs );                                      \
            return *this;                                                   \
        }                                                                   \
    };
#else
...
#if _MSC_VER && !_WIN64
#if __TBB_ATOMIC_CTORS
/* Special version of __TBB_DECL_ATOMIC that avoids gratuitous warnings from cl
   /Wp64 option. It is identical to __TBB_DECL_ATOMIC(unsigned) except that it
   replaces operator=(T) with an operator=(U) that explicitly converts the U to
   a T.  Types T and U should be type synonyms on the platform.  Type U should
   be the wider variant of T from the perspective of /Wp64. */
...
//! Specialization for atomic<void*>, for sake of not allowing arithmetic or
//! operator->.
template <>
struct __TBB_DEPRECATED_VERBOSE_MSG(
    "tbb::atomic is deprecated, use std::atomic" )
...
void store( atomic< T >& a, T value ) {
    a.template store< M >( value );
}

namespace interface6 {
//! Make an atomic for use in an initialization (list), as an alternative to
//! zero-initialization or normal assignment.
template < typename T >
