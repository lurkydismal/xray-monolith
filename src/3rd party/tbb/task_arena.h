...
// If decltype is available, the helper detects the return type of functor of
// specified type, otherwise it defines the void type.
template < typename F >
...
#if __TBB_NUMA_SUPPORT
public:
    // TODO: consider version approach to resolve backward compatibility
    // potential issues.
    struct constraints {
...
protected:
    ...
    // Despite my_numa_id is accesible, it does not exist in task_arena_base on
    // user side if TBB_PREVIEW_NUMA_SUPPORT macro is not defined by the user.
    // To be sure that my_numa_id exists in task_arena_base layout we check the
    // traits.
    // TODO: Consider increasing interface version for task_arena_base instead
    // of this runtime check.
    numa_node_id numa_id() {
        return ( my_version_and_traits & numa_support_flag ) ==
                       numa_support_flag
                   ? my_numa_id
                   : automatic;
    }
    ...
    enum {
        default_flags = 0
                        ...
                        ( task_group_context::default_traits &
                          task_group_context::exact_exception ) // 0 or 1 << 16
        ,
        exact_exception_flag =
            task_group_context::exact_exception // used to specify flag for
                                                // context directly
#endif
#if __TBB_NUMA_SUPPORT
        ,
...
#if __TBB_NUMA_SUPPORT
    task_arena_base( const constraints& constraints_,
    ...
    static int __TBB_EXPORTED_FUNC
    internal_max_concurrency( const task_arena* );
...
void __TBB_EXPORTED_FUNC isolate_within_arena( delegate_base& d,
 ...
 * Constructors set up settings only, real construction is deferred till the
 * first method invocation Destructor only removes one of the references to the
 * inner arena representation. Final destruction happens when all the references
 * (and the work) are gone.
 */
class task_arena : public internal::task_arena_base {
    friend class tbb::internal::task_scheduler_observer_v3;
    friend void task::enqueue( task&,
                               task_arena&
...
#endif
#if TBB_USE_THREADING_TOOLS
        // Actual synchronization happens in internal_initialize &
        // internal_attach. The race on setting my_initialized is benign, but
        // should be hidden from Intel(R) Inspector
        internal::as_atomic( my_initialized )
            .fetch_and_store< release >( true );
#else
        my_initialized = true;
#endif
    }
...
#if __TBB_TASK_GROUP_CONTEXT
        internal_enqueue(
            *new ( task::allocate_root( *my_context ) )
                internal::function_task< typename internal::strip< F >::type >(
...
public:
    ...
    /** Sets up settings only, real construction is deferred till the first
     * method invocation
     *  @arg max_concurrency specifies total number of slots in arena where
     * threads work
     *  @arg reserved_for_masters specifies number of slots to be used by master
     * threads only. Value of 1 is default and reflects behavior of implicit
     * arenas.
     **/
    task_arena( int max_concurrency_ = automatic,
    ...
    task_arena( const constraints& constraints_,
                unsigned reserved_for_masters = 1 )
    ...
    task_arena(
        const task_arena& s ) // copy settings but not the reference or instance
        : task_arena_base( s.my_max_concurrency, s.my_master_slots ) {}

    //! Tag class used to indicate the "attaching" constructor
    struct attach {};

    //! Creates an instance of task_arena attached to the current arena of the
    //! thread
    explicit task_arena( attach )
        : task_arena_base( automatic,
                           1 ) // use default settings if attach fails
    {
        internal_attach();
        if ( my_arena )
            my_initialized = true;
    ...
    //! Forces allocation of the resources for the task_arena as specified in
    ...
    //! Overrides concurrency level and forces initialization of internal
    //! representation
    inline void initialize( int max_concurrency_,
                            unsigned reserved_for_masters = 1 ) {
        // TODO: decide if this call must be thread-safe
        __TBB_ASSERT( !my_arena,
                      "Impossible to modify settings of an already initialized "
                      "task_arena" );
        if ( !my_initialized ) {
...
#if __TBB_NUMA_SUPPORT
    inline void initialize( constraints constraints_,
                            unsigned reserved_for_masters = 1 ) {
        // TODO: decide if this call must be thread-safe
        __TBB_ASSERT( !my_arena,
                      "Impossible to modify settings of an already initialized "
    ...
    //! Removes the reference to the internal arena representation, and destroys
    //! the external object. Not thread safe wrt concurrent invocations of other
    //! methods.
    ~task_arena() { terminate(); }

    //! Returns true if the arena is active (initialized); false otherwise.
    //! The name was chosen to match a task_scheduler_init method with the same
    //! semantics.
    bool is_active() const { return my_initialized; }

    //! Enqueues a task into the arena to process a functor, and immediately
    //! returns. Does not require the calling thread to join the arena
...
#if __TBB_TASK_PRIORITY
    //! Enqueues a task with priority p into the arena to process a functor f,
    //! and immediately returns. Does not require the calling thread to join the
    //! arena
    template < typename F >
#if __TBB_CPP11_RVALUE_REF_PRESENT
    ...
    //! If not possible to join, wraps the functor into a task, enqueues it and
    //! waits for task completion Can decrement the arena demand for workers,
    //! causing a worker to leave and free a slot to the calling thread Since
    //! C++11, the method returns the value returned by functor (prior to C++11
    //! it returns void).
    template < typename F >
    typename internal::return_type_or_void< F >::type execute( F& f ) {
        return execute_impl<
            typename internal::return_type_or_void< F >::type >( f );
    }
    ...
    //! Returns the index, aka slot number, of the calling thread in its current
    //! arena This method is deprecated and replaced with
    //! this_task_arena::current_thread_index()
    inline static int current_thread_index() { return internal_current_slot(); }

    //! Returns the maximal number of threads that can work inside the arena
    inline int max_concurrency() const {
        // Handle special cases inside the library
        return ( my_max_concurrency > 1 ) ? my_max_concurrency
                                          : internal_max_concurrency( this );
    }
};
...
//! Since C++11, the method returns the value returned by functor (prior to
//! C++11 it returns void).
template < typename F >
typename internal::return_type_or_void< F >::type isolate( F& f ) {
    return internal::isolate_impl<
        typename internal::return_type_or_void< F >::type >( f );
}
...
#endif
    arena.initialize();
    //! Note: the context of the task may differ from the context instantiated
    //! by task_arena
