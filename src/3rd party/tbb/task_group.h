...
class task_handle : internal::no_assign {
    ...
    void mark_scheduled() {
        // The check here is intentionally lax to avoid the impact of
        // interlocked operation
        if ( my_state & scheduled )
            internal::throw_exception(
                internal::eid_invalid_multiple_scheduling );
        my_state |= scheduled;
    }
...
protected:
    ...
    task_group_status internal_run_and_wait( F& f ) {
        __TBB_TRY {
            if ( !my_context.is_group_execution_cancelled() ) {
                // We need to increase the reference count of the root task to
                // notify waiters that this task group has some work in
                // progress.
    ...
    task* prepare_task( __TBB_FORWARDING_REF( F ) f ) {
        return new ( task::allocate_additional_child_of( *my_root ) )
            Task( internal::forward< F >( f ) );
    }
...
public:
    ...
    bool is_canceling() { return my_context.is_group_execution_cancelled(); }

    void cancel() { my_context.cancel_group_execution(); }
...
public:
    ...
    template < typename F >
    void run( F&& f ) {
        task::spawn( *prepare_task< internal::function_task<
                         typename internal::strip< F >::type > >(
            std::forward< F >( f ) ) );
    }
#else
    template < typename F >
...
class __TBB_DEPRECATED structured_task_group
    : public internal::task_group_base {
public:
    // TODO: add task_handle rvalues support
    template < typename F >
    task_group_status run_and_wait( task_handle< F >& h ) {
...
class wait_delegate : public delegate_base {
    void operator()() const __TBB_override { status = tg.wait(); }
...
class isolated_task_group : public task_group {
    intptr_t this_isolation() { return reinterpret_cast< intptr_t >( this ); }
...
public:
    ...
    void run( F&& f ) {
        internal::spawn_delegate sd(
            prepare_task< internal::function_task<
                typename internal::strip< F >::type > >(
                std::forward< F >( f ) ) );
    ...
    void run( const F& f ) {
        internal::spawn_delegate sd(
            prepare_task< internal::function_task< F > >( f ) );
        internal::isolate_within_arena( sd, this_isolation() );
    }
#endif
    ...
    void run( task_handle< F >& h ) {
        internal::spawn_delegate sd(
            prepare_task< internal::task_handle_task< F > >( h ) );
        internal::isolate_within_arena( sd, this_isolation() );
    }
...
task_handle< typename internal::strip< F >::type > make_task( F&& f ) {
    return task_handle< typename internal::strip< F >::type >(
        std::forward< F >( f ) );
}
#else
template < class F >
