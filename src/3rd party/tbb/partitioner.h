...
public:
    ...
    void spawn_or_delay( bool, task& b ) { task::spawn( b ); }
};
...
public:
    ...
    static void mark_task_stolen( task& t ) {
        tbb::atomic< bool >& flag =
            static_cast< flag_task* >( t.parent() )->my_child_stolen;
#if TBB_USE_THREADING_TOOLS
        // Threading tools respect lock prefix but report false-positive
        // data-race via plain store
...
//! Depth is a relative depth of recursive division inside a range pool.
//! Relative depth allows infinite absolute depth of the recursion for heavily
//! unbalanced workloads with range represented by a number that cannot fit into
//! machine word.
...
public:
    ...
    range_vector( const T& elem ) : my_head( 0 ), my_tail( 0 ), my_size( 1 ) {
        my_depth[ 0 ] = 0;
        new ( static_cast< void* >( my_pool.begin() ) )
            T( elem ); // TODO: std::move?
    }
    ~range_vector() {
        while ( !empty() )
            pop_back();
    ...
    //! max_depth starts from 0, e.g. value 2 makes 3 ranges in the pool up to
    //! two 1/4 pieces
    void split_to_fill( depth_t max_depth ) {
        while ( my_size < MaxCapacity && is_divisible( max_depth ) ) {
            ...
            new ( my_pool.begin() + prev )
                T( my_pool.begin()[ my_head ], split() ); // do 'inverse' split
...
struct adaptive_mode : partition_type_base< Partition > {
    ...
    size_t my_divisor;
    // For affinity_partitioner, my_divisor indicates the number of affinity
    // array indices the task reserves. A task which has only one index must
    // produce the right split without reserved index in order to avoid it to be
    // overwritten in note_affinity() of the created (right) task. I.e. a task
    // created deeper than the affinity array can remember must not save its
    // affinity (LIFO order)
    static const unsigned factor = 1;
    adaptive_mode()
        : my_divisor( tbb::internal::get_initial_auto_partitioner_divisor() /
                      4 * my_partition::factor ) {}
    adaptive_mode( adaptive_mode& src, split )
        : my_divisor( do_split( src, split() ) ) {}
...
//! Range.
/** If the Range has static boolean constant 'is_splittable_in_proportion' set
   to 'true', the created object splits a provided value in an
   implemenation-defined proportion; otherwise it represents equal-size split.
 */
...
struct proportion_helper<
    Range,
    typename enable_if< Range::is_splittable_in_proportion, void >::type > {
...
struct dynamic_grainsize_mode : Mode {
    ...
    bool check_being_stolen( task& t ) { // part of old should_execute_range()
        if ( !( self().my_divisor /
                Mode::my_partition::factor ) ) { // if not from the top P tasks
                                                 // of binary tree
            self().my_divisor =
    ...
    void work_balance( StartType& start, Range& range ) {
        if ( !range.is_divisible() || !self().max_depth() ) {
            ...
            internal::range_vector< Range, range_pool_size > range_pool(
                range );
            do {
                range_pool.split_to_fill(
                    self().max_depth() ); // fill range pool
                if ( self().check_for_demand( start ) ) {
                    if ( range_pool.size() > 1 ) {
                        start.offer_work( range_pool.front(),
                                          range_pool.front_depth() );
                        range_pool.pop_front();
                        continue;
                    }
                    if ( range_pool.is_divisible(
                             self().max_depth() ) ) // was not enough depth to
                                                    // fork a task
                        continue; // note: next split_to_fill() should split
                                  // range at least once
                }
    ...
    bool check_for_demand( task& t ) {
        if ( pass == my_delay ) {
            ...
            else if ( self().my_divisor &&
                      my_max_depth ) { // make balancing task
            ...
            if ( __TBB_time_stamp() < my_dst_tsc ) {
                ...
                my_max_depth--; // increase granularity since tasks seem having
                                // too small work
                return false;
            }
            my_delay = pass;
...
class static_partition_type
    : public linear_affinity_mode< static_partition_type > {
public:
...
class affinity_partition_type
    : public dynamic_grainsize_mode<
          linear_affinity_mode< affinity_partition_type > > {
...
public:
    ...
    affinity_partition_type( affinity_partition_type& p, split )
        : dynamic_grainsize_mode<
              linear_affinity_mode< affinity_partition_type > >( p, split() ),
          my_array( p.my_array ) {}
    affinity_partition_type( affinity_partition_type& p,
                             const proportional_split& split_obj )
        : dynamic_grainsize_mode<
              linear_affinity_mode< affinity_partition_type > >( p, split_obj ),
          my_array( p.my_array ) {}
    void set_affinity( task& t ) {
        if ( my_divisor ) {
            if ( !my_array[ my_head ] )
...
/** The range is initial divided into several large chunks.
    Chunks are further subdivided into smaller pieces if demand detected and
   they are divisible.
    @ingroup algorithms */
class auto_partitioner {
public:
...
private:
    ...
    // TODO: consider to make split_type public
    typedef interface9::internal::affinity_partition_type::split_type
        split_type;
};
