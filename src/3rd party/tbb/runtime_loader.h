...
#if !defined( __TBB_show_deprecation_message_runtime_loader_H ) && \
    defined( __TBB_show_deprecated_header_message )
#define __TBB_show_deprecation_message_runtime_loader_H
#pragma message( \
    "TBB Warning: tbb/runtime_loader.h is deprecated. For details, please see Deprecated Features appendix in the TBB reference manual." )
#endif
...
Link with \c tbbproxy.lib (or \c libtbbproxy.a) instead of \c tbb.lib (\c
libtbb.dylib,
\c libtbb.so).
...
All \c runtime_loader objects (in the same module, i.e. exe or dll) share some
global state. The most noticeable piece of global state is loaded TBB library.
There are some implications:

    -   Only one TBB library can be loaded per module.

    -   If one object has already loaded TBB library, another object will not
load TBB. If the loaded TBB library is suitable for the second object, both will
use TBB cooperatively, otherwise the second object will report an error.

    -   \c runtime_loader objects will not work (correctly) in parallel due to
...
public:
    ...
    enum error_mode {
        ...
        em_throw,  //!< Throw an exception of tbb::runtime_loader::error_code
                   //!< type.
    ...
    enum error_code {
        ec_ok,       //!< No errors.
        ec_bad_call, //!< Invalid function call (e. g. load() called when TBB is
                     //!< already loaded).
        ec_bad_arg,  //!< Invalid argument passed.
        ec_bad_lib,  //!< Invalid library found (e. g. \c TBB_runtime_version
                     //!< symbol not found).
    ...
    /*!
        ...
        If error mode is \c em_status, call status() to check whether TBB was
       loaded or not.
    */
    runtime_loader(
        char const* path[], //!< List of directories to search TBB in.
        int min_ver =
            TBB_INTERFACE_VERSION, //!< Minimal suitable version of TBB.
    ...
    //! Load TBB.
    /*!
        The method searches the directories specified in \c path[] array for the
       TBB library. When the library is found, it is loaded and its version is
       checked. If the version is not suitable, the library is unloaded, and the
       search continues.

        \b Note:

        For security reasons, avoid using relative directory names. For example,
       never load TBB from current (\c "."), parent (\c "..") or any other
       ...
       (\c "") on Windows. (See
       http://www.microsoft.com/technet/security/advisory/2269637.mspx for
        details.)

        Neglecting these rules may cause your program to execute 3-rd party
       malicious code.
        ...
        \b Errors:
            ...
            -   \c ec_bad_ver - TBB of unsuitable version already loaded by
       another object.
    ...
    */
    ...
    //! Report status.
    /*!
        If error mode is \c em_status, the function returns status of the last
       operation.
    */
    error_code status();
