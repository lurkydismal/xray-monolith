...
class while_group_task : public task {
    ...
    template < typename Stream, typename Body_ >
    friend class while_task;
};

//! For internal use only.
/** Gets block of iterations from a stream and packages them into a
   while_group_task.
    @ingroup algorithms */
template < typename Stream, typename Body >
...
void parallel_while< Body >::run( Stream& stream, const Body& body ) {
    ...
    while_task< Stream, Body >& w =
        *new ( my_barrier->allocate_child() )
