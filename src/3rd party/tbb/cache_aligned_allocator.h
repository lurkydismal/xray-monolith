...
/** @ingroup memory_allocation */
void* __TBB_EXPORTED_FUNC NFS_Allocate( size_t n_element,
                                        size_t element_size,
                                        void* hint );
...
//! @endcond
  ...
  // Workaround for erroneous "unreferenced parameter" warning in method
  // destroy.
#pragma warning( push )
...
public:
    ...
    pointer allocate( size_type n, const void* hint = 0 ) {
        // The "hint" argument is always ignored in NFS_Allocate thus const_cast
        // shouldn't hurt
        return pointer( internal::NFS_Allocate( n, sizeof( value_type ),
                                                const_cast< void* >( hint ) ) );
    }

    //! Free block of memory that starts on a cache line
    void deallocate( pointer p, size_type ) { internal::NFS_Free( p ); }

    //! Largest value for which method allocate might succeed.
    size_type max_size() const throw() {
        return ( ~size_t( 0 ) - internal::NFS_MaxLineSize ) /
               sizeof( value_type );
    }
...
inline bool operator==( const cache_aligned_allocator< T >&,
...
public:
    ...
    std::pmr::memory_resource* upstream_resource() const { return m_upstream; }

private:
    //! We don't know what memory resource set. Use padding to guarantee
    //! alignment
    void* do_allocate( size_t bytes, size_t alignment ) override {
        size_t cache_line_alignment = correct_alignment( alignment );
        uintptr_t base = ( uintptr_t )m_upstream->allocate(
            correct_size( bytes ) + cache_line_alignment );
...
#pragma warning( disable : 4146 4706 )
        ...
        uintptr_t result =
            ( base + cache_line_alignment ) & -cache_line_alignment;
#if _MSC_VER && !defined( __INTEL_COMPILER )
#pragma warning( pop )
#endif
    ...
    void do_deallocate( void* ptr, size_t bytes, size_t alignment ) override {
        if ( ptr ) {
            ...
            m_upstream->deallocate(
                ( void* )base,
                correct_size( bytes ) + correct_alignment( alignment ) );
        }
    }
    ...
    bool do_is_equal(
        ...
        return other_res &&
               ( this->upstream_resource() == other_res->upstream_resource() );
#else
        return false;
#endif
    }
    ...
    size_t correct_alignment( size_t alignment ) {
        __TBB_ASSERT( tbb::internal::is_power_of_two( alignment ),
