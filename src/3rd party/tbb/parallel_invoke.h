...
/** Dummy to avoid cluttering the bulk of the header with enormous amount of
 * ifdefs. **/
...
private:
    ...
    task* execute() __TBB_override {
        if ( is_recycled ) {
            ...
            __TBB_ASSERT( N == 2 || N == 3,
                          "Number of arguments passed to spawner is wrong" );
            set_ref_count( N );
            recycle_as_safe_continuation();
            internal::function_invoker< function2 >* invoker2 =
                new ( allocate_child() )
                    internal::function_invoker< function2 >( my_func2 );
            __TBB_ASSERT( invoker2, "Child task allocation failed" );
            spawn( *invoker2 );
            size_t n = N; // To prevent compiler warnings
            if ( n > 2 ) {
                internal::function_invoker< function3 >* invoker3 =
                    new ( allocate_child() )
                        internal::function_invoker< function3 >( my_func3 );
...
public:
    spawner( const function1& _func1,
...
public:
    ...
    void add_children( function&& _func ) {
        internal::function_invoker< function >* invoker =
            new ( allocate_child() ) internal::function_invoker< function >(
                std::forward< function >( _func ) );
    ...
    void add_children( function1&& _func1,
                       function2&& _func2,
                       function&&... _func ) {
        // The third argument is dummy, it is ignored actually.
        parallel_invoke_noop noop;
        typedef internal::spawner< 2, function1, function2,
                                   parallel_invoke_noop >
            spawner_type;
        spawner_type& sub_root = *new ( allocate_child() ) spawner_type(
            std::forward< function1 >( _func1 ),
            std::forward< function2 >( _func2 ), noop );
        spawn( sub_root );
        add_children( std::forward< function >( _func )... );
    }
#else
    ...
    void add_children( const function& _func ) {
        internal::function_invoker< function >* invoker =
            new ( allocate_child() )
                internal::function_invoker< function >( _func );
        __TBB_ASSERT( invoker, "Child task allocation failed" );
        spawn( *invoker );
    }
    ...
    void add_children( const function1& _func1, const function2& _func2 ) {
        ...
        internal::spawner< 2, function1, function2, parallel_invoke_noop >&
            sub_root = *new ( allocate_child() )
                           internal::spawner< 2, function1, function2,
                                              parallel_invoke_noop >(
                               _func1, _func2, noop );
        spawn( sub_root );
    }
    ...
    void add_children( const function1& _func1,
                       const function2& _func2,
                       const function3& _func3 ) {
        internal::spawner< 3, function1, function2, function3 >& sub_root =
            *new ( allocate_child() )
                internal::spawner< 3, function1, function2, function3 >(
    ...
    void run_and_finish( const F0& f0 ) {
        internal::function_invoker< F0 >* invoker =
...
public:
#if __TBB_TASK_GROUP_CONTEXT
    parallel_invoke_cleaner( int number_of_children,
                             tbb::task_group_context& context )
        : root( *new ( task::allocate_root( context ) )
                    internal::parallel_invoke_helper( number_of_children ) )
#else
    parallel_invoke_cleaner( int number_of_children, tbb::task_group_context& )
        : root( *new ( task::allocate_root() )
                    internal::parallel_invoke_helper( number_of_children ) )
#endif /* !__TBB_TASK_GROUP_CONTEXT */
    {
    }

    ~parallel_invoke_cleaner() { root.destroy( root ); }
...
template < typename... T >
struct impl_selector; // to workaround a GCC bug
...
inline task_group_context& get_context( task_group_context& tgc ) {
    return tgc;
}
...
void parallel_invoke_impl( true_type, F0&& f0, F1&& f1, F&&... f ) {
    __TBB_STATIC_ASSERT( sizeof...( F ) > 0,
                         "Variadic parallel_invoke implementation broken?" );
    // # of child tasks: f0, f1, and a task for each two elements of the pack
    // except the last
    const size_t number_of_children = 2 + sizeof...( F ) / 2;
    parallel_invoke_cleaner cleaner( number_of_children,
                                     get_context( std::forward< F >( f )... ) );
...
void parallel_invoke_impl( false_type, F0&& f0, F1&& f1, F&&... f ) {
    ...
    parallel_invoke_impl( true_type(), std::forward< F0 >( f0 ),
...
void parallel_invoke( F0&& f0, F1&& f1, F&&... f ) {
    typedef typename internal::impl_selector< internal::false_type, F... >::type
        selector_type;
    internal::parallel_invoke_impl( selector_type(), std::forward< F0 >( f0 ),
                                    std::forward< F1 >( f1 ),
                                    std::forward< F >( f )... );
}
...
void parallel_invoke( const F0& f0,
                      ...
                      const F5& f5 ) {
    task_group_context context( internal::PARALLEL_INVOKE );
    parallel_invoke< F0, F1, F2, F3, F4, F5 >( f0, f1, f2, f3, f4, f5,
                                               context );
}
...
void parallel_invoke( const F0& f0,
                      ...
                      const F8& f8 ) {
    task_group_context context( internal::PARALLEL_INVOKE );
    parallel_invoke< F0, F1, F2, F3, F4, F5, F6, F7, F8 >(
        f0, f1, f2, f3, f4, f5, f6, f7, f8, context );
}
...
void parallel_invoke( const F0& f0,
                      ...
                      const F9& f9 ) {
    task_group_context context( internal::PARALLEL_INVOKE );
    parallel_invoke< F0, F1, F2, F3, F4, F5, F6, F7, F8, F9 >(
        f0, f1, f2, f3, f4, f5, f6, f7, f8, f9, context );
}
#endif // __TBB_VARIADIC_PARALLEL_INVOKE
//@}
