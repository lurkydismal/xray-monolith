 ...
 * - <a href="./modules.html">Modules</a> (groups of functionality) implemented
 * by the library
 * - <a href="./annotated.html">Classes</a> provided by the library
 * - <a href="./files.html">Files</a> constituting the library.
 * .
 * Please note that significant part of TBB functionality is implemented in the
 * form of template functions, descriptions of which are not accessible on the
 * <a href="./annotated.html">Classes</a> tab. Use <a
 * href="./modules.html">Modules</a> or <a
 * href="./namespacemembers.html">Namespace/Namespace Members</a> tabs to find
 * them.
 *
...
/** \page concepts TBB concepts
    ...
    A concept is a set of requirements to a type, which are necessary and
sufficient for the type to model a particular behavior or a set of behaviors.
Some concepts are specific to a particular algorithm (e.g. algorithm body),
while other ones are common to several algorithms (e.g. range concept).

    All TBB algorithms make use of different classes implementing various
concepts. Implementation classes are supplied by the user as type arguments of
template parameters and/or as objects passed as function call arguments. The
library provides predefined  implementations of some concepts (e.g. several
kinds of
    \ref range_req "ranges"), while other ones must always be implemented by the
user.

    TBB defines a set of minimal requirements each concept must conform to. Here
is the list of different concepts hyperlinked to the corresponding requirements
specifications:
    - \subpage range_req
...
typedef void ( *assertion_handler_type )( const char* filename,
                                          int line,
...
#define __TBB_ASSERT_RELEASE( predicate, message )      \
    ( ( predicate ) ? ( ( void )0 )                     \
                    : rml::internal::assertion_failure( \
                          __FILE__, __LINE__, #predicate, message ) )
#else
namespace tbb {
#define __TBB_ASSERT_RELEASE( predicate, message )                            \
    ( ( predicate ) ? ( ( void )0 )                                           \
                    : tbb::assertion_failure( __FILE__, __LINE__, #predicate, \
                                              message ) )
#endif
...
void __TBB_EXPORTED_FUNC assertion_failure( const char* filename,
                                            int line,
                                            const char* expression,
                                            const char* comment );
...
/** If predicate is false, print assertion failure message.
    If the comment argument is not NULL, it is printed as part of the failure
   message. The comment argument has no other effect. */
...
#define __TBB_ASSERT( predicate, comment ) ( ( void )0 )
  //! "Extended" version is useful to suppress warnings if a variable is only
  //! used with an assert
...
//! The function returns the interface version of the TBB shared library being
//! used.
/**
 * The version it returns is determined at runtime, not at compile/link time.
 * So it can be different than the value of TBB_INTERFACE_VERSION obtained at
 * compile time.
 */
...
/**
 * @cond INTERNAL
 * @brief Identifiers declared inside namespace internal should never be used
...
/** Label for data that may be accessed from different threads, and that may
   eventually become wrapped in a formal atomic type.

    Note that no problems have yet been observed relating to the definition
   currently being empty, even if at least "volatile" would seem to be in order
   to avoid data sometimes temporarily hiding in a register (although "volatile"
   as a "poor man's atomic" lacks several other features of a proper atomic,
   some of which are now provided instead through specialized functions).

    Note that usage is intentionally compatible with a definition as qualifier
   "volatile", both as a way to have the compiler help enforce use of the label
   and to quickly rule out one potential issue.

    Note however that, with some architecture/compiler combinations, e.g. on
   IA-64 architecture, "volatile" also has non-portable memory semantics that
   are needlessly expensive for "relaxed" operations.

    Note that this must only be applied to data that will not change bit
   patterns when cast to/from an integral type of the same length; tbb::atomic
   must be used instead for, e.g., floating-point types.
...
#define __TBB_offsetof( class_name, member_name )                    \
    ( ( ptrdiff_t )&(                                                \
          reinterpret_cast< class_name* >( 0x1000 )->member_name ) - \
      0x1000 )

//! Returns address of the object containing a member with the given name and
//! address
#define __TBB_get_object_ref( class_name, member_name, member_addr ) \
    ( *reinterpret_cast< class_name* >(                              \
        ( char* )member_addr - __TBB_offsetof( class_name, member_name ) ) )
...
template < typename T >
inline void poison_pointer( T * __TBB_atomic & p ) {
...
// T is a pointer type because it will be explicitly provided by the programmer
// as a template argument; U is a referent type to enable the compiler to check
// that "ptr" is a pointer, deducing U in the process.
...
public:
#if __GNUC__
    //! Explicitly define default construction, because otherwise gcc issues
    //! gratuitous warning.
    no_assign() {}
#endif /* __GNUC__ */
};
...
inline argument_integer_type modulo_power_of_two(
    argument_integer_type arg,
    divisor_integer_type divisor ) {
    __TBB_ASSERT( is_power_of_two( divisor ),
                  "Divisor should be a power of two" );
    return ( arg & ( divisor - 1 ) );
}

//! A function to determine if arg is a power of 2 at least as big as another
//! power of 2.
...
inline bool is_power_of_two_at_least( argument_integer_type arg,
                                      power2_integer_type power2 ) {
    __TBB_ASSERT( is_power_of_two( power2 ),
                  "Divisor should be a power of two" );
...
// Following is a set of classes and functions typically used in compile-time
// "metaprogramming".
...
#if __TBB_CPP11_RVALUE_REF_PRESENT || __TBB_CPP11_DECLTYPE_PRESENT || \
    _LIBCPP_VERSION
...
#if __TBB_CPP11_SMART_POINTERS_PRESENT && __TBB_CPP11_RVALUE_REF_PRESENT && \
...
#if _MSC_VER
//! Microsoft std::allocator has non-standard extension that strips const from a
//! type.
template < typename T >
...
// Intended strictly for internal use! For public APIs (traits etc), use C++11
// analogues.
template < bool v >
...
struct select_size_t_constant {
    ...
    // truncation. The value of the right size,   which is selected by ?:, is
    // anyway not truncated or promoted.
...
// libc++ defines "pre-C++11 move and forward" similarly to ours; use it to
// avoid name conflicts in some cases.
...
template <>
struct STATIC_ASSERTION_FAILED< true >; // intentionally left undefined to cause
                                        // compile time error
...
#else
// please note condition is intentionally inverted to get a bit more
// understandable error msg
#define __TBB_STATIC_ASSERT_IMPL1( condition, msg, line )                   \
    enum {                                                                  \
        static_assert_on_line_##line =                                      \
            tbb::internal::STATIC_ASSERTION_FAILED< !( condition ) >::value \
    }

#define __TBB_STATIC_ASSERT_IMPL( condition, msg, line ) \
    __TBB_STATIC_ASSERT_IMPL1( condition, msg, line )
//! Verify condition, at compile time
#define __TBB_STATIC_ASSERT( condition, msg ) \
    __TBB_STATIC_ASSERT_IMPL( condition, msg, __LINE__ )
#endif
