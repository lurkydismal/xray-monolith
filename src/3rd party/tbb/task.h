...
class allocate_additional_child_of_proxy : no_assign {
    //! No longer used, but retained for binary layout compatibility.  Always
    //! NULL.
    task* self;
    task& parent;

public:
    explicit allocate_additional_child_of_proxy( task& parent_ )
...
/** TBB's evolution caused the "this" argument for several methods to become
   obsolete. However, for backwards binary compatibility, the new methods need
   distinct names, otherwise the One Definition Rule would be broken.  Hence the
   new methods are defined in this private base class, and then exposed in class
   task via using declarations. */
class task_base : tbb::internal::no_copy {
    __TBB_TASK_BASE_ACCESS : friend class tbb::task;
    ...
    /** Typically used in conjunction with schedule_to_reexecute to implement
       while loops. Atomically increments the reference count of t.parent() */
    static tbb::internal::allocate_additional_child_of_proxy
    allocate_additional_child_of( task& t ) {
...
//! A reference count
/** Should always be non-negative.  A signed type is used so that underflow can
 * be detected. */
typedef intptr_t reference_count;

#if __TBB_PREVIEW_RESUMABLE_TASKS
//! The flag to indicate that the wait task has been abandoned.
static const reference_count abandon_flag =
    reference_count( 1 ) << ( sizeof( reference_count ) * CHAR_BIT - 2 );
#endif
...
struct context_list_node_t {
    context_list_node_t *my_prev, *my_next;
};
...
/** This class is internal to the library.
   ...
   prefix inlined into application code. To prevent ODR violation, the class
   shall have the same layout in all application translation units. If some
   fields are conditional (e.g. enabled by preview macros) and might get
    skipped, use reserved fields to adjust the layout.

    In case task prefix size exceeds 32 or 64 bytes on IA32 and Intel64
   architectures correspondingly, consider dynamic setting of task_alignment and
   task_prefix_reservation_size based on the maximal operand size supported by
   the current CPU.
...
private:
    ...
    /** Currently it is used to broadcast cancellation requests generated both
        by users and as the result of unhandled exceptions in the
    ...
    /** Small tasks are pooled by the scheduler that allocated the task.
       ...
       scheduler, it returns the task to that other scheduler.  This policy
       avoids memory space blowup issues for memory allocators that allocate
       from thread-specific pools. */
    scheduler* origin;
...
#if __TBB_PREVIEW_RESUMABLE_TASKS
        //! Pointer to the abandoned scheduler where the current task is waited
        //! for.
    ...
    /** In the "blocking style" of programming, this field points to the parent
       task. In the "continuation-passing style" of programming, this field
       points to the continuation of the parent. */
    tbb::task* parent;
    ...
    /** In the "continuation-passing style" of programming, this field is
        ...
        In the "blocking style" of programming, this field is one more than the
       difference. */
    ...
    //! Miscellaneous state that is not directly visible to users, stored as a
...
static const int priority_critical =
    priority_stride_v4 * 3 + priority_stride_v4 / 3 * 2;
#endif
} // namespace internal
...
/** @ingroup task_scheduling
   ...
   unhandled exceptions intercepted during tasks execution. Intercepting an
   exception results in generating internal cancellation requests (which is
   processed in exactly the same way as external ones).

    The context is associated with one or more root tasks and defines the
   cancellation group that includes all the descendants of the corresponding
   root task(s). Association is established when a context object is passed as
   an argument to the task::allocate_root() method. See
   task_group_context::task_group_context for more details.

    The context can be bound to another one, and other contexts can be bound to
   it, forming a tree-like structure: parent -> this -> children. Arrows here
   designate cancellation propagation direction. If a task in a cancellation
   group is cancelled all the other tasks in this group and groups bound to it
   (as children) get cancelled too.

    IMPLEMENTATION NOTE:
    When adding new members to task_group_context or changing types of existing
   ones, update the size of both padding buffers (_leading_padding and
   _trailing_padding) appropriately. See also VERSIONING NOTE at the constructor
   definition below. **/
...
private:
    enum state {
        may_have_children = 1,
        // the following enumerations must be the last, new 2^x values must go
        // above
        next_state_value,
        low_unused_state_bit = ( next_state_value - 1 ) * 2
    };

    union {
        //! Flavor of this context: bound or isolated.
        // TODO: describe asynchronous use, and whether any memory semantics are
        // needed
    ...
    //! Used to set and maintain stack stitching point for Intel Performance
    //! Tools.
    __itt_caller itt_caller;

    //! Leading padding protecting accesses to frequently used members from
    //! false sharing.
    /** Read accesses to the field my_cancellation_requested are on the hot path
       inside the scheduler. This padding ensures that this field never shares
       the same cache line with a local variable that is frequently written to.
       **/
    char _leading_padding[ internal::NFS_MaxLineSize - 2 * sizeof( uintptr_t ) -
                           sizeof( void* ) -
    ...
    //! Scheduler instance that registered this context in its thread specific
    //! list.
    internal::generic_scheduler* my_owner;

    //! Internal state (combination of state flags, currently only
    //! may_have_children).
    uintptr_t my_state;
    ...
    //! Trailing padding protecting accesses to frequently used members from
    //! false sharing
    /** \sa _leading_padding **/
    char _trailing_padding[ internal::NFS_MaxLineSize -
                            2 * sizeof( uintptr_t ) - 2 * sizeof( void* )
#if __TBB_TASK_PRIORITY
                            - sizeof( intptr_t )
#endif /* __TBB_TASK_PRIORITY */
...
public:
    ...
    /** By default a bound context is created. That is this context will be
       bound (as child) to the context of the task calling
       ...
       Similarly priority change is propagated from the parent context to its
       children.
       ...
       algorithms cancelled as well. Such a behavior requires nested algorithms
       to use bound contexts.

        There is one good place where using isolated algorithms is beneficial.
       It is a master thread. That is if a particular algorithm is invoked
       directly from the master thread (not from a TBB task), supplying it with
       ...
       startup.
        ...
        entirely out-of-line because the run-time version must be set by the
       user code. This will become critically important for binary
       compatibility, if we ever have to change the size of the context object.

        Boosting the runtime version will also be necessary if new data fields
       are introduced in the currently unused padding areas and these fields are
       updated by inline methods. **/
    ...
    // Do not introduce standalone unbind method since it will break state
    // propagation assumptions
    __TBB_EXPORTED_METHOD ~task_group_context();

    //! Forcefully reinitializes the context after the task tree it was
    //! associated with is completed.
    ...
    //! subordinate groups.
    /** \return false if cancellation has already been requested, true
       otherwise.

        Note that canceling never fails. When false is returned, it just means
       that another thread (or this one) has already sent cancellation request
       to this context or to one of its ancestors (if this context is bound). It
       is guaranteed that when this method is concurrently called on the same
       not yet cancelled context, true will be returned by one and only one
       invocation. **/
    ...
    /** May be called only from inside a catch-block. If the context is already
        ...
        The method brings the task group associated with this context exactly
       into the state it would be in, if one of its tasks threw the currently
       pending exception during its execution. In other words, it emulates the
       actions of the scheduler's dispatch loop exception handler. **/
    ...
    /** Because the method assumes that all the tasks that used to be associated
       ...
       still in use somewhere in the task hierarchy leads to undefined behavior.
        ...
        The method does not change the FPU control settings of the context's
       parent. **/
...
protected:
    ...
    /** Singled out to ensure backward binary compatibility of the future
     * versions. **/
...
private:
    ...
    template < typename T >
    void propagate_task_group_state( T task_group_context::* mptr_state,
    ...
    // TODO: Consider adding #else stub in order to omit #if sections in other
    // code
...
class task : __TBB_TASK_BASE_ACCESS interface5::internal::task_base {
    ...
    internal::reference_count __TBB_EXPORTED_METHOD
...
public:
    ...
    enum state_type {
        ...
        allocated,
        //! task object is on free list, or is going to be put there, or was
        //! just taken off.
        freed,
...
#if __TBB_TASK_GROUP_CONTEXT
    //! Returns proxy for overloaded new that allocates a root task associated
    ...
    //! Returns proxy for overloaded new that allocates a continuation task of
    //! *this.
    /** The continuation's parent becomes the parent of *this. */
    internal::allocate_continuation_proxy& allocate_continuation() {
        return *reinterpret_cast< internal::allocate_continuation_proxy* >(
            this );
    }
    ...
    /** The caller must guarantee that the task's refcount does not become zero
       ...
       guarantee cannot be made, use method recycle_as_safe_continuation
       instead.
    ...
    /** For safety, it requires additional increment of ref_count.
        With no descendants and ref_count of 1, it has the semantics of
    ...
    void recycle_as_child_of( task& new_parent ) {
        ...
        __TBB_ASSERT(
            prefix().state == executing || prefix().state == allocated,
            "execute not running, or already recycled" );
        __TBB_ASSERT( prefix().ref_count == 0,
                      "no child tasks allowed when recycled as a child" );
        __TBB_ASSERT( p.parent == NULL, "parent must be null" );
        __TBB_ASSERT( new_parent.prefix().state <= recycle,
                      "corrupt parent's state" );
        __TBB_ASSERT( new_parent.prefix().state != freed,
    ...
    void recycle_to_reexecute() {
        __TBB_ASSERT( prefix().state == executing,
                      "execute not running, or already recycled" );
        __TBB_ASSERT( prefix().ref_count == 0,
    ...
    /** Save enqueue/spawn difference, it has the semantics of
    ...
    int add_ref_count( int count ) {
        ...
        internal::reference_count k =
            count + __TBB_FetchAndAddW( &prefix().ref_count, count );
        __TBB_ASSERT( k >= 0, "task's reference count underflowed" );
        if ( k == 0 )
            internal::call_itt_notify( internal::acquired,
                                       &prefix().ref_count );
        return int( k );
    }
    ...
    /** The task will be enqueued on the normal priority level disregarding the
        ...
        statically fixed at the moment of its enqueuing, while task group
       priority is dynamic. Thus automatic priority inheritance would be
       generally a subject to the race, which may result in unexpected behavior.

        Use enqueue() overload with explicit priority value and
       task::group_priority() method to implement such priority inheritance when
       it is really necessary. **/
#endif /* __TBB_TASK_PRIORITY */
    static void enqueue( task& t ) { t.prefix().owner->enqueue( t, NULL ); }

#if __TBB_TASK_PRIORITY
    //! Enqueue task for starvation-resistant execution on the specified
    //! priority level.
    __TBB_DEPRECATED static void enqueue( task& t, priority_t p ) {
#if __TBB_PREVIEW_CRITICAL_TASKS
        __TBB_ASSERT( p == priority_low || p == priority_normal ||
                          p == priority_high ||
                          p == internal::priority_critical,
...
#if __TBB_TASK_PRIORITY
    __TBB_DEPRECATED inline static void
    enqueue( task& t, task_arena& arena, priority_t p = priority_t( 0 ) );
#else
...
#if __TBB_TASK_GROUP_CONTEXT
        __TBB_ASSERT( !p || prefix().context == p->prefix().context,
...
#if TBB_USE_ASSERT
#if __TBB_PREVIEW_RESUMABLE_TASKS
        internal::reference_count ref_count_ =
            prefix().ref_count & ~internal::abandon_flag;
#else
        internal::reference_count ref_count_ = prefix().ref_count;
#endif
        __TBB_ASSERT( ref_count_ == int( ref_count_ ),
    ...
    //! Obsolete, and only retained for the sake of backward compatibility.
    //! Always returns true.
    ...
    /** Argument ctx specifies the new group.
        ...
        The primary purpose of this method is to associate unique task group
       context with a task allocated for subsequent enqueuing. In contrast to
       spawned tasks enqueued ones normally outlive the scope where they were
       created. This makes traditional usage model where task group context are
       allocated locally on the stack inapplicable. Dynamic allocation of
       context objects is performance inefficient. Method change_group() allows
       to make task group context object a member of the task class, and then
       associate it with its containing task object in the latter's constructor.
       **/
...
private:
    ...
    internal::task_prefix& prefix( internal::version_tag* = NULL ) const {
        return reinterpret_cast< internal::task_prefix* >(
            const_cast< task* >( this ) )[ -1 ];
    }
#if __TBB_PREVIEW_CRITICAL_TASKS
...
static void suspend_callback( void* user_callback, task::suspend_point tag ) {
    // Copy user function to a new stack to avoid a race when the previous
...
#if __TBB_TODO
    // TODO: add this method and implement&document the local execution
    // ordering. See more in generic_scheduler::local_spawn
