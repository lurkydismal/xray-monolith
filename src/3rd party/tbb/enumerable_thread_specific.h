...
inline task::suspend_point atomic_compare_and_swap(
    task::suspend_point& location,
    const task::suspend_point& value,
...
protected:
    ...
    array* allocate( size_t lg_size ) {
        ...
        array* a = static_cast< array* >(
            create_array( sizeof( array ) + n * sizeof( slot ) ) );
        a->lg_size = lg_size;
    ...
    void free( array* a ) {
        ...
        free_array( ( void* )a,
                    size_t( sizeof( array ) + n * sizeof( slot ) ) );
    }
    ...
    void table_elementwise_copy( const ets_base& other,
        ...
        for ( array* r = other.my_root; r; r = r->next ) {
            for ( size_t i = 0; i < r->size(); ++i ) {
                slot& s1 = r->at( i );
                if ( !s1.empty() ) {
                    for ( size_t j = root->start(
                              tbb::tbb_hash< key_type >()( s1.key ) );
                          ; j = ( j + 1 ) & mask ) {
                        slot& s2 = root->at( j );
                        if ( s2.empty() ) {
                            s2.ptr = add_element(
                                static_cast< ets_base< E2 >& >( *this ),
                                s1.ptr );
                            s2.key = s1.key;
                            break;
...
void* ets_base< ETS_key_type >::table_lookup( bool& exists ) {
        ...
        for ( size_t i = r->start( h );; i = ( i + 1 ) & mask ) {
            ...
            if ( s.match( k ) ) {
                if ( r == my_root ) {
                    ...
                    // Success at some other level.  Need to insert at top
                    // level.
                    exists = true;
                    found = s.ptr;
                    goto insert;
    ...
    // Key does not yet exist.  The density of slots in the table does not
    // exceed 0.5, for if this will occur a new table is allocated with double
    // the current table size, which is swapped in as the new root table.  So an
...
insert:
    // Whether a slot has been found in an older table, or if it has been
    // inserted at this level, it has already been accounted for in the total.
    // Guaranteed to be room for it, and it is not present, so search for empty
    // slot and use it.
    array* ir = my_root;
...
class ets_base< ets_key_per_instance > : public ets_base< ets_no_key > {
    ...
    virtual void* create_array( size_t _size )
        __TBB_override = 0; // _size in bytes
...
class enumerable_thread_specific_iterator
    ...
    // Ensure that Microsoft's internal template function _Val_type works
    // correctly.
...
#endif /* defined(_WIN64) && defined(_MSC_VER) */
{
    ...
    friend enumerable_thread_specific_iterator< C, T > operator+(
        ptrdiff_t offset,
...
public:
    ...
    enumerable_thread_specific_iterator()
        : my_container( NULL ), my_index( 0 ), my_value( NULL ) {}
    ...
    Value& operator*() const {
        ...
        __TBB_ASSERT( value == ( *my_container )[ my_index ].value(),
                      "corrupt cache" );
        return *value;
    }
...
enumerable_thread_specific_iterator< Container, T > operator+(
    ptrdiff_t offset,
    const enumerable_thread_specific_iterator< Container, T >& v ) {
    return enumerable_thread_specific_iterator< Container, T >(
        v.my_container, v.my_index + offset );
}
...
public:
    ...
    segmented_iterator( const SegmentedContainer& _segmented_container )
        : my_segcont(
              const_cast< SegmentedContainer* >( &_segmented_container ) ),
    ...
    segmented_iterator(
        ...
        : my_segcont( other.my_segcont ),
          ...
          // can we assign a default-constructed iterator to inner if we're at
          // the end?
    ...
    segmented_iterator& operator=( const outer_iterator& new_outer_iter ) {
        ...
        // check that this iterator points to something inside the segmented
        // container
...
class callback_leaf : public callback_base< T >, Constructor {
#if __TBB_ETS_USE_CPP11
    template < typename... P >
    callback_leaf( P&&... params )
        : Constructor( std::forward< P >( params )... ) {}
#else
    template < typename X >
    callback_leaf( const X& x ) : Constructor( x ) {}
#endif
    ...
    callback_base< T >* clone() const __TBB_override { return make( *this ); }
...
// A predicate that can be used for a compile-time compatibility check of ETS
...
#if __TBB_ETS_USE_CPP11
// A predicate that checks whether, for a variable 'foo' of type T, foo() is a
// valid expression
template < typename T >
...
public:
    static const bool value =
        ( sizeof( decide< T >( NULL ) ) == sizeof( yes ) );
};
#endif
...
initialization.
    - thread-local copies do not move (during lifetime, and excepting clear())
so the address of a copy is invariant.
...
used.
    ...
    - thread-local copies can be managed by hash-table, or can be accessed via
TLS storage for speed.
...
segmented iterator may be used to iterate over all the elements of all
thread-local copies.
...
class enumerable_thread_specific : internal::ets_base< ETS_key_type > {
    ...
    typedef typename Allocator::template rebind< padded_element >::other
        padded_allocator_type;
    typedef tbb::concurrent_vector< padded_element, padded_allocator_type >
    ...
    static void* create_local_by_copy( internal::ets_base< ETS_key_type >& base,
                                       void* p ) {
...
#if __TBB_ETS_USE_CPP11
    static void* create_local_by_move( internal::ets_base< ETS_key_type >& base,
                                       void* p ) {
        enumerable_thread_specific& ets =
            static_cast< enumerable_thread_specific& >( base );
        padded_element& lref = *ets.my_locals.grow_by( 1 );
    ...
    typedef typename Allocator::template rebind< uintptr_t >::other
    ...
    void free_array( void* _ptr, size_t _size ) __TBB_override {
        size_t nelements =
            ( _size + sizeof( uintptr_t ) - 1 ) / sizeof( uintptr_t );
        array_allocator_type().deallocate(
            reinterpret_cast< uintptr_t* >( _ptr ), nelements );
    }
...
public:
    ...
    // Iterator types
    typedef typename internal::enumerable_thread_specific_iterator<
        internal_collection_type,
        value_type >
        iterator;
    typedef typename internal::enumerable_thread_specific_iterator<
    ...
    enumerable_thread_specific()
        : my_construct_callback(
              internal::
                  callback_leaf< T, internal::construct_by_default< T > >::make(
                      /*dummy argument*/ 0 ) ) {}
...
#if __TBB_ETS_USE_CPP11
        ,
        typename = typename internal::enable_if< internal::is_callable_no_args<
            typename internal::strip< Finit >::type >::value >::type
#endif
        >
    explicit enumerable_thread_specific( Finit finit )
        : my_construct_callback(
              internal::
                  callback_leaf< T, internal::construct_by_finit< T, Finit > >::
                      make( tbb::internal::move( finit ) ) ) {
    }

    //! Constructor with exemplar. Each local instance of T is copy-constructed
    //! from the exemplar.
    explicit enumerable_thread_specific( const T& exemplar )
        : my_construct_callback(
              internal::callback_leaf<
                  T,
    ...
    explicit enumerable_thread_specific( T&& exemplar )
        : my_construct_callback(
              internal::callback_leaf< T,
                                       internal::construct_by_exemplar< T > >::
                  make( std::move( exemplar ) ) ) {}

    //! Variadic constructor with initializer arguments.  Each local instance of
    //! T is constructed by T(args...)
    template <
        typename P1,
        typename... P,
        typename = typename internal::enable_if<
            !internal::is_callable_no_args<
                typename internal::strip< P1 >::type >::value &&
            !internal::is_compatible_ets<
                T,
                typename internal::strip< P1 >::type >::value &&
            !internal::is_same_type< T, typename internal::strip< P1 >::type >::
                value >::type >
    enumerable_thread_specific( P1&& arg1, P&&... args )
        : my_construct_callback( internal::callback_leaf<
                                 T,
                                 internal::construct_by_args< T, P1, P... > >::
                                     make( std::forward< P1 >( arg1 ),
                                           std::forward< P >( args )... ) ) {}
#endif
    ...
    //! Returns reference to calling thread's local copy, creating one if
    //! necessary
...
#endif
        // Initialize my_construct_callback first, so that it is valid even if
        // rest of this routine throws an exception.
...
public:
    enumerable_thread_specific( const enumerable_thread_specific& other )
        : internal::ets_base< ETS_key_type >() /* prevents GCC warnings with
                                                  -Wextra */
    {
        internal_copy( other );
    }
    ...
    enumerable_thread_specific& operator=(
    ...
    enumerable_thread_specific& operator=(
        const enumerable_thread_specific< T, Alloc, Cachetype >& other ) {
        __TBB_ASSERT( static_cast< void* >( this ) !=
    ...
    // combine_func_t takes T by value or by [const] reference, and returns
    // nothing
    template < typename combine_func_t >
...
public:
    ...
    typedef typename internal::segmented_iterator< Container, value_type >
        iterator;
    typedef typename internal::segmented_iterator< Container, const value_type >
        const_iterator;
    ...
    size_type size() const {
        ...
        for ( typename Container::const_iterator i = my_begin; i != my_end;
              ++i ) {
            tot_size += i->size();
        }
        return tot_size;
