...
#if __TBB_CPP11_PRESENT && __TBB_CPP11_ARRAY_PRESENT && \
...
/*
    The blocked_rangeNd_impl uses make_index_sequence<N> to automatically
   generate a ctor with exactly N arguments of the type
   tbb::blocked_range<Value>. Such ctor provides an opportunity to use
   braced-init-list parameters to initialize each dimension. Use of parameters,
   whose representation is a braced-init-list, but they're not
   ...
   blocked_rangeNd_impl (and not e.g. a derived class), otherwise it would need
   to declare its own ctor facing the same problem that the impl class solves.
*/
...
public:
    ...
    //! Constructs N-dimensional range over N half-open intervals each
    //! represented as tbb::blocked_range<Value>.
    ...
    const tbb::blocked_range< value_type >& dim(
    ...
    blocked_rangeNd_impl( blocked_rangeNd_impl& r,
...
private:
    ...
    void do_split( blocked_rangeNd_impl& r, split_type proportion ) {
        __TBB_STATIC_ASSERT(
            ( is_same_type< split_type, split >::value ||
        ...
        // (!(my_it->my_begin < r_it->my_end) && !(r_it->my_end <
        // my_it->my_begin)) equals to (my_it->my_begin == r_it->my_end), but we
        // can't use operator== due to Value concept
...
#endif /* __TBB_CPP11_PRESENT && __TBB_CPP11_ARRAY_PRESENT && \
          __TBB_CPP11_TEMPLATE_ALIASES_PRESENT */
