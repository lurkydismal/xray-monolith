...
T event_info( cl_event e, cl_event_info i ) {
    T res;
    enforce_cl_retcode( clGetEventInfo( e, i, sizeof( res ), &res, NULL ),
...
T device_info( cl_device_id d, cl_device_info i ) {
    T res;
    enforce_cl_retcode( clGetDeviceInfo( d, i, sizeof( res ), &res, NULL ),
                        "Failed to get OpenCL device information" );
    return res;
}

template <>
inline std::string device_info< std::string >( cl_device_id d,
                                               cl_device_info i ) {
    size_t required;
    enforce_cl_retcode( clGetDeviceInfo( d, i, 0, NULL, &required ),
                        "Failed to get OpenCL device information" );

    char* buff = ( char* )alloca( required );
    enforce_cl_retcode( clGetDeviceInfo( d, i, required, buff, NULL ),
                        "Failed to get OpenCL device information" );
...
T platform_info( cl_platform_id p, cl_platform_info i ) {
    T res;
    enforce_cl_retcode( clGetPlatformInfo( p, i, sizeof( res ), &res, NULL ),
                        "Failed to get OpenCL platform information" );
    return res;
}

template <>
inline std::string platform_info< std::string >( cl_platform_id p,
                                                 cl_platform_info i ) {
    size_t required;
    enforce_cl_retcode( clGetPlatformInfo( p, i, 0, NULL, &required ),
                        "Failed to get OpenCL platform information" );

    char* buff = ( char* )alloca( required );
    enforce_cl_retcode( clGetPlatformInfo( p, i, required, buff, NULL ),
                        "Failed to get OpenCL platform information" );
...
public:
...
#if CL_VERSION_2_0
        if ( major_version() >= 2 )
            return ( device_info< cl_command_queue_properties >(
    ...
    bool built_in_kernel_available( const std::string& k ) const {
        ...
        // e.g. "add" is partly matched with "madd").
    ...
    bool extension_available( const std::string& ext ) const {
        ...
        // "ext" is partly matched with "ext2").
    ...
    cl_uint address_bits() const {
        return device_info< cl_uint >( my_cl_device_id,
                                       CL_DEVICE_ADDRESS_BITS );
    }

    cl_device_id device_id() const { return my_cl_device_id; }

    cl_command_queue command_queue() const { return my_cl_command_queue; }
    ...
    cl_platform_id platform_id() const {
        return device_info< cl_platform_id >( my_cl_device_id,
                                              CL_DEVICE_PLATFORM );
    }
...
private:
    ...
    friend bool operator==( opencl_device d1, opencl_device d2 ) {
        return d1.my_cl_device_id == d2.my_cl_device_id;
    }
...
inline opencl_device_list find_available_devices() {
    ...
    enforce_cl_retcode( clGetPlatformIDs( 0, NULL, &num_platforms ),
    ...
    while ( platforms_it != platforms.end() ) {
        cl_int err = clGetDeviceIDs( *platforms_it, CL_DEVICE_TYPE_ALL, 0, NULL,
                                     &num_devices );
    ...
    for ( auto p = platforms.begin(); p != platforms.end(); ++p ) {
        enforce_cl_retcode( clGetDeviceIDs( ( *p ), CL_DEVICE_TYPE_ALL,
                                            ( cl_uint )std::distance(
                                                devices_it, devices.end() ),
...
inline const opencl_device_list& available_devices() {
    ...
    static const opencl_device_list my_devices =
        internal::find_available_devices();
    return my_devices;
}
...
public:
    ...
    void call() __TBB_override { my_callback( my_data ); }
};
...
public:
    ...
    opencl_async_msg()
        : my_callback_flag_ptr( std::make_shared< tbb::atomic< bool > >() ) {
        my_callback_flag_ptr->store< tbb::relaxed >( false );
    }

    explicit opencl_async_msg( const T& data )
        : my_data( data ),
          my_callback_flag_ptr( std::make_shared< tbb::atomic< bool > >() ) {
        my_callback_flag_ptr->store< tbb::relaxed >( false );
    }
    ...
    opencl_async_msg( const T& data, cl_event event )
        : my_data( data ),
          ...
          my_callback_flag_ptr( std::make_shared< tbb::atomic< bool > >() ) {
        my_callback_flag_ptr->store< tbb::relaxed >( false );
        enforce_cl_retcode( clRetainEvent( my_event ),
                            "Failed to retain an event" );
    }

    T& data( bool wait = true ) {
        if ( my_is_event && wait ) {
            enforce_cl_retcode( clWaitForEvents( 1, &my_event ),
                                "Failed to wait for an event" );
            enforce_cl_retcode( clReleaseEvent( my_event ),
                                "Failed to release an event" );
            my_is_event = false;
        }
        return my_data;
    }
    ...
    opencl_async_msg( opencl_async_msg&& dmsg )
        : async_msg< T >( std::move( dmsg ) ),
          my_data( std::move( dmsg.my_data ) ),
          my_event( dmsg.my_event ),
          my_is_event( dmsg.my_is_event ),
    ...
    void set_event( cl_event e ) const {
        if ( my_is_event ) {
            ...
            if ( cq !=
                 event_info< cl_command_queue >( e, CL_EVENT_COMMAND_QUEUE ) )
                enforce_cl_retcode( clFlush( cq ),
    ...
    void clear_event() const {
        if ( my_is_event ) {
            enforce_cl_retcode( clFlush( event_info< cl_command_queue >(
                                    my_event, CL_EVENT_COMMAND_QUEUE ) ),
                                "Failed to flush an OpenCL command queue" );
            enforce_cl_retcode( clReleaseEvent( my_event ),
                                "Failed to release an event" );
    ...
    void register_callback( Callback c ) const {
        ...
        enforce_cl_retcode(
            clSetEventCallback( my_event, CL_COMPLETE, register_callback_func,
                                new callback< Callback, T >( c, my_data ) ),
            "Failed to set an OpenCL callback" );
    }
...
private:
    ...
    register_callback_func( cl_event,
...
public:
    ...
    ~opencl_memory() {
        if ( my_sending_event_present )
            enforce_cl_retcode(
                clReleaseEvent( my_sending_event ),
                "Failed to release an event for the OpenCL buffer" );
        enforce_cl_retcode( clReleaseMemObject( my_cl_mem ),
                            "Failed to release an memory object" );
    }

    cl_mem get_cl_mem() const { return my_cl_mem; }
    ...
    opencl_async_msg< void*, Factory > receive( const cl_event* e ) {
        ...
        // Concurrent receives are prohibited so we do not worry about
        // synchronization.
    ...
    opencl_async_msg< void*, Factory > send( opencl_device device,
                                             const cl_event* e ) {
        opencl_device::device_id_type device_id = device.my_device_id;
        if ( !my_factory->is_same_context(
                 my_curr_device_id.load< tbb::acquire >(), device_id ) ) {
            {
                tbb::spin_mutex::scoped_lock lock( my_sending_lock );
                if ( !my_factory->is_same_context(
                         my_curr_device_id.load< tbb::relaxed >(),
                         device_id ) ) {
                    __TBB_ASSERT( my_host_ptr,
                    ...
                    enforce_cl_retcode( clRetainEvent( my_sending_event ),
                                        "Failed to retain an event" );
                    my_host_ptr = NULL;
        ...
        if ( !e && my_sending_event_present )
            e = &my_sending_event;

        __TBB_ASSERT( !my_host_ptr, "The buffer has not been unmapped" );
        return e ? opencl_async_msg< void*, Factory >( NULL, *e )
                 : opencl_async_msg< void*, Factory >( NULL );
    }

    virtual void map_memory( opencl_device,
                             opencl_async_msg< void*, Factory >& ) = 0;
...
public:
    opencl_buffer_impl( size_t size, Factory& f )
        : opencl_memory< Factory >( f ), my_size( size ) {
        cl_int err;
        this->my_cl_mem =
            clCreateBuffer( this->my_factory->context(), CL_MEM_ALLOC_HOST_PTR,
                            size, NULL, &err );
    ...
    opencl_buffer_impl( cl_mem m, size_t index, size_t size, Factory& f )
        ...
        this->my_cl_mem = clCreateSubBuffer( m, 0, CL_BUFFER_CREATE_TYPE_REGION,
                                             &region, &err );
    ...
    void map_memory( opencl_device device,
                     opencl_async_msg< void*, Factory >& dmsg ) __TBB_override {
...
public:
    ...
    iterator< a > end() const {
        return access< a >() + my_impl->size() / sizeof( T );
    }
    ...
    cl_mem native_object() const { return my_impl->get_cl_mem(); }

    const opencl_buffer& memory_object() const { return *this; }

    void send( opencl_device device,
               opencl_async_msg< opencl_buffer, Factory >& dependency ) const {
        __TBB_ASSERT( dependency.data( /*wait = */ false ) == *this, NULL );
        opencl_async_msg< void*, Factory > d =
            my_impl->send( device, dependency.get_event() );
        const cl_event* e = d.get_event();
        if ( e )
            dependency.set_event( *e );
        else
    ...
    void receive(
        ...
        __TBB_ASSERT( dependency.data( /*wait = */ false ) == *this, NULL );
        opencl_async_msg< void*, Factory > d =
            my_impl->receive( dependency.get_event() );
        const cl_event* e = d.get_event();
        if ( e )
            dependency.set_event( *e );
        else
...
private:
    ...
    opencl_buffer( Factory& f, cl_mem m, size_t index, size_t size )
        : my_impl( std::make_shared< impl_type >( m,
                                                  index * sizeof( T ),
                                                  size * sizeof( T ),
                                                  f ) ) {}
    ...
    friend bool operator==( const opencl_buffer< T, Factory >& lhs,
...
public:
    ...
    opencl_subbuffer( const opencl_buffer< T, Factory >& owner,
                      size_t index,
                      size_t size )
        : opencl_buffer< T, Factory >( *owner.my_impl->factory(),
                                       owner.native_object(),
                                       index,
                                       size ),
          my_owner( owner ) {}
...
template < typename T, typename Factory >
opencl_subbuffer< T, Factory > opencl_buffer< T, Factory >::subbuffer(
    size_t index,
    size_t size ) const {
...
template < typename T >
typename std::enable_if< is_native_object_type< T >::value,
                         typename T::native_object_type >::type
get_native_object( const T& t ) {
    return t.native_object();
}

template < typename T >
typename std::enable_if< !is_native_object_type< T >::value, T >::type
get_native_object( T t ) {
    return t;
}

// send_if_memory_object checks if the T type has memory_object_type and call
// the send method for the object.
template < typename T, typename Factory >
typename std::enable_if< is_memory_object_type< T >::value >::type
send_if_memory_object( opencl_device device,
    ...
    if ( dmsg.get_event() )
        d.set_event( *dmsg.get_event() );
    mem_obj.send( device, d );
    if ( d.get_event() )
        dmsg.set_event( *d.get_event() );
}

template < typename T >
typename std::enable_if< is_memory_object_type< T >::value >::type
send_if_memory_object( opencl_device device, T& t ) {
    typedef typename T::memory_object_type mem_obj_t;
    mem_obj_t mem_obj = t.memory_object();
    opencl_async_msg< mem_obj_t, typename mem_obj_t::opencl_factory_type > dmsg(
        mem_obj );
...
template < typename T >
typename std::enable_if< !is_memory_object_type< T >::value >::type
send_if_memory_object( opencl_device, T& ) {};

// receive_if_memory_object checks if the T type has memory_object_type and call
// the receive method for the object.
template < typename T, typename Factory >
typename std::enable_if< is_memory_object_type< T >::value >::type
receive_if_memory_object( const opencl_async_msg< T, Factory >& dmsg ) {
    const T& t = dmsg.data( false );
    typedef typename T::memory_object_type mem_obj_t;
    mem_obj_t mem_obj = t.memory_object();
    opencl_async_msg< mem_obj_t, Factory > d( mem_obj );
...
template < typename T >
typename std::enable_if< !is_memory_object_type< T >::value >::type
receive_if_memory_object( const T& ) {}
...
public:
    ...
    template < typename G = std::initializer_list< int >,
               typename L = std::initializer_list< int >,
               typename = typename std::enable_if<
                   !std::is_same< typename std::decay< G >::type,
                                  opencl_range >::value >::type >
    opencl_range( G&& global_work = std::initializer_list< int >( { 0 } ),
                  L&& local_work = std::initializer_list< int >( { 0, 0,
                                                                   0 } ) ) {
        ...
        for ( int s = 0; s < 3 && g_it != global_work.end();
              ++g_it, ++l_it, ++s ) {
            __TBB_ASSERT( l_it != local_work.end(),
...
public:
    template < typename T >
    using async_msg_type =
        opencl_async_msg< T, opencl_factory< DeviceFilter > >;
            ...
            for ( size_t curr_size = 32;; curr_size <<= 1 ) {
                ...
                enforce_cl_retcode(
                    clGetKernelInfo( k.my_cl_kernel, CL_KERNEL_FUNCTION_NAME,
                                     curr_size, kernel_name.data(), &ret_size ),
                    "Failed to get kernel info" );
                if ( ret_size < curr_size )
                    break;
            }

            cl_program program;
            enforce_cl_retcode(
                clGetKernelInfo( k.my_cl_kernel, CL_KERNEL_PROGRAM,
                                 sizeof( program ), &program, &ret_size ),
                "Failed to get kernel info" );
            __TBB_ASSERT( ret_size == sizeof( program ), NULL );

            my_cl_kernel = opencl_program< factory_type >( my_factory, program )
                               .get_cl_kernel( kernel_name.data() );
        }

        ~kernel() {
            enforce_cl_retcode( clReleaseKernel( my_cl_kernel ),
                                "Failed to release a kernel" );
        }
    ...
    ~opencl_factory() {
        if ( my_devices.size() ) {
            for ( auto d = my_devices.begin(); d != my_devices.end(); ++d ) {
                enforce_cl_retcode(
                    clReleaseCommandQueue( ( *d ).my_cl_command_queue ),
                    "Failed to release a command queue" );
            }
            enforce_cl_retcode( clReleaseContext( my_cl_context ),
...
private:
    template < typename Factory >
    void enqueue_map_buffer( opencl_device device,
        ...
        void* ptr = clEnqueueMapBuffer(
            device.my_cl_command_queue, buffer.get_cl_mem(), false,
            CL_MAP_READ | CL_MAP_WRITE, 0, buffer.size(), e1 == NULL ? 0 : 1,
            e1, &e2, &err );
        enforce_cl_retcode( err, "Failed to map a buffer" );
        dmsg.data( false ) = ptr;
        dmsg.set_event( e2 );
        enforce_cl_retcode( clReleaseEvent( e2 ),
                            "Failed to release an event" );
    }

    template < typename Factory >
    void enqueue_unmap_buffer( opencl_device device,
                               opencl_memory< Factory >& memory,
                               opencl_async_msg< void*, Factory >& dmsg ) {
        cl_event const* e1 = dmsg.get_event();
        cl_event e2;
        enforce_cl_retcode(
            clEnqueueUnmapMemObject( device.my_cl_command_queue,
                                     memory.get_cl_mem(), memory.get_host_ptr(),
                                     e1 == NULL ? 0 : 1, e1, &e2 ),
    ...
    void process_one_arg( const kernel_type& kernel,
        ...
        enforce_cl_retcode(
            clSetKernelArg( kernel.my_cl_kernel, place++, sizeof( p ), &p ),
    ...
    void process_one_arg( const kernel_type& kernel,
        ...
        __TBB_ASSERT(
            ( static_cast<
                  typename std::array< cl_event, NUM_ARGS >::size_type >(
    ...
    void process_arg_list( const kernel_type& kernel,
                           std::array< cl_event, NUM_ARGS >& events,
                           int& num_events,
                           int& place,
                           const T& t,
    ...
    void process_arg_list( const kernel_type&,
                           std::array< cl_event, NUM_ARGS >&,
                           int&,
                           int& ) {}
...
public:
    ...
    void send_kernel( opencl_device device,
        ...
        const cl_event e = send_kernel_impl(
            device, kernel.my_cl_kernel, work_size, num_events, events.data() );
...
private:
    cl_event send_kernel_impl( opencl_device device,
                               const cl_kernel& kernel,
                               const range_type& work_size,
                               cl_uint num_events,
                               cl_event* event_list ) {
        const typename range_type::nd_range_type g_offset = { { 0, 0, 0 } };
        const typename range_type::nd_range_type& g_size =
            work_size.global_range();
        const typename range_type::nd_range_type& l_size =
            work_size.local_range();
        cl_uint s;
        ...
        enforce_cl_retcode(
            clEnqueueNDRangeKernel(
                device.my_cl_command_queue, kernel, s, g_offset.data(),
                g_size.data(), l_size[ 0 ] ? l_size.data() : NULL, num_events,
                num_events ? event_list : NULL, &event ),
    ...
    template < typename T, typename F >
    bool get_event_from_one_arg( cl_event& e,
                                 const opencl_async_msg< T, F >& msg ) {
    ...
    static void CL_CALLBACK finalize_callback( cl_event,
                                               cl_int event_command_exec_status,
                                               void* data ) {
        tbb::internal::suppress_unused_warning( event_command_exec_status );
        __TBB_ASSERT( event_command_exec_status == CL_COMPLETE, NULL );
        ...
        // Function pointer was created by 'new' & this callback must be called
        // once only
        delete fn_ptr;
    }
...
public:
    ...
    void finalize( opencl_device device, FinalizeFn fn, Args&... args ) {
        ...
        if ( get_event_from_args( e, args... ) ) {
            enforce_cl_retcode(
                clSetEventCallback( e, CL_COMPLETE, finalize_callback,
                                    new finalize_fn_leaf< FinalizeFn >( fn ) ),
                "Failed to set a callback" );
        }

        enforce_cl_retcode( clFlush( device.my_cl_command_queue ),
                            "Failed to flush an OpenCL command queue" );
    }
...
private:
    bool is_same_context( opencl_device::device_id_type d1,
                          opencl_device::device_id_type d2 ) {
        __TBB_ASSERT(
            d1 != opencl_device::unknown && d2 != opencl_device::unknown,
            NULL );
        // Currently, factory supports only one context so if the both devices
        // are not host it means the are in the same context.
...
private:
    ...
    void init_once() {
        ...
        enforce_cl_retcode( my_devices.size() ? CL_SUCCESS : CL_INVALID_DEVICE,
                            "No devices in the device list" );
        cl_platform_id platform_id = my_devices.begin()->platform_id();
        for ( opencl_device_list::iterator it = ++my_devices.begin();
              it != my_devices.end(); ++it )
            enforce_cl_retcode( it->platform_id() == platform_id
                                    ? CL_SUCCESS
                                    : CL_INVALID_PLATFORM,
        ...
        cl_context ctx = clCreateContext(
            context_properties, ( cl_uint )cl_device_ids.size(),
            cl_device_ids.data(), NULL, NULL, &err );
        ...
        for ( auto d = my_devices.begin(); d != my_devices.end(); d++ ) {
            ...
            if ( ( *d ).major_version() >= 2 ) {
                if ( ( *d ).out_of_order_exec_mode_on_host_present() ) {
                    ...
                    cq = clCreateCommandQueueWithProperties(
                        ctx, ( *d ).my_cl_device_id, props, &err2 );
                } else {
...
#endif
            {
                cl_command_queue_properties props =
                    ( *d ).out_of_order_exec_mode_on_host_present()
...
#endif
#endif
                cq = clCreateCommandQueue( ctx, ( *d ).my_cl_device_id, props,
                                           &err2 );
#if _MSC_VER || __INTEL_COMPILER
#pragma warning( pop )
#endif
...
struct default_device_filter {
    opencl_device_list operator()( const opencl_device_list& devices ) {
        ...
        for ( opencl_device_list::const_iterator it = devices.cbegin();
              it != devices.cend(); ++it ) {
...
class default_opencl_factory : public opencl_factory< default_device_filter >,
...
template < typename T, typename Factory >
opencl_buffer< T, Factory >::opencl_buffer( size_t size )
    : my_impl(
          std::make_shared< impl_type >( size * sizeof( T ),
                                         opencl_info::default_factory() ) ) {}
...
public:
    ...
    opencl_program( Factory& factory,
    ...
    opencl_program( const opencl_program& src )
        : my_factory( src.my_factory ),
          my_type( src.type ),
          my_arg_str( src.my_arg_str ),
          my_cl_program( src.my_cl_program ) {
...
private:
    opencl_program( Factory& factory, cl_program program )
        : my_factory( factory ), my_cl_program( program ) {
        // Set my_do_once_flag to the called state.
        std::call_once( my_do_once_flag, []() {} );
    }
    ...
    cl_kernel get_cl_kernel( const std::string& k ) const {
        ...
        enforce_cl_retcode( err,
                            std::string( "Failed to create kernel: " ) + k );
        return kernel;
    }
    ...
    public:
        file_reader( const std::string& filepath ) {
            ...
            if ( !file_descriptor.is_open() ) {
                std::string str =
                    std::string( "Could not open file: " ) + filepath;
    ...
    public:
        ...
        opencl_program_builder( Factory& f,
                                ...
                                void* user_data ) {
            cl_int err = clBuildProgram( program, num_devices, device_list,
            ...
            if ( err == CL_BUILD_PROGRAM_FAILURE ) {
                ...
                for ( auto d = devices.begin(); d != devices.end(); ++d ) {
                    std::cerr << "Build log for device: " << ( *d ).name()
                              << std::endl;
                    size_t log_size;
                    cl_int query_err = clGetProgramBuildInfo(
                        program, ( *d ).my_cl_device_id, CL_PROGRAM_BUILD_LOG,
                        0, NULL, &log_size );
                    enforce_cl_retcode( query_err,
                                        "Failed to get build log size" );
                    if ( log_size ) {
                        std::vector< char > output;
                        output.resize( log_size );
                        query_err = clGetProgramBuildInfo(
                            program, ( *d ).my_cl_device_id,
                            CL_PROGRAM_BUILD_LOG, output.size(), output.data(),
                            NULL );
                        enforce_cl_retcode( query_err,
                                            "Failed to get build output" );
    ...
    public:
        ...
        opencl_device_filter( cl_uint& num_devices,
                              cl_device_id* device_list,
                              Filter filter,
                              const char* message ) {
    ...
    void init( const std::string& ) const {
        cl_uint num_devices;
        enforce_cl_retcode(
            clGetContextInfo( my_factory.context(), CL_CONTEXT_NUM_DEVICES,
                              sizeof( num_devices ), &num_devices, NULL ),
            "Failed to get OpenCL context info" );
        if ( !num_devices )
            enforce_cl_retcode( CL_DEVICE_NOT_FOUND,
                                "No supported devices found" );
        cl_device_id* device_list =
            ( cl_device_id* )alloca( num_devices * sizeof( cl_device_id ) );
        enforce_cl_retcode(
            clGetContextInfo( my_factory.context(), CL_CONTEXT_DEVICES,
                              num_devices * sizeof( cl_device_id ), device_list,
                              NULL ),
            "Failed to get OpenCL context info" );
        ...
        switch ( my_type ) {
            case opencl_program_type::SOURCE: {
                ...
                cl_int err;
                my_cl_program = clCreateProgramWithSource( my_factory.context(),
                                                           1, s, l, &err );
                enforce_cl_retcode(
                    err,
                    std::string( "Failed to create program: " ) + my_arg_str );
                ...
                opencl_program_builder( my_factory, my_arg_str, my_cl_program,
                                        num_devices, device_list, options,
                                        /*callback*/ NULL, /*user data*/ NULL );
                break;
            }
            ...
            case opencl_program_type::PRECOMPILED: {
                ...
                std::vector< const unsigned char* > s(
                    num_devices,
                    reinterpret_cast< const unsigned char* >( fr.content() ) );
                std::vector< size_t > l( num_devices, fr.length() );
                std::vector< cl_int > bin_statuses( num_devices, -1 );
                cl_int err;
                my_cl_program = clCreateProgramWithBinary(
                    my_factory.context(), num_devices, device_list, l.data(),
                    s.data(), bin_statuses.data(), &err );
                if ( err != CL_SUCCESS ) {
                    std::string statuses_str;
                    for ( auto st = bin_statuses.begin();
                          st != bin_statuses.end(); ++st ) {
                        statuses_str += std::to_string( ( *st ) );
                    }

                    enforce_cl_retcode(
                        err, std::string( "Failed to create program, error " +
                                          std::to_string( err ) + " : " ) +
                                 my_arg_str +
                                 std::string( ", binary_statuses = " ) +
                                 statuses_str );
                }
                opencl_program_builder( my_factory, my_arg_str, my_cl_program,
                                        num_devices, device_list, options,
...
public:
    ...
    opencl_node( graph& g, const kernel_type& kernel )
        : base_type( g,
                     kernel,
                     opencl_info::default_device_selector<
                         opencl_info::default_opencl_factory >(),
                     opencl_info::default_factory() ) {
        tbb::internal::fgt_multiinput_multioutput_node(
            CODEPTR(), tbb::internal::FLOW_OPENCL_NODE, this, &this->my_graph );
    }

    opencl_node( graph& g, const kernel_type& kernel, Factory& f )
        : base_type( g,
                     kernel,
                     opencl_info::default_device_selector< Factory >(),
                     f ) {
        tbb::internal::fgt_multiinput_multioutput_node(
            CODEPTR(), tbb::internal::FLOW_OPENCL_NODE, this, &this->my_graph );
    }
...
class opencl_node< tuple< Ports... >, JP >
    : public opencl_node< tuple< Ports... >,
                          JP,
                          opencl_info::default_opencl_factory > {
    typedef opencl_node< tuple< Ports... >,
                         JP,
                         opencl_info::default_opencl_factory >
        base_type;
