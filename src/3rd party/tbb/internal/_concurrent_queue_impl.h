...
public:
    ...
    char pad3[ NFS_MaxLineSize - sizeof( size_t ) - sizeof( size_t ) -
...
private:
    ...
    void copy_item( page& dst,
        ...
        construct_item( &get_ref( dst, dindex ),
                        static_cast< const void* >( &src_item ) );
    }
    ...
    void spin_wait_until_my_turn( atomic< ticket >& counter,
                                  ticket k,
                                  concurrent_queue_rep_base& rb ) const;
...
public:
    ...
    static T& get_ref( page& p, size_t index ) {
        return ( &static_cast< padded_page* >( static_cast< void* >( &p ) )
                      ->last )[ index ];
    }
    ...
    micro_queue& assign( const micro_queue& src,
                         concurrent_queue_base_v3< T >& base,
                         item_constructor_t construct_item );

    page* make_copy( concurrent_queue_base_v3< T >& base,
                     const page* src_page,
...
template < typename T >
void micro_queue< T >::spin_wait_until_my_turn(
...
template < typename T >
void micro_queue< T >::push( const void* item,
                             ticket k,
                             concurrent_queue_base_v3< T >& base,
                             item_constructor_t construct_item ) {
    ...
    if ( tail_counter != k )
        spin_wait_until_my_turn( tail_counter, k, *base.my_rep );
...
template < typename T >
bool micro_queue< T >::pop( void* dst,
                            ticket k,
                            concurrent_queue_base_v3< T >& base ) {
    k &= -concurrent_queue_rep_base::n_queue;
    if ( head_counter != k )
    ...
    bool success = false;
    {
        micro_queue_pop_finalizer< T > finalizer(
            *this, base, k + concurrent_queue_rep_base::n_queue,
...
template < typename T >
micro_queue< T >& micro_queue< T >::assign(
    const micro_queue< T >& src,
    concurrent_queue_base_v3< T >& base,
    item_constructor_t construct_item ) {
    ...
    if ( is_valid_page( srcp ) ) {
        ...
        __TBB_TRY {
            size_t n_items = ( tail_counter - head_counter ) /
                             concurrent_queue_rep_base::n_queue;
            size_t index = modulo_power_of_two(
                head_counter / concurrent_queue_rep_base::n_queue,
                base.my_rep->items_per_page );
            size_t end_in_first_page =
                ( index + n_items < base.my_rep->items_per_page )
                    ? ( index + n_items )
                    : base.my_rep->items_per_page;

            head_page = make_copy( base, srcp, index, end_in_first_page,
                                   g_index, construct_item );
            page* cur_page = head_page;

            if ( srcp != src.tail_page ) {
                for ( srcp = srcp->next; srcp != src.tail_page;
                      srcp = srcp->next ) {
                    cur_page->next =
                        make_copy( base, srcp, 0, base.my_rep->items_per_page,
                                   g_index, construct_item );
                    cur_page = cur_page->next;
                }

                __TBB_ASSERT( srcp == src.tail_page, NULL );
                size_t last_index = modulo_power_of_two(
                    tail_counter / concurrent_queue_rep_base::n_queue,
                    base.my_rep->items_per_page );
                if ( last_index == 0 )
                    last_index = base.my_rep->items_per_page;
...
concurrent_queue_rep_base::page* micro_queue< T >::make_copy(
    concurrent_queue_base_v3< T >& base,
    const concurrent_queue_rep_base::page* src_page,
    size_t begin_in_page,
    size_t end_in_page,
    ticket& g_index,
    item_constructor_t construct_item ) {
...
public:
    micro_queue_pop_finalizer( micro_queue< T >& queue,
...
/**
 * the class inherits from concurrent_queue_rep_base and defines an array of
 * micro_queue<T>'s
 */
template < typename T >
struct concurrent_queue_rep : public concurrent_queue_rep_base {
    ...
    static size_t index( ticket k ) { return k * phi % n_queue; }
...
private:
    ...
    virtual page* allocate_page() __TBB_override {
        ...
        size_t n =
            sizeof( padded_page ) + ( r.items_per_page - 1 ) * sizeof( T );
...
protected:
    ...
    virtual ~concurrent_queue_base_v3() {
        ...
        for ( size_t i = 0; i < nq; i++ )
            __TBB_ASSERT( my_rep->array[ i ].tail_page == NULL,
                          "pages were not freed properly" );
#endif /* TBB_USE_ASSERT */
        cache_aligned_allocator< concurrent_queue_rep< T > >().deallocate(
            my_rep, 1 );
    }
    ...
    /* note that the name may be misleading, but it remains so due to a
     * historical accident. */
    void internal_finish_clear();

    //! Obsolete
    void internal_throw_exception() const { throw_exception( eid_bad_alloc ); }
...
concurrent_queue_base_v3< T >::concurrent_queue_base_v3() {
    ...
    my_rep =
        cache_aligned_allocator< concurrent_queue_rep< T > >().allocate( 1 );
    __TBB_ASSERT( ( size_t )my_rep % NFS_GetLineSize() == 0,
                  "alignment error" );
    __TBB_ASSERT( ( size_t )&my_rep->head_counter % NFS_GetLineSize() == 0,
                  "alignment error" );
    __TBB_ASSERT( ( size_t )&my_rep->tail_counter % NFS_GetLineSize() == 0,
                  "alignment error" );
    __TBB_ASSERT( ( size_t )&my_rep->array % NFS_GetLineSize() == 0,
                  "alignment error" );
    memset( static_cast< void* >( my_rep ), 0,
...
void concurrent_queue_base_v3< T >::internal_finish_clear() {
    ...
    for ( size_t i = 0; i < nq; ++i ) {
        ...
        if ( is_valid_page( tp ) ) {
            __TBB_ASSERT( r.array[ i ].head_page == tp,
                          "at most one page should remain" );
            deallocate_page( tp );
            r.array[ i ].tail_page = NULL;
        } else
            __TBB_ASSERT( !is_valid_page( r.array[ i ].head_page ),
                          "head page pointer corrupt?" );
    }
}

template < typename T >
void concurrent_queue_base_v3< T >::assign(
...
public:
    ...
    //! Set item to point to kth element.  Return true if at end of queue or
    //! item is marked valid; false otherwise.
...
bool concurrent_queue_iterator_rep< T >::get_item( T*& item, size_t k ) {
    if ( k == my_queue.my_rep->tail_counter ) {
        ...
        typename concurrent_queue_base_v3< T >::page* p =
            array[ concurrent_queue_rep< T >::index( k ) ];
        __TBB_ASSERT( p, NULL );
        size_t i = modulo_power_of_two( k / concurrent_queue_rep< T >::n_queue,
...
class concurrent_queue_iterator_base_v3 {
    ...
    friend bool operator==( const concurrent_queue_iterator< C, T >& i,
...
protected:
    ...
    concurrent_queue_iterator_base_v3& operator=(
        const concurrent_queue_iterator_base_v3& i ) {
        assign( i );
        return *this;
    }
    ...
    //! Destructor
    ~concurrent_queue_iterator_base_v3() {
        cache_aligned_allocator< concurrent_queue_iterator_rep< Value > >()
            .deallocate( my_rep, 1 );
        my_rep = NULL;
    }
};
...
template < typename Value >
concurrent_queue_iterator_base_v3< Value >::concurrent_queue_iterator_base_v3(
    const concurrent_queue_base_v3< Value >& queue ) {
    my_rep = cache_aligned_allocator< concurrent_queue_iterator_rep< Value > >()
                 .allocate( 1 );
    new ( my_rep ) concurrent_queue_iterator_rep< Value >( queue );
    size_t k = my_rep->head_counter;
    if ( !my_rep->get_item( my_item, k ) )
        advance();
}

template < typename Value >
void concurrent_queue_iterator_base_v3< Value >::assign(
    ...
    if ( my_rep != other.my_rep ) {
        ...
        if ( other.my_rep ) {
            my_rep = cache_aligned_allocator<
                         concurrent_queue_iterator_rep< Value > >()
                         .allocate( 1 );
            new ( my_rep )
                concurrent_queue_iterator_rep< Value >( *other.my_rep );
        }
    }
...
#endif /* TBB_USE_ASSERT */
    size_t i = modulo_power_of_two( k / concurrent_queue_rep< Value >::n_queue,
                                    queue.my_rep->items_per_page );
    if ( i == queue.my_rep->items_per_page - 1 ) {
        typename concurrent_queue_base_v3< Value >::page*& root =
            my_rep->array[ concurrent_queue_rep< Value >::index( k ) ];
        root = root->next;
    }
    // advance k
...
template < typename Container, typename Value >
class concurrent_queue_iterator
    : public concurrent_queue_iterator_base_v3<
          typename tbb_remove_cv< Value >::type >,
...
#endif
    //! Construct iterator pointing to head of queue.
    explicit concurrent_queue_iterator(
        const concurrent_queue_base_v3< typename tbb_remove_cv< Value >::type >&
            queue )
        : concurrent_queue_iterator_base_v3<
              typename tbb_remove_cv< Value >::type >( queue ) {}

public:
    concurrent_queue_iterator() {}

    /** If Value==Container::value_type, then this routine is the copy
       constructor. If Value==const Container::value_type, then this routine is
       a conversion constructor. */
    concurrent_queue_iterator(
        const concurrent_queue_iterator< Container,
                                         typename Container::value_type >&
            other )
        : concurrent_queue_iterator_base_v3<
              typename tbb_remove_cv< Value >::type >( other ) {}
    ...
    concurrent_queue_iterator& operator=(
        const concurrent_queue_iterator< Container,
            ...
            other ) {
        concurrent_queue_iterator_base_v3<
            typename tbb_remove_cv< Value >::type >::operator=( other );
        return *this;
    }

    //! Reference to current item
    Value& operator*() const { return *static_cast< Value* >( this->my_item ); }
...
private:
    ...
    virtual void assign_and_destroy_item( void* dst,
                                          page& src,
                                          size_t index ) = 0;
...
protected:
    ...
    void __TBB_EXPORTED_METHOD internal_set_capacity( ptrdiff_t capacity,
                                                      size_t element_size );
    ...
    //! Attempts to enqueue at tail of queue using specified operation (copy or
    //! move)
    ...
    void internal_assign( const concurrent_queue_base_v3& src,
                          copy_specifics op_type );

private:
    virtual void copy_page_item( page& dst,
                                 size_t dindex,
                                 const page& src,
                                 size_t sindex ) = 0;
};
...
protected:
    concurrent_queue_base_v8( size_t item_sz )
        : concurrent_queue_base_v3( item_sz ) {}
    ...
    internal_push_move_if_not_full( const void* src );
...
private:
    friend struct micro_queue;
    virtual void move_page_item( page& dst,
                                 size_t dindex,
                                 const page& src,
                                 size_t sindex ) = 0;
...
class concurrent_queue_iterator_base_v3 {
    ...
    void initialize( const concurrent_queue_base_v3& queue,
...
protected:
    ...
    //! Obsolete entry point for constructing iterator pointing to head of
    //! queue.
    ...
    assign( const concurrent_queue_iterator_base_v3& i );
...
public:
    ...
    explicit concurrent_queue_iterator( const concurrent_queue_base_v3& queue )
        : concurrent_queue_iterator_base_v3(
              queue,
              __TBB_offsetof( concurrent_queue_base_v3::padded_page< Value >,
                              last ) ) {}
...
public:
    ...
    //! Reference to current item
    Value& operator*() const { return *static_cast< Value* >( my_item ); }
