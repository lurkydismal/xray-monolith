...
public:
    ...
    async_storage( C&& data )
        : my_graph( nullptr ), my_data( std::forward< C >( data ) ) {
        using namespace tbb::internal;
        __TBB_STATIC_ASSERT(
            ( is_same_type< typename strip< C >::type,
    ...
    bool set( C&& data ) {
        ...
        for ( typename subscriber_list_type::iterator it = my_clients.begin();
              it != my_clients.end(); ++it ) {
            ( *it )->try_put( my_data );
        }
    ...
    task* subscribe( async_storage_client& client, graph& g ) {
        if ( !my_data_ready.load< tbb::acquire >() ) {
            ...
            if ( !my_data_ready.load< tbb::relaxed >() ) {
#if TBB_USE_ASSERT
                for ( typename subscriber_list_type::iterator it =
                          my_clients.begin();
                      it != my_clients.end(); ++it ) {
                    __TBB_ASSERT( *it != &client,
        ...
        __TBB_ASSERT( my_data_ready.load< tbb::relaxed >(),
                      "data is NOT ready" );
...
public:
    ...
    async_msg()
        : my_storage( std::make_shared< internal::async_storage< T > >() ) {}

    async_msg( const T& t )
        : my_storage( std::make_shared< internal::async_storage< T > >( t ) ) {}
    ...
    void set( const T& t ) { my_storage->set( t ); }

    void set( T&& t ) { my_storage->set( std::move( t ) ); }
