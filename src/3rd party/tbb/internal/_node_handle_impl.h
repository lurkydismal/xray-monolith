...
// TODO: Consider creation some internal class to access node_handle private
// fields without any friendly classes
...
public:
    ...
    node_handle_base& operator=( node_handle_base&& nh ) {
        internal_destroy();
        my_node = nh.my_node;
        typedef typename traits_type::propagate_on_container_move_assignment
            pocma_type;
        tbb::internal::allocator_move_assignment( my_allocator, nh.my_allocator,
                                                  pocma_type() );
        nh.deactivate();
        return *this;
    }
    ...
    void swap( node_handle_base& nh ) {
        ...
        tbb::internal::allocator_swap( my_allocator, nh.my_allocator,
                                       pocs_type() );
    }
...
protected:
    ...
    void internal_destroy() {
        if ( my_node ) {
            ...
            typename tbb::internal::allocator_rebind<
