...
public:
    ...
    // Some versions of Intel(R) C++ Compiler fail to generate an implicit
    // constructor for the class which has std::tuple as a member.
...
public:
    ...
    typedef indexer_node_FE< InputTuple, output_type, StructTypes >
        input_ports_type;
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
...
private:
    ...
    typedef indexer_node_base< InputTuple, output_type, StructTypes >
        class_type;
    ...
    void handle_operations( indexer_node_base_operation* op_list ) {
        ...
        while ( op_list ) {
            ...
            switch ( current->type ) {
                ...
                case try__put_task: {
                    current->bypass_t =
                        my_successors.try_put_task( *( current->my_arg ) );
                    __TBB_store_with_release(
                        current->status, SUCCEEDED ); // return of try_put_task
                                                      // actual return value
                } break;
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
                case add_blt_succ:
...
public:
    indexer_node_base( graph& g ) : graph_node( g ), input_ports_type() {
        indexer_helper< StructTypes, N >::set_indexer_node_pointer(
            this->my_inputs, this, g );
    ...
    indexer_node_base( const indexer_node_base& other )
        : graph_node( other.my_graph ),
          ...
          sender< output_type >() {
        indexer_helper< StructTypes, N >::set_indexer_node_pointer(
            this->my_inputs, this, other.my_graph );
        my_successors.set_owner( this );
        my_aggregator.initialize_handler( handler_type( this ) );
    }
    ...
    task* try_put_task(
        output_type const* v ) { // not a virtual method in this class
...
struct input_types< 2, InputTuple > {
    ...
    typedef typename internal::tagged_msg< size_t, first_type, second_type >
        type;
};
...
struct input_types< 3, InputTuple > {
    ...
    typedef typename internal::
        tagged_msg< size_t, first_type, second_type, third_type >
            type;
};
...
struct input_types< 4, InputTuple > {
    ...
    typedef typename internal::
        tagged_msg< size_t, first_type, second_type, third_type, fourth_type >
            type;
};
...
struct input_types< 5, InputTuple > {
    ...
    typedef typename internal::tagged_msg< size_t,
                                           first_type,
                                           second_type,
                                           third_type,
                                           fourth_type,
...
struct indexer_types
    : public input_types< tuple_size< OutputTuple >::value, OutputTuple > {
    static const int N = tbb::flow::tuple_size< OutputTuple >::value;
    typedef typename input_types< N, OutputTuple >::type output_type;
    typedef
        typename wrap_tuple_elements< N, indexer_input_port, OutputTuple >::type
            input_ports_type;
    typedef internal::
        indexer_node_FE< input_ports_type, output_type, OutputTuple >
            indexer_FE_type;
    typedef internal::
        indexer_node_base< input_ports_type, output_type, OutputTuple >
            indexer_base_type;
};

template < class OutputTuple >
class unfolded_indexer_node
    : public indexer_types< OutputTuple >::indexer_base_type {
public:
    typedef typename indexer_types< OutputTuple >::input_ports_type
        input_ports_type;
