...
struct buffer_element_type {
    // the second parameter below is void * because we can't forward-declare the
    // type itself, so we just reinterpret_cast below.
    typedef typename aligned_pair< ValueType, void* >::type type;
};

template < typename Key, // type of key within ValueType
           typename ValueType,
           typename ValueToKey,  // abstract method that returns "const Key" or
                                 // "const Key&" given ValueType
           typename HashCompare, // has hash and equal
           typename Allocator = tbb::cache_aligned_allocator<
               typename aligned_pair< ValueType, void* >::type > >
class hash_buffer : public HashCompare {
public:
    static const size_t INITIAL_SIZE =
        8; // initial size of the hash pointer table
    ...
    typedef typename Allocator::template rebind< list_array_type >::other
        pointer_array_allocator_type;
    typedef typename Allocator::template rebind< element_type >::other
...
private:
    ...
    void set_up_free_list( element_type** p_free_list,
                           list_array_type la,
                           size_t sz ) {
    ...
    struct DoCleanup {
        ...
        ~DoCleanup() {
            if ( my_pa ) {
                size_t dont_care = 0;
                internal_free_buffer( *my_pa, *my_elements, my_size,
                                      dont_care );
            }
        }
    };

    // exception-safety requires we do all the potentially-throwing operations
    // first
    void grow_array() {
        ...
        {
            DoCleanup my_cleanup( new_pointer_array, new_elements_array,
                                  new_size );
            new_elements_array = elements_array_allocator().allocate( my_size );
            new_pointer_array =
                pointer_array_allocator_type().allocate( new_size );
            ...
            for ( size_t i = 0; i < my_size; ++i ) {
                for ( element_type* op = pointer_array[ i ]; op;
                    ...
                    value_type* ov =
                        reinterpret_cast< value_type* >( &( op->first ) );
                    // could have std::move semantics
                    internal_insert_with_key( new_pointer_array, new_size,
                                              new_free_list, *ov );
                }
            }
        ...
        internal_free_buffer( pointer_array, elements_array, my_size,
                              nelements );
    ...
    // we use this method to move elements in grow_array, so can't use class
    // fields
    void internal_insert_with_key( element_type** p_pointer_array,
                                   size_t p_sz,
    ...
    // internals
    static void internal_free_buffer( pointer_array_type& pa,
                                      list_array_type& el,
                                      size_t& sz,
                                      size_t& ne ) {
        if ( pa ) {
            for ( size_t i = 0; i < sz; ++i ) {
                ...
                for ( element_type* p = pa[ i ]; p; p = p_next ) {
                    ...
                    internal::punned_cast< value_type* >( &( p->first ) )
                        ->~value_type();
                }
            }
...
public:
    ...
    bool find_ref_with_key( const Knoref& k, pointer_type& v ) {
        ...
        for ( element_type* p = pointer_array[ i ]; p;
    ...
    void delete_with_key( const Knoref& k ) {
        ...
        for ( element_type* p = pointer_array[ h ]; p;
              prev = p, p = ( element_type* )( p->second ) ) {
