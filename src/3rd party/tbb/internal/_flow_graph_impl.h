...
enum reset_flags {
    rf_reset_protocol = 0,
    rf_reset_bodies = 1 << 0, // delete the current node body, reset to a copy
                              // of the initial node body.
...
tbb::task& prioritize_task( tbb::flow::interface10::graph& g,
                            tbb::task& arena_task );
void spawn_in_graph_arena( tbb::flow::interface10::graph& g,
                           tbb::task& arena_task );
void enqueue_in_graph_arena( tbb::flow::interface10::graph& g,
                             tbb::task& arena_task );
void add_task_to_graph_reset_list( tbb::flow::interface10::graph& g,
                                   tbb::task* tp );
...
public:
    ...
    task* execute() __TBB_override {
        ...
        __TBB_ASSERT_EX( result,
                         "Number of critical tasks for scheduler and tasks"
...
class graph : tbb::internal::no_copy, public tbb::flow::graph_proxy {
...
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
                  ,
                  tbb::flow::interface11::node_priority_t node_priority =
                      tbb::flow::interface11::no_priority )
    ...
    public:
        run_and_put_task( Receiver& r, Body& body )
            : my_receiver( r ), my_body( body ) {}
        tbb::task* execute() __TBB_override {
            tbb::task* res = my_receiver.try_put_task( my_body() );
            if ( res == tbb::flow::interface11::SUCCESSFULLY_ENQUEUED )
                res = NULL;
            return res;
        }
    ...
    public:
        ...
        void operator()() const { FLOW_SPAWN( spawn_task ); }
    };
...
public:
    ...
    void increment_wait_count() { reserve_wait(); }

    void decrement_wait_count() { release_wait(); }

    //! Used to register that an external entity may still interact with the
    //! graph.
    /** The graph will not return from wait_for_all until a matching number of
    decrement_wait_count calls is made. */
    void reserve_wait() __TBB_override;

    //! Deregisters an external entity that may have interacted with the graph.
    /** The graph will not return from wait_for_all until all the number of
    decrement_wait_count calls matches the number of increment_wait_count calls.
  */
    void release_wait() __TBB_override;

    //! Spawns a task that runs a body and puts its output to a specific
    //! receiver
    /** The task is spawned as a child of the graph. This is useful for running
    tasks that need to block a wait_for_all() on the graph.  For example a
    one-off source. */
    template < typename Receiver, typename Body >
    ...
    //! Wait until graph is idle and decrement_wait_count calls equals
    //! increment_wait_count calls.
    /** The waiting thread will go off and steal work while it is block in the
     * wait_for_all. */
    void wait_for_all() {
        cancelled = false;
...
#endif
#if __TBB_TASK_GROUP_CONTEXT
            // TODO: the "if" condition below is just a work-around to support
            // the concurrent wait mode. The cancellation and exception
            // mechanisms are still broken in this mode. Consider using task
            // group not to re-implement the same functionality.
            if ( !( my_context->traits() &
    ...
    tbb::task* root_task() { return my_root_task; }
    ...
    typedef tbb::flow::interface11::
        graph_iterator< graph, tbb::flow::interface11::graph_node >
            iterator;
    typedef tbb::flow::interface11::
        graph_iterator< const graph, const tbb::flow::interface11::graph_node >
            const_iterator;
    ...
    void reset( tbb::flow::interface11::reset_flags f =
...
private:
...
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
    tbb::flow::interface11::internal::graph_task_priority_queue_t
        my_priority_queue;
#endif
    ...
    friend tbb::task& tbb::flow::interface11::internal::prioritize_task(
        graph& g,
        tbb::task& arena_task );
    friend void tbb::flow::interface11::internal::spawn_in_graph_arena(
        graph& g,
        tbb::task& arena_task );
    friend void tbb::flow::interface11::internal::enqueue_in_graph_arena(
        graph& g,
        tbb::task& arena_task );
    friend void tbb::flow::interface11::internal::add_task_to_graph_reset_list(
        graph& g,
        tbb::task* tp );
...
inline tbb::task& prioritize_task( graph& g, tbb::task& t ) {
    ...
    // TODO: change flow graph's interfaces to work with graph_task type instead
    // of tbb::task.
    graph_task* gt = static_cast< graph_task* >( &t );
    if ( gt->priority != no_priority ) {
        //! Non-preemptive priority pattern. The original task is submitted as a
        //! work item to the priority queue, and a new critical task is created
        //! to take and execute a work item with the highest known priority. The
        //! reference counting responsibility is transferred (via
        //! allocate_continuation) to the new task.
        critical_task = new ( gt->allocate_continuation() )
            priority_task_selector( g.my_priority_queue );
