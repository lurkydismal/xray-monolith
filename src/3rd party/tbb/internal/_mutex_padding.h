...
// wrapper for padding mutexes to be alone on a cache line, without requiring
// they be allocated from a pool.  Because we allow them to be defined anywhere
// they must be two cache lines in size.
...
// Pad a mutex to occupy a number of full cache lines sufficient to avoid false
// sharing with other data; space overhead is up to 2*cache_line_size-1.
...
class padded_mutex< Mutex, false >
    ...
    pad_type
        my_pad[ ( ( sizeof( Mutex ) + cache_line_size - 1 ) / cache_line_size +
                  1 ) *
                cache_line_size / sizeof( pad_type ) ];

    Mutex* impl() {
        return ( Mutex* )( ( uintptr_t( this ) | ( cache_line_size - 1 ) ) +
                           1 );
    }
