...
#if !defined( __TBB_concurrent_unordered_map_H ) && \
    !defined( __TBB_concurrent_unordered_set_H ) && \
    !defined( __TBB_concurrent_hash_map_H )
...
protected:
    ...
    friend bool operator==( const flist_iterator< M, T >& i,
...
public:
    ...
    reference operator*() const { return this->base_type::operator*(); }
...
public:
    ...
    typedef typename tbb::internal::allocator_rebind< Allocator, T >::type
        allocator_type;
    ...
    typedef
        typename tbb::internal::allocator_traits< allocator_type >::value_type
            value_type;
    typedef
        typename tbb::internal::allocator_traits< allocator_type >::size_type
            size_type;
    typedef typename tbb::internal::allocator_traits<
        allocator_type >::difference_type difference_type;
    typedef typename tbb::internal::allocator_traits< allocator_type >::pointer
        pointer;
    typedef typename tbb::internal::allocator_traits<
    ...
    private:
        // for compilers that try to generate default constructors though they
        // are not needed.
    ...
    public:
        ...
        // get() and value() is a common interface for getting access to node`s
        // element (required by node_handle)
        value_type* storage() {
        ...
        nodeptr_t atomic_set_next( nodeptr_t new_node,
                                   nodeptr_t current_node ) {
            // Try to change the next pointer on the current element to a new
            // element, only if it still points to the cached next
            nodeptr_t exchange_node =
                tbb::internal::as_atomic( my_next ).compare_and_swap(
        ...
        // Checks if this element in the list is a dummy, order enforcing node.
        // Dummy nodes are used by buckets in the hash table to quickly index
        // into the right subsection of the split-ordered list.
        bool is_dummy() const { return ( my_order_key & 0x1 ) == 0; }
    ...
    nodeptr_t create_node( sokey_t order_key,
                           ...
                           /*AllowCreate=*/tbb::internal::true_type =
                               tbb::internal::true_type() ) {
        nodeptr_t pnode = my_node_allocator.allocate( 1 );

        // TODO: use RAII scoped guard instead of explicit catch
        __TBB_TRY {
            new ( static_cast< void* >( &pnode->my_element ) )
                T( tbb::internal::forward< Arg >( t ) );
            pnode->init( order_key );
        }
        __TBB_CATCH( ... ) {
    ...
    nodeptr_t create_node_v( __TBB_FORWARDING_REF( Args )
                                 __TBB_PARAMETER_PACK args ) {
        nodeptr_t pnode = my_node_allocator.allocate( 1 );

        // TODO: use RAII scoped guard instead of explicit catch
        __TBB_TRY {
            new ( static_cast< void* >( &pnode->my_element ) )
                T( __TBB_PACK_EXPANSION(
                    tbb::internal::forward< Args >( args ) ) );
        }
        __TBB_CATCH( ... ) {
            my_node_allocator.deallocate( pnode, 1 );
            __TBB_RETHROW();
    ...
    ~split_ordered_list() {
        ...
        __TBB_ASSERT( pnode != NULL && pnode->my_next == NULL,
    ...
    allocator_type get_allocator() const { return ( my_node_allocator ); }
    ...
    iterator begin() { return first_real_iterator( raw_begin() ); }

    // Returns a first non-dummy element in the SOL
    const_iterator begin() const { return first_real_iterator( raw_begin() ); }
    ...
    bool empty() const { return ( my_element_count == 0 ); }

    // Returns the number of non-dummy elements in the list
    size_type size() const { return my_element_count; }

    // Returns the maximum size of the list, determined by the allocator
    size_type max_size() const { return my_node_allocator.max_size(); }
    ...
    raw_iterator raw_begin() { return raw_iterator( my_head ); }
    ...
    raw_iterator raw_end() { return raw_iterator( 0 ); }

    raw_const_iterator raw_end() const { return raw_const_iterator( 0 ); }
    ...
    // Returns a public iterator version of the internal iterator. Public
    // iterator must not be a dummy private iterator.
    iterator get_iterator( raw_iterator it ) {
        __TBB_ASSERT(
            it.get_node_ptr() == NULL || !it.get_node_ptr()->is_dummy(),
    ...
    // Returns a public iterator version of a first non-dummy internal iterator
    // at or after the passed in internal iterator.
    ...
    static nodeptr_t try_insert_atomic( nodeptr_t previous,
                                        nodeptr_t new_node,
                                        nodeptr_t current_node ) {
    ...
    std::pair< iterator, bool > try_insert( raw_iterator it,
                                            raw_iterator next,
                                            nodeptr_t pnode,
                                            size_type* new_count ) {
        nodeptr_t inserted_node =
            try_insert_atomic( it.get_node_ptr(), pnode, next.get_node_ptr() );

        if ( inserted_node == pnode ) {
            // If the insert succeeded, check that the order is correct and
            // increment the element count
            check_range( it, next );
            *new_count = tbb::internal::as_atomic( my_element_count )
                             .fetch_and_increment();
    ...
    raw_iterator insert_dummy( raw_iterator it, sokey_t order_key ) {
        ...
        // Create a dummy element up front, even though it may be discarded (due
        // to concurrent insertion)
        nodeptr_t dummy_node = create_node( order_key );

        for ( ;; ) {
            __TBB_ASSERT( it != last, "Invalid head list node" );

            // If the head iterator is at the end of the list, or past the point
            // where this dummy node needs to be inserted, then try to insert
            // it.
            if ( where == last || get_order_key( where ) > order_key ) {
                __TBB_ASSERT( get_order_key( it ) < order_key,
                              "Invalid node order in the list" );

                // Try to insert it in the right place
                nodeptr_t inserted_node = try_insert_atomic(
                    it.get_node_ptr(), dummy_node, where.get_node_ptr() );
                ...
                if ( inserted_node == dummy_node ) {
                    ...
                    // another thread, or a real element was inserted at exactly
                    // the same place as dummy node. Proceed with the search
                    // from the previous location where order key was known to
                    // be larger (note: this is legal only because there is no
                    // safe concurrent erase operation supported).
                    where = it;
                    ++where;
                    continue;
                }
            } else if ( get_order_key( where ) == order_key ) {
                // Another dummy node with the same value found, discard the new
                // one.
    ...
    void erase_node( raw_iterator previous,
                     raw_const_iterator& where,
    ...
    void erase_node( raw_iterator previous, raw_const_iterator& where ) {
        erase_node( previous, where,
                    /*allow_destroy*/ tbb::internal::true_type() );
    }

    // Erase the element (previous node needs to be passed because this is a
    // forward only list)
    template < typename AllowDestroy >
    iterator erase_node( raw_iterator previous,
    ...
    iterator erase_node( raw_iterator previous, const_iterator& where ) {
        return erase_node( previous, where,
                           /*allow_destroy*/ tbb::internal::true_type() );
    }
    ...
    void move_all( self_type& source ) {
        ...
        for ( raw_const_iterator it = first; it != last; ) {
            ...
            nodeptr_t dummy_node =
                pnode->is_dummy()
                    ? create_node( pnode->get_order_key() )
                    : create_node( pnode->get_order_key(), pnode->my_element );
            previous_node =
                try_insert_atomic( previous_node, dummy_node, NULL );
...
private:
    // Need to setup private fields of split_ordered_list in move constructor
    // and assignment of concurrent_unordered_base
    template < typename Traits >
    friend class concurrent_unordered_base;
    ...
    void check_range( raw_iterator first, raw_iterator last ) {
        ...
        for ( raw_iterator it = first; it != last; ++it ) {
            ...
            __TBB_ASSERT( next == raw_end() ||
                              get_order_key( next ) >= get_order_key( it ),
    ...
    typename tbb::internal::allocator_rebind< allocator_type, node >::type
        my_node_allocator;      // allocator object for nodes
...
protected:
    ...
    typedef split_ordered_list< value_type, typename Traits::allocator_type >
        solist_t;
    ...
    typedef typename solist_t::iterator
        iterator; // TODO: restore const iterator for unordered_sets
...
private:
    ...
    static size_type const pointers_per_table =
        sizeof( size_type ) * 8; // One bucket segment per bit
    static const size_type initial_bucket_load =
        4; // Initial maximum number of elements per bucket
...
protected:
    ...
    concurrent_unordered_base( size_type n_of_buckets = initial_bucket_number,
        ...
        if ( n_of_buckets == 0 )
            ++n_of_buckets;
        my_number_of_buckets = size_type( 1 )
                               << __TBB_Log2( ( uintptr_t )n_of_buckets * 2 -
                                              1 ); // round up to power of 2
        internal_init();
    }
    ...
    concurrent_unordered_base( const concurrent_unordered_base& right )
        : Traits( right.my_hash_compare ),
          my_solist( right.get_allocator() ),
            ...
            if ( !right.my_solist.empty() ) {
                ...
                for ( raw_const_iterator it = ++( right.my_solist.raw_begin() ),
                    ...
                    if ( pnode->is_dummy() ) {
                        ...
                        size_type bucket =
                            __TBB_ReverseBits( pnode->get_order_key() ) %
                            my_number_of_buckets;
                        set_bucket( bucket, node );
                    } else {
                        node = my_solist.create_node(
                            pnode->get_order_key(),
                            std::move( pnode->my_element ) );
                    }

                    previous_node = my_solist.try_insert_atomic( previous_node,
                                                                 node, NULL );
                    __TBB_ASSERT( previous_node != NULL,
                                  "Insertion of node failed. Concurrent "
                                  "inserts in constructor ?" );
                }
    ...
    concurrent_unordered_base& operator=(
        ...
        if ( this != &other ) {
            ...
            if ( pocma_t::value || this->my_allocator == other.my_allocator ) {
                ...
                if ( pocma_t::value ) {
                    ...
                    // with single direction moving
                    swap( this->my_solist.my_node_allocator,
                          other.my_solist.my_node_allocator );
                    swap( this->my_allocator, other.my_allocator );
                }
            } else {
                concurrent_unordered_base moved_copy( std::move( other ),
                                                      this->my_allocator );
    ...
    void internal_merge( SourceType& source ) {
        ...
        __TBB_STATIC_ASSERT(
            ( tbb::internal::is_same_type<
                node_type, typename SourceType::node_type >::value ),
        ...
        for ( source_iterator it = source.begin(); it != source.end(); ) {
            ...
            if ( allow_multimapping || find( get_key( *where ) ) == end() ) {
                std::pair< node_type, raw_iterator > extract_result =
                    source.internal_extract( where );

                // Remember the old order key
                sokey_t old_order_key =
                    extract_result.first.my_node->get_order_key();

                // If the insertion fails, it returns ownership of the node to
                // extract_result.first extract_result.first remains valid node
                // handle
                if ( !insert( std::move( extract_result.first ) ).second ) {
                    ...
                    __TBB_ASSERT(
                        extract_result.first.my_node->get_order_key() >=
                            current.get_node_ptr()->get_order_key(),
                        "Wrong nodes order in source container" );
                    __TBB_ASSERT(
                        next == source.my_solist.raw_end() ||
                            extract_result.first.my_node->get_order_key() <=
                                next.get_node_ptr()->get_order_key(),
                        "Wrong nodes order in source container" );

                    size_t new_count = 0; // To use try_insert()
                    bool insert_result =
                        source.my_solist
                            .try_insert( current, next,
                                         extract_result.first.my_node,
                                         &new_count )
                            .second;
                    __TBB_ASSERT_EX(
                        insert_result,
                        "Return to source must be successful. "
...
public:
    allocator_type get_allocator() const { return my_solist.get_allocator(); }

    // Size and capacity function
    bool empty() const { return my_solist.empty(); }

    size_type size() const { return my_solist.size(); }

    size_type max_size() const { return my_solist.max_size(); }

    // Iterators
    iterator begin() { return my_solist.begin(); }

    const_iterator begin() const { return my_solist.begin(); }

    iterator end() { return my_solist.end(); }

    const_iterator end() const { return my_solist.end(); }

    const_iterator cbegin() const { return my_solist.cbegin(); }

    const_iterator cend() const { return my_solist.cend(); }
    ...
    public:
        ...
        typedef
            typename concurrent_unordered_base::difference_type difference_type;
        ...
        const_range_type( const concurrent_unordered_base& a_table )
            : my_table( a_table ),
              my_begin_node( a_table.my_solist.begin() ),
        ...
        //! Set my_midpoint_node to point approximately half way between
        //! my_begin_node and my_end_node.
        void set_midpoint() const {
            if ( my_begin_node == my_end_node ) // not divisible
                my_midpoint_node = my_end_node;
            else {
                sokey_t begin_key =
                    solist_t::get_safe_order_key( my_begin_node );
                sokey_t end_key = solist_t::get_safe_order_key( my_end_node );
                size_t mid_bucket =
                    __TBB_ReverseBits( begin_key +
                                       ( end_key - begin_key ) / 2 ) %
                    my_table.my_number_of_buckets;
                while ( !my_table.is_initialized( mid_bucket ) )
                    mid_bucket = my_table.get_parent( mid_bucket );
                if ( __TBB_ReverseBits( mid_bucket ) > begin_key ) {
                    // found a dummy_node between begin and end
                    my_midpoint_node = my_table.my_solist.first_real_iterator(
                        my_table.get_bucket( mid_bucket ) );
                } else {
                ...
                {
                    sokey_t mid_key =
                        solist_t::get_safe_order_key( my_midpoint_node );
                    __TBB_ASSERT( begin_key < mid_key,
                                  "my_begin_node is after my_midpoint_node" );
                    __TBB_ASSERT( mid_key <= end_key,
    ...
    std::pair< iterator, bool > insert( const value_type& value ) {
        return internal_insert< /*AllowCreate=*/tbb::internal::true_type,
                                /*AllowDestroy=*/tbb::internal::true_type >(
            value );
    }
    ...
    std::pair< iterator, bool > emplace( Args&&... args ) {
        nodeptr_t pnode = my_solist.create_node_v(
            tbb::internal::forward< Args >( args )... );

        return internal_insert< /*AllowCreate=*/tbb::internal::false_type,
                                /*AllowDestroy=*/tbb::internal::true_type >(
            pnode->my_element, pnode );
    }
    ...
    node_type unsafe_extract( const key_type& key ) {
        pairii_t where = equal_range( key );
        if ( where.first == end() )
            return node_type(); // element was not found
    ...
    // Observers
    hasher hash_function() const { return my_hash_compare.my_hash_object; }

    key_equal key_eq() const { return my_hash_compare.my_key_compare_object; }
    ...
    // Lookup
    iterator find( const key_type& key ) { return internal_find( key ); }
    ...
    size_type count( const key_type& key ) const {
        if ( allow_multimapping ) {
            ...
            size_type item_count =
                internal_distance( answer.first, answer.second );
            return item_count;
        } else {
            return const_cast< self_type* >( this )->internal_find( key ) ==
                           end()
    ...
    size_type unsafe_bucket_count() const { return my_number_of_buckets; }
    ...
    size_type unsafe_bucket_size( size_type bucket ) {
        ...
        if ( is_initialized( bucket ) ) {
            ...
            ++it;
            for ( ; it != my_solist.raw_end() && !it.get_node_ptr()->is_dummy();
                  ++it )
                ++item_count;
        }
        return item_count;
    ...
    float max_load_factor() const { return my_maximum_bucket_size; }
    ...
    void rehash( size_type buckets ) {
        ...
        if ( current_buckets >= buckets )
            return;
        my_number_of_buckets = size_type( 1 )
                               << __TBB_Log2( ( uintptr_t )buckets * 2 -
                                              1 ); // round up to power of 2
    }
...
private:
    ...
    //  Hash APIs
    static size_type internal_distance( const_iterator first,
                                        const_iterator last ) {
        size_type num = 0;
    ...
    std::pair< iterator, bool > internal_insert(
        __TBB_FORWARDING_REF( ValueType ) value,
        ...
        for ( raw_iterator where = previous;; ) {
            ++where;
            if ( where == last ||
                 solist_t::get_order_key( where ) > order_key ||
                 // if multimapped, stop at the first item equal to us.
                 ( allow_multimapping &&
                   solist_t::get_order_key( where ) == order_key &&
                   !my_hash_compare( get_key( *where ),
                                     *pkey ) ) ) // TODO: fix negation
            {
                if ( !pnode ) {
                    pnode = my_solist.create_node(
                        order_key, tbb::internal::forward< ValueType >( value ),
                        AllowCreate() );
                    // If the value was moved, the known reference to key might
                    // be invalid
                ...
                std::pair< iterator, bool > result =
                    my_solist.try_insert( previous, where, pnode, &new_count );
                ...
                if ( result.second ) {
                    ...
                    // Insertion failed: either the same node was inserted by
                    // another thread, or another element was inserted at
                    // exactly the same place as this node. Proceed with the
                    // search from the previous location where order key was
                    // known to be larger (note: this is legal only because
                    // there is no safe concurrent erase operation supported).
                    where = previous;
                    continue;
                }
            } else if ( !allow_multimapping &&
                        solist_t::get_order_key( where ) == order_key &&
                        !my_hash_compare( get_key( *where ),
                                          *pkey ) ) // TODO: fix negation
            { // Element already in the list, return it
                ...
                return std::pair< iterator, bool >(
                    my_solist.get_iterator( where ), false );
            }
            // Move the iterator forward
            previous = where;
        }
    ...
    iterator internal_find( const key_type& key ) {
        ...
        for ( raw_iterator it = prepare_bucket( hash_key ); it != last; ++it ) {
            if ( solist_t::get_order_key( it ) > order_key ) {
                // If the order key is smaller than the current order key, the
                // element is not in the hash.
                return end();
            } else if ( solist_t::get_order_key( it ) == order_key ) {
                // The fact that order keys match does not mean that the element
                // is found. Key function comparison has to be performed to
                // check whether this is the right element. If not, keep
                // searching while order key is the same.
                if ( !my_hash_compare( get_key( *it ),
                                       key ) ) // TODO: fix negation
    ...
    std::pair< node_type, raw_iterator > internal_extract( const_iterator it ) {
        ...
        for ( raw_iterator where = previous; where != last; previous = where ) {
            ...
            if ( my_solist.get_iterator( where ) == it ) {
                ...
                my_solist.erase_node(
                    previous, it,
                    /*allow_destroy*/ tbb::internal::false_type() );
                return std::pair< node_type, raw_iterator >(
                    node_type( result.get_node_ptr() ), previous );
            }
        }
    ...
    pairii_t internal_equal_range( const key_type& key ) {
        ...
        for ( raw_iterator it = prepare_bucket( hash_key ); it != end_it;
              ++it ) {
            if ( solist_t::get_order_key( it ) > order_key ) {
                ...
                return pairii_t( end(), end() );
                        ...
                        !my_hash_compare(
                            get_key( *it ),
                            key ) ) // TODO: fix negation; also below
            {
                iterator first = my_solist.get_iterator( it );
                iterator last = first;
                do
                    ++last;
                while ( allow_multimapping && last != end() &&
                        !my_hash_compare( get_key( *last ), key ) );
    ...
    void init_bucket( size_type bucket ) {
        ...
        __TBB_ASSERT( bucket != 0,
                      "The first bucket must always be initialized" );

        size_type parent_bucket = get_parent( bucket );

        // All parent_bucket buckets have to be initialized before this bucket
        // is
        ...
        raw_iterator dummy_node =
            my_solist.insert_dummy( parent, split_order_key_dummy( bucket ) );
    ...
    void adjust_table_size( size_type total_elements, size_type current_size ) {
        ...
        if ( ( ( float )total_elements / ( float )current_size ) >
             my_maximum_bucket_size ) {
            // Double the size of the hash only if size has not changed in
            // between loads
            my_number_of_buckets.compare_and_swap( 2u * current_size,
                                                   current_size );
            // Simple "my_number_of_buckets.compare_and_swap( current_size<<1,
            // current_size );" does not work for VC8 due to overzealous
            // compiler warnings in /Wp64 mode
        }
    }
    ...
    raw_iterator get_bucket( size_type bucket ) const {
        ...
        __TBB_ASSERT( my_buckets[ segment ],
    ...
    raw_iterator prepare_bucket( sokey_t hash_key ) {
        ...
        if ( my_buckets[ segment ] == NULL ||
             my_buckets[ segment ][ index ].get_node_ptr() == NULL )
            init_bucket( bucket );
    ...
    void set_bucket( size_type bucket, raw_iterator dummy_head ) {
        ...
        if ( my_buckets[ segment ] == NULL ) {
            ...
            std::memset( static_cast< void* >( new_segment ), 0,
                         sz * sizeof( raw_iterator ) );

            if ( my_buckets[ segment ].compare_and_swap( new_segment, NULL ) !=
                 NULL )
    ...
    // A regular order key has its original hash value reversed and the last bit
    // set
    sokey_t split_order_key_regular( sokey_t order_key ) const {
        return __TBB_ReverseBits( order_key ) | 0x1;
    }

    // A dummy order key has its original hash value reversed and the last bit
    // unset
    ...
    typename tbb::internal::allocator_rebind< allocator_type,
                                              raw_iterator >::type
        my_allocator;             // Allocator object for segments
    float my_maximum_bucket_size; // Maximum size of the bucket
    atomic< raw_iterator* >
        my_buckets[ pointers_per_table ]; // The segment table
};
#if defined( _MSC_VER ) && !defined( __INTEL_COMPILER )
