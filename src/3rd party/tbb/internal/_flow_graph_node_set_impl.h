...
// Visual Studio 2019 reports an error while calling predecessor_selector::get
// and successor_selector::get Seems like the well-formed expression in trailing
// decltype is treated as ill-formed
...
public:
    // TODO: consider making graph_reference() public and consistent interface
    // to get a reference to the graph and remove get_graph_helper
    template < typename T >
...
struct node_set {
    ...
    node_set( const node_set< order::undefined, Nodes2... >& set )
        : nodes( set.nodes ) {}
...
template < typename T >
using has_output_type =
    tbb::internal::supports< T, alias_helpers::output_type >;

template < typename T >
using has_input_type = tbb::internal::supports< T, alias_helpers::input_type >;

template < typename T >
using has_input_ports_type =
    tbb::internal::supports< T, alias_helpers::input_ports_type >;

template < typename T >
using has_output_ports_type =
    tbb::internal::supports< T, alias_helpers::output_ports_type >;
...
struct is_async_node< tbb::flow::interface11::async_node< Args... > >
    : std::true_type {};

template < typename FirstPredecessor, typename... Predecessors >
node_set< order::following, FirstPredecessor, Predecessors... > follows(
    FirstPredecessor& first_predecessor,
    Predecessors&... predecessors ) {
    __TBB_STATIC_ASSERT(
        ( tbb::internal::conjunction<
            has_output_type< FirstPredecessor >,
            has_output_type< Predecessors >... >::value ),
        "Not all node's predecessors has output_type typedef" );
    __TBB_STATIC_ASSERT(
        ( tbb::internal::conjunction< is_sender< FirstPredecessor >,
                                      is_sender< Predecessors >... >::value ),
        "Not all node's predecessors are senders" );
    return node_set< order::following, FirstPredecessor, Predecessors... >(
        first_predecessor, predecessors... );
}

template < typename... Predecessors >
node_set< order::following, Predecessors... > follows(
    node_set< order::undefined, Predecessors... >& predecessors_set ) {
    __TBB_STATIC_ASSERT( ( tbb::internal::conjunction<
                             has_output_type< Predecessors >... >::value ),
                         "Not all nodes in the set has output_type typedef" );
    __TBB_STATIC_ASSERT(
        ( tbb::internal::conjunction< is_sender< Predecessors >... >::value ),
...
node_set< order::preceding, FirstSuccessor, Successors... > precedes(
    FirstSuccessor& first_successor,
    Successors&... successors ) {
    __TBB_STATIC_ASSERT( ( tbb::internal::conjunction<
                             has_input_type< FirstSuccessor >,
                             has_input_type< Successors >... >::value ),
                         "Not all node's successors has input_type typedef" );
    __TBB_STATIC_ASSERT(
        ( tbb::internal::conjunction< is_receiver< FirstSuccessor >,
                                      is_receiver< Successors >... >::value ),
        "Not all node's successors are receivers" );
    return node_set< order::preceding, FirstSuccessor, Successors... >(
        first_successor, successors... );
}

template < typename... Successors >
node_set< order::preceding, Successors... > precedes(
    node_set< order::undefined, Successors... >& successors_set ) {
    __TBB_STATIC_ASSERT( ( tbb::internal::conjunction<
                             has_input_type< Successors >... >::value ),
                         "Not all nodes in the set has input_type typedef" );
    __TBB_STATIC_ASSERT(
        ( tbb::internal::conjunction< is_receiver< Successors >... >::value ),
        "Not all nodes in the set are receivers" );
    return node_set< order::preceding, Successors... >( successors_set );
}

template < typename Node, typename... Nodes >
node_set< order::undefined, Node, Nodes... > make_node_set( Node& first_node,
                                                            Nodes&... nodes ) {
...
class successor_selector {
    ...
    static auto get_impl( NodeType& node, std::true_type )
        -> decltype( input_port< I >( node ) ) {
...
class predecessor_selector {
    template < typename NodeType >
    static auto internal_get( NodeType& node, std::true_type )
        -> decltype( output_port< I >( node ) ) {
...
#else
    static auto get( NodeType& node )
        -> decltype( get_impl( node, is_async_node< NodeType >() ) )
#endif
    {
...
public:
    ...
    static void connect_predecessors( PredecessorsTuple& predecessors,
                                      NodeType& node ) {
        make_edge( std::get< I >( predecessors ),
                   successor_selector< I >::get( node ) );
        make_edges_helper< I - 1 >::connect_predecessors( predecessors, node );
    }

    template < typename SuccessorsTuple, typename NodeType >
    static void connect_successors( NodeType& node,
                                    SuccessorsTuple& successors ) {
        make_edge( predecessor_selector< I >::get( node ),
                   std::get< I >( successors ) );
...
struct make_edges_helper< 0 > {
    ...
    static void connect_predecessors( PredecessorsTuple& predecessors,
                                      NodeType& node ) {
        make_edge( std::get< 0 >( predecessors ),
                   successor_selector< 0 >::get( node ) );
    }

    template < typename SuccessorsTuple, typename NodeType >
    static void connect_successors( NodeType& node,
                                    SuccessorsTuple& successors ) {
        make_edge( predecessor_selector< 0 >::get( node ),
                   std::get< 0 >( successors ) );
    }
};
...
void make_edges( const node_set< OrderFlagType, Args... >& s, NodeType& node ) {
    const std::size_t SetSize =
        tbb::flow::tuple_size< decltype( s.nodes ) >::value;
...
void make_edges_in_order( const node_set< order::following, Nodes... >& ns,
                          NodeType& node ) {
    make_edges( ns, node );
}

template < typename NodeType, typename... Nodes >
void make_edges_in_order( const node_set< order::preceding, Nodes... >& ns,
                          NodeType& node ) {
    make_edges( node, ns );
}
