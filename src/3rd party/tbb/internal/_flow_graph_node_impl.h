...
public:
    bool empty() const { return this->buffer_empty(); }

    const T& front() const { return this->item_buffer< T, A >::front(); }

    bool pop( T& t ) { return this->pop_front( t ); }

    void pop() { this->destroy_front(); }

    bool push( T& t ) { return this->push_back( t ); }
};
...
public:
    ...
    typedef typename A::template rebind< input_queue_type >::other
        queue_allocator_type;
    __TBB_STATIC_ASSERT(
        !( ( internal::has_policy< queueing, Policy >::value ) &&
           ( internal::has_policy< rejecting, Policy >::value ) ),
    ...
    function_input_base(
        graph& g,
        __TBB_FLOW_GRAPH_PRIORITY_ARG1( size_t max_concurrency,
                                        node_priority_t priority ) )
        : my_graph_ref( g ),
          my_max_concurrency( max_concurrency ),
          __TBB_FLOW_GRAPH_PRIORITY_ARG1( my_concurrency( 0 ),
                                          my_priority( priority ) ),
          my_queue( !internal::has_policy< rejecting, Policy >::value
                        ? new input_queue_type()
                        : NULL ),
    ...
    function_input_base( const function_input_base& src )
        : receiver< Input >(),
          ...
          my_queue( src.my_queue ? new input_queue_type() : NULL ),
          forwarder_busy( false ) {
        my_predecessors.set_owner( this );
        my_aggregator.initialize_handler( handler_type( this ) );
    }
    ...
    // TODO: pass the graph_buffer_policy to the base so it can allocate the
    // queue instead. This would be an interface-breaking change.
    ...
    task* try_put_task( const input_type& t ) __TBB_override {
        return try_put_task_impl(
            t, internal::has_policy< lightweight, Policy >() );
    }
...
protected:
    ...
    graph& graph_reference() const __TBB_override { return my_graph_ref; }

    task* try_get_postponed_task( const input_type& i ) {
        operation_type op_data(
            i, app_body_bypass ); // tries to pop an item or get_item
        my_aggregator.execute( &op_data );
...
private:
    ...
    bool forwarder_busy;
    typedef internal::aggregating_functor< class_type, operation_type >
        handler_type;
    ...
    //! Creates tasks for postponed messages if available and if concurrency
    //! allows
    ...
    task* try_put_task_impl( const input_type& t,
                             /*lightweight=*/tbb::internal::true_type ) {
    ...
    task* try_put_task_impl( const input_type& t,
                             /*lightweight=*/tbb::internal::false_type ) {
        if ( my_max_concurrency == 0 ) {
                   ...
                   ? new ( task::allocate_additional_child_of(
                         *( my_graph_ref.root_task() ) ) )
                         apply_body_task_bypass< class_type, input_type >(
                             *this, __TBB_FLOW_GRAPH_PRIORITY_ARG1(
                                        input, my_priority ) )
                   : NULL;
    }
                   ...
                   ? new ( task::allocate_additional_child_of(
                         *( my_graph_ref.root_task() ) ) )
                         forward_task_bypass< class_type >(
                             __TBB_FLOW_GRAPH_PRIORITY_ARG1( *this,
                                                             my_priority ) )
                   : NULL;
    }
...
//! Implements methods for a function node that takes a type Input as input and
//! sends
//  a type Output to its successors.
template < typename Input, typename Output, typename Policy, typename A >
class function_input
    : public function_input_base< Input,
                                  Policy,
                                  A,
                                  function_input< Input, Output, Policy, A > > {
public:
    typedef Input input_type;
    ...
    function_input( graph& g,
                    size_t max_concurrency,
                    __TBB_FLOW_GRAPH_PRIORITY_ARG1( Body& body,
                                                    node_priority_t priority ) )
        : base_type(
              g,
              __TBB_FLOW_GRAPH_PRIORITY_ARG1( max_concurrency, priority ) ),
          my_body(
              new internal::function_body_leaf< input_type, output_type, Body >(
                  body ) ),
          my_init_body(
              new internal::function_body_leaf< input_type, output_type, Body >(
                  body ) ) {}
    ...
    Body copy_function_object() {
        ...
        return dynamic_cast< internal::function_body_leaf<
            input_type, output_type, Body >& >( body_ref )
            .get_body();
    }
    ...
    task* apply_body_impl_bypass( const input_type& i ) {
        ...
        if ( base_type::my_max_concurrency != 0 ) {
            ...
            __TBB_ASSERT(
                !postponed_task || postponed_task != SUCCESSFULLY_ENQUEUED,
                NULL );
        }
#if TBB_DEPRECATED_MESSAGE_FLOW_ORDER
...
#else
        if ( postponed_task ) {
            // make the task available for other workers since we do not know
            // successors' execution policy
            internal::spawn_in_graph_arena( base_type::graph_reference(),
                                            *postponed_task );
        }
        task* successor_task = successors().try_put_task( v );
...
// helper templates to clear the successor edges of the output ports of an
// multifunction_node
template < int N >
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
// helper templates to extract the output ports of an multifunction_node from
// graph
template < int N >
struct extract_element {
    template < typename P >
    static void extract_this( P& p ) {
        ( void )tbb::flow::get< N - 1 >( p )
            .successors()
            .built_successors()
            .sender_extract( tbb::flow::get< N - 1 >( p ) );
...
struct extract_element< 1 > {
    template < typename P >
    static void extract_this( P& p ) {
        ( void )tbb::flow::get< 0 >( p )
            .successors()
            .built_successors()
            .sender_extract( tbb::flow::get< 0 >( p ) );
    }
};
#endif
...
public:
    ...
    typedef multifunction_body< input_type, output_ports_type >
    ...
    multifunction_input(
        ...
        : base_type(
              g,
              __TBB_FLOW_GRAPH_PRIORITY_ARG1( max_concurrency, priority ) ),
          my_body( new internal::multifunction_body_leaf< input_type,
                                                          output_ports_type,
                                                          Body >( body ) ),
          my_init_body(
              new internal::multifunction_body_leaf< input_type,
                                                     output_ports_type,
                                                     Body >( body ) ),
    ...
    Body copy_function_object() {
        ...
        return *static_cast< Body* >(
            dynamic_cast< internal::multifunction_body_leaf<
                input_type, output_ports_type, Body >& >( body_ref )
                .get_body_ptr() );
    }

    // for multifunction nodes we do not have a single successor as such.  So we
    // just tell the task we were successful.
    // TODO: consider moving common parts with implementation in function_input
...
protected:
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    void extract() { extract_element< N >::extract_this( my_output_ports ); }
#endif
...
typename tbb::flow::tuple_element< N, typename MOP::output_ports_type >::type&
output_port( MOP& op ) {
...
struct emit_element {
    ...
    static task* emit_this( graph& g, const T& t, P& p ) {
        // TODO: consider to collect all the tasks in task_list and spawn them
        // all at once
        task* last_task = tbb::flow::get< N - 1 >( p ).try_put_task(
            tbb::flow::get< N - 1 >( t ) );
...
struct emit_element< 1 > {
    ...
    static task* emit_this( graph& g, const T& t, P& p ) {
        task* last_task =
            tbb::flow::get< 0 >( p ).try_put_task( tbb::flow::get< 0 >( t ) );
        check_task_and_spawn( g, last_task );
...
public:
    ...
    template < typename Body >
    continue_input( graph& g,
                    __TBB_FLOW_GRAPH_PRIORITY_ARG1( Body& body,
                                                    node_priority_t priority ) )
        : continue_receiver(
              __TBB_FLOW_GRAPH_PRIORITY_ARG1( /*number_of_predecessors=*/0,
                                              priority ) ),
          my_graph_ref( g ),
          my_body(
              new internal::function_body_leaf< input_type, output_type, Body >(
    ...
    continue_input( graph& g,
                    ...
                    __TBB_FLOW_GRAPH_PRIORITY_ARG1( Body& body,
                                                    node_priority_t priority ) )
        : continue_receiver(
              __TBB_FLOW_GRAPH_PRIORITY_ARG1( number_of_predecessors,
                                              priority ) ),
          my_graph_ref( g ),
          my_body(
              new internal::function_body_leaf< input_type, output_type, Body >(
            ...
            return new ( task::allocate_additional_child_of(
                *( my_graph_ref.root_task() ) ) )
                apply_body_task_bypass< class_type, continue_msg >(
                    *this, __TBB_FLOW_GRAPH_PRIORITY_ARG1( continue_msg(),
                                                           my_priority ) );
        }
    }
...
//! Implements methods for both executable and function nodes that puts Output
//! to its successors
...
public:
    ...
    task* try_put_task(
        const output_type& i ) { // not a virtual method in this class
...
void add_nodes_impl( CompositeType* c_node,
                     bool visible,
                     const NodeType1& n1,
                     const NodeTypes&... n ) {
