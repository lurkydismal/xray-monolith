...
#if _MSC_VER
#pragma warning( disable : 4189 ) // warning 4189 -- local variable is
                                  // initialized but not referenced
#pragma warning( \
    disable      \
    : 4127 ) // warning 4127 -- while (true) has a constant expression in it
#endif
...
public:
    ...
    skip_list_node( size_type levels )
        : my_height( levels ), my_fullyLinked( false ) {
    ...
    pointer storage() { return reinterpret_cast< pointer >( &my_val ); }

    reference value() { return *storage(); }

    node_pointer next( size_type level ) const {
        __TBB_ASSERT( level < height(),
                      "Cannot get next on the level greater than height" );
        return my_next( level ).load( std::memory_order_acquire );
    }

    void set_next( size_type level, node_pointer next ) {
        __TBB_ASSERT( level < height(),
                      "Cannot set next on the level greater than height" );

        my_next( level ).store( next, std::memory_order_release );
    }

    /** @return number of layers */
    size_type height() const { return my_height; }
    ...
    lock_type acquire() { return lock_type( my_mutex ); }
...
private:
    ...
    atomic_node_pointer& my_next( size_type level ) {
        atomic_node_pointer* arr =
            reinterpret_cast< atomic_node_pointer* >( this + 1 );
        return arr[ level ];
    }

    const atomic_node_pointer& my_next( size_type level ) const {
        const atomic_node_pointer* arr =
...
public:
    ...
    // TODO: the code above does not compile in VS2015 (seems like a bug) -
    // consider enabling it for all other platforms template <typename T = void,
...
private:
    ...
    friend bool operator==( const skip_list_iterator< T, M >&,
...
bool operator==( const skip_list_iterator< NodeType, is_const1 >& lhs,
...
protected:
    ...
    using list_node_type =
        skip_list_node< value_type, typename traits_type::mutex_type >;

    using iterator = skip_list_iterator< list_node_type, /*is_const=*/false >;
    using const_iterator =
        skip_list_iterator< list_node_type, /*is_const=*/true >;
...
public:
     ...
     */
    concurrent_skip_list() : my_size( 0 ) { create_dummy_head(); }
    ...
    concurrent_skip_list( InputIt first,
                          InputIt last,
    ...
    concurrent_skip_list& operator=( const concurrent_skip_list& other ) {
        if ( this != &other ) {
            using pocca_type = typename node_allocator_traits::
                propagate_on_container_copy_assignment;
            clear();
            tbb::internal::allocator_copy_assignment(
                my_node_allocator, other.my_node_allocator, pocca_type() );
            my_compare = other.my_compare;
            my_rnd_generator = other.my_rnd_generator;
            internal_copy( other );
        }
    ...
    std::pair< iterator, bool > insert( node_type&& nh ) {
        if ( !nh.empty() ) {
            std::pair< iterator, bool > insert_result =
                internal_insert_node( nh.my_node );
    ...
    iterator unsafe_erase( iterator pos ) {
        std::pair< node_ptr, node_ptr > extract_result =
    ...
    template < typename K,
               typename = tbb::internal::is_transparent< key_compare, K >,
    ...
    node_type unsafe_extract( const_iterator pos ) {
        std::pair< node_ptr, node_ptr > extract_result =
            internal_extract( pos );
        return extract_result.first ? node_type( extract_result.first )
                                    : node_type();
    }
    ...
    template <
        typename K,
        typename = typename tbb::internal::is_transparent< key_compare, K > >
    ...
    iterator begin() { return iterator( dummy_head->next( 0 ) ); }
    ...
    void swap( concurrent_skip_list& other ) {
        ...
        tbb::internal::allocator_swap( my_node_allocator,
                                       other.my_node_allocator, pocs_type() );
    ...
    value_compare value_comp() const {
        return traits_type::value_comp( my_compare );
    }
    ...
    public:
        ...
        const_range_type( const_range_type& r, split ) : my_end( r.my_end ) {
            my_begin =
                iterator( r.my_begin.my_node_ptr->next( r.my_level - 1 ) );
...
private:
    ...
    iterator internal_find( const K& key ) {
        ...
        return ( it == end() || my_compare( key, traits_type::get_key( *it ) ) )
                   ? end()
                   : it;
    }
    ...
    pointer_type internal_find_position( size_type level,
                                         pointer_type& prev,
                                         const K& key,
    ...
    void fill_prev_next_arrays( array_type& prev_nodes,
    ...
    template < typename comparator >
    void fill_prev_next_by_ptr( array_type& prev_nodes,
                                array_type& next_nodes,
                                const_iterator it,
                                const key_type& key,
                                const comparator& cmp ) {
        ...
        std::fill( next_nodes.begin(), next_nodes.begin() + node_height,
                   erase_node );
    }
    ...
    std::pair< iterator, bool > internal_insert_node( node_ptr new_node ) {
        ...
        array_type next_nodes;
        __TBB_ASSERT( dummy_head->height() >= new_node->height(),
                      "Wrong height for new node" );

        do {
            if ( allow_multimapping ) {
                fill_prev_next_arrays( prev_nodes, next_nodes, dummy_head,
                                       get_key( new_node ),
                                       not_greater_compare( my_compare ) );
            } else {
                fill_prev_next_arrays( prev_nodes, next_nodes, dummy_head,
                                       get_key( new_node ), my_compare );
            }

            node_ptr next = next_nodes[ 0 ];
            if ( next && !allow_multimapping &&
                 !my_compare( get_key( new_node ), get_key( next ) ) ) {
    ...
    bool try_insert_node( node_ptr new_node,
                          array_type& prev_nodes,
                          array_type& next_nodes ) {
        ...
        if ( !try_lock_nodes( new_node->height(), prev_nodes, next_nodes,
                              locks ) ) {
            return false;
        }
        ...
        __TBB_ASSERT( allow_multimapping ||
                          ( ( prev_nodes[ 0 ] == dummy_head ||
                              my_compare( get_key( prev_nodes[ 0 ] ),
                                          get_key( new_node ) ) ) &&
                            ( next_nodes[ 0 ] == nullptr ||
                              my_compare( get_key( new_node ),
                                          get_key( next_nodes[ 0 ] ) ) ) ),
                      "Wrong elements order" );

        for ( size_type level = 0; level < new_node->height(); ++level ) {
            __TBB_ASSERT( prev_nodes[ level ]->height() > level, NULL );
            __TBB_ASSERT(
                prev_nodes[ level ]->next( level ) == next_nodes[ level ],
                NULL );
    ...
    bool try_lock_nodes( size_type height,
                         array_type& prevs,
                         array_type& next_nodes,
                         lock_array& locks ) {
    ...
    const_iterator internal_get_bound( const K& key,
                                       const comparator& cmp ) const {
        node_ptr prev = dummy_head;
        __TBB_ASSERT( dummy_head->height() > 0, NULL );
    ...
    // Returns node_ptr to the extracted node and node_ptr to the next node
    // after the extracted
    std::pair< node_ptr, node_ptr > internal_extract( const_iterator it ) {
        if ( it != end() ) {
            ...
            fill_prev_next_by_ptr( prev_nodes, next_nodes, it, key,
                                   my_compare );
...
private:
    ...
    size_type random_level() { return my_rnd_generator(); }

    static size_type calc_node_size( size_type height ) {
        return sizeof( list_node_type ) +
               height * sizeof( typename list_node_type::atomic_node_pointer );
    }
    ...
    node_ptr create_node( Args&&... args ) {
        ...
        node_ptr node = reinterpret_cast< node_ptr >(
            node_allocator_traits::allocate( my_node_allocator, sz ) );
        ...
        try {
            node_allocator_traits::construct( my_node_allocator,
                                              node->storage(),
    ...
    void create_dummy_head() {
        ...
        dummy_head = reinterpret_cast< node_ptr >(
            node_allocator_traits::allocate( my_node_allocator, sz ) );
        // TODO: investigate linkage fail in debug without this workaround
        auto max_level = MAX_LEVEL;

        try {
            node_allocator_traits::construct( my_node_allocator, dummy_head,
                                              max_level );
        } catch ( ... ) {
    ...
    void delete_node( node_ptr node ) {
        ...
        // Destroy value
        if ( !is_dummy )
            node_allocator_traits::destroy( my_node_allocator,
                                            node->storage() );
        // Destroy node
        node_allocator_traits::destroy( my_node_allocator, node );
        // Deallocate memory
    ...
    void delete_dummy_head() { delete_node< true >( dummy_head ); }
    ...
    void internal_move_assign( concurrent_skip_list&& other,
        ...
        tbb::internal::allocator_move_assignment(
            my_node_allocator, other.my_node_allocator, std::true_type() );
        internal_move( std::move( other ) );
    }
