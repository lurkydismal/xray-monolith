...
protected:
    ...
    typedef typename aligned_pair< item_type, buffer_item_state >::type
        buffer_item_type;
    typedef
        typename A::template rebind< buffer_item_type >::other allocator_type;
    ...
    buffer_item_type& item( size_type i ) {
        __TBB_ASSERT(
            !( size_type( &( my_array[ i & ( my_array_size - 1 ) ].second ) ) %
               alignment_of< buffer_item_state >::value ),
            NULL );
        __TBB_ASSERT(
            !( size_type( &( my_array[ i & ( my_array_size - 1 ) ].first ) ) %
    ...
    const item_type& get_my_item( size_t i ) const {
        ...
        item_type* itm = ( tbb::internal::punned_cast< item_type* >(
            &( item( i ).first ) ) );
    ...
    // move an existing item from one slot to another.  The moved-to slot must
    // be unoccupied, the moved-from slot must exist and not be reserved.  The
    // after, from will be empty, to will be occupied but not reserved
    void move_item( size_t to, size_t from ) {
        __TBB_ASSERT( !my_item_valid( to ),
                      "Trying to move to a non-empty slot" );
        __TBB_ASSERT( my_item_valid( from ),
                      "Trying to move from an empty slot" );
        set_my_item( to,
                     get_my_item( from ) ); // could have std::move semantics
        destroy_item( from );
    }
    ...
    void swap_items( size_t i, size_t j ) {
        __TBB_ASSERT( my_item_valid( i ) && my_item_valid( j ),
    ...
    const item_type& front() const {
        __TBB_ASSERT( my_item_valid( my_head ),
    ...
    const item_type& back() const {
        __TBB_ASSERT( my_item_valid( my_tail - 1 ),
                      "attempt to fetch head non-item" );
    ...
    void reserve_item( size_type i ) {
        __TBB_ASSERT( my_item_valid( i ) && !my_item_reserved( i ),
    ...
    // we have to be able to test against a new tail value without changing
    // my_tail grow_array doesn't work if we change my_tail when the old array
    // is too small
    ...
    void grow_my_array( size_t minimum_size ) {
        ...
        __TBB_ASSERT( capacity() >= my_tail - my_head,
                      "total items exceed capacity" );
        size_type new_size =
            my_array_size ? 2 * my_array_size : initial_buffer_size;
    ...
    // This is used both for reset and for grow_my_array.  In the case of
    // grow_my_array we want to retain the values of the head and tail.
...
public:
    //! Constructor
    item_buffer()
        : my_array( NULL ), my_array_size( 0 ), my_head( 0 ), my_tail( 0 ) {
        grow_my_array( initial_buffer_size );
    }

    ~item_buffer() { clean_up_buffer( /*reset_pointers*/ true ); }
