...
#if __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT
//! std::void_t internal implementation (to avoid GCC < 4.7 "template aliases"
//! absence)
template < typename... >
struct void_t {
    typedef void type;
...
struct stored_pack< T, Types... > : stored_pack< Types... > {
    ...
    // Since lifetime of original values is out of control, copies should be
    // made. Thus references should be stripped away from the deduced type.
    typename strip< T >::type leftmost_value;

    // Here rvalue references act in the same way as forwarding references,
    // as long as class template parameters were deduced via forwarding
    // references.
...
void call( F&& f, Pack&& p ) {
    strip< Pack >::type::template call< void >( std::forward< F >( f ),
                                                std::forward< Pack >( p ) );
}

template < typename Ret, typename F, typename Pack >
Ret call_and_return( F&& f, Pack&& p ) {
    return strip< Pack >::type::template call< Ret >(
        std::forward< F >( f ), std::forward< Pack >( p ) );
}
...
#elif __TBB_CPP11_VARIADIC_TEMPLATES_PRESENT && \
    __TBB_CPP11_TEMPLATE_ALIASES_PRESENT
...
using make_index_sequence =
    typename tbb::internal::make_index_sequence_impl< N >::type;
...
struct conjunction< First, Args... > : std::conditional< bool( First::value ),
                                                         conjunction< Args... >,
                                                         First >::type {};
...
using iterator_key_t = typename std::remove_const<
    typename iterator_value_t< Iter >::first_type >::type;
...
template < typename A >
using has_allocate = decltype( std::declval< alloc_ptr_t< A >& >() =
                                   std::declval< A >().allocate( 0 ) );
template < typename A >
using has_deallocate = decltype( std::declval< A >().deallocate(
    std::declval< alloc_ptr_t< A > >(),
    0 ) );

// value_type should be checked first because it can be used in other checks
...
// template parameter K and std::conditional are needed to provide immediate
// context and postpone getting is_trasparent from the compare functor until
