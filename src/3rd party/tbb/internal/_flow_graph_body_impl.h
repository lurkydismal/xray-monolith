...
struct has_policy< ExpectedPolicy, FirstPolicy, Policies... >
    : tbb::internal::bool_constant<
          has_policy< ExpectedPolicy, FirstPolicy >::value ||
...
struct has_policy< ExpectedPolicy, Policy< P > >
    : has_policy< ExpectedPolicy, P > {};

template < typename ExpectedPolicy, typename P1, typename P2 >
struct has_policy< ExpectedPolicy, Policy< P1, P2 > >
    : tbb::internal::bool_constant< has_policy< ExpectedPolicy, P1 >::value ||
                                    has_policy< ExpectedPolicy, P2 >::value > {
};
...
// provided functor that, given an object accepted by the port, will return the
/// field of type K being used for matching.
template < typename K,
           typename KHash = tbb_hash_compare< typename strip< K >::type > >
struct key_matching {
    typedef K key_type;
...
typedef interface11::internal::Policy< queueing, lightweight >
    queueing_lightweight;
typedef interface11::internal::Policy< rejecting, lightweight >
...
//! A task that calls a node's apply_body_bypass function, passing in an input
//! of type Input
//  return the task* unless it is SUCCESSFULLY_ENQUEUED, in which case return
//  NULL
...
public:
    apply_body_task_bypass( NodeType& n,
                            const Input& i
#if __TBB_PREVIEW_FLOW_GRAPH_PRIORITIES
                            ,
                            node_priority_t node_priority = no_priority )
    ...
    task* execute() __TBB_override {
        ...
        if ( next_task == SUCCESSFULLY_ENQUEUED )
            next_task = NULL;
        return next_task;
    }
};
...
//! An empty functor that takes an Input and returns a default constructed
//! Output
template < typename Input, typename Output >
struct empty_body {
...
class decrementer< T,
                   ...
                   typename tbb::internal::enable_if<
                       tbb::internal::is_integral< DecrementType >::value,
                       void >::type > : public receiver< DecrementType >,
...
protected:
    ...
    graph& graph_reference() const __TBB_override { return my_node->my_graph; }
...
public:
    ...
    typedef
        typename receiver< DecrementType >::predecessor_type predecessor_type;
...
template < typename T >
class decrementer< T, continue_msg, void > : public continue_receiver,
                                             tbb::internal::no_copy {
    T* my_node;

    task* execute() __TBB_override { return my_node->decrement_counter( 1 ); }
...
public:
    ...
    decrementer( int number_of_predecessors = 0 )
        : continue_receiver( __TBB_FLOW_GRAPH_PRIORITY_ARG1(
          ...
          // Since decrementer does not make use of possibly unconstructed owner
          // inside its constructor, my_node can be directly initialized with
          // 'this' pointer passed from the owner, hence making method
          // 'set_owner' needless.
          ,
          my_node( NULL ) {}
    void set_owner( T* node ) { my_node = node; }
