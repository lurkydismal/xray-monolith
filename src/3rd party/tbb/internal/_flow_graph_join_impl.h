...
// specialization that lets us keep a copy of the current_key for building
// results. KeyType can be a reference type.
...
struct join_helper {
    ...
    static inline void set_join_node_pointer( TupleType& my_input,
                                              PortType* port ) {
    ...
    static inline bool get_my_item( InputTuple& my_input, OutputTuple& out ) {
        bool res = tbb::flow::get< N - 1 >( my_input )
                       .get_item( tbb::flow::get< N - 1 >( out ) ); // may fail
    ...
    static inline void set_key_functors( InputTuple& my_input,
                                         KeyFuncTuple& my_key_funcs ) {
        tbb::flow::get< N - 1 >( my_input )
            .set_my_key_func( tbb::flow::get< N - 1 >( my_key_funcs ) );
        tbb::flow::get< N - 1 >( my_key_funcs ) = NULL;
        join_helper< N - 1 >::set_key_functors( my_input, my_key_funcs );
    }

    template < typename KeyFuncTuple >
    static inline void copy_key_functors( KeyFuncTuple& my_inputs,
                                          KeyFuncTuple& other_inputs ) {
        if ( tbb::flow::get< N - 1 >( other_inputs ).get_my_key_func() ) {
            tbb::flow::get< N - 1 >( my_inputs )
                .set_my_key_func( tbb::flow::get< N - 1 >( other_inputs )
                                      .get_my_key_func()
                                      ->clone() );
        }
...
struct join_helper< 1 > {
    ...
    template < typename InputTuple, typename OutputTuple >
    static inline bool reserve( InputTuple& my_input, OutputTuple& out ) {
        return tbb::flow::get< 0 >( my_input )
            .reserve( tbb::flow::get< 0 >( out ) );
    }

    template < typename InputTuple, typename OutputTuple >
    static inline bool get_my_item( InputTuple& my_input, OutputTuple& out ) {
        return tbb::flow::get< 0 >( my_input )
            .get_item( tbb::flow::get< 0 >( out ) );
    }
    ...
    static inline void set_key_functors( InputTuple& my_input,
                                         KeyFuncTuple& my_key_funcs ) {
        tbb::flow::get< 0 >( my_input )
            .set_my_key_func( tbb::flow::get< 0 >( my_key_funcs ) );
    ...
    static inline void copy_key_functors( KeyFuncTuple& my_inputs,
        ...
        if ( tbb::flow::get< 0 >( other_inputs ).get_my_key_func() ) {
            tbb::flow::get< 0 >( my_inputs )
                .set_my_key_func( tbb::flow::get< 0 >( other_inputs )
                                      .get_my_key_func()
                                      ->clone() );
        }
    }
...
private:
    ...
    typedef internal::aggregating_functor< class_type,
                                           reserving_port_operation >
        handler_type;
    friend class internal::aggregating_functor< class_type,
                                                reserving_port_operation >;
        ...
        while ( op_list ) {
            ...
            switch ( current->type ) {
                ...
                case res_item:
                    ...
                    } else if ( my_predecessors.try_reserve(
                                    *( current->my_arg ) ) ) {
                        reserved = true;
                        __TBB_store_with_release( current->status, SUCCEEDED );
                    } else {
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
                case add_blt_pred:
                    my_predecessors.internal_add_built_predecessor(
                        *( current->my_pred ) );
                    __TBB_store_with_release( current->status, SUCCEEDED );
                    break;
                case del_blt_pred:
                    my_predecessors.internal_delete_built_predecessor(
                        *( current->my_pred ) );
                    __TBB_store_with_release( current->status, SUCCEEDED );
                    break;
                case blt_pred_cnt:
...
public:
    ...
    void set_join_node_pointer( forwarding_base* join ) { my_join = join; }
...
private:
    ...
    typedef internal::aggregating_functor< class_type, queueing_port_operation >
        handler_type;
    friend class internal::aggregating_functor< class_type,
        ...
        while ( op_list ) {
            ...
            switch ( current->type ) {
                ...
                case get__item:
                    ...
                    break;
                case res_port:
                    __TBB_ASSERT( this->my_item_valid( this->my_head ),
                                  "No item to reset" );
                    this->destroy_front();
...
public:
    ...
    //! copy constructor
    queueing_port( const queueing_port& /* other */ )
        : receiver< T >(), item_buffer< T >() {
        my_join = NULL;
        my_aggregator.initialize_handler( handler_type( this ) );
    }
...
class key_matching_port : public receiver< typename TraitsType::T >,
                          public hash_buffer< typename TraitsType::K,
                                              typename TraitsType::T,
                                              typename TraitsType::TtoK,
    ...
    void handle_operations( key_matching_port_operation* op_list ) {
        ...
        while ( op_list ) {
            ...
            switch ( current->type ) {
                case try__put: {
                    bool was_inserted =
                        this->insert_with_key( current->my_val );
                    // return failure if a duplicate insertion occurs
                    __TBB_store_with_release(
                        current->status, was_inserted ? SUCCEEDED : FAILED );
                } break;
                case get__item:
                    // use current_key from FE for item
                    if ( !this->find_with_key( my_join->current_key,
                                               *( current->my_arg ) ) ) {
                        __TBB_ASSERT( false,
                                      "Failed to find item corresponding to "
                                      "current_key." );
                    }
                    __TBB_store_with_release( current->status, SUCCEEDED );
                    break;
...
protected:
    ...
    task* try_put_task( const input_type& v ) __TBB_override {
        ...
        if ( op_data.status == SUCCEEDED ) {
            rtask = my_join->increment_key_count(
                ( *( this->get_key_func() ) )( v ), false ); // may spawn
...
public:
    ...
    // copy constructor
    key_matching_port( const key_matching_port& /*other*/ )
        : receiver< input_type >(), buffer_type() {
        my_join = NULL;
        my_aggregator.initialize_handler( handler_type( this ) );
    }
...
public:
    ...
    join_node_FE( const join_node_FE& other )
        : forwarding_base( ( other.forwarding_base::graph_ref ) ),
          my_node( NULL ) {
        ports_with_no_inputs = N;
        join_helper< N >::set_join_node_pointer( my_inputs, this );
    }
    ...
    void increment_port_count() __TBB_override { ++ports_with_no_inputs; }

    // if all input_ports have predecessors, spawn forward to try and consume
    // tuples
...
protected:
    ...
    // all methods on input ports should be called under mutual exclusion from
    // join_node_base.

    bool tuple_build_may_succeed() { return !ports_with_no_inputs; }
...
public:
    ...
    typedef OutputTuple output_type;
    typedef InputTuple input_type;
    typedef join_node_base< queueing, InputTuple, OutputTuple >
        base_node_type; // for forwarding
    ...
    void reset_port_count() { ports_with_no_items = N; }
...
protected:
    ...
    bool tuple_build_may_succeed() { return !ports_with_no_items; }
...
public:
    ...
    typedef internal::type_to_key_function_body< count_element_type,
                                                 unref_key_type& >
        TtoK_function_body_type;
    typedef internal::type_to_key_function_body_leaf< count_element_type,
                                                      unref_key_type&,
                                                      key_to_count_func >
        TtoK_function_body_leaf_type;
    // this is the type of the special table that keeps track of the number of
    // discrete elements corresponding to each key that we've seen.
    ...
    typedef join_node_base< key_matching< key_type, key_hash_compare >,
                            InputTuple,
                            OutputTuple >
        base_node_type; // for forwarding
...
private:
    ...
    // returns a task pointer if the a task would have been enqueued but we
    // asked that it be returned.  Otherwise returns NULL.
    task* fill_output_buffer( unref_key_type& t,
                              bool should_enqueue,
                              bool handle_task ) {
        output_type l_out;
        task* rtask = NULL;
        bool do_fwd = should_enqueue && this->buffer_empty() &&
                      internal::is_graph_active( this->graph_ref );
        this->current_key = t;
        this->delete_with_key( this->current_key ); // remove the key
        if ( join_helper< N >::get_items( my_inputs,
                                          l_out ) ) { //  <== call back
            this->push_back( l_out );
            if ( do_fwd ) { // we enqueue if receiving an item from predecessor,
                            // not if successor asks for item
            ...
            switch ( current->type ) {
                ...
                case inc_count: { // called from input ports
                    ...
                    if ( !( this->find_ref_with_key( t, p ) ) ) {
                        ...
                        if ( !( this->find_ref_with_key( t, p ) ) ) {
                            __TBB_ASSERT(
                                false, "should find key after inserting it" );
                        }
                    }
                    if ( ++( p->my_value ) == size_t( N ) ) {
                        task* rtask = fill_output_buffer( t, true, do_enqueue );
                        __TBB_ASSERT( !rtask || !do_enqueue,
                ...
                case may_succeed: // called from BE
                    __TBB_store_with_release(
                        current->status,
                        this->buffer_empty() ? FAILED : SUCCEEDED );
                    break;
...
public:
    ...
    join_node_FE( const join_node_FE& other )
        ...
        my_node = NULL;
        join_helper< N >::set_join_node_pointer( my_inputs, this );
        join_helper< N >::copy_key_functors(
            my_inputs, const_cast< input_type& >( other.my_inputs ) );
        my_aggregator.initialize_handler( handler_type( this ) );
        TtoK_function_body_type* cfb =
            new TtoK_function_body_leaf_type( key_to_count_func() );
        this->set_key_func( cfb );
    ...
    task* increment_key_count( unref_key_type const& t, bool handle_task )
        __TBB_override { // called from input_ports
...
protected:
    ...
    // cannot lock while calling back to input_ports.  current_key will only be
    // set and reset under the aggregator, so it will remain consistent.
    bool try_to_make_tuple( output_type& out ) {
            ...
            switch ( current->type ) {
                case reg_succ: {
                    ...
                    if ( tuple_build_may_succeed() && !forwarder_busy &&
                         internal::is_graph_active( my_graph ) ) {
                        task* rtask = new ( task::allocate_additional_child_of(
                            *( my_graph.root_task() ) ) )
                            forward_task_bypass<
                ...
                case try__get:
                    if ( tuple_build_may_succeed() ) {
                        if ( try_to_make_tuple( *( current->my_arg ) ) ) {
                            tuple_accepted();
                            __TBB_store_with_release( current->status,
                                                      SUCCEEDED );
                        } else
                            __TBB_store_with_release( current->status, FAILED );
                    } else
                ...
                case do_fwrd_bypass: {
                    ...
                    task* last_task = NULL;
                    output_type out;
                    if ( tuple_build_may_succeed() ) { // checks output queue of
                                                       // FE
                        do {
                            build_succeeded = try_to_make_tuple(
                                out ); // fetch front_end of queue
                            if ( build_succeeded ) {
                                task* new_task =
                                    my_successors.try_put_task( out );
                                last_task = combine_tasks( my_graph, last_task,
                                                           new_task );
                                if ( new_task ) {
                                    tuple_accepted();
                                } else {
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
                case add_blt_succ:
                    my_successors.internal_add_built_successor(
                        *( current->my_succ ) );
                    __TBB_store_with_release( current->status, SUCCEEDED );
                    break;
                case del_blt_succ:
                    my_successors.internal_delete_built_successor(
                        *( current->my_succ ) );
                    __TBB_store_with_release( current->status, SUCCEEDED );
                    break;
                case blt_succ_cnt:
...
public:
    join_node_base( graph& g )
        : graph_node( g ), input_ports_type( g ), forwarder_busy( false ) {
    ...
    join_node_base( graph& g, FunctionTuple f )
        : graph_node( g ), input_ports_type( g, f ), forwarder_busy( false ) {
        my_successors.set_owner( this );
        input_ports_type::set_my_node( this );
        my_aggregator.initialize_handler( handler_type( this ) );
    }
...
template < int N,
           ...
           typename JP >
struct join_base {
    typedef typename internal::join_node_base<
        JP,
        typename wrap_tuple_elements< N, PT, OutputTuple >::type,
        OutputTuple >
        type;
};
...
struct join_base< N,
    ...
    typedef typename internal::join_node_base<
        key_traits_type,
        // ports type
        typename wrap_key_tuple_elements< N,
                                          key_matching_port,
                                          key_traits_type,
                                          OutputTuple >::type,
        OutputTuple >
...
//! unfolded_join_node : passes input_ports_type to join_node_base.  We build
//! the input port type
//  using tuple_element.  The class PT is the port type (reserving_port,
//  queueing_port, key_matching_port) and should match the typename.
...
public:
    typedef typename wrap_tuple_elements< N, PT, OutputTuple >::type
        input_ports_type;
    typedef OutputTuple output_type;
...
// key_matching unfolded_join_node.  This must be a separate specialization
// because the constructors differ.

template < typename OutputTuple, typename K, typename KHash >
class unfolded_join_node< 2,
                          key_matching_port,
                          OutputTuple,
                          key_matching< K, KHash > >
    : public join_base< 2,
...
public:
    typedef
        typename wrap_key_tuple_elements< 2,
                                          key_matching_port,
                                          key_matching< K, KHash >,
...
private:
    typedef join_node_base< key_matching< K, KHash >,
                            input_ports_type,
                            output_type >
        base_type;
...
public:
    ...
    unfolded_join_node( graph& g, Body0 body0, Body1 body1 )
        : base_type(
              g,
              func_initializer_type(
                  new internal::type_to_key_function_body_leaf< T0, K, Body0 >(
                      body0 ),
                  new internal::type_to_key_function_body_leaf< T1, K, Body1 >(
                      body1 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 3,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
    ...
    unfolded_join_node( graph& g, Body0 body0, Body1 body1, Body2 body2 )
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T1, K, Body1 >(
                      body1 ),
                  new internal::type_to_key_function_body_leaf< T2, K, Body2 >(
                      body2 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 4,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
...
private:
    ...
    typedef typename tbb::flow::tuple< f0_p, f1_p, f2_p, f3_p >
        func_initializer_type;
    ...
    unfolded_join_node( graph& g,
        ...
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T2, K, Body2 >(
                      body2 ),
                  new internal::type_to_key_function_body_leaf< T3, K, Body3 >(
                      body3 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 5,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
...
private:
    ...
    typedef typename tbb::flow::tuple< f0_p, f1_p, f2_p, f3_p, f4_p >
        func_initializer_type;
    ...
    unfolded_join_node( graph& g,
        ...
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T3, K, Body3 >(
                      body3 ),
                  new internal::type_to_key_function_body_leaf< T4, K, Body4 >(
                      body4 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 6,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
...
private:
    ...
    typedef typename tbb::flow::tuple< f0_p, f1_p, f2_p, f3_p, f4_p, f5_p >
        func_initializer_type;
    ...
    unfolded_join_node( graph& g,
        ...
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T4, K, Body4 >(
                      body4 ),
                  new internal::type_to_key_function_body_leaf< T5, K, Body5 >(
                      body5 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 7,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
...
private:
    ...
    typedef
        typename tbb::flow::tuple< f0_p, f1_p, f2_p, f3_p, f4_p, f5_p, f6_p >
            func_initializer_type;
    ...
    unfolded_join_node( graph& g,
        ...
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T5, K, Body5 >(
                      body5 ),
                  new internal::type_to_key_function_body_leaf< T6, K, Body6 >(
                      body6 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 8,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
...
private:
    ...
    typedef typename tbb::flow::
        tuple< f0_p, f1_p, f2_p, f3_p, f4_p, f5_p, f6_p, f7_p >
            func_initializer_type;
    ...
    unfolded_join_node( graph& g,
        ...
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T6, K, Body6 >(
                      body6 ),
                  new internal::type_to_key_function_body_leaf< T7, K, Body7 >(
                      body7 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 9,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
...
private:
    ...
    typedef typename tbb::flow::
        tuple< f0_p, f1_p, f2_p, f3_p, f4_p, f5_p, f6_p, f7_p, f8_p >
            func_initializer_type;
    ...
    unfolded_join_node( graph& g,
        ...
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T7, K, Body7 >(
                      body7 ),
                  new internal::type_to_key_function_body_leaf< T8, K, Body8 >(
                      body8 ) ) ) {
...
public:
    typedef
        typename wrap_key_tuple_elements< 10,
                                          key_matching_port,
                                          key_matching< K, KHash >,
                                          OutputTuple >::type input_ports_type;
    typedef OutputTuple output_type;
...
private:
    ...
    typedef typename tbb::flow::
        tuple< f0_p, f1_p, f2_p, f3_p, f4_p, f5_p, f6_p, f7_p, f8_p, f9_p >
            func_initializer_type;
    ...
    unfolded_join_node( graph& g,
        ...
        : base_type(
              ...
              func_initializer_type(
                  ...
                  new internal::type_to_key_function_body_leaf< T8, K, Body8 >(
                      body8 ),
                  new internal::type_to_key_function_body_leaf< T9, K, Body9 >(
                      body9 ) ) ) {
...
typename tbb::flow::tuple_element< N, typename JNT::input_ports_type >::type&
input_port( JNT& jn ) {
