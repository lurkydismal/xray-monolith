...
struct port_ref_impl {
    // "+1" since the port_ref range is a closed interval (includes its
    // endpoints).
...
// The purpose of the port_ref_impl is the pretty syntax: the deduction of a
// compile-time constant is processed from the return type. So it is possible to
// use this helper without parentheses, e.g. "port_ref<0>".
...
template < typename T >
struct is_port_ref {
    typedef typename is_port_ref_impl<
        typename tbb::internal::strip< T >::type >::type type;
};
...
struct convert_and_call_impl< A1, Args1... > {
    ...
    static void doit_impl( std::true_type x,
                           ...
                           Args1&... args1,
                           Args2&... args2 ) {
        convert_and_call_impl< port_ref_impl< N1 + 1, N2 >,
                               Args1... >::doit_impl( x, f, t,
                                                      port_ref< N1 + 1, N2 >(),
                                                      args1..., args2...,
                                                      std::get< N1 + my_delta >(
                                                          t ) );
    ...
    static void doit_impl( std::true_type,
                           ...
                           Args1&... args1,
                           Args2&... args2 ) {
        convert_and_call_impl< Args1... >::doit(
            f, t, args1..., args2..., std::get< N + my_delta >( t ) );
    }
...
struct streaming_node_traits {
    // Do not use 'using' instead of 'struct' because Microsoft Visual C++ 12.0
    // fails to compile.
    template < typename T >
    ...
    typedef tuple<
        streaming_device_with_key< typename StreamFactory::device_type,
                                   typename key_from_policy< JP >::type >,
        typename async_msg_type< Ports >::type... >
        kernel_input_tuple;

    // indexer_node parameters pack expansion workaround for VS2013 for
    // streaming_node
    typedef indexer_node< typename async_msg_type< Ports >::type... >
        indexer_node_type;
};
...
protected:
    template < typename... Args >
    void enqueue_kernel_impl( kernel_input_tuple&,
...
class kernel_executor_helper< StreamFactory,
                              KernelInputTuple,
                              typename tbb::internal::void_t<
                                  typename StreamFactory::range_type >::type > {
    typedef typename StreamFactory::device_type device_type;
    typedef typename StreamFactory::kernel_type kernel_type;
    typedef KernelInputTuple kernel_input_tuple;
    ...
    // Container for randge. It can contain either port references or real
    // range.
    ...
    struct range_mapper : public range_wrapper {
        ...
        range_type get_range( const kernel_input_tuple& ip ) const
            __TBB_override {
...
protected:
    template < typename... Args >
    void enqueue_kernel_impl( kernel_input_tuple& ip,
                              StreamFactory& factory,
                              device_type device,
                              const kernel_type& kernel,
                              Args&... args ) const {
        __TBB_ASSERT( my_range_wrapper,
                      "Range is not set. Call set_range() before running "
                      "streaming_node." );
        factory.send_kernel( device, kernel, my_range_wrapper->get_range( ip ),
                             args... );
    }
...
public:
    ...
    kernel_executor_helper( const kernel_executor_helper& executor )
        : my_range_wrapper( executor.my_range_wrapper
                                ? executor.my_range_wrapper->clone()
                                : NULL ) {}
...
| |   |              |   |    (device_with_key) O---O           |   | |   | | |
|   |                      |   |           |   |                      |   |
O---O indexer_node O---O device_selector_node O---O join_node O---O kernel_node
O---O |   |              |   | (multifunction_node) |   |           |   |
(multifunction_node) |   | O---O              |   |                      O---O
|   |                      O---O |   \--------------/   \----------------------/
\-----------/   \----------------------/   | | |
\--------------------------------------------------------------------------------------------/
*/
...
class streaming_node< tuple< Ports... >, JP, StreamFactory >
    : public composite_node<
          typename internal::
              streaming_node_traits< JP, StreamFactory, Ports... >::input_tuple,
          typename internal::streaming_node_traits< JP,
                                                    StreamFactory,
                                                    Ports... >::output_tuple >,
      public internal::kernel_executor_helper<
          StreamFactory,
          typename internal::
              streaming_node_traits< JP, StreamFactory, Ports... >::
                  kernel_input_tuple > {
    typedef typename internal::
        streaming_node_traits< JP, StreamFactory, Ports... >::input_tuple
            input_tuple;
    typedef typename internal::
        streaming_node_traits< JP, StreamFactory, Ports... >::output_tuple
            output_tuple;
...
private:
    typedef internal::streaming_device_with_key< device_type, key_type >
    ...
    typedef
        typename internal::make_sequence< NUM_OUTPUTS >::type output_sequence;

    typedef typename internal::
        streaming_node_traits< JP, StreamFactory, Ports... >::indexer_node_type
            indexer_node_type;
    typedef typename indexer_node_type::output_type indexer_node_output_type;
    typedef typename internal::
        streaming_node_traits< JP, StreamFactory, Ports... >::kernel_input_tuple
            kernel_input_tuple;
    typedef multifunction_node< indexer_node_output_type, kernel_input_tuple >
        device_selector_node;
    typedef multifunction_node< kernel_input_tuple, output_tuple >
    ...
    template < int N >
    int make_Nth_edge() {
        make_edge( internal::output_port< N >( my_device_selector_node ),
                   internal::input_port< N >( my_join_node ) );
        return 0;
    }
    ...
    void make_edges() { make_edges( input_sequence() ); }
    ...
    class device_selector : public device_selector_base,
    ...
    public:
        ...
        void operator()( const indexer_node_output_type& v,
                         typename device_selector_node::output_ports_type& op )
            __TBB_override {
            ( this->*my_dispatch_funcs[ v.tag() ] )( my_port_epoches[ v.tag() ],
                                                     v, op );
            __TBB_ASSERT(
                ( tbb::internal::is_same_type<
                    typename internal::key_from_policy< JP >::is_key_matching,
                    std::false_type >::value ) ||
                    my_port_epoches[ v.tag() ] == 0,
    ...
    private:
        typedef void ( device_selector< UserFunctor >::*send_and_put_fn_type )(
            size_t&,
            const indexer_node_output_type&,
            typename device_selector_node::output_ports_type& );
        typedef std::array< send_and_put_fn_type, NUM_INPUTS >
            dispatch_funcs_type;

        template < int... S >
        static dispatch_funcs_type create_dispatch_funcs(
            internal::sequence< S... > ) {
            dispatch_funcs_type dispatch = { { &device_selector<
                UserFunctor >::send_and_put_impl< S >... } };
            return dispatch;
        }
        ...
        key_type get_key( std::false_type, const T&, size_t& epoch ) {
            __TBB_STATIC_ASSERT(
                ( tbb::internal::is_same_type< key_type, size_t >::value ),
                "" );
            return epoch++;
        }
        ...
        template < int N >
        void send_and_put_impl(
            size_t& epoch,
            const indexer_node_output_type& v,
            typename device_selector_node::output_ports_type& op ) {
            typedef typename tuple_element<
                N + 1, typename device_selector_node::output_ports_type >::
                type::output_type elem_type;
        ...
        device_type get_device( key_type key, DevicePort& dp ) {
            typename std::unordered_map< typename std::decay< key_type >::type,
                                         epoch_desc >::iterator it =
                my_devices.find( key );
            if ( it == my_devices.end() ) {
                device_type d = my_user_functor( my_factory );
                std::tie( it, std::ignore ) =
                    my_devices.insert( std::make_pair( key, d ) );
            ...
            if ( ++e.my_request_number == NUM_INPUTS )
                my_devices.erase( it );
            return d;
        }
        ...
        std::unordered_map< typename std::decay< key_type >::type, epoch_desc >
            my_devices;
    ...
    public:
        typedef typename kernel_multifunction_node::output_ports_type
            output_ports_type;

        virtual void enqueue( kernel_input_tuple& ip,
    ...
    protected:
        ...
        args_storage_base( const args_storage_base& k )
            : tbb::internal::no_copy(),
              my_kernel( k.my_kernel ),
              my_factory( k.my_factory ) {}
    ...
    class args_storage : public args_storage_base {
        ...
        template < int N >
        bool do_try_put( const kernel_input_tuple& ip,
                         output_ports_type& op ) const {
        ...
        public:
            run_kernel_func( kernel_input_tuple& ip,
                             const streaming_node& node,
                             const args_storage& storage )
                : my_kernel_func( ip, node, storage, get< 0 >( ip ).device() ) {
            }

            // It is immpossible to use Args... because a function pointer
            // cannot be casted to a function reference implicitly. Allow the
            // compiler to deduce types for function pointers automatically.
            template < typename... FnArgs >
            void operator()( FnArgs&... args ) {
                internal::convert_and_call_impl< FnArgs... >::doit(
                    my_kernel_func, my_kernel_func.my_ip, args... );
            }
        ...
        private:
            struct kernel_func : tbb::internal::no_copy {
                ...
                kernel_func( kernel_input_tuple& ip,
                             const streaming_node& node,
                             const args_storage& storage,
                             device_type device )
                    : my_ip( ip ),
                ...
                template < typename... FnArgs >
                void operator()( FnArgs&... args ) {
                    my_node.enqueue_kernel( my_ip, my_storage.my_factory,
                                            my_device, my_storage.my_kernel,
                                            args... );
                }
            } my_kernel_func;
        };
        ...
        public:
            run_finalize_func( kernel_input_tuple& ip,
                               StreamFactory& factory,
                               FinalizeFn fn )
                : my_ip( ip ),
                  my_finalize_func( factory, get< 0 >( ip ).device(), fn ) {}
        ...
        private:
            ...
            struct finalize_func : tbb::internal::no_assign {
                ...
                finalize_func( StreamFactory& factory,
                               device_type device,
                               FinalizeFn fn )
        ...
        template < typename FinalizeFn >
        static run_finalize_func< FinalizeFn > make_run_finalize_func(
            kernel_input_tuple& ip,
            StreamFactory& factory,
            FinalizeFn fn ) {
    ...
    public:
        args_storage( const kernel_type& kernel,
                      StreamFactory& f,
                      Args&&... args )
        ...
        args_storage( const args_storage& k )
            : args_storage_base( k ), my_args_pack( k.my_args_pack ) {}

        args_storage( const args_storage_base& k, Args&&... args )
            : args_storage_base( k ),
              my_args_pack( std::forward< Args >( args )... ) {}

        void enqueue( kernel_input_tuple& ip,
                      output_ports_type& op,
                      const streaming_node& n ) __TBB_override {
            // Make const qualified args_pack (from non-const)
            const args_pack_type& const_args_pack = my_args_pack;
            // factory.enqure_kernel() gets
            //  - 'ip' tuple elements by reference and updates it (and 'ip')
            //  with dependencies
            //  - arguments (from my_args_pack) by const-reference via
            //  const_args_pack
            tbb::internal::call( run_kernel_func( ip, n, *this ),
                                 const_args_pack );

            if ( !do_try_put( ip, op, input_sequence() ) ) {
                graph& g = n.my_graph;
                // No one message was passed to successors so set a callback to
                // extend the graph lifetime until the kernel completion.
                ...
                //  - arguments (from my_args_pack) by const-reference via
                //  const_args_pack
                tbb::internal::call( make_run_finalize_func(
                                         ip, this->my_factory,
                                         [ &g ] { g.decrement_wait_count(); } ),
                                     const_args_pack );
            }
        }

        void send( device_type d ) __TBB_override {
            // factory.send() gets arguments by reference and updates these
            // arguments with dependencies (it gets but usually ignores
            // port_ref-s)
            tbb::internal::call( send_func( this->my_factory, d ),
                                 my_args_pack );
        }
    ...
    public:
        ...
        void operator()( kernel_input_tuple ip,
                         typename args_storage_base::output_ports_type& op ) {
            __TBB_ASSERT( ( my_node.my_args_storage != NULL ),
                          "No arguments storage" );
            // 'ip' is passed by value to create local copy for updating inside
            // enqueue_kernel()
    ...
    template < typename T,
               typename U = typename internal::is_port_ref< T >::type >
    ...
    struct wrap_to_async< T, std::false_type > {
        typedef typename StreamFactory::template async_msg_type<
            typename tbb::internal::strip< T >::type >
            type;
    };

    template < typename... Args >
    args_storage_base* make_args_storage( const args_storage_base& storage,
                                          Args&&... args ) const {
        // In this variadic template convert all simple types 'T' into
        // 'async_msg_type<T>'
        return new args_storage< Args... >( storage,
                                            std::forward< Args >( args )... );
    }

    void notify_new_device( device_type d ) { my_args_storage->send( d ); }

    template < typename... Args >
    void enqueue_kernel( kernel_input_tuple& ip,
                         StreamFactory& factory,
                         device_type device,
                         const kernel_type& kernel,
                         Args&... args ) const {
...
public:
    ...
    streaming_node( graph& g,
        ...
        : base_type( g ),
          my_indexer_node( g ),
          my_device_selector(
              new device_selector< DeviceSelector >( d, *this, f ) ),
          my_device_selector_node( g,
                                   serial,
                                   device_selector_body( my_device_selector ) ),
          my_join_node( g ),
          my_kernel_node( g, serial, kernel_body( *this ) )
          // By default, streaming_node maps all its ports to the kernel
          // arguments on a one-to-one basis.
          ,
          my_args_storage(
              make_args_storage( args_storage<>( kernel, f ),
    ...
    streaming_node( const streaming_node& node )
        : base_type( node.my_graph ),
          ...
          my_device_selector_node( node.my_graph,
                                   serial,
                                   device_selector_body( my_device_selector ) ),
    ...
    ~streaming_node() {
        ...
        if ( my_device_selector )
            delete my_device_selector;
    }

    template < typename... Args >
    void set_args( Args&&... args ) {
        // Copy the base class of args_storage and create new storage for
        // "Args...".
        args_storage_base* const new_args_storage = make_args_storage(
            *my_args_storage, typename wrap_to_async< Args >::type(
                                  std::forward< Args >( args ) )... );
        delete my_args_storage;
...
protected:
    void reset_node( reset_flags = rf_reset_protocol ) __TBB_override {
        __TBB_ASSERT( false, "Not implemented yet" );
    }
