...
//! A node_cache maintains a std::queue of elements of type T.  Each operation
...
protected:
    ...
    inline bool internal_empty() { return my_q.empty(); }

    // Assumes lock is held
    inline size_type internal_size() { return my_q.size(); }

    // Assumes lock is held
    inline void internal_push( T& n ) { my_q.push( &n ); }
...
// TODO: make predecessor_cache type T-independent when async_msg becomes
// regular feature
...
public:
    ...
    // If we are removing arcs (rf_clear_edges), call clear() rather than
    // reset().
    void reset() {
        if ( my_owner ) {
            for ( ;; ) {
...
// TODO: make reservable_predecessor_cache type T-independent when async_msg
// becomes regular feature
template < typename T, typename M = spin_mutex >
...
// TODO: make successor_cache type T-independent when async_msg becomes regular
// feature
...
public:
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    typedef typename edge_container< successor_type >::edge_list_type
        successor_list_type;

    edge_container< successor_type >& built_successors() {
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    edge_container< successor_type > my_built_successors;
    typedef edge_container< successor_type >::edge_list_type
        successor_list_type;
#endif
...
// TODO: make broadcast_cache type T-independent when async_msg becomes regular
// feature
template < typename T, typename M = spin_rw_mutex >
...
public:
    ...
    // as above, but call try_put_task instead, and return the last task we
    // received (if any)
#if __TBB_PREVIEW_ASYNC_MSG
    template < typename X >
    ...
    task* try_put_task( const T& t ) __TBB_override {
        ...
        while ( i != this->my_successors.end() ) {
            ...
            last_task = combine_tasks( graph_ref, last_task,
                                       new_task ); // enqueue if necessary
            if ( new_task ) {
                ++i;
            } else { // failed
...
// TODO: make round_robin_cache type T-independent when async_msg becomes
// regular feature
template < typename T, typename M = spin_rw_mutex >
