...
public:
    //! Zero value means "wait" status, all other values are "user" specified
    //! values and are defined into the scope of a class which uses "status".
    uintptr_t status;
...
/** An aggregator for collecting operations coming from multiple sources and
   executing them serially on a single thread.  operation_type must be derived
   from aggregated_operation. The parameter handler_type is a functor that will
   be passed the list of operations and is expected to handle each operation
   appropriately, setting the status of each operation to non-zero.*/
...
public:
    ...
    /** Places an operation into the waitlist (pending_operations), and either
       ...
       destroyed during execution, and so any access to it after it was put into
       the waitlist, including status check, is invalid. As a consequence,
       waiting for completion of such operation causes undefined behavior.
    */
    ...
    void execute( operation_type* op,
        ...
        // node. This thread has created the operation, and now releases it so
        // that the handler thread may handle the associated operation w/o
        // triggering a race condition; thus this tag will be acquired just
        // before the operation is handled in the handle_operations functor.
        call_itt_notify( releasing, &( op->status ) );
        // insert the operation in the queue.
        do {
            // Tools may flag the following line as a race; it is a false
            // positive: This is an atomic read; we don't provide
            // itt_hide_load_word for atomics
        ...
        if ( !res ) { // first in the list; handle the operations.
            // ITT note: &pending_operations tag covers access to the
            // handler_busy flag, which this waiting handler thread will try to
            // set before entering handle_operations.
        ...
        // not first; wait for op to be ready.
            ...
            __TBB_ASSERT( long_life_time,
                          "Waiting for an operation object that might be "
...
private:
    ...
    void start_handle_operations( handler_type& handle_operations ) {
        ...
        // ITT note: &handler_busy tag covers access to pending_operations as it
        // is passed between active and waiting handlers.  Below, the waiting
        // handler waits until the active handler releases, and the waiting
        // handler acquires &handler_busy as it becomes the active_handler. The
        // release point is at the end of this function, when all operations in
        // pending_operations have been handled by the owner of this aggregator.
        ...
        // acquire fence not necessary here due to causality rule and
        // surrounding atomics
        __TBB_store_with_release( handler_busy, uintptr_t( 1 ) );

        // ITT note: &pending_operations tag covers access to the handler_busy
        // flag itself. Capturing the state of the pending_operations signifies
        // that handler_busy has been set and a new active handler will now
        // process that list's operations.
