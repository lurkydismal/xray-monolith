...
//! Fast, unfair, spinning speculation-enabled reader-writer lock with backoff
//! and
//  writer-preference
...
class x86_rtm_rw_mutex : private spin_rw_mutex {
    ...
    // below. Make the scoped_lock public in that case.
public:
#else
private:
#endif
...
private:
    ...
    internal_acquire_writer( x86_rtm_rw_mutex::scoped_lock&,
                             bool only_speculate = false );

    //! Internal acquire read lock.
    // only_speculate == true if we're doing a try_lock, else false.
    void __TBB_EXPORTED_METHOD
    internal_acquire_reader( x86_rtm_rw_mutex::scoped_lock&,
                             bool only_speculate = false );

    //! Internal upgrade reader to become a writer.
    bool __TBB_EXPORTED_METHOD
    internal_upgrade( x86_rtm_rw_mutex::scoped_lock& );

    //! Out of line code for downgrading a writer to a reader.
    bool __TBB_EXPORTED_METHOD
    internal_downgrade( x86_rtm_rw_mutex::scoped_lock& );

    //! Internal try_acquire write lock.
    bool __TBB_EXPORTED_METHOD
    internal_try_acquire_writer( x86_rtm_rw_mutex::scoped_lock& );

    //! Internal release lock.
    void __TBB_EXPORTED_METHOD
    internal_release( x86_rtm_rw_mutex::scoped_lock& );

    static x86_rtm_rw_mutex* internal_get_mutex(
...
private:
    ...
    // The idea is to be able to reuse the acquire/release methods of
    // spin_rw_mutex and its scoped lock wherever possible.  The only way to use
    // a speculative lock is to use a scoped_lock. (because transaction_state
    // must be local)
    ...
    public:
        ...
        scoped_lock()
            : my_scoped_lock(), transaction_state( RTM_not_in_mutex ) {}

        //! Acquire lock on given mutex.
        scoped_lock( x86_rtm_rw_mutex& m, bool write = true )
            : my_scoped_lock(), transaction_state( RTM_not_in_mutex ) {
            acquire( m, write );
        }
        ...
        ~scoped_lock() {
            if ( transaction_state != RTM_not_in_mutex )
                release();
        }
        ...
        //! Release lock
        void release() {
            x86_rtm_rw_mutex* mutex =
                x86_rtm_rw_mutex::internal_get_mutex( my_scoped_lock );
            __TBB_ASSERT( mutex, "lock is not acquired" );
            __TBB_ASSERT( transaction_state != RTM_not_in_mutex,
        ...
        /** Returns whether the downgrade happened without releasing and
         * re-acquiring the lock */
        ...
        bool try_acquire( x86_rtm_rw_mutex& m, bool write = true ) {
            ...
            // cannot set the mutex, because try_acquire in spin_rw_mutex
            // depends on it being NULL.
            if ( write )
                return m.internal_try_acquire_writer( *this );
            // speculatively acquire the lock.  If this fails, do try_acquire on
            // the spin_rw_mutex.
