...
#if !defined( __TBB_show_deprecation_message_recursive_mutex_H ) && \
    defined( __TBB_show_deprecated_header_message )
#define __TBB_show_deprecation_message_recursive_mutex_H
#pragma message( \
    "TBB Warning: tbb/recursive_mutex.h is deprecated. For details, please see Deprecated Features appendix in the TBB reference manual." )
#endif
...
public:
    ...
    recursive_mutex() {
        ...
        if ( error_code )
            tbb::internal::handle_perror(
                error_code, "recursive_mutex: pthread_mutexattr_init failed" );

        pthread_mutexattr_settype( &mtx_attr, PTHREAD_MUTEX_RECURSIVE );
        error_code = pthread_mutex_init( &impl, &mtx_attr );
        if ( error_code )
            tbb::internal::handle_perror(
                error_code, "recursive_mutex: pthread_mutex_init failed" );
    ...
    void lock() {
        ...
        if ( error_code )
            tbb::internal::handle_perror(
                error_code, "recursive_mutex: pthread_mutex_lock failed" );
#endif /* _WIN32||_WIN64 */
#endif /* TBB_USE_ASSERT */
    }
