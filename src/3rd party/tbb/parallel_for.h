...
public:
    ...
    static void run( const Range& range,
        ...
        if ( !range.empty() ) {
            ...
            start_for& a = *new ( task::allocate_root() )
                               start_for( range, body, partitioner );
#else
            ...
            start_for& a = *new ( task::allocate_root( context ) )
                               start_for( range, body, partitioner );
#endif /* __TBB_TASK_GROUP_CONTEXT && !TBB_JOIN_OUTER_TASK_GROUP */
            // REGION BEGIN
            fgt_begin_algorithm( tbb::internal::PARALLEL_FOR_TASK,
                                 ( void* )&context );
    ...
    void run_body( Range& r ) {
        fgt_alg_begin_body( tbb::internal::PARALLEL_FOR_TASK,
...
public:
    ...
    void operator()( const tbb::blocked_range< Index >& r ) const {
        // A set of local variables to help the compiler with vectorization of
        // the following loop.
        Index b = r.begin();
        Index e = r.end();
        Index ms = my_step;
...
/** \name parallel_for
   ...
   parallel_for_body_req "parallel_for Body". **/
//@{
...
void parallel_for( const Range& range, const Body& body ) {
    internal::start_for< Range, Body, const __TBB_DEFAULT_PARTITIONER >::run(
...
void parallel_for( const Range& range,
    ...
    internal::start_for< Range, Body, const simple_partitioner >::run(
...
void parallel_for( const Range& range,
    ...
    internal::start_for< Range, Body, const auto_partitioner >::run(
        range, body, partitioner );
}
...
void parallel_for( const Range& range,
    ...
    internal::start_for< Range, Body, const static_partitioner >::run(
        range, body, partitioner );
}
...
void parallel_for( const Range& range,
    ...
    internal::start_for< Range, Body, affinity_partitioner >::run(
        range, body, partitioner );
}
...
//! Parallel iteration over range with auto_partitioner and user-supplied
//! context.
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_for( const Range& range,
...
//! Parallel iteration over range with static_partitioner and user-supplied
//! context.
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_for( const Range& range,
...
void parallel_for_impl( Index first,
                        Index last,
                        Index step,
                        const Function& f,
                        Partitioner& partitioner ) {
    if ( step <= 0 )
        internal::throw_exception(
            internal::eid_nonpositive_step ); // throws std::invalid_argument
    else if ( last > first ) {
        // Above "else" avoids "potential divide by zero" warning on some
        // platforms
