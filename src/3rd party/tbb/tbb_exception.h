...
enum exception_id {
    ...
    // This id is used only from inside the library and only for support of CPF
    // functionality. So, if we drop the functionality, eid_reserved1 can be
    // safely renamed and reused.
    eid_blocking_thread_join_impossible = eid_reserved1,
#endif
    eid_bad_tagged_msg_cast,
    //! The last enumerator tracks the number of defined IDs. It must remain the
    //! last one.
    /** When adding new IDs, place them immediately _before_ this comment (that
       is _after_ all the existing IDs. NEVER insert new IDs between the
       existing ones. **/
    eid_max
};
...
inline void throw_exception( exception_id eid ) {
...
//! Interface to be implemented by all exceptions TBB recognizes and propagates
//! across the threads.
/** If an unhandled exception of the type derived from tbb::tbb_exception is
   intercepted by the TBB scheduler in one of the worker threads, it is
   delivered to and re-thrown in the root thread. The root thread is the thread
   that has started the outermost algorithm or root task sharing the same
   task_group_context with the guilty algorithm/task (the one that threw the
   exception first).

    Note: when documentation mentions workers with respect to exception
   handling, masters are implied as well, because they are completely equivalent
   in this context. Consequently a root thread can be master or worker thread.

    NOTE: In case of nested algorithms or complex task hierarchies when the
   nested levels share (explicitly or by means of implicit inheritance) the task
   group context of the outermost level, the exception may be (re-)thrown
   multiple times (ultimately - in each worker on each nesting level) before
   reaching the root thread at the outermost level. IMPORTANT: if you intercept
   an exception derived from this class on a nested level, you must re-throw it
   in the catch block by means of the "throw;" operator.

    TBB provides two implementations of this interface: tbb::captured_exception
   and template class tbb::movable_exception. See their declarations for more
   info. **/
...
public:
    ...
    // empty destructor of tbb_exception from destructors of derived classes. As
    // a result, it does not create vtable for tbb_exception, which is a
    // required part of TBB binary interface. Making the destructor non-empty
    // (with just a semicolon) prevents that optimization.
    ...
    /** Make sure that if you have several levels of derivation from this
       interface you implement or override this method on the most derived
       level. The implementation is as simple as "throw *this;". Failure to do
       this will result in exception of a base class type being thrown. **/
    ...
    /** Operator delete is provided only to allow using existing smart pointers
        ...
        When overriding method move() make sure to override operator delete as
       well if memory is allocated not by TBB's scalable allocator. **/
    void operator delete( void* p ) {
        internal::deallocate_via_handler_v3( p );
    }
};

//! This class is used by TBB to propagate information about unhandled
//! exceptions into the root thread.
/** Exception of this type is thrown by TBB in the root thread (thread that
   started a parallel algorithm ) if an unhandled exception was intercepted
   during the algorithm execution in one of the workers.
...
public:
    ...
    void __TBB_EXPORTED_METHOD set( const char* name,
...
private:
    ...
    captured_exception()
        : my_dynamic(), my_exception_name(), my_exception_info() {}
...
//! Template that can be used to implement exception that transfers arbitrary
//! ExceptionData to the root thread
/** Code using TBB can instantiate this template with an arbitrary ExceptionData
   type and throw this exception object. Such exceptions are intercepted by the
   TBB scheduler and delivered to the root thread ().
    \sa tbb::tbb_exception **/
...
public:
    ...
    const char* name() const throw() __TBB_override {
    ...
    movable_exception* move() throw() __TBB_override {
        void* e =
            internal::allocate_via_handler_v3( sizeof( movable_exception ) );
        if ( e ) {
...
private:
    //! Flag specifying whether this object has been dynamically allocated (by
    //! the move method)
    bool my_dynamic;
...
private:
    ...
    tbb_exception_ptr( const captured_exception& src )
        :
          ...
          my_ptr( std::make_exception_ptr(
              src ) ) // the final function name in C++11
#else
