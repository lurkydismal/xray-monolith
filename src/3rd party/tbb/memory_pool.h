...
public:
    ...
    pointer allocate( size_type n, const void* /*hint*/ = 0 ) {
        pointer p = static_cast< pointer >(
            my_pool->malloc( n * sizeof( value_type ) ) );
        if ( !p )
...
void* memory_pool< Alloc >::allocate_request( intptr_t pool_id,
                                              size_t& bytes ) {
    memory_pool< Alloc >& self =
        *reinterpret_cast< memory_pool< Alloc >* >( pool_id );
...
#endif
template < typename Alloc >
int memory_pool< Alloc >::deallocate_request( intptr_t pool_id,
                                              void* raw_ptr,
                                              size_t raw_bytes ) {
    memory_pool< Alloc >& self =
        *reinterpret_cast< memory_pool< Alloc >* >( pool_id );
    ...
    self.my_alloc.deallocate(
        static_cast< typename Alloc::value_type* >( raw_ptr ),
        raw_bytes / unit_size );
    return 0;
}
#if __TBB_MSVC_UNREACHABLE_CODE_IGNORED
...
inline fixed_pool::fixed_pool( void* buf, size_t size )
    ...
    if ( !buf || !size )
        ...
        tbb::internal::throw_exception(
            std::invalid_argument( "Zero in parameter is invalid" ) );
