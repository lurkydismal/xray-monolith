...
#if !defined( __TBB_show_deprecation_message_task_scheduler_init_H ) && \
...
/** A thread can construct a task_scheduler_init object and keep it alive
   ...
   extent. For example it can limit concurrency level of parallel work initiated
   by the given thread. It also can be used to specify stack size of the TBB
   worker threads, though this setting is not effective if the thread pool has
   already been created.

    If a parallel construct is used without task_scheduler_init object
   previously created, the scheduler will be initialized automatically with
   default settings, and will persist until this thread exits. Default
   concurrency level is defined as described in
...
class __TBB_DEPRECATED_VERBOSE task_scheduler_init : internal::no_copy {
    enum ExceptionPropagationMode {
        ...
        propagation_mode_mask =
            propagation_mode_exact | propagation_mode_captured
    };
...
public:
    ...
    //! Argument to initialize() or constructor that causes initialization to be
    //! deferred.
    static const int deferred = -2;

    //! Ensure that scheduler exists for this thread
    /** A value of -1 lets TBB decide on the number of threads, which is usually
        maximal hardware concurrency for this process, that is the number of
       logical CPUs on the machine (possibly limited by the processor affinity
       mask of this process (Windows) or of this thread (Linux, FreeBSD). It is
       preferable option for production code because it helps to avoid nasty
       surprises when several TBB based components run side-by-side or in a
       nested fashion inside the same process.
        ...
        Doing so does no harm because the underlying scheduler is reference
       counted. */
    void __TBB_EXPORTED_METHOD initialize( int number_of_threads = automatic );

    //! The overloaded method with stack size parameter
    /** Overloading is necessary to preserve ABI compatibility */
    void __TBB_EXPORTED_METHOD initialize( int number_of_threads,
    ...
    //! terminate() that waits for worker threads termination. Throws exception
    //! on error.
    void blocking_terminate() {
        internal_blocking_terminate( /*throwing=*/true );
    }
#endif
    //! terminate() that waits for worker threads termination. Returns false on
    //! error.
    ...
    //! Shorthand for default constructor followed by call to
    ...
    task_scheduler_init( int number_of_threads = automatic,
        ...
        // communicate default exception propagation mode of the client to be
        // used when the client manually creates tasks in the master thread and
        // does not use explicit task group context object. This is necessary
        // because newer TBB binaries with exact propagation enabled by default
        // may be used by older clients that expect tbb::captured_exception
        // wrapper. All zeros mean old client - no preference.
        __TBB_ASSERT( !( thread_stack_size & propagation_mode_mask ),
                      "Requested stack size is not aligned" );
#if TBB_USE_EXCEPTIONS
        thread_stack_size |= TBB_USE_CAPTURED_EXCEPTION
    ...
    //! Destroy scheduler for this thread if thread has no other live
    //! task_scheduler_inits.
    ~task_scheduler_init() {
        if ( my_scheduler )
            terminate();
    ...
    /** Result returned by this method does not depend on whether the scheduler
        ...
        Because tbb 2.0 does not support blocking tasks yet, you may use this
       method to boost the number of threads in the tbb's internal pool, if your
       tasks are doing I/O operations. The optimal number of additional threads
       depends on how much time your tasks spend in the blocked state.
        ...
        logical CPUs available to the current process in accordance with its
       affinity mask.
       ...
       took place after this method was first invoked will not affect the number
       of worker threads in the TBB worker threads pool. */
