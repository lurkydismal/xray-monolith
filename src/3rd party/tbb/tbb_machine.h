...
/** This header provides basic platform abstraction layer by hooking up
 appropriate architecture/OS/compiler specific headers from the
 /include/tbb/machine directory. If a plug-in header does not implement all the
 required APIs, it must specify the missing ones by setting one or more of the
 following macros:
    ...
    In this case tbb_machine.h will add missing functionality based on a minimal
 set of APIs that are required to be implemented by all plug-n headers as
 described further. Note that these generic implementations may be sub-optimal
 for a particular architecture, and thus should be relied upon only after
 careful evaluation or as the last resort.

    Additionally __TBB_64BIT_ATOMICS can be set to 0 on a 32-bit architecture to
    indicate that the port is not going to support double word atomics. It may
 also be set to 1 explicitly, though normally this is not necessary as
 tbb_machine.h will set it automatically.
    ...
    and must specify the layout of aligned 16-bit and 32-bit data anywhere
 within a process (while the details of unaligned 16-bit or 32-bit data or of
 64-bit data are irrelevant). The layout must be the same at all relevant memory
 locations within the current process; in case of page-specific endianness, one
 endianness must be kept "out of sight". Possible settings, reflecting hardware
 and possibly O.S. convention, are:
    ...
    -  __TBB_ENDIAN_UNSUPPORTED to prevent undefined behavior if none of the
 above.

    Prerequisites for each architecture port
    ----------------------------------------
    The following functions and macros have no generic implementation. Therefore
 they must be implemented in each machine architecture specific header either as
 a conventional function or as a functional macro.
    ...
    __TBB_Yield()
        Signals OS that the current thread is willing to relinquish the
 remainder of its time quantum.
 ...
 ) Must be provided for 64-bit architectures if __TBB_USE_FENCED_ATOMICS is not
 set, and for 32-bit architectures if __TBB_64BIT_ATOMICS is set
    ...
    __TBB_control_consistency_helper()
        ...
        either specifically (like IA-64 architecture) or because generally
 stronger "acquire" semantics are enforced (like x86). It is always valid,
 though potentially suboptimal, to replace control with acquire on the load and
 ...
 fenced store and load operations. Depending on the particular
 architecture/compiler combination they may be a hardware fence, a compiler
 fence, both or nothing.
 **/
...
// request default implementation by setting appropriate __TBB_USE_GENERIC_XXX
// macro(s).
//
...
#define __TBB_MACHINE_DEFINE_STORE8_GENERIC_FENCED( M )                     \
    inline void __TBB_machine_generic_store8##M( volatile void* ptr,        \
                                                 int64_t value ) {          \
        for ( ;; ) {                                                        \
            int64_t result = *( volatile int64_t* )ptr;                     \
            if ( __TBB_machine_cmpswp8##M( ptr, value, result ) == result ) \
                break;                                                      \
        }                                                                   \
    }

#define __TBB_MACHINE_DEFINE_LOAD8_GENERIC_FENCED( M )                        \
    inline int64_t __TBB_machine_generic_load8##M(                            \
        ...
        return __TBB_machine_cmpswp8##M( const_cast< volatile void* >( ptr ), \
                                         anyvalue, anyvalue );                \
    }
...
// TODO: replace usage of these functions with usage of tbb::atomic, and then
// remove them
// TODO: map functions with W suffix to use cast to tbb::atomic and according
// op, i.e. as_atomic().op()
...
#define __TBB_FetchAndIncrementWacquire( P ) \
    __TBB_machine_fetchadd8acquire( P, 1 )
#define __TBB_FetchAndDecrementWrelease( P ) \
    __TBB_machine_fetchadd8release( P, ( -1 ) )
#else
...
#define __TBB_FetchAndIncrementWacquire( P ) \
    __TBB_machine_fetchadd4acquire( P, 1 )
#define __TBB_FetchAndDecrementWrelease( P ) \
    __TBB_machine_fetchadd4release( P, ( -1 ) )
...
#if __TBB_USE_GENERIC_DWORD_LOAD_STORE || __TBB_USE_GENERIC_DWORD_FETCH_ADD || \
    __TBB_USE_GENERIC_DWORD_FETCH_STORE
...
#define __TBB_ENDIANNESS __TBB_ENDIAN_DETECT
    ...
    __TBB_ENDIANNESS == __TBB_ENDIAN_UNSUPPORTED
...
//  - T must be an integer type of at most 4 bytes for the casts and
//  calculations to work
//  - T must also be less than 4 bytes to avoid compiler warnings when computing
//  mask
...
template < typename T >
inline T __TBB_MaskedCompareAndSwap( volatile T* const ptr,
                                     const T value,
                                     const T comparand ) {
    struct endianness {
    ...
    __TBB_ENDIANNESS == __TBB_ENDIAN_LITTLE
...
#error Unexpected value of __TBB_ENDIANNESS
    ...
    volatile uint32_t* const aligned_ptr =
        ( uint32_t* )( ( uintptr_t )ptr - byte_offset );

    // location of T within uint32_t for a C++ shift operation
    const uint32_t bits_to_shift =
        8 * ( endianness::is_big_endian()
                  ? ( 4 - sizeof( T ) - ( byte_offset ) )
                  : byte_offset );
    const uint32_t mask = ( ( ( uint32_t )1 << ( sizeof( T ) * 8 ) ) - 1 )
                          << bits_to_shift;
    // for signed T, any sign extension bits in cast value/comparand are
    // immediately clipped by mask
    const uint32_t shifted_comparand =
        ( ( uint32_t )comparand << bits_to_shift ) & mask;
    const uint32_t shifted_value =
        ( ( uint32_t )value << bits_to_shift ) & mask;

    for ( atomic_backoff b;; b.pause() ) {
        const uint32_t surroundings =
            *aligned_ptr & ~mask; // may have changed during the pause
        ...
        const uint32_t big_result = ( uint32_t )__TBB_machine_cmpswp4(
            aligned_ptr, big_value, big_comparand );
        if ( big_result == big_comparand // CAS succeeded
             || ( ( big_result ^ big_comparand ) & mask ) !=
                    0 ) // CAS failed and the bits of interest have changed
        {
            return T( ( big_result & mask ) >> bits_to_shift );
        } else
            continue; // CAS failed but the bits of interest were not changed
    }
}
...
inline T __TBB_CompareAndSwapGeneric( volatile void* ptr,
                                      T value,
                                      T comparand );

template <>
inline int8_t __TBB_CompareAndSwapGeneric< 1, int8_t >( volatile void* ptr,
                                                        int8_t value,
                                                        int8_t comparand ) {
#if __TBB_USE_GENERIC_PART_WORD_CAS
    return __TBB_MaskedCompareAndSwap< int8_t >( ( volatile int8_t* )ptr, value,
                                                 comparand );
#else
...
template <>
inline int16_t __TBB_CompareAndSwapGeneric< 2, int16_t >( volatile void* ptr,
                                                          int16_t value,
                                                          int16_t comparand ) {
#if __TBB_USE_GENERIC_PART_WORD_CAS
    return __TBB_MaskedCompareAndSwap< int16_t >( ( volatile int16_t* )ptr,
                                                  value, comparand );
#else
...
template <>
inline int32_t __TBB_CompareAndSwapGeneric< 4, int32_t >( volatile void* ptr,
                                                          int32_t value,
                                                          int32_t comparand ) {
    // Cast shuts up /Wp64 warning
...
template <>
inline int64_t __TBB_CompareAndSwapGeneric< 8, int64_t >( volatile void* ptr,
                                                          int64_t value,
                                                          int64_t comparand ) {
...
inline T __TBB_FetchAndAddGeneric( volatile void* ptr, T addend ) {
    ...
    for ( atomic_backoff b;; b.pause() ) {
        ...
        if ( __TBB_CompareAndSwapGeneric< S, T >( ptr, result + addend,
                                                  result ) == result )
            break;
    }
    return result;
...
inline T __TBB_FetchAndStoreGeneric( volatile void* ptr, T value ) {
    ...
    for ( atomic_backoff b;; b.pause() ) {
        result = *reinterpret_cast< volatile T* >( ptr );
        // __TBB_CompareAndSwapGeneric presumed to have full fence.
        if ( __TBB_CompareAndSwapGeneric< S, T >( ptr, value, result ) ==
             result )
            break;
    }
    return result;
...
#if __TBB_USE_GENERIC_PART_WORD_CAS
#define __TBB_machine_cmpswp1 \
    tbb::internal::__TBB_CompareAndSwapGeneric< 1, int8_t >
#define __TBB_machine_cmpswp2 \
    tbb::internal::__TBB_CompareAndSwapGeneric< 2, int16_t >
#endif

#if __TBB_USE_GENERIC_FETCH_ADD || __TBB_USE_GENERIC_PART_WORD_FETCH_ADD
#define __TBB_machine_fetchadd1 \
    tbb::internal::__TBB_FetchAndAddGeneric< 1, int8_t >
#define __TBB_machine_fetchadd2 \
    tbb::internal::__TBB_FetchAndAddGeneric< 2, int16_t >
#endif

#if __TBB_USE_GENERIC_FETCH_ADD
#define __TBB_machine_fetchadd4 \
    tbb::internal::__TBB_FetchAndAddGeneric< 4, int32_t >
#endif

#if __TBB_USE_GENERIC_FETCH_ADD || __TBB_USE_GENERIC_DWORD_FETCH_ADD
#define __TBB_machine_fetchadd8 \
    tbb::internal::__TBB_FetchAndAddGeneric< 8, int64_t >
#endif

#if __TBB_USE_GENERIC_FETCH_STORE || __TBB_USE_GENERIC_PART_WORD_FETCH_STORE
#define __TBB_machine_fetchstore1 \
    tbb::internal::__TBB_FetchAndStoreGeneric< 1, int8_t >
#define __TBB_machine_fetchstore2 \
    tbb::internal::__TBB_FetchAndStoreGeneric< 2, int16_t >
#endif

#if __TBB_USE_GENERIC_FETCH_STORE
#define __TBB_machine_fetchstore4 \
    tbb::internal::__TBB_FetchAndStoreGeneric< 4, int32_t >
#endif

#if __TBB_USE_GENERIC_FETCH_STORE || __TBB_USE_GENERIC_DWORD_FETCH_STORE
#define __TBB_machine_fetchstore8 \
    tbb::internal::__TBB_FetchAndStoreGeneric< 8, int64_t >
#endif

#if __TBB_USE_FETCHSTORE_AS_FULL_FENCED_STORE
#define __TBB_MACHINE_DEFINE_ATOMIC_SELECTOR_FETCH_STORE( S )     \
    atomic_selector< S >::word atomic_selector< S >::fetch_store( \
...
#define __TBB_machine_store8 \
    tbb::internal::__TBB_machine_generic_store8full_fence
...
/** Fenced operations use volatile qualifier to prevent compiler from optimizing
    ...
    no effect on code gen, and consistency helpers serve as a compiler fence
   (the latter being true for IA64/gcc as well to fix a bug in some gcc
   versions). This code assumes that the generated instructions will operate
   atomically, which typically requires a type that can be moved in a single
   instruction, cooperation from the compiler for effective use of such an
   instruction, and appropriate alignment of the data. **/
...
// in general, plain load and store of 32bit compiler is not atomic for 64bit
// types
...
#endif
        while ( __TBB_machine_cmpswp8( ( volatile void* )&location,
                                       ( int64_t )value, result ) != result )
...
// Relaxed operations add volatile qualifier to prevent compiler from optimizing
// them out.
/** Volatile should not incur any additional cost on IA32, Intel64, and Sparc
   TSO architectures. However on architectures with weak memory ordering
   compiler may generate code with acquire/release semantics for operations on
   volatile data. **/
template < typename T, size_t S >
struct machine_load_store_relaxed {
...
#undef __TBB_WORDSIZE // this macro is forbidden to use outside of atomic
                      // machinery
...
inline void __TBB_store_with_release( volatile size_t& location,
                                      size_t value ) {
    machine_load_store< size_t, sizeof( size_t ) >::store_with_release(
        location, value );
}
...
inline void __TBB_store_relaxed( volatile size_t& location, size_t value ) {
    machine_load_store_relaxed< size_t, sizeof( size_t ) >::store(
        const_cast< size_t& >( location ), value );
}

// Macro __TBB_TypeWithAlignmentAtLeastAsStrict(T) should be a type with
// alignment at least as strict as type T.  The type should have a trivial
// default constructor and destructor, so that arrays of that type can be
// declared without initializers. It is correct (but perhaps a waste of space)
// if __TBB_TypeWithAlignmentAtLeastAsStrict(T) expands to a type bigger than T.
// The default definition here works on machines where integers are naturally
// aligned and the strictest alignment is 64.
...
#define __TBB_DefineTypeWithAlignment( PowerOf2 )                          \
    __declspec( align(                                                     \
        PowerOf2 ) ) struct __TBB_machine_type_with_alignment_##PowerOf2 { \
        uint32_t member[ PowerOf2 / sizeof( uint32_t ) ];                  \
    };
...
__TBB_DefineTypeWithAlignment(
    8 ) // i386 ABI says that uint64_t is aligned on 4 bytes
    __TBB_DefineTypeWithAlignment( 16 ) __TBB_DefineTypeWithAlignment( 32 )
        __TBB_DefineTypeWithAlignment( 64 )

            typedef __TBB_machine_type_with_alignment_64
    __TBB_machine_type_with_strictest_alignment;

// Primary template is a declaration of incomplete type so that it fails with
// unknown alignments
template < size_t N >
...
/** Bug is that compiler sometimes returns 0 for __alignof(T) when T has not yet
   been instantiated. The work-around forces instantiation by forcing
   computation of sizeof(T) before __alignof(T). */
...
#else
#define __TBB_TypeWithAlignmentAtLeastAsStrict( T ) \
    tbb::internal::type_with_alignment< __TBB_alignof( T ) >
#endif /* __TBB_ALIGNOF_NOT_INSTANTIATED_TYPES_BROKEN */

#endif /* __TBB_TypeWithAlignmentAtLeastAsStrict */

// Template class here is to avoid instantiation of the static data for modules
// that don't use it
template < typename T >
struct reverse {
...
// Mapping historically used names to the ones expected by
...
#if !defined( _M_ARM )
    uintptr_t tmp_;
    if ( sizeof( x ) > 4 && ( tmp_ = ( ( uint64_t )x ) >> 32 ) ) {
        x = tmp_;
        result += 32;
    }
#endif
...
// lock primitives with Intel(R) Transactional Synchronization Extensions
// (Intel(R) TSX)
#if ( __TBB_x86_32 || __TBB_x86_64 ) /* only on ia32/intel64 */
inline void __TBB_TryLockByteElidedCancel() {
