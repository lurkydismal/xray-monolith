...
public:
    ...
    concurrent_priority_queue( std::initializer_list< T > init_list,
                               const allocator_type& a = allocator_type() )
    ...
    //! Copy constructor
    /** This operation is unsafe if there are pending concurrent operations on
     * the src queue. */
    concurrent_priority_queue( const concurrent_priority_queue& src )
        : mark( src.mark ),
          my_size( src.my_size ),
          data( src.data.begin(), src.data.end(), src.data.get_allocator() ) {
        my_aggregator.initialize_handler( my_functor_t( this ) );
        heapify();
    }
    ...
    concurrent_priority_queue( const concurrent_priority_queue& src,
        ...
        : mark( src.mark ),
          my_size( src.my_size ),
          data( src.data.begin(), src.data.end(), a ) {
        my_aggregator.initialize_handler( my_functor_t( this ) );
        heapify();
    }
    ...
    concurrent_priority_queue& operator=(
        const concurrent_priority_queue& src ) {
        if ( this != &src ) {
            vector_t( src.data.begin(), src.data.end(),
                      src.data.get_allocator() )
                .swap( data );
            mark = src.mark;
            my_size = src.my_size;
        }
...
#else
          // Some early version of C++11 STL vector does not have a constructor
          // of vector(vector&& , allocator). It seems that the reason is
          // absence of support of allocator_traits (stateful allocators).
          data( a )
#endif //__TBB_ALLOCATOR_TRAITS_PRESENT
    {
        ...
        if ( a != src.data.get_allocator() ) {
            ...
            data.assign( std::make_move_iterator( src.data.begin() ),
    ...
    concurrent_priority_queue& operator=( concurrent_priority_queue&& src ) {
        if ( this != &src ) {
            ...
            if ( data.get_allocator() != src.data.get_allocator() ) {
                vector_t( std::make_move_iterator( src.data.begin() ),
                          std::make_move_iterator( src.data.end() ),
                          data.get_allocator() )
                    .swap( data );
            } else
    ...
    /** This operation can be safely used concurrently with other push, try_pop
     * or emplace operations. */
    void push( const_reference elem ) {
#if __TBB_CPP11_IS_COPY_CONSTRUCTIBLE_PRESENT
        __TBB_STATIC_ASSERT( std::is_copy_constructible< value_type >::value,
                             "The type is not copy constructible. Copying push "
    ...
    //! Constructs a new element using args as the arguments for its
    //! construction and pushes it onto the queue */
    /** This operation can be safely used concurrently with other push, try_pop
     * or emplace operations. */
    ...
    /** This operation is unsafe if there are pending concurrent operations on
       the queue. Resets size, effectively emptying queue; does not free space.
...
private:
    ...
    typedef tbb::internal::aggregator< my_functor_t, cpq_operation >
        aggregator_t;
    ...
    Compare compare;
    //! Padding added to avoid false sharing
    char padding2[ NFS_MaxLineSize - ( 2 * sizeof( size_type ) ) -
                   sizeof( Compare ) ];
    ...
    void handle_operations( cpq_operation* op_list ) {
        ...
        // First pass processes all constant (amortized; reallocation may
        // happen) time pushes and pops.
        while ( op_list ) {
            // ITT note: &(op_list->status) tag is used to cover accesses to
            // op_list node. This thread is going to handle the operation, and
            // so will acquire it and perform the associated operation w/o
            // triggering a race condition; the thread that created the
            // operation is waiting on the status field, so when this thread is
            // done with the operation, it will perform a store_with_release to
            // give control back to the waiting thread in
            ...
            if ( tmp->type == POP_OP ) {
                if ( mark < data.size() &&
                    ...
                    // is higher than top
                    *( tmp->elem ) =
                        tbb::internal::move( data[ data.size() - 1 ] );
                    __TBB_store_with_release( my_size, my_size - 1 );
                    itt_store_word_with_release( tmp->status,
                                                 uintptr_t( SUCCEEDED ) );
                    data.pop_back();
                    __TBB_ASSERT( mark <= data.size(), NULL );
                ...
                __TBB_ASSERT(
                    tmp->type == PUSH_OP || tmp->type == PUSH_RVALUE_OP,
                    "Unknown operation" );
                __TBB_TRY {
                    if ( tmp->type == PUSH_OP ) {
                        push_back_helper(
                            *( tmp->elem ),
                            typename internal::use_element_copy_constructor<
                                value_type >::type() );
                    } else {
    ...
    void heapify() {
        ...
        for ( ; mark < data.size(); ++mark ) {
            ...
            do { // push to_place up the heap
                ...
                if ( !compare( data[ parent ], to_place ) )
                    break;
    ...
    void reheap() {
        ...
        while ( child < mark ) {
            ...
            if ( child + 1 < mark &&
                 compare( data[ child ], data[ child + 1 ] ) )
                ++target;
            // target now has the higher priority child
            if ( compare( data[ target ], data[ data.size() - 1 ] ) )
                break;
