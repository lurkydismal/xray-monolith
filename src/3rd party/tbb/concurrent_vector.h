...
static void* const vector_allocation_error_flag =
    reinterpret_cast< void* >( size_t( 63 ) );

//! Exception helper function
template < typename T >
void handle_unconstructed_elements( T* array, size_t n_of_elements ) {
    std::memset( static_cast< void* >( array ), 0,
                 n_of_elements * sizeof( T ) );
}
...
protected:
    ...
    public:
        ...
        template < typename T >
        T* pointer() const {
            return static_cast< T* >( const_cast< void* >( array ) );
        }
    };

    friend void enforce_segment_allocated(
        segment_value_t const& s,
    ...
    public:
        ...
        void store( segment_allocation_failed ) {
            __TBB_ASSERT( load< relaxed >() != segment_allocated(),
                          "transition from \"allocated\" to \"allocation "
                          "failed\" state looks non-logical" );
        ...
        void store( void* allocated_segment_pointer ) __TBB_NOEXCEPT( true ) {
            __TBB_ASSERT( segment_value_t( allocated_segment_pointer ) ==
                              segment_allocated(),
                          "other overloads of store should be used for marking "
                          "segment as not_used or allocation_failed" );
    ...
    concurrent_vector_base_v3() {
        ...
        // Object that is in middle of construction (i.e. its constructor is not
        // yet finished) cannot be used concurrently until the construction is
        // finished. Thus to flag other threads that construction is finished,
        // some synchronization with acquire-release semantic should be done by
        // the (external) code that uses the vector. So, no need to do the
        // synchronization inside the vector.

        my_early_size.store< relaxed >( 0 );
        my_first_block.store< relaxed >(
            0 ); // here is not default_initial_segments
    ...
    static inline segment_index_t segment_base_index_of(
    ...
    typedef void( __TBB_EXPORTED_FUNC* internal_array_op2 )( void* dst,
                                                             const void* src,
                                                             size_type n );
    ...
    void __TBB_EXPORTED_METHOD internal_reserve( size_type n,
                                                 size_type element_size,
                                                 size_type max_size );
    size_type __TBB_EXPORTED_METHOD internal_capacity() const;
    void internal_grow( size_type start,
                        size_type finish,
                        size_type element_size,
    ...
    size_type __TBB_EXPORTED_METHOD internal_grow_by( size_type delta,
                                                      ...
                                                      internal_array_op2 init,
                                                      const void* src );
    void* __TBB_EXPORTED_METHOD internal_push_back( size_type element_size,
                                                    size_type& index );
    segment_index_t __TBB_EXPORTED_METHOD
    internal_clear( internal_array_op1 destroy );
    void* __TBB_EXPORTED_METHOD internal_compact( size_type element_size,
                                                  void* table,
                                                  internal_array_op1 destroy,
                                                  internal_array_op2 copy );
    void __TBB_EXPORTED_METHOD
    internal_copy( const concurrent_vector_base_v3& src,
                   size_type element_size,
                   internal_array_op2 copy );
    void __TBB_EXPORTED_METHOD
    internal_assign( const concurrent_vector_base_v3& src,
                     size_type element_size,
                     internal_array_op1 destroy,
    ...
    void __TBB_EXPORTED_METHOD internal_resize( size_type n,
                                                size_type element_size,
                                                size_type max_size,
                                                const void* src,
                                                internal_array_op1 destroy,
                                                internal_array_op2 init );
    size_type __TBB_EXPORTED_METHOD
    internal_grow_to_at_least_with_result( size_type new_size,
                                           size_type element_size,
                                           internal_array_op2 init,
                                           const void* src );
    ...
    void __TBB_EXPORTED_METHOD
    internal_grow_to_at_least( size_type new_size,
                               size_type element_size,
                               internal_array_op2 init,
                               const void* src );
...
inline void swap( concurrent_vector_base_v3::segment_t& lhs,
...
class vector_iterator {
    ...
    friend vector_iterator< C, T > operator+(
        ptrdiff_t offset,
        const vector_iterator< C, T >& v );

    template < typename C, typename T, typename U >
    friend bool operator==( const vector_iterator< C, T >& i,
    ...
    template < typename C, typename T, typename U >
    friend ptrdiff_t operator-( const vector_iterator< C, T >& i,
                                const vector_iterator< C, U >& j );
...
public:
    ...
    vector_iterator(
        const vector_iterator< Container, typename Container::value_type >&
            other )
    ...
    vector_iterator& operator++() {
        ...
        if ( my_item ) {
            // TODO: consider using of knowledge about "first_block
            // optimization" here as well?
            if ( concurrent_vector_base::is_first_element_in_segment(
                     element_index ) ) {
                // if the iterator crosses a segment boundary, the pointer
                // become invalid as possibly next segment is in another memory
                // location
                my_item = NULL;
            } else {
                ++my_item;
    ...
    //! Pre decrement
    vector_iterator& operator--() {
        __TBB_ASSERT( my_index > 0,
                      "operator--() applied to iterator already at beginning "
...
vector_iterator< Container, T > operator+(
    ptrdiff_t offset,
...
bool operator==( const vector_iterator< Container, T >& i,
...
template < typename Container, typename T, typename U >
ptrdiff_t operator-( const vector_iterator< Container, T >& i,
                     const vector_iterator< Container, U >& j ) {
...
/** concurrent_vector is a container having the following main properties:
    - It provides random indexed access to its elements. The index of the first
element is 0.
    - It ensures safe concurrent growing its size (different threads can safely
append new elements).
    - Adding new elements does not invalidate existing iterators and does not
change indices of existing items.

@par Compatibility
    The class meets all Container Requirements and Reversible Container
Requirements from C++ Standard (See ISO/IEC 14882:2003(E), clause 23.1). But it
doesn't meet Sequence Requirements due to absence of insert() and erase()
methods.

@par Exception Safety
    Methods working with memory allocation and/or new elements construction can
throw an exception if allocator fails to allocate memory or element's default
constructor throws one. Concurrent vector's element of type T must conform to
the following requirements:
    - Throwing an exception is forbidden for destructor of T.
    - Default constructor of T must not throw an exception OR its non-virtual
destructor must safely work when its object memory is zero-initialized.
    .
    Otherwise, the program's behavior is undefined.
@par
    If an exception happens inside growth or assignment operation, an instance
of the vector becomes invalid unless it is stated otherwise in the method
documentation. Invalid state means:
    - There are no guarantees that all items were initialized by a constructor.
The rest of items is zero-filled, including item where exception happens.
    - An invalid vector instance cannot be repaired; it is unable to grow
anymore.
    - Size and capacity reported by the vector are incorrect, and calculated as
if the failed operation were successful.
    - Attempt to access not allocated elements using operator[] or iterators
results in access violation or segmentation fault exception, and in case of
using at() method a C++ exception is thrown.
    .
    If a concurrent grow operation successfully completes, all the elements it
has added to the vector will remain valid and accessible even if one of
subsequent grow operations fails.

@par Fragmentation
    Unlike an STL vector, a concurrent_vector does not move existing elements if
it needs to allocate more memory. The container is divided into a series of
contiguous arrays of elements. The first reservation, growth, or assignment
operation determines the size of the first array. Using small number of elements
as initial size incurs fragmentation that may increase element access time.
Internal layout can be optimized by method compact() that merges several smaller
arrays into one solid.
...
methods to assure elements are allocated.
    - Methods end()/rbegin()/back() are partly thread-safe since they use size()
to get the end of vector
...
@par Changes since TBB 2.0
    ...
    - First growth call specifies a number of segments to be merged in the first
allocation.
    - Fixed memory blow up for swarm of vector's instances of small size
    - Added grow_by(size_type n, const_reference t) growth using copying
constructor to init new items.
    - Added STL-like constructors.
    - Added operators ==, < and derivatives
    - Added at() method, approved for using after an exception was thrown inside
the vector
...
public:
    ...
    typedef typename internal::allocator_base< T, A >::allocator_type
        allocator_type;
    ...
    typedef internal::vector_iterator< concurrent_vector, const T >
        const_iterator;
    ...
    typedef std::reverse_iterator< const_iterator, T, const T&, const T* >
    ...
    explicit concurrent_vector( const allocator_type& a = allocator_type() )
        : internal::allocator_base< T, A >( a ),
          internal::concurrent_vector_base() {
    ...
    concurrent_vector( std::initializer_list< T > init_list,
        ...
        __TBB_CATCH( ... ) {
            ...
            ;
            internal_free_segments( table, internal_clear( &destroy_array ),
                                    my_first_block.load< relaxed >() );
            __TBB_RETHROW();
        }
    }
    ...
    //! Move constructor
    // TODO add __TBB_NOEXCEPT(true) and
    // static_assert(std::has_nothrow_move_constructor<A>::value)
    concurrent_vector( concurrent_vector&& source )
        : internal::allocator_base< T, A >( std::move( source ) ),
          internal::concurrent_vector_base() {
        vector_allocator_ptr = &internal_allocator;
    ...
    concurrent_vector( concurrent_vector&& source, const allocator_type& a )
        : internal::allocator_base< T, A >( a ),
          internal::concurrent_vector_base() {
        vector_allocator_ptr = &internal_allocator;
        // C++ standard requires instances of an allocator being compared for
        // equality, which means that memory allocated by one instance is
        // possible to deallocate with the other one.
    ...
    concurrent_vector( const concurrent_vector< T, M >& vector,
        ...
        vector_allocator_ptr = &internal_allocator;
        __TBB_TRY {
            internal_copy( vector.internal_vector_base(), sizeof( T ),
                           &copy_array );
        }
        __TBB_CATCH( ... ) {
            segment_t* table = my_segment.load< relaxed >();
    ...
    //! Construction with initial size specified by argument n, initialization
    //! by copying of t, and given allocator instance
    concurrent_vector( size_type n,
        ...
        __TBB_TRY {
            internal_resize( n, sizeof( T ), max_size(),
                             static_cast< const void* >( &t ), &destroy_array,
                             &initialize_array_by );
        }
        __TBB_CATCH( ... ) {
            segment_t* table = my_segment.load< relaxed >();
    ...
    concurrent_vector( I first,
        ...
        __TBB_TRY {
            internal_assign_range(
                first, last,
                static_cast<
                    is_integer_tag< std::numeric_limits< I >::is_integer >* >(
                    0 ) );
        }
        __TBB_CATCH( ... ) {
            segment_t* table = my_segment.load< relaxed >();
    ...
    concurrent_vector& operator=( const concurrent_vector& vector ) {
        if ( this != &vector )
            internal_assign( vector, sizeof( T ), &destroy_array, &assign_array,
                             &copy_array );
        return *this;
    }
    ...
    concurrent_vector& operator=( concurrent_vector&& other ) {
        __TBB_ASSERT( this != &other,
                      "Move assignment to itself is prohibited " );
        typedef typename tbb::internal::allocator_traits<
            A >::propagate_on_container_move_assignment pocma_t;
        if ( pocma_t::value || this->my_allocator == other.my_allocator ) {
            ...
            tbb::internal::allocator_move_assignment(
                this->my_allocator, other.my_allocator, pocma_t() );
        } else {
            internal_assign( other, sizeof( T ), &destroy_array,
                             &move_assign_array, &move_array );
        }
        return *this;
    }
#endif
    // TODO: add an template assignment operator? (i.e. with different element
    // type)
    ...
    concurrent_vector& operator=( const concurrent_vector< T, M >& vector ) {
        if ( static_cast< void* >( this ) !=
            ...
            internal_assign( vector.internal_vector_base(), sizeof( T ),
                             &destroy_array, &assign_array, &copy_array );
        return *this;
    }
    ...
    iterator grow_by( size_type delta ) {
        return iterator( *this,
                         delta ? internal_grow_by( delta, sizeof( T ),
                                                   &initialize_array, NULL )
                               : my_early_size.load() );
    }
    ...
    iterator grow_by( size_type delta, const_reference t ) {
        return iterator(
            *this,
            delta ? internal_grow_by( delta, sizeof( T ), &initialize_array_by,
    ...
    iterator grow_by( I first, I last ) {
        typename std::iterator_traits< I >::difference_type delta =
            std::distance( first, last );
        __TBB_ASSERT( delta >= 0, NULL );

        return iterator(
            *this,
            delta ? internal_grow_by( delta, sizeof( T ), &copy_range< I >,
    ...
    /** The new elements are default constructed.  Blocks until all elements in
       range [0..n) are allocated. May return while other elements are being
       constructed by other threads. Returns iterator that points to beginning
       of appended sequence. If no elements were appended, returns iterator
       pointing to nth element. */
    iterator grow_to_at_least( size_type n ) {
        size_type m = 0;
        if ( n ) {
            m = internal_grow_to_at_least_with_result(
                n, sizeof( T ), &initialize_array, NULL );
            if ( m > n )
                m = n;
        }
    ...
    /** Analogous to grow_to_at_least( size_type n ) with exception that the new
        elements are initialized by copying of t instead of default
       construction. */
    iterator grow_to_at_least( size_type n, const_reference t ) {
        size_type m = 0;
        if ( n ) {
            m = internal_grow_to_at_least_with_result(
                n, sizeof( T ), &initialize_array_by, &t );
            if ( m > n )
                m = n;
        }
        return iterator( *this, m );
    ...
    iterator emplace_back( Args&&... args ) {
        push_back_helper prolog( *this );
        new ( prolog.internal_push_back_result() )
    ...
    /** This method is thread-safe for concurrent reads, and also while growing
       the vector, as long as the calling thread has checked that index <
       size(). */
    ...
    //! Return false if vector is not empty or has elements under construction
    //! at least.
    bool empty() const { return !my_early_size; }

    //! Maximum size to which array can grow without allocating more memory.
    //! Concurrent allocations are not included in the value.
    size_type capacity() const { return internal_capacity(); }

    //! Allocate enough space to grow to size n without having to allocate more
    //! memory later.
    /** Like most of the methods provided for STL compatibility, this method is
       *not* thread safe. The capacity afterwards may be bigger than the
    ...
    reference front() {
        ...
        const segment_value_t& segment_value =
    ...
    void swap( concurrent_vector& vector ) {
        typedef typename tbb::internal::allocator_traits<
            A >::propagate_on_container_swap pocs_t;
        if ( this != &vector &&
             ( this->my_allocator == vector.my_allocator || pocs_t::value ) ) {
            concurrent_vector_base_v3::internal_swap(
                static_cast< concurrent_vector_base_v3& >( vector ) );
            tbb::internal::allocator_swap( this->my_allocator,
                                           vector.my_allocator, pocs_t() );
        }
    }

    //! Clear container while keeping memory allocated.
    /** To free up the memory, use in conjunction with method compact(). Not
     * thread safe **/
    void clear() { internal_clear( &destroy_array ); }
...
private:
    ...
    void internal_assign_n( size_type n, const_pointer p ) {
        internal_resize( n, sizeof( T ), max_size(),
                         ...
                         p ? &initialize_array_by : &initialize_array );
    }
    ...
    //! assign integer items by copying when arguments are treated as iterators.
    //! See C++ Standard 2003 23.1.1p9
    template < class I >
    ...
    // these functions are marked __TBB_EXPORTED_FUNC as they are called from
    // within the library

    //! Construct n instances of T, starting at "begin".
    static void __TBB_EXPORTED_FUNC initialize_array( void* begin,
                                                      const void*,
                                                      size_type n );

    //! Copy-construct n instances of T, starting at "begin".
    static void __TBB_EXPORTED_FUNC initialize_array_by( void* begin,
                                                         const void* src,
                                                         size_type n );

    //! Copy-construct n instances of T by copying single element pointed to by
    //! src, starting at "dst".
    static void __TBB_EXPORTED_FUNC copy_array( void* dst,
                                                const void* src,
                                                size_type n );
    ...
    static void __TBB_EXPORTED_FUNC move_array_if_noexcept( void* dst,
                                                            const void* src,
                                                            size_type n );
#endif //__TBB_MOVE_IF_NO_EXCEPT_PRESENT

#if __TBB_CPP11_RVALUE_REF_PRESENT
    //! Move-construct n instances of T, starting at "dst" by copying according
    //! element of src array.
    static void __TBB_EXPORTED_FUNC move_array( void* dst,
                                                const void* src,
                                                size_type n );

    //! Move-assign (using operator=) n instances of T, starting at "dst" by
    //! assigning according element of src array.
    static void __TBB_EXPORTED_FUNC move_assign_array( void* dst,
                                                       const void* src,
                                                       size_type n );
#endif
    //! Copy-construct n instances of T, starting at "dst" by iterator range of
    //! [p_type_erased_iterator, p_type_erased_iterator+n).
    template < typename Iterator >
    static void __TBB_EXPORTED_FUNC
    copy_range( void* dst, const void* p_type_erased_iterator, size_type n );

    //! Assign (using operator=) n instances of T, starting at "dst" by
    //! assigning according element of src array.
    static void __TBB_EXPORTED_FUNC assign_array( void* dst,
                                                  const void* src,
                                                  size_type n );
    ...
    //! Exception-aware helper class for filling a segment by exception-danger
    //! operators of user class
...
#endif
#if __TBB_MOVE_IF_NOEXCEPT_PRESENT
        void move_construct_if_noexcept( const void* src ) {
            for ( ; i < n; ++i )
                new ( &array[ i ] )
                    T( std::move_if_noexcept( as_pointer( src )[ i ] ) );
        }
...
concurrent_vector( I, I, const A& = A() ) -> concurrent_vector< T, A >;
...
#pragma warning( push )
#pragma warning( disable \
                 : 4701 ) // potentially uninitialized local variable "old"
#endif
template < typename T, class A >
...
#endif
            ;
        if ( internal_compact( sizeof( T ), &old, &destroy_array,
                               copy_or_move_array ) )
            internal_free_segments(
                old.table, pointers_per_long_table,
                old.first_block ); // free joined and unnecessary segments
    }
    __TBB_CATCH( ... ) {
        if ( old.first_block ) // free segment allocated for compacting. Only
                               // for support of exceptions in ctor of user
                               // T[ype]
...
template < typename T, class A >
void concurrent_vector< T, A >::internal_free_segments(
    segment_t table[],
    segment_index_t k,
    segment_index_t first_block ) {
    // Free the arrays
    while ( k > first_block ) {
        ...
        if ( segment_value ==
             segment_allocated() ) // check for correct segment pointer
            this->my_allocator.deallocate( ( segment_value.pointer< T >() ),
                                           segment_size( k ) );
    }
...
T& concurrent_vector< T, A >::internal_subscript( size_type index ) const {
    ...
    __TBB_ASSERT( my_segment.load< acquire >() != my_storage ||
                      k < pointers_per_short_table,
                  "index is being allocated" );
    // no need in load with acquire (load<acquire>) since thread works in own
    // space or gets the information about added elements via some form of
    // external synchronization
    // TODO: why not make a load of my_segment relaxed as well ?
    // TODO: add an assertion that my_segment[k] is properly aligned to please
    // ITT
    segment_value_t segment_value = my_segment[ k ].template load< relaxed >();
    __TBB_ASSERT(
        segment_value != segment_allocation_failed(),
        "the instance is broken by bad allocation. Use at() instead" );
    __TBB_ASSERT( segment_value != segment_not_used(),
                  "index is being allocated" );
    return ( ( segment_value.pointer< T >() ) )[ j ];
}

template < typename T, class A >
T& concurrent_vector< T, A >::internal_subscript_with_exceptions(
    size_type index ) const {
    if ( index >= my_early_size )
        internal::throw_exception(
            internal::eid_out_of_range ); // throw std::out_of_range
    size_type j = index;
    segment_index_t k = segment_base_index_of( j );
    // TODO: refactor this condition into separate helper function, e.g.
    // fits_into_small_table
    if ( my_segment.load< acquire >() == my_storage &&
         k >= pointers_per_short_table )
        internal::throw_exception(
            internal::eid_segment_range_error ); // throw std::range_error
    // no need in load with acquire (load<acquire>) since thread works in own
    // space or gets
    // the information about added elements via some form of external
    // synchronization
    // TODO: why not make a load of my_segment relaxed as well ?
    // TODO: add an assertion that my_segment[k] is properly aligned to please
    // ITT
...
void concurrent_vector< T, A >::internal_assign_iterators( I first, I last ) {
    ...
    while ( sz < n ) {
        internal_loop_guide loop( sz, my_segment[ k ]
    ...
    internal_loop_guide loop(
        n,
        my_segment[ k ].template load< relaxed >().template pointer< void >() );
    loop.iterate( first );
}

template < typename T, class A >
void concurrent_vector< T, A >::initialize_array( void* begin,
                                                  const void*,
                                                  size_type n ) {
    internal_loop_guide loop( n, begin );
    loop.init();
}

template < typename T, class A >
void concurrent_vector< T, A >::initialize_array_by( void* begin,
                                                     const void* src,
                                                     size_type n ) {
    internal_loop_guide loop( n, begin );
    loop.init( src );
...
template < typename T, class A >
void concurrent_vector< T, A >::copy_array( void* dst,
                                            const void* src,
                                            size_type n ) {
...
#if __TBB_CPP11_RVALUE_REF_PRESENT
template < typename T, class A >
void concurrent_vector< T, A >::move_array( void* dst,
                                            const void* src,
                                            size_type n ) {
    internal_loop_guide loop( n, dst );
...
void concurrent_vector< T, A >::move_assign_array( void* dst,
                                                   ...
                                                   size_type n ) {
    internal_loop_guide loop( n, dst );
    loop.move_assign( src );
}
#endif

#if __TBB_MOVE_IF_NOEXCEPT_PRESENT
template < typename T, class A >
void concurrent_vector< T, A >::move_array_if_noexcept( void* dst,
                                                        const void* src,
                                                        size_type n ) {
    internal_loop_guide loop( n, dst );
    loop.move_construct_if_noexcept( src );
}
#endif //__TBB_MOVE_IF_NOEXCEPT_PRESENT

template < typename T, class A >
template < typename I >
void concurrent_vector< T, A >::copy_range( void* dst,
                                            const void* p_type_erased_iterator,
                                            size_type n ) {
    internal_loop_guide loop( n, dst );
    loop.iterate( *(
        static_cast< I* >( const_cast< void* >( p_type_erased_iterator ) ) ) );
}

template < typename T, class A >
void concurrent_vector< T, A >::assign_array( void* dst,
                                              const void* src,
                                              size_type n ) {
    internal_loop_guide loop( n, dst );
    loop.assign( src );
...
void concurrent_vector< T, A >::destroy_array( void* begin, size_type n ) {
    ...
    for ( size_type j = n; j > 0; --j )
        array[ j - 1 ]
            .~T(); // destructors are supposed to not throw any exceptions
}
#if defined( _MSC_VER ) && !defined( __INTEL_COMPILER )
#pragma warning( pop )
...
inline bool operator==( const concurrent_vector< T, A1 >& a,
                        const concurrent_vector< T, A2 >& b ) {
    // TODO: call size() only once per vector (in operator==)
    //  Simply:    return a.size() == b.size() && std::equal(a.begin(), a.end(),
    //  b.begin());
    if ( a.size() != b.size() )
        return false;
