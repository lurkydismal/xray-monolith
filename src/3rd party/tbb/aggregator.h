...
public:
    ...
    void operator()( aggregator_operation* op_list ) const {
        while ( op_list ) {
            ...
            // operation data. The executing thread "acquires" the tag (see
            // start()) and then performs the associated operation w/o
            // triggering a race condition diagnostics. A thread that created
            // the operation is waiting for its status (see execute_impl()), so
            // when this thread is done with the operation, it will "release"
            // the tag and update the status (see finish()) to give control back
            // to the waiting thread.
            basic_operation_base& request =
                static_cast< basic_operation_base& >( *op_list );
            // IMPORTANT: need to advance op_list to op_list->next() before
...
/** An aggregator for collecting operations coming from multiple sources and
   executing them serially on a single thread. */
template < typename handler_type >
class aggregator_ext : tbb::internal::no_copy {
public:
    aggregator_ext( const handler_type& h )
        : handler_busy( 0 ), handle_operations( h ) {
        mailbox = NULL;
    }

    //! EXPERT INTERFACE: Enter a user-made operation into the aggregator's
    //! mailbox.
...
protected:
    /** Place operation in mailbox, then either handle mailbox or wait for the
       operation to be completed by a different thread. */
    void execute_impl( aggregator_operation& op ) {
        aggregator_operation* res;

        // ITT note: &(op.status) tag is used to cover accesses to this
        // operation. This thread has created the operation, and now releases it
        // so that the handler thread may handle the associated operation w/o
        ...
        do {
            // ITT may flag the following line as a race; it is a false
        ...
        if ( !res ) { // first in the list; handle the operations
            // ITT note: &mailbox tag covers access to the handler_busy flag,
            ...
            spin_wait_while_eq(
                op.status, uintptr_t( aggregator_operation::agg_waiting ) );
...
private:
    ...
    void start_handle_operations() {
        ...
        // waits until the active handler releases, and the waiting handler
        // acquires &handler_busy as it becomes the active_handler. The release
        // point is at the end of this function, when all operations in mailbox
        // have been handled by the owner of this aggregator.
        ...
        // Capturing the state of the mailbox signifies that handler_busy has
        // been set and a new active handler will now process that list's
        // operations.
