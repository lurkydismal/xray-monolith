...
private:
    ...
    static filter* not_in_pipeline() {
        return reinterpret_cast< filter* >( intptr_t( -1 ) );
    }
...
protected:
    ...
    static const unsigned char version_mask = 0x7
                                              << 1; // bits 1-3 are for version
public:
    enum mode {
        ...
        serial_out_of_order =
            current_version | filter_is_serial | filter_is_out_of_order,
...
protected:
    explicit filter( bool is_serial_ )
        : next_filter_in_pipeline( not_in_pipeline() ),
          ...
          my_filter_mode(
              static_cast< unsigned char >( ( is_serial_ ? serial : parallel ) |
    ...
    explicit filter( mode filter_mode )
        : next_filter_in_pipeline( not_in_pipeline() ),
          my_input_buffer( NULL ),
          my_filter_mode( static_cast< unsigned char >(
              filter_mode | exact_exception_propagation ) ),
          prev_filter_in_pipeline( not_in_pipeline() ),
          my_pipeline( NULL ),
          next_segment( NULL ) {}
...
public:
    ...
    bool is_serial() const { return bool( my_filter_mode & filter_is_serial ); }

    //! True if filter must receive stream in order.
    bool is_ordered() const {
        return ( my_filter_mode & ( filter_is_out_of_order |
                                    filter_is_serial ) ) == filter_is_serial;
    }
    ...
    bool object_may_be_null() {
        return ( my_filter_mode & filter_may_emit_null ) ==
    ...
    //! Destroy filter.
    /** If the filter was added to a pipeline, the pipeline must be destroyed
     * first. */
...
protected:
    explicit thread_bound_filter( mode filter_mode )
        : filter(
              static_cast< mode >( filter_mode | filter::filter_is_bound ) ) {
        __TBB_ASSERT( filter_mode & filter::filter_is_serial,
...
public:
    ...
    /** This interface is blocking.
        Returns 'success' if an item was processed.
        Returns 'end_of_stream' if there are no more items to process.
        Never returns 'item_not_available', as it blocks until another return
       condition applies. */
...
/** @ingroup algorithms */
class __TBB_DEPRECATED_MSG(
    "tbb::pipeline is deprecated, use tbb::parallel_pipeline" ) pipeline {
public:
    //! Construct empty pipeline.
    __TBB_EXPORTED_METHOD pipeline();

    /** Though the current implementation declares the destructor virtual, do
       not rely on this detail.  The virtualness is deprecated and may disappear
       in future versions of TBB. */
...
// Emulate std::is_trivially_copyable (false positives not allowed, false
// negatives suboptimal but safe).
#if __TBB_CPP11_TYPE_PROPERTIES_PRESENT
template < typename T >
...
template <>
struct tbb_trivially_copyable< wchar_t > {
    enum { value = true };
};
...
struct use_allocator {
    enum {
        value =
            sizeof( T ) > sizeof( void* ) || !tbb_trivially_copyable< T >::value
    };
};
...
class concrete_filter : public tbb::filter {
    ...
    void* operator()( void* input ) __TBB_override {
        ...
        u_pointer output_u = u_helper::create_token(
...
public:
    concrete_filter( tbb::filter::mode filter_mode, const Body& body )
        : filter( static_cast< tbb::filter::mode >( filter_mode |
                                                    filter_may_emit_null ) ),
          my_body( body ) {}
};
...
class concrete_filter< void, void, Body > : public filter {
    ...
    void* operator()( void* ) __TBB_override {
        ...
        my_body( control );
        void* output =
            control.is_pipeline_stopped ? NULL : ( void* )( intptr_t )-1;
        return output;
    }
...
/** It primarily serves as RAII class that deletes heap-allocated filter
 * instances. */
class pipeline_proxy {
...
class filter_node : tbb::internal::no_copy {
    /** Count must be atomic because it is hidden state for user, but might be
     * shared by threads. */
...
class filter_t {
    ...
    friend filter_t< T_, U_ > operator&( const filter_t< T_, V_ >&,
...
public:
    ...
    void operator=( const filter_t< T, U >& rhs ) {
        // Order of operations below carefully chosen so that reference counts
        // remain correct in unlikely event that remove_ref throws exception.
...
inline internal::pipeline_proxy::pipeline_proxy(
