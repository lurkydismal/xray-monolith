...
/** The function uses scalable_free if scalable allocator is available and free
 * if not*/
void __TBB_EXPORTED_FUNC deallocate_via_handler_v3( void* p );

//! Allocates memory using MallocHandler
/** The function uses scalable_malloc if scalable allocator is available and
 * malloc if not*/
...
public:
    ...
    pointer allocate( size_type n, const void* /*hint*/ = 0 ) {
        return pointer(
            internal::allocate_via_handler_v3( n * sizeof( value_type ) ) );
    }
...
inline bool operator==( const tbb_allocator< T >&, const tbb_allocator< U >& ) {
    return true;
}
...
template < typename T1,
           ...
           typename T2,
           template < typename X2 > class B2 >
inline bool operator==( const zero_allocator< T1, B1 >& a,
