...
public:
    ...
    using node_type = tbb::internal::node_handle<
        key_type,
        value_type,
        internal::skip_list_node< value_type, mutex_type >,
        allocator_type >;
    ...
    class value_compare {
    public:
        // TODO: these member types are deprecated in C++17, do we need to let
        // them
    ...
    static const key_type& get_key( const_reference val ) { return val.first; }
}; // class map_traits
...
class concurrent_map
    : public internal::concurrent_skip_list<
          map_traits< Key,
                      Value,
                      Comp,
                      internal::concurrent_geometric_level_generator< 64 >,
                      64,
                      Allocator,
                      false > > {
    using traits_type =
...
public:
    ...
    template < class InputIt >
    concurrent_map( InputIt first, InputIt last, const allocator_type& alloc )
    ...
    void merge(
        concurrent_map< key_type, mapped_type, C2, Allocator >& source ) {
    ...
    template < typename C2 >
    void merge(
        concurrent_multimap< key_type, mapped_type, C2, Allocator >& source ) {
        this->internal_merge( source );
    }
...
concurrent_map( It, It, Args... )
    ->internal::c_map_t< concurrent_map,
                         internal::iterator_key_t< It >,
                         internal::iterator_mapped_t< It >,
                         Args... >;
...
public:
    ...
    template < class InputIt >
    concurrent_multimap( InputIt first,
                         InputIt last,
                         const key_compare& comp = Comp(),
                         const allocator_type& alloc = allocator_type() )
        : base_type( first, last, comp, alloc ) {}
    ...
    concurrent_multimap& operator=( const concurrent_multimap& other ) {
        return static_cast< concurrent_multimap& >(
...
concurrent_multimap( It, It, Args... )
    ->internal::c_map_t< concurrent_multimap,
                         internal::iterator_key_t< It >,
                         internal::iterator_mapped_t< It >,
                         Args... >;

template < typename Key, typename T, typename... Args >
concurrent_multimap( std::initializer_list< std::pair< const Key, T > >,
                     Args... )
