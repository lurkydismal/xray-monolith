...
class finish_reduce : public flag_task {
    ...
    task* execute() __TBB_override {
        if ( has_right_zombie ) {
            ...
            my_body->join( *s );
            // Body::join() won't be called if canceled. Defer destruction to
            // destructor
        }
        if ( my_context == left_child )
            itt_store_word_with_release(
                static_cast< finish_reduce* >( parent() )->my_body, my_body );
        return NULL;
    }
...
public:
...
#else
            // Bound context prevents exceptions from body to affect nesting or
            // sibling algorithms, and allows users to handle exceptions safely
            // by wrapping parallel_for in the try-block.
            task_group_context context( PARALLEL_REDUCE );
            task::spawn_root_and_wait( *new ( task::allocate_root(
                context ) ) start_reduce( range, &body, partitioner ) );
    ...
    void offer_work( typename Partitioner::split_type& split_obj ) {
        task* tasks[ 2 ];
        allocate_sibling( static_cast< task* >( this ), tasks,
                          sizeof( start_reduce ), sizeof( finish_type ) );
...
//! allocate right task with new parent
// TODO: 'inline' here is to avoid multiple definition error but for sake of
// code size this should not be inlined
inline void allocate_sibling( task* start_reduce_task,
                              task* tasks[],
                              size_t start_bytes,
                              size_t finish_bytes ) {
    tasks[ 0 ] =
        &start_reduce_task->allocate_continuation().allocate( finish_bytes );
...
task* start_reduce< Range, Body, Partitioner >::execute() {
    ...
    if ( my_context == right_child ) {
        ...
        if ( !itt_load_word_with_acquire(
                 parent_ptr
                     ->my_body ) ) { // TODO: replace by is_stolen_task() or by
                                     // parent_ptr->ref_count() == 2???
            my_body = new ( parent_ptr->zombie_space.begin() )
                Body( *my_body, split() );
        ...
        __TBB_ASSERT(
            my_context == root_task,
            NULL ); // because left leaf spawns right leafs without recycling
...
//! Task type used to combine the partial results of
...
class start_deterministic_reduce : public task {
    ...
    //! Constructor used for root task
    start_deterministic_reduce( const Range& range,
                                Body& body,
                                Partitioner& partitioner )
...
public:
    static void run( const Range& range,
        ...
        if ( !range.empty() ) {
            ...
            task::spawn_root_and_wait(
                *new ( task::allocate_root() )
                    start_deterministic_reduce( range, &body, partitioner ) );
#else
            // Bound context prevents exceptions from body to affect nesting or
            // sibling algorithms, and allows users to handle exceptions safely
            // by wrapping parallel_for in the try-block.
            task_group_context context( PARALLEL_REDUCE );
            task::spawn_root_and_wait(
                *new ( task::allocate_root( context ) )
                    start_deterministic_reduce( range, body, partitioner ) );
#endif /* __TBB_TASK_GROUP_CONTEXT && !TBB_JOIN_OUTER_TASK_GROUP */
        }
    }
    ...
    void offer_work( typename Partitioner::split_type& split_obj ) {
        task* tasks[ 2 ];
        allocate_sibling( static_cast< task* >( this ), tasks,
                          sizeof( start_deterministic_reduce ),
                          sizeof( finish_type ) );
        new ( ( void* )tasks[ 0 ] ) finish_type( my_body );
        new ( ( void* )tasks[ 1 ] ) start_deterministic_reduce(
            *this, *static_cast< finish_type* >( tasks[ 0 ] ), split_obj );
        spawn( *tasks[ 1 ] );
    }
...
public:
    lambda_reduce_body( const Value& identity,
                        const RealBody& body,
...
/** \page parallel_reduce_body_req Requirements on parallel_reduce body
    ...
    - \code Body::Body( Body&, split ); \endcode        Splitting constructor.
                                                        Must be able to run
concurrently with operator() and method \c join
    - \code Body::~Body(); \endcode                     Destructor
    - \code void Body::operator()( Range& r ); \endcode Function call operator
applying body to range \c r and accumulating the result
    - \code void Body::join( Body& b ); \endcode        Join results.
                                                        The result in \c b
should be merged into the result of \c this
**/

/** \page parallel_reduce_lambda_req Requirements on parallel_reduce anonymous
function objects (lambda functions) TO BE DOCUMENTED
**/

/** \name parallel_reduce
    See also requirements on \ref range_req "Range" and \ref
   parallel_reduce_body_req "parallel_reduce Body". **/
//@{
...
void parallel_reduce( const Range& range, Body& body ) {
    internal::start_reduce< Range, Body, const __TBB_DEFAULT_PARTITIONER >::run(
...
void parallel_reduce( const Range& range,
    ...
    internal::start_reduce< Range, Body, const simple_partitioner >::run(
...
void parallel_reduce( const Range& range,
    ...
    internal::start_reduce< Range, Body, const auto_partitioner >::run(
        range, body, partitioner );
}
...
void parallel_reduce( const Range& range,
    ...
    internal::start_reduce< Range, Body, const static_partitioner >::run(
        range, body, partitioner );
}
...
void parallel_reduce( const Range& range,
    ...
    internal::start_reduce< Range, Body, affinity_partitioner >::run(
        range, body, partitioner );
}

#if __TBB_TASK_GROUP_CONTEXT
//! Parallel iteration with reduction, default partitioner and user-supplied
//! context.
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_reduce( const Range& range,
...
void parallel_reduce( const Range& range,
                      ...
                      task_group_context& context ) {
    internal::start_reduce< Range, Body, const simple_partitioner >::run(
        range, body, partitioner, context );
}

//! Parallel iteration with reduction, auto_partitioner and user-supplied
//! context
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_reduce( const Range& range,
...
//! Parallel iteration with reduction, static_partitioner and user-supplied
//! context
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_reduce( const Range& range,
...
//! Parallel iteration with reduction, affinity_partitioner and user-supplied
//! context
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_reduce( const Range& range,
...
/** parallel_reduce overloads that work with anonymous function objects
    (see also \ref parallel_reduce_lambda_req "requirements on parallel_reduce
...
Value parallel_reduce( const Range& range,
                       ...
                       const Reduction& reduction ) {
    internal::lambda_reduce_body< Range, Value, RealBody, Reduction > body(
        identity, real_body, reduction );
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        const __TBB_DEFAULT_PARTITIONER >::run( range, body,
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        const simple_partitioner >::run( range, body, partitioner );
    return body.result();
}
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        const auto_partitioner >::run( range, body, partitioner );
    return body.result();
}
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        const static_partitioner >::run( range, body, partitioner );
    return body.result();
}
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        affinity_partitioner >::run( range, body, partitioner );
    return body.result();
}
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        const simple_partitioner >::run( range, body, partitioner, context );
    return body.result();
}
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        const auto_partitioner >::run( range, body, partitioner, context );
    return body.result();
}
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        const static_partitioner >::run( range, body, partitioner, context );
    return body.result();
}
...
Value parallel_reduce( const Range& range,
    ...
    internal::start_reduce<
        Range,
        internal::lambda_reduce_body< Range, Value, RealBody, Reduction >,
        affinity_partitioner >::run( range, body, partitioner, context );
    return body.result();
}
#endif /* __TBB_TASK_GROUP_CONTEXT */

//! Parallel iteration with deterministic reduction and default simple
//! partitioner.
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_deterministic_reduce( const Range& range, Body& body ) {
    internal::start_deterministic_reduce<
        Range, Body, const simple_partitioner >::run( range, body,
                                                      simple_partitioner() );
}
...
void parallel_deterministic_reduce( const Range& range,
                                    ...
                                    const static_partitioner& partitioner ) {
    internal::start_deterministic_reduce<
        Range, Body, const static_partitioner >::run( range, body,
                                                      partitioner );
}

#if __TBB_TASK_GROUP_CONTEXT
//! Parallel iteration with deterministic reduction, default simple partitioner
...
void parallel_deterministic_reduce( const Range& range,
                                    ...
                                    task_group_context& context ) {
    internal::start_deterministic_reduce<
        Range, Body, const simple_partitioner >::run( range, body, partitioner,
                                                      context );
}

//! Parallel iteration with deterministic reduction, static partitioner and
//! user-supplied context.
/** @ingroup algorithms **/
template < typename Range, typename Body >
void parallel_deterministic_reduce( const Range& range,
                                    ...
                                    task_group_context& context ) {
    internal::start_deterministic_reduce<
        Range, Body, const static_partitioner >::run( range, body, partitioner,
                                                      context );
}
#endif /* __TBB_TASK_GROUP_CONTEXT */
...
Value parallel_deterministic_reduce( const Range& range,
                                     ...
                                     const RealBody& real_body,
                                     const Reduction& reduction ) {
    return parallel_deterministic_reduce( range, identity, real_body, reduction,
                                          simple_partitioner() );
}
