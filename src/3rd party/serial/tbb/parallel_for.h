...
#if !defined( __TBB_show_deprecation_message_parallel_for_H ) && \
    defined( __TBB_show_deprecated_header_message )
#define __TBB_show_deprecation_message_parallel_for_H
#pragma message( \
    "TBB Warning: serial/tbb/parallel_for.h is deprecated. For details, please see Deprecated Features appendix in the TBB reference manual." )
#endif
...
void start_for< Range, Body, Partitioner >::execute() {
    if ( !my_range.is_divisible() || !my_partition.is_divisible() ) {
        ...
        this->execute(); // Execute the left interval first to keep the serial
                         // order.
...
__TBB_DEPRECATED_VERBOSE void parallel_for( const Range& range,
                                            const Body& body ) {
    serial::interface9::
...
/** @ingroup algorithms **/
template < typename Range, typename Body >
__TBB_DEPRECATED_VERBOSE void parallel_for(
    const Range& range,
    const Body& body,
    const simple_partitioner& partitioner ) {
    serial::interface9::start_for< Range, Body, const simple_partitioner >::run(
...
__TBB_DEPRECATED_VERBOSE void parallel_for(
    ...
    serial::interface9::start_for< Range, Body, const auto_partitioner >::run(
        range, body, partitioner );
}
...
__TBB_DEPRECATED_VERBOSE void parallel_for(
    ...
    serial::interface9::start_for< Range, Body, const static_partitioner >::run(
        range, body, partitioner );
}
...
__TBB_DEPRECATED_VERBOSE void parallel_for(
    ...
    serial::interface9::start_for< Range, Body, affinity_partitioner >::run(
        range, body, partitioner );
}

//! Implementation of parallel iteration over stepped range of integers with
//! explicit step and partitioner (ignored)
...
//! Parallel iteration over a range of integers with explicit step and default
//! partitioner
template < typename Index, typename Function >
__TBB_DEPRECATED_VERBOSE void parallel_for( Index first,
                                            Index last,
                                            Index step,
                                            const Function& f ) {
...
//! Parallel iteration over a range of integers with default step and default
//! partitioner
template < typename Index, typename Function >
__TBB_DEPRECATED_VERBOSE void parallel_for( Index first,
                                            Index last,
