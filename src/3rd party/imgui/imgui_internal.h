...
#if defined( IMGUI_ENABLE_SSE4_2 ) && \
    !defined( IMGUI_USE_LEGACY_CRC32_ADLER ) && !defined( __EMSCRIPTEN__ )
...
// Utilities
// (other types which are not forwarded declared are: ImBitArray<>, ImSpan<>,
// ImSpanAllocator<>, ImPool<>, ImChunkStream<>)
...
// ImGui
struct ImGuiBoxSelectState; // Box-selection state (currently used by
                            // multi-selection, could potentially be used by
                            // others)
...
struct ImGuiMenuColumns;      // Simple column measurement, currently used for
                              // MenuItem() only
struct ImGuiMultiSelectState; // Multi-selection persistent state (for focused
                              // selection).
struct ImGuiMultiSelectTempData; // Multi-selection temporary state (while
                                 // traversing).
struct ImGuiNavItemData; // Result of a keyboard/gamepad directional navigation
                         // move query result
...
typedef int ImGuiOldColumnFlags;  // -> enum ImGuiOldColumnFlags_     // Flags:
                                  // for BeginColumns()
typedef int ImGuiLogFlags; // -> enum ImGuiLogFlags_           // Flags: for
                           // LogBegin() text capturing function
typedef int
    ImGuiNavRenderCursorFlags; // -> enum ImGuiNavRenderCursorFlags_//Flags: for
                               // RenderNavCursor()
...
#define IMGUI_DEBUG_LOG_FONT( ... )                           \
    do {                                                      \
        if ( g.DebugLogFlags & ImGuiDebugLogFlags_EventFont ) \
            IMGUI_DEBUG_LOG( __VA_ARGS__ );                   \
    } while ( 0 )
...
#define IM_MEMALIGN( _OFF, _ALIGN )       \
    ( ( ( _OFF ) + ( ( _ALIGN ) - 1 ) ) & \
      ~( ( _ALIGN ) -                     \
         1 ) ) // Memory align e.g. IM_ALIGN(0,4)=0, IM_ALIGN(1,4)=4,
...
#define IM_TRUNC( _VAL ) \
    ( ( float )( int )( _VAL ) ) // ImTrunc() is not inlined in MSVC debug
                                 // builds
#define IM_ROUND( _VAL ) ( ( float )( int )( ( _VAL ) + 0.5f ) ) //
#define IM_STRINGIFY_HELPER( _X ) #_X
#define IM_STRINGIFY( _X ) \
    IM_STRINGIFY_HELPER(   \
        _X ) // Preprocessor idiom to stringify e.g. an integer.
...
// Helpers: Sorting
#ifndef ImQsort
static inline void ImQsort( void* base,
                            size_t count,
...
IMGUI_API const char* ImStrchrRange(
...
IMGUI_API const char* ImStristr(
    const char* haystack,
    const char* haystack_end,
    const char* needle,
    const char* needle_end ); // Find a substring in a string range.
IMGUI_API void ImStrTrimBlanks(
    char* str ); // Remove leading and trailing blanks from a buffer.
IMGUI_API const char* ImStrSkipBlank(
    const char* str ); // Find first non-blank character.
IMGUI_API int ImStrlenW(
    const ImWchar* str ); // Computer string length (ImWchar string)
...
IMGUI_API int ImTextCountLines(
    const char* in_text,
    const char*
        in_text_end ); // return number of lines taken by text. trailing
...
// Helpers: Geometry
IMGUI_API ImVec2 ImBezierCubicCalc( const ImVec2& p1,
                                    const ImVec2& p2,
                                    const ImVec2& p3,
                                    const ImVec2& p4,
                                    float t );
...
IMGUI_API ImVec2 ImBezierCubicClosestPointCasteljau(
    ...
    const ImVec2& p4,
    const ImVec2& p,
    float tess_tol ); // For auto-tessellated curves you can use tess_tol =
...
#define IM_BITARRAY_TESTBIT( _ARRAY, _N )                              \
    ( ( _ARRAY[ ( _N ) >> 5 ] & ( ( ImU32 )1 << ( ( _N ) & 31 ) ) ) != \
      0 ) // Macro version of ImBitArrayTestBit(): ensure args have side-effect
          // or are costly!
...
// Helper: ImGuiStorage
IMGUI_API ImGuiStoragePair* ImLowerBound( ImGuiStoragePair* in_begin,
...
#define IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC( _RAD, _MAXERROR )         \
    ImClamp( IM_ROUNDUP_TO_EVEN( ( int )ImCeil(                         \
                 IM_PI / ImAcos( 1 - ImMin( ( _MAXERROR ), ( _RAD ) ) / \
                                         ( _RAD ) ) ) ),                \
...
// You may want to create your own instance of you try to ImDrawList completely
// without ImGui. In that case, watch out for future changes to this structure.
struct IMGUI_API ImDrawListSharedData {
    ...
    float FontSize; // Current/default font size (optional, for simplified
                    // AddText overload)
    float
        FontScale; // Current/default font scale (== FontSize / Font->FontSize)
    ...
    // Lookup tables
    ImVec2
        ArcFastVtx[ IM_DRAWLIST_ARCFAST_TABLE_SIZE ]; // Sample points on the
    ...
    ImU8 CircleSegmentCounts[ 64 ]; // Precomputed segment count for given
                                    // radius before we calculate it dynamically
...
struct ImDrawDataBuilder {
    ImVector< ImDrawList* >*
        Layers[ 2 ]; // Pointers to global layers for: regular, tooltip.
                     // LayersP[0] is owned by DrawData.
...
// - input: PushItemFlag() manipulates g.CurrentItemFlags,
// g.NextItemData.ItemFlags, ItemAdd() calls may add extra flags too.
// - output: stored in g.LastItemData.ItemFlags
enum ImGuiItemFlagsPrivate_ {
    // Controlled by user
    ImGuiItemFlags_Disabled =
        1 << 10, // false     // Disable interactions (DOES NOT affect visuals.
                 // DO NOT mix direct use of this with BeginDisabled(). See
                 // BeginDisabled()/EndDisabled() for full disable feature, and
                 // github #211).
    ImGuiItemFlags_ReadOnly =
        1 << 11, // false     // [ALPHA] Allow hovering interactions but
                 // underlying value is not changed.
    ImGuiItemFlags_MixedValue =
        1 << 12, // false     // [BETA] Represent a mixed/indeterminate value,
                 // generally multi-selection where values differ. Currently
                 // only supported by Checkbox() (later should support all sorts
                 // of widgets)
    ImGuiItemFlags_NoWindowHoverableCheck =
        1 << 13, // false     // Disable hoverable check in ItemHoverable()
    ImGuiItemFlags_AllowOverlap =
        1 << 14, // false     // Allow being overlapped by another widget.
                 // Not-hovered to Hovered transition deferred by a frame.
    ImGuiItemFlags_NoNavDisableMouseHover =
        1 << 15, // false     // Nav keyboard/gamepad mode doesn't disable hover
    ...
    ImGuiItemFlags_Inputable =
        1 << 20, // false     // [WIP] Auto-activate input mode when tab
                 // focused. Currently only used and supported by a few items
    ...
    ImGuiItemFlags_IsMultiSelect =
        1 << 22, // false     // Set by SetNextItemSelectionUserData()

    ImGuiItemFlags_Default_ =
        ImGuiItemFlags_AutoClosePopups, // Please don't change, use
                                        // PushItemFlag() instead.

    // Obsolete
    // ImGuiItemFlags_SelectableDontClosePopup =
    // !ImGuiItemFlags_AutoClosePopups, // Can't have a redirect as we inverted
    // the behavior
};
...
enum ImGuiItemStatusFlags_ {
    ...
    ImGuiItemStatusFlags_HoveredRect =
        1 << 0, // Mouse position is within item rectangle (does NOT mean that
                // the window is in correct z-order and can be hovered!, this is
                // only one part of the most-common IsItemHovered test)
    ...
    ImGuiItemStatusFlags_ToggledSelection =
        1 << 3, // Set when Selectable(), TreeNode() reports toggling a
                // selection. We can't report "Selected", only state changes, in
                // order to easily handle clipping with less issues.
    ...
    ImGuiItemStatusFlags_HasClipRect =
        1 << 9, // g.LastItemData.ClipRect is valid.
    ImGuiItemStatusFlags_HasShortcut =
        1 << 10, // g.LastItemData.Shortcut valid. Set by SetNextItemShortcut()
                 // -> ItemAdd().

// Additional status + semantic for ImGuiTestEngine
#ifdef IMGUI_ENABLE_TEST_ENGINE
    ImGuiItemStatusFlags_Openable =
        1 << 20, // Item is an openable (e.g. TreeNode)
    ...
    ImGuiItemStatusFlags_Inputable =
        1
        << 24, // Item is a text-inputable (e.g. InputText, SliderXXX, DragXXX)
#endif
};
...
enum ImGuiInputTextFlagsPrivate_ {
    ...
    ImGuiInputTextFlags_MergedItem =
        1 << 27, // For internal use by TempInputText(), will skip calling
...
enum ImGuiButtonFlagsPrivate_ {
    ...
    ImGuiButtonFlags_PressedOnClickReleaseAnywhere =
        1 << 6, // return true on click + release even if the release event is
                // not done while hovering the item
    ...
    // use ImGuiItemFlags_ButtonRepeat instead.
    ImGuiButtonFlags_FlattenChildren =
        1 << 11, // allow interactions even if a child window is overlapping
    ImGuiButtonFlags_AllowOverlap =
        1 << 12, // require previous frame HoveredId to either match id or be
                 // null before being usable.
    // ImGuiButtonFlags_DontClosePopups      = 1 << 13,  // disable
    // automatically closing parent popup on press ImGuiButtonFlags_Disabled = 1
    // << 14,  // disable interactions -> use BeginDisabled() or
    // ImGuiItemFlags_Disabled
    ImGuiButtonFlags_AlignTextBaseLine =
        1
        << 15, // vertically align button to match text baseline - ButtonEx()
    ...
    ImGuiButtonFlags_NoNavFocus =
        1
        << 18, // don't override navigation focus when activated (FIXME: this is
               // essentially used every time an item uses ImGuiItemFlags_NoNav,
...
enum ImGuiSliderFlagsPrivate_ {
    ...
    ImGuiSliderFlags_ReadOnly =
        1 << 21, // Consider using g.NextItemData.ItemFlags |=
...
enum ImGuiTreeNodeFlagsPrivate_ {
    ...
    ImGuiTreeNodeFlags_UpsideDownArrow =
        1 << 29, // FIXME-WIP: Turn Down arrow into an Up arrow, for reversed
                 // trees (#6517)
...
enum ImGuiTooltipFlags_ {
    ...
    ImGuiTooltipFlags_OverridePrevious =
        1
        << 1, // Clear/ignore previously submitted tooltip (defaults to append)
};
...
enum ImGuiLogFlags_ {
    ...
    ImGuiLogFlags_OutputMask_ =
        ImGuiLogFlags_OutputTTY | ImGuiLogFlags_OutputFile |
...
struct IMGUI_API ImGuiMenuColumns {
    ...
    ImU16 OffsetMark;
    ImU16 Widths[ 4 ]; // Width of:   Icon, Label, Shortcut, Mark  (accumulators
                       // for current frame)
...
// Forward declare imstb_textedit.h structure + make its main configuration
// define accessible
...
struct IMGUI_API ImGuiInputTextState {
    ...
    ImGuiInputTextFlags
        Flags;   // copy of InputText() flags. may be used to check if e.g.
                 // ImGuiInputTextFlags_Password is set.
    ImGuiID ID;  // widget id owning the text state
    int TextLen; // UTF-8 length of the string in TextA (in bytes)
    const char* TextSrc; // == TextA.Data unless read-only, in which case == buf
                         // passed to InputText(). Field only set and valid
                         // _inside_ the call InputText() call.
    ImVector< char >
        TextA; // main UTF8 buffer. TextA.Size is a buffer size! Should always
               // be >= buf_size passed by user (and of course >= CurLenA + 1).
    ImVector< char > TextToRevertTo; // value to revert to when pressing Escape
                                     // = backup of end-user buffer at the time
                                     // of focus (in UTF-8, unaltered)
    ImVector< char >
        CallbackTextBackup; // temporary storage for callback to support
                            // automatic reconcile of undo-stack
    ...
    bool WantReloadUserBuf; // force a reload of user buf so it may be modified
    ...
    //   strcpy(my_buf, "hello");
    //   if (ImGuiInputTextState* state = ImGui::GetInputTextState(id)) // id
    //   may be ImGui::GetItemID() is last item
...
struct ImGuiNextWindowData {
    ...
    ImVec2
        MenuBarOffsetMinVal; // (Always on) This is not exposed publicly, so we
...
struct ImGuiNextItemData {
    ImGuiNextItemDataFlags
        HasFlags; // Called HasFlags instead of Flags to avoid mistaking this
    ImGuiItemFlags
        ItemFlags; // Currently only tested/used for ImGuiItemFlags_AllowOverlap
                   // and ImGuiItemFlags_HasSelectionUserData.
    // Non-flags members are NOT cleared by ItemAdd() meaning they are still
    // valid during NavProcessItem()
    ...
    ImGuiDataTypeStorage
        RefVal; // Not exposed yet, for ImGuiInputTextFlags_ParseEmptyAsRefVal
...
struct ImGuiLastItemData {
    ImGuiID ID;
    ImGuiItemFlags
        ItemFlags; // See ImGuiItemFlags_ (called 'InFlags' before v1.91.4).
    ImGuiItemStatusFlags StatusFlags; // See ImGuiItemStatusFlags_
    ImRect Rect;                      // Full rectangle
    ImRect NavRect; // Navigation scoring rectangle (not displayed)
    // Rarely used fields are not explicitly cleared, only valid when the
    // corresponding ImGuiItemStatusFlags are set.
...
// - To implement ImGuiTreeNodeFlags_NavLeftJumpsBackHere: store the minimum
// amount of data
//   which we can't infer in TreePop(), to perform the equivalent of
//   NavApplyItemToResult(). Only stored when the node is a potential candidate
...
struct ImGuiWindowStackData {
    ...
    ImGuiErrorRecoveryState
        StackSizesInBegin;         // Store size of various stacks for asserting
...
struct ImGuiPopupData {
    ...
    ImGuiWindow* Window; // Resolved on BeginPopup() - may stay unresolved if
                         // user never calls OpenPopup()
    ImGuiWindow* RestoreNavWindow; // Set on OpenPopup(), a NavWindow that will
                                   // be restored on popup close
    int ParentNavLayer;   // Resolved on BeginPopup(). Actually a ImGuiNavLayer
    ...
    ImVec2 OpenMousePos;  // Set on OpenPopup(), copy of mouse position at the
                          // time of opening popup
...
#define ImGuiKey_NavGamepadActivate                                \
    ( g.IO.ConfigNavSwapGamepadButtons ? ImGuiKey_GamepadFaceRight \
                                       : ImGuiKey_GamepadFaceDown )
#define ImGuiKey_NavGamepadCancel                                 \
    ( g.IO.ConfigNavSwapGamepadButtons ? ImGuiKey_GamepadFaceDown \
...
#define ImGuiKeyOwner_Any \
    ( ( ImGuiID )0 ) // Accept key that have an owner, UNLESS a call to
                     // SetKeyOwner() explicitly used
                     // ImGuiInputFlags_LockThisFrame or
                     // ImGuiInputFlags_LockUntilRelease.
#define ImGuiKeyOwner_NoOwner \
    ( ( ImGuiID ) - 1 ) // Require key to have no owner.
// #define ImGuiKeyOwner_None ImGuiKeyOwner_NoOwner  // We previously called
// this 'ImGuiKeyOwner_None' but it was inconsistent with our pattern that _None
// values == 0 and quite dangerous. Also using _NoOwner makes the IsKeyPressed()
// calls more explicit.
...
struct ImGuiKeyRoutingData {
    ...
    ImU16 Mods; // Technically we'd only need 4-bits but for simplify we store
                // ImGuiMod_ values which need 16-bits.
...
struct ImGuiKeyOwnerData {
    ImGuiID OwnerCurr;
    ImGuiID OwnerNext;
    bool LockThisFrame; // Reading this key requires explicit owner id (until
                        // end of frame). Set by ImGuiInputFlags_LockThisFrame.
    bool LockUntilRelease; // Reading this key requires explicit owner id (until
                           // key is released). Set by
                           // ImGuiInputFlags_LockUntilRelease. When this is
                           // true LockThisFrame is always true as well.
...
enum ImGuiInputFlagsPrivate_ {
    // Flags for IsKeyPressed(), IsKeyChordPressed(), IsMouseClicked(),
    // Shortcut()
    ...
    ImGuiInputFlags_RepeatUntilKeyModsChangeFromNone =
        1 << 6, // Stop repeating when released OR if keyboard mods are leaving
                // the None state. Allows going from Mod+Key to Key by releasing
                // Mod.
    ...
    // input-owner-aware code steal keys from non-input-owner-aware code. If all
    // code is input-owner-aware locking would never be necessary.
    ImGuiInputFlags_LockThisFrame =
        1 << 20, // Further accesses to key data will require EXPLICIT owner ID
                 // (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared
                 // at end of frame.
    ImGuiInputFlags_LockUntilRelease =
        1 << 21, // Further accesses to key data will require EXPLICIT owner ID
                 // (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared
    ...
    ImGuiInputFlags_CondHovered =
        1 << 22, // Only set if item is hovered (default to both)
    ImGuiInputFlags_CondActive =
        1 << 23, // Only set if item is active (default to both)
    ...
    ImGuiInputFlags_RouteTypeMask_ =
        ImGuiInputFlags_RouteActive | ImGuiInputFlags_RouteFocused |
        ImGuiInputFlags_RouteGlobal | ImGuiInputFlags_RouteAlways,
    ImGuiInputFlags_RouteOptionsMask_ = ImGuiInputFlags_RouteOverFocused |
...
enum ImGuiActivateFlags_ {
    ...
    ImGuiActivateFlags_PreferTweak =
        1 << 1, // Favor activation for tweaking with arrows or gamepad (e.g.
                // for Slider/Drag). Default for Space key and if keyboard is
                // not used.
    ImGuiActivateFlags_TryToPreserveState =
        1 << 2, // Request widget to preserve state if it can (e.g. InputText
                // will try to preserve cursor/selection)
...
enum ImGuiScrollFlags_ {
    ...
    ImGuiScrollFlags_KeepVisibleEdgeX =
        1 << 0, // If item is not visible: scroll as little as possible on X
                // axis to bring item back into view [default for X axis]
    ImGuiScrollFlags_KeepVisibleEdgeY =
        1 << 1, // If item is not visible: scroll as little as possible on Y
                // axis to bring item back into view [default for Y axis for
    ...
    ImGuiScrollFlags_AlwaysCenterY =
        1 << 5, // Always center the result item on Y axis [default for Y axis
                // for appearing window)
...
enum ImGuiNavRenderCursorFlags_ {
    ...
    ImGuiNavRenderCursorFlags_AlwaysDraw =
        1 << 2, // Draw rectangular highlight if (g.NavId == id) even when
                // g.NavCursorVisible == false, aka even when using the mouse.
    ImGuiNavRenderCursorFlags_NoRounding = 1 << 3,
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiNavHighlightFlags_None =
        ImGuiNavRenderCursorFlags_None, // Renamed in 1.91.4
    ImGuiNavHighlightFlags_Compact =
        ImGuiNavRenderCursorFlags_Compact, // Renamed in 1.91.4
    ImGuiNavHighlightFlags_AlwaysDraw =
        ImGuiNavRenderCursorFlags_AlwaysDraw, // Renamed in 1.91.4
    ImGuiNavHighlightFlags_NoRounding =
        ImGuiNavRenderCursorFlags_NoRounding, // Renamed in 1.91.4
#endif
};
...
enum ImGuiNavMoveFlags_ {
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId =
        1 << 4, // Allow scoring and considering the current NavId as a move
                ...
                // request, if we are pressing PageDown from the bottom-most
                // item we need to stay in place)
    ...
    ImGuiNavMoveFlags_ScrollToEdgeY =
        1 << 6, // Force scrolling to min/max (used by Home/End) // FIXME-NAV:
                // Aim to remove or reword, probably unnecessary
    ...
    ImGuiNavMoveFlags_IsPageMove = 1
                                   << 11, // Identify a PageDown/PageUp request.
    ...
    ImGuiNavMoveFlags_NoSetNavCursorVisible =
        1 << 14, // Do not alter the nav cursor visible state
...
struct ImGuiNavItemData {
    ...
    float
        DistBox; //      Move    // Best candidate box distance to current NavId
...
enum ImGuiTypingSelectFlags_ {
    ...
    ImGuiTypingSelectFlags_AllowBackspace =
        1 << 0, // Backspace to delete character inputs. If using: ensure
                // GetTypingSelectRequest() is not called more than once per
                // frame (filter by e.g. focus state)
...
struct IMGUI_API ImGuiTypingSelectRequest {
    ...
    int SearchBufferLen;
    const char* SearchBuffer; // Search buffer contents (use full string. unless
                              // SingleCharMode is set, in which case use
                              // SingleCharSize).
    bool SelectRequest; // Set when buffer was modified this frame, requesting a
                        // selection.
    bool SingleCharMode; // Notify when buffer contains same character repeated,
                         // to implement special mode. In this situation it
                         // preferred to not display any on-screen search
                         // indication.
    ImS8 SingleCharSize; // Length in bytes of first letter codepoint (1 for
                         // ascii, 2-4 for UTF-8). If
                         // (SearchBufferLen==RepeatCharSize) only 1 letter has
                         // been input.
};
...
enum ImGuiOldColumnFlags_ {
    ...
    ImGuiOldColumnFlags_GrowParentContentsSize =
        1 << 4, // Restore pre-1.51 behavior of extending the parent window
                // contents size but _without affecting the columns width at
                // all_. Will eventually remove.
...
struct ImGuiBoxSelectState {
    ...
    ImVec2 StartPosRel; // Start position in window-contents relative space (to
                        // support scrolling)
    ImVec2 EndPosRel;   // End position in window-contents relative space
    ImVec2
        ScrollAccum; // Scrolling accumulator (to behave at high-frame spaces)
    ImGuiWindow* Window;

    // Temporary/Transient data
    bool UnclipMode;   // (Temp/Transient, here in hot area). Set/cleared by the
                       // BeginMultiSelect()/EndMultiSelect() owning active
                       // box-select.
    ImRect UnclipRect; // Rectangle where ItemAdd() clipping may be temporarily
                       // disabled. Need support by multi-select supporting
                       // widgets.
    ImRect BoxSelectRectPrev; // Selection rectangle in absolute coordinates
                              // (derived every frame from BoxSelectStartPosRel
                              // and MousePos)
...
struct IMGUI_API ImGuiMultiSelectTempData {
    ImGuiMultiSelectIO IO; // MUST BE FIRST FIELD. Requests are set and returned
                           // by BeginMultiSelect()/EndMultiSelect() + written
                           // to by user during the loop.
    ImGuiMultiSelectState* Storage;
    ImGuiID FocusScopeId; // Copied from g.CurrentFocusScopeId (unless another
                          // selection scope was pushed manually)
    ImGuiMultiSelectFlags Flags;
    ImVec2 ScopeRectMin;
    ImVec2 BackupCursorMaxPos;
    ImGuiSelectionUserData
        LastSubmittedItem; // Copy of last submitted item data, used to merge
                           // output ranges.
    ImGuiID BoxSelectId;
    ImGuiKeyChord KeyMods;
    ImS8 LoopRequestSetAll; // -1: no operation, 0: clear all, 1: select all.
    bool IsEndIO;           // Set when switching IO from BeginMultiSelect() to
                            // EndMultiSelect() state.
    bool IsFocused; // Set if currently focusing the selection scope (any item
                    // of the selection). May be used if you have custom
                    // shortcut associated to selection.
    bool IsKeyboardSetRange; // Set by BeginMultiSelect() when using
                             // Shift+Navigation. Because scrolling may be
                             // affected we can't afford a frame of lag with
                             // Shift+Navigation.
    bool NavIdPassedBy;
    bool RangeSrcPassedBy; // Set by the item that matches RangeSrcItem.
    bool RangeDstPassedBy; // Set by the item that matches NavJustMovedToId when
                           // IsSetRange is set.
...
struct IMGUI_API ImGuiMultiSelectState {
    ...
    int LastSelectionSize; // Set by BeginMultiSelect() based on optional info
                           // provided by user. May be -1 if unknown.
    ImS8 RangeSelected;    // -1 (don't have) or true/false
    ImS8 NavIdSelected;    // -1 (don't have) or true/false
    ImGuiSelectionUserData RangeSrcItem; //
    ImGuiSelectionUserData NavIdItem;    // SetNextItemSelectionUserData() value
                                      // for NavId (if part of submitted items)
...
enum ImGuiDockNodeFlagsPrivate_ {
    ...
    ImGuiDockNodeFlags_CentralNode =
        1
        << 11, // Saved // The central node has 2 main properties: stay visible
               // when empty, only use "remaining" spaces from its neighbor.
    ImGuiDockNodeFlags_NoTabBar =
        1 << 12, // Saved // Tab bar is completely unavailable. No triangle in
                 // the corner to enable it back.
    ImGuiDockNodeFlags_HiddenTabBar =
        1 << 13, // Saved // Tab bar is hidden, with a triangle in the corner to
                 // show it again (NB: actual tab-bar instance may be destroyed
                 // as this is only used for single-window tab bar)
    ...
    // because the desirable sharing/inheriting/copy-flag-on-split behaviors are
    // quite difficult to design and understand. The two public flags
    ...
    ImGuiDockNodeFlags_NoDockingSplitOther =
        1
        << 19, //       // Disable this node from splitting other windows/nodes.
    ...
    ImGuiDockNodeFlags_NoResizeFlagsMask_ = ( int )ImGuiDockNodeFlags_NoResize |
                                            ImGuiDockNodeFlags_NoResizeX |
                                            ImGuiDockNodeFlags_NoResizeY,

    // When splitting, those local flags are moved to the inheriting child,
    // never duplicated
    ImGuiDockNodeFlags_LocalFlagsTransferMask_ =
        ( int )ImGuiDockNodeFlags_NoDockingSplit |
        ImGuiDockNodeFlags_NoResizeFlagsMask_ |
        ( int )ImGuiDockNodeFlags_AutoHideTabBar |
...
struct IMGUI_API ImGuiDockNode {
    ...
    ImGuiDockNode*
        ChildNodes[ 2 ]; // [Split node only] Child nodes (left/right or
    ...
    ImVec2 SizeRef; // [Split node only] Last explicitly written-to size
                    // (overridden when using a splitter affecting the node),
    ...
    bool IsVisible : 1; // Set to false when the node is hidden (usually
                        // disabled as it has no active window)
    bool IsFocused : 1;
    bool IsBgDrawnThisFrame : 1;
    bool HasCloseButton
        : 1; // Provide space for a close button (if any of the docked window
             // has one). Note that button may be hidden on window without one.
...
struct ImGuiViewportP : public ImGuiViewport {
    ...
    bool LastFocusedHadNavWindow; // Instead of maintaining a LastFocusedWindow
                                  // (which may harder to correctly maintain),
                                  // we merely store weither NavWindow != NULL
                                  // last time the viewport was focused.
    short PlatformMonitor;
    int BgFgDrawListsLastFrame[ 2 ]; // Last frame number the background (0) and
                                     // foreground (1) draw lists were used
    ImDrawList*
        BgFgDrawLists[ 2 ]; // Convenience background (0) and foreground (1)
                            // draw lists. We use them to draw software mouser
                            // cursor when io.MouseDrawCursor is set and to draw
                            // most debug overlays.
    ...
    // - BeginMainMenuBar() and DockspaceOverViewport() tend to use work area to
    // avoid stepping over existing contents.
    // - Generally 'safeAreaInsets' in iOS land, 'DisplayCutout' in Android
    // land.
    ImVec2 WorkInsetMin; // Work Area inset locked for the frame. GetWorkRect()
                         // always fits within GetMainRect().
    ImVec2 WorkInsetMax; // "
    ImVec2 BuildWorkInsetMin; // Work Area inset accumulator for current frame,
                              // to become next frame's WorkInset
...
struct ImGuiWindowSettings {
    ...
    short
        DockOrder; // Order of the last time the window was visible within its
...
struct ImGuiSettingsHandler {
    ...
    void* ( *ReadOpenFn )(
        ...
        const char* name ); // Read: Called when entering into a new ini entry
                            // e.g. "[Window][Name]"
...
// - Only dispatch error if _EXPR: evaluate as assert (similar to an assert
// macro).
// - The message will always be a string literal, in order to increase
// likelihood of being display by an assert handler.
// - See 'Demo->Configuration->Error Handling' and ImGuiIO definitions for
// details on error handling.
// - Read https://github.com/ocornut/imgui/wiki/Error-Handling for details on
// error handling.
...
// The error callback is currently not public, as it is expected that only
// advanced users will rely on it.
...
enum ImGuiDebugLogFlags_ {
    ...
    ImGuiDebugLogFlags_EventError =
        1 << 0, // Error submitted by IM_ASSERT_USER_ERROR()
    ...
    ImGuiDebugLogFlags_EventMask_ =
        ...
        ImGuiDebugLogFlags_EventNav | ImGuiDebugLogFlags_EventClipper |
        ImGuiDebugLogFlags_EventSelection | ImGuiDebugLogFlags_EventIO |
        ImGuiDebugLogFlags_EventFont | ImGuiDebugLogFlags_EventInputRouting |
...
struct ImGuiStackLevelInfo {
    ...
    char Desc[ 57 ]; // Arbitrarily sized buffer to hold a result (FIXME: could
                     // replace Results[] with a chunk stream?) FIXME: Now that
                     // we added CTRL+C this should be fixed.
...
struct ImGuiIDStackTool {
    int LastActiveFrame;
    int StackLevel;  // -1: query stack and resize Results, >= 0: individual
                     // stack level
...
struct ImGuiContext {
    ...
    float FontSize; // (Shortcut) == FontBaseSize *
                    // g.CurrentWindow->FontWindowScale == window->FontSize().
    ...
    char ContextName[ 16 ]; // Storage for a context name (to facilitate
                            // debugging multi-context setups)
    ...
    ImVector< ImGuiInputEvent >
        InputEventsQueue; // Input events which will be trickled/written into IO
                          // structure.
    ImVector< ImGuiInputEvent >
        InputEventsTrail; // Past input events processed in NewFrame(). This is
                          // to allow domain-specific application to access e.g
                          // mouse/pen trail.
    ...
    ImVector< ImGuiWindow* > WindowsFocusOrder; // Root windows, sorted in focus
                                                // order, back to front.
    ImVector< ImGuiWindow* >
    ...
    ImGuiWindow*
        HoveredWindowBeforeClear; // Window the mouse is hovering. Filled even
                                  // with _NoMouse. This is currently useful for
                                  // multi-context compositors.
    ImGuiWindow*
        MovingWindow; // Track the window we clicked on (in order to preserve
                      // focus). The actual window that is moved is generally
                      // MovingWindow->RootWindowDockTree.
    ImGuiWindow* WheelingWindow; // Track the window we started mouse-wheeling
                                 // on. Until a timer elapse or mouse has moved,
                                 // generally keep scrolling the same window
                                 // even if during the course of scrolling the
                                 // mouse ends up hovering a child window.
    ...
    ImGuiID DebugDrawIdConflicts; // Set when we detect multiple items with the
                                  // same identifier
    ImGuiID DebugHookIdInfo; // Will call core hooks: DebugHookIdInfo() from
                             // GetID functions, used by ID Stack Tool [next
                             // HoveredId/ActiveId to not pull in an extra
                             // cache-line]
    ...
    bool HoveredIdIsDisabled; // At least one widget passed the rect test, but
                              // has been discarded by disabled flag or popup
                              // inhibit. May be true even if HoveredId == 0.
    bool ItemUnclipByLog;     // Disable ItemAdd() clipping, essentially a
                              // memory-locality friendly copy of LogEnabled
    ImGuiID ActiveId;         // Active widget
    ImGuiID
        ActiveIdIsAlive; // Active widget has been seen this frame (we can't use
    ...
    bool ActiveIdAllowOverlap; // Active widget allows another widget to steal
                               ...
                               // but not always)
    bool ActiveIdNoClearOnFocusLoss;   // Disable losing active id if the active
                                       // id window gets unfocused.
    bool ActiveIdHasBeenPressedBefore; // Track whether the active id led to a
                                       // press (this is to allow changing
                                       // between PressOnClick and
                                       // PressOnRelease without pressing
                                       // twice). Used by range_select branch.
    ...
    ImGuiDataTypeStorage
        ActiveIdValueOnActivation; // Backup of initial value at the time of
                                   // activation. ONLY SET BY SPECIFIC WIDGETS:
                                   // DragXXX and SliderXXX.
    ImGuiID
    ...
    // ImGuiKeyOwner_NoOwner (== -1) or a custom ID.
    // - Routing is requested ahead of time for a given chord (Key + Mods) and
    // granted in NewFrame().
    double
        LastKeyModsChangeTime; // Record the last time key mods changed (affect
                               // repeat delay when using shortcut logic)
    double
        LastKeyModsChangeFromNoneTime; // Record the last time key mods changed
                                       // away from being 0 (affect repeat delay
                                       // when using shortcut logic)
    ...
    bool ActiveIdUsingAllKeyboardKeys; // Active widget will want to read all
                                       // keyboard keys inputs. (this is a
                                       // shortcut for not taking ownership of
                                       // 100+ keys, frequently used by drag
                                       // operations)
    ImGuiKeyChord
        DebugBreakInShortcutRouting; // Set to break in
                                     // SetShortcutRouting()/Shortcut() calls.
    // ImU32                 ActiveIdUsingNavInputMask;          // [OBSOLETE]
    // Since (IMGUI_VERSION_NUM >= 18804) : 'g.ActiveIdUsingNavInputMask |= (1
    // << ImGuiNavInput_Cancel);' becomes --> 'SetKeyOwner(ImGuiKey_Escape,
    ...
    ImGuiID DebugLocateId; // Storage for DebugLocateItemOnHover() feature: this
                           // is read by ItemAdd() so we keep it in a hot/cached
                           // location
    ...
    // Shared stacks
    ImGuiCol DebugFlashStyleColorIdx; // (Keep close to ColorStack to share
                                      // cache line)
    ...
    ImVector< ImGuiFocusScopeData >
        FocusScopeStack; // Stack for PushFocusScope()/PopFocusScope() -
                         // inherited by BeginChild(), pushed into by Begin()
    ...
    ImVector< ImGuiPopupData > BeginPopupStack; // Which level of BeginPopup()
                                                // we are in (reset every frame)
    ...
    ImVector< ImGuiViewportP* >
        Viewports; // Active viewports (always 1+, and generally 1 unless
                   // multi-viewports are enabled). Each viewports hold their
                   // copy of ImDrawData.
    ImGuiViewportP*
    ...
    ImGuiViewportP*
        MouseLastHoveredViewport; // Last known viewport that was hovered by
                                  // mouse (even if we are not hovering any
                                  // viewport any more) + honoring the _NoInputs
                                  // flag.
    ...
    ImRect
        PlatformMonitorsFullWorkRect; // Bounding box of all platform monitors
    int ViewportCreatedCount; // Unique sequential creation counter (mostly for
                              // testing/debugging)
    int PlatformWindowsCreatedCount; // Unique sequential creation counter
                                     // (mostly for testing/debugging)
    int ViewportFocusedStampCount; // Every time the front-most window changes,
                                   // we stamp its viewport with an incrementing
                                   // counter

    // Keyboard/Gamepad Navigation
    bool NavCursorVisible; // Nav focus cursor/rectangle is visible? We hide it
                           // after a mouse click. We show it after a nav move.
    bool NavHighlightItemUnderNav; // Disable mouse hovering highlight.
                                   // Highlight navigation focused item instead
                                   // of mouse hovered item.
    // bool                  NavDisableHighlight;                // Old name for
    // !g.NavCursorVisible before 1.91.4 (2024/10/18). OPPOSITE VALUE
    // (g.NavDisableHighlight == !g.NavCursorVisible) bool NavDisableMouseHover;
    // // Old name for g.NavHighlightItemUnderNav before 1.91.1 (2024/10/18)
    // this was called When user starts using keyboard/gamepad, we hide mouse
    // hovering highlight until mouse is touched again.
    bool NavMousePosDirty; // When set we will update mouse position if
                           // io.ConfigNavMoveSetMousePos is set (not enabled by
                           // default)
    ...
    ImGuiID
        NavActivateId; // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space)
                       ...
                       // IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId :
    ...
    ImGuiID
        NavNextActivateId; // Set by ActivateItem(), queued until next frame.
    ...
    ImGuiSelectionUserData
        NavLastValidSelectionUserData; // Last valid data passed to
                                       ...
                                       // current window. Not reset when
                                       // focusing an item that doesn't have
                                       // selection data.
    ...
    ImGuiNavItemData
        NavTabbingResultFirst; // First tabbing request candidate within
                               // NavWindow and flattened hierarchy

    // Navigation: record of last move request
    ImGuiID
        NavJustMovedFromFocusScopeId; // Just navigated from this focus scope id
    ...
    ImGuiID
        NavJustMovedToFocusScopeId; // Just navigated to this focus scope id
                                    // (result of a successfully MoveRequest).
    ...
    ImGuiKeyChord
        ConfigNavWindowingKeyNext; // = ImGuiMod_Ctrl | ImGuiKey_Tab (or
                                   // ImGuiMod_Super | ImGuiKey_Tab on OS X).
                                   // For reconfiguration (see #4828)
    ImGuiKeyChord
        ConfigNavWindowingKeyPrev;   // = ImGuiMod_Ctrl | ImGuiMod_Shift |
                                     // ImGuiKey_Tab (or ImGuiMod_Super |
                                     // ImGuiMod_Shift | ImGuiKey_Tab on OS X)
    ImGuiWindow* NavWindowingTarget; // Target window when doing CTRL+Tab (or
                                     // Pad Menu + FocusPrev/Next), this window
    ...
    bool DragDropWithinSource; // Set when within a
                               // BeginDragDropXXX/EndDragDropXXX block for a
                               // drag source.
    ...
    ImRect DragDropTargetRect; // Store rectangle of current target candidate
                               // (we favor small targets when overlapping)
    ImRect
        DragDropTargetClipRect; // Store ClipRect at the time of item's drawing
    ImGuiID DragDropTargetId;
    ImGuiDragDropFlags DragDropAcceptFlags;
    float DragDropAcceptIdCurrRectSurface; // Target item surface (we resolve
                                           // overlapping targets by
    ...
    int MultiSelectTempDataStacked; // Temporary multi-select data size (because
                                    // we leave previous instances undestructed,
    ...
    float HoverItemDelayClearTimer; // Currently used by IsItemHovered(): grace
                                    // time before g.TooltipHoverTimer gets
                                    // cleared.
    ImGuiID HoverItemUnlockedStationaryId; // Mouse has once been stationary on
                                           // this item. Only reset after
                                           // departing the item.
    ImGuiID HoverWindowUnlockedStationaryId; // Mouse has once been stationary
                                             // on this window. Only reset after
    ...
    ImGuiID TempInputId; // Temporary text input when CTRL+clicking on a slider,
                         // etc.
    ...
    ImGuiID ColorEditCurrentID; // Set temporarily while inside of the
                                // parent-most ColorEdit4/ColorPicker4 (because
                                // they call each others).
    ImGuiID ColorEditSavedID;   // ID we are saving/restoring HS for
    float ColorEditSavedHue; // Backup of last Hue associated to LastColor, so
                             // we can restore Hue in lossy RGB<>HSV round trips
    float ColorEditSavedSat; // Backup of last Saturation associated to
                             // LastColor, so we can restore Saturation in lossy
                             // RGB<>HSV round trips
    ...
    short ScrollbarSeekMode; // 0: scroll to clicked location, -1/+1: prev/next
                             // page.
    float ScrollbarClickDeltaToGrabCenter; // When scrolling to mouse location:
                                           // distance between mouse and center
                                           // of grab box, normalized in parent
                                           // space.
    ...
    ImGuiWindow* TooltipPreviousWindow; // Window of last tooltip submitted
                                        // during the frame
    ImVector< char >
        ClipboardHandlerData; // If no custom clipboard handler is defined
    ImVector< ImGuiID >
        MenusIdSubmittedThisFrame; // A list of menu IDs that were rendered at
                                   // least once
    ...
    ImGuiPlatformImeData
        PlatformImeDataPrev; // Previous frame data. When changed we call the
    ...
    // Settings
    bool SettingsLoaded;
    float SettingsDirtyTimer; // Save .ini Settings to memory when time reaches
                              // zero
    ...
    int LogDepthToExpandDefault; // Default/stored value for LogDepthMaxExpand
                                 // if not specified in the LogXXX function
                                 // call.

    // Error Handling
    ImGuiErrorCallback
        ErrorCallback; // = NULL. May be exposed in public API eventually.
    void* ErrorCallbackUserData; // = NULL
    ImVec2 ErrorTooltipLockedPos;
    bool ErrorFirst;
    int ErrorCountCurrentFrame; // [Internal] Number of errors submitted this
                                // frame.
    ...
    ImU8 DebugLocateFrames;    // For DebugLocateItemOnHover(). This is used
    ...
    // Misc
    float FramerateSecPerFrame[ 60 ]; // Calculate estimate of framerate for
                                      // user over the last 60 frames..
...
struct IMGUI_API ImGuiWindowTempData {
    ...
    ImVec1 ColumnsOffset; // Offset to the current column (if ColumnsCurrent >
                          // 0). FIXME: This and the above should be a stack to
                          // allow use cases like Tree->Column->Tree. Need
                          // revamp columns API.
    ImVec1 GroupOffset;
    ImVec2 CursorStartPosLossyness; // Record the loss of precision of
                                    // CursorStartPos due to really large
                                    // scrolling amount. This is used by clipper
                                    // to compensate and fix the most common use
                                    // case of large scroll area.
    ...
    ImU32 TreeHasStackDataDepthMask; // Store whether given depth has
                                     // ImGuiTreeNodeStackData data. Could be
                                     // turned into a ImU64 if necessary.
    ...
    // allows us to not heap allocate for short-lived windows which are not
    // using those settings.
    float ItemWidth;   // Current item width (>0.0: width in pixels, <0.0: align
                       // xx pixels to the right of window).
    float TextWrapPos; // Current text wrap pos.
    ImVector< float >
        ItemWidthStack; // Store item widths to restore (attention: .back() is
                        // not == ItemWidth)
    ImVector< float >
        TextWrapPosStack; // Store text wrap pos to restore (attention: .back()
                          // is not == TextWrapPos)
};
...
struct IMGUI_API ImGuiWindow {
    ...
    ImGuiID
        ViewportId; // We backup the viewport id (since the viewport may
    ...
    float TitleBarHeight,
        MenuBarHeight; // Note that those used to be function before 2024/05/28.
                       // If you have old code calling TitleBarHeight() you can
                       // change it to TitleBarHeight.
    ...
    ImGuiID ChildId;     // ID of corresponding item in parent window (for
    ...
    ImVec2 Scroll;
    ImVec2 ScrollMax;
    ImVec2 ScrollTarget; // target scroll position. stored as cursor position
                         // with scrolling canceled out, so the highest point is
                         // always 0.0f. (FLT_MAX for no change)
    ImVec2 ScrollTargetCenterRatio;  // 0.0f = scroll so that target position is
                                     // at top, 0.5f = scroll so that target
    ...
    ImVec2 ScrollbarSizes; // Size taken by each scrollbars on their smaller
                           // axis. Pay attention! ScrollbarSizes.x == width of
                           // the vertical scrollbar, ScrollbarSizes.y = height
    ...
    short
        BeginCountPreviousFrame; // Number of Begin() during the previous frame
    short
        BeginOrderWithinParent; // Begin() order within immediate parent window,
    ...
    ImGuiCond SetWindowCollapsedAllowFlags
        : 8; // store acceptable condition flags for SetNextWindowCollapsed()
             // use.
    ...
    ImRect WorkRect; // Initially covers the whole scrolling region. Reduced by
                     ...
                     // WindowPadding*1.0f on each side. This is meant to
    ...
    ImRect ParentWorkRect; // Backup of WorkRect before entering a container
                           ...
                           // SpanAllColumns functions to easily access. Stacked
                           // containers are responsible for maintaining this.
                           // // FIXME-WORKRECT: Could be a stack?
    ImRect ClipRect; // Current clipping/scissoring rectangle, evolve as we are
                     // using PushClipRect(), etc. ==
                     // DrawList->clip_rect_stack.back().
    ImRect ContentRegionRect; // FIXME: This is currently confusing/misleading.
                              // It is essentially WorkRect but not handling of
                              // scrolling. We currently rely on it as
                              // right/bottom aligned sizing operation need some
                              // size to rely on.
    ...
    float LastTimeActive; // Last timestamp the window was Active (using float
                          // as we don't need high precision there)
    ...
    float FontDpiScale;
    float FontRefSize;  // This is a copy of window->CalcFontSize() at the time
                        // of Begin(), trying to phase out CalcFontSize()
                        // especially as it may be called on non-current window.
    ...
    ImGuiWindow*
        ParentWindow; // If we are a child _or_ popup _or_ docked window, this
    ...
    ImGuiWindow* RootWindowPopupTree; // Point to ourself or first ancestor that
                                      // is not a child window. Cross through
                                      // popups parent<>child.
    ImGuiWindow*
    ...
    ImGuiWindow*
        ParentWindowForFocusRoute; // Set to manual link a window to its logical
                                   // parent so that Shortcut() chain are
                                   // honoerd (e.g. Tool linked to Document)

    ImGuiWindow*
        NavLastChildNavWindow; // When going to the menu bar, we remember the
                               // child window we came from. (This could
                               // probably be made implicit if we kept g.Windows
                               // sorted by last focused including child
                               // window.)
    ...
    ImVec2 NavPreferredScoringPosRel
        [ ImGuiNavLayer_COUNT ]; // Preferred X/Y position updated when moving
                                 // on a given axis, reset to FLT_MAX.
    ImGuiID NavRootFocusScopeId; // Focus Scope ID at the time of Begin()

    int MemoryDrawListIdxCapacity; // Backup of last idx/vtx count, so when
                                   // waking up the window we can preallocate
                                   // and avoid iterative alloc/copy
...
enum ImGuiTabBarFlagsPrivate_ {
    ...
    ImGuiTabBarFlags_SaveSettings =
        1 << 22, // FIXME: Settings are handled by the docking system, this only
                 // request the tab bar to mark settings dirty when reordering
                 // tabs
};
...
enum ImGuiTabItemFlagsPrivate_ {
    ...
    ImGuiTabItemFlags_Unsorted =
        1 << 23, // [Docking] Trailing tabs with the _Unsorted flag will be
...
struct ImGuiTabItem {
    ...
    float
        RequestedWidth; // Width optionally requested by caller, -1.0f is unused
    ...
    ImS16 IndexDuringLayout; // Index only used during TabBarLayout(). Tabs gets
                             // reordered so 'Tabs[n].IndexDuringLayout == n'
                             // but may mismatch during additions.
...
struct IMGUI_API ImGuiTabBar {
    ...
    float ItemSpacingY;
    ImVec2
        FramePadding; // style.FramePadding locked at the time of BeginTabBar()
    ImVec2 BackupCursorPos;
    ImGuiTextBuffer TabsNames; // For non-docking tab bar we re-append names in
                               // a contiguous buffer.
...
struct ImGuiTableColumn {
    ...
    float InitStretchWeightOrWidth; // Value passed to TableSetupColumn(). For
                                    // Width it is a content width (_without
                                    // padding_).
    ImRect ClipRect;                // Clipping rectangle for the column
    ImGuiID UserID; // Optional, value passed to TableSetupColumn()
    float
        WorkMinX; // Contents region min ~(MinX + CellPaddingX + CellSpacingX1)
                  // == cursor start position when entering column
    float
        WorkMaxX; // Contents region max ~(MaxX - CellPaddingX - CellSpacingX2)
    ...
    float ContentMaxXHeadersUsed; // Contents maximum position for headers rows
                                  ...
                                  // automatically softclip itself + report
                                  // ideal desired size, to avoid creating
                                  // extraneous draw calls
    float ContentMaxXHeadersIdeal;
    ImS16 NameOffset; // Offset into parent ColumnsNames[]
    ImGuiTableColumnIdx
        DisplayOrder; // Index within Table's IndexToDisplayOrder[] (column may
                      // be reordered by users)
    ImGuiTableColumnIdx IndexWithinEnabledSet; // Index within enabled/visible
                                               // set (<= IndexToDisplayOrder)
    ImGuiTableColumnIdx
    ...
    ImGuiTableColumnIdx
        SortOrder; // Index of this column within sort specs, -1 if not sorting
                   // on this column, 0 for single-sort, may be >0 on multi-sort
    ...
    bool IsEnabled; // IsUserEnabled && (Flags & ImGuiTableColumnFlags_Disabled)
                    // == 0
    bool IsUserEnabled; // Is the column not marked Hidden by the user?
                        // (unrelated to being off view, e.g. clipped by
                        // scrolling).
...
struct ImGuiTableInstanceData {
    ...
    float
        LastTopHeadersRowHeight; // Height of first consecutive header rows from
...
struct IMGUI_API ImGuiTable {
    ...
    ImBitArrayPtr
        EnabledMaskByIndex; // Column Index -> IsEnabled map (== not hidden by
                            // user/api) in a format adequate for iterating
                            // column without touching cold data
    ImBitArrayPtr VisibleMaskByIndex; // Column Index -> IsVisibleX|IsVisibleY
                                      // map (== not hidden by user/api && not
    ...
    int CurrentRow;
    int CurrentColumn;
    ImS16 InstanceCurrent; // Count of BeginTable() calls with same ID in the
                           // same frame (generally 0). This is a little bit
                           // similar to BeginCount for a window, but multiple
                           // table with same ID look are multiple tables, they
                           // are just synched.
    ImS16 InstanceInteracted; // Mark which instance (generally 0) of the same
                              // ID is being interacted with
    float RowPosY1;
    float RowPosY2;
    float RowMinHeight; // Height submitted to TableNextRow()
    float
        RowCellPaddingY; // Top and bottom padding. Reloaded during row change.
    ...
    float OuterPaddingX;
    float CellPaddingX;  // Padding from each borders. Locked in
                         // BeginTable()/Layout.
    ...
    float ColumnsAutoFitWidth; // Sum of ideal column width in order nothing to
                               // be clipped, used for auto-fitting and content
                               // width submission in outer window
    ...
    float ResizeLockMinContentsX2; // Lock minimum contents width while resizing
                                   // down in order to not create feedback
    ...
    ImRect InnerRect; // InnerRect but without decoration. As with OuterRect,
                      // for non-scrolling tables, InnerRect.Max.y is
    ImRect WorkRect;
    ImRect InnerClipRect;
    ImRect BgClipRect; // We use this to cpu-clip cell background color fill,
                       // evolve during the frame as we cross frozen rows
                       // boundaries
    ImRect Bg0ClipRectForDrawCmd; // Actual ImDrawCmd clip rect for BG0/1
                                  // channel. This tends to be ==
                                  // OuterWindow->ClipRect at BeginTable()
    ...
    ImRect HostClipRect; // This is used to check if we can eventually merge our
                         ...
                         // current window.
    ImRect
        HostBackupInnerClipRect; // Backup of InnerWindow->ClipRect during
                                 // PushTableBackground()/PopTableBackground()
    ImGuiWindow* OuterWindow;    // Parent window for the table
    ImGuiWindow* InnerWindow; // Window holding the table data (== OuterWindow
                              // or a child window)
    ...
    ImVector< ImGuiTableColumnSortSpecs >
        SortSpecsMulti; // FIXME-OPT: Using a small-vector pattern would be
                        // good.
    ...
    ImGuiTableColumnIdx
        FreezeRowsCount; // Actual frozen row count (== FreezeRowsRequest, or ==
                         // 0 when no scrolling offset)
    ImGuiTableColumnIdx FreezeColumnsRequest; // Requested frozen columns count
    ImGuiTableColumnIdx FreezeColumnsCount;   // Actual frozen columns count (==
                                              // FreezeColumnsRequest, or == 0
                                              // when no scrolling offset)
    ImGuiTableColumnIdx RowCellDataCurrent;   // Index of current RowCellData[]
                                              // entry in current row
    ImGuiTableDrawChannelIdx
    ...
    bool DisableDefaultContextMenu; // Disable default context menu contents.
                                    // You may submit your own using
                                    // TableBeginContextMenuPopup()/EndPopup()
    bool IsSettingsRequestLoad;
    bool IsSettingsDirty; // Set when table settings have changed and needs to
                          // be reported into ImGuiTableSetttings data.
...
// ImGuiTableTempData: e.g. SortSpecs. sizeof() ~ 136 bytes.
struct IMGUI_API ImGuiTableTempData {
    ...
    ImVec2 HostBackupCursorMaxPos;  // Backup of InnerWindow->DC.CursorMaxPos at
                                    // the end of BeginTable()
    ImVec1 HostBackupColumnsOffset; // Backup of OuterWindow->DC.ColumnsOffset
...
// "Debug" window) If this ever crashes because g.CurrentWindow is NULL, it
// means that either:
...
IMGUI_API void SetWindowPos( ImGuiWindow* window,
                             const ImVec2& pos,
                             ImGuiCond cond = 0 );
IMGUI_API void SetWindowSize( ImGuiWindow* window,
                              const ImVec2& size,
                              ImGuiCond cond = 0 );
IMGUI_API void SetWindowCollapsed( ImGuiWindow* window,
                                   bool collapsed,
                                   ImGuiCond cond = 0 );
IMGUI_API void SetWindowHitTestHole( ImGuiWindow* window,
                                     const ImVec2& pos,
                                     const ImVec2& size );
...
IMGUI_API ImGuiWindow* FindBottomMostVisibleWindowWithinBeginStack(
    ImGuiWindow* window );
...
IMGUI_API void FindHoveredWindowEx(
    const ImVec2& pos,
...
IMGUI_API void RemoveContextHook( ImGuiContext* context,
...
IMGUI_API void SetWindowViewport( ImGuiWindow* window,
                                  ImGuiViewportP* viewport );
IMGUI_API void SetCurrentViewport( ImGuiWindow* window,
                                   ImGuiViewportP* viewport );
IMGUI_API const ImGuiPlatformMonitor* GetViewportPlatformMonitor(
...
IMGUI_API ImGuiWindowSettings* FindWindowSettingsByWindow(
    ImGuiWindow* window );
...
IMGUI_API void SetScrollFromPosX( ImGuiWindow* window,
                                  float local_x,
                                  float center_x_ratio );
IMGUI_API void SetScrollFromPosY( ImGuiWindow* window,
                                  float local_y,
                                  float center_y_ratio );

// Early work-in-progress API (ScrollToItem() will become public)
IMGUI_API void ScrollToItem( ImGuiScrollFlags flags = 0 );
IMGUI_API void ScrollToRect( ImGuiWindow* window,
                             const ImRect& rect,
...
IMGUI_API void MarkItemEdited(
    ImGuiID id ); // Mark data associated to given item as "edited", used by
                  // IsItemDeactivatedAfterEdit() function.
IMGUI_API void PushOverrideID(
    ImGuiID id ); // Push given value as-is at the top of the ID stack (whereas
                  // PushID combines old and new hashes)
...
IMGUI_API bool ItemHoverable( const ImRect& bb,
                              ImGuiID id,
...
IMGUI_API void ShrinkWidths( ImGuiShrinkWidthItem* items,
                             int count,
                             float width_excess );
...
IMGUI_API void LogSetNextTextDecoration( const char* prefix,
                                         const char* suffix );

// Childs
IMGUI_API bool BeginChildEx( const char* name,
                             ImGuiID id,
...
IMGUI_API void ClosePopupsOverWindow(
...
IMGUI_API bool BeginViewportSideBar( const char* name,
                                     ...
                                     float size,
                                     ImGuiWindowFlags window_flags );
IMGUI_API bool BeginMenuEx( const char* label,
                            const char* icon,
                            bool enabled = true );
...
// Combos
IMGUI_API bool BeginComboPopup( ImGuiID popup_id,
                                const ImRect& bb,
...
IMGUI_API void NavMoveRequestSubmit( ImGuiDir move_dir,
                                     ImGuiDir clip_dir,
...
IMGUI_API void NavMoveRequestResolveWithPastTreeNode(
...
IMGUI_API void SetNavID( ImGuiID id,
...
IMGUI_API int CalcTypematicRepeatAmount( float t0,
                                         float t1,
                                         float repeat_delay,
                                         float repeat_rate );
...
IMGUI_API void SetKeyOwner( ImGuiKey key,
                            ImGuiID owner_id,
                            ImGuiInputFlags flags = 0 );
IMGUI_API void SetKeyOwnersForKeyChord( ImGuiKeyChord key,
                                        ImGuiID owner_id,
                                        ImGuiInputFlags flags = 0 );
IMGUI_API void SetItemKeyOwner(
    ImGuiKey key,
    ImGuiInputFlags
        flags ); // Set key owner to last item if it is hovered or active.
...
IMGUI_API bool IsKeyPressed(
    ...
    ImGuiID owner_id =
        0 ); // Important: when transitioning from old to new IsKeyPressed():
...
// - When a policy (except for ImGuiInputFlags_RouteAlways *) is set, Shortcut()
// will register itself with SetShortcutRouting(),
...
//   IsKeyChordPressed(key) and bypassing route registration and check)
// - When using one of the routing option:
//   - The default route is ImGuiInputFlags_RouteFocused (accept inputs if
//   window is in focus stack. Deep-most focused window takes inputs. ActiveId
//   takes inputs over deep-most focused window.)
//   - Routes are requested given a chord (key + modifiers) and a routing
//   policy.
...
//   - Each route may be granted to a single owner. When multiple requests are
//   made we have policies to select the winning route (e.g. deep most window).
//   - Multiple read sites may use the same owner id can all access the granted
//   route.
//   - When owner_id is 0 we use the current Focus Scope ID as a owner ID in
//   order to identify our location.
// - You can chain two unrelated windows in the focus stack using
// SetWindowParentWindowForFocusRoute()
//   e.g. if you have a tool window associated to a document, and you want
//   document shortcuts to run when the tool is focused.
IMGUI_API bool Shortcut( ImGuiKeyChord key_chord,
                         ImGuiInputFlags flags,
                         ImGuiID owner_id );
...
IMGUI_API ImGuiKeyRoutingData* GetShortcutRoutingData(
    ImGuiKeyChord key_chord );
...
IMGUI_API void DockContextClearNodes(
    ImGuiContext* ctx,
    ImGuiID root_id,
...
IMGUI_API void DockContextQueueUndockNode( ImGuiContext* ctx,
                                           ImGuiDockNode* node );
IMGUI_API void DockContextProcessUndockWindow(
    ImGuiContext* ctx,
    ImGuiWindow* window,
...
IMGUI_API bool DockContextCalcDropPosForDocking( ImGuiWindow* target,
                                                 ...
                                                 bool split_outer,
                                                 ImVec2* out_pos );
IMGUI_API ImGuiDockNode* DockContextFindNodeByID( ImGuiContext* ctx,
                                                  ImGuiID id );
...
IMGUI_API void SetWindowDock( ImGuiWindow* window,
                              ImGuiID dock_id,
                              ImGuiCond cond );
...
IMGUI_API void DockBuilderRemoveNodeDockedWindows(
    ImGuiID node_id,
...
IMGUI_API void DockBuilderCopyDockSpace(
    ImGuiID src_dockspace_id,
    ImGuiID dst_dockspace_id,
    ImVector< const char* >* in_window_remap_pairs );
IMGUI_API void DockBuilderCopyNode( ImGuiID src_node_id,
                                    ImGuiID dst_node_id,
...
// Typing-Select API
// (provide Windows Explorer style "select items by typing partial name" +
// "cycle through items by typing same letter" feature) (this is currently not
// documented nor used by main library, but should work. See
// "widgets_typingselect" in imgui_test_suite for usage code. Please let us know
// if you use this!)
IMGUI_API ImGuiTypingSelectRequest* GetTypingSelectRequest(
    ImGuiTypingSelectFlags flags = ImGuiTypingSelectFlags_None );
IMGUI_API int TypingSelectFindMatch( ImGuiTypingSelectRequest* req,
                                     int items_count,
...
IMGUI_API int TypingSelectFindNextSingleCharMatch(
    ...
    void* user_data,
    int nav_item_idx );
IMGUI_API int TypingSelectFindBestLeadingMatch(
    ImGuiTypingSelectRequest* req,
    int items_count,
    const char* ( *get_item_name_func )( void*, int ),
    void* user_data );
...
// Box-Select API
IMGUI_API bool BeginBoxSelect( const ImRect& scope_rect,
                               ImGuiWindow* window,
                               ImGuiID box_select_id,
                               ImGuiMultiSelectFlags ms_flags );
IMGUI_API void EndBoxSelect( const ImRect& scope_rect,
                             ImGuiMultiSelectFlags ms_flags );

// Multi-Select API
IMGUI_API void MultiSelectItemHeader( ImGuiID id,
                                      bool* p_selected,
                                      ImGuiButtonFlags* p_button_flags );
IMGUI_API void MultiSelectItemFooter( ImGuiID id,
                                      bool* p_selected,
                                      bool* p_pressed );
IMGUI_API void MultiSelectAddSetAll( ImGuiMultiSelectTempData* ms,
                                     bool selected );
IMGUI_API void MultiSelectAddSetRange( ImGuiMultiSelectTempData* ms,
                                       bool selected,
                                       int range_dir,
...
IMGUI_API ImGuiOldColumns* FindOrCreateColumns( ImGuiWindow* window,
                                                ImGuiID id );
...
IMGUI_API void TableAngledHeadersRowEx( ImGuiID row_id,
                                        float angle,
...
IMGUI_API void TableDrawDefaultContextMenu(
    ImGuiTable* table,
...
IMGUI_API const char* TableGetColumnName( const ImGuiTable* table,
                                          int column_n );
IMGUI_API ImGuiID TableGetColumnResizeID( ImGuiTable* table,
                                          int column_n,
                                          int instance_no = 0 );
IMGUI_API float TableCalcMaxColumnWidth( const ImGuiTable* table,
                                         int column_n );
...
IMGUI_API bool BeginTabBarEx( ImGuiTabBar* tab_bar,
                              const ImRect& bb,
                              ImGuiTabBarFlags flags );
IMGUI_API ImGuiTabItem* TabBarFindTabByID( ImGuiTabBar* tab_bar,
                                           ImGuiID tab_id );
...
IMGUI_API void TabBarQueueReorder( ImGuiTabBar* tab_bar,
                                   ImGuiTabItem* tab,
                                   int offset );
...
IMGUI_API void TabItemSpacing( const char* str_id,
                               ImGuiTabItemFlags flags,
                               float width );
...
IMGUI_API void RenderFrame( ImVec2 p_min,
                            ...
                            bool borders = true,
                            float rounding = 0.0f );
IMGUI_API void RenderFrameBorder( ImVec2 p_min,
                                  ImVec2 p_max,
                                  float rounding = 0.0f );
...
IMGUI_API void RenderNavCursor(
    const ImRect& bb,
    ImGuiID id,
    ImGuiNavRenderCursorFlags flags =
        ImGuiNavRenderCursorFlags_None ); // Navigation highlight
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
IMGUI_API void RenderCheckMark( ImDrawList* draw_list,
                                ImVec2 pos,
                                ImU32 col,
                                float sz );
IMGUI_API void RenderArrowPointingAt( ImDrawList* draw_list,
                                      ...
                                      ImGuiDir direction,
                                      ImU32 col );
IMGUI_API void RenderArrowDockMenu( ImDrawList* draw_list,
                                    ImVec2 p_min,
                                    float sz,
                                    ImU32 col );
...
// Widgets
IMGUI_API void TextEx( const char* text,
                       const char* text_end = NULL,
...
IMGUI_API void SeparatorTextEx( ImGuiID id,
                                const char* label,
...
IMGUI_API bool CollapseButton( ImGuiID id,
                               const ImVec2& pos,
...
IMGUI_API bool ScrollbarEx( const ImRect& bb,
                            ...
                            ImS64 avail_v,
                            ImS64 contents_v,
                            ImDrawFlags draw_rounding_flags = 0 );
...
IMGUI_API bool SliderBehaviorT( const ImRect& bb,
                                ...
                                ImRect* out_grab_bb );
template < typename T >
IMGUI_API T RoundScalarWithFormatT( const char* format,
                                    ImGuiDataType data_type,
                                    T v );
template < typename T >
...
IMGUI_API void DataTypeApplyOp( ImGuiDataType data_type,
                                ...
                                const void* arg_1,
                                const void* arg_2 );
IMGUI_API bool DataTypeApplyFromText( const char* buf,
                                      ImGuiDataType data_type,
                                      void* p_data,
                                      const char* format,
                                      void* p_data_when_empty = NULL );
IMGUI_API int DataTypeCompare( ImGuiDataType data_type,
                               const void* arg_1,
                               const void* arg_2 );
IMGUI_API bool DataTypeClamp( ImGuiDataType data_type,
                              void* p_data,
                              const void* p_min,
                              const void* p_max );
...
// Color
IMGUI_API void ColorTooltip( const char* text,
                             const float* col,
...
IMGUI_API void ShadeVertsLinearUV( ImDrawList* draw_list,
                                   ...
                                   const ImVec2& uv_b,
                                   bool clamp );
IMGUI_API void ShadeVertsTransformPos( ImDrawList* draw_list,
                                       int vert_start_idx,
                                       int vert_end_idx,
...
IMGUI_API void ErrorRecoveryTryToRecoverState(
    const ImGuiErrorRecoveryState* state_in );
IMGUI_API void ErrorRecoveryTryToRecoverWindowState(
    const ImGuiErrorRecoveryState* state_in );
...
IMGUI_API void DebugTextUnformattedWithLocateItem( const char* line_begin,
                                                   const char* line_end );
IMGUI_API void DebugLocateItem(
    ImGuiID target_id ); // Call sparingly: only 1 at the same time!
...
IMGUI_API void DebugHookIdInfo( ImGuiID id,
                                ImGuiDataType data_type,
                                const void* data_id,
...
IMGUI_API void DebugNodeWindowsList( ImVector< ImGuiWindow* >* windows,
                                     const char* label );
IMGUI_API void DebugNodeWindowsListByBeginStackParent(
    ImGuiWindow** windows,
    int windows_size,
    ImGuiWindow* parent_in_begin_stack );
IMGUI_API void DebugNodeViewport( ImGuiViewportP* viewport );
IMGUI_API void DebugNodePlatformMonitor( ImGuiPlatformMonitor* monitor,
                                         const char* label,
                                         int idx );
...
// TreeNodeUpdateNextOpen(id, flags); }   // Renamed in 1.89 inline bool
// IsKeyPressedMap(ImGuiKey key, bool repeat = true)                   {
// IM_ASSERT(IsNamedKey(key)); return IsKeyPressed(key, repeat); } // Removed
// in 1.87: Mapping from named key is always identity!
...
// updates. Conceptually this could be in ImGuiPlatformIO, but we are far from
...
IMGUI_API void ImFontAtlasBuildMultiplyCalcLookupTable(
    unsigned char out_table[ 256 ],
    float in_multiply_factor );
IMGUI_API void ImFontAtlasBuildMultiplyRectAlpha8(
    ...
    int h,
    int stride );
IMGUI_API void ImFontAtlasBuildGetOversampleFactors( const ImFontConfig* cfg,
