...
static int _FindPlatformMonitorForPos( const ImVec2& pos ) {
    ...
    for ( int monitor_n = 0; monitor_n < g.PlatformIO.Monitors.Size;
        ...
        if ( ImRect( monitor.MainPos, monitor.MainPos + monitor.MainSize )
                 .Contains( pos ) )
            return monitor_n;
    }
    return -1;
...
void WrapMousePos() {
    ...
    if ( g.IO.ConfigFlags & ImGuiConfigFlags_ViewportsEnable ) {
        const int monitor_index =
            _FindPlatformMonitorForPos( g.IO.MousePosPrev );
        if ( monitor_index == -1 )
            return;
        const ImGuiPlatformMonitor& monitor =
            g.PlatformIO.Monitors[ monitor_index ];
        WrapMousePosEx( ImRect(
            monitor.MainPos,
            monitor.MainPos + monitor.MainSize - ImVec2( 1.0f, 1.0f ) ) );
    } else
#endif
    {
        ImGuiViewport* viewport = GetMainViewport();
        WrapMousePosEx( ImRect( viewport->Pos, viewport->Pos + viewport->Size -
                                                   ImVec2( 1.0f, 1.0f ) ) );
    }
}
...
void LockMousePos() {
    static ImVec2
        mouse_pos_when_activated{}; // It's ok to use a static variable here,
                                    // because only one widget will ever be
