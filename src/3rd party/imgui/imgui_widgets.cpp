...
static ImVec2 InputTextCalcTextSize( ImGuiContext* ctx,
            ...
            if ( lines_skippable > 0 ) {
                ...
                while ( line < text_end && lines_skipped < lines_skippable ) {
                    ...
                    if ( !line_end )
                        line_end = text_end;
                    if ( ( flags &
                           ImGuiTextFlags_NoWidthForLargeClippedText ) == 0 )
                        text_size.x = ImMax( text_size.x,
...
void ImGui::LabelTextV( const char* label, const char* fmt, va_list args ) {
    ...
    ImFormatStringToTempBufferV( &value_text_begin, &value_text_end, fmt,
                                 args );
...
//------------------------------------------------------------------------------------------------------------------------------------------------
// The behavior of the return-value changes when ImGuiItemFlags_ButtonRepeat is
// set:
//                                         Repeat+                  Repeat+
//                                         Repeat+             Repeat+
...
// - FIXME: For refactor we could output flags, incl mouse hovered vs nav
// keyboard vs nav triggered etc.
//   And better standardize how widgets use 'GetColor32((held && hovered) ? ...
//   : hovered ? ...)' vs 'GetColor32(held ? ... : hovered ? ...);' For mouse
//   feedback we typically prefer the 'held && hovered' test, but for nav
//   feedback not always. Outputting hovered=true on Activation may be
//   misleading.
// - Since v1.91.2 (Sept 2024) we included io.ConfigDebugHighlightIdConflicts
// feature.
//   One idiom which was previously valid which will now emit a warning is when
//   using multiple overlayed ButtonBehavior() with same ID and different
//   MouseButton (see #8030). You can fix it by:
//       (1) switching to use a single ButtonBehavior() with multiple
//       _MouseButton flags.
//    or (2) surrounding those calls with
//    PushItemFlag(ImGuiItemFlags_AllowDuplicateId, true); ... PopItemFlag()
bool ImGui::ButtonBehavior( const ImRect& bb,
    ...
    ImGuiItemFlags item_flags =
        ( g.LastItemData.ID == id ? g.LastItemData.ItemFlags
                                  : g.CurrentItemFlags );
    ...
    if ( ( flags & ImGuiButtonFlags_PressedOnMask_ ) == 0 )
        flags |= ( item_flags & ImGuiItemFlags_ButtonRepeat )
    ...
    if ( hovered ) {
        ...
        const bool mods_ok =
            !( flags & ImGuiButtonFlags_NoKeyModsAllowed ) ||
    ...
    if ( g.NavActivateDownId == id ) {
        ...
        if ( !nav_activated_by_inputs &&
            ...
            const ImGuiKeyData* key3 =
                GetKeyData( ImGuiKey_NavGamepadActivate );
            const float t1 =
        ...
        if ( g.ActiveIdSource == ImGuiInputSource_Mouse ) {
            ...
            if ( mouse_button == -1 ) {
                ...
                bool release_anywhere =
                    ( flags &
                      ImGuiButtonFlags_PressedOnClickReleaseAnywhere ) != 0;
         ...
         style.FramePadding.y <
             window->DC
                 .CurrLineTextBaseOffset ) // Try to vertically align buttons
                                           ...
                                           // so that text baseline matches (bit
                                           // hacky, since it shouldn't be a
                                           // flag)
...
bool ImGui::InvisibleButton( const char* str_id,
    ...
    if ( !ItemAdd( bb, id, NULL,
                   ( flags & ImGuiButtonFlags_EnableNav )
                       ? ImGuiItemFlags_None
...
bool ImGui::CollapseButton( ImGuiID id,
    ...
    if ( IsItemActive() && IsMouseDragging( 0 ) )
        StartMouseMovingWindowOrNode(
            window, dock_node,
            true ); // Undock from window/collapse menu button
...
ImRect ImGui::GetWindowScrollbarRect( ImGuiWindow* window, ImGuiAxis axis ) {
    ...
    if ( axis == ImGuiAxis_X )
        return ImRect( inner_rect.Min.x + border_size,
                       ImMax( outer_rect.Min.y + border_size,
...
bool ImGui::ScrollbarEx( const ImRect& bb_frame,
    ...
    bb.Expand( ImVec2(
        ...
        -ImClamp( IM_TRUNC( ( bb_frame_height - 2.0f ) * 0.5f ), 0.0f,
                  3.0f ) ) );
    ...
    const ImS64 scroll_max =
        ImMax( ( ImS64 )1, size_contents_v - size_visible_v );
    float scroll_ratio =
        ImSaturate( ( float )*p_scroll_v / ( float )scroll_max );
    ...
    if ( held && allow_interaction && grab_h_norm < 1.0f ) {
        ...
        const int held_dir = ( clicked_v_norm < grab_v_norm ) ? -1
                             : ( clicked_v_norm > grab_v_norm + grab_h_norm )
                                 ...
                                 : 0;
        if ( g.ActiveIdIsJustActivated ) {
            // On initial click when held_dir == 0 (clicked over grab):
            // calculate the distance between mouse and the center of the grab
            const bool scroll_to_clicked_location =
                ( g.IO.ConfigScrollbarScrollByPage == false || g.IO.KeyShift ||
                  held_dir == 0 );
            g.ScrollbarSeekMode =
                scroll_to_clicked_location ? 0 : ( short )held_dir;
        ...
        if ( g.ScrollbarSeekMode == 0 ) {
            ...
            *p_scroll_v = ( ImS64 )( scroll_v_norm * scroll_max );
            ...
            if ( IsMouseClicked( ImGuiMouseButton_Left,
                ...
                *p_scroll_v = ImClamp(
                    *p_scroll_v + ( ImS64 )( page_dir * size_visible_v ),
                    ( ImS64 )0, scroll_max );
            }
        }
    ...
    if ( axis == ImGuiAxis_X )
        grab_rect =
            ImRect( ImLerp( bb.Min.x, bb.Max.x, grab_v_norm ), bb.Min.y,
...
void ImGui::Image( ImTextureID user_texture_id,
    ...
    if ( border_size > 0.0f )
        window->DrawList->AddRect( bb.Min, bb.Max, GetColorU32( border_col ),
                                   0.0f, ImDrawFlags_None, border_size );
...
// - ImageButton() adds style.FramePadding*2.0f to provided size. This is in
// order to facilitate fitting an image in a button.
// - ImageButton() draws a background based on regular Button() color +
// optionally an inner background if specified. (#8165) // FIXME: Maybe that's
// not the best design?
...
// - old ImageButton() used ImTextureID as item id (created issue with multiple
// buttons with same image, transient texture id values, opaque computation of
// ID)
// - new ImageButton() requires an explicit 'const char* str_id'
// - old ImageButton() had frame_padding' override argument.
// - new ImageButton() always use style.FramePadding.
/*
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const
ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const
ImVec4& tint_col)
{
    // Default to using texture ID as ID. User can still push string/integer
prefixes. PushID((ImTextureID)(intptr_t)user_texture_id); if (frame_padding >=
0) PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2((float)frame_padding,
(float)frame_padding)); bool ret = ImageButton("", user_texture_id, size, uv0,
uv1, bg_col, tint_col); if (frame_padding >= 0) PopStyleVar(); PopID(); return
ret;
}
*/
...
void ImGui::TextLinkOpenURL( const char* label, const char* url ) {
    ...
    SetItemTooltip( LocalizeGetMsg( ImGuiLocKey_OpenLink_s ),
                    url ); // It is more reassuring for user to _always_ display
                           // URL when we same as label
    ...
    if ( flags & ImGuiSeparatorFlags_Vertical ) {
        ...
        if ( ItemAdd( bb, 0 ) ) {
            ...
            window->DrawList->AddRectFilled(
                bb.Min, bb.Max, GetColorU32( ImGuiCol_Separator ) );
            if ( g.LogEnabled )
...
void ImGui::SeparatorTextEx( ImGuiID id,
    ...
    const ImRect bb( pos,
                     ImVec2( window->WorkRect.Max.x, pos.y + min_size.y ) );
...
bool ImGui::BeginComboPopup( ImGuiID popup_id,
    ...
    PushStyleVarX(
        ...
        g.Style.FramePadding
            .x ); // Horizontally align ourselves with the framed text
    ...
    IM_ASSERT(
        ...
        g.LastItemData.Rect.Min.y ==
            preview_data->PreviewRect.Min
                .y ); // Didn't call after BeginCombo/EndCombo block or forgot
                      // to pass ImGuiComboFlags_CustomPreview flag?
...
void ImGui::EndComboPreview() {
    ...
    if ( window->DC.CursorMaxPos.x < preview_data->PreviewRect.Max.x &&
        ...
        if ( draw_list->CmdBuffer.Size >
             1 ) // Unlikely case that the PushClipRect() didn't create a
                 // command
        {
...
void ImGui::DataTypeApplyOp( ImGuiDataType data_type,
    ...
    switch ( data_type ) {
        case ImGuiDataType_S8:
            if ( op == '+' ) {
                *( ImS8* )output = ImAddClampOverflow( *( const ImS8* )arg1,
                                                       *( const ImS8* )arg2,
                                                       IM_S8_MIN, IM_S8_MAX );
            }
            if ( op == '-' ) {
                *( ImS8* )output = ImSubClampOverflow( *( const ImS8* )arg1,
                                                       *( const ImS8* )arg2,
                                                       IM_S8_MIN, IM_S8_MAX );
            }
            return;
        case ImGuiDataType_U8:
            if ( op == '+' ) {
                *( ImU8* )output = ImAddClampOverflow( *( const ImU8* )arg1,
                                                       *( const ImU8* )arg2,
                                                       IM_U8_MIN, IM_U8_MAX );
            }
            if ( op == '-' ) {
                *( ImU8* )output = ImSubClampOverflow( *( const ImU8* )arg1,
                                                       *( const ImU8* )arg2,
                                                       IM_U8_MIN, IM_U8_MAX );
            }
            return;
        case ImGuiDataType_S16:
            if ( op == '+' ) {
                *( ImS16* )output = ImAddClampOverflow(
                    *( const ImS16* )arg1, *( const ImS16* )arg2, IM_S16_MIN,
                    IM_S16_MAX );
            }
            if ( op == '-' ) {
        ...
        case ImGuiDataType_U16:
            if ( op == '+' ) {
                *( ImU16* )output = ImAddClampOverflow(
                    *( const ImU16* )arg1, *( const ImU16* )arg2, IM_U16_MIN,
                    IM_U16_MAX );
            }
            if ( op == '-' ) {
        ...
        case ImGuiDataType_S32:
            if ( op == '+' ) {
                *( ImS32* )output = ImAddClampOverflow(
                    *( const ImS32* )arg1, *( const ImS32* )arg2, IM_S32_MIN,
                    IM_S32_MAX );
            }
            if ( op == '-' ) {
        ...
        case ImGuiDataType_U32:
            if ( op == '+' ) {
                *( ImU32* )output = ImAddClampOverflow(
                    *( const ImU32* )arg1, *( const ImU32* )arg2, IM_U32_MIN,
                    IM_U32_MAX );
            }
            if ( op == '-' ) {
        ...
        case ImGuiDataType_S64:
            if ( op == '+' ) {
                *( ImS64* )output = ImAddClampOverflow(
                    *( const ImS64* )arg1, *( const ImS64* )arg2, IM_S64_MIN,
                    IM_S64_MAX );
            }
            if ( op == '-' ) {
        ...
        case ImGuiDataType_U64:
            if ( op == '+' ) {
                *( ImU64* )output = ImAddClampOverflow(
                    *( const ImU64* )arg1, *( const ImU64* )arg2, IM_U64_MIN,
                    IM_U64_MAX );
            }
            if ( op == '-' ) {
...
bool ImGui::DataTypeClamp( ImGuiDataType data_type,
    ...
    switch ( data_type ) {
        case ImGuiDataType_S8:
            return DataTypeClampT< ImS8 >(
                ( ImS8* )p_data, ( const ImS8* )p_min, ( const ImS8* )p_max );
        case ImGuiDataType_U8:
            return DataTypeClampT< ImU8 >(
                ( ImU8* )p_data, ( const ImU8* )p_min, ( const ImU8* )p_max );
        case ImGuiDataType_S16:
...
bool ImGui::DragBehaviorT( ImGuiDataType data_type,
    ...
    const bool is_bounded =
        ( v_min < v_max ) ||
        ( ( v_min == v_max ) &&
          ( v_min != 0.0f || ( flags & ImGuiSliderFlags_ClampZeroRange ) ) );
    const bool is_wrapped =
        is_bounded && ( flags & ImGuiSliderFlags_WrapAround );
    ...
    if ( g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
         IsMouseDragPastThreshold(
             0, g.IO.MouseDragThreshold * DRAG_MOUSE_THRESHOLD_FACTOR ) ) {
        ...
        const float tweak_factor = ( flags & ImGuiSliderFlags_NoSpeedTweaks )
                                       ? 1.0f
...
bool ImGui::DragBehavior( ImGuiID id,
    ...
    IM_ASSERT(
        ...
        "Invalid ImGuiSliderFlags flags! Has the legacy 'float power' argument "
...
bool ImGui::DragScalar( const char* label,
    ...
    const bool hovered =
        ItemHoverable( frame_bb, id, g.LastItemData.ItemFlags );
    ...
    if ( !temp_input_is_active ) {
        ...
        // Store initial value (not used by main lib but available as a
        // convenience but some mods e.g. to revert)
        if ( make_active )
            memcpy( &g.ActiveIdValueOnActivation, p_data,
    ...
    if ( temp_input_is_active ) {
        // Only clamp CTRL+Click input when ImGuiSliderFlags_ClampOnInput is set
        // (generally via ImGuiSliderFlags_AlwaysClamp)
        bool clamp_enabled = false;
        if ( ( flags & ImGuiSliderFlags_ClampOnInput ) &&
             ( p_min != NULL || p_max != NULL ) ) {
            const int clamp_range_dir =
                ( p_min != NULL && p_max != NULL )
    ...
    if ( is_logarithmic ) {
                  ...
                  0.0f ) // Range crosses zero, so split into two portions
        {
            float zero_point_center =
                ...
                ( ( float )v_max -
                  ( float )
                      v_min ); // The zero point in parametric space.  There's
                               // an argument we should take the logarithmic
                               // nature into account when calculating this, but
                               // for now this should do (and the most common
                               // case of a symmetrical range works fine)
            ...
            if ( v == 0.0f )
                ...
                result =
                    ( 1.0f - ( float )( ImLog( -( FLOATTYPE )v_clamped /
                                               logarithmic_zero_epsilon ) /
                                        ImLog( -v_min_fudged /
                                               logarithmic_zero_epsilon ) ) ) *
                    zero_point_snap_L;
            ...
            result =
                1.0f -
                ( float )( ImLog( -( FLOATTYPE )v_clamped / -v_max_fudged ) /
...
TYPE ImGui::ScaleValueFromRatioT( ImGuiDataType data_type,
    ...
    if ( is_logarithmic ) {
        ...
        float t_with_flip =
            flipped ? ( 1.0f - t ) : t; // t, but flipped if necessary to
             ...
             0.0f ) // Range crosses zero, so we have to do this in two parts
        {
            ...
            if ( t_with_flip >= zero_point_snap_L &&
                ...
                result = ( TYPE )0.0f; // Special case to make getting exactly
                                       // zero possible (the epsilon prevents it
                                       // otherwise)
            ...
            result =
                ( TYPE )( v_min_fudged * ImPow( v_max_fudged / v_min_fudged,
                                                ( FLOATTYPE )t_with_flip ) );
    } else {
        // Linear slider
        const bool is_floating_point = ( data_type == ImGuiDataType_Float ) ||
...
bool ImGui::SliderBehaviorT( const ImRect& bb,
    ...
    const float v_range_f =
        ( float )( v_min < v_max
                       ? v_max - v_min
                       : v_min - v_max ); // We don't need high precision for
                                          // what we do with it.
    ...
    if ( g.ActiveId == id ) {
        ...
        if ( g.ActiveIdSource == ImGuiInputSource_Mouse ) {
            ...
            if ( input_delta != 0.0f ) {
                ...
                const int decimal_precision =
                    is_floating_point ? ImParseFormatPrecision( format, 3 ) : 0;
                if ( decimal_precision > 0 ) {
                    input_delta /= 100.0f; // Keyboard/Gamepad tweak speeds in %
                                           // of slider bounds
                    if ( tweak_slow )
                        input_delta /= 10.0f;
                } else {
                    if ( ( v_range_f >= -100.0f && v_range_f <= 100.0f &&
                        ...
                        input_delta =
                            ...
                            v_range_f; // Keyboard/Gamepad tweak speeds in
                                       // integer steps
                    else
                        input_delta /= 100.0f;
                }
            ...
            if ( g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated ) {
                                    ...
                                    // when already past the limits
                {
                    ...
                    if ( is_floating_point &&
                        ...
                        v_new = RoundScalarWithFormatT< TYPE >(
        ...
        if ( set_new_value ) {
            ...
            if ( is_floating_point &&
                 !( flags & ImGuiSliderFlags_NoRoundToFormat ) )
                v_new =
                    RoundScalarWithFormatT< TYPE >( format, data_type, v_new );
...
bool ImGui::SliderScalar( const char* label,
    ...
    if ( temp_input_is_active ) {
        ...
        const bool clamp_enabled =
            ( flags & ImGuiSliderFlags_ClampOnInput ) != 0;
        return TempInputScalar( frame_bb, id, label, data_type, p_data, format,
                                clamp_enabled ? p_min : NULL,
                                clamp_enabled ? p_max : NULL );
    }
...
void ImParseFormatSanitizeForPrinting( const char* fmt_in,
    ...
    IM_ASSERT( ( size_t )( fmt_end - fmt_in + 1 ) <
               fmt_out_size ); // Format is too long, let us know if this
                               // happens to you!
    while ( fmt_in < fmt_end ) {
        char c = *fmt_in++;
        if ( c != '\'' && c != '$' &&
             c != '_' ) // Custom flags provided by stb_sprintf.h. POSIX 2008
                        // also supports '.
            *( fmt_out++ ) = c;
    }
...
// FIXME: Among other things, setting ImGuiItemFlags_AllowDuplicateId in
// LastItemData is currently correct but the expected relationship between
// TempInputXXX functions and LastItemData is a little fishy.
...
void ImGui::SetNextItemRefVal( ImGuiDataType data_type, void* p_data ) {
    ...
    memcpy( &g.NextItemData.RefVal, p_data,
            DataTypeGetInfo( data_type )->Size );
}
...
bool ImGui::InputScalar( const char* label,
    ...
    IM_ASSERT( ( flags & ImGuiInputTextFlags_EnterReturnsTrue ) ==
               0 ); // Not supported by InputScalar(). Please open an issue if
                    // you this would be useful to you. Otherwise use
    ...
    if ( ( flags & ImGuiInputTextFlags_DisplayEmptyRefVal ) &&
        ...
        DataTypeFormatString( buf, IM_ARRAYSIZE( buf ), data_type, p_data,
                              format );

    // Disable the MarkItemEdited() call in InputText but keep
    // ImGuiItemStatusFlags_Edited. We call MarkItemEdited() ourselves by
    // comparing the actual data rather than the string.
    ...
    if ( p_step == NULL ) {
        if ( InputText( label, buf, IM_ARRAYSIZE( buf ), flags ) )
            value_changed = DataTypeApplyFromText(
                ...
                ( flags & ImGuiInputTextFlags_ParseEmptyRefVal )
                    ? p_data_default
                    : NULL );
    } else {
...
bool ImGui::InputText( const char* label,
    ...
    return InputTextEx( label, NULL, buf, ( int )buf_size, ImVec2( 0, 0 ),
    ...
    IM_ASSERT(
        !( flags &
           ImGuiInputTextFlags_Multiline ) ); // call InputTextMultiline() or
                                              // InputTextEx() manually if you
                                              // need multi-line + hint.
    return InputTextEx( label, hint, buf, ( int )buf_size, ImVec2( 0, 0 ),
                        flags, callback, user_data );
}
...
static ImVec2 InputTextCalcTextSize( ImGuiContext* ctx,
    ...
    while ( s < text_end ) {
        ...
        const float char_width = ( ( int )c < font->IndexAdvanceX.Size
...
// wchar) With our UTF-8 use of stb_textedit:
// - STB_TEXTEDIT_GETCHAR is nothing more than a a "GETBYTE". It's only used to
// compare to ascii or to copy blocks of text so we are fine.
// - One exception is the STB_TEXTEDIT_IS_SPACE feature which would expect a
// full char in order to handle full-width space such as 0x3000 (see
// ImCharIsBlankW).
...
static float STB_TEXTEDIT_GETWIDTH( ImGuiInputTextState* obj,
                                    ...
                                    int char_idx ) {
    unsigned int c;
    ImTextCharFromUtf8( &c, obj->TextSrc + line_start_idx + char_idx,
...
static void STB_TEXTEDIT_LAYOUTROW( StbTexteditRow* r,
    ...
    const ImVec2 size = InputTextCalcTextSize( obj->Ctx, text + line_start_idx,
                                               text + obj->TextLen,
...
static int IMSTB_TEXTEDIT_GETNEXTCHARINDEX_IMPL( ImGuiInputTextState* obj,
                                                 int idx ) {
    if ( idx >= obj->TextLen )
        return obj->TextLen + 1;
    unsigned int c;
    return idx + ImTextCharFromUtf8( &c, obj->TextSrc + idx,
                                     obj->TextSrc + obj->TextLen );
}

static int IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL( ImGuiInputTextState* obj,
                                                 int idx ) {
    if ( idx <= 0 )
        return -1;
    const char* p =
        ImTextFindPreviousUtf8Codepoint( obj->TextSrc, obj->TextSrc + idx );
...
static int is_word_boundary_from_right( ImGuiInputTextState* obj, int idx ) {
    ...
    const char* prev_p =
        ImTextFindPreviousUtf8Codepoint( obj->TextSrc, curr_p );
    unsigned int curr_c;
...
static void STB_TEXTEDIT_DELETECHARS( ImGuiInputTextState* obj,
                                      int pos,
                                      int n ) {
...
static bool STB_TEXTEDIT_INSERTCHARS( ImGuiInputTextState* obj,
                                      int pos,
                                      const char* new_text,
                                      int new_text_len ) {
    ...
    if ( new_text_len + text_len + 1 > obj->TextA.Size ) {
        if ( !is_resizable )
            return false;
        obj->TextA.resize(
            text_len + ImClamp( new_text_len, 32, ImMax( 256, new_text_len ) ) +
            1 );
...
// We added an extra indirection where 'Stb' is heap-allocated, in order
// facilitate the work of bindings generators.
...
void ImGuiInputTextState::OnCharPressed( unsigned int c ) {
    ...
    // The changes we had to make to stb_textedit_key made it very much UTF-8
    // specific which is not too great.
    char utf8[ 5 ];
...
// Those functions are not inlined in imgui_internal.h, allowing us to hide
// ImStbTexteditState from that header.
...
// nuisance. Historically they existed because STB_TEXTEDIT_INSERTCHARS() etc.
// worked on our ImWchar buffer, but nowadays they both work on UTF-8 data.
// Should aim to merge both.
...
void ImGui::PushPasswordFont() {
    ...
    IM_ASSERT( out_font->Glyphs.Size == 0 &&
               out_font->IndexAdvanceX.Size == 0 &&
               out_font->IndexLookup.Size == 0 );
    PushFont( out_font );
}
...
static bool InputTextFilterCharacter( ImGuiContext* ctx,
    ...
    if ( c < 0x20 ) {
        ...
        pass |=
            ( c == '\t' ) && ( flags & ImGuiInputTextFlags_AllowTabInput ) != 0;
        if ( !pass )
            return false;
...
// Find the shortest single replacement we can make to get from old_buf to
// new_buf Note that this doesn't directly alter state->TextA, state->TextLen.
// They are expected to be made valid separately.
...
static void InputTextReconcileUndoState( ImGuiInputTextState* state,
                                         const char* old_buf,
                                         int old_length,
                                         const char* new_buf,
                                         int new_length ) {
...
bool ImGui::InputTextEx( const char* label,
    ...
    if ( is_multiline ) // Open group before calling GetID() because groups
                        // tracks id created within their scope (including the
                        // scrollbar)
        BeginGroup();
    const ImGuiID id = window->GetID( label );
    const ImVec2 label_size = CalcTextSize( label, NULL, true );
    ...
    if ( is_multiline ) {
        ...
        PushStyleVar( ImGuiStyleVar_WindowPadding,
                      ImVec2( 0, 0 ) ); // Ensure no clip rect so mouse hover
                                        // can reach FramePadding edges
        bool child_visible =
            BeginChildEx( label, id, frame_bb.GetSize(),
    ...
    // Ensure mouse cursor is set even after switching to keyboard/gamepad mode.
    // May generalize further? (#6417)
    bool hovered = ItemHoverable(
        frame_bb, id,
    ...
    const bool init_reload_from_user_buf =
        ( state != NULL && state->WantReloadUserBuf );
    ...
    if ( init_reload_from_user_buf ) {
        ...
        IM_ASSERT( new_len + 1 <= buf_size &&
                   "Is your input buffer properly zero-terminated?" );
        state->WantReloadUserBuf = false;
        InputTextReconcileUndoState( state, state->TextA.Data, state->TextLen,
                                     buf, new_len );
        state->TextA.resize( buf_size +
                             1 ); // we use +1 to make sure that .Data is always
        ...
        if ( !is_readonly ) {
            state->TextA.resize(
                buf_size + 1 ); // we use +1 to make sure that .Data is always
                                // pointing to at least an empty string.
            memcpy( state->TextA.Data, buf, state->TextLen + 1 );
        }
        ...
        if ( flags & ImGuiInputTextFlags_ElideLeft )
            state->Scroll.x +=
                ImMax( 0.0f, CalcTextSize( buf ).x - frame_size.x +
                                 style.FramePadding.x * 2.0f );
    ...
    if ( g.ActiveId == id ) {
        ...
        // FIXME: The reason we don't use Shortcut() is we would need a routing
        // flag to specify multiple mods, or to all mods combinaison into
        // individual shortcuts.
        const ImGuiKey always_owned_keys[] = {
            ImGuiKey_LeftArrow,   ImGuiKey_RightArrow, ImGuiKey_Enter,
        ...
        // Read-only mode always ever read from source buffer. Refresh TextLen
        // when active.
        if ( is_readonly && state != NULL )
            state->TextLen = ( int )strlen( buf );
        // if (is_readonly && state != NULL)
        //     state->TextA.clear(); // Uncomment to facilitate debugging, but
        //     we otherwise prefer to keep/amortize th allocation.
    }
    if ( state != NULL )
    ...
    if ( g.ActiveId == id ) {
        ...
        } else if ( hovered && io.MouseClickedCount[ 0 ] >= 2 &&
                    !io.KeyShift ) {
    ...
    if ( g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id ) {
        ...
        const bool is_select_all =
            Shortcut( ImGuiMod_Ctrl | ImGuiKey_A, 0, id );
        ...
        if ( IsKeyPressed( ImGuiKey_LeftArrow ) ) {
        ...
        } else if ( IsKeyPressed( ImGuiKey_Delete ) && !is_readonly &&
                    !is_cut ) {
            ...
            if ( g.PlatformIO.Platform_SetClipboardTextFn != NULL ) {
                // SetClipboardText() only takes null terminated strings +
                // state->TextSrc may point to read-only user buffer, so we need
                // to make a copy.
            ...
            if ( const char* clipboard = GetClipboardText() ) {
                         ...
                         // operation
                {
                    ...
                    stb_textedit_paste( state, state->Stb,
    ...
    if ( g.ActiveId == id ) {
        ...
        // FIXME-OPT: We always reapply the live buffer back to the input buffer
        // before clearing ActiveId, even though strictly speaking it wasn't
        // modified on this frame. Should mark dirty state from the stb_textedit
        // callbacks. If we do that, need to ensure that as special case,
        // 'validated == true' also writes back. This also allows the user to
        // use InputText() without maintaining any user-side storage. (please
        // note that if you use this property along
        // ImGuiInputTextFlags_CallbackResize you can end up with your temporary
        // string object unnecessarily allocating once a frame, either store
        // your string data, either if you don't then don't use
        // ImGuiInputTextFlags_CallbackResize).
        const bool apply_edit_back_to_user_buffer =
            true; // !revert_edit || (validated && (flags &
        ...
        if ( apply_edit_back_to_user_buffer ) {
            ...
            if ( ( flags & ( ImGuiInputTextFlags_CallbackCompletion |
                ...
                if ( event_flag ) {
                    ...
                    // FIXME-OPT: Undo stack reconcile needs a backup of the
                    // data until we rework API, see #7925
                    char* callback_buf = is_readonly ? buf : state->TextA.Data;
                    IM_ASSERT( callback_buf == state->TextSrc );
                    state->CallbackTextBackup.resize( state->TextLen + 1 );
                    memcpy( state->CallbackTextBackup.Data, callback_buf,
                            state->TextLen + 1 );
                    ...
                    if ( buf_dirty ) {
                        // Callback may update buffer and thus set buf_dirty
                        ...
                        InputTextReconcileUndoState(
                            state, state->CallbackTextBackup.Data,
                            state->CallbackTextBackup.Size - 1,
                            callback_data.Buf, callback_data.BufTextLen );
                        state->TextLen =
                            callback_data
    ...
    if ( render_cursor || render_selection ) {
...

        {
            ...
            const char* cursor_ptr =
                render_cursor ? text_begin + state->Stb->cursor : NULL;
            const char* selmin_ptr =
                render_selection ? text_begin + ImMin( state->Stb->select_start,
                                                       state->Stb->select_end )
                                 : NULL;
            ...
            // measuring distance
            cursor_offset.x =
                InputTextCalcTextSize( &g, ImStrbol( cursor_ptr, text_begin ),
                                       cursor_ptr )
                    .x;
            ...
            if ( selmin_line_no >= 0 ) {
                select_start_offset.x =
                    InputTextCalcTextSize(
                        &g, ImStrbol( selmin_ptr, text_begin ), selmin_ptr )
                        .x;
        ...
        if ( render_cursor && state->CursorFollow ) {
            ...
            if ( is_multiline ) {
                ...
                const float scroll_max_y =
                    ImMax( ( text_size.y + style.FramePadding.y * 2.0f ) -
                               inner_size.y,
                           0.0f );
        ...
        if ( render_selection ) {
            ...
            for ( const char* p = text_selected_begin;
                  p < text_selected_end; ) {
                ...
                if ( rect_pos.y < clip_rect.y ) {
                    ...
                    ImVec2 rect_size = InputTextCalcTextSize(
        ...
        // ImDrawList crash.
        // FIXME-OPT: Multiline could submit a smaller amount of contents to
        // AddText() since we already iterated through it.
        if ( is_multiline ||
        ...
        if ( is_multiline ||
            ...
            if ( flags & ImGuiInputTextFlags_ElideLeft )
                draw_pos.x =
                    ImMin( draw_pos.x, frame_bb.Max.x -
    ...
    if ( is_multiline ) {
        ...
        // child ItemAdd() call specifies the ImGuiItemFlags_Inputable (see
        // #4761, #7870)...
        Dummy( ImVec2( text_size.x, text_size.y + style.FramePadding.y ) );
        g.NextItemData.ItemFlags |= ( ImGuiItemFlags )ImGuiItemFlags_Inputable |
...
void ImGui::DebugNodeInputTextState( ImGuiInputTextState* state ) {
    ...
    Text( "CurLenA: %d, Cursor: %d, Selection: %d..%d", state->TextLen,
          stb_state->cursor, stb_state->select_start, stb_state->select_end );
    Text( "BufCapacityA: %d", state->BufCapacity );
    Text( "(Internal Buffer: TextA Size: %d, Capacity: %d)", state->TextA.Size,
          state->TextA.Capacity );
    Text( "has_preferred_x: %d (%.2f)", stb_state->has_preferred_x,
          stb_state->preferred_x );
    Text(
        "undo_point: %d, redo_point: %d, undo_char_point: %d, redo_char_point: "
        "%d",
...
static void ColorEditRestoreH( const float* col, float* H ) {
    ...
    if ( g.ColorEditSavedID != g.ColorEditCurrentID ||
         g.ColorEditSavedColor != ImGui::ColorConvertFloat4ToU32( ImVec4(
                                      col[ 0 ], col[ 1 ], col[ 2 ], 0 ) ) )
        return;
...
// resetting.
static void ColorEditRestoreHS( const float* col,
                                float* H,
                                float* S,
                                float* V ) {
    ImGuiContext& g = *GImGui;
    ...
    if ( ( flags & ( ImGuiColorEditFlags_DisplayRGB |
        ...
        for ( int n = 0; n < components; n++ ) {
            ...
            if ( flags & ImGuiColorEditFlags_Float ) {
                value_changed |= DragFloat( ids[ n ], &f[ n ], 1.0f / 255.0f,
                                            0.0f, hdr ? 0.0f : 1.0f,
        ...
        if ( InputText( "##Text", buf, IM_ARRAYSIZE( buf ),
            ...
            if ( alpha )
                r = sscanf( p, "%02X%02X%02X%02X", ( unsigned int* )&i[ 0 ],
                            ...
                            ( unsigned int* )&i[ 3 ] ); // Treat at unsigned (%X
                                                        // is unsigned)
            else
...
bool ImGui::ColorPicker4( const char* label,
    ...
    const bool is_readonly =
        ( ( g.NextItemData.ItemFlags | g.CurrentItemFlags ) &
    ...
    if ( flags & ImGuiColorEditFlags_PickerHueWheel ) {
        ...
        int hue_cursor_segments = draw_list->_CalcCircleAutoSegmentCount(
            hue_cursor_rad ); // Lock segment count so the +1 one matches
                              // others.
...
bool ImGui::ColorButton( const char* desc_id,
    ...
    if ( flags &
        ...
        flags &= ~( ImGuiColorEditFlags_AlphaNoBg |
                    ImGuiColorEditFlags_AlphaPreviewHalf );
    ...
    if ( ( flags & ImGuiColorEditFlags_AlphaPreviewHalf ) &&
        ...
        if ( col_source.w < 1.0f &&
             ( flags & ImGuiColorEditFlags_AlphaNoBg ) == 0 )
            RenderColorRectWithAlphaCheckerboard(
    ...
    if ( ( flags & ImGuiColorEditFlags_NoBorder ) == 0 ) {
        if ( g.Style.FrameBorderSize > 0.0f )
            ...
            window->DrawList->AddRect(
                ...
                rounding ); // Color button are often in need of some sort of
                            // border
    }
...
void ImGui::SetColorEditOptions( ImGuiColorEditFlags flags ) {
    ...
    IM_ASSERT( ImIsPowerOfTwo(
        flags & ImGuiColorEditFlags_DataTypeMask_ ) ); // Check only 1 option is
                                                       // selected
    IM_ASSERT( ImIsPowerOfTwo(
        flags &
...
void ImGui::ColorTooltip( const char* text,
    ...
    ColorButton( "##preview", cf,
                 ( flags & flags_to_forward ) | ImGuiColorEditFlags_NoTooltip,
                 sz );
    SameLine();
    if ( ( flags & ImGuiColorEditFlags_InputRGB ) ||
         !( flags & ImGuiColorEditFlags_InputMask_ ) ) {
        ...
        if ( flags & ImGuiColorEditFlags_NoAlpha )
            ...
            Text( "H: %.3f, S: %.3f, V: %.3f, A: %.3f", col[ 0 ], col[ 1 ],
                  col[ 2 ], col[ 3 ] );
    }
    EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup( const float* col,
    ...
    if ( BeginPopup( "Copy" ) ) {
        ...
        if ( Selectable( buf ) )
            SetClipboardText( buf );
        if ( !( flags & ImGuiColorEditFlags_NoAlpha ) ) {
            ImFormatString( buf, IM_ARRAYSIZE( buf ), "#%02X%02X%02X%02X", cr,
                            cg, cb, ca );
            if ( Selectable( buf ) )
                SetClipboardText( buf );
        }
    ...
    if ( allow_opt_picker ) {
        ...
        for ( int picker_type = 0; picker_type < 2; picker_type++ ) {
            ...
            if ( Selectable( "##selectable", false, 0,
                             picker_size ) ) // By default, Selectable() is
                                             // closing popup
                g.ColorEditOptions =
            ...
            memcpy(
                &previewing_ref_col, ref_col,
                sizeof( float ) *
                    ( ( picker_flags & ImGuiColorEditFlags_NoAlpha ) ? 3
                                                                     : 4 ) );
...
bool ImGui::TreeNodeBehavior( ImGuiID id,
    ...
    const bool span_all_columns_label =
        ( flags & ImGuiTreeNodeFlags_LabelSpanAllColumns ) != 0 &&
        ( g.CurrentTable != NULL );
    ImRect frame_bb;
    ...
    const bool is_multi_select =
        ( g.LastItemData.ItemFlags & ImGuiItemFlags_IsMultiSelect ) != 0;
    if ( is_multi_select ) // We absolutely need to distinguish open vs select
                           // so _OpenOnArrow comes by default
    ...
    if ( !is_leaf ) {
        if ( pressed && g.DragDropHoldJustPressedId != id ) {
            ...
            if ( !is_open ) // When using Drag and Drop "hold to open" we keep
...
bool ImGui::Selectable( const char* label,
    ...
    // widgets), so navigation can be resumed with keyboard/gamepad
    if ( pressed ||
    ...
    if ( is_visible ) {
        ...
        if ( highlighted || selected ) {
            // Between 1.91.0 and 1.91.4 we made selected Selectable use an
            // arbitrary lerp between _Header and _HeaderHovered. Removed that
            // now. (#8106)
            ImU32 col =
                GetColorU32( ( held && highlighted ) ? ImGuiCol_HeaderActive
    ...
    if ( is_visible )
        RenderTextClipped(
            pos, ImVec2( window->WorkRect.Max.x, pos.y + size.y ), label, NULL,
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(
...
static int ImStrimatchlen( const char* s1,
                           const char* s1_end,
                           const char* s2 ) {
    int match_len = 0;
...
int ImGui::TypingSelectFindMatch( ImGuiTypingSelectRequest* req,
                                  ...
                                  int nav_item_idx ) {
    if ( req == NULL ||
         req->SelectRequest == false ) // Support NULL parameter so both calls
...
int ImGui::TypingSelectFindNextSingleCharMatch(
    ...
    for ( int idx = 0; idx < items_count; idx++ ) {
        ...
        if ( ImStrimatchlen( req->SearchBuffer,
            ...
            continue;
        if ( return_next_match ) // Return next matching item after current
                                 // item.
            return idx;
...
int ImGui::TypingSelectFindBestLeadingMatch(
    ...
    for ( int idx = 0; idx < items_count; idx++ ) {
        ...
        const int match_len = ImStrimatchlen(
            req->SearchBuffer, req->SearchBuffer + req->SearchBufferLen,
            item_name );
...
static void BoxSelectScrollWithMouseDrag( ImGuiBoxSelectState* bs,
    ...
    for ( int n = 0; n < 2; n++ ) // each axis
    {
        ...
        const float speed_multiplier = ImLinearRemapClamp(
            g.FontSize, g.FontSize * 5.0f, 1.0f, 4.0f,
...
bool ImGui::BeginBoxSelect( const ImRect& scope_rect,
                            ImGuiWindow* window,
    ...
    if ( ms_flags &
         ImGuiMultiSelectFlags_ScopeWindow ) // Box-select scrolling only
                                             // happens with ScopeWindow
        curr_end_pos_abs =
...
void ImGui::EndBoxSelect( const ImRect& scope_rect,
    ...
    bs->EndPosRel = WindowPosAbsToRel(
        window, ImClamp( g.IO.MousePos, scope_rect.Min,
                         scope_rect.Max ) ); // Clamp stored position according
                                             // to current scrolling view
    ...
    window->DrawList->AddRect(
        box_select_r.Min, box_select_r.Max,
        GetColorU32( ImGuiCol_NavCursor ) ); // FIXME-MULTISELECT: Styling
...
static ImRect CalcScopeRect( ImGuiMultiSelectTempData* ms,
                             ImGuiWindow* window ) {
    ImGuiContext& g = *GImGui;
    if ( ms->Flags & ImGuiMultiSelectFlags_ScopeRect ) {
        ...
        // InnerClipRect?
        scope_rect.Min =
            ImMin( scope_rect.Min + ImVec2( window->DecoInnerSizeX1,
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect( ImGuiMultiSelectFlags flags,
    ...
    // FIXME: Workaround to the fact we override CursorMaxPos, meaning size
    // measurement are lost. (#8250) They should perhaps be stacked properly?
    if ( ImGuiTable* table = g.CurrentTable )
        if ( table->CurrentColumn != -1 )
            TableEndCell( table ); // This is currently safe to call multiple
                                   // time. If that properly is lost we can
    ...
    if ( flags & ( ImGuiMultiSelectFlags_BoxSelect1d |
        ...
        if ( BeginBoxSelect( CalcScopeRect( ms, window ), window,
                             ms->BoxSelectId, flags ) )
    ...
    if ( scope_hovered && g.HoveredId == 0 && g.ActiveId == 0 ) {
        ...
        if ( ms->Flags & ImGuiMultiSelectFlags_ClearOnClickVoid )
            if ( IsMouseReleased( 0 ) &&
                 IsMouseDragPastThreshold( 0 ) == false &&
        ...
        if ( ms->IsKeyboardSetRange ) {
            ...
            if ( is_range_src || is_range_dst ||
                ...
                selected = ( storage->RangeSelected != 0 );
                        ...
                        ( ms->Flags & ImGuiMultiSelectFlags_NoAutoClear ) ==
                            0 ) {
                // Clear other items
                selected = false;
            }
...
void ImGui::MultiSelectAddSetAll( ImGuiMultiSelectTempData* ms,
                                  bool selected ) {
...
void ImGui::MultiSelectAddSetRange( ImGuiMultiSelectTempData* ms,
                                    bool selected,
                                    int range_dir,
...
void ImGui::DebugNodeMultiSelectState( ImGuiMultiSelectState* storage ) {
    ...
    bool open = TreeNode( ( void* )( intptr_t )storage->ID,
                          "MultiSelect 0x%08X in '%s'%s", storage->ID,
    ...
    if ( !open )
        return;
    Text(
        "RangeSrcItem = %" IM_PRId64 " (0x%" IM_PRIX64 "), RangeSelected = %d",
...
bool ImGuiSelectionBasicStorage::GetNextSelectedItem( void** opaque_it,
    ...
    if ( PreserveOrder && it == NULL && it_end != NULL )
        ImQsort(
            _Storage.Data.Data, ( size_t )_Storage.Data.Size,
...
static void ImGuiSelectionBasicStorage_BatchFinish(
    ImGuiSelectionBasicStorage* selection,
    bool selected,
    int size_before_amends ) {
    ImGuiStorage* storage = &selection->_Storage;
...
void ImGuiSelectionBasicStorage::ApplyRequests( ImGuiMultiSelectIO* ms_io ) {
    ...
    for ( ImGuiSelectionRequest& req : ms_io->Requests ) {
        if ( req.Type == ImGuiSelectionRequestType_SetAll ) {
            ...
            if ( selection_changes == 1 ||
                 ( selection_changes < Size / 100 ) ) {
                ...
                for ( int idx = ( int )req.RangeFirstItem;
                    ...
                    SetItemSelected( GetStorageIdFromIndex( idx ),
                                     req.Selected );
            } else {
...
// If you don't need a label you can pretty much directly use
// ImGui::BeginChild() with ImGuiChildFlags_FrameStyle. Tip: To have a list
// filling the entire window width, use size.x = -FLT_MIN and pass an
// non-visible label e.g. "##empty" Tip: If your vertical size is calculated
// from an item count (e.g. 10 * item_height) consider adding a fractional part
...
bool ImGui::BeginMenuBar() {
    ...
    const float border_top = ImMax(
        IM_ROUND( window->WindowBorderSize * 0.5f - window->TitleBarHeight ),
        0.0f );
    ...
    ImRect clip_rect(
        ...
        ImFloor( ImMax(
            bar_rect.Min.x,
            bar_rect.Max.x - ImMax( window->WindowRounding, border_half ) ) ),
...
void ImGui::EndMenuBar() {
    ...
    if ( NavMoveRequestButNoResultYet() &&
        ...
        if ( nav_earliest_child->ParentWindow == window &&
            ...
            // FIXME-NAV: How to deal with this when not using
            // g.IO.ConfigNavCursorVisibleAuto?
...
bool ImGui::BeginMainMenuBar() {
    ...
    // Temporarily disable _NoSavedSettings, in the off-chance that tables or
    // child windows submitted within the menu-bar may want to use settings.
    // (#8356)
...
void ImGui::EndMainMenuBar() {
    ...
    if ( !g.CurrentWindow->DC.MenuBarAppending ) {
        IM_ASSERT_USER_ERROR(
            0,
            "Calling EndMainMenuBar() not from a menu-bar!" ); // Not
                                                               // technically
                                                               // testing that
                                                               // it is the main
                                                               // menu bar
    ...
    g.CurrentWindow->Flags |=
        ImGuiWindowFlags_NoSavedSettings; // Restore _NoSavedSettings (#8356)
...
bool ImGui::BeginMenuEx( const char* label, const char* icon, bool enabled ) {
    ...
    if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
        ...
        float icon_w =
            ( icon && icon[ 0 ] ) ? CalcTextSize( icon, NULL ).x : 0.0f;
    ...
    const bool hovered =
        ( g.HoveredId == id ) && enabled && !g.NavHighlightItemUnderNav;
    if ( menuset_is_open )
        PopItemFlag();
                                   ...
                                   // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if ( g.HoveredWindow == window && child_menu_window != NULL ) {
            ...
            tc.y = ta.y +
                   ImMin( ( tc.y + pad_farmost_h ) - ta.y, +ref_unit * 8.0f );
...
bool ImGui::MenuItemEx( const char* label,
    ...
    if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
        ...
        PushStyleVarX( ImGuiStyleVar_ItemSpacing, style.ItemSpacing.x * 2.0f );
        pressed =
            Selectable( "", selected, selectable_flags, ImVec2( w, 0.0f ) );
        PopStyleVar();
...
static ImU32 TabBarCalcTabID( ImGuiTabBar* tab_bar,
                              const char* label,
                              ImGuiWindow* docked_window );
static float TabBarCalcMaxTabWidth();
static float TabBarScrollClamp( ImGuiTabBar* tab_bar, float scrolling );
static void TabBarScrollToTab( ImGuiTabBar* tab_bar,
                               ImGuiID tab_id,
...
static float TabBarCalcScrollableWidth( ImGuiTabBar* tab_bar,
                                        ImGuiTabBarSection* sections ) {
    return tab_bar->BarRect.GetWidth() - sections[ 0 ].Width -
...
static void ImGui::TabBarLayout( ImGuiTabBar* tab_bar ) {
    ...
    if ( ( tab_bar->WidthAllTabsIdeal > tab_bar->BarRect.GetWidth() &&
        ...
        if ( ImGuiTabItem* scroll_and_select_tab =
            ...
            if ( ( scroll_and_select_tab->Flags & ImGuiTabItemFlags_Button ) ==
                 0 )
    ...
    if ( width_excess >= 1.0f &&
        ...
        for ( int tab_n = shrink_data_offset;
            ...
            float shrinked_width =
                IM_TRUNC( g.ShrinkWidthBuffer[ tab_n ].Width );
    ...
    for ( int section_n = 0; section_n < 3; section_n++ ) {
        ...
        if ( section_n == 2 )
            tab_offset = ImMin(
                ImMax( 0.0f, tab_bar->BarRect.GetWidth() - section->Width ),
                tab_offset );
...
ImGuiTabItem* ImGui::TabBarFindMostRecentlySelectedTabForActiveWindow(
    ImGuiTabBar* tab_bar ) {
    ImGuiTabItem* most_recently_selected_tab = NULL;
    for ( int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++ ) {
        ImGuiTabItem* tab = &tab_bar->Tabs[ tab_n ];
...
void ImGui::TabBarAddTab( ImGuiTabBar* tab_bar,
    ...
    new_tab.LastFrameVisible =
        tab_bar->CurrFrameVisible; // Required so BeginTabBar() doesn't ditch
                                   // the tab
...
void ImGui::TabBarQueueReorderFromMousePos( ImGuiTabBar* tab_bar,
    ...
    for ( int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir ) {
        ...
        if ( ( dir < 0 && mouse_pos.x > x1 ) ||
             ( dir > 0 && mouse_pos.x < x2 ) )
            break;
    }
...
void ImGui::TabItemSpacing( const char* str_id,
                            ImGuiTabItemFlags flags,
                            float width ) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::TabItemEx( ImGuiTabBar* tab_bar,
    ...
    ImVec2 size = TabItemCalcSize(
        label,
        ( p_open != NULL ) || ( flags & ImGuiTabItemFlags_UnsavedDocument ) );
    ...
    if ( g.DragDropActive &&
         !g.DragDropPayload.IsDataType(
             IMGUI_PAYLOAD_TYPE_WINDOW ) ) // FIXME: May be an opt-in property
                                           // of the payload to disable this
        button_flags |= ImGuiButtonFlags_PressedOnDragDropHold;
    ...
    if ( held && docked_window && g.ActiveId == id &&
    ...
    if ( held && single_floating_window_node && IsMouseDragging( 0, 0.0f ) ) {
        ...
        if ( can_undock ) {
            // We use a variable threshold to distinguish dragging tabs within a
            // tab bar and extracting them out of the tab bar
            bool undocking_tab =
                                  ...
                                  //g.IO.KeyShift)
            {
                ...
                // GetForegroundDrawList()->AddRect(ImVec2(bb.Min.x -
                // threshold_x, bb.Min.y - threshold_y), ImVec2(bb.Max.x +
                // threshold_x, bb.Max.y + threshold_y), IM_COL32_WHITE); //
                // [DEBUG]

                float distance_from_edge_y = ImMax(
                    bb.Min.y - g.IO.MousePos.y, g.IO.MousePos.y - bb.Max.y );
                if ( distance_from_edge_y >= threshold_y )
                    undocking_tab = true;
                if ( drag_distance_from_edge_x > threshold_x )
                    if ( ( drag_dir < 0 &&
                           TabBarGetTabOrder( tab_bar, tab ) == 0 ) ||
                         ( drag_dir > 0 && TabBarGetTabOrder( tab_bar, tab ) ==
                                               tab_bar->Tabs.Size - 1 ) )
                        undocking_tab = true;
            }
            ...
            if ( undocking_tab ) {
                // Undock
                // FIXME: refactor to share more code with e.g.
    ...
    if ( is_visible ) {
        ...
        if ( tab_contents_visible &&
            ...
            ImU32 overline_col = GetColorU32(
                tab_bar_focused ? ImGuiCol_TabSelectedOverline
                                : ImGuiCol_TabDimmedSelectedOverline );
            if ( style.TabRounding > 0.0f ) {
                float rounding = style.TabRounding;
                display_draw_list->PathArcToFast(
                    tl + ImVec2( +rounding, +rounding ), rounding, 7, 9 );
                display_draw_list->PathArcToFast(
                    tr + ImVec2( -rounding, +rounding ), rounding, 9, 11 );
                display_draw_list->PathStroke( overline_col, 0,
                                               style.TabBarOverlineSize );
            } else {
                display_draw_list->AddLine(
                    tl - ImVec2( 0.5f, 0.5f ), tr - ImVec2( 0.5f, 0.5f ),
        ...
        TabItemLabelAndCloseButton(
            ...
            tab_just_unsaved ? ( flags & ~ImGuiTabItemFlags_UnsavedDocument )
                             : flags,
            tab_bar->FramePadding, label, id, close_button_id,
            tab_contents_visible, &just_closed, &text_clipped );
        ...
        // though we are technically hovering our parent) That state is copied
        // to window->DockTabItemStatusFlags by our caller.
        if ( docked_window && ( hovered || g.HoveredId == close_button_id ) )
            g.LastItemData.StatusFlags |= ImGuiItemStatusFlags_HoveredWindow;

        // Tooltip
        // (Won't work over the close button because ItemOverlap systems messes
        // up with HoveredIdTimer-> seems ok) (We test IsItemHovered() to
