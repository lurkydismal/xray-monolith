...
#define IM_ARRAYSIZE( _ARR )    \
    ( ( int )( sizeof( _ARR ) / \
               sizeof( *( _ARR ) ) ) ) // Size of a static C-style array. Don't
                                       // use on pointers!
...
struct ImGuiPlatformIO;     // Interface between platform/renderer backends and
                        // ImGui (e.g. Clipboard, IME, Multi-Viewport support).
                        // Extends ImGuiIO.
...
struct ImGuiTextFilter; // Helper to parse and apply text filters (e.g.
                        // "aaaaa[,bbbbb][,ccccc]")
struct ImGuiViewport;   // A Platform Window (always 1 unless multi-viewport are
                      // enabled. One per platform window to output to). In the
                      // future may represent Platform Monitor
struct ImGuiWindowClass; // Window class (rare/advanced uses: provide hints to
...
IMGUI_API void ShowDemoWindow(
    bool* p_open =
        NULL ); // create Demo window. demonstrate most ImGui features. call
...
// "Debug".
IMGUI_API bool Begin( const char* name,
                      bool* p_open = NULL,
...
IMGUI_API void SetNextWindowContentSize(
    const ImVec2& size ); // set next window content size (~ scrollable client
                          // area, which enforce the range of scrollbars). Not
                          // including window decorations (title bar, menu bar,
                          // etc.) nor WindowPadding. set an axis to 0.0f to
...
IMGUI_API void SetNextWindowScroll(
    const ImVec2& scroll ); // set next window scrolling value (use < 0.0f to
                            // not affect a given axis).
...
IMGUI_API void SetNextWindowViewport(
    ImGuiID viewport_id ); // set next window viewport
...
IMGUI_API void SetWindowFontScale(
    float scale ); // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if
                   // you want to scale all windows. This is an old API! For
                   // correct scaling, prefer to reload font + rebuild
                   // ImFontAtlas + call style.ScaleAllSizes().
...
IMGUI_API void SetWindowFocus(
    const char* name ); // set named window to be focused / top-most. use NULL
                        // to remove focus.
...
IMGUI_API void PushItemWidth(
    float item_width ); // push width of items for common large "item+label"
                        // widgets. >0.0f: width in pixels, <0.0f align xx
                        // pixels to the right of window (so -FLT_MIN always
...
IMGUI_API void SetNextItemWidth(
    float item_width ); // set width of the _next_ common large "item+label"
                        ...
                        // to the right of window (so -FLT_MIN always align
                        // width to the right side)
IMGUI_API float
...
GetColorU32( ImGuiCol idx,
             float alpha_mul =
                 1.0f ); // retrieve given style color with style alpha applied
                         // and optional extra alpha multiplier, packed as a
...
GetColorU32( ImU32 col,
             float alpha_mul =
                 1.0f ); // retrieve given color with style alpha applied,
                         // packed as a 32-bit value suitable for ImDrawList
...
IMGUI_API void SetCursorScreenPos(
    const ImVec2& pos ); // cursor position, absolute coordinates. THIS IS YOUR
                         // BEST FRIEND.
...
IMGUI_API void Indent(
    float indent_w =
        0.0f ); // move content position toward the right, by indent_w, or
                // style.IndentSpacing if indent_w <= 0
IMGUI_API void Unindent(
    float indent_w =
        0.0f ); // move content position back to the left, by indent_w, or
                // style.IndentSpacing if indent_w <= 0
...
IMGUI_API void PushID(
    const char* str_id ); // push string into the ID stack (will hash string).
IMGUI_API void PushID( const char* str_id_begin,
                       const char* str_id_end ); // push string into the ID
                                                 // stack (will hash string).
IMGUI_API void PushID(
    const void* ptr_id ); // push pointer into the ID stack (will hash pointer).
IMGUI_API void PushID(
    int int_id );       // push integer into the ID stack (will hash integer).
...
IMGUI_API bool CheckboxFlags( const char* label,
                              unsigned int* flags,
                              unsigned int flags_value );
IMGUI_API bool RadioButton(
    const char* label,
    bool active ); // use with e.g. if (RadioButton("one", my_value==1)) {
                   // my_value = 1; }
...
Bullet(); // draw a small circle + keep the cursor on the same line. advance
          ...
          // that TreeNode() uses
IMGUI_API bool TextLink(
    const char* label ); // hyperlink text button, return true when clicked
IMGUI_API void TextLinkOpenURL(
    const char* label,
    const char* url = NULL ); // hyperlink text button, automatically open
                              // file/url when clicked
...
// - ImageButton() draws a background based on regular Button() color +
// optionally an inner background if specified.
...
IMGUI_API bool ImageButton( const char* str_id,
...
// created.
IMGUI_API bool BeginCombo( const char* label,
...
// 5 pixels to increase value by 1). For keyboard/gamepad navigation, minimum
...
IMGUI_API bool SliderFloat(
    ...
    ImGuiSliderFlags flags =
        0 ); // adjust format to decorate the value with a prefix or a suffix
...
IMGUI_API bool InputInt( const char* label,
                         ...
                         int step_fast = 100,
                         ImGuiInputTextFlags flags = 0 );
IMGUI_API bool InputInt2( const char* label,
                          int v[ 2 ],
                          ImGuiInputTextFlags flags = 0 );
IMGUI_API bool InputInt3( const char* label,
                          int v[ 3 ],
                          ImGuiInputTextFlags flags = 0 );
IMGUI_API bool InputInt4( const char* label,
                          int v[ 4 ],
                          ImGuiInputTextFlags flags = 0 );
...
IMGUI_API bool ColorEdit4( const char* label,
                           float col[ 4 ],
                           ImGuiColorEditFlags flags = 0 );
IMGUI_API bool ColorPicker3( const char* label,
                             float col[ 3 ],
                             ImGuiColorEditFlags flags = 0 );
...
IMGUI_API bool ColorButton( const char* desc_id,
                            ...
                            const ImVec2& size = ImVec2(
                                0,
                                0 ) ); // display a color square/button, hover
...
IMGUI_API bool TreeNodeEx( const char* str_id,
                           ImGuiTreeNodeFlags flags,
                           const char* fmt,
                           ... ) IM_FMTARGS( 3 );
IMGUI_API bool TreeNodeEx( const void* ptr_id,
                           ImGuiTreeNodeFlags flags,
                           const char* fmt,
                           ... ) IM_FMTARGS( 3 );
...
IMGUI_API void SetNextItemStorageID(
    ImGuiID storage_id ); // set id to use for open/close storage (default to
                          // same as item id).
...
IMGUI_API bool Selectable(
    ...
    const ImVec2& size = ImVec2(
        0,
        0 ) ); // "bool selected" carry the selection state (read-only).
               // Selectable() is clicked is returns true so you can modify your
               // selection state. size.x==0.0: use remaining width, size.x>0.0:
               // specify width. size.y==0.0: use label height, size.y>0.0:
               // specify height
IMGUI_API bool Selectable(
    const char* label,
    bool* p_selected,
...
// [BETA]
// - This enables standard multi-selection/range-selection idioms
// (CTRL+Mouse/Keyboard, SHIFT+Mouse/Keyboard, etc.) in a way that also allow a
// clipper to be used.
// - ImGuiSelectionUserData is often used to store your item index within the
// current view (but may store something else).
...
//   which is suited to advanced trees setups already implementing filters and
//   clipper. We will work simplifying the current demo.
// - 'selection_size' and 'items_count' parameters are optional and used by a
// few features. If they are costly for you to compute, you may avoid them.
IMGUI_API ImGuiMultiSelectIO* BeginMultiSelect( ImGuiMultiSelectFlags flags,
                                                int selection_size = -1,
                                                int items_count = -1 );
IMGUI_API ImGuiMultiSelectIO* EndMultiSelect();
IMGUI_API void SetNextItemSelectionUserData(
    ImGuiSelectionUserData selection_user_data );
IMGUI_API bool
IsItemToggledSelection(); // Was the last item selection state toggled? Useful
                          // if you need the per-item information _before_
                          // reaching EndMultiSelect(). We only returns toggle
...
IMGUI_API void Value( const char* prefix,
                      float v,
...
IMGUI_API void SetItemTooltip( const char* fmt, ... )
    IM_FMTARGS( 1 ); // set a text-only tooltip if preceding item was hovered.
...
IMGUI_API bool TableSetColumnIndex(
    int column_n ); // append into the specified column. Return true when column
                    // is visible.
...
IMGUI_API int
TableGetHoveredColumn(); // return hovered column. return -1 when table is not
                         // hovered. return columns_count if the unused space at
                         // the right of visible columns is hovered. Can also
                         // use (TableGetColumnFlags() &
...
IMGUI_API void Columns( int count = 1,
                        const char* id = NULL,
                        bool borders = true );
...
IMGUI_API bool TabItemButton(
    const char* label,
    ImGuiTabItemFlags flags =
        0 ); // create a Tab behaving like a button. return true when clicked.
...
// - Use DockSpaceOverViewport() to create a window covering the screen or a
// specific viewport + a dockspace inside it.
...
IMGUI_API ImGuiID DockSpace( ImGuiID dockspace_id,
                             const ImVec2& size = ImVec2( 0, 0 ),
                             ImGuiDockNodeFlags flags = 0,
                             const ImGuiWindowClass* window_class = NULL );
IMGUI_API ImGuiID
DockSpaceOverViewport( ImGuiID dockspace_id = 0,
                       const ImGuiViewport* viewport = NULL,
                       ImGuiDockNodeFlags flags = 0,
                       const ImGuiWindowClass* window_class = NULL );
IMGUI_API void SetNextWindowDockID(
    ImGuiID dock_id,
...
IMGUI_API void SetNextWindowClass(
    const ImGuiWindowClass*
        window_class ); // set next window class (control docking compatibility
                        // + provide hints to platform backend via custom
                        // viewport flags and platform parent/child
                        // relationship)
...
IMGUI_API void LogToTTY(
    int auto_open_depth = -1 ); // start logging to tty (stdout)
IMGUI_API void LogToFile(
    int auto_open_depth = -1,
    const char* filename = NULL ); // start logging to file
IMGUI_API void LogToClipboard(
    int auto_open_depth = -1 ); // start logging to OS clipboard
...
IMGUI_API bool BeginDragDropSource(
    ImGuiDragDropFlags flags =
        0 ); // call after submitting an item which may be dragged. when this
             // return true, you can call SetDragDropPayload() +
             // EndDragDropSource()
...
// - BeginDisabled(false)/EndDisabled() essentially does nothing but is provided
// to facilitate use of boolean expressions (as a micro-optimization: if you
// have tens of thousands of BeginDisabled(false)/EndDisabled() pairs, you might
// want to reformulate your code to avoid making those calls)
...
// Focus, Activation
IMGUI_API void SetItemDefaultFocus(); // make last item the default focused item
                                      // of of a newly appearing window.
...
IMGUI_API void SetNavCursorVisible(
    bool visible ); // alter visibility of keyboard/gamepad cursor. by default:
                    // show when using an arrow key, hide when clicking with
                    // mouse.
...
IMGUI_API ImDrawList* GetBackgroundDrawList(
    ImGuiViewport* viewport =
        NULL ); // get background draw list for the given viewport or viewport
                // associated to the current window. this draw list will be the
                // first rendering one. Useful to quickly draw shapes/text
                // behind dear imgui contents.
IMGUI_API ImDrawList* GetForegroundDrawList(
    ImGuiViewport* viewport =
        NULL ); // get foreground draw list for the given viewport or viewport
                // associated to the current window. this draw list will be the
                // top-most rendered one. Useful to quickly draw shapes/text
...
IMGUI_API const char* GetStyleColorName(
    ImGuiCol idx ); // get a string corresponding to the enum value (for
                    // display, saving, etc.).
...
// defined by each backends. This was obsoleted in 1.87 (2022-02) and completely
// removed in 1.91.5 (2024-11). See
// https://github.com/ocornut/imgui/issues/4921)
// - (legacy: any use of ImGuiKey will assert when key < 512 to detect passing
// legacy native/user indices)
...
IMGUI_API bool IsKeyReleased(
    ImGuiKey key ); // was key released (went from Down to !Down)?
IMGUI_API bool IsKeyChordPressed(
    ImGuiKeyChord
        key_chord ); // was key chord (mods + key) pressed, e.g. you can pass
                     // 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord. This
                     // doesn't do any routing or focus check, please consider
                     // using Shortcut() function instead.
...
IMGUI_API const char* GetKeyName(
    ImGuiKey key ); // [DEBUG] returns English name of the key. Those names a
                    // provided for debugging purpose and are not meant to be
                    // saved persistently not compared.
IMGUI_API void SetNextFrameWantCaptureKeyboard(
    bool want_capture_keyboard ); // Override io.WantCaptureKeyboard flag next
                                  ...
                                  // e.g. force capture keyboard when your
                                  // widget is being hovered. This is equivalent
                                  // to setting "io.WantCaptureKeyboard =
                                  // want_capture_keyboard"; after the next
                                  // NewFrame() call.

// Inputs Utilities: Shortcut Testing & Routing [BETA]
// - ImGuiKeyChord = a ImGuiKey + optional
// ImGuiMod_Alt/ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Super.
//       ImGuiKey_C                          // Accepted by functions taking
//       ImGuiKey or ImGuiKeyChord arguments) ImGuiMod_Ctrl | ImGuiKey_C //
//       Accepted by functions taking ImGuiKeyChord arguments)
//   only ImGuiMod_XXX values are legal to combine with an ImGuiKey. You CANNOT
//   combine two ImGuiKey values.
// - The general idea is that several callers may register interest in a
// shortcut, and only one owner gets it.
...
//   - IsKeyChordPressed() compares mods and call IsKeyPressed() -> function has
//   no side-effect.
//   - Shortcut() submits a route, routes are resolved, if it currently can be
//   routed it calls IsKeyChordPressed() -> function has (desirable)
//   side-effects as it can prevents another call from getting the route.
...
// - One common use case would be to allow your items to disable standard inputs
// behaviors such
//   as Tab or Alt key handling, Mouse Wheel scrolling, etc.
//   e.g. Button(...); SetItemKeyOwner(ImGuiKey_MouseWheelY); to make
//   hovering/activating a button disable wheel for scrolling.
// - Reminder ImGuiKey enum include access to mouse buttons and gamepad, so key
// ownership can apply to them.
...
IMGUI_API void SetItemKeyOwner(
    ImGuiKey key ); // Set key owner to last item ID if it is hovered or active.
                    // Equivalent to 'if (IsItemHovered() || IsItemActive()) {
...
IMGUI_API bool IsMouseReleased(
    ImGuiMouseButton
        button ); // did mouse button released? (went from Down to !Down)
...
IMGUI_API bool IsMouseReleasedWithDelay(
    ImGuiMouseButton button,
    float delay ); // delayed mouse release (use very sparingly!). Generally
                   // used with 'delay >= io.MouseDoubleClickTime' + combined
                   // with a 'io.MouseClickedLastCount==1' test. This is a very
                   // rarely used UI idiom, but some apps use this: e.g. MS
                   // Explorer single click on an icon to rename.
IMGUI_API int GetMouseClickedCount(
    ImGuiMouseButton
        button ); // return the number of successive mouse-clicks at the time
...
IMGUI_API bool IsMousePosValid(
    const ImVec2* mouse_pos =
        NULL ); // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there
...
IMGUI_API bool IsMouseDragging(
    ...
    float lock_threshold =
        -1.0f ); // is mouse dragging? (uses io.MouseDraggingThreshold if
                 // lock_threshold < 0.0f)
IMGUI_API ImVec2 GetMouseDragDelta(
    ImGuiMouseButton button = 0,
    float lock_threshold =
        -1.0f ); // return the delta from the initial clicking position while
                 // the mouse button is pressed or was just released. This is
                 // locked and return 0.0f until the mouse moves past a distance
                 // threshold at least once (uses io.MouseDraggingThreshold if
                 // lock_threshold < 0.0f)
...
IMGUI_API void SetMouseCursor(
    ImGuiMouseCursor cursor_type ); // set desired mouse cursor shape
IMGUI_API void SetNextFrameWantCaptureMouse(
    bool want_capture_mouse ); // Override io.WantCaptureMouse flag next frame
                               // (said flag is left for your application to
                               // handle, typical when true it instucts your app
                               // to ignore inputs). This is equivalent to
                               // setting "io.WantCaptureMouse =
                               // want_capture_mouse;" after the next NewFrame()
                               // call.
...
// current window.
IMGUI_API void
UpdatePlatformWindows(); // call in main loop. will call
                         // CreateWindow/ResizeWindow/etc. platform functions
                         // for each secondary viewport, and DestroyWindow for
                         // each inactive viewport.
IMGUI_API void RenderPlatformWindowsDefault(
    void* platform_render_arg = NULL,
    void* renderer_render_arg =
        NULL ); // call in main loop. will call RenderWindow/SwapBuffers
                // platform functions for each secondary viewport which doesn't
                // have the ImGuiViewportFlags_Minimized flag set. May be
...
IMGUI_API ImGuiViewport* FindViewportByID(
    ImGuiID id ); // this is a helper for backends.
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_NoCollapse =
        1
        << 5, // Disable user collapsing window by double-clicking on it. Also
    ...
    ImGuiWindowFlags_NoNavInputs =
        1 << 16, // No keyboard/gamepad navigation within the window
    ImGuiWindowFlags_NoNavFocus =
        1 << 17, // No focusing toward this window with keyboard/gamepad
                 // navigation (e.g. skipped by CTRL+TAB)
    ImGuiWindowFlags_UnsavedDocument =
        1 << 18, // Display a dot next to the title. When used in a tab/docking
                 // context, tab is selected when clicking the X + closure is
                 // not assumed (will wait for user to stop submitting the tab).
    ...
    ImGuiWindowFlags_DockNodeHost =
        1 << 23, // Don't use! For internal use by Begin()/NewFrame()
...
// Obsolete names
    ...
    ImGuiWindowFlags_NavFlattened =
        1 << 29, // Obsoleted in 1.90.9: Use ImGuiChildFlags_NavFlattened in
                 // BeginChild() call.
    ImGuiWindowFlags_AlwaysUseWindowPadding =
        1 << 30, // Obsoleted in 1.90.0: Use
                 // ImGuiChildFlags_AlwaysUseWindowPadding in BeginChild() call.
#endif
};

// Flags for ImGui::BeginChild()
// (Legacy: bit 0 must always correspond to ImGuiChildFlags_Borders to be
...
enum ImGuiChildFlags_ {
    ...
    ImGuiChildFlags_NavFlattened =
        1 << 8, // [BETA] Share focus scope, allow keyboard/gamepad navigation
                // to cross over parent border to this child or between sibling
                // child windows.

// Obsolete names
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiChildFlags_Border =
        ImGuiChildFlags_Borders, // Renamed in 1.91.1 (August 2024) for
                                 // consistency.
#endif
};
...
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_NoNav =
        1 << 1, // false    // Disable any form of focusing (keyboard/gamepad
                // directional navigation and SetKeyboardFocusHere() calls).
    ImGuiItemFlags_NoNavDefaultFocus =
        1 << 2, // false    // Disable item being a candidate for default focus
                // (e.g. used by title bar items).
    ImGuiItemFlags_ButtonRepeat =
        1 << 3, // false    // Any button-like behavior will have repeat mode
                // enabled (based on io.KeyRepeatDelay and io.KeyRepeatRate
                // values). Note that you can also call IsItemActive() after any
                // button to tell if it is being held.
    ImGuiItemFlags_AutoClosePopups =
        1 << 4, // true     // MenuItem()/Selectable() automatically close their
                // parent popup window.
    ImGuiItemFlags_AllowDuplicateId =
        1 << 5, // false    // Allow submitting an item with the same identifier
                // as an item already submitted this frame without triggering a
                // warning tooltip if io.ConfigDebugHighlightIdConflicts is set.
};
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_EnterReturnsTrue =
        1 << 6, // Return 'true' when Enter is pressed (as opposed to every time
                // the value was modified). Consider using
    ...
    ImGuiInputTextFlags_ParseEmptyRefVal =
        1 << 13, // InputFloat(), InputInt(), InputScalar() etc. only: parse
                 // empty string as zero value.
    ImGuiInputTextFlags_DisplayEmptyRefVal =
        1 << 14, // InputFloat(), InputInt(), InputScalar() etc. only: when
    ...
    ImGuiInputTextFlags_ElideLeft =
        1 << 17, // When text doesn't fit, elide left side to ensure right side
                 // stays visible. Useful for path/filenames. Single-line only!
    ...
    ImGuiInputTextFlags_CallbackAlways =
        1 << 20, // Callback on each iteration. User code may query cursor
                 // position, modify text buffer.
    ImGuiInputTextFlags_CallbackCharFilter =
        1 << 21, // Callback on character inputs to replace or discard them.
    ...
    ImGuiInputTextFlags_CallbackResize =
        1 << 22, // Callback on buffer capacity changes request (beyond
                 ...
                 // which hold a cache of their Size). You will be provided a
                 // new BufSize in the callback and NEED to honor it. (see
    ...
    ImGuiInputTextFlags_CallbackEdit =
        1 << 23, // Callback on any edit. Note that InputText() already returns
                 ...
                 // callback is useful to manipulate the underlying buffer while
                 // focus is active.

    // Obsolete names
    // ImGuiInputTextFlags_AlwaysInsertMode  =
    // ImGuiInputTextFlags_AlwaysOverwrite   // [renamed in 1.82] name was not
    // matching behavior
};
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_OpenOnDoubleClick =
        1 << 6, // Open on double-click instead of simple click (default for
                // multi-select unless any _OpenOnXXX behavior is set
                // explicitly). Both behaviors may be combined.
    ImGuiTreeNodeFlags_OpenOnArrow =
        1 << 7, // Open when clicking on the arrow part (default for
                // multi-select unless any _OpenOnXXX behavior is set
                // explicitly). Both behaviors may be combined.
    ...
    ImGuiTreeNodeFlags_Bullet =
        1 << 9, // Display a bullet instead of arrow. IMPORTANT: node can still
                // be marked open/close if you don't set the _Leaf flag!
    ImGuiTreeNodeFlags_FramePadding =
        1 << 10, // Use FramePadding (even for an unframed text node) to
                 // vertically align text baseline to regular widget height.
                 // Equivalent to calling AlignTextToFramePadding() before the
                 // node.
    ImGuiTreeNodeFlags_SpanAvailWidth =
        1 << 11, // Extend hit box to the right-most edge, even if not framed.
                 ...
                 // on the same line without using AllowOverlap mode.
    ImGuiTreeNodeFlags_SpanFullWidth =
        1 << 12, // Extend hit box to the left-most and right-most edges (cover
                 // the indent area).
    ImGuiTreeNodeFlags_SpanLabelWidth =
        1 << 13, // Narrow hit box + narrow hovering highlight, will only cover
                 // the label text.
    ImGuiTreeNodeFlags_SpanAllColumns =
        1 << 14, // Frame will span all columns of its container table (label
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
        1
        << 17, // (WIP) Nav: left direction may move to this TreeNode() from any
    ...
    ImGuiTreeNodeFlags_SpanTextWidth =
        ImGuiTreeNodeFlags_SpanLabelWidth, // Renamed in 1.90.7
#endif
};
...
enum ImGuiPopupFlags_ {
    ...
    ImGuiPopupFlags_MouseButtonRight =
        1, // For BeginPopupContext*(): open on Right Mouse release. Guaranteed
    ...
    ImGuiPopupFlags_NoReopen =
        1 << 5, // For OpenPopup*(), BeginPopupContext*(): don't reopen same
                // popup if already open (won't reposition, won't reinitialize
                // navigation)
    ...
    ImGuiPopupFlags_NoOpenOverItems =
        1 << 8, // For BeginPopupContextWindow(): don't return true when
                // hovering items, only when hovering empty space
    ImGuiPopupFlags_AnyPopupId = 1
                                 << 10, // For IsPopupOpen(): ignore the ImGuiID
                                        // parameter and test for any popup.
    ImGuiPopupFlags_AnyPopupLevel =
        1 << 11, // For IsPopupOpen(): search/test at any level of the popup
...
enum ImGuiSelectableFlags_ {
    ...
    ImGuiSelectableFlags_NoAutoClosePopups =
        1 << 0, // Clicking this doesn't close parent popup window (overrides
                // ImGuiItemFlags_AutoClosePopups)
    ...
    ImGuiSelectableFlags_AllowOverlap =
        1 << 4, // (WIP) Hit testing to allow subsequent widgets to overlap this
                // one
    ImGuiSelectableFlags_Highlight =
        1 << 5, // Make the item be displayed as if it is hovered
...
enum ImGuiTabItemFlags_ {
    ...
    ImGuiTabItemFlags_NoReorder = 1
                                  << 5, // Disable reordering this tab or having
    ...
    ImGuiTabItemFlags_NoAssumedClosure =
        1
        << 8, // Tab is selected when trying to close + closure is not
...
enum ImGuiFocusedFlags_ {
    ...
    ImGuiFocusedFlags_RootWindow = 1 << 1, // Test from root window (top most
                                           // parent of the current hierarchy)
    ImGuiFocusedFlags_AnyWindow =
        1 << 2, // Return true if any window is focused. Important: If you are
                // trying to tell how to dispatch your low-level inputs, do NOT
                // use this. Use 'io.WantCaptureMouse' instead! Please read the
                // FAQ!
    ImGuiFocusedFlags_NoPopupHierarchy =
        1
        << 3, // Do not consider popup hierarchy (do not treat popup emitter as
...
enum ImGuiHoveredFlags_ {
    ...
    ImGuiHoveredFlags_NoPopupHierarchy =
        1 << 3, // IsWindowHovered() only: Do not consider popup hierarchy (do
    ...
    ImGuiHoveredFlags_AllowWhenBlockedByPopup =
        1 << 5, // Return true even if a popup window is normally blocking
    ...
    ImGuiHoveredFlags_AllowWhenOverlappedByItem =
        1
        << 8, // IsItemHovered() only: Return true even if the item uses
    ...
    ImGuiHoveredFlags_AllowWhenDisabled =
        1
        << 10, // IsItemHovered() only: Return true even if the item is disabled
    ImGuiHoveredFlags_NoNavOverride =
        1 << 11, // IsItemHovered() only: Disable using keyboard/gamepad
    ...
    // may to use ImGuiHoveredFlags_ForTooltip (stationary + delay) so the
    // tooltip doesn't show too often.
    ...
    ImGuiHoveredFlags_Stationary =
        1 << 13, // Require mouse to be stationary for
                 // style.HoverStationaryDelay (~0.15 sec) _at least one time_.
                 // After this, can move on same item/window. Using the
    ...
    ImGuiHoveredFlags_DelayShort =
        1
        << 15, // IsItemHovered() only: Return true after style.HoverDelayShort
               // elapsed (~0.15 sec) (shared between items) + requires mouse to
               // be stationary for style.HoverStationaryDelay (once per item).
    ImGuiHoveredFlags_DelayNormal =
        1
        << 16, // IsItemHovered() only: Return true after style.HoverDelayNormal
               // elapsed (~0.40 sec) (shared between items) + requires mouse to
               // be stationary for style.HoverStationaryDelay (once per item).
    ImGuiHoveredFlags_NoSharedDelay =
        1 << 17, // IsItemHovered() only: Disable shared delay system where
...
enum ImGuiDockNodeFlags_ {
    ...
    ImGuiDockNodeFlags_KeepAliveOnly =
        1 << 0, //       // Don't display the dockspace node but keep it alive.
                //       Windows docked into this dockspace node won't be
                //       undocked.
    ...
    ImGuiDockNodeFlags_PassthruCentralNode =
        1
        << 3, //       // Enable passthru dockspace: 1) DockSpace() will render
              //       a ImGuiCol_WindowBg background covering everything
    ...
    ImGuiDockNodeFlags_NoDockingSplit =
        1
        << 4, //       // Disable other windows/nodes from splitting this node.
    ImGuiDockNodeFlags_NoResize =
        1 << 5, // Saved // Disable resizing node using the splitter/separators.
                // Useful with programmatically setup dockspaces.
    ImGuiDockNodeFlags_AutoHideTabBar =
        1 << 6, //       // Tab bar will automatically hide when there is a
                //       single window in the dock node.
...
enum ImGuiDragDropFlags_ {
    ...
    ImGuiDragDropFlags_SourceNoDisableHover =
        1 << 1, // By default, when dragging we clear data so that
                ...
                // code submitting tooltips. This flag disables this behavior so
    ...
    ImGuiDragDropFlags_SourceExtern =
        1 << 4, // External source (from outside of dear imgui), won't attempt
    ...
    ImGuiDragDropFlags_PayloadNoCrossContext =
        1 << 6, // Hint to specify that the payload may not be copied outside
                // current dear imgui context.
    ImGuiDragDropFlags_PayloadNoCrossProcess =
        1 << 7, // Hint to specify that the payload may not be copied outside
                // current process.
...
enum ImGuiDataType_ {
    ...
    ImGuiDataType_Bool, // bool (provided for user convenience, not supported by
                        // scalar widgets)
    ImGuiDataType_String, // char* (provided for user convenience, not supported
                          // by scalar widgets)
    ImGuiDataType_COUNT
};
...
// 0 to 511 are left unused and were legacy native/opaque key values (< 1.87).
// Support for legacy keys was completely removed in 1.91.5.
// Read details about the 1.87+ transition :
...
enum ImGuiKey : int {
    ...
    // - On macOS, we swap Cmd(Super) and Ctrl keys at the time of the
    // io.AddKeyEvent() call.
    ImGuiMod_None = 0,
...
// ImGuiKey_NamedKey_BEGIN) index.

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiKey_COUNT =
        ImGuiKey_NamedKey_END, // Obsoleted in 1.91.5 because it was extremely
                               // misleading (since named keys don't start at 0
                               // anymore)
    ImGuiMod_Shortcut = ImGuiMod_Ctrl, // Removed in 1.90.7, you can now simply
                                       // use ImGuiMod_Ctrl
...
// (and for upcoming extended versions of IsKeyPressed(), IsMouseClicked(),
// Shortcut(), SetKeyOwner(), SetItemKeyOwner() that are still in
// imgui_internal.h) Don't mistake with ImGuiInputTextFlags! (which is for
// ImGui::InputText() function)
enum ImGuiInputFlags_ {
    ImGuiInputFlags_None = 0,
    ImGuiInputFlags_Repeat =
        1 << 0, // Enable repeat. Return true on successive repeats. Default for
                // legacy IsKeyPressed(). NOT Default for legacy
                // IsMouseClicked(). MUST BE == 1.

    // Flags for Shortcut(), SetNextItemShortcut()
    // - Routing policies: RouteGlobal+OverActive >> RouteActive or RouteFocused
    // (if owner is active item) >> RouteGlobal+OverFocused >> RouteFocused (if
    // in focused window stack) >> RouteGlobal.
    // - Default policy is RouteFocused. Can select only 1 policy among all
    // available.
    ImGuiInputFlags_RouteActive = 1 << 10, // Route to active item only.
    ImGuiInputFlags_RouteFocused =
        1 << 11, // Route to windows in the focus stack (DEFAULT). Deep-most
                 // focused window takes inputs. Active item takes inputs over
                 // deep-most focused window.
    ImGuiInputFlags_RouteGlobal =
        1 << 12, // Global route (unless a focused window or active item
    ...
    ImGuiInputFlags_RouteOverFocused =
        1 << 14, // Option: global route: higher priority than focused route
                 // (unless active item in focused route).
    ImGuiInputFlags_RouteOverActive =
        1 << 15, // Option: global route: higher priority than active item.
                 // Unlikely you need to use that: will interfere with every
                 // active items, e.g. CTRL+A registered by InputText will be
                 // overridden by this. May not be fully honored as
                 // user/internal code is likely to always assume they can
                 // access keys when active.
    ImGuiInputFlags_RouteUnlessBgFocused =
        1 << 16, // Option: global route: will not be applied if underlying
    ...
    ImGuiInputFlags_RouteFromRootWindow =
        1 << 17, // Option: route evaluated from the point of view of root
                 // window rather than current window.

    // Flags for SetNextItemShortcut()
    ImGuiInputFlags_Tooltip =
        1 << 18, // Automatically display a tooltip when hovering item [BETA]
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NoMouseCursorChange =
        1 << 5, // Instruct backend to not alter mouse cursor shape and
                ...
                // change mouse cursor. You may want to honor requests from
                // imgui by reading GetMouseCursor() yourself instead.
    ImGuiConfigFlags_NoKeyboard =
        1 << 6, // Instruct dear imgui to disable keyboard inputs and
                // interactions. This is done by ignoring keyboard events and
                // clearing existing states.
    ...
    ImGuiConfigFlags_ViewportsEnable =
        1 << 10, // Viewport enable flags (require both
                 // ImGuiBackendFlags_PlatformHasViewports +
                 // ImGuiBackendFlags_RendererHasViewports set by the respective
                 // backends)
    ImGuiConfigFlags_DpiEnableScaleViewports =
        1
        << 14, // [BETA: Don't use] FIXME-DPI: Reposition and resize imgui
    ...
    ImGuiConfigFlags_DpiEnableScaleFonts =
        1 << 15, // [BETA: Don't use] FIXME-DPI: Request bitmap-scaled fonts to
                 ...
                 // correct way to handle DPI is _currently_ to replace the
                 // atlas and/or fonts in the Platform_OnChangedViewport
                 // callback, but this is all early work in progress.

    // User storage (to allow your backend/engine to communicate to code that
    // may be shared between multiple projects. Those flags are NOT used by core
    // Dear ImGui)
    ...
    ImGuiConfigFlags_NavEnableSetMousePos =
        1 << 2, // [moved/renamed in 1.91.4] -> use bool
                // io.ConfigNavMoveSetMousePos
    ImGuiConfigFlags_NavNoCaptureKeyboard =
        1 << 3, // [moved/renamed in 1.91.4] -> use bool
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseHoveredViewport =
        1 << 11, // Backend Platform supports calling io.AddMouseViewportEvent()
                 ...
                 // viewports with the ImGuiViewportFlags_NoInputs flag (Win32
                 // backend, GLFW 3.30+ backend can do this, SDL backend
                 // cannot). If this cannot be done, Dear ImGui needs to use a
                 // flawed heuristic to find the viewport under.
...
enum ImGuiCol_ {
    ...
    ImGuiCol_TabSelected, // Tab background, when tab-bar is focused & tab is
                          // selected
    ImGuiCol_TabSelectedOverline, // Tab horizontal overline, when tab-bar is
                                  // focused & tab is selected
    ImGuiCol_TabDimmed, // Tab background, when tab-bar is unfocused & tab is
                        // unselected
    ...
    ImGuiCol_DockingEmptyBg, // Background color for empty node (e.g.
                             // CentralNode with no window docked into it)
    ImGuiCol_PlotLines,
    ...
    ImGuiCol_TabUnfocusedActive =
        ImGuiCol_TabDimmedSelected,             // [renamed in 1.90.9]
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_EnableNav =
        1 << 3, // InvisibleButton(): do not disable navigation/tabbing.
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_NoAlpha =
        1 << 1, //              // ColorEdit, ColorPicker, ColorButton: ignore
                //              Alpha component (will only read 3 components
    ...
    // Alpha preview
    // - Prior to 1.91.8 (2025/01/21): alpha was made opaque in the preview by
    // default using old name ImGuiColorEditFlags_AlphaPreview.
    ...
    ImGuiColorEditFlags_AlphaOpaque =
        1 << 11, //              // ColorEdit, ColorPicker, ColorButton: disable
                 //              alpha in the preview,. Contrary to _NoAlpha it
                 //              may still be edited when calling
                 //              ColorEdit4()/ColorPicker4(). For ColorButton()
                 //              this does the same as _NoAlpha.
    ImGuiColorEditFlags_AlphaNoBg =
        1 << 12, //              // ColorEdit, ColorPicker, ColorButton: disable
                 //              rendering a checkerboard background behind
                 //              transparent color.
    ImGuiColorEditFlags_AlphaPreviewHalf =
        1 << 13, //              // ColorEdit, ColorPicker, ColorButton: display
    ...
    ImGuiColorEditFlags_DisplayRGB =
        1 << 20, // [Display]    // ColorEdit: override _display_ type among
                 // RGB/HSV/Hex. ColorPicker: select any combination using one
                 // or more of RGB/HSV/Hex.
    ...
    ImGuiColorEditFlags_AlphaMask_ =
        ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaOpaque |
...
// Obsolete names
    ...
    ImGuiColorEditFlags_AlphaPreview =
        0, // [Removed in 1.91.8] This is the default now. Will display a
           // checkerboard unless ImGuiColorEditFlags_AlphaNoBg is set.
#endif
...
// per-item flags. There is shared behavior flag too: ImGuiIO:
// io.ConfigDragClickToInputText)
enum ImGuiSliderFlags_ {
    ImGuiSliderFlags_None = 0,
    ImGuiSliderFlags_Logarithmic =
        1 << 5, // Make the widget logarithmic (linear otherwise). Consider
                // using ImGuiSliderFlags_NoRoundToFormat with this if using a
                // format-string with small amount of digits.
    ImGuiSliderFlags_NoRoundToFormat =
        1 << 6, // Disable rounding underlying value to match precision of the
                // display format string (e.g. %.3f values are rounded to those
                // 3 digits).
    ImGuiSliderFlags_NoInput = 1
                               << 7, // Disable CTRL+Click or Enter key allowing
                                     // to input text directly into the widget.
    ImGuiSliderFlags_WrapAround =
        1 << 8, // Enable wrapping around from max to min and from min to max.
    ...
    ImGuiSliderFlags_ClampZeroRange =
        1
        << 10, // Clamp even if min==max==0.0f. Otherwise due to legacy reason
    ...
    ImGuiSliderFlags_NoSpeedTweaks =
        1
        << 11, // Disable keyboard modifiers altering tweak speed. Useful if you
    ...
    ImGuiSliderFlags_InvalidMask_ =
        0x7000000F, // [Internal] We treat using those bits as being potentially
                    // a 'float power' argument from the previous API that has
                    // got miscast to this enum, and will trigger an assert if
                    // needed.
};
...
enum ImGuiMouseCursor_ {
    ...
    ImGuiMouseCursor_ResizeNESW, // When hovering over the bottom-left corner of
                                 // a window
    ImGuiMouseCursor_ResizeNWSE, // When hovering over the bottom-right corner
                                 // of a window
...
enum ImGuiMouseSource : int {
    ...
    ImGuiMouseSource_TouchScreen, // Input is coming from a touch screen (no
                                  // hovering prior to initial press, less
                                  // precise initial press aiming, dual-axis
                                  // wheeling possible).
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_SizingFixedSame =
        2 << 13, // Columns default to _WidthFixed or _WidthAuto (if resizable
                 // or not resizable), matching the maximum contents width of
                 // all columns. Implicitly enable
    ...
    ImGuiTableFlags_SizingStretchSame =
        4 << 13, // Columns default to _WidthStretch with default weights all
                 // equal, unless overridden by TableSetupColumn(). Sizing Extra
                 // Options
    ImGuiTableFlags_NoHostExtendX =
        1 << 16, // Make outer width auto-fit to columns, overriding
                 // outer_size.x value. Only available when ScrollX/ScrollY are
    ...
    ImGuiTableFlags_PreciseWidths =
        1 << 19, // Disable distributing remainder width to stretched columns
                 // (width allocation on a 100-wide table with 3 columns:
                 // Without this flag: 33,33,34. With this flag: 33,33,33). With
                 // larger number of columns, resizing will appear to be less
                 // smooth. Clipping
    ...
    ImGuiTableFlags_NoPadInnerX =
        1 << 23, // Disable inner padding between columns (double inner padding
                 // if BordersOuterV is on, single inner padding if
                 // BordersOuterV is off). Scrolling
    ImGuiTableFlags_ScrollX =
        1
        << 24, // Enable horizontal scrolling. Require 'outer_size' parameter of
    ...
    ImGuiTableFlags_SortTristate =
        1
        << 27, // Allow no sorting, disable default sorting. TableGetSortSpecs()
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_WidthFixed =
        1 << 4, // Column will not stretch. Preferable with horizontal scrolling
                // enabled (default if table sizing policy is _SizingFixedFit
    ...
    ImGuiTableColumnFlags_NoHeaderLabel =
        1 << 12, // TableHeadersRow() will submit an empty label for this
                 // column. Convenient for some small columns. Name will still
                 // appear in context menu or in angled headers. You may append
                 // into this cell by calling TableSetColumnIndex() right after
    ...
    ImGuiTableColumnFlags_PreferSortDescending =
        1 << 15, // Make the initial sort direction Descending when first
    ...
    ImGuiTableColumnFlags_IndentDisable =
        1 << 17, // Ignore current Indent value when entering cell (default for
                 // columns > 0). Indentation changes _within_ the cell will
                 // still be honored.
    ...
    ImGuiTableColumnFlags_IsVisible =
        1
        << 25, // Status: is visible == is enabled AND not clipped by scrolling.
...
struct ImGuiTableColumnSortSpecs {
    ...
    ImS16 SortOrder; // Index within parent ImGuiTableSortSpecs (always stored
                     // in order starting from 0, tables sorted on a single
                     // criteria will always have a 0 here)
    ImGuiSortDirection SortDirection; // ImGuiSortDirection_Ascending or
...
struct ImVector {
    ...
    inline T* erase( const T* it, const T* it_last ) {
        ...
        memmove( Data + off, Data + off + count,
                 ( ( size_t )Size - ( size_t )off - ( size_t )count ) *
                     sizeof( T ) );
        Size -= ( int )count;
        return Data + off;
    }
...
struct ImGuiStyle {
    ...
    float
        DisabledAlpha; // Additional alpha multiplier applied by
    ...
    ImVec2 WindowMinSize;   // Minimum window size. This is a global setting. If
                            // you want to constrain individual windows, use
    ...
    ImVec2 ItemInnerSpacing; // Horizontal and vertical spacing between within
                             ...
                             // its label).
    ImVec2 CellPadding; // Padding within a table cell. Cellpadding.x is locked
                        // for entire table. CellPadding.y may be altered
    ...
    float TabMinWidthForCloseButton; // Minimum width for close button to appear
                                     // on an unselected tab when hovered. Set
                                     // to 0.0f to always show when hovering,
                                     // set to FLT_MAX to never show close
    ...
    float TabBarOverlineSize; // Thickness of tab-bar overline, which highlights
                              // the selected tab-bar.
    float TableAngledHeadersAngle; // Angle of angled headers (supported values
                                   // range from -50.0f degrees to +50.0f
                                   // degrees).
    ImVec2 TableAngledHeadersTextAlign; // Alignment of angled headers within
                                        // the cell
    ImGuiDir
        ColorButtonPosition; // Side of the color button in the ColorEdit4
    ...
    ImVec2
        SelectableTextAlign; // Alignment of selectable text. Defaults to (0.0f,
                             // 0.0f) (top-left aligned). It's generally
                             // important to keep this left-aligned if you want
                             // to lay multiple items on a same line.
    float SeparatorTextBorderSize; // Thickness of border in SeparatorText()
    ImVec2
        SeparatorTextAlign; // Alignment of text within the separator. Defaults
    ...
    ImVec2 SeparatorTextPadding; // Horizontal offset of text from each edge of
                                 ...
                                 // be == FramePadding.y.
    ImVec2 DisplayWindowPadding; // Apply to regular windows: amount which we
                                 // enforce to keep visible when moving near
                                 // edges of your screen.
    ImVec2 DisplaySafeAreaPadding; // Apply to every windows, menus, popups,
                                   // tooltips: amount where we avoid displaying
                                   // contents. Adjust if you cannot see the
                                   // edges of your screen (e.g. on a TV where
    ...
    bool AntiAliasedLinesUseTex; // Enable anti-aliased lines/borders using
                                 ...
                                 // beginning of the frame (copied to
                                 // ImDrawList).
    ...
    float CircleTessellationMaxError; // Maximum error (in pixels) allowed when
                                      // using AddCircle()/AddCircleFilled() or
                                      // drawing rounded corner rectangles with
                                      // no explicit segment count specified.
                                      // Decrease for higher quality but more
                                      // geometry.
    ImVec4 Colors[ ImGuiCol_COUNT ];

    // Behaviors
    // (It is possible to modify those fields mid-frame if specific behavior
    // need it, unlike e.g. configuration fields in ImGuiIO)
    ...
    ImGuiHoveredFlags
        HoverFlagsForTooltipMouse; // Default flags when using
                                   ...
                                   // or BeginItemTooltip()/SetItemTooltip()
                                   // while using mouse.
    ImGuiHoveredFlags
...
// ImGuiIO.
//-----------------------------------------------------------------------------
// Also see ImGui::GetPlatformIO() and ImGuiPlatformIO struct for OS/platform
// related functions: clipboard, IME etc.
//-----------------------------------------------------------------------------
...
struct ImGuiIO {
    ...
    const char*
        IniFilename; // = "imgui.ini"    // Path to .ini file (important:
                     // default "imgui.ini" is relative to current working
                     // dir!). Set NULL to disable automatic .ini loading/saving
                     // or if you want to manually call LoadIniSettingsXXX() /
    ...
    bool FontAllowUserScaling; // = false          // [OBSOLETE] Allow user
                               // scaling text of individual window with
                               // CTRL+Wheel.
    ...
    ImVec2 DisplayFramebufferScale; // = (1, 1)         // For retina display or
                                    ...
                                    // are different from framebuffer
                                    // coordinates. This generally ends up in
                                    // ImDrawData::FramebufferScale.

    // Keyboard/Gamepad Navigation options
    bool ConfigNavSwapGamepadButtons; // = false          // Swap
                                      // Activate<>Cancel (A<>B) buttons,
                                      // matching typical "Nintendo/Japanese
                                      // style" gamepad layout.
    bool ConfigNavMoveSetMousePos;    // = false          // Directional/tabbing
                                   // navigation teleports the mouse cursor. May
                                   // be useful on TV/console systems where
                                   // moving a virtual mouse is difficult. Will
                                   // update io.MousePos and set
                                   // io.WantSetMousePos=true.
    bool ConfigNavCaptureKeyboard; // = true           // Sets
                                   // io.WantCaptureKeyboard when io.NavActive
                                   // is set.
    bool ConfigNavEscapeClearFocusItem;   // = true           // Pressing Escape
                                          // can clear focused item + navigation
                                          // id/highlight. Set to false if you
                                          // want to always keep highlight on.
    bool ConfigNavEscapeClearFocusWindow; // = false          // Pressing Escape
                                          // can clear focused window as well
                                          // (super set of
                                          // io.ConfigNavEscapeClearFocusItem).
    bool ConfigNavCursorVisibleAuto; // = true           // Using directional
                                     // navigation key makes the cursor visible.
                                     // Mouse click hides the cursor.
    bool ConfigNavCursorVisibleAlways; // = false          // Navigation cursor
                                       // is always visible.

    // Docking options (when ImGuiConfigFlags_DockingEnable is set)
    bool ConfigDockingNoSplit;   // = false          // Simplified docking mode:
                                 // disable window splitting, so docking is
                                 // limited to merging multiple windows together
                                 // into tab-bars.
    bool ConfigDockingWithShift; // = false          // Enable docking with
                                 // holding Shift key (reduce visual noise,
    ...
    bool ConfigDockingAlwaysTabBar; // = false          // [BETA] [FIXME: This
                                    ...
                                    // every single floating window display
                                    // within a docking node.
    bool
        ConfigDockingTransparentPayload; // = false          // [BETA] Make
    ...
    bool ConfigViewportsNoAutoMerge; // = false;         // Set to make all
                                     // floating imgui windows always create
                                     // their own viewport. Otherwise, they are
                                     // merged into the main host viewports when
                                     // overlapping it. May also set
                                     // ImGuiViewportFlags_NoAutoMerge on
                                     // individual viewport.
    bool ConfigViewportsNoTaskBarIcon; // = false          // Disable default OS
                                       // task bar icon flag for secondary
                                       // viewports. When a viewport doesn't
                                       // want a task bar icon,
                                       // ImGuiViewportFlags_NoTaskBarIcon will
                                       // be set on it.
    ...
    bool ConfigViewportsNoDefaultParent; // = false          // Disable default
                                         ...
                                         // expecting the platform backend to
                                         // setup a parent/child relationship
                                         // between the OS windows (some backend
                                         // may ignore this). Set to true if you
                                         // want the default to be 0, then all
                                         // viewports will be top-level OS
                                         // windows.
    ...
    bool ConfigMacOSXBehaviors; // = defined(__APPLE__) // Swap Cmd<>Ctrl keys +
                                // OS X style text editing cursor movement using
                                // Alt instead of Ctrl, Shortcuts using
                                // Cmd/Super instead of Ctrl, Line/Text Start
                                // and End using Cmd+Arrows instead of Home/End,
                                // Double click selects by word instead of
                                // selecting whole text, Multi-selection in
    ...
    bool ConfigInputTextEnterKeepActive; // = false          // [BETA] Pressing
                                         // Enter will keep item active and
                                         // select contents (single-line only).
    bool ConfigDragClickToInputText;     // = false          // [BETA] Enable
                                     // turning DragXXX widgets into text input
                                     // with a simple mouse click-release
                                     // (without moving). Not desirable on
    ...
    bool ConfigWindowsResizeFromEdges; // = true           // Enable resizing of
                                       ...
                                       // ImGuiBackendFlags_HasMouseCursors for
                                       // better mouse cursor feedback. (This
                                       // used to be a per-window
                                       // ImGuiWindowFlags_ResizeFromAnySide
                                       // flag)
    bool
        ConfigWindowsMoveFromTitleBarOnly; // = false      // Enable allowing to
                                           // move windows only when clicking on
                                           // their title bar. Does not apply to
                                           // windows without a title bar.
    bool ConfigWindowsCopyContentsWithCtrlC; // = false      // [EXPERIMENTAL]
                                             // CTRL+C copy the contents of
                                             // focused window into the
                                             // clipboard. Experimental because:
                                             // (1) has known issues with nested
                                             // Begin/End pairs (2) text output
                                             // quality varies (3) text output
                                             // is in submission order rather
                                             // than spatial order.
    bool ConfigScrollbarScrollByPage; // = true           // Enable scrolling
                                      // page by page when clicking outside the
                                      // scrollbar grab. When disabled, always
                                      // scroll to clicked location. When
                                      // enabled, Shift+Click scrolls to clicked
                                      // location.
    float ConfigMemoryCompactTimer; // = 60.0f          // Timer (in seconds) to
                                    // free transient windows/tables memory
                                    // buffers when unused. Set to -1.0f to
                                    // disable.

    // Inputs Behaviors
    // (other variables, ones which are expected to be tweaked within UI code,
    // are exposed in ImGuiStyle)
    float MouseDoubleClickTime; // = 0.30f          // Time for a double-click,
                                // in seconds.
    float MouseDoubleClickMaxDist; // = 6.0f           // Distance threshold to
                                   // stay in to validate a double-click, in
                                   // pixels.
    ...
    float KeyRepeatDelay; // = 0.275f         // When holding a key/button, time
                          ...
                          // in Repeat mode, etc.).
    float KeyRepeatRate;  // = 0.050f         // When holding a key/button, rate
                          // at which it repeats, in seconds.
    ...
    //    - Recovery after a programming error (native code or scripting
    //    language - the later tends to facilitate iterating on code while
    //    running).
    //    - Recovery after running an exception handler or any error processing
    //    which may skip code after an error has been detected.
    // - Error recovery is not perfect nor guaranteed! It is a feature to ease
    // development.
    //   You not are not supposed to rely on it in the course of a normal
    //   application run.
    // - Functions that support error recovery are using IM_ASSERT_USER_ERROR()
    // instead of IM_ASSERT().
    // - By design, we do NOT allow error recovery to be 100% silent. One of the
    // three options needs to be checked!
    // - Always ensure that on programmers seats you have at minimum Asserts or
    // Tooltips enabled when making direct imgui API calls!
    //   Otherwise it would severely hinder your ability to catch and correct
    //   mistakes!
    // Read https://github.com/ocornut/imgui/wiki/Error-Handling for details.
    // - Programmer seats: keep asserts (default), or disable asserts and keep
    // error tooltips (new and nice!)
    // - Non-programmer seats: maybe disable asserts, but make sure errors are
    // resurfaced (tooltips, visible log entries, use callback etc.)
    // - Recovery after error/exception: record stack sizes with
    // ErrorRecoveryStoreState(), disable assert, set log callback (to e.g.
    // trigger high-level breakpoint), recover with
    // ErrorRecoveryTryToRecoverState(), restore settings.
    bool ConfigErrorRecovery; // = true       // Enable error recovery support.
                              // Some errors won't be detected and lead to
                              // direct crashes if recovery is disabled.
    bool ConfigErrorRecoveryEnableAssert; // = true       // Enable asserts on
                                          // recoverable error. By default call
                                          // IM_ASSERT() when returning from a
                                          // failing IM_ASSERT_USER_ERROR()
    bool ConfigErrorRecoveryEnableDebugLog; // = true       // Enable debug log
    ...
    // - The Item Picker tool will be available regardless of this being
    // enabled, in order to maximize its discoverability.
    // - Requires a debugger being attached, otherwise IM_DEBUG_BREAK() options
    // will appear to crash your application.
    //   e.g. io.ConfigDebugIsDebuggerPresent = ::IsDebuggerPresent() on Win32,
    //   or refer to ImOsIsDebuggerPresent()
    //   imgui_test_engine/imgui_te_utils.cpp for a Unix compatible version).
    bool ConfigDebugIsDebuggerPresent; // = false          // Enable various
                                       // tools calling IM_DEBUG_BREAK().

    // Tools to detect code submitting items with conflicting/duplicate IDs
    // - Code should use PushID()/PopID() in loops, or append "##xx" to
    // same-label identifiers.
    ...
    bool ConfigDebugHighlightIdConflicts; // = true           // Highlight and
                                          // show an error message when multiple
    ...
    // for many users.
    // - We expect to update the API eventually. In the meanwhile we provide
    // tools to facilitate checking user-code behavior.
    bool ConfigDebugBeginReturnValueOnce; // = false          // First-time
                                          // calls to Begin()/BeginChild() will
                                          // return false. NEEDS TO BE SET AT
                                          // APPLICATION BOOT TIME if you don't
    ...
    bool ConfigDebugBeginReturnValueLoop; // = false          // Some calls to
                                          ...
                                          // = io.KeyShift" in your main loop
                                          // then occasionally press SHIFT.
                                          // Windows should be flickering while
                                          // running.
    ...
    bool ConfigDebugIniSettings; // = false          // Save .ini data with
                                 // extra comments (particularly helpful for
    ...
    // Nowadays those would be stored in ImGuiPlatformIO but we are leaving them
    // here for legacy reasons. Optional: Platform/Renderer backend name
    // (informational only! will be displayed in About Window) + User data for
    ...
    IMGUI_API void AddKeyEvent(
        ...
        bool down ); // Queue a new key down/up event. Key should be
                     // "translated" (as in, generally ImGuiKey_A matches the
                     // key end-user would use to emit an 'A' character)
    IMGUI_API void AddKeyAnalogEvent(
        ImGuiKey key,
        bool down,
        float v ); // Queue a new key down/up event for analog values (e.g.
                   // ImGuiKey_Gamepad_ values). Dead-zones should be handled by
                   // the backend.
    ...
    IMGUI_API void AddMouseSourceEvent(
        ImGuiMouseSource
            source ); // Queue a mouse source change (Mouse/TouchScreen/Pen)
    IMGUI_API void AddMouseViewportEvent(
        ImGuiID id ); // Queue a mouse hovered viewport. Requires backend to set
                      // ImGuiBackendFlags_HasMouseHoveredViewport to call this
    ...
    IMGUI_API void AddInputCharacter(
        unsigned int c ); // Queue a new character input
    IMGUI_API void AddInputCharacterUTF16(
        ImWchar16 c ); // Queue a new character input from a UTF-16 character,
    ...
    IMGUI_API void SetKeyEventNativeData(
        ...
        int native_legacy_index =
            -1 ); // [Optional] Specify index for legacy <1.87 IsKeyXXX()
                  // functions with native indices + specify native keycode,
                  // scancode.
    IMGUI_API void SetAppAcceptingEvents(
        bool accepting_events ); // Set master flag for accepting key/mouse/text
                                 ...
                                 // native dialog boxes that are interrupting
                                 // your application loop/refresh, and you want
                                 // to disable events being queued while your
                                 // app is frozen.
    IMGUI_API void ClearEventsQueue(); // Clear all incoming events.
    IMGUI_API void
    ClearInputKeys(); // Clear current keyboard/gamepad state + current frame
                      // text input buffer. Equivalent to releasing all
                      // keys/buttons.
    IMGUI_API void ClearInputMouse(); // Clear current mouse state.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    IMGUI_API void
    ClearInputCharacters(); // [Obsoleted in 1.89.8] Clear the current frame
                            // text input buffer. Now included within
                            // ClearInputKeys().
#endif

    //------------------------------------------------------------------
    // Output - Updated by NewFrame() or EndFrame()/Render()
    // (when reading from the io.WantCaptureMouse, io.WantCaptureKeyboard flags
    // to dispatch your inputs, it is
    ...
    bool WantCaptureMouse; // Set when Dear ImGui will use mouse inputs, in this
                           ...
                           // inputs to imgui). (e.g. unclicked mouse is
    ...
    bool WantCaptureKeyboard; // Set when Dear ImGui will use keyboard inputs,
                              // in this case do not dispatch them to your main
    ...
    bool WantTextInput;       // Mobile/console: when set, you may display an
                        // on-screen keyboard. This is set by Dear ImGui when it
                        // wants textual keyboard input to happen (e.g. when a
                        // InputText widget is active).
    bool WantSetMousePos; // MousePos has been altered, backend should
    ...
    bool NavActive;  // Keyboard/Gamepad navigation is currently allowed (will
                     // handle ImGuiKey_NavXXX events) = a window is focused and
                     // it doesn't use the ImGuiWindowFlags_NoNavInputs flag.
    bool NavVisible; // Keyboard/Gamepad navigation highlight is visible and
                     // allowed (will handle ImGuiKey_NavXXX events).
    float Framerate; // Estimate of application framerate (rolling average over
                     // 60 frames, based on io.DeltaTime), in frame per second.
                     // Solely for convenience. Slow applications may not want
                     // to use a moving average or may want to reset underlying
    ...
    //------------------------------------------------------------------
    // [Internal] Dear ImGui will maintain those fields. Forward compatibility
    // not guaranteed!
    //------------------------------------------------------------------
    ...
    // write to those directly, call the AddXXX functions above instead)
    // (reading from those variables is fair game, as they are extremely
    // unlikely to be moving anywhere)
    ImVec2
        MousePos; // Mouse position, in pixels. Set to ImVec2(-FLT_MAX,
                  // -FLT_MAX) if mouse is unavailable (on another screen, etc.)
    bool MouseDown[ 5 ]; // Mouse buttons: 0=left, 1=right, 2=middle + extras
                         // (ImGuiMouseButton_COUNT == 5). Dear ImGui mostly
                         // uses left and right buttons. Other buttons allow us
                         // to track if the mouse is being used by your
                         // application + available to user as a convenience via
                         // IsMouse** API.
    float MouseWheel;    // Mouse wheel Vertical: 1 unit scrolls about 5 lines
                         // text. >0 scrolls Up, <0 scrolls Down. Hold SHIFT to
                         // turn vertical scroll into horizontal scroll.
    ...
    ImGuiID
        MouseHoveredViewport; // (Optional) Modify using
                              // io.AddMouseViewportEvent(). With
                              // multi-viewports: viewport the OS mouse is
                              // hovering. If possible _IGNORING_ viewports with
                              // the ImGuiViewportFlags_NoInputs flag is much
                              // better (few backends can handle that). Set
                              // io.BackendFlags |=
                              // ImGuiBackendFlags_HasMouseHoveredViewport if
    ...
    ImGuiKeyChord
        KeyMods; // Key mods flags (any of
                 ...
                 // merged into flags. Read-only, updated by NewFrame()
    ImGuiKeyData
        KeysData[ ImGuiKey_NamedKey_COUNT ]; // Key state for all known keys.
                                             // Use IsKeyXXX() functions to
                                             // access this.
    bool WantCaptureMouseUnlessPopupClose;   // Alternative to WantCaptureMouse:
                                             ...
                                             // == false) when a click over void
                                             // is expected to close a popup.
    ...
    bool MouseDoubleClicked[ 5 ]; // Has mouse button been double-clicked? (same
                                  // as MouseClickedCount[x] == 2)
    ImU16 MouseClickedCount[ 5 ]; // == 0 (not clicked), == 1 (same as
                                  // MouseClicked[]), == 2 (double-clicked), ==
                                  // 3 (triple-clicked) etc. when going from
                                  // !Down to Down
    ImU16 MouseClickedLastCount[ 5 ]; // Count successive number of clicks.
                                      // Stays valid after mouse release. Reset
                                      // after another click is done.
    bool MouseReleased[ 5 ];          // Mouse button went from Down to !Down
    double MouseReleasedTime[ 5 ];    // Time of last released (rarely used! but
                                   // useful to handle delayed single-click when
                                   // trying to disambiguate them from
                                   // double-click).
    bool
        MouseDownOwned[ 5 ]; // Track if button was clicked inside a dear imgui
                             // window or over void blocked by a popup. We don't
                             // request mouse capture from the application if
                             // click started outside ImGui bounds.
    bool MouseDownOwnedUnlessPopupClose[ 5 ]; // Track if button was clicked
                                              // inside a dear imgui window.
    bool MouseWheelRequestAxisSwap; // On a non-Mac system, holding SHIFT
                                    // requests WheelY to perform the equivalent
                                    // of a WheelX event. On a Mac system this
                                    // is already enforced by the system.
    bool MouseCtrlLeftAsRightClick; // (OSX) Set to true when the current click
                                    // was a ctrl-click that spawned a simulated
                                    // right click
    float MouseDownDuration[ 5 ];   // Duration the mouse button has been down
                                    // (0.0f == just clicked)
    float MouseDownDurationPrev[ 5 ]; // Previous time the mouse button has been
                                      // down
    ...
    // Legacy: before 1.87, we required backend to fill io.KeyMap[]
    // (imgui->native map) during initialization and io.KeysDown[] (native
    // indices) every frame. This is still temporarily supported as a legacy
    // feature. However the new preferred scheme is for backend to call
    // io.AddKeyEvent().
    //   Old (<1.87): ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_Space])
    //   --> New (1.87+) ImGui::IsKeyPressed(ImGuiKey_Space) Old (<1.87):
    //   ImGui::IsKeyPressed(MYPLATFORM_KEY_SPACE)                  --> New
    //   (1.87+) ImGui::IsKeyPressed(ImGuiKey_Space)
    // Read https://github.com/ocornut/imgui/issues/4921 for details.
    // int       KeyMap[ImGuiKey_COUNT];             // [LEGACY] Input: map of
    // indices into the KeysDown[512] entries array which represent your
    // "native" keyboard state. The first 512 are now unused and should be kept
    // zero. Legacy backend will write into KeyMap[] using ImGuiKey_ indices
    // which are always >512. bool      KeysDown[ImGuiKey_COUNT];           //
    // [LEGACY] Input: Keyboard keys that are pressed (ideally left in the
    // "native" order your engine has access to keyboard keys, so you can use
    // your own defines/enums for keys). This used to be [512] sized. It is now
    // ImGuiKey_COUNT to allow legacy io.KeysDown[GetKeyIndex(...)] to work
    // without an overflow. float     NavInputs[ImGuiNavInput_COUNT];     //
    // [LEGACY] Since 1.88, NavInputs[] was removed. Backends from 1.60 to 1.86
    // won't build. Feed gamepad inputs via io.AddKeyEvent() and
    // ImGuiKey_GamepadXXX enums. void*     ImeWindowHandle; // [Obsoleted
    // in 1.87] Set ImGuiViewport::PlatformHandleRaw instead. Set this to your
    // HWND to get automatic IME cursor positioning.

    // Legacy: before 1.91.1, clipboard functions were stored in ImGuiIO instead
    // of ImGuiPlatformIO. As this is will affect all users of custom
    // engines/backends, we are providing proper legacy redirection (will
    // obsolete).
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
// InputText() already returns true on edit + you can always use IsItemEdited().
// The callback is useful to manipulate the underlying buffer while focus is
// active.
...
struct ImGuiInputTextCallbackData {
    ...
    // - However, during Completion/History/Always callback, Buf always points
    // to our own internal data (it is not the same as your buffer)! Changes to
    // it will be reflected into your own buffer shortly after the callback.
    // - To modify the text buffer in a callback, prefer using the InsertChars()
    // / DeleteChars() function. InsertChars() will take care of calling the
    // resize callback if necessary.
    ...
    ImWchar EventChar; // Character input                      // Read-write //
                       // [CharFilter] Replace character with another one, or
                       // set to zero to drop. return 1 is equivalent to setting
                       // EventChar=0;
    ...
    int BufTextLen; // Text length (in bytes)               // Read-write   //
                    ...
                    // zero-terminator storage. In C land: == strlen(some_text),
                    // in C++ land: string.length()
...
struct ImGuiWindowClass {
    ImGuiID ClassId; // User data. 0 = Default class (unclassed). Windows of
                     // different classes cannot be docked with each others.
    ImGuiID
        ParentViewportId; // Hint for the platform backend. -1: use default. 0:
                          // request platform backend to not parent the
                          // platform. != 0: request platform backend to create
                          // a parent<>child relationship between the platform
                          // windows. Not conforming backends are free to e.g.
                          // parent every viewport to the main viewport or not.
    ImGuiID FocusRouteParentWindowId; // ID of parent window for shortcut focus
                                      // route evaluation, e.g. Shortcut() call
                                      // from Parent Window will succeed when
    ...
    ImGuiViewportFlags
        ViewportFlagsOverrideClear; // Viewport flags to clear when a window of
                                    // this class owns a viewport. This allows
                                    // you to enforce OS decoration or task bar
                                    // icon, override the defaults on a
                                    // per-window basis.
    ImGuiTabItemFlags
        TabItemFlagsOverrideSet; // [EXPERIMENTAL] TabItem flags to set when a
                                 // window of this class gets submitted into a
                                 // dock node tab bar. May use with
                                 // ImGuiTabItemFlags_Leading or
                                 // ImGuiTabItemFlags_Trailing.
    ImGuiDockNodeFlags
        DockNodeFlagsOverrideSet; // [EXPERIMENTAL] Dock node flags to set when
                                  // a window of this class is hosted by a dock
                                  // node (it doesn't have to be selected!)
    bool DockingAlwaysTabBar; // Set to true to enforce single floating windows
                              // of this class always having their own docking
                              // node (equivalent of setting the global
...
struct ImGuiStorage {
    ...
    // Advanced: for quicker full rebuild of a storage (instead of an
    // incremental one), you may add all your contents and then sort once.
    IMGUI_API void BuildSortByKey();
    // Obsolete: use on your own storage if you know only integer are being
    // stored (open/close all tree nodes)
    IMGUI_API void SetAllInt( int val );

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // typedef ::ImGuiStoragePair ImGuiStoragePair;  // 1.90.8: moved type
    // outside struct
#endif
};
...
struct ImGuiListClipper {
    ...
    float StartPosY;   // [Internal] Cursor position at the time of Begin() or
                       // after table frozen rows are all processed
    double
        StartSeekOffsetY; // [Internal] Account for frozen rows in a table and
                          // initial loss of precision in very large windows.
    void* TempData;       // [Internal] Internal data

    // items_count: Use INT_MAX if you don't know how many items you have (in
    // which case the cursor won't be advanced in the final step, and you can
    // call SeekCursorForItem() manually if you need) items_height: Use -1.0f to
    ...
    // Step() if you need a range of items to not be clipped, regardless of
    // their visibility. (Due to alignment / padding of certain items it is
    // possible that an extra item may be included on either end of the display
    // range).
    ...
    IMGUI_API void IncludeItemsByIndex(
        ...
        int item_end ); // item_end is exclusive e.g. use (42, 42+1) to make
                        // item 42 never clipped.

    // Seek cursor toward given item. This is automatically called while
    // stepping.
    // - The only reason to call this is: you can use
    // ImGuiListClipper::Begin(INT_MAX) if you don't know item count ahead of
    // time.
    // - In this case, after all steps are done, you'll want to call
    // SeekCursorForItem(item_count).
...
// - Add '#define IMGUI_DEFINE_MATH_OPERATORS' before including this file (or in
// imconfig.h) to access courtesy maths operators for ImVec2 and ImVec4.
// - We intentionally provide ImVec2*float but not float*ImVec2: this is rare
...
// their imconfig file.
// - Any setting other than the default will need custom backend support. The
// only standard backend that supports anything else than the default is
// DirectX9.
...
#define IM_COL32( R, G, B, A )                 \
    ( ( ( ImU32 )( A ) << IM_COL32_A_SHIFT ) | \
      ( ( ImU32 )( B ) << IM_COL32_B_SHIFT ) | \
      ( ( ImU32 )( G ) << IM_COL32_G_SHIFT ) | \
...
struct ImColor {
    ...
    constexpr ImColor( ImU32 rgba )
        : Value( ( float )( ( rgba >> IM_COL32_R_SHIFT ) & 0xFF ) *
                     ( 1.0f / 255.0f ),
                 ( float )( ( rgba >> IM_COL32_G_SHIFT ) & 0xFF ) *
                     ( 1.0f / 255.0f ),
                 ( float )( ( rgba >> IM_COL32_B_SHIFT ) & 0xFF ) *
                     ( 1.0f / 255.0f ),
...
//-----------------------------------------------------------------------------
// [SECTION] Multi-Select API flags and structures (ImGuiMultiSelectFlags,
// ImGuiSelectionRequestType, ImGuiSelectionRequest, ImGuiMultiSelectIO,
// ImGuiSelectionBasicStorage)
//-----------------------------------------------------------------------------

// Multi-selection system
// Documentation at: https://github.com/ocornut/imgui/wiki/Multi-Select
// - Refer to 'Demo->Widgets->Selection State & Multi-Select' for demos using
// this.
// - This system implements standard multi-selection idioms
// (CTRL+Mouse/Keyboard, SHIFT+Mouse/Keyboard, etc)
//   with support for clipper (skipping non-visible items), box-select and many
//   other details.
// - Selectable(), Checkbox() are supported but custom widgets may use it as
// well.
// - TreeNode() is technically supported but... using this correctly is more
// complicated: you need some sort of linear/random access to your tree,
//   which is suited to advanced trees setups also implementing filters and
//   clipper. We will work toward simplifying and demoing it.
// - In the spirit of Dear ImGui design, your code owns actual selection data.
//   This is designed to allow all kinds of selection storage you may use in
//   your application e.g. set/map/hash.
// About ImGuiSelectionBasicStorage:
// - This is an optional helper to store a selection state and apply selection
// requests.
// - It is used by our demos and provided as a convenience to quickly implement
// multi-selection. Usage:
// - Identify submitted items with SetNextItemSelectionUserData(), most likely
// using an index into your current data-set.
// - Store and maintain actual selection data using persistent object
// identifiers.
// - Usage flow:
//     BEGIN - (1) Call BeginMultiSelect() and retrieve the ImGuiMultiSelectIO*
//     result.
//           - (2) Honor request list (SetAll/SetRange requests) by updating
//           your selection data. Same code as Step 6.
//           - (3) [If using clipper] You need to make sure RangeSrcItem is
//           always submitted. Calculate its index and pass to
//           clipper.IncludeItemByIndex(). If storing indices in
//           ImGuiSelectionUserData, a simple
//           clipper.IncludeItemByIndex(ms_io->RangeSrcItem) call will work.
//     LOOP  - (4) Submit your items with SetNextItemSelectionUserData() +
//     Selectable()/TreeNode() calls. END   - (5) Call EndMultiSelect() and
//     retrieve the ImGuiMultiSelectIO* result.
//           - (6) Honor request list (SetAll/SetRange requests) by updating
//           your selection data. Same code as Step 2.
//     If you submit all items (no clipper), Step 2 and 3 are optional and will
//     be handled by each item themselves. It is fine to always honor those
//     steps.
// About ImGuiSelectionUserData:
// - This can store an application-defined identifier (e.g. index or pointer)
// submitted via SetNextItemSelectionUserData().
// - In return we store them into RangeSrcItem/RangeFirstItem/RangeLastItem and
// other fields in ImGuiMultiSelectIO.
// - Most applications will store an object INDEX, hence the chosen name and
// type. Storing an index is natural, because
//   SetRange requests will give you two end-points and you will need to
//   iterate/interpolate between them to update your selection.
// - However it is perfectly possible to store a POINTER or another IDENTIFIER
// inside ImGuiSelectionUserData.
//   Our system never assume that you identify items by indices, it never
//   attempts to interpolate between two values.
// - If you enable ImGuiMultiSelectFlags_NoRangeSelect then it is guaranteed
// that you will never have to interpolate
//   between two ImGuiSelectionUserData, which may be a convenient way to use
//   part of the feature with less code work.
// - As most users will want to store an index, for convenience and to reduce
// confusion we use ImS64 instead of void*,
//   being syntactically easier to downcast. Feel free to reinterpret_cast and
//   store a pointer inside.

// Flags for BeginMultiSelect()
enum ImGuiMultiSelectFlags_ {
    ImGuiMultiSelectFlags_None = 0,
    ImGuiMultiSelectFlags_SingleSelect =
        1 << 0, // Disable selecting more than one item. This is available to
                // allow single-selection code to share same code/logic if
                // desired. It essentially disables the main purpose of
                // BeginMultiSelect() tho!
    ImGuiMultiSelectFlags_NoSelectAll =
        1 << 1, // Disable CTRL+A shortcut to select all.
    ImGuiMultiSelectFlags_NoRangeSelect =
        1 << 2, // Disable Shift+selection mouse/keyboard support (useful for
                // unordered 2D selection). With BoxSelect is also ensure
                // contiguous SetRange requests are not combined into one. This
                // allows not handling interpolation in SetRange requests.
    ImGuiMultiSelectFlags_NoAutoSelect =
        1 << 3, // Disable selecting items when navigating (useful for e.g.
                // supporting range-select in a list of checkboxes).
    ImGuiMultiSelectFlags_NoAutoClear =
        1 << 4, // Disable clearing selection when navigating or selecting
                // another one (generally used with
                // ImGuiMultiSelectFlags_NoAutoSelect. useful for e.g.
                // supporting range-select in a list of checkboxes).
    ImGuiMultiSelectFlags_NoAutoClearOnReselect =
        1 << 5, // Disable clearing selection when clicking/selecting an already
                // selected item.
    ImGuiMultiSelectFlags_BoxSelect1d =
        1 << 6, // Enable box-selection with same width and same x pos items
                // (e.g. full row Selectable()). Box-selection works better with
                // little bit of spacing between items hit-box in order to be
                // able to aim at empty space.
    ImGuiMultiSelectFlags_BoxSelect2d =
        1
        << 7, // Enable box-selection with varying width or varying x pos items
              // support (e.g. different width labels, or 2D layout/grid). This
              // is slower: alters clipping logic so that e.g. horizontal
              // movements will update selection of normally clipped items.
    ...
    ImGuiMultiSelectFlags_ClearOnClickVoid =
        1
        << 10, // Clear selection when clicking on empty location within scope.
    ImGuiMultiSelectFlags_ScopeWindow =
        1 << 11, // Scope for _BoxSelect and _ClearOnClickVoid is whole window
                 // (Default). Use if BeginMultiSelect() covers a whole window
                 // or used a single time in same window.
    ImGuiMultiSelectFlags_ScopeRect =
        1 << 12, // Scope for _BoxSelect and _ClearOnClickVoid is rectangle
    ...
    ImGuiMultiSelectFlags_SelectOnClick =
        1 << 13, // Apply selection on mouse down when clicking on unselected
                 // item. (Default)
    ImGuiMultiSelectFlags_SelectOnClickRelease =
        1 << 14, // Apply selection on mouse release when clicking an unselected
                 // item. Allow dragging an unselected item without altering
                 // selection.
    // ImGuiMultiSelectFlags_RangeSelect2d       = 1 << 15,  // Shift+Selection
    // uses 2d geometry instead of linear sequence, so possible to use
    // Shift+up/down to select vertically in grid. Analogous to what BoxSelect
    // does.
    ImGuiMultiSelectFlags_NavWrapX =
        1 << 16, // [Temporary] Enable navigation wrapping on X axis. Provided
                 // as a convenience because we don't have a design for the
                 // general Nav API for this yet. When the more general feature
                 // be public we may obsolete this flag in favor of new one.
};

// Main IO structure returned by BeginMultiSelect()/EndMultiSelect().
// This mainly contains a list of selection requests.
// - Use 'Demo->Tools->Debug Log->Selection' to see requests as they happen.
// - Some fields are only useful if your list is dynamic and allows deletion
// (getting post-deletion focus/state right is shown in the demo)
// - Below: who reads/writes each fields? 'r'=read, 'w'=write, 'ms'=multi-select
...
struct ImGuiMultiSelectIO {
    ...
    ImGuiSelectionUserData
        RangeSrcItem; //  ms:w  app:r     /                // (If using clipper)
                      //  Begin: Source item (often the first selected item)
                      //  must never be clipped: use
                      //  clipper.IncludeItemByIndex() to ensure it is
                      //  submitted.
    ImGuiSelectionUserData
        NavIdItem; //  ms:w, app:r     /                // (If using deletion)
                   //  Last known SetNextItemSelectionUserData() value for NavId
                   //  (if part of submitted items).
    bool NavIdSelected; //  ms:w, app:r     /        app:r   // (If using
                        //  deletion) Last known selection state for NavId (if
                        //  part of submitted items).
    bool RangeSrcReset; //        app:w     /  ms:r          // (If using
                        //        deletion) Set before EndMultiSelect() to reset
    ...
    int ItemsCount; //  ms:w, app:r     /        app:r   // 'int items_count'
                    ...
                    //  convenience, allowing simpler calls to your
                    //  ApplyRequests handler. Not used internally.
};
...
enum ImGuiSelectionRequestType {
    ...
    ImGuiSelectionRequestType_SetRange, // Request app to select/unselect
                                        // [RangeFirstItem..RangeLastItem] items
                                        // (inclusive) based on value of
                                        // Selected. Only EndMultiSelect()
                                        // request this, app code can read after
                                        // BeginMultiSelect() and it will always
                                        // be false.
};
...
struct ImGuiSelectionRequest {
    ...
    bool
        Selected; //  ms:w, app:r     /  ms:w, app:r   // Parameter for
                  //  SetAll/SetRange requests (true = select, false = unselect)
    ImS8 RangeDirection; //                  /  ms:w  app:r   // Parameter for
                         //                  SetRange request: +1 when
                         //                  RangeFirstItem comes before
                         //                  RangeLastItem, -1 otherwise. Useful
                         //                  if you want to preserve selection
                         //                  order on a backward Shift+Click.
    ImGuiSelectionUserData
        RangeFirstItem; //                  /  ms:w, app:r   // Parameter for
                        //                  SetRange request (this is generally
                        //                  == RangeSrcItem when shift selecting
                        //                  from top to bottom).
    ImGuiSelectionUserData
        RangeLastItem; //                  /  ms:w, app:r   // Parameter for
                       //                  SetRange request (this is generally
                       //                  == RangeSrcItem when shift selecting
                       //                  from bottom to top). Inclusive!
};

// Optional helper to store multi-selection state + apply multi-selection
// requests.
// - Used by our demos and provided as a convenience to easily implement basic
// multi-selection.
// - Iterate selection with 'void* it = NULL; ImGuiID id; while
// (selection.GetNextSelectedItem(&it, &id)) { ... }'
//   Or you can check 'if (Contains(id)) { ... }' for each possible object if
//   their number is not too high to iterate.
// - USING THIS IS NOT MANDATORY. This is only a helper and not a required API.
// To store a multi-selection, in your application you could:
// - Use this helper as a convenience. We use our simple key->value ImGuiStorage
// as a std::set<ImGuiID> replacement.
// - Use your own external storage: e.g. std::set<MyObjectId>,
// std::vector<MyObjectId>, interval trees, intrusively stored selection etc. In
// ImGuiSelectionBasicStorage we:
// - always use indices in the multi-selection API (passed to
// SetNextItemSelectionUserData(), retrieved in ImGuiMultiSelectIO)
// - use the AdapterIndexToStorageId() indirection layer to abstract how
// persistent selection data is derived from an index.
// - use decently optimized logic to allow queries and insertion of very large
// selection sets.
// - do not preserve selection order.
// Many combinations are possible depending on how you prefer to store your
// items and how you prefer to store your selection. Large applications are
// likely to eventually want to get rid of this indirection layer and do their
// own thing. See https://github.com/ocornut/imgui/wiki/Multi-Select for details
// and pseudo-code using this helper.
struct ImGuiSelectionBasicStorage {
    // Members
    int Size; //          // Number of selected items, maintained by this
              //          helper.
    bool PreserveOrder; // = false  // GetNextSelectedItem() will return ordered
                        // selection (currently implemented by two additional
                        // sorts of selection. Could be improved)
    ...
    int _SelectionOrder;   // [Internal] Increasing counter to store selection
                           // order
    ImGuiStorage _Storage; // [Internal] Selection set. Think of this as similar
                           // to e.g. std::set<ImGuiID>. Prefer not accessing
                           // directly: iterate with GetNextSelectedItem().

    // Methods
    IMGUI_API ImGuiSelectionBasicStorage();
    IMGUI_API void ApplyRequests(
        ImGuiMultiSelectIO*
            ms_io ); // Apply selection requests coming from BeginMultiSelect()
                     // and EndMultiSelect() functions. It uses 'items_count'
                     // passed to BeginMultiSelect()
    IMGUI_API bool Contains(
        ImGuiID id ) const; // Query if an item id is in selection.
    ...
    IMGUI_API void SetItemSelected(
        ImGuiID id,
        bool selected ); // Add/remove an item from selection (generally done by
...
// Optional helper to apply multi-selection requests to existing randomly
// accessible storage. Convenient if you want to quickly wire multi-select API
// on e.g. an array of bool or items storing their own selection state.
struct ImGuiSelectionExternalStorage {
    ...
    void ( *AdapterSetItemSelected )(
        ...
        int idx,
        bool selected ); // e.g. AdapterSetItemSelected =
                         // [](ImGuiSelectionExternalStorage* self, int idx,
                         // bool selected) {
                         // ((MyItems**)self->UserData)[idx]->Selected =
                         // selected; }

    // Methods
    IMGUI_API ImGuiSelectionExternalStorage();
    IMGUI_API void ApplyRequests(
        ImGuiMultiSelectIO* ms_io ); // Apply selection requests by using
...
struct ImDrawCmd {
    ...
    ImTextureID TextureId;  // 4-8  // User-provided texture ID. Set by user in
                            ...
                            // Image*() functions. Ignore if never using images
    ...
    unsigned int
        ElemCount; // 4    // Number of indices (multiple of 3) to be rendered
                   // as triangles. Vertices are stored in the callee
                   // ImDrawList's vtx_buffer[] array, indices in idx_buffer[].
    ImDrawCallback UserCallback; // 4-8  // If != NULL, call the function
                                 // instead of rendering the vertices. clip_rect
                                 // and texture_id will be set normally.
    void* UserCallbackData; // 4-8  // Callback user data (when UserCallback !=
                            // NULL). If called AddCallback() with size == 0,
                            // this is a copy of the AddCallback() argument. If
                            // called AddCallback() with size > 0, this is
                            // pointing to a buffer where data is stored.
    int UserCallbackDataSize;   // 4 // Size of callback user data when using
                                // storage, otherwise 0.
    int UserCallbackDataOffset; // 4 // [Internal] Offset of callback user data
                                // when using storage, otherwise -1.

    ImDrawCmd() {
        memset( this, 0, sizeof( *this ) );
    } // Also ensure our padding fields are zeroed

    // Since 1.83: returns ImTextureID associated with this draw call. Warning:
    // DO NOT assume this is always same as 'TextureId' (we will change this
    // function for an upcoming feature)
...
enum ImDrawFlags_ {
    ...
    ImDrawFlags_RoundCornersNone =
        1 << 8, // AddRect(), AddRectFilled(), PathRect(): disable rounding on
                // all corners (when rounding > 0.0f). This is NOT zero, NOT an
                // implicit flag!
...
// You are totally free to apply whatever transformation matrix you want to the
...
struct ImDrawList {
    ...
    float _FringeScale; // [Internal] anti-alias fringe is scaled by this value,
                        // this helps to keep things sharp while zooming at
                        // vertex buffer content
    const char* _OwnerName; // Pointer to owner window's name for debugging

    // If you want to create ImDrawList instances, pass them
    // ImGui::GetDrawListSharedData(). (advanced: you may create and use your
    // own ImDrawListSharedData so you can use ImDrawList without ImGui, but
    ...
    IMGUI_API void PushClipRect(
        ...
        bool intersect_with_current_clip_rect =
            false ); // Render-level scissoring. This is passed down to your
                     // render function but not used for CPU-side coarse
                     // clipping. Prefer using higher-level
                     // ImGui::PushClipRect() to affect logic (hit-testing and
                     // widget culling)
    ...
    //   textures to provide cheaper and higher-quality circles. Use AddNgon()
    //   and AddNgonFilled() functions if you need to guarantee a specific
    //   number of sides.
    IMGUI_API void AddLine( const ImVec2& p1,
                            const ImVec2& p2,
                            ImU32 col,
    ...
    IMGUI_API void AddQuad( const ImVec2& p1,
                            ...
                            ImU32 col,
                            float thickness = 1.0f );
    IMGUI_API void AddQuadFilled( const ImVec2& p1,
                                  const ImVec2& p2,
                                  const ImVec2& p3,
                                  const ImVec2& p4,
                                  ImU32 col );
    IMGUI_API void AddTriangle( const ImVec2& p1,
                                const ImVec2& p2,
                                const ImVec2& p3,
                                ImU32 col,
                                float thickness = 1.0f );
    IMGUI_API void AddTriangleFilled( const ImVec2& p1,
                                      const ImVec2& p2,
                                      const ImVec2& p3,
                                      ImU32 col );
    IMGUI_API void AddCircle( const ImVec2& center,
                              ...
                              int num_segments = 0,
                              float thickness = 1.0f );
    IMGUI_API void AddCircleFilled( const ImVec2& center,
                                    float radius,
                                    ImU32 col,
                                    int num_segments = 0 );
    IMGUI_API void AddNgon( const ImVec2& center,
                            float radius,
                            ImU32 col,
                            int num_segments,
                            float thickness = 1.0f );
    IMGUI_API void AddNgonFilled( const ImVec2& center,
                                  float radius,
                                  ImU32 col,
                                  int num_segments );
    IMGUI_API void AddEllipse( const ImVec2& center,
                               const ImVec2& radius,
                               ImU32 col,
                               float rot = 0.0f,
                               int num_segments = 0,
                               float thickness = 1.0f );
    IMGUI_API void AddEllipseFilled( const ImVec2& center,
                                     const ImVec2& radius,
                                     ImU32 col,
                                     float rot = 0.0f,
                                     int num_segments = 0 );
    IMGUI_API void AddText( const ImVec2& pos,
                            ...
                            const char* text_end = NULL );
    IMGUI_API void AddText( ImFont* font,
                            float font_size,
                            const ImVec2& pos,
                            ImU32 col,
                            const char* text_begin,
    ...
    // complexity. Provided as a convenience fo user but not used by main
    // library.
    IMGUI_API void AddPolyline( const ImVec2* points,
                                ...
                                ImDrawFlags flags,
                                float thickness );
    IMGUI_API void AddConvexPolyFilled( const ImVec2* points,
                                        int num_points,
                                        ImU32 col );
    IMGUI_API void AddConcavePolyFilled( const ImVec2* points,
                                         int num_points,
                                         ImU32 col );
    ...
    // the rectangle.
    // - "uv_min" and "uv_max" represent the normalized texture coordinates to
    // use for those corners. Using (0,0)->(1,1) texture coordinates will
    ...
    IMGUI_API void PathEllipticalArcTo( const ImVec2& center,
                                        const ImVec2& radius,
                                        float rot,
                                        float a_min,
                                        float a_max,
    ...
    // - May be used to alter render state (change sampler, blending, current
    // shader). May be used to emit custom rendering commands (difficult to do
    // correctly, but possible).
    // - Use special ImDrawCallback_ResetRenderState callback to instruct
    // backend to reset its render state to the default.
    // - Your rendering loop must check for 'UserCallback' in ImDrawCmd and call
    // the function instead of rendering triangles. All standard backends are
    // honoring this.
    // - For some backends, the callback may access selected render-states
    // exposed by the backend in a ImGui_ImplXXXX_RenderState structure pointed
    // to by platform_io.Renderer_RenderState.
    // - IMPORTANT: please be mindful of the different level of indirection
    // between using size==0 (copying argument) and using size>0 (copying
    // pointed data into a buffer).
    //   - If userdata_size == 0: we copy/store the 'userdata' argument as-is.
    //   It will be available unmodified in ImDrawCmd::UserCallbackData during
    //   render.
    //   - If userdata_size > 0,  we copy/store 'userdata_size' bytes pointed to
    //   by 'userdata'. We store them in a buffer stored inside the drawlist.
    //   ImDrawCmd::UserCallbackData will point inside that buffer so you have
    //   to retrieve data from there. Your callback may need to use
    //   ImDrawCmd::UserCallbackDataSize if you expect dynamically-sized data.
    //   - Support for userdata_size > 0 was added in v1.91.4, October 2024. So
    //   earlier code always only allowed to copy/store a simple void*.
    ...
    // float radius_y, float rot, float a_min, float a_max, int num_segments =
    // 0) { PathEllipticalArcTo(center, ImVec2(radius_x, radius_y), rot, a_min,
    // a_max, num_segments); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void
    // AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
    // const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) {
    // AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } //
    // OBSOLETED in 1.80 (Jan 2021) inline  void  PathBezierCurveTo(const
    // ImVec2& p2, const ImVec2& p3, const ImVec2& p4, int num_segments = 0) {
    ...
    IMGUI_API void _PathArcToFastEx( const ImVec2& center,
                                     float radius,
                                     int a_min_sample,
                                     int a_max_sample,
                                     int a_step );
    IMGUI_API void _PathArcToN( const ImVec2& center,
                                float radius,
                                float a_min,
                                float a_max,
                                int num_segments );
...
struct ImDrawData {
    ...
    int TotalVtxCount; // For convenience, sum of all ImDrawList's
                       // VtxBuffer.Size
    ImVector< ImDrawList* >
        CmdLists; // Array of ImDrawList* to render. The ImDrawLists are owned
                  // by ImGuiContext and only pointed to from here.
    ImVec2 DisplayPos;  // Top-left position of the viewport to render (==
                        // top-left of the orthogonal projection matrix to use)
                        // (== GetMainViewport()->Pos for the main viewport, ==
    ...
    ImVec2 FramebufferScale; // Amount of pixels for each unit of DisplaySize.
    ...
    IMGUI_API void ScaleClipRects(
        const ImVec2&
            fb_scale ); // Helper to scale the ClipRect field of each ImDrawCmd.
                        // Use if your final output buffer is at a different
                        // scale than Dear ImGui expects, or if there is a
                        // difference between your window resolution and
...
struct ImFontConfig {
    ...
    bool FontDataOwnedByAtlas; // true     // TTF/OTF data ownership taken by
                               // the container ImFontAtlas (will delete memory
                               // itself).
    bool MergeMode;  // false    // Merge into previous ImFont, so you can
                     // combine multiple inputs font into one ImFont (e.g. ASCII
                     // font + icons + Japanese glyphs). You may want to use
                     // GlyphOffset.y when merge font of different heights.
    bool PixelSnapH; // false    // Align every glyph AdvanceX to pixel
                     // boundaries. Useful e.g. if you are merging a non-pixel
                     // aligned font with the default font. If enabled, you can
                     // set OversampleH/V to 1.
    int FontNo;      // 0        // Index of font within TTF/OTF file
    int OversampleH; // 0 (2)    // Rasterize at higher quality for sub-pixel
                     // positioning. 0 == auto == 1 or 2 depending on size. Note
                     // the difference between 2 and 3 is minimal. You can
                     // reduce this to 1 for large glyphs save memory. Read
                     // https://github.com/nothings/stb/blob/master/tests/oversample/README.md
                     // for details.
    int OversampleV;  // 0 (1)    // Rasterize at higher quality for sub-pixel
                      // positioning. 0 == auto == 1. This is not really useful
                      // as we don't use sub-pixel positions on the Y axis.
    ...
    ImVec2
        GlyphExtraSpacing; // 0, 0     // Extra spacing (in pixels) between
                           // glyphs when rendered: essentially add to
    ...
    float RasterizerMultiply; // 1.0f     // Linearly brighten (>1.0f) or darken
                              // (<1.0f) font output. Brightening small fonts
                              // may be a good workaround to make them more
                              // readable. This is a silly thing we may remove
                              // in the future.
...
struct ImFontGlyph {
    unsigned int Colored
        : 1; // Flag to indicate glyph is colored and should generally ignore
             // tinting (make it usable with no shift on little-endian as this
             // is used in loops)
...
struct ImFontAtlasCustomRect {
    ...
    unsigned int GlyphID
        : 31; // Input    // For custom font glyphs only (ID < 0x110000)
    unsigned int GlyphColored : 1; // Input  // For custom font glyphs only:
                                   // glyph is colored, removed tinting.
...
enum ImFontAtlasFlags_ {
    ...
    ImFontAtlasFlags_NoBakedLines =
        1 << 2, // Don't build thick line textures into the atlas (save a little
                // texture memory, allow support for point/nearest filtering).
                // The AntiAliasedLinesUseTex features uses them, otherwise they
...
struct ImFontAtlas {
    ...
    ClearInputData(); // Clear input data (all ImFontConfig structures including
                      // sizes, TTF data, glyph ranges, etc.) = all the data
                      // used to build the texture and fonts.
    IMGUI_API void
    ClearFonts(); // Clear input+output font data (same as ClearInputData() +
    ...
    // User is in charge of copying the pixels into graphics memory (e.g. create
    // a texture with your engine). Then store your texture handle with
    // SetTexID(). The pitch is always = Width * BytesPerPixels (1 or 4)
    ...
    // string are UTF-8 and NOT in your local code page. Read
    ...
    IMGUI_API const ImWchar*
    GetGlyphRangesGreek(); // Default + Greek and Coptic
    IMGUI_API const ImWchar*
    ...
    // this may help some backends decide of preferred texture format.
    // - You can also request your rectangles to be mapped as font glyph (given
    // a font + Unicode point),
    ...
    int TexDesiredWidth; // Texture width desired by user before Build(). Must
                         // be a power-of-two. If have many glyphs your graphics
                         // API have texture size restrictions you may want to
                         // increase texture width to decrease height.
    int TexGlyphPadding; // FIXME: Should be called "TexPackPadding". Padding
                         // between glyphs within texture in pixels. Defaults
                         // to 1. If your rendering method doesn't rely on
                         // bilinear filtering you may set this to 0 (will also
    ...
    bool TexPixelsUseColors; // Tell whether our texture data is known to use
                             // colors (rather than just alpha channel), in
                             // order to help backend select a format.
    unsigned char*
    ...
    unsigned int FontBuilderFlags; // Shared flags (for all fonts) for custom
                                   // font builder. THIS IS BUILD IMPLEMENTATION
                                   // DEPENDENT. Per-font override is also
    ...
    // [Obsolete]
    // typedef ImFontAtlasCustomRect    CustomRect;         // OBSOLETED
    // in 1.72+ typedef ImFontGlyphRangesBuilder GlyphRangesBuilder; //
    // OBSOLETED in 1.67+
};
...
struct ImFont {
    ...
    ImVector< float >
        IndexAdvanceX; // 12-16 // out // Sparse. Glyphs->AdvanceX in a directly
                       // indexable way (cache-friendly for CalcTextSize
                       // functions which only this info, and are often
                       // bottleneck in large UI).
    float FallbackAdvanceX; // 4     // out // = FallbackGlyph->AdvanceX
    float FontSize; // 4     // in  // Height of characters/line, set during
                    // loading (don't change after loading)

    // [Internal] Members: Hot ~28/40 bytes (for RenderText loop)
    ImVector< ImU16 > IndexLookup;  // 12-16 // out // Sparse. Index glyphs by
                                    // Unicode code-point.
    ...
    // Conceptually ConfigData[] is the list of font sources merged to create
    // this font.
    ImFontAtlas* ContainerAtlas; // 4-8   // out // What we has been loaded into
    const ImFontConfig*
        ConfigData; // 4-8   // in  // Pointer within ContainerAtlas->ConfigData
                    // to ConfigDataCount instances
    short ConfigDataCount; // 2     // in  // Number of ImFontConfig involved in
                           // creating this font. Usually 1, or >1 when merging
                           // multiple font sources into one ImFont.
    short EllipsisCharCount; // 1     // out // 1 or 3
    ImWchar EllipsisChar;    // 2-4   // out // Character used for ellipsis
                             // rendering ('...').
    ImWchar FallbackChar;    // 2-4   // out // Character used if a glyph isn't
                             // found (U+FFFD, '?')
    float EllipsisWidth;     // 4     // out // Total ellipsis Width
    float EllipsisCharStep;  // 4     // out // Step between characters when
                             // EllipsisCount > 0
    float Scale; // 4     // in  // Base font scale (1.0f), multiplied by the
                 // per-window font scale which you can adjust with
                 // SetWindowFontScale()
    float Ascent, Descent;   // 4+4   // out // Ascent: distance from top to
                             // bottom of e.g. 'A' [0..FontSize] (unscaled)
    int MetricsTotalSurface; // 4     // out // Total surface in pixels to get
                             // an idea of the font rasterization/texture cost
                             // (not exact, we approximate the cost of padding
                             // between glyphs)
    bool DirtyLookupTables;  // 1     // out //
    ImU8 Used8kPagesMap[ ( IM_UNICODE_CODEPOINT_MAX + 1 ) / 8192 /
                         8 ]; // 1 bytes if ImWchar=ImWchar16, 16 bytes if
                              // ImWchar==ImWchar32. Store 1-bit for each block
                              // of 4K codepoints that has one active glyph.
                              // This is mainly used to facilitate iterations
    ...
    IMGUI_API ImVec2 CalcTextSizeA( float size,
                                    ...
                                    const char* text_begin,
                                    const char* text_end = NULL,
                                    const char** remaining = NULL ); // utf8
    ...
    IMGUI_API void AddRemapChar(
        ...
        bool overwrite_dst =
            true ); // Makes 'dst' character/glyph points to 'src'
                    // character/glyph. Currently needs to be called AFTER fonts
                    // have been built.
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_OwnedByApp =
        1 << 2, // Platform Window: Is created/managed by the user application?
    ...
    ImGuiViewportFlags_NoDecoration =
        1 << 3, // Platform Window: Disable platform decorations: title bar,
                ...
                // ImGuiConfigFlags_ViewportsDecoration is set we only set this
                // on popups/tooltips)
    ImGuiViewportFlags_NoTaskBarIcon =
        1 << 4, // Platform Window: Disable platform task bar icon (generally
    ...
    ImGuiViewportFlags_NoRendererClear =
        1 << 8, // Platform Window: Renderer doesn't need to clear the
                // framebuffer ahead (because we will fill it entirely).
    ImGuiViewportFlags_NoAutoMerge =
        1 << 9, // Platform Window: Avoid merging this window into another host
                // window. This can only be set via ImGuiWindowClass viewport
                // flags override (because we need to now ahead if we are going
                // to create a viewport in the first place!).
    ImGuiViewportFlags_TopMost =
        1 << 10, // Platform Window: Display on top (for tooltips only).
    ImGuiViewportFlags_CanHostOtherWindows =
        1 << 11, // Viewport can host multiple imgui windows (secondary
                 // viewports are associated to a single window). // FIXME: In
                 // practice there's still probably code making the assumption
                 // that this is always and only on the MainViewport. Will fix
                 // once we add support for "no main viewport".
...
struct ImGuiViewport {
    ...
    ImVec2 Pos;  // Main Area: Position of the viewport (Dear ImGui coordinates
                 // are the same as OS desktop/native coordinates)
    ...
    // backend for a custom engine, it is possible that both Renderer and
    // Platform will be handled by the same system and you may not need to use
    // all the UserData/Handle fields. The library never uses those fields, they
    // are merely storage to facilitate backend implementation.
    void* RendererUserData; // void* to hold custom data structure for the
                            // renderer (e.g. swap chain, framebuffers etc.).
                            // generally set by your Renderer_CreateWindow
                            // function.
    void* PlatformUserData; // void* to hold custom data structure for the OS /
                            // platform (e.g. windowing info, render context).
                            // generally set by your Platform_CreateWindow
                            // function.
    void* PlatformHandle; // void* to hold higher-level, platform window handle
                          // (e.g. HWND, GLFWWindow*, SDL_Window*), for
                          // FindViewportByPlatformHandle().
    void* PlatformHandleRaw; // void* to hold lower-level, platform-native
                             // window handle (under Win32 this is expected to
                             // be a HWND, unused for other platforms), when
                             // using an abstraction layer like GLFW or SDL
                             // (where PlatformHandle would be a SDL_Window*)
    bool PlatformWindowCreated; // Platform window has been created
                                // (Platform_CreateWindow() has been called).
                                // This is false during the first frame where a
                                // viewport is being created.
    bool PlatformRequestMove; // Platform window requested move (e.g. window was
                              // moved by the OS / host window manager,
                              // authoritative position will be OS window
                              // position)
    ...
    bool PlatformRequestClose; // Platform window requested closure (e.g. window
                               // was moved by the OS / host window manager,
                               // e.g. pressing ALT-F4)
...
struct ImGuiPlatformIO {
    ...
    void ( *Platform_SetClipboardTextFn )( ImGuiContext* ctx,
                                           const char* text );
    ...
    void ( *Platform_SetImeDataFn )( ImGuiContext* ctx,
                                     ImGuiViewport* viewport,
                                     ImGuiPlatformImeData* data );
    void* Platform_ImeUserData;
    // void      (*SetPlatformImeDataFn)(ImGuiViewport* viewport,
    // ImGuiPlatformImeData* data); // [Renamed to
    // platform_io.PlatformSetImeDataFn in 1.91.1]

    // Optional: Platform locale
    // [Experimental] Configure decimal point e.g. '.' or ',' useful for some
    ...
    // For reference, the second column shows which function are generally
    // calling the Platform Functions:
    //   N = ImGui::NewFrame()                        ~ beginning of the dear
    //   imgui frame: read info from platform/OS windows (latest size/position)
    //   F = ImGui::Begin(), ImGui::EndFrame()        ~ during the dear imgui
    //   frame U = ImGui::UpdatePlatformWindows()           ~ after the dear
    //   imgui frame: create and update all platform/OS windows R =
    ...
    // The handlers are designed so we can mix and match two imgui_impl_xxxx
    // files, one Platform backend and one Renderer backend. Custom engine
    // backends will often provide both Platform and Renderer interfaces
    // together and so may not need to use all functions. Platform functions are
    // typically called _before_ their Renderer counterpart, apart from Destroy
    // which are called the other way.

    // Platform Backend functions (e.g. Win32, GLFW, SDL) -------------------
    // Called by -----
    void ( *Platform_CreateWindow )(
        ImGuiViewport* vp ); // . . U . .  // Create a new platform window for
                             // the given viewport
    void ( *Platform_DestroyWindow )( ImGuiViewport* vp ); // N . U . D  //
    void ( *Platform_ShowWindow )(
        ImGuiViewport* vp ); // . . U . .  // Newly created windows are
                             // initially hidden so SetWindowPos/Size/Title can
                             // be called on them before showing the window
    void ( *Platform_SetWindowPos )(
        ImGuiViewport* vp,
        ImVec2 pos ); // . . U . .  // Set platform window position (given the
                      // upper-left corner of client area)
    ImVec2 ( *Platform_GetWindowPos )( ImGuiViewport* vp ); // N . . . .  //
    void ( *Platform_SetWindowSize )(
        ImGuiViewport* vp,
        ImVec2 size ); // . . U . .  // Set platform window client area size
                       // (ignoring OS decorations such as OS title bar etc.)
    ImVec2 ( *Platform_GetWindowSize )(
        ImGuiViewport*
            vp ); // N . . . .  // Get platform window client area size
    ...
    bool ( *Platform_GetWindowMinimized )(
        ImGuiViewport*
            vp ); // N . . . .  // Get platform window minimized state. When
                  // minimized, we generally won't attempt to get/set size and
                  // contents will be culled more easily
    void ( *Platform_SetWindowTitle )(
        ImGuiViewport* vp,
        const char* str ); // . . U . .  // Set platform window title (given an
                           // UTF-8 string)
    void ( *Platform_SetWindowAlpha )(
        ImGuiViewport* vp,
        float alpha ); // . . U . .  // (Optional) Setup global transparency
    ...
    void ( *Platform_RenderWindow )(
        ImGuiViewport* vp,
        void* render_arg ); // . . . R .  // (Optional) Main rendering (platform
                            // side! This is often unused, or just setting a
                            // "current" context for OpenGL bindings).
                            // 'render_arg' is the value passed to
                            // RenderPlatformWindowsDefault().
    void ( *Platform_SwapBuffers )(
        ImGuiViewport* vp,
        void* render_arg ); // . . . R .  // (Optional) Call Present/SwapBuffers
                            // (platform side! This is often unused!).
                            // 'render_arg' is the value passed to
                            // RenderPlatformWindowsDefault().
    float ( *Platform_GetWindowDpiScale )(
        ImGuiViewport*
            vp ); // N . . . .  // (Optional) [BETA] FIXME-DPI: DPI handling:
                  // Return DPI scale for this viewport. 1.0f = 96 DPI.
    void ( *Platform_OnChangedViewport )(
        ImGuiViewport*
            vp ); // . F . . .  // (Optional) [BETA] FIXME-DPI: DPI handling:
                  // Called during Begin() every time the viewport we are
                  // outputting into changes, so backend has a chance to swap
                  // fonts to adjust style.
    ImVec4 ( *Platform_GetWindowWorkAreaInsets )(
        ImGuiViewport* vp ); // N . . . .  // (Optional) [BETA] Get initial work
                             // area inset for the viewport (won't be covered by
                             // main menu bar, dockspace over viewport etc.).
                             // Default to (0,0),(0,0). 'safeAreaInsets' in iOS
                             // land, 'DisplayCutout' in Android land.
    int ( *Platform_CreateVkSurface )(
        ImGuiViewport* vp,
        ImU64 vk_inst,
        const void* vk_allocators,
        ImU64* out_vk_surface ); // (Optional) For a Vulkan Renderer to call
                                 // into Platform code (since the surface
                                 // creation needs to tie them both).

    // Renderer Backend functions (e.g. DirectX, OpenGL, Vulkan) ------------
    // Called by -----
    void ( *Renderer_CreateWindow )(
        ImGuiViewport* vp ); // . . U . .  // Create swap chain, frame buffers
                             // etc. (called after Platform_CreateWindow)
    ...
    void ( *Renderer_SetWindowSize )(
        ImGuiViewport* vp,
        ImVec2 size ); // . . U . .  // Resize swap chain, frame buffers etc.
                       // (called after Platform_SetWindowSize)
    void ( *Renderer_RenderWindow )(
        ImGuiViewport* vp,
        void* render_arg ); // . . . R .  // (Optional) Clear framebuffer, setup
                            // render target, then render the
                            // viewport->DrawData. 'render_arg' is the value
                            // passed to RenderPlatformWindowsDefault().
    void ( *Renderer_SwapBuffers )(
        ImGuiViewport* vp,
        void* render_arg ); // . . . R .  // (Optional) Call
                            // Present/SwapBuffers. 'render_arg' is the value
                            // passed to RenderPlatformWindowsDefault().

    // (Optional) Monitor list
    // - Updated by: app/backend. Update every frame to dynamically support
    // changing monitor or DPI configuration.
    // - Used by: dear imgui to query DPI info, clamp popups/tooltips within
    // same monitor and not have them straddle monitors.
...
struct ImGuiPlatformMonitor {
    ImVec2 MainPos, MainSize; // Coordinates of the area displayed on this
                              // monitor (Min = upper left, Max = bottom right)
    ImVec2 WorkPos,
        WorkSize; // Coordinates without task bars / side bars / menu bars. Used
                  // to avoid positioning popups/tooltips inside this region. If
                  // you don't have this info, please copy the value for
                  // MainPos/MainSize.
...
// platform_io.Platform_SetImeDataFn() function.
...
IMGUI_API ImVec2
GetWindowContentRegionMin(); // Content boundaries min for the window (roughly
                             // (0,0)-Scroll), in window-local coordinates. You
                             // should never need this. Always use
                             // GetCursorScreenPos() and
                             // GetContentRegionAvail()!
IMGUI_API ImVec2
GetWindowContentRegionMax(); // Content boundaries max for the window (roughly
                             // (0,0)+Size-Scroll), in window-local coordinates.
                             // You should never need this. Always use
                             // GetCursorScreenPos() and
                             // GetContentRegionAvail()!
...
// window_flags); } // Unnecessary as true == ImGuiChildFlags_Borders static
// inline bool BeginChild(ImGuiID id, const ImVec2& size_arg, bool borders,
// ImGuiWindowFlags window_flags)        { return BeginChild(id, size_arg,
// borders ? ImGuiChildFlags_Borders : ImGuiChildFlags_None, window_flags); } //
// Unnecessary as true == ImGuiChildFlags_Borders
...
// IMGUI_API bool      ImageButton(ImTextureID user_texture_id, const ImVec2&
// size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), int
// frame_padding = -1, const ImVec4& bg_col = ImVec4(0, 0, 0, 0), const ImVec4&
// tint_col = ImVec4(1, 1, 1, 1)); // --> Use new ImageButton() signature
// (explicit item id, regular FramePadding). Refer to code in 1.91 if you want
// to grab a copy of this version.
//-- OBSOLETED in 1.88 (from May 2022)
// static inline void  CaptureKeyboardFromApp(bool want_capture_keyboard = true)
// { SetNextFrameWantCaptureKeyboard(want_capture_keyboard); } // Renamed as
// name was misleading + removed default value. static inline void
// CaptureMouseFromApp(bool want_capture_mouse = true)                         {
// SetNextFrameWantCaptureMouse(want_capture_mouse); }       // Renamed as name
// was misleading + removed default value.
...
// ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END); const ImGuiKeyData*
// key_data = GetKeyData(key); return (ImGuiKey)(key_data - g.IO.KeysData); } //
...
// ImGuiKey_XXXX values will return the same value! static inline ImGuiKey
// GetKeyIndex(ImGuiKey key)                                                {
...
// the first place.
// typedef ImGuiKeyChord ImGuiModFlags;      // == int. We generally use
// ImGuiKeyChord to mean "a ImGuiKey or-ed with any number of ImGuiMod_XXX
// value", so you may store mods in there. enum ImGuiModFlags_ {
// ImGuiModFlags_None = 0, ImGuiModFlags_Ctrl = ImGuiMod_Ctrl,
// ImGuiModFlags_Shift = ImGuiMod_Shift, ImGuiModFlags_Alt = ImGuiMod_Alt,
// ImGuiModFlags_Super = ImGuiMod_Super }; typedef ImGuiKeyChord
// ImGuiKeyModFlags; // == int enum ImGuiKeyModFlags_ { ImGuiKeyModFlags_None =
// 0, ImGuiKeyModFlags_Ctrl = ImGuiMod_Ctrl, ImGuiKeyModFlags_Shift =
// ImGuiMod_Shift, ImGuiKeyModFlags_Alt = ImGuiMod_Alt, ImGuiKeyModFlags_Super =
// ImGuiMod_Super };
