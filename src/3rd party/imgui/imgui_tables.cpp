...
bool ImGui::BeginTableEx( const char* name,
    ...
    if ( ++g.TablesTempDataStacked > g.TablesTempData.Size )
        g.TablesTempData.resize( g.TablesTempDataStacked,
                                 ImGuiTableTempData() );
    ...
    if ( use_child_window ) {
        ...
        if ( ( previous_flags &
            ...
            if ( ( g.NextWindowData.Flags &
                   ImGuiNextWindowDataFlags_HasScroll ) == 0 )
    ...
    // Make borders not overlap our contents by offsetting HostClipRect (#6765,
    // #7428, #3752) (we normally shouldn't alter HostClipRect as we rely on
    // TableMergeDrawChannels() expanding non-clipped column toward the limits
    ...
    if ( inner_window != outer_window ) {
        // FIXME: Because inner_window's Scrollbar doesn't know about border
        // size, since it's not encoded in window->WindowBorderSize, it already
        // overlaps it and doesn't need an extra offset. Ideally we should be
        // able to pass custom border size with different x/y values to
        // BeginChild().
        if ( flags & ImGuiTableFlags_BordersOuterV ) {
            ...
            if ( inner_window->DecoOuterSizeX2 == 0.0f )
                table->HostClipRect.Max.x =
                    ImMax( table->HostClipRect.Max.x - TABLE_BORDER_SIZE,
                           table->HostClipRect.Min.x );
        }
        if ( flags & ImGuiTableFlags_BordersOuterH ) {
            ...
            if ( inner_window->DecoOuterSizeY2 == 0.0f )
                table->HostClipRect.Max.y =
                    ImMax( table->HostClipRect.Max.y - TABLE_BORDER_SIZE,
                           table->HostClipRect.Min.y );
        }
    }
...
static void TableSetupColumnFlags( ImGuiTable* table,
    ...
    if ( ( flags & ImGuiTableColumnFlags_WidthMask_ ) == 0 ) {
        ...
        IM_ASSERT( ImIsPowerOfTwo(
            flags &
            ImGuiTableColumnFlags_WidthMask_ ) ); // Check that only 1 of each
                                                  // set is used.
    }
    ...
    column->Flags =
        flags | ( column->Flags & ImGuiTableColumnFlags_StatusMask_ );
...
void ImGui::TableUpdateLayout( ImGuiTable* table ) {
    ...
    const float width_avail =
        ImMax( 1.0f, ( ( ( table->Flags & ImGuiTableFlags_ScrollX ) &&
    ...
    for ( int column_n = 0; column_n < table->ColumnsCount; column_n++ ) {
        ...
        if ( column->Flags & ImGuiTableColumnFlags_WidthStretch ) {
            ...
            column->WidthRequest = IM_TRUNC(
                ImMax( width_avail_for_stretched_columns * weight_ratio,
                       table->MinColumnWidth ) +
                0.01f );
    ...
    for ( int order_n = 0; order_n < table->ColumnsCount; order_n++ ) {
        ...
        // Initial nav layer: using FreezeRowsCount, NOT FreezeRowsRequest, so
        // Header line changes layer when frozen
        column->NavLayerCurrent =
            ( ImS8 )( table->FreezeRowsCount > 0
                          ? ImGuiNavLayer_Menu
                          : ( ImGuiNavLayer )table->NavLayer );
    ...
    table->InnerWindow->WorkRect.Max.y =
        ImClamp( window_content_max_y - g.Style.CellPadding.y,
    ...
    if ( table->Flags & ImGuiTableFlags_NoClip )
        ...
        inner_window->DrawList->PushClipRect(
            inner_window->InnerClipRect.Min, inner_window->InnerClipRect.Max,
...
void ImGui::TableUpdateBorders( ImGuiTable* table ) {
    ...
    const float hit_y2_body = ImMax(
        table->OuterRect.Max.y,
        hit_y1 + table_instance->LastOuterHeight - table->AngledHeadersHeight );
...
void ImGui::EndTable() {
    ...
    IM_ASSERT_USER_ERROR(
        inner_window->IDStack.back() == table_instance->TableInstanceID,
    ...
    if ( inner_window != outer_window ) {
        ...
        inner_window->DC.NavLayersActiveMask |=
            1 << table->NavLayer; // So empty table don't appear to navigate
                                  // differently.
...
void ImGui::TableSetupScrollFreeze( int columns, int rows ) {
    ...
    for ( int column_n = 0; column_n < table->FreezeColumnsRequest;
          column_n++ ) {
...
void ImGui::TableEndRow( ImGuiTable* table ) {
            ...
            for ( ImGuiTableCellData* cell_data = &table->RowCellData[ 0 ];
                ...
                cell_bg_rect.Min.x =
                    ImMax( cell_bg_rect.Min.x,
                           column->ClipRect.Min
                               .x ); // So that first column after frozen one
                                     // gets clipped when scrolling
    ...
    if ( unfreeze_rows_request ) {
        ...
        const float y0 =
            ImMax( table->RowPosY2 + 1, table->InnerClipRect.Min.y );
    ...
    const int channels_for_dummy =
        ( table->ColumnsEnabledCount < table->ColumnsCount ||
          ( memcmp( table->VisibleMaskByIndex, table->EnabledMaskByIndex,
                    ImBitArrayGetStorageSizeInBytes( table->ColumnsCount ) ) !=
            0 ) )
            ? +1
            : 0;
...
void ImGui::TableMergeDrawChannels( ImGuiTable* table ) {
    ...
    if ( merge_group_mask != 0 ) {
        ...
        for ( int merge_group_n = 0;
              merge_group_n < IM_ARRAYSIZE( merge_groups ); merge_group_n++ ) {
            if ( int merge_channels_count =
                ...
                for ( int n = 0; n < ( size_for_masks_bitarrays_one >> 2 );
                      n++ )
                ...
                for ( int n = 0;
                    ...
                    IM_ASSERT( channel->_CmdBuffer.Size == 1 &&
                               merge_clip_rect.Contains( ImRect(
...
static ImU32 TableGetColumnBorderCol( ImGuiTable* table,
                                      int order_n,
                                      int column_n ) {
...
void ImGui::TableDrawBorders( ImGuiTable* table ) {
    ...
    if ( table->Flags & ImGuiTableFlags_BordersInnerV ) {
        for ( int order_n = 0; order_n < table->ColumnsCount; order_n++ ) {
            ...
            if ( column->MaxX <=
                 column->ClipRect.Min
                     .x ) // FIXME-TABLE FIXME-STYLE: Assume BorderSize==1, this
                          // is problematic if we want to increase the border
                          // size..
                continue;
    ...
    if ( table->Flags & ImGuiTableFlags_BordersOuter ) {
        ...
        if ( ( table->Flags & ImGuiTableFlags_BordersOuter ) ==
            ...
            inner_drawlist->AddRect( outer_border.Min, outer_border.Max,
...
static inline ImGuiSortDirection TableGetColumnAvailSortDirection(
    ImGuiTableColumn* column,
    int n ) {
    IM_ASSERT( n < column->SortDirectionsAvailCount );
    return ( ImGuiSortDirection )( ( column->SortDirectionsAvailList >>
                                     ( n << 1 ) ) &
                                   0x03 );
}
...
void ImGui::TableFixColumnSortDirection( ImGuiTable* table,
                                         ImGuiTableColumn* column ) {
    if ( column->SortOrder == -1 || ( column->SortDirectionsAvailMask &
                                      ( 1 << column->SortDirection ) ) != 0 )
        return;
    column->SortDirection =
...
void ImGui::TableSetColumnSortDirection( int column_n,
    ...
    if ( append_to_sort_specs )
        for ( int other_column_n = 0; other_column_n < table->ColumnsCount;
              other_column_n++ )
            sort_order_max = ImMax(
                sort_order_max, table->Columns[ other_column_n ].SortOrder );
...
void ImGui::TableSortSpecsSanitize( ImGuiTable* table ) {
    ...
    if ( need_fix_linearize || need_fix_single_sort_order ) {
        ...
        for ( int sort_n = 0; sort_n < sort_order_count; sort_n++ ) {
            ...
            for ( int column_n = 0; column_n < table->ColumnsCount; column_n++ )
                if ( ( fixed_mask & ( ( ImU64 )1 << ( ImU64 )column_n ) ) ==
                         0 &&
    ...
    for ( int column_n = 0; column_n < table->ColumnsCount; column_n++ )
        if ( IM_BITARRAY_TESTBIT( table->EnabledMaskByIndex, column_n ) )
            if ( table->Columns[ column_n ].Flags &
                ...
                width = ImMax(
                    width, CalcTextSize( TableGetColumnName( table, column_n ),
                                         NULL, true )
                               .x );
...
// intentionally written to not make much use of internal functions, to give you
// better direction if you need to write your own.
...
void ImGui::TableHeadersRow() {
    ...
    // Call layout if not already done. This is automatically done by
    // TableNextRow: we do it here _only_ to make it easier to debug-step in
    // TableUpdateLayout(). Your own version of this function doesn't need this.
    if ( !table->IsLayoutLocked )
        TableUpdateLayout( table );
    ...
    for ( int column_n = 0; column_n < columns_count; column_n++ ) {
        ...
        // Push an id to allow empty/unnamed headers. This is also idiomatic as
        // it ensure there is a consistent ID path to access columns (for e.g.
        // automation)
...
void ImGui::TableHeader( const char* label ) {
    ...
    float label_height = ImMax(
        label_size.y, table->RowMinHeight - table->RowCellPaddingY * 2.0f );
    ...
    if ( ( table->Flags & ImGuiTableFlags_Sortable ) &&
        ...
        if ( column->SortOrder != -1 )
            sort_arrow = true;
        if ( column->SortOrder > 0 ) {
            ImFormatString( sort_order_suf, IM_ARRAYSIZE( sort_order_suf ),
    ...
    if ( ( table->Flags & ImGuiTableFlags_Sortable ) &&
        ...
        if ( column->SortOrder != -1 ) {
            float x =
                ImMax( cell_r.Min.x, cell_r.Max.x - w_arrow - w_sort_text );
            float y = label_pos.y;
            if ( column->SortOrder > 0 ) {
...
void ImGui::TableAngledHeadersRowEx( ImGuiID row_id,
    ...
    const float row_height = ImTrunc( ImFabs(
        ImRotate( ImVec2( max_label_width,
    ...
    const ImVec2 header_angled_vector =
        unit_right *
        ( row_height / -sin_a ); // vector from bottom-left to top-left, and
    ...
    for ( int pass = 0; pass < 2; pass++ )
        for ( int order_n = 0; order_n < data_count; order_n++ ) {
            ...
            if ( pass == 0 ) {
                // Draw shape
                draw_list->AddQuadFilled( bg_shape[ 0 ], bg_shape[ 1 ],
                                          bg_shape[ 2 ], bg_shape[ 3 ],
                                          request->BgColor0 );
    ...
    PopClipRect();
    PopClipRect();
    table->TempData->AngledHeadersExtraWidth = ImMax(
        0.0f, max_x - table->Columns[ table->RightMostEnabledColumn ].MaxX );
}
...
void ImGui::TableOpenContextMenu( int column_n ) {
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    if ( column_n == -1 && table->CurrentColumn !=
                               -1 ) // When called within a column automatically
...
void ImGui::TableDrawDefaultContextMenu(
    ...
    if ( flags_for_section_to_display & ImGuiTableFlags_Resizable ) {
        if ( column != NULL ) {
            const bool can_resize =
                !( column->Flags & ImGuiTableColumnFlags_NoResize ) &&
                column->IsEnabled;
            if ( MenuItem( LocalizeGetMsg( ImGuiLocKey_TableSizeOne ), NULL,
                           false, can_resize ) ) // "###SizeOne"
                TableSetColumnWidthAutoSingle( table, column_n );
        }
        ...
        if ( table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount &&
            ...
            size_all_desc = LocalizeGetMsg(
                ImGuiLocKey_TableSizeAllDefault ); // "###SizeAll" All stretch
                                                   // or mixed
    ...
    if ( flags_for_section_to_display & ImGuiTableFlags_Reorderable ) {
        if ( MenuItem( LocalizeGetMsg( ImGuiLocKey_TableResetOrder ), NULL,
...
void ImGui::TableLoadSettings( ImGuiTable* table ) {
    ...
    if ( table->SettingsOffset == -1 ) {
        ...
        if ( settings->ColumnsCount !=
             table->ColumnsCount ) // Allow settings if columns count changed.
...
void ImGui::DebugNodeTable( ImGuiTable* table ) {
    ...
    if ( table->InstanceCurrent > 0 )
        Text(
            "** %d instances of same table! Some data below will refer to last "
            "instance.",
            table->InstanceCurrent + 1 );
...
void ImGui::SetColumnOffset( int column_index, float offset ) {
    ...
    if ( !( columns->Flags & ImGuiOldColumnFlags_NoForceWithinWindow ) )
        offset = ImMin(
            offset, columns->OffMaxX - g.Style.ColumnsMinSpacing *
