...
//  (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Multi-viewport
...
//  ImGuiPlatformIO interface. 2025-01-06: DirectX10: Expose selected render
//  state in ImGui_ImplDX10_RenderState, which you can access in 'void*
...
static ImGui_ImplDX10_Data* ImGui_ImplDX10_GetBackendData() {
    return ImGui::GetCurrentContext()
               ? ( ImGui_ImplDX10_Data* )ImGui::GetIO().BackendRendererUserData
               : nullptr;
}
...
static void ImGui_ImplDX10_DestroyFontsTexture() {
    ...
    if ( bd->pFontTextureView ) {
        ...
        ImGui::GetIO().Fonts->SetTexID(
            0 ); // We copied bd->pFontTextureView to io.Fonts->TexID so let's
                 // clear that as well.
    }
}
    ...
    // Create the vertex shader
    {
        ...
        D3D10_INPUT_ELEMENT_DESC local_layout[] = {
            { "POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
              ( UINT )offsetof( ImDrawVert, pos ), D3D10_INPUT_PER_VERTEX_DATA,
              0 },
            { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
              ( UINT )offsetof( ImDrawVert, uv ), D3D10_INPUT_PER_VERTEX_DATA,
              0 },
            { "COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
              ( UINT )offsetof( ImDrawVert, col ), D3D10_INPUT_PER_VERTEX_DATA,
              0 },
        };
...
static void ImGui_ImplDX10_DestroyWindow( ImGuiViewport* viewport ) {
    ...
    if ( ImGui_ImplDX10_ViewportData* vd =
             ( ImGui_ImplDX10_ViewportData* )viewport->RendererUserData ) {
        if ( vd->SwapChain )
...
static void ImGui_ImplDX10_SetWindowSize( ImGuiViewport* viewport,
                                          ImVec2 size ) {
    ImGui_ImplDX10_Data* bd = ImGui_ImplDX10_GetBackendData();
    ImGui_ImplDX10_ViewportData* vd =
        ( ImGui_ImplDX10_ViewportData* )viewport->RendererUserData;
    if ( vd->RTView ) {
        vd->RTView->Release();
        vd->RTView = nullptr;
    ...
    if ( vd->SwapChain ) {
        ...
        if ( pBackBuffer == nullptr ) {
            fprintf(
                stderr,
                "ImGui_ImplDX10_SetWindowSize() failed creating buffers.\n" );
            return;
        }
        bd->pd3dDevice->CreateRenderTargetView( pBackBuffer, nullptr,
                                                &vd->RTView );
...
static void ImGui_ImplDX10_RenderViewport( ImGuiViewport* viewport, void* ) {
    ...
    if ( !( viewport->Flags & ImGuiViewportFlags_NoRendererClear ) )
        bd->pd3dDevice->ClearRenderTargetView( vd->RTView,
                                               ( float* )&clear_color );
