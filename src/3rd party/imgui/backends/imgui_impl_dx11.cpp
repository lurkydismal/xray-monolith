...
//  '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'. [X]
//  Renderer: Multi-viewport support (multiple windows). Enable with
...
//  2025-XX-XX: Platform: Added support for multiple windows via the
//  ImGuiPlatformIO interface. 2025-01-06: DirectX11: Expose
//  VertexConstantBuffer in ImGui_ImplDX11_RenderState. Reset projection matrix
//  in ImDrawCallback_ResetRenderState handler. 2024-10-07: DirectX11: Changed
//  default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07:
//  DirectX11: Expose selected render state in ImGui_ImplDX11_RenderState, which
//  you can access in 'void* platform_io.Renderer_RenderState' during draw
//  callbacks. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch
//  to C++11. 2021-06-29: Reorganized backend to pull data from a single
//  structure to facilitate usage with multiple-contexts (all g_XXXX access
//  changed to bd->XXXX). 2021-05-19: DirectX11: Replaced direct access to
//  ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a
//  requirement) 2021-02-18: DirectX11: Change blending equation to preserve
//  alpha in output buffer. 2019-08-01: DirectX11: Fixed code querying the
//  Geometry Shader state (would generally error with Debug layer enabled).
//  2019-07-21: DirectX11: Backup, clear and restore Geometry Shader is any is
//  bound when calling ImGui_ImplDX11_RenderDrawData. Clearing
//  Hull/Domain/Compute shaders without backup/restore. 2019-05-29: DirectX11:
//  Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX11: Added
//  support for special ImDrawCallback_ResetRenderState callback to reset render
//  state. 2018-12-03: Misc: Added #pragma comment statement to automatically
//  link with d3dcompiler.lib when using D3DCompile(). 2018-11-30: Misc: Setting
//  up io.BackendRendererName so it can be displayed in the About Window.
//  2018-08-01: DirectX11: Querying for IDXGIFactory instead of IDXGIFactory1 to
//  increase compatibility. 2018-07-13: DirectX11: Fixed unreleased resources in
//  Init and Shutdown functions. 2018-06-08: Misc: Extracted
//  imgui_impl_dx11.cpp/.h away from the old combined DX11+Win32 example.
//  2018-06-08: DirectX11: Use draw_data->DisplayPos and draw_data->DisplaySize
//  to setup projection matrix and clipping rectangle. 2018-02-16: Misc:
//  Obsoleted the io.RenderDrawListsFn callback and exposed
//  ImGui_ImplDX11_RenderDrawData() in the .h file so you can call it yourself.
...
static void ImGui_ImplDX11_SetupRenderState( ImDrawData* draw_data,
    ...
    if ( device_ctx->Map( bd->pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD,
    ...
    device_ctx->HSSetShader( nullptr, nullptr,
                             0 ); // In theory we should backup and restore this
                                  // as well.. very infrequently used..
...
void ImGui_ImplDX11_RenderDrawData( ImDrawData* draw_data ) {
    ...
    struct BACKUP_DX11_STATE {
        ...
        ID3D11ClassInstance *PSInstances[ 256 ], *VSInstances[ 256 ],
            *GSInstances[ 256 ]; // 256 is max according to PSSetShader
                                 // documentation
    ...
    device->OMGetBlendState( &old.BlendState, old.BlendFactor,
                             &old.SampleMask );
    ...
    device->IAGetIndexBuffer( &old.IndexBuffer, &old.IndexBufferFormat,
                              &old.IndexBufferOffset );
    device->IAGetVertexBuffers( 0, 1, &old.VertexBuffer,
    ...
    for ( int n = 0; n < draw_data->CmdListsCount; n++ ) {
        ...
        for ( int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++ ) {
            ...
            if ( pcmd->UserCallback != nullptr ) {
                ...
                const D3D11_RECT r = { ( LONG )clip_min.x, ( LONG )clip_min.y,
    ...
    device->IASetIndexBuffer( old.IndexBuffer, old.IndexBufferFormat,
                              old.IndexBufferOffset );
    if ( old.IndexBuffer )
        old.IndexBuffer->Release();
    device->IASetVertexBuffers( 0, 1, &old.VertexBuffer,
                                &old.VertexBufferStride,
                                &old.VertexBufferOffset );
    if ( old.VertexBuffer )
...
static void ImGui_ImplDX11_DestroyFontsTexture() {
    ...
    if ( bd->pFontTextureView ) {
        ...
        ImGui::GetIO().Fonts->SetTexID(
            0 ); // We copied data->pFontTextureView to io.Fonts->TexID so let's
                 // clear that as well.
    }
}
...
bool ImGui_ImplDX11_CreateDeviceObjects() {
    ...
    // D3DCOMPILER_DLL_A) If you would like to use this DX11 sample code but
    // remove this dependency you can:
    //  1) compile once, save the compiled shader blobs into a file or source
    //  code and pass them to CreateVertexShader()/CreatePixelShader()
    //  [preferred solution] 2) use code to detect any version of the DLL and
    ...
    // Create the vertex shader
    {
        ...
        if ( FAILED( D3DCompile( vertexShader, strlen( vertexShader ), nullptr,
            ...
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to
                          // get error showing in (const
        ...
        D3D11_INPUT_ELEMENT_DESC local_layout[] = {
            { "POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
              ( UINT )offsetof( ImDrawVert, pos ), D3D11_INPUT_PER_VERTEX_DATA,
              0 },
            { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
              ( UINT )offsetof( ImDrawVert, uv ), D3D11_INPUT_PER_VERTEX_DATA,
              0 },
            { "COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
              ( UINT )offsetof( ImDrawVert, col ), D3D11_INPUT_PER_VERTEX_DATA,
              0 },
        };
        ...
        // Create the constant buffer
        {
            ...
            desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
            desc.MiscFlags = 0;
            bd->pd3dDevice->CreateBuffer( &desc, nullptr,
    ...
    // Create the pixel shader
    {
        ...
        if ( FAILED( D3DCompile( pixelShader, strlen( pixelShader ), nullptr,
    ...
    // Create depth-stencil State
    {
        ...
        bd->pd3dDevice->CreateDepthStencilState( &desc,
...
bool ImGui_ImplDX11_Init( ID3D11Device* device,
    ...
    if ( device->QueryInterface( IID_PPV_ARGS( &pDXGIDevice ) ) == S_OK )
        if ( pDXGIDevice->GetParent( IID_PPV_ARGS( &pDXGIAdapter ) ) == S_OK )
            if ( pDXGIAdapter->GetParent( IID_PPV_ARGS( &pFactory ) ) ==
                 S_OK ) {
...
static void ImGui_ImplDX11_CreateWindow( ImGuiViewport* viewport ) {
    ...
    // PlatformHandleRaw should always be a HWND, whereas PlatformHandle might
    // be a higher-level handle (e.g. GLFWWindow*, SDL_Window*). Some backends
    // will leave PlatformHandleRaw == 0, in which case we assume PlatformHandle
    // will contain the HWND.
...
static void ImGui_ImplDX11_DestroyWindow( ImGuiViewport* viewport ) {
    ...
    // RendererUserData == nullptr since we didn't create the data for it.
    if ( ImGui_ImplDX11_ViewportData* vd =
             ( ImGui_ImplDX11_ViewportData* )viewport->RendererUserData ) {
        if ( vd->SwapChain )
...
static void ImGui_ImplDX11_SetWindowSize( ImGuiViewport* viewport,
                                          ImVec2 size ) {
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
