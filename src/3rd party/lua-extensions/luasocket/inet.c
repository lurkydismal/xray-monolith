...
int inet_meth_getpeername( lua_State* L, p_socket ps, int family ) {
    ...
    err = getnameinfo( ( struct sockaddr* )&peer, peer_len, name,
                       INET6_ADDRSTRLEN, port, sizeof( port ),
...
const char* inet_tryconnect( p_socket ps,
    ...
    /* try resolving */
    err = socket_gaistrerror(
        getaddrinfo( address, serv, connecthints, &resolved ) );
    if ( err != NULL ) {
        if ( resolved )
            freeaddrinfo( resolved );
        return err;
    ...
    for ( iterator = resolved; iterator; iterator = iterator->ai_next ) {
        ...
        if ( current_family != iterator->ai_family || *ps == SOCKET_INVALID ) {
            socket_destroy( ps );
            err =
                inet_trycreate( ps, iterator->ai_family, iterator->ai_socktype,
...
\*-------------------------------------------------------------------------*/
const char* inet_tryaccept( p_socket server,
                            int family,
                            p_socket client,
                            p_timeout tm ) {
    socklen_t len;
...
const char* inet_trybind( p_socket ps,
    ...
    /* try resolving */
    err = socket_gaistrerror(
        getaddrinfo( address, serv, bindhints, &resolved ) );
    if ( err ) {
        if ( resolved )
            freeaddrinfo( resolved );
        return err;
    ...
    for ( iterator = resolved; iterator; iterator = iterator->ai_next ) {
        ...
        err = socket_strerror( socket_bind(
            ps, ( SA* )iterator->ai_addr, ( socklen_t )iterator->ai_addrlen ) );
