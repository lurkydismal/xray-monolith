...
static t_opt optget[] = { { "bindtodevice", opt_get_bindtodevice },
...
static int meth_accept( lua_State* L ) {
    ...
    t_socket sock;
    const char* err =
        inet_tryaccept( &server->sock, server->family, &sock, tm );
...
static int meth_connect( lua_State* L ) {
    ...
    err = inet_tryconnect( &tcp->sock, &tcp->family, address, port, &tcp->tm,
                           &connecthints );
...
\*-------------------------------------------------------------------------*/
static int meth_shutdown( lua_State* L ) {
    /* SHUT_RD,  SHUT_WR,  SHUT_RDWR  have  the value 0, 1, 2, so we can use
     * method index directly */
