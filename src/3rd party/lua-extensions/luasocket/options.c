...
static int opt_ip6_setmembership( lua_State* L,
                                  p_socket ps,
                                  int level,
                                  int name );
...
static int opt_set( lua_State* L,
                    p_socket ps,
                    int level,
                    int name,
                    void* val,
                    int len );
static int opt_get( lua_State* L,
                    p_socket ps,
                    int level,
                    int name,
                    void* val,
...
int opt_set_bindtodevice( lua_State* L, p_socket ps ) {
    ...
    return luaL_error(
        L, "SO_BINDTODEVICE is not supported on this operating system" );
#else
    const char* dev = luaL_checkstring( L, 3 );
    return opt_set( L, ps, SOL_SOCKET, SO_BINDTODEVICE, ( char* )dev,
                    strlen( dev ) + 1 );
#endif
}
...
int opt_set_linger( lua_State* L, p_socket ps ) {
    ...
    if ( !lua_istable( L, 3 ) )
        auxiliar_typeerror( L, 3, lua_typename( L, LUA_TTABLE ) );
    lua_pushstring( L, "on" );
    lua_gettable( L, 3 );
...
int opt_set_ip_multicast_if( lua_State* L, p_socket ps ) {
    ...
    return opt_set( L, ps, IPPROTO_IP, IP_MULTICAST_IF, ( char* )&val,
                    sizeof( val ) );
}
...
int opt_get_ip_multicast_if( lua_State* L, p_socket ps ) {
    ...
    if ( getsockopt( *ps, IPPROTO_IP, IP_MULTICAST_IF, ( char* )&val, &len ) <
         0 ) {
        lua_pushnil( L );
