...
#define _O_TEXT 0
#define _O_BINARY 0
#define lfs_setmode( L, file, m )                                             \
    ( ( void )( ( void )file, m ),                                            \
      luaL_error( L, LUA_QL( "setmode" ) " not supported on this platform" ), \
      -1 )
...
static int _file_lock( lua_State* L,
    ...
    /* lkmode valid values are:
       LK_LOCK    Locks the specified bytes. If the bytes cannot be locked, the
       program immediately tries again after 1 second. If, after 10 attempts,
       the bytes cannot be locked, the constant returns an error. LK_NBLCK Locks
       the specified bytes. If the bytes cannot be locked, the constant returns
       an error. LK_NBRLCK  Same as _LK_NBLCK. LK_RLCK    Same as _LK_LOCK.
       LK_UNLCK   Unlocks the specified bytes, which must have been previously
       locked.
...
static int make_dir( lua_State* L ) {
    ...
    fail = mkdir( path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP |
...
struct _stat_members members[] = { { "mode", push_st_mode },
                                   { "dev", push_st_dev },
                                   { "ino", push_st_ino },
                                   { "nlink", push_st_nlink },
                                   { "uid", push_st_uid },
