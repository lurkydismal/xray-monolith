...
void xr_dsa::generate_params() {
    ...
    // sprintf_s					(random_string, "%s",
    // rnd_seed);
    unsigned char* rnd_seed =
        static_cast< unsigned char* >( ( void* )random_string );
    unsigned int rnd_ssize = xr_strlen( random_string );
    DSA* tmp_dsa_params =
        DSA_generate_parameters( key_bit_length, rnd_seed, rnd_ssize, &counter,
                                 &long_ret, dsa_genparams_cb, NULL );
    ...
    VERIFY( tmp_dsa_params->priv_key->top * sizeof( u32 ) ==
            private_key_length );
    ...
    VERIFY( DSA_sign( 0, debug_digest, sizeof( debug_digest ), sig, &siglen,
                      tmp_dsa_params ) == 1 );
    ...
    VERIFY( DSA_verify( 0, debug_digest, sizeof( debug_digest ), sig, siglen,
                        tmp_dsa_params ) == 1 );

    VERIFY( DSA_verify( 0, debug_bad_digest, sizeof( debug_bad_digest ), sig,
                        siglen, tmp_dsa_params ) == 0 );
