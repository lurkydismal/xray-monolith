...
public:
    ...
    void _init( ISpatial_DB* _space,
                u32 _mask,
                const Fvector& _start,
                const Fvector& _dir,
                float _range ) {
        mask = _mask;
        ray.pos.set( _start );
    ...
    void walk( xr_vector< ISpatialShared >& R,
               ISpatial_NODE* N,
               Fvector& n_C,
               float n_R ) {
...
void ISpatial_DB::q_ray( xr_vector< ISpatialShared >& R,
                         u32 _o,
                         u32 _mask_and,
                         const Fvector& _start,
                         const Fvector& _dir,
    ...
    spatial_ray_walker W( CPU::ID.feature && _CPU_FEATURE_SSE,
                          !!( _o & O_ONLYFIRST ), !!( _o & O_ONLYNEAREST ) );
