...
public:
    ...
    inline_ BOOL Contains( const AABB& aabb ) const {
        // I assume if all 8 box vertices are inside the sphere, so does the
        // whole box. Sounds ok but maybe there's a better way?
...
#ifdef OLD
private:
    Sphere PlanarCircumscribe3( const Point& p0,
                                const Point& p1,
                                const Point& p2 );
    Sphere Circumscribe4( const Point& p0,
                          const Point& p1,
                          const Point& p2,
                          const Point& p3 );
    Sphere MinFix3( int n,
                    Point** perm,
                    Point* fixed0,
                    Point* fixed1,
                    Point* fixed2 );
    ...
    Sphere MinimalSphere4( const Point& p0,
                           const Point& p1,
                           const Point& p2,
                           const Point& p3 );
#endif
