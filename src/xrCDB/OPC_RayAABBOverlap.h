...
//
// The point of intersection is not computed anymore. The distance to impact is
// not needed anymore since we now have two different queries for segments or
// rays.
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Computes a ray-AABB overlap test using the separating axis theorem. Ray
 * is cached within the class.
 *	\param		center	[in] AABB center
 *	\param		extents	[in] AABB extents
 *	\return		true on overlap
 */
...
inline_ BOOL RayCollider::RayAABBOverlap( const Point& center,
    ...
    //	float Dx = mOrigin.x - center.x;	if(_abs(Dx) > extents.x &&
    // Dx*mDir.x>=0.0f)	return FALSE; 	float Dy = mOrigin.y - center.y;
    // if(_abs(Dy) > extents.y && Dy*mDir.y>=0.0f)	return FALSE; 	float Dz
    // = mOrigin.z - center.z;	if(_abs(Dz) > extents.z && Dz*mDir.z>=0.0f)
    // return FALSE;
    ...
    //((SIR(Dx)-1)^SIR(mDir.x))>=0.0f)	return FALSE; 	float Dy = mOrigin.y -
    // center.y;	if(GREATER(Dy, extents.y) &&
    // ((SIR(Dy)-1)^SIR(mDir.y))>=0.0f) return FALSE; 	float Dz = mOrigin.z -
    // center.z;	if(GREATER(Dz, extents.z) &&
    // ((SIR(Dz)-1)^SIR(mDir.z))>=0.0f)	return FALSE;
