...
IC BOOL TestAABB( const Fvector& bMax,
    ...
    if ( rP[ 0 ] < bMin[ 0 ] ) {
        ...
        coord[ 0 ] = bMin[ 0 ];
        MaxT[ 0 ] = ( bMin[ 0 ] - rP[ 0 ] ) /
                    rD[ 0 ]; // Calculate T distances to candidate planes
    } else if ( rP[ 0 ] > bMax[ 0 ] ) {
        Inside = FALSE;
        coord[ 0 ] = bMax[ 0 ];
        MaxT[ 0 ] = ( bMax[ 0 ] - rP[ 0 ] ) /
    ...
    if ( rP[ 1 ] < bMin[ 1 ] ) {
        ...
        coord[ 1 ] = bMin[ 1 ];
        MaxT[ 1 ] = ( bMin[ 1 ] - rP[ 1 ] ) /
                    rD[ 1 ]; // Calculate T distances to candidate planes
    } else if ( rP[ 1 ] > bMax[ 1 ] ) {
        Inside = FALSE;
        coord[ 1 ] = bMax[ 1 ];
        MaxT[ 1 ] = ( bMax[ 1 ] - rP[ 1 ] ) /
    ...
    if ( rP[ 2 ] < bMin[ 2 ] ) {
        ...
        coord[ 2 ] = bMin[ 2 ];
        MaxT[ 2 ] = ( bMin[ 2 ] - rP[ 2 ] ) /
                    rD[ 2 ]; // Calculate T distances to candidate planes
    } else if ( rP[ 2 ] > bMax[ 2 ] ) {
        Inside = FALSE;
        coord[ 2 ] = bMax[ 2 ];
        MaxT[ 2 ] = ( bMax[ 2 ] - rP[ 2 ] ) /
...
void XRCollide::raypick_fast( const box* B,
                              const Fvector& rC,
                              const Fvector& rD ) {
        ...
        if ( P.distance_to_sqr( C ) < rmodel_range_sq ) {
            if ( B->leaf() ) {
                ...
                for ( int i = 0; i < B->num_tris; i++ ) {
                    ...
                    if ( TestRayTri( rmodel_C, rmodel_D, T.verts, rp_inf.u,
                                     rp_inf.v, rp_inf.range,
                                     ray_flags & RAY_CULL ) ) {
                        if ( rp_inf.range > 0 ) {
                            if ( rmodel_L2W ) {
                                rmodel_L2W->transform_tiny( rp_inf.p[ 0 ],
                                                            *T.verts[ 0 ] );
                                rmodel_L2W->transform_tiny( rp_inf.p[ 1 ],
                                                            *T.verts[ 1 ] );
                                rmodel_L2W->transform_tiny( rp_inf.p[ 2 ],
                                                            *T.verts[ 2 ] );
                            } else {
...
void XRCollide::raypick_fast_nearest( const box* B,
                                      const Fvector& rC,
                                      const Fvector& rD ) {
    // 1. XForm ray from parent to local space
    Fvector C, D, P;
...
void XRCollide::RayPick( const Fmatrix* parent,
                         const Model* o1,
                         const Fvector& C,
                         const Fvector& D,
                         float max_range ) {
