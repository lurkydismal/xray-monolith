...
inline_ BOOL RayCollider::RayTriOverlap( const Point& vert0,
    ...
    if ( mCulling ) {
        ...
        //		if(IR(u)&0x80000000 || u>det)
        // return FALSE;
        if ( IR( mStabbedFace.mU ) & 0x80000000 ||
             IR( mStabbedFace.mU ) > IR( det ) )
            return FALSE;
        ...
        if ( IR( mStabbedFace.mV ) & 0x80000000 ||
             mStabbedFace.mU + mStabbedFace.mV > det )
            return FALSE;
        ...
        //		if(IR(u)&0x80000000 || u>1.0f)
        // return FALSE;
        if ( IR( mStabbedFace.mU ) & 0x80000000 ||
             IR( mStabbedFace.mU ) > IEEE_1_0 )
            return FALSE;
        ...
        if ( IR( mStabbedFace.mV ) & 0x80000000 ||
             mStabbedFace.mU + mStabbedFace.mV > 1.0f )
            return FALSE;
