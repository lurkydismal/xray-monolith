 ...
 *	\endcode
 *
 *	3) Setup object callbacks. Geometry & topology are NOT stored in the
 * collision system, in order to save some ram. So, when the system needs them
 * to perform accurate intersection tests, you're requested to provide the
 * triangle-vertices corresponding to a given face index.
 *
 *	Ex:
 *
 *	\code
 *		static void ColCallback(udword triangleindex, VertexPointers&
 * triangle, udword user_data)
 *		{
 *			// Get back Mesh0 or Mesh1 (you also can use 2 different
 * callbacks) Mesh* MyMesh = (Mesh*)user_data;
 *			// Get correct triangle in the app-controlled database
 *			const Triangle* Tri =
 * MyMesh->GetTriangle(triangleindex);
 ...
 *	\endcode
 *
 *	Of course, you should make this callback as fast as possible. And you're
 * also not supposed to modify the geometry *after* the collision trees have
 * been built. The alternative was to store the geometry & topology in the
 * collision system as well (as in RAPID) but we have found this approach to
 * waste a lot of ram in many cases.
 *
 *	Since version 1.2 you can also use plain pointers. It's a tiny bit
 ...
 *
 *	\code
 *		Model0.GetUsedBytes()	= number of bytes used for this
 * collision tree TC.GetNbBVBVTests()		= number of BV-BV overlap tests
 * performed during last query TC.GetNbPrimPrimTests()	= number of
 * Triangle-Triangle overlap tests performed during last query
 *		TC.GetNbBVPrimTests()	= number of Triangle-BV overlap tests
...
OPCODE_Model::OPCODE_Model()
    : mSource( null ), mTree( null ), mNoLeaf( false ), mQuantized( false ) {
...
bool OPCODE_Model::Build( const OPCODECREATE& create ) {
    ...
    // Check topology. If the model contains degenerate faces, collision report
    // can be wrong in some cases. e.g. it happens with the standard MAX teapot.
    // So clean your meshes first... If you don't have a mesh cleaner you can
    // try this: www.codercorner.com/Consolidation.zip
    ...
    if ( NbDegenerate )
        Log( "OPCODE WARNING: found %d degenerate faces in model! Collision "
             "might report wrong results!\n",
             NbDegenerate );
