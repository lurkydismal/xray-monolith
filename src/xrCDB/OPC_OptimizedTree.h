...
class OPCODE_API AABBCollisionNode {
    ...
    // yield better results (assuming UNC-like informed traversal methods). I
    // borrowed this idea from PQP. The usual "size" otherwise, is the largest
    // box extent. In SOLID that extent is computed on-the-fly each time it's
    // needed (the best approach IMHO). In RAPID the rotation matrix is permuted
    // so that Extent[0] is always the greatest, which saves looking for it at
    // runtime. On the other hand, it yields matrices whose determinant is not
    // 1, i.e. you can't encode them anymore as unit quaternions. Not a very
    // good strategy.
};
...
class OPCODE_API AABBQuantizedNode {
    ...
    // NB: for quantized nodes I don't feel like computing a square-magnitude
    // with integers all over the place.......!
};
