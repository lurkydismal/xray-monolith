...
bool OBB::ContainsPoint( const Point& p ) const {
    ...
    float f = mRot2.m[ 0 ][ 0 ] * RelPoint.x + mRot2.m[ 0 ][ 1 ] * RelPoint.y +
              mRot2.m[ 0 ][ 2 ] * RelPoint.z;
    if ( f >= mExtents.x || f <= -mExtents.x )
        return false;

    f = mRot2.m[ 1 ][ 0 ] * RelPoint.x + mRot2.m[ 1 ][ 1 ] * RelPoint.y +
        mRot2.m[ 1 ][ 2 ] * RelPoint.z;
    if ( f >= mExtents.y || f <= -mExtents.y )
        return false;

    f = mRot2.m[ 2 ][ 0 ] * RelPoint.x + mRot2.m[ 2 ][ 1 ] * RelPoint.y +
        mRot2.m[ 2 ][ 2 ] * RelPoint.z;
...
void OBB::Create( const AABB& aabb, const Matrix4x4& mat ) {
    ...
    // Here we have the same as OBB::Rotate(mat) where the obb is (mCenter,
    // mExtents, Identity).
...
bool OBB::ComputePoints( Point* pts ) const {
    ...
    pts[ 0 ] =
        mCenter - Axis0 * mExtents.x + Axis1 * mExtents.y + Axis2 * mExtents.z;
    pts[ 1 ] =
        mCenter + Axis0 * mExtents.x + Axis1 * mExtents.y + Axis2 * mExtents.z;
    pts[ 2 ] =
        mCenter + Axis0 * mExtents.x - Axis1 * mExtents.y + Axis2 * mExtents.z;
