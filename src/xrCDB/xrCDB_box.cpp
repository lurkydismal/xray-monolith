...
public:
    ...
    ICF bool _tri() {
        ...
        float min, max;
        // Find min, max of the triangle in x-direction, and test for overlap in
        // X
        ...
        if ( bClass3 ) {
            ...
            // - fabs precomputed (half less work, possible since extents are
            // always >0)
    ...
    void _stab( const AABBNoLeafNode* node ) {
        // Actual box-box test
        if ( !_box( ( Fvector& )node->mAABB.mCenter,
                    ( Fvector& )node->mAABB.mExtents ) )
            return;
...
void COLLIDER::box_query( const MODEL* m_def,
    ...
    cform_box_collider BC( !!( box_mode & OPT_FULL_TEST ),
...
struct obb_collider {
    ...
    void _stab( const AABBNoLeafNode* node ) {
        ...
        if ( !obb.intersectAABB( ( Fvector& )node->mAABB.mCenter,
                                 ( Fvector& )node->mAABB.mExtents ) )
            return;
