 ...
 *	This is a vanilla AABB tree, without any particular optimization. It
 * contains anonymous references to user-provided primitives, which can
 * theoretically be anything - triangles, boxes, etc. Each primitive is
 * surrounded by an AABB, regardless of the primitive's nature. When the
 * primitive is a triangle, the resulting tree can be converted into an
 * optimized tree. If the primitive is a box, the resulting tree can be used for
 * culling - VFC or occlusion -, assuming you cull on a mesh-by-mesh basis
 * (modern way).
 *
 *	\class		AABBTree
 *	\author		Pierre Terdiman
...
AABBTreeNode::AABBTreeNode()
    : mP( null ), mN( null ), mNbPrimitives( 0 ), mNodePrimitives( null ) {}
...
void AABBTreeNode::destroy( AABBTreeBuilder* _tree ) {
    ...
    mNodePrimitives =
        null; // This was just a shortcut to the global list => no release
    mNbPrimitives = 0;
}
...
udword AABBTreeNode::Split( udword axis, AABBTreeBuilder* builder ) {
    ...
    float SplitValue = builder->GetSplittingValueEx( mNodePrimitives,
                                                     mNbPrimitives, mBV, axis );

    udword NbPos = 0;
    // Loop through all node-related primitives. Their indices range from
    // mNodePrimitives[0] to mNodePrimitives[mNbPrimitives-1]. Those indices map
    // the global list in the tree builder.
    for ( udword i = 0; i < mNbPrimitives; i++ ) {
        // Get index in global list
        udword Index = mNodePrimitives[ i ];

        // Test against the splitting value. The primitive value is tested
        // against the enclosing-box center. [We only need an approximate
        // partition of the enclosing box here.]
 ...
 *	Note a perfectly-balanced tree is not well-suited to collision detection
 * anyway.
 *
 *	\param		builder		[in] the tree builder
 ...
 *	A well-balanced tree should have a log(n) depth. A degenerate tree O(n)
 * depth.
...
udword AABBTree::ComputeDepth() const {
    ...
    struct Local {
        static void _UpdateDepth( const AABBTreeNode* curnode,
                                  udword& depth,
                                  udword& current ) {
            // Checkings
            if ( !curnode )
 ...
 *	A complete tree is made of 2*N-1 nodes, where N is the number of
