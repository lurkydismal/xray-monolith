...
bool PlanesCollider::Collide( PlanesCache& cache,
    ...
    if ( !model->HasLeafNodes() ) {
        if ( model->IsQuantized() )
            return Collide( cache, planes, nb_planes,
                            ( const AABBQuantizedNoLeafTree* )model->GetTree(),
                            worldm );
        else
            return Collide( cache, planes, nb_planes,
                            ( const AABBNoLeafTree* )model->GetTree(), worldm );
    } else {
        if ( model->IsQuantized() )
            return Collide( cache, planes, nb_planes,
...
BOOL PlanesCollider::InitQuery( PlanesCache& cache,
    ...
    if ( worldm ) {
        ...
        //		for(udword i=0;i<nb_planes;i++)	mPlanes[i] = planes[i] *
        //InvWorldM;
    ...
    if ( TemporalCoherenceEnabled() ) {
        ...
        if ( FirstContactEnabled() ) {
            ...
            if ( mTouchedPrimitives->GetNbEntries() ) {
                ...
                // planes (and set contact status if needed)
...
void PlanesCollider::_Collide( const AABBCollisionNode* node,
                               udword clipmask ) {
    // Test the box against the planes. If the box is completely culled, so are
    // its children, hence we exit.
    udword OutClipMask;
    if ( !PlanesAABBOverlap( node->mAABB.mCenter, node->mAABB.mExtents,
                             OutClipMask, clipmask ) )
        return;

    // If the box is completely included, so are its children. We don't need to
    // do extra tests, we can immediately output a list of visible children.
    // Those ones won't need to be clipped.
    if ( !OutClipMask ) {
        // Set contact status
        mFlags |= OPC_CONTACT;
        _Dump( node );
    ...
    // Else the box straddles one or several planes, so we need to recurse down
    // the tree.
