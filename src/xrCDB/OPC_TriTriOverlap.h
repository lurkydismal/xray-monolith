//! if OPC_TRITRI_EPSILON_TEST is true then we do a check (if |dv|<EPSILON then
//! dv=0.0;) else no check is done (which is less robust, but faster)
...
//! Edge to edge test based on Franlin Antonio's gem: "Faster Line Segment
...
#define EDGE_AGAINST_TRI_EDGES( V0, V1, U0, U1, U2 )                     \
    {                                                                    \
        ...
        const float Ax =                                                 \
            ( ( const float* )V1 )[ i0 ] - ( ( const float* )V0 )[ i0 ]; \
        const float Ay =                                                 \
            ( ( const float* )V1 )[ i1 ] - ( ( const float* )V0 )[ i1 ]; \
...
#define POINT_IN_TRI( V0, U0, U1, U2 )                                         \
    {                                                                          \
        ...
        float b =                                                              \
            -( ( ( const float* )U1 )[ i0 ] - ( ( const float* )U0 )[ i0 ] );  \
        float c = -a * ( ( const float* )U0 )[ i0 ] -                          \
                  b * ( ( const float* )U0 )[ i1 ];                            \
        float d0 = a * ( ( const float* )V0 )[ i0 ] +                          \
                   b * ( ( const float* )V0 )[ i1 ] + c;                       \
                                                                               \
        a = ( ( const float* )U2 )[ i1 ] - ( ( const float* )U1 )[ i1 ];       \
        b = -( ( ( const float* )U2 )[ i0 ] - ( ( const float* )U1 )[ i0 ] );  \
        c = -a * ( ( const float* )U1 )[ i0 ] -                                \
            b * ( ( const float* )U1 )[ i1 ];                                  \
        const float d1 = a * ( ( const float* )V0 )[ i0 ] +                    \
                         b * ( ( const float* )V0 )[ i1 ] + c;                 \
                                                                               \
        a = ( ( const float* )U0 )[ i1 ] - ( ( const float* )U2 )[ i1 ];       \
        b = -( ( ( const float* )U0 )[ i0 ] - ( ( const float* )U2 )[ i0 ] );  \
        c = -a * ( ( const float* )U2 )[ i0 ] -                                \
            b * ( ( const float* )U2 )[ i1 ];                                  \
        const float d2 = a * ( ( const float* )V0 )[ i0 ] +                    \
                         b * ( ( const float* )V0 )[ i1 ] + c;                 \
        if ( d0 * d1 > 0.0f ) {                                                \
            if ( d0 * d2 > 0.0f )                                              \
                return TRUE;                                                   \
