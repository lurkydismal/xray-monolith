 ...
 *	This class performs a stabbing query on an AABB tree, i.e. does a
 * ray-mesh collision.
 *
 ...
 *		- A parameter t such as a point P on the line (P0,P1) is P =
 * Origin + t * Direction
 *		- t = 0  -->  P = P0
 *		- t = d  -->  P = P1
 *
 ...
 *
 *		In Opcode, we support segment queries, which yield half-line
 * queries by setting d = +infinity. We don't support line-queries. If you need
 * them, shift the origin along the ray by an appropriate margin.
 *
 *		In short, the lower bound is always 0, and you can setup the
 * higher bound "d" with RayCollider::SetMaxDist().
 *
 *		Query	|segment			|half-line
 *	|line
 *		--------|-------------------|---------------|----------------
 ...
 *	FIRST CONTACT:
 *
 *		- You can setup "first contact" mode or "all contacts" mode with
 * RayCollider::SetFirstContact().
 *		- In "first contact" mode we return as soon as the ray hits one
 * face. If can be useful e.g. for shadow feelers, where you want to know
 * whether the path to the light is _free or not (a boolean answer is enough).
 *		- In "all contacts" mode we return all faces hit by the ray.
 *
 *	TEMPORAL COHERENCE:
 *
 *		- You can enable or disable temporal coherence with
 * RayCollider::SetTemporalCoherence().
 *		- It currently only works in "first contact" mode.
 *		- If temporal coherence is enabled, the previously hit triangle
 * is cached during the first query. Then, next queries start by colliding the
 * ray against the cached triangle. If they still collide, we return
 * immediately.
 *
 *	CLOSEST HIT:
 *
 *		- You can enable or disable "closest hit" with
 * RayCollider::SetClosestHit().
 *		- It currently only works in "all contacts" mode.
 *		- If closest hit is enabled, faces are sorted by distance
 * on-the-fly and the closest one only is reported.
 *
 *	BACKFACE CULLING:
 *
 *		- You can enable or disable backface culling with
 * RayCollider::SetCulling().
 *		- If culling is enabled, ray will not hit back faces (only front
 * faces).
 *
 *
 *
...
#define HANDLE_CONTACT( prim )                                              \
    ...
    if ( mStabbedFaces ) {                                                  \
        if ( !mClosestHit || !mStabbedFaces->GetNbFaces() ) {               \
            ...
            CollisionFace* Current =                                        \
                const_cast< CollisionFace* >( mStabbedFaces->GetFaces() );  \
...
#define STAB_PRIM( prim )                                                     \
    ...
    if ( RayTriOverlap( *VP.Vertex[ 0 ], *VP.Vertex[ 1 ],                     \
                        *VP.Vertex[ 2 ] ) ) {                                 \
        /* Intersection point is valid if: */                                 \
        /* - distance is positive (else it can just be a face behind the orig \
         * point) */                                                          \
        /* - distance is smaller than a given max distance (useful for shadow \
         * feelers) */                                                        \
...
#define STAB_PRIM( prim )                                                     \
    const IndexedTriangle* Tri = &mFaces[ prim ];                             \
                                                                              \
    /* Perform ray-tri overlap test and return */                             \
    if ( RayTriOverlap( mVerts[ Tri->mVRef[ 0 ] ], mVerts[ Tri->mVRef[ 1 ] ], \
                        mVerts[ Tri->mVRef[ 2 ] ] ) ) {                       \
        /* Intersection point is valid if: */                                 \
        /* - distance is positive (else it can just be a face behind the orig \
         * point) */                                                          \
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Generic stabbing query for generic OPCODE models. After the call, access
 * the results:
 *	- with GetContactStatus()
 *	- in the user-provided destination array
 *
 *	\param		world_ray		[in] stabbing ray in world space
 *	\param		model			[in] Opcode model to collide
 * with
 *	\param		world			[in] model's world matrix, or
 * null
 *	\param		cache			[in] a possibly cached face
 * index, or null
...
bool RayCollider::Collide( const Ray& world_ray,
    ...
    if ( !model->HasLeafNodes() ) {
        if ( model->IsQuantized() )
            return Collide( world_ray,
                            ( const AABBQuantizedNoLeafTree* )model->GetTree(),
                            world, cache );
        else
            return Collide( world_ray,
                            ( const AABBNoLeafTree* )model->GetTree(), world,
                            cache );
    } else {
        if ( model->IsQuantized() )
            return Collide( world_ray,
                            ( const AABBQuantizedTree* )model->GetTree(), world,
                            cache );
        else
            return Collide( world_ray,
                            ( const AABBCollisionTree* )model->GetTree(), world,
                            cache );
    }
}
 ...
 *
 *	\param		world_ray		[in] stabbing ray in world space
 *	\param		world			[in] object's world matrix, or
 * null
 *	\param		faceid		[in] index of previously stabbed
 * triangle
 *	\return		contact status
 *	\warning	SCALE NOT SUPPORTED. The matrix must contain rotation &
 * translation parts only.
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL RayCollider::InitQuery( const Ray& world_ray,
    ...
    if ( TemporalCoherenceEnabled() && FirstContactEnabled() && faceid &&
         *faceid != INVALID_ID ) {
        // Request vertices from the app
        VertexPointers VP;
#ifdef OPC_USE_CALLBACKS
        ...
        if ( RayTriOverlap( *VP.Vertex[ 0 ], *VP.Vertex[ 1 ],
                            *VP.Vertex[ 2 ] ) ) {
            // Intersection point is valid if:
            // - distance is positive (else it can just be a face behind the
            // orig point)
            // - distance is smaller than a given max distance (useful for
            // shadow feelers)
            if ( mStabbedFace.mDistance > 0.0f &&
                 mStabbedFace.mDistance < mMaxDist ) {
                // Set contact status
                mFlags |= OPC_CONTACT;
...
bool RayCollider::Collide( const Ray& world_ray,
    ...
    // This is typically called for a scene tree, full of -AABBs-, not full of
    // triangles. So we don't really have "primitives" to deal with. Hence it
    // doesn't work with "FirstContact" + "TemporalCoherence".
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void RayCollider::_UnboundedStab( const AABBTreeNode* node,
                                  Container& box_indices ) {
