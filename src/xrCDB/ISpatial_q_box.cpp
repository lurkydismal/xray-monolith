...
public:
    spatial_box_walker( ISpatial_DB* _space,
                        u32 _mask,
    ...
    void walk( xr_vector< ISpatialShared >& R,
        ...
        for ( ISpatialShared S : N->items ) {
            ...
            Fbox sB;
            sB.set( sC.x - sR, sC.y - sR, sC.z - sR, sC.x + sR, sC.y + sR,
                    sC.z + sR );
            if ( !sB.intersect( box ) )
                continue;
...
void ISpatial_DB::q_box( xr_vector< ISpatialShared >& R,
                         u32 _o,
                         u32 _mask,
                         const Fvector& _center,
                         const Fvector& _size ) {
...
void ISpatial_DB::q_sphere( xr_vector< ISpatialShared >& R,
                            u32 _o,
                            u32 _mask,
                            const Fvector& _center,
                            const float _radius ) {
