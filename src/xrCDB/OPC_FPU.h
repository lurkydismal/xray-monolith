...
//! Computes 1.0f / sqrtf(x). Comes from Quake3. Looks like the first one I had
//! above. See http://www.magic-software.com/3DGEDInvSqrt.html
        ...
        inline_ void SetFPU()
        {
                ...
                // location pointed to by pwOldCW.
                {
                        ...
                        if (wSave & 0x300 ||            // Not single mode
                                0x3f != (wSave & 0x3f) ||   // Exceptions
   enabled wSave & 0xC00)              // Not round to nearest mode
                        {
                                __asm
                                {
                                        mov ax, wSave
                                        and ax, not 300h    ;; single mode
                                        or  ax, 3fh         ;; disable all
   exceptions and ax, not 0xC00   ;; round to nearest mode mov wTemp, ax fldcw
   wTemp
                                }
                        }
                }
...
*/
//! This function computes the slowest possible floating-point value (you can
//! also directly use FLT_EPSILON)
