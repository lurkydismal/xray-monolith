 ...
 *	\param		box			[in] collision AABB in world
 * space
    ...
    if ( !model->HasLeafNodes() ) {
        if ( model->IsQuantized() )
            ...
            return Collide( cache, box,
                            ( const AABBNoLeafTree* )model->GetTree() );
    } else {
        if ( model->IsQuantized() )
            return Collide( cache, box,
...
BOOL AABBCollider::InitQuery( AABBCache& cache, const CollisionAABB& box ) {
    ...
    if ( TemporalCoherenceEnabled() ) {
        ...
        if ( FirstContactEnabled() ) {
            ...
            // We're interested in all contacts =>test the _new_ real box N(ew)
            // against the previous fat box P(revious):
            if ( mBox.IsInside( cache.FatBox ) ) {
                ...
                // Make a fat box so that coherence will work for subsequent
                // frames
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Checks the AABB completely contains the box. In which case we can end
 * the query sooner.
