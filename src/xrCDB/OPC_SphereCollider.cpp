 ...
 *	This class performs a collision test between a sphere and an AABB tree.
 * You can use this to do a standard player vs world collision, in a
 * Nettle/Telemachos way. It doesn't suffer from all reported bugs in those two
 * classic codes - the "_new_" one by Paul Nettle is a debuggued version I
 * think. Collision response can be driven by reported collision data - it works
 * extremely well for me. In sake of efficiency, all meshes (that is, all AABB
 * trees) should of course also be kept in an extra hierarchical structure
 * (octree, whatever).
 *
...
#define SPHERE_PRIM( prim, flag )                            \
    ...
    if ( SphereTriOverlap( *VP.Vertex[ 0 ], *VP.Vertex[ 1 ], \
                           *VP.Vertex[ 2 ] ) ) {             \
        /* Set contact status */                             \
        mFlags |= flag;                                      \
...
#define SPHERE_PRIM( prim, flag )                          \
    ...
    const IndexedTriangle* Tri = &mFaces[ prim ];          \
                                                           \
    /* Perform sphere-tri overlap test */                  \
    if ( SphereTriOverlap( mVerts[ Tri->mVRef[ 0 ] ],      \
                           mVerts[ Tri->mVRef[ 1 ] ],      \
                           mVerts[ Tri->mVRef[ 2 ] ] ) ) { \
        /* Set contact status */                           \
        mFlags |= flag;                                    \
...
bool SphereCollider::Collide( SphereCache& cache,
    ...
    if ( !model->HasLeafNodes() ) {
        if ( model->IsQuantized() )
            return Collide( cache, sphere,
                            ( const AABBQuantizedNoLeafTree* )model->GetTree(),
                            worlds, worldm );
        else
            return Collide( cache, sphere,
                            ( const AABBNoLeafTree* )model->GetTree(), worlds,
                            worldm );
    } else {
        if ( model->IsQuantized() )
            return Collide( cache, sphere,
    ...
    if ( TemporalCoherenceEnabled() ) {
        ...
        if ( FirstContactEnabled() ) {
            ...
            if ( mTouchedPrimitives->GetNbEntries() ) {
                ...
                // Perform overlap test between the cached triangle and the
                // sphere (and set contact status if needed)
                SPHERE_PRIM( PreviouslyTouchedFace, OPC_TEMPORAL_CONTACT )
            }
            // else no face has been touched during previous query
            // => we'll have to perform a normal query
        } else {
            // We're interested in all contacts =>test the _new_ real sphere
            // N(ew) against the previous fat sphere P(revious):
            ...
            if ( cache.Center.SquareDistance( mCenter ) < r * r ) {
                ...
                // Make a fat sphere so that coherence will work for subsequent
                // frames
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Checks the sphere completely contains the box. In which case we can end
 * the query sooner.
...
inline_ BOOL SphereCollider::SphereContainsBox( const Point& bc,
    ...
    // box. Sounds ok but maybe there's a better way?
    Point p;
    p.x = bc.x + be.x;
    p.y = bc.y + be.y;
    p.z = bc.z + be.z;
...
void SphereCollider::_Collide( const AABBTreeNode* node ) {
    ...
    if ( node->IsLeaf() ) {
        mTouchedPrimitives->Add( node->GetPrimitives(),
                                 node->GetNbPrimitives() );
    } else {
        if ( SphereContainsBox( Center, Extents ) ) {
            mTouchedPrimitives->Add( node->GetPrimitives(),
                                     node->GetNbPrimitives() );
