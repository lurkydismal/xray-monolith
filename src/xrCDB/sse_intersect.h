...
ICF BOOL isect_fpu( const Fvector& min,
    ...
    if ( ray.pos[ 0 ] < min[ 0 ] ) {
        ...
        if ( uf( ray.inv_dir[ 0 ] ) )
            MaxT[ 0 ] =
                ( min[ 0 ] - ray.pos[ 0 ] ) *
                ray.inv_dir[ 0 ]; // Calculate T distances to candidate planes
    ...
    if ( ray.pos[ 1 ] < min[ 1 ] ) {
        ...
        if ( uf( ray.inv_dir[ 1 ] ) )
            MaxT[ 1 ] =
                ( min[ 1 ] - ray.pos[ 1 ] ) *
                ray.inv_dir[ 1 ]; // Calculate T distances to candidate planes
    ...
    if ( ray.pos[ 2 ] < min[ 2 ] ) {
        ...
        if ( uf( ray.inv_dir[ 2 ] ) )
            MaxT[ 2 ] =
                ( min[ 2 ] - ray.pos[ 2 ] ) *
                ray.inv_dir[ 2 ]; // Calculate T distances to candidate planes
...
#define rotatelps( ps ) \
    _mm_shuffle_ps( ( ps ), ( ps ), 0x39 ) // a,b,c,d -> b,c,d,a
...
static const float _MM_ALIGN16
    ps_cst_plus_inf[ 4 ] = { flt_plus_inf, flt_plus_inf, flt_plus_inf,
                             flt_plus_inf },
                       ps_cst_minus_inf[ 4 ] = { -flt_plus_inf, -flt_plus_inf,
                                                 -flt_plus_inf, -flt_plus_inf };
...
ICF BOOL isect_sse( const aabb_t& box, const ray_t& ray, float& dist ) {
    ...
    const __m128 box_min = loadps( &box.min ), box_max = loadps( &box.max ),
                 pos = loadps( &ray.pos ), inv_dir = loadps( &ray.inv_dir );
...
ICF BOOL _box_fpu( ray_t& ray,
    ...
    float n_vR = 2 * n_R;
    Fbox BB;
    BB.set( n_C.x - n_vR, n_C.y - n_vR, n_C.z - n_vR, n_C.x + n_vR,
...
ICF BOOL _box_sse( ray_t& ray,
    ...
    */
    ...
    __m128 NC = _mm_unpacklo_ps( _mm_load_ss( ( float* )&n_C.x ),
                                 _mm_load_ss( ( float* )&n_C.y ) );
    NR = _mm_add_ss( NR, NR );
