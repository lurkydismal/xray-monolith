...
xr_vector< StaticCformPackage* >& StaticCformCache() {
    // Keep the cache alive until process termination. xrCore's allocators can
    // be destroyed before function-local static containers during engine
    // shutdown.
    static xr_vector< StaticCformPackage* >* cache =
        xr_new< xr_vector< StaticCformPackage* > >();
    return *cache;
}
...
void EvictStaticCformCacheUnderPressure() {
    ...
    while ( !cache.empty() && GlobalMemoryStatusEx( &memory ) ) {
        ...
        Msg( "* [LEVEL CACHE] CFORM evicted under memory pressure: %s",
             package->key.c_str() );
...
//----------------------------------------------------------------------
int CObjectSpace::GetNearest( xr_vector< ISpatialShared >& q_spatial,
    ...
    for ( ; it != end; it++ ) {
        ...
        Fsphere mS = { O->SpatialComponent->spatial.sphere.P,
                       O->SpatialComponent->spatial.sphere.R };
        if ( Q.intersect( mS ) )
...
int CObjectSpace::GetNearest( xr_vector< CObject* >& q_nearest,
                              ...
                              float range ) {
    CObject* O = obj->Owner();
    return GetNearest( q_nearest, O->SpatialComponent->spatial.sphere.P,
                       range + O->SpatialComponent->spatial.sphere.R, O );
}
...
void CObjectSpace::PrepareStatic( LPCSTR level_path ) {
    ...
    CDB::TRI* triangles =
        reinterpret_cast< CDB::TRI* >( vertices + header.vertcount );
    StaticCformPackage* package = xr_new< StaticCformPackage >();
    package->key = key;
    package->identity = identity;
    package->bounds = header.aabb;
    package->model.build( vertices, header.vertcount, triangles,
                          header.facecount );
    FS.r_close( reader );
    cache.push_back( package );
    EvictStaticCformCacheUnderPressure();
...
void CObjectSpace::Load( LPCSTR path,
                         LPCSTR fname,
    ...
    const u64 identity = CformIdentity( *file );
    {
        ...
        for ( auto it = cache.begin(); it != cache.end(); ++it ) {
            ...
            if ( !package->materials_remapped && build_callback ) {
                build_callback( Static.get_verts(), Static.get_verts_count(),
                                Static.get_tris(), Static.get_tris_count(),
                                nullptr );
...
void CObjectSpace::Create( Fvector* verts,
                           CDB::TRI* tris,
                           const hdrCFORM& H,
                           CDB::build_callback build_callback,
                           bool init_bounds ) {
