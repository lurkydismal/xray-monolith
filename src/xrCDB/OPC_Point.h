...
enum PointComponent {
    ...
    _W = 3, ///< W component.
    _FORCE_DWORD =
        0x7fffffff, ///< Forces the enumeration to be stored as a 32-bit value.
};
...
public:
    ...
    /// Constructs a point with all components initialized to the specified
    /// value.
    ///
    ...
    /// @return Reference to this point.
    inline_ Point& Mac2( const Point& a,
                         const Point& b,
                         float scalarb,
                         const Point& c,
                         float scalarc ) {
    ...
    /// @param scalarc Scalar multiplier applied to @p c.
    /// @return Reference to this point.
    inline_ Point& Msc2( const Point& a,
                         const Point& b,
                         float scalarb,
                         const Point& c,
                         float scalarc ) {
    ...
    /// @return Reference to this point.
    inline_ Point& Mult2( const Matrix3x3& mat1,
                          const Point& a1,
    ...
    /// @param t Interpolation factor, typically in the range [0, 1].
    /// @return Reference to this point.
    inline_ Point& Herp( const Point& p0,
                         const Point& p1,
                         const Point& p2,
                         const Point& p3,
                         float t ) {
    ...
    /// @return Reference to this point.
    inline_ Point& Transform( const Point& r,
    ...
    /// @param linpos Translation vector.
    /// @return Reference to this point.
    inline_ Point& InvTransform( const Point& r,
                                 const Matrix3x3& rotpos,
                                 const Point& linpos );
    ...
    /// Replaces each component with the smaller of the two corresponding
    /// values.
    ///
    ...
    ///
    /// @return True if the squared magnitude is smaller than the zero
...
/// Bit mask applied when increasing or decreasing floating-point values during
/// tweaking.
...
#define TWEAKNOTMASK ~TWEAKMASK
    ...
    /// Expands the point slightly by increasing the magnitude of non-negative
    /// components.
    ...
    /// Shrinks the point slightly by decreasing the magnitude of negative
    /// components.
    ...
    inline_ float Distance( const Point& b ) const {
        return _sqrt( ( x - b.x ) * ( x - b.x ) + ( y - b.y ) * ( y - b.y ) +
                      ( z - b.z ) * ( z - b.z ) );
    }
    ...
    inline_ udword VectorCode() const {
        return ( IR( x ) >> 31 ) | ( ( IR( y ) & SIGN_BITMASK ) >> 30 ) |
               ( ( IR( z ) & SIGN_BITMASK ) >> 29 );
    }
    ...
    void ProjectToScreen( float halfrenderwidth,
    ...
    inline_ bool operator==( const Point& p ) const {
        return ( ( IR( x ) == IR( p.x ) ) && ( IR( y ) == IR( p.y ) ) &&
                 ( IR( z ) == IR( p.z ) ) );
    }
    ...
    inline_ Point& operator*=( const Matrix4x4& mat ) {
        ...
        float xp = x * Mat->m[ 0 ][ 0 ] + y * Mat->m[ 1 ][ 0 ] +
                   z * Mat->m[ 2 ][ 0 ] + Mat->m[ 3 ][ 0 ];
        float yp = x * Mat->m[ 0 ][ 1 ] + y * Mat->m[ 1 ][ 1 ] +
                   z * Mat->m[ 2 ][ 1 ] + Mat->m[ 3 ][ 1 ];
        float zp = x * Mat->m[ 0 ][ 2 ] + y * Mat->m[ 1 ][ 2 ] +
                   z * Mat->m[ 2 ][ 2 ] + Mat->m[ 3 ][ 2 ];
