...
#define USE_MINMAX // if defined, use (Min, Max) AABBs, else use (Center,
                   // Extents) ones.

//! Declarations of type-independent methods (implemented in the .cpp)
#define AABB_COMMON_METHODS                                              \
    AABB& Add( const AABB& aabb );                                       \
    float CalculateBoxArea( const icePoint& eye, const Matrix4x4& mat,   \
                            float width, float height, int& num ) const; \
...
public:
    ...
    inline_ bool Intersect( const AABB& a ) const {
        if ( mMax.x < a.mMin.x || a.mMax.x < mMin.x || mMax.y < a.mMin.y ||
             a.mMax.y < mMin.y || mMax.z < a.mMin.z || a.mMax.z < mMin.z )
            return false;
    ...
    inline_ bool Intersect( const AABB& a, udword axis ) const {
        if ( ( ( const float* )mMax )[ axis ] <
                 ( ( const float* )a.mMin )[ axis ] ||
             ( ( const float* )a.mMax )[ axis ] <
                 ( ( const float* )mMin )[ axis ] )
            return false;
        return true;
    }
     ...
     *	Original code by Charles Bloom on the GD-Algorithm list. (I slightly
     * modified it)
     *	\param		mtx			[in] the transform matrix
     *	\param		aabb		[out] the transformed AABB [can be
     * *this]
     */
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    inline_ void Rotate( const Matrix4x4& mtx, AABB& aabb ) const {
        // The three edges transformed: you can efficiently transform an X-only
        // vector3 by just getting the "X" column of the matrix
        Point vx, vy, vz;
        mtx.GetRow( 0, vx );
        vx *= ( mMax.x - mMin.x );
        mtx.GetRow( 1, vy );
...
public:
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /**
     *	The standard intersection method from Gamasutra. Just here to check its
     * speed against the one above.
     *	\param		a		[in] the other AABB
     *	\return		true on intersection
     */
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    inline_ void Rotate( const Matrix4x4& mtx, AABB& aabb ) const {
        ...
        // Compute _new_ extents. FPU code & CPU code have been interleaved for
        // improved performance.
        Point Ex( mtx.m[ 0 ][ 0 ] * mExtents.x, mtx.m[ 0 ][ 1 ] * mExtents.x,
                  mtx.m[ 0 ][ 2 ] * mExtents.x );
        IR( Ex.x ) &= 0x7fffffff;
        IR( Ex.y ) &= 0x7fffffff;
        IR( Ex.z ) &= 0x7fffffff;

        Point Ey( mtx.m[ 1 ][ 0 ] * mExtents.y, mtx.m[ 1 ][ 1 ] * mExtents.y,
                  mtx.m[ 1 ][ 2 ] * mExtents.y );
        IR( Ey.x ) &= 0x7fffffff;
        IR( Ey.y ) &= 0x7fffffff;
        IR( Ey.z ) &= 0x7fffffff;

        Point Ez( mtx.m[ 2 ][ 0 ] * mExtents.z, mtx.m[ 2 ][ 1 ] * mExtents.z,
                  mtx.m[ 2 ][ 2 ] * mExtents.z );
        IR( Ez.x ) &= 0x7fffffff;
        IR( Ez.y ) &= 0x7fffffff;
        IR( Ez.z ) &= 0x7fffffff;
