...
public:
    ...
    void Stab( const AABBNoLeafNode* node, u32 mask ) {
        ...
        EFC_Visible result = Box( ( Fvector& )node->mAABB.mCenter,
                                  ( Fvector& )node->mAABB.mExtents, mask );
        if ( fcvNone == result )
            return;

        // 1st chield
        if ( node->HasLeaf() )
            Prim( node->GetPrimitive() );
        else
...
void COLLIDER::frustum_query( const MODEL* m_def, const CFrustum& F ) {
    ...
    // Get nodes
    const AABBNoLeafNode* pNodes =
        ( ( AABBNoLeafTree* )m_def->tree->GetTree() )->GetNodes();
    const DWORD mask = F.getMask();
    r_clear();

    cform_frustum_collider BC( !!( frustum_mode & OPT_FULL_TEST ),
