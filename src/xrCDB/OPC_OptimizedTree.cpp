...
//! Compilation flag:
//! - true to fix quantized boxes (i.e. make sure they enclose the original
//! ones)
//! - false to see the effects of quantization errors (faster, but wrong results
//! in some cases)
static bool gFixQuantized = true;

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Builds an implicit tree from a standard one. An implicit tree is a
 * complete tree (2*N-1 nodes) whose negative box pointers and primitive
 * pointers have been made implicit, hence packing 3 pointers in one.
 *
 ...
 *	else					remaining bits are a P-node
 * pointer, and N = P + 1
 *
 *	\relates	AABBCollisionNode
 *	\fn			_BuildCollisionTree(AABBCollisionNode* linear,
 * const udword boxid, udword& curid, const AABBTreeNode* curnode)
...
static void _BuildCollisionTree( AABBCollisionNode* linear,
    ...
    // Current node from input tree is "curnode". Must be flattened into
    // "linear[boxid]".
    ...
    if ( curnode->IsLeaf() ) {
        ...
        // To make the negative one implicit, we must store P and N in
        // successive order
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Builds a "no-leaf" tree from a standard one. This is a tree whose leaf
 ...
 *	\fn			_BuildNoLeafTree(AABBNoLeafNode* linear, const
 * udword boxid, udword& curid, const AABBTreeNode* curnode)
 *	\param		linear		[in] base address of destination nodes
 *	\param		boxid		[in] index of destination node
 *	\param		curid		[in] current running index
 *	\param		curnode		[in] current node from input tree
...
bool AABBCollisionTree::Build( AABBTree* tree ) {
    ...
    Log( "AABB Collision tree: %d nodes, %d bytes - Alignment: %d\n", mNbNodes,
         GetUsedBytes(), Alignment( udword( mNodes ) ) );
#endif
...
// Quantization notes:
// - We could use the highest bits of mData to store some more quantized bits.
// Dequantization code
//   would be slightly more complex, but number of overlap tests would be
//   reduced (and anyhow those bits are currently wasted). Of course it's not
//   possible if we move to 16 bits mData.
// - Something like "16 bits floats" could be tested, to bypass the int-to-float
// conversion.
// - A dedicated BV-BV test could be used, dequantizing while testing for
// overlap. (i.e. it's some
//   lazy-dequantization which may save some work in case of early exits). At
//   the very least some muls could be saved by precomputing several more
//   matrices. But maybe not worth the pain.
// - Do we need to dequantize anyway? Not doing the extents-related muls only
// implies the box has
//   been scaled, for example.
// - The deeper we move into the hierarchy, the smaller the extents should be.
// May not need a fixed
//   number of quantization bits. Even better, could probably be best
//   delta-encoded.
...
#define PERFORM_QUANTIZATION                                                   \
    /* Quantize */                                                             \
    ( ( float* )mNodes[ i ].mAABB.mCenter )[ 0 ] =                             \
        sword( Nodes[ i ].mAABB.mCenter.x * CQuantCoeff.x );                   \
    ( ( float* )mNodes[ i ].mAABB.mCenter )[ 1 ] =                             \
        sword( Nodes[ i ].mAABB.mCenter.y * CQuantCoeff.y );                   \
    ( ( float* )mNodes[ i ].mAABB.mCenter )[ 2 ] =                             \
        sword( Nodes[ i ].mAABB.mCenter.z * CQuantCoeff.z );                   \
    ( ( float* )mNodes[ i ].mAABB.mExtents )[ 0 ] =                            \
        uword( Nodes[ i ].mAABB.mExtents.x * EQuantCoeff.x );                  \
    ( ( float* )mNodes[ i ].mAABB.mExtents )[ 1 ] =                            \
        uword( Nodes[ i ].mAABB.mExtents.y * EQuantCoeff.y );                  \
    ( ( float* )mNodes[ i ].mAABB.mExtents )[ 2 ] =                            \
        uword( Nodes[ i ].mAABB.mExtents.z * EQuantCoeff.z );                  \
    /* Fix quantized boxes */                                                  \
    if ( gFixQuantized ) {                                                     \
        ...
        /* For each axis */                                                    \
        for ( udword j = 0; j < 3; j++ ) { /* Dequantize the box center */     \
            float qc = float( ( ( float* )mNodes[ i ].mAABB.mCenter )[ j ] ) * \
                       ( ( float* )mCenterCoeff )[ j ];                        \
            bool FixMe = true;                                                 \
            do { /* Dequantize the box extent */                               \
                float qe =                                                     \
                    float( ( ( float* )mNodes[ i ].mAABB.mExtents )[ j ] ) *   \
                    ( ( float* )mExtentsCoeff )[ j ];                          \
                /* Compare real & dequantized values */                        \
                if ( qc + qe < ( ( float* )Max )[ j ] ||                       \
                     qc - qe > ( ( float* )Min )[ j ] )                        \
