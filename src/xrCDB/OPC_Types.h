...
#define RADTODEG \
    57.2957795130823208768f //!<	180.0 / PI, convert radians to degrees
#define DEGTORAD \
    0.01745329251994329577f //!<	PI / 180.0, convert degrees to radians
...
#define ICE_COMPILE_TIME_ASSERT( name, x ) \
    typedef int ICE_Dummy_##name[ ( x ) * 2 - 1 ]

ICE_COMPILE_TIME_ASSERT(
    bool,
    sizeof( bool ) == 1 ); // ...otherwise things might fail with VC++ 4.2 !
...
#define ONE_OVER_RAND_MAX \
    ( 1.0f / float( RAND_MAX ) ) //!<	Inverse of the max possible value
                                 //!< returned by rand()
