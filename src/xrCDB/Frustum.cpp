...
EFC_Visible CFrustum::testSAABB( Fvector& c,
                                 float r,
                                 const float* mM,
                                 u32& test_mask ) const {
    u32 bit = 1;
...
void CFrustum::CreateFromPlanes( Fplane* p, int count ) {
    ...
    for ( int i = 0; i < count; i++ ) {
        float denom =
            1.0f / planes[ i ].n.magnitude(); // Get magnitude of Vector
        planes[ i ].n.x *= denom;
        planes[ i ].n.y *= denom;
        planes[ i ].n.z *= denom;
        planes[ i ].d *= denom;
...
void CFrustum::CreateOccluder( Fvector* p,
                               int count,
                               Fvector& vBase,
                               CFrustum& clip ) {
    ...
    // here we have all edges marked accordenly to 'open' / 'closed'
    // classification
    _clear();
...
BOOL CFrustum::CreateFromClipPoly( Fvector* p,
                                   int count,
                                   Fvector& vBase,
                                   CFrustum& clip ) {
    VERIFY( count < FRUSTUM_MAXPLANES );
