...
//  the XRCDB_EXPORTS symbol defined on the command line. this symbol should not
//  be defined on any project that uses this DLL. This way any other project
//  whose source files include this file see XRCDB_API functions as being
...
// Build callback
typedef void __stdcall build_callback( Fvector* V,
                                       int Vcnt,
                                       TRI* T,
                                       int Tcnt,
                                       void* params );
...
class XRCDB_API MODEL {
    ...
    enum { S_READY = 0, S_INIT = 1, S_BUILD = 2, S_forcedword = u32( -1 ) };
...
public:
    ...
    void build_internal( Fvector* V,
                         int Vcnt,
                         TRI* T,
                         int Tcnt,
                         build_callback* bc = NULL,
                         void* bcp = NULL );
    void build( Fvector* V,
                int Vcnt,
                TRI* T,
                int Tcnt,
                build_callback* bc = NULL,
...
enum {
    ...
    OPT_ONLYNEAREST = ( 1 << 2 ),
    OPT_FULL_TEST =
        ( 1 << 3 ) // for box & frustum queries - enable class III test(s)
};
...
#pragma pack( pop )
    ...
    void add_face( const Fvector& v0,
                   const Fvector& v1,
                   const Fvector& v2,
                   u16 material,
                   u16 sector );
    void add_face_D( const Fvector& v0,
                     const Fvector& v1,
                     const Fvector& v2,
                     u32 dummy );
    void add_face_packed( const Fvector& v0,
                          const Fvector& v1,
                          const Fvector& v2,
                          u16 material,
                          u16 sector,
                          float eps = EPS );
    void add_face_packed_D( const Fvector& v0,
                            const Fvector& v1,
                            const Fvector& v2,
                            u32 dummy,
                            float eps = EPS );
...
public:
    ...
    //		__declspec(noinline) CollectorPacked &operator=	(const
    // CollectorPacked &object)
    //		{
    //			verts
    //		}
