...
#define FRUSTUM_P_LRTB \
    ( FRUSTUM_P_LEFT | FRUSTUM_P_RIGHT | FRUSTUM_P_TOP | FRUSTUM_P_BOTTOM )
...
public:
    ICF EFC_Visible AABB_OverlapPlane( const fplane& P,
                                       const float* mM ) const {
        // calc extreme pts (neg,pos) along normal axis (pos in dir of norm,
        // etc.)
...
public:
    ...
    void CreateOccluder( Fvector* p,
                         int count,
                         Fvector& vBase,
                         CFrustum& clip );
    BOOL CreateFromClipPoly( Fvector* p,
                             int count,
                             Fvector& vBase,
                             CFrustum& clip );
    ...
    EFC_Visible testSAABB( Fvector& c,
                           float r,
                           const float* mM,
                           u32& test_mask ) const;
