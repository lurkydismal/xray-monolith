 ...
 * settings and callbacks have been defined.
 *	\return		null if everything is ok, else a string describing the
 * problem
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 * pointers and a colliding pair of primitives
    ...
    */
    ...
    if ( cache.HullTest ) {
        if ( cache.Model0->GetHull() && cache.Model1->GetHull() ) {
            struct Local {
                static Point* SVCallback( const Point& sv,
                                          udword& previndex,
                                          udword user_data ) {
    ...
    if ( !cache.Model0->HasLeafNodes() ) {
        if ( cache.Model0->IsQuantized() ) {
            const AABBQuantizedNoLeafTree* T0 =
                ( const AABBQuantizedNoLeafTree* )cache.Model0->GetTree();
            const AABBQuantizedNoLeafTree* T1 =
                ( const AABBQuantizedNoLeafTree* )cache.Model1->GetTree();
            Status = Collide( T0, T1, world0, world1, &cache );
        } else {
            const AABBNoLeafTree* T0 =
                ( const AABBNoLeafTree* )cache.Model0->GetTree();
            const AABBNoLeafTree* T1 =
                ( const AABBNoLeafTree* )cache.Model1->GetTree();
            Status = Collide( T0, T1, world0, world1, &cache );
        }
    } else {
        if ( cache.Model0->IsQuantized() ) {
            const AABBQuantizedTree* T0 =
                ( const AABBQuantizedTree* )cache.Model0->GetTree();
            const AABBQuantizedTree* T1 =
                ( const AABBQuantizedTree* )cache.Model1->GetTree();
            Status = Collide( T0, T1, world0, world1, &cache );
        } else {
            const AABBCollisionTree* T0 =
                ( const AABBCollisionTree* )cache.Model0->GetTree();
            const AABBCollisionTree* T1 =
                ( const AABBCollisionTree* )cache.Model1->GetTree();
            Status = Collide( T0, T1, world0, world1, &cache );
        }
    }
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void AABBTreeCollider::InitQuery( const Matrix4x4* world0,
                                  const Matrix4x4* world1 ) {
    // Reset stats & contact status
    ...
    for ( udword i = 0; i < 3; i++ ) {
        for ( udword j = 0; j < 3; j++ ) {
            // Epsilon value prevents floating-point inaccuracies (strategy
            // borrowed from RAPID)
 ...
 *	\param		cache	[in] cache for a pair of previously colliding
 * primitives
 ...
 *	\param		tree1			[in] AABB tree from second
 * object
 *	\param		world0			[in] world matrix for first
 * object
 *	\param		world1			[in] world matrix for second
 * object
 *	\param		cache			[in/out] cache for a pair of
 * previously colliding primitives
 *	\return		true if success
 *	\warning	SCALE NOT SUPPORTED. The matrices must contain rotation
 * & translation parts only.
 */
...
bool AABBTreeCollider::Collide( const AABBQuantizedTree* tree0,
    ...
    const Point a( float( N0->mAABB.mExtents[ 0 ] ) * mExtentsCoeff0.x,
                   float( N0->mAABB.mExtents[ 1 ] ) * mExtentsCoeff0.y,
                   float( N0->mAABB.mExtents[ 2 ] ) * mExtentsCoeff0.z );
    const Point Pa( float( N0->mAABB.mCenter[ 0 ] ) * mCenterCoeff0.x,
    ...
    const Point b( float( N1->mAABB.mExtents[ 0 ] ) * mExtentsCoeff1.x,
                   float( N1->mAABB.mExtents[ 1 ] ) * mExtentsCoeff1.y,
                   float( N1->mAABB.mExtents[ 2 ] ) * mExtentsCoeff1.z );
    const Point Pb( float( N1->mAABB.mCenter[ 0 ] ) * mCenterCoeff1.x,
...
// The normal AABB tree can use 2 different descent rules (with different
// performances)
...
void AABBTreeCollider::_Collide( const AABBCollisionNode* b0,
    ...
    if ( !BoxBoxOverlap( b0->mAABB.mExtents, b0->mAABB.mCenter,
                         b1->mAABB.mExtents, b1->mAABB.mCenter ) )
        return;
    ...
    if ( b1->IsLeaf() ||
         ( !b0->IsLeaf() && ( b0->GetSize() > b1->GetSize() ) ) ) {
...
void AABBTreeCollider::PrimTest( udword id0, udword id1 ) {
    ...
    if ( TriTriOverlap( *VP0.Vertex[ 0 ], *VP0.Vertex[ 1 ], *VP0.Vertex[ 2 ],
                        u0, u1, u2 ) ) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Leaf-leaf test for a previously fetched triangle from tree A (in B's
 * space) and a _new_ leaf from B.
...
#endif
    // Perform triangle-triangle overlap test
    if ( TriTriOverlap( mLeafVerts[ 0 ], mLeafVerts[ 1 ], mLeafVerts[ 2 ],
                        *VP.Vertex[ 0 ], *VP.Vertex[ 1 ], *VP.Vertex[ 2 ] ) ) {
        // Keep track of colliding pairs
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Leaf-leaf test for a previously fetched triangle from tree B (in A's
 * space) and a _new_ leaf from A.
...
void AABBTreeCollider::_Collide( const AABBNoLeafNode* a,
    ...
    // Perform BV-BV overlap test
    if ( !BoxBoxOverlap( a->mAABB.mExtents, a->mAABB.mCenter, b->mAABB.mExtents,
                         b->mAABB.mCenter ) )
        return;
    ...
    if ( a->HasLeaf() ) {
#ifdef OPC_USE_CALLBACKS
        FETCH_LEAF( a->GetPrimitive(), mObjCallback0, mUserData0, mR0to1,
                    mT0to1 )
#else
        ...
        if ( BHasLeaf ) {
#ifdef OPC_USE_CALLBACKS
            FETCH_LEAF( b->GetPrimitive(), mObjCallback1, mUserData1, mR1to0,
                        mT1to0 )
#else
        ...
        if ( BHasLeaf2 ) {
#ifdef OPC_USE_CALLBACKS
            FETCH_LEAF( b->GetPrimitive2(), mObjCallback1, mUserData1, mR1to0,
                        mT1to0 )
#else
    ...
    if ( a->HasLeaf2() ) {
#ifdef OPC_USE_CALLBACKS
        FETCH_LEAF( a->GetPrimitive2(), mObjCallback0, mUserData0, mR0to1,
                    mT0to1 )
#else
...
void AABBTreeCollider::_Collide( const AABBQuantizedNode* b0,
    ...
    if ( b1->IsLeaf() ||
        ...
        const Point negPa( float( Box->mCenter[ 0 ] ) * mCenterCoeff0.x,
                           float( Box->mCenter[ 1 ] ) * mCenterCoeff0.y,
                           float( Box->mCenter[ 2 ] ) * mCenterCoeff0.z );
        const Point nega( float( Box->mExtents[ 0 ] ) * mExtentsCoeff0.x,
                          float( Box->mExtents[ 1 ] ) * mExtentsCoeff0.y,
        ...
        // Dequantize box
        Box = &b0->GetPos()->mAABB;
        const Point posPa( float( Box->mCenter[ 0 ] ) * mCenterCoeff0.x,
                           float( Box->mCenter[ 1 ] ) * mCenterCoeff0.y,
                           float( Box->mCenter[ 2 ] ) * mCenterCoeff0.z );
        const Point posa( float( Box->mExtents[ 0 ] ) * mExtentsCoeff0.x,
                          float( Box->mExtents[ 1 ] ) * mExtentsCoeff0.y,
                          float( Box->mExtents[ 2 ] ) * mExtentsCoeff0.z );
        ...
        const Point negPb( float( Box->mCenter[ 0 ] ) * mCenterCoeff1.x,
                           float( Box->mCenter[ 1 ] ) * mCenterCoeff1.y,
                           float( Box->mCenter[ 2 ] ) * mCenterCoeff1.z );
        const Point negb( float( Box->mExtents[ 0 ] ) * mExtentsCoeff1.x,
                          float( Box->mExtents[ 1 ] ) * mExtentsCoeff1.y,
        ...
        // Dequantize box
        Box = &b1->GetPos()->mAABB;
        const Point posPb( float( Box->mCenter[ 0 ] ) * mCenterCoeff1.x,
                           float( Box->mCenter[ 1 ] ) * mCenterCoeff1.y,
                           float( Box->mCenter[ 2 ] ) * mCenterCoeff1.z );
        const Point posb( float( Box->mExtents[ 0 ] ) * mExtentsCoeff1.x,
                          float( Box->mExtents[ 1 ] ) * mExtentsCoeff1.y,
                          float( Box->mExtents[ 2 ] ) * mExtentsCoeff1.z );
...
void AABBTreeCollider::_CollideTriBox( const AABBQuantizedNoLeafNode* b ) {
    ...
    const Point Pb( float( bb->mCenter[ 0 ] ) * mCenterCoeff1.x,
                    float( bb->mCenter[ 1 ] ) * mCenterCoeff1.y,
                    float( bb->mCenter[ 2 ] ) * mCenterCoeff1.z );
    const Point eb( float( bb->mExtents[ 0 ] ) * mExtentsCoeff1.x,
                    float( bb->mExtents[ 1 ] ) * mExtentsCoeff1.y,
...
void AABBTreeCollider::_CollideBoxTri( const AABBQuantizedNoLeafNode* b ) {
    // Dequantize box
    const QuantizedAABB* bb = &b->mAABB;
    const Point Pa( float( bb->mCenter[ 0 ] ) * mCenterCoeff0.x,
                    float( bb->mCenter[ 1 ] ) * mCenterCoeff0.y,
                    float( bb->mCenter[ 2 ] ) * mCenterCoeff0.z );
    const Point ea( float( bb->mExtents[ 0 ] ) * mExtentsCoeff0.x,
                    float( bb->mExtents[ 1 ] ) * mExtentsCoeff0.y,
...
void AABBTreeCollider::_Collide( const AABBQuantizedNoLeafNode* a,
    ...
    const Point Pa( float( ab->mCenter[ 0 ] ) * mCenterCoeff0.x,
                    float( ab->mCenter[ 1 ] ) * mCenterCoeff0.y,
                    float( ab->mCenter[ 2 ] ) * mCenterCoeff0.z );
    const Point ea( float( ab->mExtents[ 0 ] ) * mExtentsCoeff0.x,
                    float( ab->mExtents[ 1 ] ) * mExtentsCoeff0.y,
