...
/*	inline_ void StoreDwords(udword* dest, udword nb, udword value)
        {
                // The asm code below **SHOULD** be equivalent to one of those C
   versions
...
*/
#define SIZEOFOBJECT \
    sizeof( *this ) //!< Gives the size of current object. Avoid some mistakes
                    //!< (e.g. "sizeof(this)").
...
#define CHECKALLOC( x )       \
    if ( !x )                 \
        return SetIceError(   \
            "Out of memory.", \
            EC_OUTOFMEMORY ); //!< Standard alloc checking. HANDLE WITH CARE.
#else
#define CHECKALLOC( x ) \
    if ( !x )           \
        return false;
