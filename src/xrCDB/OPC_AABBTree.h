...
protected:                                                                   \
    ...
    /* Following data always belong to the BV-tree, regardless of what the   \
     * tree actually contains.*/                                             \
    /* Whatever happens we need the two children and the enclosing volume.*/ \
    volume mBV; /* Global bounding-volume enclosing all the node-related     \
                   primitives */                                             \
    baseclass* mP;                                                           \
    baseclass* mN;
...
protected:
    // Tree-dependent data
    udword* mNodePrimitives; //!< Node-related primitives (shortcut to a
...
private:
    udword* mIndices; //!< Indices in the app list. Indices are reorganized
                      //!< during build.
    // Stats
