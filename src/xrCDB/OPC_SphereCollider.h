...
struct OPCODE_API SphereCache : VolumeCache {
    SphereCache()
        : Center( 0.0f, 0.0f, 0.0f ), FatRadius2( 0.0f ), FatCoeff( 1.1f ) {}

    // Cached faces signature
    Point Center; //!< Sphere used when performing the query resulting in cached
                  //!< faces
    float FatRadius2; //!< Sphere used when performing the query resulting in
                      //!< cached faces
    // User settings
...
public:
     ...
     *	\param		sphere			[in] collision sphere in local
     * space
     *	\param		model			[in] Opcode model to collide
     * with
     *	\param		worlds			[in] sphere's world matrix, or
     * null
     *	\param		worldm			[in] model's world matrix, or
     * null
...
protected:
    ...
    BOOL SphereTriOverlap( const Point& vert0,
                           const Point& vert1,
                           const Point& vert2 );
    // Init methods
