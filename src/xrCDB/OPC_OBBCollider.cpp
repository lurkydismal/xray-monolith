...
#define OBB_PRIM( primindex )                                       \
    ...
    TransformPoint( mLeafVerts[ 0 ], *VP.Vertex[ 0 ], mRModelToBox, \
                    mTModelToBox );                                 \
    TransformPoint( mLeafVerts[ 1 ], *VP.Vertex[ 1 ], mRModelToBox, \
                    mTModelToBox );                                 \
    TransformPoint( mLeafVerts[ 2 ], *VP.Vertex[ 2 ], mRModelToBox, \
                    mTModelToBox );                                 \
...
#define OBB_PRIM( primindex )                                               \
    ...
    /* Transform them in a common space */                                  \
    TransformPoint( mLeafVerts[ 0 ], mVerts[ T->mVRef[ 0 ] ], mRModelToBox, \
                    mTModelToBox );                                         \
    TransformPoint( mLeafVerts[ 1 ], mVerts[ T->mVRef[ 1 ] ], mRModelToBox, \
                    mTModelToBox );                                         \
    TransformPoint( mLeafVerts[ 2 ], mVerts[ T->mVRef[ 2 ] ], mRModelToBox, \
                    mTModelToBox );                                         \
    /* Perform triangle-box overlap test */                                 \
    if ( TriBoxOverlap() ) {                                                \
        /* Set contact status */                                            \
 ...
 *	\param		box			[in] collision OBB in local
 * space
...
bool OBBCollider::Collide( OBBCache& cache,
    ...
    if ( !model->HasLeafNodes() ) {
        if ( model->IsQuantized() )
            return Collide( cache, box,
                            ( const AABBQuantizedNoLeafTree* )model->GetTree(),
                            worldb, worldm );
        else
            return Collide( cache, box,
                            ( const AABBNoLeafTree* )model->GetTree(), worldb,
                            worldm );
    } else {
        if ( model->IsQuantized() )
            return Collide( cache, box,
                            ( const AABBQuantizedTree* )model->GetTree(),
                            worldb, worldm );
        else
            return Collide( cache, box,
                            ( const AABBCollisionTree* )model->GetTree(),
                            worldb, worldm );
    }
}
...
BOOL OBBCollider::InitQuery( OBBCache& cache,
    ...
    mBBx1 = Box.x * mAR.m[ 0 ][ 0 ] + Box.y * mAR.m[ 1 ][ 0 ] +
            Box.z * mAR.m[ 2 ][ 0 ];
    mBBy1 = Box.x * mAR.m[ 0 ][ 1 ] + Box.y * mAR.m[ 1 ][ 1 ] +
            Box.z * mAR.m[ 2 ][ 1 ];
    mBBz1 = Box.x * mAR.m[ 0 ][ 2 ] + Box.y * mAR.m[ 1 ][ 2 ] +
            Box.z * mAR.m[ 2 ][ 2 ];
    ...
    if ( TemporalCoherenceEnabled() ) {
        ...
        // => check results from previous frame before performing the collision
        // query
        if ( FirstContactEnabled() ) {
            // We're only interested in the first contact found => test the
            // unique previously touched face
            if ( mTouchedPrimitives->GetNbEntries() ) {
                // Get index of previously touched face = the first entry in the
                // array
                udword PreviouslyTouchedFace =
                    mTouchedPrimitives->GetEntry( 0 );

                // Then reset the array:
                // - if the overlap test below is successful, the index we'll
                // get added back anyway
                // - if it isn't, then the array should be reset anyway for the
                // normal query
                mTouchedPrimitives->Reset();

                // Perform overlap test between the cached triangle and the box
                // (and set contact status if needed)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Checks the OBB completely contains the box. In which case we can end the
 * query sooner.
...
inline_ BOOL OBBCollider::OBBContainsBox( const Point& bc, const Point& be ) {
    ...
    \
            f = p.x * mRModelToBox.m[0][0] + p.y * mRModelToBox.m[1][0] + p.z *
    mRModelToBox.m[2][0];	if(f>mB0.x || f<mB1.x) return FALSE;\
            f = p.x * mRModelToBox.m[0][1] + p.y * mRModelToBox.m[1][1] + p.z *
    mRModelToBox.m[2][1];	if(f>mB0.y || f<mB1.y) return FALSE;\ f = p.x *
    mRModelToBox.m[0][2] + p.y * mRModelToBox.m[1][2] + p.z *
    mRModelToBox.m[2][2];	if(f>mB0.z || f<mB1.z) return FALSE;
    ...
    */
    ...
    // - test AABB-in-AABB
    float NCx = bc.x * mRModelToBox.m[ 0 ][ 0 ] +
                bc.y * mRModelToBox.m[ 1 ][ 0 ] +
                bc.z * mRModelToBox.m[ 2 ][ 0 ];
    float NEx = _abs( mRModelToBox.m[ 0 ][ 0 ] * be.x ) +
                _abs( mRModelToBox.m[ 1 ][ 0 ] * be.y ) +
                _abs( mRModelToBox.m[ 2 ][ 0 ] * be.z );
    ...
    float NCy = bc.x * mRModelToBox.m[ 0 ][ 1 ] +
                bc.y * mRModelToBox.m[ 1 ][ 1 ] +
                bc.z * mRModelToBox.m[ 2 ][ 1 ];
    float NEy = _abs( mRModelToBox.m[ 0 ][ 1 ] * be.x ) +
                _abs( mRModelToBox.m[ 1 ][ 1 ] * be.y ) +
                _abs( mRModelToBox.m[ 2 ][ 1 ] * be.z );
    ...
    float NCz = bc.x * mRModelToBox.m[ 0 ][ 2 ] +
                bc.y * mRModelToBox.m[ 1 ][ 2 ] +
                bc.z * mRModelToBox.m[ 2 ][ 2 ];
    float NEz = _abs( mRModelToBox.m[ 0 ][ 2 ] * be.x ) +
                _abs( mRModelToBox.m[ 1 ][ 2 ] * be.y ) +
                _abs( mRModelToBox.m[ 2 ][ 2 ] * be.z );
