...
public:
    ...
    // fpu
    ICF BOOL _box_fpu( const Fvector& bCenter,
    ...
    // sse
    ICF BOOL _box_sse( const Fvector& bCenter,
                       const Fvector& bExtents,
                       float& dist ) {
        aabb_t box;
        /*
    ...
    IC bool _tri( u32* p, float& u, float& v, float& range ) {
        ...
        if ( bCull ) {
            ...
            u = tvec.dotproduct(
                pvec ); // calculate U parameter and test bounds
            if ( u < 0.f || u > det )
                return false;
            ...
            if ( v < 0.f || u + v > det )
                return false;
            range = edge2.dotproduct( qvec ); // calculate t, scale parameters,
...
void COLLIDER::ray_query( const MODEL* m_def,
    ...
    if ( !_finite( r_start.x ) || !_finite( r_start.y ) ||
         !_finite( r_start.z ) ) {
        VERIFY(
            "![COLLIDER::ray_query] r_start has NaN members, abort ray_query" );
        return;
    }
    ...
    cform_ray_collider RC(
        CPU::ID.feature && _CPU_FEATURE_SSE, !!( ray_mode & OPT_CULL ),
        !!( ray_mode & OPT_ONLYFIRST ), !!( ray_mode & OPT_ONLYNEAREST ) );
