...
class CSharedObj : public CSingleton< CSharedObj< SHARED_TYPE, KEY_TYPE > > {
    ...
    typedef
        typename xr_map< KEY_TYPE, SHARED_TYPE* >::iterator SHARED_DATA_MAP_IT;

public:
    CSharedObj() {};

    virtual ~CSharedObj() {
        for ( SHARED_DATA_MAP_IT it = _shared_tab.begin();
              it != _shared_tab.end(); ++it ) {
            xr_delete( it->second );
        }
    }
...
//	void Smth() {
//		Msg("shared: a = [%d] b =[%d]", inherited_shared::get_sd()->a,
// inherited_shared::get_sd()->b);
//	}
// };
