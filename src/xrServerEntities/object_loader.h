...
struct CLoader {
    ...
    struct CHelper1 {
        template < bool a >
        IC static void load_data( T& data, M& stream, const P& ) {
            if constexpr ( a ) {
                data.load( stream );
            } else {
                STATIC_CHECK( !is_polymorphic< T >::result,
                              Cannot_load_polymorphic_classes_as_binary_data );
    ...
    struct CHelper {
        ...
        IC static void load_data( T& data, M& stream, const P& p ) {
            if constexpr ( pointer ) {
                CLoader< M, P >::load_data(
                    *( data =
                           xr_new< typename object_type_traits::remove_pointer<
                               T >::type >() ),
                    stream, p );
            } else {
                CHelper1< T >::template load_data<
                    object_type_traits::
                        is_base_and_derived_or_same_from_template<
                            IPureLoadableObject, T >::value >( data, stream,
                                                               p );
            }
        }
    };
    ...
    struct CHelper3 {
        ...
        struct has_value_compare {
            ...
            static object_type_traits::detail::yes select(
                object_type_traits::detail::other<
                    typename _P::value_compare >* );
            template < typename _P >
            static object_type_traits::detail::no select( ... );

            enum {
                value = sizeof( object_type_traits::detail::yes ) ==
                        sizeof( select< T >( 0 ) )
            };
        };
        ...
        struct is_tree_structure {
            enum { value = has_value_compare< T >::value };
        };
        ...
        template < typename T1, typename T2 >
        IC static void add( T1& data, T2& value ) {
            add_helper< T1, T2 >::template add<
                is_tree_structure< T1 >::value >( data, value );
        }
    ...
    struct CHelper4 {
        ...
        IC static void load_data( T& data, M& stream, const P& p ) {
            if constexpr ( a ) {
                ...
                CHelper< T >::template load_data<
                    object_type_traits::is_pointer< T >::value >( data, stream,
                                                                  p );
            }
        }
    };
    ...
    template < typename T1, typename T2 >
    IC static void load_data( std::pair< T1, T2 >& data,
                              M& stream,
                              const P& p ) {
        if ( p( data,
                const_cast< object_type_traits::remove_const< T1 >::type& >(
                    data.first ),
                true ) ) {
    ...
    IC static void load_data( svector< T, size >& data,
                              M& stream,
                              const P& p ) {
        if ( p.can_clear() )
            data.clear();
    ...
    template < typename T1, typename T2 >
    IC static void load_data( std::queue< T1, T2 >& data,
                              M& stream,
                              const P& p ) {
        if ( p.can_clear() ) {
            while ( !data.empty() )
    ...
    template < template < typename _1, typename _2 > class T1,
               typename T2,
               typename T3 >
    IC static void load_data( T1< T2, T3 >& data,
                              M& stream,
                              const P& p,
                              bool ) {
        if ( p.can_clear() ) {
    ...
    template < template < typename _1, typename _2, typename _3 > class T1,
               typename T2,
               typename T3,
               typename T4 >
    IC static void load_data( T1< T2, T3, T4 >& data,
                              M& stream,
                              const P& p,
                              bool ) {
        if ( p.can_clear() ) {
    ...
    template < typename T1, typename T2 >
    IC static void load_data( xr_stack< T1, T2 >& data,
                              M& stream,
                              const P& p ) {
    ...
    IC static void load_data( std::priority_queue< T1, T2, T3 >& data,
                              M& stream,
                              const P& p ) {
        load_data( data, stream, p, true );
    }
    ...
    template < typename T >
    IC static void load_data( T& data, M& stream, const P& p ) {
        CHelper4< T >::template load_data<
            object_type_traits::is_stl_container< T >::value >( data, stream,
                                                                p );
    }
};
