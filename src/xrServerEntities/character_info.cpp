...
void CCharacterInfo::InitSpecificCharacter( shared_str new_id ) {
    ...
    if ( Rank().value() == NO_RANK )
        if ( m_SpecificCharacter.RankDef().min !=
             m_SpecificCharacter.RankDef().max ) {
            int rank = ::Random.randI( m_SpecificCharacter.RankDef().min,
                                       m_SpecificCharacter.RankDef().max );
            SetRank( rank );
        } else
            SetRank( m_SpecificCharacter.RankDef().max );
    if ( Reputation().value() == NO_REPUTATION )
        if ( m_SpecificCharacter.ReputationDef().min !=
             m_SpecificCharacter.ReputationDef().max ) {
            int rep = ::Random.randI( m_SpecificCharacter.ReputationDef().min,
                                      m_SpecificCharacter.ReputationDef().max );
            SetReputation( rep );
        } else
...
void CCharacterInfo::load_shared( LPCSTR ) {
    ...
    if ( !spec_char ) {
        ...
        if ( min_reputation != NO_REPUTATION &&
             max_reputation != NO_REPUTATION ) {
            ...
            if ( max_reputation != min_reputation )
                data()->m_Reputation =
                    ::Random.randI( min_reputation, max_reputation );
            else
                data()->m_Reputation = max_reputation;
        } else
            data()->m_Reputation =
                pXML->ReadInt( "reputation", 0, NO_REPUTATION );
    } else
