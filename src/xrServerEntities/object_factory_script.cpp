...
void CObjectFactory::register_script_class( LPCSTR client_class,
                                            LPCSTR server_class,
                                            LPCSTR clsid,
                                            LPCSTR script_clsid ) {
#ifndef NO_XR_GAME
    ::luabind::object client;
    if ( !ai().script_engine().function_object( client_class, client,
                                                LUA_TUSERDATA ) ) {
        ai().script_engine().script_log(
            eLuaMessageTypeError, "Cannot register class %s", client_class );
        return;
    }
#endif
    ::luabind::object server;
    if ( !ai().script_engine().function_object( server_class, server,
                                                LUA_TUSERDATA ) ) {
        ai().script_engine().script_log(
            eLuaMessageTypeError, "Cannot register class %s", server_class );
        return;
    }
...
void CObjectFactory::register_script_class( LPCSTR unknown_class,
                                            LPCSTR clsid,
                                            LPCSTR script_clsid ) {
    ::luabind::object creator;
    if ( !ai().script_engine().function_object( unknown_class, creator,
                                                LUA_TUSERDATA ) ) {
        ai().script_engine().script_log(
            eLuaMessageTypeError, "Cannot register class %s", unknown_class );
        return;
    }
    add( xr_new< CObjectItemScript >(
#ifndef NO_XR_GAME
...
void CObjectFactory::register_script() const {
    ...
    for ( ; I != E; ++I )
        instance = std::move( instance )
                       .enum_( "_clsid" )[::luabind::value(
                           *( *I )->script_clsid(), int( I - B ) ) ];

    lua_State* L = ai().script_engine().lua();
    ::luabind::module( L )[ std::move(
        instance ) ]; // это представление нельзя обработать как таблицу
    ...
    lua_setglobal(
        L, "clsid_table" ); // это представление можно обработать как таблицу :)
}
...
void CObjectFactory::script_register( lua_State* L ) {
    module(
        L )[ class_< CObjectFactory >( "object_factory" )
                 .def( "register",
                       ( void ( CObjectFactory::* )( LPCSTR, LPCSTR, LPCSTR,
                                                     LPCSTR ) )(
