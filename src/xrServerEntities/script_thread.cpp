...
// extern "C" __declspec(dllimport) lua_State *lua_newcthread(lua_State *OL, int
// cstacksize);
...
CScriptThread::CScriptThread( LPCSTR caNamespaceName,
    ...
    if ( !do_string ) {
        ...
        xr_sprintf( S, "function %s()\n%s\nend\n", main_function,
                    caNamespaceName );
        int l_iErrorCode = luaL_loadbuffer(
            ai().script_engine().lua(), S, xr_strlen( S ), "@console_command" );
        if ( !l_iErrorCode ) {
            l_iErrorCode = lua_pcall( ai().script_engine().lua(), 0, 0, 0 );
            if ( l_iErrorCode ) {
                ai().script_engine().print_output(
                    ai().script_engine().lua(), *m_script_name, l_iErrorCode );
    ...
    //		m_virtual_machine	=
    //lua_newcthread(ai().script_engine().lua(),0);
    ...
    if ( ai().script_engine().debugger() )
       ...
       //		print_stack_		(ai().script_engine().lua());
       //		m_thread_reference	=
       //luaL_ref(ai().script_engine().lua(),LUA_REGISTRYINDEX); 		print_stack_
       //(ai().script_engine().lua());

    //		if (g_ca_stdout[0]) {
    //			fputc
    //(0,stderr); 			ai().script_engine().script_log
    //(ScriptStorage::eLuaMessageTypeInfo,"%s",g_ca_stdout); 			fflush
    //(stderr);
    //		}
    //		Msg					("lua get top
    ...
    if ( ai().script_engine().debugger() &&
        ...
        lua_sethook( lua(), CDbgLuaHelper::hookLua,
                     LUA_MASKLINE | LUA_MASKCALL | LUA_MASKRET, 0 );
    else
#endif // #ifdef USE_DEBUGGER
        lua_sethook( lua(), CScriptEngine::lua_hook_call,
                     LUA_MASKLINE | LUA_MASKCALL | LUA_MASKRET, 0 );
#endif // #ifdef DEBUG
#endif // #ifndef USE_LUA_STUDIO
    ...
    if ( !ai().script_engine().load_buffer( lua(), S, xr_strlen( S ),
                                            "@_thread_main" ) )
        return;
...
CScriptThread::~CScriptThread() {
    ...
    luaL_unref( ai().script_engine().lua(), LUA_REGISTRYINDEX,
                m_thread_reference );
#endif
#if 0
	}
...
bool CScriptThread::update() {
    ...
    if ( l_iErrorCode && ( l_iErrorCode != LUA_YIELD ) ) {
        ai().script_engine().print_output( lua(), *script_name(),
                                           l_iErrorCode );
        ai().script_engine().on_error( ai().script_engine().lua() );
#ifdef DEBUG
        print_stack( lua() );
        ...
        if ( l_iErrorCode != LUA_YIELD ) {
            ...
            ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeInfo,
                                             "Script %s is finished!",
                                             *m_script_name );
#endif // DEBUG
        } else {
            VERIFY2( !lua_gettop( lua() ),
                     "Do not pass any value to coroutine.yield()!" );
        }
    }
