    ...
    while ( lua_getstack( L, level++, &ar ) ) {
        ...
        if ( level > LEVELS1 && firstpart ) {
            ...
            if ( !lua_getstack( L, level + LEVELS2, &ar ) )
                ...
                lua_pushliteral( L, "       ...\n" ); /* too many levels */
                while ( lua_getstack( L, level + LEVELS2,
                                      &ar ) ) /* find last levels */
                    level++;
            }
            firstpart = 0;
...
void CDbgLuaHelper::func_hook( lua_State* l, lua_Debug* ar ) {
    ...
    m_pThis->debugger()->FunctionHook( szSource, ar->currentline,
...
bool CDbgLuaHelper::GetCalltip( const char* szWord,
                                char* szCalltip,
                                int sz_calltip ) {
    int nLevel = debugger()->GetStackTraceLevel();
    lua_Debug ar;
    if ( lua_getstack( L, nLevel, &ar ) ) {
        ...
        while ( ( name = lua_getlocal( L, &ar, i++ ) ) != NULL ) {
            if ( xr_strcmp( name, szWord ) == 0 ) {
                ...
                xr_sprintf( szCalltip, sz_calltip, "local %s : %s ", name,
                            szRet );
                lua_pop( L, 1 ); /* remove variable value */
                return true;
            }
...
bool CDbgLuaHelper::Eval( const char* szCode, char* szRet, int szret_size ) {
    ...
    if ( status )
        ...
        if ( status ) {
            ...
            xr_sprintf( szRet, szret_size, "%s",
                        szErr2 ? ( szErr2 + 2 ) : szErr );
        } else
...
void CDbgLuaHelper::Describe( char* szRet, int nIndex, int szRet_size ) {
    ...
    switch ( ntype ) {
        ...
        case LUA_TBOOLEAN:
            xr_sprintf( value, "%s",
                        lua_toboolean( L, nIndex ) ? "true" : "false" );
            break;
        default:
            value[ 0 ] = '\0';
            break;
...
void CDbgLuaHelper::CoverGlobals() {
    ...
    if ( lua_getstack( L, nLevel, &ar ) ) {
        ...
        while ( ( name = lua_getlocal( L, &ar, i++ ) ) != NULL ) {
            ...
            lua_rawget( L,
                        LUA_GLOBALSINDEX ); /* SAVE name lvalue name gvalue */
...
void CDbgLuaHelper::DrawVariable( lua_State* l,
                                  const char* name,
                                  bool bOpenTable ) {
    Variable var;
    ...
    switch ( ntype ) {
        ...
        case LUA_TTABLE:
            ...
            /*	case LUA_TUSERDATA:{
               ...
               static_cast<::luabind::detail::object_rep*>(lua_touserdata(L,
               -1));
                                    ::luabind::detail::lua_reference& r =
               obj->get_lua_table(); lua_State * ls = NULL; r.get(ls);
                                    DrawTable(ls, name);
                                    return;
                            }break;*/
