...
int engine::lua_sethook( lua_State* L,
                         lua_Hook func,
                         lua_mask_type mask,
                         int count ) {
...
lua_Debug* engine::lua_debug_create() {
    VERIFY( m_instance_count <
            sizeof( m_instances ) / sizeof( m_instances[ 0 ] ) );
...
void engine::log( log_message_types const message_type,
                  char const* const message ) {}

char* engine::class_name( char* const buffer,
...
void engine::type_convert_class( char* const buffer,
                                 unsigned int const size,
                                 lua_State* state,
                                 int index ) {
    ::luabind::detail::object_rep* object =
        ::luabind::detail::is_class_object( state, index );
...
static bool is_luabind_class( lua_State* state, int const index ) {
    ::luabind::detail::class_rep* class_rep =
        static_cast< ::luabind::detail::class_rep* >(
            lua_touserdata( state, index ) );
    if ( !class_rep )
        return ( false );

    if ( class_rep->get_class_type() ==
         ::luabind::detail::class_rep::lua_class )
        return ( true );

    if ( ::luabind::detail::class_registry::get_registry( state )->find_class(
             class_rep->type() ) != class_rep )
        return ( false );

    return ( true );
}

bool engine::type_convert_instance( char* buffer,
                                    unsigned int const size,
                                    lua_State* state,
                                    int index ) {
...
void engine::type_convert_userdata( char* buffer,
                                    unsigned int const size,
                                    lua_State* state,
                                    int index ) {
...
bool engine::type_to_string( char* const buffer,
                             unsigned int const size,
...
void engine::fill_class_info( cs::lua_studio::backend& backend,
                              char* const buffer,
                              unsigned int const size,
                              ::luabind::detail::object_rep* object,
                              ::luabind::detail::class_rep* class_rep,
    ...
    for ( u32 i = 0; I != E; ++I ) {
        ...
        string4096 type;
        bool use_in_description;
        backend.type_to_string( type, sizeof( type ), state, -1,
                                use_in_description );

        string4096 value;
        cs::lua_studio::icon_type icon_type;
        backend.value_to_string( value, sizeof( value ), state, -1, icon_type,
                                 false );
        ...
        if ( use_in_description )
            stream += xr_sprintf( stream, size - ( stream - buffer ),
                                  "%s[%s]=%s ", ( *I ).first, type, value );
        else
            stream += xr_sprintf( stream, size - ( stream - buffer ), "%s=%s ",
                                  ( *I ).first, value );
...
void engine::value_convert_class( cs::lua_studio::backend& backend,
                                  char* buffer,
                                  unsigned int size,
                                  ::luabind::detail::class_rep* class_rep,
                                  lua_State* state,
...
bool engine::value_convert_instance( cs::lua_studio::backend& backend,
                                     char* buffer,
                                     unsigned int size,
                                     ::luabind::detail::object_rep* object,
                                     lua_State* state ) {
...
bool engine::value_convert_instance( cs::lua_studio::backend& backend,
    ...
    if ( full_description &&
         !value_convert_instance( backend, buffer, size, object, state ) )
        value_convert_class( backend, buffer, size, object->crep(), state,
                             index, icon_type, full_description );
    else
        sz_cpy( buffer, size, " " );
...
bool engine::value_to_string( cs::lua_studio::backend& backend,
                              char* const buffer,
                              unsigned int const size,
                              lua_State* const state,
                              int const index,
            ...
            if ( !::luabind::detail::is_class_object( state, index ) ) {
                ...
                ::luabind::detail::class_rep* class_rep =
                    static_cast< ::luabind::detail::class_rep* >(
                        lua_touserdata( state, index ) );
                VERIFY( class_rep );
                value_convert_class( backend, buffer, size, class_rep, state,
                                     index, icon_type, full_description );
                return ( true );
            }

            if ( value_convert_instance( backend, buffer, size, state, index,
                                         icon_type, full_description ) )
                return ( true );
...
void engine::push_class( lua_State* const state, char const* const id ) {
    ::luabind::detail::object_rep* object =
        ::luabind::detail::is_class_object( state, -1 );
    VERIFY( object );
...
void engine::push_class_instance( lua_State* const state,
                                  char const* const id ) {
    ::luabind::detail::object_rep* object =
        ::luabind::detail::is_class_object( state, -1 );
    if ( !object ) {
...
void engine::push_user_data( lua_State* const state,
                             char const* const id,
                             cs::lua_studio::icon_type const icon_type ) {
    switch ( icon_type ) {
...
bool engine::push_value( lua_State* const state,
                         char const* const id,
                         cs::lua_studio::icon_type const icon_type ) {
...
void engine::fill_class_data( cs::lua_studio::backend& backend,
                              cs::lua_studio::value_to_expand& value_to_expand,
                              lua_State* const state ) {
    ::luabind::detail::object_rep* object =
        static_cast< ::luabind::detail::object_rep* >(
            lua_touserdata( state, -2 ) );
    ::luabind::detail::class_rep* _class =
        static_cast< ::luabind::detail::class_rep* >(
            lua_touserdata( state, -1 ) );
...
void engine::expand_class( cs::lua_studio::backend& backend,
                           cs::lua_studio::value_to_expand& value,
                           lua_State* const state ) {
    int start = lua_gettop( state );

    ::luabind::detail::class_rep* class_object =
        static_cast< ::luabind::detail::class_rep* >(
            lua_touserdata( state, -1 ) );
    ...
    ::luabind::detail::object_rep* object =
        ::luabind::detail::is_class_object( state, -2 );
    if ( !object )
        lua_pushnil( state );
...
void engine::expand_class_instance(
    ...
    if ( object->crep() ) {
        ...
        backend.value_to_string( value, sizeof( value ), state, -1, icon_type,
                                 true );
        value_to_expand.add_value( class_rep->name(), type, value,
...
void engine::expand_user_data( cs::lua_studio::backend& backend,
                               cs::lua_studio::value_to_expand& value,
                               lua_State* const state ) {
    ::luabind::detail::object_rep* object =
        ::luabind::detail::is_class_object( state, -1 );
...
bool engine::expand_value( cs::lua_studio::backend& backend,
                           cs::lua_studio::value_to_expand& value,
                           lua_State* const state ) {
    switch ( lua_type( state, -1 ) ) {
        case engine::lua_type_nil:
