...
ObjectFactory::CLIENT_BASE_CLASS* CObjectItemScript::client_object() const {
    ...
    try {
        object =
            ::luabind::object_cast< ObjectFactory::CLIENT_SCRIPT_BASE_CLASS* >(
...
ObjectFactory::SERVER_BASE_CLASS* CObjectItemScript::server_object(
    ...
    try {
        ...
        try {
            ...
            Msg( "Exception raised while creating server object from section "
                 "[%s]",
                 section );
            return ( 0 );
        }
        object =
            ::luabind::object_cast< ObjectFactory::SERVER_SCRIPT_BASE_CLASS* >(
                *instance, ::luabind::adopt< ::luabind::result >() );
        xr_delete( instance );
    } catch ( const std::exception& e ) {
        Msg( "Exception [%s] raised while casting and adopting script server "
             "object from section [%s]",
             e.what(), section );
        return ( 0 );
    } catch ( ... ) {
        Msg( "Exception raised while creating script server object from "
             "section [%s]",
             section );
        return ( 0 );
    }
