...
void CSpecificCharacter::InitXmlIdToIndex() {
    ...
    if ( !id_to_index::file_str )
        id_to_index::file_str =
            pSettings->r_string( "profiles", "specific_characters_files" );
}
...
void CSpecificCharacter::load_shared( LPCSTR ) {
    ...
    R_ASSERT3( !( data()->m_bNoRandom && data()->m_bDefaultForCommunity ),
               "cannot set 'no_random' and 'team_default' flags "
               "simultaneously, profile id",
               *shared_str( item_data.id ) );
    ...
    for ( int i = 0; i < dialogs_num; ++i ) {
        shared_str dialog_name =
            pXML->Read( pXML->GetLocalRoot(), "actor_dialog", i, "" );
        data()->m_ActorDialogs.push_back( dialog_name );
    }

    ::luabind::functor< ::luabind::object > funct;
    if ( ai().script_engine().functor( "_G.CSpecificCharacterDialogList",
                                       funct ) ) {
        ...
        if ( output && output.type() == LUA_TTABLE ) {
            ...
            for ( ; i != e; ++i ) {
                ...
                if ( v.type() == LUA_TSTRING ) {
                    shared_str dialog_name =
                        ::luabind::object_cast< LPCSTR >( v );
                    // Msg("character_id %s, dialog_name %s", character_name,
                    // dialog_name.c_str());
                    data()->m_ActorDialogs.push_back( dialog_name );
                }
            }
    ...
    data()->m_fHitProbabilityFactor =
        pXML->ReadFlt( "hit_probability_factor", 0, 1.f );
    data()->m_crouch_type = pXML->ReadInt( "crouch_type", 0, 0 );
    data()->m_upgrade_mechanic =
        ( pXML->ReadInt( "mechanic_mode", 0, 0 ) == 1 );

    data()->m_critical_wound_weights =
        pXML->Read( "critical_wound_weights", 0, "1" );
    ...
    R_ASSERT3( team != NULL,
               "'community' field not fulfiled for specific character",
               *m_OwnId );
    ...
    if ( data()->m_Community.index() == NO_COMMUNITY_INDEX )
        Debug.fatal( DEBUG_INFO,
                     "wrong 'community' '%s' in specific character %s ", team,
                     *m_OwnId );
    ...
    if ( min_rank != NO_RANK && max_rank != NO_RANK ) {
        ...
        R_ASSERT3( rank != NO_RANK,
                   "'rank' field not fulfiled for specific character",
                   *m_OwnId );
        RankDef().min = rank;
        RankDef().max = rank;
    }
    ...
    int min_reputation =
        pXML->ReadAttribInt( "reputation", 0, "min", NO_REPUTATION );
    int max_reputation =
        pXML->ReadAttribInt( "reputation", 0, "max", NO_REPUTATION );
    if ( min_reputation != NO_REPUTATION && max_reputation != NO_REPUTATION ) {
        ...
        R_ASSERT3( rep != NO_REPUTATION,
                   "'reputation' field not fulfiled for specific character",
                   *m_OwnId );
    ...
    if ( pXML->NavigateToNode( pXML->GetLocalRoot(), "money", 0 ) ) {
        ...
        MoneyDef().inf_money =
            !!pXML->ReadAttribInt( "money", 0, "infinitive" );
        MoneyDef().max_money =
            _max( MoneyDef().max_money, MoneyDef().min_money ); // :)
    } else {
    ...
    if ( ai().script_engine().functor( "_G.CSpecificCharacterInit",
                                       init_funct ) ) {
        ::luabind::object table =
            ::luabind::newtable( ai().script_engine().lua() );
        table[ "name" ] = Name();
        ...
        if ( output && output.type() == LUA_TTABLE ) {
            data()->m_sGameName =
                ::luabind::object_cast< LPCSTR >( output[ "name" ] );
            data()->m_sBioText = CStringTable().translate(
                ::luabind::object_cast< LPCSTR >( output[ "bio" ] ) );

            data()->m_Community.set(
                ::luabind::object_cast< LPCSTR >( output[ "community" ] ) );
            if ( data()->m_Community.index() == NO_COMMUNITY_INDEX )
                Debug.fatal(
                    DEBUG_INFO,
                    "wrong 'community' '%s' in specific character %s ",
                    ::luabind::object_cast< LPCSTR >( output[ "community" ] ),
                    *m_OwnId );

            data()->m_icon_name =
                ::luabind::object_cast< LPCSTR >( output[ "icon" ] );
            data()->m_StartDialog =
                output[ "start_dialog" ].type() == LUA_TSTRING
                    ? ::luabind::object_cast< LPCSTR >(
                          output[ "start_dialog" ] )
                    : NULL;
            data()->m_fPanic_threshold =
                ::luabind::object_cast< float >( output[ "panic_threshold" ] );
            data()->m_fHitProbabilityFactor = ::luabind::object_cast< float >(
                output[ "hit_probability_factor" ] );
            data()->m_crouch_type =
                ::luabind::object_cast< int >( output[ "crouch_type" ] );
            data()->m_upgrade_mechanic =
                ::luabind::object_cast< bool >( output[ "mechanic_mode" ] );
            data()->m_critical_wound_weights = ::luabind::object_cast< LPCSTR >(
                output[ "critical_wound_weights" ] );
            data()->m_sVisual =
                ::luabind::object_cast< LPCSTR >( output[ "visual" ] );
            data()->m_sNpcConfigSect =
                ::luabind::object_cast< LPCSTR >( output[ "npc_config" ] );
            data()->m_sound_voice_prefix =
                ::luabind::object_cast< LPCSTR >( output[ "snd_config" ] );
            data()->m_terrain_sect =
                ::luabind::object_cast< LPCSTR >( output[ "terrain_sect" ] );

            data()->m_sSupplySpawn =
                ::luabind::object_cast< LPCSTR >( output[ "supplies" ] );
            if ( !data()->m_sSupplySpawn.empty() ) {
                xr_string& str = data()->m_sSupplySpawn;
                xr_string::size_type pos = str.find( "\\n" );
                while ( xr_string::npos != pos ) {
            ...
            RankDef().min =
                _min( ::luabind::object_cast< int >( output[ "rank_min" ] ),
                      ::luabind::object_cast< int >( output[ "rank_max" ] ) );
            RankDef().max =
                _max( ::luabind::object_cast< int >( output[ "rank_min" ] ),
                      ::luabind::object_cast< int >( output[ "rank_max" ] ) );

            ReputationDef().min = _min(
                ::luabind::object_cast< int >( output[ "reputation_min" ] ),
                ::luabind::object_cast< int >( output[ "reputation_max" ] ) );
            ReputationDef().max = _max(
                ::luabind::object_cast< int >( output[ "reputation_min" ] ),
                ::luabind::object_cast< int >( output[ "reputation_max" ] ) );

            MoneyDef().min_money =
                _min( ::luabind::object_cast< int >( output[ "money_min" ] ),
                      ::luabind::object_cast< int >( output[ "money_max" ] ) );
            MoneyDef().max_money =
                _max( ::luabind::object_cast< int >( output[ "money_min" ] ),
                      ::luabind::object_cast< int >( output[ "money_max" ] ) );
            MoneyDef().inf_money =
                ::luabind::object_cast< bool >( output[ "money_infinitive" ] );
        }
    }
