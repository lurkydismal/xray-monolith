    ...
    switch ( dbg->event ) {
        case LUA_HOOKCALL: {
            ...
            if ( m_current_stack_level &&
                 lua_getstack( L, 1, m_stack[ m_current_stack_level - 1 ] ) )
    ...
    for ( int j = m_current_stack_level - 1, k = 0; j >= 0; --j, ++k ) {
        ...
        if ( !l_tDebugInfo.name )
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError, "%2d : [%s] %s(%d) : %s",
                k, l_tDebugInfo.what, l_tDebugInfo.short_src,
                l_tDebugInfo.currentline, "" );
        else if ( !xr_strcmp( l_tDebugInfo.what, "C" ) )
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError, "%2d : [C  ] %s", k,
                l_tDebugInfo.name );
        else
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError, "%2d : [%s] %s(%d) : %s",
