...
#define DECLARE_SPECIALIZATION( TO, FROM, METHOD )                             \
    template <>                                                                \
    struct has_dcast< TO*, FROM* > : std::true_type {                          \
        static_assert( std::is_convertible_v<                                  \
                           decltype( std::declval< FROM* >()->METHOD() ),      \
                           TO* >,                                              \
                       "Specialization Error: " #METHOD                        \
                       " does not return " #TO "*" );                          \
...
DECLARE_SPECIALIZATION( IKinematicsAnimated,
                        IRenderVisual,
...
DECLARE_SPECIALIZATION( CSE_ALifeGroupAbstract,
                        CSE_Abstract,
                        cast_group_abstract )
DECLARE_SPECIALIZATION( CSE_ALifeSchedulable, CSE_Abstract, cast_schedulable )
DECLARE_SPECIALIZATION( CSE_ALifeInventoryItem,
                        CSE_Abstract,
                        cast_inventory_item )
DECLARE_SPECIALIZATION( CSE_ALifeTraderAbstract,
                        CSE_Abstract,
...
DECLARE_SPECIALIZATION( CSE_ALifeDynamicObject,
                        CSE_Abstract,
                        cast_alife_dynamic_object )
...
DECLARE_SPECIALIZATION( CSE_ALifeItemDetector,
                        CSE_Abstract,
                        cast_item_detector )
DECLARE_SPECIALIZATION( CSE_ALifeMonsterAbstract,
                        CSE_Abstract,
                        cast_monster_abstract )
DECLARE_SPECIALIZATION( CSE_ALifeHumanAbstract,
                        CSE_Abstract,
                        cast_human_abstract )
DECLARE_SPECIALIZATION( CSE_ALifeAnomalousZone,
                        CSE_Abstract,
                        cast_anomalous_zone )
DECLARE_SPECIALIZATION( CSE_ALifeTrader, CSE_Abstract, cast_trader )
DECLARE_SPECIALIZATION( CSE_ALifeCreatureAbstract,
                        CSE_Abstract,
                        cast_creature_abstract )
DECLARE_SPECIALIZATION( CSE_ALifeSmartZone, CSE_Abstract, cast_smart_zone )
DECLARE_SPECIALIZATION( CSE_ALifeOnlineOfflineGroup,
                        CSE_Abstract,
                        cast_online_offline_group )
...
DECLARE_SPECIALIZATION( CWeaponMagazined,
                        CInventoryItem,
                        cast_weapon_magazined )
DECLARE_SPECIALIZATION( CWeaponKnife, CInventoryItem, cast_weapon_knife )
DECLARE_SPECIALIZATION( CWeaponBinoculars,
                        CInventoryItem,
                        cast_weapon_binoculars )
DECLARE_SPECIALIZATION( CWeaponMagazinedWGrenade,
                        CInventoryItem,
                        cast_weapon_magazined_w_grenade )
...
DECLARE_SPECIALIZATION( CGrenadeLauncher,
                        CInventoryItem,
                        cast_addon_grenade_launcher )
...
DECLARE_SPECIALIZATION( CExplosiveRocket,
                        CPhysicsShellHolder,
                        cast_explosive_rocket )
...
DECLARE_SPECIALIZATION( CUsableScriptObject,
                        CGameObject,
                        cast_usable_script_object )
...
DECLARE_SPECIALIZATION( CObject, ISpatial, dcast_CObject )
    ...
    defined( XRRENDER_R3_EXPORTS ) || defined( XRRENDER_R4_EXPORTS )
#include "../xrCDB/ISpatial.h"
...
using clean_type_t = typename std::remove_cv_t<
    std::remove_reference_t< std::remove_pointer_t< _Ty > > >;
...
inline constexpr bool is_smart_ptr_v =
    is_smart_ptr< std::remove_cv_t< std::remove_reference_t< T > > >::value;
...
template <
    ...
    typename = std::enable_if_t<
        !std::is_same< clean_type_t< _To >, clean_type_t< _From& > >::value > >
...
__forceinline _To smart_cast( const _From& ref ) {
    ...
    if constexpr ( is_smart_ptr_v< _From > ) {
        auto casted_ptr =
            smart_cast< _ToPtr >( const_cast< _From* >( ref.get() ) );
        if ( !casted_ptr )
            throw std::bad_cast{};
        return *casted_ptr;
    } else // It's just a regular object/engine-class
...
__forceinline _To smart_cast( _To ptr ) {
    return ptr;
}
