...
//        static doug_lea_allocator	s_allocator( s_fake_array, s_arena_size,
//        "lua" );
...
static void put_function( lua_State* state,
                          u8 const* buffer,
...
#ifndef USE_DEBUGGER
    STATIC_CHECK(
        false, Do_Not_Define_USE_LUA_STUDIO_macro_without_USE_DEBUGGER_macro );
#endif //! USE_DEBUGGER
...
bool LoadKernelScriptToGlobal( lua_State* L, const char* name ) {
    ...
    // FixedFileName = "kernel\\" + FixedFileName; //When this line is enabled
    // all lua files are not being loaded after loading a game
    ...
    if ( FS.exist( FileName, "$game_scripts$", FixedFileName.data() ) ) {
        ...
        if ( luaL_loadbuffer( L, ( const char* )l_tpFileReader->pointer(),
                              l_tpFileReader->length(), NameSpace ) ) {
            lua_settop( L, start );
            return false;
        } else {
            int errFuncId = -1;
            int l_iErrorCode =
                lua_pcall( L, 0, 0, ( -1 == errFuncId ) ? 0 : errFuncId );
            if ( l_iErrorCode ) {
#ifdef DEBUG
...
void CScriptStorage::reinit() {
    ...
    if ( !Core.ParamsData.test( ECoreParams::nojit ) ) {
        luajit::open_lib( lua(), LUA_JITLIBNAME, luaopen_jit );
#ifndef DEBUG
        put_function( lua(), opt_lua_binary, sizeof( opt_lua_binary ),
                      "jit.opt" );
        put_function( lua(), opt_inline_lua_binary, sizeof( opt_lua_binary ),
                      "jit.opt_inline" );
        dojitopt( lua(), "2" );
#endif //! DEBUG
    }
    ...
    if ( Core.ParamsData.test( ECoreParams::_g ) )
        file_header = file_header_new; // AVO: I get fatal crash at the start if
                                       // this is used
    else
...
void CScriptStorage::DebuggerAttach() {
    ...
    int l_iErrorCode =
        luaL_loadbuffer( lua(), S, xr_strlen( S ), "@console_command" );
...
int CScriptStorage::vscript_log( ScriptStorage::ELuaMessageType tLuaMessageType,
                                 LPCSTR caFormat,
                                 va_list marker ) {
#ifndef NO_XRGAME_SCRIPT_ENGINE
#ifdef DEBUG
    if ( !psAI_Flags.test( aiLua ) &&
         ( tLuaMessageType != ScriptStorage::eLuaMessageTypeError ) )
        return ( 0 );
#endif //-DEBUG
#endif //! NO_XRGAME_SCRIPT_ENGINE
...
#ifndef NO_XRGAME_SCRIPT_ENGINE
    // AVO: allow LUA debug prints (i.e.:
    // ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
    // make_string("CWeapon [%s]: cannot access class member
    // Weapon_IsScopeAttached!", object().cNameSect().c_str()).c_str());)
#ifndef DEBUG
    ...
    for ( int i = 0; lua_getstack( L, i, &l_tDebugInfo ); ++i ) {
        ...
        if ( !l_tDebugInfo.name ) {
            script_log_no_stack( ScriptStorage::eLuaMessageTypeError,
                                 "%2d : [%s] %s(%d) : %s", i, l_tDebugInfo.what,
                                 l_tDebugInfo.short_src,
                                 l_tDebugInfo.currentline, "" );
            // script_log(ScriptStorage::eLuaMessageTypeError, "%2d : [%s]
            // %s(%d) : %s", i, l_tDebugInfo.what, l_tDebugInfo.short_src,
            // l_tDebugInfo.currentline, "");
        } else {
            if ( !xr_strcmp( l_tDebugInfo.what, "C" ) ) {
                script_log_no_stack( ScriptStorage::eLuaMessageTypeError,
                                     "%2d : [C  ] %s", i, l_tDebugInfo.name );
                // script_log(ScriptStorage::eLuaMessageTypeError, "%2d : [C  ]
                // %s", i, l_tDebugInfo.name);
            } else {
                script_log_no_stack( ScriptStorage::eLuaMessageTypeError,
                                     "%2d : [%s] %s(%d) : %s", i,
                                     l_tDebugInfo.what, l_tDebugInfo.short_src,
                                     l_tDebugInfo.currentline,
                                     l_tDebugInfo.name );
                // script_log(ScriptStorage::eLuaMessageTypeError, "%2d : [%s]
                // %s(%d) : %s", i, l_tDebugInfo.what, l_tDebugInfo.short_src,
...
int __cdecl CScriptStorage::script_log_no_stack(
    ScriptStorage::ELuaMessageType tLuaMessageType,
    LPCSTR caFormat,
    ... ) {
    va_list marker;
...
int __cdecl CScriptStorage::script_log(
    ScriptStorage::ELuaMessageType tLuaMessageType,
    LPCSTR caFormat,
    ... ) {
    va_list marker;
...
bool CScriptStorage::parse_namespace( LPCSTR caNamespaceName,
                                      LPSTR b,
                                      u32 const b_size,
                                      LPSTR c,
                                      u32 const c_size ) {
    ...
    *c = 0;
    ...
    for ( int i = 0;; ++i ) {
        if ( !xr_strlen( S ) ) {
            script_log( ScriptStorage::eLuaMessageTypeError,
                        "the namespace name %s is incorrect!",
                        caNamespaceName );
            return ( false );
        }
        LPSTR S1 = strchr( S, '.' );
...
bool CScriptStorage::load_buffer( lua_State* L,
                                  LPCSTR caBuffer,
                                  size_t tSize,
                                  LPCSTR caScriptName,
    ...
    if ( caNameSpaceName && xr_strcmp( "_G", caNameSpaceName ) ) {
        ...
        if ( !parse_namespace( caNameSpaceName, a, sizeof( a ), b,
                               sizeof( b ) ) )
            return ( false );
        ...
        __try {
...
#ifdef DEBUG
                script =
                    ( LPSTR )Memory.mem_alloc( total_size, "lua script file" );
#else  //! DEBUG
            ...
            R_ASSERT2( errcode,
                       ...
                       "exception!" );
#ifdef DEBUG
            script = ( LPSTR )Memory.mem_alloc(
                total_size, "lua script file (after exception)" );
#else  // #ifdef DEBUG
            script = ( LPSTR )Memory.mem_alloc( total_size );
#endif // #ifdef DEBUG
            dynamic_allocation = true;
        ...
        l_iErrorCode =
            luaL_loadbuffer( L, script, tSize + str_len, caScriptName );
...
static xr_string join_list( const xr_vector< xr_string >& items_vec,
                            xr_string delim = "\n" ) {
    xr_string ret;
...
static bool unlocalRegex( xr_set< xr_string >& unlocals,
                          xr_string& s,
                          const std::regex& pattern,
                          const int group,
                          const xr_string& replacement ) {
    if ( std::regex_match( s, pattern ) ) {
        ...
        if ( unlocals.find( variable ) != unlocals.end() ) {
            Msg( "[unlocalRegex] found variable %s to unlocal",
                 variable.c_str() );
...
bool CScriptStorage::do_file( LPCSTR caScriptName, LPCSTR caNameSpaceName ) {
    if ( !unlocalizerPassed ) {
        auto file_list = FS.file_list_open( "$game_config$", "unlocalizers\\",
                                            FS_RootOnly | FS_ListFiles );
        if ( !file_list ) {
            ...
            for ( ; i != e; ++i ) {
                ...
                FS.update_path(
                    file_name, "$game_config$",
                    ( xr_string( "unlocalizers\\" ) + id ).c_str() );
                ...
                for ( ; i != e; ++i ) {
                    auto sectionName =
                        xr_string( ( *i ).Name.c_str() ).ToLowerCase();
                    if ( unlocalizers.find( sectionName ) ==
                         unlocalizers.end() ) {
                        // construct set that contains top level variables to
                        // delocalize by section name
                        unlocalizers[ sectionName ].clear();
                        Msg( "creating unlocalizer for script %s",
                             sectionName.c_str() );
                    }
                    auto& data = ( *i ).Data;
                    for ( auto& item : data ) {
                        unlocalizers[ sectionName ].insert(
                            xr_string( item.first.c_str() ) );
                        Msg( "adding variable %s for unlocalizer for script %s",
                             item.first.c_str(), sectionName.c_str() );
                    }
                }
                xr_delete( config );
            }
    ...
    if ( !l_tpFileReader ) {
        script_log( eLuaMessageTypeError, "Cannot open file \"%s\"",
                    caScriptName );
        return ( false );
    }
    ...
    if ( unlocalizers.find( loweredNameSpaceName ) != unlocalizers.end() ) {
        ...
        for ( auto& s : tokens ) {
            ...
            // local a = ...
            // local a
            // local a,b,c = ... (if one of a,b,c is in unlocalizers list - all
            // of them will be unlocalized) local x; local y; - unsupported yet
            ...
            if ( std::regex_match( s, pattern ) ) {
                ...
                auto variables =
                    variablesAndValues[ 0 ].SplitStringMulti( "," );
                    ...
                    if ( unlocals.find( v ) != unlocals.end() ) {
                        ...
                        if ( !hasValue ) {
                            ...
                            if ( std::regex_match( s, r ) ) {
                                ...
                                s = xr_string(
                                    ( std::string( noncomments[ 1 ] ) +
                                      "= nil " +
                                      std::string( noncomments[ 2 ] ) )
                                        .c_str() );
                            } else {
                                s += " = nil";
                            }
    ...
    if ( unlocalPerformed ) {
        bufferLoaded = load_buffer( lua(), scriptContents, scriptLength,
                                    l_caLuaFileName, caNameSpaceName );
    } else {
        l_tpFileReader->rewind();
        bufferLoaded = load_buffer(
            lua(), static_cast< LPCSTR >( l_tpFileReader->pointer() ),
            ( size_t )l_tpFileReader->length(), l_caLuaFileName,
            caNameSpaceName );
    }
    ...
    if ( 0 ) //.
    {
        for ( int i = 0; lua_type( lua(), -i - 1 ); i++ )
            Msg( "%2d : %s", -i - 1,
                 lua_typename( lua(), lua_type( lua(), -i - 1 ) ) );
    }

    // because that's the first and the only call of the main chunk - there is
    // no point to compile it
    //	luaJIT_setmode	(lua(),0,LUAJIT_MODE_ENGINE|LUAJIT_MODE_OFF);
    //// Oles
    int l_iErrorCode = lua_pcall(
        lua(), 0, 0, ( -1 == errFuncId ) ? 0 : errFuncId ); // new_Andy
...
bool CScriptStorage::load_file_into_namespace( LPCSTR caScriptName,
                                               LPCSTR caNamespaceName ) {
    int start = lua_gettop( lua() );
...
bool CScriptStorage::namespace_loaded( LPCSTR N, bool remove_from_stack ) {
    ...
    for ( ;; ) {
        ...
        if ( lua_isnil( lua(), -1 ) ) {
            ...
            VERIFY( start == lua_gettop( lua() ) );
            FATAL(
                " Error : the namespace name is already being used by the "
                "non-table object!\n" );
            return ( false );
        }
        lua_remove( lua(), -2 );
...
bool CScriptStorage::object( LPCSTR identifier, int type ) {
    ...
    while ( lua_next( lua(), -2 ) ) {
        if ( ( lua_type( lua(), -1 ) == type ) &&
             !xr_strcmp( identifier, lua_tostring( lua(), -2 ) ) ) {
...
bool CScriptStorage::object( LPCSTR namespace_name,
                             LPCSTR identifier,
                             int type ) {
    int start = lua_gettop( lua() );
    if ( xr_strlen( namespace_name ) &&
         !namespace_loaded( namespace_name, false ) ) {
        VERIFY( lua_gettop( lua() ) == start );
        return ( false );
    }
...
struct raii_guard : private xray::noncopyable {
    ...
    ~raii_guard() {
        ...
        {
#ifdef DEBUG
            static bool const break_on_assert =
                Core.ParamsData.test( ECoreParams::break_on_assert );
#else  //! DEBUG
            static bool const break_on_assert =
                false; // Alundaio: Can't get a proper stack trace with this
                       // enabled
#endif //-DEBUG
            if ( !m_error_code )
                return;
...
bool CScriptStorage::print_output( lua_State* L,
                                   LPCSTR caScriptFileName,
                                   int iErorCode ) {
    if ( iErorCode )
    ...
    if ( !xr_strcmp( S, "cannot resume dead coroutine" ) ) {
        VERIFY2( "Please do not return any values from main!!!",
                 caScriptFileName );
#ifdef USE_DEBUGGER
#ifndef USE_LUA_STUDIO
        if ( ai().script_engine().debugger() &&
             ai().script_engine().debugger()->Active() ) {
        ...
        if ( !iErorCode )
            script_log( ScriptStorage::eLuaMessageTypeInfo, "Output from %s",
                        caScriptFileName );
        script_log( iErorCode ? ScriptStorage::eLuaMessageTypeError
                              : ScriptStorage::eLuaMessageTypeMessage,
                    "%s", S );
#ifdef USE_DEBUGGER
#ifndef USE_LUA_STUDIO
        if ( ai().script_engine().debugger() &&
...
void CScriptStorage::print_error( lua_State* L, int iErrorCode ) {
    switch ( iErrorCode ) {
        ...
        case LUA_YIELD: {
            script_log( ScriptStorage::eLuaMessageTypeInfo,
                        "Thread is yielded" );
            break;
        }
        default:
...
void CScriptStorage::flush_log() {
    ...
    strconcat( sizeof( log_file_name ), log_file_name, Core.ApplicationName,
               "_", Core.UserName, "_lua.log" );
