...
static void initialize_lua_studio( lua_State* state,
                                   cs::lua_studio::world*& world,
                                   lua_studio_engine*& engine ) {
    engine = 0;
    world = 0;
    ...
    if ( !s_script_debugger_handle ) {
        Msg( "! cannot load %s dynamic library",
    ...
    s_create_world = ( create_world_function_type )GetProcAddress(
        s_script_debugger_handle, "_cs_lua_studio_backend_create_world@12" );
    R_ASSERT2( s_create_world,
               "can't find function \"cs_lua_studio_backend_create_world\"" );

    s_destroy_world = ( destroy_world_function_type )GetProcAddress(
        s_script_debugger_handle, "_cs_lua_studio_backend_destroy_world@4" );
    R_ASSERT2( s_destroy_world,
               "can't find function \"cs_lua_studio_backend_destroy_world\" in "
               "the library" );
...
static void finalize_lua_studio( lua_State* state,
                                 cs::lua_studio::world*& world,
                                 lua_studio_engine*& engine ) {
    world->remove( state );
...
xr_vector< xr_string > get_lua_stack( lua_State* L ) {
    ...
    for ( int i = 0; lua_getstack( L, i, &l_tDebugInfo ); ++i ) {
        ...
        if ( !l_tDebugInfo.name ) {
            res.push_back( make_string( "%2d : [%s] %s(%d) : %s", i,
                                        l_tDebugInfo.what,
            ...
            if ( !xr_strcmp( l_tDebugInfo.what, "C" ) ) {
                res.push_back(
                    make_string( "%2d : [C  ] %s", i, l_tDebugInfo.name )
                        .c_str() );
            } else {
                res.push_back(
                    make_string( "%2d : [%s] %s(%d) : %s", i, l_tDebugInfo.what,
                                 l_tDebugInfo.short_src,
...
void CScriptEngine::lua_error( lua_State* L ) {
    ...
    auto error_str =
        make_string( "\n%s\n\nLUA error: %s\n\nCheck log for details",
                     lua_error_line.c_str(), lua_tostring( L, -1 ) );
...
void lua_cast_failed( lua_State* L, LUABIND_TYPE_INFO info ) {
    ...
    Debug.fatal( DEBUG_INFO, "LUA error: cannot cast lua value to %s",
                 info->name() );
}
...
void CScriptEngine::setup_callbacks() {
...
#endif
    {
        ...
        ::luabind::set_error_callback_not_crash(
            CScriptEngine::lua_error_not_crash );
#endif
...
#endif // #ifndef USE_LUA_STUDIO
       //	lua_sethook
       //(lua(), lua_hook_call,	LUA_MASKLINE|LUA_MASKCALL|LUA_MASKRET,	0);
...
void CScriptEngine::remove_script_process(
    const EScriptProcessors& process_id ) {
...
void CScriptEngine::process_file_if_exists( LPCSTR file_name,
    ...
    string_path S, S1;
    if ( m_reload_modules ||
         ( *file_name && !namespace_loaded( file_name ) ) ) {
        FS.update_path( S, "$game_scripts$",
        ...
        if ( !warn_if_not_exist && !FS.exist( S ) ) {
            ...
            {
                print_stack();
                Msg( "* trying to access variable %s, which doesn't exist, or "
                     "to load script %s, which doesn't exist too",
                     file_name, S );
                m_stack_is_ready = true;
            }
#endif
...
void CScriptEngine::register_script_classes() {
    ...
    m_class_registrators = READ_IF_EXISTS( l_tpIniFile, r_string, "common",
                                           "class_registrators", "" );
    xr_delete( l_tpIniFile );
    ...
    for ( u32 i = 0; i < n; ++i ) {
        ...
        if ( !functor( I, result ) ) {
            script_log( eLuaMessageTypeError,
                        "Cannot load class registrator %s!", I );
            continue;
        }
...
bool CScriptEngine::function_object( LPCSTR function_to_call,
    ...
    parse_script_namespace( function_to_call, name_space, sizeof( name_space ),
