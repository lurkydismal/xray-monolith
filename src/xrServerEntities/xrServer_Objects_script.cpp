...
struct CWrapperBase : public T, public ::luabind::wrap_base {
    ...
    static void STATE_Write_static( inherited* ptr, NET_Packet* p1 ) {
        Log(
            "Attempt to call pure virtual method STATE_Write in CSE_Abstract" );
    ...
    static void UPDATE_Read_static( inherited* ptr, NET_Packet* p1 ) {
        Log(
            "Attempt to call pure virtual method UPDATE_Read in CSE_Abstract" );
    ...
    static void UPDATE_Write_static( inherited* ptr, NET_Packet* p1 ) {
        Log( "Attempt to call pure virtual method UPDATE_Write in "
             "CSE_Abstract" );
    ...
    module( L )
        [ class_< IPureLoadableObject< IReader > >( "ipure_alife_load_object" ),
          ...
          class_< IPureSerializeObject< IReader, IWriter >,
                  bases< IPureLoadableObject< IReader >,
                         IPureSavableObject< IWriter > > >(
...
void CSE_Abstract::script_register( lua_State* L ) {
    ...
    module(
        L )[ class_< CSE_Abstract, WrapType, CPureServerObject >(
                 "cse_abstract" )
                 ...
                 .def( "STATE_Read", &BaseType::STATE_Read,
                       &WrapType::STATE_Read_static )
                 .def( "STATE_Write", &BaseType::STATE_Write,
                       &WrapType::STATE_Write_static )
                 .def( "UPDATE_Read", &BaseType::UPDATE_Read,
                       &WrapType::UPDATE_Read_static )
                 .def( "UPDATE_Write", &BaseType::UPDATE_Write,
...
void CSE_Spectator::script_register( lua_State* L ) {
    module( L )[ luabind_class_abstract1( CSE_Spectator, "cse_spectator",
                                          CSE_Abstract ) ];
}

void CSE_Temporary::script_register( lua_State* L ) {
    module( L )[ luabind_class_abstract1( CSE_Temporary, "cse_temporary",
                                          CSE_Abstract ) ];
}
