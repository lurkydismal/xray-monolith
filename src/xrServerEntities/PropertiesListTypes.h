...
public:
    // base events
    typedef xr_delegate< void( PropValue* ) > TOnChange;
...
public:
    typedef xr_delegate< void( PropValue*, T& ) > TOnBeforeEditEvent;
    typedef xr_delegate< bool( PropValue*, T& ) > TOnAfterEditEvent;
...
public:
    ...
    IC xr_string GetDrawText() {
        VERIFY( !values.empty() );
        return m_Flags.is( flMixed )
                   ? xr_string( "(mixed)" )
                   : values.front()->GetDrawText( OnDrawTextEvent );
    }
...
public:
    typedef xr_delegate< void( CanvasValue*, CanvasValue*, bool& ) >
        TOnTestEqual;
    typedef xr_delegate<
        void( CanvasValue*, void* /* TCanvas* */, const Irect& ) >
        TOnDrawCanvasEvent;
...
public:
    CanvasValue( const shared_str& val, int h )
...
public:
    RStringVec value;
    int btn_num;
    typedef xr_delegate< void( ButtonValue*, bool&, bool& ) > TOnBtnClick;
    TOnBtnClick OnBtnClickEvent;

    enum { flFirstOnly = ( 1 << 0 ) };
...
public:
    typedef xr_delegate< void( ShortcutValue*, const xr_shortcut&, bool& ) >
        TOnValidateResult;
...
public:
    typedef xr_delegate< void( PropValue*, xr_string& ) > TOnBeforeEditEvent;
    typedef xr_delegate< bool( PropValue*, xr_string& ) > TOnAfterEditEvent;
    TOnBeforeEditEvent OnBeforeEditEvent;
    TOnAfterEditEvent OnAfterEditEvent;
...
public:
    CTextValue( LPSTR val, int _lim )
        : value( val ), init_value( val ), lim( _lim ) {
        OnBeforeEditEvent = 0;
        OnAfterEditEvent = 0;
    };
...
public:
    ...
    typedef xr_delegate< void( ChooseValue* ) > TOnChooseValueFill;
...
public:
    ChooseValue( shared_str* val,
        ...
        : RTextValue( val ),
          m_ChooseID( cid ),
          m_StartPath( path ),
          subitem( sub_item_count ),
          m_Items( 0 ),
          m_FillParam( param ),
          OnChooseFillEvent( 0 ),
          OnDrawThumbnailEvent( 0 ),
          m_ChooseFlags( choose_flags ) {}
};
...
public:
    VectorValue( Fvector* val,
                 float mn,
                 float mx,
                 float increment,
                 int decimal )
        : NumericValue< Fvector >( val ) {
        lim_mn.set( mn, mn, mn );
        lim_mx.set( mx, mx, mx );
...
public:
    ...
    virtual bool Equal( PropValue* val ) {
        return !!this->value->equal( *( ( FlagValue< T >* )val )->value, mask );
    }

    virtual const T& GetValue() { return *this->value; }

    virtual void ResetValue() {
        this->value->set( mask, this->init_value.is( mask ) );
    }
...
public:
    TokenValue( T* val, xr_token* _token )
        : TokenValueCustom( _token ), CustomValue< T >( val ) {};
...
public:
    RTokenValue( T* val, xr_rtoken* _token, u32 _t_cnt )
        : CustomValue< T >( val ), RTokenValueCustom( _token, _t_cnt ) {};
...
public:
    TokenValueSH( u32* val, const Item* _items, u32 _cnt )
        : CustomValue< u32 >( val ), cnt( _cnt ), items( _items ) {};
...
public:
    RListValue( shared_str* val, shared_str* _items, u32 cnt )
        : RTextValue( val ), items( _items ), item_count( cnt ) {};
...
public:
    CListValue( LPSTR val, u32 sz, xr_string* _items, u32 cnt )
        : CTextValue( val, sz ), items( _items ), item_count( cnt ) {};
