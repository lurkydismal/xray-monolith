...
// Reutskiy and Dmitriy Iassenev 	Description : Server base object
////////////////////////////////////////////////////////////////////////////
...
u16 script_server_object_version() {
    ...
    if ( !initialized ) {
        initialized = true;
        if ( !pSettings->section_exist( script_section ) ||
             !pSettings->line_exist( script_section, current_version ) )
            script_version = 0;
...
CSE_Abstract::CSE_Abstract( LPCSTR caSection ) {
    ...
    if ( pSettings->line_exist( caSection, "custom_data" ) ) {
        pcstr const raw_file_name =
            pSettings->r_string( caSection, "custom_data" );
...
#pragma warning( disable : 4238 )
        m_ini_file = xr_new< CInifile >(
            &IReader( ( void* )( *( m_ini_string ) ), m_ini_string.size() ),
...
void CSE_Abstract::Spawn_Write( NET_Packet& tNetPacket, BOOL bLocal ) {
    ...
    u16 client_data_size =
        ( u16 )client_data.size(); // не может быть больше 256 байт
    tNetPacket.w_u16( client_data_size );
    //	Msg
    //("SERVER:saving:save:%d bytes:%d:%s",client_data_size,ID,s_name_replace ?
    // s_name_replace : "");
...
static enum EGameTypes {
    ...
    GAME_DEATHMATCH = 2,
    //	GAME_CTF							= 3,
    //	GAME_ASSAULT						= 4,	// Team1
    //- assaulting, Team0 - Defending
    GAME_CS = 5,
...
BOOL CSE_Abstract::Spawn_Read( NET_Packet& tNetPacket ) {
    ...
    if ( m_wVersion > 70 ) {
        u16 client_data_size =
            ( m_wVersion > 93 ) ? tNetPacket.r_u16() : tNetPacket.r_u8();
        // не может быть больше 256 байт
        if ( client_data_size > 0 ) {
            //			Msg
            //("SERVER:loading:load:%d
            // bytes:%d:%s",client_data_size,ID,s_name_replace ? s_name_replace
            // :
            //"");
    ...
    if ( m_wVersion < 112 ) {
        ...
        if ( m_wVersion > 83 ) {
            ...
            xr_string temp;
            tNetPacket.r_stringZ(
                temp );         // tNetPacket.r_stringZ(m_spawn_control);
...
void CSE_Abstract::load( NET_Packet& tNetPacket ) {
    ...
    u16 client_data_size =
        ( m_wVersion > 93 )
            ? tNetPacket.r_u16()
            : tNetPacket.r_u8(); // не может быть больше 256 байт
    if ( client_data_size > 0 ) {
#ifdef DEBUG
        //		Msg
        //("SERVER:loading:load:%d
        ...
        if ( !client_data.empty() )
            Msg( "CSE_Abstract::load: client_data is cleared for [%d][%s]", ID,
                 name_replace() );
#endif // DEBUG
        client_data.clear();
    }
...
xr_token game_types[] = { { "any_game", eGameIDNoGame },
...
void CSE_Abstract::FillProps( LPCSTR pref, PropItemVec& items ) {
    ...
    #ifdef XRGAME_EXPORTS
    #	ifdef DEBUG
            PHelper().CreateToken8		(items,	PrepareKey(pref,"Game
    Type"),			&s_gameid,		game_types);
        PHelper().CreateU16			(items,	PrepareKey(pref,
    "Respawn Time (s)"),	&RespawnTime,	0,43200);
...
{
        tNetPacket.w
(&m_spawn_count,sizeof(m_spawn_count)); tNetPacket.w
(&m_last_spawn_time,sizeof(m_last_spawn_time)); tNetPacket.w
(&m_next_spawn_time,sizeof(m_next_spawn_time));
}

void CSE_Abstract::load_update				(NET_Packet &tNetPacket)
{
        tNetPacket.r
(&m_spawn_count,sizeof(m_spawn_count)); tNetPacket.r
(&m_last_spawn_time,sizeof(m_last_spawn_time)); tNetPacket.r
(&m_next_spawn_time,sizeof(m_next_spawn_time));
}
/**/
