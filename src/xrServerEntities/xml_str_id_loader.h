...
public:
    ...
    static const ITEM_DATA* GetById( const shared_str& str_id,
                                     bool no_assert = false );
    static const ITEM_DATA* GetByIndex( int index, bool no_assert = false );

    static const int IdToIndex( const shared_str& str_id,
                                int default_index = T_INDEX( -1 ),
                                bool no_assert = false ) {
        const ITEM_DATA* item = GetById( str_id, no_assert );
        return item ? item->index : default_index;
    }

    static const shared_str IndexToId( int index,
                                       shared_str default_id = NULL,
                                       bool no_assert = false ) {
...
TEMPLATE_SPECIALIZATION
const typename ITEM_DATA* CSXML_IdToIndex::GetById( const shared_str& str_id,
                                                    bool no_assert ) {
...
TEMPLATE_SPECIALIZATION
const typename ITEM_DATA* CSXML_IdToIndex::GetByIndex( int index,
                                                       bool no_assert ) {
...
typename void CSXML_IdToIndex::InitInternal() {
    ...
    for ( int it = 0; it < count; ++it ) {
        ...
        for ( int i = 0; i < items_num; ++i ) {
            LPCSTR item_name =
                uiXml->ReadAttrib( uiXml->GetRoot(), tag_name, i, "id", NULL );

            string256 buf;
            xr_sprintf( buf, "id for item don't set, number %d in %s", i,
                        xml_file );
            ...
            R_ASSERT3( m_pItemDataVector->end() == t_it, "duplicate item id",
                       item_name );
