...
LRESULT CScriptDebugger::_SendMessage( u32 message,
                                       WPARAM wParam,
                                       LPARAM lParam ) {
...
LRESULT CScriptDebugger::DebugMessage( UINT nMsg,
                                       WPARAM wParam,
                                       LPARAM lParam ) {
    CMailSlotMsg msg;
...
void CScriptDebugger::LineHook( const char* szFile, int nLine ) {
    ...
    if ( HasBreakPoint( szFile, nLine ) || m_nMode == DMOD_STEP_INTO ||
         m_nMode == DMOD_BREAK ||
         ( m_nMode == DMOD_STEP_OVER && m_nLevel <= 0 ) ||
         ( m_nMode == DMOD_STEP_OUT && m_nLevel < 0 ) ||
         ( m_nMode == DMOD_RUN_TO_CURSOR &&
           xr_strcmp( m_strPathName, szFile ) && m_nLine == nLine ) ) {
        DebugBreak( szFile, nLine );
        GetBreakPointsFromIde();
    }
}

void CScriptDebugger::FunctionHook( const char* szFile,
                                    int nLine,
                                    BOOL bCall ) {
    if ( m_nMode == DMOD_STOP )
        return;
...
void CScriptDebugger::AddStackTrace( const char* szDesc,
                                     const char* szFile,
                                     int nLine ) {
    StackTrace st;
...
void CScriptDebugger::AddGlobalVariable( const char* name,
                                         const char* type,
                                         const char* value ) {
    Variable var;
