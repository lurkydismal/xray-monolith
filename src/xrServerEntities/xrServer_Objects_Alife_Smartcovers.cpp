...
bool parse_bool( ::luabind::object const& table, LPCSTR identifier ) {
    VERIFY2( table.type() == LUA_TTABLE,
             "invalid loophole description passed" );
    ::luabind::object result = table[ identifier ];
    VERIFY2( result.type() != LUA_TNIL,
             make_string( "cannot read boolean value %s", identifier ) );
    VERIFY2( result.type() == LUA_TBOOLEAN,
             make_string( "cannot read boolean value %s", identifier ) );
...
BOOL is_combat_cover( shared_str const& table_id ) {
    ...
    bool result =
        ai().script_engine().function_object( temp, table, LUA_TTABLE );

    VERIFY2( result,
             make_string( "bad or missing description in smart_cover [%s]",
                          table_id.c_str() ) );
    ...
    if ( value.type() == LUA_TNIL ) {
        Msg( "! is_combat_cover flag not found for smart_cover [%s], forcing "
             "to \"true\"",
             table_id.c_str() );
        return ( TRUE );
    }
...
CSE_SmartCover::CSE_SmartCover( LPCSTR section )
    : CSE_ALifeDynamicObject( section ) {
#ifdef XRSE_FACTORY_EXPORTS
    fp_data.inc();
#endif // XRSE_FACTORY_EXPORTS

    m_enter_min_enemy_distance =
        pSettings->r_float( section, "enter_min_enemy_distance" );
    m_exit_min_enemy_distance =
        pSettings->r_float( section, "exit_min_enemy_distance" );
    m_is_combat_cover = pSettings->r_bool( section, "is_combat_cover" );
    m_can_fire =
        m_is_combat_cover ? true : pSettings->r_bool( section, "can_fire" );
...
void CSE_SmartCover::FillProps( LPCSTR pref, PropItemVec& items ) {
    ...
    PHelper().CreateFloat( items,
                           PrepareKey( pref, *s_name, "hold position time" ),
                           &m_hold_position_time, 0.f, 60.f );
    RListValue* value = PHelper().CreateRList(
        items, PrepareKey( pref, *s_name, "description" ), &m_description,
        &*fp_data.smart_covers.begin(), fp_data.smart_covers.size() );
    value->OnChangeEvent.bind( this, &CSE_SmartCover::OnChangeDescription );

    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "enter min enemy distance" ),
        &m_enter_min_enemy_distance, 0.f, 100.f );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "exit min enemy distance" ),
        &m_exit_min_enemy_distance, 0.f, 100.f );

    if ( is_combat_cover( m_description ) ) {
        PHelper().CreateBOOL( items,
                              PrepareKey( pref, *s_name, "is combat cover" ),
                              &m_is_combat_cover );
        PHelper().CreateBOOL( items, PrepareKey( pref, *s_name, "can fire" ),
                              &m_can_fire );
    }
...
LPCSTR parse_string( ::luabind::object const& table, LPCSTR identifier ) {
    ...
    ::luabind::object result = table[ identifier ];
    VERIFY2( result.type() != LUA_TNIL,
             make_string( "cannot read string value %s", identifier ) );
    VERIFY2( result.type() == LUA_TSTRING,
             make_string( "cannot read string value %s", identifier ) );
...
Fvector parse_fvector( ::luabind::object const& table, LPCSTR identifier ) {
    ...
    ::luabind::object result = table[ identifier ];
    VERIFY2( result.type() != LUA_TNIL,
             make_string( "cannot read vector value %s", identifier ) );
    return ( ::luabind::object_cast< Fvector >( result ) );
}

float parse_float( ::luabind::object const& table,
                   LPCSTR identifier,
    ...
    VERIFY2( lua_result.type() != LUA_TNIL,
             make_string( "cannot read number value %s", identifier ) );
    VERIFY2( lua_result.type() == LUA_TNUMBER,
             make_string( "cannot read number value %s", identifier ) );
    float result = ::luabind::object_cast< float >( lua_result );
    VERIFY2( result >= min_threshold,
             make_string( "invalid read number value %s", identifier ) );
    VERIFY2( result <= max_threshold,
             make_string( "invalid number value %s", identifier ) );
    return ( result );
}

void parse_table( ::luabind::object const& table,
                  LPCSTR identifier,
    ...
    VERIFY2( result.type() != LUA_TNIL,
             make_string( "cannot read table value %s", identifier ) );
    VERIFY2( result.type() == LUA_TTABLE,
             make_string( "cannot read table value %s", identifier ) );
}
...
shared_str parse_vertex( ::luabind::object const& table,
                         LPCSTR identifier,
                         bool const& in ) {
...
void CSE_SmartCover::set_enterable( shared_str const& id ) {
    ...
    xr_vector< SSCDrawHelper >::iterator found = std::find_if(
        m_draw_data.begin(), m_draw_data.end(), id_predicate( id ) );
    // VERIFY2						(found !=
    // m_draw_data.end(), id.c_str());
...
void CSE_SmartCover::check_enterable_loopholes(
    shared_str const& description ) {
    string256 temp;
    xr_strcpy( temp, "smart_covers.descriptions." );
    ...
    bool result =
        ai().script_engine().function_object( temp, transitions, LUA_TTABLE );
    VERIFY2( result, make_string(
                         "bad or missing transitions table in smart_cover [%s]",
                         temp ) );
    ...
    for ( ; I != E; ++I ) {
        ...
        shared_str vertex_0 =
            smart_cover::parse_vertex( table, "vertex0", true );
...
void CSE_SmartCover::fill_visuals() {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( I->animation_id.size() == 0 ) {
            Msg( "cover [%s] doesn't have idle_2_fire animation",
                 I->string_identifier.c_str() );
            return;
        }
        ...
        visual_data tmp;
        tmp.visual = visual;
        tmp.matrix.rotation( I->enter_direction,
...
void draw_frustum( CDUInterface* du,
                   float FOV,
                   float _FAR,
                   float A,
                   Fvector& P,
...
void CSE_SmartCover::load_draw_data() {
    ...
    bool result =
        ai().script_engine().function_object( temp, loopholes, LUA_TTABLE );

    if ( !result ) {
        Msg( "no or invalid smart cover description (bad or missing loopholes "
             "table in smart_cover [%s])",
             temp );
        return;
        //		VERIFY2					(result,
        //make_string("bad or missing loopholes table in smart_cover [%s]",
        //temp));
    }
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( m_available_loopholes.is_valid() ) {
            ...
            for ( ; i != e; ++i ) {
                LPCSTR const loophole_id =
                    ::luabind::object_cast< LPCSTR >( i.key() );
        ...
        if ( H.fov_direction.square_magnitude() < EPS_L ) {
            Msg( "! fov direction for loophole %s is setup incorrectly",
                 H.string_identifier.c_str() );
        ...
        if ( H.enter_direction.square_magnitude() < EPS_L ) {
            Msg( "! enter direction for loophole %s is setup incorrectly",
                 H.string_identifier.c_str() );
        ...
        /*		::luabind::object	transitions;
           ...
           transitions);
                        ...
                        for ( ; I != E; ++I) {
                                ...
                                VERIFY2
           (transition.type() == LUA_TTABLE, "invalid loophole description
           passed"); shared_str			action_from =
           smart_cover::parse_vertex(transition, "action_from", true); if
           (action_from != "idle") continue; shared_str
           action_to = smart_cover::parse_vertex(transition, "action_to",
           false); if (action_to != "fire") continue;

                                ::luabind::object		result;
                                parse_table			(transition,
           "animations", result);
        ...
        */
        ...
        H.animation_id =
            make_string( "loophole_%s_visual", H.string_identifier.c_str() )
                .c_str();
    }
...
void CSE_SmartCover::on_render( CDUInterface* du,
    ...
    for ( ; it != it_e; ++it ) {
        ...
        du->OutText( pos, H.string_identifier.c_str(),
                     color_rgba( 255, 255, 255, 255 ) );

        // du->DrawBox(H.point_position,Fvector().set(0.2f,0.2f,0.2f),TRUE,TRUE,color_rgba(255,0,0,80),color_rgba(0,255,0,255));
        // du->DrawFlag(H.point_position, 0, 1.0f, 1, 1,
        // color_rgba(0,255,0,255), FALSE); du->DrawCylinder(Fidentity, pos,
        // Fvector().set(0.f, 1.f, 0.f), 1.f, .05f, color_rgba(0,255,0,255),
        // color_rgba(0,255,0,255), TRUE, FALSE);

        Fvector dir = H.fov_direction;
        parent.transform_dir( dir );
        Fvector up = parent.j;
        draw_frustum( du, H.fov, H.range, 1.f, pos, dir, up,
