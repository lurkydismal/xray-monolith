...
#ifndef MASTER_GOLD
    ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeMessage,
                                     "%s", caMessage );
#else
    Log( caMessage );
...
struct profile_timer_script {
    ...
    IC float time() const {
        FPU::m64r();
        float result =
            ( float( double( m_accumulator ) / double( CPU::clk_per_second ) ) *
              1000000.f );
        FPU::m24r();
        return ( result );
    }
...
IC profile_timer_script operator+( const profile_timer_script& portion0,
...
// IC	std::ostream& operator<<(std::ostream &stream, profile_timer_script
// &timer)
// {
// 	stream					<< timer.time();
// 	return					(stream);
...
void CScriptEngine::script_register( lua_State* L ) {
    module( L )[
        ...
        def( "log", &LuaLog1 ), def( "print_stack", &PrintStack ),
        ...
        def( "editor", &is_editor ), def( "bit_and", &bit_and ),
        def( "bit_or", &bit_or ), def( "bit_xor", &bit_xor ),
        def( "bit_not", &bit_not ), def( "user_name", &user_name ),
        def( "time_global", &script_time_global ),
        def( "time_global_async", &script_time_global_async ),
        def( "add_scope_radii", &AddScope ),
#ifdef XRGAME_EXPORTS
        def( "device", &get_device ),
        def( "is_enough_address_space_available",
             &is_enough_address_space_available_impl ),
#endif //-XRGAME_EXPORTS
    ...
    // function(L, "is_enough_address_space_available",
    // is_enough_address_space_available_impl); #endif //-XRGAME_EXPORTS
}
