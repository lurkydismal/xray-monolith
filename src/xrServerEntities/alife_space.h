...
#define INVALID_SPAWN_STORY_ID \
    ALife::_SPAWN_STORY_ID( -1 ) ///< Invalid spawn story identifier.
...
enum EWeaponAddonStatus {
    ...
    eAddonPermanent =
        1, ///< The addon is built into the weapon and cannot be detached.
    eAddonAttachable =
        2, ///< The addon may be attached to or detached from the weapon.
};
...
#ifndef _EDITOR
/// Maps hit type names from configuration files to their corresponding EHitType
/// values.
...
/// Defines aliases for a vector of ALife objects that participate in the
/// scheduler and its iterator type.
DEFINE_VECTOR( CSE_ALifeSchedulable*, SCHEDULE_P_VECTOR, SCHEDULE_P_IT );

/// Defines aliases for a map of object IDs to dynamic ALife objects and its
/// iterator type.
DEFINE_MAP( _OBJECT_ID,
            CSE_ALifeDynamicObject*,
            D_OBJECT_P_MAP,
            D_OBJECT_P_PAIR_IT );

/// Defines aliases for a map of story IDs to dynamic ALife objects and its
/// iterator type.
