...
IC CSObjectItemSingle::CObjectItemSingle( const CLASS_ID& clsid,
                                          LPCSTR script_clsid )
...
ObjectFactory::CLIENT_BASE_CLASS* CSObjectItemSingle::client_object() const {
    FATAL(
        "Cannot instantiate client object, because client class is not "
        "declared!" );
    return ( 0 );
}
#endif
...
TEMPLATE_SPECIALIZATION
ObjectFactory::SERVER_BASE_CLASS* CSObjectItemSingle::server_object(
...
IC CObjectItemSingle< _unknown_type, true >::CObjectItemSingle(
...
CObjectItemSingle< _unknown_type, true >::client_object() const {
    return ( xr_new< CLIENT_TYPE >()->_construct() );
}

template < typename _unknown_type >
ObjectFactory::SERVER_BASE_CLASS*
CObjectItemSingle< _unknown_type, true >::server_object(
    LPCSTR section ) const {
    FATAL(
        "Cannot instantiate server object, because server class is not "
        "declared!" );
    return ( 0 );
}
#endif
