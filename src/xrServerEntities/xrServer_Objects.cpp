...
CSE_SpawnGroup::CSE_SpawnGroup				(LPCSTR caSection) :
...
void CSE_SpawnGroup::STATE_Read				(NET_Packet
&tNetPacket, u16 size)
{
        if (m_wVersion < 84)
        ...
        if (m_wVersion > 80) {
                if (m_wVersion < 84) {
                        ...
                        if (m_wVersion < 85) {
                                tNetPacket.r_u64
(m_min_spawn_interval); tNetPacket.r_u64		(m_max_spawn_interval);
                        }
                }
        }
}

void CSE_SpawnGroup::STATE_Write			(NET_Packet
&tNetPacket)
{
}

void CSE_SpawnGroup::UPDATE_Read			(NET_Packet
&tNetPacket)
{
}

void CSE_SpawnGroup::UPDATE_Write			(NET_Packet
&tNetPacket)
{
}

#ifndef XRGAME_EXPORTS
void CSE_SpawnGroup::FillProps				(LPCSTR pref,
PropItemVec& values)
{
        inherited::FillProps		(pref,values);
        PHelper().CreateFlag32
(values,PrepareKey(pref,*s_name,"Spawn\\spawn single item only"),
&m_spawn_flags,	flSpawnSingleItemOnly);
}
#endif // #ifndef XRGAME_EXPORTS
/**/
...
CSE_AbstractVisual::CSE_AbstractVisual( LPCSTR section )
    : inherited1( section ), inherited2( section ) {}
