...
IC void CScriptEngine::add_script_process( const EScriptProcessors& process_id,
    ...
    //	CScriptProcessStorage::const_iterator	I =
    // m_script_processes.find(process_id); 	VERIFY
    //(I == m_script_processes.end());
    m_script_processes.insert( std::make_pair( process_id, script_process ) );
}

CScriptProcess* CScriptEngine::script_process(
    const EScriptProcessors& process_id ) const {
    CScriptProcessStorage::const_iterator I =
...
IC void CScriptEngine::parse_script_namespace( LPCSTR function_to_call,
                                               LPSTR name_space,
    ...
    if ( !J )
        ...
        CopyMemory( name_space, function_to_call,
                    u32( J - function_to_call ) * sizeof( char ) );
...
IC bool CScriptEngine::functor(
    ...
    if ( !m_cache_valid ) {
        ...
        // Create cache key
        FunctorCacheKey key{ function_to_call,
                             typeid( _result_type ).hash_code() };

        // Try to find in cache
        auto it = m_functor_cache.find( key );
        if ( it != m_functor_cache.end() ) {
            lua_function =
                ::luabind::object_cast< ::luabind::functor< _result_type > >(
                    it->second );
            return true;
        }
    }
    ...
    lua_function =
        ::luabind::object_cast< ::luabind::functor< _result_type > >( object );
