...
static int win32_acquire_lock( MLOCK_T* sl ) {
    for ( ;; ) {
        ...
        if ( !InterlockedCompareExchange( ( void** )sl, ( void* )1,
                                          ( void* )0 ) )
            return 0;
...
#define PREACTION( M )                           \
    ( ( GLOBALLY_INITIALIZE() || use_lock( M ) ) \
...
#define get_mstate_for( p )                                                \
    ( ( mstate )( ( ( mchunkptr )( ( char* )( p ) + ( chunksize( p ) ) ) ) \
                      ->prev_foot ^                                        \
                  mparams.magic ) )
    ...
    if ( is_small( size ) ) {
        ...
        if ( treemap_is_marked( m, tidx ) ) {
            ...
            while ( t != 0 && chunksize( t ) != size ) {
                t = t->child[ ( sizebits >> ( SIZE_T_BITSIZE - SIZE_T_ONE ) ) &
                              1 ];
                sizebits <<= 1;
            }
            if ( t != 0 ) {
    ...
    {                                                                          \
        ...
        if ( F == B )                                                          \
            clear_smallmap( M, I );                                            \
                      ...
                      ( F == smallbin_at( M, I ) || ok_address( M, F ) ) &&    \
    ...
    if ( MORECORE_CONTIGUOUS && !use_noncontiguous( m ) ) {
        ...
        if ( ss == 0 ) { /* First time through or recovery */
            ...
            if ( base != CMFAIL ) {
                ...
                /* Can't call MORECORE if size is negative when treated as
                 * signed */
             ...
             */
            asize = granularity_align( nb - m->topsize + TOP_FOOT_SIZE +
                                       SIZE_T_ONE );
        ...
        if ( tbase == CMFAIL ) {  /* Cope with partial failure */
            if ( br != CMFAIL ) { /* Try to use/extend the space we did get */
                if ( asize < HALF_MAX_SIZE_T &&
                    ...
                    size_t esize = granularity_align( nb + TOP_FOOT_SIZE +
                                                      SIZE_T_ONE - asize );
        ...
        if ( m->topsize > pad ) {
            ...
            if ( !is_extern_segment( sp ) ) {
                if ( is_mmapped_segment( sp ) ) {
                    ...
                    if ( extra >=
                         HALF_MAX_SIZE_T ) /* Avoid wrapping negative */
...
static void* internal_memalign( mstate m, size_t alignment, size_t bytes ) {
    ...
    if ( alignment <
         MIN_CHUNK_SIZE ) /* must be at least a minimum chunk size */
                ...
                */
                char* br = ( char* )mem2chunk(
                    ( size_t )( ( ( size_t )( mem + alignment - SIZE_T_ONE ) ) &
                                -alignment ) );
                char* pos =
                    ( ( size_t )( br - ( char* )( p ) ) >= MIN_CHUNK_SIZE )
                        ? br
                        : br + alignment;
                ...
                if ( is_mmapped(
                         p ) ) { /* For mmapped chunks, just adjust offset */
    ...
    */
    ...
    if ( !PREACTION( gm ) ) {
        ...
        if ( bytes <= MAX_SMALL_REQUEST ) {
            ...
            binmap_t smallbits;
            nb =
                ( bytes < MIN_REQUEST ) ? MIN_CHUNK_SIZE : pad_request( bytes );
            idx = small_index( nb );
...
mspace create_mspace( size_t capacity, int locked ) {
    ...
    if ( capacity < ( size_t )-( msize + TOP_FOOT_SIZE + mparams.page_size ) ) {
        size_t rs =
            ( ( capacity == 0 ) ? mparams.granularity
