...
void setup_location_types_section( GameGraph::TERRAIN_VECTOR& m_vertex_types,
    ...
    for ( ; I != E; ++I ) {
        ...
        for ( u32 j = 0; j < GameGraph::LOCATION_TYPE_COUNT; ++j )
            terrain_mask.tMask[ j ] =
                GameGraph::_LOCATION_ID( atoi( _GetItem( S, j, I ) ) );
...
void setup_location_types_line( GameGraph::TERRAIN_VECTOR& m_vertex_types,
                                LPCSTR string ) {
    string16 I;
    GameGraph::STerrainPlace terrain_mask;
    terrain_mask.tMask.resize( GameGraph::LOCATION_TYPE_COUNT );

    u32 N = _GetItemCount( string ) / GameGraph::LOCATION_TYPE_COUNT *
            GameGraph::LOCATION_TYPE_COUNT;
    ...
    for ( u32 i = 0; i < N; ) {
        for ( u32 j = 0; j < GameGraph::LOCATION_TYPE_COUNT; ++j, ++i )
            terrain_mask.tMask[ j ] =
                GameGraph::_LOCATION_ID( atoi( _GetItem( string, i, I ) ) );
        m_vertex_types.push_back( terrain_mask );
    }
}

void setup_location_types( GameGraph::TERRAIN_VECTOR& m_vertex_types,
                           CInifile const* ini,
                           LPCSTR string ) {
...
CSE_ALifeTraderAbstract::CSE_ALifeTraderAbstract( LPCSTR caSection ) {
    ...
    m_sCharacterProfile = READ_IF_EXISTS( pSettings, r_string, caSection,
...
#pragma todo( "Dima to Yura, MadMax : Remove that hacks, please!" )
    ...
    // а потом делаем случайный выбор
        ...
        for ( int i = 0; i <= CSpecificCharacter::GetMaxIndex(); i++ ) {
            ...
            for ( std::size_t j = 0; j < spec_char.data()->m_Classes.size();
                  j++ ) {
                if ( char_info.data()->m_Class ==
                     spec_char.data()->m_Classes[ j ] ) {
                    class_found = true;
                    break;
                }
            }
            if ( !char_info.data()->m_Class.size() || class_found ) {
                // запомнить пподходящий персонаж с флажком
                ...
                if ( char_info.data()->m_Rank == NO_RANK ||
                     _abs( spec_char.Rank() - char_info.data()->m_Rank ) <
                         RANK_DELTA ) {
                    if ( char_info.data()->m_Reputation == NO_REPUTATION ||
                         _abs( spec_char.Reputation() -
                               char_info.data()->m_Reputation ) <
                             REPUTATION_DELTA ) {
#ifdef XRGAME_EXPORTS
                        int* count = NULL;
                        if ( ai().get_alife() )
                            count = ai().alife()
                                        .registry()
                                        .get< CSpecificCharacterRegistry >()
                                        .object( id, true );
        ...
        if ( m_DefaultCharacters.empty() ) {
            if ( !m_CheckedCharacters.empty() ) {
                char_info.m_SpecificCharacterId =
                    m_CheckedCharacters[ Random.randI(
                        m_CheckedCharacters.size() ) ];
...
#ifdef XRGAME_EXPORTS
        if ( m_CheckedCharacters.empty() )
            char_info.m_SpecificCharacterId = m_DefaultCharacters[ Random.randI(
                m_DefaultCharacters.size() ) ];
        else
            char_info.m_SpecificCharacterId = m_CheckedCharacters[ Random.randI(
                m_CheckedCharacters.size() ) ];
#else
        char_info.m_SpecificCharacterId =
...
void CSE_ALifeTraderAbstract::set_specific_character(
    shared_str new_spec_char ) {
    ...
    if ( m_SpecificCharacter.size() ) {
        if ( ai().get_alife() )
            ai().alife().registry().get< CSpecificCharacterRegistry >().remove(
                m_SpecificCharacter, true );
    }
#endif
    ...
    if ( ai().get_alife() ) {
        ...
        int a = 1;
        ai().alife().registry().get< CSpecificCharacterRegistry >().add(
            m_SpecificCharacter, a, true );
    }
#endif
    ...
    if ( NO_COMMUNITY_INDEX == m_community_index ) {
        ...
        CSE_ALifeCreatureAbstract* creature =
            smart_cast< CSE_ALifeCreatureAbstract* >( base() );
        if ( creature )
    ...
    CSE_ALifeMonsterAbstract* monster =
        smart_cast< CSE_ALifeMonsterAbstract* >( base() );
    if ( monster && selected_char.terrain_sect().size() ) {
        setup_location_types_section( monster->m_tpaTerrain, pSettings,
                                      *( selected_char.terrain_sect() ) );
    }
    //----
    if ( NO_RANK == m_rank ) {
        if ( selected_char.RankDef().min != selected_char.RankDef().max )
            m_rank = ::Random.randI( selected_char.RankDef().min,
                                     selected_char.RankDef().max );
        else
            m_rank = selected_char.RankDef().max;
    }

    if ( NO_REPUTATION == m_reputation ) {
        if ( selected_char.ReputationDef().min !=
             selected_char.ReputationDef().max )
            m_reputation = ::Random.randI( selected_char.ReputationDef().min,
                                           selected_char.ReputationDef().max );
        else
...
void CSE_ALifeCustomZone::FillProps( LPCSTR pref, PropItemVec& items ) {
    ...
    PHelper().CreateU32(
        items, PrepareKey( pref, *s_name, "on/off mode\\Shift time (sec)" ),
        &m_start_time_shift, 0, 100000 );
    PHelper().CreateU32(
        items, PrepareKey( pref, *s_name, "on/off mode\\Enabled time (sec)" ),
        &m_enabled_time, 0, 100000 );
    PHelper().CreateU32(
        items, PrepareKey( pref, *s_name, "on/off mode\\Disabled time (sec)" ),
        &m_disabled_time, 0, 100000 );
}
#endif // #ifndef XRGAME_EXPORTS
...
void CSE_ALifeAnomalousZone::FillProps( LPCSTR pref, PropItemVec& items ) {
    inherited::FillProps( pref, items );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "offline interactive radius" ),
        &m_offline_interactive_radius, 0.f, 100.f );
    PHelper().CreateU16(
        items,
        PrepareKey( pref, *s_name, "ALife\\Artefact spawn places count" ),
        &m_artefact_spawn_count, 32, 256 );
    PHelper().CreateFlag32(
        items, PrepareKey( pref, *s_name, "ALife\\Visible for AI" ), &m_flags,
        flVisibleForAI );
}
#endif // #ifndef XRGAME_EXPORTS
...
void CSE_ALifeZoneVisual::FillProps( LPCSTR pref, PropItemVec& values ) {
    ...
    PHelper().CreateChoose(
        values, PrepareKey( pref, abstract->name(), "Attack animation" ),
        &attack_animation, smSkeletonAnims, 0, ( void* )*visual_name );
}
#endif // #ifndef XRGAME_EXPORTS
//-------------------------------------------------------------------------
////////////////////////////////////////////////////////////////////////////
...
CSE_ALifeCreatureAbstract::CSE_ALifeCreatureAbstract( LPCSTR caSection )
    ...
    m_ef_weapon_type = READ_IF_EXISTS( pSettings, r_u32, caSection,
                                       "ef_weapon_type", u32( -1 ) );
    m_ef_detector_type = READ_IF_EXISTS( pSettings, r_u32, caSection,
                                         "ef_detector_type", u32( -1 ) );
...
void CSE_ALifeCreatureAbstract::on_death( CSE_Abstract* killer ) {
    ...
    m_game_death_time = ai().get_alife() ? alife().time_manager().game_time()
                                         : Level().GetGameTime();
    fHealth = -1.f;
}
...
void CSE_ALifeCreatureAbstract::FillProps( LPCSTR pref, PropItemVec& items ) {
    inherited::FillProps( pref, items );
    PHelper().CreateU8( items, PrepareKey( pref, *s_name, "Team" ), &s_team, 0,
                        64, 1 );
    PHelper().CreateU8( items, PrepareKey( pref, *s_name, "Squad" ), &s_squad,
                        0, 64, 1 );
    PHelper().CreateU8( items, PrepareKey( pref, *s_name, "Group" ), &s_group,
                        0, 64, 1 );
    PHelper().CreateFloat( items,
                           PrepareKey( pref, *s_name, "Personal", "Health" ),
                           &fHealth, 0, 2, 5 );
}
#endif // #ifndef XRGAME_EXPORTS
...
void CSE_ALifeCreatureAbstract::set_killer_id(
    ALife::_OBJECT_ID const killer_id ) {
...
CSE_ALifeMonsterAbstract::CSE_ALifeMonsterAbstract( LPCSTR caSection )
    ...
    m_fCurrentLevelGoingSpeed =
        READ_IF_EXISTS( pSettings, r_float, caSection,
                        "current_level_going_speed", m_fGoingSpeed );

    setup_location_types( m_tpaTerrain, pSettings,
                          pSettings->r_string( caSection, "terrain" ) );
...

    {
        ...
        svector< float, ALife::eHitTypeMax >::iterator B = m_fpImmunityFactors
                                                               .begin(),
                                                       I = B;
        svector< float, ALife::eHitTypeMax >::iterator E =
            m_fpImmunityFactors.end();
        ...
        for ( ; I != E; ++I ) {
            xr_strcpy( S,
                       ALife::g_cafHitType2String( ALife::EHitType( I - B ) ) );
    ...
    if ( pSettings->line_exist( caSection, "retreat_threshold" ) )
        m_fRetreatThreshold =
            pSettings->r_float( caSection, "retreat_threshold" );
    else
    ...
    m_rank = ( pSettings->line_exist( caSection, "rank" ) )
                 ? pSettings->r_s32( caSection, "rank" )
                 : 0;

#ifdef XRGAME_EXPORTS
    m_stay_after_death_time_interval = generate_time(
        1, 1, 1,
        pSettings->r_u32( "monsters_common", "stay_after_death_time_interval" ),
        0, 0 );
#endif // XRGAME_EXPORTS
}
...
CSE_Abstract* CSE_ALifeMonsterAbstract::init() {
    ...
    if ( spawn_ini().section_exist( "alife" ) &&
         spawn_ini().line_exist( "alife", "terrain" ) )
        setup_location_types( m_tpaTerrain, &spawn_ini(),
                              spawn_ini().r_string( "alife", "terrain" ) );
...
void CSE_ALifeMonsterAbstract::FillProps( LPCSTR pref, PropItemVec& items ) {
    ...
    PHelper().CreateFlag32(
        items, PrepareKey( pref, *s_name, "ALife\\No move in offline" ),
        &m_flags, flOfflineNoMove );
    PHelper().CreateFlag32(
        items, PrepareKey( pref, *s_name, "Use smart terrain tasks" ), &m_flags,
        flUseSmartTerrains );

    if ( pSettings->line_exist( s_name, "SpaceRestrictionSection" ) ) {
        LPCSTR gcs = pSettings->r_string( s_name, "SpaceRestrictionSection" );
        PHelper().CreateChoose(
            items, PrepareKey( pref, *s_name, "out space restrictions" ),
            &m_out_space_restrictors, smSpawnItem, 0, ( void* )gcs, 16 );
        PHelper().CreateChoose(
            items, PrepareKey( pref, *s_name, "in space restrictions" ),
            &m_in_space_restrictors, smSpawnItem, 0, ( void* )gcs, 16 );
    }
}
#endif // #ifndef XRGAME_EXPORTS

bool CSE_ALifeMonsterAbstract::need_update( CSE_ALifeDynamicObject* object ) {
    return ( CSE_ALifeSchedulable::need_update( object ) &&
             ( get_health() > EPS_L ) );
}
#ifdef XRGAME_EXPORTS
...
bool CSE_ALifeMonsterAbstract::has_detector() {
    ...
    for ( ; I != E; ++I ) {
        CSE_ALifeItemDetector* detector = smart_cast< CSE_ALifeItemDetector* >(
...
void CSE_ALifeMonsterRat::FillProps( LPCSTR pref, PropItemVec& items ) {
    ...
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Personal", "Field of view" ),
        &fEyeFov, 0, 170, 10 );
    PHelper().CreateFloat( items,
                           PrepareKey( pref, *s_name, "Personal", "Eye range" ),
                           &fEyeRange, 0, 300, 10 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Personal", "Minimum speed" ),
        &fMinSpeed, 0, 10, 0.1f );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Personal", "Maximum speed" ),
        &fMaxSpeed, 0, 10, 0.1f );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Personal", "Attack speed" ),
        &fAttackSpeed, 0, 10, 0.1f );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Personal", "Pursuit distance" ),
        &fMaxPursuitRadius, 0, 300, 10 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Personal", "Home distance" ),
        &fMaxHomeRadius, 0, 300, 10 );
    // morale
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Morale", "Success attack quant" ),
        &fMoraleSuccessAttackQuant, -100, 100, 5 );
    PHelper().CreateFloat( items,
                           PrepareKey( pref, *s_name, "Morale", "Death quant" ),
                           &fMoraleDeathQuant, -100, 100, 5 );
    PHelper().CreateFloat( items,
                           PrepareKey( pref, *s_name, "Morale", "Fear quant" ),
                           &fMoraleFearQuant, -100, 100, 5 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Morale", "Restore quant" ),
        &fMoraleRestoreQuant, -100, 100, 5 );
    PHelper().CreateU16(
        items, PrepareKey( pref, *s_name, "Morale", "Restore time interval" ),
        &u16MoraleRestoreTimeInterval, 0, 65535, 500 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Morale", "Minimum value" ),
        &fMoraleMinValue, -100, 100, 5 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Morale", "Maximum value" ),
        &fMoraleMaxValue, -100, 100, 5 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Morale", "Normal value" ),
        &fMoraleNormalValue, -100, 100, 5 );
    // attack
    PHelper().CreateFloat( items,
                           PrepareKey( pref, *s_name, "Attack", "Hit power" ),
                           &fHitPower, 0, 200, 5 );
    PHelper().CreateU16( items,
                         PrepareKey( pref, *s_name, "Attack", "Hit interval" ),
                         &u16HitInterval, 0, 65535, 500 );
    PHelper().CreateFloat( items,
                           PrepareKey( pref, *s_name, "Attack", "Distance" ),
                           &fAttackDistance, 0, 300, 10 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Attack", "Maximum angle" ),
        &fAttackAngle, 0, 180, 10 );
    PHelper().CreateFloat(
        items, PrepareKey( pref, *s_name, "Attack", "Success probability" ),
        &fAttackSuccessProbability, 0, 100, 1 );
}
#endif // #ifndef XRGAME_EXPORTS

bool CSE_ALifeMonsterRat::bfUseful() {
    return ( !smart_cast< CSE_ALifeGroupAbstract* >( this ) &&
             ( get_health() <= EPS_L ) );
}
...
void CSE_ALifeHumanAbstract::FillProps( LPCSTR pref, PropItemVec& items ) {
    ...
    PHelper().CreateFlag32( items,
                            PrepareKey( pref, *s_name, "Group behaviour" ),
                            &m_flags, flGroupBehaviour );
}
#endif // #ifndef XRGAME_EXPORTS
...
void CSE_ALifeOnlineOfflineGroup::STATE_Read( NET_Packet& tNetPacket,
                                              u16 size ) {
    inherited1::STATE_Read( tNetPacket, size );
...
#ifndef XRGAME_EXPORTS
void CSE_ALifeOnlineOfflineGroup::FillProps( LPCSTR pref,
                                             PropItemVec& values ) {
    inherited1::FillProps( pref, values );
}
#endif // #ifndef XRGAME_EXPORTS
