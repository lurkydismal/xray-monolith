...
struct CSaver {
                ...
                CHelper1< T >::template save_data<
                    object_type_traits::
                        is_base_and_derived_or_same_from_template<
                            IPureSavableObject, T >::value >( data, stream, p );
            }
        }
    };
    ...
    IC static void save_data( const std::pair< T1, T2 >& data,
                              M& stream,
                              const P& p ) {
    ...
    IC static void save_data( const xr_vector< bool >& data,
                              M& stream,
                              const P& p ) {
        stream.w_u32( ( u32 )data.size() );
    ...
    IC static void save_data( const svector< T, size >& data,
                              M& stream,
                              const P& p ) {
        stream.w_u32( ( u32 )data.size() );
    ...
    template < typename T1, typename T2 >
    IC static void save_data( const std::queue< T1, T2 >& data,
                              M& stream,
                              const P& p ) {
    ...
    template < template < typename _1, typename _2 > class T1,
               typename T2,
               typename T3 >
    IC static void save_data( const T1< T2, T3 >& data,
                              M& stream,
                              const P& p,
                              bool ) {
        T1< T2, T3 > temp = data;
    ...
    template < template < typename _1, typename _2, typename _3 > class T1,
               ...
               typename T3,
               typename T4 >
    IC static void save_data( const T1< T2, T3, T4 >& data,
                              M& stream,
                              const P& p,
                              bool ) {
    ...
    template < typename T1, typename T2 >
    IC static void save_data( const xr_stack< T1, T2 >& data,
                              M& stream,
                              const P& p ) {
    ...
    IC static void save_data( const std::priority_queue< T1, T2, T3 >& data,
                              M& stream,
                              const P& p ) {
        save_data( data, stream, p, true );
    }
