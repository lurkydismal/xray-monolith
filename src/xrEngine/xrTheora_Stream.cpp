...
BOOL CTheoraStream::ParseHeaders() {
    ...
    while ( !stateflag ) {
        ...
        while ( ogg_sync_pageout( &o_sync_state, &o_page ) > 0 ) {
            ...
            if ( !header_count &&
                 theora_decode_header( &t_info, &t_comment, &o_packet ) >= 0 ) {
                // it is theora
    ...
    while ( ( header_count && header_count < 3 ) ) {
        ...
        while ( header_count && ( header_count < 3 ) &&
                0 != ( ret = ogg_stream_packetout( &o_stream_state,
                                                   &o_packet ) ) ) {
            if ( ret < 0 ) {
                fprintf(
                    stderr,
    ...
    fpms = ( ( float )t_info.fps_numerator / ( float )t_info.fps_denominator ) /
           1000.f;
            ...
            while ( FALSE == result ) {
                ...
                if ( ogg_stream_packetout( &o_stream_state, &o_packet ) > 0 &&
                    ...
                    if ( d_frame < k_frame ) {
                        ...
                        // AVO: commented out VERIFY to get COC start in Debug -
                        // something is wrong with video stream VERIFY((0 !=
                        // d_frame%key_rate) || (0 == d_frame%key_rate) &&
                        // theora_packet_iskeyframe(&o_packet));
                        continue;
                    }
                    BOOL is_key = theora_packet_iskeyframe( &o_packet );
                    // AVO: commented out VERIFY to get COC start in Debug -
                    // something is wrong with video stream VERIFY((d_frame !=
                    // k_frame) || ((d_frame == k_frame) && is_key));
                    //  real decode
