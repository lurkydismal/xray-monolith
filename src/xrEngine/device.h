...
protected:
    ...
    // AVO:
    CTimer
        frame_timer; // TODO: ïðîâåðèòü, íå äóáëèðóåòñÿ-ëè ñõîæèé òàéìåð (alpet)
    //-AVO
...
public:
    ...
    ICF void remove_from_seq_parallel( const xr_delegate< void() >& delegate ) {
        xr_vector< xr_delegate< void() > >::iterator I =
    ...
    // demonized: Perceivable distance depending on FOV, so that objects will
    // behave normal in binoculars
...
public:
    ...
    bool xr_stdcall on_message( HWND hWnd,
                                UINT uMsg,
                                WPARAM wParam,
                                LPARAM lParam,
                                LRESULT& result );
