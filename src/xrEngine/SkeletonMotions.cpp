...
BOOL motions_value::load( LPCSTR N, IReader* data, vecBones* bones ) {
    ...
    if ( MP ) {
        ...
        u16 part_bone_cnt = 0;
        string128 buf;
        R_ASSERT3( vers <= xrOGF_SMParamsVersion,
                   "Invalid OGF/OMF version:", N );
        ...
        for ( u16 part_i = 0; part_i < part_count; part_i++ ) {
            ...
            while ( !PART )
                PART = m_partition.create();
            MP->r_stringZ( buf, sizeof( buf ) );
            PART->Name = _strlwr( buf );
            PART->bones.resize( MP->r_u16() );

            for ( xr_vector< u32 >::iterator b_it = PART->bones.begin();
                  b_it < PART->bones.end(); b_it++ ) {
                MP->r_stringZ( buf, sizeof( buf ) );
        ...
        if ( part_bone_cnt != ( u16 )bones->size() ) {
            bRes = false;
            Msg( "!Different bone count[%s] [Object: '%d' <-> Motions: '%d']",
                 N, bones->size(), part_bone_cnt );
        }
#else
        VERIFY3( part_bone_cnt == ( u16 )bones->size(),
                 "Different bone count '%s'", N );
#endif
        if ( bRes ) {
        ...
        Debug.fatal( DEBUG_INFO, "Old skinned model version unsupported! (%s)",
                     N );
    }
    if ( !bRes )
        return false;
...
motions_value* motions_container::dock( shared_str key,
                                        IReader* data,
                                        vecBones* bones ) {
...
void motions_container::dump() {
    ...
    for ( u32 k = 0; it != _E; k++, it++ ) {
        ...
        Msg( "#%3d: [%3d/%5d Kb] - %s", k, it->second->m_dwReference,
             it->second->mem_usage() / 1024, it->first.c_str() );
    }
