...
u32 CLAItem::InterpolateBGR( int frame ) {
    ...
    return color_rgba( color_get_B( c ), color_get_G( c ), color_get_R( c ),
                       color_get_A( c ) );
}
    ...
    if ( fs ) {
        ...
        if ( OBJ ) {
            ...
            for ( int count = 1; O; count++ ) {
                ...
                I->Load( *O );
                if ( version == 0 ) {
                    for ( CLAItem::KeyPairIt it = I->Keys.begin();
                          it != I->Keys.end(); it++ )
                        it->second = subst_alpha( bgr2rgb( it->second ),
                                                  color_get_A( it->second ) );
                }
                Items.push_back( I );
                O->close();
...
#ifdef _EDITOR
void ELightAnimLibrary::RemoveObject( LPCSTR _fname,
                                      EItemType type,
                                      bool& res ) {
    if ( TYPE_FOLDER == type ) {
        res = true;
