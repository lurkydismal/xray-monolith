...
SPPInfo& SPPInfo::add( const SPPInfo& ppi ) {
    ...
    if ( ppi.cm_tex1.size() ) {
        if ( cm_tex1.size() ) {
            cm_tex2 = ppi.cm_tex1;
            cm_interpolate =
                1.0f - cm_influence / ( cm_influence + ppi.cm_influence );
        } else {
            cm_tex1 = ppi.cm_tex1;
...
SPPInfo& SPPInfo::lerp( const SPPInfo& def, const SPPInfo& to, float factor ) {
    ...
    pp.noise.intensity = to.noise.intensity; // + (to.noise.intensity -
                                             // def.noise.intensity) * factor;
    pp.noise.grain =
        to.noise.grain; // + (to.noise.grain - def.noise.grain) * factor;
    ...
    pp.cm_influence +=
        def.cm_influence + ( to.cm_influence - def.cm_influence ) * factor;
    pp.cm_interpolate += def.cm_interpolate +
                         ( to.cm_interpolate - def.cm_interpolate ) * factor;
...
CCameraManager::~CCameraManager() {
    for ( EffectorCamIt it = m_EffectorsCam.begin(); it != m_EffectorsCam.end();
          it++ )
        xr_delete( *it );
    for ( EffectorPPIt it = m_EffectorsPP.begin(); it != m_EffectorsPP.end();
          it++ )
        xr_delete( *it );
}
...
void CCameraManager::RemoveHudMotionEffectors() {
    for ( EffectorCamIt it = m_EffectorsCam.begin();
          it != m_EffectorsCam.end(); ) {
    ...
    for ( EffectorCamIt it = m_EffectorsCam_added_deffered.begin();
          it != m_EffectorsCam_added_deffered.end(); ) {
        if ( ( *it )->IsHudMotionEffector() ) {
            OnEffectorReleased( *it );
...
ECamEffectorType CCameraManager::RequestCamEffectorId() {
    ...
    for ( index = ( ECamEffectorType )effCustomEffectorStartID;
          GetCamEffector( index ); index = ( ECamEffectorType )( index + 1 ) ) {
        ;
    }
    return index;
}
...
EEffectorPPType CCameraManager::RequestPPEffectorId() {
    ...
    for ( index = ( EEffectorPPType )effCustomEffectorStartID;
          GetPPEffector( index ); index = ( EEffectorPPType )( index + 1 ) ) {
        ;
    }
    return index;
}
...
void CCameraManager::Update( const Fvector& P,
                             const Fvector& D,
                             const Fvector& N,
                             float fFOV_Dest,
                             float fASPECT_Dest,
    ...
    m_cam_info.fAspect =
        m_cam_info.fAspect * dst + ( fASPECT_Dest * aspect ) * src;
...
void CCameraManager::ApplyDevice( float _viewport_near ) {
    ...
    Device.mProject.build_projection( deg2rad( Device.fFOV ),
                                      m_cam_info.fAspect, _viewport_near,
                                      m_cam_info.fFar );
    Device.mProjectHud.build_projection( deg2rad( psHUD_FOV * 83.f ),
