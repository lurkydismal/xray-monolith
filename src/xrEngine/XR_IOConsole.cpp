...
void CConsole::OutFont( LPCSTR text, float& pos_y ) {
    ...
    if ( str_length > scr_width ) // 1024.0f
    {
        ...
        int ln = 0;
        PSTR one_line =
            ( PSTR )_alloca( ( CONSOLE_BUF_SIZE + 1 ) * sizeof( char ) );

        while ( text[ sz ] &&
                ( ln + sz < CONSOLE_BUF_SIZE - 5 ) ) // перенос строк
        {
...
void CConsole::OnRender() {
    ...
    if ( psDeviceFlags.test( rsCameraPos ) ||
         psDeviceFlags.test( rsStatistic ) || Device.Statistic->errors.size() )
        Device.Statistic->Show();

    if ( !m_hShader_back ) {
        m_hShader_back = xr_new< FactoryPtr< IUIShader > >();
        ( *m_hShader_back )
            ->create( "hud\\default", "ui\\ui_console" ); // "ui\\ui_empty"
    }

    if ( !pFont ) {
        pFont = xr_new< CGameFont >( "hud_font_di",
                                     CGameFont::fsDeviceIndependent );
        pFont->SetHeightI( 0.025f );
    }
    if ( !pFont2 ) {
        pFont2 = xr_new< CGameFont >( "hud_font_di2",
                                      CGameFont::fsDeviceIndependent );
    ...
    if ( ( g_pGameLevel && g_pGameLevel->bReady ) ||
         ( g_pGamePersistent && g_pGamePersistent->m_pMainMenu &&
    ...
    if ( !m_disable_tips && m_tips.size() ) {
        ...
        switch ( m_tips_mode ) {
            ...
            case 2:
                shift_x = scr_x *
                          ( ioc_d + pFont->SizeOf_( m_cur_cmd.c_str() ) + d1 );
                break;
            case 3:
        ...
        {
            pFont->OutI( -1.0f + shift_x, fMaxY + i * LDIST, "%s",
                         ( *itb ).text.c_str() );
    ...
    if ( total_filled > 0 ) {
        ...
        for ( u32 i = ( u32 )scroll_delta; i < total_filled; ++i ) {
            const xr_string& logLine =
                m_log_history.GetLooped( newest_idx - 1 - i );
    ...
    pFont->OutI( 0.95f - 0.03f * qn, fMaxY - 2.0f * LDIST, "[%d]",
                 m_log_line_counter );
...
void CConsole::DrawBackgrounds( bool bGame ) {
    ...
    UIRender->StartPrimitive( 6 + 12 + ( VIEW_TIPS_COUNT + 1 ) * 6 + 12,
                              IUIRender::ptTriList, IUIRender::pttTL );
    ...
    vecTipsEx::iterator ite = m_tips.end();
    for ( ; itb != ite; ++itb ) {
        if ( pFont->SizeOf_( ( *itb ).text.c_str() ) >
             pFont->SizeOf_( max_str ) ) {
    ...
    if ( tips_sz > VIEW_TIPS_COUNT ) {
        ...
        float u_height =
            ( back_height * ( float )VIEW_TIPS_COUNT ) / float( tips_sz );
        if ( u_height < 0.5f * font_h ) {
            u_height = 0.5f * font_h;
        }

        // float u_pos = (back_height - u_height) * float(m_start_tip) /
        // float(tips_sz);
...
void CConsole::ExecuteCommand( LPCSTR cmd_str, bool record_cmd ) {
    ...
    if ( record_cmd ) {
        ...
        if ( m_last_cmd.c_str() == 0 ||
             xr_strcmp( m_last_cmd.c_str(), edt ) != 0 ) {
            Msg( "%s %s", c, edt );
            add_cmd_history( edt );
            m_last_cmd = edt;
            SaveHistory();
...
void CConsole::SelectCommand() {
    ...
    VERIFY( 0 <= m_cmd_history_idx &&
            m_cmd_history_idx < ( int )m_cmd_history.size() );

    vecHistory::reverse_iterator it_rb =
        m_cmd_history.rbegin() + m_cmd_history_idx;
...
IConsole_Command* CConsole::find_next_cmd( LPCSTR in_str,
                                           shared_str& out_str ) {
...
bool CConsole::add_next_cmds( LPCSTR in_str, vecTipsEx& out_v ) {
    ...
    for ( u32 i = cur_count; i < MAX_TIPS_COUNT * 2; ++i ) // fake=protect
    {
        temp._set( cc->Name() );
        bool dup =
            ( std::find( out_v.begin(), out_v.end(), temp ) != out_v.end() );
        if ( !dup ) {
            TipString ts( temp );
            out_v.push_back( ts );
            res = true;
    ...
    for ( ; itb != ite; ++itb ) {
        ...
        if ( name_sz >= in_sz ) {
            ...
            if ( !stricmp( name2.c_str(), in_str ) ) {
                shared_str temp;
                temp._set( name );
                bool dup = ( std::find( out_v.begin(), out_v.end(), temp ) !=
                             out_v.end() );
                if ( !dup ) {
...
void CConsole::select_for_filter( LPCSTR filter_str,
                                  vecTips& in_v,
                                  vecTipsEx& out_v ) {
