...
const u32 clQUERY_TOPLEVEL =
    ( 1 << 4 );                        // get only top level of model box/sphere
...
struct clQueryCollision {
    ...
    IC void AddTri( const Fmatrix& m,
                    const CDB::TRI* one,
                    const Fvector* verts ) {
        clQueryTri T;
...
enum /*ENGINE_API*/ ECollisionFormType { cftObject, cftShape };
