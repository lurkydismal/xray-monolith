...
static int Read_GIF_Fn( GifFileType* handle,
                        GifByteType* buffer,
                        int bytesToRead ) {
    CStreamReader* const reader =
        static_cast< CStreamReader* >( handle->UserData );
    if ( reader->eof() )
        return 0;
...
static IC void write_pixel( u8* image,
                            u32 offset,
                            u8 red,
                            u8 green,
                            u8 blue,
...
static IC void copy_gif_image_rect( GifFileType* gif,
                                    u8* dstImage,
                                    const u8* srcImage,
                                    GifWord top,
                                    GifWord left,
    ...
    for ( GifWord y = 0; y < height; ++y ) {
        ...
        for ( GifWord x = 0; x < width; ++x ) {
            ...
            const u32 offset =
                static_cast< u32 >( ( canY * gif->SWidth + canX ) * 4 );
...
static IC void fill_gif_image_rect( GifFileType* gif,
                                    u8* dstImage,
                                    GifWord top,
                                    GifWord left,
                                    GifWord width,
...
static bool decode_gif( GifFileType* gif,
                        u8* tempImage,
                        u8* tempLine,
    ...
    do {
        if ( DGifGetRecordType( gif, &record_type ) != GIF_OK ) {
            Msg( "! [ERROR]: Failed to read GIF record (%s) in '%s'",
                 get_gif_error_string( gif ), fname );
            break;
        }
        ...
        switch ( record_type ) {
            case GifRecordType::EXTENSION_RECORD_TYPE: {
                ...
                if ( DGifGetExtension( gif, &code, &ext ) != GIF_OK ) {
                    Msg( "! [ERROR]: Failed to read GIF extension (%s) in '%s'",
                         get_gif_error_string( gif ), fname );
                    done = true;
                    break;
                }
                ...
                while ( ext ) {
                    if ( code == GRAPHICS_EXT_FUNC_CODE ) {
                        ...
                        lastDelay =
                            static_cast< u32 >( tempGCB.DelayTime ) * 10;
                    ...
                    if ( DGifGetExtensionNext( gif, &ext ) != GIF_OK ) {
                        Msg( "! [ERROR]: Failed to read next GIF extension "
                             "(%s) in '%s'",
                             get_gif_error_string( gif ), fname );
                        done = true;
                        break;
            ...
            case GifRecordType::IMAGE_DESC_RECORD_TYPE: {
                if ( DGifGetImageDesc( gif ) != GIF_OK ) {
                    Msg( "! [ERROR]: Failed to read GIF image description (%s) "
                         "in '%s'",
                         get_gif_error_string( gif ), fname );
                    done = true;
                    break;
                ...
                if ( gif->Image.Interlace ) {
                    ...
                    for ( GifWord i = 0; i < 4 && !done; ++i ) {
                        for ( GifWord y = InterlacedOffset[ i ]; y < height;
                              y += InterlacedJumps[ i ] ) {
                            if ( DGifGetLine( gif, tempLine, width ) !=
                                 GIF_OK ) {
                                Msg( "! [ERROR]: Failed to read GIF image line "
                                     "(%s) in '%s'",
                                     get_gif_error_string( gif ), fname );
                                done = true;
                                break;
                            ...
                            for ( GifWord x = 0; x < width; ++x ) {
                                ...
                                const u32 offset = static_cast< u32 >(
                                    ( canY * gif->SWidth + canX ) * 4 );
                                const int idx = tempLine[ x ];

                                VERIFY( idx < colorMap->ColorCount );

                                if ( transparentIdx == NO_TRANSPARENT_COLOR ||
                                     idx != transparentIdx ) {
                                    const GifColorType c =
                                        colorMap->Colors[ idx ];
                                    write_pixel( tempImage, offset, c.Red,
                                                 c.Green, c.Blue, 0xFF );
                                }
                            }
                        }
                    ...
                    for ( GifWord y = 0; y < height; ++y ) {
                        ...
                        for ( GifWord x = 0; x < width; ++x ) {
                            ...
                            if ( transparentIdx == NO_TRANSPARENT_COLOR ||
                                ...
                                write_pixel( tempImage, offset, c.Red, c.Green,
                                             c.Blue, 0xFF );
                            }
                        }
                    }
                ...
                if ( !done ) {
                    u8* const imgData =
                        static_cast< u8* >( Memory.mem_alloc( imageSize ) );
            ...
            default: {
                Msg( "! [ERROR]: Unexpected GIF record [%s] in '%s'",
                     ToString_GifRecordType( record_type ), fname );
                done = true;
                break;
            }
        }
...
bool CGIFResource::Load( const char* fname ) {
    ...
    const bool decodeResult =
        decode_gif( gif, tempImage, tempLine, m_Images, m_ImageSize, fname );
