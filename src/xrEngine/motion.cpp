...
BOOL COMotion::ScaleKeys( float from_time, float to_time, float scale_factor ) {
    BOOL bRes = TRUE;
    for ( int ch = 0; ch < ctMaxChannel; ch++ )
        if ( FALSE == ( bRes = envs[ ch ]->ScaleKeys(
                            from_time, to_time, scale_factor, 1.f / fFPS ) ) )
            break;
    return bRes;
}
BOOL COMotion::NormalizeKeys( float from_time, float to_time, float speed ) {
    ...
    for ( KeyIt it = E->keys.begin() + 1; it != E->keys.end(); it++ ) {
        if ( ( *it )->time > from_time ) {
            if ( ( *it )->time < to_time + EPS ) {
                ...
                Fvector PT, T, R;
                _Evaluate( t0, PT, R );
                for ( float tm = t0 + 1.f / fFPS; tm <= ( *it )->time;
                      tm += EPS_L ) {
                    _Evaluate( tm, T, R );
    ...
    for ( int ch = 0; ch < ctMaxChannel; ch++ ) {
        ...
        for ( KeyIt k_it = E->keys.begin(); k_it != E->keys.end();
              k_it++, f_it++ )
            ( *k_it )->time = *f_it;
    }
...
void CSMotion::Clear() {
    for ( BoneMotionIt bm_it = bone_mots.begin(); bm_it != bone_mots.end();
          bm_it++ ) {
        for ( int ch = 0; ch < ctMaxChannel; ch++ )
...
bool CSMotion::Load( IReader& F ) {
    ...
    if ( vers == 0x0004 ) {
        ...
        for ( BoneMotionIt bm_it = bone_mots.begin(); bm_it != bone_mots.end();
              bm_it++ ) {
            bm_it->SetName(
                itoa( int( bm_it - bone_mots.begin() ), temp_buf, 10 ) );
        ...
        if ( vers == 0x0005 ) {
            ...
            bone_mots.resize( F.r_u32() );
            string64 buf;
            for ( BoneMotionIt bm_it = bone_mots.begin();
                  bm_it != bone_mots.end(); bm_it++ ) {
