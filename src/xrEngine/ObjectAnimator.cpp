    ...
    if ( ext ) {
        ...
        if ( 0 == xr_strcmp( ext, ".anm" ) ) {
            ...
            for ( u32 i = 0; i < dwMCnt; i++ ) {
                ...
                if ( !bRes )
                    FATAL(
                        "ERROR: Can't load motion. Incorrect file version." );
                m_Motions.push_back( M );
            }
            FS.r_close( F );
        }
...
COMotion* CObjectAnimator::Play( bool loop, LPCSTR name ) {
    if ( name && name[ 0 ] ) {
        MotionIt it = std::lower_bound( m_Motions.begin(), m_Motions.end(),
                                        name, motion_find_pred );
        if ( ( it != m_Motions.end() ) &&
             ( 0 == xr_strcmp( ( *it )->Name(), name ) ) ) {
            bLoop = loop;
            SetActiveMotion( *it );
            m_MParam.Play();
            return *it;
...
void CObjectAnimator::DrawPath() {
    ...
    if ( m_Current ) {
        ...
        u32 clr = 0xffffffff;
        path_points.clear();
        for ( float t = min_t; ( t < max_t ) || fsimilar( t, max_t, EPS_L );
              t += 1 / 30.f ) {
        ...
        if ( !path_points.empty() )
            DU_impl.DrawPrimitiveL( D3DPT_LINESTRIP, path_points.size() - 1,
                                    path_points.begin(), path_points.size(),
                                    clr, true, false );
        CEnvelope* E = m_Current->Envelope();
        for ( KeyIt k_it = E->keys.begin(); k_it != E->keys.end(); k_it++ ) {
            m_Current->_Evaluate( ( *k_it )->time, T, r );
            if ( EDevice.m_Camera.GetPosition().distance_to_sqr( T ) <
                 50.f * 50.f ) {
                DU_impl.DrawCross( T, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, clr,
                                   false );
                DU_impl.OutText(
                    T,
                    AnsiString().sprintf( "K: %3.3f", ( *k_it )->time ).c_str(),
