...
//------------------------------------------------------------------------------
void CLensFlareDescriptor::SetSource( float fRadius,
                                      BOOL ign_color,
                                      LPCSTR tex_name,
                                      LPCSTR sh_name ) {
...
void CLensFlareDescriptor::SetGradient( float fMaxRadius,
                                        float fOpacity,
                                        LPCSTR tex_name,
                                        LPCSTR sh_name ) {
...
void CLensFlareDescriptor::AddFlare( float fRadius,
                                     float fOpacity,
                                     float fPosition,
                                     LPCSTR tex_name,
                                     LPCSTR sh_name ) {
...
void CLensFlareDescriptor::load( CInifile* pIni, LPCSTR sect ) {
    ...
    m_Flags.set( flGradient,
                 CInifile::IsBOOL( pIni->r_string( sect, "gradient" ) ) );
    ...
    m_StateBlendUpSpeed =
        1.f / ( _max( pIni->r_float( sect, "blend_rise_time" ), 0.f ) + EPS_S );
    m_StateBlendDnSpeed =
        1.f / ( _max( pIni->r_float( sect, "blend_down_time" ), 0.f ) + EPS_S );
...
void CLensFlareDescriptor::OnDeviceCreate() {
    // shaders
    m_Gradient.m_pRender->CreateShader( *m_Gradient.shader,
                                        *m_Gradient.texture );
    ...
    for (FlareIt it=m_Flares.begin(); it!=m_Flares.end(); it++) it->hShader =
    CreateShader(*it->texture,*it->shader);
    */
}
...
struct STranspParam {
    ...
    STranspParam( collide::ray_cache* p,
                  const Fvector& _P,
                  const Fvector& _D,
                  float _f,
...
IC BOOL material_callback( collide::rq_result& result, LPVOID params ) {
    ...
    if ( result.O ) {
        ...
        if ( K && ( result.element > 0 ) )
            vis = g_pGamePersistent->MtlTransparent(
                K->LL_GetData( u16( result.element ) ).game_mtl_idx );
    } else {
        CDB::TRI* T =
            g_pGameLevel->ObjectSpace.GetStaticTris() + result.element;
...
void CLensFlare::OnFrame( shared_str id ) {
    ...
    CLensFlareDescriptor* desc =
        id.size() ? g_pGamePersistent->Environment().add_flare( m_Palette, id )
                  : 0;
    ...
    switch ( m_State ) {
        ...
        case lfsShow:
            m_StateBlend =
                m_Current ? ( m_StateBlend + m_Current->m_StateBlendUpSpeed *
                                                 Device.fTimeDelta * tf )
                          : 1.f + EPS;
            if ( m_StateBlend >= 1.f ||
                 g_pGamePersistent->Environment().m_paused ) {
                m_StateBlend = 1.f;
                m_State = lfsIdle;
            }
            break;
        case lfsHide:
            m_StateBlend =
                m_Current ? ( m_StateBlend - m_Current->m_StateBlendDnSpeed *
                                                 Device.fTimeDelta * tf )
                          : 0.f - EPS;
            if ( m_StateBlend <= 0.f ||
                 g_pGamePersistent->Environment().m_paused ) {
                m_State = lfsShow;
                m_Current = desc;
                m_StateBlend = m_Current ? m_Current->m_StateBlendUpSpeed *
                                               Device.fTimeDelta * tf
                                         : 0;
            }
            break;
    }
    // Msg ("%6d : [%s] -> [%s]", Device.dwFrame,
    // state_to_string(previous_state), state_to_string(m_State));
    ...
    R_ASSERT( _valid( vSunDir ) );
    STranspParam TP( &m_ray_cache[ 0 ], Device.vCameraPosition, vSunDir, 1000.f,
                     EPS_L );
    ...
    for ( int i = 0; i < MAX_RAYS; ++i ) {
        ...
        if ( m_ray_cache[ i ].result &&
             m_ray_cache[ i ].similar( TP.P, TP.D, TP.f ) ) {
            // similar with previous query == 0
            TP.vis = 0.f;
        } else {
            float _u, _v, _range;
            if ( CDB::TestRayTri( TP.P, TP.D, m_ray_cache[ i ].verts, _u, _v,
                                  _range, false ) &&
                ...
                if ( g_pGameLevel->ObjectSpace.RayQuery(
                         r_dest, RD, material_callback, &TP, NULL, o_main ) )
    ...
    float _u,_v,_range;
    if
    (CDB::TestRayTri(TP.P,TP.D,m_ray_cache.verts,_u,_v,_range,false)&&(_range>0
    && _range<TP.f)){ TP.vis = 0.f; }else{
    // cache outdated. real query.
    r_dest.r_clear ();
    if (g_pGameLevel->ObjectSpace.RayQuery
    (r_dest,RD,material_callback,&TP,NULL,o_main)) m_ray_cache.result = FALSE ;
    }
    }
    ...
    */
    ...
    if ( m_Current->m_Flags.is( CLensFlareDescriptor::flGradient ) ) {
        ...
        if ( _abs( scr_pos.x ) > sun_blend )
            kx = ( ( sun_max - ( float )_abs( scr_pos.x ) ) ) /
                 ( sun_max - sun_blend );
        if ( _abs( scr_pos.y ) > sun_blend )
            ky = ( ( sun_max - ( float )_abs( scr_pos.y ) ) ) /
                 ( sun_max - sun_blend );

        if ( !( ( _abs( scr_pos.x ) > sun_max ) ||
                ( _abs( scr_pos.y ) > sun_max ) ) ) {
...
void CLensFlare::Render( BOOL bSun, BOOL bFlares, BOOL bGradient ) {
    ...
    u32 VS_Offset;
    FVF::LIT *pv = (FVF::LIT*)
    RCache.Vertex.Lock(MAX_Flares*4,hGeom.stride(),VS_Offset);
    ...
    u32 c = color.get();
    pv->set (vecLight.x+vecSx.x-vecSy.x, vecLight.y+vecSx.y-vecSy.y,
    vecLight.z+vecSx.z-vecSy.z, c, 0, 0); pv++; pv->set
    (vecLight.x+vecSx.x+vecSy.x, vecLight.y+vecSx.y+vecSy.y,
    vecLight.z+vecSx.z+vecSy.z, c, 0, 1); pv++; pv->set
    (vecLight.x-vecSx.x-vecSy.x, vecLight.y-vecSx.y-vecSy.y,
    vecLight.z-vecSx.z-vecSy.z, c, 1, 0); pv++; pv->set
    (vecLight.x-vecSx.x+vecSy.x, vecLight.y-vecSx.y+vecSy.y,
    vecLight.z-vecSx.z+vecSy.z, c, 1, 1); pv++; _2render.push_back
    (m_Current->m_Source.hShader);
    }
    }
    if (fBlend>=EPS_L)
    {
    ...
    for (CLensFlareDescriptor::FlareIt it=m_Current->m_Flares.begin();
    it!=m_Current->m_Flares.end(); it++){ CLensFlareDescriptor::SFlare& F = *it;
    ...
    color.mul_rgba ( cl );
    u32 c = color.get();
    pv->set (vec.x+vecSx.x-vecSy.x, vec.y+vecSx.y-vecSy.y,
    vec.z+vecSx.z-vecSy.z, c, 0, 0); pv++; pv->set (vec.x+vecSx.x+vecSy.x,
    vec.y+vecSx.y+vecSy.y, vec.z+vecSx.z+vecSy.z, c, 0, 1); pv++; pv->set
    (vec.x-vecSx.x-vecSy.x, vec.y-vecSx.y-vecSy.y, vec.z-vecSx.z-vecSy.z, c, 1,
    0); pv++; pv->set (vec.x-vecSx.x+vecSy.x, vec.y-vecSx.y+vecSy.y,
    vec.z-vecSx.z+vecSy.z, c, 1, 1); pv++; _2render.push_back (it->hShader);
    }
    }
    }
    // gradient
...
shared_str CLensFlare::AppendDef( CEnvironment& environment,
                                  CInifile* pIni,
                                  LPCSTR sect ) {
    if ( !sect || ( 0 == sect[ 0 ] ) )
        return "";
