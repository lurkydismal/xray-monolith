...
void CDemoPlay::stat_Stop() {
    ...
    */
    ...
    Msg( "* [DEMO] FPS: average[%f], min[%f], max[%f], middle[%f]",
         rfps_average, rfps_min, rfps_max, rfps_middlepoint );
    ...
    if ( g_bBenchmark ) {
        ...
        res.w_float( "general", "renderer",
                     float( ::Render->get_generation() ) / 10.f,
                     "dx-level required" );
...
void spline1( float t, Fvector* p, Fvector* ret ) {
    ...
    m[ 1 ] =
        ( 0.5f * ( ( 3.0f * t3 ) + ( -5.0f * t2 ) + ( 0.0f * t ) + 2.0f ) );
