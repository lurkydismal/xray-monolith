...
static void __stdcall build_callback( Fvector* V,
                                      int Vcnt,
                                      CDB::TRI* T,
                                      int Tcnt,
                                      void* params ) {
...
bool IGame_Level::Load( u32 dwNum ) {
    ...
    if ( !FS.exist( temp, "$level$", "level.ltx" ) )
        Debug.fatal( DEBUG_INFO, "Can't find level configuration file '%s'.",
                     temp );
    ...
    R_ASSERT2( XRCL_PRODUCTION_VERSION == H.XRLC_version,
    ...
    NativeLoadExecutor::Batch level_load_batch =
        load_executor.BeginBatch( load_executor.CurrentGeneration() );
    xr_task_group fallback_level_tasks;
    auto submit_level_task = [ &load_executor, &level_load_batch,
                               &fallback_level_tasks ](
                                 NativeLoadPriority priority, auto&& work ) {
        if ( level_load_batch.Valid() )
            load_executor.Submit( level_load_batch, priority,
                                  std::forward< decltype( work ) >( work ) );
        else
            fallback_level_tasks.run(
                std::forward< decltype( work ) >( work ) );
    };
    ObjectSpace.Load(
        []( Fvector* V, int Vcnt, CDB::TRI* T, int Tcnt, void* params ) {
            g_pGameLevel->Load_GameSpecific_CFORM( T, Tcnt );
        } );
    ...
    Msg( "* [LEVEL LOAD] game-specific before: %d ms",
         game_specific_before_timer.GetElapsed_ms() );
    ...
    barrier_timer.Start();
    drain_level_tasks();
    Msg( "* [LEVEL LOAD] CFORM/AI barrier: %d ms",
         barrier_timer.GetElapsed_ms() );
...
void IGame_Level::OnRender() {
    ...
    if ( !g_dedicated_server ) {
        ...
        if ( measure_precache )
            pApp->LoadSessionRecordPrecacheLevel( calculate_ticks,
                                                  render_ticks );
    } else {
...
void IGame_Level::OnFrame() {
    ...
    // Ambience
    if ( Sounds_Random.size() &&
         ( Device.dwTimeGlobal > Sounds_Random_dwNextTime ) ) {
        Sounds_Random_dwNextTime =
            Device.dwTimeGlobal + ::Random.randI( 10000, 20000 );
        Fvector pos;
        pos.random_dir()
            .normalize()
            .mul( ::Random.randF( 30, 100 ) )
...
void IGame_Level::SoundEvent_Register( ref_sound_data_ptr S, float range ) {
    ...
    g_SpatialSpace->q_box( snd_ER, 0, STYPE_REACTTOSOUND, snd_position,
                           bb_size );
    ...
    for ( ; it != end; it++ ) {
        ...
        VERIFY( _valid( Power ) );
        if ( Power > EPS_S ) {
            float occ = Sound->get_occlusion_to( ( *it )->spatial.sphere.P,
                                                 snd_position );
            VERIFY( _valid( occ ) );
            Power *= occ;
            if ( Power > EPS_S ) {
...
void IGame_Level::SoundEvent_OnDestDestroy( Feel::Sound* obj ) {
    ...
    snd_Events.erase(
        std::remove_if( snd_Events.begin(), snd_Events.end(), rem_pred( obj ) ),
        snd_Events.end() );
}
