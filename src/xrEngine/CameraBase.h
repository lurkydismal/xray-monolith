...
#if !defined( \
    AFX_CAMERABASE_H__B11F8AE1_1213_11D4_B4E3_4854E82A090D__INCLUDED_ )
...
template < typename T >
IC void tviewport_size( CRenderDeviceBase& D,
                        float _viewport_near,
                        const T& cam_info,
                        float& h_w,
                        float& h_h ) {
    h_h = _viewport_near * tan( deg2rad( cam_info.Fov() ) / 2.f );
    VERIFY2( _valid( h_h ), make_string( "invalide viewporrt params fov: %f ",
                                         cam_info.Fov() ) );
...
template < typename T >
IC void viewport_size( float _viewport_near,
                       const T& cam_info,
                       float& h_w,
                       float& h_h ) {
