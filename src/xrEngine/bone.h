...
public:
    ...
    Fmatrix
        mRenderTransform; // final x-form matrix (model_base -> bone -> model)
...
public:
    ...
    IC BOOL _BCL callback_overwrite() {
...
public:
    ...
    void _BCL set_callback( u32 Type,
                            BoneCallback C,
                            void* Param,
...
struct ECORE_API SJointIKData {
    ...
    EJointType type;
    SJointLimit
        limits[ 3 ]; // by [axis XYZ on joint] and[Z-wheel,X-steer on wheel]
    float spring_factor;
    float damping_factor;
    ...
    void Export( IWriter& F ) {
        ...
        for ( int k = 0; k < 3; k++ ) {
            ...
            // направление вращения в ОДЕ отличается от направления вращение в
            // X-Ray поэтому меняем знак у лимитов
            // F.w_float (_min(-limits[k].limit.x,-limits[k].limit.y)); // min
            // (swap special for ODE) F.w_float
            // (_max(-limits[k].limit.x,-limits[k].limit.y)); // max (swap
            // special for ODE)

            VERIFY( _min( -limits[ k ].limit.x, -limits[ k ].limit.y ) ==
                    -limits[ k ].limit.y );
            VERIFY( _max( -limits[ k ].limit.x, -limits[ k ].limit.y ) ==
                    -limits[ k ].limit.x );
...
public:
    ...
    void SetRestParams( float length,
    ...
    IC float _BCL engine_lo_limit( u8 k ) const {
        return -IK_data.limits[ k ].limit.y;
    }

    IC float _BCL engine_hi_limit( u8 k ) const {
        return -IK_data.limits[ k ].limit.x;
    }

    IC float _BCL editor_lo_limit( u8 k ) const {
        return IK_data.limits[ k ].limit.x;
    }

    IC float _BCL editor_hi_limit( u8 k ) const {
        return IK_data.limits[ k ].limit.y;
    }
...
public:
    virtual u32 mem_usage() {
        u32 sz =
            sizeof( *this ) + sizeof( vecBones::value_type ) * children.size();
        for ( ChildFacesVecIt c_it = child_faces.begin();
              c_it != child_faces.end(); ++c_it )
            sz +=
                c_it->size() * sizeof( FacesVec::value_type ) + sizeof( *c_it );
        return sz;
    }
};
