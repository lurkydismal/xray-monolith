...
static void ImGui_UpdateKeyboardCodePage( UINT32& keyboard_code_page ) {
    // Retrieve keyboard code page, required for handling of non-Unicode
    // Windows.
    ...
    if ( ::GetLocaleInfoA(
             ...
             ( LPSTR )&keyboard_code_page, sizeof( keyboard_code_page ) ) == 0 )
        keyboard_code_page =
...
void ide::InitBackend() {
    ...
    io.GetClipboardTextFn = []( void* user_data ) -> const char* {
        ...
        os_clipboard::paste_from_clipboard( &bd.clipboard_text_data[ 0 ],
                                            2048 );
...
void ide::IR_OnMouseMove( int /*x*/, int /*y*/ ) {
    ...
    ImGuiIO& io = ImGui::GetIO();
    io.AddMousePosEvent( static_cast< float >( p.x ),
...
void ide::IR_OnKeyboardRelease( int key ) {
    ...
    switch ( key ) {
        case DIK_LCONTROL:
            check( ImGuiMod_Ctrl, DIK_RCONTROL );
            break;
        case DIK_RCONTROL:
            check( ImGuiMod_Ctrl, DIK_LCONTROL );
            break;
        case DIK_LSHIFT:
            check( ImGuiMod_Shift, DIK_RSHIFT );
            break;
        case DIK_RSHIFT:
            check( ImGuiMod_Shift, DIK_LSHIFT );
            break;
        case DIK_LALT:
            check( ImGuiMod_Alt, DIK_RALT );
            break;
        case DIK_RALT:
            check( ImGuiMod_Alt, DIK_LALT );
            break;
        case DIK_LWIN:
            check( ImGuiMod_Super, DIK_RWIN );
            break;
        case DIK_RWIN:
            check( ImGuiMod_Super, DIK_LWIN );
            break;
    }
...
void ide::InputChar( WPARAM param ) {
    ...
    wchar_t wch = 0;
    ::MultiByteToWideChar( keyboard_code_page, MB_PRECOMPOSED, ( char* )&param,
                           1, &wch, 1 );
