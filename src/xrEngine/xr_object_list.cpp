...
CObject* CObjectList::FindObjectByName( shared_str name ) {
    for ( Objects::iterator I = objects_active.begin();
          I != objects_active.end(); I++ )
        if ( ( *I )->cName().equal( name ) )
            return ( *I );
    for ( Objects::iterator I = objects_sleeping.begin();
          I != objects_sleeping.end(); I++ )
        if ( ( *I )->cName().equal( name ) )
            return ( *I );
    return NULL;
}
...
CObject* CObjectList::FindObjectByCLS_ID( CLASS_ID cls ) {
    {
        Objects::iterator O = std::find_if(
            objects_active.begin(), objects_active.end(), fClassEQ( cls ) );
        if ( O != objects_active.end() )
            return *O;
    }
    {
        Objects::iterator O = std::find_if(
            objects_sleeping.begin(), objects_sleeping.end(), fClassEQ( cls ) );
...
void CObjectList::SingleUpdate( CObject* O ) {
...
#ifdef DEBUG
        // if (O->getDestroy())
        // Msg ("- !!!processing_enabled ->destroy_queue.push_back %s[%d] frame
        // [%d]",O->cName().c_str(), O->ID(), Device.dwFrame);
#endif // #ifdef DEBUG
    ...
    // Msg ("[%d][0x%08x]IAmNotACrowAnyMore (CObjectList::SingleUpdate)",
    // Device.dwFrame, fast_dynamic_cast<void*>(O));

    O->UpdateCL();
#ifdef DEBUG
    VERIFY3( O->dbg_update_cl == Device.dwFrame,
             "Broken sequence of calls to 'UpdateCL'", *O->cName() );
#endif
#if 0 // ndef DEBUG
	__try
	{
#endif
    if ( O->H_Parent() &&
         ( O->H_Parent()->getDestroy() || O->H_Root()->getDestroy() ) ) {
        // Push to destroy-queue if it isn't here already
        Msg( "! ERROR: incorrect destroy sequence for object[%d:%s], "
             "section[%s], parent[%d:%s]",
             O->ID(), *O->cName(), *O->cNameSect(), O->H_Parent()->ID(),
             *O->H_Parent()->cName() );
    }
#if 0 // ndef DEBUG
	}
...
void CObjectList::clear_crow_vec( Objects& o ) {
    for ( u32 _it = 0; _it < o.size(); _it++ ) {
        // Msg ("[%d][0x%08x]IAmNotACrowAnyMore (clear_crow_vec)",
        // Device.dwFrame, fast_dynamic_cast<void*>(o[_it]));
...
void CObjectList::Update( bool bForce ) {
    ...
    if ( !Device.Paused() || bForce ) {
        ...
        if ( Device.fTimeDelta > EPS_S || bForce ) {
...

            {
                ...
                Device.Statistic->UpdateClient_total =
    ...
    if ( mt_Scheduler )
        Device.seqParallelBeforRender.push_back(
            xr_make_delegate( this, &CObjectList::ProcessDestroyQueue ) );
    else
        ProcessDestroyQueue();
}
...
#ifdef DEBUG
            if ( debug_destroy )
                Msg( "Destroying object[%x][%x] [%d][%s] frame[%d]",
                     fast_dynamic_cast< void* >( obj ), obj, obj->ID(),
                     *obj->cName(), Device.dwFrame );
#endif // DEBUG
...
u32 CObjectList::net_Export( NET_Packet* _Packet,
                             u32 start,
                             u32 max_object_size ) {
        ...
        if ( P->net_Relevant() && !P->getDestroy() ) {
            ...
            if ( size >= 256 ) {
                Debug.fatal( DEBUG_INFO,
                             ...
                             *P->cName(), P->ID(), size, Packet.w_tell(),
                             position );
            }
#endif
            if ( g_Dump_Export_Obj ) {
...
void CObjectList::Load() {
    R_ASSERT( /*map_NETID.empty() &&*/ objects_active.empty() &&
              force_destroy_queue.empty() && destroy_queue.empty() &&
              objects_sleeping.empty() && objects_relcase_active.empty() &&
              objects_relcase_sleeping.empty() );
}
...
void CObjectList::ClearProcessDestroyQueueFromDevice() {
    ...
    Device.seqParallelBeforRender.erase(
        std::remove( Device.seqParallelBeforRender.begin(),
                     Device.seqParallelBeforRender.end(), Callback ),
...
void CObjectList::Unload() {
    ...
    if ( objects_sleeping.size() || objects_active.size() )
        Msg( "! objects-leaked: %d",
             objects_sleeping.size() + objects_active.size() );

    // Destroy objects
    while ( objects_sleeping.size() ) {
        CObject* O = objects_sleeping.back();
        Msg( "! [%x] s[%4d]-[%s]-[%s]", O, O->ID(), *O->cNameSect(),
             *O->cName() );
        O->setDestroy( true );
    ...
    while ( objects_active.size() ) {
        ...
        Msg( "! [%x] a[%4d]-[%s]-[%s]", O, O->ID(), *O->cNameSect(),
             *O->cName() );
        O->setDestroy( true );
...
void CObjectList::Destroy( CObject* O ) {
    ...
    // Full teardown queues objects in active/sleeping order and destroys them
    // in reverse order. Keep the stable-erase fallback for every other path.
    if ( !objects_active.empty() && objects_active.back() == O ) {
        objects_active.pop_back();
        if ( O->net_RelcaseNeeded() )
            o_remove_relcase( objects_relcase_active, O );
        VERIFY( std::find( objects_active.begin(), objects_active.end(), O ) ==
                objects_active.end() );
        VERIFY( std::find( objects_sleeping.begin(), objects_sleeping.end(),
                           O ) == objects_sleeping.end() );
        ...
        Objects::iterator _i =
            std::find( objects_active.begin(), objects_active.end(), O );
        if ( _i != objects_active.end() ) {
            objects_active.erase( _i );
            if ( O->net_RelcaseNeeded() )
                o_remove_relcase( objects_relcase_active, O );
            VERIFY( std::find( objects_active.begin(), objects_active.end(),
                               O ) == objects_active.end() );
            VERIFY( std::find( objects_sleeping.begin(), objects_sleeping.end(),
                               O ) == objects_sleeping.end() );
        } else {
            Objects::iterator _ii = std::find( objects_sleeping.begin(),
                                               objects_sleeping.end(), O );
            if ( _ii != objects_sleeping.end() ) {
                objects_sleeping.erase( _ii );
                if ( O->net_RelcaseNeeded() )
                    o_remove_relcase( objects_relcase_sleeping, O );
                VERIFY( std::find( objects_sleeping.begin(),
...
#ifdef DEBUG
    RELCASE_CALLBACK_VEC::iterator It =
        std::find( m_relcase_callbacks.begin(), m_relcase_callbacks.end(), cb );
...
void CObjectList::dump_list( Objects& v, LPCSTR reason ) {
    ...
    for ( ; it != it_e; ++it )
        Msg(
            "%x - name [%s] ID[%d] parent[%s] getDestroy()=[%s]", ( *it ),
            ( *it )->cName().c_str(), ( *it )->ID(),
...
void CObjectList::register_object_to_destroy( CObject* object_to_destroy ) {
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( !O->getDestroy() && O->H_Parent() == object_to_destroy ) {
            Msg( "setDestroy called, but not-destroyed child found parent[%d] "
                 "child[%d]",
                 object_to_destroy->ID(), O->ID(), Device.dwFrame );
            O->setDestroy( TRUE );
        }
    }
...
#ifdef DEBUG
bool CObjectList::registered_object_to_destroy(
    const CObject* object_to_destroy ) const {
    return ( std::find( destroy_queue.begin(), destroy_queue.end(),
                        object_to_destroy ) != destroy_queue.end() );
}
#endif // DEBUG
