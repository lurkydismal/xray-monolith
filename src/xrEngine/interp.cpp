...
static float bez2( st_Key* key0, st_Key* key1, float time ) {
    ...
    return bezier( key0->value, y, key1->param[ 1 ] + key1->value, key1->value,
                   t );
}
...
static float outgoing( st_Key* key0p, st_Key* key0, st_Key* key1 ) {
    ...
    switch ( key0->shape ) {
        case SHAPE_TCB:
            a = ( 1.0f - key0->tension ) * ( 1.0f + key0->continuity ) *
                ( 1.0f + key0->bias );
            b = ( 1.0f - key0->tension ) * ( 1.0f - key0->continuity ) *
                ( 1.0f - key0->bias );
        ...
        case SHAPE_HERM:
            ...
            if ( key0p )
                out *=
                    ( key1->time - key0->time ) / ( key1->time - key0p->time );
            break;
...
static float incoming( st_Key* key0, st_Key* key1, st_Key* key1n ) {
    ...
    switch ( key1->shape ) {
        ...
        case SHAPE_TCB:
            a = ( 1.0f - key1->tension ) * ( 1.0f - key1->continuity ) *
                ( 1.0f + key1->bias );
            b = ( 1.0f - key1->tension ) * ( 1.0f + key1->continuity ) *
                ( 1.0f - key1->bias );
            d = key1->value - key0->value;
        ...
        case SHAPE_HERM:
            ...
            if ( key1n )
                in *=
                    ( key1->time - key0->time ) / ( key1n->time - key0->time );
            break;
...
float evalEnvelope( CEnvelope* env, float time ) {
    st_Key *key0, *key1, *skey, *ekey, *skey_n, *ekey_p, *key0_p = 0,
                                                         *key1_n = 0;
    ...
    if ( time < skey->time ) {
        switch ( env->behavior[ 0 ] ) {
            ...
            case BEH_LINEAR:
                out =
                    outgoing( 0, skey, skey_n ) / ( skey_n->time - skey->time );
    ...
    // use post-behavior if time is after last key time
        ...
        switch ( env->behavior[ 1 ] ) {
            ...
            case BEH_LINEAR:
                in =
                    incoming( ekey_p, ekey, 0 ) / ( ekey->time - ekey_p->time );
