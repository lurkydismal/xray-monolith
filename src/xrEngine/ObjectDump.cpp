...
ENGINE_API std::string dbg_object_base_dump_string( const CObject* obj ) {
    ...
    return make_string( "object name: %s, section name: %s, visual name: %s \n",
                        obj->cName().c_str(), obj->cNameSect().c_str(),
...
ENGINE_API std::string dbg_object_poses_dump_string( const CObject* obj ) {
    ...
    for ( u32 i = 0; i < ps_size; ++i ) {
        ...
        buf += ( make_string( " \n %d, time: %d pos: %s ", i, svp.dwTime,
                              get_string( svp.vPosition ).c_str() ) );
    }
...
ENGINE_API std::string dbg_object_visual_geom_dump_string(
    const CObject* obj ) {
    if ( !obj || !obj->Visual() )
        return std::string( "" );
...
ENGINE_API std::string dbg_object_props_dump_string( const CObject* obj ) {
    ...
    return make_string(
               " net_ID :%d, bActiveCounter :%d, bEnabled :%s, bVisible :%s, "
               "bDestroy :%s, \n net_Local %s, net_Ready :%s, net_SV_Update "
               ":%s, crow :%s, bPreDestroy : %s ",
               props.net_ID, props.bActiveCounter,
               get_string( bool( !!props.bEnabled ) ).c_str(),
               get_string( bool( !!props.bVisible ) ).c_str(),
               get_string( bool( !!props.bDestroy ) ).c_str(),
               get_string( bool( !!props.net_Local ) ).c_str(),
               get_string( bool( !!props.net_Ready ) ).c_str(),
               get_string( bool( !!props.net_SV_Update ) ).c_str(),
               get_string( bool( !!props.crow ) ).c_str(),
               get_string( bool( !!props.bPreDestroy ) ).c_str() ) +
           make_string(
               "\n dbg_update_cl: %d, dwFrame_UpdateCL: %d, dwFrame_AsCrow "
               ":%d, Device.dwFrame :%d, Device.dwTimeGlobal: %d \n",
