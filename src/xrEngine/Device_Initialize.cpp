...
void CRenderDevice::initialize_editor() {
    ...
    m_editor_initialize = ( initialize_function_ptr )GetProcAddress(
        m_editor_module, "initialize" );
    VERIFY( m_editor_initialize );

    m_editor_finalize =
        ( finalize_function_ptr )GetProcAddress( m_editor_module, "finalize" );
...
#ifdef INGAME_EDITOR
    if ( Core.ParamsData.test( ECoreParams::editor ) )
        initialize_editor();
#endif // #ifdef INGAME_EDITOR

    // Unless a substitute hWnd has been specified, create a window to render
    // into
    if ( m_hWnd == NULL ) {
        ...
        m_hWnd = CreateWindowEx( WS_EX_TOPMOST, wndclass,
                                 "S.T.A.L.K.E.R.: Anomaly", m_dwWindowStyle,
