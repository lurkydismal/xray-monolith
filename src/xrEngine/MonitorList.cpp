...
typedef LONG( WINAPI* PFN_GetDisplayConfigBufferSizes )( UINT32,
                                                         UINT32*,
                                                         UINT32* );
typedef LONG( WINAPI* PFN_QueryDisplayConfig )( UINT32,
                                                UINT32*,
                                                void*,
                                                UINT32*,
                                                void*,
...
bool TryGetDeviceDesc( const WCHAR* devicePath, char* buf, size_t bufSize ) {
    ...
    CONFIGRET cr = CM_Get_Device_Interface_PropertyW(
        devicePath, &DEVPKEY_Device_InstanceId, &propType,
    ...
    size = sizeof( desc );
    propType = 0;
    cr = CM_Get_DevNode_PropertyW( devInst, &DEVPKEY_Device_DeviceDesc,
                                   &propType, reinterpret_cast< PBYTE >( desc ),
                                   &size, 0 );
    if ( cr != CR_SUCCESS || propType != DEVPROP_TYPE_STRING ||
         desc[ 0 ] == L'\0' )
        return false;

    WideCharToMultiByte( CP_ACP, 0, desc, -1, buf, ( int )bufSize - 1, nullptr,
                         nullptr );
    buf[ bufSize - 1 ] = '\0';
    return true;
}
...
void BuildFriendlyNameMap( GdiToInfoMap& out ) {
    ...
    auto pfnGetBufs = ( PFN_GetDisplayConfigBufferSizes )GetProcAddress(
        hUser32, "GetDisplayConfigBufferSizes" );
    auto pfnQuery = ( PFN_QueryDisplayConfig )GetProcAddress(
        hUser32, "QueryDisplayConfig" );
    auto pfnGetDevInfo = ( PFN_DisplayConfigGetDeviceInfo )GetProcAddress(
        hUser32, "DisplayConfigGetDeviceInfo" );

    if ( !pfnGetBufs || !pfnQuery || !pfnGetDevInfo )
        return;

    UINT32 num_paths = 0, num_modes = 0;
    if ( pfnGetBufs( QDC_ONLY_ACTIVE_PATHS, &num_paths, &num_modes ) !=
         ERROR_SUCCESS )
        return;
    if ( num_paths == 0 )
        return;
    ...
    if ( pfnQuery( QDC_ONLY_ACTIVE_PATHS, &num_paths, paths.data(), &num_modes,
    ...
    for ( UINT32 i = 0; i < num_paths; ++i ) {
        ...
        char gdi[ 64 ] = {};
        WideCharToMultiByte( CP_ACP, 0, src.viewGdiDeviceName, -1, gdi,
                             sizeof( gdi ) - 1, nullptr, nullptr );
        out[ gdi ] = info;
    }
}

xr_string MakeLabel( const MonitorInfo& info,
                     const char* gdi_fallback,
                     bool suffix_needed ) {
    ...
    if ( tech_str )
        xr_sprintf( suffix, sizeof( suffix ), " (%s-%u)", tech_str,
                    info.connector_instance + 1 );
    else
        xr_sprintf( suffix, sizeof( suffix ), " (OUT-%u)",
                    info.connector_instance + 1 );
...
static BOOL CALLBACK EnumMonitorsProc( HMONITOR hMon,
                                       HDC,
                                       LPRECT,
                                       LPARAM lParam ) {
    ...
    e.base_label =
        ( it != ctx->friendly_map->end() && !it->second.friendly_name.empty() )
...
void fill_vid_monitor_list() {
    ...
    xr_vector< OutputEntry > outputs;
    {
        ...
        if ( !EnumDisplayMonitors( NULL, NULL, EnumMonitorsProc,
                                   reinterpret_cast< LPARAM >( &ctx ) ) ||
             outputs.empty() )
            Msg( "! vid_monitor: EnumDisplayMonitors returned no monitors, "
                 "list will be Auto-only" );
    }
    ...
    for ( u32 i = 0; i < N; ++i ) {
        ...
        if ( it != friendly_map.end() )
            final_label =
                MakeLabel( it->second, e.gdi_name.c_str(), needs_suffix );
        else
...
HMONITOR ResolveSelectedMonitor() {
    ...
    Msg( "! vid_monitor: '%s' not found on this system, using Auto",
         name.c_str() );
    return NULL;
}
