...
line_edit_control::~line_edit_control() {
    ...
    buffer_vector< Base* > actions( m_actions, array_size, &m_actions[ 0 ],
                                    &m_actions[ 0 ] + array_size );
...
void line_edit_control::init( u32 str_buffer_size, init_mode mode ) {
    ...
    if ( mode == im_read_only ) {
        assign_callback( DIK_A, ks_Ctrl,
                         Callback( this, &line_edit_control::select_all_buf ) );
        assign_callback(
            DIK_C, ks_Ctrl,
            Callback( this, &line_edit_control::copy_to_clipboard ) );
        assign_callback(
            DIK_INSERT, ks_Ctrl,
            Callback( this, &line_edit_control::copy_to_clipboard ) );

        assign_callback( DIK_HOME, ks_free,
                         Callback( this, &line_edit_control::move_pos_home ) );
        assign_callback( DIK_END, ks_free,
                         Callback( this, &line_edit_control::move_pos_end ) );
        assign_callback( DIK_LEFT, ks_free,
                         Callback( this, &line_edit_control::move_pos_left ) );
        assign_callback( DIK_RIGHT, ks_free,
                         Callback( this, &line_edit_control::move_pos_right ) );
        assign_callback(
            DIK_LEFT, ks_Ctrl,
            Callback( this, &line_edit_control::move_pos_left_word ) );
        assign_callback(
            DIK_RIGHT, ks_Ctrl,
            Callback( this, &line_edit_control::move_pos_right_word ) );

        // Antglobes: Include support for numpad keys only when numlock is
        // toggle on home=7, 1=end, 4=left, 6=right
        assign_callback( DIK_NUMPAD7, ks_NumLock,
                         Callback( this, &line_edit_control::move_pos_home ) );
        assign_callback( DIK_NUMPAD1, ks_NumLock,
                         Callback( this, &line_edit_control::move_pos_end ) );
        assign_callback( DIK_NUMPAD4, ks_NumLock,
                         Callback( this, &line_edit_control::move_pos_left ) );
        assign_callback( DIK_NUMPAD6, ks_NumLock,
                         Callback( this, &line_edit_control::move_pos_right ) );
        assign_callback(
            DIK_NUMPAD4, ks_NumLk_Ctrl,
            Callback( this, &line_edit_control::move_pos_left_word ) );
        assign_callback(
            DIK_NUMPAD6, ks_NumLk_Ctrl,
            Callback( this, &line_edit_control::move_pos_right_word ) );
        ...
        assign_callback(
            DIK_INSERT, ks_free,
            Callback( this, &line_edit_control::flip_insert_mode ) );
        assign_callback( DIK_A, ks_Ctrl,
                         Callback( this, &line_edit_control::select_all_buf ) );
        assign_callback( DIK_Z, ks_Ctrl,
                         Callback( this, &line_edit_control::undo_buf ) );
        ...
        assign_callback(
            DIK_V, ks_Ctrl,
            Callback( this, &line_edit_control::paste_from_clipboard ) );
        assign_callback(
            DIK_X, ks_Ctrl,
            Callback( this, &line_edit_control::cut_to_clipboard ) );
        ...
        assign_callback(
            DIK_BACK, ks_free,
            Callback( this, &line_edit_control::delete_selected_back ) );
        assign_callback(
            DIK_DELETE, ks_free,
            Callback( this, &line_edit_control::delete_selected_forward ) );
        assign_callback(
            DIK_BACK, ks_Ctrl,
            Callback( this, &line_edit_control::delete_word_back ) );
        assign_callback(
            DIK_DELETE, ks_Ctrl,
            Callback( this, &line_edit_control::delete_word_forward ) );

        assign_callback( DIK_LSHIFT, ks_Ctrl,
                         Callback( this, &line_edit_control::SwitchKL ) );
        assign_callback( DIK_LSHIFT, ks_Alt,
                         Callback( this, &line_edit_control::SwitchKL ) );
...
void line_edit_control::create_char_pair( u32 const dik,
                                          char c,
                                          char c_shift,
                                          bool translate ) {
    if ( m_actions[ dik ] ) {
    ...
    m_actions[ dik ] =
        xr_new< text_editor::type_pair >( dik, c, c_shift, translate );
}

void line_edit_control::assign_callback( u32 const dik,
                                         key_state state,
                                         Callback const& callback ) {
    VERIFY( dik < DIK_COUNT );
    Base* prev_action = m_actions[ dik ];
    m_actions[ dik ] =
        xr_new< text_editor::callback_base >( callback, dik, state );
...
void line_edit_control::on_frame() {
    ...
    {
    Msg( " cur_time=%.2f re=%d acc=%.2f rep_time=%.2f", cur_time, bRepeat,
    fAccel, rep_time );
    }*/
}
...
void line_edit_control::update_bufs() {
    ...
    strncpy_s( m_buf3, m_buffer_size, m_edit_str + m_p2 + ds,
               edit_size - m_p2 - ds );

    m_need_update = true;
    m_last_changed_frame = Device.dwFrame;
    // if ( m_cursor_view ) {
    // Msg( " m_p1=%d m_p2=%d cur=%d sstart=%d", m_p1, m_p2, m_cur_pos,
    // m_select_start ); }
}
...
void line_edit_control::add_inserted_text() {
    ...
    strncpy_s( buf + m_p1, m_buffer_size, m_inserted,
               _min( new_size, m_buffer_size - m_p1 ) ); // part 2

    u8 ds = ( m_insert_mode && m_p2 < old_edit_size ) ? 1 : 0;
    strncpy_s( buf + m_p1 + new_size, m_buffer_size, m_edit_str + m_p2 + ds,
               _min( old_edit_size - m_p2 - ds,
                     m_buffer_size - m_p1 - new_size ) ); // part 3
    buf[ m_buffer_size ] = 0;
...
void line_edit_control::delete_selected( bool back ) {
    ...
    if ( edit_len > 0 ) {
        if ( back ) {
            ...
            strncpy_s( m_undo_buf, m_buffer_size, m_edit_str + m_p1 - dp,
                       m_p2 - m_p1 + dp );
            strncpy_s( m_edit_str + m_p1 - dp, m_buffer_size, m_edit_str + m_p2,
                       edit_len - m_p2 );
            m_cur_pos = m_p1 - dp;
        } else {
            u8 dn = ( ( m_p1 == m_p2 ) && m_p2 < edit_len ) ? 1 : 0;
            strncpy_s( m_undo_buf, m_buffer_size, m_edit_str + m_p1,
                       m_p2 - m_p1 + dn );
            strncpy_s( m_edit_str + m_p1, m_buffer_size, m_edit_str + m_p2 + dn,
                       edit_len - m_p2 - dn );
            m_cur_pos = m_p1;
        }
        clamp_cur_pos();
