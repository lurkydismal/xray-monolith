...
const float fuzzy_guaranteed =
    0.001f;                        // distance which is supposed 100% visible
const float lr_granularity = 0.1f; // assume similar positions

class ENGINE_API Vision : private pure_relcase, private pure_relcase_visual {
private:
...
public:
    ...
    void feel_vision_get( xr_vector< CObject* >& R ) {
        ...
        if ( feel_visible.size() > 0xffff ) {
            Msg( "![feel_vision_get] abnormally high size of feel_visible, "
                 "clear and skip" );
    ...
    Fvector feel_vision_get_vispoint( CObject* _O ) {
        ...
        xrSRWLockGuard guard( &lock_visible, true );
        auto it =
            std::find_if( feel_visible.begin(), feel_visible.end(),
