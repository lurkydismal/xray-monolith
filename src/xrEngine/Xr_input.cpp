...
CInput::CInput( BOOL bExclusive, int deviceForInit ) {
    ...
    if ( !pDI )
        CHK_DX( DirectInput8Create( GetModuleHandle( NULL ),
    ...
    if ( deviceForInit & keyboard_device_key )
        CHK_DX( CreateInputDevice( &pKeyboard, GUID_SysKeyboard,
                                   &c_dfDIKeyboard, kb_input_flags,
                                   KEYBOARDBUFFERSIZE ) );

    // MOUSE
    if ( deviceForInit & mouse_device_key )
        CHK_DX( CreateInputDevice( &pMouse, GUID_SysMouse, &c_dfDIMouse2,
                                   mouse_input_flags, MOUSEBUFFERSIZE ) );
...
//-----------------------------------------------------------------------------
HRESULT CInput::CreateInputDevice( LPDIRECTINPUTDEVICE8* device,
                                   GUID guidDevice,
                                   const DIDATAFORMAT* pdidDataFormat,
                                   u32 dwFlags,
                                   u32 buf_size ) {
    // Obtain an interface to the input device
    //. CHK_DX( pDI->CreateDeviceEx( guidDevice, IID_IDirectInputDevice8,
    //(void**)device, NULL ) );
    CHK_DX( pDI->CreateDevice( guidDevice, /*IID_IDirectInputDevice8,*/ device,
                               NULL ) );
    ...
    {
        HRESULT _hr =
            ( *device )->SetCooperativeLevel( RDEVICE.m_hWnd, dwFlags );
...
void CInput::KeyUpdate() {
    ...
    hr = pKeyboard->GetDeviceData( sizeof( DIDEVICEOBJECTDATA ), &od[ 0 ],
                                   &dwElements, 0 );
    ...
    if ( !b_altF4 && KBState[ DIK_F4 ] &&
         ( KBState[ DIK_RMENU ] || KBState[ DIK_LMENU ] ) ) {
        b_altF4 = TRUE;
...
#ifndef _EDITOR
    const bool dispatch_input =
        !Device.dwPrecacheFrame && !( pApp && pApp->LoadSessionActive() );
    if ( dispatch_input )
#endif
    {
...
#ifndef _EDITOR
                if ( key == DIK_TAB && ( iGetAsyncKeyState( DIK_RMENU ) ||
                                         iGetAsyncKeyState( DIK_LMENU ) ) )
                    b_alt_tab = true;
#endif
            }
        }
...
bool CInput::get_dik_name( int dik, LPSTR dest_str, int dest_sz ) {
    ...
    int cnt = WideCharToMultiByte( CP_ACP, 0, keyname.wsz, -1, dest_str,
                                   dest_sz, NULL, NULL );
    if ( cnt == -1 ) {
        Msg( "! cant convert dik_name for dik[%d], prop=[%S]", dik,
             keyname.wsz );
        return false;
    }
    return ( cnt != -1 );
...
bool CInput::dik_to_text( int dik,
                          bool shift,
                          bool caps,
                          bool ctrl,
                          bool alt,
    ...
    wchar_t wbuf[ 8 ] = {};
    const int rc =
        ToUnicodeEx( vk, scan, ks, wbuf, ( int )std::size( wbuf ), 0, hkl );
    ...
    if ( rc <= 0 ) {
        ...
        if ( rc == -1 ) {
            BYTE ks0[ 256 ] = {};
            wchar_t dummy[ 8 ] = {};
            ToUnicodeEx( vk, scan, ks0, dummy, ( int )std::size( dummy ), 0,
                         hkl );
        }
        return false;
    }
    ...
    const int mb = WideCharToMultiByte( 1251, WC_NO_BEST_FIT_CHARS, wbuf, rc,
                                        dest, dest_sz - 1, "?", &usedDefault );
    if ( mb <= 0 )
        return false;
...
#endif // #ifdef INGAME_EDITOR
                        RDEVICE.m_hWnd,
        ( exclusive ? DISCL_EXCLUSIVE : DISCL_NONEXCLUSIVE ) |
            DISCL_FOREGROUND );
    pKeyboard->Acquire();
