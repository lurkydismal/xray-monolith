...
void _draw_cam_pos( CGameFont* pFont ) {
    ...
    pFont->Out( 10, 600, "CAMERA POSITION:  [%3.2f,%3.2f,%3.2f]",
                VPUSH( Device.vCameraPosition ) );
    pFont->SetHeight( sz );
    pFont->OnRender();
}
...
void CStats::Show() {
    ...
    if ( Device.fTimeDelta > EPS_S ) {
        ...
        if ( RenderTOTAL.result > EPS_S ) {
            ...
            fTPS = fInv * fTPS + fOne * float( rendered_polies ) /
                                     ( RenderTOTAL.result * 1000.f );
            // fTPS = fInv*fTPS +
    ...
    if ( vtune.enabled() ) {
        ...
        pFont->OutSet(
            Device.dwWidth / 2.0f + ( pFont->SizeOf_( "--= tune =--" ) / 2.0f ),
...
#define PPP( a ) ( 100.f * float( a ) / float( EngineTOTAL.result ) )
        ...
        F.OutNext(
            "uClients:    %2.2fms, %2.1f%%, crow(%d)/active(%d)/total(%d)",
        ...
        F.OutNext( "aiThink:     %2.2fms, %d", AI_Think.result,
                   AI_Think.count );
        F.OutNext( "  aiRange:   %2.2fms, %d", AI_Range.result,
                   AI_Range.count );
...
#define PPP( a ) ( 100.f * float( a ) / float( RenderTOTAL.result ) )
        ...
        F.OutNext( "clRAY:       %2.2fms, %d, %2.0fK", clRAY.result,
                   clRAY.count, r_ps );
        F.OutNext( "clBOX:       %2.2fms, %d, %2.0fK", clBOX.result,
                   clBOX.count, b_ps );
        ...
        F.OutNext( "netServer:   %2.2fms, %d", netServer.result,
                   netServer.count );
        F.OutNext( "netClientCompressor:   %2.2fms",
                   netClientCompressor.result );
        F.OutNext( "netServerCompressor:   %2.2fms",
                   netServerCompressor.result );
       ...
       //  F.OutSet (640,0);
        ...
        RCache.stat.r.s_flora_lods.verts/1024.f, RCache.stat.r.s_flora_lods.dips
        ); F.OutNext ("dynamic:       %3.1f/%d",
        RCache.stat.r.s_dynamic.verts/1024.f, RCache.stat.r.s_dynamic.dips );
        F.OutNext ("  dynamic_sw:  %3.1f/%d",
        RCache.stat.r.s_dynamic_sw.verts/1024.f, RCache.stat.r.s_dynamic_sw.dips
        ...
        RCache.stat.r.s_dynamic_1B.dips ); F.OutNext ("  dynamic_2B:  %3.1f/%d",
        RCache.stat.r.s_dynamic_2B.verts/1024.f, RCache.stat.r.s_dynamic_2B.dips
    ...
    if ( /*psDeviceFlags.test(rsStatistic) ||*/ psDeviceFlags.test(
        rsCameraPos ) ) {
        _draw_cam_pos( pFont );
        pFont->OnRender();
    };
    ...
    if ( !g_bDisableRedText ) {
        ...
        if ( psDeviceFlags.test( rsStatistic ) ) {
            ...
            F.OutSkip();
            // if (fMem_calls>1500)   F.OutNext ("MMGR calls > 1500:%3.1f",
            // fMem_calls);
...
#else
        for ( u32 it = ( u32 )_max( int( 0 ),
                                    ( int )errors.size() - g_ErrorLineCount );
              it < errors.size(); it++ )
            F.OutNext( "%s", errors[ it ].c_str() );
#endif
        F.OnRender();
        ...
        for ( ; _I != _E; _I++ ) {
            ...
            if ( item._3D ) {
                ...
                if ( g_stats_flags.is( st_sound_min_dist ) )
                    DU->DrawSphere( Fidentity, item.params.position,
                                    item.params.min_distance, 0x400000FF,
                                    0xFF0000FF, true, true );
                if ( g_stats_flags.is( st_sound_max_dist ) )
                    DU->DrawSphere( Fidentity, item.params.position,
                                    item.params.max_distance, 0x4000FF00,
                                    0xFF008000, true, true );

                xr_string out_txt =
                    ( out_txt.size() && g_stats_flags.is( st_sound_info_name ) )
                        ? item.name.c_str()
                        : "";
                ...
                if ( item.game_object ) {
                    if ( g_stats_flags.is( st_sound_ai_dist ) )
                        DU->DrawSphere( Fidentity, item.params.position,
                                        item.params.max_ai_distance, 0x80FF0000,
                ...
                if ( g_stats_flags.is_any( st_sound_info_name |
                                           st_sound_info_object ) &&
                     item.name.size() )
                    DU->OutText( item.params.position, out_txt.c_str(),
