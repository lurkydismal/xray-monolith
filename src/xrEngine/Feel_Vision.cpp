...
struct SFeelParam {
    ...
    SFeelParam( Vision* _parent,
                Vision::feel_visible_Item* _item,
...
IC BOOL feel_vision_test_callback( const collide::ray_defs& rd,
                                   CObject* object,
                                   LPVOID user_data ) {
...
void Vision::feel_vision_relcase( CObject* object ) {
    {
        xrSRWLockGuard guard( &lock_query, false );
        seen.erase( std::remove( seen.begin(), seen.end(), object ),
                    seen.end() );
        query.erase( std::remove( query.begin(), query.end(), object ),
                     query.end() );
        diff.erase( std::remove( diff.begin(), diff.end(), object ),
                    diff.end() );
    }
    {
        xrSRWLockGuard guard( &lock_visible, false );
        feel_visible.erase(
            std::remove_if( feel_visible.begin(), feel_visible.end(),
                            [ object ]( const feel_visible_Item& item ) {
...
void Vision::feel_vision_query( Fmatrix& mFull ) {
    ...
    if ( seen.size() > 1 ) {
        ...
        xr_vector< CObject* >::iterator end =
...
void Vision::feel_vision_update( Fvector& P, float dt, float vis_threshold ) {
    PROF_EVENT( "feel_vision_update" );
    {
        ...
        if ( !query.empty() ) {
            diff.resize( _max( seen.size(), query.size() ) );
            xr_vector< CObject* >::iterator E =
                std::set_difference( query.begin(), query.end(), seen.begin(),
                                     seen.end(), diff.begin() );
        ...
        if ( f > fuzzy_guaranteed ) {
            ...
            if ( I->Cache.result && I->Cache.similar( P, D, f ) ) {
                ...
                if ( CDB::TestRayTri( P, D, I->Cache.verts, _u, _v, _range,
                    ...
                    auto cb = []( collide::rq_result& result,
                                  LPVOID params ) -> BOOL {
                        SFeelParam* fp = ( SFeelParam* )params;
                        float vis = fp->parent->feel_vision_mtl_transp(
                            result.O, result.element );
                        fp->vis *= vis;
                        if ( nullptr == result.O && fis_zero( vis ) ) {
                            ...
                            fp->item->Cache.verts[ 0 ].set(
                                V[ T->verts[ 0 ] ] );
                            fp->item->Cache.verts[ 1 ].set(
                                V[ T->verts[ 1 ] ] );
                            fp->item->Cache.verts[ 2 ].set(
                                V[ T->verts[ 2 ] ] );
                        }
                    ...
                    if ( g_pGameLevel->ObjectSpace.RayQuery(
                             RQR, RD, cb, &feel_params,
                             feel_vision_test_callback, m_owner ) )
#else
                    if ( g_pGameLevel->ObjectSpace.RayQuery(
                             RQR, RD, cb, &feel_params, nullptr, m_owner ) )
#endif
                    {
...
#ifdef SPATIAL_CHANGE
                if ( object->SpatialComponent->spatial.type &
                     STYPE_FEELVISIONIGNORE )
