...
void XRay::Engine::PreRenderThread() {
    ...
    if ( g_pGamePersistent && g_pGamePersistent->pEnvironment &&
...
void XRay::Engine::CalculateBonesThread() {
    ...
    ViewBase.CreateFromMatrix( Device.mFullTransform_saved,
                               FRUSTUM_P_LRTB | FRUSTUM_P_FAR );

    static xr_vector< ISpatialShared > spatials = {};
    spatials.clear();
    g_SpatialSpace->q_sphere(
        spatials, ISpatial_DB::O_ORDERED,
        STYPE_RENDERABLE + STYPE_RENDERABLESHADOW + STYPE_PARTICLE +
            STYPE_LIGHTSOURCE,
    ...
    spatialsSnapshot.clear();
    {
        for ( ISpatialShared spatial : spatials ) {
            ...
            float distSq = Device.vCameraPosition_saved.distance_to_sqr(
                spatial->spatial.sphere.P );
            if ( !ViewBase.testSphere_dirty( spatial->spatial.sphere.P,
            ...
            if ( ( spatial->spatial.type &
                   ( STYPE_PARTICLE | STYPE_LIGHTSOURCE ) ) ||
            ...
            IKinematics* pKin =
                renderable->renderable.visual->dcast_PKinematics();
            if ( !pKin )
                continue;
    ...
    static auto sortFunc = []( const SpatialSnapshot& _1,
                               const SpatialSnapshot& _2 ) noexcept {
...
void XRay::Engine::GameThread() {
    ...
    // demonized: While Renderer prepares frame and GPU renders it, use time
    // opportunity to repeatedly call Lua GC with small step value Reduces
    // stutters since less work will be done in main GC step or no work at all
    static auto LuaGC = []() {
        PROF_EVENT( "seqLuaGC" );
        // Do at least once
        do {
