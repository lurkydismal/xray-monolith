...
CEnvironment::CEnvironment() : CurrentEnv( 0 ), m_ambients_config( 0 ) {
    ...
    CopyMemory( &CloudsVerts.front(), verts,
                CloudsVerts.size() * sizeof( Fvector ) );
    const u16* indices;
    CloudsIndices.resize( xrHemisphereIndices( 2, indices ) );
    CopyMemory( &CloudsIndices.front(), indices,
                CloudsIndices.size() * sizeof( u16 ) );
    ...
    m_ambients_config =
        xr_new< CInifile >( FS.update_path( file_name, "$game_config$",
    ...
    m_effects_config =
        xr_new< CInifile >( FS.update_path( file_name, "$game_config$",
                            ...
                            TRUE, TRUE, FALSE );
    m_suns_config = xr_new< CInifile >(
        FS.update_path( file_name, "$game_config$", "environment\\suns.ltx" ),
        TRUE, TRUE, FALSE );
    m_sun_pos_config =
        xr_new< CInifile >( FS.update_path( file_name, "$game_config$",
    ...
    m_thunderbolt_collections_config = xr_new< CInifile >(
        FS.update_path( file_name, "$game_config$",
    ...
    p_min_dist =
        _min( .95f, config->r_float( "environment", "min_dist_factor" ) );
...
void CEnvironment::SetGameTime( float game_time, float time_factor ) {
    ...
    if ( m_paused ) {
        g_pGameLevel->SetEnvironmentGameTimeFactor(
            iFloor( fGameTime * 1000.f ), fTimeFactor );
        return;
    }
#endif
    if ( bWFX )
...
bool CEnvironment::SetWeatherFX( shared_str name ) {
    ...
    if ( name.size() ) {
        ...
        R_ASSERT3( it != WeatherFXs.end(), "Invalid weather effect name.",
                   *name );
        ...
        if ( Current[ 0 ]->exec_time > Current[ 1 ]->exec_time ) {
            ...
            current_length = ( DAY_LENGTH - Current[ 0 ]->exec_time ) +
                             Current[ 1 ]->exec_time;
            current_weight = x / current_length;
        } else {
            current_length = Current[ 1 ]->exec_time - Current[ 0 ]->exec_time;
            current_weight =
                ( fGameTime - Current[ 0 ]->exec_time ) / current_length;
        }
        clamp( current_weight, 0.f, 1.f );

        std::sort( CurrentWeather->begin(), CurrentWeather->end(),
                   sort_env_etl_pred );
        ...
        C0->exec_time = NormalizeTime(
            fGameTime -
            ( ( rewind_tm / ( Current[ 1 ]->exec_time - fGameTime ) ) *
                  current_length -
              rewind_tm ) );
        C1->copy( *Current[ 1 ] );
        C1->exec_time = NormalizeTime( start_tm );
        for ( EnvIt t_it = CurrentWeather->begin() + 2;
              t_it != CurrentWeather->end() - 1; t_it++ )
            ( *t_it )->exec_time =
                NormalizeTime( start_tm + ( *t_it )->exec_time_loaded );
        SelectEnv( PrevWeather, WFX_end_desc[ 0 ], CE->exec_time );
        SelectEnv( PrevWeather, WFX_end_desc[ 1 ],
                   WFX_end_desc[ 0 ]->exec_time + 0.5f );
        ...
        // for (EnvIt l_it=CurrentWeather->begin(); l_it!=CurrentWeather->end();
        // l_it++) Msg (". Env: '%s' Tm:
        // %3.2f",*(*l_it)->m_identifier.c_str(),(*l_it)->exec_time);
#endif
    } else {
#ifndef _EDITOR
...
bool CEnvironment::StartWeatherFXFromTime( shared_str name, float time ) {
    ...
    for ( EnvIt it = CurrentWeather->begin(); it != CurrentWeather->end();
          it++ )
        ( *it )->exec_time =
            NormalizeTime( ( *it )->exec_time - wfx_time + time );
...
#ifdef WEATHER_LOGGING
    Msg( "WFX - end. Weather: '%s' Desc: '%s'/'%s' GameTime: %3.2f",
         CurrentWeatherName.c_str(), Current[ 0 ]->m_identifier.c_str(),
         Current[ 1 ]->m_identifier.c_str(), fGameTime );
#endif
}
...
void CEnvironment::SelectEnvs( EnvVec* envs,
                               CEnvDescriptor*& e0,
                               CEnvDescriptor*& e1,
                               float gt ) {
    EnvIt env = std::lower_bound( envs->begin(), envs->end(), gt, lb_env_pred );
...
#ifdef WEATHER_LOGGING
            Msg( "Weather: '%s' Desc: '%s' Time: %3.2f/%3.2f",
                 CurrentWeatherName.c_str(), Current[ 1 ]->m_identifier.c_str(),
...
void CEnvironment::lerp( float& current_weight ) {
    ...
    current_weight = TimeWeight( fGameTime, Current[ 0 ]->exec_time,
                                 Current[ 1 ]->exec_time );
    // modifiers
    CEnvModifier EM;
    EM.far_plane = 0;
    ...
    float mpower = 0;
    for ( xr_vector< CEnvModifier >::iterator mit = Modifiers.begin();
          mit != Modifiers.end(); mit++ )
        mpower += EM.sum( *mit, view );

    // final lerp
    if ( !m_paused )
        CurrentEnv->lerp( this, *Current[ 0 ], *Current[ 1 ], current_weight,
                          EM, mpower );
...
void CEnvironment::set_lerp( float current_weight ) {
    ...
    float new_t0 =
        NormalizeTime( fGameTime - current_weight * interval_length );
...
void CEnvironment::OnFrame() {
    ...
    float WindVel =
        _max( CurrentEnv->wind_velocity, ps_ssfx_wind_trees.w * 1000 );
    ...
    if ( !::Render->is_sun_static() && fGameTime > 18000.f &&
         fGameTime < 79000.f )
    ...
    VERIFY2( CurrentEnv->sun_dir.y < 0,
             "Invalid sun direction settings in lerp" );
#endif // #ifndef MASTER_GOLD

    PerlinNoise1D->SetFrequency( wind_gust_factor * MAX_NOISE_FREQ );
    wind_strength_factor = clampr(
        PerlinNoise1D->GetContinious( Device.fTimeGlobal ) + 0.5f, 0.f, 1.f );
    ...
    if ( m_paused ) {
        ...
        l_id = ( current_weight < 0.5f ) ? Current[ 0 ]->lens_flare_id
                                         : Current[ 1 ]->lens_flare_id;
        t_id = ( current_weight < 0.5f ) ? Current[ 0 ]->tb_id
                                         : Current[ 1 ]->tb_id;
    }

    eff_LensFlare->OnFrame( l_id );
    eff_Thunderbolt->OnFrame( t_id, CurrentEnv->bolt_period,
                              CurrentEnv->bolt_duration );
    eff_Rain->OnFrame();
...
void CEnvironment::calculate_config_sun_dir() {
    ...
    CurrentEnv->sun_dir.setHP( deg2rad( real_sun_alt ),
...
void CEnvironment::calculate_dynamic_sun_dir() {
    ...
    // Declination
    float D = 0.396372f - 22.91327f * _cos( g ) + 4.02543f * _sin( g ) -
    ...
    float TC = 0.004297f + 0.107029f * _cos( g ) - 1.837877f * _sin( g ) -
    ...
    float cosSZA =
        _sin( LatitudeR ) * _sin( deg2rad( D ) ) +
        _cos( LatitudeR ) * _cos( deg2rad( D ) ) * _cos( deg2rad( SHA ) );
    ...
    if ( !fis_zero( sin_SZA_X_cos_Latitude ) )
        cosAZ = ( _sin( deg2rad( D ) ) - _sin( LatitudeR ) * _cos( SZA ) ) /
                sin_SZA_X_cos_Latitude;

    clamp( cosAZ, -1.0f, 1.0f );
    float AZ = acosf( cosAZ ) + PI; // AVO: sun direction fix

    const Fvector2 minAngle =
        Fvector2().set( deg2rad( 1.0f ), deg2rad( 3.0f ) );
...
SThunderboltDesc* CEnvironment::thunderbolt_description(
    CInifile& config,
...
SThunderboltCollection* CEnvironment::thunderbolt_collection(
    xr_vector< SThunderboltCollection* >& collection,
    shared_str const& id ) {
...
CLensFlareDescriptor* CEnvironment::add_flare(
    xr_vector< CLensFlareDescriptor* >& collection,
    shared_str const& id ) {
