...
extern int PASCAL IntroDSHOW_wnd( HINSTANCE hInstC,
                                  HINSTANCE hInstP,
                                  LPSTR lpCmdLine,
                                  int nCmdShow );
...
ULONGLONG startup_begin_time;

void LogStartupMenuReady() {
    static bool logged = false;
    if ( !logged ) {
        logged = true;
        Msg( "* [STARTUP] total to main menu: %llu ms",
             GetTickCount64() - startup_begin_time );
    }
...
static LPCSTR month_id[ 12 ] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                 "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };

static int days_in_month[ 12 ] = { 31, 28, 31, 30, 31, 30,
                                   31, 31, 30, 31, 30, 31 };
...
PROTECT_API void InitSettings() {
    ...
    tmp_functor.bind( &tmp_excluder,
                      &path_excluder_predicate::is_allow_include );
    pSettingsAuth =
        xr_new< CInifile >( systemPath, TRUE, TRUE, FALSE, 0, tmp_functor );
...
LPCSTR xr_ToUTF8( LPCSTR input, int max_length ) {
    ...
    int32_t conv_len =
        ucnv_toUChars( conv_from, &converted[ 0 ], converted.size(), input,
                       strlen( input ), &errorCode );
    if ( errorCode != U_ZERO_ERROR ) {
        Msg( "[Discord RPC] Failed to convert string! (%s)",
             std::to_string( errorCode ).c_str() );
        return input;
    }
    ...
    // needs to be static so the data buffer is still valid after this function
    // returns
    static xr_string g;
    g.clear();
    ...
    int32_t u8_len =
        ucnv_fromUChars( conv_u8, &g[ 0 ], g.size(), &converted[ 0 ],
                         converted.size(), &errorCode );
    if ( errorCode != U_ZERO_ERROR ) {
        Msg( "[Discord RPC] Failed to convert string! (%s)",
             std::to_string( errorCode ).c_str() );
        return input;
...
void updateDiscordPresence() {
    ...
    // In Game
        ...
        if ( discord_gameinfo.level_name && discord_gameinfo.currenttime ) {
            snprintf( levelname_time, 128, "%s | %s",
                      discord_gameinfo.level_name,
        ...
        if ( discord_gameinfo.faction && discord_gameinfo.faction_name ) {
            discordPresence.GetAssets().SetSmallImage(
                discord_gameinfo.faction );
            char rank_faction_rep[ 128 ];
            if ( discord_gameinfo.rank_name && discord_gameinfo.reputation )
                snprintf( rank_faction_rep, 128, "%s | %s",
                          discord_gameinfo.rank_name,
                          discord_gameinfo.reputation );
            else
                snprintf( rank_faction_rep, 128,
                          discord_gameinfo.faction_name );
        ...
        if ( discord_gameinfo.gamemode ) {
            if ( discord_gameinfo.task_name &&
                ...
                snprintf( details_buffer, 128, "%s | %s",
                          discord_gameinfo.gamemode,
        ...
        // Health
            ...
            if ( discord_gameinfo.ironman && discord_gameinfo.lives_left ) {
                if ( discord_gameinfo.lives_left == 0 ||
                     discord_gameinfo.lives_left > 1 )
                    snprintf( state_buffer, 128, "%s: %i | %i %s",
                              discord_strings.health, discord_gameinfo.health,
                              discord_gameinfo.lives_left,
            ...
            if ( discord_gameinfo.ironman && discord_gameinfo.lives_left ) {
                ...
                if ( real_lives == 0 || real_lives > 1 )
                    snprintf( state_buffer, 128, "%s | %i %s",
                              discord_strings.dead, real_lives,
                              discord_strings.livesleft );
                else
                    snprintf( state_buffer, 128, "%s | %i %s",
                              discord_strings.dead, real_lives,
    ...
    discord_core->ActivityManager().UpdateActivity(
        discordPresence, []( discord::Result result ) {} );
}

void Init_Discord() {
    auto result = discord::Core::Create( 477910171964801060,
...
void Startup() {
    ...
    execUserScript();
    InitSound2();
    Msg( "* [STARTUP] sound and user config: %d ms",
         phase_timer.GetElapsed_ms() );
    ...
    // ...command line for auto start
    {
        ...
        if ( Core.ParamsData.test( ECoreParams::start ) )
            Console->Execute( pStartup + 1 );
    }
    {
        LPCSTR pStartup = strstr( Core.Params, "-load " );
        if ( Core.ParamsData.test( ECoreParams::load ) )
            Console->Execute( pStartup + 1 );
    }
    ...
    g_pGamePersistent =
        ( IGame_Persistent* )NEW_INSTANCE( CLSID_GAME_PERSISTANT );
    g_SpatialSpace = xr_new< ISpatial_DB >();
    g_SpatialSpacePhysic = xr_new< ISpatial_DB >();
    g_SpatialSpaceLights = xr_new< ISpatial_DB >();
    Msg( "* [STARTUP] application and game persistent: %d ms",
         phase_timer.GetElapsed_ms() );
    ...
    if ( use_reshade )
        ...
        Msg( "[ReShade]: ReShade not installed or version too old - didn't "
             "load compatibility addon" );
...
_declspec( dllexport ) DWORD AmdPowerXpressRequestHighPerformance =
    0x00000001; // PowerXpress or Hybrid Graphics
}
...
for (u32 git=0; git<in_size; git++) p_in[git] = (u8)::Random.randI (8); //
garbage bytes += in_size;
...
for (u32 tit=0; tit<in_size; tit++) R_ASSERT(p_in[tit] == p_in_tst[tit]); //
garbage
...
{
IntroDSHOW_wnd
(g_hInstance,g_hPrevInstance,"GameData\\Stalker_Intro.avi",g_nCmdShow);
...
struct damn_keys_filter {
    ...
    damn_keys_filter() {
        ...
        SystemParametersInfo( SPI_GETSTICKYKEYS, dwStickyKeysStructSize,
                              ( PVOID )&StickyKeysStruct, 0 );
        SystemParametersInfo( SPI_GETFILTERKEYS, dwFilterKeysStructSize,
                              ( PVOID )&FilterKeysStruct, 0 );
        SystemParametersInfo( SPI_GETTOGGLEKEYS, dwToggleKeysStructSize,
...
int APIENTRY WinMain_impl( HINSTANCE hInstance,
    ...
    if ( !IsDebuggerPresent() ) {
        ...
        typedef BOOL( __stdcall * HeapSetInformation_type )(
            HANDLE, HEAP_INFORMATION_CLASS, PVOID, SIZE_T );
        HeapSetInformation_type const heap_set_information =
            ( HeapSetInformation_type )GetProcAddress( kernel32,
                                                       "HeapSetInformation" );
...
#define STALKER_PRESENCE_MUTEX "Local\\STALKER-COP"
    ...
    string256 mutexName = {};
    xr_sprintf( mutexName, sizeof( mutexName ), STALKER_PRESENCE_MUTEX "_%08x",
                pathHash );
    ...
    // Title window
    logoWindow = CreateDialog( GetModuleHandle( NULL ),
                               MAKEINTRESOURCE( IDD_STARTUP ), 0, logDlgProc );
    ...
    SetWindowPos( logoWindow,
                  ...
                  HWND_NOTOPMOST,
#endif // NDEBUG
                  x, y, splashW, splashH, SWP_SHOWWINDOW );
    ...
    Msg( "* [STARTUP] core and filesystem: %llu ms",
         GetTickCount64() - early_phase_time );
...

    {
        ...
        for ( FS_FileSet::iterator it = fset.begin(); it != fset.end(); it++ ) {
            // skip virtual files from .db? archives, only interested in loose
            // files
            if ( ( *it ).attrib != 0 )
                continue;
        ...
        Msg( "* [STARTUP] gamedata listing: %d ms",
             startup_phase_timer.GetElapsed_ms() );
    }
...
#ifndef DEDICATED_SERVER
    {
        ...
        Msg( "* [STARTUP] engine/input/console: %d ms",
             startup_phase_timer.GetElapsed_ms() );
        ...
        if ( Core.ParamsData.test( ECoreParams::r2a ) )
            ...
            CCC_LoadCFG_custom* pTmp =
                xr_new< CCC_LoadCFG_custom >( "renderer " );
            pTmp->Execute( Console->ConfigFile );
            xr_delete( pTmp );
        }
        Msg( "* [STARTUP] renderer config: %d ms",
             startup_phase_timer.GetElapsed_ms() );
#else
    Console->Execute( "renderer renderer_r1" );
        ...
        if ( /*xr_strlen(g_sLaunchOnExit_params) && */ xr_strlen(
            g_sLaunchOnExit_app ) ) {
            ...
            char const* temp_wf = ( xr_strlen( g_sLaunchWorkingFolder ) > 0 )
                                      ...
                                      : NULL;
            CreateProcess( g_sLaunchOnExit_app, g_sLaunchOnExit_params, NULL,
                           NULL, FALSE, 0, NULL, temp_wf, &si, &pi );
        }
#ifndef DEDICATED_SERVER
#ifdef NO_MULTI_INSTANCES
...
extern BOOL DllMainXrCore( HANDLE hinstDLL,
                           DWORD ul_reason_for_call,
                           LPVOID lpvReserved );
extern BOOL DllMainXrPhysics( HANDLE hModule,
                              DWORD ul_reason_for_call,
                              LPVOID lpReserved );

// extern BOOL DllMainXrGame(HANDLE hModule, u32 ul_reason_for_call, LPVOID
// lpReserved);
//
// extern BOOL DllMainXrRenderR1(HANDLE hModule, DWORD  ul_reason_for_call,
// LPVOID lpReserved); extern BOOL DllMainXrRenderR2(HANDLE hModule, DWORD
// ul_reason_for_call, LPVOID lpReserved); extern BOOL DllMainXrRenderR3(HANDLE
// hModule, DWORD  ul_reason_for_call, LPVOID lpReserved); extern BOOL
...
void CApplication::OnEvent( EVENT E, u64 P1, u64 P2 ) {
    if ( E == eQuit ) {
        ...
        LoadSessionStartEvent(
            !xr_strcmp( game_params.m_new_or_load, "new" )    ? "new-game"
            : !xr_strcmp( game_params.m_new_or_load, "load" ) ? "menu-save"
                                                              : "start" );
        Level_Current = u32( -1 );
        ...
        if ( g_pGameLevel ) {
            ...
            if ( ( FALSE == Engine.Event.Peek( "KERNEL:quit" ) ) &&
                 ( FALSE == Engine.Event.Peek( "KERNEL:start" ) ) ) {
                Console->Execute( "main_menu off" );
                Console->Execute( "main_menu on" );
            }
        }
        ...
        g_pGamePersistent->Start(
            "" ); // server_options.c_str()); - no prefetch !
...
void CApplication::LoadSessionBegin( LPCSTR scenario ) {
	...
	{
#endif
    m_load_session.native_generation =
        NativeLoadExecutor::Instance().BeginGeneration();
    if ( Device.m_pRender )
        m_load_session.resource_generation =
            Device.m_pRender->ResourcesBeginLoadGeneration();
#if 0
	}
	catch (...)
	{
...
void CApplication::LoadSessionCancel( LPCSTR reason ) {
    ...
    if ( m_load_session.native_generation )
        NativeLoadExecutor::Instance().CancelGeneration(
            m_load_session.native_generation );
#if 0
	}
	catch (...)
    ...
    if ( Device.m_pRender && m_load_session.resource_generation )
        Device.m_pRender->ResourcesAbortLoadGeneration(
            m_load_session.resource_generation );
#if 0
	}
	catch (...)
...
#endif
    Msg( "* [load-session] cancelled scenario=%s after %u ms (%s)",
         m_load_session.scenario,
         Device.TimerAsync() - m_load_session.started_at,
...
void CApplication::LoadSessionPhaseBegin( ELoadSessionPhase phase ) {
    if ( !m_load_session.active || phase >= LoadSessionPhaseCount ||
         m_load_session.phase_running[ phase ] )
        return;
...
void CApplication::LoadSessionPhaseEnd( ELoadSessionPhase phase ) {
    ...
    m_load_session.phase_elapsed[ phase ] +=
        Device.TimerAsync() - m_load_session.phase_started_at[ phase ];
...
void CApplication::LoadSessionRecordPrecacheFrame( u64 wall_ticks,
                                                   u64 frame_move_ticks,
                                                   u64 seq_render_ticks,
                                                   u64 end_ticks,
                                                   u64 secondary_wait_ticks ) {
...
void CApplication::LoadSessionRecordPrecacheLevel( u64 calculate_ticks,
                                                   u64 render_ticks ) {
    if ( !m_load_session.active || !m_load_session.precache_started )
        return;
...
void CApplication::LoadSessionRecordClientEvent( bool spawn,
                                                 u16 destination,
                                                 u16 type,
...
void CApplication::LoadSessionTryFinish( bool level_ready,
                                         bool control_ready,
                                         bool queues_drained ) {
    if ( !m_load_session.active || !m_load_session.precache_started ||
         Device.dwPrecacheFrame || !g_loading_events.empty() || !level_ready ||
         !control_ready || !queues_drained )
        return;
...
#endif
    if ( m_load_session.native_generation )
        NativeLoadExecutor::Instance().FinalizeGeneration(
            m_load_session.native_generation );
#if 0
	}
	catch (...)
...
#endif
    if ( Device.m_pRender && m_load_session.resource_generation )
        Device.m_pRender->ResourcesFinalizeLoadGeneration(
            m_load_session.resource_generation );
    if ( Device.m_pRender )
        Device.m_pRender->ResourcesDestroyNecessaryTextures();
    ...
    Msg( "* [load-session] engine ready scenario=%s: %u ms",
         m_load_session.scenario, now - m_load_session.started_at );
    Msg( "* [load-session] client order: hash=%016llx, spawns=%u, events=%u",
         m_load_session.client_event_hash, m_load_session.client_spawn_count,
         m_load_session.client_event_count );
    Msg( "* [load-session] phases: teardown=%u ms, server/lua=%u ms, native "
         "level=%u ms, "
    ...
    const u64 serial_ticks = m_load_session.precache_frame_move_ticks +
                             m_load_session.precache_seq_render_ticks +
                             m_load_session.precache_end_ticks +
                             m_load_session.precache_secondary_wait_ticks;
    const u64 serial_other_ticks =
        m_load_session.precache_wall_ticks > serial_ticks
            ? m_load_session.precache_wall_ticks - serial_ticks
            : 0;
    const u64 measured_render_ticks =
        m_load_session.precache_level_calculate_ticks +
        m_load_session.precache_level_render_ticks +
        m_load_session.precache_loadscreen_ticks;
    const u64 render_other_ticks =
        m_load_session.precache_seq_render_ticks > measured_render_ticks
            ? m_load_session.precache_seq_render_ticks - measured_render_ticks
            : 0;
    Msg( "* [load-session] precache perf: frames=%u, "
         "calls(level/loadscreen/present)=%u/%u/%u, "
         "wall=%.2f ms, frame move=%.2f ms, seq render=%.2f ms, level "
         "calculate/render=%.2f/%.2f ms, "
         ...
         m_load_session.precache_loadscreen_calls,
         m_load_session.precache_present_calls,
         to_ms( m_load_session.precache_wall_ticks ),
         to_ms( m_load_session.precache_frame_move_ticks ),
         to_ms( m_load_session.precache_seq_render_ticks ),
         to_ms( m_load_session.precache_level_calculate_ticks ),
         to_ms( m_load_session.precache_level_render_ticks ),
         to_ms( m_load_session.precache_loadscreen_ticks ),
         to_ms( render_other_ticks ),
         to_ms( m_load_session.precache_end_ticks ),
         to_ms( m_load_session.precache_present_ticks ),
         to_ms( m_load_session.precache_secondary_wait_ticks ),
         to_ms( serial_other_ticks ) );
...
void CApplication::Level_Append( LPCSTR folder ) {
    ...
    if ( FS.exist( "$game_levels$", N1 ) && FS.exist( "$game_levels$", N2 ) &&
         FS.exist( "$game_levels$", N3 ) && FS.exist( "$game_levels$", N4 ) ) {
        sLevelInfo LI;
...
void CApplication::Level_Scan() {
    ...
    xr_vector< char* >* folder =
        FS.file_list_open( "$game_levels$", FS_ListFolders | FS_RootOnly );
...
void InitLauncher() {
    ...
    if ( 0 == hLauncher )
        R_CHK( GetLastError() );
    R_ASSERT2( hLauncher,
               "xrLauncher DLL raised exception during loading or there is no "
               "xrLauncher.dll at all" );

    pLauncher = ( LauncherFunc* )GetProcAddress( hLauncher, "RunXRLauncher" );
    R_ASSERT2( pLauncher,
               "Cannot obtain RunXRLauncher function from xrLauncher.dll" );
};
...
void CApplication::load_draw_internal() {
    ...
    back_text_coords.lt.set
    (0,0);back_text_coords.rb.add(back_text_coords.lt,back_size);
    back_coords.lt.set (offs, offs);
    back_coords.rb.add(back_coords.lt,back_size);

    back_coords.lt.mul (k);back_coords.rb.mul(k);

    back_text_coords.lt.x/=tsz.x; back_text_coords.lt.y/=tsz.y;
    back_text_coords.rb.x/=tsz.x; back_text_coords.rb.y/=tsz.y; pv = (FVF::TL*)
    RCache.Vertex.Lock(4,ll_hGeom.stride(),Offset); pv->set (back_coords.lt.x,
    back_coords.rb.y, C,back_text_coords.lt.x, back_text_coords.rb.y); pv++;
    pv->set (back_coords.lt.x, back_coords.lt.y, C,back_text_coords.lt.x,
    back_text_coords.lt.y); pv++; pv->set (back_coords.rb.x, back_coords.rb.y,
    C,back_text_coords.rb.x, back_text_coords.rb.y); pv++; pv->set
    (back_coords.rb.x, back_coords.lt.y, C,back_text_coords.rb.x,
    back_text_coords.lt.y); pv++; RCache.Vertex.Unlock (4,ll_hGeom.stride());
    ...
    back_text_coords.lt.set
    (0,768);back_text_coords.rb.add(back_text_coords.lt,back_size);
    back_coords.lt.set (379 ,726);back_coords.rb.add(back_coords.lt,back_size);

    back_coords.lt.mul (k);back_coords.rb.mul(k);

    back_text_coords.lt.x/=tsz.x; back_text_coords.lt.y/=tsz.y;
    back_text_coords.rb.x/=tsz.x; back_text_coords.rb.y/=tsz.y;
    ...
    pv->set (back_coords.lt.x+pos_delta*idx+offs, back_coords.rb.y+offs,
    0+EPS_S, 1, clr, back_text_coords.lt.x+tc_delta*idx, back_text_coords.rb.y);
    pv++; pv->set (back_coords.lt.x+pos_delta*idx+offs, back_coords.lt.y+offs,
    0+EPS_S, 1, clr, back_text_coords.lt.x+tc_delta*idx, back_text_coords.lt.y);
    pv++;
    }
