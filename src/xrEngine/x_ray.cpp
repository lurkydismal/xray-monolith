...
static LPSTR month_id[ 12 ] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
PROTECT_API void InitSettings() {
    ...
    pSettingsAuth =
        xr_new< CInifile >( fname, TRUE, TRUE, FALSE, 0, tmp_functor );
...
int APIENTRY WinMain( HINSTANCE hInstance,
                      ...
                      char* lpCmdLine,
                      int nCmdShow ) {
    // Initialize LuaJIT low-memory pool FIRST, before any DLLs load and
    // fragment the lower 2GB address space.
    XR_EARLY_INIT();

    // Enable per-monitor DPI awareness so GetMonitorInfo returns real pixel
    // sizes. Without this, monitors with different DPI scaling report wrong
    // resolutions (e.g. a 1920x1080 secondary monitor reports 2400x1290 when
    // primary is at 125%). Uses dynamic loading since _WIN32_WINNT is too old
    // for these APIs. Try Win10 1703+ API first, fall back to Win 8.1+ API,
    // silently skip on Win 7 or older.
    {
        bool dpi_set = false;
        HMODULE user32 = GetModuleHandleA( "user32.dll" );
        if ( user32 ) {
            typedef BOOL( WINAPI * pfnSetProcessDpiAwarenessContext )( HANDLE );
            auto fn = ( pfnSetProcessDpiAwarenessContext )GetProcAddress(
                user32, "SetProcessDpiAwarenessContext" );
            if ( fn )
                dpi_set = fn( /*DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2*/ (
                              HANDLE )-4 ) != FALSE;
        }
        if ( !dpi_set ) {
            ...
            if ( shcore ) {
                ...
                auto fn = ( pfnSetProcessDpiAwareness )GetProcAddress(
