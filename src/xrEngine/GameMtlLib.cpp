...
u64 MaterialPairKey( int first, int second ) {
    return ( u64( static_cast< u32 >( first ) ) << 32 ) |
...
void CGameMtlLibrary::Load() {
    ...
    for ( u16 index = 0; index < materials.size(); ++index ) {
        ...
        materialByName.emplace(
            NormalizeMaterialName( material->m_Name.c_str() ), material );
    ...
    if ( FS.exist( materialsLtxName, _game_data_, "materials\\materials",
                   ".ltx" ) ) {
        ...
        for ( const auto& sec : materialsLtx->sections() ) {
            ...
            if ( material == materials.end() ) {
                ...
                materialIndexById.emplace(
                    M->ID, static_cast< u16 >( materials.size() ) );
                materials.push_back( M );
                materialByName.emplace(
                    NormalizeMaterialName( M->m_Name.c_str() ), M );
                ++addedMaterials;
            } else {
                M = *material;
                ++changedMaterials;
            ...
            if ( materialsLtx->line_exist( M->m_Name, "desc" ) )
                M->m_Desc = materialsLtx->r_string( M->m_Name, "desc" );

            if ( materialsLtx->line_exist( M->m_Name, "flag_breakable" ) )
                M->Flags.set(
                    SGameMtl::flBreakable,
                    materialsLtx->r_bool( M->m_Name, "flag_breakable" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_bounceable" ) )
                M->Flags.set(
                    SGameMtl::flBounceable,
                    materialsLtx->r_bool( M->m_Name, "flag_bounceable" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_skidmark" ) )
                M->Flags.set(
                    SGameMtl::flSkidmark,
                    materialsLtx->r_bool( M->m_Name, "flag_skidmark" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_bloodmark" ) )
                M->Flags.set(
                    SGameMtl::flBloodmark,
                    materialsLtx->r_bool( M->m_Name, "flag_bloodmark" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_climable" ) )
                M->Flags.set(
                    SGameMtl::flClimable,
                    materialsLtx->r_bool( M->m_Name, "flag_climable" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_passable" ) )
                M->Flags.set(
                    SGameMtl::flPassable,
                    materialsLtx->r_bool( M->m_Name, "flag_passable" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_dynamic" ) )
                M->Flags.set(
                    SGameMtl::flDynamic,
                    materialsLtx->r_bool( M->m_Name, "flag_dynamic" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_liquid" ) )
                M->Flags.set(
                    SGameMtl::flLiquid,
                    materialsLtx->r_bool( M->m_Name, "flag_liquid" ) );
            if ( materialsLtx->line_exist( M->m_Name,
                                           "flag_suppress_shadows" ) )
                M->Flags.set( SGameMtl::flSuppressShadows,
                              materialsLtx->r_bool( M->m_Name,
                                                    "flag_suppress_shadows" ) );
            if ( materialsLtx->line_exist( M->m_Name,
                                           "flag_suppress_wallmarks" ) )
                M->Flags.set( SGameMtl::flSuppressWallmarks,
                              materialsLtx->r_bool(
                                  M->m_Name, "flag_suppress_wallmarks" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_actor_obstacle" ) )
                M->Flags.set(
                    SGameMtl::flActorObstacle,
                    materialsLtx->r_bool( M->m_Name, "flag_actor_obstacle" ) );
            if ( materialsLtx->line_exist( M->m_Name,
                                           "flag_bullet_no_ricochet" ) )
                M->Flags.set( SGameMtl::flNoRicoshet,
                              materialsLtx->r_bool(
                                  M->m_Name, "flag_bullet_no_ricochet" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_injurious" ) )
                M->Flags.set(
                    SGameMtl::flInjurious,
                    materialsLtx->r_bool( M->m_Name, "flag_injurious" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_shootable" ) )
                M->Flags.set(
                    SGameMtl::flShootable,
                    materialsLtx->r_bool( M->m_Name, "flag_shootable" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_transparent" ) )
                M->Flags.set(
                    SGameMtl::flTransparent,
                    materialsLtx->r_bool( M->m_Name, "flag_transparent" ) );
            if ( materialsLtx->line_exist( M->m_Name, "flag_slowdown" ) )
                M->Flags.set(
                    SGameMtl::flSlowDown,
                    materialsLtx->r_bool( M->m_Name, "flag_slowdown" ) );

            if ( materialsLtx->line_exist( M->m_Name, "friction" ) )
                M->fPHFriction = materialsLtx->r_float( M->m_Name, "friction" );
            if ( materialsLtx->line_exist( M->m_Name, "damping" ) )
                M->fPHDamping = materialsLtx->r_float( M->m_Name, "damping" );
            if ( materialsLtx->line_exist( M->m_Name, "spring" ) )
                M->fPHSpring = materialsLtx->r_float( M->m_Name, "spring" );
            if ( materialsLtx->line_exist( M->m_Name,
                                           "bounce_start_velocity" ) )
                M->fPHBounceStartVelocity =
                    materialsLtx->r_float( M->m_Name, "bounce_start_velocity" );
            if ( materialsLtx->line_exist( M->m_Name, "bouncing" ) )
                M->fPHBouncing = materialsLtx->r_float( M->m_Name, "bouncing" );
            if ( materialsLtx->line_exist( M->m_Name, "shoot_factor" ) )
                M->fShootFactor =
                    materialsLtx->r_float( M->m_Name, "shoot_factor" );
            if ( materialsLtx->line_exist( M->m_Name, "shoot_factor_mp" ) )
                M->fShootFactorMP =
                    materialsLtx->r_float( M->m_Name, "shoot_factor_mp" );
            if ( materialsLtx->line_exist( M->m_Name, "bounce_damage_factor" ) )
                M->fBounceDamageFactor =
                    materialsLtx->r_float( M->m_Name, "bounce_damage_factor" );
            if ( materialsLtx->line_exist( M->m_Name,
                                           "vis_transparency_factor" ) )
                M->fVisTransparencyFactor = materialsLtx->r_float(
                    M->m_Name, "vis_transparency_factor" );
            if ( materialsLtx->line_exist( M->m_Name,
                                           "sound_occlusion_factor" ) )
                M->fSndOcclusionFactor = materialsLtx->r_float(
                    M->m_Name, "sound_occlusion_factor" );
            if ( materialsLtx->line_exist( M->m_Name, "flotation_factor" ) )
                M->fFlotationFactor =
                    materialsLtx->r_float( M->m_Name, "flotation_factor" );
            if ( materialsLtx->line_exist( M->m_Name, "injurious_factor" ) )
                M->fInjuriousSpeed =
                    materialsLtx->r_float( M->m_Name, "injurious_factor" );
            if ( materialsLtx->line_exist( M->m_Name, "density_factor" ) )
                M->fDensityFactor =
                    materialsLtx->r_float( M->m_Name, "density_factor" );
        }
        xr_delete( materialsLtx );
        Msg( "* [materials.ltx] applied: added=%u changed=%u", addedMaterials,
             changedMaterials );
    }
    ...
    for ( const auto& mat : materials ) {
        ...
        Msg( "flag_breakable = %s",
             mat->Flags.test( SGameMtl::flBreakable ) ? "true" : "false" );
        Msg( "flag_bounceable = %s",
             mat->Flags.test( SGameMtl::flBounceable ) ? "true" : "false" );
        Msg( "flag_skidmark = %s",
             mat->Flags.test( SGameMtl::flSkidmark ) ? "true" : "false" );
        Msg( "flag_bloodmark = %s",
             mat->Flags.test( SGameMtl::flBloodmark ) ? "true" : "false" );
        Msg( "flag_climable = %s",
             mat->Flags.test( SGameMtl::flClimable ) ? "true" : "false" );
        Msg( "flag_passable = %s",
             mat->Flags.test( SGameMtl::flPassable ) ? "true" : "false" );
        Msg( "flag_dynamic = %s",
             mat->Flags.test( SGameMtl::flDynamic ) ? "true" : "false" );
        Msg( "flag_liquid = %s",
             mat->Flags.test( SGameMtl::flLiquid ) ? "true" : "false" );
        Msg(
            "flag_suppress_shadows = %s",
            mat->Flags.test( SGameMtl::flSuppressShadows ) ? "true" : "false" );
        Msg( "flag_suppress_wallmarks = %s",
             mat->Flags.test( SGameMtl::flSuppressWallmarks ) ? "true"
                                                              : "false" );
        Msg( "flag_actor_obstacle = %s",
             mat->Flags.test( SGameMtl::flActorObstacle ) ? "true" : "false" );
        Msg( "flag_bullet_no_ricochet = %s",
             mat->Flags.test( SGameMtl::flNoRicoshet ) ? "true" : "false" );
        Msg( "flag_injurious = %s",
             mat->Flags.test( SGameMtl::flInjurious ) ? "true" : "false" );
        Msg( "flag_shootable = %s",
             mat->Flags.test( SGameMtl::flShootable ) ? "true" : "false" );
        Msg( "flag_transparent = %s",
             mat->Flags.test( SGameMtl::flTransparent ) ? "true" : "false" );
        Msg( "flag_slowdown = %s",
             mat->Flags.test( SGameMtl::flSlowDown ) ? "true" : "false" );
    ...
    basePairsMs =
        loadTimer.GetElapsed_ms() - baseMaterialsMs - materialOverridesMs;
    ...
    for ( SGameMtlPair* pair : material_pairs ) {
        pairByMaterials.emplace(
            MaterialPairKey( pair->GetMtl0(), pair->GetMtl1() ), pair );
        biggestPairId = std::max( biggestPairId, pair->ID );
    }

    string_path materialPairsLtxName;
    if ( FS.exist( materialPairsLtxName, _game_data_,
        ...
        for ( const auto& sec : materialsLtx->sections() ) {
            ...
            if ( pairNames.size() < 2 ) {
                Msg( "![material_pairs.ltx] encountered wrongly defined pair "
                     "%s, two materials are required",
                     secStr.c_str() );
                continue;
            }

            const auto firstMaterial = materialByName.find(
                NormalizeMaterialName( pairNames[ 0 ].c_str() ) );
            const auto secondMaterial = materialByName.find(
                NormalizeMaterialName( pairNames[ 1 ].c_str() ) );
            const int m1 = firstMaterial == materialByName.end()
                               ? GAMEMTL_NONE_ID
                               : firstMaterial->second->ID;
            const int m2 = secondMaterial == materialByName.end()
                               ? GAMEMTL_NONE_ID
                               : secondMaterial->second->ID;

            if ( m1 == GAMEMTL_NONE_ID ) {
                Msg( "![material_pairs.ltx] encountered unknown material %s in "
                     "string %s, skip",
                     pairNames[ 0 ].c_str(), secStr.c_str() );
                continue;
            }
            if ( m2 == GAMEMTL_NONE_ID ) {
                Msg( "![material_pairs.ltx] encountered unknown material %s in "
                     "string %s, skip",
                     pairNames[ 1 ].c_str(), secStr.c_str() );
                continue;
            }
            ...
            if ( materialsLtx->line_exist( sec.Name, "collide_particles" ) ) {
                auto s =
                    materialsLtx->r_string( sec.Name, "collide_particles" );
        ...
        xr_delete( materialsLtx );
        Msg( "* [material_pairs.ltx] applied: added=%u changed=%u", addedPairs,
             changedPairs );
    }
    pairOverridesMs = loadTimer.GetElapsed_ms() - baseMaterialsMs -
                      materialOverridesMs - basePairsMs;

#ifdef DEBUG_PRINT_MATERIAL
    for ( const auto& mat : material_pairs ) {
        Msg( "[%s@%s]", GetMaterialByID( mat->mtl0 )->m_Name.c_str(),
             GetMaterialByID( mat->mtl1 )->m_Name.c_str() );
        Msg( "mtl0 = %d", mat->mtl0 );
        Msg( "mtl1 = %d", mat->mtl1 );
        Msg( "id = %d", mat->ID );
        Msg( "id_parent = %d", mat->ID_parent );
        Msg( "props = %d", mat->OwnProps.get() );
        Msg( "flag_breaking_sounds = %s",
             mat->OwnProps.test( SGameMtlPair::flBreakingSounds ) ? "true"
                                                                  : "false" );
        Msg( "flag_step_sounds = %s",
             mat->OwnProps.test( SGameMtlPair::flStepSounds ) ? "true"
                                                              : "false" );
        Msg( "flag_collide_sounds = %s",
             mat->OwnProps.test( SGameMtlPair::flCollideSounds ) ? "true"
                                                                 : "false" );
        Msg( "flag_collide_particles = %s",
             mat->OwnProps.test( SGameMtlPair::flCollideParticles ) ? "true"
                                                                    : "false" );
        Msg( "flag_collide_marks = %s",
             mat->OwnProps.test( SGameMtlPair::flCollideMarks ) ? "true"
                                                                : "false" );
    ...
    materialSoundNames.erase(
        std::unique( materialSoundNames.begin(), materialSoundNames.end() ),
        materialSoundNames.end() );
    if ( Sound )
        Sound->source_prefetch_prepare( materialSoundNames );
    for ( SGameMtlPair* pair : material_pairs )
        pair->CreateSoundResources();
    soundResourcesMs = loadTimer.GetElapsed_ms() - baseMaterialsMs -
                       materialOverridesMs - basePairsMs - pairOverridesMs;
     ...
     */
    FS.r_close( F );
    Msg( "* [STARTUP/MATERIALS] base=%u overrides=%u pairs=%u "
         "pair-overrides=%u sounds=%u total=%u ms",
