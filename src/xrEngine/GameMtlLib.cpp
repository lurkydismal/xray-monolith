...
void CGameMtlLibrary::Load() {
    ...
    if ( FS.exist( materialsLtxName, _game_data_, "materials\\materials",
        ...
        for ( const auto& sec : materialsLtx->sections() ) {
            ...
            if ( material == materials.end() ) {
                ...
                Msg( "[materials.ltx] Adding new material %s, id %d",
                     M->m_Name.c_str(), M->ID );
            } else {
                M = *material;

                Msg( "[materials.ltx] Changing existing material %s, id %d",
                     M->m_Name.c_str(), M->ID );
            }
        ...
        for ( const auto& sec : materialsLtx->sections() ) {
            ...
            if ( m1 == GAMEMTL_NONE_ID ) {
                Msg( "![material_pairs.ltx] encountered unknown material %s in "
                     "string %s, skip",
                     materials[ 0 ].c_str(), secStr.c_str() );
                continue;
            }
            if ( m2 == GAMEMTL_NONE_ID ) {
                Msg( "![material_pairs.ltx] encountered unknown material %s in "
                     "string %s, skip",
                     materials[ 1 ].c_str(), secStr.c_str() );
                continue;
            }

            auto material = std::find_if(
                material_pairs.begin(), material_pairs.end(),
                [ &m1, &m2 ]( SGameMtlPair* m ) {
            ...
            if ( material == material_pairs.end() ) {
                ...
                Msg( "[material_pairs.ltx] Adding new material pair %s | %s, "
                     "id %d",
                     GetMaterialByID( M->GetMtl0() )->m_Name.c_str(),
                     GetMaterialByID( M->GetMtl1() )->m_Name.c_str(), M->ID );
            } else {
                M = *material;

                Msg( "[material_pairs.ltx] Changing existing material pair %s "
                     "| %s, id %d",
                     GetMaterialByID( M->GetMtl0() )->m_Name.c_str(),
                     GetMaterialByID( M->GetMtl1() )->m_Name.c_str(), M->ID );
            }
    ...
    for ( GameMtlPairIt p_it = material_pairs.begin();
        ...
        SGameMtlPair* S = *p_it;
        int idx0 = GetMaterialIdx( S->mtl0 ) * material_count +
                   GetMaterialIdx( S->mtl1 );
        int idx1 = GetMaterialIdx( S->mtl1 ) * material_count +
                   GetMaterialIdx( S->mtl0 );
