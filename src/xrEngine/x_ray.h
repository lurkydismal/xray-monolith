...
public:
    ...
    void LoadSessionRecordPrecacheFrame( u64 wall_ticks,
                                         u64 frame_move_ticks,
                                         u64 seq_render_ticks,
                                         u64 end_ticks,
                                         u64 secondary_wait_ticks );
    void LoadSessionRecordPrecacheLevel( u64 calculate_ticks,
                                         u64 render_ticks );
    void LoadSessionRecordPrecacheLoadscreen( u64 ticks );
    void LoadSessionRecordPrecachePresent( u64 ticks );
    void LoadSessionRecordClientEvent( bool spawn,
                                       u16 destination,
                                       u16 type,
                                       const void* packet_data,
                                       u32 packet_size );
    void LoadSessionTryFinish( bool level_ready,
                               bool control_ready,
                               bool queues_drained );

    bool LoadSessionActive() const { return m_load_session.active; }

    bool LoadSessionPrecacheStarted() const {
        return m_load_session.precache_started;
    }
