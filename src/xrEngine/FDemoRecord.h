#if !defined( \
    AFX_FDEMORECORD_H__D7638760_FB61_11D3_B4E3_4854E82A090D__INCLUDED_ )
...
class ENGINE_API CDemoRecord : public CEffectorCam,
...
public:
    CDemoRecord( const char* name,
                 float life_time = 60 * 60 * 1000,
                 BOOL return_ctrl_inputs = 0 );
    CDemoRecord( const char* name,
                 xr_unordered_set< CDemoRecord* >* pDemoRecords,
                 BOOL isInputBlocked = 0,
                 float life_time = 60 * 60 * 1000,
                 BOOL return_ctrl_inputs = 0 );
    virtual ~CDemoRecord();
