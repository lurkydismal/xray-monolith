...
int __cdecl Lua::LuaOut( Lua::ELuaMessageType tLuaMessageType,
                         LPCSTR caFormat,
                         ... ) {
#ifndef ENGINE_BUILD
...
void Script::vfLoadStandardScripts( CLuaVirtualMachine* tpLuaVM ) {
    ...
    for ( u32 i = 0; i < caNamespaceName; ++i ) {
        FS.update_path(
            S, "$game_scripts$",
            strconcat( S1, _GetItem( caScriptString, i, I ), ".script" ) );
        bfLoadFile( tpLuaVM, S, true );
        if ( bfIsObjectPresent( tpLuaVM, "_G", xr_strcat( I, "_initialize" ),
                                LUA_TFUNCTION ) )
...
#ifdef DEBUG
    lua_sethook( tpLuaVM, LuaHookCall,
                 LUA_HOOKCALL | LUA_HOOKRET | LUA_HOOKLINE | LUA_HOOKTAILRET,
                 0 );
#endif
...
bool Script::bfLoadFile( CLuaVirtualMachine* tpLuaVM,
                         LPCSTR caScriptName,
                         bool bCall ) {
    ...
    if ( !xr_strlen( l_caNamespaceName ) )
        return (
            bfLoadFileIntoNamespace( tpLuaVM, caScriptName, "_G", bCall ) );
    else
        return ( bfLoadFileIntoNamespace( tpLuaVM, caScriptName,
                                          l_caNamespaceName, bCall ) );
}
#endif

bool bfCreateNamespaceTable( CLuaVirtualMachine* tpLuaVM,
    ...
    for ( ;; ) {
        if ( !xr_strlen( S ) ) {
            lua_pop( tpLuaVM, 1 );
            LuaOut( Lua::eLuaMessageTypeError,
                    "the namespace name %s is incorrect!", caNamespaceName );
            xr_free( S2 );
            return ( false );
        }
        LPSTR S1 = strchr( S, '.' );
        ...
        if ( lua_isnil( tpLuaVM, -1 ) ) {
            ...
            lua_pop( tpLuaVM, 2 );
            LuaOut( Lua::eLuaMessageTypeError,
                    "the namespace name %s is already being used by the "
                    "non-table object!",
                    caNamespaceName );
            return ( false );
        }
...
bool Script::bfLoadBuffer( CLuaVirtualMachine* tpLuaVM,
                           LPCSTR caBuffer,
                           size_t tSize,
                           LPCSTR caScriptName,
                           LPCSTR caNameSpaceName ) {
    ...
    if ( caNameSpaceName ) {
        ...
        l_iErrorCode =
            luaL_loadbuffer( tpLuaVM, script, tSize + str_len, caScriptName );
        xr_free( script );
    } else
        l_iErrorCode =
            luaL_loadbuffer( tpLuaVM, caBuffer, tSize, caScriptName );
...
bool bfDoFile( CLuaVirtualMachine* tpLuaVM,
               LPCSTR caScriptName,
               LPCSTR caNameSpaceName,
               bool bCall ) {
...
bool Script::bfLoadFileIntoNamespace( CLuaVirtualMachine* tpLuaVM,
                                      LPCSTR caScriptName,
                                      LPCSTR caNamespaceName,
                                      bool bCall ) {
...
CLuaVirtualMachine* Script::get_namespace_table( CLuaVirtualMachine* tpLuaVM,
                                                 LPCSTR N ) {
    if ( !xr_strlen( N ) )
        return ( tpLuaVM );
    lua_pushstring( tpLuaVM, "_G" );
...
bool Script::bfIsObjectPresent( CLuaVirtualMachine* tpLuaVM,
                                LPCSTR identifier,
                                int type ) {
    lua_pushnil( tpLuaVM );
    while ( lua_next( tpLuaVM, -2 ) ) {
        if ( ( lua_type( tpLuaVM, -1 ) == type ) &&
             !xr_strcmp( identifier, lua_tostring( tpLuaVM, -2 ) ) ) {
            lua_pop( tpLuaVM, 3 );
            return ( true );
        }
        lua_pop( tpLuaVM, 1 );
...
bool Script::bfIsObjectPresent( CLuaVirtualMachine* tpLuaVM,
                                LPCSTR namespace_name,
                                LPCSTR identifier,
                                int type ) {
    if ( xr_strlen( namespace_name ) &&
         !bfGetNamespaceTable( tpLuaVM, namespace_name ) )
        return ( false );
    return ( bfIsObjectPresent( tpLuaVM, identifier, type ) );
}

::luabind::object Script::lua_namespace_table( CLuaVirtualMachine* tpLuaVM,
                                               LPCSTR namespace_name ) {
    string256 S1;
