...
void CObject::MakeMeCrow() {
    ...
    if ( ( u32 )_InterlockedCompareExchange(
             ( long* )&dwFrame_AsCrow, device_frame_id, object_frame_id ) ==
         device_frame_id )
        return;
...
void CObject::cNameVisual_set( shared_str N ) {
    ...
    if ( *N && N[ 0 ] ) {
        ...
        */
        ...
        // The children classes will see that renderable.visual is gone, but
        // actual deletion will happen after the callback to prevent nasty stuff
        IRenderVisual* old_v = renderable.visual;
...
void CObject::processing_activate() {
    VERIFY3( 255 != Props.bActiveCounter,
             "Invalid sequence of processing enable/disable calls: overflow",
             *cName() );
...
void CObject::processing_deactivate() {
    VERIFY3( 0 != Props.bActiveCounter,
             "Invalid sequence of processing enable/disable calls: underflow",
             *cName() );
    Props.bActiveCounter--;
...
void CObject::setEnabled( BOOL _enabled ) {
    if ( Props.bDestroy || !SpatialComponent.get() )
        return;
    if ( _enabled ) {
        Props.bEnabled = 1;
        if ( collidable.model )
...
// float CObject::Radius () const { VERIFY2(renderable.visual,*cName()); return
// renderable.visual->vis.sphere.R; }
...
// Purpose :
//----------------------------------------------------------------------
CObject::CObject() : dwFrame_AsCrow( u32( -1 ) ) {
    // Transform
    Props.storage = 0;
    setID( ( u16 )-1 );
...
#ifdef CBULLETMANAGER_EX
    BulletCheckVisual = !!READ_IF_EXISTS( pSettings, r_bool, section,
                                          "bullet_check_visual", FALSE );
#endif
}
...
BOOL CObject::net_Spawn( CSE_Abstract* data ) {
    ...
    if ( 0 == collidable.model ) {
        if ( pSettings->line_exist( cNameSect(), "cform" ) ) {
            R_ASSERT3( *NameVisual,
                       "Model isn't assigned for object, but cform requisted",
                       *cName() );
...
void CObject::spatial_update( float eps_P, float eps_R ) {
    ...
    if ( bUpdate ) {
        ...
        if ( SpatialComponent->spatial.node_ptr ) {
            // Object registered!
            if ( !fsimilar( Radius(), SpatialComponent->spatial.sphere.R,
                            eps_R ) )
                spatial_move();
            else {
                Fvector C;
...
void CObject::UpdateCL() {
    ...
    if ( Device.dwFrame == dbg_update_cl )
        Debug.fatal( DEBUG_INFO, "'UpdateCL' called twice per frame for %s",
                     *cName() );
    ...
    if ( Parent && SpatialComponent->spatial.node_ptr )
        Debug.fatal(
            DEBUG_INFO,
            "Object %s has parent but is still registered inside spatial DB",
            *cName() );
    ...
    if ( ( 0 == collidable.model ) &&
        ...
        Debug.fatal(
            DEBUG_INFO,
            "Object %s registered as 'collidable' but has no collidable model",
            *cName() );
#endif
    ...
    if ( Parent == g_pGameLevel->CurrentViewEntity() )
        ...
        if ( dist < CROW_RADIUS * CROW_RADIUS )
            MakeMeCrow();
                    ...
                    Visual()->getVisData().hom_frame + 2 > Device.dwFrame ) &&
...
void CObject::shedule_Update( u32 T ) {
    // consistency check
    // Msg ("-SUB-:[%x][%s]
    // CObject::shedule_Update",fast_dynamic_cast<void*>(this),*cName());
    ...
    else if (Device.vCameraPosition.distance_to_sqr(Position()) <
    CROW_RADIUS*CROW_RADIUS) MakeMeCrow ();
    */
}
...
CObject* CObject::H_SetParent( CObject* new_parent, bool just_before_destroy ) {
    ...
    VERIFY2( ( new_parent == 0 ) || ( old_parent == 0 ),
             "Before set parent - execute H_SetParent(0)" );
