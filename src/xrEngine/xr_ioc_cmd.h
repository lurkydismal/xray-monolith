...
public:
    ...
    void InvalidSyntax() {
        ...
        g_SASH.OnConsoleInvalidSyntax(
            false, "~ Invalid syntax in call to '%s'", cName );
...
public:
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        TStatus str;
        xr_sprintf( str, sizeof( str ), "%s (current) [on/off]",
                    value->test( mask ) ? "on" : "off" );
        tips.push_back( str );
    }
};
...
public:
    CCC_Token( LPCSTR N, u32* V, xr_token* T )
        : IConsole_Command( N ), value( V ), tokens( T ) {};
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_sprintf( I, sizeof( I ), "float value in range [%3.3f,%3.3f]", min,
                    max );
    }

    virtual void fill_tips( vecTips& tips, u32 mode ) {
        TStatus str;
        xr_sprintf( str, sizeof( str ), "%3.5f (current) [%3.3f,%3.3f]", *value,
                    min, max );
        tips.push_back( str );
...
public:
    ...
    virtual void Status( TStatus& S ) {
        xr_sprintf( S, sizeof( S ), "(%f, %f, %f)", value->x, value->y,
                    value->z );
    }

    virtual void Info( TInfo& I ) {
        xr_sprintf( I, sizeof( I ), "vector3 in range [%e,%e,%e]-[%e,%e,%e]",
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        ...
        xr_sprintf( str, sizeof( str ),
                    "(%e, %e, %e) (current) [(%e,%e,%e)-(%e,%e,%e)]", value->x,
                    value->y, value->z, min.x, min.y, min.z, max.x, max.y,
                    max.z );
        tips.push_back( str );
        IConsole_Command::fill_tips( tips, mode );
    }
...
public:
    ...
    virtual void Status( TStatus& S ) {
        xr_sprintf( S, sizeof( S ), "(%f, %f, %f, %f)", value->x, value->y,
                    value->z, value->w );
    }

    virtual void Info( TInfo& I ) {
        xr_sprintf( I, sizeof( I ),
                    "vector4 in range [%e,%e,%e,%e]-[%e,%e,%e,%e]", min.x,
                    min.y, min.z, min.w, max.x, max.y, max.z, max.w );
    }

    virtual void fill_tips( vecTips& tips, u32 mode ) {
        TStatus str;
        xr_sprintf( str, sizeof( str ),
                    "(%e, %e, %e, %e) (current) [(%e,%e,%e,%e)-(%e,%e,%e,%e)]",
                    value->x, value->y, value->z, value->w, min.x, min.y, min.z,
                    min.w, max.x, max.y, max.z, max.w );
        tips.push_back( str );
        IConsole_Command::fill_tips( tips, mode );
    }
};
...
public:
    ...
    virtual void Status( TStatus& S ) {
        xr_sprintf( S, sizeof( S ), "(%i, %i, %i)", value->x, value->y,
                    value->z );
    }

    virtual void Info( TInfo& I ) {
        xr_sprintf( I, sizeof( I ), "ivector3 in range [%i,%i,%i]-[%i,%i,%i]",
                    min.x, min.y, min.z, max.x, max.y, max.z );
    }
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        TStatus str;
        xr_sprintf( str, sizeof( str ),
                    "(%i, %i, %i) (current) [(%i,%i,%i)-(%i,%i,%i)]", value->x,
                    value->y, value->z, min.x, min.y, min.z, max.x, max.y,
                    max.z );
        tips.push_back( str );
        IConsole_Command::fill_tips( tips, mode );
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( 4 != sscanf( args, "%i,%i,%i,%i", &v.x, &v.y, &v.z, &v.w ) ) {
            if ( 4 !=
                 sscanf( args, "(%i,%i,%i,%i)", &v.x, &v.y, &v.z, &v.w ) ) {
                InvalidSyntax();
                return;
            }
        }
    ...
    virtual void Status( TStatus& S ) {
        xr_sprintf( S, sizeof( S ), "(%i, %i, %i, %i)", value->x, value->y,
                    value->z, value->w );
    }

    virtual void Info( TInfo& I ) {
        xr_sprintf( I, sizeof( I ),
                    "ivector4 in range [%i,%i,%i,%i]-[%i,%i,%i,%i]", min.x,
                    min.y, min.z, min.w, max.x, max.y, max.z, max.w );
    }
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        TStatus str;
        xr_sprintf( str, sizeof( str ),
                    "(%i, %i, %i, %i) (current) [(%i,%i,%i,%i)-(%i,%i,%i,%i)]",
                    value->x, value->y, value->z, value->w, min.x, min.y, min.z,
                    min.w, max.x, max.y, max.z, max.w );
        tips.push_back( str );
        IConsole_Command::fill_tips( tips, mode );
...
public:
    CCC_Color( LPCSTR N, u32* v )
        : temp( Ivector4().set( color_get_R( *v ),
                                color_get_G( *v ),
                                color_get_B( *v ),
                                color_get_A( *v ) ) ),
          CCC_IVector4( N,
                        &temp,
                        Ivector4().set( 0, 0, 0, 0 ),
                        Ivector4().set( 255, 255, 255, 255 ) ) {
        color = v;
    };
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_sprintf( I, sizeof( I ), "integer value in range [%d,%d]", min,
                    max );
    }

    virtual void fill_tips( vecTips& tips, u32 mode ) {
        TStatus str;
        xr_sprintf( str, sizeof( str ), "%d (current) [%d,%d]", *value, min,
                    max );
        tips.push_back( str );
        IConsole_Command::fill_tips( tips, mode );
    }
...
public:
    CCC_String( LPCSTR N, LPSTR V, int _size = 2 )
        : IConsole_Command( N ), value( V ), size( _size ) {
    ...
    virtual void Execute( LPCSTR args ) {
        strncpy_s( value, size, !xr_strcmp( args, "(NULL)" ) ? "" : args,
                   size - 1 );
    }
