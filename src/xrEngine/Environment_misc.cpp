...
CEnvAmbient::SEffect* CEnvAmbient::create_effect( CInifile& config,
                                                  LPCSTR id ) {
    ...
    if ( config.line_exist( id, "sound" ) )
        result->sound.create( config.r_string( id, "sound" ), st_Effect,
                              sg_SourceType );

    if ( config.line_exist( id, "wind_blast_strength" ) ) {
        result->wind_blast_strength =
            config.r_float( id, "wind_blast_strength" );
        ...
        result->wind_blast_out_time =
            config.r_float( id, "wind_blast_out_time" );
        return ( result );
    }
...
CEnvAmbient::SSndChannel* CEnvAmbient::create_sound_channel( CInifile& config,
                                                             LPCSTR id ) {
...
void CEnvAmbient::load( CInifile& ambients_config,
    ...
    for ( u32 i = 0; i < cnt; ++i )
        m_sound_channels[ i ] = create_sound_channel(
            sound_channels_config, _GetItem( channels, i, tmp ) );

    // effects
    m_effect_period.set(
        iFloor( ambients_config.r_float( sect, "min_effect_period" ) * 1000.f ),
        iFloor( ambients_config.r_float( sect, "max_effect_period" ) *
                1000.f ) );
    ...
    for ( u32 k = 0; k < cnt; ++k )
        m_effects[ k ] =
            create_effect( effects_config, _GetItem( effs, k, tmp ) );
...
#define C_CHECK( C )                                                           \
    if ( C.x < 0 || C.x > 5 || C.y < 0 || C.y > 5 || C.z < 0 || C.z > 5 ) {    \
...
void CEnvDescriptor::load( CEnvironment& environment, CInifile& config ) {
    ...
    R_ASSERT3( ( tm.x >= 0 ) && ( tm.x < 24 ) && ( tm.y >= 0 ) &&
                   ( tm.y < 60 ) && ( tm.z >= 0 ) && ( tm.z < 60 ),
    ...
    clouds_texture_name =
        config.r_string( m_identifier.c_str(), "clouds_texture" );
    LPCSTR cldclr = config.r_string( m_identifier.c_str(), "clouds_color" );
    float multiplier = 0, save = 0;
    sscanf( cldclr, "%f,%f,%f,%f,%f", &clouds_color.x, &clouds_color.y,
            &clouds_color.z, &clouds_color.w, &multiplier );
    save = clouds_color.w;
    ...
    if ( config.line_exist( m_identifier.c_str(), "sky_rotation" ) )
        sky_rotation =
            deg2rad( config.r_float( m_identifier.c_str(), "sky_rotation" ) );
    else
        sky_rotation = 0;
    ...
    wind_direction =
        deg2rad( config.r_float( m_identifier.c_str(), "wind_direction" ) );
    ...
    lens_flare_id = environment.eff_LensFlare->AppendDef(
        environment, environment.m_suns_config,
    ...
    bolt_period = ( tb_id.size() ) ? config.r_float( m_identifier.c_str(),
                                                     "thunderbolt_period" )
                                   : 0.f;
    bolt_duration = ( tb_id.size() ) ? config.r_float( m_identifier.c_str(),
                                                       "thunderbolt_duration" )
                                     : 0.f;
    env_ambient = config.line_exist( m_identifier.c_str(), "ambient" )
                      ? environment.AppendEnvAmb(
                            config.r_string( m_identifier.c_str(), "ambient" ) )
                      : 0;

    if ( config.line_exist( m_identifier.c_str(), "sun_shafts_intensity" ) )
        m_fSunShaftsIntensity =
            config.r_float( m_identifier.c_str(), "sun_shafts_intensity" );

    if ( config.line_exist( m_identifier.c_str(), "water_intensity" ) )
        m_fWaterIntensity =
            config.r_float( m_identifier.c_str(), "water_intensity" );

    // lemurten
    if ( config.line_exist( m_identifier.c_str(), "hemi_vibrance" ) )
        m_fHemiVibrance =
            config.r_float( m_identifier.c_str(), "hemi_vibrance" );

    if ( config.line_exist( m_identifier.c_str(), "hemi_contrast" ) )
        m_fHemiContrast =
            config.r_float( m_identifier.c_str(), "hemi_contrast" );

    if ( config.line_exist( m_identifier.c_str(), "wet_surface_factor" ) )
        m_fWetSurfaces =
            config.r_float( m_identifier.c_str(), "wet_surface_factor" );

    if ( config.line_exist( m_identifier.c_str(),
                            "volumetric_intensity_factor" ) )
        volumetric_intensity_factor = config.r_float(
            m_identifier.c_str(), "volumetric_intensity_factor" );

    if ( config.line_exist( m_identifier.c_str(),
                            "volumetric_distance_factor" ) )
        volumetric_distance_factor = config.r_float(
            m_identifier.c_str(), "volumetric_distance_factor" );

#ifdef TREE_WIND_EFFECT
    if ( config.line_exist( m_identifier.c_str(), "tree_amplitude_intensity" ) )
        m_fTreeAmplitudeIntensity =
            config.r_float( m_identifier.c_str(), "tree_amplitude_intensity" );
#endif

    bloom_threshold =
        config.line_exist( m_identifier.c_str(), "bloom_threshold" )
            ? config.r_float( m_identifier.c_str(), "bloom_threshold" )
            : 3.5f;
    bloom_exposure =
        config.line_exist( m_identifier.c_str(), "bloom_exposure" )
            ? config.r_float( m_identifier.c_str(), "bloom_exposure" )
            : 3.0f;
    bloom_sky_intensity =
        config.line_exist( m_identifier.c_str(), "bloom_sky_intensity" )
            ? config.r_float( m_identifier.c_str(), "bloom_sky_intensity" )
            : 0.6f;
...
void CEnvDescriptorMixer::lerp( CEnvironment* env,
    ...
    if ( Mdf.use_flags.test( eViewDist ) )
        far_plane = ( fi * A.far_plane + f * B.far_plane + Mdf.far_plane ) *
                    psVisDistance * modif_power;
    else
    ...
    m_fSunShaftsIntensity =
        fi * A.m_fSunShaftsIntensity + f * B.m_fSunShaftsIntensity;
    ...
    volumetric_intensity_factor =
        fi * A.volumetric_intensity_factor + f * B.volumetric_intensity_factor;
    volumetric_distance_factor =
        fi * A.volumetric_distance_factor + f * B.volumetric_distance_factor;
    ...
    bloom_sky_intensity =
        fi * A.bloom_sky_intensity + f * B.bloom_sky_intensity;
    ...
    if ( rain_density > 0.f )
        env->wetness_factor +=
            ( rain_density * ssfx_wetness_multiplier.x ) / 10000.f;
    else
...
CEnvAmbient* CEnvironment::AppendEnvAmb( const shared_str& sect ) {
    ...
    Ambients.back()->load( *m_ambients_config, *m_sound_channels_config,
                           *m_effects_config, sect );
...
void CEnvironment::PrepareLevelModifiers( LPCSTR canonical_level_path,
                                          xr_vector< CEnvModifier >& result ) {
    ...
    if ( FS.exist( path.c_str() ) ) {
        ...
        R_ASSERT3( fs, "Cannot open level environment modifiers",
                   path.c_str() );
        u32 id = 0;
        u32 ver = 0x0015;
        u32 sz;
...
void CEnvironment::load_level_specific_ambients() {
    ...
    string_path path;
    strconcat( sizeof( path ), path, "environment\\ambients\\",
               level_name.c_str(), ".ltx" );
...
void CEnvironment::load_sun() {
    ...
    do {
        ...
        float sun_alt =
            m_sun_pos_config->r_float( sun_identifier, "sun_altitude" );
        float sun_long =
            m_sun_pos_config->r_float( sun_identifier, "sun_longitude" );
...
void CEnvironment::load_weathers() {
    ...
    for ( ; _I != _E; _I++ ) {
        R_ASSERT3( _I->second.size() > 1, "Environment in weather must >=2",
                   *_I->first );
...
void CEnvironment::load_weather_effects() {
    ...
    file_list_type* file_list =
        FS.file_list_open( "$game_weather_effects$", "" );
    VERIFY( file_list );
    xr_string id;
    file_list_type::const_iterator i = file_list->begin();
    file_list_type::const_iterator e = file_list->end();
...
void CEnvironment::load() {
    ...
    if ( Sound ) {
        ...
        auto collect_values = [ &sounds ]( CInifile* config, LPCSTR key ) {
            for ( const auto& section : config->sections() )
                if ( config->line_exist( section.Name, key ) )
                    sounds.emplace_back(
                        config->r_string( section.Name, key ) );
        };
        ...
        sounds.erase( std::unique( sounds.begin(), sounds.end() ),
                      sounds.end() );
    ...
    load_sun();
    Msg( "* [STARTUP/ENV LOAD] sounds=%u weather=%u effects=%u sun=%u total=%u "
         "ms",
         soundPrepareMs, weatherMs, effectsMs,
         startupTimer.GetElapsed_ms() - soundPrepareMs - weatherMs - effectsMs,
