...
Image* new_image( int xsize, int ysize ) /* create a blank image */
{
    ...
    if ( ( 0 != ( image = ( Image* )xr_malloc( sizeof( Image ) ) ) ) &&
         ( 0 != ( image->data = ( Pixel* )xr_malloc( ysize * xsize *
                                                     sizeof( Pixel ) ) ) ) ) {
...
void get_row( Pixel* row, Image* image, int y ) {
    if ( ( y < 0 ) || ( y >= image->ysize ) )
        return;
    CopyMemory( row, image->data + ( y * image->span ),
...
float Mitchell_filter( float t ) {
    ...
    if ( t < 1.0f ) {
        ...
        t = ( ( ( -1.0f * B - 6.0f * C ) * ( t * tt ) ) +
              ( ( 6.0f * B + 30.0f * C ) * tt ) +
              ( ( -12.0f * B - 48.0f * C ) * t ) + ( 8.0f * B + 24.f * C ) );
        return ( t / 6.0f );
    }
    return ( 0.0 );
}
...
void imf_Process( u32* dstI,
    ...
    if ( xscale < 1.0 ) {
        ...
        for ( i = 0; i < dst.xsize; ++i ) {
            contrib[ i ].n = 0;
            contrib[ i ].p = ( CONTRIB* )xr_malloc( ( int )( width * 2 + 1 ) *
                                                    sizeof( CONTRIB ) );
            ZeroMemory( contrib[ i ].p,
                        ( int )( width * 2 + 1 ) * sizeof( CONTRIB ) );
        ...
        for ( i = 0; i < dst.xsize; ++i ) {
            contrib[ i ].n = 0;
            contrib[ i ].p = ( CONTRIB* )xr_malloc( ( int )( fwidth * 2 + 1 ) *
                                                    sizeof( CONTRIB ) );
            ZeroMemory( contrib[ i ].p,
                        ( int )( fwidth * 2 + 1 ) * sizeof( CONTRIB ) );
            center = float( i ) / xscale;
    ...
    for ( k = 0; k < tmp->ysize; ++k ) {
        ...
        for ( i = 0; i < tmp->xsize; ++i ) {
            ...
            put_pixel( tmp, i, k,
                       color_rgba( CC( w_r ), CC( w_g ), CC( w_b ),
                                   CC( w_a + 0.5f ) ) );
        }
    }
    xr_free( raster );
