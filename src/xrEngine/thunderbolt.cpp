...
SThunderboltDesc::SThunderboltDesc()
    : m_GradientTop( 0 ), m_GradientCenter( 0 ) {}
...
void SThunderboltDesc::create_top_gradient( CInifile& pIni,
                                            shared_str const& sect ) {
    ...
    m_GradientTop->m_pFlare->CreateShader( *m_GradientTop->shader,
                                           *m_GradientTop->texture );
}

void SThunderboltDesc::create_center_gradient( CInifile& pIni,
                                               shared_str const& sect ) {
    m_GradientCenter = xr_new< SFlare >();
    m_GradientCenter->shader = pIni.r_string( sect, "gradient_center_shader" );
    m_GradientCenter->texture =
        pIni.r_string( sect, "gradient_center_texture" );
    m_GradientCenter->fRadius =
        pIni.r_fvector2( sect, "gradient_center_radius" );
    m_GradientCenter->fOpacity =
        pIni.r_float( sect, "gradient_center_opacity" );
    m_GradientCenter->m_pFlare->CreateShader( *m_GradientCenter->shader,
                                              *m_GradientCenter->texture );
}
...
void SThunderboltDesc::load( CInifile& pIni, shared_str const& sect ) {
    ...
    'lightning_model'."); l_model = ::Render->model_CreateDM(F); FS.r_close (F);
    */
...
void SThunderboltCollection::load( CInifile* pIni,
                                   CInifile* thunderbolts,
                                   LPCSTR sect ) {
    section = sect;
    ...
    for ( int tb_idx = 0; tb_idx < tb_count; tb_idx++ ) {
        ...
        if ( pIni->r_line( sect, tb_idx, &N, &V ) )
            palette.push_back(
                g_pGamePersistent->Environment().thunderbolt_description(
                    *thunderbolts, N ) );
    }
}
...
CEffect_Thunderbolt::CEffect_Thunderbolt() {
    ...
    // geom
    // hGeom_model.create (D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1,
    // RCache.Vertex.Buffer(), RCache.Index.Buffer());
    ...
    // p_var_long = deg2rad ( pSettings->r_float (
    // "environment","delta_longitude" )); p_min_dist = _min
    // (.95f,pSettings->r_float ( "environment","min_dist_factor" )); p_tilt =
    // deg2rad (pSettings->r_float ( "environment","tilt" )); p_second_prop =
    // pSettings->r_float ( "environment","second_propability" ); clamp
    // (p_second_prop,0.f,1.f); p_sky_color = pSettings->r_float (
    // "environment","sky_color" ); p_sun_color = pSettings->r_float (
    // "environment","sun_color" ); p_fog_color = pSettings->r_float (
    // "environment","fog_color" );
}

CEffect_Thunderbolt::~CEffect_Thunderbolt() {
    for ( CollectionVecIt d_it = collection.begin(); d_it != collection.end();
          d_it++ )
        xr_delete( *d_it );
    collection.clear();
    // hGeom_model.destroy ();
...
shared_str CEffect_Thunderbolt::AppendDef( CEnvironment& environment,
                                           CInifile* pIni,
                                           CInifile* thunderbolts,
                                           LPCSTR sect ) {
    if ( !sect || ( 0 == sect[ 0 ] ) )
        return "";
    for ( CollectionVecIt it = collection.begin(); it != collection.end();
          it++ )
        if ( ( *it )->section == sect )
            return ( *it )->section;
    collection.push_back(
        environment.thunderbolt_collection( pIni, thunderbolts, sect ) );
    return collection.back()->section;
}

BOOL CEffect_Thunderbolt::RayPick( const Fvector& s,
                                   const Fvector& d,
                                   float& dist ) {
    BOOL bRes = TRUE;
#ifdef _EDITOR
...
void CEffect_Thunderbolt::Bolt( shared_str id, float period, float lt ) {
    ...
    current = g_pGamePersistent->Environment()
                  .thunderbolt_collection( collection, id )
                  ->GetRandomDesc();
    VERIFY( current );
    ...
    float alt =
        environment
            .p_var_alt; // Random.randF(environment.p_var_alt.x,environment.p_var_alt.y);
    float lng = Random.randF( sun_h - environment.p_var_long + PI,
                              sun_h + environment.p_var_long + PI );
    float dist =
        Random.randF( FAR_DIST * environment.p_min_dist, FAR_DIST * .95f );
    ...
    if ( next_v < environment.p_second_prop ) {
        ...
        next_lightning_time = Device.fTimeGlobal + period +
                              Random.randF( -period * 0.3f, period * 0.3f );
        current->snd.play_no_feedback(
            0, 0, dist / 300.f, &pos, 0, 0,
...
void CEffect_Thunderbolt::OnFrame( shared_str id,
                                   float period,
                                   float duration ) {
    ...
    if ( bEnabled != enabled ) {
        bEnabled = enabled;
        next_lightning_time = Device.fTimeGlobal + period +
                              Random.randF( -period * 0.5f, period * 0.5f );
    ...
    if ( state == stWorking ) {
        ...
        Fvector fClr;
        int frame;
        u32 uClr = current->color_anim->CalculateRGB( current_time / life_time,
                                                      frame );
        ...
        if ( ::Render->get_generation() == IRender_interface::GENERATION_R2 ) {
            ...
            g_pGamePersistent->Environment().CurrentEnv->sun_dir =
                current_direction;
            VERIFY2(
                g_pGamePersistent->Environment().CurrentEnv->sun_dir.y < 0,
...
void CEffect_Thunderbolt::Render() {
    if ( state == stWorking ) {
        ...
        (IRender_DetailModel::fvfVertexOut*) RCache.Vertex.Lock (vCount_Lock,
        hGeom_model->vb_stride, v_offset); u16* i_ptr = RCache.Index.Lock
        ...
        RCache.Render
        (D3DPT_TRIANGLELIST,v_offset,0,vCount_Lock,i_offset,iCount_Lock/3);
        ...
        u32 VS_Offset;
        FVF::LIT *pv = (FVF::LIT*)
        RCache.Vertex.Lock(8,hGeom_gradient.stride(),VS_Offset);
        // top
        {
        u32 c_val =
        iFloor(current->m_GradientTop.fOpacity*lightning_phase*255.f); u32 c =
        color_rgba(c_val,c_val,c_val,c_val); vecSx.mul (Device.vCameraRight,
        current->m_GradientTop.fRadius.x*lightning_size); vecSy.mul
        (Device.vCameraTop, -current->m_GradientTop.fRadius.y*lightning_size);
        pv->set (current_xform.c.x+vecSx.x-vecSy.x,
        current_xform.c.y+vecSx.y-vecSy.y, current_xform.c.z+vecSx.z-vecSy.z, c,
        0, 0); pv++; pv->set (current_xform.c.x+vecSx.x+vecSy.x,
        current_xform.c.y+vecSx.y+vecSy.y, current_xform.c.z+vecSx.z+vecSy.z, c,
        0, 1); pv++; pv->set (current_xform.c.x-vecSx.x-vecSy.x,
        current_xform.c.y-vecSx.y-vecSy.y, current_xform.c.z-vecSx.z-vecSy.z, c,
        1, 0); pv++; pv->set (current_xform.c.x-vecSx.x+vecSy.x,
        current_xform.c.y-vecSx.y+vecSy.y, current_xform.c.z-vecSx.z+vecSy.z, c,
        1, 1); pv++;
        }
        // center
        ...
        iFloor(current->m_GradientTop.fOpacity*lightning_phase*255.f); u32 c =
        color_rgba(c_val,c_val,c_val,c_val); vecSx.mul (Device.vCameraRight,
        current->m_GradientCenter.fRadius.x*lightning_size); vecSy.mul
        (Device.vCameraTop,
        -current->m_GradientCenter.fRadius.y*lightning_size); pv->set
