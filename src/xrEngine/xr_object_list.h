...
public:
    ...
    struct SRelcasePair {
        ...
        SRelcasePair( int* id, RELCASE_CALLBACK cb )
            : m_ID( id ), m_Callback( cb ) {}
...
public:
    ...
    u32 net_Export( NET_Packet* P,
                    u32 _start,
                    u32 _count ); // return next start
    ...
    IC bool destroy_queues_empty() const {
        return force_destroy_queue.empty() && destroy_queue.empty();
    }
