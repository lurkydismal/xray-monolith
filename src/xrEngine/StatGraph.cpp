...
void CStatGraph::OnDeviceCreate() {
    ...
    // hGeomLine.create
    // (FVF::F_TL0uv,RCache.Vertex.Buffer(),RCache.Index.Buffer());
...
void CStatGraph::OnRender() {
    ...
    {
    pv_Tri_start =
    (FVF::TL0uv*)RCache.Vertex.Lock(TriElem,hGeomTri->vb_stride,dwOffsetTri);
    pv_Tri = pv_Tri_start;
    ...
    {
    pv_Line_start =
    (FVF::TL0uv*)RCache.Vertex.Lock(LineElem,hGeomLine->vb_stride,dwOffsetLine);
...
FVF::TL0uv* pv_start =
(FVF::TL0uv*)RCache.Vertex.Lock(4,hGeomTri->vb_stride,dwOffset); FVF::TL0uv* pv
= pv_start;
// base rect
...
pv_start = (FVF::TL0uv*)RCache.Vertex.Lock( 2 + 2*grid.x + Num_H_LinesUp*2 +
Num_H_LinesDwn*2, hGeomLine->vb_stride,dwOffset); pv = pv_start;
...
for (ElementsDeqIt it=pelements->begin()+1; it!=pelements->end() &&
it!=pelements->end()+1; it++)
{
...
for (MarkersDeqIt it=pmarkers->begin(); it!=pmarkers->end() &&
it!=pmarkers->end()+1; it++)
{
