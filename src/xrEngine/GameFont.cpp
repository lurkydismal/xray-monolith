...
#endif
#ifdef _EDITOR
unsigned short int mbhMulti2Wide( wide_char* WideStr,
                                  wide_char* WidePos,
                                  const unsigned short int WideStrSize,
                                  const char* MultiStr ) {
    return 0;
};
#endif
...
CGameFont::CGameFont( LPCSTR section, u32 flags ) {
    ...
    const auto FindTextureName = [ & ]( LPCSTR _sect ) {
        static const char* tex_names[] = { "texture800", "texture",
                                           "texture1600", "texture2160" };
        int idx = 1;
    ...
    Initialize( pSettings->r_string( section, "shader" ),
                FindTextureName( section ) );
...
void CGameFont::Initialize( LPCSTR cShader, LPCSTR cTextureName ) {
    ...
    nNumChars = 0x100;
    TCMap =
        ( Fvector* )xr_realloc( ( void* )TCMap, nNumChars * sizeof( Fvector ) );

    if ( ini->section_exist( "mb_symbol_coords" ) ) {
        nNumChars = 0x10000;
        TCMap = ( Fvector* )xr_realloc( ( void* )TCMap,
                                        nNumChars * sizeof( Fvector ) );
        uFlags |= fsMultibyte;
        ...
        if ( ini->section_exist( "char widths" ) ) {
            ...
            for ( u32 i = 0; i < nNumChars; i++ ) {
                ...
                TCMap[ i ].set( ( i % cpl ) * fHeight, ( i / cpl ) * fHeight,
                                w );
            }
        } else {
            ...
            for ( u32 i = 0; i < nNumChars; i++ )
                TCMap[ i ].set( ( i % cpl ) * width, ( i / cpl ) * fHeight,
                                width );
        }
    }
...
#define DI2PX( x )                                                           \
    float( iFloor( ( x + 1 ) * float( ::Render->getTarget()->get_width() ) * \
                   0.5f ) )
#define DI2PY( y )                                                            \
    float( iFloor( ( y + 1 ) * float( ::Render->getTarget()->get_height() ) * \
                   0.5f ) )
...
u32 CGameFont::smart_strlen( const char* S ) {
    return ( IsMultibyte() ? mbhMulti2Wide( NULL, NULL, 0, S )
                           : xr_strlen( S ) );
}
...
u16 CGameFont::SplitByWidth( u16* puBuffer,
                             u16 uBufferSize,
                             float fTargetWidth,
                             const char* pszText ) {
    ...
    for ( u16 i = 1; i <= len; i++ ) {
        ...
        if ( ( ( fCurWidth + fDelta ) > fTargetWidth ) && // overlength
             ( !IsBadStartCharacter(
                 wsStr[ i ] ) ) && // can start with this character
             ( i < len ) &&        // is not the last character
             ( ( i > 1 ) &&
               ( !IsBadEndCharacter(
                   wsStr[ i - 1 ] ) ) ) // && // do not stop the string on a
                                        // "bad" character ( ( i > 1 ) && ( ! (
                                        // ( IsAlphaCharacter( wsStr[ i - 1 ] )
                                        // ) && ( IsAlphaCharacter( wsStr[ i ] )
                                        // ) ) ) ) // do not split numbers or
                                        // words
        ) {
            fCurWidth = fDelta;
...
void CGameFont::MasterOut( BOOL bCheckDevice,
                           BOOL bUseCoords,
                           BOOL bScaleCoords,
                           BOOL bUseSkip,
                           float _x,
...
#define MASTER_OUT( CHECK_DEVICE, USE_COORDS, SCALE_COORDS, USE_SKIP, X, Y, \
                    SKIP, FMT )                                             \
    {                                                                       \
        va_list p;                                                          \
        va_start( p, fmt );                                                 \
        MasterOut( CHECK_DEVICE, USE_COORDS, SCALE_COORDS, USE_SKIP, X, Y,  \
                   SKIP, FMT, p );                                          \
        va_end( p );                                                        \
    }
...
float CGameFont::SizeOf_( const char cChar ) {
    return (
        GetCharTC(
            ( u16 )( u8 )( ( ( IsMultibyte() && cChar == ' ' ) ) ? 0 : cChar ) )
            .z *
        vInterval.x );
}
