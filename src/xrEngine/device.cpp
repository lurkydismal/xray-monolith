...
void CRenderDevice::End( void ) {
    ...
    if ( dwPrecacheFrame ) {
        ...
        if ( !dwPrecacheFrame ) {
            ...
            // Msg("* End of synchronization A[%d] R[%d]", b_is_Active,
            // b_is_Ready);
    ...
    m_pRender->End();
    if ( measure_precache )
        pApp->LoadSessionRecordPrecachePresent( CPU::QPC() -
                                                present_started_at );
...
void CRenderDevice::PreCache( u32 amount,
                              bool b_draw_loadscreen,
                              bool b_wait_user_input ) {
...
void CRenderDevice::on_idle() {
    ...
    const bool precache_before_frame =
        pApp && pApp->LoadSessionMeasurePrecache();
    const u64 frame_started_at = precache_before_frame ? CPU::QPC() : 0;
    FrameMove();
    const bool measure_precache_frame =
        pApp && pApp->LoadSessionMeasurePrecache();
    const u64 frame_move_finished_at = measure_precache_frame ? CPU::QPC() : 0;
    const u64 measured_frame_started_at =
        precache_before_frame ? frame_started_at : frame_move_finished_at;
    const u64 frame_move_ticks =
        precache_before_frame ? frame_move_finished_at - frame_started_at : 0;
    ...
    mProjectHud.build_projection(
        deg2rad( psHUD_FOV * 83.f ), fASPECT, R_VIEWPORT_NEAR,
        g_pGamePersistent->Environment().CurrentEnv->far_plane );
    mProjectCam.build_projection(
        deg2rad( 83.f ), fASPECT, R_VIEWPORT_NEAR,
        g_pGamePersistent->Environment().CurrentEnv->far_plane );
    ...
    GData.prev_pos[ 0 ].set( Device.vCameraPosition.x, Device.vCameraPosition.y,
                             Device.vCameraPosition.z, -1 );
    GData.prev_dir[ 0 ].set( 0.0f, -99.0f, 0.0f, 1.0f );

    for ( int pBend = 1; pBend < _min( 16, ps_ssfx_grass_interactive.y + 1 );
          pBend++ ) {
        GData.prev_pos[ pBend ].set( GData.pos[ pBend ].x, GData.pos[ pBend ].y,
                                     GData.pos[ pBend ].z,
                                     GData.radius_curr[ pBend ] );
        GData.prev_dir[ pBend ].set( GData.dir[ pBend ].x, GData.dir[ pBend ].y,
                                     GData.dir[ pBend ].z, GData.str[ pBend ] );
    }
    ...
    D3DXMatrixInverse( ( D3DXMATRIX* )&mInvFullTransform, 0,
                       ( D3DXMATRIX* )&mFullTransform );
    ...
    if ( b_is_Active && Begin() ) {
        ...
        const u64 seq_render_started_at =
            measure_precache_frame ? CPU::QPC() : 0;
    ...
    const u64 secondary_wait_started_at =
        measure_precache_frame ? CPU::QPC() : 0;
    secondary_tasks.wait();
    if ( measure_precache_frame ) {
        const u64 frame_finished_at = CPU::QPC();
        pApp->LoadSessionRecordPrecacheFrame(
            frame_finished_at - measured_frame_started_at, frame_move_ticks,
            seq_render_ticks, end_ticks,
            frame_finished_at - secondary_wait_started_at );
    }
...
void mt_DiscordThread( void* ) {
    while ( true ) {
        ...
        if ( use_discord && psDeviceFlags2.test( rsDiscord ) ) {
            ...
            Sleep(
                1000 ); // Sleep for 1 second if Discord is not used or disabled
        }
    }
}
...
void CRenderDevice::Run() {
    ...
    Msg( "* [STARTUP] app start callbacks: %d ms",
         app_start_timer.GetElapsed_ms() );
...
void CRenderDevice::FrameMove() {
    ...
    if ( psDeviceFlags.test( rsConstantFPS ) ) {
        ...
        // fTimeDelta = 0.7f * fTimeDelta + 0.3f*fPreviousFrameTime; // smooth
        ...
        fTimeGlobal =
            TimerGlobal.GetElapsed_sec(); // float(qTime)*CPU::cycles2seconds;
...
void CRenderDevice::Pause( BOOL bOn, BOOL bTimer, BOOL bSound, LPCSTR reason ) {
    ...
    if ( bOn ) {
        ...
        if ( bTimer &&
             ( !g_pGamePersistent || g_pGamePersistent->CanBePaused() ) ) {
...
void CRenderDevice::OnWM_Activate( WPARAM wParam, LPARAM lParam ) {
    ...
    BOOL bActive =
        ( ( fActive != WA_INACTIVE ) && ( !fMinimized ) ) ? TRUE : FALSE;
            ...
            if ( Device.b_hide_cursor ) {
                ...
                if ( m_hWnd ) {
                    ...
                    MapWindowPoints( m_hWnd, nullptr,
                                     reinterpret_cast< LPPOINT >( &winRect ),
                                     2 );
                    ClipCursor( &winRect );
                }
    ...
    if ( bActive != Device.b_is_Active ) {
        ...
        if ( Device.b_is_Active ) {
            ...
            app_inactive_time +=
                TimerMM.GetElapsed_ms() - app_inactive_time_start;
...
void CRenderDevice::CSecondVPParams::SetSVPActive(
    bool bState ) //--#SM+#-- +SecondVP+
{
    isActive = bState;
    if ( g_pGamePersistent != NULL )
        g_pGamePersistent->m_pGShaderConstants->m_blender_mode.z =
            ( isActive ? 1.0f : 0.0f );
}
