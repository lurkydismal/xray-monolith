...
bool Script::bfPrintOutput( CLuaVirtualMachine* tpLuaVirtualMachine,
    ...
    for ( int i = -1;; --i )
        if ( lua_isstring( tpLuaVirtualMachine, i ) ) {
            ...
            if ( !xr_strcmp( S, "cannot resume dead coroutine" ) ) {
                LuaOut( Lua::eLuaMessageTypeInfo, "Script %s is finished",
                        caScriptFileName );
                return ( true );
            } else {
                if ( !i && !iErorCode )
                    LuaOut( Lua::eLuaMessageTypeInfo, "Output from %s",
                            caScriptFileName );
                LuaOut( iErorCode ? Lua::eLuaMessageTypeError
                                  : Lua::eLuaMessageTypeMessage,
                        "%s", S );
            }
        } else {
            for ( i = 0;; ++i )
                if ( lua_isstring( tpLuaVirtualMachine, i ) ) {
                    LPCSTR S = lua_tostring( tpLuaVirtualMachine, i );
                    if ( !xr_strcmp( S, "cannot resume dead coroutine" ) ) {
                        LuaOut( Lua::eLuaMessageTypeInfo,
                                "Script %s is finished", caScriptFileName );
                        return ( true );
                    } else {
                        if ( !i && !iErorCode )
                            LuaOut( Lua::eLuaMessageTypeInfo, "Output from %s",
...
void Script::vfPrintError( CLuaVirtualMachine* tpLuaVirtualMachine,
                           int iErrorCode ) {
    switch ( iErrorCode ) {
        case LUA_ERRRUN: {
...
bool Script::bfListLevelVars( CLuaVirtualMachine* tpLuaVirtualMachine,
                              int iStackLevel ) {
    ...
    // i = 1;
    // while (NULL != (name = lua_getlocal(tpLuaVirtualMachine, &l_tDebugInfo,
    // i++))) { Msg (" local %d %s", i-1, name); lua_pop (tpLuaVirtualMachine,
    // 1); /* remove variable value */
    // }
    //
    // lua_getinfo (tpLuaVirtualMachine, "f", &l_tDebugInfo); /* retrieves
    // function */ i = 1; while (NULL != (name =
    // lua_getupvalue(tpLuaVirtualMachine, -1, i++))) { Msg (" upvalue %d %s",
    // i-1, name); lua_pop (tpLuaVirtualMachine, 1); /* remove upvalue value */
    // }
    // return (true);
    return ( false );
}
