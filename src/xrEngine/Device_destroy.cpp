...
void CRenderDevice::Reset( bool precache ) {
    ...
    if ( g_screenmode == 1 ) {
        ...
        SetWindowPos( Device.m_hWnd, HWND_TOP, monX, monY, w, h,
                      SWP_FRAMECHANGED );
    }
...
bool CRenderDevice::ChangeOutputMonitor( HMONITOR hTargetMon ) {
    ...
    if ( s_swap_in_progress )
        return false;
    if ( IsIconic( m_hWnd ) ) {
        Msg( "* vid_monitor: window is minimised, deferring monitor switch to "
             "restart" );
        return false;
    }
    ...
    bool switched = m_pRender->SwitchOutputMonitor(
        hTargetMon, m_hWnd, g_screenmode, psCurrentVidMode[ 0 ],
        psCurrentVidMode[ 1 ] );
