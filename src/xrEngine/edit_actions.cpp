...
callback_base::callback_base( Callback const& callback,
                              u32 const dik,
                              key_state state ) {
    m_callback = callback;
    m_run_state = state;
...
void callback_base::on_key_press( line_edit_control* const control ) {
    if ( control->get_key_state( m_run_state ) ) {
        if ( ( !is_numpad_numkey( m_dik ) ) ||
             ( control->get_num_lock_state() ) &&
                 ( ( m_run_state == ks_NumLock &&
                     ( !control->get_key_state( ks_Ctrl ) &&
                       !control->get_key_state( ks_Alt ) ) ) ||
                   ( m_run_state == ks_NumLk_Ctrl &&
                     control->get_key_state( ks_Ctrl ) ) ||
                   ( m_run_state == ks_NumLk_Alt &&
                     control->get_key_state( ks_Alt ) ) ) ) {
            m_callback();
            return;
        }
...
void type_pair::on_key_press( line_edit_control* const control ) {
    ...
    if ( m_translate ) {
        if ( pInput->dik_to_text( ( int )m_dik, shift, caps, ctrl, alt, altgr,
                                  out, sizeof( out ) ) ) {
