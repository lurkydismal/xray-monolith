...
public:
    ...
    void Attach( IEventReceiver* H ) {
        if ( std::find( Handlers.begin(), Handlers.end(), H ) ==
             Handlers.end() )
            Handlers.push_back( H );
    }

    void Detach( IEventReceiver* H ) {
        xr_vector< IEventReceiver* >::iterator I =
            std::find( Handlers.begin(), Handlers.end(), H );
        if ( I != Handlers.end() )
            Handlers.erase( I );
    }
...
EVENT CEventAPI::Create( const char* N ) {
    CS.Enter();
    CEvent E( N );
    for ( xr_vector< CEvent* >::iterator I = Events.begin(); I != Events.end();
          I++ ) {
        if ( ( *I )->Equal( E ) ) {
            EVENT F = *I;
            F->dwRefCount++;
...
void CEventAPI::Destroy( EVENT& E ) {
    ...
    E->dwRefCount--;
    if ( E->dwRefCount == 0 ) {
        xr_vector< CEvent* >::iterator I =
            std::find( Events.begin(), Events.end(), E );
