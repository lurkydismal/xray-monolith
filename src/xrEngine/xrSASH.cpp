...
bool xrSASH::Init( const char* pszParam ) {
    ...
    //    Msg("oa:: Version: %d.%d.%d.%d", ver.Major, ver.Minor, ver.Minor,
    //    ver.Custom);
...
void xrSASH::ReportNative( LPCSTR pszTestName ) {
    ...
    // res.w_float ("general","test float", float(1.0f)/10.f, "dx-level
    // required" ); res.w_float ("general","renderer",
    // float(::Render->get_generation())/10.f, "dx-level required" );
    // res.w_float ("general","average", rfps_average, "average for this run" );
    // res.w_float ("general","middle", rfps_middlepoint, "per-frame
    // middle-point");
    float fTotal = 0;
    float fNumFrames = 0;
    for ( u32 it = 0; it < m_aFrimeTimes.size(); it++ ) {
    ...
    // Output statistics
    res.w_float( "general", "average", fNumFrames / fTotal,
...
// }
//
// void xrSASH::DescribeOption(char* pszOptionName, const oaOptionDependency&
// Dependency)
//{
//     oaNamedOptionStruct Option;
//     oaInitOption(&Option);
...
//     if (pMask)
//     {
//         xr_sprintf(CmdBuf, "%s %s", pOption->Name, (pOption->Value.Bool ? "1"
//         : "0"));
//     }
//     else if (pToken)
//     {
...
// }
//
// void xrSASH::Message(oaErrorType MessageType, const char* pszMsg, va_list&
// mark)
//{
//     VERIFY(m_bInited);
//
