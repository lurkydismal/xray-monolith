...
void CEnvelope::FindNearestKey( float t,
                                KeyIt& min_k,
                                KeyIt& max_k,
                                float eps ) {
    for ( KeyIt k_it = keys.begin(); k_it != keys.end(); k_it++ ) {
...
BOOL CEnvelope::ScaleKeys( float from_time,
                           float to_time,
                           float scale_factor,
                           float eps ) {
    ...
    if ( min_k != keys.end() && min_k != max_k ) {
        ...
        for ( ; it != max_k; it++ ) {
            float new_time =
                offset + t0 + ( ( *it )->time - t0 ) * scale_factor;
            offset += ( ( new_time - ( *( it - 1 ) )->time ) -
                        ( ( *it )->time - t0 ) );
            t0 = ( *it )->time;
            ( *it )->time = new_time;
        }
...
void CEnvelope::LoadA( IReader& F ) {
    ...
    if ( strstr( buf, "{ Envelope" ) ) {
        ...
        for ( u32 i = 0; i < keys.size(); i++ ) {
            ...
            F.r_string( buf, sizeof( buf ) );
            int cnt =
                sscanf( buf, "Key %f %f %f %f %f %f %f %f %f", f + 0, f + 1,
        ...
        F.r_string( buf, sizeof( buf ) );
        int cnt =
            sscanf( buf, "Behaviors %d %d", behavior[ 0 ], behavior[ 1 ] );
        R_ASSERT( cnt == 2 );
    }
}
