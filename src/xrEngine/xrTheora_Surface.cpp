...
BOOL CTheoraSurface::Load( const char* fname ) {
    ...
    if ( res ) {
        ...
        if ( m_alpha ) {
            ...
            VERIFY( m_rgb->t_info.frame_height ==
                    m_alpha->t_info.frame_height );
            VERIFY( m_rgb->t_info.pixelformat == m_alpha->t_info.pixelformat );
        }
#endif
        //. VERIFY3
        //(btwIsPow2(m_rgb->t_info.frame_width)&&btwIsPow2(m_rgb->t_info.frame_height),"Invalid
        //size.",fname);
        ...
        if ( !bShaderYUV2RGB ) {
            for ( u32 h = 0; h < height; ++h ) {
                ...
                for ( u32 w = 0; w < width; ++w ) {
                    ...
                    int G = clampr( ( 298 * C - 100 * D - 208 * E + 128 ) >> 8,
                                    0, 255 );
            ...
            for ( u32 y_h = 0, uv_h = 0; y_h < height;
                  y_h += 2, ++uv_h, pos += buff_double_step ) {
...
void CTheoraSurface::open_sdl_video() {
    ...
    sdl_screen = SDL_SetVideoMode( t_info.frame_width, t_info.frame_height, 0,
                                   SDL_SWSURFACE );
    if ( sdl_screen == NULL ) {
        msg( "Unable to set %dx%d video: %s", t_info.frame_width,
             t_info.frame_height, SDL_GetError() );
        return;
    }

    sdl_yuv_overlay = SDL_CreateYUVOverlay(
        t_info.frame_width, t_info.frame_height, SDL_YV12_OVERLAY, sdl_screen );
...
void CTheoraSurface::write_sdl_video() {
    ...
    for ( i = 0; i < sdl_yuv_overlay->h; i++ )
        mem_copy(
            ...
            t_yuv_buffer.y + crop_offset + t_yuv_buffer.y_stride * i,
            sdl_yuv_overlay->w );
    crop_offset = ( t_info.offset_x / 2 ) +
                  ( t_yuv_buffer.uv_stride ) * ( t_info.offset_y / 2 );
    for ( i = 0; i < sdl_yuv_overlay->h / 2; i++ ) {
        mem_copy(
            sdl_yuv_overlay->pixels[ 1 ] + sdl_yuv_overlay->pitches[ 1 ] * i,
            t_yuv_buffer.v + crop_offset + t_yuv_buffer.uv_stride * i,
            sdl_yuv_overlay->w / 2 );
        mem_copy(
            sdl_yuv_overlay->pixels[ 2 ] + sdl_yuv_overlay->pitches[ 2 ] * i,
            t_yuv_buffer.u + crop_offset + t_yuv_buffer.uv_stride * i,
            sdl_yuv_overlay->w / 2 );
    }
