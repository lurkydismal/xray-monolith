...
class ENGINE_API IRender_Light : public xr_resource, public ISpatialOwner {
public:
    enum LT {
        DIRECT = 0,
        POINT = 1,
...
public:
    ...
    virtual void set_cm_textures( const shared_str& tex0,
                                  const shared_str& tex1 ) = 0;
...
public:
    ...
    virtual void add_Occluder(
        Fbox2& bb_screenspace ) = 0; // mask screen region as oclluded (-1..1,
                                     // -1..1)
    // virtual void add_StaticWallmark (ref_shader& S, const Fvector& P, float
    // s, CDB::TRI* T, Fvector* V)=0;
    ...
    // 0; virtual void add_SkeletonWallmark (const Fmatrix* xf, CKinematics*
    // obj, ref_shader& sh, const Fvector& start, const Fvector& dir, float
    // size)=0;
    //  Prefer this function when possible
    ...
    virtual void model_CollectTextures( LPCSTR name,
    ...
    virtual void TakeScreenshot( LPCSTR path,
                                 Fvector2 dimensions,
                                 DxEncoding encoding = eDXE_A8R8G8B8 ) = 0;
