...
BOOL CCF_DynamicMesh::_RayQuery( const collide::ray_defs& Q,
    ...
    struct spick {
        ...
        spick( const collide::ray_defs& Q_,
               const CObject& obj_,
               IKinematics& K_ )
            : Q( Q_ ), obj( obj_ ), K( K_ ) {}
        ...
        bool operator()( collide::rq_result& r ) {
            ...
            VERIFY( r.O == &obj );
            bool res = K.PickBone( obj.XFORM(), br, Q.range, Q.start, Q.dir,
                                   ( u16 )r.element );
            if ( res ) {
                r.range = br.dist;
            }
    ...
    } pick( ( collide::ray_defs& )( Q ), ( const CObject& )( *owner ),
            ( IKinematics& )( *K ) );

    R.r_results().erase( std::remove_if( R.r_results().begin() + s_count,
                                         R.r_results().end(), pick ),
                         R.r_results().end() );
    /*
    for( collide::rq_result* i = R.r_begin() + s_count; i < R.r_end(); ++i )
    {
    IKinematics::pick_result r;
    if( K->PickBone( owner->XFORM(), r, Q.range, Q.start, Q.dir, (u16)
