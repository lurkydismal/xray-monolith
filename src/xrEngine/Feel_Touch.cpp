...
void Touch::feel_touch_update( Fvector& C, float R ) {
    ...
    if ( n_end != n_begin ) {
        // Process results (NEW)
        for ( xr_vector< CObject* >::iterator it = n_begin; it != n_end;
              it++ ) {
            CObject* O = *it;
            if ( O->getDestroy() )
            ...
            if ( std::find( feel_touch.begin(), feel_touch.end(), O ) ==
                 feel_touch.end() ) {
                // check for deny
                BOOL bDeny = FALSE;
                for ( u32 dit = 0; dit < feel_touch_disable.size(); dit++ )
    ...
    for ( int d = 0; d < int( feel_touch.size() ); d++ ) {
        ...
        if ( O->getDestroy() || !feel_touch_contact( O ) ||
             ( std::find( n_begin, n_end, O ) == n_end ) )
