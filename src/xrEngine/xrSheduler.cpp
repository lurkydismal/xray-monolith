...
bool CSheduler::internal_Unregister( ISheduled* O,
                                     BOOL RT,
                                     bool warn_on_not_found ) {
    ...
    if ( warn_on_not_found )
        Msg( "! scheduled object %s tries to unregister but is not registered",
             *O->shedule_Name() );
#endif // DEBUG
...
bool CSheduler::Registered( ISheduled* object ) const {
    ...
    if ( !count && ( m_current_step_obj == object ) ) {
        VERIFY2( m_processing_now,
                 "trying to unregister self unregistering object while not "
                 "processing now" );
        count = 1;
    }
...
void CSheduler::Unregister( ISheduled* A ) {
    ...
    if ( !( ( !A->shedule.b_RT && m_processing_now ) ||
            ( A->shedule.b_RT && m_processing_nowRT ) ) ) {
...
void CSheduler::ProcessStep() {
    ...
    u32 batchSize = g_pGameLevel && g_pGameLevel->schedulerFlush
                        ? 65536
                        : SchedulerBatchSize;
    ...
    int i = 0;
    {
        for ( i = 0; i < ItemsBatch.size(); ++i ) {
            ...
            // Stop if really underestimated the cost of batch, check every 8th
            // item
            if ( ( i % 8 ) == 0 && Device.dwPrecacheFrame == 0 &&
                 CPU::QPC() > cycles_limit ) {
            ...
            T.Object->shedule_Update( clampr(
                Elapsed, u32( 1 ),
                u32( _max( u32( T.Object->shedule.t_max ), u32( 1000 ) ) ) ) );
    ...
    if ( SchedulerLog )
        Msg( "Sheduler: cycles_start[%llu] cycles_limit[%llu] qpc_freq[%llu] "
             "Target[%f] psShedulerReaction[%f] NewTarget[%f] ItemsCount[%d] "
             "ItemsBatch[%d], ItemsActuallyProcessed [%d]",
             cycles_start, cycles_limit, CPU::qpc_freq, target,
             psShedulerReaction, psShedulerTarget, ItemsCount,
             ItemsBatch.size(), i );

    // Reinsertion
    {
        // Strategy
        // Rule of thumb: if k < n / log2(n), push_heap is faster. With default
        // batch size of 128 its always faster, start choosing only with 256
        // Otherwise, dump and heapify
        // For n=3000, log2(n) is ~11.5. n/11.5 is ~260.
        // We use a simple approximation: n >> 4 (which is n / 16) for a
...
void CSheduler::UpdateDeferred() {
    ...
    cycles_limit = CPU::qpc_freq * u64( iCeil( psShedulerCurrent ) ) / 1000ull +
                   cycles_start;
