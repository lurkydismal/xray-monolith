...
BOOL CAviPlayerCustom::Load( char* fname ) {
    ...
    MMRESULT res;
    if ( MMSYSERR_NOERROR != ( res = mmioDescend( hmmioFile, &mmckinfoParent,
                                                  NULL, MMIO_FINDRIFF ) ) ) {
    ...
    if ( mmckinfoParent.cksize != ( DWORD )mmioRead( hmmioFile, ( HPSTR )&strh,
                                                     mmckinfoParent.cksize ) ) {
        mmioClose( hmmioFile, 0 );
        return FALSE;
    }
    ...
    if ( mmckinfoParent.cksize != ( DWORD )mmioRead( hmmioFile,
                                                     ( HPSTR )&m_biInFormat,
                                                     mmckinfoParent.cksize ) ) {
        mmioClose( hmmioFile, 0 );
        return FALSE;
    ...
    if ( ICERR_OK !=
         ICDecompressBegin( m_aviIC, &m_biInFormat, &m_biOutFormat ) ) {
        return FALSE;
    }
    ...
    if ( MMSYSERR_NOERROR != ( res = mmioDescend( hmmioFile, &mmckinfoSubchunk,
                                                  NULL, MMIO_FINDLIST ) ) ||
    ...
    if ( mmckinfoSubchunk.cksize !=
         ( DWORD )mmioRead( hmmioFile, ( HPSTR )m_pMovieData,
    ...
    if ( mmckinfoSubchunk.cksize !=
         ( DWORD )mmioRead( hmmioFile, ( HPSTR )m_pMovieIndex,
                            mmckinfoSubchunk.cksize ) ) {
        xr_free( m_pMovieIndex );
        m_pMovieIndex = NULL;
        xr_free( m_pMovieData );
...
BOOL CAviPlayerCustom::DecompressFrame( DWORD dwFrameNum ) {
    ...
    DWORD dwFlags;
    dwFlags = ( pCurrFrameIndex->dwFlags & AVIIF_KEYFRAME )
                  ? 0
    ...
    if ( ICERR_OK !=
         ICDecompress( m_aviIC, dwFlags, &m_biInFormat,
        ...
        if ( dwCurrFrame == m_dwFrameCurrent + 1 ) {
            ...
            *pDest = m_pDecompressedBuf;
            ...
            if ( !( m_pMovieIndex[ dwCurrFrame ].dwFlags & AVIIF_KEYFRAME ) ) {
                ...
                // делаем PreRoll от ближайшего предидущего ключевого кадра до
                // Заданного-1
                PreRoll( dwCurrFrame );
            }
...
// минимум проверок на валидность переданного для преролла кадра - нужна
// скорость
VOID CAviPlayerCustom::PreRoll( DWORD dwFrameNum ) {
    ...
    for ( i = ( int )dwFrameNum - 1; i > 0; i-- ) {
        ...
        if ( ( int )m_dwFrameCurrent == i ) {
            ...
            // декомпрессим все последующие НЕключевые кадры с флагами PREROLL &
            // NOTKEYFRAME
            for ( i++; i < ( int )dwFrameNum; i++ ) {
                pCurrFrameIndex = &m_pMovieIndex[ i ];

                DWORD dwFlags;
                dwFlags = ICDECOMPRESS_PREROLL | ICDECOMPRESS_NOTKEYFRAME |
                          ICDECOMPRESS_HURRYUP;
                m_biInFormat.biSizeImage = pCurrFrameIndex->dwChunkLength;
                dwFlags |=
                    ( m_biInFormat.biSizeImage ) ? 0 : ICDECOMPRESS_NULLFRAME;

                res = ICDecompress(
                    m_aviIC, dwFlags, &m_biInFormat,
                    ( m_pMovieData + pCurrFrameIndex->dwChunkOffset +
                      8 ) /*m_pCompressedBuf*/,
                    &m_biOutFormat, m_pDecompressedBuf );
    ...
    res = ICDecompress(
        m_aviIC, ICDECOMPRESS_PREROLL | ICDECOMPRESS_HURRYUP, &m_biInFormat,
...
DWORD CAviPlayerCustom::CalcFrame() {
    ...
    return DWORD( floor( ( RDEVICE.dwTimeContinual - m_dwFirstFrameOffset ) *
                         m_fCurrentRate / 1000.0f ) ) %
           m_dwFrameTotal;
}
