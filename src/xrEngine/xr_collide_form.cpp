...
bool CCF_Skeleton::_ElementCenter( u16 elem_id, Fvector& e_center ) {
    ...
    ElementVecIt it = std::lower_bound( elements.begin(), elements.end(),
                                        elem_id, pred_find_elem );
...
IC bool RAYvsOBB( const Fmatrix& IM,
                  const Fvector& b_hsize,
                  const Fvector& S,
                  const Fvector& D,
                  float& R,
                  BOOL bCull ) {
    Fbox E = { -b_hsize.x, -b_hsize.y, -b_hsize.z,
               b_hsize.x,  b_hsize.y,  b_hsize.z };
    // XForm world-2-local
    Fvector SL, DL, PL;
    ...
    if ( ( rp_res == Fbox::rpOriginOutside ) ||
         ( !bCull && ( rp_res == Fbox::rpOriginInside ) ) ) {
...
IC bool RAYvsSPHERE( const Fsphere& s_sphere,
                     const Fvector& S,
                     const Fvector& D,
                     float& R,
                     BOOL bCull ) {
    ...
    VERIFY( R >= 0.f );
    return ( ( rp_res == Fsphere::rpOriginOutside ) ||
             ( !bCull && ( rp_res == Fsphere::rpOriginInside ) ) );
}

IC bool RAYvsCYLINDER( const Fcylinder& c_cylinder,
                       const Fvector& S,
                       const Fvector& D,
                       float& R,
                       BOOL bCull ) {
    ...
    VERIFY( R >= 0.f );
    return ( ( rp_res == Fcylinder::rpOriginOutside ) ||
             ( !bCull && ( rp_res == Fcylinder::rpOriginInside ) ) );
}

CCF_Skeleton::CCF_Skeleton( CObject* O ) : ICollisionForm( O, cftObject ) {
    // getVisData
    IRenderVisual* pVisual = O->Visual();
    // IKinematics* K = PKinematics(pVisual); VERIFY3(K,"Can't create skeleton
    // without Kinematics.",*O->cNameVisual());
    IKinematics* K = PKinematics( pVisual );
    VERIFY3( K, "Can't create skeleton without Kinematics.",
             *O->cNameVisual() );
...
void CCF_Skeleton::BuildState() {
    ...
    // Non-blocking: BuildState runs inside the IK callback from CalculateBones,
    // which already holds the current skeleton's UCalc_Mutex; a blocking
    // acquire of this neighbour skeleton's lock could cross-lock two objects in
    // opposite order on two threads (AB-BA deadlock). If it's mid-recalc
    // elsewhere, read its 1-frame-stale transforms instead of blocking.
    ...
    for ( ElementVecIt I = elements.begin(); I != elements.end(); I++ ) {
        ...
        VERIFY2( DET( Mbone ) > EPS,
                 ( make_string( "0 scale bone matrix, %d \n", I->elem_id ) +
                   dbg_object_full_dump_string( owner ) )
                     .c_str() );
#endif
        switch ( I->type ) {
            case SBoneShape::stBox: {
                ...
                // \n", I->elem_id ) + dbg_object_full_dump_string( owner )
                // ).c_str() );
                ...
                if ( !b ) {
                    ...
                    Msg( "! ERROR: bone_id=[%d], world_pos[%f,%f,%f]",
                         I->elem_id, VPUSH( TW.c ) );
...
BOOL CCF_Skeleton::_RayQuery( const collide::ray_defs& Q,
    ...
    float aft[ 2 ];
    int quant;
    Fsphere::ERP_Result res =
        w_bv_sphere.intersect( Q.start, Q.dir, tgt_dist, quant, aft );
    if ( ( Fsphere::rpNone == res ) ||
         ( ( Fsphere::rpOriginOutside == res ) && ( aft[ 0 ] > tgt_dist ) ) )
        return FALSE;
    ...
    for ( ElementVecIt I = elements.begin(); I != elements.end(); I++ ) {
        ...
        switch ( I->type ) {
            case SBoneShape::stBox:
                res = RAYvsOBB( I->b_IM, I->b_hsize, Q.start, Q.dir, range,
                                Q.flags & CDB::OPT_CULL );
                break;
            case SBoneShape::stSphere:
                res = RAYvsSPHERE( I->s_sphere, Q.start, Q.dir, range,
                                   Q.flags & CDB::OPT_CULL );

                break;
            case SBoneShape::stCylinder:
                res = RAYvsCYLINDER( I->c_cylinder, Q.start, Q.dir, range,
                                     Q.flags & CDB::OPT_CULL );
                break;
        }
        if ( res ) {
            bHIT = TRUE;
            R.append_result( owner, range, I->elem_id,
                             Q.flags & CDB::OPT_ONLYNEAREST );
...
BOOL CCF_EventBox::_RayQuery( const collide::ray_defs& Q,
                              collide::rq_results& R ) {
    return FALSE;
}
...
BOOL CCF_Shape::_RayQuery( const collide::ray_defs& Q,
                           collide::rq_results& R ) {
    ...
    for ( u32 el = 0; el < shapes.size(); el++ ) {
        ...
        switch ( shape.type ) {
            case 0: {
                // sphere
                Fsphere::ERP_Result rp_res =
                    shape.data.sphere.intersect( dS, dD, range );
                if ( ( rp_res == Fsphere::rpOriginOutside ) ||
                     ( !( Q.flags & CDB::OPT_CULL ) &&
                       ( rp_res == Fsphere::rpOriginInside ) ) ) {
                    bHIT = TRUE;
                    R.append_result( owner, range, el,
                                     Q.flags & CDB::OPT_ONLYNEAREST );
                    if ( Q.flags & CDB::OPT_ONLYFIRST )
                        return TRUE;
                }
