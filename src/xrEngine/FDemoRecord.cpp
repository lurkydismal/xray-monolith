...
CDemoRecord::CDemoRecord( const char* name,
                          float life_time,
    ...
    if ( file ) {
        ...
        if ( return_ctrl_inputs ) {
            ...
            float distanceInFrontOfActor =
                3.0f; // Set this to the desired distance
            Fvector cameraOffset = m_Camera.k;
            cameraOffset.mul(
                -distanceInFrontOfActor ); // Note the negative sign
...
CDemoRecord::CDemoRecord( const char* name,
                          ...
                          float life_time,
                          BOOL return_ctrl_inputs )
    : CDemoRecord( name, life_time, return_ctrl_inputs ) {
    ...
    // when demo_record is launched, mouse controls are redirected to camera
    // movements, so we hide the cursor
    GetUICursor().Hide();
}
    ...
    switch ( m_Stage ) {
        ...
        case DEVICE_RESET_PRECACHE_FRAME_COUNT + 30: {
            ...
            if ( m_iLMScreenshotFragment == -1 )
                xr_sprintf( tmp, sizeof( tmp ), "map_%s",
                            *g_pGameLevel->name() );
            else
                xr_sprintf( tmp, sizeof( tmp ), "map_%s#%d",
                            *g_pGameLevel->name(), m_iLMScreenshotFragment );
            ...
            if ( m_iLMScreenshotFragment == -1 ||
                 m_iLMScreenshotFragment == 4 ) {
                psHUD_Flags.assign( s_hud_flag );

                BOOL bDevReset =
                    !psDeviceFlags.equal( s_dev_flags, rsFullscreen );
...
void CDemoRecord::MakeCubeMapFace( Fvector& D, Fvector& N ) {
    ...
    switch ( m_Stage ) {
        ...
        case 5:
            N.set( cmNorm[ m_Stage ] );
            D.set( cmDir[ m_Stage ] );
            Render->Screenshot( IRender_interface::SM_FOR_CUBEMAP,
                                itoa( m_Stage, buf, 10 ) );
            break;
        case 6:
            Render->Screenshot( IRender_interface::SM_FOR_CUBEMAP,
...
BOOL CDemoRecord::ProcessCam( SCamEffectorInfo& info ) {
    ...
    if ( m_bMakeScreenshot ) {
        ...
        if ( IR_GetKeyState( DIK_F1 ) ) {
            ...
            pApp->pFontSystem->OutNext(
                "= Level Map ScreenShot(High Quality)" );
        ...
        if ( m_CameraBoundaryEnabled ) {
            ...
            if ( _abs( m_Position.x - m_Actor_Position.x ) >
                 m_fCameraBoundary ) {
                x = m_Starting_Position.x;
            }
            if ( _abs( m_Position.y - m_Actor_Position.y ) >
                 m_fCameraBoundary ) {
                y = m_Starting_Position.y;
            }
            if ( _abs( m_Position.z - m_Actor_Position.z ) >
                 m_fCameraBoundary ) {
...
void CDemoRecord::IR_OnKeyboardPress( int dik ) {
    if ( isInputBlocked ) {
        ...
        // Added dik == DIK_RCONTROL as extra keybind to support keyboards with
        // no numpad Added dik == DIK_TAB as extra keybind that can also be used
        // if return_ctrl_inputs is enabled. This key press event can be
        // captured by the Demo Record invoker
        if ( dik == DIK_MULTIPLY || dik == DIK_RCONTROL ||
             ( dik == DIK_TAB && return_ctrl_inputs ) )
            m_b_redirect_input_to_level = !m_b_redirect_input_to_level;

        if ( m_b_redirect_input_to_level ) {
            // control inputs are redirected to the invoker
            if ( return_ctrl_inputs ) {
                // if the invoker is a script that launched demo_record with
                // return_ctrl_inputs enabled we show the cursor
                GetUICursor().Show();
            }
            ...
            if ( GetUICursor().IsVisible() ) {
                // mouse controls are back to the camera movements, so we hide
                // the cursor
                GetUICursor().Hide();
            }
            // we end up here if controls have not been redirected to the
            // invoker if we launched demo_record_return_ctrl_inputs we return
            // the event DIK_TAB key press also to the invoker
            if ( dik == DIK_TAB && return_ctrl_inputs ) {
                // Sends upstream the DIK_TAB keyboard press event. This key
                // toggles controls between demo_record and its launcher (game
                // console or script) upon relinquishing the controls (to
                // gameconsole/scripts), if the DIK_TAB key is pressed (captured
                // by DemoRecord regardless) we end up here but we like to let
                // know the invoker that the DIK_TAB key was pressed and
                // controls are back into DemoRecord hands
                g_pGameLevel->IR_OnKeyboardPress( dik );
                return;
            }
        }
        ...
        if ( dik == DIK_ESCAPE ) {
            StopDemo();
            // if we launched demo_record_return_ctrl_inputs we return the event
            // ESCAPE key press also to the launcher entity
            if ( return_ctrl_inputs ) {
                // we also show the cursor
                GetUICursor().Show();
                // sends upstream the DIK_ESCAPE keyboard press event. This key
                // quit demo_record and returns controls to its launcher (game
                // console or script) This can help scripts that execute command
                // demo_record_photomode to know when the demo_record has exited
                g_pGameLevel->IR_OnKeyboardPress( dik );
            }
        }
...
void CDemoRecord::IR_OnKeyboardRelease( int dik ) {
    ...
    if ( m_b_redirect_input_to_level ) {
        ...
        if ( dik == DIK_F12 && return_ctrl_inputs ) {
            // if demo_record_return_ctrl_inputs is enabled we return the event
            // F12 key release also to the launcher entity
            g_pGameLevel->IR_OnKeyboardRelease( dik );
        }
    }
}
...
void CDemoRecord::IR_OnKeyboardHold( int dik ) {
    ...
    switch ( dik ) {
        ...
        case DIK_NUMPAD4:
            vR_delta.y -= 1.0f;
            break;  // Turn Right
        case DIK_C: // Added DIK_C as extra keybind to support keyboards with no
                    // numpad
        case DIK_NUMPAD9:
            vR_delta.z -= 2.0f;
            break;  // tilt Right
        case DIK_Z: // Added DIK_Z as extra keybind to support keyboards with no
                    // numpad
        case DIK_NUMPAD7:
            vR_delta.z += 2.0f;
            break; // tilt left
...
void CDemoRecord::IR_OnMouseMove( int dx, int dy ) {
    ...
    if ( dx || dy ) {
        ...
        vR_delta.x += ( ( psMouseInvert.test( 1 ) ) ? -1 : 1 ) * float( dy ) *
                      scale * ( 3.f / 4.f ); // pitch
    }
    update_whith_timescale( m_vR, vR_delta );
}
