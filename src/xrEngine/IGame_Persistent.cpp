...
bool IsMainMenuActive() {
    return g_pGamePersistent && g_pGamePersistent->m_pMainMenu &&
...
void IGame_Persistent::OnAppStart() {
    ...
    m_textures_prefetch_config = xr_new< CInifile >(
        FS.update_path( file_name, "$game_config$", "prefetch\\textures.ltx" ),
        TRUE, TRUE, FALSE );
}
...
void IGame_Persistent::Prefetch() {
    ...
    if ( m_textures_prefetch_config->section_exist( "prefetch_folders" ) ) {
        const CInifile::Sect& section =
            m_textures_prefetch_config->r_section( "prefetch_folders" );
        texture_folders.reserve( section.Data.size() );
        for ( CInifile::SectCIt item = section.Data.begin();
              item != section.Data.end(); ++item )
            texture_folders.push_back(
                { item->first,
                  item->second.size() && !xr_strcmp( *item->second, "*" ) } );
    }
    if ( m_textures_prefetch_config->section_exist( "prefetch_textures" ) ) {
        const CInifile::Sect& section =
            m_textures_prefetch_config->r_section( "prefetch_textures" );
    ...
    strconcat( sizeof( object_section ), object_section, "prefetch_objects_",
               m_game_params.m_game_type );
    if ( pSettings->section_exist( object_section ) ) {
        ...
        for ( CInifile::SectCIt item = section.Data.begin();
            ...
            if ( pSettings->section_exist( item->first.c_str() ) &&
                ...
                object_visuals.push_back(
                    pSettings->r_string( item->first.c_str(), "visual" ) );
    }
    ...
    auto submit = [ &executor ]( NativeLoadPriority priority, auto&& work ) {
        if ( prefetch_batch.Valid() )
            executor.Submit( prefetch_batch, priority,
                             std::forward< decltype( work ) >( work ) );
        else
            prefetch_task.run( std::forward< decltype( work ) >( work ) );
    };
    submit( NativeLoadPriority::ShaderTexture, [ texture_folders,
                                                 texture_names ]() {
        {
            ...
            const auto loadFileFolder = [ & ]( LPCSTR _folder ) {
                ...
                for ( FS_FileSet::iterator it = fset.begin(); it != fset.end();
                      it++ )
                    Device.m_pRender->ResourcesPrefetchCreateTexture(
                        it->name.c_str() );
            };
            ...
            for ( const texture_folder& item : texture_folders ) {
                if ( item.recursive ) {
                    string_path folder;
                    FS.update_path( folder, "$game_textures$",
                                    item.name.c_str() );
                    xr_strcat( folder, sizeof( folder ), "\\" );
                    xr_vector< LPSTR >* subfolders =
                        FS.file_list_open( folder, FS_ListFolders );
                    if ( subfolders ) {
                        for ( LPSTR subfolder : *subfolders ) {
                            string_path path;
                            strconcat( sizeof( path ), path, folder,
                                       subfolder );
                            loadFileFolder( path );
                        }
                    }
...
void IGame_Persistent::UpdateParticles() {
    ...
    while ( !ps_needtoplay.empty() ) {
        intrusive_ptr< CPS_Instance > pInstance =
            std::move( ps_needtoplay.back() );
        ps_needtoplay.pop_back();
        pInstance->Play( false );
    }

    ps_active.insert( ps_active.end(),
    ...
    ps_active.erase(
        std::remove_if( ps_active.begin(), ps_active.end(), eraseFunc ),
        ps_active.end() );
}
...
void IGame_Persistent::OnAssetsChanged() {
    ...
    Device.m_pRender
        ->OnAssetsChanged(); // Resources->m_textures_description.Load();
#endif
}

void IGame_Persistent::GrassBendersUpdate( u16 id,
                                           u8& data_idx,
                                           u32& data_frame,
                                           Fvector& position,
                                           float init_radius,
    ...
    if ( data_idx != NULL ) {
        // Explosions can take the mem spot, unassign and try to get a spot
        // later.
    ...
    // Check Distance
    if ( CheckDistance ) {
        if ( position.distance_to_xz_sqr( Device.vCameraPosition ) >
             ps_ssfx_grass_interactive.z ) {
    ...
    float rad =
        data_idx == NULL
            ? 1.0
            : std::max( 1.0f,
                        grass_shader_data.radius_curr[ data_idx ] + 0.5f );
    ...
    if ( data_idx == NULL ) {
        ...
        if ( grass_shader_data.id[ idx ] == NULL ) {
            data_idx = idx;
            GrassBendersSet( idx, id, position, Fvector3().set( 0, -99, 0 ), 0,
                             0, 0.0f, init_radius, BENDER_ANIM_DEFAULT, true );
...
void IGame_Persistent::GrassBendersAddExplosion( u16 id,
                                                 Fvector position,
                                                 Fvector3 dir,
                                                 float fade,
                                                 float speed,
    ...
    for ( int idx = 1; idx < ps_ssfx_grass_interactive.y + 1; idx++ ) {
        ...
        if ( grass_shader_data.anim[ idx ] != BENDER_ANIM_EXPLOSION ) {
            // Add 99 to the ID to avoid conflicts between explosions and basic
            // benders happening at the same time with the same ID.
            GrassBendersSet( idx, id + 99, position, dir, fade, speed,
                             intensity, radius, BENDER_ANIM_EXPLOSION, true );
...
void IGame_Persistent::GrassBendersAddShot( u16 id,
                                            Fvector position,
                                            Fvector3 dir,
                                            float fade,
                                            float speed,
    ...
    for ( int idx = 1; idx < ps_ssfx_grass_interactive.y + 1; idx++ ) {
        ...
        if ( grass_shader_data.id[ idx ] == id ) {
            ...
            GrassBendersSet( idx, id, position, dir, fade, speed, currentSTR,
                             radius, BENDER_ANIM_EXPLOSION, false );
            grass_shader_data.str_target[ idx ] += intensity;
            AddAt = -1;
            break;
        } else {
    ...
    if ( AddAt != -1 ) {
        GrassBendersSet( AddAt, id, position, dir, fade, speed, 0.001f, radius,
                         BENDER_ANIM_EXPLOSION, true );
    ...
    for ( int idx = 1; idx < ps_ssfx_grass_interactive.y + 1; idx++ ) {
        if ( grass_shader_data.id[ idx ] != NULL ) {
            switch ( grass_shader_data.anim[ idx ] ) {
                case BENDER_ANIM_EXPLOSION: // Internal Only ( You can use
                                            // BENDER_ANIM_PULSE for anomalies )
                {
                    // Radius
                    grass_shader_data.time[ idx ] +=
                        Device.fTimeDelta * grass_shader_data.speed[ idx ];
                    grass_shader_data.radius_curr[ idx ] =
                        grass_shader_data.radius[ idx ] *
                        std::min( 1.0f, grass_shader_data.time[ idx ] );

                    grass_shader_data.str_target[ idx ] =
                        std::min( 1.0f, grass_shader_data.str_target[ idx ] );

                    // Easing
                    float diff = abs( grass_shader_data.str[ idx ] -
                                      grass_shader_data.str_target[ idx ] );
                    diff = std::max( 0.1f, diff );

                    // Intensity
                    if ( grass_shader_data.str_target[ idx ] <=
                         grass_shader_data.str[ idx ] ) {
                        grass_shader_data.str[ idx ] -=
                            Device.fTimeDelta * grass_shader_data.fade[ idx ] *
                            diff;
                    } else {
                        grass_shader_data.str[ idx ] +=
                            Device.fTimeDelta * grass_shader_data.speed[ idx ] *
                ...
                case BENDER_ANIM_WAVY: {
                    // Anim Speed
                    grass_shader_data.time[ idx ] +=
                        Device.fTimeDelta * 1.5f *
                        grass_shader_data.speed[ idx ];
                ...
                case BENDER_ANIM_SUCK: {
                    ...
                    // Perlin Noise
                    float curve = clampr( PerlinNoise1D->GetContinious(
                                              grass_shader_data.time[ idx ] ) +
                                              0.5f,
                                          0.f, 1.f ) *
                                  -1.0;
                    ...
                    grass_shader_data.str[ idx ] =
                        curve * grass_shader_data.str_target[ idx ];
                } break;

                case BENDER_ANIM_BLOW: {
                    // Anim Speed
                    grass_shader_data.time[ idx ] +=
                        Device.fTimeDelta * 1.2f *
                        grass_shader_data.speed[ idx ];
                ...
                case BENDER_ANIM_PULSE: {
                    ...
                    if ( grass_shader_data.radius_curr[ idx ] >=
                         grass_shader_data.radius[ idx ] )
                        grass_shader_data.str[ idx ] += GrassBenderToValue(
                            grass_shader_data.str[ idx ], 0.0f,
                            grass_shader_data.speed[ idx ] * 0.6f, true );
                ...
                case BENDER_ANIM_DEFAULT:
                    ...
                    grass_shader_data.str[ idx ] += GrassBenderToValue(
                        grass_shader_data.str[ idx ],
                        grass_shader_data.str_target[ idx ], 2.0f, true );
...
void IGame_Persistent::GrassBendersRemoveById( u16 id ) {
    // Search by Object ID ( Used when removing benders
    // CPHMovementControl::DestroyCharacter() )
...
void IGame_Persistent::GrassBendersReset( u8 idx ) {
    // Reset Everything
    GrassBendersSet( idx, NULL, Fvector3().set( 0, 0, 0 ),
                     Fvector3().set( 0, -99, 0 ), 0, 0, 0, 0,
                     BENDER_ANIM_DEFAULT, true );
    grass_shader_data.str_target[ idx ] = 0;
}

void IGame_Persistent::GrassBendersSet( u8 idx,
                                        u16 id,
                                        Fvector position,
                                        Fvector3 dir,
                                        float fade,
...
float IGame_Persistent::GrassBenderToValue( float& current,
                                            float go_to,
                                            float intensity,
                                            bool use_easing ) {
    float diff = abs( current - go_to );

    float r_value = Device.fTimeDelta * intensity *
                    ( use_easing ? std::min( 0.5f, diff ) : 1.0f );
