...
u32 g_screenmode = 1;
xr_token screen_mode_tokens[] = { { "fullscreen", 2 },
                                  { "borderless", 1 },
                                  { "windowed", 0 },
                                  { 0, 0 } };
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        for ( it = Console->Commands.begin(); it != Console->Commands.end();
              it++ ) {
        ...
        Log( "Key: Up   / Down            === Prev / Next command in tips "
             "list " );
        Log(
            "Key: Ctrl + Up / Ctrl + Down === Prev / Next executing command " );
        Log( "Key: Left, Right, Home, End {+Shift/+Ctrl}       === Navigation "
             "in text " );
        Log( "Key: PageUp / PageDown      === Scrolling history " );
        Log( "Key: Tab  / Shift + Tab     === Next / Prev possible command "
             "from list" );
...
public:
    CCC_DumpCVars( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = TRUE;
    }
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        xr_strcpy( cfg_full_name,
                   ( xr_strlen( args ) > 0 ) ? args : Console->ConfigFile );

        bool b_abs_name =
            xr_strlen( cfg_full_name ) > 2 && cfg_full_name[ 1 ] == ':';
...
class CCC_Start : public IConsole_Command {
    ...
    void protect_Name_strlwr( LPSTR str ) {
        ...
        int begin_p =
            xr_strlen( str ) - xr_strlen( name1 ) + xr_strlen( name_str );
        if ( begin_p < 1 ) {
            return;
        }
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( xr_strlen( op_demo ) ) {
            Engine.Event.Defer( "KERNEL:start_mp_demo",
                                u64( xr_strdup( op_demo ) ), 0 );
        } else {
            Engine.Event.Defer(
                "KERNEL:start",
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( cnt == 2 ) {
            const bool changed = ( psCurrentVidMode[ 0 ] != _w ) ||
    ...
    virtual void Status( TStatus& S ) {
        xr_sprintf( S, sizeof( S ), "%dx%d", psCurrentVidMode[ 0 ],
                    psCurrentVidMode[ 1 ] );
    }
...
public:
    CCC_Screenmode( LPCSTR N )
        : CCC_Token( N, &g_screenmode, screen_mode_tokens ) {};
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( ( prev_mode != g_screenmode ) ) {
            // TODO: If you enable the debug layer for DX11 and switch between
            // fullscreen and windowed a few times, you'll occasionally see the
            // following error in the output: DXGI ERROR:
            // IDXGISwapChain::Present: The application has not called
            // ResizeBuffers or re-created the SwapChain after a fullscreen or
            // windowed transition. Flip model swapchains
            // (DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL and
            // DXGI_SWAP_EFFECT_FLIP_DISCARD) are required to do so. [
            // MISCELLANEOUS ERROR #117: ] This shouldn't happen if we're
            // calling reset on FS/Windowed transitions, I tried logging where
            // Present() is called as well as where ResizeBuffers and
            // SetFullscreenState are called, and the debug output looks like
            // this:
            //
            // Present()
            // SetFullscreenState()
            ...
            // Present()
            //
            // Which makes no sense, it's impossible for us to have called
            // Present between SetFullscreenState and ResizeBuffers so DX11 must
            ...
            //
            // but the fixes make no sense and contradicts MSDN, and this isn't
            // a major priority since ResizeBuffers is called immediately after
            // (before our Present call) so it works, just so stupid
        ...
        MapWindowPoints( Device.m_hWnd, nullptr,
                         reinterpret_cast< LPPOINT >( &winRect ), 2 );
        ClipCursor( &winRect );
    }
};
//-----------------------------------------------------------------------
...
Msg("Local BBox (%f, %f) - (%f, %f)", g_DR_LM_Min.x, g_DR_LM_Min.z,
g_DR_LM_Max.x, g_DR_LM_Max.z);
}
};
...
{
public:
CCC_DR_UsePoints(LPCSTR N, int* V, int _min=0, int _max=999) : CCC_Integer(N, V,
_min, _max) {}; virtual void Save (IWriter *F) {};
};
#endif
*/
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        // If sound system is fully initialized, use switch_device which
        // validates and sets name Otherwise (during init phase), just set the
        // name for _initialize(1) to use
...
public:
    ...
    virtual void Execute( LPCSTR args ) override {
        ...
        if ( !Device.ChangeOutputMonitor( h ) )
            Msg(
                "! vid_monitor: live switch unavailable; restart to apply '%s'",
                args );
    }
...
public:
    CCC_SoundParamsSmoothing( LPCSTR N, int* V, int _min = 0, int _max = 999 )
...
void CCC_Register() {
    ...
    CMD4( CCC_Float, "snd_efx_environment_change_time",
          &snd_efx_environment_change_time, 0.f, 3.f );
    ...
    CMD4( CCC_Float, "snd_distance_based_delay_power",
          &soundSmoothingParams::distanceBasedDelayPower, 0.f, 2.f );
    CMD4( CCC_Float, "snd_distance_based_delay_min_distance",
          &soundSmoothingParams::distanceBasedDelayMinDistance, 15.f, 100.f );

    // Pitch variation power
    CMD4( CCC_Float, "snd_pitch_variation_power",
          &soundSmoothingParams::pitchVariationPower, 0.f, 4.f );

    // Doppler effect power
    CMD4( CCC_Float, "snd_doppler_power", &soundSmoothingParams::power, 0.f,
          5.f );
    CMD4( CCC_SoundParamsSmoothing, "snd_doppler_smoothing",
          &soundSmoothingParams::steps, 1, 100 );

    // EFX Reverb overwrite
    CMD4( CCC_Integer, "snd_efx_reverb_overwrite", &reverb_overwrite, FALSE,
          TRUE );

    CMD4( CCC_Float, "snd_efx_reverb_overwrite_density", &psReverbDensity, 0.f,
          1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_diffusion", &psReverbDiffusion,
          0.f, 1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_gain", &psReverbGain, 0.f, 1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_gain_hf", &psReverbGainHF, 0.f,
          1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_gain_lf", &psReverbGainLF, 0.f,
          1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_decay_time", &psReverbDecayTime,
          0.1f, 20.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_decay_hf_ratio",
          &psReverbDecayHFRatio, 0.1f, 20.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_decay_lf_ratio",
          &psReverbDecayLFRatio, 0.1f, 20.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_reflections_gain",
          &psReverbReflectionsGain, 0.f, 3.16f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_reflections_delay",
          &psReverbReflectionsDelay, 0.f, 0.3f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_late_reverb_gain",
          &psReverbLateReverbGain, 0.f, 10.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_late_reverb_delay",
          &psReverbLateReverbDelay, 0.f, 0.1f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_echo_time", &psReverbEchoTime,
          0.075f, 0.25f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_echo_depth", &psReverbEchoDepth,
          0.f, 1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_modulation_time",
          &psReverbModulationTime, 0.04f, 4.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_modulation_depth",
          &psReverbModulationDepth, 0.f, 1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_air_absorption_gain_hf",
          &psReverbAirAbsorptionGainHF, 0.892f, 1.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_hf_reference",
          &psReverbHFReference, 1000.f, 20000.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_lf_reference",
          &psReverbLFReference, 20.f, 1000.f );
    CMD4( CCC_Float, "snd_efx_reverb_overwrite_room_rolloff_factor",
          &psReverbRoomRolloffFactor, 0.f, 10.f );
    CMD4( CCC_Integer, "snd_efx_reverb_overwrite_decay_hf_limit",
          &psReverbDecayHFLimit, FALSE, TRUE );
    ...
    // CMD4(CCC_Integer, "sv_dedicated_server_update_rate",
    // &g_svDedicateServerUpdateReate, 1, 1000);
    ...
    if ( Core.ParamsData.test( ECoreParams::dbgdev ) )
        CMD4( CCC_Float, "g_freelook_z_offset_factor", &g_freelook_z_offset,
              -3.f, 3.f );

    CMD4( CCC_Float, "g_ironsights_zoom_factor", &g_ironsights_factor, 1.f,
          2.f );
    CMD4( CCC_Vector3, "ssfx_wetness_multiplier", &ssfx_wetness_multiplier,
          Fvector3().set( 0.1f, 0.1f, 0.0f ),
          Fvector3().set( 20.0f, 20.0f, 0.0f ) );
    ...
    // CMD4(CCC_Float, "scope_drift_amount", &scope_drift_amount, -999.f,
    // 999.f);
