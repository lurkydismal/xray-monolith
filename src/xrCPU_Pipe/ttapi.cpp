...
DWORD ttapi_Init( _processor_info* ID ) {
    ...
    // 1. Use standard library to detect cores (More reliable than legacy cpuid
    // structs)
    ttapi_workers_count = std::thread::hardware_concurrency();

    // Fallback if detection fails
    if ( ttapi_workers_count == 0 )
        ttapi_workers_count = 4;
    ...
    ttapi_dwSlowIter =
        0; // Modern schedulers prefer immediate yield over slow-spinning

    // 4. Allocate Control Structures
    // Helper threads count
    ttapi_threads_count = ttapi_workers_count - 1;

    if ( ( ttapi_threads_handles = ( LPHANDLE )malloc(
               sizeof( HANDLE ) * ttapi_threads_count ) ) == NULL )
        return 0;
    if ( ( ttapi_worker_params = ( PTTAPI_WORKER_PARAMS )malloc(
               sizeof( TTAPI_WORKER_PARAMS ) * ttapi_workers_count ) ) == NULL )
        return 0;

    // Clear params
    memset( ttapi_worker_params, 0,
    ...
    for ( DWORD i = 0; i < ttapi_threads_count; i++ ) {
        ...
        // Create the thread using standard WinAPI (compatible with existing
        // handles array)
        ttapi_threads_handles[ i ] =
            CreateThread( NULL, 0, &ttapiThreadProc, &ttapi_worker_params[ i ],
                          0, &dwThreadId );
...
// Assume that caller is smart enough to use ttapi_GetWorkersCount() to get
// number of available slots
VOID ttapi_AddWorker( LPPTTAPI_WORKER_FUNC lpWorkerFunc,
                      LPVOID lpvWorkerFuncParams ) {
    // Assigning parameters
    ttapi_worker_params[ ttapi_assigned_workers ].lpWorkerFunc = lpWorkerFunc;
    ttapi_worker_params[ ttapi_assigned_workers ].lpvWorkerFuncParams =
        lpvWorkerFuncParams;
...
VOID ttapi_Done() {
    ...
    WaitForMultipleObjects( ttapi_threads_count, ttapi_threads_handles, TRUE,
                            INFINITE );
