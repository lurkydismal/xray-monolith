...
public:
    ...
    bool IsCurrentThreadIteratingOnClients() const {
        if ( now_iterating_in_net_players ||
             now_iterating_in_net_players_disconn ) {
    ...
    void ForEachClientDo( ActionFunctor& functor ) {
        ...
        csPlayers.Enter();
        // LogStackTrace(
        //	make_string("-S- Entered to csPlayers [%d]",
    ...
    u32 ForFoundClientsDo( SearchPredicate const& predicate,
    ...
    /*template<typename ActionFunctor>
    void		ForEachDisconnectedClientDo		(ActionFunctor &
functor)
    {
            //Msg("-S- Entering to csPlayers [%d]", GetCurrentThreadId());
            csPlayers.Enter();
            //LogStackTrace(
            //	make_string("-S- Entered to csPlayers [%d]",
GetCurrentThreadId()).c_str()); now_iterating_in_net_players_disconn	=
true; #ifdef DEBUG iterator_thread_id = GetCurrentThreadId(); #endif
            std::for_each(net_Players_disconnected.begin(),
net_Players_disconnected.end(), functor); now_iterating_in_net_players_disconn
=	false;
            //Msg("-S- Leaving from csPlayers [%d]", GetCurrentThreadId());
            csPlayers.Leave();
    }
    template<typename SearchPredicate>
    IClient*	FindAndEraseDisconnectedClient	(SearchPredicate const &
predicate)
    {
            //Msg("-S- Entering to csPlayers [%d]", GetCurrentThreadId());
            csPlayers.Enter();
            //LogStackTrace(
            //	make_string("-S- Entered to csPlayers [%d]",
GetCurrentThreadId()).c_str()); VERIFY(!now_iterating_in_net_players_disconn);
    ...
    template<typename SearchPredicate>
    IClient*	GetFoundDisconnectedClient		(SearchPredicate const &
predicate)
    {
            //Msg("-S- Entering to csPlayers [%d]", GetCurrentThreadId());
            csPlayers.Enter();
...
true; #ifdef DEBUG iterator_thread_id = GetCurrentThreadId(); #endif
    ...
    void		AddNewDisconnectedClient		(IClient*
new_client)
    {
            //Msg("-S- Entering to csPlayers [%d]", GetCurrentThreadId());
            csPlayers.Enter();
