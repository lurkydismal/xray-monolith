...
#define NET_PROTECTED_SERVER_STR \
    "Access denied by protected server for this player!"
...
void ip_address::set( LPCSTR src_string ) {
    u32 buff[ 4 ];
    int cnt = sscanf( src_string, "%d.%d.%d.%d", &buff[ 0 ], &buff[ 1 ],
                      &buff[ 2 ], &buff[ 3 ] );
    if ( cnt == 4 ) {
...
xr_string ip_address::to_string() const {
    string128 res;
    xr_sprintf( res, sizeof( res ), "%d.%d.%d.%d", m_data.a1, m_data.a2,
                m_data.a3, m_data.a4 );
    return res;
}
...
void IBannedClient::Load( CInifile& ini, const shared_str& sect ) {
    ...
    int res_t =
        sscanf( time_to.c_str(), "%02d.%02d.%d_%02d:%02d:%02d",
                &_tm_banned.tm_mday, &_tm_banned.tm_mon, &_tm_banned.tm_year,
                &_tm_banned.tm_hour, &_tm_banned.tm_min, &_tm_banned.tm_sec );
    VERIFY( res_t == 6 );
    ...
    Msg( "- loaded banned client %s to %s", HAddr.to_string().c_str(),
         BannedTimeTo().c_str() );
}

void IBannedClient::Save( CInifile& ini ) {
    ini.w_string( HAddr.to_string().c_str(), "time_to",
                  BannedTimeTo().c_str() );
}
...
xr_string IBannedClient::BannedTimeTo() const {
    ...
    xr_sprintf( res, sizeof( res ), "%02d.%02d.%d_%02d:%02d:%02d",
                _tm_banned->tm_mday, _tm_banned->tm_mon + 1,
                _tm_banned->tm_year + 1900, _tm_banned->tm_hour,
                _tm_banned->tm_min, _tm_banned->tm_sec );
...
void IClient::_SendTo_LL( const void* data, u32 size, u32 flags, u32 timeout ) {
    R_ASSERT( server );
    server->IPureServer::SendTo_LL( ID, const_cast< void* >( data ), size,
                                    flags, timeout );
}
...
IPureServer::EConnect IPureServer::Connect( LPCSTR options,
    ...
    // sertanly we can use game_descr structure for determinig level_name, but
    // for backward compatibility we save next line...
    ...
    if ( strstr( options, "maxplayers=" ) ) {
        ...
        if ( strchr( sMaxPlayers, '/' ) )
            strncpy_s( tmpStr, sMaxPlayers,
                       strchr( sMaxPlayers, '/' ) - sMaxPlayers );
        else
    ...
    if ( strstr( options, "portsv=" ) ) {
        ...
        if ( strchr( ServerPort, '/' ) )
            strncpy_s( tmpStr, ServerPort,
                       strchr( ServerPort, '/' ) - ServerPort );
        else
    ...
    if ( !psNET_direct_connect ) {
       ...
       // Create the IDirectPlay8Client object.
        HRESULT CoCreateInstanceRes = CoCreateInstance(
            XR_GUID( CLSID_DirectPlay8Server ), NULL, CLSCTX_INPROC_SERVER,
...
#else
        CHK_DX(
            NET->Initialize( this, Handler, DPNINITIALIZE_DISABLEPARAMVAL ) );
#endif

        BOOL bSimulator = FALSE;
        if ( Core.ParamsData.test( ECoreParams::netsim ) )
            bSimulator = TRUE;
        ...
        CHK_DX( NET->SetServerInfo( &dpPlayerInfo, NULL, NULL,
                                    DPNSETSERVERINFO_SYNC ) );

        // Set server/session description
        WCHAR SessionNameUNICODE[ 4096 ];
        CHK_DX( MultiByteToWideChar( CP_ACP, 0, session_name, -1,
                                     SessionNameUNICODE, 4096 ) );
        // Set server/session description
        ...
        dpAppDesc.dwMaxPlayers =
            ( m_bDedicated ) ? ( dwMaxPlayers + 2 ) : ( dwMaxPlayers + 1 );
        ...
        if ( xr_strlen( password_str ) ) {
            CHK_DX( MultiByteToWideChar( CP_ACP, 0, password_str, -1,
                                         SessionPasswordUNICODE, 4096 ) );
            dpAppDesc.dwFlags |= DPNSESSION_REQUIREPASSWORD;
            dpAppDesc.pwszPassword = SessionPasswordUNICODE;
        };

        // Create our IDirectPlay8Address Device Address, --- Set the SP for our
        // Device Address
        net_Address_device = NULL;
        CHK_DX( CoCreateInstance( XR_GUID( CLSID_DirectPlay8Address ), NULL,
                                  CLSCTX_INPROC_SERVER,
                                  XR_GUID( IID_IDirectPlay8Address ),
                                  ( LPVOID* )&net_Address_device ) );
        CHK_DX( net_Address_device->SetSP(
            bSimulator ? &CLSID_NETWORKSIMULATOR_DP8SP_TCPIP
                       : &XR_GUID( CLSID_DP8SP_TCPIP ) ) );

        DWORD dwTraversalMode = DPNA_TRAVERSALMODE_NONE;
        CHK_DX( net_Address_device->AddComponent(
            DPNA_KEY_TRAVERSALMODE, &dwTraversalMode, sizeof( dwTraversalMode ),
            DPNA_DATATYPE_DWORD ) );
        ...
        while ( HostSuccess != S_OK ) {
            CHK_DX( net_Address_device->AddComponent(
                DPNA_KEY_PORT, &psNET_Port, sizeof( psNET_Port ),
                DPNA_DATATYPE_DWORD ) );
            ...
            if ( HostSuccess != S_OK ) {
                //			xr_string res =
                // Debug.error2string(HostSuccess);
                if ( bPortWasSet ) {
...
HRESULT IPureServer::net_Handler( u32 dwMessageType, PVOID pMessage ) {
    ...
    switch ( dwMessageType ) {
        ...
        case DPN_MSGID_CREATE_PLAYER: {
            ...
            HRESULT _hr =
                NET->GetClientInfo( msg->dpnidPlayer, Pinfo, &bufferSize, 0 );
            ...
            // string64			cname;
            // CHK_DX( WideCharToMultiByte( CP_ACP, 0, Pinfo->pwszName, -1,
            // cname, sizeof(cname) , 0, 0 ) );
        ...
        case DPN_MSGID_DESTROY_PLAYER: {
            ...
            IClient* tmp_client =
                net_players.GetFoundClient( ClientIdSearchPredicate(
                    static_cast< ClientID >( msg->dpnidPlayer ) ) );
            if ( tmp_client ) {
        ...
        case DPN_MSGID_RECEIVE: {
            ...
            if ( ( m_size > 2 * sizeof( u32 ) ) &&
                ...
                if ( m_size == sizeof( MSYS_PING ) ) {
                    ...
                    IPureServer::SendTo_Buf(
                        ID, m_data, m_size,
                        net_flags( FALSE, FALSE, TRUE, TRUE ) );
                }
            } else {
                MultipacketReciever::RecievePacket(
                    pMsg->pReceiveData, pMsg->dwReceiveDataSize, m_sender );
            }
        } break;
        ...
        case DPN_MSGID_INDICATE_CONNECT: {
            ...
            // first connected client is SV_Client so if it is NULL then this
            // server client tries to connect ;)
            if ( SV_Client &&
                 !m_ip_filter.is_ip_present( HAddr.m_data.data ) ) {
...
void IPureServer::SendTo_Buf( ClientID id,
                              void* data,
                              u32 size,
                              u32 dwFlags,
                              u32 dwTimeout ) {
    IClient* tmp_client =
        net_players.GetFoundClient( ClientIdSearchPredicate( id ) );
    VERIFY( tmp_client );
    tmp_client->MultipacketSender::SendPacket( data, size, dwFlags, dwTimeout );
}

void IPureServer::SendTo_LL( ClientID ID /*DPNID ID*/,
                             void* data,
                             u32 size,
                             u32 dwFlags,
                             u32 dwTimeout ) {
    ...
    if ( time_global - stats.dwSendTime >= 999 ) {
        stats.dwBytesPerSec =
            ( stats.dwBytesPerSec * 9 + stats.dwBytesSended ) / 10;
    ...
    DPNHANDLE hAsync = 0;
    HRESULT _hr = NET->SendTo( ID.value(), &desc, 1, dwTimeout, 0, &hAsync,
...
void IPureServer::SendTo( ClientID ID /*DPNID ID*/,
                          NET_Packet& P,
                          u32 dwFlags,
                          u32 dwTimeout ) {
...
void IPureServer::SendBroadcast_LL( ClientID exclude,
                                    void* data,
                                    u32 size,
                                    u32 dwFlags ) {
    ...
    struct ClientSenderFunctor {
        ...
        ClientSenderFunctor( IPureServer* owner,
                             void* data,
                             u32 size,
                             u32 dwFlags )
            : m_owner( owner ),
...
void IPureServer::SendBroadcast( ClientID exclude,
                                 NET_Packet& P,
                                 u32 dwFlags ) {
...
BOOL IPureServer::HasBandwidth( IClient* C ) {
    ...
    HRESULT hr;
    if ( psNET_ServerUpdate != 0 &&
         ( dwTime - C->dwTime_LastUpdate ) > dwInterval ) {
...
/*bool			IPureServer::DisconnectClient	(IClient* C)
{
        ...
        HRESULT res = NET->DestroyClient(C->ID.value(), Reason,
xr_strlen(Reason)+1, 0); CHK_DX(res); return true;
}*/
...
bool IPureServer::DisconnectClient( IClient* C, LPCSTR Reason ) {
    ...
    HRESULT res =
        NET->DestroyClient( C->ID.value(), Reason, xr_strlen( Reason ) + 1, 0 );
    CHK_DX( res );
    return true;
}
...
bool IPureServer::DisconnectAddress( const ip_address& Address,
    ...
    struct ToDisconnectFillerFunctor {
        ...
        ToDisconnectFillerFunctor( IPureServer* owner,
                                   buffer_vector< IClient* >* dest_disconnect,
...
bool IPureServer::GetClientAddress( IDirectPlay8Address* pClientAddress,
                                    ip_address& Address,
                                    DWORD* pPort ) {
    WCHAR wstrHostname[ 256 ] = { 0 };
    DWORD dwSize = sizeof( wstrHostname );
    DWORD dwDataType = 0;
    CHK_DX( pClientAddress->GetComponentByName( DPNA_KEY_HOSTNAME, wstrHostname,
                                                &dwSize, &dwDataType ) );

    string256 HostName;
    CHK_DX( WideCharToMultiByte( CP_ACP, 0, wstrHostname, -1, HostName,
                                 sizeof( HostName ), 0, 0 ) );
    ...
    if ( pPort != NULL ) {
        ...
        CHK_DX( pClientAddress->GetComponentByName(
            DPNA_KEY_PORT, &dwPort, &dwPortSize, &dwPortDataType ) );
        *pPort = dwPort;
    };
...
bool IPureServer::GetClientAddress( ClientID ID,
                                    ip_address& Address,
                                    DWORD* pPort ) {
...
void IPureServer::UnBanAddress( const ip_address& Address ) {
    if ( !GetBannedClient( Address ) ) {
        Msg( "! Can't find address %s in ban list.",
...
void IPureServer::Print_Banned_Addreses() {
    ...
    for ( u32 i = 0; i < BannedAddresses.size(); i++ ) {
        ...
        Msg( "- %s to %s", pBClient->HAddr.to_string().c_str(),
             pBClient->BannedTimeTo().c_str() );
    }
