    ...
    bool operator==( const ip_address& other ) const {
        return ( m_data.data == other.m_data.data ) ||
               ( ( m_data.a1 == other.m_data.a1 ) &&
                 ...
                 ( m_data.a3 == other.m_data.a3 ) && ( m_data.a4 == 0 ) );
    }
};
...
private:
    virtual void _SendTo_LL( const void* data,
                             u32 size,
                             u32 flags,
                             u32 timeout );
};
...
protected:
    ...
    bool GetClientAddress( IDirectPlay8Address* pClientAddress,
                           ip_address& Address,
                           DWORD* pPort = NULL );
...
public:
    ...
    virtual void SendTo_LL( ClientID ID,
                            ...
                            u32 dwTimeout = 0 );
    virtual void SendTo_Buf( ClientID ID,
                             void* data,
                             u32 size,
                             u32 dwFlags = DPNSEND_GUARANTEED,
                             u32 dwTimeout = 0 );
    ...
    void SendTo( ClientID ID,
                 ...
                 u32 dwFlags = DPNSEND_GUARANTEED,
                 u32 dwTimeout = 0 );
    void SendBroadcast_LL( ClientID exclude,
                           void* data,
                           u32 size,
                           u32 dwFlags = DPNSEND_GUARANTEED );
    ...
    // IC u32					client_Count		()
    // { return net_Players.size(); } IC IClient*
    // client_Get			(u32 num)	{ return
    // net_Players[num]; }

    // IC u32					disconnected_client_Count
    // ()			{ return net_Players_disconnected.size(); } IC
    // IClient*				disconnected_client_Get
    ...
    // template<typename ActionFunctor>
    // void
    // ForEachDisconnectedClientDo(ActionFunctor & action) {
    // net_players.ForEachDisconnectedClientDo(action); };
#ifdef DEBUG
    bool IsPlayersMonitorLockedByMe() const {
        return net_players.IsCurrentThreadIteratingOnClients() &&
               !sender_functor_invoked;
    };
#endif
