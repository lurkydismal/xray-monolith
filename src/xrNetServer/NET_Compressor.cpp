...
#define PPM_TOP_VALUE \
    ( ( NET_Compressor::code_value )1 << ( PPM_CODE_BITS - 1 ) )
...
void NET_Compressor::start_encoding		( BYTE* dest, u32 header_size )
{
        dest			+=	header_size-1;
        RNGC.low		=	0;				// Full
code range RNGC.range		=	PPM_TOP_VALUE; RNGC.buffer =	0;
    RNGC.help		=	0;				// No bytes to
follow RNGC.bytecount	=	0; RNGC.ptr		=	dest;
}
...
{
        while( RNGC.range <= PPM_BOTTOM_VALUE )     // do we need
renormalisation?
    {
                if( RNGC.low < code_value(0xff) << SHIFT_BITS )  // no carry
possible --> output
        {
            ...
            RNGC.buffer	= (BYTE)(RNGC.low >> SHIFT_BITS);
                ...
                else if( RNGC.low & PPM_TOP_VALUE ) // carry now, no future
carry
        {
...
u32 NET_Compressor::done_encoding	( )
{
    ...
    RNGC.bytecount		+= 3;
        ...
        u32 tmp = ((RNGC.low & (PPM_BOTTOM_VALUE-1)) <
((RNGC.bytecount&0xffffffL)>>1)) ? (RNGC.low >> SHIFT_BITS) : (RNGC.low >>
SHIFT_BITS) + 1;
...
void NET_Compressor::decode_normalize	( )
{
    ...
    {
                RNGC.low    =   (RNGC.low<<8) |
((RNGC.buffer<<EXTRA_BITS)&0xff); RNGC.buffer =   RNGC.byte_in(); RNGC.low    |=
RNGC.buffer >> (8-EXTRA_BITS); RNGC.range	<<= 8;
    }
}
...
// lt_f is the lower end (frequency sum of < symbols)
// tot_f is the total interval length (total frequency sum)
void NET_Compressor::decode_update		( freq sy_f, freq lt_f, freq
tot_f )
{
...
u16 NET_Compressor::Compress( BYTE* dest,
                              const u32& dest_size,
                              BYTE* src,
                              const u32& count ) {
    ...
    if ( !psNET_direct_connect && g_net_compressor_enabled &&
         b_compress_packet ) {
        CS.Enter();
        compressed_size =
            offset + ENCODE( dest + offset, dest_size - offset, src, count );
    ...
    if ( compressed_size < count ) {
        ...
        *( ( u32* )( dest + 1 ) ) = crc;
        ...
        Msg( "#compress %u->%u  %02X (%08X)", count, compressed_size, *dest,
             *( ( u32* )( src + 1 ) ) );
#endif
        ...
        fprintf( CompressionDump, "%s compress %2.0f%% %u->%u\r\n",
                 compressor_name,
                 100.0f * float( compressed_size ) / float( count ), count,
                 compressed_size );
...
u16 NET_Compressor::Decompress( BYTE* dest,
                                const u32& dest_size,
                                BYTE* src,
                                const u32& count ) {
    VERIFY( dest );
...
#if NET_LOG_COMPRESSION
    Msg( "#decompress %u  %02X (%08X)", count, src[ 0 ],
         *( ( u32* )( src + 1 ) ) );
#endif
    ...
    R_ASSERT2( crc == *( ( u32* )( src + 1 ) ),
               make_string( "crc is different! (0x%08x != 0x%08x)", crc,
                            *( ( u32* )( src + 1 ) ) ) );
#endif // NET_USE_COMPRESSION_CRC

    CS.Enter();
    u32 uncompressed_size =
        DECODE( dest, dest_size, src + offset, count - offset );
    CS.Leave();
...
void NET_Compressor::DumpStats( bool brief ) {
    xr_map< u32, SCompressorStats::SStatPacket >::const_iterator it =
        m_stats.m_packets.begin();
    xr_map< u32, SCompressorStats::SStatPacket >::const_iterator it_e =
        m_stats.m_packets.end();
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( !brief ) {
            Msg( "size[%d] count[%d] unlucky[%d] avg_c[%d]", it->first,
                 it->second.hit_count, it->second.unlucky_attempts,
                 iFloor( float( it->second.compressed_size ) /
                         float( it->second.hit_count ) ) );
        }
    }
