...
/*#ifdef DEBUG
void PrintParsedPacket(const char* message, u16 message_type, const void*
packet_data, u32 packet_size)
{
        ...
        if (msg_type == message_type)
        {
                if (message_type == 1) //M_SPAWN
                {
                        ...
                        u8
temp_gt; tNetPacket.r_u8				(temp_gt
); u8							s_RP; tNetPacket.r_u8
(s_RP			); Fvector
o_Position, o_Angle; tNetPacket.r_vec3			(o_Position
); tNetPacket.r_vec3			(o_Angle		); u16
RespawnTime, ID; tNetPacket.r_u16			(RespawnTime	);
                        tNetPacket.r_u16			(ID
);

                        Msg("%s M_SPAWN for [%s]-[%d]", message, s_name.c_str(),
ID); } else
                {
                        Msg("%s NOT_IMPLEMENTED_PRINT type[%d]", message,
message_type);
                }
        }
}
...
void MultipacketSender::SendPacket( const void* packet_data,
                                    u32 packet_sz,
                                    u32 flags,
                                    u32 timeout ) {
    _buf_cs.Enter();
    ...
    if ( ( buf->buffer.B.count + packet_sz + sizeof( u16 ) >=
           NET_PacketSizeLimit ) ||
...
void MultipacketSender::_FlushSendBuffer( u32 timeout, Buffer* buf ) {
    ...
    if ( buf->buffer.B.count ) {
        ...
        if ( Core.ParamsData.test( ECoreParams::dump_traffic ) ) {
            static bool first_time = true;
            FILE* dump =
                fopen( "raw-out-traffic.bins", ( first_time ) ? "wb" : "ab" );
        ...
        _SendTo_LL( packet_data, comp_sz + sizeof( MultipacketHeader ),
                    buf->last_flags, timeout );
...
void MultipacketReciever::RecievePacket( const void* packet_data,
                                         u32 packet_sz,
                                         u32 param ) {
    MultipacketHeader* header = ( MultipacketHeader* )packet_data;
    ...
    Compressor.Decompress( data, sizeof( data ),
                           ( u8* )packet_data + sizeof( MultipacketHeader ),
    ...
    if ( Core.ParamsData.test( ECoreParams::dump_traffic ) ) {
        static bool first_time = true;
        FILE* dump =
            fopen( "raw-in-traffic.bins", ( first_time ) ? "wb" : "ab" );
    ...
    while ( processed_sz < header->unpacked_size ) {
        u32 size = ( is_multi_packet ) ? u32( *( ( u16* )dat ) )
