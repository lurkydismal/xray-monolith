...
const GUID XR_GUID( CLSID_DirectPlay8Client ) = {
    ...
    0x5aba,
    0x429f,
    { 0x8b, 0xdf, 0xc5, 0x4d, 0x03, 0x25, 0x3d, 0xc2 } };
...
const GUID XR_GUID( CLSID_DirectPlay8Server ) = {
    ...
    0x6830,
    0x43d7,
    { 0x83, 0x5d, 0x0b, 0x5a, 0xd8, 0x29, 0x56, 0xa2 } };
...
const GUID XR_GUID( CLSID_DirectPlay8Peer ) = {
    ...
    0x375e,
    0x4458,
    { 0xa2, 0x72, 0xb1, 0x38, 0xe2, 0xf8, 0x0a, 0x6a } };
...
const GUID XR_GUID( CLSID_DirectPlay8NATResolver ) = {
    ...
    0xcbf7,
    0x48bd,
    { 0x9a, 0x69, 0x34, 0xa5, 0x5e, 0x0d, 0x89, 0x41 } };
...
const GUID XR_GUID( IID_IDirectPlay8Client ) = {
    ...
    0x241b,
    0x11d3,
    { 0xae, 0xa7, 0x00, 0x60, 0x97, 0xb0, 0x14, 0x11 } };
...
const GUID XR_GUID( IID_IDirectPlay8ThreadPool ) = {
    ...
    0x4a46,
    0x4a0d,
    { 0x89, 0xb2, 0x04, 0x5b, 0x4d, 0x66, 0x64, 0x25 } };
...
const GUID XR_GUID( CLSID_DP8SP_IPX ) = {
    ...
    0x628d,
    0x11d2,
    { 0xae, 0x0f, 0x00, 0x60, 0x97, 0xb0, 0x14, 0x11 } };
...
const GUID XR_GUID( CLSID_DP8SP_BLUETOOTH ) = {
    ...
    0x3027,
    0x4b9a,
    { 0x95, 0x6e, 0xc7, 0x72, 0xb3, 0xf7, 0x80, 0x06 } };

const GUID XR_GUID( CLSID_DirectPlay8Address ) = {
    0x934a9523,
    0xa3ca,
    0x4bc5,
    { 0xad, 0xa0, 0xd6, 0xd9, 0x5d, 0x97, 0x94, 0x21 } };

const GUID XR_GUID( IID_IDirectPlay8Address ) = {
    0x83783300,
    0x4063,
    0x4c8a,
    { 0x9d, 0xb3, 0x82, 0x83, 0x0a, 0x7f, 0xeb, 0x31 } };
...
NET_Packet* INetQueue::Create() {
    ...
    // cs.Enter		();
    // #ifdef _DEBUG
    //		Msg ("- INetQueue::Create - ready %d, unused %d", ready.size(),
    // unused.size()); #endif
    if ( unused.empty() ) {
...
NET_Packet* INetQueue::Retreive() {
    ...
    // cs.Enter		();
    // #ifdef _DEBUG
    //			Msg ("INetQueue::Retreive - ready %d, unused %d",
    // ready.size(), unused.size()); #endif
    if ( !ready.empty() )
        P = ready.front();
    //---------------------------------------------
    else {
...
void INetQueue::Release() {
    // cs.Enter		();
    // #ifdef _DEBUG
    //		Msg ("INetQueue::Release - ready %d, unused %d", ready.size(),
    // unused.size()); #endif
    VERIFY( !ready.empty() );
    //---------------------------------------------
    u32 tmp_time = GetTickCount() - 60000;
...
void IPureClient::_Recieve( const void* data, u32 data_size, u32 /*param*/ ) {
    ...
    if ( ( data_size >= 2 * sizeof( u32 ) ) && ( cfg->sign1 == 0x12071980 ) &&
        ...
        if ( psNET_Flags.test( NETFLAG_LOG_CL_PACKETS ) ) {
            ...
            if ( pClNetLog )
                pClNetLog->LogData( timeServer(), const_cast< void* >( data ),
                                    data_size, TRUE );
        }
...
BOOL IPureClient::Connect( LPCSTR options ) {
    ...
    if ( !psNET_direct_connect ) {
        ...
        HRESULT CoCreateInstanceRes = CoCreateInstance(
            XR_GUID( CLSID_DirectPlay8Client ), NULL, CLSCTX_INPROC_SERVER,
        ...
        // Device Address
        net_Address_device = NULL;
        R_CHK( CoCreateInstance( XR_GUID( CLSID_DirectPlay8Address ), NULL,
                                 CLSCTX_INPROC_SERVER,
                                 XR_GUID( IID_IDirectPlay8Address ),
                                 ( LPVOID* )&net_Address_device ) );
        R_CHK( net_Address_device->SetSP(
            bSimulator ? &CLSID_NETWORKSIMULATOR_DP8SP_TCPIP
                       : &XR_GUID( CLSID_DP8SP_TCPIP ) ) );

        // Create our IDirectPlay8Address Server Address, --- Set the SP for our
        // Server Address
        WCHAR ServerNameUNICODE[ 256 ];
        R_CHK( MultiByteToWideChar( CP_ACP, 0, server_name, -1,
                                    ServerNameUNICODE, 256 ) );
        ...
        R_CHK( net_Address_server->SetSP(
            bSimulator ? &CLSID_NETWORKSIMULATOR_DP8SP_TCPIP
                       : &XR_GUID( CLSID_DP8SP_TCPIP ) ) );
        R_CHK( net_Address_server->AddComponent(
            DPNA_KEY_HOSTNAME, ServerNameUNICODE,
            2 * u32( wcslen( ServerNameUNICODE ) + 1 ),
            DPNA_DATATYPE_STRING ) );
        R_CHK( net_Address_server->AddComponent( DPNA_KEY_PORT, &psSV_Port,
                                                 sizeof( psSV_Port ),
                                                 DPNA_DATATYPE_DWORD ) );
        ...
        R_CHK( MultiByteToWideChar( CP_ACP, 0, user_name_str, -1,
                                    ClientNameUNICODE, 256 ) );
        ...
        if ( stricmp( server_name, "localhost" ) == 0 ) {
            ...
            HRESULT res = S_FALSE;
            while ( res != S_OK ) {
                R_CHK( net_Address_device->AddComponent(
                    DPNA_KEY_PORT, &c_port, sizeof( c_port ),
                    DPNA_DATATYPE_DWORD ) );
            ...
            R_CHK(
                NET->GetApplicationDesc( dpServerDesc, &dpServerDescSize, 0 ) );
            if ( !dpServerDesc->dwApplicationReservedDataSize ||
                 !dpServerDesc->pvApplicationReservedData ) {
                OnInvalidHost();
                return FALSE;
            }
            CopyMemory( &m_game_description,
                        dpServerDesc->pvApplicationReservedData,
            ...
            if ( dpServerDesc->pwszSessionName ) {
                ...
                R_CHK( WideCharToMultiByte(
                    CP_ACP, 0, dpServerDesc->pwszSessionName, -1, dpSessionName,
...
HRESULT IPureClient::net_Handler( u32 dwMessageType, PVOID pMessage ) {
    ...
    switch ( dwMessageType ) {
        case DPN_MSGID_ENUM_HOSTS_RESPONSE: {
            ...
            if ( pDesc->dwApplicationReservedDataSize &&
                 pDesc->pvApplicationReservedData ) {
                R_ASSERT( pDesc->dwApplicationReservedDataSize ==
                          sizeof( m_game_description ) );
                CopyMemory( &m_game_description,
                            pDesc->pvApplicationReservedData,
            ...
            if ( !bHostRegistered ) {
                ...
                // Copy the Host Address
                R_CHK( pEnumHostsResponseMsg->pAddressSender->Duplicate(
                    &NODE.pHostAddress ) );
                CopyMemory( &NODE.dpAppDesc, pDesc,
                ...
                if ( pDesc->pwszSessionName ) {
                    string4096 dpSessionName;
                    R_CHK( WideCharToMultiByte(
                        CP_ACP, 0, pDesc->pwszSessionName, -1, dpSessionName,
                        sizeof( dpSessionName ), 0, 0 ) );
                    NODE.dpSessionName = ( char* )( &dpSessionName[ 0 ] );
                }
        ...
        case DPN_MSGID_RECEIVE: {
            ...
            MultipacketReciever::RecievePacket( pMsg->pReceiveData,
                                                pMsg->dwReceiveDataSize );
        } break;
        case DPN_MSGID_TERMINATE_SESSION: {
            PDPNMSG_TERMINATE_SESSION pMsg =
                ( PDPNMSG_TERMINATE_SESSION )pMessage;
            ...
            if ( m_size != 0 ) {
                ...
                Msg( "- Session terminated : %s",
                     ( ::Debug.error2string( m_hResultCode ) ) );
#endif
            }
        }; break;
        default: {
            ...
            switch ( dwMessageType ) {
                ...
                case DPN_MSGID_CONNECT_COMPLETE: {
                    PDPNMSG_CONNECT_COMPLETE pMsg =
                        ( PDPNMSG_CONNECT_COMPLETE )pMessage;
#ifdef DEBUG
                    //					const char* x =
                    ...
                    if ( pMsg->dwApplicationReplyDataSize ) {
                        string256 ResStr = "";
                        strncpy_s( ResStr,
                                   ( char* )( pMsg->pvApplicationReplyData ),
...
void IPureClient::OnMessage( void* data, u32 size ) {
    ...
    P->timeReceive = timeServer_Async(); // TimerAsync
                                         // (device_timer);
...
void IPureClient::SendTo_LL( void* data,
                             u32 size,
                             u32 dwFlags,
                             u32 dwTimeout ) {
    if ( net_Disconnected )
    ...
    DPNHANDLE hAsync = 0;
    HRESULT hr = NET->Send( &desc, 1, dwTimeout, 0, &hAsync,
                            dwFlags | DPNSEND_COALESCE );

    //	Msg("- Client::SendTo_LL [%d]", size);
    if ( FAILED( hr ) ) {
        Msg( "! ERROR: Failed to send net-packet, reason: %s",
             ::Debug.error2string( hr ) );
...
BOOL IPureClient::net_HasBandwidth() {
    ...
    if ( psNET_direct_connect ) {
        if ( 0 != psNET_ClientUpdate &&
             ( dwTime - net_Time_LastUpdate ) > dwInterval ) {
            net_Time_LastUpdate = dwTime;
            return TRUE;
        } else
            return FALSE;
    } else if ( 0 != psNET_ClientUpdate &&
                ( dwTime - net_Time_LastUpdate ) > dwInterval ) {
        HRESULT hr;
        R_ASSERT( NET );
...
void IPureClient::Sync_Thread() {
    ...
    for ( ; NET && !net_Disconnected; ) {
        ...
        __try {
            ...
            if ( FAILED( NET->Send( &desc, 1, 0, 0, &hAsync,
                                    net_flags( FALSE, FALSE, TRUE ) ) ) ) {
                Msg( "* CLIENT: SyncThread: EXIT. (failed to send - "
                     "disconnected?)" );
                break;
            }
...
void IPureClient::Sync_Average() {
    ...
    //	Msg("* CLIENT: d(%d), dc(%d),
    // s(%d)",net_TimeDelta,net_TimeDelta_Calculated,size);
}
...
bool IPureClient::GetServerAddress( ip_address& pAddress, DWORD* pPort ) {
    *pPort = 0;
    ...
    CHK_DX( net_Address_server->GetComponentByName(
        DPNA_KEY_HOSTNAME, wstrHostname, &dwHostNameSize,
        &dwHostNameDataType ) );
    ...
    //.	pAddress[0]				= (char)(*(struct in_addr
    //*)*pHostEnt->h_addr_list).s_net; .	pAddress[1]
    //= (char)(*(struct in_addr *)*pHostEnt->h_addr_list).s_host; .
    // pAddress[2]				= (char)(*(struct in_addr
    //*)*pHostEnt->h_addr_list).s_lh; .	pAddress[3]
    //= (char)(*(struct in_addr *)*pHostEnt->h_addr_list).s_impno;
