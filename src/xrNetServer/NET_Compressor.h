#if !defined( \
    AFX_NET_COMPRESSOR_H__21E1ED1C_BF92_4BF0_94A8_18A27486EBFD__INCLUDED_ )
...
class XRNETSERVER_API NET_Compressor {
    ...
    struct SCompressorStats {
        ...
        struct SStatPacket {
            ...
            SStatPacket()
                : hit_count( 0 ), unlucky_attempts( 0 ), compressed_size( 0 ) {}
        };
...
public:
    ...
    u16 Compress( BYTE* dest,
                  const u32& dest_size,
                  BYTE* src,
                  const u32& count ); // return size of compressed
    u16 Decompress( BYTE* dest,
                    const u32& dest_size,
                    BYTE* src,
                    const u32& count ); // return size of compressed
