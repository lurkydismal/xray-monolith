...
struct subnet_comparator {
    bool operator()( subnet_item const* left, subnet_item const* right ) const {
        return ( ( left->subnet_ip.data & left->subnet_mask ) <
                 ( right->subnet_ip.data & right->subnet_mask ) );
    }
};
...
struct ip_searcher {
    bool operator()( subnet_item const* left, subnet_item const* right ) const {
        if ( left->subnet_mask ) {
            return ( ( left->subnet_ip.data & left->subnet_mask ) <
                     ( right->subnet_ip.data & left->subnet_mask ) );
        }
        return ( ( left->subnet_ip.data & right->subnet_mask ) <
                 ( right->subnet_ip.data & right->subnet_mask ) );
    }
};
...
u32 ip_filter::load() {
    ...
    for ( u32 i = 0, line_count = ini.line_count( SUBNET_LIST_SECT_NAME );
        ...
        unsigned int parsed_params = sscanf_s(
            address, "%u.%u.%u.%u/%u", &parse_data[ 0 ], &parse_data[ 1 ],
            &parse_data[ 2 ], &parse_data[ 3 ], &parse_data[ 4 ] );
        if ( ( parsed_params != 5 ) || ( parse_data[ 0 ] > 255 ) ||
             ( parse_data[ 1 ] > 255 ) || ( parse_data[ 2 ] > 255 ) ||
             ( parse_data[ 3 ] > 255 ) || ( parse_data[ 4 ] == 0 ) ) {
            VERIFY2(
                0, make_string( "! ERROR: bad subnet: %s", address ).c_str() );
            xr_delete( tmp_item );
            continue;
        }

        tmp_item->subnet_ip.data = parse_data[ 3 ] | ( parse_data[ 2 ] << 8 ) |
                                   ( parse_data[ 1 ] << 16 ) |
                                   ( parse_data[ 0 ] << 24 );
    ...
    std::sort( m_all_subnets.begin(), m_all_subnets.end(),
               subnet_comparator() );
...
bool ip_filter::is_ip_present( u32 ip_address ) {
    ...
    return std::binary_search( m_all_subnets.begin(), m_all_subnets.end(),
                               &tmp_fake_item, ip_searcher() );
}

void ip_filter::unload() {
    for ( subnets_coll_t::iterator
              i = m_all_subnets.begin(), // delete_data(m_all_subnets);
          ie = m_all_subnets.end();
          i != ie; ++i ) {
        xr_delete( *i );
    }
