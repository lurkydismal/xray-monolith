...
{
    return _CPU_FEATURE_MMX | _CPU_FEATURE_SSE | _CPU_FEATURE_SSE2 | _CPU_FEATURE_3DNOW |
        _CPU_FEATURE_SSE3 | _CPU_FEATURE_SSSE3 | _CPU_FEATURE_SSE4_1 | _CPU_FEATURE_SSE4_2 |
        _CPU_FEATURE_MWAIT | _CPU_FEATURE_HTT;
}
}
...
TEST(CpuidFeatureConstants, PublicStringBuffersReserveSpaceForNullTerminators)
{
...
TEST(Cpuid, PopulatesProcessorInfoUsingHostCpuData)
{
    ...
    EXPECT_EQ(processorInfo.v_name[sizeof(processorInfo.v_name) - 1], '\0');
    EXPECT_EQ(processorInfo.model_name[sizeof(processorInfo.model_name) - 1], '\0');
    EXPECT_LE(std::strlen(processorInfo.v_name), sizeof(processorInfo.v_name) - 1);
    EXPECT_LE(std::strlen(processorInfo.model_name), sizeof(processorInfo.model_name) - 1);
