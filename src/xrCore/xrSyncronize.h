...
#define MUTEX_PROFILE_ID( a ) \
    STRINGIZER( CONCATENIZE( MUTEX_PROFILE_PREFIX_ID, a ) )
...
// Non-blocking variant: tries to acquire the lock and never blocks. If another
// thread already holds it, owns_lock() is false and the section is left
// unguarded. Use where a blocking acquire could deadlock (e.g. taking a second
// object's lock while holding one) and proceeding without the lock is
// acceptable.
