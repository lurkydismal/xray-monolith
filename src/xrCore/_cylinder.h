...
public:
    ...
    IC int intersect( const _vector3< T >& start,
        ...
        _vector3< T > kD;
        kD.set( kU.dotproduct( dir ), kV.dotproduct( dir ),
                kW.dotproduct( dir ) );
#ifdef DEBUG
        ...
        _vector3< T > kP;
        kP.set( kU.dotproduct( kDiff ), kV.dotproduct( kDiff ),
                kW.dotproduct( kDiff ) );
    ...
    IC ERP_Result intersect( const _vector3< T >& start,
                             const _vector3< T >& dir,
                             T& dist ) const {
        T afT[ 2 ];
        ecode code[ 2 ];
        ...
        if ( 0 != ( cnt = intersect( start, dir, afT, code ) ) ) {
            ...
            return b_result ? ( o_inside ? rpOriginInside : rpOriginOutside )
                            : rpNone;
        } else {
            return rpNone;
        }
...
BOOL _valid( const _cylinder< T >& c ) {
    return _valid( c.m_center ) && _valid( c.m_direction ) &&
           _valid( c.m_height ) && _valid( c.m_radius );
}
