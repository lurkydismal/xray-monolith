...
void CInifile::insert_item( Sect* tgt, Item& I ) {
    // demonized
    // DLTX: add or remove item from the section parameter if it has a structure
    // of "name = item1, item2, item3, ..." >name = item will add item to the
    // list <name = item will remove item from the list
    if ( *I.first ) {
        ModifyListType op =
            static_cast< ModifyListType >( I.first.c_str()[ 0 ] );
        if ( op == ModifyListType::Insert || op == ModifyListType::Remove ) {
            // push_back then back because vector must be created first if it
            // doesn't exist via []
            OverrideModifyListData[ tgt->Name ].push_back( I );
            OverrideModifyListData[ tgt->Name ].back().insertionIndex =
                OverrideModifyListData[ tgt->Name ].size();
            return;
        }
    }
...
static const std::regex& GetCachedRegex( const xr_string& pattern ) {
    ...
    if ( it == g_RegexCache.end() ) {
        auto result =
            g_RegexCache.emplace( pattern, std::regex( pattern.c_str() ) );
...
static void MergeParentSet( RStringVec* ParentsBase,
                            RStringVec* ParentsOverride,
                            bool bIncludeRemovers ) {
    ...
    for ( const auto& CurrentParentStr : *ParentsOverride ) {
        ...
        auto new_end = std::remove_if(
            ParentsBase->begin(), ParentsBase->end(),
...
void CInifile::loadFile( const string_path _fn,
...
#endif
    {
        ...
        LTXLoad( I, inc_path, false, currentFileName, depth
#ifndef _EDITOR
                 ,
                 allow_include_func
#endif
...
void CInifile::StashCurrentSection( Sect*& CurrentBase,
    ...
    if ( CurrentBase ) {
        ...
        if ( SectIt != BaseData.end() &&
             SectIt->first.equal( CurrentBase->Name ) ) {
            Debug.fatal( DEBUG_INFO,
                         "[DLTX] Duplicate section '%s' wasn't marked as an "
                         "override.\n\nOverride section by prefixing it with "
                         "'!' (![%s]) or give it a unique name.\n\nCheck this "
                         "file and its DLTX mods:\n\"%s\",\nfile with section "
                         "\"%s\",\nfile with duplicate \"%s\"",
                         *CurrentBase->Name, *CurrentBase->Name, m_file_name,
                         SectionToFilename[ CurrentBase->Name ].c_str(),
                         currentFileName );
        } else {
    ...
    if ( CurrentOverride ) {
        ...
        if ( SectIt != OverrideData.end() &&
             SectIt->first.equal( CurrentOverride->Name ) ) {
            ...
            OverrideToFilename[ CurrentOverride->Name ].insert(
                currentFileName );
        }
...
void CInifile::SortAndFilterSection( Sect& Data ) {
    ...
    // 1. Sort by Key, then by Depth (Ascending), then by insertionOrder
    // (Descending).
    ...
    // 2. Linear pass to keep the first (lowest depth) of each key group, but
    // the item inserted last at that depth wins
...
void CInifile::LTXLoad( IReader* F,
                        LPCSTR path,
                        BOOL bIsRootFile,
                        string_path currentFileName,
                        int depth
    ...
    static auto InsertParentStringsInMap =
        []( shared_str SectionName,
            xr_unordered_flat_map< shared_str, RStringVec >& ParentMap ) {
    ...
    static auto GetParentsSetFromString = []( const char* ParentString,
                                              MezzStringBuffer& str2 ) {
    ...
    static auto GetRegexMatch = []( const xr_string& InputString,
                                    const xr_string& PatternString ) {
    ...
    static auto IsFullRegexMatch = []( const xr_string& InputString,
                                       const xr_string& PatternString ) {
        const std::regex& Pattern = GetCachedRegex( PatternString );
    ...
    while ( !F->eof() || ( bIsRootFile && !bHasLoadedModFiles ) ) {
        if ( !F->eof() ) {
            ...
            StashCurrentSection( CurrentBase, CurrentOverride,
                                 currentFileName );
            ...
            _splitpath_s( m_file_name, split_drive, split_drive.GetSize(),
                          split_dir, split_dir.GetSize(), split_name,
                          split_name.GetSize(), NULL, 0 );

            xr_string FilePath =
                xr_string( split_drive ) + xr_string( split_dir );
            xr_string FileName = split_name;

            // Collect all files that could potentially be confused as a root
            // file by our mod files
            FS_FileSet AmbiguousFiles;
            FS.file_list( AmbiguousFiles, FilePath.c_str(), FS_ListFiles,
                          ( FileName + "_*.ltx" ).c_str() );

            // Collect all matching mod files
            FS_FileSet ModFiles;
            FS.file_list( ModFiles, FilePath.c_str(), FS_ListFiles,
                          ( "mod_" + FileName + "_*.ltx" ).c_str() );
            ...
            for ( auto It = ModFiles.begin(); It != ModFiles.end(); ++It ) {
                ...
                // Determine if we should load this mod file, or if it's meant
                // for a different root file
                static auto bIsModfileMeantForMeFunc =
                    []( FS_FileSet AmbiguousFiles, xr_string ModFileName ) {
                        for ( auto It2 = AmbiguousFiles.begin();
                              It2 != AmbiguousFiles.end(); ++It2 ) {
                            xr_string name = It2->name.c_str();
                            xr_string AmbiguousFileName = xr_string(
                                GetRegexMatch( name, "^.+(?=.ltx$)" ).c_str() );
                            xr_string AmbiguousFileMatchPattern =
                                xr_string( "mod_" ) + AmbiguousFileName +
                                xr_string( "_.+.ltx" );

                            if ( IsFullRegexMatch(
                                     ModFileName,
                                     AmbiguousFileMatchPattern.c_str() ) ) {
                                return false;
                            }
                        }
                ...
                bool bIsModfileMeantForMe =
                    bIsModfileMeantForMeFunc( AmbiguousFiles, ModFileName );
                ...
                loadFile( ( FilePath + ModFileNameStr ).c_str(),
                          FilePath.c_str(), ModFileName.c_str(),
                          currentFileName, d
#ifndef _EDITOR
                          ,
                          allow_include_func
        ...
        if ( str[ 0 ] && ( str[ 0 ] == '#' ) && strstr( str, "#include" ) ) {
            ...
            if ( _GetItem( str, 1, inc_name, '"' ) ) {
                ...
                if ( strstr( inc_name, "*.ltx" ) ) {
                    ...
                    for ( FS_FileSet::iterator it = fset.begin();
                          it != fset.end(); it++ ) {
                        LPCSTR _name = it->name.c_str();
                        string_path _fn;
                        strconcat( sizeof( _fn ), _fn, inc_path, _name );

                        // Include file, increase depth by 1 from either mod
                        // file or base file
                        loadFile( _fn, inc_path, _name, currentFileName,
                                  depth + 1
#ifndef _EDITOR
                                  ,
                                  allow_include_func
                ...
                // Include file, increase depth by 1 from either mod file or
                // base file
                else
                    loadFile( fn, inc_path, inc_name, currentFileName, depth + 1
#ifndef _EDITOR
                              ,
                              allow_include_func
#endif
            ...
            xr_string SecName =
                xr_string( str )
                    .substr( SectionNameStartPos,
                             strchr( str, ']' ) - str - SectionNameStartPos )
                    .c_str();
            for ( auto i = SecName.begin(); i != SecName.end(); ++i ) {
                *i = tolower( *i );
            }
            Msg( "[DLTX] [%s] Encountered %s, mark section to delete",
                 m_file_name, str.GetBuffer() );
            ...
            if ( 0 != inherited_names ) {
                VERIFY2( m_flags.test( eReadOnly ),
                         "Allow for readonly mode only." );
                inherited_names += 2;

                auto CurrentParents =
                    GetParentsSetFromString( inherited_names, str2 );

                if ( bIsCurrentSectionOverride ) {
                    auto* SectionParents = InsertParentStringsInMap(
                        SecName.c_str(), OverrideParentDataMap );
                    MergeParentSet( SectionParents, &CurrentParents, true );
                } else {
                    auto* SectionParents = InsertParentStringsInMap(
                        SecName.c_str(), BaseParentDataMap );
                    MergeParentSet( SectionParents, &CurrentParents, true );
                }
            }
                ...
                *t = 0;
                ...
                if ( bInsideSTR ) {
                    while ( bInsideSTR ) {
                        ...
                        R_ASSERT2(
                            xr_strlen( value_raw ) + xr_strlen( str_add_raw ) <
                                value_raw.GetSize(),
                            make_string(
                                "Incorrect inifile format: section[%s], "
                                "variable[%s]. Odd number of quotes (\") "
                                "found, but should be even.",
                                ( CurrentBase
                                      ? CurrentBase->Name.c_str()
                                      : ( CurrentOverride
                                              ? CurrentOverride->Name.c_str()
                                              : "unknown" ) ),
                                name ) );
                        xr_strcat( value_raw, value_raw.GetSize(),
                                   str_add_raw );
            ...
            if ( !I.first ) {
                if ( print_dltx_warnings )
                    Msg( "~[DLTX] WARNING: Malformed line %s in file %s, can't "
                         "get key name, skipping, section data might be "
                         "altered unexpectedly",
                         currentLine.c_str(), currentFileName );
                continue;
            }
            I.second = bIsDelete ? DLTX_DELETE.c_str()
                                 : ( str2[ 0 ] ? str2.GetBuffer() : NULL );

            auto fname = toLowerCaseCopy(
                trimCopy( getFilename( std::string( currentFileName ) ) ) );
    ...
    StashCurrentSection( CurrentBase, CurrentOverride, currentFileName );

    // Create empty sections that were marked with @[ and weren't defined
    // normally
...
CInifile::Items CInifile::MergeSections(
    ...
    while ( b_it != BaseItems.end() || o_it != OverrideItems.end() ) {
        ...
        if ( cmp < 0 ) {
            ...
            // Check for DLTX_DELETE token and in Base+Mod merge, Add to
            // DeletedItems for later CSV processing
            if ( o_it->second == DLTX_DELETE ) {
                if ( IsMergingBaseAndMod ) {
                    DeletedItems.insert( o_it->first );
                }
            ...
            // 1. In Base+Mod merge, Add to DeletedItems for later CSV
            // processing
...
CInifile::Items CInifile::EvaluateSection( shared_str SectionName,
    ...
    if ( Evaluations.IsInStack( SectionName ) ) {
        Debug.fatal(
            DEBUG_INFO,
            "[DLTX] Section '%s' has cyclical dependencies. Cycle loop %s. "
            "Check this file and its DLTX mods: %s, mod file %s",
            SectionName.c_str(),
            Evaluations.GetRecursionStackAsString().c_str(), m_file_name,
            currentFileName );
    }
    ...
    auto* BaseParents = ( BaseParentsIt != BaseParentDataMap.end() )
                            ? &BaseParentsIt->second
                            : nullptr;
    auto* OverrideParents = ( OverrideParentsIt != OverrideParentDataMap.end() )
                                ? &OverrideParentsIt->second
                                : nullptr;
    ...
    if ( OverrideParents && !BaseParents ) {
        ...
        BaseParents = ( BaseParentsIt != BaseParentDataMap.end() )
                          ? &BaseParentsIt->second
                          : nullptr;
        ...
        for ( const auto& parent : *BaseParents ) {
            ...
            if ( BaseDataIt == BaseData.end() ) {
                ...
                if ( OverrideDataIt != OverrideData.end() ) {
                    if ( print_dltx_warnings ) {
                        Msg( "~[DLTX] WARNING: Section '%s' has parent '%s' "
                             "that is defined as Override. Creating parent for "
                             "backwards compatibility. Check this file and its "
                             "DLTX mods: %s, mod file %s",
                             SectionName.c_str(), parent.c_str(), m_file_name,
                             currentFileName );
                    }
                    BaseData[ parent ].Name = parent;
                } else {
                    if ( print_dltx_warnings ) {
                        Msg( "~[DLTX] WARNING: Section '%s' inherits from "
                             "non-existent section '%s'. Creating fallback "
                             "empty parent section. Check this file and its "
                             "DLTX mods: %s, mod file %s",
                             SectionName.c_str(), parent.c_str(), m_file_name,
                             currentFileName );
                    }
            ...
            Items ParentData =
                EvaluateSection( parent, Evaluations, currentFileName );
            ResolvedParents = MergeSections( ResolvedParents, ParentData,
                                             DeletedItems, false );
        }
    }
    ...
    if ( o_it != OverrideData.end() ) {
        ResolvedBaseAndMods = MergeSections(
            ResolvedBaseAndMods, o_it->second.Data, DeletedItems, true );
        OverrideData.erase( o_it );
    }
    Items CurrentResult = MergeSections( ResolvedParents, ResolvedBaseAndMods,
                                         DeletedItems, false );
    ...
    static auto split_list = []( shared_str items, char delimiter = ',' ) {
        ...
        while ( true ) {
            if ( *current == delimiter || *current == '\0' ) {
                ...
                while ( t_start < t_end &&
                        isspace( ( unsigned char )*t_start ) )
                    t_start++;
                // Trim trailing whitespace
                while ( t_end > t_start &&
                        isspace( ( unsigned char )*( t_end - 1 ) ) )
                    t_end--;
    ...
    static auto join_list = []( const xr_vector< std::string_view >& items_vec,
                                char delimiter = ',' ) {
        if ( items_vec.empty() )
            return xr_string();
    ...
    if ( OverrideModifyListData.find( SectionName ) !=
         OverrideModifyListData.end() ) {
        // 1. Pre-sort the modifications by key (ignoring the > / < prefix for
        // the sort) and insertionIndex This allows us to walk through
        // CurrentSect.Data and OverrideModifyListData simultaneously.
        auto& overrideData = OverrideModifyListData[ SectionName ];
        if ( !overrideData.empty() ) {
            std::sort( overrideData.begin(), overrideData.end(),
                       []( const Item& a, const Item& b ) {
                           int res =
                               xr_strcmp( ( *a.first ) + 1, ( *b.first ) + 1 );
                           if ( res != 0 )
                               return res < 0;
            ...
            while ( data_it != CurrentResult.end() ||
                    mod_it != overrideData.end() ) {
                ...
                // 2. Identify if we have an existing item and any mods for this
                // key
                Item* existing = ( data_it != CurrentResult.end() &&
                                   data_it->first == active_key )
                                     ? &( *data_it )
                                     : nullptr;

                // 3. Process all mods for this specific key in a sub-loop
                if ( mod_it != overrideData.end() &&
                     xr_strcmp( ( *mod_it->first ) + 1, active_key ) == 0 ) {
                    ...
                    if ( existing ) {
                        ...
                        if ( DeletedItems.find( active_key ) ==
                             DeletedItems.end() ) {
                    ...
                    // Apply all mods for this key (e.g., <item, then >newitem1,
                    // then >newitem3)
                    while ( mod_it != overrideData.end() &&
                            xr_strcmp( ( *mod_it->first ) + 1, active_key ) ==
                                0 ) {
                        if ( exists_in_output && mod_it->second != NULL ) {
                            ModifyListType op = static_cast< ModifyListType >(
                                ( *mod_it->first )[ 0 ] );

                            auto sect_it_items_vec =
                                split_list( working_item.second );
                            ...
                            if ( op == ModifyListType::Insert ) {
                                ...
                                sect_it_items_vec.erase(
                                    std::remove_if(
                                        sect_it_items_vec.begin(),
                                        sect_it_items_vec.end(),
                                        [ &I_items_vec ](
                                            std::string_view existing_item ) {
                                            // Check if the existing item is in
                                            // our "to remove" list
                                            return std::find(
                                                       I_items_vec.begin(),
                                                       I_items_vec.end(),
                            ...
                            working_item.second =
                                join_list( sect_it_items_vec ).c_str();
                    ...
                    // No mods for this key, just move the data item if it
                    // exists
                    if ( existing ) {
    ...
    return Evaluations.ResolvedCache[ SectionName ] =
...
void CInifile::Load( IReader* F,
    ...
    _splitpath_s( m_file_name, split_drive, split_drive.GetSize(), split_dir,
                  split_dir.GetSize(), split_name, split_name.GetSize(),
                  split_ext, split_ext.GetSize() );
    ...
    // Start with depth 0
    LTXLoad( F, path, true, currentFileName, 0
#ifndef _EDITOR
             ,
             allow_include_func
#endif
    ...
    for ( auto& s : SectionsToDelete ) {
        ...
        if ( it != ResolvedData.end() ) {
            ...
            if ( s_it != OverrideData.end() ) {
                Msg( "[DLTX] [%s] Deleting overrides for section %s",
                     m_file_name, s.c_str() );
        ...
        if ( print_dltx_warnings ) {
            for ( const auto& [ k, v ] : OverrideData ) {
                ...
                if ( override_filenames != OverrideToFilename.end() ) {
                    for ( const auto& override_filename :
                          override_filenames->second ) {
                        Msg(
                            "~[DLTX] WARNING: Attemped to override section "
                            "'%s', which doesn't exist. Ensure that a base "
                            "section with the same name is loaded first. Check "
                            "this file and its DLTX mods: %s, mod file %s",
                            k.c_str(), m_file_name, override_filename.c_str() );
                    }
                }
            }
        }
...
void CInifile::DLTX_print( LPCSTR sec, LPCSTR line ) {
    ...
    auto A = std::lower_bound( I.Data.begin(), I.Data.end(), line,
                               item_comparator() );
...
LPCSTR CInifile::DLTX_getFilenameOfLine( LPCSTR sec, LPCSTR line ) {
    ...
    if ( !line_exist( sec, line ) ) {
        Msg( "![DLTX_getFilenameOfLine] no line %s exists in section %s", line,
             sec );
        return nullptr;
    }
...
void CInifile::save_as( IWriter& writer, bool bcheck ) const {
    ...
    for ( RootCIt r_it = DATA.begin(); r_it != DATA.end(); ++r_it ) {
        ...
        if ( bcheck ) {
            xr_sprintf( temp, sizeof( temp ), "; %d %d",
                        ( *r_it ).Name._get()->intrusive_ref_count(),
                        xr_strlen( ( *r_it ).Name._get()->value ) );
            writer.w_string( temp );
        }
        ...
        for ( const Item& I : ( *r_it ).Data ) {
            if ( *I.first ) {
                if ( *I.second ) {
                    ...
                    // only name and value
                    xr_sprintf( temp, sizeof( temp ), "%8s%-32s = %-32s", " ",
                                I.first.c_str(), val );
                } else {
                    // only name
                    xr_sprintf( temp, sizeof( temp ), "%8s%-32s = ", " ",
                                I.first.c_str() );
                }
            } else {
                // no name, so no value
...
BOOL CInifile::line_exist( LPCSTR S, LPCSTR L ) const {
    ...
    Sect& I = r_section( S );
    auto A =
        std::lower_bound( I.Data.begin(), I.Data.end(), L, item_comparator() );
...
CInifile::Sect& CInifile::r_section( LPCSTR S ) const {
    R_ASSERT2(
        S && strlen( S ),
        "Empty section (null\\'') passed into CInifile::r_section(). See info "
        "above ^, check your configs and 'call stack'." ); //--#SM+#--

    char section[ 256 ];
    xr_strcpy( section, sizeof( section ), S );
    strlwr( section );
    RootCIt I =
        std::lower_bound( DATA.begin(), DATA.end(), section, sect_pred );
    if ( !( I != DATA.end() && xr_strcmp( *( *I ).Name, section ) == 0 ) ) {
        Debug.fatal( DEBUG_INFO,
                     "Can't open section '%s'. Please attach [*.ini_log] file "
                     "to your bug report",
                     S );
    }
...
BOOL CInifile::r_bool( LPCSTR S, LPCSTR L ) const {
    ...
    VERIFY2(
        xr_strlen( C ) <= 5,
        make_string( "\"%s\" is not a valid bool value, section[%s], line[%s]",
                     C, S, L ) );
    char B[ 8 ];
...
BOOL CInifile::r_line( const shared_str& S,
                       int L,
                       const char** N,
                       const char** V ) const {
...
void CInifile::w_string( LPCSTR S, LPCSTR L, LPCSTR V, LPCSTR comment ) {
    ...
    if ( !section_exist( sect ) ) {
        ...
        RootIt I =
            std::lower_bound( DATA.begin(), DATA.end(), sect, sect_pred );
    ...
    // #endif
    auto it = std::lower_bound( data.Data.begin(), data.Data.end(), I.first,
                                item_comparator() );
    ...
    if ( it != data.Data.end() && it->first.equal( I.first ) ) {
        ...
        if ( 0 == xr_strcmp( *it->first, *I.first ) ) {
            ...
            R_ASSERT2( b, make_string( "name[%s] already exist in section[%s]",
                                       line, sect )
                              .c_str() );
            it->second = I.second;
...
void CInifile::w_color( LPCSTR S, LPCSTR L, u32 V, LPCSTR comment ) {
    string128 temp;
    xr_sprintf( temp, sizeof( temp ), "%d,%d,%d,%d", color_get_R( V ),
                color_get_G( V ), color_get_B( V ), color_get_A( V ) );
    w_string( S, L, temp, comment );
}

void CInifile::w_ivector2( LPCSTR S,
                           LPCSTR L,
                           const Ivector2& V,
                           LPCSTR comment ) {
    string128 temp;
...
void CInifile::w_ivector3( LPCSTR S,
                           LPCSTR L,
                           const Ivector3& V,
                           LPCSTR comment ) {
    string128 temp;
...
void CInifile::w_ivector4( LPCSTR S,
                           LPCSTR L,
                           const Ivector4& V,
                           LPCSTR comment ) {
    string128 temp;
...
void CInifile::w_fvector2( LPCSTR S,
                           LPCSTR L,
                           const Fvector2& V,
                           LPCSTR comment ) {
    string128 temp;
...
void CInifile::w_fvector3( LPCSTR S,
                           LPCSTR L,
                           const Fvector3& V,
                           LPCSTR comment ) {
    string128 temp;
...
void CInifile::w_fvector4( LPCSTR S,
                           LPCSTR L,
                           const Fvector4& V,
                           LPCSTR comment ) {
    string128 temp;
...
void CInifile::remove_line( LPCSTR S, LPCSTR L ) {
    ...
    if ( line_exist( S, L ) ) {
        ...
        auto A = std::lower_bound( data.Data.begin(), data.Data.end(), L,
                                   item_comparator() );
