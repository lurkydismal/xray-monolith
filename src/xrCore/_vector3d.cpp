...
typename Fvector::SelfRef Fvector::ema( Self& target, unsigned int steps ) {
    ...
    x = x +
        std::min( 1.f, smoothing_alpha * ( delta / steps ) ) * ( target.x - x );
    y = y +
        std::min( 1.f, smoothing_alpha * ( delta / steps ) ) * ( target.y - y );
    z = z +
        std::min( 1.f, smoothing_alpha * ( delta / steps ) ) * ( target.z - z );
    return *this;
}
