...
bool EFS_Utils::GetOpenName( LPCSTR initial,
                             xr_string& buffer,
                             bool bMulti,
                             LPCSTR offset,
                             int start_flt_ext ) {
    ...
    BROWSEINFO info={NULL,NULL,path,"title",BIF_USENEWUI,BrowseCallbackProc,
    (LPARAM)g_SHBF_Folder }; SHBrowseForFolder (&info);
    */
    /*
     {
     ...
     */
    // bool bRes = false;
    bool bRes = GetOpenNameInternal( initial, buf, sizeof( buf ), bMulti,
                                     offset, start_flt_ext );
...
xr_string EFS_Utils::AppendFolderToName( xr_string& tex_name,
                                         int depth,
                                         BOOL full_name ) {
    string1024 nm;
