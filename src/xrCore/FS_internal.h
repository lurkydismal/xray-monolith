...
public:
    CFileWriter( const char* name, bool exclusive ) {
        ...
        if ( exclusive ) {
            int handle = _sopen( fName.c_str(),
                                 _O_WRONLY | _O_TRUNC | _O_CREAT | _O_BINARY,
                                 SH_DENYWR );
            if ( handle == -1 )
                Msg( "!Can't create file: '%s'. Error: '%s'.", fName.c_str(),
                     _sys_errlist[ errno ] );
            hf = _fdopen( handle, "wb" );
        } else {
            hf = fopen( fName.c_str(), "wb" );
            if ( hf == 0 )
                Msg( "!Can't write file: '%s'. Error: '%s'.", fName.c_str(),
                     _sys_errlist[ errno ] );
        }
    }
    ...
    virtual void w( const void* _ptr, u32 count ) {
        if ( ( 0 != hf ) && ( 0 != count ) ) {
            ...
            u8* ptr = ( u8* )_ptr;
            int req_size;
            for ( req_size = count; req_size > mb_sz;
                  req_size -= mb_sz, ptr += mb_sz ) {
                size_t W = fwrite( ptr, mb_sz, 1, hf );
                R_ASSERT3( W == 1,
                           "Can't write mem block to file. Disk maybe full.",
                           _sys_errlist[ errno ] );
            }
            if ( req_size ) {
...
public:
    CTempReader( void* _data, int _size, int _iterpos )
...
public:
    CPackReader( void* _base, void* _data, int _size )
        : IReader( _data, _size ) {
        base_address = _base;
    }
    virtual ~CPackReader();
