...
void addShaderSourceCrc32( const void* sourceData,
                           u32 sourceSize,
                           LPCSTR shaderPath,
                           u32& crc,
                           xr_vector< ShaderSourceDependency >* dependencies );

IReader* openShaderInclude( LPCSTR shaderPath,
                            LPCSTR includeName,
                            string_path& resolved ) {
    string_path includePath;
    strconcat( sizeof( includePath ), includePath, shaderPath ? shaderPath : "",
               includeName );
...
void addIncludedShaderCrc32(
    LPCSTR shaderPath,
    LPCSTR includeName,
    u32& crc,
    xr_vector< ShaderSourceDependency >* dependencies ) {
    string_path resolved;
    IReader* includeReader =
        openShaderInclude( shaderPath, includeName, resolved );
    if ( !includeReader ) {
        Msg( "! Shader source CRC: can't find include '%s', skipping it for "
             "cache validation",
             includeName );
        return;
    }
    ...
    if ( dependencies ) {
        ...
        if ( file )
            dependencies->push_back( { resolved, file->crc, file->size_real,
                                       file->size_compressed, file->modif } );
    }
    addShaderSourceCrc32( includeReader->pointer(), includeReader->length(),
                          shaderPath, crc, dependencies );
    FS.r_close( includeReader );
}

void parseShaderIncludes( const char* sourceData,
                          u32 sourceSize,
                          LPCSTR shaderPath,
                          u32& crc,
                          xr_vector< ShaderSourceDependency >* dependencies ) {
...
void addShaderSourceCrc32( const void* sourceData,
    ...
    parseShaderIncludes( static_cast< const char* >( sourceData ), sourceSize,
                         shaderPath, crc, dependencies );
}
...
bool dependenciesUnchanged( const ShaderSourceCrcCacheEntry& entry ) {
    for ( const ShaderSourceDependency& dependency : entry.dependencies ) {
        ...
        if ( !file || file->crc != dependency.crc ||
             file->size_real != dependency.size_real ||
             file->size_compressed != dependency.size_compressed ||
             file->modif != dependency.modif ) {
            return false;
        }
    }
    return true;
...
u32 getShaderSourceCrc32( const void* sourceData,
                          u32 sourceSize,
                          LPCSTR shaderPath ) {
    u32 crc = 0;
...
u32 getShaderSourceCrc32Cached( const void* sourceData,
                                u32 sourceSize,
                                LPCSTR shaderPath,
                                LPCSTR sourceName,
                                LPCSTR target ) {
    ...
    string_path resolved;
    if ( const CLocatorAPI::file* source =
             FS.exist( resolved, "$game_shaders$", relative.c_str() ) ) {
        string128 identity;
        xr_sprintf( identity, "|%08x:%08x:%08x:%08x", source->crc,
                    source->size_real, source->size_compressed, source->modif );
        key = resolved;
        key += identity;
    } else {
        string32 size;
    ...
    addShaderSourceCrc32( sourceData, sourceSize, shaderPath, entry.crc,
                          &entry.dependencies );
    {
        xrCriticalSectionGuard guard( ShaderSourceCrcCacheLock() );
