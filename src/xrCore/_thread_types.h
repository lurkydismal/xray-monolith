...
public:
    ...
    class Batch {
        ...
        explicit Batch( std::shared_ptr< BatchState > value )
    ...
    Batch BeginBatch( GenerationId id ) {
        std::lock_guard< std::mutex > guard( mutex );
        if ( !current_generation || current_generation->id != id ||
             !current_generation->accepting || current_generation->cancelled )
            return {};
    ...
    bool Submit( const Batch& batch,
                 NativeLoadPriority priority,
                 Function&& function ) {
        return SubmitImpl(
            batch, priority,
    ...
    bool Submit( const Batch& batch,
        ...
        return SubmitImpl(
            ...
            std::function< void() >( std::forward< Function >( function ) ),
            std::function< void() >(
                std::forward< CancelFunction >( cancel ) ) );
    }
    ...
    void Wait( const Batch& batch, bool help = true ) {
        ...
        for ( ;; ) {
            ...
            batch_state->completed.wait(
                guard, [ &batch_state ] { return !batch_state->pending; } );
        }
    ...
    bool IsCurrent( GenerationId id ) const {
        std::lock_guard< std::mutex > guard( mutex );
        return current_generation && current_generation->id == id &&
...
private:
    struct GenerationState {
        ...
        std::shared_ptr< xr_task_group > pumps =
            std::make_shared< xr_task_group >();
    };
    ...
    static constexpr size_t PriorityCount =
        static_cast< size_t >( NativeLoadPriority::Count );
    ...
    bool PopWorkLocked( const std::shared_ptr< GenerationState >& generation,
                        WorkItem& result ) {
    ...
    void RecordFailureLocked(
        const std::shared_ptr< GenerationState >& generation,
    ...
    void SchedulePumpsLocked(
        const std::shared_ptr< GenerationState >& generation ) {
        u32 pumps_to_start = std::min( worker_limit - generation->active_pumps,
                                       generation->queued );
        while ( pumps_to_start-- ) {
            ...
            {
#endif
            generation->pumps->run(
                [ this, generation ] { Pump( generation ); } );
#if 0
            }
            catch (...)
            {
    ...
    bool SubmitImpl( const Batch& batch,
                     NativeLoadPriority priority,
                     std::function< void() > function,
                     std::function< void() > cancel ) {
        const auto batch_state = batch.state;
        if ( !batch_state || !function ||
             priority >= NativeLoadPriority::Count )
            return false;

        std::lock_guard< std::mutex > guard( mutex );
        const auto& generation = batch_state->generation;
        if ( current_generation != generation || !generation->accepting ||
             generation->cancelled || batch_state->closed )
            return false;

        WorkItem work{ generation, batch_state, std::move( function ),
                       std::move( cancel ) };
        queues[ static_cast< size_t >( priority ) ].push_back(
            std::move( work ) );
        ++batch_state->pending;
        ++generation->pending;
        ++generation->queued;
    ...
    void Execute( WorkItem& work ) {
        ...
        {
            std::lock_guard< std::mutex > guard( mutex );
            execute = current_generation == work.generation &&
    ...
    void WaitForGeneration(
        ...
        for ( ;; ) {
            ...
            std::unique_lock< std::mutex > guard( mutex );
            generation->completed.wait(
                guard, [ &generation ] { return !generation->pending; } );
        }

        std::unique_lock< std::mutex > guard( mutex );
        generation->pumps_idle.wait(
            guard, [ &generation ] { return !generation->active_pumps; } );
    }

    void FinishGeneration( const std::shared_ptr< GenerationState >& generation,
                           bool cancel ) {
        if ( !cancel ) {
