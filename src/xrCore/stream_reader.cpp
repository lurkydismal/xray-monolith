...
void CStreamReader::map( const u32& new_offset ) {
    ...
    m_current_map_view_of_file =
        ( u8* )MapViewOfFile( m_file_mapping_handle, FILE_MAP_READ, 0,
...
CStreamReader* CStreamReader::open_chunk( const u32& chunk_id ) {
    ...
    result->construct( file_mapping_handle(), m_start_offset + tell(), size,
                       m_archive_size, m_window_size );
    return ( result );
}
...
void CStreamReader::r_stringZ( shared_str& dest ) {
    ...
    do {
        ...
        u32 current_chunk_size =
            static_cast< u32 >( end_ptr - m_current_pointer );
