...
public:
    ...
    IC SelfRef
    transpose( const _matrix< T >& matSource ) // faster version of transpose
    {
        _11 = matSource._11;
        _12 = matSource._21;
        _13 = matSource._31;
                ...
                tresh = 0.0f;
            {
                ...
                if ( i > 3 && _abs( d.x ) + g == _abs( d.x ) &&
                    ...
                    if ( _abs( h ) + g == _abs( h ) )
                        ...
                        t = 1.0f /
                            ( _abs( theta ) + _sqrt( 1.0f + theta * theta ) );
                        if ( theta < 0.0f )
                            t = -t;
                    }
                    c = 1.0f / _sqrt( 1 + t * t );
...
#undef ROT
    ...
    IC SelfCRef sMxVpV( Tvector& R,
                        float s1,
                        const Tvector& V1,
                        const Tvector& V2 ) const {
        R.x = s1 * ( m[ 0 ][ 0 ] * V1.x + m[ 0 ][ 1 ] * V1.y +
                     m[ 0 ][ 2 ] * V1.z ) +
              V2.x;
        R.y = s1 * ( m[ 1 ][ 0 ] * V1.x + m[ 1 ][ 1 ] * V1.y +
                     m[ 1 ][ 2 ] * V1.z ) +
              V2.y;
        R.z = s1 * ( m[ 2 ][ 0 ] * V1.x + m[ 2 ][ 1 ] * V1.y +
                     m[ 2 ][ 2 ] * V1.z ) +
              V2.z;
        return *this;
    }
    ...
    IC SelfRef sMTxV( Tvector& R, float s1, const Tvector& V1 ) const {
        ...
        R.z = s1 *
              ( m[ 0 ][ 2 ] * V1.x + m[ 1 ][ 2 ] * V1.y + m[ 2 ][ 2 ] * V1.z );
        return *this;
    }
    ...
    IC void transform_dir( _vector2< T >& dest,
                           const _vector2< T >& v ) const // preferred to use
    {
