...
// Counter behaviors
enum class CounterPolicy { Atomic, NonAtomic };

template < CounterPolicy Counter = CounterPolicy::Atomic >
struct ref_count_storage {
protected:
    std::conditional_t< Counter == CounterPolicy::Atomic, xr_atomic_u32, u32 >
        __ref_count;
...
enum class DeletionPolicy { Immediate, Deferred, Strict };
...
template < DeletionPolicy Policy = DeletionPolicy::Immediate,
           CounterPolicy Counter = CounterPolicy::Atomic,
           bool Virtual = true >
struct __declspec( novtable ) intrusive_base_impl
    : public intrusive_base_marker,
      ref_count_storage< Counter >,
      std::
          conditional_t< Virtual, destructor_virtual, destructor_non_virtual > {
...
// Strict policy - forbid calling xr_delete<ptr.get()>, must have protected
// destructor
template < CounterPolicy Counter >
struct __declspec( novtable )
intrusive_base_impl< DeletionPolicy::Strict, Counter >
    : public intrusive_base_marker, ref_count_storage< Counter > {
    // This makes the policy visible to the smart pointer
    ...
    // This allows 'xr_delete(base_ptr)' to work even though the destructor is
    // protected.
...
private:
    ...
    // via their base pointer. This invokes the virtual destructor chain
    // correctly but ensures we only need to friend xr_special_free in THIS
    // class, not in every derived class.
    IC void _release( intrusive_base_impl* object ) { xr_delete( object ); }
};

using intrusive_base =
    intrusive_base_impl< DeletionPolicy::Immediate, CounterPolicy::Atomic >;
using intrusive_base_nonatomic =
    intrusive_base_impl< DeletionPolicy::Immediate, CounterPolicy::NonAtomic >;
using intrusive_base_deferred =
    intrusive_base_impl< DeletionPolicy::Deferred, CounterPolicy::Atomic >;
using intrusive_base_deferred_nonatomic =
    intrusive_base_impl< DeletionPolicy::Deferred, CounterPolicy::NonAtomic >;
using intrusive_base_strict =
    intrusive_base_impl< DeletionPolicy::Strict, CounterPolicy::Atomic >;
using intrusive_base_strict_nonatomic =
    intrusive_base_impl< DeletionPolicy::Strict, CounterPolicy::NonAtomic >;
...
private:
    ...
    // Safety Enforcement
    // This asserts that 'object_type' does NOT have a public destructor if
    // policy is Strict (intrusive_base_strict). If this triggers, it means you
    // forgot to make your destructor protected. Note: std::is_destructible_v is
    // false if the destructor is protected/private.
    static_assert(
        !( deletion_policy == DeletionPolicy::Strict &&
           std::is_destructible_v< object_type > ),
    ...
    static_assert( !virtual_destructor || std::is_polymorphic_v< object_type >,
                   "intrusive_ptr<T>: T must be polymorphic because the base "
                   "class is virtual" );
...
public:
    // Allow intrusive_ptrs of different types to access private m_object for
    // conversion
    template < typename U >
