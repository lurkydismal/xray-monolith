...
extern void register_file_mapping( void* address,
                                   const u32& size,
                                   LPCSTR file_name );
...
public:
    ...
    virtual ~IWriter() {
        R_ASSERT3( chunk_pos.empty(), "Opened chunk not closed.",
                   fName.c_str() );
    }
...
public:
    ...
    IC float r_float_q16( float min, float max ) {
        u16 val = r_u16();
        float A = ( float( val ) * ( max - min ) ) / 65535.f +
                  min; // floating-point-error possible
    ...
    IC float r_float_q8( float min, float max ) {
        u8 val = r_u8();
        float A = ( float( val ) / 255.0001f ) * ( max - min ) +
                  min; // floating-point-error possible
        VERIFY( ( A >= min ) && ( A <= max ) );
        return A;
    }
    ...
    IC BOOL r_chunk( u32 ID,
                     void* dest ) // чтение XR Chunk'ов (4b-ID,4b-size,??b-data)
    {
    ...
    r_chunk_safe( u32 ID,
                  void* dest,
                  u32 dest_size ) // чтение XR Chunk'ов (4b-ID,4b-size,??b-data)
    {
        u32 dwSize = ( ( implementation_type* )this )->find_chunk( ID );
        if ( dwSize != 0 ) {
...
public:
    ...
    IC void advance( int cnt ) {
        Pos += cnt;
        VERIFY( ( Pos <= Size ) && ( Pos >= 0 ) );
    }; // AVO: commented out to allow COC to run in debug. Was failing when
       // reading one of *.thm. UNDONE after non fatal VERIFY implementation
...
public:
    ...
    // iterators
    IReader* open_chunk_iterator( u32& ID,
                                  IReader* previous = NULL ); // NULL=first
