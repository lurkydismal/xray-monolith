...
template < typename T >
struct handles_own_init< T, std::void_t< typename T::NoZeroMemoryNew > >
    : T::NoZeroMemoryNew {};
...
IC T* xr_new( Args&&... args ) {
    ...
    if constexpr ( bZero ) {
        /*if (size > 1024)
            Msg("~ [MEM] allocating object of %s, %zu", typeid(T).name(),
           size);*/
    }
...
IC void xr_delete( T* const& ptr ) {
    if ( ptr ) {
        xr_special_free< std::is_polymorphic< T >::value, T >()(
            const_cast< T*& >( ptr ) );
...
xr_unique_ptr< T > xr_make_unique( ARGS&&... args ) {
    ...
    return xr_unique_ptr< T >( static_cast< T* >( TypeMem ),
                               xr_special_free< false, T >{} );
}
