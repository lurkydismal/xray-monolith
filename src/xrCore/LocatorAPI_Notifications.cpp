...
CFS_PathNotificator::CFS_PathNotificator() : CThread( 0 ) {
    FMutex = CreateMutex(
        NULL, true /* initial owner - must be Release'd by this thread*/,
        NULL );
    if ( FMutex )
...
void CFS_PathNotificator::RegisterPath( FS_Path& path ) {
    ...
    for ( PathIt it = events.begin(); it != events.end(); it++ )
        if ( ( it->FDirectory == dir ) &&
             ( it->bRecurse == path.m_Flags.is( FS_Path::flRecurse ) ) )
            return;
...
void CFS_PathNotificator::Execute( void ) {
    ...
    for ( PathIt it = events.begin(); it != events.end(); it++ ) {
        Path& P = *it;
        P.FWaitHandle = FindFirstChangeNotification(
            P.FDirectory.c_str(), P.bRecurse, FNotifyOptionFlags );
        if ( P.FWaitHandle == INVALID_HANDLE_VALUE )
#ifndef __BORLANDC__
            Debug.fatal(
                DEBUG_INFO,
                "Can't create notify handle for path: '%s'\nwith error: '%s'",
    ...
    while ( !Terminated ) {
        ...
        int Obj = WaitForMultipleObjects( hHandles.size(), &*hHandles.begin(),
                                          false, INFINITE );
...
void CLocatorAPI::SetEventNotification() {
    ...
    FThread->FNotifyOptionFlags = FILE_NOTIFY_CHANGE_FILE_NAME |
...
void CLocatorAPI::ClearEventNotification() {
    if ( FThread ) {
        FThread->Terminate();
        ReleaseMutex(
            FThread->FMutex ); // this current thread must release the mutex
        xr_delete( FThread );
    }
