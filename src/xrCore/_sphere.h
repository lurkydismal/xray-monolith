...
public:
    ...
    ICF ERP_Result intersect( const _vector3< T >& S,
                              const _vector3< T >& D,
                              T range,
                              int& quantity,
                              T afT[ 2 ] ) const {
    ...
    /*
     int quantity;
     float afT[2];
     Fsphere::ERP_Result result =
     sS.intersect(ray.pos,ray.fwd_dir,range,quantity,afT);

     if (Fsphere::rpOriginInside ||
     ((result==Fsphere::rpOriginOutside)&&(afT[0]<range))){ if (b_nearest) {
     switch(result){
     ...
     */
    ICF ERP_Result intersect_full( const _vector3< T >& start,
                                   const _vector3< T >& dir,
                                   T& dist ) const;

    ICF ERP_Result intersect( const _vector3< T >& start,
                              const _vector3< T >& dir,
                              T& dist ) const {
        int quantity;
        T afT[ 2 ];
    ...
    IC ERP_Result intersect2( const _vector3< T >& S,
                              const _vector3< T >& D,
                              T& range ) const {
        _vector3< T > Q;
        Q.sub( P, S );
...
template < typename T >
ICF _sphere< T >::ERP_Result _sphere< T >::intersect_full(
    const _vector3< T >& start,
    const _vector3< T >& dir,
    T& dist ) const {
    int quantity;
    float afT[ 2 ];
...
void XRCORE_API Fsphere_compute( Fsphere& dest,
                                 const Fvector* verts,
                                 int count );
