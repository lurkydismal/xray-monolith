...
public:
    ICF void _set( T* rhs ) {
        if ( 0 != rhs )
            rhs->dwReference.fetch_add( 1, std::memory_order_relaxed );
        _dec();
        p_ = rhs;
    }
...
inline bool operator==( resptr_core< T, D > const& a,
                        resptr_core< U, D > const& b ) {
...
resptr_core< T, D > dynamic_pointer_cast( resptr_core< U, D > const& p ) {
    return fast_dynamic_cast< T* >( p.get() );
}
