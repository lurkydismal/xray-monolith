...
int _cpuid( _processor_info* pinfo ) {
    ...
    *reinterpret_cast< int* >( pinfo->v_name + 8 ) = data[ 0 ][ 2 ];
    ...
    if ( f_1_EDX[ 23 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_MMX );
    if ( f_1_EDX[ 25 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_SSE );
    if ( f_1_EDX[ 26 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_SSE2 );
    if ( isAmd && f_81_EDX[ 31 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_3DNOW );

    if ( f_1_ECX[ 0 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_SSE3 );
    if ( f_1_ECX[ 9 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_SSSE3 );
    if ( f_1_ECX[ 19 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_SSE4_1 );
    if ( f_1_ECX[ 20 ] )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_SSE4_2 );

    __cpuid( cpui.data(), 1 );

    const bool hasMWait = ( cpui[ 2 ] & 0x8 ) > 0;
    if ( hasMWait )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_MWAIT );
    ...
    auto ptr = reinterpret_cast< PSYSTEM_LOGICAL_PROCESSOR_INFORMATION >(
        buffer.get() );
    ...
    if ( logicalProcessorCount != processorCoreCount )
        pinfo->feature |= static_cast< u32 >( _CPU_FEATURE_HTT );
