...
void Detect() {
    ...
    // We do this without changing priority; modern CPUs have very stable RDTSC
    // latencies.
    clk_overhead = 0;
    for ( int i = 0; i < 256; i++ ) {
        u64 start = GetCLK();
...
void _initialize_cpu( void ) {
    Msg( "* Detected CPU: %s [%s], F%d/M%d/S%d, %.2f mhz, %d-clk 'rdtsc'",
         CPU::ID.model_name, CPU::ID.v_name, CPU::ID.family, CPU::ID.model,
         CPU::ID.stepping, float( CPU::clk_per_second / u64( 1000000 ) ),
...
void thread_name( const char* name ) {
    ...
    tn.dwFlags = 0;
    __try {
        RaiseException( 0x406D1388, 0, sizeof( tn ) / sizeof( DWORD ),
                        ( ULONG_PTR* )&tn );
...
void thread_spawn( thread_t* entry,
                   const char* name,
                   unsigned stack,
                   void* arglist ) {
...
void spline2( float t, Fvector* p, Fvector* ret ) {
    ...
    ret->x = ( p[ 0 ].x * m[ 0 ] + p[ 1 ].x * m[ 1 ] + p[ 2 ].x * m[ 2 ] +
               p[ 3 ].x * m[ 3 ] ) /
             6.0f;
    ret->y = ( p[ 0 ].y * m[ 0 ] + p[ 1 ].y * m[ 1 ] + p[ 2 ].y * m[ 2 ] +
               p[ 3 ].y * m[ 3 ] ) /
             6.0f;
    ret->z = ( p[ 0 ].z * m[ 0 ] + p[ 1 ].z * m[ 1 ] + p[ 2 ].z * m[ 2 ] +
               p[ 3 ].z * m[ 3 ] ) /
             6.0f;
}
...
void spline3( float t, Fvector* p, Fvector* ret ) {
    ...
    float b1 =
        d *
        ( 2 * b13 * t * ( t2 - 3 * t + 3 ) + 2 * b12 * ( t3 - 3 * t2 + 2 ) +
