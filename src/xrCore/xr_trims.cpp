...
LPSTR _ReplaceItems( LPCSTR src,
                     int idx_start,
                     int idx_end,
                     LPCSTR new_items,
                     LPSTR dst,
...
#ifdef M_BORLAND
AnsiString& _ReplaceItem( LPCSTR src,
                          int index,
                          LPCSTR new_item,
                          AnsiString& dst,
                          char separator ) {
...
AnsiString& _ReplaceItems( LPCSTR src,
                           int idx_start,
                           int idx_end,
                           LPCSTR new_items,
                           AnsiString& dst,
...
AnsiString _ListToSequence( const AStringVec& lst ) {
    ...
    if ( lst.size() ) {
        out = lst.front();
        for ( AStringVec::const_iterator s_it = lst.begin() + 1;
              s_it != lst.end(); s_it++ )
            out += AnsiString( "," ) + ( *s_it );
    }
    return out;
...
xr_string _ListToSequence( const SStringVec& lst ) {
    ...
    if ( lst.size() ) {
        out = lst.front();
        for ( SStringVec::const_iterator s_it = lst.begin() + 1;
              s_it != lst.end(); s_it++ )
            out += xr_string( "," ) + ( *s_it );
    }
    return out;
...
shared_str _ListToSequence( const RStringVec& lst ) {
    ...
    if ( lst.size() ) {
        out = *lst.front();
        for ( RStringVec::const_iterator s_it = lst.begin() + 1;
              s_it != lst.end(); s_it++ ) {
            out += ",";
            out += **s_it;
        }
