...
LPSTR XRCORE_API strconcat( int dest_sz,
                            char* dest,
                            const char* S1,
                            const char* S2 );
LPSTR XRCORE_API strconcat( int dest_sz,
...
#else // _EDITOR
// obsolete: should be deleted as soon borland work correctly with new
// strconcats
...
// dest = S1+S2+S3
IC char* strconcat( int dest_sz,
                    char* dest,
                    const char* S1,
                    const char* S2,
                    const char* S3 ) {
    return xr_strcat( xr_strcat( xr_strcpy( dest, dest_sz, S1 ), dest_sz, S2 ),
                      dest_sz, S3 );
}
...
IC char* strconcat( int dest_sz,
                    ...
                    const char* S4 ) {
    return xr_strcat(
        xr_strcat( xr_strcat( xr_strcpy( dest, dest_sz, S1 ), dest_sz, S2 ),
                   dest_sz, S3 ),
        dest_sz, S4 );
}
...
IC char* strconcat( int dest_sz,
    ...
    return xr_strcat(
        xr_strcat(
            xr_strcat( xr_strcat( xr_strcat( xr_strcpy( dest, dest_sz, S1 ),
                                             dest_sz, S2 ),
                                  dest_sz, S3 ),
                       dest_sz, S4 ),
            dest_sz, S5 ),
...
bool XRCORE_API _strconcatSingle( char*& destPtr,
                                  char* pDestEnd,
                                  const char* src );
...
#define STRCONCAT( dest, ... )                                             \
    do {                                                                   \
        xray::core::detail::string_tupples                                 \
            STRCONCAT_tupples_unique_identifier( __VA_ARGS__ );            \
        u32 STRCONCAT_buffer_size =                                        \
            STRCONCAT_tupples_unique_identifier.size();                    \
...
#define STRCONCAT( dest, ... )                                              \
    do {                                                                    \
        xray::core::detail::string_tupples                                  \
            STRCONCAT_tupples_unique_identifier( __VA_ARGS__ );             \
        ( dest ) =                                                          \
            ( LPSTR )_alloca( STRCONCAT_tupples_unique_identifier.size() ); \
        STRCONCAT_tupples_unique_identifier.concat( dest );                 \
    } while ( 0 )
...
char* xr_strconcat( StringReceiverType& receiver, ArgList... args ) {
    static_assert(
        std::is_array< StringReceiverType >::value ); // must be array...
    static_assert(
        std::is_same< typename std::remove_extent< StringReceiverType >::type,
                      char >::value ); // ... of chars
    ...
    if ( errorSource ) {
        Debug.fatal( DEBUG_INFO,
                     ...
                     sizeof( StringReceiverType ), errorSource,
                     &receiver[ 0 ] );
    }

    *pStrCursor =
        '\0'; // Guaranteed safe because of the -1 check in _strconcatSingle
    return &receiver[ 0 ];
}
