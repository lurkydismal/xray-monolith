...
public:
    ...
    IC SelfRef mul( SelfCRef q1l, SelfCRef q2l ) {
        ...
        w = ( ( q1l.w * q2l.w ) - ( q1l.x * q2l.x ) - ( q1l.y * q2l.y ) -
              ( q1l.z * q2l.z ) );

        x = ( ( q1l.w * q2l.x ) + ( q1l.x * q2l.w ) + ( q1l.y * q2l.z ) -
              ( q1l.z * q2l.y ) );

        y = ( ( q1l.w * q2l.y ) - ( q1l.x * q2l.z ) + ( q1l.y * q2l.w ) +
              ( q1l.z * q2l.x ) );

        z = ( ( q1l.w * q2l.z ) + ( q1l.x * q2l.y ) - ( q1l.y * q2l.x ) +
              ( q1l.z * q2l.w ) );
        return *this;
    }
    ...
    ICF SelfRef slerp( SelfCRef Q0, SelfCRef Q1, T tm ) {
        ...
        if ( !( ( T( 0 ) <= tm ) && ( tm <= T( 1 ) ) ) )
            Debug.fatal( DEBUG_INFO,
                         "Quaternion::slerp - invalid 'tm' arrived: %f", tm );
#endif

        T cosom = ( Q0.w * Q1.w ) + ( Q0.x * Q1.x ) + ( Q0.y * Q1.y ) +
                  ( Q0.z * Q1.z );
    ...
    IC BOOL cmp( SelfCRef Q, T Tolerance = 0.0001f ) {
        if ( // they are the same but with opposite signs
            ( ( _abs( x + Q.x ) <= Tolerance ) &&
              ...
              ( _abs( w + Q.w ) <=
                Tolerance ) ) || // they are the same with same signs
            ( ( _abs( x - Q.x ) <= Tolerance ) &&
              ( _abs( y - Q.y ) <= Tolerance ) &&
              ( _abs( z - Q.z ) <= Tolerance ) &&
