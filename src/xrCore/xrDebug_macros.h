...
#define R_ASSERT2( expr, e2 )                                    \
    do {                                                         \
        static bool ignore_always = false;                       \
        if ( !ignore_always && !( expr ) )                       \
            ::Debug.fail( _TRE( #expr ), _TRE( e2 ), DEBUG_INFO, \
                          ignore_always );                       \
    } while ( 0 )
#define R_ASSERT3( expr, e2, e3 )                                            \
    do {                                                                     \
        static bool ignore_always = false;                                   \
        if ( !ignore_always && !( expr ) )                                   \
            ::Debug.fail( _TRE( #expr ), _TRE( e2 ), _TRE( e3 ), DEBUG_INFO, \
                          ignore_always );                                   \
    } while ( 0 )
#define R_ASSERT4( expr, e2, e3, e4 )                                        \
    do {                                                                     \
        static bool ignore_always = false;                                   \
        if ( !ignore_always && !( expr ) )                                   \
            ::Debug.fail( _TRE( #expr ), _TRE( e2 ), _TRE( e3 ), _TRE( e4 ), \
...
#define R_CHK2( expr, e2 )                                            \
    do {                                                              \
        ...
        HRESULT hr = expr;                                            \
        if ( !ignore_always && FAILED( hr ) )                         \
            ::Debug.error( hr, _TRE( #expr ), _TRE( e2 ), DEBUG_INFO, \
                           ignore_always );                           \
    } while ( 0 )
...
// # define CHK_DX(expr) do {static bool ignore_always = false; HRESULT hr =
// expr; if (!ignore_always && FAILED(hr))
// ::Debug.soft_fail(/*hr,*/#expr,DEBUG_INFO);} while(0)
#define CHK_DX( a ) a
#else
#define NODEFAULT FATAL( "nodefault reached" )
#define VERIFY( expr )                                        \
