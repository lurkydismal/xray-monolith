...
inline constexpr float EPS_S = 0.0000001f;
inline constexpr float EPS = 0.0000100f;
inline constexpr float EPS_L = 0.0010000f;

#undef M_SQRT1_2
inline constexpr float M_SQRT1_2 = 0.7071067811865475244008443621048f; // 490;
...
inline constexpr float PI_MUL_2 = 6.2831853071795864769252867665590f;
inline constexpr float PI_MUL_3 = 9.4247779607693797153879301498385f;
inline constexpr float PI_MUL_4 = 12.566370614359172953850573533118f;
inline constexpr float PI_MUL_6 = 18.849555921538759430775860299677f;
inline constexpr float PI_MUL_8 = 25.132741228718345907701147066236f;
inline constexpr float PI_DIV_2 = 1.5707963267948966192313216916398f;
inline constexpr float PI_DIV_3 = 1.0471975511965977461542144610932f;
inline constexpr float PI_DIV_4 = 0.7853981633974483096156608458199f;
inline constexpr float PI_DIV_6 = 0.5235987755982988730771072305466f;
inline constexpr float PI_DIV_8 = 0.3926990816987241548078304229099f;
...
template < class T >
inline constexpr T _lerp( const T& _val_a,
                          const T& _val_b,
                          const float& _factor ) {
...
template < class T >
inline constexpr T _lerpc( const T& _val_a,
                           const T& _val_b,
                           const float& _factor ) {
...
IC bool are_ordered( float const value0,
                     float const value1,
                     float const value2 ) {
...
IC float angle_inertion( float src,
                         float tgt,
                         float speed,
                         float clmp,
                         float dt ) {
...
IC float angle_inertion_var( float src,
                             float tgt,
                             float min_speed,
                             float max_speed,
                             float clmp,
    ...
    src = angle_normalize_signed( src );
    float speed = _abs( ( max_speed - min_speed ) *
                        angle_difference( tgt, src ) / clmp ) +
                  min_speed;
...
IC Fvector EulerYawPitchRollInertion( const Fvector& current_ypr,
                                      const Fvector& target_ypr,
                                      float speed,
                                      float dt ) {
    Fvector diff;
    ...
    if ( diff_length < EPS_S )
        return Fvector{ current_ypr.x, current_ypr.y, current_ypr.z };
    ...
    return Fvector{ angle_normalize( current_ypr.x + diff.x ),
                    angle_normalize( current_ypr.y + diff.y ),
                    angle_normalize( current_ypr.z + diff.z ) };
};
...
template < class T >
IC _matrix< T >& _matrix< T >::mk_xform( const _quaternion< T >& Q,
                                         const Tvector& V ) {
    T xx = Q.x * Q.x;
    T yy = Q.y * Q.y;
    T zz = Q.z * Q.z;
...
IC _quaternion< T >& _quaternion< T >::set( const _matrix< T >& M ) {
    ...
    if ( trace > 0.0f ) {
        ...
        // in the unusual case the original trace fails to produce a good sqrt,
        // try others...
        switch ( biggest ) {
            case A:
...
#pragma deprecated( "MIN", "MAX", "ABS", fabs, fabsf, sqrt, sqrtf, malloc,  \
                    free, calloc, realloc, memmove, memset, strdup, strlen, \
