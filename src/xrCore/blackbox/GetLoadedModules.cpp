...
BOOL __stdcall GetLoadedModules( DWORD dwPID,
    ...
    if ( 0 != uiCount ) {
        ...
        ASSERT( FALSE ==
                IsBadWritePtr( paModArray, uiCount * sizeof( HMODULE ) ) );
    }
#endif
    ...
    if ( ( VER_PLATFORM_WIN32_NT == stOSVI.dwPlatformId ) &&
        ...
        return (
            NT4GetLoadedModules( dwPID, uiCount, paModArray, pdwRealCount ) );
    } else {
        // Win9x and Win2K go through tool help.
        return ( TLHELPGetLoadedModules( dwPID, uiCount, paModArray,
                                         pdwRealCount ) );
    }
}
