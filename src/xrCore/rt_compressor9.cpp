...
u32 rtc9_compress( void* dst, u32 dst_len, const void* src, u32 src_len ) {
    ...
    if ( _LZO_Dictionary ) {
        r = lzo1x_999_compress_dict( ( const lzo_byte* )src,
...
u32 rtc9_decompress( void* dst, u32 dst_len, const void* src, u32 src_len ) {
    ...
    if ( _LZO_Dictionary ) {
        r = lzo1x_decompress_dict_safe( ( const lzo_byte* )src,
                                        ( lzo_uint )src_len, ( lzo_byte* )dst,
                                        ( lzo_uintp )&out_size, NULL,
                                        _LZO_Dictionary, _LZO_DictionarySize );
    } else {
        r = lzo1x_decompress( ( const lzo_byte* )src, ( lzo_uint )src_len,
                              ( lzo_byte* )dst, ( lzo_uintp )&out_size, NULL );
    }
