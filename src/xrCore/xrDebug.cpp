...
void xrDebug::backend( const char* reason,
    ...
    INT_PTR res = -1;
#ifdef XRCORE_STATIC
    MessageBox( NULL, tmp, "X-Ray error",
                MB_OK | MB_ICONERROR | MB_SYSTEMMODAL );
#else
    res = DialogBox( GetModuleHandle( MODULE_NAME ),
                     MAKEINTRESOURCE( IDD_STOP ), NULL, DialogProc );
#endif
    switch ( res ) {
        case -1:
        case IDC_STOP:
...
int __cdecl _out_of_memory( size_t size ) {
    Debug.fatal( DEBUG_INFO, "Out of memory. Memory request: %d K",
                 size / 1024 );
    return 1;
}
...
void xrDebug::_initialize( const bool& dedicated ) {
    ...
    _set_new_handler(
        _out_of_memory ); // exception-handler for 'out of memory' condition
