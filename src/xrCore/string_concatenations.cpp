...
void process( u32 const index, u32 const count, LPCSTR* strings ) {
    ...
    LPSTR temp = ( LPSTR )_alloca( ( count * ( max_string_size + 4 ) + 1 ) *
                                   sizeof( **strings ) );
    LPSTR k = temp;
    *k++ = '[';
    for ( u32 i = 0; i < count; ++i ) {
        for ( LPCSTR j = strings[ i ], e = j + max_string_size; *j && j < e;
              ++k, ++j )
            *k = *j;
    ...
    *k = 0;
    ...
    Debug.fatal(
        DEBUG_INFO,
        make_string( "buffer overflow: cannot concatenate strings(%d):\r\n%s",
                     index, temp )
            .c_str() );
}

template < u32 count >
static inline void process( LPSTR& i,
                            LPCSTR e,
                            u32 const index,
                            LPCSTR ( &strings )[ count ] ) {
    VERIFY( i <= e );
    VERIFY( index < count );
...
void check_stack_overflow( u32 stack_increment ) {
    __try {
        ...
        p;
    } __except ( xray::core::detail::stack_overflow_exception_filter(
        GetExceptionCode() ) ) {
        _resetstkoflw();
    }
}
