...
public:
    xr_StackWalker()
        : StackWalker( StackWalker::StackWalkOptions::RetrieveSymbol |
...
protected:
    virtual void OnOutput( LPCSTR szText ) {
        ...
        if ( sLowered.find( ".dll" ) != std::string::npos )
            return;
        if ( sLowered.find( ".drv" ) != std::string::npos )
            return;
        if ( sLowered.find( "__scrt_common_main_seh" ) != std::string::npos )
            return;
        if ( s.find( "ERROR: SymGetSymFromAddr64" ) != std::string::npos )
            return;
        if ( s.find( "ERROR: SymGetLineFromAddr64" ) != std::string::npos )
            return;
        if ( sLowered.find( "filename not available" ) != std::string::npos )
            return;
        if ( sLowered.find( "function-name not available" ) !=
             std::string::npos )
            return;
        trim( s );
        Msg( "%s", s.c_str() );
...
void xrDebug::gather_info( const char* expression,
    ...
    LPCSTR endline = "\n";
    LPCSTR prefix = "[error]";
    bool extended_description =
        ( description && !argument0 && strchr( description, '\n' ) );
    for ( int i = 0; i < 2; ++i ) {
        if ( !i )
            buffer += xr_sprintf(
                buffer, assertion_size - u32( buffer - buffer_base ),
                "%sFATAL ERROR%s%s", endline, endline, endline );
        buffer +=
            xr_sprintf( buffer, assertion_size - u32( buffer - buffer_base ),
        ...
        if ( extended_description ) {
            buffer += xr_sprintf( buffer,
                                  assertion_size - u32( buffer - buffer_base ),
                                  "%s%s%s", endline, description, endline );
            if ( argument0 ) {
                if ( argument1 ) {
                    buffer += xr_sprintf(
                        buffer, assertion_size - u32( buffer - buffer_base ),
                        "%s%s", argument0, endline );
                    buffer += xr_sprintf(
                        buffer, assertion_size - u32( buffer - buffer_base ),
                        "%s%s", argument1, endline );
                } else
                    buffer += xr_sprintf(
                        buffer, assertion_size - u32( buffer - buffer_base ),
                        "%s%s", argument0, endline );
    ...
    if ( !IsDebuggerPresent() &&
         !strstr( GetCommandLine(), "-no_call_stack_assert" ) ) {
        if ( shared_str_initialized )
            Msg( "stack trace:\n" );
        ...
        buffer +=
            xr_sprintf( buffer, assertion_size - u32( buffer - buffer_base ),
                        "stack trace:%s%s", endline, endline );
        ...
        //
        // #ifdef USE_OWN_ERROR_MESSAGE_WINDOW
        //            buffer += xr_sprintf(buffer, assertion_size - u32(buffer -
        //            buffer_base), "%s%s", g_stackTrace[i], endline);
        // #endif //-USE_OWN_ERROR_MESSAGE_WINDOW
        //        }
...
void xrDebug::backend( const char* expression,
                       const char* description,
                       const char* argument0,
                       const char* argument1,
                       const char* file,
    ...
    gather_info( expression, description, argument0, argument1, file, line,
                 function, assertion_info, sizeof( assertion_info ) );
    ...
    MessageBox( NULL, assertion_info, "Fatal Error",
                MB_OK | MB_ICONERROR | MB_SYSTEMMODAL );
...
void xrDebug::backend( const char* expression,
    ...
    buffer += xr_sprintf(
        buffer, sizeof( assertion_info ) - u32( buffer - &assertion_info[ 0 ] ),
        "Press CONTINUE to continue execution and ignore all the errors of "
        "this type%s%s",
        endline, endline );
#endif // USE_OWN_ERROR_MESSAGE_WINDOW
...
#ifdef XRCORE_STATIC
    MessageBox( NULL, assertion_info, "X-Ray error",
                MB_OK | MB_ICONERROR | MB_SYSTEMMODAL );
#else
#ifdef USE_OWN_ERROR_MESSAGE_WINDOW
    ShowCursor( true );
    ...
    int result =
        MessageBox( GetTopWindow( NULL ), assertion_info, "Fatal Error",
                    /*MB_CANCELTRYCONTINUE*/ MB_OK | MB_ICONERROR |
                        /*MB_SYSTEMMODAL |*/ MB_DEFBUTTON1 | MB_SETFOREGROUND );
...
void xrDebug::error( long hr,
                     const char* expr,
                     const char* file,
                     int line,
                     const char* function,
                     bool& ignore_always ) {
    backend( expr, error2string( hr ), 0, 0, file, line, function,
             ignore_always );
}
...
void xrDebug::error( long hr,
                     ...
                     const char* function,
                     bool& ignore_always ) {
    backend( expr, error2string( hr ), e2, 0, file, line, function,
             ignore_always );
}
...
void xrDebug::fail( const char* e1,
                    ...
                    const char* function,
                    bool& ignore_always ) {
    backend( e1, "assertion failed", 0, 0, file, line, function,
             ignore_always );
}
...
void xrDebug::soft_fail( LPCSTR e1,
                         const std::string& e2,
                         LPCSTR file,
                         int line,
                         LPCSTR function ) {
    if ( !ignore_verify )
        Msg( "! VERIFY_FAILED: %s[%d] {%s}  %s %s", file, line, function, e1,
             e2.c_str() );
}
...
void xrDebug::soft_fail( LPCSTR e1,
                         ...
                         LPCSTR function ) {
    if ( !ignore_verify )
        Msg( "! VERIFY_FAILED: %s[%d] {%s}  %s %s %s", file, line, function, e1,
             e2, e3 );
}
...
void xrDebug::soft_fail( LPCSTR e1,
                         ...
                         LPCSTR function ) {
    if ( !ignore_verify )
        Msg( "! VERIFY_FAILED: %s[%d] {%s}  %s %s %s %s", file, line, function,
             e1, e2, e3, e4 );
}
...
void xrDebug::soft_fail( LPCSTR e1,
                         ...
                         LPCSTR function ) {
    if ( !ignore_verify )
        Msg( "! VERIFY_FAILED: %s[%d] {%s}  %s %s %s %s %s", file, line,
             function, e1, e2, e3, e4, e5 );
}

//-AVO

void __cdecl xrDebug::fatal( const char* file,
                             int line,
                             const char* function,
                             const char* F,
                             ... ) {
    ...
    backend( nullptr, "fatal error", buffer, 0, file, line, function,
             ignore_always );
}
...
int out_of_memory_handler( size_t size ) {
    ...
    if ( g_full_memory_stats_callback )
        ...
        int eco_strings = ( int )g_pStringContainer->stat_economy(
            eco_strings_count, eco_strings_unique_count );
        int eco_smem = ( int )g_pSharedMemoryContainer->stat_economy();
        Msg( "* [x-ray]: process heap[%llu K]", process_heap / 1024 );
        Msg( "* [x-ray]: strings: memory[%ld K], count[%lu], unique[%lu]",
             eco_strings / 1024, eco_strings_count, eco_strings_unique_count );
        Msg( "* [x-ray]: shared: memory[%ld K]", eco_smem );
    }

    Debug.fatal( DEBUG_INFO, "Out of memory. Memory request: %lld K",
                 size / 1024 );
    return 1;
}
...
void CALLBACK PreErrorHandler( INT_PTR ) {
    ...
    __try {
        ...
        if ( ( log_folder[ 0 ] != '\\' ) && ( log_folder[ 1 ] != ':' ) ) {
            ...
            strconcat( sizeof( log_folder ), log_folder, current_folder, "\\",
                       relative_path );
        }
...
void SetupExceptionHandler( const bool& dedicated ) {
    ...
    BT_SetFlags( /**/
                 BTF_DETAILEDMODE | /**BTF_EDIETMAIL | /**/ BTF_ATTACHREPORT /**| BTF_LISTPROCESSES /**| BTF_SHOWADVANCEDUI /**| BTF_SCREENCAPTURE/**/ );
...
typedef LONG WINAPI
UnhandledExceptionFilterType( struct _EXCEPTION_POINTERS* pExceptionInfo );
...
void save_mini_dump( _EXCEPTION_POINTERS* pExceptionInfo ) {
    ...
    if ( GetModuleFileName( NULL, szDbgHelpPath, _MAX_PATH ) ) {
        ...
        if ( pSlash ) {
            xr_strcpy( pSlash + 1,
                       sizeof( szDbgHelpPath ) - ( pSlash - szDbgHelpPath ),
                       "DBGHELP.DLL" );
        ...
        if ( pDump ) {
            ...
            if ( hFile != INVALID_HANDLE_VALUE ) {
                ...
                BOOL bOK = pDump( GetCurrentProcess(), GetCurrentProcessId(),
                                  hFile, dump_flags, &ExInfo, NULL, NULL );
                if ( bOK ) {
                    xr_sprintf( szScratch, "Saved dump file to '%s'",
                                szDumpPath );
                    szResult = szScratch;
                    // retval = EXCEPTION_EXECUTE_HANDLER;
                } else {
                    xr_sprintf( szScratch,
...
void xrDebug::_initialize( const bool& dedicated ) {
    ...
    m_on_dialog = 0;
    std::set_new_handler(
        def_new_handler ); // exception-handler for 'out of memory' condition
    // ::SetUnhandledExceptionFilter (UnhandledFilter); // exception handler to
...
void _terminate() {
    ...
    Debug.gather_info( nullptr, "Unexpected application termination", 0, 0,
    ...
    LPCSTR endline = "\r\n";
    LPSTR buffer = assertion_info + xr_strlen( assertion_info );
    buffer += xr_sprintf( buffer, sizeof( buffer ),
                          "Press OK to abort execution%s", endline );
...
static void handler_base( LPCSTR reason_string ) {
    ...
    Debug.backend( nullptr, reason_string, 0, 0, DEBUG_INFO, ignore_always );
}
...
static void invalid_parameter_handler( const wchar_t* expression,
    ...
    // errno_t err =
    if ( expression )
        wcstombs_s( &converted_chars, expression_, sizeof( expression_ ),
                    expression,
    ...
    Debug.backend( expression_, "invalid parameter", 0, 0, file_, line,
                   function_, ignore_always );
}
