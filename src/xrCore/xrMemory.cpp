...
#ifdef DEBUG_MEMORY_MANAGER
    if ( !Core.ParamsData.test( ECoreParams::memo ) )
...
void xrMemory::_destroy() {
    ...
    // The actual str_container will be destroyed only when no shared_str
    // instances hold it
...
void xrMemory::mem_statistic( LPCSTR fn ) {
    ...
    for ( u32 it = 0; it < debug_info.size(); it++ ) {
        ...
        fprintf( Fa, "0x%08X[%2d]: %8d %s\n", *( u32* )( &debug_info[ it ]._p ),
                 pool_id, debug_info[ it ]._size, debug_info[ it ]._name );
    }
    ...
    */
    ...
    bytes",g_pStringContainer->stat_economy()); if (g_pSharedMemoryContainer)
    Msg ("memstat: shared_mem: economy: %d
...
XRCORE_API BOOL is_stack_ptr( void* _ptr ) {
    ...
    ptrdiff_t difference = ( ptrdiff_t )_abs(
        s64( ptrdiff_t( ptr_local ) - ptrdiff_t( ptr_refsound ) ) );
