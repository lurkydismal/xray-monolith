...
void xrMemory::dbg_register( void* _p, size_t _size, const char* _name ) {
    ...
    if ( ( _size == MEMORY_LEAK_SIZE ) && _name &&
         !xr_strcmp( MEMORY_LEAK_DESCRIPTION, _name ) ) {
        static int i = 0;
        string2048 temp;
        xr_sprintf( temp, sizeof( temp ),
                    "____[%s][%d] : 0x%8x [REGISTER][%d]\n", _name, _size,
                    ( u32 )( ( size_t )_p ), i++ );
...
void xrMemory::dbg_unregister( void* _p ) {
...
#ifdef DEBUG_MEMORY_LEAK
        if ( ( debug_info[ _found ]._size == MEMORY_LEAK_SIZE ) &&
             debug_info[ _found ]._name &&
             !xr_strcmp( MEMORY_LEAK_DESCRIPTION,
                         debug_info[ _found ]._name ) ) {
            string2048 temp;
            xr_sprintf( temp, sizeof( temp ),
                        "____[%s][%d] : 0x%8x [UNREGISTER]\n",
                        debug_info[ _found ]._name, debug_info[ _found ]._size,
                        ( u32 )( ( size_t )_p ) );
            OutputDebugString( temp );
        }
#endif
    ...
    if ( debug_info_update > 1024 * 100 ) {
        debug_info_update = 0;
        debug_info.erase(
            std::remove_if( debug_info.begin(), debug_info.end(), pred_mdbg ),
            debug_info.end() );
        dbg_check();
    }
...
void xrMemory::dbg_check() {
    ...
    R_ASSERT2( HeapValidate( GetProcessHeap(), 0, 0 ),
               "CorePanic: Win32 heap corruption" );
...
XRCORE_API void dbg_dump_leaks_prepare() {
    ...
    for ( u32 it = 0; it < Memory.debug_info.size(); it++ ) {
        ...
        if ( 0 == Memory.debug_info[ it ]._name )
            continue;
        Memory.debug_info[ it ]._name =
            xr_strdup( Memory.debug_info[ it ]._name );
    }
