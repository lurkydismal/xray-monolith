...
class XRCORE_API xr_string : public std::basic_string< char,
                                                       std::char_traits< char >,
                                                       xalloc< char > > {
public:
    typedef std::basic_string< char, std::char_traits< char >, xalloc< char > >
        Super;

    xr_string( const MezzStringBuffer& buf ) : Super( buf.GetBuffer() ) {}
    ...
    xrStringVector SplitStringMulti( xr_string separator = " ",
                                     bool includeSeparators = false,
                                     bool trimStrings = false ) const;
    xrStringVector SplitStringLimit( xr_string separator = " ",
                                     int limit = 0,
                                     bool trimStrings = false ) const;
...
// All str_value will persist in str_container, make counter non atomic, change
// if there will be erase functionality
struct XRCORE_API str_value
    : public intrusive_base_impl< DeletionPolicy::Deferred,
                                  CounterPolicy::NonAtomic,
                                  false > {
    char* value;
    size_t hash;
    u32 length;
    ...
    str_value( char* s )
        : value( s ),
          ...
          length( xr_strlen( s ) ) {};
    str_value( char* s, size_t hash, u32 length )
        : value( s ), hash( hash ), length( length ) {};
...
class XRCORE_API str_container
    : public intrusive_base_impl< DeletionPolicy::Immediate,
                                  CounterPolicy::NonAtomic,
                                  false > {
private:
    struct pool_block {
...
IC xr_string UTF8_to_CP1251( xr_string const& utf8 ) {
    if ( !utf8.empty() && IsUTF8( utf8.data() ) ) {
        int wchlen = MultiByteToWideChar( CP_UTF8, 0, utf8.c_str(), utf8.size(),
                                          nullptr, 0 );
        if ( wchlen > 0 && wchlen != 0xFFFD ) {
            xr_vector< wchar_t > wbuf( wchlen );
            MultiByteToWideChar( CP_UTF8, 0, utf8.c_str(), utf8.size(),
                                 &wbuf[ 0 ], wchlen );
            xr_vector< char > buf( wchlen );
            WideCharToMultiByte( 1251, 0, &wbuf[ 0 ], wchlen, &buf[ 0 ], wchlen,
                                 0, 0 );
