...
private:
    ...
    void Realloc() {
        ...
        TNode* newNodes =
            ( TNode* )allocator::alloc( sizeof( TNode ) * newLimit );
        VERIFY( newNodes );
    ...
    IC void getLR(
        TNode* N,
        xr_vector< T, typename allocator::template helper< T >::result >& D ) {
        if ( N->left )
            getLR( N->left, D );
        D.push_back( N->val );
        if ( N->right )
    ...
    IC void getLR_P(
        TNode* N,
        xr_vector< TNode*,
                   typename allocator::template helper< TNode* >::result >&
            D ) {
        if ( N->left )
            getLR_P( N->left, D );
        D.push_back( N );
...
public:
    ...
    IC void getANY_P(
        xr_vector< void*,
                   typename allocator::template helper< void* >::result >& D ) {
        D.resize( size() );
        void** _it = &*D.begin();
        TNode* _end = end();
        for ( TNode* cur = begin(); cur != _end; cur++, _it++ )
    ...
    IC u32 get_index( TNode* N ) { return ( u32 )( N - nodes ); }

    // Get the pointer back from the index
    IC TNode* get_node( u32 index ) { return nodes + index; }
};
