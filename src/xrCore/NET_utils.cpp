...
void NET_Packet::r_seek( u32 pos ) {
    // AVO: changed changed condition to <= as all net packet script utils are
    // using r_seek(0) to read the entire packet.
    VERIFY( pos <= B.count );
    // VERIFY(pos < B.count);
    r_pos = pos;
}
