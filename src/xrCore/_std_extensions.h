...
#ifdef _EDITOR
IC char* strncpy_s( char* strDestination,
                    size_t sizeInBytes,
...
IC char* xr_strcpy( char* strDestination,
                    size_t sizeInBytes,
                    const char* strSource ) {
...
IC char* xr_strcat( char* strDestination,
                    size_t sizeInBytes,
                    const char* strSource ) {
...
inline errno_t xr_strcpy( LPSTR destination,
                          size_t const destination_size,
                          LPCSTR source ) {
    return strcpy_s( destination, destination_size, source );
}

inline errno_t xr_strcat( LPSTR destination,
                          size_t const buffer_size,
                          LPCSTR source ) {
    return strcat_s( destination, buffer_size, source );
}

inline int __cdecl xr_sprintf( LPSTR destination,
                               size_t const buffer_size,
                               LPCSTR format_string,
                               ... ) {
    va_list args;
...
inline int __cdecl xr_sprintf( LPSTR destination,
    ...
    va_list args;
    va_start( args, format_string );
    return vsnprintf_s( destination, buffer_size, buffer_size - 1,
                        format_string, args );
}
...
extern XRCORE_API u32 path_crc32( const char* path,
                                  u32 len ); // ignores '/' and '\'
