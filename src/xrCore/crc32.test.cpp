...
TEST( CRC32_PublicAPI, SeedChangesResult ) {
    ...
    const u32 a = crc32( text, 4, 0 );

    const u32 b = crc32( text, 4, 0x12345678u );

    EXPECT_NE( a, b );
}

TEST( CRC32_Internal, SSE42KnownValues ) {
    EXPECT_EQ( crc32_internal::crc32_sse42( "123456789", 9, 0xffffffffu ),
               0x1CF96D7Cu );
}

TEST( CRC32_Internal, TableKnownValues ) {
    EXPECT_EQ(
        crc32_internal::crc32_table_based( "123456789", 9, 0xffffffffu ) ^
            0xffffffffu,
        0xCBF43926u );
}

TEST( CRC32_Internal, TablePathIsStable ) {
    const std::string input = "X-Ray Engine regression test";

    const u32 result =
        crc32_internal::crc32_table_based(
            input.data(), static_cast< u32 >( input.size() ), 0xffffffffu ) ^
        0xffffffffu;
...
TEST( CRC32_Internal, SSE42PathIsStable ) {
    ...
    const u32 result = crc32_internal::crc32_sse42(
        input.data(), static_cast< u32 >( input.size() ), 0xffffffffu );
...
TEST( CRC32_Internal, DifferentAlgorithmsAreNotCompared ) {
    ...
    const u32 sse = crc32_internal::crc32_sse42( text, 9, 0xffffffffu );

    const u32 table =
        crc32_internal::crc32_table_based( text, 9, 0xffffffffu ) ^ 0xffffffffu;
...
TEST( CRC32_Internal, IncrementalMatchesSinglePass ) {
    ...
    const u32 crc =
        crc32_internal::crc32_sse42( ab.data(), ab.size() - 1, 0xffffffffu );

    u32 split =
        crc32_internal::crc32_sse42( a.data(), a.size() - 1, 0xffffffffu );

    split = crc32_internal::crc32_sse42( b.data(), b.size() - 1, split );
...
TEST( CRC32, BinaryData ) {
    ...
    EXPECT_NE( crc32( data.data(), static_cast< u32 >( data.size() ) ), 0u );
}

TEST( PathCRC32, SeparatorsAreIgnored ) {
    const char* unix = "gamedata/config/system.ltx";

    const char* windows = "gamedata\\config\\system.ltx";

    EXPECT_EQ( path_crc32( unix, static_cast< u32 >( strlen( unix ) ) ),
               path_crc32( windows, static_cast< u32 >( strlen( windows ) ) ) );
}

TEST( PathCRC32, EmptyPath ) {
    EXPECT_EQ( path_crc32( "", 0 ), 0u );
}

TEST( PathCRC32, FilenameCaseMatters ) {
    const char* a = "gamedata/config/system.ltx";

    const char* b = "gamedata/config/System.ltx";

    EXPECT_NE( path_crc32( a, strlen( a ) ), path_crc32( b, strlen( b ) ) );
}
} // namespace
