...
struct find_chunk_auto_timer {
    ...
    ~find_chunk_auto_timer() {
        if ( g_initialize_cpu_called ) {
            g_find_chunk_counter.ticks +=
                g_find_chunk_counter.timer.GetElapsed_ticks();
        }
    }
};
        ...
        while ( !eof() ) {
            ...
            if ( ( dwType & ( ~CFS_CompressMark ) ) == ID ) {
                ...
                if ( ( ID & 0x7ffffff0 ) == 0x810 ) // is it a thm chunk ID?
                {
                    ...
                    if ( pos + length !=
                         size ) // not the last chunk in the file?
                    {
                        bool ok = true;
                        ...
                        if ( !ok ) // size incorrect?
                        {
                            length = 0;
                            while ( pos + length <
                                    size ) // find correct size, up to eof
                            {
                                impl().seek( pos + length );
                                if ( pos + length <= size - 8 &&
                                     ( r_u32() & 0x7ffffff0 ) == 0x810 )
                                    break;
                                // found start of next section
                                length++;
                            }
                            Msg( "THM chunk %d fixed, wrong size = %d, correct "
                                 "size = %d",
                                 ID, dwSize, length );
                        }
                    }
...
IC u32 IReaderBase< T >::find_chunk( u32 ID, BOOL* bCompressed ) {
    ...
    if ( !m_test ) {
        ...
        ( ( std::vector< std::pair< u32, u32 > >* )&m_test->id2pos )
            ->reserve( num_chunks );
        ...
        while ( !eof() ) {
            ...
            m_test->id2pos.insert(
                IReaderBase_Test::id2pos_container::value_type( dwId, dwPos ) );
