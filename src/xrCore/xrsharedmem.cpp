...
smem_value* smem_container::dock( u32 dwCRC, u32 dwLength, void* ptr ) {
    ...
    cdb::iterator it = std::lower_bound( container.begin(), container.end(),
                                         value, smem_search );
...
void smem_container::clean() {
    ...
    for ( ; it != end; it++ )
        if ( 0 == ( *it )->dwReference.load( std::memory_order_relaxed ) )
            xr_free( *it );
    container.erase(
        remove( container.begin(), container.end(), ( smem_value* )0 ),
        container.end() );
    if ( container.empty() )
        container.clear();
    cs.Leave();
...
void smem_container::dump() {
    ...
    for ( ; it != end; it++ )
        fprintf( F, "%4u : crc[%6x], %u bytes\n",
                 ( *it )->dwReference.load( std::memory_order_relaxed ),
                 ( *it )->dwCRC, ( *it )->dwLength );
    fclose( F );
    cs.Leave();
}
...
u32 smem_container::stat_economy() {
    ...
    for ( ; it != end; it++ ) {
        ...
        counter -= node_size;
        counter += s64(
            ( s64( ( *it )->dwReference.load( std::memory_order_relaxed ) ) -
              1 ) *
            s64( ( *it )->dwLength ) );
    }
    cs.Leave();
