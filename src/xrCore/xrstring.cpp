...
str_container::str_container( str_container_constructor_key ) {
    ...
    if ( !p )
        Debug.fatal( DEBUG_INFO,
                     "str_container, failed to allocate block size %zu",
                     block_size );
    storage.reserve( 16 );
...
char* str_container::alloc_in_pool( str_c s, u32 len ) {
    ...
    // Round up to 4-byte boundary, more performant but takes more memory,
    // around 300KB more for 200k strings
    b.used = ( b.used + 3 ) & ~3;
...
intrusive_ptr< str_value > str_container::dock( str_c value ) {
    ...
    // Most of the time, the string already exists. Use shared lock
    {
        ...
        for ( auto& item : buffer[ slot ] ) {
            if ( hash == item.hash && len == item.length &&
                 std::memcmp( value, item.value, len ) == 0 )
                return &item;
        }
    }
    ...
    // String not found, upgrade to exclusive lock
    {
        ...
        // Double-check: Did another thread create this while we were swapping
        // locks?
        for ( auto& item : buffer[ slot ] ) {
            if ( hash == item.hash && len == item.length &&
                 std::memcmp( value, item.value, len ) == 0 )
...
void str_container::erase( str_c value ) {
    ...
    {
            if (hash == it->hash && len == it->length && std::memcmp(value,
    it->value, len) == 0) it = buffer[slot].erase_after(before); else
            {
                    before = it;
                    it++;
            }
...
void str_container::dump() {
    ...
    for ( const auto& list : buffer ) {
        for ( const auto& s : list )
            fprintf( F, "ref[%d]-len[%d] : %s\n", s.intrusive_ref_count(),
                     xr_strlen( s.value ), s.value );
    }
    fclose( F );
}
    ...
    for ( const auto& list : buffer ) {
        for ( const auto& s : list ) {
            ...
            xr_sprintf( temp, sizeof( temp ), "ref[%d]-len[%d] : %s\n",
                        s.intrusive_ref_count(), xr_strlen( s.value ),
                        s.value );
            W->w_string( temp );
        }
    }
...
void str_container::dump_console() {
    ...
    Msg(
        "* [x-ray]: shared strings: pool blocks[%lu], count[%lu], unique[%lu], "
        "load factor[%.2f], max_collisions[%lu], shared_str objects[%lu]",
        storage.size(), total_strings, set.size(), load_factor, max_collisions,
...
u32 str_container::stat_economy( u32& count, u32& unique ) {
    ...
    xrSRWLockGuard guard( &rwlock, true );
    u32 size =
        sizeof( *this ) + sizeof( shared_str ) * intrusive_ref_count() - 1;
    ...
    Msg(
        "* [x-ray]: shared strings: pool blocks[%lu], count[%lu], unique[%lu], "
        "load factor[%.2f], max_collisions[%lu], shared_str objects[%lu]",
        storage.size(), count, unique, load_factor, max_collisions,
        intrusive_ref_count() - 1 );
    return size;
}
    ...
    for ( ; StrCursor < size(); ++StrCursor ) {
        if ( at( StrCursor ) == splitCh ) {
            if ( ( StrCursor - SubStrBeginCursor ) > 0 ) {
                ...
                Result.emplace_back(
                    xr_string( &at( SubStrBeginCursor ), Len ) );
...
xr_string::xr_string( LPCSTR Str, u32 Size ) : Super( Str, Size ) {}

xr_string::xr_string( Super&& other ) : Super( other ) {}

xr_string::xr_string( LPCSTR Str ) : Super( Str ) {}
...
xr_vector< xr_string > xr_string::Split( u32 NumberOfSplits, ... ) const {
    ...
    for ( u32 i = 0; i < NumberOfSplits; ++i ) {
        ...
        for ( xr_string& str : Result ) {
            ...
            intermediateTokens.insert( intermediateTokens.end(),
                                       TokenStrResult.begin(),
                                       TokenStrResult.end() );
        }

        if ( !intermediateTokens.empty() ) {
            Result.clear();
            Result.insert( Result.begin(), intermediateTokens.begin(),
                           intermediateTokens.end() );
...
xr_string xr_string::Join( xrStringVector::iterator beginIter,
...
// String utils
SStringVec xr_string::SplitStringMulti( xr_string separator,
    ...
    while ( std::getline( stringStream, line ) ) {
        ...
        while ( ( pos = line.find_first_of( separator, prev ) ) !=
                xr_string::npos ) {
            if ( pos > prev )
...
SStringVec xr_string::SplitStringLimit( xr_string separator,
                                        int limit,
                                        bool trimStrings ) const {
    std::stringstream stringStream( std::string( this->c_str() ) );
    xr_string line;
            ...
            if ( limit > 0 ) {
                if ( wordVector.size() >= limit ) {
                    wordVector.push_back(
                        line.substr( prev, xr_string::npos ) );
                    return wordVector;
                }
            }
        }
...
xr_string xr_string::ReplaceAll( const xr_string& from,
                                 const xr_string& to ) const {
    xr_string result = *this;
    if ( from.empty() )
        return result;

    size_t start_pos = 0;
    while ( ( start_pos = result.find( from, start_pos ) ) !=
            xr_string::npos ) {
