...
private:
    ...
    bool OpenStartupLooseCache( IReader*& reader,
                                LPCSTR name,
                                const file& desc );
    void ProcessArchive( LPCSTR path );
    void ProcessOne( LPCSTR path,
                     const _finddata_t& entry,
                     u32 parallel_depth = 0 );
...
private:
    void check_cached_files( LPSTR fname,
                             const u32& fname_size,
                             const file& desc,
                             LPCSTR& source_name );

    void file_from_cache_impl( IReader*& R, LPSTR fname, const file& desc );
    void file_from_cache_impl( CStreamReader*& R,
                               LPSTR fname,
                               const file& desc );
    template < typename T >
    void file_from_cache( T*& R,
                          LPSTR fname,
                          const u32& fname_size,
                          const file& desc,
                          LPCSTR& source_name );
    ...
    bool check_for_file( LPCSTR path,
                         LPCSTR _fname,
                         string_path& fname,
                         const file*& desc );
...
public:
    ...
    xr_vector< LPSTR >* file_list_open( LPCSTR initial,
                                        LPCSTR folder,
                                        u32 flags = FS_ListFiles );
    ...
    FS_Path* append_path( LPCSTR path_alias,
                          LPCSTR root,
                          LPCSTR add,
                          BOOL recursive );
    LPCSTR update_path( string_path& dest, LPCSTR initial, LPCSTR src );

    int file_list( FS_FileSet& dest,
                   LPCSTR path,
                   u32 flags = FS_ListFiles,
                   LPCSTR mask = 0 );
    ...
    void auth_generate( xr_vector< shared_str >& ignore,
