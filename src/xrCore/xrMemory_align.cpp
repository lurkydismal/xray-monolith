 ...
 *
 * Purpose:
 *       Allocate of block of aligned memory aligned on the alignment of at
 * least size bytes from the heap and return a pointer to it.
 *
 * Entry:
...
void* __stdcall xr_aligned_offset_malloc( size_t size,
                                          size_t align,
                                          size_t offset ) {
...
void* __stdcall xr_aligned_realloc( void* memblock,
                                    size_t size,
                                    size_t alignment ) {
...
void* __stdcall xr_aligned_offset_realloc( void* memblock,
                                           size_t size,
                                           size_t align,
                                           size_t offset ) {
    ...
    if ( ( stptr + align + PTR_SZ + gap ) < ( uintptr_t )memblock ) {
        ...
        if ( ( ptr = ( uintptr_t )_expand( ( void* )stptr, reqsz ) ) ==
             ( uintptr_t )NULL ) {
            if ( ( ptr = ( uintptr_t )malloc( reqsz ) ) == ( uintptr_t )NULL )
                return NULL;
            bFree = 1;
 ...
 *
 * Purpose:
 *       Frees the aligned memory block which was allocated using
 * _aligned_malloc or _aligned_memory.
 *
 * Entry:
