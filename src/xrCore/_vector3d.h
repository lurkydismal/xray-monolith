...
public:
    ...
    ICF BOOL similar( const Self& v, T E = EPS_L ) const {
        return _abs( x - v.x ) < E && _abs( y - v.y ) < E &&
               _abs( z - v.z ) < E;
    };
    ...
    // CrossProduct
    ICF SelfRef crossproduct( const Self& v1,
                              const Self& v2 ) // (v1,v2) -> this
    {
    ...
    // Barycentric coords
    IC SelfRef
    from_bary( const Self& V1, const Self& V2, const Self& V3, T u, T v, T w ) {
    ...
    IC SelfRef from_bary( const Self& V1,
                          const Self& V2,
                          const Self& V3,
                          const Self& B ) {
    ...
    IC SelfRef from_bary4( const Self& V1,
                           const Self& V2,
                           const Self& V3,
                           const Self& V4,
                           T u,
    ...
    IC SelfRef mknormal_non_normalized( const Self& p0,
                                        const Self& p1,
                                        const Self& p2 ) {
        _vector3< T > v01, v12;
        v01.sub( p1, p0 );
    ...
    IC SelfRef project( const Self& other ) { return project( *this, other ); }

    IC static void generate_orthonormal_basis( const _vector3< T >& dir,
                                               _vector3< T >& up,
                                               _vector3< T >& right ) {
        T fInvLength;
    ...
    IC static void generate_orthonormal_basis_normalized(
        _vector3< T >& dir,
        _vector3< T >& up,
        _vector3< T >& right ) {
        T fInvLength;
    ...
    IC SelfRef ema( Self& target, unsigned int steps ) {
        ...
        if ( steps <= 1 ||
             ( fis_zero( x ) && fis_zero( y ) && fis_zero( z ) ) ) {
            x = target.x;
            y = target.y;
            z = target.z;
            return *this;
        ...
        x = x + std::min( 1.f, smoothing_alpha * ( delta / steps ) ) *
                    ( target.x - x );
        y = y + std::min( 1.f, smoothing_alpha * ( delta / steps ) ) *
                    ( target.y - y );
        z = z + std::min( 1.f, smoothing_alpha * ( delta / steps ) ) *
                    ( target.z - z );
        return *this;
    }
};
...
IC BOOL exact_normalize( float* a ) {
    ...
    if ( aa1 > aa0 ) {
        ...
        {
            if ( aa0 <= 0 ) {
                // dDEBUGMSG ("vector has zero size"); ... this messace is
                // annoying
...
ICF u32 GetFvectorHash( const Fvector& pos ) {
    ...
    // Forces a single bit change in the input to cascade across all 32 bits of
    // the output, completely shattering the IEEE 754 float memory layout.
    h ^= h >> 16;
    h *= 0x85ebca6bu;
    h ^= h >> 13;
    h *= 0xc2b2ae35u;
