...
TEST(XrCoreCrc32, IncrementalCrcMatchesSinglePass)
{
    ...
    const u32 single =
        crc32("hello world", sizeof("hello world") - 1);

    const u32 incremental =
        crc32(
            second,
            sizeof(second) - 1,
...
TEST(XrCoreRToken, IdIsAfterName)
{
    ...
    EXPECT_LT(
        offsetof(xr_rtoken, name),
        offsetof(xr_rtoken, id)
    );
}
