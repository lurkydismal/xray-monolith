...
public:
    ...
    IC SelfRef or_( const T mask ) {
        flags |= mask;
        return *this;
    }

    IC SelfRef or_( const Self& f, const T mask ) {
        flags = f.flags | mask;
        return *this;
    }

    IC SelfRef and_( const T mask ) {
        flags &= mask;
        return *this;
    }

    IC SelfRef and_( const Self& f, const T mask ) {
        flags = f.flags & mask;
        return *this;
    }
...
class xr_bitsetflags {
    static_assert( std::is_enum_v< EnumT >,
                   "xr_bitsetflags can only be specialized for enum types" );

    using UnderlyingT = typename std::make_unsigned_t<
        typename std::underlying_type_t< EnumT > >;
...
public:
    ...
    [[nodiscard]] IC bool all() const noexcept { return bits_.all(); }

    [[nodiscard]] IC bool any() const noexcept { return bits_.any(); }

    [[nodiscard]] IC bool none() const noexcept { return bits_.none(); }
    ...
    IC xr_bitsetflags& zero() noexcept { return reset(); }
    ...
    IC xr_bitsetflags& one() noexcept { return set(); }
    ...
    IC xr_bitsetflags& invert() noexcept { return flip(); }
