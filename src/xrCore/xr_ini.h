...
public:
    struct XRCORE_API Item {
        ...
        // depth determines load order of DLTX overrides, lower depth is more
        // important depth order: DLTX mod_file -> its includes -> Base file ->
        // its includes
        int depth;

        // Insertion index will determine what kv pair in overrides will win
        // even if the depth is the same
        u32 insertionIndex;
    ...
    static IC BOOL IsBOOL( LPCSTR B ) {
        return ( xr_strcmp( B, "on" ) == 0 || xr_strcmp( B, "yes" ) == 0 ||
                 xr_strcmp( B, "true" ) == 0 || xr_strcmp( B, "1" ) == 0 );
    }
    ...
    static void GetCacheStats( u64& files_cached,
        ...
        for ( const auto& file_pair : CachedData ) {
            ...
            for ( const auto& sect_pair : file_pair.second ) {
                ...
                total_bytes +=
                    sizeof( sect_pair.Name ) + sizeof( sect_pair.Data );
                ...
                for ( const auto& d : sect_pair.Data ) {
                    ...
                    total_bytes += sizeof( d.depth ) +
                                   sizeof( d.insertionIndex ) +
                                   sizeof( d.first ) + sizeof( d.second ) +
                                   sizeof( d.filename );
                }
            }
        }
...
private:
    ...
    xr_unordered_flat_map< shared_str, xr_unordered_flat_set< shared_str > >
        OverrideToFilename;
    ...
    struct EvaluationsContext {
        ...
        bool IsInStack( const shared_str& section ) const {
            return std::find( RecursionStack.begin(), RecursionStack.end(),
                              section ) != RecursionStack.end();
        }
...
private:
    void loadFile( const string_path _fn,
    ...
    void StashCurrentSection( Sect*& CurrentBase,
                              Sect*& CurrentOverride,
                              string_path currentFileName );
    Items EvaluateSection( shared_str SectionName,
                           EvaluationsContext& Evaluations,
                           string_path currentFileName );
    Items MergeSections( const Items& BaseItems,
...
public:
    ...
    void w_ivector2( LPCSTR S,
                     LPCSTR L,
                     const Ivector2& V,
                     LPCSTR comment = 0 );
    void w_ivector3( LPCSTR S,
                     LPCSTR L,
                     const Ivector3& V,
                     LPCSTR comment = 0 );
    void w_ivector4( LPCSTR S,
                     LPCSTR L,
                     const Ivector4& V,
                     LPCSTR comment = 0 );
    void w_fvector2( LPCSTR S,
                     LPCSTR L,
                     const Fvector2& V,
                     LPCSTR comment = 0 );
    void w_fvector3( LPCSTR S,
                     LPCSTR L,
                     const Fvector3& V,
                     LPCSTR comment = 0 );
    void w_fvector4( LPCSTR S,
                     LPCSTR L,
                     const Fvector4& V,
                     LPCSTR comment = 0 );
