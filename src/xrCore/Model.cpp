...
inline TMP_TYPE CLAMP( const TMP_TYPE& X,
                       const TMP_TYPE& LoX,
                       const TMP_TYPE& HiX ) {
...
PPM_CONTEXT::write( int o, FILE* fp )
{
    ...
    for (p=Stats+1;p <= Stats+NumStats;p++) {
        if (p[0].Freq > p[-1].Freq) {
            STATE* p1=p;
            do { SWAP(p1[0],p1[-1]); } while (--p1 != Stats && p1[0].Freq >
p1[-1].Freq);
        }
        if (p[0].Freq == p[-1].Freq && p[0].Successor && !p[-1].Successor) {
            STATE* p1=p;
            do { SWAP(p1[0],p1[-1]); } while (--p1 != Stats && p1[0].Freq ==
p1[-1].Freq && !p1[-1].Successor);
        }
    }
...
static void RestoreModelRare( PPM_CONTEXT* pc1,
    ...
    for ( pc = MaxContext, pText = HeapStart; pc != pc1; pc = pc->Suffix )
        if ( --( pc->NumStats ) == 0 ) {
            pc->Flags =
                ( pc->Flags & 0x10 ) + 0x08 * ( pc->Stats->Symbol >= 0x40 );
            p = pc->Stats;
    ...
    if ( MRMethod > MRM_FREEZE ) {
        ...
        GlueCount = 0;
        OrderFall = MaxOrder;
    } else if ( MRMethod == MRM_RESTART ||
                GetUsedMemory() < ( SubAllocatorSize >> 1 ) ) {
...
static inline void UpdateModel( PPM_CONTEXT* MinContext ) {
    ...
    *pText++ = FSymbol;
    ...
    for ( Flag = 0x08 * ( FSymbol >= 0x40 ); pc1 != MinContext;
          pc1 = pc1->Suffix ) {
        if ( ( ns1 = pc1->NumStats ) != 0 ) {
            if ( ( ns1 & 1 ) != 0 ) {
                p = ( PPM_CONTEXT::STATE* )ExpandUnits( pc1->Stats,
                                                        ( ns1 + 1 ) >> 1 );
                if ( !p )
                    goto RESTART_MODEL;
                pc1->Stats = p;
...
inline void PPM_CONTEXT::encodeBinSymbol( int symbol ) {
    ...
    STATE& rs = oneState();
    WORD& bs = BinSumm[ QTable[ rs.Freq - 1 ] ]
                      [ indx + ( ( RunLength >> 26 ) & 0x20 ) ];
...
inline SEE2_CONTEXT* PPM_CONTEXT::makeEscFreq2() const {
    ...
    if ( NumStats != 0xFF ) {
        t = Suffix->NumStats;
        psee2c = SEE2Cont[ QTable[ NumStats + 2 ] - 3 ] +
...
static void _STDCALL StartModelRare( int MaxOrder, MR_METHOD MRMethod ) {
    ...
    if ( first_time ) {
        ...
        {
            OrderFall = ::MaxOrder;
            for ( PPM_CONTEXT* pc = MaxContext; pc && pc->Suffix;
                  pc = pc->Suffix )
                OrderFall--;
            return;
        }
        ...
        static const WORD InitBinEsc[] = { 0x3CDD, 0x1F3F, 0x59BF, 0x48F3,
        ...
        /*
                if( !trained_model || _PPMD_E_GETC(trained_model) > MaxOrder )
                {
                    ...
                    {
                        MaxContext->Stats[i].Symbol=i;
           MaxContext->Stats[i].Freq=1; MaxContext->Stats[i].Successor=NULL;
                    }
                }
                else
                {
