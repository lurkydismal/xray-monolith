...
void register_file_mapping( void* address, const u32& size, LPCSTR file_name ) {
    ...
    g_file_mappings.insert( std::make_pair(
        *( u32* )&address, std::make_pair( size, shared_str( file_name ) ) ) );
...
void unregister_file_mapping( void* address, const u32& size ) {
    ...
    // Msg ("--unregister_file_mapping(%2d): [0x%08x]%s", g_file_mapped_count +
    // 1, *((u32*)&address), (*I).second.second.c_str());
...
XRCORE_API void dump_file_mappings() {
    ...
    for ( ; I != E; ++I )
        Msg( "* [0x%08x][%d][%s]", ( *I ).first, ( *I ).second.first,
...
static errno_t open_internal( LPCSTR fn, int& handle ) {
    return (
        _sopen_s( &handle, fn, _O_RDONLY | _O_BINARY, _SH_DENYNO, _S_IREAD ) );
}
...
void FileCompress( const char* fn, const char* sign, void* data, u32 size ) {
    ...
    int H =
        open( fn, O_BINARY | O_CREAT | O_WRONLY | O_TRUNC, S_IREAD | S_IWRITE );
    R_ASSERT2( H > 0, fn );
    _write( H, &M, 8 );
...
void* FileDecompress( const char* fn, const char* sign, u32* size ) {
    ...
    if ( strncmp( M, F, 8 ) != 0 ) {
        F[ 8 ] = 0;
        Msg( "FATAL: signatures doesn't match, file(%s) / requested(%s)", F,
             sign );
    }
...
#ifdef DEBUG
    BOOL bShow = FALSE;
    if ( fast_dynamic_cast< CFileReader* >( this ) )
        bShow = TRUE;
    if ( fast_dynamic_cast< CVirtualFileReader* >( this ) )
        bShow = TRUE;
    if ( bShow ) {
        FS.dwOpenCounter++;
    }
...
CVirtualFileRW::CVirtualFileRW( const char* cFileName ) {
    // Open the file
    hSrcFile = CreateFile( cFileName, GENERIC_READ | GENERIC_WRITE,
                           FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0 );
    R_ASSERT3( hSrcFile != INVALID_HANDLE_VALUE, cFileName,
    ...
    R_ASSERT3( hSrcMap != INVALID_HANDLE_VALUE, cFileName,
               Debug.error2string( GetLastError() ) );
...
CVirtualFileReader::CVirtualFileReader( const char* cFileName ) {
    // Open the file
    hSrcFile =
        CreateFile( cFileName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
                    0, OPEN_EXISTING, 0, 0 );
    R_ASSERT3( hSrcFile != INVALID_HANDLE_VALUE, cFileName,
               Debug.error2string( GetLastError() ) );
    Size = ( int )GetFileSize( hSrcFile, NULL );
