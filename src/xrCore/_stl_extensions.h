...
public:
    ...
    template < class _Other >
    xalloc< T >& operator=( const xalloc< _Other >& ) {
        return ( *this );
    }
...
inline bool operator==( const xalloc< _Ty >&, const xalloc< _Other >& ) {
    return ( true );
}
...
public:
    ...
    const_reference operator[]( size_type _Pos ) const {
        {
            VERIFY2( _Pos < size(),
                     make_string( "index is out of range: index requested[%d], "
                                  "size of container[%d]",
                                  _Pos, size() )
                         .c_str() );
        }
...
// vector<bool>
template <>
class xr_vector< bool, xalloc< bool > >
    : public std::vector< bool, xalloc< bool > > {
private:
...
template < class T,
           class Hasher = xr_hash< T >,
           class Equal = std::equal_to< T > >
using xr_unordered_set = robin_hood::unordered_node_set< T, Hasher, Equal >;

template < typename K,
           class V,
           class Hasher = xr_hash< K >,
           class Equal = std::equal_to< K > >
using xr_unordered_flat_map =
    robin_hood::unordered_flat_map< K, V, Hasher, Equal >;

template < class T,
           class Hasher = xr_hash< T >,
           class Equal = std::equal_to< T > >
using xr_unordered_flat_set =
    robin_hood::unordered_flat_set< T, Hasher, Equal >;
...
template < typename K,
           ...
           typename allocator = xalloc< std::pair< const K, V > > >
using xr_unordered_flat_map =
    std::unordered_map< K, V, Hasher, Traits, allocator >;
...
template < class T,
           ...
           typename allocator = xalloc< T > >
using xr_unordered_flat_set =
    std::unordered_set< T, Hasher, Traits, allocator >;
...
public:
    ...
    mapped_type& at( const key_type& key ) {
        auto map_it = m_lookup.find( key );
        R_ASSERT3( map_it != m_lookup.end(), "xr_ordered_map, key not found ",
                   key );
...
template < typename Key,
           typename T,
           size_t MaxKeys = 65536,
           size_t INVALID_INDEX = Key( -1 ) >
class xr_sparse_map {
    // Ensure Key is an unsigned integer
    static_assert( std::is_integral< Key >::value,
                   "Key must be an integral type." );
    static_assert(
        std::is_unsigned< Key >::value,
        "Key must be an unsigned type (to avoid negative indexing)." );

    // Ensure Key fits within MaxKeys (if MaxKeys is small)
    static_assert( std::numeric_limits< Key >::max() >= MaxKeys - 1,
                   "Key type is too small for MaxKeys." );
...
public:
    ...
    using const_reverse_iterator =
        typename xr_vector< value_type >::const_reverse_iterator;
...
public:
    xr_sparse_map() : m_sparse( MaxKeys, INVALID_INDEX ) {
        m_dense.reserve( std::min( static_cast< size_type >( MaxKeys ),
                                   static_cast< size_type >( 16384 ) ) );
    }
...
private:
    ...
    sparse_index_t dense_size() const {
        return static_cast< sparse_index_t >( m_dense.size() );
    }
...
public:
    ...
    T& operator[]( const Key& key ) {
        // Calling emplace with no extra arguments perfectly forwards an empty
        // list, resulting in T() being constructed (which acts identically to
        // T{}). It returns a pair; .first gets the iterator, ->second gets the
        // value reference.
...
public:
    ...
    iterator erase( iterator pos ) {
        ...
        sparse_index_t idx = static_cast< sparse_index_t >(
            std::distance( m_dense.begin(), pos ) );
        erase_at_index( idx );
    ...
    size_t count( Key key ) const { return contains( key ) ? 1 : 0; }

    T& at( Key key ) {
        check_bounds( key );
        sparse_index_t idx = m_sparse[ key ];
        R_ASSERT3( idx != INVALID_INDEX, "xr_sparse_map: Item not found by ID ",
                   key );
...
template < typename V,
           ...
           class _EqualKey = std::equal_to< V >,
           typename allocator = xalloc< V > >
class xr_hash_multiset
    : public std::hash_multiset< V, _HashFcn, _EqualKey, allocator > {
public:
    u32 size() const { return ( u32 ) __super::size(); }
};
...
class xr_hash_map
    : public std::hash_map< K, V, _HashFcn, _EqualKey, allocator > {
public:
    u32 size() const { return ( u32 ) __super::size(); }
};
template < typename K,
           ...
           class _EqualKey = std::equal_to< K >,
           typename allocator = xalloc< std::pair< K, V > > >
class xr_hash_multimap
    : public std::hash_multimap< K, V, _HashFcn, _EqualKey, allocator > {
public:
    u32 size() const { return ( u32 ) __super::size(); }
};
#else
...
struct pred_str {
    IC bool operator()( const char* x, const char* y ) const noexcept {
