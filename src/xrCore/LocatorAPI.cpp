...
struct CLocatorAPI::ArchiveDataView {
    ...
    ArchiveDataView( void* mapped_base, u32 mapped_size, LPCSTR name )
        : base( mapped_base ), size( mapped_size ) {
#ifdef FS_DEBUG
...
public:
    CArchiveReader( xr_shared_ptr< CLocatorAPI::ArchiveDataView > view,
                    void* data,
                    int size )
...
public:
    explicit CStartupLooseReader( xr_shared_ptr< const xr_vector< u8 > > data )
        : IReader( const_cast< u8* >( data->data() ),
...
struct InitialNameHash {
    size_t operator()( const xr_string& value ) const {
        size_t result = sizeof( size_t ) == 8
                            ? size_t( 14695981039346656037ull )
                            : size_t( 2166136261u );
...
struct CLocatorAPI::StartupLooseCache {
    enum class State : u8 { Queued, Loading, Ready, Failed };
    ...
    static xr_shared_ptr< const xr_vector< u8 > > Read( const Entry& entry ) {
        ...
        if ( !GetFileInformationByHandle( file, &before ) ||
             before.nFileSizeHigh || before.nFileSizeLow != entry.size ) {
            CloseHandle( file );
            return {};
        }
        ...
        while ( total_read < entry.size ) {
            DWORD bytes_read = 0;
            if ( !ReadFile( file, buffer->data() + total_read,
                            entry.size - total_read, &bytes_read, nullptr ) ||
                 !bytes_read )
                break;
            total_read += bytes_read;
        }
        BY_HANDLE_FILE_INFORMATION after;
        const bool unchanged = GetFileInformationByHandle( file, &after ) &&
                               !after.nFileSizeHigh &&
    ...
    void Worker() {
        ...
        if ( workers_remaining.fetch_sub( 1 ) == 1 ) {
            Msg( "* [STARTUP/VFS] loose-cache prepared=%u failed=%u "
                 "promoted=%u demand-wait=%llu ms",
                 prepared.load(), failed.load(), promoted.load(),
                 demand_wait_ms.load() );
        }
    }
};
...
void _check_open_file( const shared_str& _fname ) {
    xr_vector< _open_file >::iterator it = std::find_if(
        g_open_files.begin(), g_open_files.end(), eq_fname_check( _fname ) );
...
_open_file& find_free_item( const shared_str& _fname ) {
    xr_vector< _open_file >::iterator it = std::find_if(
        g_open_files.begin(), g_open_files.end(), eq_fname_free( _fname ) );
    if ( it == g_open_files.end() ) {
...
void _unregister_open_file( T* _r ) {
    ...
    xr_vector< _open_file >::iterator it = std::find_if(
        g_open_files.begin(), g_open_files.end(), eq_pointer< T >( _r ) );
...
void CLocatorAPI::Register( LPCSTR name,
                            u32 vfs,
                            u32 crc,
                            u32 ptr,
                            u32 size_real,
...
void CLocatorAPI::CommitInitialFiles( u64 scan_started_at ) {
    using InitialFileMap =
        xr_unordered_map< xr_string, InitialFileData, InitialNameHash >;
    ...
    for ( const InitialFileRecord& record : m_initial_files ) {
        InitialFileData data = { record.vfs,
                                 record.crc,
                                 record.ptr,
                                 record.size_real,
    ...
    std::sort( sorted.begin(), sorted.end(),
               []( const InitialFileMapEntry* left,
                   const InitialFileMapEntry* right ) {
                   return xr_strcmp( left->first.c_str(),
                                     right->first.c_str() ) < 0;
               } );
    ...
    for ( const InitialFileMapEntry* entry : sorted ) {
        ...
        file desc = { xr_strdup( entry->first.c_str() ),
                      source.vfs,
                      source.crc,
                      source.ptr,
                      source.size_real,
                      source.size_compressed,
                      source.modif };
    ...
    for ( const file& entry : m_files ) {
        catalog_hash =
            crc32( entry.name, xr_strlen( entry.name ) + 1, catalog_hash );
        ...
        catalog_hash =
            crc32( &entry.size_real, sizeof( entry.size_real ), catalog_hash );
        catalog_hash = crc32( &entry.size_compressed,
                              sizeof( entry.size_compressed ), catalog_hash );
        catalog_hash =
            crc32( &entry.modif, sizeof( entry.modif ), catalog_hash );
    }
    ...
    const u64 discovery_ms = scan_total_ms > m_initial_archive_index_ms
                                 ...
                                 : 0;
    Msg( "* [STARTUP/VFS] discovery=%llu ms index=%llu ms replay=%llu ms "
         "sort=%llu ms commit=%llu ms files=%u replacements=%u catalog=%08x",
         discovery_ms, m_initial_archive_index_ms, replay_ms, sort_ms,
         commit_ms, m_files.size(), replacements, catalog_hash );
...
void CLocatorAPI::StartStartupLooseCache() {
    ...
    for ( LPCSTR alias :
          { "$game_config$", "$game_scripts$", "$game_textures$" } ) {
        ...
        for ( const file& desc : m_files ) {
            if ( desc.vfs != u32( -1 ) || !desc.size_real ||
                 strncmp( desc.name, prefix.c_str(), prefix.size() ) != 0 )
                continue;
            if ( texture_thm ) {
...
bool CLocatorAPI::OpenStartupLooseCache( IReader*& reader,
                                         LPCSTR name,
                                         const file& desc ) {
    StartupLooseCache* cache = m_startup_loose_cache;
    if ( !cache )
    ...
    bool load = false;
    {
        ...
        if ( cache->stopping || found == cache->entries.end() )
            return false;
        entry = found->second;
        if ( !entry->valid || entry->size != desc.size_real ||
             entry->modif != desc.modif )
            return false;
...
void CLocatorAPI::LoadArchive( archive& A, LPCSTR entrypoint ) {
    ...
    if ( A.header ) {
        ...
        if ( 0 == stricmp( read_path.c_str(), "gamedata" ) ) {
            ...
            if ( P != pathes.end() ) {
                ...
                xr_strcpy( fs_entry_point, sizeof( fs_entry_point ),
                           root->m_Path );
            }
            ...
            xr_strcat( fs_entry_point, sizeof( fs_entry_point ),
                       read_path.c_str() + xr_strlen( alias_name ) + 1 );
        }
    } else {
    ...
    hdr->close();
    if ( m_initial_build )
        m_initial_archive_index_ms +=
            GetTickCount64() - initial_index_started_at;
...
xr_shared_ptr< CLocatorAPI::ArchiveDataView > CLocatorAPI::GetArchiveDataView(
    archive& A ) {
    if ( xr_shared_ptr< ArchiveDataView > view =
             std::atomic_load( &A.data_view ) )
        return view;
    ...
    if ( !base ) {
        ...
        Msg( "! [STARTUP/VFS] full archive mapping failed, using per-file "
             "views: %s (%s)",
             A.path.c_str(), Debug.error2string( GetLastError() ) );
        return {};
    }

    xr_shared_ptr< ArchiveDataView > view =
        xr_make_shared< ArchiveDataView >( base, A.size, A.path.c_str() );
...
void CLocatorAPI::ProcessArchive( LPCSTR _path ) {
    ...
    if ( bProcessArchiveLoading ||
         Core.ParamsData.test( ECoreParams::auto_load_arch ) )
        LoadArchive( A );
    else
        A.close();
}
...
void CLocatorAPI::ProcessOne( LPCSTR path,
                              const _finddata_t& entry,
                              u32 parallel_depth ) {
    string_path N;
    ...
    if ( entry.attrib & _A_SUBDIR ) {
        ...
        if ( 0 == xr_strcmp( entry.name, ".." ) )
            return;
        xr_strcat( N, "\\" );
        Register( N, 0xffffffff, 0, 0, entry.size, entry.size,
                  ( u32 )entry.time_write );
        Recurse( N, parallel_depth ? parallel_depth - 1 : 0 );
    } else {
        if ( strext( N ) && ( 0 == strncmp( strext( N ), ".db", 3 ) ||
                              0 == strncmp( strext( N ), ".xdb", 4 ) ) )
            ProcessArchive( N );
        else
            Register( N, 0xffffffff, 0, 0, entry.size, entry.size,
                      ( u32 )entry.time_write );
...
bool ignore_name( const char* _name ) {
    ...
    return ( _name[ 0 ] == '.' && _name[ 1 ] == 's' && _name[ 2 ] == 'v' &&
             _name[ 3 ] == 'n' && _name[ 4 ] == 0 );
}
...
bool ignore_path( const char* _path ) {
    HANDLE h =
        CreateFile( _path, 0, 0, NULL, OPEN_EXISTING,
    ...
    if ( !files.empty() ) {
        ...
        if ( parallel_depth && !m_initial_build )
            xr_parallel_foreach(
                ...
                [ this, path, parallel_depth ]( const _finddata_t& entry ) {
...
static void searchForFsltx( const char* fs_name, string_path& fsltxPath ) {
    // #TODO: Update code, when std::filesystem is out (not much work, standards
    ...
    if ( tryPathFunc( "../", fsltxPath ) )
        return;

    // same for application path (fix for launching the game from discord)
    if ( tryPathFunc( Core.ApplicationPath, fsltxPath ) )
        return;
    ...
    if ( tryPathFunc( test_path, fsltxPath ) )
        return;
}
...
void CLocatorAPI::_initialize( u32 flags,
    ...
    if ( m_Flags.is( flTargetFolderOnly ) ) {
        ...
        while ( !pFSltx->eof() ) {
            ...
            FS_Path* P = new FS_Path(
                ( p_it != pathes.end() ) ? p_it->second->m_Path : root, lp_add,
                lp_def, lp_capt, fl );
            bNoRecurse = !( fl & FS_Path::flRecurse );
            bool already_scanned = false;
            for ( const xr_string& scanned_root : recursive_roots ) {
                if ( !_strnicmp( P->m_Path, scanned_root.c_str(),
                                 scanned_root.size() ) ) {
            ...
            // CHECK_OR_EXIT		(I.second,"The file 'fsgame.ltx' is
            // corrupted (it contains duplicated lines).\nPlease reinstall the
            // game or fix the problem manually.");
        }
    ...
    Msg( "FS: %d files cached %d archives, %lldKb memory used.", m_files.size(),
         m_archives.size(), ( M2 - M1 ) / 1024 );
    ...
    if ( Core.ParamsData.test( ECoreParams::overlaypath ) ) {
        ...
        sscanf( strstr( Core.Params, "-overlaypath " ) + 13, "%[^ ] ",
                c_newAppPathRoot );
        ...
        if ( pAppdataPath ) {
            ...
            rescan_path( pAppdataPath->m_Path,
                         pAppdataPath->m_Flags.is( FS_Path::flRecurse ) );
        }
    }
...
void CLocatorAPI::_destroy() {
    ...
    pathes.clear();
    for ( archives_it a_it = m_archives.begin(); a_it != m_archives.end();
          a_it++ ) {
...
const CLocatorAPI::file* CLocatorAPI::exist( const char* path,
                                             const char* name ) {
    string_path temp;
...
const CLocatorAPI::file* CLocatorAPI::exist( string_path& fn,
                                             LPCSTR path,
                                             LPCSTR name ) {
...
xr_vector< char* >* CLocatorAPI::file_list_open( const char* initial,
                                                 const char* folder,
                                                 u32 flags ) {
    string_path N;
...
xr_vector< char* >* CLocatorAPI::file_list_open( const char* _path,
                                                 u32 flags ) {
    R_ASSERT( _path );
    VERIFY( flags );
    ...
    for ( ++I; I != m_files.end(); I++ ) {
        ...
        if ( ( *end_symbol ) != '\\' ) {
            ...
            if ( ( flags & FS_RootOnly ) &&
                 ( strstr( entry_begin, "\\" ) != end_symbol ) )
                continue; // folder in folder
...
void CLocatorAPI::file_list_close( xr_vector< char* >*& lst ) {
    if ( lst ) {
        for ( xr_vector< char* >::iterator I = lst->begin(); I != lst->end();
              I++ )
            xr_free( *I );
        xr_delete( lst );
    }
...
int CLocatorAPI::file_list( FS_FileSet& dest,
                            LPCSTR path,
                            u32 flags,
                            LPCSTR mask ) {
    R_ASSERT( path );
    ...
    for ( ++I; I != m_files.end(); ++I ) {
        ...
        if ( ( *end_symbol ) != '\\' ) {
            ...
            if ( b_mask ) {
                bool bOK = false;
                for ( SStringVecIt it = masks.begin(); it != masks.end();
                      it++ ) {
            ...
            dest.insert(
                FS_File( entry_begin, entry.size_real, entry.modif, fl ) );
        }
    }
    return dest.size();
}

void CLocatorAPI::check_cached_files( LPSTR fname,
                                      const u32& fname_size,
                                      const file& desc,
                                      LPCSTR& source_name ) {
    ...
    if ( fit != m_files.end() ) {
        // use
        const file& fc = *fit;
        if ( ( fc.size_real == desc.size_real ) &&
             ( fc.modif == desc.modif ) ) {
            // use
        } else {
            // copy & use
            Msg( "copy: db[%X],cache[%X] - '%s', ", desc.modif, fc.modif,
                 fname );
            bCopy = TRUE;
        }
    } else {
    ...
    if ( bCopy ) {
        ...
        Register( fname_in_cache, 0xffffffff, 0, 0, desc.size_real,
                  desc.size_real, desc.modif );
    }
...
void CLocatorAPI::file_from_cache_impl( IReader*& R,
                                        LPSTR fname,
                                        const file& desc ) {
...
void CLocatorAPI::file_from_cache_impl( CStreamReader*& R,
                                        LPSTR fname,
                                        const file& desc ) {
...
template < typename T >
void CLocatorAPI::file_from_cache( T*& R,
                                   LPSTR fname,
                                   const u32& fname_size,
                                   const file& desc,
                                   LPCSTR& source_name ) {
...
void CLocatorAPI::file_from_archive( IReader*& R,
                                     LPCSTR fname,
                                     const file& desc ) {
    // Archived one
    archive& A = m_archives[ desc.vfs ];
    if ( const xr_shared_ptr< ArchiveDataView > view =
             GetArchiveDataView( A ) ) {
        R_ASSERT3( u64( desc.ptr ) + desc.size_compressed <= view->size,
                   "archive entry is outside mapping", fname );
    ...
    // Compressed
    u8* dest = xr_alloc< u8 >( desc.size_real );
    rtc_decompress( dest, desc.size_real, ptr + ptr_offs,
...
void CLocatorAPI::file_from_archive( CStreamReader*& R,
                                     LPCSTR fname,
                                     const file& desc ) {
    archive& A = m_archives[ desc.vfs ];
    R_ASSERT2( desc.size_compressed == desc.size_real,
               make_string( "cannot use stream reading for compressed data %s, "
                            "do not compress data to be streamed",
                            fname ) );

    R = xr_new< CStreamReader >();
    R->construct( A.hSrcMap, desc.ptr, desc.size_compressed, A.size,
...
void CLocatorAPI::copy_file_to_build( T*& r, LPCSTR source_name ) {
    ...
    FS_Path* P;
    // if
    // (!(source_name==strstr(source_name,(P=get_path("$server_root$"))->m_Path)||
    ...
    if ( position == source_name )
        update_path( cpy_name, "$build_copy$",
                     source_name + xr_strlen( fs_root ) );
    else
    ...
    if ( 0 == xr_strcmp( ext, ".dds" ) ) {
        ...
        update_path( e_cpy_name, "$textures$",
                     source_name + xr_strlen( P->m_Path ) );
        // tga
        *strext( e_cpy_name ) = 0;
...
bool CLocatorAPI::check_for_file( LPCSTR path,
                                  LPCSTR _fname,
                                  string_path& fname,
                                  const file*& desc ) {
    xrCriticalSectionGuard guard( m_scan_lock );
...
void CLocatorAPI::w_close( IWriter*& S ) {
    if ( S ) {
        ...
        if ( bReg ) {
            struct _stat st;
            _stat( fname, &st );
            Register( fname, 0xffffffff, 0, 0, st.st_size, st.st_size,
                      ( u32 )st.st_mtime );
        }
    }
}
...
FS_Path* CLocatorAPI::append_path( LPCSTR path_alias,
                                   LPCSTR root,
                                   LPCSTR add,
                                   BOOL recursive ) {
    VERIFY( root /**&&root[0]/**/ );
...
LPCSTR CLocatorAPI::update_path( string_path& dest,
                                 LPCSTR initial,
                                 LPCSTR src ) {
...
void CLocatorAPI::set_file_age( LPCSTR nm, u32 age ) {
    ...
    if ( 0 != res ) {
        Msg( "!Can't set file age: '%s'. Error: '%s'", nm,
             _sys_errlist[ errno ] );
    } else {
        // update record
        files_it I = file_find_it( nm );
...
BOOL CLocatorAPI::can_write_to_folder( LPCSTR path ) {
    if ( path && path[ 0 ] ) {
        string_path temp;
        LPCSTR fn = "$!#%TEMP%#!$.$$$";
        strconcat( sizeof( temp ), temp, path,
                   path[ xr_strlen( path ) - 1 ] != '\\' ? "\\" : "", fn );
