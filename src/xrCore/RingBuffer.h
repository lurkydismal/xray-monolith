...
public:
    ...
    RingBuffer() {
        ...
        // otherwise, the default constructor will be called, clearing all
        // without us
    }
    ...
    unsigned int GetHead() const { return PosHead; }
    ...
    const StoredType& Get( unsigned int Pos ) const { return Buffer[ Pos ]; }

    const StoredType& GetLooped( unsigned int Pos ) const {
        return Buffer[ Pos % GetSize() ];
    }

    StoredType& Get( unsigned int Pos ) { return Buffer[ Pos ]; }
    ...
    // write elements to storage, but don't move head pointer. So after this the
    // head will point to first copied element
    bool WriteFromHeadNoMove( const StoredType* pElems,
                              unsigned int ElemsCount ) {
        if ( ElemsCount >= BufferSize )
            return false;

        // don't use Push. Push works great for single item, for thing like
        // history and etc. But for large arrays it's better have classic
        // forward (like) data chain.
        unsigned int RemainElems = ElemsCount;

        if ( int FirstChunkSize = BufferSize - GetHead() ) {
            unsigned int CopySize =
                FirstChunkSize >= ElemsCount ? ElemsCount : FirstChunkSize;
            memcpy( &Buffer[ PosHead ], pElems,
                    CopySize * sizeof( StoredType ) );
            RemainElems =
                FirstChunkSize >= ElemsCount ? 0 : ElemsCount - FirstChunkSize;
        }

        if ( RemainElems > 0 ) {
            memcpy( &Buffer[ 0 ], pElems + ( ElemsCount - RemainElems ),
                    RemainElems * sizeof( StoredType ) );
        }
