...
{
    effect_exception tmp{AL_INVALID_ENUM, "Invalid dedicated integer property 0x%04x", param};
    std::terminate();
}
void Dedicated_setParamiv(EffectProps*, ALenum param, const int*)
{
    effect_exception tmp{AL_INVALID_ENUM, "Invalid dedicated integer-vector property 0x%04x", param};
    std::terminate();
}
void Dedicated_setParamf(EffectProps *props, ALenum param, float val)
{
    ...
    case AL_DEDICATED_GAIN:
        ...
        {
            effect_exception tmp{AL_INVALID_VALUE, "Dedicated gain out of range"};
            std::terminate();
        }
    ...
    default:
        effect_exception tmp{AL_INVALID_ENUM, "Invalid dedicated float property 0x%04x", param};
        std::terminate();
    }
}
