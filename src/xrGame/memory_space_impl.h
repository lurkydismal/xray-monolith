...
IC void CObjectParams< T >::fill( const T* game_object ) {
    ...
    m_orientation =
        game_object ? orientation( game_object ) : SRotation( 0.f, 0.f, 0.f );
#endif

    m_level_vertex_id =
        game_object ? game_object->ai_location().level_vertex_id() : u32( -1 );
    //	if (game_object && ai().get_level_graph() &&
    //ai().level_graph().valid_vertex_id(m_level_vertex_id) &&
    //!ai().level_graph().inside(m_level_vertex_id,game_object->Position())) {
    //		m_position			=
    //ai().level_graph().vertex_position(m_level_vertex_id); 		m_position.y
    //= game_object->Position().y; 		return;
    //	}

    if ( game_object ) {
        game_object->Center( m_position );
        m_position.set( game_object->Position().x, m_position.y,
...
template < typename T >
IC void CMemoryObject< T >::fill( const T* game_object,
                                  const T* self,
