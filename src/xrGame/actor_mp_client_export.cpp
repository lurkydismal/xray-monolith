...
// 3     health is form 0.f to 1.f, so use only 8 bits for it, the rest one we
// will use for mask 1     removed flags 12    removed logical position 12
// quantized camera stuff and model yaw 3     removed team, squad, group 0.125
// body state flags 6     removed logical acceleration 6     removed logical
// velocity 3.5   quantized radiation 0.5   quantized active_slot 2     dead
// actors are not synchronized at all, number of synchronization items is no
// more 0.875 physics state enabled 12    removed angular velocity 9 quantized
// linear velocity 12    removed force 12    removed torque 16    removed
// quaternion

///	TODO
// 4   u32 move time to global packet (+ much less QPC calls)
// 12  3*float write unaffected_r_torso stuff only in case when greande is an
// active slot (?) X   mask all the changes we sent
...
// 3  camera
// 4
// inventory_active_slot,body_state_flags,health,radiation,physics_state_enabled

void CActorMP::fill_state( actor_mp_state& state ) {
    if ( OnClient() ) {
        // R_ASSERT						(g_Alive());
        // R_ASSERT2
        // (PHGetSyncItemsNumber() == 1,make_string("PHGetSyncItemsNumber()
        // returned %d, health = %.2f",PHGetSyncItemsNumber(),GetfHealth()));
    }
...
void CActorMP::net_Export( NET_Packet& packet ) {
    if ( OnClient() ) {
        // Msg("net_Export: ID is: 0x%08x, is going to send health %2.04f",
        // this->ID(), m_state_holder.state().health); R_ASSERT
        // (g_Alive()); R_ASSERT
        // (PHGetSyncItemsNumber() == 1);
    }
    R_ASSERT2( valid_pos( m_state_holder.state().position ),
               "trying to export bad position" );
