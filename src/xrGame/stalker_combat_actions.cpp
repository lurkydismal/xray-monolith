...
CStalkerActionGetItemToKill::CStalkerActionGetItemToKill( CAI_Stalker* object,
                                                          LPCSTR action_name )
...
void CStalkerActionGetItemToKill::initialize() {
    ...
    object().sight().setup(
        CSightAction( object().m_best_found_item_to_kill
                          ? &object().m_best_found_item_to_kill->object()
                          : 0,
                      true ) );
...
void CStalkerActionGetItemToKill::finalize() {
    ...
    object().sight().clear();
    object().sight().setup(
        CSightAction( SightManager::eSightTypePathDirection, false ) );
...
void CStalkerActionGetItemToKill::execute() {
    ...
    object().movement().set_level_dest_vertex(
        object()
            .m_best_found_item_to_kill->object()
            .ai_location()
            .level_vertex_id() );
    object().movement().set_desired_position(
        &object().m_best_found_item_to_kill->object().Position() );
    ...
    EBodyState body_state =
        ( object().movement().body_state() == eBodyStateCrouch )
            ? eBodyStateCrouch
            : eBodyStateStand;
    object().movement().set_body_state(
        object().movement().body_state_combat_override( wo, body_state ) );
#else
    object().movement().set_body_state( object().movement().body_state() ==
                                                eBodyStateCrouch
                                            ? eBodyStateCrouch
                                            : eBodyStateStand );
#endif
...
CStalkerActionMakeItemKilling::CStalkerActionMakeItemKilling(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionMakeItemKilling::execute() {
    ...
    object().movement().set_level_dest_vertex(
        object().m_best_found_ammo->object().ai_location().level_vertex_id() );
    object().movement().set_desired_position(
        &object().m_best_found_ammo->object().Position() );
    object().movement().set_desired_direction( 0 );
    object().movement().set_path_type( MovementManager::ePathTypeLevelPath );
    object().movement().set_detail_path_type(
        DetailPathManager::eDetailPathTypeSmooth );
    ...
    object()
        ...
        .set_vector3d( object().memory().enemy().selected()->Position() );
    object().set_goal( eObjectActionIdle );
}
...
void CStalkerActionRetreatFromEnemy::execute() {
    ...
    if ( mem_object.m_object ) {
        object().m_ce_far->setup( mem_object.m_object_params.m_position, 0.f,
                                  300.f );
        point = ai().cover_manager().best_cover(
            object().Position(), 30.f, *object().m_ce_far,
            CStalkerMovementRestrictor( m_object, true ) );
        if ( !point ) {
            object().m_ce_far->setup( mem_object.m_object_params.m_position,
                                      0.f, 300.f );
            point = ai().cover_manager().best_cover(
                object().Position(), 50.f, *object().m_ce_far,
                CStalkerMovementRestrictor( m_object, true ) );
        }
    }
    ...
    if ( point ) {
        ...
        if ( object().memory().visual().visible_now(
...
#endif
    //	object().movement().set_movement_type
    //(eMovementTypeRun); 	object().sight().setup
    //(CSightAction(SightManager::eSightTypePathDirection));
...
void CStalkerActionGetReadyToKill::execute() {
    ...
    EWorldOperators wo = eWorldOperatorGetReadyToKill;
    EBodyState body_state = eBodyStateStand;
    if ( object().movement().detail().distance_to_target() >
         g_ai_close_move_distance )
        object().movement().set_body_state(
            object().movement().body_state_combat_override( wo, body_state ) );
#else
    if ( object().movement().detail().distance_to_target() >
    ...
    //	if
    //(object().memory().visual().visible_now(object().memory().enemy().selected()))
    //		object().sight().setup
    //(CSightAction(object().memory().enemy().selected(),true)); 	else
    //		object().sight().setup
...
CStalkerActionKillEnemy::CStalkerActionKillEnemy( CAI_Stalker* object,
                                                  LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionKillEnemy::initialize() {
    ...
    object().movement().set_mental_state( eMentalStateDanger );
    //	object().movement().set_body_state
    //(m_storage->property(eWorldPropertyUseCrouchToLookOut) ? eBodyStateCrouch
    //: eBodyStateStand);
    object().movement().set_movement_type( eMovementTypeStand );
    m_storage->set_property( eWorldPropertyLookedOut, false );
    m_storage->set_property( eWorldPropertyPositionHolded, false );
...
void CStalkerActionKillEnemy::execute() {
    ...
    // Alundaio: Prevent Stalkers from shooting at walls for prolonged periods
    // due to kill if not visible
...
CStalkerActionTakeCover::CStalkerActionTakeCover( CAI_Stalker* object,
                                                  LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionTakeCover::initialize() {
    ...
    m_movement_type =
        g_ai_move_to_cover_run ? eMovementTypeRun : eMovementTypeWalk;
        ...
        if ( object().agent_manager().member().can_cry_noninfo_phrase() )
            if ( object().Position().distance_to_sqr(
                ...
                if ( object().memory().visual().visible_now(
                    ...
                    object().sound().play( eStalkerSoundBackup, 0, 0, 6000,
                                           4000 );
    }
#endif
}
...
void CStalkerActionTakeCover::execute() {
    ...
    if ( point ) {
        ...
        if ( point != m_last_notified_cover ) {
            ...
            if ( ai().script_engine().functor(
                     "_G.CAI_Stalker__OnTakeCoverDestination", funct ) ) {
                const CEntityAlive* enemy =
                    object().memory().enemy().selected();
                funct( object().lua_game_object(), point->position(),
            ...
            for ( auto& it : object().agent_manager().member().members() ) {
                ...
                if ( teammate->memory().enemy().selected() ) {
                    ...
                    u32 target_vertex_id =
                        ai().level_graph().vertex_id( stack_pos );
                    if ( ai().level_graph().valid_vertex_id(
                             target_vertex_id ) &&
                         ai().level_graph().is_accessible(
                             target_vertex_id ) ) {
                        object().movement().set_level_dest_vertex(
                            target_vertex_id );
    ...
    if ( object().memory().visual().visible_now( enemy ) ) {
        ...
        aim_ready();
        // Alundaio: Prevent stalkers from staring at floor or ceiling for this
        // action
        u32 const level_time =
            object().memory().visual().visible_object_time_last_seen(
                mem_object.m_object );
        if ( Device.dwTimeGlobal >= level_time + 3000 &&
             _abs( object().Position().y -
                   mem_object.m_object_params.m_position.y ) > 3.5f ) {
            Fvector3 Vpos = { mem_object.m_object_params.m_position.x,
                              object().Position().y + 1.f,
                              mem_object.m_object_params.m_position.z };
            object().sight().setup(
                CSightAction( SightManager::eSightTypePosition, Vpos, true ) );
        } else
            object().sight().setup(
                CSightAction( SightManager::eSightTypePosition,
                              mem_object.m_object_params.m_position, true ) );
...
CStalkerActionLookOut::CStalkerActionLookOut( CAI_Stalker* object,
                                              LPCSTR action_name )
    : inherited( object, action_name ) {
...
void CStalkerActionLookOut::initialize() {
    ...
    if ( Device.dwTimeGlobal >=
         m_last_change_time + g_ai_crouch_look_out_delta ) {
        m_storage->set_property( eWorldPropertyUseCrouchToLookOut,
                                 !!m_crouch_look_out_random.random( 2 ) );
    ...
    EBodyState body_state =
        m_storage->property( eWorldPropertyUseCrouchToLookOut )
            ? eBodyStateCrouch
            : eBodyStateStand;
    object().movement().set_body_state(
        object().movement().body_state_combat_override( wo, body_state ) );
...
float current_cover( CAI_Stalker* object ) {
    ...
    BOOL result = Level().ObjectSpace.RayPick(
        position, direction, 10.f, collide::rqtStatic, ray_query_result, NULL );
...
void CStalkerActionLookOut::execute() {
    ...
    float dist_to_enemy = object().Position().distance_to(
    ...
    const CCoverPoint* point = ai().cover_manager().best_cover(
        object().Position(), 10.f, *object().m_ce_close );
    //,CStalkerMovementRestrictor(m_object,true,false));
    if ( !point || ( point->position().similar( object().Position() ) &&
                     object().movement().path_completed() ) ) {
        object().m_ce_close->setup( position, 0.f, 170.f, 10.f );
        point = ai().cover_manager().best_cover( object().Position(), 30.f,
                                                 *object().m_ce_close );
        //,CStalkerMovementRestrictor(m_object,true,false));
    }
...
CStalkerActionHoldPosition::CStalkerActionHoldPosition( CAI_Stalker* object,
                                                        LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionHoldPosition::initialize() {
    ...
    set_inertia_time( g_ai_hold_position_inertia_base +
                      ::Random32.random( g_ai_hold_position_inertia_random ) );
    object().brain().affect_cover( true );
}
...
void CStalkerActionHoldPosition::execute() {
    ...
    if ( object().agent_manager().member().cover_detouring() &&
         fire_make_sense() ) {
        //		object().sound().play
        //(eStalkerSoundDetour,3000,3000,10000,10000);
        object().sound().play( eStalkerSoundNeedBackup, 3000, 3000, 10000,
                               10000 );
        fire();
    } else {
        aim_ready();
...
CStalkerActionDetourEnemy::CStalkerActionDetourEnemy( CAI_Stalker* object,
                                                      LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionDetourEnemy::initialize() {
    ...
    if ( /**(Random.randF(1.f) < .8f) && /**/ object()
        ...
        object().agent_manager().location().add( xr_new< CDangerCoverLocation >(
            ...
            Device.dwTimeGlobal, g_ai_cover_detour_time,
...
void CStalkerActionDetourEnemy::execute() {
    ...
    if ( object().movement().path_completed() ) {
        ...
        object().m_ce_angle->setup(
            position, 10.f, object().ffGetRange(),
            mem_object.m_object_params.m_level_vertex_id );
        const CCoverPoint* point = ai().cover_manager().best_cover(
            object().Position(), 10.f, *object().m_ce_angle,
            CStalkerMovementRestrictor( m_object, true ) );
        if ( !point ) {
            object().m_ce_angle->setup(
                position, 10.f, object().ffGetRange(),
                mem_object.m_object_params.m_level_vertex_id );
            point = ai().cover_manager().best_cover(
                object().Position(), 30.f, *object().m_ce_angle,
                CStalkerMovementRestrictor( m_object, true ) );
        }
...
CStalkerActionPostCombatWait::CStalkerActionPostCombatWait( CAI_Stalker* object,
                                                            LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionPostCombatWait::initialize() {
    ...
    object().movement().set_movement_type(
        eMovementTypeRun ); // Alundaio: Changed from walk to run
        ...
        if ( object().inventory().ItemFromSlot( INV_SLOT_2 ) ) {
            ...
            if ( object().inventory().ActiveItem() && temp &&
                 ( object().inventory().ActiveItem()->object().ID() ==
                   temp->ID() ) )
                object().set_goal(
                    action, object().inventory().ItemFromSlot( INV_SLOT_2 ) );
        }
    }
    ...
    if ( object().memory().enemy().last_enemy() &&
         object().memory().visual().visible_now(
             object().memory().enemy().last_enemy() ) )
        object().sight().setup( CSightAction(
            object().memory().enemy().last_enemy(), true, true ) );
}
...
CStalkerActionHideFromGrenade::CStalkerActionHideFromGrenade(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {}
    ...
    if ( !object().memory().enemy().selected() )
        ...
        if ( !mem_object.m_object ) {
            ...
            if ( !m_object->memory().visual().visible_now(
                     object().memory().enemy().selected() ) ) {
                if ( position.distance_to_sqr( object().Position() ) <
                     _sqr( 5.f ) )
                    object().sight().setup( CSightAction(
                        SightManager::eSightTypePathDirection, true, true ) );
                else
...
CStalkerActionSuddenAttack::CStalkerActionSuddenAttack( CAI_Stalker* object,
                                                        LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionSuddenAttack::execute() {
    ...
    // Alundaio: Removed check to allow stalkers to sneak up on enemy even if
    // they are in a group. if
    // (object().agent_manager().member().combat_members().size() > 1)
    //	m_storage->set_property	(eWorldPropertyUseSuddenness,false);
    // Alundaio: END
    ...
    /* 	if (!visible_now)
            {
                    u32 target_vertex_id =
       object().movement().level_path().dest_vertex_id(); if
       (object().ai_location().level_vertex_id() == target_vertex_id) {
                            m_storage->set_property(eWorldPropertyUseSuddenness,
       false); return;
                    }
            } */

    float distance = object().Position().distance_to(
        mem_object.m_object_params.m_position );
    if ( distance >= 15.f ) {
#ifdef COMBAT_BODY_STATE_OVERRIDE
        ...
        if ( distance >= 8.f ) {
            ...
            EBodyState body_state = eBodyStateStand;
            object().movement().set_body_state(
                object().movement().body_state_combat_override( wo,
                                                                body_state ) );
#else
            object().movement().set_body_state( eBodyStateStand );
#endif
            ...
            if ( distance >= 6.f ) {
                ...
                EBodyState body_state = eBodyStateCrouch;
                object().movement().set_body_state(
                    object().movement().body_state_combat_override(
                        wo, body_state ) );
#else
                object().movement().set_body_state( eBodyStateCrouch );
#endif
...
CStalkerActionKillEnemyIfPlayerOnThePath::
    CStalkerActionKillEnemyIfPlayerOnThePath( CAI_Stalker* object,
                                              LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionKillEnemyIfPlayerOnThePath::execute() {
    ...
    object().sight().setup(
        CSightAction( object().memory().enemy().selected(), true, true ) );
...
CStalkerActionCriticalHit::CStalkerActionCriticalHit( CAI_Stalker* object,
                                                      LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionCriticalHit::initialize() {
    ...
    if ( object().inventory().ActiveItem() && object().best_weapon() &&
        ...
        if ( object().memory().enemy().selected() ) {
            u32 min_queue_size, max_queue_size, min_queue_interval,
                max_queue_interval;
            float distance =
                object().memory().enemy().selected()->Position().distance_to(
                    object().Position() );
            ...
            object().CObjectHandler::set_goal(
                eObjectActionIdle, object().best_weapon(), min_queue_size,
                max_queue_size, min_queue_interval, max_queue_interval );
        } else {
            object().CObjectHandler::set_goal( eObjectActionIdle,
                                               object().best_weapon() );
    ...
    object().sight().setup(
        CSightAction( SightManager::eSightTypeCurrentDirection, true, true ) );
...
CStalkerCombatActionThrowGrenade::CStalkerCombatActionThrowGrenade(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerCombatActionThrowGrenade::initialize() {
    ...
    const CInventoryItem* grenade =
        object().inventory().ItemFromSlot( GRENADE_SLOT );
    VERIFY( grenade );
...
void CStalkerCombatActionThrowGrenade::execute() {
    ...
    Fvector const enemy_direction =
        Fvector().sub( enemy_position, object().Position() ).normalize_safe();
    Fvector const head_direction =
        Fvector().setHP( -object().movement().m_head.current.yaw,
    ...
    object().throw_target( enemy_position, enemy_vertex_id,
                           const_cast< CEntityAlive* >( enemy ) );
    ...
    select_queue_params( distance, min_queue_size, max_queue_size,
                         min_queue_interval, max_queue_interval );
    object().CObjectHandler::set_goal( eObjectActionFire1, &grenade->object(),
...
CStalkerCombatActionSmartCover::CStalkerCombatActionSmartCover(
    CAI_Stalker* object,
    LPCSTR action_name )
...
void CStalkerCombatActionSmartCover::execute() {
    ...
    u32 const level_time =
        object().memory().visual().visible_object_time_last_seen( enemy );
