...
CStalkerSearchPlanner::CStalkerSearchPlanner( CAI_Stalker* object,
                                              LPCSTR action_name )
    : inherited( object, action_name ) {}

void CStalkerSearchPlanner::setup( CAI_Stalker* object,
...
void CStalkerSearchPlanner::initialize() {
    ...
    CScriptActionPlanner::m_storage.set_property(
        eWorldPropertyEnemyLocationReached, false );
    CScriptActionPlanner::m_storage.set_property(
        eWorldPropertyAmbushLocationReached, false );
}
...
void CStalkerSearchPlanner::add_evaluators() {
    add_evaluator( eWorldPropertyPureEnemy,
                   xr_new< CStalkerPropertyEvaluatorConst >(
                       true, "is_there_enemies_delayed" ) );
    add_evaluator(
        eWorldPropertyEnemyLocationReached,
        xr_new< CStalkerPropertyEvaluatorMember >(
            ( CPropertyStorage* )0, eWorldPropertyEnemyLocationReached, true,
            true, "enemy location reached" ) );
    add_evaluator(
        eWorldPropertyAmbushLocationReached,
        xr_new< CStalkerPropertyEvaluatorMember >(
            ( CPropertyStorage* )0, eWorldPropertyAmbushLocationReached, true,
            true, "ambush location reached" ) );
}
...
void CStalkerSearchPlanner::add_actions() {
    ...
    action = xr_new< CStalkerActionReachEnemyLocation >(
        m_object, CActionBase< CScriptGameObject >::m_storage,
