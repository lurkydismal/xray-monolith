    ...
    for ( ; I != E; ++I ) {
        ...
        if ( m_bCreateSpawnPositions ) {
            ...
            CSE_ALifeMonsterAbstract* l_tpALifeMonsterAbstract =
                smart_cast< CSE_ALifeMonsterAbstract* >( J );
            if ( l_tpALifeMonsterAbstract )
                l_tpALifeMonsterAbstract->o_torso.yaw =
                    angle_normalize_signed( ( I - B ) / N * PI_MUL_2 );
        }
...
void CSE_ALifeGroupAbstract::switch_offline() {
    CSE_ALifeDynamicObject* object =
        smart_cast< CSE_ALifeDynamicObject* >( base() );
    VERIFY( object );
    ...
    if ( I != E ) {
        ...
        CSE_ALifeMonsterAbstract* tpGroup =
            smart_cast< CSE_ALifeMonsterAbstract* >( this );
        if ( tpGroupMember && tpGroup ) {
            ...
            tpGroup->m_tGraphID =
                ai().cross_table().vertex( dwNodeID ).game_vertex_id();
            tpGroup->m_fDistanceToPoint =
                ai().cross_table().vertex( dwNodeID ).distance();
            tpGroup->m_tNextGraphID = tpGroup->m_tGraphID;
            u16 wNeighbourCount =
                ai().game_graph().vertex( tpGroup->m_tGraphID )->edge_count();
            CGameGraph::const_iterator i, e;
            ai().game_graph().begin( tpGroup->m_tGraphID, i, e );
            tpGroup->m_tPrevGraphID =
                ( *( i + object->randI( 0, wNeighbourCount ) ) ).vertex_id();
        }
        object->alife().remove_online( tpGroupMember, false );
        ++I;
    }
    for ( ; I != E; ++I )
        object->alife().remove_online( ai().alife().objects().object( *I ),
                                       false );
...
bool CSE_ALifeGroupAbstract::synchronize_location() {
    ...
    if ( object->m_tGraphID != member.m_tGraphID ) {
        if ( !object->m_bOnline )
            object->alife().graph().change( object, object->m_tGraphID,
                                            member.m_tGraphID );
        else
...
void CSE_ALifeGroupAbstract::try_switch_offline() {
    ...
    for ( ; i < N; ++i ) {
        // casting group member to the abstract monster to get access to the
        // Health property
        CSE_ALifeMonsterAbstract* tpGroupMember =
            smart_cast< CSE_ALifeMonsterAbstract* >(
        ...
        if ( tpGroupMember->g_Alive() ) {
            ...
            if ( !tpGroupMember->can_switch_online() )
                ...
                // switch group offline since not all the group members are
                // ready to switch offline
                break;

            if ( I->alife().graph().actor()->o_Position.distance_to(
                     tpGroupMember->o_Position ) <=
        ...
        CSE_ALifeInventoryItem* item =
            smart_cast< CSE_ALifeInventoryItem* >( tpGroupMember );
        if ( item && item->attached() ) {
            CSE_ALifeDynamicObject* object =
                ai().alife().objects().object( tpGroupMember->ID_Parent, true );
            if ( object )
                object->detach( item );
        }
        ...
        // and remove it from the graph point but do not remove it from the
        // current level map
        CSE_ALifeInventoryItem* l_tpALifeInventoryItem =
            smart_cast< CSE_ALifeInventoryItem* >( tpGroupMember );
        if ( !l_tpALifeInventoryItem || !l_tpALifeInventoryItem->attached() )
            I->alife().graph().remove( tpGroupMember, tpGroupMember->m_tGraphID,
                                       false );
