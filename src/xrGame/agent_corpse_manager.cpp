...
bool CAgentCorpseManager::process_corpse( CMemberOrder& member ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        float dist_sqr = ( *I ).corpse()->Position().distance_to_sqr(
            member.object().Position() );
        if ( dist_sqr < min_dist_sqr ) {
            if ( ( *I ).reactor() &&
                 ( ( *I ).reactor()->Position().distance_to_sqr(
                       ( *I ).corpse()->Position() ) <= min_dist_sqr ) )
                continue;
...
void CAgentCorpseManager::react_on_member_death() {
    for ( ;; ) {
        bool changed = false;
        CAgentMemberManager::MEMBER_STORAGE::iterator I =
            object().member().combat_members().begin();
        CAgentMemberManager::MEMBER_STORAGE::iterator E =
...

    {
        ...
        m_corpses.erase( std::remove_if( m_corpses.begin(), m_corpses.end(),
