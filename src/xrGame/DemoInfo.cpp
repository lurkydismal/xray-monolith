...
/*
void stream_read_demostring(CStreamReader* stream, shared_str & dest_result, u32
max_size)
{
        R_ASSERT(stream);
        R_ASSERT(max_size > 0);
...
void demo_player_info::load_from_player( game_PlayerState* player_state ) {
    ...
    m_spots = m_frags - ( player_state->m_iTeamKills * 2 ) -
              player_state->m_iSelfKills + ( m_artefacts * 3 );
    ...
    if ( ( tmp_game->Type() == eGameIDDeathmatch ) && ( tmp_team != 2 ) ) {
        tmp_team =
            0; // in deathmatch if player is not spectator, he is in green team
    }
...
void demo_info::load_from_game() {
    ...
    if ( tmp_game->local_player &&
         ( xr_strlen( tmp_game->local_player->getName() ) > 0 ) ) {
