...
void CClientSpawnManager::add( ALife::_OBJECT_ID requesting_id,
...
void CClientSpawnManager::remove( REQUESTED_REGISTRY& registry,
                                  ALife::_OBJECT_ID requesting_id,
                                  ALife::_OBJECT_ID requested_id,
                                  bool no_warning ) {
    ...
    if ( I == registry.end() ) {
        ai().script_engine().script_log( eLuaMessageTypeError,
                                         ...
                                         "with id %d from object with id %d!",
...
void CClientSpawnManager::remove( ALife::_OBJECT_ID requesting_id,
                                  ALife::_OBJECT_ID requested_id ) {
    REQUEST_REGISTRY::iterator I = m_registry.find( requesting_id );
    if ( I == m_registry.end() ) {
        ai().script_engine().script_log( eLuaMessageTypeError,
...
void CClientSpawnManager::callback( CSpawnCallback& spawn_callback,
                                    CObject* object ) {
    ...
    CScriptGameObject* script_game_object =
        !game_object ? 0 : game_object->lua_game_object();
...
void CClientSpawnManager::merge_spawn_callbacks(
    CSpawnCallback& new_callback,
    CSpawnCallback& old_callback ) {
...
void CClientSpawnManager::dump( ALife::_OBJECT_ID requesting_id ) const {
    ...
    for ( ; I != E; ++I ) {
        ...
        REQUESTED_REGISTRY::const_iterator e = ( *I ).second.end();
        for ( ; i != e; ++i )
            Msg( "! CClientSpawnManager::dump[id %d waits for %d]",
                 requesting_id, ( *i ).first );
    }
}
#endif // DEBUG
