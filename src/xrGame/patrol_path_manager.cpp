...
IC bool CPatrolPathManager::accessible(
    const CPatrolPath::CVertex* vertex ) const {
...
void CPatrolPathManager::select_point( const Fvector& position,
                                       u32& dest_vertex_id ) {
    ...
    if ( !actual() || !m_path->vertex( m_curr_point_index ) ) {
        switch ( m_start_type ) {
            case ePatrolStartTypeFirst: {
                ...
                VERIFY3( accessible( vertex ) || show_restrictions( m_object ),
            ...
            case ePatrolStartTypeNearest: {
                vertex =
                    m_path->point( position, CAccessabilityEvaluator( this ) );
                VERIFY3( accessible( vertex ) || show_restrictions( m_object ),
                         *m_path_name, *m_game_object->cName() );
                break;
            }
            case ePatrolStartTypePoint: {
                VERIFY3( m_path->vertex( m_start_point_index ), *m_path_name,
                         *m_game_object->cName() );
            ...
            case ePatrolStartTypeNext: {
                if ( m_prev_point_index != u32( -1 ) ) {
                    if ( ( m_prev_point_index + 1 ) < m_path->vertex_count() ) {
                        ...
                        u32 next_point_id =
                            get_next_point( m_prev_point_index );
        ...
        if ( !( vertex || show_restrictions( m_object ) ) ) {
            ...
            VERIFY2( vertex || show_restrictions( m_object ),
                     make_string( "any vertex in patrol path [%s] is "
                                  "inaccessible for object [%s]",
                                  *m_path_name, *m_game_object->cName() ) );
        ...
        R_ASSERT2(
            ...
            make_string( "patrol path[%s], point on path [%s],object [%s]",
                         *m_path_name, *vertex->data().name(),
                         *m_game_object->cName() ) );
...
#else
        if ( !m_game_object->Position().similar( vertex->data().position(),
                                                 .1f ) ) {
            dest_vertex_id = vertex->data().level_vertex_id();
            m_dest_position = vertex->data().position();
    ...
    VERIFY3(
        m_path->vertex( m_curr_point_index ) || show_restrictions( m_object ),
        *m_path_name, *m_game_object->cName() );

    m_game_object->callback( GameObject::ePatrolPathInPoint )(
        m_game_object->lua_game_object(),
    ...
    VERIFY3( m_path->vertex( target ) || show_restrictions( m_object ),
             *m_path_name, *m_game_object->cName() );

    m_prev_point_index = m_curr_point_index;
    m_curr_point_index = target;
    dest_vertex_id =
        m_path->vertex( m_curr_point_index )->data().level_vertex_id();
    m_dest_position = m_path->vertex( m_curr_point_index )->data().position();
    VERIFY3( accessible( m_dest_position ) || show_restrictions( m_object ),
             *m_path_name, *m_game_object->cName() );
    m_actuality = true;
    m_completed = false;
}
...
void CPatrolPathManager::set_previous_point( int point_index ) {
    ...
    if ( !m_path->vertex( point_index ) ) {
        ai().script_engine().script_log(
            ...
            "Start point violates path bounds %s (object %s)!", *m_path_name,
            *m_game_object->cName() );
        return;
    }
    VERIFY( m_path );
