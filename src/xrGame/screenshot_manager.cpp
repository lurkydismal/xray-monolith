...
#pragma pack( pop )
    ...
    rgb24map* dest = reinterpret_cast< rgb24map* >(
        rgba ); // WARNING sorce and dest stored in one place ...
    ...
    for ( int y = 0; y < RESULT_HEIGHT; ++y ) {
        for ( int x = 0; x < RESULT_WIDTH; ++x ) {
            ...
            ( *dest )[ y ][ x ].r =
                *reinterpret_cast< u8* >( &rgba[ rgba_index ] );
            ( *dest )[ y ][ x ].g =
                *( reinterpret_cast< u8* >( &rgba[ rgba_index ] ) + 1 );
            ( *dest )[ y ][ x ].b =
                *( reinterpret_cast< u8* >( &rgba[ rgba_index ] ) + 2 );
        }
    }
    *width = RESULT_WIDTH;
...
void screenshot_manager::make_jpeg_file() {
    ...
    u32 width = *sizes;
    u32 height = *( ++sizes );
    u8* rgb24data = reinterpret_cast< u8* >( m_result_writer.pointer() +
                                             2 * sizeof( u32 ) );
    ...
    realloc_jpeg_buffer( m_result_writer.size() +
                         screenshots::writer::info_max_size );
...
void screenshot_manager::sign_jpeg_file() {
    screenshots::writer tmp_writer( m_jpeg_buffer, m_jpeg_buffer_size,
...
void screenshot_manager::shedule_Update( u32 dt ) {
    R_ASSERT( m_state & making_screenshot ||
    ...
    if ( is_make_in_progress && ( m_defered_ssframe_counter == 0 ) ) {
        if ( !m_make_done_event ) {
            ...
            sign_jpeg_file();
            compress_image();
            m_complete_callback( m_buffer_for_compress,
        ...
        /*	//---------
                        ...
                        if (!m_result_writer.size())
                        {
                                ...
                                FS.update_path(screen_shot_path,
        "$screenshots$", "tmp_response.dds"); IReader* tmp_reader =
        FS.r_open(screen_shot_path); if (tmp_reader)
                                {
                                        m_result_writer.w(tmp_reader->pointer(),
        tmp_reader->length()); FS.r_close(tmp_reader);
                                }
                        }
        #endif //#ifdef DEBUG*/
    ...
    if ( is_drawing_downloads() ) {
        static_cast< game_cl_mp* >( Level().game )
            ->draw_all_active_binder_states();
    }
}
...
void screenshot_manager::process_screenshot( bool singlecore ) {
    ...
    thread_spawn( &screenshot_manager::screenshot_maker_thread,
                  "screenshot_maker", 0, this );
}
...
void screenshot_manager::screenshot_maker_thread( void* arg_ptr ) {
    ...
    screenshot_manager* this_ptr =
        static_cast< screenshot_manager* >( arg_ptr );
    DWORD wait_result =
...
void screenshot_manager::compress_image() {
    ...
    m_buffer_for_compress_size =
        ppmd_compress( m_buffer_for_compress, m_buffer_for_compress_capacity,
