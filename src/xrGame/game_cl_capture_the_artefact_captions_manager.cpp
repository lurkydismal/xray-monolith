    ...
    if ( ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ) {
        if ( smart_cast< CActor* >( Level().CurrentControlEntity() ) ) {
            ...
            if ( m_can_show_payspawn )
                parent_game_ui->SetPressJumpMsgCaption(
...
void CTAGameClCaptionsManager::ShowScoreCaptions() {
    ...
    u32 win_str_size =
        st.translate( "mp_team_wins" ).size() + xr_strlen( team_name ) + 1;
    char* win_str = static_cast< char* >( _alloca( win_str_size ) );
    xr_sprintf( win_str, win_str_size, st.translate( "mp_team_wins" ).c_str(),
                team_name );
...
u32 CTAGameClCaptionsManager::SetWarmupTime( u32 current_warmup_time,
                                             u32 current_time ) {
    u32 ret_value = 0;
    ...
    if ( time_remains > 10000 ) {
        strconcat( sizeof( warmup_message ), warmup_message,
                   *st.translate( "mp_time2start" ), " ", time_str );
    } else {
        if ( time_remains < 1000 )
            ...
            strconcat( sizeof( warmup_message ), warmup_message,
                       *st.translate( "mp_ready" ), "...", time_str );
        }
    };
    return ret_value;
}

void CTAGameClCaptionsManager::SetTimeLimit( u32 time_limit,
                                             u32 current_time ) {
