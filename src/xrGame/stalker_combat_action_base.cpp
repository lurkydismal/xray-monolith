...
float g_ai_aim_fire_angle = PI_DIV_8; // Replaced start_fire_angle_difference
                                      // constant for Modded Exes config

CStalkerActionCombatBase::CStalkerActionCombatBase( CAI_Stalker* object,
                                                    LPCSTR action_name )
...
void CStalkerActionCombatBase::fire() {
    ...
    const MonsterSpace::SBoneRotation& current_angles =
        object().movement().head_orientation();
    if ( angle_difference( -yaw, current_angles.current.yaw ) >
         g_ai_aim_fire_angle ) {
        aim_ready();
        return;
    }
...
void CStalkerActionCombatBase::aim_ready() {
    ...
    select_queue_params( distance, min_queue_size, max_queue_size,
                         min_queue_interval, max_queue_interval );
    object().CObjectHandler::set_goal(
        eObjectActionAimReady1, object().best_weapon(), min_queue_size,
        max_queue_size, min_queue_interval, max_queue_interval );
}
...
void CStalkerActionCombatBase::aim_ready_force_full() {
    ...
    select_queue_params( distance, min_queue_size, max_queue_size,
                         min_queue_interval, max_queue_interval );
    object().CObjectHandler::set_goal(
        eObjectActionAimForceFull1, object().best_weapon(), min_queue_size,
        max_queue_size, min_queue_interval, max_queue_interval );
}
...
void CStalkerActionCombatBase::select_queue_params(
    ...
    // Per-NPC fire-queue scaling (set_fire_queue_scale, unset = 1.0). Applied
    // to the band the weapon type + distance selected above, so any
    // [fire_queue_params] retune stays respected; the object handler re-rolls
    // from these scaled bounds. Size clamps to >= 1 so a scale below one
    // degrades to single shots, never to silence.
    ...
    if ( ( size_k != 1.f ) || ( interval_k != 1.f ) ) {
        min_queue_size =
            _max( u32( 1 ), u32( float( min_queue_size ) * size_k + .5f ) );
        max_queue_size = _max( min_queue_size,
                               u32( float( max_queue_size ) * size_k + .5f ) );
        min_queue_interval =
            u32( float( min_queue_interval ) * interval_k + .5f );
        max_queue_interval =
            _max( min_queue_interval,
                  u32( float( max_queue_interval ) * interval_k + .5f ) );
    }
}

void CStalkerActionCombatBase::play_panic_sound( u32 max_start_time,
                                                 u32 min_start_time,
                                                 u32 max_stop_time,
                                                 u32 min_stop_time,
                                                 u32 id ) {
    object().sound().play( object().memory().enemy().selected()->human_being()
                               ? eStalkerSoundPanicHuman
                               : eStalkerSoundPanicMonster,
                           max_start_time, min_start_time, max_stop_time,
                           min_stop_time, id );
}

void CStalkerActionCombatBase::play_attack_sound( u32 max_start_time,
                                                  u32 min_start_time,
                                                  u32 max_stop_time,
                                                  u32 min_stop_time,
                                                  u32 id ) {
...
#ifdef DEBUG
    {
        if ( object().agent_manager().member().combat_members().empty() )
            Msg( "! I am in combat, but there is no combat members at all "
                 "(including me), npc[%s],team[%d],squad[%d],group[%d]",
                 *object().cName(), object().g_Team(), object().g_Squad(),
                 object().g_Group() );
    }
#endif // DEBUG
    ...
    object().sound().play( sound_type, max_start_time, min_start_time,
                           max_stop_time, min_stop_time, id );
}

void CStalkerActionCombatBase::play_start_search_sound( u32 max_start_time,
                                                        u32 min_start_time,
                                                        u32 max_stop_time,
                                                        u32 min_stop_time,
                                                        u32 id ) {
    ...
    object().sound().play( search_with_allies ? eStalkerSoundSearch1WithAllies
                                              : eStalkerSoundSearch1NoAllies,
                           max_start_time, min_start_time, max_stop_time,
                           min_stop_time, id );
}

void CStalkerActionCombatBase::play_enemy_lost_sound( u32 max_start_time,
                                                      u32 min_start_time,
                                                      u32 max_stop_time,
                                                      u32 min_stop_time,
                                                      u32 id ) {
    ...
    object().sound().play( search_with_allies ? eStalkerSoundEnemyLostWithAllies
...
void CStalkerActionCombatBase::setup_cover( CCoverPoint const& cover ) {
    if ( cover.m_is_smart_cover ) {
        object().movement().target_params().cover_id(
            static_cast< smart_cover::cover const& >( cover ).id() );
        return;
    }
