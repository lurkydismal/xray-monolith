...
void target_selector::setup( animation_planner* object,
                             CPropertyStorage* storage ) {
    inherited::setup( object, storage );
    //	inherited_planner::m_use_log = true;
    //	inherited_action::m_use_log = true;
    CActionPlanner::m_storage.set_property(
        eWorldPropertyLookedOut, m_random.randF() <= .7f ? true : false );
    CActionPlanner::m_storage.set_property(
        eWorldPropertyLoopholeTooMuchTimeFiring, false );
    add_evaluators();
    add_actions();

    CWorldState target;
    target.add_condition(
        CWorldProperty( eWorldPropertyPlannerHasTarget, true ) );
...
void target_selector::add_actions() {
    ...
    action = xr_new< target_provider >( &object(), "lookout",
                                        eWorldPropertyLookedOut, 0 );
    ...
    action = xr_new< target_fire >( &object(), "fire",
                                    eWorldPropertyLoopholeFire, 0 );
    ...
    action = xr_new< target_fire_no_lookout >(
        &object(), "fire_no_lookout", eWorldPropertyLoopholeFireNoLookout, 0 );
    add_condition( action, eWorldPropertyLoopholeCanFireAtEnemy, true );
    ...
    action =
        xr_new< default_behaviour_planner >( &object(), "default_behaviour" );
