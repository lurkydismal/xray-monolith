...
private:
    ...
    typedef server_updates_compressor::send_ready_updates_t::const_iterator
        update_iterator_t;
    ...
    u32 OnDelayedMessage( NET_Packet& P,
                          ClientID sender ); // Non-Zero means broadcasting with
                                             // "flags" as returned
...
public:
    ...
    void PerformMigration( CSE_Abstract* E,
                           xrClientData* from,
                           xrClientData* to );
    ...
    void Prepare_connect_spawn( CSE_Abstract* E,
                                xr_vector< PreparedClientSpawn >& prepared );
    void Perform_connect_spawn( const PreparedClientSpawn& prepared,
                                xrClientData* to,
                                NET_Packet& P );
    void Perform_transfer( NET_Packet& PR,
                           NET_Packet& PT,
                           CSE_Abstract* what,
                           CSE_Abstract* from,
                           CSE_Abstract* to );
    ...
    void Process_event_ownership( NET_Packet& P,
                                  ClientID sender,
                                  u32 time,
                                  u16 ID,
                                  BOOL bForced = FALSE );
    bool Process_event_reject( NET_Packet& P,
                               const ClientID sender,
                               const u32 time,
                               const u16 id_parent,
                               const u16 id_entity,
                               bool send_message = true );
    void Process_event_destroy( NET_Packet& P,
                                ClientID sender,
                                u32 time,
                                u16 ID,
                                NET_Packet* pEPack );
    void Process_event_activate( NET_Packet& P,
                                 const ClientID sender,
                                 const u32 time,
                                 const u16 id_parent,
                                 const u16 id_entity,
    ...
    xrClientData* SelectBestClientToMigrateTo( CSE_Abstract* E,
                                               BOOL bForceAnother = FALSE );
    void SendConnectResult( IClient* CL,
                            u8 res,
                            u8 res1,
...
protected:
    ...
    bool CheckAdminRights( const shared_str& user,
...
public:
    ...
    virtual u32 OnMessage( NET_Packet& P,
                           ClientID sender ); // Non-Zero means broadcasting
    ...
    virtual void SendTo_LL( ClientID ID,
                            void* data,
                            u32 size,
                            u32 dwFlags = DPNSEND_GUARANTEED,
                            u32 dwTimeout = 0 );
    void SecureSendTo( xrClientData* xrCL,
                       NET_Packet& P,
                       u32 dwFlags = DPNSEND_GUARANTEED,
                       u32 dwTimeout = 0 );
    virtual void SendBroadcast( ClientID exclude,
                                NET_Packet& P,
                                u32 dwFlags = DPNSEND_GUARANTEED );
    ...
    virtual IClient* client_Find_Get(
        ClientID ID ); // Find earlier disconnected client
    ...
    static LPCSTR get_map_download_url( LPCSTR level_name,
