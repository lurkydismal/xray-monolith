...
void bone_group::init( CActor* actor_ptr ) {
    ...
    m_bone_groups.insert(
        std::make_pair( V->LL_BoneID( "bip01_head" ), gid_head ) );

    m_bone_groups.insert(
        std::make_pair( V->LL_BoneID( "eye_left" ), gid_eyes ) );
    m_bone_groups.insert(
        std::make_pair( V->LL_BoneID( "eye_right" ), gid_eyes ) );

    m_bone_groups.insert(
        std::make_pair( V->LL_BoneID( "bip01_spine" ), gid_spine ) );
    m_bone_groups.insert(
        std::make_pair( V->LL_BoneID( "bip01_spine1" ), gid_spine ) );
    m_bone_groups.insert(
        std::make_pair( V->LL_BoneID( "bip01_spine2" ), gid_spine ) );
    m_bone_groups.insert(
        std::make_pair( V->LL_BoneID( "bip01_spine2" ), gid_spine ) );
}
