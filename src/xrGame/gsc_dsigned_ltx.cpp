...
void gsc_dsigned_ltx_writer::sign_and_save( IWriter& writer ) {
    ...
    shared_str tmp_sign_value =
        sign( m_mem_writer.pointer(), m_mem_writer.tell() );
...
bool gsc_dsigned_ltx_reader::load_and_verify( u8* buffer, u32 const size ) {
    ...
    dsign_section -= 3; // \r\n[
    u32 const dsign_sect_size =
        size - static_cast< u32 >( ( u8* )dsign_section - buffer );
    ...
    shared_str ltx_dsign =
        tmp_dsign_reader.r_string( dsign_secion, "sign_hash" );

    *dsign_section = 0;
    xr_strcat( dsign_section, dsign_sect_size, ltx_date.c_str() );
    u32 new_size =
        size - dsign_sect_size + ltx_date.size() + 1; // xr_strlen(buffer)
