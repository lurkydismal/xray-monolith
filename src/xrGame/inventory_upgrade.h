...
struct functor : public functor_base< return_type > {
    IC return_type operator()() const { return functr( this->parameter ); }
};
...
struct functor< void > : public functor_base< void > {
    IC void operator()() const { this->functr( this->parameter ); }
};
...
struct functor2< void > : public functor< void > {
    ...
    IC void operator()() const { this->functr( this->parameter, parameter2 ); }
};
...
public:
    Upgrade();
    virtual ~Upgrade();
    void construct( const shared_str& upgrade_id,
    ...
    virtual UpgradeStateResult can_install( CInventoryItem& item,
                                            bool loading );
