...
private:
    struct SID_Block {
        ...
        IC bool operator<( const SID_Block& b ) const {
            return ( m_tCount &&
                     ( ( m_tTimeID < b.m_tTimeID ) || !b.m_tCount ) );
        }
    };
...
private:
    ...
    IC VALUE_ID tfGetFromBlock( SID_Block& l_tID_Block, VALUE_ID tValueID ) {
        ...
        if ( tInvalidValueID == tValueID )
            return ( VALUE_ID( l_tID_Block.m_tpIDs[ --l_tID_Block.m_tCount ] ) +
                     l_tBlockID * tBlockSize + tMinValue );
        ...
        R_ASSERT2( l_tID_Block.m_tpIDs + l_tID_Block.m_tCount != l_tpBlockID,
...
public:
    IC CID_Generator() {
        ...
        for ( u32 j = 0; j < m_tBlockCount; ++j )
            std::reverse(
                m_tppBlocks[ j ].m_tpIDs,
                m_tppBlocks[ j ].m_tpIDs + m_tppBlocks[ j ].m_tCount );
    }

    IC VALUE_ID tfGetID( VALUE_ID tValueID = tInvalidValueID ) {
        if ( tInvalidValueID != tValueID )
            return ( tfGetFromBlock(
                m_tppBlocks[ tfGetBlockByValue( tValueID ) ], tValueID ) );

        R_ASSERT2( m_available_count, "Not enough IDs" );
        SID_Block* I =
            std::min_element( m_tppBlocks, m_tppBlocks + m_tBlockCount );
...
#endif
        l_tID_Block.m_tpIDs[ l_tID_Block.m_tCount++ ] =
            TYPE_ID( ( tValueID - tMinValue ) % tBlockSize );
