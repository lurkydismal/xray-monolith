...
struct SPHContactDBGDraw : public SPHDBGDrawAbsract {
    ...
    void render() {
        ...
        Level().debug_renderer().draw_aabb(
            pos, .01f, .01f, .01f,
            D3DCOLOR_XRGB( 255 * is_cyl, 0, 255 * !is_cyl ) );
        Fvector dir;
        dir.set( norm );
        dir.mul( depth * 100.f );
        dir.add( pos );
...
void DBG_DrawTri( const Fvector& v0,
                  const Fvector& v1,
                  const Fvector& v2,
                  u32 ac,
                  bool solid ) {
...
void DBG_DrawMatrix( const Fmatrix& m, float size, u8 a /* = 255*/ ) {
    ...
    DBG_DrawPHAbstruct(
        xr_new< SPHDBGDrawLine >( m.c, to, D3DCOLOR_XRGB( a, 0, 0 ) ) );
    to.add( m.c, Fvector().mul( m.j, size ) );
    DBG_DrawPHAbstruct(
        xr_new< SPHDBGDrawLine >( m.c, to, D3DCOLOR_XRGB( 0, a, 0 ) ) );
    to.add( m.c, Fvector().mul( m.k, size ) );
    DBG_DrawPHAbstruct(
        xr_new< SPHDBGDrawLine >( m.c, to, D3DCOLOR_XRGB( 0, 0, a ) ) );
}
...
template < int ax >
void DBG_DrawRotation( float ang0,
                       float ang1,
                       const Fmatrix& m,
                       const Fvector& l,
                       float size,
    ...
    for ( u32 i = 0; tessel > i; ++i ) {
        ...
        to1.add( from, tmp );
        DBG_DrawPHAbstruct(
            xr_new< SPHDBGDrawTri >( from, to0, to1, ac, solid ) );
    }
}

void DBG_DrawRotationX( const Fmatrix& m,
                        float ang0,
                        float ang1,
                        float size,
                        u32 ac,
                        bool solid,
                        u32 tessel ) {
    DBG_DrawRotation< 0 >( ang0, ang1, m, Fvector().set( 0, 0, 1 ), size, ac,
                           solid, tessel );
}

void DBG_DrawRotationY( const Fmatrix& m,
                        float ang0,
                        float ang1,
                        float size,
                        u32 ac,
                        bool solid,
                        u32 tessel ) {
    DBG_DrawRotation< 1 >( ang0, ang1, m, Fvector().set( 1, 0, 0 ), size, ac,
                           solid, tessel );
}

void DBG_DrawRotationZ( const Fmatrix& m,
                        float ang0,
                        float ang1,
                        float size,
                        u32 ac,
                        bool solid,
                        u32 tessel ) {
    DBG_DrawRotation< 2 >( ang0, ang1, m, Fvector().set( 0, 1, 0 ), size, ac,
                           solid, tessel );
}
    ...
    virtual void render() {
        ...
        if ( !fsimilar( dbg_text_current_height_scale,
            ...
            UI().Font().pFontStat->SetHeight(
                UI().Font().pFontStat->GetHeight() * dbg_text_height_scale /
                dbg_text_current_height_scale );
...
void DBG_DrawPHAbstruct( SPHDBGDrawAbsract* a ) {
    ...
    switch ( dbg_ph_draw_mode ) {
        ...
        case dmCashed:
            ...
            break;
        case dmCashedSecondary:
            push( dbg_draw_cashed_secondary, a );
            break;
        case dmSimple:
...
void DBG_RenderUpdate() {
    ...
    dbg_draw_cashed.insert( dbg_draw_cashed.end(),
                            dbg_draw_cashed_secondary.begin(),
                            dbg_draw_cashed_secondary.end() );
...
void DBG_DrawStatBeforeFrameStep() {
    if ( ph_dbg_draw_mask.test( phDbgDrawObjectStatistics ) ) {
        ...
        obj_count =
            obj_count * 0.9f + float( physics_world()->ObjectsNumber() ) * 0.1f;
        update_obj_count =
            update_obj_count * 0.9f +
            float( physics_world()->UpdateObjectsNumber() ) * 0.1f;
...
void DBG_DrawStatAfterFrameStep() {
    if ( ph_dbg_draw_mask.test( phDbgDrawObjectStatistics ) ) {
        ...
        fdbg_islands_num =
            0.9f * fdbg_islands_num + 0.1f * float( dbg_islands_num );
        fdbg_bodies_num =
            0.9f * fdbg_bodies_num + 0.1f * float( dbg_bodies_num );
        fdbg_joints_num =
            0.9f * fdbg_joints_num + 0.1f * float( dbg_joints_num );
        fdbg_contacts_num =
            0.9f * fdbg_contacts_num + 0.1f * float( dbg_contacts_num );
    ...
    if ( ph_dbg_draw_mask.test( phDbgDrawCashedTriesStat ) ) {
        ...
        fdbg_saved_tries_for_active_objects =
            0.9f * fdbg_saved_tries_for_active_objects +
            0.1f * float( dbg_saved_tries_for_active_objects );
        fdbg_total_saved_tries = 0.9f * fdbg_total_saved_tries +
                                 0.1f * float( dbg_total_saved_tries );
        DBG_OutText( "Ph Number of cashed tries in active objects %5.0f",
                     fdbg_saved_tries_for_active_objects );
        ...
        fdbg_reused_queries_per_step.new_val(
            float( dbg_reused_queries_per_step ) );
        fdbg_new_queries_per_step.new_val( float( dbg_new_queries_per_step ) );
        DBG_OutText( "Ph tri_queries_per_step %5.2f",
                     fdbg_new_queries_per_step.val );
        DBG_OutText( "Ph reused_tri_queries_per_step %5.2f",
                     fdbg_reused_queries_per_step.val );
        DBG_OutText( "------------------------------" );
    }
    // draw_frame=!draw_frame;
}
...
void CFunctionGraph::AddMarker( CStatGraph::EStyle Style,
                                float pos,
                                u32 Color ) {
    VERIFY( IsActive() );
...
void PH_DBG_SetTrackObject() {
    ...
    if ( g_pGameLevel )
        trace_object =
            Level().Objects.FindObjectByName( PH_DBG_ObjectTrackName() );
}
...
static LPCSTR name_blend_type( CBlend::ECurvature blend ) {
    static xr_token token_blend[] = { { "eFREE_SLOT", CBlend::eFREE_SLOT },
...
void DBG_AnimBlend( IKinematicsAnimated& ka, const CBlend& B ) {
    ...
    if ( dbg_track_obj_flags.test( dbg_track_obj_blends_motion_name ) ) {
        std::pair< LPCSTR, LPCSTR > motion =
            ka.LL_MotionDefName_dbg( B.motionID );
        DBG_OutText( "motion : name %s, set: %s ", motion.first,
                     motion.second );
    }
    if ( dbg_track_obj_flags.test( dbg_track_obj_blends_time ) )
        DBG_OutText( "time current: %f, time total: %f, frame %d ",
                     B.timeCurrent, B.timeTotal, B.dwFrame );
    ...
    if ( dbg_track_obj_flags.test( dbg_track_obj_blends_mix_params ) )
        DBG_OutText( "accrue: %f, fallof: %f, speed: %f ", B.blendAccrue,
                     B.blendFalloff, B.speed );

    if ( dbg_track_obj_flags.test( dbg_track_obj_blends_flags ) )
        DBG_OutText(
            "bonepart: %d, channel: %d, stop_at_end: %s, fall_at_end: %s ",
            B.bone_or_part, B.channel, name_bool( B.stop_at_end ),
            name_bool( B.fall_at_end ) );
    if ( dbg_track_obj_flags.test( dbg_track_obj_blends_state ) )
        DBG_OutText( "state: %s, playing: %s, stop_at_end_callback: %s ",
                     name_blend_type( B.blend_state() ), name_bool( B.playing ),
                     name_bool( B.stop_at_end_callback ) );
    // DBG_OutText( "callback: %p callback param: %p", B.Callback,
    // B.CallbackParam );
}
...
static void DBG_DrawTarckObj() {
    ...
    if ( !trace_object ) {
        // trace_object= Level().Objects.FindObjectByName(
...
void DBG_DrawBones( const Fmatrix& xform, IKinematics* K ) {
    ...
    for ( u16 i = 0; i < nbb; ++i ) {
        ...
        if ( BI_NONE != bp ) {
            ...
            DBG_DrawLine( Fmatrix().mul_43( xform, bone_pos ).c,
                          Fmatrix().mul_43( xform, pbi.mTransform ).c,
...
void DBG_PhysBones( CObject& O ) {
    ...
    for ( u16 i = 0; i < nb_elements; ++i ) {
        ...
        DBG_DrawMatrix( e->XFORM(),
                        0.1f ); // Fmatrix().mul_43( O.XFORM(),e->XFORM())
        CPhysicsElement* pE = ( e )->parent_element();
        if ( pE )
            DBG_DrawLine( e->XFORM().c, pE->XFORM().c,
...
void DBG_DrawBind( CObject& O ) {
    ...
    for ( u16 i = 0; i < nbb; ++i ) {
        ...
        u16 bp = bd.GetParentID();
        if ( BI_NONE != bp ) {
            DBG_DrawLine( Fmatrix().mul_43( O.XFORM(), binds[ i ] ).c,
                          Fmatrix().mul_43( O.XFORM(), binds[ bp ] ).c,
    ...
    if ( bip01_bi != BI_NONE )
        DBG_DrawPoint( Fmatrix().mul_43( O.XFORM(), binds[ bip01_bi ] ).c, 0.1,
...
bool is_trace_obj( CPHObject* obj ) {
    return trace_object &&
           smart_cast< CObject* >( obj->ref_object() ) == trace_object;
}
...
void DBG_ObjBeforeStep( CPHObject* obj ) {
    if ( is_trace_obj( obj ) ) {
        ...
        if ( obj->ref_object()->ObjectPPhysicsShell() ) {
            obj->ref_object()->ObjectPPhysicsShell()->dbg_draw_force(
                0.1f, D3DCOLOR_XRGB( 0, 0, 255 ) );
            Fmatrix form = Fidentity;
            obj->ref_object()->ObjectPPhysicsShell()->GetGlobalTransformDynamic(
                &form );
...
void DBG_ObjAfterStep( CPHObject* obj ) {
    if ( is_trace_obj( obj ) ) {
        if ( obj->ref_object()->ObjectPPhysicsShell() ) {
            obj->ref_object()->ObjectPPhysicsShell()->dbg_draw_velocity(
                0.1f, D3DCOLOR_XRGB( 255, 0, 0 ) );
        }
...
class CPHDebugOutput : public IDebugOutput {
    ...
    // ang1, float size, u32 ac, bool solid = false, u32 tessel = 7 ) = 0;
    // virtual	void DBG_DrawRotationZ( const Fmatrix &m, float ang0, float
    // ang1, float size, u32 ac, bool solid = false, u32 tessel = 7 ) = 0;
    ...
    // =0; virtual	void DBG_PhysBones( CObject &O )
    ...
    virtual float dbg_vel_collid_damage_to_display() {
        ...
        // Msg( "%s, _14_=%f ", dump_string( make_string( "%s.i, ", name
        // ).c_str(), form.i ).c_str( ) , form._14_ );
        // Msg( "%s, _24_=%f ", dump_string( make_string( "%s.j, ", name
        // ).c_str(), form.j ).c_str( ) , form._24_ ); Msg( "%s, _34_=%f ",
        // dump_string( make_string( "%s.k, ", name ).c_str(), form.k ).c_str( )
        // , form._34_  ); Msg( "%s, _44_=%f ", dump_string( make_string( "%s.c,
        // ", name ).c_str(), form.c ).c_str( ) , form._44_ );
    }
