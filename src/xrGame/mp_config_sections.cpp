...
static const char* important_sections[] = { "mp_actor",
                                            "mp_actor_damage",
                                            "mp_actor_immunities",
                                            "mp_actor_condition",
                                            "rank_base",
...
mp_config_sections::mp_config_sections()
    ...
    for ( int i = 0; i < ( sizeof( important_sections ) / sizeof( char* ) );
          ++i ) {
...
bool mp_config_sections::dump_one( CMemoryWriter& dest ) {
    ...
    CInifile::Sect& tmp_sect =
        pSettings->r_section( m_current_dump_sect->c_str() );
...
void mp_active_params::dump( IAnticheatDumpable const* dumpable_obj,
                             LPCSTR sect_name_key,
                             CInifile& dest_dumper ) {
    LPCSTR obj_sect_name = "";
    if ( dumpable_obj ) {
        shared_str const tmp_obj_section =
            dumpable_obj->GetAnticheatSectionName();
