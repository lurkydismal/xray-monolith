...
#define TEAM_COUNT 4
    ...
    void GenerateNewName( char const* old_name,
                          char* dest,
                          u32 const dest_size );
...
public:
    ...
    virtual void
    OnRoundEnd(); //	round_end_reason			// конец раунда
...
public:
    ...
    virtual void OnHit( u16 id_hitter,
                        u16 id_hitted,
                        NET_Packet& P ); // кто-то получил Hit
    ...
    virtual void Update();
    virtual void net_Export_State( NET_Packet& P,
                                   ClientID id_to ); // full state
    virtual void net_Export_Update( NET_Packet& P,
                                    ClientID id_to,
                                    ClientID id );
    // just incremental update for specific client
    virtual void net_Export_GameTime(
        NET_Packet& P ); // update GameTime only for remote clients
    ...
    void AddDelayedEvent( NET_Packet& tNetPacket,
                          u16 type,
                          u32 time,
                          ClientID sender );
    void ProcessDelayedEvent();
    // this method will delete all events for entity that already not exist (in
    // case when player was kicked)
