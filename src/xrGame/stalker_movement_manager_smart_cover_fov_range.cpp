...
//	Description : stalker movement manager class with smart covers fov and
//range stuff
////////////////////////////////////////////////////////////////////////////
...
bool stalker_movement_manager_smart_cover::fill_enemy_position(
...
bool stalker_movement_manager_smart_cover::enemy_in_fov() const {
    ...
    if ( !m_current.cover()->is_position_in_fov( *m_current.cover_loophole(),
                                                 position ) )
        return ( false );

    if ( !m_current.cover()->is_position_in_range( *m_current.cover_loophole(),
                                                   position ) )
        return ( false );
...
bool stalker_movement_manager_smart_cover::in_current_loophole_fov(
    Fvector const& position ) const {
    VERIFY( m_current.cover() || entering_smart_cover_with_animation() );
    VERIFY( entering_smart_cover_with_animation() ||
            m_current.cover_loophole() );
    if ( m_current.cover() )
        return ( m_current.cover()->is_position_in_fov(
            *m_current.cover_loophole(), position ) );

    smart_cover::cover const* cover =
        ai().cover_manager().smart_cover( m_enter_cover_id );
    VERIFY( cover );

    smart_cover::loophole const* loophole =
        cover->description()->loophole( m_enter_loophole_id );
    return ( cover->is_position_in_fov( *loophole, position ) );
}

bool stalker_movement_manager_smart_cover::in_current_loophole_range(
    Fvector const& position ) const {
    VERIFY( m_current.cover() || entering_smart_cover_with_animation() );
    VERIFY( entering_smart_cover_with_animation() ||
            m_current.cover_loophole() );
    if ( m_current.cover() )
        return ( m_current.cover()->is_position_in_range(
            *m_current.cover_loophole(), position ) );
