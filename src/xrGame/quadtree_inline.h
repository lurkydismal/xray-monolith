...
IC CSQuadTree::CQuadTree( const Fbox& box,
                          float min_cell_size,
                          u32 max_node_count,
                          u32 max_list_item_count ) {
    m_leaf_count = 0;
    ...
    m_max_depth = std::abs(
        iFloor( log( 2.f * m_radius / min_cell_size ) / log( 2.f ) + .5f ) );
...
TEMPLATE_SPECIALIZATION
IC u32 CSQuadTree::neighbour_index( const Fvector& position,
                                    Fvector& center,
...
TEMPLATE_SPECIALIZATION
IC void CSQuadTree::nearest( const Fvector& position,
                             float radius,
...
IC void CSQuadTree::nearest( const Fvector& position,
    ...
    if ( depth == m_max_depth ) {
        ...
        CListItem* leaf = ( ( CListItem* )( ( void* )( node ) ) );
        for ( ; leaf; leaf = leaf->m_next )
            if ( leaf->m_object->position().distance_to_xz_sqr( position ) <=
                 radius_sqr )
    ...
    if ( _abs( position.z - center.z ) < radius ) {
        if ( _abs( position.x - center.x ) < radius ) {
            if ( _sqr( position.z - center.z ) + _sqr( position.x - center.x ) <
                 _sqr( radius ) ) {
                nearest( position, radius, objects, node->m_neighbours[ 0 ],
                         next_center.set( center.x - distance, center.y,
                                          center.z - distance ),
                         distance, depth + 1 );
            ...
            nearest( position, radius, objects, node->m_neighbours[ index ],
                     next_center, distance, depth + 1 );

            if ( position.z > center.z ) {
                if ( index & 1 )
                    nearest( position, radius, objects,
                             node->m_neighbours[ index == 1 ? 0 : 2 ],
                             next_center.set(
                                 center.x + ( index == 1 ? -1 : 1 ) * distance,
                                 center.y, center.z - distance ),
                             distance, depth + 1 );
            } else if ( !( index & 1 ) )
                nearest(
                    position, radius, objects,
                    node->m_neighbours[ !index ? 1 : 3 ],
                    next_center.set( center.x + ( !index ? -1 : 1 ) * distance,
                                     center.y, center.z + distance ),
                    distance, depth + 1 );
            ...
            if ( position.x > center.x ) {
                if ( index > 1 )
                    nearest(
                        position, radius, objects,
                        node->m_neighbours[ index == 2 ? 0 : 1 ],
                        next_center.set(
                            center.x - distance, center.y,
                            center.z + ( index == 2 ? -1 : 1 ) * distance ),
                        distance, depth + 1 );
            } else if ( index < 2 )
                nearest( position, radius, objects,
                         node->m_neighbours[ !index ? 2 : 3 ],
                         next_center.set(
                             center.x + distance, center.y,
                             center.z + ( !index ? -1 : 1 ) * distance ),
                         distance, depth + 1 );
...
IC _object_type* CSQuadTree::remove( const _object_type* object,
    ...
    u32 index = neighbour_index( object->position(), center, distance );
    VERIFY( index < 4 );
    _object_type* _object = remove( object, node->m_neighbours[ index ], center,
                                    distance, depth + 1 );
    if ( node->m_neighbours[ index ] || node->m_neighbours[ 0 ] ||
         node->m_neighbours[ 1 ] || node->m_neighbours[ 2 ] ||
         node->m_neighbours[ 3 ] )
        return ( _object );
    m_nodes->remove( node );
    return ( _object );
...
TEMPLATE_SPECIALIZATION
IC void CSQuadTree::all( xr_vector< _object_type* >& objects,
                         CQuadNode* node,
                         int depth ) const {
    if ( !node )
        return;
