...
void CCar::DbgCreatePlots() {
    ...
    m_dbg_power_rpm.Init(
        CFunctionGraph::type_function( this, &CCar::Parabola ), m_min_rpm,
        m_max_rpm, 0, y_pos, 500, y_w, 1000, D3DCOLOR_XRGB( 0, 0, 255 ) );
    m_dbg_power_rpm.AddMarker( CStatGraph::stVert, 0,
                               D3DCOLOR_XRGB( 255, 0, 0 ) );
    m_dbg_power_rpm.AddMarker( CStatGraph::stHor, 0,
    ...
    m_dbg_torque_rpm.Init(
        CFunctionGraph::type_function( this, &CCar::TorqueRpmFun ), m_min_rpm,
        m_max_rpm, 0, y_pos, 500, y_w, 1000 );
    m_dbg_torque_rpm.AddMarker( CStatGraph::stVert, 0,
                                D3DCOLOR_XRGB( 255, 0, 0 ) );
    m_dbg_torque_rpm.AddMarker( CStatGraph::stHor, 0,
    ...
    if ( b_auto_switch_transmission &&
         ph_dbg_draw_mask.test( phDbgDrawCarAllTrnsm ) ) {
        xr_vector< Fvector >::iterator i = m_gear_ratious.begin() + 1,
                                       e = m_gear_ratious.end();
        for ( ; i < e; i++ ) {
            float r = 4 * m_dbg_torque_rpm.ResolutionX();
            m_dbg_torque_rpm.AddMarker( CStatGraph::stVert, ( *i )[ 1 ] + r,
                                        D3DCOLOR_XRGB( 255, 255, 0 ) );
            m_dbg_torque_rpm.AddMarker( CStatGraph::stVert, ( *i )[ 2 ] + r,
                                        D3DCOLOR_XRGB( 0, 255, 255 ) );
            r = 4 * m_dbg_power_rpm.ResolutionX();
            m_dbg_power_rpm.AddMarker( CStatGraph::stVert, ( *i )[ 1 ] + r,
                                       D3DCOLOR_XRGB( 255, 255, 0 ) );
            m_dbg_power_rpm.AddMarker( CStatGraph::stVert, ( *i )[ 2 ] + r,
                                       D3DCOLOR_XRGB( 0, 255, 255 ) );
        }
    }
    //--------------------------------------
    m_dbg_dynamic_plot = xr_new< CStatGraph >();
    m_dbg_dynamic_plot->SetRect( 0, y_pos, 500, y_w,
    ...
    //--------------------------------------
    m_dbg_dynamic_plot->AddMarker( CStatGraph::stHor, 0,
                                   D3DCOLOR_XRGB( 255, 0, 0 ) );
    xr_vector< Fvector >::iterator i = m_gear_ratious.begin() + 1,
                                   e = m_gear_ratious.end();
    for ( ; i < e; i++ ) {
        m_dbg_dynamic_plot->AddMarker( CStatGraph::stHor,
                                       ( *i )[ 1 ] / rpm_pow_max_ratio,
                                       D3DCOLOR_XRGB( 127, 0, 0 ) );
        m_dbg_dynamic_plot->AddMarker( CStatGraph::stHor,
                                       ( *i )[ 2 ] / rpm_pow_max_ratio,
                                       D3DCOLOR_XRGB( 0, 0, 127 ) );
    }
    //////////////////////////////
    e_state_drive = state;
...
void CCar::DbgUbdateCl() {
    if ( m_pPhysicsShell && OwnerActor() &&
        ...
        if ( ph_dbg_draw_mask.test( phDbgDrawCarDynamics ) ) {
            ...
            UI().Font().pFontStat->OutNext(
                "Power:      [%3.2f]",
                m_current_engine_power / ( 0.8f * 1000.f ) );
            UI().Font().pFontStat->OutNext(
                "rpm:      [%3.2f]",
                m_current_rpm / ( 1.f / 60.f * 2.f * M_PI ) );
            UI().Font().pFontStat->OutNext( "wheel torque:      [%3.2f]",
                                            RefWheelCurTorque() );
            UI().Font().pFontStat->OutNext( "engine torque:      [%3.2f]",
                                            EngineCurTorque() );
            ...
            // HUD().pFontStat->OutNext("Vel Magnitude:
            // [%3.2f]",m_PhysicMovementControl->GetVelocityMagnitude());
            // HUD().pFontStat->OutNext("Vel Actual:
            // [%3.2f]",m_PhysicMovementControl->GetVelocityActual());
        }

        if ( ph_dbg_draw_mask.test( phDbgDrawCarPlots ) && b_plots ) {
            float cur_torque = EngineCurTorque();
            m_dbg_dynamic_plot->AppendItem( m_current_engine_power,
                                            D3DCOLOR_XRGB( 0, 0, 255 ) );
            m_dbg_dynamic_plot->AppendItem( cur_torque / torq_pow_max_ratio,
                                            D3DCOLOR_XRGB( 0, 255, 0 ), 1 );
            m_dbg_dynamic_plot->AppendItem( m_current_rpm / rpm_pow_max_ratio,
                                            D3DCOLOR_XRGB( 255, 0, 0 ), 2 );

            m_dbg_dynamic_plot->UpdateMarkerPos(
                0, m_current_rpm / rpm_pow_max_ratio );
            ...
            m_dbg_power_rpm.UpdateMarker(
                3, m_gear_ratious[ m_current_transmission_num ][ 2 ] );
            m_dbg_power_rpm.UpdateMarker(
                4, m_gear_ratious[ m_current_transmission_num ][ 1 ] );
