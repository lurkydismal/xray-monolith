...
CInventory::CInventory() {
    ...
    while ( pSettings->line_exist( "inventory", slot_persistent ) &&
        ...
        m_slots[ k ].m_bPersistent =
            !!pSettings->r_bool( "inventory", slot_persistent );
...
void CInventory::Take( CGameObject* pObj,
                       bool bNotActivate,
    ...
    pIItem->AllowTrade();
    // if net_Import for pObj arrived then the pObj will pushed to CrPr list
    // (correction prediction) usually net_Import arrived for objects that not
    // has a parent object.. for unknown reason net_Import arrived for object
    // that has a parent, so correction prediction schema will crash
    ...
    switch ( pIItem->m_ItemCurrPlace.type ) {
        ...
        case eItemPlaceRuck:
            ...
            break;
        case eItemPlaceSlot:
            result = Slot( pIItem->m_ItemCurrPlace.slot_id, pIItem,
                           bNotActivate, strict_placement );
            if ( !result )
                pIItem->m_ItemCurrPlace.type = eItemPlaceUndefined;
#ifdef DEBUG
            if ( !result )
    ...
    if ( pIItem->CurrPlace() == eItemPlaceUndefined ) {
        if ( !pIItem->RuckDefault() ) {
            if ( CanPutInSlot( pIItem, pIItem->BaseSlot() ) ) {
                result = Slot( pIItem->BaseSlot(), pIItem, bNotActivate,
                               strict_placement );
                VERIFY( result );
    ...
    if ( CurrentGameUI() ) {
        ...
        } else if ( CurrentGameUI()->GetActorMenu().GetMenuMode() ==
                    mmDeadBodySearch ) {
...
bool CInventory::DropItem( CGameObject* pObj,
    ...
    switch ( pIItem->CurrPlace() ) {
        case eItemPlaceBelt: {
            ...
            TIItemContainer::iterator temp_iter =
                std::find( m_belt.begin(), m_belt.end(), pIItem );
        ...
        case eItemPlaceRuck: {
            ...
            TIItemContainer::iterator temp_iter =
                std::find( m_ruck.begin(), m_ruck.end(), pIItem );
...
#ifdef DEBUG
                        Msg( "---DropItem activating slot [-1], forced, "
                             "Frame[%d]",
                             Device.dwFrame );
#endif // #ifdef DEBUG
...
#ifdef DEBUG
                        Msg( "---DropItem activating slot [-1], Frame[%d]",
                             Device.dwFrame );
#endif // #ifdef DEBUG
    ...
    TIItemContainer::iterator it =
        std::find( m_all.begin(), m_all.end(), pIItem );
    if ( it != m_all.end() )
        m_all.erase( it );
    else
...
bool CInventory::Slot( u16 slot_id,
                       PIItem pIItem,
                       bool bNotActivate,
                       bool strict_placement ) {
    VERIFY( pIItem );
    ...
    if ( !IsGameTypeSingle() ) {
        u16 real_parent = pIItem->object().H_Parent()
                              ? pIItem->object().H_Parent()->ID()
                              : u16( -1 );
        if ( GetOwner()->object_id() != real_parent ) {
            Msg( "! WARNING: CL: actor [%d] tries to place to slot not own "
                 "item [%d], that has parent [%d]",
    ...
    if ( !strict_placement && !CanPutInSlot( pIItem, slot_id ) ) {
        ...
        Msg( "there is item %s[%d,%x] in slot %d[%d,%x]",
             ...
             ItemFromSlot( pIItem->GetSlot() ), pIItem->GetSlot(),
             pIItem->object().ID(), pIItem );
#endif
        //.		if(m_slots[pIItem->GetSlot()].m_pIItem == pIItem &&
        //!bNotActivate ) .			Activate(pIItem->GetSlot());
    ...
    TIItemContainer::iterator it_ruck =
        std::find( m_ruck.begin(), m_ruck.end(), pIItem );
    TIItemContainer::iterator it_belt =
        std::find( m_belt.begin(), m_belt.end(), pIItem );
    if ( !IsGameTypeSingle() ) {
        if ( it_ruck != m_ruck.end() ) {
            ...
            R_ASSERT2( GetOwner()->object_id() == real_parent,
                       make_string( "! ERROR: CL: actor [%d] doesn't contain "
                                    "[%d], real parent is [%d]",
                                    GetOwner()->object_id(),
...
#ifdef MP_LOGGING
        Msg( "--- Actor [%d] places to slot item [%d]", GetOwner()->object_id(),
             pIItem->object_id() );
...
#ifdef DEBUG
        Msg( "---To Slot: activating slot [%d], Frame[%d]", slot_id,
             Device.dwFrame );
#endif // #ifdef DEBUG
        Activate( slot_id );
    }
...
bool CInventory::Ruck( PIItem pIItem, bool strict_placement ) {
    ...
    if ( !IsGameTypeSingle() ) {
        u16 real_parent = pIItem->object().H_Parent()
                              ...
                              : u16( -1 );
        if ( GetOwner()->object_id() != real_parent ) {
            Msg( "! WARNING: CL: actor [%d] tries to place to ruck not own "
                 "item [%d], that has parent [%d]",
                 GetOwner()->object_id(), pIItem->object_id(), real_parent );
            return false;
        }
    ...
    if ( in_slot ) {
        ...
        if ( !IsGameTypeSingle() ) {
            u16 item_parent_id = pIItem->object().H_Parent()
                                     ? pIItem->object().H_Parent()->ID()
                                     : u16( -1 );
            u16 inventory_owner_id = GetOwner()->object_id();
            R_ASSERT2(
                item_parent_id == inventory_owner_id,
                make_string( "! ERROR: CL: Actor[%d] tries to place to ruck "
                             "not own item [%d], real item owner is [%d]",
                             inventory_owner_id, pIItem->object_id(),
                             item_parent_id )
                    .c_str() );
#ifdef MP_LOGGING
            Msg( "--- Actor [%d] place to ruck item [%d]", inventory_owner_id,
                 pIItem->object_id() );
#endif
        }
    }
...
void CInventory::Activate( u16 slot, bool bForce ) {
    ...
    if ( GetActiveSlot() == slot ||
         ( GetNextActiveSlot() == slot && !bForce ) ) {
        //		if (m_iNextActiveSlot != slot) {
        //			LPCSTR const name = smart_cast<CGameObject
        //const*>(m_pOwner)->cName().c_str(); 			if (
        //!xr_strcmp("jup_b43_stalker_assistant_pri6695", name) ) 				LogStackTrace
        //(""); 			Msg				( "[%6d][%s]
        //CInventory::Activate changing next active slot to %d",
        //Device.dwTimeGlobal, name, slot );
        //		}
        m_iNextActiveSlot = slot;
#ifdef DEBUG
        //		Msg("--- There's no need to activate slot [%d], next
...
#ifdef DEBUG
    //	Msg("--- Activating slot [%d], inventory owner: [%s], Frame[%d]", slot,
    //m_pOwner->Name(), Device.dwFrame);
#endif // #ifdef DEBUG
    ...
    if ( GetActiveSlot() == NO_ACTIVE_SLOT ) {
        if ( tmp_item ) {
            //			if ( m_iNextActiveSlot != slot) {
            //				LPCSTR const name =
            //smart_cast<CGameObject const*>(m_pOwner)->cName().c_str(); 				if (
            //!xr_strcmp("jup_b43_stalker_assistant_pri6695", name) )
            //					LogStackTrace	("");
            //				Msg				(
            //"[%6d][%s] CInventory::Activate changing next active slot2 to %d",
            //Device.dwTimeGlobal, name, slot );
            //			}
            m_iNextActiveSlot = slot;
        } else {
    ...
    // активный слот задействован
...
#ifdef DEBUG
            //			Msg("--- Inventory owner [%s]: send deactivate
            //item [%s]", m_pOwner->Name(), active_item->NameItem());
#endif         // #ifdef DEBUG
        } else // in case where weapon is going to destroy
        {
            ...
            //!				LPCSTR const name =
            //!smart_cast<CGameObject const*>(m_pOwner)->cName().c_str();
            //				if (
            //!xr_strcmp("jup_b43_stalker_assistant_pri6695", name) )
            //					LogStackTrace	("");
            //!				Msg
            //!("[%6d][%s] CInventory::Activate changing active slot from %d to
            //!%d", Device.dwTimeGlobal, name, m_iActiveSlot, slot );
            //!			}
        ...
        //			LPCSTR const name = smart_cast<CGameObject
        //const*>(m_pOwner)->cName().c_str(); 			if (
        //!xr_strcmp("jup_b43_stalker_assistant_pri6695", name) && !slot )
        //				LogStackTrace	("");
        //			Msg				( "[%6d][%s]
        //CInventory::Activate changing next active slot3 to %d",
        //Device.dwTimeGlobal, name, slot );
        //		}
        m_iNextActiveSlot = slot;
    }
        ...
        if ( m_iActiveSlot != m_iNextActiveSlot ) {
            ...
            if ( Level().CurrentViewEntity() == pActor_owner ) {
                if ( ( m_iNextActiveSlot != NO_ACTIVE_SLOT ) &&
                     ...
                     !g_player_hud->allow_activation(
                         ItemFromSlot( m_iNextActiveSlot )->cast_hud_item() ) )
                    return;
            }
            if ( ActiveItem() ) {
                ...
                if ( !hi->IsHidden() ) {
                    if ( hi->GetState() == CHUDState::eIdle &&
                         hi->GetNextState() == CHUDState::eIdle )
            ...
            auto prev_obj =
                ActiveItem() ? ActiveItem()->object().lua_game_object() : NULL;
            auto prev_slot = m_iActiveSlot;
            m_iActiveSlot = GetNextActiveSlot();
            auto obj =
                ActiveItem() ? ActiveItem()->object().lua_game_object() : NULL;
            if ( ai().script_engine().functor( "_G.CActor_OnChangedSlot",
                                               funct ) ) {
                funct( m_iActiveSlot, obj, prev_slot, prev_obj );
            }
        } else if ( ( GetNextActiveSlot() != NO_ACTIVE_SLOT ) && ActiveItem() &&
...
PIItem CInventory::Same( const PIItem pIItem, bool bSearchRuck ) const {
    ...
    for ( TIItemContainer::const_iterator it = list.begin(); list.end() != it;
          ++it ) {
...
PIItem CInventory::SameSlot( const u16 slot,
                             PIItem pIItem,
                             bool bSearchRuck ) const {
    if ( slot == NO_ACTIVE_SLOT )
        return NULL;
...
float CInventory::CalcTotalWeight() {
    float weight = 0;
    for ( TIItemContainer::const_iterator it = m_all.begin(); m_all.end() != it;
          ++it )
        weight += ( *it )->Weight();
...
u32 CInventory::dwfGetSameItemCount( LPCSTR caSection, bool SearchAll ) {
    ...
    for ( TIItemContainer::iterator l_it = l_list.begin(); l_list.end() != l_it;
          ++l_it ) {
...
u32 CInventory::dwfGetGrenadeCount( LPCSTR caSection, bool SearchAll ) {
    ...
    for ( TIItemContainer::iterator l_it = l_list.begin(); l_list.end() != l_it;
          ++l_it ) {
        PIItem l_pIItem = *l_it;
        if ( l_pIItem->object().CLS_ID == CLSID_GRENADE_F1 ||
...
bool CInventory::Eat( PIItem pIItem ) {
...
#ifdef MP_LOGGING
    Msg( "--- Actor [%d] use or eat [%d][%s]", entity_alive->ID(),
         pItemToEat->object().ID(), pItemToEat->object().cNameSect().c_str() );
#endif // MP_LOGGING
    /*
            ...
            {
                    if
       (!funct(smart_cast<CGameObject*>(pItemToEat->object().H_Parent())->lua_game_object(),
    ...
    */
    if ( Actor()->m_inventory == this ) {
        ...
        if ( pItemToEat->IsUsingCondition() &&
             pItemToEat->GetRemainingUses() < 1 && pItemToEat->CanDelete() )
...
CInventoryItem* CInventory::tpfGetObjectByIndex( int iIndex ) {
    if ( ( iIndex >= 0 ) && ( iIndex < ( int )m_all.size() ) ) {
        TIItemContainer& l_list = m_all;
        int i = 0;
        for ( TIItemContainer::iterator l_it = l_list.begin();
              l_list.end() != l_it; ++l_it, ++i )
            if ( i == iIndex )
                return ( *l_it );
    } else {
...
bool CInventory::CanTakeItem( CInventoryItem* inventory_item ) const {
    ...
    for ( it = m_all.begin(); it != m_all.end(); it++ )
        if ( ( *it )->object().ID() == inventory_item->object().ID() )
            break;
    VERIFY3( it == m_all.end(), "item already exists in inventory",
             *inventory_item->object().cName() );

    CActor* pActor = smart_cast< CActor* >( m_pOwner );
    // актер всегда может взять вещь
    if ( !pActor && ( TotalWeight() + inventory_item->Weight() >
                      m_pOwner->MaxCarryWeight() ) )
        return false;
...
u32 CInventory::BeltWidth() const {
    ...
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CInventory_BeltWidth", funct ) ) {
        artefact_count = funct( artefact_count );
    }
    return artefact_count; // m_iMaxBelt;
}

void CInventory::AddAvailableItems( TIItemContainer& items_container,
                                    bool for_trade,
                                    bool bOverride ) const {
    for ( TIItemContainer::const_iterator it = m_ruck.begin();
          m_ruck.end() != it; ++it ) {
        PIItem pIItem = *it;
        if ( !for_trade || pIItem->CanTrade() ) {
            if ( bOverride ) {
                ::luabind::functor< bool > funct;
                if ( ai().script_engine().functor(
                         "actor_menu_inventory.CInventory_ItemAvailableToTrade",
                         funct ) ) {
                    if ( !funct(
                             m_pOwner->cast_game_object()->lua_game_object(),
    ...
    if ( m_bBeltUseful ) {
        for ( TIItemContainer::const_iterator it = m_belt.begin();
              m_belt.end() != it; ++it ) {
            PIItem pIItem = *it;
            if ( !for_trade || pIItem->CanTrade() ) {
                if ( bOverride ) {
                    ::luabind::functor< bool > funct;
                    if ( ai().script_engine().functor(
                             "actor_menu_inventory.CInventory_"
                             "ItemAvailableToTrade",
                             funct ) ) {
                        if ( !funct( m_pOwner->cast_game_object()
                                         ->lua_game_object(),
                                     pIItem->cast_game_object()
                                         ->lua_game_object() ) )
                            continue;
                    }
                }
...
void CInventory::TryActivatePrevSlot() {
    ...
    if ( ( ( ActiveSlot == NO_ACTIVE_SLOT ) ||
        ...
        if ( prev_active_item && !IsSlotBlocked( prev_active_item ) &&
             m_slots[ PrevActiveSlot ].CanBeActivated() ) {
#ifndef MASTER_GOLD
            Msg( "Set slots blocked: activating prev slot [%d], Frame[%d]",
                 PrevActiveSlot, Device.dwFrame );
...
void CInventory::TryDeactivateActiveSlot() {
    ...
    PIItem active_item =
        ( ActiveSlot != NO_ACTIVE_SLOT ) ? ItemFromSlot( ActiveSlot ) : NULL;
    PIItem next_active_item = ( NextActiveSlot != NO_ACTIVE_SLOT )
                                  ? ItemFromSlot( NextActiveSlot )
                                  : NULL;

    if ( active_item && ( IsSlotBlocked( active_item ) ||
                          !m_slots[ ActiveSlot ].CanBeActivated() ) ) {
#ifndef MASTER_GOLD
        Msg( "Set slots blocked: activating slot [-1], Frame[%d]",
             Device.dwFrame );
#endif // #ifndef MASTER_GOLD
        ...
        SetPrevActiveSlot( ActiveSlot );
                ...
                ( IsSlotBlocked( next_active_item ) ||
                  !m_slots[ NextActiveSlot ].CanBeActivated() ) ) {
        Activate( NO_ACTIVE_SLOT );
