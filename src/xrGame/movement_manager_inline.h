...
IC void CMovementManager::set_body_orientation(
    const CBoneRotation& orientation ) {
    m_body = orientation;
}

IC const CMovementManager::CBoneRotation& CMovementManager::body_orientation()
    const {
    return ( m_body );
}

template < typename T >
IC bool CMovementManager::accessible( T position_or_vertex_id,
                                      float radius ) const {
...
IC CMovementManager::CGameVertexParams* CMovementManager::base_game_params()
    const {
    return ( m_base_game_selector );
}

IC CMovementManager::CBaseParameters* CMovementManager::base_level_params()
    const {
    return ( m_base_level_selector );
}

IC CMovementManager::CGameLocationSelector& CMovementManager::game_selector()
    const {
