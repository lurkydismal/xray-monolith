...
void CHM_Static::Update() {
    ...
    // *****	perform TASKs
    for ( int taskid = 0; ( taskid < tasksPerFrame ) && ( !task.empty() );
          ++taskid ) {
        Slot* S = task.back();
        task.pop_back();
        S->bReady = TRUE;
        ...
        bb.max.set( bb.min.x + dhm_size, view.y + limit_up,
                    bb.min.z + dhm_size );
        bb.grow( EPS_L );

        // Select polygons
        XRC.BBoxMode( 0 ); // BBOX_TRITEST
        XRC.BBoxCollide( precalc_identity,
        ...
        for ( int z = 0; z < dhm_precision; ++z ) {
            for ( int x = 0; x < dhm_precision; ++x ) {
                float rx = ( float( x ) / float( dhm_precision ) ) * dhm_size +
                           bb.min.x;
                float rz = ( float( z ) / float( dhm_precision ) ) * dhm_size +
                           bb.min.z;
                float ry = bb.min.y - 5;
                Fvector pos;
                ...
                for ( u32 tid = 0; tid < polys.size(); ++tid ) {
                    if ( RAPID::TestRayTri( pos, dir, polys[ tid ].v, r_u, r_v,
                                            r_range, TRUE ) ) {
                        if ( r_range >= 0 ) {
