...
IC const xr_vector< CSoundPlayer::CSoundSingle >& CSoundPlayer::playing_sounds()
    const {
    ...
    if ( !only_playing ) {
        for ( ; I != E; ++I )
            if ( ( *I ).m_sound->_feedback() ||
                 ( ( *I ).m_start_time <= Device.dwTimeGlobal ) )
                ++count;
    } else {
        for ( ; I != E; ++I )
            if ( ( *I ).m_sound->_feedback() )
...
IC ref_sound* CSoundPlayer::CSoundCollection::add( ESoundTypes type,
                                                   LPCSTR name ) const {
