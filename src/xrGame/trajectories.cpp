...
static void trajectory_get_position( Fvector& result,
                                     const Fvector& start_position,
                                     const Fvector& velocity,
                                     const Fvector& gravity,
                                     const float& time ) {
    // result = start_position + velocity*t + gravity*t^2/2
    result.mad( start_position, velocity, time )
...
static float trajectory_pick_error( float low,
                                    float high,
    ...
    Fvector max_error;
    trajectory_get_position( max_error, position, velocity, gravity,
                             max_error_time );
...
static float trajectory_select_pick_time( const float& start_low,
                                          float high,
                                          const Fvector& start,
                                          const Fvector& velocity,
                                          const Fvector& gravity,
    ...
    while ( !fsimilar( low, high, time_epsilon ) ) {
        float distance = trajectory_pick_error( start_low, check_time, start,
                                                velocity, gravity );
...
static bool trajectory_check_collision(
    float low,
    float high,
    ...
    collide::ray_defs ray_defs( start, start_to_target, distance, CDB::OPT_CULL,
                                collide::rqtBoth );
    ...
    if ( box_size.magnitude() <= epsilon )
        Level().ObjectSpace.RayQuery( temp_rq_results, ray_defs,
                                      trajectory_query_callback, &range, NULL,
                                      self_object );
    else {
        Fvector box_center;
        ...
        Fvector box_y_axis;
        Fvector box_x_axis;
        if ( _abs( box_z_axis.x ) > epsilon ||
             _abs( box_z_axis.z ) > epsilon ) {
        ...
        box_result = !Level().ObjectSpace.BoxQuery(
            box_center, box_z_axis, box_y_axis, box_size, out_collide_tris );
    }
...
bool trajectory_intersects_geometry(
    ...
    const Fvector gravity =
        Fvector().set( 0.f, -physics_world()->Gravity(), 0.f );
    const float epsilon = .1f;

    float low = 0.f;
    const float high = trajectory_time;
    for ( ;; ) {
        float time = trajectory_select_pick_time(
            low, high, trajectory_start, trajectory_start, gravity, epsilon );

        if ( !trajectory_check_collision(
                 low, time, trajectory_start, trajectory_velocity, gravity,
                 self_object, ignored_object, collide_position, temp_rq_results,
                 box_size, out_trajectory_picks, out_collide_tris ) ) {
            if ( fsimilar( time, high ) &&
                 collide_position.similar( trajectory_end, .2f ) )
                break;
