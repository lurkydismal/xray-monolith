...
IC const CVisionParameters& CVisualMemoryManager::current_state() const {
    if ( m_stalker ) {
        return ( m_stalker->movement().mental_state() == eMentalStateDanger )
                   ? m_danger
                   : m_free;
    } else if ( m_object ) {
...
u32 CVisualMemoryManager::visible_object_time_last_seen(
    const CObject* object ) const {
    VISIBLES::iterator I =
        std::find( m_objects->begin(), m_objects->end(), object_id( object ) );
    if ( I != m_objects->end() )
...
bool CVisualMemoryManager::visible_right_now(
    ...
    VISIBLES::const_iterator I = std::find( objects().begin(), objects().end(),
...
float CVisualMemoryManager::object_visible_distance(
    const CGameObject* game_object,
    float& object_distance ) const {
    Fvector eye_position = Fvector().set( 0.f, 0.f, 0.f ), eye_direction;
    Fmatrix eye_matrix;
    float object_range = flt_max, object_fov = flt_max;

    if ( m_object ) {
        eye_matrix = smart_cast< IKinematics* >( m_object->Visual() )
                         ->LL_GetTransform( u16( m_object->eye_bone ) );
        ...
        if ( m_stalker ) {
            eye_direction.setHP( -m_stalker->movement().m_head.current.yaw,
            ...
            // if its a monster
            const MonsterSpace::SBoneRotation& head_orient =
                m_object->head_orientation();
            eye_direction.setHP( -head_orient.current.yaw,
                                 -head_orient.current.pitch );
        }
    } else {
        Fvector dummy;
        float _0, _1;
        m_client->camera( eye_position, eye_direction, dummy, object_fov, _0,
                          _1, object_range );
    }
    ...
    if ( m_object )
        m_object->update_range_fov( object_range, object_fov,
                                    m_object->eye_range,
    ...
    float distance =
        ( 1.f - alpha / fov ) * ( max_view_distance - min_view_distance ) +
        min_view_distance;
...
float CVisualMemoryManager::get_object_velocity(
    ...
    // Alundaio: no need to check velocity on anything but stalkers, mutants and
    // actor
    ...
    if ( ( game_object->ps_Size() < 2 ) ||
         ( not_yet_visible_object.m_prev_time ==
           game_object->ps_Element( game_object->ps_Size() - 2 ).dwTime ) )
        return ( 0.f );

    CObject::SavedPosition pos0 =
        game_object->ps_Element( game_object->ps_Size() - 2 );
    CObject::SavedPosition pos1 =
        game_object->ps_Element( game_object->ps_Size() - 1 );
...
float CVisualMemoryManager::get_visible_value( const CGameObject* game_object,
    ...
    if ( ai().script_engine().functor(
             "visual_memory_manager.get_visible_value", funct ) )
        return ( funct( m_object ? m_object->lua_game_object() : 0,
                        game_object ? game_object->lua_game_object() : 0,
                        time_delta, current_state().m_time_quant, luminocity,
                        current_state().m_velocity_factor, object_velocity,
...
CNotYetVisibleObject* CVisualMemoryManager::not_yet_visible_object(
    const CGameObject* game_object ) {
    START_PROFILE( "Memory Manager/visuals/not_yet_visible_object" )
    xr_vector< CNotYetVisibleObject >::iterator I = std::find_if(
        m_not_yet_visible_objects.begin(), m_not_yet_visible_objects.end(),
...
void CVisualMemoryManager::add_not_yet_visible_object(
    const CNotYetVisibleObject& not_yet_visible_object ) {
    xr_vector< CNotYetVisibleObject >::iterator I = std::find_if(
        m_not_yet_visible_objects.begin(), m_not_yet_visible_objects.end(),
...
u32 CVisualMemoryManager::get_prev_time(
    const CGameObject* game_object ) const {
...
bool CVisualMemoryManager::visible( const CGameObject* game_object,
    ...
    if ( distance < object_distance ) {
        if ( object ) {
            ...
            return ( object->m_value >=
                     current_state().m_visibility_threshold );
        }
        return ( false );
    }
    ...
    if ( !object ) {
        ...
        new_object.m_value = get_visible_value(
            game_object, distance, object_distance, time_delta,
    ...
    object->m_update_time = Device.dwTimeGlobal;
    object->m_value +=
        get_visible_value( game_object, distance, object_distance, time_delta,
                           get_object_velocity( game_object, *object ),
                           object_luminocity( game_object ) );
    clamp( object->m_value, 0.f,
           current_state().m_visibility_threshold + EPS_L );
...
#ifndef MASTER_GOLD
    if ( smart_cast< CActor const* >( object ) &&
         psAI_Flags.test( aiIgnoreActor ) ) {
        return true;
    } else
#endif // MASTER_GOLD

        if ( CBaseMonster const* const monster =
                 smart_cast< CBaseMonster const* >( object ) ) {
...
void CVisualMemoryManager::add_visible_object( const CObject* object,
    ...
    if ( !game_object ||
         ( !fictitious && !visible( game_object, time_delta ) ) )
        return;
    //	STOP_PROFILE
    ...
    if ( m_objects->end() == J ) {
        ...
        if ( m_max_object_count <= m_objects->size() ) {
            xr_vector< CVisibleObject >::iterator I =
                std::min_element( m_objects->begin(), m_objects->end(),
...
void CVisualMemoryManager::add_visible_object(
    const CVisibleObject visible_object ) {
...
void CVisualMemoryManager::check_visibles() const {
    ...
    for ( ; I != E; ++I ) {
        ...
        xr_vector< Feel::Vision::feel_visible_Item >::iterator i =
            m_object->feel_visible.begin();
        xr_vector< Feel::Vision::feel_visible_Item >::iterator e =
            m_object->feel_visible.end();
        for ( ; i != e; ++i )
...
bool CVisualMemoryManager::visible( u32 _level_vertex_id,
                                    float yaw,
                                    float eye_fov ) const {
    Fvector direction;
    direction.sub( ai().level_graph().vertex_position( _level_vertex_id ),
                   m_object->Position() );
...
float CVisualMemoryManager::feel_vision_mtl_transp( CObject* O, u32 element ) {
    ...
    if ( O ) {
        ...
        if ( 0 != V ) {
            ...
            vis = GMLib.GetMaterialByIdx( B.game_mtl_idx )
...
struct CVisibleObjectPredicateEx {
    ...
    bool operator()( const MemorySpace::CNotYetVisibleObject&
                         not_yet_visible_object ) const {
        if ( !m_object )
...
CVisibleObject* CVisualMemoryManager::visible_object(
    const CGameObject* game_object ) {
    if ( !m_objects )
        return nullptr;
    ...
    VISIBLES::iterator I =
        std::find_if( m_objects->begin(), m_objects->end(),
                      CVisibleObjectPredicateEx( game_object ) );
    if ( I == m_objects->end() )
        return ( 0 );
    return ( &*I );
}
...
void CVisualMemoryManager::update( float time_delta ) {
    ...
    START_PROFILE( "Memory Manager/visuals/update/make_invisible" ) {
        ...
        xr_vector< CVisibleObject >::iterator E = m_objects->end();
        for ( ; I != E; ++I )
            if ( ( *I ).m_level_time + current_state().m_still_visible_time <
                 Device.dwTimeGlobal )
    ...
    START_PROFILE( "Memory Manager/visuals/update/make_not_yet_visible" ) {
        ...
        xr_vector< CNotYetVisibleObject >::iterator E =
    ...
    // verifying if object is online
    {
        m_not_yet_visible_objects.erase(
            std::remove_if( m_not_yet_visible_objects.begin(),
                            m_not_yet_visible_objects.end(),
                            SRemoveOfflinePredicate() ),
            m_not_yet_visible_objects.end() );
    }
    ...
    if ( m_object && g_actor && m_object->is_relation_enemy( Actor() ) ) {
        ...
        if ( I != m_not_yet_visible_objects.end() ) {
            SetActorVisibility(
                m_object->ID(),
                clampr( ( *I ).m_value / visibility_threshold(), 0.f, 1.f ) );
        } else
...
static inline bool is_object_valuable_to_save(
    CCustomMonster const* const self,
    MemorySpace::CVisibleObject const& object ) {
    CEntityAlive const* const entity_alive =
        smart_cast< CEntityAlive const* >( object.m_object );
    if ( !entity_alive )
        return false;
