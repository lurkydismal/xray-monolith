...
bool stalker_movement_manager_obstacles::simulate_path_navigation() {
    ...
    while ( !detail().completed( current_position,
...
void stalker_movement_manager_obstacles::save_current_state() {
    ...
    m_detail_current_index = detail().path().empty()
                                 ? u32( -1 )
...
IC void stalker_movement_manager_obstacles::remove_query_objects(
...
void stalker_movement_manager_obstacles::build_level_path() {
    ...
    save_current_state();
    m_static_obstacles.inactive_query().copy(
        m_static_obstacles.active_query() );
    m_static_obstacles.inactive_query().update_objects( object().Position(),
                                                        10000.f );

#ifndef MASTER_GOLD
    if ( !psAI_Flags.test( aiObstaclesAvoidingStatic ) )
        m_dynamic_obstacles.inactive_query().copy(
