...
#define TEMPLATE_SPECIALIZATION                               \
    template < typename _data_type, typename _vertex_id_type, \
               typename _graph_type >

#define CSGraphVertex CVertex< _data_type, _vertex_id_type, _graph_type >

TEMPLATE_SPECIALIZATION
IC CSGraphVertex::CVertex( const _data_type& data,
...
TEMPLATE_SPECIALIZATION
IC const typename CSGraphVertex::_edge_type* CSGraphVertex::edge(
    const _vertex_id_type& vertex_id ) const {
    typename EDGES::const_iterator I =
        std::find( edges().begin(), edges().end(), vertex_id );
    if ( m_edges.end() == I )
        return ( 0 );
    return ( &*I );
}
...
TEMPLATE_SPECIALIZATION
IC typename CSGraphVertex::_edge_type* CSGraphVertex::edge(
    const _vertex_id_type& vertex_id ) {
    typename EDGES::iterator I =
        std::find( m_edges.begin(), m_edges.end(), vertex_id );
    if ( m_edges.end() == I )
        return ( 0 );
    return ( &*I );
}
...
TEMPLATE_SPECIALIZATION
IC void CSGraphVertex::add_edge( CVertex* vertex,
                                 const _edge_weight_type& edge_weight ) {
    //	EDGES::iterator			I =
    //std::find(m_edges.begin(),m_edges.end(),vertex->vertex_id()); 	VERIFY
    //(m_edges.end() == I);
...
IC void CSGraphVertex::on_edge_addition( CVertex* vertex ) {
    ...
    //std::find(m_vertices.begin(),m_vertices.end(),vertex); 	VERIFY
    //(I == m_vertices.end());
