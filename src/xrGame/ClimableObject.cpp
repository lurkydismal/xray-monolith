...
CClimableObject::CClimableObject()
    : m_pStaticShell( NULL ), m_material( u16( -1 ) ), m_radius( 0.0f ) {}
...
BOOL CClimableObject::net_Spawn( CSE_Abstract* DC ) {
    ...
    m_radius = _max( _max( m_box.m_halfsize.x, m_box.m_halfsize.y ),
                     m_box.m_halfsize.z );
    ...
    SORT(
        b._11, m_axis.set( XFORM().i );
        m_axis.mul( m_box.m_halfsize.x ), m_side.set( XFORM().i );
        m_side.mul( m_box.m_halfsize.x ), m_norm.set( XFORM().i );
        ...
        m_axis.mul( m_box.m_halfsize.y ), m_side.set( XFORM().j );
        m_side.mul( m_box.m_halfsize.y ), m_norm.set( XFORM().j );
        ...
        m_axis.mul( m_box.m_halfsize.z ), m_side.set( XFORM().k );
        m_side.mul( m_box.m_halfsize.z ), m_norm.set( XFORM().k );
    ...
    m_pStaticShell =
        P_BuildLeaderGeomShell( this, ObjectContactCallback, m_box );
...
bool CClimableObject::InTouch( CPHCharacter* actor ) const {
    ...
    return ( DDToPlain( actor, dir ) <
                 foot_radius + m_norm.magnitude() + normal_tolerance &&
...
bool CClimableObject::InRange( CPHCharacter* actor ) const {
    ...
    return AxDistToLowerP( actor ) > -down_leader_extension_tolerance &&
           AxDistToUpperP( actor ) + actor->FootRadius() >
               -up_leader_extension_tolerance;
}

bool CClimableObject::BeforeLadder( CPHCharacter* actor,
    ...
    return d.dotproduct( n ) <
           -( width + actor->FootRadius() / 2.f + tolerance );
}
...
void CClimableObject::ObjectContactCallback( bool& do_colide,
                                             bool bo1,
                                             dContact& c,
                                             SGameMtl* /*material_1*/,
                                             SGameMtl* /*material_2*/ ) {
    ...
    if ( usr_data_ch && usr_data_ch->ph_object &&
         usr_data_ch->ph_object->CastType() == CPHObject::tpCharacter )
    ...
    VERIFY( usr_data_lad );
    this_object =
        static_cast< CClimableObject* >( usr_data_lad->ph_ref_object );
    if ( this_object ) {
...
void CClimableObject ::OnRender() {
    if ( !dbg_net_Draw_Flags.test( dbg_draw_climbable ) &&
    ...
    // form.mulA(XFORM());
    Level().debug_renderer().draw_obb( XFORM(), m_box.m_halfsize,
