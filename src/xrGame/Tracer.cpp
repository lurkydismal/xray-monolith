...
CTracer::CTracer() {
    ...
    m_tracer_length_k = READ_IF_EXISTS(
        pSettings, r_float, "bullet_manager", "tracer_length_k",
        1.0f ); // momopate: Allow for customizable tracer length
...
IC void FillSprite_Circle( const Fvector& pos,
                           const float width,
                           const float length,
                           u32 color ) {
    ...
    //	Tri 1
    UIRender->PushPoint( d.x + pos.x, d.y + pos.y, d.z + pos.z, color,
                         t_crcl.min.x, t_crcl.max.y ); // 0.f,1.f);
    UIRender->PushPoint( a.x + pos.x, a.y + pos.y, a.z + pos.z, color,
                         t_crcl.min.x, t_crcl.min.y ); // 0.f,0.f);
    UIRender->PushPoint( c.x + pos.x, c.y + pos.y, c.z + pos.z, color,
                         t_crcl.max.x, t_crcl.max.y ); // 1.f,1.f);
    //	Tri 2
    UIRender->PushPoint( c.x + pos.x, c.y + pos.y, c.z + pos.z, color,
                         t_crcl.max.x, t_crcl.max.y ); // 1.f,1.f);
    UIRender->PushPoint( a.x + pos.x, a.y + pos.y, a.z + pos.z, color,
                         t_crcl.min.x, t_crcl.min.y ); // 0.f,0.f);
    UIRender->PushPoint( b.x + pos.x, b.y + pos.y, b.z + pos.z, color,
                         t_crcl.max.x, t_crcl.min.y ); // 1.f,0.f);
...
IC void FillSprite_Line( const Fvector& pos,
                         const Fvector& dir,
                         const float width,
                         const float length,
                         u32 color ) {
    ...
    //	Tri 1
    UIRender->PushPoint( d.x + pos.x, d.y + pos.y, d.z + pos.z, color,
                         t_tracer.min.x, t_tracer.max.y );
    UIRender->PushPoint( a.x + pos.x, a.y + pos.y, a.z + pos.z, color,
                         t_tracer.min.x, t_tracer.min.y );
    UIRender->PushPoint( c.x + pos.x, c.y + pos.y, c.z + pos.z, color,
                         t_tracer.max.x, t_tracer.max.y );
    //	Tri 2
    UIRender->PushPoint( c.x + pos.x, c.y + pos.y, c.z + pos.z, color,
                         t_tracer.max.x, t_tracer.max.y );
    UIRender->PushPoint( a.x + pos.x, a.y + pos.y, a.z + pos.z, color,
...
void CTracer::Render( const Fvector& pos,
                      ...
                      float speed,
                      bool bActor ) {
    if ( ::Render->ViewBase.testSphere_dirty( ( Fvector& )center,
                                              length * .5f ) ) {
        ...
        if ( bActor ) {
            ...
            //			float f_distance	=
            //Device.vCameraPosition.distance_to(pos);

            FillSprite_Circle( pos, k_speed * width * m_circle_size_k,
                               k_speed * width * m_circle_size_k,
                               m_aColors[ colorID ] );
        }

        FillSprite_Line( center, dir, width * .5f,
                         length * .5f * m_tracer_length_k,
                         m_aColors[ colorID ] );
    }
}
