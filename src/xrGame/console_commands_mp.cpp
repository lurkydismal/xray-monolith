...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        if ( !OnServer() )
            return;
        if ( Level().Server ) {
            Level().Server->game->round_end_reason =
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "clear client net statistic" );
    }
};
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "clear server net statistic" );
    }
};
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        for ( u32 CO = 0; CO < _max( CLObjNum, SVObjNum ); CO++ ) {
            if ( CO < CLObjNum && CO < SVObjNum ) {
                CSE_Abstract* pEntity =
                    Level().Server->ID_to_entity( SObjID[ CO ] );
                CObject* pObj = Level().Objects.net_Find( CObjID[ CO ] );
                char color = ( pObj->ID() == pEntity->ID ) ? '-' : '!';

                Msg( "%c%4d: Client - %20s[%5d] <===> Server - %s [%d]", color,
                     CO + 1, *( pObj->cNameSect() ), pObj->ID(),
                     pEntity->s_name.c_str(), pEntity->ID );
            } else {
                if ( CO < CLObjNum ) {
                    CObject* pObj = Level().Objects.net_Find( CObjID[ CO ] );
                    Msg( "! %2d: Client - %s [%d] <===> Server - "
                         "-----------------",
                         CO + 1, *( pObj->cNameSect() ), pObj->ID() );
                } else {
                    CSE_Abstract* pEntity =
...
struct SearcherClientByName {
    ...
    SearcherClientByName( LPCSTR name ) {
        strncpy_s( player_name, sizeof( player_name ), name,
                   sizeof( player_name ) - 1 );
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        IClient* tmp_client =
            Level().Server->FindClient( SearcherClientByName( PlayerName ) );
        if ( tmp_client &&
             ( tmp_client != Level().Server->GetServerClient() ) ) {
            Msg( "Disconnecting : %s", PlayerName );
            xrClientData* tmpxrclient =
                static_cast< xrClientData* >( tmp_client );
            if ( !tmpxrclient->m_admin_rights.m_has_admin_rights ) {
                Level().Server->DisconnectClient( tmp_client,
                                                  STRING_KICKED_BY_SERVER );
            } else {
...
public:
    CCC_KickPlayerByID( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( !strncmp( args, LAST_PRINTED_PLAYER_STR,
                       sizeof( LAST_PRINTED_PLAYER_STR ) - 1 ) ) {
            client_id = last_printed_player;
        } else {
            u32 tmp_client_id;
            if ( sscanf_s( args, "%u", &tmp_client_id ) != 1 ) {
                Msg( "! ERROR: bad command parameters." );
                Msg( "Kick player. Format: \"sv_kick_id <player session id | "
                     "\'%s\'>\". To receive list of players ids see "
                     "sv_listplayers",
        ...
        if ( tmp_client &&
            ...
            if ( !tmpxrclient->m_admin_rights.m_has_admin_rights ) {
                ...
                Msg( "! Can't disconnect client with admin rights %u",
                     client_id.value() );
            }
        } else {
...
static char const* exclude_raid_from_args( LPCSTR args,
                                           LPSTR dest,
                                           size_t dest_size ) {
...
public:
    CCC_MakeScreenshot( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Execute( LPCSTR args_ ) {
        ...
        ClientID client_id = 0;
        if ( !strncmp( args_, LAST_PRINTED_PLAYER_STR,
                       sizeof( LAST_PRINTED_PLAYER_STR ) - 1 ) ) {
            client_id = last_printed_player;
        } else {
            u32 tmp_client_id;
            if ( sscanf_s( args_, "%u", &tmp_client_id ) != 1 ) {
                Msg( "! ERROR: bad command parameters." );
                Msg( "Make screenshot. Format: \"make_screenshot <player "
                     "session id | \'%s\'> <ban_time_in_sec>\". To receive "
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, make_string( "Make screenshot. Format: \"make_screenshot "
                            "<player session id | \'%s\'> <ban_time_in_sec>\". "
...
public:
    CCC_MakeConfigDump( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Execute( LPCSTR args_ ) {
        ...
        if ( !strncmp( args_, LAST_PRINTED_PLAYER_STR,
            ...
            if ( sscanf_s( args_, "%u", &tmp_client_id ) != 1 ) {
                Msg( "! ERROR: bad command parameters." );
                Msg( "Make screenshot. Format: \"make_config_dump <player "
                     "session id | \'%s\'> <ban_time_in_sec>\". To receive "
                     "list of players ids see sv_listplayers",
                     LAST_PRINTED_PLAYER_STR );
                return;
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, make_string( "Make config dump. Format: \"make_config_dump "
                            "<player session id | \'%s\'> <ban_time_in_sec>\". "
                            "To receive list of players ids see sv_listplayers",
                            LAST_PRINTED_PLAYER_STR )
                   .c_str() );
...
public:
    CCC_SetDemoPlaySpeed( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Set demo play speed (0.0, 8.0]" );
    }
...
protected:
    ...
    bool ParseControlString( LPCSTR args_string ) {
        ...
        sscanf_s( args_string, "%16s %32s", action_name, sizeof( action_name ),
    ...
    inline LPCSTR GetInfoString() {
        return "<roundstart,kill,die,artefacttake,artefactdrop,artefactdeliver>"
               " [player name]";
    }
}; // class DemoPlayControlArgParser

class CCC_DemoPlayPauseOn : public IConsole_Command,
                            public DemoPlayControlArgParser {
public:
    CCC_DemoPlayPauseOn( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Info( TInfo& I ) {
        ...
        STRCONCAT( info_str,
                   "Play demo until specified event (then pause playing). "
                   "Format: mpdemoplay_pause_on ",
...
public:
    CCC_DemoPlayCancelPauseOn( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Cancels mpdemoplay_pause_on." );
    }
}; // class CCC_DemoPlayCancelPauseOn

class CCC_DemoPlayRewindUntil : public IConsole_Command,
                                public DemoPlayControlArgParser {
public:
    CCC_DemoPlayRewindUntil( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Info( TInfo& I ) {
        LPCSTR info_str = NULL;
        STRCONCAT( info_str,
                   "Rewind demo until specified event (then pause playing). "
                   "Format: mpdemoplay_rewind_until ",
                   DemoPlayControlArgParser::GetInfoString() );
        xr_strcpy( I, info_str );
    }
...
public:
    CCC_DemoPlayStopRewind( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Stops rewinding (mpdemoplay_rewind_until)." );
    }
}; // class CCC_DemoPlayStopRewind

class CCC_DemoPlayRestart : public IConsole_Command {
public:
    CCC_DemoPlayRestart( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
...
public:
    CCC_MulDemoPlaySpeed( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Increases demo play speed" );
    };
};

class CCC_DivDemoPlaySpeed : public IConsole_Command {
public:
    CCC_DivDemoPlaySpeed( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Decreases demo play speed" );
    };
};

class CCC_ScreenshotAllPlayers : public IConsole_Command {
public:
    CCC_ScreenshotAllPlayers( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
    ...
    virtual void Execute( LPCSTR args_ ) {
        ...
        if ( !tmp_functor.admin_client ) {
            Msg( "! ERROR: only radmin can make screenshots (use \"ra "
                 "login\")" );
            return;
        }
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I,
                   "Make screenshot of each player in the game. Format: "
                   "\"screenshot_all" );
    }
...
public:
    CCC_ConfigsDumpAll( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
    ...
    virtual void Execute( LPCSTR args_ ) {
        ...
        tmp_functor.admin_client = exclude_command_initiator( args_ );
        if ( !tmp_functor.admin_client ) {
            Msg( "! ERROR: only radmin can make config dumps (use \"ra "
                 "login\")" );
            return;
        }
        Level().Server->ForEachClientDo( tmp_functor );
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I,
                   "Make config dump of each player in the game. Format: "
                   "\"config_dump_all" );
    }
...
public:
    CCC_BanPlayerByCDKEY( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Execute( LPCSTR args_ ) {
        if ( !g_pGameLevel || !Level().Server || !Level().Server->game )
            return;
        game_sv_mp* tmp_sv_game =
            smart_cast< game_sv_mp* >( Level().Server->game );
        if ( !tmp_sv_game )
            return;
        ...
        if ( !strncmp( args_, LAST_PRINTED_PLAYER_STR,
                       sizeof( LAST_PRINTED_PLAYER_STR ) - 1 ) ) {
            client_id = last_printed_player;
            if ( sscanf_s( args_ + sizeof( LAST_PRINTED_PLAYER_STR ), "%d",
                           &ban_time ) != 1 ) {
                Msg( "! ERROR: bad command parameters." );
                Msg( "Ban player. Format: \"sv_banplayer <player session id | "
                     "\'%s\'> <ban_time_in_sec>\". To receive list of players "
                     "ids see sv_listplayers",
                     LAST_PRINTED_PLAYER_STR );
                return;
            }
        } else {
        ...
        IClient* to_disconnect = tmp_sv_game->BanPlayer(
            client_id, ban_time, exclude_command_initiator( args_ ) );
        if ( to_disconnect ) {
            Level().Server->DisconnectClient( to_disconnect,
                                              STRING_KICKED_BY_SERVER );
        } else {
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, make_string( "Ban player. Format: \"sv_banplayer <player "
                            "session id | \'%s\'> <ban_time_in_sec>\". To "
...
public:
    CCC_BanPlayerByCDKEYDirectly( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Execute( LPCSTR args_ ) {
        ...
        char hex_digest[ 64 ];
        s32 ban_time = 0;
        if ( sscanf_s( args_, "%s %i", &hex_digest, sizeof( hex_digest ),
                       &ban_time ) != 2 ) {
            Msg( "! ERROR: bad command parameters." );
            Msg( "Ban player. Format: \"sv_banplayer_by_digest <hex digest> "
                 "<ban_time_in_sec>\". To get player hex digest you can enter: "
                 "sv_listplayers_banned" );
            return;
        }

        tmp_sv_game->BanPlayerDirectly( hex_digest, ban_time,
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I,
            "Ban player by hex digest (CAREFULLY: low level command). Format: "
            "\"sv_banplayer_by_digest <hex digest> <ban_time_in_sec>\". To get "
            "player hex digest you can enter: sv_listplayers_banned" );
    }
};

class CCC_UnBanPlayerByIndex : public IConsole_Command {
public:
    CCC_UnBanPlayerByIndex( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Execute( LPCSTR args_ ) {
        ...
        if ( !strncmp( args_, LAST_PRINTED_PLAYER_BANNED_STR,
                       sizeof( LAST_PRINTED_PLAYER_BANNED_STR ) - 1 ) ) {
            tmp_sv_game->UnBanPlayer( last_printed_player_banned );
        } else {
            size_t player_index = 0;
            if ( sscanf_s( args_, "%u", &player_index ) != 1 ) {
                Msg( "! ERROR: bad command parameters." );
                Msg( " Unban player. Format: \"sv_unbanplayer <banned player "
                     "index | \'%s\'>. To receive list of banned players se "
                     "sv_listplayers_banned",
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, make_string( "Unban player. Format: \"sv_unbanplayer <banned "
                            "player index | \'%s\'>. To receive list of banned "
                            "players see sv_listplayers_banned",
                            LAST_PRINTED_PLAYER_BANNED_STR )
                   .c_str() );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args_ ) {
        ...
        //-----------
        string4096 buff;
        exclude_raid_from_args( args_, buff,
                                sizeof( buff ) ); // xr_strcpy(buff, args_);
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, "Ban Player by IP. Format: \"sb_banplayer_ip <ip address>\"" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        string4096 buff;
        exclude_raid_from_args( args, buff,
                                sizeof( buff ) ); // xr_strcpy(buff, args_);
        if ( !xr_strlen( buff ) )
            return;
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        struct PlayersEnumerator {
            ...
            void operator()( IClient* client ) {
                ...
                ip_address Address;
                DWORD dwPort = 0;
                Level().Server->GetClientAddress( client->ID, Address,
                                                  &dwPort );
                string512 tmp_string;
                xr_sprintf( tmp_string,
                            "- (player session id : %u), (name : %s), (ip: "
                            "%s), (ping: %u);",
                            client->ID.value(), l_pC->ps->getName(),
                            Address.to_string().c_str(), l_pC->ps->ping );
                if ( filter_string ) {
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, "List Players. Format: \"sv_listplayers [ filter string ]\"" );
    }
};
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I,
                   "List of Banned Players. Format: \"sv_listplayers_banned [ "
                   "filter string ]\"" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        if ( !OnServer() )
            return;
        if ( !xr_strlen( args ) ) {
            Msg( "Changing level, version and game type. Arguments: <level "
                 "name> <level version> <game type>" );
            return;
        }
        ...
        sscanf_s( args, "%255s %255s %255s", LevelName, sizeof( LevelName ),
                  LevelVersion, sizeof( LevelVersion ), GameType,
                  sizeof( GameType ) );
        ...
        if ( !bMapFound ) {
            Msg( "! Level [%s][%s] not found for [%s]!", LevelName,
                 LevelVersion, GameType );
            return;
        }
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I,
                   "Changing level, version and game type. Arguments: <level "
                   "name> <level version> <game type>" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        string1024 argsNew;
        xr_sprintf( argsNew, "%s %s %s", Level().name().c_str(),
                    Level().version().c_str(), args );
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        if ( g_pGameLevel && Level().Server && OnServer() &&
             Level().Server->game ) {
            EGameIDs type = Level().Server->game->Type();
            TStatus str;
            xr_sprintf( str, sizeof( str ),
                        "%s  (current game type)  [dm,tdm,ah,cta]",
                        GameTypeToString( type, true ) );
            tips.push_back( str );
        }
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        LevelName[ 0 ] = 0;
        LevelVersion[ 0 ] = 0;
        sscanf_s( args, "%255s %255s", LevelName, sizeof( LevelName ),
                  LevelVersion, sizeof( LevelVersion ) );

        string1024 argsNew;
        xr_sprintf( argsNew, "%s %s %s", LevelName, LevelVersion,
                    Level().Server->game->type_name() );
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, "Changing Game Type. Arguments: <level name> <level version>" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( !c )
            strncpy_s( MapName, sizeof( MapName ), args,
                       sizeof( MapName ) - 1 );
        else {
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "List maps in map rotation list" );
    }
};
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Switch to Next Map in map rotation list" );
    }
};
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Switch to Previous Map in map rotation list" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        game_sv_Deathmatch* gameDM =
            smart_cast< game_sv_Deathmatch* >( Level().Server->game );
        if ( !gameDM )
            return;
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Activating pointed Anomaly set" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        game_sv_ArtefactHunt* g =
            smart_cast< game_sv_ArtefactHunt* >( Level().Server->game );
...
public:
    ...
    virtual void Execute( LPCSTR arguments ) {
        ...
        if ( strstr( arguments, "login" ) == arguments ) {
            string512 user;
            string512 pass;
            if ( 2 == sscanf( arguments + xr_strlen( "login" ) + 1, "%s %s",
                              user, pass ) ) {
                NET_Packet P;
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( Level().Server && Level().Server->game ) {
            game_sv_TeamDeathmatch* tdmGame =
                smart_cast< game_sv_TeamDeathmatch* >( Level().Server->game );
            game_sv_CaptureTheArtefact* ctaGame =
                smart_cast< game_sv_CaptureTheArtefact* >(
                    Level().Server->game );
            if ( tdmGame ) {
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "swap teams for artefacthunt game" );
    }
};
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "Shows current server settings" );
    }
};
...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I, "valid arguments is [info info_full on off]" );
    }
};
...
void register_mp_console_commands() {
    ...
    CMD4( CCC_Integer, "net_cl_icurvesize",
          ( int* )&g_cl_InterpolationMaxPoints, 0, 2000 );
    ...
    CMD4( CCC_Integer, "sv_savescreenshots", &g_sv_mp_save_proxy_screenshots, 0,
          1 );
    ...
    //	CMD4(CCC_Integer,		"sv_statistic_save_auto",
    //&g_bStatisticSaveAuto, 0, 1);
#ifndef MASTER_GOLD
    CMD4( CCC_AuthCheck, "sv_no_auth_check", &g_SV_Disable_Auth_Check, 0, 1 );
#endif // MASTER_GOLD

    CMD4( CCC_Integer, "sv_artefact_spawn_force", &g_SV_Force_Artefact_Spawn, 0,
          1 );

    CMD4( CCC_Integer, "net_dbg_dump_update_write", &g_Dump_Update_Write, 0,
          1 );
...
#ifdef DEBUG
    CMD4( CCC_Integer, "sv_skip_winner_waiting", &g_sv_Skip_Winner_Waiting, 0,
          1 );

    CMD4( CCC_Integer, "sv_wait_for_players_ready",
          &g_sv_Wait_For_Players_Ready, 0, 1 );
#endif
    CMD1( CCC_StartTeamMoney, "sv_startteammoney" );

    CMD4( CCC_Integer, "sv_hail_to_winner_time", &G_DELAYED_ROUND_TIME, 0, 60 );

    //. CMD4(CCC_Integer,		"sv_pending_wait_time",
    //&g_sv_Pending_Wait_Time, 0, 60000);

    CMD4( CCC_Integer, "sv_client_reconnect_time",
          ( int* )&g_sv_Client_Reconnect_Time, 0, 60 );

    CMD4( CCC_SV_Integer, "sv_rpoint_freeze_time",
          ( int* )&g_sv_base_dwRPointFreezeTime, 0, 60000 );
    CMD4( CCC_SV_Integer, "sv_vote_enabled", &g_sv_base_iVotingEnabled, 0,
          0x00FF );

    CMD4( CCC_SV_Integer, "sv_spectr_freefly",
          ( int* )&g_sv_mp_bSpectator_FreeFly, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_spectr_firsteye",
          ( int* )&g_sv_mp_bSpectator_FirstEye, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_spectr_lookat",
          ( int* )&g_sv_mp_bSpectator_LookAt, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_spectr_freelook",
          ( int* )&g_sv_mp_bSpectator_FreeLook, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_spectr_teamcamera",
          ( int* )&g_sv_mp_bSpectator_TeamCamera, 0, 1 );
    CMD4( CCC_Integer, "cl_mpdemosave", ( int* )&g_cl_save_demo, 0, 1 );

    CMD4( CCC_SV_Integer, "sv_vote_participants",
          ( int* )&g_sv_mp_bCountParticipants, 0, 1 );
    CMD4( CCC_SV_Float, "sv_vote_quota", &g_sv_mp_fVoteQuota, 0.0f, 1.0f );
    CMD4( CCC_SV_Float, "sv_vote_time", &g_sv_mp_fVoteTime, 0.5f, 10.0f );

    CMD4( CCC_SV_Integer, "sv_forcerespawn", ( int* )&g_sv_dm_dwForceRespawn, 0,
          3600 ); // sec
    ...
    CMD4( CCC_SV_Integer, "sv_dmgblockindicator",
          ( int* )&g_sv_dm_bDamageBlockIndicators, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_dmgblocktime", ( int* )&g_sv_dm_dwDamageBlockTime,
          0, 600 ); // sec
    CMD4( CCC_SV_Integer, "sv_anomalies_enabled",
          ( int* )&g_sv_dm_bAnomaliesEnabled, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_anomalies_length",
          ( int* )&g_sv_dm_dwAnomalySetLengthTime, 0, 180 ); // min
    CMD4( CCC_SV_Integer, "sv_pda_hunt", ( int* )&g_sv_dm_bPDAHunt, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_warm_up", ( int* )&g_sv_dm_dwWarmUp_MaxTime, 0,
          3600 ); // sec

    CMD4( CCC_Integer, "sv_max_ping_limit", ( int* )&g_sv_dwMaxClientPing, 1,
          2000 );

    CMD4( CCC_SV_Integer, "sv_auto_team_balance",
          ( int* )&g_sv_tdm_bAutoTeamBalance, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_auto_team_swap", ( int* )&g_sv_tdm_bAutoTeamSwap,
          0, 1 );
    CMD4( CCC_SV_Integer, "sv_friendly_indicators",
          ( int* )&g_sv_tdm_bFriendlyIndicators, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_friendly_names", ( int* )&g_sv_tdm_bFriendlyNames,
          0, 1 );
    CMD4( CCC_SV_Float, "sv_friendlyfire", &g_sv_tdm_fFriendlyFireModifier,
          0.0f, 2.0f );
    CMD4( CCC_SV_Integer, "sv_teamkill_limit", &g_sv_tdm_iTeamKillLimit, 0,
          100 );
    CMD4( CCC_SV_Integer, "sv_teamkill_punish",
          ( int* )&g_sv_tdm_bTeamKillPunishment, 0, 1 );

    CMD4( CCC_SV_Integer, "sv_artefact_respawn_delta",
          ( int* )&g_sv_ah_dwArtefactRespawnDelta, 0, 600 ); // sec
    CMD4( CCC_SV_Integer, "sv_artefacts_count", ( int* )&g_sv_ah_dwArtefactsNum,
          1, 100 );
    CMD4( CCC_SV_Integer, "sv_artefact_stay_time",
          ( int* )&g_sv_ah_dwArtefactStayTime, 0, 180 ); // min
    CMD4( CCC_SV_Integer, "sv_reinforcement_time",
          ( int* )&g_sv_ah_iReinforcementTime, -1, 3600 ); // sec
    CMD4( CCC_SV_Integer, "sv_bearercantsprint",
          ( int* )&g_sv_ah_bBearerCantSprint, 0, 1 );
    CMD4( CCC_SV_Integer, "sv_shieldedbases", ( int* )&g_sv_ah_bShildedBases, 0,
          1 );
    CMD4( CCC_SV_Integer, "sv_returnplayers",
          ( int* )&g_sv_ah_bAfReturnPlayersToBases, 0, 1 );
    CMD1( CCC_SwapTeams, "g_swapteams" );
#ifdef DEBUG
    CMD4( CCC_SV_Integer, "sv_ignore_money_on_buy",
          ( int* )&g_sv_dm_bDMIgnore_Money_OnBuy, 0, 1 );
#endif
    ...
    //-----------------
    CMD3( CCC_Token, "sv_adm_menu_ban_time", &g_sv_adm_menu_ban_time,
          g_ban_times ); // min
    //	CMD4(CCC_Integer,		"sv_adm_menu_ban_time",
    //(int*)&g_sv_adm_menu_ban_time, 1, 60); //min
    CMD4( CCC_Integer, "sv_adm_menu_ping_limit",
          ( int* )&g_sv_adm_menu_ping_limit, 1, 200 ); // min

    CMD4( CCC_Integer, "sv_invincible_time", ( int* )&g_sv_cta_dwInvincibleTime,
          0, 60 ); // sec
    CMD4( CCC_Integer, "sv_artefact_returning_time",
          ( int* )&g_sv_cta_artefactReturningTime, 0, 5 * 60 ); // sec
    CMD4( CCC_Integer, "sv_activated_return",
          ( int* )&g_sv_cta_activatedArtefactRet, 0, 1 )
    CMD4( CCC_Integer, "sv_show_player_scores_time",
          ( int* )&g_sv_cta_PlayerScoresDelayTime, 1, 20 ); // sec
    CMD4( CCC_Integer, "sv_cta_runkup_to_arts_div",
          ( int* )&g_sv_cta_rankUpToArtsCountDiv, 0, 10 );
    CMD1( CCC_CompressorStatus, "net_compressor_status" );
    CMD4( CCC_SV_Integer, "net_compressor_enabled",
          ( int* )&g_net_compressor_enabled, 0, 1 );
    CMD4( CCC_SV_Integer, "net_compressor_gather_stats",
          ( int* )&g_net_compressor_gather_stats, 0, 1 );
    CMD1( CCC_MpStatistics, "sv_dump_online_statistics" );
    CMD4( CCC_SV_Integer, "sv_dump_online_statistics_period",
          ( int* )&g_sv_mp_iDumpStatsPeriod, 0, 60 ); // min
#ifdef DEBUG
    CMD4( CCC_SV_Integer, "cl_dbg_min_ping", ( int* )&lag_simmulator_min_ping,
          0, 1000 );
    CMD4( CCC_SV_Integer, "cl_dbg_max_ping", ( int* )&lag_simmulator_max_ping,
          0, 1000 );
#endif
    ...
    CMD4( CCC_Integer, "sv_traffic_optimization_level",
          ( int* )&g_sv_traffic_optimization_level, 0, 7 );
}
