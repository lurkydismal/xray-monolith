...
u16 last_updates_cache::add_update( u16 const entity_id,
                                    NET_Packet const& update ) {
    ...
    if ( ( tmp_entity->second.B.count == update.B.count ) &&
         ( !memcmp( tmp_entity->second.B.data, update.B.data,
                    update.B.count ) ) ) {
...
u16 last_updates_cache::get_last_equpdates( u16 const entity_id,
                                            NET_Packet const& update ) {
    last_update_t* tmp_entity = search_entity( entity_id );
    if ( !tmp_entity )
        return 0;
...
last_updates_cache::last_update_t* last_updates_cache::search_entity(
    u16 const entity_id ) {
    for ( u32 i = 0; i < cache_entities_size; ++i ) {
...
last_updates_cache::last_update_t* last_updates_cache::search_most_expired(
    u32 const current_time,
    u32 const update_size ) {
    STATIC_CHECK( cache_entities_size > 1,
                  cache_entities_size__must_be_greater_than_one );
...
server_updates_compressor::server_updates_compressor() {
    u32 const need_to_reserve =
        ( start_compress_buffer_size / sizeof( m_acc_buff.B.data ) ) + 1;
...
void server_updates_compressor::init_compression() {
    ...
    compression::init_lzo( m_lzo_working_memory, m_lzo_working_buffer,
                           m_lzo_dictionary );
}
...
void server_updates_compressor::begin_updates() {
    ...
    if ( ( g_sv_traffic_optimization_level & eto_ppmd_compression ) ||
        ...
        m_ready_for_send.front()->w_u8(
            static_cast< u8 >( g_sv_traffic_optimization_level ) );
    ...
    if ( ( g_sv_traffic_optimization_level & eto_ppmd_compression ) ||
        ...
        if ( g_sv_traffic_optimization_level & eto_ppmd_compression ) {
            m_compress_buf.B.count = ppmd_trained_compress(
                m_compress_buf.B.data, sizeof( m_compress_buf.B.data ),
                m_acc_buff.B.data, m_acc_buff.B.count, m_trained_stream );
        } else {
            m_compress_buf.B.count = sizeof( m_compress_buf.B.data );
            lzo_compress_dict(
                m_acc_buff.B.data, m_acc_buff.B.count, m_compress_buf.B.data,
                ( lzo_uint* )&m_compress_buf.B.count, m_lzo_working_memory,
        ...
        if ( dst_packet->w_tell() + m_compress_buf.B.count +
                 ( sizeof( u16 ) * 2 + 1 ) <
             sizeof( dst_packet->B.data ) ) {
...
void server_updates_compressor::write_update_for( u16 const enity,
                                                  NET_Packet& update ) {
    if ( g_sv_traffic_optimization_level & eto_last_change ) {
        // if (m_updates_cache.get_last_equpdates(enity, update) >=
        // max_eq_packets)
    ...
    //(sizeof(u16)*2 + 1) ::= w_begin(2) + compress_type(1) + zero_end(2)
    if ( m_acc_buff.w_tell() + update.w_tell() + ( sizeof( u16 ) * 2 + 1 ) >=
         sizeof( m_acc_buff.B.data ) ) {
...
void server_updates_compressor::end_updates(
    send_ready_updates_t::const_iterator& b,
    ...
    if ( g_sv_write_updates_bin ) {
        ...
        for ( send_ready_updates_t::const_iterator i = b; i != e; ++i ) {
            dbg_update_bins_writer->w_u16(
                static_cast< u16 >( ( *i )->B.count ) );
