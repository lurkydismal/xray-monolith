...
void CStringTable::Init() {
    ...
    pData->m_sLanguage = READ_IF_EXISTS( pSettings, r_string, "string_table",
                                         "language", "eng" );

    // demonized: parse english files first, then they will be replaced by
    // current locale
    if ( use_english_text_for_missing_translations &&
         xr_strcmp( pData->m_sLanguage, "eng" ) != 0 ) {
        FS_FileSet fset;
    ...
    // Discord
    snprintf( discord_strings.mainmenu, 128,
              xr_ToUTF8( *CStringTable().translate( "st_main_menu" ) ) );
    snprintf( discord_strings.paused, 128,
              xr_ToUTF8( *CStringTable().translate( "st_pause_menu" ) ) );
    snprintf( discord_strings.loading, 128,
              xr_ToUTF8( *CStringTable().translate( "st_loading" ) ) );
    snprintf( discord_strings.health, 128,
              xr_ToUTF8( *CStringTable().translate( "st_ui_health_sensor" ) ) );
    snprintf( discord_strings.dead, 128,
              xr_ToUTF8( *CStringTable().translate( "st_player_dead" ) ) );
    snprintf(
        discord_strings.livesleft, 128,
        xr_ToUTF8( *CStringTable().translate( "st_hardcore_lives_left" ) ) );
    snprintf( discord_strings.livesleftsingle, 128,
              xr_ToUTF8( *CStringTable().translate(
                  "st_hardcore_lives_left_single" ) ) );
    snprintf(
        discord_strings.livespossessed, 128,
        xr_ToUTF8( *CStringTable().translate( "st_azazel_lives_possessed" ) ) );
    snprintf( discord_strings.livespossessedsingle, 128,
              xr_ToUTF8( *CStringTable().translate(
                  "st_azazel_lives_possessed_single" ) ) );
    snprintf( discord_strings.godmode, 128,
              xr_ToUTF8( *CStringTable().translate( "st_godmode" ) ) );
...
void CStringTable::Load( LPCSTR xml_file_full, LPCSTR lang_in ) {
    ...
    for ( int i = 0; i < string_num; ++i ) {
        LPCSTR string_name =
            uiXml.ReadAttrib( uiXml.GetRoot(), "string", i, "id", NULL );

        VERIFY3( pData->m_StringTable.find( string_name ) ==
                     pData->m_StringTable.end(),
                 "duplicate string table id", string_name );

        LPCSTR string_text =
            uiXml.Read( uiXml.GetRoot(), "string:text", i, NULL );

        if ( m_bWriteErrorsToLog && string_text )
            Msg( "[string table] '%s' no translation in '%s'", string_name,
                 lang );

        R_ASSERT3( string_text, "string table entry does not have a text",
                   string_name );
...
void CStringTable::ReparseKeyBindings() {
    ...
    for ( ; it != it_e; ++it ) {
        pData->m_StringTable[ it->first ] =
            ParseLine( *it->second, *it->first, false );
    }
}

void CStringTable::ReloadLanguage() {
    if ( 0 == xr_strcmp( READ_IF_EXISTS( pSettings, r_string, "string_table",
                                         "language", "eng" ),
                         *( pData->m_sLanguage ) ) )
        return;
