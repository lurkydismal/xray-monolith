...
void CSightAction::remove_links( CObject* object ) {
    ...
    m_vector3d.setHP( -this->object().movement().m_head.target.yaw,
                      this->object().movement().m_head.target.pitch );
}

bool CSightAction::target_reached() {
    return ( !!fsimilar(
        angle_normalize_signed( object().movement().m_head.target.yaw ),
...
void CSightAction::execute_direction() {
    m_vector3d.getHP( object().movement().m_head.target.yaw,
...
void CSightAction::execute_position( Fvector const& look_position ) {
    if ( m_torso_look )
        object().sight().SetFirePointLookAngles(
            m_vector3d, object().movement().m_head.target.yaw,
...
#ifdef SIGHT_TEST
    Msg( "%6d %s", Device.dwTimeGlobal,
         m_torso_look ? "eSightTypeFirePosition" : "eSightTypePosition" );
#endif
}
...
void CSightAction::execute_object() {
    ...
    if ( cpActor ) {
        ...
        if ( pActor->HUDview() ) {
            ...
            Fvector pos;
            kinematics->LL_GetBoneWorldPosition( bone_id, pActor->XFORM(),
                                                 pos );
            look_pos = pos;
            look_pos.y -= 0.9f;
        }
    ...
    const CEntityAlive* entity_alive =
        smart_cast< const CEntityAlive* >( m_object_to_look );
    ...
    if ( m_torso_look )
        object().sight().SetFirePointLookAngles(
            look_pos, object().movement().m_head.target.yaw,
...
#ifdef SIGHT_TEST
    Msg( "%6d %s", Device.dwTimeGlobal,
         m_torso_look ? "eSightTypeFireObject" : "eSightTypeObject" );
#endif
}

void CSightAction::execute_cover() {
    if ( m_torso_look )
        object().sight().SetLessCoverLook(
            m_object->ai_location().level_vertex(), PI, m_path );
    else
        object().sight().SetLessCoverLook(
            m_object->ai_location().level_vertex(), m_path );
#ifdef SIGHT_TEST
    Msg( "%6d %s [%f] -> [%f]", Device.dwTimeGlobal,
         m_torso_look ? "eSightTypeFireCover" : "eSightTypeCover",
...
#ifdef SIGHT_TEST
    Msg( "%6d %s", Device.dwTimeGlobal,
         m_torso_look ? "eSightTypeFireSearch" : "eSightTypeSearch" );
#endif
}
...
void CSightAction::execute_cover_look_over() {
    switch ( m_internal_state ) {
        ...
        fall_back:
#endif // #ifndef DEBUG
            if ( ( m_start_state_time + m_stop_state_time <
                   Device.dwTimeGlobal ) &&
                 target_reached() ) {
                m_start_state_time = Device.dwTimeGlobal;
                m_stop_state_time = 3500;
                ...
                object().movement().m_head.target.yaw =
                    m_cover_yaw + ::Random.randF( -PI_DIV_8, PI_DIV_8 );
            }
            break;
        }
        case 1: {
        ...
        default: {
#ifdef DEBUG
            FATAL( make_string( "m_internal_state = %d, object[0x%08x]",
                                m_internal_state, this )
                       .c_str() );
#else  // #ifdef DEBUG
            m_internal_state = 0;
            goto fall_back;
...
bool CSightAction::change_head_speed() const {
    return ( ( SightManager::eSightTypeCoverLookOver == m_sight_type ) &&
             ( m_internal_state != 2 ) );
}
...
void CSightAction::predict_object_position( bool use_exact_position ) {
    ...
    if ( std::find( objects.begin(), objects.end(), m_object_to_look ) ==
         objects.end() ) {
    ...
    if ( use_exact_position )
        m_vector3d = m_object->feel_vision_get_vispoint(
            const_cast< CGameObject* >( m_object_to_look ) );
    else
        m_vector3d = m_object->sight().object_position();

    u32 const count = m_object_to_look->ps_Size();
    if ( count > 1 ) {
        CObject::SavedPosition const current_position =
            m_object_to_look->ps_Element( count - 1 );
        VERIFY( Device.dwTimeGlobal >= current_position.dwTime );

        CObject::SavedPosition previous_position =
            m_object_to_look->ps_Element( count - 2 );
        for ( int i = 3;
              ( current_position.dwTime == previous_position.dwTime ) &&
              ( i <= ( int )count );
              ++i )
        ...
        if ( Device.dwTimeGlobal - previous_position.dwTime < 300 ) {
            if ( current_position.dwTime > previous_position.dwTime ) {
                Fvector offset = Fvector().sub( current_position.vPosition,
                                                previous_position.vPosition );
                offset.y = 0.f;
                Fvector const velocity =
                    Fvector( offset ).div( float( current_position.dwTime -
                                                  previous_position.dwTime ) /
                                           1000.f );
                float const predict_time =
                    m_object->aim_predict_time(); //*Device.fTimeDelta;
...
void CSightAction::execute_fire_object() {
    switch ( m_state_fire_object ) {
        case 0: {
            ...
            if ( m_object_to_look->Position().distance_to_sqr(
                     m_object->Position() ) < _sqr( 5.f ) )
                break;

            //			Msg
            //("%6d switch to mode 1", Device.dwTimeGlobal);
        ...
        case 1: {
            if ( Device.dwTimeGlobal >= m_state_fire_switch_time + 1500 ) {
                if ( m_object_to_look->Position().distance_to_sqr(
                         m_object->Position() ) > _sqr( 5.f ) ) {
                    if ( !m_holder_start_position.similar( m_object->Position(),
                                                           .05f ) ) {
                        m_vector3d = m_object->sight().object_position();
                        m_already_switched = false;
                        //						Msg
                    ...
                    if ( !m_object_start_position.similar(
                             m_object_to_look->Position(), .05f ) ) {
                        m_vector3d = m_object->sight().object_position();
                        //						Msg
