...
public:
    ...
    Fvector calculate_predicted_enemy_pos( float const prediction_factor,
        ...
        if ( predictiton_delta_sec > 0.4f ) {
            if ( m_last_prediction_time != 0 ) {
                if ( predictiton_delta_sec < 2.f ) {
                    Fvector const move_delta =
                        enemy_pos - m_last_update_enemy_pos;
                    m_predicted_enemy_velocity =
                        move_delta / predictiton_delta_sec;
                } else {
                    m_predicted_enemy_velocity = cr_fvector3( 0.f );
                }
            }
        ...
        return enemy_pos + ( m_predicted_enemy_velocity * self2enemy_time *
                             prediction_factor );
    }
