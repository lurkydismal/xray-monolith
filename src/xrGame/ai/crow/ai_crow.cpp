...
void CAI_Crow::SAnim::Load( IKinematicsAnimated* visual, LPCSTR prefix ) {
    ...
    for ( int i = 0;
          ( i < MAX_ANIM_COUNT ) && ( m_Animations.size() < MAX_ANIM_COUNT );
          ++i ) {
        string128 sh_anim;
...
void CAI_Crow::SSound::Load( LPCSTR prefix ) {
    ...
    for ( int i = 0;
          ( i < MAX_SND_COUNT ) && ( m_Sounds.size() < MAX_SND_COUNT ); ++i ) {
        string64 name;
...
void CAI_Crow::init() {
    st_current = eUndef;
    st_target = eFlyIdle;
    vGoalDir.set( 10.0f * ( Random.randF() - Random.randF() ),
...
void CAI_Crow::Load( LPCSTR section ) {
    ...
    fIdleSoundTime =
        fIdleSoundDelta + fIdleSoundDelta * Random.randF( -.5f, .5f );
#ifdef DEBUG
    VERIFY2( valid_pos( Position() ),
             dbg_valide_pos_string( Position(), this,
                                    "CAI_Crow::Load( LPCSTR section )" ) );
#endif
}
...
#ifdef DEBUG
    VERIFY2( valid_pos( Position() ),
             dbg_valide_pos_string( Position(), this, "CAI_Crow::net_Spawn" ) );
#endif
    return R;
}
...
void CAI_Crow::switch2_FlyUp() {
    smart_cast< IKinematicsAnimated* >( Visual() )
        ->PlayCycle( m_Anims.m_fly.GetRandom() );
}

void CAI_Crow::switch2_FlyIdle() {
    smart_cast< IKinematicsAnimated* >( Visual() )
        ->PlayCycle( m_Anims.m_idle.GetRandom() );
}
...
void CAI_Crow::switch2_DeathDead() {
    ...
    //
    smart_cast< IKinematicsAnimated* >( Visual() )
        ->PlayCycle( m_Anims.m_death_dead.GetRandom() );
}
...
void CAI_Crow::switch2_DeathFall() {
    ...
    smart_cast< IKinematicsAnimated* >( Visual() )
        ->PlayCycle( m_Anims.m_death.GetRandom(), TRUE, cb_OnHitEndPlaying,
                     this );
}
...
void CAI_Crow::state_DeathFall() {
    ...
    if ( bPlayDeathIdle ) {
        smart_cast< IKinematicsAnimated* >( Visual() )
            ->PlayCycle( m_Anims.m_death_idle.GetRandom() );
        bPlayDeathIdle = false;
    }
#ifdef DEBUG
    VERIFY2( valid_pos( Position() ),
             dbg_valide_pos_string( Position(), this,
                                    "CAI_Crow::state_DeathFall()" ) );
#endif
}

void CAI_Crow::Die( CObject* who ) {
    inherited::Die( who );
    processing_activate(); // enable UpdateCL for dead crows - especially for
                           // physics support
    // and do it especially before Creating physics shell or it definitely
    // throws processing enable/disable calls: underflow
    CreateSkeleton();
...
collide::rq_result GetPickResult( Fvector pos,
                                  Fvector dir,
                                  float range,
                                  CObject* ignore );
void CAI_Crow::shedule_Update( u32 DT ) {
    ...
    if ( ( eDeathFall != st_current ) && ( eDeathDead != st_current ) ) {
        ...
        if ( fGoalChangeTime <= 0 ) {
            fGoalChangeTime += fGoalChangeDelta +
                               fGoalChangeDelta * Random.randF( -0.5f, 0.5f );

            Level().ObjectSpace.GetNearest( nearbyObjects, Position(), 300.0f,
                                            NULL );
            for ( CObject* obj : nearbyObjects ) {
                if ( CEntityAlive* entity = smart_cast< CEntityAlive* >( obj );
                     entity && !entity->g_Alive() ) {
            ...
            Fvector vP;
            if ( !deadNPCs.empty() ) {
                CEntityAlive* targetNPC =
                    deadNPCs[ Random.randI( 0, deadNPCs.size() ) ];
            ...
            collide::rq_result RQ =
                GetPickResult( Position(), dest_dir, range, this );
            if ( RQ.element >= 0 )
                vP = Fvector( Position() ).mad( dest_dir, RQ.range );

            collide::rq_result RQ2 =
                GetPickResult( Position(), Direction(), range, this );
            if ( RQ2.element >= 0 )
        ...
        if ( fIdleSoundTime <= 0 ) {
            fIdleSoundTime =
                fIdleSoundDelta + fIdleSoundDelta * Random.randF( -0.5f, 0.5f );
            // if (st_current==eFlyIdle)
            ::Sound->play_at_pos( m_Sounds.m_idle.GetRandom(), H_Root(),
                                  Position() );
        }
        fIdleSoundTime -= fDT;
    }
    ...
    VERIFY2( valid_pos( Position() ),
             dbg_valide_pos_string( Position(), this,
                                    " CAI_Crow::shedule_Update\t\t(u32 DT)" ) );
#endif
}
...
void CAI_Crow::net_Import( NET_Packet& P ) {
    ...
    VERIFY2(
        valid_pos( Position() ),
        dbg_valide_pos_string( Position(), this,
                               " CAI_Crow::net_Import	(NET_Packet& P)" ) );
#endif
}

//---------------------------------------------------------------------
void CAI_Crow::HitSignal( float /**HitAmount/**/,
                          Fvector& /**local_dir/**/,
                          CObject* who,
                          s16 /**element/**/ ) {
...
//---------------------------------------------------------------------
void CAI_Crow::HitImpulse( float /**amount/**/,
                           Fvector& /**vWorldDir/**/,
                           Fvector& /**vLocalDir/**/ ) {}
...
void CAI_Crow::CreateSkeleton() {
    m_pPhysicsShell = P_build_Shell(
        this, false,
        ( BONE_P_MAP* )0 ); // P_build_SimpleShell(this,0.3f,false);
...
// void CAI_Crow::Hit	(float P, Fvector &dir, CObject* who, s16
...
void CAI_Crow::Hit( SHit* pHDS ) {
    ...
    const CGameObject* who_object =
        smart_cast< const CGameObject* >( pHDS->who );
