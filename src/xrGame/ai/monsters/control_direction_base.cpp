...
void CControlDirectionBase::face_target( const Fvector& position,
                                         u32 delay,
                                         float add_yaw ) {
    ...
    yaw +=
        ( m_man->direction().is_from_right( position ) ) ? add_yaw : -add_yaw;
...
void CControlDirectionBase::face_target( const CObject* obj,
                                         u32 delay,
                                         float add_yaw ) {
...
void CControlDirectionBase::use_path_direction( bool reversed ) {
    ...
    m_heading.target =
        angle_normalize( ( reversed ) ? ( -yaw + PI ) : ( -yaw ) );
}
...
void CControlDirectionBase::update_frame() {
    SControlDirectionData* ctrl_data =
