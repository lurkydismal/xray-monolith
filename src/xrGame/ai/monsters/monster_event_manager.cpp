...
void CMonsterEventManager::add_delegate( EEventType event,
                                         typeEvent delegate ) {
...
void CMonsterEventManager::remove_delegate( EEventType event,
                                            typeEvent delegate ) {
    EVENT_MAP_IT it = m_event_storage.find( event );
    if ( it == m_event_storage.end() )
        return;

    for ( EVENT_VECTOR_IT it_del = it->second.begin();
          it_del != it->second.end(); ++it_del ) {
...
void CMonsterEventManager::raise( EEventType event, IEventData* data ) {
    ...
    EVENT_VECTOR_IT it_del =
        std::remove_if( it->second.begin(), it->second.end(), pred_remove() );
    it->second.erase( it_del, it->second.end() );
}

void CMonsterEventManager::clear() {
    for ( EVENT_MAP_IT I_map = m_event_storage.begin();
          I_map != m_event_storage.end(); I_map++ ) {
        I_map->second.clear();
    }
