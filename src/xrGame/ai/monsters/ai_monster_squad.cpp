...
CMonsterSquad::CMonsterSquad()
    : leader( 0 ),
      m_home_danger_end_tick( 0 ),
      m_home_danger_mode_time( 8000 ) {
...
void CMonsterSquad::UpdateCommand( const CEntity* pE,
                                   const SSquadCommand& com ) {
...
void CMonsterSquad::UpdateSquadCommands() {
    ...
    for ( MEMBER_COMMAND_MAP_IT it = m_commands.begin(); it != m_commands.end();
          it++ ) {
...
bool CMonsterSquad::is_locked_cover( u32 node ) {
    return ( std::find( m_locked_covers.begin(), m_locked_covers.end(),
                        node ) != m_locked_covers.end() );
}
...
void CMonsterSquad::unlock_cover( u32 node ) {
    NODES_VECTOR_IT it =
        std::find( m_locked_covers.begin(), m_locked_covers.end(), node );
...
u8 CMonsterSquad::get_count( const CEntity* object, float radius ) {
    ...
    for ( MEMBER_GOAL_MAP_IT it_goal = m_goals.begin();
          it_goal != m_goals.end(); ++it_goal ) {
        SMemberGoal goal = it_goal->second;
        if ( ( goal.entity != 0 ) && ( goal.entity != object ) &&
             ( goal.entity->g_Alive() ) ) {
            if ( goal.entity->Position().distance_to( object->Position() ) <
                 radius )
                count++;
        }
    }
...
bool CMonsterSquad::is_locked_corpse( const CEntityAlive* corpse ) {
    return ( std::find( m_locked_corpses.begin(), m_locked_corpses.end(),
                        corpse ) != m_locked_corpses.end() );
}
...
void CMonsterSquad::unlock_corpse( const CEntityAlive* corpse ) {
    CORPSES_VECTOR_IT it =
        std::find( m_locked_corpses.begin(), m_locked_corpses.end(), corpse );
...
squad_grouping_behaviour::squad_grouping_behaviour( CEntity* self,
    ...
    : self( self ),
      squad( NULL ),
      steering_behaviour::grouping::params( cohesion_factor,
                                            separate_factor,
                                            max_separate_range ) {
    VERIFY( self );
}
