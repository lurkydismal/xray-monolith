...
CMonsterSquadManager::~CMonsterSquadManager() {
    for ( u32 team_id = 0; team_id < team.size(); team_id++ ) {
        for ( u32 squad_id = 0; squad_id < team[ team_id ].size();
              squad_id++ ) {
            for ( u32 group_id = 0;
                  group_id < team[ team_id ][ squad_id ].size(); group_id++ ) {
...
void CMonsterSquadManager::register_member( u8 team_id,
                                            u8 squad_id,
                                            u8 group_id,
                                            CEntity* e ) {
...
void CMonsterSquadManager::remove_member( u8 team_id,
                                          u8 squad_id,
                                          u8 group_id,
                                          CEntity* e ) {
...
CMonsterSquad* CMonsterSquadManager::get_squad( u8 team_id,
                                                u8 squad_id,
                                                u8 group_id ) {
    VERIFY( ( team_id < team.size() ) &&
            ( squad_id < team[ team_id ].size() ) &&
            ( group_id < team[ team_id ][ squad_id ].size() ) );
