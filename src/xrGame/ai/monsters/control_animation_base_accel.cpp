...
void CControlAnimationBase::accel_chain_add( EMotionAnim anim1,
                                             EMotionAnim anim2 ) {
    SEQ_VECTOR v_temp;
...
bool CControlAnimationBase::accel_chain_get( float cur_speed,
                                             EMotionAnim target_anim,
                                             EMotionAnim& new_anim,
                                             float& a_speed ) {
    VERIFY2( _abs( cur_speed ) < 1000,
             "CControlAnimationBase cur_speed too big" );
    ...
    for ( I = B; I != E; I++ ) {
        ...
        VERIFY2( _abs( tmp ) < 1000,
                 "CControlAnimationBase GetAnimSpeed returns too big speed" );
        a_speed = tmp * cur_speed / best_param->velocity.linear;
        VERIFY2( _abs( a_speed ) < 1000,
                 "CControlAnimationBase a_speed too big" );
        return true;
    }
    return false;
}
...
bool CControlAnimationBase::accel_chain_test() {
    ...
    // пройти по всем Chain-векторам
    for ( VELOCITY_CHAIN_VEC_IT I = m_accel.chain.begin();
          I != m_accel.chain.end(); I++ ) {
        ...
        for ( SEQ_VECTOR_IT IT = I->begin() + 1; IT != I->end(); IT++ ) {
            ...
            float from = anim_from->velocity.velocity.linear *
                         anim_from->velocity.max_factor;
            float to = anim_to->velocity.velocity.linear *
                       anim_to->velocity.min_factor;

            xr_sprintf( error_msg,
                        "Incompatible speed ranges. Monster[%s] From animation "
                        " [%s] To animation [%s]",
                        *m_object->cName(), *anim_from->target_name,
                        *anim_to->target_name );
...
bool CControlAnimationBase::accel_check_braking( float before_interval,
    ...
    if ( m_man->path_builder()
             ...
             .velocity == MonsterMovement::eVelocityParameterStand )
        return ( braking_mode = false );

    float acceleration = accel_get( eAV_Braking );
    float braking_dist =
        ( nominal_speed * ( ( braking_mode )
                                ? nominal_speed
                                : m_man->movement().velocity_current() ) ) /
        ( 2 * acceleration );
    ...
    for ( u32 i = m_man->path_builder().detail().curr_travel_point_index() + 1;
        ...
        if ( m_man->path_builder().detail().path()[ i ].velocity ==
