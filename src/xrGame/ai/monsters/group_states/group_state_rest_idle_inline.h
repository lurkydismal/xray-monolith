...
CStateGroupRestIdleAbstract::CStateGroupRestIdle( _Object* obj )
    : inherited( obj ) {
    add_state( eStateRest_WalkToCover,
               xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    add_state( eStateRest_LookOpenPlace,
    ...
    if ( object->b_end_state_eat ) {
        ...
        if ( m_target_node == u32( -1 ) ) {
            const CCoverPoint* point =
                object->CoverMan->find_cover( object->Home->get_home_point(), 1,
...
void CStateGroupRestIdleAbstract::reselect_state() {
    ...
    if ( ( prev_substate == u32( -1 ) && m_target_node != u32( -1 ) ) ||
         ( prev_substate == eStateRest_WalkGraphPoint ) ) {
        select_state( eStateRest_WalkToCover );
        return;
    }

    if ( ( prev_substate == eStateRest_WalkToCover ) ||
         ( prev_substate == u32( -1 ) ) ) {
...
void CStateGroupRestIdleAbstract::setup_substates() {
    ...
    if ( current_substate == eStateRest_WalkGraphPoint ) {
        ...
        if ( object->Position().distance_to( data.point ) > 8.f ) {
            ...
            if ( object->m_start_smelling == u32( -1 ) ||
                 object->m_start_smelling >
                     u32( 4 ) + object->m_smelling_count ) {
