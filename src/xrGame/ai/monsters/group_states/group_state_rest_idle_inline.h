...
CStateGroupRestIdleAbstract::CStateGroupRestIdle( _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateRest_WalkToCover,
                     xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    this->add_state( eStateRest_LookOpenPlace,
                     xr_new< CStateMonsterLookToPoint< _Object > >( obj ) );
...
void CStateGroupRestIdleAbstract::initialize() {
    ...
    if ( this->object->b_end_state_eat ) {
        ...
        if ( m_target_node == u32( -1 ) ) {
            const CCoverPoint* point = this->object->CoverMan->find_cover(
                this->object->Home->get_home_point(), 1,
                this->object->Home->get_min_radius() );
            if ( !point )
                return;
        ...
        if ( m_target_node == u32( -1 ) ) {
            const CCoverPoint* point = this->object->CoverMan->find_cover(
                this->object->Home->get_home_point(), 1,
                this->object->Home->get_mid_radius() );
            if ( !point )
                return;
            m_target_node = point->level_vertex_id();
        }
...
void CStateGroupRestIdleAbstract::reselect_state() {
    ...
    if ( ( this->prev_substate == u32( -1 ) && m_target_node != u32( -1 ) ) ||
         ( this->prev_substate == eStateRest_WalkGraphPoint ) ) {
        this->select_state( eStateRest_WalkToCover );
        return;
    }

    if ( ( this->prev_substate == eStateRest_WalkToCover ) ||
...
void CStateGroupRestIdleAbstract::setup_substates() {
    ...
    if ( this->current_substate == eStateRest_WalkGraphPoint ) {
        ...
        if ( this->object->Position().distance_to( data.point ) > 8.f ) {
            ...
            if ( this->object->m_start_smelling == u32( -1 ) ||
                 this->object->m_start_smelling >
                     u32( 4 ) + this->object->m_smelling_count ) {
                m_move_type = ( Random.randI( 2 ) );
                this->object->m_start_smelling =
                    m_move_type ? u32( 1 ) : u32( -1 );
        ...
        data.action.action =
            m_move_type ? ACT_WALK_FWD : ACT_HOME_WALK_SMELLING;
