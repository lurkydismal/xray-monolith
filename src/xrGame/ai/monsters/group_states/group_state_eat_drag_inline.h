...
void CStateGroupDragAbstract::initialize() {
    ...
    if ( capture && !capture->Failed() ) {
        ...
        if ( m_cover_vertex_id == u32( -1 ) ||
             object->Position().distance_to( m_cover_position ) < 2.f ||
...
void CStateGroupDragAbstract::execute() {
    ...
    if ( m_cover_vertex_id != u32( -1 ) ) {
        ...
        object->path().set_retreat_from_point(
            object->EatedCorpse->Position() );
    }
...
bool CStateGroupDragAbstract::check_completion() {
    ...
    if ( m_cover_vertex_id != u32( -1 ) ) {
        ...
        if ( m_corpse_start_position.distance_to( object->Position() ) >
             object->Home->get_min_radius() )
            return true;
    }
