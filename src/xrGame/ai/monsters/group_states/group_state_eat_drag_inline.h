...
void CStateGroupDragAbstract::initialize() {
    ...
    if ( !ini->section_exist( "capture_used_bones" ) ||
    ...
    struct callback : public CPHCaptureBoneCallback {
        ...
        bool operator()( u16 bone ) {
            ...
            for ( ; m_K->LL_GetBoneRoot() != bi; ) {
                ...
                if ( std::find_if( use_bones, use_bones_end, cmp ) !=
                     use_bones_end )
                    return ( true );
    ...
    IPHCapture* capture =
        this->object->character_physics_support()->movement()->PHCapture();
    if ( capture && !capture->Failed() ) {
        m_cover_vertex_id = this->object->Home->get_place_in_min_home();
        if ( m_cover_vertex_id != u32( -1 ) ) {
            m_cover_position =
                ai().level_graph().vertex_position( m_cover_vertex_id );
        } else
            m_cover_position = this->object->Position();
        if ( m_cover_vertex_id == u32( -1 ) ||
             this->object->Position().distance_to( m_cover_position ) < 2.f ||
...
void CStateGroupDragAbstract::execute() {
    ...
    if ( m_cover_vertex_id != u32( -1 ) ) {
        this->object->path().set_target_point( m_cover_position,
                                               m_cover_vertex_id );
    } else {
        this->object->path().set_retreat_from_point(
            this->object->EatedCorpse->Position() );
    }
...
bool CStateGroupDragAbstract::check_completion() {
    ...
    if ( m_cover_vertex_id != u32( -1 ) ) {
        ...
        if ( m_corpse_start_position.distance_to( this->object->Position() ) >
             this->object->Home->get_min_radius() )
            return true;
    }
