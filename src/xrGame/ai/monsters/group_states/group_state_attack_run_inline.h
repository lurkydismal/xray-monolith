...
CStateGroupAttackRunAbstract::CStateGroupAttackRun( _Object* obj )
    : inherited( obj ) {
...
void CStateGroupAttackRunAbstract::execute() {
    ...
    if ( Device.dwTimeGlobal > m_memorized_tick + memory_update_ms ) {
        m_predicted_vel =
            ( enemy_pos - m_memorized_pos ) *
            ( 1000.f / ( Device.dwTimeGlobal - m_memorized_tick ) );
    ...
    // classify if predicted enemy going to the left or right (viewed from our
    // eyes)
    const Fvector linear_prediction =
        enemy_pos + m_predicted_vel * prediction_time;
    ...
    const float angle = self2enemy_dist > epsilon
                            ? 0.5f * prediction_time *
                                  magnitude( m_predicted_vel ) / self2enemy_dist
                            : 0;

    h_angle = angle_normalize( h_angle + ( predicted_left ? +1 : -1 ) * angle );

    const Fvector radial_prediction =
        self_pos +
    ...
    u32 vertex = ai().level_graph().check_position_in_direction(
        old_vertex, self_pos, target );
    ...
    this->object->path().set_rebuild_time(
        this->object->get_attack_rebuild_time() );
    this->object->path().set_use_covers( false );
    // this->object->path().set_cover_params
    // (0.1f, 30.f, 1.f, 30.f);
    ...
    if ( !enemy_at_home ||
         Device.dwTimeGlobal < this->time_state_started + m_encircle_time ) {
...
// {
// 	// установка параметров функциональных блоков
// 	this->object->set_action
// (ACT_RUN); 	this->object->anim().accel_activate
// (eAT_Aggressive); 	this->object->anim().accel_set_braking		(false);
// 	this->object->path().set_target_point
// (this->object->EnemyMan.get_enemy_position(),
// this->object->EnemyMan.get_enemy_vertex());
// 	this->object->path().set_rebuild_time
// (this->object->get_attack_rebuild_time());
// 	this->object->path().set_use_covers			();
// 	this->object->path().set_cover_params
// (0.1f, 30.f, 1.f, 30.f); 	this->object->path().set_try_min_time
// (false); 	this->object->set_state_sound
...
//
// 	//	if ( g_bDogsDirMode )
// 	if ( Device.dwTimeGlobal-this->time_state_started < m_max_encircle_time
// )
// 	{
//  		CMonsterSquad *squad	=
//  monster_squad().get_squad(this->object); 		if (squad && squad->SquadActive())
//  {
//  			// Получить команду
//  			SSquadCommand command;
//  			squad->GetCommand(this->object, command);
//
//  			if (command.type == SC_ATTACK) {
//   				this->object->path().set_use_dest_orient
//   (true);
...
bool CStateGroupAttackRunAbstract::check_completion() {
    ...
    // 	if (
    // !ai().level_graph().valid_vertex_id(this->object->EnemyMan.get_enemy_vertex())
    // )
    // 	{
    // 		return true;
    // 	}
