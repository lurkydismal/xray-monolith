...
void CStateGroupAttackRunAbstract::execute() {
    ...
    if ( !enemy_at_home ||
         Device.dwTimeGlobal < time_state_started + m_encircle_time ) {
...
// {
// 	// установка параметров функциональных блоков
// 	object->set_action (ACT_RUN); 	object->anim().accel_activate
// (eAT_Aggressive); 	object->anim().accel_set_braking		(false);
// 	object->path().set_target_point
// (object->EnemyMan.get_enemy_position(), object->EnemyMan.get_enemy_vertex());
// 	object->path().set_rebuild_time
// (object->get_attack_rebuild_time()); 	object->path().set_use_covers
// (); 	object->path().set_cover_params			(0.1f, 30.f, 1.f, 30.f);
// 	object->path().set_try_min_time			(false);
...
bool CStateGroupAttackRunAbstract::check_completion() {
    ...
    // 	if (
    // !ai().level_graph().valid_vertex_id(object->EnemyMan.get_enemy_vertex())
    // )
    // 	{
    // 		return true;
    // 	}
