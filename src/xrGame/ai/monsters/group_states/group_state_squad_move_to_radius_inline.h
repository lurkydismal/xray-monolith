...
#define CStateGroupSquadMoveToRadiusExAbstract \
    CStateGroupSquadMoveToRadiusEx< _Object >
...
void CStateGroupSquadMoveToRadiusExAbstract::execute() {
    ...
    if ( squad && squad->SquadActive() ) {
        if ( squad->get_index( this->object ) != u8( -1 ) ) {
            float m_Angle = ( PI - PI_DIV_2 ) /
                            ( squad->squad_alife_count() - 1 ) *
                            ( squad->get_index( this->object ) - 1 );
            float m_Delta_Angle =
                Random.randF( PI_DIV_3 / ( squad->squad_alife_count() - 1 ) );
            ...
            m_heading = angle_normalize( m_heading - ( PI - PI_DIV_3 ) / 2 +
                                         m_Angle + m_Delta_Angle );
            to_direction.setHP( m_heading, m_pitch );
            data.point.x =
                m_enemy_position.x + data.completion_dist * to_direction.x;
            data.point.y =
                m_enemy_position.y + data.completion_dist * to_direction.y;
            data.point.z =
                m_enemy_position.z + data.completion_dist * to_direction.z;
...
bool CStateGroupSquadMoveToRadiusExAbstract::check_completion() {
    if ( data.action.time_out != 0 ) {
        if ( this->time_state_started + data.action.time_out <
             Device.dwTimeGlobal )
            return true;
    }
    if ( this->object->Position().distance_to_xz(
             this->object->EnemyMan.get_enemy()->Position() ) <
...
#define CStateGroupSquadMoveToRadiusAbstract \
    CStateGroupSquadMoveToRadius< _Object >
