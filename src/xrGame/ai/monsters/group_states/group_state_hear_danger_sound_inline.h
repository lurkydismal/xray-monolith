...
CStateGroupHearDangerousSoundAbstract::CStateGroupHearDangerousSound(
    _Object* obj )
    : inherited( obj ) {
    add_state( eStateHearDangerousSound_Hide,
               xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    add_state( eStateSquad,
               xr_new< CStateMonsterMoveToPoint< _Object > >( obj ) );
    add_state( eStateHearDangerousSound_Home,
...
void CStateGroupHearDangerousSoundAbstract::reselect_state() {
    ...
    if ( get_state( eStateHearDangerousSound_Home )
...
void CStateGroupHearDangerousSoundAbstract::setup_substates() {
    ...
    if ( current_substate == eStateSquad ) {
        ...
        if ( object->control().path_builder().get_node_in_radius(
            ...
            Fvector dest_pos = random_position( squad->GetLeader()->Position(),
                                                LEADER_RADIUS );
            if ( !object->control().path_builder().restrictions().accessible(
                     dest_pos ) ) {
                data.vertex = object->control()
                                  .path_builder()
                                  .restrictions()
    ...
    if ( current_substate == eStateHearDangerousSound_Hide ) {
        ...
        m_target_node =
            object->Home->get_place_in_max_home_to_direction( home2sound );
