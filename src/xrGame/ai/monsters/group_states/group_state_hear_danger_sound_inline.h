...
#define CStateGroupHearDangerousSoundAbstract \
    CStateGroupHearDangerousSound< _Object >

#define LEADER_RADIUS 20.f
#define FIND_POINT_ATTEMPTS 5

TEMPLATE_SPECIALIZATION
CStateGroupHearDangerousSoundAbstract::CStateGroupHearDangerousSound(
    _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateHearDangerousSound_Hide,
                     xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    this->add_state( eStateSquad,
                     xr_new< CStateMonsterMoveToPoint< _Object > >( obj ) );
    this->add_state(
        eStateHearDangerousSound_Home,
...
void CStateGroupHearDangerousSoundAbstract::reselect_state() {
    ...
    if ( this->get_state( eStateHearDangerousSound_Home )
    ...
    if ( squad->SquadActive() &&
         squad->GetCommand( this->object ).type == SC_REST ) {
...
void CStateGroupHearDangerousSoundAbstract::setup_substates() {
    ...
    if ( this->current_substate == eStateSquad ) {
        ...
        if ( this->object->control().path_builder().get_node_in_radius(
                 squad->GetLeader()->ai_location().level_vertex_id(), 8.f,
                 LEADER_RADIUS, FIND_POINT_ATTEMPTS, data.vertex ) ) {
            data.point = ai().level_graph().vertex_position( data.vertex );
        } else {
            Fvector dest_pos = random_position( squad->GetLeader()->Position(),
                                                LEADER_RADIUS );
            if ( !this->object->control()
                      .path_builder()
                      .restrictions()
                      .accessible( dest_pos ) ) {
                data.vertex = this->object->control()
                                  .path_builder()
                                  .restrictions()
                                  .accessible_nearest( dest_pos, data.point );
            } else {
                data.point = dest_pos;
                data.vertex = u32( -1 );
            }
    ...
    if ( this->current_substate == eStateHearDangerousSound_Hide ) {
        ...
        m_target_node = this->object->Home->get_place_in_max_home_to_direction(
            home2sound );
