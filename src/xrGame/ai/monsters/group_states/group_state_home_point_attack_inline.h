...
#define CStateGroupAttackMoveToHomePointAbstract \
    CStateGroupAttackMoveToHomePoint< _Object >

namespace detail {
namespace dog {
const float scare_distance2enemy =
    20.f; // distance on which dog can be scared of enemy
} // namespace dog
} // namespace detail
...
CStateGroupAttackMoveToHomePointAbstract::CStateGroupAttackMoveToHomePoint(
    _Object* obj )
    : inherited( obj ) {
    m_target_node = 0;
    m_skip_camp = false;
    m_state_started = 0;
    this->add_state( eStateAttack_HomePoint_Hide,
                     xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    this->add_state( eStateAttack_HomePoint_LookOpenPlace,
...
bool CStateGroupAttackMoveToHomePointAbstract::check_start_conditions() {
    ...
    if ( enemy_inaccessible() ) {
        ...
        if ( m_last_tick_enemy_inaccessible &&
             current_time() - m_last_tick_enemy_inaccessible > 3000 ) {
            m_first_tick_enemy_inaccessible = 0;
            m_last_tick_enemy_inaccessible = 0;
        }
    }
