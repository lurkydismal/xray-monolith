...
CStateGroupRestAbstract::CStateGroupRest( _Object* obj ) : inherited( obj ) {
    ...
    this->add_state( eStateRest_Idle,
                     xr_new< CStateGroupRestIdle< _Object > >( obj ) );
...
void CStateGroupRestAbstract::initialize() {
    ...
    time_for_sleep = 0;
    time_for_life = time() + this->object->m_min_life_time +
                    Random.randI( 10 ) * this->object->m_min_life_time;
            ...
            if ( move_to_home_point )
                ...
                if ( this->prev_substate == eStateRest_Sleep ) {
                    if ( time() <= time_for_sleep ) {
                        ...
                        time_for_life =
                            time() + this->object->m_min_life_time +
                            Random.randI( 10 ) * this->object->m_min_life_time;
                ...
                if ( !use_to_do ) {
                    if ( time() > time_for_life &&
                         this->object->Home->at_min_home(
                             this->object->Position() ) ) {
                        this->object->set_current_animation( 8 );
                        this->select_state( eStateCustom );
                        this->object->saved_state = eStateRest_Sleep;
                        time_for_sleep =
                            time() + this->object->m_min_sleep_time +
                            Random.randI( 5 ) * this->object->m_min_sleep_time;
                        use_to_do = true;
                        this->object->b_state_check = false;
                        this->get_state_current()->execute();
                        this->prev_substate = this->current_substate;
                        ...
                        if ( this->object->saved_state != eStateRest_Sleep &&
                             this->prev_substate == eStateCustom &&
                             this->object->get_number_animation() >= u32( 8 ) &&
                             this->object->get_number_animation() <
                                 u32( 12 ) ) {
                            this->object->set_current_animation(
                                this->object->get_number_animation() +
                                u32( 1 ) );
                            this->select_state( eStateCustom );
                            this->object->b_state_check = false;
