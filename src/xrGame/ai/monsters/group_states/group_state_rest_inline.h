...
void CStateGroupRestAbstract::initialize() {
    ...
    time_for_sleep = 0;
    time_for_life = time() + object->m_min_life_time +
                    Random.randI( 10 ) * object->m_min_life_time;
            ...
            if ( move_to_home_point )
                ...
                if ( !use_to_do ) {
                    if ( time() > time_for_life &&
                         object->Home->at_min_home( object->Position() ) ) {
                        object->set_current_animation( 8 );
                        select_state( eStateCustom );
                        object->saved_state = eStateRest_Sleep;
                        time_for_sleep =
                            time() + object->m_min_sleep_time +
                            Random.randI( 5 ) * object->m_min_sleep_time;
                        use_to_do = true;
                        object->b_state_check = false;
                        get_state_current()->execute();
                        prev_substate = current_substate;
                        ...
                        if ( object->saved_state != eStateRest_Sleep &&
                             prev_substate == eStateCustom &&
                             object->get_number_animation() >= u32( 8 ) &&
                             object->get_number_animation() < u32( 12 ) ) {
                            object->set_current_animation(
                                object->get_number_animation() + u32( 1 ) );
                            select_state( eStateCustom );
                            object->b_state_check = false;
                            get_state_current()->execute();
                            prev_substate = current_substate;
