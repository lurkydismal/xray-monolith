...
void CStateGroupPanicRunAbstract::execute() {
    ...
    this->object->path().set_target_point(
        this->object->Home->get_place_in_max_home_to_direction( enemy2home ) );
...
bool CStateGroupPanicRunAbstract::check_completion() {
    float dist_to_enemy = this->object->Position().distance_to(
        this->object->EnemyMan.get_enemy_position() );
    u32 time_delta =
        Device.dwTimeGlobal - this->object->EnemyMan.get_enemy_time_last_seen();
