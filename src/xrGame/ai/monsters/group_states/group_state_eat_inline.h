...
void CStateGroupEatAbstract::finalize() {
    ...
    if ( ( corpse == object->EatedCorpse ) && object->EatedCorpse ) {
        const_cast< CEntityAlive* >( object->EatedCorpse )->m_use_timeout =
            object->m_corpse_use_timeout;
        const_cast< CEntityAlive* >( object->EatedCorpse )
...
void CStateGroupEatAbstract::reselect_state() {
    ...
    /*
       ...
       select_state(eStateEat_CheckCorpse); return; }

            if (prev_substate == eStateEat_CheckCorpse) {
                    if (object->ability_can_drag())
       select_state(eStateEat_Drag); else { if
       (get_state(eStateEat_Eat)->check_start_conditions())
                                    select_state(eStateEat_Eat);
                            else
    ...
    if ( prev_substate == eStateEat_CorpseApproachWalk ) {
        ...
        // Lain: added
        if ( object->ability_can_drag() &&
             object->check_eated_corpse_draggable() ) {
...
void CStateGroupEatAbstract::setup_substates() {
    ...
    if ( current_substate == eStateEat_CorpseApproachRun ) {
        ...
        if ( ( corpse->m_pPhysicsShell == NULL ) ||
            ...
            nearest_bone_pos = object->character_physics_support()
                                   ->movement()
