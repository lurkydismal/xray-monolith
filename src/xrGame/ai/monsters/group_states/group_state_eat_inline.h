...
CStateGroupEatAbstract::CStateGroupEat( _Object* obj ) : inherited( obj ) {
    corpse = NULL;
    m_time_last_eat = 0;
    this->add_state( eStateEat_CorpseApproachRun,
                     xr_new< CStateMonsterMoveToPoint< _Object > >( obj ) );
    this->add_state( eStateEat_CorpseApproachWalk,
                     xr_new< CStateMonsterMoveToPoint< _Object > >( obj ) );
    this->add_state( eStateEat_CheckCorpse,
                     xr_new< CStateMonsterCustomAction< _Object > >( obj ) );
    this->add_state( eStateEat_Eat,
                     xr_new< CStateGroupEating< _Object > >( obj ) );
    this->add_state( eStateEat_WalkAway,
                     xr_new< CStateMonsterHideFromPoint< _Object > >( obj ) );
    this->add_state( eStateEat_Rest,
                     xr_new< CStateMonsterCustomAction< _Object > >( obj ) );
    this->add_state( eStateEat_Drag,
                     xr_new< CStateGroupDrag< _Object > >( obj ) );
...
void CStateGroupEatAbstract::finalize() {
    ...
    if ( ( corpse == this->object->EatedCorpse ) &&
         this->object->EatedCorpse ) {
        const_cast< CEntityAlive* >( this->object->EatedCorpse )
            ->m_use_timeout = this->object->m_corpse_use_timeout;
        const_cast< CEntityAlive* >( this->object->EatedCorpse )
            ->set_lock_corpse( false );
    }
    if ( this->object->character_physics_support()->movement()->PHCapture() )
        this->object->character_physics_support()
            ->movement()
            ->PHReleaseObject();
...
void CStateGroupEatAbstract::critical_finalize() {
    ...
    if ( ( corpse == this->object->EatedCorpse ) && this->object->EatedCorpse &&
         check_completion() ) {
        if ( this->object->character_physics_support()
                 ->movement()
                 ->PHCapture() )
            this->object->character_physics_support()
                ->movement()
...
void CStateGroupEatAbstract::reselect_state() {
    ...
    /*
       ...
       this->select_state(eStateEat_CheckCorpse); return; }

            if (this->prev_substate == eStateEat_CheckCorpse) {
                    if (this->object->ability_can_drag())
       this->select_state(eStateEat_Drag); else { if
       (this->get_state(eStateEat_Eat)->check_start_conditions())
                                    this->select_state(eStateEat_Eat);
                            else
    ...
    if ( this->prev_substate == eStateEat_CorpseApproachWalk ) {
        if ( !this->get_state( eStateEat_CorpseApproachWalk )
                  ->check_completion() ) {
            this->select_state( eStateEat_CorpseApproachWalk );
            return;
        }
        // Lain: added
        if ( this->object->ability_can_drag() &&
             this->object->check_eated_corpse_draggable() ) {
...
void CStateGroupEatAbstract::setup_substates() {
    ...
    if ( this->current_substate == eStateEat_CorpseApproachRun ) {
        ...
        if ( ( corpse->m_pPhysicsShell == NULL ) ||
             ( !corpse->m_pPhysicsShell->isActive() ) ) {
            nearest_bone_pos = corpse->Position();
        } else
            nearest_bone_pos = this->object->character_physics_support()
                                   ->movement()
                                   ->PHCaptureGetNearestElemPos( corpse );
...
bool CStateGroupEatAbstract::check_start_conditions() {
    ...
    return (
        ...
        this->object->Home->at_home(
            this->object->CorpseMan.get_corpse()->Position() ) &&
        hungry() &&
...
bool CStateGroupEatAbstract::hungry() {
    return ( ( m_time_last_eat == 0 ) ||
             ( m_time_last_eat + TIME_NOT_HUNGRY < time() ) );
}
