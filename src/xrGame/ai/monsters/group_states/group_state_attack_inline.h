...
CStateGroupAttackAbstract::CStateGroupAttack( _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateAttack_Run,
                     xr_new< CStateGroupAttackRun< _Object > >( obj ) );
    this->add_state( eStateAttack_Melee,
                     xr_new< CStateMonsterAttackMelee< _Object > >( obj ) );
    this->add_state( eStateAttack_RunAttack,
                     xr_new< CStateMonsterAttackRunAttack< _Object > >( obj ) );
    this->add_state( eStateAttack_Attack_On_Run,
                     xr_new< CStateMonsterAttackOnRun< _Object > >( obj ) );
    this->add_state( eStateAttack_RunAway,
                     xr_new< CStateMonsterHideFromPoint< _Object > >( obj ) );
    this->add_state( eStateAttack_FindEnemy,
                     xr_new< CStateMonsterFindEnemy< _Object > >( obj ) );
    this->add_state(
        eStateAttack_MoveToHomePoint,
        xr_new< CStateMonsterAttackMoveToHomePoint< _Object > >( obj ) );

    this->add_state( eStateCustom,
                     xr_new< CStateGroupSquadMoveToRadius< _Object > >( obj ) );
    this->add_state( eStateAttack_AttackHidden,
                     xr_new< CStateGroupSquadMoveToRadius< _Object > >( obj ) );
    this->add_state( eStateAttackCamp,
                     xr_new< CStateGroupSquadMoveToRadius< _Object > >( obj ) );
    this->add_state(
        eStateAttack_Steal,
        xr_new< CStateGroupSquadMoveToRadiusEx< _Object > >( obj ) );
    this->add_state( eStateAttack_ControlFire,
                     xr_new< CStateCustomGroup< _Object > >( obj ) );
}
...
void CStateGroupAttackAbstract::initialize() {
    ...
    if ( squad ) {
        ...
        if ( squad->get_index( this->object ) == u8( -1 ) ) {
            ...
            monster_squad()
                .get_squad( this->object )
                ->set_squad_index( this->object->EnemyMan.get_enemy() );
        }
    ...
    // this->object->EnemyMan.update(); - may cause enemy returned by EnemyMan
    // be invalid next execute
...
void CStateGroupAttackAbstract::execute() {
    ...
    if ( check_home_point() ) {
        ...
        } else if ( this->get_state( eStateAttack_Melee )
        ...
        if ( !can_attack_on_move && b_melee ) {
            ...
            // [TODO] make specific state and replace run_away state (to avoid
            // rotation jumps)
            if ( check_behinder() ) {
            ...
            if ( aggressive ) {
                ...
                this->select_state( can_attack_on_move
                                        ? eStateAttack_Attack_On_Run
                                        : eStateAttack_Run );
            } else {
                switch ( this->prev_substate ) {
                    ...
                    case eStateAttack_AttackHidden:
                        if ( this->get_state_current()->check_completion() ) {
                            ...
                            if ( this->object->Position().distance_to(
                                     enemy->Position() ) >
                                 17.f + m_delta_distance ) {
                    ...
                    case eStateCustom:
                        if ( this->get_state_current()->check_completion() ) {
                            ...
                            if ( this->object->Position().distance_to(
                                     enemy_pos ) > 11.f + m_delta_distance ) {
                                this->select_state( eStateAttack_AttackHidden );
                                m_drive_out = false;
                            }
                        }
                    ...
                    case eStateAttack_ControlFire:
                        if ( this->object->Position().distance_to( enemy_pos ) >
                                 7.f + m_delta_distance ||
                             Device.dwTimeGlobal - m_time_start_drive_out >
                                 this->object->m_drive_out_time ) {
                            if ( this->object->get_custom_anim_state() ) {
                                this->object->anim_end_reinit();
                            }

                            if ( Device.dwTimeGlobal - m_time_start_drive_out >
                                 this->object->m_drive_out_time ) {
                                m_drive_out = true;
                            }
...
bool CStateGroupAttackAbstract::check_home_point() {
    ...
    if ( this->prev_substate != eStateAttack_MoveToHomePoint ) {
        if ( this->get_state( eStateAttack_MoveToHomePoint )
                 ->check_start_conditions() ) {
            return true;
        }
    } else {
...
void CStateGroupAttackAbstract::setup_substates() {
    ...
    if ( this->current_substate == eStateAttack_Steal ) {
        ...
        data.completion_dist =
            15.f + m_delta_distance; // get exactly to the point
    ...
    if ( this->current_substate == eStateAttack_AttackHidden ) {
        ...
        data.action.time_out = 0; // do not use time out
        data.completion_dist =
            10.f + m_delta_distance; // get exactly to the point
        data.time_to_rebuild = this->object->get_attack_rebuild_time();
        data.accelerated = true;
        data.braking = false;
        data.accel_type = eAT_Aggressive;
    ...
    if ( this->current_substate == eStateCustom ) {
        ...
        if ( m_drive_out ) {
            ...
            data.completion_dist =
                6.f + m_delta_distance; // get exactly to the point
        }
        data.time_to_rebuild = this->object->get_attack_rebuild_time();
        data.accelerated = true;
        data.braking = false;
