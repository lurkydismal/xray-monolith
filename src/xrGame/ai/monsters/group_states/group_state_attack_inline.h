...
void CStateGroupAttackAbstract::initialize() {
    ...
    if ( squad ) {
        ...
        if ( squad->get_index( object ) == u8( -1 ) ) {
            ...
            monster_squad().get_squad( object )->set_squad_index(
    ...
    // object->EnemyMan.update(); - may cause enemy returned by EnemyMan be
    // invalid next execute
...
void CStateGroupAttackAbstract::execute() {
    ...
    if ( check_home_point() ) {
        ...
        } else if ( get_state( eStateAttack_Melee )
        ...
        if ( !can_attack_on_move && b_melee ) {
            ...
            if ( aggressive ) {
                ...
                select_state( can_attack_on_move ? eStateAttack_Attack_On_Run
                                                 : eStateAttack_Run );
            } else {
                switch ( prev_substate ) {
                    ...
                    case eStateAttack_AttackHidden:
                        if ( get_state_current()->check_completion() ) {
                            ...
                            if ( object->Position().distance_to(
                                     enemy->Position() ) >
                    ...
                    case eStateCustom:
                        if ( get_state_current()->check_completion() ) {
                            ...
                            if ( object->Position().distance_to( enemy_pos ) >
                                 11.f + m_delta_distance ) {
                                select_state( eStateAttack_AttackHidden );
                                m_drive_out = false;
                            }
                        }
...
bool CStateGroupAttackAbstract::check_home_point() {
    ...
    if ( prev_substate != eStateAttack_MoveToHomePoint ) {
        if ( get_state( eStateAttack_MoveToHomePoint )
                 ->check_start_conditions() ) {
            return true;
        }
    } else {
