...
#define CStatePoltergeistAttackHiddenAbstract \
    CStatePoltergeistAttackHidden< _Object >

#define DIST_TO_PATH_END 1.5f

TEMPLATE_SPECIALIZATION
CStatePoltergeistAttackHiddenAbstract::CStatePoltergeistAttackHidden(
    _Object* obj )
    : inherited( obj ) {
    this->add_state(
...
void CStatePoltergeistAttackHiddenAbstract::select_target_for_move() {
    ...
    float const fly_radius =
        this->object->get_fly_around_distance() * m_fly_radius_factor;
    ...
    if ( current_time() > m_fly_side_select_tick ) {
        bool left_side =
            self2enemy.x * self2front.z - self2enemy.z * self2front.x > 0.f;
        if ( !( rand() % 2 ) ) {
            left_side ^= true;
        }

        m_fly_left = left_side;
        m_fly_side_select_tick =
            current_time() +
            ( u32 )( 1000 *
                     this->object->get_fly_around_change_direction_time() );
    }
    ...
    for ( u32 index = 0; index < move_scan_points; ++index ) {
        float const angle =
            move_scan_angle * ( index + 1 ) * ( m_fly_left ? -1.f : +1.f );
...
void CStatePoltergeistAttackHiddenAbstract::execute() {
    ...
    this->object->sound().play( MonsterSound::eMonsterSoundAggressive, 0, 0,
                                this->object->db().m_dwAttackSndDelay );
}
