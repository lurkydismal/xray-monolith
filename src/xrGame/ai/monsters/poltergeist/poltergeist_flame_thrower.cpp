...
void CPolterFlame::load( LPCSTR section ) {
    ...
    m_sound.create( pSettings->r_string( section, "flame_sound" ), st_Effect,
                    SOUND_TYPE_WORLD );

    m_particles_prepare =
        pSettings->r_string( section, "flame_particles_prepare" );
    ...
    read_delay( section, "flame_scan_delay_min_max", m_scan_delay_min,
                m_scan_delay_max );

    // load scan effector
    LPCSTR ppi_section =
        pSettings->r_string( section, "flame_scan_effector_section" );
    m_scan_effector_info.duality.h =
        pSettings->r_float( ppi_section, "duality_h" );
    m_scan_effector_info.duality.v =
    ...
    m_scan_effector_info.noise.intensity =
        pSettings->r_float( ppi_section, "noise_intensity" );
    m_scan_effector_info.noise.grain =
        pSettings->r_float( ppi_section, "noise_grain" );
    m_scan_effector_info.noise.fps =
    ...
    m_scan_sound.create( pSettings->r_string( section, "flame_scan_sound" ),
...
void CPolterFlame::create_flame( const CObject* target_object ) {
    ...
    Fvector target_point =
        get_head_position( const_cast< CObject* >( target_object ) );
...
void CPolterFlame::select_state( SFlameElement* elem, EFlameState state ) {
    ...
    switch ( elem->state ) {
        case ePrepare:
            ...
            m_object->PlayParticles( m_particles_prepare, elem->position,
                                     elem->target_dir, TRUE );
            break;
        case eFire:
            // start fire particles
            elem->particles_object = m_object->PlayParticles(
                m_particles_fire, elem->position, elem->target_dir, FALSE );
            break;
        case eStop:
            // stop fire particles
            ...
            m_object->PlayParticles( m_particles_stop, elem->position,
                                     elem->target_dir, TRUE );
...
void CPolterFlame::update_schedule() {
                ...
                if ( elem->time_started + m_time_fire_play < time() )
                    ...
                    if ( elem->time_last_hit + m_hit_delay < time() ) {
                        ...
                        if ( Level().ObjectSpace.RayPick(
                                 elem->position, elem->target_dir, m_length,
                                 collide::rqtBoth, rq, NULL ) ) {
                            if ( ( rq.O == elem->target_object ) &&
                                 ( rq.range < m_length ) ) {
                                float hit_value;
                                hit_value = m_hit_value -
                                            m_hit_value * rq.range / m_length;

                                NET_Packet P;
                                SHit HS;
                                HS.GenHeader( GE_HIT,
                                              elem->target_object->ID() );
                                //					u_EventGen
                                //(P,GE_HIT, element->target_object->ID());
                                HS.whoID = ( m_object->ID() );    //					P.w_u16
                                                                  //(ID());
                                HS.weaponID = ( m_object->ID() ); //					P.w_u16
                                                                  //(ID());
                                HS.dir =
                                    ( elem->target_dir ); //					P.w_dir
                                                          //(element->target_dir);
                                HS.power = ( hit_value ); //					P.w_float
                                                          //(m_flame_hit_value);
                                HS.boneID = ( BI_NONE );  //					P.w_s16
                                                          //(BI_NONE);
                                HS.p_in_bone_space =
                                    ( Fvector().set( 0.f, 0.f, 0.f ) );
                                //					P.w_vec3
                                //(Fvector().set(0.f,0.f,0.f));
                                HS.impulse = ( 0.f ); //					P.w_float
                                                      //(0.f);
                                HS.hit_type =
                                    ( ALife::
                                          eHitTypeBurn ); //					P.w_u16
    ...
    m_flames.erase(
        std::remove_if( m_flames.begin(), m_flames.end(), remove_predicate() ),
        m_flames.end() );

    bool const detected = m_object->get_current_detection_level() >=
                          m_object->get_detection_success_level();

    CEntityAlive const* enemy = Actor();
    // check if we can create another flame
    if ( m_object->g_Alive() && enemy && m_flames.size() < m_count &&
         !m_object->get_actor_ignore() && detected ) {
        // check aura radius and accessibility
        float dist = enemy->Position().distance_to( m_object->Position() );
        if ( ( dist < m_pmt_aura_radius ) &&
             m_object->control().path_builder().accessible(
                 enemy->Position() ) ) {
            // check timing
...
void CPolterFlame::on_destroy() {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( ( *I )->particles_object )
            Particles::Details::Destroy( ( *I )->particles_object );
...
bool CPolterFlame::get_valid_flame_position( const CObject* target_object,
                                             Fvector& res_pos ) {
    ...
    for ( u32 i = 0; i < FIND_POINT_ATTEMPT_COUNT; i++ ) {
        ...
        vertex_position = ai().level_graph().vertex_position(
            Obj->ai_location().level_vertex_id() );
        new_pos.mad( vertex_position, dir,
                     Random.randF( m_min_flame_dist, m_max_flame_dist ) );
    ...
    float angle = ai().level_graph().vertex_high_cover_angle(
        Obj->ai_location().level_vertex_id(), PI_DIV_6, std::less< float >() );
