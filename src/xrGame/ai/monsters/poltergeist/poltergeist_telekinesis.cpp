...
void CPolterTele::load( LPCSTR section ) {
    ...
    m_pmt_radius =
        READ_IF_EXISTS( pSettings, r_float, section, "Tele_Find_Radius", 10.f );
    m_pmt_object_min_mass = READ_IF_EXISTS( pSettings, r_float, section,
                                            "Tele_Object_Min_Mass", 40.f );
    m_pmt_object_max_mass = READ_IF_EXISTS( pSettings, r_float, section,
                                            "Tele_Object_Max_Mass", 500.f );
    m_pmt_object_count =
        READ_IF_EXISTS( pSettings, r_u32, section, "Tele_Object_Count", 10 );
    m_pmt_time_to_hold =
        READ_IF_EXISTS( pSettings, r_u32, section, "Tele_Hold_Time", 3000 );
    m_pmt_time_to_wait =
        READ_IF_EXISTS( pSettings, r_u32, section, "Tele_Wait_Time", 3000 );
    m_pmt_time_to_wait_in_objects = READ_IF_EXISTS(
        pSettings, r_u32, section, "Tele_Delay_Between_Objects_Time", 500 );
    m_pmt_distance =
        READ_IF_EXISTS( pSettings, r_float, section, "Tele_Distance", 50.f );
    m_pmt_object_height = READ_IF_EXISTS( pSettings, r_float, section,
                                          "Tele_Object_Height", 10.f );
    m_pmt_time_object_keep = READ_IF_EXISTS( pSettings, r_u32, section,
                                             "Tele_Time_Object_Keep", 10000 );
    m_pmt_raise_speed =
        READ_IF_EXISTS( pSettings, r_float, section, "Tele_Raise_Speed", 3.f );
    m_pmt_raise_time_to_wait_in_objects =
        READ_IF_EXISTS( pSettings, r_u32, section,
                        "Tele_Delay_Between_Objects_Raise_Time", 500 );
    m_pmt_fly_velocity = READ_IF_EXISTS( pSettings, r_float, section,
                                         "Tele_Fly_Velocity", 30.f );
    m_pmt_object_collision_damage = READ_IF_EXISTS(
        pSettings, r_float, section, "Tele_Collision_Damage", 0.5f );
    ::Sound->create( m_sound_tele_hold,
                     pSettings->r_string( section, "sound_tele_hold" ),
                     st_Effect, SOUND_TYPE_WORLD );
    ::Sound->create( m_sound_tele_throw,
...
void CPolterTele::update_schedule() {
    ...
    if ( m_object->get_current_detection_level() <
         m_object->get_detection_success_level() )
        return;
    ...
    switch ( m_state ) {
        case eStartRaiseObjects:
            ...
            if ( m_state == eStartRaiseObjects ) {
                if ( m_object->CTelekinesis::get_objects_count() >=
                     m_pmt_object_count ) {
                    m_state = eRaisingObjects;
                    m_time = time();
                }
        ...
        case eFireObjects:
            if ( m_time + m_time_next < time() ) {
                ...
                m_time = time();
                m_time_next = m_pmt_time_to_wait_in_objects / 2 +
                              Random.randI( m_pmt_time_to_wait_in_objects / 2 );
            }
...
public:
    ...
    bool operator()( const CObject_ptr& tpObject1,
                     const CObject_ptr& tpObject2 ) const {
...
bool CPolterTele::trace_object( CObject* obj, const Fvector& target ) {
    ...
    if ( Level().ObjectSpace.RayPick( trace_from, dir, range, collide::rqtBoth,
                                      l_rq, obj ) ) {
        if ( l_rq.O == Actor() )
            return true;
    }
...
void CPolterTele::tele_find_objects( xr_vector< CObject* >& objects,
    ...
    for ( u32 i = 0; i < m_nearest.size(); i++ ) {
        ...
        CCustomMonster* custom_monster =
            smart_cast< CCustomMonster* >( m_nearest[ i ] );
        ...
        if ( trace_object( obj, center ) ||
             trace_object( obj, get_head_position( Actor() ) ) )
...
bool CPolterTele::tele_raise_objects() {
    ...
    std::sort(
        tele_objects.begin(), tele_objects.end(),
    ...
    // if (tele_objects.size() > m_pmt_tele_object_count) tele_objects.resize
    // (m_pmt_tele_object_count);

    //// активировать
    // for (u32 i=0; i<tele_objects.size(); i++) {
    //	CPhysicsShellHolder *obj = smart_cast<CPhysicsShellHolder
    //*>(tele_objects[i]);
    ...
    if ( !tele_objects.empty() ) {
        CPhysicsShellHolder* obj =
            smart_cast< CPhysicsShellHolder* >( tele_objects[ 0 ] );

        // применить телекинез на объект
        bool rotate = false;

        CTelekineticObject* tele_obj = m_object->CTelekinesis::activate(
            obj, m_pmt_raise_speed, m_pmt_object_height, m_pmt_time_object_keep,
            rotate );
...

{
    ...
    SCollisionHitCallback( CPoltergeist* obj,
                           float pmt_object_collision_damage )
        : m_object( obj ),
...
void CPolterTele::tele_fire_objects() {
    for ( u32 i = 0; i < m_object->CTelekinesis::get_objects_total_count();
          i++ ) {
        CTelekineticObject tele_object =
            m_object->CTelekinesis::get_object_by_index( i );
        ...
        if ( ( tele_object.get_state() == TS_Raise ) ||
            ...
            const float fire_time =
                hobj->Position().distance_to( enemy_pos ) / m_pmt_fly_velocity;
            hobj->set_collision_hit_callback( xr_new< SCollisionHitCallback >(
                m_object, m_pmt_object_collision_damage ) );
