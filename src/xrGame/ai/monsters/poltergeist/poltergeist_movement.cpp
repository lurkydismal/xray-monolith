...
void CPoltergeisMovementManager::move_along_path(
    ...
    float dist =
        desirable_speed * time_delta; // пройденное расстояние в соостветствие с
                                      // желаемой скоростью
    ...
    // обновить detail().curr_travel_point_index() в соответствие с текущей
    // позицией
    while ( detail().curr_travel_point_index() < detail().path().size() - 2 ) {
        ...
        float pos_dist_to_next_point = dest_position.distance_to(
            detail()
                .path()[ detail().curr_travel_point_index() + 1 ]
                .position );
    ...
    while ( dist > dist_to_target ) {
        ...
        if ( detail().curr_travel_point_index() + 1 >= detail().path().size() )
            ...
            dist -= dist_to_target;
            ++detail().m_current_travel_point;
            if ( ( detail().curr_travel_point_index() + 1 ) >=
                 detail().path().size() )
                break;
            target.set( detail()
                            .path()[ detail().curr_travel_point_index() + 1 ]
