...
void CPolterSpecialAbility::load( LPCSTR section ) {
    ...
    m_sound_base.create( pSettings->r_string( section, "Sound_Idle" ),
                         st_Effect, SOUND_TYPE_MONSTER_TALKING );
...
void CPolterSpecialAbility::on_hide() {
    ...
    m_particles_object =
        m_object->PlayParticles( m_particles_hidden, m_object->Position(),
                                 Fvector().set( 0.0f, 0.1f, 0.0f ), false );
    m_particles_object_electro =
        m_object->PlayParticles( m_particles_idle, m_object->Position(),
                                 Fvector().set( 0.0f, 0.1f, 0.0f ), false );
}
...
void CPolterSpecialAbility::on_die() {
    ...
    m_object->PlayParticles( m_particles_death, particles_position,
                             Fvector().set( 0.0f, 1.0f, 0.0f ), TRUE, FALSE );
...
void CPolterSpecialAbility::on_hit( SHit* pHDS ) {
    if ( m_object->g_Alive() &&
         ...
         ( Device.dwFrame != m_last_hit_frame ) ) {
        if ( BI_NONE != pHDS->bone() ) {
            ...
            m_object->PlayParticles( m_particles_damage, start_pos,
...
void CPoltergeist::PhysicalImpulse( const Fvector& position ) {
    ...
    CPhysicsShellHolder* obj =
        smart_cast< CPhysicsShellHolder* >( m_nearest[ index ] );
...
void CPoltergeist::StrangeSounds( const Fvector& position ) {
    ...
    for ( u32 i = 0; i < TRACE_ATTEMPT_COUNT; i++ ) {
        ...
        if ( Level().ObjectSpace.RayPick( position, dir, TRACE_DISTANCE,
                                          collide::rqtStatic, l_rq, NULL ) ) {
            if ( l_rq.range < TRACE_DISTANCE ) {
                // Получить пару материалов
                CDB::TRI* pTri =
                    Level().ObjectSpace.GetStaticTris() + l_rq.element;
                SGameMtlPair* mtl_pair = GMLib.GetMaterialPair(
                    material().self_material_idx(), pTri->material );
                if ( !mtl_pair )
                    continue;
...
#ifdef DEBUG
                    CLONE_MTL_SOUND_CHECK( m_strange_sound, mtl_pair,
                                           CollideSounds );
#endif
                    CLONE_MTL_SOUND_DO( m_strange_sound, mtl_pair,
                                        CollideSounds );

                    Fvector pos;
                    pos.mad( position, dir,
                             ( ( l_rq.range - 0.1f > 0 ) ? l_rq.range - 0.1f
                                                         : l_rq.range ) );
