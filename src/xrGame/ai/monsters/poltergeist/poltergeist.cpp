...
void CPoltergeist::Load( LPCSTR section ) {
    ...
    anim().AddAnim( eAnimAttack, "stand_attack_", -1, &velocity_turn, PS_STAND,
                    "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r" );
    anim().AddAnim( eAnimDie, "stand_idle_", 0, &velocity_none, PS_STAND,
                    "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r" );
    anim().AddAnim( eAnimMiscAction_00, "fall_down_", -1, &velocity_none,
                    PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l",
                    "fx_stand_r" );
    anim().AddAnim( eAnimMiscAction_01, "fly_", -1, &velocity_none, PS_STAND,
                    "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r" );
    ...
    anim().AddAnim( eAnimEat, "stand_eat_", -1, &velocity_none, PS_STAND,
                    "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r" );
    anim().AddAnim( eAnimLookAround, "stand_look_around_", -1, &velocity_none,
                    PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l",
                    "fx_stand_r" );
    anim().AddAnim( eAnimWalkDamaged, "stand_walk_dmg_", -1, &velocity_walk_dmg,
                    PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l",
                    "fx_stand_r" );
    anim().AddAnim( eAnimRunDamaged, "stand_walk_dmg_", -1, &velocity_run_dmg,
                    PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l",
                    "fx_stand_r" );
    ...
    m_height_change_velocity = READ_IF_EXISTS( pSettings, r_float, section,
                                               "Height_Change_Velocity", 0.5f );
    m_height_change_min_time = READ_IF_EXISTS( pSettings, r_u32, section,
                                               "Height_Change_Min_Time", 3000 );
    m_height_change_max_time = READ_IF_EXISTS(
        pSettings, r_u32, section, "Height_Change_Max_Time", 10000 );
    m_height_min =
        READ_IF_EXISTS( pSettings, r_float, section, "Height_Min", 0.4f );
    m_height_max =
        READ_IF_EXISTS( pSettings, r_float, section, "Height_Max", 2.f );

    m_fly_around_level = READ_IF_EXISTS( pSettings, r_float, section,
                                         "detection_fly_around_level", 5.f );
    m_fly_around_distance = READ_IF_EXISTS(
        pSettings, r_float, section, "detection_fly_around_distance", 15.f );

    m_fly_around_change_direction_time =
        READ_IF_EXISTS( pSettings, r_float, section,
                        "detection_fly_around_change_direction_time", 7 );
    ...
    m_detection_pp_effector_name = READ_IF_EXISTS(
        pSettings, r_string, section, "detection_pp_effector_name", "" );
    m_detection_near_range_factor = READ_IF_EXISTS(
        pSettings, r_float, section, "detection_near_range_factor", 2.f );
    m_detection_far_range_factor = READ_IF_EXISTS(
        pSettings, r_float, section, "detection_far_range_factor", 1.f );
    m_detection_speed_factor = READ_IF_EXISTS( pSettings, r_float, section,
                                               "detection_speed_factor", 1.f );
    m_detection_loose_speed = READ_IF_EXISTS( pSettings, r_float, section,
                                              "detection_loose_speed", 5.f );
    m_detection_far_range = READ_IF_EXISTS( pSettings, r_float, section,
                                            "detection_far_range", 20.f );
    m_detection_success_level = READ_IF_EXISTS(
        pSettings, r_float, section, "detection_success_level", 4.f );
    m_detection_max_level = READ_IF_EXISTS( pSettings, r_float, section,
                                            "detection_max_level", 100.f );
...
void CPoltergeist::update_detection() {
    ...
    float const time_passed_sec =
        float( Device.dwTimeGlobal - m_last_detection_time ) / 1000.f;
    m_last_detection_time = Device.dwTimeGlobal;

    if ( !get_actor_ignore() && time_passed_sec > 0.f &&
         time_passed_sec < 2.f && dist2actor < get_detection_far_range() ) {
        float const relative_range = dist2actor / get_detection_far_range();
        float const range_factor =
            relative_range * get_detection_far_range_factor() +
            ( 1.f - relative_range ) * get_detection_near_range_factor();

        float const speed_factor = get_detection_speed_factor();
        float const raw_speed =
            m_last_actor_pos.distance_to( actor_pos ) / time_passed_sec;

        float const speed = powf( 1.f + raw_speed, speed_factor ) - 1.f;

        float const actor_psy_immunity =
            Actor()->conditions().GetHitImmunity( ALife::eHitTypeTelepatic );
        m_current_detection_level +=
            time_passed_sec * 0.03f * actor_psy_immunity * range_factor * speed;
    }

    m_current_detection_level -= time_passed_sec * get_detection_loose_speed();
    m_current_detection_level =
        ( m_current_detection_level < 0 ) ? 0.f : m_current_detection_level;
    ...
    if ( m_current_detection_level > 0.01f && m_detection_pp_effector_name &&
         m_detection_pp_effector_name[ 0 ] ) {
        if ( !m_detection_pp_type_index ) {
            for ( m_detection_pp_type_index =
                      ( u32 )effPoltergeistTeleDetectStartEffect;
                  Actor()->Cameras().GetPPEffector(
                      ( EEffectorPPType )m_detection_pp_type_index );
                  ++m_detection_pp_type_index ) {
                ;
            }

            AddEffector( Actor(), m_detection_pp_type_index,
                         m_detection_pp_effector_name,
                         GET_KOEFF_FUNC(
                             this, &CPoltergeist::get_post_process_factor ) );
        }
...
void CPoltergeist::UpdateCL() {
    ...
    def_lerp( m_height, target_height, m_height_change_velocity,
              client_update_fdelta() );
...
void CPoltergeist::UpdateHeight() {
    ...
    if ( time_height_updated < cur_time ) {
        time_height_updated =
            cur_time +
            Random.randI( m_height_change_min_time, m_height_change_max_time );
...
float CPoltergeist::get_detection_success_level() {
    return override_if_debug( "detection_success_level",
