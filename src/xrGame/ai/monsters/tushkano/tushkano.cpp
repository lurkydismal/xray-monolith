...
void CTushkano::Load( LPCSTR section ) {
    ...
    anim().AddAnim( eAnimStandTurnLeft, "stand_turn_left_", -1, &velocity_turn,
                    PS_STAND );
    anim().AddAnim( eAnimStandTurnRight, "stand_turn_right_", -1,
    ...
    // anim().AddAnim(eAnimSteal,			"stand_crawl_",
    // -1, &velocity_steal,			PS_STAND);
    // anim().AddAnim(eAnimSitIdle,			"sit_idle_",
    // -1, &velocity_none,				PS_SIT);
    ...
    // anim().AddTransition(PS_STAND,		PS_SIT,
    // eAnimStandSitDown,	false); anim().AddTransition(PS_SIT,
    // PS_STAND,	eAnimSitStandUp,	false);
