...
void CControlMovementBase::load( LPCSTR section ) {
    ...
    load_velocity( section, "Velocity_WalkFwdNormal",
                   eVelocityParameterWalkNormal );
    load_velocity( section, "Velocity_WalkSmelling",
                   eVelocityParameterWalkSmelling );
    load_velocity( section, "Velocity_WalkGrowl", eVelocityParameterWalkGrowl );
    load_velocity( section, "Velocity_RunFwdNormal",
                   eVelocityParameterRunNormal );
    load_velocity( section, "Velocity_WalkFwdDamaged",
                   eVelocityParameterWalkDamaged );
    load_velocity( section, "Velocity_RunFwdDamaged",
    ...
    m_man->path_builder().detail().add_velocity(
        eVelocityParameterIdle, CDetailPathManager::STravelParams(
                                    velocity_param.velocity.linear,
                                    velocity_param.velocity.angular_path,
                                    velocity_param.velocity.angular_real ) );
}

void CControlMovementBase::load_velocity( LPCSTR section,
                                          LPCSTR line,
                                          u32 velocity_id ) {
    SVelocityParam velocity_param;
    ...
    m_man->path_builder().detail().add_velocity(
        velocity_id, CDetailPathManager::STravelParams(
                         velocity_param.velocity.linear,
                         velocity_param.velocity.angular_path,
                         velocity_param.velocity.angular_real ) );
}
...
void CControlMovementBase::update_frame() {
    SControlMovementData* ctrl_data = ( SControlMovementData* )m_man->data(
...
float CControlMovementBase::get_velocity_from_path() {
    ...
    if ( fis_zero( current_velocity.linear_velocity ) &&
        ...
        m_object->dir().set_heading_speed(
            next_velocity.real_angular_velocity );
    } else {
