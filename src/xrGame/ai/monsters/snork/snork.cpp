...
#define STAND_FX_ALL \
    STAND_FX_FRONT, STAND_FX_BACK, STAND_FX_LEFT, STAND_FX_RIGHT
...
void CSnork::Load( LPCSTR section ) {
    ...
    anim().AddAnim( eAnimStandIdle, "stand_idle_", -1, &velocity_none, PS_STAND,
                    STAND_FX_ALL );
    anim().AddAnim( eAnimStandDamaged, "stand_idle_damaged_", -1,
                    &velocity_none, PS_STAND, STAND_FX_ALL );
    anim().AddAnim( eAnimWalkDamaged, "stand_walk_damaged_", -1,
                    &velocity_walk_dmg, PS_STAND, STAND_FX_ALL );
    anim().AddAnim( eAnimRunDamaged, "stand_run_damaged_", -1,
                    &velocity_run_dmg, PS_STAND, STAND_FX_ALL );
    anim().AddAnim( eAnimStandTurnLeft, "stand_turn_ls_", -1, &velocity_turn,
                    PS_STAND, STAND_FX_ALL );
    ...
    anim().AddAnim( eAnimRunTurnLeft, { "stand_run_look_left_", true }, -1,
                    &velocity_run, PS_STAND, STAND_FX_ALL );
    anim().AddAnim( eAnimRunTurnRight, { "stand_run_look_right_", true }, -1,
                    &velocity_run, PS_STAND, STAND_FX_ALL );
...
void CSnork::reinit() {
    ...
    move().load_velocity( *cNameSect(), "Velocity_JumpGround",
                          MonsterMovement::eSnorkVelocityParameterJumpGround );
    com_man().load_jump_data(
        "stand_attack_2_0", 0, "stand_attack_2_1", "stand_somersault_0",
        u32( -1 ), MonsterMovement::eSnorkVelocityParameterJumpGround, 0 );
...
void CSnork::UpdateCL() {
    ...
    const CCoverPoint* point =
        CoverMan->find_cover( obj->Position(), 10.f, 30.f );
...
float CSnork::trace( const Fvector& dir ) {
    ...
    if ( Level().ObjectSpace.RayPick( trace_from, dir, trace_dist,
                                      collide::rqtStatic, l_rq, this ) ) {
