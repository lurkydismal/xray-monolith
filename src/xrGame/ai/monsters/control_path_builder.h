...
class CControlPathBuilder : public CControl_ComPure< SControlPathBuilderData >,
...
public:
    ...
    bool valid_and_accessible( Fvector& pos,
                               u32 node ); // validate with a small correction
    bool is_moving_on_path();

    bool get_node_in_radius( u32 src_node,
                             float min_radius,
                             float max_radius,
                             u32 attempts,
                             u32& dest_node );
    ...
    static u32 find_nearest_vertex( const u32& level_vertex_id,
