...
IC bool CAI_Rat::bfCheckIfOutsideAIMap( Fvector& tTemp1 ) {
    ...
    if ( !ai().level_graph().valid_vertex_id( dwNewNode ) ||
         !ai().level_graph().inside( *ai_location().level_vertex(),
                                     QueryPos ) ) {
        dwNewNode = ai().level_graph().vertex( ai_location().level_vertex_id(),
                                               tTemp1 );
        tpNewNode = ai().level_graph().vertex( dwNewNode );
    }
    return ( !ai().level_graph().valid_vertex_id( dwNewNode ) ||
             !ai().level_graph().inside( *tpNewNode, QueryPos ) );
};
...
bool CAI_Rat::calc_node( Fvector const& next_position ) {
    ...
    const CLevelGraph::CVertex* tpNewNode = ai_location().level_vertex();
    CLevelGraph::CPosition QueryPos;
    bool a = !ai().level_graph().valid_vertex_id( dwNewNode ) ||
             !ai().level_graph().valid_vertex_position( next_position );
    if ( !a ) {
        ai().level_graph().vertex_position( QueryPos, next_position );
        a = !ai().level_graph().inside( *ai_location().level_vertex(),
                                        QueryPos );
    }
    if ( a ) {
        dwNewNode = ai().level_graph().vertex( ai_location().level_vertex_id(),
    ...
    if ( ai().level_graph().valid_vertex_id( dwNewNode ) &&
         ai().level_graph().inside( *tpNewNode, QueryPos ) ) {
        m_tNewPosition.y = ai().level_graph().vertex_plane_y(
            *tpNewNode, next_position.x, next_position.z );
        if ( movement().restrictions().accessible( m_tNewPosition ) ||
             !movement().restrictions().accessible( Position() ) )
            return true;
    }
    return false;
}
...
void CAI_Rat::move( bool bCanAdjustSpeed, bool bStraightForward ) {
    ...
    if ( ( angle_difference( movement().m_body.target.yaw,
                             movement().m_body.current.yaw ) > PI_DIV_6 ) ) {
        make_turn();
        return;
    }
    ...
    if ( m_bNoWay &&
         ( !m_turning ||
           ( angle_difference( movement().m_body.target.yaw,
                               movement().m_body.current.yaw ) < EPS_L ) ) ) {
        if ( ( Device.dwTimeGlobal - m_previous_query_time > TIME_TO_RETURN ) ||
             ( !m_previous_query_time ) ) {
            movement().m_body.target.yaw = movement().m_body.current.yaw + PI;
            movement().m_body.target.yaw =
                angle_normalize( movement().m_body.target.yaw );
            Fvector tTemp;
            tTemp.setHP( -movement().m_body.target.yaw,
        ...
        for ( ; i != e; ++i ) {
            for ( int j = 0; j < iPointCount; ++j )
                if ( ai().game_graph().mask(
                    ...
                    m_time_to_change_graph_point = Device.dwTimeGlobal +
                                                   ::Random32.random( 60000 ) +
                                                   60000;
                    return;
                }
        }
                ...
                if ( ai().game_graph().mask(
                    ...
                    if ( iBranches == iChosenBranch ) {
                        ...
                        m_time_to_change_graph_point =
                            Device.dwTimeGlobal + ::Random32.random( 60000 ) +
                            60000;
                        return;
                    }
                    ++iBranches;
...
Fvector CAI_Rat::get_next_target_point() {
    ...
    if ( Position().distance_to( ai().level_graph().vertex_position(
             vertex->data().level_vertex_id() ) ) < 1.5f ) {
...
void CAI_Rat::draw_way() {
    ...
    for ( u32 i = 1; i < m_path->vertex_count(); i++ ) {
        ...
        P1 = ai().level_graph().vertex_position(
            vertex->data().level_vertex_id() );
        vertex = m_path->vertex( i );
        P2 = ai().level_graph().vertex_position(
            vertex->data().level_vertex_id() );
        ...
        Level().debug_renderer().draw_ellipse( m_sphere,
                                               D3DCOLOR_XRGB( 0, 255, 255 ) );
        // Level().debug_renderer().draw_aabb
        // (P1,0.5f,0.5f,0.5f,D3DCOLOR_XRGB(0,255,255));
    }
