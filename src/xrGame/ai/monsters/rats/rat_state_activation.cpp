...
void CAI_Rat::activate_state_free_active() {
    ...
    if ( bfCheckIfGoalChanged() ) {
        if ( ( Position().distance_to( m_tSpawnPosition ) >
               m_fStableDistance ) ||
...
void CAI_Rat::activate_turn() {
    ...
    m_dest_direction.x = ( m_enemy_position.x - m_temp.x ) /
                         m_temp.distance_to( m_enemy_position );
    m_dest_direction.y = ( m_enemy_position.y - m_temp.y ) /
                         m_temp.distance_to( m_enemy_position );
    m_dest_direction.z = ( m_enemy_position.z - m_temp.z ) /
                         m_temp.distance_to( m_enemy_position );
...
void CAI_Rat::activate_state_attack_range() {
    ...
    if ( m_attack_rebuild &&
         Device.dwTimeGlobal - time_attack_rebuild > 5000 ) {
