...
IC void CAI_Rat::add_active_member( bool bForceActive ) {
    CGroupHierarchyHolder& Group = Level()
                                       ...
                                       .squad( g_Squad() )
                                       .group( g_Group() );
    if ( !m_bActive && ( bForceActive || ( Group.m_dwAliveCount *
                                               m_dwActiveCountPercent / 100 >=
    ...
    // Msg("* Group : alive[%2d],
    // active[%2d]",Group.m_dwAliveCount,Group.m_dwActiveCount);
};
...
IC void CAI_Rat::vfAddStandingMember() {
    CGroupHierarchyHolder& Group = Level()
                                       ...
                                       .squad( g_Squad() )
                                       .group( g_Group() );
    if ( ( Group.m_dwAliveCount * m_dwStandingCountPercent / 100 >=
...
IC bool CAI_Rat::bfCheckIfSoundFrightful() {
    return ( ( ( m_tLastSound.eSoundType & SOUND_TYPE_WEAPON_BULLET_HIT ) ==
               SOUND_TYPE_WEAPON_BULLET_HIT ) ||
             ( ( m_tLastSound.eSoundType & SOUND_TYPE_WEAPON_SHOOTING ) ==
               SOUND_TYPE_WEAPON_SHOOTING ) );
};

IC void CAI_Rat::update_morale_broadcast( float const& fValue,
                                          float const& /**fRadius/**/ ) {
    CGroupHierarchyHolder& Group = Level()
                                       .seniority_holder()
                                       .team( g_Team() )
