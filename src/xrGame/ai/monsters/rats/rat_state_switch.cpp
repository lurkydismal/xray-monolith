...
bool CAI_Rat::switch_if_position() {
    if ( memory().enemy().selected()->Position().distance_to( Position() ) >
         m_fAttackDistance ) {
        return true;
    }
    return false;
...
CAI_Rat::ERatStates CAI_Rat::get_state() {
    return ERatStates( dwfChooseAction(
        ...
        aiRatAttackMelee, aiRatAttackMelee, aiRatAttackMelee, aiRatRetreat,
        aiRatRetreat, this, 30.f ) );
}

bool CAI_Rat::switch_if_porsuit() {
    if ( m_home_position.distance_to(
             memory().enemy().selected()->Position() ) > m_fMaxPursuitRadius ) {
        return true;
    }
    return false;
}
...
void CAI_Rat::set_dir() {
    if ( ( Device.dwTimeGlobal - m_previous_query_time > TIME_TO_GO ) ||
        ...
        if ( squad && squad->SquadActive() ) {
            float m_delta_Angle =
                angle_normalize( ( PI * 2 ) / squad->squad_alife_count() );
            float m_heading, m_pitch;
            Fvector m_temp, m_dest_direction;
            m_temp = squad->GetLeader()->Position();
            m_dest_direction.x = ( m_temp.x - m_enemy_position.x ) /
                                 m_temp.distance_to( m_enemy_position );
            m_dest_direction.y = ( m_temp.y - m_enemy_position.y ) /
                                 m_temp.distance_to( m_enemy_position );
            m_dest_direction.z = ( m_temp.z - m_enemy_position.z ) /
                                 m_temp.distance_to( m_enemy_position );
            m_dest_direction.getHP( m_heading, m_pitch );
            m_heading = angle_normalize(
                m_heading + m_delta_Angle * squad->get_index( this ) );
...
bool CAI_Rat::switch_to_free_recoil() {
    if ( ( m_tLastSound.dwTime >= m_dwLastUpdateTime ) &&
         ( !m_tLastSound.tpEntity ||
           ( ( !switch_to_eat() || ( memory().item().selected()->ID() !=
...
bool CAI_Rat::switch_if_diff() {
    ...
    if ( angle_difference( movement().m_body.current.yaw, sTemp.yaw ) >
         m_fAttackAngle ) {
        return true;
    }
    return false;
