...
CAI_Rat::CAI_Rat() : m_behaviour_manager( 0 ) {
    ...
    //	m_behaviour_manager->add
    //(xr_new<steering_behaviour::alignment>(this),	.5f);
}
...
void CAI_Rat::init() {
    ...
    m_tGoalDir.set( 10.0f * ( Random.randF() - Random.randF() ),
...
void CAI_Rat::init_state_manager() {
    ...
    m_state_manager->add_state( aiRatFreeActive,
                                xr_new< rat_state_free_active >() );
    m_state_manager->add_state( aiRatFreePassive,
                                xr_new< rat_state_free_passive >() );
    m_state_manager->add_state( aiRatAttackRange,
                                xr_new< rat_state_attack_range >() );
    m_state_manager->add_state( aiRatAttackMelee,
                                xr_new< rat_state_attack_melee >() );
    m_state_manager->add_state( aiRatUnderFire,
                                xr_new< rat_state_under_fire >() );
    ...
    m_state_manager->add_state( aiRatFreeRecoil,
                                xr_new< rat_state_free_recoil >() );
    m_state_manager->add_state( aiRatReturnHome,
                                xr_new< rat_state_return_home >() );
    m_state_manager->add_state( aiRatEatCorpse,
                                xr_new< rat_state_eat_corpse >() );
...
void CAI_Rat::reload( LPCSTR section ) {
    ...
    sound().add( pSettings->r_string( section, "sound_death" ), 100,
                 SOUND_TYPE_MONSTER_DYING, 0, u32( eRatSoundMaskDie ),
                 eRatSoundDie, head_bone_name );
    sound().add( pSettings->r_string( section, "sound_hit" ), 100,
                 SOUND_TYPE_MONSTER_INJURING, 1, u32( eRatSoundMaskInjuring ),
                 eRatSoundInjuring, head_bone_name );
    sound().add( pSettings->r_string( section, "sound_attack" ), 100,
                 SOUND_TYPE_MONSTER_ATTACKING, 2, u32( eRatSoundMaskAttack ),
                 eRatSoundAttack, head_bone_name );
    sound().add( pSettings->r_string( section, "sound_voice" ), 100,
                 SOUND_TYPE_MONSTER_TALKING, 4, u32( eRatSoundMaskVoice ),
                 eRatSoundVoice, head_bone_name );
    sound().add( pSettings->r_string( section, "sound_eat" ), 100,
                 SOUND_TYPE_MONSTER_EATING, 3, u32( eRatSoundMaskEat ),
...
void CAI_Rat::Load( LPCSTR section ) {
    ...
    m_fChangeActiveStateProbability =
        pSettings->r_float( section, "ChangeActiveStateProbability" );
    ...
    m_dwStandingCountPercent =
        pSettings->r_s32( section, "StandingCountPercent" );
    ...
    m_fAttackStraightDistance =
        pSettings->r_float( section, "AttackStraightDistance" );
    m_fStableDistance = pSettings->r_float( section, "StableDistance" );
    m_fWallMinTurnValue =
        pSettings->r_float( section, "WallMinTurnValue" ) / 180.f * PI;
    m_fWallMaxTurnValue =
        pSettings->r_float( section, "WallMaxTurnValue" ) / 180.f * PI;
    ...
    m_fMoraleDeathDistance =
        pSettings->r_float( section, "MoraleDeathDistance" );
    ...
    m_fNullASpeed = pSettings->r_float( section, "AngularStandSpeed" ) / 180.f *
                    PI; // PI_MUL_2
    m_fMinASpeed = pSettings->r_float( section, "AngularMinSpeed" ) / 180.f *
                   PI; // PI_MUL_2
    m_fMaxASpeed =
        pSettings->r_float( section, "AngularMaxSpeed" ) / 180.f * PI; //.2f
    m_fAttackASpeed = pSettings->r_float( section, "AngularAttackSpeed" ) /
                      180.f * PI; //.15f;
...
BOOL CAI_Rat::net_Spawn( CSE_Abstract* DC ) {
    ...
    for ( int j = 0; j < iPointCount; ++j )
        if ( ai().game_graph().mask(
                 ...
                 ai().game_graph()
                     .vertex( ai_location().game_vertex_id() )
                     ->vertex_type() ) ) {
            m_time_to_change_graph_point =
                Device.dwTimeGlobal + ::Random32.random( 60000 ) + 60000;
            break;
        }
    ...
    if ( !m_spawn_ini || !m_spawn_ini->section_exist( "patrol" ) ||
         !m_spawn_ini->line_exist( "patrol", "way" ) ) {
        m_walk_on_way = false;
    } else {
        m_walk_on_way = true;
    }
    ...
    if ( m_walk_on_way ) {
        ...
        m_path = ai().patrol_paths().path(
            m_spawn_ini->r_string( "patrol", "way" ) );
    }
...
void CAI_Rat::CreateSkeleton() {
    ...
    element->SetMaterial(
        smart_cast< IKinematics* >( Visual() )
            ->LL_GetData(
                smart_cast< IKinematics* >( Visual() )->LL_GetBoneRoot() )
            .game_mtl_idx );
    ...
    if ( !fsimilar( 0.f, m_saved_impulse ) ) {
        m_pPhysicsShell->applyHit( m_saved_hit_position, m_saved_hit_dir,
                                   m_saved_impulse, 0, m_saved_hit_type );
    }
    /*
    IKinematics* M		= smart_cast<IKinematics*>(Visual());
    VERIFY(M); m_pPhysicsShell		= P_create_Shell();

    //get bone instance
    int id=M->LL_BoneID("bip01_pelvis");
    CBoneInstance& instance=M->LL_GetBoneInstance
    (id);
...
void CAI_Rat::shedule_Update( u32 dt ) {
    if ( !monster_squad().get_squad( this )->GetLeader() ||
         !monster_squad().get_squad( this )->GetLeader()->g_Alive() ) {
...
void CAI_Rat::UpdateCL() {
    ...
    if ( !Useful() ) {
        ...
        if ( squad && squad->SquadActive() && squad->GetLeader() == this &&
...
// void CAI_Rat::Hit(float P,Fvector &dir,CObject*who,s16 element,Fvector
// p_in_object_space,float impulse, ALife::EHitType hit_type /*=
// ALife::eHitTypeWound*/)
void CAI_Rat::Hit( SHit* pHDS ) {
    //	inherited::Hit
    //(P,dir,who,element,p_in_object_space,impulse, hit_type);
    inherited::Hit( pHDS );
    if ( !m_pPhysicsShell ) {
