...
void CAI_Rat::FreeHuntingActive() {
    ...
    CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE(
        ( memory().enemy().selected() &&
          ( ( memory().enemy().selected()->Position().distance_to(
                  m_tSafeSpawnPosition ) < m_fMaxPursuitRadius ) ||
            ( Position().distance_to( m_tSafeSpawnPosition ) >
              m_fMaxHomeRadius ) ) ),
        aiRatAttackRun );

    CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE( m_fMorale < m_fMoraleNormalValue,
                                              aiRatUnderFire );

    if ( ( m_tLastSound.dwTime >= m_dwLastUpdateTime ) &&
         ( !m_tLastSound.tpEntity ||
           ( ( !memory().item().selected() ||
               ( memory().item().selected()->ID() !=
                 m_tLastSound.tpEntity->ID() ) ) &&
             ( m_tLastSound.tpEntity->g_Team() != g_Team() ) ) ) &&
         !memory().enemy().selected() ) {
        SWITCH_TO_NEW_STATE_THIS_UPDATE( aiRatFreeRecoil );
    }

    CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE( memory().item().selected(),
                                              aiRatEatCorpse );
    ...
    if ( bfCheckIfGoalChanged() ) {
        if ( m_bStateChanged ||
             ( Position().distance_to( m_tSpawnPosition ) >
               m_fStableDistance ) ||
             ( ::Random.randF( 0, 1 ) > m_fChangeActiveStateProbability ) )
            if ( Position().distance_to( m_tSafeSpawnPosition ) >
                 m_fMaxHomeRadius )
    ...
    if ( m_bStateChanged ||
         ( fis_zero( m_fSpeed ) &&
           ( angle_difference( movement().m_body.target.yaw,
...
void CAI_Rat::UnderFire() {
    ...
    if ( memory().enemy().selected() ) {
        ...
        if ( m_tLastSound.dwTime >= m_dwLastUpdateTime ) {
            ...
            if ( m_bStateChanged ) {
                Fvector tTemp;
                tTemp.setHP( -movement().m_body.current.yaw,
...
void CAI_Rat::AttackFire() {
    ...
    CHECK_IF_GO_TO_NEW_STATE(
        ( memory().enemy().selected()->Position().distance_to( Position() ) >
          m_fAttackDistance ),
        aiRatAttackRun )
    ...
    CHECK_IF_GO_TO_NEW_STATE( angle_difference( movement().m_body.current.yaw,
                                                sTemp.yaw ) > m_fAttackAngle,
                              aiRatAttackRun )
...
void CAI_Rat::AttackRun() {
    ...
    ERatStates eState = ERatStates( dwfChooseAction(
    ...
    if ( eState != m_eCurrentState ) {
        eState = ERatStates( dwfChooseAction(
            0 * m_dwActionRefreshRate, m_fAttackSuccessProbability,
            m_fAttackSuccessProbability, m_fAttackSuccessProbability,
            m_fAttackSuccessProbability, g_Team(), g_Squad(), g_Group(),
            m_eCurrentState, m_eCurrentState, m_eCurrentState, aiRatRetreat,
            aiRatRetreat, this, 30.f ) );
    ...
    CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE(
        memory().enemy().selected() &&
            ( m_tSafeSpawnPosition.distance_to(
                  memory().enemy().selected()->Position() ) >
              m_fMaxPursuitRadius ),
        aiRatReturnHome );
    ...
    CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE(
        ( memory().enemy().selected()->Position().distance_to( Position() ) <=
          m_fAttackDistance ) &&
            ( angle_difference( movement().m_body.target.yaw, sTemp.yaw ) <=
              m_fAttackAngle ),
        aiRatAttackFire )

    if ( ( Device.dwTimeGlobal - m_previous_query_time > TIME_TO_GO ) ||
         !m_previous_query_time ) {
...
void CAI_Rat::Retreat() {
    ...
    if ( memory().enemy().selected() &&
        ...
        if ( ( memory().enemy().selected()->Position().distance_to(
                   Position() ) <= m_fAttackDistance ) &&
             ( angle_difference( movement().m_body.target.yaw, sTemp.yaw ) >
               m_fAttackAngle ) ) {
            m_fSpeed = 0.f;
            return;
...
void CAI_Rat::Pursuit() {
    ...
    if ( memory().enemy().selected() &&
         ( Device.dwTimeGlobal -
               memory().memory( memory().enemy().selected() ).m_level_time >=
           m_dwLostMemoryTime ) ) {
        memory().enable( memory().enemy().selected(), false );
        GO_TO_PREV_STATE_THIS_UPDATE;
    }

    CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE( memory().enemy().selected(),
                                              aiRatAttackRun );

    CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE(
        ( m_fMorale < m_fMoraleNormalValue ), aiRatUnderFire );
...
void CAI_Rat::FreeRecoil() {
    ...
    CHECK_IF_GO_TO_PREV_STATE_THIS_UPDATE( m_dwLastUpdateTime >
                                           m_dwLostRecoilTime + 2000 );
...
void CAI_Rat::EatCorpse() {
    ...
    CHECK_IF_GO_TO_PREV_STATE_THIS_UPDATE(
        !memory().item().selected() || ( m_fMorale < m_fMoraleNormalValue ) );
    ...
    direction.getHP( y, p );
    if ( a &&
         angle_difference( y, -movement().m_body.current.yaw ) < PI_DIV_6 ) {
        m_fSpeed = 0;
        if ( Device.dwTimeGlobal - m_previous_query_time > m_dwHitInterval ) {
            m_previous_query_time = Device.dwTimeGlobal;
            const CEntityAlive* const_corpse =
                smart_cast< const CEntityAlive* >( memory().item().selected() );
            VERIFY( const_corpse );
