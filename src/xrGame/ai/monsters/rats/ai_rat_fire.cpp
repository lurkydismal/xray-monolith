...
void CAI_Rat::Exec_Action( float /**dt/**/ ) {
    switch ( m_tAction ) {
        case eRatActionAttackBegin: {
            ...
            sound().play(
                eRatSoundAttack ); //,0,0,m_dwHitInterval+1,m_dwHitInterval);
            if ( memory().enemy().selected() &&
                ...
                tDirection.sub( memory().enemy().selected()->Position(),
                                this->Position() );
                vfNormalizeSafe( tDirection );

                if ( this->Local() && memory().enemy().selected() ) {
                    CEntityAlive* entity_alive = const_cast< CEntityAlive* >(
                    ...
                    NET_Packet l_P;
                    SHit HS;
                    HS.GenHeader( GE_HIT, id_to ); //					u_EventGen
                                                   //(l_P,GE_HIT, id_to);
                    HS.whoID = ( id_from );        //					l_P.w_u16
                                                   //(id_from);
                    HS.weaponID = ( id_from );     //					l_P.w_u16
                                                   //(id_from);
                    HS.dir = ( tDirection );       //					l_P.w_dir
                                                   //(tDirection);
                    HS.power = ( m_fHitPower );    //					l_P.w_float
                                                   //(m_fHitPower);
                    HS.boneID = ( 0 );             //					l_P.w_s16
                                                   //(0);
                    HS.p_in_bone_space =
                        ( position_in_bone_space ); //					l_P.w_vec3
                                                    //(position_in_bone_space);
                    HS.impulse = ( 0.f );           //					l_P.w_float
                                                    //(0.f);
                    HS.hit_type =
                        ( ALife::eHitTypeWound ); //					l_P.w_u16
...
void CAI_Rat::HitSignal( float amount,
                         Fvector& vLocalDir,
                         CObject* who,
                         s16 /**element/**/ ) {
    // Save event
...
float CAI_Rat::evaluate( const CItemManager* manager,
                         const CGameObject* object ) const {
    const CEntityAlive* entity_alive =
        smart_cast< const CEntityAlive* >( object );
    VERIFY( entity_alive );
    if ( !entity_alive->g_Alive() ) {
        if ( ( Device.dwTimeGlobal - entity_alive->GetLevelDeathTime() <
               m_dwEatCorpseInterval ) &&
             ( entity_alive->m_fFood > 0 ) &&
             ( m_bEatMemberCorpses ||
               ( entity_alive->g_Team() != g_Team() ) ) &&
             ( m_bCannibalism || ( entity_alive->CLS_ID != CLS_ID ) ) )
            return ( entity_alive->m_fFood * entity_alive->m_fFood ) *
                   Position().distance_to( entity_alive->Position() );
        else
            return ( flt_max );
    } else
        return ( flt_max );
    ...
    switch ( m_eCurrentState ) {
        ...
        case aiRatFreePassive: {
            if ( m_fMorale < m_fMoraleNormalValue ) {
                m_fMorale +=
                    m_fMoraleRestoreQuant; //*(1.f -
                                           //fDistance/m_fMoraleNullRadius);
                if ( m_fMorale > m_fMoraleNormalValue )
                    m_fMorale = m_fMoraleNormalValue;
            } else if ( m_fMorale > m_fMoraleNormalValue ) {
                m_fMorale -=
                    m_fMoraleRestoreQuant; //*(fDistance/m_fMoraleNullRadius);
                if ( m_fMorale < m_fMoraleNormalValue )
                    m_fMorale = m_fMoraleNormalValue;
            }
            break;
        ...
        case aiRatUnderFire:
        case aiRatRetreat: {
            // m_fMorale += fDistance <= m_fMoraleNullRadius ?
            // m_fMoraleRestoreQuant : 0; m_fMorale +=
            // m_fMoraleRestoreQuant*(m_fMoraleNullRadius/fDistance);
        ...
        case aiRatReturnHome: {
            // m_fMorale += m_fMoraleRestoreQuant*(1.f -
            // fDistance/m_fMoraleNullRadius); m_fMorale +=
            // m_fMoraleRestoreQuant*(m_fMoraleNullRadius/fDistance);
            m_fMorale += m_fMoraleRestoreQuant;
            break;
        }
