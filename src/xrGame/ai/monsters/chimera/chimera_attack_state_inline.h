...
ChimeraAttackState< Object >::ChimeraAttackState( Object* obj )
    : inherited( obj ) {}
...
float ChimeraAttackState< Object >::calculate_min_run_distance() const {
    ...
    float const jump_max_radius =
        this->object->com_man().get_jump_control()->get_max_distance();

    VERIFY( h < jump_max_radius );
    float const min_run_part2 =
        _sqrt( jump_max_radius * jump_max_radius - h * h );
...
void ChimeraAttackState< Object >::finalize() {
    if ( m_state == state_rotate || m_state == state_prepare_jump ) {
        if ( this->object->control().get_capturer( ControlCom::eControlPath ) ==
             m_capturer ) {
            this->object->control().release( m_capturer,
                                             ControlCom::eControlPath );
        }
        if ( this->object->control().get_capturer(
                 ControlCom::eControlMovement ) == m_capturer ) {
            this->object->control().release( m_capturer,
                                             ControlCom::eControlMovement );
        }
...
template < class Object >
bool ChimeraAttackState< Object >::check_control_start_conditions(
...
template < class Object >
bool ChimeraAttackState< Object >::check_if_jump_possible(
    Fvector const& target ) {
    m_allow_jump = true;
    bool const possible =
        this->object->com_man().check_if_jump_possible( target, true );
    m_allow_jump = false;
    if ( !possible )
        return false;
    ...
    if ( this->object->com_man().get_jump_control()->jump_intersect_geometry(
             target, ( CObject* )this->object->EnemyMan.get_enemy() ) )
        return false;
...
template < class Object >
bool ChimeraAttackState< Object >::jump( Fvector const& target,
                                         bool attack_jump ) {
    ...
    m_allow_jump = true;
    bool const jumped = this->object->com_man().jump_if_possible(
        target, ( CEntityAlive* )this->object->EnemyMan.get_enemy(),
...
bool ChimeraAttackState< Object >::select_target_for_move() {
    ...
    if ( dist2attack_pos < 5.f ) {
        ...
        if ( m_run_side == run_side_undefined ||
             current_time() > m_run_side_select_tick ) {
            bool left_side =
                self2enemy.x * self2behind.z - self2enemy.z * self2behind.x >
                0.f;
            if ( !( rand() % 2 ) ) {
                left_side ^= true;
            }
        ...
        for ( ; index < move_scan_points; ++index ) {
            float const angle = move_scan_angle * ( index + 1 ) *
                                ( m_run_side == run_side_left ? -1.f : +1.f );

            Fvector const scan_point =
                enemy_pos + rotate_point( enemy2self, angle );
...
template < class Object >
bool ChimeraAttackState< Object >::select_target_for_jump(
    enum_action const action ) {
    CEntityAlive const* const enemy = this->object->EnemyMan.get_enemy();
    Fvector const enemy_pos = enemy->Position();
    ...
    float const attack_radius =
        3 + float( rand() % ( u32 )( get_attack_radius() - 3 ) );
    ...
    for ( u32 index = 0; index < num_scan_points; ++index ) {
        ...
        float const angle =
            scan_angle * ( ( index / 2 ) + 1 ) * ( left_side ? -1.f : +1.f );

        Fvector const scan_point =
            enemy_pos + rotate_point( behind_point - enemy_pos, angle );
...
void ChimeraAttackState< Object >::set_turn_animation() {
    bool const rotate_right =
        this->object->control().direction().is_from_right( m_target );
    this->object->anim().set_override_animation(
        rotate_right ? eAnimFastStandTurnRight : eAnimFastStandTurnLeft, 0 );
}
...
void ChimeraAttackState< Object >::execute() {
    ...
    u32 const max_attack_jumps =
        this->object->get_attack_params().num_attack_jumps;
    u32 const max_prepare_jumps =
        this->object->get_attack_params().num_prepare_jumps;
    ...
    bool const can_prepare_jump =
        ...
        current_time() >
            m_last_jump_time +
                this->object->get_attack_params().prepare_jump_timeout;
    bool const can_attack_jump =
        !preparing_state &&
        current_time() >
            m_last_jump_time +
                this->object->get_attack_params().attack_jump_timeout;
    ...
    if ( m_state == state_rotate ) {
        ...
        if ( m_attack_jump ) {
            ...
            if ( !ai().level_graph().valid_vertex_position( m_target ) ) {
                ...
                m_target =
                    ai().level_graph().vertex_position( m_target_vertex );
            } else {
        ...
        bool const good_aiming =
            _abs( self2target_yaw - self_dir_yaw ) < deg2rad( 20.f );
        ...
        if ( !good_aiming || in_stealth ) {
            if ( good_aiming && in_stealth )
                this->object->anim().set_override_animation( eAnimPrepareAttack,
                                                             0 );
            else
                set_turn_animation();
            ...
            if ( valid_target ) {
                ...
                float length = this->object->anim().get_animation_length(
                    eAnimUpperAttack, 0 );
        ...
        if ( select_target_for_jump( can_attack_jump ? action_attack
                                                     : action_jump ) ) {
            m_target_vertex = ai().level_graph().vertex_id( m_target );
            ...
            if ( can_attack_from_behind ) {
                m_stealth_end_tick =
                    current_time() +
                    this->object->get_attack_params().stealth_timeout;
            }

            this->object->control().capture( m_capturer,
                                             ControlCom::eControlPath );
            this->object->control().capture( m_capturer,
                                             ControlCom::eControlMovement );
