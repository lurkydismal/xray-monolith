...
float ChimeraAttackState< Object >::calculate_min_run_distance() const {
    ...
    float const jump_max_radius =
        object->com_man().get_jump_control()->get_max_distance();
...
void ChimeraAttackState< Object >::finalize() {
    if ( m_state == state_rotate || m_state == state_prepare_jump ) {
        if ( object->control().get_capturer( ControlCom::eControlPath ) ==
             m_capturer ) {
            object->control().release( m_capturer, ControlCom::eControlPath );
        }
        if ( object->control().get_capturer( ControlCom::eControlMovement ) ==
             m_capturer ) {
            object->control().release( m_capturer,
...
bool ChimeraAttackState< Object >::check_if_jump_possible(
    ...
    m_allow_jump = true;
    bool const possible =
        object->com_man().check_if_jump_possible( target, true );
    m_allow_jump = false;
    if ( !possible )
        return false;
    ...
    if ( object->com_man().get_jump_control()->jump_intersect_geometry(
             target, ( CObject* )object->EnemyMan.get_enemy() ) )
        return false;
...
bool ChimeraAttackState< Object >::jump( Fvector const& target,
    ...
    bool const jumped = object->com_man().jump_if_possible(
        target, ( CEntityAlive* )object->EnemyMan.get_enemy(),
...
void ChimeraAttackState< Object >::set_turn_animation() {
    bool const rotate_right =
        object->control().direction().is_from_right( m_target );
...
void ChimeraAttackState< Object >::execute() {
    ...
    bool const can_prepare_jump =
        preparing_state &&
        current_time() >
            m_last_jump_time + object->get_attack_params().prepare_jump_timeout;
    bool const can_attack_jump =
        !preparing_state &&
        current_time() >
            m_last_jump_time + object->get_attack_params().attack_jump_timeout;
    ...
    if ( m_state == state_rotate ) {
        ...
        if ( !good_aiming || in_stealth ) {
            ...
            if ( valid_target ) {
                ...
                float length =
                    object->anim().get_animation_length( eAnimUpperAttack, 0 );
        ...
        if ( select_target_for_jump( can_attack_jump ? action_attack
            ...
            if ( can_attack_from_behind ) {
                m_stealth_end_tick =
                    current_time() +
                    object->get_attack_params().stealth_timeout;
            }

            object->control().capture( m_capturer, ControlCom::eControlPath );
            object->control().capture( m_capturer,
                                       ControlCom::eControlMovement );
