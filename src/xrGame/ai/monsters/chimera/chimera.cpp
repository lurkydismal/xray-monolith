...
void CChimera::Load( LPCSTR section ) {
    ...
    anim().AddAnim( eAnimFastStandTurnLeft, "stand_run_turn_90_ls_", -1,
                    &m_velocity_rotate, PS_STAND );
    anim().AddAnim( eAnimFastStandTurnRight, "stand_run_turn_90_rs_", -1,
                    &m_velocity_rotate, PS_STAND );
    ...
    anim().AddAnim( eAnimPrepareAttack, "stand_agressive_idle_", -1,
                    &velocity_none, PS_STAND );
    anim().AddAnim( eAnimSteal, "stand_walk_", -1, &velocity_steal, PS_STAND );
    anim().AddAnim( eAnimDie, "stand_idle_", -1, &velocity_none, PS_STAND );
    anim().AddAnim( eAnimThreaten, "stand_idle_", -1, &velocity_none,
                    PS_STAND );

    anim().AddAnim( eAnimRunTurnLeft, "stand_run_turn_ls_", -1, &velocity_run,
                    PS_STAND );
    anim().AddAnim( eAnimRunTurnRight, "stand_run_turn_rs_", -1, &velocity_run,
                    PS_STAND );

    anim().AddAnim( eAnimUpperAttack, "jump_attack_", -1,
                    &m_velocity_jump_start, PS_STAND );
    ...
    m_attack_params.attack_radius =
        READ_IF_EXISTS( pSettings, r_float, section, "attack_radius", 10.f );
    m_attack_params.prepare_jump_timeout = READ_IF_EXISTS(
        pSettings, r_u32, section, "prepare_jump_timeout", 2000 );
    m_attack_params.attack_jump_timeout = READ_IF_EXISTS(
        pSettings, r_u32, section, "attack_jump_timeout", 1000 );
    m_attack_params.stealth_timeout =
        READ_IF_EXISTS( pSettings, r_u32, section, "stealth_timeout", 2000 );
    m_attack_params.force_attack_distance = READ_IF_EXISTS(
        pSettings, r_float, section, "force_attack_distance", 8 );
    m_attack_params.num_attack_jumps =
        READ_IF_EXISTS( pSettings, r_u32, section, "num_attack_jumps", 4 );
    m_attack_params.num_prepare_jumps =
        READ_IF_EXISTS( pSettings, r_u32, section, "num_prepare_jumps", 2 );
#ifdef DEBUG
...
EAction CChimera::CustomVelocityIndex2Action( u32 velocity_index ) {
    switch ( velocity_index ) {
        case MonsterMovement::eChimeraVelocityParameterJumpGround:
            return ACT_RUN;
        case MonsterMovement::eChimeraVelocityParameterPrepare:
            return ACT_RUN;
    }
