...
bool CTelekineticObject::init( CTelekinesis* tele,
...
void CTelekineticObject::set_sound( const ref_sound& snd_hold,
                                    const ref_sound& snd_throw ) {
...
void CTelekineticObject::raise( float step ) {
    ...
    // Tosox
    // Clamp elem_size so objects with many elements don't hover to high up in
    // the air Common physic objects have elem_size 1-2 while bodies have about
    // 11
    ...
    if ( OnServer() )
        ( object->m_pPhysicsShell->get_ElementByStoreOrder( 0 ) )
            ->applyGravityAccel( dir );
...
void CTelekineticObject::keep() {
    ...
    // if (time_keep_updated + KEEP_IMPULSE_UPDATE > Device.dwTimeGlobal)
    // return;
    ...
    if ( cur_h > target_height + 0.6f )
        ...
        dir.set( Random.randF( -1.0f, 1.0f ), Random.randF( -1.0f, 1.0f ),
...
void CTelekineticObject::release() {
    ...
    if ( OnServer() ) {
        ...
        object->m_pPhysicsShell->applyImpulse(
            dir_inv, 0.5f * object->m_pPhysicsShell->getMass() );
    }
    // state = TS_None;
...
void CTelekineticObject::fire( const Fvector& target, float power ) {
    ...
    if ( OnServer() ) {
        // выполнить бросок
        for ( u32 i = 0; i < object->m_pPhysicsShell->get_ElementsNumber();
              i++ )
            object->m_pPhysicsShell->get_ElementByStoreOrder( u16( i ) )
                ->applyImpulse(
                    dir, power * 20.f * object->m_pPhysicsShell->getMass() /
                             object->m_pPhysicsShell->Elements().size() );
    };
};
...
void CTelekineticObject::rotate() {
    ...
    if ( OnServer() )
        object->m_pPhysicsShell->applyImpulse(
            dir, 2.5f * object->m_pPhysicsShell->getMass() );
}
