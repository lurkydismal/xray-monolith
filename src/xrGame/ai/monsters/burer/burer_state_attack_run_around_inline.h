...
void CStateBurerAttackRunAroundAbstract::initialize() {
    ...
    dir_to_enemy.sub( object->EnemyMan.get_enemy()->Position(),
                      object->Position() );
    dir_to_enemy.normalize();

    dir_from_enemy.sub( object->Position(),
    ...
    if ( dist > 30.f ) {
        ...
        dest_direction.sub( object->EnemyMan.get_enemy()->Position(),
                            selected_point );
...
bool CStateBurerAttackRunAroundAbstract::check_completion() {
    if ( ( time_started + object->m_max_runaway_time < Device.dwTimeGlobal ) ||
         ( object->control().path_builder().is_moving_on_path() &&
           object->control().path_builder().is_path_end( 2.f ) ) ) {
