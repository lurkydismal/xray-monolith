...
CStateBurerAttackRunAroundAbstract::CStateBurerAttackRunAround( _Object* obj )
    : inherited( obj ) {
    time_started = 0;
}
...
void CStateBurerAttackRunAroundAbstract::initialize() {
    ...
    dir_to_enemy.sub( this->object->EnemyMan.get_enemy()->Position(),
                      this->object->Position() );
    dir_to_enemy.normalize();

    dir_from_enemy.sub( this->object->Position(),
    ...
    if ( dist > 30.f ) {
        // бежать к врагу
        selected_point.mad( this->object->Position(), dir_to_enemy,
                            DIST_QUANT );
    } else if ( ( dist < 20.f ) && ( dist > 4.f ) ) {
        // убегать от врага
        selected_point.mad( this->object->Position(), dir_from_enemy,
                            DIST_QUANT );
        dest_direction.sub( this->object->EnemyMan.get_enemy()->Position(),
                            selected_point );
        dest_direction.normalize();
    } else {
        // выбрать случайную позицию
        selected_point =
            random_position( this->object->Position(), DIST_QUANT );
        dest_direction.sub( this->object->EnemyMan.get_enemy()->Position(),
                            selected_point );
        dest_direction.normalize();
    }
...
bool CStateBurerAttackRunAroundAbstract::check_completion() {
    if ( ( time_started + this->object->m_max_runaway_time <
           Device.dwTimeGlobal ) ||
         ( this->object->control().path_builder().is_moving_on_path() &&
           this->object->control().path_builder().is_path_end( 2.f ) ) ) {
