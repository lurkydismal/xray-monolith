...
void CStateBurerAttack< Object >::initialize() {
    ...
    // 	CMonsterSquad *squad					=
    // monster_squad().get_squad(object); 	if ( squad )
    // 	{
...
void CStateBurerAttack< Object >::execute() {
    ...
    if ( object->conditions().GetHealth() <=
    ...
    bool const anti_aim_ready =
        get_state( eStateBurerAttack_AntiAim )->check_start_conditions();
    m_allow_anti_aim = false;

    bool const gravi_ready =
        get_state( eStateBurerAttack_Gravi )->check_start_conditions();
    bool const shield_ready =
        get_state( eStateBurerAttack_Shield )->check_start_conditions();
    bool const tele_ready =
        get_state( eStateBurerAttack_Tele )->check_start_conditions();
    ...
    if ( gravi_ready ) {
        ...
        select_state( eStateBurerAttack_AntiAim );
                ...
                current_substate != eStateBurerAttack_RunAround ) {
    ...
    if ( m_lost_delta_health ||
        ...
        bool const good_aiming =
            angle_between_vectors( self2enemy, object->Direction() ) <
            deg2rad( 20.f );
        ...
        if ( !good_aiming ) {
            bool const rotate_right =
                object->control().direction().is_from_right( enemy_pos );
