...
CStateBurerAttack< Object >::CStateBurerAttack( Object* obj )
    : inherited( obj ) {
    this->add_state( eStateBurerAttack_Tele,
                     xr_new< CStateBurerAttackTele< Object > >( obj ) );
    this->add_state( eStateBurerAttack_Gravi,
                     xr_new< CStateBurerAttackGravi< Object > >( obj ) );
    this->add_state( eStateBurerAttack_Melee,
                     xr_new< CStateBurerAttackMelee< Object > >( obj ) );
    this->add_state( eStateBurerAttack_FaceEnemy,
                     xr_new< CStateMonsterLookToPoint< Object > >( obj ) );
    this->add_state( eStateBurerAttack_RunAround,
                     xr_new< CStateBurerAttackRunAround< Object > >( obj ) );
    this->add_state( eStateBurerAttack_Shield,
                     xr_new< CStateBurerShield< Object > >( obj ) );
    this->add_state( eStateBurerAttack_AntiAim,
                     xr_new< CStateBurerAntiAim< Object > >( obj ) );
    this->add_state( eStateAttack_Run,
                     xr_new< CStateMonsterAttackRun< Object > >( obj ) );
    this->add_state( eStateCustomMoveToRestrictor,
                     xr_new< CStateMonsterMoveToRestrictor< Object > >( obj ) );
...
void CStateBurerAttack< Object >::initialize() {
    ...
    // 	CMonsterSquad *squad					=
    // monster_squad().get_squad(this->object); 	if ( squad )
    // 	{
    // 		squad->InformSquadAboutEnemy
...
void CStateBurerAttack< Object >::execute() {
    ...
    if ( this->object->conditions().GetHealth() <=
         m_last_health - burer::health_delta ) {
        m_last_health = this->object->conditions().GetHealth();
    ...
    bool const anti_aim_ready =
        this->get_state( eStateBurerAttack_AntiAim )->check_start_conditions();
    m_allow_anti_aim = false;

    bool const gravi_ready =
        this->get_state( eStateBurerAttack_Gravi )->check_start_conditions();
    bool const shield_ready =
        this->get_state( eStateBurerAttack_Shield )->check_start_conditions();
    bool const tele_ready =
        this->get_state( eStateBurerAttack_Tele )->check_start_conditions();
    ...
    if ( gravi_ready ) {
        ...
        this->select_state( eStateBurerAttack_AntiAim );
                ...
                this->current_substate != eStateBurerAttack_RunAround ) {
    ...
    bool const in_runaway_range =
        self2enemy_dist < this->object->m_runaway_distance;
    bool const in_normal_range =
        self2enemy_dist < this->object->m_normal_distance;
    ...
    if ( this->get_state( eStateCustomMoveToRestrictor )
    ...
    if ( m_lost_delta_health ||
         ( in_runaway_range &&
           current_time() > m_next_runaway_allowed_tick ) ) {
        m_lost_delta_health = false;
        ...
        bool const good_aiming =
            angle_between_vectors( self2enemy, this->object->Direction() ) <
            deg2rad( 20.f );
        ...
        if ( !good_aiming ) {
            bool const rotate_right =
                this->object->control().direction().is_from_right( enemy_pos );
...
template < typename Object >
bool CStateBurerAttack< Object >::check_control_start_conditions(
