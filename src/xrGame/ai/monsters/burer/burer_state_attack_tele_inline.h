...
void CStateBurerAttackTele< Object >::execute() {
    ...
    switch ( m_action ) {
        case ACTION_TELE_STARTED:
            ...
            if ( !time_started ) {
                float const time =
                    object->anim().get_animation_length( eAnimTelekinesis, 0 );
        ...
        case ACTION_TELE_FIRE: {
            ...
            ExecuteTeleFire();
            float const time =
                object->anim().get_animation_length( eAnimTeleFire, 0 );
            m_anim_end_tick = current_time() + TTime( time * 1000 );
...
void CStateBurerAttackTele< Object >::deactivate() {
    ...
    if ( object->CTelekinesis::is_active() ) {
        for ( u32 i = 0; i < object->CTelekinesis::get_objects_total_count();
              i++ ) {
            CPhysicsShellHolder* cur_object =
                object->CTelekinesis::get_object_by_index( i ).get_object();
    ...
    for ( u32 i = 0; i < object->CTelekinesis::get_objects_total_count();
        ...
        CPhysicsShellHolder* const cur_object =
            object->CTelekinesis::get_object_by_index( i ).object;
        if ( !cur_object || !cur_object->m_pPhysicsShell ||
             !cur_object->m_pPhysicsShell->isActive() ) {
            continue;
        }
...
bool CStateBurerAttackTele< Object >::check_completion() {
    float dist = object->EnemyMan.get_enemy()->Position().distance_to(
        object->Position() );
    ...
    for ( u32 i = 0; i < tpObjects.size(); i++ ) {
        ...
        if ( grenade || // grenades are handled by HandleGrenades function
             ...
             ( obj->m_pPhysicsShell->getMass() <
               object->m_tele_object_min_mass ) ||
             ( obj->m_pPhysicsShell->getMass() >
               object->m_tele_object_max_mass ) ||
             ( obj == object ) ||
...
void CStateBurerAttackTele< Object >::FireAllToEnemy() {
    ...
    for ( u32 i = 0; i < object->CTelekinesis::get_objects_count(); ++i ) {
        ...
        SCollisionHitCallback* const hit_callback =
            is_actor ? xr_new< SCollisionHitCallback >( object ) : nullptr;
...
void CStateBurerAttackTele< Object >::ExecuteTeleFire() {
    ...
    SCollisionHitCallback* const hit_callback =
        enemy->cast_actor() ? xr_new< SCollisionHitCallback >( object )
                            : nullptr;
...
void CStateBurerAttackTele< Object >::SelectObjects() {
    std::sort(
        ...
        best_object_predicate2( object->Position(),
                                object->EnemyMan.get_enemy()->Position() ) );
    ...
    for ( u32 i = 0; i < tele_objects.size(); ++i ) {
        ...
        bool const rotate =
            object->m_monster_type != CBaseMonster::eMonsterTypeIndoor;

        CTelekineticObject* tele_obj = object->CTelekinesis::activate(
            obj, object->m_tele_raise_speed, height, 10000, rotate );

        tele_obj->set_sound( object->sound_tele_hold,
                             object->sound_tele_throw );
        ...
        if ( object->CTelekinesis::get_objects_count() >=
             object->m_tele_max_handled_objects ) {
            break;
        }
    }
}
...
void CStateBurerAttackTele< Object >::HandleGrenades() {
    ...
    m_nearest.clear_not_free();
    Level().ObjectSpace.GetNearest( m_nearest, object->Position(),
                                    object->m_tele_find_radius, NULL );
    ...
    for ( u32 i = 0; i < m_nearest.size(); ++i ) {
        ...
        if ( object->CTelekinesis::get_objects_count() >=
             object->m_tele_max_handled_objects + 1 ) {
            break;
        }
    }
}
