...
void CStateBurerAttackTele< Object >::execute() {
    ...
    switch ( m_action ) {
        case ACTION_TELE_STARTED:
            ...
            if ( !time_started ) {
                float const time = this->object->anim().get_animation_length(
                    eAnimTelekinesis, 0 );
        ...
        case ACTION_TELE_FIRE: {
            ...
            ExecuteTeleFire();
            float const time =
                this->object->anim().get_animation_length( eAnimTeleFire, 0 );
            m_anim_end_tick = current_time() + TTime( time * 1000 );
...
void CStateBurerAttackTele< Object >::deactivate() {
    ...
    if ( this->object->CTelekinesis::is_active() ) {
        for ( u32 i = 0;
              i < this->object->CTelekinesis::get_objects_total_count(); i++ ) {
            CPhysicsShellHolder* cur_object =
                this->object->CTelekinesis::get_object_by_index( i )
                    .get_object();
            if ( !cur_object || !cur_object->m_pPhysicsShell ||
                 !cur_object->m_pPhysicsShell->isActive() ) {
                continue;
            }
    ...
    for ( u32 i = 0; i < this->object->CTelekinesis::get_objects_total_count();
          ++i ) {
        CPhysicsShellHolder* const cur_object =
            this->object->CTelekinesis::get_object_by_index( i ).object;
        if ( !cur_object || !cur_object->m_pPhysicsShell ||
             !cur_object->m_pPhysicsShell->isActive() ) {
            continue;
        }
...
bool CStateBurerAttackTele< Object >::check_completion() {
    float dist = this->object->EnemyMan.get_enemy()->Position().distance_to(
...
template < typename Object >
void CStateBurerAttackTele< Object >::FindFreeObjects(
    xr_vector< CObject* >& tpObjects,
    const Fvector& pos ) {
    Level().ObjectSpace.GetNearest( tpObjects, pos,
                                    this->object->m_tele_find_radius, NULL );

    for ( u32 i = 0; i < tpObjects.size(); i++ ) {
        CPhysicsShellHolder* obj =
            smart_cast< CPhysicsShellHolder* >( tpObjects[ i ] );
        CCustomMonster* custom_monster =
            smart_cast< CCustomMonster* >( tpObjects[ i ] );
        ...
        if ( grenade || // grenades are handled by HandleGrenades function
             !obj || !obj->PPhysicsShell() ||
             !obj->PPhysicsShell()->isActive() ||
             custom_monster && ( !g_telekinetic_objects_include_corpses ||
                                 custom_monster->g_Alive() ) ||
             ( obj->spawn_ini() &&
               obj->spawn_ini()->section_exist( "ph_heavy" ) ) ||
             ( obj->m_pPhysicsShell->getMass() <
               this->object->m_tele_object_min_mass ) ||
             ( obj->m_pPhysicsShell->getMass() >
               this->object->m_tele_object_max_mass ) ||
             ( obj == this->object ) ||
...
void CStateBurerAttackTele< Object >::FindObjects() {
    ...
    float dist = this->object->EnemyMan.get_enemy()->Position().distance_to(
        this->object->Position() );
    Fvector dir;
    dir.sub( this->object->EnemyMan.get_enemy()->Position(),
             this->object->Position() );
    dir.normalize();
    ...
    tele_objects.erase( std::unique( tele_objects.begin(), tele_objects.end() ),
                        tele_objects.end() );
}
...
void CStateBurerAttackTele< Object >::FireAllToEnemy() {
    ...
    CEntityAlive* const enemy =
        const_cast< CEntityAlive* >( this->object->EnemyMan.get_enemy() );
    const bool is_actor = enemy->cast_actor() != nullptr;
    const Fvector enemy_pos = get_head_position( enemy );

    for ( u32 i = 0; i < this->object->CTelekinesis::get_objects_count();
          ++i ) {
        u32 const prev_num_objects =
            this->object->CTelekinesis::get_objects_count();
        ...
        float const dist_to_enemy =
            cur_object->Position().distance_to( enemy_pos );
        float const fire_time =
            dist_to_enemy / this->object->m_tele_fly_velocity;

        SCollisionHitCallback* const hit_callback =
            is_actor ? xr_new< SCollisionHitCallback >( this->object )
                     : nullptr;
        cur_object->set_collision_hit_callback( hit_callback );
        this->object->CTelekinesis::fire_t( cur_object, enemy_pos, fire_time );

        u32 const new_num_objects =
            this->object->CTelekinesis::get_objects_count();
...
void CStateBurerAttackTele< Object >::ExecuteTeleContinue() {
    if ( time_started + this->object->m_tele_time_to_hold >
         Device.dwTimeGlobal )
        return;
    ...
    while ( i < this->object->CTelekinesis::get_objects_count() ) {
        ...
        if ( ( tele_object.get_state() == TS_Keep ) &&
             ( tele_object.time_keep_started + 1500 < Device.dwTimeGlobal ) ) {
            object_found = true;
            break;
        } else
            i++;
    ...
    if ( object_found ) {
        ...
        if ( !IsActiveObjects() ||
             ( time_started + MAX_TIME_CHECK_FAILURE < Device.dwTimeGlobal ) ) {
            m_action = ACTION_COMPLETED;
        }
    }
}
...
void CStateBurerAttackTele< Object >::ExecuteTeleFire() {
    if ( !selected_object ||
         ( selected_object && selected_object->getDestroy() ) ) {
    ...
    float const dist_to_enemy =
        selected_object->Position().distance_to( enemy_pos );
    float const fire_time = dist_to_enemy / this->object->m_tele_fly_velocity;

    SCollisionHitCallback* const hit_callback =
        enemy->cast_actor() ? xr_new< SCollisionHitCallback >( this->object )
                            : nullptr;
...
public:
    ...
    bool operator()( const CGameObject* tpObject1,
                     const CGameObject* tpObject2 ) const {
...
void CStateBurerAttackTele< Object >::SelectObjects() {
    ...
    for ( u32 i = 0; i < tele_objects.size(); ++i ) {
        ...
        if ( this->object->m_monster_type ==
             CBaseMonster::eMonsterTypeIndoor ) {
            height *= 0.7f;
        }

        bool const rotate =
            this->object->m_monster_type != CBaseMonster::eMonsterTypeIndoor;

        CTelekineticObject* tele_obj = this->object->CTelekinesis::activate(
            obj, this->object->m_tele_raise_speed, height, 10000, rotate );

        tele_obj->set_sound( this->object->sound_tele_hold,
                             this->object->sound_tele_throw );
        ...
        if ( this->object->CTelekinesis::get_objects_count() >=
             this->object->m_tele_max_handled_objects ) {
            break;
        }
    }
}
...
template < typename Object >
void xr_stdcall
CStateBurerAttackTele< Object >::OnGrenadeDestroyed( CGrenade* const grenade ) {
...
void CStateBurerAttackTele< Object >::HandleGrenades() {
    ...
    m_nearest.clear_not_free();
    Level().ObjectSpace.GetNearest( m_nearest, this->object->Position(),
                                    this->object->m_tele_find_radius, NULL );

    for ( u32 i = 0; i < m_nearest.size(); ++i ) {
        CGrenade* grenade = smart_cast< CGrenade* >( m_nearest[ i ] );

        if ( !grenade || !grenade->PPhysicsShell() ||
        ...
        grenade->set_destroy_callback( CGrenade::destroy_callback(
            this, &CStateBurerAttackTele< Object >::OnGrenadeDestroyed ) );

        float const height = 2.5f;
        bool const rotate = false;

        CTelekineticObject* tele_obj = this->object->CTelekinesis::activate(
            grenade, 3.f, height, 10000, rotate );
        tele_obj->set_sound( this->object->sound_tele_hold,
                             this->object->sound_tele_throw );
        this->object->StartTeleObjectParticle( grenade );

        if ( this->object->CTelekinesis::get_objects_count() >=
             this->object->m_tele_max_handled_objects + 1 ) {
            break;
        }
    }
}
