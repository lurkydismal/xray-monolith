...
CStateBurerShield< Object >::CStateBurerShield( Object* obj )
    : inherited( obj ) {
    m_started = false;
...
void CStateBurerShield< Object >::initialize() {
    ...
    MotionID motion;
    this->object->anim().get_animation_info( eAnimShieldStart, 0, motion,
                                             m_shield_start_anim_length_sec );
}

template < class Object >
void CStateBurerShield< Object >::execute() {
    if ( !m_started ) // && current_time() > m_last_shield_started +
                      // TTime(m_shield_start_anim_length_sec*1000) )
    {
        m_started = true;
        this->object->ActivateShield();
    }

    if ( m_started && this->object->m_shield_keep_particle != 0 &&
         current_time() > m_next_particle_allowed ) {
        this->object->CParticlesPlayer::StartParticles(
            this->object->m_shield_keep_particle, Fvector().set( 0, 1, 0 ),
            this->object->ID(), -1, true );

        m_next_particle_allowed =
            current_time() + this->object->m_shield_keep_particle_period;
    }

    this->object->face_enemy();
    this->object->set_action( ACT_STAND_IDLE );

    this->object->anim().set_override_animation( m_started ? eAnimShieldContinue
                                                           : eAnimShieldStart );
}
