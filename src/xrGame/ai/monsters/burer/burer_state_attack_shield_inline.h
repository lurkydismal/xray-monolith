...
void CStateBurerShield< Object >::initialize() {
    ...
    MotionID motion;
    object->anim().get_animation_info( eAnimShieldStart, 0, motion,
...
void CStateBurerShield< Object >::execute() {
    ...
    if ( m_started && object->m_shield_keep_particle != 0 &&
         current_time() > m_next_particle_allowed ) {
        object->CParticlesPlayer::StartParticles(
            object->m_shield_keep_particle, Fvector().set( 0, 1, 0 ),
            object->ID(), -1, true );

        m_next_particle_allowed =
            current_time() + object->m_shield_keep_particle_period;
    }

    object->face_enemy();
    object->set_action( ACT_STAND_IDLE );

    object->anim().set_override_animation( m_started ? eAnimShieldContinue
                                                     : eAnimShieldStart );
}
