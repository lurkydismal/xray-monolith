...
void CBurer::reload( LPCSTR section ) {
    ...
    // add specific sounds
    sound().add( pSettings->r_string( section, "sound_gravi_attack" ),
                 DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING,
                 MonsterSound::eHighPriority + 2,
                 u32( MonsterSound::eBaseChannel ), eMonsterSoundGraviAttack,
                 "head" );

    sound().add( pSettings->r_string( section, "sound_tele_attack" ),
                 DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING,
                 MonsterSound::eHighPriority + 3,
                 u32( MonsterSound::eBaseChannel ), eMonsterSoundTeleAttack,
                 "head" );
}
...
void CBurer::Load( LPCSTR section ) {
    ...
    // anim().AddReplacedAnim		(&m_bDamaged, eAnimStandIdle,
    // eAnimStandDamaged); anim().AddReplacedAnim		(&m_bDamaged,
    // eAnimRun,			eAnimRunDamaged); anim().AddReplacedAnim
    // (&m_bDamaged, eAnimWalkFwd,		eAnimWalkDamaged);
    ...
    particle_gravi_prepare =
        pSettings->r_string( section, "Particle_Gravi_Prepare" );
    particle_tele_object =
        pSettings->r_string( section, "Particle_Tele_Object" );

    ::Sound->create( sound_gravi_wave,
                     pSettings->r_string( section, "sound_gravi_wave" ),
                     st_Effect, SOUND_TYPE_WORLD );
    ::Sound->create( sound_tele_hold,
                     pSettings->r_string( section, "sound_tele_hold" ),
                     st_Effect, SOUND_TYPE_WORLD );
    ::Sound->create( sound_tele_throw,
    ...
    m_gravi.impulse_to_objects =
        pSettings->r_float( section, "Gravi_Impulse_To_Objects" );
    m_gravi.impulse_to_enemy =
        pSettings->r_float( section, "Gravi_Impulse_To_Enemy" );
    m_gravi.hit_power = pSettings->r_float( section, "Gravi_Hit_Power" );

    m_weight_to_stamina_hit = READ_IF_EXISTS( pSettings, r_float, section,
                                              "weight_to_stamina_hit", 0.02f );
    m_weapon_drop_stamina_k = READ_IF_EXISTS( pSettings, r_float, section,
                                              "weapon_drop_stamina_k", 3.f );

    m_runaway_distance =
        READ_IF_EXISTS( pSettings, r_float, section, "runaway_distance", 6.f );
    m_normal_distance =
        READ_IF_EXISTS( pSettings, r_float, section, "normal_distance", 12.f );
    m_max_runaway_time =
        READ_IF_EXISTS( pSettings, r_u32, section, "max_runaway_time", 5000 );

    m_weapon_drop_velocity = READ_IF_EXISTS( pSettings, r_float, section,
                                             "weapon_drop_velocity", 8 );

    m_shield_cooldown =
        READ_IF_EXISTS( pSettings, r_u32, section, "shield_cooldown", 4000 );
    m_shield_time =
        READ_IF_EXISTS( pSettings, r_u32, section, "shield_time", 3000 );
    m_shield_keep_particle = READ_IF_EXISTS( pSettings, r_string, section,
                                             "shield_keep_particle", 0 );
    m_shield_keep_particle_period = READ_IF_EXISTS(
        pSettings, r_u32, section, "shield_keep_particle_period", 1000 );

    m_tele_max_handled_objects =
        pSettings->r_u32( section, "Tele_Max_Handled_Objects" );
    m_tele_max_time =
        READ_IF_EXISTS( pSettings, r_u32, section, "Tele_Max_Time", 10000 );
    m_tele_time_to_hold = pSettings->r_u32( section, "Tele_Time_To_Hold" );
    m_tele_object_min_mass =
        pSettings->r_float( section, "Tele_Object_Min_Mass" );
    m_tele_object_max_mass =
        pSettings->r_float( section, "Tele_Object_Max_Mass" );
    m_tele_find_radius = pSettings->r_float( section, "Tele_Find_Radius" );
    m_tele_min_distance =
        READ_IF_EXISTS( pSettings, r_float, section, "tele_min_distance", 8 );
    m_tele_max_distance =
        READ_IF_EXISTS( pSettings, r_float, section, "tele_max_distance", 30 );
    m_tele_raise_speed =
        READ_IF_EXISTS( pSettings, r_float, section, "tele_raise_speed", 5.f );
    m_tele_fly_velocity = READ_IF_EXISTS( pSettings, r_float, section,
                                          "tele_fly_velocity", 30.f );
    m_tele_object_height = READ_IF_EXISTS( pSettings, r_float, section,
                                           "tele_object_height", 2.f );
    ...
    anim().AddAnim( eAnimShieldStart, "stand_shield_", -1, &velocity_turn,
                    PS_STAND );
    //, 	"fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
    anim().AddAnim( eAnimShieldContinue, "stand_shield_idle_", -1,
    ...
    anim().AddAnim( eAnimRunTurnLeft, "stand_run_fwd_turn_left_", -1,
                    &velocity_run, PS_STAND );
    anim().AddAnim( eAnimRunTurnRight, "stand_run_fwd_turn_right_", -1,
                    &velocity_run, PS_STAND );
    ...
    // "fx_stand_b", "fx_stand_l", "fx_stand_r");
    // 	anim().AddAnim(eAnimCheckCorpse,	"sit_check_corpse_",	-1,
    // &velocity_none,		PS_SIT); //, 	"fx_stand_f", "fx_stand_b",
    // "fx_stand_l", "fx_stand_r"); 	anim().AddAnim(eAnimSitStandUp,
    // "sit_stand_up_",		-1, &velocity_none,		PS_SIT); //,
    // "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
...
void CBurer::PostLoad( LPCSTR section ) {
    ...
    m_anti_aim->set_callback(
        anti_aim_ability::hit_callback( this, &CBurer::StaminaHit ) );
}
...
void xr_stdcall CBurer::StaminaHit() {
    ...
    bool do_weapon_drop = Actor()->conditions().GetPower() <
                          stamina_hit * m_weapon_drop_stamina_k;

    Actor()->conditions().PowerHit( stamina_hit, false );

    if ( do_weapon_drop ) {
        ::luabind::functor< bool > funct;
        if ( ai().script_engine().functor( "_G.CBurer_BeforeWeaponDropCallback",
                                           funct ) )
            do_weapon_drop = funct( this->lua_game_object(),
                                    active_weapon->lua_game_object() );
    }
    ...
    if ( do_weapon_drop ) {
        ...
        active_weapon->SetActivationSpeedOverride( normalize( dir ) *
...
void CBurer::UpdateGraviObject() {
    ...
    if ( !m_gravi_object.enemy ||
         ( m_gravi_object.enemy && m_gravi_object.enemy->getDestroy() ) ) {
    ...
    if ( Level().ObjectSpace.RayPick( new_pos, dir, trace_dist,
                                      collide::rqtBoth, l_rq, NULL ) ) {
        const CObject* enemy =
            smart_cast< const CObject* >( m_gravi_object.enemy );
        if ( ( l_rq.O == enemy ) && ( l_rq.range < trace_dist ) ) {
            ...
            if ( b_enemy_visible ) {
                ...
                HitEntity( m_gravi_object.enemy, m_gravi.hit_power,
                           m_gravi.impulse_to_enemy, impulse_dir,
    ...
    intrusive_ptr< CParticlesObject > ps =
        Particles::Details::Create( particle_gravi_wave, TRUE );
    ...
    Level().ObjectSpace.GetNearest( m_nearest, m_gravi_object.cur_pos,
                                    m_gravi.radius, NULL );
    ...
    for ( u32 i = 0; i < m_nearest.size(); i++ ) {
        ...
        obj->m_pPhysicsShell->applyImpulse(
            dir, m_gravi.impulse_to_objects * obj->m_pPhysicsShell->getMass() );
    }
...
void CBurer::StartGraviPrepare() {
    ...
    pA->CParticlesPlayer::StartParticles(
        particle_gravi_prepare, Fvector().set( 0.0f, 0.1f, 0.0f ), pA->ID() );
}
...
void CBurer::StopGraviPrepare() {
    ...
    if ( !pA )
        return;
    pA->CParticlesPlayer::StopParticles( particle_gravi_prepare, BI_NONE,
                                         true );
}
...
void CBurer::StartTeleObjectParticle( CGameObject* pO ) {
    ...
    if ( !PP )
        return;
    PP->StartParticles( particle_tele_object, Fvector().set( 0.0f, 0.1f, 0.0f ),
                        pO->ID() );
}
...
void CBurer::Hit( SHit* pHDS ) {
    if ( m_shield_active && pHDS->hit_type == ALife::eHitTypeFireWound &&
         Device.dwFrame != last_hit_frame ) {
        // вычислить позицию и направленность партикла
        Fmatrix pos;
        // CParticlesPlayer::MakeXFORM(this,element,Fvector().set(0.f,0.f,1.f),p_in_object_space,pos);
        CParticlesPlayer::MakeXFORM( this, pHDS->bone(), pHDS->dir,
                                     pHDS->p_in_bone_space, pos );

        // установить particles
        intrusive_ptr< CParticlesObject > ps =
            Particles::Details::Create( particle_fire_shield, TRUE );
...
void CBurer::face_enemy() {
    ...
    bool const good_aiming =
        angle_between_vectors( self2enemy, Direction() ) < deg2rad( 20.f );
