...
CStateManagerBurer::CStateManagerBurer( CBurer* monster )
    : inherited( monster ) {
    ...
    add_state(
        ...
        xr_new< CStateMonsterHearInterestingSound< CBurer > >( monster ) );
    add_state( eStateHearDangerousSound,
               xr_new< CStateMonsterHearDangerousSound< CBurer > >( monster ) );
    add_state( eStateHitted,
               xr_new< CStateMonsterHitted< CBurer > >( monster ) );
    add_state( eStateBurerScanning,
               xr_new< CStateMonsterCustomAction< CBurer > >( monster ) );
}
    ...
    if ( object->EnemyMan.get_enemy() ) {
        switch ( object->EnemyMan.get_danger_type() ) {
            ...
            case eWeak:
                ...
                ( object->HitMemory.get_last_hit_time() + 10000 >
                  Device.dwTimeGlobal ) )
        state = eStateHitted;
...
void CStateManagerBurer::setup_substates() {
    if ( current_substate == eStateBurerScanning ) {
        ...
        get_state_current()->fill_data_with( &data,
                                             sizeof( SStateDataAction ) );
        return;
    }
}
