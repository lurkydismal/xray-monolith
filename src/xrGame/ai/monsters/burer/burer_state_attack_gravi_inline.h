...
bool CStateBurerAttackGravi< Object >::check_start_conditions() {
    ...
    if ( !this->object->control().direction().is_face_target(
             this->object->EnemyMan.get_enemy(), deg( 45 ) ) )
        return false;
...
void CStateBurerAttackGravi< Object >::ExecuteGraviStart() {
    float const time =
        this->object->anim().get_animation_length( eAnimGraviFire, 0 );
...
void CStateBurerAttackGravi< Object >::ExecuteGraviContinue() {
    ...
    float time_to_hold = ( abs( dist - this->object->m_gravi.min_dist ) /
                           this->object->m_gravi.min_dist );
...
void CStateBurerAttackGravi< Object >::ExecuteGraviFire() {
    ...
    this->object->m_gravi_object.activate( this->object->EnemyMan.get_enemy(),
                                           from_pos, target_pos );
    ...
    g_pGamePersistent->GrassBendersAddExplosion(
        this->object->ID(), from_pos, this->object->Direction(), 1.33f, 3.0f,
        ps_ssfx_grass_interactive.w, 13.0f );
}
