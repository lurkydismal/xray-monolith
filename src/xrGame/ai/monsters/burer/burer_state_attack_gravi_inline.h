...
bool CStateBurerAttackGravi< Object >::check_start_conditions() {
    ...
    if ( !object->control().direction().is_face_target(
             object->EnemyMan.get_enemy(), deg( 45 ) ) )
        return false;
...
void CStateBurerAttackGravi< Object >::ExecuteGraviContinue() {
    ...
    float time_to_hold =
        ( abs( dist - object->m_gravi.min_dist ) / object->m_gravi.min_dist );
...
void CStateBurerAttackGravi< Object >::ExecuteGraviFire() {
    ...
    object->m_gravi_object.activate( object->EnemyMan.get_enemy(), from_pos,
                                     target_pos );
    ...
    g_pGamePersistent->GrassBendersAddExplosion(
        object->ID(), from_pos, object->Direction(), 1.33f, 3.0f,
