...
void CControlJump::load( LPCSTR section ) {
    ...
    m_trace_ground_range =
        pSettings->r_float( section, "jump_ground_trace_range" );
    m_hit_trace_range = pSettings->r_float( section, "jump_hit_trace_range" );
    m_build_line_distance =
        pSettings->r_float( section, "jump_build_line_distance" );
    ...
    if ( pSettings->line_exist( section, "jump_auto_aim_factor" ) )
        m_auto_aim_factor =
            pSettings->r_float( section, "jump_auto_aim_factor" );
}
...
void CControlJump::on_release() {
    ...
    SControlDirectionData* ctrl_data_dir =
        ( SControlDirectionData* )m_man->data( this, ControlCom::eControlDir );
    ...
    if ( m_data.target_object &&
         m_data.flags.test( SControlJumpData::eUseAutoAim ) ) {
        m_object->character_physics_support()
            ->movement()
            ->PHCharacter()
            ->SetAirControlFactor( 0.f );
    }
...
void CControlJump::start_jump( const Fvector& point ) {
    ...
    if ( is_flag( SControlJumpData::ePrepareSkip ) ) {
        ...
        if ( is_flag( SControlJumpData::ePrepareInMove ) ) {
            // get animation time
            float time = m_man->animation().motion_time(
                m_data.state_prepare_in_move.motion, m_object->Visual() );
            // set acceleration and velocity
            SVelocityParam& vel = m_object->move().get_velocity(
            ...
            target_point.mad( m_object->Position(), m_object->Direction(),
                              dist );
            if ( m_man->path_builder().accessible( target_point ) ) {
                // нода в прямой видимости?
                m_man->path_builder().restrictions().add_border(
                ...
                if ( ai().level_graph().valid_vertex_id( node ) &&
                     m_man->path_builder().accessible( node ) )
                    prepared = true;
            }
            ...
            if ( prepared ) {
                if ( m_man->build_path_line(
                    ...
                    SControlPathBuilderData* ctrl_path =
                        ( SControlPathBuilderData* )m_man->data(
        ...
        if ( !prepared ) {
            VERIFY( m_data.state_prepare.motion.valid() ||
                    is_flag( SControlJumpData::eGlideOnPrepareFailed ) );
...
void CControlJump::select_next_anim_state() {
    ...
    if ( ( m_anim_state_current == eStateGlide ) &&
         ( m_anim_state_prev == eStateGlide ) )
        if ( is_flag( SControlJumpData::eGlidePlayAnimOnce ) )
            return;

    //---------------------------------------------------------------------------------------------------
    // start new animation
    SControlAnimationData* ctrl_data = ( SControlAnimationData* )m_man->data(
    ...
    switch ( m_anim_state_current ) {
        case eStatePrepare:
            ctrl_data->global.set_motion( m_data.state_prepare.motion );
            break;
        case eStatePrepareInMove:
            ctrl_data->global.set_motion( m_data.state_prepare_in_move.motion );
            break;
        case eStateGlide:
            ctrl_data->global.set_motion( m_data.state_glide.motion );
            break;
        case eStateGround:
            ctrl_data->global.set_motion( m_data.state_ground.motion );
            break;
        default:
            NODEFAULT;
    }
...
float CControlJump::relative_time() {
    float time =
        ( ( Device.dwTimeGlobal - m_time_started ) / 1000.f ) / m_jump_time;
    if ( time > 1.f )
        time = 1.f;
...
void CControlJump::update_frame() {
    ...
    if ( m_anim_state_current == eStateGlide && in_auto_aim() ) {
        ...
        SControlDirectionData* ctrl_data_dir =
            ( SControlDirectionData* )m_man->data( this,
                                                   ControlCom::eControlDir );
        VERIFY( ctrl_data_dir );

        ctrl_data_dir->heading.target_angle =
            m_man->direction().angle_to_target(
                m_data.target_object->Position() );

        float cur_yaw, target_yaw;
        m_man->direction().get_heading( cur_yaw, target_yaw );
        ctrl_data_dir->heading.target_speed =
            angle_difference( cur_yaw, target_yaw ) / m_jump_time;
        ctrl_data_dir->linear_dependency = false;

        // 		ctrl_data->set_speed
        // (m_man->animation().current_blend()->timeTotal /
    ...
    if ( m_man->path_builder().is_moving_on_path() ) {
        ...
        SControlMovementData* ctrl_move = ( SControlMovementData* )m_man->data(
            this, ControlCom::eControlMovement );
        VERIFY( ctrl_move );
...
bool CControlJump::is_on_the_ground() {
    ...
    if ( Level().ObjectSpace.RayPick( trace_from, direction,
                                      m_trace_ground_range, collide::rqtStatic,
                                      l_rq, m_object ) ) {
...
void CControlJump::grounding() {
    if ( ( m_data.state_ground.velocity_mask == u32( -1 ) ) ||
         is_flag( SControlJumpData::eGroundSkip ) ||
    ...
    target_position.mad( m_object->Position(), m_object->Direction(),
                         m_build_line_distance );

    if ( !m_man->build_path_line(
             this, target_position, u32( -1 ),
             m_data.state_ground.velocity_mask |
                 MonsterMovement::eVelocityParameterStand ) ) {
        stop();
    } else {
        SControlPathBuilderData* ctrl_path =
            ( SControlPathBuilderData* )m_man->data( this,
                                                     ControlCom::eControlPath );
        VERIFY( ctrl_path );
        ctrl_path->enable = true;
        m_man->lock( this, ControlCom::eControlPath );
...
Fvector CControlJump::get_target( CObject* obj ) {
    ...
    CBoneInstance& bone = smart_cast< IKinematics* >( obj->Visual() )
                              ->LL_GetBoneInstance( bone_id );
...
void CControlJump::calculate_jump_time( Fvector const& target,
                                        bool const check_force_factor ) {
    float ph_time =
        m_object->character_physics_support()->movement()->JumpMinVelTime(
            target );
...
void CControlJump::on_event( ControlCom::EEventType type,
                             ControlCom::IEventData* data ) {
    if ( type == ControlCom::eventVelocityBounce ) {
        SEventVelocityBounce* event_data = ( SEventVelocityBounce* )data;
        // !TEMP!
        if ( ( event_data->m_ratio < 0 ) && !m_velocity_bounced &&
             ( m_jump_time != 0 ) ) {
            if ( is_on_the_ground() ) {
        ...
        SControlAnimationData* ctrl_data =
            ( SControlAnimationData* )m_man->data(
                this, ControlCom::eControlAnimation );
        VERIFY( ctrl_data );
        ...
        if ( ( m_anim_state_current == eStateGlide ) &&
            ...
            m_object->character_physics_support()->movement()->Jump(
                m_target_position, m_jump_time );
            m_time_started = time();
            ...
            // set angular speed in exclusive force mode
            SControlDirectionData* ctrl_data_dir =
                ( SControlDirectionData* )m_man->data(
                    this, ControlCom::eControlDir );
            VERIFY( ctrl_data_dir );

            if ( !m_data.flags.test( SControlJumpData::eUseAutoAim ) ||
                 !m_data.target_object ) {
                ctrl_data_dir->heading.target_angle =
                    m_man->direction().angle_to_target( m_target_position );
            }
            ...
            ctrl_data->set_speed( ( current_blend ? current_blend->timeTotal /
                                                        current_blend->speed
                                                  : 1.0f ) /
                                  m_jump_time );
        } else
...
void CControlJump::hit_test() {
    ...
    if ( Level().ObjectSpace.RayPick( trace_from, m_object->Direction(),
                                      m_hit_trace_range, collide::rqtObject,
                                      l_rq, m_object ) ) {
        if ( ( l_rq.O == m_data.target_object ) &&
             ( l_rq.range < m_hit_trace_range ) ) {
    ...
    if ( m_object_hitted )
        m_object->HitEntityInJump(
            smart_cast< CEntity* >( m_data.target_object ) );
}
...
bool CControlJump::jump_intersect_geometry( Fvector const& target,
    ...
    Fvector const traj_start =
        m_object->Position() + Fvector().set( 0, 1.2f, 0 );
    Fvector const traj_target = target + Fvector().set( 0, 1.2f, 0 ) -
                                ( normalize( start_to_target ) * 1 );

    if ( trajectory_intersects_geometry(
             m_jump_time, traj_start, traj_target, velocity, collide_position,
             m_object, ignored_object, temp_rq_results, pass_jump_picks,
...
bool CControlJump::can_jump( Fvector const& target,
                             bool const aggressive_jump ) {
    if ( m_time_next_allowed != 0 ) {
        // in aggressive mode we can jump after 1/3 of m_delay_after_jump
        if ( m_time_next_allowed -
                 ( int )aggressive_jump * ( 2 * m_delay_after_jump / 3 ) >
             Device.dwTimeGlobal ) {
            return false;
        }
    }
    ...
    const float test_min_distance =
        aggressive_jump ? _min( 1.f, m_min_distance ) : m_min_distance;
    ...
    if ( !is_flag( SControlJumpData::ePrepareSkip ) &&
        ...
        if ( !is_flag( SControlJumpData::ePrepareInMove ) ) {
            ...
            if ( !good_trace_res ) {
                // cannot prepare in move, so check if can prepare in stand
                // state
                if ( !m_data.state_prepare.motion.valid() )
                    return false;
            }
