...
void CControlPathBuilderBase::reinit() {
    ...
    if ( !m_cover_approach )
        m_cover_approach = xr_new< CCoverEvaluatorCloseToEnemy >(
            &m_man->path_builder().restrictions() );
...
void CControlPathBuilderBase::on_event( ControlCom::EEventType type,
...
void CControlPathBuilderBase::on_start_control(
...
void CControlPathBuilderBase::set_target_accessible( STarget& target,
...
void CControlPathBuilderBase::travel_point_changed() {
    if ( m_man->path_builder().detail().curr_travel_point_index() >=
         m_man->path_builder().detail().path().size() - 1 ) {
        on_path_end();
    }
}
...
bool CControlPathBuilderBase::global_failed() {
    return ( m_time_global_failed_started + pmt_global_failed_duration >
             time() );
}
