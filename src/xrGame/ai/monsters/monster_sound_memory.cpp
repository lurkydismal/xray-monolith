...
TSoundDangerValue tagSoundElement::ConvertSoundType( ESoundTypes stype ) {
    ...
    CHECK_SOUND_TYPE( stype, SOUND_TYPE_WEAPON_EMPTY_CLICKING,
                      WEAPON_EMPTY_CLICKING );
    CHECK_SOUND_TYPE( stype, SOUND_TYPE_WEAPON_BULLET_HIT,
    ...
    CHECK_SOUND_TYPE( stype, SOUND_TYPE_WORLD_OBJECT_BREAKING,
                      OBJECT_BREAKING );
    CHECK_SOUND_TYPE( stype, SOUND_TYPE_WORLD_OBJECT_COLLIDING,
                      OBJECT_FALLING );
...
void CMonsterSoundMemory::HearSound( const CObject* who,
                                     int eType,
...
SoundElem& CMonsterSoundMemory::GetSound() {
    ...
    xr_vector< SoundElem >::iterator it =
        std::max_element( Sounds.begin(), Sounds.end() );
    return ( *it );
}
...
void CMonsterSoundMemory::UpdateHearing() {
    ...
    // пересчитать value
    for ( xr_vector< SoundElem >::iterator I = Sounds.begin();
          I != Sounds.end(); ++I )
...
bool CMonsterSoundMemory::get_sound_from_object( const CObject* obj,
                                                 SoundElem& value ) {
    for ( u32 i = 0; i < Sounds.size(); i++ )
        if ( Sounds[ i ].who == obj ) {
            value = Sounds[ i ];
...
bool CMonsterSoundMemory::hear_help_sound() {
    if ( ( m_time_help_sound == 0 ) ||
         ( m_time_help_sound + time_help_sound_remember < time() ) )
        return false;
    return true;
}

void CMonsterSoundMemory::check_help_sound( int eType, u32 node ) {
    if ( ( eType & SOUND_TYPE_MONSTER_ATTACKING ) !=
         SOUND_TYPE_MONSTER_ATTACKING )
        return;
    if ( ( eType & SOUND_TYPE_MONSTER_INJURING ) !=
