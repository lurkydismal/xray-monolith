...
enum EPState { PS_STAND, PS_SIT, PS_LIE, PS_STAND_UPPER };
...
struct SCurrentAnimationInfo {
    ...
    struct {
        IC void _set_current( float v ) {
            current = v;
            VERIFY2( _abs( v ) < 1000,
                     "_set_current(). monster speed is too big" );
        }

        IC void _set_target( float v ) {
            target = v;
            VERIFY2( _abs( v ) < 1000,
                     "_set_target(). monster speed is too big" );
        }
...
enum EAccelType { eAT_Calm, eAT_Aggressive };

enum EAccelValue { eAV_Accel, eAV_Braking };
