...
void check_path( const CBaseMonster* monster, const CPatrolPath* path ) {
    VERIFY2(
        ai().game_graph()
                ...
                ->level_id() == ai().level_graph().level_id(),
        make_string(
            "invalid patrol path [%s] as home specified for monster "
            "[%s]\nmonster is on level %s\npatrol path is on level %s",
            *path->m_name, *monster->cName(),
            *ai().game_graph()
                 .header()
                 .level( ai().game_graph()
                             .vertex( monster->ai_location().game_vertex_id() )
                             ->level_id() )
                 .name(),
            *ai().game_graph()
                 .header()
...
void CMonsterHome::load( LPCSTR line ) {
    ...
    if ( m_object->spawn_ini() &&
         m_object->spawn_ini()->section_exist( line ) ) {
        m_path = ai().patrol_paths().safe_path(
            m_object->spawn_ini()->r_string( line, "path" ), false, true );
        ...
        VERIFY3( m_radius_max > m_radius_min,
                 "Error: Wrong home point radius specified for monster ",
                 *m_object->cName() );

        if ( m_object->spawn_ini()->line_exist( line, "radius_middle" ) ) {
            m_radius_middle =
                m_object->spawn_ini()->r_float( line, "radius_middle" );
            if ( m_radius_middle > m_radius_max ||
                 m_radius_middle < m_radius_min ) {
                m_radius_middle =
        ...
        if ( m_object->spawn_ini()->line_exist( line, "min_move_dist" ) ) {
            min_move_dist =
                m_object->spawn_ini()->r_u32( line, "min_move_dist" );
            ;
        }
        if ( m_object->spawn_ini()->line_exist( line, "max_move_dist" ) ) {
            max_move_dist =
                m_object->spawn_ini()->r_u32( line, "max_move_dist" );
            ;
        }
...
void CMonsterHome::setup( LPCSTR path_name,
                          float min_radius,
                          float max_radius,
                          bool aggressive,
                          float middle_radius ) {
...
void CMonsterHome::setup( u32 lv_ID,
                          float min_radius,
                          float max_radius,
                          bool aggressive,
                          float middle_radius ) {
...
u32 CMonsterHome::get_place_in_min_home() {
    ...
    if ( !m_path ) {
        ...
        const CPatrolPath::CVertex* vertex =
            m_path->vertex( Random.randI( m_path->vertex_count() ) );
        input_node = vertex->data().level_vertex_id();
    }
    m_object->control().path_builder().get_node_in_radius(
        input_node, 1, m_radius_min, 5, result );

    if ( result == u32( -1 ) ) {
        // TODO: find more acceptable decision, than return its level_vertex_id,
        // if !accessible
...
u32 CMonsterHome::get_place_in_mid_home() {
    ...
    int m_move_dist =
        min_move_dist + Random.randI( max_move_dist - min_move_dist );
    float m_res_dist = 0.f + m_move_dist;
    if ( !at_mid_home( m_object->Position() ) ||
         at_min_home( m_object->Position() ) ) {
        if ( !m_path ) {
            if ( !ai().level_graph().valid_vertex_id( m_level_vertex_id ) )
                input_node = m_object->ai_location().level_vertex_id();
            else
        ...
        m_object->control().path_builder().get_node_in_radius(
            input_node, m_radius_min, m_radius_middle, 5, result );
    } else {
        int i = 0;
        ...
        do {
            ...
            !ai().level_graph().valid_vertex_position( m_dest_direction ) &&
            i <= 10 );
        if ( ai().level_graph().valid_vertex_position( m_dest_direction ) ) {
            ...
            input_node = m_object->ai_location().level_vertex_id();
            m_object->control().path_builder().get_node_in_radius(
                input_node, m_res_dist - 1, m_res_dist, 5, result );
        }
    }
    ...
    if ( !ai().level_graph().valid_vertex_id( result ) ||
         !at_mid_home( ai().level_graph().vertex_position( result ) ) )
...
u32 CMonsterHome::get_place_in_max_home_to_direction( Fvector to_direction ) {
    ...
    float m_res_dist = m_radius_middle +
                       ...
                       ( m_radius_max - m_radius_middle ) / 10;
    float m_heading, m_pitch;
    do {
        i++;
    ...
    } while ( !ai().level_graph().valid_vertex_position( m_dest_direction ) &&
              i <= 10 );
    ...
    if ( result == u32( -1 ) ) {
        if ( ai().level_graph().valid_vertex_id( input_node ) &&
            ...
            m_res_dist = m_radius_min + ( m_radius_middle - m_radius_min ) / 2 -
                         ( m_radius_middle - m_radius_min ) / 10;
            i = 0;
            input_node = u32( -1 );
            do {
                i++;
                ...
                m_dest_direction.setHP( m_heading, m_pitch );
                m_dest_direction.x =
                    m_home_point.x + m_res_dist * m_dest_direction.x;
                m_dest_direction.y =
                    m_home_point.y + m_res_dist * m_dest_direction.y;
                m_dest_direction.z =
                    m_home_point.z + m_res_dist * m_dest_direction.z;
            } while (
                !ai().level_graph().valid_vertex_position( m_dest_direction ) &&
                i <= 10 );
            ...
            if ( input_node != u32( -1 ) )
                m_object->control().path_builder().get_node_in_radius(
                    input_node, 1, ( m_radius_max - m_radius_middle ) / 2, 5,
                    result );
...
u32 CMonsterHome::get_place_in_max_home() {
    ...
    m_object->control().path_builder().get_node_in_radius(
        input_node, m_radius_min, m_radius_max, 5, result );
...
u32 CMonsterHome::get_place() {
    ...
    if ( !m_path ) {
        if ( ai().level_graph().valid_vertex_id( m_level_vertex_id ) )
            m_object->control().path_builder().get_node_in_radius(
                m_level_vertex_id, m_radius_min,
        ...
        if ( result == u32( -1 ) ) {
            ...
            if ( result == u32( -1 ) ) {
                m_object->control().path_builder().get_node_in_radius(
                    m_object->ai_location().level_vertex_id(), 2, 3, 10,
                    result );
                if ( result == u32( -1 ) )
        ...
        // get_random node
        m_object->control().path_builder().get_node_in_radius(
            vertex->data().level_vertex_id(), m_radius_min,
            m_radius_min + ( m_radius_max - m_radius_min ) / 2, 5, result );
        if ( result == u32( -1 ) ) {
            // TODO: find more acceptable decision, than return its
            // level_vertex_id, if !accessible
            if ( m_object->control().path_builder().accessible(
                     vertex->data().level_vertex_id() ) )
...
u32 CMonsterHome::get_place_in_cover() {
    if ( !m_path ) {
        ...
        // find cover
        const CCoverPoint* point = m_object->CoverMan->find_cover(
            vertex->data().position(), vertex->data().position(), m_radius_min,
            m_radius_min + ( m_radius_max - m_radius_min ) / 2 );
        if ( point )
            return point->level_vertex_id();
    }
...
bool CMonsterHome::at_home( const Fvector& pos, float const radius ) {
    if ( !m_path ) {
        ...
        for ( u32 i = 0; i < m_path->vertex_count(); i++ ) {
            ...
            float dist = pos.distance_to(
                ai().level_graph().vertex_position( level_vertex ) );
            if ( dist < radius )
                return true;
        }
    }
...
Fvector CMonsterHome::get_home_point() {
    ...
    if ( !m_path->vertices().empty() )
        return ( ai().level_graph().vertex_position(
            m_path->vertex( 0 )->data().level_vertex_id() ) );
