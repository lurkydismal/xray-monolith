...
const char* dbg_states[] = { "eStatePrepare", "eStateExecute", "eStateFinalize",
                             "eStateNone" };
...
void CAnimationTriple::on_release() {
    ...
    if ( ( m_data.capture_type & ControlCom::eCapturePath ) ==
         ControlCom::eCapturePath )
        m_man->release( this, ControlCom::eControlPath );

    if ( ( m_data.capture_type & ControlCom::eCaptureMovement ) ==
         ControlCom::eCaptureMovement )
        m_man->release( this, ControlCom::eControlMovement );

    if ( ( m_data.capture_type & ControlCom::eCaptureDir ) ==
         ControlCom::eCaptureDir )
...
void CAnimationTriple::on_event( ControlCom::EEventType,
                                 ControlCom::IEventData* ) {
    select_next_state();
}
...
void CAnimationTriple::select_next_state() {
    ...
    if ( ( m_current_state == eStateExecute ) && m_data.execute_once &&
