...
void CStateControlCampAbstract::execute() {
    ...
    Fvector point;
    point.mad( this->object->Position(), Fvector().setHP( m_target_angle, 0.f ),
               3.f );

    this->object->custom_dir().head_look_point( point );
    this->object->custom_anim().set_body_state(
        CControllerAnimation::eTorsoIdle,
        CControllerAnimation::eLegsTypeSteal );
}
...
bool CStateControlCampAbstract::check_completion() {
    if ( this->object->EnemyMan.see_enemy_now() )
        return true;
    if ( this->time_state_started + 2000 < time() )
        return true;
...
void CStateControlCampAbstract::update_target_angle() {
    if ( m_time_next_updated > time() )
        return;
    m_time_next_updated =
        time() + Random.randI( TIME_POINT_CHANGE_MIN, TIME_POINT_CHANGE_MAX );
