...
void CControllerPsyHit::load( LPCSTR section ) {
    m_min_tube_dist =
        pSettings->r_float( section, "tube_condition_min_distance" );
}
...
bool CControllerPsyHit::check_start_conditions() {
    ...
    if ( m_object->Position().distance_to( Actor()->Position() ) <
         m_min_tube_dist )
        return false;
...
void CControllerPsyHit::activate() {
    ...
    ctrl_dir->heading.target_angle =
        m_man->direction().angle_to_target( Actor()->Position() );
...
void CControllerPsyHit::on_event( ControlCom::EEventType type,
...
void CControllerPsyHit::play_anim() {
    SControlAnimationData* ctrl_anim = ( SControlAnimationData* )m_man->data(
...
bool check_actor_visibility( const Fvector trace_from,
    ...
    // (trace_from,trace_to,D3DCOLOR_XRGB(0,150,150));

    collide::rq_result l_rq;
    l_rq.O = NULL;
    Level().ObjectSpace.RayPick( trace_from, trace_dir, dist, collide::rqtBoth,
                                 l_rq, object );
...
bool CControllerPsyHit::check_conditions_final() {
    ...
    if ( m_object->Position().distance_to_xz( Actor()->Position() ) <
         m_min_tube_dist - 2 )
        return false;
...
void CControllerPsyHit::death_glide_start() {
    ...
    IKinematics* k =
        m_object->Visual() ? m_object->Visual()->dcast_PKinematics() : nullptr;
    u16 bone_id = BI_NONE;
    if ( k ) {
    ...
    Fmatrix m;
    Fvector target_pos =
        k ? m.mul_43( m_object->XFORM(), k->LL_GetTransform_safed( bone_id ) ).c
          : m_object->Position();
    ...
    // demonized: disable actor_psy_immunity affecting camera behaviour of
    // controller attack, very buggy Finetune target_pos a bit
    ...
    Actor()->Cameras().AddCamEffector( xr_new< CControllerPsyHitCamEffector >(
        eCEControllerPsyHit, src_pos, target_pos,
    ...
    Actor()->character_physics_support()->movement()->ApplyImpulse(
        dir, Actor()->GetMass() * 530.f );
...
void CControllerPsyHit::death_glide_end() {
    ...
    monster->m_sound_tube_hit_left.play_at_pos(
        Actor(), Fvector().set( -1.f, 0.f, 1.f ), sm_2D );
    monster->m_sound_tube_hit_right.play_at_pos(
        Actor(), Fvector().set( 1.f, 0.f, 1.f ), sm_2D );
...
void CControllerPsyHit::set_sound_state( ESoundState state ) {
    CController* monster = smart_cast< CController* >( m_object );
    if ( state == ePrepare ) {
        monster->m_sound_tube_prepare.play_at_pos(
            Actor(), Fvector().set( 0.f, 0.f, 0.f ), sm_2D );
    } else if ( state == eStart ) {
        if ( monster->m_sound_tube_prepare._feedback() )
            monster->m_sound_tube_prepare.stop();

        monster->m_sound_tube_start.play_at_pos(
            Actor(), Fvector().set( 0.f, 0.f, 0.f ), sm_2D );
        monster->m_sound_tube_pull.play_at_pos(
            Actor(), Fvector().set( 0.f, 0.f, 0.f ), sm_2D );
    } else if ( state == eHit ) {
        if ( monster->m_sound_tube_start._feedback() )
        ...
        // monster->m_sound_tube_hit_left.play_at_pos(Actor(),
        // Fvector().set(-1.f, 0.f, 1.f), sm_2D);
        // monster->m_sound_tube_hit_right.play_at_pos(Actor(),
        // Fvector().set(1.f, 0.f, 1.f), sm_2D);
    } else if ( state == eNone ) {
        if ( monster->m_sound_tube_start._feedback() )
            monster->m_sound_tube_start.stop();
