...
void CController::Load( LPCSTR section ) {
    ...
    //	m_UIControlFX.Init(pSettings->r_string(section, "control_fx_texture"),
    //"hud\\default",0,0,0); 	m_UIControlFX2.Init(pSettings->r_string(section,
    //"control_fx_texture2"), "hud\\default",0,0,0);

    m_max_controlled_number =
        pSettings->r_u8( section, "Max_Controlled_Count" );
    m_controlled_objects.reserve( m_max_controlled_number );

    anim().accel_load( section );
    // anim().accel_chain_add		(eAnimWalkFwd,		eAnimRun);
    // anim().accel_chain_add		(eAnimWalkDamaged,
    // eAnimRunDamaged);

    ::Sound->create( control_start_sound,
                     pSettings->r_string( section, "sound_control_start" ),
                     st_Effect, SOUND_TYPE_WORLD );
    ::Sound->create( control_hit_sound,
                     pSettings->r_string( section, "sound_control_hit" ),
                     st_Effect, SOUND_TYPE_WORLD );
    ...
    m_control_effector.ppi.duality.h =
        pSettings->r_float( ppi_section, "duality_h" );
    m_control_effector.ppi.duality.v =
        pSettings->r_float( ppi_section, "duality_v" );
    ...
    m_control_effector.ppi.noise.intensity =
        pSettings->r_float( ppi_section, "noise_intensity" );
    m_control_effector.ppi.noise.grain =
        pSettings->r_float( ppi_section, "noise_grain" );
    m_control_effector.ppi.noise.fps =
        pSettings->r_float( ppi_section, "noise_fps" );
    VERIFY( !fis_zero( m_control_effector.ppi.noise.fps ) );

    sscanf( pSettings->r_string( ppi_section, "color_base" ), "%f,%f,%f",
            &m_control_effector.ppi.color_base.r,
            &m_control_effector.ppi.color_base.g,
            &m_control_effector.ppi.color_base.b );
    sscanf( pSettings->r_string( ppi_section, "color_gray" ), "%f,%f,%f",
            &m_control_effector.ppi.color_gray.r,
            &m_control_effector.ppi.color_gray.g,
            &m_control_effector.ppi.color_gray.b );
    sscanf( pSettings->r_string( ppi_section, "color_add" ), "%f,%f,%f",
            &m_control_effector.ppi.color_add.r,
            &m_control_effector.ppi.color_add.g,
    ...
    m_control_effector.time_attack =
        pSettings->r_float( ppi_section, "time_attack" );
    m_control_effector.time_release =
        pSettings->r_float( ppi_section, "time_release" );

    m_control_effector.ce_time = pSettings->r_float( ppi_section, "ce_time" );
    m_control_effector.ce_amplitude =
        pSettings->r_float( ppi_section, "ce_amplitude" );
    m_control_effector.ce_period_number =
        pSettings->r_float( ppi_section, "ce_period_number" );
    m_control_effector.ce_power = pSettings->r_float( ppi_section, "ce_power" );

    SVelocityParam& velocity_none =
        move().get_velocity( MonsterMovement::eVelocityParameterIdle );
    SVelocityParam& velocity_turn =
    ...
    anim().AddAnim( eAnimStandTurnRight, "stand_turn_rs_", -1, &velocity_turn,
                    PS_STAND );
    anim().AddAnim( eAnimStandDamaged, "stand_idle_dmg_", -1, &velocity_none,
                    PS_STAND );
    ...
    anim().AddAnim( eAnimWalkFwd, "stand_walk_fwd_", -1, &velocity_walk,
                    PS_STAND );
    anim().AddAnim( eAnimWalkDamaged, "stand_walk_fwd_", -1, &velocity_walk,
                    PS_STAND );
    anim().AddAnim( eAnimRun, "stand_walk_fwd_", -1, &velocity_walk, PS_STAND );
    anim().AddAnim( eAnimRunDamaged, "stand_walk_fwd_", -1, &velocity_walk,
                    PS_STAND );
    anim().AddAnim( eAnimAttack, "stand_attack_", -1, &velocity_turn,
                    PS_STAND );
    anim().AddAnim( eAnimSteal, "stand_steal_", -1, &velocity_steal, PS_STAND );
    anim().AddAnim( eAnimCheckCorpse, "stand_check_corpse_", -1, &velocity_none,
                    PS_STAND );
    anim().AddAnim( eAnimDie, "stand_die_", -1, &velocity_none, PS_STAND );
    anim().AddAnim( eAnimStandSitDown, "stand_sit_down_", -1, &velocity_none,
                    PS_STAND );
    anim().AddAnim( eAnimSitStandUp, "sit_stand_up_", -1, &velocity_none,
                    PS_SIT );
    anim().AddAnim( eAnimSleep, "sit_sleep_", -1, &velocity_none, PS_SIT );

    // anim().AddAnim(eAnimStandIdle,		"new_torso_steal_",
    // -1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimStandTurnLeft,	"new_torso_steal_",
    // -1, &velocity_turn,		PS_STAND);
    // anim().AddAnim(eAnimStandTurnRight,	"new_torso_steal_",
    // -1, &velocity_turn,		PS_STAND);
    // anim().AddAnim(eAnimStandDamaged,	"new_torso_steal_",
    // -1, &velocity_none,		PS_STAND); anim().AddAnim(eAnimSitIdle,
    // "sit_idle_",			-1, &velocity_none,		PS_SIT);
    // anim().AddAnim(eAnimEat,			"sit_eat_",
    // -1, &velocity_none,		PS_SIT); anim().AddAnim(eAnimWalkFwd,
    // "new_torso_steal_",			-1, &velocity_steal,
    // PS_STAND); anim().AddAnim(eAnimWalkDamaged,	"new_torso_steal_",
    // -1, &velocity_steal,	PS_STAND); anim().AddAnim(eAnimRun,
    // "new_torso_steal_",			-1,	&velocity_steal,
    // PS_STAND); anim().AddAnim(eAnimRunDamaged,
    // "new_torso_steal_",			-1, &velocity_steal,
    // PS_STAND); anim().AddAnim(eAnimAttack,			"stand_attack_",
    // -1, &velocity_turn,		PS_STAND); anim().AddAnim(eAnimSteal,
    // "new_torso_steal_",			-1, &velocity_steal,
    // PS_STAND); anim().AddAnim(eAnimCheckCorpse,	"stand_check_corpse_",
    // -1,	&velocity_none,		PS_STAND); anim().AddAnim(eAnimDie,
    // "stand_die_",			-1, &velocity_none,
    // PS_STAND); anim().AddAnim(eAnimStandSitDown,	"stand_sit_down_",
    // -1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimSitStandUp,		"sit_stand_up_",
    // -1, &velocity_none,		PS_SIT); anim().AddAnim(eAnimSleep,
    // "sit_sleep_",			-1, &velocity_none,		PS_SIT);

    // anim().AddAnim(eAnimStandIdle,		"stand_idle_",
    // -1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimStandTurnLeft,	"stand_turn_ls_",
    // -1, &velocity_turn,		PS_STAND);
    // anim().AddAnim(eAnimStandTurnRight,	"stand_turn_rs_",
    // -1, &velocity_turn,		PS_STAND);
    // anim().AddAnim(eAnimStandDamaged,	"stand_idle_dmg_",
    // -1, &velocity_none,		PS_STAND); anim().AddAnim(eAnimSitIdle,
    // "sit_idle_",			-1, &velocity_none,		PS_SIT);
    // anim().AddAnim(eAnimEat,			"sit_eat_",
    // -1, &velocity_none,		PS_SIT); anim().AddAnim(eAnimWalkFwd,
    // "stand_walk_fwd_",		-1, &velocity_walk,
    // PS_STAND); anim().AddAnim(eAnimWalkDamaged,	"stand_walk_dmg_",
    // -1, &velocity_walk_dmg,	PS_STAND); anim().AddAnim(eAnimRun,
    // "run_scared_",			-1,	&velocity_run,
    // PS_STAND); anim().AddAnim(eAnimRunDamaged, "stand_run_dmg_",
    // -1, &velocity_run_dmg,	PS_STAND); anim().AddAnim(eAnimAttack,
    // "stand_attack_",		-1, &velocity_turn,		PS_STAND);
    // anim().AddAnim(eAnimSteal,			"stand_steal_",
    // -1, &velocity_steal,	PS_STAND); anim().AddAnim(eAnimCheckCorpse,
    // "stand_check_corpse_",	-1,	&velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimDie,			"stand_die_",
    // -1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimStandSitDown,	"stand_sit_down_",
    // -1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimSitStandUp,		"sit_stand_up_",
    // -1, &velocity_none,		PS_SIT); anim().AddAnim(eAnimSleep,
    // "sit_sleep_",			-1, &velocity_none,		PS_SIT);
    ...
    m_sound_aura_left_channel.create(
        "monsters\\controller\\controller_psy_aura_l", st_Effect,
        sg_SourceType );
    m_sound_aura_right_channel.create(
        "monsters\\controller\\controller_psy_aura_r", st_Effect,
        sg_SourceType );
    m_sound_aura_hit_left_channel.create(
        "monsters\\controller\\controller_psy_hit_l", st_Effect,
        sg_SourceType );
    m_sound_aura_hit_right_channel.create(
        "monsters\\controller\\controller_psy_hit_l", st_Effect,
        sg_SourceType );

    m_sound_tube_start.create( "monsters\\controller\\controller_first_hit",
                               st_Effect, sg_SourceType );
    m_sound_tube_pull.create( "monsters\\controller\\controller_whoosh",
                              st_Effect, sg_SourceType );
    m_sound_tube_hit_left.create(
        "monsters\\controller\\controller_final_hit_l", st_Effect,
        sg_SourceType );
    m_sound_tube_hit_right.create(
        "monsters\\controller\\controller_final_hit_r", st_Effect,
        sg_SourceType );

    m_sound_tube_prepare.create(
        "monsters\\controller\\controller_tube_prepare", st_Effect,
        sg_SourceType );
...
bool CController::is_community_friend_overrides(
    const CEntityAlive* entity_alive ) const {
    const CInventoryOwner* IO =
        smart_cast< const CInventoryOwner* >( entity_alive );
    if ( !IO )
        return false;
    if ( const_cast< CEntityAlive* >( entity_alive )->cast_base_monster() )
        return false;

    return ( std::find( m_friend_community_overrides.begin(),
...
void CController::UpdateControlled() {
    ...
    if ( EnemyMan.get_enemy() ) {
        ...
        if ( entity ) {
            if ( !entity->is_under_control() &&
                 ( m_controlled_objects.size() < m_max_controlled_number ) ) {
                // взять под контроль
                entity->set_under_control( this );
                entity->set_task_follow( this );
                m_controlled_objects.push_back(
                    const_cast< CEntityAlive* >( EnemyMan.get_enemy() ) );
            }
        }
    }
...
void CController::set_controlled_task( u32 task ) {
    ...
    for ( u32 i = 0; i < m_controlled_objects.size(); i++ ) {
        CControlledEntityBase* entity =
            smart_cast< CControlledEntityBase* >( m_controlled_objects[ i ] );
...
void CController::InitThink() {
    for ( u32 i = 0; i < m_controlled_objects.size(); i++ ) {
        CBaseMonster* base =
            smart_cast< CBaseMonster* >( m_controlled_objects[ i ] );
        if ( !base )
            continue;
...
void CController::play_control_sound_start() {
    ...
    control_start_sound.play_at_pos(
        const_cast< CEntityAlive* >( EnemyMan.get_enemy() ), pos );
}
...
void CController::play_control_sound_hit() {
    ...
    control_hit_sound.play_at_pos(
        const_cast< CEntityAlive* >( EnemyMan.get_enemy() ), pos );
}

void CController::reload( LPCSTR section ) {
    inherited::reload( section );
    com_man().ta_fill_data( anim_triple_control, "stand_sit_down_attack_0",
                            "control_attack_0", "sit_stand_up_attack_0", true,
                            false );
}
...
void CController::control_hit() {
    ...
    // start postprocess
    CActor* pA = const_cast< CActor* >(
        smart_cast< const CActor* >( EnemyMan.get_enemy() ) );
    if ( !pA )
        return;

    Actor()->Cameras().AddCamEffector( xr_new< CMonsterEffectorHit >(
        m_control_effector.ce_time, m_control_effector.ce_amplitude,
        m_control_effector.ce_period_number, m_control_effector.ce_power ) );
    Actor()->Cameras().AddPPEffector( xr_new< CMonsterEffector >(
        m_control_effector.ppi, m_control_effector.time,
...
void CController::UpdateCL() {
    ...
    if ( active_control_fx ) {
        ...
        float percent =
            float( ( Device.dwTimeGlobal - time_control_hit_started ) ) /
            float( time_to_show );
        ...
        if ( percent < TEXTURE_SIZE_PERCENT ) {
            ...
            StaticDrawableWrapper* s =
                CurrentGameUI()->AddCustomStatic( "controller_fx", true );

            float x1 =
                Device.dwWidth / 2 - ( ( Device.dwWidth / 2 ) * percent );
            float y1 =
                Device.dwHeight / 2 - ( ( Device.dwHeight / 2 ) * percent );
            float x2 =
                Device.dwWidth / 2 + ( ( Device.dwWidth / 2 ) * percent );
            float y2 =
                Device.dwHeight / 2 + ( ( Device.dwHeight / 2 ) * percent );
            ...
            StaticDrawableWrapper* s =
                CurrentGameUI()->AddCustomStatic( "controller_fx2", true );

            float x1 =
                Device.dwWidth / 2 - ( ( Device.dwWidth / 2 ) * percent2 );
            float y1 =
                Device.dwHeight / 2 - ( ( Device.dwHeight / 2 ) * percent2 );
            float x2 =
                Device.dwWidth / 2 + ( ( Device.dwWidth / 2 ) * percent2 );
            float y2 =
                Device.dwHeight / 2 + ( ( Device.dwHeight / 2 ) * percent2 );
...
bool CController::can_psy_fire() {
    ...
    float dir_yaw =
        Fvector().sub( EnemyMan.get_enemy()->Position(), Position() ).getH();
...
bool CController::can_tube_fire() {
    ...
    if ( 0 && m_tube_at_once ) {
        if ( EnemyMan.get_enemy() && EnemyMan.see_enemy_now() &&
             m_psy_hit->check_start_conditions() ) {
            return true;
        }
    ...
    if ( EnemyMan.get_enemy()->Position().distance_to( Position() ) <
         m_tube_condition_min_distance )
        return false;
...
void CController::TranslateActionToPathParams() {
    ...
    u32 vel_mask = ( m_bDamaged ? MonsterMovement::eVelocityParamsWalkDamaged
                                : MonsterMovement::eVelocityParamsWalk );
    u32 des_mask =
        ( m_bDamaged ? MonsterMovement::eVelocityParameterWalkDamaged
...
void CController::HitEntity( const CEntity* pEntity,
                             float fDamage,
                             float impulse,
                             Fvector& dir,
    ...
    inherited::HitEntity( pEntity, fDamage, impulse, dir, hit_type,
                          draw_hit_marks );
}
...
CBaseMonster::SDebugInfo CController::show_debug_info() {
    ...
    for ( u32 i = 0; i < m_controlled_objects.size(); i++ ) {
        ...
        DBG().level_info( this ).add_item( my_pos, new_pos,
                                           D3DCOLOR_XRGB( 0, 255, 255 ) );
        DBG().level_info( this ).add_item( enemy_pos, new_pos,
                                           D3DCOLOR_XRGB( 0, 255, 255 ) );
    }
...
void CController::debug_on_key( int key ) {
    switch ( key ) {
        case DIK_MINUS:
            // m_sound_aura_left_channel.play_at_pos(Level().CurrentEntity(),
            // Fvector().set(-1.f, 0.f, 1.f), sm_2D);
            // m_sound_aura_right_channel.play_at_pos(Level().CurrentEntity(),
            // Fvector().set(1.f, 0.f, 1.f), sm_2D);
        ...
        case DIK_EQUALS:
            ...
            if ( !fsimilar( P1.square_magnitude(), 0.f ) &&
                 !fsimilar( P2.square_magnitude(), 0.f ) ) {
                const CCoverPoint* cover =
                    CoverMan->find_cover( P1, P2, 10.f, 40.f );
                if ( cover ) {
                    DBG().level_info( this ).remove_item( 3 );
                    DBG().level_info( this ).add_item( cover->position(), 0.8f,
                                                       COLOR_RED, 3 );
                }
            }

            // m_sound_aura_hit_left_channel.play_at_pos(Level().CurrentEntity(),
            // Fvector().set(-1.f, 0.f, 1.f), sm_2D);
            // m_sound_aura_hit_right_channel.play_at_pos(Level().CurrentEntity(),
            // Fvector().set(1.f, 0.f, 1.f), sm_2D);
            break;
    }
}
