...
CStateControllerAttackAbstract::CStateControllerAttack( _Object* obj )
    : inherited( obj ) {
    this->add_state(
        eStateAttack_MoveToHomePoint,
        xr_new< CStateMonsterAttackMoveToHomePoint< CController > >( obj ) );
    ...
    if ( !this->object->enemy_accessible() && state_id == eStateAttack_Run ) {
        ...
        if ( _abs( angle ) > deg2rad( 30.f ) ) {
            bool const rotate_right =
                this->object->control().direction().is_from_right(
                    enemy->Position() );
