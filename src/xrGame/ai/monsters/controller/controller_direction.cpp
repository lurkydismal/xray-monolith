...
void CControllerDirection::bone_callback( CBoneInstance* B ) {
    CControllerDirection* this_class =
        static_cast< CControllerDirection* >( B->callback_param() );
...
void CControllerDirection::assign_bones() {
    ...
    IKinematics* kinematics =
        smart_cast< IKinematics* >( m_controller->Visual() );

    m_bone_spine = &kinematics->LL_GetBoneInstance(
        kinematics->LL_BoneID( "bip01_spine" ) );
    m_bone_head = &kinematics->LL_GetBoneInstance(
        kinematics->LL_BoneID( "bip01_head" ) );

    if ( !m_controller->PPhysicsShell() ) {
        // нельзя ставить колбеки, если создан физ шел - у него стоят свои
        // колбеки!!!
...
void CControllerDirection::head_look_point( const Fvector& look_point ) {
    ...
    Fvector()
        .sub( look_point, get_head_position( m_controller ) )
        .getHP( dir_yaw, dir_pitch );
    ...
    float dy = _abs( angle_normalize_signed(
        dir_yaw - cur_yaw ) ); // дельта, на которую нужно поворачиваться

    bone_angle_head = _pmt_head_bone_limit /
                      ( _pmt_head_bone_limit + _pmt_torso_bone_limit ) * dy;
    bone_angle_torso = _pmt_torso_bone_limit /
                       ( _pmt_head_bone_limit + _pmt_torso_bone_limit ) * dy;
    ...
    if ( fis_zero( target_dy ) )
        ...
        bone_speed =
            _pmt_min_speed +
            _pmt_rotation_speed *
                ( _abs( ( x_spine.cur_yaw + x_head.cur_yaw ) -
                        ( bone_angle_head + bone_angle_torso ) ) /
                  ( 2 * ( _pmt_head_bone_limit + _pmt_torso_bone_limit ) ) );
    // set motion
    m_bones.SetMotion( m_bone_spine, AXIS_X, bone_angle_torso, bone_speed,
                       1000 );
    ...
    // clamp				(bone_angle_head,
    // -_pmt_head_bone_limit,	_pmt_head_bone_limit); clamp
    // (bone_angle_torso,	-_pmt_torso_bone_limit, _pmt_torso_bone_limit);
    //
    // m_bones.SetMotion(m_bone_spine, AXIS_Y, bone_angle_torso,
    // _pmt_rotation_speed, 1000); m_bones.SetMotion(m_bone_head,	AXIS_Y,
    // bone_angle_head,	_pmt_rotation_speed, 1000);
}
