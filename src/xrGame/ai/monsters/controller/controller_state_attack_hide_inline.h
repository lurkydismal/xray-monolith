...
void CStateControllerHideAbstract::execute() {
    ...
    if ( object->HitMemory.get_last_hit_time() >
         object->EnemyMan.get_enemy_time_last_seen() ) {
        Fvector pos;
        pos.mad( object->Position(), object->HitMemory.get_last_hit_dir(),
                 5.f );
        pos.y += 1.5f;
        object->custom_dir().head_look_point( pos );
    } else
        object->custom_dir().head_look_point(
            object->EnemyMan.get_enemy_position() );

    object->custom_anim().set_body_state( CControllerAnimation::eTorsoRun,
                                          CControllerAnimation::eLegsTypeRun );
}
...
void CStateControllerHideAbstract::select_target_point() {
    ...
    m_state_fast_run =
        ( target.position.distance_to( object->Position() ) > 20.f );
