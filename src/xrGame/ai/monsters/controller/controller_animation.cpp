...
void CControllerAnimation::reinit() {
    ...
    m_man->animation().add_anim_event( m_torso[ eTorsoPsyAttack ],
                                       _pmt_psy_attack_time,
                                       CControlAnimation::eAnimationHit );
...
void CControllerAnimation::on_event( ControlCom::EEventType type,
...
void CControllerAnimation::load() {
    ...
    m_legs[ eLegsStealBkwd ] =
        skeleton->ID_Cycle_Safe( "new_walk_steal_beack_0" );
    ...
    m_legs[ eLegsRunStrafeLeftDamaged ] =
        skeleton->ID_Cycle_Safe( "new_run_fwd_0" );
    m_legs[ eLegsRunStrafeRightDamaged ] =
        skeleton->ID_Cycle_Safe( "new_run_fwd_0" );

    m_torso[ eTorsoIdle ] = skeleton->ID_Cycle_Safe( "new_torso_idle_0" );
    m_torso[ eTorsoSteal ] = skeleton->ID_Cycle_Safe( "new_torso_steal_0" );
    m_torso[ eTorsoPsyAttack ] =
        skeleton->ID_Cycle_Safe( "new_torso_attack_0" );
    ...
    add_path_rotation( eLegsTypeStealMotion, ( PI - PI_DIV_4 ),
                       eLegsStealBkwdLeft );
    add_path_rotation( eLegsTypeStealMotion, -( PI - PI_DIV_4 ),
                       eLegsStealBkwdRight );
...
void CControllerAnimation::add_path_rotation( ELegsActionType action,
                                              float angle,
                                              ELegsActionType type ) {
    SPathRotations rot;
    rot.angle = angle;
...
void CControllerAnimation::set_path_direction() {
    float cur_yaw = Fvector()
                        .sub( m_controller->custom_dir().get_head_look_point(),
                              m_object->Position() )
                        .getH();
    ...
    m_object->dir().set_heading(
        angle_normalize( target_yaw + path_rot.angle ) );
...
void CControllerAnimation::select_torso_animation() {
    ...
    if ( ( ctrl_data->torso.get_motion() != target_motion ) ||
         m_wait_torso_anim_end ) {
...
void CControllerAnimation::select_legs_animation() {
    ...
    if ( is_moving() ) {
        ...
        for ( LEGS_MOTION_MAP_IT it = m_legs.begin(); it != m_legs.end();
              it++ ) {
            if ( ( it->first & m_current_legs_action ) ==
                 m_current_legs_action ) {
...
CControllerAnimation::SPathRotations CControllerAnimation::get_path_rotation(
    float cur_yaw ) {
    float target_yaw = m_man->path_builder().detail().direction().getH();
    target_yaw = angle_normalize( -target_yaw );
    ...
    PATH_ROTATIONS_VEC_IT it_best =
        m_path_rotations[ m_current_legs_action ].begin();
    float best_diff = flt_max;
    for ( PATH_ROTATIONS_VEC_IT it =
              m_path_rotations[ m_current_legs_action ].begin();
...
void CControllerAnimation::set_body_state( ETorsoActionType torso,
                                           ELegsActionType legs ) {
...
bool CControllerAnimation::is_moving() {
    ...
    if ( ( ( m_current_legs_action & eLegsTypeStealMotion ) !=
           eLegsTypeStealMotion ) &&
...
void CControllerAnimation::set_path_params() {
    bool moving_action =
        ( ( m_current_legs_action & eLegsTypeStealMotion ) ==
          eLegsTypeStealMotion ) ||
        ( ( m_current_legs_action & eLegsTypeWalk ) == eLegsTypeWalk ) ||
        ( ( m_current_legs_action & eLegsTypeRun ) == eLegsTypeRun );
