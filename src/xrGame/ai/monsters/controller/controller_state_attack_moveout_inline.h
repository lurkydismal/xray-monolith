...
void CStateControlMoveOutAbstract::execute() {
    ...
    this->object->custom_anim().set_body_state(
        CControllerAnimation::eTorsoSteal,
        CControllerAnimation::eLegsTypeStealMotion );
}
...
bool CStateControlMoveOutAbstract::check_completion() {
    if ( this->object->EnemyMan.see_enemy_now() )
        return true;
    if ( this->time_state_started + 10000 < time() )
        return true;
    if ( this->object->HitMemory.is_hit() &&
         this->object->HitMemory.get_last_hit_time() >
             this->time_state_started )
        return true;

    if ( this->object->EnemyMan.get_enemy_position().distance_to(
             this->object->Position() ) < 1.f )
        return true;
...
void CStateControlMoveOutAbstract::update_target_point() {
    if ( m_state == eMoveToNodeEnemyLastSeen ) {
        if ( this->object->Position().similar( m_target_position, 0.05f ) )
            m_state = eMoveToLastEnemyLocation;
    }

    if ( m_state == eMoveToNodeEnemyLastSeen ) {
        if ( this->object->EnemyMan.get_my_vertex_enemy_last_seen() !=
             u32( -1 ) )
            m_target_node =
                this->object->EnemyMan.get_my_vertex_enemy_last_seen();
        else
...
void CStateControlMoveOutAbstract::update_look_point() {
    if ( this->object->HitMemory.get_last_hit_time() >
         this->object->EnemyMan.get_enemy_time_last_seen() ) {
        m_look_point.mad( this->object->Position(),
                          this->object->HitMemory.get_last_hit_dir(), 5.f );
        m_look_point.y += 1.5f;
    ...
    if ( ( Random.randI( 100 ) < LOOK_COVER_PROBABILITY ) &&
         ( m_last_time_look_point_updated != 0 ) ) {
        float angle = ai().level_graph().vertex_low_cover_angle(
            this->object->ai_location().level_vertex_id(), deg( 10 ),
            std::greater< float >() );
        m_look_point.mad( this->object->Position(),
                          Fvector().setHP( angle, 0.f ), 3.f );
        m_current_delay = DEFAULT_LOOK_POINT_CHANGE_DELAY;
    } else {
