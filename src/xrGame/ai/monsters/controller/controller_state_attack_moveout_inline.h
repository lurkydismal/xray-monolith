...
bool CStateControlMoveOutAbstract::check_completion()
{
	...
	if (this->object->HitMemory.is_hit() &&
		this->object->HitMemory.get_last_hit_time() > this->time_state_started)
		return true;
...
void CStateControlMoveOutAbstract::update_look_point()
{
	...
	{
		float angle = ai().level_graph().vertex_low_cover_angle(this->object->ai_location().level_vertex_id(),deg(10),
		                                                        std::greater<float>());
