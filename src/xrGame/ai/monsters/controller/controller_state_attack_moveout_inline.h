...
void CStateControlMoveOutAbstract::execute() {
    ...
    object->custom_anim().set_body_state(
...
bool CStateControlMoveOutAbstract::check_completion() {
    ...
    if ( object->EnemyMan.get_enemy_position().distance_to(
             object->Position() ) < 1.f )
        return true;
...
void CStateControlMoveOutAbstract::update_look_point() {
    if ( object->HitMemory.get_last_hit_time() >
        ...
        m_look_point.mad( object->Position(),
                          object->HitMemory.get_last_hit_dir(), 5.f );
        m_look_point.y += 1.5f;
    ...
    if ( ( Random.randI( 100 ) < LOOK_COVER_PROBABILITY ) &&
        ...
        float angle = ai().level_graph().vertex_low_cover_angle(
            object->ai_location().level_vertex_id(), deg( 10 ),
            std::greater< float >() );
        m_look_point.mad( object->Position(), Fvector().setHP( angle, 0.f ),
                          3.f );
        m_current_delay = DEFAULT_LOOK_POINT_CHANGE_DELAY;
    } else {
