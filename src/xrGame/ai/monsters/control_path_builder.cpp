...
CControlPathBuilder::CControlPathBuilder( CCustomMonster* monster )
    : CMovementManager( monster ) {}
...
void CControlPathBuilder::update_schedule() {
    ...
    // the one and only reason is because of the restriction-change, so wait
    // until position and node will be in valid state
    if ( m_data.path_type != MovementManager::ePathTypePatrolPath ) {
        if ( !accessible( m_data.target_position ) ||
             ( ( m_data.target_node != u32( -1 ) ) &&
               ( !accessible( m_data.target_node ) ) ) ) {
            return;
        }
    }
...
// Special Build Path
//////////////////////////////////////////////////////////////////////////
bool CControlPathBuilder::build_special( const Fvector& target,
                                         u32 node,
                                         u32 vel_mask ) {
    ...
    if ( !path_completed() &&
         ( detail().time_path_built() >= Device.dwTimeGlobal ) )
        return true;
...
bool CControlPathBuilder::is_path_end( float dist_to_end ) {
    ...
    for ( u32 i = detail().curr_travel_point_index() + 1;
          i < detail().path().size() - 1; i++ ) {
        cur_dist_to_end += detail().path()[ i ].position.distance_to(
...
bool CControlPathBuilder::valid_destination( const Fvector& pos, u32 node ) {
    return ( ai().level_graph().valid_vertex_id( node ) &&
...
void CControlPathBuilder::fix_position( const Fvector& pos,
                                        u32 node,
                                        Fvector& res_pos ) {
    ...
    if ( !accessible( res_pos ) ) {
        u32 level_vertex_id = restrictions().accessible_nearest(
            Fvector().set( res_pos ), res_pos );

#ifdef DEBUG
        if ( level_vertex_id != node ) {
            Msg( "! src_node[%d] res_node[%d] src_pos[%f,%f,%f] "
                 "res_pos[%f,%f,%f]",
                 node, level_vertex_id, VPUSH( pos ), VPUSH( res_pos ) );
        }
        VERIFY3( ( level_vertex_id == node ) ||
...
bool CControlPathBuilder::is_moving_on_path() {
    return ( !detail().completed( inherited_com::m_object->Position() ) &&
             enabled() );
}

bool CControlPathBuilder::get_node_in_radius( u32 src_node,
                                              float min_radius,
                                              float max_radius,
                                              u32 attempts,
                                              u32& dest_node ) {
    ...
    for ( u32 i = 0; i < attempts; i++ ) {
        ...
        new_pos.mad( vertex_position, dir,
                     Random.randF( min_radius, max_radius ) );

        restrictions().add_border( vertex_position, new_pos );
        dest_node = ai().level_graph().check_position_in_direction(
            src_node, vertex_position, new_pos );
        restrictions().remove_border();
...
bool CControlPathBuilder::can_use_distributed_computations( u32 option ) const {
    ...
    if ( Actor()->memory().visual().visible_right_now(
             inherited_com::m_object ) )
        return false;
