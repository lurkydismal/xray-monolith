...
void CAI_Dog::Load( LPCSTR section ) {
    ...
    if ( pSettings->line_exist( section, "corpse_use_timeout" ) ) {
        m_corpse_use_timeout =
            1000 * pSettings->r_u32( section, "corpse_use_timeout" );
    } else {
    ...
    SVelocityParam& velocity_walk_smell =
        move().get_velocity( MonsterMovement::eVelocityParameterWalkSmelling );
    SVelocityParam& velocity_walk_growl =
        move().get_velocity( MonsterMovement::eVelocityParameterWalkGrowl );
    ...
    anim().AddAnim( eAnimRunDamaged, "stand_run_dmg_", -1, &velocity_run_dmg,
                    PS_STAND );

    anim().AddAnim( eAnimRunTurnLeft, "stand_run_turn_left_", -1, &velocity_run,
                    PS_STAND );
    ...
    anim().AddAnim( eAnimCheckCorpse, "stand_check_corpse_", -1, &velocity_none,
                    PS_STAND );
    anim().AddAnim( eAnimDragCorpse, "stand_drage_", -1, &velocity_drag,
                    PS_STAND );
    // anim().AddAnim(eAnimSniff,		"stand_sniff_",
    // -1, &velocity_none,		PS_STAND); anim().AddAnim(eAnimHowling,
    // "stand_howling_",		-1,	&velocity_none,
    // PS_STAND);

    // anim().AddAnim(eAnimJumpGlide,   	"jump_glide_",
    // -1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimJumpGlide,   	"stand_jump_left_",
    // 0, &velocity_none,		PS_STAND);
    anim().AddAnim( eAnimJumpGlide, "stand_jump_left_", 0, &velocity_none,
                    PS_STAND );

    anim().AddAnim( eAnimSteal, "stand_walk_fwd_", -1, &velocity_steal,
                    PS_STAND );
    anim().AddAnim( eAnimThreaten, "stand_threaten_", -1, &velocity_none,
                    PS_STAND );

    anim().AddAnim( eAnimSitLieDown, "sit_lie_down_", -1, &velocity_none,
                    PS_SIT );
    anim().AddAnim( eAnimStandSitDown, "stand_sit_down_", -1, &velocity_none,
                    PS_STAND );
    anim().AddAnim( eAnimSitStandUp, "sit_stand_up_", -1, &velocity_none,
                    PS_SIT );
    // anim().AddAnim(eAnimLieToSleep,	"lie_to_sleep_",		-1,
    // &velocity_none,		PS_LIE);
    anim().AddAnim( eAnimLieSitUp, "lie_to_sit_", -1, &velocity_none, PS_LIE );

    anim().AddAnim( eAnimJumpLeft, "stand_jump_left_", -1, &velocity_none,
                    PS_STAND );
    anim().AddAnim( eAnimJumpRight, "stand_jump_right_", -1, &velocity_none,
                    PS_STAND );

    /////////////mob home

    anim().AddAnim( eAnimHomeWalkSmelling, "stand_walk_smelling_", -1,
                    &velocity_walk_smell, PS_STAND );
    anim().AddAnim( eAnimHomeWalkGrowl, "stand_growl_walk_", -1,
                    &velocity_walk_growl, PS_STAND );

    /////////////end mob home

    // define transitions
    // order : 1. [anim -> anim]	2. [anim->state]	3. [state ->
    // anim]		4. [state -> state]
    ...
    anim().AddTransition( PS_SIT, PS_STAND, eAnimSitStandUp, false,
                          SKIP_IF_AGGRESSIVE );
    anim().AddTransition( PS_LIE, PS_SIT, eAnimLieSitUp, false,
                          SKIP_IF_AGGRESSIVE );
...
void CAI_Dog::reinit() {
    ...
    // com_man().add_rotation_jump_data("stand_jump_left_0",
    // "stand_jump_left_0", "stand_jump_right_0" ,"stand_jump_right_0",
    // deg(179)); com_man().add_melee_jump_data("stand_jump_left_0",
    // "stand_jump_right_0");
...
bool CAI_Dog::is_night() {
    u32 year = 0, month = 0, day = 0, hours = 0, mins = 0, secs = 0,
        milisecs = 0;
    split_time( Level().GetGameTime(), year, month, day, hours, mins, secs,
                milisecs );
    if ( hours <= 6 || hours >= 21 ) {
        return true;
    }
...
void CAI_Dog::start_animation() {
    ...
    smart_cast< IKinematicsAnimated* >( Visual() )
        ->PlayCycle( get_current_animation(), TRUE, animation_end, this );
    b_state_end = true;
}
...
void CAI_Dog::animation_end( CBlend* B ) {
    ...
    ( ( CAI_Dog* )B->CallbackParam )
        ->com_man()
        .script_release( ControlCom::eControlAnimation );
...
void CAI_Dog::reload( LPCSTR section ) {
    ...
    com_man().load_jump_data( 0, "jump_ataka_01", "jump_ataka_02",
                              "jump_ataka_03",
