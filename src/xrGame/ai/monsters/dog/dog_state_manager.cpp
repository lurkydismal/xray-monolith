...
CStateManagerDog::CStateManagerDog( CAI_Dog* monster ) : inherited( monster ) {
    ...
    add_state( eStateAttack,
               xr_new< CStateGroupAttack< CAI_Dog > >( monster ) );
    add_state( eStateEat, xr_new< CStateGroupEat< CAI_Dog > >( monster ) );
    add_state(
        eStateHearInterestingSound,
        xr_new< CStateMonsterHearInterestingSound< CAI_Dog > >( monster ) );
    add_state( eStateHearDangerousSound,
               xr_new< CStateGroupHearDangerousSound< CAI_Dog > >( monster ) );
    add_state( eStateHitted,
               xr_new< CStateMonsterHitted< CAI_Dog > >( monster ) );
    add_state( eStateControlled,
               xr_new< CStateMonsterControlled< CAI_Dog > >( monster ) );
    add_state( eStateHearHelpSound,
               xr_new< CStateMonsterHearHelpSound< CAI_Dog > >( monster ) );
...
void CStateManagerDog::execute() {
    ...
    if ( enemy ) {
        ...
        if ( squad ) {
            ...
            if ( object->Position().distance_to( enemy_pos ) <
                 detail::dog::atack_decision_maxdist ) {
                squad->set_home_in_danger();
            }
    ...
    if ( !object->is_under_control() ) {
        if ( atack ) {
            ...
            // comment by Lain: ||
            // monster_squad().get_squad(object)->GetCommand(object).type ==
            // SC_REST) {
    ...
    if ( prev_substate != current_substate &&
         object->get_custom_anim_state() ) {
