...
void CAI_Flesh::Load( LPCSTR section ) {
    ...
    anim().AddAnim( eAnimAttack, "stand_attack_", -1, &velocity_turn,
                    PS_STAND );
    anim().AddAnim( eAnimAttackFromBack, "stand_attack_back_", -1,
                    &velocity_none, PS_STAND );
    anim().AddAnim( eAnimCheckCorpse, "stand_eat_", 1, &velocity_none,
                    PS_STAND );
...
// возвращает true, если после выполнения этой функции необходимо прервать
// обработку т.е. если активирована последовательность
...
// Параметры: ConeVertex - вершина конуса, ConeAngle - угол конуса (между
// поверхностью и высотой) ConeDir - направление конуса, SphereCenter - центр
// сферы, SphereRadius - радиус сферы
bool CAI_Flesh::ConeSphereIntersection( Fvector ConeVertex,
                                        float ConeAngle,
                                        Fvector ConeDir,
