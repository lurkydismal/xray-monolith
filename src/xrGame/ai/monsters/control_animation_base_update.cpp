...
// DEBUG purpose only
const char* dbg_anim_name_table[] = { "eAnimStandIdle",
                                      "eAnimStandTurnLeft",
                                      "eAnimStandTurnRight",
...
void CControlAnimationBase::update() {
    ...
    if ( m_object->control().path_builder().is_moving_on_path() &&
        ...
        m_object->dir().use_path_direction(
...
void CControlAnimationBase::set_override_animation( EMotionAnim anim,
                                                    u32 index ) {
...
void CControlAnimationBase::set_override_animation_script( EMotionAnim anim,
                                                           u32 index ) {
    if ( m_override_animation == anim )
        return;
...
void CControlAnimationBase::set_override_animation( pcstr name ) {
    for ( u32 anim_type = 0; anim_type < m_anim_storage.size(); ++anim_type ) {
        ...
        if ( strstr( name, anim_name ? anim_name : "" ) == name ) {
            pcstr const anim_index_string =
                name + anim_item->target_name.size();
...
void CControlAnimationBase::SetTurnAnimation() {
    ...
    if ( IsStandCurAnim() && ( anim_state == PS_STAND ) &&
         ( !fis_zero( delta_yaw ) ) ) {
    ...
    if ( m_object->control().path_builder().is_moving_on_path() &&
         m_object->path().enabled() && ( delta_yaw > MOVE_TURN_ANGLE ) ) {
        m_object->SetTurnAnimation( turn_left );
        return;
    }
}
...
void CControlAnimationBase::SelectVelocities() {
    ...
    if ( b_moving ) {
        ...
        // если сейчас стоит на месте и есть след точка (т.е. должен быть в
        // движении), то реализовать поворот на месте, а дальше форсировать
        // скорость со следующей точки
        ...
        path_vel.set( _abs( current_velocity.linear_velocity ),
                      current_velocity.real_angular_velocity );
    }
    ...
    anim_vel.set( item_it->velocity.velocity.linear,
                  item_it->velocity.velocity.angular_real );
    ...
    if ( !m_object->state_invisible && !fis_zero( anim_vel.linear ) ) {
        ...
        if ( accel_chain_get( m_man->movement().real_velocity(),
                              cur_anim_info().get_motion(), new_anim,
                              a_speed ) ) {
    ...
    if ( m_object->state_invisible )
        ...
        if ( m_tAction == ACT_ATTACK ) {
            ...
            m_object->dir().set_heading_speed(
                vel * m_object->m_melee_rotation_factor );
...
void CControlAnimationBase::CheckVelocityBounce() {
    Fvector temp_vec;
    m_object->character_physics_support()->movement()->GetCharacterVelocity(
        temp_vec );
    ...
    float ratio = ( ( prev_speed > cur_speed ) ? ( prev_speed / cur_speed )
