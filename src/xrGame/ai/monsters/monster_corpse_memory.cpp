...
void CMonsterCorpseMemory::update() {
    for ( xr_vector< const CGameObject* >::const_iterator I =
        ...
        if ( monster->memory().visual().visible_now( *I ) ) {
            const CEntityAlive* p_corpse =
                smart_cast< const CEntityAlive* >( *I );
...
void CMonsterCorpseMemory::remove_non_actual() {
    ...
    for ( CORPSE_MAP_IT it = m_objects.begin(), nit; it != m_objects.end();
          it = nit ) {
        nit = it;
        ++nit;
...
CORPSE_MAP_IT CMonsterCorpseMemory::find_best_corpse() {
    ...
    for ( CORPSE_MAP_IT I = m_objects.begin(); I != m_objects.end(); I++ ) {
        if ( I->second.position.distance_to( monster->Position() ) <
             min_dist ) {
