...
CTelekineticObject* CTelekinesis::activate( CPhysicsShellHolder* obj,
                                            float strength,
                                            float height,
                                            u32 max_time_keep,
                                            bool rot ) {
    ...
    if ( !tele_object->init( this, obj, strength, height, max_time_keep,
                             rot ) ) {
...
void CTelekinesis::deactivate( CPhysicsShellHolder* obj ) {
    ...
    TELE_OBJECTS_IT it =
        std::find_if( objects.begin(), objects.end(), SFindPred( obj ) );
    if ( it == objects.end() )
        return;
...
void CTelekinesis::fire( CPhysicsShellHolder* obj,
...
void CTelekinesis::fire_t( CPhysicsShellHolder* obj,
                           const Fvector& target,
                           float time ) {
    TELE_OBJECTS_IT it =
        std::find_if( objects.begin(), objects.end(), SFindPred( obj ) );
