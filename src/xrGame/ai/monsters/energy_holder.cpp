...
void CEnergyHolder::reload( LPCSTR section, LPCSTR prefix, LPCSTR suffix ) {
    ...
    m_restore_vel = pSettings->r_float(
        section, strconcat( sizeof( line_name ), line_name, prefix,
                            "Energy_Restore_Velocity", suffix ) );
    m_decline_vel = pSettings->r_float(
        section, strconcat( sizeof( line_name ), line_name, prefix,
                            "Energy_Decline_Velocity", suffix ) );
    m_critical_value = pSettings->r_float(
        section, strconcat( sizeof( line_name ), line_name, prefix,
                            "Energy_Critical_Value", suffix ) );
    m_activate_value = pSettings->r_float(
        section, strconcat( sizeof( line_name ), line_name, prefix,
                            "Energy_Activate_Value", suffix ) );
    m_aggressive_restore_vel = pSettings->r_float(
        section, strconcat( sizeof( line_name ), line_name, prefix,
                            "Energy_Aggressive_Restore_Velocity", suffix ) );
...
void CEnergyHolder::schedule_update() {
    ...
    if ( !is_active() )
        m_value +=
            m_aggressive ? m_aggressive_restore_vel * dt : m_restore_vel * dt;
    else
