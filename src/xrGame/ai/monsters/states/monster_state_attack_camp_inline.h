...
bool CStateMonsterAttackCampAbstract::check_start_conditions()
{
	...
	// check enemy
	if (!this->object->EnemyMan.get_enemy()) return false;

	// check distance to enemy
	if (this->object->Position().distance_to(this->object->EnemyMan.get_enemy_position()) < MIN_DISTANCE_TO_ENEMY) return false;
