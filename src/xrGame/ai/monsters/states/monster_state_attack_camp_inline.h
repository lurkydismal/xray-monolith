...
CStateMonsterAttackCampAbstract::CStateMonsterAttackCamp( _Object* obj )
    : inherited( obj ) {
    add_state( eStateAttackCamp_Hide,
               xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    add_state( eStateAttackCamp_Camp,
               xr_new< CStateMonsterLookToPoint< _Object > >( obj ) );
    add_state( eStateAttackCamp_StealOut,
...
bool CStateMonsterAttackCampAbstract::check_completion() {
    ...
    if ( current_substate == eStateAttackCamp_Camp ) {
        if ( object->EnemyMan.see_enemy_now() )
            return true;
        if ( object->HitMemory.get_last_hit_time() >
             get_state_current()->time_started() )
            return true;
    }

    if ( object->EnemyMan.get_enemy()->Position().distance_to(
             object->Position() ) < 5.f )
        return true;
...
bool CStateMonsterAttackCampAbstract::check_start_conditions() {
    ...
    if ( object->Position().distance_to(
             object->EnemyMan.get_enemy_position() ) < MIN_DISTANCE_TO_ENEMY )
        return false;
    ...
    // try to get cover
    const CCoverPoint* point = object->CoverMan->find_cover(
        object->EnemyMan.get_enemy_position(), 10.f, 30.f );
    if ( !point )
        return false;
