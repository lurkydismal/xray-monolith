...
CStateMonsterAttackCampAbstract::CStateMonsterAttackCamp( _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateAttackCamp_Hide,
                     xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    this->add_state( eStateAttackCamp_Camp,
                     xr_new< CStateMonsterLookToPoint< _Object > >( obj ) );
    this->add_state(
        eStateAttackCamp_StealOut,
        xr_new< CStateMonsterAttackCampStealOut< _Object > >( obj ) );
}
...
bool CStateMonsterAttackCampAbstract::check_completion() {
    ...
    if ( this->current_substate == eStateAttackCamp_Camp ) {
        ...
        if ( this->object->HitMemory.get_last_hit_time() >
             this->get_state_current()->time_started() )
            return true;
    }

    if ( this->object->EnemyMan.get_enemy()->Position().distance_to(
             this->object->Position() ) < 5.f )
        return true;
...
bool CStateMonsterAttackCampAbstract::check_start_conditions() {
    ...
    const CCoverPoint* point = this->object->CoverMan->find_cover(
        this->object->EnemyMan.get_enemy_position(), 10.f, 30.f );
    if ( !point )
        return false;
...
void CStateMonsterAttackCampAbstract::reselect_state() {
    ...
    if ( this->prev_substate == eStateAttackCamp_Camp ) {
        if ( this->get_state( eStateAttackCamp_StealOut )
