...
CStateMonsterRestAbstract::CStateMonsterRest( _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateRest_Sleep,
                     xr_new< CStateMonsterRestSleep< _Object > >( obj ) );
    this->add_state( eStateRest_WalkGraphPoint,
                     xr_new< CStateMonsterRestWalkGraph< _Object > >( obj ) );
    this->add_state( eStateRest_Idle,
                     xr_new< CStateMonsterRestIdle< _Object > >( obj ) );
    this->add_state( eStateRest_Fun,
                     xr_new< CStateMonsterRestFun< _Object > >( obj ) );
    this->add_state( eStateSquad_Rest,
                     xr_new< CStateMonsterSquadRest< _Object > >( obj ) );
    this->add_state( eStateSquad_RestFollow,
                     xr_new< CStateMonsterSquadRestFollow< _Object > >( obj ) );
    this->add_state(
        eStateCustomMoveToRestrictor,
        xr_new< CStateMonsterMoveToRestrictor< _Object > >( obj ) );
    this->add_state(
        eStateRest_MoveToHomePoint,
        xr_new< CStateMonsterRestMoveToHomePoint< _Object > >( obj ) );
    this->add_state(
        eStateSmartTerrainTask,
        xr_new< CStateMonsterSmartTerrainTask< _Object > >( obj ) );
}
...
void CStateMonsterRestAbstract::execute() {
    ...
    } else if ( this->get_state( eStateSmartTerrainTask )
    ...
    if ( captured_by_smart_terrain )
        ...
        } else if ( this->get_state( eStateCustomMoveToRestrictor )
                        ->check_start_conditions() )
            move_to_restrictor = true;
        ...
        if ( move_to_restrictor )
            ...
            if ( this->prev_substate == eStateRest_MoveToHomePoint ) {
                if ( !this->get_state( eStateRest_MoveToHomePoint )
                          ->check_completion() )
                    move_to_home_point = true;
            } else if ( this->get_state( eStateRest_MoveToHomePoint )
                            ->check_start_conditions() )
                move_to_home_point = true;
            ...
            if ( move_to_home_point )
                ...
                if ( monster_squad()
                         ...
                         ->GetCommand( this->object )
                         .type == SC_REST ) {
                    this->select_state( eStateSquad_Rest );
                    use_squad = true;
                } else if ( monster_squad()
                                .get_squad( this->object )
                                ->GetCommand( this->object )
                                .type == SC_FOLLOW ) {
                ...
                if ( !use_squad ) {
                    ...
                    else if ( time_idle_selected + TIME_IDLE + TIME_IDLE / 2 >
                              time() )
