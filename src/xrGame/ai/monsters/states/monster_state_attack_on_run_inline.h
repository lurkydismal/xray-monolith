...
void ATTACK_ON_RUN_STATE::choose_next_atack_animation() {
    ...
    for ( int i = 0; i < 2; ++i ) {
        u32 num_animations =
            object->anim().get_animation_variants_count( animation_types[ i ] );
...
bool ATTACK_ON_RUN_STATE::check_control_start_conditions(
    ...
    if ( type == ControlCom::eControlRotationJump ) {
        ...
        float const self2enemy_mag =
            enemy_pos.distance_to( object->Position() );
...
void ATTACK_ON_RUN_STATE::calculate_predicted_enemy_pos() {
    float const prediction_factor =
        object->get_attack_on_move_prediction_factor();
...
void ATTACK_ON_RUN_STATE::update_aim_side() {
    CEntityAlive const* const enemy =
        m_attacking ? m_enemy_to_attack : object->EnemyMan.get_enemy();
    ...
    TTime const update_side_period =
        ( TTime )( object->get_attack_on_move_update_side_period() * 1000 );
...
void ATTACK_ON_RUN_STATE::update_movement_target() {
    ...
    TTime const max_go_close_time =
        ( TTime )( 1000 * object->get_attack_on_move_max_go_close_time() );
    float const far_radius = object->get_attack_on_move_far_radius();
    ...
    if ( m_phaze == go_prepare ) {
        ...
        if ( angle_between_vectors( object->Direction(), self2enemy ) >
                 deg2rad( 140.f ) &&
    ...
    if ( m_attacking ) {
        ...
        for ( ENEMIES_MAP::const_iterator it = memory.begin();
            ...
            float const melee_max_distance =
                object->MeleeChecker.get_max_distance();
            float const melee_min_distance =
                object->MeleeChecker.get_min_distance();
            ...
            // object->EnemyMan.see_enemy_now(enemy);
...
void ATTACK_ON_RUN_STATE::execute() {
    ...
    object->path().set_rebuild_time(
        m_attacking ? 20 : 150 ); // object->get_attack_rebuild_time());
...
bool ATTACK_ON_RUN_STATE::check_start_conditions() {
    float dist =
        object->MeleeChecker.distance_to_enemy( object->EnemyMan.get_enemy() );
    ...
    if ( !object->control().direction().is_face_target(
             object->EnemyMan.get_enemy(), deg( 30 ) ) ) {
        return false;
    }
