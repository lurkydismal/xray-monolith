...
TEMPLATE_SIGNATURE
ATTACK_ON_RUN_STATE::CStateMonsterAttackOnRun( _Object* obj )
    : inherited( obj ) {}
...
void ATTACK_ON_RUN_STATE::choose_next_atack_animation() {
    EMotionAnim animation_types[ 2 ] = { eAnimAttackOnRunLeft,
                                         eAnimAttackOnRunRight };

    for ( int i = 0; i < 2; ++i ) {
        u32 num_animations = this->object->anim().get_animation_variants_count(
            animation_types[ i ] );
...
TEMPLATE_SIGNATURE
bool ATTACK_ON_RUN_STATE::check_control_start_conditions(
    ...
    if ( type == ControlCom::eControlRotationJump ) {
        ...
        float const self2enemy_mag =
            enemy_pos.distance_to( this->object->Position() );

        return m_phaze == go_close && m_can_do_rotation_jump &&
               self2enemy_mag < 10.f;
    }
...
void ATTACK_ON_RUN_STATE::set_movement_phaze( phaze const new_phaze ) {
    ...
    if ( m_phaze == go_close ) {
        ...
        bool left_side = ( ( self2enemy.x * self_dir.z ) -
                           ( self2enemy.z * self_dir.x ) ) > 0.f;
...
void ATTACK_ON_RUN_STATE::calculate_predicted_enemy_pos() {
    float const prediction_factor =
        this->object->get_attack_on_move_prediction_factor();

    float const epsilon = 0.0001f;
    CEntityAlive const* const enemy = this->object->EnemyMan.get_enemy();
    Fvector const enemy_pos = enemy->Position();
    float const self2enemy_mag =
        magnitude( enemy_pos - this->object->Position() );
    ...
    float const self2enemy_time =
        self_velocity > epsilon ? self2enemy_mag / self_velocity : 0;

    float const predictiton_delta_sec =
        ( current_time() - m_last_prediction_time ) / 1000.f;
    ...
    m_predicted_enemy_pos = enemy_pos + m_predicted_enemy_velocity *
                                            self2enemy_time * prediction_factor;

    if ( magnitude( m_predicted_enemy_pos - this->object->Position() ) <
         0.01f ) {
        m_predicted_enemy_pos = enemy_pos;
...
void ATTACK_ON_RUN_STATE::update_aim_side() {
    CEntityAlive const* const enemy =
        m_attacking ? m_enemy_to_attack : this->object->EnemyMan.get_enemy();

    Fvector const self_dir = this->object->Direction();
    Fvector const self_to_enemy = enemy->Position() - this->object->Position();

    aim_side const new_attack_side =
        ( self_dir.x * self_to_enemy.z - self_dir.z * self_to_enemy.x > 0 )
            ? right
            : left;

    TTime const update_side_period =
        ( TTime )( this->object->get_attack_on_move_update_side_period() *
                   1000 );
...
inline bool is_valid_point_to_move( Fvector const& point, u32* out_vertex ) {
    ...
    CLevelGraph::CPosition vertex_pos =
        ai().level_graph().vertex_position( point );
...
void ATTACK_ON_RUN_STATE::update_movement_target() {
    ...
    debug::text_tree& text_tree =
        DBG().get_text_tree().find_or_add( "ActorView" );
    text_tree.add_line( "attacking", m_attacking );
#endif // DEBUG_STATE

    TTime const max_go_close_time =
        ( TTime )( 1000 *
                   this->object->get_attack_on_move_max_go_close_time() );
    float const far_radius = this->object->get_attack_on_move_far_radius();
    float const attack_radius =
        this->object->get_attack_on_move_attack_radius();
    ...
    if ( m_phaze == go_prepare ) {
        if ( current_time() > m_phaze_chosen_time + prepare_time * 1000 ) {
            set_movement_phaze( go_close );
        } else if ( self2predicted_mag < 3.f &&
                    current_time() > m_phaze_chosen_time + 3000 ) {
            set_movement_phaze( go_close );
        } else if ( m_go_far_start_point.distance_to( self_pos ) >
                    ( far_radius * 2 ) ) {
            set_movement_phaze( go_close );
        ...
        if ( angle_between_vectors( this->object->Direction(), self2enemy ) >
                 deg2rad( 140.f ) &&
...
#ifdef DEBUG_STATE
    text_tree.add_line( "phaze",
                        m_phaze == go_prepare ? "preparation" : "close" );
    text_tree.add_line( "reach_old_target", m_reach_old_target );
#endif // DEBUG_STATE

    if ( m_reach_old_target ) {
        self2target = m_target - self_pos;
        if ( magnitude( m_target - self_pos ) < 1.f ||
             current_time() > m_reach_old_target_start_time + 1000 ) {
            m_reach_old_target = false;
            set_movement_phaze( go_prepare );
        }
    } else if ( m_phaze == go_prepare ) {
        ...
        float const offs_angle =
            _max( deg2rad( 30.f ), offs_length / far_radius ) *
        ...
        Fvector const predicted2target =
            normalize( predicted2part ) * far_radius;
        ...
        float const dist2atack_point =
            _sqrt( self2predicted_mag * self2predicted_mag -
        ...
        Fvector const self2atack_point = cr_fvector3(
        ...
        Fvector dir2target =
            Fvector()
                .crossproduct( self2predicted, cr_fvector3( 0, 1, 0 ) )
                .normalize();
#ifdef DEBUG_STATE
        ...
        float const self2target_mag = _sqrt(
            far_radius * far_radius - self2predicted_mag * self2predicted_mag );
    ...
    Fvector const enemy_vertex_pos =
        ai().level_graph().vertex_position( enemy_vertex );
    u32 const target_vertex = ai().level_graph().check_position_in_direction(
        enemy_vertex, enemy_vertex_pos, m_target );

    bool const target_to_enemy_clear =
        ai().level_graph().valid_vertex_id( target_vertex );

    if ( ( m_phaze == go_close && !target_to_enemy_clear ) ||
         !is_valid_point_to_move( m_target, &m_target_vertex ) ) {
        if ( m_phaze == go_close ) {
            m_target_vertex = enemy->ai_location().level_vertex_id();
            m_target = ai().level_graph().vertex_position( m_target_vertex );
            m_predicted_enemy_pos = m_target;

            if ( this->object->ai_location().level_vertex_id() ==
                 m_target_vertex )
                set_movement_phaze( go_prepare );
        } else {
...
void ATTACK_ON_RUN_STATE::select_prepare_fallback_target() {
    ...
    for ( u32 index = 0; index < move_scan_points; ++index ) {
        ...
        Fvector const scan_point =
            enemy_pos +
            rotate_point( Fvector().set( far_radius, 0, 0 ), angle );
...
void ATTACK_ON_RUN_STATE::update_attack() {
    if ( m_attacking ) {
        if ( current_time() > m_attack_end_time ) {
            ...
            EMotionAnim override_animation =
                this->object->anim().get_override_animation();
            if ( override_animation == eAnimAttackOnRunLeft ||
                ...
                !this->object->anim().has_override_animation() &&
                m_phaze == go_close ) {
        ENEMIES_MAP const& memory = this->object->EnemyMemory.get_memory();
        CEntityAlive const* const main_enemy =
            this->object->EnemyMan.get_enemy();
        ...
        for ( ENEMIES_MAP::const_iterator it = memory.begin();
            ...
            Fvector const enemy_pos =
                enemy == main_enemy ? m_predicted_enemy_pos : enemy->Position();
            ...
            float const prepare_atack_dist =
                m_animation_hit_time[ m_attack_side ] * velocity;
            float const melee_max_distance =
                this->object->MeleeChecker.get_max_distance();
            float const melee_min_distance =
                this->object->MeleeChecker.get_min_distance();

            float const allowed_atack_distance =
                ( prepare_atack_dist + melee_max_distance ) * 0.9f;
            float const disallowed_atack_distance =
                ( prepare_atack_dist + melee_min_distance * 0.5f ) * 0.9f;

            Fvector self_direction_xz = this->object->Direction();
            self_direction_xz.y = 0;
            float const attack_angle =
                angle_between_vectors( self_to_enemy_xz, self_direction_xz );

            bool const good_attack_angle = attack_angle < deg2rad( 30.f );

            // bool const see_enemy_now		=
            // this->object->EnemyMan.see_enemy_now(enemy);
            bool const good_attack_dist =
                current_atack_dist < allowed_atack_distance &&
        ...
        if ( can_attack ) {
            ...
            EMotionAnim const anim = m_attack_side == left
                                         ...
                                         : eAnimAttackOnRunRight;
            bool const got_animation_info =
                this->object->anim().get_animation_info(
...
void ATTACK_ON_RUN_STATE::execute() {
    ...
    this->object->path().set_rebuild_time(
        m_attacking ? 20 : 150 ); // this->object->get_attack_rebuild_time());
    ...
    // 			this->object->path().set_dest_direction
    // (command.direction);
    // 		}
    // 	}
}
...
bool ATTACK_ON_RUN_STATE::check_start_conditions() {
    float dist = this->object->MeleeChecker.distance_to_enemy(
        this->object->EnemyMan.get_enemy() );
    ...
    if ( !this->object->control().direction().is_face_target(
             this->object->EnemyMan.get_enemy(), deg( 30 ) ) ) {
        return false;
    }
