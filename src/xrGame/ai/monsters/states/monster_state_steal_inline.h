...
CStateMonsterStealAbstract::CStateMonsterSteal( _Object* obj )
    : inherited( obj ) {}
...
bool CStateMonsterStealAbstract::check_conditions() {
    ...
    if ( !this->object->EnemyMan.get_flags().is(
             FLAG_ENEMY_STATS_NOT_READY ) ) {
        // Enemy is not moving fast
        if ( this->object->EnemyMan.get_flags().is(
                 FLAG_ENEMY_GO_FARTHER_FAST ) )
            return false;
    ...
    // if (this->object->control().path_builder().detail().time_path_built() >=
    // time_state_started) { 	if (this->object->path().get_path_angle() >
