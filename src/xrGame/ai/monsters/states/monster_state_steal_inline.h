...
bool CStateMonsterStealAbstract::check_conditions()
{
	...
	if (this->object->EnemyMan.get_enemies_count() > 1) return false;
	...
	if (!this->object->EnemyMan.get_flags().is(FLAG_ENEMY_STATS_NOT_READY))
	{
		...
		if (this->object->EnemyMan.get_flags().is(FLAG_ENEMY_GO_FARTHER_FAST)) return false;

		// Enemy doesn't know about me
		if (!this->object->EnemyMan.get_flags().is(FLAG_ENEMY_DOESNT_KNOW_ABOUT_ME)) return false;
	}
	...
	//	if (this->object->path().get_path_angle() > STEAL_MAX_PATH_ANGLE)	return false;
	//}
