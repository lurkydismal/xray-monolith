...
#define CStateMonsterMoveToRestrictorAbstract \
    CStateMonsterMoveToRestrictor< _Object >
...
void CStateMonsterMoveToRestrictorAbstract::initialize() {
    ...
    Fvector position;
    u32 node = this->object->control()
                   .path_builder()
                   .restrictions()
                   .accessible_nearest( this->object->Position(), position );
    if ( node != ( u32 )-1 &&
         this->object->control().path_builder().restrictions().accessible(
             node ) )
        this->object->path().set_target_point(
            ai().level_graph().vertex_position( node ), node );
}
...
bool CStateMonsterMoveToRestrictorAbstract::check_start_conditions() {
    return ( !this->object->control().path_builder().accessible(
