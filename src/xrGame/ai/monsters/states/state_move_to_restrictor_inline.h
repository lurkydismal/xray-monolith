...
void CStateMonsterMoveToRestrictorAbstract::initialize() {
    ...
    Fvector position;
    u32 node =
        object->control().path_builder().restrictions().accessible_nearest(
            object->Position(), position );
    if ( node != ( u32 )-1 &&
         object->control().path_builder().restrictions().accessible( node ) )
...
bool CStateMonsterMoveToRestrictorAbstract::check_start_conditions() {
    return (
        !object->control().path_builder().accessible( object->Position() ) );
}
