...
#define CStateMonsterMoveAroundPointAbstract \
    CStateMonsterMoveAroundPoint< _Object >
...
void CStateMonsterMoveAroundPointAbstract::execute() {
    // if (data.vertex != u32(-1)) object->MoveToTarget(data.point,
    // data.vertex); else object->MoveToTarget(data.point);
