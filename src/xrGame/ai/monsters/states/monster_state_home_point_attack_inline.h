...
#define CStateMonsterAttackMoveToHomePointAbstract \
    CStateMonsterAttackMoveToHomePoint< _Object >
...
CStateMonsterAttackMoveToHomePointAbstract::CStateMonsterAttackMoveToHomePoint(
    _Object* obj )
    : inherited( obj ) {}
...
void CStateMonsterAttackMoveToHomePointAbstract::select_target() {
    ...
    if ( m_target_node == u32( -1 ) )
        this->object->control().path_builder().get_node_in_radius(
            self_node, 5, 25, 10, m_target_node );
...
void CStateMonsterAttackMoveToHomePointAbstract::execute() {
    if ( m_target_node == u32( -1 ) ) {
        ...
        float const dist_to_target =
            this->object->Position().distance_to_xz( m_target_pos );
        if ( dist_to_target < 2 )
            select_target();
    }
