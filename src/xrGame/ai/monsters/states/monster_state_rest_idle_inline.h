...
CStateMonsterRestIdleAbstract::CStateMonsterRestIdle( _Object* obj )
    : inherited( obj ) {
...
void CStateMonsterRestIdleAbstract::initialize() {
    ...
    const CCoverPoint* point = this->object->CoverMan->find_cover(
        this->object->Position(), 5.f, 10.f );
    if ( !point ) {
        point = this->object->CoverMan->find_cover( this->object->Position(),
                                                    10.f, 30.f );
        if ( !point )
            return;
    }
