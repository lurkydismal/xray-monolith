...
CStateMonsterDangerMoveToHomePointAbstract::CStateMonsterDangerMoveToHomePoint(
    _Object* obj )
    : inherited( obj ) {
    add_state( eStatePanic_HomePoint_Hide,
               xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    add_state( eStatePanic_HomePoint_LookOpenPlace,
               xr_new< CStateMonsterLookToPoint< _Object > >( obj ) );
    add_state( eStatePanic_HomePoint_Camp,
...
bool CStateMonsterDangerMoveToHomePointAbstract::check_start_conditions() {
    return ( !object->Home->at_home() &&
             !object->Home->at_home( get_most_danger_pos() ) );
}
...
bool CStateMonsterDangerMoveToHomePointAbstract::check_completion() {
    if ( object->HitMemory.get_last_hit_time() > time_state_started )
        return true;
    if ( m_skip_camp && ( prev_substate != u32( -1 ) ) &&
         ( prev_substate != eStatePanic_HomePoint_Hide ) )
        return true;
