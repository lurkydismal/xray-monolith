...
#define CStateMonsterDangerMoveToHomePointAbstract \
    CStateMonsterDangerMoveToHomePoint< _Object >
...
CStateMonsterDangerMoveToHomePointAbstract::CStateMonsterDangerMoveToHomePoint(
    _Object* obj )
    : inherited( obj ) {
    this->add_state( eStatePanic_HomePoint_Hide,
                     xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    this->add_state( eStatePanic_HomePoint_LookOpenPlace,
                     xr_new< CStateMonsterLookToPoint< _Object > >( obj ) );
    this->add_state( eStatePanic_HomePoint_Camp,
...
bool CStateMonsterDangerMoveToHomePointAbstract::check_start_conditions() {
    return ( !this->object->Home->at_home() &&
             !this->object->Home->at_home( get_most_danger_pos() ) );
}
...
bool CStateMonsterDangerMoveToHomePointAbstract::check_completion() {
    if ( this->object->HitMemory.get_last_hit_time() >
        ...
        return true;
    if ( m_skip_camp && ( this->prev_substate != u32( -1 ) ) &&
         ( this->prev_substate != eStatePanic_HomePoint_Hide ) )
        return true;
