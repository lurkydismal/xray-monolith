...
CStateMonsterSquadRestFollowAbstract::CStateMonsterSquadRestFollow(
    _Object* obj )
    : inherited( obj ) {
    add_state( eStateSquad_RestFollow_Idle,
               xr_new< CStateMonsterCustomAction< _Object > >( obj ) );
    add_state( eStateSquad_RestFollow_WalkToPoint,
...
void CStateMonsterSquadRestFollowAbstract::initialize() {
    ...
    SSquadCommand& command =
        monster_squad().get_squad( object )->GetCommand( object );
...
void CStateMonsterSquadRestFollowAbstract::reselect_state() {
    SSquadCommand& command =
        monster_squad().get_squad( object )->GetCommand( object );
    if ( command.position.distance_to( object->Position() ) <
...
void CStateMonsterSquadRestFollowAbstract::setup_substates() {
    ...
    if ( current_substate == eStateSquad_RestFollow_WalkToPoint ) {
        ...
        Fvector dest_pos =
            monster_squad().get_squad( object )->GetCommand( object ).position;
