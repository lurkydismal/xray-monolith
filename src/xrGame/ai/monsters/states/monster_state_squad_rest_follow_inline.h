...
#define CStateMonsterSquadRestFollowAbstract \
    CStateMonsterSquadRestFollow< _Object >
...
CStateMonsterSquadRestFollowAbstract::CStateMonsterSquadRestFollow(
    _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateSquad_RestFollow_Idle,
                     xr_new< CStateMonsterCustomAction< _Object > >( obj ) );
    this->add_state( eStateSquad_RestFollow_WalkToPoint,
...
void CStateMonsterSquadRestFollowAbstract::initialize() {
    ...
    SSquadCommand& command =
        monster_squad().get_squad( this->object )->GetCommand( this->object );
...
void CStateMonsterSquadRestFollowAbstract::reselect_state() {
    SSquadCommand& command =
        monster_squad().get_squad( this->object )->GetCommand( this->object );
    if ( command.position.distance_to( this->object->Position() ) <
         Random.randF( STOP_DISTANCE, STAY_DISTANCE ) ) {
...
void CStateMonsterSquadRestFollowAbstract::setup_substates() {
    ...
    if ( this->current_substate == eStateSquad_RestFollow_WalkToPoint ) {
        ...
        Fvector dest_pos = monster_squad()
