...
CStateMonsterPanicAbstract::CStateMonsterPanic( _Object* obj )
    : inherited( obj ) {
    this->add_state( eStatePanic_Run,
                     xr_new< CStateMonsterPanicRun< _Object > >( obj ) );
    this->add_state(
        eStatePanic_FaceUnprotectedArea,
        xr_new< CStateMonsterLookToUnprotectedArea< _Object > >( obj ) );
    this->add_state(
        eStatePanic_MoveToHomePoint,
...
void CStateMonsterPanicAbstract::reselect_state() {
    if ( this->get_state( eStatePanic_MoveToHomePoint )
...
void CStateMonsterPanicAbstract::check_force_state() {
    if ( ( this->current_substate == eStatePanic_FaceUnprotectedArea ) ) {
        // если видит врага
        if ( this->object->EnemyMan.get_enemy_time_last_seen() ==
             Device.dwTimeGlobal ) {
            this->select_state( eStatePanic_Run );
            return;
        }
        // если получил hit
        if ( this->object->HitMemory.get_last_hit_time() + 5000 >
             Device.dwTimeGlobal ) {
            this->select_state( eStatePanic_Run );
            return;
        }
