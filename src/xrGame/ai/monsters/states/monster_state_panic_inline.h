...
CStateMonsterPanicAbstract::CStateMonsterPanic( _Object* obj )
    : inherited( obj ) {
    add_state( eStatePanic_Run,
               xr_new< CStateMonsterPanicRun< _Object > >( obj ) );
    add_state( eStatePanic_FaceUnprotectedArea,
               xr_new< CStateMonsterLookToUnprotectedArea< _Object > >( obj ) );
    add_state( eStatePanic_MoveToHomePoint,
...
void CStateMonsterPanicAbstract::check_force_state() {
    if ( ( current_substate == eStatePanic_FaceUnprotectedArea ) ) {
        // если видит врага
        if ( object->EnemyMan.get_enemy_time_last_seen() ==
             Device.dwTimeGlobal ) {
            select_state( eStatePanic_Run );
            return;
        }
        // если получил hit
        if ( object->HitMemory.get_last_hit_time() + 5000 >
             Device.dwTimeGlobal ) {
            select_state( eStatePanic_Run );
            return;
        }
