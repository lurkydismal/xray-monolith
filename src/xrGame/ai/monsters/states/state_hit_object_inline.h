...
void CStateMonsterHitObjectAbstract::execute() {
    ...
    if ( !m_hitted &&
         ( time_state_started + TIME_POINTBREAK < Device.dwTimeGlobal ) ) {
        m_hitted = true;
        ...
        dir.add( Fvector().sub( target->Position(), object->Position() ),
                 object->Direction() );
        dir.normalize();
        target->m_pPhysicsShell->applyImpulse(
            dir, IMPULSE * target->m_pPhysicsShell->getMass() );
    }
}
...
bool CStateMonsterHitObjectAbstract::check_start_conditions() {
    ...
    Level().ObjectSpace.GetNearest( m_nearest_objects, object->Position(),
                                    object->Radius() - 0.5f, object() );
