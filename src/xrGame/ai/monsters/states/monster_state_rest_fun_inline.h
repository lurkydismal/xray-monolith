...
CStateMonsterRestFunAbstract::CStateMonsterRestFun( _Object* obj )
    : inherited( obj ) {
    time_last_hit = 0;
}
...
void CStateMonsterRestFunAbstract::execute() {
    ...
    Fvector dir;
    dir.sub( this->object->CorpseMan.get_corpse_position(),
    ...
    if ( ( dist < this->object->db().m_fDistToCorpse + 0.5f ) &&
         ( time_last_hit + MIN_DELAY < Device.dwTimeGlobal ) ) {
        CEntityAlive* corpse =
            const_cast< CEntityAlive* >( this->object->CorpseMan.get_corpse() );
        CPhysicsShellHolder* target =
            smart_cast< CPhysicsShellHolder* >( corpse );

        if ( target && target->m_pPhysicsShell ) {
            Fvector dir;
            dir.add(
                Fvector().sub( target->Position(), this->object->Position() ),
            ...
            // выполнить бросок
            for ( u32 i = 0; i < target->m_pPhysicsShell->get_ElementsNumber();
                  i++ ) {
                target->m_pPhysicsShell->get_ElementByStoreOrder( ( u16 )i )
                    ->applyImpulse(
                        dir, IMPULSE_TO_CORPSE *
                                 target->m_pPhysicsShell->getMass() /
...
bool CStateMonsterRestFunAbstract::check_start_conditions() {
    return ( ( this->object->CorpseMan.get_corpse() != 0 ) &&
