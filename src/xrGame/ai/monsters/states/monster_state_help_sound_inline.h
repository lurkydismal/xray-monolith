...
CStateMonsterHearHelpSoundAbstract::CStateMonsterHearHelpSound( _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateHearHelpSound_MoveToDest,
                     xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    this->add_state(
        eStateHearHelpSound_LookAround,
...
bool CStateMonsterHearHelpSoundAbstract::check_start_conditions() {
    ...
    if ( this->object->Home->has_home() )
        return this->object->Home->at_home( ai().level_graph().vertex_position(
            this->object->SoundMemory.hear_help_sound_node() ) );
