...
#define CStateMonsterRestMoveToHomePointAbstract \
    CStateMonsterRestMoveToHomePoint< _Object >
...
void CStateMonsterRestMoveToHomePointAbstract::execute() {
    this->object->path().set_target_point(
        ai().level_graph().vertex_position( m_target_node ), m_target_node );
    this->object->anim().accel_activate( EAccelType(
        this->object->Home->is_aggressive() ? eAT_Aggressive : eAT_Calm ) );
    ...
    this->object->set_action(
        this->object->Home->is_aggressive() ? ACT_RUN : ACT_WALK_FWD );
    this->object->set_state_sound( this->object->Home->is_aggressive()
...
bool CStateMonsterRestMoveToHomePointAbstract::check_completion() {
    return (
        ( this->object->ai_location().level_vertex_id() == m_target_node ) &&
