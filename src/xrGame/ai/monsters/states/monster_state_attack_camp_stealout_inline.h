...
#define CStateMonsterAttackCampStealOutAbstract \
    CStateMonsterAttackCampStealOut< _Object >

TEMPLATE_SPECIALIZATION
CStateMonsterAttackCampStealOutAbstract::CStateMonsterAttackCampStealOut(
    _Object* obj )
    : inherited( obj ) {}
...
bool CStateMonsterAttackCampStealOutAbstract::check_completion() {
    ...
    Fvector pos = ai().level_graph().vertex_position(
        this->object->EnemyMan.get_my_vertex_enemy_last_seen() );
    if ( ( this->object->Position().distance_to( pos ) < 2.f ) &&
         this->object->control().path_builder().is_path_end( 0.f ) )
        return true;
