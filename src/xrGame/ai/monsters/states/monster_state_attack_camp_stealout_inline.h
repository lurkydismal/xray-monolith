...
bool CStateMonsterAttackCampStealOutAbstract::check_completion()
{
	...
	if (this->time_state_started + STATE_EXECUTE_TIME < time()) return true;

	Fvector pos = ai().level_graph().vertex_position(this->object->EnemyMan.get_my_vertex_enemy_last_seen());
	if ((this->object->Position().distance_to(pos) < 2.f) && this->object->control().path_builder().is_path_end(0.f)) return true;
