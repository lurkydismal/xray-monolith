...
CStateMonsterAttackMeleeAbstract::CStateMonsterAttackMelee( _Object* obj )
    : inherited( obj ) {}
...
void CStateMonsterAttackMeleeAbstract::execute() {
    ...
    if ( this->object->control().direction().is_face_target(
             this->object->EnemyMan.get_enemy(), PI_DIV_3 ) )
        this->object->dir().face_target( this->object->EnemyMan.get_enemy(),
                                         800 );
    else
        this->object->dir().face_target( this->object->EnemyMan.get_enemy(), 0,
                                         deg( 15 ) );
...
bool CStateMonsterAttackMeleeAbstract::check_start_conditions() {
    return ( this->object->MeleeChecker.can_start_melee(
...
bool CStateMonsterAttackMeleeAbstract::check_completion() {
    return ( this->object->MeleeChecker.should_stop_melee(
        this->object->EnemyMan.get_enemy() ) );
}
