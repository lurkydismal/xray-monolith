...
CStateMonsterControlledAbstract::CStateMonsterControlled( _Object* obj )
    : inherited( obj ) {
    this->add_state(
        eStateControlled_Attack,
        xr_new< CStateMonsterControlledAttack< _Object > >( obj ) );
    this->add_state(
        eStateControlled_Follow,
        xr_new< CStateMonsterControlledFollow< _Object > >( obj ) );
}
    ...
    switch ( this->object->get_data().m_task ) {
        ...
        case eTaskAttack: {
            ...
            if ( !enemy || enemy->getDestroy() || !enemy->g_Alive() ) {
                this->object->get_data().m_object =
                    this->object->get_controller();
                this->select_state( eStateControlled_Follow );
            } else
