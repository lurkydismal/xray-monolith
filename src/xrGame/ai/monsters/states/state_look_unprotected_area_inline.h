...
#define CStateMonsterLookToUnprotectedAreaAbstract \
    CStateMonsterLookToUnprotectedArea< _Object >
...
TEMPLATE_SPECIALIZATION
CStateMonsterLookToUnprotectedAreaAbstract::
...
void CStateMonsterLookToUnprotectedAreaAbstract::initialize() {
    ...
    float angle = ai().level_graph().vertex_high_cover_angle(
        this->object->ai_location().level_vertex_id(), PI_DIV_6,
        std::less< float >() );
...
void CStateMonsterLookToUnprotectedAreaAbstract::execute() {
    ...
    if ( data.sound_type != u32( -1 ) ) {
        if ( data.sound_delay != u32( -1 ) )
            this->object->sound().play( data.sound_type, 0, 0,
                                        data.sound_delay );
        else
