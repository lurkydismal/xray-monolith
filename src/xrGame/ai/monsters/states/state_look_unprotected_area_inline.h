...
void CStateMonsterLookToUnprotectedAreaAbstract::initialize()
{
	...
	float angle = ai().level_graph().vertex_high_cover_angle(this->object->ai_location().level_vertex_id(), PI_DIV_6,
	                                                         std::less<float>());
...
bool CStateMonsterLookToUnprotectedAreaAbstract::check_completion()
{
	...
	else if (!this->object->control().direction().is_turning()) return true;
