...
#define CStateMonsterAttackRunAttackAbstract \
    CStateMonsterAttackRunAttack< _Object >
...
bool CStateMonsterAttackRunAttackAbstract::check_start_conditions() {
    ...
    target_position.mad( this->object->Position(), this->object->Direction(),
                         this->object->db().m_run_attack_path_dist );

    // if
    // (!this->object->control().path_builder().build_special(target_position,
    // u32(-1), MonsterMovement::eVelocityParamsRunAttack)) return false; else
    // this->object->path().enable_path();
