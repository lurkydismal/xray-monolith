...
bool CStateMonsterAttackRunAttackAbstract::check_start_conditions() {
    ...
    target_position.mad( object->Position(), object->Direction(),
                         object->db().m_run_attack_path_dist );

    // if (!object->control().path_builder().build_special(target_position,
    // u32(-1), MonsterMovement::eVelocityParamsRunAttack)) return false; else
    // object->path().enable_path();
