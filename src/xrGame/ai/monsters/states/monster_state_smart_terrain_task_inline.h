...
#define CStateMonsterSmartTerrainTaskAbstract \
    CStateMonsterSmartTerrainTask< _Object >
...
CStateMonsterSmartTerrainTaskAbstract::CStateMonsterSmartTerrainTask(
    ...
    this->add_state(
        ...
        xr_new< CStateMonsterSmartTerrainTaskGraphWalk< _Object > >( obj ) );
    this->add_state( eStateSmartTerrainTaskLevelPathWalk,
                     xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    this->add_state( eStateSmartTerrainTaskWaitCapture,
...
bool CStateMonsterSmartTerrainTaskAbstract::check_start_conditions() {
    ...
    CSE_ALifePsyDogPhantom* phantom =
        smart_cast< CSE_ALifePsyDogPhantom* >( monster );
    if ( phantom )
        return false;
...
void CStateMonsterSmartTerrainTaskAbstract::reselect_state() {
    if ( this->prev_substate == u32( -1 ) ) {
        if ( this->get_state( eStateSmartTerrainTaskGamePathWalk )
...
void CStateMonsterSmartTerrainTaskAbstract::check_force_state() {
    ...
    CALifeSmartTerrainTask* task =
        monster->brain().smart_terrain().task( monster );
