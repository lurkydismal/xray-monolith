...
void CStateMonsterHittedMoveOutAbstract::execute() {
    ...
    if ( target.node != u32( -1 ) )
        ...
        this->object->path().set_target_point(
            this->object->HitMemory.get_last_hit_position() );

    float dist = this->object->HitMemory.get_last_hit_position().distance_to(
...
void CStateMonsterHittedMoveOutAbstract::select_target() {
    if ( !this->object->GetCoverCloseToPoint(
             this->object->HitMemory.get_last_hit_position(), 10.f, 20.f, 0.f,
             15.f, target.position, target.node ) ) {
        target.node = u32( -1 );
    }
}
