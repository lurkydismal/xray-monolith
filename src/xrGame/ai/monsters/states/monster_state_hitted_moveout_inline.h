...
void CStateMonsterHittedMoveOutAbstract::execute() {
    ...
    if ( target.node != u32( -1 ) )
        ...
        object->path().set_target_point(
            object->HitMemory.get_last_hit_position() );

    float dist = object->HitMemory.get_last_hit_position().distance_to(
        object->Position() );
...
void CStateMonsterHittedMoveOutAbstract::select_target() {
    if ( !object->GetCoverCloseToPoint(
             object->HitMemory.get_last_hit_position(), 10.f, 20.f, 0.f, 15.f,
