...
#define CStateMonsterControlledFollowAbstract \
    CStateMonsterControlledFollow< _Object >
...
CStateMonsterControlledFollowAbstract::CStateMonsterControlledFollow(
    _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateControlled_Follow_Wait,
                     xr_new< CStateMonsterCustomAction< _Object > >( obj ) );
    this->add_state( eStateControlled_Follow_WalkToObject,
...
void CStateMonsterControlledFollowAbstract::reselect_state() {
    CControlledEntityBase* entity =
        smart_cast< CControlledEntityBase* >( this->object );
    VERIFY( entity );
    const CEntity* target_object = entity->get_data().m_object;

    float dist =
        this->object->Position().distance_to( target_object->Position() );
    this->select_state( dist < Random.randF( STOP_DISTANCE, STAY_DISTANCE )
...
void CStateMonsterControlledFollowAbstract::setup_substates() {
    ...
    if ( this->current_substate == eStateControlled_Follow_WalkToObject ) {
        ...
        if ( !this->object->control().path_builder().restrictions().accessible(
                 dest_pos ) ) {
