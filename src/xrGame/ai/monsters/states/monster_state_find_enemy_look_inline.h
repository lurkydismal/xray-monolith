...
CStateMonsterFindEnemyLookAbstract::CStateMonsterFindEnemyLook( _Object* obj )
    : inherited( obj ) {
    this->add_state( eStateFindEnemy_LookAround_MoveToPoint,
                     xr_new< CStateMonsterMoveToPoint< _Object > >( obj ) );
    this->add_state( eStateFindEnemy_LookAround_LookAround,
                     xr_new< CStateMonsterCustomAction< _Object > >( obj ) );
    this->add_state( eStateFindEnemy_LookAround_TurnToPoint,
...
void CStateMonsterFindEnemyLookAbstract::reselect_state() {
    if ( ( current_stage == 1 ) || ( current_stage == 3 ) ) {
        ...
        target_point.mad( start_position, current_dir,
                          Random.randF( 4.f, 5.f ) );
        this->select_state( ( Random.randI( 2 ) )
                                ? eStateFindEnemy_LookAround_MoveToPoint
