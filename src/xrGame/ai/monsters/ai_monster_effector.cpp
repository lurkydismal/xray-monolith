...
// CMonsterEffector
//////////////////////////////////////////////////////////////////////////
CMonsterEffector::CMonsterEffector( const SPPInfo& ppi,
                                    float life_time,
                                    float attack_time,
                                    float release_time,
                                    float spec_factor )
...
BOOL CMonsterEffector::Process( SPPInfo& pp ) {
    ...
    else if ( ( time_past_perc >= m_attack ) &&
...
CMonsterEffectorHit::CMonsterEffectorHit( float time,
                                          float amp,
                                          float periods,
                                          float power )
    ...
    offset.set( Random.randF( 1, 2 ), Random.randF( 1, 6 ),
                Random.randF( 1, 6 ) );
}
...
BOOL CMonsterEffectorHit::ProcessCam( SCamEffectorInfo& info ) {
    ...
    Fvector dangle;
    dangle.x = cur_amp / offset.x *
               _sin( period_all / offset.x * ( 1.0f - time_left_perc ) );
    dangle.y = cur_amp / offset.y *
               _cos( period_all / offset.y * ( 1.0f - time_left_perc ) );
    dangle.z = cur_amp / offset.z *
               _sin( period_all / offset.z * ( 1.0f - time_left_perc ) );
