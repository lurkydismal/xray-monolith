...
void CMonsterSquad::ProcessAttack() {
    ...
    for ( MEMBER_GOAL_MAP_IT it_goal = m_goals.begin();
          it_goal != m_goals.end(); it_goal++ ) {
    ...
    for ( ENEMY_MAP_IT it_enemy = m_enemy_map.begin();
          it_enemy != m_enemy_map.end(); ++it_enemy ) {
        ...
        // 		{
        // 			assign_monsters_target_dirs(it_enemy->second,
        // it_enemy->first);
        // 		}
        // 		else
        // 		{
        // 			Attack_AssignTargetDir(it_enemy->second,
        // it_enemy->first);
        // 		}
    }
}
...
void CMonsterSquad::set_rat_squad_index( const CEntity* m_enemy ) {
    ...
    for ( ENEMY_MAP_IT it_enemy = m_enemy_maps.begin();
          it_enemy != m_enemy_maps.end(); it_enemy++ ) {
        get_index_in_rat_squad( it_enemy->second, it_enemy->first );
    }
}

void CMonsterSquad::get_index_in_rat_squad( ENTITY_VEC& members,
                                            const CEntity* m_enemy ) {
    u8 m_index = 0;
...
void CMonsterSquad::get_index_in_squad( ENTITY_VEC& members,
                                        const CEntity* m_enemy ) {
    u8 m_index = 0;
    std::sort( members.begin(), members.end(), sort_predicate( m_enemy ) );
    while ( !members.empty() ) {
        ...
        pEntity = members.back();
        pEntity->cast_entity_alive()->m_squad_index = m_index;
        smart_cast< CBaseMonster& >( *pEntity )
            .SetEnemy( smart_cast< const CEntityAlive* >( m_enemy ) );
        members.pop_back();
    }
}
...
void CMonsterSquad::Attack_AssignTargetDir( ENTITY_VEC& members,
    ...
    while ( !members.empty() ) {
        ...
        if ( angle_normalize_signed( h2 - h1 ) > 0 ) {
            // right
            if ( ( next_right_yaw < PI ) &&
                 !fsimilar( next_right_yaw, PI, PI / 60.f ) )
                b_add_left = false;
            else
                b_add_left = true;
        } else {
            // left
            if ( ( next_left_yaw < PI ) &&
                 !fsimilar( next_left_yaw, PI, PI / 60.f ) )
                b_add_left = true;
            else
                b_add_left = false;
        }
...
Fvector CMonsterSquad::calc_monster_target_dir( CBaseMonster* monster,
                                                const CEntity* enemy ) {
    VERIFY( monster );
    VERIFY( enemy );
...
void CMonsterSquad::assign_monsters_target_dirs( ENTITY_VEC& members,
                                                 const CEntity* enemy ) {
