...
public:
    ...
    virtual void SetScriptControl( const bool bScriptControl,
    ...
    virtual void LookPosition( Fvector to_point,
                               float angular_speed = PI_DIV_3 );
    // каждый монстр может по-разному реализвать эту функ (e.g. кровосос с
    // поворотом головы и т.п.)
    ...
    bool GetCoverFromEnemy( const Fvector& enemy_pos,
                            Fvector& position,
                            u32& vertex_id );
    bool GetCoverFromPoint( const Fvector& pos,
                            Fvector& position,
                            u32& vertex_id,
                            float min_dist,
                            float max_dist,
                            float radius );
    bool GetCoverCloseToPoint( const Fvector& dest_pos,
                               float min_dist,
                               float max_dist,
                               float deviation,
                               float radius,
...
public:
    ...
    void settings_read( CInifile const* ini,
                        LPCSTR section,
...
public:
    ...
    void Hit_Wound( CObject* object,
                    float value,
                    const Fvector& dir,
                    float impulse );
    CParticlesObject* PlayParticles( const shared_str& name,
                                     const Fvector& position,
                                     const Fvector& dir,
                                     BOOL auto_remove = TRUE,
                                     BOOL xformed = TRUE );
    void load_effector( LPCSTR section,
                        LPCSTR line,
...
protected:
    ...
    void fill_bones_body_parts( LPCSTR body_part,
