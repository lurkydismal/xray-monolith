...
CBaseMonster::CBaseMonster()
    ...
    m_pPhysics_support = xr_new< CCharacterPhysicsSupport >(
        CCharacterPhysicsSupport::etBitting, this );
...
void CBaseMonster::update_pos_by_grouping_behaviour() {
    ...
    const float dt = 0.001f * ( Device.dwTimeGlobal -
                                m_last_grouping_behaviour_update_tick );
    ...
    u32 new_vertex = ai().level_graph().check_position_in_direction(
        old_vertex, old_pos, new_pos );
    ...
    new_vertex = ai().level_graph().check_position_in_direction(
        old_vertex, old_pos, new_pos );
...
bool accessible_epsilon( CBaseMonster* const object,
                         Fvector const pos,
                         float epsilon ) {
    ...
    for ( u32 i = 0; i < sizeof( offsets ) / sizeof( offsets[ 0 ] ); ++i ) {
        if ( object->movement().restrictions().accessible( pos +
                                                           offsets[ i ] ) )
            return true;
    }
...
static bool enemy_inaccessible( CBaseMonster* const object ) {
    ...
    Fvector const enemy_vert_pos = ai().level_graph().vertex_position(
...
bool CBaseMonster::enemy_accessible() {
    ...
    if ( EnemyMan.get_enemy() ) {
        u32 const enemy_vertex =
            EnemyMan.get_enemy()->ai_location().level_vertex_id();
...
void CBaseMonster::update_enemy_accessible_and_at_home_info() {
    ...
    if ( ::enemy_inaccessible( this ) ) {
        ...
        if ( m_last_tick_enemy_inaccessible &&
             Device.dwTimeGlobal - m_last_tick_enemy_inaccessible > 3000 ) {
            m_first_tick_enemy_inaccessible = 0;
            m_last_tick_enemy_inaccessible = 0;
        }
    }
...
void CBaseMonster::Hit( SHit* pHDS ) {
    ...
    if ( ai().script_engine().functor( "_G.CBaseMonster__BeforeHitCallback",
                                       funct ) ) {
...
bool CBaseMonster::useful( const CItemManager* manager,
    ...
    // because level_vertex_id is not valid, so this code syncs vertex_id with
    // position
    if ( !ai().level_graph().valid_vertex_id(
...
float CBaseMonster::evaluate( const CItemManager* manager,
                              const CGameObject* object ) const {
    return ( 0.f );
}
...
void CBaseMonster::SetTurnAnimation( bool turn_left ) {
    ( turn_left ) ? anim().SetCurAnim( eAnimStandTurnLeft )
                  : anim().SetCurAnim( eAnimStandTurnRight );
}
...
void CBaseMonster::set_state_sound( u32 type, bool once ) {
    if ( once ) {
        ...
        // handle situation, when monster want to play attack sound for the
        // first time
        if ( ( type == MonsterSound::eMonsterSoundAggressive ) &&
            ...
            u8 objects_count =
                monster_squad().get_squad( this )->get_count( this, 20.f );
            ...
            switch ( type ) {
                case MonsterSound::eMonsterSoundIdle:
                    ...
                    if ( Actor()->Position().distance_to( Position() ) >
                         db().m_fDistantIdleSndRange ) {
                        delay = u32( float( db().m_dwDistantIdleSndDelay ) *
                                     _sqrt( float( objects_count ) ) );
                        type = MonsterSound::eMonsterSoundIdleDistant;
                    } else {
                        delay = u32( float( db().m_dwIdleSndDelay ) *
                                     _sqrt( float( objects_count ) ) );
                    }

                    break;
                case MonsterSound::eMonsterSoundEat:
                    delay = u32( float( db().m_dwEatSndDelay ) *
                                 _sqrt( float( objects_count ) ) );
                    break;
...
CParticlesObject* CBaseMonster::PlayParticles( const shared_str& name,
                                               const Fvector& position,
                                               const Fvector& dir,
                                               BOOL auto_remove,
                                               BOOL xformed ) {
    CParticlesObject* ps =
        Particles::Details::Create( name.c_str(), auto_remove ).get();
    ...
    matrix.identity();
    matrix.k.set( dir );
    Fvector::generate_orthonormal_basis_normalized( matrix.k, matrix.j,
                                                    matrix.i );
...
void CBaseMonster::load_effector( LPCSTR section,
                                  LPCSTR line,
                                  SAttackEffector& effector ) {
    ...
    sscanf( pSettings->r_string( ppi_section, "color_base" ), "%f,%f,%f",
            &effector.ppi.color_base.r, &effector.ppi.color_base.g,
            &effector.ppi.color_base.b );
    sscanf( pSettings->r_string( ppi_section, "color_gray" ), "%f,%f,%f",
            &effector.ppi.color_gray.r, &effector.ppi.color_gray.g,
            &effector.ppi.color_gray.b );
    sscanf( pSettings->r_string( ppi_section, "color_add" ), "%f,%f,%f",
            &effector.ppi.color_add.r, &effector.ppi.color_add.g,
...
bool CBaseMonster::check_eated_corpse_draggable() {
    ...
    if ( IKinematics* K = p_corpse->Visual()->dcast_PKinematics() ) {
        if ( CInifile* ini = K->LL_UserData() ) {
            return ini->section_exist( "capture_used_bones" ) &&
                   ini->line_exist( "capture_used_bones", "bones" );
        }
    }
...
float CBaseMonster::get_attack_on_move_max_go_close_time() {
    return override_if_debug( "aom_max_go_close_time",
                              m_attack_on_move_params.max_go_close_time );
}

float CBaseMonster::get_attack_on_move_far_radius() {
    float radius = override_if_debug( "aom_far_radius",
                                      m_attack_on_move_params.far_radius );
    clamp( radius, 0.f, 100.f );
    return radius;
}

float CBaseMonster::get_attack_on_move_attack_radius() {
    return override_if_debug( "aom_attack_radius",
                              m_attack_on_move_params.attack_radius );
}

float CBaseMonster::get_attack_on_move_update_side_period() {
    return override_if_debug( "aom_update_side_period",
                              m_attack_on_move_params.update_side_period );
}

float CBaseMonster::get_attack_on_move_prediction_factor() {
    return override_if_debug( "aom_prediction_factor",
                              m_attack_on_move_params.prediction_factor );
}

float CBaseMonster::get_attack_on_move_prepare_radius() {
    return override_if_debug( "aom_prepare_radius",
                              m_attack_on_move_params.prepare_radius );
}

float CBaseMonster::get_attack_on_move_prepare_time() {
    return override_if_debug( "aom_prepare_time",
                              m_attack_on_move_params.prepare_time );
}
...
void CBaseMonster::update_eyes_visibility() {
    ...
    if ( !( left_eye_bone_id != u16( -1 ) &&
            right_eye_bone_id != u16( -1 ) ) ) {
        Msg( "%s, section %s, id %d left_eye_bone_id != u16(-1) && "
             "right_eye_bone_id != u16(-1), crash",
             cName().c_str(), cNameSect().c_str(), ID() );
        R_ASSERT( left_eye_bone_id != u16( -1 ) &&
                  right_eye_bone_id != u16( -1 ) );
    }

    bool eyes_visible =
        !g_Alive() || get_screen_space_coverage_diagonal() > 0.05f;
...
bool CBaseMonster::is_paused() const {
    ...
    xr_sprintf( id_paused_var_name, sizeof( id_paused_var_name ), "%d_paused",
                id );
