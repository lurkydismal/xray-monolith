...
void CBaseMonster::feel_sound_new( CObject* who,
                                   int eType,
    ...
    if ( !entity &&
         ( ( eType & SOUND_TYPE_ITEM_HIDING ) == SOUND_TYPE_ITEM_HIDING ) )
        return;

    if ( entity && ( !EnemyMan.is_enemy( entity ) ) ) {
        SoundMemory.check_help_sound( eType,
    ...
    if ( power >= db().m_fSoundThreshold ) {
        SoundMemory.HearSound( who, eType, Position, power,
                               Device.dwTimeGlobal );
    }
}
...
void CBaseMonster::HitEntity( const CEntity* pEntity,
                              float fDamage,
                              float impulse,
                              Fvector& dir,
    ...
    if ( EnemyMan.get_enemy() == pEntity ) {
        ...
        SHit HS;
        HS.GenHeader( GE_HIT,
                      pEntityNC->ID() ); //		u_EventGen
                                         //(l_P,GE_HIT, pEntityNC->ID());
        ...
        if ( pEntityNC == Actor() && draw_hit_marks ) {
            ...
            StaticDrawableWrapper* s =
                CurrentGameUI()->AddCustomStatic( "monster_claws", false );
            ...
            CEffectorCam* ce = Actor()->Cameras().GetCamEffector(
                ( ECamEffectorType )effBigMonsterHit );
            if ( !ce ) {
                const shared_str& eff_sect =
                    pSettings->r_string( cNameSect(), "actor_hit_effect" );
                if ( eff_sect.c_str() ) {
                    int id = -1;
                    ...
                    float ang_diff =
                        angle_difference( cam_dir.getH(), dir.getH() );
                    Fvector cp;
                    ...
                    AddEffector( Actor(), effBigMonsterHit, sect_name,
                                 fDamage );
                }
            }
            //////////////////////////////////////////////////////////////////////////
...
bool CBaseMonster::feel_vision_isRelevant( CObject* O ) {
    ...
    if ( ( O->SpatialComponent->spatial.type & STYPE_VISIBLEFORAI ) !=
         STYPE_VISIBLEFORAI )
        return false;
...
void CBaseMonster::HitSignal( float amount,
                              Fvector& vLocalDir,
                              CObject* who,
                              s16 element ) {
    if ( !g_Alive() )
    ...
    callback( GameObject::eHit )(
        lua_game_object(), amount, vLocalDir,
...
void CBaseMonster::Hit_Wound( CObject* object,
                              float value,
                              const Fvector& dir,
                              float impulse ) {
    NET_Packet P;
