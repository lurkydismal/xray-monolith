...
void CBaseMonster::GenerateNewOffsetFromLeader() {
    float const min_squad_offset =
        READ_IF_EXISTS( pSettings, r_float, "monsters_common",
                        "script_move_min_offset_from_leader", 3.f );
    float const max_squad_offset =
        READ_IF_EXISTS( pSettings, r_float, "monsters_common",
                        "script_move_max_offset_from_leader", 9.f );

    float const offset_magnitude =
        min_squad_offset +
        ( max_squad_offset - min_squad_offset ) * Random.randF( 1.f );
    Fvector offset = Fvector().set( offset_magnitude, 0, 0 );

    m_offset_from_leader =
        rotate_point( offset, deg2rad( 360.f ) * Random.randF( 1.f ) );
    m_offset_from_leader_chosen_tick = Device.dwTimeGlobal;
}

//////////////////////////////////////////////////////////////////////////

bool CBaseMonster::AssignGamePathIfNeeded( Fvector const target_pos,
                                           u32 const level_vertex ) {
    GameGraph::_GRAPH_ID const self_game_vertex =
    ...
    bool level_vertex_is_valid =
        ai().level_graph().valid_vertex_id( target_level_vertex );

    if ( !level_vertex_is_valid ) {
        if ( m_action_target_pos == target_pos &&
    ...
    if ( level_vertex_is_valid ) {
        ...
        GameGraph::_GRAPH_ID const target_game_vertex =
            ai().cross_table().vertex( target_level_vertex ).game_vertex_id();
        bool const game_vertex_is_valid =
            ai().game_graph().valid_vertex_id( target_game_vertex );
        ...
        if ( game_vertex_is_valid && self_game_vertex != target_game_vertex ) {
            ...
            control().path_builder().set_path_type(
...
bool CBaseMonster::bfAssignMovement( CScriptEntityAction* tpEntityAction ) {
    ...
    if ( control().path_builder().detail().time_path_built() >=
         tpEntityAction->m_tActionCondition.m_tStartTime ) {
        if ( ( l_tMovementAction.m_fDistToEnd > 0 ) &&
             control().path_builder().is_path_end(
                 l_tMovementAction.m_fDistToEnd ) ) {
            l_tMovementAction.m_bCompleted = true;
        }
        if ( control().path_builder().actual_all() &&
             control().path_builder().path_completed() ) {
            l_tMovementAction.m_bCompleted = true;
            return false;
        }
    }
    ...
    switch ( l_tMovementAction.m_tGoalType ) {
        ...
        case CScriptMovementAction::eGoalTypePatrolPath:
            ...
            control().path_builder().patrol().set_path(
                l_tMovementAction.m_path, l_tMovementAction.m_path_name );
            control().path_builder().patrol().set_start_type(
                l_tMovementAction.m_tPatrolPathStart );
            control().path_builder().patrol().set_route_type(
                l_tMovementAction.m_tPatrolPathStop );
            control().path_builder().patrol().set_random(
                l_tMovementAction.m_bRandom );
            if ( l_tMovementAction.m_previous_patrol_point != u32( -1 ) ) {
                control().path_builder().patrol().set_previous_point(
        ...
        case CScriptMovementAction::eGoalTypeFollowLeader: {
            ...
            bool const should_follow_leader =
                leader_id != ( ALife::_OBJECT_ID )( -1 ) && leader_id != ID();
            CCustomMonster* const leader =
                should_follow_leader
                    ? smart_cast< CCustomMonster* >(
                          Level().Objects.net_Find( leader_id ) )
                    : NULL;

            if ( !should_follow_leader || !leader ||
                 ( leader && !leader->GetScriptControl() ) ) {
                if ( AssignGamePathIfNeeded(
                         l_tMovementAction.m_tDestinationPosition,
                         invalid_vertex_id ) )
                    break;
                ...
                if ( Device.dwTimeGlobal >
                     m_offset_from_leader_chosen_tick + 5000 ) {
                ...
                for ( u32 tries = 0; tries < 3; ++tries ) {
                    vertex_id = ai().level_graph().check_position_in_direction(
                        leader->ai_location().level_vertex_id(), leader_pos,
                ...
                path().set_target_point( leader->Position() +
                                         m_offset_from_leader );
            }
...
bool CBaseMonster::bfAssignAnimation( CScriptEntityAction* tpEntityAction ) {
    ...
    switch ( l_tAnimAction.m_tAnimAction ) {
        case eAA_StandIdle:
            anim().m_tAction = ACT_STAND_IDLE;
            break;
        case eAA_CapturePrepare:
            anim().m_tAction = ACT_CAPTURE_PREPARE;
            break;
        case eAA_SitIdle:
...
bool CBaseMonster::bfAssignMonsterAction(
...
void CBaseMonster::ProcessScripts() {
    ...
    if ( psAI_Flags.test( aiMonsterDebug ) ) {
        ...
        DBG()
            .object_info( this, this )
            .add_item( "Under script", D3DCOLOR_XRGB( 255, 0, 0 ), 1 );
    } else {
...
CEntity* CBaseMonster::GetCurrentEnemy() {
    ...
    if ( EnemyMan.get_enemy() )
        enemy = const_cast< CEntity* >(
            static_cast< CEntity const* >( EnemyMan.get_enemy() ) );
...
CEntity* CBaseMonster::GetCurrentCorpse() {
    ...
    if ( CorpseMan.get_corpse() )
        corpse = const_cast< CEntity* >(
            smart_cast< const CEntity* >( CorpseMan.get_corpse() ) );
...
void CBaseMonster::SetScriptControl( const bool bScriptControl,
                                     shared_str caScriptName ) {
    if ( StateMan )
    ...
    if ( !m_bScriptControl && bScriptControl ) {
        ...
        if ( control().path_builder().path_type() !=
             MovementManager::ePathTypeGamePath ) {
