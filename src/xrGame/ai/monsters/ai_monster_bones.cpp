...
void bonesBone::Turn( u32 dt ) {
    ...
    float cur_speed =
        params.r_speed *
        _cos( PI_DIV_2m - PIm * _abs( params.target_yaw - params.cur_yaw ) /
                              params.dist_yaw );
...
void bonesManipulation::SetMotion( CBoneInstance* bone,
                                   u8 axis,
                                   float target_yaw,
                                   float r_speed,
                                   u32 t ) {
    ...
    m_Bones[ index ].params.dist_yaw =
        angle_difference( target_yaw, m_Bones[ index ].params.cur_yaw );
    if ( t > freeze_time )
        freeze_time = t;
...
void bonesManipulation::Update( CBoneInstance* bone, u32 cur_time ) {
    ...
    // если выполняется наращивание угла и ни одна кость не повернулась
    // (достигли таргета...)
    if ( !bones_were_turned && !in_return_state ) {
        ...
        if ( ( 0 != time_started ) &&
             ( time_started + freeze_time < cur_time ) ) {
            // время вышло?
            time_started = 0;
            ...
            for ( u32 i = 0; i < m_Bones.size(); ++i ) {
                ...
                m_Bones[ i ].params.dist_yaw =
                    _abs( m_Bones[ i ].params.target_yaw -
                          m_Bones[ i ].params.cur_yaw );
            }
            bActive = false;
        }
    }
...
bonesAxis& bonesManipulation::GetBoneParams( CBoneInstance* bone,
                                             u8 axis_used ) {
    // найти бону bone в m_Bones
    for ( u32 i = 0; i < m_Bones.size(); ++i ) {
        if ( ( m_Bones[ i ].bone == bone ) &&
             ( m_Bones[ i ].axis == axis_used ) ) {
