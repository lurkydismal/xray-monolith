...
void CControlRotationJump::activate() {
    ...
    float yaw = Fvector()
                    .sub( m_object->EnemyMan.get_enemy()->Position(),
                          m_object->Position() )
                    .getH();
...
void CControlRotationJump::on_release() {
    ...
    m_time_next_rotation_jump =
        Device.dwTimeGlobal +
        Random.randI( ROTATION_JUMP_DELAY_MIN, ROTATION_JUMP_DELAY_MAX );
}
...
bool CControlRotationJump::check_start_conditions() {
    ...
    SVelocityParam& velocity_run = m_object->move().get_velocity(
        MonsterMovement::eVelocityParameterRunNormal );
    if ( !fsimilar( m_man->movement().velocity_current(),
                    velocity_run.velocity.linear, START_SPEED_DELTA ) )
        return false;

    return true;
}

void CControlRotationJump::on_event( ControlCom::EEventType type,
                                     ControlCom::IEventData* dat ) {
    switch ( type ) {
        case ControlCom::eventAnimationEnd:
            if ( ( m_stage == eStop ) &&
                 ( m_data.flags.is( SControlRotationJumpData::eRotateOnce ) ==
                   FALSE ) )
                build_line_second();
            else
...
void CControlRotationJump::stop_at_once() {
    m_time = m_man->animation().motion_time(
        m_right_side ? m_data.anim_stop_rs : m_data.anim_stop_ls,
        m_object->Visual() );
    ...
    float target_yaw;
    if ( m_data.flags.is( SControlRotationJumpData::eRotateOnce ) &&
         m_object->EnemyMan.get_enemy() ) {
        // if rotate once so rotate to enemy
        Fvector dir_to_enemy;
        dir_to_enemy.sub( m_object->EnemyMan.get_enemy()->Position(),
                          m_object->Position() );
        dir_to_enemy.normalize();
        target_yaw = angle_normalize( -dir_to_enemy.getH() );
    } else {
        target_yaw = angle_normalize(
            -m_object->Direction().getH() +
            ( m_right_side ? m_data.turn_angle : -m_data.turn_angle ) );
    }
    ...
    ctrl_data_dir->heading.target_speed =
        angle_difference( cur_yaw, target_yaw ) / m_time;
    ...
    ctrl_data->global.set_motion( m_right_side ? m_data.anim_stop_rs
                                               : m_data.anim_stop_ls );
...
void CControlRotationJump::build_line_first() {
    ...
    // path distance
    m_dist = ( m_target_velocity * m_target_velocity -
               m_start_velocity * m_start_velocity ) /
             ( 2 * m_accel );
    ...
    u32 velocity_mask = MonsterMovement::eVelocityParameterStand |
                        MonsterMovement::eVelocityParameterRunNormal;
    m_stage = eStop;
...
void CControlRotationJump::build_line_second() {
    ...
    // get animation time
    m_time = m_man->animation().motion_time(
        m_right_side ? m_data.anim_run_rs : m_data.anim_run_ls,
        m_object->Visual() );
    ...
    if ( !m_man->build_path_line( this, target_position, u32( -1 ),
        ...
        ctrl_data->global.set_motion( m_right_side ? m_data.anim_run_rs
                                                   : m_data.anim_run_ls );
        ctrl_data->global.actual = false;
    }
}
