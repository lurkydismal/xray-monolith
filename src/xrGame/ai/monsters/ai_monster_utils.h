...
class CEntity;
extern Fvector get_valid_position( const CEntity* entity,
...
IC bool is_angle_between( float yaw, float yaw_from, float yaw_to ) {
    ...
    if ( ( angle_difference( yaw, yaw_from ) < diff ) &&
         ( angle_difference( yaw, yaw_to ) < diff ) )
        return true;
    else
        return false;
}
...
// LTX routines
//////////////////////////////////////////////////////////////////////////
IC void read_delay( LPCSTR section,
                    LPCSTR name,
                    u32& delay_min,
                    u32& delay_max ) {
...
IC void read_distance( LPCSTR section,
                       LPCSTR name,
                       float& dist_min,
                       float& dist_max ) {
