...
public:
    ...
    // manual setup
    void setup( CBaseMonster* object,
    ...
    virtual void evaluate_smart_cover( smart_cover::cover const* smart_cover,
                                       float const& weight );
};
...
CCoverEvaluator::CCoverEvaluator( CRestrictedObject* object )
    : inherited( object ) {
    m_object = NULL;
...
void CCoverEvaluator::setup( CBaseMonster* object,
                             const Fvector& position,
                             float min_pos_distance,
                             float max_pos_distance,
                             float deviation ) {
...
void CCoverEvaluator::evaluate_cover( const CCoverPoint* cover_point,
                                      float weight ) {
#ifdef DEBUG
    // DBG().level_info(this).add_item(cover_point->position(),
    ...
    float dest_distance =
        m_dest_position.distance_to( cover_point->position() );

    if ( ( dest_distance <= m_min_distance ) &&
         ( m_current_distance > dest_distance ) )
        return;

    if ( ( dest_distance >= m_max_distance ) &&
         ( m_current_distance < dest_distance ) )
        return;
...
void CCoverEvaluator::evaluate_smart_cover(
...
CMonsterCoverManager::CMonsterCoverManager( CBaseMonster* monster )
    : m_object( monster ) {
    m_ce_best = 0;
}
...
void CMonsterCoverManager::load() {
    m_ce_best = xr_new< CCoverEvaluator >(
        &( m_object->control().path_builder().restrictions() ) );
}

const CCoverPoint* CMonsterCoverManager::find_cover( const Fvector& position,
                                                     float min_pos_distance,
                                                     float max_pos_distance,
                                                     float deviation ) {
    m_ce_best->setup( m_object, position, min_pos_distance, max_pos_distance,
                      deviation );
    const CCoverPoint* point = ai().cover_manager().best_cover(
        m_object->Position(), 30.f, *m_ce_best );

    return point;
}

// найти лучший ковер относительно "position"
const CCoverPoint* CMonsterCoverManager::find_cover( const Fvector& src_pos,
                                                     const Fvector& dest_pos,
                                                     float min_pos_distance,
                                                     float max_pos_distance,
                                                     float deviation ) {
    m_ce_best->setup( m_object, dest_pos, min_pos_distance, max_pos_distance,
                      deviation );
    const CCoverPoint* point =
        ai().cover_manager().best_cover( src_pos, 30.f, *m_ce_best );
    return point;
}
...
void CMonsterCoverManager::less_cover_direction( Fvector& dir ) {
    float angle = ai().level_graph().vertex_high_cover_angle(
        m_object->ai_location().level_vertex_id(), deg( 10 ),
        std::greater< float >() );
    ...
    for ( float ang = angle; angle_difference( ang, angle ) < ANGLE_DISP;
          ang = angle_normalize( ang - ANGLE_DISP_STEP ) ) {
        direction.setHP( ang, 0.f );

        if ( Level().ObjectSpace.RayPick( trace_from, direction,
                                          TRACE_STATIC_DIST, collide::rqtStatic,
                                          l_rq, m_object ) ) {
    ...
    angle = angle_normalize( angle_from +
                             angle_difference( angle_from, angle_to ) / 2 );
    dir.setHP( angle, 0.f );
}
