...
void CControlManagerCustom::on_event( ControlCom::EEventType type,
...
//////////////////////////////////////////////////////////////////////////
void CControlManagerCustom::ta_fill_data( SAnimationTripleData& data,
                                          LPCSTR s1,
                                          LPCSTR s2,
                                          LPCSTR s3,
                                          bool execute_once,
    ...
    IKinematicsAnimated* skel_animated =
...
void CControlManagerCustom::ta_activate( const SAnimationTripleData& data ) {
    ...
    SAnimationTripleData* ctrl_data = ( SAnimationTripleData* )m_man->data(
        this, ControlCom::eControlTripleAnimation );
    VERIFY( ctrl_data );
...
bool CControlManagerCustom::ta_is_active( const SAnimationTripleData& data ) {
    ...
    return ( ( ctrl_data->pool[ 0 ] == data.pool[ 0 ] ) &&
             ( ctrl_data->pool[ 1 ] == data.pool[ 1 ] ) &&
             ( ctrl_data->pool[ 2 ] == data.pool[ 2 ] ) );
}
...
void CControlManagerCustom::seq_init() {
    ...
    SAnimationSequencerData* ctrl_data =
        ( SAnimationSequencerData* )m_man->data(
            this, ControlCom::eControlSequencer );
    if ( !ctrl_data )
        return;
...
void CControlManagerCustom::jump( CObject* obj, const SControlJumpData& ta ) {
    ...
    SControlJumpData* ctrl_data =
        ( SControlJumpData* )m_man->data( this, ControlCom::eControlJump );
    VERIFY( ctrl_data );
    ...
    ctrl_data->state_prepare_in_move.velocity_mask =
        ta.state_prepare_in_move.velocity_mask;
...
void CControlManagerCustom::load_jump_data( LPCSTR s1,
                                            LPCSTR s2,
                                            LPCSTR s3,
                                            LPCSTR s4,
    ...
    IKinematicsAnimated* skel_animated =
        smart_cast< IKinematicsAnimated* >( m_object->Visual() );
    if ( !skel_animated ) {
        return; // monster is dead, so no skeleton (early return due to bug:
                // 18755)
    }
    ...
    if ( s1 ) {
        m_jump->setup_data().state_prepare.motion =
            skel_animated->ID_Cycle_Safe( s1 );
        VERIFY( m_jump->setup_data().state_prepare.motion );
    } else
        m_jump->setup_data().state_prepare.motion.invalidate();

    if ( s2 ) {
        m_jump->setup_data().state_prepare_in_move.motion =
            skel_animated->ID_Cycle_Safe( s2 );
        VERIFY( m_jump->setup_data().state_prepare_in_move.motion );
        m_jump->setup_data().flags.assign_or(
            SControlJumpData::ePrepareInMove );
    } else
        m_jump->setup_data().state_prepare_in_move.motion.invalidate();

    m_jump->setup_data().state_glide.motion =
        skel_animated->ID_Cycle_Safe( s3 );
    VERIFY( m_jump->setup_data().state_glide.motion );

    if ( s4 ) {
        m_jump->setup_data().state_ground.motion =
            skel_animated->ID_Cycle_Safe( s4 );
        VERIFY( m_jump->setup_data().state_ground.motion );
    } else {
        m_jump->setup_data().state_ground.motion.invalidate();
        m_jump->setup_data().flags.assign_or( SControlJumpData::eGroundSkip );
    }

    if ( !s1 && !s2 ) {
        m_jump->setup_data().flags.assign_or( SControlJumpData::ePrepareSkip );
    }

    m_jump->setup_data().flags.assign_or(
        SControlJumpData::eGlidePlayAnimOnce );
    m_jump->setup_data().flags.assign_or(
        SControlJumpData::eGlideOnPrepareFailed );
...
void CControlManagerCustom::jump( const Fvector& position ) {
    ...
    ctrl_data->flags.assign_or( SControlJumpData::ePrepareSkip );
    ctrl_data->force_factor = -1.f;
...
void CControlManagerCustom::script_jump( const Fvector& position,
                                         float factor ) {
    if ( !m_man->check_start_conditions( ControlCom::eControlJump ) )
        return;
...
void CControlManagerCustom::check_attack_jump() {
    ...
    CEntityAlive* target =
        const_cast< CEntityAlive* >( m_object->EnemyMan.get_enemy() );
    if ( !m_jump->can_jump( target ) )
        return;

    if ( m_man->check_start_conditions( ControlCom::eControlJump ) ) {
        m_jump->setup_data().flags.set( SControlJumpData::ePrepareSkip, false );
        m_jump->setup_data().flags.set( SControlJumpData::eUseTargetPosition,
                                        false );
...
bool CControlManagerCustom::check_if_jump_possible( Fvector const& target,
...
bool CControlManagerCustom::jump_if_possible(
    ...
    bool const aggressive_jump =
        target_object ? m_object->can_use_agressive_jump( target_object )
                      : NULL;
    ...
    m_jump->setup_data().flags.set( SControlJumpData::eUseAutoAim,
                                    use_direction_to_target );
    m_jump->setup_data().flags.set( SControlJumpData::eUseTargetPosition,
                                    true );
    m_jump->setup_data().flags.set( SControlJumpData::eDontUseVelocityBounce,
                                    !use_velocity_bounce );
...
void CControlManagerCustom::check_jump_over_physics() {
    ...
    for ( u32 i = m_man->path_builder().detail().curr_travel_point_index();
        ...
        const DetailPathManager::STravelPathPoint& travel_point =
            m_man->path_builder().detail().path()[ i ];

        // получить список объектов вокруг врага
        m_nearest.clear_not_free();
        Level().ObjectSpace.GetNearest( m_nearest, travel_point.position,
                                        m_object->Radius(), NULL );

        for ( u32 k = 0; k < m_nearest.size(); k++ ) {
            CPhysicsShellHolder* obj =
                smart_cast< CPhysicsShellHolder* >( m_nearest[ k ] );
            if ( !obj || !obj->PPhysicsShell() ||
                 !obj->PPhysicsShell()->isActive() || ( obj->Radius() < 0.5f ) )
                continue;
            if ( m_object->Position().distance_to( obj->Position() ) <
                 MAX_DIST_SUM / 2 )
                continue;

            Fvector dir =
                Fvector().sub( travel_point.position, m_object->Position() );
            ...
            m_jump->setup_data().flags.set( SControlJumpData::ePrepareSkip,
                                            true );
...
void CControlManagerCustom::check_rotation_jump() {
    ...
    SControlRotationJumpData* ctrl_data =
        ( SControlRotationJumpData* )m_man->data(
...
void CControlManagerCustom::add_rotation_jump_data( LPCSTR left1,
                                                    LPCSTR left2,
                                                    LPCSTR right1,
                                                    LPCSTR right2,
                                                    float angle,
...
void CControlManagerCustom::check_threaten() {
    ...
    SControlThreatenData* ctrl_data = ( SControlThreatenData* )m_man->data(
        this, ControlCom::eControlThreaten );
    VERIFY( ctrl_data );
...
void CControlManagerCustom::check_melee_jump() {
    ...
    SControlMeleeJumpData* ctrl_data = ( SControlMeleeJumpData* )m_man->data(
        this, ControlCom::eControlMeleeJump );
    VERIFY( ctrl_data );
...
// Fill Rotation Data
//////////////////////////////////////////////////////////////////////////
void CControlManagerCustom::fill_rotation_data( SControlRotationJumpData& data,
                                                LPCSTR left1,
                                                LPCSTR left2,
                                                LPCSTR right1,
                                                LPCSTR right2,
...
void CControlManagerCustom::critical_wound( LPCSTR anim ) {
    ...
    SControlCriticalWoundData* ctrl_data =
        ( SControlCriticalWoundData* )m_man->data(
