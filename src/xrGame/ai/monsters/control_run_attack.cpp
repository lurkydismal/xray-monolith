...
void CControlRunAttack::activate() {
    ...
    ctrl_dir->heading.target_angle = m_man->direction().angle_to_target(
        m_object->EnemyMan.get_enemy()->Position() );
    ...
    ctrl_anim->global.set_motion(
        smart_cast< IKinematicsAnimated* >( m_object->Visual() )
            ->ID_Cycle_Safe( "stand_attack_run_0" ) );
...
bool CControlRunAttack::check_start_conditions() {
    ...
    if ( !fsimilar( m_man->movement().velocity_current(),
                    velocity_run.velocity.linear, 2.f ) )
        return false;
...
void CControlRunAttack::on_event( ControlCom::EEventType type,
                                  ControlCom::IEventData* dat ) {
    switch ( type ) {
        case ControlCom::eventAnimationEnd:
            m_time_next_attack =
                time() + Random.randI( m_min_delay, m_max_delay );
        ...
        case ControlCom::eventAnimationStart: // handle blend params
        {
            ...
            SVelocityParam& velocity =
                m_object->move().get_velocity( velocity_mask );

            // distance
            float path_dist = anim_time * velocity.velocity.linear;

            Fvector dir;
            dir.sub( m_object->EnemyMan.get_enemy()->Position(),
                     m_object->Position() );
            dir.normalize_safe();
            ...
            if ( !m_man->build_path_line(
                ...
                SControlMovementData* ctrl_move =
                    ( SControlMovementData* )m_man->data(
