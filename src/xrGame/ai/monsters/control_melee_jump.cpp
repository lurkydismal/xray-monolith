...
bool CControlMeleeJump::check_start_conditions() {
    ...
    if ( m_man->direction().is_face_target( enemy_position, CHECK_YAW ) )
        return false;
    if ( enemy_position.distance_to( m_object->Position() ) >
...
void CControlMeleeJump::activate() {
    ...
    MotionID motion =
        ( ( m_man->direction().is_from_right( target_yaw ) ) ? m_data.anim_rs
                                                             : m_data.anim_ls );
    float anim_time =
        m_man->animation().motion_time( motion, m_object->Visual() );
    ...
    ctrl_data_dir->heading.target_speed =
        angle_difference( cur_yaw, target_yaw ) / anim_time;
...
void CControlMeleeJump::on_event( ControlCom::EEventType type,
