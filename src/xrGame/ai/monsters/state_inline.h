...
void CStateAbstract::execute() {
    ...
    if ( current_substate == u32( -1 ) ) {
        ...
        if ( current_substate == u32( -1 ) ) {
            ...
            if ( CBaseMonster* p_monster =
                     smart_cast< CBaseMonster* >( object ) ) {
...
void CStateAbstract::add_debug_info( debug::text_tree& root_s ) {
    ...
    if ( !substates.size() ) {
        ...
        for ( SubStates::const_iterator i = substates.begin(),
                                        e = substates.end();
              i != e; ++i ) {
            TextTree& current_state_s =
                root_s.add_line( EMonsterState( ( *i ).first ) );
...
EMonsterState CStateAbstract::get_state_type() {
    ...
    return ( ( state == eStateUnknown ) ? EMonsterState( current_substate )
                                        : state );
}
...
TEMPLATE_SPECIALIZATION
bool CStateAbstract::check_control_start_conditions(
