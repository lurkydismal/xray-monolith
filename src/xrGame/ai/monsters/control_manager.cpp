...
char* make_xrstr( ControlCom::EControlType e ) {
    switch ( e ) {
        ...
        case ControlCom::eControlPath:
            return "eControlPath";
        case ControlCom::eControlDir:
            return "eControlDir";
        case ControlCom::eControlAnimation:
            return "eControlAnimation";
        case ControlCom::eControlSequencer:
            return "eControlSequencer";
        case ControlCom::eControlTripleAnimation:
            return "eControlTripleAnimation";
        case ControlCom::eControlJump:
            return "eControlJump";
        case ControlCom::eControlRotationJump:
            return "eControlRotationJump";
        case ControlCom::eControlRunAttack:
            return "eControlRunAttack";
        case ControlCom::eControlThreaten:
            return "eControlThreaten";
        case ControlCom::eControlMeleeJump:
            return "eControlMeleeJump";
        case ControlCom::eControlAnimationBase:
            return "eControlAnimationBase";
        case ControlCom::eControlMovementBase:
            return "eControlMovementBase";
        case ControlCom::eControlPathBase:
            return "eControlPathBase";
        case ControlCom::eControlDirBase:
            return "eControlDirBase";
        case ControlCom::eControlCustom:
            return "eControlCustom";
        case ControlCom::eComCustom1:
            return "eComCustom1";
        case ControlCom::eComCriticalWound:
            return "eComCriticalWound";
        case ControlCom::eControllersCount:
            return "eControllersCount";
        default:
            return "eControlInvalid";
    }
}
...
void CControl_Manager::load( LPCSTR section ) {
    ...
    for ( CONTROLLERS_MAP_IT it = m_control_elems.begin();
          it != m_control_elems.end(); ++it )
...
void CControl_Manager::update_frame() {
    ...
    for ( COM_VEC_IT it = m_active_elems.begin(); it != m_active_elems.end();
          ++it ) {
        // update coms
        if ( ( *it ) )
            ( *it )->update_frame();
    ...
    m_active_elems.erase(
        std::remove_if( m_active_elems.begin(), m_active_elems.end(),
                        predicate_remove() ),
        m_active_elems.end() );
}
...
void CControl_Manager::notify( ControlCom::EEventType event,
...
// Messaging
//////////////////////////////////////////////////////////////////////////
void CControl_Manager::subscribe( CControl_Com* com,
                                  ControlCom::EEventType type ) {
    m_listeners[ type ].push_back( com );
}

void CControl_Manager::unsubscribe( CControl_Com* com,
                                    ControlCom::EEventType type ) {
...
ControlCom::IComData* CControl_Manager::data( CControl_Com* who,
...
void CControl_Manager::set_base_controller( CControl_Com* com,
                                            ControlCom::EControlType type ) {
...
// capture
void CControl_Manager::capture( CControl_Com* com,
                                ControlCom::EControlType type ) // who, type
{
    CControl_Com* target = m_control_elems[ type ];
...
bool CControl_Manager::check_capturer( CControl_Com* com,
                                       ControlCom::EControlType type ) {
    CControl_Com* target = m_control_elems[ type ];
    CControl_Com* capturer = target->ced()->capturer();
    return ( capturer == com );
...
void CControl_Manager::release( CControl_Com* com,
                                ControlCom::EControlType type ) // who, type
{
    CControl_Com* target = m_control_elems[ type ];
    CControl_Com* capturer = target->ced()->capturer();
...
void CControl_Manager::lock( CControl_Com* com,
                             ControlCom::EControlType type ) {
...
void CControl_Manager::unlock( CControl_Com* com,
                               ControlCom::EControlType type ) {
    VERIFY( is_pure( m_control_elems[ type ] ) );
    VERIFY( m_control_elems[ type ]->ced()->capturer() == com );
...
void CControl_Manager::path_stop( CControl_Com* com ) {
    SControlPathBuilderData* ctrl_path =
        ( SControlPathBuilderData* )data( com, ControlCom::eControlPath );
    VERIFY( ctrl_path );
...
void CControl_Manager::move_stop( CControl_Com* com ) {
    SControlMovementData* ctrl_move =
        ( SControlMovementData* )data( com, ControlCom::eControlMovement );
    VERIFY( ctrl_move );
...
void CControl_Manager::dir_stop( CControl_Com* com ) {
    SControlDirectionData* ctrl_dir =
        ( SControlDirectionData* )data( com, ControlCom::eControlDir );
    VERIFY( ctrl_dir );
...
bool CControl_Manager::build_path_line( CControl_Com* com,
...
void CControl_Manager::check_active_com( CControl_Com* com, bool b_add ) {
    if ( b_add ) {
        if ( com->is_active() && !com->ced()->is_locked() ) {
            COM_VEC_IT it =
                std::find( m_active_elems.begin(), m_active_elems.end(), com );
...
void CControl_Manager::add_debug_info( debug::text_tree& root_s ) {
    u32 index = 0;
    for ( CONTROLLERS_MAP_IT it = m_control_elems.begin();
          it != m_control_elems.end(); ++it, ++index ) {
        if ( !it->second->is_inited() )
            continue;

        debug::text_tree& con_s =
            root_s.add_line( make_xrstr( it->first ), it->second->is_active() );

        if ( it->second->ced() ) {
            con_s.add_line(
                "Capturer",
                it->second->ced()->capturer()
