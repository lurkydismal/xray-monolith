...
void CScanningAbilityAbstract::load( LPCSTR section ) {
    ::Sound->create( sound_scan, pSettings->r_string( section, "scan_sound" ),
    ...
    velocity_threshold =
        pSettings->r_float( section, "scan_velocity_threshold" );
    decrease_value = pSettings->r_float( section, "scan_decrease_value" );
    scan_trace_time_freq =
        pSettings->r_float( section, "scan_trace_time_freq" );

    VERIFY( !fis_zero( scan_trace_time_freq ) );

    // load scan effector
    LPCSTR ppi_section =
        pSettings->r_string( section, "scan_effector_section" );
    ...
    m_effector_info.noise.intensity =
        pSettings->r_float( ppi_section, "noise_intensity" );
    m_effector_info.noise.grain =
    ...
    sscanf( pSettings->r_string( ppi_section, "color_base" ), "%f,%f,%f",
            &m_effector_info.color_base.r, &m_effector_info.color_base.g,
            &m_effector_info.color_base.b );
    sscanf( pSettings->r_string( ppi_section, "color_gray" ), "%f,%f,%f",
            &m_effector_info.color_gray.r, &m_effector_info.color_gray.g,
            &m_effector_info.color_gray.b );
    sscanf( pSettings->r_string( ppi_section, "color_add" ), "%f,%f,%f",
            &m_effector_info.color_add.r, &m_effector_info.color_add.g,
            &m_effector_info.color_add.b );
...
void CScanningAbilityAbstract::schedule_update() {
    ...
    if ( state == eStateNotActive ) {
        if ( scan_obj->Position().distance_to( object->Position() ) <
             scan_radius )
    ...
    if ( state == eStateScanning ) {
        ...
        if ( vel > velocity_threshold ) {
            ...
            if ( time_last_trace + u32( 1000 / scan_trace_time_freq ) <
                 Device.dwTimeGlobal ) {
            ...
            if ( sound_scan._feedback() )
                ...
                if ( object->can_scan ) {
                    ...
                    // постпроцесс
                    // TODO: make this postprocess with static check (only one
                    // for all scanners)
                    Actor()->Cameras().AddPPEffector(
                        xr_new< CMonsterEffector >(
                            m_effector_info, m_effector_time,
                            m_effector_time_attack, m_effector_time_release ) );
...
float CScanningAbilityAbstract::get_velocity( CObject* obj ) {
    ...
    return (
        actor->character_physics_support()->movement()->GetVelocityActual() );
}
