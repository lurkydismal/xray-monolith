...
BOOL CPPEffectorPsyDogAura::update() {
    ...
    if ( m_effector_state == eStatePermanent ) {
        ...
        m_factor = float( Device.dwTimeGlobal - m_time_state_started ) /
                   float( m_time_to_fade );
...
void CPsyDogAura::update_schedule() {
    ...
    CVisualMemoryManager::VISIBLES::const_iterator I =
        m_actor->memory().visual().objects().begin();
    CVisualMemoryManager::VISIBLES::const_iterator E =
        m_actor->memory().visual().objects().end();
    for ( ; I != E; ++I ) {
    ...
    for ( ; it != m_object->m_storage.end(); ++it ) {
        if ( ( *it )->EnemyMan.get_enemy() == m_actor )
            ...
            ENEMIES_MAP::const_iterator I =
                ( *it )->EnemyMemory.get_memory().begin();
            ENEMIES_MAP::const_iterator E =
                ( *it )->EnemyMemory.get_memory().end();
            for ( ; I != E; ++I ) {
                if ( I->first == m_actor ) {
                    m_time_phantom_saw_actor =
                        _max( m_time_phantom_saw_actor, I->second.time );
                }
            }
        }
    ...
    bool const close_to_actor =
        m_actor ? m_object->Position().distance_to( m_actor->Position() ) < 30
                : false;
    bool const need_be_active =
        ( ( m_time_actor_saw_phantom + 2000 > time() ) ||
          ( m_time_phantom_saw_actor + 10000 > time() ) ) &&
        close_to_actor;
    if ( active() ) {
        if ( !need_be_active ) {
