...
void CPsyDog::Load( LPCSTR section ) {
    ...
    m_min_phantoms_count =
        READ_IF_EXISTS( pSettings, r_u8, section, "Min_Phantoms_Count", 1 );
    ...
    if ( pSettings->line_exist( section, "Time_Phantom_Respawn" ) )
        m_time_phantom_respawn =
            pSettings->r_u32( section, "Time_Phantom_Respawn" );
    else
        m_time_phantom_respawn =
            pSettings->r_u32( section, "Time_Phantom_Appear" );
}
...
bool CPsyDog::spawn_phantom() {
    u32 node;
    if ( !control().path_builder().get_node_in_radius(
             ai_location().level_vertex_id(), 4, 8, 5, node ) )
        return false;

    // set id to created server object
    // Alundaio:
    LPCSTR phantomSection =
        READ_IF_EXISTS( pSettings, r_string, this->get_section(),
                        "phantom_section", "psy_dog_phantom" );
    CSE_Abstract* phantom = Level().spawn_item(
        phantomSection, ai().level_graph().vertex_position( node ), node,
        0xffff, true );
    // Alundaio: END
    CSE_ALifeMonsterBase* pSE_Monster =
        smart_cast< CSE_ALifeMonsterBase* >( phantom );
    VERIFY( pSE_Monster );
...
void CPsyDog::delete_all_phantoms() {
    for ( xr_vector< CPsyDogPhantom* >::iterator it = m_storage.begin();
          it != m_storage.end(); it++ )
...
BOOL CPsyDogPhantom::net_Spawn( CSE_Abstract* dc ) {
    ...
    CSE_ALifeMonsterBase* se_monster =
        smart_cast< CSE_ALifeMonsterBase* >( dc );
    ...
    // --------------------------------------------------------------------------------
    m_particles_appear =
        pSettings->r_string( *cNameSect(), "particles_appear" );
    m_particles_disappear =
        pSettings->r_string( *cNameSect(), "particles_disappear" );
...
void CPsyDogPhantom::Think() {
    ...
    // move().get_velocity(MonsterMovement::eVelocityParameterRunNormal); if
    // (control().movement().real_velocity() < 2*velocity_run.velocity.linear/3)
    // return;
    if ( EnemyMan.get_enemy() )
        if ( !control().direction().is_face_target( EnemyMan.get_enemy(),
                                                    PI_DIV_6 ) )
            return;
    ...
    u32 node = ai().level_graph().check_position_in_direction(
        ai_location().level_vertex_id(), Position(), target );
    control().path_builder().restrictions().remove_border();
    // if (!ai().level_graph().valid_vertex_id(node) ||
    // !control().path_builder().accessible(node)) return;

    if ( ai().level_graph().valid_vertex_id( node ) &&
         control().path_builder().accessible( node ) ) {
        target.y += 1.f;
        com_man().jump( target );
    }
    ...
    CParticlesPlayer::StartParticles( m_particles_appear,
    ...
    Actor()->Cameras().AddCamEffector( xr_new< CMonsterEffectorHit >(
        m_appear_effector.ce_time, m_appear_effector.ce_amplitude,
        m_appear_effector.ce_period_number, m_appear_effector.ce_power ) );
    Actor()->Cameras().AddPPEffector( xr_new< CMonsterEffector >(
        m_appear_effector.ppi, m_appear_effector.time,
        m_appear_effector.time_attack, m_appear_effector.time_release ) );
}

// void CPsyDogPhantom::Hit(float P,Fvector &dir,CObject*who,s16 element,Fvector
...
void CPsyDogPhantom::net_Destroy() {
    Fvector center;
    Center( center );
    PlayParticles( m_particles_disappear, center,
...
void CPsyDogPhantom::try_to_register_to_parent() {
    ...
    if ( obj ) {
        ...
        movement().restrictions().add_restrictions(
            m_parent->movement().restrictions().out_restrictions(),
