...
void CControlThreaten::activate() {
    ...
    m_man->animation().add_anim_event( skel->LL_MotionID( m_data.animation ),
                                       m_data.time,
...
void CControlThreaten::update_schedule() {
    ...
    if ( m_object->EnemyMan.get_enemy() ) {
        SControlDirectionData* ctrl_dir = ( SControlDirectionData* )m_man->data(
...
void CControlThreaten::on_event( ControlCom::EEventType type,
