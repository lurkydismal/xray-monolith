...
CAnomalyDetector::CAnomalyDetector( CBaseMonster* monster )
    : m_object( monster ) {}

CAnomalyDetector::~CAnomalyDetector() {}

void CAnomalyDetector::load( LPCSTR section ) {
    m_radius = READ_IF_EXISTS( pSettings, r_float, section,
                               "Anomaly_Detect_Radius", 15.f );
    m_time_to_rememeber = READ_IF_EXISTS(
        pSettings, r_u32, section, "Anomaly_Detect_Time_Remember", 30000 );
}
...
void CAnomalyDetector::update_schedule() {
    ...
    for ( ANOMALY_INFO_VEC_IT it = m_storage.begin(); it != m_storage.end();
          it++ ) {
    ...
    m_object->control().path_builder().restrictions().add_restrictions(
        temp_out_restrictors, temp_in_restrictors );
    ...
    m_object->control().path_builder().restrictions().remove_restrictions(
        temp_out_restrictors, temp_in_restrictors );

    // remove from storage
    m_storage.erase( std::remove_if( m_storage.begin(), m_storage.end(),
...
void CAnomalyDetector::on_contact( CObject* obj ) {
    ...
    if ( custom_zone->restrictor_type() ==
    ...
    ANOMALY_INFO_VEC_IT it =
        std::find( m_storage.begin(), m_storage.end(), custom_zone );
    if ( it != m_storage.end() )
        return;
