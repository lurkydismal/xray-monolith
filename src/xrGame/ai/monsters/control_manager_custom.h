...
class CControlManagerCustom : public CControl_ComBase {
    ...
    DEFINE_VECTOR( SControlRotationJumpData,
                   ROT_JUMP_DATA_VEC,
                   ROT_JUMP_DATA_VEC_IT );
...
public:
    ...
    void seq_switch(); // Перейти в следующее состояние, если такового не
                       // имеется - завершить
    ...
    void ta_pointbreak();
    void ta_fill_data( SAnimationTripleData& data,
                       LPCSTR s1,
                       LPCSTR s2,
                       LPCSTR s3,
                       bool execute_once,
                       bool skip_prep,
                       u32 capture_type = ControlCom::eCaptureDir |
    ...
    //-------------------------------------------------------------------------------
    // Rotation Jump
    void add_rotation_jump_data( LPCSTR left1,
                                 LPCSTR left2,
                                 LPCSTR right1,
                                 LPCSTR right2,
                                 float angle,
...
private:
    ...
    void fill_rotation_data( SControlRotationJumpData& data,
                             LPCSTR left1,
                             LPCSTR left2,
                             LPCSTR right1,
                             LPCSTR right2,
