...
void CControlAnimation::play() {
    ...
    if ( m_data.global.blend ) {
        if ( m_data.get_speed() > 0 ) {
            m_data.global.blend->speed =
                m_data.get_speed(); // TODO: make factor
        } else {
...
void CControlAnimation::play_part( SAnimationPart& part,
                                   PlayCallback callback ) {
    VERIFY( part.get_motion().valid() );

    u16 bone_or_part =
        m_skeleton_animated->LL_GetMotionDef( part.get_motion() )->bone_or_part;
    ...
    // m_object->Visual()->dcast_PKinematicsAnimated(); Msg
    // ("%6d Playing animation : %s , %s , Object %s",Device.dwTimeGlobal,
    // K->LL_MotionDefName_dbg(part.motion).first,K->LL_MotionDefName_dbg(part.motion).second,
    // *(m_object->cName()));
#endif

    part.blend = m_skeleton_animated->LL_PlayCycle(
        bone_or_part, part.get_motion(), TRUE, callback, this );
    ...
    if ( ( part.get_motion() != m_data.torso.get_motion() ) && part.blend )
        m_object->CStepManager::on_animation_start( part.get_motion(),
                                                    part.blend );

    ANIMATION_EVENT_MAP_IT it = m_anim_events.find( part.get_motion() );
    if ( it != m_anim_events.end() ) {
        for ( ANIMATION_EVENT_VEC_IT event_it = it->second.begin();
...
void CControlAnimation::add_anim_event( MotionID motion,
                                        float time_perc,
                                        u32 id ) {
    ...
    if ( it != m_anim_events.end() ) {
        ...
        for ( ANIMATION_EVENT_VEC_IT I = anim_vec.begin(); I != anim_vec.end();
              ++I ) {
...
void CControlAnimation::check_events( SAnimationPart& part ) {
    if ( part.get_motion().valid() && part.actual && part.blend ) {
        ...
        if ( it != m_anim_events.end() ) {
            float cur_perc =
                float( Device.dwTimeGlobal - part.time_started ) /
                ( ( part.blend->timeTotal / part.blend->speed ) * 1000 );
            ...
            for ( ANIMATION_EVENT_VEC_IT event_it = it->second.begin();
                ...
                if ( !event.handled && ( event.time_perc < cur_perc ) ) {
                    ...
                    // gen event
                    SAnimationSignalEventData anim_event(
                        part.get_motion(), event.time_perc, event.event_id );
                    m_man->notify( ControlCom::eventAnimationSignal,
                                   &anim_event );
                }
            }
        }
