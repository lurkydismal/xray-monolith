...
typedef struct tagSoundElement {
    ...
    IC void SetConvert( const CObject* who,
                        int eType,
    ...
    void CalcValue( TTime cur_time, const Fvector& cur_pos ) {
        value =
            FACTOR_SOUND_TYPE * u32( NONE_DANGEROUS_SOUND - WEAPON_SHOOTING ) -
            iFloor( FACTOR_DISTANCE * cur_pos.distance_to( position ) ) -
            FACTOR_DELTA_TIME * iFloor( float( ( cur_time - time ) / 1000 ) ) +
            FACTOR_SOUND_POWER * iFloor( power );
    }
} SoundElem;
...
public:
    ...
    void HearSound( const CObject* who,
                    int eType,
                    const Fvector& Position,
                    float power,
                    TTime time );
    ...
    void GetSound( SoundElem& s,
                   bool& bDangerous ); // возвращает самый опасный звук
    SoundElem& GetSound();
