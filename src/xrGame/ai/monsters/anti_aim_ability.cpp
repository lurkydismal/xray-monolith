...
static pcstr const s_anti_aim_detection_gain_speed_string =
    "anti_aim_detection_gain_speed";
static pcstr const s_anti_aim_detection_loose_speed_string =
    "anti_aim_detection_loose_speed";
} // namespace detail

anti_aim_ability::anti_aim_ability( CBaseMonster* const object )
    : m_object( object ) {
    m_effector_id = 0;
    m_max_angle = 0.5f;
...
void anti_aim_ability::load_from_ini( CInifile const* ini,
                                      pcstr const section ) {
    using namespace detail;

    m_timeout =
        READ_IF_EXISTS( ini, r_float, section, s_anti_aim_timeout_string, 5.f );
    m_freeze_time = READ_IF_EXISTS( ini, r_float, section,
                                    s_anti_aim_freeze_time_string, 1.f );
    m_max_angle = READ_IF_EXISTS( ini, r_float, section,
                                  s_anti_aim_max_angle_string, 0.5f );
    m_detection_gain_speed = READ_IF_EXISTS(
        ini, r_float, section, s_anti_aim_detection_gain_speed_string, 1.f );
    m_detection_loose_speed = READ_IF_EXISTS(
        ini, r_float, section, s_anti_aim_detection_loose_speed_string, 0.1f );
    pcstr effectors = READ_IF_EXISTS( ini, r_string, section,
                                      s_anti_aim_effectors_string, NULL );
...
void anti_aim_ability::activate() {
    ...
    m_animation_hit_tick = Device.dwTimeGlobal +
                           ( TTime )( m_object->anim().get_animation_hit_time(
                                          eAnimAntiAimAbility, 0 ) *
                                      1000 );

    MotionID motion;
    float anim_length;
    m_object->anim().get_animation_info( eAnimAntiAimAbility, 0, motion,
                                         anim_length );
    ...
    m_animation_end_tick =
        Device.dwTimeGlobal + ( TTime )( anim_length * 1000 );
...
void anti_aim_ability::start_camera_effector() {
    ...
    pcstr const effector_name =
        m_effectors[ rand() % m_effectors.size() ].c_str();
    ...
    if ( pSettings->line_exist( effector_name, "cam_eff_hud_affect" ) ) {
        cam_eff->SetHudAffect(
            !!pSettings->r_bool( effector_name, "cam_eff_hud_affect" ) );
    }

    LPCSTR fn = pSettings->r_string( effector_name, "cam_eff_name" );
    cam_eff->Start( fn );

    m_camera_effector_end_tick =
        Device.dwTimeGlobal + ( TTime )( cam_eff->GetAnimatorLength() * 1000 );
    m_camera_effector_end_tick =
        _max( m_camera_effector_end_tick, m_animation_end_tick );
...
void anti_aim_ability::do_deactivate() {
    if ( is_active() ) {
        if ( m_object->Visual() ) // when m_object is destroying, visual might
                                  // be dead already
        {
...
void anti_aim_ability::deactivate() {
    ...
    if ( Actor() ) {
        Actor()->Cameras().RemoveCamEffector(
            ( ECamEffectorType )m_effector_id );
    }
...
float anti_aim_ability::calculate_angle() const {
    ...
    float const max_deviation =
        angle_between_vectors( to_monster_center, to_monster_head );
    float const deviation =
        angle_between_vectors( to_monster_center, self_dir );
...
void anti_aim_ability::update_schedule() {
    ...
    if ( Device.dwTimeGlobal <
         m_last_activated_tick + ( TTime )( m_timeout * 1000 ) ) {
#ifdef DEBUG_STATE
    ...
    float const detect_delta =
        ( Device.dwTimeGlobal - m_last_detection_tick ) / 1000.f;
    m_last_detection_tick = Device.dwTimeGlobal;

    float const angle = calculate_angle();
    float const average_angle =
        _min( m_max_angle, ( angle + m_last_angle ) / 2 );
    float const relative_angle = ( m_max_angle - average_angle ) / m_max_angle;
    float const detect_speed =
        can_detect() ? _sqr( relative_angle ) * m_detection_gain_speed : 0;

    m_detection_level +=
        ( detect_speed - m_detection_loose_speed ) * detect_delta;
