...
void CMonsterEnemyManager::update()
{
	...
	if ((prev_enemy == enemy) && (time_last_seen != Device.dwTimeGlobal)) flags.assign_or(FLAG_ENEMY_LOST_SIGHT);
	if (prev_enemy && !prev_enemy->g_Alive()) flags.assign_or(FLAG_ENEMY_DIE);
	if (!enemy_see_me) flags.assign_or(FLAG_ENEMY_DOESNT_SEE_ME);
	...
	if (prev_enemy == enemy)
	{
		...
		if (_abs(dist_now - dist_prev) < 0.2f) flags.assign_or(FLAG_ENEMY_STANDING);
		else
		{
			if (dist_now < dist_prev) flags.assign_or(FLAG_ENEMY_GO_CLOSER);
			else flags.assign_or(FLAG_ENEMY_GO_FARTHER);

			if (_abs(dist_now - dist_prev) < 1.2f)
			{
				if (dist_now < dist_prev) flags.assign_or(FLAG_ENEMY_GO_CLOSER_FAST);
				else flags.assign_or(FLAG_ENEMY_GO_FARTHER_FAST);
			}
		}

		if (flags.is(FLAG_ENEMY_STANDING) && flags.is(FLAG_ENEMY_DOESNT_SEE_ME)) flags.assign_or(
			FLAG_ENEMY_DOESNT_KNOW_ABOUT_ME);
	}
	else flags.assign_or(FLAG_ENEMY_STATS_NOT_READY);
