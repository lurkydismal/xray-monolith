...
void CMonsterEnemyManager::update() {
    if ( m_script_enemy &&
         ( m_script_enemy->getDestroy() || !m_script_enemy->g_Alive() ) ) {
        script_enemy();
    }
    if ( forced ) {
    ...
    switch ( dwfChooseAction( 0, monster->panic_threshold(), 0.f, 0.f, 0.f,
                              monster->g_Team(), monster->g_Squad(),
                              monster->g_Group(), 0, 1, 2, 3, 4, monster,
                              30.f ) ) {
        case 4:
        case 3:
        case 2:
    ...
    if ( ( prev_enemy == enemy ) && ( time_last_seen != Device.dwTimeGlobal ) )
        flags.assign_or( FLAG_ENEMY_LOST_SIGHT );
    if ( prev_enemy && !prev_enemy->g_Alive() )
        flags.assign_or( FLAG_ENEMY_DIE );
    if ( !enemy_see_me )
        flags.assign_or( FLAG_ENEMY_DOESNT_SEE_ME );
    ...
    if ( prev_enemy == enemy ) {
        ...
        if ( _abs( dist_now - dist_prev ) < 0.2f )
            flags.assign_or( FLAG_ENEMY_STANDING );
        else {
            if ( dist_now < dist_prev )
                flags.assign_or( FLAG_ENEMY_GO_CLOSER );
            else
                flags.assign_or( FLAG_ENEMY_GO_FARTHER );

            if ( _abs( dist_now - dist_prev ) < 1.2f ) {
                if ( dist_now < dist_prev )
                    flags.assign_or( FLAG_ENEMY_GO_CLOSER_FAST );
                else
                    flags.assign_or( FLAG_ENEMY_GO_FARTHER_FAST );
            }
        }

        if ( flags.is( FLAG_ENEMY_STANDING ) &&
             flags.is( FLAG_ENEMY_DOESNT_SEE_ME ) )
            flags.assign_or( FLAG_ENEMY_DOESNT_KNOW_ABOUT_ME );
    } else
        flags.assign_or( FLAG_ENEMY_STATS_NOT_READY );
...
bool CMonsterEnemyManager::enemy_see_me_now() {
    if ( Actor() == enemy ) {
        ...
        CCustomMonster* cm =
            const_cast< CEntityAlive* >( enemy )->cast_custom_monster();
        if ( cm ) {
...
bool CMonsterEnemyManager::is_faced( const CEntityAlive* object0,
                                     const CEntityAlive* object1 ) {
    if ( object0->Position().distance_to( object1->Position() ) >
         object0->ffGetRange() ) {
        return false;
    }
    ...
    fYawFov = angle_normalize_signed(
        ( _abs( fYawFov ) + _abs( atanf( 1.f / tPosition.distance_to(
                                                   object1->Position() ) ) ) ) /
        2.f );
    ...
    if ( ( angle_difference( yaw1, yaw2 ) <= fYawFov ) &&
         ( angle_difference( pitch1, pitch2 ) <= fPitchFov ) )
        return ( true );
    return ( false );
}

bool CMonsterEnemyManager::is_enemy( const CEntityAlive* obj ) {
    return ( ( monster->g_Team() != obj->g_Team() ) &&
             monster->is_relation_enemy( obj ) && obj->g_Alive() );
}
...
u32 CMonsterEnemyManager::see_enemy_duration() {
    return ( ( m_time_start_see_enemy == 0 )
                 ? 0
                 : ( time() - m_time_start_see_enemy ) );
}
