...
class CControlAnimation : public CControl_ComPure< SControlAnimationData > {
    ...
    DEFINE_VECTOR( SAnimationEvent,
                   ANIMATION_EVENT_VEC,
                   ANIMATION_EVENT_VEC_IT );
    DEFINE_MAP( MotionID,
                ANIMATION_EVENT_VEC,
                ANIMATION_EVENT_MAP,
                ANIMATION_EVENT_MAP_IT );
...
public:
    enum EAnimationEventType { eAnimationHit = u32( 0 ), eAnimationCustom };
};

// get motion time, when just MotionID available
IC float CControlAnimation::motion_time( MotionID motion_id,
