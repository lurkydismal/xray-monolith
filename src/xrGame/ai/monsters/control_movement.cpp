...
void CControlMovement::update_frame() {
    velocity_lerp( m_velocity_current, m_data.velocity_target, m_data.acc,
...
float CControlMovement::real_velocity() {
    CPHMovementControl* movement_control =
        m_object->character_physics_support()->movement();
    ...
    if ( movement_control->IsCharacterEnabled() ) {
        ...
        VERIFY2(
            _abs( tmp ) < 1000,
            "movement_control->GetXZActVelInGoingDir() returns too big speed" );
        return tmp;
    }
