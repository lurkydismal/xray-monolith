...
char* dbg_action_name_table[] = {
    "ACT_STAND_IDLE",  "ACT_SIT_IDLE", "ACT_LIE_IDLE", "ACT_WALK_FWD",
...
void CControlAnimationBase::reinit() {
    ...
    AA_reload(
        pSettings->r_string( *( m_object->cNameSect() ), "attack_params" ) );
...
void CControlAnimationBase::on_event( ControlCom::EEventType type,
    ...
    switch ( type ) {
        ...
        case ControlCom::eventAnimationSignal: {
            SAnimationSignalEventData* event_data =
                ( SAnimationSignalEventData* )data;
...
float CControlAnimationBase::get_animation_length( EMotionAnim anim,
                                                   u32 index ) const {
    MotionID motion;
    float length;
...
bool CControlAnimationBase::get_animation_info( EMotionAnim anim,
                                                u32 index,
                                                MotionID& motion,
                                                float& length ) const {
    ...
    STRCONCAT( animation_name_buffer, anim_it->target_name,
               itoa( index, index_string_buffer, 10 ) );

    IKinematicsAnimated* animated =
...
float CControlAnimationBase::get_animation_hit_time( EMotionAnim anim,
                                                     u32 index ) const {
...
u32 CControlAnimationBase::get_animation_variants_count(
    EMotionAnim anim ) const {
    SAnimItem* anim_it = m_anim_storage[ anim ];
    VERIFY( anim_it );
...
void CControlAnimationBase::select_animation( bool anim_end ) {
    ...
    MotionID cur_anim =
        smart_cast< IKinematicsAnimated* >( m_object->Visual() )
            ->ID_Cycle_Safe( strconcat( sizeof( s2 ), s2, *anim_it->target_name,
                                        itoa( index, s1, 10 ) ) );
    if ( !cur_anim.valid() )
        FATAL( s2 );
...
bool CControlAnimationBase::CheckTransition( EMotionAnim from,
                                             EMotionAnim to ) {
    ...
    while ( !bVectEmpty ) {
        ...
        bool from_is_good =
            ( ( I->from.state_used ) ? ( I->from.state == state_from )
                                     : ( I->from.anim == cur_from ) );
        bool target_is_good =
            ( ( I->target.state_used ) ? ( I->target.state == state_to )
                                       : ( I->target.anim == to ) );
...
void CControlAnimationBase::CheckReplacedAnim() {
    for ( REPLACED_ANIM_IT it = m_tReplacedAnims.begin();
          m_tReplacedAnims.end() != it; ++it )
        if ( ( cur_anim_info().get_motion() == it->cur_anim ) &&
             ( *( it->flag ) == true ) ) {
...
SAAParam& CControlAnimationBase::AA_GetParams( LPCSTR anim_name ) {
    ...
    MotionID motion = smart_cast< IKinematicsAnimated* >( m_object->Visual() )
                          ->LL_MotionID( anim_name );

    for ( AA_VECTOR_IT it = m_attack_anims.begin(); it != m_attack_anims.end();
          it++ ) {
        if ( it->motion == motion )
            return ( *it );
    }
    ...
    VERIFY3( FALSE,
             "Error! No animation in AA_VECTOR! Animation = ", anim_name );
...
void CControlAnimationBase::FX_Play( EHitSide side, float amount ) {
    if ( fx_time_last_play + FX_CAN_PLAY_MIN_INTERVAL >
         m_object->m_dwCurrentTime )
        return;
...
EAction CControlAnimationBase::VelocityIndex2Action( u32 velocity_index ) {
    switch ( velocity_index ) {
        ...
        case MonsterMovement::eVelocityParameterDrag:
            return ACT_DRAG;
        case MonsterMovement::eVelocityParameterWalkSmelling:
            return ACT_HOME_WALK_SMELLING;
        case MonsterMovement::eVelocityParameterWalkGrowl:
            return ACT_HOME_WALK_GROWL;
            // jump
            // case MonsterMovement::eBloodsuckerVelocityParameterJumpGround:
            // return ACT_JUMP;
...
EAction CControlAnimationBase::GetActionFromPath() {
    ...
    u32 cur_point_velocity_index =
        m_object->movement()
            .detail()
            .path()[ m_object->movement().detail().curr_travel_point_index() ]
            .velocity;
    action = VelocityIndex2Action( cur_point_velocity_index );

    u32 next_point_velocity_index = u32( -1 );
    if ( m_object->movement().detail().path().size() >
         m_object->movement().detail().curr_travel_point_index() + 1 )
        next_point_velocity_index =
            m_object->movement()
                .detail()
                .path()
                    [ m_object->movement().detail().curr_travel_point_index() +
                      1 ]
                .velocity;

    if ( ( cur_point_velocity_index ==
...
void CControlAnimationBase::ValidateAnimation() {
    ...
    bool is_moving_on_path =
        m_object->control().path_builder().is_moving_on_path();

    if ( is_moving_on_path && is_moving_anim ) {
        m_object->dir().use_path_direction( cur_anim_info().get_motion() ==
                                            eAnimDragCorpse );
        return;
    }
...
void CControlAnimationBase::UpdateAnimCount() {
    IKinematicsAnimated* skel =
        smart_cast< IKinematicsAnimated* >( m_object->Visual() );
    xr_vector< u32 > subjectsToDelete;

    for ( ANIM_ITEM_VECTOR_IT it = m_anim_storage.begin();
          it != m_anim_storage.end(); it++ ) {
        if ( !( *it ) )
            continue;
        ...
        for ( int i = 0;; ++i ) {
            strconcat( sizeof( s_temp ), s_temp, *( ( *it )->target_name ),
                       itoa( i, s, 10 ) );
            LPCSTR name = s_temp;
        ...
        if ( count != 0 ) {
            ...
            subjectsToDelete.push_back(
                std::distance( m_anim_storage.begin(), it ) );
        } else {
            xr_sprintf( s, "Error! No animation: %s for monster %s",
                        *( ( *it )->target_name ), *m_object->cName() );
            R_ASSERT2( count != 0, s );
        }
    }
    for ( u32 idx : subjectsToDelete ) {
        ...
        while ( true ) {
            auto it = std::find_if(
                m_tReplacedAnims.begin(), m_tReplacedAnims.end(),
                [ idx ]( const SReplacedAnim& ranim ) {
...
CMotionDef* CControlAnimationBase::get_motion_def( SAnimItem* it, u32 index ) {
    ...
    IKinematicsAnimated* skeleton_animated =
        smart_cast< IKinematicsAnimated* >( m_object->Visual() );
    const MotionID& motion_id = skeleton_animated->ID_Cycle_Safe( strconcat(
        sizeof( s2 ), s2, *it->target_name, itoa( index, s1, 10 ) ) );
    return ( skeleton_animated->LL_GetMotionDef( motion_id ) );
}

void CControlAnimationBase::AddAnimTranslation( const MotionID& motion,
                                                LPCSTR str ) {
...
void CControlAnimationBase::check_hit( MotionID motion, float time_perc ) {
    ...
    if ( should_hit )
        m_object->HitEntity( enemy, params.hit_power, params.impulse,
                             params.impulse_dir );
...
void CControlAnimationBase::AA_reload( LPCSTR section ) {
    ...
    for ( u32 i = 0; pSettings->r_line( section, i, &anim_name, &val ); ++i ) {
        ...
        if ( _GetItemCount( val ) == 1 ) {
            ...
            for ( u32 k = 0; pSettings->r_line( compound_section, k,
                                                &unused_line_name, &val );
                  ++k ) {
                parse_anim_params( val, anim );

                m_attack_anims.push_back( anim );
                m_man->animation().add_anim_event(
                    anim.motion, anim.time, CControlAnimation::eAnimationHit );
            }
        } else {
            parse_anim_params( val, anim );
