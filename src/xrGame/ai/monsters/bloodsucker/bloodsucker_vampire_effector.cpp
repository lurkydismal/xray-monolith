...
BOOL CVampirePPEffector::Process( SPPInfo& pp ) {
    ...
    if ( time_past_perc < TIME_ATTACK ) {
        ...
        float time_past_sine_perc = ( time_past_perc - TIME_ATTACK ) *
                                    ( 1 / ( 1 - TIME_ATTACK + TIME_ATTACK ) );
...
CVampireCameraEffector::CVampireCameraEffector( float time,
                                                const Fvector& src,
                                                const Fvector& tgt )
    ...
    dangle_target.set( Random.randFs( DELTA_ANGLE_X ),
                       Random.randFs( DELTA_ANGLE_Y ),
                       Random.randFs( DELTA_ANGLE_Z ) );
...
BOOL CVampireCameraEffector::ProcessCam( SCamEffectorInfo& info ) {
    ...
    float cur_dist =
        m_dist * ( _sqrt( 0.5f * 0.5f -
                          ( time_passed - 0.5f ) * ( time_passed - 0.5f ) ) );
    ...
    if ( time_left_perc < 0.2f ) {
        ...
        angle_lerp( dangle_current.x, dangle_target.x,
                    _abs( dangle_current.x / fLifeTime + 0.001f ),
                    Device.fTimeDelta );
        angle_lerp( dangle_current.y, dangle_target.y,
                    _abs( dangle_current.y / fLifeTime + 0.001f ),
                    Device.fTimeDelta );
        angle_lerp( dangle_current.z, dangle_target.z,
                    _abs( dangle_current.z / fLifeTime + 0.001f ),
                    Device.fTimeDelta );
    } else {
        if ( angle_lerp( dangle_current.x, dangle_target.x, ANGLE_SPEED,
                         Device.fTimeDelta ) ) {
            dangle_target.x = Random.randFs( DELTA_ANGLE_X );
        }

        if ( angle_lerp( dangle_current.y, dangle_target.y, ANGLE_SPEED,
                         Device.fTimeDelta ) ) {
            dangle_target.y = Random.randFs( DELTA_ANGLE_Y );
        }

        if ( angle_lerp( dangle_current.z, dangle_target.z, ANGLE_SPEED,
                         Device.fTimeDelta ) ) {
