...
CAlienEffector::CAlienEffector( ECamEffectorType type, CAI_Bloodsucker* obj )
    ...
    dangle_target.set( angle_normalize( Random.randFs( DELTA_ANGLE_X ) ),
    ...
    Fvector::generate_orthonormal_basis(
        m_prev_eye_matrix.k, m_prev_eye_matrix.j, m_prev_eye_matrix.i );
    m_inertion = 1.f;
...
BOOL CAlienEffector::ProcessCam( SCamEffectorInfo& info ) {
    ...
    float rel_dist =
        m_prev_eye_matrix.c.distance_to( cur_matrix.c ) / MAX_CAMERA_DIST;
...
void CBloodsuckerAlien::activate() {
    ...
    // Start effector
    m_effector_pp = xr_new< CAlienEffectorPP >(
        m_object->pp_vampire_effector, EFFECTOR_ID_GEN( EEffectorPPType ) );
