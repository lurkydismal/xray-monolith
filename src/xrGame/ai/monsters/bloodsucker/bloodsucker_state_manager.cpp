...
CStateManagerBloodsucker::CStateManagerBloodsucker( CAI_Bloodsucker* monster )
    : inherited( monster ) {
    add_state(
        eStateCustom,
        xr_new< CStateCaptureJumpBloodsucker< CAI_Bloodsucker > >( monster ) );
    add_state( eStateRest,
               xr_new< CStateMonsterRest< CAI_Bloodsucker > >( monster ) );
    add_state( eStatePanic,
               xr_new< CStateMonsterPanic< CAI_Bloodsucker > >( monster ) );

    add_state( eStateAttack,
               xr_new< CStateMonsterAttack< CAI_Bloodsucker > >( monster ) );
    // add_state(eStateAttack,
    // xr_new<CBloodsuckerStateAttack<CAI_Bloodsucker> >
    // (monster));

    add_state( eStateEat,
               xr_new< CStateMonsterEat< CAI_Bloodsucker > >( monster ) );
    add_state( eStateHearInterestingSound,
               xr_new< CStateMonsterHearInterestingSound< CAI_Bloodsucker > >(
                   monster ) );
    add_state( eStateHearDangerousSound,
               xr_new< CStateMonsterHearDangerousSound< CAI_Bloodsucker > >(
                   monster ) );
    add_state( eStateHitted,
               xr_new< CStateMonsterHitted< CAI_Bloodsucker > >( monster ) );
    add_state( eStateVampire_Execute,
               xr_new< CStateBloodsuckerVampireExecute< CAI_Bloodsucker > >(
                   monster ) );
}
...
void CStateManagerBloodsucker::drag_object() {
    ...
    IKinematics* const kinematics =
        smart_cast< IKinematics* >( ph_obj->Visual() );
    if ( !kinematics ) {
        return;
    }
    ...
    IPHCapture* const capture =
        object->character_physics_support()->movement()->PHCapture();
...
void CStateManagerBloodsucker::execute() {
    ...
    if ( !object->is_drag_anim_jump() && !object->is_animated() ) {
        ...
        // 	if ( (prev_substate != eStateHearInterestingSound) && (state_id
