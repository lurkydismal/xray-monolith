...
CStateBloodsuckerVampireAbstract::CStateBloodsuckerVampire( _Object* obj )
    : inherited( obj ) {
    add_state( eStateVampire_ApproachEnemy,
               xr_new< CStateBloodsuckerVampireApproach< _Object > >( obj ) );
    add_state( eStateVampire_Execute,
               xr_new< CStateBloodsuckerVampireExecute< _Object > >( obj ) );
    add_state( eStateVampire_RunAway,
               xr_new< CStateMonsterHideFromPoint< _Object > >( obj ) );
    add_state( eStateVampire_Hide,
               xr_new< CStateBloodsuckerVampireHide< _Object > >( obj ) );
}
...
bool CStateBloodsuckerVampireAbstract::check_start_conditions() {
    ...
    if ( CAI_Bloodsucker::m_time_last_vampire +
             this->object->m_vampire_min_delay >
         Device.dwTimeGlobal )
        return false;
...
bool CStateBloodsuckerVampireAbstract::check_completion() {
    ...
    if ( ( this->current_substate == eStateVampire_Hide ) &&
         this->get_state_current()->check_completion() )
        return true;

    // если враг изменился
    if ( enemy != this->object->EnemyMan.get_enemy() )
        return true;

    // если актера уже контролит другой кровосос
    if ( ( this->current_substate != eStateVampire_Execute ) &&
         this->object->CControlledActor::is_controlling() )
        return true;
