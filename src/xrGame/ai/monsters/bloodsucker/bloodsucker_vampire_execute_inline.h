...
#define CStateBloodsuckerVampireExecuteAbstract \
    CStateBloodsuckerVampireExecute< _Object >
...
void CStateBloodsuckerVampireExecuteAbstract::execute() {
    if ( !this->object->CControlledActor::is_turning() &&
    ...
    if ( angle_between_vectors( this->object->Direction(), enemy_to_self ) <
             deg2rad( 20.f ) &&
...
bool CStateBloodsuckerVampireExecuteAbstract::check_start_conditions() {
    ...
    u32 const vertex_id = ai().level_graph().check_position_in_direction(
        this->object->ai_location().level_vertex_id(), this->object->Position(),
        enemy->Position() );
    ...
    if ( !this->object->control().direction().is_face_target( enemy,
                                                              PI_DIV_2 ) )
        return false;
