...
CBloodsuckerStateAttackAbstract::CBloodsuckerStateAttack( _Object* obj )
    : inherited_attack( obj ) {
    add_state( eStateAttack_Hide,
               xr_new< CStateMonsterBackstubEnemy< _Object > >( obj ) );
    add_state( eStateVampire_Execute,
               xr_new< CStateBloodsuckerVampireExecute< _Object > >( obj ) );
}
    ...
    if ( this->check_home_point() )
        ...
        if ( !b_melee && ( this->prev_substate == eStateAttack_Melee ) ) {
            ...
            if ( b_melee ) {
                ...
                // [TODO] make specific state and replace run_away state (to
                // avoid ratation jumps)
                // if (check_behinder())
...
bool CBloodsuckerStateAttackAbstract::check_vampire() {
    if ( this->prev_substate != eStateVampire_Execute ) {
        if ( this->get_state( eStateVampire_Execute )
                 ->check_start_conditions() )
            return true;
    } else {
...
bool CBloodsuckerStateAttackAbstract::check_hiding() {
    const bool health_step_lost =
        this->object->conditions().GetHealth() <
...
void CBloodsuckerStateAttackAbstract::setup_substates() {
    ...
    if ( this->current_substate == eStateAttack_Hide ) {
        ...
        state->fill_data_with(
            &data,
            sizeof( CStateMonsterBackstubEnemy< _Object >::StateParams ) );
        return;
    }
}
...
void CStateMonsterBackstubEnemy< _Object >::initialize() {
    ...
    m_encircle_end_tick =
        Device.dwTimeGlobal + detail::bloodsucker::encircle_time;
...
void CStateMonsterBackstubEnemy< _Object >::execute() {
    ...
    if ( this->object->conditions().GetHealth() <
        ...
        m_next_change_behaviour_tick =
            Device.dwTimeGlobal + detail::bloodsucker::change_behaviour_time;
        m_last_health = this->object->conditions().GetHealth();
    ...
    if ( data.action.sound_type != u32( -1 ) ) {
        this->object->set_state_sound( data.action.sound_type,
                                       data.action.sound_delay == u32( -1 ) );
    }
}
...
bool CStateMonsterBackstubEnemy< _Object >::check_completion() {
    ...
    const bool real_path_end =
        fis_zero( data.completion_dist )
            ? ( data.point.distance_to_xz( this->object->Position() ) <
                ai().level_graph().header().cell_size() )
            : true;

    if ( this->object->control().path_builder().is_path_end(
