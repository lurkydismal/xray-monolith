...
#define CStateBloodsuckerPredatorLiteAbstract \
    CStateBloodsuckerPredatorLite< _Object >
...
bool CStateBloodsuckerPredatorLiteAbstract::check_completion() {
    if ( object->EnemyMan.see_enemy_now() &&
         ( object->Position().distance_to(
               object->EnemyMan.get_enemy()->Position() ) < 4.f ) ) {
        object->set_berserk();
        return true;
    }
    ...
    if ( prev_substate == eStatePredator_Camp ) {
        if ( object->HitMemory.get_last_hit_time() > time_state_started ) {
            if ( object->EnemyMan.get_enemy() &&
                 ( object->EnemyMan.get_enemy()->Position().distance_to(
                       object->Position() ) < 10.f ) ) {
                object->set_berserk();
            } else
...
void CStateBloodsuckerPredatorLiteAbstract::select_camp_point() {
    ...
    if ( m_target_node == u32( -1 ) ) {
        const CCoverPoint* point =
            object->CoverMan->find_cover( object->Position(), 20.f, 30.f );
        if ( point ) {
