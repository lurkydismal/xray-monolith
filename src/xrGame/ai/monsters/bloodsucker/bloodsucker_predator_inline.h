...
CStateBloodsuckerPredatorAbstract::CStateBloodsuckerPredator( _Object* obj )
    : inherited( obj ) {
    add_state( eStatePredator_MoveToCover,
               xr_new< CStateMonsterMoveToPointEx< _Object > >( obj ) );
    add_state( eStatePredator_LookOpenPlace,
               xr_new< CStateMonsterLookToPoint< _Object > >( obj ) );
    add_state( eStatePredator_Camp,
...
bool CStateBloodsuckerPredatorAbstract::check_start_conditions() {
    if ( Actor()->memory().visual().visible_now( this->object ) )
        return false;
    return true;
}
...
bool CStateBloodsuckerPredatorAbstract::check_completion() {
    ...
    if ( this->object->EnemyMan.get_enemy() &&
         ...
         ( this->object->Position().distance_to(
               this->object->EnemyMan.get_enemy()->Position() ) < 4.f ) )
        return true;
...
void CStateBloodsuckerPredatorAbstract::check_force_state() {
    if ( ( this->current_substate == eStatePredator_Camp ) &&
         ( m_time_start_camp + TIME_TO_RESELECT_CAMP < time() ) ) {
...
void CStateBloodsuckerPredatorAbstract::select_camp_point() {
    ...
    if ( m_target_node == u32( -1 ) ) {
        const CCoverPoint* point = this->object->CoverMan->find_cover(
            this->object->Position(), 10.f, 30.f );
        if ( point ) {
