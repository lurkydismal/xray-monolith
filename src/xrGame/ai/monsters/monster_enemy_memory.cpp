...
void CMonsterEnemyMemory::update() {
    ...
    if ( monster_hit_memory.is_hit() &&
         time() < monster_hit_memory.get_last_hit_time() + 1000 ) {
        if ( CEntityAlive* enemy = smart_cast< CEntityAlive* >(
                 monster->HitMemory.get_last_hit_object() ) ) {
            if ( monster->CCustomMonster::useful( &monster->memory().enemy(),
                                                  enemy ) &&
                 monster->Position().distance_to( enemy->Position() ) <
                     monster->get_feel_enemy_who_just_hit_max_distance() ) {
                add_enemy( enemy );

                bool const self_is_dog =
                    !!smart_cast< const CAI_Dog* >( monster );
                if ( self_is_dog ) {
                    CMonsterSquad* const squad =
                        monster_squad().get_squad( monster );
    ...
    if ( monster->SoundMemory.IsRememberSound() && g_actor &&
        ...
        if ( dangerous && Device.dwTimeGlobal < sound.time + 2000 ) {
            if ( CEntityAlive const* enemy =
                     smart_cast< CEntityAlive const* >( sound.who ) ) {
                float const xz_dist =
                    monster->Position().distance_to_xz( enemy->Position() );
                float const y_dist =
                    _abs( monster->Position().y - enemy->Position().y );

                if ( monster->CCustomMonster::useful(
                         &monster->memory().enemy(), enemy ) &&
                     y_dist < 10 &&
                     xz_dist <
                         monster
                             ->get_feel_enemy_who_made_sound_max_distance() ) {
                    add_enemy( enemy );
    ...
    for ( objects_list::const_iterator I = objects.begin(); I != objects.end();
          ++I ) {
    ...
    float const feel_enemy_max_distance =
        monster->get_feel_enemy_max_distance();
    if ( g_actor ) {
        float const xz_dist =
            monster->Position().distance_to_xz( g_actor->Position() );
        float const y_dist =
            _abs( monster->Position().y - g_actor->Position().y );

        if ( xz_dist < feel_enemy_max_distance && y_dist < 10 &&
    ...
    for ( ENEMIES_MAP_IT it = m_objects.begin(); it != m_objects.end(); it++ ) {
        ...
        it->second.danger =
            ( 1 + relation_value * relation_value * relation_value ) /
            ( 1 + dist );
    }
}
...
void CMonsterEnemyMemory::remove_non_actual() {
    ...
    for ( ENEMIES_MAP_IT it = m_objects.begin(), nit; it != m_objects.end();
          it = nit ) {
        nit = it;
        ++nit;
...
ENEMIES_MAP_IT CMonsterEnemyMemory::find_best_enemy() {
    ...
    if ( it == m_objects.end() ) {
        // find any
        max_value = 0.f;
        for ( ENEMIES_MAP_IT I = m_objects.begin(); I != m_objects.end();
              I++ ) {
            if ( I->second.danger > max_value ) {
                max_value = I->second.danger;
                it = I;
