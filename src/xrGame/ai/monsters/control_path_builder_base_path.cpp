...
void CControlPathBuilderBase::find_target_point_set() {
    ...
    if ( m_target_type == eMoveToTarget ) {
        ...
        if ( m_man->path_builder().valid_and_accessible(
                 new_position, m_target_found.node() ) ) {
        ...
        if ( !m_man->path_builder().accessible( m_target_found.position() ) ) {
            ...
            m_target_found.set_node(
                m_man->path_builder().restrictions().accessible_nearest(
    ...
    if ( m_target_type == eRetreatFromTarget ) {
        ...
        m_target_found.set_position(
            Fvector( m_target_found.position() )
                .mad( m_object->Position(), dir, pmt_find_point_dist ) );
    }
...
void CControlPathBuilderBase::find_node() {
    ...
    if ( ai().level_graph().valid_vertex_id( m_target_found.node() ) &&
        ...
        m_man->path_builder().fix_position(
            Fvector().set( m_target_found.position() ), m_target_found.node(),
            new_position );
        m_target_found.set_position( new_position );
        return;
    }
    ...
    if ( ai().level_graph().valid_vertex_position(
             m_target_found.position() ) ) {
        m_target_found.set_node(
            ai().level_graph().vertex_id( m_target_found.position() ) );
        if ( ai().level_graph().valid_vertex_id( m_target_found.node() ) &&
             m_man->path_builder().accessible( m_target_found.node() ) ) {
            // корректировка позиции
            Fvector new_position = m_target_found.position();
            m_man->path_builder().fix_position(
                Fvector().set( m_target_found.position() ),
    ...
    if ( m_cover_info.use_covers ) {
        m_cover_approach->setup( m_target_found.position(),
                                 m_cover_info.min_dist, m_cover_info.max_dist,
    ...
    m_target_found.set_node( m_man->path_builder().find_nearest_vertex(
        m_object->ai_location().level_vertex_id(), m_target_found.position(),
        30.f ) );
    m_target_found.set_position(
        ai().level_graph().vertex_position( m_target_found.node() ) );
}
