...
float monster_aura::override_if_debug( pcstr var_name,
...
float monster_aura::calculate() const {
    float const distance =
        m_object->Position().distance_to( Actor()->Position() );

    float const epsilon = 0.0001f;

    using namespace detail;
    float linear_factor =
        override_if_debug( s_linear_factor_string, m_linear_factor );
    float quadratic_factor =
        override_if_debug( s_quadratic_factor_string, m_quadratic_factor );
    float max_power = override_if_debug( s_max_power_string, m_max_power );
    float max_distance =
        override_if_debug( s_max_distance_string, m_max_distance );
    ...
    float const power =
        linear_factor / distance + quadratic_factor / distance * distance;

    return ( power < max_power ) ? power : max_power;
}

void monster_aura::load_from_ini( CInifile const* ini,
                                  pcstr const section,
                                  bool enable_for_dead_default ) {
    using namespace detail;
    ...
    m_pp_effector_name =
        READ_IF_EXISTS( ini, r_string, section, pp_effector_name_string, NULL );
    m_pp_highest_at =
        READ_IF_EXISTS( ini, r_float, section, pp_highest_at_string, 1.f );
    m_linear_factor =
        READ_IF_EXISTS( ini, r_float, section, linear_factor_string, 0.f );
    m_quadratic_factor =
        READ_IF_EXISTS( ini, r_float, section, quadratic_factor_string, 0.f );
    m_max_power =
        READ_IF_EXISTS( ini, r_float, section, max_power_string, 0.f );
    m_max_distance =
        READ_IF_EXISTS( ini, r_float, section, max_distance_string, 0.f );
    m_enable_for_dead = !!READ_IF_EXISTS(
        ini, r_bool, section, enable_for_dead_string, enable_for_dead_default );
    pcstr const sound_name =
        READ_IF_EXISTS( ini, r_string, section, sound_string, NULL );
    pcstr const detect_sound_name =
        READ_IF_EXISTS( ini, r_string, section, detect_sound_string, NULL );
    ...
    if ( m_pp_effector_name || m_max_power || m_max_distance || sound_name ||
         detect_sound_name )
        m_enabled = true;
}
...
float monster_aura::get_post_process_factor() const {
    using namespace detail;
    float pp_highest_at =
        override_if_debug( s_pp_highest_at_string, m_pp_highest_at );
...
void monster_aura::play_detector_sound() {
    ...
    if ( distance < m_max_distance ) {
        ...
        float freq = ( distance / m_max_distance ) *
                     ( m_max_power / ( power ? power : 1.0f ) );
        if ( distance > 0.65f * m_max_distance )
            freq *= 0.5f;
        float cur_period = 0.1f + ( 2.0f - 0.1f ) * freq;

        if ( m_detect_snd_time > cur_period ) {
            m_detect_sound.play_at_pos( Actor(), Fvector().set( 0.f, 1.f, 0.f ),
                                        sm_2D );
            m_detect_snd_time = 0.0f;
        } else
...
void monster_aura::update_schedule() {
    ...
    if ( !m_sound._feedback() ) {
        m_sound.play_at_pos( Actor(), Fvector().set( 0.f, 1.f, 0.f ),
                             sm_Looped | sm_2D );
    }
    ...
    if ( pp_factor > 0.01f ) {
        if ( !m_pp_index ) {
            ...
            AddEffector( Actor(), m_pp_index, m_pp_effector_name,
                         GET_KOEFF_FUNC(
                             this, &monster_aura::get_post_process_factor ) );
        }
