...
void CCat::Load( LPCSTR section ) {
    ...
    anim().AddAnim( eAnimJumpLeft, "stand_jump_ls_", -1, &velocity_none,
                    PS_STAND );
    anim().AddAnim( eAnimJumpRight, "stand_jump_rs_", -1, &velocity_none,
                    PS_STAND );
...
void CCat::reinit() {
    ...
    // com_man().add_rotation_jump_data(0,0,"run_turn_180_r_0",
    // "run_turn_180_r_1", deg(179));
}
...
void CCat::CheckSpecParams( u32 spec_params ) {
    ...
    if ( ( spec_params & ASP_ROTATION_JUMP ) == ASP_ROTATION_JUMP ) {
        ...
        // float delta_yaw =
        // angle_difference(yaw,movement().m_body.current.yaw); float time =
        // anim().GetCurAnimTime(); new_angular_velocity = delta_yaw / time;
