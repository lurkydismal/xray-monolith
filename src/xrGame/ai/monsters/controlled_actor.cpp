...
void CControlledActor::update_turn() {
    ...
    Fvector()
        .sub( m_target_point, P )
        .getHP( cam_target_yaw, cam_target_pitch );
    ...
    // YAW
    float speed_factor =
        angle_difference( cam_current_yaw, cam_target_yaw ) / MAX_DIST;
    ...
    // PITCH
    speed_factor =
        angle_difference( cam_current_pitch, cam_target_pitch ) / MAX_DIST;
    clamp( speed_factor, 0.f, 1.f );
    if ( speed_factor > 0.5f )
        speed_factor = 1.f - speed_factor;
