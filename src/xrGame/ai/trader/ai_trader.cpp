...
void CAI_Trader::Load( LPCSTR section ) {
    ...
    // fHealth							=
    // pSettings->r_float	(section,"Health");
...
bool CAI_Trader::bfAssignSound( CScriptEntityAction* tpEntityAction ) {
    ...
    // CScriptSoundAction	&l_tAction	=
    // tpEntityAction->m_tSoundAction; m_cur_head_anim_type =
    // l_tAction.m_tHeadAnimType;
...
BOOL CAI_Trader::net_Spawn( CSE_Abstract* DC ) {
    ...
    shedule.t_min = 100;
    shedule.t_max =
        2500; // This equaltiy is broken by Dima :-( // 30 * NET_Latency / 4;
    ...
    switch ( type ) {
        ...
        case GE_OWNERSHIP_REJECT: {
            ...
            bool dont_create_shell =
                ( type == GE_TRADE_SELL ) || just_before_destroy;

            Obj->SetTmpPreDestroy( just_before_destroy );
            inventory().DropItem( smart_cast< CGameObject* >( Obj ),
                                  just_before_destroy, dont_create_shell );
            // if(inventory().DropItem(smart_cast<CGameObject*>(Obj),
            // just_before_destroy)) 	Obj->H_SetParent(0, just_before_destroy);
            ////moved to DropItem
        } break;
...
void CAI_Trader::g_fireParams( const CHudItem* pHudItem,
                               Fvector& P,
                               Fvector& D ) {
...
ALife::ERelationType CAI_Trader::tfGetRelationType(
...
bool CAI_Trader::AllowItemToTrade( CInventoryItem const* item,
