...
public:
    ...
    virtual void Think();
    virtual void SelectAnimation( const Fvector& _view,
                                  const Fvector& _move,
                                  float speed );
    ...
    void AdjustHeadOrientation( float targetPitch,
                                float targetYaw,
                                float targetRoll );
    ...
    virtual bool useful( const CItemManager* manager,
                         const CGameObject* object ) const;
    virtual float evaluate( const CItemManager* manager,
                            const CGameObject* object ) const;
    virtual bool useful( const CEnemyManager* manager,
    ...
    //	virtual CInventoryItem				*GetCurrentEquipment
    //() const; <- moved to InventoryOwner::GetCurrentOutfit
...
private:
    void can_kill_entity( const Fvector& position,
                          const Fvector& direction,
                          float distance,
                          collide::rq_results& rq_storage );
    void can_kill_entity_from( const Fvector& position,
                               Fvector direction,
                               float distance );
...
private:
    struct CTradeItem {
        ...
        IC CTradeItem( CInventoryItem* item,
...
protected:
    u32 fill_items( CInventory& inventory,
                    CGameObject* old_owner,
                    ALife::_OBJECT_ID new_owner_id );
    ...
    void select_items();
    void transfer_item( CInventoryItem* item,
                        CGameObject* old_owner,
                        CGameObject* new_owner );

    void update_sell_info();
    bool tradable_item( CInventoryItem* inventory_item,
                        const u16& current_owner_id );
    bool can_sell( CInventoryItem* item );

    bool non_conflicted( const CInventoryItem* item,
                         const CWeapon* new_weapon ) const;
    bool enough_ammo( const CWeapon* new_weapon ) const;
    bool conflicted( const CInventoryItem* item,
...
public:
    ...
    virtual void update_range_fov( float& new_range,
                                   float& new_fov,
                                   float start_range,
                                   float start_fov );
...
public:
    typedef xr_delegate< void( const CCoverPoint*, const CCoverPoint* ) >
        on_best_cover_changed_delegate;
...
private:
    ...
    void compute_enemy_distances( float& minimum_enemy_distance,
                                  float& maximum_enemy_distance );
    void update_best_cover_actuality( const Fvector& position_to_cover_from );
    void on_best_cover_changed( const CCoverPoint* new_cover,
...
public:
    void subscribe_on_best_cover_changed(
        const on_best_cover_changed_delegate& delegate );
    void unsubscribe_on_best_cover_changed(
        const on_best_cover_changed_delegate& delegate );
...
private:
    bool throw_check_error( float low,
                            float high,
                            const Fvector& start,
                            const Fvector& velocity,
                            const Fvector& gravity );
    void check_throw_trajectory( const float& throw_time );
    void throw_target_impl( const Fvector& position,
...
private:
    ...
    void fill_bones_body_parts( LPCSTR bone_id,
...
private:
    ...
    // Per-NPC aim params, read per frame in CSightManager::Exec_Look (swing
    // speed + firing cone) and CSightAction (target lead). Negative = unset:
    // the getters fall back to the live console vars (ai_aim_min_speed /
    // ai_aim_min_angle / ai_aim_max_angle / ai_aim_predict_time), so an unset
    // NPC and existing cvar consumers keep exact vanilla behavior.
    // set_aim_params with a negative component reverts that component to the
    // global.
    ...
    // Per-NPC fire-queue scaling, applied in
    // CStalkerActionCombatBase::select_queue_params to the burst size and
    // inter-burst interval the combat planner rolls (scaled size clamps to >=
    // 1). Negative = unset: the getters return 1.0 and the planner fires exact
    // vanilla queues. Script set_item queues, the death mag dump and player
    // weapons are deliberately not touched.
...
public:
    ...
    IC void set_aim_params( float max_angle,
                            float min_angle,
                            float min_speed,
                            float predict_time );
