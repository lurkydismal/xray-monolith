...
bool CAI_Stalker::bfAssignMovement( CScriptEntityAction* tpEntityAction ) {
    ...
    CScriptAnimationAction& l_tAnimationAction =
        tpEntityAction->m_tAnimationAction;
    ...
    sight().setup( l_tWatchAction.m_tWatchType,
...
bool CAI_Stalker::bfAssignWatch( CScriptEntityAction* tpEntityAction ) {
    ...
    //	float			&yaw = movement().m_head.target.yaw, &pitch =
    ...
    switch ( l_tWatchAction.m_tGoalType ) {
        case CScriptWatchAction::eGoalTypeObject: {
            if ( !xr_strlen( l_tWatchAction.m_bone_to_watch ) )
                l_tWatchAction.m_tpObjectToWatch->Center(
                    l_tWatchAction.m_tWatchVector );
            else {
                CBoneInstance& l_tBoneInstance =
                    smart_cast< IKinematics* >(
                        l_tWatchAction.m_tpObjectToWatch->Visual() )
                        ->LL_GetBoneInstance(
                            smart_cast< IKinematics* >(
                                l_tWatchAction.m_tpObjectToWatch->Visual() )
    ...
    if ( ( CScriptWatchAction::eGoalTypeWatchType !=
           l_tWatchAction.m_tGoalType ) &&
         ( angle_difference( movement().m_head.target.yaw,
                             movement().m_head.current.yaw ) < EPS_L ) &&
         ( angle_difference( movement().m_head.target.pitch,
                             movement().m_head.current.pitch ) < EPS_L ) )
        l_tWatchAction.m_bCompleted = true;
    else
...
bool CAI_Stalker::bfAssignObject( CScriptEntityAction* tpEntityAction ) {
    ...
    CInventoryItem* l_tpInventoryItem =
        smart_cast< CInventoryItem* >( l_tObjectAction.m_tpObject );

    if ( !inherited::bfAssignObject( tpEntityAction ) ||
         !l_tObjectAction.m_tpObject || !l_tpInventoryItem ) {
    ...
    CWeaponMagazined* l_tpWeaponMagazined =
    ...
    switch ( l_tObjectAction.m_tGoalType ) {
            ...
            if ( inventory().ActiveItem() && l_tpWeapon ) {
                if ( l_tpWeapon->GetAmmoElapsed() ) {
                    ...
                    if ( l_tpWeapon->GetSuitableAmmoTotal() ) {
                        //						CObjectHandler::set_goal
                        //(eObjectActionFire1,l_tObjectAction.m_tpObject);
                        //						inventory().Action(kWPN_RELOAD,
                        //CMD_START);
                    } else
        ...
        case eObjectActionReload1: {
            ...
            if ( inventory().ActiveItem()->object().ID() ==
                 l_tObjectAction.m_tpObject->ID() ) {
                //				inventory().Action(kWPN_FIRE,
                //CMD_STOP);
            ...
            //			if (inventory().ActiveItem()) {
            //				inventory().Action(kWPN_FIRE,
            //CMD_STOP); 				if (CWeapon::eReload != l_tpWeapon->STATE)
            //					inventory().Action(kWPN_RELOAD,
            //CMD_START); 				else 					l_tObjectAction.m_bCompleted = true;
            //			}
            //			else
            //				ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"cannot
            //reload active item because it is not selected!");
            break;
        }
        case eObjectActionActivate: {
            ...
            //			if (inventory().ActiveItem() &&
            //(inventory().ActiveItem()->ID() == l_tpInventoryItem->ID())) 				if
            //(l_tpWeapon && (CWeapon::eIdle == l_tpWeapon->STATE))
            //				l_tObjectAction.m_bCompleted = true;
            return ( ( l_tObjectAction.m_bCompleted =
                           ( CObjectHandler::goal_reached() ) ) == false );
        ...
        case eObjectActionTake: {
            if ( inventory().GetItemFromInventory(
                     *l_tObjectAction.m_tpObject->cName() ) ) {
                ai().script_engine().script_log(
                    ScriptStorage::eLuaMessageTypeError,
