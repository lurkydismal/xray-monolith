...
void try_change_current_entity() {
    ...
    CFrustum frustum;
    frustum.CreateFromMatrix( Device.mFullTransform,
    ...
    for ( ; I != E; ++I ) {
        CCustomMonster* current =
            smart_cast< CCustomMonster* >( ( *I )->dcast_CObject() );
        if ( !current )
            continue;
...
void restore_actor() {
    ...
    CHudItem* pHudItem =
        smart_cast< CHudItem* >( g_debug_actor->inventory().ActiveItem() );
    if ( pHudItem ) {
...
void draw_planner( const planner_type& brain,
                   LPCSTR start_indent,
                   LPCSTR indent,
                   LPCSTR planner_id ) {
    ...
    CScriptActionPlannerAction* planner =
        smart_cast< CScriptActionPlannerAction* >(
            &_brain.action( brain.solution().front() ) );
    if ( planner )
        draw_planner( *planner, start_indent, indent,
                      _brain.action2string( brain.solution().front() ) );

    DBG_OutText( "%s ", start_indent );
    DBG_OutText( "%splanner %s", start_indent, planner_id );
    DBG_OutText( "%s%sevaluators  : %d", start_indent, indent,
                 brain.evaluators().size() );
    DBG_OutText( "%s%soperators   : %d", start_indent, indent,
                 brain.operators().size() );
    DBG_OutText( "%s%sselected    : %s", start_indent, indent,
                 _brain.action2string( brain.solution().front() ) );
    // solution
    DBG_OutText( "%s%ssolution", start_indent, indent );
    for ( int i = 0; i < ( int )brain.solution().size(); ++i )
        DBG_OutText( "%s%s%s%s", start_indent, indent, indent,
    ...
    for ( ; I != E; ++I ) {
        xr_vector< planner_type::COperatorCondition >::const_iterator J =
            std::lower_bound(
                brain.current_state().conditions().begin(),
                brain.current_state().conditions().end(),
                planner_type::CWorldProperty( ( *I ).first, false ) );
        char temp = '?';
        if ( ( J != brain.current_state().conditions().end() ) &&
             ( ( *J ).condition() == ( *I ).first ) ) {
            temp = ( *J ).value() ? '+' : '-';
            DBG_OutText( "%s%s%s    %5c : [%d][%s]", start_indent, indent,
                         indent, temp, ( *I ).first,
    ...
    for ( ; I != E; ++I ) {
        xr_vector< planner_type::COperatorCondition >::const_iterator J =
            std::lower_bound(
                brain.target_state().conditions().begin(),
                brain.target_state().conditions().end(),
                planner_type::CWorldProperty( ( *I ).first, false ) );
        char temp = '?';
        if ( ( J != brain.target_state().conditions().end() ) &&
             ( ( *J ).condition() == ( *I ).first ) ) {
            temp = ( *J ).value() ? '+' : '-';
            DBG_OutText( "%s%s%s    %5c : [%d][%s]", start_indent, indent,
                         indent, temp, ( *I ).first,
...
LPCSTR animation_name( CAI_Stalker* self, const MotionID& animation ) {
    ...
    IKinematicsAnimated* skeleton_animated =
        smart_cast< IKinematicsAnimated* >( self->Visual() );
    VERIFY( skeleton_animated );
    LPCSTR name = skeleton_animated->LL_MotionDefName_dbg( animation ).first;
    return ( name );
}

void draw_restrictions( const shared_str& restrictions,
                        LPCSTR start_indent,
                        LPCSTR indent,
                        LPCSTR header ) {
...
void CAI_Stalker::debug_text() {
    ...
    DBG_OutText( "%s%sFOV         : %f", indent, indent,
                 rad2deg( object_fov ) );
    if ( g_Alive() ) {
        DBG_OutText( "%s%sobjects     : %d", indent, indent,
                     memory().visual().objects().size() );
        DBG_OutText( "%s%snot yet     : %d", indent, indent,
                     memory().visual().not_yet_visible_objects().size() );
        DBG_OutText( "%s%sin frustum  : %d", indent, indent,
                     memory().visual().raw_objects().size() );
        if ( memory().visual().visible_now( actor ) )
            DBG_OutText( "%s%sactor       : visible", indent, indent );
        else {
            MemorySpace::CNotYetVisibleObject* object =
                memory().visual().not_yet_visible_object( actor );
            if ( object && !fis_zero( object->m_value ) )
                DBG_OutText( "%s%sactor       : not yet visible : %f", indent,
                             indent, object->m_value );
            else
        ...
        if ( memory().sound().sound() ) {
            ...
            DBG_OutText( "%s%s%spower     : %f", indent, indent, indent,
                         memory().sound().sound()->m_power );
            DBG_OutText( "%s%s%sobject    : %s", indent, indent, indent,
                         memory().sound().sound()->m_object
                             ? *memory().sound().sound()->m_object->cName()
                             : "unknown" );
        ...
        DBG_OutText( "%s%sobjects     : %d", indent, indent,
                     memory().hit().objects().size() );
        ALife::_OBJECT_ID object_id = memory().hit().last_hit_object_id();
        DBG_OutText( "%s%slast hit object id   : %d", indent, indent,
                     object_id );
        CObject* object = ( object_id == ALife::_OBJECT_ID( -1 ) )
                              ? 0
                              : Level().Objects.net_Find( object_id );
        DBG_OutText( "%s%slast hit object name : %s", indent, indent,
                     object ? *object->cName() : "" );
        ...
        if ( memory().hit().hit() ) {
            ...
            DBG_OutText( "%s%s%sobject    : %s", indent, indent, indent,
                         memory().hit().hit()->m_object
                             ? *memory().hit().hit()->m_object->cName()
                             : "unknown" );
    ...
    if ( inventory().ActiveItem() ) {
        DBG_OutText( "%s%scan kill member   : %s", indent, indent,
                     can_kill_member() ? "+" : "-" );
        DBG_OutText( "%s%scan kill enemy    : %s", indent, indent,
                     can_kill_enemy() ? "+" : "-" );
        DBG_OutText( "%s%spick distance     : %f", indent, indent,
                     pick_distance() );
        DBG_OutText( "%s%sfire make sense   : %s", indent, indent,
                     fire_make_sense() ? "+" : "-" );
        DBG_OutText( "%s%sactor is enemy    : %c", indent, indent,
                     is_relation_enemy( actor ) ? '+' : '-' );
        DBG_OutText( "%s%sis enemy to actor : %c", indent, indent,
                     actor->is_relation_enemy( this ) ? '+' : '-' );
    }

    DBG_OutText( "%s%sobjects     : %d", indent, indent,
                 memory().enemy().objects().size() );
    if ( g_Alive() ) {
        CEnemyManager::OBJECTS::const_iterator I =
            memory().enemy().objects().begin();
        CEnemyManager::OBJECTS::const_iterator E =
            memory().enemy().objects().end();
        for ( ; I != E; ++I )
            DBG_OutText(
                "%s%s%s%s : %s", indent, indent, indent, *( *I )->cName(),
    ...
    if ( memory().enemy().selected() ) {
        ...
        if ( g_Alive() ) {
            if ( true || !g_mt_config.test( mtAiVision ) )
                VERIFY( !memory().visual().visible_now(
                            memory().enemy().selected() ) ||
                        ( fuzzy > 0.f ) );
            DBG_OutText(
                "%s%s%svisible   : %s %f", indent, indent, indent,
        ...
        if ( g_Alive() ) {
            float interval = ( 1.f - panic_threshold() ) * .25f, left = -1.f,
                  right = -1.f;
            LPCSTR description = "invalid";
            u32 result = dwfChooseAction(
                2000, 1.f - interval, 1.f - 2 * interval, 1.f - 3 * interval,
                panic_threshold(), g_Team(), g_Squad(), g_Group(), 0, 1, 2, 3,
                4, this, 300.f );
            switch ( result ) {
                case 0: {
                    description = "attack";
            ...
            DBG_OutText( "%s%s%svictory   : [%5.2f%%,%5.2f%%] -> %s", indent,
                         indent, indent, 100.f * right, 100.f * left,
                         description );
        }
    }
    // danger
    ...
    DBG_OutText( "%s%sobjects     : %d", indent, indent,
                 memory().danger().objects().size() );
    if ( memory().danger().selected() &&
         memory().danger().selected()->object() ) {
        DBG_OutText( "%s%sselected", indent, indent );
        DBG_OutText( "%s%s%stype      : %s", indent, indent, indent,
        ...
        DBG_OutText( "%s%s%stime      : %.3f (%.3f)", indent, indent, indent,
                     ...
                     float( Device.dwTimeGlobal -
                            memory().danger().selected()->time() ) /
                         1000.f );
        DBG_OutText( "%s%s%sinitiator : %s", indent, indent, indent,
        ...
        if ( memory().danger().selected()->dependent_object() &&
            ...
            DBG_OutText(
                "%s%s%sdependent : %s", indent, indent, indent,
                *memory().danger().selected()->dependent_object()->cName() );
            if ( g_Alive() )
                DBG_OutText(
                    "%s%s%svisible   : %s", indent, indent, indent,
                    memory().visual().visible_now(
                        smart_cast< const CGameObject* >(
    ...
    DBG_OutText( "%s%sundetected  : %s", indent, indent,
                 undetected_anomaly() ? "+" : "-" );
    DBG_OutText( "%s%sinside      : %s", indent, indent,
                 inside_anomaly() ? "+" : "-" );
    ...
    if ( g_Alive() ) {
        DBG_OutText( "%smembers           : %d", indent,
                     agent_manager().member().members().size() );
        DBG_OutText( "%senemies           : %d", indent,
                     agent_manager().enemy().enemies().size() );
        DBG_OutText( "%scorpses           : %d", indent,
                     agent_manager().corpse().corpses().size() );
        DBG_OutText( "%sdanger locations  : %d", indent,
                     agent_manager().location().locations().size() );
        DBG_OutText( "%smembers in combat : %d", indent,
                     agent_manager().member().combat_members().size() );
        if ( g_Alive() )
            DBG_OutText( "%sI am in combat    : %s", indent,
                         agent_manager().member().registered_in_combat( this )
                             ? "+"
                             : "-" );
        DBG_OutText( "%smembers in detour : %d", indent,
                     agent_manager().member().in_detour() );
        if ( g_Alive() )
            DBG_OutText(
                "%sI am in detour    : %s", indent,
    ...
    DBG_OutText( "%s%sobjects             : %d", indent, indent,
                 inventory().m_all.size() );
    DBG_OutText( "%s%sactive item         : %s", indent, indent,
                 inventory().ActiveItem()
                     ? *inventory().ActiveItem()->object().cName()
                     : "" );
    DBG_OutText( "%s%sbest weapon         : %s", indent, indent,
                 best_weapon() ? *best_weapon()->object().cName() : "" );
    DBG_OutText( "%s%sitem to kill        : %s", indent, indent,
                 item_to_kill() ? *m_best_item_to_kill->object().cName() : "" );
    DBG_OutText( "%s%sitem can kill       : %s", indent, indent,
                 item_can_kill() ? "+" : "-" );
    DBG_OutText( "%s%smemory item to kill : %s", indent, indent,
                 remember_item_to_kill()
                     ? *m_best_found_item_to_kill->object().cName()
                     : "" );
    DBG_OutText( "%s%smemory ammo         : %s", indent, indent,
                 remember_ammo() ? *m_best_found_ammo->object().cName() : "" );
    DBG_OutText( "%s%sinfinite ammo       : %s", indent, indent,
                 m_infinite_ammo ? "+" : "-" );
    DBG_OutText(
        "%s%sitem to spawn       : %s", indent, indent,
        item_to_spawn().size() ? *item_to_spawn() : "no item to spawn" );
    DBG_OutText( "%s%sammo in box to spawn: %d", indent, indent,
    ...
    if ( weapon ) {
        ...
        DBG_OutText( "%s%squeue size          : %d", indent, indent,
                     weapon->GetQueueSize() );
    ...
    if ( inventory().ActiveItem() ) {
        ...
        if ( weapon ) {
            DBG_OutText( "%s%s%sstrapped       : %s", indent, indent, indent,
                         weapon_strapped( weapon ) ? "+" : "-" );
            DBG_OutText( "%s%s%sunstrapped     : %s", indent, indent, indent,
                         weapon_unstrapped( weapon ) ? "+" : "-" );
            DBG_OutText( "%s%s%sammo           : %d", indent, indent, indent,
                         weapon->GetAmmoElapsed() );
            DBG_OutText( "%s%s%smagazine       : %d", indent, indent, indent,
                         weapon->GetAmmoMagSize() );
            DBG_OutText( "%s%s%stotal ammo     : %d", indent, indent, indent,
                         weapon->GetSuitableAmmoTotal() );
        }
    }
    ...
    draw_planner( movement().animation_selector().planner(), indent, indent,
                  "smart cover planner" );
    ...
    DBG_OutText( "%s%sforward     : [%c]", indent, indent,
                 animation().forward_blend_callbacks() ? '+' : '-' );
    DBG_OutText( "%s%sbackward    : [%c]", indent, indent,
                 animation().backward_blend_callbacks() ? '+' : '-' );

    DBG_OutText( "%s%shead        : [%s][%f]", indent, indent,
                 animation_name( this, animation().head().animation() ),
                 animation().head().blend()
                     ? animation().head().blend()->timeCurrent
                     : 0.f );
    DBG_OutText( "%s%storso       : [%s][%f]", indent, indent,
                 animation_name( this, animation().torso().animation() ),
                 animation().torso().blend()
                     ? animation().torso().blend()->timeCurrent
                     : 0.f );
    DBG_OutText( "%s%slegs        : [%s][%f]", indent, indent,
                 animation_name( this, animation().legs().animation() ),
                 animation().legs().blend()
                     ? animation().legs().blend()->timeCurrent
                     : 0.f );
    DBG_OutText( "%s%sglobal      : [%s][%f]", indent, indent,
                 animation_name( this, animation().global().animation() ),
                 animation().global().blend()
                     ? animation().global().blend()->timeCurrent
                     : 0.f );
    DBG_OutText( "%s%sscript      : [%s][%f]", indent, indent,
                 animation_name( this, animation().script().animation() ),
                 animation().script().blend()
                     ? animation().script().blend()->timeCurrent
                     : 0.f );

    // movement
    DBG_OutText( " " );
    DBG_OutText( "%smovement", indent );
    DBG_OutText( "%s%senabled         : %s", indent, indent,
                 movement().enabled() ? "+" : "-" );
    ...
    DBG_OutText( "%s%smovement type   : %s (current)", indent, indent,
                 movement_type( movement().movement_type() ) );
    DBG_OutText( "%s%smovement type   : %s (target)", indent, indent,
    ...
    DBG_OutText( "%s%sposition        : [%f][%f][%f]", indent, indent,
                 VPUSH( Position() ) );
    DBG_OutText( "%s%slevel vertex id : %d", indent, indent,
                 ai_location().level_vertex_id() );
    DBG_OutText( "%s%sgame vertex id  : %d", indent, indent,
                 ai_location().game_vertex_id() );

    if ( movement().path_type() == MovementManager::ePathTypePatrolPath ) {
        DBG_OutText( "%s%spatrol", indent, indent );
        DBG_OutText( "%s%s%spath          : %s", indent, indent, indent,
                     *movement().patrol().path_name() );
        DBG_OutText( "%s%s%scompleted     : %s", indent, indent, indent,
                     movement().patrol().completed() ? "+" : "-" );
        DBG_OutText( "%s%s%scurrent point : %d", indent, indent, indent,
                     movement().patrol().get_current_point_index() );
        if ( movement().patrol().get_path() &&
             movement().patrol().get_path()->vertex(
                 movement().patrol().get_current_point_index() ) )
            DBG_OutText( "%s%s%sextrapolate   : %s", indent, indent, indent,
                         movement().patrol().extrapolate_path() ? "+" : "-" );
        else
            DBG_OutText( "%s%s%sextrapolate   : unknown", indent, indent,
                         indent );
    }
    ...
    if ( movement().path_type() == MovementManager::ePathTypeGamePath ) {
        ...
        DBG_OutText( "%s%s%spath size     : %d", indent, indent, indent,
    ...
    DBG_OutText( "%s%s%sstart vertex  : %d", indent, indent, indent,
                 movement().level_path().path().empty()
                     ? -1
        ...
        CDetailPathManager::STravelParams current_velocity =
            movement().detail().velocity(
                movement()
                    ...
                    .path()[ movement().detail().curr_travel_point_index() ]
                    .velocity );
        DBG_OutText( "%s%s%s%slinear    : %f", indent, indent, indent, indent,
                     current_velocity.linear_velocity );
        DBG_OutText( "%s%s%s%sangular   : %f deg", indent, indent, indent,
                     indent, rad2deg( current_velocity.angular_velocity ) );
        DBG_OutText( "%s%s%s%sangular(R): %f deg", indent, indent, indent,
                     indent,
                     rad2deg( current_velocity.real_angular_velocity ) );
        DBG_OutText( "%s%s%sspeed(calc)   : %f", indent, indent, indent,
                     movement().speed() );
        DBG_OutText(
            "%s%s%sspeed(physics): %f", indent, indent, indent,
            movement().speed( character_physics_support()->movement() ) );
    }

    if ( movement().detail().use_dest_orientation() )
        DBG_OutText( "%s%s%sorientation   : + [%f][%f][%f]", indent, indent,
                     indent, VPUSH( movement().detail().dest_direction() ) );
    else
        DBG_OutText( "%s%s%sorientation   : -", indent, indent, indent );

    DBG_OutText( "%s%ssmart covers", indent, indent );
    DBG_OutText( "%s%s%ssmart cover current : %s", indent, indent, indent,
                 movement().current_params().cover_id().c_str() );
    DBG_OutText( "%s%s%ssmart cover target  : %s", indent, indent, indent,
                 movement().target_params().cover_id().c_str() );
    DBG_OutText( "%s%s%sloophole current    : %s", indent, indent, indent,
                 movement().current_params().cover()
                     ? movement().current_params().cover_loophole_id()
                     : "" );
    DBG_OutText( "%s%s%sloophole target     : %s", indent, indent, indent,
                 movement().target_params().cover()
                     ? movement().target_params().cover_loophole_id()
                     : "" );

    DBG_OutText( "%s%s%sidle min time       : %.2f", indent, indent, indent,
                 movement().idle_min_time() );
    DBG_OutText( "%s%s%sidle max time       : %.2f", indent, indent, indent,
                 movement().idle_max_time() );
    DBG_OutText( "%s%s%slookout min time    : %.2f", indent, indent, indent,
                 movement().lookout_min_time() );
    DBG_OutText( "%s%s%slookout max time    : %.2f", indent, indent, indent,
                 movement().lookout_max_time() );

    DBG_OutText( "%s%s%sapply loophole direction distance : %.2f", indent,
                 indent, indent,
                 movement().apply_loophole_direction_distance() );

    DBG_OutText( "%s%s%suse smart covers only : %c", indent, indent, indent,
                 use_smart_covers_only() ? '+' : '-' );
    DBG_OutText( "%s%s%sin smart cover      : %c", indent, indent, indent,
                 lua_game_object()->in_smart_cover() ? '+' : '-' );

    if ( movement().current_params().cover() ) {
        if ( movement().current_params().cover_loophole() ) {
            if ( movement().current_params().cover_fire_position() ) {
                DBG_OutText(
                    "%s%s%sin loophole fov     : %c", indent, indent, indent,
                    movement().in_current_loophole_fov(
                        *movement().current_params().cover_fire_position() )
                        ? '+'
                        : '-' );
                DBG_OutText(
                    "%s%s%sin loophole range   : %c", indent, indent, indent,
                ...
                DBG_OutText(
                    "%s%s%sin loophole fov     : %c", indent, indent, indent,
                    movement().in_current_loophole_fov( movement()
                                                            .current_params()
                                                            .cover_fire_object()
                                                            ->Position() )
                        ? '+'
                        : '-' );
                DBG_OutText( "%s%s%sin loophole range   : %c", indent, indent,
                             indent,
                             movement().in_current_loophole_range(
                                 movement()
                                     .current_params()
                ...
                Fvector position =
                    memory().memory_position( memory().enemy().selected() );
                DBG_OutText(
                    "%s%s%sin loophole fov     : %c", indent, indent, indent,
    ...
    if ( movement().restrictions().out_restrictions().size() ||
        ...
        strconcat( sizeof( temp ), temp, indent, indent, indent );
        draw_restrictions( movement().restrictions().out_restrictions(), temp,
                           indent, "out" );
        draw_restrictions( movement().restrictions().in_restrictions(), temp,
                           indent, "in" );
        draw_restrictions( movement().restrictions().base_out_restrictions(),
                           temp, indent, "base out" );
        draw_restrictions( movement().restrictions().base_in_restrictions(),
                           temp, indent, "base in" );
    }
    ...
    DBG_OutText( "%s%scollections : %d", indent, indent,
                 sound().objects().size() );

    {
        u32 object_count = 0;
        CSoundPlayer::SOUND_COLLECTIONS::const_iterator I =
            sound().objects().begin();
        CSoundPlayer::SOUND_COLLECTIONS::const_iterator E =
            sound().objects().end();
        for ( ; I != E; ++I )
    ...
    {
        xr_vector< CSoundPlayer::CSoundSingle >::const_iterator I =
            sound().playing_sounds().begin();
        xr_vector< CSoundPlayer::CSoundSingle >::const_iterator E =
            sound().playing_sounds().end();
        for ( ; I != E; ++I )
            DBG_OutText(
                "%s%s%s[%s]%s", indent, indent, indent,
                ( Device.dwTimeGlobal < ( *I ).m_start_time )
                    ? "not yet started"
                    : ( ( *I ).m_sound->_feedback() ? "playing"
                                                    : "already played" ),
                ( *I ).m_sound->_handle()
                    ? ( *I ).m_sound->_handle()->file_name()
                    : "no source" );
    }
    ...
    switch ( sight().current_action().sight_type() ) {
        ...
        case SightManager::eSightTypeDirection: {
            DBG_OutText( "%s%sdirection       : [%f][%f][%f]", indent, indent,
        ...
        case SightManager::eSightTypeObject: {
            DBG_OutText( "%s%sobject          : %s", indent, indent,
                         *sight().current_action().object().cName() );
            DBG_OutText(
                "%s%sposition        : [%f][%f][%f]", indent, indent,
        ...
        case SightManager::eSightTypeFireObject: {
            ...
            DBG_OutText(
                "%s%sposition        : [%f][%f][%f]", indent, indent,
                VPUSH( sight().current_action().object().Position() ) );
            DBG_OutText( "%s%svisible point   : %s", indent, indent,
                         false ? "-" : "+" );
            break;
        }
        case SightManager::eSightTypeAnimationDirection: {
...
void CAI_Stalker::dbg_draw_vision() {
    ...
    CNotYetVisibleObject* object = memory().visual().not_yet_visible_object(
        smart_cast< CGameObject* >( Level().CurrentEntity() ) );
    string64 out_text;
...
public:
    IC ray_query_param( CCustomMonster* holder,
...
BOOL _ray_query_callback( collide::rq_result& result, LPVOID params ) {
    ...
    param->m_points->push_back( Fvector().mad(
        param->m_start_position, param->m_direction, result.range ) );

    float power =
        param->m_holder->feel_vision_mtl_transp( result.O, result.element );
    param->m_power *= power;
...
void fill_points( CCustomMonster* self,
    ...
    collide::ray_defs ray_defs( position, direction, distance, CDB::OPT_CULL,
                                collide::rqtBoth );
    VERIFY( !fis_zero( ray_defs.dir.square_magnitude() ) );

    ray_query_param params( self,
                            self->memory().visual().transparency_threshold(),
                            distance, position, direction, points );

    Level().ObjectSpace.RayQuery( rq_storage, ray_defs, _ray_query_callback,
                                  &params, NULL, self );

    pick_distance = params.m_pick_distance;
}

void draw_visiblity_rays( CCustomMonster* self,
    ...
    fill_points( self, start_position, direction, distance, rq_storage, points,
                 pick_distance );

    if ( fsimilar( pick_distance, distance ) &&
         !dest_position.similar( points.back() ) )
        points.push_back( dest_position );

    VERIFY( points.size() > 1 );

    Fvector size = Fvector().set( .05f, .05f, .05f );
    Level().debug_renderer().draw_aabb( points.front(), size.x, size.y, size.z,
...

    {
        ...
        for ( ; I != E; ++I ) {
            Level().debug_renderer().draw_line( Fidentity, *( I - 1 ), *I,
                                                D3DCOLOR_XRGB( 0, 255, 0 ) );
            Level().debug_renderer().draw_aabb( *I, size.x, size.y, size.z,
                                                D3DCOLOR_XRGB( 0, 255, 0 ) );
        }
    }

    Level().debug_renderer().draw_aabb( points.back(), size.x, size.y, size.z,
...
void CAI_Stalker::dbg_draw_visibility_rays() {
    ...
    const CEntityAlive* enemy =
        memory().enemy().selected() ? memory().enemy().selected() : Actor();
    if ( enemy ) {
...
static Fmatrix aim_on_actor( Fvector const& bone_position,
    ...
    if ( debug_draw ) {
        ...
        Fvector const weapon_position_target =
            Fvector().mad( weapon_position, weapon_direction,
                           weapon_position.distance_to( target ) );
        renderer.draw_line( Fidentity, weapon_position, weapon_position_target,
    ...
    Fvector current_point =
        Fvector().mad( weapon_position, weapon_direction, offset );
    ...
    Fvector const circle_center =
        Fvector().mad( bone_position, direction_target, to_circle_center );
    ...
    float const circle_radius_sqr =
        sphere_radius_sqr - _sqr( to_circle_center );
    VERIFY( circle_radius_sqr >= 0.f );
    float const circle_radius = _sqrt( circle_radius_sqr );
    Fvector const target_point = Fvector().mad(
        circle_center, center2projection_direction, circle_radius );
    Fvector const current_direction =
        Fvector().sub( current_point, bone_position ).normalize();
    Fvector const target_direction =
        Fvector().sub( target_point, bone_position ).normalize();
    ...
    Fmatrix transform0;
    {
        Fvector cross_product =
            Fvector().crossproduct( current_direction, target_direction );
        float const sin_alpha = cross_product.magnitude();
        if ( !fis_zero( sin_alpha ) ) {
            float const cos_alpha =
                current_direction.dotproduct( target_direction );
            transform0.rotation( cross_product.div( sin_alpha ),
                                 atan2f( sin_alpha, cos_alpha ) );
        } else {
            ...
            if ( fsimilar( _abs( dot_product ), 0.f ) )
                ...
                cross_product.crossproduct( current_direction,
                                            direction_target );
                transform0.rotation( cross_product.normalize(),
    ...
    Fmatrix transform1;
    {
        Fvector const new_direction =
            Fvector().sub( target, target_point ).normalize();
        Fvector old_direction;
        transform0.transform_dir( old_direction, weapon_direction );
        Fvector cross_product =
            Fvector().crossproduct( old_direction, new_direction );
        float const sin_alpha = cross_product.magnitude();
        if ( !fis_zero( sin_alpha ) ) {
            float const cos_alpha = old_direction.dotproduct( new_direction );
            transform1.rotation( cross_product.div( sin_alpha ),
                                 atan2f( sin_alpha, cos_alpha ) );
        } else {
            float const dot_product =
                current_direction.dotproduct( target_direction );
            if ( fsimilar( _abs( dot_product ), 0.f ) )
                ...
                VERIFY( fsimilar( _abs( dot_product ), 1.f ) );
                transform1.rotation( target_direction,
                                     dot_product > 0.f ? 0.f : PI );
            }
        }
    }
    ...
    if ( debug_draw ) {
        ...
        transform.transform_dir(
            temp.c, Fvector().sub( weapon_position, bone_position ) );
...
static void fill_bones( CAI_Stalker& self,
                        Fmatrix const& transform,
                        IKinematicsAnimated* kinematics_animated,
                        LPCSTR animation_id,
                        bool const local ) {
    ...
    for ( u16 i = 0; i < MAX_PARTS; ++i ) {
        ...
        for ( u32 j = 0; j < blend_count; ++j ) {
            ...
            CBlend* const new_blend = kinematics_animated->LL_PlayCycle(
                i, blend->motionID, TRUE, 0, 0, 1 );
            VERIFY( new_blend );
            *new_blend = *blend;
            new_blend->channel = 1;
        }
    ...
    for ( u16 i = 0; i < bone_count; ++i ) {
        ...
        bones.push_back( local ? matrix
                               : Fmatrix().mul_43( start_transform, matrix ) );
    }
...
static void _BCL test_callback( CBoneInstance* B ) {
    ...
    callback_param* params =
        static_cast< callback_param* >( B->callback_param() );
    VERIFY( params );
...
#ifdef DEBUG_RENDER
static void draw_bones( IKinematics& kinematics,
...
static void draw_animation_bones( CAI_Stalker& self,
                                  Fmatrix const& transform,
                                  IKinematicsAnimated* kinematics_animated,
                                  LPCSTR animation_id ) {
    IKinematics* kinematics = smart_cast< IKinematics* >( kinematics_animated );

    u16 spine_bone_id = ( u16 )kinematics->LL_BoneID(
        pSettings->r_string( self.cNameSect().c_str(), "bone_spin" ) );
    u16 shoulder_bone_id = ( u16 )kinematics->LL_BoneID(
        pSettings->r_string( self.cNameSect().c_str(), "bone_shoulder" ) );
    u16 weapon_bone_id0 = ( u16 )kinematics->LL_BoneID(
        pSettings->r_string( self.cNameSect().c_str(), "weapon_bone0" ) );
    u16 weapon_bone_id1 = ( u16 )kinematics->LL_BoneID(
    ...
    draw_bones( *kinematics, Fvector().set( .011f, .011f, .011f ),
                D3DCOLOR_XRGB( 0, 255, 0 ), D3DCOLOR_XRGB( 0, 255, 255 ),
                &transform );
#endif // #ifdef DEBUG_RENDER
    ...
    Fvector const vLoadedFirePoint =
        pSettings->r_fvector3( weapon->cNameSect(), "fire_point" );
    ...
    spine_offset =
        aim_on_actor( spine_bone.c, weapon_bone_position,
                      weapon_bone_direction.normalize(), target, false );

    callback_param param;
    param.transform = Fmatrix( transform )
                          .invert()
                          .mulB_43( spine_offset )
                          .mulB_43( Fmatrix( transform ) );
    {
        Fvector temp;
    ...
    kinematics->LL_GetBoneInstance( spine_bone_id )
        .set_callback( bctCustom, &test_callback, &param );
    ...
    shoulder_offset =
        aim_on_actor( shoulder_bone.c, weapon_bone_position,
                      weapon_bone_direction.normalize(), target, true );

    callback_param param2;
    param2.transform = Fmatrix( transform )
                           .invert()
                           .mulB_43( shoulder_offset )
                           .mulB_43( Fmatrix( transform ) );
    ;
    param2.xform = transform;
    param2.kinematics = kinematics;
    param2.bone_id = shoulder_bone_id;
    kinematics->LL_GetBoneInstance( shoulder_bone_id )
        .set_callback( bctCustom, &test_callback, &param2 );
    ...
    if ( self.inventory().ActiveItem() ) {
        CWeapon* weapon =
            smart_cast< CWeapon* >( self.inventory().ActiveItem() );
        if ( weapon ) {
            Fvector position = weapon->get_LastFP();
            Fvector direction = weapon->get_LastFD();
            renderer.draw_line( Fidentity, position,
                                Fvector().mad( position, direction,
    ...
    Fvector const weapon_position_target =
        Fvector().mad( weapon_bone_position, weapon_bone_direction,
                       weapon_bone_position.distance_to( target ) );
    ...
    draw_bones( *kinematics, Fvector().set( .01f, .01f, .01f ),
                D3DCOLOR_XRGB( 0, 0, 255 ), D3DCOLOR_XRGB( 255, 255, 0 ) );

    renderer.draw_obb( g_stalker_skeleton[ weapon_bone_id0 ],
                       Fvector().set( .01f, .01f, .01f ),
                       D3DCOLOR_XRGB( 255, 0, 0 ) );
    renderer.draw_obb( g_stalker_skeleton[ weapon_bone_id1 ],
                       Fvector().set( .01f, .01f, .01f ),
                       D3DCOLOR_XRGB( 255, 0, 0 ) );
    renderer.draw_line( Fidentity, g_stalker_skeleton[ weapon_bone_id0 ].c,
...
//	draw_animation_bones		(*this, XFORM(), kinematics, "loophole_3_attack_in_0");
    ...
    if ( inventory().ActiveItem() ) {
        ...
        Level().debug_renderer().draw_line(
            Fidentity, position, temp, D3DCOLOR_XRGB( 0 * 255, 255, 0 * 255 ) );
    }
    ...
    if ( IsMyCamera() ) {
        ...
        if ( std::find( objects.begin(), objects.end(),
                        memory().enemy().selected() ) != objects.end() ) {
            Fvector position = feel_vision_get_vispoint(
                const_cast< CEntityAlive* >( memory().enemy().selected() ) );
...

    {
        ...
        current_direction.setHP( -movement().m_head.current.yaw,
                                 -movement().m_head.current.pitch );
        current_direction.normalize();
        current_direction.mul( 4.f );
        current_direction.add( eye_matrix.c );
        target_direction.setHP( -movement().m_head.target.yaw,
                                -movement().m_head.target.pitch );
        target_direction.normalize();
        target_direction.mul( 4.f );
        target_direction.add( eye_matrix.c );
        target_direction.y = eye_matrix.c.y + 0.4f;

        Level().debug_renderer().draw_line( Fidentity, eye_matrix.c,
                                            current_direction,
                                            D3DCOLOR_XRGB( 255, 0, 255 ) );
        Level().debug_renderer().draw_line( Fidentity, eye_matrix.c,
                                            target_direction,
...

    {
        ...
        c0.y += 2.f;
        c1.setHP( -movement().m_body.current.yaw,
                  -movement().m_body.current.pitch );
        c1.add( c0 );
        Level().debug_renderer().draw_line( Fidentity, c0, c1,
                                            D3DCOLOR_XRGB( 0, 255, 0 ) );

        t0.y += 2.f;
        t1.setHP( -movement().m_body.target.yaw,
                  -movement().m_body.target.pitch );
        t1.add( t0 );
        Level().debug_renderer().draw_line( Fidentity, t0, t1,
                                            D3DCOLOR_XRGB( 255, 0, 0 ) );
    }

    if ( memory().danger().selected() &&
         ai().level_graph().valid_vertex_position(
             memory().danger().selected()->position() ) ) {
        ...
        position.y += 1.f;
        Level().debug_renderer().draw_aabb(
            position, half_size - .01f, 1.f,
            ai().level_graph().header().cell_size() * .5f - .01f,
            D3DCOLOR_XRGB( 0 * 255, 255, 0 * 255 ) );

        if ( ai().level_graph().valid_vertex_id( level_vertex_id ) ) {
            LevelGraph::CVertex* v =
                ai().level_graph().vertex( level_vertex_id );
            ...
            for ( u32 i = 0, j = 0; i < 36; ++i ) {
                float value = ai().level_graph().high_cover_in_direction(
                    float( 10 * i ) / 180.f * PI, v );
                ...
                Level().debug_renderer().draw_line(
                    Fidentity, position, direction,
                    D3DCOLOR_XRGB( 0, 0, 255 ) );
                value = ai().level_graph().compute_high_square(
                    float( 10 * i ) / 180.f * PI, PI / 2.f, v );
                if ( value > best_value ) {
                    best_value = value;
                    j = i;
                }
            ...
            direction.set(
                position.x - half_size * float( v->high_cover( 0 ) ) / 15.f,
                position.y, position.z );
            Level().debug_renderer().draw_line( Fidentity, position, direction,
                                                D3DCOLOR_XRGB( 255, 0, 0 ) );

            direction.set(
                position.x, position.y,
                position.z + half_size * float( v->high_cover( 1 ) ) / 15.f );
            Level().debug_renderer().draw_line( Fidentity, position, direction,
                                                D3DCOLOR_XRGB( 255, 0, 0 ) );

            direction.set(
                position.x + half_size * float( v->high_cover( 2 ) ) / 15.f,
                position.y, position.z );
            Level().debug_renderer().draw_line( Fidentity, position, direction,
                                                D3DCOLOR_XRGB( 255, 0, 0 ) );
            ...
            direction.set(
                position.x, position.y,
                position.z - half_size * float( v->high_cover( 3 ) ) / 15.f );
            Level().debug_renderer().draw_line( Fidentity, position, direction,
                                                D3DCOLOR_XRGB( 255, 0, 0 ) );

            float value = ai().level_graph().high_cover_in_direction(
                float( 10 * j ) / 180.f * PI, v );
            ...
            // low
            {
                ...
                for ( u32 i = 0, j = 0; i < 36; ++i ) {
                    float value = ai().level_graph().low_cover_in_direction(
                        float( 10 * i ) / 180.f * PI, v );
                    direction.setHP( float( 10 * i ) / 180.f * PI, 0 );
                    direction.normalize();
                    direction.mul( value * half_size );
                    ...
                    Level().debug_renderer().draw_line(
                        Fidentity, position, direction,
                        D3DCOLOR_XRGB( 0, 0, 255 ) );
                    value = ai().level_graph().compute_low_square(
                        float( 10 * i ) / 180.f * PI, PI / 2.f, v );
                    if ( value > best_value ) {
                        best_value = value;
                        j = i;
                ...
                direction.set(
                    position.x - half_size * float( v->low_cover( 0 ) ) / 15.f,
                    position.y, position.z );
                Level().debug_renderer().draw_line(
                    Fidentity, position, direction,
                    D3DCOLOR_XRGB( 255, 0, 0 ) );

                direction.set( position.x, position.y,
                ...
                direction.set(
                    position.x + half_size * float( v->low_cover( 2 ) ) / 15.f,
                    position.y, position.z );
                Level().debug_renderer().draw_line(
                    Fidentity, position, direction,
                    D3DCOLOR_XRGB( 255, 0, 0 ) );
                ...
                direction.y = position.y;
                Level().debug_renderer().draw_line(
                    Fidentity, position, direction, D3DCOLOR_XRGB( 0, 0, 0 ) );
            }
        }
    }
