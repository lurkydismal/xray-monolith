...
void CAI_Stalker::subscribe_on_best_cover_changed(
    const on_best_cover_changed_delegate& delegate ) {
    VERIFY( m_cover_delegates.end() == std::find( m_cover_delegates.begin(),
                                                  m_cover_delegates.end(),
                                                  delegate ) );
    m_cover_delegates.push_back( delegate );
}

void CAI_Stalker::unsubscribe_on_best_cover_changed(
    const on_best_cover_changed_delegate& delegate ) {
    cover_delegates::iterator I = std::find(
        m_cover_delegates.begin(), m_cover_delegates.end(), delegate );
    VERIFY( I != m_cover_delegates.end() );
    m_cover_delegates.erase_fast( I );
}

void CAI_Stalker::on_best_cover_changed( const CCoverPoint* new_cover,
...

    {
        ...
        if ( ai().script_engine().functor( "_G.CAI_Stalker__OnBestCoverChanged",
                                           funct ) ) {
            const bool is_smart = new_cover && new_cover->m_is_smart_cover;
            LPCSTR cover_name = "";
            if ( is_smart )
                cover_name =
                    *static_cast< smart_cover::cover const* >( new_cover )
                         ->object()
                         .cName();
            funct( lua_game_object(),
                   new_cover ? &new_cover->position() : nullptr, is_smart,
                   cover_name );
        }
    }
}

void CAI_Stalker::compute_enemy_distances( float& minimum_enemy_distance,
                                           float& maximum_enemy_distance ) {
...

    {
        ...
        if ( ai().script_engine().functor( "_G.CAI_Stalker__GetMinCombatDist",
                                           min_funct ) ) {
            float result =
                min_funct( lua_game_object(), minimum_enemy_distance );
            if ( _valid( result ) && result >= 0.f )
                minimum_enemy_distance = result;
            else
                ai().script_engine().script_log(
                    ScriptStorage::eLuaMessageTypeError,
                    "CAI_Stalker [%s]: _g.CAI_Stalker__GetMinCombatDist "
                    "returned invalid value %f, ignoring",
                    *cName(), result );
        }
    }
    {
        ::luabind::functor< float > max_funct;
        if ( ai().script_engine().functor( "_G.CAI_Stalker__GetMaxCombatDist",
                                           max_funct ) ) {
            float result =
                max_funct( lua_game_object(), maximum_enemy_distance );
            if ( _valid( result ) && result >= 0.f )
                maximum_enemy_distance = result;
            else
                ai().script_engine().script_log(
                    ScriptStorage::eLuaMessageTypeError,
                    "CAI_Stalker [%s]: _g.CAI_Stalker__GetMaxCombatDist "
                    "returned invalid value %f, ignoring",
                    *cName(), result );
        }
    }
...
const CCoverPoint* CAI_Stalker::find_best_cover(
    ...
    m_ce_best->can_use_smart_covers( best_wpn && best_wpn->cast_weapon() &&
                                     best_wpn->BaseSlot() == INV_SLOT_3 );

    m_ce_best->setup( position_to_cover_from, minimum_enemy_distance,
                      maximum_enemy_distance, minimum_enemy_distance );
    const CCoverPoint* point = ai().cover_manager().best_cover(
        Position(), g_ai_cover_search_near_radius, *m_ce_best,
    ...
    point = ai().cover_manager().best_cover(
        Position(), g_ai_cover_search_far_radius, *m_ce_best,
        CStalkerMovementRestrictor( this, true ) );
    return ( point );
}

float CAI_Stalker::best_cover_value( const Fvector& position_to_cover_from ) {
    m_ce_best->setup( position_to_cover_from, MIN_SUITABLE_ENEMY_DISTANCE,
                      170.f, MIN_SUITABLE_ENEMY_DISTANCE );
    m_ce_best->initialize( Position(), true );
    m_ce_best->evaluate(
        m_best_cover,
        CStalkerMovementRestrictor( this, true ).weight( m_best_cover ) );
...
void CAI_Stalker::update_best_cover_actuality(
    const Fvector& position_to_cover_from ) {
    if ( !m_best_cover_actual )
        return;
    ...
    if ( m_best_cover->m_is_smart_cover ) {
        ...
        smart_cover::cover const* cover =
            static_cast< smart_cover::cover const* >( m_best_cover );
        smart_cover::loophole* loophole = cover->best_loophole(
            position_to_cover_from, value, false,
    ...
    if ( m_best_cover->position().distance_to_sqr( position_to_cover_from ) <
         _sqr( MIN_SUITABLE_ENEMY_DISTANCE ) ) {
        m_best_cover_actual = false;
#if 0 // def _DEBUG
    ...
    //		m_best_cover_actual				= false;
    //		Msg
    //("* [%6d][%s] cover became too bad2",Device.dwTimeGlobal,*cName());
    //		return;
    //	}
...
#ifdef _DEBUG
    //	Msg
    //("* [%6d][%s] advance search performed",Device.dwTimeGlobal,*cName());
#endif
#ifdef _DEBUG
    ...
    m_ce_best->setup( position_to_cover_from, MIN_SUITABLE_ENEMY_DISTANCE,
                      170.f, MIN_SUITABLE_ENEMY_DISTANCE );
    m_best_cover = ai().cover_manager().best_cover(
        Position(), 10.f, *m_ce_best,
        CStalkerMovementRestrictor( this, true ) );
}
...
const CCoverPoint* CAI_Stalker::best_cover(
    ...
    if ( best_cover != m_best_cover ) {
        // Let Lua veto replacing the held cover before on_best_cover_changed
        // clears the cover props. allow = false keeps m_best_cover. Only when a
        // cover is already held; no functor means allow.
        bool allow = true;
        if ( m_best_cover ) {
            luabind::functor< bool > repick_funct;
            if ( ai().script_engine().functor(
                     "_G.CAI_Stalker__AllowCoverRepick", repick_funct ) )
    ...
    m_best_cover_value =
        m_best_cover ? best_cover_value( position_to_cover_from ) : flt_max;
...
#ifdef _DEBUG
    //	Msg
    //("* [%6d][%s] on_enemy_change",Device.dwTimeGlobal,*cName());
#endif
}

void CAI_Stalker::on_danger_location_add( const CDangerLocation& location ) {
    const bool in_open = !m_best_cover;
    const bool cover_threatened =
        !in_open && m_best_cover->position().distance_to_sqr(
                        location.position() ) <= _sqr( location.m_radius );

    if ( cover_threatened ) {
#ifdef _DEBUG
        //		Msg
        //("* [%6d][%s] on_danger_add",Device.dwTimeGlobal,*cName());
#endif
        m_best_cover_actual = false;
    }

    {
        ::luabind::functor< void > funct;
        if ( ai().script_engine().functor(
                 "_G.CAI_Stalker__OnDangerLocationAdd", funct ) )
            funct( lua_game_object(), location.position(), location.m_radius,
                   in_open, cover_threatened );
    }
}

void CAI_Stalker::on_danger_location_remove( const CDangerLocation& location ) {
    if ( !m_best_cover ) {
        if ( Position().distance_to_sqr( location.position() ) <=
             _sqr( location.m_radius ) ) {
#ifdef _DEBUG
            //			Msg
            //("* [%6d][%s] on_danger_remove",Device.dwTimeGlobal,*cName());
#endif
            m_best_cover_actual = false;
        }
    ...
    if ( m_best_cover->position().distance_to_sqr( location.position() ) <=
         _sqr( location.m_radius ) ) {
#ifdef _DEBUG
        //		Msg
        //("* [%6d][%s] on_danger_remove",Device.dwTimeGlobal,*cName());
...
#ifdef _DEBUG
    //	Msg
    //("* [%6d][%s] cover is blocked",Device.dwTimeGlobal,*cName());
#endif
    m_best_cover_actual = false;
}
