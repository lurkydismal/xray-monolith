...
bool CAI_Stalker::useful( const CItemManager* manager,
    ...
    if ( explosive && smart_cast< const CInventoryItem* >( object ) )
        agent_manager().location().add( xr_new< CDangerObjectLocation >(
            object, Device.dwTimeGlobal, DANGER_INFINITE_INTERVAL,
    ...
    if ( explosive && ( explosive->CurrentParentID() != 0xffff ) ) {
        ...
        CEntityAlive* entity_alive = smart_cast< CEntityAlive* >(
            Level().Objects.net_Find( explosive->CurrentParentID() ) );
        if ( entity_alive )
            memory().danger().add( CDangerObject(
                entity_alive, object->Position(), Device.dwTimeGlobal,
    ...
    CInventoryItem* inventory_item_non_const = const_cast< CInventoryItem* >(
        smart_cast< const CInventoryItem* >( object ) );
...
float CAI_Stalker::evaluate( const CItemManager* manager,
                             const CGameObject* object ) const {
...
bool CAI_Stalker::useful( const CEnemyManager* manager,
...
ALife::ERelationType CAI_Stalker::tfGetRelationType(
    const CEntityAlive* tpEntityAlive ) const {
    const CInventoryOwner* pOtherIO =
        smart_cast< const CInventoryOwner* >( tpEntityAlive );

    ALife::ERelationType relation = ALife::eRelationTypeDummy;

    if ( pOtherIO && !( const_cast< CEntityAlive* >( tpEntityAlive )
                            ->cast_base_monster() ) )
        relation = RELATION_REGISTRY().GetRelationType(
            static_cast< const CInventoryOwner* >( this ), pOtherIO );
...
void CAI_Stalker::react_on_grenades() {
    CMemberOrder::CGrenadeReaction& reaction =
        agent_manager().member().member( this ).grenade_reaction();
    ...
    //	u32							interval =
    // AFTER_GRENADE_DESTROYED_INTERVAL;
    const CMissile* missile =
        smart_cast< const CMissile* >( reaction.m_grenade );
    //	if (missile && (missile->destroy_time() > Device.dwTimeGlobal))
    //		interval				=
    // missile->destroy_time() - Device.dwTimeGlobal +
    // AFTER_GRENADE_DESTROYED_INTERVAL;
    ...
    if ( missile && agent_manager().member().group_behaviour() ) {
        //		Msg						("%6d :
        // Stalker %s : grenade
        // reaction",Device.dwTimeGlobal,*m_object->cName());
        ...
        */
        if ( initiator ) {
            ...
            else if ( missile->Position().distance_to( Position() ) <
                      FRIENDLY_GRENADE_ALARM_DIST ) {
                u32 const time =
                    missile->destroy_time() >= Device.dwTimeGlobal
...
void CAI_Stalker::react_on_member_death() {
    CMemberOrder::CMemberDeathReaction& reaction =
        agent_manager().member().member( this ).member_death_reaction();
    if ( !reaction.m_processing )
        return;
...

    {
        ...
        if ( ai().script_engine().functor(
                 "_G.CAI_Stalker__OnMemberDeathReaction", funct ) )
            funct( lua_game_object(),
                   reaction.m_member ? reaction.m_member->lua_game_object()
                                     : nullptr,
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( !member->memory().enemy().selected() ) {
            if ( !memory().danger().selected() &&
                 member->memory().danger().selected() )
                memory().danger().add( *member->memory().danger().selected() );
            continue;
        }

        // Alundaio: Only transfer enemy if I can see member at this very
        // moment!
        if ( !memory().visual().visible_now( member ) )
            continue;
        // Alundaio: END

        memory().make_object_visible_somewhen(
            member->memory().enemy().selected() );
        break;
    }
}
