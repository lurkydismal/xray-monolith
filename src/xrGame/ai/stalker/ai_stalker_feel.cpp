...
bool CAI_Stalker::bfCheckForNodeVisibility( u32 dwNodeID, bool bIfRayPick ) {
    return ( memory().visual().visible( dwNodeID, movement().m_head.current.yaw,
                                        ffGetFov() ) );
}
...
bool CAI_Stalker::feel_touch_contact( CObject* O ) {
    ...
    // demonized: add g_ai_die_in_anomaly == 0 and
    // m_enable_anomalies_pathfinding check when 0 - disable pathfinding around
    // anomaly
    if ( !( g_ai_die_in_anomaly || m_enable_anomalies_pathfinding ) ) {
        CCustomZone* sr = smart_cast< CCustomZone* >( O );
        if ( sr &&
             ( sr->SpatialComponent->spatial.type & STYPE_VISIBLEFORAI ) ) {
            return false;
        }
    }
...
bool CAI_Stalker::feel_touch_on_contact( CObject* O ) {
    ...
    // demonized: add g_ai_die_in_anomaly == 0 and m_enable_anomalies_damage
    // check when 0 - prevent any damage from anomalies
...
void CAI_Stalker::feel_touch_delete( CObject* O ) {
    ignored_touched_objects_type::iterator i = std::find(
        m_ignored_touched_objects.begin(), m_ignored_touched_objects.end(), O );
