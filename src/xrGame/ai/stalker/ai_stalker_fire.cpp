    ...
    if ( !movement().path_completed() ) {
        if ( movement().movement_type() == eMovementTypeWalk ) {
            if ( movement().body_state() == eBodyStateStand ) {
                base *= ( m_disp_walk_stand * g_dispersion_factor +
                          g_dispersion_base );
            } else {
                base *= ( m_disp_walk_crouch * g_dispersion_factor +
                          g_dispersion_base );
            }
        } else if ( movement().movement_type() == eMovementTypeRun ) {
            if ( movement().body_state() == eBodyStateStand ) {
                base *= ( m_disp_run_stand * g_dispersion_factor +
                          g_dispersion_base );
            } else {
                base *= ( m_disp_run_crouch * g_dispersion_factor +
                          g_dispersion_base );
            }
        }
    } else {
        ...
        if ( movement().body_state() == eBodyStateStand ) {
            if ( zoom_state() && hasScope ) {
                base *= ( m_disp_stand_stand_zoom * g_dispersion_factor +
                          g_dispersion_base );
            } else {
                base *= ( m_disp_stand_stand * g_dispersion_factor +
                          g_dispersion_base );
            }
        } else if ( movement().body_state() == eBodyStateCrouch ) {
            if ( zoom_state() && hasScope ) {
                base *= ( m_disp_stand_crouch_zoom * g_dispersion_factor +
                          g_dispersion_base );
            } else {
                base *= ( m_disp_stand_crouch * g_dispersion_factor +
                          g_dispersion_base );
            }
        } else
            base *= ( m_disp_run_stand * g_dispersion_factor +
                      g_dispersion_base ); // fallback to worst aim if state
                                           // could not determined, this should
                                           // never happen (tm)
    }

    ::luabind::functor< float > func;
    if ( ai().script_engine().functor( "_g.CAI_Stalker__GetWeaponAccuracy",
                                       func ) ) {
        base =
            func( lua_game_object(), W ? W->lua_game_object() : nullptr, base,
                  movement().body_state(), movement().movement_type() );
    }

    return base;
}

void CAI_Stalker::g_fireParams( const CHudItem* pHudItem,
                                Fvector& P,
                                Fvector& D ) {
    //.	VERIFY				(inventory().ActiveItem());
    if ( !inventory().ActiveItem() ) {
#ifdef DEBUG
        Msg( "! CAI_Stalker::g_fireParams() : "
             "VERIFY(inventory().ActiveItem())" );
#endif // DEBUG
        P = Position();
    ...
    if ( !animation().script_animations().empty() ||
        ...
        if ( sniper_fire_mode() )
            D.setHP( -movement().m_head.target.yaw,
    ...
    switch ( movement().body_state() ) {
        case eBodyStateStand: {
            if ( movement().movement_type() == eMovementTypeStand ) {
                ...
                D.setHP( -movement().m_head.current.yaw,
...
void CAI_Stalker::g_WeaponBones( int& L, int& R1, int& R2 ) {
    ...
    R2 = r_finger2;
    if ( !animation().script_animations().empty() &&
         animation().script_animations().front().hand_usage() )
        L = R2;
    else
        L = l_finger1;
}
...
void CAI_Stalker::Hit( SHit* pHDS ) {
    // хит может меняться в зависимости от ранга (новички получают больше хита,
    // чем ветераны)
    SHit HDS = *pHDS;
    HDS.add_wound = true;
    ...
    //{
    //     LPCSTR bone_name =
    //     smart_cast<IKinematics*>(this->Visual())->LL_BoneName_dbg(HDS.boneID);
    ...
    if ( Core.ParamsData.test( ECoreParams::dbgbullet ) )
        Msg( "CAI_Stalker::Hit hit_type=%d | hit_power(%f)*m_fRankImmunity(%f) "
             "= %f",
             ( u32 )HDS.hit_type, HDS.power, m_fRankImmunity, hit_power );
    ...
    if ( m_boneHitProtection && HDS.hit_type == ALife::eHitTypeFireWound ) {
        ...
        if ( !fis_zero( BoneArmor, EPS ) ) {
            if ( ap > BoneArmor ) {
                float d_hit_power =
                    ( ap - BoneArmor ) / ( ap * m_boneHitProtection->APScale );
                ...
                if ( Core.ParamsData.test( ECoreParams::dbgbullet ) )
                    Msg( "CAI_Stalker::Hit AP(%f) > BoneArmor(%f) "
                         "[HitFracNpc=%f] modified hit_power=%f",
                         ap, BoneArmor, m_boneHitProtection->m_fHitFracNpc,
                         hit_power );
            } else {
    ...
    if ( g_Alive() ) {
        {
            const CEntityAlive* entity_alive =
                smart_cast< const CEntityAlive* >( pHDS->initiator() );
            if ( entity_alive && entity_alive->g_Alive() &&
                 tfGetRelationType( entity_alive ) ==
                     ALife::eRelationTypeEnemy ) {
                movement().set_mental_state( eMentalStateDanger );
                if ( !memory().visual().visible_now( entity_alive ) ) {
                    CNotYetVisibleObject new_object;
                    new_object.m_object =
                        smart_cast< const CGameObject* >( pHDS->initiator() );
                    ...
                    memory().visual().add_visible_object( pHDS->initiator(),
                                                          Device.fTimeDelta );
                }
            }
        }
        ...
        if ( !already_critically_wounded ) {
            const CCoverPoint* cover =
                agent_manager().member().member( this ).cover();
            if ( !invulnerable() && cover && HDS.initiator() &&
                ...
                agent_manager().location().add( xr_new< CDangerCoverLocation >(
                    cover, Device.dwTimeGlobal, g_ai_cover_danger_time,
                    g_ai_cover_danger_radius ) );
            }
        }

        const CEntityAlive* entity_alive =
            smart_cast< const CEntityAlive* >( HDS.initiator() );
        ...
        if ( !wounded() && !already_critically_wounded ) {
            bool became_critically_wounded =
...
#pragma todo( \
    "Dima to Dima : forward-back bone impulse direction has been determined incorrectly!" )
                ...
                IKinematics* tpKinematics =
                    smart_cast< IKinematics* >( Visual() );
#ifdef DEBUG
                ...
                if ( !already_critically_wounded &&
                    ...
                    if ( HDS.who ) {
                        CAI_Stalker* stalker =
                            smart_cast< CAI_Stalker* >( HDS.who );
...

                    {
                        ...
                        if ( ai().script_engine().functor(
                                 "_G.CAI_Stalker__OnCriticallyWounded",
                                 funct ) ) {
                            CScriptHit tLuaHit( &HDS );
                            funct( this->lua_game_object(), &tLuaHit,
                                   HDS.boneID );
                        }
                    }
                }
    ...
    if ( g_Alive() && ( !m_hit_callback || m_hit_callback( &HDS ) ) ) {
        ...
        if ( ai().script_engine().functor( "_G.CAI_Stalker__BeforeHitCallback",
                                           funct ) ) {
        ...
        memory().hit().add( damage_factor * HDS.damage(), HDS.direction(),
                            HDS.who, HDS.boneID );
    }
...
void CAI_Stalker::HitSignal( float amount,
                             Fvector& vLocalDir,
                             CObject* who,
                             s16 element ) {}
...
void CAI_Stalker::OnItemDrop( CInventoryItem* inventory_item,
    ...
    if ( inventory().ActiveItem() &&
         ( inventory().ActiveItem() != inventory_item ) )
        return;
...
void CAI_Stalker::update_best_item_info_impl() {
    ...
    if ( ai().script_engine().functor( "_g.update_best_weapon", funct ) ) {
        CGameObject* cur_itm =
            smart_cast< CGameObject* >( m_best_item_to_kill );
        CScriptGameObject* GO =
            funct( this->lua_game_object(),
                   cur_itm ? cur_itm->lua_game_object() : NULL );
        CInventoryItem* bw =
            GO ? smart_cast< CInventoryItem* >( &GO->object() ) : NULL;
        if ( bw ) {
    ...
    /* Alundaio: This is what causes stalkers to switch weapons during combat;
    It's stupid if	( m_item_actuality && m_best_item_to_kill &&
            ...
            ai().ef_storage().non_alife().enemy()	=
    memory().enemy().selected() ? memory().enemy().selected() : this;
            ai().ef_storage().non_alife().member_item()	=
    &m_best_item_to_kill->object(); float
    value; value
    = ai().ef_storage().m_pfWeaponEffectiveness->ffGetValue(); if
    (fsimilar(value,m_best_item_value)) return;
    }
    */
    ...
    // try to find the best item which can kill
    {
        ...
        for ( ; I != E; ++I ) {
            if ( ( *I )->can_kill() ) {
                ...
                if ( memory().enemy().selected() )
                    value =
                        ai().ef_storage().m_pfWeaponEffectiveness->ffGetValue();
                else
                    value = ( float )( *I )->Cost();

                if ( !fsimilar( value, m_best_item_value ) &&
                     ( value < m_best_item_value ) )
                    continue;
                ...
                if ( m_best_item_to_kill &&
                     ( ( *I )->Cost() <= m_best_item_to_kill->Cost() ) )
                    continue;
    ...
    {
        xr_vector< const CGameObject* >::const_iterator I =
            memory().item().objects().begin();
        xr_vector< const CGameObject* >::const_iterator E =
            memory().item().objects().end();
        for ( ; I != E; ++I ) {
            const CInventoryItem* inventory_item =
                smart_cast< const CInventoryItem* >( *I );
            if ( !inventory_item ||
                 !memory().item().useful( &inventory_item->object() ) )
                continue;
    ...
    for ( ; I != E; ++I ) {
        ...
        const CInventoryItem* item =
            inventory_item->can_kill( memory().item().objects() );
        if ( item ) {
            ai().ef_storage().non_alife().member_item() =
                &inventory_item->object();
            float value =
...
bool CAI_Stalker::ready_to_kill() {
    return ( m_best_item_to_kill && inventory().ActiveItem() &&
             ( inventory().ActiveItem()->object().ID() ==
               m_best_item_to_kill->object().ID() ) &&
...
bool CAI_Stalker::ready_to_detour() {
    ...
    return ( ( float )weapon->GetAmmoElapsed() >
             ( float )weapon->GetAmmoMagSize() * g_ai_reload_threshold );
}
...
public:
    IC ray_query_param( const CAI_Stalker* holder,
...
void CAI_Stalker::can_kill_entity( const Fvector& position,
    ...
    ray_query_param params( this, memory().visual().transparency_threshold(),
                            distance );

    Level().ObjectSpace.RayQuery( rq_storage, ray_defs, ray_query_callback,
                                  &params, NULL, this );
...
void CAI_Stalker::can_kill_entity_from( const Fvector& position,
                                        Fvector direction,
                                        float distance ) {
    m_pick_distance = 0.f;
    rq_storage.r_clear();
...
IC float CAI_Stalker::start_pick_distance() const {
    ...
    return ( _max( result, memory().enemy().selected()->Position().distance_to(
                               Position() ) +
                               1.f ) );
}
...
bool CAI_Stalker::undetected_anomaly() {
    return ( inside_anomaly() ||
             brain().CStalkerPlanner::m_storage.property(
                 StalkerDecisionSpace::eWorldPropertyAnomaly ) );
}
...
bool CAI_Stalker::zoom_state() const {
    ...
    if ( ( movement().movement_type() != eMovementTypeStand ) &&
         ( movement().body_state() != eBodyStateCrouch ) &&
...
void CAI_Stalker::update_range_fov( float& new_range,
                                    float& new_fov,
                                    float start_range,
                                    float start_fov ) {
    ...
    VERIFY2( start_fov < 180.f,
             make_string( "[%s] %f", cName().c_str(), start_fov ) );
    VERIFY2( fov < 180.f,
             make_string(
                 "fix addon multiplier for weapon or scope %s (fov=%f)",
...
bool CAI_Stalker::fire_make_sense() {
    ...
    if ( ( pick_distance() + g_ai_fire_range_extension ) <
         Position().distance_to( enemy->Position() ) )
        return ( false );

    if ( _abs( Position().y - enemy->Position().y ) >
         g_ai_fire_max_height_diff )
        return ( false );
    ...
    u32 last_time_seen =
        memory().visual().visible_object_time_last_seen( enemy );
...
void CAI_Stalker::on_weapon_shot_start( CWeapon* weapon ) {
    ...
    {
        ::luabind::functor< void > funct;
        if ( ai().script_engine().functor( "_G.CAI_Stalker__OnWeaponShotStart",
                                           funct ) )
            funct( lua_game_object(),
                   weapon ? weapon->lua_game_object() : nullptr );
    }
}
...
void CAI_Stalker::on_weapon_shot_stop() {
    {
        ::luabind::functor< void > funct;
        if ( ai().script_engine().functor( "_G.CAI_Stalker__OnWeaponShotStop",
                                           funct ) )
            funct( lua_game_object() );
    }
}
...
float CAI_Stalker::missile_throw_force() {
    ...
    if ( ai().script_engine().functor( "_G.CAI_Stalker__GetMissileThrowForce",
                                       funct ) ) {
        ...
        if ( _valid( result ) && result > 0.f )
            ...
            ai().script_engine().script_log(
                ...
                "CAI_Stalker [%s]: _g.CAI_Stalker__GetMissileThrowForce "
                "returned invalid value %f (must be > 0), ignoring",
                *cName(), result );
    }
...
void CAI_Stalker::compute_throw_miss( u32 const vertex_id ) {
    ...
    for ( u32 i = 0; i < try_count; ++i ) {
        ...
        Fvector const check_position = Fvector().mad(
            m_throw_target_position, direction, throw_miss_radius );
...
void CAI_Stalker::throw_target_impl( const Fvector& position,
                                     CObject* throw_ignore_object ) {
...
bool CAI_Stalker::throw_check_error( float low,
                                     float high,
...
void CAI_Stalker::check_throw_trajectory( const float& throw_time ) {
    ...
    if ( !trajectory_intersects_geometry(
             throw_time, m_throw_position, m_throw_target_position,
             m_throw_velocity, m_throw_collide_position, this,
             m_throw_ignore_object, rq_storage, trajectory_picks, collide_tris,
             box_size ) ) {
...
void CAI_Stalker::update_throw_params() {
    ...
    CMissile* const pMissile =
        fast_dynamic_cast< CMissile* >( inventory().ActiveItem() );
    if ( pMissile ) {
        static const LPCSTR third_person_offset_id =
            "third_person_throw_point_offset";

        if ( !pSettings->line_exist( pMissile->cNameSect(),
                                     third_person_offset_id ) ) {
            ...
            m_throw_position.add( pSettings->r_fvector3(
                pMissile->cNameSect(), third_person_offset_id ) );
    }
#endif

    static float const distances[] = { 30.f, 40.f, 50.f, 60.f };
    VERIFY( g_SingleGameDifficulty <
    ...
    float time =
        ThrowMinVelTime( m_throw_velocity, physics_world()->Gravity() );
    TransferenceToThrowVel( m_throw_velocity, time,
...
bool CAI_Stalker::critically_wounded() {
    ...
    if ( !brain().CStalkerPlanner::m_storage.property(
...
void CAI_Stalker::remove_critical_hit() {
    ...
    animation().global().remove_callback( CStalkerAnimationPair::CALLBACK_ID(
        this, &CAI_Stalker::remove_critical_hit ) );
}

void CAI_Stalker::critical_wounded_state_start() {
    brain().CStalkerPlanner::m_storage.set_property(
        StalkerDecisionSpace::eWorldPropertyCriticallyWounded, true );

    animation().global().add_callback( CStalkerAnimationPair::CALLBACK_ID(
        this, &CAI_Stalker::remove_critical_hit ) );
}
...
bool CAI_Stalker::can_cry_enemy_is_wounded() const {
    ...
    if ( brain().current_action_id() !=
         StalkerDecisionSpace::eWorldOperatorCombatPlanner )
        return ( false );
    ...
    planner_type* planner =
        smart_cast< planner_type* >( &brain().current_action() );
    VERIFY( planner );
...
void CAI_Stalker::on_critical_wound_initiator(
    const CAI_Stalker* critically_wounded ) {
    if ( !g_Alive() )
        return;
...
void CAI_Stalker::on_enemy_wounded_or_killed(
    const CAI_Stalker* wounded_or_killed ) {
    if ( !g_Alive() )
        return;
...

    {
        ::luabind::functor< void > funct;
        if ( ai().script_engine().functor(
                 "_G.CAI_Stalker__OnEnemyWoundedOrKilled", funct ) ) {
            if ( wounded_or_killed )
                funct( lua_game_object(),
                       wounded_or_killed->lua_game_object() );
            else
                ai().script_engine().script_log(
                    ScriptStorage::eLuaMessageTypeError,
                    "CAI_Stalker [%s]: OnEnemyWoundedOrKilled called with null "
                    "victim, skipping Lua hook",
                    *cName() );
        }
    }
}
