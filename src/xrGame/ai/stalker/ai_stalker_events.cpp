    ...
    switch ( type ) {
        ...
        case GE_OWNERSHIP_TAKE: {
            ...
            if ( inventory().CanTakeItem(
                     smart_cast< CInventoryItem* >( _O ) ) ) {
                O->H_SetParent( this );
                inventory().Take( _O, true, false );
                if ( !inventory().ActiveItem() && GetScriptControl() &&
                     smart_cast< CShootingObject* >( O ) )
...
#ifndef SILENCE
                Msg( "TAKE - can't take! - Dropping for valid server "
                     "information %s (%d)",
                     *O->cName(), O->ID() );
#endif
            }
...
void CAI_Stalker::feel_touch_new( CObject* O ) {
    ...
    if ( ( O->SpatialComponent->spatial.type | STYPE_VISIBLEFORAI ) !=
         O->SpatialComponent->spatial.type )
        return;
    ...
    if ( !wounded() && !critically_wounded() && I && I->useful_for_NPC() &&
         can_take( I ) ) {
#ifndef SILENCE
        Msg( "Taking item %s (%d)!", I->object().cName().c_str(),
             I->object().ID() );
#endif
        // Tosox
        ...
        if ( ai().script_engine().functor(
                 "_G.CAI_Stalker__OnBeforeOwnershipTake", func ) ) {
            if ( !func( lua_game_object(),
                        I->cast_game_object()->lua_game_object() ) ) {
                return;
            }
        }
    ...
    VERIFY2(
        std::find( m_ignored_touched_objects.begin(),
                   m_ignored_touched_objects.end(),
                   O ) == m_ignored_touched_objects.end(),
        make_string( "object %s is already in ignroed touched objects list",
                     O->cName().c_str() ) );
