...
void CPhantom::Load( LPCSTR section ) {
    ...
    LPCSTR snd_name = 0;
    m_state_data[ stBirth ].particles =
        pSettings->r_string( section, "particles_birth" );
    snd_name = pSettings->r_string( section, "sound_birth" );
    if ( snd_name && snd_name[ 0 ] )
        m_state_data[ stBirth ].sound.create( snd_name, st_Effect,
                                              sg_SourceType );

    m_state_data[ stFly ].particles =
        pSettings->r_string( section, "particles_fly" );
    snd_name = pSettings->r_string( section, "sound_fly" );
    if ( snd_name && snd_name[ 0 ] )
        m_state_data[ stFly ].sound.create( snd_name, st_Effect,
                                            sg_SourceType );

    m_state_data[ stContact ].particles =
        pSettings->r_string( section, "particles_contact" );
    snd_name = pSettings->r_string( section, "sound_contact" );
    if ( snd_name && snd_name[ 0 ] )
        m_state_data[ stContact ].sound.create( snd_name, st_Effect,
                                                sg_SourceType );

    m_state_data[ stShoot ].particles =
        pSettings->r_string( section, "particles_shoot" );
    snd_name = pSettings->r_string( section, "sound_shoot" );
    if ( snd_name && snd_name[ 0 ] )
        m_state_data[ stShoot ].sound.create( snd_name, st_Effect,
                                              sg_SourceType );
}

BOOL CPhantom::net_Spawn( CSE_Abstract* DC ) {
    CSE_ALifeCreaturePhantom* OBJ =
        smart_cast< CSE_ALifeCreaturePhantom* >( DC );
    VERIFY( OBJ );
    ...
    SwitchToState(
        stBirth ); // initial state (changed on load method in inherited::)

    OBJ->set_killer_id( u16(
        -1 ) ); // Alundaio: Hack to prevent strange crash with dynamic phantoms
    ...
    VERIFY( K->LL_GetMotionDef( m_state_data[ stBirth ].motion )->flags &
            esmStopAtEnd );
    VERIFY( K->LL_GetMotionDef( m_state_data[ stContact ].motion )->flags &
            esmStopAtEnd );
    VERIFY( K->LL_GetMotionDef( m_state_data[ stShoot ].motion )->flags &
            esmStopAtEnd );
    ...
    if ( new_state != m_CurState ) {
        ...
        switch ( new_state ) {
            case stBirth: {
                ...
                K->PlayCycle( sdata.motion, TRUE, animation_end_callback,
                              this );
            } break;
            case stFly: {
...
void CPhantom::OnFlyState() {
    ...
    if ( g_Alive() ) {
        ...
        if ( vP.distance_to_sqr( vE ) < _sqr( Radius() + m_enemy->Radius() ) ) {
            ...
            SHit HDS( power, Fvector().set( 0, 0, 1 ), this, BI_NONE,
                      Fvector().set( 0, 0, 0 ), impulse,
...
void CPhantom::UpdateFlyMedia() {
    ...
    if ( m_fly_particles ) {
        Fvector vel;
        vel.sub( m_enemy->Position(), Position() )
            .normalize_safe()
            .mul( fSpeed );
...
//---------------------------------------------------------------------
// void CPhantom::Hit	(float P, Fvector &dir, CObject* who, s16
...
void CPhantom::Hit( SHit* pHDS ) {
    ...
    if ( g_Alive() ) {
        //		SetfHealth		(-1.f);
        //		inherited::Hit
        //(P,dir,who,element,p_in_object_space,impulse/100.f, hit_type);
        inherited::Hit( pHDS );
    }
}
...
CParticlesObject* CPhantom::PlayParticles( const shared_str& name,
                                           BOOL bAutoRemove,
                                           const Fmatrix& xform ) {
    CParticlesObject* ps =
        Particles::Details::Create( name.c_str(), bAutoRemove ).get();
...
void CPhantom::PsyHit( const CObject* object, float value ) {
    ...
    HS.boneID = ( BI_NONE );                // bone			//	P.w_s16
                                            // (BI_NONE);
    ...
    HS.hit_type =
        ( ALife::eHitTypeTelepatic ); //	P.w_u16
                                      //(u16(ALife::eHitTypeTelepatic));
    HS.Write_Packet( P );
