...
bool bfGetActionSuccessProbability(
    ...
    int i = 0, j = 0, I = ( int )Members.size(),
        J = ( int )VisibleEnemies.size();
    xr_vector< const CEntityAlive* >::const_iterator II =
        VisibleEnemies.begin();
    for ( ; ( i < I ) && ( j < J ); ) {
        ai().ef_storage().non_alife().member() =
            smart_cast< CEntityAlive* >( Members[ i ] );
        if ( !( ai().ef_storage().non_alife().member() ) ||
             !( ai().ef_storage().non_alife().member()->g_Alive() ) ) {
            ++i;
            continue;
        }
        ai().ef_storage().non_alife().enemy() = *II;
        if ( !( ai().ef_storage().non_alife().enemy() ) ||
             !( ai().ef_storage().non_alife().enemy()->g_Alive() ) ) {
            ++j;
            ++II;
            continue;
        }
        float fProbability = fSuccessProbabilityFunction.ffGetValue() / 100.f,
              fCurrentProbability;
        if ( fProbability > fMinProbability ) {
            ...
            for ( ++i; ( i < I ) && ( j < J ); ++i ) {
                ...
                fProbability =
                    1.0f - fSuccessProbabilityFunction.ffGetValue() / 100.f;
...
u32 dwfChooseAction( u32 dwActionRefreshRate,
    ...
    CGroupHierarchyHolder& Group =
        Level().seniority_holder().team( dwTeam ).squad( dwSquad ).group(
            dwGroup );

    if ( Device.dwTimeGlobal - Group.m_dwLastActionTime <
         dwActionRefreshRate ) {
    ...
    if ( !tpEntity )
        ...
        for ( int k = 0; k < ( int )Group.members().size(); ++k ) {
            if ( Group.members()[ k ]->g_Alive() &&
                 ( ( Group.members()[ k ]->SpatialComponent->spatial.type &
                     STYPE_VISIBLEFORAI ) == STYPE_VISIBLEFORAI ) )
                if ( tpEntity->Position().distance_to(
                         Group.members()[ k ]->Position() ) < fGroupDistance ) {
                    if ( !stalker ) {
                        Members.push_back( Group.members()[ k ] );
                        continue;
                    }
                    ...
                    const CAI_Stalker* member =
                        smart_cast< CAI_Stalker* >( Group.members()[ k ] );
                    if ( !member ) {
                        Members.push_back( Group.members()[ k ] );
                        continue;
                    }

                    if ( Group.agent_manager().member().registered_in_combat(
                             member ) )
                        Members.push_back( Group.members()[ k ] );
    ...
    if ( bfGetActionSuccessProbability(
             Members, VisibleEnemies, fMinProbability0,
        ...
        return ( a0 );
                    ...
                    Members, VisibleEnemies, fMinProbability1,
                    *ai().ef_storage().m_pfVictoryProbability ) ) {
        Group.m_dwLastActionTime = Device.dwTimeGlobal;
        ...
        return ( a1 );
                    ...
                    Members, VisibleEnemies, fMinProbability2,
                    *ai().ef_storage().m_pfVictoryProbability ) ) {
        Group.m_dwLastActionTime = Device.dwTimeGlobal;
        ...
        return ( a2 );
                    ...
                    Members, VisibleEnemies, fMinProbability3,
                    *ai().ef_storage().m_pfVictoryProbability ) ) {
        Group.m_dwLastActionTime = Device.dwTimeGlobal;
...
void CAniVector::Load( IKinematicsAnimated* tpKinematics, LPCSTR caBaseName ) {
    ...
    for ( int i = 0;; ++i )
        if ( !!( tpMotionDef = tpKinematics->ID_Cycle_Safe( strconcat(
                     sizeof( S1 ), S1, caBaseName, itoa( i, S2, 10 ) ) ) ) ) {
            A.push_back( tpMotionDef );
#ifdef DEBUG
            if ( psAI_Flags.test( aiAnimation ) )
                Msg( "* Loaded animation %s", S1 );
#endif
        } else if ( !!( tpMotionDef = tpKinematics->ID_FX_Safe(
                            strconcat( sizeof( S1 ), S1, caBaseName,
                                       itoa( i, S2, 10 ) ) ) ) ) {
            A.push_back( tpMotionDef );
#ifdef DEBUG
            if ( psAI_Flags.test( aiAnimation ) )
