...
void xrGameSpyServer::OnCDKey_Validation( int LocalID,
                                          int res,
                                          char* errormsg ) {
    ClientID ID;
    ID.set( u32( LocalID ) );
    ...
    if ( 0 != res ) {
        ...
        SendConnectResult( CL, u8( res ), ecr_cdkey_validation_failed,
                           errormsg );
    }
};

void xrGameSpyServer::OnCDKey_ReValidation( int LocalID,
                                            int hint,
                                            char* challenge ) {
    ClientID ID;
    ID.set( u32( LocalID ) );
