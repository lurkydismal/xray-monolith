...
void CExplosive::Load( CInifile const* ini, LPCSTR section ) {
    ...
    m_eHitTypeBlast =
        ALife::g_tfString2HitType( ini->r_string( section, "hit_type_blast" ) );
    m_eHitTypeFrag =
        ALife::g_tfString2HitType( ini->r_string( section, "hit_type_frag" ) );
    ...
    m_fFragAirRes =
        READ_IF_EXISTS( ini, r_float, section, "frags_air_resistance", 1.0f );
    m_bFragTracer =
        READ_IF_EXISTS( ini, r_bool, section, "frags_tracer", false );
    m_bFrag4to1Tracer =
        READ_IF_EXISTS( ini, r_bool, section, "frags_4to1_tracer", false );
    m_bFragMagneticBeamShot = READ_IF_EXISTS(
        ini, r_bool, section, "frags_magnetic_beam_shot",
        false ); // Probably has literal zero use cases but, you know, what if?
    m_bFragAllowRicochet =
        READ_IF_EXISTS( ini, r_bool, section, "frags_allow_ricochet", true );
    u8FragColorID =
        READ_IF_EXISTS( ini, r_u8, section, "frags_tracer_color_ID", 0 );
    ...
    sscanf( ini->r_string( section, "light_color" ), "%f,%f,%f",
    ...
    m_layered_sounds.LoadSound( ini, section, "snd_explode", "sndExplode",
                                false, m_eSoundExplode );
    ...
    if ( ini->line_exist( section, "explode_effector_sect_name" ) ) {
        effector.effect_sect_name =
            ini->r_string( section, "explode_effector_sect_name" );
    } else {
        effector.effect_sect_name =
            ini->r_string( "explode_effector", "effect_sect_name" );
    }
    ...
    if ( ini->line_exist( section, "hide_in_explosion" ) ) {
        ...
        if ( ini->line_exist( section, "explode_hide_duration" ) ) {
            m_fExplodeHideDurationMax =
                ini->r_float( section, "explode_hide_duration" );
        }
    }
    ...
    if ( ini->line_exist( section, "dynamic_explosion_particles" ) )
        m_bDynamicParticles =
            ini->r_bool( section, "dynamic_explosion_particles" );

#ifdef EXPLOSIVE_CHANGE
    m_on_explode_callback._set(
        READ_IF_EXISTS( ini, r_string, section, "on_explode", "" ) );
#endif
}
...
ICF static BOOL grenade_hit_callback( collide::rq_result& result,
                                      LPVOID params ) {
...
float CExplosive::ExplosionEffect( collide::rq_results& storage,
                                   CExplosive* exp_obj,
    ...
    if ( blasted_obj->PPhysicsShell() &&
         blasted_obj->PPhysicsShell()->isActive() ) {
    ...
    for ( u16 i = 0; i < TEST_RAYS_PER_OBJECT; ++i ) {
        ...
        float l_S = effective_volume *
                    ( _abs( l_dir.dotproduct( obj_xform.i ) ) / l_d.x +
                      _abs( l_dir.dotproduct( obj_xform.j ) ) / l_d.y +
                      _abs( l_dir.dotproduct( obj_xform.k ) ) / l_d.z );
        effect += _sqrt( l_S / max_s ) * TestPassEffect( l_source_p, l_dir, mag,
                                                         expl_radius, storage,
                                                         blasted_obj );
#endif
    }
...
float CExplosive::TestPassEffect( const Fvector& source_p,
                                  const Fvector& dir,
                                  float range,
                                  float ef_radius,
    ...
    float dist_factor =
        sq_ef_radius /
        ( range * range * ( exp_dist_extinction_factor - 1.f ) + sq_ef_radius );
    float shoot_factor = 1.f;
    if ( range > EPS_L ) {
        VERIFY( !fis_zero( dir.square_magnitude() ) );
        collide::ray_defs RD( source_p, dir, range, CDB::OPT_CULL,
                              collide::rqtBoth );
        ...
        SExpQParams ep;
#endif
        g_pGameLevel->ObjectSpace.RayQuery( storage, RD, grenade_hit_callback,
                                            &ep, NULL, blasted_obj );
...
void CExplosive::Explode() {
    ...
    g_pGamePersistent->GrassBendersAddExplosion(
        cast_game_object()->ID(), pos, Fvector().set( 0, -99, 0 ), 1.33f,
        ps_ssfx_int_grass_params_2.y, ps_ssfx_int_grass_params_2.x,
        m_fBlastRadius * 2.0f );
    ...
    m_layered_sounds.PlaySound( "sndExplode", pos,
                                smart_cast< CObject* >( this ), false, false,
                                ( u8 )-1 );
    ...
    Fvector vel;
    smart_cast< CPhysicsShellHolder* >( cast_game_object() )
        ->PHGetLinearVell( vel );

    Fmatrix explode_matrix;
    explode_matrix.identity();
    explode_matrix.j.set( dir );
    Fvector::generate_orthonormal_basis( explode_matrix.j, explode_matrix.i,
                                         explode_matrix.k );
    explode_matrix.c.set( pos );

    intrusive_ptr< CParticlesObject > pStaticPG = Particles::Details::Create(
        *m_sExplodeParticles, !m_bDynamicParticles );
    if ( m_bDynamicParticles )
        m_pExpParticle = pStaticPG.get();
    ...
    for ( int i = 0; i < m_iFragsNum; ++i ) {
        ...
        cartridge.bullet_material_idx =
            GMLib.GetMaterialIdx( WEAPON_MATERIAL_NAME );
        cartridge.m_flags.set( CCartridge::cfTracer,
                               m_bFragTracer ? TRUE : FALSE );
        cartridge.m_flags.set( CCartridge::cfRicochet,
                               m_bFragAllowRicochet ? TRUE : FALSE );
        cartridge.m_flags.set( CCartridge::cfMagneticBeam,
                               m_bFragMagneticBeamShot ? TRUE : FALSE );

        Level().BulletManager().AddBullet(
            pos, frag_dir, m_fFragmentSpeed, m_fFragHit, m_fFragHitImpulse,
            Initiator(), cast_game_object()->ID(), m_eHitTypeFrag,
            m_fFragsRadius, cartridge, m_fFragAirRes, SendHits, false, i + 1 );
    }
    ...
    g_SpatialSpace->q_sphere( ISpatialResult, 0, STYPE_COLLIDEABLE, pos,
                              m_fBlastRadius );
    ...
    for ( u32 o_it = 0; o_it < ISpatialResult.size(); o_it++ ) {
        ...
        CPhysicsShellHolder* pGameObject =
            smart_cast< CPhysicsShellHolder* >( spatial->dcast_CObject() );
    ...
    if ( pActor ) {
        ...
        if ( dist_to_actor < max_dist )
            if ( effector.effect_sect_name != "$no_effector" ) {
                AddEffector( pActor, effExplodeHit, effector.effect_sect_name,
...
void CExplosive::UpdateCL() {
    ...
    if ( m_fExplodeDuration < 0.f && m_blasted_objects.empty() ) {
        ...
        if ( !m_bHideInExplosion && !m_bAlreadyHidden ) {
            if ( m_fExplodeHideDurationMax <=
                 ( m_fExplodeDurationMax - m_fExplodeDuration ) ) {
                HideExplosive();
            }
        }
        UpdateExplosionPos();
        ...
        if ( m_pLight && m_pLight->get_active() && m_fLightTime > 0 ) {
            if ( m_fExplodeDuration >
                 ( m_fExplodeDurationMax - m_fLightTime ) ) {
                float scale = ( m_fExplodeDuration -
                                ( m_fExplodeDurationMax - m_fLightTime ) ) /
                              m_fLightTime;
...
void CExplosive::OnBeforeExplosion() {
    ...
    if ( m_on_explode_callback.size() ) {
        ...
        if ( ai().script_engine().functor( m_on_explode_callback.c_str(),
                                           lua_function ) ) {
    ...
    if ( m_bHideInExplosion ) {
        HideExplosive();
        //	Msg("---------CExplosive OnBeforeExplosion setVisible(false)
        //[%d] frame[%d]",cast_game_object()->ID(), Device.dwFrame);
    }
}
...
void CExplosive::HideExplosive() {
    ...
    GO->setVisible( FALSE );
    GO->setEnabled( FALSE );
    CPhysicsShell* phshell =
        ( smart_cast< CPhysicsShellHolder* >( GO ) )->PPhysicsShell();
    if ( phshell ) {
        phshell->Disable();
...
void CExplosive::GenExplodeEvent( const Fvector& pos, const Fvector& normal ) {
    ...
    //		return;
    VERIFY(
        !m_explosion_flags.test( flExplodEventSent ) ); //! m_bExplodeEventSent
    VERIFY( 0xffff != Initiator() );

    NET_Packet P;
    cast_game_object()->u_EventGen( P, GE_GRENADE_EXPLODE,
                                    cast_game_object()->ID() );
    P.w_u16( Initiator() );
    P.w_vec3( pos );
    P.w_vec3( normal );
...
void CExplosive::FindNormal( Fvector& normal ) {
    ...
    BOOL result = Level().ObjectSpace.RayPick(
        pos, dir, cast_game_object()->Radius(), collide::rqtBoth, RQ, NULL );
    if ( !result || RQ.O ) {
        normal.set( 0, 1, 0 );
        ...
        normal.mknormal( pVerts[ pTri->verts[ 0 ] ], pVerts[ pTri->verts[ 1 ] ],
                         pVerts[ pTri->verts[ 2 ] ] );
    }
}
...
void CExplosive::GetRaySourcePos( CExplosive* exp_obj,
                                  const Fvector& expl_center,
                                  Fvector& p ) {
    if ( exp_obj ) {
...
void CExplosive::ExplodeWaveProcessObject( collide::rq_results& storage,
                                           CPhysicsShellHolder* l_pGO ) {
    Fvector l_goPos;
    if ( l_pGO->Visual() )
        l_pGO->Center( l_goPos );
    else
        return; // мне непонятно зачем наносить хит от взрыва по объектам не
                // имеющим вижуал - поэтому игнорируем
    ...
    float l_effect =
        ExplosionEffect( storage, this, l_pGO, m_vExplodePos, m_fBlastRadius );
    ...
    if ( l_impuls > .001f || l_hit > 0.001 ) {
        ...
        float rmag = _sqrt( m_fUpThrowFactor * m_fUpThrowFactor + 1.f +
                            2.f * m_fUpThrowFactor * l_dir.y );
        l_dir.y += m_fUpThrowFactor;
        // rmag -модуль l_dir после l_dir.y += m_fUpThrowFactor,
        // модуль=_sqrt(l_dir^2+y^2+2.*(l_dir,y)),y=(0,m_fUpThrowFactor,0) (до
        // этого модуль l_dir =1)
...
void CExplosive::ExplodeWaveProcess() {
    BLASTED_OBJECTS_I I = std::remove_if(
        m_blasted_objects.begin(), m_blasted_objects.end(), SRemovePred() );
...
void CExplosive::ActivateExplosionBox( const Fvector& size,
                                       Fvector& in_out_pos ) {
    CPhysicsShellHolder* self_obj =
        smart_cast< CPhysicsShellHolder* >( cast_game_object() );
...
void CExplosive::net_Relcase( CObject* O ) {
    ...
    BLASTED_OBJECTS_I I =
        std::find( m_blasted_objects.begin(), m_blasted_objects.end(),
...
void CExplosive::UpdateExplosionParticles() {
    if ( !m_bDynamicParticles || m_pExpParticle == NULL ||
         !m_pExpParticle->IsPlaying() )
        return;
    CGameObject* GO = cast_game_object();
    if ( !GO )
        return;
