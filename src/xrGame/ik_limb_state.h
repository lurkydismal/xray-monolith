...
public:
    ...
    ik_limb_state( const CIKLimb* l, const ik_limb_state& s )
        : state( s.state ), limb( l ) {}
...
public:
    ...
    IC void get_calculate_state( calculate_state& s ) const {
        ...
        s.blending =
            valide() && ( state.blending || state.foot_step != s.foot_step );
...
IC bool state_valide( const calculate_state& prev_state ) {
    ...
    return ( Device.dwTimeGlobal <=
             ( prev_state.calc_time + UCalc_Interval + Device.dwTimeDelta ) );
}
