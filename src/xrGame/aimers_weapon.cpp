...
weapon::weapon( CGameObject* object,
                LPCSTR animation_id,
    ...
    CBoneData const& bone_data =
        m_kinematics.LL_GetData( m_bones_ids[ weapon_bone_id0 ] );
    ...
    CBoneInstance& bone =
        m_kinematics.LL_GetBoneInstance( m_bones_ids[ bone_id0 ] );
...
void weapon::compute_bone( u32 const bone_id ) {
    ...
    u32 bones_ids[] = { bone_id, weapon_bone_id0, weapon_bone_id1,
...
#pragma todo( "Dima to Dima: use function from weapon to get this information" )
    ...
    Fvector vLoadedFirePoint =
        pSettings->r_fvector3( m_weapon.cNameSect(), "fire_point" );
    Fmatrix transform;
    ...
    aim_at_position( m_bones[ bone_id ].c, position, direction,
                     m_result[ bone_id ] );
    VERIFY( _valid( m_result[ bone_id ] ) );
    VERIFY( _valid( m_start_transform ) );
    VERIFY( _valid( Fmatrix( m_start_transform ).invert() ) );
    m_result[ bone_id ] = Fmatrix( m_start_transform )
                              .invert()
                              .mulB_43( m_result[ bone_id ] )
                              .mulB_43( m_start_transform );
}
