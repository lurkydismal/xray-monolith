...
// Main
/*game_PlayerState*	game_sv_GameState::get_it
(u32 it)
{
        xrClientData*	C	= (xrClientData*)m_server->client_Get
(it); if (0==C)			return 0; else				return
C->ps;
}*/
...
/*ClientID				game_sv_GameState::get_it_2_id
(u32 it)
{
        xrClientData*	C	= (xrClientData*)m_server->client_Get
(it); if (0==C){ ClientID clientID;clientID.set(0); return clientID;
        }
        else				return C->ID;
}

LPCSTR				game_sv_GameState::get_name_it
(u32 it)
{
        xrClientData*	C	= (xrClientData*)m_server->client_Get
(it); if (0==C)			return 0; else				return
...
game_PlayerState* game_sv_GameState::get_eid( u16 id ) // if exist
{
    ...
    xrClientData* tmp_client =
        static_cast< xrClientData* >( m_server->FindClient( tmp_predicate ) );
    if ( tmp_client )
    ...
    struct alife_counter {
        ...
        void operator()( IClient* client ) {
            ...
            if ( tmp_client->ps->team == team ) {
                count +=
                    tmp_client->ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD )
                        ? 0
                        : 1;
            }
        }
...
string64& game_sv_GameState::get_option_s( LPCSTR lst,
                                           LPCSTR name,
                                           LPCSTR def ) {
    static string64 ret;
...
struct player_exporter {
    ...
    player_exporter( ClientID to,
                     game_PlayerState* to_playerstate,
                     NET_Packet* P ) {
        counter = 0;
        to_client = to;
        p_to_send = P;
...
void game_sv_GameState::net_Export_State( NET_Packet& P, ClientID to ) {
    ...
    xrClientData* tmp_client =
        static_cast< xrClientData* >( m_server->GetClientByID( to ) );
...
void game_sv_GameState::net_Export_Update( NET_Packet& P,
                                           ClientID id_to,
                                           ClientID id ) {
...
void game_sv_GameState::net_Export_GameTime( NET_Packet& P ) {
    // #pragma todo("It should be done via single message, why always pass this
    // data?") #if 0 Syncronize GameTime
    P.w_u64( GetGameTime() );
...
void game_sv_GameState::Create( shared_str& options ) {
        ...
        if ( 0 != ( O = F->open_chunk( RPOINT_CHUNK ) ) ) {
            for ( int id = 0; O->find_chunk( id ); ++id ) {
                ...
                if ( GameType != EGameIDs( u16( -1 ) ) ) {
                    if ( ( Type() == eGameIDCaptureTheArtefact ) &&
                         ( GameType & eGameIDCaptureTheArtefact ) ) {
                        team = team - 1;
                        R_ASSERT2(
                            ( ( team >= 0 ) && ( team < 4 ) ) ||
                                ( type != rptActorSpawn ),
                            "Problem with CTA Team indexes. Propably you have "
                            "added rpoint of team 0 for cta game type." );
                    }
                    if ( ( !( GameType & eGameIDDeathmatch ) &&
                           ( Type() == eGameIDDeathmatch ) ) ||
                         ( !( GameType & eGameIDTeamDeathmatch ) &&
                           ( Type() == eGameIDTeamDeathmatch ) ) ||
                         ( !( GameType & eGameIDArtefactHunt ) &&
                           ( Type() == eGameIDArtefactHunt ) ) ||
                         ( !( GameType & eGameIDCaptureTheArtefact ) &&
                           ( Type() == eGameIDCaptureTheArtefact ) ) ) {
                        continue;
                    };
                };
                switch ( type ) {
                    case rptActorSpawn: {
                        ...
                        for ( int i = 0; i < int( rpoints[ team ].size() ) - 1;
                              i++ ) {
            ...
            if ( l_tpIniFile->r_string( type_name(), "script" ) )
                ai().script_engine().add_script_process(
                    ...
                    xr_new< CScriptProcess >(
                        "game",
                        l_tpIniFile->r_string( type_name(), "script" ) ) );
            else
                ai().script_engine().add_script_process(
                    ScriptEngine::eScriptProcessorGame,
    ...
    if ( strstr( Core.Params, svcfg_ltx_name ) ) {
        ...
        sscanf( strstr( Core.Params, svcfg_ltx_name ) + sz, "%[^ ] ",
                svcfg_name );
...
void game_sv_GameState::ReadOptions( shared_str& options ) {
    g_sv_base_dwRPointFreezeTime =
        get_option_i( *options, "rpfrz", g_sv_base_dwRPointFreezeTime / 1000 ) *
        1000;
    ...
    g_sv_base_iVotingEnabled =
        get_option_i( *options, "vote", ( g_sv_base_iVotingEnabled ) );
    //---------------------------
...
#ifdef DEBUG
        Msg( "--- game_sv_GameState RPoint for Spectators uses team [%d]",
             l_uc_team );
#endif // #ifdef DEBUG
    } else {
        CSE_ALifeCreatureAbstract* tpTeamed =
            smart_cast< CSE_ALifeCreatureAbstract* >( E );
        if ( tpTeamed ) {
            l_uc_team = tpTeamed->g_team();
#ifdef DEBUG
            Msg(
                "--- game_sv_GameState RPoint for AlifeCreature uses team [%d]",
                l_uc_team );
#endif // #ifdef DEBUG
        } else {
            R_ASSERT2( false /*tpTeamed*/,
                       "Non-teamed object is assigning to respawn point!" );
        }
    }
    R_ASSERT2(
        l_uc_team < TEAM_COUNT,
...
void game_sv_GameState::OnHit( u16 id_hitter, u16 id_hitted, NET_Packet& P ) {
    ...
    //	CSE_ALifeCreatureActor*		a_hitter		= smart_cast
    //<CSE_ALifeCreatureActor*> (e_hitter);
    CSE_ALifeCreatureActor* a_hitted =
        smart_cast< CSE_ALifeCreatureActor* >( e_hitted );
...
void game_sv_GameState::OnEvent( NET_Packet& tNetPacket,
                                 u16 type,
                                 u32 time,
                                 ClientID sender ) {
    switch ( type ) {
        ...
        case GAME_EVENT_ON_HIT: {
            ...
            if ( !e_src ) // && !IsGameTypeSingle() added by andy because of
                          // Phantom does not have server entity
            {
                if ( IsGameTypeSingle() )
                    break;
        ...
        case GAME_EVENT_CREATE_PLAYER_STATE: {
            ...
            R_ASSERT2( CL, make_string(
                               "M_CREATE_PLAYER_STATE: client 0x%08x not found",
                               sender.value() )
                               .c_str() );
        ...
        default: {
            string16 tmp;
            R_ASSERT3( 0, "Game Event not implemented!!!",
                       itoa( type, tmp, 10 ) );
        };
    };
}
...
void game_sv_GameState::AddDelayedEvent( NET_Packet& tNetPacket,
                                         u16 type,
                                         u32 time,
                                         ClientID sender ) {
...
void game_sv_GameState::CleanDelayedEventFor( u16 id_entity_victim ) {
    ...
    m_event_queue->EraseEvents( fastdelegate::MakeDelegate(
        &event_deleter, &EventDeleterPredicate::PredicateDelVictim ) );
}
...
public:
    ...
    bool __stdcall Predicate( GameEvent* const ge ) {
        if ( ge && ( ge->sender == m_client_id ) ) {
            Msg( "- Erasing event for not valid client [0x%08x]",
                 m_client_id.value() );
            return true;
        }
        return false;
...
private:
    EventDeleteForClientPredicate& operator=(
        EventDeleteForClientPredicate const& copy ) {}
...
void game_sv_GameState::CleanDelayedEventFor( ClientID const& clientId ) {
    ...
    m_event_queue->EraseEvents( fastdelegate::MakeDelegate(
        &event_deleter, &EventDeleteForClientPredicate::Predicate ) );
}

void game_sv_GameState::CleanDelayedEvents() {
    EventDeleterPredicate event_deleter;
    m_event_queue->EraseEvents( fastdelegate::MakeDelegate(
        &event_deleter, &EventDeleterPredicate::PredicateForAll ) );
}
...
void game_sv_GameState::MapRotation_ListMaps() {
    ...
    for ( u32 i = 0; i < m_pMapRotation_List.size(); i++ ) {
        ...
        if ( i == 0 )
            Msg( "~   %d. %s (%s) (current)", i + 1,
                 st.translate( R.map_name ).c_str(), R.map_name.c_str() );
        else
            Msg( "  %d. %s (%s)", i + 1, st.translate( R.map_name ).c_str(),
                 R.map_name.c_str() );
    }
    Msg( "- --------------------------------" );
};
...
shared_str game_sv_GameState::level_name(
    const shared_str& server_options ) const {
...
shared_str game_sv_GameState::parse_level_version(
    const shared_str& server_options ) {
    const char* map_ver = strstr( server_options.c_str(), map_ver_string );
    string128 result_version;
    if ( map_ver ) {
        map_ver += sizeof( map_ver_string );
        if ( strchr( map_ver, '/' ) )
            strncpy_s( result_version, map_ver,
                       strchr( map_ver, '/' ) - map_ver );
        else
...
shared_str game_sv_GameState::parse_level_name(
    const shared_str& server_options ) {
    string64 l_name = "";
...
void game_sv_GameState::on_death( CSE_Abstract* e_dest, CSE_Abstract* e_src ) {
    CSE_ALifeCreatureAbstract* creature =
        smart_cast< CSE_ALifeCreatureAbstract* >( e_dest );
    if ( !creature )
        return;
...
void game_sv_GameState::OnRender() {
    ...
    Fvector V0, V1;
    u32 TeamColors[TEAM_COUNT] = {D3DCOLOR_XRGB(255, 0, 0), D3DCOLOR_XRGB(0,
255, 0), D3DCOLOR_XRGB(0, 0, 255), D3DCOLOR_XRGB(255, 255, 0)};
//	u32 TeamColorsDist[TEAM_COUNT] = {color_argb(128, 255, 0, 0),
color_argb(128, 0, 255, 0), color_argb(128, 0, 0, 255), color_argb(128, 255,
255, 0)};
    ...
    if (dbg_net_Draw_Flags.test(dbg_draw_rp))
    {
            for (int t=0; t<TEAM_COUNT; t++)
            {
                    for (u32 i=0; i<rpoints[t].size(); i++)
                    {
                            ...
                            T.identity();
                            Level().debug_renderer().draw_line(Fidentity, V0,
V1, TeamColors[t]);
                            ...
                            {
                                    game_PlayerState* PS		=
get_it			(p_it); if (!PS) continue; if
(PS->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD)) continue; CObject* pPlayer =
Level().Objects.net_Find(PS->GameID); if (!pPlayer) continue;
...
(rp.P.distance_to(pPlayer->Position())<=0.4f)
                            ...
                            Level().debug_renderer().draw_ellipse(T,
TeamColors[t]);
/*
                            r = rpoints_MinDist[t];
                            T.identity();
...
/*-/
    ...
    if (dbg_net_Draw_Flags.test(dbg_draw_actor_alive))
    {
            for (u32 p_it=0; p_it<get_players_count(); ++p_it)
            {
                    game_PlayerState* PS		=	get_it
(p_it); if (!PS) continue; if (PS->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD))
continue; CObject* pPlayer = Level().Objects.net_Find(PS->GameID); if (!pPlayer)
continue;
...
public:
    NameSearcherPredicate( char const* name, IClient const* to_exclude )
        : m_name( name ), m_exclude( to_exclude ) {}
...
bool game_sv_GameState::FindPlayerName( char const* name,
                                        IClient const* to_exclude ) {
    R_ASSERT( name );
...
void game_sv_GameState::GenerateNewName( char const* old_name,
                                         char* dest,
                                         u32 const dest_size ) {
...
void game_sv_GameState::CheckPlayerName( xrClientData* CL ) {
    ...
    if ( CL->ps->m_account.name()
             .size() ) // in case of logging from gamespy login page
    {
