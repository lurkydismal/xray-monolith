...
BOOL CBlackGraviArtefact::net_Spawn( CSE_Abstract* DC ) {
    ...
    intrusive_ptr< CParticlesObject > pStaticPG =
        Particles::Details::Create( "anomaly\\galantine", FALSE );
    Fmatrix pos;
    // pos.rotateY(1.57);
    // pos.mulA(pos);
...
void CBlackGraviArtefact::feel_touch_delete( CObject* O ) {
    ...
    if ( pGameObject && !pArtefact ) {
        m_GameObjectList.erase( std::find(
            m_GameObjectList.begin(), m_GameObjectList.end(), pGameObject ) );
    }
}
...
void CBlackGraviArtefact::GraviStrike() {
    ...
    for ( GAME_OBJECT_LIST_it it = m_GameObjectList.begin();
        ...
        float impulse =
            100.f * m_fStrikeImpulse *
            ( 1.f - ( distance / m_fRadius ) * ( distance / m_fRadius ) );

        if ( impulse > .001f ) {
            //?			BOOL		enabled = getEnabled();
            //?			setEnabled	(FALSE);
            impulse *= CExplosive::ExplosionEffect(
                rq_storage, NULL, pGameObject, Position(), m_fRadius );
            //?			setEnabled	(enabled);
        }
        ...
        if ( pGameObject->m_pPhysicsShell )
            hit_power = 0;
                  ...
                  pEntityAlive->character_physics_support()
                      ->movement()
                      ->CharacterExist() )
            hit_power = 0;
        else
