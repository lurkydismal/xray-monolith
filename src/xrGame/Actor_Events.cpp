    ...
    switch ( type ) {
        ...
        case GE_OWNERSHIP_TAKE: {
            ...
            VERIFY2(
                Obj,
                make_string(
                    "GE_OWNERSHIP_TAKE: Object not found. object_id = [%d]",
                    id )
                    .c_str() );
            if ( !Obj ) {
                Msg( "! GE_OWNERSHIP_TAKE: Object not found. object_id = [%d]",
                     id );
                break;
            }
            ...
            if ( inventory().CanTakeItem(
                ...
                if ( IsGameTypeSingle() ) {
                    ...
                    Msg( "! ERROR: Actor [%d][%s]  tries to drop on take "
                         "[%d][%s]",
                         ID(), Name(), _GO->ID(), _GO->cNameSect().c_str() );
                }
            }
        ...
        case GE_OWNERSHIP_REJECT: {
            ...
            VERIFY( GO->H_Parent() );
            if ( !GO->H_Parent() ) {
                Msg( "! ERROR: Actor [%d][%s] tries to reject item [%d][%s] "
                     "that has no parent",
            ...
            if ( GO->H_Parent()->ID() != ID() ) {
                ...
                Msg( "! ERROR: Actor [%d][%s] tries to drop not own item "
                     "[%d][%s], his parent is [%d][%s]",
                     ID(), Name(), GO->ID(), GO->cNameSect().c_str(),
                     real_parent->ID(), real_parent->Name() );
                break;
            }

            if ( !Obj->getDestroy() &&
                 inventory().DropItem( GO, just_before_destroy,
                                       dont_create_shell ) ) {
                // O->H_SetParent(0,just_before_destroy);//moved to DropItem
                // feel_touch_deny(O,2000);
                Level().m_feel_deny.feel_touch_deny( Obj, 1000 );

                // [12.11.07] Alexander Maniluk: extended GE_OWNERSHIP_REJECT
                // packet for drop item to selected position
        ...
        case GEG_PLAYER_ACTIVATEARTEFACT: {
            ...
            if ( type == GEG_PLAYER_ACTIVATEARTEFACT ) {
                ...
                VERIFY2( pArtefact,
                         make_string( "GEG_PLAYER_ACTIVATEARTEFACT: Artefact "
                             ...
                             .c_str() );
                if ( !pArtefact ) {
                    Msg( "! GEG_PLAYER_ACTIVATEARTEFACT: Artefact not found. "
                         "artefact_id = [%d]",
                         id );
                    break; // 1
                }
            ...
            switch ( type ) {
                case GEG_PLAYER_ITEM2SLOT: {
                    ...
                    bool bDoNotActivate =
                        P.r_u8() ==
                        1; // conservatively preserve old behaviour unless very
                           // specifically (value == 1) requested otherwise
                ...
                case GEG_PLAYER_ITEM_EAT:
                    ...
                    if ( iitem && ai().script_engine().functor(
                                      "_G.CInventory__eat", funct ) ) {
