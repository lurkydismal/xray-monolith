...
/*
void type_motion::set_motion( IKinematicsAnimated* k, CInifile* ini, LPCSTR
type, LPCSTR dir, edirection id_dir )
{
        if( ini->line_exist( type, dir ) )
                        anims[ u16( id_dir ) ] = xr_new<rnd_motion>()->setup( k,
ini->r_string( type, dir ) );
}
*/
xr_token motion_dirs[] = { { "front", type_motion::front },
                           { "back", type_motion::back },
                           { "left", type_motion::left },
                           { "right", type_motion::right },
                           { 0, 0 } };

void type_motion::set_motion( IKinematicsAnimated* k,
                              u16 id_motion,
                              LPCSTR dir_anim ) {
...
type_motion* type_motion::setup( IKinematicsAnimated* k,
                                 CInifile const* ini,
                                 LPCSTR section,
                                 LPCSTR type ) {
...
#ifdef DEBUG
            if ( death_anim_debug )
                Msg( "death anims: load: no setings in section %s for %s",
                     section, type );
#endif
            return this;
        }
        R_ASSERT( xr_strlen( line ) <
                  sizeof( string1024 ) ); // loop var 'sdir_anim'
        const int num = _GetItemCount( line, '/' );
#ifdef DEBUG
        if ( death_anim_debug && num == 0 )
            Msg( "death anims: load: no setings in section %s for %s", section,
                 type );
#endif
        for ( int i = 0; num > i; ++i ) {
...
#ifdef DEBUG
            if ( death_anim_debug )
                Msg( "death anims: load: loaded %s from section %s for %s",
                     sdir_anim, section, type );
#endif
        }
    }
#ifdef DEBUG
...
/*
type_motion* type_motion::setup( IKinematicsAnimated* k, CInifile* ini, LPCSTR
type, u16 id_type )
{
        anims.resize( dirs_number, 0 );
...
MotionID death_anims::motion( CEntityAlive& ea,
                              const SHit& H,
                              float& angle ) const {
    angle = 0;
    if ( anims.empty() ) {
#ifdef DEBUG
        if ( death_anim_debug )
            Msg( " death anims: obj: %s no death motions loaded ",
                 ea.cName().c_str() );
#endif
    ...
    MotionID m;
    xr_vector< type_motion* >::const_iterator i = anims.begin(),
                                              e = anims.end();
    for ( ; e != i; ++i )
...
#ifdef DEBUG
    if ( death_anim_debug )
        Msg( " death anims: obj: %s no conditions evaluated  returns random ",
             ea.cName().c_str() );
#endif
    angle = 0;
    return rnd_anims.motion();
