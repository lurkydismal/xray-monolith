...
BOOL CDestroyablePhysicsObject::net_Spawn( CSE_Abstract* DC ) {
    ...
    // R_ASSERT2(ini->section_exist("destroyed"),"destroyable_object must have
    // -destroyed- section in model user data");
    CPHDestroyable::Init();
    ...
    if ( ini ) {
        ...
        if ( ini->section_exist( "sound" ) )
            m_destroy_sound.create( ini->r_string( "sound", "break_sound" ),
                                    st_Effect, sg_SourceType );
        if ( ini->section_exist( "particles" ) )
            m_destroy_particles =
                ini->r_string( "particles", "destroy_particles" );
    }
...
// void CDestroyablePhysicsObject::Hit
// (float P,Fvector &dir,CObject *who,s16 element,Fvector p_in_object_space,
...
void CDestroyablePhysicsObject::Destroy() {
    ...
    const CGameObject* who_object =
        smart_cast< const CGameObject* >( FatalHit().initiator() );
...
bool CDestroyablePhysicsObject::CanRemoveObject() {
    return !CParticlesPlayer::IsPlaying() &&
           !m_destroy_sound._feedback(); //&& sound!
}
