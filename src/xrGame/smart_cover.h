...
class cover : public CCoverPoint,
              private debug::make_final< cover >,
              private xray::noncopyable {
public:
    typedef intrusive_ptr< smart_cover::description > DescriptionPtr;
...
public:
    cover( object const& object,
           DescriptionPtr description,
           bool is_combat_cover,
           bool can_fire,
    ...
    u32 const& action_level_vertex_id( loophole const& loophole,
                                       shared_str const& action_id ) const;
    loophole* best_loophole( Fvector const& position,
                             float& value,
                             bool const& use_default_behaviour,
                             bool is_smart_cover_entered ) const;
    IC DescriptionPtr const& description() const;
    void evaluate_loophole( Fvector const& position,
                            loophole*& source,
                            loophole*& result,
                            float& value,
                            bool is_smart_cover_entered ) const;
    IC shared_str const& id() const;
    void evaluate_loophole_for_default_usage( Fvector const& position,
                                              smart_cover::loophole*& source,
                                              smart_cover::loophole*& result,
                                              float& value ) const;
    bool is_position_in_fov( loophole const& source,
                             Fvector const& position ) const;
    bool is_position_in_danger_fov( loophole const& source,
                                    Fvector const& position ) const;
    bool is_position_in_range( loophole const& source,
                               Fvector const& position ) const;
    bool in_min_acceptable_range( loophole const& source,
...
#ifdef DEBUG
private:
    bool loophole_path( shared_str const& source_raw,
