...
void CUIStaticItem::SetHeadingPivot( const Fvector2& p,
...
void CUIStaticItem::RenderInternal( const Fvector2& in_pos ) {
    ...
    if ( R && R->size() ) {
        for ( u32 k = 0; k < R->size() - 2; ++k ) {
            UIRender->PushPoint( ( *R )[ 0 + 0 ].pt.x, ( *R )[ 0 + 0 ].pt.y, 0,
                                 dwColor, ( *R )[ 0 + 0 ].uv.x,
                                 ( *R )[ 0 + 0 ].uv.y );
            UIRender->PushPoint( ( *R )[ k + 1 ].pt.x, ( *R )[ k + 1 ].pt.y, 0,
                                 dwColor, ( *R )[ k + 1 ].uv.x,
                                 ( *R )[ k + 1 ].uv.y );
            UIRender->PushPoint( ( *R )[ k + 2 ].pt.x, ( *R )[ k + 2 ].pt.y, 0,
                                 dwColor, ( *R )[ k + 2 ].uv.x,
                                 ( *R )[ k + 2 ].uv.y );
        }
    }
...
void CUIStaticItem::Render() {
    ...
    UIRender->StartPrimitive(
        UI().HasCustomClip() ? UI().ActiveClipFrustum().ClipBudget( 4 ) : 8,
...
void CUIStaticItem::Render( float angle ) {
    ...
    UIRender->SetShader( *hShader );
    UIRender->StartPrimitive( 32, IUIRender::ptTriList,
