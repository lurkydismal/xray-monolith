...
void CLevel::remove_objects() {
    ...
    const auto load_queues_drained = [ this ]() {
        if ( Objects.o_count() != 0 || !Objects.destroy_queues_empty() ||
             !game_spawn_queue.empty() || !net_msg_Empty() ) {
            return false;
        }
#ifdef SPAWN_ANTIFREEZE
        xrSRWLockGuard guard( prefetch_lock, true );
        return game_events->queue.empty() && !spawn_prefetch_busy &&
               prefetch_events->empty() && spawn_events->queue.empty() &&
               spawn_events_data->empty();
#else
    ...
    while ( loop ) {
        ...
        if ( load_queues_drained() )
            ...
            --loop;
            Msg( "Objects removal next loop. Active objects count=%d",
                 Objects.o_count() );
        }
    }
    ...
    //= xr_new<CSeniorityHierarchyHolder>();
    if ( !IsGameTypeSingle() )
...
void CLevel::net_Stop() {
    ...
    if ( IsDemoPlay() && m_current_spectator ) // destroying demo spectator ...
    {
        ...
        SetControlEntity(
            NULL ); // m_current_spectator == CurrentControlEntity()
    ...
    // WARNING ! remove_objects() uses this flag, so position of this line must
    // e here ..
...
u32 CLevel::Objects_net_Save( NET_Packet* _Packet,
                              u32 start,
                              u32 max_object_size ) {
    ...
    for ( ; start < Objects.o_count(); start++ ) {
        ...
        if ( P && !P->getDestroy() && P->net_SaveRelevant() ) {
            ...
            //			Msg
            //("save:saved:%d bytes:%d:%s",size,P->ID(),*P->cName());
            if ( size >= 65536 ) {
...
#ifdef DEBUG
        VERIFY2( Server->IsPlayersMonitorLockedByMe() == false,
        ...
        switch ( res1 ) {
            case ecr_data_verification_failed: // Standart error
            {
                if ( strstr( ResultStr, "Data verification failed. Cheater?" ) )
                    MainMenu()->SetErrorDialog(
                        CMainMenu::ErrDifferentVersion );
            } break;
            case ecr_cdkey_validation_failed: // GameSpy CDKey
            {
                if ( !xr_strcmp( ResultStr, "Invalid CD Key" ) )
                    MainMenu()->SetErrorDialog(
                        CMainMenu::ErrCDKeyInvalid ); //, ResultStr);
                if ( !xr_strcmp( ResultStr, "CD Key in use" ) )
                    MainMenu()->SetErrorDialog(
                        CMainMenu::ErrCDKeyInUse ); //, ResultStr);
                if ( !xr_strcmp( ResultStr,
                                 "Your CD Key is disabled. Contact customer "
                                 "service." ) )
                    MainMenu()->SetErrorDialog(
                        CMainMenu::ErrCDKeyDisabled ); //, ResultStr);
            } break;
            ...
            case ecr_have_been_banned: {
                if ( !xr_strlen( ResultStr ) ) {
                    MainMenu()->OnSessionTerminate(
                        CStringTable()
                            .translate( "st_you_have_been_banned" )
                            .c_str() );
                } else {
                    MainMenu()->OnSessionTerminate(
...
void CLevel::ClearAllObjects() {
    ...
    while ( ParentFound ) {
        ...
        for ( u32 i = 0; i < CLObjNum; i++ ) {
            ...
            //-------------------------------------------------------------
#ifdef DEBUG
            Msg( "Rejection of %s[%d] from %s[%d]", *( pObj->cNameSect() ),
                 pObj->ID(), *( pObj->H_Parent()->cNameSect() ),
                 pObj->H_Parent()->ID() );
#endif
        };
        ProcessGameEvents();
