...
//{
//	CCustomMonster*		M = static_cast<CCustomMonster*>
//(B->Callback_Param);
//
//	Fmatrix					spin;
...
void CCustomMonster::Load( LPCSTR section ) {
    ...
    // Fvector	vBOX0_center= pSettings->r_fvector3
    // (section,"ph_box0_center"	); Fvector	vBOX0_size	=
    // pSettings->r_fvector3	(section,"ph_box0_size"		); bb.set
    // (vBOX0_center,vBOX0_center); bb.grow(vBOX0_size);
    // m_PhysicMovementControl->SetBox		(0,bb);

    //// m_PhysicMovementControl: BOX
    // Fvector	vBOX1_center= pSettings->r_fvector3
    // (section,"ph_box1_center"	); Fvector	vBOX1_size	=
    // pSettings->r_fvector3	(section,"ph_box1_size"		); bb.set
    // (vBOX1_center,vBOX1_center); bb.grow(vBOX1_size);
    ...
    // float	cs_min		= pSettings->r_float
    // (section,"ph_crash_speed_min"	); float	cs_max		=
    // pSettings->r_float	(section,"ph_crash_speed_max"	); float
    // mass		= pSettings->r_float	(section,"ph_mass"
    ...
    /*
    float af, gf, wf;
    af					= pSettings->r_float
    (section,"ph_friction_air"	); gf					=
    pSettings->r_float	(section,"ph_friction_ground"); wf
    = pSettings->r_float	(section,"ph_friction_wall"	);
...
void CCustomMonster::reinit() {
    ...
    m_critical_wound_decrease_quant =
        pSettings->r_float( cNameSect(), "critical_wound_decrease_quant" );
...
void CCustomMonster::reload( LPCSTR section ) {
    ...
    m_far_plane_factor =
        READ_IF_EXISTS( pSettings, r_float, section, "far_plane_factor", 1.f );
    m_fog_density_factor = READ_IF_EXISTS( pSettings, r_float, section,
                                           "fog_density_factor", .05f );
...
void CCustomMonster::shedule_Update( u32 DT ) {
    ...
    if ( Remote() ) {
        ...
        if ( temp > 0 ) {
            ...
            // R = Radius();
            //////////////////////////////////////
            /// #pragma todo("Oles to all AI guys: perf/logical problem: Only
            /// few objects needs 'feel_touch' why to call update for
            /// everybody?")
...
void CCustomMonster::net_update::lerp( CCustomMonster::net_update& A,
                                       CCustomMonster::net_update& B,
                                       float f ) {
    //
...
void CCustomMonster::UpdateCL() {
    ...
    m_client_update_delta = ( u32 )std::min(
        Device.dwTimeGlobal - m_last_client_update_time, u32( 100 ) );
    m_last_client_update_time = Device.dwTimeGlobal;
    ...
    smart_cast<IKinematics*>(Visual()); VERIFY
    (kinematics); kinematics->CalculateBones	();
    }
    */

    if ( g_mt_config.test( mtSoundPlayer ) )
        Device.seqParallel.push_back(
            xr_make_delegate( this, &CCustomMonster::update_sound_player ) );
    else {
    ...
    if ( ( dwTime > N.dwTimeStamp ) || ( NET.size() < 2 ) ) {
        ...
        for ( u32 id = 0; id < NET.size() - 1; ++id ) {
            if ( ( NET[ id ].dwTimeStamp <= dwTime ) &&
                 ( dwTime <= NET[ id + 1 ].dwTimeStamp ) )
                select = id;
        }
        if ( select >= 0 ) {
            // Interpolate state
...
void CCustomMonster::UpdatePositionAnimation() {
    ...
    movement().on_frame( character_physics_support()->movement(),
                         NET_Last.p_pos );
    STOP_PROFILE
...
void CCustomMonster::eye_pp_s0() {
    ...
    eye_matrix.setHPB( -rotation.current.yaw + m_fEyeShiftYaw,
                       -rotation.current.pitch, 0 );

#ifdef HOLDERCUSTOM_NEW
    if ( cast_stalker() && cast_stalker()->Holder() ) {
        eye_matrix.setHPB( Direction().getH() + m_fEyeShiftYaw,
                           Direction().getP(), 0 );
    }
#endif
...
void CCustomMonster::update_range_fov( float& new_range,
                                       float& new_fov,
                                       float start_range,
                                       float start_fov ) {
    ...
    float current_fog_density =
        GamePersistent().Environment().CurrentEnv->fog_density;
    // 0=no_fog, 1=full_fog, >1 = super-fog
#pragma todo( \
    "v2v3v4: Let's see, maybe it will stay like this because it's much cheaper and more logical." )
    float current_far_plane =
        ai().get_alife() ? ai().alife().online_distance()
                         : GamePersistent().Environment().CurrentEnv->far_plane;
    ...
    if ( cast_stalker() && cast_stalker()->Holder() ) {
        ...
        CWeaponStatMgun* stm =
            smart_cast< CWeaponStatMgun* >( cast_stalker()->Holder() );
        if ( stm ) {
...
void CCustomMonster::eye_pp_s1() {
    ...
    if ( g_Alive() ) {
        ...
        update_range_fov(
            new_range, new_fov,
            human_being()
                ? memory().visual().current_state().m_max_view_distance *
                      eye_range
                : eye_range,
            eye_fov );
#else
...
void CCustomMonster::eye_pp_s2() {
    ...
    Device.secondary_tasks.run( [ = ]() {
        ...
        feel_vision_update( eye_matrix.c, float( dwDT ) / 1000.f,
                            memory().visual().transparency_threshold() );
    } );
...
void CCustomMonster::UpdateCamera() {
    ...
    if ( g_Alive() )
        update_range_fov(
            ...
            memory().visual().current_state().m_max_view_distance * eye_range,
            eye_fov );
    g_pGameLevel->Cameras().Update( eye_matrix.c, eye_matrix.k, eye_matrix.j,
                                    new_fov, .75f, new_range, 0 );
}

void CCustomMonster::HitSignal( float /**perc/**/,
                                Fvector& /**vLocalDir/**/,
                                CObject* /**who/**/ ) {}
...
BOOL CCustomMonster::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( !movement().net_Spawn( DC ) || !inherited::net_Spawn( DC ) ||
    ...
    if ( ai().get_level_graph() && UsedAI_Locations() &&
        ...
        if ( ai().game_graph().valid_vertex_id( E->m_tNextGraphID ) &&
             ( ai().game_graph().vertex( E->m_tNextGraphID )->level_id() ==
               ai().level_graph().level_id() ) &&
             movement().restrictions().accessible(
                 ai().game_graph()
                     .vertex( E->m_tNextGraphID )
                     ->level_vertex_id() ) )
            movement().set_game_dest_vertex( E->m_tNextGraphID );

        if ( movement().restrictions().accessible(
            ...
            if ( level_vertex_id != ( u32 )-1 &&
                 movement().restrictions().accessible( level_vertex_id ) ) {
    ...
    eye_bone =
        smart_cast< IKinematics* >( Visual() )
            ->LL_BoneID( pSettings->r_string( cNameSect(), "bone_head" ) );
    ...
    shedule.t_min = 100;
    shedule.t_max =
        250; // This equaltiy is broken by Dima :-( // 30 * NET_Latency / 4;
...
// void CCustomMonster::Hit(float P, Fvector &dir,CObject* who, s16
...
bool CCustomMonster::is_special_killer( CObject* obj ) {
    return ( obj && ( std::find( m_killer_clsids.begin(), m_killer_clsids.end(),
                                 obj->CLS_ID ) != m_killer_clsids.end() ) );
}
...
void CCustomMonster::feel_sound_new( CObject* who,
                                     int type,
...
bool CCustomMonster::useful( const CItemManager* manager,
                             const CGameObject* object ) const {
    return ( memory().item().useful( object ) );
}

float CCustomMonster::evaluate( const CItemManager* manager,
                                const CGameObject* object ) const {
    return ( memory().item().evaluate( object ) );
}

bool CCustomMonster::useful( const CEnemyManager* manager,
                             const CEntityAlive* object ) const {
    return ( memory().enemy().useful( object ) );
}

float CCustomMonster::evaluate( const CEnemyManager* manager,
                                const CEntityAlive* object ) const {
    return ( memory().enemy().evaluate( object ) );
}

bool CCustomMonster::useful( const CDangerManager* manager,
                             const CDangerObject& object ) const {
    return ( memory().danger().useful( object ) );
}

float CCustomMonster::evaluate( const CDangerManager* manager,
                                const CDangerObject& object ) const {
...
bool CCustomMonster::update_critical_wounded( const u16& bone_id,
                                              const float& power ) {
    ...
    float time_delta =
        m_last_hit_time
            ? float( Device.dwTimeGlobal - m_last_hit_time ) / 1000.f
            : 0.f;
    m_critical_wound_accumulator +=
        power - m_critical_wound_decrease_quant * time_delta;
...
extern void dbg_draw_frustum( float FOV,
                              float _FAR,
                              float A,
                              Fvector& P,
                              Fvector& D,
...
void CCustomMonster::OnRender() {
...

    {
        ...
        for ( ; i != e; ++i )
            Level().debug_renderer().draw_aabb(
                ai().level_graph().vertex_position( *i ), radius, radius,
                radius, D3DCOLOR_XRGB( 255, 22, 255 ) );
    }

    for ( int i = 0; i < 1; ++i ) {
        const xr_vector< CDetailPathManager::STravelPoint >& keys =
            !i ? movement().detail().m_key_points
               : movement().detail().m_key_points;
        const xr_vector< DetailPathManager::STravelPathPoint >& path =
            !i ? movement().detail().path() : movement().detail().path();
        u32 color0 =
            !i ? D3DCOLOR_XRGB( 0, 255, 0 ) : D3DCOLOR_XRGB( 0, 0, 255 );
        u32 color1 =
            !i ? D3DCOLOR_XRGB( 255, 0, 0 ) : D3DCOLOR_XRGB( 255, 255, 0 );
        u32 color2 =
            !i ? D3DCOLOR_XRGB( 0, 0, 255 ) : D3DCOLOR_XRGB( 0, 255, 255 );
        u32 color3 =
            !i ? D3DCOLOR_XRGB( 255, 255, 255 ) : D3DCOLOR_XRGB( 255, 0, 255 );
        ...
        float radius1 = !i ? .2f : .3f;
        {
            for ( u32 I = 1; I < path.size(); ++I ) {
                ...
                if ( !fis_zero( P1.distance_to_sqr( P2 ), EPS_L ) )
                    Level().debug_renderer().draw_line( Fidentity, P1, P2,
                                                        color0 );
                if ( ( path.size() - 1 ) == I ) // песледний box?
                    Level().debug_renderer().draw_aabb( P1, radius0, radius0,
                                                        radius0, color1 );
                else
                    Level().debug_renderer().draw_aabb( P1, radius0, radius0,
                                                        radius0, color2 );
            ...
            for ( u32 I = 1; I < keys.size(); ++I ) {
                ...
                temp = keys[ I - 1 ];
                Fvector P1;
                P1.set( temp.position.x,
                        ai().level_graph().vertex_plane_y( temp.vertex_id ),
                        temp.position.y );
                P1.y += 0.1f;

                temp = keys[ I ];
                Fvector P2;
                P2.set( temp.position.x,
                        ai().level_graph().vertex_plane_y( temp.vertex_id ),
                        temp.position.y );
                P2.y += 0.1f;
                ...
                if ( !fis_zero( P1.distance_to_sqr( P2 ), EPS_L ) )
                    Level().debug_renderer().draw_line( Fidentity, P1, P2,
                                                        color1 );
                Level().debug_renderer().draw_aabb( P1, radius1, radius1,
                                                    radius1, color3 );
            }
        }
    }
    {
        ...
        P1.y += 1.f;
        Level().debug_renderer().draw_aabb( P1, .5f, 1.f, .5f,
                                            D3DCOLOR_XRGB( 255, 0, 0 ) );
    }
    if ( g_Alive() ) {
        if ( memory().enemy().selected() ) {
            Fvector P1 = memory()
                             .memory( memory().enemy().selected() )
                             .m_object_params.m_position;
            P1.y += 1.f;
            Level().debug_renderer().draw_aabb( P1, 1.f, 1.f, 1.f,
                                                D3DCOLOR_XRGB( 0, 0, 0 ) );
        }
    ...
    if ( psAI_Flags.test( aiFrustum ) ) {
        ...
        dbg_draw_frustum( new_fov, new_range, 1, eye_matrix.c, eye_matrix.k,
                          eye_matrix.j );
    }
    ...
    for ( ; I != E; ++I ) {
        ...
        Fvector const traj_start =
            pick.center - pick.z_axis * pick.sizes.z * 0.5f * inv_z;
        Fvector const traj_end =
            pick.center + pick.z_axis * pick.sizes.z * 0.5f * inv_z;

        Fvector const z_offs[] = {
            ( pick.x_axis * pick.sizes.x * 0.5f ) +
                ( pick.y_axis * pick.sizes.y * 0.5f ),
            ( -pick.x_axis * pick.sizes.x * 0.5f ) +
                ( pick.y_axis * pick.sizes.y * 0.5f ),
            ( pick.x_axis * pick.sizes.x * 0.5f ) -
                ( pick.y_axis * pick.sizes.y * 0.5f ),
        ...
        Level().debug_renderer().draw_line( Fidentity, traj_start,
                                            traj_start + z_normal,
                                            D3DCOLOR_XRGB( 128, 255, 128 ) );
        Level().debug_renderer().draw_line( Fidentity, traj_end,
                                            traj_end - z_normal,
                                            D3DCOLOR_XRGB( 128, 255, 128 ) );

        for ( u32 i = 0; i < sizeof( z_offs ) / sizeof( z_offs[ 0 ] ); ++i )
            Level().debug_renderer().draw_line(
                Fidentity, traj_start + z_offs[ i ], traj_end + z_offs[ i ],
                D3DCOLOR_XRGB( 255, 255, 128 ) );

        Fvector const hor_start =
            pick.center - pick.x_axis * pick.sizes.x * 0.5f * inv_x;
        Fvector const hor_end =
            pick.center + pick.x_axis * pick.sizes.x * 0.5f * inv_x;

        Fvector const x_offs[] = {
            ( pick.y_axis * pick.sizes.y * 0.5f ) +
                ( pick.z_axis * pick.sizes.z * 0.5f ),
            ( -pick.y_axis * pick.sizes.y * 0.5f ) +
                ( pick.z_axis * pick.sizes.z * 0.5f ),
            ( pick.y_axis * pick.sizes.y * 0.5f ) -
                ( pick.z_axis * pick.sizes.z * 0.5f ),
        ...
        Level().debug_renderer().draw_line( Fidentity, hor_start,
                                            hor_start + x_normal,
                                            D3DCOLOR_XRGB( 128, 255, 128 ) );
        Level().debug_renderer().draw_line( Fidentity, hor_end,
                                            hor_end - x_normal,
                                            D3DCOLOR_XRGB( 128, 255, 128 ) );

        for ( u32 i = 0; i < sizeof( x_offs ) / sizeof( x_offs[ 0 ] ); ++i )
            Level().debug_renderer().draw_line(
                Fidentity, hor_start + x_offs[ i ], hor_end + x_offs[ i ],
                D3DCOLOR_XRGB( 255, 255, 128 ) );

        Fvector const ver_start =
            pick.center - pick.y_axis * pick.sizes.y * 0.5f * inv_y;
        Fvector const ver_end =
            pick.center + pick.y_axis * pick.sizes.y * 0.5f * inv_y;
        ...
        Level().debug_renderer().draw_line( Fidentity, ver_start,
                                            ver_start + y_normal,
                                            D3DCOLOR_XRGB( 128, 255, 128 ) );
        Level().debug_renderer().draw_line( Fidentity, ver_end,
                                            ver_end - y_normal,
                                            D3DCOLOR_XRGB( 128, 255, 128 ) );

        for ( u32 i = 0; i < sizeof( y_offs ) / sizeof( y_offs[ 0 ] ); ++i )
            Level().debug_renderer().draw_line(
                Fidentity, ver_start + y_offs[ i ], ver_end + y_offs[ i ],
                D3DCOLOR_XRGB( 255, 255, 128 ) );

        Level().debug_renderer().draw_line( Fidentity, traj_start, traj_end,
                                            D3DCOLOR_XRGB( 255, 0, 0 ) );
    }
    ...
    for ( u32 i = 0; i < m_jump_collide_tris.size(); i += 3 ) {
        ...
        Level().debug_renderer().draw_line( unit, v1, v2,
                                            D3DCOLOR_XRGB( 255, 255, 255 ) );
        Level().debug_renderer().draw_line( unit, v1, v3,
                                            D3DCOLOR_XRGB( 255, 255, 255 ) );
        Level().debug_renderer().draw_line( unit, v2, v3,
                                            D3DCOLOR_XRGB( 255, 255, 255 ) );
    }
}
#endif // DEBUG
...
void CCustomMonster::destroy_anim_mov_ctrl() {
    ...
    movement().enable_movement(
        m_movement_enabled_before_animation_controller );

    float roll;
    XFORM().getHPB( movement().m_body.current.yaw,
                    movement().m_body.current.pitch, roll );
...
Fvector CCustomMonster::spatial_sector_point() {
    return ISpatialOwner::spatial_sector_point().add(
        Fvector().set( 0.f, Radius() * .5f, 0.f ) );
}
