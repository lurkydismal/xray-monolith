...
struct SPhraseDialogData : CSharedResource {
    ...
    // произвольное число - приоритет диалога (0 по умолчанию), может быть
    // отрицательным в окне выбора у актера диалоги будут сортироваться по этому
    // значению от меньшего (снизу) к большему (сверху)
    int m_iPriority;
};
...
private:
    typedef CSharedClass< SPhraseDialogData, shared_str, false >
        inherited_shared;
...
public:
    ...
    virtual bool Precondition( const CGameObject* pSpeaker1,
    ...
    CPhraseDialogManager* OurPartner(
...
protected:
    ...
    void AddPhrase( CUIXml* pXml,
                    XML_NODE* phrase_node,
                    const shared_str& phrase_id,
                    const shared_str& prev_phrase_id );

public:
    CPhrase* AddPhrase( LPCSTR text,
                        const shared_str& phrase_id,
                        const shared_str& prev_phrase_id,
                        int goodwil_level );

    CPhrase* AddPhrase_script( LPCSTR text,
                               LPCSTR phrase_id,
                               LPCSTR prev_phrase_id,
                               int goodwil_level ) {
