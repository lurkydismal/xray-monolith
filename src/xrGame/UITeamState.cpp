...
int UITeamState::InitScrollPanels() {
    XML_NODE* scroll_panels_root =
        mainUiXml->NavigateToNode( "scroll_panels", 0 );
    VERIFY2( scroll_panels_root, "scroll_panels tag not found" );
    XML_NODE* tempRoot = mainUiXml->GetLocalRoot();

    int panels_count =
        mainUiXml->ReadAttribInt( scroll_panels_root, "count", 0 );
    ...
    for ( int i = 0; i < panels_count; ++i ) {
        ...
        CUIXmlInit::InitScrollView( *mainUiXml, "scroll_panel", 0,
                                    temp_panel.first );
...
inline UITeamState::TScrollPanels::size_type
UITeamState::GetNeedScrollPanelIndex() {
...
void UITeamState::ReStoreAllPlayers() {
    ...
    for ( PlayerIter i = myPlayers.begin(); i != ite; ++i ) {
        ...
        m_scroll_panels[ i->second.m_panel_number ].first->RemoveWindow(
            i->second.m_player_wnd );
        TScrollPanels::size_type new_store_panel = GetNeedScrollPanelIndex();

        VERIFY( new_store_panel < m_scroll_panels.size() );
        m_scroll_panels[ new_store_panel ].first->AddWindow(
            i->second.m_player_wnd, false );
...
void UITeamState::AddPlayer( ClientID const& clientId ) {
...
#ifdef DEBUG
    Msg( "--- UITeamState: adding player (ClientID = 0x%08x) to %d team "
         "(0x%08x)",
         clientId.value(), myTeam, this );
#endif // #ifdef DEBUG

    UIPlayerItem* tempPlayerItem = xr_new< UIPlayerItem >(
        static_cast< ETeam >( ps->team ), clientId, this, m_teamPanels );

    VERIFY2( tempPlayerItem,
             make_string( "failed to create player with ClientID = 0x%08x",
                          clientId.value() )
                 .c_str() );
    ...
    myPlayers.insert( std::make_pair(
        clientId, TPlayerItem( tempPlayerItem, panel_index ) ) );
}
...
bool UITeamState::UpdatePlayer( ClientID const& clientId ) {
    ...
    if ( tempIter != myPlayers.end() ) {
        retVal = true;
        game_cl_GameState::PLAYERS_MAP& playersMap = Game().players;
        game_cl_GameState::PLAYERS_MAP::iterator pi =
            playersMap.find( clientId );
        VERIFY2(
            pi != playersMap.end(),
            "player not found in Game().player list, but in UI list it exist" );
        game_PlayerState* ps = pi->second;
        // it can be NULL... and player will be removed by player item window
        VERIFY( ps );
        /*if (!ps)
        {
                Msg("--- Player state of ClientID = 0x%08x is NULL",
        clientId.value()); return true;
        }*/
        if ( Game().IsPlayerInTeam( ps, myTeam ) == false ) {
            RemovePlayer( clientId );
            retVal = false; // tricky step :) player will be added by
                            // UITeamPanels::UpdatePlayer method :)
        }
...
s32 UITeamState::GetSummaryFrags() const {
    ...
    for ( PlCIter i = playersMap.begin(); i != eiter; ++i ) {
        ...
        R_ASSERT( tempGame );
        if ( static_cast< ETeam >( tempGame->ModifyTeam( ps->team ) ) ==
             myTeam )
...
void UITeamState::Update() {
    if ( toDeletePlayers.size() ) {
        ...
        for ( xr_vector< ClientID >::iterator i = toDeletePlayers.begin();
            ...
            VERIFY2( tempIter != myPlayers.end(),
                     "player not found while deleting" );
#ifdef DEBUG
            Msg( "--- UITeamState: deleting player (ClientID = 0x%08x) from %d "
                 "team (0x%08x)",
                 i->value(), myTeam, this );
#endif // #ifdef DEBUG
            VERIFY( m_scroll_panels.size() > tempIter->second.m_panel_number );
            m_scroll_panels[ tempIter->second.m_panel_number ]
                .first->RemoveWindow( tempIter->second.m_player_wnd );
    ...
    for ( TScrollPanels::iterator it = m_scroll_panels.begin(); it != ite;
          ++it ) {
