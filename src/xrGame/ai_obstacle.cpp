...
IC Fvector construct_position( u32 level_vertex_id, float x, float z ) {
    return ( Fvector().set(
        x, ai().level_graph().vertex_plane_y( level_vertex_id, x, z ), z ) );
}

IC bool ai_obstacle::inside( const Fvector& position,
...
IC bool ai_obstacle::inside( const u32& vertex_id ) const {
    ...
    return ( inside( construct_position( vertex_id, position.x + offset,
                                         position.z + offset ),
                     EPS_L, .3f, 6 ) ||
             inside( construct_position( vertex_id, position.x + offset,
                                         position.z - offset ),
                     EPS_L, .3f, 6 ) ||
             inside( construct_position( vertex_id, position.x - offset,
                                         position.z + offset ),
             ...
             inside( construct_position( vertex_id, position.x - offset,
                                         position.z - offset ),
                     EPS_L, .3f, 6 ) ||
             inside( Fvector().set( position.x, position.y, position.z ), EPS_L,
                     .3f, 6 ) );
}
...
void ai_obstacle::compute_matrix( Fmatrix& result, const Fvector& additional ) {
    ...
    if ( visible_bone_count == 1 ) {
        result.mul_43( before_scale_matrix,
                       Fmatrix().scale( last_half_size.add( additional ) ) );
        return;
    }
...
void ai_obstacle::prepare_inside( Fvector& min, Fvector& max ) {
    ...
    Fmatrix matrix;
    float half_cell_size = ( use_additional_radius ? 1.f : 0.f ) *
...
void ai_obstacle::compute_impl() {
    ...
    const CPosition& min_vertex_position =
        level_graph.vertex_position( min_position );
    const CPosition& max_vertex_position =
        level_graph.vertex_position( max_position );
