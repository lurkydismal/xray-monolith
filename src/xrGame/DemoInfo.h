...
// if the string is longer than max_size, then reads max_size-1 bytes of string,
// and jumps to the end of string void stream_read_demostring	(CStreamReader*
// stream, shared_str & dest_rest, u32 max_size = STREAM_DEMOSTRING_MAX_SIZE);
// void stream_write_demostring(IWriter* writer, shared_str const &
// string_to_write, u32 max_size = STREAM_DEMOSTRING_MAX_SIZE);

class demo_player_info : private xray::noncopyable {
private:
    shared_str m_name;
    s16 m_frags;
    s16 m_deaths;
...
class demo_info : private xray::noncopyable {
private:
...
public:
    ...
    typedef bool ( *sorting_less_comparator )( demo_player_info const*,
                                               demo_player_info const* );
