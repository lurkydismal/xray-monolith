...
private:
    u32 guess_vertex_id( u32 const& current_vertex_id,
...
public:
    ...
    IC u32 max_x() const;
    IC u32 max_z() const;
    IC void begin( const CVertex& vertex,
                   const_iterator& begin,
                   const_iterator& end ) const;
    IC void begin( const CVertex* vertex,
                   const_iterator& begin,
                   const_iterator& end ) const;
    IC void begin( u32 vertex_id,
                   const_iterator& begin,
                   const_iterator& end ) const;
    ...
    IC void unpack_xz( const CLevelGraph::CPosition& vertex_position,
                       u32& x,
                       u32& z ) const;
    IC void unpack_xz( const CLevelGraph::CPosition& vertex_position,
                       int& x,
    ...
    IC const Fvector
    vertex_position( const CLevelGraph::CPosition& source_position ) const;
    IC const Fvector& vertex_position(
    ...
    IC const Fvector vertex_position( const CVertex* vertex ) const;
    IC float vertex_plane_y( const CVertex& vertex,
                             const float X,
                             const float Z ) const;
    IC float vertex_plane_y( const CVertex* vertex,
                             const float X,
                             const float Z ) const;
    IC float vertex_plane_y( const u32 vertex_id,
                             const float X,
                             const float Z ) const;
    IC float vertex_plane_y( const CVertex& vertex ) const;
    IC float vertex_plane_y( const CVertex* vertex ) const;
    IC float vertex_plane_y( const u32 vertex_id ) const;
    IC bool inside( const CVertex& vertex,
                    const CLevelGraph::CPosition& vertex_position ) const;
    IC bool inside( const CVertex& vertex,
                    const Fvector& vertex_position ) const;
    IC bool inside( const CVertex* vertex,
                    const CLevelGraph::CPosition& vertex_position ) const;
    IC bool inside( const CVertex* vertex,
                    const Fvector& vertex_position ) const;
    IC bool inside( const u32 vertex_id,
                    const CLevelGraph::CPosition& vertex_position ) const;
    ...
    IC float distance( const Fvector& position,
    ...
    IC ELineIntersections intersect( float x1,
                                     float y1,
                                     float x2,
                                     float y2,
                                     float x3,
                                     ...
                                     float* x,
                                     float* y ) const;
    IC ELineIntersections intersect_no_check( float x1,
                                              float y1,
                                              float x2,
                                              float y2,
                                              float x3,
    ...
    IC void intersect( SSegment& segment,
                       const SContour& contour0,
                       const SContour& contour1 ) const;
    IC float nearest( Fvector& destination,
                      const Fvector& position,
                      const Fvector& point0,
                      const Fvector& point1 ) const;
    ...
    IC float nearest( Fvector& destination,
                      const Fvector& position,
                      const SContour& contour ) const;
    IC bool intersect( Fvector& destination,
                       const Fvector& v1,
                       const Fvector& v2,
                       const Fvector& v3,
                       const Fvector& v4 ) const;
    IC float square( float a1, float b1, float alpha = PI_DIV_2 ) const;
    IC float compute_square( float angle,
                             float AOV,
                             float b0,
                             float b1,
                             float b2,
                             float b3 ) const;
    IC float compute_high_square( float angle,
                                  float AOV,
                                  const CVertex* vertex ) const;
    IC float compute_low_square( float angle,
                                 float AOV,
                                 const CVertex* vertex ) const;
    ...
    float cover_in_direction( float angle,
                              float b0,
                              float b1,
                              float b2,
                              float b3 ) const;
    IC float high_cover_in_direction( float angle,
                                      const CVertex* vertex ) const;
    ...
    IC float vertex_high_cover_angle( u32 vertex_id,
                                      float inc_angle,
                                      _predicate compare_predicate ) const;
    template < class _predicate >
    IC float vertex_low_cover_angle( u32 vertex_id,
                                     float inc_angle,
                                     _predicate compare_predicate ) const;
    ...
    void choose_point( const Fvector& start_point,
                       const Fvector& finish_point,
                       const SContour& contour,
                       int vertex_id,
                       Fvector& temp_point,
                       int& saved_index ) const;
    IC bool check_vertex_in_direction( u32 start_vertex_id,
                                       const Fvector& start_position,
                                       u32 finish_vertex_id ) const;
    IC u32 check_position_in_direction( u32 start_vertex_id,
                                        const Fvector& start_position,
                                        const Fvector& finish_position ) const;
    float check_position_in_direction( u32 start_vertex_id,
                                       const Fvector& start_position,
                                       const Fvector& finish_position,
                                       const float max_distance ) const;
    float mark_nodes_in_direction( u32 start_vertex_id,
                                   const Fvector& start_position,
    ...
    float mark_nodes_in_direction( u32 start_vertex_id,
                                   ...
                                   xr_vector< u32 >& vertex_stack,
                                   xr_vector< bool >* vertex_marks ) const;
    float farthest_vertex_in_direction(
        u32 start_vertex_id,
    ...
    bool create_straight_path( u32 start_vertex_id,
                               const Fvector& start_point,
                               const Fvector& finish_point,
    ...
    IC bool create_straight_path( u32 start_vertex_id,
                                  const Fvector2& start_point,
                                  const Fvector2& finish_point,
    ...
    template < typename P >
    IC void iterate_vertices( const Fvector& min_position,
    ...
    bool check_vertex_in_direction_slow( u32 start_vertex_id,
                                         const Fvector2& start_position,
                                         u32 finish_vertex_id ) const;
    ...
    bool neighbour_in_direction( const Fvector& direction,
                                 u32 start_vertex_id ) const;
    void nearby_vertices( const Fvector& position,
                          float radius,
