...
void modify( const int& vertex_id, Fbox& bounding_box ) {
    ...
    for ( ; I != E; ++I )
        bounding_box.modify(
            graph.vertex( graph.value( vertex_id, I ) )->game_point() );
}
...
void CLevelGraph::draw_edge( const int& vertex_id0, const int& vertex_id1 ) {
    ...
    if ( psAI_Flags.test( aiDrawGameGraphRealPos ) ) {
        ...
        position0 =
            convert_position( graph.vertex( vertex_id0 )->game_point() );
        position1 =
            convert_position( graph.vertex( vertex_id1 )->game_point() );
    }
    ...
    //	RCache.dbg_DrawAABB
    //(position0,radius,radius,radius,vertex_color); 	RCache.dbg_DrawAABB
    //(position1,radius,radius,radius,vertex_color); 	RCache.dbg_DrawLINE
    //(Fidentity,position0,position1,edge_color);
}
...
void CLevelGraph::draw_stalkers( const int& vertex_id ) {
    ...
    const OBJECT_REGISTRY& objects =
        ai().alife().graph().objects()[ vertex_id ].objects();
    ...
    if ( show_text ) {
        ...
        for ( ; I != E; ++I ) {
            ...
            CSE_ALifeHumanStalker* stalker =
                smart_cast< CSE_ALifeHumanStalker* >( object );
            if ( !stalker )
                continue;
            ...
            const float& walked_distance =
                ( path.size() < 2 )
                    ? 0.f
                    : stalker->brain().movement().detail().walked_distance();
            //			font.OutNext
            //("%s",stalker->name_replace());
            ...
            if ( psAI_Flags.test( aiDrawGameGraphRealPos ) )
                ...
                position = convert_position(
                    graph.vertex( stalker->m_tGraphID )->game_point() );
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( psAI_Flags.test( aiDrawGameGraphRealPos ) ) {
            ...
            position0 = convert_position(
                graph.vertex( game_vertex_id0 )->game_point() );
            position1 = convert_position(
                graph.vertex( game_vertex_id1 )->game_point() );
            distance = graph.vertex( game_vertex_id0 )
                           ->game_point()
                           .distance_to(
                               graph.vertex( game_vertex_id1 )->game_point() );
        }
    ...
    if ( show_text ) {
        ...
        for ( ; I != E; ++I ) {
            ...
            const float& walked_distance =
                ( path.size() < 2 )
                    ? 0.f
                    : monster->brain().movement().detail().walked_distance();
            //			font.OutNext
            //("%s",monster->name_replace());
            ...
            if ( psAI_Flags.test( aiDrawGameGraphRealPos ) )
                ...
                position = convert_position(
                    graph.vertex( monster->m_tGraphID )->game_point() );
...
void CLevelGraph::draw_game_graph() {
    ...
    // draw back plane
    Fvector vertices[ 4 ];
    xform.transform_tiny(
        vertices[ 0 ], Fvector().set( center.x - bounds.x, center.y + bounds.y,
                                      center.z + bounds.z ) );
    xform.transform_tiny(
        vertices[ 1 ], Fvector().set( center.x + bounds.x, center.y + bounds.y,
                                      center.z + bounds.z ) );
    xform.transform_tiny(
        vertices[ 2 ], Fvector().set( center.x - bounds.x, center.y - bounds.y,
                                      center.z - bounds.z ) );
    xform.transform_tiny(
        vertices[ 3 ], Fvector().set( center.x + bounds.x, center.y - bounds.y,
                                      center.z - bounds.z ) );
    ...
    for (int i=0; i<(int)ai().game_graph().header().vertex_count(); ++i) {
            ...
            for ( ; I != E; ++I) {
                    Fvector t2 =
    ai().game_graph().vertex((*I).vertex_id())->game_point(); t2.y += .6f;
                    NORMALIZE_VECTOR(t2);
            ...
            //out of screen
            if (S.z < 0 || S.w < 0)
    continue; if (S.x < -1.f || S.x > 1.f || S.y<-1.f || S.x>1.f)
    continue; F->SetSizeI	(0.05f/_sqrt(_abs(S.w)));
    ...
    }
    {
            ...
            if( path.size() ){
                    Fvector t1 =
    ai().game_graph().vertex(path.back())->game_point(); t1.y += .6f;
                    NORMALIZE_VECTOR(t1);
                    Level().debug_renderer().draw_aabb(t1,.05f,.05f,.05f,D3DCOLOR_XRGB(0,0,255));
                    for (int i=(int)path.size() - 2; i>=0;--i) {
                            Fvector t2 =
    ai().game_graph().vertex(path[i])->game_point(); t2.y += .6f;
                            NORMALIZE_VECTOR(t2);
    ...
    if (GameID() == eGameIDSingle && ai().get_alife()) {
            {
                    GameGraph::_LEVEL_ID	J =
    ai().game_graph().vertex(ai().alife().graph().actor()->m_tGraphID)->level_id();
                    for (int i=0,
    n=(int)ai().game_graph().header().vertex_count(); i<n; ++i) { if
    (ai().game_graph().vertex(i)->level_id() != J) continue; Fvector t1 =
    ai().game_graph().vertex(i)->level_point(), t2 =
    ai().game_graph().vertex(i)->game_point(); t1.y += .6f; t2.y += .6f;
                            NORMALIZE_VECTOR(t2);
                            ...
                            if (S.z < 0 || S.w < 0)
    continue; if (S.x < -1.f || S.x > 1.f || S.y<-1.f || S.x>1.f)
    continue; F->SetSizeI	(0.1f/_sqrt(_abs(S.w)));
                            F->SetColor(0xffffffff);
                            F->OutI(S.x,-S.y,"%d",i);
                    }
            }
    ...
    ai().alife().objects().objects().end(); for ( ; I != E; ++I) {
                    {
                            CSE_ALifeMonsterAbstract *tpALifeMonsterAbstract =
    smart_cast<CSE_ALifeMonsterAbstract *>((*I).second); if
    (tpALifeMonsterAbstract && tpALifeMonsterAbstract->m_bDirectControl &&
    !tpALifeMonsterAbstract->m_bOnline) { CSE_ALifeHumanAbstract *tpALifeHuman =
    smart_cast<CSE_ALifeHumanAbstract *>(tpALifeMonsterAbstract); if
    (tpALifeHuman && tpALifeHuman->brain().movement().detail().path().size()) {
                                            Fvector t1 =
    ai().game_graph().vertex(tpALifeHuman->brain().movement().detail().path().back())->game_point();
                                            t1.y += .6f;
                                            NORMALIZE_VECTOR(t1);
                                            Level().debug_renderer().draw_aabb(t1,.05f,.05f,.05f,D3DCOLOR_XRGB(0,0,255));
                                            for (int
    i=(int)tpALifeHuman->brain().movement().detail().path().size() - 2;
    i>=0;--i) { Fvector t2 =
    ai().game_graph().vertex(tpALifeHuman->brain().movement().detail().path()[i])->game_point();
                                                    t2.y += .6f;
                                                    NORMALIZE_VECTOR(t2);
                                                    Level().debug_renderer().draw_aabb(t2,.05f,.05f,.05f,D3DCOLOR_XRGB(0,0,255));
                                                    Level().debug_renderer().draw_line(Fidentity,t1,t2,D3DCOLOR_XRGB(0,0,255));
                                    ...
                                    if
    (tpALifeMonsterAbstract->m_fDistanceToPoint > EPS_L) { Fvector t1 =
    ai().game_graph().vertex(tpALifeMonsterAbstract->m_tGraphID)->game_point();
                                            Fvector t2 =
    ai().game_graph().vertex(tpALifeMonsterAbstract->m_tNextGraphID)->game_point();
                                            t2.sub(t1);
                                            ...
                                            Fvector t1 =
    ai().game_graph().vertex((*I).second->m_tGraphID)->game_point(); t1.y +=
    .6f; NORMALIZE_VECTOR(t1);
                                            Level().debug_renderer().draw_aabb(t1,.05f,.05f,.05f,D3DCOLOR_XRGB(255,0,0));
                                    }
                            }
                            else {
                                    CSE_ALifeInventoryItem
    *l_tpALifeInventoryItem = smart_cast<CSE_ALifeInventoryItem*>((*I).second);
                                    if (l_tpALifeInventoryItem &&
    !l_tpALifeInventoryItem->attached()) { Fvector t1 =
    ai().game_graph().vertex((*I).second->m_tGraphID)->game_point(); t1.y +=
    .6f; NORMALIZE_VECTOR(t1);
    ...
    *tpALifeCreatureActor = smart_cast<CSE_ALifeCreatureActor*>((*I).second); if
    (tpALifeCreatureActor) { Fvector t1 =
    ai().game_graph().vertex((*I).second->m_tGraphID)->game_point(); t1.y +=
    .6f; NORMALIZE_VECTOR(t1);
                                                    ...
                                                    CSE_ALifeTrader
    *tpALifeTrader = smart_cast<CSE_ALifeTrader*>((*I).second); if
    (tpALifeTrader) { Fvector t1 =
    ai().game_graph().vertex((*I).second->m_tGraphID)->game_point(); t1.y +=
    .6f; NORMALIZE_VECTOR(t1);
                                                            ...
                                                            CSE_ALifeSmartZone
    *smart_zone = smart_cast<CSE_ALifeSmartZone*>((*I).second); if (smart_zone)
    { Fvector t1 =
    ai().game_graph().vertex((*I).second->m_tGraphID)->game_point(); t1.y +=
    .6f; NORMALIZE_VECTOR(t1);
                                                                    Level().debug_renderer().draw_aabb(t1,.05f,.05f,.05f,D3DCOLOR_XRGB(255,0,0));
