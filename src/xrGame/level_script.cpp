...
CScriptGameObject* tpfGetActor() {
    ...
    if ( first_time )
        ai().script_engine().script_log( eLuaMessageTypeError,
                                         "Do not use level.actor function!" );
    first_time = false;
...
CScriptGameObject* get_object_by_name( LPCSTR caObjectName ) {
    ...
    if ( first_time )
        ai().script_engine().script_log( eLuaMessageTypeError,
                                         "Do not use level.object function!" );
    first_time = false;

    CGameObject* l_tpGameObject = smart_cast< CGameObject* >(
        Level().Objects.FindObjectByName( caObjectName ) );
    if ( l_tpGameObject )
...
CScriptGameObject* get_object_by_id( u16 id ) {
    CGameObject* pGameObject =
        smart_cast< CGameObject* >( Level().Objects.net_Find( id ) );
    if ( !pGameObject )
        return nullptr;
...
// demonized: Sets weather and force updates next environment so the
// interpolation will happen between current environment and next weather's
// environment
...
#endif // #ifdef INGAME_EDITOR
        return (
            g_pGamePersistent->Environment().SetWeatherFX( weather_name ) );
...
bool start_weather_fx_from_time( LPCSTR weather_name, float time ) {
    ...
    if ( !Device.editor() )
#endif // #ifdef INGAME_EDITOR
        return ( g_pGamePersistent->Environment().StartWeatherFXFromTime(
            weather_name, time ) );
...
u32 get_time_days() {
    u32 year = 0, month = 0, day = 0, hours = 0, mins = 0, secs = 0,
        milisecs = 0;
    split_time( ( g_pGameLevel && Level().game )
                    ? Level().GetGameTime()
...
u32 vertex_in_direction( u32 level_vertex_id,
                         Fvector direction,
                         float max_distance ) {
    if ( !ai().level_graph().valid_vertex_id( level_vertex_id ) ) {
        return u32( -1 );
    ...
    u32 result = u32( -1 );
    ai().level_graph().farthest_vertex_in_direction(
        level_vertex_id, start_position, finish_position, result, 0 );
    return ( ai().level_graph().valid_vertex_id( result ) ? result
                                                          : level_vertex_id );
}
...
void map_pan_to( LPCSTR level_name, float x, float z, bool zoom_in ) {
    ...
    if ( !mapWnd )
        return;
    mapWnd->SetTargetMap( shared_str( level_name ), Fvector2().set( x, z ),
                          zoom_in );
}
...
void add_call( const ::luabind::functor< bool >& condition,
...
void remove_call( const ::luabind::functor< bool >& condition,
                  const ::luabind::functor< void >& action ) {
...
void add_call( const ::luabind::object& lua_object,
               LPCSTR condition,
               LPCSTR action ) {
    //	try{
    //		CPHScriptObjectCondition
    //*c=xr_new<CPHScriptObjectCondition>(lua_object,condition);
    //		CPHScriptObjectAction
    //*a=xr_new<CPHScriptObjectAction>(lua_object,action);
    ::luabind::functor< bool > _condition =
        object_cast< ::luabind::functor< bool > >( lua_object[ condition ] );
    ::luabind::functor< void > _action =
        object_cast< ::luabind::functor< void > >( lua_object[ action ] );
    CPHScriptObjectConditionN* c =
        xr_new< CPHScriptObjectConditionN >( lua_object, _condition );
    CPHScriptObjectActionN* a =
        xr_new< CPHScriptObjectActionN >( lua_object, _action );
...
void remove_call( const ::luabind::object& lua_object,
                  LPCSTR condition,
                  LPCSTR action ) {
...
void add_call( const ::luabind::object& lua_object,
               ...
               const ::luabind::functor< void >& action ) {
    CPHScriptObjectConditionN* c =
        xr_new< CPHScriptObjectConditionN >( lua_object, condition );
    CPHScriptObjectActionN* a =
        xr_new< CPHScriptObjectActionN >( lua_object, action );
    Level().ph_commander_scripts().add_call( c, a );
}
...
void disable_input() {
    // "unpress" all keys when we disable level input! (but keep input devices
    // aquired)
...
void iterate_sounds( LPCSTR prefix,
                     u32 max_count,
                     const CScriptCallbackEx< void >& callback ) {
...
void iterate_sounds1( LPCSTR prefix,
                      u32 max_count,
...
void iterate_sounds2( LPCSTR prefix,
                      u32 max_count,
...
float add_cam_effector( LPCSTR fn, int id, bool cyclic, LPCSTR cb_func ) {
    CAnimatorCamEffectorScriptCB* e =
        xr_new< CAnimatorCamEffectorScriptCB >( cb_func );
...
float add_cam_effector( LPCSTR fn,
                        int id,
                        bool cyclic,
                        LPCSTR cb_func,
                        float cam_fov ) {
...
bool getCamEffectorTransformData( ::luabind::object& t, LPCSTR animationFile ) {
    ...
    if ( !FS.exist( full_path, "$level$", animationFile ) )
        if ( !FS.exist( full_path, "$game_anims$", animationFile ) ) {
            Msg( "![getCamEffectorTransformData] Can't find motion file '%s'.",
                 animationFile );
            return false;
        }
        ...
        if ( 0 == xr_strcmp( ext, ".anm" ) ) {
            ...
            if ( M.LoadMotion( full_path ) ) {
                ...
                for ( const auto& p : mapOrder ) {
                    ...
                    ::luabind::object keyData =
                        ::luabind::newtable( ai().script_engine().lua() );
                    int i = 1;
                    for ( KeyIt k_it = keys.begin(); k_it != keys.end();
                          k_it++ ) {
                        ::luabind::object data =
                            ::luabind::newtable( ai().script_engine().lua() );
                        data[ "time" ] = ( *k_it )->time;
                ...
                Msg( "![getCamEffectorTransformData] failed to load file '%s'.",
                     animationFile );
                return false;
            }
        } else {
            Msg( "![getCamEffectorTransformData] file has incorrect extension "
                 "'%s'.",
                 animationFile );
            return false;
        }
    }
    Msg( "![getCamEffectorTransformData] unknown error, file '%s'.",
         animationFile );
    return false;
}

// demonized: Set custom camera position and direction with movement smoothing
// (for cutscenes, etc)
void set_cam_position_direction( Fvector& position,
                                 Fvector& direction,
...
void set_cam_position_direction( Fvector& position,
                                 ...
                                 unsigned int smoothing,
                                 bool hudEnabled ) {
    set_cam_position_direction( position, direction, smoothing, hudEnabled,
                                false );
}
...
void set_cam_effector_factor( int id, float factor ) {
    CAnimatorCamEffectorScriptCB* e =
        smart_cast< CAnimatorCamEffectorScriptCB* >(
            Actor()->Cameras().GetCamEffector( ( ECamEffectorType )id ) );
    if ( e )
        e->SetPower( factor );
}
...
void remove_pp_effector( int id ) {
    CPostprocessAnimator* pp = smart_cast< CPostprocessAnimator* >(
        Actor()->Cameras().GetPPEffector( ( EEffectorPPType )id ) );
...
int g_community_goodwill( LPCSTR _community, int _entity_id ) {
    ...
    return RELATION_REGISTRY().GetCommunityGoodwill( c.index(),
                                                     u16( _entity_id ) );
}
...
void g_set_community_goodwill( LPCSTR _community, int _entity_id, int val ) {
    CHARACTER_COMMUNITY c;
    c.set( _community );
    RELATION_REGISTRY().SetCommunityGoodwill( c.index(), u16( _entity_id ),
                                              val );
}
...
void g_change_community_goodwill( LPCSTR _community, int _entity_id, int val ) {
    CHARACTER_COMMUNITY c;
    c.set( _community );
    RELATION_REGISTRY().ChangeCommunityGoodwill( c.index(), u16( _entity_id ),
                                                 val );
}
...
int g_get_community_relation( LPCSTR comm_from, LPCSTR comm_to ) {
    ...
    return RELATION_REGISTRY().GetCommunityRelation( community_from.index(),
                                                     community_to.index() );
}
...
void g_set_community_relation( LPCSTR comm_from, LPCSTR comm_to, int value ) {
    ...
    RELATION_REGISTRY().SetCommunityRelation( community_from.index(),
                                              community_to.index(), value );
}

int g_get_general_goodwill_between( u16 from, u16 to ) {
    CHARACTER_GOODWILL presonal_goodwill =
        RELATION_REGISTRY().GetGoodwill( from, to );
    VERIFY( presonal_goodwill != NO_GOODWILL );

    CSE_ALifeTraderAbstract* from_obj = smart_cast< CSE_ALifeTraderAbstract* >(
        ai().alife().objects().object( from ) );
    CSE_ALifeTraderAbstract* to_obj = smart_cast< CSE_ALifeTraderAbstract* >(
        ai().alife().objects().object( to ) );

    if ( !from_obj || !to_obj ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "RELATION_REGISTRY::get_general_goodwill_between  : cannot convert "
            "obj to CSE_ALifeTraderAbstract!" );
        return ( 0 );
    }
    ...
    CHARACTER_GOODWILL community_to_community_goodwill =
        RELATION_REGISTRY().GetCommunityRelation( from_obj->Community(),
                                                  to_obj->Community() );

    return presonal_goodwill + community_to_obj_goodwill +
           community_to_community_goodwill;
}

void refresh_npc_names() {
    CALifeObjectRegistry::OBJECT_REGISTRY alobjs =
        ai().alife().objects().objects();
    ...
    for ( ; it != it_e; it++ ) {
        CSE_ALifeTraderAbstract* tr =
            smart_cast< CSE_ALifeTraderAbstract* >( it->second );
        if ( tr ) {
            tr->m_character_name =
                TranslateName( tr->m_character_name_str.c_str() );

            if ( g_pGameLevel ) {
                CObject* obj = g_pGameLevel->Objects.net_Find( it->first );
                if ( obj ) {
                    CInventoryOwner* owner =
                        smart_cast< CInventoryOwner* >( obj );
                    if ( owner ) {
...
void start_tutorial( LPCSTR name ) {
    ...
    if ( g_tutorial2 )
        g_tutorial->m_pStoredInputReceiver =
            g_tutorial2->m_pStoredInputReceiver;
}
...
void set_weather_value_numric( LPCSTR name, float val ) {
    ...
    if ( 0 == xr_strcmp( name, "sky_rotation" ) )
        ...
        Msg( "~xrGame\level_script.cpp (set_weather_value_numric) | [%s] is "
             "not a valid numric weather parameter to set",
             name );
}
...
void set_weather_value_vector( LPCSTR name,
                               float x,
                               float y,
                               float z,
                               float w = 0 ) {
    ...
    if ( 0 == xr_strcmp( name, "sky_color" ) )
        ...
        Msg( "~xrGame\level_script.cpp (set_weather_value_vector) | [%s] is "
             "not a valid vector weather parameter to set",
             name );
}
...
void set_weather_value_string( LPCSTR name, LPCSTR newval ) {
    ...
    if ( 0 == xr_strcmp( name, "clouds_texture" ) ) {
        ...
        E.lens_flare_id = environment()->eff_LensFlare->AppendDef(
            *environment(), environment()->m_suns_config, newval );
    } else if ( 0 == xr_strcmp( name, "thunderbolt_collection" ) ) {
        E.tb_id = environment()->eff_Thunderbolt->AppendDef(
        ...
        Msg( "~xrGame\level_script.cpp (set_weather_value_string) | [%s] is "
             "not a valid string weather parameter to set",
             name );
}
...
void boost_weather_value( LPCSTR name, float value ) {
    if ( 0 == xr_strcmp( name, "ambient_color" ) )
        ...
        Msg( "~xrGame\level_script.cpp (boost_weather_value)| [%s] is not a "
             "valid weather parameter to boost",
             name );
}
...
void sun_time( int hour, int minute ) {
    ...
    CEnvDescriptor& E = *environment()->CurrentEnv;
    E.sun_dir.setHP( deg2rad( real_sun_alt ), deg2rad( real_sun_long ) );
...
void hud_adj_offs( int off, int idx, float x, float y, float z ) {
    ...
    // Fire point/dir ; shell point
        ...
        g_player_hud->m_adjust_firepoint_shell[ off ][ idx - 10 ].set( x, y,
                                                                       z );
    }
...
u32 PlayHudMotion( u8 hand,
                   LPCSTR itm_name,
                   LPCSTR anm_name,
                   bool bMixIn = true,
                   float speed = 1.f ) {
    return g_player_hud->script_anim_play( hand, itm_name, anm_name, bMixIn,
                                           speed );
}
...
void remove_hud_model( LPCSTR section ) {
    ...
    if ( itm0 &&
         itm0->m_parent_hud_item->object().cNameSect().equal( section ) ) {
    ...
    if ( itm1 &&
         itm1->m_parent_hud_item->object().cNameSect().equal( section ) ) {
...
void AddBullet( Fvector pos,
    ...
    Level().BulletManager().AddBullet( pos, dir, speed, power, impulse, sender,
                                       sender, hit_type, max_dist, *_temp,
                                       air_resistance, true );
    delete_data( _temp );
}
...
void AddBullet( ::luabind::object t ) {
    if ( t && t.type() == LUA_TTABLE ) {
        ...
        ALife::EHitType hit_type =
            ::luabind::object_cast< ALife::EHitType >( t[ "hit_type" ] );
        float max_dist = ::luabind::object_cast< float >( t[ "max_dist" ] );
        LPCSTR ammo_sect = ::luabind::object_cast< LPCSTR >( t[ "ammo_sect" ] );
        float air_resistance =
            ::luabind::object_cast< float >( t[ "air_resistance" ] );

        // momopate: add senderweapon id to the table as to not break most
        // damage mods, defaults to sender
        ...
        Level().BulletManager().AddBullet(
            pos, dir, speed, power, impulse, sender, senderweapon, hit_type,
            max_dist, *_temp, air_resistance, true );
        delete_data( _temp );
    } else {
...
const Fvector3 world2ui_with_depth( Fvector pos,
                                    bool hud = false,
    ...
    if ( !allow_offscreen ) {
        if ( v_res.z < 0 || v_res.w < 0 )
            return { -9999, 0 };
        if ( abs( v_res.x ) > 1.f || abs( v_res.y ) > 1.f )
            return { -9999, 0 };
    }
...
// demonized: unproject ui coordinates (ie mouse cursor coordinates) to world
// coordinates returns position and underlying object id if found. If there is
// no object, obj_id will be 65535
void ui2world( Fvector2 pos, bool allow_offscreen, Fvector& res, u16& obj_id ) {
    res.set( 0, 0, 0 );
    if ( !allow_offscreen && ( pos.x < 0 || pos.x > UI_BASE_WIDTH ||
                               pos.y < 0 || pos.y > UI_BASE_HEIGHT ) ) {
        return;
    }
    ...
    // 4x4 invert of camera matrix
    {
        ...
        float mProjectDet =
            m._11 * ( m._22 * m._33 * m._44 + m._23 * m._34 * m._42 +
                      m._24 * m._32 * m._43 - m._24 * m._33 * m._42 -
                      m._23 * m._32 * m._44 - m._22 * m._34 * m._43 ) -
            m._21 * ( m._12 * m._33 * m._44 + m._13 * m._34 * m._42 +
                      m._14 * m._32 * m._43 - m._14 * m._33 * m._42 -
                      m._13 * m._32 * m._44 - m._12 * m._34 * m._43 ) +
            m._31 * ( m._12 * m._23 * m._44 + m._13 * m._24 * m._42 +
                      m._14 * m._22 * m._43 - m._14 * m._23 * m._42 -
                      m._13 * m._22 * m._44 - m._12 * m._24 * m._43 ) -
            m._41 * ( m._12 * m._23 * m._34 + m._13 * m._24 * m._32 +
                      m._14 * m._22 * m._33 - m._14 * m._23 * m._32 -
                      m._13 * m._22 * m._34 - m._12 * m._24 * m._33 );

        Fmatrix mProjectAdjugate;
        mProjectAdjugate._11 = m._22 * m._33 * m._44 + m._23 * m._34 * m._42 +
                               m._24 * m._32 * m._43 - m._24 * m._33 * m._42 -
                               m._23 * m._32 * m._44 - m._22 * m._34 * m._43;
        mProjectAdjugate._12 = -m._12 * m._33 * m._44 - m._13 * m._34 * m._42 -
                               m._14 * m._32 * m._43 + m._14 * m._33 * m._42 +
                               m._13 * m._32 * m._44 + m._12 * m._34 * m._43;
        mProjectAdjugate._13 = m._12 * m._23 * m._44 + m._13 * m._24 * m._42 +
                               m._14 * m._22 * m._43 - m._14 * m._23 * m._42 -
                               m._13 * m._22 * m._44 - m._12 * m._24 * m._43;
        mProjectAdjugate._14 = -m._12 * m._23 * m._34 - m._13 * m._24 * m._32 -
                               m._14 * m._22 * m._33 + m._14 * m._23 * m._32 +
                               m._13 * m._22 * m._34 + m._12 * m._24 * m._33;

        mProjectAdjugate._21 = -m._21 * m._33 * m._44 - m._23 * m._34 * m._41 -
                               m._24 * m._31 * m._43 + m._24 * m._33 * m._41 +
                               m._23 * m._31 * m._44 + m._21 * m._34 * m._43;
        mProjectAdjugate._22 = m._11 * m._33 * m._44 + m._13 * m._34 * m._41 +
                               m._14 * m._31 * m._43 - m._14 * m._33 * m._41 -
                               m._13 * m._31 * m._44 - m._11 * m._34 * m._43;
        mProjectAdjugate._23 = -m._11 * m._23 * m._44 - m._13 * m._24 * m._41 -
                               m._14 * m._21 * m._43 + m._14 * m._23 * m._41 +
                               m._13 * m._21 * m._44 + m._11 * m._24 * m._43;
        mProjectAdjugate._24 = m._11 * m._23 * m._34 + m._13 * m._24 * m._31 +
                               m._14 * m._21 * m._33 - m._14 * m._23 * m._31 -
                               m._13 * m._21 * m._34 - m._11 * m._24 * m._33;

        mProjectAdjugate._31 = m._21 * m._32 * m._44 + m._22 * m._34 * m._41 +
                               m._24 * m._31 * m._42 - m._24 * m._32 * m._41 -
                               m._22 * m._31 * m._44 - m._21 * m._34 * m._42;
        mProjectAdjugate._32 = -m._11 * m._32 * m._44 - m._12 * m._34 * m._41 -
                               m._14 * m._31 * m._42 + m._14 * m._32 * m._41 +
                               m._12 * m._31 * m._44 + m._11 * m._34 * m._42;
        mProjectAdjugate._33 = m._11 * m._22 * m._44 + m._12 * m._24 * m._41 +
                               m._14 * m._21 * m._42 - m._14 * m._22 * m._41 -
                               m._12 * m._21 * m._44 - m._11 * m._24 * m._42;
        mProjectAdjugate._34 = -m._11 * m._22 * m._34 - m._12 * m._24 * m._31 -
                               m._14 * m._21 * m._32 + m._14 * m._22 * m._31 +
                               m._12 * m._21 * m._34 + m._11 * m._24 * m._32;

        mProjectAdjugate._41 = -m._21 * m._32 * m._43 - m._22 * m._33 * m._41 -
                               m._23 * m._31 * m._42 + m._23 * m._32 * m._41 +
                               m._22 * m._31 * m._43 + m._21 * m._33 * m._42;
        mProjectAdjugate._42 = m._11 * m._32 * m._43 + m._12 * m._33 * m._41 +
                               m._13 * m._31 * m._42 - m._13 * m._32 * m._41 -
                               m._12 * m._31 * m._43 - m._11 * m._33 * m._42;
        mProjectAdjugate._43 = -m._11 * m._22 * m._43 - m._12 * m._23 * m._41 -
                               m._13 * m._21 * m._42 + m._13 * m._22 * m._41 +
                               m._12 * m._21 * m._43 + m._11 * m._23 * m._42;
        mProjectAdjugate._44 = m._11 * m._22 * m._33 + m._12 * m._23 * m._31 +
                               m._13 * m._21 * m._32 - m._13 * m._22 * m._31 -
                               m._12 * m._21 * m._33 - m._11 * m._23 * m._32;
    ...
    res.set( pos.x, pos.y, 1 );
    {
        ...
        auto y = res.y;
        auto z = res.z;
        float w = 1.0 / ( e[ 0 ][ 3 ] * x + e[ 1 ][ 3 ] * y + e[ 2 ][ 3 ] * z +
                          e[ 3 ][ 3 ] );

        res.x = ( e[ 0 ][ 0 ] * x + e[ 1 ][ 0 ] * y + e[ 2 ][ 0 ] * z +
                  e[ 3 ][ 0 ] ) *
                w;
        res.y = ( e[ 0 ][ 1 ] * x + e[ 1 ][ 1 ] * y + e[ 2 ][ 1 ] * z +
                  e[ 3 ][ 1 ] ) *
                w;
        res.z = ( e[ 0 ][ 2 ] * x + e[ 1 ][ 2 ] * y + e[ 2 ][ 2 ] * z +
                  e[ 3 ][ 2 ] ) *
                w;
    }
    ...
    obj_id = 65535;
    if ( Level().ObjectSpace.RayPick( start, dir, range, collide::rqtBoth, R,
                                      ignore ) ) {
...
const float get_env_rads() {
    ...
    return CurrentGameUI()
        ->UIMainIngameWnd->get_hud_states()
        ->get_main_sensor_value();
}
...
void g_send( NET_Packet& P,
             ...
             bool bSendImmediately = 0 ) {
    Level().Send( P, net_flags( bReliable, bSequential, bHighPriority,
                                bSendImmediately ) );
}

// can spawn entities like bolts, phantoms, ammo, etc. which normally crash when
// using alife():create()
void spawn_section( LPCSTR sSection,
                    Fvector3 vPosition,
                    u32 LevelVertexID,
                    u16 ParentID,
                    bool bReturnItem = false ) {
    Level().spawn_item( sSection, vPosition, LevelVertexID, ParentID,
                        bReturnItem );
}

enum ETraceTarget { TT_CAMERA = 0, TT_WEAPON, TT_DEVICE, TT_ACTOR, TT_MAX };
...
void actor_set_safemode( bool status ) {
    if ( Actor()->is_safemode() != status ) {
        CWeapon* wep =
            smart_cast< CWeapon* >( Actor()->inventory().ActiveItem() );
...
// KD: raypick
bool ray_pick( const Fvector& start,
               const Fvector& dir,
               float range,
               collide::rq_target tgt,
               script_rq_result& script_R,
...
CScriptGameObject* get_view_entity_script() {
    CGameObject* pGameObject =
        smart_cast< CGameObject* >( Level().CurrentViewEntity() );
    if ( !pGameObject )
        return ( 0 );
...
void iterate_nearest( const Fvector& pos,
                      float radius,
    ...
    std::sort( m_nearest.begin(), m_nearest.end(),
               [ &pos ]( CObject* o1, CObject* o2 ) {
                   return o1->Position().distance_to_sqr( pos ) <
                          o2->Position().distance_to_sqr( pos );
               } );
...
LPCSTR PickMaterial( const Fvector& start_pos,
                     const Fvector& dir,
                     float trace_dist,
                     CScriptGameObject* ignore_obj ) {
    collide::rq_result result;
    BOOL reach_wall =
        Level().ObjectSpace.RayPick(
            start_pos, dir, trace_dist, collide::rqtStatic, result,
...
CScriptIniFile* GetVisualUserdata( LPCSTR visual ) {
    ...
    CScriptIniFile* ini =
        xr_new< CScriptIniFile >( UD, FS.get_path( "$game_config$" )->m_Path );
    FS.r_close( data );
    UD->close();
...
void change_game_news_show_time( CUIWindow* CUIWindowPItem, float show_time ) {
    ...
    pItem->SetColorAnimation( "ui_main_msgs_short",
                              LA_ONLYALPHA | LA_TEXTCOLOR | LA_TEXTURECOLOR,
                              show_time );
}
...
void update_pda_news_from_uiwindow( CUIWindow* CUIWindowPItem ) {
    ...
    GAME_NEWS_VECTOR& news_vector =
        Actor()->game_news_registry->registry().objects();
    GAME_NEWS_IT newsVectorRes =
        std::find_if( news_vector.begin(), news_vector.end(),
                      [ &news ]( const GAME_NEWS_DATA& d ) {
    ...
    if ( newsVectorRes != news_vector.end() ) {
        ...
        Msg( "![update_pda_news_from_uiwindow] cannot find news by text %s",
             news->news_text.c_str() );
    }
}
...
void take_screenshot( LPCSTR path,
                      Fvector2 dimensions,
                      IRender_interface::DxEncoding dx_encoding ) {
...
void CLevel::script_register( lua_State* L ) {
    ...
    module( L, "debug_render" )
        [ def( "add_object",
               ( ( DBG_ScriptObject * ( * )( u32, DebugRenderType ) ) &
                 add_object ) ),
          def( "add_object",
               ( ( DBG_ScriptObject * ( * )( LPCSTR, DebugRenderType ) ) &
                 add_object ) ),
          def( "remove_object", ( ( void ( * )( u32 ) )&remove_object ) ),
          def( "remove_object", ( ( void ( * )( LPCSTR ) )&remove_object ) ),
          def( "get_object",
               ( ( DBG_ScriptObject * ( * )( u32 ) ) & get_object ) ),
          def( "get_object",
               ( ( DBG_ScriptObject * ( * )( LPCSTR ) ) & get_object ) ),
          def( "get_flags", get_flags ), def( "set_flags", set_flags ) ];

    module(
        L )[ class_< enum_exporter< ETraceTarget > >( "ETraceTarget" )
                 .enum_( "trace_targets" )
    ...
    // Antglobes: DirectX Encodings (Parsed as DX10/11 equivalent if renderer 3+
    // used)
    module(
        L )[ class_< enum_exporter< IRender_interface::DxEncoding > >(
                 "DxEncoding" )
                 .enum_( "DxEncoding" )
                     [ value( "A8R8G8B8",
                              int( IRender_interface::eDXE_A8R8G8B8 ) ),
    ...
    // Alundaio: Extend level namespace exports
        ...
        def( "get_target_obj",
             ( ( CScriptGameObject * ( * )() ) &
               g_get_target_obj ) ), // intentionally named to what is in xray
                                     // extensions
        def( "get_target_obj",
             ( ( CScriptGameObject * ( * )( ETraceTarget ) ) &
               g_get_target_obj ) ), // intentionally named to what is in xray
                                     // extensions

        def( "get_target_dist", ( ( float ( * )() )&g_get_target_dist ) ),
        def( "get_target_dist",
             ( ( float ( * )( ETraceTarget ) )&g_get_target_dist ) ),

        def( "get_target_element",
             ( ( u32 ( * )() )&g_get_target_element ) ), // Can get bone cursor
                                                         // is targetting
        def( "get_target_element",
             ( ( u32 ( * )(
                 ETraceTarget ) )&g_get_target_element ) ), // Can get bone
                                                            // cursor is
                                                            // targetting
        ...
        def( "get_target_pos",
             ( ( Fvector ( * )( ETraceTarget ) )&g_get_target_pos ) ),
        // antglobes: Get Sun Position in World Space
        def( "get_sun_pos", ( ( Fvector ( * )() )&get_sun_pos ) ),
        // antglobes: Get the float represenation of the sun's color in rgb
        // format
        ...
        def( "take_screenshot",
             ( ( void ( * )(
                 LPCSTR, Ivector2,
                 IRender_interface::DxEncoding ) )&take_screenshot ) ),
        // def("add_cam_effector", ((float (*)(LPCSTR, int, bool,
        // LPCSTR))&add_cam_effector)),

        // demonized: get result of crosshair ray query
        def( "get_target_result",
             ( ( script_rq_result ( * )() )&g_get_target_result ) ),
        def( "get_target_result", ( ( script_rq_result ( * )(
                                      ETraceTarget ) )&g_get_target_result ) ),
        ...
        def( "object_by_id",
             ( ( CScriptGameObject * ( * )( u16 ) ) & get_object_by_id ) ),
        def( "object_by_id",
             ( ( CScriptGameObject * ( * )() ) & get_object_by_id ) ),
        def( "object_by_id",
        ...
        def( "debug_actor", tpfGetActor ), def( "check_object", check_object ),
#endif
        ...
        def( "rain_factor", rain_factor ), def( "rain_wetness", rain_wetness ),
        def( "rain_hemi", rain_hemi ),
        def( "patrol_path_exists", patrol_path_exists ),
        def( "vertex_position", vertex_position ), def( "name", get_name ),
        ...
        def( "map_get_object_minimap_spot_static",
             map_get_minimap_spot_static ),
        ...
        def( "add_call",
             ( ( void ( * )( const ::luabind::functor< bool >&,
                             const ::luabind::functor< void >& ) )&add_call ) ),
        def( "add_call",
             ( ( void ( * )( const ::luabind::object&,
                             const ::luabind::functor< bool >&,
                             const ::luabind::functor< void >& ) )&add_call ) ),
        def( "add_call", ( ( void ( * )( const ::luabind::object&, LPCSTR,
                                         LPCSTR ) )&add_call ) ),
        def( "remove_call",
             ( ( void ( * )(
                 const ::luabind::functor< bool >&,
                 const ::luabind::functor< void >& ) )&remove_call ) ),
        def(
            "remove_call",
            ( ( void ( * )(
                const ::luabind::object&, const ::luabind::functor< bool >&,
                const ::luabind::functor< void >& ) )&remove_call ) ),
        def( "remove_call", ( ( void ( * )( const ::luabind::object&, LPCSTR,
                                            LPCSTR ) )&remove_call ) ),
        ...
        def( "add_cam_effector",
             ( ( float ( * )( LPCSTR, int, bool, LPCSTR,
                              float ) )&add_cam_effector ) ),
        def( "add_cam_effector",
             ( ( float ( * )( LPCSTR, int, bool, LPCSTR, float,
                              bool ) )&add_cam_effector ) ),
        def( "add_cam_effector",
             ( ( float ( * )( LPCSTR, int, bool, LPCSTR, float, bool,
                              float ) )&add_cam_effector ) ),

        // demonized: Set custom camera position and direction with movement
        // smoothing (for cutscenes, etc)
        def( "set_cam_custom_position_direction",
             ( ( void ( * )( Fvector&, Fvector&, unsigned int, bool,
                             bool ) )&set_cam_position_direction ) ),
        def( "remove_hud_motion_cam_effectors",
             &remove_hud_motion_cam_effectors ),
        def( "set_cam_custom_position_direction",
             ( ( void ( * )( Fvector&, Fvector&, unsigned int,
                             bool ) )&set_cam_position_direction ) ),
        def( "set_cam_custom_position_direction",
             ( ( void ( * )( Fvector&, Fvector&,
                             unsigned int ) )&set_cam_position_direction ) ),
        def( "set_cam_custom_position_direction",
             ( ( void ( * )( Fvector&,
                             Fvector& ) )&set_cam_position_direction ) ),
        def( "remove_cam_custom_position_direction",
             &remove_cam_position_direction ),
        ...
        def( "add_bullet", ( ( void ( * )( Fvector, Fvector, float, float,
                                           float, u16, ALife::EHitType, float,
                                           LPCSTR, float ) )&AddBullet ) ),
        ...
        def( "remove_attachment",
             ( void ( * )( script_attachment* ) )&RemoveAttachment ),
        ...
        module( L,
                "actor_stats" )[ def( "add_points", &add_actor_points ),
                                 def( "add_points_str", &add_actor_points_str ),
                                 def( "get_points", &get_actor_points ) ];
    module( L )
        [ class_< CRayPick >( "ray_pick" )
              .def( constructor<>() )
              .def( constructor< Fvector&, Fvector&, float, collide::rq_target,
                                 CScriptGameObject* >() )
          ...
          class_< script_rq_result >( "rq_result" )
              ...
              .def_readonly( "material_flags",
                             &script_rq_result::pMaterialFlags )
              .def_readonly( "material_phfriction",
                             &script_rq_result::fPHFriction )
              .def_readonly( "material_phdamping",
                             &script_rq_result::fPHDamping )
              .def_readonly( "material_phspring", &script_rq_result::fPHSpring )
              .def_readonly( "material_phbounce_start_velocity",
                             &script_rq_result::fPHBounceStartVelocity )
              .def_readonly( "material_phbouncing",
                             &script_rq_result::fPHBouncing )
              .def_readonly( "material_flotation_factor",
                             &script_rq_result::fFlotationFactor )
              .def_readonly( "material_shoot_factor",
                             &script_rq_result::fShootFactor )
              .def_readonly( "material_shoot_factor_mp",
                             &script_rq_result::fShootFactorMP )
              .def_readonly( "material_bounce_damage_factor",
                             &script_rq_result::fBounceDamageFactor )
              .def_readonly( "material_injurious_speed",
                             &script_rq_result::fInjuriousSpeed )
              .def_readonly( "material_vis_transparency_factor",
                             &script_rq_result::fVisTransparencyFactor )
              .def_readonly( "material_snd_occlusion_factor",
                             &script_rq_result::fSndOcclusionFactor )
              .def_readonly( "material_density_factor",
                             &script_rq_result::fDensityFactor )
              .def( constructor<>() ),
    ...
    module( L, "weather" )[ def( "get_value_numric", get_weather_value_numric ),
    ...
    module( L, "hud_adjust" )[ def( "enabled", hud_adj_state ),
    ...
    module( L, "relation_registry" )
        [ def( "community_goodwill", &g_community_goodwill ),
          ...
          def( "get_general_goodwill_between",
               &g_get_general_goodwill_between ) ];
    module( L, "game" )
        [ class_< xrTime >( "CTime" )
              .enum_( "date_format" )
                  [ value( "DateToDay",
                           int( InventoryUtilities::edpDateToDay ) ),
                    value( "DateToMonth",
                           int( InventoryUtilities::edpDateToMonth ) ),
                    value( "DateToYear",
                           int( InventoryUtilities::edpDateToYear ) ) ]
              .enum_( "time_format" )
                  [ value( "TimeToHours",
                           int( InventoryUtilities::etpTimeToHours ) ),
                    value( "TimeToMinutes",
                           int( InventoryUtilities::etpTimeToMinutes ) ),
                    value( "TimeToSeconds",
                           int( InventoryUtilities::etpTimeToSeconds ) ),
                    value( "TimeToMilisecs",
                           int( InventoryUtilities::etpTimeToMilisecs ) ) ]
              .def( constructor<>() )
              ...
              .def( "set", &xrTime::set )
              .def( "get", &xrTime::get,
                    out_value< 2 >() + out_value< 3 >() + out_value< 4 >() +
                        out_value< 5 >() + out_value< 6 >() + out_value< 7 >() +
                        out_value< 8 >() )
          ...
          def( "ui2world", ( void ( * )( Fvector2, Fvector&, u16& ) )&ui2world,
               pure_out_value< 2 >() + pure_out_value< 3 >() ),
          def( "ui2world", ( void ( * )( Fvector&, Fvector&, u16& ) )&ui2world,
               pure_out_value< 2 >() + pure_out_value< 3 >() ),
          def( "ui2world_offscreen",
               ( void ( * )( Fvector2, Fvector&, u16& ) )&ui2world_offscreen,
               pure_out_value< 2 >() + pure_out_value< 3 >() ),
          def( "ui2world_offscreen",
               ( void ( * )( Fvector&, Fvector&, u16& ) )&ui2world_offscreen,
               pure_out_value< 2 >() + pure_out_value< 3 >() ),

          // demonized: adjust game news time
          def( "change_game_news_show_time", &change_game_news_show_time ),
          def( "update_pda_news_from_uiwindow",
               &update_pda_news_from_uiwindow ),
