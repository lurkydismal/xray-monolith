...
IC void CCar::SWheel::applywheelCollisionParams( const dxGeomUserData* ud,
                                                 bool& do_colide,
                                                 dContact& c,
                                                 SGameMtl* material_1,
                                                 SGameMtl* material_2 ) {
    if ( ud && ud->object_callbacks &&
         ud->object_callbacks->HasCallback( WheellCollisionCallback ) ) {
        SWheelCollisionParams& cp =
            *( ( SWheelCollisionParams* )( ud->callback_data ) );
        dSurfaceParameters& sp = c.surface;
        sp.mu *= cp.mu_factor;
        MulSprDmp( sp.soft_cfm, sp.soft_cfm, cp.spring_factor,
                   cp.damping_factor );
    }
}

void CCar::SWheel::WheellCollisionCallback( bool& do_colide,
                                            bool bo1,
                                            dContact& c,
                                            SGameMtl* material_1,
                                            SGameMtl* material_2 ) {
...
void CCar::SWheel::Init() {
    ...
    R_ASSERT2( bone->second.element,
               "No Element was created for wheel. Check collision is set" );
    bone->second.element->set_DynamicLimits( default_l_limit,
                                             default_w_limit * 100.f );
    ...
    // R_ASSERT2(dJointGetType(joint->GetDJoint())==dJointTypeHinge2,"No wheel
    // join was set for a wheel, only wheel-joint valid!!!");
    R_ASSERT2( joint->IsWheelJoint(),
...
void CCar::SWheel::Load( LPCSTR section ) {
    ...
    if ( ini->section_exist( section ) ) {
        collision_params.damping_factor =
            READ_IF_EXISTS( ini, r_float, section, "damping_factor",
                            collision_params.damping_factor );
        collision_params.spring_factor =
            READ_IF_EXISTS( ini, r_float, section, "spring_factor",
                            collision_params.spring_factor );
        collision_params.mu_factor =
            READ_IF_EXISTS( ini, r_float, section, "friction_factor",
                            collision_params.mu_factor );
    } else if ( ini->section_exist( "wheels_params" ) ) {
        collision_params.damping_factor =
            ini->r_float( "wheels_params", "damping_factor" );
        collision_params.spring_factor =
            ini->r_float( "wheels_params", "spring_factor" );
        collision_params.mu_factor =
            ini->r_float( "wheels_params", "friction_factor" );
    }
}
...
void CCar::SWheel::ApplyDriveAxisTorque( float torque ) {
    if ( !joint )
        return;
    // dJointSetHinge2Param(joint->GetDJoint(),
    // dParamFMax2,torque);//car->m_axle_friction
...
void CCar::SWheelDrive::Init() {
    ...
    /*CBoneData& bone_data =
    smart_cast<IKinematics*>(pwheel->car->Visual())->LL_GetData(u16(pwheel->bone_id));
    switch (bone_data.IK_data.type)
    {
    case jtWheel:
            pos_fvd =
    bone_map.find(pwheel->bone_id)->second.element->mXFORM.k.x; break;
...
void CCar::SWheelDrive::UpdatePower() {
    ...
    pwheel->ApplyDriveAxisTorque( c->RefWheelCurTorque() / gear_factor *
                                  c->ThrottleFactor() );
}
...
void CCar::SWheelSteer::Init() {
    IKinematics* pKinematics =
        smart_cast< IKinematics* >( pwheel->car->Visual() );
    pwheel->Init();
    ( bone_map.find( pwheel->bone_id ) )
        ->second.joint->GetLimits( lo_limit, hi_limit, 0 );
    /*CBoneData& bone_data = pKinematics->LL_GetData(u16(pwheel->bone_id));
    switch (bone_data.IK_data.type)
    {
    case jtWheel:
            pos_right =
    bone_map.find(pwheel->bone_id)->second.element->mXFORM.i.y;
    ...
    */
    pos_right = -1.f;
    float steering_torque = pKinematics->LL_UserData()->r_float(
        "car_definition", "steering_torque" );
    VERIFY( pwheel );
    pwheel->ApplySteerAxisTorque( steering_torque );
    VERIFY( pwheel->joint );
    // dJointSetHinge2Param(pwheel->joint->GetDJoint(), dParamFudgeFactor,
    // 0.005f/steering_torque);
...
float CCar::SWheelSteer::GetSteerAngle() {
    ...
    VERIFY( pwheel->joint );
    return -pos_right *
           pwheel->joint->GetAxisAngle(
               0 ); // dJointGetHinge2Angle1 (pwheel->joint->GetDJoint());
}
...
void CCar::SWheelBreak::Load( LPCSTR section ) {
    ...
    hand_break_torque = READ_IF_EXISTS( ini, r_float, "car_definition",
                                        "hand_break_torque", break_torque );
    if ( ini->section_exist( section ) ) {
        break_torque = READ_IF_EXISTS( ini, r_float, section, "break_torque",
                                       break_torque );
        hand_break_torque = READ_IF_EXISTS(
            ini, r_float, section, "hand_break_torque", hand_break_torque );
    }
}
