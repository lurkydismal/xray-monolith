...
void __stdcall account_manager::only_log_creation_cb( bool success,
                                                      char const* descr ) {
    if ( success ) {
...
void __stdcall account_manager::only_log_profdel_cb( bool success,
                                                     char const* descr ) {
    if ( success ) {
...
void __stdcall account_manager::only_log_profiles( u32 const profiles_count,
    ...
    if ( profiles_count ) {
        ...
        for ( profiles_store_t::const_iterator i = m_result_profiles.begin(),
                                               ie = m_result_profiles.end();
              i != ie; ++i ) {
            Msg( "- %s", i->c_str() );
        }
        return;
    }
    Msg( "- No GameSpy account profiles found: %s",
         description ? description : "unknown error" );
}

void __stdcall account_manager::only_log_suggestions(
    u32 const profiles_count,
    char const* description ) {
    if ( description )
        Msg( "- GameSpy suggested unique nicks: %s", description );

    for ( suggested_nicks_t::const_iterator i = m_suggested_nicks.begin(),
                                            ie = m_suggested_nicks.end();
          i != ie; ++i ) {
        Msg( "- %s", i->c_str() );
    }
}
...
bool account_manager::verify_unique_nick( char const* unick ) {
    ...
    if ( unick_length < GP_UNIQUENICK_MIN_LEN ) {
        Msg( "! ERROR: unique nick name is too short (must be greater that 2 "
             "character)" );
...
bool account_manager::verify_email( char const* email ) {
    ...
    if ( ( tmp_char == tmp_endchar ) || ( tmp_char == email ) ||
         ( tmp_char + 1 == tmp_endchar ) || !isalnum( *( tmp_char + 1 ) ) ||
         !isalnum( *( tmp_char - 1 ) ) ) {
        Msg( "! ERROR: bad email" );
...
void account_manager::create_profile( char const* nick,
    ...
    if ( !opcb )
        m_account_creation_cb.bind( this,
                                    &account_manager::only_log_creation_cb );
    else
        m_account_creation_cb = opcb;

    if ( !verify_nick( nick ) || !verify_unique_nick( unique_nick ) ||
         !verify_email( email ) || !verify_password( password ) ) {
...
void account_manager::delete_profile( account_operation_cb dpcb ) {
    if ( !dpcb ) {
        m_profile_deleting_cb.bind( this,
                                    &account_manager::only_log_profdel_cb );
    } else {
...
void account_manager::get_account_profiles_raw(
    get_account_params_t const& args,
    account_profiles_cb profiles_cb ) {
...
void account_manager::search_for_email_raw(
    search_for_email_params_t const& email,
...
void account_manager::suggest_unique_nicks_raw(
    suggest_uniqie_nicks_params_t const& unick,
    suggest_nicks_cb sncb ) {
...
void __cdecl account_manager::new_user_cb( GPConnection* connection,
                                           void* arg,
                                           void* param ) {
    // GPNewUserResponseArg* creation_resp =
    // static_cast<GPNewUserResponseArg*>(arg); VERIFY(creation_resp != NULL);
...
void __cdecl account_manager::user_nicks_cb( GPConnection* connection,
                                             void* arg,
                                             void* param ) {
    // account_manager* tmp_inst			=
    // static_cast<account_manager*>(param); VERIFY(tmp_inst);
    // GPGetUserNicksResponseArg* tmp_arg	=
    // static_cast<GPGetUserNicksResponseArg*>(arg);
    //
...
void __cdecl account_manager::unicks_suggestion_cb( GPConnection* connection,
    ...
    // account_manager* tmp_inst				=
    // static_cast<account_manager*>(param); VERIFY(tmp_inst);
    // GPSuggestUniqueNickResponseArg*	tmp_arg =
    // static_cast<GPSuggestUniqueNickResponseArg*>(arg); VERIFY(tmp_arg);
...
void __cdecl account_manager::delete_profile_cb( GPConnection* connection,
    ...
    // account_manager* tmp_inst				=
    // static_cast<account_manager*>(param); VERIFY(tmp_inst);
    // GPDeleteProfileResponseArg* tmp_arg		=
    // static_cast<GPDeleteProfileResponseArg*>(arg); if (tmp_arg->result !=
    // GP_NO_ERROR)
    //{
    ...
    // MainMenu()->GetLoginMngr(); VERIFY(tmp_lmngr);
...
void __cdecl account_manager::search_profile_cb( GPConnection* connection,
    ...
    // account_manager* tmp_inst				=
    // static_cast<account_manager*>(param); VERIFY(tmp_inst);
    // GPProfileSearchResponseArg* tmp_arg		=
    // static_cast<GPProfileSearchResponseArg*>(arg);
    //
