...
void CWeaponSSRS::Load( LPCSTR section ) {
    ...
    fAiOneShotTime =
        READ_IF_EXISTS( pSettings, r_float, section, "ai_rpm", fOneShotTime );
}
...
void CWeaponSSRS::UnloadRocket() {
    while ( getRocketCount() > 0 ) {
        Msg( "%s:%d [%d]-[%s]", __FUNCTION__, __LINE__, getRocketCount(),
             getCurrentRocket()->cNameSect_str() );
        NET_Packet P;
...
void CWeaponSSRS::state_Fire( float dt ) {
    if ( iAmmoElapsed > 0 ) {
        ...
        while ( !m_magazine.empty() && fShotTimeCounter < 0 &&
            ...
            float launch_speed = READ_IF_EXISTS(
                pSettings, r_float, ammo_name, "ammo_grenade_vel",
                CRocketLauncher::m_fLaunchSpeed );
#endif
            if ( E ) {
                CInventoryOwner* io =
                    smart_cast< CInventoryOwner* >( H_Parent() );
                if ( NULL == io->inventory().ActiveItem() ) {
                    Log( "current_state", GetState() );
                    Log( "next_state", GetNextState() );
                    Log( "item_sect", cNameSect().c_str() );
            ...
            Fvector::generate_orthonormal_basis(
                launch_matrix.k, launch_matrix.j, launch_matrix.i );
            ...
            VERIFY2( _valid( launch_matrix ),
                     "CWeaponSSRS::state_Fire. Invalid launch_matrix" );
