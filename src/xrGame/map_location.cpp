...
CMapLocation::CMapLocation( LPCSTR type, u16 object_id ) {
    ...
    m_actual_time = 0;
    m_owner_se_object = ( ai().get_alife() )
                            ? ai().alife().objects().object( m_objectID, true )
                            : NULL;
...
void CMapLocation::LoadSpot( LPCSTR type, bool bReload ) {
    ...
    R_ASSERT3( g_uiSpotXml->NavigateToNode( path_base, 0 ),
               "XML node not found in file map_spots.xml", path_base );
    ...
    if ( node ) {
        ...
        m_spot_border_names[ 0 ] = g_uiSpotXml->ReadAttrib(
            path, 0, "spot_a", "level_map_spot_border" );
        m_spot_border_names[ 1 ] =
            g_uiSpotXml->ReadAttrib( path, 0, "spot_na", "" );
    ...
    if ( node ) {
        ...
        m_spot_border_names[ 2 ] = g_uiSpotXml->ReadAttrib(
            path, 0, "spot_a", "mini_map_spot_border" );
        m_spot_border_names[ 3 ] =
            g_uiSpotXml->ReadAttrib( path, 0, "spot_na", "" );
    ...
    if ( node ) {
        ...
        m_spot_border_names[ 4 ] = g_uiSpotXml->ReadAttrib(
            path, 0, "spot_a", "complex_map_spot_border" );
        m_spot_border_names[ 5 ] =
            g_uiSpotXml->ReadAttrib( path, 0, "spot_na", "" );
    ...
    if ( m_minimap_spot == NULL && m_level_spot == NULL &&
         m_complex_spot == NULL ) {
        DisableSpot();
    }
}
...
const Fvector2& CMapLocation::CalcDirection() {
    if ( Level().CurrentViewEntity() &&
         Level().CurrentViewEntity()->ID() == m_objectID ) {
        m_cached.m_Direction.set( Device.vCameraDirection.x,
        ...
        if ( m_cached.m_graphID != m_owner_se_object->m_tGraphID ) {
            m_cached.m_LevelName =
                ai().game_graph()
                    .header()
                    .level( ai().game_graph()
                                .vertex( m_owner_se_object->m_tGraphID )
                                ->level_id() )
                    .name();
...
void CMapLocation::UpdateSpot( CUICustomMap* map, CMapSpot* sp ) {
    if ( map->MapName() == GetLevelName() ) {
        ...
        if ( b_alife && m_flags.test( eHideInOffline ) &&
             !m_owner_se_object->m_bOnline ) {
            return;
        }

        if ( b_alife && m_owner_se_object->m_flags.test(
                            CSE_ALifeObject::flVisibleForMap ) == FALSE ) {
            return;
        }

        if ( IsGameTypeSingle() ) {
            CGameTask* ml_task =
                Level().GameTaskManager().HasGameTask( this, true );
            if ( ml_task ) {
        ...
        m_position_on_map = map->ConvertRealToLocal(
            position, ( map->Heading() ) ? false : true );
        ...
        bool b_pointer =
            ( GetSpotPointer( sp ) && map->NeedShowPointer( wnd_rect ) );

        if ( map->Heading() ) {
            m_position_on_map =
                map->ConvertRealToLocal( position, true ); // for drawing
            sp->SetWndPos( m_position_on_map );
        }
        ...
        VERIFY( Actor() );
        GraphEngineSpace::CGameVertexParams params(
            Actor()->locations().vertex_types(), flt_max );
        bool res = ai().graph_engine().search(
            ai().game_graph(), Actor()->ai_location().game_vertex_id(),
        ...
        if ( res ) {
            ...
            if ( !bDone && bbb ) {
                Msg( "! Error. Path from actor to selected map spot does not "
                     "contain level changer :(" );
                Msg( "Path:" );
                ...
                for ( ; it != it_e; ++it ) {
                    ...
                    Msg( "[%d] level[%s]", ( *it ),
                         *ai().game_graph()
                              .header()
                              .level(
                                  ai().game_graph().vertex( *it )->level_id() )
                              .name() );
                }
                ...
                for ( lit = g_lchangers.begin(); lit != lit_e; ++lit ) {
                    GameGraph::_GRAPH_ID gid =
                        ( *lit )->ai_location().game_vertex_id();
                    Msg( "[%d]", gid );
                    ...
                    Msg( "lch_name=%s pos=%f %f %f",
                         *ai().game_graph()
                              .header()
                              .level(
                                  ai().game_graph().vertex( gid )->level_id() )
                              .name(),
                         p.x, p.y, p.z );
                }
            };
            if ( bDone ) {
                ...
                if ( it != it_e ) {
                    ...
                    if ( Actor()->Position().distance_to_sqr( p ) >
                         45.0f * 45.0f ) {
                        Fvector2 position;
                        position.set( p.x, p.z );
                        m_position_on_map =
                            map->ConvertRealToLocal( position, false );
                        UpdateSpotPointer( map, GetSpotPointer( sp ) );
                    }
                }
            }
...
void CMapLocation::UpdateSpotPointer( CUICustomMap* map, CMapSpotPointer* sp ) {
    ...
    if ( map->GetPointerTo( m_position_on_map, sp->GetWidth() / 2, pointer_pos,
                            heading ) ) {
        ...
        Fvector2 tt =
            map->ConvertLocalToReal( m_position_on_map, map->BoundRect() );
        Fvector ttt;
        ttt.set( tt.x, 0.0f, tt.y );

        if ( IsGameTypeSingle() ) {
            float dist_to_target =
                Level().CurrentEntity()->Position().distance_to( ttt );
            CGameTask* task =
                Level().GameTaskManager().HasGameTask( this, true );
            if ( task ) {
...
CMapSpot* CMapLocation::GetSpotBorder( CMapSpot* sp ) {
    ...
    if ( PointerEnabled() ) {
        ...
        // inactive state
        if ( sp == m_level_spot ) {
            if ( NULL == m_level_map_spot_border_na &&
                 m_spot_border_names[ 1 ].size() ) {
                m_level_map_spot_border_na = xr_new< CMapSpot >( this );
                m_level_map_spot_border_na->Load(
                    g_uiSpotXml, m_spot_border_names[ 1 ].c_str() );
            }
            return m_level_map_spot_border_na;
        } else if ( sp == m_minimap_spot ) {
            if ( NULL == m_mini_map_spot_border_na &&
                 m_spot_border_names[ 3 ].size() ) {
                m_mini_map_spot_border_na = xr_new< CMapSpot >( this );
                m_mini_map_spot_border_na->Load(
                    g_uiSpotXml, m_spot_border_names[ 3 ].c_str() );
            }
            return m_mini_map_spot_border_na;
        } else if ( sp == m_complex_spot ) {
            if ( NULL == m_complex_spot_border_na &&
                 m_spot_border_names[ 5 ].size() ) {
                m_complex_spot_border_na = xr_new< CMapSpot >( this );
                m_complex_spot_border_na->Load(
                    g_uiSpotXml, m_spot_border_names[ 5 ].c_str() );
            }
...
CRelationMapLocation::CRelationMapLocation( const shared_str& type,
                                            u16 object_id,
                                            u16 pInvOwnerActorID )
...
bool CRelationMapLocation::Update() {
    ...
    if ( m_owner_se_object ) {
        ...
        pAct = smart_cast< CSE_ALifeTraderAbstract* >(
            ai().alife().objects().object( m_pInvOwnerActorID, true ) );
        if ( !pEnt || !pAct )
            return false;

        m_last_relation = RELATION_REGISTRY().GetRelationType( pEnt, pAct );
        CSE_ALifeCreatureAbstract* pCreature =
            smart_cast< CSE_ALifeCreatureAbstract* >( m_owner_se_object );
        ...
        pEnt = smart_cast< CInventoryOwner* >(
            Level().Objects.net_Find( m_objectID ) );
        pAct = smart_cast< CInventoryOwner* >(
            Level().Objects.net_Find( m_pInvOwnerActorID ) );
        if ( !pEnt || !pAct )
            return false;
    ...
    // update visibility
    bool vis_res = true;
    if ( m_last_relation == ALife::eRelationTypeEnemy ||
         m_last_relation == ALife::eRelationTypeWorstEnemy ) {
        ...
        if ( _object_ ) {
            ...
            if ( ea && !ea->g_Alive() )
                ...
                const CGameObject* pObj =
                    smart_cast< const CGameObject* >( _object_ );
                CActor* pAct = smart_cast< CActor* >(
                    Level().Objects.net_Find( m_pInvOwnerActorID ) );
                CHelmet* helm = smart_cast< CHelmet* >(
                    pAct->inventory().ItemFromSlot( HELMET_SLOT ) );
                if ( helm && helm->m_fShowNearestEnemiesDistance ) {
                    if ( pAct->Position().distance_to( pObj->Position() ) <
                         helm->m_fShowNearestEnemiesDistance )
                        vis_res = true;
                    else
                        vis_res =
                            Actor()->memory().visual().visible_now( pObj );
                } else
    ...
    if ( bAlive == false ) {
        ...
        if ( _object_ ) {
            ...
            CActor* pAct = smart_cast< CActor* >(
                Level().Objects.net_Find( m_pInvOwnerActorID ) );
            if ( /*pAct->Position().distance_to_sqr(pObj->Position()) < 100.0F
                    && */
                 abs( pObj->Position().y - pAct->Position().y ) < 3.0f )
                vis_res = true;
            else
                vis_res = false;
        } else
    ...
    if ( m_b_visible ) {
        ...
        if ( Level().MapManager().GetMapLocationsForObject(
                 m_objectID, find_locations_res ) ) {
            xr_vector< CMapLocation* >::iterator it =
                find_locations_res.begin();
            xr_vector< CMapLocation* >::iterator it_e =
                find_locations_res.end();
            for ( ; it != it_e; ++it ) {
                CMapLocation* ml = ( *it );
                if ( ml == this )
                    continue;
                ...
                m_b_minimap_visible =
                    m_b_minimap_visible && ( ml->MiniMapSpot() == NULL );
                m_b_levelmap_visible =
                    m_b_levelmap_visible && ( ml->LevelMapSpot() == NULL );
            }
        }
    }
