...
player_hud_motion* player_hud_motion_container::find_motion(
    ...
    for ( ; it != it_e; ++it ) {
        const shared_str& s =
            ( true ) ? ( *it ).m_alias_name : ( *it ).m_base_name;
        if ( s == name )
            return &( *it );
    }
    return NULL;
...
void player_hud_motion_container::load( IKinematicsAnimated* model,
    ...
    for ( ; _b != _e; ++_b ) {
        if ( strstr( _b->first.c_str(), "anm_" ) == _b->first.c_str() ) {
            ...
            VERIFY2(
                pm->m_animations.size(),
                make_string( "motion not found [%s]", pm->m_base_name.c_str() )
                    .c_str() );
        }
    }
}
...
void attachable_hud_item::set_bone_visible( const shared_str& bone_name,
                                            BOOL bVisibility,
                                            BOOL bSilent ) {
    u16 bone_id;
    BOOL bVisibleNow;
    ...
    if ( bone_id == BI_NONE ) {
        if ( bSilent )
            return;
        R_ASSERT2(
            0, make_string( "model [%s] has no bone [%s]",
...
void attachable_hud_item::update( bool bForce ) {
    ...
    if ( !!m_measures.m_prop_flags.test( hud_item_measures::e_16x9_mode_now ) !=
         is_16x9 )
        m_measures.load( m_sect_name, m_model );

    Fvector ypr = m_parent->m_adjust_mode ? m_parent->m_adjust_obj[ 1 ]
                                          : m_measures.m_item_attach[ 1 ];
    ypr.mul( PI / 180.f );
    m_attach_offset.setHPB( ypr.x, ypr.y, ypr.z );
    m_attach_offset.translate_over( m_parent->m_adjust_mode
                                        ? m_parent->m_adjust_obj[ 0 ]
                                        : m_measures.m_item_attach[ 0 ] );
    ...
    if ( m_attach_place_idx == SCOPE_ATTACH_IDX ) {
        ...
        hud_rotation.rotateX(
            m_parent->attached_item( 0 )->attach_mount_offset_rot().x );

        Fmatrix hud_rotation_y;
        hud_rotation_y.identity();
        hud_rotation_y.rotateY(
            m_parent->attached_item( 0 )->attach_mount_offset_rot().y );
        hud_rotation.mulA_43( hud_rotation_y );

        hud_rotation_y.identity();
        hud_rotation_y.rotateZ(
            m_parent->attached_item( 0 )->attach_mount_offset_rot().z );
        hud_rotation.mulA_43( hud_rotation_y );

        hud_rotation.translate_over(
            m_parent->attached_item( 0 )->attach_mount_offset_pos() );
        m_item_transform.mulB_43( hud_rotation );
    } else {
        m_parent->calc_transform( m_attach_place_idx, m_attach_offset,
                                  m_item_transform,
                                  m_measures.m_bLeadGunLeftHand );
...
void attachable_hud_item::setup_firedeps( firedeps& fd ) {
    ...
    if ( m_measures.m_prop_flags.test( hud_item_measures::e_fire_point ) ) {
        ...
        fire_mat.transform_tiny(
            fd.vLastFP, m_parent->m_adjust_mode
                            ? m_parent->m_adjust_firepoint_shell[ 0 ][ 0 ]
                            : m_measures.m_fire_point_offset );
        m_item_transform.transform_tiny( fd.vLastFP );

        fd.vLastFD.set( m_parent->m_adjust_mode
                            ? m_parent->m_adjust_firepoint_shell[ 1 ][ 0 ]
                            : m_measures.m_fire_direction );
        ...
        Fvector::generate_orthonormal_basis_normalized(
            fd.m_FireParticlesXForm.k, fd.m_FireParticlesXForm.j,
            fd.m_FireParticlesXForm.i );

        VERIFY( _valid( fd.m_FireParticlesXForm ) );

        // demonized: transforms for fire bone/point silencer, they should be
        // identical to above if they dont exist
        {
            Fmatrix& fire_mat =
                m_model->LL_GetTransform( m_measures.m_fire_bone_silencer );
            fire_mat.transform_tiny(
                fd.vLastFPSilencer,
                m_parent->m_adjust_mode
                    ? m_parent->m_adjust_firepoint_shell[ 0 ][ 0 ]
                    : m_measures.m_fire_point_silencer );
    ...
    if ( m_measures.m_prop_flags.test( hud_item_measures::e_fire_point2 ) ) {
        ...
        fire_mat.transform_tiny(
            fd.vLastFP2, m_parent->m_adjust_mode
                             ? m_parent->m_adjust_firepoint_shell[ 0 ][ 1 ]
                             : m_measures.m_fire_point2_offset );
    ...
    if ( m_measures.m_prop_flags.test( hud_item_measures::e_shell_point ) ) {
        ...
        fire_mat.transform_tiny(
            fd.vLastSP, m_parent->m_adjust_mode
                            ? m_parent->m_adjust_firepoint_shell[ 1 ][ 1 ]
                            : m_measures.m_shell_point_offset );
...
void attachable_hud_item::render( IDSGraphManager* DM ) {
    ...
    if ( m_parent_hud_item->has_object() &&
         m_parent_hud_item->object().GetAttachments()->size() ) {
...
void hud_item_measures::load( const shared_str& sect_name, IKinematics* K ) {
    ...
    if ( m_prop_flags.test( e_fire_point ) ) {
        ...
        bone_name = READ_IF_EXISTS( pSettings, r_string, sect_name, "fire_bone",
                                    "wpn_body" );
        ...
        if ( m_fire_bone == BI_NONE ) {
            Msg( "![%s] Invalid or not found fire_bone %s", sect_name.c_str(),
                 bone_name.c_str() );
        }

        m_fire_point_offset = pSettings->r_fvector3( sect_name, "fire_point" );
        m_fire_direction =
            READ_IF_EXISTS( pSettings, r_fvector3, sect_name, "fire_direction",
                            Fvector().set( 0.f, 0.f, 1.0f ) );
    } else
        m_fire_point_offset.set( 0, 0, 0 );

    m_prop_flags.set( e_fire_point2,
                      pSettings->line_exist( sect_name, "fire_bone2" ) );
    if ( m_prop_flags.test( e_fire_point2 ) ) {
        bone_name = READ_IF_EXISTS( pSettings, r_string, sect_name,
                                    "fire_bone2", "wpn_body" );
        ...
        if ( m_fire_bone2 == BI_NONE ) {
            Msg( "![%s] Invalid or not found fire_bone2 %s", sect_name.c_str(),
                 bone_name.c_str() );
        }

        m_fire_point2_offset =
            pSettings->r_fvector3( sect_name, "fire_point2" );
    } else
        m_fire_point2_offset.set( 0, 0, 0 );

    // demonized: fire point for silencer
    m_prop_flags.set(
        e_fire_point_silencer,
        pSettings->line_exist( sect_name, "fire_bone_silencer" ) );
    bone_name =
        READ_IF_EXISTS( pSettings, r_string, sect_name, "fire_bone_silencer",
                        READ_IF_EXISTS( pSettings, r_string, sect_name,
                                        "fire_bone", "wpn_body" ) );
    m_fire_bone_silencer = K->LL_BoneID( bone_name );
    m_fire_point_silencer.set(
        pSettings->line_exist( sect_name, "fire_point_silencer" )
            ? pSettings->r_fvector3( sect_name, "fire_point_silencer" )
            : m_fire_point_offset );

    m_prop_flags.set( e_shell_point,
    ...
    if ( m_prop_flags.test( e_shell_point ) ) {
        ...
        if ( m_shell_bone == BI_NONE ) {
            Msg( "![%s] Invalid or not found shell_bone %s", sect_name.c_str(),
                 bone_name.c_str() );
        }

        m_shell_point_offset =
            pSettings->r_fvector3( sect_name, "shell_point" );
    } else
    ...
    // DaimeneX: base_hud_offset_pos and base_hud_offset_rot. Editable variables
    // that allow weapon position adjustements that don't require aim tweaks
    ...
    strconcat( sizeof( val_name ), val_name, "aim_hud_offset_alt_pos",
               _prefix );
    m_hands_offset[ 0 ][ 3 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );
    strconcat( sizeof( val_name ), val_name, "aim_hud_offset_alt_rot",
               _prefix );
    m_hands_offset[ 1 ][ 3 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );

    // Safemode / lowered weapon position
    strconcat( sizeof( val_name ), val_name, "lowered_hud_offset_pos",
               _prefix );
    m_hands_offset[ 0 ][ 4 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );
    strconcat( sizeof( val_name ), val_name, "lowered_hud_offset_rot",
               _prefix );
    m_hands_offset[ 1 ][ 4 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );
    ...
    m_strafe_offset[ 0 ][ 0 ] = READ_IF_EXISTS(
        pSettings, r_fvector3, sect_name, val_name, vDefStrafeValue );
    strconcat( sizeof( val_name ), val_name, "strafe_hud_offset_rot", _prefix );
    m_strafe_offset[ 1 ][ 0 ] = READ_IF_EXISTS(
        pSettings, r_fvector3, sect_name, val_name, vDefStrafeValue );

    //--> Поворот в стрейфе
    strconcat( sizeof( val_name ), val_name, "strafe_aim_hud_offset_pos",
               _prefix );
    m_strafe_offset[ 0 ][ 1 ] = READ_IF_EXISTS(
        pSettings, r_fvector3, sect_name, val_name, vDefStrafeValue );
    strconcat( sizeof( val_name ), val_name, "strafe_aim_hud_offset_rot",
               _prefix );
    m_strafe_offset[ 1 ][ 1 ] = READ_IF_EXISTS(
        pSettings, r_fvector3, sect_name, val_name, vDefStrafeValue );

    //--> Параметры стрейфа
    bool bStrafeEnabled =
        READ_IF_EXISTS( pSettings, r_bool, sect_name, "strafe_enabled", false );
    bool bStrafeEnabled_aim = READ_IF_EXISTS( pSettings, r_bool, sect_name,
                                              "strafe_aim_enabled", false );
    float fFullStrafeTime = READ_IF_EXISTS( pSettings, r_float, sect_name,
                                            "strafe_transition_time", .5f );
    float fFullStrafeTime_aim = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "strafe_aim_transition_time", .5f );
    float fStrafeCamLFactor = READ_IF_EXISTS( pSettings, r_float, sect_name,
                                              "strafe_cam_limit_factor", 0.5f );
    float fStrafeCamLFactor_aim = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "strafe_cam_limit_aim_factor", 1.0f );
    float fStrafeMinAngle = READ_IF_EXISTS( pSettings, r_float, sect_name,
                                            "strafe_cam_min_angle", 0.0f );
    float fStrafeMinAngle_aim = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "strafe_cam_aim_min_angle", 7.0f );

    //--> (Data 1)
    m_strafe_offset[ 2 ][ 0 ].set( ( bStrafeEnabled ? 1.0f : 0.0f ),
                                   fFullStrafeTime, NULL ); // normal
    m_strafe_offset[ 2 ][ 1 ].set( ( bStrafeEnabled_aim ? 1.0f : 0.0f ),
                                   fFullStrafeTime_aim, NULL ); // aim-GL

    //--> (Data 2)
    m_strafe_offset[ 3 ][ 0 ].set( fStrafeCamLFactor, fStrafeMinAngle,
                                   NULL ); // normal
    m_strafe_offset[ 3 ][ 1 ].set( fStrafeCamLFactor_aim, fStrafeMinAngle_aim,
                                   NULL ); // aim-GL

    // Загрузка параметров смещения / инерции
    m_inertion_params.m_tendto_speed = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "inertion_tendto_speed", TENDTO_SPEED );
    m_inertion_params.m_tendto_speed_aim =
        READ_IF_EXISTS( pSettings, r_float, sect_name,
                        "inertion_tendto_aim_speed", TENDTO_SPEED_AIM );
    m_inertion_params.m_tendto_ret_speed =
        READ_IF_EXISTS( pSettings, r_float, sect_name,
                        "inertion_tendto_ret_speed", TENDTO_SPEED_RET );
    m_inertion_params.m_tendto_ret_speed_aim =
        READ_IF_EXISTS( pSettings, r_float, sect_name,
                        "inertion_tendto_ret_aim_speed", TENDTO_SPEED_RET_AIM );

    m_inertion_params.m_min_angle = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "inertion_min_angle", INERT_MIN_ANGLE );
    m_inertion_params.m_min_angle_aim =
        READ_IF_EXISTS( pSettings, r_float, sect_name, "inertion_min_angle_aim",
                        INERT_MIN_ANGLE_AIM );

    m_inertion_params.m_offset_LRUD = READ_IF_EXISTS(
        pSettings, r_fvector4, sect_name, "inertion_offset_LRUD",
        Fvector4().set( ORIGIN_OFFSET ) );
    m_inertion_params.m_offset_LRUD_aim = READ_IF_EXISTS(
        pSettings, r_fvector4, sect_name, "inertion_offset_LRUD_aim",
        Fvector4().set( ORIGIN_OFFSET_AIM ) );

    // Загрузка параметров смещения при стрельбе
    m_shooting_params.bShootShake = READ_IF_EXISTS(
        pSettings, r_bool, sect_name, "shooting_hud_effect", false );
    m_shooting_params.m_shot_max_offset_LRUD =
        READ_IF_EXISTS( pSettings, r_fvector4, sect_name, "shooting_max_LRUD",
                        Fvector4().set( 0, 0, 0, 0 ) );
    m_shooting_params.m_shot_max_offset_LRUD_aim =
        READ_IF_EXISTS( pSettings, r_fvector4, sect_name,
                        "shooting_max_LRUD_aim", Fvector4().set( 0, 0, 0, 0 ) );
    m_shooting_params.m_shot_offset_BACKW =
        READ_IF_EXISTS( pSettings, r_fvector2, sect_name,
                        "shooting_backward_offset", Fvector2().set( 0, 0 ) );
    m_shooting_params.m_ret_speed = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "shooting_ret_speed", 1.0f );
    m_shooting_params.m_ret_speed_aim = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "shooting_ret_aim_speed", 1.0f );
    m_shooting_params.m_min_LRUD_power = READ_IF_EXISTS(
        pSettings, r_float, sect_name, "shooting_min_LRUD_power", 0.0f );

    //--#SM+# End--

    strconcat( sizeof( val_name ), val_name, "attach_base_hud_offset_pos",
               _prefix );
    m_hands_offset[ 0 ][ 6 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );
    strconcat( sizeof( val_name ), val_name, "attach_base_hud_offset_rot",
               _prefix );
    m_hands_offset[ 1 ][ 6 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );

    strconcat( sizeof( val_name ), val_name, "attach_mount_hud_offset_pos",
               _prefix );
    m_hands_offset[ 0 ][ 7 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );
    strconcat( sizeof( val_name ), val_name, "attach_mount_hud_offset_rot",
               _prefix );
    m_hands_offset[ 1 ][ 7 ] =
        READ_IF_EXISTS( pSettings, r_fvector3, sect_name, val_name, vZero );

    m_fFreelookZOffset = READ_IF_EXISTS( pSettings, r_float, sect_name,
                                         "freelook_z_offset_mul", 0.f );
    m_bLeadGunLeftHand =
        READ_IF_EXISTS( pSettings, r_bool, sect_name, "lh_lead_gun", false );

    m_attach_scale =
        READ_IF_EXISTS( pSettings, r_float, sect_name, "attach_scale", 1 );
}
...
void attachable_hud_item::load( const shared_str& sect_name ) {
    ...
    IKinematicsAnimated* visual =
        ::Render->model_Create( visual_name )->dcast_PKinematicsAnimated();
    ::Render->hud_loading = false;
    R_ASSERT2( visual, make_string( "could not create model %s, section %s",
                                    visual_name, sect_name.c_str() ) );
...
player_hud_motion* attachable_hud_item::find_motion(
    const shared_str& anm_name ) {
    R_ASSERT( strstr( anm_name.c_str(), "anm_" ) == anm_name.c_str() );
    string256 anim_name_r;
    bool is_16x9 = UI().is_widescreen();
    xr_sprintf( anim_name_r, "%s%s", anm_name.c_str(),
    ...
    R_ASSERT2( anm, make_string( "model [%s] has no motion alias defined [%s]",
                                 m_sect_name.c_str(), anm_name.c_str() )
                        .c_str() );
    VERIFY2(
        anm->m_animations.size(),
...
u32 attachable_hud_item::anim_play( const shared_str& anm_name_b,
    ...
    u32 ret = 0;
    if ( m_attach_place_idx != SCOPE_ATTACH_IDX ) {
        ret = g_player_hud->anim_play( m_attach_place_idx, M.mid, bMixIn, md,
                                       speed );
    }
    ...
    if ( m_model->dcast_PKinematicsAnimated() ) {
        ...
        R_ASSERT3( M2.valid(),
                   make_string( "model has no motion [idle], section %s",
                                m_sect_name.c_str() )
                       .c_str(),
                   pSettings->r_string( m_sect_name, "item_visual" ) );
        ...
        for ( u16 pid = 0; pid < pc; ++pid )
            CBlend* B = ka->PlayCycle( pid, M2, ( !!bMixIn && bMixIn2 ), 0, 0,
                                       0, speed );
    ...
    if ( m_parent_hud_item->has_object() ) {
        ...
        // CObject*		parent_object =
        // static_cast_checked<CObject*>(&m_parent_hud_item->object());

        if ( IsGameTypeSingle() &&
             parent_object.H_Parent() == Level().CurrentControlEntity() ) {
            CActor* current_actor = static_cast_checked< CActor* >(
...
player_hud::player_hud() {
    ...
    m_bone_callback_params.insert(
        mk_pair( r_finger0, xr_new< BoneCallbackParams >() ) );
    m_bone_callback_params.insert(
        mk_pair( r_finger01, xr_new< BoneCallbackParams >() ) );
    m_bone_callback_params.insert(
        mk_pair( r_finger02, xr_new< BoneCallbackParams >() ) );
    // m_bone_callback_params.insert(mk_pair(bip01_r_finger1,
    // xr_new<BoneCallbackParams>()));
    // m_bone_callback_params.insert(mk_pair(bip01_r_finger11,
    // xr_new<BoneCallbackParams>()));
    // m_bone_callback_params.insert(mk_pair(bip01_r_finger12,
    // xr_new<BoneCallbackParams>()));
    ...
    for ( int i = 0; i < move_anms_end; i++ ) {
        ...
        char temp[ 20 ];
        string512 tmp;
        strconcat( sizeof( temp ), temp, "movement_layer_",
                   std::to_string( i ).c_str() );

        if ( pSettings->line_exist( "hud_movement_layers", temp ) ) {
            LPCSTR layer_def =
                pSettings->r_string( "hud_movement_layers", temp );
            R_ASSERT2(
                _GetItemCount( layer_def ) > 0,
                make_string( "Wrong definition for [hud_movement_layers] %s",
                             temp ) );
...
void player_hud::FingerCallback( CBoneInstance* B ) {
    BoneCallbackParams* params =
        static_cast< BoneCallbackParams* >( B->callback_param() );
...
void player_hud::load( const shared_str& player_hud_sect, bool force ) {
    ...
    const shared_str& model_name =
        pSettings->r_string( player_hud_sect, "visual" );
    ::Render->hud_loading = true;
    m_model = smart_cast< IKinematicsAnimated* >(
        ::Render->model_Create( model_name.c_str() ) );
    m_model_2 = smart_cast< IKinematicsAnimated* >( ::Render->model_Create(
        pSettings->line_exist( player_hud_sect, "visual_2" )
            ? pSettings->r_string( player_hud_sect, "visual_2" )
            : model_name.c_str() ) );
    ...
    bool b_reload = ( m_attached_items[ 0 ] != nullptr ||
                      m_attached_items[ 1 ] != nullptr );
    ...
    u16 bone_r_finger01 =
        m_model->dcast_PKinematics()->LL_BoneID( "r_finger01" );
    u16 bone_r_finger02 =
        m_model->dcast_PKinematics()->LL_BoneID( "r_finger02" );

    // u16 bone_r_triggerfinger0 =
    // m_model->dcast_PKinematics()->LL_BoneID("bip01_r_finger1"); u16
    // bone_r_triggerfinger01 =
    // m_model->dcast_PKinematics()->LL_BoneID("bip01_r_finger11"); u16
    // bone_r_triggerfinger02 =
    // m_model->dcast_PKinematics()->LL_BoneID("bip01_r_finger12");

    m_model->dcast_PKinematics()
        ->LL_GetBoneInstance( bone_r_finger0 )
        .set_callback( bctCustom, FingerCallback,
                       m_bone_callback_params[ r_finger0 ] );
    m_model->dcast_PKinematics()
        ->LL_GetBoneInstance( bone_r_finger01 )
        .set_callback( bctCustom, FingerCallback,
                       m_bone_callback_params[ r_finger01 ] );
    m_model->dcast_PKinematics()
        ->LL_GetBoneInstance( bone_r_finger02 )
        .set_callback( bctCustom, FingerCallback,
                       m_bone_callback_params[ r_finger02 ] );

    // m_model->dcast_PKinematics()->LL_GetBoneInstance(bone_r_triggerfinger0).set_callback(bctCustom,
    // FingerCallback, m_bone_callback_params[bip01_r_finger1]);
    // m_model->dcast_PKinematics()->LL_GetBoneInstance(bone_r_triggerfinger01).set_callback(bctCustom,
    // FingerCallback, m_bone_callback_params[bip01_r_finger11]);
    // m_model->dcast_PKinematics()->LL_GetBoneInstance(bone_r_triggerfinger02).set_callback(bctCustom,
    // FingerCallback, m_bone_callback_params[bip01_r_finger12]);
    ...
    for ( ; _b != _e; ++_b ) {
        if ( strstr( _b->first.c_str(), "ancor_" ) == _b->first.c_str() ) {
            ...
            m_ancors.push_back(
                m_model->dcast_PKinematics()->LL_BoneID( _bone ) );
        }
    }
...
void player_hud::render_hud( IDSGraphManager* DM ) {
    bool b_r0 = ( ( m_attached_items[ 0 ] &&
                  ...
                  script_anim_part == 0 || script_anim_part == 2 );
    bool b_r1 = ( ( m_attached_items[ 1 ] &&
                    m_attached_items[ 1 ]->need_renderable() ) ||
                  script_anim_part == 1 || script_anim_part == 2 );
    ...
    if ( script_anim_item_model ) {
        DM->add_Dynamic( script_anim_item_model->dcast_RenderVisual(),
                         &m_item_pos );
    }
...
u32 player_hud::motion_length_script( LPCSTR section,
                                      LPCSTR anm_name,
                                      float speed ) {
    ...
    if ( !phm ) {
        Msg( "!script motion [%s] not found in section [%s]", anm_name,
             section );
        return 0;
    }
...
u32 player_hud::motion_length( const shared_str& anim_name,
                               const shared_str& hud_name,
                               const CMotionDef*& md ) {
    ...
    R_ASSERT2( pm,
               make_string( "hudItem model [%s] has no motion with alias [%s]",
                            hud_name.c_str(), anim_name.c_str() )
                   .c_str() );
    return motion_length( pm->m_animations[ 0 ].mid, md, 1.f );
}

u32 player_hud::motion_length( const MotionID& M,
                               const CMotionDef*& md,
                               float speed ) {
    ...
    if ( md != nullptr && md->flags & esmStopAtEnd ) {
        ...
        return iFloor( 0.5f + 1000.f * motion->GetLength() /
                                  ( md->Dequantize( md->speed ) * speed ) );
    }
    return 0;
}
...
void player_hud::update( const Fmatrix& cam_trans ) {
    ...
    if ( control_factor > 0 ) {
        ...
        float new_pitch = old_pitch > 0.f
                              ? old_pitch * ( 1.f - psHUD_FOV )
                              : old_pitch * ( 1.f - ( psHUD_FOV / 2.f ) );
        ...
        Fvector::generate_orthonormal_basis_normalized( trans.k, trans.j,
                                                        trans.i );
    }
    ...
    // freelook pitch compensation (avoid seeing missing arm parts or weapon
    // parts that are not meant to be visible)
    if ( ( m_attached_items[ 0 ] || m_attached_items[ 1 ] ) &&
         control_factor > 0 && sub_z < 0 ) {
        float z1, z2, z_factor;
        z_factor = ( sub_z * control_factor );
        z1 = ( g_freelook_z_offset
                   ? g_freelook_z_offset
                   : m_attached_items[ m_attached_items[ 0 ] ? 0 : 1 ]
                         ->m_measures.m_fFreelookZOffset );
        z2 = ( g_freelook_z_offset
                   ? g_freelook_z_offset
                   : m_attached_items[ m_attached_items[ 1 ] ? 1 : 0 ]
                         ->m_measures.m_fFreelookZOffset );
    ...
    if ( m_attached_items[ 1 ] ) {
        m_attached_items[ 1 ]->m_parent_hud_item->UpdateHudAdditional(
            trans_2 );
        if ( wep && wep->IsZoomed() )
    ...
    if ( script_anim_part == 2 ||
         ( script_anim_part && !m_attached_items[ 0 ] &&
           !m_attached_items[ 1 ] ) ) {
        ...
        hand_pos.lerp( script_anim_part == 0 ? m1pos : m2pos,
                       script_anim_offset[ 0 ], script_anim_offset_factor );
        hand_rot.lerp( script_anim_part == 0 ? m1rot : m2rot,
                       script_anim_offset[ 1 ], script_anim_offset_factor );
    ...
    for ( script_layer* anm : m_script_layers ) {
        ...
        if ( anm->blend_amount > 0.f ) {
            if ( anm->anm->bLoop || anm->anm->anim_param().t_current <
                                        anm->anm->anim_param().max_t )
    ...
    need_blend[ 0 ] =
        ( ( script_anim_part == 0 || script_anim_part == 2 ) ||
          ( m_attached_items[ 0 ] &&
            m_attached_items[ 0 ]->m_parent_hud_item->NeedBlendAnm() ) );
    need_blend[ 1 ] =
        ( ( script_anim_part == 1 || script_anim_part == 2 ) ||
          ( m_attached_items[ 1 ] &&
            m_attached_items[ 1 ]->m_parent_hud_item->NeedBlendAnm() ) );

    for ( movement_layer* anm : m_movement_layers ) {
        if ( !anm || !anm->anm ||
             ( !anm->active && anm->blend_amount[ 0 ] == 0.f &&
               anm->blend_amount[ 1 ] == 0.f ) )
            continue;
    ...
    // single hand offset smoothing + syncing back to other hand animation on
    // end
    if ( script_anim_part != u8( -1 ) ) {
        script_anim_offset_factor += Device.fTimeDelta * 2.5f;

        if ( m_bStopAtEndAnimIsRunning &&
             Device.dwTimeGlobal >= script_anim_end )
            StopScriptAnim();
    } else
...
void player_hud::updateMovementLayerState() {
    ...
    if ( need_blend ) {
        ...
        if ( m_attached_items[ 0 ] &&
             m_attached_items[ 0 ]->m_parent_hud_item->has_object() &&
             m_attached_items[ 0 ]->m_parent_hud_item->object().cast_weapon() )
            wep = m_attached_items[ 0 ]
                      ->m_parent_hud_item->object()
                      .cast_weapon();
        ...
        if ( pActor->AnyMove() ) {
            ...
            if ( wep && wep->IsZoomed() ) {
                state.bCrouch ? m_movement_layers[ eAimCrouch ]->Play()
                              : m_movement_layers[ eAimWalk ]->Play();
...
void player_hud::PlayBlendAnm( LPCSTR name,
                               u8 part,
                               float speed,
                               float power,
                               bool bLooped,
    ...
    script_layer* anm =
        xr_new< script_layer >( name, part, speed, power, bLooped, pivot_bone );
...
float player_hud::SetBlendAnmTime( LPCSTR name, float time ) {
    for ( script_layer* anm : m_script_layers ) {
        if ( !xr_strcmp( *anm->m_name, name ) ) {
            ...
            float speed = ( anm->anm->anim_param().max_t -
                            anm->anm->anim_param().t_current ) /
                          time;
            anm->anm->Speed() = speed;
            return speed;
        }
...
void play_blend( player_hud* hud,
                 u8 pid,
                 const MotionID& M,
                 BOOL bMixIn,
                 float speed,
                 bool script_anim = false ) {
    switch ( pid ) {
        case 0: {
            if ( !script_anim && hud->script_anim_part == 2 )
                return;
        ...
        case 1:
            if ( !script_anim && hud->script_anim_part == 1 )
                return;
            ...
            break;
        case 2:
            if ( !script_anim && hud->script_anim_part == 0 )
                return;
...
void player_hud::StopScriptAnim() {
    ...
    if ( part > 2 ) {
        if ( print_bone_warnings ) {
            Msg( "![player_hud::StopScriptAnim()] invalid script_anim_part %d, "
                 "must be < 3",
                 part );
...
u32 player_hud::anim_play( u16 part,
                           const MotionID& M,
                           BOOL bMixIn,
                           const CMotionDef*& md,
                           float speed,
...
void player_hud::update_script_item() {
    ...
    calc_transform( m_attach_idx, m_attach_offset, m_item_pos,
                    script_anim_lead_gun );

    if ( script_anim_item_model ) {
        script_anim_item_model->UpdateTracks();
        script_anim_item_model->dcast_PKinematics()
...
u32 player_hud::script_anim_play( u8 hand,
                                  LPCSTR section,
                                  LPCSTR anm_name,
                                  bool bMixIn,
                                  float speed ) {
    ...
    Fvector offs =
        READ_IF_EXISTS( pSettings, r_fvector3, section, pos.c_str(), def );
    Fvector rrot =
        READ_IF_EXISTS( pSettings, r_fvector3, section, rot.c_str(), def );

    if ( pSettings->line_exist( section, "item_visual" ) ) {
        ::Render->hud_loading = true;
        script_anim_item_model =
            ::Render
                ->model_Create( pSettings->r_string( section, "item_visual" ) )
                ->dcast_PKinematicsAnimated();
        ::Render->hud_loading = false;
        item_pos[ 0 ] = READ_IF_EXISTS( pSettings, r_fvector3, section,
                                        "item_position", def );
        item_pos[ 1 ] = READ_IF_EXISTS( pSettings, r_fvector3, section,
                                        "item_orientation", def );
        script_anim_item_attached =
            READ_IF_EXISTS( pSettings, r_bool, section, "item_attached", true );
        m_attach_idx =
            READ_IF_EXISTS( pSettings, r_u8, section, "attach_place_idx", 0 );
        script_anim_lead_gun =
            READ_IF_EXISTS( pSettings, r_bool, section, "lh_lead_gun", false );
    ...
    const motion_descr& M =
        phm->m_animations[ Random.randI( phm->m_animations.size() ) ];
    ...
    if ( script_anim_item_model ) {
        ...
        R_ASSERT3( M2.valid(), "model %s has no motion [idle] ",
                   pSettings->r_string( m_sect_name, "item_visual" ) );

        u16 root_id =
            script_anim_item_model->dcast_PKinematics()->LL_GetBoneRoot();
        CBoneInstance& root_binst =
            script_anim_item_model->dcast_PKinematics()->LL_GetBoneInstance(
                root_id );
        root_binst.set_callback_overwrite( TRUE );
        root_binst.mTransform.identity();
        ...
        for ( u16 pid = 0; pid < pc; ++pid )
            CBlend* B = script_anim_item_model->PlayCycle( pid, M2, bMixIn, 0,
                                                           0, 0, speed );
...
bool player_hud::allow_activation( CHudItem* item ) {
    if ( script_anim_part != u8( -1 ) )
        ...
        return m_attached_items[ 1 ]->m_parent_hud_item->CheckCompatibility(
            item );
    else
        return true;
}
...
void player_hud::attach_item( CHudItem* item ) {
    ...
    if ( m_attached_items[ item_idx ] != pi ) {
        ...
        if ( item_idx == 0 && m_attached_items[ 1 ] )
            m_attached_items[ 1 ]->m_parent_hud_item->CheckCompatibility(
                item );
...
// sync anim of other part to selected part (1 = sync to left hand anim; 2 =
// sync to right hand anim)
void player_hud::re_sync_anim( u8 part ) {
    u32 bc = part == 1 ? m_model_2->LL_PartBlendsCount( part )
                       : m_model->LL_PartBlendsCount( part );
    for ( u32 bidx = 0; bidx < bc; ++bidx ) {
        CBlend* BR = part == 1 ? m_model_2->LL_PartBlend( part, bidx )
                               : m_model->LL_PartBlend( part, bidx );
        if ( !BR )
            continue;
        ...
        for ( u16 pid = 0; pid < pc; ++pid ) {
            if ( pid == 0 ) {
                ...
                CBlend* B = part == 1 ? m_model->PlayCycle( pid, M, TRUE )
                                      : m_model_2->PlayCycle( pid, M, TRUE );
                B->timeCurrent = BR->timeCurrent;
                B->speed = BR->speed;
            }
        }
...
void player_hud::detach_item_idx( u16 idx ) {
    ...
    if ( idx == 1 ) {
        ...
        if ( m_attached_items[ 1 ] ) {
            // fix for a rare case where the right hand stays visible on screen
            // after detaching the right hand's attached item
            player_hud_motion* pm =
                m_attached_items[ 1 ]->m_hand_motions->find_motion(
                    "anm_idle" );
...
bool player_hud::allow_script_anim() {
    if ( m_attached_items[ 0 ] &&
         ( m_attached_items[ 0 ]->m_parent_hud_item->IsPending() ||
           m_attached_items[ 0 ]->m_parent_hud_item->GetState() ==
               CHudItem::EHudStates::eBore ) )
        return false;
    else if ( m_attached_items[ 1 ] &&
              ( m_attached_items[ 1 ]->m_parent_hud_item->IsPending() ||
                m_attached_items[ 1 ]->m_parent_hud_item->GetState() ==
                    CHudItem::EHudStates::eBore ) )
        return false;
...
void player_hud::calc_transform( u16 attach_slot_idx,
                                 const Fmatrix& offset,
                                 Fmatrix& result,
                                 bool leadGun ) {
    IKinematics* kin = ( attach_slot_idx == 0 )
                           ? m_model->dcast_PKinematics()
                           : m_model_2->dcast_PKinematics();
    Fmatrix ancor_m =
        kin->LL_GetTransform( m_ancors[ ( leadGun ? 0 : attach_slot_idx ) ] );
    result.mul( ( attach_slot_idx == 0 ) ? m_transform : m_transform_2,
                ancor_m );
...
void player_hud::OnMovementChanged( ACTOR_DEFS::EMoveCommand cmd ) {
    if ( cmd == 0 ) {
        if ( m_attached_items[ 0 ] ) {
            if ( m_attached_items[ 0 ]->m_parent_hud_item->GetState() ==
                 CHUDState::eIdle )
                m_attached_items[ 0 ]->m_parent_hud_item->PlayAnimIdle();
        }
        if ( m_attached_items[ 1 ] ) {
            if ( m_attached_items[ 1 ]->m_parent_hud_item->GetState() ==
                 CHUDState::eIdle )
                m_attached_items[ 1 ]->m_parent_hud_item->PlayAnimIdle();
        }
        if ( m_attached_items[ SCOPE_ATTACH_IDX ] ) {
            if ( m_attached_items[ SCOPE_ATTACH_IDX ]
                     ->m_parent_hud_item->GetState() == CHUDState::eIdle )
                m_attached_items[ SCOPE_ATTACH_IDX ]
                    ->m_parent_hud_item->PlayAnimIdle();
        }
    } else {
        if ( m_attached_items[ 0 ] )
        ...
        if ( m_attached_items[ SCOPE_ATTACH_IDX ] )
            m_attached_items[ SCOPE_ATTACH_IDX ]
                ->m_parent_hud_item->OnMovementChanged( cmd );
    }

    ::luabind::functor< void > func;
    if ( ai().script_engine().functor( "_g.player_hud__OnMovementChanged",
                                       func ) ) {
        func( cmd );
    }
...
bool nearwall_callback( int target,
                        float ofs,
                        const Fvector& dir,
                        Fmatrix& mat ) {
    ::luabind::functor< void > on_nearwall;
    if ( !ai().script_engine().functor( "_G.CActorHudOnNearWall",
                                        on_nearwall ) ) {
        return false;
    }
...
void update_nearwall( int target,
                      const attachable_hud_item* item,
                      Fmatrix& nearwall ) {
...
void apply_nearwall( Fmatrix& transform,
                     Fmatrix& nearwall,
                     attachable_hud_item* item ) {
...
void player_hud::OnFrame() {
    ...
    if ( g_nearwall == NW_POS ) {
        ...
        if ( m_attached_items[ 1 ] ) {
            ...
            if ( m_attached_items[ 0 ] ) {
                // And it is a weapon...
                CWeapon* pWeapon = smart_cast< CWeapon* >(
                    m_attached_items[ 0 ]->m_parent_hud_item );
                if ( pWeapon ) {
                    // Interpolate between main and off -hand transforms based
                    // on aim factor
        ...
        if ( m_attached_items[ SCOPE_ATTACH_IDX ] ) {
            CHudItem* parent =
                m_attached_items[ SCOPE_ATTACH_IDX ]->m_parent_hud_item;
            m_attached_items[ SCOPE_ATTACH_IDX ]->m_item_transform.mulB_43(
                nearwall_0 );
        }
    }
}
...
void player_hud::net_Relcase( CObject* obj ) {
    ...
    if ( m_attached_items[ SCOPE_ATTACH_IDX ] )
        m_attached_items[ SCOPE_ATTACH_IDX ]->m_parent_hud_item->net_Relcase(
            obj );
}
