...
CSpaceRestrictionAbstract::accessible_neighbour_border( T& restriction,
    ...
    VERIFY2( !m_accessible_neighbour_border.empty(),
             make_string( "space restrictor %s has no accessible neighbours "
                          "(border size[%d])",
                          *name(), border().size() ) );
...
template < typename T >
IC bool CSpaceRestrictionAbstract::accessible_neighbours(
    T& restriction,
    u32 level_vertex_id,
    bool out_restriction ) {
...
template < typename T >
IC void CSpaceRestrictionAbstract::prepare_accessible_neighbour_border(
    T& restriction,
    bool out_restriction ) {
