...
void target_provider::setup( animation_planner* object,
...
void target_idle::execute() {
    ...
    m_storage->set_property(
        StalkerDecisionSpace::eWorldPropertyLoopholeTooMuchTimeFiring, false );
}
...
void target_fire::initialize() {
    if ( this->m_object->m_object->agent_manager().enemy().enemies().size() >
         1 )
...
void target_fire::execute() {
    ...
    if ( this->m_object->m_object->ready_to_kill() ) {
        CWeapon* weapon = smart_cast< CWeapon* >(
            this->m_object->m_object->m_best_item_to_kill );
        if ( weapon ) {
    ...
    m_storage->set_property(
        StalkerDecisionSpace::eWorldPropertyLoopholeTooMuchTimeFiring, true );
}
...
void target_fire_no_lookout::initialize() {
    m_storage->set_property( StalkerDecisionSpace::eWorldPropertyLookedOut,
                             false );
    inherited::initialize();
}
