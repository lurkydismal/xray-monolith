...
#define TEMPLATE_SPEZIALIZATION                                              \
    template < typename _key_type, typename _data_type, typename _predicate, \
               bool use_time_limit, typename _cycle_type,                    \
               bool use_first_update >

#define CSSafeMapIterator                                                \
    CSafeMapIterator< _key_type, _data_type, _predicate, use_time_limit, \
                      _cycle_type, use_first_update >
...
TEMPLATE_SPEZIALIZATION
IC void CSSafeMapIterator::add( const _key_type& id,
                                _data_type* value,
                                bool no_assert ) {
...
#ifdef DEBUG
        THROW2( no_assert,
                "Specified object has been already found in the registry!" );
#endif
        return;
    }
...
IC bool CSSafeMapIterator::time_over() {
    return ( use_time_limit && !m_first_update &&
             ( m_timer.GetElapsed_sec() >= m_max_process_time ) );
}
...
TEMPLATE_SPEZIALIZATION
IC const typename CSSafeMapIterator::_REGISTRY& CSSafeMapIterator::objects()
    const {
    return ( m_objects );
}
...
IC u32 CSSafeMapIterator::update( const _update_predicate& predicate,
                                  bool const iterate_as_first_time_next_time ) {
    if ( empty() )
        return ( 0 );
    ...
    u32 i = 0;
    for ( ; ( I != m_objects.end() ) && !time_over() &&
            predicate( I, m_cycle_count, true );
          ++i ) {
        update_next();
