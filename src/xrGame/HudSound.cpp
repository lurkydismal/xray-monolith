...
void InitHudSoundSettings() {
    ...
    psHUDStepSoundVolume =
        pSettings->r_float( "hud_sound", "hud_step_sound_vol_k" );
}
...
void HUD_SOUND_ITEM::PlaySound( HUD_SOUND_ITEM& hud_snd,
    ...
    // Alundaio: Sanity, don't allow PlaySound of index greater then the size,
    // just play last index
    if ( index == u8( -1 ) ) {
...
HUD_SOUND_ITEM* HUD_SOUND_COLLECTION::FindSoundItem( LPCSTR alias,
                                                     bool b_assert ) {
    xr_vector< HUD_SOUND_ITEM >::iterator it =
        std::find( m_sound_items.begin(), m_sound_items.end(), alias );
...
void HUD_SOUND_COLLECTION::PlaySound( LPCSTR alias,
    ...
    if ( snd_item )
        HUD_SOUND_ITEM::PlaySound( *snd_item, position, parent, hud_mode,
                                   looped, index, volume_mult );
}
...
// Alundaio:
/*
It's usage is to play a group of sounds HUD_SOUND_ITEMs as if they were a single
layered entity. This is a achieved by wrapping the class around
HUD_SOUND_COLLECTION and tagging them with the same alias. This way, when one
for example sndShot is played, it will play all the sound items with the same
alias.
*/
//----------------------------------------------------------
...
void HUD_SOUND_COLLECTION_LAYERED::SetPosition( LPCSTR alias,
                                                const Fvector& pos ) {
    xr_vector< HUD_SOUND_COLLECTION >::iterator it = m_sound_items.begin();
    xr_vector< HUD_SOUND_COLLECTION >::iterator it_e = m_sound_items.end();
...
void HUD_SOUND_COLLECTION_LAYERED::UpdateAllSoundsPositions(
    const Fvector& P ) {
...
void HUD_SOUND_COLLECTION_LAYERED::PlaySound( LPCSTR alias,
                                              const Fvector& position,
                                              const CObject* parent,
                                              bool hud_mode,
                                              bool looped,
    ...
    if ( ai().script_engine().functor( "_G.COnBeforePlayHudSound", funct ) ) {
        const CGameObject* parent_game_object =
            smart_cast< const CGameObject* >( parent );
        if ( parent_game_object ) {
            ...
            if ( output && output.type() == LUA_TTABLE ) {
                ...
                float volume_mult_ex =
                    volume_mult_ex_obj.type() != LUA_TNUMBER
                        ? 1
                        : ::luabind::object_cast< float >( volume_mult_ex_obj );
                volume_mult = volume_mult * volume_mult_ex;

                LPCSTR section =
                    ::luabind::object_cast< LPCSTR >( output[ "section" ] );
                LPCSTR line =
                    ::luabind::object_cast< LPCSTR >( output[ "line" ] );
                if ( !section ) {
                    if ( line ) {
                        Msg( "!_G.COnBeforePlayHudSound callback, "
                             "HUD_SOUND_COLLECTION_LAYERED::PlaySound, failed "
                             "to override sound item %s, no section specified",
                             alias );
                    }
                } else if ( !line ) {
                    if ( section ) {
                        Msg( "!_G.COnBeforePlayHudSound callback, "
                             "HUD_SOUND_COLLECTION_LAYERED::PlaySound, failed "
                             "to override sound item %s, no line specified",
                             alias );
                    }
                } else {
                    ...
                    if ( !FindSoundItem( new_alias.c_str(), false ) ) {
                        ...
                        if ( old_sound ) {
                            ...
                            LoadSound( section, line, alias_to_play,
                                       old_sound->m_b_exclusive,
                                       old_sound->m_type );

                            // Crash if override sound was defined but wasn't
                            // loaded correctly
                            R_ASSERT2(
                                FindSoundItem( alias_to_play, false ),
                                make_string(
                                    "!_G.COnBeforePlayHudSound callback, "
                                    "HUD_SOUND_COLLECTION_LAYERED::PlaySound, "
                                    "failed to override sound item %s with %s, "
                                    "failed to load section %s line %s",
                                    alias, alias_to_play, section, line ) );
                        } else {
                            Msg( "!_G.COnBeforePlayHudSound callback, "
                                 "HUD_SOUND_COLLECTION_LAYERED::PlaySound, "
                                 "failed to override sound item %s with %s, "
                                 "sound item by original alias %s not found",
                                 alias, new_alias.c_str(), alias );
                        }
                    } else {
                        alias_to_play = new_alias.c_str();
                        // Msg("HUD_SOUND_COLLECTION_LAYERED::PlaySound,
                        // overriding sound item %s with %s, already exists",
                        // alias, alias_to_play);
                    }
                }
            }
    ...
    for ( ; it != it_e; ++it ) {
        if ( it->m_alias == alias_to_play && volume_mult > EPS_S )
            it->PlaySound( alias_to_play, position, parent, hud_mode, looped,
                           index, volume_mult );
    }
}

HUD_SOUND_ITEM* HUD_SOUND_COLLECTION_LAYERED::FindSoundItem( LPCSTR alias,
                                                             bool b_assert ) {
    xr_vector< HUD_SOUND_COLLECTION >::iterator it = m_sound_items.begin();
    xr_vector< HUD_SOUND_COLLECTION >::iterator it_e = m_sound_items.end();
...
void HUD_SOUND_COLLECTION_LAYERED::LoadSound( LPCSTR section,
                                              LPCSTR line,
                                              LPCSTR alias,
                                              bool exclusive,
                                              int type ) {
...
void HUD_SOUND_COLLECTION_LAYERED::LoadSound( CInifile const* ini,
                                              LPCSTR section,
                                              LPCSTR line,
                                              LPCSTR alias,
                                              bool exclusive,
