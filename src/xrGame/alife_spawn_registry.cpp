...
void CALifeSpawnRegistry::load( IReader& file_stream, LPCSTR game_name ) {
    ...
    R_ASSERT2( file_stream.find_chunk( SPAWN_CHUNK_DATA ),
               "Cannot find chunk SPAWN_CHUNK_DATA!" );
    ...
    bool file_exists =
        !!FS.exist( file_name, "$game_spawn$", *m_spawn_name, ".spawn" );
...
void CALifeSpawnRegistry::load( LPCSTR spawn_name ) {
    ...
    string_path file_name;
    R_ASSERT3( FS.exist( file_name, "$game_spawn$", *m_spawn_name, ".spawn" ),
               "Can't find spawn file:", *m_spawn_name );
...
void CALifeSpawnRegistry::load( IReader& file_stream, xrGUID* save_guid ) {
    ...
    chunk->close();
    R_ASSERT2(
        !save_guid || ( *save_guid == header().guid() ) ||
...
void CALifeSpawnRegistry::load_updates( IReader& stream ) {
    ...
    for ( IReader* chunk = stream.open_chunk_iterator( vertex_id ); chunk;
        ...
        const SPAWN_GRAPH::CVertex* vertex =
            m_spawns.vertex( ALife::_SPAWN_ID( vertex_id ) );
        VERIFY( vertex );
...
void CALifeSpawnRegistry::build_root_spawns() {
    ...
    xr_vector< ALife::_SPAWN_ID >::iterator I =
        std::set_difference( m_temp0.begin(), m_temp0.end(), m_temp1.begin(),
                             m_temp1.end(), m_spawn_roots.begin() );
...
void CALifeSpawnRegistry::build_story_spawns() {
    ...
    for ( ; I != E; ++I ) {
        CSE_ALifeObject* object =
            smart_cast< CSE_ALifeObject* >( &( *I ).second->data()->object() );
        VERIFY( object );
        if ( object->m_spawn_story_id == INVALID_SPAWN_STORY_ID )
            continue;

        m_spawn_story_ids.insert(
            std::make_pair( object->m_spawn_story_id, ( *I ).first ) );
    }
}
