...
bool CPHSkeleton::Spawn( CSE_Abstract* D ) {
    ...
    if ( po->_flags.test( CSE_PHSkeleton::flSpawnCopy ) ) {
        source = smart_cast< CPHSkeleton* >(
            Level().Objects.net_Find( po->source_id ) );
        VERIFY( source );
    }
    if ( source ) {
        ...
        if ( K ) {
            ...
            if ( ini && ini->section_exist( "collide_parts" ) ) {
                ...
                if ( ini->line_exist( "collide_parts",
...
void CPHSkeleton::Update( u32 dt ) {
    ...
    if ( b_removing && Device.dwTimeGlobal > m_remove_time &&
        ...
        #ifdef DEBUG
                        Msg					("ge_destroy:
        [%d] - %s",obj->ID(),*(obj->cName())); #endif if (obj->Local())
        obj->u_EventSend			(P);
        */
        b_removing = false;
    }
...
void CPHSkeleton::SpawnCopy() {
    if ( PPhysicsShellHolder()->Local() ) {
        CSE_Abstract* D =
            F_entity_Create( "ph_skeleton_object" ); //*cNameSect()
        R_ASSERT( D );
        /////////////////////////////////////////////////////////////////////////////////////////////
        CSE_ALifePHSkeletonObject* l_tpALifePhysicObject =
            smart_cast< CSE_ALifePHSkeletonObject* >( D );
...
void CPHSkeleton::UnsplitSingle( CPHSkeleton* SO ) {
    ...
    mask0.invert();
    mask1.assign_and( mask0.flags ); // second part mask
    ...
    SO->CopySpawnInit();
    CopySpawnInit();
    VERIFY3( CheckObjectSize( pKinematics ), *( O->cNameVisual() ),
             "Object unsplit whith no size" );
    VERIFY3( CheckObjectSize( newKinematics ), *( O->cNameVisual() ),
             "Object unsplit whith no size" );
}
...
void CPHSkeleton::SetAutoRemove(
    u32 time /*=CSE_PHSkeleton::existence_time*/ ) {
    b_removing = true;
