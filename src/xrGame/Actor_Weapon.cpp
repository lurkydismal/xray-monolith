...
#define GetWeaponParam( pWeapon, func_name, def_value ) \
    ( ( pWeapon ) ? ( pWeapon->func_name ) : def_value )
...
float CActor::GetWeaponAccuracy() const {
    ...
    // momopate: optionally make pdm values affect ads, have a smoother
    // transition in and out of the aiming bonus
    float disp_aim;
    bool is_zooming =
        ( W && ( ( g_smooth_ads_transition &&
                   GetWeaponParam( W, GetInertionAimFactor(), 1.0f ) < 1.0f ) ||
                 ( !g_smooth_ads_transition && IsZoomAimingMode() &&
                   !GetWeaponParam( W, IsRotatingToZoom(), false ) ) ) );
    if ( is_zooming ) {
        if ( !g_apply_pdm_to_ads )
            ...
            if ( !g_smooth_ads_transition )
                ...
                disp_aim =
                    _lerp( m_fDispAim, 1.0f,
                           GetWeaponParam( W, GetInertionAimFactor(), 1.0f ) );
        }
    } else
        disp_aim = 1.0;

    float dispersion = m_fDispBase * GetWeaponParam( W, Get_PDM_Base(), 1.0f ) *
                       GetWeaponParam( W, Get_Silencer_PDM_Base(), 1.0f ) *
                       GetWeaponParam( W, Get_Scope_PDM_Base(), 1.0f ) *
    ...
    if ( g_State( state ) ) {
        ...
        float AVelocity =
            ( g_fix_avelocity_spread ) ? fCurAVelocity : state.fAVelocity;

        // fAVelocity = angle velocity
        dispersion *=
            ( 1.0f + ( AVelocity / VEL_A_MAX ) * m_fDispVelFactor *
                         GetWeaponParam( W, Get_PDM_Vel_F(), 1.0f ) *
                         GetWeaponParam( W, Get_Silencer_PDM_Vel(), 1.0f ) *
                         GetWeaponParam( W, Get_Scope_PDM_Vel(), 1.0f ) *
                         GetWeaponParam( W, Get_Launcher_PDM_Vel(), 1.0f ) );

        // fVelocity = linear velocity
        dispersion *=
            ( 1.0f + ( state.fVelocity / VEL_MAX ) * m_fDispVelFactor *
                         GetWeaponParam( W, Get_PDM_Vel_F(), 1.0f ) *
                         GetWeaponParam( W, Get_Silencer_PDM_Vel(), 1.0f ) *
                         GetWeaponParam( W, Get_Scope_PDM_Vel(), 1.0f ) *
                         GetWeaponParam( W, Get_Launcher_PDM_Vel(), 1.0f ) );
        ...
        if ( bAccelerated || !state.bCrouch ) {
            dispersion *=
                ( 1.0f +
                  m_fDispAccelFactor *
                      GetWeaponParam( W, Get_PDM_Accel_F(), 1.0f ) *
                      GetWeaponParam( W, Get_Silencer_PDM_Accel(), 1.0f ) *
                      GetWeaponParam( W, Get_Scope_PDM_Accel(), 1.0f ) *
                      GetWeaponParam( W, Get_Launcher_PDM_Accel(), 1.0f ) );
        }

        if ( state.bCrouch ) {
            dispersion *=
                ( 1.0f + m_fDispCrouchFactor *
                             GetWeaponParam( W, Get_PDM_Crouch(), 1.0f ) );
            if ( !bAccelerated ) {
                dispersion *= ( 1.0f + m_fDispCrouchNoAccelFactor *
                                           GetWeaponParam(
                                               W, Get_PDM_Crouch_NA(), 1.0f ) );
            }
        }
    }
...
void CActor::g_fireParams( const CHudItem* pHudItem,
                           Fvector& fire_pos,
                           Fvector& fire_dir ) {
...
BOOL CActor::g_State( SEntityState& state ) const {
    ...
    state.fVelocity =
        character_physics_support()->movement()->GetVelocityActual();
...
void CActor::SelectBestWeapon( CObject* O ) {
    ...
    if ( ( pWeapon || pGrenade || pArtefact ) && pIItem ) {
        ...
        if ( ( GameID() == eGameIDArtefactHunt ) ||
             ( GameID() == eGameIDCaptureTheArtefact ) ) // only for test...
        {
            if ( pIItem->BaseSlot() == INV_SLOT_2 ||
                 pIItem->BaseSlot() == INV_SLOT_3 ) {
                CInventoryItem* pIItemInSlot =
                    inventory().ItemFromSlot( pIItem->BaseSlot() );
    ...
    for ( int i = 0; i < 4; i++ ) {
        if ( inventory().ItemFromSlot( BestWeaponSlots[ i ] ) ) {
            if ( inventory().GetActiveSlot() != BestWeaponSlots[ i ] ) {
                PIItem best_item =
                    inventory().ItemFromSlot( BestWeaponSlots[ i ] );
                if ( best_item && best_item->can_kill() ) {
#ifdef DEBUG
                    Msg( "--- Selecting best weapon [%d], Frame[%d]",
                         BestWeaponSlots[ i ], Device.dwFrame );
#endif // #ifdef DEBUG
...
void CActor::on_weapon_shot_start( CWeapon* weapon ) {
    ...
    CCameraShotEffector* effector = smart_cast< CCameraShotEffector* >(
        Cameras().GetCamEffector( eCEShot ) );
    if ( !effector ) {
...
void CActor::SpawnAmmoForWeapon( CInventoryItem* pIItem ) {
    ...
    ///	CWeaponAmmo* pAmmo = smart_cast<CWeaponAmmo*>(inventory().GetAny(
    ///(pWM->m_ammoTypes[0].c_str()) ));
    //	if (!pAmmo)
...
void CActor::RemoveAmmoForWeapon( CInventoryItem* pIItem ) {
    ...
    CWeaponAmmo* pAmmo = smart_cast< CWeaponAmmo* >(
        inventory().GetAny( pWM->m_ammoTypes[ 0 ].c_str() ) );
    if ( !pAmmo )
        return;
    ...
    if (pWM->m_ammoTypes[0] != pAmmo->CInventoryItem::object().cNameSect())
    continue; CanRemove = false; break;
    };
