...
bool CTrade::CanTrade() {
    ...
    Level().ObjectSpace.GetNearest( m_nearest, pThis.base->Position(), 2.f,
                                    NULL );
    if ( !m_nearest.empty() ) {
    ...
    // Объект рядом
    float dist =
        pPartner.base->Position().distance_to( pThis.base->Position() );
    ...
    float Res =
        rad2deg( _abs( yaw - yaw2 ) < PI ? _abs( yaw - yaw2 )
...
void CTrade::TransferItem( CInventoryItem* pItem, bool bBuying, bool bFree ) {
    ...
    if ( bBuying )
        pPartner.inv_owner->set_money(
            pPartner.inv_owner->get_money() + dwTransferMoney, false );
    else
        pThis.inv_owner->set_money(
            pThis.inv_owner->get_money() + dwTransferMoney, false );
    ...
    if ( ( pPartner.type == TT_ACTOR ) || ( pThis.type == TT_ACTOR ) ) {
        ...
        Actor()->callback( GameObject::eTradeSellBuyItem )(
            pItem->object().lua_game_object(), bDir, dwTransferMoney );
    }
}
...
CInventory& CTrade::GetTradeInv( SInventoryOwner owner ) {
    ...
    // return ((TT_TRADER == owner.type) ? (*owner.inv_owner->m_trade_storage) :
    // (owner.inv_owner->inventory()));
...
u32 CTrade::GetItemPrice( PIItem pItem, bool b_buying, bool b_free ) {
    ...
    // computing base_cost
    float base_cost;
    if ( pArtefact && ( pThis.type == TT_ACTOR ) &&
         ( pPartner.type == TT_TRADER ) ) {
    ...
    // for "dead" weapon we use 10% from base cost, for "good" weapon we use
    // full base cost
    float exponent = 0.0f;
    if ( b_buying ) {
    ...
    float condition_factor = powf( pItem->GetCondition() * 0.9f + .1f,
                                   exponent > 0.0f ? exponent : 0.75f );

    // computing relation factor
    float relation_factor;

    CHARACTER_GOODWILL attitude =
        RELATION_REGISTRY().GetAttitude( pThis.inv_owner, pPartner.inv_owner );
    ...
    if ( buying ) {
        if ( !pThis.inv_owner->trade_parameters().enabled(
                 CTradeParameters::action_buy( 0 ),
                 pItem->object().cNameSect() ) )
            return 0;
        p_trade_factors = &pThis.inv_owner->trade_parameters().factors(
        ...
        if ( !pThis.inv_owner->trade_parameters().enabled(
            ...
            return 0;
        p_trade_factors = &pThis.inv_owner->trade_parameters().factors(
            CTradeParameters::action_sell( 0 ), pItem->object().cNameSect() );
    }
    ...
    if ( trade_factors.friend_factor() <= trade_factors.enemy_factor() )
        action_factor =
            ...
            ( trade_factors.enemy_factor() - trade_factors.friend_factor() ) *
                ( 1.f - relation_factor );
    else
        action_factor =
            trade_factors.enemy_factor() +
            ( trade_factors.friend_factor() - trade_factors.enemy_factor() ) *
                relation_factor;
    ...
    u32 result =
        iFloor( base_cost * condition_factor * action_factor * deficit_factor );

    // use some script discounts
    ::luabind::functor< float > func;
    if ( b_buying )
        R_ASSERT( ai().script_engine().functor(
            "trade_manager.get_buy_discount", func ) );
    else
        R_ASSERT( ai().script_engine().functor(
            "trade_manager.get_sell_discount", func ) );
