...
struct SAttackGoodwillStorage {
    ...
    void load( LPCSTR prefix ) {
        ...
        community_member_attack_goodwill =
            pSettings->r_s32( ACTIONS_POINTS_SECT, s );
...
void RELATION_REGISTRY::Action( CEntityAlive* from,
                                CEntityAlive* to,
                                ERelationAction action ) {
    static CHARACTER_GOODWILL friend_kill_goodwill =
        pSettings->r_s32( ACTIONS_POINTS_SECT, "friend_kill_goodwill" );
    static CHARACTER_GOODWILL neutral_kill_goodwill =
        pSettings->r_s32( ACTIONS_POINTS_SECT, "neutral_kill_goodwill" );
    static CHARACTER_GOODWILL enemy_kill_goodwill =
        pSettings->r_s32( ACTIONS_POINTS_SECT, "enemy_kill_goodwill" );
    ...
    //(с) мин. время через которое снова будет зарегестрировано сообщение об
    // атаке на персонажа
    static u32 min_attack_delta_time =
        u32( 1000.f * pSettings->r_float( ACTIONS_POINTS_SECT,
    ...
    static CHARACTER_GOODWILL community_member_fight_help_goodwill =
        pSettings->r_s32( ACTIONS_POINTS_SECT,
    ...
    if ( stalker ) {
        ...
        relation = GetRelationType( smart_cast< CInventoryOwner* >( stalker ),
                                    inv_owner_from );
    }
    ...
    switch ( action ) {
        case ATTACK: {
            if ( actor ) {
                ...
                if ( Device.dwTimeGlobal - fight_data_from->attack_time <
                     min_attack_delta_time )
                    break;
                ...
                if ( fight_data ) {
                    ...
                    if ( defending_stalker ) {
                        CAI_Stalker* attacking_stalker =
                            smart_cast< CAI_Stalker* >(
                                Level().Objects.net_Find(
                                    fight_data->attacker ) );
                        Action( actor, defending_stalker,
                                attacking_stalker ? FIGHT_HELP_HUMAN
                                                  : FIGHT_HELP_MONSTER );
                    }
                }
            }
            ...
            if ( stalker ) {
                ...
                const CEntityAlive* stalker_enemy =
                    stalker->memory().enemy().selected();
                if ( actor && stalker_enemy ) {
                    const CInventoryOwner* const_inv_owner_from =
                        inv_owner_from;
                    if ( stalker_enemy->human_being() ) {
                        ...
                        ALife::ERelationType relation_to_actor =
                            GetRelationType( const_inv_owner_stalker_enemy,
                                             const_inv_owner_from );

                        if ( relation_to_actor == ALife::eRelationTypeEnemy )
                            bDangerScheme = true;
                    }
                }
                SAttackGoodwillStorage* st =
                    bDangerScheme ? &gw_danger : &gw_free;
                ...
                // сталкер при нападении на членов своей же группировки
                // отношения не меняют (считается, что такое нападение всегда
                // случайно)
                //  change relation only for pairs actor->stalker, do not use
                //  pairs stalker->stalker
                bool stalker_attack_team_mate = stalker && stalker_from;
                if ( delta_goodwill && !stalker_attack_team_mate ) {
                    // изменить отношение ко всем членам атакованой группы (если
                    // такая есть) как к тому кого атаковали
                    CGroupHierarchyHolder& group =
                        Level()
                            .seniority_holder()
                            .team( stalker->g_Team() )
                    ...
                    for ( std::size_t i = 0; i < group.members().size(); i++ ) {
                        ChangeGoodwill( group.members()[ i ]->ID(), from->ID(),
                                        delta_goodwill );
                    }

                    //*(CHARACTER_GOODWILL)( stalker->Sympathy() *
                    //(float)(delta_goodwill));
                    CHARACTER_GOODWILL community_goodwill =
                        ( CHARACTER_GOODWILL )( stalker->Sympathy() *
                                                ( float )( st->community_member_attack_goodwill ) );
                    if ( community_goodwill ) {
                        ChangeCommunityGoodwill( stalker->Community(),
                                                 from->ID(),
                                                 community_goodwill );
                    }
                }
        ...
        case KILL: {
            if ( stalker ) {
                ...
                ALife::ERelationType relation_before_attack =
                    ALife::eRelationTypeDummy;
                // if(fight_data_from)
                //	relation_before_attack =
                // fight_data_from->defender_to_attacker; else
                ...
                // отношения не меняют (считается, что такое нападение всегда
                // случайно)
                bool stalker_kills_team_mate =
                    stalker_from &&
                    ( stalker_from->Community() == stalker->Community() );

                if ( delta_goodwill && !stalker_kills_team_mate ) {
                    // изменить отношение ко всем членам группы (если такая
                    // есть) убитого, кроме него самого
                    CGroupHierarchyHolder& group =
                        Level()
                            .seniority_holder()
                            .team( stalker->g_Team() )
                    ...
                    for ( std::size_t i = 0; i < group.members().size(); i++ ) {
                        if ( stalker->ID() != group.members()[ i ]->ID() ) {
                            ChangeGoodwill( group.members()[ i ]->ID(),
                                            from->ID(), delta_goodwill );
                        }
                    }
                    ...
                    CHARACTER_GOODWILL community_goodwill =
                        ( CHARACTER_GOODWILL )( stalker->Sympathy() *
                                                ( float )( community_member_kill_goodwill ) );
                    if ( community_goodwill ) {
                        ChangeCommunityGoodwill( stalker->Community(),
                                                 from->ID(),
                                                 community_goodwill );
                ...
                delta_rank = CHARACTER_RANK::rank_kill_points(
                    CHARACTER_RANK::ValueToIndex( stalker->Rank() ) );
                if ( delta_rank )
        ...
        case FIGHT_HELP_MONSTER: {
            if ( stalker && stalker->g_Alive() ) {
                ...
                if ( delta_goodwill ) {
                    ...
                    CHARACTER_GOODWILL community_goodwill =
                        ( CHARACTER_GOODWILL )( stalker->Sympathy() *
                                                ( float )( community_member_fight_help_goodwill ) );
                    if ( community_goodwill ) {
                        ChangeCommunityGoodwill( stalker->Community(),
                                                 from->ID(),
                                                 community_goodwill );
