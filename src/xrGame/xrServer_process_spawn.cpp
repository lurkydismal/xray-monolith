...
CSE_Abstract* xrServer::Process_spawn( NET_Packet& P,
                                       ClientID sender,
                                       BOOL bSpawnWithClientsMainEntityAsParent,
    ...
    if ( E->RespawnTime && ( 0xffff == E->ID_Phantom ) ) {
        ...
        Phantom->ID_Phantom =
            Phantom->ID; // Self-linked to avoid phantom-breeding
        Phantom->owner = NULL;
    ...
    // log
    // Msg		("- SERVER: Spawning '%s'(%d,%d,%d) as #%d, on '%s'",
    // E->s_name_replace, E->g_team(), E->g_squad(), E->g_group(), E->ID,
    // CL?CL->Name:"*SERVER*");
    return E;
}
