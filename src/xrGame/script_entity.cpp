...
void CScriptEntity::SetScriptControl( const bool bScriptControl,
                                      shared_str caSciptName ) {
    ...
    /*
            ...
            if (bScriptControl)
                    ai().script_engine().script_log
    (ScriptStorage::eLuaMessageTypeInfo,"Script %s set object %s under its
    control",*caSciptName,*object().cName()); else
                    ai().script_engine().script_log
    (ScriptStorage::eLuaMessageTypeInfo,"Script %s freed object %s from its
    control",*caSciptName,*object().cName()); #endif
    */
    if ( !bScriptControl )
...
bool CScriptEntity::CheckTypeVisibility( const char* section_name ) {
    ...
    CVisualMemoryManager::VISIBLES::const_iterator I =
        m_monster->memory().visual().objects().begin();
    CVisualMemoryManager::VISIBLES::const_iterator E =
        m_monster->memory().visual().objects().end();
    for ( ; I != E; ++I ) {
        VERIFY( ( *I ).m_object );
...
void CScriptEntity::AddAction( const CScriptEntityAction* tpEntityAction,
                               bool bHighPriority ) {
    bool empty = m_tpActionQueue.empty();
    if ( !bHighPriority || m_tpActionQueue.empty() )
        m_tpActionQueue.push_back(
            xr_new< CScriptEntityAction >( *tpEntityAction ) );
    else {
        VERIFY( m_tpActionQueue.front() );
        CScriptEntityAction* l_tpEntityAction =
            xr_new< CScriptEntityAction >( *m_tpActionQueue.front() );
        vfFinishAction( m_tpActionQueue.front() );
        xr_delete( m_tpActionQueue.front() );
        m_tpActionQueue.front() = l_tpEntityAction;
        m_tpActionQueue.insert(
            m_tpActionQueue.begin(),
            xr_new< CScriptEntityAction >( *tpEntityAction ) );
    }
...
void CScriptEntity::vfUpdateParticles() {
    if ( !GetCurrentAction() )
        return;
    CScriptParticleAction& l_tParticleAction =
        GetCurrentAction()->m_tParticleAction;
    if ( xr_strlen( l_tParticleAction.m_caBoneName ) ) {
        intrusive_ptr< CParticlesObject > l_tpParticlesObject =
            l_tParticleAction.m_tpParticleSystem;
        l_tpParticlesObject->UpdateParent(
            GetUpdatedMatrix( l_tParticleAction.m_caBoneName,
...
void CScriptEntity::vfUpdateSounds() {
    ...
    if ( xr_strlen( l_tSoundAction.m_caBoneName ) && m_current_sound &&
         m_current_sound->_feedback() )
        m_current_sound->_feedback()->set_position(
            GetUpdatedMatrix( l_tSoundAction.m_caBoneName,
...
void CScriptEntity::vfFinishAction( CScriptEntityAction* tpEntityAction ) {
    ...
    if ( !tpEntityAction->m_tParticleAction.m_bAutoRemove )
        Particles::Details::Destroy(
            tpEntityAction->m_tParticleAction.m_tpParticleSystem );
}
...
void CScriptEntity::ProcessScripts() {
    ...
    while ( !m_tpActionQueue.empty() ) {
        ...
        if ( true /*psAI_Flags.is(aiLua)*/ ) {
            object().callback( GameObject::eActionTypeRemoved )(
                object().lua_game_object(), u32( eActionTypeRemoved ) );
        }
    ...
    if ( m_tpActionQueue.empty() ) {
        ...
        if ( empty_queue )
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeInfo,
                "Object %s has an empty script queue!", *object().cName() );
#endif
        return;
    }
    ...
    if ( l_tpEntityAction->m_tWatchAction.m_bCompleted && !l_bCompleted )
        object().callback( GameObject::eActionTypeWatch )(
            object().lua_game_object(), u32( eActionTypeWatch ) );
    ...
    if ( l_tpEntityAction->m_tSoundAction.m_bCompleted && !l_bCompleted )
        object().callback( GameObject::eActionTypeSound )(
            object().lua_game_object(), u32( eActionTypeSound ) );
    ...
    if ( l_tpEntityAction->m_tParticleAction.m_bCompleted && !l_bCompleted )
        object().callback( GameObject::eActionTypeParticle )(
            object().lua_game_object(), u32( eActionTypeParticle ) );
    ...
    if ( l_tpEntityAction->m_tObjectAction.m_bCompleted && !l_bCompleted )
        object().callback( GameObject::eActionTypeObject )(
            object().lua_game_object(), u32( eActionTypeObject ) );
...
bool CScriptEntity::bfAssignWatch( CScriptEntityAction* tpEntityAction ) {
    return ( GetCurrentAction() &&
             !GetCurrentAction()->m_tWatchAction.m_bCompleted );
}

bool CScriptEntity::bfAssignMonsterAction(
    CScriptEntityAction* tpEntityAction ) {
    if ( GetCurrentAction() &&
         GetCurrentAction()->m_tMonsterAction.m_bCompleted )
        return ( false );
...
bool CScriptEntity::bfAssignAnimation( CScriptEntityAction* tpEntityAction ) {
    ...
    IKinematicsAnimated& tVisualObject =
        *( smart_cast< IKinematicsAnimated* >( object().Visual() ) );
    m_tpNextAnimation = tVisualObject.ID_Cycle_Safe(
        *GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay );
    m_use_animation_movement_controller =
        GetCurrentAction()
            ->m_tAnimationAction.m_use_animation_movement_controller;
    return ( true );
}

const Fmatrix CScriptEntity::GetUpdatedMatrix( shared_str caBoneName,
                                               const Fvector& tPositionOffset,
    ...
    if ( m_current_sound ) {
        if ( !m_current_sound->_feedback() )
            ...
            //%s",Device.dwTimeGlobal,*l_tSoundAction.m_caSoundToPlay);
                ...
                m_current_sound->play_at_pos(
                    m_object, l_tMatrix.c,
                    l_tSoundAction.m_bLooped ? sm_Looped : 0 );
        ...
        if ( xr_strlen( l_tSoundAction.m_caSoundToPlay ) ) {
            ...
            m_current_sound->create( *l_tSoundAction.m_caSoundToPlay, st_Effect,
                                     l_tSoundAction.m_sound_type );
        } else
            l_tSoundAction.m_bCompleted = true;
    }
...
bool CScriptEntity::bfAssignParticles( CScriptEntityAction* tpEntityAction ) {
    CScriptParticleAction& l_tParticleAction =
        tpEntityAction->m_tParticleAction;
        ...
        if ( true /** !l_tParticleAction.m_tpParticleSystem/**/ )
            if ( !l_tParticleAction.m_bStartedToPlay ) {
                ...
                l_tParticleAction.m_tpParticleSystem->UpdateParent( l_tMatrix,
                                                                    zero_vel );
                l_tParticleAction.m_tpParticleSystem->play_at_pos(
                    l_tMatrix.c );
...
bool CScriptEntity::bfAssignObject( CScriptEntityAction* tpEntityAction ) {
    return ( GetCurrentAction() &&
             !GetCurrentAction()->m_tObjectAction.m_bCompleted );
}

bool CScriptEntity::bfAssignMovement( CScriptEntityAction* tpEntityAction ) {
    CScriptMovementAction& l_tMovementAction =
        tpEntityAction->m_tMovementAction;
    ...
    if ( !m_monster ) {
        ai().script_engine().script_log(
            eLuaMessageTypeError,
            "Cannot assign a movement action not to a monster!" );
        return ( true );
    }
    ...
    switch ( l_tMovementAction.m_tGoalType ) {
        case CScriptMovementAction::eGoalTypeObject: {
            CGameObject* l_tpGameObject =
                smart_cast< CGameObject* >( l_tMovementAction.m_tpObjectToGo );
#ifdef DEBUG
            THROW2( l_tpGameObject,
                    "eGoalTypeObject specified, but no object passed!" );
#else
            ...
            m_monster->movement().set_level_dest_vertex(
        ...
        case CScriptMovementAction::eGoalTypePatrolPath: {
            ...
            m_monster->movement().patrol().set_path(
                l_tMovementAction.m_path, l_tMovementAction.m_path_name );
            m_monster->movement().patrol().set_start_type(
                l_tMovementAction.m_tPatrolPathStart );
            m_monster->movement().patrol().set_route_type(
                l_tMovementAction.m_tPatrolPathStop );
            m_monster->movement().patrol().set_random(
                l_tMovementAction.m_bRandom );
            if ( l_tMovementAction.m_previous_patrol_point != u32( -1 ) ) {
                m_monster->movement().patrol().set_previous_point(
                    l_tMovementAction.m_previous_patrol_point );
            }
            break;
        }
        ...
        case CScriptMovementAction::eGoalTypePathPosition: {
            ...
            if ( !ai().level_graph().valid_vertex_id( vertex_id ) ) {
                string256 S;
                xr_sprintf( S,
                            "Cannot find corresponding level vertex for the "
                            "specified position [%f][%f][%f] for monster %s",
                            VPUSH( l_tMovementAction.m_tDestinationPosition ),
                            *m_monster->cName() );
        ...
        case CScriptMovementAction::eGoalTypePathNodePosition: {
            VERIFY( ai().level_graph().valid_vertex_id(
        ...
        case CScriptMovementAction::eGoalTypeNoPathPosition: {
            ...
            if ( m_monster->movement().detail().path().empty() ||
                 ( m_monster->movement()
                       .detail()
                       .path()[ m_monster->movement().detail().path().size() -
                                1 ]
                       .position.distance_to(
                           l_tMovementAction.m_tDestinationPosition ) >
                   .1f ) ) {
                m_monster->movement().detail().m_path.resize( 2 );
                m_monster->movement().detail().m_path[ 0 ].position =
                    object().Position();
                m_monster->movement().detail().m_path[ 1 ].position =
                    l_tMovementAction.m_tDestinationPosition;
                m_monster->movement().detail().m_current_travel_point = 0;
            }

            if ( m_monster->movement().detail().m_path[ 1 ].position.similar(
                     object().Position(), .2f ) )
                l_tMovementAction.m_bCompleted = true;
    ...
    if ( m_monster->movement().actual_all() &&
         m_monster->movement().path_completed() )
        l_tMovementAction.m_bCompleted = true;
...
LPCSTR CScriptEntity::GetPatrolPathName() {
    ...
    if ( !GetScriptControl() ) {
        ai().script_engine().script_log(
            eLuaMessageTypeError,
            "Object %s is not under script control while you are trying to get "
            "patrol path name!",
            *m_object->cName() );
        return "";
    }
...
void ScriptCallBack( CBlend* B ) {
    CScriptEntity* l_tpScriptMonster =
        static_cast< CScriptEntity* >( B->CallbackParam );
    VERIFY( l_tpScriptMonster );
    if ( l_tpScriptMonster->GetCurrentAction() && !B->bone_or_part ) {
        if ( !l_tpScriptMonster->GetCurrentAction()
                  ->m_tAnimationAction.m_bCompleted )
            l_tpScriptMonster->object().callback(
                GameObject::eActionTypeAnimation )(
                l_tpScriptMonster->object().lua_game_object(),
                u32( eActionTypeAnimation ) );

        l_tpScriptMonster->m_tpScriptAnimation.invalidate();
        l_tpScriptMonster->GetCurrentAction()->m_tAnimationAction.m_bCompleted =
            true;
...
bool CScriptEntity::bfScriptAnimation() {
    ...
    if ( GetScriptControl() && GetCurrentAction() &&
        ...
        for ( u16 i = 0; i < MAX_PARTS; ++i ) {
            ...
            blend = skeleton_animated->LL_PlayCycle( i, animation, TRUE,
                                                     ScriptCallBack, this );
            if ( !blend )
                continue;
            result = blend;
...
const CScriptEntityAction* CScriptEntity::GetActionByIndex(
    u32 action_index ) const {
    return ( m_tpActionQueue[ action_index ] );
}

void CScriptEntity::sound_callback( const CObject* object,
                                    int sound_type,
    ...
    m_saved_sounds.push_back(
        CSavedSound( object->ID(), sound_type, position, sound_power ) );
}
...
void CScriptEntity::process_sound_callbacks() {
    ...
    for ( ; I != E; ++I ) {
        ...
        object().callback( GameObject::eSound )(
            object().lua_game_object(), ( *I ).m_game_object_id,
            ( *I ).m_sound_type, ( *I ).m_position, ( *I ).m_sound_power );
    }
