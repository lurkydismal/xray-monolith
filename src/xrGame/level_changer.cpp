...
BOOL CLevelChanger::net_Spawn( CSE_Abstract* DC ) {
    ...
    CSE_ALifeLevelChanger* l_tpALifeLevelChanger =
        smart_cast< CSE_ALifeLevelChanger* >( l_tpAbstract );
    ...
    if ( ai().get_level_graph() ) {
        ...
        ai_location().level_vertex(
            ai().level_graph().vertex( u32( -1 ), Position() ) );
...
void CLevelChanger::feel_touch_new( CObject* tpObject ) {
    ...
    if ( pGameSP )
        pGameSP->ChangeLevel( m_game_vertex_id, m_level_vertex_id, m_position,
                              m_angles, p, r, b, m_invite_str, m_b_enabled );
...
bool CLevelChanger::get_reject_pos( Fvector& p, Fvector& r ) {
    ...
    if ( m_ini_file && m_ini_file->section_exist( "pt_move_if_reject" ) ) {
        ...
        const CPatrolPath* patrol_path =
            ai().patrol_paths().safe_path( p_name, false, true );
        VERIFY( patrol_path );
...
bool CLevelChanger::feel_touch_contact( CObject* object ) {
    ...
    bRes = bRes && smart_cast< CActor* >( object ) &&
           smart_cast< CActor* >( object )->g_Alive();
    return bRes;
}
...
void CLevelChanger::update_actor_invitation() {
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( m_entrance_time + 5.0f < Device.fTimeGlobal ) {
            ...
            if ( pGameSP )
                pGameSP->ChangeLevel( m_game_vertex_id, m_level_vertex_id,
                                      m_position, m_angles, p, r, b,
                                      m_invite_str, m_b_enabled );
