...
private:
    ...
    public:
        IC bool operator()( shared_str const& lhs,
                            shared_str const& rhs ) const {
            return ( lhs._get() < rhs._get() );
        }
    };

public:
    typedef associative_vector< shared_str, action*, action_predicate >
        ActionList;
    typedef xr_vector< shared_str > TransitionData;
    typedef smart_cover::action::Animations Animations;
    typedef CGraphAbstract< xr_empty, float, shared_str, TransitionData >
        TransitionGraph;
...
public:
    ...
    Animations const& action_animations( shared_str const& action_id,
                                         shared_str const& animation_id ) const;
    TransitionData const& transition_animations(
