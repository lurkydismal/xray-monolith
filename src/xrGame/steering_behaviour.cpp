...
vec evade::calc_acceleration() {
    ...
    const vec pos2dest_norm =
        ( dest2pos_mag > detail::near_zero )
...
vec containment::calc_acceleration() {
    ...
    for ( params::Probes::iterator i = m_p_params->probes.begin(),
                                   e = m_p_params->probes.end();
          i != e; ++i ) {
...
vec grouping::calc_acceleration() {
    ...
    for ( m_p_params->first_nearest( cur_nearest );
        ...
        if ( point2pos_mag < m_p_params->max_separate_range ) {
            const vec pos2dest_norm =
                ( point2pos_mag > detail::near_zero )
                    ? ( point2pos * ( 1.f / point2pos_mag ) )
                    : normalize( ( *m_p_params->pf_random_dir )() );

            steer = steer + pos2dest_norm *
                                calc_dist_factor( m_p_params->separation_factor,
                                                  point2pos_mag );
        }
    ...
    if ( pos2center_mag > detail::near_zero ) {
        steer = steer + normalize( pos2center ) *
                            calc_dist_factor( m_p_params->cohesion_factor,
                                              pos2center_mag );
    }
...
vec manager::calc_acceleration() {
    ...
    for ( Behaviours::iterator i = m_behaviours.begin(), e = m_behaviours.end();
          i != e; ++i ) {
        base* p_base = ( *i );
...
void manager::remove_scheduled() {
    for ( Behaviours::iterator i = m_schedule_remove.begin(),
                               e = m_schedule_remove.end();
          i != e; ++i ) {
