...
void CIKFoot::Create( IKinematics* K, LPCSTR section, u16 bones[ 4 ] ) {
    ...
    if ( section ) {
        ...
        m_foot_normal.v =
            Kinematics()->LL_UserData()->r_fvector3( section, "foot_normal" );
        m_foot_direction.v = Kinematics()->LL_UserData()->r_fvector3(
            section, "foot_direction" );
    }
    set_toe( bones );
}

struct envc : private xray::noncopyable, public SEnumVerticesCallback {
    Fvector& pos;
    Fvector start_pos;
    ...
    envc( const Fmatrix& _i_bind_transform, const Fvector& _ax, Fvector& _pos )
        : SEnumVerticesCallback(),
          i_bind_transform( _i_bind_transform ),
          ax( _ax ),
          pos( _pos ) {
        start_pos.set( 0, 0, 0 );
    }
    ...
    void operator()( const Fvector& p ) {
        ...
        if ( Fvector().sub( lpos, start_pos ).dotproduct( ax ) >
             Fvector().sub( pos, start_pos ).dotproduct( ax ) )
            pos.set( lpos );
    }
};
...
void CIKFoot::set_toe( u16 bones[ 4 ] ) {
    ...
    m_heel_position.v.add( Fvector().mul(
        foot_dir,
        Fvector().sub( m_toe_position.v, pos ).dotproduct( foot_dir ) *
            0.2f ) );
    ...
    m_foot_width = ( Fvector().sub( m_toe_position.v, b2to_ref.c ) )
                       .dotproduct( foot_normal );
}
...
Fmatrix& CIKFoot::foot_to_ref_bone_transform( Fmatrix& m ) const {
    ...
    m.mul_43(
        Fmatrix().invert( Kinematics()->LL_GetTransform( m_foot_bone_id ) ),
...
Fmatrix& CIKFoot::foot_to_ref_bone( Fmatrix& ref_bone,
                                    const Fmatrix& foot ) const {
    if ( m_ref_bone == 2 ) {
        ref_bone = foot;
        return ref_bone;
    }
...
Fmatrix& CIKFoot::ref_bone_to_foot( Fmatrix& foot,
                                    const Fmatrix& ref_bone ) const {
    if ( m_ref_bone == 2 ) {
        foot = ref_bone;
        return foot;
    }
    ...
    /*
            ...
            //m.mul_43( Fmatrix().invert(
       Kinematics()->LL_GetTransform(m_bones[2] )
       ),Kinematics()->LL_GetTransform( m_bones[3] ) );
...
ik_goal_matrix::e_collide_state CIKFoot::CollideFoot(
    ...
    float dfoot_tri =
        -p.d - p.n.dotproduct(
                   global_bone_pos ); // dist from foot bone pos to tri plain
    Fvector axp;
    ...
    axp.sub(
        Fvector().mul( ax, axp.dotproduct( ax ) ) ); // vector from nc_toe to ax
...
bool CIKFoot::make_shift( Fmatrix& xm,
                          const Fvector& cl_point,
                          bool collide,
                          const Fplane& p,
...
bool CIKFoot::GetFootStepMatrix( ik_goal_matrix& m,
...
ik_goal_matrix::e_collide_state CIKFoot::rotate( Fmatrix& xm,
                                                 const Fplane& p,
                                                 const Fvector& foot_normal,
                                                 const Fvector& global_point,
                                                 bool collide ) const {
    Fvector ax;
    ...
    if ( !fis_zero( s ) ) {
        ...
        ref_bone_to_foot( xm );
        if ( collide )
            cl_state = CollideFoot( angle, angle, global_point, foot_normal,
                                    xm.c, p, ax );
        // if( cld.m_pick_dir )
        Fvector c = xm.c;
...
bool CIKFoot::GetFootStepMatrix( ik_goal_matrix& m,
    ...
    global_anim.transform_tiny(
        global_point, local_point ); // non collided toe in global space
    Fvector foot_normal;
    ...
    if ( cld.m_collide_point == ik_foot_geom::heel ||
         cld.m_collide_point == ik_foot_geom::side ) {
        Fmatrix foot;
...
#endif
        Fmatrix foot_to_ref;
        ref_bone_to_foot_transform( foot_to_ref )
            .transform_tiny( local_point, heel );
    }
    ...
    if ( b_make_shift &&
         make_shift( xm, local_point, collide, p, cld.m_pick_dir ) )
        switch ( cl_state ) {
            case ik_goal_matrix::cl_aligned:
                break;
    ...
    if ( !fsimilar( _abs( DET( g_anim ) - 1.f ), _abs( DET( m.get() ) - 1.f ),
                    0.001f ) )
...
u16 CIKFoot::get_ref_bone( const Fmatrix& foot_transform,
...
void CIKFoot::SetFootGeom( ik_foot_geom& fg,
                           const Fmatrix& ref_bone,
                           const Fmatrix& object_matrix ) const {
    Fmatrix gl_bone;
    ...
    Fvector heel;
    Fvector pos_hill;
    Fmatrix foot = ( Fmatrix().mul_43( object_matrix,
                                       ref_bone_to_foot( foot, ref_bone ) ) );
...
void CIKFoot::Collide( SIKCollideData& cld,
