...
IC void CALifeGraphRegistry::change( CSE_ALifeDynamicObject* object,
                                     GameGraph::_GRAPH_ID tGraphPointID,
                                     GameGraph::_GRAPH_ID tNextGraphPointID ) {
    VERIFY3( object->used_ai_locations() /** && (object->interactive() ||
                                            object->m_bOnline)/**/
             ,
             *object->s_name, object->name_replace() );
    ...
    object->m_tNodeID =
        ai().game_graph().vertex( object->m_tGraphID )->level_vertex_id();
}
...
IC void CALifeGraphRegistry::assign( CSE_ALifeMonsterAbstract* monster ) {
    ...
    for ( ; i != e; ++i )
        if ( ( *i ).distance() > monster->m_fDistance ) {
            monster->m_fDistanceFromPoint =
                ( *i ).distance() - monster->m_fDistance;
            break;
        }
}
...
IC const CALifeGraphRegistry::GRAPH_REGISTRY& CALifeGraphRegistry::objects()
    const {
    return ( m_objects );
}

template < typename F >
IC void CALifeGraphRegistry::iterate_objects(
    GameGraph::_GRAPH_ID game_vertex_id,
    const F& f ) {
    iterate( ( ( CGraphPointInfo& )( objects()[ game_vertex_id ] ) ).objects(),
             f );
}
