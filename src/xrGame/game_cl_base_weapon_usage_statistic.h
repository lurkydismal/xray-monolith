...
struct Player_Statistic {
    ...
    u32 create_victims_table(
        victims_table& victims_table ); // retutns size in bytes of table
    u32 create_bone_table(
        bone_table& bone_table ); // retutns size in bytes of table
};
...
struct Bullet_Check_Request {
    ...
    Bullet_Check_Request()
        : BulletID( 0 ), BoneID( 0 ), Result( false ), Processed( false ) {};

    Bullet_Check_Request( u32 ID, s16 BID )
...
struct WeaponUsageStatistic {
    ...
    virtual void OnBullet_Hit( SBullet* pBullet,
                               u16 TargetID,
                               s16 element,
                               Fvector HitLocation );
