...
//-------------------------------------------------------
xrGameSpyServer::EConnect xrGameSpyServer::Connect(
    shared_str& session_name,
    ...
    //	m_bCheckCDKey = game->get_option_i
    //(*session_name,"cdkey",0) != 0; 	m_bCheckCDKey = game->get_option_i
    //(*session_name,"public",0) != 0;
    //--------------------------------------------//
...
bool xrGameSpyServer::NeedToCheckClient_GameSpy_CDKey( IClient* CL ) {
    if ( !m_bCDKey_Initialized ||
         ( CL == GetServerClient() && g_dedicated_server ) ) {
        return false;
    };
    ...
    switch ( type ) {
        case M_GAMESPY_CDKEY_VALIDATION_CHALLENGE_RESPOND: {
            ...
            if ( bytesRemain == 0 || bytesRemain > sizeof( ResponseStr ) ) {
                ...
                Msg( "! WARNING: Validation challenge respond from client [%s] "
                     "is %s. DoS attack?",
                     clientIp.c_str(),
       ...
       // m_GCDServer.AuthUser(int(CL->ID.value()), CL->m_cAddress.m_data.data,
       // CL->m_pChallengeString, ResponseStr, this);
       // xr_strcpy(CL->m_guid,128,this->GCD_Server()->GetKeyHash(CL->ID.value()));
            } else {
                Msg( "Server : Respond accepted, ReAuthenticate client." );
                // m_GCDServer.ReAuthUser(int(CL->ID.value()),
                // CL->m_iCDKeyReauthHint, ResponseStr);
            }
    ...
    if ( FS.exist( fn ) ) {
        ...
        if ( inif.section_exist( "users" ) ) {
            if ( inif.line_count( "users" ) != 0 ) {
                ...
                xr_strcpy( res,
                           "# Server started as protected, using users list." );
                Msg( res );
                return;
            } else {
        ...
        xr_strcpy(
            res,
            "File <server_users.ltx> not found in folder <$app_data_root$>." );
    } // if FS.exist(fn)
...
void xrGameSpyServer::GetServerInfo( CServerInfo* si ) {
    ...
    si->AddItem( "GameSpy port", itoa( iGameSpyBasePort, tmp, 10 ),
                 RGB( 200, 5, 155 ) );
