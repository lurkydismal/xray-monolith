...
IC bool CALifeSpawnRegistry::time_limit( CSE_Abstract& abstract,
...
IC bool CALifeSpawnRegistry::spawned_item( CSE_Abstract& abstract,
                                           SPAWN_IDS& objects ) const {
    SPAWN_IDS::iterator I =
        std::lower_bound( objects.begin(), objects.end(), abstract.m_tSpawnID );
...
IC bool CALifeSpawnRegistry::object_existance_limit(
...
IC bool CALifeSpawnRegistry::can_spawn( CSE_Abstract& abstract,
                                        ALife::_TIME_ID game_time,
                                        SPAWN_IDS& objects ) const {
    return ( enabled_spawn( abstract ) && !count_limit( abstract ) &&
             !time_limit( abstract, game_time ) &&
             !object_existance_limit( abstract, objects ) );
}

void CALifeSpawnRegistry::fill_new_spawns_single( SPAWN_GRAPH::CVertex* vertex,
                                                  SPAWN_IDS& spawns,
                                                  ALife::_TIME_ID game_time,
                                                  SPAWN_IDS& objects ) {
    if ( !!vertex->data()->object().m_spawn_flags.is(
             CSE_Abstract::flSpawnIfDestroyedOnly ) &&
    ...
    //	float						group_probability =
    // vertex->data()->object().m_spawn_probability;
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( accumulator > probability ) {
            ...
            fill_new_spawns( m_spawns.vertex( ( *I ).vertex_id() ), spawns,
                             game_time, objects );
            return;
        }
    }
...
void CALifeSpawnRegistry::fill_new_spawns( SPAWN_GRAPH::CVertex* vertex,
                                           SPAWN_IDS& spawns,
                                           ALife::_TIME_ID game_time,
                                           SPAWN_IDS& objects ) {
    VERIFY( vertex );
    ...
    if ( !!vertex->data()->object().m_spawn_flags.is(
             CSE_Abstract::flSpawnSingleItemOnly ) ) {
...
void CALifeSpawnRegistry::fill_new_spawns( SPAWN_IDS& spawns,
                                           ALife::_TIME_ID game_time,
                                           SPAWN_IDS& objects ) {
