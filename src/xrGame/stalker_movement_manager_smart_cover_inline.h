...
IC void stalker_movement_manager_smart_cover::property_storage(
...
IC void stalker_movement_manager_smart_cover::non_animated_loophole_change(
    bool const& value ) {
    m_non_animated_loophole_change = value;
}

IC float const&
stalker_movement_manager_smart_cover::apply_loophole_direction_distance()
    const {
    return ( m_apply_loophole_direction_distance );
}

IC void stalker_movement_manager_smart_cover::apply_loophole_direction_distance(
    float const& value ) {
    m_apply_loophole_direction_distance = value;
}

IC stalker_movement_manager_smart_cover::target_selector_type&
stalker_movement_manager_smart_cover::target_selector() {
    VERIFY( m_target_selector );
    return ( *m_target_selector );
}

IC bool
stalker_movement_manager_smart_cover::entering_smart_cover_with_animation()
    const {
...
IC void stalker_movement_manager_smart_cover::check_can_kill_enemy(
    bool const value ) {
    m_check_can_kill_enemy = value;
}

IC void stalker_movement_manager_smart_cover::combat_behaviour(
    bool const value ) {
