...
IC bool CAI_Stalker::CTradeItem::operator<(
    const CTradeItem& trade_item ) const {
...
bool CAI_Stalker::tradable_item( CInventoryItem* inventory_item,
    ...
    return (
        trade_parameters().enabled( CTradeParameters::action_sell( 0 ),
                                    inventory_item->object().cNameSect() ) );
}

u32 CAI_Stalker::fill_items( CInventory& inventory,
    ...
    for ( ; I != E; ++I ) {
        ...
        m_temp_items.push_back(
            CTradeItem( *I, old_owner->ID(), new_owner_id ) );
        result += ( *I )->Cost();
    }
...
void CAI_Stalker::transfer_item( CInventoryItem* item,
                                 CGameObject* old_owner,
...
IC void CAI_Stalker::buy_item_virtual( CTradeItem& item ) {
    ...
    if ( m_current_trader )
        m_current_trader->set_money(
            m_current_trader->get_money() + item.m_item->Cost(), true );
}
...
void CAI_Stalker::choose_weapon(
    ALife::EWeaponPriorityType weapon_priority_type ) {
        ...
        switch ( weapon_priority_type ) {
            ...
            case ALife::eWeaponPriorityTypeSecondary: {
                if ( 5 != j )
                    continue;
                current_value =
                    ai().ef_storage().m_pfSmallWeaponValue->ffGetValue();
                if ( I->m_item->m_ItemCurrPlace.type == eItemPlaceSlot )
                    current_value += 10.0f;
                break;
            }
            case ALife::eWeaponPriorityTypePrimary: {
                if ( ( 6 != j ) && ( 7 != j ) && ( 8 != j ) && ( 9 != j ) &&
                    ...
                    continue;
                current_value =
                    ai().ef_storage().m_pfMainWeaponValue->ffGetValue();
                if ( I->m_item->m_ItemCurrPlace.type == eItemPlaceSlot )
                    current_value += 10.0f;
                break;
            }
...
void CAI_Stalker::choose_detector() {
    ...
    for ( ; I != E; ++I ) {
        ...
        CCustomDetector* detector =
            smart_cast< CCustomDetector* >( ( *I ).m_item );
        if ( !detector )
            continue;
...
bool CAI_Stalker::can_sell( CInventoryItem* item ) {
    ...
    update_sell_info();
    xr_vector< CTradeItem >::const_iterator I = std::find(
        m_temp_items.begin(), m_temp_items.end(), item->object().ID() );
    VERIFY( I != m_temp_items.end() );
    return ( ( *I ).m_new_owner_id != ID() );
}

bool CAI_Stalker::AllowItemToTrade( CInventoryItem const* item,
                                    const SInvItemPlace& place ) const {
    if ( !g_Alive() )
        return ( trade_parameters().enabled( CTradeParameters::action_show( 0 ),
                                             item->object().cNameSect() ) );

    return ( const_cast< CAI_Stalker* >( this )->can_sell(
        const_cast< CInventoryItem* >( item ) ) );
}

bool CAI_Stalker::non_conflicted( const CInventoryItem* item,
...
bool CAI_Stalker::enough_ammo( const CWeapon* new_weapon ) const {
    ...
    for ( ; I != E; ++I ) {
        if ( std::find( new_weapon->m_ammoTypes.begin(),
...
bool CAI_Stalker::conflicted( const CInventoryItem* item,
...
void CAI_Stalker::remove_personal_only_ammo( const CInventoryItem* item ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        for ( ; i != e; ++i ) {
            ...
            if ( std::find( temp->m_ammoTypes.begin(), temp->m_ammoTypes.end(),
                            *I ) == temp->m_ammoTypes.end() )
                continue;
...
void CAI_Stalker::update_conflicted( CInventoryItem* item,
                                     const CWeapon* new_weapon ) {
    if ( non_conflicted( item, new_weapon ) )
        return;
...
void CAI_Stalker::on_after_take( const CGameObject* object ) {
    ...
    if ( !READ_IF_EXISTS( pSettings, r_bool, cNameSect(),
                          "use_single_item_rule", true ) )
        return;
