...
struct SBullet {
    ...
    union {
        struct {
            ...
            //.			u16			skipped_frame	: 1
            //;			//пропуск первой отрисовки
            u16 aim_bullet : 1; // прицеленная пуля( вылетевшая первой после
                                // длительного молчания оружия (1-3 сек.))
            u16 magnetic_beam : 1;
            // магнитный луч (нет отклонения после пробивания, не падает
            // скорость после пробивания)
        };
...
private:
    ...
    struct _event {
        EventType Type;
        BOOL dynamic;
        BOOL Repeated; // последовательное повторное попадание в динамический
                       // объект
    ...
    static void CalculateNewVelocity( Fvector& dest_new_vel,
                                      Fvector const& old_velocity,
                                      float ar,
                                      float life_time );
...
protected:
    ...
    // cколько процентов энергии потеряет пуля при столкновении с материалом
...
protected:
    ...
    static BOOL test_callback( const collide::ray_defs& rd,
                               CObject* object,
                               LPVOID params );
    ...
    // попадание по любому объекту, на выходе - импульс и сила переданные пулей
    // объекту
    bool ObjectHit( SBullet_Hit* hit_res,
                    SBullet* bullet,
                    const Fvector& end_point,
                    collide::rq_result& R,
                    u16 target_material,
    ...
    void FireShotmark( SBullet* bullet,
                       const Fvector& vDir,
                       const Fvector& vEnd,
                       collide::rq_result& R,
                       u16 target_material,
    ...
    bool trajectory_check_error( Fvector& previous_position,
                                 collide::rq_results& rq_storage,
                                 SBullet& bullet,
                                 float& low,
                                 float& high,
...
public:
    ...
    void Load();
    void Clear();
    void AddBullet( const Fvector& position,
                    const Fvector& direction,
    ...
    bool ValidateHitDynamicVisualMesh( SBullet& B,
                                       collide::rq_result& R,
