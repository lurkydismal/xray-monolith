...
void action::load_animations( ::luabind::object const& table ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        MonsterSpace::EBodyState body_state =
            ( MonsterSpace::EBodyState )parse_int( tmp, "body_state" );
        MonsterSpace::EMovementType movement_type =
            ( MonsterSpace::EMovementType )parse_int( tmp, "movement_type" );
        animation_action* animation = xr_new< animation_action >(
            pos, anim_id, body_state, movement_type );
...
animation_action const& action::animation(
    ...
    Animations::const_iterator found =
        std::find_if( m_animations.begin(), m_animations.end(),
                      body_state_predicate( target_body_state ) );

    if ( found == m_animations.end() ) {
#ifndef MASTER_GOLD
        Msg( "! There is no animation which can transfer bot to body_state "
             "[%i], selecting random transition",
             target_body_state );
