...
#define TEMPLATE_SPECIALIZATION                                          \
    template <                                                           \
        ...
        template <                                                       \
            typename _builder,                                           \
            typename _allocator > class _builder_allocator_constructor > \
    template < template < typename T > class _vertex,                    \
...
TEMPLATE_SPECIALIZATION
IC typename CConstructorManagerBuilderAllocator::CGraphVertex&
CConstructorManagerBuilderAllocator::create_vertex(
    const _index_type& vertex_id ) {
    return ( inherited::create_vertex( inherited_allocator::create_vertex(),
                                       vertex_id ) );
}
