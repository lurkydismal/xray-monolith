...
#endif
    CBoneInstance& B =
        m_pKinematicsC->LL_GetBoneInstance( m_pKinematicsC->LL_GetBoneRoot() );
...
IC bool is_blending_in( CBlend& b ) {
    return b.blend_state() == CBlend::eAccrue &&
           b.blendPower - EPS > b.blendAmount;
}
...
void animation_movement_controller::InitalPositionBlending(
    const Fmatrix& to ) {
#ifdef DEBUG
    if ( dbg_draw_animation_movement_controller ) {
    ...
    /*
       ...
       blend_linear_speed*Device.fTimeDelta,
...
static void get_animation_root_position( Fmatrix& pos,
                                         IKinematics* K,
                                         IKinematicsAnimated* KA,
                                         CBlend* control_blend ) {
    VERIFY( KA );
    VERIFY( K );
...
void animation_movement_controller::animation_root_position( Fmatrix& pos ) {
    get_animation_root_position( pos, m_pKinematicsC, m_pKinematicsA,
                                 m_control_blend );
}
...
void animation_movement_controller::OnFrame() {
    ...
    // m_pKinematicsC->Bone_GetAnimPos( root_pos,
    // m_pKinematicsC->LL_GetBoneRoot( ), u8(-1), true );
    Fmatrix root_pos;
    ...
    /*
            if( IsActive() && IsBlending() )
            {
                    ...
                    struct scb : public IterateBlendsCallback, private
       xray::noncopyable
                    {
...
void animation_movement_controller::NewBlend( CBlend* B,
                                              const Fmatrix& new_matrix,
                                              bool local_animation ) {
    /*
#ifdef	DEBUG
    LPCSTR old_anim_name	= m_pKinematicsC->dcast_PKinematicsAnimated(
)->LL_MotionDefName_dbg( ControlBlend( )->motionID ).first; LPCSTR old_anim_set
= m_pKinematicsC->dcast_PKinematicsAnimated( )->LL_MotionDefName_dbg(
ControlBlend( )->motionID ).second; LPCSTR new_anim_name	=
m_pKinematicsC->dcast_PKinematicsAnimated( )->LL_MotionDefName_dbg( B->motionID
).first; LPCSTR new_anim_set		=
m_pKinematicsC->dcast_PKinematicsAnimated( )->LL_MotionDefName_dbg( B->motionID
).second;
...
and already another started anim: %s anim set: %s",
    ...
    if( !ControlBlend( )->stop_at_end )
            Msg( " ! obj movement anim  : %s anim set: %s  is not stop-at-end
but fallowed in chain by another obj movement anim: %s anim set: %s",
                    old_anim_name,old_anim_set,new_anim_name,new_anim_set
                    );
    if( !B->stop_at_end )
            Msg( " ! obj movement anim  : %s anim set: %s  is not stop-at-end
but fallowing after another obj movement anim: %s anim set: %s",
                    new_anim_name,new_anim_set,old_anim_name,old_anim_set
                    );
#endif
    //	VERIFY(  );
    ...
    */
    // CMotion* m_curr =
    // smart_cast<IKinematicsAnimated*>(m_pKinematicsC)->LL_GetRootMotion(m_control_blend->motionID);
    // CMotion* m_new =
    // smart_cast<IKinematicsAnimated*>(m_pKinematicsC)->LL_GetRootMotion(B->motionID);
    VERIFY( IsActive() );
    ...
    bool set_blending =
        !m_poses_blending.target_reached( m_control_blend->timeCurrent );
    ...
    if ( stopped ) {
        ...
        m_control_blend->timeCurrent =
            m_control_blend->timeTotal - SAMPLE_SPF; //(SAMPLE_SPF+EPS);
        Fmatrix root;
...
#ifdef DEBUG
    VERIFY( !IsActive() || inital_position_blending ||
            cmp_matrix( DBG_previous_position, m_pObjXForm, EPS, EPS ) );
#endif
}
...
void animation_movement_controller::RootBoneCallback( CBoneInstance* B ) {
    ...
    animation_movement_controller* O =
        ( animation_movement_controller* )( B->callback_param() );
...
#endif
    R_ASSERT2( _valid( B->mTransform ),
               "animation_movement_controller::RootBoneCallback" );
}
...
void animation_movement_controller::SetPosesBlending() {
    ...
    poses_blending blending(
        m_pObjXForm, Fmatrix().mul_43( m_startObjXForm, root ), blending_time );
