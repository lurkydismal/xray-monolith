...
IC void ImGui_SetNextWindowPos( Fvector2& pos ) {
    ImGui::SetNextWindowPos( *( ImVec2* )&pos, 0,
                             *( ImVec2* )&Fvector2().set( 0, 0 ) );
}

IC void ImGui_SetNextWindowPos( Fvector2& pos, ImGuiCond cond ) {
    ImGui::SetNextWindowPos( *( ImVec2* )&pos, cond,
                             *( ImVec2* )&Fvector2().set( 0, 0 ) );
}
...
IC void ImGui_SetNextWindowSizeConstraints( Fvector2& size_min,
                                            Fvector2& size_max ) {
    ImGui::SetNextWindowSizeConstraints( *( ImVec2* )&size_min,
                                         *( ImVec2* )&size_max );
}
...
IC bool ImGui_InvisibleButton( LPCSTR label ) {
    return ImGui::InvisibleButton( label, *( ImVec2* )&Fvector2().set( 80, 22 ),
                                   0 );
}
...
IC void ImGui_ProgressBar( float fraction,
                           Fvector2 size = Fvector2{ FLT_MAX, 0 },
                           LPCSTR overlay = 0 ) {
    ImGui::ProgressBar( fraction, *( ImVec2* )&size, overlay );
}

IC bool ImGui_DragFloat2( LPCSTR name,
                          Fvector2& vec,
                          float speed = 1.f,
                          float min = 0.f,
                          float max = 0.f,
    ...
    return ImGui::DragFloat2( name, ( float* )&vec, speed, min, max, format,
                              flags );
}

IC bool ImGui_DragFloat3( LPCSTR name,
                          Fvector& vec,
                          float speed = 1.f,
                          float min = 0.f,
                          float max = 0.f,
                          LPCSTR format = "%.3f",
                          ImGuiSliderFlags flags = 0 ) {
    return ImGui::DragFloat3( name, ( float* )&vec, speed, min, max, format,
                              flags );
}

IC bool ImGui_DragFloat4( LPCSTR name,
                          Fvector4& vec,
                          float speed = 1.f,
                          float min = 0.f,
                          float max = 0.f,
                          LPCSTR format = "%.3f",
                          ImGuiSliderFlags flags = 0 ) {
    return ImGui::DragFloat4( name, ( float* )&vec, speed, min, max, format,
                              flags );
}

IC bool ImGui_ColorPicker3( LPCSTR name,
                            Fcolor& color,
                            ImGuiColorEditFlags flags = 0 ) {
    return ImGui::ColorPicker3( name, ( float* )&color, flags );
}

IC bool ImGui_ColorPicker4( LPCSTR name,
                            Fcolor& color,
                            ImGuiColorEditFlags flags = 0 ) {
    return ImGui::ColorPicker4( name, ( float* )&color, flags );
}

IC bool ImGui_ColorEdit3( LPCSTR name,
                          Fcolor& color,
                          ImGuiColorEditFlags flags = 0 ) {
    return ImGui::ColorEdit3( name, ( float* )&color, flags );
}

IC bool ImGui_ColorEdit4( LPCSTR name,
                          Fcolor& color,
                          ImGuiColorEditFlags flags = 0 ) {
    return ImGui::ColorEdit4( name, ( float* )&color, flags );
}

IC bool ImGui_SliderFloat2( LPCSTR name,
                            Fvector2& vec,
                            float min = 0.f,
                            float max = 0.f,
                            LPCSTR format = "%.3f",
...
IC bool ImGui_SliderFloat3( LPCSTR name,
                            Fvector& vec,
                            float min = 0.f,
                            float max = 0.f,
                            LPCSTR format = "%.3f",
...
IC bool ImGui_SliderFloat4( LPCSTR name,
                            Fvector4& vec,
                            float min = 0.f,
                            float max = 0.f,
                            LPCSTR format = "%.3f",
...
IC LPCSTR ImGui_InputText( LPCSTR label,
                           LPCSTR text,
                           int capacity = 100,
                           ImGuiInputTextFlags flags = 0,
                           bool& changed = ugly_hack ) {
    imgui_text_buffer = ( text && xr_strlen( text ) ) ? text : "";
    changed = ImGui::InputText( label, ( char* )imgui_text_buffer.c_str(),
                                capacity, flags );
    return imgui_text_buffer.c_str();
}

IC LPCSTR ImGui_InputTextMultiline( LPCSTR label,
                                    LPCSTR text,
                                    int capacity = 100,
                                    Fvector2 size = Fvector2{ 0, 0 },
                                    ImGuiInputTextFlags flags = 0,
                                    bool& changed = ugly_hack ) {
    imgui_text_buffer = ( text && xr_strlen( text ) ) ? text : "";
    changed =
        ImGui::InputTextMultiline( label, ( char* )imgui_text_buffer.c_str(),
                                   capacity, *( ImVec2* )&size, flags );
    return imgui_text_buffer.c_str();
}

IC LPCSTR ImGui_InputTextWithHint( LPCSTR label,
                                   LPCSTR hint,
                                   LPCSTR text,
                                   int capacity = 100,
                                   ImGuiInputTextFlags flags = 0,
    ...
    changed = ImGui::InputTextWithHint(
        label, hint, ( char* )imgui_text_buffer.c_str(), capacity, flags );
    return imgui_text_buffer.c_str();
}
...
IC bool ImGui_InputFloat2( LPCSTR name,
                           Fvector2& vec,
                           LPCSTR format = "%.3f",
                           ImGuiInputTextFlags flags = 0 ) {
...
IC bool ImGui_InputFloat3( LPCSTR name,
                           Fvector& vec,
                           LPCSTR format = "%.3f",
                           ImGuiInputTextFlags flags = 0 ) {
...
IC bool ImGui_InputFloat4( LPCSTR name,
                           Fvector4& vec,
                           LPCSTR format = "%.3f",
                           ImGuiInputTextFlags flags = 0 ) {
...
IC bool ImGui_ColorButton( LPCSTR name,
                           Fcolor& color,
                           ImGuiColorEditFlags flags = 0,
                           Fvector2 size = Fvector2{ 0, 0 } ) {
    return ImGui::ColorButton( name, *( ImVec4* )&color, flags,
                               *( ImVec2* )&size );
}

IC bool ImGui_Selectable( LPCSTR name,
                          bool selected,
...
IC bool ImGui_BeginTable( LPCSTR name,
                          int colums,
...
IC void ImGui_TableSetBgColor( ImGuiTableBgTarget target,
                               Fcolor& color,
                               int column_n = -1 ) {
...
IC Fvector2 ImGui_CalcTextSize( LPCSTR text,
                                bool hide_after_double_hash = false,
                                float wrap_width = -1.f ) {
    return *( Fvector2* )&ImGui::CalcTextSize( text, 0, hide_after_double_hash,
                                               wrap_width );
}

IC bool ImGui_IsMouseHoveringRect( Fvector2 min, Fvector2 max ) {
    return ImGui::IsMouseHoveringRect( *( ImVec2* )&min, *( ImVec2* )&max,
                                       true );
}
...
IC Fvector2 ImGui_GetMouseDragDelta( ImGuiMouseButton button = 0,
                                     float lock_treshold = -1.f ) {
