...
extern void setup_location_types_line(
    GameGraph::TERRAIN_VECTOR& m_vertex_types,
    LPCSTR string );

CSE_ALifeItemWeapon* CSE_ALifeOnlineOfflineGroup::tpfGetBestWeapon(
...
bool CSE_ALifeOnlineOfflineGroup::need_update(
...
void CSE_ALifeOnlineOfflineGroup::register_member(
    ALife::_OBJECT_ID member_id ) {
    ...
    VERIFY( ( monster->m_group_id == 0xffff ) ||
...
void CSE_ALifeOnlineOfflineGroup::unregister_member(
    ALife::_OBJECT_ID member_id ) {
...
CSE_ALifeOnlineOfflineGroup::MEMBER* CSE_ALifeOnlineOfflineGroup::member(
    ALife::_OBJECT_ID member_id,
    bool no_assert ) {
    MEMBERS::iterator I = m_members.find( member_id );
    if ( I == m_members.end() ) {
        if ( !no_assert )
            Msg( "! There is no member with id %d in the OnlineOfflineGroup id "
                 "%d",
                 member_id, ID );
        VERIFY( no_assert );
        return ( 0 );
...
void CSE_ALifeOnlineOfflineGroup::try_switch_online() {
    ...
    MEMBERS::iterator E = m_members.end();
    for ( ; I != E; ++I ) {
        VERIFY3( ( *I ).second->g_Alive(),
                 "Incorrect situation : some of the OnlineOffline group "
                 "members is dead",
                 ( *I ).second->name_replace() );
        VERIFY3(
            ( *I ).second->can_switch_online(),
            "Incorrect situation : some of the OnlineOffline group members "
            "cannot be switched online due to their personal properties",
            ( *I ).second->name_replace() );
        VERIFY3(
            ( *I ).second->can_switch_offline(),
            "Incorrect situation : some of the OnlineOffline group members "
            "cannot be switched online due to their personal properties",
            ( *I ).second->name_replace() );
        if ( alife().graph().actor()->o_Position.distance_to(
                 ( *I ).second->o_Position ) > alife().online_distance() ) {
            continue;
        }
        inherited1::try_switch_online();
        return;
...
void CSE_ALifeOnlineOfflineGroup::try_switch_offline() {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( alife().graph().actor()->o_Position.distance_to(
                 ( *I ).second->o_Position ) <= alife().offline_distance() )
            return;
    }
...
void CSE_ALifeOnlineOfflineGroup::on_after_game_load() {
    ...
    ALife::_OBJECT_ID* temp = ( ALife::_OBJECT_ID* )_alloca(
        m_members.size() * sizeof( ALife::_OBJECT_ID ) );
...
CSE_ALifeOnlineOfflineGroup::MEMBERS const&
CSE_ALifeOnlineOfflineGroup::squad_members() const {
    return m_members;
}
...
void CSE_ALifeOnlineOfflineGroup::force_change_position( Fvector position ) {
    ...
    GameGraph::_GRAPH_ID new_graph_vertex =
        ai().cross_table().vertex( new_level_vertex ).game_vertex_id();
    o_Position = position;
