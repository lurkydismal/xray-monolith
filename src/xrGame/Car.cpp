...
CCar::CCar() {
    ...
    active_camera = 0;
    camera[ ectFirst ] = xr_new< CCameraFirstEye >(
    ...
    camera[ ectChase ] =
        xr_new< CCameraLook >( this, CCameraBase::flRelativeLink );
...
#ifdef DEBUG
    VERIFY2( fsimilar( DET( B->mTransform ), 1.f, DET_CHECK_EPS ),
             "Bones receive returns 0 matrix" );
#endif
...
void CCar::Load( LPCSTR section ) {
    ...
    {
        LPCSTR str =
            READ_IF_EXISTS( pSettings, r_string, section, "type", nullptr );
        m_type = eCarTypeDef;
        if ( str && strlen( str ) ) {
            if ( strcmp( str, "def" ) == 0 ) {
                m_type = eCarTypeDef;
    ...
    m_on_before_hit_callback = READ_IF_EXISTS( pSettings, r_string, section,
                                               "on_before_hit", nullptr );
    m_on_before_use_callback = READ_IF_EXISTS( pSettings, r_string, section,
                                               "on_before_use", nullptr );
    m_on_before_engine_callback = READ_IF_EXISTS( pSettings, r_string, section,
                                                  "on_before_engine", nullptr );
    m_on_key_board_callback =
        READ_IF_EXISTS( pSettings, r_string, section, "on_key_board", nullptr );
...
BOOL CCar::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( pUserData->section_exist( "visual_memory_definition" ) ) {
        ...
        m_memory->reload(
            pUserData->r_string( "visual_memory_definition", "section" ) );
    }
    ...
    m_camera_bone_def =
        ini->line_exist( cfg, "camera_bone_def" )
            ? K->LL_BoneID( ini->r_string( cfg, "camera_bone_def" ) )
            : BI_NONE;
    m_camera_bone_aim =
        ini->line_exist( cfg, "camera_bone_aim" )
            ? K->LL_BoneID( ini->r_string( cfg, "camera_bone_aim" ) )
            : BI_NONE;
    m_zoom_factor_def =
        READ_IF_EXISTS( ini, r_float, cfg, "zoom_factor_def", 1.0F );
    m_zoom_factor_aim =
        READ_IF_EXISTS( ini, r_float, cfg, "zoom_factor_aim", 1.0F );
    ...
    m_remote_control =
        !!READ_IF_EXISTS( ini, r_bool, cfg, "remote_control", FALSE );
...
void CCar::ActorObstacleCallback( bool& do_colide,
                                  bool bo1,
                                  dContact& c,
                                  SGameMtl* material_1,
                                  SGameMtl* material_2 ) {
...
void CCar::SpawnInitPhysics( CSE_Abstract* D ) {
    ...
    K->CalculateBones( TRUE );
    Init();
    // inits m_driving_wheels,m_steering_wheels,m_breaking_wheels values using
    // recieved in ParceDefinitions & from bone_map
...
void CCar::RestoreNetState( CSE_PHSkeleton* po ) {
    ...
    {
        xr_map< u16, SDoor >::iterator i, e;
        xr_vector< CSE_ALifeCar::SDoorState >::iterator ii =
            co->door_states.begin();
        i = m_doors.begin();
        e = m_doors.end();
        for ( ; i != e; ++i, ++ii ) {
    ...
    {
        xr_map< u16, SWheel >::iterator i, e;
        xr_vector< CSE_ALifeCar::SWheelState >::iterator ii =
            co->wheel_states.begin();
        i = m_wheels_map.begin();
        e = m_wheels_map.end();
        for ( ; i != e; ++i, ++ii ) {
            i->second.RestoreNetState( *ii );
    ...
    ////////////////////////////////////////////////////////////////////
            {
                    ...
                    CPHActivationShape activation_shape;//Fvector
    start_box;m_PhysicMovementControl.Box().getsize(start_box);
...
void CCar::shedule_Update( u32 dt ) {
    ...
    if ( CDelayedActionFuse::isActive() &&
         CDelayedActionFuse::Update( GetfHealth() ) ) {
        // CarExplode();
    }
    if ( b_exploded && !m_explosion_flags.test( flExploding ) &&
         !getEnabled() ) //! m_bExploding
        setEnabled( TRUE );
#ifdef DEBUG
    DbgSheduleUpdate();
#endif
...
#else
        if ( eacFirstEye == active_camera->tag &&
...
void CCar::UpdateCL() {
    ...
    if ( m_car_weapon ) {
        ...
        if ( m_memory )
            m_memory->set_camera( m_car_weapon->ViewCameraPos(),
                                  m_car_weapon->ViewCameraDir(),
...
void CCar::OnHUDDraw( CCustomHUD* hud, IDSGraphManager* DM ) {
    ...
    UI().Font().pFontStat->OutNext( "Position:      [%3.2f, %3.2f, %3.2f]",
                                    VPUSH( Position() ) );
    UI().Font().pFontStat->OutNext( "Velocity:      [%3.2f]",
                                    velocity.magnitude() );

#endif
}

// void CCar::Hit(float P,Fvector &dir,CObject * who,s16 element,Fvector
...
void CCar::Hit( SHit* pHDS ) {
    ...
    if ( m_on_before_hit_callback && strlen( m_on_before_hit_callback ) ) {
        ...
        if ( ai().script_engine().functor( m_on_before_hit_callback,
                                           lua_function ) ) {
    ...
    if ( ai().script_engine().functor( "_G.CCar__OnHit", hitFunct ) ) {
        ...
        CScriptHit tLuaHit( &HDS );
        hitFunct( lua_game_object(),
                  whoGO ? whoGO->lua_game_object() : ( CScriptGameObject* )0,
                  &tLuaHit, HDS.boneID );
    }
...
void CCar::PHHit( SHit& H ) {
    ...
    if ( CPHUpdateObject::IsActive() ) {
        ...
        if ( !fis_zero( mag ) ) {
            ...
            m_pPhysicsShell->applyHit( H.bone_space_position(), vimpulse, mag,
                                       H.bone(), H.type() );
        }
    } else {
...
bool CCar::attach_Actor( CGameObject* actor ) {
    ...
    if ( m_type == eCarTypeDef ) {
        ...
        if ( ini->line_exist( "car_definition", "driver_place" ) )
            id = K->LL_BoneID(
                ini->r_string( "car_definition", "driver_place" ) );
        else {
            Owner()->setVisible( 0 );
            id = K->LL_GetBoneRoot();
        }
...
bool CCar::Enter( const Fvector& pos,
                  const Fvector& dir,
...
void CCar::ParseDefinitions() {
    ...
    R_ASSERT2( ini,
               "Car has no description !!! See ActorEditor Object - UserData" );
    ...
    fill_wheel_vector( READ_IF_EXISTS( ini, r_string, "car_definition",
                       ...
                       m_driving_wheels );
    fill_wheel_vector( READ_IF_EXISTS( ini, r_string, "car_definition",
                                       "steering_wheels", nullptr ),
                       m_steering_wheels );
    fill_wheel_vector( READ_IF_EXISTS( ini, r_string, "car_definition",
                                       "breaking_wheels", nullptr ),
                       m_breaking_wheels );
    fill_exhaust_vector(
        READ_IF_EXISTS( ini, r_string, "car_definition", "exhausts", nullptr ),
        m_exhausts );
    fill_doors_map(
        READ_IF_EXISTS( ini, r_string, "car_definition", "doors", nullptr ),
        m_doors );
#else
    fill_wheel_vector( ini->r_string( "car_definition", "driving_wheels" ),
                       m_driving_wheels );
    fill_wheel_vector( ini->r_string( "car_definition", "steering_wheels" ),
                       m_steering_wheels );
    fill_wheel_vector( ini->r_string( "car_definition", "breaking_wheels" ),
                       m_breaking_wheels );
    fill_exhaust_vector( ini->r_string( "car_definition", "exhausts" ),
                         m_exhausts );
    ...
    m_power_increment_factor =
        READ_IF_EXISTS( ini, r_float, "car_definition",
                        "power_increment_factor", m_power_increment_factor );
    m_rpm_increment_factor =
        READ_IF_EXISTS( ini, r_float, "car_definition", "rpm_increment_factor",
                        m_rpm_increment_factor );
    m_power_decrement_factor =
        READ_IF_EXISTS( ini, r_float, "car_definition",
                        "power_decrement_factor", m_power_increment_factor );
    m_rpm_decrement_factor =
        READ_IF_EXISTS( ini, r_float, "car_definition", "rpm_decrement_factor",
                        m_rpm_increment_factor );
    m_power_neutral_factor =
        READ_IF_EXISTS( ini, r_float, "car_definition", "power_neutral_factor",
                        m_power_neutral_factor );
    R_ASSERT2( m_power_neutral_factor > 0.1f && m_power_neutral_factor < 1.f,
               "power_neutral_factor must be 0 - 1 !!" );
    if ( ini->line_exist( "car_definition", "exhaust_particles" ) ) {
        m_exhaust_particles =
            ini->r_string( "car_definition", "exhaust_particles" );
    }

    b_auto_switch_transmission =
        !!ini->r_bool( "car_definition", "auto_transmission" );
    ...
    R_ASSERT2( ini->section_exist( "transmission_gear_ratio" ),
               "no section transmission_gear_ratio" );
    m_gear_ratious.push_back(
        ini->r_fvector3( "transmission_gear_ratio", "R" ) );
    ...
    for ( int i = 1; true; ++i ) {
        ...
        if ( !ini->line_exist( "transmission_gear_ratio", rat_num ) )
            break;
        Fvector gear_rat =
            ini->r_fvector3( "transmission_gear_ratio", rat_num );
    ...
    if ( ini->section_exist( "animations" ) ) {
        m_driver_anim_type =
            ini->r_u16( "animations", "driver_animation_type" );
    }
...
void CCar::Init() {
    ...
    if ( ini->line_exist( "car_definition", "steer" ) ) {
        m_bone_steer = pKinematics->LL_BoneID(
            ini->r_string( "car_definition", "steer" ) );
        VERIFY2( fsimilar( DET( pKinematics->LL_GetTransform( m_bone_steer ) ),
                           1.f, EPS_L ),
                 "BBADD MTX" );
        pKinematics->LL_GetBoneInstance( m_bone_steer )
            .set_callback( bctPhysics, cb_Steer, this );
    }
    m_steer_angle = 0.f;
    // ref_wheel.Init();
    m_ref_radius = ini->r_float( "car_definition",
                                 "reference_radius" ); // ref_wheel.radius;
    b_exploded = false;
    b_engine_on = false;
    b_clutch = false;
    b_starting = false;
    ...
    m_root_transform.set( bone_map.find( pKinematics->LL_GetBoneRoot() )
                              ->second.element->mXFORM );
    m_current_transmission_num = 0;
    m_pPhysicsShell->set_DynamicScales( 1.f, 1.f );
    CDamagableItem::Init( GetfHealth(), 3 );
    float l_time_to_explosion = READ_IF_EXISTS( ini, r_float, "car_definition",
                                                "time_to_explosion", 120.f );
    CDelayedActionFuse::Initialize( l_time_to_explosion,
                                    CDamagableItem::DamageLevelToHealth( 2 ) );
    {
        xr_map< u16, SWheel >::iterator i, e;
        i = m_wheels_map.begin();
        e = m_wheels_map.end();
    ...
    if ( ini->section_exist( "damage_items" ) ) {
        ...
        for ( CInifile::SectCIt I = data.Data.begin(); I != data.Data.end();
              I++ ) {
            ...
            if ( i != m_wheels_map.end() )
                i->second.CDamagableHealthItem::Init(
                    float( atof( *item.second ) ), 2 );
            else {
                xr_map< u16, SDoor >::iterator i = m_doors.find( index );
                R_ASSERT3( i != m_doors.end(),
                           "only wheel and doors bones allowed for damage defs",
                           *item.first );
                i->second.CDamagableHealthItem::Init(
                    float( atof( *item.second ) ), 1 );
            }
        }
    }
...
void CCar::Revert() {
    m_pPhysicsShell->applyForce(
        0, 1.5f * EffectiveGravity() * m_pPhysicsShell->getMass(), 0 );
}
...
void CCar::UpdatePower() {
    ...
    if ( b_auto_switch_transmission && !b_transmission_switching ) {
        VERIFY2( CurrentTransmission() < m_gear_ratious.size(),
                 "wrong transmission" );
...
void CCar::TransmissionUp() {
    ...
    transmission > max_transmition_num ? transmission = max_transmition_num
                                       : transmission;
    Transmission( transmission );
}
...
bool CCar::Use( const Fvector& pos,
                const Fvector& dir,
                const Fvector& foot_pos ) {
#ifdef CAR_NEW
    ...
    collide::ray_defs Q(
        ...
        collide::rqtObject ); // CDB::OPT_ONLYFIRST CDB::OPT_ONLYNEAREST
...
void CCar::InitParabola() {
    ...
    // m_c = m_max_power* (3.f*m_power_rpm
    // - 4.f*m_torque_rpm)/(m_power_rpm-m_torque_rpm)/2.f/m_power_rpm; m_a =
    // -m_max_power/(m_power_rpm-m_torque_rpm)/m_power_rpm/m_power_rpm/2.f; m_b
    // =
    // m_max_power*m_torque_rpm/(m_power_rpm-m_torque_rpm)/m_power_rpm/m_power_rpm;

    m_a = expf( ( m_power_rpm - m_torque_rpm ) / ( 2.f * m_power_rpm ) ) *
          m_max_power / m_power_rpm;
    m_b = m_torque_rpm;
...
float CCar::EnginePower() {
    ...
    if ( value > m_current_engine_power )
        return value * m_power_increment_factor +
               m_current_engine_power * ( 1.f - m_power_increment_factor );
    else
        return value * m_power_decrement_factor +
               m_current_engine_power * ( 1.f - m_power_decrement_factor );
}
...
float CCar::EngineDriveSpeed() {
    ...
    if ( calc_rpm > m_current_rpm )
        return ( 1.f - m_rpm_increment_factor ) * m_current_rpm +
               m_rpm_increment_factor * calc_rpm;
    else
        return ( 1.f - m_rpm_decrement_factor ) * m_current_rpm +
               m_rpm_decrement_factor * calc_rpm;

    // if(drive_speed<phInfinity) return
    // dFabs(drive_speed*m_current_gear_ratio); else
    // return 0.f;
}
...
void CCar::UpdateFuel( float time_delta ) {
    ...
    if ( m_current_rpm > m_min_rpm )
        m_fuel -=
            time_delta * ( m_current_rpm - m_min_rpm ) * m_fuel_consumption;
    else
...
void CCar::OnEvent( NET_Packet& P, u16 type ) {
    ...
    switch ( type ) {
        case GE_OWNERSHIP_TAKE: {
            ...
            if ( GetInventory()->CanTakeItem(
                     smart_cast< CInventoryItem* >( O ) ) ) {
                O->H_SetParent( this );
        ...
        case GE_OWNERSHIP_REJECT: {
            ...
            GetInventory()->DropItem( smart_cast< CGameObject* >( O ),
                                      just_before_destroy,
                                      just_before_destroy );
...
void CCar::PhDataUpdate( float step ) {
    ...
    m_steer_angle = m_steering_wheels.begin()->GetSteerAngle() * 0.1f +
                    m_steer_angle * 0.9f;
...
// void CCar::object_contactCallbackFun(bool& do_colide,dContact& c,SGameMtl *
// ,SGameMtl * )
//{
//
...
IC void CCar::fill_wheel_vector( LPCSTR S, xr_vector< T >& type_wheels ) {
    ...
    for ( int i = 0; i < count; ++i ) {
        ...
        if ( J == bone_map.end() ) {
            ...
            SWheel& wheel =
                ( m_wheels_map.insert( mk_pair( bone_id, SWheel( this ) ) ) )
                    .first->second;
...
float CCar::RefWheelCurTorque() {
    if ( b_transmission_switching )
        return 0.f;
    return EngineCurTorque() * ( ( m_current_gear_ratio < 0.f )
                                     ? -m_current_gear_ratio
                                     : m_current_gear_ratio );
}
...
void CCar::SetExplodeTime( u32 et ) {
    CDelayedActionFuse::Initialize( float( et ) / 1000.f,
                                    CDamagableItem::DamageLevelToHealth( 2 ) );
}
