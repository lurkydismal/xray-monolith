...
IC bool CTradeParameters::enabled( _action_type type,
                                   const shared_str& section ) const {
...
template < typename _action_type >
IC const CTradeFactors& CTradeParameters::factors(
    _action_type type,
    const shared_str& section ) const {
...
template < typename _action_type >
IC void CTradeParameters::process( _action_type type,
                                   CInifile& ini_file,
                                   const shared_str& section ) {
    R_ASSERT2( ini_file.section_exist( section ),
               make_string( "cannot find section %s", *section ) );
    ...
    for ( ; I != E; ++I ) {
        ...
        LPCSTR param2 = _GetItemCount( *( *I ).second ) >= 2
                            ? _GetItem( *( *I ).second, 1, temp1 )
                            : param1;

        _action.enable(
            ( *I ).first,
            CTradeFactors( ( float )atof( param1 ), ( float )atof( param2 ) ) );
    }
}

template < typename _action_type >
IC void CTradeParameters::default_factors(
    _action_type type,
    const CTradeFactors& trade_factors ) {
