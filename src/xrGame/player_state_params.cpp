...
u32 const player_death_counter::get_u32_param() {
    ...
    u32 death_count = tmp_local_player
                          ? static_cast< u32 >( tmp_local_player->m_iDeaths )
                          : u32( -1 );
    return death_count;
}
...
player_artdeliver_counter::player_artdeliver_counter(
...
u32 const player_artdeliver_counter::get_u32_param() {
    game_PlayerState* tmp_local_player = m_owner->get_local_player();
    u32 arts_count =
        tmp_local_player ? static_cast< u32 >( tmp_local_player->af_count ) : 0;
    return arts_count;
}
...
u32 const player_rawkill_counter::get_u32_param() {
    // u32 rawkill_count = m_local_player ? m_local_player->m_iKillsInRowCurr :
    // 0;
    return m_raw_kills;
}
...
u32 const player_state_move::get_u32_param() {
    ...
    return state.bCrouch | ( state.bSprint << 1 ) | ( state.bJump << 2 ) |
           ( state.bFall << 3 );
}
...
player_state_ang_velocity::player_state_ang_velocity(
    game_state_accumulator* owner )
    : inherited( owner ) {}
