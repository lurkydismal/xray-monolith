...
void description::load_loopholes( shared_str const& table_id ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        smart_cover::loophole* loophole =
            xr_new< smart_cover::loophole >( table );
        VERIFY( std::find_if( m_loopholes.begin(), m_loopholes.end(),
                              id_predicate( *loophole ) ) ==
                m_loopholes.end() );
        m_loopholes.push_back( loophole );
    }

    VERIFY2( !m_loopholes.empty(),
             make_string( "smart_cover [%s] doesn't have loopholes",
                          m_table_id.c_str() ) );
    VERIFY2( std::find_if( m_loopholes.begin(), m_loopholes.end(),
                           usable_predicate() ) != m_loopholes.end(),
             make_string( "smart_cover [%s] doesn't have usable loopholes",
                          m_table_id.c_str() ) );
}
...
void description::process_loopholes() {
    ...
    for ( ; I != E; ++I ) {
        ::loophole* current = *I;
        current->enterable( m_transitions.edge( transform_vertex( "", true ),
                                                current->id() ) != 0 );
        current->exitable( m_transitions.edge(
                               current->id(), transform_vertex( "", false ) ) !=
                           0 );
    }

    VERIFY2( std::find_if( m_loopholes.begin(), m_loopholes.end(),
                           enterable_predicate() ) != m_loopholes.end(),
             make_string( "smart_cover [%s] doesn't have enterable loopholes",
                          m_table_id.c_str() ) );

    VERIFY2( std::find_if( m_loopholes.begin(), m_loopholes.end(),
                           exitable_predicate() ) != m_loopholes.end(),
             make_string( "smart_cover [%s] doesn't have exitable loopholes",
                          m_table_id.c_str() ) );
}
...
void description::load_transitions( shared_str const& table_id ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        TransitionGraph::CEdge* edge =
            m_transitions.edge( vertex_0_id, vertex_1_id );
        load_actions( table, edge->data() );
    }
}

void description::load_actions( ::luabind::object const& table,
                                description::ActionsList& result ) {
...
IC void delete_data( const CGraphAbstract< _data_type,
                                           _edge_weight_type,
                                           _vertex_id_type,
                                           _edge_data_type >& graph_ ) {
    typedef CGraphAbstract< _data_type, _edge_weight_type, _vertex_id_type,
                            _edge_data_type >
        Graph;
    ...
    for ( typename Vertices::iterator vi = verts.begin(); vi != verts.end();
          ++vi ) {
        typename Graph::CVertex* vert = ( *vi ).second;
        delete_data( vert->data() );

        Edges& edges = const_cast< Edges& >( vert->edges() );
        for ( typename Edges::iterator ei = edges.begin(); ei != edges.end();
              ++ei ) {
...
loophole const* description::loophole( shared_str const& loophole_id ) const {
    ...
    Loopholes::const_iterator found = std::find_if(
        m_loopholes.begin(), m_loopholes.end(), id_predicate( loophole_id ) );
