...
void xrClientsPool::Clear() {
    for ( dclients_t::iterator i = m_dclients.begin(), ie = m_dclients.end();
          i != ie; ++i ) {
...
bool const xrClientsPool::expired_client_deleter::operator()(
    dclient& right ) const {
...
void xrClientsPool::ClearExpiredClients() {
    ...
    tmp_deleter.m_expire_time =
        g_sv_Client_Reconnect_Time * 60 * 1000; // in minutes
    m_dclients.erase(
        std::remove_if( m_dclients.begin(), m_dclients.end(), tmp_deleter ),
        m_dclients.end() );
}
...
bool const xrClientsPool::pooled_client_finder::operator()(
    dclient const& right ) const {
    ...
    if ( !xr_strcmp( m_new_client->ps->getName(),
                     right.m_client->ps->getName() ) )
        return true;
...
xrClientData* xrClientsPool::Get( xrClientData* new_client ) {
    ...
    dclients_t::iterator tmp_iter =
        std::find_if( m_dclients.begin(), m_dclients.end(), tmp_finder );
