...
public:
    inline sight_manager_enable_guard( CSightManager& manager,
                                       bool const value )
        : m_manager( manager ), m_value( value ) {}
...
void stalker_movement_manager_base::reload( LPCSTR section ) {
    ...
    m_velocities = &stalker_velocity_holder().collection(
        pSettings->r_string( section, "movement_speeds" ) );
    init_velocity_masks();
}
...
void stalker_movement_manager_base::init_velocity_masks() {
    ...
    add_velocity(
        ...
        m_velocities->velocity( eMentalStateFree, eBodyStateStand,
                                eMovementTypeWalk, eMovementDirectionForward ),
        PI * 100.f / 180.f, cf * PI * 100.f / 180.f );
    add_velocity(
        eVelocityRunFreePositive,
        m_velocities->velocity( eMentalStateFree, eBodyStateStand,
                                eMovementTypeRun, eMovementDirectionForward ),
        PI_DIV_8 / 2, cf * PI_DIV_8 / 2 );
    add_velocity(
        eVelocityWalkDangerStandPositive,
        m_velocities->velocity( eMentalStateDanger, eBodyStateStand,
                                eMovementTypeWalk, eMovementDirectionForward ),
        100 * PI, cf * PI );
    add_velocity(
        eVelocityWalkDangerCrouchPositive,
        m_velocities->velocity( eMentalStateDanger, eBodyStateCrouch,
                                eMovementTypeWalk, eMovementDirectionForward ),
        100 * PI, cf * PI_DIV_2 );
    add_velocity(
    ...
    add_velocity(
        ...
        m_velocities->velocity( eMentalStatePanic, eBodyStateStand,
                                eMovementTypeRun, eMovementDirectionForward ),
        PI_DIV_8 / 2, cf * PI_DIV_8 / 2 );
...
IC void stalker_movement_manager_base::setup_head_speed(
    stalker_movement_params& movement_params ) {
...
IC void stalker_movement_manager_base::add_velocity( int mask,
                                                     float linear,
                                                     float compute_angular,
                                                     float angular ) {
    detail().add_velocity( mask, CDetailPathManager::STravelParams(
                                     linear, compute_angular, angular ) );
}

IC float stalker_movement_manager_base::path_direction_angle() {
    if ( !path().empty() &&
         ( path().size() > detail().curr_travel_point_index() + 1 ) ) {
        Fvector t;
...
void stalker_movement_manager_base::set_desired_position(
    const Fvector* position ) {
    VERIFY2( !position || accessible( *position ) ||
...
bool stalker_movement_manager_base::script_control() {
    ...
    object().m_fCurSpeed =
        object().GetCurrentAction()->m_tMovementAction.m_fSpeed;
    set_desirable_speed( object().m_fCurSpeed );
    return ( true );
}

void stalker_movement_manager_base::setup_movement_params(
    stalker_movement_params& movement_params ) {
    ...
    if ( movement_params.desired_position() ) {
        if ( !restrictions().accessible(
                 *movement_params.desired_position() ) ) {
            Fvector temp;
            const u32 nearest_vertex_id = restrictions().accessible_nearest(
                *movement_params.desired_position(), temp );
            if ( !ai().level_graph().valid_vertex_id( nearest_vertex_id ) ) {
                Msg( "! [stalker_movement_manager_base::setup_movement_params] "
                     "cannot find an accessible "
        ...
        if ( ( movement_params.m_path_type !=
               MovementManager::ePathTypePatrolPath ) &&
             ( movement_params.m_path_type !=
               MovementManager::ePathTypeGamePath ) &&
             ( movement_params.m_path_type !=
               MovementManager::ePathTypeNoPath ) ) {
            ...
            if ( !ai().level_graph().valid_vertex_id( dest_vertex_id ) ) {
                ...
                if ( object().brain().initialized() ) {
                    LPCSTR current_action_name =
                        object().brain().current_action().m_action_name;
                    action_name =
                        current_action_name ? current_action_name : "<unnamed>";
                }

                const u32 current_vertex_id =
                    object().ai_location().level_vertex_id();
                Msg( "! [stalker_movement_manager_base::setup_movement_params] "
                     "invalid destination vertex [%u], "
                     "object [%s], action [%s], current vertex [%u]",
                     dest_vertex_id, object().cName().c_str(), action_name,
                     current_vertex_id );

                if ( ai().level_graph().valid_vertex_id( current_vertex_id ) ) {
                    set_nearest_accessible_position( object().Position(),
                                                     current_vertex_id );
                ...
                if ( !ai().level_graph().valid_vertex_id( dest_vertex_id ) ) {
                    Msg( "! "
                         "[stalker_movement_manager_base::setup_movement_"
                         "params] cannot recover destination "
            ...
            if ( !restrictions().accessible( dest_vertex_id ) ) {
                Fvector temp;
                const u32 nearest_vertex_id = restrictions().accessible_nearest(
                    ai().level_graph().vertex_position( dest_vertex_id ),
                    temp );
                if ( !ai().level_graph().valid_vertex_id(
                         nearest_vertex_id ) ) {
                    Msg( "! "
                         "[stalker_movement_manager_base::setup_movement_"
                         "params] cannot find an accessible "
                         "vertex near destination [%u] for object [%s], "
                         "forcing standstill",
                         dest_vertex_id, object().cName().c_str() );
                    movement_params.m_movement_type = eMovementTypeStand;
                } else {
                    level_path().set_dest_vertex( nearest_vertex_id );
                    detail().set_dest_position( temp );
                ...
                Fvector vertex_position =
                    ai().level_graph().vertex_position( dest_vertex_id );
                VERIFY2( restrictions().accessible( vertex_position ) ||
                             show_restrictions( &restrictions() ),
                         make_string(
                             "vertex_id[%d],position[%f][%f][%f],object[%s]",
                             dest_vertex_id, VPUSH( vertex_position ),
                             *object().cName() ) );
...
void stalker_movement_manager_base::setup_velocities(
    stalker_movement_params& movement_params ) {
    ...
    switch ( movement_params.m_movement_type ) {
        ...
        default: {
            ...
            velocity_mask &= u32( -1 ) ^ ( eVelocityNegativeVelocity |
                                           eVelocityPositiveVelocity );
        }
    }
    ...
    if ( velocity_mask & eVelocityDanger ) {
        ...
        detail().set_velocity_mask( velocity_mask | eVelocityWalk |
                                    eVelocityStanding );
    }
}

void stalker_movement_manager_base::parse_velocity_mask(
    stalker_movement_params& movement_params ) {
    if ( path().empty() ||
         ( detail().curr_travel_point_index() != m_last_turn_index ) )
        m_last_turn_index = u32( -1 );

    sight_manager_enable_guard guard( object().sight(), true );

    if ( ( movement_params.m_movement_type == eMovementTypeStand ) ||
         path().empty() ||
         ( path().size() <= detail().curr_travel_point_index() ) ||
         path_completed() || !actual() ) {
    ...
    DetailPathManager::STravelPathPoint point =
        path()[ detail().curr_travel_point_index() ];
    CDetailPathManager::STravelParams current_velocity =
        detail().velocity( point.velocity );
    ...
    if ( fis_zero( current_velocity.linear_velocity ) ) {
        ...
        if ( ( movement_params.m_mental_state != eMentalStateFree ) ||
             //				(object().sight().current_action().sight_type()
             //!= SightManager::eSightTypePathDirection) ||
        ...
        if ( movement_params.m_mental_state != eMentalStateDanger ) {
            if ( movement_params.m_mental_state == eMentalStatePanic ) {
                ...
                if ( !fis_zero( path_direction_angle(),
                                path_velocity.real_angular_velocity ) ) {
                    setup_body_orientation();
                    guard.enable( false );
    ...
    switch ( point.velocity & eVelocityMentalState ) {
        case eVelocityFree: {
#ifdef DEBUG
            if ( m_object->brain().current_action_id() ==
                 StalkerDecisionSpace::eWorldOperatorCombatPlanner ) {
                CStalkerCombatPlanner& planner =
                    smart_cast< CStalkerCombatPlanner& >(
                        m_object->brain().current_action() );
                if ( planner.current_action_id() !=
                     StalkerDecisionSpace::eWorldOperatorKillWoundedEnemy )
                    Msg( "~ stalker %s is doing bad thing (action %s)",
                         *m_object->cName(),
    ...
    VERIFY2( ( movement_params.m_mental_state != eMentalStateFree ) ||
                 movement_params.m_body_state != eBodyStateCrouch,
             *object().cName() );
...
void stalker_movement_manager_base::set_nearest_accessible_position(
    ...
    if ( !ai().level_graph().valid_vertex_id( level_vertex_id ) ) {
        Msg( "! "
             "[stalker_movement_manager_base::set_nearest_accessible_position] "
             "invalid source vertex [%u] "
             "for object [%s]",
             level_vertex_id, object().cName().c_str() );
        return;
    }
    ...
    if ( !ai().level_graph().inside( level_vertex_id, desired_position ) )
        ...
        desired_position.y = ai().level_graph().vertex_plane_y(
            level_vertex_id, desired_position.x, desired_position.z );

    if ( !restrictions().accessible( desired_position ) ) {
        level_vertex_id = restrictions().accessible_nearest(
            Fvector().set( desired_position ), desired_position );
        if ( !ai().level_graph().valid_vertex_id( level_vertex_id ) ) {
            Msg( "! "
                 "[stalker_movement_manager_base::set_nearest_accessible_"
                 "position] cannot find an accessible "
                 "vertex for object [%s]",
                 object().cName().c_str() );
            return;
        }
    ...
    VERIFY2( restrictions().accessible( level_vertex_id ) ||
                 show_restrictions( &restrictions() ),
             *object().cName() );
    set_level_dest_vertex( level_vertex_id );

    VERIFY2( restrictions().accessible( desired_position ) ||
                 show_restrictions( &restrictions() ),
             *object().cName() );
...
void stalker_movement_manager_base::update( u32 time_delta ) {
    VERIFY( ( m_target.m_mental_state != eMentalStateFree ) ||
            ( m_target.m_body_state != eBodyStateCrouch ) );
...
void stalker_movement_manager_base::update(
    stalker_movement_params& movement_params ) {
...
void stalker_movement_manager_base::on_travel_point_change(
    const u32& previous_travel_point_index ) {
...
void stalker_movement_manager_base::on_restrictions_change() {
    ...
    if ( m_current.desired_position() &&
         !restrictions().accessible( *m_current.desired_position() ) )
        set_nearest_accessible_position();
}

float stalker_movement_manager_base::speed(
    const EMovementDirection& movement_direction ) {
    VERIFY( movement_type() != eMovementTypeStand );

    return ( m_velocities->velocity( mental_state(), body_state(),
                                     movement_type(), movement_direction ) );
}

void stalker_movement_manager_base::setup_speed_from_animation(
    const float& speed ) {
...
bool stalker_movement_manager_base::is_object_on_the_way(
...
IC float distance_to_line( const Fvector& p0,
                           const Fvector& p1,
                           const Fvector& p2 ) {
    if ( p0.similar( p2 ) )
        return ( 0.f );
...
void stalker_movement_manager_base::update_object_on_the_way(
    const CGameObject* object,
    const float& distance ) {
    if ( !actual() )
        return;
    ...
    for ( ; I != E; ++I ) {
        if ( distance_to_line( ( *( I - 1 ) ).position, ( *I ).position,
                               position ) < 1.f ) {
        ...
        current_distance +=
            ( *( I - 1 ) ).position.distance_to( ( *I ).position );
...
void stalker_movement_manager_base::check_for_bad_path(
    stalker_movement_params& movement_params ) {
    ...
    if ( detail().completed( object().Position(),
    ...
    float distance =
        path[ point_index + 1 ].position.distance_to( object().Position() );
    Fvector current_direction = Fvector().sub( path[ point_index + 1 ].position,
                                               path[ point_index ].position );
...
void stalker_movement_manager_base::set_level_dest_vertex(
