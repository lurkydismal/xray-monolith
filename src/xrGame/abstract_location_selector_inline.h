...
IC bool CSelectorTemplate::actual( const _vertex_id_type start_vertex_id,
                                   bool path_completed ) {
    if ( !used() ||
         ( ( ( m_last_query_time + m_query_interval ) > Device.dwTimeGlobal ) &&
           !path_completed ) )
        return ( true );
...
TEMPLATE_SPECIALIZATION
IC void CSelectorTemplate::select_location(
    const _vertex_id_type start_vertex_id,
    bool path_completed ) {
    if ( used() && ( ( ( m_last_query_time + m_query_interval ) <=
                       Device.dwTimeGlobal ) ||
                     path_completed ) ) {
        perform_search( start_vertex_id );
        if ( !failed() && dest_vertex_id )
...
IC void CSelectorTemplate::perform_search( const _vertex_id_type vertex_id ) {
    ...
    ai().graph_engine().search( *m_graph, start_vertex_id, start_vertex_id, 0,
                                *m_evaluator );
