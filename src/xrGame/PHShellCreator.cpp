...
void CPHShellSimpleCreator::CreatePhysicsShell() {
    ...
    if ( owner->m_pPhysicsShell->get_ElementsNumber() == 0 ) {
        Msg( " ! Error: world item visual [%s] has no elements!",
             pKinematics->getDebugName().c_str() );
    } else if ( !owner->m_pPhysicsShell->get_ElementByStoreOrder( 0 )
                     ->has_geoms() ) {
        Msg( " ! Error: world item visual [%s] has no shape!",
             pKinematics->getDebugName().c_str() );
    }
