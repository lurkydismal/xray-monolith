...
IC void CStalkerAnimationManager::play_script_impl() {
    ...
    script().play( m_skeleton_animated, script_play_callback,
...
IC void CStalkerAnimationManager::play_global_impl(
    ...
    bool const& animation_movement_controller ) {
    torso().reset();
    legs().reset();

    global().animation( animation );
    global().play( m_skeleton_animated, global_play_callback,
                   animation_movement_controller, true, false,
...
IC void CStalkerAnimationManager::play_global_impl(
    ...
    global().animation( animation );
    global().play( m_skeleton_animated, global_play_callback, false, false,
                   false );
}
#endif // USE_HEAD_BONE_PART_FAKE
...
bool CStalkerAnimationManager::play_global() {
    ...
    const MotionID& global_animation =
        assign_global_animation( animation_movement_controller );
    if ( !global_animation ) {
        clear_unsafe_callbacks();
        global().reset();
        return ( false );
...
void CStalkerAnimationManager::play_legs() {
    ...
    if ( !first_time && !result && legs().blend() ) {
        ...
        m_previous_speed =
            ( m_target_speed - m_previous_speed ) * amount + m_previous_speed;
    }

    legs().play( m_skeleton_animated, legs_play_callback, false, false,
    ...
    try {
        ...
        Msg( "! error in stalker [%s], profile [%s] with visual [%s]",
             ...
             object()
                 ...
                 ->CharacterInfo()
                 .GetSpecificCharacterId()
                 .c_str(),
             object().cNameVisual().c_str() );

        for ( auto& invItem :
              m_object->cast_inventory_owner()->m_inventory->m_all ) {
            Msg( "# Inventory item: [%s] - %s", invItem->m_section_id.c_str(),
                 invItem->m_name.c_str() );
        }
