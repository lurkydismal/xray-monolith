...
void SProjectorLight::Load( const CInifile* ini, LPCSTR sec ) {
    if ( ini->line_exist( sec, "color_animator" ) ) {
        SetLanim(
            READ_IF_EXISTS( ini, r_string, sec, "color_animator", nullptr ) );
    }
    ...
    SetColor( READ_IF_EXISTS( ini, r_fcolor, sec, "color",
                              Fcolor().set( 0.0F, 0.0F, 0.0F, 0.0F ) ) );
    SetRange( READ_IF_EXISTS( ini, r_float, sec, "range", 0.0F ) );
    SetCone(
        deg2rad( READ_IF_EXISTS( ini, r_float, sec, "spot_angle", 0.0F ) ) );
    ...
    SetGlowTexture(
        READ_IF_EXISTS( ini, r_string, sec, "glow_texture", nullptr ) );

    IKinematics* K = O->Visual()->dcast_PKinematics();
    m_light_bid = ini->line_exist( sec, "light_bone" )
                      ? K->LL_BoneID( ini->r_string( sec, "light_bone" ) )
                      : BI_NONE;
    VERIFY( m_light_bid != BI_NONE );
    m_guide_bid = ini->line_exist( sec, "guide_bone" )
                      ? K->LL_BoneID( ini->r_string( sec, "guide_bone" ) )
                      : BI_NONE;
...
void SProjectorLight::UpdateCL() {
    ...
    Fmatrix xfm = Fmatrix().mul_43(
        O->XFORM(),
        O->Visual()->dcast_PKinematics()->LL_GetTransform( m_light_bid ) );
...
void SProjectorLight::Switch( bool val ) {
    if ( m_light->get_active() != val ) {
        ...
        if ( m_guide_bid != BI_NONE ) {
            O->Visual()->dcast_PKinematics()->LL_SetBoneVisible(
                m_guide_bid, ( val ) ? TRUE : FALSE, TRUE );
        }
    }
}
...
void SProjectorControl::Load( const CInifile* ini, LPCSTR sec ) {
    ...
    m_rotate_x_speed =
        READ_IF_EXISTS( ini, r_float, sec, "rotate_x_speed", 10.0F );
    m_rotate_y_speed =
        READ_IF_EXISTS( ini, r_float, sec, "rotate_y_speed", 10.0F );
...
void SProjectorControl::SetBoneCallbacks( bool enable ) {
    if ( enable ) {
        CBoneInstance& biX = smart_cast< IKinematics* >( O->Visual() )
                                 ->LL_GetBoneInstance( m_rotate_x_bone );
        biX.set_callback( bctCustom, BoneCallbackX, this );
        CBoneInstance& biY = smart_cast< IKinematics* >( O->Visual() )
                                 ->LL_GetBoneInstance( m_rotate_y_bone );
        biY.set_callback( bctCustom, BoneCallbackY, this );
    } else {
        CBoneInstance& biX = smart_cast< IKinematics* >( O->Visual() )
                                 ->LL_GetBoneInstance( m_rotate_x_bone );
        biX.set_callback( bctPhysics, O->PPhysicsShell()->GetBonesCallback(),
                          O->PPhysicsShell()->get_Element( m_rotate_x_bone ) );
        CBoneInstance& biY = smart_cast< IKinematics* >( O->Visual() )
                                 ->LL_GetBoneInstance( m_rotate_y_bone );
        biY.set_callback( bctPhysics, O->PPhysicsShell()->GetBonesCallback(),
                          O->PPhysicsShell()->get_Element( m_rotate_y_bone ) );
    }
}

void SProjectorControl::ClampRotationHorz( float& tgt_val,
                                           const float& cur_val,
                                           const float& lim_min,
                                           const float& lim_max ) {
...
void _BCL SProjectorControl::BoneCallbackX( CBoneInstance* B ) {
    SProjectorControl* P =
        static_cast< SProjectorControl* >( B->callback_param() );
    ...
    switch ( id ) {
        case CProjector::eDesiredPos:
            Fvector vec =
                Fmatrix()
                    .mul_43( O->XFORM(),
                             O->Visual()->dcast_PKinematics()->LL_GetTransform(
                                 m_rotate_y_bone ) )
                    .c;
