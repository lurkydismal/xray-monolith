...
void game_cl_mp::AddMessageMenu( LPCSTR menu_section,
                                 LPCSTR snd_path,
                                 LPCSTR team_prefix ) {
    if ( !menu_section )
        return;
    ...
    for ( u32 i = 0; i < 10; i++ ) {
        ...
        for ( u32 s = 1; s <= 16; s++ ) {
            ...
            xr_sprintf( FileName_Voice, "%s%s%d\\voice_%s%d", snd_path,
                        team_prefix, 1, SoundName, s );
            ...
            for ( int t = 1; t <= GetTeamCount(); t++ ) {
                xr_sprintf( FileName_Voice, "%s%s%d\\voice_%s%d", snd_path,
                            team_prefix, t, SoundName, s );
                xr_sprintf( FileName_Radio, "%s%s%d\\radio_%s%d", snd_path,
                            team_prefix, t, SoundName, s );
                if ( FS.exist( fn, "$game_sounds$", FileName_Voice, ".ogg" ) &&
...
void game_cl_mp::LoadMessagesMenu( LPCSTR menus_section ) {
    ...
    if ( !pSettings->section_exist( menus_section ) )
        return;
    shared_str Sounds_Path =
        pSettings->r_string( menus_section, "sounds_path" );
    shared_str Team_Prefix =
        READ_IF_EXISTS( pSettings, r_string, menus_section, "team_prefix", "" );
    ...
    for ( int i = 0; i < 10; i++ ) {
        ...
        if ( !pSettings->line_exist( menus_section, *LineName ) )
            break;
        shared_str menu_section =
            pSettings->r_string( menus_section, *LineName );
...
void game_cl_mp::OnMessageSelected( CUISpeechMenu* pMenu, u8 PhraseID ) {
    ...
    if ( m_aMessageMenus.empty() )
        return;
    MESSAGEMENUS_it it =
        std::find( m_aMessageMenus.begin(), m_aMessageMenus.end(), pMenu );
    ...
    u8 VariantID =
        ( pMMessage->aVariants.size() <= 1 )
            ? 0
            : u8( ::Random.randI( pMMessage->aVariants.size() ) & 0xff );
...
void game_cl_mp::OnSpeechMessage( NET_Packet& P ) {
    ...
    if ( ps->team == local_player->team ) {
        ...
        if ( CurrentGameUI() )
            CurrentGameUI()->m_pMessagesWnd->AddChatMessage(
                st.translate( pMMessage->pMessage.c_str() ).c_str(),
                ps->getName() );

        if ( !Level().MapManager().HasMapLocation( FRIEND_RADION_LOCATION,
                                                   ps->GameID ) ) {
            ( Level().MapManager().AddMapLocation( FRIEND_RADION_LOCATION,
                                                   ps->GameID ) )
                ->EnablePointer();
        }
    }
    ...
    if ( ps->team == local_player->team ) {
        if ( ps == local_player ) {
            pMSound->mSound_Voice.play_at_pos( NULL, Fvector().set( 0, 0, 0 ),
                                               sm_2D, 0 );
        } else {
            pMSound->mSound_Radio.play_at_pos( NULL, Fvector().set( 0, 0, 0 ),
                                               sm_2D, 0 );
        }
        Msg( "%s said: %s", ps->getName(),
             *CStringTable().translate( pMMessage->pMessage ) );
    } else {
