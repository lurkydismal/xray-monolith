...
void CUIZoneMap::Init() {
    ...
    m_clock_wnd =
        UIHelper::CreateStatic( uiXml, "minimap:clock_wnd", &m_background );

    BOOL bRotate =
        uiXml.ReadAttribInt( "minimap:level_frame", 0, "rotate", TRUE );
    BOOL bRounded =
        uiXml.ReadAttribInt( "minimap:level_frame", 0, "rounded", TRUE );
    BOOL bAspect =
        uiXml.ReadAttribInt( "minimap:level_frame", 0, "aspect", TRUE );
    int ratioMode =
        uiXml.ReadAttribInt( "minimap:level_frame", 0, "ratio_mode", 0 );
    ...
    if ( IsGameTypeSingle() ) {
        ...
        xml_init.InitTextWnd( uiXml, "minimap:static_counter:text_static", 0,
                              &m_Counter_text );
...
void CUIZoneMap::OnSectorChanged( int sector ) {
    ...
    xr_sprintf( sub_texture, "%s#%d", m_activeMap->m_texture.c_str(),
                m_current_map_idx );

    if ( map_idx == u8( -1 ) )
        xr_sprintf( sub_texture, "%s", m_activeMap->m_texture.c_str() );

    m_activeMap->InitTextureEx( sub_texture,
                                m_activeMap->m_shader_name.c_str() );
}
