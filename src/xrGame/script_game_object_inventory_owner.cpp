...
bool CScriptGameObject::GiveInfoPortion( LPCSTR info_id ) {
    CInventoryOwner* pInventoryOwner =
        smart_cast< CInventoryOwner* >( &object() );
    if ( !pInventoryOwner )
        return false;
...
void _AddIconedTalkMessage( LPCSTR caption,
                            LPCSTR text,
                            LPCSTR texture_name,
                            LPCSTR templ_name );

void CScriptGameObject::AddIconedTalkMessage( LPCSTR caption,
                                              LPCSTR text,
                                              LPCSTR texture_name,
                                              LPCSTR templ_name ) {
...
void _AddIconedTalkMessage( LPCSTR caption,
    ...
    if ( pGameSP->TalkMenu->IsShown() ) {
        pGameSP->TalkMenu->AddIconedMessage(
            caption, text, texture_name,
            templ_name ? templ_name : "iconed_answer_item" );
    }
}

void _give_news( LPCSTR caption,
                 LPCSTR news,
                 LPCSTR texture_name,
                 int delay,
                 int show_time,
...
void CScriptGameObject::GiveGameNews( LPCSTR caption,
                                      LPCSTR news,
                                      LPCSTR texture_name,
                                      int delay,
                                      int show_time ) {
...
void CScriptGameObject::ForEachInventoryItems(
    ...
    if ( !owner ) {
        ai().script_engine().script_log(
            ...
            "CScriptGameObject::ForEachInventoryItems non-CInventoryOwner "
            "object !!!" );
        return;
    }
...
// 1
void CScriptGameObject::IterateInventory( ::luabind::functor< bool > functor,
                                          ::luabind::object object ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &this->object() );
    if ( !inventory_owner ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::IterateInventory "
                                         "non-CInventoryOwner object !!!" );
        return;
    }
...
void CScriptGameObject::IterateRuck( ::luabind::functor< bool > functor,
                                     ::luabind::object object ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &this->object() );
    if ( !inventory_owner ) {
...
void CScriptGameObject::IterateBelt( ::luabind::functor< bool > functor,
                                     ::luabind::object object ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &this->object() );
    if ( !inventory_owner ) {
...
void CScriptGameObject::IterateInventoryBox( ::luabind::functor< bool > functor,
                                             ::luabind::object object ) {
    CInventoryBox* inventory_box =
        smart_cast< CInventoryBox* >( &this->object() );
    if ( !inventory_box ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
    ...
    for ( ; I != E; ++I ) {
        CGameObject* GO =
            smart_cast< CGameObject* >( Level().Objects.net_Find( *I ) );
        if ( GO )
...
void CScriptGameObject::MarkItemDropped( CScriptGameObject* item, bool flag ) {
    ...
    if ( !inventory_owner ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::MarkItemDropped "
                                         "non-CInventoryOwner object !!!" );
        return;
    }

    CInventoryItem* inventory_item =
        smart_cast< CInventoryItem* >( &item->object() );
    if ( !inventory_item ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::MarkItemDropped "
                                         "non-CInventoryItem object !!!" );
        return;
    }
...
void CScriptGameObject::UnloadMagazine( bool bKeepAmmo ) {
    ...
    CWeaponMagazined* weapon_magazined =
        smart_cast< CWeaponMagazined* >( &object() );
    if ( !weapon_magazined ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::UnloadMagazine "
                                         "non-CWeaponMagazined object !!!" );
        return;
    }

    CAI_Stalker* stalker =
        smart_cast< CAI_Stalker* >( weapon_magazined->H_Parent() );
...
void CScriptGameObject::SetCanBeHarmed( bool state ) {
    ...
    if ( !ent ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CEntityAlive [%s]: cannot access class member "
                         "set_can_be_harmed!",
...
bool CScriptGameObject::CanBeHarmed() {
    ...
    if ( !ent ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CEntityAlive [%s]: cannot access class member can_be_harmed!",
                object().cNameSect().c_str() )
                .c_str() );
        return false;
    }
...
void CScriptGameObject::DropItemAndTeleport( CScriptGameObject* pItem,
                                             Fvector position ) {
    DropItem( pItem );
...
void CScriptGameObject::MoveItemToRuck( CScriptGameObject* pItem ) {
    ...
    if ( !owner ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::MoveItemToRuck "
                                         "non-CInventoryOwner object !!!" );
        return;
    }
...
void CScriptGameObject::MoveItemToSlot( CScriptGameObject* pItem,
                                        u16 slot_id,
                                        bool doNotActivate ) {
    CInventoryOwner* owner = smart_cast< CInventoryOwner* >( &object() );
    CInventoryItem* item = smart_cast< CInventoryItem* >( &pItem->object() );
    if ( !owner ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::MoveItemToSlot "
                                         "non-CInventoryOwner object !!!" );
        return;
    }
...
void CScriptGameObject::MoveItemToBelt( CScriptGameObject* pItem ) {
    ...
    if ( !owner ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::MoveItemToBelt "
                                         "non-CInventoryOwner object !!!" );
        return;
    }
...
void CScriptGameObject::ItemAllowTrade( CScriptGameObject* pItem ) {
    ...
    if ( !owner || !item ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CScriptGameObject::ItemAllowTrade "
                                         "non-CInventoryOwner object !!!" );
        return;
    }
    item->AllowTrade();
...
void CScriptGameObject::TransferItem( CScriptGameObject* pItem,
                                      CScriptGameObject* pForWho ) {
    if ( !pItem || !pForWho ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "cannot transfer NULL item" );
        return;
    }
    ...
    if ( !pIItem ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "Cannot transfer not CInventoryItem item" );
        return;
    }
...
void CScriptGameObject::TakeItem( CScriptGameObject* pItem ) {
    if ( !pItem ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "!CScriptGameObject::TakeItem | cannot take NULL item" );
        return;
    }
    ...
    if ( !pIItem ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "!CScriptGameObject::TakeItem | Cannot take not CInventoryItem "
            "item" );
        return;
    }

    // In case of an existing parent, transfer item as usual
    // probably doesn't work on NPC for now and needs fixing if needed in the
    // future
    if ( pIItem->object().H_Parent() ) {
        const CInventoryOwner* inventory_owner =
            smart_cast< CInventoryOwner* >( pIItem->object().H_Parent() );
        const CInventoryBox* inventory_box =
            smart_cast< CInventoryBox* >( pIItem->object().H_Parent() );
        const CGameObject* parentGO =
            smart_cast< CGameObject* >( pIItem->object().H_Parent() );
        if ( ( inventory_owner || inventory_box ) && parentGO ) {
            ...
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                "!CScriptGameObject::TakeItem | Unknown parent type found?" );
        }

        return; // added return here just in case parent isn't identified as
                // inventory owner or a box
    }
...
IC void GiveMoneySafe( CInventoryOwner* owner, int money, bool bSendEvent ) {
    ...
    if ( money > 0 ) {
        ...
        if ( std::numeric_limits< u32 >::max() - current_money < u_money )
            owner->set_money( std::numeric_limits< u32 >::max(),
                              bSendEvent ); // Cap at maximum
        else
            owner->set_money( current_money + u_money, bSendEvent );
    } else if ( money < 0 ) {
        // Safe conversion of negative int to unsigned, guarding against INT_MIN
        // trap
        u32 u_deduction =
            static_cast< u32 >( std::abs( static_cast< long long >( money ) ) );
...
void CScriptGameObject::TransferMoney( int money, CScriptGameObject* pForWho ) {
    if ( !pForWho ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "cannot transfer money for NULL object" );
        return;
    }
    CInventoryOwner* pOurOwner = smart_cast< CInventoryOwner* >( &object() );
    VERIFY( pOurOwner );
    CInventoryOwner* pOtherOwner =
        smart_cast< CInventoryOwner* >( &pForWho->object() );
    VERIFY( pOtherOwner );

    if ( pOurOwner->get_money() < money ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "Character does not have enought money" );
        return;
    }

    // Leave negative money transfer possibility intact, unknown how it is used
    // in 3rd party
...
int CScriptGameObject::GetGoodwill( CScriptGameObject* pToWho ) {
    ...
    return RELATION_REGISTRY().GetGoodwill( pInventoryOwner->object_id(),
                                            pToWho->object().ID() );
}

void CScriptGameObject::SetGoodwill( int goodwill,
                                     CScriptGameObject* pWhoToSet ) {
    CInventoryOwner* pInventoryOwner =
        smart_cast< CInventoryOwner* >( &object() );
    ...
    RELATION_REGISTRY().SetGoodwill( pInventoryOwner->object_id(),
                                     pWhoToSet->object().ID(), goodwill );
}

void CScriptGameObject::ForceSetGoodwill( int goodwill,
                                          CScriptGameObject* pWhoToSet ) {
    CInventoryOwner* pInventoryOwner =
        smart_cast< CInventoryOwner* >( &object() );
    ...
    RELATION_REGISTRY().ForceSetGoodwill( pInventoryOwner->object_id(),
                                          pWhoToSet->object().ID(), goodwill );
}

void CScriptGameObject::ChangeGoodwill( int delta_goodwill,
                                        CScriptGameObject* pWhoToSet ) {
    CInventoryOwner* pInventoryOwner =
        smart_cast< CInventoryOwner* >( &object() );
    ...
    RELATION_REGISTRY().ChangeGoodwill( pInventoryOwner->object_id(),
                                        pWhoToSet->object().ID(),
                                        delta_goodwill );
}
...
void CScriptGameObject::SetRelation( ALife::ERelationType relation,
                                     CScriptGameObject* pWhoToSet ) {
    CInventoryOwner* pInventoryOwner =
        smart_cast< CInventoryOwner* >( &object() );
    ...
    CInventoryOwner* pOthersInventoryOwner =
        smart_cast< CInventoryOwner* >( &pWhoToSet->object() );
    ...
    RELATION_REGISTRY().SetRelationType( pInventoryOwner, pOthersInventoryOwner,
                                         relation );
}
...
int CScriptGameObject::GetCommunityGoodwill_obj( LPCSTR community ) {
    ...
    return RELATION_REGISTRY().GetCommunityGoodwill(
        c.index(), pInventoryOwner->object_id() );
}

void CScriptGameObject::SetCommunityGoodwill_obj( LPCSTR community,
                                                  int goodwill ) {
    CInventoryOwner* pInventoryOwner =
        smart_cast< CInventoryOwner* >( &object() );
    ...
    RELATION_REGISTRY().SetCommunityGoodwill(
        c.index(), pInventoryOwner->object_id(), goodwill );
}
...
int CScriptGameObject::GetAttitude( CScriptGameObject* pToWho ) {
    ...
    CInventoryOwner* pOthersInventoryOwner =
        smart_cast< CInventoryOwner* >( &pToWho->object() );
    VERIFY( pOthersInventoryOwner );
    return RELATION_REGISTRY().GetAttitude( pInventoryOwner,
                                            pOthersInventoryOwner );
}
...
int CScriptGameObject::CharacterRank() {
    ...
    if ( !monster ) {
        ...
        if ( !pInventoryOwner ) {
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                "CharacterRank available only for InventoryOwner and "
                "BaseMonster" );
            return 0;
        }
...
void CScriptGameObject::SetCharacterCommunity( LPCSTR comm,
    ...
    if ( community.index() < 0 ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "SetCharacterCommunity failed for '%'",
                                         comm );
        return;
    }
...
void CScriptGameObject::RunTalkDialog( CScriptGameObject* pToWho,
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "RunTalkDialog applicable only for actor" );
        return;
    }

    CInventoryOwner* pPartner =
        smart_cast< CInventoryOwner* >( &pToWho->object() );
    VERIFY( pPartner );
...
void construct_restriction_vector( shared_str restrictions,
                                   xr_vector< ALife::_OBJECT_ID >& result ) {
    result.clear();
    string64 temp;
    u32 n = _GetItemCount( *restrictions );
    for ( u32 i = 0; i < n; ++i ) {
        CObject* object = Level().Objects.FindObjectByName(
...
void CScriptGameObject::add_restrictions( LPCSTR out, LPCSTR in ) {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CRestrictedObject [%s]: cannot access class member "
                         "add_restrictions!",
                         object().cNameSect().c_str() )
                .c_str() );
        return;
...
bool CScriptGameObject::accessible_vertex_id( u32 level_vertex_id ) {
    ...
    THROW2( ai().level_graph().valid_vertex_id( level_vertex_id ),
            "Cannot check if level vertex id is accessible, because it is "
            "invalid" );
    return ( monster->movement().restrictions().accessible( level_vertex_id ) );
}

u32 CScriptGameObject::accessible_nearest( const Fvector& position,
                                           Fvector& result ) {
    CCustomMonster* monster = smart_cast< CCustomMonster* >( &object() );
    if ( !monster ) {
        ai().script_engine().script_log(
    ...
    if ( monster->movement().restrictions().accessible( position ) ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CRestrictedObject : you use accessible_nearest when position is "
            "already accessible!" );
        return ( u32( -1 ) );
    }
    return ( monster->movement().restrictions().accessible_nearest( position,
                                                                    result ) );
}

void CScriptGameObject::enable_attachable_item( bool value ) {
    CAttachableItem* attachable_item =
        smart_cast< CAttachableItem* >( &object() );
    if ( !attachable_item ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CAttachableItem [%s]: cannot access class member "
...
void CScriptGameObject::night_vision_allowed( bool value ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member enable_night_vision!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
void CScriptGameObject::enable_torch( bool value ) {
    ...
    if ( !torch ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CTorch [%s]: cannot access class member enable_torch!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
bool CScriptGameObject::torch_enabled() const {
    ...
    if ( !torch ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CTorch [%s]: cannot access class member torch_enabled!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::attachable_item_load_attach( LPCSTR section ) {
    ...
    if ( attachable_item->object().H_Parent() ) {
        // reattach
        CAttachmentOwner* AO = smart_cast< CAttachmentOwner* >(
            attachable_item->object().H_Parent() );
        if ( AO )
            AO->reattach_items();
    }
}
...
#ifdef DEBUG
    ai().script_engine().script_log(
        eLuaMessageTypeMessage, "CScriptGameObject::RestoreWeapon called!!!" );
...
#ifdef DEBUG
    ai().script_engine().script_log(
        eLuaMessageTypeMessage, "CScriptGameObject::HideWeapon called!!!" );
    ai().script_engine().print_stack();
#endif // #ifdef DEBUG
...
int CScriptGameObject::Weapon_GrenadeLauncher_Status() {
    ...
    if ( !weapon ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CWeapon [%s]: cannot access class member "
...
int CScriptGameObject::Weapon_Scope_Status() {
    ...
    if ( !weapon ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CWeapon [%s]: cannot access class member Weapon_Scope_Status!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
bool CScriptGameObject::Weapon_IsGrenadeLauncherAttached() {
    ...
    if ( !weapon ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CWeapon [%s]: cannot access class member "
                         "Weapon_IsGrenadeLauncherAttached!",
                         object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
int CScriptGameObject::animation_slot() const {
    ...
    if ( !hud_item ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CHudItem [%s]: cannot access class member animation_slot!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( u32( -1 ) );
    }
...
CScriptGameObject* CScriptGameObject::active_device() const {
    ...
    if ( !inventory_owner ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CInventoryOwner [%s]: cannot access class member "
                         "active_detector!",
                         object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    ...
    CInventoryItem* result =
        inventory_owner->inventory().ItemFromSlot( DETECTOR_SLOT );
    if ( result ) {
...
void CScriptGameObject::show_device( bool bFast ) {
    ...
    if ( !inventory_owner ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CInventoryOwner [%s]: cannot access class member show_device!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
void CScriptGameObject::hide_device( bool bFast ) {
    ...
    if ( !inventory_owner ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CInventoryOwner [%s]: cannot access class member hide_device!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
CScriptGameObject* CScriptGameObject::item_in_slot( u32 slot_id ) const {
    ...
    CInventoryItem* result =
        slot_id != NO_ACTIVE_SLOT
            ? inventory_owner->inventory().ItemFromSlot( ( u16 )slot_id )
            : ( 0 );
    return ( result ? result->object().lua_game_object() : 0 );
}

void CScriptGameObject::GiveTaskToActor( CGameTask* t,
                                         u32 dt,
                                         bool bCheckExisting,
                                         u32 t_timer ) {
...
u32 CScriptGameObject::active_slot() {
    ...
    if ( !inventory_owner ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CInventoryOwner [%s]: cannot access class member active_slot!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
void CScriptGameObject::enable_movement( bool enable ) {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CCustomMonster [%s]: cannot access class member "
                         "movement_enabled!",
                         object().cNameSect().c_str() )
                .c_str() );
        return;
...
bool CScriptGameObject::can_throw_grenades() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CAI_Stalker [%s]: cannot access class member "
                         "can_throw_grenades!",
                         object().cNameSect().c_str() )
                .c_str() );
        return ( false );
...
void CScriptGameObject::group_throw_time_interval( u32 throw_time_interval ) {
    ...
    stalker->agent_manager().member().throw_time_interval(
        throw_time_interval );
}
...
void CScriptGameObject::aim_time( CScriptGameObject* weapon, u32 aim_time ) {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member aim_time!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
void CScriptGameObject::set_aim_params( float max_angle,
                                        float min_angle,
                                        float min_speed,
                                        float predict_time ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
...
bool CScriptGameObject::can_kill_enemy() {
    ...
    // Callable from script at any time; the engine's own readers
    // (CObjectActionFire) only run with a weapon out. No active item = no fire
    // point, so no shot clearance to compute.
...
void CScriptGameObject::aim_bone_id( LPCSTR bone_id ) {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member aim_bone_id!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
// Force-plant enemy as a visible-memory object on this stalker
// (CMemoryManager:: make_object_visible_somewhen - the same call the engine
// uses when distributing wounded targets across a squad,
// agent_enemy_manager.cpp). Enemy selection scores a currently seen enemy far
// above hit/sound memory, so an enemy injected only via hit memory never wins
// selection; this puts him in the seen class, where the engine's nearest-seen
// logic takes over.
void CScriptGameObject::make_enemy_visible( CScriptGameObject* enemy ) {
    ...
    const CEntityAlive* entity =
        smart_cast< const CEntityAlive* >( &enemy->object() );
    if ( !entity ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CAI_Stalker : make_enemy_visible : "
                                         "enemy is not an alive entity!" );
        return;
    }
...
CCoverPoint const* CScriptGameObject::find_best_cover(
    Fvector position_to_cover_from ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member find_best_cover!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ...
            make_string( "CAI_Stalker [%s]: cannot access class member "
                         ...
                         this->object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }

    smart_cover::object const* const smart_object =
        smart_cast< smart_cover::object const* >( &object->object() );
    if ( !smart_object ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CAI_Stalker : suitable_smart_cover: "
                                         "passed non-smart_cover object!" );
        return ( false );
    }
...
void CScriptGameObject::register_door() {
    // VERIFY2(!m_door, make_string("object %s has been registered as a door
    // already", m_game_object->cName().c_str()));
    if ( !m_door )
        m_door = ai().doors().register_door(
            *smart_cast< CPhysicObject* >( m_game_object ) );
    //	Msg
    //( "registering door 0x%-08x", m_door );
}

void CScriptGameObject::unregister_door() {
    // VERIFY2(m_door, make_string("object %s is not a door",
    // m_game_object->cName().c_str())); 	Msg
    //( "UNregistering door 0x%-08x", m_door );
    if ( m_door ) {
...
void CScriptGameObject::on_door_is_open() {
    // VERIFY2(m_door, make_string("object %s hasn't been registered as a door
    // already", m_game_object->cName().c_str()));
    if ( m_door )
...
// Alundaio: Methods for exporting the ability to detach/attach addons for
// magazined weapons
...
void CScriptGameObject::Weapon_AddonAttach( CScriptGameObject* item ) {
    ...
    if ( !weapon ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CWeaponMagazined [%s]: cannot access class member "
                         "Weapon_AddonAttach!",
                         object().cNameSect().c_str() )
                .c_str() );
        return;
...
bool CScriptGameObject::InstallUpgrade( LPCSTR upgrade ) {
    ...
    if ( !item ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CInventoryItem [%s]: cannot access class member "
                         "InstallUpgrade!",
                         object().cNameSect().c_str() )
                .c_str() );
        return false;
    ...
    return ai().alife().inventory_upgrade_manager().upgrade_install(
        *item, upgrade, false );
}
...
bool CScriptGameObject::HasUpgrade( LPCSTR upgrade ) {
    ...
    if ( !item ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CInventoryItem [%s]: cannot access class member HasUpgrade!",
                object().cNameSect().c_str() )
                .c_str() );
        return false;
    }
...
void CScriptGameObject::IterateInstalledUpgrades(
    const ::luabind::functor< bool >& functor ) {
    CInventoryItem* Item = smart_cast< CInventoryItem* >( &object() );
    if ( !Item )
        return;
...
CScriptGameObject* CScriptGameObject::ItemOnBelt( u32 item_id ) const {
    ...
    if ( belt->size() < item_id ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "item_on_belt: item id outside belt!" );
        return ( 0 );
    }
...
bool CScriptGameObject::IsOnBelt( CScriptGameObject* obj ) const {
    CInventoryItem* inventory_item =
        smart_cast< CInventoryItem* >( &( obj->object() ) );
    if ( !inventory_item ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CInventoryItem [%s]: cannot access class member is_on_belt!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
    ...
    if ( !inventory_owner ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CInventoryOwner [%s]: cannot access class member is_on_belt!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
float CScriptGameObject::GetActorMaxWeight() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member GetActorMaxWeight!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetActorMaxWeight( float max_weight ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member SetActorMaxWeight!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
float CScriptGameObject::GetActorMaxWalkWeight() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CActor [%s]: cannot access class member "
...
float CScriptGameObject::GetAdditionalMaxWeight() const {
    ...
    if ( !outfit && !pBackpack ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CCustomOutfit [%s]: cannot access class member "
...
float CScriptGameObject::Weight() const {
    ...
    if ( !inventory_item ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CSciptEntity [%s]: cannot access class member Weight!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetWeight( float w ) {
    ...
    if ( !inventory_item ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CSciptEntity [%s]: cannot access class member SetWeight!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
float CScriptGameObject::GetActorJumpSpeed() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member GetActorJumpSpeed!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetActorJumpSpeed( float jump_speed ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member SetActorJumpSpeed!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
    pActor->m_fJumpSpeed = jump_speed;
    pActor->character_physics_support()->movement()->SetJumpUpVelocity(
        pActor->m_fJumpSpeed );
}
...
float CScriptGameObject::GetActorWalkAccel() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member GetActorWalkAccel!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetActorWalkAccel( float val ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member SetActorWalkAccel!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
float CScriptGameObject::GetActorWalkBackCoef() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member GetActorWalkBackCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetActorWalkBackCoef( float val ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member SetActorWalkBackCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
float CScriptGameObject::GetActorLookoutCoef() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member GetActorLookoutCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetActorLookoutCoef( float val ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member SetActorLookoutCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
float CScriptGameObject::GetActorCrouchCoef() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member GetActorCrouchCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetActorCrouchCoef( float val ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member SetActorCrouchCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
float CScriptGameObject::GetActorClimbCoef() const {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member GetActorClimbCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::SetActorClimbCoef( float val ) {
    ...
    if ( !pActor ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CActor [%s]: cannot access class member SetActorClimbCoef!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
