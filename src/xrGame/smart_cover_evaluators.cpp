...
in_cover_evaluator::in_cover_evaluator( CAI_Stalker* object,
...
cover_actual_evaluator::cover_actual_evaluator( CAI_Stalker* object,
                                                LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
cover_entered_evaluator::cover_entered_evaluator( CAI_Stalker* object,
                                                  LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type loophole_actual_evaluator::evaluate() {
    ...
    if ( object().movement().current_params().cover_loophole() !=
         object().movement().target_params().cover_loophole() )
        return ( false );
...
_value_type loophole_hit_long_ago_evaluator::evaluate() {
    return ( ( m_object->time_object_hit() + m_time_to_wait ) <
             Device.dwTimeGlobal );
}
...
_value_type is_action_available_evaluator::evaluate() {
    ...
    return ( m_object->m_object->movement()
                 .current_params()
                 .cover_loophole()
                 ->is_action_available( m_action_id ) );
}
...
loophole_exitable_evaluator::loophole_exitable_evaluator(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type loophole_exitable_evaluator::evaluate() {
    ...
    return (
        object().movement().current_params().cover_loophole()->exitable() );
}
...
can_exit_loophole_with_animation::can_exit_loophole_with_animation(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
#ifdef DEBUG
        Msg( "transition guard(cover): [%s] -> [%s]",
...
#ifdef DEBUG
        Msg( "transition guard(loophole): [%s] -> [%s]",
...
default_behaviour_evaluator::default_behaviour_evaluator(
    animation_planner* object,
    LPCSTR evaluator_name )
    : inherited( object, evaluator_name ) {}

_value_type default_behaviour_evaluator::evaluate() {
    return ( m_object->m_object->movement().default_behaviour() ||
...
can_fire_at_enemy_evaluator::can_fire_at_enemy_evaluator(
    animation_planner* object,
    LPCSTR evaluator_name )
    : inherited( object, evaluator_name ) {}
