...
void CCar::SDoor::Init() {
    ...
    if ( !joint )
        return;
    // R_ASSERT2(dJointGetType(joint->GetDJoint())==dJointTypeHinge,"Wrong door
    // joint!!! Only simple joint valid for a door and only one axis can be
    // active, check other axes are zerro limited !!!");
    R_ASSERT2(
        joint->IsHingeJoint(),
        "Wrong door joint!!! Only simple joint valid for a door and only one "
        "axis can be active, check other axes are zerro limited !!!" );
    ...
    joint->PSecond_element()->get_Extensions(
        jaxis, janchor.dotproduct( jaxis ), lo_ext, hi_ext );
    ...
    if ( jaxis_in_door.x > jaxis_in_door.y ) {
        if ( jaxis_in_door.x > jaxis_in_door.z ) {
            joint->PSecond_element()->get_Extensions(
                door_transform.j, janchor.dotproduct( door_transform.j ),
                lo_ext, hi_ext );
            ...
            door_plane_axes.y = 1;
            joint->PSecond_element()->get_Extensions(
                door_transform.k, janchor.dotproduct( door_transform.k ),
                lo_ext, hi_ext );
            ext = hi_ext - lo_ext;
            ...
            door_plane_axes.y = 1;
            joint->PSecond_element()->get_Extensions(
                door_transform.i, janchor.dotproduct( door_transform.i ),
                lo_ext, hi_ext );
            ext = hi_ext - lo_ext;
            if ( ext > door_plane_ext.y ) {
                door_dir_sign = hi_ext > -lo_ext ? 1.f : -1.f;
    ...
    door_test.normalize();
    joint->PFirst_element()->get_Extensions(
        door_test, door_transform.c.dotproduct( door_test ), lo_ext, hi_ext );
    if ( hi_ext > -lo_ext )
        pos_open = -1.f;
    else
        pos_open = 1.f;
    ...
    torque = shoulder.magnitude() * joint->PSecond_element()->getMass() *
             pcar->m_doors_torque_factor * 10.f;
    state = opened;
    //	Close();
}
...
void CCar::SDoor::ClosedToOpening() {
    ...
    CBoneInstance& bone_instance =
        pKinematics->LL_GetBoneInstance( u16( bone_id ) );
    bone_instance.set_callback( bctPhysics,
                                pcar->PPhysicsShell()->GetBonesCallback(),
...
bool CCar::SDoor::IsFront( const Fvector& pos, const Fvector& dir ) {
    ...
    dif1.sub( pos );
    // dif.normalize_safe();
    return ( dif1.dotproduct( tdir ) > dif.dotproduct( tdir ) &&
             abs( c_to_d.dotproduct( tdir ) ) < dif1.dotproduct( tdir ) );
}
...
bool CCar::SDoor::IsInArea( const Fvector& pos, const Fvector& dir ) {
    if ( !joint ) {
        ...
        c_to_d.sub( pf.c, dif1 );
        dif1.sub( pos );
        return 2.f * abs( c_to_d.dotproduct( pcar->XFORM().i ) ) >
               abs( dif1.dotproduct( pcar->XFORM().i ) );
    }
    ...
    c = anchor_to_pos.dotproduct( closed_door_norm ) *
        anchor_to_pos.dotproduct( door_norm );
    if ( a < ( signum > 0.f ? hie : -loe ) && a > 0.f &&
         b < ( signum > 0.f ? hie : -loe ) && b > 0.f &&
         anchor_to_pos.dotproduct( closed_door_norm ) *
                 anchor_to_pos.dotproduct( door_norm ) <
             0.f )
        return true;
    else
...
void CCar::SDoor::GetExitPosition( Fvector& pos ) {
    if ( !joint ) {
        ...
        pos.set( pf.c );
        Fvector add, add1;
        MAX_OF( abs( pf.i.y ), add.set( pf.i );
                add.mul( bb.m_halfsize.x * fsignum( pf.i.y ) ), abs( pf.j.y ),
                add.set( pf.j );
                add.mul( bb.m_halfsize.y * fsignum( pf.j.y ) ), abs( pf.k.y ),
                add.set( pf.k );
                add.mul( bb.m_halfsize.z * fsignum( pf.k.y ) ) );
        pos.sub( add );

        MIN_OF( bb.m_halfsize.x, add1.set( pf.i );
                add1.mul( bb.m_halfsize.x ), bb.m_halfsize.y, add1.set( pf.j );
                add1.mul( bb.m_halfsize.y ), bb.m_halfsize.z, add1.set( pf.k );
    ...
    closed_door_form_in_object.transform_dir( closed_door_dir,
                                              door_dir_in_door );
    pcar->XFORM().transform_dir( closed_door_dir );
    ...
    joint->PSecond_element()->get_Extensions( door_axis, center_prg, lo_ext,
                                              hi_ext );
    add.set( door_axis );
    ...
    joint->PSecond_element()->get_Extensions( door_dir, center_prg, lo_ext,
                                              hi_ext );
    closed_door_dir.normalize();
...
bool CCar::SDoor::TestPass( const Fvector& pos, const Fvector& dir ) {
    ...
    add.mul( -sub.dotproduct( closed_door_norm ) /
             ( dir.dotproduct( closed_door_norm ) ) );
    ...
    if ( !( center_prg + hi_ext > point_prg ) ||
         !( center_prg + lo_ext < point_prg ) )
        return false;
...
bool CCar::SDoor::CanEnter( const Fvector& pos,
                            const Fvector& dir,
                            const Fvector& foot_pos ) {
    // if(!joint) return true;//temp for fake doors
    return ( state == opened || state == broken || !joint ) &&
           TestPass( foot_pos, dir ) && IsInArea( pos, dir ); //
}
...
void CCar::SDoor::SDoorway::Init( SDoor* adoor ) {
    door = adoor;
    Fmatrix door_transform;
    door->joint->PSecond_element()->InterpolateGlobalTransform(
        &door_transform );
    door->closed_door_form_in_object.set(
        door->joint->PSecond_element()->mXFORM );
    Fvector jaxis, janchor;
    ...
    if ( _abs( door_axis_in_door.x ) > _abs( door_axis_in_door.y ) ) {
        if ( _abs( door_axis_in_door.x ) > _abs( door_axis_in_door.z ) ) {
            ...
            door_plane_axes.y = 0; // door axis is x (door_plane_axes.y stores
                                   // door axis direction (i,j,k)=(0,1,2)
            door->joint->PSecond_element()->get_Extensions(
                door_transform.i, janchor.dotproduct( door_transform.i ),
                lo_ext, hi_ext );
            door->door_plane_ext.y =
                hi_ext - lo_ext; // door extension along door axis

            door->joint->PSecond_element()->get_Extensions(
                door_transform.j, janchor.dotproduct( door_transform.j ),
                lo_ext, hi_ext );
            door_plane_ext.x = hi_ext - lo_ext; // door extensions
            door_plane_axes.x = 1; // door_plane_axes.x stores door direction it
                                   // may be j or k in this point
