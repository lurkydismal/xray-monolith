...
void CCC_CreateGameSpyAccount::Execute( LPCSTR args ) {
    ...
    sscanf_s( args, "%s %s %s %s", tmp_nick, sizeof( tmp_nick ), tmp_unick,
              sizeof( tmp_unick ), tmp_email, sizeof( tmp_email ), tmp_password,
              sizeof( tmp_password ) );
...
void CCC_GapySpyListProfiles::Execute( LPCSTR args ) {
    ...
    sscanf_s( args, "%s %s", tmp_email, sizeof( tmp_email ), tmp_password,
              sizeof( tmp_password ) );
...
void CCC_GameSpyLogin::Execute( LPCSTR args ) {
    ...
    sscanf_s( args, "%s %s %s", tmp_email, sizeof( tmp_email ), tmp_nick,
              sizeof( tmp_nick ), tmp_password, sizeof( tmp_password ) );
    ...
    // VERIFY(tmp_lmngr);
    // tmp_lmngr->login(tmp_email, tmp_nick, tmp_password,
    // gamespy_gp::login_operation_cb());
}
...
void CCC_GameSpyPrintProfile::Execute( LPCSTR args ) {
    ...
    // MainMenu()->GetLoginMngr(); gamespy_gp::profile const * tmp_profile =
    // tmp_lmngr->get_current_profile(); if (tmp_profile)
    //{
    //	Msg("- Current profile:");
...
void CCC_GameSpySuggestUNicks::Execute( LPCSTR args ) {
    ...
    // VERIFY(tmp_amngr);
    // tmp_amngr->suggest_unique_nicks(tmp_unick,
    // gamespy_gp::suggest_nicks_cb());
}
...
void CCC_GameSpyProfile::Execute( LPCSTR args ) {
    ...
    // gamespy_profile::profile_store* tmp_prof_store	=
    // MainMenu()->GetProfileStore(); VERIFY(tmp_prof_store);
    //
    // gamespy_gp::profile const * tmp_curr_prof	=
    // tmp_lmngr->get_current_profile(); if (!tmp_curr_prof)
    //{
    //	Msg("- No profile. You are not loged in.");
    //	return;
    // }
