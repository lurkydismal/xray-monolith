...
void CLevel::OnSecureMessage( NET_Packet& P ) {
    ...
    u32 checksum = secure_messaging::decrypt(
        dec_packet.B.data, dec_packet.B.count, m_secret_key );
    u32 real_checksum = 0;
    P.r_u32( real_checksum );
    VERIFY( checksum == real_checksum ); // cheater tries to change incoming
                                         // data packet - need crash
    xrSRWLockGuard g( prefetch_lock );
    game_events->insert(
        dec_packet ); // if checksum != real_checksum will be delayed crash ...
}
...
void CLevel::SecureSend( NET_Packet& P, u32 dwFlags, u32 dwTimeout ) {
    ...
    u32 checksum =
        secure_messaging::encrypt( P.B.data, P.B.count, m_secret_key );
