...
public:
    typedef xr_delegate< void( u32, u32, NET_Packet& ) >
        msg_type_subtype_func_t;
    message_filter();
    ~message_filter();
...
private:
    struct msg_type_subtype_t {
        msg_type_subtype_t( u16 const type = 0,
                            u32 const subtype = 0,
                            u32 const recv_time = 0 )
            : msg_type( type ),
              dest_obj_id( 0 ),
              msg_subtype( subtype ),
              msg_receive_time( recv_time ) {}
    ...
    void dbg_print_msg( NET_Packet& packet,
                        msg_type_subtype_t const& msg_type );
    typedef associative_vector< msg_type_subtype_t, msg_type_subtype_func_t >
        filters_map_t;
