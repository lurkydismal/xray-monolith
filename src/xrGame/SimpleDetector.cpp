...
void CUIArtefactDetectorSimple::Flash( bool bOn, float fRelPower ) {
    ...
    if ( bOn ) {
        ...
        m_turn_off_flash_time =
            Device.dwTimeGlobal + iFloor( fRelPower * 1000.0f );
    } else {
...
void CUIArtefactDetectorSimple::setup_internals() {
    ...
    m_flash_light->set_range( pSettings->r_float(
        m_parent->HudItemData()->m_sect_name, "flash_light_range" ) );
    ...
    m_on_off_light->set_range( pSettings->r_float(
        m_parent->HudItemData()->m_sect_name, "onoff_light_range" ) );
...
void CUIArtefactDetectorSimple::update() {
    ...
    if ( m_parent->IsAttachedToHUD() ) {
        ...
        if ( m_turn_off_flash_time &&
             m_turn_off_flash_time < Device.dwTimeGlobal )
            Flash( false, 0.0f );
