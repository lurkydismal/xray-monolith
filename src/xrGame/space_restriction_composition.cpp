...
IC void CSpaceRestrictionComposition::merge( CBaseRestrictionPtr restriction ) {
    ...
    m_border.insert( m_border.begin(), restriction->border().begin(),
                     restriction->border().end() );
}
...
void CSpaceRestrictionComposition::initialize() {
    ...
    for ( u32 i = 0; i < n; ++i )
        if ( !m_space_restriction_holder
                  ->restriction( _GetItem( *m_space_restrictors, i, element ) )
                  ->initialized() )
            return;
    ...
    for ( u32 i = 0; i < n; ++i ) {
        SpaceRestrictionHolder::CBaseRestrictionPtr restriction =
            m_space_restriction_holder->restriction(
                _GetItem( *m_space_restrictors, i, element ) );
    ...
    // computing almost minimum sphere which covers all the almost minimum
    // spheres
    Fbox3 temp;
    ...
    for ( u32 i = 1; i < n; ++i )
        m_sphere.R =
            _max( m_sphere.R,
                  m_sphere.P.distance_to( spheres[ i ].P ) + spheres[ i ].R );
...
void CSpaceRestrictionComposition::test_correctness() {
...

    {
        ...
        RESTRICTIONS::iterator E = m_restrictions.end();
        for ( ; I != E; ++I )
            m_test_storage.insert( m_test_storage.end(),
                                   ( *I )->object().m_test_storage.begin(),
...

    {
        ...
        m_test_storage.erase(
            std::unique( m_test_storage.begin(), m_test_storage.end() ),
            m_test_storage.end() );
    }
    ...
    xr_vector< u32 > nodes;
    {
        ...
        RESTRICTIONS::iterator E = m_restrictions.end();
        for ( ; I != E; ++I ) {
            VERIFY3( !( *I )->object().m_test_storage.empty(),
                     "Restrictor has no border", *( *I )->object().name() );
            nodes.clear();
            ai().level_graph().set_mask( border() );
            ai().graph_engine().search( ai().level_graph(),
                                        ( *I )->object().m_test_storage.back(),
                                        ( *I )->object().m_test_storage.back(),
...
void CSpaceRestrictionComposition::check_restrictor_type() {
    ...
    VERIFY3( restrictor, "you are trying to use object as a restrictor",
             *m_space_restrictors );
    VERIFY2(
        restrictor->restrictor_type() == RestrictionSpace::eRestrictorTypeNone,
        "you are trying to restrict yourself with restrictor with type "
        "eRestrictorTypeNone" );
    VERIFY2(
        restrictor->restrictor_type() != RestrictionSpace::eRestrictorTypeNone,
