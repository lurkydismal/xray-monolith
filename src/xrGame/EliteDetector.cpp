...
void CUIArtefactDetectorElite::construct( CEliteDetector* p ) {
    ...
    Fvector _map_attach_p =
        pSettings->r_fvector3( m_parent->cNameSect(), "ui_p" );
    Fvector _map_attach_r =
        pSettings->r_fvector3( m_parent->cNameSect(), "ui_r" );

    _map_attach_r.mul( PI / 180.f );
    m_map_attach_offset.setHPB( _map_attach_r.x, _map_attach_r.y,
                                _map_attach_r.z );
...
void CUIArtefactDetectorElite::Draw() {
    ...
    UI().ScreenFrustumLIT().CreateFromRect(
        Frect().set( rp.x, rp.y, wrk_sz.x, wrk_sz.y ) );
...
void CUIArtefactDetectorElite::GetUILocatorMatrix( Fmatrix& _m ) {
    ...
    u16 bid = m_parent->HudItemData()->m_model->LL_BoneID( READ_IF_EXISTS(
        pSettings, r_string, *m_parent->HudItemData()->m_sect_name,
        "detector_ui_bone", "cover" ) );

    Fmatrix cover_bone =
        m_parent->HudItemData()->m_model->LL_GetTransform( bid );
...
void CUIArtefactDetectorElite::RegisterItemToDraw(
    const Fvector& p,
    const shared_str& palette_idx ) {
    xr_map< shared_str, CUIStatic* >::iterator it =
        m_palette.find( palette_idx );
    if ( it == m_palette.end() ) {
        Msg( "! RegisterItemToDraw. static not found for [%s]",
             palette_idx.c_str() );
        return;
    }
