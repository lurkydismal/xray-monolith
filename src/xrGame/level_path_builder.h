...
public:
    ...
    IC void setup( const u32& start_vertex_id,
                   const u32& dest_vertex_id,
    ...
    void register_to_process() {
        // demonized: Move next_retry_time check above
        // m_object->m_wait_for_distributed_computation set to fix never
        // resetting flag leading to stuck monsters
        if ( Device.dwTimeGlobal < m_next_retry_time )
            return;

        m_object->m_wait_for_distributed_computation = true;
        Device.seqParallel.push_back(
            xr_make_delegate( this, &CLevelPathBuilder::process ) );
    }

    void process_impl( bool separate_compute = false ) {
        m_object->m_wait_for_distributed_computation = false;
        m_object->level_path().build_path( m_start_vertex_id,
                                           m_dest_vertex_id );

        if ( m_object->level_path().failed() ) {
            if ( m_use_delay_after_fail )
                m_next_retry_time = Device.dwTimeGlobal +
                                    Random.randI( time_to_wait_after_fail_min,
                                                  time_to_wait_after_fail_max );
        ...
        m_object->detail().set_start_direction(
            Fvector().setHP( -m_object->m_body.current.yaw, 0 ) );

        if ( m_precise_position )
            m_object->detail().set_dest_position( *m_precise_position );

        inherited::setup( m_object->level_path().path(),
                          m_object->level_path().intermediate_index() );
        inherited::process_impl( separate_compute );
    }

    void __stdcall process() {
        PROF_EVENT( "CLevelPathBuilder::process" );
        // demonized: disable the check since we already do it in
        // register_to_process
        /*if (Device.dwTimeGlobal < m_next_retry_time)
                return;*/
