...
void AddEffector( CActor* A, int type, const shared_str& sect_name ) {
    ...
    if ( pSettings->line_exist( sect_name, "cam_eff_name" ) ) {
        ...
        if ( pSettings->line_exist( sect_name, "cam_eff_hud_affect" ) ) {
            bool b_hud_affect =
                !!pSettings->r_bool( sect_name, "cam_eff_hud_affect" );
...
void AddEffector( CActor* A,
                  int type,
                  const shared_str& sect_name,
                  CEffectorController* ec ) {
    if ( pSettings->line_exist( sect_name, "pp_eff_name" ) ) {
        bool bCyclic = !!pSettings->r_bool( sect_name, "pp_eff_cyclic" );
        CPostprocessAnimatorControlled* pp_anm =
            xr_new< CPostprocessAnimatorControlled >( ec );
        pp_anm->SetType( ( EEffectorPPType )type );
        pp_anm->SetCyclic( bCyclic );
        pp_anm->bOverlap = !!pSettings->r_bool( sect_name, "pp_eff_overlap" );
        LPCSTR fn = pSettings->r_string( sect_name, "pp_eff_name" );
    ...
    if ( pSettings->line_exist( sect_name, "cam_eff_name" ) ) {
        bool bCyclic = !!pSettings->r_bool( sect_name, "cam_eff_cyclic" );
        CCameraEffectorControlled* cam_anm =
            xr_new< CCameraEffectorControlled >( ec );
        cam_anm->SetType( ( ECamEffectorType )type );
        cam_anm->SetCyclic( bCyclic );
...
void AddEffector( CActor* A,
    ...
    if ( pSettings->line_exist( sect_name, "cam_eff_name" ) ) {
        bool bCyclic = !!pSettings->r_bool( sect_name, "cam_eff_cyclic" );
        CAnimatorCamLerpEffector* cam_anm =
            xr_new< CAnimatorCamLerpEffector >();
...
void AddEffector( CActor* A,
    ...
    if ( pSettings->line_exist( sect_name, "pp_eff_name" ) ) {
        bool bCyclic = !!pSettings->r_bool( sect_name, "pp_eff_cyclic" );
        CPostprocessAnimatorLerpConst* pp_anm =
            xr_new< CPostprocessAnimatorLerpConst >();
        pp_anm->SetType( ( EEffectorPPType )type );
        pp_anm->SetCyclic( bCyclic );
    ...
    if ( pSettings->line_exist( sect_name, "cam_eff_name" ) ) {
        bool bCyclic = !!pSettings->r_bool( sect_name, "cam_eff_cyclic" );
        CAnimatorCamLerpEffectorConst* cam_anm =
...
CAnimatorCamLerpEffectorConst::CAnimatorCamLerpEffectorConst()
    : m_factor( 0.0f ) {
    SetFactorFunc(
        GET_KOEFF_FUNC( this, &CAnimatorCamLerpEffectorConst::GetFactor ) );
}
...
CCameraEffectorControlled::CCameraEffectorControlled( CEffectorController* c )
    ...
    SetFactorFunc(
        GET_KOEFF_FUNC( m_controller, &CEffectorController::GetFactor ) );
}
...
void SndShockEffector::Update() {
    ...
    if ( y > 0.f ) {
        psSoundVFactor =
            y * ( m_stored_volume - m_stored_volume * SND_MIN_VOLUME_FACTOR ) +
            m_stored_volume * SND_MIN_VOLUME_FACTOR;
    }
}
...
BOOL CControllerPsyHitCamEffector::ProcessCam( SCamEffectorInfo& info ) {
    ...
    if ( angle_lerp( m_dangle_current.x, m_dangle_target.x, ANGLE_SPEED,
                     Device.fTimeDelta ) ) {
        m_dangle_target.x = angle_normalize( Random.randFs( DELTA_ANGLE_X ) );
    }

    if ( angle_lerp( m_dangle_current.y, m_dangle_target.y, ANGLE_SPEED,
                     Device.fTimeDelta ) ) {
        m_dangle_target.y = angle_normalize( Random.randFs( DELTA_ANGLE_Y ) );
    }

    if ( angle_lerp( m_dangle_current.z, m_dangle_target.z, ANGLE_SPEED,
                     Device.fTimeDelta ) ) {
...
bool similar_cam_info( const SCamEffectorInfo& c1,
                       const SCamEffectorInfo& c2 ) {
    return ( c1.p.similar( c2.p, EPS_L ) && c1.d.similar( c2.d, EPS_L ) &&
             c1.n.similar( c2.n, EPS_L ) && c1.r.similar( c2.r, EPS_L ) );
}
...
void cam_effector_sub( const SCamEffectorInfo& c1,
                       const SCamEffectorInfo& c2,
                       SCamEffectorInfo& dest ) {
...
bool CActorCameraManager::ProcessCameraEffector( CEffectorCam* eff ) {
    ...
    if ( res ) {
        if ( eff->GetHudAffect() ) {
            ...
            cam_effector_add( diff,
                              m_cam_info_hud ); // m_cam_info_hud += difference
        }
