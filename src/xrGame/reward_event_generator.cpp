...
reward_event_generator::reward_event_generator( u32 const max_rewards_per_game )
    ...
    event_action_delegate_t tmp_delegate(
        this, &reward_event_generator::AddRewardTask );
    m_state_event_checker =
        xr_new< rewarding_state_events >( m_state_accum, tmp_delegate );

    m_event_handlers =
        xr_new< rewarding_event_handlers >( m_state_accum, tmp_delegate );
...
void reward_event_generator::OnBullet_Fire( u16 sender,
                                            u16 sender_weapon_id,
                                            const Fvector& position,
                                            const Fvector& direction ) {
    m_state_accum->OnBullet_Fire( sender, sender_weapon_id, position,
                                  direction );
    m_event_handlers->OnBullet_Fire( sender, sender_weapon_id, position,
                                     direction );
}

void reward_event_generator::OnBullet_Hit( CObject const* hitter,
...
void reward_event_generator::OnPlayerTakeArtefact(
...
void reward_event_generator::OnPlayerDropArtefact(
    game_PlayerState const* ps ) {
...
void reward_event_generator::OnPlayerBringArtefact(
    game_PlayerState const* ps ) {
...
void reward_event_generator::OnPlayerKilled(
    ...
    m_event_handlers->OnPlayerKilled( killer_id, target_id, weapon_id,
                                      kill_type );
...
void __stdcall reward_event_generator::AddRewardTask( u32 award_id ) {
    ...
    // gamespy_profile::enum_awards_t tmp_award_type =
    // static_cast<gamespy_profile::enum_awards_t>(award_id); VERIFY(award_id <
    // gamespy_profile::at_awards_count);

    // gamespy_gp::login_manager* tmp_lmngr			=
    // MainMenu()->GetLoginMngr(); VERIFY(tmp_lmngr); gamespy_gp::profile const
    // * tmp_curr_prof		= tmp_lmngr->get_current_profile(); if
    // (!tmp_curr_prof)
    //{
    ...
    // game_cl_mp*	tmp_mp_game		=
    // smart_cast<game_cl_mp*>(&Game()); VERIFY(tmp_mp_game);
