...
public:
    ...
    IC bool operator()( CALifeLevelRegistry::_iterator& i,
                        u64 cycle_count,
                        bool ) const {
    ...
    IC void operator()( CALifeLevelRegistry::_iterator& i,
...
CALifeUpdateManager::CALifeUpdateManager( xrServer* server, LPCSTR section )
    ...
    m_update_monster_factor =
        pSettings->r_float( section, "update_monster_factor" );
...
CALifeUpdateManager::~CALifeUpdateManager() {
    ...
    Device.remove_from_seq_parallel(
        xr_make_delegate( this, &CALifeUpdateManager::update ) );
}
...
void CALifeUpdateManager::set_process_time( int microseconds ) {
    graph().set_process_time( float( microseconds ) -
                              float( microseconds ) * update_monster_factor() /
                                  1000000.f );
}
...
bool CALifeUpdateManager::change_level( NET_Packet& net_packet ) {
    ...
    if ( ai().script_engine().functor(
             "_G.CALifeUpdateManager__on_before_change_level", funct ) )
        funct( &net_packet );
#endif
    ...
    GameGraph::_GRAPH_ID holder_safe_graph_vertex_id =
        GameGraph::_GRAPH_ID( -1 );
    ...
    net_packet.r( &graph().actor()->m_tGraphID,
                  sizeof( graph().actor()->m_tGraphID ) );
    net_packet.r( &graph().actor()->m_tNodeID,
                  sizeof( graph().actor()->m_tNodeID ) );
    ...
    if ( ai().game_graph().valid_vertex_id( graph().actor()->m_tGraphID ) ) {
        ...
        destination_path +=
            *ai().game_graph().header().level( destination_level ).name();
        destination_path += "\\";
        pApp->LoadSessionSetScenario(
            ::Render->level_StaticCacheReady( destination_path.c_str() )
                ? "visited-transition"
                : "unseen-transition" );
    ...
    strconcat( sizeof( autoave_name ), autoave_name, Core.UserName, " - ",
               "autosave" );
    LPCSTR temp0 = strstr( **m_server_command_line, "/" );
    VERIFY( temp0 );
    string256 temp;
    *m_server_command_line =
        strconcat( sizeof( temp ), temp, autoave_name, temp0 );
...
void CALifeUpdateManager::load( LPCSTR game_name,
                                bool no_assert,
                                bool new_only ) {
    ...
    if ( new_only || !CALifeStorageManager::load( game_name ) ) {
        R_ASSERT3( new_only || no_assert && xr_strlen( game_name ),
                   "Cannot find the specified saved game ", game_name );
        new_game( game_name );
    }
...
void CALifeUpdateManager::set_switch_online( ALife::_OBJECT_ID id,
                                             bool value ) {
...
void CALifeUpdateManager::set_switch_offline( ALife::_OBJECT_ID id,
                                              bool value ) {
    CSE_ALifeDynamicObject* object = objects().object( id );
    VERIFY( object );
...
void CALifeUpdateManager::jump_to_level( LPCSTR level_name ) const {
    const CGameGraph::SLevel& level =
        ai().game_graph().header().level( level_name );
    ...
    bool failed = !ai().graph_engine().search(
        ai().game_graph(), graph().actor()->m_tGraphID,
        GameGraph::_GRAPH_ID( -1 ), 0, evaluator );
    if ( failed ) {
#ifndef MASTER_GOLD
        Msg( "! Cannot build path via game graph from the current level to the "
             "level %s!",
             level_name );
#endif // #ifndef MASTER_GOLD
        float min_dist = flt_max;
        Fvector current = ai().game_graph()
                              .vertex( graph().actor()->m_tGraphID )
                              ->game_point();
        ...
        for ( GameGraph::_GRAPH_ID i = 0; i < n; ++i )
            if ( ai().game_graph().vertex( i )->level_id() == level.id() ) {
                float distance =
                    ai().game_graph().vertex( i )->game_point().distance_to_sqr(
                        current );
        ...
        if ( !ai().game_graph().vertex( dest ) ) {
            Msg( "! There is no game vertices on the level %s, cannot jump to "
                 "the specified level",
                 level_name );
            return;
        }
...
void CALifeUpdateManager::teleport_object( ALife::_OBJECT_ID id,
    ...
    if ( psAI_Flags.test( aiALife ) ) {
        Msg(
            "[LSS] teleporting object [%s][%s][%d] from level [%s], position "
            "[%f][%f][%f] to level [%s], position [%f][%f][%f]",
            object->name_replace(), *object->s_name, object->ID,
            *( ai().game_graph()
                   .header()
                   .level( ai().game_graph()
                               .vertex( object->m_tGraphID )
                               ->level_id() )
                   .name() ),
            VPUSH(
            ...
            *( ai().game_graph()
                   .header()
                   .level(
                       ai().game_graph().vertex( game_vertex_id )->level_id() )
                   .name() ),
            VPUSH(
    ...
    CSE_ALifeMonsterAbstract* monster_abstract =
        smart_cast< CSE_ALifeMonsterAbstract* >( object );
    if ( monster_abstract )
...
void CALifeUpdateManager::add_restriction(
    ...
    CSE_ALifeDynamicObject* object = objects().object( id, true );
    if ( !object ) {
        Msg( "! cannot add restriction with id %d to the entity with id %d, "
             "because there is no creature with the specified id",
             restriction_id, id );
        return;
    }

    CSE_ALifeDynamicObject* object_restrictor =
        objects().object( restriction_id, true );
    if ( !object_restrictor ) {
        Msg( "! cannot add restriction with id %d to the entity with id %d, "
             "because there is no space restrictor with the specified id",
             restriction_id, id );
        return;
    }

    CSE_ALifeCreatureAbstract* creature =
        smart_cast< CSE_ALifeCreatureAbstract* >( object );
    if ( !creature ) {
        Msg( "! cannot add restriction with id %d to the entity with id %d, "
             "because there is an object with the specified id, but it is not "
             "a creature",
             restriction_id, id );
        return;
    }

    CSE_ALifeSpaceRestrictor* restrictor =
        smart_cast< CSE_ALifeSpaceRestrictor* >( object_restrictor );
    if ( !restrictor ) {
        Msg( "! cannot add restriction with id %d to the entity with id %d, "
             "because there is an object with the specified id, but it is not "
             "a space restrictor",
...
#ifdef DEBUG
            if ( std::find( creature->m_dynamic_out_restrictions.begin(),
                            creature->m_dynamic_out_restrictions.end(),
                            restriction_id ) !=
                 creature->m_dynamic_out_restrictions.end() ) {
                Msg( "! cannot add out-restriction with id %d, name %s to the "
                     "entity with id %d, name %s, because it is already added",
                     restriction_id, restrictor->name_replace(), id,
...
#ifdef DEBUG
            if ( std::find( creature->m_dynamic_in_restrictions.begin(),
                            creature->m_dynamic_in_restrictions.end(),
                            restriction_id ) !=
                 creature->m_dynamic_in_restrictions.end() ) {
                LogStackTrace( "cannot add in-restriction stack trace" );
                Msg( "! cannot add in-restriction with id %d, name %s to the "
                     "entity with id %d, name %s, because it is already added",
                     restriction_id, restrictor->name_replace(), id,
                     creature->name_replace() );
...
void CALifeUpdateManager::remove_restriction(
    ...
    CSE_ALifeDynamicObject* object = objects().object( id, true );
    if ( !object ) {
        Msg( "! cannot remove restriction with id %d to the entity with id %d, "
             "because there is no creature with the specified id",
             restriction_id, id );
        return;
    }
    ...
    switch ( restriction_type ) {
        case RestrictionSpace::eRestrictorTypeOut: {
            xr_vector< ALife::_OBJECT_ID >::iterator I = std::find(
                creature->m_dynamic_out_restrictions.begin(),
                creature->m_dynamic_out_restrictions.end(), restriction_id );
            if ( I == creature->m_dynamic_out_restrictions.end() ) {
                Msg( "~ cannot remove restriction with id [%d][%s] to the "
                     "entity with id [%d][%s], because it is not added",
                     restriction_id, object_restrictor->name_replace(), id,
                     object->name_replace() );
                return;
            }
        ...
        case RestrictionSpace::eRestrictorTypeIn: {
            xr_vector< ALife::_OBJECT_ID >::iterator I = std::find(
                creature->m_dynamic_in_restrictions.begin(),
...
void CALifeUpdateManager::remove_all_restrictions(
    ...
    CSE_ALifeDynamicObject* object = objects().object( id, true );
    if ( !object ) {
        Msg( "! cannot remove restrictions to the entity with id %d, because "
             "there is no creature with the specified id",
             id );
        return;
    }
    ...
    CSE_ALifeCreatureAbstract* creature =
        smart_cast< CSE_ALifeCreatureAbstract* >( object );
    if ( !creature ) {
        Msg( "! cannot remove restriction to the entity with id %d, because "
             "there is an object with the specified id, but it is not a "
             "creature",
             id );
        return;
    }
