...
#define FASTMODE_DISTANCE \
    ( 50.f ) // distance to camera from sphere, when zone switches to fast
             // update sequence
...
void CArtefact::Load( LPCSTR section ) {
    ...
    if ( m_bLightsEnabled ) {
        sscanf( pSettings->r_string( section, "trail_light_color" ), "%f,%f,%f",
                &m_TrailLightColor.r, &m_TrailLightColor.g,
                &m_TrailLightColor.b );
        m_fTrailLightRange = pSettings->r_float( section, "trail_light_range" );
    }

    m_fHealthRestoreSpeed =
        pSettings->r_float( section, "health_restore_speed" );
    m_fRadiationRestoreSpeed =
        pSettings->r_float( section, "radiation_restore_speed" );
    m_fSatietyRestoreSpeed =
        pSettings->r_float( section, "satiety_restore_speed" );
    m_fPowerRestoreSpeed = pSettings->r_float( section, "power_restore_speed" );
    m_fBleedingRestoreSpeed =
        pSettings->r_float( section, "bleeding_restore_speed" );

    if ( pSettings->section_exist(
             pSettings->r_string( section, "hit_absorbation_sect" ) ) ) {
        m_ArtefactHitImmunities.LoadImmunities(
            pSettings->r_string( section, "hit_absorbation_sect" ), pSettings );
    }
    m_bCanSpawnZone =
        !!pSettings->line_exist( "artefact_spawn_zones", section );
...
void CArtefact::Interpolate() {
    ...
    while ( p->NET_IItem.size() >
            1 ) // in real no interpolation, just get latest state
    {
...
void CArtefact::shedule_Update( u32 dt ) {
    ...
    if ( H_Parent() )
        ...
        float cam_distance =
            Device.vCameraPosition.distance_to( center ) - Radius();
...
void CArtefact::StartLights() {
    ...
    bool const b_light_shadow = !!READ_IF_EXISTS(
        pSettings, r_bool, cNameSect(), "idle_light_shadow", false );
    bool const b_light_volumetric = !!READ_IF_EXISTS(
        pSettings, r_bool, cNameSect(), "idle_light_volumetric", false );

    m_pTrailLight->set_shadow( b_light_shadow );

    if ( b_light_volumetric ) {
        float const f_light_volumetric_quality =
            READ_IF_EXISTS( pSettings, r_float, cNameSect(),
                            "idle_light_volumetric_quality", .5f );
        float const f_light_volumetric_power =
            READ_IF_EXISTS( pSettings, r_float, cNameSect(),
                            "idle_light_volumetric_power", .5f );
        float const f_light_volumetric_range =
            READ_IF_EXISTS( pSettings, r_float, cNameSect(),
                            "idle_light_volumetric_range", .5f );
...
bool CArtefact::Action( u16 cmd, u32 flags ) {
    switch ( cmd ) {
        case kWPN_FIRE: {
            ...
            if ( flags & CMD_STOP && m_bCanSpawnZone &&
                 GetState() == eActivating ) {
                SwitchState( eIdle );
                return true;
            }
        } break;
...
void CArtefact::FollowByPath( LPCSTR path_name,
                              int start_idx,
                              Fvector magic_force ) {
    if ( m_detectorObj )
...
void SArtefactDetectorsSupport::SetVisible( bool b ) {
    ...
    if ( b ) {
        LPCSTR curr = pSettings->r_string(
            m_parent->cNameSect().c_str(),
        ...
        m_parent->CParticlesPlayer::StartParticles(
            curr, bone_id, Fvector().set( 0, 1, 0 ), m_parent->ID() );

        curr = pSettings->r_string( m_parent->cNameSect().c_str(),
                                    ( b ) ? "det_show_snd" : "det_hide_snd" );
...
void SArtefactDetectorsSupport::Blink() {
    ...
    R_ASSERT2( K, m_parent->cNameSect().c_str() );
    LPCSTR bone =
        pSettings->r_string( m_parent->cNameSect().c_str(), "particles_bone" );
    u16 bone_id = K->LL_BoneID( bone );
    R_ASSERT2( bone_id != BI_NONE, bone );

    m_parent->CParticlesPlayer::StartParticles(
        curr, bone_id, Fvector().set( 0, 1, 0 ), m_parent->ID(), 1000, true );
}
...
void SArtefactDetectorsSupport::UpdateOnFrame() {
    if ( m_currPatrolPath && !m_parent->getVisible() ) {
        if ( m_parent->Position().distance_to( m_destPoint ) < 2.0f ) {
            ...
            if ( b != e ) {
                ...
                m_currPatrolVertex =
                    m_currPatrolPath->vertex( ( *b ).vertex_id() );
        ...
        if ( v.square_magnitude() < ( 0.7f * 0.7f ) || ( cosa < cos_et ) ) {
...

            {
                ...
                if ( Level().ObjectSpace.RayPick(
                         start, dir_down, ground_check_dist, collide::rqtBoth,
                         R, m_parent ) ) {
    ...
    if ( m_parent->getVisible() && m_parent->GetAfRank() != 0 &&
         m_switchVisTime + 5000 < Device.dwTimeGlobal )
        SetVisible( false );

    u32 dwDt = 2 * 3600 * 1000 / 10; // 2 hour of game time
    if ( !m_parent->getVisible() &&
         m_switchVisTime + dwDt < Device.dwTimeGlobal ) {
        m_switchVisTime = Device.dwTimeGlobal;
        if ( m_parent->Position().distance_to( Device.vCameraPosition ) >
             40.0f )
            Blink();
    }
}

void SArtefactDetectorsSupport::FollowByPath( LPCSTR path_name,
                                              int start_idx,
                                              Fvector force ) {
