...
void CSightManager::SetPointLookAngles( const Fvector& tPosition,
                                        float& yaw,
                                        float& pitch,
...
bool CSightManager::aim_target( Fvector& my_position,
                                Fvector& aim_target,
    ...
    if ( GO && GO->cast_entity() && GO->cast_entity()->g_Alive() &&
         ( GO->cast_actor() || GO->cast_stalker() ) ) {
        if ( GO->cast_actor() && GO->cast_actor()->HUDview() )
            ...
            kinematics->LL_GetBoneWorldPosition( bone_id, object->XFORM(),
                                                 aim_target );
        }
        return ( true );
    }
    ...
    m_object->XFORM().transform_tiny(
        my_position,
        Fvector().set( .2f, my_position.y - m_object->Position().y, 0.f ) );
#else
...
void CSightManager::SetFirePointLookAngles( const Fvector& tPosition,
                                            float& yaw,
                                            float& pitch,
                                            Fvector const& look_position,
                                            const CGameObject* object ) {
...
void CSightManager::SetLessCoverLook( const CLevelGraph::CVertex* tpNode,
                                      bool bDifferenceLook ) {
    SetDirectionLook();
...
void CSightManager::SetLessCoverLook( const CLevelGraph::CVertex* tpNode,
                                      float fMaxHeadTurnAngle,
                                      bool bDifferenceLook ) {
    float fAngleOfView, range,
        fMaxSquare = -1.f, fBestAngle = object().movement().m_head.target.yaw;
    m_object->update_range_fov( range, fAngleOfView, m_object->eye_range,
                                m_object->eye_fov );
    ...
    if ( bDifferenceLook && !m_object->movement().detail().path().empty() &&
         ( m_object->movement().detail().path().size() - 1 >
    ...
    if ( !bDifferenceLook || !bOk )
        for ( float fIncrement =
                  object().movement().m_body.target.yaw - fMaxHeadTurnAngle;
              fIncrement <=
              object().movement().m_body.target.yaw + fMaxHeadTurnAngle;
              fIncrement += fMaxHeadTurnAngle / 18.f ) {
            float fSquare = ai().level_graph().compute_high_square(
                -fIncrement, fAngleOfView, tpNode );
        ...
        float fMaxSquareSingle = -1.f,
              fSingleIncrement = object().movement().m_head.target.yaw;
        for ( float fIncrement =
                  object().movement().m_body.target.yaw - fMaxHeadTurnAngle;
              fIncrement <=
              object().movement().m_body.target.yaw + fMaxHeadTurnAngle;
              fIncrement += 2 * fMaxHeadTurnAngle / 60.f ) {
            float fSquare0 = ai().level_graph().compute_high_square(
                -fIncrement, fAngleOfView, tpNode );
            float fSquare1 = ai().level_graph().compute_high_square(
                -fIncrement, fAngleOfView, tpNextNode );
    ...
    object().movement().m_head.target.yaw =
        angle_normalize_signed( fBestAngle );
...
bool CSightManager::GetDirectionAnglesByPrevPositions( float& yaw,
                                                       float& pitch ) {
    Fvector tDirection;
