...
void CCar::Fly_Load( LPCSTR section ) {
    m_control_neutral =
        READ_IF_EXISTS( pSettings, r_float, section, "control_neutral", 0.0F );
    m_control_ele_max =
        READ_IF_EXISTS( pSettings, r_float, section, "control_ele_max", 0.0F );
    m_control_pit_max =
        READ_IF_EXISTS( pSettings, r_float, section, "control_pit_max", 0.0F );
    m_control_rol_max =
        READ_IF_EXISTS( pSettings, r_float, section, "control_rol_max", 0.0F );
    m_control_yaw_max = deg2rad( READ_IF_EXISTS( pSettings, r_float, section,
                                                 "control_yaw_max", 0.0F ) );
    m_control_ele_inc =
        READ_IF_EXISTS( pSettings, r_float, section, "control_ele_inc", 0.0F );
    m_control_pit_inc =
        READ_IF_EXISTS( pSettings, r_float, section, "control_pit_inc", 0.0F );
    m_control_rol_inc =
        READ_IF_EXISTS( pSettings, r_float, section, "control_rol_inc", 0.0F );
    m_control_yaw_inc = deg2rad( READ_IF_EXISTS( pSettings, r_float, section,
                                                 "control_yaw_inc", 0.0F ) );

    m_fly_weight_min =
        READ_IF_EXISTS( pSettings, r_float, section, "fly_weight_min", 1.0F );
...
BOOL CCar::Fly_net_Spawn( CSE_Abstract* DC ) {
    ...
    m_body_bid = ini->line_exist( fly_sec, "body_bone" )
                     ? K->LL_BoneID( ini->r_string( fly_sec, "body_bone" ) )
                     : BI_NONE;

    {
        ...
        for ( int k = 0; k < n; k++ ) {
            ...
            if ( bone_id != BI_NONE ) {
                ...
                I.E->set_DynamicLimits( default_l_limit,
                                        default_w_limit * 1000.F );
            }
        }
        R_ASSERT3( m_rotor_bones.size(), "fly_definition no rotors",
                   cNameSect_str() );
    }
    {
        m_drive_bones.clear();
        LPCSTR str =
            READ_IF_EXISTS( ini, r_string, fly_sec, "drive_bones", NULL );
        ...
        R_ASSERT3( m_drive_bones.size(), "fly_definition no drive_bones",
                   cNameSect_str() );
    }

    m_rotor_force_max =
        READ_IF_EXISTS( ini, r_float, fly_sec, "rotor_force_max", 0.0F );
    m_rotor_speed_max =
        READ_IF_EXISTS( ini, r_float, fly_sec, "rotor_speed_max", 0.0F );
...
void CCar::Fly_PhDataUpdate( float step ) {
    ...
    if ( b_engine_on && m_rotor_bones.size() && m_drive_bones.size() ) {
        ...
        if ( m_control_ele == eControlEle_NA &&
             m_control_pit == eControlPit_NA &&
             m_control_rol == eControlRol_NA ) {
            Fvector velocity;
            m_pPhysicsShell->get_LinearVel( velocity );
            if ( velocity.magnitude() > EPS_L ) {
                float force = mass * FlyWeightScale() *
                              __min( velocity.magnitude(), m_control_neutral );
                Fvector vec =
                    Fvector().invert( velocity ).normalize_safe().mul( force );
        ...
        switch ( m_control_ele ) {
            case eControlEle_UP:
            case eControlEle_DW: {
                float force = mass * FlyWeightScale() * m_control_ele_max /
                              m_drive_bones.size();
                Fvector vec =
                    Fvector()
                        .set( 0.0F, 1.0F, 0.0F )
                        .mul( ( m_control_ele == eControlEle_UP ) ? force
                                                                  : -force );
        ...
        switch ( m_control_pit ) {
            case eControlPit_FS:
            case eControlPit_BS: {
                float force = mass * FlyWeightScale() * m_control_pit_max /
                              m_drive_bones.size();
                Fvector vec =
                    Fvector()
                        .set( 0.0F, 0.0F, 1.0F )
                        .mul( ( m_control_pit == eControlPit_FS ) ? force
                                                                  : -force );
                for ( auto& I : m_drive_bones ) {
                    I.E->applyRelForce( vec.x, vec.y, vec.z );
                }
        ...
        switch ( m_control_rol ) {
            case eControlRol_RS:
            case eControlRol_LS: {
                float force = mass * FlyWeightScale() * m_control_rol_max /
                              m_drive_bones.size();
                Fvector vec =
                    Fvector()
                        .set( 1.0F, 0.0F, 0.0F )
                        .mul( ( m_control_rol == eControlRol_RS ) ? force
                                                                  : -force );
                for ( auto& I : m_drive_bones ) {
                    I.E->applyRelForce( vec.x, vec.y, vec.z );
                }
        ...
        switch ( m_control_yaw ) {
            ...
            case eControlYaw_LS: {
                ...
                Fvector vec =
                    Fvector()
                        .set( 0.0F, 1.0F, 0.0F )
                        .mul( ( m_control_yaw == eControlYaw_RS ) ? force
                                                                  : -force );
...
void CCar::Fly_RotorUpdate() {
    if ( b_engine_on ) {
        for ( auto& I : m_rotor_bones ) {
            if ( I.spinning != true ) {
                I.spinning = true;
                I.J->SetForceAndVelocity(
                    m_rotor_force_max,
                    ( I.clockwise ) ? m_rotor_speed_max : -m_rotor_speed_max,
                    1 );
            }
        }
    } else {
...
void CCar::ControlPressEleUp( bool status ) {
    ...
    if ( status ) {
        m_control_ele =
            ( m_control_press_ele_dw ) ? eControlEle_NA : eControlEle_UP;
    } else {
        m_control_ele =
            ( m_control_press_ele_dw ) ? eControlEle_DW : eControlEle_NA;
    }
}
...
void CCar::ControlPressEleDw( bool status ) {
    ...
    if ( status ) {
        m_control_ele =
            ( m_control_press_ele_up ) ? eControlEle_NA : eControlEle_DW;
    } else {
        m_control_ele =
            ( m_control_press_ele_up ) ? eControlEle_UP : eControlEle_NA;
    }
}
...
void CCar::ControlPressYawRs( bool status ) {
    ...
    if ( status ) {
        m_control_yaw =
            ( m_control_press_yaw_ls ) ? eControlYaw_NA : eControlYaw_RS;
    } else {
        m_control_yaw =
            ( m_control_press_yaw_ls ) ? eControlYaw_LS : eControlYaw_NA;
    }
}
...
void CCar::ControlPressYawLs( bool status ) {
    ...
    if ( status ) {
        m_control_yaw =
            ( m_control_press_yaw_rs ) ? eControlYaw_NA : eControlYaw_LS;
    } else {
        m_control_yaw =
            ( m_control_press_yaw_rs ) ? eControlYaw_RS : eControlYaw_NA;
    }
}
...
void CCar::ControlPressPitFs( bool status ) {
    ...
    if ( status ) {
        m_control_pit =
            ( m_control_press_pit_bs ) ? eControlPit_NA : eControlPit_FS;
    } else {
        m_control_pit =
            ( m_control_press_pit_bs ) ? eControlPit_BS : eControlPit_NA;
    }
}
...
void CCar::ControlPressPitBs( bool status ) {
    ...
    if ( status ) {
        m_control_pit =
            ( m_control_press_pit_fs ) ? eControlPit_NA : eControlPit_BS;
    } else {
        m_control_pit =
            ( m_control_press_pit_fs ) ? eControlPit_FS : eControlPit_NA;
    }
}
...
void CCar::ControlPressRolRs( bool status ) {
    ...
    if ( status ) {
        m_control_rol =
            ( m_control_press_rol_ls ) ? eControlRol_NA : eControlRol_RS;
    } else {
        m_control_rol =
            ( m_control_press_rol_ls ) ? eControlRol_LS : eControlRol_NA;
    }
}
...
void CCar::ControlPressRolLs( bool status ) {
    ...
    if ( status ) {
        m_control_rol =
            ( m_control_press_rol_rs ) ? eControlRol_NA : eControlRol_LS;
    } else {
        m_control_rol =
            ( m_control_press_rol_rs ) ? eControlRol_RS : eControlRol_NA;
    }
}
...
void CCar::Fly_OnMouseMove( int dx, int dy ) {
    ...
    float scale =
        ( cam->f_fov / g_fov ) * psMouseSens * psMouseSensScale / 50.0F;
    if ( dx ) {
