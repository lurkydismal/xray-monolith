...
void CCar::SVisualCamera::net_Spawn( CSE_Abstract* DC ) {
    ...
    m_rotate_x_speed =
        deg2rad( READ_IF_EXISTS( ini, r_float, def, "rotate_x_speed", 20.0F ) );
    m_rotate_y_speed =
        deg2rad( READ_IF_EXISTS( ini, r_float, def, "rotate_y_speed", 20.0F ) );
...
void _BCL CCar::SVisualCamera::BoneCallbackX( CBoneInstance* B ) {
    CCar::SVisualCamera* P =
        static_cast< CCar::SVisualCamera* >( B->callback_param() );
...
void CCar::SVisualCamera::SetBoneCallbacks( bool val ) {
    CBoneInstance& biX =
        Car()->Visual()->dcast_PKinematics()->LL_GetBoneInstance(
            m_rotate_x_bone );
    CBoneInstance& biY =
        Car()->Visual()->dcast_PKinematics()->LL_GetBoneInstance(
            m_rotate_y_bone );
    if ( val ) {
        ...
        biX.set_callback(
            bctPhysics, Car()->PPhysicsShell()->GetBonesCallback(),
            Car()->PPhysicsShell()->get_Element( m_rotate_x_bone ) );
        biY.set_callback(
            bctPhysics, Car()->PPhysicsShell()->GetBonesCallback(),
            Car()->PPhysicsShell()->get_Element( m_rotate_y_bone ) );
    }
}
...
void CCar::SVisualCamera::OnMouseMove( int dx, int dy ) {
    float scale = ( Car()->Camera()->f_fov / g_fov ) * psMouseSens *
                  psMouseSensScale / 50.0F;
    if ( dx ) {
    ...
    if ( dx ) {
        float d = float( dy ) * scale * ( psMouseInvert.test( 1 ) ? -1 : 1 ) *
                  psMouseSensVerticalK * 0.75F;
        m_desire_ang.x -= d;
