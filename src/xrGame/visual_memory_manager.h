...
private:
    ...
    float m_vision_speed =
        1.0f; // per-NPC vision-speed factor; scales get_visible_value (1.0 =
              // vanilla). Deliberately not reset in reinit()/reload(): persists
              // while online, re-set from script on spawn.
...
protected:
    IC void fill_object( CVisibleObject& visible_object,
...
public:
    float object_visible_distance( const CGameObject* game_object,
                                   float& object_distance ) const;
    float object_luminocity( const CGameObject* game_object ) const;
    float get_visible_value( const CGameObject* game_object,
                             float distance,
                             float object_distance,
                             float time_delta,
                             float object_velocity,
                             float luminocity ) const;
    float get_object_velocity(
        const CGameObject* game_object,
...
public:
    ...
    void add_not_yet_visible_object(
        const CNotYetVisibleObject& not_yet_visible_object );
    CNotYetVisibleObject* not_yet_visible_object(
        const CGameObject* game_object );
