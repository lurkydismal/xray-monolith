    ...
    while ( !P.r_eof() ) {
        ...
        if ( E ) {
            ...
            if ( ( P.r_tell() - _pos ) != size ) {
                ...
                Debug.fatal(
                    DEBUG_INFO,
                    "Beer from the creator of '%s'; initiator: 0x%08x, "
                    "r_tell() = %d, pos = %d, objectID = %d, size = %d",
...
void xrServer::Process_save( NET_Packet& P, ClientID sender ) {
    ...
    while ( !P.r_eof() ) {
        ...
        if ( _size != ( _pos_end - _pos_start ) ) {
            Msg( "! load/save mismatch, object: '%s'",
                 E ? E->name_replace() : "unknown" );
