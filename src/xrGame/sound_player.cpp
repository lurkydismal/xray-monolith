...
u32 CSoundPlayer::add( LPCSTR prefix,
                       u32 max_count,
                       ESoundTypes type,
                       u32 priority,
                       u32 mask,
    ...
    typedef CSoundCollectionStorage::SOUND_COLLECTION_PAIR
        SOUND_COLLECTION_PAIR;
    const SOUND_COLLECTION_PAIR& pair =
        sound_collection_storage().object( sound_params );
    VERIFY( pair.first == ( CSoundCollectionParams& )sound_params );
    VERIFY( pair.second );
    m_sounds.insert( std::make_pair(
        internal_type, std::make_pair( sound_params, pair.second ) ) );
...
bool CSoundPlayer::check_sound_legacy( u32 internal_type ) const {
    ...
    if ( m_sounds.end() == J ) {
        ...
        ai().script_engine().script_log(
            ...
            "Can't find sound with internal type %d (sound_script = %d)",
            internal_type, StalkerSpace::eStalkerSoundScript );
#endif
        return ( false );
    }
...
void CSoundPlayer::remove_inappropriate_sounds( u32 sound_mask ) {
    m_playing_sounds.erase(
        std::remove_if( m_playing_sounds.begin(), m_playing_sounds.end(),
...
void CSoundPlayer::update_playing_sounds() {
    ...
    for ( ; I != E; ++I ) {
        if ( ( *I ).m_sound->_feedback() )
            ( *I ).m_sound->_feedback()->set_position(
                compute_sound_point( *I ) );
        else if ( !( *I ).started() &&
...
void CSoundPlayer::play( u32 internal_type,
                         u32 max_start_time,
                         u32 min_start_time,
                         u32 max_stop_time,
                         u32 min_stop_time,
...
#ifdef DEBUG
        Msg( "- There are no sounds in sound collection \"%s\" with internal "
             "type %d (sound_script = %d)",
             *sound.m_sound_prefix, internal_type,
    ...
    sound_single.m_bone_id = smart_cast< IKinematics* >( m_object->Visual() )
                                 ->LL_BoneID( sound.m_bone_name );
    ...
    sound_single.m_sound->clone( ( *I ).second.second->random( random_id ),
    ...
    sound_single.m_stop_time =
        sound_single.m_start_time +
        iFloor( sound_single.m_sound->get_length_sec() * 1000.0f ) +
        random_time;
    m_playing_sounds.push_back( sound_single );
    string256 name;
    xr_sprintf( name, "%s%s%i", *sound.m_sound_player_prefix,
                *sound.m_sound_prefix, ( random_id + 1 ) );
    // Msg("Sound name \"%s\"", name);
    ::luabind::functor< void > funct;
    if ( ai().script_engine().functor( "_G.CAI_Stalker_OnPhraseCallback",
                                       funct ) ) {
    ...
    if ( Device.dwTimeGlobal >= m_playing_sounds.back().m_start_time )
        m_playing_sounds.back().play_at_pos(
            m_object, compute_sound_point( m_playing_sounds.back() ) );
}
...
IC Fvector CSoundPlayer::compute_sound_point( const CSoundSingle& sound ) {
    ...
    l_tMatrix.mul_43( m_object->XFORM(),
                      smart_cast< IKinematics* >( m_object->Visual() )
                          ->LL_GetBoneInstance( sound.m_bone_id )
                          .mTransform );
    return ( l_tMatrix.c );
}
