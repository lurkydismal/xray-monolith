...
void CHudItem::Load( LPCSTR section ) {
    ...
    m_hud_fov_add_mod = READ_IF_EXISTS( pSettings, r_float, section,
                                        "hud_fov_addition_modifier", 0.f );
    m_nearwall_dist_min =
        READ_IF_EXISTS( pSettings, r_float, section, "nearwall_dist_min", .2f );
    m_nearwall_dist_max =
        READ_IF_EXISTS( pSettings, r_float, section, "nearwall_dist_max", 1.f );
    m_nearwall_target_hud_fov = READ_IF_EXISTS(
        pSettings, r_float, section, "nearwall_target_hud_fov", 0.27f );
    m_nearwall_speed_mod = READ_IF_EXISTS( pSettings, r_float, section,
                                           "nearwall_speed_mod", 10.f );
    m_base_fov = READ_IF_EXISTS( pSettings, r_float, section, "hud_fov", 0.f );

    // Rezy - Custom Script 3D UI
    script_ui_funct = READ_IF_EXISTS( pSettings, r_string, section,
                                      "custom_ui_func", nullptr );

    if ( script_ui_funct ) {
        script_ui_bone = READ_IF_EXISTS( pSettings, r_string, section,
                                         "custom_ui_bone", "wpn_body" );
        script_ui_offset[ 0 ] =
            READ_IF_EXISTS( pSettings, r_fvector3, section, "custom_ui_pos",
                            Fvector().set( 0.f, 0.f, 0.f ) );
        script_ui_offset[ 1 ] =
            READ_IF_EXISTS( pSettings, r_fvector3, section, "custom_ui_rot",
                            Fvector().set( 0.f, 0.f, 0.f ) );

        script_ui_offset[ 1 ].mul( PI / 180.f );
        script_ui_matrix.setHPB( script_ui_offset[ 1 ].x,
                                 script_ui_offset[ 1 ].y,
                                 script_ui_offset[ 1 ].z );
    ...
    if ( _hud_render && !IsHidden() ) {
        ...
        if ( !object().H_Parent() || ( !_hud_render && !IsHidden() ) ) {
            ...
            CInventoryOwner* owner =
                smart_cast< CInventoryOwner* >( object().H_Parent() );
            if ( owner ) {
...
void CHudItem::OnStateSwitch( u32 S, u32 oldState ) {
        ...
        case eBore:
            ...
            if ( TryPlayAnimBore() ) {
                if ( IsAttachedToHUD() ) {
                    ...
                    m_sounds.PlaySound( "sndBore", P, object().H_Root(),
                                        !!GetHUDmode(), false,
    ...
    if ( ai().script_engine().functor( "_G.CHudItem__OnStateSwitch", funct ) ) {
        funct( smart_cast< CGameObject* >( this )->lua_game_object(), S,
               oldState );
    }
}
...
void CHudItem::OnAnimationEnd( u32 state ) {
    ...
    if ( A )
        A->callback( GameObject::eActorHudAnimationEnd )(
...
void CHudItem::UpdateNearWall() {
    if ( g_nearwall && ParentIsActor() &&
         Level().CurrentViewEntity() == object().H_Parent() ) {
        if ( g_nearwall_trace == NT_CAM ) {
            ...
            m_nearwall_factor =
                1 - ( ( dist - m_nearwall_dist_min ) / GetNearWallRange() );
        } else if ( g_nearwall_trace == NT_ITEM ) {
            // Take the item's trace range and invert it, as negative ranges
            // encode penetration distance
...
void CHudItem::UpdateCL() {
    if ( m_current_motion_def ) {
        if ( m_bStopAtEndAnimIsRunning ) {
            const xr_vector< motion_marks >& marks =
                m_current_motion_def->marks;
            if ( !marks.empty() ) {
                float motion_prev_time =
                    ( ( float )m_dwMotionCurrTm - ( float )m_dwMotionStartTm ) /
                    1000.0f;
                float motion_curr_time = ( ( float )Device.dwTimeGlobal -
                                           ( float )m_dwMotionStartTm ) /
                                         1000.0f;
                ...
                for ( ; it != it_e; ++it ) {
                    ...
                    const motion_marks::interval* Iprev =
                        M.pick_mark( motion_prev_time );
                    const motion_marks::interval* Icurr =
                        M.pick_mark( motion_curr_time );
...
void CHudItem::OnMotionMark( u32 state, const motion_marks& M ) {
    ...
    if ( ai().script_engine().functor( "_G.CHudItem__OnMotionMark", funct ) )
        funct( state, *M.name, object().lua_game_object(),
               object().lua_game_object() ? object().lua_game_object()->Parent()
                                          : nullptr );
}
...
void CHudItem::OnH_B_Independent( bool just_before_destroy ) {
    ...
    {
    g_player_hud->detach_item(this);
    Msg("---Detaching hud item [%s][%d]", this->HudSection().c_str(),
    this->object().ID());
    }*/
    // SetHudItemData			(NULL);
}
...
void CHudItem::on_a_hud_attach() {
    if ( script_ui_funct && nullptr == script_ui ) {
        ...
        if ( ai().script_engine().functor( script_ui_funct, funct ) ) {
            ...
            CUIWindow* pScriptWnd =
                ret ? smart_cast< CUIWindow* >( ret ) : ( 0 );
            if ( pScriptWnd )
                script_ui = pScriptWnd;
            else
                Msg( "[%s]: Failed to load script UI [%s]!",
                     object().cNameSect_str(), script_ui_funct );
        } else
            Msg( "[%s]: Script UI functor [%s] does not exist!",
                 object().cNameSect_str(), script_ui_funct );
    }
}
...
u32 CHudItem::PlayHUDMotion( shared_str M,
                             BOOL bMixIn,
                             CHudItem* W,
                             u32 state,
                             float speed,
    ...
    if ( IsAttachedToHUD() ) {
        ...
        if ( ai().script_engine().functor( "_G.CHudItem__PlayHUDMotion",
                                           funct ) ) {
            ...
            ::luabind::object const& output =
                funct( table, object().lua_game_object(),
                       object().lua_game_object()
                           ? object().lua_game_object()->Parent()
                           : nullptr );
            if ( output && output.type() == LUA_TTABLE ) {
                M = ::luabind::object_cast< LPCSTR >( output[ "anm_name" ] );
                bMixIn =
                    ::luabind::object_cast< bool >( output[ "anm_mixin" ] );
                bMixIn2 =
                    ::luabind::object_cast< bool >( output[ "anm_mixin2" ] );
                state = ::luabind::object_cast< u32 >( output[ "anm_state" ] );
                speed =
                    ::luabind::object_cast< float >( output[ "anm_speed" ] );
...
u32 CHudItem::PlayHUDMotion_noCB( const shared_str& motion_name,
                                  BOOL bMixIn,
                                  float speed,
                                  bool bMixIn2 ) {
    ...
    if ( bDebug && item().m_pInventory ) {
        Msg( "-[%s] as[%d] [%d]anim_play [%s][%d]",
             ...
             item().m_pInventory->GetActiveSlot(), item().object_id(),
             motion_name.c_str(), Device.dwFrame );
    }
    if ( IsAttachedToHUD() ) {
        return HudItemData()->anim_play(
            motion_name, bMixIn, m_current_motion_def, m_started_rnd_anim_idx,
            speed, bMixIn2 );
    } else {
        m_started_rnd_anim_idx = 0;
        return g_player_hud->motion_length( motion_name, HudSection(),
                                            m_current_motion_def );
    }
}
...
void CHudItem::PlayBlendAnm( LPCSTR name,
                             float speed,
                             float power,
                             bool stop_old ) {
    u8 part = ( object().cast_weapon()->IsZoomed()
                    ? 2
                    : ( g_player_hud->attached_item( 1 ) ? 0 : 2 ) );
...
bool CHudItem::HudAnimationExist( LPCSTR anim_name ) {
    if ( IsAttachedToHUD() ) // First person
    {
        ...
        u16 attach_place_idx =
            pSettings->r_u16( HudItemData()->m_sect_name, "attach_place_idx" );
        xr_sprintf( anim_name_r, "%s%s", anim_name,
                    ( ( attach_place_idx == 1 ) && is_16x9 ) ? "_16x9" : "" );
        player_hud_motion* anm =
            HudItemData()->m_hand_motions->find_motion( anim_name_r );
        if ( anm )
            return true;
    ...
    {
        if ( g_player_hud->motion_length( anim_name, HudSection(),
                                          m_current_motion_def ) > 100 )
            return true;
    }
#ifdef DEBUG
...
void CHudItem::PlayAnimIdleMoving() {
    PlayHUDMotion(
        ...
        isActorAccelerated( Actor()->MovingState(), false ) ? 1.f : .75f );
}
...
void CHudItem::ApplyAimModifiers( Fmatrix& matrix ) {
    ...
    if ( !firepos ) {
        if ( pActor->HUDview() ) {
            ...
            // If we're in third-person, project the actor position onto the HUD
            // pick vector
            matrix.c = Fvector().mad(
                hud_pick.defs.start, hud_pick.defs.dir,
                Fvector()
                    .sub( pActor->Position(), hud_pick.defs.start )
                    .dotproduct( hud_pick.defs.dir ) );
        }
    }
    ...
    if ( !aimpos ) {
        ...
        Fvector target = Fvector().mad( hud_pick.defs.start, hud_pick.defs.dir,
                                        hud_pick.defs.range );
...
void CHudItem::Ray( SPickParam& pp ) {
    ...
    if ( GetHUDmode() ) {
        ...
        Fmatrix proj = Fmatrix().build_projection(
            deg2rad( GetBaseHudFov() * 83.f ), Device.fASPECT, R_VIEWPORT_NEAR,
    ...
    if ( !GetHUDmode() && HUD().FireposActive() ) {
        // If we're in third-person with firepos active, use the actor's head
        // bone
        eye_pos = pActor->XFORM().c;
        auto model = pActor->Visual()->dcast_PKinematics();
        eye_pos.add(
            model->LL_GetTransform( model->LL_BoneID( "bip01_head" ) ).c );
    } else {
    ...
    if ( HUD().DoPick( pn ) ) {
        ...
        if ( GetHUDmode() ) {
            ...
            pn.defs.start.add(
                Fvector().mul( pn.defs.dir, pn.result.range * 0.99 ) );

            // Trace to the camera
            pn.defs.dir =
                Fvector().sub( Device.vCameraPosition, pn.defs.start );
            pn.defs.range = pn.defs.dir.magnitude();
            pn.defs.dir.normalize();
            HUD().DoPick( pn );
...
void VisualCallbackHud( IKinematics* tpKinematics ) {
    CHudItem* game_object =
        static_cast< CHudItem* >( ( tpKinematics->GetUpdateCallbackParam() ) );
    VERIFY( game_object );

    CHudItem::HUD_CALLBACK_VECTOR_IT I =
        game_object->visual_callbacks().begin();
...
void CHudItem::add_visual_callback( hud_visual_callback* callback ) {
    if ( !HudItemData()->m_model )
        return;
    HUD_CALLBACK_VECTOR_IT I = std::find( visual_callbacks().begin(),
...
void CHudItem::remove_visual_callback( hud_visual_callback* callback ) {
    if ( !HudItemData()->m_model )
        return;
    HUD_CALLBACK_VECTOR_IT I = std::find( m_visual_callback.begin(),
