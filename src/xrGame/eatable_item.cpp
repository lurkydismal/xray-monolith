...
void CEatableItem::Load( LPCSTR section ) {
    ...
    m_iRemainingUses = m_iMaxUses =
        READ_IF_EXISTS( pSettings, r_u8, section, "max_uses", 1 );
    m_bRemoveAfterUse =
        READ_IF_EXISTS( pSettings, r_bool, section, "remove_after_use", TRUE );
    m_fWeightFull = m_weight;
    m_fWeightEmpty =
        READ_IF_EXISTS( pSettings, r_float, section, "empty_weight", 0.0f );
...
bool CEatableItem::UseBy( CEntityAlive* entity_alive ) {
    ...
    for ( u8 i = 0; i < ( u8 )eBoostMaxCount; i++ ) {
        if ( pSettings->line_exist( m_physic_item->cNameSect().c_str(),
            ...
            entity_alive->conditions().ApplyBooster(
                B, m_physic_item->cNameSect() );
        }
    }
