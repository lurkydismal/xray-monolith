...
class ik_foot_geom {
public:
    enum e_collide_point { toe = 0, heel, side, none = -1 };

    IC void set( const Fvector& toe,
                 const Fvector& heel,
                 const Fvector& side ) {
        _toe = toe;
        _heel = heel;
    ...
    IC bool is_valid() const {
        ...
        return test.contains( pos_toe() ) && test.contains( pos_heel() ) &&
               test.contains( pos_side() );
    }

    ik_foot_geom()
        : _toe( Fvector().set( -FLT_MAX, -FLT_MAX, -FLT_MAX ) ),
...
struct SIKCollideData {
    ...
    SIKCollideData()
        : m_pick_dir( Fvector().set( 0, -1, 0 ) ),
          collided( false ),
          m_collide_point( ik_foot_geom::toe ) {}
};
