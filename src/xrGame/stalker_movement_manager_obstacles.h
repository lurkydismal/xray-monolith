...
class stalker_movement_manager_obstacles
    : public stalker_movement_manager_base {
private:
...
public:
    ...
    float is_going_through( Fmatrix const& matrix,
...
private:
    IC void remove_query_objects( const Fvector& position,
                                  const float& radius );
    ...
    void process_query( CPHMovementControl* movement_control,
                        Fvector& dest_position,
                        float time_delta );
    void move_along_path_impl( CPHMovementControl* movement_control,
                               Fvector& dest_position,
                               float time_delta );
...
#ifdef DEBUG
    inline doors::actor const& get_doors_actor() const {
        return *m_doors_actor;
    }
#endif // #ifdef DEBUG
