...
void CWeaponBM16::Load( LPCSTR section ) {
    ...
    m_sounds.LoadSound( section, "snd_reload_1", "sndReload1", true,
                        m_eSoundShot );
}
...
void CWeaponBM16::PlayAnimShoot() {
    switch ( m_magazine.size() ) {
        case 1:
            PlayHUDMotion( "anm_shot_1", TRUE, this, GetState(), 1.f, 0.f,
                           false );
            break;
        case 2:
            PlayHUDMotion( "anm_shot_2", TRUE, this, GetState(), 1.f, 0.f,
                           false );
            break;
    }
}
...
void CWeaponBM16::PlayAnimShow() {
    switch ( m_magazine.size() ) {
        case 0:
            PlayHUDMotion( "anm_show_0", TRUE, this, GetState(), 1.f, 0.f,
                           false );
            break;
        case 1:
            PlayHUDMotion( "anm_show_1", TRUE, this, GetState(), 1.f, 0.f,
                           false );
            break;
        case 2:
            PlayHUDMotion( "anm_show_2", TRUE, this, GetState(), 1.f, 0.f,
                           false );
            break;
    }
}
                ...
                switch ( m_magazine.size() ) {
                    case 0: {
                        HudAnimationExist( "anm_idle_moving_crouch_0" )
                            ? PlayHUDMotion( "anm_idle_moving_crouch_0", TRUE,
                                             NULL, GetState() )
                            : PlayHUDMotion( "anm_idle_moving_0", TRUE, NULL,
                                             GetState(), .7f );
                    } break;
                    case 1: {
                        HudAnimationExist( "anm_idle_moving_crouch_1" )
                            ? PlayHUDMotion( "anm_idle_moving_crouch_1", TRUE,
                                             NULL, GetState() )
                            : PlayHUDMotion( "anm_idle_moving_1", TRUE, NULL,
                                             GetState(), .7f );
                    } break;
                    case 2: {
                        HudAnimationExist( "anm_idle_moving_crouch_2" )
                            ? PlayHUDMotion( "anm_idle_moving_crouch_2", TRUE,
                                             NULL, GetState() )
                            : PlayHUDMotion( "anm_idle_moving_2", TRUE, NULL,
                                             GetState(), .7f );
                    } break;
                };
