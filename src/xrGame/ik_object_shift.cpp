...
void object_shift::dbg_draw( const Fmatrix& current_pos,
                             const extrapolation::points& predict,
                             const Fvector& start ) const {
    Fvector p0;
    ...
    for ( u16 i = 0; i < nb_points; ++i ) {
        ...
        float fshift1 =
            current + delta_shift( time_passed + time_quant * ( i + 1 ) );
        Fvector v0 = Fvector()
                         .add( p0, Fvector().mul( vadd, float( i ) ) )
                         .add( Fvector().set( 0, fshift0, 0 ) );
        Fvector v1 = Fvector()
                         .add( p0, Fvector().mul( vadd, float( i + 1 ) ) )
                         .add( Fvector().set( 0, fshift1, 0 ) );

        DBG_DrawLine( v0, v1, D3DCOLOR_XRGB( 0, 255, 0 ) );
    }

    float start_shift = current + delta_shift( time_passed );
    float end_shift = current + delta_shift( time_passed + time );
    DBG_DrawLine( Fvector().add( p0, Fvector().set( 0, start_shift, 0 ) ),
...
float object_shift::delta_shift( float delta_time ) const {
    ...
    return speed * ( delta_time ) + accel * sq_time / 2.f +
           aaccel * sq_time * delta_time / 6.f;
}

bool square_equation( float a,
                      float b,
                      float c,
                      float& x0,
                      float& x1 ) // returns true if has real roots
{
    float d = b * b - 4.f * a * c;
    if ( d < 0.f )
        return false;
...
// float clamp_taget_to_max_possible_shift_speed_return_shift_taget(float
// &taget, float current, float speed, float time )
//{
...
void object_shift::set_taget( float taget_, float time ) {
    ...
    // float x = clamp_taget_to_max_possible_shift_speed_return_shift_taget(
    // taget, current, speed, time );
