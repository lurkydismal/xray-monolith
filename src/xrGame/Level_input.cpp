...
#define CURRENT_ENTITY()                                                \
    ( game ? ( ( GameID() == eGameIDSingle ) ? CurrentEntity()          \
...
ENGINE_API int mouse_button_2_key[] = { MOUSE_1, MOUSE_2, MOUSE_3, MOUSE_4,
...
void CLevel::IR_OnKeyboardPress( int key ) {
    ...
    if ( g_block_all_except_movement ) {
        if ( !( _curr < kCAM_1 || _curr == kWPN_FIRE || _curr == kPAUSE ||
                _curr == kDROP || _curr == kSCREENSHOT || _curr == kQUIT ||
                _curr == kCONSOLE || _curr == kQUICK_LOAD ||
                _curr == kQUICK_SAVE ) )
            return;
    }
    ...
    if ( _curr == kPAUSE ) {
        ...
        if ( !g_block_pause && ( IsGameTypeSingle() || IsDemoPlay() ) ) {
            ...
            if ( psActorFlags.test( AF_NO_CLIP ) )
                Device.Pause( !Device.Paused(), TRUE, TRUE,
                              "li_pause_key_no_clip" );
            else
#endif // DEBUG
    ...
    if ( b_ui_exist && CurrentGameUI()->GetActorMenu().IsShown() &&
         CurrentGameUI()->GetActorMenu().b_sort_hotkeys ) {
        switch ( key ) {
            case DIK_1:
            case DIK_2:
            case DIK_3:
            ...
            case DIK_0: {
                ...
                if ( CurrentGameUI()->GetActorMenu().m_sort_buttons.size() >
                         tab &&
                     CurrentGameUI()
                         ->GetActorMenu()
                         .m_sort_buttons.at( tab )
                         ->IsEnabled() )
    ...
    if ( !g_bDisableAllInput ) {
        ...
        if ( pda && CurrentGameUI()->TopInputReceiver() ==
                        pda ) // Fix PDA hotkey input for disabled state
            if ( pda->IsShown() && !pda->IsEnabled() &&
                 pda->OnKeyboardAction( key, WINDOW_KEY_PRESSED ) )
                return;
    }
    ...
    if ( g_bDisableAllInput || !bReady || !b_ui_exist )
        return;
...
#ifdef DEBUG
        FS.get_path
("$game_config$")->m_Flags.set(FS_Path::flNeedRescan, TRUE); FS.get_path
("$game_scripts$")->m_Flags.set(FS_Path::flNeedRescan, TRUE); FS.rescan_pathes
(); #endif // DEBUG Console->Execute("load_last_save"); return;
        }
    */
    ...
    switch ( key ) {
        case DIK_F7: {
            if ( GameID() != eGameIDSingle )
                return;
            FS.get_path( "$game_config$" )
                ->m_Flags.set( FS_Path::flNeedRescan, TRUE );
            FS.get_path( "$game_scripts$" )
                ->m_Flags.set( FS_Path::flNeedRescan, TRUE );
            FS.rescan_pathes();
...
#ifdef DEBUG
            if ( !m_bEnvPaused )
                SetEnvironmentGameTimeFactor( GetEnvironmentGameTime(),
                                              g_fTimeFactor );
#else  // DEBUG
            SetEnvironmentGameTimeFactor( GetEnvironmentGameTime(),
                                          g_fTimeFactor );
        ...
        case DIK_F4: {
            ...
            if ( i < n ) {
                ...
                bOk = false;
                for ( ++i; i < n; ++i ) {
                    CEntityAlive* tpEntityAlive = smart_cast< CEntityAlive* >(
                        Objects.o_get_by_iterator( i ) );
                    if ( tpEntityAlive ) {
                        bOk = true;
                        break;
                    }
                ...
                if ( !bOk )
                    for ( i = 0; i < j; ++i ) {
                        CEntityAlive* tpEntityAlive =
                            smart_cast< CEntityAlive* >(
                                Objects.o_get_by_iterator( i ) );
                        if ( tpEntityAlive ) {
                            bOk = true;
                            break;
                ...
                if ( bOk ) {
                    ...
                    if ( pActor ) {
                        ...
                        CHudItem* pHudItem = smart_cast< CHudItem* >(
        ...
        case DIK_F5: {
            if ( CBaseMonster* pBM =
                     smart_cast< CBaseMonster* >( CurrentEntity() ) ) {
                DBG().log_debug_info();
            }
            break;
        }
