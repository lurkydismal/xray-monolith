...
void CWeaponKnife::Load( LPCSTR section ) {
    ...
    m_Splash1PerVictimsHCount =
        pSettings->r_u32( section, "splash1_pervictim_hcount" );
    m_Splash2HitsCount = pSettings->r_u32( section, "splash2_hits_count" );
#ifdef DEBUG
    m_dbg_data.m_pick_vectors.reserve(
        std::max( m_Splash1HitsCount, m_Splash2HitsCount ) );
#endif
    m_NextHitDivideFactor =
        pSettings->r_float( section, "splash_hit_divide_factor" );
...
void CWeaponKnife::KnifeStrike( const Fvector& pos, const Fvector& dir ) {
    ...
    if ( real_victim ) {
        float new_khit = m_eHitType == m_eHitType_1
                             ? float( m_Splash1PerVictimsHCount )
                             : float( m_Splash2HitsCount );
        MakeShot( pos, dir, new_khit );
        return;
    }

    shot_targets_t dest_hits( _alloca( sizeof( Fvector ) * m_hits_count ),
                              m_hits_count );
...
void CWeaponKnife::MakeShot( Fvector const& pos,
                             Fvector const& dir,
                             float const k_hit ) {
    ...
    Level().BulletManager().AddBullet( pos, dir, m_fStartBulletSpeed,
                                       fCurrentHit, fHitImpulse_cur,
                                       H_Parent()->ID(), ID(), m_eHitType,
                                       fireDistance, cartridge, 1.f, SendHit );
}
...
void CWeaponKnife::LoadFireParams( LPCSTR section ) {
    ...
    shared_str s_sHitPowerCritical_2 =
        pSettings->r_string_wb( section, "hit_power_critical_2" );

    string32 buffer;

    SetHit2Power( ( float )atof( _GetItem( *s_sHitPower_2, 0, buffer ) ) );
    SetHit2PowerCritical(
        ( float )atof( _GetItem( *s_sHitPowerCritical_2, 0, buffer ) ) );
    SetHit2Impulse( pSettings->r_float( section, "hit_impulse_2" ) );
    m_eHitType_2 = ALife::g_tfString2HitType(
...
void CWeaponKnife::SetHit1Power( float power ) {
    ...
    fvHitPower_1[ egdNovice ] = fvHitPower_1[ egdStalker ] =
        fvHitPower_1[ egdVeteran ] = fvHitPower_1[ egdMaster ];
}

void CWeaponKnife::SetHit2Power( float power ) {
    fvHitPower_2[ egdMaster ] = power;
    fvHitPower_2[ egdNovice ] = fvHitPower_2[ egdStalker ] =
...
void CWeaponKnife::SetHit1PowerCritical( float power ) {
    ...
    fvHitPowerCritical_1[ egdNovice ] = fvHitPowerCritical_1[ egdStalker ] =
        fvHitPowerCritical_1[ egdVeteran ] = fvHitPowerCritical_1[ egdMaster ];
}

void CWeaponKnife::SetHit2PowerCritical( float power ) {
    fvHitPowerCritical_2[ egdMaster ] = power;
    fvHitPowerCritical_2[ egdNovice ] = fvHitPowerCritical_2[ egdStalker ] =
        fvHitPowerCritical_2[ egdVeteran ] = fvHitPowerCritical_2[ egdMaster ];
}
...
void CWeaponKnife::OnRender() {
    ...
    if ( g_bDrawBulletHit ) {
        for ( dbg_draw_data::spheres_t::const_iterator
                  i = m_dbg_data.m_spheres.begin(),
                  ie = m_dbg_data.m_spheres.end();
              i != ie; ++i ) {
            float sc_r = i->second;
        ...
        /*
        Fmatrix	sphere				= Fmatrix().scale(.05f, .05f,
        .05f); sphere.c					= m_dbg_data.m_pos;
        renderer.draw_ellipse		(sphere, D3DCOLOR_XRGB(255, 0, 0));
        renderer.draw_line			(Fidentity, m_dbg_data.m_pos,
        m_dbg_data.m_endpos, D3DCOLOR_XRGB(255, 255, 0));

        sphere.c					= m_dbg_data.m_endpos;
        renderer.draw_ellipse		(sphere, D3DCOLOR_XRGB(100, 255, 0));*/
        // Fvector victim_end			(m_dbg_data.m_pos);
        // victim_end.add (m_dbg_data.m_pick_vector); renderer.draw_line
        // (Fidentity, m_dbg_data.m_pos, victim_end, D3DCOLOR_XRGB(0, 255,
        // 255));
    }
    float hit_power = 1.f;
    for ( dbg_draw_data::targets_t::const_iterator
              i = m_dbg_data.m_targets_vectors.begin(),
    ...
    for ( dbg_draw_data::obbes_t::const_iterator
              i = m_dbg_data.m_target_boxes.begin(),
              ie = m_dbg_data.m_target_boxes.end();
          i != ie; ++i ) {
        Fmatrix tmp_matrix;
        tmp_matrix.set( i->m_rotate.i, i->m_rotate.j, i->m_rotate.k,
                        i->m_translate );
        renderer.draw_obb( tmp_matrix, i->m_halfsize,
...
static bool intersect( Fobb bone, Fsphere const& query ) {
    ...
    return ( new_position.x >= -1.f ) && ( new_position.y >= -1.f ) &&
           ( new_position.z >= -1.f ) && ( new_position.x <= 1.f ) &&
           ( new_position.y <= 1.f ) && ( new_position.z <= 1.f );
}
...
static bool intersect( Fcylinder const& bone, Fsphere const& query ) {
    ...
    float const axe_projection2_sqr =
        bone2query.square_magnitude() - _sqr( axe_projection );
    ...
    Fvector const center_direction =
        Fvector( bone.m_direction ).mul( axe_projection >= 0.f ? 1.f : -1.f );
    Fvector const circle_center =
        Fvector( bone.m_center ).mad( center_direction, half_height );
    ...
    float const circle_radius =
        _sqrt( _sqr( query.R ) - _sqr( distance2plane ) );
    Fvector const sphere_circle_center =
        Fvector( query.P ).mad( center_direction, -distance2plane );
    float const distance2center_sqr =
        circle_center.distance_to_sqr( sphere_circle_center );
...
void CWeaponKnife::GetVictimPos( CEntityAlive* victim, Fvector& pos_dest ) {
    /*VERIFY(victim);
    IKinematics*	tmp_kinem	=
    smart_cast<IKinematics*>(victim->Visual()); u16 hit_bone_id
    = tmp_kinem->LL_BoneID(m_SplashHitBone); if (hit_bone_id != BI_NONE)
    {
            Fmatrix			tmp_matrix;
...
void CWeaponKnife::fill_shots_list( victims_shapes_list_t& victims_shapres,
    ...
    for ( victims_shapes_list_t::iterator i = victims_shapres.begin(),
        ...
        switch ( curr_bone.m_bone_element->type ) {
            ...
            case SBoneShape::stSphere: {
                intersect_res =
                    intersect( curr_bone.m_bone_element->s_sphere, query );
                break;
            } break;
            case SBoneShape::stCylinder: {
                intersect_res =
                    intersect( curr_bone.m_bone_element->c_cylinder, query );
                break;
            } break;
        }; // switch (tmp_bone_data.shape.type)*/
        if ( intersect_res ) {
            victims_hits_count_t::iterator tmp_vhits_iter =
                m_victims_hits_count.find( curr_bone.m_victim_id );
            if ( m_perv_hits_count &&
                 ( tmp_vhits_iter == m_victims_hits_count.end() ) ) {
                m_victims_hits_count.insert(
                    std::make_pair( curr_bone.m_victim_id, u16( 1 ) ) );
            } else if ( m_perv_hits_count &&
                        ( tmp_vhits_iter->second < m_perv_hits_count ) ) {
...
void CWeaponKnife::make_hit_sort_vectors( Fvector& basis_hit_specific,
                                          float& max_dist ) {
...
u32 CWeaponKnife::SelectHitsToShot( shot_targets_t& dst_dirs,
                                    Fvector const& f_pos ) {
    ...
    for ( victims_list_t::const_iterator i = tmp_victims_list.begin(),
                                         ie = tmp_victims_list.end();
          i != ie; ++i ) {
        summ_shapes_count += get_entity_bones_count( *i );
    }
    victims_shapes_list_t tmp_shapes_list(
        _alloca( summ_shapes_count *
                 sizeof( victims_shapes_list_t::value_type ) ),
        summ_shapes_count );
    ...
    std::sort( tmp_shapes_list.begin(), tmp_shapes_list.end(),
               shapes_compare_predicate );
...
bool CWeaponKnife::SelectBestHitVictim( Fvector const& f_pos,
    ...
    g_SpatialSpace->q_sphere( m_spartial_query_res, 0, STYPE_COLLIDEABLE,
                              fendpos_dest, m_splash_radius );

    if ( ( m_eHitType == m_eHitType_2 ) && ( !m_spartial_query_res.empty() ) ) {
        spartial_base_t::value_type tmp_best_victim = NULL;
        best_victim_selector tmp_selector( tmp_parent->ID(), fendpos_dest,
                                           spartial_prefetch_radius,
                                           tmp_best_victim );
        std::for_each( m_spartial_query_res.begin(), m_spartial_query_res.end(),
                       tmp_selector );
        m_spartial_query_res.clear();
        if ( tmp_best_victim )
            m_spartial_query_res.push_back( tmp_best_victim );
    } else {
        victim_filter tmp_filter( tmp_parent->ID(), fendpos_dest,
                                  spartial_prefetch_radius );
        m_spartial_query_res.erase(
            std::remove_if( m_spartial_query_res.begin(),
                            m_spartial_query_res.end(), tmp_filter ),
            m_spartial_query_res.end() );
    }
    return !m_spartial_query_res.empty();
}

BOOL CWeaponKnife::RayQueryCallback( collide::rq_result& result,
                                     LPVOID this_ptr ) {
...
CObject* CWeaponKnife::TryPick( Fvector const& start_pos,
                                Fvector const& dir,
                                float const dist ) {
    collide::ray_defs tmp_rdefs( start_pos, dir, dist, CDB::OPT_FULL_TEST,
                                 collide::rqtObject );
    m_ray_query_results.r_clear();
    m_last_picked_obj = NULL;
    VERIFY( H_Parent() );
    m_except_id = H_Parent()->ID();
    Level().ObjectSpace.RayQuery( m_ray_query_results, tmp_rdefs,
                                  &CWeaponKnife::RayQueryCallback,
                                  static_cast< LPVOID >( this ), NULL, NULL );
...
bool CWeaponKnife::victim_filter::operator()(
    spartial_base_t::value_type const& left ) const {
