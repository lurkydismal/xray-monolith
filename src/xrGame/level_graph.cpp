...
#endif
{
    ...
    m_row_length = iFloor( ( header().box().max.z - header().box().min.z ) /
                               header().cell_size() +
                           EPS_L + 1.5f );
    m_column_length = iFloor( ( header().box().max.x - header().box().min.x ) /
                                  header().cell_size() +
                              EPS_L + 1.5f );
...
u32 CLevelGraph::vertex( u32 current_node_id, const Fvector& position ) const {
    ...
    if ( valid_vertex_position( position ) ) {
        ...
        if ( valid_vertex_id( _vertex_id ) ) {
            // so, there is a node which corresponds with x and z to the
            // position
            bool ok = true;
            if ( valid_vertex_id( current_node_id ) ) {
                {
                ...
                float y0 =
                    vertex_plane_y( current_node_id, position.x, position.z );
...
u32 CLevelGraph::vertex_id( const Fvector& position ) const {
    VERIFY2( valid_vertex_position( position ),
             make_string( "invalid position for CLevelGraph::vertex_id "
                          "specified: [%f][%f][%f]",
                          VPUSH( position ) ) );

    if ( !valid_vertex_position( position ) ) {
        Msg( "{@} --- Invalid position for CLevelGraph::vertex_id specified: "
             "[%f][%f][%f]",
             VPUSH( position ) );
        return ( u32( -1 ) );
    }
...
u32 CLevelGraph::guess_vertex_id( u32 const& current_vertex_id,
    ...
    for ( u32 i = start_x; i <= stop_x; ++i ) {
        for ( u32 j = start_z; j <= stop_z; ++j ) {
            ...
            float best_distance =
                nearest( best_point, position, vertex_contour );
            for ( ++I; I != E; ++I ) {
                if ( ( *I ).position().xz() != test_xz )
                    break;
