...
CActor* Actor() {
    R_ASSERT2(
        ...
        "Actor() method invokation must be only in Single Player game!" );
    VERIFY( g_actor );
...
void CActor::net_Export( NET_Packet& P ) // export to server
{
    ...
    P.w_float /*w_angle8*/ (
        angle_normalize( r_model_yaw ) ); // Device.vCameraDirection.getH());//
    P.w_float /*w_angle8*/ (
        angle_normalize( unaffected_r_torso.yaw ) ); //(r_torso.yaw);
    P.w_float /*w_angle8*/ (
        angle_normalize( unaffected_r_torso.pitch ) ); //(r_torso.pitch);
    P.w_float /*w_angle8*/ (
        angle_normalize( unaffected_r_torso.roll ) ); //(r_torso.roll);
    P.w_u8( u8( g_Team() ) );
    P.w_u8( u8( g_Squad() ) );
    P.w_u8( u8( g_Group() ) );
    ...
    if ( H_Parent() || ( GameID() == eGameIDSingle ) ||
         ( ( NumItems > 1 ) && OnClient() ) )
        NumItems = 0;
...
static void w_vec_q8( NET_Packet& P,
                      const Fvector& vec,
                      const Fvector& min,
                      const Fvector& max ) {
...
static void r_vec_q8( NET_Packet& P,
                      Fvector& vec,
                      const Fvector& min,
                      const Fvector& max ) {
...
BOOL CActor::net_Spawn( CSE_Abstract* DC ) {
    ...
    /*
            if (OnClient())
            {
                    if (!pStatGraph)
                    {
                            ...
                            pStatGraph->SetRect(0, g_Y, Device.dwWidth, 100,
       0xff000000, 0xff000000); g_Y += 110; if (g_Y > 700) g_Y = 100;
                            pStatGraph->SetGrid(0, 0.0f, 10, 1.0f, 0xff808080,
       0xffffffff); pStatGraph->SetMinMax(0, 10, 300);
    ...
    */
    ...
    // Alun: In theory it will call SwitchNightVision 'true' when outfit or
    // helmet spawn and moved to slot if m_bNightVisionOn is true
    m_bNightVisionOn = !!m_trader_flags.test(
...
void CActor::SetCallbacks() {
    ...
    V->LL_GetBoneInstance( u16( spine0_bone ) )
        .set_callback( bctCustom, Spin0Callback, this );
    V->LL_GetBoneInstance( u16( spine1_bone ) )
        .set_callback( bctCustom, Spin1Callback, this );
    V->LL_GetBoneInstance( u16( shoulder_bone ) )
        .set_callback( bctCustom, ShoulderCallback, this );
    V->LL_GetBoneInstance( u16( head_bone ) )
        .set_callback( bctCustom, HeadCallback, this );
}
...
void CActor::OnChangeVisual() {
    ...
    if ( V ) {
        ...
        m_eye_left =
            smart_cast< IKinematics* >( Visual() )->LL_BoneID( "eye_left" );
        m_eye_right =
            smart_cast< IKinematics* >( Visual() )->LL_BoneID( "eye_right" );
        m_r_hand = smart_cast< IKinematics* >( Visual() )
                       ->LL_BoneID( pSettings->r_string( *cNameSect(),
                                                         "weapon_bone0" ) );
        m_l_finger1 = smart_cast< IKinematics* >( Visual() )
                          ->LL_BoneID( pSettings->r_string( *cNameSect(),
                                                            "weapon_bone1" ) );
        m_r_finger2 = smart_cast< IKinematics* >( Visual() )
                          ->LL_BoneID( pSettings->r_string( *cNameSect(),
                                                            "weapon_bone2" ) );
        //-------------------------------------------------------------------------------
        m_neck =
            smart_cast< IKinematics* >( Visual() )->LL_BoneID( "bip01_neck" );
        m_l_clavicle = smart_cast< IKinematics* >( Visual() )
                           ->LL_BoneID( "bip01_l_clavicle" );
        m_r_clavicle = smart_cast< IKinematics* >( Visual() )
                           ->LL_BoneID( "bip01_r_clavicle" );
        m_spine2 =
            smart_cast< IKinematics* >( Visual() )->LL_BoneID( "bip01_spine2" );
        m_spine1 =
            smart_cast< IKinematics* >( Visual() )->LL_BoneID( "bip01_spine1" );
        m_spine =
...
void ACTOR_DEFS::net_update::lerp( ACTOR_DEFS::net_update& A,
                                   ACTOR_DEFS::net_update& B,
                                   float f ) {
    //	float invf		= 1.f-f;
    //	//
...
void CActor::PH_B_CrPr() // actions & operations before physic
                         // correction-prediction steps
{
    ...
    if ( g_Alive() ) {
        CrPr_SetActivated( true );
        {
            ...
            pIStart->Vel =
                character_physics_support()->movement()->GetVelocity();
            pIStart->o_model = angle_normalize( r_model_yaw );
            pIStart->o_torso.yaw = angle_normalize( unaffected_r_torso.yaw );
            pIStart->o_torso.pitch =
                angle_normalize( unaffected_r_torso.pitch );
...
void CActor::PH_I_CrPr() // actions & operations between two phisic prediction
                         // steps
{
    // store recalculated data, then we able to restore it after small future
    // prediction 	if (!m_bHasUpdate) return;
    if ( !CrPr_IsActivated() )
        return;
    if ( g_Alive() ) {
        ////////////////////////////////////
...
void CActor::CalculateInterpolationParams() {
    ...
    pIStart->Pos				= Position();
    pIStart->Vel				=
    m_PhysicMovementControl->GetVelocity(); pIStart->o_model = r_model_yaw;
    ...
    */
    ...
    if ( m_bInInterpolation ) {
        ...
        float factor = float( CurTime - m_dwIStartTime ) /
                       ( m_dwIEndTime - m_dwIStartTime );
        if ( factor > 1.0f )
            factor = 1.0f;

        float c = factor;
        for ( u32 k = 0; k < 3; k++ ) {
            SP0[ k ] = c * ( c * ( c * SCoeff[ k ][ 0 ] + SCoeff[ k ][ 1 ] ) +
                             SCoeff[ k ][ 2 ] ) +
                       SCoeff[ k ][ 3 ];
            SP1[ k ] = ( c * c * SCoeff[ k ][ 0 ] * 3 +
                         c * SCoeff[ k ][ 1 ] * 2 + SCoeff[ k ][ 2 ] ) /
                       3; //     3       !!!!

            HP0[ k ] = c * ( c * ( c * HCoeff[ k ][ 0 ] + HCoeff[ k ][ 1 ] ) +
                             HCoeff[ k ][ 2 ] ) +
                       HCoeff[ k ][ 3 ];
            HP1[ k ] = ( c * c * HCoeff[ k ][ 0 ] * 3 +
                         c * HCoeff[ k ][ 1 ] * 2 + HCoeff[ k ][ 2 ] ) /
                       3; //     3       !!!!
        };

        SP1.add( SP0 );
    } else {
        if ( LastState.linear_vel.x == 0 && LastState.linear_vel.y == 0 &&
             LastState.linear_vel.z == 0 ) {
            HP1.sub( RecalculatedState.position,
                     RecalculatedState.previous_position );
        } else {
    ...
    */
    ...
    u32 ConstTime =
        u32( ( fixed_step - physics_world()->FrameTime() ) * 1000 ) +
    ...
    if ( g_Alive() && m_bInInterpolation ) {
        ...
        if ( CurTime >= m_dwIEndTime ) {
            ...
            if ( m_dwIEndTime != m_dwIStartTime )
                factor = float( CurTime - m_dwIStartTime ) /
                         ( m_dwIEndTime - m_dwIStartTime );
            ...
            //			r_model_yaw		= angle_lerp
            //(IStart.o_model,IEnd.o_model,		factor);
            unaffected_r_torso.yaw =
                angle_lerp( IStart.o_torso.yaw, IEnd.o_torso.yaw, factor );
            unaffected_r_torso.pitch =
                angle_lerp( IStart.o_torso.pitch, IEnd.o_torso.pitch, factor );
            unaffected_r_torso.roll =
                angle_lerp( IStart.o_torso.roll, IEnd.o_torso.roll, factor );

            for ( u32 k = 0; k < 3; k++ ) {
                IPosL[ k ] = NewPos[ k ];
                IPosS[ k ] = factor * ( factor * ( factor * SCoeff[ k ][ 0 ] +
                                                   SCoeff[ k ][ 1 ] ) +
                                        SCoeff[ k ][ 2 ] ) +
                             SCoeff[ k ][ 3 ];
                IPosH[ k ] = factor * ( factor * ( factor * HCoeff[ k ][ 0 ] +
                                                   HCoeff[ k ][ 1 ] ) +
                                        HCoeff[ k ][ 2 ] ) +
                             HCoeff[ k ][ 3 ];
            };
            ...
            switch ( g_cl_InterpolationType ) {
                case 0: {
                    ...
                    SpeedVector.div( float( m_dwIEndTime - m_dwIStartTime ) /
                                     1000.0f );
                } break;
                case 1: {
                    for ( int k = 0; k < 3; k++ )
                        SpeedVector[ k ] =
                            ( factor * factor * SCoeff[ k ][ 0 ] * 3 +
                              factor * SCoeff[ k ][ 1 ] * 2 +
                              SCoeff[ k ][ 2 ] ) /
                            3; //     3       !!!!
...
std::lower_bound(B,E,u64(ph_world->m_steps_num-1)); if (I != E)
        ...
        {
                SMemoryPosStack.push_back(SMemoryPos(Time0, Time1,
ph_world->m_steps_num, State)); if (SMemoryPosStack.front().dwTime0 <
(Level().timeServer() - 2000)) SMemoryPosStack.pop_front();
        };
};
...
void CActor::load( IReader& input_packet ) {
    ...
    input_packet.r_stringZ( g_quick_use_slots[ 0 ],
                            sizeof( g_quick_use_slots[ 0 ] ) );
    input_packet.r_stringZ( g_quick_use_slots[ 1 ],
                            sizeof( g_quick_use_slots[ 1 ] ) );
    input_packet.r_stringZ( g_quick_use_slots[ 2 ],
                            sizeof( g_quick_use_slots[ 2 ] ) );
    input_packet.r_stringZ( g_quick_use_slots[ 3 ],
                            sizeof( g_quick_use_slots[ 3 ] ) );
}
...
void CActor::OnRender_Network() {
    ...
    if ( g_Alive() ) {
        if ( dbg_net_Draw_Flags.test( dbg_draw_autopickupbox ) ) {
            ...
            Level().debug_renderer().draw_aabb( bc, bd.x, bd.y, bd.z,
        ...
        if ( dbg_net_Draw_Flags.test( dbg_draw_actor_alive ) && V ) {
            if ( this != Level().CurrentViewEntity() ||
                ...
                BoneMatrixRes.mul(V->LL_GetTransform(i), BoneMatrix);
                        ...
                        Level().debug_renderer().draw_obb(BoneMatrix,
                BoneOBB.m_halfsize, color_rgba(0, 255, 0, 255));
                };
                */
                CCF_Skeleton* Skeleton =
                    smart_cast< CCF_Skeleton* >( collidable.model );
                if ( Skeleton ) {
                    Skeleton->_dbg_refresh();

                    const CCF_Skeleton::ElementVec& Elements =
                        Skeleton->_GetElements();
                    for ( CCF_Skeleton::ElementVec::const_iterator I =
                              Elements.begin();
                          I != Elements.end(); I++ ) {
                        if ( !I->valid() )
                            continue;
                        switch ( I->type ) {
                            case SBoneShape::stBox: {
                                ...
                                Level().debug_renderer().draw_obb(
                                    M, h_size, color_rgba( 0, 255, 0, 255 ) );
                            } break;
                            case SBoneShape::stCylinder: {
                                ...
                                Fvector h_size;
                                h_size.set( I->c_cylinder.m_radius,
                                            I->c_cylinder.m_radius,
                                            I->c_cylinder.m_height * 0.5f );
                                Fvector::generate_orthonormal_basis( M.k, M.j,
                                                                     M.i );
                                Level().debug_renderer().draw_obb(
                                    M, h_size, color_rgba( 0, 127, 255, 255 ) );
                            } break;
                            case SBoneShape::stSphere: {
                                Fmatrix l_ball;
                                l_ball.scale( I->s_sphere.R, I->s_sphere.R,
                                              I->s_sphere.R );
                                l_ball.translate_add( I->s_sphere.P );
                                Level().debug_renderer().draw_ellipse(
                                    l_ball, color_rgba( 0, 255, 0, 255 ) );
                            } break;
                        };
                    };
                }
        ...
        dbg_draw_piramid(
            Position(), character_physics_support()->movement()->GetVelocity(),
            size, -r_model_yaw, color_rgba( 128, 255, 128, 255 ) );
        dbg_draw_piramid( IStart.Pos, IStart.Vel, size, -IStart.o_model,
        ...
        Fvector point0S, point1S, point0H, point1H, point0L, point1L,
            *ppoint0 = NULL, *ppoint1 = NULL;
        Fvector tS, tH;
        ...
        for ( int i = 0; i < 11; i++ ) {
            ...
            for ( u32 k = 0; k < 3; k++ ) {
                point1S[ k ] =
                    c * ( c * ( c * SCoeff[ k ][ 0 ] + SCoeff[ k ][ 1 ] ) +
                          SCoeff[ k ][ 2 ] ) +
                    SCoeff[ k ][ 3 ];
                point1H[ k ] =
                    c * ( c * ( c * HCoeff[ k ][ 0 ] + HCoeff[ k ][ 1 ] ) +
                          HCoeff[ k ][ 2 ] ) +
                    HCoeff[ k ][ 3 ];
                point1L[ k ] =
                    IStart.Pos[ k ] + c * ( IEnd.Pos[ k ] - IStart.Pos[ k ] );
            };
            if ( i != 0 ) {
                Level().debug_renderer().draw_line( *pM, *ppoint0, *ppoint1,
                                                    cColor );
            };
            point0S.set( point1S );
            point0H.set( point1H );
        ...
        for ( i = 0; i < 2; i++ ) {
            ...
            for ( u32 k = 0; k < 3; k++ ) {
                ...
                tS[ k ] = ( c * c * SCoeff[ k ][ 0 ] * 3 +
                            c * SCoeff[ k ][ 1 ] * 2 + SCoeff[ k ][ 2 ] ) /
                          3; //     3       !!!!
                tH[ k ] = ( c * c * HCoeff[ k ][ 0 ] * 3 +
                            c * HCoeff[ k ][ 1 ] * 2 + HCoeff[ k ][ 2 ] );
            };
        ...
        if ( !pLastPos->empty() ) {
            ...
            for ( ; It != pLastPos->end(); It++ ) {
                ...
                Level().debug_renderer().draw_aabb( Pos2, size / 5, size / 5,
                                                    size / 5, sColor );
                Pos1 = *It;
            };
        };
        ...
        //(PS, size, size, size, color_rgba(128, 128, 255, 255));
        //		Level().debug_renderer().draw_aabb
        //(PH, size, size, size, color_rgba(255, 128, 128, 255));
        /////////////////////////////////////////////////////////////////////////////////
    } else {
        if ( !( dbg_net_Draw_Flags.is_any( dbg_draw_actor_dead ) ) )
            return;
        ...
        if ( !m_States.empty() ) {
            ...
            if ( !g_Alive() && PHGetSyncItemsNumber() > 2 ) {
                ...
                Level().debug_renderer().draw_aabb(
                    LC, LS.x, LS.y, LS.z, color_rgba( 255, 128, 128, 255 ) );
                //-----------------------------------------------------------------
            };
        }
    }
...
void CActor::SetHitInfo( CObject* who,
                         CObject* weapon,
                         s16 element,
                         Fvector Pos,
                         Fvector Dir ) {
...
#ifndef MASTER_GOLD
        Msg( "On hit health loss of actor[%d], last hitter[%d]", ID(),
             m_iLastHitterID );
...
void CActor::OnCriticalHitHealthLoss() {
...
#ifdef DEBUG
    Msg( "%s killed by hit from %s %s", *cName(),
         ( pLastHitter ? *( pLastHitter->cName() ) : "" ),
         ( ( pLastHittingWeapon && pLastHittingWeapon != pLastHitter )
               ? *( pLastHittingWeapon->cName() )
               : "" ) );
#endif
    //-------------------------------------------------------------------
    if ( m_iLastHitterID != u16( -1 ) ) {
    ...
    if ( m_s16LastHittedElement > 0 ) {
        if ( m_s16LastHittedElement == m_head ) {
            CWeaponMagazined* pWeaponMagazined =
                smart_cast< CWeaponMagazined* >( pLastHittingWeapon );
            if ( pWeaponMagazined ) {
        ...
        } else if ( ( m_s16LastHittedElement == m_eye_left ) ||
                    ( m_s16LastHittedElement == m_eye_right ) ) {
            ...
            while ( ParentBone ) {
                ParentBone =
                    pKinematics->LL_GetData( ParentBone ).GetParentID();
    ...
    P.w_u16(
        ( m_iLastHittingWeaponID && m_iLastHitterID != m_iLastHittingWeaponID )
...
void CActor::OnPlayHeadShotParticle( NET_Packet P ) {
    ...
    //  particles
    intrusive_ptr< CParticlesObject > ps =
        Particles::Details::Create( m_sHeadShotParticle.c_str(), TRUE );
