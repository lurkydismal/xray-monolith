...
void CActor::camUpdateLadder( float dt ) {
    ...
    IElevatorState* es =
        character_physics_support()->movement()->ElevatorState();
    ...
    switch ( cam_freelook ) {
        ...
        case eflEnabling: {
            ...
            if ( C->lim_pitch.similar( { -1.5, 1.5 } ) ) {
                ...
                // Fix camera jump if freelook key is pressed right after
                // loading a save.
                ...
                if ( cam_pitch < -f_Freelook_cam_limit_p ) {
                    float diff_p = angle_difference(
                        cam_pitch, -f_Freelook_cam_limit_p + .05f );
                    if ( diff_p < .025 )
                    ...
                    C->lim_pitch.set( -f_Freelook_cam_limit_p,
                                      f_Freelook_cam_limit_p );
...
bool CActor::CanUseFreelook() {
    ...
    Estate state =
        character_physics_support()->movement()->ElevatorState()->State();
    if ( ( state != clbNoLadder && state != clbNone && state != clbNoState ) ||
         m_holder )
        return false;
        ...
        if ( msl && msl->GetState() > CHudItem::eLastBaseState )
            ...
            switch ( state ) {
                ...
                case CWeapon::eReload:
                    return g_freelook_while_reloading;
                default:
                    return true; // If it's none of the states mentioned above
                                 // it would return true on line 278 anyways.
            }
        }
    }
...
ICF void calc_gl_point( Fvector& pt,
                        const Fmatrix& xform,
                        float radius,
                        float angle ) {
...
ICF BOOL test_point( const Fvector& pt,
                     xrXRC& xrc,
                     const Fmatrix33& mat,
                     const Fvector& ext ) {
    ...
    for ( ; it != end; it++ ) {
        CDB::RESULT& O = *it;
        if ( GMLib.GetMaterialByIdx( O.material )
                 ->Flags.is( SGameMtl::flPassable ) )
            continue;
...
IC bool test_point( const Fvector& pt,
                    const Fmatrix33& mat,
                    const Fvector& ext,
                    CActor* actor ) {
    ...
    fmat.k.set( mat.k );
    fmat.c.set( pt );
    // IPhysicsShellHolder * ve = smart_cast<IPhysicsShellHolder*> (
    // Level().CurrentEntity() ) ;
    VERIFY( actor );
...
void dbg_draw_viewport( const T& cam_info, float _viewport_near ) {
    ...
    const Fvector near_plane_center = Fvector().mad(
        cam_info.Position(), cam_info.Direction(), _viewport_near );
    float h_w, h_h;
    ...
    const Fvector top_left =
        Fvector().sub( near_plane_center, right ).add( up );
    const Fvector top_right =
        Fvector().add( near_plane_center, right ).add( up );
    const Fvector bottom_left =
        Fvector().sub( near_plane_center, right ).sub( up );
    const Fvector bottom_right =
        Fvector().add( near_plane_center, right ).sub( up );
    ...
    DBG_DrawLine( cam_info.Position(), bottom_left,
                  D3DCOLOR_XRGB( 255, 0, 0 ) );
    DBG_DrawLine( cam_info.Position(), bottom_right,
                  D3DCOLOR_XRGB( 255, 0, 0 ) );
...
IC void get_cam_oob( Fvector& bc,
                     Fvector& bd,
                     Fmatrix33& mat,
                     const Fmatrix& xform,
...
IC void get_cam_oob( Fvector& bd,
                     Fmatrix& mat,
                     const Fmatrix& xform,
                     const SRotation& r_torso,
                     float alpha,
...
void CActor::cam_Update( float dt, float fFOV ) {
    ...
    if ( this == Level().CurrentControlEntity() ) {
        if ( eacFirstEye == cam_active ) {
            CHudItem* pItem =
                smart_cast< CHudItem* >( inventory().ActiveItem() );
            CHudItem* pDevice = smart_cast< CHudItem* >(
                inventory().ItemFromSlot( DETECTOR_SLOT ) );

            if ( pItem && pItem->IsAttachedToHUD() && pDevice &&
                 pDevice->IsAttachedToHUD() )
    ...
    if ( GamePersistent().GameType() != eGameIDSingle && ik_cam_shift &&
        ...
        current_ik_cam_shift = cam_smooth_k * current_ik_cam_shift +
                               y_shift * ( 1.f - cam_smooth_k );
    } else
    ...
    if ( CurrentHeight != CameraHeight() ) {
        CurrentHeight =
            ( CurrentHeight * ( 1.0f - HeightInterpolationSpeed * dt ) ) +
    ...
    if ( cam_active == eacFirstEye ) {
        bool firstPersonDeathDiedNow =
            firstPersonDeath && !g_Alive() && m_FPCam;
        if ( firstPersonDeathDiedNow ) {
            ...
            camDir.set( adjustedCamDir.getH(), adjustedCamDir.getP(),
            ...
            Fvector::generate_orthonormal_basis_normalized( dir, dirUp,
                                                            dirRight );
    ...
    fCurAVelocity =
        vPrevCamDir.sub( cameras[ eacFirstEye ]->vDirection ).magnitude() /
        Device.fTimeDelta;
    vPrevCamDir = cameras[ eacFirstEye ]->vDirection;

    // Высчитываем разницу между предыдущим и текущим Yaw \ Pitch от 1-го лица
    // //--#SM+ Begin#--
    ...
    fFPCamYawMagnitude =
        freelook_last_frame
            ? 0.f
            : ( 1.f - freelook_cam_control ) *
                  ( angle_difference_signed( cam_yaw_prev, cam_yaw_cur ) /
                    Device.fTimeDelta ); // L+ / R-
    fFPCamPitchMagnitude =
        freelook_last_frame
            ? 0.f
            : ( 1.f - freelook_cam_control ) *
                  ( angle_difference_signed( cam_pitch_prev, cam_pitch_cur ) /
                    Device.fTimeDelta ); // U+ / D-
...
#ifdef DEBUG
void dbg_draw_frustum( float FOV,
                       float _FAR,
                       float A,
                       Fvector& P,
                       Fvector& D,
