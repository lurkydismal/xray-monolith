...
#define CGameLocationSelector \
    CBaseLocationSelector< CGameGraph, _VertexEvaluator, _vertex_id_type >

TEMPLATE_SPECIALIZATION
IC CGameLocationSelector::CBaseLocationSelector(
    CRestrictedObject* object,
    CLocationManager* location_manager )
    : inherited( object ) {
...
TEMPLATE_SPECIALIZATION
IC void CGameLocationSelector::set_selection_type(
...
TEMPLATE_SPECIALIZATION
IC void CGameLocationSelector::select_location(
...
IC void CGameLocationSelector::select_random_location(
    ...
    const GameGraph::TERRAIN_VECTOR& vertex_types =
        m_location_manager->vertex_types();
    ...
    for ( ; i != e; ++i ) {
        ...
        if ( ( m_graph->vertex( ( *i ).vertex_id() )->level_id() !=
        ...
        const u8* curr_types =
            m_graph->vertex( ( *i ).vertex_id() )->vertex_type();
    ...
    if ( !branch_factor ) {
        if ( ( start_vertex_id != m_previous_vertex_id ) &&
             accessible( m_previous_vertex_id ) )
...
TEMPLATE_SPECIALIZATION
IC bool CGameLocationSelector::actual( const _vertex_id_type start_vertex_id,
                                       bool path_completed ) {
...
TEMPLATE_SPECIALIZATION
IC bool CGameLocationSelector::accessible(
    const _vertex_id_type vertex_id ) const {
    return ( m_restricted_object
                 ? m_restricted_object->accessible(
                       m_graph->vertex( vertex_id )->level_vertex_id() )
                 : true );
}
