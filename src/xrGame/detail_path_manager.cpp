...
bool CDetailPathManager::valid() const {
    ...
    if ( m_state_patrol_path )
        b = b && ( fis_zero(
                     m_path[ m_last_patrol_point ].position.distance_to_xz_sqr(
...
Fvector CDetailPathManager::direction() const {
    if ( ( m_path.size() < 2 ) ||
         ( m_path.size() <= m_current_travel_point + 1 ) )
        return ( Fvector().set( 0, 0, 1 ) );

    Fvector direction;
    direction.sub( m_path[ m_current_travel_point + 1 ].position,
                   m_path[ m_current_travel_point ].position );
...
void CDetailPathManager::build_path( const xr_vector< u32 >& level_path,
    ...
    if ( valid( m_start_position ) && valid( m_dest_position ) ) {
        ...
        if ( failed() ) {
#ifndef MASTER_GOLD
            Msg( "! DetailPathManager has failed : from [%f,%f,%f] to "
                 "[%f,%f,%f]",
                 VPUSH(
                     ai().level_graph().vertex_position( level_path.front() ) ),
                 VPUSH( ai().level_graph().vertex_position(
                     level_path.back() ) ) );
#endif // #ifndef MASTER_GOLD
#ifdef DEBUG
            Msg( "! DetailPathManager has failed for object %s : from "
                 "[%f,%f,%f] to [%f,%f,%f]",
                 m_restricted_object ? *m_restricted_object->object().cName()
                                     : "unknown",
                 VPUSH(
                     ai().level_graph().vertex_position( level_path.front() ) ),
                 VPUSH( ai().level_graph().vertex_position(
            ...
            xr_vector< STravelParamsIndex >::const_iterator I =
                m_start_params.begin();
            xr_vector< STravelParamsIndex >::const_iterator E =
                m_start_params.end();
            for ( ; I != E; ++I )
                Msg( "[%d] : [%f][%f]", ( *I ).index, ( *I ).linear_velocity,
                     ( *I ).angular_velocity );
...
void CDetailPathManager::update_distance_to_target() {
    ...
    xr_vector< STravelPathPoint >::const_iterator I =
        path().begin() + curr_travel_point_index() + 1;
...
void CDetailPathManager::on_travel_point_change(
...
u32 CDetailPathManager::location_on_path( const CGameObject* object,
                                          float distance,
