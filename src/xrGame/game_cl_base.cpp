...
void game_cl_GameState::net_import_GameTime( NET_Packet& P ) {
    ...
    Level().SetEnvironmentGameTimeFactor( GameEnvironmentTime,
...
struct not_exsiting_clients_deleter {
    ...
    not_exsiting_clients_deleter( existing_clients_vector_t* exist,
                                  game_PlayerState** local_player,
                                  ClientID* client_id )
        : exist_clients( exist ),
    ...
    bool operator()( game_cl_GameState::PLAYERS_MAP::value_type& value ) {
        ...
        existing_clients_vector_t::iterator tmp_iter =
            std::find( exist_clients->begin(), exist_clients->end(),
                       value.first // key
            );
...
void game_cl_GameState::net_import_state( NET_Packet& P ) {
    ...
    buffer_vector< ClientID > valid_players(
        _alloca( sizeof( ClientID ) * ( p_count + 1 ) ), ( p_count + 1 ) );
    ...
    for ( u16 p_it = 0; p_it < p_count; ++p_it ) {
        ...
        if ( I != players.end() ) {
            ...
            {
                game_PlayerState::skip_Import(
                    P ); // this mean that local_player not created yet ..
                continue;
            }
    ...
    not_exsiting_clients_deleter tmp_deleter( &valid_players, &local_player,
                                              &local_svdpnid );

    players.erase(
        std::remove_if( players.begin(), players.end(), tmp_deleter ),
        players.end() );
...
void game_cl_GameState::net_import_update( NET_Packet& P ) {
    ...
    /*VERIFY2(I != players.end(),
            make_string("Player ClientID = %d not found in players map",
       ID.value()).c_str());*/
    if ( players.end() != I ) {
...
void game_cl_GameState::TranslateGameMessage( u32 msg, NET_Packet& P ) {
    ...
    switch ( msg ) {
        case GAME_EVENT_PLAYER_CONNECTED: {
            ...
            xr_sprintf( Text, "%s%s %s%s", Color_Teams[ 0 ], PS->getName(),
                        Color_Main, *st.translate( "mp_connected" ) );
            if ( CurrentGameUI() )
        ...
        case GAME_EVENT_PLAYER_DISCONNECTED: {
            ...
            xr_sprintf( Text, "%s%s %s%s", Color_Teams[ 0 ], PlayerName,
                        Color_Main, *st.translate( "mp_disconnected" ) );
            if ( CurrentGameUI() )
                CurrentGameUI()->CommonMessageOut( Text );
            //---------------------------------------
        ...
        case GAME_EVENT_PLAYER_ENTERED_GAME: {
            ...
            xr_sprintf( Text, "%s%s %s%s", Color_Teams[ 0 ], PlayerName,
                        Color_Main, *st.translate( "mp_entered_game" ) );
            if ( CurrentGameUI() )
                CurrentGameUI()->CommonMessageOut( Text );
        } break;
        default: {
...
void game_cl_GameState::set_type_name( LPCSTR s ) {
    ...
    if ( OnClient() ) {
        xr_strcpy( g_pGamePersistent->m_game_params.m_game_type,
                   m_game_type_name.c_str() );
