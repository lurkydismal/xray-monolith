...
#include "PHDebug.h"
#endif
moving_bones_snd_player::moving_bones_snd_player( IKinematics* K,
                                                  CInifile* ini,
                                                  LPCSTR section,
                                                  const Fmatrix& object )
    : bone_id( BI_NONE ),
      min_factor( -FLT_MAX ),
      max_factor( -FLT_MAX ),
...
void moving_bones_snd_player::update( float time_delta, CGameObject& object ) {
    ...
    smothed_velocity =
        smothed_velocity * sm_factor + aw_speed * ( 1.f - sm_factor );
    ...
    if ( dbg_moving_bones_snd_player ) {
        ...
        DBG_OutText( "velocity factor = base_speed/smoothed speed=       : %f",
                     factor );
        DBG_OutText(
            "min_factor=						"
            "				 : %f",
        ...
        // Msg( "smoothed angular speed: %f, angular speed: %f, factor: %f time
        // delta: %f ", smothed_velocity, aw_speed, factor, time_delta );
    }
#endif
...
moving_bones_snd_player* create_moving_bones_snd_player( CInifile* ini,
                                                         IKinematics& K,
                                                         const Fmatrix& obj ) {
    if ( !ini || !ini->section_exist( "moving_bones_snd_player" ) )
        return 0;
    return xr_new< moving_bones_snd_player >( &K, ini,
                                              "moving_bones_snd_player", obj );
}

moving_bones_snd_player* create_moving_bones_snd_player( CGameObject& O ) {
    IKinematics* K = smart_cast< IKinematics* >( O.Visual() );
    VERIFY( K );
    moving_bones_snd_player* ret =
        create_moving_bones_snd_player( O.spawn_ini(), *K, O.XFORM() );
    if ( ret )
        return ret;
    return create_moving_bones_snd_player( K->LL_UserData(), *K, O.XFORM() );
}

void moving_bones_snd_player::load( IKinematics& K,
                                    CInifile& ini,
                                    LPCSTR section,
                                    const Fmatrix& object ) {
    ...
    VERIFY2(
        min_factor > 0.f,
        make_string( "moving_bones_snd_player: bad params: min_factor: %f < 0 ",
                     min_factor ) );
    VERIFY2(
        max_factor > 0.f,
        make_string( "moving_bones_snd_player: bad params: max_factor: %f < 0 ",
                     max_factor ) );
    VERIFY2( max_factor > 0.f,
             make_string(
                 "moving_bones_snd_player: bad params: base_velocity: %f < 0 ",
                 base_velocity ) );
    VERIFY2( min_factor <= max_factor,
             make_string( "moving_bones_snd_player: bad params: min_factor %f "
                          "> max_factor %f, ",
