...
bool CActor::feel_touch_contact( CObject* O ) {
    ...
    if ( inventory_owner &&
         inventory_owner != smart_cast< CInventoryOwner* >( this ) ) {
        // CPhysicsShellHolder* sh=smart_cast<CPhysicsShellHolder*>(O);
...
ICF static BOOL info_trace_callback( collide::rq_result& result,
                                     LPVOID params ) {
    ...
    if ( result.O ) {
        ...
        if ( GMLib.GetMaterialByIdx( T->material )
...
BOOL CActor::CanPickItem( const CFrustum& frustum,
                          const Fvector& from,
                          CObject* item ) {
    if ( !item->getVisible() )
        return FALSE;
...
CActor::pickup_result_t CActor::PickupModeUpdate() {
    if ( !m_bPickupMode )
        return { true, false }; // kUSE key pressed
    if ( !IsGameTypeSingle() )
        return { true, false };
    ...
    if ( m_pObjectWeLookingAt && m_pObjectWeLookingAt->cast_inventory_item() &&
        ...
        // Tronex: ability to prevent item picking up if the export returns
        // false
        ::luabind::functor< bool > func;
        if ( ai().script_engine().functor(
                 "bind_stalker_ext.actor_on_item_before_pickup", func ) ) {
...
void CActor::PickupModeUpdate_COD( pickup_result_t pickup_result ) {
    ...
    if ( !g_Alive() || eacFirstEye != cam_active ||
         !psDeviceFlags2.test( rsCODPickup ) ) {
    ...
    for ( u32 o_it = 0; o_it < ISpatialResult.size(); o_it++ ) {
        ...
        CInventoryItem* pIItem =
            smart_cast< CInventoryItem* >( spatial->dcast_CObject() );
        ...
        CGrenade* pGrenade =
            smart_cast< CGrenade* >( spatial->dcast_CObject() );
        if ( pGrenade && !pGrenade->Useful() )
            continue;

        CMissile* pMissile =
            smart_cast< CMissile* >( spatial->dcast_CObject() );
        ...
        B.mad( cam_Active()->vPosition, cam_Active()->vDirection,
               tmp.dotproduct( cam_Active()->vDirection ) );
    ...
    if ( pNearestItem ) {
        ...
        frustum.CreateFromMatrix( Device.mFullTransform,
                                  FRUSTUM_P_LRTB | FRUSTUM_P_FAR );
        if ( !CanPickItem( frustum, Device.vCameraPosition,
                           &pNearestItem->object() ) )
            pNearestItem = NULL;
    }
    if ( pNearestItem && pNearestItem->cast_game_object() ) {
        if ( Level().m_feel_deny.is_object_denied(
                 pNearestItem->cast_game_object() ) )
            pNearestItem = NULL;
    }
    if ( pNearestItem && pNearestItem->cast_game_object() ) {
...

    {
        ...
        if ( ai().script_engine().functor( "_G.CPickupModeUpdate_COD",
                                           func ) ) {
            func( pNearestItem && pNearestItem->cast_game_object()
                      ? pNearestItem->cast_game_object()->lua_game_object()
                      : nullptr );
        }
    }
    ...
    if ( pNearestItem && m_bPickupMode ) {
        ...
        if ( !pickup_result.callback_handled ) {
            ...
            if ( ai().script_engine().functor(
                     "bind_stalker_ext.actor_on_item_before_pickup", func ) ) {
                bool allow_pickup =
                    func( pNearestItem->cast_game_object()->lua_game_object() );
        ...
        CUsableScriptObject* pUsableObject =
            smart_cast< CUsableScriptObject* >( pNearestItem );
...
void CActor::Check_for_AutoPickUp() {
    ...
    Fbox APU_Box;
    APU_Box.set( Fvector().sub( bc, m_AutoPickUp_AABB ),
                 Fvector().add( bc, m_AutoPickUp_AABB ) );

    xr_vector< ISpatialShared > ISpatialResult;
    g_SpatialSpace->q_box( ISpatialResult, 0, STYPE_COLLIDEABLE, bc,
                           m_AutoPickUp_AABB );
    ...
    for ( u32 o_it = 0; o_it < ISpatialResult.size(); o_it++ ) {
        ...
        if ( APU_Box.Pick( pIItem->object().Position(),
                           pIItem->object().Position() ) ) {
            if ( GameID() == eGameIDDeathmatch ||
                 GameID() == eGameIDTeamDeathmatch ) {
...
void CActor::feel_sound_new( CObject* who,
                             int type,
