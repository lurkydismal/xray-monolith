...
CStalkerPropertyEvaluatorALife::CStalkerPropertyEvaluatorALife(
    CAI_Stalker* object,
...
CStalkerPropertyEvaluatorAlive::CStalkerPropertyEvaluatorAlive(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorItems::CStalkerPropertyEvaluatorItems(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorEnemies::evaluate() {
    ...
    if ( Device.dwTimeGlobal <
         m_object->memory().enemy().last_enemy_time() + m_time_to_wait )
        return ( true );
...
CStalkerPropertyEvaluatorSeeEnemy::CStalkerPropertyEvaluatorSeeEnemy(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorEnemySeeMe::CStalkerPropertyEvaluatorEnemySeeMe(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorEnemySeeMe::evaluate() {
    ...
    const CCustomMonster* enemy_monster =
        smart_cast< const CCustomMonster* >( enemy );
    if ( enemy_monster )
...
CStalkerPropertyEvaluatorItemToKill::CStalkerPropertyEvaluatorItemToKill(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorItemCanKill::CStalkerPropertyEvaluatorItemCanKill(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorFoundItemToKill::
    CStalkerPropertyEvaluatorFoundItemToKill( CAI_Stalker* object,
                                              LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorFoundAmmo::CStalkerPropertyEvaluatorFoundAmmo(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorReadyToKillSmartCover::
    CStalkerPropertyEvaluatorReadyToKillSmartCover( CAI_Stalker* object,
                                                    LPCSTR evaluator_name,
                                                    u32 min_ammo_count )
    : inherited( object, evaluator_name, min_ammo_count ) {}

_value_type CStalkerPropertyEvaluatorReadyToKillSmartCover::evaluate() {
    if ( m_object->movement().current_params().cover() &&
         !m_object->movement().current_params().cover()->can_fire() )
        return ( true );
...
CStalkerPropertyEvaluatorAnomaly::CStalkerPropertyEvaluatorAnomaly(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorAnomaly::evaluate() {
    ...
    // infinite/super long bolts throwing issue workaround: do not start another
    // cycle exclusion: while physically inside a zone so anomaly avoidance
    // keeps working
    if ( !m_object->inside_anomaly() &&
         Device.dwTimeGlobal < m_object->m_anomaly_detect_suppress_until )
        return ( false );
    ...
    u32 result =
        dwfChooseAction( 2000, m_object->panic_threshold(), 0.f, 0.f, 0.f,
                         m_object->g_Team(), m_object->g_Squad(),
                         m_object->g_Group(), 0, 1, 2, 3, 4, m_object, 300.f );
    return ( !result );
}
...
CStalkerPropertyEvaluatorPanic::CStalkerPropertyEvaluatorPanic(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorSmartTerrainTask::
    CStalkerPropertyEvaluatorSmartTerrainTask( CAI_Stalker* object,
                                               LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorEnemyReached::CStalkerPropertyEvaluatorEnemyReached(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorEnemyReached::evaluate() {
    ...
    ALife::_OBJECT_ID processor_id =
        object().agent_manager().enemy().wounded_processor( enemy );
    if ( processor_id != object().ID() )
        return ( false );

    return ( ( object().Position().distance_to_sqr( enemy->Position() ) <=
...
CStalkerPropertyEvaluatorPlayerOnThePath::
    CStalkerPropertyEvaluatorPlayerOnThePath( CAI_Stalker* object,
                                              LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorEnemyCriticallyWounded::
    CStalkerPropertyEvaluatorEnemyCriticallyWounded( CAI_Stalker* object,
                                                     LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorEnemyCriticallyWounded::evaluate() {
    ...
    const CAI_Stalker* enemy_stalker =
        smart_cast< const CAI_Stalker* >( enemy );
    if ( !enemy_stalker )
        return ( false );

    return (
        const_cast< CAI_Stalker* >( enemy_stalker )->critically_wounded() );
}
...
CStalkerPropertyEvaluatorShouldThrowGrenade::
    CStalkerPropertyEvaluatorShouldThrowGrenade( CAI_Stalker* object,
                                                 LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorShouldThrowGrenade::evaluate() {
    ...
    if ( !m_storage->property( eWorldPropertyInCover ) &&
         !m_storage->property( eWorldPropertyPositionHolded ) &&
         !m_storage->property( eWorldPropertyEnemyDetoured ) )
        return ( false );

    // do not throw grenades too often
    if ( object().last_throw_time() + object().throw_time_interval() >=
         Device.dwTimeGlobal )
        return ( false );
    ...
    // setup throw target
    object().throw_target( position, enemy_vertex_id,
    ...
    if ( object().inventory().ItemFromSlot( GRENADE_SLOT ) ==
    ...
    // Lua hook: allow scripts to cancel the throw (e.g. rank-based grenade
    // denial)
    {
        ::luabind::functor< bool > funct;
        if ( ai().script_engine().functor( "_G.CAI_Stalker__ShouldThrow",
                                           funct ) ) {
...
CStalkerPropertyEvaluatorTooFarToKillEnemy::
    CStalkerPropertyEvaluatorTooFarToKillEnemy( CAI_Stalker* object,
                                                LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorTooFarToKillEnemy::evaluate() {
    ...
    return ( object().too_far_to_kill_enemy(
...
CStalkerPropertyEvaluatorLowCover::CStalkerPropertyEvaluatorLowCover(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
