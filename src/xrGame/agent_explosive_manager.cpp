...
void CAgentExplosiveManager::remove_links( CObject* object ) {
    TO_BE_DESTROYED::iterator I =
        std::find( m_explosives_to_remove.begin(), m_explosives_to_remove.end(),
                   object->ID() );
    if ( I != m_explosives_to_remove.end() )
        m_explosives_to_remove.erase_fast( I );

    EXPLOSIVES::iterator J =
        std::find( m_explosives.begin(), m_explosives.end(), object->ID() );
    if ( J != m_explosives.end() )
        m_explosives.erase_fast( J );
}

void CAgentExplosiveManager::register_explosive(
    const CExplosive* explosive,
    const CGameObject* game_object ) {
    {
        xr_vector< CDangerExplosive >::iterator I =
            std::find( m_explosives.begin(), m_explosives.end(), explosive );
        if ( I != m_explosives.end() )
            return;
    }
    {
        TO_BE_DESTROYED::iterator I =
            std::find( m_explosives_to_remove.begin(),
                       m_explosives_to_remove.end(), game_object->ID() );
        if ( I != m_explosives_to_remove.end() )
            return;
    }

    m_explosives_to_remove.push_back( game_object->ID() );
    m_explosives.push_back(
        CDangerExplosive( explosive, game_object, 0, Device.dwTimeGlobal ) );
    ...
    if ( missile && ( missile->destroy_time() > Device.dwTimeGlobal ) )
        interval = missile->destroy_time() - Device.dwTimeGlobal +
                   AFTER_GRENADE_DESTROYED_INTERVAL;

    object().location().add( xr_new< CDangerObjectLocation >(
        game_object, Device.dwTimeGlobal, interval, GRENADE_RADIUS ) );
}
...
bool CAgentExplosiveManager::process_explosive( CMemberOrder& member ) {
    ...
    for ( ; I != E; ++I ) {
        if ( !member.object().memory().visual().visible_now(
                 ( *I ).m_game_object ) )
            continue;

        float dist_sqr = ( *I ).m_game_object->Position().distance_to_sqr(
            member.object().Position() );
        if ( dist_sqr < min_dist_sqr ) {
            if ( ( *I ).m_reactor &&
                 ( ( *I ).m_reactor->Position().distance_to_sqr(
                       ( *I ).m_game_object->Position() ) <= min_dist_sqr ) )
                continue;
...

    {
        ...
        for ( ; I != E; ++I ) {
            ...
            CMemberOrder::CGrenadeReaction& reaction =
                object().member().member( ( *I ).m_reactor ).grenade_reaction();
        ...
        m_explosives.erase(
            std::remove_if( m_explosives.begin(), m_explosives.end(),
