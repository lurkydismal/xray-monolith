...
#include "../3rd_party/crypto/crypto.h"
...
game_cl_mp::game_cl_mp() {
    ...
    /*	pBuySpawnMsgBox		= xr_new<CUIMessageBoxEx>();
            ...
            pBuySpawnMsgBox->AddCallback("msg_box", MESSAGE_BOX_YES_CLICKED,
       CUIWndCallback::void_function(this, &game_cl_mp::OnBuySpawn)); string1024
       BuySpawnText; xr_sprintf(BuySpawnText, "You can buy a spawn for %d $.
       Press Yes to pay.", abs(m_iSpawn_Cost));
...
bool game_cl_mp::OnKeyboardPress( int key ) {
...
#ifdef DEBUG
            Msg( "---I'm ready (ID = %d) sending player ready packet !!!",
                 GO->ID() );
#endif // #ifdef DEBUG
            NET_Packet P;
...
#ifdef DEBUG
            Msg( "---I'm not ready, is_actor = %d, is_spectator = %d", is_actor,
                 is_spectator );
#endif // #ifdef DEBUG
            return false;
        }
    ...
    if ( ( game_phase != GAME_PHASE_INPROGRESS ) && ( kQUIT != key ) &&
         ( kCONSOLE != key ) && ( kCHAT != key ) &&
         ( kSHOW_ADMIN_MENU != key ) && ( kVOTE_BEGIN != key ) &&
         ( kVOTE != key ) && ( kVOTEYES != key ) && ( kVOTENO != key ) ) {
        return true;
    }
    ...
    if ( ( game_phase == GAME_PHASE_INPROGRESS ) ||
        ...
        switch ( key ) {
            ...
            case kCHAT_TEAM: {
                CUIChatWnd* pChatWnd =
                    m_game_ui_custom->m_pMessagesWnd->GetChatWnd();
                R_ASSERT( !pChatWnd->IsShown() );
                string512 prefix;
                xr_sprintf(
                    prefix, "%s> ",
                    st.translate( ( kCHAT_TEAM == key ) ? "st_mp_say_to_team"
                                                        : "st_mp_say_to_all" )
                        .c_str() );
            ...
            case kSHOW_ADMIN_MENU: {
                ...
                if ( local_player &&
                    ...
                    m_pAdminMenuWindow->ShowMessageBox(
                        CUIMessageBox::MESSAGEBOX_RA_LOGIN );
            } break;
            case kVOTE_BEGIN: {
                if ( IsVotingEnabled() && !IsVotingActive() )
                    VotingBegin();
                else
                    OnCantVoteMsg( st.translate( ( IsVotingEnabled() )
                                                     ? "st_mp_only_one_voting"
                                                     : "st_mp_disabled_voting" )
                                       .c_str() );
            } break;
            case kVOTE: {
                if ( IsVotingEnabled() && IsVotingActive() )
                    ...
                    if ( !IsVotingEnabled() )
                        OnCantVoteMsg(
                            st.translate( "st_mp_disabled_voting" ).c_str() );
                    else
                        OnCantVoteMsg(
                            st.translate( "st_mp_no_current_voting" ).c_str() );
                }
            } break;
            case kVOTEYES: {
                if ( IsVotingEnabled() && IsVotingActive() )
...
u32 Color_Teams_u32[ 3 ] = { color_rgba( 255, 240, 190, 255 ),
                             color_rgba( 64, 255, 64, 255 ),
                             color_rgba( 64, 64, 255, 255 ) };
LPCSTR Color_Teams[ 3 ] = { "%c[255,255,240,190]", "%c[255,64,255,64]",
                            "%c[255,64,64,255]" };
...
void game_cl_mp::TranslateGameMessage( u32 msg, NET_Packet& P ) {
    ...
    switch ( msg ) {
        ...
        case GAME_EVENT_VOTE_START: {
            xr_sprintf( Text, "%s%s", Color_Main,
                        *st.translate( "mp_voting_started_msg" ) );
            if ( CurrentGameUI() )
                CurrentGameUI()->CommonMessageOut( Text );
            OnVoteStart( P );
        } break;
        ...
        case GAME_EVENT_MAKE_DATA: {
            ...
            if ( etype == e_screenshot_request ) {
                ...
                PrepareToReceiveFile( tmp_client, client_name,
                ...
                Msg( "! File transfer error: from client [%u]: %s",
                     tmp_client.value(), error_msg.c_str() );
            }
        } break;
...
void game_cl_mp::ChatSay( LPCSTR phrase, bool bAll ) {
    ...
    P.w_s16( ( bAll ) ? -1
                      : local_player->team ); // -1 = all, 0 = green, 1 = blue
...
void game_cl_mp::OnWarnMessage( NET_Packet* P ) {
    ...
    if ( msg_type == 1 ) {
        ...
        if ( CurrentGameUI() ) {
            ...
            StaticDrawableWrapper* ss =
                CurrentGameUI()->AddCustomStatic( _buff, true );
...
void game_cl_mp::OnChatMessage( NET_Packet* P ) {
    ...
    switch ( team ) {
        case 0:
            Msg( "%s: %s : %s", *st.translate( "mp_chat" ), PlayerName.c_str(),
                 ChatMsg.c_str() );
            break;
        case 1:
            Msg( "- %s: %s : %s", *st.translate( "mp_chat" ),
                 PlayerName.c_str(), ChatMsg.c_str() );
            break;
        case 2:
            Msg( "@ %s: %s : %s", *st.translate( "mp_chat" ),
                 PlayerName.c_str(), ChatMsg.c_str() );
    ...
    if ( Level().CurrentViewEntity() && CurrentGameUI() )
        CurrentGameUI()->m_pMessagesWnd->AddChatMessage( ChatMsg,
                                                         colPlayerName );
};
...
void game_cl_mp::shedule_Update( u32 dt ) {
    ...
    switch ( Phase() ) {
        case GAME_PHASE_PENDING: {
            // CUIChatWnd* pChatWnd =
            // CurrentGameUI()->m_pMessagesWnd->GetChatWnd(); if (pChatWnd &&
            // pChatWnd->IsShown()) 	StartStopMenu(pChatWnd, false);
        ...
        default: {
            CUIChatWnd* pChatWnd =
                CurrentGameUI()->m_pMessagesWnd->GetChatWnd();
    ...
    if ( ( cur_game_state != GAME_PHASE_INPROGRESS ) &&
         ( cur_game_state != GAME_PHASE_PENDING ) ) {
        ...
        if ( m_pVoteRespondWindow &&
             m_pVoteRespondWindow->IsShown() ) // && IsVotingActive())
        {
...
void game_cl_mp::OnPlayerVoted( game_PlayerState* ps ) {
    ...
    CStringTable st;
    string1024 resStr;
    xr_sprintf( resStr, "%s\"%s\" %s%s %s\"%s\"", Color_Teams[ ps->team ],
                ps->getName(), Color_Main, *st.translate( "mp_voted" ),
                ps->m_bCurrentVoteAgreed ? Color_Green : Color_Red,
                *st.translate( ps->m_bCurrentVoteAgreed ? "mp_voted_yes"
                                                        : "mp_voted_no" ) );
    if ( CurrentGameUI() )
...
void game_cl_mp::LoadTeamData( const shared_str& TeamName ) {
    ...
    if ( pSettings->section_exist( TeamName ) ) {
        ...
        LPCSTR ShaderTexture =
            pSettings->r_string( TeamName, "indicator_texture" );
...
void game_cl_mp::OnPlayerKilled( NET_Packet& P ) {
    ...
    if ( m_reward_generator )
        m_reward_generator->OnPlayerKilled(
            KillerID, KilledID, WeaponID,
            std::make_pair( KillType, SpecialKill ) );
    //-----------------------------------------------------------
    ...
    if ( !pPlayer ) {
#ifndef MASTER_GOLD
        Msg( "! Non existant player[%d] killed by [%d] with [%d]", KilledID,
             KillerID, WeaponID );
        ...
        case KT_HIT: // from hit
        {
            ...
            if ( pWeapon ) {
                CInventoryItem* pIItem =
                    smart_cast< CInventoryItem* >( pWeapon );
                if ( pIItem ) {
                    ...
                    if ( smart_cast< CExplosiveItem* >( pIItem ) ) {
                        ...
                        KMS.m_initiator.m_rect.x2 =
                            KMS.m_initiator.m_rect.x1 + 31;
                        KMS.m_initiator.m_rect.y2 =
                            KMS.m_initiator.m_rect.y1 + 30;
                        xr_sprintf( sWeapon,
                                    *st.translate( "mp_by_explosion" ) );
                    } else {
                        KMS.m_initiator.m_rect = pIItem->GetKillMsgRect();
                        KMS.m_initiator.m_rect.rb.add(
                            KMS.m_initiator.m_rect.lt );
                        xr_sprintf( sWeapon, "%s %s",
                                    st.translate( "mp_from" ).c_str(),
                                    pIItem->NameShort() );
                    }
                } else {
                    CCustomZone* pAnomaly =
                        smart_cast< CCustomZone* >( pWeapon );
                    if ( pAnomaly ) {
                        KMS.m_initiator.m_shader = GetKillEventIconsShader();
                        KMS.m_initiator.m_rect.x1 = 1;
                        KMS.m_initiator.m_rect.y1 = 202;
            ...
            if ( pKiller || pOKiller ) {
                if ( !pKiller ) {
                    CCustomZone* pAnomaly =
                        smart_cast< CCustomZone* >( pOKiller );
                    if ( pAnomaly ) {
                        KMS.m_initiator.m_shader = GetKillEventIconsShader();
                        KMS.m_initiator.m_rect.x1 = 1;
                        KMS.m_initiator.m_rect.y1 = 202;
                ...
                if ( pKiller ) {
                    KMS.m_killer.m_name = pKiller ? pKiller->getName()
                                                  : *( pOKiller->cNameSect() );
                    KMS.m_killer.m_color =
                        pKiller
                            ? Color_Teams_u32[ ModifyTeam( pKiller->team ) + 1 ]
                            : Color_Neutral_u32;
                };
            };
            //-------------------------------------------
            switch ( SpecialKill ) {
                ...
                {
                    BONUSES_it it = std::find( m_pBonusList.begin(),
                                               m_pBonusList.end(), "headshot" );
                    if ( it != m_pBonusList.end() && ( *it == "headshot" ) ) {
                        ...
                        KMS.m_ext_info.m_rect.x2 =
                            pBS->IconRects[ 0 ].x1 + pBS->IconRects[ 0 ].x2;
                        KMS.m_ext_info.m_rect.y2 =
                            pBS->IconRects[ 0 ].y1 + pBS->IconRects[ 0 ].y2;
                    };
            ...
            if ( KilledID == KillerID ) {
                ...
                //-------------------------------------
                Msg( sWeapon[ 0 ] ? "%s killed himself by %s"
                                  : "%s killed himself",
                     *KMS.m_killer.m_name, sWeapon[ 0 ] ? sWeapon + 5 : "" );
            } else {
                //-------------------------------------
                Msg( "%s killed %s %s%s", *KMS.m_killer.m_name,
                     *KMS.m_victim.m_name, sWeapon,
                     sSpecial[ 0 ] ? sSpecial : "" );
            }
        } break;
            //-----------------------------------------------------------
        case KT_BLEEDING: // from bleeding
        {
            ...
            if ( !pKiller ) {
                ...
                if ( pAnomaly ) {
                    ...
                    Msg( "%s died from bleeding, thanks to anomaly",
                         *KMS.m_victim.m_name );
                    break;
                }
            };
            ...
            if ( pKiller ) {
                KMS.m_killer.m_name =
                    pKiller ? pKiller->getName() : *( pOKiller->cNameSect() );
                KMS.m_killer.m_color =
                    pKiller ? Color_Teams_u32[ ModifyTeam( pKiller->team ) + 1 ]
                            : Color_Neutral_u32;
                //-----------------------------------------------------------------------
                Msg( "%s died from bleeding, thanks to %s ",
                     *KMS.m_victim.m_name, *KMS.m_killer.m_name );
            } else {
                //-----------------------------------------------------------------
...
void game_cl_mp::OnPlayerChangeName( NET_Packet& P ) {
    ...
    string1024 resStr;
    xr_sprintf( resStr, "%s\"%s\" %s%s %s\"%s\"", Color_Teams[ Team ], OldName,
                Color_Main, *st.translate( "mp_is_now" ), Color_Teams[ Team ],
                NewName );
    if ( CurrentGameUI() )
        CurrentGameUI()->CommonMessageOut( resStr );
    Msg( NewName );
...
void game_cl_mp::OnRankChanged( u8 OldRank ) {
    ...
    xr_sprintf( RankStr, "%s : %s", *st.translate( "mp_your_rank" ),
                *st.translate( READ_IF_EXISTS( pSettings, r_string, tmp,
                                               "rank_name", "" ) ) );
    if ( CurrentGameUI() )
...
void game_cl_mp::net_import_state( NET_Packet& P ) {
    ...
    m_bSpectator_FreeFly =
        ( m_u8SpectatorModes & ( 1 << CSpectator::eacFreeFly ) ) != 0;
    m_bSpectator_FirstEye =
        ( m_u8SpectatorModes & ( 1 << CSpectator::eacFirstEye ) ) != 0;
    m_bSpectator_LookAt =
        ( m_u8SpectatorModes & ( 1 << CSpectator::eacLookAt ) ) != 0;
    m_bSpectator_FreeLook =
        ( m_u8SpectatorModes & ( 1 << CSpectator::eacFreeLook ) ) != 0;
    m_bSpectator_TeamCamera =
        ( m_u8SpectatorModes & ( 1 << CSpectator::eacMaxCam ) ) != 0;
}

bool game_cl_mp::Is_Spectator_Camera_Allowed(
...
void game_cl_mp::OnEventMoneyChanged( NET_Packet& P ) {
    ...
    shared_str BonusStr = ( NumBonuses > 1 )
                              ? "Your bonuses : "
                              : ( ( NumBonuses == 1 ) ? "Your bonus : " : "" );
    for ( u8 i = 0; i < NumBonuses; i++ ) {
        ...
        BONUSES_it it = std::find( m_pBonusList.begin(), m_pBonusList.end(),
                                   BName.c_str() );
        if ( it != m_pBonusList.end() && ( *it == BName.c_str() ) ) {
            ...
            BMS.m_initiator.m_rect.x2 =
                pBS->IconRects[ RectID ].x1 + pBS->IconRects[ RectID ].x2;
            BMS.m_initiator.m_rect.y2 =
                pBS->IconRects[ RectID ].y1 + pBS->IconRects[ RectID ].y2;
        };
...
void game_cl_mp::SendPlayerStarted() {
    ...
    R_ASSERT2( map_name && ( xr_strlen( map_name ) > 0 ),
               "map name not present" );
    ...
    for ( u32 i = 0; i < BonusCount; i++ ) {
        ...
        if ( !strstr( name, "new_rank" ) ) {
            ...
            if ( pSettings->line_exist( "mp_bonus_icons", IconShader ) ) {
                NewBonus.IconShader->create(
                    "hud\\default",
                    pSettings->r_string( "mp_bonus_icons", IconShader ) );
            }
            Frect IconRect;
            IconRect.x1 = READ_IF_EXISTS( pSettings, r_float, "mp_bonus_icons",
                                          IconX, 0 );
            IconRect.y1 = READ_IF_EXISTS( pSettings, r_float, "mp_bonus_icons",
                                          IconY, 0 );
            IconRect.x2 = READ_IF_EXISTS( pSettings, r_float, "mp_bonus_icons",
                                          IconW, 0 );
            IconRect.y2 = READ_IF_EXISTS( pSettings, r_float, "mp_bonus_icons",
                                          IconH, 0 );
            NewBonus.IconRects.push_back( IconRect );
        } else {
            CUITextureMaster::GetTextureShader( "ui_hud_status_blue_01",
                                                NewBonus.IconShader );
        ...
        case M_REMOTE_CONTROL_AUTH: {
            ...
            if ( !g_dedicated_server ) {
                ...
                if ( 0 == stricmp( buff, "Access permitted." ) )
                    ...
                    m_pAdminMenuWindow->ShowMessageBox(
                        CUIMessageBox::MESSAGEBOX_OK, buff );
            }
...
#ifdef DEBUG
            Msg( "* screenshot: %d of %d bytes sent ...", bytes_sent,
                 data_size );
#endif
        } break;
...
void game_cl_mp::reinit_compress_buffer( u32 need_size ) {
    ...
    void* new_buffer =
        xr_realloc( buffer_for_compress, buffer_for_compress_size );
...
void game_cl_mp::SendCollectedData( u8 const* buffer,
                                    u32 buffer_size,
    ...
    file_transfer::sending_state_callback_t sending_cb =
        fastdelegate::MakeDelegate( this,
                                    &game_cl_mp::sending_screenshot_callback );
    ...
    Level().m_file_transfer->start_transfer_file(
        upload_memory_writer.pointer(), upload_memory_writer.size(), sending_cb,
        uncompressed_size );
};

void game_cl_mp::generate_file_name( string_path& file_name,
                                     LPCSTR file_suffix,
                                     SYSTEMTIME const& date_time ) {
    xr_sprintf( file_name, "%02d%02d%02d-%02d%02d%02d_%s",
                date_time.wYear % 100, date_time.wMonth, date_time.wDay,
                date_time.wHour, date_time.wMinute, date_time.wSecond,
                file_suffix );
}
...
LPCSTR game_cl_mp::make_file_name( LPCSTR session_id, string_path& dest ) {
    ...
    size_t start_pos = 0;
    size_t char_pos;
    while ( ( char_pos = strcspn( dest + start_pos, denied_symbols ) ) <
...
void game_cl_mp::start_receive_server_info( ClientID const& svclient_id ) {
    ...
    tmp_binder->m_frnode = Level().m_file_transfer->start_receive_file(
        tmp_binder->m_writer, svclient_id, receiving_cb_info );

    R_ASSERT2( tmp_binder->m_frnode,
               "failed to initialise server logo receiving" );
}

void game_cl_mp::PrepareToReceiveFile( ClientID const& from_client,
    ...
    tmp_binder->m_frnode = Level().m_file_transfer->start_receive_file(
        tmp_binder->m_writer, from_client, receiving_cb );
...
void __stdcall game_cl_mp::fr_callback_binder::receiving_file_callback(
    ...
    switch ( status ) {
        case file_transfer::receiving_data: {
            Msg( "* file: %d of %d bytes received ...", bytes_received,
                 data_size );
            ...
            switch ( m_response_type ) {
                case e_screenshot_response: {
                    m_owner->decompress_and_save_screenshot(
                        m_file_name.c_str(), m_writer.pointer(),
                        m_writer.size(), m_frnode->get_user_param() );
                } break;
...
void __stdcall game_cl_mp::fr_callback_binder::receiving_serverinfo_callback(
    ...
    switch ( status ) {
        case file_transfer::receiving_data: {
#ifdef DEBUG
            Msg( "* serverinfo: %d of %d bytes received ...", bytes_received,
                 data_size );
#endif
            m_downloaded_size = bytes_received;
            m_max_size = data_size;
        ...
        case file_transfer::receiving_complete: {
            ...
            R_ASSERT2( m_owner->m_game_ui_custom || g_dedicated_server,
                       "game ui not initialized" );
            if ( m_owner->m_game_ui_custom )
                m_owner->extract_server_info( m_writer.pointer(),
                                              m_writer.size() );
            m_active = false;
        } break;
    };
...
void game_cl_mp::decompress_and_save_screenshot( LPCSTR file_name,
                                                 u8* data,
                                                 u32 data_size,
                                                 u32 file_size ) {
    if ( !file_size ) {
    ...
    u32 original_size = ppmd_decompress(
        buffer_for_compress, buffer_for_compress_size, data, data_size );
...
void game_cl_mp::decompress_and_process_config( LPCSTR file_name,
                                                u8* data,
                                                u32 data_size,
                                                u32 file_size ) {
    if ( !file_size ) {
    ...
    u32 original_size =
        ppmd_decompress_mt( buffer_for_compress, buffer_for_compress_size, data,
                            data_size, tmp_cb );
...
void game_cl_mp::add_detected_cheater( shared_str const& file_name,
                                       string256 diff ) {
...
struct old_detected_cheater {
    bool operator()( game_cl_mp::detected_cheater_t const& cheater ) {
        if ( cheater.m_detect_time +
                 game_cl_mp::detected_cheater_t::max_showing_time <=
             Device.dwTimeGlobal )
            return true;
        return false;
    }
...
void game_cl_mp::extract_server_info( u8* data_ptr, u32 data_size ) {
    ...
    buffer_vector< const_buffer_t > tmp_vector(
        _alloca( sizeof( const_buffer_t ) * 2 ), 2 );
    split_received_to_buffers( data_ptr, data_size, tmp_vector );
    if ( tmp_vector.empty() ) {
        Msg( "! ERROR: received corrupted server info" );
        return;
    }
    tmp_ui_mp_game->SetServerLogo( tmp_vector[ 0 ].first,
                                   tmp_vector[ 0 ].second );
    if ( tmp_vector.size() > 1 ) {
        tmp_ui_mp_game->SetServerRules( tmp_vector[ 1 ].first,
                                        tmp_vector[ 1 ].second );
    }
}
...
void game_cl_mp::ReInitRewardGenerator( game_PlayerState* local_ps ) {
    if ( !m_reward_generator ) {
        m_reward_generator =
            xr_new< award_system::reward_event_generator >( u32( -1 ) );
...
bool game_cl_mp::IsLocalPlayerInitialized() const {
    ...
    game_cl_GameState::PLAYERS_MAP::const_iterator pi =
        playersMap.find( local_svdpnid );
    return pi != playersMap.end();
}

bool game_cl_mp::RequestPlayersInfo(
    player_info_reply_cb_t const pinfo_repl_cb ) {
