...
template < typename T >
IC void CAgentMemoryManager::update_memory_masks( const squad_mask_type& mask,
                                                  T& objects ) {
    typename T::iterator I = objects.begin();
    typename T::iterator E = objects.end();
    for ( ; I != E; ++I ) {
...
void CAgentMemoryManager::object_information( const CObject* object,
                                              u32& level_time,
                                              Fvector& position ) {
    {
        VISIBLES::const_iterator I = std::find(
            visibles().begin(), visibles().end(), object_id( object ) );
        if ( visibles().end() != I ) {
    ...
    {
        SOUNDS::const_iterator I =
            std::find( sounds().begin(), sounds().end(), object_id( object ) );
        if ( ( sounds().end() != I ) &&
             ( level_time < ( *I ).m_last_level_time ) ) {
            level_time = ( *I ).m_last_level_time;
            position = ( *I ).m_object_params.m_position;
    ...
    {
        HITS::const_iterator I =
            std::find( hits().begin(), hits().end(), object_id( object ) );
        if ( ( hits().end() != I ) &&
             ( level_time < ( *I ).m_last_level_time ) ) {
            level_time = ( *I ).m_last_level_time;
            position = ( *I ).m_object_params.m_position;
