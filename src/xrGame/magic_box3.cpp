...
bool MagicBox3::intersects( const MagicBox3& rkBox1 ) const {
    ...
    fR = _abs( afAD[ 2 ] );
    fR1 = afEB[ 0 ] * aafAbsC[ 2 ][ 0 ] + afEB[ 1 ] * aafAbsC[ 2 ][ 1 ] +
          afEB[ 2 ] * aafAbsC[ 2 ][ 2 ];
    fR01 = afEA[ 2 ] + fR1;
    if ( fR > fR01 )
        return false;
    ...
    fR0 = afEA[ 0 ] * aafAbsC[ 0 ][ 2 ] + afEA[ 1 ] * aafAbsC[ 1 ][ 2 ] +
          afEA[ 2 ] * aafAbsC[ 2 ][ 2 ];
    fR01 = fR0 + afEB[ 2 ];
    if ( fR > fR01 )
        return false;
...
void MagicBox3::ComputeVertices( Fvector* akVertex ) const {
    Fvector akEAxis[ 3 ] = { Fvector().mul( m_akAxis[ 0 ], m_afExtent[ 0 ] ),
                             Fvector().mul( m_akAxis[ 1 ], m_afExtent[ 1 ] ),
                             Fvector().mul( m_akAxis[ 2 ], m_afExtent[ 2 ] ) };

    akVertex[ 0 ] = Fvector()
                        .sub( m_kCenter, akEAxis[ 0 ] )
                        .sub( akEAxis[ 1 ] )
                        .sub( akEAxis[ 2 ] );
    akVertex[ 1 ] = Fvector()
                        .add( m_kCenter, akEAxis[ 0 ] )
                        .sub( akEAxis[ 1 ] )
                        .sub( akEAxis[ 2 ] );
    akVertex[ 2 ] = Fvector()
                        .add( m_kCenter, akEAxis[ 0 ] )
