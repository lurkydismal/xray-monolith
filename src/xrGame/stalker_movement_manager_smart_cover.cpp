...
void stalker_movement_manager_smart_cover::enter_smart_cover() {
    ...
    if ( !m_current.cover() && ( m_enter_cover_id != "" ) &&
         ( ( m_target.cover_id() != m_enter_cover_id ) ||
           ( m_target.cover_loophole_id() != m_enter_loophole_id ) ) ) {
#ifdef DEBUG
        Msg( "setting up cover: %s (%s)", m_enter_cover_id.c_str(),
             m_enter_cover_id.c_str() );
#endif // #ifdef DEBUG
...
MotionID stalker_movement_manager_smart_cover::select_animation(
    bool& animation_movement_controller ) {
    if ( !object().g_Alive() )
        return ( MotionID() );

    animation_movement_controller = true;
    VERIFY( m_entering_smart_cover_with_animation ||
            current_transition().animation().has_animation() );
...
void stalker_movement_manager_smart_cover::on_frame(
...
void stalker_movement_manager_smart_cover::modify_animation( CBlend* blend ) {
    ...
    CMotionDef* motion_def =
        smart_cast< IKinematicsAnimated* >( object().Visual() )
            ->LL_GetMotionDef( blend->motionID );
    VERIFY( motion_def );
...
void stalker_movement_manager_smart_cover::reach_enter_location(
    u32 const& time_delta ) {
    ...
    m_target.cover()->object().XFORM().transform_tiny(
        position, current_transition().animation().position() );
    ...
    if ( !accessible( level_vertex_id ) || !accessible( position ) ) {
        if ( !ai().level_graph().inside( level_vertex_id, position ) )
            ...
            position.y = ai().level_graph().vertex_plane_y(
                level_vertex_id, position.x, position.z );

        if ( !restrictions().accessible( position ) ) {
            level_vertex_id = restrictions().accessible_nearest(
                Fvector().set( position ), position );
        ...
        VERIFY2( restrictions().accessible( position ) ||
                     show_restrictions( &restrictions() ),
                 *object().cName() );
    ...
    if ( target_params().cover()->can_fire() ) {
        CInventoryItem const* const inventory_item =
...
bool stalker_movement_manager_smart_cover::target_approached(
...
void stalker_movement_manager_smart_cover::loophole_path(
    smart_cover::cover const& cover,
    shared_str const& source_raw,
    shared_str const& target_raw,
    ...
    path.clear_not_free();
    R_ASSERT2(
        ai().graph_engine().search( cover.description()->transitions(), source,
                                    target, &path, parameters ),
        make_string( "cannot build path via loopholes [%s] -> [%s] (cover %s)",
                     source_raw.c_str(), target_raw.c_str(),
...
void stalker_movement_manager_smart_cover::bind_global_selector() {
    ...
    animation.global_modifier( CStalkerAnimationManager::AnimationModifier(
        &animation_selector(),
        &smart_cover::animation_selector::modify_animation ) );
#endif // #ifdef DEBUG
    ...
    Fvector position =
        m_current.cover()->fov_position( *m_current.cover_loophole() );
    Fvector direction =
        m_current.cover()->enter_direction( *m_current.cover_loophole() );
    object().animation().global().target_matrix( position, direction );
}
...
void stalker_movement_manager_smart_cover::target_selector(
    CScriptCallbackEx< void > const& callback ) {
...
void stalker_movement_manager_smart_cover::target_idle() {
    //	if (!m_current.cover()) {
    //		Msg
    //("! Cannot set target idle. Bad or absent smart_cover."); 		return;
    //	}

    //	if (!m_current.cover_loophole()->is_action_available("idle")) {
    //		Msg
    //("! Cannot set target idle. Loophole has no such action."); 		return;
    //	}

    m_target_selector->object().target(
        StalkerDecisionSpace::eWorldPropertyLoopholeIdle );
}

void stalker_movement_manager_smart_cover::target_lookout() {
    //	if (!m_current.cover()) {
    //		Msg
    //("! Cannot set target lookout. Bad or absent smart_cover."); 		return;
    //	}

    //	if (!m_current.cover_loophole()->is_action_available("lookout")) {
    //		Msg
    //("! Cannot set target lookout. Loophole has no such action."); 		return;
    //	}
...
void stalker_movement_manager_smart_cover::target_fire() {
    //	if (!m_current.cover()) {
    //		Msg
    //("! Cannot set target fire. Bad or absent smart_cover."); 		return;
    //	}

    //	if (!m_current.cover_loophole()->is_action_available("fire")) {
    //		Msg
    //("! Cannot set target fire. Loophole has no such action."); 		return;
    //	}

    //	if (!enemy_in_fov()) {
    //		Msg
    //("! Cannot set target fire. Enemy is not in current loophole's fov.");
    //		return;
    //	}

    m_target_selector->object().target(
        StalkerDecisionSpace::eWorldPropertyLoopholeFire );
}

void stalker_movement_manager_smart_cover::target_fire_no_lookout() {
    //	if (!current_params().cover()) {
    //		Msg
    //("! Cannot set target fire_no_lookout. Bad or absent smart_cover.");
    //		return;
    //	}

    //	if
    //(!current_params().cover_loophole()->is_action_available("fire_no_lookout"))
    //{ 		Msg								("!
    //Cannot set target fire_no_lookout. Loophole has no such action."); 		return;
    //	}

    m_target_selector->object().target(
        StalkerDecisionSpace::eWorldPropertyLoopholeFireNoLookout );
}
