...
#define TEMPLATE_SPECIALIZATION                                    \
    template < typename T1, typename T2, typename T3, typename T4, \
               typename T5, bool T6, typename T7, typename T8,     \
               typename _DataStorage, typename _Parameters,        \
               typename _dist_type, typename _index_type,          \
               typename _iteration_type >

#define CSolverPathManager                                            \
    CPathManager< CProblemSolver< T1, T2, T3, T4, T5, T6, T7, T8 >,   \
                  _DataStorage, _Parameters, _dist_type, _index_type, \
                  _iteration_type >
...
TEMPLATE_SPECIALIZATION
IC void CSolverPathManager::setup( const _Graph* _graph,
...
TEMPLATE_SPECIALIZATION
IC bool CSolverPathManager::is_goal_reached(
    const _index_type& vertex_id ) const {
    return ( this->graph->is_goal_reached( vertex_id ) );
}

TEMPLATE_SPECIALIZATION
IC const _index_type& CSolverPathManager::get_value(
    const_iterator& i,
...
TEMPLATE_SPECIALIZATION
IC const typename CSolverPathManager::_edge_type& CSolverPathManager::edge(
    const_iterator& i ) const {
    return ( ( *i ).m_operator_id );
}

TEMPLATE_SPECIALIZATION
IC _dist_type CSolverPathManager::evaluate( const _index_type& node_index1,
...
TEMPLATE_SPECIALIZATION
IC _dist_type
CSolverPathManager::estimate( const _index_type& vertex_id ) const {
    VERIFY( this->graph );
    //	return
    //((_dist_type)graph->get_edge_weight(vertex_id,start_node_index,m_iterator));
...
IC void CSolverPathManager::create_path( T& vertex,
    ...
    if ( m_edge_path )
        this->data_storage.get_edge_path( *m_edge_path, &vertex,
                                          reverse_order );
}
