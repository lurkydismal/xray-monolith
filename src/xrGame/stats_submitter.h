...
class stats_submitter : public ISheduled, private xray::noncopyable {
public:
    ...
    void quick_reward_with_award( enum_awards_t const award_id,
                                  gamespy_gp::profile const* profile );

    void set_best_scores( all_best_scores_t const* scores,
                          gamespy_gp::profile const* profile,
                          store_operation_cb opcb );
    void quick_set_best_scores( all_best_scores_t const* scores,
                                gamespy_gp::profile const* profile );
...
private:
    ...
    // gsi_u8
    // m_atlas_connection_id[SC_CONNECTION_GUID_SIZE]; SCReportPtr
    // m_atlas_report;
    ...
    void __stdcall onlylog_operation( bool const result,
                                      char const* err_descr );

    static void fill_private_key(
