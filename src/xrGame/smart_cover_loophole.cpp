...
loophole::loophole( ::luabind::object const& description )
    : m_fov( 0.f ), m_range( 0.f ) {
    VERIFY2( description.type() == LUA_TTABLE,
    ...
    m_danger_fov_direction =
        parse_fvector( description, "danger_fov_direction" );
    if ( m_danger_fov_direction.square_magnitude() < EPS_L ) {
        Msg( "! danger fov direction for loophole %s is setup incorrectly",
             m_id.c_str() );
    ...
    m_danger_fov =
        deg2rad( parse_float( description, "danger_fov", 0.f, 360.f ) );
...
void loophole::add_action( LPCSTR type, ::luabind::object const& table ) {
    ...
    VERIFY( std::find_if( m_actions.begin(), m_actions.end(),
                          id_predicate( type ) ) == m_actions.end() );
...
void loophole::fill_transitions( ::luabind::object const& transitions_table ) {
    VERIFY2( transitions_table.type() == LUA_TTABLE,
             "invalid loophole description passed" );
    ...
    for ( ; I != E; ++I ) {
        ...
        for ( ; i != e; ++i ) {
            ...
            VERIFY2(
                std::find( tmp.begin(), tmp.end(), animation ) == tmp.end(),
                make_string( "duplicated_animation found: %s",
                             animation.c_str() ) );
        ...
        TransitionGraph::CEdge* edge =
            m_transitions.edge( action_from, action_to );
        VERIFY( !tmp.empty() );
        edge->data() = tmp;
    }
}
...
smart_cover::action::Animations const& loophole::action_animations(
    ...
    VERIFY2( found != m_actions.end(),
             make_string( "action [%s] not present in loophole [%s]",
                          action_id.c_str(), m_id.c_str() ) );

    return found->second->animations( m_id, animation_id );
}

loophole::TransitionData const& loophole::transition_animations(
    shared_str const& action_from,
    shared_str const& action_to ) const {
    TransitionGraph::CEdge const* edge =
        m_transitions.edge( transform_vertex( action_from, true ),
                            transform_vertex( action_to, false ) );
    VERIFY2( edge, make_string( "transition [%s]->[%s] absent",
                                action_from.c_str(), action_to.c_str() ) );
    return ( edge->data() );
}
