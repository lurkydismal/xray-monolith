//////////////////////////////////////////////////////////////////////////
// relation_registry_fights.cpp:	реестр для хранения данных об отношении
// персонажа к
//									другим
//персонажам
//////////////////////////////////////////////////////////////////////////
...
RELATION_REGISTRY::FIGHT_DATA* RELATION_REGISTRY::FindFight(
    u16 object_id,
    bool by_attacker ) {
    FIGHT_VECTOR& fights = fight_registry();
    for ( FIGHT_VECTOR_IT it = fights.begin(); it != fights.end(); it++ ) {
        FIGHT_DATA& fight_data = *it;
        u16 id_to_find =
            by_attacker ? fight_data.attacker : fight_data.defender;
...
bool fight_time_pred( RELATION_REGISTRY::FIGHT_DATA& fight_data ) {
    ...
    static u32 fight_remember_time =
        u32( 1000.f *
             pSettings->r_float( ACTIONS_POINTS_SECT, "fight_remember_time" ) );
...
void RELATION_REGISTRY::UpdateFightRegister() {
    FIGHT_VECTOR& fights = fight_registry();
    FIGHT_VECTOR_IT it =
        std::remove_if( fights.begin(), fights.end(), fight_time_pred );
