...
CPHMovementControl::CPHMovementControl( CObject* parent ) {
    ...
    if ( debug_output().ph_dbg_draw_mask1().test( ph_m1_DbgTrackObject ) &&
         ( !!pObject->cName() ) &&
         stricmp( debug_output().PH_DBG_ObjectTrackName(),
                  *pObject->cName() ) == 0 ) {
        Msg( "CPHMovementControl::CPHMovementControl %s (constructor) "
             "%f,%f,%pObjectf",
             debug_output().PH_DBG_ObjectTrackName(), pObject->Position().x,
    ...
    aabb.set( -def_X_SIZE_2, 0, -def_Z_SIZE_2, def_X_SIZE_2, def_Y_SIZE_2 * 2,
              def_Z_SIZE_2 );
    fMass = 100;
...
static ALife::EHitType DefineCollisionHitType( u16 material_idx ) {
    if ( GMLib.GetMaterialByIdx( material_idx )
...
void CPHMovementControl::Calculate( Fvector& vAccel,
                                    const Fvector& camDir,
                                    float /**ang_speed/**/,
                                    float jump,
                                    float /**dt/**/,
                                    bool /**bLight/**/ ) {
    ...
    if ( cdi->HitCallback() )
        cdi->HitCallback()->call( gcontact_Initiator, fMinCrashSpeed,
                                  fMaxCrashSpeed, fContactSpeed,
                                  gcontact_HealthLost, cdi );
...
void CPHMovementControl::UpdateCollisionDamage() {
    ...
    if ( fContactSpeed > fMinCrashSpeed ) {
        gcontact_HealthLost = ( ( fContactSpeed - fMinCrashSpeed ) ) /
                              ( fMaxCrashSpeed - fMinCrashSpeed );
        VERIFY( m_character );
        m_character->SetHitType(
            DefineCollisionHitType( m_character->LastMaterialIDX() ) );
    }
}
...
bool CPHMovementControl::MakeJumpPath(
    xr_vector< DetailPathManager::STravelPathPoint >& out_path,
    u32& travel_point,
    ...
    Fvector const deviation =
        self_to_enemy - self_to_enemy_projection_on_self_vel;
...
void CPHMovementControl::Calculate(
    const xr_vector< DetailPathManager::STravelPathPoint >& in_path,
    float speed,
    u32& travel_point,
    float& precision ) {
#ifdef DEBUG
    if ( debug_output().ph_dbg_draw_mask1().test( ph_m1_DbgTrackObject ) &&
         ...
         stricmp( debug_output().PH_DBG_ObjectTrackName(),
                  *pObject->cName() ) == 0 ) {
        Msg( "CPHMovementControl::Calculate in %s (Object Position) %f,%f,%f",
             debug_output().PH_DBG_ObjectTrackName(), pObject->Position().x,
             pObject->Position().y, pObject->Position().z );
        Msg( "CPHMovementControl::Calculate in %s "
             "(CPHMovementControl::vPosition) %f,%f,%f",
             debug_output().PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y,
             vPosition.z );
    }
#endif
    ...
    bool const add_deviation =
        MakeJumpPath( replacing_path, travel_point, deviation );

    xr_vector< DetailPathManager::STravelPathPoint > const& path =
        add_deviation ? replacing_path : in_path;
    ...
    if ( m_path_size == 0 ) {
        ...
        if ( ( m_path_size - 1 ) > ( int )travel_point )
            dir.sub( path[ travel_point + 1 ].position,
                     path[ travel_point ].position );
        else
        ...
        if ( m_path_size == 1 ) {
            speed = 0.f;
            vPosition.set(
                new_position ); // todo - insert it in PathNearestPoint
            index = 0;
            ...
            if ( m_start_index < m_path_size ) {
                PathNearestPointFindUp( path, new_position, index, radius,
                                        near_line );
                PathNearestPointFindDown( path, new_position, index, radius,
                                          near_line );
            }
...
void CPHMovementControl::PathNearestPoint(
    const xr_vector< DetailPathManager::STravelPathPoint >& path,
    const Fvector& new_position,
    int& index,
    bool& near_line ) {
    ...
    for ( ; i < m_path_size - 1; ++i ) {
        const Fvector &first = path[ i ].position,
                      &second = path[ i + 1 ].position;
        ...
        {
            if ( after_line ) // after previous line && befor this line = near
                              // first point
            {
        ...
        {
            if ( from_second.dotproduct( dir ) <
                 0.f ) // befor second && after first = near line
            {
    ...
    if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
        ...
        Msg( "CPHMovementControl::Calculate out %s (Object Position) %f,%f,%f",
             PH_DBG_ObjectTrackName(), pObject->Position().x,
             pObject->Position().y, pObject->Position().z );
        Msg( "CPHMovementControl::Calculate out %s "
             "(CPHMovementControl::vPosition) %f,%f,%f",
...
void CPHMovementControl::PathNearestPointFindUp(
    const xr_vector< DetailPathManager::STravelPathPoint >& path,
    const Fvector& new_position,
    int& index,
    float radius,
    ...
    for ( ; i < m_path_size - 1; ++i ) {
        ...
        if ( from_first_dir < 0.f ) // before this line
        {
            ...
            if ( after_line ) // after previous line && before this line = near
                              // first point
            {
                if ( temp < m_path_distance ) {
                    m_path_distance = temp;
...
void CPHMovementControl::PathNearestPointFindDown(
    ...
    for ( ; i > 1; --i ) {
        const Fvector &first = path[ i - 1 ].position,
                      &second = path[ i ].position;
        from_first.sub( new_position, first );
        from_second.sub( new_position, second );
        dir.sub( second, first );
        dir.normalize_safe();
        ...
        {
            if ( from_first_dir >
                 0.f ) // after second && before first = near line (going down)
            {
                vtemp.set( dir );
...
void CPHMovementControl::CorrectPathDir(
    const Fvector& real_path_dir,
    const xr_vector< DetailPathManager::STravelPathPoint >& path,
    int index,
    ...
    if ( fis_zero( plane_motion, epsilon ) ) {
        if ( !fis_zero( plane_motion, EPS ) ) {
            ...
            corrected_path_dir.sub( path[ index + 1 ].position,
                                    path[ index ].position );
            corrected_path_dir.normalize_safe();
            CorrectPathDir( corrected_path_dir, path, index + 1,
                            corrected_path_dir );
        } else {
            corrected_path_dir.set( real_path_dir );
        }
...
void CPHMovementControl::PathDIrLine(
    const xr_vector< DetailPathManager::STravelPathPoint >& path,
    int index,
    float distance,
    float precesition,
...
void CPHMovementControl::PathDIrPoint(
    const xr_vector< DetailPathManager::STravelPathPoint >& path,
    int index,
    float distance,
    float precesition,
...
void CPHMovementControl::SetActorRestrictorRadius( ERestrictionType rt,
                                                   float r ) {
...
void CPHMovementControl::Load( LPCSTR section ) {
    ...
    xr_token retrictor_types[] = { { "actor", rtActor },
    ...
    if ( pSettings->line_exist( section, "actor_restrictor" ) )
        SetRestrictionType( ERestrictionType( pSettings->r_token(
            section, "actor_restrictor", retrictor_types ) ) );
    fCollisionDamageFactor =
        READ_IF_EXISTS( pSettings, r_float, section,
                        "ph_collision_damage_factor", fCollisionDamageFactor );
    R_ASSERT3( fCollisionDamageFactor <= 1.f, "ph_collision_damage_factor > 1.",
               section );
...
void CPHMovementControl::SetPosition( const Fvector& P ) {
    ...
    if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
         ( !!pObject->cName() ) &&
         stricmp( PH_DBG_ObjectTrackName(), *pObject->cName() ) == 0 ) {
        Msg( "CPHMovementControl::SetPosition %s (Object Position) %f,%f,%f",
             PH_DBG_ObjectTrackName(), pObject->Position().x,
             pObject->Position().y, pObject->Position().z );
        Msg( "CPHMovementControl::SetPosition %s "
             "(CPHMovementControl::vPosition) %f,%f,%f",
             PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z );
    }
#endif
...
bool CPHMovementControl::TryPosition( Fvector& pos ) {
    ...
    if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
         ( !!pObject->cName() ) &&
         stricmp( PH_DBG_ObjectTrackName(), *pObject->cName() ) == 0 ) {
        Msg( "CPHMovementControl::TryPosition %s (Object Position) %f,%f,%f",
             PH_DBG_ObjectTrackName(), pObject->Position().x,
             pObject->Position().y, pObject->Position().z );
        Msg( "CPHMovementControl::TryPosition %s "
             "(CPHMovementControl::vPosition) %f,%f,%f",
             PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z );
    }
#endif
    ...
    if ( m_character->b_exist ) {
        bool ret = m_character->TryPosition( pos, b_exect_position ) &&
                   !bExernalImpulse;
...
void CPHMovementControl::GetPosition( Fvector& P ) {
    ...
    if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
         ( !!pObject->cName() ) &&
         stricmp( PH_DBG_ObjectTrackName(), *pObject->cName() ) == 0 ) {
        Msg( "CPHMovementControl::GetPosition %s (Object Position) %f,%f,%f",
             PH_DBG_ObjectTrackName(), pObject->Position().x,
             pObject->Position().y, pObject->Position().z );
        Msg( "CPHMovementControl::GetPosition %s "
             "(CPHMovementControl::vPosition) %f,%f,%f",
             PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z );
    }
#endif
    P.set( vPosition );
    VERIFY_BOUNDARIES2( vPosition, ph_boundaries(),
                        m_character->PhysicsRefObject(),
...
void CPHMovementControl::AllocateCharacterObject( CharacterType type ) {
    ...
    if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
         ( !!pObject->cName() ) &&
         stricmp( PH_DBG_ObjectTrackName(), *pObject->cName() ) == 0 ) {
        Msg( "CPHMovementControl::AllocateCharacterObject %s (Object Position) "
             "%f,%f,%f",
             PH_DBG_ObjectTrackName(), pObject->Position().x,
             pObject->Position().y, pObject->Position().z );
        Msg( "CPHMovementControl::AllocateCharacterObject %s "
             "(CPHMovementControl::vPosition) %f,%f,%f",
             PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z );
    }
#endif
...
void CPHMovementControl::PHCaptureObject( CPhysicsShellHolder* object,
                                          CPHCaptureBoneCallback* cb ) {
    if ( m_capture )
        return;

    if ( !object || !object->PPhysicsShell() ||
         !object->m_pPhysicsShell->isActive() )
        return;

    m_capture = phcapture_create(
        m_character, object, static_cast< NearestToPointCallback* >( cb ) );
}
...
Fvector CPHMovementControl::PHCaptureGetNearestElemPos(
    const CPhysicsShellHolder* object ) {
    R_ASSERT3( ( object->m_pPhysicsShell != NULL ),
               "NO Phisics Shell for object ", *object->cName() );

    CPhysicsElement* ph_elem =
        object->m_pPhysicsShell->NearestToPoint( vPosition );
...
Fmatrix CPHMovementControl::PHCaptureGetNearestElemTransform(
...
void CPHMovementControl::DestroyCharacter() {
    ...
    if ( m_character->PhysicsRefObject() != NULL )
        g_pGamePersistent->GrassBendersRemoveById(
            m_character->PhysicsRefObject()->ObjectID() );
...
void CPHMovementControl::Jump( const Fvector& end_point, float time ) {
    Jump( smart_cast< CGameObject* >( m_character->PhysicsRefObject() )
              ->Position(),
          end_point, time );
}

void CPHMovementControl::Jump( const Fvector& start_point,
...
void CPHMovementControl::GetJumpMinVelParam( Fvector& min_vel,
                                             float& time,
                                             JumpType& type,
                                             const Fvector& end_point ) {
...
float CPHMovementControl::JumpMinVelTime( const Fvector& end_point ) {
    return ThrowMinVelTime(
        Fvector().sub( end_point, smart_cast< CGameObject* >(
                                      m_character->PhysicsRefObject() )
                                      ->Position() ),
        physics_world()->Gravity() );
}

void CPHMovementControl::GetJumpParam( Fvector& velocity,
                                       JumpType& type,
                                       const Fvector& end_point,
                                       float time ) {
    Fvector velosity;
    velosity.sub( smart_cast< CGameObject* >( m_character->PhysicsRefObject() )
                      ->Position(),
                  end_point );
    TransferenceToThrowVel( velosity, time, physics_world()->Gravity() );
...
void CPHMovementControl::CreateCharacter() {
    ...
    if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
         ...
         stricmp( PH_DBG_ObjectTrackName(), *pObject->cName() ) == 0 ) {
        Msg(
            "CPHMovementControl::CreateCharacter %s (Object Position) %f,%f,%f",
            PH_DBG_ObjectTrackName(), pObject->Position().x,
            pObject->Position().y, pObject->Position().z );
        Msg( "CPHMovementControl::CreateCharacter %s "
             "(CPHMovementControl::vPosition) %f,%f,%f",
             PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z );
    }
#endif
    m_character->SetPosition( vPosition );
    m_character->SetCollisionDamageFactor( fCollisionDamageFactor *
                                           fCollisionDamageFactor );
    trying_times[ 0 ] = trying_times[ 1 ] = trying_times[ 2 ] =
        trying_times[ 3 ] = u32( -1 );
...
void CPHMovementControl::InterpolateBox( DWORD id, float k ) {
    ...
    if ( !m_character || !m_character->b_exist )
        return;
    dVector3 size = { aabb.x2 - aabb.x1, aabb.y2 - aabb.y1, aabb.z2 - aabb.z1 };
    dVector3 to_size = { boxes[ id ].x2 - boxes[ id ].x1,
                         boxes[ id ].y2 - boxes[ id ].y1,
                         boxes[ id ].z2 - boxes[ id ].z1 };
    dVectorInterpolate( size, to_size, k );
    m_character->SetBox( size );
}

void CPHMovementControl::ApplyHit( const Fvector& dir,
                                   const float P,
                                   ALife::EHitType hit_type ) {
    VERIFY( m_character );
    if ( !m_character->CastActorCharacter() )
        return;
    if ( ( Environment() == CPHMovementControl::peOnGround ||
           Environment() == CPHMovementControl::peAtWall ) ) {
        switch ( hit_type ) {
...
BOOL CPHMovementControl::BorderTraceCallback( collide::rq_result& result,
                                              LPVOID params ) {
...
void CPHMovementControl::TraceBorder( const Fvector& prev_position ) {
    ...
    g_pGameLevel->ObjectSpace.RayQuery(
        storage, RD, BorderTraceCallback, &p, NULL,
...
void CPHMovementControl::UpdateObjectBox( CPHCharacter* ach ) {
    ...
    float R = _abs( poses_dir.dotproduct( plane_i ) * cbox.x ) +
              _abs( poses_dir.dotproduct( plane_k ) * cbox.z );
...
void CPHMovementControl::SetPathDir( const Fvector& v ) {
    ...
    if ( _abs( _vPathDir.x ) > 1000 || _abs( _vPathDir.y ) > 1000 ||
         _abs( _vPathDir.z ) > 1000 ) {
...
void CPHMovementControl::VirtualMoveTo( const Fvector& in_pos,
                                        Fvector& out_pos ) {
    ...
    public:
        ph_character_state_save( CPHCharacter* character )
            ...
            character_->SetObjectContactCallback(
                virtual_move_collide_callback );
            character_->SetObjectContactCallbackData(
                static_cast< CPHObject* >( character ) );
...
void CPHMovementControl::SetOjectContactCallback(
...
void CPHMovementControl::SetPhysicsRefObject(
    CPhysicsShellHolder* ref_object ) {
    VERIFY( m_character );
...
void CPHMovementControl::actor_calculate( Fvector& vAccel,
                                          const Fvector& camDir,
                                          float ang_speed,
                                          float jump,
                                          float dt,
