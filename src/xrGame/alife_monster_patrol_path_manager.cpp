...
CALifeMonsterPatrolPathManager::CALifeMonsterPatrolPathManager(
    object_type* object ) {
    VERIFY( object );
    m_object = object;
...
const CALifeMonsterPatrolPathManager::_GRAPH_ID&
CALifeMonsterPatrolPathManager::target_game_vertex_id() const {
...
const u32& CALifeMonsterPatrolPathManager::target_level_vertex_id() const {
    return (
        path().vertex( m_current_vertex_index )->data().level_vertex_id() );
}
...
void CALifeMonsterPatrolPathManager::select_nearest() {
    m_current_vertex_index = u32( -1 );
    Fvector global_position = ai().game_graph()
    ...
    for ( ; I != E; ++I ) {
        if ( ( *I ).second->data().game_vertex_id() ==
             object().get_object().m_tGraphID ) {
            m_current_vertex_index = ( *I ).second->vertex_id();
            break;
        }

        float distance = global_position.distance_to(
            ai().game_graph()
                .vertex( ( *I ).second->data().game_vertex_id() )
                ->game_point() );
...
void CALifeMonsterPatrolPathManager::navigate() {
    const CPatrolPath::CVertex& vertex =
        *path().vertex( m_current_vertex_index );
    ...
    if ( !branching_factor ) {
        switch ( route_type() ) {
            ...
            case PatrolPathManager::ePatrolRouteTypeContinue: {
                ...
                VERIFY( vertex.edges().front().vertex_id() ==
                        m_previous_vertex_index );
    ...
    u32 chosen =
        use_randomness() ? object().get_object().randI( branching_factor ) : 0;
    u32 branch = 0;
    for ( I = B; I != E; ++I ) {
        if ( *I == m_previous_vertex_index )
            continue;
