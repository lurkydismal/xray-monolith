...
IC bool ignore_object( CObject* O ) {
    VERIFY( O );
    if (
        static_cast< CGameObject* >( O )->cast_entity_alive()
...
IC void tri_plane( const Fvector& v0,
                   const Fvector& v1,
                   const Fvector& v2,
                   Fplane& p ) {
...
IC void tri_plane( const CDB::TRI& tri, Fplane& p ) {
    ...
    tri_plane( pVerts[ tri.verts[ 0 ] ], pVerts[ tri.verts[ 1 ] ],
               pVerts[ tri.verts[ 2 ] ], p );
}

IC bool get_plane_static( ik_pick_result& r,
                          Fvector& next_pos,
                          float& next_range,
    ...
    VERIFY( Level().ObjectSpace.GetStaticModel()->get_tris_count() >
            R.element );
    ...
    if ( ignore_tri( *tri ) ) {
        ...
        float u, v, d;
        VERIFY(
            !( CDB::TestRayTri( next_pos, pick_v, r.triangle, u, v, d, true ) &&
               d > 0.f ) );
#endif // DEBUG
        return false;
    }
...
IC bool get_plane_dynamic( ik_pick_result& r,
                           Fvector& next_pos,
                           float& next_range,
                           const collide::rq_result R,
                           float pick_dist,
    ...
    if ( V ) {
        ...
        if ( K ) {
            ...
            if ( K->PickBone( R.O->XFORM(), res, dist, pos, pick_v,
                              ( u16 )R.element ) ) {
                // cld.collided = true;
...
IC bool get_plane( ik_pick_result& r,
                   ...
                   const Fvector& pick_v ) {
    if ( !R.O )
        return get_plane_static( r, next_pos, next_range, R, pick_dist, pos,
                                 pick_v );
    else
        return get_plane_dynamic( r, next_pos, next_range, R, pick_dist, pos,
                                  pick_v );
}
...
bool Pick( ik_pick_result& r, const ik_pick_query& q, CObject* ignore_object ) {
    ...
    while ( g_pGameLevel->ObjectSpace.RayPick(
        pos, q.dir(), range, collide::rqtBoth, R, ignore_object ) ) {
    ...
    if ( ph_dbg_draw_mask1.test( phDbgDrawIKCollision ) && collided && !R.O ) {
        ...
        if ( tri ) {
            DBG_DrawTri( tri, Level().ObjectSpace.GetStaticVerts(),
...
// void DBG_DrawTri( const Fvector& v0, const Fvector& v1, const Fvector& v2,
// u32 ac, bool solid );

void chose_best_plane( Fplane& p,
                       const Fvector& v,
                       Fplane& p0,
                       Fplane& p1,
                       Fplane& p2 ) {
    MAX_OF( p0.n.dotproduct( v ), p = p0, p1.n.dotproduct( v ), p = p1,
            p2.n.dotproduct( v ), p = p2 );
}
...
void ik_foot_collider::collide( SIKCollideData& cld,
                                const ik_foot_geom& foot_geom,
                                CGameObject* O,
                                bool foot_step ) {
    ...
    const Fvector pos_toe = Fvector().sub(
        foot_geom.pos_toe(), Fvector().mul( toe_pick_v, collide_dist ) );
    ik_pick_query q_toe( ik_foot_geom::toe, pos_toe, toe_pick_v, pick_dist );

    ////////////////////////////////////////////////////////////////////////////////////////
    const Fvector hill_pick_v = cld.m_pick_dir;
    const Fvector pos_heel = Fvector().sub(
        foot_geom.pos_heel(), Fvector().mul( hill_pick_v, collide_dist ) );
    ik_pick_query q_heel( ik_foot_geom::heel, pos_heel, hill_pick_v,
                          pick_dist );

    //////////////////////////////////////////////////////////////////////////////////////////
    const Fvector side_pick_v = cld.m_pick_dir;
    const Fvector pos_side = Fvector().sub(
        foot_geom.pos_side(), Fvector().mul( side_pick_v, collide_dist ) );
    ik_pick_query q_side( ik_foot_geom::side, pos_side, side_pick_v,
                          pick_dist );
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const float foot_length =
        Fvector().sub( pos_toe, pos_heel ).magnitude() * 1.5f;
    ...
    bool toe_heel_compatible =
        cld.collided && heel_collided &&
        Fvector().sub( r_heel.position, r_toe.position ).magnitude() <
            foot_length;
    bool toe_side_compatible =
        cld.collided && side_collided &&
        Fvector().sub( r_side.position, r_toe.position ).magnitude() <
            foot_length;
    ...
    */
    ...
    if ( toe_heel_compatible && toe_side_compatible ) {
        ...
        if ( ph_dbg_draw_mask1.test( phDbgDrawIKCollision ) ) {
            DBG_DrawPoint( pos_toe, 0.01, D3DCOLOR_XRGB( 255, 0, 0 ) );
            if ( cld.collided ) {
                DBG_DrawTri( r_toe.position, r_heel.position, r_side.position,
                             D3DCOLOR_XRGB( 0, 0, 255 ), false );
            }
        }
#endif
        previous_data = cld;
    ...
    /*
            float u,v,d;
            if( ( !cld.collided ||
                      !( CDB::TestRayTri(pos_hill, pick_v, r_toe.triangle, u, v,
       d, true ) && d > 0.f ) ) && Pick( r_hill, pos_hill, pick_v, pick_dist, O
       ) )
            {
                    ...
                    //		Fvector pos_foot = Fvector().sub( foot.c,
       Fvector( ).mul( pick_v, collide_dist ) );
                    //		if( cld.collided && Pick( r_foot, pos_foot,
       pick_v, pick_dist, O ) )
                    //		{
                    //			Fplane plane;
                    //			tri_plane( r_toe.position,
       r_hill.position , r_foot.position, plane );

                    //			DBG_DrawTri(r_toe.position,
       r_hill.position, r_foot.position , D3DCOLOR_XRGB( 255, 255, 255 ), false
       );
                    //
                    //			if( plane.n.dotproduct( r_hill.p.n ) <
       0.f )
                    //			{
                    //				plane.n.invert();
                    ...
                    //
                    //			}
                    //			chose_best_plane( cld.m_plane, pick_v,
       plane, r_hill.p, r_toe.p   );
                    //		}
                    //}
