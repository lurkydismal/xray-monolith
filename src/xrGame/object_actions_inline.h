...
template < typename _item_type >
IC void CObjectActionBase< _item_type >::set_property(
...
void CObjectActionBase< _item_type >::prevent_weapon_state_switch_ugly() {
    // smart_cast<CHudItem&>(object().inventory().ActiveItem()->object()).SetState
    // ( CHUDState::eIdle );
    // smart_cast<CHudItem&>(object().inventory().ActiveItem()->object()).SetNextState
    // ( CHUDState::eIdle ); object().inventory().SetActiveSlot
...
template < typename _item_type >
IC CObjectActionMember< _item_type >::CObjectActionMember(
    _item_type* item,
    CAI_Stalker* owner,
    CPropertyStorage* storage,
    _condition_type condition_id,
