    ...
    for ( u32 i = 0; i < count; ++i ) {
        ...
        if ( itm_index != u32( -1 ) ) {
            ...
            m_wpn_groups.push_back(
                std::make_pair( static_cast< u16 >( itm_index ), gid ) );
        }
    };
}
...
void ammunition_group::init( CItemMgr const* item_manager ) {
    ...
    init_ammunition_group( item_manager, gid_sniper_rifels,
                           tmp_string.c_str() );

    tmp_string = pSettings->r_string( dm_team_data, "heavy_weapons" );
    init_ammunition_group( item_manager, gid_heavy_weapons,
                           tmp_string.c_str() );
    ...
    init_ammunition_group( item_manager, gid_double_barred,
                           tmp_string.c_str() );

    tmp_string = pSettings->r_string( dm_team_data, "gid_hand_grenades" );
    init_ammunition_group( item_manager, gid_hand_grenades,
                           tmp_string.c_str() );
...
bool ammunition_group::is_item_in_group( u16 item_id,
                                         enum_group_id gid ) const {
    if ( gid == gid_any )
        return true;

    ammun_groups_map_t::const_iterator tmp_iter =
        std::find( m_wpn_groups.begin(), m_wpn_groups.end(),
