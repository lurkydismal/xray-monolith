...
random_min              =1       ; х массу объекта = величина случайно
направленного импульса ; с случайн				о выбранной
точкой приложения в пределах нового обекта random_hit_imp         =0.1     ; х
величена хит - импульса =............

;ref_bone                       ; кость из по которой определяется скорость для
частей у который связь не задана по умолчанию рут imp_transition_factor  =0.1 ;
фактор с которым прикладывается хит по исходному объекту ко всем частям
...
void CPHDestroyable::InitServerObject( CSE_Abstract* D ) {
    ...
    CSE_ALifeDynamicObjectVisual* l_tpALifeDynamicObject =
        smart_cast< CSE_ALifeDynamicObjectVisual* >( D );
    ...
    if ( ai().get_alife() )
        l_tpALifeDynamicObject->m_tGraphID =
            ai().game_graph().current_level_vertex();
    else
...
void CPHDestroyable::Load( CInifile* ini, LPCSTR section ) {
    ...
    if ( ini->line_exist( section, "destroyed_vis_name" ) ) {
        ...
        m_destroyed_obj_visual_names.push_back(
            ini->r_string( section, "destroyed_vis_name" ) );
    } else {
...
void CPHDestroyable::Load( LPCSTR section ) {
    ...
    if ( pSettings->line_exist( section, "destroyed_vis_name" ) ) {
        m_flags.set( fl_destroyable, TRUE );
        m_destroyed_obj_visual_names.push_back(
            pSettings->r_string( section, "destroyed_vis_name" ) );
    }
}
...
void CPHDestroyable::NotificatePart( CPHDestroyableNotificate* dn ) {
    ...
    IKinematics* own_K =
        smart_cast< IKinematics* >( PPhysicsShellHolder()->Visual() );
    IKinematics* new_K =
        smart_cast< IKinematics* >( dn->PPhysicsShellHolder()->Visual() );
    ...
    if ( own_ini && own_ini->section_exist( "impulse_transition_to_parts" ) ) {
        random_min =
            own_ini->r_float( "impulse_transition_to_parts", "random_min" );
        random_hit_imp =
            own_ini->r_float( "impulse_transition_to_parts", "random_hit_imp" );
        ////////////////////////////////////////////////////////
        if ( own_ini->line_exist( "impulse_transition_to_parts", "ref_bone" ) )
            ref_bone = own_K->LL_BoneID( own_ini->r_string(
                "impulse_transition_to_parts", "ref_bone" ) );
        imp_transition_factor = own_ini->r_float( "impulse_transition_to_parts",
                                                  "imp_transition_factor" );
        lv_transition_factor = own_ini->r_float( "impulse_transition_to_parts",
                                                 "lv_transition_factor" );
        av_transition_factor = own_ini->r_float( "impulse_transition_to_parts",
                                                 "av_transition_factor" );

        if ( own_ini->section_exist( "collide_parts" ) ) {
            if ( own_ini->line_exist( "collide_parts", "small_object" ) ) {
                new_shell->SetSmall();
            }
            if ( own_ini->line_exist( "collide_parts",
    ...
    if ( new_ini &&
         new_ini->section_exist( "impulse_transition_from_source_bone" ) ) {
        // random_min
        // =new_ini->r_float("impulse_transition_from_source_bone","random_min");
        // random_hit_imp
        // =new_ini->r_float("impulse_transition_from_source_bone","random_hit_imp");
        ////////////////////////////////////////////////////////
        if ( new_ini->line_exist( "impulse_transition_from_source_bone",
                                  "ref_bone" ) )
            ref_bone = own_K->LL_BoneID( new_ini->r_string(
                "impulse_transition_from_source_bone", "ref_bone" ) );
        imp_transition_factor = new_ini->r_float(
            "impulse_transition_from_source_bone", "imp_transition_factor" );
        lv_transition_factor = new_ini->r_float(
            "impulse_transition_from_source_bone", "lv_transition_factor" );
        av_transition_factor = new_ini->r_float(
            "impulse_transition_from_source_bone", "av_transition_factor" );
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for ( u16 i = 0; i < new_el_number; ++i ) {
        ...
        if ( m_fatal_hit.is_valide() && m_fatal_hit.bone() != BI_NONE ) {
            ...
            e->applyImpulseVsGF(
                pos, m_fatal_hit.direction(),
                m_fatal_hit.phys_impulse() * imp_transition_factor );
        ...
        // Fvector
        // res_avell;res_avell.set(cast_fv(dBodyGetAngularVel(own_body)));
        Fvector res_avell;
    ...
    if ( ps ) {
        if ( own_ini && own_ini->section_exist( "autoremove_parts" ) ) {
            ps->SetAutoRemove(
                1000 *
                ( READ_IF_EXISTS( own_ini, r_u32, "autoremove_parts", "time",
                                  ps->DefaultExitenceTime() ) ) );
        }

        if ( new_ini && new_ini->section_exist( "autoremove" ) ) {
            ps->SetAutoRemove(
                1000 * ( READ_IF_EXISTS( new_ini, r_u32, "autoremove", "time",
                                         ps->DefaultExitenceTime() ) ) );
        }
    }
}
...
void CPHDestroyable::NotificateDestroy( CPHDestroyableNotificate* dn ) {
    ...
    if ( !m_depended_objects ) {
        xr_vector< CPHDestroyableNotificate* >::iterator
            i = m_notificate_objects.begin(),
