...
void CWeaponShotEffector::Shot( CWeapon* weapon ) {
    ...
    CCartridge* ammo =
        !weapon->m_magazine.empty() ? &weapon->m_magazine.back() : ( 0 );
    ...
    float angle = m_cam_recoil.Dispersion *
                  ...
                  weapon->cur_launcher_koef.cam_dispersion * k_cam_disp;
    angle += m_cam_recoil.DispersionInc *
             weapon->cur_silencer_koef.cam_disper_inc *
             weapon->cur_scope_koef.cam_disper_inc *
             weapon->cur_launcher_koef.cam_disper_inc * ( float )m_shot_numer;
    Shot2( angle );
}

void CWeaponShotEffector::Shot2( float angle ) {
    m_angle_vert += angle * ( m_cam_recoil.DispersionFrac +
                              m_Random.randF( -1.0f, 1.0f ) *
                                  ( 1.0f - m_cam_recoil.DispersionFrac ) );

    clamp( m_angle_vert, -m_cam_recoil.MaxAngleVert,
           m_cam_recoil.MaxAngleVert );
    ...
    if ( g_decouple_horz_recoil ) {
        ...
        m_angle_horz += ( m_angle_vert / m_cam_recoil.MaxAngleVert ) * rdm *
                        m_cam_recoil.StepAngleHorz;
    }

    clamp( m_angle_horz, -m_cam_recoil.MaxAngleHorz,
           m_cam_recoil.MaxAngleHorz );
...
void CWeaponShotEffector::Relax() {
    ...
    float relax_speed_horz = ( fis_zero( time_to_relax ) )
                                 ? 0.0f
                                 : _abs( m_angle_horz ) / time_to_relax;
