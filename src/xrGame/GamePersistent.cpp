...
//{ return Memory.mem_alloc(size,"ODE");			} 	static
//void *	ode_realloc	(void *ptr, size_t oldsize, size_t newsize)
//{ return Memory.mem_realloc(ptr,newsize,"ODE");	} 	static	void
//ode_free	(void *ptr, size_t size)
//{ return xr_free(ptr); } #else // DEBUG_MEMORY_MANAGER 	static	void *
//ode_alloc	(size_t size)
//{ return xr_malloc(size);			} 	static	void *
//ode_realloc	(void *ptr, size_t oldsize, size_t newsize)	{ return
//xr_realloc(ptr,newsize);	} 	static	void	ode_free	(void
//*ptr, size_t size)					{ return xr_free(ptr);
...
void CGamePersistent::RegisterModel( IRenderVisual* V ) {
    ...
    switch ( V->getType() ) {
        ...
        case MT_SKELETON_RIGID: {
            ...
            R_ASSERT2( GMLib.GetMaterialByIdx( def_idx )->Flags.is(
                           SGameMtl::flDynamic ),
            ...
            for ( u16 k = 0; k < cnt; k++ ) {
                ...
                if ( *( bd.game_mtl_name ) ) {
                    ...
                    R_ASSERT2( GMLib.GetMaterialByIdx( bd.game_mtl_idx )
                                   ->Flags.is( SGameMtl::flDynamic ),
...
void CGamePersistent::OnAppStart() {
    ...
    Msg( "* [STARTUP] native barrier: wait=%d materials-work=%u ms",
         timer.GetElapsed_ms(), materialsMs );
    timer.Start();
...
EGameIDs ParseStringToGameType( LPCSTR str ) {
    ...
    else if ( !xr_strcmp( str, "capturetheartefact" ) ||
              !xr_strcmp( str, "cta" ) )
...
void CGamePersistent::UpdateGameType() {
    ...
    m_game_params.m_e_game_type =
        ParseStringToGameType( m_game_params.m_game_type );
...
void CGamePersistent::WeathersUpdate() {
    ...
    if ( g_pGameLevel && !g_dedicated_server ) {
        ...
        int data_set =
            ( Random.randF() < ( 1.f - Environment().CurrentEnv->weight ) ) ? 0
                                                                            : 1;

        CEnvDescriptor* const _env = Environment().Current[ data_set ];
        VERIFY( _env );

        CEnvAmbient* env_amb = Environment().m_paused
                                   ? Environment().CurrentEnv->env_ambient
                                   : _env->env_ambient;
        if ( env_amb ) {
            ...
            for ( size_t idx = 0; I != E; ++I, ++idx ) {
                ...
                if ( ambient_sound_next_time[ idx ] == 0 ) {
                    ambient_sound_next_time[ idx ] =
                        Device.dwTimeGlobal + ch.get_rnd_sound_first_time();
                    ...
                    pos.z = _sin( angle );
                    pos.normalize()
                        .mul( ch.get_rnd_sound_dist() )
                        .add( Device.vCameraPosition );
                    pos.y += 10.f;
                    snd.play_at_pos( 0, pos );

                    if ( !snd._handle() ||
                         Core.ParamsData.test( ECoreParams::nosound ) )
                        continue;

                    VERIFY( snd._handle() );
                    u32 _length_ms = iFloor( snd.get_length_sec() * 1000.0f );
                    ambient_sound_next_time[ idx ] = Device.dwTimeGlobal +
                                                     _length_ms +
                                                     ch.get_rnd_sound_time();
                }
            }

            // start effect
            if ( ( FALSE == bIndoor ) && ( 0 == ambient_particles ) &&
                 Device.dwTimeGlobal > ambient_effect_next_time ) {
                CEnvAmbient::SEffect* eff = env_amb->get_rnd_effect();
                if ( eff ) {
                    Environment().wind_gust_factor = eff->wind_gust_factor;
                    ambient_effect_next_time =
                        Device.dwTimeGlobal + env_amb->get_rnd_effect_time();
                    ambient_effect_stop_time =
                        Device.dwTimeGlobal + eff->life_time;
                    ambient_effect_wind_start = Device.fTimeGlobal;
                    ambient_effect_wind_in_time =
                        Device.fTimeGlobal + eff->wind_blast_in_time;
                    ambient_effect_wind_end =
                        Device.fTimeGlobal + eff->life_time / 1000.f;
                    ambient_effect_wind_out_time = Device.fTimeGlobal +
                                                   eff->life_time / 1000.f +
                                                   eff->wind_blast_out_time;
                    ambient_effect_wind_on = true;

                    ambient_particles = Particles::Details::Create(
                        eff->particles.c_str(), FALSE, false );
                    Fvector pos;
                    Fvector offset = eff->offset;
                    offset.x +=
                        Random.randF( 0.5f, 5.f ) *
                        ( Random.randF( 0.f, 1.f ) < 0.5f ? -1.f : 1.f );
                    offset.z +=
                        Random.randF( 0.5f, 5.f ) *
                        ( Random.randF( 0.f, 1.f ) < 0.5f ? -1.f : 1.f );
                    ...
                    Environment().wind_blast_strength_stop_value =
                        eff->wind_blast_strength;

                    if ( Environment().wind_blast_strength_start_value ==
                         0.f ) {
                        Environment().wind_blast_start_time.set(
                            0.f, eff->wind_blast_direction.x,
                            eff->wind_blast_direction.y,
                            eff->wind_blast_direction.z );
                    } else {
                        Environment().wind_blast_start_time.set(
                            0.f, Environment().wind_blast_direction.x,
        ...
        if ( Device.fTimeGlobal >= ambient_effect_wind_start &&
             Device.fTimeGlobal <= ambient_effect_wind_in_time &&
             ambient_effect_wind_on ) {
            float delta =
                ambient_effect_wind_in_time - ambient_effect_wind_start;
            float t;
            if ( delta != 0.f ) {
            ...
            Environment().wind_strength_factor =
                Environment().wind_blast_strength_start_value +
                t * ( Environment().wind_blast_strength_stop_value -
                      Environment().wind_blast_strength_start_value );
        }
        ...
        if ( Device.fTimeGlobal >= ambient_effect_wind_end &&
             ambient_effect_wind_on ) {
            Environment().wind_blast_strength_start_value =
                Environment().wind_strength_factor;
        ...
        if ( Device.fTimeGlobal >= ambient_effect_wind_end &&
             Device.fTimeGlobal <= ambient_effect_wind_out_time ) {
            float delta =
                ambient_effect_wind_out_time - ambient_effect_wind_end;
            float t;
            if ( delta != 0.f ) {
        ...
        if ( Device.fTimeGlobal > ambient_effect_wind_out_time &&
             ambient_effect_wind_out_time != 0.f ) {
...
void CGamePersistent::start_logo_intro() {
    if ( Device.dwPrecacheFrame == 0 ) {
        ...
        if ( !g_dedicated_server &&
             0 == xr_strlen( m_game_params.m_game_or_spawn ) &&
             NULL == g_pGameLevel ) {
            VERIFY( NULL == m_intro );
...
void CGamePersistent::game_loaded() {
    if ( Device.dwPrecacheFrame <= 2 ) {
        if ( g_pGameLevel && g_pGameLevel->bReady &&
            ...
            m_intro->m_on_destroy_event.bind(
                this, &CGamePersistent::update_game_loaded );

            // demonized: Reset mouse state on loading the game
            pInput->resetMouseState();

            // demonized
            // Callback for when loading screen happens and "Press Any Key to
            // Continue" prompt appears
            ::luabind::functor< void > funct;
            if ( ai().script_engine().functor( "_G.OnLoadingScreenKeyPrompt",
                                               funct ) ) {
                funct();
            }
        }

        else if ( g_pGameLevel && g_pGameLevel->bReady &&
                  m_game_params.m_e_game_type == eGameIDSingle ) {
            Msg( "intro_start game_loaded" );
            ...
            // Callback for when player dismisses loading screen after "Press
            // Any Key to Continue" pressed
            if ( ai().script_engine().functor( "_G.OnLoadingScreenDismissed",
                                               funct ) ) {
                funct();
            }
        }
...
void CGamePersistent::update_game_loaded() {
    ...
    // Callback for when player dismisses loading screen after "Press Any Key to
    // Continue" pressed
    ::luabind::functor< void > funct;
    if ( ai().script_engine().functor( "_G.OnLoadingScreenDismissed",
                                       funct ) ) {
...
void CGamePersistent::OnFrame() {
    ...
    if ( !g_dedicated_server && Device.dwPrecacheFrame == 0 && !m_intro &&
         m_intro_event.empty() )
        load_screen_renderer.stop();
    ...
    if ( Device.Paused() ) {
        if ( Level().IsDemoPlay() ) {
            CSpectator* tmp_spectr =
                smart_cast< CSpectator* >( Level().CurrentControlEntity() );
            if ( tmp_spectr ) {
                tmp_spectr->UpdateCL(); // updating spectator in pause (pause
                                        // ability of demo play)
            }
        }
#ifndef MASTER_GOLD
        if ( Level().CurrentViewEntity() && IsGameTypeSingle() ) {
            if ( !g_actor ||
                 ( g_actor->ID() != Level().CurrentViewEntity()->ID() ) ) {
                CCustomMonster* custom_monster = smart_cast< CCustomMonster* >(
                ...
                if ( g_actor ) {
                    ...
                    if ( psActorFlags.test( AF_NO_CLIP ) ) {
                        ...
                        CSE_Abstract* e =
                            Level().Server->ID_to_entity( Actor()->ID() );
                        VERIFY( e );
                        CSE_ALifeCreatureActor* s_actor =
                            smart_cast< CSE_ALifeCreatureActor* >( e );
                        VERIFY( s_actor );
                        xr_vector< u16 >::iterator it =
                            s_actor->children.begin();
    ...
    if ( 0 != pDemoFile ) {
        if ( Device.dwTimeGlobal > uTime2Change ) {
            ...
            u32 o_time;
            sscanf( params, "%[^,],%[^,],%[^,],%d", o_server, o_client, o_demo,
                    &o_time );

            // Start _new level + demo
            Engine.Event.Defer( "KERNEL:disconnect" );
            Engine.Event.Defer( "KERNEL:start",
                                size_t( xr_strdup( _Trim( o_server ) ) ),
                                size_t( xr_strdup( _Trim( o_client ) ) ) );
            Engine.Event.Defer( "GAME:demo",
                                size_t( xr_strdup( _Trim( o_demo ) ) ),
                                u64( o_time ) );
...
void CGamePersistent::ImGui_OnRender( LPCSTR name ) {
    ...
    if ( ai().script_engine().functor( "callbacks_gameobject.on_imgui_render",
                                       imgui_render ) ) {
        imgui_render( name );
    }
}
...
void CGamePersistent::LoadTitle( bool change_tip, shared_str map_name ) {
    ...
    if ( change_tip ) {
        ...
        if ( is_single ) {
            R_ASSERT( ai().script_engine().functor( "loadscreen.get_tip_number",
                                                    m_functor ) );
            tip_num = m_functor( map_name.c_str() );
        } else {
            R_ASSERT( ai().script_engine().functor(
                "loadscreen.get_mp_tip_number", m_functor ) );
            tip_num = m_functor( map_name.c_str() );
        }
        ...
        //		tip_num = 83;
        xr_sprintf(
            buff, "%s%d:", CStringTable().translate( "ls_tip_number" ).c_str(),
            tip_num );
        shared_str tmp = buff;
...
void CGamePersistent::UpdateDof() {
    ...
    static float diff_far =
        pSettings->r_float( "zone_pick_dof", "far" ); // 70.0f;
    static float diff_near =
        pSettings->r_float( "zone_pick_dof", "near" ); //-70.0f;
    ...
    m_dof[ 1 ].add( diff );
    ( m_dof[ 0 ].x < m_dof[ 2 ].x )
        ? clamp( m_dof[ 1 ].x, m_dof[ 0 ].x, m_dof[ 2 ].x )
        : clamp( m_dof[ 1 ].x, m_dof[ 2 ].x, m_dof[ 0 ].x );
    ( m_dof[ 0 ].y < m_dof[ 2 ].y )
        ? clamp( m_dof[ 1 ].y, m_dof[ 0 ].y, m_dof[ 2 ].y )
        : clamp( m_dof[ 1 ].y, m_dof[ 2 ].y, m_dof[ 0 ].y );
    ( m_dof[ 0 ].z < m_dof[ 2 ].z )
        ? clamp( m_dof[ 1 ].z, m_dof[ 0 ].z, m_dof[ 2 ].z )
