...
bool ReadRegistryValue( LPCSTR rKeyName, DWORD rKeyType, void* value ) {
    ...
    res = RegQueryValueEx( hKey, rKeyName, NULL, &rKeyType, ( LPBYTE )rBuf,
                           &KeyValueSize );
    if ( hKey != 0 )
        RegCloseKey( hKey );
...
bool WriteRegistryValue( LPCSTR rKeyName, DWORD rKeyType, const void* value ) {
    ...
    long res =
        RegOpenKeyEx( REGISTRY_BASE, REGISTRY_PATH, 0, KEY_WRITE, &hKey );
    ...
    res = RegSetValueEx( hKey, rKeyName, NULL, rKeyType, ( LPBYTE )value,
                         KeyValueSize );
...
u32 const ReadRegistry_BinaryValue( LPCSTR rKeyName,
                                    u8* buffer_dest,
    ...
    res = RegQueryValueEx( hKey, rKeyName, NULL, &value_type, buffer_dest,
                           &tmp_buffer_size );
...
void WriteRegistry_BinaryValue( LPCSTR rKeyName,
                                u8 const* buffer_src,
                                u32 const buffer_size ) {
    HKEY hKey;
    ...
    res = RegSetValueEx( hKey, rKeyName, NULL, REG_BINARY, buffer_src,
                         buffer_size );
