...
#define CDoubleLinkedList                                                \
    CDataStorageDoubleLinkedList< sorted >::CDataStorage< _data_storage, \
                                                          _vertex >

TEMPLATE_SPECIALIZATION
IC CDoubleLinkedList::CDataStorage( const u32 vertex_count,
                                    const _dist_type _max_distance )
...
TEMPLATE_SPECIALIZATION
IC void CDoubleLinkedList::decrease_opened( CGraphVertex& vertex,
    ...
    if ( vertex.f() <= m_list_head->next()->f() ) {
        ...
        if ( vertex.prev()->f() - vertex.f() <
             m_switch_factor * ( vertex.f() - m_list_head->next()->f() ) )
...
TEMPLATE_SPECIALIZATION
IC typename CDoubleLinkedList::CGraphVertex& CDoubleLinkedList::get_best()
    const {
    VERIFY( !is_opened_empty() );
    if ( sorted )
...
TEMPLATE_SPECIALIZATION
IC void CDoubleLinkedList::set_switch_factor(
    const _dist_type _switch_factor ) {
    if ( !sorted )
        NODEFAULT;
