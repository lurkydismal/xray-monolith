...
void Upgrade::construct( const shared_str& upgrade_id,
    ...
    m_description =
        CStringTable().translate( pSettings->r_string( id(), "description" ) );
    ...
    VERIFY2( pSettings->section_exist( section_str ),
             make_string( "Upgrade <%s> : settings section [%s] not exist!",
                          id_str(), section_str ) );
    VERIFY2( pSettings->line_count( section_str ),
             make_string( "Upgrade <%s> : settings section [%s] is empty !",
                          id_str(), section_str ) );

    m_section._set( section_str );

    // precondition_functor
    LPCSTR precondition_functor_str =
        pSettings->r_string( id(), "precondition_functor" );
    m_preconditions.parameter =
        pSettings->r_string( id(), "precondition_parameter" );
    m_preconditions.parameter2 = m_section.c_str();
    R_ASSERT2(
        ai().script_engine().functor( precondition_functor_str,
                                      m_preconditions.functr ),
        make_string(
            "Failed to get precondition functor in section[%s], functor[%s]",
    ...
    LPCSTR prereq_functor_str =
        pSettings->r_string( id(), "prereq_functor" ); // prerequisites_functor
    //	LPCSTR tooltip_functor_str	= pSettings->r_string( id(),
    //"prereq_tooltip_functor" );
    m_prerequisites.parameter =
        pSettings->r_string( id(), "prereq_params" ); // prerequisites_params
    m_prerequisites.parameter2 = m_section.c_str();
    //	m_tooltip.parameter			= pSettings->r_string( id(),
    //"prereq_params" );
    R_ASSERT2(
        ai().script_engine().functor( prereq_functor_str,
                                      m_prerequisites.functr ),
        make_string(
            "Failed to get prerequisites functor in section[%s], functor[%s]",
    ...
    VERIFY2( properties.size(),
             make_string( "Upgrade <%s> : property is empty !", id_str() ) );
    ...
    for ( u8 i = 0; i < max_properties_count; i++ ) {
        ...
        if ( prop.size() ) {
            ...
            VERIFY2( manager_r.get_property( prop ),
                     make_string( "Upgrade <%s> : property [%s] is unknown "
                                  "(not found in upgrade manager) !",
                                  id_str(), prop.c_str() ) );
        }
    }
...
bool Upgrade::check_scheme_index( Ivector2 const& scheme_index ) {
    return ( m_scheme_index.x == scheme_index.x &&
             m_scheme_index.y == scheme_index.y );
}
