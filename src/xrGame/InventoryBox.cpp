...
void CInventoryBox::OnEvent( NET_Packet& P, u16 type ) {
    ...
    switch ( type ) {
        ...
        case GE_OWNERSHIP_TAKE: {
            ...
            VERIFY( pIItem );
            if ( CurrentGameUI() ) {
                if ( CurrentGameUI()->GetActorMenu().GetMenuMode() ==
                     mmDeadBodySearch ) {
                    if ( this == CurrentGameUI()->GetActorMenu().GetInvBox() )
                        CurrentGameUI()->OnInventoryAction( pIItem,
                                                            GE_OWNERSHIP_TAKE );
                }
            };
        } break;
        ...
        case GE_OWNERSHIP_REJECT: {
            ...
            if ( m_in_use ) {
                ...
                Actor()->callback( GameObject::eInvBoxItemTake )(
                    this->lua_game_object(), GO->lua_game_object() );
            }
        } break;
    };
}
...
void CInventoryBox::AddAvailableItems(
    TIItemContainer& items_container ) const {
