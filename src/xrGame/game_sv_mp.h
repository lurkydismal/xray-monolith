...
protected:
    ...
    virtual void DestroyAllPlayerItems(
        ClientID id_who ); // except rukzak and artefact :)
...
protected:
    virtual void SendPlayerKilledMessage( u16 KilledID,
                                          KILL_TYPE KillType,
                                          u16 KillerID,
                                          u16 WeaponID,
    ...
    void SpawnWeapon4Actor( u16 actorId,
                            LPCSTR N,
                            u8 Addons,
    ...
    virtual void SetAmmoForWeapon(
        CSE_ALifeItemWeapon* weapon,
        u8 Addons,
        game_PlayerState::PLAYER_ITEMS_LIST& playerItems,
        ammo_diff_t& ammo_diff );
    void ChargeAmmo( CSE_ALifeItemWeapon* weapon,
                     LPCSTR ammo_string,
                     game_PlayerState::PLAYER_ITEMS_LIST& playerItems,
                     ammo_diff_t& ammo_diff );
    void ChargeGrenades( CSE_ALifeItemWeapon* weapon,
                         LPCSTR grenades_string,
                         game_PlayerState::PLAYER_ITEMS_LIST& playerItems );
    ...
    // [14.11.07] Alexander Maniluk: added this method for renewing health and
    // power of all actors in game. This method sets the health of all CActor
    // objects (on server site) to 1.0, after that server Update will send to
    // all clients new states of health.
...
public:
    ...
    IClient* BanPlayer( ClientID const& client_id,
                        s32 ban_time_sec,
                        xrClientData* initiator );
    void BanPlayerDirectly( char const* client_hex_digest,
                            s32 ban_time_sec,
                            xrClientData* initiator );
...
protected:
    ...
    s32 ExcludeBanTimeFromVoteStr( char const* vote_string,
                                   char* new_vote_str,
                                   u32 new_vote_str_size );

public:
    virtual void WritePlayerStats( CInifile& ini,
                                   LPCSTR sect,
                                   xrClientData* pCl );
