...
void CEncyclopediaArticle::load_shared( LPCSTR ) {
    ...
    if ( ltx ) {
        ...
        Frect tex_rect;
        tex_rect.x1 =
            float( pSettings->r_u32( ltx, "inv_grid_x" ) * INV_GRID_WIDTH );
        tex_rect.y1 =
            float( pSettings->r_u32( ltx, "inv_grid_y" ) * INV_GRID_HEIGHT );
        tex_rect.x2 =
            float( pSettings->r_u32( ltx, "inv_grid_width" ) * INV_GRID_WIDTH );
        tex_rect.y2 = float( pSettings->r_u32( ltx, "inv_grid_height" ) *
                             INV_GRID_HEIGHT );
    ...
    if ( data()->image.GetShader() && data()->image.GetShader()->inited() ) {
        ...
        if ( r.width() < minSize ) {
            ...
            r.x2 += dx;
            data()->image.SetTextureOffset(
                dx / 2, data()->image.GetTextureOffeset()[ 1 ] );
        }
        ...
        if ( r.height() < minSize ) {
            ...
            r.y2 += dy;
            data()->image.SetTextureOffset(
                data()->image.GetTextureOffeset()[ 0 ], dy / 2 );
        }
    ...
    // Тип статьи
    xr_string atricle_type =
        pXML->ReadAttrib( pNode, "article_type", "encyclopedia" );
    ...
    data()->ui_template_name =
        pXML->ReadAttrib( pNode, "ui_template", "common" );
}
