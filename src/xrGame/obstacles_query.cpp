...
void obstacles_query::set_intersection( const obstacles_query& query ) {
    ...
    OBSTACLES::value_type* temp =
        ( OBSTACLES::value_type* )_alloca( buffer_size );
    Memory.mem_copy( temp, &*obstacles().begin(), buffer_size );
    m_obstacles.erase(
        std::set_intersection( temp, temp + n, query.obstacles().begin(),
                               query.obstacles().end(), m_obstacles.begin() ),
        m_obstacles.end() );
...
void obstacles_query::merge( const AREA& object_area ) {
    ...
    m_area.erase( std::set_union( temp, temp + area_size, object_area.begin(),
                                  object_area.end(), m_area.begin() ),
                  m_area.end() );
}
...
bool obstacles_query::merge( const Fvector& position,
                             const float& radius,
...
bool obstacles_query::objects_changed( const Fvector& position,
...
struct too_far_predicate {
    ...
    IC bool operator()(
        const std::pair< const CGameObject*, u32 >& object ) const {
        ...
        for ( ; I != E; ++I ) {
            ...
            const float distance_sqr =
                vertex_position.distance_to_sqr( m_position );
...
bool obstacles_query::remove_objects( const Fvector& position,
                                      const float& radius ) {
    update_objects( position, radius );
...
void obstacles_query::remove_links( CObject* object ) {
    OBSTACLES::iterator I =
        m_obstacles.find( smart_cast< CGameObject* >( object ) );
    if ( I == m_obstacles.end() )
        return;
