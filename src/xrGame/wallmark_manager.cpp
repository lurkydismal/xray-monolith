...
void CWalmarkManager::AddWallmark( const Fvector& dir,
    ...
    if ( pMaterial->Flags.is( SGameMtl::flBloodmark ) ) {
        ...
        if ( !wallmarks_vector.empty() ) {
            ::Render->add_StaticWallmark( &wallmarks_vector, end_point,
...
/*
void CWalmarkManager::PlaceWallmark(const Fvector& dir, const Fvector&
start_pos, float trace_dist, float wallmark_size, SHADER_VECTOR&
...
void CWalmarkManager::PlaceWallmarks( const Fvector& start_pos ) {
    ...
    //(xr_make_delegate(this,&CWalmarkManager::StartWorkflow));

    StartWorkflow();
}

float Distance( const Fvector& rkPoint,
...
void CWalmarkManager::StartWorkflow() {
    ...
    XRC.box_options( 0 );
    XRC.box_query( Level().ObjectSpace.GetStaticModel(), m_pos,
                   Fvector().set( m_trace_dist, m_trace_dist, m_trace_dist ) );
    ...
    */
    for ( CDB::RESULT* Res = R_begin; Res != R_end; ++Res ) {
        ...
        float dist =
            Distance( m_pos, _tri, pfSParam, pfTParam, end_point, pdir );
        ...
        */
        ...
        if ( test > 0.f ) {
            if ( Level().ObjectSpace.RayTest(
                     m_pos, pdir, test, collide::rqtStatic, NULL, m_owner ) ) {
                ++_ray_test;
                continue;
            }
        }
        if ( fis_zero( pfSParam ) || fis_zero( pfTParam ) ||
             fsimilar( pfSParam, 1.0f ) || fsimilar( pfTParam, 1.0f ) ) {
            ++_tri_not_plane;
            continue;
        }

        if ( dist <= m_trace_dist ) {
            ::Render->add_StaticWallmark( &*m_wallmarks, end_point,
                                          m_wallmark_size, _t, V_array );
            ++wm_count;
        } else
            ++_not_dist;
    }
    /*
            Msg("----------------------------------");
            Msg("tri count=%d",
       XRC.r_count()); Msg("far_dist=%d",
       _not_dist); Msg("RayTest = %d",
       _ray_test); Msg("c==tdBehind = %d",
       _tri_behind); Msg	("c!=tdPlane && dist>ndist = %d",
       _tri_not_plane); Msg("Wallmarks added = %d",
       wm_count); Msg("Time: %d",
       T.GetElapsed_ms());
