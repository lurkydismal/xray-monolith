...
bool CPHMovementControl::ActivateBoxDynamic( DWORD id,
    ...
    if ( character_exist && trying_times[ id ] != u32( -1 ) ) {
        // Fvector
        // dif;dif.sub(trying_poses[id],cast_fv(dBodyGetPosition(m_character->get_body())));
        Fvector character_body_pos;
        m_character->get_body_position( character_body_pos );
        Fvector dif;
        dif.sub( trying_poses[ id ], character_body_pos );
        if ( Device.dwTimeGlobal - trying_times[ id ] < 500 &&
             dif.magnitude() < 0.05f )
            return false;
    }
    if ( !m_character ||
         ( m_character->PhysicsRefObject() &&
           m_character->PhysicsRefObject()->ObjectPPhysicsShell() ) )
        return false;
    DWORD old_id = BoxID();
    ...
    bool ret = ::ActivateBoxDynamic( this, character_exist, id, num_it,
                                     num_steps, resolve_depth );
    ...
    if ( !ret && character_exist ) {
        ...
        m_character->GetBodyPosition(
            trying_poses
                [ id ] ); //.set(cast_fv(dBodyGetPosition(m_character->get_body())));
    } else {
