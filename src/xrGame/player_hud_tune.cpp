...
// "press SHIFT+NUM 0-return 1-hud_pos 2-hud_rot 3-itm_pos 4-itm_rot
// 5-fire_point 6-fire_2_point 7-shell_point";
...
void attachable_hud_item::tune( Ivector values ) {
    ...
    if ( hud_adj_mode == 3 || hud_adj_mode == 4 ) {
        ...
        if ( ( values.x ) || ( values.y ) || ( values.z ) ) {
            ...
            Msg( "item_position				= %f,%f,%f",
                 m_measures.m_item_attach[ 0 ].x,
                 m_measures.m_item_attach[ 0 ].y,
                 m_measures.m_item_attach[ 0 ].z );
            Msg( "item_orientation			= %f,%f,%f",
                 m_measures.m_item_attach[ 1 ].x,
                 m_measures.m_item_attach[ 1 ].y,
    ...
    if ( hud_adj_mode == 5 || hud_adj_mode == 6 || hud_adj_mode == 7 ) {
        ...
        if ( ( values.x ) || ( values.y ) || ( values.z ) ) {
            Msg( "[%s]", m_sect_name.c_str() );
            Msg( "fire_point				= %f,%f,%f",
                 m_measures.m_fire_point_offset.x,
                 m_measures.m_fire_point_offset.y,
                 m_measures.m_fire_point_offset.z );
            Msg( "fire_point2			= %f,%f,%f",
                 m_measures.m_fire_point2_offset.x,
                 m_measures.m_fire_point2_offset.y,
                 m_measures.m_fire_point2_offset.z );
            Msg( "shell_point			= %f,%f,%f",
                 m_measures.m_shell_point_offset.x,
                 m_measures.m_shell_point_offset.y,
                 m_measures.m_shell_point_offset.z );
            Log( "-----------" );
        }
    }
...
void attachable_hud_item::debug_draw_firedeps() {
    ...
    if ( hud_adj_mode == 5 || hud_adj_mode == 6 || hud_adj_mode == 7 ||
        ...
        if ( hud_adj_mode == 5 || bForce )
            render.draw_aabb( fd.vLastFP, 0.005f, 0.005f, 0.005f,
                              D3DCOLOR_XRGB( 255, 0, 0 ) );

        if ( hud_adj_mode == 6 )
            render.draw_aabb( fd.vLastFP2, 0.005f, 0.005f, 0.005f,
                              D3DCOLOR_XRGB( 0, 0, 255 ) );

        if ( hud_adj_mode == 7 )
            render.draw_aabb( fd.vLastSP, 0.005f, 0.005f, 0.005f,
...
void player_hud::tune( Ivector _values ) {
    ...
    if ( hud_adj_mode == 1 || hud_adj_mode == 2 ) {
        ...
        u8 idx = m_attached_items[ hud_adj_item_idx ]
                     ->m_parent_hud_item->GetCurrentHudOffsetIdx();
        if ( idx )
            _curr_dr /= 20.0f;

        Fvector& pos_ =
            ( idx != 0 )
                ? m_attached_items[ hud_adj_item_idx ]->hands_offset_pos()
                : m_attached_items[ hud_adj_item_idx ]->hands_attach_pos();
        Fvector& rot_ =
            ( idx != 0 )
                ? m_attached_items[ hud_adj_item_idx ]->hands_offset_rot()
        ...
        if ( ( values.x ) || ( values.y ) || ( values.z ) ) {
            if ( idx == 0 ) {
                Msg(
                    "[%s]",
                    m_attached_items[ hud_adj_item_idx ]->m_sect_name.c_str() );
                Msg( "hands_position%s				= %f,%f,%f",
                     ( is_16x9 ) ? "_16x9" : "", pos_.x, pos_.y, pos_.z );
                Msg( "hands_orientation%s			= %f,%f,%f",
                     ( is_16x9 ) ? "_16x9" : "", rot_.x, rot_.y, rot_.z );
                Log( "-----------" );
            } else if ( idx == 1 ) {
                Msg(
                    "[%s]",
                    m_attached_items[ hud_adj_item_idx ]->m_sect_name.c_str() );
                Msg( "aim_hud_offset_pos%s				= "
                     "%f,%f,%f",
                     ( is_16x9 ) ? "_16x9" : "", pos_.x, pos_.y, pos_.z );
                Msg( "aim_hud_offset_rot%s				= "
                     "%f,%f,%f",
                     ( is_16x9 ) ? "_16x9" : "", rot_.x, rot_.y, rot_.z );
                Log( "-----------" );
            } else if ( idx == 2 ) {
                Msg(
                    "[%s]",
                    m_attached_items[ hud_adj_item_idx ]->m_sect_name.c_str() );
                Msg( "gl_hud_offset_pos%s				= "
                     "%f,%f,%f",
                     ( is_16x9 ) ? "_16x9" : "", pos_.x, pos_.y, pos_.z );
                Msg( "gl_hud_offset_rot%s				= "
                     "%f,%f,%f",
                     ( is_16x9 ) ? "_16x9" : "", rot_.x, rot_.y, rot_.z );
                Log( "-----------" );
            }
...
void hud_draw_adjust_mode() {
    ...
    if ( pInput->iGetAsyncKeyState( DIK_LSHIFT ) && hud_adj_mode )
        _text =
            "press SHIFT+NUM 0-return 1-hud_pos 2-hud_rot 3-itm_pos 4-itm_rot "
            "5-fire_point 6-fire_2_point 7-shell_point 8-pos_step 9-rot_step";
