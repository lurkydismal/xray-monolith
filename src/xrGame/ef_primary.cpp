...
IC CLASS_ID CBaseFunction::clsid_member_item() const {
    ...
    if ( ef_storage().non_alife().member_item() )
        ...
        VERIFY2( ef_storage().alife().member_item(),
                 "No object specified for evaluation function" );
...
IC CLASS_ID CBaseFunction::clsid_enemy_item() const {
    ...
    if ( ef_storage().non_alife().enemy_item() )
        ...
        VERIFY2( ef_storage().alife().enemy_item(),
                 "No object specified for evaluation function" );
...
IC CLASS_ID CBaseFunction::clsid_member() const {
    ...
    if ( ef_storage().non_alife().member() )
        ...
        VERIFY2( ef_storage().alife().member(),
                 "No object specified for evaluation function" );
        const CSE_ALifeDynamicObject* l_tpALifeDynamicObject =
            smart_cast< const CSE_ALifeDynamicObject* >(
                ef_storage().alife().member() );
        VERIFY2( l_tpALifeDynamicObject,
                 "Invalid object passed to the evaluation function" );
...
IC CLASS_ID CBaseFunction::clsid_enemy() const {
    ...
    if ( ef_storage().non_alife().enemy() )
        ...
        VERIFY2( ef_storage().alife().enemy(),
                 "No object specified for evaluation function" );
        const CSE_ALifeDynamicObject* l_tpALifeDynamicObject =
            smart_cast< const CSE_ALifeDynamicObject* >(
...
float CDistanceFunction::ffGetValue() {
    if ( ef_storage().non_alife().member() )
        return ( ef_storage().non_alife().member()->Position().distance_to(
            ef_storage().non_alife().enemy()->Position() ) );
    else
...
float CPersonalHealthFunction::ffGetValue() {
    if ( ef_storage().non_alife().member() ) {
        ...
        const CSE_ALifeMonsterAbstract* l_tpALifeMonsterAbstract =
            smart_cast< const CSE_ALifeMonsterAbstract* >(
                ef_storage().alife().member() );
        VERIFY3( l_tpALifeMonsterAbstract,
                 "Invalid object passed to the evaluation function ",
                 m_caName );
...
float CPersonalWeaponTypeFunction::ffGetTheBestWeapon() {
    ...
    if ( ef_storage().non_alife().member() &&
         ef_storage().non_alife().member_item() )
        return ( float( dwfGetWeaponType() ) );

    if ( ef_storage().non_alife().member() ) {
        const CInventoryOwner* tpInventoryOwner =
            smart_cast< const CInventoryOwner* >(
                ef_storage().non_alife().member() );
        if ( tpInventoryOwner ) {
            ...
            for ( ; I <= E; ++I ) {
                ...
                if ( iitem ) {
                    ...
                    if ( tpCustomWeapon &&
                         ( tpCustomWeapon->GetSuitableAmmoTotal( true ) >
                           tpCustomWeapon->GetAmmoMagSize() / 10 ) ) {
        ...
        if ( !ef_storage().alife().member() ||
             !ef_storage().alife().member()->m_tpCurrentBestWeapon )
            return ( 0 );
...
float CPersonalWeaponTypeFunction::ffGetValue() {
    ...
    if ( ef_storage().non_alife().member() )
        if ( ef_storage().non_alife().member()->natural_weapon() )
            result =
                ( float )ef_storage().non_alife().member()->ef_weapon_type();
        else
...
float CEnemyRukzakWeightFunction::ffGetValue() {
    ...
    if ( ef_storage().non_alife().member() ) {
        ...
        //smart_cast<CSE_ALifeHumanAbstract*>(ef_storage().alife().member()); 		if
        //(l_tpALifeHumanAbstract) 			m_fLastValue	=
...
float CGraphPointType0::ffGetValue() {
    return ( ai().game_graph()
                 .vertex( ef_storage().alife().member_item()->m_tGraphID )
                 ->vertex_type()[ 0 ] );
}
...
float CPersonalEyeRange::ffGetValue() {
    const CSE_ALifeMonsterAbstract* l_tpALifeMonsterAbstract =
        smart_cast< const CSE_ALifeMonsterAbstract* >(
            ef_storage().alife().member() );
    VERIFY3( l_tpALifeMonsterAbstract,
             "Invalid object passed to the evaluation function ", m_caName );
...
float CPersonalMaxHealth::ffGetValue() {
    ...
    const CSE_ALifeGroupAbstract* l_tpALifeGroupAbstract =
        smart_cast< const CSE_ALifeGroupAbstract* >(
            ef_storage().alife().member() );
...
float CItemDeterioration::ffGetValue() {
    if ( ef_storage().non_alife().member_item() ) {
        const CWeapon* weapon = smart_cast< const CWeapon* >(
            ef_storage().non_alife().member_item() );
        if ( weapon )
            return ( 1.f - weapon->GetCondition() );

#pragma todo( \
    "Dima to Dima : Append ItemDeterioration with non-ALife non-weapon branch" )
        return ( 0.f );
    } else {
        const CSE_ALifeInventoryItem* l_tpALifeInventoryItem =
            smart_cast< const CSE_ALifeInventoryItem* >(
                ef_storage().alife().member_item() );
        R_ASSERT2( l_tpALifeInventoryItem,
                   "Non-item object specified for the ItemDeterioration "
                   "evaluation function" );
...
float CEquipmentPreference::ffGetValue() {
    if ( ef_storage().non_alife().member() ) {
#pragma todo( \
    "Dima to Dima : Append EquipmentPreference with non-ALife branch" )
        return ( 0 );
    } else {
        CSE_ALifeHumanAbstract* l_tpALifeHumanAbstract =
        ...
        R_ASSERT2(
            ...
            "Non-human object in EquipmentPreference evaluation function" );
        return (
            l_tpALifeHumanAbstract->brain().m_cpEquipmentPreferences
                [ ef_storage().m_pfEquipmentType->dwfGetDiscreteValue() ] );
    }
}
...
float CMainWeaponPreference::ffGetValue() {
    if ( ef_storage().non_alife().member() ) {
#pragma todo( \
    "Dima to Dima : Append MainWeaponPreference with non-ALife branch" )
        return ( 0 );
    } else {
        CSE_ALifeHumanAbstract* l_tpALifeHumanAbstract =
            smart_cast< CSE_ALifeHumanAbstract* >(
        ...
        R_ASSERT2(
            ...
            "Non-human object in EquipmentPreference evaluation function" );
        return (
            l_tpALifeHumanAbstract->brain().m_cpMainWeaponPreferences
                [ ef_storage().m_pfMainWeaponType->dwfGetDiscreteValue( iFloor(
                    ef_storage().m_pfMainWeaponType->ffGetMaxResultValue() +
                    .5f ) ) ] );
    }
}
#else
...
#pragma todo( \
    "Dima to Dima : Append EquipmentPreference with non-ALife branch" )
        ...
        R_ASSERT2(
            ...
            "Non-human object in EquipmentPreference evaluation function" );
        return (
            l_tpALifeHumanAbstract->m_cpEquipmentPreferences
                [ ef_storage().m_pfEquipmentType->dwfGetDiscreteValue() ] );
    }
}
...
#pragma todo( \
    "Dima to Dima : Append MainWeaponPreference with non-ALife branch" )
        ...
        R_ASSERT2(
            ...
            "Non-human object in EquipmentPreference evaluation function" );
        return (
            l_tpALifeHumanAbstract->m_cpMainWeaponPreferences
                [ ef_storage().m_pfMainWeaponType->dwfGetDiscreteValue( iFloor(
                    ef_storage().m_pfMainWeaponType->ffGetMaxResultValue() +
                    .5f ) ) ] );
    }
...
#pragma todo( "Dima to Dima : Append WeaponAmmoCount with non-ALife branch" )
        ...
        R_ASSERT2( l_tpALifeHumanAbstract,
                   "Non-human object in WeaponAmmoCount evaluation function" );
        return (
            l_tpALifeHumanAbstract->brain().objects().get_available_ammo_count(
                smart_cast< const CSE_ALifeItemWeapon* >(
                    ef_storage().alife().member_item() ),
    ...
    if ( fTemp <= m_fMinResultValue )
        ...
        if ( l_tpALifeItemWeapon && l_tpALifeItemWeapon->m_caAmmoSections ) {
            ...
            if ( fTemp <= 3 * l_dwBoxSize )
                return (
                    iFloor( 1 * float( dwDiscretizationValue ) / 10 + .5f ) );
            return ( iFloor( 2 * float( dwDiscretizationValue ) / 10 + .5f ) );
        } else
            return ( dwDiscretizationValue - 1 );
    }
...
float CEnemyDistanceToGraphPoint::ffGetValue() {
    CSE_ALifeDynamicObject* l_tpALifeDynamicObject =
        smart_cast< CSE_ALifeDynamicObject* >( ef_storage().alife().enemy() );
    R_ASSERT3( l_tpALifeDynamicObject,
               "Invalid object passed to the evaluation function ", m_caName );
