...
void game_sv_mp::Update() {
    ...
    for ( u32 i = 0; i < m_CorpseList.size(); ) {
        ...
        if ( !pCorpseObj->children.empty() ) {
            Msg( "corpse [%d] childern not empty [%d]", CorpseID,
                 Device.dwFrame );
            i++;
            continue;
        }
    ...
    if ( g_sv_mp_iDumpStatsPeriod ) {
        ...
        if ( g_sv_mp_iDumpStats_last + g_sv_mp_iDumpStatsPeriod <=
             curr_minutes ) {
...
void game_sv_mp::OnRoundStart() {
    ...
    struct ready_clearer {
        void operator()( IClient* client ) {
            ...
            if ( !tmp_ps )
                return;
            tmp_ps->resetFlag( GAME_PLAYER_FLAG_READY +
...
void game_sv_mp::OnRoundEnd() {
    ...
    string64 res_str;
    xr_strcpy( res_str,
               get_token_name( round_end_result_str, round_end_reason ) );
...
struct real_sender {
    ...
    real_sender( xrServer* server,
                 NET_Packet* Packet,
                 u32 flags = DPNSEND_GUARANTEED ) {
...
void game_sv_mp::KillPlayer( ClientID id_who, u16 GameID ) {
    ...
    if ( xrCData && xrCData->ps &&
         xrCData->ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ) {
#ifdef DEBUG
...
void game_sv_mp::OnEvent( NET_Packet& P, u16 type, u32 time, ClientID sender ) {
    switch ( type ) {
        case GAME_EVENT_PLAYER_KILLED: // playerKillPlayer
        {
            ...
            if ( l_pC && l_pC->ps )
                Msg( "--- GAME_EVENT_PLAYER_KILLED: sender [%d][0x%08x]",
                     l_pC->ps->GameID, sender );
#endif // #ifdef DEBUG
            OnPlayerKilled( P );
        } break;
        case GAME_EVENT_PLAYER_HITTED: {
            ...
            xrClientData* l_pC = m_server->ID_to_client( sender );
            if ( l_pC && l_pC->ps )
                Msg( "--- GAME_EVENT_PLAYER_HITTED: sender [%d][0x%08x]",
                     l_pC->ps->GameID, sender );
#endif // #ifdef DEBUG
            OnPlayerHitted( P );
        } break;
        ...
        case GAME_EVENT_PLAYER_BUYMENU_OPEN: {
            xrClientData const* pClient =
                ( xrClientData const* )m_server->ID_to_client( sender );
            if ( pClient ) {
                OnPlayerOpenBuyMenu( pClient );
            } else {
                VERIFY2( pClient,
                         make_string( "unknown client [0x%08x] sended "
                                      "GAME_EVENT_PLAYER_BUYMENU_OPEN message",
                                      sender.value() )
                             .c_str() );
#ifndef MASTER_GOLD
...
bool game_sv_mp::CheckPlayerMapName( ClientID const& clientID, NET_Packet& P ) {
    ...
    if ( xr_strcmp( Level().name().c_str(), temp_map_name ) ) {
        Msg( "! Player 0x%08x has incorrect map name", clientID,
             temp_map_name );
...
void game_sv_mp::SpectatorModes_UnPack( u8 SpectrModesPacked ) {
    g_sv_mp_bSpectator_FreeFly =
        ( SpectrModesPacked & ( 1 << CSpectator::eacFreeFly ) ) != 0;
    g_sv_mp_bSpectator_FirstEye =
        ( SpectrModesPacked & ( 1 << CSpectator::eacFirstEye ) ) != 0;
    g_sv_mp_bSpectator_LookAt =
        ( SpectrModesPacked & ( 1 << CSpectator::eacLookAt ) ) != 0;
    g_sv_mp_bSpectator_FreeLook =
        ( SpectrModesPacked & ( 1 << CSpectator::eacFreeLook ) ) != 0;
    g_sv_mp_bSpectator_TeamCamera =
        ( SpectrModesPacked & ( 1 << CSpectator::eacMaxCam ) ) != 0;
};
...
void game_sv_mp::SpawnPlayer( ClientID id, LPCSTR N ) {
    ...
    Msg( "* %s [%d] respawned as %s", get_name_id( id ), E->ID,
         ( 0 == pA ) ? "spectator" : "actor" );
    spawn_end( E, id );
...
bool game_sv_mp::GetPosAngleFromActor( ClientID id,
                                       Fvector& Pos,
                                       Fvector& Angle ) {
    ...
    Angle.set( -pActor->cam_Active()->pitch, -pActor->cam_Active()->yaw,
               -pActor->cam_Active()->roll );
...
/*void	game_sv_mp::SpawnWeaponForActor		(u16 actorId,  LPCSTR N, bool
isScope, bool isGrenadeLauncher, bool isSilencer)
{
                u8 addon_flags = 0;
                if(isScope)
    ...
    for ( int i = 0; i < ammoc_count; ++i ) {
        ...
        while ( temp_iter != playerItems.end() ) {
            ...
            if ( ( ammo_magsize - weapon->a_elapsed ) <= box_size ) {
                ...
                ammo_diff.second =
                    box_size - ( ammo_magsize - weapon->a_elapsed );
...
void game_sv_mp::SetAmmoForWeapon(
    ...
    shared_str ammo_classes =
        pSettings->r_string( weapon->s_name, "ammo_class" );
    R_ASSERT2( ammo_classes.size() < 512,
               make_string( "ammo_class parameter of [%s] is too large",
                            weapon->s_name.c_str() )
                   .c_str() );
    VERIFY2( ammo_classes.size(),
             make_string( "ammo_class parameter of [%s] not found",
                          weapon->s_name.c_str() )
                 .c_str() );

    if ( !ammo_classes.size() ) {
#ifdef DEBUG
        Msg( "! WARNING: not found ammo_class for [%s]",
             weapon->s_name.c_str() );
#endif
        weapon->a_elapsed = 0;
    } else {
    ...
    if ( ( Addons & CSE_ALifeItemWeapon::eWeaponAddonGrenadeLauncher ) ||
        ...
        shared_str grenade_classes =
            pSettings->r_string( weapon->s_name, "grenade_class" );
        R_ASSERT2( grenade_classes.size() < 512,
                   make_string( "grenade_class parameter of [%s] is too large",
                                weapon->s_name.c_str() )
                       .c_str() );
        if ( !grenade_classes.size() ) {
#ifdef DEBUG
            Msg( "! WARNING: not found grenade_class for [%s]",
                 weapon->s_name.c_str() );
#endif
            weapon->a_elapsed_grenades.unpack_from_byte( 0 );
            return;
...
void game_sv_mp::SpawnAmmoDifference( u16 actorId,
                                      ammo_diff_t const& ammo_diff ) {
    ...
    CSE_ALifeItemAmmo* temp_ammo =
        smart_cast< CSE_ALifeItemAmmo* >( ammo_entity );
...
void game_sv_mp::SpawnWeapon4Actor(
    u16 actorId,
    LPCSTR N,
    u8 Addons,
    game_PlayerState::PLAYER_ITEMS_LIST& playerItems ) {
...
void game_sv_mp::OnDestroyObject( u16 eid_who ) {
    CORPSE_LIST_it it =
        std::find( m_CorpseList.begin(), m_CorpseList.end(), eid_who );
...
bool game_sv_mp::OnNextMap() {
    ...
    string1024 Command;
    xr_sprintf( Command, "sv_changelevel %s %s", R.map_name.c_str(),
                R.map_ver.c_str() );
...
s32 game_sv_mp::ExcludeBanTimeFromVoteStr( char const* vote_string,
                                           char* new_vote_str,
                                           u32 new_vote_str_size ) {
    if ( !vote_string || !xr_strlen( vote_string ) )
        return 0;

    s32 ret_time = 0;
    strncpy_s( new_vote_str, new_vote_str_size, vote_string,
               new_vote_str_size - 1 );
...
void game_sv_mp::OnVoteStart( LPCSTR VoteCommand, ClientID sender ) {
    ...
    if ( ( tmp_command_len < 256 ) &&
        ...
        strncpy_s( CommandParams, VoteCommand + xr_strlen( CommandName ) + 1,
                   255 );
        CommandParams[ 255 ] = 0;
    }
    ...
    if ( m_bVotingReal ) {
        if ( !stricmp( votecommands[ i ].name, "changeweather" ) ) {
            ...
            m_pVoteCommand.printf( "%s %s", votecommands[ i ].command,
                                   WeatherTime );
            xr_sprintf( resVoteCommand, "%s %s", votecommands[ i ].name,
                        WeatherName );
        } else if ( !stricmp( votecommands[ i ].name, "changemap" ) ) {
            string256 LevelName;
            string256 LevelVersion;
            sscanf_s( CommandParams, "%255s %255s", LevelName,
                      sizeof( LevelName ), LevelVersion,
                      sizeof( LevelVersion ) );
#ifdef DEBUG
            Msg( "--- Starting vote for changing level to: %s[%s]", LevelName,
                 LevelVersion );
#endif // #ifdef DEBUG
            LevelName[ 255 ] = 0;
            LevelVersion[ 255 ] = 0;
            ...
            STRCONCAT( sv_vote_command, votecommands[ i ].command, " ",
                       LevelName, " ", LevelVersion );
            m_pVoteCommand = sv_vote_command;
            xr_sprintf( resVoteCommand, "%s %s [%s]", votecommands[ i ].name,
                        LevelName, LevelVersion );
            ...
            if ( tmp_client ) {
                m_pVoteCommand.printf( "sv_kick_id %u",
                                       tmp_client->ID.value() );
            } else {
                m_pVoteCommand.printf(
                    "%s %s", votecommands[ i ].command,
                    CommandParams ); // backward compatibility
            }
            xr_strcpy( resVoteCommand, VoteCommand );
        } else if ( !stricmp( votecommands[ i ].name, "ban" ) ) {
            string256 tmp_victim_name;
            s32 ban_time = ExcludeBanTimeFromVoteStr(
                CommandParams, tmp_victim_name, sizeof( tmp_victim_name ) );
            // if (ban_time)
            //{
            SearcherClientByName tmp_predicate( tmp_victim_name );
            IClient* tmp_client = m_server->FindClient( tmp_predicate );
            if ( tmp_client ) {
                m_pVoteCommand.printf( "sv_banplayer %u %d",
                                       tmp_client->ID.value(), ban_time );
            } else {
                Msg( "! ERROR: can't find player with name %s",
                     tmp_victim_name );
            }
            //} else
            //{
    ...
    m_started_player = tmp_functor.pStartedPlayer
                           ? tmp_functor.pStartedPlayer->ps->getName()
                           : "";
...
void game_sv_mp::UpdateVote() {
    ...
    if ( m_uVoteStartTime + u32( g_sv_mp_fVoteTime * 60000 ) > CurTime ) {
        if ( vote_stats.NumAgreed >
             ( NumAgainst + vote_stats.NumParticipated ) ) {
            VoteSucceed = true;
        } else {
            return;
        }
        ...
        if ( g_sv_mp_bCountParticipants )
            VoteSucceed =
                ( float( vote_stats.NumAgreed ) /
                  float( vote_stats.NumParticipated + NumAgainst ) ) >=
                g_sv_mp_fVoteQuota;
        else
            VoteSucceed =
                ( float( vote_stats.NumAgreed ) /
                  float( vote_stats.NumToCount ) ) >= g_sv_mp_fVoteQuota;
    };
...
void game_sv_mp::SetPlayersDefItems( game_PlayerState* ps ) {
    ...
    for ( int i = 1; i <= ps->rank; i++ ) {
        ...
        for ( u32 it = 0; it < ps->pItemList.size(); it++ ) {
            ...
            //			if (!GetTeamItem_ByID(&pWpnS,
            //&(TeamList[ps->team].aWeapons), *pItemID)) continue;
            ...
            xr_strcpy( NewItemStr, sizeof( NewItemStr ),
                       pSettings->r_string( RankStr, ItemStr ) );
            //			if (!GetTeamItem_ByName(&pWpnS,
            //&(TeamList[ps->team].aWeapons), NewItemStr)) continue;
            if ( m_strWeaponsData->GetItemIdx( NewItemStr ) == u32( -1 ) )
                continue;

            //			*pItemID = pWpnS->SlotItem_ID;
            *pItemID =
                u16( m_strWeaponsData->GetItemIdx( NewItemStr ) & 0xffff );
        }
    }
    //---------------------------------------------------
    for ( u32 it = 0; it < ps->pItemList.size(); it++ ) {
        ...
        if ( pSettings->line_exist( WeaponName, "ammo_class" ) ) {
            ...
            xr_strcpy( wpnAmmos,
                       pSettings->r_string( WeaponName, "ammo_class" ) );
            _GetItem( wpnAmmos, 0, BaseAmmoName );
            AmmoID =
                u16( m_strWeaponsData->GetItemIdx( BaseAmmoName ) & 0xffff );
        };
        //		if (!pWpnS->WeaponBaseAmmo.size()) continue;
        //		WeaponDataStruct* pWpnAmmo = NULL;
        //		if (!GetTeamItem_ByName(&pWpnAmmo,
        //&(TeamList[ps->team].aWeapons), *(pWpnS->WeaponBaseAmmo))) continue;
        if ( AmmoID == u16( -1 ) )
            continue;
...
void game_sv_mp::OnPlayerKilled( NET_Packet P ) {
    ...
    if ( !ps_killed ) {
        CEntity* entity =
            smart_cast< CEntity* >( Level().Objects.net_Find( KilledID ) );

#ifndef MASTER_GOLD
        Msg( "! ERROR:  killed entity is null ! (entitty [%d][%s]), killer id "
             "[%d][%s], Frame [%d]",
             KilledID, entity ? entity->cName().c_str() : "unknown", KillerID,
             ps_killer ? ps_killer->getName() : "unknown", Device.dwFrame );
#endif // #ifndef MASTER_GOLD
        return;
    }
#ifdef MP_LOGGING
    Msg( "--- Player [%d] killed player [%d], Frame [%d]", KillerID, KilledID,
         Device.dwFrame );
#endif
    ...
    if ( KillType == KT_BLEEDING ) {
        Game().m_WeaponUsageStatistic->OnBleedKill( ps_killer, ps_killed,
                                                    WeaponID );
    }
    //---------------------------------------------------
    SendPlayerKilledMessage(
        ( ps_killed ) ? ps_killed->GameID : KilledID, KillType,
...
void game_sv_mp::OnPlayerHitted( NET_Packet P ) {
    ...
    if ( !CheckTeams() || ( PSHitted->team != PSHitter->team ) ) {
        ...
        Player_AddExperience(
            PSHitter,
            dHealth * pCurRank->m_aRankDiff_ExpBonus[ PSHitted->rank ] );
    };
};

void game_sv_mp::SendPlayerKilledMessage( u16 KilledID,
                                          KILL_TYPE KillType,
                                          u16 KillerID,
                                          u16 WeaponID,
...
void game_sv_mp::OnPlayerSelectSpectator( NET_Packet& P, ClientID sender ) {
    ...
    if ( pClient->owner ) {
        CSE_ALifeCreatureActor* pA =
            smart_cast< CSE_ALifeCreatureActor* >( pClient->owner );
        if ( pA ) {
...
void game_sv_mp::LoadRanks() {
    ...
    for ( int i = 0;; i++ ) {
        ...
        NewRank.m_iBonusMoney = READ_IF_EXISTS( pSettings, r_s32, RankSect,
                                                "rank_aquire_money", 0 );
        shared_str RDEB_str =
            pSettings->r_string( RankSect, "rank_diff_exp_bonus" );
        ...
        R_ASSERT2( ( TermsCount != 0 && TermsCount <= MAX_TERMS ),
                   "Error Number of Terms for Rank" );
...
/*
bool	game_sv_mp::GetTeamItem_ByID		(WeaponDataStruct** pRes,
TEAM_WPN_LIST* pWpnList, u16 ItemID)
{
        if (!pWpnList) return false;
        TEAM_WPN_LIST_it pWpnI	= std::find(pWpnList->begin(), pWpnList->end(),
(ItemID)); if (pWpnI == pWpnList->end() || !((*pWpnI) == (ItemID))) return
false; *pRes = &(*pWpnI); return true;
};

bool	game_sv_mp::GetTeamItem_ByName		(WeaponDataStruct**
pRes,TEAM_WPN_LIST* pWpnList, LPCSTR ItemName)
{
        if (!pWpnList) return false;
        TEAM_WPN_LIST_it pWpnI	= std::find(pWpnList->begin(), pWpnList->end(),
ItemName); if (pWpnI == pWpnList->end() || !((*pWpnI) == ItemName)) return
false; *pRes = &(*pWpnI); return true;
};
*/
void game_sv_mp::Player_AddBonusMoney( game_PlayerState* ps,
                                       s32 MoneyAmount,
                                       SPECIAL_KILL_TYPE Reason,
                                       u8 Kill ) {
    if ( !ps )
        return;
    //-----------------------------
    if ( MoneyAmount )
        ps->m_aBonusMoney.push_back(
            Bonus_Money_Struct( MoneyAmount, u8( Reason & 0xff ), Kill ) );
    //-----------------------------
...
void game_sv_mp::ReadOptions( shared_str& options ) {
    ...
    SpectatorModes = u8(
        get_option_i( *options, "spectrmds", s32( SpectatorModes ) ) & 0x00ff );
    SpectatorModes_UnPack( SpectatorModes );

    g_sv_dwMaxClientPing =
        get_option_i( *options, "maxping", g_sv_dwMaxClientPing );
    ...
    float EnvTimeFactor =
        float( atof( TimeFactor ) ) * GetEnvironmentGameTimeFactor();
...
void game_sv_mp::RenewAllActorsHealth() {
    struct actor_health_renewer {
        void operator()( IClient* client ) {
            xrClientData* l_pC = static_cast< xrClientData* >( client );
            VERIFY2( l_pC->ps,
                     make_string( "player state of client, ClientID = 0x%08x",
                                  l_pC->ID.value() )
                         .c_str() );
            if ( !l_pC || !l_pC->ps ) {
                return;
            ...
            CActor* pActor = smart_cast< CActor* >(
                Level().Objects.net_Find( l_pC->ps->GameID ) );
            ...
            VERIFY2( pActor, make_string( "client object on server of actor "
                                          "GameID = 0x%08x, not found",
                                          l_pC->ps->GameID )
                                 .c_str() );
            if ( pActor ) {
...
void game_sv_mp::RejectGameItem( CSE_Abstract* entity ) {
    ...
    if ( smart_cast< CSE_ALifeItemGrenade* >( entity ) ) {
        CGrenade* grenade =
            smart_cast< CGrenade* >( Level().Objects.net_Find( entity->ID ) );
    ...
    VERIFY2( e_parent, make_string( "RejectGameItem: parent not found. "
                                    ...
                                    entity->ID, entity->ID_Parent )
                           .c_str() );
    if ( !e_parent ) {
        Msg( "! ERROR (RejectGameItem): parent not found. entity_id = [%d], "
             "parent_id = [%d]",
             entity->ID, entity->ID_Parent );
        return;
    }
...
void game_sv_mp::DumpOnlineStatistic() {
    ...
    ini.w_u32( current_section.c_str(), "players_total_cnt",
               m_server->GetClientsCount() );

    xr_sprintf( str_buff, "\"%s\"",
                CStringTable().translate( Level().name().c_str() ).c_str() );
    ini.w_string( current_section.c_str(), "current_map_name", str_buff );

    xr_sprintf( str_buff, "%s",
                CStringTable().translate( type_name() ).c_str() );
    ...
    for ( u32 idx = 0; it != it_e; ++it, ++idx ) {
        ...
        xr_sprintf(
            str_buff, "\"%s\"",
            CStringTable().translate( ( *it ).map_name.c_str() ).c_str() );
...
void game_sv_mp::WritePlayerStats( CInifile& ini,
                                   LPCSTR sect,
                                   xrClientData* pCl ) {
    ...
    ini.w_u32( sect, "online_time_sec",
               ( Level().timeServer() - pCl->ps->m_online_time ) / 1000 );

    if ( Game().m_WeaponUsageStatistic->CollectData() ) {
        Player_Statistic& plstats = *(
            Game().m_WeaponUsageStatistic->FindPlayer( pCl->ps->getName() ) );
...
void game_sv_mp::WriteGameState( CInifile& ini,
                                 LPCSTR sect,
                                 bool bRoundResult ) {
    if ( !bRoundResult )
...
void game_sv_mp::async_statistics_collector::set_responded(
    ClientID clientID ) {
...
bool game_sv_mp::CheckStatisticsReady() {
    ...
    if ( m_async_stats.all_ready() ||
         ( m_async_stats_request_time + g_sv_dwMaxClientPing ) <
             Device.dwTimeGlobal ) {
        DumpRoundStatistics();
...
void game_sv_mp::DumpRoundStatistics() {
    ...
    ini.w_string( current_section.c_str(), "start_time",
                  m_round_start_time_str );
    ...
    ini.w_string( current_section.c_str(), "current_map_name_internal",
                  str_buff );
...
void game_sv_mp::DestroyAllPlayerItems( ClientID id_who ) // except rukzak
{
    ...
    VERIFY2( xrCData, make_string( "client (ClientID = 0x%08x) not found",
                                   id_who.value() )
                          .c_str() );
    VERIFY( xrCData->ps );
    game_PlayerState* ps = xrCData->ps;
#ifndef MASTER_GOLD
    Msg( "---Destroying player [%s] items before spawning new bought items.",
         ps->getName() );
#endif // #ifndef MASTER_GOLD

    CActor* pActor =
        smart_cast< CActor* >( Level().Objects.net_Find( ps->GameID ) );
    if ( !pActor )
        return;

    TIItemContainer::const_iterator iie = pActor->inventory().m_all.end();
    for ( TIItemContainer::const_iterator ii =
              pActor->inventory().m_all.begin();
          ii != iie; ++ii ) {
        //		VERIFY(*ii);
        R_ASSERT2( *ii, make_string( "PIItem in player`s inventory not found. "
                                     "Destroy all items of actor[%d]",
                                     ps->GameID )
                            .c_str() );
        ...
        //		R_ASSERT2( tempEntity, make_string("entity not found
        //[%d]. Destroy all items of actor[%d]", object_id, ps->GameID).c_str()
        //);
        VERIFY2( tempEntity,
                 make_string(
                     "entity not found [%d]. Destroy all items of actor[%d]",
                     object_id, ps->GameID )
                     .c_str() );
        if ( !tempEntity ) {
            Msg( "! ERROR: entity not found [%d]. Destroy all items of "
                 "actor[%d]",
                 object_id, ps->GameID );
            continue;
        }
...
bool game_sv_mp::IsPlayerBanned( char const* hexstr_digest,
                                 shared_str& by_who ) {
...
IClient* game_sv_mp::BanPlayer( ClientID const& client_id,
                                s32 ban_time_sec,
                                xrClientData* initiator ) {
    if ( client_id == m_server->GetServerClient()->ID ) {
        Msg( "! ERROR: can't ban server client." );
        return NULL;
    }
    xrClientData* client_to_ban =
        static_cast< xrClientData* >( m_server->ID_to_client( client_id ) );
...
void game_sv_mp::BanPlayerDirectly( char const* client_hex_digest,
                                    s32 ban_time_sec,
                                    xrClientData* initiator ) {
    m_cdkey_ban_list.ban_player_ll( client_hex_digest, ban_time_sec,
                                    initiator );
}
...
void game_sv_mp::OnPlayerChangeName( NET_Packet& P, ClientID sender ) {
    ...
    if ( sv && sv->IsPublicServer() ) {
        Msg( "Player \"%s\" try to change name on \"%s\" at public server.",
             ps->getName(), NewName );
    ...
    Game().m_WeaponUsageStatistic->ChangePlayerName( old_name.c_str(),
                                                     ps->getName() );
