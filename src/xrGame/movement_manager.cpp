...
void CMovementManager::Load( LPCSTR section ) {
    ...
    m_base_game_selector =
        xr_new< CGameVertexParams >( locations().vertex_types() );
    m_base_level_selector = xr_new< CBaseParameters >();

    m_game_location_selector = xr_new< CGameLocationSelector >(
        m_restricted_object, m_location_manager );
    ...
    m_patrol_path_manager =
        xr_new< CPatrolPathManager >( m_restricted_object, m_object );
...
void CMovementManager::set_game_dest_vertex(
...
void CMovementManager::set_level_dest_vertex( u32 const& level_vertex_id ) {
    if ( !ai().level_graph().valid_vertex_id( level_vertex_id ) ) {
        Msg( "! [CMovementManager::set_level_dest_vertex] ignoring invalid "
             "level vertex id [%u] for object [%s]",
...
const xr_vector< DetailPathManager::STravelPathPoint >& CMovementManager::path()
    const {
    return ( detail().path() );
}
                ...
                if ( ai().level_graph().valid_vertex_id(
                    ...
                    if ( !restrictions().accessible(
                        ...
                        level_path().set_dest_vertex(
                            restrictions().accessible_nearest(
...
bool CMovementManager::actual_all() const {
    ...
    switch ( m_path_type ) {
        case ePathTypeGamePath:
            return ( game_path().actual() && level_path().actual() &&
                     detail().actual() );
        case ePathTypeLevelPath:
            return ( level_path().actual() && detail().actual() );
        case ePathTypePatrolPath:
            return ( patrol().actual() && level_path().actual() &&
                     detail().actual() );
        case ePathTypeNoPath:
...
void CMovementManager::teleport( u32 game_vertex_id ) {
    ...
    GameGraph::_GRAPH_ID _game_vertex_id =
        ( GameGraph::_GRAPH_ID )game_vertex_id;
    u32 _level_vertex_id =
        ai().game_graph().vertex( _game_vertex_id )->level_vertex_id();
    Fvector position =
        ai().game_graph().vertex( _game_vertex_id )->level_point();
...
bool CMovementManager::distance_to_destination_greater(
    const float& distance_to_check ) const {
    if ( path().size() < 2 )
        return ( true );
    ...
    for ( u32 i = detail().curr_travel_point_index(),
              n = detail().path().size() - 1;
          i < n; ++i ) {
        accumulator += detail().path()[ i ].position.distance_to(
            detail().path()[ i + 1 ].position );
...
void CMovementManager::verify_detail_path() {
    if ( detail().path().empty() || !detail().actual() ||
         detail().completed( detail().dest_position() ) )
        return;
    ...
    float distance = 0.f;
    for ( u32 i = detail().curr_travel_point_index() + 1,
              n = detail().path().size();
          i < n; ++i ) {
        ...
        distance += detail().path()[ i ].position.distance_to(
            detail().path()[ i - 1 ].position );
...
void CMovementManager::on_restrictions_change() {
    //	Msg
    //("[%6d][%s][on_restrictions_change]",Device.dwTimeGlobal,*object().cName());
    m_path_actuality = false;
...
bool CMovementManager::can_use_distributed_computations( u32 option ) const {
    ...
    // Fix for bug which makes mutants running in one place. Something happens
    // with the thread, and the mutants will wait for a thread which will never
    // finish running. Many thanks to Arszi for finding this <3 demonized: add
    // cvar switch for toggling it
    return ( !monsterStuckFix && !m_build_at_once &&
             g_mt_config.test( option ) && !object().getDestroy() );
}

void CMovementManager::on_frame( CPHMovementControl* movement_control,
                                 Fvector& dest_position ) {
    if ( enabled() && ( m_path_state != ePathStatePathVerification ) &&
         ( m_path_state != ePathStatePathCompleted ) )
        update_path();

    m_allow_rebuild_smoothing =
        !actual() && ( m_restriction_rebuild_smooth_frames > 0 ) &&
        !detail().path().empty() &&
    ...
    move_along_path( movement_control, dest_position,
                     object().client_update_fdelta() );

    // Update Grass benders
    g_pGamePersistent->GrassBendersUpdate(
        object().ID(), grassbender_id, grassbender_frame, object().Position(),
        -1.0f, 1.0f, true );
}

void CMovementManager::on_travel_point_change(
...
void CMovementManager::enable_movement( bool enabled ) {
    ...
    if ( !enabled && m_enabled )
        ...
        if ( enabled && !m_enabled &&
             !object().Position().similar( m_on_disable_object_position ) )
            m_path_actuality = false;
    }
...
Fvector CMovementManager::predict_position( const float& time_delta ) const {
    ...
    return ( predict_position( time_delta, object().Position(), travel_point,
                               prediction_speed() ) );
}
