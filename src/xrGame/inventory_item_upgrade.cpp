...
bool CInventoryItem::has_upgrade_group( const shared_str& upgrade_group_id ) {
    ...
    for ( ; it != it_e; ++it ) {
        inventory::upgrade::Upgrade* upgrade =
            ai().alife().inventory_upgrade_manager().get_upgrade( *it );
...
bool CInventoryItem::equal_upgrades(
    Upgrades_type const& other_upgrades ) const {
...
void CInventoryItem::net_Spawn_install_upgrades(
    Upgrades_type saved_upgrades ) // net_Spawn
{
    ...
    ai().alife().inventory_upgrade_manager().init_install(
        *this ); // from pSettings

    Upgrades_type::iterator ib = saved_upgrades.begin();
    Upgrades_type::iterator ie = saved_upgrades.end();
    for ( ; ib != ie; ++ib ) {
        ai().alife().inventory_upgrade_manager().upgrade_install(
            *this, ( *ib ), true );
    }
}
...
bool CInventoryItem::install_upgrade_impl( LPCSTR section, bool test ) {
    bool result =
        process_if_exists( section, "cost", &CInifile::r_u32, m_cost, test );
    result |= process_if_exists( section, "inv_weight", &CInifile::r_float,
                                 m_weight, test );

    bool result2 = false;
    if ( BaseSlot() != NO_ACTIVE_SLOT ) {
        BOOL value = m_flags.test( FRuckDefault );
        result2 = process_if_exists_set( section, "default_to_ruck",
        ...
        result2 = process_if_exists_set( section, "sprint_allowed",
                                         &CInifile::r_bool, value, test );
        if ( result2 && !test ) {
        ...
        result |= process_if_exists( section, "control_inertion_factor",
                                     &CInifile::r_float,
                                     m_fControlInertionFactor, test );
    }

    LPCSTR str;
    result2 = process_if_exists_set( section, "immunities_sect",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test )
        CHitImmunity::LoadImmunities( str, pSettings );

    result2 = process_if_exists_set( section, "immunities_sect_add",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test )
...
void CInventoryItem::pre_install_upgrade() {
    ...
    if ( weapon ) {
        ...
        if ( weapon->GrenadeLauncherAttachable() &&
             weapon->IsGrenadeLauncherAttached() ) {
