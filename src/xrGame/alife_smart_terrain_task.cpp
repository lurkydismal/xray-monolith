...
void CALifeSmartTerrainTask::setup_patrol_point(
    const shared_str& patrol_path_name,
    const u32& patrol_point_index ) {
    VERIFY( !m_patrol_point );

    const CPatrolPath* patrol_path =
        ai().patrol_paths().path( patrol_path_name );
    VERIFY( patrol_path );
...
#ifdef DEBUG
        VERIFY3( ai().game_graph().valid_vertex_id(
                     patrol_point().game_vertex_id() ),
                 *m_patrol_path_name, *m_patrol_point->name() );
#endif
