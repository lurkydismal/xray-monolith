...
struct delete_predicate_by_time {
    bool operator()( Feel::Touch::DenyTouch const& left,
                     DWORD const expire_time ) const {
...
struct objects_ptrs_equal {
    bool operator()( Feel::Touch::DenyTouch const& left,
                     CObject const* const right ) const {
        if ( left.O == right )
            return true;
        return false;
    }
...
void GlobalFeelTouch::feel_touch_update( Fvector& P, float R ) {
    ...
    xr_vector< Feel::Touch::DenyTouch >::iterator new_end = std::remove_if(
        ...
        [ expire_time =
              Device.dwTimeGlobal ]( const Feel::Touch::DenyTouch& left ) {
