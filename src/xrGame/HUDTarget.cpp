...
CrosshairSettings g_crosshair_camera_far =
    CrosshairSettings( { CROSSHAIR_SHOW | CROSSHAIR_RECON },
                       "hud\\cursor",
                       "ui\\cursor_dot",
                       40.f,
                       1.f,
                       ...
                       40.f,
                       .5f );
CrosshairSettings g_crosshair_camera_near =
    CrosshairSettings( { CROSSHAIR_USE_SHADER },
                       "hud\\cursor",
                       "ui\\cursor_cross",
                       40.f,
                       16.f,
...
CrosshairSettings g_crosshair_weapon_far =
    CrosshairSettings( { CROSSHAIR_SHOW | CROSSHAIR_RECON },
                       "hud\\cursor",
                       "ui\\cursor_plus",
                       40.f,
                       ...
                       40.f,
                       .5f );
CrosshairSettings g_crosshair_weapon_near =
    CrosshairSettings( { CROSSHAIR_RECON | CROSSHAIR_USE_SHADER },
                       "hud\\cursor",
                       "ui\\cursor_cross",
                       40.f,
                       16.f,
...
CrosshairSettings g_crosshair_device_far =
    CrosshairSettings( { CROSSHAIR_RECON },
                       "hud\\cursor",
                       "ui\\cursor_plus",
                       40.f,
                       1.f,
                       ...
                       40.f,
                       .5f );
CrosshairSettings g_crosshair_device_near =
    CrosshairSettings( { CROSSHAIR_RECON | CROSSHAIR_USE_SHADER },
                       "hud\\cursor",
                       "ui\\cursor_cross",
                       40.f,
...
void TargetCrosshair::IntegratePosition( const SPickParam& pp,
                                         float dist,
                                         bool is_far ) {
...
void TargetCrosshair::IntegrateOpacity( const SPickParam& pp,
                                        float opacity_target ) {
    // Interpolate opacity offset toward target
    opacity = lerp( opacity, opacity_target,
                    Device.fTimeDelta * settings.occlusion_fade_rate );
}
...
void TargetCrosshair::Update( const SPickParam& pp, bool is_far ) {
    ...
    u32 color_crosshair =
        ( color_readout & color_rgba( 0xff, 0xff, 0xff, 0 ) ) ==
                ( C_WHITE & color_rgba( 0xff, 0xff, 0xff, 0 ) )
            ? settings.color
            : color_readout;

    // Modulate by global crosshair color
    color_crosshair = D3DCOLOR_RGBA(
        ( u8 )( ( ( color_get_R( color_crosshair ) / 255.f ) *
                  ( color_get_R( g_crosshair_color ) / 255.f ) ) *
                255.f ),
        ( u8 )( ( ( color_get_G( color_crosshair ) / 255.f ) *
                  ( color_get_G( g_crosshair_color ) / 255.f ) ) *
                255.f ),
        ( u8 )( ( ( color_get_B( color_crosshair ) / 255.f ) *
                  ( color_get_B( g_crosshair_color ) / 255.f ) ) *
                255.f ),
        ( u8 )( ( ( color_get_A( color_crosshair ) / 255.f ) *
                  ( color_get_A( g_crosshair_color ) / 255.f ) ) *
                255.f ) );

    // Modulate color alpha
    DWORD alpha_mask = 0xff000000;
    color_crosshair =
        subst_alpha( color_crosshair, u8( iFloor( 255.f * opacity ) ) );
    ...
    bool occluded = is_occluded(
        Fvector().add( pp.defs.start, Fvector().mul( pp.defs.dir, dist ) ) );
...
void CrosshairPair::Update( const SPickParam& pp ) {
    ...
    float t = remap( pos.w / zFar, crosshair_near.settings.depth / zFar,
                     crosshair_far.settings.depth / zFar, 0.f, 1.f );
    float near_size =
        pos.w *
        lerp( crosshair_near.settings.size, crosshair_far.settings.size, t ) *
        ( Device.fFOV / 90.f );
    ...
    float far_size =
        mat._44 * crosshair_far.settings.size * ( Device.fFOV / 90.f );
    crosshair_far.crosshair.SetScale( far_size );
}

void CrosshairPair::RenderAimLine( Fvector va, Fvector vb ) const {
    Fvector2 scr_size = { float( ::Render->getTarget()->get_width() ),
    ...
    UIRender->StartPrimitive( verts, IUIRender::ptLineStrip,
...
void CrosshairPair::Render( const SPickParam& pp ) {
    BOOL b_do_rendering = ( psHUD_Flags.is( HUD_CROSSHAIR | HUD_CROSSHAIR_RT |
                                            HUD_CROSSHAIR_RT2 ) );
    if ( !b_do_rendering )
        return;
    ...
    if ( crosshair_near.Is( CROSSHAIR_LINE ) ||
         crosshair_far.Is( CROSSHAIR_LINE ) )
        RenderAimLine(
            Fvector().sub( pp.barrel_matrix.c,
                           Fvector().mul( pp.barrel_matrix.k, .05f ) ),
            pp.barrel_matrix.c );
}

CHUDTarget::CHUDTarget()
    : m_camera(
          CrosshairPair( g_crosshair_camera_near, g_crosshair_camera_far ) ),
      m_weapon(
          CrosshairPair( g_crosshair_weapon_near, g_crosshair_weapon_far ) ),
      m_device(
          CrosshairPair( g_crosshair_device_near, g_crosshair_device_far ) ) {
...
void CHUDTarget::Render() {
    ...
    m_weapon.crosshair_near.recon.SetDoTransform( firepos_active ||
                                                  aimpos_active );
    m_weapon.crosshair_far.recon.SetDoTransform( firepos_active ||
                                                 aimpos_active );
    m_device.crosshair_near.recon.SetDoTransform( firepos_active ||
                                                  aimpos_active );
    m_device.crosshair_far.recon.SetDoTransform( firepos_active ||
                                                 aimpos_active );
