...
static bool sort_team_players_by_spots( demo_player_info const* left,
                                        demo_player_info const* right ) {
...
demo_info const* demo_info_loader::get_demofile_info( LPCSTR demo_file_name ) {
    ...
    demo_info_cache_t::const_iterator tmp_iter =
        m_demo_info_cache.find( tmp_fn );
    if ( tmp_iter == m_demo_info_cache.end() ) {
        demo_info* tmp_demoinfo = load_demofile( demo_file_name );
        R_ASSERT( tmp_demoinfo );
        return m_demo_info_cache
            .insert(
                std::make_pair( shared_str( demo_file_name ), tmp_demoinfo ) )
            .first->second;
    }
