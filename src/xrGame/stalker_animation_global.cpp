...
void CStalkerAnimationManager::global_play_callback( CBlend* blend ) {
    ...
    //	Msg							("[%6d] global
    //callback [%s][%s]", Device.dwTimeGlobal, pair_id.first, pair_id.second);
...
MotionID CStalkerAnimationManager::global_critical_hit() {
    ...
    VERIFY2(
        weapon,
        make_string( "current active item: %s",
    ...
    return ( global().select(
        m_data_storage->m_part_animations.A[ eBodyStateStand ]
            .m_global
            .A[ object().critical_wound_type() + 6 * ( animation_slot - 1 ) ]
            .A,
...
MotionID CStalkerAnimationManager::assign_global_animation(
    ...
    if ( eMovementTypeRun == object().movement().movement_type() &&
         b_state == eBodyStateStand )
        return ( MotionID() );

    return global().select(
        m_data_storage->m_part_animations.A[ b_state ].m_global.A[ 1 ].A );
}
