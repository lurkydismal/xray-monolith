...
CSE_ALifeDynamicObject* alife_object( const CALifeSimulator* self,
...
bool valid_object_id( const CALifeSimulator* self,
                      ALife::_OBJECT_ID object_id ) {
    VERIFY( self );
...
CSE_ALifeDynamicObject* alife_object( const CALifeSimulator* self,
    ...
    for ( CALifeObjectRegistry::OBJECT_REGISTRY::const_iterator it =
              self->objects().objects().begin();
...
CSE_ALifeDynamicObject* alife_story_object( const CALifeSimulator* self,
                                            ALife::_STORY_ID id ) {
...
void generate_story_ids( STORY_PAIRS& result,
    ...
    while ( Ini->r_line( section_name, k, &N, &V ) ) {
        ...
        R_ASSERT2( xr_strcmp( *temp, INVALID_ID_STRING ),
                   invalid_id_redefinition );
...
void kill_entity0( CALifeSimulator* alife,
                   CSE_ALifeMonsterAbstract* monster,
                   const GameGraph::_GRAPH_ID& game_vertex_id ) {
...
void add_in_restriction( CALifeSimulator* alife,
                         CSE_ALifeMonsterAbstract* monster,
                         ALife::_OBJECT_ID id ) {
    alife->add_restriction( monster->ID, id,
                            RestrictionSpace::eRestrictorTypeIn );
}

void add_out_restriction( CALifeSimulator* alife,
                          CSE_ALifeMonsterAbstract* monster,
                          ALife::_OBJECT_ID id ) {
    alife->add_restriction( monster->ID, id,
                            RestrictionSpace::eRestrictorTypeOut );
}

void remove_in_restriction( CALifeSimulator* alife,
                            CSE_ALifeMonsterAbstract* monster,
                            ALife::_OBJECT_ID id ) {
    alife->remove_restriction( monster->ID, id,
                               RestrictionSpace::eRestrictorTypeIn );
}

void remove_out_restriction( CALifeSimulator* alife,
                             CSE_ALifeMonsterAbstract* monster,
                             ALife::_OBJECT_ID id ) {
    alife->remove_restriction( monster->ID, id,
                               RestrictionSpace::eRestrictorTypeOut );
...
CSE_ALifeDynamicObject* CALifeSimulator__create( CALifeSimulator* self,
                                                 ALife::_SPAWN_ID spawn_id ) {
    const CALifeSpawnRegistry::SPAWN_GRAPH::CVertex* vertex =
        ai().alife().spawns().spawns().vertex( spawn_id );
    THROW2( vertex, "Invalid spawn id!" );

    CSE_ALifeDynamicObject* spawn =
        smart_cast< CSE_ALifeDynamicObject* >( &vertex->data()->object() );
    THROW( spawn );
...
CSE_Abstract* CALifeSimulator__spawn_item(
    CALifeSimulator* self,
    LPCSTR section,
    const Fvector& position,
    u32 level_vertex_id,
    GameGraph::_GRAPH_ID game_vertex_id ) {
    THROW( self );
    return ( self->spawn_item( section, position, level_vertex_id,
                               game_vertex_id, ALife::_OBJECT_ID( -1 ) ) );
}

CSE_Abstract* CALifeSimulator__spawn_item2( CALifeSimulator* self,
                                            LPCSTR section,
                                            const Fvector& position,
                                            u32 level_vertex_id,
                                            GameGraph::_GRAPH_ID game_vertex_id,
    ...
    if ( id_parent == ALife::_OBJECT_ID( -1 ) )
        return ( self->spawn_item( section, position, level_vertex_id,
                                   game_vertex_id, id_parent ) );

    CSE_ALifeDynamicObject* object =
        ai().alife().objects().object( id_parent, true );
    if ( !object ) {
    ...
    CSE_Abstract* item = self->spawn_item( section, position, level_vertex_id,
                                           game_vertex_id, id_parent, false );
...
// Alundaio: Allows to call alife():register(se_obj) manually afterward so that
// packet editing can be done safely when spawning object with a parent
CSE_Abstract* CALifeSimulator__spawn_item3( CALifeSimulator* self,
                                            LPCSTR section,
                                            const Fvector& position,
                                            u32 level_vertex_id,
                                            GameGraph::_GRAPH_ID game_vertex_id,
                                            ...
                                            bool reg = true ) {
    if ( reg == true )
        return CALifeSimulator__spawn_item2( self, section, position,
...
CSE_Abstract* CALifeSimulator__spawn_ammo( CALifeSimulator* self,
                                           LPCSTR section,
                                           const Fvector& position,
                                           u32 level_vertex_id,
                                           GameGraph::_GRAPH_ID game_vertex_id,
    ...
    //	if (id_parent == ALife::_OBJECT_ID(-1))
    //		return
    //(self->spawn_item(section,position,level_vertex_id,game_vertex_id,id_parent));
    ...
    if ( !object || !object->m_bOnline ) {
        CSE_Abstract* item = self->spawn_item(
            section, position, level_vertex_id, game_vertex_id, id_parent );
...
ALife::_SPAWN_ID CALifeSimulator__spawn_id(
    CALifeSimulator* self,
    ALife::_SPAWN_STORY_ID spawn_story_id ) {
    return ( ( ( const CALifeSimulator* )self )
                 ->spawns()
                 .spawn_id( spawn_story_id ) );
}

void CALifeSimulator__release( CALifeSimulator* self,
                               CSE_Abstract* object,
                               bool ) {
    VERIFY( self );
...
LPCSTR get_level_name( const CALifeSimulator* self, int level_id ) {
    ...
    GameGraph::LEVEL_MAP::const_iterator I =
        levels.find( ( GameGraph::_LEVEL_ID )level_id );
    if ( I == levels.end() ) {
        return NULL;
    }
    LPCSTR result = *ai().game_graph()
                         .header()
                         .level( ( GameGraph::_LEVEL_ID )level_id )
                         .name();
    return ( result );
}
...
KNOWN_INFO_VECTOR* registry( const CALifeSimulator* self,
                             const ALife::_OBJECT_ID& id ) {
    THROW( self );
    return (
        self->registry().get< CInfoPortionRegistry >().object( id, true ) );
}

bool has_info( const CALifeSimulator* self,
               const ALife::_OBJECT_ID& id,
               LPCSTR info_id ) {
    ...
    if ( std::find_if( known_info->begin(), known_info->end(),
                       CFindByIDPred( has_info ) ) == known_info->end() )
        return ( false );

    return ( true );
}

bool dont_has_info( const CALifeSimulator* self,
                    const ALife::_OBJECT_ID& id,
                    LPCSTR info_id ) {
    THROW( self );
...
void AlifeGiveInfo( const CALifeSimulator* alife,
                    const ALife::_OBJECT_ID& id,
                    LPCSTR info_id ) {
    KNOWN_INFO_VECTOR* known_info =
        alife->registry().get< CInfoPortionRegistry >().object( id, true );
    if ( !known_info )
        return;
    ...
    if ( std::find_if( known_info->begin(), known_info->end(),
                       CFindByIDPred( AlifeGiveInfo ) ) == known_info->end() ) {
...
void AlifeRemoveInfo( const CALifeSimulator* alife,
                      const ALife::_OBJECT_ID& id,
                      LPCSTR info_id ) {
    KNOWN_INFO_VECTOR* known_info =
        alife->registry().get< CInfoPortionRegistry >().object( id, true );
    ...
    known_info->erase( std::find_if( known_info->begin(), known_info->end(),
                                     CFindByIDPred( AlifeRemoveInfo ) ),
                       known_info->end() );
}

// Alundaio: teleport object
void teleport_object( CALifeSimulator* alife,
                      ALife::_OBJECT_ID id,
                      GameGraph::_GRAPH_ID game_vertex_id,
                      u32 level_vertex_id,
                      const Fvector& position ) {
...
void IterateInfo( const CALifeSimulator* alife,
                  const ALife::_OBJECT_ID& id,
...
CSE_Abstract* try_to_clone_object( CALifeSimulator* self,
                                   CSE_Abstract* object,
                                   LPCSTR section,
                                   const Fvector& position,
                                   u32 level_vertex_id,
                                   ...
                                   ALife::_OBJECT_ID id_parent,
                                   bool bRegister = true ) {
    CSE_ALifeItemWeaponMagazined* wpnmag =
        smart_cast< CSE_ALifeItemWeaponMagazined* >( object );
    if ( wpnmag ) {
        CSE_Abstract* absClone =
            self->spawn_item( section, position, level_vertex_id,
                              game_vertex_id, id_parent, false );
        if ( !absClone )
            return ( 0 );

        CSE_ALifeItemWeaponMagazined* clone =
            smart_cast< CSE_ALifeItemWeaponMagazined* >( absClone );
        if ( !clone )
            return ( 0 );
...
void CALifeSimulator__iterate_objects(
...
void CALifeSimulator__iterate_objects_without_actor(
    const CALifeSimulator* self,
    const luabind::functor< bool >& functor ) {
    const CALifeObjectRegistry& objects = self->objects();
    for ( const auto& se_obj : objects.objects() ) {
        if ( se_obj.first != 0 && functor( se_obj.second ) )
            break;
    }
}
...
void CALifeSimulator__iterate_level_objects_of_clsid(
    const CALifeSimulator* self,
    int script_clsid_val,
    const luabind::functor< bool >& functor ) {
    if ( !self->graph().level_exists() )
        return;
    const auto& level_reg = self->graph().level();
    for ( const auto& entry : level_reg.objects() ) {
        if ( entry.second->m_script_clsid != script_clsid_val )
            continue;
        if ( functor( entry.second ) )
            break;
    }
}
...
struct alife_object_iterator {
    ...
    alife_object_iterator( const CALifeObjectRegistry::OBJECT_REGISTRY* c )
        : container( c ),
          it( c ? c->begin()
                : CALifeObjectRegistry::OBJECT_REGISTRY::const_iterator() ) {}
...
struct alife_object_without_actor_iterator {
    ...
    alife_object_without_actor_iterator(
        const CALifeObjectRegistry::OBJECT_REGISTRY* c )
        : container( c ),
          it( c ? c->begin()
                : CALifeObjectRegistry::OBJECT_REGISTRY::const_iterator() ) {}
...
::luabind::object alife_object_ids( const CALifeSimulator* self,
                                    const bool keytable = false,
                                    const bool withActor = false ) {
    VERIFY( self );
...
::luabind::object alife_objects( const CALifeSimulator* self,
                                 const bool keytable = false,
                                 const bool withActor = false ) {
    VERIFY( self );
...
xr_vector< u16 >& get_children( const CALifeSimulator* self,
                                CSE_Abstract* object ) {
    VERIFY( self );
...
void CALifeSimulator::script_register( lua_State* L ) {
    module( L )
        [ class_< alife_object_iterator >( "alife_object_iterator" )
              ...
              .def( "__call",
                    &alife_object_iterator::next ), // for Lua for-loop

          class_< alife_object_without_actor_iterator >(
              "alife_object_without_actor_iterator" )
              .def( "next", &alife_object_iterator::next )
              .def( "__call",
                    &alife_object_iterator::next ), // for Lua for-loop
          ...
          class_< CALifeUpdateManager >( "alife_simulator_manager" )
              .def( "set_process_time",
                    &CALifeUpdateManager::set_process_time ),

          class_< CALifeSimulator, bases< CALifeUpdateManager > >(
              "alife_simulator" )
              .def( "valid_object_id", &valid_object_id )
              .def( "level_id", &get_level_id )
              .def( "level_name", &get_level_name )
              .def( "object", ( CSE_ALifeDynamicObject *
                                ( * )( const CALifeSimulator*,
                                       ALife::_OBJECT_ID ) )( alife_object ) )
              .def( "object",
                    ( CSE_ALifeDynamicObject * ( * )( const CALifeSimulator*,
                                                      ALife::_OBJECT_ID,
                                                      bool ))( alife_object ) )
              .def( "story_object",
                    ( CSE_ALifeDynamicObject *
                      ( * )( const CALifeSimulator*, ALife::_STORY_ID ) )(
                        alife_story_object ) )
              .def( "set_switch_online",
                    ( void ( CALifeSimulator::* )( ALife::_OBJECT_ID, bool ) )(
                        &CALifeSimulator::set_switch_online ) )
              .def( "set_switch_offline",
                    ( void ( CALifeSimulator::* )( ALife::_OBJECT_ID, bool ) )(
                        &CALifeSimulator::set_switch_offline ) )
              .def( "set_interactive",
                    ( void ( CALifeSimulator::* )( ALife::_OBJECT_ID, bool ) )(
                        &CALifeSimulator::set_interactive ) )
              ...
              .def( "remove_all_restrictions",
                    &CALifeSimulator::remove_all_restrictions )
              ...
              .def( "iterate_level_objects_of_clsid",
                    &CALifeSimulator__iterate_level_objects_of_clsid )
              /*.def("iterate_objects_without_actor",
              &CALifeSimulator__iterate_objects_without_actor)
              .def("objects_iter", &alife_object_iter)
              .def("objects_without_actor_iter",
              &alife_object_without_actor_iter)*/
...

    {
        if ( spawn_story_ids.empty() )
            generate_story_ids(
                spawn_story_ids, INVALID_SPAWN_STORY_ID, "spawn_story_ids",
        ...
        ::luabind::class_< class_exporter< class_exporter< CALifeSimulator > > >
            instance( "spawn_story_ids" );

        for ( const auto& pair : spawn_story_ids )
            instance = std::move( instance )
                           .enum_( "_spawn_story_ids" )[::luabind::value(
                               *pair.first, pair.second ) ];
