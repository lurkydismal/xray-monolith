...
bool player_legs_controller::resolve_config( CActor* actor,
    ...
    shared_str current_outfit =
        outfit ? outfit->object().cNameSect() : shared_str( "actor" );
    ...
    if ( outfit ) {
        if ( pSettings->line_exist( m_last_outfit_sect, "legs_visual" ) ) {
            sect = m_last_outfit_sect;
            m_last_model =
                pSettings->r_string( m_last_outfit_sect, "legs_visual" );
            model = m_last_model;
            return true;
        }
        ...
        if ( pSettings->line_exist( m_last_outfit_sect, "actor_visual" ) ) {
            sect = m_last_outfit_sect;
            m_last_model =
                pSettings->r_string( m_last_outfit_sect, "actor_visual" );
            model = m_last_model;
            return true;
        }

        warn_once(
            "outfit [%s] has no legs_visual or actor_visual, fallback to "
            "default",
            m_last_outfit_sect.c_str() );
    }
...
bool player_legs_controller::ensure_model( const shared_str& sect,
                                           const shared_str& model ) {
...
void player_legs_controller::copy_bones_from_actor( CActor* actor,
    ...
    root_bi.mRenderTransform.mul_43(
        root_bi.mTransform, m_model->LL_GetData( legs_root ).m2b_transform );
    ...
    if ( bone_count == actor_K->LL_BoneCount() ) {
        for ( u16 i = 0; i < bone_count; ++i ) {
            ...
            m_model->LL_GetTransform_R( i ).set(
                actor_K->LL_GetTransform_R( i ) );
        }
    } else {
        for ( auto& [ bonename, ID ] : *m_model->LL_Bones() ) {
            auto BoneID = actor_K->LL_BoneID( bonename );
            if ( BoneID != BI_NONE ) {
                m_model->LL_GetTransform( ID ).set(
                    actor_K->LL_GetTransform( BoneID ) );
                m_model->LL_GetTransform_R( ID ).set(
                    actor_K->LL_GetTransform_R( BoneID ) );
            }
        }
    }

    if ( !isShadowPass ) {
        if ( auto BoneID = m_model->LL_BoneID( "bip01_spine" );
             BoneID != BI_NONE ) {
            auto& BoneInstance = m_model->LL_GetData( BoneID );
            auto& transform =
                m_model->LL_GetTransform( BoneInstance.GetParentID() );
            transform.c.y += legs_spine_offset_y;
            m_model->Bone_Calculate( &BoneInstance, &transform );
        }

        static LPCSTR bonesToHide[] = { "bip01_neck", "bip01_l_upperarm",
                                        "bip01_r_upperarm" };
...
void player_legs_controller::update( CActor* actor, bool isShadowPass ) {
    ...
    if ( legs_attach_to_camera && pDemoRecords.empty() )
        m_legs_transform.c.set( Device.vCameraPosition.x, m_legs_transform.c.y,
    ...
    float offset =
        m_fwd_offset.has_value() ? m_fwd_offset.value() : legs_fwd_offset;
