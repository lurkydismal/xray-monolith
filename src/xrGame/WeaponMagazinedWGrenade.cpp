...
CWeaponMagazinedWGrenade::CWeaponMagazinedWGrenade( ESoundTypes eSoundType )
    : CWeaponMagazined( eSoundType ) {
    m_ammoType2 = 0;
...
void CWeaponMagazinedWGrenade::Load( LPCSTR section ) {
    ...
    //// Sounds
    m_sounds.LoadSound( section, "snd_shoot_grenade", "sndShotG", true,
                        m_eSoundShot );
    m_sounds.LoadSound( section, "snd_reload_grenade", "sndReloadG", true,
                        m_eSoundReload );
    m_sounds.LoadSound( section, "snd_switch", "sndSwitch", true,
                        m_eSoundReload );

    m_sFlameParticles2 =
        pSettings->r_string( section, "grenade_flame_particles" );

    if ( m_eGrenadeLauncherStatus == ALife::eAddonPermanent ) {
        CRocketLauncher::m_fLaunchSpeed =
            pSettings->r_float( section, "grenade_vel" );
    }
    LoadLauncherKoeffs();
...
BOOL CWeaponMagazinedWGrenade::net_Spawn( CSE_Abstract* DC ) {
    CSE_ALifeItemWeapon* const weapon =
        smart_cast< CSE_ALifeItemWeapon* >( DC );
    R_ASSERT( weapon );
    if ( IsGameTypeSingle() ) {
    ...
    m_DefaultCartridge2.Load( m_ammoTypes2[ m_ammoType2 ].c_str(),
                              m_ammoType2 );

    if ( !IsGameTypeSingle() ) {
        if ( !m_bGrenadeMode && IsGrenadeLauncherAttached() &&
             !getRocketCount() && iAmmoElapsed2 ) {
            ...
            shared_str fake_grenade_name =
                pSettings->r_string( grenade_name, "fake_grenade_name" );

            CRocketLauncher::SpawnRocket( *fake_grenade_name, this );
        }
    } else {
        xr_vector< CCartridge >* pM = NULL;
        bool b_if_grenade_mode =
            ( m_bGrenadeMode && iAmmoElapsed && !getRocketCount() );
        if ( b_if_grenade_mode )
            pM = &m_magazine;

        bool b_if_simple_mode =
            ( !m_bGrenadeMode && m_magazine2.size() && !getRocketCount() );
        if ( b_if_simple_mode )
            pM = &m_magazine2;

        if ( b_if_grenade_mode || b_if_simple_mode ) {
            shared_str fake_grenade_name = pSettings->r_string(
                pM->back().m_ammoSect, "fake_grenade_name" );
    ...
    if ( IsGrenadeLauncherAttached() ) {
        if ( !m_bGrenadeMode ) {
            ...
            if ( HudAnimationExist( "anm_switch_mode_w_gl" ) ) {
                SetPending( TRUE );
                iAmmoElapsed == 0 &&
                        HudAnimationExist( "anm_switch_mode_w_gl_empty" )
                    ? PlayHUDMotion( "anm_switch_mode_w_gl_empty", TRUE, this,
                                     eSwitchMode )
                    : PlayHUDMotion( "anm_switch_mode_w_gl", TRUE, this,
                                     eSwitchMode );
            } else
                UpdateFireMode();
...
void CWeaponMagazinedWGrenade::PerformSwitchGL() {
    ...
    if ( m_bGrenadeMode && !getRocketCount() ) {
        shared_str fake_grenade_name = pSettings->r_string(
            m_ammoTypes[ m_ammoType ].c_str(), "fake_grenade_name" );
...
void CWeaponMagazinedWGrenade::AmmoTypeForEach2(
    const ::luabind::functor< bool >& funct ) {
...
void CWeaponMagazinedWGrenade::state_Fire( float dt ) {
    ...
    if ( m_bGrenadeMode ) {
        /*
        fTime					-=dt;
        while (fTime<=0 && (iAmmoElapsed>0) && (IsWorking() ||
        m_bFireSingleShot))
        {
        ++m_iShotNum;
        OnShot			();
...
void CWeaponMagazinedWGrenade::LaunchGrenade() {
    ...
    R_ASSERT( m_bGrenadeMode );
    {
        ...
        float launch_speed =
            READ_IF_EXISTS( pSettings, r_float, ammo_name, "ammo_grenade_vel",
                            CRocketLauncher::m_fLaunchSpeed );
#endif
        Fvector p1, d;
        p1.set( get_LastFP2() );
        ...
        Fvector::generate_orthonormal_basis( launch_matrix.k, launch_matrix.j,
                                             launch_matrix.i );

        launch_matrix.c.set( p1 );

        if ( IsGameTypeSingle() && IsZoomed() &&
             smart_cast< CActor* >( H_Parent() ) &&
             g_launcher_dynamic_range_zoom ) {
            H_Parent()->setEnabled( FALSE );
            setEnabled( FALSE );

            collide::rq_result RQ;
            BOOL HasPick = Level().ObjectSpace.RayPick(
                p1, d, 300.0f, collide::rqtStatic, RQ, this );
            ...
            if ( HasPick ) {
                ...
                u8 canfire0 = TransferenceAndThrowVelToThrowDir(
                    Transference, launch_speed, EffectiveGravity(), res );
#else
                u8 canfire0 = TransferenceAndThrowVelToThrowDir(
                    Transference, CRocketLauncher::m_fLaunchSpeed,
...
#endif
        VERIFY2(
            _valid( launch_matrix ),
            "CWeaponMagazinedWGrenade::SwitchState. Invalid launch_matrix!" );
        CRocketLauncher::LaunchRocket( launch_matrix, d, zero_vel );

        CExplosiveRocket* pGrenade =
            smart_cast< CExplosiveRocket* >( getCurrentRocket() );
        VERIFY( pGrenade );
...
bool CWeaponMagazinedWGrenade::Detach( LPCSTR item_section_name,
                                       bool b_spawn_item ) {
    if ( ALife::eAddonAttachable == m_eGrenadeLauncherStatus &&
         0 != ( m_flagsAddOnState &
                CSE_ALifeItemWeapon::eWeaponAddonGrenadeLauncher ) &&
...
void CWeaponMagazinedWGrenade::InitAddons() {
    ...
    if ( GrenadeLauncherAttachable() ) {
        if ( IsGrenadeLauncherAttached() ) {
            CRocketLauncher::m_fLaunchSpeed =
                pSettings->r_float( *m_sGrenadeLauncherName, "grenade_vel" );
            ApplyLauncherKoeffs();
        } else {
            ResetLauncherKoeffs();
        }
...
void CWeaponMagazinedWGrenade::LoadLauncherKoeffs() {
    if ( m_eGrenadeLauncherStatus == ALife::eAddonAttachable ) {
        ...
        m_launcher_koef.cam_dispersion = READ_IF_EXISTS(
            pSettings, r_float, sect, "cam_dispersion_k", 1.0f );
        m_launcher_koef.cam_disper_inc = READ_IF_EXISTS(
            pSettings, r_float, sect, "cam_dispersion_inc_k", 1.0f );
        m_launcher_koef.pdm_base =
            READ_IF_EXISTS( pSettings, r_float, sect, "PDM_disp_base_k", 1.0f );
        m_launcher_koef.pdm_accel = READ_IF_EXISTS( pSettings, r_float, sect,
                                                    "PDM_disp_accel_k", 1.0f );
        m_launcher_koef.pdm_vel =
            READ_IF_EXISTS( pSettings, r_float, sect, "PDM_disp_vel_k", 1.0f );
        m_launcher_koef.crosshair_inertion = READ_IF_EXISTS(
            pSettings, r_float, sect, "crosshair_inertion_k", 1.0f );
        m_launcher_koef.zoom_rotate_time = READ_IF_EXISTS(
        ...
        if ( !m_bGrenadeMode )
            iAmmoElapsed == 0 && HudAnimationExist( "anm_show_empty_w_gl" )
                ? PlayHUDMotion( "anm_show_empty_w_gl", FALSE, this, GetState(),
                                 1.f, 0.f, false )
                : PlayHUDMotion( "anm_show_w_gl", FALSE, this, GetState(), 1.f,
                                 0.f, false );
        else
            iAmmoElapsed == 0 && HudAnimationExist( "anm_show_empty_g" )
                ? PlayHUDMotion( "anm_show_empty_g", FALSE, this, GetState(),
                                 1.f, 0.f, false )
                : PlayHUDMotion( "anm_show_g", FALSE, this, GetState(), 1.f,
                                 0.f, false );
    } else
    ...
    if ( IsGrenadeLauncherAttached() ) {
        if ( bMisfire ) {
            if ( HudAnimationExist( "anm_reload_misfire_w_gl" ) ) {
                PlayHUDMotion( "anm_reload_misfire_w_gl", TRUE, this,
                               GetState() );
                bClearJamOnly = true;
                return;
            } else
            ...
            if ( iAmmoElapsed == 0 ) {
                if ( HudAnimationExist( "anm_reload_empty_w_gl" ) )
                    PlayHUDMotion( "anm_reload_empty_w_gl", TRUE, this,
                                   GetState() );
                else
                    PlayHUDMotion( "anm_reload_w_gl", TRUE, this, GetState() );
            } else {
        ...
        if ( IsZoomed() ) {
            if ( m_bGrenadeMode )
                iAmmoElapsed == 0 && HudAnimationExist( "anm_idle_empty_g_aim" )
                    ? PlayHUDMotion( "anm_idle_empty_g_aim", TRUE, NULL,
                                     GetState() )
                    : PlayHUDMotion( "anm_idle_g_aim", TRUE, NULL, GetState() );
            else
                iAmmoElapsed == 0 &&
                        HudAnimationExist( "anm_idle_empty_w_gl_aim" )
                    ? PlayHUDMotion( "anm_idle_empty_w_gl_aim", TRUE, NULL,
                                     GetState() )
                    : PlayHUDMotion( "anm_idle_w_gl_aim", TRUE, NULL,
                                     GetState() );
        } else {
            int act_state = 0;
            ...
            if ( m_bGrenadeMode ) {
                if ( act_state == 0 || psDeviceFlags2.test( rsBlendMoveAnims ) )
                    iAmmoElapsed == 0 && HudAnimationExist( "anm_idle_empty_g" )
                        ? PlayHUDMotion( "anm_idle_empty_g", TRUE, NULL,
                                         GetState() )
                        : PlayHUDMotion( "anm_idle_g", TRUE, NULL, GetState() );
                else if ( act_state == 1 )
                    iAmmoElapsed == 0 &&
                            HudAnimationExist( "anm_idle_sprint_empty_g" )
                        ? PlayHUDMotion( "anm_idle_sprint_empty_g", TRUE, NULL,
                                         GetState() )
                        : PlayHUDMotion( "anm_idle_sprint_g", TRUE, NULL,
                                         GetState() );
                else if ( act_state == 2 ) {
                    iAmmoElapsed == 0 &&
                            HudAnimationExist( "anm_idle_moving_empty_g" )
                        ? PlayHUDMotion( "anm_idle_moving_empty_g", TRUE, NULL,
                                         GetState() )
                        : PlayHUDMotion( "anm_idle_moving_g", TRUE, NULL,
                                         GetState() );
                } else if ( act_state == 3 ) {
#ifdef NEW_ANIMS // AVO: custom move animation
                    iAmmoElapsed == 0 && HudAnimationExist(
                                             "anm_idle_moving_crouch_empty_g" )
                        ? PlayHUDMotion( "anm_idle_moving_crouch_empty_g", TRUE,
                                         NULL, GetState() )
                    : iAmmoElapsed == 0 &&
                            HudAnimationExist( "anm_idle_moving_empty_g" )
                        ? PlayHUDMotion( "anm_idle_moving_empty_g", TRUE, NULL,
                ...
                if ( act_state == 0 || psDeviceFlags2.test( rsBlendMoveAnims ) )
                    iAmmoElapsed == 0 &&
                            HudAnimationExist( "anm_idle_empty_w_gl" )
                        ? PlayHUDMotion( "anm_idle_empty_w_gl", TRUE, NULL,
                                         GetState() )
                        : PlayHUDMotion( "anm_idle_w_gl", TRUE, NULL,
                                         GetState() );
                else if ( act_state == 1 )
                    iAmmoElapsed == 0 &&
                            HudAnimationExist( "anm_idle_sprint_empty_w_gl" )
                        ? PlayHUDMotion( "anm_idle_sprint_empty_w_gl", TRUE,
                                         NULL, GetState() )
                        : PlayHUDMotion( "anm_idle_sprint_w_gl", TRUE, NULL,
                                         GetState() );
                else if ( act_state == 2 ) {
                    iAmmoElapsed == 0 &&
                            HudAnimationExist( "anm_idle_moving_empty_w_gl" )
                        ? PlayHUDMotion( "anm_idle_moving_empty_w_gl", TRUE,
                                         NULL, GetState() )
                        : PlayHUDMotion( "anm_idle_moving_w_gl", TRUE, NULL,
                                         GetState() );
                } else if ( act_state == 3 ) {
#ifdef NEW_ANIMS // AVO: custom move animation
                    iAmmoElapsed == 0 &&
                        ...
                        ? PlayHUDMotion( "anm_idle_moving_crouch_empty_w_gl",
                                         TRUE, NULL, GetState() )
                    : iAmmoElapsed == 0 &&
                            HudAnimationExist( "anm_idle_moving_empty_w_gl" )
                        ? PlayHUDMotion( "anm_idle_moving_empty_w_gl", TRUE,
    ...
    if ( m_bGrenadeMode ) {
        if ( iAmmoElapsed > 1 || !HudAnimationExist( "anm_shot_g_l" ) ) {
            if ( !IsZoomed() || !HudAnimationExist( "anm_shots_g_aim" ) )
                PlayHUDMotion( "anm_shots_g", TRUE, this, GetState(), 1.f, 0.f,
                               false );
            else
                PlayHUDMotion( "anm_shots_g_aim", TRUE, this, GetState(), 1.f,
                               0.f, false );
        } else {
            if ( !IsZoomed() || !HudAnimationExist( "anm_shot_g_l_aim" ) )
                PlayHUDMotion( "anm_shot_g_l", TRUE, this, GetState(), 1.f, 0.f,
                               false );
            else
                PlayHUDMotion( "anm_shot_g_l_aim", TRUE, this, GetState(), 1.f,
                               0.f, false );
        }
    } else {
        ...
        if ( IsGrenadeLauncherAttached() )
            if ( iAmmoElapsed > 1 || !HudAnimationExist( "anm_shot_w_gl_l" ) ) {
                ...
                if ( !IsZoomed() ||
                     !HudAnimationExist( "anm_shot_w_gl_l_aim" ) )
                    PlayHUDMotion( "anm_shot_w_gl_l", TRUE, this, GetState(),
                                   1.f, 0.f, false );
                else
                    PlayHUDMotion( "anm_shot_w_gl_l_aim", TRUE, this,
                                   GetState(), 1.f, 0.f, false );
            }
        else
...
void CWeaponMagazinedWGrenade::PlayAnimModeSwitch() {
    if ( m_bGrenadeMode )
        ...
        : HudAnimationExist( "anm_switch_g" )
...
bool CWeaponMagazinedWGrenade::TryPlayAnimBore() {
    if ( IsGrenadeLauncherAttached() ) {
        if ( m_bGrenadeMode ) {
            if ( iAmmoElapsed == 0 &&
                 HudAnimationExist( "anm_bore_empty_g" ) ) {
            ...
            if ( iAmmoElapsed == 0 &&
                 HudAnimationExist( "anm_bore_empty_w_gl" ) ) {
...
void CWeaponMagazinedWGrenade::load( IReader& input_packet ) {
    ...
    if ( b && !m_bGrenadeMode ) {
        Msg( "![%s] ERROR: CWeaponMagazinedWGrenade::load: m_bGrenadeMode = "
             "%d, failed to switch to grenade mode",
             Name(), m_bGrenadeMode );
        return;
    }
    ...
    if ( sz > 0xffff ) {
        Msg( "![%s] ERROR: CWeaponMagazinedWGrenade::load: current magazine "
             "size %zu for underbarrel is too big, truncate to 1",
             Name(), sz );
        sz = 1;
    }
...
bool CWeaponMagazinedWGrenade::IsNecessaryItem( const shared_str& item_sect ) {
    return ( std::find( m_ammoTypes.begin(), m_ammoTypes.end(), item_sect ) !=
                 m_ammoTypes.end() ||
             std::find( m_ammoTypes2.begin(), m_ammoTypes2.end(), item_sect ) !=
                 m_ammoTypes2.end() );
}
...
bool CWeaponMagazinedWGrenade::install_upgrade_ammo_class( LPCSTR section,
                                                           bool test ) {
    LPCSTR str;
    ...
    if ( result2 && !test ) {
        xr_vector< shared_str >& ammo_types =
            m_bGrenadeMode ? m_ammoTypes2 : m_ammoTypes;
        ammo_types.clear();
...
bool CWeaponMagazinedWGrenade::install_upgrade_impl( LPCSTR section,
                                                     bool test ) {
    LPCSTR str;
    ...
    result |= process_if_exists( section, "launch_speed", &CInifile::r_float,
                                 m_fLaunchSpeed, test );

    result2 = process_if_exists_set( section, "snd_shoot_grenade",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        m_sounds.LoadSound( section, "snd_shoot_grenade", "sndShotG", false,
                            m_eSoundShot );
    }
    result |= result2;

    result2 = process_if_exists_set( section, "snd_reload_grenade",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        m_sounds.LoadSound( section, "snd_reload_grenade", "sndReloadG", true,
                            m_eSoundReload );
    ...
    result2 = process_if_exists_set( section, "snd_switch", &CInifile::r_string,
                                     str, test );
    if ( result2 && !test ) {
        m_sounds.LoadSound( section, "snd_switch", "sndSwitch", true,
                            m_eSoundReload );
...
void CWeaponMagazinedWGrenade::net_Spawn_install_upgrades(
    Upgrades_type saved_upgrades ) {
    // do not delete this
...
bool CWeaponMagazinedWGrenade::GetBriefInfo( II_BriefInfo& info ) {
    ...
    if ( unlimited_ammo() || at_size == 0 ) {
        ...
        xr_sprintf( int_str, "%d",
                    m_bGrenadeMode ? GetAmmoCount2( ammo_type )
        ...
        if ( at_size >= 1 ) {
            xr_sprintf(
                int_str, "%d",
                m_bGrenadeMode ? GetAmmoCount2( 0 ) : GetAmmoCount( 0 ) );
            info.fmj_ammo._set( int_str );
        }
        if ( at_size >= 2 ) {
            xr_sprintf(
                int_str, "%d",
                m_bGrenadeMode ? GetAmmoCount2( 1 ) : GetAmmoCount( 1 ) );
            info.ap_ammo._set( int_str );
        }
        if ( at_size >= 3 ) {
            xr_sprintf(
                int_str, "%d",
                m_bGrenadeMode ? GetAmmoCount2( 2 ) : GetAmmoCount( 2 ) );
            info.third_ammo._set( int_str );
        }
        //-Alundaio
    }
    ...
    if ( ae != 0 && m_magazine.size() != 0 ) {
        ...
        info.name._set( CStringTable().translate(
