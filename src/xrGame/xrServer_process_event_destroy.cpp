...
xr_string xrServer::ent_name_safe( u16 eid ) {
    ...
    if ( e_dest )
        xr_sprintf( buff, "[%d][%s:%s]", eid, e_dest->name(),
                    e_dest->name_replace() );
    else
...
void xrServer::Process_event_destroy( NET_Packet& P,
                                      ClientID sender,
                                      u32 time,
                                      u16 ID,
                                      NET_Packet* pEPack ) {
    ...
    if ( dbg_net_Draw_Flags.test( dbg_destroy ) )
        Msg( "sv destroy object %s [%d]", ent_name_safe( id_dest ).c_str(),
             Device.dwFrame );
#endif

    CSE_Abstract* e_dest =
        game->get_entity_from_eid( id_dest ); // кто должен быть уничтожен
    if ( !e_dest ) {
#ifndef MASTER_GOLD
...
#ifdef MP_LOGGING
    Msg( "--- SV: Process destroy: parent [%d] item [%d][%s]", parent_id,
         id_dest, e_dest->name() );
    ...
    if ( 0xffff == parent_id && NULL == pEventPack ) {
        ...
        NET_Packet tmpP;
        if ( 0xffff != parent_id &&
             Process_event_reject( P, sender, time, parent_id, ID, false ) ) {
