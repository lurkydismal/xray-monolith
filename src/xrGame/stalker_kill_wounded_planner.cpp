...
CStalkerKillWoundedPlanner::CStalkerKillWoundedPlanner( CAI_Stalker* object,
                                                        LPCSTR action_name )
    : inherited( object, action_name ) {}

CStalkerKillWoundedPlanner::~CStalkerKillWoundedPlanner() {}

void CStalkerKillWoundedPlanner::setup( CAI_Stalker* object,
                                        CPropertyStorage* storage ) {
    inherited::setup( object, storage );

    CScriptActionPlanner::m_storage.set_property(
        eWorldPropertyWoundedEnemyPrepared, false );
...
void CStalkerKillWoundedPlanner::update() {
    ...
    //		inherited_action::m_storage->set_property
    //(eWorldPropertyKilledWounded,true); 	else
    //		inherited_action::m_storage->set_property
...
void CStalkerKillWoundedPlanner::initialize() {
    ...
    CScriptActionPlanner::m_storage.set_property(
        eWorldPropertyWoundedEnemyPrepared, false );
    CScriptActionPlanner::m_storage.set_property(
        eWorldPropertyWoundedEnemyAimed, false );
    CScriptActionPlanner::m_storage.set_property( eWorldPropertyPausedAfterKill,
                                                  false );

    inherited_action::m_storage->set_property( eWorldPropertyKilledWounded,
                                               true );
}
...
void CStalkerKillWoundedPlanner::finalize() {
    ...
    if ( object().memory().enemy().selected() ) {
        ...
        object().movement().set_mental_state(
            MonsterSpace::eMentalStateDanger );
    }
}
...
void CStalkerKillWoundedPlanner::add_evaluators() {
    ...
    add_evaluator( eWorldPropertyWoundedEnemyReached,
                   xr_new< CStalkerPropertyEvaluatorEnemyReached >(
                       m_object, "is enemy reached" ) );

    add_evaluator(
        eWorldPropertyWoundedEnemyPrepared,
        xr_new< CStalkerPropertyEvaluatorMember >(
            ( CPropertyStorage* )0, eWorldPropertyWoundedEnemyPrepared, true,
            true, "is enemy prepared" ) );
    add_evaluator( eWorldPropertyWoundedEnemyAimed,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyWoundedEnemyAimed,
                       true, true, "is enemy aimed" ) );
    add_evaluator( eWorldPropertyPausedAfterKill,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyPausedAfterKill,
                       true, true, "is paused after enemy kill" ) );
}
...
void CStalkerKillWoundedPlanner::add_actions() {
    ...
    action =
        xr_new< CStalkerActionReachWounded >( m_object, "reach wounded enemy" );
    ...
    action =
        xr_new< CStalkerActionAimWounded >( m_object, "aim at wounded enemy" );
    add_condition( action, eWorldPropertyPausedAfterKill, false );
    ...
    action = xr_new< CStalkerActionPrepareWounded >( m_object,
                                                     "prepare wounded enemy" );
    add_condition( action, eWorldPropertyPausedAfterKill, false );
    add_condition( action, eWorldPropertyWoundedEnemyReached, true );
    ...
    action =
        xr_new< CStalkerActionKillWounded >( m_object, "kill wounded enemy" );
    add_condition( action, eWorldPropertyWoundedEnemyReached, true );
    ...
    action =
        xr_new< CStalkerActionPauseAfterKill >( m_object, "pause after kill" );
