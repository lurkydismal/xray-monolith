...
loophole_action_base::loophole_action_base( CAI_Stalker* object,
                                            LPCSTR action_name )
    : inherited( object, action_name ) {}

Fvector loophole_action_base::nearest_loophole_direction(
    ...
    if ( fov_direction_left.dotproduct( direction ) >
         fov_direction_right.dotproduct( direction ) )
...
void loophole_action_base::process_fire_position( bool const& change_sight ) {
    ...
    Fvector const& position =
        *object().movement().current_params().cover_fire_position();
...
void loophole_action_base::process_fire_object( bool const& change_sight ) {
    ...
    CGameObject const* fire_object =
        object().movement().current_params().cover_fire_object();
    if ( object().movement().in_current_loophole_fov(
             fire_object->Position() ) ) {
    ...
    object().sight().setup( CSightAction(
        SightManager::eSightTypeDirection,
        nearest_loophole_direction( fire_object->Position() ), true ) );
...
void loophole_action_base::process_default( bool const& change_sight ) {
    ...
    if ( object().movement().current_params().cover() !=
         object().movement().target_params().cover() )
        return;
...
bool loophole_action_base::process_enemy( bool const& change_sight ) {
    ...
    if ( enemy_in_fov() ) {
        if ( object().memory().visual().visible_now( enemy ) )
            ...
            object().sight().setup( CSightAction(
                SightManager::eSightTypePosition,
                object().memory().memory_position( enemy ), true ) );
    ...
    object().sight().setup(
        CSightAction( SightManager::eSightTypeDirection,
                      nearest_loophole_direction( enemy->Position() ), true ) );
    object().sight().update();
...
void loophole_action::initialize() {
    ...
    ActionAnimations const& animations =
        object()
            ...
            .current_params()
            .cover_loophole()
            ->action_animations( m_action_id, animation_id );
...
loophole_action_no_sight::loophole_action_no_sight( CAI_Stalker* object,
                                                    LPCSTR action_name )
    : inherited( object, action_name ) {}
...
transition::transition( CAI_Stalker* object,
                        LPCSTR action_name,
                        LPCSTR action_from,
                        LPCSTR action_to,
...
void transition::initialize() {
    ...
    TransitionData const& animations =
        object()
            ...
            .current_params()
            .cover_loophole()
            ->transition_animations( m_action_from, m_action_to );
    m_animation = animations[ m_random.randI( animations.size() ) ];
}
...
void loophole_lookout::initialize() {
    ...
    object().sight().bone_aiming( m_animation,
                                  CSightManager::animation_frame_start,
                                  CSightManager::aiming_head );
}
...
void idle_2_fire_transition::initialize() {
    ...
    object().sight().bone_aiming( m_animation,
                                  CSightManager::animation_frame_end,
                                  CSightManager::aiming_weapon );
    setup_sight( true );
    //	object().sight().enable
    //(false);
