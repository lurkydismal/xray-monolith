...
bool CSpaceRestrictionBase::inside( u32 level_vertex_id,
...
IC bool CSpaceRestrictionBase_inside( CSpaceRestrictionBase* self,
...
bool CSpaceRestrictionBase::inside( u32 level_vertex_id,
    ...
    if ( partially_inside )
        return ( CSpaceRestrictionBase_inside(
                     this,
                     construct_position( level_vertex_id, position.x + offset,
                                         position.z + offset ),
                     radius ) ||
                 ...
                 CSpaceRestrictionBase_inside(
                     ...
                     radius ) ||
                 CSpaceRestrictionBase_inside(
                     this, Fvector().set( position.x, position.y, position.z ),
                     radius ) );
    else
        return ( CSpaceRestrictionBase_inside(
                     this,
...
struct SortByXZ_predicate {
    IC bool operator()( u32 v0, u32 v1 ) const {
        return ( ai().level_graph().vertex( v0 )->position().xz() <
                 ai().level_graph().vertex( v1 )->position().xz() );
    }
};
