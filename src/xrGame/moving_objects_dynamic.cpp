...
struct priority {
    IC static bool standing( const moving_object* object ) {
        ...
        return ( !!fis_zero( object->position().distance_to_sqr(
            object->predict_position( 1.f ) ) ) );
    }

    IC static bool predicate( const moving_object* _0,
                              const moving_object* _1 ) {
        if ( standing( _0 ) ) {
            if ( standing( _1 ) )
                return ( _0 < _1 );
    ...
    IC static bool predicate2( const moving_objects::COLLISION_TIME& _0,
                               const moving_objects::COLLISION_TIME& _1 ) {
        ...
        return (
            predicate( _0.second.second.second, _1.second.second.second ) );
    }
};
...
struct already_wait_predicate {
    ...
    IC bool operator()( moving_object* object ) const {
        moving_objects::COLLISIONS::const_iterator I = std::find_if(
            m_collisions->begin(), m_collisions->end(), collision( object ) );
        if ( I == m_collisions->end() )
            return ( false );

        if ( ( *I ).second.second.first == object ) {
            if ( ( *I ).second.first ==
                 moving_objects::possible_action_1_can_wait_2 )
                return ( true );
        ...
        if ( ( *I ).second.second.second == object ) {
            if ( ( *I ).second.first ==
                 moving_objects::possible_action_2_can_wait_1 )
                return ( true );
...
void moving_objects::fill_nearest_moving( moving_object* object ) {
    ...
    float linear_velocity =
        next_position.distance_to( object->position() ) / time_to_check;
    ...
    m_nearest_moving.erase(
        std::remove_if( m_nearest_moving.begin(), m_nearest_moving.end(),
                        &already_computed::predicate ),
        m_nearest_moving.end() );

    u32 size = m_nearest_moving.size();
    moving_object** temp =
        ( moving_object** )_alloca( size * sizeof( moving_object* ) );
    std::copy( m_nearest_moving.begin(), m_nearest_moving.end(), temp );
    std::sort( temp, temp + size );
    m_nearest_moving.erase(
        std::set_difference( temp, temp + size, m_visited_emitters.begin(),
...
void moving_objects::generate_emitters() {
    ...
    m_collision_emitters.erase(
        std::set_difference( m_nearest_moving.begin(), m_nearest_moving.end(),
                             m_collision_emitters.begin(),
                             m_collision_emitters.begin() + size,
                             m_collision_emitters.begin() + size ),
        m_collision_emitters.end() );

    std::inplace_merge( m_collision_emitters.begin(),
                        m_collision_emitters.begin() + size,
                        m_collision_emitters.end() );
}

bool moving_objects::exchange_all( moving_object* previous,
                                   moving_object* next,
                                   const u32& collision_count ) {
    bool result = true;
...
bool moving_objects::fill_collisions( moving_object* object,
                                      const Fvector& object_position,
                                      const float& time_to_check ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( priority ) {
            if ( !collided_dynamic( object, object_position, ( *I ),
                                    ( *I )->predict_position( time_to_check ),
                                    action ) )
                continue;
            ...
            if ( !collided_dynamic( ( *I ),
                                    ( *I )->predict_position( time_to_check ),
                                    object, object_position, action ) )
                continue;
        ...
        m_collisions.push_back( std::make_pair(
            ...
            std::make_pair( action,
                            std::make_pair( priority ? object : ( *I ),
                                            !priority ? object : ( *I ) ) ) ) );
...

    {
        ...
        COLLISION_TIME* collisions = ( COLLISION_TIME* )_alloca(
            collision_count * sizeof( COLLISION_TIME ) );
        ...
        for ( ; I != E; ++I ) {
            ...
            if ( priority ) {
                if ( !collided_dynamic( object, object_position, test,
                                        test->position() ) )
                    continue;
            } else {
                if ( !collided_dynamic( test, test->position(), object,
                                        object_position ) )
                    continue;
            }
            ...
            m_collisions.push_back( std::make_pair(
                time_to_check,
                std::make_pair(
                    priority ? possible_action_2_can_wait_1
                             : possible_action_1_can_wait_2,
                    std::make_pair( priority ? object : test,
        ...
        for ( ; i != e; ++i ) {
            if ( ( *i ).second.second.first == object ) {
                if ( exchange_all( ( *i ).second.second.second, object,
                                   collision_count ) )
                    continue;
            ...
            if ( !exchange_all( ( *i ).second.second.first, object,
                                collision_count ) )
                ( *i ).second.second.first = 0;
        }
...
void moving_objects::generate_collisions( moving_object* object ) {
    ...
    float linear_velocity =
        dest_position.distance_to( object->position() ) / time_to_check;
    float distance_to_check = time_to_check * linear_velocity;
    u32 step_count = iFloor( distance_to_check / step_to_check + .5f );
    for ( u32 i = 0; ( i < step_count ) && !m_nearest_moving.empty();
          ++i, remove_already_waited() ) {
        if ( !i ) {
        ...
        if ( !fill_collisions( object,
                               object->predict_position( i * step_to_check ),
                               i * step_to_check ) )
            return;
    }
}
...
void moving_objects::resolve_collisions() {
    ...
    moving_object** collidees =
        ( moving_object** )_alloca( collidee_count * sizeof( moving_object* ) );
    {
        ...
        collidee_count = u32(
            std::unique( collidees, collidees + collidee_count ) - collidees );
    }

    u32 decision_count = collidee_count;
    decision* decisions =
        ( decision* )_alloca( decision_count * sizeof( decision ) );
        ...
        for ( ; I != E; ++I ) {
            switch ( ( *I ).second.first ) {
                case possible_action_1_can_wait_2: {
                    decision* object = std::find_if(
                        decisions, decisions + decision_count,
                        decision_predicate( ( *I ).second.second.first ) );
                    VERIFY( object != ( decisions + decision_count ) );
                    *object = std::make_pair( object->first,
                                              moving_object::action_wait );
                    //					(*I).second.second.second->dynamic_query().merge((*I).second.second.first->dynamic_query());
                    ( *I ).second.second.second->dynamic_query().add(
                        &object->first->object() );
                    continue;
                }
                case possible_action_2_can_wait_1: {
                    ...
                    *object = std::make_pair( object->first,
                    ...
                    ( *I ).second.second.first->dynamic_query().add(
                        &object->first->object() );
                    continue;
                }
                default:
...
void moving_objects::query_action_dynamic( moving_object* object ) {
    ...
    while ( !m_collision_emitters.empty() ) {
        ...
        m_visited_emitters.insert(
            std::lower_bound( m_visited_emitters.begin(),
                              m_visited_emitters.end(), object ),
            object );
