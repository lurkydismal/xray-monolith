...
void CMincer::OnStateSwitch( EZoneState new_state ) {
    if ( m_eZoneState != eZoneStateBlowout && new_state == eZoneStateBlowout ) {
        ...
        for ( it = m_ObjectInfoMap.begin(); m_ObjectInfoMap.end() != it;
              ++it ) {
            CPhysicsShellHolder* GO =
                smart_cast< CPhysicsShellHolder* >( ( *it ).object );
            if ( GO )
                Telekinesis().activate( GO, m_fThrowInImpulse, m_fTeleHeight,
                                        100000 );
        }
    }
...
void CMincer::Load( LPCSTR section ) {
    ...
    m_fActorBlowoutRadiusPercent =
        pSettings->r_float( section, "actor_blowout_radius_percent" );
}
...
void CMincer::AffectThrow( SZoneObjectInfo* O,
                           CPhysicsShellHolder* GO,
...
void CMincer::NotificateDestroy( CPHDestroyableNotificate* dn ) {
    ...
    if ( PP && *m_sParticlesSkeleton ) {
        PP->StartParticles( m_sParticlesSkeleton, Fvector().set( 0, 1, 0 ),
                            ID() );
    }
    ...
    CreateHit( obj->ID(), ID(), throw_in_dir, power, 0, position_in_bone_space,
               impulse, ALife::eHitTypeExplosion );
}

void CMincer::AffectPullAlife( CEntityAlive* EA,
                               const Fvector& throw_in_dir,
                               float dist ) {
    ...
    if ( !smart_cast< CActor* >( EA ) ) {
        ...
        CreateHit( EA->ID(), ID(), throw_in_dir, power, 0, pos_in_bone_space,
                   0.0f, m_eHitTypeBlowout );
    }
    inherited::AffectPullAlife( EA, throw_in_dir, dist );
}

float CMincer::BlowoutRadiusPercent( CPhysicsShellHolder* GO ) {
    return ( !smart_cast< CActor* >( GO ) ? m_fBlowoutRadiusPercent
                                          : m_fActorBlowoutRadiusPercent );
}
