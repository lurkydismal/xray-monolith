...
action_base* animation_selector::current_operator() const {
    return ( &smart_cast< smart_cover::action_base& >(
        m_planner->current_action() ) );
}

MotionID animation_selector::select_animation(
    ...
    // Slipch told me, that timeCurrent can decrease (setup to 0.f) during
    // animation playing therefore we need here to clamp previous value
...
void animation_selector::modify_animation( CBlend* blend ) {
    ...
    CMotionDef* motion_def =
        m_skeleton_animated->LL_GetMotionDef( blend->motionID );
    VERIFY( motion_def );
...
void animation_selector::setup( CAI_Stalker* object,
