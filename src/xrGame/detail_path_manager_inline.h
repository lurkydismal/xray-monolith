...
IC bool CDetailPathManager::completed( const Fvector& position,
                                       bool bRealCompleted ) const {
    return ( completed( position, bRealCompleted, m_current_travel_point ) );
}

IC const xr_vector< DetailPathManager::STravelPathPoint >&
CDetailPathManager::path() const {
    return ( m_path );
}

IC xr_vector< DetailPathManager::STravelPathPoint >&
CDetailPathManager::path() {
    return ( m_path );
}

IC const DetailPathManager::STravelPathPoint&
CDetailPathManager::curr_travel_point() const {
...
IC u32 CDetailPathManager::curr_travel_point_index() const {
    VERIFY2( !m_path.empty() && ( m_current_travel_point < m_path.size() ),
             make_string( "path[%d], current[%d]", m_path.size(),
                          m_current_travel_point ) );
    return ( m_current_travel_point );
}

IC void CDetailPathManager::set_start_position(
    const Fvector& start_position ) {
    m_start_position = start_position;
}

IC void CDetailPathManager::set_start_direction(
    const Fvector& start_direction ) {
...
IC void CDetailPathManager::set_dest_position( const Fvector& dest_position ) {
    ...
    if ( !( !m_restricted_object ||
            m_restricted_object->accessible( dest_position ) ) ) {
        LogStackTrace( "error call stack" );
    }
#endif // DEBUG
    THROW2( !m_restricted_object ||
                m_restricted_object->accessible( dest_position ),
            "Old movement destination is not accessible after changing "
            "restrictions!" );
...
IC void CDetailPathManager::set_dest_direction(
    const Fvector& dest_direction ) {
...
IC void CDetailPathManager::adjust_point( const Fvector2& source,
                                          float yaw,
                                          float magnitude,
                                          Fvector2& dest ) const {
    dest.x = -_sin( yaw );
...
IC void CDetailPathManager::assign_angle( float& angle,
    ...
    if ( !start && ( ( direction_type == eDirectionTypePP ) ||
                     ( direction_type == eDirectionTypeNN ) ) )
        if ( angle <= 0.f )
...
IC bool CDetailPathManager::compute_circles( STrajectoryPoint& point,
    ...
    circles[ 0 ].center.y =
        -point.direction.x * point.radius + point.position.y;
    circles[ 1 ].center.x =
        -point.direction.y * point.radius + point.position.x;
...
IC void CDetailPathManager::set_use_dest_orientation(
    const bool use_dest_orientation ) {
    m_actuality =
        m_actuality && ( use_dest_orientation == m_use_dest_orientation );
...
IC void CDetailPathManager::set_state_patrol_path(
    const bool state_patrol_path ) {
...
IC const CDetailPathManager::STravelParams& CDetailPathManager::velocity(
    const u32& velocity_id ) const {
    VELOCITIES::const_iterator I = m_movement_params.find( velocity_id );
    VERIFY( m_movement_params.end() != I );
    return ( ( *I ).second );
}

IC void CDetailPathManager::add_velocity( const u32& velocity_id,
                                          const STravelParams& params ) {
...
IC void CDetailPathManager::extrapolate_length( float extrapolate_length ) {
    m_actuality =
        m_actuality && ( fsimilar( m_extrapolate_length, extrapolate_length ) );
...
IC const CDetailPathManager::VELOCITIES& CDetailPathManager::velocities()
    const {
...
#ifdef DEBUG
IC xr_vector< CDetailPathManager::STravelPoint >&
CDetailPathManager::key_points() {
    return ( m_key_points );
}
#endif // DEBUG
