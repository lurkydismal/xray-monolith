...
void writer::set_player_cdkey_digest( shared_str const& cdkeydigest ) {
    m_info_data.w_string( ss_info_secion, ss_player_digest_key,
                          cdkeydigest.c_str() );
}

/*void writer::set_admin_name				(shared_str const &
admin_name)
{
        m_info_data.w_string(ss_info_secion, ss_admin_name_key,
admin_name.c_str());
}*/

u32 const writer::write_info( sha_process_yielder* yielder ) {
    string64 time_string;
    m_info_data.w_string( ss_info_secion, ss_creation_date,
                          current_time( time_string ) );

    char* info_start =
        static_cast< char* >( ( void* )( m_buffer + m_buffer_info_pos ) );

    info_start[ 0 ] = 0;
    xr_strcat( info_start, info_max_size,
               m_info_data.r_string( ss_info_secion, ss_player_name_key ) );
    xr_strcat( info_start, info_max_size,
               m_info_data.r_string( ss_info_secion, ss_player_digest_key ) );
    // xr_strcat(info_start, info_max_size, m_info_data.r_string(ss_info_secion,
    // ss_admin_name_key));
    ...
    m_info_data.w_string( ss_info_secion, ss_digital_sign_key,
                          tmp_sign_res.c_str() );

    CMemoryWriter tmp_writer;
    m_info_data.save_as( tmp_writer );
    CopyMemory( m_buffer + m_buffer_info_pos, tmp_writer.pointer(),
                tmp_writer.size() );
