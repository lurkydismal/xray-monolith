...
void CWeaponBinoculars::Load( LPCSTR section ) {
    ...
    // Sounds
    m_sounds.LoadSound( section, "snd_zoomin", "sndZoomIn", false,
                        SOUND_TYPE_ITEM_USING );
    m_sounds.LoadSound( section, "snd_zoomout", "sndZoomOut", false,
                        SOUND_TYPE_ITEM_USING );
...
void CWeaponBinoculars::OnZoomIn() {
    if ( H_Parent() && !IsZoomed() ) {
        ...
        m_sounds.PlaySound( "sndZoomIn", H_Parent()->Position(), H_Parent(),
                            b_hud_mode );
...
void CWeaponBinoculars::OnZoomOut() {
    if ( H_Parent() && IsZoomed() && !IsRotatingToZoom() ) {
        ...
        bool b_hud_mode = ( Level().CurrentEntity() == H_Parent() );
        m_sounds.PlaySound( "sndZoomOut", H_Parent()->Position(), H_Parent(),
                            b_hud_mode );
        VERIFY( m_binoc_vision );
        xr_delete( m_binoc_vision );
    }
...
bool CWeaponBinoculars::render_item_ui_query() {
    ...
    return b_is_active_item && H_Parent() && IsZoomed() && ZoomTexture() &&
           !IsRotatingToZoom() && m_binoc_vision;
}
...
// demonized: new zoom delta change to have same multiple between steps for same
// visual change with each step
...
void CWeaponBinoculars::ZoomInc() {
    if ( binoculars_dynamic_zoom_check && !m_zoom_params.m_bUseDynamicZoom )
        return;

    float delta, min_zoom_factor;
    if ( zoomFlags.test( NEW_ZOOM ) ) {
        newGetZoomData( m_zoom_params.m_fScopeZoomFactor,
                        m_zoom_params.m_fZoomStepCount, delta, min_zoom_factor,
                        czoom );
    } else {
        GetZoomData(
            m_zoom_params.m_fScopeZoomFactor, m_zoom_params.m_fZoomStepCount,
            m_zoom_params.m_fMinBaseZoomFactor, delta, min_zoom_factor );
    }

    float f = useNewZoomDeltaAlgorithm ? GetZoomFactor() * delta
                                       : GetZoomFactor() - delta;
...
void CWeaponBinoculars::ZoomDec() {
    ...
    float f = useNewZoomDeltaAlgorithm
                  ? GetZoomFactor() / std::max( delta, 0.001f )
                  : GetZoomFactor() + delta;
    clamp( f, m_zoom_params.m_fScopeZoomFactor, min_zoom_factor );
    SetZoomFactor( f );
    czoom = f;
