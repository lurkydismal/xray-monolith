...
void CHangingLamp::Center( Fvector& C ) const {
    if ( renderable.visual ) {
        renderable.xform.transform_tiny(
            C, renderable.visual->getVisData().sphere.P );
    } else {
        C.set( XFORM().c );
    }
}
...
float CHangingLamp::Radius() const {
    return ( renderable.visual ) ? renderable.visual->getVisData().sphere.R
                                 : EPS;
}
...
BOOL CHangingLamp::net_Spawn( CSE_Abstract* DC ) {
    ...
    CSE_ALifeObjectHangingLamp* lamp =
        smart_cast< CSE_ALifeObjectHangingLamp* >( e );
    R_ASSERT( lamp );
    ...
    light_render->set_shadow( READ_IF_EXISTS(
        pSettings, r_bool, cNameSect(), "shadow",
        !!lamp->flags.is( CSE_ALifeObjectHangingLamp::flCastShadow ) ) );
    light_render->set_volumetric( READ_IF_EXISTS(
        pSettings, r_bool, cNameSect(), "volumetric",
        !!lamp->flags.is( CSE_ALifeObjectHangingLamp::flVolumetric ) ) );
    light_render->set_type(
        lamp->flags.is( CSE_ALifeObjectHangingLamp::flTypeSpot )
            ? IRender_Light::SPOT
            : IRender_Light::POINT );
    ...
    if ( lamp->flags.is( CSE_ALifeObjectHangingLamp::flPointAmbient ) ) {
        ...
        light_ambient->set_shadow( READ_IF_EXISTS(
            pSettings, r_bool, cNameSect(), "ambient_shadow", false ) );
        light_ambient->set_volumetric( READ_IF_EXISTS(
            pSettings, r_bool, cNameSect(), "ambient_volumetric", false ) );
    ...
    if ( lamp->flags.is( CSE_ALifeObjectHangingLamp::flPhysic ) ) {
        if ( !Visual() )
            Msg( "! WARNING: lamp, obj name [%s],flag physics set, but has no "
                 "visual",
                 *cName() );
        NeedUpdate = true;
    }
...
void CHangingLamp::SpawnInitPhysics( CSE_Abstract* D ) {
    CSE_ALifeObjectHangingLamp* lamp =
        smart_cast< CSE_ALifeObjectHangingLamp* >( D );
    if ( lamp->flags.is( CSE_ALifeObjectHangingLamp::flPhysic ) )
        CreateBody( lamp );
    if ( smart_cast< IKinematics* >( Visual() ) ) {
        smart_cast< IKinematics* >( Visual() )->CalculateBones_Invalidate();
    ...
    if ( Alive() ) {
        if ( NeedUpdate ) {
            ...
            if ( light_bone != BI_NONE ) {
                Fmatrix& M = smart_cast< IKinematics* >( Visual() )
                                 ->LL_GetTransform_safed( light_bone );
                xf.mul( XFORM(), M );
                if ( fis_zero( DET( xf ) ) )
                    xf = XFORM();
            } else {
                    ...
                    if ( ambient_bone != BI_NONE ) {
                        Fmatrix& M =
                            smart_cast< IKinematics* >( Visual() )
                                ->LL_GetTransform_safed( ambient_bone );
                        xf.mul( XFORM(), M );
        ...
        if ( lanim ) {
            int frame;
            u32 clr = lanim->CalculateBGR( Device.fTimeGlobal,
...
void CHangingLamp::SetLanim( LPCSTR name,
                             bool bFlicker,
                             int flickerChance,
                             float flickerDelay,
                             float framerate ) {
...
void CHangingLamp::TurnOff() {
    ...
    if ( Visual() ) {
        ...
        VERIFY2( K->LL_GetBonesVisible() != 0,
                 make_string( "can not Turn Off lamp: %s, visual %s - because "
                              "all bones become invisible",
...
void CHangingLamp::Hit( SHit* pHDS ) {
    ...
    callback( GameObject::eHit )(
        lua_game_object(), HDS.power, HDS.dir,
        smart_cast< const CGameObject* >( HDS.who )->lua_game_object(),
        HDS.bone() );
    BOOL bWasAlive = Alive();

    if ( m_pPhysicsShell )
        m_pPhysicsShell->applyHit( pHDS->p_in_bone_space, pHDS->dir,
                                   pHDS->impulse, pHDS->boneID,
                                   pHDS->hit_type );
...
void CHangingLamp::CreateBody( CSE_ALifeObjectHangingLamp* lamp ) {
    ...
    if ( fixed_bones ) {
        ...
        bone_map.insert(
            mk_pair( pKinematics->LL_GetBoneRoot(), physicsBone() ) );
    }
    ...
    disable_params.Load(
        smart_cast< IKinematics* >( Visual() )->LL_UserData() );
    m_pPhysicsShell->set_DisableParams( disable_params );
    ApplySpawnIniToPhysicShell( &lamp->spawn_ini(), m_pPhysicsShell,
                                fixed_bones[ 0 ] != '\0' );
}
