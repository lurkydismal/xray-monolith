...
#define WIND_RADIUS \
    ( 4 * Radius() ) // расстояние до актера, когда появляется ветер
#define FASTMODE_DISTANCE \
    ( 100.f ) // distance to camera from sphere, when zone switches to fast
              // update sequence
...
void CCustomZone::Load( LPCSTR section ) {
    ...
    m_eHitTypeBlowout =
        ALife::g_tfString2HitType( pSettings->r_string( section, "hit_type" ) );

    m_zone_flags.set( eIgnoreNonAlive,
                      pSettings->r_bool( section, "ignore_nonalive" ) );
    m_zone_flags.set( eIgnoreSmall,
                      pSettings->r_bool( section, "ignore_small" ) );
    m_zone_flags.set( eIgnoreArtefact,
                      pSettings->r_bool( section, "ignore_artefacts" ) );

    // загрузить времена для зоны
    m_StateTime[ eZoneStateIdle ] = -1;
    m_StateTime[ eZoneStateAwaking ] =
        pSettings->r_s32( section, "awaking_time" );
    m_StateTime[ eZoneStateBlowout ] =
        pSettings->r_s32( section, "blowout_time" );
    m_StateTime[ eZoneStateAccumulate ] =
        pSettings->r_s32( section, "accamulate_time" );
    ...
    if ( pSettings->line_exist( section, "blowout_particles" ) )
        m_sBlowoutParticles =
            pSettings->r_string( section, "blowout_particles" );
    ...
    if ( pSettings->line_exist( section, "entrance_small_particles" ) )
        m_sEntranceParticlesSmall =
            pSettings->r_string( section, "entrance_small_particles" );
    if ( pSettings->line_exist( section, "entrance_big_particles" ) )
        m_sEntranceParticlesBig =
            pSettings->r_string( section, "entrance_big_particles" );

    if ( pSettings->line_exist( section, "hit_small_particles" ) )
        m_sHitParticlesSmall =
            pSettings->r_string( section, "hit_small_particles" );
    if ( pSettings->line_exist( section, "hit_big_particles" ) )
        m_sHitParticlesBig =
            pSettings->r_string( section, "hit_big_particles" );

    if ( pSettings->line_exist( section, "idle_small_particles" ) )
        m_sIdleObjectParticlesBig =
            pSettings->r_string( section, "idle_big_particles" );

    if ( pSettings->line_exist( section, "idle_big_particles" ) )
        m_sIdleObjectParticlesSmall =
            pSettings->r_string( section, "idle_small_particles" );

    if ( pSettings->line_exist( section, "idle_particles_dont_stop" ) )
        m_zone_flags.set(
            eIdleObjectParticlesDontStop,
    ...
    if ( pSettings->line_exist( section, "bolt_entrance_particles" ) ) {
        ...
        m_zone_flags.set( eBoltEntranceParticles,
                          ( m_sBoltEntranceParticles.size() != 0 ) );
    }

    if ( pSettings->line_exist( section, "blowout_particles_time" ) ) {
        m_dwBlowoutParticlesTime =
            pSettings->r_u32( section, "blowout_particles_time" );
        if ( s32( m_dwBlowoutParticlesTime ) >
             m_StateTime[ eZoneStateBlowout ] ) {
    ...
    if ( pSettings->line_exist( section, "blowout_light_time" ) ) {
        m_dwBlowoutLightTime =
            pSettings->r_u32( section, "blowout_light_time" );
    ...
    if ( pSettings->line_exist( section, "blowout_sound_time" ) ) {
        m_dwBlowoutSoundTime =
            pSettings->r_u32( section, "blowout_sound_time" );
    ...
    if ( pSettings->line_exist( section, "blowout_explosion_time" ) ) {
        m_dwBlowoutExplosionTime =
            pSettings->r_u32( section, "blowout_explosion_time" );
        if ( s32( m_dwBlowoutExplosionTime ) >
             m_StateTime[ eZoneStateBlowout ] ) {
    ...
    m_zone_flags.set( eBlowoutWind,
                      pSettings->r_bool( section, "blowout_wind" ) );
    if ( m_zone_flags.test( eBlowoutWind ) ) {
        m_dwBlowoutWindTimeStart =
            pSettings->r_u32( section, "blowout_wind_time_start" );
        m_dwBlowoutWindTimePeak =
            pSettings->r_u32( section, "blowout_wind_time_peak" );
        m_dwBlowoutWindTimeEnd =
            pSettings->r_u32( section, "blowout_wind_time_end" );
        R_ASSERT( m_dwBlowoutWindTimeStart < m_dwBlowoutWindTimePeak );
        R_ASSERT( m_dwBlowoutWindTimePeak < m_dwBlowoutWindTimeEnd );

        if ( ( s32 )m_dwBlowoutWindTimeEnd <
             m_StateTime[ eZoneStateBlowout ] ) {
            m_dwBlowoutWindTimeEnd =
                u32( m_StateTime[ eZoneStateBlowout ] - 1 );
#ifndef MASTER_GOLD
        ...
        m_fBlowoutWindPowerMax =
            pSettings->r_float( section, "blowout_wind_power" );
    }
    ...
    if ( pSettings->line_exist( section, "blowout_disable_idle" ) ) {
        m_zone_flags.set(
            eBlowoutDisableIdle,
            pSettings->r_bool( section, "blowout_disable_idle" ) );
    }

    // загрузить параметры световой вспышки от взрыва
    m_zone_flags.set( eBlowoutLight,
                      pSettings->r_bool( section, "blowout_light" ) );
    if ( m_zone_flags.test( eBlowoutLight ) ) {
        ...
        m_fLightTimeLeft = 0;
        LPCSTR light_anim = ( READ_IF_EXISTS( pSettings, r_string, section,
                                              "blowout_light_anim", ( 0 ) ) );
        if ( light_anim )
    ...
    if ( m_zone_flags.test( eIdleLight ) ) {
        ...
        m_zone_flags.set(
            eIdleLightVolumetric,
            pSettings->r_bool( section, "idle_light_volumetric" ) );
        volumetric_distance =
            READ_IF_EXISTS( pSettings, r_float, section,
                            "idle_light_volumetric_distance", .5f );
        volumetric_intensity =
            READ_IF_EXISTS( pSettings, r_float, section,
                            "idle_light_volumetric_intensity", .5f );
        volumetric_quality = READ_IF_EXISTS(
            pSettings, r_float, section, "idle_light_volumetric_quality", .5f );

        m_zone_flags.set( eIdleLightShadow,
                          pSettings->r_bool( section, "idle_light_shadow" ) );
        m_zone_flags.set( eIdleLightR1,
                          pSettings->r_bool( section, "idle_light_r1" ) );
    }

    bool use = !!READ_IF_EXISTS( pSettings, r_bool, section,
                                 "use_secondary_hit", false );
    m_zone_flags.set( eUseSecondaryHit, use );
    if ( use )
        m_fSecondaryHitPower =
            pSettings->r_float( section, "secondary_hit_power" );

    m_ef_anomaly_type = pSettings->r_u32( section, "ef_anomaly_type" );
    m_ef_weapon_type = pSettings->r_u32( section, "ef_weapon_type" );

    m_zone_flags.set( eAffectPickDOF,
                      pSettings->r_bool( section, "pick_dof_effector" ) );
}
...
BOOL CCustomZone::net_Spawn( CSE_Abstract* DC ) {
    ...
    m_zone_flags.set( eUseOnOffTime,
                      ( m_TimeToDisable != 0 ) && ( m_TimeToEnable != 0 ) );

    // добавить источники света
    bool br1 = ( 0 == psDeviceFlags.test( rsR2 | rsR3 |
                                          rsR4 ) ); // Alundaio: rsR4 flag

    bool render_ver_allowed =
        !br1 || ( br1 && m_zone_flags.test( eIdleLightR1 ) );
    ...
    if ( spawn_ini() &&
         spawn_ini()->line_exist( "fast_mode", "always_fast" ) ) {
        m_zone_flags.set( eAlwaysFastmode,
                          spawn_ini()->r_bool( "fast_mode", "always_fast" ) );
    }
    return ( TRUE );
}
...
void CCustomZone::UpdateWorkload( u32 dt ) {
    ...
    if ( m_zone_flags.test( eUseSecondaryHit ) &&
         m_eZoneState != eZoneStateIdle &&
...
void CCustomZone::shedule_Update( u32 dt ) {
    ...
    if ( IsEnabled() ) {
        ...
        for ( OBJECT_INFO_VEC_IT it = m_ObjectInfoMap.begin();
            ...
            if ( ( !info.small_object && m_iDisableHitTime != -1 &&
                   ( int )info.dw_time_in_zone > m_iDisableHitTime ) ||
                 ( info.small_object && m_iDisableHitTimeSmall != -1 &&
                   ( int )info.dw_time_in_zone > m_iDisableHitTimeSmall ) ) {
                if ( !pEntityAlive || !pEntityAlive->g_Alive() )
                    info.zone_ignore = true;
            }
            if ( m_iDisableIdleTime != -1 &&
                 ( int )info.dw_time_in_zone > m_iDisableIdleTime ) {
                if ( !pEntityAlive || !pEntityAlive->g_Alive() )
            ...
            if ( pEntityAlive ) {
                ...
                if ( ai().script_engine().functor(
                         "_G.CCustomZone_BeforeActivateCallback", funct ) )
                    info.zone_ignore = !funct( this->lua_game_object(),
                                               pObject->lua_game_object() );
            }
        ...
        float act_distance =
            Level().CurrentControlEntity()->Position().distance_to( P ) - s.R;
        if ( act_distance > FASTMODE_DISTANCE &&
             !m_zone_flags.test( eAlwaysFastmode ) )
            o_switch_2_slow();
        else
            o_switch_2_fast();
        ...
        if ( act_distance < ps_ssfx_int_grass_params_1.w )
            ...
            if ( grassbender_id ) {
                IGame_Persistent::grass_data& GData =
                    g_pGamePersistent->grass_shader_data;

                // If the ID doesn't match... Just remove the grassbender_id.
                if ( GData.id[ grassbender_id ] == ID() ) {
                    GData.str_target[ grassbender_id ] +=
                        g_pGamePersistent->GrassBenderToValue(
                            GData.str_target[ grassbender_id ], 0.0f, 4.0f,
                            false );

                    // Remove ( Don't worry, GrassBenderToValue() it's going to
                    // get the == 0 )
                    if ( GData.str_target[ grassbender_id ] == 0 )
                        g_pGamePersistent->GrassBendersRemoveByIndex(
                            grassbender_id );
                } else {
                    grassbender_id = NULL;
                }
...
void CCustomZone::feel_touch_new( CObject* O ) {
    ...
    if ( ( object_info.small_object && m_zone_flags.test( eIgnoreSmall ) ) ||
         ( object_info.nonalive_object &&
           m_zone_flags.test( eIgnoreNonAlive ) ) ||
...
void CCustomZone::feel_touch_delete( CObject* O ) {
    ...
    OBJECT_INFO_VEC_IT it = std::find( m_ObjectInfoMap.begin(),
                                       m_ObjectInfoMap.end(), pGameObject );
...
float CCustomZone::RelativePower( float dist, float nearest_shape_radius ) {
    ...
    float power =
        ( radius < dist )
            ? 0
            : ( 1.f - m_fAttenuation * ( dist / radius ) * ( dist / radius ) );
...
void CCustomZone::PlayIdleParticles( bool bIdleLight ) {
    ...
    if ( *m_sIdleParticles ) {
        if ( !m_pIdleParticles ) {
            m_pIdleParticles =
                Particles::Details::Create( m_sIdleParticles.c_str(), FALSE );
...
void CCustomZone::UpdateIdleLight() {
    ...
    int frame = 0;
    u32 clr = m_pIdleLAnim->CalculateBGR( Device.fTimeGlobal,
                                          frame ); // возвращает в формате BGR
    Fcolor fclr;
    fclr.set( ( float )color_get_B( clr ) / 255.f,
              ( float )color_get_G( clr ) / 255.f,
              ( float )color_get_R( clr ) / 255.f, 1.f );
...
void CCustomZone::PlayBlowoutParticles() {
    ...
    intrusive_ptr< CParticlesObject > pParticles =
        Particles::Details::Create( *m_sBlowoutParticles, TRUE );
    pParticles->UpdateParent( XFORM(), zero_vel );
    pParticles->Play( false );

    m_fBlowoutTimeLeft =
        ( float )Device.dwTimeGlobal + m_BendGrass_Blowout_time;
}
    ...
    if ( particle_str.size() ) {
        ...
        if ( PP ) {
            ...
            if ( play_bone != BI_NONE )
                PP->StartParticles( particle_str, play_bone,
...
void CCustomZone::PlayEntranceParticles( CGameObject* pObject ) {
    ...
    Fvector vel;
    CPhysicsShellHolder* shell_holder =
        smart_cast< CPhysicsShellHolder* >( pObject );
    if ( shell_holder )
    ...
    if ( PP ) {
        ...
        if ( play_bone != BI_NONE ) {
            intrusive_ptr< CParticlesObject > pParticles =
                Particles::Details::Create( particle_str, TRUE );
            Fmatrix xform;
            Fvector dir;
            ...
            PP->MakeXFORM( pObject, play_bone, dir, Fvector().set( 0, 0, 0 ),
                           xform );
            pParticles->UpdateParent( xform, vel );
            pParticles->Play( false );
        }
    }
    if ( m_zone_flags.test( eBoltEntranceParticles ) &&
         smart_cast< CBolt* >( pObject ) )
        ...
        switch ( s.type ) {
            case 0: // sphere
            {
                ...
                for ( float i = 0; i < ki; ++i ) {
                    ...
                    pParticles = Particles::Details::Create(
                        m_sBoltEntranceParticles.c_str(), TRUE );
...
void CCustomZone::PlayBulletParticles( Fvector& pos ) {
    ...
    intrusive_ptr< CParticlesObject > pParticles =
        Particles::Details::Create( *m_sEntranceParticlesSmall, TRUE );
...
void CCustomZone::StopObjectIdleParticles( CGameObject* pObject ) {
    if ( m_zone_flags.test( eIdleObjectParticlesDontStop ) &&
         !pObject->cast_actor() )
        return;
    ...
    OBJECT_INFO_VEC_IT it =
        std::find( m_ObjectInfoMap.begin(), m_ObjectInfoMap.end(), pObject );
...
void CCustomZone::UpdateBlowoutLight() {
    ...
    if ( m_fLightTimeLeft > ( float )Device.dwTimeGlobal ) {
        ...
        if ( m_pBlowLAnim ) {
            ...
            m_pLight->set_color( m_LightColor.r * scale, m_LightColor.g * scale,
...
void CCustomZone::UpdateBlowout() {
    ...
    if ( m_zone_flags.test( eBlowoutWind ) &&
         m_dwBlowoutWindTimeStart >= ( u32 )m_iPreviousStateTime &&
    ...
    if ( m_dwBlowoutExplosionTime >= ( u32 )m_iPreviousStateTime &&
        ...
        if ( m_BendGrass_Blowout )
            g_pGamePersistent->GrassBendersAddExplosion(
                ID(), Position(), Fvector().set( 0, -99, 0 ), 1.33f,
                m_BendGrass_Blowout_speed, 1.0f, m_BendGrass_Blowout_radius );
    }
}
...
void CCustomZone::OnMove() {
    if ( m_dwLastTimeMoved == 0 ) {
        ...
        float time_delta =
            float( Device.dwTimeGlobal - m_dwLastTimeMoved ) / 1000.f;
        m_dwLastTimeMoved = Device.dwTimeGlobal;
        ...
        if ( grassbender_id ) {
            ...
            if ( g_pGamePersistent->grass_shader_data.id[ grassbender_id ] ==
                 ID() )
                g_pGamePersistent->grass_shader_data.pos[ grassbender_id ] =
                    Position();
        }
    }
}
...
void CCustomZone::UpdateWind() {
    ...
    if ( m_fDistanceToCurEntity > WIND_RADIUS ||
         m_dwBlowoutWindTimeEnd < ( u32 )m_iStateTime ) {
        StopWind();
        return;
    }
    ...
    if ( m_dwBlowoutWindTimePeak > ( u32 )m_iStateTime ) {
        g_pGamePersistent->Environment().wind_strength_factor =
            m_fBlowoutWindPowerMax +
            ( m_fStoreWindPower - m_fBlowoutWindPowerMax ) *
                float( m_dwBlowoutWindTimePeak - ( u32 )m_iStateTime ) /
                float( m_dwBlowoutWindTimePeak - m_dwBlowoutWindTimeStart );
        clamp( g_pGamePersistent->Environment().wind_strength_factor, 0.f,
               1.f );
    } else {
        g_pGamePersistent->Environment().wind_strength_factor =
            m_fBlowoutWindPowerMax +
...
void CCustomZone::CreateHit( u16 id_to,
    ...
    if ( OnServer() ) {
        ...
        SHit Hit = SHit( hit_power, hdir, this, bone_id, pos_in_bone,
                         hit_impulse, hit_type, 0.0f, false );
...
void CCustomZone::net_Relcase( CObject* O ) {
    ...
    OBJECT_INFO_VEC_IT it =
        std::find( m_ObjectInfoMap.begin(), m_ObjectInfoMap.end(), GO );
    if ( it != m_ObjectInfoMap.end() ) {
        exit_Zone( *it );
        m_ObjectInfoMap.erase_fast( it );
    }
    ...
    if ( m_actor_effector && m_actor_effector->m_pActor &&
         m_actor_effector->m_pActor->ID() == GO->ID() )
        m_actor_effector->Stop();
...
void CCustomZone::PlayAccumParticles() {
    ...
    if ( m_sAccumParticles.size() ) {
        intrusive_ptr< CParticlesObject > pParticles =
            Particles::Details::Create( *m_sAccumParticles, TRUE );
        pParticles->UpdateParent( XFORM(), zero_vel );
        pParticles->Play( false );
    }
...
void CCustomZone::PlayAwakingParticles() {
    ...
    if ( m_sAwakingParticles.size() ) {
        intrusive_ptr< CParticlesObject > pParticles =
            Particles::Details::Create( *m_sAwakingParticles, TRUE );
        pParticles->UpdateParent( XFORM(), zero_vel );
        pParticles->Play( false );
    }
...
void CCustomZone::UpdateOnOffState() {
    ...
    bool dest_state;
    u32 t = ( Device.dwTimeGlobal - m_StartTime + m_TimeShift ) %
            ( m_TimeToEnable + m_TimeToDisable );
    if ( t < m_TimeToEnable ) {
        dest_state = true;
...
bool CCustomZone::feel_touch_on_contact( CObject* O ) {
    if ( ( SpatialComponent->spatial.type | STYPE_VISIBLEFORAI ) !=
         SpatialComponent->spatial.type )
        return ( false );
...
BOOL CCustomZone::AlwaysTheCrow() {
    bool b_idle =
        ZoneState() == eZoneStateIdle || ZoneState() == eZoneStateDisabled;
...
void CCustomZone::CalcDistanceTo( const Fvector& P,
                                  float& dist,
                                  float& radius ) {
...
void CCustomZone::load( IReader& input_packet ) {
    ...
    CCustomZone::EZoneState temp =
        static_cast< CCustomZone::EZoneState >( input_packet.r_u8() );
...
void CCustomZone::GrassZoneUpdate() {
    ...
    // If m_BendGrass_Blowout_time is not set, use m_eZoneState to detect
    // activation
    ...
    if ( IsActive )
        targetAnim = ( m_BendGrass_whenactive_anim > -1 )
                         ? m_BendGrass_whenactive_anim
                         : m_BendGrass_idle_anim;
    else
    ...
    if ( targetAnim > 0 ||
         ( grassbender_id > 0 && GData.anim[ grassbender_id ] > 0 ) )
        g_pGamePersistent->GrassBendersUpdate(
            ID(), grassbender_id, grassbender_frame, Position(),
            m_BendGrass_idle_radius, 0.0f, false );
    else
        g_pGamePersistent->GrassBendersRemoveByIndex( grassbender_id );
    ...
    if ( GData.anim[ grassbender_id ] != targetAnim ) {
        GData.str_target[ grassbender_id ] +=
            g_pGamePersistent->GrassBenderToValue(
                GData.str_target[ grassbender_id ], 0.0f, 7.5f, false );
    ...
    if ( IsActive ) {
        if ( m_BendGrass_whenactive_speed >= 0 )
            GData.speed[ grassbender_id ] +=
                g_pGamePersistent->GrassBenderToValue(
                    GData.speed[ grassbender_id ], m_BendGrass_whenactive_speed,
                    10.0f, true );
        ...
        if ( m_BendGrass_whenactive_str >= 0 )
            GData.str_target[ grassbender_id ] +=
                g_pGamePersistent->GrassBenderToValue(
                    GData.str_target[ grassbender_id ],
                    m_BendGrass_whenactive_str, 10.0f, true );
    } else {
        GData.speed[ grassbender_id ] += g_pGamePersistent->GrassBenderToValue(
            GData.speed[ grassbender_id ], m_BendGrass_idle_speed, 10.0f,
            true );
        GData.str_target[ grassbender_id ] +=
            g_pGamePersistent->GrassBenderToValue(
                GData.str_target[ grassbender_id ], m_BendGrass_idle_str, 10.0f,
                true );
    }
}
