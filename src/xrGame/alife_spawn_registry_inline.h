...
IC void CALifeSpawnRegistry::assign_artefact_position(
    CSE_ALifeAnomalousZone* anomaly,
    CSE_ALifeDynamicObject* object ) const {
    object->m_tGraphID = anomaly->m_tGraphID;
    VERIFY3( anomaly->m_artefact_spawn_count,
             "Anomaly is outside of the AI-map but is used for artefact "
             "generation : ",
             anomaly->name_replace() );
    u32 index = anomaly->m_artefact_position_offset +
                anomaly->randI( anomaly->m_artefact_spawn_count );
    ...
    if ( psAI_Flags.test( aiALife ) ) {
        Msg( "[LSS] Zone %s[%f][%f][%f] %d: generated artefact position "
             "%s[%f][%f][%f]",
             anomaly->name_replace(), VPUSH( anomaly->o_Position ),
             anomaly->m_artefact_position_offset, object->name_replace(),
...
IC const ALife::_SPAWN_ID& CALifeSpawnRegistry::spawn_id(
    const ALife::_SPAWN_STORY_ID& spawn_story_id ) const {
    SPAWN_STORY_IDS::const_iterator I =
        m_spawn_story_ids.find( spawn_story_id );
