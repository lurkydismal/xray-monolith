...
u8 game_sv_TeamDeathmatch::AutoTeam() {
    ...
    return ( team_calculator.m_teams[ 0 ] > team_calculator.m_teams[ 1 ] ) ? 2
                                                                           : 1;
}
...
void game_sv_TeamDeathmatch::AutoBalanceTeams() {
    ...
    NumToMove =
        ( tmp_functor.l_teams[ MaxTeam ] - tmp_functor.l_teams[ MinTeam ] ) / 2;
    if ( !NumToMove )
        return;
    ///////////////////////////////////////////////////////////////////////
    while ( NumToMove ) {
        ...
        R_ASSERT( autob_functor.lowest_player &&
                  autob_functor.lowest_player->ps );
...
void game_sv_TeamDeathmatch::OnPlayerSelectTeam( NET_Packet& P,
                                                 ClientID sender ) {
...
void game_sv_TeamDeathmatch::UpdateTeamScore( game_PlayerState* ps_killer,
    ...
    if ( !ps_killer )
        return;
    SetTeamScore( ps_killer->team - 1, GetTeamScore( ps_killer->team - 1 ) +
                                           ps_killer->frags() - OldKills );
}

KILL_RES game_sv_TeamDeathmatch::GetKillResult( game_PlayerState* pKiller,
...
bool game_sv_TeamDeathmatch::OnKillResult( KILL_RES KillResult,
...
bool game_sv_TeamDeathmatch::checkForFragLimit() {
    if ( g_sv_dm_dwFragLimit &&
         ( ( teams[ 0 ].score >= g_sv_dm_dwFragLimit ) ||
...
void game_sv_TeamDeathmatch::OnPlayerHitPlayer( u16 id_hitter,
                                                u16 id_hitted,
                                                NET_Packet& P ) {
...
void game_sv_TeamDeathmatch::WriteGameState( CInifile& ini,
                                             LPCSTR sect,
                                             bool bRoundResult ) {
...
BOOL game_sv_TeamDeathmatch::OnTouchItem( CSE_ActorMP* actor,
                                          CSE_Abstract* item ) {
    VERIFY( actor );
    VERIFY( item );
...
void game_sv_TeamDeathmatch::OnDetachItem( CSE_ActorMP* actor,
                                           CSE_Abstract* item ) {
    R_ASSERT( actor );
    R_ASSERT( item );
    if ( item->m_tClassID == CLSID_OBJECT_PLAYERS_BAG ) {
...
BOOL game_sv_TeamDeathmatch::OnTouch( u16 eid_who,
                                      u16 eid_what,
                                      BOOL bForced ) {
    CSE_ActorMP* e_who =
...
void game_sv_TeamDeathmatch::RespawnPlayer( ClientID id_who,
                                            bool NoSpectator ) {
