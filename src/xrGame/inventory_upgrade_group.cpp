...
void Group::construct( const shared_str& group_id,
    ...
    VERIFY2( pSettings->section_exist( m_id ),
             make_string( "Upgrade <%s> : group section [%s] does not exist!",
                          parent_upgrade.id_str(), m_id.c_str() ) );

    LPCSTR upgrades_str = pSettings->r_string( m_id, "elements" );
    VERIFY2( upgrades_str,
             make_string( "in upgrade group <%s> elements are empty!",
                          m_id.c_str() ) );
...
void Group::add_parent_upgrade( UpgradeBase& parent_upgrade ) {
    if ( std::find( m_parent_upgrades.begin(), m_parent_upgrades.end(),
                    &parent_upgrade ) == m_parent_upgrades.end() ) {
...
UpgradeStateResult Group::can_install( CInventoryItem& item,
                                       UpgradeBase& test_upgrade,
                                       bool loading ) {
    ...
    for ( ; ib != ie; ++ib ) {
        ...
        if ( !item.has_upgrade_group( U->parent_group_id() ) ) {
            if ( loading ) {
                FATAL(
                    make_string(
                        "Loading item: Upgrade <%s> of inventory item [%s] (id "
                        "= %d) can`t be installed! Error = result_e_parents",
                        test_upgrade.id_str(), item.m_section_id.c_str(),
                        item.object_id() )
                        .c_str() );
            }
        ...
        if ( item.has_upgrade( ( *ib )->id() ) ) {
            if ( loading ) {
                FATAL(
                    make_string(
                        "Loading item: Upgrade <%s> of inventory item [%s] (id "
                        "= %d) can`t be installed! Error = result_e_group",
                        test_upgrade.id_str(), item.m_section_id.c_str(),
                        item.object_id() )
                        .c_str() );
            }
