...
cover::cover( smart_cover::object const& object,
    ...
    for ( ; J != K; ++J, ++i ) {
        ...
        VERIFY2(
            ...
            make_string( "invalid vertex id: smart cover[%s], loophole [%s]",
...
void cover::vertex( smart_cover::loophole const& loophole,
                    smart_cover::loophole_data& loophole_data ) {
    CLevelGraph const& graph = ai().level_graph();
    ...
    VERIFY2( graph.valid_vertex_id( loophole_data.m_level_vertex_id ),
             make_string( "invalid vertex id: smart cover[%s], loophole [%s]",
                          m_object.cName().c_str(), loophole.id().c_str() ) );
    ...
    for ( ; I != E; ++I )
        if ( ( *I ).second->movement() ) {
            ...
            VERIFY2( graph.valid_vertex_id( level_vertex_id ),
                     make_string( "invalid vertex id: loophole [%s]",
                                  loophole.id().c_str() ) );
            loophole_data.m_action_vertices.push_back(
                std::make_pair( ( *I ).first, level_vertex_id ) );
        }
}
...
public:
    ...
    IC bool operator()(
        smart_cover::loophole_data::Action const& action ) const {
        return ( m_id._get() == action.first._get() );
    }
};

u32 const& cover::action_level_vertex_id( smart_cover::loophole const& loophole,
                                          shared_str const& action_id ) const {
    Vertices::const_iterator found = std::find_if(
        m_vertices.begin(), m_vertices.end(), id_predicate( loophole.id() ) );
    ...
    loophole_data::ActionVertices::const_iterator found2 = std::find_if(
        ...
        found->second.m_action_vertices.end(), id_predicate( action_id ) );
...
smart_cover::loophole* cover::best_loophole(
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( use_default_behaviour )
            evaluate_loophole_for_default_usage( position, loophole, result,
                                                 value );
        else
            evaluate_loophole( position, loophole, result, value,
                               is_smart_cover_entered );
    }

    return ( result );
}

void cover::evaluate_loophole( Fvector const& position,
                               smart_cover::loophole*& source,
                               smart_cover::loophole*& result,
                               float& value,
                               bool const is_smart_cover_entered ) const {
    VERIFY( source );
    VERIFY2( _valid( position ),
             make_string( "[%f][%f][%f]", VPUSH( position ) ) );

    if ( !source->usable() )
        return;

    Fvector fov_position = this->fov_position( *source );
    VERIFY2( _valid( fov_position ),
             make_string( "[%f][%f][%f]", VPUSH( fov_position ) ) );
    ...
    VERIFY2( _valid( direction ),
             make_string( "[%f][%f][%f]", VPUSH( direction ) ) );
...
void cover::evaluate_loophole_for_default_usage( Fvector const& position,
                                                 smart_cover::loophole*& source,
                                                 smart_cover::loophole*& result,
                                                 float& value ) const {
    VERIFY( source );
...
u32 const& cover::level_vertex_id(
    smart_cover::loophole const& loophole ) const {
    Vertices::const_iterator I = std::find_if(
        m_vertices.begin(), m_vertices.end(), loophole_predicate( &loophole ) );
    VERIFY( I != m_vertices.end() );
    return ( ( *I ).second.m_level_vertex_id );
}

#ifdef DEBUG
bool cover::loophole_path( shared_str const& source_raw,
                           shared_str const& target_raw ) const {
    ...
    bool result = ai().graph_engine().search( m_description->transitions(),
                                              source, target, 0, parameters );

    VERIFY2(
        result,
        make_string( "failde to find loophole path [%s]->[%s] in cover [%s]",
...
void cover::check_loopholes_connectivity() const {
    ...
    for ( ; I != E; ++I ) {
        ...
        for ( ; J != E; ++J ) {
            ...
            VERIFY2( loophole_path( lhs, rhs ),
                     make_string(
                         "failed to find path [%s -> %s] in smart_cover [%s]",
                         lhs.c_str(), rhs.c_str(),
                         m_description->table_id().c_str() ) );
            VERIFY2( loophole_path( rhs, lhs ),
                     make_string(
                         "failed to find path [%s -> %s] in smart_cover [%s]",
                         rhs.c_str(), lhs.c_str(),
                         m_description->table_id().c_str() ) );
        ...
        VERIFY2(
            ...
            make_string( "failed to find path [%s -> %s] in smart_cover [%s]",
                         lhs.c_str(), exit.c_str(),
                         m_description->table_id().c_str() ) );
        VERIFY2(
            loophole_path( enter, lhs ),
            make_string( "failed to find path [%s -> %s] in smart_cover [%s]",
                         enter.c_str(), lhs.c_str(),
                         m_description->table_id().c_str() ) );
    }
}
#endif // DEBUG

static bool in_fov( Fvector const& position,
                    Fvector const& fov_position,
                    Fvector const& fov_direction,
...
bool cover::is_position_in_fov( smart_cover::loophole const& source,
                                Fvector const& position ) const {
    return in_fov( position, fov_position( source ), fov_direction( source ),
                   source.fov() );
}

bool cover::is_position_in_danger_fov( smart_cover::loophole const& source,
                                       Fvector const& position ) const {
    return in_fov( position, fov_position( source ),
                   danger_fov_direction( source ), source.danger_fov() );
}

bool cover::is_position_in_range( smart_cover::loophole const& source,
                                  Fvector const& position ) const {
    Fvector fov_position = this->fov_position( source );
...
bool cover::in_min_acceptable_range( smart_cover::loophole const& source,
                                     Fvector const& position,
