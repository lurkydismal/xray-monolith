...
struct S2DVert {
    ...
    void rotate_pt( const Fvector2& pivot,
                    const float cosA,
                    const float sinA,
                    const float kx );
};
...
class C2DFrustum {
    ...
    Frect m_rect;
    bool m_force_clip; // when true, ClipPoly always runs even if the poly is
                       // fully inside m_rect
public:
    ...
    // Half-plane clip along directed edge a->b: keeps the LEFT side -- points p
    // with (b-a) x (p-a) > 0. n is the outward normal (toward the discarded
    // side);
    ...
    u32 ClipBudget( u32 src_verts ) const {
        return 3 * ( src_verts + ( u32 )planes.size() - 2 );
    }
};
