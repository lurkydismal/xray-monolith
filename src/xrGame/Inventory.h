...
public:
    ...
    // if just_before_destroy is true, then activate will be forced (because
    // deactivate message will not deliver)
    ...
    void Activate(
        u16 slot,
        /*EActivationReason reason=eGeneral, */ bool bForce = false );
    ...
    PIItem ActiveItem() const {
        return ( m_iActiveSlot == NO_ACTIVE_SLOT )
                   ? NULL
