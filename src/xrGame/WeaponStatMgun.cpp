...
#ifdef STATIONARYMGUN_NEW
    camera[ eCamFirst ] = xr_new< CCameraFirstEye >(
...
void CWeaponStatMgun::SetBoneCallbacks() {
    ...
    Visual()
        ->dcast_PKinematics()
        ->LL_GetTransform( m_rotate_y_bone )
        .getHPB( vec );
    m_cur_x_rot = 0.0F;
    m_cur_y_rot = -vec.x;
    ...
    CBoneInstance& biX = smart_cast< IKinematics* >( Visual() )
                             ->LL_GetBoneInstance( m_rotate_x_bone );
    biX.set_callback( bctCustom, BoneCallbackX, this );
    CBoneInstance& biY = smart_cast< IKinematics* >( Visual() )
                             ->LL_GetBoneInstance( m_rotate_y_bone );
...
void CWeaponStatMgun::ResetBoneCallbacks() {
    CBoneInstance& biX = smart_cast< IKinematics* >( Visual() )
                             ->LL_GetBoneInstance( m_rotate_x_bone );
#ifdef STATIONARYMGUN_NEW
    biX.set_callback( bctPhysics, PPhysicsShell()->GetBonesCallback(),
                      PPhysicsShell()->get_Element( m_rotate_x_bone ) );
#else
    biX.reset_callback();
#endif
    ...
    CBoneInstance& biY = smart_cast< IKinematics* >( Visual() )
                             ->LL_GetBoneInstance( m_rotate_y_bone );
#ifdef STATIONARYMGUN_NEW
    biY.set_callback( bctPhysics, PPhysicsShell()->GetBonesCallback(),
                      PPhysicsShell()->get_Element( m_rotate_y_bone ) );
#else
    biY.reset_callback();
#endif
...
void CWeaponStatMgun::Load( LPCSTR section ) {
    ...
    m_overheat_enabled =
        pSettings->line_exist( section, "overheat_enabled" )
            ? !!pSettings->r_bool( section, "overheat_enabled" )
            : false;
    m_overheat_time_quant = READ_IF_EXISTS( pSettings, r_float, section,
                                            "overheat_time_quant", 0.025f );
    m_overheat_decr_quant = READ_IF_EXISTS( pSettings, r_float, section,
                                            "overheat_decr_quant", 0.002f );
    m_overheat_threshold = READ_IF_EXISTS( pSettings, r_float, section,
                                           "overheat_threshold", 110.f );
    m_overheat_particles =
        READ_IF_EXISTS( pSettings, r_string, section, "overheat_particles",
                        "damage_fx\\burn_creatures00" );

    m_bEnterLocked =
        !!READ_IF_EXISTS( pSettings, r_bool, section, "lock_enter", false );
    m_bExitLocked =
        !!READ_IF_EXISTS( pSettings, r_bool, section, "lock_exit", false );

    VERIFY( !fis_zero( camMaxAngle ) );
    VERIFY( !fis_zero( camRelaxSpeed ) );

#ifdef STATIONARYMGUN_NEW
    m_sounds.LoadSound( section, "snd_shoot", "sndShoot", false,
                        SOUND_TYPE_WEAPON_SHOOTING );
    m_sounds.LoadSound( section, "snd_empty", "sndEmpty", false,
                        SOUND_TYPE_WEAPON_EMPTY_CLICKING );
    m_sounds.LoadSound( section, "snd_reload", "sndReload", true,
                        SOUND_TYPE_WEAPON_RECHARGING );

    m_zoom_factor_def =
        READ_IF_EXISTS( pSettings, r_float, section, "zoom_factor_def", 1.0F );
    m_zoom_factor_aim =
        READ_IF_EXISTS( pSettings, r_float, section, "zoom_factor_aim", 1.0F );
    m_reload_delay =
        READ_IF_EXISTS( pSettings, r_float, section, "reload_delay", 0.0F );
    m_single_shot_wpn = !!READ_IF_EXISTS( pSettings, r_bool, section,
                                          "is_single_shot_wpn", FALSE );
    m_unlimited_ammo =
        !!READ_IF_EXISTS( pSettings, r_bool, section, "unlimited_ammo", false );
    m_reload_consume_callback = READ_IF_EXISTS( pSettings, r_string, section,
                                                "reload_consume", nullptr );
    m_shot_effector._set(
        READ_IF_EXISTS( pSettings, r_string, section, "shot_effector", "" ) );
    ...
    iMagazineSize =
        READ_IF_EXISTS( pSettings, r_s32, section, "ammo_mag_size", 1 );
    SetAmmoElapsed( READ_IF_EXISTS( pSettings, r_s32, section, "ammo_elapsed",
                                    iMagazineSize ) );

    fireDispersionOwnerScale = READ_IF_EXISTS(
        pSettings, r_float, section, "fire_dispersion_owner_scale", 1.0F );
    if ( pSettings->line_exist( cNameSect_str(), "on_before_use" ) ) {
        m_on_before_use_callback = READ_IF_EXISTS(
            pSettings, r_string, cNameSect_str(), "on_before_use", "" );
    }
    if ( pSettings->line_exist( cNameSect_str(), "on_range_fov" ) ) {
        m_on_range_fov_callback = READ_IF_EXISTS(
            pSettings, r_string, cNameSect_str(), "on_range_fov", "" );
    }
    ...
    if ( pSettings->line_exist( cNameSect_str(), "barrels" ) ) {
        ...
        if ( iMagazineSize && m_barrels.size() &&
             ( iMagazineSize % m_barrels.size() > 0 ) ) {
            Msg( "ERROR: [%s][%s] the magazine size %d is not divisible to the "
                 "number of barrels %d",
                 cName().c_str(), cNameSect_str(), iMagazineSize,
                 m_barrels.size() );
            FATAL( "" );
        }
...
BOOL CWeaponStatMgun::net_Spawn( CSE_Abstract* DC ) {
    ...
    m_rotate_x_bone = K->LL_BoneID(
        pUserData->r_string( "mounted_weapon_definition", "rotate_x_bone" ) );
    m_rotate_y_bone = K->LL_BoneID(
        pUserData->r_string( "mounted_weapon_definition", "rotate_y_bone" ) );
    m_fire_bone = K->LL_BoneID(
        pUserData->r_string( "mounted_weapon_definition", "fire_bone" ) );
    m_camera_bone = K->LL_BoneID(
        pUserData->r_string( "mounted_weapon_definition", "camera_bone" ) );
    ...
    m_lim_x_rot.set( bdX.IK_data.limits[ 0 ].limit.x,
                     bdX.IK_data.limits[ 0 ].limit.y );
    CBoneData& bdY = K->LL_GetData( m_rotate_y_bone );
    VERIFY( bdY.IK_data.type == jtJoint );
    m_lim_y_rot.set( bdY.IK_data.limits[ 1 ].limit.x,
                     bdY.IK_data.limits[ 1 ].limit.y );
...
#ifdef STATIONARYMGUN_NEW
    /*
    Initial directions are always H = 0 P = 0. Even when the bones were created
    with angled directions. Example: When a bone is created with H = 60 degrees,
    the bone has a H = 60 degrees angle to the model. But it is H = 0 degree to
    the bone itself.
    */
    m_cur_x_rot = 0;
    m_cur_y_rot = 0;
    ...
    m_rotate_x_speed =
        deg2rad( READ_IF_EXISTS( ini, r_float, mwd, "rotate_x_speed", 10.0F ) );
    m_rotate_y_speed =
        deg2rad( READ_IF_EXISTS( ini, r_float, mwd, "rotate_y_speed", 10.0F ) );
    m_drop_bone = ini->line_exist( mwd, "drop_bone" )
                      ? K->LL_BoneID( ini->r_string( mwd, "drop_bone" ) )
                      : BI_NONE;
    m_actor_bone = ini->line_exist( mwd, "actor_bone" )
                       ? K->LL_BoneID( ini->r_string( mwd, "actor_bone" ) )
                       : BI_NONE;

    m_exit_position = READ_IF_EXISTS( ini, r_fvector3, mwd, "exit_position",
                                      Fvector().set( 0, 0, 0 ) );
    m_camera_bone_def =
        ini->line_exist( mwd, "camera_bone_def" )
            ? K->LL_BoneID( ini->r_string( mwd, "camera_bone_def" ) )
            : BI_NONE;
    m_camera_bone_aim =
        ini->line_exist( mwd, "camera_bone_aim" )
            ? K->LL_BoneID( ini->r_string( mwd, "camera_bone_aim" ) )
            : BI_NONE;
    m_camera_position = READ_IF_EXISTS( ini, r_fvector3, mwd, "camera_position",
                                        Fvector().set( 0, 1, 0 ) );
    ...
    if ( ini->section_exist( "animation" ) ) {
        m_animation.SetAnimation( SStmAnimActor::eAnimBody,
                                  READ_IF_EXISTS( ini, r_string, "animation",
                                                  "idle_body", nullptr ) );
        m_animation.SetAnimation( SStmAnimActor::eAnimLegs,
                                  READ_IF_EXISTS( ini, r_string, "animation",
                                                  "idle_legs", nullptr ) );
    }
...
#ifdef STATIONARYMGUN_NEW
    {
        ...
        for ( int idx = 0; idx < E->m_barrels.size(); idx++ ) {
            if ( idx < m_barrels.size() ) {
                m_barrels.at( idx ).SetAmmoElapsed(
                    E->m_barrels.at( idx ).a_elapsed );
            }
        }
    ...
    LPCSTR custom_cam_first =
        READ_IF_EXISTS( ini, r_string, "camera", "cam_first", nullptr );
    if ( custom_cam_first && pSettings->section_exist( custom_cam_first ) ) {
        camera[ eCamFirst ]->Load( custom_cam_first );
    }

    LPCSTR custom_cam_chase =
        READ_IF_EXISTS( ini, r_string, "camera", "cam_chase", nullptr );
    ...
    {
        /* Hack. net_spawn() of CScriptBinderObjectWrapper runs first. This
         * allows overriding configs read in engine net_Spawn(). */
        LPCSTR str = READ_IF_EXISTS( pSettings, r_string, cNameSect_str(),
                                     "net_spawn_after", nullptr );
...
void CWeaponStatMgun::UpdateEx( float fov ) {
...
#ifdef HOLDERCUSTOM_NEW
#ifdef STATIONARYMGUN_NEW
    /* Dead owner. Kick out. Just in case. We kick them out when on death
     * callback already. */
    ...
    if ( m_actor_bone != BI_NONE ) {
        Fvector pos;
        Fmatrix xfm =
            Visual()->dcast_PKinematics()->LL_GetTransform( m_actor_bone );
        xfm.transform_tiny( pos, m_actor_offsets );
        Owner()->XFORM().mul_43( XFORM(), xfm.translate_over( pos ) );
    } else {
        /* Should not let this happen. It indicates that there is no actor bone
         * or it is invalid. Should fix. */
...
void CWeaponStatMgun::UpdateBarrelDir() {
    ...
    /*
            Take m_i_bind_y_xform as base for both bone x and bone_rotate_y
       assuming bone_rotate_y always has 0 pitch. And reset dep before
       calculating.
    */
    {
        // x angle
    ...
    }
    {
        ...
        ClampRotationHorz( m_tgt_y_rot, m_cur_y_rot, -m_lim_y_rot.y,
                           -m_lim_y_rot.x );
    }
    ...
    switch ( GetState() ) {
        ...
        case eStateFire:
            m_cur_x_rot =
                angle_inertion_var( m_cur_x_rot, m_tgt_x_rot, m_rotate_x_speed,
                                    m_rotate_x_speed, PI, Device.fTimeDelta );
            m_cur_y_rot =
                angle_inertion_var( m_cur_y_rot, m_tgt_y_rot, m_rotate_y_speed,
                                    m_rotate_y_speed, PI, Device.fTimeDelta );
            break;
        case eStateReload:
            m_cur_x_rot = m_bind_x_rot;
            break;
    ...
    m_cur_x_rot = angle_inertion_var( m_cur_x_rot, m_tgt_x_rot, 0.5f, 3.5f,
...
void CWeaponStatMgun::cam_Update( float dt, float fov ) {
    ...
    switch ( cam->tag ) {
        case eCamFirst: {
            ...
            if ( m_camera_bone_def != BI_NONE ) {
                bone_id = ( IsCameraZoom() && ( m_camera_bone_aim != BI_NONE ) )
                              ? m_camera_bone_aim
                              : m_camera_bone_def;
            }
            Fmatrix xfm =
                Visual()->dcast_PKinematics()->LL_GetTransform( bone_id );
    ...
    float zoom_factor =
        ( IsCameraZoom() ) ? m_zoom_factor_aim : m_zoom_factor_def;
    ...
    Camera()->yaw = angle_inertion_var( Camera()->yaw, des_cam_dir.x, 0.5f,
                                        7.5f, PI_DIV_6, Device.fTimeDelta );
    Camera()->pitch = angle_inertion_var( Camera()->pitch, des_cam_dir.y, 0.5f,
                                          7.5f, PI_DIV_6, Device.fTimeDelta );
...
void CWeaponStatMgun::ClampRotationHorz( float& tgt_val,
                                         const float& cur_val,
                                         const float& lim_min,
                                         const float& lim_max ) {
    ...
    if ( abs( lim_min ) < PI || abs( lim_max ) < PI ) {
        ...
        if ( tgt_val < lim_min || tgt_val > lim_max ) {
            if ( angle_difference( tgt_val, lim_min ) <
                 angle_difference( tgt_val, lim_max ) )
                tgt_val = lim_min;
            else
                tgt_val = lim_max;
        }
        /* If the rotation to reach tgt_val from cur_val crosses 180 degrees in
         * the back, make it swings around 0. */
    ...
    switch ( id ) {
        case eWpnDesiredPos: {
            Fvector vec =
                Fmatrix()
                    .mul_43( XFORM(),
                             Visual()->dcast_PKinematics()->LL_GetTransform(
                                 m_rotate_y_bone ) )
                    .c;
...
void CWeaponStatMgun::CreateSkeleton( CSE_Abstract* po ) {
    ...
    ApplySpawnIniToPhysicShell( &po->spawn_ini(), m_pPhysicsShell,
                                fixed_bones.size() > 0 );
    ApplySpawnIniToPhysicShell( K->LL_UserData(), m_pPhysicsShell,
                                fixed_bones.size() > 0 );
}
...
void CWeaponStatMgun::IgnoreOwnerCallback( bool& do_colide,
                                           bool bo1,
                                           dContact& c,
                                           SGameMtl* mt1,
                                           SGameMtl* mt2 ) {
    ...
    dxGeomUserData* gd1 = bo1 ? PHRetrieveGeomUserData( c.geom.g1 )
                              : PHRetrieveGeomUserData( c.geom.g2 );
    dxGeomUserData* gd2 = bo1 ? PHRetrieveGeomUserData( c.geom.g2 )
                              : PHRetrieveGeomUserData( c.geom.g1 );
    CGameObject* obj =
        ( gd1 ) ? smart_cast< CGameObject* >( gd1->ph_ref_object ) : nullptr;
    CGameObject* who =
        ( gd2 ) ? smart_cast< CGameObject* >( gd2->ph_ref_object ) : nullptr;
    if ( !obj || !who ) {
        return;
    }
...
bool CWeaponStatMgun::Use( const Fvector& pos,
                           const Fvector& dir,
    ...
    if ( m_on_before_use_callback && strlen( m_on_before_use_callback ) ) {
        ::luabind::functor< bool > lua_function;
        if ( ai().script_engine().functor( m_on_before_use_callback,
                                           lua_function ) ) {
...
void CWeaponStatMgun::UpdateSound() {
    if ( IsActive() &&
         ( GetState() == eStateIdle || GetState() == eStateFire ) ) {
        m_sound_mgr.RotatePlay( true );
        bool play = false;
        if ( m_sound_mgr.m_rotating_x &&
             !fsimilar( m_cur_x_rot, m_tgt_x_rot, EPS_L ) )
            play = true;
        if ( m_sound_mgr.m_rotating_y &&
             !fsimilar( m_cur_y_rot, m_tgt_y_rot, EPS_L ) )
            play = true;
...
void CWeaponStatMgun::UpdateAnimation() {
    ...
    if ( OwnerActor() ) {
        if ( anim_body && strlen( anim_body ) ) {
            ...
            if ( mid_body.idx != OwnerActor()->m_current_torso.idx ) {
                // Msg("%s:%d %s old:%d new:%d", __FUNCTION__, __LINE__,
                // Owner()->cNameSect_str(), OwnerActor()->m_current_torso.idx,
                // mid_body.idx);
                A->PlayCycle( mid_body );
                OwnerActor()->m_current_torso = mid_body;
                OwnerActor()->CStepManager::on_animation_start( MotionID(),
                                                                nullptr );
            }
        }
        ...
        if ( anim_legs && strlen( anim_legs ) ) {
            ...
            if ( mid_legs.idx != OwnerActor()->m_current_legs.idx ) {
                // Msg("%s:%d %s old:%d new:%d", __FUNCTION__, __LINE__,
                // Owner()->cNameSect_str(), OwnerActor()->m_current_legs.idx,
                // mid_legs.idx);
    ...
    if ( stalker ) {
        ...
        if ( anim_body && strlen( anim_body ) ) {
            MotionID mid_body = A->ID_Cycle( anim_body );
            if ( mid_body.idx !=
                 stalker->animation().torso().animation().idx ) {
                // Msg("%s:%d %s BF: old:%d new:%d", __FUNCTION__, __LINE__,
                // Owner()->cNameSect_str(),
                // stalker->animation().torso().animation().idx, mid_body.idx);
                A->PlayCycle( mid_body );
                stalker->animation().torso().animation( mid_body );
                stalker->CStepManager::on_animation_start( MotionID(),
                                                           nullptr );
            }
        }
        if ( anim_legs && strlen( anim_legs ) ) {
            ...
            if ( mid_legs.idx != stalker->animation().legs().animation().idx ) {
                // Msg("%s:%d BF: %s old:%d new:%d", __FUNCTION__, __LINE__,
                // Owner()->cNameSect_str(),
                // stalker->animation().legs().animation().idx, mid_legs.idx);
                A->PlayCycle( mid_legs );
...
void CWeaponStatMgun::SetTraverseLimitHorz( Fvector2 vec ) {
    m_lim_y_rot.set( std::min( deg2rad( -vec.y ), 0.0F ),
                     std::max( deg2rad( -vec.x ), 0.0F ) );
...
void CWeaponStatMgun::SetTraverseLimitVert( Fvector2 vec ) {
    m_lim_x_rot.set( std::min( deg2rad( -vec.y ), 0.0F ),
                     std::max( deg2rad( -vec.x ), 0.0F ) );
...
BOOL CWeaponStatMgun::SStmSound::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( ini->section_exist( snd ) ) {
        ...
        if ( ini->line_exist( snd, "rotate_snd" ) ) {
            m_rotate_snd.create( ini->r_string( snd, "rotate_snd" ), st_Effect,
                                 sg_SourceType );
        }
        m_rotate_bid = ini->line_exist( snd, "rotate_bid" )
                           ? K->LL_BoneID( ini->r_string( snd, "rotate_bid" ) )
                           : BI_NONE;
        m_rotating_x =
            !!READ_IF_EXISTS( ini, r_bool, snd, "rotating_x", FALSE );
        m_rotating_y =
            !!READ_IF_EXISTS( ini, r_bool, snd, "rotating_y", FALSE );
    }
    return TRUE;
}
...
void CWeaponStatMgun::SStmSound::Update() {
    if ( m_rotate_snd._feedback() && ( m_rotate_bid != BI_NONE ) ) {
        Fvector pos =
            Fmatrix()
                .mul_43( m_stm->XFORM(),
                         m_stm->Visual()->dcast_PKinematics()->LL_GetTransform(
                             m_rotate_bid ) )
                .c;
...
void CWeaponStatMgun::SStmSound::RotatePlay( bool state ) {
    if ( state ) {
        ...
        if ( m_rotate_snd._feedback() == nullptr ) {
            m_rotate_snd.play_at_pos( m_stm->cast_game_object(), pos,
                                      sm_Looped );
        }
    } else {
        m_rotate_snd.stop();
