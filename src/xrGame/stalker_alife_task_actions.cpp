...
CStalkerActionSolveZonePuzzle::CStalkerActionSolveZonePuzzle(
    CAI_Stalker* object,
    LPCSTR action_name )
...
void CStalkerActionSolveZonePuzzle::initialize() {
    ...
    object().sight().setup(
        CSightAction( SightManager::eSightTypeCover, false, true ) );
#else
#if 1
    //		object().movement().set_desired_position	(0);
    object().movement().set_desired_direction( 0 );
...
//(CSightAction(SightManager::eSightTypeCurrentDirection));
    ...
    object().movement().patrol().set_path(
        "test_sight", PatrolPathManager::ePatrolStartTypeNearest,
        PatrolPathManager::ePatrolRouteTypeContinue );
...
void CStalkerActionSolveZonePuzzle::execute() {
    ...
    if ( Device.dwTimeGlobal >= m_stop_weapon_handling_time )
        if ( !object().best_weapon() )
            ...
            object().CObjectHandler::set_goal( eObjectActionStrapped,
                                               object().best_weapon() );
    else
        object().CObjectHandler::set_goal( eObjectActionIdle,
                                           object().best_weapon() );
...
#if 1
    //		object().throw_target
    //(g_actor->Position(), g_actor); 		if (object().throw_enabled()) {
    //			object().CObjectHandler::set_goal
    ...
    object().CObjectHandler::set_goal( eObjectActionFire1,
                                       object().best_weapon() );
#else
#if 1
    const CWeapon* weapon =
        smart_cast< const CWeapon* >( object().best_weapon() );
    VERIFY( weapon );
    ...
    VERIFY( weapon );
    //			Msg
    //("weapon %s is strapped : %c",*weapon->cName(),weapon->strapped_mode() ?
    //'+' : '-');

    static u32 m_time_to_strap = 0;
    static u32 m_time_to_idle = 0;
    if ( !object().inventory().ActiveItem() ||
         ( object().inventory().GetActiveSlot() == INV_SLOT_2 ) ) {
        if ( !m_time_to_strap )
...
CStalkerActionSmartTerrain::CStalkerActionSmartTerrain( CAI_Stalker* object,
                                                        LPCSTR action_name )
    : inherited( object, action_name ) {
...
void CStalkerActionSmartTerrain::initialize() {
    ...
    //	object().movement().set_desired_position		(0);
    object().movement().set_desired_direction( 0 );
    object().movement().game_selector().set_selection_type(
        eSelectionTypeMask );
    object().movement().set_detail_path_type(
        DetailPathManager::eDetailPathTypeSmooth );
    object().movement().set_body_state( eBodyStateStand );
...
void CStalkerActionSmartTerrain::execute() {
    ...
    CALifeSmartTerrainTask* task =
        stalker->brain().smart_terrain().task( stalker );
