...
void xrServer::Process_event( NET_Packet& P, ClientID sender ) {
    ...
    // correct timestamp with server-unique-time (note: direct message
    // correction)
    P.r_u32( timestamp );
    ...
    switch ( type ) {
        ...
        case GEG_PLAYER_ACTIVATEARTEFACT: {
            Process_event_activate( P, sender, timestamp, destination,
                                    P.r_u16(), true );
            break;
        };
        case GE_INV_ACTION: {
        ...
        case GE_TRANSFER_AMMO: {
            ...
            CSE_Abstract* e_entity =
                game->get_entity_from_eid( id_entity ); // кто отдает
            if ( !e_entity )
                break;
        ...
        case GE_DIE: {
            ...
            if ( ( game->Type() != eGameIDSingle ) && l_pC && l_pC->owner ) {
                Msg( "* [%2d] killed by [%2d] - sended by [0x%08x]", id_dest,
                     id_src, l_pC->ID.value() );
            }
            ...
            if ( game->Type() != eGameIDSingle )
                Msg( "* [%2d] is [%s:%s]", id_dest, *e_dest->s_name,
                     e_dest->name_replace() );
#endif // #ifndef MASTER_GOLD

            CSE_Abstract* e_src =
                game->get_entity_from_eid( id_src ); // кто убил
            if ( !e_src ) {
...
#ifndef MASTER_GOLD
            if ( game->Type() != eGameIDSingle )
                Msg( "* [%2d] is [%s:%s]", id_src, *e_src->s_name,
                     e_src->name_replace() );
#endif // #ifndef MASTER_GOLD
        ...
        case GE_INSTALL_UPGRADE: {
            ...
            CSE_ALifeInventoryItem* iitem =
                smart_cast< CSE_ALifeInventoryItem* >( receiver );
            if ( !iitem ) {
                break;
            }
        ...
        case GE_INV_BOX_STATUS: {
            ...
            CSE_ALifeInventoryBox* box =
                smart_cast< CSE_ALifeInventoryBox* >( receiver );
            if ( !box ) {
                break;
            }
        ...
        case GE_INV_OWNER_STATUS: {
            ...
            CSE_ALifeTraderAbstract* iowner =
                smart_cast< CSE_ALifeTraderAbstract* >( receiver );
            if ( !iowner ) {
                break;
            }
        ...
        case GEG_PLAYER_USE_BOOSTER: {
            if ( receiver && receiver->owner &&
                 ( receiver->owner != SV_Client ) ) {
                NET_Packet tmp_packet;
                CGameObject::u_EventGen( tmp_packet, GEG_PLAYER_USE_BOOSTER,
                                         receiver->ID );
        ...
        case GE_MONEY: {
            ...
            CSE_ALifeTraderAbstract* pTa =
                smart_cast< CSE_ALifeTraderAbstract* >( e_dest );
