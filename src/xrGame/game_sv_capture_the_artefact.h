...
private:
    ...
    struct MyTeam {
        ...
        MyTeam( TEAM_DATA_LIST::size_type indexInTeamList,
                u16 pCount,
    ...
    void AddAnomalyChanges( NET_Packet& packet,
    ...
    void MoveArtefactToPoint( CSE_ALifeItemArtefact* artefact,
    ...
    typedef associative_vector< xrClientData const*, buyMenuPlayerState >
        TBuyMenuPlayerStates;
    TBuyMenuPlayerStates m_buyMenuPlayerStates;
    virtual void OnPlayerOpenBuyMenu(
        xrClientData const*
            pclient ); // this method invokes only if player dead
    virtual void OnPlayerCloseBuyMenu( xrClientData const* pclient );
    // if client state buyMenuPlayerReadyToSpawn respawn player
    void OnCloseBuyMenuFromAll(); // just clears buy menu player states
                                  // associative vector
    ...
    // void DestroyAllPlayerItems(ClientID id_who);	//except rukzak and
    // artefact :)
    ...
    void StartNewRound();
    void ActorDeliverArtefactOnBase( CSE_ActorMP* actor,
                                     ETeam actorTeam,
                                     ETeam teamOfArtefact );
    void DropArtefact( CSE_ActorMP* aOwner,
                       CSE_ALifeItemArtefact* artefact,
                       Fvector const* dropPosition = NULL );
    ...
    KILL_RES GetKillResult( game_PlayerState* pKiller,
                            game_PlayerState* pVictim );
    bool OnKillResult( KILL_RES KillResult,
                       game_PlayerState* pKiller,
                       game_PlayerState* pVictim );
    void OnGiveBonus( KILL_RES KillResult,
                      game_PlayerState* pKiller,
                      game_PlayerState* pVictim,
                      KILL_TYPE KillType,
...
public:
    ...
    virtual void LoadSkinsForTeam( const shared_str& caSection,
    ...
    virtual void OnPlayerHitPlayer( u16 id_hitter,
                                    u16 id_hitted,
                                    NET_Packet& P );
    virtual void OnPlayerHitPlayer_Case( game_PlayerState* ps_hitter,
    ...
    virtual BOOL OnTouch(
        u16 eid_who,
        u16 eid_target,
        BOOL bForced = FALSE ); // TRUE=allow ownership, FALSE=denied
