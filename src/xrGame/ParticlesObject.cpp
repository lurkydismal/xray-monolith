...
CParticlesObject::CParticlesObject( LPCSTR p_name,
                                    BOOL bAutoRemove,
...
void CParticlesObject::Init( LPCSTR p_name,
                             IRender_Sector* S,
                             BOOL bAutoRemove ) {
    m_bLooped = false;
    m_bStopping = false;
    ...
    if ( !g_dedicated_server ) {
        ...
        IParticleCustom* V = renderable.visual
                                 ? renderable.visual->dcast_ParticleCustom()
                                 : NULL;
        VERIFY( V );
    ...
    if ( time_limit > 0.f ) {
        ...
        if ( bAutoRemove ) {
            R_ASSERT3( !m_bAutoRemove,
                       "Can't set auto-remove flag for looped particle system.",
                       p_name );
        } else {
            m_iLifeTime = 0;
            m_bLooped = true;
...
void CParticlesObject::UpdateSpatial() {
    ...
    if ( _valid( vis.sphere ) ) {
        ...
        if ( 0 == SpatialComponent->spatial.type ) {
            ...
            bool bMove = false;
            if ( !P.similar( SpatialComponent->spatial.sphere.P,
                             EPS_L * 10.f ) )
                bMove = true;
            if ( !fsimilar( R, SpatialComponent->spatial.sphere.R, 0.15f ) )
                bMove = true;
            if ( bMove ) {
                SpatialComponent->spatial.sphere.set( P, R );
                spatial_move();
...
const shared_str CParticlesObject::Name() {
    ...
    IParticleCustom* V =
        renderable.visual ? renderable.visual->dcast_ParticleCustom() : NULL;
    VERIFY( V );
    return ( V ) ? V->Name() : "";
}

intrusive_ptr< CParticlesObject > Particles::Details::Create(
    LPCSTR p_name,
    BOOL bAutoRemove,
    bool remove_on_game_load ) {
    intrusive_ptr< CParticlesObject > Particle =
        xr_new< CParticlesObject >( p_name, bAutoRemove, remove_on_game_load );
