...
void message_filter::filter( u16 const& msg_type,
    ...
    R_ASSERT2( m_filters.find( msgid ) == m_filters.end(),
               "message filter already exist" );
    m_filters.insert( std::make_pair( msgid, found_func ) );
}

void message_filter::remove_filter( u16 const& msg_type,
                                    u32 const& msg_subtype ) {
    msg_type_subtype_t msgid( msg_type, msg_subtype );
...
void message_filter::check_new_data( NET_Packet& packet ) {
    ...
    if ( packet_mtype.msg_type == M_EVENT_PACK ) {
        ...
        while ( !packet.r_eof() ) {
            ...
            R_ASSERT2( packet_mtype.msg_type != M_EVENT_PACK,
                       "M_EVENT_PACK in M_EVENT_PACK" );
            ...
            if ( tmp_iter != m_filters.end() ) {
                tmp_iter->second( packet_mtype.msg_type,
                                  packet_mtype.msg_subtype, tmp_packet );
            }
        }
    } else {
        ...
        filters_map_t::iterator tmp_iter = m_filters.find( packet_mtype );
        if ( tmp_iter != m_filters.end() ) {
            tmp_iter->second( packet_mtype.msg_type, packet_mtype.msg_subtype,
                              packet );
        }
    }
...
void message_filter::dbg_set_message_log_file(
    string_path const& message_log_file ) {
...
void message_filter::dbg_print_msg( NET_Packet& packet,
                                    msg_type_subtype_t const& msg_type ) {
    ...
    switch ( msg_type.msg_type ) {
        case M_EVENT: {
            switch ( msg_type.msg_subtype ) {
                case GE_DESTROY: {
                    xr_sprintf(
                        tmp_string, "--- CL_EVENT [%7u][%5u]: GE_DESTROY",
                        msg_type.msg_receive_time, msg_type.dest_obj_id );
                } break;
                case GE_OWNERSHIP_TAKE: {
                    ...
                    xr_sprintf(
                        tmp_string,
                        "--- CL_EVENT [%7u][%5u]: GE_OWNERSHIP_TAKE    [%d]",
                        msg_type.msg_receive_time, msg_type.dest_obj_id,
                        tmp_id_what );
                } break;
                case GE_OWNERSHIP_REJECT: {
                    ...
                    xr_sprintf(
                        tmp_string,
                        "--- CL_EVENT [%7u][%5u]: GE_OWNERSHIP_REJECT  [%d]",
                        msg_type.msg_receive_time, msg_type.dest_obj_id,
                        tmp_id_what );
                } break;
                default: {
                    xr_sprintf( tmp_string,
                                "--- CL_EVENT [%7u][%5u]: EVENT_ID=[%d]",
                                msg_type.msg_receive_time, msg_type.dest_obj_id,
        ...
        case M_GAMEMESSAGE: {
            switch ( msg_type.msg_subtype ) {
                case GAME_EVENT_PLAYER_KILLED: {
                    xr_sprintf( tmp_string,
                                "--- GM_EVENT [%7u]: GAME_EVENT_PLAYER_KILLED",
                                msg_type.msg_receive_time );
                } break;
                case GAME_EVENT_ROUND_STARTED: {
                    xr_sprintf( tmp_string,
                                "--- GM_EVENT [%7u]: GAME_EVENT_ROUND_STARTED",
                                msg_type.msg_receive_time );
                } break;
                case GAME_EVENT_ARTEFACT_TAKEN: {
                    xr_sprintf( tmp_string,
                                "--- GM_EVENT [%7u]: GAME_EVENT_ARTEFACT_TAKEN",
                                msg_type.msg_receive_time );
                } break;
                default: {
                    xr_sprintf(
                        tmp_string, "--- GM_EVENT [%7u]: GAME_EVENT_ID=[%d]",
                        msg_type.msg_receive_time, msg_type.msg_subtype );
                };
            }
        } break;
        case M_CHAT_MESSAGE: {
        ...
        case M_SPAWN: {
            xr_sprintf( tmp_string, "--- M_SPAWN                [%7u]",
                        msg_type.msg_receive_time );
        } break;
        case M_SV_CONFIG_NEW_CLIENT: {
            xr_sprintf( tmp_string, "--- M_SV_CONFIG_NEW_CLIENT [%7u]",
                        msg_type.msg_receive_time );
        } break;
        case M_SV_CONFIG_GAME: {
            xr_sprintf( tmp_string, "--- M_SV_CONFIG_GAME       [%7u]",
                        msg_type.msg_receive_time );
        } break;
        case M_SV_CONFIG_FINISHED: {
            xr_sprintf( tmp_string, "--- M_SV_CONFIG_FINISHED   [%7u]",
                        msg_type.msg_receive_time );
        } break;
        default: {
            xr_sprintf( tmp_string, "--- MESSAGE_ID[%u]         [%7u]",
                        msg_type.msg_type, msg_type.msg_receive_time );
        };
    }; // switch (m_type)
