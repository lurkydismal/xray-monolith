...
BOOL game_sv_CaptureTheArtefact::isFriendlyFireEnabled() {
        ...
        case GAME_PHASE_INPROGRESS:
            ...
            if ( Get_ReinforcementTime_msec() ) {
                if ( nextReinforcementTime <= currentTime ) {
                    RespawnDeadPlayers();
                    nextReinforcementTime =
                        currentTime + Get_ReinforcementTime_msec();
                }
            }
            if ( CheckForRoundEnd() ) {
                ...
                nextReinforcementTime =
                    currentTime + Get_PlayerScoresDelayTime_msec();
                signal_Syncronize();
            }
            break;
...
void game_sv_CaptureTheArtefact::SM_CheckViewSwitching() {
    if ( !m_pSM_CurViewEntity ||
         !smart_cast< CActor* >( m_pSM_CurViewEntity ) ||
    ...
    if ( gameCTA ) {
        ...
        if ( pObject && smart_cast< CActor* >( pObject ) ) {
            ...
            gameCTA->SetSpectrModeMsgCaption(
                "Server works in spectator mode" );
    }
}
...
void game_sv_CaptureTheArtefact::SM_SwitchOnNextActivePlayer() {
    ...
    if ( !tmp_functor.PPlayersCount ) {
        ...
        xrClientData* C = tmp_functor.PossiblePlayers[::Random.randI(
            ( int )tmp_functor.PPlayersCount ) ];
...
void game_sv_CaptureTheArtefact::net_Export_State( NET_Packet& P,
                                                   ClientID id_to ) {
    inherited::net_Export_State( P, id_to );
    VERIFY( TeamList.size() >= 2 );
    // this is a bad style, because each update state time there is a 4
    // unnecessary bytes transmitting but client needs to know Game identifiers
    // of artefact objects
    VERIFY( teams.size() >= 2 );
...
void game_sv_CaptureTheArtefact::net_Export_Update( NET_Packet& P,
                                                    ClientID id_to,
                                                    ClientID id ) {
...
void game_sv_CaptureTheArtefact::OnPlayerConnect( ClientID id_who ) {
    ...
    if ( ( g_dedicated_server || m_bSpectatorMode ) &&
         ( xrCData == m_server->GetServerClient() ) ) {
...
void game_sv_CaptureTheArtefact::OnPlayerDisconnect( ClientID id_who,
                                                     LPSTR Name,
                                                     u16 GameID ) {
    ...
    TeamsMap::iterator artefactOwnerTeam =
        std::find_if( teams.begin(), te, [ & ]( const TeamPair& tp ) {
            return SearchOwnerIdFunctor()( tp, GameID );
        } );
    if ( artefactOwnerTeam != te ) {
        DropArtefact( artefactOwnerTeam->second.artefactOwner,
                      artefactOwnerTeam->second.artefact );
    }
...
void game_sv_CaptureTheArtefact::OnPlayerReady( ClientID id_who ) {
    if ( m_phase == GAME_PHASE_PENDING ) {
        ...
        xrClientData* xrCData =
            ( xrClientData* )m_server->ID_to_client( id_who );
        game_PlayerState* ps = get_id( id_who );
        ...
        // it's a fake :(
        if ( !ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ||
             ( Get_ReinforcementTime_msec() == 0 ) || ps->IsSkip() ) {
            return;
        }
        //------------------------------------------------------------
        ...
        CSE_ALifeCreatureActor* pA =
            smart_cast< CSE_ALifeCreatureActor* >( pOwner );
        if ( pA ) {
            TGameIDToBoughtFlag::const_iterator buyer_iter =
                m_dead_buyers.find( id_who );
            int buy_value = 0;
...
void game_sv_CaptureTheArtefact::Create( shared_str& options ) {
    ...
    R_ASSERT2( rpoints[ 0 ].size(),
               "rpoints for green team players not found" );
    ...
    if ( !pSettings->section_exist( m_sBaseWeaponCostSection ) ) {
        VERIFY2( 0,
                 "No section for base weapon cost for this type of the Game!" );
        return;
    };
...
#endif // #ifndef MASTER_GOLD
    m_iMoney_for_BuySpawn = READ_IF_EXISTS(
        pSettings, r_s32, "capturetheartefact_gamedata", "spawn_cost", -10000 );
    m_not_free_ammo_str =
        READ_IF_EXISTS( pSettings, r_string, "capturetheartefact_gamedata",
                        "not_free_ammo", "" );
}
...
void game_sv_CaptureTheArtefact::OnRoundStart() {
    ...
    bool m_bFastRestartBefore =
        m_bFastRestart; // fake, because next inherited::OnRoundStart() sets it
                        // to false :(
    ...
    if ( m_bFastRestartBefore ) {
        ...
        nextReinforcementTime =
            Level().timeServer() + ( Get_ReinforcementTime_msec() / 5 );
    }
    ReSpawnArtefacts();
    LoadAnomalySet();
...
void game_sv_CaptureTheArtefact::BalanceTeams() {
    ...
    if ( tmp_team_counter.l_teams[ etGreenTeam ] ==
         tmp_team_counter.l_teams[ etBlueTeam ] )
        return;
    ...
    NumToMove = ( tmp_team_counter.l_teams[ MaxTeam ] -
                  tmp_team_counter.l_teams[ MinTeam ] ) /
                2;
...
void game_sv_CaptureTheArtefact::OnRoundEnd() {
    ...
    struct spectator_spawner {
        ...
        void operator()( IClient* client ) {
            ...
            if ( ps->IsSkip() )
                return;
            if ( l_pC->owner && smart_cast< CActor* >( Level().Objects.net_Find(
                                    l_pC->owner->ID ) ) ) {
                m_server->Perform_destroy( l_pC->owner,
                                           net_flags( TRUE, TRUE ) );
            }
...
void game_sv_CaptureTheArtefact::OnPlayerSelectSkin( NET_Packet& P,
                                                     ClientID sender ) {
    xrClientData* l_pC = m_server->ID_to_client( sender );
    R_ASSERT2( l_pC, make_string( "Client data not found, id = <%d>",
                                  sender.value() ) );
    s8 l_skin;
    P.r_s8( l_skin );
...
void game_sv_CaptureTheArtefact::OnPlayerSelectTeam( NET_Packet& P,
                                                     ClientID sender ) {
    xrClientData* l_pC = m_server->ID_to_client( sender );
    R_ASSERT2( l_pC, make_string( "Client data not found, id = <%d>",
                                  sender.value() ) );
...
void game_sv_CaptureTheArtefact::OnPlayerChangeTeam(
    game_PlayerState* playerState,
    s8 team ) {
    VERIFY2( playerState, "game_PlayerState is NULL" );
    // search team with minimum players count
    if ( team == -1 ) {
        TeamsMap::iterator minTeam =
            std::min_element( teams.begin(), teams.end(), MinPlayersFunctor() );
        // we can solve the problem with team identifiers if we change
        // playerState->team type to ETeam
...
void game_sv_CaptureTheArtefact::OnPlayerChangeSkin( ClientID id_who,
                                                     s8 skin ) {
    game_PlayerState* ps_who = get_id( id_who );
    ...
    if ( skin == -1 ) {
        TeamsMap::iterator tempPlayerTeam =
            teams.find( static_cast< ETeam >( ps_who->team ) );
        VERIFY2( tempPlayerTeam != teams.end(),
                 make_string( "Player team (%d) not found", ps_who->team ) );
        TEAM_DATA_LIST::size_type teamIndex =
            tempPlayerTeam->second.indexOfTeamInList;
        VERIFY2( TeamList.size() > teamIndex,
                 make_string( "team index not valid: (%d)", teamIndex ) );
        s32 maxSkinIndex =
            static_cast< s32 >( TeamList[ teamIndex ].aSkins.size() );
...
void game_sv_CaptureTheArtefact::OnPlayerSelectSpectator( NET_Packet& P,
                                                          ClientID sender ) {
...
void game_sv_CaptureTheArtefact::LoadTeamData( ETeam eteam,
    ...
    if ( !pSettings->section_exist( m_sBaseWeaponCostSection ) ) {
        R_ASSERT2( 0,
                   "No section for base weapon cost for the Capture The "
                   "Artefact game !" );
        return;
    };
    m_strWeaponsData->Load( m_sBaseWeaponCostSection );
    ...
    LoadDefItemsForTeam( caSection,
                         /*&NewTeam.aWeapons, */ &NewTeam.aDefaultItems );

    shared_str artefactName;

    VERIFY2(
        pSettings->line_exist( caSection, "artefact" ),
    ...
    if ( pSettings->section_exist( caSection ) ) {
        ...
        NewTeam.m_iM_TargetSucceed =
            GetMoneyAmount( caSection, "target_succeed" );
        NewTeam.m_iM_TargetSucceedAll =
            GetMoneyAmount( caSection, "target_succeed_all" );
        NewTeam.m_iM_TargetFailed =
            GetMoneyAmount( caSection, "target_failed" );
        ...
        NewTeam.m_iM_RoundWin_Minor =
            GetMoneyAmount( caSection, "round_win_minor" );
        NewTeam.m_iM_RoundLoose_Minor =
            GetMoneyAmount( caSection, "round_loose_minor" );

        NewTeam.m_iM_RivalsWipedOut =
            GetMoneyAmount( caSection, "rivals_wiped_out" );

        NewTeam.m_iM_ClearRunBonus =
            GetMoneyAmount( caSection, "clear_run_bonus" );
        //---------------------------------------------------------------------------
        if ( pSettings->line_exist( caSection, "kill_while_invincible" ) )
            NewTeam.m_fInvinsibleKillModifier =
                pSettings->r_float( caSection, "kill_while_invincible" );
        else
    ...
    teams.insert( std::make_pair(
        eteam, MyTeam( newTeamId, 0, caSection, artefactName ) ) );
};
...
bool game_sv_CaptureTheArtefact::LoadAnomaliesItems(
    ...
    if ( !level_ini_file->line_exist( CTA_ANOMALY_SET_BASE_NAME,
                                      ini_set_id ) ) {
        Msg( "! Warning: \"permanent\" string not found in [%s]",
             CTA_ANOMALY_SET_BASE_NAME );
        return false;
    }
...
void game_sv_CaptureTheArtefact::AddAnomalyChanges(
    ...
    for ( TAnomIter i = anomalies.begin(); i != ie; ++i ) {
        ...
        VERIFY2(
            ( packet.w_tell() + temp_packet.B.count ) < NET_PacketSizeLimit,
...
void game_sv_CaptureTheArtefact::SendAnomalyStates() {
    ...
    AddAnomalyChanges( event_pack, m_AnomaliesPermanent,
                       CCustomZone::eZoneStateIdle );

    typedef TAnomalySet::iterator TAnomSetIter;
    TAnomSetIter ie = m_AnomalySet.end();

    // this order of for blocks is necessarily, because enabling of zones must
    // be at last
    for ( TAnomSetIter i = m_AnomalySet.begin(); i != ie; ++i ) {
        if ( !i->second )
            AddAnomalyChanges( event_pack, i->first,
                               CCustomZone::eZoneStateDisabled );
    }
    for ( TAnomSetIter i = m_AnomalySet.begin(); i != ie; ++i ) {
        if ( i->second )
            AddAnomalyChanges( event_pack, i->first,
...
void game_sv_CaptureTheArtefact::CheckAnomalyUpdate( u32 currentTime ) {
    if ( ( m_dwLastAnomalyStartTime + Get_AnomalySetLengthTime_msec() ) <=
         currentTime )
        ReStartRandomAnomaly();
}

s32 game_sv_CaptureTheArtefact::GetMoneyAmount( const shared_str& caSection,
                                                const char* caMoneyStr ) {
...
void game_sv_CaptureTheArtefact::LoadArtefactRPoints() {
    ...
    if ( FS.exist( fn_game, "$level$", "level.game" ) ) {
        ...
        if ( ( O = F->open_chunk( RPOINT_CHUNK ) ) != 0 ) {
            for ( int id = 0; O->find_chunk( id ); ++id ) {
                ...
                O->r_fvector3( R.P );
                O->r_fvector3( R.A );
                // in editor there green team has index 1 so we must decrement
                // this value
                ...
                VERIFY2( ( ( team >= 0 ) && ( team < 4 ) ) ||
                             ( type != rptActorSpawn ),
                         "Problem with team indexes. In Editor green team id = "
                         "1, blue team id = 2, but in game - 0, 1." );
    ...
    for ( TeamsMap::const_iterator i = teams.begin(); i != teams.end(); i++ ) {
        if ( !i->second.rPointInitialized ) {
            VERIFY2( false,
                     make_string( "Not found RPoint for team %d", i->first )
                         .c_str() );
        }
    }
}

void game_sv_CaptureTheArtefact::LoadSkinsForTeam(
    const shared_str& caSection,
    TEAM_SKINS_NAMES* pTeamSkins ) {
...
void game_sv_CaptureTheArtefact::LoadDefItemsForTeam(
    const shared_str& caSection,
    ...
    for ( u32 i = 0; i < count; ++i ) {
        ...
        pDefItems->push_back(
            u16( m_strWeaponsData->GetItemIdx( ItemName ) & 0xffff ) );
    };
};

void game_sv_CaptureTheArtefact::SpawnWeaponsForActor( CSE_Abstract* pE,
                                                       game_PlayerState* ps ) {
    ...
    while ( ps->pItemList.size() ) {
        ...
        SpawnWeapon4Actor( pA->ID,
                           *m_strWeaponsData->GetItemName( ItemID & 0x00FF ),
                           u8( ( ItemID & 0xFF00 ) >> 0x08 ), ps->pItemList );
        // Game().m_WeaponUsageStatistic->OnWeaponBought(ps,
        // *m_strWeaponsData->GetItemName(ItemID& 0x00FF));
...
KILL_RES game_sv_CaptureTheArtefact::GetKillResult(
...
bool game_sv_CaptureTheArtefact::OnKillResult( KILL_RES KillResult,
                                               game_PlayerState* pKiller,
                                               game_PlayerState* pVictim ) {
    if ( !pKiller || !pVictim ) {
        return false;
    ...
    switch ( KillResult ) {
        ...
        case KR_RIVAL: {
            ...
            pKiller->m_iKillsInRowMax =
                _max( pKiller->m_iKillsInRowCurr, pKiller->m_iKillsInRowMax );
            if ( pTeam ) {
                s32 ResMoney = pTeam->m_iM_KillRival;
                if ( pKiller->testFlag( GAME_PLAYER_FLAG_INVINCIBLE ) )
                    ResMoney =
                        s32( ResMoney * pTeam->m_fInvinsibleKillModifier );
                ...
                if ( pKiller->m_iTeamKills >= Get_TeamKillLimit() ) {
                    struct killer_searcher {
                        ...
                        bool operator()( IClient* client ) {
                            xrClientData* pCL =
                                static_cast< xrClientData* >( client );
...
void game_sv_CaptureTheArtefact::OnGiveBonus( KILL_RES KillResult,
                                              game_PlayerState* pKiller,
                                              game_PlayerState* pVictim,
                                              KILL_TYPE KillType,
    ...
    switch ( KillResult ) {
        case KR_RIVAL: {
                    ...
                    switch ( SpecialKillType ) {
                        case SKT_HEADSHOT: {
                            Player_AddExperience(
                                pKiller, READ_IF_EXISTS( pSettings, r_float,
                                                         "mp_bonus_exp",
                                                         "headshot", 0 ) );
                            Player_AddBonusMoney(
                                pKiller,
                                READ_IF_EXISTS( pSettings, r_s32,
                                                "mp_bonus_money", "headshot",
                                                0 ),
                                SKT_HEADSHOT );
                        } break;
                        case SKT_EYESHOT: {
                            ...
                            if ( pWeaponA ) {
                                switch ( pWeaponA->m_tClassID ) {
                                    case CLSID_OBJECT_W_KNIFE: {
                                        Player_AddExperience(
                                            pKiller,
                                            READ_IF_EXISTS( pSettings, r_float,
                                                            "mp_bonus_exp",
                                                            "knife_kill", 0 ) );
                                        Player_AddBonusMoney(
                                            pKiller,
            ...
            if ( pKiller->m_iKillsInRowCurr ) {
                string64 tmpStr;
                xr_sprintf( tmpStr, "%d_kill_in_row",
                            pKiller->m_iKillsInRowCurr );
                Player_AddBonusMoney(
                    pKiller,
                    READ_IF_EXISTS( pSettings, r_s32, "mp_bonus_money", tmpStr,
                                    0 ),
                    SKT_KIR, u8( pKiller->m_iKillsInRowCurr & 0xff ) );
            };
        } break;
        default: {
        } break;
...
// copy past :(
void game_sv_CaptureTheArtefact::OnPlayerHitPlayer( u16 id_hitter,
                                                    u16 id_hitted,
                                                    NET_Packet& P ) {
...
void game_sv_CaptureTheArtefact::OnPlayerHitPlayer_Case(
    ...
    if ( ps_hitter && ps_hitted ) {
        if ( ps_hitter->team == ps_hitted->team && ps_hitter != ps_hitted ) {
            ...
            pHitS->impulse *=
                ( GetFriendlyFire() > 1.0f ) ? GetFriendlyFire() : 1.0f;
        }
    }
...
void game_sv_CaptureTheArtefact::OnPlayerKillPlayer(
    game_PlayerState* ps_killer,
    game_PlayerState* ps_killed,
    KILL_TYPE KillType,
    SPECIAL_KILL_TYPE SpecialKillType,
    CSE_Abstract* pWeaponA ) {
    inherited::OnPlayerKillPlayer( ps_killer, ps_killed, KillType,
                                   SpecialKillType, pWeaponA );
    ...
    if ( CanGiveBonus )
        OnGiveBonus( KillRes, ps_killer, ps_killed, KillType, SpecialKillType,
                     pWeaponA );
    Game().m_WeaponUsageStatistic->OnPlayerKillPlayer( ps_killer, KillType,
                                                       SpecialKillType );
    signal_Syncronize();
}

void game_sv_CaptureTheArtefact::DropArtefact( CSE_ActorMP* aOwner,
...
void game_sv_CaptureTheArtefact::ProcessPlayerDeath(
    game_PlayerState* playerState ) {
    ...
    if ( l_pC ) {
        TGameIDToBoughtFlag::iterator buyer_iter =
            m_dead_buyers.find( l_pC->ID );
        if ( buyer_iter != m_dead_buyers.end() )
            buyer_iter->second = 0;
        else
    ...
    TeamsMap::iterator te = teams.end();
    TeamsMap::iterator childArtefactTeam =
        std::find_if( teams.begin(), te, [ & ]( const TeamPair& tp ) {
            return SearchOwnerIdFunctor()( tp, playerState->GameID );
        } );
    if ( childArtefactTeam != te ) {
        DropArtefact( childArtefactTeam->second.artefactOwner,
                      childArtefactTeam->second.artefact );
        /*
        VERIFY2(childArtefactTeam->second.artefactOwner, "dead player hasn't an
        artefact"); VERIFY2(childArtefactTeam->second.artefact, "trying to
        reject not existing team artefact"); NET_Packet P; u_EventGen
        (P,GE_OWNERSHIP_REJECT, childArtefactTeam->second.artefactOwner->ID);
        P.w_u16
        (childArtefactTeam->second.artefact->ID);
        //m_server->SendBroadcast(BroadcastCID, P, net_flags(TRUE, TRUE));
        m_server->Process_event_reject(P, m_server->GetServerClient()->ID, 0,
...
/*void game_sv_CaptureTheArtefact::ProcessPlayerKill(game_PlayerState *
playerState)
{
        if (!playerState)
                return;
...
void game_sv_CaptureTheArtefact::ReSpawnArtefacts() {
    ...
    for ( TeamsMap::iterator ti = teams.begin(); ti != te; ti++ ) {
        VERIFY2( !!ti->second.artefactName,
                 make_string( "not found artefact class name for team %d",
                              ti->first )
                     .c_str() );
        CSE_ALifeItemArtefact* tempSvEntity =
            smart_cast< CSE_ALifeItemArtefact* >(
...
BOOL game_sv_CaptureTheArtefact::OnTouch( u16 eid_who,
                                          u16 eid_target,
                                          BOOL bForced ) {
    CSE_ActorMP* e_who =
        smart_cast< CSE_ActorMP* >( m_server->ID_to_entity( eid_who ) );
    ...
    /*VERIFY2(e_who,
            make_string("no actor (id = %id) touches the target (id =
       %d)").c_str());*/
    ...
    /*VERIFY(e_what	); // <- not used because IMHO next code work
     * faster...*/
    TeamsMap::iterator te = teams.end();
    ...
    if ( artefactOfTeam != te ) {
        ...
        VERIFY( tempArtefact );
        // searching this owner for determinig if the actor already has an
        // artefact
        /*if (std::find_if(
                        teams.begin(),
                        te,
        ...
        if ( artefactOfTeam->first == ps_who->team ) {
            /*Msg("server artefact position (%2.04f,%2.04f,%2.04f), rpoint
            position (%2.04f,%2.04f,%2.04f)", tempArtefact->o_Position.x,
            tempArtefact->o_Position.y, tempArtefact->o_Position.z,
                    tempRPoint.P.x, tempRPoint.P.y, tempRPoint.P.z);
            Msg("server actor position: (%2.04f, %2.04f, %2.04f)",
                    e_who->o_Position.x, e_who->o_Position.y,
            e_who->o_Position.z);*/
            if ( !tempArtefact->o_Position.similar(
                     tempRPoint.P, g_sv_cta_artefactsBaseRadius ) ) {
                if ( !Get_ActivatedArtefactRet() ) {
                    ...
                    VERIFY( TeamList.size() >=
                            artefactOfTeam->second.indexOfTeamInList );
                    Player_AddMoney(
                        ps_who,
                        TeamList[ artefactOfTeam->second.indexOfTeamInList ]
                            .m_iM_KillRival );

                    /*Set_RankUp_Allowed(true);
                    Player_AddExperience(ps_who, READ_IF_EXISTS(pSettings,
                    r_float, "mp_bonus_exp","target_succeed",0));
                    Set_RankUp_Allowed(false);*/
                ...
                if ( std::find_if(
                         teams.begin(), te, [ & ]( const TeamPair& tp ) {
                             return SearchOwnerIdFunctor()( tp, e_who->ID );
                         } ) != te ) {
                    return FALSE;
                }
            ...
            if ( std::find_if( teams.begin(), te, [ & ]( const TeamPair& tp ) {
                     return SearchOwnerIdFunctor()( tp, e_who->ID );
                 } ) != te ) {
                return FALSE;
            }
...
BOOL game_sv_CaptureTheArtefact::OnTouchItem( CSE_ActorMP* actor,
                                              CSE_Abstract* item ) {
    VERIFY( actor );
    VERIFY( item );
    ...
    if ( ( item->m_tClassID == CLSID_OBJECT_PLAYERS_BAG ) &&
        ...
        if ( !item->children.empty() ) {
            ...
            while ( !item->children.empty() ) {
                CSE_Abstract* e_child_item =
                    get_entity_from_eid( item->children.back() );
                if ( e_child_item ) {
                    if ( !OnTouch( actor->ID, e_child_item->ID, FALSE ) ) {
                        ...
                        m_server->Process_event_reject(
                            P, m_server->GetServerClient()->ID, 0, item->ID,
                            e_child_item->ID );
                        continue;
                    }
                }

                m_server->Perform_transfer( PacketReject, PacketTake,
                                            e_child_item, item, actor );
        ...
        if ( isPDAHuntEnabled() && actor->owner && actor->owner->ps ) {
            Player_AddBonusMoney(
                actor->owner->ps,
                READ_IF_EXISTS( pSettings, r_s32, "mp_bonus_money", "pda_taken",
                                0 ),
                SKT_PDA );
        };
...
void game_sv_CaptureTheArtefact::OnDetach( u16 eid_who, u16 eid_target ) {
    ...
    /*VERIFY2(e_who,
            make_string("failed to get actor entity (id =
       %d)",eid_who).c_str());*/
...
void game_sv_CaptureTheArtefact::FillDeathActorRejectItems(
    CSE_ActorMP* actor,
    ...
    if ( active_slot != NO_ACTIVE_SLOT ) {
        ...
        if ( !server_item ) {
#ifndef MASTER_GOLD
            Msg( "! ERROR: server entity is NULL, object ID[%d]",
                 item->object_id() );
#endif // #ifndef MASTER_GOLD
            return;
        }
...
void game_sv_CaptureTheArtefact::OnDetachItem( CSE_ActorMP* actor,
                                               CSE_Abstract* item ) {
    R_ASSERT( actor );
    R_ASSERT( item );
    if ( item->m_tClassID == CLSID_OBJECT_PLAYERS_BAG ) {
        ...
        xr_vector< CSE_Abstract* > to_reject;
        // may be there is a sense to move next invokation into the ProcessDeath
        // method...
        ...
        for ( xr_vector< u16 >::const_iterator it = actor->children.begin();
            ...
            if ( std::find( to_reject.begin(), to_reject.end(), e_item ) !=
                 to_reject.end() )
                continue;
            ...
            } else if ( m_strWeaponsData->GetItemIdx( e_item->s_name ) !=
                        u32( -1 ) ) {
        ...
        for ( xr_vector< CSE_Abstract* >::const_iterator tr_it =
                  to_transfer.begin();
              tr_it != tr_it_e; ++tr_it ) {
            m_server->Perform_transfer( PacketReject, PacketTake, *tr_it, actor,
                                        item );
            EventPack.w_u8( u8( PacketReject.B.count ) );
            EventPack.w( &PacketReject.B.data, PacketReject.B.count );
            EventPack.w_u8( u8( PacketTake.B.count ) );
...
void game_sv_CaptureTheArtefact::MoveArtefactToPoint(
    CSE_ALifeItemArtefact* artefact,
    RPoint const& toPoint ) {
    VERIFY( artefact );
    // artefact->o_Position.set(toPoint.P);

    VERIFY( artefact->cast_inventory_item() );
    artefact->cast_inventory_item()->State.position.set(
        toPoint.P ); // settings position to server object

    CArtefact* OArtefact =
        smart_cast< CArtefact* >( Level().Objects.net_Find( artefact->ID ) );

    R_ASSERT2(
        OArtefact,
        make_string(
            "artefact not found. artefact_id = [%d]. CTA:MoveArtefactToPoint()",
            artefact->ID )
            .c_str() );
...
void game_sv_CaptureTheArtefact::MoveLifeActors() {
    ...
    struct players_teleporter {
        ...
        void operator()( IClient* client ) {
            ...
            if ( l_pC->ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) )
                return;
            VERIFY2( tempPacket.w_tell() < ( NET_PacketSizeLimit - 256 ),
                     "max packet size exceeds !" );
    ...
    MovePacket.w( tmp_functor.tempPacket.B.data,
                  tmp_functor.tempPacket.B.count );
    m_server->SendBroadcast( BroadcastCID, MovePacket,
                             net_flags( TRUE, TRUE ) );
}
...
void game_sv_CaptureTheArtefact::RespawnDeadPlayers() {
    struct deadplayers_respawner {
        ...
        void operator()( IClient* client ) {
            xrClientData* l_pC = static_cast< xrClientData* >( client );
            // R_VERIFY2(l_pC && l_pC->ps && l_pC->owner, make_string("client
            // #%d has problem with states", i).c_str());
...
void game_sv_CaptureTheArtefact::OnObjectEnterTeamBase( u16 id,
                                                        u16 zone_team ) {
...
void game_sv_CaptureTheArtefact::OnObjectLeaveTeamBase( u16 id,
                                                        u16 zone_team ) {
    CSE_Abstract* e_who = m_server->ID_to_entity( id );
    if ( !e_who )
        return;
...
void game_sv_CaptureTheArtefact::ActorDeliverArtefactOnBase(
    CSE_ActorMP* actor,
    ETeam actorTeam,
    ETeam teamOfArtefact ) {
    VERIFY( actor );
    ...
    DropArtefact( actor, artefactOfTeam->second.artefact,
                  &artefactOfTeam->second.artefactRPoint.P );
    ...
    TEAM_DATA_LIST::const_iterator teamIter =
        TeamList.begin() + teams[ actorTeam ].indexOfTeamInList;

    R_ASSERT2( teamIter != TeamList.end(),
               make_string( "deliver artefact team (%d) not found in TeamList",
                            actorTeam )
                   .c_str() );
    ...
    Set_RankUp_Allowed( true );
    Player_AddExperience( ps,
                          READ_IF_EXISTS( pSettings, r_float, "mp_bonus_exp",
                                          "target_succeed", 0 ) );
...
void game_sv_CaptureTheArtefact::StartNewRound() {
    ...
    tmp_functor.bind( this,
                      &game_sv_CaptureTheArtefact::PrepareClientForNewRound );
    m_server->ForEachClientDoSender( tmp_functor );
    MoveLifeActors();
...
void game_sv_CaptureTheArtefact::PrepareClientForNewRound( IClient* client ) {
    ...
    // VERIFY2(clientData && clientData->ps && clientData->owner, "bad client
    // data");
    ...
    // m_server->SendTo(clientData->ID,P,net_flags(TRUE,TRUE));
    assign_RP( static_cast< CSE_ALifeCreatureActor* >( clientData->owner ),
               ps );
}
...
void game_sv_CaptureTheArtefact::CheckForArtefactReturning( u32 currentTime ) {
    ...
    for ( TeamsMap::iterator ti = teams.begin(); ti != te; ++ti ) {
        ...
        if ( artefact->o_Position.similar( artefact_rpoint,
                                           g_sv_cta_artefactsBaseRadius ) )
            continue;
        ...
        if ( ti->second.IsArtefactActivated() ) {
            ...
            CSE_ActorMP* e_who = smart_cast< CSE_ActorMP* >(
                m_server->ID_to_entity( ti->second.last_activator_id ) );
            ...
            if ( e_who ) {
                ...
                Player_AddMoney( ps_who,
                                 TeamList[ ps_who->team ].m_iM_KillRival );

                /*Set_RankUp_Allowed(true);
                Player_AddExperience(ps_who, READ_IF_EXISTS(pSettings, r_float,
                "mp_bonus_exp","target_succeed",0));
                Set_RankUp_Allowed(false);*/
...
void game_sv_CaptureTheArtefact::CheckForArtefactDelivering() {
    ...
    for ( TeamsMap::iterator ti = teams.begin(); ti != te; ++ti ) {
        ...
        if ( !xrCData ) {
#ifdef MP_LOGGING
            Msg( "! WARNING: bad actor [%d] tries to deliver artefact",
                 tempActor->ID );
        ...
        VERIFY2( myTeamIter != teams.end(),
                 make_string( "artefact team (%d) not found", artefactOfTeam )
                     .c_str() );
        // if some one alredy took your artefact or own artefact not on base,
        // continue
        Fvector3& myArtPoint = myTeamIter->second.artefactRPoint.P;
        if ( ( myTeamIter->second.artefactOwner ) ||
             ( !myTeamIter->second.artefact->o_Position.similar(
                 myArtPoint, g_sv_cta_artefactsBaseRadius ) ) ) {
            continue;
        }
        if ( tempActor->o_Position.similar( myTeamIter->second.artefactRPoint.P,
                                            g_sv_cta_artefactsBaseRadius ) ) {
...
BOOL game_sv_CaptureTheArtefact::CheckForRoundEnd() {
    ...
    if ( !GetTimeLimit() )
        return FALSE;
    if ( ( Level().timeServer() - StartTime() ) >
         u32( GetTimeLimit() * 60000 ) ) {
...
bool game_sv_CaptureTheArtefact::ResetInvincibility( ClientID const clientId ) {
    ...
    if ( !tempClient ) // when the time comes, we'll delete this element in
                       // collection
        return false;
...
void game_sv_CaptureTheArtefact::RespawnPlayer( ClientID id_who,
                                                bool NoSpectator ) {
...
void game_sv_CaptureTheArtefact::OnPostCreate( u16 id_who ) {
    ...
    CSE_ALifeCustomZone* custom_zone =
        smart_cast< CSE_ALifeCustomZone* >( entity );
    if ( !custom_zone )
        return;
...
void game_sv_CaptureTheArtefact::ReadOptions( shared_str& options ) {
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    g_sv_dm_bAnomaliesEnabled =
        ( get_option_i( *options, "ans", ( isAnomaliesEnabled() ? 1 : 0 ) ) !=
          0 );
    g_sv_dm_dwAnomalySetLengthTime = get_option_i(
        *options, "anslen", g_sv_dm_dwAnomalySetLengthTime ); // in (min)
    g_sv_dm_bPDAHunt = ( get_option_i( *options, "pdahunt",
                                       ( isPDAHuntEnabled() ? 1 : 0 ) ) != 0 );
    g_sv_dm_bDamageBlockIndicators =
        ( get_option_i( *options, "dmbi",
                        ( g_sv_dm_bDamageBlockIndicators ? 1 : 0 ) ) != 0 );
    g_sv_dm_dwWarmUp_MaxTime =
        get_option_i( *options, "warmup", g_sv_dm_dwWarmUp_MaxTime );
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    //-------------------------------------------------------------------------
    g_sv_tdm_bAutoTeamBalance =
        get_option_i( *options, "abalance",
                      ( g_sv_tdm_bAutoTeamBalance ? 1 : 0 ) ) != 0;
    g_sv_tdm_bAutoTeamSwap =
        get_option_i( *options, "aswap", ( g_sv_tdm_bAutoTeamSwap ? 1 : 0 ) ) !=
        0;
    g_sv_tdm_bFriendlyIndicators =
        get_option_i( *options, "fi",
                      ( g_sv_tdm_bFriendlyIndicators ? 1 : 0 ) ) != 0;
    g_sv_tdm_bFriendlyNames =
        get_option_i( *options, "fn", ( g_sv_tdm_bFriendlyNames ? 1 : 0 ) ) !=
        0;

    float fFF =
        get_option_f( *options, "ffire", g_sv_tdm_fFriendlyFireModifier );
    g_sv_tdm_fFriendlyFireModifier = fFF;
    //-------------------------------------------------------------------------
    g_sv_ah_dwArtefactsNum =
        get_option_i( *options, "anum", g_sv_ah_dwArtefactsNum );
    //----------------------------------------------------------------------------
    g_sv_ah_iReinforcementTime =
        get_option_i( *options, "reinf", g_sv_ah_iReinforcementTime );
    if ( g_sv_ah_iReinforcementTime <= 0 )
        g_sv_ah_iReinforcementTime = 1;
    //----------------------------------------------------------------------------
    g_sv_cta_dwInvincibleTime = get_option_i(
        *options, "dmgblock", g_sv_cta_dwInvincibleTime ); // in (sec)
    g_sv_cta_artefactReturningTime = get_option_i(
        *options, "artrettime", g_sv_cta_artefactReturningTime ); // in (sec)
    g_sv_cta_activatedArtefactRet = get_option_i(
        *options, "actret", g_sv_cta_activatedArtefactRet ); // in (sec)

    m_bSpectatorMode = false;
    if ( !g_dedicated_server &&
         ( get_option_i( *options, "spectr", -1 ) != -1 ) ) {
...
void game_sv_CaptureTheArtefact::ClearReadyFlagFromAll() {
    struct readyclearer_from_all {
        void operator()( IClient* client ) {
            ...
            tmp_client->ps->resetFlag( GAME_PLAYER_FLAG_READY +
...
void game_sv_CaptureTheArtefact::WriteGameState( CInifile& ini,
                                                 LPCSTR sect,
                                                 bool bRoundResult ) {
...
bool game_sv_CaptureTheArtefact::Player_Check_Rank( game_PlayerState* ps ) {
    ...
    u32 max_art_count =
        std::max( teams[ etGreenTeam ].score, teams[ etBlueTeam ].score );
