...
void CCartridge::Load( LPCSTR section, u8 LocalAmmoType, float ap_mod ) {
    ...
    //.	param_s.kCritical			= pSettings->r_float(section,
    //"k_hit_critical");
    param_s.kImpulse = pSettings->r_float( section, "k_impulse" );
    // m_kPierce				= pSettings->r_float(section,
    // "k_pierce");
    param_s.kAP = pSettings->r_float( section, "k_ap" );
    param_s.kAP *= ap_mod;
    param_s.k_cam_dispersion =
        READ_IF_EXISTS( pSettings, r_float, section, "k_cam_dispersion", 1.0f );
    param_s.u8ColorID =
        READ_IF_EXISTS( pSettings, r_u8, section, "tracer_color_ID", 0 );
    param_s.kBulletSpeed =
        READ_IF_EXISTS( pSettings, r_float, section, "k_bullet_speed", 1.0f );

    if ( pSettings->line_exist( section, "k_air_resistance" ) )
        param_s.kAirRes = pSettings->r_float( section, "k_air_resistance" );
    else
        param_s.kAirRes =
            pSettings->r_float( BULLET_MANAGER_SECTION, "air_resistance_k" );
    ...
    param_s.tracer_silenced =
        READ_IF_EXISTS( pSettings, r_bool, section, "tracer_silenced", false );
    ...
    if ( pSettings->line_exist( section, "can_be_unlimited" ) )
        m_flags.set( cfCanBeUnlimited,
                     pSettings->r_bool( section, "can_be_unlimited" ) );
...
void CWeaponAmmo::Load( LPCSTR section ) {
    ...
    //.	cartridge_param.kCritical	= pSettings->r_float(section,
    //"k_hit_critical");
    cartridge_param.kImpulse = pSettings->r_float( section, "k_impulse" );
    // m_kPierce				= pSettings->r_float(section,
    // "k_pierce");
    cartridge_param.kAP = pSettings->r_float( section, "k_ap" );
    cartridge_param.u8ColorID =
        READ_IF_EXISTS( pSettings, r_u8, section, "tracer_color_ID", 0 );

    if ( pSettings->line_exist( section, "k_air_resistance" ) )
        cartridge_param.kAirRes =
            pSettings->r_float( section, "k_air_resistance" );
    else
        cartridge_param.kAirRes =
            pSettings->r_float( BULLET_MANAGER_SECTION, "air_resistance_k" );
...
CInventoryItem* CWeaponAmmo::can_make_killing(
    ...
    for ( ; I != E; ++I ) {
        ...
        xr_vector< shared_str >::const_iterator i =
            std::find( weapon->m_ammoTypes.begin(), weapon->m_ammoTypes.end(),
                       cNameSect() );
