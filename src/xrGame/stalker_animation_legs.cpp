...
IC float CStalkerAnimationManager::legs_switch_factor() const {
    if ( ( m_target_direction == eMovementDirectionForward ) &&
         ( m_current_direction == eMovementDirectionBackward ) )
        return ( 0.f );

    if ( ( m_target_direction == eMovementDirectionBackward ) &&
         ( m_current_direction == eMovementDirectionForward ) )
        return ( 0.f );

    if ( ( m_target_direction == eMovementDirectionLeft ) &&
         ( m_current_direction == eMovementDirectionRight ) )
        return ( 0.f );

    if ( ( m_target_direction == eMovementDirectionRight ) &&
...
bool CStalkerAnimationManager::need_look_back() const {
    ...
    if ( ( m_change_direction_time + need_look_back_time_delay ) >
         Device.dwTimeGlobal )
        return ( false );
...
void CStalkerAnimationManager::legs_assign_direction(
    float switch_factor,
    ...
    if ( ( Device.dwTimeGlobal - m_direction_start ) <=
         ( u32 )iFloor( switch_factor * direction_switch_interval ) )
        return;
...
MotionID CStalkerAnimationManager::legs_move_animation() {
    ...
    VERIFY( ( movement.body_state() == eBodyStateStand ) ||
    ...
    if ( eMentalStateDanger != movement.mental_state() ) {
        ...
        if ( movement.movement_type() == eMovementTypeRun &&
             movement.mental_state() == eMentalStatePanic )
            return ( m_data_storage->m_part_animations.A[ eBodyStateStand ]
                         .m_movement.A[ 2 ]
                         .A[ 4 ]
                         .A[ 0 ] );
...
MotionID CStalkerAnimationManager::legs_no_move_animation() {
    ...
    const xr_vector< MotionID >& animation =
