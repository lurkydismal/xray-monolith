...
IC MagicBox3& moving_objects::continuous_box(
    ...
    if ( use_box_enlargement &&
         ( object->action() != moving_object::action_move ) ) {
...
struct collision_predicate {
    ...
    IC bool operator()(
        const moving_objects::COLLISION_TIME& collision_time ) const {
...
void moving_objects::resolve_collision_first( boxes& current,
                                              moving_object* object0,
                                              moving_object* object1,
    ...
    if ( target._0.intersects( start._1 ) ) {
        MSG( "%6d [t0s1] [%s][%s]", Device.dwFrame, *object0->object().cName(),
             *object1->object().cName() );
        action = possible_action_1_can_wait_2;
        return;
    }

    if ( target._1.intersects( start._0 ) ) {
        MSG( "%6d [t1s0] [%s][%s]", Device.dwFrame, *object0->object().cName(),
             *object1->object().cName() );
    ...
    if ( start._0.intersects( current._1 ) ) {
        MSG( "%6d [s0c1] [%s][%s]", Device.dwFrame, *object0->object().cName(),
             *object1->object().cName() );
        action = possible_action_2_can_wait_1;
        return;
    }
    ...
    if ( start._1.intersects( current._0 ) ) {
        MSG( "%6d [s1c0] [%s][%s]", Device.dwFrame, *object0->object().cName(),
             *object1->object().cName() );
        action = possible_action_1_can_wait_2;
        return;
    }
    ...
    if ( current._0.intersects( target._1 ) ) {
        MSG( "%6d [c0t1] [%s][%s]", Device.dwFrame, *object0->object().cName(),
             *object1->object().cName() );
        action = possible_action_2_can_wait_1;
        return;
    }
    ...
    if ( current._1.intersects( target._0 ) ) {
        MSG( "%6d [c1t0] [%s][%s]", Device.dwFrame, *object0->object().cName(),
             *object1->object().cName() );
        action = possible_action_1_can_wait_2;
        return;
    }
...
void moving_objects::resolve_collision_previous(
    ...
    if ( action == possible_action_2_can_wait_1 ) {
        if ( target._1.intersects( start._0 ) ) {
            MSG( "%6d [t1s0] [%s][%s]", Device.dwFrame,
                 *object0->object().cName(), *object1->object().cName() );
            return;
        }

        if ( start._0.intersects( current._1 ) ) {
            MSG( "%6d [s0c1] [%s][%s]", Device.dwFrame,
                 *object0->object().cName(), *object1->object().cName() );
            return;
        }

        if ( current._0.intersects( target._1 ) ) {
            MSG( "%6d [c0t1] [%s][%s]", Device.dwFrame,
                 *object0->object().cName(), *object1->object().cName() );
            return;
        }
    }
...
void moving_objects::resolve_collision( boxes& current,
                                        moving_object* object0,
...
#endif // 0
    VERIFY2(
        object0->action_frame() != Device.dwFrame,
        make_string( "%d %s", Device.dwFrame, *object0->object().cName() ) );
    VERIFY2(
        object0->action_frame() < Device.dwFrame,
        make_string( "%d %s", Device.dwFrame, *object0->object().cName() ) );

    VERIFY2(
        object1->action_frame() != Device.dwFrame,
        make_string( "%d %s", Device.dwFrame, *object0->object().cName() ) );
    VERIFY2(
        object1->action_frame() < Device.dwFrame,
        make_string( "%d %s", Device.dwFrame, *object0->object().cName() ) );
    ...
    bool first_time =
        ( std::find_if(
              m_previous_collisions.begin(), m_previous_collisions.end(),
              collision_predicate( std::make_pair( object0, object1 ) ) ) ==
    ...
    if ( object1->action() == moving_object::action_wait ) {
        ...
        action = possible_actions( action ^ ( possible_action_1_can_wait_2 |
                                              possible_action_2_can_wait_1 ) );
        return;
    }

    VERIFY2( false,
             make_string( "NODEFAULT: [%s][%s]", *object0->object().cName(),
                          *object1->object().cName() ) );
}

bool moving_objects::collided_dynamic( moving_object* object0,
                                       const Fvector& position0,
                                       moving_object* object1,
                                       const Fvector& position1,
                                       boxes& result ) const {
...
bool moving_objects::collided_dynamic( moving_object* object0,
    ...
    resolve_collision( current, object0, position0, object1, position1,
                       action );
    return ( true );
}
