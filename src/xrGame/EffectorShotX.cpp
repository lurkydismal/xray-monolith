...
/*CCameraShotEffectorX::CCameraShotEffectorX	(float max_angle, float
relax_speed, float max_angle_horz, float step_angle_horz, float angle_frac) :
CCameraShotEffector(max_angle, relax_speed, max_angle_horz, step_angle_horz,
angle_frac)
{
}
*/
...
void	CCameraShotEffectorX::UpdateActorCamera		(float dPitch, float
dYaw)
{
        if (!m_pActor) return;
        ...
        if (pACam->bClampYaw)
clamp(pACam->yaw,pACam->lim_yaw[0],pACam->lim_yaw[1]); if (pACam->bClampPitch)
