...
CStalkerDangerPlanner::CStalkerDangerPlanner( CAI_Stalker* object,
                                              LPCSTR action_name )
    : inherited( object, action_name ) {}

void CStalkerDangerPlanner::setup( CAI_Stalker* object,
...
void CStalkerDangerPlanner::add_evaluators() {
    ...
    add_evaluator( eWorldPropertyDangerUnknown,
                   xr_new< CStalkerPropertyEvaluatorDangerUnknown >(
                       m_object, "danger unknown" ) );
    add_evaluator( eWorldPropertyDangerInDirection,
                   xr_new< CStalkerPropertyEvaluatorDangerInDirection >(
                       m_object, "danger in direction" ) );
    add_evaluator( eWorldPropertyDangerGrenade,
                   xr_new< CStalkerPropertyEvaluatorDangerWithGrenade >(
                       m_object, "danger with grenade" ) );
    add_evaluator( eWorldPropertyDangerBySound,
                   xr_new< CStalkerPropertyEvaluatorDangerBySound >(
                       m_object, "danger by sound" ) );
}

void CStalkerDangerPlanner::add_actions() {
    CActionPlannerActionScript< CAI_Stalker >* action;

    action = xr_new< CStalkerDangerUnknownPlanner >( m_object,
                                                     "danger unknown planner" );
    ...
    action = xr_new< CStalkerDangerInDirectionPlanner >(
        m_object, "danger in direction planner" );
    add_condition( action, eWorldPropertyDangerInDirection, true );
    add_effect( action, eWorldPropertyDanger, false );
    add_operator( eWorldOperatorDangerInDirectionPlanner, action );

    action = xr_new< CStalkerDangerGrenadePlanner >( m_object,
                                                     "danger grenade planner" );
    add_condition( action, eWorldPropertyDangerGrenade, true );
    add_effect( action, eWorldPropertyDanger, false );
    add_operator( eWorldOperatorDangerGrenadePlanner, action );

    action = xr_new< CStalkerDangerBySoundPlanner >(
        m_object, "danger by sound planner" );
