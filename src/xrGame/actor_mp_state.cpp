...
IC bool is_similar( const Fvector& _0, const Fvector& _1 ) {
    return ( fsimilar( _0.x, _1.x, EPS ) && fsimilar( _0.y, _1.y, EPS ) &&
...
IC void write( const u32& bit_count,
               const u32& value,
               u32& current,
               u32& output ) {
...
bool actor_mp_state_holder::relevant( const actor_mp_state& state ) {
    ...
    m_mask |= is_similar( m_state.physics_linear_velocity.x,
                          state.physics_linear_velocity.x )
                  ? physics_linear_velocity_x_flag
                  : 0;
    m_mask |= is_similar( m_state.physics_linear_velocity.y,
                          state.physics_linear_velocity.y )
                  ? physics_linear_velocity_y_flag
                  : 0;
    m_mask |= is_similar( m_state.physics_linear_velocity.z,
                          state.physics_linear_velocity.z )
                  ? physics_linear_velocity_z_flag
                  : 0;
    m_mask |= is_similar( m_state.physics_position.x, state.physics_position.x )
                  ? physics_position_x_flag
                  : 0;
    m_mask |= is_similar( m_state.physics_position.y, state.physics_position.y )
                  ? physics_position_y_flag
                  : 0;
    m_mask |= is_similar( m_state.physics_position.z, state.physics_position.z )
                  ? physics_position_z_flag
                  : 0;
    m_mask |=
        is_similar( m_state.model_yaw, state.model_yaw ) ? model_yaw_flag : 0;
    m_mask |= is_similar( m_state.camera_yaw, state.camera_yaw )
                  ? camera_yaw_flag
                  : 0;
    m_mask |= is_similar( m_state.camera_pitch, state.camera_pitch )
                  ? camera_pitch_flag
                  : 0;
    m_mask |= is_similar( m_state.camera_roll, state.camera_roll )
                  ? camera_roll_flag
                  : 0;
    m_mask |=
        is_similar( m_state.inventory_active_slot, state.inventory_active_slot )
            ? inventory_active_slot_flag
            : 0;
    m_mask |= is_similar( m_state.body_state_flags, state.body_state_flags )
                  ? body_state_flags_flag
                  : 0;
    m_mask |= is_similar( m_state.health, state.health ) ? health_flag : 0;
    m_mask |=
        is_similar( m_state.radiation, state.radiation ) ? radiation_flag : 0;
    m_mask |=
        is_similar( m_state.physics_state_enabled, state.physics_state_enabled )
            ? physics_state_enabled_flag
            : 0;
#ifdef USE_LOGIC_ACCELERATION
    m_mask |= is_similar( m_state.logic_acceleration, state.logic_acceleration )
                  ? logic_acceleration_flag
                  : 0;
#endif // USE_LOGIC_ACCELERATION
#endif // USE_DIFFERENCES
    m_state = state;
...
void actor_mp_state_holder::write( NET_Packet& packet ) {
    ...
    clamp( m_state.physics_linear_velocity.x, min_linear_velocity_component,
           max_linear_velocity_component );
    clamp( m_state.physics_linear_velocity.y, min_linear_velocity_component,
           max_linear_velocity_component );
    clamp( m_state.physics_linear_velocity.z, min_linear_velocity_component,
           max_linear_velocity_component );

    if ( check( physics_linear_velocity_x_flag ) )
        packet.w_float_q8( m_state.physics_linear_velocity.x,
                           min_linear_velocity_component,
                           max_linear_velocity_component );
    if ( check( physics_linear_velocity_y_flag ) )
        packet.w_float_q8( m_state.physics_linear_velocity.y,
                           min_linear_velocity_component,
                           max_linear_velocity_component );
    if ( check( physics_linear_velocity_z_flag ) )
        packet.w_float_q8( m_state.physics_linear_velocity.z,
                           min_linear_velocity_component,
                           max_linear_velocity_component );
    ...
    if ( check( model_yaw_flag ) )
        packet.w_float( m_state.model_yaw );
    // packet.w_float_q8	(m_state.model_yaw		,0.f
    // ,PI_MUL_2);
    ...
    if ( check( inventory_active_slot_flag ) )
        ::write( inventory_active_slot_bits, m_state.inventory_active_slot,
                 current, output );
    if ( check( body_state_flags_flag ) )
        ::write( body_state_flags_bits, m_state.body_state_flags, current,
                 output );
    if ( check( health_flag ) )
        ::write( health_bits, pack( m_state.health, health_bits ), current,
                 output );
    if ( check( radiation_flag ) )
        ::write( radiation_bits, pack( m_state.radiation, radiation_bits ),
                 current, output );
    if ( check( physics_state_enabled_flag ) )
        ::write( physics_state_enabled_bits, m_state.physics_state_enabled,
                 current, output );
...
void actor_mp_state_holder::read( NET_Packet& packet ) {
    ...
    if ( check( physics_linear_velocity_x_flag ) )
        packet.r_float_q8( m_state.physics_linear_velocity.x,
                           min_linear_velocity_component,
                           max_linear_velocity_component );
    if ( check( physics_linear_velocity_y_flag ) )
        packet.r_float_q8( m_state.physics_linear_velocity.y,
                           min_linear_velocity_component,
                           max_linear_velocity_component );
    if ( check( physics_linear_velocity_z_flag ) )
        packet.r_float_q8( m_state.physics_linear_velocity.z,
                           min_linear_velocity_component,
                           max_linear_velocity_component );
    if ( check( physics_position_x_flag ) )
    ...
    if ( check( model_yaw_flag ) )
        packet.r_float( m_state.model_yaw );
    ...
    if ( check( inventory_active_slot_flag ) )
        m_state.inventory_active_slot =
            ::read( inventory_active_slot_bits, current, output );
    if ( check( body_state_flags_flag ) )
        m_state.body_state_flags =
            ::read( body_state_flags_bits, current, output );
    if ( check( health_flag ) )
        m_state.health =
            unpack( ::read( health_bits, current, output ), health_bits );
    if ( check( radiation_flag ) )
        m_state.radiation =
            unpack( ::read( radiation_bits, current, output ), radiation_bits );
    if ( check( physics_state_enabled_flag ) )
        m_state.physics_state_enabled =
            ::read( physics_state_enabled_bits, current, output );
}
