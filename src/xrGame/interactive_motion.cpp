...
void interactive_motion_diagnostic( LPCSTR message,
    ...
    Msg( "death anims - interactive_motion:- %s, motion: %s, obj: %s, model:  "
         "%s ",
         message, motion_name, O->cName().c_str(), O->cNameVisual().c_str() );
#endif
}
...
void interactive_motion::setup( const MotionID& m,
                                CPhysicsShell* s,
                                float _angle ) {
    VERIFY( s );
    VERIFY( m.valid() );
...
void interactive_motion::anim_callback( CBlend* B ) {
    ...
    ( ( interactive_motion* )( B->CallbackParam ) )
...
void interactive_motion::play() {
    ...
    smart_cast< IKinematicsAnimated* >( shell->PKinematics() )
        ->PlayCycle( motion, TRUE, anim_callback, this );
    state_start();
}
