    ...
    if ( !g_dedicated_server ) {
        ...
        m_pMB_ErrDlgs[ PatchDownloadSuccess ]->AddCallbackStr(
            ...
            CUIWndCallback::void_function( this,
                                           &CMainMenu::OnRunDownloadedPatch ) );
        m_pMB_ErrDlgs[ PatchDownloadSuccess ]->AddCallbackStr(
        ...
        m_pMB_ErrDlgs[ DownloadMPMap ]->AddCallbackStr(
            "button_yes", MESSAGE_BOX_YES_CLICKED,
            CUIWndCallback::void_function( this,
                                           &CMainMenu::OnDownloadMPMap ) );

        // m_account_mngr			=
        // xr_new<gamespy_gp::account_manager>
        // (m_pGameSpyFull->GetGameSpyGP()); m_login_mngr =
        // xr_new<gamespy_gp::login_manager> (m_pGameSpyFull); m_profile_store
        // = xr_new<gamespy_profile::profile_store>	(m_pGameSpyFull);
        // m_stats_submitter		=
        // xr_new<gamespy_profile::stats_submitter>	(m_pGameSpyFull);
        // m_atlas_submit_queue	= xr_new<atlas_submit_queue>
        // (m_stats_submitter);
    }
...
CMainMenu::~CMainMenu() {
    ...
    // xr_delete (m_account_mngr); xr_delete
    // (m_login_mngr); xr_delete
    // (m_profile_store); xr_delete
    // (m_stats_submitter); xr_delete
    // (m_atlas_submit_queue);
    //
...
void CMainMenu::ReadTextureInfo() {
    FS_FileSet fset;
    FS.file_list( fset, "$game_config$", FS_ListFiles,
                  "ui\\textures_descr\\*.xml" );
...
void CMainMenu::OnFrame() {
    ...
    // screenshot stuff
    if ( m_Flags.test( flGameSaveScreenshot ) &&
         Device.dwFrame > m_screenshotFrame ) {
        m_Flags.set( flGameSaveScreenshot, FALSE );
        ::Render->Screenshot( IRender_interface::SM_FOR_GAMESAVE,
                              m_screenshot_name );
    ...
    if ( IsActive() ) {
        ...
        if ( b_is_16_9 != m_activatedScreenRatio ) {
            ReloadUI();
            m_startDialog->SendMessage( m_startDialog, MAIN_MENU_RELOADED,
                                        NULL );
        }
    }
}
...
void CMainMenu::Screenshot( IRender_interface::ScreenshotMode mode,
                            LPCSTR name ) {
...
void CMainMenu::UnregisterPPDraw( CUIWindow* w ) {
    m_pp_draw_wnds.erase(
        std::remove( m_pp_draw_wnds.begin(), m_pp_draw_wnds.end(), w ),
        m_pp_draw_wnds.end() );
}
...
void CMainMenu::DestroyInternal( bool bForce ) {
    if ( m_startDialog &&
         ( ( m_deactivated_frame < Device.dwFrame + 4 ) || bForce ) )
        xr_delete( m_startDialog );
}
...
void CMainMenu::OnNewPatchFound( LPCSTR VersionName, LPCSTR URL ) {
    ...
    if ( !m_pMB_ErrDlgs[ NewPatchFound ] ) {
        ...
        shared_str tmpText;
        tmpText.printf( m_pMB_ErrDlgs[ NewPatchFound ]->GetText(), VersionName,
                        URL );
...
void CMainMenu::OnDownloadPatch( CUIWindow*, void* ) {
    ...
    // m_pGameSpyFull->GetGameSpyHTTP()->DownloadFile(*m_sPatchURL,
    // *m_sPatchFileName);
}
...
void CMainMenu::OnSessionTerminate( LPCSTR reason ) {
    if ( m_NeedErrDialog == SessionTerminate &&
         ( Device.dwTimeGlobal - m_start_time ) < 8000 )
        return;
...
void CMainMenu::OnDownloadPatchProgress( u64 bytesReceived, u64 totalSize ) {
    m_sPDProgress.Progress =
        ( float( bytesReceived ) / float( totalSize ) ) * 100.0f;
};
