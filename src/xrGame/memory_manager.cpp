...
CMemoryManager::CMemoryManager( CEntityAlive* entity_alive,
...
void CMemoryManager::update( float time_delta ) {
    ...
    if ( m_stalker )
        registered_in_combat =
            m_stalker->agent_manager().member().registered_in_combat(
                m_stalker );
...
void CMemoryManager::update( const xr_vector< T >& objects, bool add_enemies ) {
    ...
    squad_mask_type mask =
        m_stalker ? m_stalker->agent_manager().member().mask( m_stalker ) : 0;
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( add_enemies ) {
            const CEntityAlive* entity_alive =
                smart_cast< const CEntityAlive* >( ( *I ).m_object );
...
CMemoryInfo CMemoryManager::memory( const CObject* object ) const {
    ...
    VERIFY( game_object );
    squad_mask_type mask =
        m_stalker ? m_stalker->agent_manager().member().mask( m_stalker )
                  : squad_mask_type( -1 );

    {
        typename xr_vector< CVisibleObject >::const_iterator I =
            std::find( visual().objects().begin(), visual().objects().end(),
                       object_id( object ) );
        if ( visual().objects().end() != I ) {
            ( CMemoryObject< CGameObject >& )result =
                ( CMemoryObject< CGameObject >& )( *I );
    ...
    {
        typename xr_vector< CSoundObject >::const_iterator I =
            std::find( sound().objects().begin(), sound().objects().end(),
                       object_id( object ) );
        if ( ( sound().objects().end() != I ) &&
             ( level_time < ( *I ).m_level_time ) ) {
            ( CMemoryObject< CGameObject >& )result =
                ( CMemoryObject< CGameObject >& )( *I );
    ...
    {
        typename xr_vector< CHitObject >::const_iterator I =
            std::find( hit().objects().begin(), hit().objects().end(),
                       object_id( object ) );
        if ( ( hit().objects().end() != I ) &&
             ( level_time < ( *I ).m_level_time ) ) {
            ( CMemoryObject< CGameObject >& )result =
                ( CMemoryObject< CGameObject >& )( *I );
...
void CMemoryManager::make_object_visible_somewhen( const CEntityAlive* enemy ) {
    if ( !enemy ||
         enemy->getDestroy() ) // safety check if enemy disappears (usual
                               // scenario for fast fight command)
        return;

    squad_mask_type mask =
        stalker().agent_manager().member().mask( &stalker() );
    ...
    MemorySpace::CVisibleObject* obj1 =
        object().memory().visual().visible_object( enemy );
    if ( obj1 ) {
        //		Msg						("[%6d]
        //make_object_visible_somewhen [%s] =
        //%x",Device.dwTimeGlobal,*enemy->cName(),obj1->m_squad_mask.get());
        obj1->visible( mask, prev );
    } else {
        Msg( "![CMemoryManager::make_object_visible_somewhen] tried to make "
             "obj1 visible, but its nullptr" );
    }
}
