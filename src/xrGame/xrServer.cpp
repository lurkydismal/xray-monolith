...
CSE_Abstract* xrServer::ID_to_entity( u16 ID ) {
    // #pragma todo("??? to all : ID_to_entity - must be replaced to
    // 'game->entity_from_eid()'")
    if ( 0xffff == ID )
        return 0;
...
void xrServer::client_Destroy( IClient* C ) {
    ...
    if ( alife_client ) {
        CSE_Abstract* pOwner =
            static_cast< xrClientData* >( alife_client )->owner;
        ...
        do {
            it = std::find( m_aDelayedPackets.begin(), m_aDelayedPackets.end(),
                            pp );
...
void xrServer::MakeUpdatePackets() {
    ...
    for ( ; I != E; ++I ) {
        ...
        // write specific data
        {
            ...
            u32 ObjectSize =
                u32( tmpPacket.w_tell() - position ) - sizeof( u8 );
...
void xrServer::SendUpdatePacketsToAll() {
    ...
    for ( update_iterator_t i = m_update_begin; i != m_update_end; ++i ) {
        ...
        if ( to_send.B.count > 2 ) {
            ...
            SendBroadcast( GetServerClient()->ID, to_send,
                           net_flags( FALSE, TRUE ) );
            if ( Level().IsDemoSave() ) {
    ...
    ClientID sender ) // Non-Zero means broadcasting with "flags" as returned
{
    ...
    // R_ASSERT2						(CL,
    // make_string("packet type [%d]",type).c_str());

    switch ( type ) {
        case M_CLIENT_REQUEST_CONNECTION_DATA: {
            IClient* tmp_client =
                net_players.GetFoundClient( ClientIdSearchPredicate( sender ) );
            VERIFY( tmp_client );
        ...
        case M_REMOTE_CONTROL_CMD: {
            if ( CL->m_admin_rights.m_has_admin_rights ) {
                string1024 buff;
                P.r_stringZ( buff );
                Msg( "* Radmin [%s] is running command: %s", CL->ps->getName(),
                     buff );
    ...
    ClientID sender ) // Non-Zero means broadcasting with "flags" as returned
{
    ...
    switch ( type ) {
        ...
        case M_CREATE_PLAYER_STATE: {
            game->AddDelayedEvent( P, GAME_EVENT_CREATE_PLAYER_STATE, 0,
                                   sender );
        } break;
        ...
        case M_STATISTIC_UPDATE_RESPOND: {
            // client method for collecting statistics are called from two
            // places : 1 - this, 2 - game_sv_mp::WritePlayerStats
            if ( GameID() != eGameIDSingle ) {
                ...
                if ( CL ) {
                    ...
                    if ( static_cast< IClient* >( CL ) != GetServerClient() ) {
                        ...
                        u32 tmp_pid =
                            tmp_ps != NULL ? tmp_ps->m_account.profile_id() : 0;
                        Game().m_WeaponUsageStatistic->OnUpdateRespond(
                            &P, CL->m_cdkey_digest, tmp_pid );
                    }
                } else {
                    Msg( "! ERROR: SV: update respond received from unknown "
                         "sender" );
                }
            }
            // if (SV_Client) SendTo	(SV_Client->ID, P, net_flags(TRUE,
            // TRUE));
        } break;
        case M_PLAYER_FIRE: {
            if ( game )
        ...
        case M_REMOTE_CONTROL_AUTH: {
            ...
            if ( 0 == stricmp( user.c_str(), "logoff" ) ) {
                ...
                if ( res ) {
                    ...
                    Msg( "# User [%s] logged as remote administrator.",
                         user.c_str() );
                } else
                    Msg( "# User [%s] tried to login as remote administrator. "
                         "Access denied.",
                         user.c_str() );
            }
            NET_Packet P_answ;
...
bool xrServer::CheckAdminRights( const shared_str& user,
                                 const shared_str& pass,
                                 string512& reason ) {
    bool res = false;
    string_path fn;
    ...
    if ( FS.exist( fn ) ) {
        ...
        if ( ini.line_exist( "radmins", user.c_str() ) ) {
            if ( ini.r_string( "radmins", user.c_str() ) == pass ) {
                ...
                xr_strcpy( reason, sizeof( reason ),
                           "Access denied. Wrong password." );
            }
        } else
            xr_strcpy( reason, sizeof( reason ),
                       "Access denied. No such user." );
    } else
...
void xrServer::SendTo_LL( ClientID ID,
                          void* data,
                          u32 size,
                          u32 dwFlags,
                          u32 dwTimeout ) {
    if ( ( SV_Client && SV_Client->ID == ID ) || ( psNET_direct_connect ) ) {
        ...
        VERIFY2( pClient && pClient->flags.bConnected,
                 "trying to send packet to disconnected client" );
...
void xrServer::SendBroadcast( ClientID exclude, NET_Packet& P, u32 dwFlags ) {
    ...
    struct ClientSenderFunctor {
        ...
        ClientSenderFunctor( xrServer* owner,
                             void* data,
                             u32 size,
                             u32 dwFlags )
            : m_owner( owner ),
...
void xrServer::entity_Destroy( CSE_Abstract*& P ) {
    ...
    if ( dbg_net_Draw_Flags.test( dbg_destroy ) )
        Msg( "xrServer::entity_Destroy : [%d][%s][%s]", P->ID, P->name(),
             P->name_replace() );
#endif
    R_ASSERT( P );
    entities.erase( P->ID );
...
bool xrServer::verify_entities() const {
    ...
    for ( ; I != E; ++I ) {
        VERIFY2( ( *I ).first != 0xffff,
                 "SERVER : Invalid entity id as a map key - 0xffff" );
        VERIFY2( ( *I ).second, "SERVER : Null entity object in the map" );
        VERIFY3( ( *I ).first == ( *I ).second->ID,
                 "SERVER : ID mismatch - map key doesn't correspond to the "
                 "real entity ID",
...
void xrServer::verify_entity( const CSE_Abstract* entity ) const {
    ...
    if ( entity->ID_Parent != 0xffff ) {
        ...
        VERIFY2( J != entities.end(),
                 make_string( "SERVER : Cannot find parent in the map [%s][%s]",
                              entity->name_replace(), entity->name() )
                     .c_str() );
        VERIFY3( ( *J ).second, "SERVER : Null entity object in the map",
                 entity->name_replace() );
        VERIFY3( ( *J ).first == ( *J ).second->ID,
                 "SERVER : ID mismatch - map key doesn't correspond to the "
                 "real entity ID",
                 ( *J ).second->name_replace() );
        VERIFY3( std::find( ( *J ).second->children.begin(),
                            ( *J ).second->children.end(),
                            entity->ID ) != ( *J ).second->children.end(),
                 "SERVER : Parent/Children relationship mismatch - Object has "
                 "parent, but corresponding parent doesn't have children",
                 ( *J ).second->name_replace() );
    }
    ...
    for ( ; I != E; ++I ) {
        VERIFY3( *I != 0xffff, "SERVER : Invalid entity children id - 0xffff",
                 entity->name_replace() );
        xrS_entities::const_iterator J = entities.find( *I );
        VERIFY3( J != entities.end(),
                 "SERVER : Cannot find children in the map",
                 entity->name_replace() );
        VERIFY3( ( *J ).second, "SERVER : Null entity object in the map",
                 entity->name_replace() );
        VERIFY3( ( *J ).first == ( *J ).second->ID,
        ...
        VERIFY3( ( *J ).second->ID_Parent == entity->ID,
                 "SERVER : Parent/Children relationship mismatch - Object has "
                 "children, but children doesn't have parent",
                 ( *J ).second->name_replace() );
    }
}
    ...
    struct MaxPingClientDisconnector {
        ...
        void operator()( IClient* client ) {
            ...
            if ( ps->ping > g_sv_dwMaxClientPing &&
                 Client->m_ping_warn.m_dwLastMaxPingWarningTime +
                         g_sv_time_for_ping_check <
                     Device.dwTimeGlobal ) {
                ++Client->m_ping_warn.m_maxPingWarnings;
                Client->m_ping_warn.m_dwLastMaxPingWarningTime =
                    Device.dwTimeGlobal;

                if ( Client->m_ping_warn.m_maxPingWarnings >=
                     g_sv_maxPingWarningsCount ) {
                    // kick
                    LPSTR reason;
                    STRCONCAT( reason, CStringTable()
...
void xrServer::GetServerInfo( CServerInfo* si ) {
    ...
    si->AddItem( "Server port", itoa( GetPort(), tmp, 10 ),
                 RGB( 128, 128, 255 ) );
    LPCSTR time =
        InventoryUtilities::GetTimeAsString(
            Device.dwTimeGlobal, InventoryUtilities::etpTimeToSecondsAndDay )
            .c_str();
    ...
    if ( game->Type() == eGameIDDeathmatch ||
         game->Type() == eGameIDTeamDeathmatch ) {
        xr_strcat( tmp256, " [" );
        xr_strcat( tmp256, itoa( g_sv_dm_dwFragLimit, tmp, 10 ) );
        xr_strcat( tmp256, "] " );
    } else if ( game->Type() == eGameIDArtefactHunt ||
                game->Type() == eGameIDCaptureTheArtefact ) {
        xr_strcat( tmp256, " [" );
    ...
    if ( game->Type() == eGameIDArtefactHunt ||
         game->Type() == eGameIDCaptureTheArtefact ) {
        xr_strcat( tmp256, " RT [" );
    ...
    if ( g_pGameLevel ) {
        time = InventoryUtilities::GetGameTimeAsString(
...
void xrServer::AddCheater( shared_str const& reason,
                           ClientID const& cheaterID ) {
...
void xrServer::KickCheaters() {
    for ( cheaters_t::iterator i = m_cheaters.begin(), ie = m_cheaters.end();
          i != ie; ++i ) {
        IClient* tmp_client = GetClientByID( i->cheater_id );
        if ( !tmp_client ) {
            Msg( "! ERROR: KickCheaters: client [%u] not found",
                 i->cheater_id );
            continue;
        }
...
void xrServer::MakeScreenshot( ClientID const& admin_id,
                               ClientID const& cheater_id ) {
    if ( ( cheater_id == SV_Client->ID ) && g_dedicated_server ) {
        return;
    }
    for ( int i = 0;
          i < sizeof( m_screenshot_proxies ) / sizeof( clientdata_proxy* );
          ++i ) {
        if ( !m_screenshot_proxies[ i ]->is_active() ) {
            m_screenshot_proxies[ i ]->make_screenshot( admin_id, cheater_id );
            Msg( "* admin [%d] is making screeshot of client [%d]", admin_id,
                 cheater_id );
            return;
        }
    }
    Msg( "! ERROR: SV: not enough file transfer proxies for downloading "
         "screenshot, please try later ..." );
}

void xrServer::MakeConfigDump( ClientID const& admin_id,
                               ClientID const& cheater_id ) {
    if ( ( cheater_id == SV_Client->ID ) && g_dedicated_server ) {
        return;
    }
    for ( int i = 0;
        ...
        if ( !m_screenshot_proxies[ i ]->is_active() ) {
            ...
            Msg( "* admin [%d] is making config dump of client [%d]", admin_id,
                 cheater_id );
            return;
        }
    }
    Msg( "! ERROR: SV: not enough file transfer proxies for downloading file, "
         "please try later ..." );
}
...
void xrServer::initialize_screenshot_proxies() {
    for ( int i = 0;
          i < sizeof( m_screenshot_proxies ) / sizeof( clientdata_proxy* );
          ++i ) {
        m_screenshot_proxies[ i ] =
            xr_new< clientdata_proxy >( m_file_transfers );
    }
}
