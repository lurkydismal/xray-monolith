...
void CSightManager::reload( LPCSTR section ) {
    m_max_left_angle = deg2rad( READ_IF_EXISTS(
        pSettings, r_float, section, "max_left_torso_angle", 90.f ) );
    m_max_right_angle = deg2rad( READ_IF_EXISTS(
        pSettings, r_float, section, "max_right_torso_angle", 60.f ) );
}
...
static inline float select_speed( float const distance,
                                  float const speed,
...
void CSightManager::Exec_Look( float time_delta ) {
    ...
    if ( object().cName() == "level_prefix_stalker" ) {
        Msg( "[%6d][%s] BEFORE BODY [%f] -> [%f]", Device.dwTimeGlobal,
             object().cName().c_str(), object().movement().m_body.current.yaw,
             object().movement().m_body.target.yaw );
        Msg( "[%6d][%s] BEFORE HEAD [%f] -> [%f]", Device.dwTimeGlobal,
             object().cName().c_str(), object().movement().m_head.current.yaw,
    ...
    // angle_normalize_signed(head.current.yaw),   0xff00ff00, 0 );
    // s_stats_graph->AppendItem			(
    // angle_normalize_signed(head.current.pitch), 0xffff0000, 1 );

#ifdef DEBUG
    if ( g_ai_dbg_sight )
        Msg( "%6d [%s] before body[%f]->[%f], head[%f]->[%f]",
             Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw,
             body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
    vfValidateAngleDependency( body.current.yaw, body.target.yaw,
                               head.current.yaw );
#ifdef DEBUG
    if ( g_ai_dbg_sight )
        Msg( "%6d [%s] after  body[%f]->[%f], head[%f]->[%f]",
             Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw,
             body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
    ...
    m_object->angle_lerp_bounds(
        ...
        select_speed( angle_difference( body.current.yaw, body.target.yaw ),
                      body_speed, m_object->aim_min_speed(),
                      m_object->aim_min_angle(), m_object->aim_max_angle() ),
        time_delta );
    m_object->angle_lerp_bounds(
        body.current.pitch, body.target.pitch,
        select_speed( angle_difference( body.current.pitch, body.target.pitch ),
                      body_speed, m_object->aim_min_speed(),
                      m_object->aim_min_angle(), m_object->aim_max_angle() ),
        time_delta );
    ...
    m_object->angle_lerp_bounds(
        ...
        select_speed( angle_difference( head.current.yaw, head.target.yaw ),
                      head_speed, m_object->aim_min_speed(),
                      m_object->aim_min_angle(), m_object->aim_max_angle() ),
        time_delta );
    m_object->angle_lerp_bounds(
        head.current.pitch, head.target.pitch,
        select_speed( angle_difference( head.current.pitch, head.target.pitch ),
                      head_speed, m_object->aim_min_speed(),
                      m_object->aim_min_angle(), m_object->aim_max_angle() ),
        time_delta );
...
#ifdef DEBUG
    if ( g_ai_dbg_sight )
        Msg( "%6d [%s] after2 body[%f]->[%f], head[%f]->[%f]",
             Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw,
             body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
    ...
    if ( object().cName() == "level_prefix_stalker" ) {
        Msg( "[%6d][%s] AFTER  BODY [%f] -> [%f]", Device.dwTimeGlobal,
             object().cName().c_str(), object().movement().m_body.current.yaw,
             object().movement().m_body.target.yaw );
        Msg( "[%6d][%s] AFTER  HEAD [%f][%f] -> [%f][%f]", Device.dwTimeGlobal,
             object().cName().c_str(), object().movement().m_head.current.yaw,
...
#ifdef DEBUG
    if ( g_ai_dbg_sight )
        Msg( "%6d [%s] after3 body[%f]->[%f], head[%f]->[%f]",
             Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw,
             body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
...
void CSightManager::setup( const CSightAction& sight_action ) {
    ...
    if ( !m_actions.empty() &&
         ( *( *m_actions.begin() ).second == sight_action ) )
        return;

    clear();
    add_action( 0,
                xr_new< CSightControlAction >( 1.f, u32( -1 ), sight_action ) );
}
...
void CSightManager::update() {
    ...
    if ( !m_turning_in_place ) {
        if ( angle_difference( object().movement().m_body.current.yaw,
             ...
             ( left_angle( -object().movement().m_head.current.yaw,
...
Fvector CSightManager::aiming_position() const {
    ...
    switch ( current_action().sight_type() ) {
        case eSightTypeCurrentDirection: {
            VERIFY2(
                _valid( object().Position() ),
                make_string( "[%f][%f][%f]", VPUSH( object().Position() ) ) );
            VERIFY2(
        ...
        case eSightTypePathDirection: {
            result.mad(
                object().Position(),
                Fvector().setHP( -object().movement().m_head.target.yaw,
                                 -object().movement().m_head.target.pitch ),
                fake_distance );
            VERIFY2( result.magnitude() < 100000.f,
                     make_string( "[%f][%f][%f] [%f][%f] [%f]",
        ...
        case eSightTypeDirection: {
            ...
            result.mad( object().Position(), current_action().vector3d(),
                        fake_distance );
            VERIFY2( result.magnitude() < 100000.f,
                     make_string( "[%f][%f][%f] [%f][%f][%f] [%f]",
                                  VPUSH( object().Position() ),
        ...
        case eSightTypeObject: {
            ...
            VERIFY2( result.magnitude() < 100000.f,
                     make_string( "[%f][%f][%f]", VPUSH( result ) ) );
            VERIFY( _valid( result ) );
            break;
        }
        ...
        case eSightTypeAnimationDirection: {
            result.mad(
                object().Position(),
                Fvector().setHP( -object().movement().m_body.current.yaw,
    ...
    VERIFY2( result.magnitude() < 100000.f,
             make_string( "[%f][%f][%f] [%f][%f] [%f]", VPUSH( result ) ) );
    VERIFY( _valid( result ) );
    return ( result );
}
...
void CSightManager::process_action( float const time_delta ) {
    ...
    Fvector const& factors =
        current_action().use_torso_look() ? s_danger_factors : s_free_factors;
    ...
    m_current.m_head.m_factor = lerp( m_current.m_head.m_factor, factors.x,
                                      s_factor_lerp_speed * time_delta );
    VERIFY( _valid( m_current.m_head.m_factor ) );

    VERIFY( _valid( m_current.m_shoulder.m_factor ) );
    m_current.m_shoulder.m_factor =
        lerp( m_current.m_shoulder.m_factor, factors.y,
              s_factor_lerp_speed * time_delta );
    VERIFY( _valid( m_current.m_shoulder.m_factor ) );

    VERIFY( _valid( m_current.m_spine.m_factor ) );
    m_current.m_spine.m_factor = lerp( m_current.m_spine.m_factor, factors.z,
                                       s_factor_lerp_speed * time_delta );
    VERIFY( _valid( m_current.m_spine.m_factor ) );
    ...
    m_current.m_head.m_rotation.setXYZ(
        Fvector( angles ).mul( m_current.m_head.m_factor ) );
    m_current.m_shoulder.m_rotation.setXYZ(
        Fvector( angles ).mul( m_current.m_shoulder.m_factor ) );
    m_current.m_spine.m_rotation.setXYZ(
        Fvector( angles ).mul( m_current.m_spine.m_factor ) );
}

void CSightManager::compute_aiming( float const time_delta,
                                    float const angular_speed ) {
    switch ( m_aiming_type ) {
        ...
        case aiming_weapon: {
            ...
            bool forward_blend_callbacks =
                object().animation().forward_blend_callbacks();
            bool backward_blend_callbacks =
                object().animation().backward_blend_callbacks();
            ...
            aimers::weapon aimer(
                &object(), m_animation_id.c_str(),
                m_animation_frame == animation_frame_start, aiming_position(),
        ...
        case aiming_head: {
            ...
            object().animation().remove_bone_callbacks();
            VERIFY( _valid( aiming_position() ) );
            aimers::bone< 3 > aimer( &object(), m_animation_id.c_str(),
                                     m_animation_frame == animation_frame_start,
                                     aiming_position(), bones );
            if ( forward_blend_callbacks )
                object().animation().assign_bone_blend_callbacks( true );
            else {
                if ( backward_blend_callbacks )
            ...
            if ( !forward_blend_callbacks && !backward_blend_callbacks ) {
                if ( !fis_zero( time_delta ) ) {
                    ...
                    if ( m_object->animation_movement() )
                        slerp_rotations(
                            time_delta,
                            m_object->animation_movement()->IsBlending()
                                ? .1f
                                : angular_speed );
                }
...
static void slerp_rotations( float const time_delta,
                             float const angular_speed,
                             Fmatrix& current,
                             Fmatrix const& target ) {
...
void CSightManager::slerp_rotations( float const time_delta,
                                     float const angular_speed ) {
    ::slerp_rotations( time_delta, angular_speed, m_current.m_spine.m_rotation,
                       m_target.m_spine.m_rotation );
    ::slerp_rotations( time_delta, angular_speed,
                       m_current.m_shoulder.m_rotation,
                       m_target.m_shoulder.m_rotation );
    ::slerp_rotations( time_delta, angular_speed, m_current.m_head.m_rotation,
                       m_target.m_head.m_rotation );
}
...
void CSightManager::adjust_orientation() {
    ...
    SBoneRotation& body = object().movement().m_body;
    object().XFORM().getXYZ( body.current.pitch, body.current.yaw,
                             body.current.roll );
