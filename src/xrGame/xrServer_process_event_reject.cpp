...
bool xrServer::Process_event_reject( NET_Packet& P,
    ...
    VERIFY2( e_entity, make_string( "entity not found. parent_id = [%d], "
                           ...
                           .c_str() );
    if ( !e_entity ) {
        Msg( "! ERROR on rejecting: entity not found. parent_id = [%d], "
             "entity_id = [%d], frame = [%d].",
             id_parent, id_entity, Device.dwFrame );
        return false;
    }
    ...
    VERIFY2( e_parent, make_string( "parent not found. parent_id = [%d], "
                           ...
                           .c_str() );
    if ( !e_parent ) {
        Msg( "! ERROR on rejecting: parent not found. parent_id = [%d], "
             "entity_id = [%d], frame = [%d].",
             id_parent, id_entity, Device.dwFrame );
        return false;
    }
...
#ifdef MP_LOGGING
    Msg( "--- SV: Process reject: parent[%d][%s], item[%d][%s]", id_parent,
         e_parent->name_replace(), id_entity, e_entity->name() );
#endif // MP_LOGGING
    ...
    if ( c == C.end() ) {
        ...
        for ( const u16& childID : e_parent->children ) {
            clildrenList.append( "! " )
                .append( game->get_entity_from_eid( childID )->name_replace() )
                .append( "\n" );
        }
        Msg( "! WARNING: SV: can't find child [%s] of parent [%s]! Children "
             "list:\n%s",
             e_entity->name_replace(), e_parent->name_replace(),
             clildrenList.c_str() );
        return false;
    }
    ...
    if ( 0xffff == e_entity->ID_Parent ) {
        ...
        Msg( "! ERROR: can't detach independant object. entity[%s][%d], "
             ...
             e_entity->name_replace(), id_entity, e_parent->name_replace(),
             id_parent, e_entity->s_name.c_str() );
    ...
    if ( e_entity->ID_Parent != id_parent ) {
        Msg( "! ERROR: e_entity->ID_Parent = [%d]  parent = [%d][%s]  "
             "entity_id = [%d]  frame = [%d]",
             e_entity->ID_Parent, id_parent, e_parent->name_replace(),
