...
void CDamageManager::reload( LPCSTR section, CInifile const* ini ) {
    ...
    if ( section_exist ) {
        ...
        if ( ini->line_exist( section, "default" ) ) {
            ...
            m_default_hit_factor =
                ( float )atof( _GetItem( value, 0, buffer ) );
            m_default_wound_factor =
                ( float )atof( _GetItem( value, 2, buffer ) );
        }
    }
...
void CDamageManager::load_section( LPCSTR section, CInifile const* ini ) {
    ...
    for ( CInifile::SectCIt i = damages.Data.begin(); damages.Data.end() != i;
          ++i ) {
        if ( xr_strcmp( *( *i ).first, "default" ) ) {
            ...
            if ( BI_NONE == bone )
                Debug.fatal( DEBUG_INFO,
                             "!CDamageManager::load_section ERROR: BI_NONE == "
                             "bone, section %s, bone %s",
                             section, i->first.c_str() );
            CBoneInstance& bone_instance =
                kinematics->LL_GetBoneInstance( u16( bone ) );
            bone_instance.set_param(
                0, ( float )atof( _GetItem( *( *i ).second, 0, buffer ) ) );
            bone_instance.set_param(
                1, ( float )atoi( _GetItem( *( *i ).second, 1, buffer ) ) );
            bone_instance.set_param(
                2, ( float )atof( _GetItem( *( *i ).second, 2, buffer ) ) );
            if ( _GetItemCount( *( *i ).second ) < 4 ) {
                bone_instance.set_param(
                    3, ( float )atof( _GetItem( *( *i ).second, 0, buffer ) ) );
            } else {
                bone_instance.set_param(
                    3, ( float )atof( _GetItem( *( *i ).second, 3, buffer ) ) );
            }
            if ( 0 == bone && ( fis_zero( bone_instance.get_param( 0 ) ) ||
                                fis_zero( bone_instance.get_param( 2 ) ) ) ) {
                string256 error_str;
                xr_sprintf( error_str,
                            "hit_scale and wound_scale for root bone cannot be "
                            "zero. see section [%s]",
                            section );
...
void CDamageManager::HitScale( const int element,
                               float& hit_scale,
                               float& wound_scale,
                               bool aim_bullet ) {
