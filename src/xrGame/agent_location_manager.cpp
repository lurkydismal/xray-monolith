...
IC CAgentLocationManager::CDangerLocationPtr CAgentLocationManager::location(
    const Fvector& position ) {
    LOCATIONS::iterator I =
        std::find_if( m_danger_locations.begin(), m_danger_locations.end(),
...
bool CAgentLocationManager::suitable( CAI_Stalker* object,
                                      const CCoverPoint* location,
                                      bool use_enemy_info ) const {
    CAgentMemberManager::const_iterator I =
        this->object().member().members().begin();
    CAgentMemberManager::const_iterator E =
        this->object().member().members().end();
    for ( ; I != E; ++I ) {
        if ( ( *I )->object().ID() == object->ID() )
            continue;

        if ( !( *I )->cover() ) {
            if ( this->object().member().registered_in_combat(
                     &( *I )->object() ) )
                continue;

            if ( ( *I )->object().Position().distance_to_sqr(
                     location->position() ) <= _sqr( 5.f ) )
                return ( false );
        ...
        if ( ( *I )->cover()->m_position.distance_to_sqr(
                 location->position() ) <= _sqr( 5.f ) )
            // so member cover is too close
            //			if
            //((*I)->object().Position().distance_to_sqr(location->position())
    ...
    if ( use_enemy_info ) {
        CAgentEnemyManager::ENEMIES::const_iterator I =
            this->object().enemy().enemies().begin();
        CAgentEnemyManager::ENEMIES::const_iterator E =
            this->object().enemy().enemies().end();
        for ( ; I != E; ++I )
            if ( ( *I ).m_enemy_position.distance_to_sqr(
                     location->position() ) <
...
void CAgentLocationManager::make_suitable( CAI_Stalker* object,
                                           const CCoverPoint* location ) const {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( ( *I )->cover()->m_position.distance_to_sqr(
                 location->position() ) <= _sqr( 5.f ) ) {
            //			Msg
            //("%6d : object [%s] disabled cover for object
...
void CAgentLocationManager::remove_old_danger_covers() {
    m_danger_locations.erase(
        std::remove_if( m_danger_locations.begin(), m_danger_locations.end(),
                        CRemoveOldDangerCover( &object().member() ) ),
        m_danger_locations.end() );
}

float CAgentLocationManager::danger( const CCoverPoint* cover,
                                     CAI_Stalker* member ) const {
    float result = 1;
    ...
    for ( ; I != E; ++I ) {
        ...
        float distance =
            1.f + ( *I )->position().distance_to( cover->position() );
        if ( distance > ( *I )->m_radius )
            continue;

        result *= float( Device.dwTimeGlobal - ( *I )->m_level_time ) /
                  float( ( *I )->m_interval );
    }
