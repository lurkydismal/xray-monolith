...
ICF static BOOL GetPickDist_Callback( collide::rq_result& result,
                                      LPVOID params ) {
    collide::rq_result* RQ = ( collide::rq_result* )params;

    if ( result.O ) {
        if ( CCustomRocket* pRocket =
                 smart_cast< CCustomRocket* >( result.O ) ) {
            if ( !pRocket->Useful() )
                return TRUE;
        }
        ...
        if ( pMtl && ( pMtl->Flags.is( SGameMtl::flPassable ) ||
                       pMtl->Flags.is( SGameMtl::flActorObstacle ) ) )
            return TRUE;
    }
...
collide::rq_result GetPickResult( Fvector pos,
    ...
    collide::ray_defs RD( pos, dir, RQ.range, CDB::OPT_FULL_TEST,
                          collide::rqtBoth );
    Level().ObjectSpace.RayQuery( RQR, RD, GetPickDist_Callback, &RQ, nullptr,
                                  ignore );
    return RQ;
}
...
void CCameraLook::UpdateDistance( Fvector& point ) {
    ...
    g_pGameLevel->ObjectSpace.RayPick( point, vDir, dist + covariance,
                                       collide::rqtBoth, R, parent );

    float d = psCamSlideInert * prev_d +
              ( 1.f - psCamSlideInert ) * ( R.range - covariance );
    prev_d = d;
...
void CCameraLook::OnActivate( CCameraBase* old_cam ) {
    if ( old_cam && ( m_Flags.is( flRelativeLink ) ==
                      old_cam->m_Flags.is( flRelativeLink ) ) ) {
        yaw = old_cam->yaw;
    ...
    if ( !m_locked_enemy ) {
        ...
        if ( pInput->iGetAsyncKeyState( cam_dik ) ) {
            const CVisualMemoryManager::VISIBLES& vVisibles =
                Actor()->memory().visual().objects();
            CVisualMemoryManager::VISIBLES::const_iterator v_it =
                vVisibles.begin();
            float _nearest_dst = flt_max;

            for ( ; v_it != vVisibles.end(); ++v_it ) {
                const CObject* _object_ = ( *v_it ).m_object;
                if ( !Actor()->memory().visual().visible_now(
                ...
                float d =
                    object_->Position().distance_to_xz( Actor()->Position() );
                if ( !m_locked_enemy || d < _nearest_dst ) {
                    m_locked_enemy = object_;
                    _nearest_dst = d;
                }
            }
            //.			if(m_locked_enemy) Msg("enemy is %s",
            //*m_locked_enemy->cNameSect() );
        }
    } else {
        if ( !pInput->iGetAsyncKeyState( cam_dik ) ) {
            m_locked_enemy = NULL;
...
void CCameraLook2::UpdateAutoAim() {
    ...
    yaw = angle_inertion_var( yaw, xyz.y, m_autoaim_inertion_yaw.x,
                              m_autoaim_inertion_yaw.y, PI, Device.fTimeDelta );

    pitch =
        angle_inertion_var( pitch, xyz.x, m_autoaim_inertion_pitch.x,
                            m_autoaim_inertion_pitch.y, PI, Device.fTimeDelta );
}

void CCameraLook2::Load( LPCSTR section ) {
    CCameraLook::Load( section );
    m_cam_offset = pSettings->r_fvector3( section, "offset" );
    m_autoaim_inertion_yaw =
        pSettings->r_fvector2( section, "autoaim_speed_y" );
    m_autoaim_inertion_pitch =
        pSettings->r_fvector2( section, "autoaim_speed_x" );
}
