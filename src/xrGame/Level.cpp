...
void CLevel::ProcessPrefetchEvents( void* args ) {
    ...
    while ( true ) {
        WaitForSingleObject(
            prefetch_thread_signal,
            INFINITE ); // wait for prefetch queue event to be signaled
...

        {
            ...
            if ( prefetch_events->empty() ) {
                if ( spawn_antifreeze_debug )
                    Msg( "[ProcessPrefetchEvents] called, but prefetch_events "
                         "queue is empty" );
        ...
        prefetch_event_queue saved_prefetch_events;
        {
            ...
            saved_prefetch_events.swap(
                *prefetch_events ); // move the events to temp queue, so we can
                                    // continue processing prefetch_events in
                                    // the main thread
            pausePrefetchThreadSignal();
        }
        ...
        for ( const auto& E : saved_prefetch_events ) {
            for ( const auto& model : E.models ) {
                ...
                if ( not_prefetched ) {
                    if ( spawn_antifreeze_debug )
                        Msg( "[ProcessPrefetchEvents] Prefetching model '%s' "
                             "for spawn event",
                             model.c_str() );
                    ...
                    {
                        xrSRWLockGuard g( prefetch_lock );
                        prefetched_models->insert(
                            model ); // add model to prefetched models set to
...
// demonized
                ...
                if ( M_SPAWN == ID ) {
                    ...
                    if ( isValidToPrefetch( parent_id, section, obj_id, P ) ) {
                        ...
                        if ( !models.empty() ) {
                            ...
                            if ( spawn_antifreeze_debug )
                                Msg( "[ProcessGameEvents] added M_SPAWN to "
                                     "prefetch_events: section %s, obj_id %d, "
                                     "parent_id %d, event_id %d",
                                     section.c_str(), obj_id, parent_id, dest );
    ...
    if ( !events_to_prefetch.empty() ) {
        ...
        prefetch_events->insert( prefetch_events->end(),
                                 events_to_prefetch.begin(),
                                 events_to_prefetch.end() );
        unpausePrefetchThreadSignal();
    }
#endif
