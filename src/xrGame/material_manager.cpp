...
CMaterialManager::CMaterialManager( CObject* object,
...
void CMaterialManager::Load( LPCSTR section ) {
    R_ASSERT3( pSettings->line_exist( section, "material" ),
               "Material not found in the section ",
               *( m_object->cNameSect() ) );
    m_my_material_idx =
        GMLib.GetMaterialIdx( pSettings->r_string( section, "material" ) );
    ...
    if ( debug_character_material_load ) {
        ...
        if ( entity_alive ) {
            ...
            VERIFY( m );
            Msg( "(CMaterialManager::Load(LPCSTR section)) material: %s loaded "
                 "for %s, from section: %s ",
...
void CMaterialManager::reinit() {
    ...
    CEntityAlive* entity_alive = smart_cast< CEntityAlive* >( m_object );
    if ( entity_alive ) {
        if ( !smart_cast< CBaseMonster* >(
                 entity_alive ) ) // для мобов не нужно устанавливть реальный
                                  // материал, у них нет эти настроек материалов
            entity_alive->character_physics_support()
                ->movement()
                ->SetPLastMaterialIDX( &m_last_material_idx );
        entity_alive->character_physics_support()->movement()->SetMaterial(
            m_my_material_idx );
#ifdef DEBUG
        if ( debug_character_material_load ) {
            VERIFY( GAMEMTL_NONE_IDX != m_my_material_idx );
            SGameMtl* m = GMLib.GetMaterialByIdx( m_my_material_idx );
            VERIFY( m );
            Msg( "(CMaterialManager::reinit) material: %s loaded for %s ",
                 m->m_Name.c_str(), entity_alive->cName().c_str() );
        }
#endif
    }
}
...
void CMaterialManager::update( float time_delta,
                               float volume,
                               float step_time,
                               bool standing ) {
    VERIFY( GAMEMTL_NONE_IDX != m_my_material_idx );
    ...
    SGameMtlPair* mtl_pair =
        GMLib.GetMaterialPair( m_my_material_idx, m_last_material_idx );
    VERIFY3( mtl_pair, "Undefined material pair: ",
             *GMLib.GetMaterialByIdx( m_last_material_idx )->m_Name );
