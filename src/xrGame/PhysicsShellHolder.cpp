...
void CPhysicsShellHolder::create_physic_shell() {
    VERIFY( !m_pPhysicsShell );
    IPhysicShellCreator* shell_creator =
        smart_cast< IPhysicShellCreator* >( this );
    if ( shell_creator )
...
bool CPhysicsShellHolder::has_shell_collision_place(
...
void CPhysicsShellHolder::correct_spawn_pos() {
    ...
    R_ASSERT2( _valid( c ),
               make_string( "object: %s model: %s ", cName().c_str(),
                            cNameVisual().c_str() ) );
...
void CPhysicsShellHolder::activate_physic_shell() {
    ...
    if ( !IsGameTypeSingle() ) {
        if ( !smart_cast< CCustomRocket* >( this ) &&
             !smart_cast< CGrenade* >( this ) )
...
void CPhysicsShellHolder::OnChangeVisual() {
    ...
    if ( 0 == renderable.visual ) {
        ...
        VERIFY( !character_physics_support() ||
                !character_physics_support()->interactive_motion() );
        if ( m_pPhysicsShell )
...
void CPhysicsShellHolder::save( NET_Packet& output_packet ) {
    ...
    if ( PPhysicsShell() && PPhysicsShell()->isActive() ) {
        enable_state =
            u8( PPhysicsShell()->isEnabled() ? stEnable : stDisable );
    }
...
// void						SetWeaponHideState( u16 State,
// bool bSet )
...
CPhysicsShellHolder::ObjectPhCollisionDamageReceiver() {
...
void CPhysicsShellHolder::BonceDamagerCallback( float& damage_factor ) {
    ...
    // phs=static_cast<CPhysicsShellHolder*>(o_damager)->character_physics_support();
...
std::string CPhysicsShellHolder::dump( EDumpType type ) const {
    switch ( type ) {
        case base:
            return dbg_object_base_dump_string( this );
            break;
        case poses:
            return dbg_object_poses_dump_string( this );
            break;
        case vis_geom:
            return dbg_object_visual_geom_dump_string( this );
            break;
        case props:
            return dbg_object_props_dump_string( this );
            break;
        case full:
            return dbg_object_full_dump_string( this );
            break;
        case full_capped:
            return dbg_object_full_capped_dump_string( this );
            break;
        default:
            NODEFAULT;
...
#if 1
void CPhysicsShellHolder::IgnoreCollisionCallback( bool& do_colide,
                                                   bool bo1,
                                                   dContact& c,
                                                   SGameMtl* material_1,
                                                   SGameMtl* material_2 ) {
    ...
    CPhysicsShellHolder* a =
        ( obj ) ? smart_cast< CPhysicsShellHolder* >( obj ) : NULL;
    if ( a == NULL ) {
        return;
    }
    ...
    if ( b ) {
        if ( who->cast_actor() || who->cast_stalker() ||
             who->cast_base_monster() ) {
