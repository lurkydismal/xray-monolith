...
CStalkerDeathPlanner::CStalkerDeathPlanner( CAI_Stalker* object,
                                            LPCSTR action_name )
    : inherited( object, action_name ) {}

CStalkerDeathPlanner::~CStalkerDeathPlanner() {}

void CStalkerDeathPlanner::setup( CAI_Stalker* object,
...
void CStalkerDeathPlanner::add_evaluators() {
    add_evaluator(
        ...
        xr_new< CStalkerPropertyEvaluatorConst >( false, "resurrecting" ) );
    add_evaluator( eWorldPropertyDead,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyDead, true, true,
                       "completely dead" ) );
}
