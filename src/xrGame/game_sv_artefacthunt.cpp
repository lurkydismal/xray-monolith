...
int g_sv_ah_iReinforcementTime =
    15; // 0 - Immediate, -1 - after artefact spawn , other - reinforcement
...
void game_sv_ArtefactHunt::Create( shared_str& options ) {
    ...
    m_iMoney_for_BuySpawn = READ_IF_EXISTS(
        pSettings, r_s32, "artefacthunt_gamedata", "spawn_cost", -10000 );
    //---------------------------------------------------------------
...
KILL_RES game_sv_ArtefactHunt::GetKillResult( game_PlayerState* pKiller,
...
bool game_sv_ArtefactHunt::OnKillResult( KILL_RES KillResult,
...
void game_sv_ArtefactHunt::OnGiveBonus( KILL_RES KillResult,
                                        game_PlayerState* pKiller,
                                        game_PlayerState* pVictim,
                                        KILL_TYPE KillType,
    ...
    switch ( KillResult ) {
        case KR_RIVAL: {
            if ( pVictim->GameID == m_iAfBearerMenaceID )
                Player_AddExperience(
                    pKiller, READ_IF_EXISTS( pSettings, r_float, "mp_bonus_exp",
                                             "assist_kill", 0 ) );

            inherited::OnGiveBonus( KR_RIVAL, pKiller, pVictim, KillType,
        ...
        default: {
            inherited::OnGiveBonus( KillResult, pKiller, pVictim, KillType,
                                    SpecialKillType, pWeaponA );
        } break;
    }
}

void game_sv_ArtefactHunt::OnPlayerKillPlayer(
...
void game_sv_ArtefactHunt::OnPlayerReady( ClientID id ) {
    ...
    switch ( m_phase ) {
        case GAME_PHASE_INPROGRESS: {
            ...
            if ( pS &&
                 ( Get_ReinforcementTime() != 0 &&
                   !xrCData->ps->m_bPayForSpawn ) &&
...
bool game_sv_ArtefactHunt::assign_rp_tmp( game_PlayerState* ps_who,
    ...
    struct rpoints_blocker {
        ...
        void operator()( IClient* client ) {
            ...
            if ( rp->P.distance_to( tmp_player->Position() ) <= 0.4f &&
                 !ffind ) {
                blocked = true;
...
void game_sv_ArtefactHunt::assign_RP( CSE_Abstract* E,
                                      game_PlayerState* ps_who ) {
    ...
    u32 Team = RP_2_Use( E );
#ifdef DEBUG
    Msg( "--- ArtefactHunt RPoint for %s uses team %d", ps_who->getName(),
         Team );
#endif // #ifdef DEBUG
...
#ifdef DEBUG
        Msg( "--- No free Arthfacthunt rpoints found, trying to find rpoints "
             "with enemies!" );
#endif // #ifdef DEBUG
    ...
    if ( rpID.empty() && !rpIDEnemy.empty() ) {
        ...
        SetRP( E, &r );
        //---------------------------------------------------------------------
        game_PlayerState* PSE =
            static_cast< xrClientData* >(
                m_server->GetClientByID( EnemyIt[ PointID ] ) )
                ->ps;
        R_ASSERT2( PSE, "Where is Enemy!!!" );
        CGameObject* pPlayer = smart_cast< CGameObject* >(
            Level().Objects.net_Find( PSE->GameID ) );
...
void game_sv_ArtefactHunt::SetRP( CSE_Abstract* E, RPoint* pRP ) {
    ...
    if ( std::find( rpointsBlocked.begin(), rpointsBlocked.end(), pRP ) ==
         rpointsBlocked.end() ) {
...
void game_sv_ArtefactHunt::CheckRPUnblock() {
    ...
    rpointsBlocked.erase(
        std::remove_if( rpointsBlocked.begin(), rpointsBlocked.end(),
...
BOOL game_sv_ArtefactHunt::OnTouch( u16 eid_who, u16 eid_what, BOOL bForced ) {
    ...
    if ( A ) {
        CSE_ALifeItemArtefact* pIArtefact =
            smart_cast< CSE_ALifeItemArtefact* >( e_what );
        if ( pIArtefact ) {
                    ...
                    if ( pTeam ) {
                        struct experience_adder {
                            ...
                            void operator()( IClient* client ) {
                                ...
                                if ( !l_pC->net_Ready || pstate->IsSkip() ||
                                     pstate->team != ps_who->team )
                                    return;
...
void game_sv_ArtefactHunt::OnObjectEnterTeamBase( u16 id, u16 zone_team ) {
    ...
    if ( eActor ) {
        if ( eActor->g_team() == zone_team ) {
            ...
            xr_vector< u16 >::iterator c =
                std::find( C.begin(), C.end(), m_dwArtefactID );
            if ( C.end() != c ) {
...
void game_sv_ArtefactHunt::OnObjectLeaveTeamBase( u16 id, u16 zone_team ) {
    ...
    CSE_ALifeCreatureActor* eActor =
        smart_cast< CSE_ALifeCreatureActor* >( e_who );
    //	CSE_ALifeTeamBaseZone*	eZoneBase = smart_cast<CSE_ALifeTeamBaseZone*>
    //(e_zone);
    if ( eActor /*&& eZoneBase*/ ) {
        if ( eActor->g_team() == zone_team ) {
            game_PlayerState* ps = eActor->owner->ps;
...
void game_sv_ArtefactHunt::OnArtefactOnBase( ClientID id_who ) {
    ...
    if ( pTeam ) {
        ...
        Player_AddExperience(
            ps, READ_IF_EXISTS( pSettings, r_float, "mp_bonus_exp",
                                "target_succeed", 0 ) );
        ps->af_count++;
        ...
        struct money_for_team_adder {
            ...
            void operator()( IClient* client ) {
                ...
                if ( pstate->team == ps->team ) {
                    m_owner->Player_AddMoney( pstate,
                                              pTeam->m_iM_TargetSucceedAll );
                    m_owner->Player_AddExperience(
                        pstate,
...
void game_sv_ArtefactHunt::SpawnArtefact() {
    ...
    if ( pSettings->line_exist( "artefacthunt_gamedata", "artefact" ) )
        E = spawn_begin(
            pSettings->r_string( "artefacthunt_gamedata", "artefact" ) );
    else
        return;
...
void game_sv_ArtefactHunt::Update() {
    ...
    switch ( Phase() ) {
        ...
        case GAME_PHASE_TEAM2_ELIMINATED: {
            if ( m_delayedTeamEliminated &&
                 m_TeamEliminatedDelay < Device.TimerAsync() ) {
            ...
            if ( m_dwWarmUp_CurTime == 0 ) {
                ...
                if ( Get_ReinforcementTime() > 0 ) {
                    ...
                    if ( m_dwNextReinforcementTime < CurTime ) {
                        RespawnAllNotAlivePlayers();
                        m_dwNextReinforcementTime =
                            CurTime + Get_ReinforcementTime() * 1000;
                    }
                };
                //---------------------------------------------------
    ...
    struct all_players_ready_cond {
        ...
        void operator()( IClient* client ) {
            ...
            if ( !l_pC->net_Ready || ps->IsSkip() ||
                 ps->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) )
                return;
            else {
...
void game_sv_ArtefactHunt::OnCreate( u16 id_who ) {
    ...
    if ( !pEntity )
        return;
    CSE_ALifeItemArtefact* pIArtefact =
        smart_cast< CSE_ALifeItemArtefact* >( pEntity );
    if ( pIArtefact )
        m_dwArtefactID = pIArtefact->ID;
    CSE_ALifeTeamBaseZone* pTeamBase =
        smart_cast< CSE_ALifeTeamBaseZone* >( pEntity );
...
void game_sv_ArtefactHunt::OnTimelimitExceed() {
    ...
    m_phase = u16( ( winning_team ) ? GAME_PHASE_TEAM2_SCORES
...
void game_sv_ArtefactHunt::Artefact_PrepareForSpawn() {
    ...
    m_dwArtefactSpawnTime =
        Device.dwTimeGlobal + Get_ArtefactsRespawnDelta() * 1000;
...
void game_sv_ArtefactHunt::Artefact_PrepareForRemove() {
    m_dwArtefactRemoveTime =
        Device.dwTimeGlobal + Get_ArtefactsStayTime() * 60000;
...
void game_sv_ArtefactHunt::RespawnAllNotAlivePlayers() {
    ...
    m_dwNextReinforcementTime =
        Level().timeServer() + Get_ReinforcementTime() * 1000;
};
    ...
    struct alife_players_teleporter {
        ...
        void operator()( IClient* client ) {
            ...
            CSE_ALifeCreatureActor* pA =
                smart_cast< CSE_ALifeCreatureActor* >( l_pC->owner );
            CActor* pActor =
                smart_cast< CActor* >( Level().Objects.net_Find( ps->GameID ) );
            if ( !pA || !pActor )
                return;
...
void game_sv_ArtefactHunt::CheckForTeamElimination() {
    ...
    //-----------------------------------------------------------------------------
    m_phase = u16( ( WinTeam == 1 ) ? GAME_PHASE_TEAM2_ELIMINATED
                                    : GAME_PHASE_TEAM1_ELIMINATED );
    switch_Phase( m_phase );

    OnDelayedTeamEliminated(); // OnDelayedRoundEnd( eRoundEnd_FragLimit ); //??
                               // "Team Eliminated"
    RemoveArtefact();
}
...
void game_sv_ArtefactHunt::CheckForTeamWin() {
    ...
    m_phase = u16( ( WinTeam == 2 ) ? GAME_PHASE_TEAM2_SCORES
                                    : GAME_PHASE_TEAM1_SCORES );
    switch_Phase( m_phase );
...
void game_sv_ArtefactHunt::check_Player_for_Invincibility(
    game_PlayerState* ps ) {
    if ( !ps )
        return;
...
void game_sv_ArtefactHunt::ReadOptions( shared_str& options ) {
    ...
    //-------------------------------
    g_sv_ah_dwArtefactRespawnDelta = get_option_i(
        *options, "ardelta", g_sv_ah_dwArtefactRespawnDelta ); // sec
    g_sv_ah_dwArtefactsNum =
        get_option_i( *options, "anum", g_sv_ah_dwArtefactsNum );
    g_sv_ah_dwArtefactStayTime =
        get_option_i( *options, "astime", g_sv_ah_dwArtefactStayTime );
...
void game_sv_ArtefactHunt::OnRender() {
    if ( dbg_net_Draw_Flags.test( dbg_draw_rp ) ) {
        ...
        for ( u32 i = 0; i < Artefact_rpoints.size(); i++ ) {
            ...
            T.identity();
            Level().debug_renderer().draw_line( Fidentity, V0, V1,
...
void game_sv_ArtefactHunt::OnPlayerHitPlayer_Case( game_PlayerState* ps_hitter,
                                                   game_PlayerState* ps_hitted,
                                                   SHit* pHitS ) {
    // if (pHitS->hit_type != ALife::eHitTypePhysicStrike)
    //{
    if ( ps_hitted->testFlag( GAME_PLAYER_FLAG_ONBASE ) &&
         Get_ShieldedBases() ) {
        pHitS->power = 0;
        pHitS->impulse = 0;
    }
...
void game_sv_ArtefactHunt::OnPlayerHitPlayer( u16 id_hitter,
                                              u16 id_hitted,
                                              NET_Packet& P ) {
    ...
    if ( !ps_hitter || !ps_hitted )
        return;
    if ( ps_hitter->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ||
         ps_hitted->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) )
        return;
...
void game_sv_ArtefactHunt::WriteGameState( CInifile& ini,
                                           LPCSTR sect,
                                           bool bRoundResult ) {
...
/*void game_sv_ArtefactHunt::DestroyAllPlayerItems(ClientID id_who)	//except
rukzak
{
        xrClientData* xrCData = m_server->ID_to_client(id_who);

        VERIFY2(xrCData,
                make_string("client (ClientID = 0x%08x) not found",
id_who.value()).c_str()); VERIFY(xrCData->ps); game_PlayerState*	ps
=	xrCData->ps; #ifndef MASTER_GOLD Msg("---Destroying player [%s] items
before spawning new bought items.", ps->getName()); #endif // #ifndef
MASTER_GOLD

        CActor* pActor =
smart_cast<CActor*>(Level().Objects.net_Find(ps->GameID)); if (!pActor) return;

        TIItemContainer::const_iterator iie = pActor->inventory().m_all.end();
        for (TIItemContainer::const_iterator ii =
pActor->inventory().m_all.begin(); ii != iie; ++ii)
        {
                VERIFY(*ii);
