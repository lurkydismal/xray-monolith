...
void server_site::stop_transfer_sessions(
    buffer_vector< dst_src_pair_t > const& tsessions )
...
void server_site::stop_receiving_sessions(
    buffer_vector< ClientID > const& rsessions ) // notifies receiving_timeout
{
...
void server_site::update_transfer() {
    ...
    for ( transfer_sessions_t::iterator ti = m_transfers.begin(),
          ...
          ti != tie; ++ti ) {
        IClient* tmp_client =
            Level().Server->GetClientByID( ti->first.first ); // dst
        if ( !tmp_client ) {
            Msg( "! ERROR: SV: client [%u] not found for transfering file",
                 ti->first );
        ...
        tmp_ftnode->calculate_chunk_size( tmp_client->stats.getPeakBPS(),
                                          tmp_client->stats.getBPS() );
        ...
        Level().Server->SendTo( tmp_client->ID, tmp_packet,
                                net_flags( TRUE, TRUE, TRUE ) );
        if ( complete ) {
...
void server_site::on_message( NET_Packet* packet, ClientID const& sender ) {
    ...
    switch ( command ) {
        case receive_data: {
            receiving_sessions_t::iterator temp_iter =
                m_receivers.find( sender );
            if ( temp_iter == m_receivers.end() ) {
                NET_Packet reject_packet;
                make_reject_packet( reject_packet, ClientID( 0 ) );
                Level().Server->SendTo( sender, reject_packet,
                                        net_flags( TRUE, TRUE, TRUE ) );
                break;
            }
...
void server_site::stop_obsolete_receivers() {
    ...
    for ( receiving_sessions_t::iterator i = m_receivers.begin(),
        ...
        if ( !i->second->get_downloaded_size() ) {
            ...
            } else if ( ( current_time - i->second->get_last_read_time() ) >
                        MAX_START_WAIT_TIME ) {
                i->second->signal_callback( receiving_timeout );
                to_stop_receivers.push_back( i->first );
            }
        } else {
            if ( ( current_time - i->second->get_last_read_time() ) >
                 MAX_FT_WAIT_TIME ) {
                i->second->signal_callback( receiving_timeout );
                to_stop_receivers.push_back( i->first );
            }
...
void server_site::start_transfer_file(
    ...
    if ( is_transfer_active( to_client, from_client ) ) {
        Msg( "! ERROR: SV: transfering file to client [%d] already active.",
             to_client );
        return;
    }
    filetransfer_node* ftnode = xr_new< filetransfer_node >(
        file_name, data_max_chunk_size, tstate_callback );
...
void server_site::start_transfer_file(
    ...
    filetransfer_node* ftnode = xr_new< filetransfer_node >(
        &mem_writer, mem_writer_max_size, data_max_chunk_size, tstate_callback,
        user_param );
    m_transfers.insert(
        std::make_pair( std::make_pair( to_client, from_client ), ftnode ) );
}
...
void server_site::start_transfer_file(
    ...
    filetransfer_node* ftnode = xr_new< filetransfer_node >(
        data_ptr, data_size, data_max_chunk_size, tstate_callback, user_param );
    m_transfers.insert(
        std::make_pair( std::make_pair( to_client, from_client ), ftnode ) );
}
...
void server_site::start_transfer_file(
    ...
    filetransfer_node* ftnode = xr_new< filetransfer_node >(
        &vector_of_buffers, data_max_chunk_size, tstate_callback, user_param );
    m_transfers.insert(
        std::make_pair( std::make_pair( to_client, from_client ), ftnode ) );
}
...
void server_site::stop_transfer_file( dst_src_pair_t const& tkey ) {
    ...
    if ( temp_iter == m_transfers.end() ) {
        Msg( "! ERROR: SV: no file transfer for client [%d] found from client "
             "[%d].",
    ...
    if ( !temp_iter->second->is_complete() ) {
        ...
        if ( Level().Server->GetClientByID( tkey.first ) ) {
            Level().Server->SendTo( tkey.first, abort_packet,
                                    net_flags( TRUE, TRUE, TRUE ) );
        }
    }
...
filereceiver_node* server_site::start_receive_file(
    ...
    if ( temp_iter != m_receivers.end() ) {
        Msg( "! ERROR: SV: file already receiving from client [%d]",
             from_client );
        return NULL;
    }
    filereceiver_node* frnode =
        xr_new< filereceiver_node >( file_name, rstate_callback );
...
filereceiver_node* server_site::start_receive_file(
    ...
    filereceiver_node* frnode =
        xr_new< filereceiver_node >( &mem_writer, rstate_callback );
    m_receivers.insert( std::make_pair( from_client, frnode ) );
    return frnode;
}
...
void server_site::stop_receive_file( ClientID const& from_client ) {
    receiving_sessions_t::iterator temp_iter = m_receivers.find( from_client );
    if ( temp_iter == m_receivers.end() ) {
        Msg( "! ERROR: SV: no file receiving from client [%u] found",
             from_client );
        return;
    }
    if ( !temp_iter->second->is_complete() ) {
        NET_Packet reject_packet;
        make_reject_packet( reject_packet, ClientID( 0 ) );
        Level().Server->SendTo( from_client, reject_packet,
                                net_flags( TRUE, TRUE, TRUE ) );
    }
    xr_delete( temp_iter->second );
...
bool server_site::is_transfer_active( ClientID const& to_client,
                                      ClientID const& from_client ) const {
    transfer_sessions_t::const_iterator temp_iter =
        m_transfers.find( std::make_pair( to_client, from_client ) );
    if ( temp_iter == m_transfers.end() )
        return false;
    return true;
}
...
bool server_site::is_receiving_active( ClientID const& from_client ) const {
    receiving_sessions_t::const_iterator temp_iter =
        m_receivers.find( from_client );
    if ( temp_iter == m_receivers.end() )
        return false;
    return true;
}
...
void client_site::update_transfer() {
    if ( is_transfer_active() && m_transfering->is_ready_to_send() ) {
        ...
        m_transfering->calculate_chunk_size( peer_stats.getPeakBPS(),
                                             peer_stats.getBPS() );
#ifdef DEBUG
...
void client_site::on_message( NET_Packet* packet ) {
    ...
    switch ( command ) {
        case receive_data: {
            receiving_sessions_t::iterator tmp_iter =
                m_receivers.find( from_client );
        ...
        case abort_receive: {
            ...
            if ( tmp_iter != m_receivers.end() ) {
                ...
                Msg( "! WARNING: CL: server sent unknown abort receive "
                     "message" );
            }
        } break;
        case receive_rejected: {
            ...
            if ( is_transfer_active() ) {
                ...
                Msg( "! WARNING: CL: server sent unknown receive reject "
                     "message" );
            }
        } break;
    };
...
void client_site::start_transfer_file(
    ...
    m_transfering = xr_new< filetransfer_node >( file_name, data_min_chunk_size,
                                                 tstate_callback );
...
void client_site::start_transfer_file(
    ...
    m_transfering = xr_new< filetransfer_node >(
        data, size, data_min_chunk_size, tstate_callback, size_to_allocate );
}
...
filereceiver_node* client_site::start_receive_file(
    ...
    if ( is_receiving_active( from_client ) ) {
        Msg( "! ERROR: CL: file already receiving from client [%d]",
             from_client );
        return NULL;
    }
    filereceiver_node* frnode =
...
void client_site::stop_receive_file( ClientID const& from_client ) {
    receiving_sessions_t::iterator temp_iter = m_receivers.find( from_client );
    if ( temp_iter == m_receivers.end() ) {
        Msg( "! ERROR: CL: no file receiving from client [%u] found",
             from_client );
        return;
    }
    if ( !temp_iter->second->is_complete() ) {
...
void client_site::stop_receiving_sessions(
    buffer_vector< ClientID > const& rsessions ) {
    for ( buffer_vector< ClientID >::const_iterator i = rsessions.begin(),
                                                    ie = rsessions.end();
          i != ie; ++i ) {
        stop_receive_file( *i );
...
void client_site::dbg_init_statgraph() {
    ...
    m_stat_graph->SetMinMax( float( data_min_chunk_size ),
                             float( data_max_chunk_size ), 1000 );
...
void client_site::dbg_update_statgraph() {
    ...
    if ( m_transfering ) {
        m_stat_graph->AppendItem( float( m_transfering->get_chunk_size() ),
                                  0xff00ff00, 0 );
    }
}
#endif
