...
inline BONE::bone( CGameObject* object,
                   LPCSTR animation_id,
...
void BONE::compute_bones( u32 const bone_id ) {
    ...
    CBoneInstance& bone =
        m_kinematics.LL_GetBoneInstance( m_bones_ids[ bone_id ] );
...
void BONE::compute_bone( u32 const bone_id ) {
    ...
    if ( bone_id < bone_count - 1 ) {
        aim_at_position( m_bones[ bone_id ].c, m_bones[ bone_count - 1 ].c,
                         m_bones[ bone_count - 1 ].k, m_result[ bone_id ] );
        return;
    }
