...
inline void aimers::base::fill_bones(
    ...
    if ( !m_animation_start || !m_object.animation_movement() ||
    ...
    if ( m_animation_start ) {
        for ( u16 i = 0; i < MAX_PARTS; ++i ) {
            ...
            for ( u32 j = 0; j < blend_count; ++j ) {
                ...
                CBlend* const new_blend = m_animated.LL_PlayCycle(
                    i, blend->motionID, TRUE, 0, 0, channel_id );
                VERIFY( new_blend );
                *new_blend = *blend;
        ...
        for ( u16 i = 0; i < MAX_PARTS; ++i ) {
            CBlend* const blend = m_animated.LL_PlayCycle( i, m_animation_id, 0,
                                                           0, 0, channel_id );
            if ( blend )
                blend->timeCurrent =
                    m_animation_start
                        ? 0.f
                        : ( blend->timeTotal - ( SAMPLE_SPF + EPS ) );
        }
    }

    {
        ...
        for ( ; i != e; ++i ) {
            ...
            // m_kinematics.Bone_GetAnimPos(local_bones[*i], bones_ids[*i],
            // channel_mask, false);
            m_kinematics.LL_GetBoneLocalTransform( bones_ids[ *i ],
                                                   local_bones[ *i ] );
