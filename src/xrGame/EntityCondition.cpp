...
void CEntityCondition::ClearWounds() {
    for ( WOUND_VECTOR_IT it = m_WoundVector.begin(); m_WoundVector.end() != it;
          ++it )
        xr_delete( *it );
    m_WoundVector.clear();
...
void CEntityCondition::LoadCondition( LPCSTR entity_section ) {
    ...
    m_use_limping_state = !!( READ_IF_EXISTS( pSettings, r_bool, section,
                                              "use_limping_state", FALSE ) );
    m_limping_threshold =
        READ_IF_EXISTS( pSettings, r_float, section, "limping_threshold", .5f );

    m_fKillHitTreshold = READ_IF_EXISTS( pSettings, r_float, section,
                                         "killing_hit_treshold", 0.0f );
    m_fLastChanceHealth = READ_IF_EXISTS( pSettings, r_float, section,
                                          "last_chance_health", 0.0f );
    m_fInvulnerableTimeDelta = READ_IF_EXISTS( pSettings, r_float, section,
                                               "invulnerable_time", 0.0f ) /
                               1000.f;
    m_fBleedSpeedK =
        READ_IF_EXISTS( pSettings, r_float, section, "bleed_speed_k", 1.f );
}
...
void CEntityCondition::UpdateWounds() {
    ...
    m_WoundVector.erase(
        std::remove_if( m_WoundVector.begin(), m_WoundVector.end(),
                        &RemoveWoundPred ),
        m_WoundVector.end() );
}

void CEntityCondition::UpdateConditionTime() {
    u64 _cur_time = ( GameID() == eGameIDSingle ) ? Level().GetGameTime()
                                                  : Level().timeServer();
...
void CEntityCondition::UpdateCondition() {
    ...
    if ( Device.fTimeGlobal > m_fInvulnerableTime ) {
        ...
        if ( curr_health > m_fKillHitTreshold &&
             curr_health + m_fDeltaHealth < 0 ) {
...
float CEntityCondition::HitOutfitEffect( float hit_power,
    ...
    CCustomOutfit* pOutfit =
        ( CCustomOutfit* )pInvOwner->inventory().ItemFromSlot( OUTFIT_SLOT );
    CHelmet* pHelmet =
        ( CHelmet* )pInvOwner->inventory().ItemFromSlot( HELMET_SLOT );
    if ( !pOutfit && !pHelmet )
        return hit_power;

    float new_hit_power = hit_power;
    if ( pOutfit )
        new_hit_power = pOutfit->HitThroughArmor( hit_power, element, ap,
                                                  add_wound, hit_type );

    if ( pHelmet )
        new_hit_power = pHelmet->HitThroughArmor( new_hit_power, element, ap,
                                                  add_wound, hit_type );

    if ( bDebug )
        Msg( "new_hit_power = %.3f  hit_type = %s  ap = %.3f", new_hit_power,
             ALife::g_cafHitType2String( hit_type ), ap );
...
float CEntityCondition::HitPowerEffect( float power_loss ) {
    ...
    CHelmet* pHelmet =
        ( CHelmet* )pInvOwner->inventory().ItemFromSlot( HELMET_SLOT );
    CBackpack* pBackpack =
        ( CBackpack* )pInvOwner->inventory().ItemFromSlot( BACKPACK_SLOT );
...
CWound* CEntityCondition::AddWound( float hit_power,
                                    ALife::EHitType hit_type,
                                    u16 element ) {
...
static inline void applyBeforeHitAfterCalcsCallback(
    CEntityAlive* target,
    const ::luabind::functor< void >& funct,
    SHit* pHDS,
    float& hit_power,
    const float hit_part = 1 ) {
...
CWound* CEntityCondition::ConditionHit( SHit* pHDS ) {
    ...
    bool const is_special_hit_2_self =
        ( pHDS->who == m_object ) && ( pHDS->boneID == BI_NONE );

    bool bAddWound = pHDS->add_wound;

    float hit_power_org = pHDS->damage();
    float hit_power = hit_power_org;
    hit_power = HitOutfitEffect( hit_power_org, pHDS->hit_type, pHDS->boneID,
                                 pHDS->armor_piercing, bAddWound );

    // demonized: add lua callback before hit but after calculations
    // don't call if there is no target
    ::luabind::functor< void > funct;
    bool has_func =
        ai().script_engine().functor( "_G.CBeforeHitAfterCalcs", funct );
    ...
    switch ( pHDS->hit_type ) {
        case ALife::eHitTypeTelepatic:
            ...
            if ( hit_power < 0.f )
                hit_power = 0.f;
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostTelepaticImmunity;

            // demonized: add lua callback before hit but after calculations
            if ( has_func ) {
                applyBeforeHitAfterCalcsCallback( m_object, funct, pHDS,
                                                  hit_power, m_fHealthHitPart );
            }
        ...
        case ALife::eHitTypeBurn:
            hit_power *=
                GetHitImmunity( ALife::eHitTypeBurn ) - m_fBoostBurnImmunity;

            // demonized: add lua callback before hit but after calculations
            if ( has_func ) {
                applyBeforeHitAfterCalcsCallback(
                    m_object, funct, pHDS, hit_power,
                    m_fHealthHitPart * m_fHitBoneScale );
            }
        ...
        case ALife::eHitTypeChemicalBurn:
            ...
            if ( hit_power < 0.f )
                hit_power = 0.f;
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostChemicalBurnImmunity;
        ...
        case ALife::eHitTypeShock:
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostShockImmunity;
        ...
        case ALife::eHitTypeRadiation:
            ...
            if ( hit_power < 0.f )
                hit_power = 0.f;
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostRadiationImmunity;
        ...
        case ALife::eHitTypeExplosion:
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostExplImmunity;
        ...
        case ALife::eHitTypeStrike:
            ...
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostStrikeImmunity;
        ...
        case ALife::eHitTypeFireWound:
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostFireWoundImmunity;
        ...
        case ALife::eHitTypeWound:
            hit_power *=
                GetHitImmunity( pHDS->hit_type ) - m_fBoostWoundImmunity;
    ...
    if ( bDebug && !is_special_hit_2_self ) {
        Msg( "%s hitted in %s with %f[%f]", m_object->Name(),
             smart_cast< IKinematics* >( m_object->Visual() )
                 ->LL_BoneName_dbg( pHDS->boneID ),
             m_fHealthLost * 100.0f, hit_power_org );
    }
    // раны добавляются только живому
    if ( bAddWound && GetHealth() > 0 ) {
        return AddWound( hit_power * m_fWoundBoneScale, pHDS->hit_type,
                         pHDS->boneID );
    } else {
        return NULL;
    }
...
void CEntityCondition::UpdateHealth() {
    float bleeding_speed =
        BleedingSpeed() * m_fDeltaTime * change_v().m_fV_Bleeding;
...
void CEntityCondition::UpdateRadiation() {
    if ( m_fRadiation > 0 ) {
        m_fDeltaRadiation -= change_v().m_fV_Radiation * m_fDeltaTime;

        m_fDeltaHealth -= CanBeHarmed() ? change_v().m_fV_RadiationHealth *
                                              m_fRadiation * m_fDeltaTime
                                        : 0.0f;
    }
}
...
void CEntityCondition::save( NET_Packet& output_packet ) {
    ...
    if ( is_alive ) {
        ...
        for ( WOUND_VECTOR_IT it = m_WoundVector.begin();
              m_WoundVector.end() != it; it++ )
...
bool CEntityCondition::ApplyInfluence( const SMedicineInfluenceValues& V,
...
bool CEntityCondition::ApplyBooster( const SBooster& B,
                                     const shared_str& sect ) {
    return true;
}
...
void SMedicineInfluenceValues::Load( const shared_str& sect ) {
    ...
    fMaxPowerUp = READ_IF_EXISTS( pSettings, r_float, sect.c_str(),
                                  "eat_max_power", 0.0f );
    fAlcohol =
        READ_IF_EXISTS( pSettings, r_float, sect.c_str(), "eat_alcohol", 0.0f );
    fTimeTotal = READ_IF_EXISTS( pSettings, r_float, sect.c_str(),
                                 "apply_time_sec", -1.0f );
}
...
void SBooster::Load( const shared_str& sect, EBoostParams type ) {
    ...
    switch ( type ) {
        case eBoostHpRestore:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_health_restore" );
            break;
        case eBoostPowerRestore:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_power_restore" );
            break;
        case eBoostRadiationRestore:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_radiation_restore" );
            break;
        case eBoostBleedingRestore:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_bleeding_restore" );
            break;
        case eBoostMaxWeight:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_max_weight" );
            break;
        case eBoostBurnImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_burn_immunity" );
            break;
        case eBoostShockImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_shock_immunity" );
            break;
        case eBoostRadiationImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_radiation_immunity" );
            break;
        case eBoostTelepaticImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_telepat_immunity" );
            break;
        case eBoostChemicalBurnImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_chemburn_immunity" );
            break;
        case eBoostExplImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_explosion_immunity" );
            break;
        case eBoostStrikeImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_strike_immunity" );
            break;
        case eBoostFireWoundImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_fire_wound_immunity" );
            break;
        case eBoostWoundImmunity:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_wound_immunity" );
            break;
        case eBoostRadiationProtection:
            fBoostValue = pSettings->r_float( sect.c_str(),
                                              "boost_radiation_protection" );
            break;
        case eBoostTelepaticProtection:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_telepat_protection" );
            break;
        case eBoostChemicalBurnProtection:
            fBoostValue =
                pSettings->r_float( sect.c_str(), "boost_chemburn_protection" );
            break;
        default:
            NODEFAULT;
    }
