...
class file_reader : private xray::noncopyable {
public:
    file_reader() {};
...
public:
    explicit buffers_vector_reader(
...
public:
    filetransfer_node( shared_str const& file_name,
                       u32 const chunk_size,
                       sending_state_callback_t const& callback );
    filetransfer_node( u8* data,
                       u32 const data_size,
                       u32 const chunk_size,
                       sending_state_callback_t const& callback,
                       u32 user_param = 0 );
    filetransfer_node( buffer_vector< mutable_buffer_t >* vector_of_buffers,
                       u32 const chunk_size,
                       sending_state_callback_t const& callback,
                       u32 user_param = 0 );
    filetransfer_node( CMemoryWriter* src_writer,
                       u32 const max_size,
                       u32 const chunk_size,
                       sending_state_callback_t const& callback,
                       u32 user_param = 0 );
    ...
    bool make_data_packet(
        NET_Packet& packet ); // returns true if this is a last packet ...
