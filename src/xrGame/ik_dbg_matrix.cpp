...
void dbg_matrises::next_state( SCalculateData& cd ) {
    ...
    if ( cd.m_limb->ref_bone() == 2 ) {
        ...
        dbg_m.b3goal_gl.mul_43( dbg_m.b2goal_gl,
                                cd.m_limb->transform( m_dbg, 2, 3 ) );

        dbg_m.b2goal_lc.mul_43( Fmatrix().invert( *cd.m_obj ),
                                dbg_m.b2goal_gl );
        dbg_m.b3goal_lc.mul_43( Fmatrix().invert( *cd.m_obj ),
                                dbg_m.b3goal_gl );
        dbg_m.b3tob3_goal.mul( Fmatrix().invert( dbg_m.b2goal_gl ),
                               dbg_m.b3goal_gl );

        dbg_m.b2start_gl = cd.m_limb->sv_state.goal( m_dbg );
        dbg_m.b3start_gl.mulB_43( cd.m_limb->transform( m_dbg, 2, 3 ) );

        dbg_m.b2start_lc.mul_43( Fmatrix().invert( *cd.m_obj ),
                                 dbg_m.b2start_gl );
        dbg_m.b3start_lc.mul_43( Fmatrix().invert( *cd.m_obj ),
                                 dbg_m.b3start_gl );
        dbg_m.b3tob3_start.mul( Fmatrix().invert( dbg_m.b2goal_gl ),
                                dbg_m.b3goal_gl );

    } else {
        dbg_m.b3goal_gl = cd.state.goal;
        dbg_m.b2goal_gl.mul_43( dbg_m.b3goal_gl,
                                cd.m_limb->transform( m_dbg, 3, 2 ) );
        ...
        dbg_m.b2start_gl.mul_43( dbg_m.b3start_gl,
                                 cd.m_limb->transform( m_dbg, 3, 2 ) );
