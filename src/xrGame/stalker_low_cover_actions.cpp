...
CStalkerActionGetReadyToKillLowCover::CStalkerActionGetReadyToKillLowCover(
    CAI_Stalker* object,
    LPCSTR action_name )
...
CStalkerActionKillEnemyLowCover::CStalkerActionKillEnemyLowCover(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {}
...
CStalkerActionHoldPositionLowCover::CStalkerActionHoldPositionLowCover(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {}
        ...
        if ( object().agent_manager().member().can_detour() ||
            ...
            CStalkerCombatPlanner& planner =
                smart_cast< CStalkerCombatPlanner& >(
                    object().brain().current_action() );
            planner.CScriptActionPlanner::m_storage.set_property(
                eWorldPropertyLookedOut, true );
            planner.CScriptActionPlanner::m_storage.set_property(
                eWorldPropertyPositionHolded, true );
            planner.CScriptActionPlanner::m_storage.set_property(
                eWorldPropertyInCover, false );
        }
    }

    if ( object().agent_manager().member().cover_detouring() &&
         fire_make_sense() ) {
        object().sound().play( StalkerSpace::eStalkerSoundNeedBackup, 3000,
                               3000, 10000, 10000 );
        fire();
    } else {
        aim_ready();
