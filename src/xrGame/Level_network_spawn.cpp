...
void CLevel::cl_Process_Spawn( NET_Packet& P ) {
    ...
    //-------------------------------------------------
    //.	Msg ("M_SPAWN - %s[%d][%x] - %d %d", *s_name,  E->ID, E,E->ID_Parent,
    //Device.dwFrame);
    //-------------------------------------------------
...
void CLevel::g_sv_Spawn( CSE_Abstract* E ) {
...
#ifdef DEBUG
    //	Msg					("* CLIENT: Spawn: %s, ID=%d",
    //*E->s_name, E->ID);
#endif

    // Optimization for single-player only	- minimize traffic between
    // client and server
    ...
    if ( !O->net_Spawn( E ) ) {
        ...
        if ( ( E->s_flags.is( M_SPAWN_OBJECT_LOCAL ) ) &&
            ...
            if ( IsDemoPlayStarted() ) {
                ...
                if ( CurrentEntity() != NULL ) {
                    CGameObject* pGO =
                        smart_cast< CGameObject* >( CurrentEntity() );
                    if ( pGO )
        ...
        if ( smart_cast< CGameObject* >( O ) ) {
            smart_cast< CGameObject* >( O )->callback(
                GameObject::eNetSpawnAfter )();
        }
#endif
    }
    ...
    if ( g_bMEMO ) {
        lua_gc( ai().script_engine().lua(), LUA_GCCOLLECT, 0 );
        lua_gc( ai().script_engine().lua(), LUA_GCCOLLECT, 0 );
        Msg( "* %20s : %lld bytes, %d ops", *E->s_name,
             Memory.mem_usage() - E_mem, Memory.stat_calls );
    }
#endif // DEBUG_MEMORY_MANAGER
}

CSE_Abstract* CLevel::spawn_item( LPCSTR section,
