...
struct script_layer {
    ...
    script_layer( LPCSTR name,
                  u8 part,
                  float speed = 1.f,
                  float power = 1.f,
                  bool looped = true,
                  LPCSTR pivot_bone = nullptr ) {
        m_name = name;
        m_part = part;
        m_power = power;
...
struct hud_item_measures {
    enum {
        ...
        e_shell_point = ( 1 << 2 ),
        e_16x9_mode_now = ( 1 << 3 ),
        e_fire_point_silencer = ( 1 << 4 )
    };

    Flags8 m_prop_flags;

    Fvector m_item_attach[ 2 ]; // pos,rot
    Fvector m_hands_offset[ 2 ]
                          [ 8 ]; // pos,rot/ normal,aim,GL,aim_alt,safemode,
                                 // normal2, attach_base, attach_mount --#SM+#--
    Fvector m_strafe_offset[ 4 ][ 2 ]; // pos,rot,data1,data2/ normal,aim-GL
                                       // --#SM+#--
...
struct attachable_hud_item {
    ...
    void set_bone_visible( const shared_str& bone_name,
                           BOOL bVisibility,
                           BOOL bSilent = FALSE );
    ...
    u32 anim_play( const shared_str& anim_name,
                   BOOL bMixIn,
                   const CMotionDef*& md,
                   u8& rnd,
                   float speed = 0,
...
public:
    ...
    void render_item_ui();
    bool render_item_ui_query();
    u32 anim_play( u16 part,
                   const MotionID& M,
                   BOOL bMixIn,
                   const CMotionDef*& md,
                   float speed,
                   u16 override_part = u16( -1 ) );
    u32 script_anim_play( u8 hand,
                          LPCSTR itm_name,
                          LPCSTR anm_name,
                          bool bMixIn = true,
                          float speed = 1.f );
    ...
    // Movement animation layers: 0 = aim_walk, 1 = aim_crouch, 2 = crouch, 3 =
    // walk, 4 = run, 5 = sprint
    ...
    void calc_transform( u16 attach_slot_idx,
    ...
    u32 motion_length( const shared_str& anim_name,
...
public:
    ...
    Fvector m_adjust_offset[ 2 ]
                           [ 10 ];   // pos,rot/ normal,aim,GL,aim_alt,safemode,
                                     // normal2, attach_base, attach_mount, aim
                                     // for attach, alt aim for attach
    ...
    xr_map< EBoneCallbackParam, BoneCallbackParams* >
        m_bone_callback_params; // bonename,params
    ...
    void reset_thumb( bool bForce ) {
        if ( bForce ) {
            ...
            m_bone_callback_params[ r_finger01 ]->m_current.set( 0.f, 0.f,
                                                                 0.f );
            m_bone_callback_params[ r_finger02 ]->m_current.set( 0.f, 0.f,
                                                                 0.f );
        }
    ...
    /*void reset_triggerfinger(bool bForce)
    {
            if (bForce)
            {
                    m_bone_callback_params[bip01_r_finger1]->m_current.set(0.f,
    0.f, 0.f); m_bone_callback_params[bip01_r_finger11]->m_current.set(0.f, 0.f,
    0.f); m_bone_callback_params[bip01_r_finger12]->m_current.set(0.f, 0.f,
    0.f);
            }

            m_bone_callback_params[bip01_r_finger1]->m_target.set(0.f, 0.f,
    0.f); m_bone_callback_params[bip01_r_finger11]->m_target.set(0.f, 0.f, 0.f);
            m_bone_callback_params[bip01_r_finger12]->m_target.set(0.f, 0.f,
    0.f);
    }*/
