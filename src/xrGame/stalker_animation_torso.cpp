...
MotionID CStalkerAnimationManager::aim_animation(
    ...
    if ( animation[ 6 ].A.size() < 7 ) {
        // Alundaio: No need for message as many npcs have special danger move
        // and very few don't
#ifdef DEBUG
        Msg(
    ...
    switch ( index ) {
        case 0:
            return ( animation[ 6 ].A[ 4 ] );
        case 2:
            return ( animation[ 6 ].A[ 5 ] );
        case 3:
            return ( animation[ 6 ].A[ 6 ] );
        default:
            NODEFAULT;
    }
...
void CStalkerAnimationManager::torso_play_callback( CBlend* blend ) {
    ...
    if ( animation.m_looking_back ) {
        animation.m_change_direction_time =
            Device.dwTimeGlobal + need_look_back_time_interval;
        animation.m_looking_back = 0;
    }
}

MotionID CStalkerAnimationManager::no_object_animation(
    const EBodyState& body_state ) const {
    const CAI_Stalker& stalker = object();
    const stalker_movement_manager_smart_cover& movement = stalker.movement();
    const xr_vector< CAniVector >& animation =
        m_data_storage->m_part_animations.A[ body_state ].m_torso.A[ 0 ].A;

    if ( eMentalStateFree == movement.mental_state() ) {
        VERIFY3( eBodyStateStand == movement.body_state(),
...
MotionID CStalkerAnimationManager::unknown_object_animation(
    u32 slot,
    const EBodyState& body_state ) const {
    // animation shortcuts
    typedef CStalkerAnimationState STATE;
    const xr_vector< STATE >& part_animations =
        m_data_storage->m_part_animations.A;
    const xr_vector< CAniVector >& animation =
        part_animations[ body_state ].m_torso.A[ slot ].A;
    const xr_vector< CAniVector >& animation_stand =
        part_animations[ eBodyStateStand ].m_torso.A[ slot ].A;
    ...
    switch ( id ) {
        ...
        case ObjectHandlerSpace::eWorldOperatorQueueWait2: {
            ...
            if ( eMovementTypeWalk == movement.movement_type() ) {
                if ( ( body_state == eBodyStateStand ) && ( slot == 2 ) &&
                     need_look_back() )
...
MotionID CStalkerAnimationManager::weapon_animation(
    u32 slot,
    const EBodyState& body_state ) {
    const xr_vector< CAniVector >& animation =
    ...
    if ( eMentalStatePanic == object().movement().mental_state() &&
         eMovementTypeRun == object().movement().movement_type() &&
         body_state == eBodyStateStand &&
         !fis_zero( object().movement().speed(
             object().character_physics_support()->movement() ) ) ) {
        ...
        case CWeapon::eFire2: {
            ...
            if ( eMovementTypeWalk == movement.movement_type() ) {
                if ( ( body_state == eBodyStateStand ) && ( slot == 2 ) &&
                     need_look_back() )
                    return (
                        animation[ 13 + m_looking_back - 1 ].A[ 1 /**1/**/ ] );
                else
...
MotionID CStalkerAnimationManager::missile_animation(
    u32 slot,
    const EBodyState& body_state ) {
    VERIFY( m_missile );
