// exxZERO Time Stamp AddIn. Document modified at : Thursday, March 07, 2002
// 14:12:50 , by user : Oles , from computer : OLES
#include "HitMarker.h"
...
CHitMarker::CHitMarker() {
    ...
    InitShader_Grenade(
        pSettings->r_string( "hud_hitmark", "grenade_mark_texture" ) );
}
...
void SHitMark::Draw( float cam_dir ) {
    ...
    m_UIStaticItem->SetTextureColor(
        subst_alpha( m_UIStaticItem->GetTextureColor(), color_get_A( clr ) ) );
...
SGrenadeMark::SGrenadeMark( const ui_shader& sh, CGrenade* grn ) {
    ...
    float xs = 640.0f;
    float ys = 640.0f;
    m_UIStaticItem->SetPos( ( UI_BASE_WIDTH - xs ) * 0.5f,
                            ( UI_BASE_HEIGHT - ys ) * 0.5f );
...
bool SGrenadeMark::IsActive() const {
    return ( 2.0f * ( Device.fTimeGlobal - m_LastTime ) <
             m_LightAnim->Length_sec() );
}

void SGrenadeMark::Draw( float cam_dir ) {
    int frame;
    u32 clr = m_LightAnim->CalculateRGB(
        2.0f * ( Device.fTimeGlobal - m_LastTime ), frame );
    m_UIStaticItem->SetTextureColor(
        subst_alpha( m_UIStaticItem->GetTextureColor(), color_get_A( clr ) ) );
