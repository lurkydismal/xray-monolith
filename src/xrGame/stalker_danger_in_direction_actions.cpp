...
CStalkerActionDangerInDirectionTakeCover::
    CStalkerActionDangerInDirectionTakeCover( CAI_Stalker* object,
                                              LPCSTR action_name )
...
void CStalkerActionDangerInDirectionTakeCover::initialize() {
    ...
    float distance =
        object().memory().danger().selected()->position().distance_to(
            object().Position() );
...
CStalkerActionDangerInDirectionLookOut::CStalkerActionDangerInDirectionLookOut(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {
...
void CStalkerActionDangerInDirectionLookOut::execute() {
    ...
    if ( !point || ( point->position().similar( object().Position() ) &&
        ...
        object().m_ce_close->setup( position, 10.f, 170.f, 10.f );
        point = ai().cover_manager().best_cover(
            object().Position(), 30.f, *object().m_ce_close,
            CStalkerMovementRestrictor( m_object, true, false ) );
    }
...
CStalkerActionDangerInDirectionHoldPosition::
    CStalkerActionDangerInDirectionHoldPosition( CAI_Stalker* object,
                                                 LPCSTR action_name )
    : inherited( object, action_name ) {}
...
CStalkerActionDangerInDirectionDetour::CStalkerActionDangerInDirectionDetour(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionDangerInDirectionDetour::execute() {
    ...
    CMemoryInfo mem_object = object().memory().memory(
        object().memory().danger().selected()->object() );
...
CStalkerActionDangerInDirectionSearch::CStalkerActionDangerInDirectionSearch(
    CAI_Stalker* object,
    LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionDangerInDirectionSearch::execute() {
    ...
    if ( object().movement().path_completed() ) {
        object().m_ce_ambush->setup(
            position, mem_object.m_self_params.m_position, 10.f );
        const CCoverPoint* point = ai().cover_manager().best_cover(
            position, 10.f, *object().m_ce_ambush,
            CStalkerMovementRestrictor( m_object, true ) );
        if ( !point ) {
            object().m_ce_ambush->setup(
                position, mem_object.m_self_params.m_position, 10.f );
            point = ai().cover_manager().best_cover(
                position, 30.f, *object().m_ce_ambush,
                CStalkerMovementRestrictor( m_object, true ) );
        }
        ...
        if ( object().movement().path_completed() && completed() ) {
            if ( object().memory().danger().selected()->object() )
                object().memory().enable(
                    object().memory().danger().selected()->object(), false );
            else
