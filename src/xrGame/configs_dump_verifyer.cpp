...
bool const configs_verifyer::verify_dsign( u8* data,
    ...
    *dst_buffer = 0;
    u32 dst_size =
        static_cast< u32 >( ( data + data_size ) - ( u8* )dst_buffer );
    ...
    shared_str tmp_dsign =
        tmp_ini.r_string( cd_info_secion, cd_digital_sign_key );
    ...
    CopyMemory( sha_checksum, m_verifyer.get_sha_checksum(),
                sizeof( sha_checksum ) );

    return true;
}

LPCSTR configs_verifyer::get_section_diff( CInifile::Sect* sect_ptr,
    ...
    for ( CInifile::SectCIt cit = sect_ptr->Data.begin(),
                            ciet = sect_ptr->Data.end();
          cit != ciet; ++cit ) {
        shared_str const& tmp_value = cit->second;
        shared_str real_value;
        if ( tmp_active_param ) {
            if ( active_params.line_exist( sect_ptr->Name.c_str(),
                                           cit->first ) ) {
                real_value = active_params.r_string( sect_ptr->Name.c_str(),
                                                     cit->first.c_str() );
                if ( tmp_value != real_value ) {
                    LPCSTR tmp_key_str = NULL;
                    STRCONCAT( tmp_key_str, sect_ptr->Name.c_str(),
                               "::", cit->first.c_str() );
                    STRCONCAT( diff_str, tmp_key_str, " = ", tmp_value.c_str(),
                               ",right = ", real_value.c_str() );
        ...
        if ( !pSettings->line_exist( sect_ptr->Name, cit->first ) ) {
            STRCONCAT( diff_str, "line ", sect_ptr->Name.c_str(),
                       "::", cit->first.c_str(), " not found" );
            strncpy_s( dst_diff, diff_str, sizeof( dst_diff ) - 1 );
            dst_diff[ sizeof( dst_diff ) - 1 ] = 0;
            return dst_diff;
        }
        real_value =
            pSettings->r_string( sect_ptr->Name.c_str(), cit->first.c_str() );
        if ( tmp_value != real_value ) {
            LPCSTR tmp_key_str = NULL;
            STRCONCAT( tmp_key_str, sect_ptr->Name.c_str(),
                       "::", cit->first.c_str() );
...
bool const configs_verifyer::verify( u8* data,
                                     u32 data_size,
                                     string256& diff ) {
    ...
    if ( std::search( data, data_end_ptr, ( u8* )check_substr,
                      ( u8* )end_checksubstr ) == data_end_ptr ) {
    ...
    while ( tmp_ini.line_exist( active_params_section, tmp_digit ) ) {
        LPCSTR tmp_ap_section =
            tmp_ini.r_string( active_params_section, tmp_digit );
        tmp_active_params.w_string( active_params_section, tmp_digit,
                                    tmp_ap_section );
    ...
    tmp_sha_checksum.start_calculate( m_orig_config_body.pointer(),
    ...
    if ( memcmp( tmp_checksum, tmp_sha_checksum.pointer(),
                 sizeof( tmp_checksum ) ) ) {
