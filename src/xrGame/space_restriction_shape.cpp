...
struct CBorderMergePredicate {
    ...
    IC void operator()( const CLevelGraph::CVertex& vertex ) const {
        if ( m_restriction->inside( ai().level_graph().vertex_id( &vertex ),
                                    true ) &&
             !m_restriction->inside( ai().level_graph().vertex_id( &vertex ),
                                     false ) )
            m_restriction->m_border.push_back(
                ai().level_graph().vertex_id( &vertex ) );
    }
...
struct CShapeTestPredicate {
    ...
    IC void operator()( const CLevelGraph::CVertex& vertex ) const {
        if ( m_restriction->inside( ai().level_graph().vertex_id( &vertex ),
                                    false ) )
            m_restriction->m_test_storage.push_back(
                ai().level_graph().vertex_id( &vertex ) );
    }
};
#endif
...
void CSpaceRestrictionShape::fill_shape( const CCF_Shape::shape_def& shape ) {
    ...
    switch ( shape.type ) {
        case 0: {
            start.sub( Fvector().set( shape.data.sphere.P ),
                       Fvector().set( shape.data.sphere.R, 0.f,
                                      shape.data.sphere.R ) );
            dest.add( Fvector().set( shape.data.sphere.P ),
                      Fvector().set( shape.data.sphere.R, 0.f,
                                     shape.data.sphere.R ) );
        ...
        case 1: {
            Fvector points[ 8 ] = { Fvector().set( -.5f, -.5f, -.5f ),
    ...
    ai().level_graph().iterate_vertices( start, dest,
                                         CBorderMergePredicate( this ) );

#ifdef DEBUG
    ai().level_graph().iterate_vertices( start, dest,
                                         CShapeTestPredicate( this ) );
#endif
}
...
void CSpaceRestrictionShape::build_border() {
    m_border.clear();
    CCF_Shape* shape =
        smart_cast< CCF_Shape* >( m_restrictor->collidable.model );
    VERIFY( shape );
    xr_vector< CCF_Shape::shape_def >::const_iterator I =
        shape->Shapes().begin();
    ...
    VERIFY3( !border().empty(), "space restrictor has no border",
             *m_restrictor->cName() );
...
void CSpaceRestrictionShape::test_correctness() {
    ...
    xr_vector< u32 > nodes;
    ai().graph_engine().search( ai().level_graph(), m_test_storage.back(),
                                m_test_storage.back(), &nodes,
