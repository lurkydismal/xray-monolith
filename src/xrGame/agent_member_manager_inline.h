...
IC CAgentMemberManager::CAgentMemberManager( CAgentManager* object )
    : m_last_throw_time( 0 ), m_throw_time_interval( 0 ) {
    VERIFY( object );
    m_object = object;
    m_actuality = true;
    m_combat_mask = 0;
...
IC const CAgentMemberManager::MEMBER_STORAGE& CAgentMemberManager::members()
    const {
    return ( m_members );
}
...
IC CMemberOrder& CAgentMemberManager::member( const CAI_Stalker* object ) {
    iterator I = std::find_if( members().begin(), members().end(),
                               CMemberPredicate( object ) );
    VERIFY( I != members().end() );
    return ( **I );
}

IC MemorySpace::squad_mask_type CAgentMemberManager::mask(
...
IC CAgentMemberManager::iterator CAgentMemberManager::member(
    MemorySpace::squad_mask_type mask ) {
...
IC const CAgentMemberManager::squad_mask_type&
CAgentMemberManager::combat_mask() const {
