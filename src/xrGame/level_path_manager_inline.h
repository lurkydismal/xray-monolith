...
#define CLevelManagerTemplate                                         \
    CBasePathManager< CLevelGraph, _VertexEvaluator, _vertex_id_type, \
                      _index_type >
...
IC bool CLevelManagerTemplate::actual() const {
    return (
        inherited::actual( m_object->object().ai_location().level_vertex_id(),
                           dest_vertex_id() ) );
}

TEMPLATE_SPECIALIZATION
IC void CLevelManagerTemplate::build_path(
    const _vertex_id_type start_vertex_id,
    const _vertex_id_type dest_vertex_id ) {
    START_PROFILE( "Build Path/Level Path" );

    THROW( ai().level_graph().valid_vertex_id( start_vertex_id ) &&
           ai().level_graph().valid_vertex_id( dest_vertex_id ) );
    ...
    if ( failed() ) {
        Msg( "! NPC %s couldn't build path from \n~ [%d][%f][%f][%f]\n~ to\n~ "
             ...
             *m_object->object().cName(), start_vertex_id,
             VPUSH( ai().level_graph().vertex_position( start_vertex_id ) ),
             dest_vertex_id,
             VPUSH( ai().level_graph().vertex_position( dest_vertex_id ) ) );
    }
#endif
...
IC void CLevelManagerTemplate::before_search(
    ...
    if ( m_object ) {
        ...
        VERIFY( !m_object->applied() ||
                ai().level_graph().is_accessible( start_vertex_id ) );
        VERIFY( !m_object->applied() ||
                ai().level_graph().is_accessible( dest_vertex_id ) );
    }
}
...
TEMPLATE_SPECIALIZATION
IC bool CLevelManagerTemplate::check_vertex(
    const _vertex_id_type vertex_id ) const {
    return ( inherited::check_vertex( vertex_id ) &&
             ( !m_object || object().accessible( vertex_id ) ) );
}
