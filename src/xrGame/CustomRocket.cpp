...
void CCustomRocket::SetLaunchParams( const Fmatrix& xform,
    ...
    m_time_to_explode =
        Device.fTimeGlobal +
        pSettings->r_float( cNameSect(), "force_explode_time" ) / 1000.0f;
#ifdef DEBUG
    gbg_rocket_speed1 = 0;
    gbg_rocket_speed2 = 0;
#endif
...
void CCustomRocket::activate_physic_shell() {
    ...
    if ( m_pPhysicsShell->isActive() )
        return;
    VERIFY2( _valid( m_LaunchXForm ),
             "CCustomRocket::activate_physic_shell. Invalid m_LaunchXForm!" );
    ...
    //\n",m_vLaunchVelocity.x,m_vLaunchVelocity.y,m_vLaunchVelocity.z);
    //	}
    m_pPhysicsShell->Activate( m_LaunchXForm, m_vLaunchVelocity,
                               m_vLaunchAngularVelocity );
...
void CCustomRocket::create_physic_shell() {
    ...
    float radius;
    CHOOSE_MAX(
        obb.m_halfsize.x, ax.set( obb.m_rotate.i ); ax.mul( obb.m_halfsize.x );
        radius = _min( obb.m_halfsize.y, obb.m_halfsize.z );
        obb.m_halfsize.y /= 2.f;
        obb.m_halfsize.z /= 2.f, obb.m_halfsize.y, ax.set( obb.m_rotate.j );
        ax.mul( obb.m_halfsize.y );
        radius = _min( obb.m_halfsize.x, obb.m_halfsize.z );
        obb.m_halfsize.x /= 2.f;
        obb.m_halfsize.z /= 2.f, obb.m_halfsize.z, ax.set( obb.m_rotate.k );
...
void CCustomRocket::ObjectContactCallback( bool& do_colide,
                                           bool bo1,
                                           dContact& c,
                                           SGameMtl* material_1,
                                           SGameMtl* material_2 ) {
    ...
    SGameMtl* material = 0;
    CCustomRocket* l_this =
        l_pUD1 ? smart_cast< CCustomRocket* >( l_pUD1->ph_ref_object ) : NULL;
    Fvector vUp;
    if ( !l_this ) {
        l_this = l_pUD2 ? smart_cast< CCustomRocket* >( l_pUD2->ph_ref_object )
                        : NULL;
    ...
    CGameObject* l_pOwner =
        l_pUD1 ? smart_cast< CGameObject* >( l_pUD1->ph_ref_object ) : NULL;
    if ( !l_pOwner || l_pOwner == ( CGameObject* )l_this )
        l_pOwner =
            l_pUD2 ? smart_cast< CGameObject* >( l_pUD2->ph_ref_object ) : NULL;
            ...
            if ( !l_pUD1 || !l_pUD2 ) {
                ...
                if ( l_pUD->pushing_neg ) {
                    ...
                    if ( velocity.square_magnitude() > EPS ) {
                        ...
                        Triangle neg_tri;
                        CalculateTriangle(
                            l_pUD->neg_tri, g, neg_tri,
                            Level().ObjectSpace.GetStaticVerts() );
                        float cosinus = velocity.dotproduct(
                            *( ( Fvector* )neg_tri.norm ) );
...
//.	DBG_ClosedCashedDraw(50000);
            ...
            if ( ph_dbg_draw_mask.test( phDbgDrawExplosionPos ) )
                DBG_DrawPoint(
                    l_pos, 0.05f,
                    D3DCOLOR_XRGB( 255, 255, ( !corrected_pos ) * 255 ) );
#endif
...
void CCustomRocket::reload( LPCSTR section ) {
    ...
    if ( pSettings->line_exist( section, "snd_fly_sound" ) ) {
        m_flyingSound.create( pSettings->r_string( section, "snd_fly_sound" ),
...
void CCustomRocket::OnH_B_Independent( bool just_before_destroy ) {
    ...
    //-------------------------------------------
    m_pOwner =
        H_Parent() ? smart_cast< CGameObject* >( H_Parent()->H_Root() ) : NULL;
    //-------------------------------------------
}
...
void CCustomRocket::UpdateEngine() {
    ...
    if ( !m_pPhysicsShell )
        Msg( "! CCustomRocket::UpdateEngine called, but m_pPhysicsShell is "
             "NULL" );
...
void CCustomRocket::StartLights() {
    ...
    m_pTrailLight->set_color( m_TrailLightColor.r, m_TrailLightColor.g,
                              m_TrailLightColor.b );
...
void CCustomRocket::UpdateParticles() {
    ...
    Fvector::generate_orthonormal_basis( particles_xform.k, particles_xform.j,
                                         particles_xform.i );
...
void CCustomRocket::StartEngineParticles() {
    ...
    if ( !m_sEngineParticles )
        return;
    m_pEngineParticles =
        Particles::Details::Create( *m_sEngineParticles, FALSE );
...
void CCustomRocket::StartFlyParticles() {
    ...
    VERIFY3( m_pFlyParticles->IsLooped(),
             "must be a looped particle system for rocket fly: %s",
             *m_sFlyParticles );
}
