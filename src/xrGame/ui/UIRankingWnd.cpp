...
CUIRankingWnd::~CUIRankingWnd() {
    ...
    RANKINGCOC_VEC_IT be = m_coc_ranking_vec.begin(),
                      en = m_coc_ranking_vec.end();
    for ( ; be != en; be++ )
        xr_delete( *be );
...
void CUIRankingWnd::Init() {
    ...
    for ( u8 i = 0; i < m_stat_count; ++i ) {
        ...
        pos.x = m_stat_caption[ i ]->GetWndPos().x +
                m_stat_caption[ i ]->GetWndSize().x + 5.0f;
        m_stat_info[ i ]->SetWndPos( pos );
    }
    xml.SetLocalRoot( stored_root );

    // Achievements
    m_achievements_background =
        UIHelper::CreateFrameWindow( xml, "achievements_background", this );
    ...
    VERIFY2( pSettings->section_exist( section ),
             make_string( "Section [%s] does not exist!", section ) );
    ...
    // Alundaio: CoC Rankings
    m_coc_ranking_background =
        UIHelper::CreateFrameWindow( xml, "coc_ranking_background", this );
    ...
    if ( ai().script_engine().functor( "pda.get_rankings_array_size",
                                       getRankingArraySize ) ) {
        topRankCount = getRankingArraySize();
    }

    for ( u8 i = 1; i <= topRankCount; i++ ) {
        CUIRankingsCoC* character_rank_item =
            xr_new< CUIRankingsCoC >( m_coc_ranking );
    ...
    CUIXmlInit::InitScrollView( xml, "coc_ranking_wnd_actor", 0,
                                m_coc_ranking_actor_view );
...
void CUIRankingWnd::add_achievement( CUIXml& xml,
                                     shared_str const& achiev_id ) {
    CUIAchievements* achievement = xr_new< CUIAchievements >( m_achievements );
    VERIFY2( pSettings->section_exist( achiev_id ),
             make_string( "Section [%s] does not exist!", achiev_id ) );
    ...
    achievement->SetRepeatable(
        !!READ_IF_EXISTS( pSettings, r_bool, achiev_id, "repeatable", false ) );
...
void CUIRankingWnd::update_info() {
    ...
    RANKINGCOC_VEC_IT begin = m_coc_ranking_vec.begin(),
                      end = m_coc_ranking_vec.end();
    for ( ; begin != end; begin++ )
        ( *begin )->Update();
...
void CUIRankingWnd::get_statistic() {
    /*
    string128 buf;
    InventoryUtilities::GetTimePeriodAsString(buf, sizeof(buf),
    Level().GetStartGameTime(), Level().GetGameTime());
