...
CUIHudStatesWnd::CUIHudStatesWnd()
    ...
    m_health_blink =
        pSettings->r_float( "actor_condition", "hud_health_blink" );
...
ALife::EInfluenceType CUIHudStatesWnd::get_indik_type(
...
void CUIHudStatesWnd::InitFromXml( CUIXml& xml, LPCSTR path ) {
    ...
    m_ui_health_bar =
        UIHelper::CreateProgressBar( xml, "progress_bar_health", this );
    m_ui_health_bar_show = true;
    m_ui_stamina_bar =
        UIHelper::CreateProgressBar( xml, "progress_bar_stamina", this );
    ...
    /*
            m_resist_back[ALife::infl_rad]  = UIHelper::CreateStatic( xml,
       "resist_back_rad", this ); m_resist_back[ALife::infl_fire] =
       UIHelper::CreateStatic( xml, "resist_back_fire", this );
            m_resist_back[ALife::infl_acid] = UIHelper::CreateStatic( xml,
       "resist_back_acid", this ); m_resist_back[ALife::infl_psi]  =
       UIHelper::CreateStatic( xml, "resist_back_psi", this );
            // electra = no has CStatic!!
    */
    m_indik[ ALife::infl_rad ] =
        UIHelper::CreateStatic( xml, "indik_rad", this );
    m_indik[ ALife::infl_fire ] =
        UIHelper::CreateStatic( xml, "indik_fire", this );
    m_indik[ ALife::infl_acid ] =
        UIHelper::CreateStatic( xml, "indik_acid", this );
    m_indik[ ALife::infl_psi ] =
    ...
    m_ui_weapon_fmj_ammo =
        UIHelper::CreateTextWnd( xml, "static_fmj_ammo", this );
    m_ui_weapon_ap_ammo =
        UIHelper::CreateTextWnd( xml, "static_ap_ammo", this );

    // Alundaio: Option to display a third ammo type
    m_ui_weapon_third_ammo =
        UIHelper::CreateTextWnd( xml, "static_third_ammo", this );
    //-Alundaio
    ...
    /*
            m_bleeding_lev1 = UIHelper::CreateStatic( xml, "bleeding_level_1",
       this ); m_bleeding_lev1->Show( false );

            m_bleeding_lev2 = UIHelper::CreateStatic( xml, "bleeding_level_2",
       this ); m_bleeding_lev2->Show( false );

            m_bleeding_lev3 = UIHelper::CreateStatic( xml, "bleeding_level_3",
       this ); m_bleeding_lev3->Show( false );

            m_radiation_lev1 = UIHelper::CreateStatic( xml, "radiation_level_1",
       this ); m_radiation_lev1->Show( false );

            m_radiation_lev2 = UIHelper::CreateStatic( xml, "radiation_level_2",
       this ); m_radiation_lev2->Show( false );

            m_radiation_lev3 = UIHelper::CreateStatic( xml, "radiation_level_3",
       this ); m_radiation_lev3->Show( false );
...
void CUIHudStatesWnd::Load_section() {
    ...
    //	m_actor_radia_factor = pSettings->r_float( "radiation_zone_detector",
    //"actor_radia_factor" );
    ...
    Load_section_type( ALife::infl_electra,
                       "electra_zone_detector" ); // no uistatic
}

void CUIHudStatesWnd::Load_section_type( ALife::EInfluenceType type,
                                         LPCSTR section ) {
...
void CUIHudStatesWnd::UpdateHealth( CActor* actor ) {
    ...
    if ( m_ui_health_bar_show == false ) {
        ...
        m_ui_health_bar->SetProgressPos( iCeil( cur_health * 100.0f * 35.f ) /
                                         35.f );
    ...
    if ( m_ui_stamina_bar_show == false ) {
        ...
        m_ui_stamina_bar->SetProgressPos( iCeil( cur_stamina * 100.0f * 35.f ) /
                                          35.f );
    ...
    /*
            ...
            if ( outfit )
            {
                    ...
                    m_ui_armor_bar->SetProgressPos( outfit->GetCondition() *
       100.0f );
            }
            else
            {
...
void CUIHudStatesWnd::UpdateActiveItemInfo( CActor* actor ) {
    ...
    if ( item ) {
        ...
        CWeaponMagazinedWGrenade* wpn =
            smart_cast< CWeaponMagazinedWGrenade* >( item );
        ...
        if ( wpnm ) {
            if ( wpnm->m_ammoType == 0 )
                m_ui_weapon_fmj_ammo->SetTextColor(
                    color_rgba( 238, 155, 23, 255 ) );
            else if ( wpnm->m_ammoType == 1 )
                m_ui_weapon_ap_ammo->SetTextColor(
                    color_rgba( 238, 155, 23, 255 ) );
            else if ( wpnm->m_ammoType == 2 )
                m_ui_weapon_third_ammo->SetTextColor(
                    color_rgba( 238, 155, 23, 255 ) );
        }
        //-Alundaio
    }
...
void CUIHudStatesWnd::SetAmmoIcon( const shared_str& sect_name ) {
    ...
    Frect texture_rect;
    texture_rect.x1 =
        pSettings->r_float( sect_name, "inv_grid_x" ) * INV_GRID_WIDTH;
    texture_rect.y1 =
        pSettings->r_float( sect_name, "inv_grid_y" ) * INV_GRID_HEIGHT;
    texture_rect.x2 =
        pSettings->r_float( sect_name, "inv_grid_width" ) * INV_GRID_WIDTH;
    texture_rect.y2 =
        pSettings->r_float( sect_name, "inv_grid_height" ) * INV_GRID_HEIGHT;
    ...
    if ( pSettings->line_exist( sect_name, "icons_texture" ) ) {
        LPCSTR icons_texture =
            pSettings->r_string( sect_name, "icons_texture" );
        m_ui_weapon_icon->SetShader(
...
void CUIHudStatesWnd::UpdateZones() {
    ...
    // float actor_radia = m_actor->conditions().GetRadiation() *
    // m_actor_radia_factor; m_radia_hit = _max( m_zone_cur_power[it_rad],
    // actor_radia );
    ...
    float zone_max_power =
        actor->conditions().GetZoneMaxPower( ALife::infl_rad );
    ...
    */
    ...
    for ( ; itb != ite; ++itb ) {
        ...
        */
        ...
        float fRelPow =
            ( dist_to_zone / ( rad_zone + ( z_type == ALife::infl_max_count )
                                   ? 5.0f
                                   : m_zone_feel_radius[ z_type ] + 0.1f ) ) -
            0.1f;
        ...
        if ( ( z_type != ALife::infl_max_count ) &&
             ( m_zone_cur_power[ z_type ] < power ) ) // max
        {
            m_zone_cur_power[ z_type ] = power;
        }

        if ( dist_to_zone <
             rad_zone + 0.9f * ( ( z_type == ALife::infl_max_count )
                                     ? 5.0f
                                     : m_zone_feel_radius[ z_type ] ) ) {
            fRelPow *= 0.6f;
        ...
        zone_info.cur_period =
            zone_type->freq.x +
            ( zone_type->freq.y - zone_type->freq.x ) * ( fRelPow * fRelPow );
        ...
        if ( zone_info.snd_time > zone_info.cur_period ) {
            ...
            if ( ai().script_engine().functor( "_G.CZone_Touch", funct ) ) {
                if ( funct( pZone->lua_game_object() ) )
                    HUD_SOUND_ITEM::PlaySound( zone_type->detect_snds,
                                               Fvector().set( 0, 0, 0 ), NULL,
                                               true, false );
            }
        } else {
...
void CUIHudStatesWnd::UpdateIndicatorType( CActor* actor,
    ...
    */
    ...
    switch ( type ) {
        case ALife::infl_rad:
            texture = "ui_inGame2_triangle_Radiation_";
            break;
        case ALife::infl_fire:
            texture = "ui_inGame2_triangle_Fire_";
            break;
        case ALife::infl_acid:
            texture = "ui_inGame2_triangle_Biological_";
            break;
        case ALife::infl_psi:
            texture = "ui_inGame2_triangle_Psy_";
            break;
        default:
            NODEFAULT;
    }
    ...
    CHelmet* helmet = smart_cast< CHelmet* >(
        actor->inventory().ItemFromSlot( HELMET_SLOT ) );
    float protect =
        ( outfit ) ? outfit->GetDefHitTypeProtection( hit_type ) : 0.0f;
    ...
    CEntityCondition::BOOSTER_MAP& cur_booster_influences =
        actor->conditions().GetCurBoosterInfluences();
    ...
    actor->conditions().SetZoneDanger(
        ( hit_power - protect ) /
            actor->conditions().GetZoneMaxPower( hit_type ),
        type );
}
...
void CUIHudStatesWnd::SwitchLA( bool state, ALife::EInfluenceType type )
{
        ...
        if ( state )
        {
                m_indik[type]->SetColorAnimation( m_lanim_name.c_str(),
LA_CYCLIC|LA_TEXTURECOLOR); m_cur_state_LA[type] = true;
        }
        else
        {
