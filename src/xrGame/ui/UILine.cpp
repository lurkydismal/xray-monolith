...
CUILine::~CUILine() {
    ...
    if ( !bOK )
        Msg( "CUILine::~CUILine()!!!!!!!!!!!!!!!!!!!!!!! cannot find window in "
             "list" );
#endif
}
...
const CUILine* CUILine::Cut2Pos( Position& pos, bool to_first ) {
    ...
    for ( int i = 0; i <= last; i++ ) {
        ...
        if ( m_subLines[ i ]
                 .m_last_in_line ) // check if this subline must be last in line
        {
            for ( int j = 0; j <= i; j++ )
    ...
    if ( to_first )
        m_tmpLine->AddSubLine(
            m_subLines[ last + 1 ].Cut2Pos( pos.word_1.last_space() ) );
    else
        m_tmpLine->AddSubLine(
            m_subLines[ last + 1 ].Cut2Pos( pos.word_2.last_space() ) );
