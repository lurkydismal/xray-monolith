...
void CStoreHierarchy::LoadLevel( CUIXml& xml,
                                 int index,
                                 item* _item,
                                 int depth_level ) {
    ...
    if ( depth_level > 0 && _item->m_btn_xml_name.size() ) {
        ...
        CUIXmlInit::InitTabButtonMP( xml, _item->m_btn_xml_name.c_str(), 0,
                                     btn );
        btn->m_btn_id = _item->m_name;
        int horz =
            xml.ReadAttribInt( _item->m_btn_xml_name.c_str(), 0, "horz_al", 0 );
...
void CStoreHierarchy::InitItemsInGroup( const shared_str& sect, item* _itm ) {
    if ( !_itm ) {
        ...
        VERIFY2( !pSettings->line_exist( sect, "team_name" ),
                 make_string( "there is no line [team_name] in section [%s]",
                              sect.c_str() ) );
    ...
    if ( !_itm->HasSubLevels() ) {
        ...
        for ( u32 i = 0; i < n; ++i ) {
            ...
            VERIFY3( g_mp_restrictions.GetItemGroup( buff ).size(),
                     "item has no group in restrictions", buff );
        }
#ifndef MASTER_GOLD
...
const CStoreHierarchy::item& CStoreHierarchy::item::Child(
...
CStoreHierarchy::item* CStoreHierarchy::FindItem(
    const shared_str& name_sect,
    CStoreHierarchy::item* recurse_from ) {
    if ( !recurse_from )
        recurse_from = m_root;
    ...
    if ( recurse_from->HasSubLevels() ) {
        ...
        xr_vector< CStoreHierarchy::item* >::const_iterator it =
            recurse_from->m_childs.begin();
        xr_vector< CStoreHierarchy::item* >::const_iterator it_e =
            recurse_from->m_childs.end();

        for ( ; it != it_e; ++it )
            if ( FindItem( name_sect, *it ) )
                return *it;
    } else {
        xr_vector< shared_str >::const_iterator it =
            recurse_from->m_items_in_group.begin();
        xr_vector< shared_str >::const_iterator it_e =
            recurse_from->m_items_in_group.end();
