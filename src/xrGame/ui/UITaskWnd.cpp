...
void CUITaskWnd::Init() {
    ...
    AddCallback(
        m_cbTreasures, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUITaskWnd::OnShowTreasures ) );
    m_bTreasuresEnabled = true;

    m_cbPrimaryObjects =
        UIHelper::CreateCheck( xml, "filter_primary_objects", this );
    m_cbPrimaryObjects->SetCheck( true );
    AddCallback( m_cbPrimaryObjects, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUITaskWnd::OnShowPrimaryObjects ) );
    m_bPrimaryObjectsEnabled = true;

    m_cbSecondaryTasks =
        UIHelper::CreateCheck( xml, "filter_secondary_tasks", this );
    m_cbSecondaryTasks->SetCheck( true );
    AddCallback( m_cbSecondaryTasks, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUITaskWnd::OnShowSecondaryTasks ) );
    m_bSecondaryTasksEnabled = true;

    m_cbQuestNpcs = UIHelper::CreateCheck( xml, "filter_quest_npcs", this );
    m_cbQuestNpcs->SetCheck( true );
    AddCallback(
        m_cbQuestNpcs, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUITaskWnd::OnShowQuestNpcs ) );
    ...
    m_center_background =
        UIHelper::CreateStatic( xml, "center_background", this );
    ...
    AddCallback(
        m_pStoryLineTaskItem, WINDOW_LBUTTON_DB_CLICK,
    ...
    AddCallback(
        m_btn_focus, BUTTON_DOWN,
        CUIWndCallback::void_function( this, &CUITaskWnd::OnTask1DbClicked ) );

    m_BtnTaskListWnd = UIHelper::Create3tButton( xml, "btn_second_task", this );
    AddCallback(
        m_BtnTaskListWnd, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUITaskWnd::OnShowTaskListWnd ) );
...
void CUITaskWnd::Update() {
    ...
    if ( m_pStoryLineTaskItem->show_hint &&
         m_pStoryLineTaskItem->OwnerTask() ) {
        m_pMapWnd->ShowHintTask( m_pStoryLineTaskItem->OwnerTask(),
...
void CUITaskWnd::ReloadTaskInfo() {
    ...
    if ( t &&
         ( t->m_map_object_id == u16( -1 ) || t->m_map_location.size() == 0 ) )
    ...
    for ( ; b != e; b++ ) {
        ...
        if ( spot == "treasure" )
            m_bTreasuresEnabled ? b->location->EnableSpot()
                                : b->location->DisableSpot();
        else if ( spot == "primary_object" )
            m_bPrimaryObjectsEnabled ? b->location->EnableSpot()
                                     : b->location->DisableSpot();
        else if ( spot == "secondary_task_location" ||
                  spot == "secondary_task_location_complex_timer" )
            ( /*b->location->SpotEnabled() && */ m_bSecondaryTasksEnabled )
                ? b->location->EnableSpot()
                : b->location->DisableSpot();
        else if ( spot == "ui_pda2_trader_location" ||
                  spot == "ui_pda2_mechanic_location" ||
                  spot == "ui_pda2_scout_location" ||
                  spot == "ui_pda2_quest_npc_location" ||
                  spot == "ui_pda2_medic_location" ||
                  spot == "ui_pda2_actor_box_location" ||
                  spot == "ui_pda2_actor_sleep_location" )
            m_bQuestNpcsEnabled ? b->location->EnableSpot()
                                : b->location->DisableSpot();
    }
    ...
    u32 task_count =
        Level().GameTaskManager().GetTaskCount( eTaskStateInProgress );
    if ( task_count ) {
        u32 task_index =
            Level().GameTaskManager().GetTaskIndex( t, eTaskStateInProgress );
        string32 buf;
...
void CUITaskWnd::TaskShowMapSpot( CGameTask* task, bool show ) {
    ...
    if ( ml ) {
        if ( show ) {
            ml->EnableSpot();
            ml->CalcPosition();
            m_pMapWnd->SetTargetMap( ml->GetLevelName(), ml->GetPosition(),
                                     true );
        } else {
            ml->DisableSpot();
        }
