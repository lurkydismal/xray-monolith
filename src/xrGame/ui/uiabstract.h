...
public:
    ...
    IC void GetWndRect( Frect& res ) const {
        switch ( m_alignment ) {
            case waNone:
                res.set( m_wndPos.x, m_wndPos.y, m_wndPos.x + m_wndSize.x,
                         m_wndPos.y + m_wndSize.y );
                break;
            case waCenter: {
                float half_w = m_wndSize.x / 2.0f;
                float half_h = m_wndSize.y / 2.0f;
                res.set( m_wndPos.x - half_w, m_wndPos.y - half_h,
                         m_wndPos.x + half_w, m_wndPos.y + half_h );
            } break;
            default:
                NODEFAULT;
        };
