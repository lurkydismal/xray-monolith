...
CUIPropertiesBox* CUIPropertiesBox::AddSubmenu( LPCSTR label ) {
    R_ASSERT2( GetParent(),
               "AddSubmenu: menu must be attached before adding submenus" );
    ...
    sub->InitPropertiesBox( Fvector2().set( 0, 0 ),
                            Fvector2().set( GetWidth(), 300.0f ) );
...
bool CUIPropertiesBox::OnMouseAction( float x,
    ...
    if ( mouse_action == WINDOW_MOUSE_WHEEL_DOWN ||
        ...
        for ( CUIPropertiesBox* b = root; b;
              b = ( b->m_active_submenu && b->m_active_submenu->IsShown() )
                      ? b->m_active_submenu
                      : NULL ) {
            Frect r;
            b->GetAbsoluteRect( r );
        ...
        if ( target->m_UIListWnd.GetPadSize().y >
             target->m_UIListWnd.GetHeight() )
