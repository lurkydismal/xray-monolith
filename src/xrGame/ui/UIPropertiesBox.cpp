...
void CUIPropertiesBox::InitPropertiesBox( Fvector2 pos, Fvector2 size ) {
    ...
    m_UIListWnd.SetWndSize(
        Fvector2().set( size.x - OFFSET_X * 2, size.y - OFFSET_Y * 2 ) );
}
...
void CUIPropertiesBox::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    if ( pWnd == &m_UIListWnd ) {
        if ( msg == LIST_ITEM_CLICKED ) {
            ...
            {
                Hide();
                if ( m_parent_sub_menu ) // if i have a parent sub menu, close
                                         // it
...
void CUIPropertiesBox::ShowSubMenu() {
    ...
    tmp_pbox_pos.y += m_item_sub_menu_initiator->GetWndPos().y +
                      ( m_item_sub_menu_initiator->GetHeight() / 2 );

    float right_limit =
        tmp_pbox_pos.x + GetWidth() + m_sub_property_box->GetWidth();
...
bool CUIPropertiesBox::AddItem( LPCSTR str, void* pData, u32 tag_value ) {
    ...
    itm->SetData( pData );
    if ( m_sub_property_box ) {
        AddCallback( itm, WINDOW_FOCUS_RECEIVED,
                     CUIWndCallback::void_function(
                         this, &CUIPropertiesBox::OnItemReceivedFocus ) );
        Register( itm );
    }
    return true;
}
...
void CUIPropertiesBox::Show( const Frect& parent_rect, const Fvector2& point ) {
    ...
    if ( point.x - prop_size.x > parent_rect.x1 &&
         point.y + prop_size.y < parent_rect.y2 ) {
        prop_pos.set( point.x - prop_size.x, point.y );
    } else if ( point.x - prop_size.x > parent_rect.x1 &&
                point.y - prop_size.y > parent_rect.y1 ) {
        prop_pos.set( point.x - prop_size.x, point.y - prop_size.y );
    } else if ( point.x + prop_size.x < parent_rect.x2 &&
                point.y - prop_size.y > parent_rect.y1 ) {
...
bool CUIPropertiesBox::OnMouseAction( float x,
                                      float y,
...
void CUIPropertiesBox::AutoUpdateSize() {
    ...
    sz.y = m_UIListWnd.GetItemHeight() * m_UIListWnd.GetSize() +
           m_UIListWnd.GetVertIndent();
    sz.x =
        float( m_UIListWnd.GetLongestLength() + m_UIListWnd.GetHorizIndent() ) +
        2;
    SetWndSize( sz );
...
bool CUIPropertiesBox::OnKeyboardAction( int dik,
