...
CUIDragDropReferenceList::CUIDragDropReferenceList() {
    AddCallbackStr( "cell_item_reference", WINDOW_LBUTTON_DB_CLICK,
                    CUIWndCallback::void_function(
                        this, &CUIDragDropReferenceList::OnItemDBClick ) );
}
...
void CUIDragDropReferenceList::Initialize() {
    for ( int i = 0; i < m_container->CellsCapacity().x; i++ ) {
        ...
        Fvector2 pos = Fvector2().set(
            ( m_container->CellSize().x + m_container->CellsSpacing().x ) * i,
            0 );
        m_references.back()->SetAutoDelete( true );
        m_references.back()->SetWndPos( pos );
        m_references.back()->SetWndSize( Fvector2().set(
            m_container->CellSize().x, m_container->CellSize().y ) );
...
void CUIDragDropReferenceList::LoadItemTexture( LPCSTR section,
                                                Ivector2 cell_pos ) {
    CUIStatic* ref = m_references[ cell_pos.x ];
...
void CUIDragDropReferenceList::OnItemDBClick( CUIWindow* w, void* pData ) {
    ...
    ITEMS_REFERENCES_VEC_IT it =
        std::find( m_references.begin(), m_references.end(), ref );
    if ( it != m_references.end() ) {
        u8 index = u8( it - m_references.begin() );
        CActor* actor = smart_cast< CActor* >( Level().CurrentViewEntity() );
        if ( actor ) {
            PIItem itm = actor->inventory().GetAny(
                ACTOR_DEFS::g_quick_use_slots[ index ] );
            if ( itm )
                inherited::RemoveItem(
                    GetCellAt( Ivector2().set( index, 0 ) ).m_item, false );
        }
        xr_strcpy( ACTOR_DEFS::g_quick_use_slots[ index ], "" );
    ...
    if ( actor ) {
        ...
        if ( vec.x != -1 && vec.y != -1 ) {
            ...
            if ( vec2.x != -1 && vec2.y != -1 ) {
                ...
                xr_strcpy( ACTOR_DEFS::g_quick_use_slots[ vec.x ],
                           ACTOR_DEFS::g_quick_use_slots[ index ] );
                xr_strcpy( ACTOR_DEFS::g_quick_use_slots[ index ],
                           tmp.c_str() );
