...
bool CUIScrollArrowButton::OnMouseAction( float x,
                                          float y,
...
CUIScrollArrowButton::CUIScrollArrowButton() : m_applied_state( S_Enabled ) {
...
void CUITabScrollArrows::Build( CUITabButton* ref ) {
    ...
    for ( int s = 0; s < 2; ++s ) {
        ...
        arrow->InitButton( Fvector2().set( 0.0f, 0.0f ),
                           Fvector2().set( width, height ) );
...
void CUITabScrollArrows::Layout( float view_right, float strip_y ) {
    for ( int s = 0; s < 2; ++s )
        if ( m_arrow[ s ] )
            m_arrow[ s ]->SetWndPos(
                Fvector2().set( ( s == eLeft ) ? 0.0f : view_right, strip_y ) );
}
