...
void CUICustomMap::Init_internal( const shared_str& name,
                                  CInifile& pLtx,
...
// meters->pixels (relatively own left-top pos)
{
    ...
    if ( !Heading() ) {
        ...
        rotation_( res.x, res.y, GetHeading(), res.x, res.y,
                   for_drawing ? UI().get_current_kx() : 1.0f );
...
// meters->pixels (relatively own left-top pos)
{
    ...
    res.y = ( bound_rect.height() - ( src.y - bound_rect.lt.y ) ) *
            GetCurrentZoom().x;
...
bool CUICustomMap::GetPointerTo( const Fvector2& src,
                                 float item_radius,
                                 Fvector2& pos,
                                 float& heading ) {
...
bool CUIGlobalMap::OnMouseAction( float x, float y, EUIMessages mouse_action ) {
    ...
    if ( mouse_action == WINDOW_MOUSE_MOVE &&
         ( FALSE == pInput->iGetAsyncBtnState( 0 ) ) ) {
        if ( MapWnd() ) {
            MapWnd()->Hint( MapName() );
            return true;
        }
...
void CUIGlobalMap::Init_internal( const shared_str& name,
                                  CInifile& pLtx,
                                  const shared_str& sect_name,
                                  LPCSTR sh_name ) {
...
float CUIGlobalMap::CalcOpenRect( const Fvector2& center_point,
                                  Frect& map_desired_rect,
                                  float tgt_zoom ) {
    Fvector2 new_center_pt;
    // calculate desired rect in new zoom
    map_desired_rect.set( 0.0f, 0.0f, BoundRect().width() * tgt_zoom,
                          BoundRect().height() * tgt_zoom );

    // calculate center point in new zoom (center_point is in identity global
    // map space)
    ...
    Fvector2 delta_pos;
    delta_pos.set( new_center_pt.x - vis_w * 0.5f,
                   new_center_pt.y - vis_h * 0.5f );
...
void CUILevelMap::Draw() {
    if ( MapWnd() ) {
        ...
        if ( m_label && m_label_scale_max > 0.0f )
            m_label->SetVisible( !!pda_show_map_labels &&
                                 ( gmz < m_label_scale_max ) );
...
void CUILevelMap::Init_internal( const shared_str& name,
                                 CInifile& pLtx,
                                 const shared_str& sect_name,
                                 LPCSTR sh_name ) {
    inherited::Init_internal( name, pLtx, sect_name, sh_name );
    ...
    if ( pGameIni->line_exist( MapName(), "label" ) ) {
        ...
        m_label_scale_max =
            pGameIni->line_exist( MapName(), "label_scale_max" )
    ...
    if ( FALSE == fsimilar( kw, kh, EPS_L ) ) {
        Msg( " --incorrect global rect definition for map [%s]  kw=%f kh=%f",
             *MapName(), kw, kh );
        Msg( " --try x2=%f or  y2=%f",
             m_GlobalRect.x1 + kh * BoundRect().width(),
...
void CUILevelMap::Update() {
    ...
    if ( m_bCursorOverWindow ) {
        ...
        if ( Device.dwTimeGlobal > ( m_dwFocusReceiveTime + 500 ) ) {
            if ( fsimilar( MapWnd()->GlobalMap()->GetCurrentZoom().x,
                           MapWnd()->GlobalMap()->GetMinZoom(), EPS_L ) )
...
void CUIMiniMap::Init_internal( const shared_str& name,
                                CInifile& pLtx,
                                const shared_str& sect_name,
                                LPCSTR sh_name ) {
    inherited::Init_internal( name, pLtx, sect_name, sh_name );
...
void CUIMiniMap::Draw() {
    ...
    UIRender->StartPrimitive( segments_count * 3, IUIRender::ptTriList,
    ...
    Fvector2 m_scale_;
    m_scale_.set( float( Device.dwWidth ) / UI_BASE_WIDTH,
                  float( Device.dwHeight ) / UI_BASE_HEIGHT );
    ...
    for ( u32 idx = 0; idx < segments_count - 2; ++idx ) {
        UIRender->PushPoint( S[ 0 + 0 ].pt.x, S[ 0 + 0 ].pt.y, 0, color,
                             S[ 0 + 0 ].uv.x, S[ 0 + 0 ].uv.y );
        UIRender->PushPoint( S[ idx + 2 ].pt.x, S[ idx + 2 ].pt.y, 0, color,
                             S[ idx + 2 ].uv.x, S[ idx + 2 ].uv.y );
        UIRender->PushPoint( S[ idx + 1 ].pt.x, S[ idx + 1 ].pt.y, 0, color,
                             S[ idx + 1 ].uv.x, S[ idx + 1 ].uv.y );
    }
...
bool CUIMiniMap::GetPointerTo( const Fvector2& src,
                               float item_radius,
                               Fvector2& pos,
                               float& heading ) {
    if ( !IsRounded() )
