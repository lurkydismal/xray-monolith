...
void xr_stdcall CUIMpTradeWnd::OnBtnPresetDefaultClicked( CUIWindow* w,
                                                          void* d ) {
    CheckDragItemToDestroy();
    {
        if ( pInput->iGetAsyncKeyState( DIK_LSHIFT ) ) {
...
#ifdef DEBUG
    Msg( "current level=[%s]",
         m_store_hierarchy->CurrentLevel().m_name.c_str() );
#endif // #ifdef DEBUG
    SetCurrentItem( NULL );
...
void CUIMpTradeWnd::FillUpSubLevelButtons() {
    ...
    for ( u32 i = 0; i < root_cnt; ++i ) {
        const CStoreHierarchy::item& it =
            m_store_hierarchy->CurrentLevel().ChildAtIdx( i );
        CUITabButtonMP* btn = it.m_button;
        btn->m_temp_id = it.m_name;
        Register( btn );
        //		btn->SetWndPos					(pos);
        //		pos.add
        //(btn->GetWndSize().y); 		pos.y
        //+= 40.0f; 		pos.x
        //= 40.0f;
