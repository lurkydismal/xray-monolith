...
u32 get_rank( const shared_str& section ) {
    ...
    // R_ASSERT3(res != -1, "cannot find rank for", section.c_str()); // AVO:
    // "cannot find rank for" fix
    return res;
}
...
void CRestrictions::InitGroups() {
    ...
    AddRestriction4rank(
        _RANK_COUNT, pSettings->r_string( "rank_base", "amount_restriction" ) );
    ...
    for ( u32 i = 0; i < _RANK_COUNT; ++i ) {
        ...
        AddRestriction4rank(
            i, pSettings->r_string( rank, "amount_restriction" ) );
        m_names[ i ] = CStringTable().translate(
            pSettings->r_string( rank, "rank_name" ) );
    }
...
void CRestrictions::AddGroup( LPCSTR group, LPCSTR lst ) {
    ...
    for ( u32 j = 0; j < count; ++j ) {
        ...
        VERIFY3( _exist.size() == 0, "item has duplicate record in groups",
                 singleItem );
#endif
...
RESTR CRestrictions::GetRestr( const shared_str& item ) {
    ...
    R_ASSERT3( n > 0 && _cnt == 1, "invalid record format <name_sect:rank>",
               item.c_str() );
    ret.name = _name;
...
u32 CRestrictions::GetItemCount( const shared_str& item_sect ) const {
    ...
    VERIFY4( res, "group has no restrictions for rank", group_name.c_str(),
             GetRankName( m_rank ).c_str() );
...
u32 CRestrictions::GetGroupCount( const shared_str& group_name ) const {
    ...
    VERIFY3( res, "group has no restrictions for rank",
             GetRankName( m_rank ).c_str() );

    return res->second;
}

CRestrictions::restr_item* CRestrictions::find_restr_item_internal(
    const u32& rank,
    const shared_str& group_name ) {
    VERIFY( m_bInited );
...
const CRestrictions::restr_item* CRestrictions::find_restr_item(
    const u32& rank,
    const shared_str& group_name ) const {
    VERIFY( m_bInited );
