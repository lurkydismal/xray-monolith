...
CUIFrameLineWnd::CUIFrameLineWnd()
    : bHorizontal( true ), m_bTextureVisible( false ) {
    m_texture_color = color_argb( 255, 255, 255, 255 );
}

void CUIFrameLineWnd::InitFrameLineWnd( LPCSTR base_name,
                                        Fvector2 pos,
                                        Fvector2 size,
                                        bool horizontal ) {
...
void CUIFrameLineWnd::InitFrameLineWnd( Fvector2 pos,
                                        Fvector2 size,
                                        bool horizontal ) {
...
void CUIFrameLineWnd::InitTexture( LPCSTR texture, LPCSTR sh_name ) {
    ...
    CUITextureMaster::InitTexture(
        ...
        m_tex_rect[ flFirst ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_e" ), sh_name, m_shader,
        m_tex_rect[ flSecond ] );
    if ( bHorizontal ) {
        R_ASSERT2( fsimilar( m_tex_rect[ flFirst ].height(),
                             m_tex_rect[ flSecond ].height() ),
                   texture );
        R_ASSERT2( fsimilar( m_tex_rect[ flFirst ].height(),
                             m_tex_rect[ flBack ].height() ),
                   texture );
    } else {
        R_ASSERT2( fsimilar( m_tex_rect[ flFirst ].width(),
                             m_tex_rect[ flSecond ].width() ),
                   texture );
        R_ASSERT2( fsimilar( m_tex_rect[ flFirst ].width(),
                             m_tex_rect[ flBack ].width() ),
                   texture );
    }
}
...
void draw_rect( Fvector2 LTp,
    ...
    // Frame-lines push their vertices directly, so a custom clip must be
    // applied here in software
    if ( UI().HasCustomClip() ) {
        sPoly2D S;
        S.resize( 4 );
        ...
        if ( R && R->size() ) {
            for ( u32 k = 0; k < R->size() - 2; ++k ) {
                UIRender->PushPoint( ( *R )[ 0 ].pt.x, ( *R )[ 0 ].pt.y, 0, clr,
                                     ( *R )[ 0 ].uv.x, ( *R )[ 0 ].uv.y );
                UIRender->PushPoint( ( *R )[ k + 1 ].pt.x, ( *R )[ k + 1 ].pt.y,
                                     0, clr, ( *R )[ k + 1 ].uv.x,
                                     ( *R )[ k + 1 ].uv.y );
                UIRender->PushPoint( ( *R )[ k + 2 ].pt.x, ( *R )[ k + 2 ].pt.y,
                                     0, clr, ( *R )[ k + 2 ].uv.x,
                                     ( *R )[ k + 2 ].uv.y );
            }
        }
...
void CUIFrameLineWnd::DrawElements() {
    ...
    if ( bHorizontal ) {
        back_len = rect.width() - m_tex_rect[ flFirst ].width() -
                   m_tex_rect[ flSecond ].width();
        if ( back_len < 0.0f )
            rect.x2 -= back_len;
        ...
        back_len = rect.height() - m_tex_rect[ flFirst ].height() -
                   m_tex_rect[ flSecond ].height();
        if ( back_len < 0 )
            rect.y2 -= back_len;
    ...
    if ( UI().HasCustomClip() )
        prim_count =
            ( prim_count / 6 ) * UI().ActiveClipFrustum().ClipBudget( 4 );

    UIRender->StartPrimitive( prim_count, IUIRender::ptTriList,
...
bool CUIFrameLineWnd::inc_pos( Frect& rect,
                               int counter,
                               int i,
                               Fvector2& LTp,
                               Fvector2& RBp,
    ...
    {
        if ( ( bHorizontal &&
               rect.lt.x + m_tex_rect[ flSecond ].width() + EPS_L >=
                   rect.rb.x ) ||
             ( !bHorizontal &&
               rect.lt.y + m_tex_rect[ flSecond ].height() + EPS_L >=
                   rect.rb.y ) )
            return false;
        ...
        bool b_draw_reminder =
            ( bHorizontal ) ? ( rect.lt.x + m_tex_rect[ flBack ].width() >
                                rect.rb.x - m_tex_rect[ flSecond ].width() )
                            : ( rect.lt.y + m_tex_rect[ flBack ].height() >
                                rect.rb.y - m_tex_rect[ flSecond ].height() );
        if ( b_draw_reminder ) {
            // draw reminder
            float rem_len =
                ( bHorizontal )
