...
CUIScrollView::CUIScrollView( CUIFixedScrollBar* scroll_bar ) {
    ...
    AddCallback(
        m_VScrollBar, SCROLLBAR_VSCROLL,
        CUIWndCallback::void_function( this, &CUIScrollView::OnScrollV ) );
}
...
void CUIScrollView::InitScrollView() {
    ...
    CUIFixedScrollBar* tmp_scroll =
        smart_cast< CUIFixedScrollBar* >( m_VScrollBar );
    if ( tmp_scroll )
        tmp_scroll->InitScrollBar( Fvector2().set( GetWndSize().x, 0.0f ),
                                   false, *m_scrollbar_profile );
    else {
    ...
    Fvector2 sc_pos = {
        m_VScrollBar->GetWndPos().x - m_VScrollBar->GetWndSize().x,
...
void CUIScrollView::RecalcSize() {
    ...
    if ( m_sort_function ) {
        ...
        std::sort( m_pad->GetChildWndList().begin(),
                   m_pad->GetChildWndList().end(), m_sort_function );
    }

    if ( GetVertFlip() ) {
        for ( WINDOW_LIST::reverse_iterator it =
                  m_pad->GetChildWndList().rbegin();
        ...
        for ( WINDOW_LIST_it it = m_pad->GetChildWndList().begin();
              m_pad->GetChildWndList().end() != it; ++it ) {
            ( *it )->SetWndPos( item_pos );
            item_pos.y += ( *it )->GetWndSize().y;
            item_pos.y += m_vertInterval;
            pad_size.y += ( *it )->GetWndSize().y;
    ...
    if ( m_flags.test( eInverseDir ) )
        m_pad->SetWndPos( Fvector2().set( m_pad->GetWndPos().x,
                                          GetHeight() - m_pad->GetHeight() ) );
...
bool CUIScrollView::OnMouseAction( float x,
                                   float y,
...
float CUIScrollView::GetDesiredChildWidth() {
    if ( NeedShowScrollBar() )
        return GetWidth() - m_VScrollBar->GetWidth() - m_rightIndent -
               m_leftIndent;
    else
