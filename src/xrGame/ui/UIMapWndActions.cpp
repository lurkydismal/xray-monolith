...
public:
    CSomeMapAction( LPCSTR action_name )
...
public:
    CSomeMapEvaluator( LPCSTR evaluator_name )
        : inherited( ( CUIMapWnd* )NULL, evaluator_name ) {}
...
public:
    CEvaluatorTargetMapShown( LPCSTR evaluator_name = 0 )
...
public:
    CEvaluatorMapMinimized( LPCSTR evaluator_name = 0 )
        : inherited( evaluator_name ) {};
    virtual bool evaluate();
};
...
public:
    CEvaluatorMapResized( LPCSTR evaluator_name = 0 )
        : inherited( evaluator_name ) {};
    virtual bool evaluate();
};
...
public:
    CEvaluatorMapConst( bool val = false, LPCSTR evaluator_name = 0 )
        : inherited( evaluator_name ) {
        ret_value = val;
    };
...
void CMapActionPlanner::setup( CUIMapWnd* object ) {
    ...
    add_evaluator( ePropTargetMapShown, xr_new< CEvaluatorTargetMapShown >(
                                            "ePropTargetMapShown" ) );
    add_evaluator( ePropMapMinimized,
                   xr_new< CEvaluatorMapMinimized >( "ePropMapMinimized" ) );
    add_evaluator( ePropMapResized,
                   xr_new< CEvaluatorMapResized >( "ePropMapResized" ) );
    add_evaluator( ePropMapIdle,
                   xr_new< CEvaluatorMapConst >( false, "ePropMapIdle" ) );
...
void CMapActionZoomControl::init_internal() {
    float dist = m_object->GlobalMap()->CalcOpenRect(
        m_object->m_tgtCenter, m_desiredMapRect, m_targetZoom );
    bool bMove = !fis_zero( dist, EPS_L );
    bool bZoom = !fsimilar( m_targetZoom,
                            m_object->GlobalMap()->GetCurrentZoom().x, EPS_L );
    m_endMovingTime = Device.fTimeGlobal;
    // Cap duration at map_zoom_time. The original dist/map_resize_speed scaling
    // produced multi-second eases on enlarged maps, which blocked user pan
    // during the ease.
    if ( bZoom )
        m_endMovingTime += map_zoom_time;
    else if ( bMove )
        m_endMovingTime += _min( _max( dist / map_resize_speed, min_move_time ),
                                 map_zoom_time );
}
...
void CMapActionZoomControl::execute() {
    ...
    if ( m_endMovingTime > Device.fTimeGlobal ) {
        ...
        current_rect.x1 +=
            ( ( m_desiredMapRect.x1 - current_rect.x1 ) / time_to ) * dt;
        current_rect.y1 +=
            ( ( m_desiredMapRect.y1 - current_rect.y1 ) / time_to ) * dt;
        current_rect.x2 +=
            ( ( m_desiredMapRect.x2 - current_rect.x2 ) / time_to ) * dt;
        current_rect.y2 +=
            ( ( m_desiredMapRect.y2 - current_rect.y2 ) / time_to ) * dt;
...
bool CEvaluatorMapMinimized::evaluate() {
    if ( m_storage->property( 1 ) )
        return true;
    bool res = !!fsimilar( m_object->GlobalMap()->GetCurrentZoom().x,
                           m_object->GlobalMap()->GetMinZoom(), EPS_L );
    return res;
}
