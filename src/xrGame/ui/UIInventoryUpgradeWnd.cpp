...
void CUIInventoryUpgradeWnd::InitInventory( CInventoryItem* item,
                                            bool can_upgrade ) {
    m_inv_item = item;
    ...
    } else if ( smart_cast< CCustomOutfit* >( item ) ||
                smart_cast< CHelmet* >( item ) ||
                smart_cast< CBackpack* >( item ) ) {
        is_shader = true;
    ...
    if ( m_item && is_shader ) {
        ...
        Frect texture_rect;
        texture_rect.lt.set( item_upgrade_grid_rect.x1,
                             item_upgrade_grid_rect.y1 );
        texture_rect.rb.set( item_upgrade_grid_rect.x2,
                             item_upgrade_grid_rect.y2 );
        texture_rect.rb.add( texture_rect.lt );
        m_item->GetUIStaticItem().SetTextureRect( texture_rect );
        m_item->TextureOn();
        m_item->SetStretchTexture( true );
        Fvector2 v_r = Fvector2().set( item_upgrade_grid_rect.x2,
                                       item_upgrade_grid_rect.y2 );
        if ( UI().is_widescreen() )
            v_r.x *= 0.8f;
...
bool CUIInventoryUpgradeWnd::install_item( CInventoryItem& inv_item,
                                           bool can_upgrade ) {
    m_scheme_wnd->DetachAll();
    m_back->DetachAll();

    bool can_repair =
        CurrentGameUI()->GetActorMenu().CanRepairItem( &inv_item );
    m_btn_repair->Enable( can_repair );

    if ( !can_repair ) {
        LPCSTR message =
            CurrentGameUI()->GetActorMenu().RepairQuestion( &inv_item, false );
        if ( message != nullptr )
    ...
    if ( !can_upgrade ) {
#ifdef DEBUG
        Msg( "Inventory item <%s> cannot upgrade - Mechanic say.",
    ...
    if ( !scheme_name ) {
#ifdef DEBUG
        Msg( "Inventory item <%s> does not contain upgrade scheme.",
             inv_item.m_section_id.c_str() );
#endif // DEBUG
        m_current_scheme = NULL;
        return false;
    ...
    for ( ; ib != ie; ++ib ) {
        ...
        LPCSTR upgrade_name = get_manager().get_upgrade_by_index(
            inv_item, ui_item->get_scheme_index() );
        ...
        ui_item->set_texture(
            UIUpgrade::LAYER_POINT,
            m_point_textures[ UIUpgrade::STATE_ENABLED ].c_str() ); // default
        ui_item->set_texture(
            UIUpgrade::LAYER_COLOR,
            m_cell_textures[ UIUpgrade::STATE_ENABLED ].c_str() ); // default
    }
...
void CUIInventoryUpgradeWnd::OnMesBoxYes() {
    if ( get_manager().upgrade_install( *m_inv_item, m_cur_upgrade_id,
                                        false ) ) {
        VERIFY( m_pParentWnd );
        CUIActorMenu* parent_wnd = smart_cast< CUIActorMenu* >( m_pParentWnd );
        if ( parent_wnd ) {
            ...
            ai().script_engine().functor(
                "inventory_upgrades.effect_upgrade_item", funct );
            if ( funct ) {
...
void CUIInventoryUpgradeWnd::HighlightHierarchy(
