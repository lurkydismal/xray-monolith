...
void CUIBoosterInfo::InitFromXml( CUIXml& xml ) {
    ...
    for ( u32 i = 0; i < eBoostExplImmunity; ++i ) {
        ...
        LPCSTR name =
            CStringTable().translate( boost_influence_caption[ i ] ).c_str();
        ...
        if ( pSettings->line_exist( section.c_str(),
            ...
            switch ( type ) {
                ...
                case eBoostBurnImmunity:
                    max_val =
                        actor->conditions().GetZoneMaxPower( ALife::infl_fire );
                    break;
                case eBoostShockImmunity:
                    max_val = actor->conditions().GetZoneMaxPower(
                        ALife::infl_electra );
                    break;
                ...
                case eBoostTelepaticProtection:
                    max_val =
                        actor->conditions().GetZoneMaxPower( ALife::infl_psi );
                    break;
                case eBoostChemicalBurnImmunity:
                case eBoostChemicalBurnProtection:
                    max_val =
                        actor->conditions().GetZoneMaxPower( ALife::infl_acid );
                    break;
            }
            val /= max_val;
