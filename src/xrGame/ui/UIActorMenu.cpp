...
void CUIActorMenu::SetActor( CInventoryOwner* io ) {
    ...
    if ( IsGameTypeSingle() ) {
        if ( io )
            m_ActorCharacterInfo->InitCharacter(
                m_pActorInvOwner->object_id() );
        else
...
void CUIActorMenu::Update() {
    ...
    switch ( m_currMenuMode ) {
        ...
        case mmTrade: {
            if ( m_pPartnerInvOwner->inventory().ModifyFrame() !=
                 m_trade_partner_inventory_state )
...
void CUIActorMenu::CheckDistance() {
    ...
    if ( pPartnerGO ) {
        if ( ( pActorGO->Position().distance_to( pPartnerGO->Position() ) >
               3.0f ) &&
...
PIItem CUIActorMenu::CurrentIItem() {
    return ( m_pCurrentCellItem ) ? ( PIItem )m_pCurrentCellItem->m_pData
                                  : NULL;
}
...
void CUIActorMenu::InfoCurItem( CUICellItem* cell_item ) {
    ...
    if ( compare_slot != NO_ACTIVE_SLOT ) {
        compare_item =
            m_pActorInvOwner->inventory().ItemFromSlot( compare_slot );
    }

    if ( GetMenuMode() == mmTrade ) {
        CInventoryOwner* item_owner = smart_cast< CInventoryOwner* >(
            current_item->m_pInventory->GetOwner() );
        ...
        if(ammo)
        {
                for( u32 j = 0; j < cell_item->ChildsCount(); ++j )
                {
                        ...
                        CInventoryOwner* ammo_owner =
        smart_cast<CInventoryOwner*>(jitem->m_pInventory->GetOwner());
                        if(ammo_owner && ammo_owner==m_pActorInvOwner)
                                tmp_price = m_partner_trade->GetItemPrice(jitem,
        true); else tmp_price = m_partner_trade->GetItemPrice(jitem, false);
        ...
        else if ( current_item->GetCondition() <
                  m_pPartnerInvOwner->trade_parameters()
    ...
    fit_in_rect(
        m_ItemInfo,
        Frect().set( 0.0f, 0.0f, UI_BASE_WIDTH - dx_pos, UI_BASE_HEIGHT ),
        10.0f, dx_pos );
}
...
void CUIActorMenu::highlight_item_slot( CUICellItem* cell_item ) {
    ...
    if ( slot_id == HELMET_SLOT || slot_id == BACKPACK_SLOT ) {
        ...
        if ( outfit ) {
            if ( slot_id == HELMET_SLOT && !outfit->bIsHelmetAvaliable ||
                 slot_id == BACKPACK_SLOT && !outfit->bIsBackpackAvaliable )
                return;
        }
    }
    ...
    if ( eatable ) {
        if ( cell_item->OwnerList() &&
             GetListType( cell_item->OwnerList() ) == iQuickSlot )
            return;
    ...
    if ( artefact ) {
        if ( cell_item->OwnerList() &&
             GetListType( cell_item->OwnerList() ) == iActorBelt )
            return;
...
void CUIActorMenu::highlight_armament( PIItem item,
                                       CUIDragDropListEx* ddlist ) {
...
void CUIActorMenu::highlight_ammo_for_weapon( PIItem weapon_item,
                                              CUIDragDropListEx* ddlist ) {
    VERIFY( weapon_item );
    VERIFY( ddlist );
    ...
    CWeaponMagazinedWGrenade* wg =
        smart_cast< CWeaponMagazinedWGrenade* >( weapon_item );
    if ( wg ) {
        if ( wg->IsGrenadeLauncherAttached() && wg->m_ammoTypes2.size() ) {
            ammo_types.insert( ammo_types.end(), wg->m_ammoTypes2.begin(),
                               wg->m_ammoTypes2.end() );
        }
    }
...
void CUIActorMenu::highlight_weapons_for_ammo( PIItem ammo_item,
                                               CUIDragDropListEx* ddlist ) {
    VERIFY( ammo_item );
    VERIFY( ddlist );
...
bool CUIActorMenu::highlight_addons_for_weapon( PIItem weapon_item,
                                                CUICellItem* ci ) {
    PIItem item = ( PIItem )ci->m_pData;
    if ( !item ) {
        return false;
...
void CUIActorMenu::highlight_weapons_for_addon( PIItem addon_item,
                                                CUIDragDropListEx* ddlist ) {
    VERIFY( addon_item );
    VERIFY( ddlist );
    ...
    CGrenadeLauncher* pGrenadeLauncher =
        smart_cast< CGrenadeLauncher* >( addon_item );
...
void CUIActorMenu::CallMessageBoxYesNo( LPCSTR text ) {
    ...
    m_message_box_yes_no->func_on_ok =
        CUIWndCallback::void_function( this, &CUIActorMenu::OnMesBoxYes );
    m_message_box_yes_no->func_on_no =
        CUIWndCallback::void_function( this, &CUIActorMenu::OnMesBoxNo );
...
void CUIActorMenu::UpdateActorMP() {
    if ( !&Level() || !Level().game || !Game().local_player ||
         !m_pActorInvOwner || IsGameTypeSingle() ) {
        m_ActorCharacterInfo->ClearInfo();
    ...
    m_ActorCharacterInfo->InitCharacterMP( Game().local_player->getName(),
                                           "ui_npc_u_nebo_1" );
}

bool CUIActorMenu::CanSetItemToList( PIItem item,
                                     CUIDragDropListEx* l,
                                     u16& ret_slot ) {
...
void CUIActorMenu::UpdateConditionProgressBars() {
    for ( u8 i = 1; i <= m_slot_count; ++i ) {
        ...
        if ( m_pInvSlotProgress[ i ] )
            m_pInvSlotProgress[ i ]->SetProgressPos(
                itm ? iCeil( itm->GetCondition() * 10.f ) / 10.f : 0 );
    }
