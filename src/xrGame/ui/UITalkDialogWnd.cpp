...
void CUITalkDialogWnd::InitTalkDialogWnd() {
    ...
    //	CUIXmlInit::InitStatic		(*m_uiXml, "right_character_icon", 0,
    //&UIOurIcon);

    //	CUIXmlInit::InitStatic		(*m_uiXml, "left_character_icon", 0,
    //&UIOthersIcon);
    ...
    //	CUIXmlInit::InitStatic		(*m_uiXml, "frame_bottom", 0,
    //&UIDialogFrameBottom);

    // основной фрейм диалога
    //	AttachChild					(&UIDialogFrameTop);
    //	CUIXmlInit::InitStatic		(*m_uiXml, "frame_top", 0,
    //&UIDialogFrameTop);
    ...
    CUIXmlInit::InitScrollView( *m_uiXml, "questions_list", 0,
                                UIQuestionsList );
    ...
    // m_btn_pos[2].x				=
    // (m_btn_pos[0].x+m_btn_pos[1].x)/2.0f; m_btn_pos[2].y
    // = m_btn_pos[0].y;
    //  шрифт для индикации имени персонажа в окне разговора
    CUIXmlInit::InitFont( *m_uiXml, "font", 0, m_iNameTextColor,
                          m_pNameTextFont );
    ...
    AddCallbackStr( "question_item", LIST_ITEM_CLICKED,
                    CUIWndCallback::void_function(
                        this, &CUITalkDialogWnd::OnQuestionClicked ) );
    AddCallback( &UIToTradeButton, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUITalkDialogWnd::OnTradeClicked ) );
    //	AddCallback
    //(&UIToExitButton,BUTTON_CLICKED,CUIWndCallback::void_function(this,
...
void CUITalkDialogWnd::AddQuestion( LPCSTR str,
                                    LPCSTR value,
                                    int number,
                                    bool b_finalizer ) {
    CUIQuestionItem* itm =
        xr_new< CUIQuestionItem >( m_uiXml, "question_item" );
    itm->Init( value, str );
...
void CUITalkDialogWnd::AddAnswer( LPCSTR SpeakerName,
                                  LPCSTR str,
                                  bool bActor ) {
    CUIAnswerItem* itm = xr_new< CUIAnswerItem >(
        m_uiXml, bActor ? "actor_answer_item" : "other_answer_item" );
...
void CUITalkDialogWnd::AddIconedAnswer( LPCSTR caption,
                                        LPCSTR text,
                                        LPCSTR texture_name,
                                        LPCSTR templ_name ) {
    CUIAnswerItemIconed* itm =
        xr_new< CUIAnswerItemIconed >( m_uiXml, templ_name );
...
void CUITalkDialogWnd::UpdateButtonsLayout( bool b_disable_break,
                                            bool trade_enabled ) {
...
CUIQuestionItem::CUIQuestionItem( CUIXml* xml_doc, LPCSTR path ) {
    ...
    Register( m_text );
    AddCallback( m_text, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIQuestionItem::OnTextClicked ) );
...
void CUIQuestionItem::Init( LPCSTR val, LPCSTR text ) {
    ...
    float new_h =
        _max( m_min_height, m_text->GetWndPos().y + m_text->GetHeight() );
    SetHeight( new_h );
}
...
void CUIAnswerItemIconed::Init( LPCSTR text,
                                LPCSTR name,
                                LPCSTR texture_name ) {
    xr_string res;
    res += name;
