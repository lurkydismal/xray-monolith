...
void CUISkinSelectorWnd::UpdateSkins() {
    for ( int i = 0; i < 6; i++ ) {
        if ( !!m_shader )
            m_pImage[ i ]->InitTextureEx( m_skins[ i + m_firstSkin ].c_str(),
                                          m_shader.c_str() );
        else
        ...
        xr_vector< int >::iterator it = std::find(
            m_skinsEnabled.begin(), m_skinsEnabled.end(), i + m_firstSkin );
...
void CUISkinSelectorWnd::Init( const char* strSectionName ) {
    ...
    CUIXmlInit::InitStatic( xml_doc, "skin_selector:background", 0,
                            m_pBackground );
    CUIXmlInit::InitStatic( xml_doc, "skin_selector:image_frames", 0,
                            m_pFrames );
    ...
    CUIXmlInit::Init3tButton( xml_doc, "skin_selector:btn_spectator", 0,
                              m_pBtnSpectator );
    CUIXmlInit::Init3tButton( xml_doc, "skin_selector:btn_autoselect", 0,
                              m_pBtnAutoSelect );
    CUIXmlInit::Init3tButton( xml_doc, "skin_selector:btn_back", 0,
                              m_pBtnBack );
...
void CUISkinSelectorWnd::OnBtnOK() {
    ...
    if ( m_iActiveIndex == -1 ) {
        m_iActiveIndex =
            m_skinsEnabled[::Random.randI( m_skinsEnabled.size() ) ];
    }
    game->OnSkinMenu_Ok();
}

bool CUISkinSelectorWnd::OnMouseAction( float x,
                                        float y,
                                        EUIMessages mouse_action ) {
    return CUIWindow::OnMouseAction( x, y, mouse_action );
}

bool CUISkinSelectorWnd::OnKeyboardAction( int dik,
    ...
    if ( dik >= DIK_1 && dik < ( int )right_border + DIK_1 ) {
        ...
        xr_vector< int >::iterator It =
            std::find( m_skinsEnabled.begin(), m_skinsEnabled.end(), NewIndex );
...
void CUISkinSelectorWnd::SetCurSkin( int skin ) {
    R_ASSERT2( skin >= -1 && skin <= ( int )m_skins.size(),
               "invalid skin index" );

    m_iActiveIndex = skin;

    if ( m_iActiveIndex != -1 && ( m_iActiveIndex < m_firstSkin ||
                                   m_iActiveIndex > m_firstSkin + 5 ) ) {
