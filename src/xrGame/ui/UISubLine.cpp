...
CUISubLine::~CUISubLine() {
    ...
    if ( !bOK )
        Msg( "CUISubLine::~CUISubLine()!!!!!!!!!!!!!!!!!!!!!!! cannot find "
             "window in list" );
#endif
}
...
const CUISubLine* CUISubLine::Cut2Pos( int i ) {
    R_ASSERT2( i < ( int )m_text.size(),
               make_string( "CUISubLine::Cut2Pos - invalid parameter [%d][%d]",
                            i, m_text.size() )
                   .c_str() );
...
void CUISubLine::Draw( CGameFont* pFont, float x, float y ) const {
    ...
    pFont->Out( UI().ClientToScreenScaledX( x ),
                UI().ClientToScreenScaledY( y ), "%s", m_text.c_str() );
}
