...
void CUIMapLocationHint::SetInfoTask( CGameTask* task ) {
    ...
    S->AdjustHeightToText();
    // float new_w						=
    // S->GetWndPos().x + S->GetWndSize().x + 20.0f;

    S = m_info[ "t_time" ];
    S->TextItemControl()->SetText(
        InventoryUtilities::GetTimeAndDateAsString( task->m_ReceiveTime )
            .c_str() );

    Fvector2 pos = S->GetWndPos();
    pos.y = m_info[ "t_caption" ]->GetWndPos().y +
            m_info[ "t_caption" ]->GetWndSize().y + 7;
    S->SetWndPos( pos );
    ...
    if ( b_rem ) {
        ...
        InventoryUtilities::GetTimePeriodAsString( buff, sizeof( buff ),
                                                   Level().GetGameTime(),
                                                   task->m_TimeToComplete );

        strconcat( sizeof( buff2 ), buff2,
                   CStringTable().translate( "ui_st_time_remains" ).c_str(),
                   " ", buff );
        S->TextItemControl()->SetText( buff2 );
    }
    pos = S->GetWndPos();
    pos.y = m_info[ "t_time" ]->GetWndPos().y +
            m_info[ "t_time" ]->GetWndSize().y + 7;
    S->SetWndPos( pos );
...

    {
        ...
        pos.y = _max( pos.y, m_info[ "t_icon" ]->GetWndPos().y +
                                 m_info[ "t_icon" ]->GetWndSize().y + 7 );
        m_info[ "t_hint_text" ]->SetWndPos( pos );
    }

    pos.x = m_info[ "t_hint_text" ]->GetWndPos().x +
            m_info[ "t_hint_text" ]->GetWndSize().x + 20.0f;
    pos.y = m_info[ "t_hint_text" ]->GetWndPos().y +
            m_info[ "t_hint_text" ]->GetWndSize().y + 20.0f;
    SetWndSize( pos );
}
