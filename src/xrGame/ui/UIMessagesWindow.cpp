...
// Description:	Window with MP chat and Game Log ( with PDA messages in single
// and Kill Messages in MP) Created:		22.04.2005 Author:
// Serge Vynnychenko Mail:		narrator@gsc-game.kiev.ua
//
...
void CUIMessagesWindow::Init( float x, float y, float width, float height ) {
    ...
    if ( IsGameTypeSingle() ) {
        ...
        XML_NODE* pending_chat_list =
            xml.NavigateToNode( CHAT_LOG_LIST_PENDING );

        if ( pending_chat_list ) {
            m_pending_chat_log_rect.x1 =
                xml.ReadAttribFlt( CHAT_LOG_LIST_PENDING, 0, "x" );
            m_pending_chat_log_rect.y1 =
                xml.ReadAttribFlt( CHAT_LOG_LIST_PENDING, 0, "y" );
            m_pending_chat_log_rect.x2 =
                xml.ReadAttribFlt( CHAT_LOG_LIST_PENDING, 0, "width" );
            m_pending_chat_log_rect.y2 =
                xml.ReadAttribFlt( CHAT_LOG_LIST_PENDING, 0, "height" );
...
void CUIMessagesWindow::AddIconedPdaMessage( GAME_NEWS_DATA* news ) {
    ...
    LPCSTR time_str =
        InventoryUtilities::GetTimeAsString(
            news->receive_time, InventoryUtilities::etpTimeToMinutes )
            .c_str();
    ...
    if ( ai().script_engine().functor(
             "_G.CUIMessagesWindow__AddIconedPdaMessage", funct ) ) {
        CUIWindow* CUIWindowPItem = pItem;
        CUIScrollView* CUIScrollViewPItem = m_pGameLog;
        funct( CUIWindowPItem, &pItem->UITimeText, &pItem->UICaptionText,
               &pItem->UIMsgText, &pItem->UIIcon, CUIScrollViewPItem );
    }
