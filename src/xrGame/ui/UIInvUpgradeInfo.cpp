...
bool UIInvUpgradeInfo::init_upgrade( Upgrade_type* upgr,
    ...
    if ( m_upgrade->is_known() ) {
        ...
        R_ASSERT2( ai().script_engine().functor( cost_func_str, cost_func ),
                   "Failed to get cost" );
        m_cost->SetText( cost_func( m_upgrade->section() ) );
        m_cost->Show( true );

        inventory::upgrade::UpgradeStateResult upg_res =
            m_upgrade->can_install( *inv_item, false );
        inventory::upgrade::UpgradeStateResult upg_res_script =
            m_upgrade->get_preconditions();
        string512 str_res = "";
        ...
        if ( upg_res == inventory::upgrade::result_e_installed ) {
            ...
            xr_sprintf(
                str_res, sizeof( str_res ), "%s",
                CStringTable().translate( "st_upgr_installed" ).c_str() );
        ...
        else if ( upg_res_script ==
                  inventory::upgrade::result_e_precondition_money )
            xr_sprintf( str_res, sizeof( str_res ), "%s:\\n - %s",
                        CStringTable().translate( "st_upgr_disable" ).c_str(),
    ...
    new_size.y = m_properties_wnd->GetWndPos().y +
                 m_properties_wnd->GetWndSize().y + 10.0f;
    SetWndSize( new_size );
