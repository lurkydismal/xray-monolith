...
UIUpgrade::Upgrade_type* UIUpgrade::get_upgrade() {
    Upgrade_type* res =
        ai().alife().inventory_upgrade_manager().get_upgrade( m_upgrade_id );
    VERIFY( res );
    return res;
}
...
// -----------------------------------------------------------------------------------
void UIUpgrade::load_from_xml( CUIXml& ui_xml,
                               int i_column,
                               int i_cell,
                               Frect const& t_cell_item ) {
...
bool UIUpgrade::OnMouseAction( float x, float y, EUIMessages mouse_action ) {
    ...
    if ( mouse_action == WINDOW_LBUTTON_UP ||
         mouse_action == WINDOW_RBUTTON_UP ) {
        m_button_state = BUTTON_FREE;
        return true;
    }
...
void UIUpgrade::update_item( CInventoryItem* inv_item ) {
    ...
    inventory::upgrade::UpgradeStateResult res =
        get_upgrade()->can_install( *inv_item, false );
...
void CUIUpgradePoint::load_from_xml( CUIXml& ui_xml, int i_cell ) {
    ...
    SetWndPos( Fvector2().set( point_x * ( UI().is_widescreen() ? 0.8f : 1.0f ),
                               point_y ) );
...
bool CUIUpgradePoint::OnMouseAction( float x,
                                     float y,
...
void CUIUpgradePoint::OnFocusReceive() {
    ...
    m_parent_upgrade->get_upgrade_window()->set_info_cur_upgrade(
        m_parent_upgrade->get_upgrade() );
    m_parent_upgrade->highlight_relation( true );
