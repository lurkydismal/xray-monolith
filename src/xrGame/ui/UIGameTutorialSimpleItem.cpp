...
bool CUISequenceSimpleItem::IsPlaying() {
    ...
    return ( m_time_start + m_time_length ) >
           ( Device.dwTimeContinual / 1000.0f );
}
...
void CUISequenceSimpleItem::Load( CUIXml* xml, int idx ) {
    ...
    m_flags.set( etiGrabInput, ( 0 == _stricmp( str_grab_input, "on" ) ||
                                 0 == _stricmp( str_grab_input, "1" ) ) );
    ...
    for ( int idx = 0; idx < actions_count; ++idx ) {
        ...
        itm.m_bfinalize =
            !!xml->ReadAttribInt( "action", idx, "finalize", FALSE );
    ...
    for ( int i = 0; i < cnt; ++i ) {
        ...
        _si->m_visible = false;
        _si->m_wnd =
            smart_cast< CUIStatic* >( find_child_window( m_UIWindow, sname ) );
        VERIFY( _si->m_wnd );
        ...
        if ( UI().is_widescreen() ) {
            ...
            XML_NODE* ws_rect =
                xml->NavigateToNode( autostatic_node, "widescreen_rect", 0 );
            if ( ws_rect ) {
...
float CUISequenceSimpleItem::current_factor() {
    if ( m_time_start < 0.0f || fis_zero( m_time_length ) )
        ...
        return ( ( Device.dwTimeContinual / 1000.0f ) - m_time_start ) /
               m_time_length;
}

void CUISequenceSimpleItem::Update() {
    inherited::Update();
    float _start = ( m_time_start < 0.0f )
                       ? ( float( Device.dwTimeContinual ) / 1000.0f )
                       : m_time_start;
    ...
    for ( ; _I != _E; ++_I ) {
        SSubItem& s = *_I;
        bool bPlaying = ( gt > ( _start + s.m_start - EPS ) ) &&
                        ( gt < ( _start + s.m_start + s.m_length + EPS ) );
    ...
    if ( g_pGameLevel &&
         ( !m_pda_section || 0 == xr_strlen( m_pda_section ) ) ) {
        ...
        if ( ui_game_sp ) {
            if ( ui_game_sp->GetPdaMenu().IsShown() ||
                 ...
                 ( MainMenu()->IsActive() &&
                   !m_owner->m_flags.test( CUISequencer::etsOverMainMenu ) ) )
    ...
    for ( u32 idx = 0; idx < m_actions.size(); ++idx ) {
        ...
        if ( b ) {
            ...
            bool functor_exists = ai().script_engine().functor(
                itm.m_functor.c_str(), functor_to_call );
