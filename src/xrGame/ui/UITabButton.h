...
public:
    ...
    // Reproduce this tab at (pos,size): background kind, art, seam overlap,
    // text colours. Caller fills id/caption.
    CUITabButton* Clone( const Fvector2& pos, const Fvector2& size ) const;

    static u32 SlantPoly( Fvector2* out,
                          u32 max,
                          const Fvector2& pos,
                          const Fvector2& size,
                          float slant );
...
private:
    Frect ArtRegion()
        const; // background art rect, in atlas texels (internal to CapWidthUI)
};
