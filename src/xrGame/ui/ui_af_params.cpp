...
LPCSTR af_restore_caption[] = // ALife::EConditionRestoreType
    {
        "ui_inv_health",   "ui_inv_satiety",   "ui_inv_power",
        "ui_inv_bleeding", "ui_inv_radiation",
};
...
void CUIArtefactParams::InitFromXml( CUIXml& xml ) {
    ...
    for ( u32 i = 0; i < 9; ++i ) {
        ...
        LPCSTR name =
            CStringTable().translate( af_immunity_caption[ i ] ).c_str();
    ...
    for ( u32 i = 0; i < ALife::eRestoreTypeMax; ++i ) {
        ...
        LPCSTR name =
            CStringTable().translate( af_restore_caption[ i ] ).c_str();
...
void CUIArtefactParams::SetInfo( CInventoryItem& pInvItem ) {
    ...
    for ( u32 i = 0; i < 9; ++i ) {
        shared_str const& sect =
            pSettings->r_string( af_section, "hit_absorbation_sect" );
        ...
        max_val =
            actor->conditions().GetZoneMaxPower( ( ALife::EInfluenceType )i );
        val /= max_val;
...
void UIArtefactParamItem::Init( CUIXml& xml, LPCSTR section ) {
    ...
    m_sign_inverse =
        ( xml.ReadAttribInt( "value", 0, "sign_inverse", 0 ) == 1 );
