...
void CUIDemoPlayControl::Init() {
    ...
    CUIXmlInit::InitStatic( xml_doc, "demo_play_control:background", 0,
                            m_background );

    CUIXmlInit::Init3tButton( xml_doc, "demo_play_control:btn_restart", 0,
                              m_restart_btn );
    CUIXmlInit::Init3tButton( xml_doc, "demo_play_control:btn_decrease_speed",
                              0, m_decrease_speed_btn );
    CUIXmlInit::Init3tButton( xml_doc, "demo_play_control:btn_play_pause", 0,
                              m_play_pause_btn );
    CUIXmlInit::Init3tButton( xml_doc, "demo_play_control:btn_increase_speed",
                              0, m_increase_speed_btn );
    CUIXmlInit::Init3tButton( xml_doc, "demo_play_control:btn_rewind_until", 0,
                              m_rewind_until_btn );
    CUIXmlInit::Init3tButton( xml_doc, "demo_play_control:btn_repeat_rewind", 0,
                              m_repeat_rewind_btn );
    CUIXmlInit::InitProgressBar( xml_doc, "demo_play_control:progress", 0,
                                 m_progress_bar );
    CUIXmlInit::InitTextWnd( xml_doc, "demo_play_control:static_demo_status", 0,
                             m_static_demo_status );
    CUIWindow tmp_prop_boxes_wnd;
    CUIXmlInit::InitWindow( xml_doc, "demo_play_control:rewind_property_boxes",
                            0, &tmp_prop_boxes_wnd );

    Register( m_restart_btn );
    AddCallback(
        m_restart_btn, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUIDemoPlayControl::OnRestart ) );
    Register( m_decrease_speed_btn );
    AddCallback( m_decrease_speed_btn, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIDemoPlayControl::OnDecresaseSpeed ) );
    Register( m_play_pause_btn );
    AddCallback( m_play_pause_btn, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIDemoPlayControl::OnPlayPause ) );
    Register( m_increase_speed_btn );
    AddCallback( m_increase_speed_btn, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIDemoPlayControl::OnIncreaseSpeed ) );
    Register( m_rewind_until_btn );
    AddCallback( m_rewind_until_btn, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIDemoPlayControl::OnRewindUntil ) );
    Register( m_repeat_rewind_btn );

    AddCallback( m_rewind_type, PROPERTY_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIDemoPlayControl::OnRewindTypeSelected ) );
    AddCallback( m_all_players, PROPERTY_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIDemoPlayControl::OnRewindPlayerSelected ) );

    AddCallback( m_repeat_rewind_btn, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIDemoPlayControl::OnRepeatRewind ) );
    ...
    m_rewind_type_pos.x +=
        m_background->GetWidth() - m_rewind_type->GetWidth() - 14.0f;
...
void CUIDemoPlayControl::InitRewindTypeList() {
    CStringTable st;
    m_rewind_type->InitPropertiesBox( Fvector2().set( 0, 0 ),
                                      Fvector2().set( 100, 200 ) );
    m_rewind_type->AddItem( st.translate( "mpd_rewind_until_start" ).c_str(),
                            NULL, eRewindUntilStart );
    m_rewind_type->AddItem( st.translate( "mpd_rewind_until_kill" ).c_str(),
                            NULL, eRewindUntilKill );
    m_rewind_type->AddItem( st.translate( "mpd_rewind_until_die" ).c_str(),
                            NULL, eRewindUntilDie );
    m_rewind_type->AddItem( st.translate( "mpd_rewind_until_arttake" ).c_str(),
                            NULL, eRewindUntilArtTake );
    m_rewind_type->AddItem( st.translate( "mpd_rewind_until_artdrop" ).c_str(),
                            NULL, eRewindUntilArtDrop );
    m_rewind_type->AddItem(
        st.translate( "mpd_rewind_until_artdeliver" ).c_str(), NULL,
...
void CUIDemoPlayControl::InitAllPlayers() {
    ...
    m_players =
        xr_new< players_collection_t >( m_players_store, players_count );

    CStringTable st;
    m_all_players->InitPropertiesBox( Fvector2().set( 0, 0 ),
                                      Fvector2().set( 100, 200 ) );
    m_all_players->AddItem( st.translate( "mpd_any_player" ).c_str(), NULL,
                            0 ); // warning ! zero tag means Any player !
    ...
    for ( u32 i = 0; i != players_count; ++i ) {
        ...
        m_all_players->AddItem( tmp_player_name, NULL,
                                i + 1 ); // warning ! player_index = tag - 1 !!!
    }
    m_all_players->AutoUpdateSize();
    m_all_players->Hide();
}

bool CUIDemoPlayControl::OnKeyboardAction( int dik,
                                           EUIMessages keyboard_action ) {
    if ( ( dik == DIK_LCONTROL ) &&
         ( keyboard_action == WINDOW_KEY_RELEASED ) ) {
...
void CUIDemoPlayControl::OnRewindTypeSelected( CUIWindow* w, void* d ) {
    ...
    float right_limit =
        pos.x + m_rewind_type->GetWidth() + m_all_players->GetWidth();
...
void CUIDemoPlayControl::OnRewindPlayerSelected( CUIWindow* w, void* d ) {
    ...
    m_rewind_type->Hide(); // old mechanism auto-closed the type box; do it
                           // explicitly now
...
void CUIDemoPlayControl::OnRepeatRewind( CUIWindow* w, void* d ) {
    ...
    switch ( m_last_rewind_type ) {
        case eRewindUntilStart: {
            rewind_result = m_demo_play_control->rewind_until(
                demoplay_control::on_round_start, shared_str(), tmpcb );
        }; break;
        case eRewindUntilKill: {
            rewind_result = m_demo_play_control->rewind_until(
                demoplay_control::on_kill, m_last_rewind_target, tmpcb );
        }; break;
        case eRewindUntilDie: {
            rewind_result = m_demo_play_control->rewind_until(
                demoplay_control::on_die, m_last_rewind_target, tmpcb );
        }; break;
        case eRewindUntilArtTake: {
            rewind_result = m_demo_play_control->rewind_until(
                demoplay_control::on_artefactcapturing, m_last_rewind_target,
                tmpcb );
        }; break;
        case eRewindUntilArtDrop: {
            rewind_result = m_demo_play_control->rewind_until(
                demoplay_control::on_artefactloosing, m_last_rewind_target,
                tmpcb );
        }; break;
        case eRewindUntilArtDeliver: {
            rewind_result = m_demo_play_control->rewind_until(
                demoplay_control::on_artefactdelivering, m_last_rewind_target,
                tmpcb );
        }; break;
        default:
            NODEFAULT;
...
void CUIDemoPlayControl::Update() {
    ...
    STRCONCAT( demo_play_string,
               Device.Paused() ? st.translate( "mpdemoplay_paused" )
