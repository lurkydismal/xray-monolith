...
void CUIServerInfo::Init() {
    ...
    CUIXmlInit::InitStatic( xml_doc, "server_info:background", 0,
                            m_background );
    CUIXmlInit::InitScrollView( xml_doc, "server_info:text_desc", 0,
                                m_text_desc );
    ...
    CUIXmlInit::Init3tButton( xml_doc, "server_info:btn_spectator", 0,
                              m_btn_spectator );
...
void CUIServerInfo::InitCallbacks() {
    ...
    AddCallback(
        m_btn_next, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUIServerInfo::OnNextBtnClick ) );
    AddCallback( m_btn_spectator, BUTTON_CLICKED,
                 CUIWndCallback::void_function(
                     this, &CUIServerInfo::OnSpectatorBtnClick ) );
    // AddCallback	(this,				WINDOW_KEY_PRESSED,
    // CUIWndCallback::void_function(this, &CUIServerInfo::OnNextBtnClick));
}
...
void CUIServerInfo::SetServerRules( u8 const* data_ptr, u32 const data_size ) {
    ...
    strncpy_s( tmp_string, reinterpret_cast< char const* >( data_ptr ),
               new_size );
