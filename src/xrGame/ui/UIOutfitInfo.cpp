...
LPCSTR immunity_names[] = {
    "burn_immunity",       "shock_immunity",     "chemical_burn_immunity",
    "radiation_immunity",  "telepatic_immunity", "wound_immunity",
    "fire_wound_immunity", "strike_immunity",    "explosion_immunity",
};
...
void CUIOutfitImmunity::InitFromXml( CUIXml& xml_doc,
                                     LPCSTR base_str,
                                     u32 hit_type ) {
...
void CUIOutfitImmunity::SetProgressValue( float cur, float comp ) {
    ...
    if ( cur != comp ) {
        ...
        xr_sprintf( buf, sizeof( buf ), adj_value >= 0 ? "+%.1f%%" : "%.1f%%",
                    adj_value );
        m_value.SetText( buf );
...
void CUIOutfitInfo::InitFromXml( CUIXml& xml_doc ) {
    ...
    */
    ...
    m_items[ ALife::eHitTypeFireWound ]->InitFromXml(
        xml_doc, base_str, ALife::eHitTypeFireWound );
    ...
    m_items[ ALife::eHitTypeWound ]->InitFromXml( xml_doc, base_str,
                                                  ALife::eHitTypeWound );
    ...
    m_items[ ALife::eHitTypeStrike ]->InitFromXml( xml_doc, base_str,
    ...
    m_items[ ALife::eHitTypeExplosion ]->InitFromXml(
        xml_doc, base_str, ALife::eHitTypeExplosion );
    ...
    m_items[ ALife::eHitTypeBurn ]->InitFromXml( xml_doc, base_str,
                                                 ALife::eHitTypeBurn );
    ...
    m_items[ ALife::eHitTypeShock ]->InitFromXml( xml_doc, base_str,
                                                  ALife::eHitTypeShock );
    ...
    m_items[ ALife::eHitTypeChemicalBurn ]->InitFromXml(
        xml_doc, base_str, ALife::eHitTypeChemicalBurn );
    ...
    m_items[ ALife::eHitTypeRadiation ]->InitFromXml(
        xml_doc, base_str, ALife::eHitTypeRadiation );
    ...
    m_items[ ALife::eHitTypeTelepatic ]->InitFromXml(
        xml_doc, base_str, ALife::eHitTypeTelepatic );
...
void CUIOutfitInfo::UpdateInfo( CCustomOutfit* cur_outfit,
                                CCustomOutfit* slot_outfit ) {
    ...
    if ( m_items[ ALife::eHitTypeFireWound ] ) {
        ...
        float cur =
            cur_outfit->GetBoneArmor( spine_bone ) * cur_outfit->GetCondition();
...
void CUIOutfitInfo::UpdateInfo( CHelmet* cur_helmet, CHelmet* slot_helmet ) {
    ...
    if ( m_items[ ALife::eHitTypeFireWound ] ) {
        ...
        float cur =
            cur_helmet->GetBoneArmor( spine_bone ) * cur_helmet->GetCondition();
        float slot = ( slot_helmet ) ? slot_helmet->GetBoneArmor( spine_bone ) *
                                           slot_helmet->GetCondition()
                                     : cur;
