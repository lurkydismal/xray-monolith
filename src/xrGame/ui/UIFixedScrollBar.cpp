...
void CUIFixedScrollBar::InitScrollBar( Fvector2 pos,
                                       bool horizontal,
                                       LPCSTR profile ) {
    string256 _path;
    CUIXml xml_doc;
    ...
    m_ScrollBoxOffset.x =
        xml_doc.ReadAttribInt( profile, 0, "scroll_box_offset_x", 0 );
    m_ScrollBoxOffset.y =
        xml_doc.ReadAttribInt( profile, 0, "scroll_box_offset_y", 0 );
...
void CUIFixedScrollBar::UpdateScrollBar() {
    if ( IsShown() ) {
        ...
        if ( m_iMaxPos == m_iMinPos )
            m_iMaxPos++;
        float box_sz = float( m_ScrollWorkArea ) *
                       float( m_iPageSize ? m_iPageSize : 1 ) /
                       float( m_iMaxPos - m_iMinPos );
        if ( IsRelevant() ) {
            if ( m_bIsHorizontal ) {
                // set width
                clamp( box_sz,
                       _min( GetHeight(), GetWidth() - m_IncButton->GetWidth() -
                                              m_DecButton->GetWidth() ),
                       GetWidth() - m_IncButton->GetWidth() -
                           m_DecButton->GetWidth() - 2 * m_ScrollBoxOffset.x );
                m_ScrollBox->SetWidth( box_sz );
                // set pos
                int pos = PosViewFromScroll( iFloor( box_sz ),
                                             iFloor( GetHeight() ) );
                m_ScrollBox->SetWndPos( Fvector2().set(
                    float( pos ), m_ScrollBox->GetWndRect().top ) );
                m_IncButton->SetWndPos( Fvector2().set(
                    GetWidth() - m_IncButton->GetWidth(), 0.0f ) );
            } else {
                // set height
                clamp(
                    box_sz,
                    _min( GetWidth(), GetHeight() - m_IncButton->GetHeight() -
                                          m_DecButton->GetHeight() ),
                    GetHeight() - m_IncButton->GetHeight() -
                        m_DecButton->GetHeight() - 2 * m_ScrollBoxOffset.y );
                m_ScrollBox->SetHeight( box_sz );
                // set pos
                int pos =
                    PosViewFromScroll( iFloor( box_sz ), iFloor( GetWidth() ) );
                m_ScrollBox->SetWndPos( Fvector2().set(
                    m_ScrollBox->GetWndRect().left, float( pos ) ) );
                m_IncButton->SetWndPos( Fvector2().set(
                    0.0f, GetHeight() - m_IncButton->GetHeight() ) );
            }
        }
    }
    if ( IsRelevant() ) {
...
void CUIFixedScrollBar::ClampByViewRect() {
    if ( m_bIsHorizontal ) {
        if ( m_ScrollBox->GetWndRect().left <=
             m_DecButton->GetWidth() + m_ScrollBoxOffset.x )
            m_ScrollBox->SetWndPos(
                Fvector2().set( m_ScrollBoxOffset.x + m_DecButton->GetWidth(),
                                m_ScrollBox->GetWndRect().top ) );
        else if ( m_ScrollBox->GetWndRect().right >=
                  m_IncButton->GetWndPos().x - m_ScrollBoxOffset.x )
            m_ScrollBox->SetWndPos( Fvector2().set(
                m_IncButton->GetWndRect().left - m_ScrollBox->GetWidth() -
                    m_ScrollBoxOffset.x,
                m_ScrollBox->GetWndRect().top ) );
    } else {
        if ( m_ScrollBox->GetWndRect().top <=
             m_DecButton->GetHeight() + m_ScrollBoxOffset.y )
            m_ScrollBox->SetWndPos( Fvector2().set(
                m_ScrollBox->GetWndRect().left,
                m_ScrollBoxOffset.y + m_DecButton->GetHeight() ) );
        else if ( m_ScrollBox->GetWndRect().bottom >=
                  m_IncButton->GetWndPos().y - m_ScrollBoxOffset.y )
            m_ScrollBox->SetWndPos( Fvector2().set(
                m_ScrollBox->GetWndRect().left, m_IncButton->GetWndPos().y -
...
bool CUIFixedScrollBar::OnKeyboardHold( int dik ) {
    if ( dik == MOUSE_1 &&
         ( last_hold_tm + m_hold_delay ) < Device.dwTimeContinual ) {
        if ( OnMouseDownEx() ) {
...
bool CUIFixedScrollBar::OnMouseAction( float x,
                                       float y,
    ...
    switch ( mouse_action ) {
        ...
        case WINDOW_MOUSE_MOVE: {
            ...
            // bool over_x = ( x >= -512.0f && x < (m_ScrollBox->GetWidth()  +
            // 512.0f) ); bool over_y = ( y >= -512.0f && y <
            // (m_ScrollBox->GetHeight() + 512.0f) ); if ( over_x && over_y )
            //	cursor_over = true;
            ...
            if ( im_capturer && cursor_over ) {
                ...
                m_ScrollBox->GetMessageTarget()->SendMessage( m_ScrollBox,
                                                              SCROLLBOX_MOVE );
            }
            if ( !cursor_over ) {
                SetCapture( m_ScrollBox, false );
...
void CUIFixedScrollBar::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    if ( pWnd == m_DecButton ) {
        ...
        if ( msg == SCROLLBOX_MOVE ) {
            ...
            ClampByViewRect();
            if ( m_bIsHorizontal ) {
                SetPosScrollFromView( m_ScrollBox->GetWndPos().x,
                                      m_ScrollBox->GetWidth(), GetHeight() );
                if ( GetMessageTarget() )
                    GetMessageTarget()->SendMessage( this, SCROLLBAR_HSCROLL );
            } else {
                SetPosScrollFromView( m_ScrollBox->GetWndPos().y,
                                      m_ScrollBox->GetHeight(), GetWidth() );
                if ( GetMessageTarget() )
...
void CUIFixedScrollBar::SetPosScrollFromView( float view_pos,
                                              float view_size,
                                              float view_offs ) {
    ...
    m_iScrollPos =
        work_size
            ? iFloor( ( ( pos / work_size ) * ( scroll_size ) + m_iMinPos ) )
            : 0;
