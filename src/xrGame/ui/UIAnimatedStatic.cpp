...
void CUIAnimatedStatic::Update() {
    ...
    if ( m_bParamsChanged && 0 != m_uFrameCount ) {
        ...
        oneFrameDuration = iCeil( m_uAnimationDuration /
                                  static_cast< float >( m_uFrameCount ) );
...
void CUIAnimatedStatic::SetFrame( const u32 frameNum ) {
    ...
    texture_rect.lt.set( m_pos.x + currCol * m_uFrameWidth,
                         m_pos.y + currRow * m_uFrameHeight );
...
void CUISleepStatic::Update() {
    ...
    Frect r =
        Frect().set( ( float )start_pixel, 0.0f, ( float )end_pixel, 128.0f );
    m_UIStaticItem.SetTextureRect( r );
    m_UIStaticItem.SetSize( Fvector2().set(
        iFloor( ( end_pixel - start_pixel ) * UI().get_current_kx() ), 128 ) );
    m_UIStaticItem.SetPos( pos.x, pos.y );
    if ( end_pixel2 > 0 ) {
        r.set( ( float )start_pixel2, 0.0f, ( float )end_pixel2, 128.0f );
        m_UIStaticItem2.SetTextureRect( r );
        m_UIStaticItem2.SetSize( Fvector2().set(
            iFloor( end_pixel2 * UI().get_current_kx() ), 128 ) );
        m_UIStaticItem2.SetPos(
            m_UIStaticItem.GetPosX() + m_UIStaticItem.GetSize().x,
            m_UIStaticItem.GetPosY() );
    } else
