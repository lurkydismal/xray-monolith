...
void CUIStatsPlayerList::Init( CUIXml& xml_doc, LPCSTR path ) {
    ...
    m_bStatus_mode =
        xml_doc.ReadAttribInt( path, 0, "status_mode", 0 ) ? true : false;
    m_bSpectator = xml_doc.ReadAttribInt( path, 0, "spectator", 0 )
                       ? true
                       : m_bStatus_mode;
    ...
    for ( int i = 0; i < tabsCount; ++i ) {
        ...
        if ( 0 == xr_strcmp( name, "artefacts" ) &&
             GameID() != eGameIDArtefactHunt )
            continue;
    ...
    CUIXmlInit::InitFont(
        xml_doc, strconcat( sizeof( _path ), _path, path, ":text_format" ), 0,
        m_i.c, m_i.f );
    m_i.h = xml_doc.ReadAttribFlt(
        strconcat( sizeof( _path ), _path, path, ":text_format" ), 0, "height",
        25 );
...
void CUIStatsPlayerList::InitHeader( CUIXml& xml_doc, LPCSTR path ) {
    string256 _path;
    CUIXmlInit::InitStatic(
        xml_doc, strconcat( sizeof( _path ), _path, path, ":list_header" ), 0,
        m_header );
    ...
    CUIXmlInit::InitFont(
        xml_doc,
        strconcat( sizeof( _path ), _path, path, ":list_header:text_format" ),
        0, m_h.c, m_h.f );
    float indent = 5;
    if ( !m_bSpectator || m_bStatus_mode ) {
        for ( u32 i = 0; i < m_field_info.size(); ++i ) {
            ...
            st->SetWndSize( Fvector2().set( m_field_info[ i ].width,
                                            m_header->GetHeight() ) );
...
void CUIStatsPlayerList::InitTeamHeader( CUIXml& xml_doc, LPCSTR path ) {
    ...
    CUIXmlInit::InitWindow(
        xml_doc, strconcat( sizeof( _path ), _path, path, ":team_header" ), 0,
        m_header_team );
    ...
    CUIXmlInit::InitStatic(
        xml_doc, strconcat( sizeof( _path ), _path, path, ":team_header:logo" ),
        0, logo );
    ...
    CUIXmlInit::InitFont(
        xml_doc,
        strconcat( sizeof( _path ), _path, path, ":team_header:text_format" ),
        0, t.c, t.f );
    t.h = m_header_team->GetHeight();

    m_header_text = xr_new< CUITextWnd >();
    m_header_text->SetAutoDelete( true );
    CUIXmlInit::InitTextWnd(
        xml_doc,
        strconcat( sizeof( _path ), _path, path, ":team_header:header" ), 0,
        m_header_text );
...
void CUIStatsPlayerList::Update() {
    ...
    if ( GameID() == eGameIDArtefactHunt && !m_bSpectator ) {
        game_cl_ArtefactHunt* game =
            static_cast< game_cl_ArtefactHunt* >( &Game() );
        pl_artefacts = game->teams[ m_CurTeam - 1 ].score;
        xr_sprintf( teaminfo, "%s: %u, %s: %u, %s: %d",
                    *st.translate( "mp_artefacts_upcase" ), pl_artefacts,
                    *st.translate( "mp_players" ), pl_count,
                    *st.translate( "mp_frags_upcase" ), pl_frags );
        m_header_text->SetText( teaminfo );
    } else if ( GameID() == eGameIDTeamDeathmatch && !m_bSpectator ) {
        game_cl_TeamDeathmatch* game =
            static_cast< game_cl_TeamDeathmatch* >( &Game() );
        pl_frags = game->teams[ m_CurTeam - 1 ].score;
        xr_sprintf( teaminfo, "%s: %d, %s: %u",
                    *st.translate( "mp_frags_upcase" ), pl_frags,
                    *st.translate( "mp_players" ), pl_count );
        m_header_text->SetText( teaminfo );
    }
    ...
    if ( n < 0 ) {
        ...
        for ( int i = 0; i < n; i++ ) {
            CUIStatsPlayerInfo* pi =
                xr_new< CUIStatsPlayerInfo >( &m_field_info, m_i.f, m_i.c );
            pi->InitPlayerInfo(
                Fvector2().set( 0, 0 ),
                Fvector2().set( this->GetDesiredChildWidth(), m_i.h ) );
