...
bool CUIMpTradeWnd::TryToSellItem( SBuyItemInfo* sell_itm,
    ...
    if ( !is_helper_item ) {
        ...
        _item_cost =
            m_item_mngr->GetItemCost( sell_itm->m_name_sect, GetRank() );
    }
    ...
    u32 cnt_in_shop =
        GetItemCount( sell_itm->m_name_sect, SBuyItemInfo::e_shop );
    ...
    if ( cnt_in_shop != 0 ) {
        ...
        if ( m_store_hierarchy->CurrentLevel().HasItem( iinfo->m_name_sect ) ) {
            ...
            int accel_idx = m_store_hierarchy->CurrentLevel().GetItemIdx(
                iinfo->m_name_sect );
            VERIFY( accel_idx != -1 );
            iinfo->m_cell_item->SetAccelerator(
                ( accel_idx > 10 ) ? 0 : DIK_1 + accel_idx );
            iinfo->m_cell_item->SetCustomDraw(
                xr_new< CUICellItemTradeMenuDraw >( this, iinfo ) );
        }
    }
    if ( _item_cost != 0 ) {
    ...
    if ( list_idx == e_pistol || list_idx == e_rifle || list_idx == e_outfit ) {
        ...
        if ( _list->ItemsCount() ) {
            ...
            if ( !b_res ) {
                ...
                bool b_res2 = TryToBuyItem(
                    to_sell, bf_check_money | bf_ignore_team | bf_own_itm,
                    NULL );
                R_ASSERT( b_res2 );
                to_sell->SetState( SBuyItemInfo::e_undefined ); // hack
                to_sell->SetState( _stored_state );             // hack
            } else if ( to_sell->GetState() == SBuyItemInfo::e_sold ||
                        to_sell->GetState() == SBuyItemInfo::e_undefined )
                // maybe in shop now
                DestroyItem( to_sell );
...
bool CUIMpTradeWnd::TryToBuyItem( SBuyItemInfo* buy_itm,
    ...
    bool b_can_buy =
        is_helper || CheckBuyPossibility( buy_item_name, buy_flags, false );
    if ( !b_can_buy )
        return false;
    ...
    if ( 0 == ( bf_ignore_team & buy_flags ) ) {
        if ( GameID() == eGameIDCaptureTheArtefact ) {
            game_cl_CaptureTheArtefact* cta_game =
                smart_cast< game_cl_CaptureTheArtefact* >( &Game() );
            if ( cta_game && !cta_game->LocalPlayerCanBuyItem( buy_item_name ) )
                return false;
        } else {
            game_cl_Deathmatch* dm_game =
                smart_cast< game_cl_Deathmatch* >( &Game() );
    ...
    {
        cell_itm = iinfo->m_cell_item->OwnerList()->RemoveItem(
...
bool CUIMpTradeWnd::CheckBuyPossibility( const shared_str& sect_name,
    ...
    if ( ( buy_flags & bf_check_money ) ) {
        if ( GetMoneyAmount() < _item_cost ) {
            if ( !b_silent )
                xr_sprintf(
                    info_buffer, "%s. %s. %s[%d] %s[%d]",
                    CStringTable().translate( "ui_inv_cant_buy_item" ).c_str(),
                    CStringTable()
                        .translate( "ui_inv_not_enought_money" )
                        .c_str(),
    ...
    if ( b_can_buy && ( buy_flags & bf_check_rank_restr ) &&
         !g_mp_restrictions.IsAvailable( sect_name ) ) {
        if ( !b_silent )
            xr_sprintf(
                info_buffer, "%s. %s. %s[%s] %s[%s] ",
                CStringTable().translate( "ui_inv_cant_buy_item" ).c_str(),
                CStringTable().translate( "ui_inv_rank_restr" ).c_str(),
                CStringTable().translate( "ui_inv_has" ).c_str(),
                g_mp_restrictions.GetRankName( GetRank() ).c_str(),
                CStringTable().translate( "ui_inv_need" ).c_str(),
                g_mp_restrictions.GetRankName( get_rank( sect_name ) )
                    .c_str() );
        b_can_buy = false;
    }
        ...
        if ( cnt_have >= cnt_restr ) {
            if ( !b_silent )
                xr_sprintf(
                    ...
                    CStringTable()
                        .translate( "ui_inv_you_already_have" )
                        .c_str(),
                    cnt_have );
            b_can_buy = false;
        }
...
void CUIMpTradeWnd::RenewShopItem( const shared_str& sect_name,
                                   bool b_just_bought ) {
    if ( m_store_hierarchy->CurrentLevel().HasItem( sect_name ) ) {
        CUIDragDropListEx* pList = m_list[ e_shop ];
        SBuyItemInfo* pitem =
            CreateItem( sect_name, SBuyItemInfo::e_shop, true );
        ...
        if ( pitem->m_cell_item->OwnerList() != pList ) {
            int accel_idx =
                m_store_hierarchy->CurrentLevel().GetItemIdx( sect_name );
            pitem->m_cell_item->SetAccelerator(
                ( accel_idx > 9 ) ? 0 : DIK_1 + accel_idx );

            pitem->m_cell_item->SetCustomDraw(
                xr_new< CUICellItemTradeMenuDraw >( this, pitem ) );
...
void CUIMpTradeWnd::ItemToBelt( const shared_str& sectionName ) {
    R_ASSERT2( m_item_mngr->GetItemIdx( sectionName ) != u32( -1 ),
               sectionName.c_str() );
