    ...
    if ( r1.x == r2.x ) {
        ...
        if ( r1.y == r2.y ) {
            if ( item1->object().cNameSect() == item2->object().cNameSect() )
                ...
                return ( item1->object().cNameSect() >
                         item2->object().cNameSect() );
        }
...
bool InventoryUtilities::FreeRoom_inBelt( TIItemContainer& item_list,
                                          PIItem _item,
                                          int width,
                                          int height ) {
    ...
    for ( xr_vector< PIItem >::iterator it = item_list.begin();
          ( item_list.end() != it ) && found_place; ++it ) {
        PIItem pItem = *it;
        ...
        if ( found_place ) {
            for ( k = 0; k < iWH.y; ++k ) {
                for ( m = 0; m < iWH.x; ++m ) {
                    ruck_room[ ( place_row + k ) * width + place_col + m ] =
                        true;
                }
            }
        }
    ...
    // remove
    item_list.erase( std::remove( item_list.begin(), item_list.end(), _item ),
                     item_list.end() );
...
const ui_shader& InventoryUtilities::GetEquipmentIconsShader() {
    if ( !g_EquipmentIconsShader ) {
        ...
        ( *g_EquipmentIconsShader )
            ->create( "hud\\default", "ui\\ui_icon_equipment" );
    }
...
ui_shader& InventoryUtilities::GetCustomIconTextureShader( LPCSTR name ) {
    xr_map< LPCSTR, ui_shader* >::iterator it =
        g_CustomIconShaders.find( name );
...
const ui_shader& InventoryUtilities::GetOutfitUpgradeIconsShader() {
    if ( !g_OutfitUpgradeIconsShader ) {
        ...
        ( *g_OutfitUpgradeIconsShader )
            ->create( "hud\\default", "ui\\ui_actor_armor" );
    }
...
const ui_shader& InventoryUtilities::GetWeaponUpgradeIconsShader() {
    if ( !g_WeaponUpgradeIconsShader ) {
        ...
        ( *g_WeaponUpgradeIconsShader )
            ->create( "hud\\default", "ui\\ui_actor_weapons" );
    }
...
const shared_str InventoryUtilities::GetGameDateAsString(
    EDatePrecision datePrec,
    char dateSeparator ) {
...
const shared_str InventoryUtilities::GetGameTimeAsString(
    ETimePrecision timePrec,
    char timeSeparator ) {
    return GetTimeAsString( Level().GetGameTime(), timePrec, timeSeparator );
}

const shared_str InventoryUtilities::GetTimeAndDateAsString(
    ALife::_TIME_ID time ) {
    string256 buf;
...
const shared_str InventoryUtilities::GetTimeAsString( ALife::_TIME_ID time,
                                                      ETimePrecision timePrec,
                                                      char timeSeparator,
                                                      bool full_mode ) {
    string32 bufTime;
    ...
    switch ( timePrec ) {
        ...
        case etpTimeToSeconds:
            if ( full_mode || hours > 0 ) {
                xr_sprintf( bufTime, "%02i%c%02i%c%02i", hours, timeSeparator,
                            mins, timeSeparator, secs );
                break;
            }
            if ( mins > 0 ) {
        ...
        case etpTimeToMilisecs:
            xr_sprintf( bufTime, "%02i%c%02i%c%02i%c%02i", hours, timeSeparator,
                        mins, timeSeparator, secs, timeSeparator, milisecs );
            break;
        case etpTimeToSecondsAndDay: {
            int total_day = ( int )( time / ( 1000 * 60 * 60 * 24 ) );
            xr_sprintf( bufTime, sizeof( bufTime ), "%dd %02i%c%02i%c%02i",
                        total_day, hours, timeSeparator, mins, timeSeparator,
                        secs );
            break;
        }
        default:
...
const shared_str InventoryUtilities::GetDateAsString( ALife::_TIME_ID date,
                                                      EDatePrecision datePrec,
                                                      char dateSeparator ) {
    string64 bufDate;
    ...
    LPCSTR month_str =
        CStringTable().translate( st_months[ month - 1 ] ).c_str();
    ...
    switch ( datePrec ) {
        ...
        case edpDateToMonth:
            ...
            break;
        case edpDateToDay:
            xr_sprintf( bufDate, "%s %d%c %04i", month_str, day, dateSeparator,
                        year );
            break;
        default:
            R_ASSERT( !"Unknown type of date precision" );
...
LPCSTR InventoryUtilities::GetTimePeriodAsString( LPSTR _buff,
                                                  u32 buff_sz,
                                                  ALife::_TIME_ID _from,
                                                  ALife::_TIME_ID _to ) {
    ...
    if ( month1 != month2 || yrdiff > 0 )
        cnt = xr_sprintf( _buff + cnt, buff_sz - cnt, "%d %s",
                          month2 + ( yrdiff - month1 ),
...
void InventoryUtilities::UpdateWeightStr( CUITextWnd& wnd,
                                          CUITextWnd& wnd_max,
...
void InitCharacterInfoStrings() {
    ...
    if ( !charInfoReputationStrings ) {
        ...
        // Reputation
        LoadStrings( charInfoReputationStrings, relationsLtxSection,
                     reputationgField );
    }
    ...
    if ( !charInfoGoodwillStrings ) {
        ...
        // Goodwills
        LoadStrings( charInfoGoodwillStrings, relationsLtxSection,
                     goodwillField );
    }
}
...
LPCSTR InventoryUtilities::GetRankAsText( CHARACTER_RANK_VALUE rankID ) {
    ...
    CharInfoStrings::const_iterator cit =
        charInfoRankStrings->upper_bound( rankID );
...
LPCSTR InventoryUtilities::GetReputationAsText(
    CHARACTER_REPUTATION_VALUE rankID ) {
    InitCharacterInfoStrings();

    CharInfoStrings::const_iterator cit =
        charInfoReputationStrings->upper_bound( rankID );
...
LPCSTR InventoryUtilities::GetGoodwillAsText( CHARACTER_GOODWILL goodwill ) {
    ...
    CharInfoStrings::const_iterator cit =
        charInfoGoodwillStrings->upper_bound( goodwill );
...
void InventoryUtilities::SendInfoToLuaScripts( shared_str info ) {
    ...
    if ( info == shared_str( "ui_talk_show" ) ) {
        ...
        R_ASSERT(
            ai().script_engine().functor( "pda.actor_menu_mode", funct ) );
        funct( mode );
    }
