...
void CUIDragDropListEx::InitDragDropList( Fvector2 pos, Fvector2 size ) {
    ...
    m_vScrollBar->InitScrollBar( Fvector2().set( size.x, 0.0f ), size.y,
                                 false );
    m_vScrollBar->SetWndPos(
        Fvector2().set( m_vScrollBar->GetWndPos().x - m_vScrollBar->GetWidth(),
                        m_vScrollBar->GetWndPos().y ) );
}

void CUIDragDropListEx::OnScrollV( CUIWindow* w, void* pData ) {
    m_container->SetWndPos( Fvector2().set(
        m_container->GetWndPos().x, float( -m_vScrollBar->GetScrollPos() ) ) );
}
...
void CUIDragDropListEx::DestroyDragItem() {
    if ( m_selected_item && m_drag_item &&
         m_drag_item->ParentItem() == m_selected_item ) {
...
Fvector2 CUIDragDropListEx::GetDragItemPosition() {
    return GetUICursor()
        .GetCursorPosition(); // return m_drag_item->GetPosition();
}
...
void CUIDragDropListEx::OnItemRButtonClick( CUIWindow* w, void* pData ) {
    //*	OnItemSelected						(w, pData); //
    //instead call function "SetCurrentItem(itm)";
    CUICellItem* itm = smart_cast< CUICellItem* >( w );
...
void CUIDragDropListEx::Draw() {
    ...
    if ( 0 && bDebug ) {
        ...
        Ivector2 pt =
            m_container->PickCell( GetUICursor().GetCursorPosition() );
...
void CUIDragDropListEx::ReinitScroll() {
    ...
    //		m_vScrollBar->SetPageSize
    //(iFloor(GetWndSize().y/float(CellSize().y)));
    m_vScrollBar->SetPageSize( 1 /*CellSize().y*/ );

    m_container->SetWndPos( Fvector2().set( 0, 0 ) );
}

bool CUIDragDropListEx::OnMouseAction( float x,
                                       float y,
                                       EUIMessages mouse_action ) {
...
void CUIDragDropListEx::SetItem( CUICellItem* itm,
                                 Fvector2 abs_pos ) // start at cursor pos
{
    if ( m_container->AddSimilar( itm ) )
        return;
    ...
    if ( m_container->ValidCell( dest_cell_pos ) &&
         m_container->IsRoomFree( dest_cell_pos, itm->GetGridSize() ) )
        SetItem( itm, dest_cell_pos );
    else
        SetItem( itm );
}
...
void CUIDragDropListEx::SetItem( CUICellItem* itm,
                                 Ivector2 cell_pos ) // start at cell
{
    if ( m_container->AddSimilar( itm ) )
        return;
...
CUICellItem* CUIDragDropListEx::RemoveItem( CUICellItem* itm,
                                            bool force_root ) {
...
CUICellItem* CUICellContainer::FindSimilar( CUICellItem* itm ) {
    ...
    for ( WINDOW_LIST_it it = m_ChildWndList.begin();
          m_ChildWndList.end() != it; ++it ) {
#ifdef DEBUG
        ...
        if ( iitem && iitem->m_pInventory ) {
            if ( iitem->m_pInventory->ItemFromSlot( iitem->BaseSlot() ) ==
                 iitem )
                continue;
...
void CUICellContainer::PlaceItemAtPos( CUICellItem* itm, Ivector2& cell_pos ) {
    ...
    itm->SetWndSize(
        Fvector2().set( ( m_cellSize.x * cs.x ), ( m_cellSize.y * cs.y ) ) );
    if ( !m_pParentDragDropList->GetVirtualCells() )
        itm->SetWndPos( Fvector2().set(
            ( ( m_cellSpacing.x + m_cellSize.x ) * cell_pos.x ),
            ( ( m_cellSpacing.y + m_cellSize.y ) * cell_pos.y ) ) );
    else {
        Ivector2 alignment_vec =
            m_pParentDragDropList->GetVirtualCellsAlignment();
        Fvector2 pos = Fvector2().set( 0, 0 );
        if ( alignment_vec.x == 1 )
            pos.x = ( m_pParentDragDropList->GetWndSize().x -
                      cs.x * ( m_cellSpacing.x + m_cellSize.x ) ) /
                    2;
        ...
        if ( alignment_vec.y == 1 )
            pos.y = ( m_pParentDragDropList->GetWndSize().y -
                      cs.y * ( m_cellSpacing.y + m_cellSize.y ) ) /
                    2;
...
bool CUICellContainer::IsRoomFree( const Ivector2& pos,
                                   const Ivector2& _size ) {
    Ivector2 tmp;
...
void CUICellContainer::GetTexUVLT( Fvector2& uv,
                                   u32 col,
                                   u32 row,
                                   u8 select_mode ) {
    switch ( select_mode ) {
...
Ivector2 CUICellContainer::TopVisibleCell() {
    return Ivector2().set( 0,
                           iFloor( m_pParentDragDropList->ScrollPos() /
                                   float( CellSize().y + m_cellSpacing.y ) ) );
}
...
void CUICellContainer::Grow() {
    SetCellsCapacity(
        Ivector2().set( m_cellsCapacity.x, m_cellsCapacity.y + 1 ) );
}

void CUICellContainer::Shrink() {}

bool CUICellContainer::ValidCell( const Ivector2& pos ) const {
    return !( pos.x < 0 || pos.y < 0 || pos.x >= m_cellsCapacity.x ||
              pos.y >= m_cellsCapacity.y );
}
...
Ivector2 CUICellContainer::PickCell( const Fvector2& abs_pos ) {
    ...
    ap.mul( -1 );
    res.x = iFloor(
        ap.x / ( m_cellSize.x + m_cellSpacing.x * ( m_cellsCapacity.x - 1 ) /
                                    m_cellsCapacity.x ) );
    res.y = iFloor(
        ap.y / ( m_cellSize.y + m_cellSpacing.y * ( m_cellsCapacity.y - 1 ) /
                                    m_cellsCapacity.y ) );
    if ( !ValidCell( res ) )
        res.set( -1, -1 );
    return res;
...
void CUICellContainer::Draw() {
    ...
    tgt_cells.x2 =
        iFloor( ( float( clientArea.width() ) + float( cell_sz.x ) - EPS ) /
                float( cell_sz.x ) ) +
        tgt_cells.lt.x;
    tgt_cells.y2 =
        iFloor( ( float( clientArea.height() ) + float( cell_sz.y ) - EPS ) /
                float( cell_sz.y ) ) +
        tgt_cells.lt.y;
...
#define tx 0.25f
    ...
    UI().ClientToScreenScaled( f_len, float( CellSize().x ),
                               float( CellSize().y ) );
    UI().ClientToScreenScaled( sp_len, float( CellsSpacing().x ),
                               float( CellsSpacing().y ) );

    GetCellsInRange( tgt_cells, m_cells_to_draw );

    // fill cell buffer
    u32 max_prim_cnt =
        ( ( tgt_cells.width() + 1 ) * ( tgt_cells.height() + 1 ) * 6 );
    UIRender->StartPrimitive( max_prim_cnt, IUIRender::ptTriList,
                              UI().m_currentPointType );

    //	u32 cell_i = 0;
    for ( int x = 0; x <= tgt_cells.width(); ++x ) {
        for ( int y = 0; y <= tgt_cells.height(); ++y /*, ++cell_i*/ ) {
            Fvector2 rect_offset;
            rect_offset.set( ( drawLT.x + f_len.x * x + sp_len.x * x ),
            ...
            for ( u32 k = 0; k < 6; ++k ) {
                ...
                // pv->set			(iFloor(drawLT.x + p.x*(f_len.x)
                // + f_len.x*x)-0.5f, 				 iFloor(drawLT.y + p.y*(f_len.y) +
                //f_len.y*y)-0.5f, 				 0xFFFFFFFF,tp.x+uv.x,tp.y+uv.y);
                UIRender->PushPoint(
                    iFloor( rect_offset.x + p.x * ( f_len.x ) ) - 0.5f,
                    iFloor( rect_offset.y + p.y * ( f_len.y ) ) - 0.5f, 0,
                    m_pParentDragDropList->back_color, tp.x + uv.x,
                    tp.y + uv.y );
            } // for k
        } // for y
    } // for x
    ...
    if ( m_cells_to_draw.size() ) {
        ...
        for ( ; it != m_cells_to_draw.end(); ++it ) // all cells
        {
            CUICell& cell = ( *it );
            if ( !cell.Empty() &&
                 ( cell.m_item->m_drawn_frame != Device.dwFrame ) ) {
                cell.m_item->Draw();
            }
        }
    }
