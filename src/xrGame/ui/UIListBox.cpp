...
CUIListBoxItem* CUIListBox::AddTextItem( LPCSTR text ) {
    ...
    pItem->SetWndSize(
        Fvector2().set( GetDesiredChildWidth(), m_def_item_height ) );
...
CUIListBoxItem* CUIListBox::AddItem() {
    ...
    item->InitFrameLineWnd(
        Fvector2().set( 0, 0 ),
        Fvector2().set( GetDesiredChildWidth() - 5, m_def_item_height ) );
    ...
    if ( m_pad->IsChild( pWnd ) ) {
        switch ( msg ) {
            case LIST_ITEM_SELECT:
                GetMessageTarget()->SendMessage( this, LIST_ITEM_SELECT,
                                                 pData );
                break;
...
float CUIListBox::GetLongestLength() {
    ...
    for ( WINDOW_LIST_it it = m_pad->GetChildWndList().begin();
        ...
        if ( item ) {
            ...
            float tmp_len =
                item->GetFont()->SizeOf_( lines->m_text.c_str() ); // all ok
