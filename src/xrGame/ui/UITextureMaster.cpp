...
void CUITextureMaster::ParseShTexInfo( LPCSTR xml_file ) {
    ...
    for ( int fi = 0; fi < files_num; ++fi ) {
        ...
        for ( int i = 0; i < num; i++ ) {
            ...
            info.rect.x2 =
                xml.ReadAttribFlt( node, "texture", i, "width" ) + info.rect.x1;
            info.rect.y1 = xml.ReadAttribFlt( node, "texture", i, "y" );
            info.rect.y2 = xml.ReadAttribFlt( node, "texture", i, "height" ) +
                           info.rect.y1;
            shared_str id = xml.ReadAttrib( node, "texture", i, "id" );
            /* avo: fix issue when values were not updated (silently skipped)
            when same key is encountered more than once. This is how std::map is
            designed.
            /* Also used more efficient C++11 std::map::emplace method instead
            of outdated std::pair::make_pair */
            /* XXX: avo: note that xxx.insert(mk_pair(v1,v2)) pattern is used
             * extensively throughout solution so there is a good potential for
...
void CUITextureMaster::InitTexture( const shared_str& texture_name,
    ...
    if ( it != m_textures.end() ) {
        ...
        tc->SetSize( Fvector2().set( it->second.rect.width(),
...
void CUITextureMaster::GetTextureShader( const shared_str& texture_name,
                                         ui_shader& sh ) {
    xr_map< shared_str, TEX_INFO >::iterator it;
    it = m_textures.find( texture_name );

    R_ASSERT3( it != m_textures.end(), "can't find texture",
               texture_name.c_str() );
