...
void CUIItemInfo::InitItemInfo( LPCSTR xml_name ) {
    ...
    if ( uiXml.NavigateToNode( "static_name", 0 ) ) {
        ...
        m_complex_desc =
            ( uiXml.ReadAttribInt( "static_name", 0, "complex_desc", 0 ) == 1 );
    }
    ...
    if ( uiXml.NavigateToNode( "descr_list", 0 ) ) {
        ...
        // UIDesc_line						=
        // xr_new<CUIStatic>(); AttachChild
        // (UIDesc_line); UIDesc_line->SetAutoDelete		(true);
        // xml_init.InitStatic				(uiXml,
        // "description_line", 0, UIDesc_line);
        ...
        m_desc_info.bShowDescrText =
            !!uiXml.ReadAttribInt( "descr_list", 0, "only_text_info", 1 );
        m_b_FitToHeight =
            !!uiXml.ReadAttribInt( "descr_list", 0, "fit_to_height", 0 );
        xml_init.InitScrollView( uiXml, "descr_list", 0, UIDesc );
        xml_init.InitFont( uiXml, "descr_list:font", 0, m_desc_info.uDescClr,
                           m_desc_info.pDescFont );
    }
    ...
    if ( uiXml.NavigateToNode( "image_static", 0 ) ) {
        ...
        UIItemImageSize.set( UIItemImage->GetWidth(),
                             UIItemImage->GetHeight() );
    }
...
//-- Tronex
LPCSTR CUIItemInfo::GetItemName( CInventoryItem& pInvItem,
                                 LPCSTR m_item_name ) {
...
LPCSTR CUIItemInfo::GetItemShortName( CInventoryItem& pInvItem,
                                      LPCSTR m_item_short_name ) {
    ::luabind::functor< LPCSTR > functorGetShortName;
    if ( ai().script_engine().functor( "ui_item.item_short_name",
                                       functorGetShortName ) ) {
        CGameObject* GO = pInvItem.cast_game_object();
        if ( GO )
            return functorGetShortName( GO->lua_game_object(),
                                        m_item_short_name );
    }
    return m_item_short_name;
}

LPCSTR CUIItemInfo::GetItemDescription( CInventoryItem& pInvItem,
                                        LPCSTR m_item_description ) {
    ::luabind::functor< LPCSTR > functorGetDescription;
    if ( ai().script_engine().functor( "ui_item.item_description",
                                       functorGetDescription ) ) {
        CGameObject* GO = pInvItem.cast_game_object();
        if ( GO )
            return functorGetDescription( GO->lua_game_object(),
                                          m_item_description );
    }
...
void CUIItemInfo::InitItem( CUICellItem* pCellItem,
    ...
    string256 str;
    if ( UIName ) {
        UIName->SetText( GetItemName(
            *pInvItem, pInvItem->NameItem() ) ); //(pInvItem->NameItem());
    ...
    if ( UIWeight ) {
        ...
        if ( !weight ) {
            if ( CWeaponAmmo* ammo =
                     fast_dynamic_cast< CWeaponAmmo* >( pInvItem ) ) {
                // its helper item, m_boxCur is zero, so recalculate via
                // CInventoryItem::Weight()
    ...
    if ( UICost && IsGameTypeSingle() && item_price != u32( -1 ) ) {
        xr_sprintf( str, "%d RU",
                    item_price ); // will be owerwritten in multiplayer
        UICost->SetText( str );
    ...
    if ( UITradeTip && IsGameTypeSingle() ) {
        ...
        if ( trade_tip == NULL )
            ...
            UITradeTip->SetText(
                CStringTable().translate( trade_tip ).c_str() );
    ...
    if ( UIDesc ) {
        ...
        if ( m_desc_info.bShowDescrText ) {
            ...
            pItem->SetText(
                GetItemDescription( *pInvItem, *pInvItem->ItemDescription() ) );
        ...
        if ( m_b_FitToHeight ) {
            UIDesc->SetWndSize( Fvector2().set( UIDesc->GetWndSize().x,
                                                UIDesc->GetPadSize().y ) );
            Fvector2 new_size;
    ...
    if ( UIItemImage ) {
        // Загружаем картинку
        if ( pSettings->line_exist( m_pInvItem->m_section_id.c_str(),
                                    "icons_texture" ) ) {
            LPCSTR icons_texture = pSettings->r_string(
                m_pInvItem->m_section_id.c_str(), "icons_texture" );
        ...
        Frect texture_rect;
        texture_rect.lt.set( item_grid_rect.x1 * INV_GRID_WIDTH,
                             item_grid_rect.y1 * INV_GRID_HEIGHT );
        texture_rect.rb.set( item_grid_rect.x2 * INV_GRID_WIDTH,
                             item_grid_rect.y2 * INV_GRID_HEIGHT );
...
void CUIItemInfo::TryAddConditionInfo( CInventoryItem& pInvItem,
                                       CInventoryItem* pCompareItem ) {
...
void CUIItemInfo::TryAddOutfitInfo( CInventoryItem& pInvItem,
                                    CInventoryItem* pCompareItem ) {
    CCustomOutfit* outfit = smart_cast< CCustomOutfit* >( &pInvItem );
    CHelmet* helmet = smart_cast< CHelmet* >( &pInvItem );
    if ( outfit && UIOutfitInfo ) {
        CCustomOutfit* comp_outfit =
            smart_cast< CCustomOutfit* >( pCompareItem );
