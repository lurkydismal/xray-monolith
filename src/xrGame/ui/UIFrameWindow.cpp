...
void CUIFrameWindow::InitTextureEx( LPCSTR texture, LPCSTR sh_name ) {
    ...
    string256 buf;
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_back" ), sh_name, m_shader,
        m_tex_rect[ fmBK ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_l" ), sh_name, m_shader,
        m_tex_rect[ fmL ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_r" ), sh_name, m_shader,
        m_tex_rect[ fmR ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_t" ), sh_name, m_shader,
        m_tex_rect[ fmT ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_b" ), sh_name, m_shader,
        m_tex_rect[ fmB ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_lt" ), sh_name, m_shader,
        m_tex_rect[ fmLT ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_rb" ), sh_name, m_shader,
        m_tex_rect[ fmRB ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_rt" ), sh_name, m_shader,
        m_tex_rect[ fmRT ] );
    CUITextureMaster::InitTexture(
        strconcat( sizeof( buf ), buf, texture, "_lb" ), sh_name, m_shader,
        m_tex_rect[ fmLB ] );

    R_ASSERT2(
        fsimilar( m_tex_rect[ fmLT ].height(), m_tex_rect[ fmT ].height() ),
        texture );
    R_ASSERT2(
        fsimilar( m_tex_rect[ fmLT ].height(), m_tex_rect[ fmRT ].height() ),
        texture );
    //	R_ASSERT2(fsimilar(m_tex_rect[fmL].height(),
    // m_tex_rect[fmBK].height()),texture );
    R_ASSERT2(
        fsimilar( m_tex_rect[ fmL ].height(), m_tex_rect[ fmR ].height() ),
        texture );
    R_ASSERT2(
        fsimilar( m_tex_rect[ fmLB ].height(), m_tex_rect[ fmB ].height() ),
        texture );
    R_ASSERT2(
        fsimilar( m_tex_rect[ fmLB ].height(), m_tex_rect[ fmRB ].height() ),
        texture );

    R_ASSERT2(
        fsimilar( m_tex_rect[ fmLT ].width(), m_tex_rect[ fmL ].width() ),
        texture );
    R_ASSERT2(
        fsimilar( m_tex_rect[ fmLT ].width(), m_tex_rect[ fmLB ].width() ),
        texture );

    //	R_ASSERT2(fsimilar(m_tex_rect[fmT].width(),
    // m_tex_rect[fmBK].width()),texture );
    R_ASSERT2( fsimilar( m_tex_rect[ fmT ].width(), m_tex_rect[ fmB ].width() ),
               texture );

    R_ASSERT2(
        fsimilar( m_tex_rect[ fmRT ].width(), m_tex_rect[ fmR ].width() ),
        texture );
    R_ASSERT2(
        fsimilar( m_tex_rect[ fmRT ].width(), m_tex_rect[ fmRB ].width() ),
        texture );
}
...
void CUIFrameWindow::DrawElements() {
    ...
    back_len.x =
        rect.width() - m_tex_rect[ fmLT ].width() - m_tex_rect[ fmRT ].width();
    back_len.y = rect.height() - m_tex_rect[ fmLT ].height() -
                 m_tex_rect[ fmRB ].height();
    ...
    if ( back_len.x > 0.0f && back_len.y > 0.0f ) // back
        cnt = iCeil( back_len.x / m_tex_rect[ fmBK ].width() ) *
              iCeil( back_len.y / m_tex_rect[ fmBK ].height() );

    rect_count += cnt;

    rect_count *= 6;

    UIRender->StartPrimitive( rect_count, IUIRender::ptTriList,
...
bool CUIFrameWindow::get_points( Frect const& r,
                                 int i,
                                 Fvector2& LTp,
                                 Fvector2& RBp,
                                 Fvector2& LTt,
...
void CUIFrameWindow::draw_tile_line( Frect rect,
                                     int i,
                                     bool b_horz,
                                     Fvector2 const& ts ) {
    Fvector2 LTt, RBt;
