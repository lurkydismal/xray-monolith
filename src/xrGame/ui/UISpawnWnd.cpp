...
void CUISpawnWnd::Init() {
    ...
    CUIXmlInit::InitStatic( xml_doc, "team_selector:background", 0,
                            m_pBackground );
    CUIXmlInit::InitStatic( xml_doc, "team_selector:image_frames_tl", 0,
                            m_pFrames[ 0 ] );
    ...
    CUIXmlInit::Init3tButton( xml_doc, "team_selector:btn_spectator", 0,
                              m_pBtnSpectator );
    CUIXmlInit::Init3tButton( xml_doc, "team_selector:btn_autoselect", 0,
                              m_pBtnAutoSelect );
    CUIXmlInit::Init3tButton( xml_doc, "team_selector:btn_back", 0,
                              m_pBtnBack );
}
...
void CUISpawnWnd::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    if ( BUTTON_CLICKED == msg ) {
        ...
        VERIFY( game );
        // game_cl_TeamDeathmatch * tdm = smart_cast<game_cl_TeamDeathmatch
        // *>(&(Game()));
        if ( pWnd == m_pImage1 )
            game->OnTeamSelect( 0 );
...
bool CUISpawnWnd::OnKeyboardAction( int dik, EUIMessages keyboard_action ) {
    ...
    game_cl_mp* game = smart_cast< game_cl_mp* >( &Game() );
    VERIFY( game );
    // game_cl_TeamDeathmatch * dm = smart_cast<game_cl_TeamDeathmatch
    // *>(&(Game()));
