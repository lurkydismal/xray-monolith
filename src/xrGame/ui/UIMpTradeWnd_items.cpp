...
void SBuyItemInfo::SetState( const EItmState& s ) {
    ...
    switch ( m_item_state ) {
        ...
        case e_bought: {
            VERIFY2( s == e_shop || s == e_sold,
                     "incorrect SBuyItemInfo::SetState sequence" );

            m_item_state = e_shop;
            break;
        };
        case e_sold: {
            VERIFY2( s == e_own || s == e_bought,
                     "incorrect SBuyItemInfo::SetState sequence" );
            m_item_state = e_own;
            break;
        };
...
LPCSTR _state_names[] = { "e_undefined", "e_bought", "e_sold", "e_own",
                          "e_shop" };
...
CInventoryItem* CUIMpTradeWnd::CreateItem_internal(
    const shared_str& name_sect ) {
...
SBuyItemInfo* CUIMpTradeWnd::CreateItem( const shared_str& name_sect,
                                         SBuyItemInfo::EItmState type,
                                         bool find_if_exist ) {
    SBuyItemInfo* iinfo =
        ( find_if_exist ) ? FindItem( name_sect, type ) : NULL;
    if ( iinfo )
        return iinfo;
...
SBuyItemInfo* CUIMpTradeWnd::FindItem( SBuyItemInfo::EItmState state ) {
    ...
    ITEMS_vec_cit it =
        std::find_if( m_all_items.begin(), m_all_items.end(), eq );
    return ( it == m_all_items.end() ) ? NULL : ( *it );
}

SBuyItemInfo* CUIMpTradeWnd::FindItem( const shared_str& name_sect,
                                       SBuyItemInfo::EItmState type ) {
...
SBuyItemInfo* CUIMpTradeWnd::FindItem( CUICellItem* item ) {
    ...
    R_ASSERT2( 0,
               "buy menu data corruption. cant find corresponding "
               "SBuyItemInfo* for CellItem" );
    return NULL;
}
...
void CUIMpTradeWnd::DeleteHelperItems( CUIDragDropListEx* list ) {
    ...
    for ( ITEMS_vec::iterator it = m_all_items.begin(); it != m_all_items.end();
          ++it ) {
        ...
        if ( item->GetState() != SBuyItemInfo::e_bought &&
             item->GetState() != SBuyItemInfo::e_own ) {
            continue;
        }

        if ( item->m_cell_item->IsHelper() ) {
            if ( std::find( to_sell.begin(), to_sell.end(), item ) ==
                 to_sell.end() ) {
    ...
    for ( ITEMS_vec::iterator it = to_sell.begin(); it != to_sell.end();
          ++it ) {
...
void CUIMpTradeWnd::CreateHelperItems( xr_vector< shared_str >& ammo_types ) {
    for ( xr_vector< shared_str >::iterator it = ammo_types.begin();
          it != ammo_types.end(); ++it ) {
        const shared_str& ammo_name = *it;
        if ( !m_store_hierarchy->FindItem( ammo_name ) ) {
            continue;
        }

        SBuyItemInfo* ammo_item =
            CreateItem( ammo_name, SBuyItemInfo::e_undefined, false );
        ammo_item->m_cell_item->SetIsHelper( true );
        TryToBuyItem( ammo_item, bf_normal, NULL );
    }
}

void CUIMpTradeWnd::CreateHelperItems(
    CUIDragDropListEx* list,
    const CStoreHierarchy::item* shop_level ) {
    for ( xr_vector< shared_str >::const_iterator it =
              shop_level->m_items_in_group.begin();
          it != shop_level->m_items_in_group.end(); ++it ) {
        shared_str item_name = *it;
        CUIDragDropListEx* match_list = GetMatchedListForItem( item_name );

        if ( match_list == list ) {
            SBuyItemInfo* new_item =
                CreateItem( item_name, SBuyItemInfo::e_undefined, false );

            CUIInventoryCellItem* inventory_cell_item;
            if ( ( inventory_cell_item =
                       fast_dynamic_cast< CUIInventoryCellItem* >(
                           new_item->m_cell_item ) ) != NULL ) {
...
void CUIMpTradeWnd::CreateHelperItems( CUIDragDropListEx* list ) {
    ...
    CInventoryItem* parent_item =
        ( CInventoryItem* )parent_list->GetItemIdx( 0 )->m_pData;
    CWeapon* wpn = smart_cast< CWeapon* >( parent_item );
    R_ASSERT( wpn );

    CreateHelperItems( wpn->m_ammoTypes );

    if ( CWeaponMagazinedWGrenade* wpn2 =
             smart_cast< CWeaponMagazinedWGrenade* >( parent_item ) ) {
...
void CUIMpTradeWnd::UpdateCorrespondingItemsForList(
    CUIDragDropListEx* _list ) {
    ...
    if ( _list->ItemsCount() != 0 ) {
        ...
        CInventoryItem* main_item =
            ( CInventoryItem* )_list->GetItemIdx( 0 )->m_pData;
    ...
    while ( !_tmp_list.empty() ) {
        ...
        if ( !bNecessary ) {
            ...
            xr_list< SBuyItemInfo* >::iterator tmp_it =
                find( _tmp_list.begin(), _tmp_list.end(), res_info );
...
struct eq_name_state_comparer {
    ...
    eq_name_state_comparer( const shared_str& _name,
                            SBuyItemInfo::EItmState _state )
        : m_name( _name ), m_state( _state ) {}

    bool operator()( SBuyItemInfo* info ) {
        return ( info->m_name_sect == m_name ) &&
               ( info->GetState() == m_state );
    }
};
...
struct eq_name_state_addon_comparer {
    ...
    eq_name_state_addon_comparer( const shared_str& _name,
                                  SBuyItemInfo::EItmState _state,
                                  u8 ad )
        : m_name( _name ), m_state( _state ), m_addons( ad ) {}

    bool operator()( SBuyItemInfo* info ) {
        if ( ( info->m_name_sect == m_name ) &&
             ( info->GetState() == m_state ) ) {
...
struct eq_group_state_comparer {
    ...
    eq_group_state_comparer( const shared_str& _group,
                             SBuyItemInfo::EItmState _state )
        : m_group( _group ), m_state( _state ) {}

    bool operator()( SBuyItemInfo* info ) {
        if ( !info->m_cell_item->IsHelper() &&
             ( info->GetState() == m_state ) ) {
            const shared_str& _grp =
                g_mp_restrictions.GetItemGroup( info->m_name_sect );
            return ( _grp == m_group );
        } else
            return false;
    }
...
u32 CUIMpTradeWnd::GetItemCount( SBuyItemInfo::EItmState state ) const {
    return ( u32 )std::count_if( m_all_items.begin(), m_all_items.end(),
                                 eq_state_comparer( state ) );
}

u32 CUIMpTradeWnd::GetItemCount( const shared_str& name_sect,
                                 SBuyItemInfo::EItmState state ) const {
    return ( u32 )std::count_if( m_all_items.begin(), m_all_items.end(),
                                 eq_name_state_comparer( name_sect, state ) );
}
...
u32 CUIMpTradeWnd::GetItemCount( const shared_str& name_sect,
    ...
    return ( u32 )std::count_if(
        m_all_items.begin(), m_all_items.end(),
        eq_name_state_addon_comparer( name_sect, state, addon ) );
}

u32 CUIMpTradeWnd::GetGroupCount( const shared_str& name_group,
                                  SBuyItemInfo::EItmState state ) const {
    return ( u32 )std::count_if( m_all_items.begin(), m_all_items.end(),
                                 eq_group_state_comparer( name_group, state ) );
}
...
struct preset_eq {
    ...
    preset_eq( const shared_str& _name, u8 ad )
        : m_name( _name ), m_addon( ad ) {};

    bool operator()( const _preset_item& pitem ) {
        return ( pitem.sect_name == m_name ) &&
               ( pitem.addon_state == m_addon );
    };
};
...
void CUIMpTradeWnd::StorePreset( ETradePreset idx,
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( !( iinfo->GetState() == SBuyItemInfo::e_bought ||
                iinfo->GetState() == SBuyItemInfo::e_own ) )
            continue;
        ...
        preset_items::iterator fit = std::find_if(
            v.begin(), v.end(), preset_eq( iinfo->m_name_sect, addon_state ) );
        if ( fit != v.end() )
            continue;
...
void CUIMpTradeWnd::ApplyPreset( ETradePreset idx ) {
    ...
    for ( ; it != it_e; ++it ) {
        ...
        for ( u32 i = _cnt; i < _one.count; ++i ) {
            SBuyItemInfo* pitem =
                CreateItem( _one.sect_name, SBuyItemInfo::e_undefined, false );
            ...
            if ( !b_res ) {
                ...
                if ( _one.addon_state ) {
                    for ( u32 i = 0; i < 3; ++i ) {
                        ...
                        SBuyItemInfo* addon_item = CreateItem(
                            addon_name, SBuyItemInfo::e_undefined, false );
                        bool b_res_addon =
                            TryToBuyItem( addon_item, bf_normal, pitem );
                        if ( !b_res_addon )
        ...
        do {
            ...
            if ( iinfo ) {
                ...
                if ( IsAddonAttached( iinfo, at_scope ) ) {
                    SBuyItemInfo* detached_addon =
                        DetachAddon( iinfo, at_scope );
                ...
                if ( IsAddonAttached( iinfo, at_glauncher ) ) {
                    SBuyItemInfo* detached_addon =
                        DetachAddon( iinfo, at_glauncher );
                    detached_addon->SetState( SBuyItemInfo::e_undefined );
                    detached_addon->SetState( SBuyItemInfo::e_shop );
                    detached_addon->m_cell_item->SetOwnerList( NULL );
                }
...
void CUIMpTradeWnd::SellAll() {
    ...
    do {
        ...
        if ( iinfo ) {
            ...
            CUICellItem* citem = iinfo->m_cell_item->OwnerList()->RemoveItem(
                iinfo->m_cell_item, false );
...
void CUIMpTradeWnd::CheckDragItemToDestroy() {
    if ( CUIDragDropListEx::m_drag_item &&
        ...
        CUIDragDropListEx* _drag_owner =
            CUIDragDropListEx::m_drag_item->ParentItem()->OwnerList();
...
#ifndef MASTER_GOLD
    Msg( "CUIMpTradeWnd::DumpAllItems.total[%d] reason [%s]",
         m_all_items.size(), s );
    ITEMS_vec_cit it = m_all_items.begin();
    ITEMS_vec_cit it_e = m_all_items.end();
    for ( ; it != it_e; ++it ) {
        SBuyItemInfo* iinfo = *it;
        Msg( "[%s] state[%s]", iinfo->m_name_sect.c_str(),
             iinfo->GetStateAsText() );
    }
    Msg( "------" );
...
void CUICellItemTradeMenuDraw::OnDraw( CUICellItem* cell ) {
    ...
    bool b_can_buy_rank = m_trade_wnd->CheckBuyPossibility(
        m_info_item->m_name_sect, CUIMpTradeWnd::bf_check_rank_restr, true );

    if ( !b_can_buy_rank ) {
        cell->SetTextureColor( m_trade_wnd->m_item_color_restr_rank );
        return;
    }
    bool b_can_buy_money = m_trade_wnd->CheckBuyPossibility(
        m_info_item->m_name_sect, CUIMpTradeWnd::bf_check_money, true );
    if ( !b_can_buy_money ) {
