...
bool CServerList::NeedToRefreshCurServer() {
    ...
    // (CUIListItemServer*)m_list[LST_SERVER].GetSelectedItem(); if(!pItem)
    return false;
    // return browser().HasAllKeys(pItem->GetInfo()->info.Index) == false;
};

void CServerList::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    if ( m_bShowServerInfo && LIST_ITEM_CLICKED == msg &&
         &m_list[ LST_SERVER ] == pWnd ) {
        ...
        CUIListItemServer* item = smart_cast< CUIListItemServer* >(
            m_list[ LST_SERVER ].GetSelectedItem() );
        if ( !item )
            return;
        xr_string command;

        item->CreateConsoleCommand(
            command, m_playerName.c_str(),
...
void CServerList::FillUpDetailedServerInfo() {
    ...
    CUIListItemServer* pItem =
        ( CUIListItemServer* )m_list[ LST_SERVER ].GetSelectedItem();
    // if(pItem)
    //{
    //	ServerInfo srvInfo;
    ...
    //(m_list[LST_SRV_PROP].GetWidth()/2);

    //		pItemAdv->AddTextField
    //(gi.InfoData.c_str(), m_list[LST_SRV_PROP].GetWidth()/2);
    //	}
    //}
    // else
    ClearDetailedServerInfo();
...
void CServerList::UpdateSizes() {
    ...
    int page_size = ( m_list[ LST_SERVER ].GetSize() *
                          m_list[ LST_SERVER ].GetItemHeight() <
                      height )
                        ? 0
                        : int( height );
...
bool CServerList::IsValidItem( ServerInfo& item ) {
    ...
    // result &= !m_sf.empty ? (m_sf.empty == (item.m_ServerNumPlayers == 0))
    // : true; result &= !m_sf.full ? (m_sf.full == (item.m_ServerNumPlayers ==
    // item.m_ServerMaxPlayers))	: true; result &= !m_sf.with_pass ?
    // (m_sf.with_pass == item.m_bPassword)
    // : true; result &= !m_sf.without_pass ? (m_sf.without_pass !=
    // item.m_bPassword)						: true;
    // result &= !m_sf.without_ff ? (m_sf.without_ff != item.m_bFFire)
    // : true; result &= !m_sf.listen_servers ? (m_sf.listen_servers !=
    // item.m_bDedicated)					: true;
...
void CServerList::InitFromXml( CUIXml& xml_doc, LPCSTR path ) {
    ...
    string256 buf;
    CUIXmlInit::InitListBox( xml_doc,
                             strconcat( sizeof( buf ), buf, path, ":list" ), 0,
                             &m_list[ LST_SERVER ] );
    m_fListH[ 0 ] = m_list[ LST_SERVER ].GetHeight();
    m_fListH[ 1 ] = xml_doc.ReadAttribFlt( buf, 0, "height2" );
    CUIXmlInit::InitListBox(
        xml_doc,
        strconcat( sizeof( buf ), buf, path, ":list_server_properties" ), 0,
        &m_list[ LST_SRV_PROP ] );
    CUIXmlInit::InitListBox(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":list_players_list" ), 0,
        &m_list[ LST_PLAYERS ] );
    CUIXmlInit::InitFrameWindow(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":frame" ), 0,
        &m_frame[ LST_SERVER ] );
    CUIXmlInit::InitFrameWindow(
        xml_doc,
        strconcat( sizeof( buf ), buf, path, ":list_server_properties:frame" ),
        0, &m_frame[ LST_SRV_PROP ] );
    CUIXmlInit::InitFrameWindow(
        xml_doc,
        strconcat( sizeof( buf ), buf, path, ":list_players_list:frame" ), 0,
        &m_frame[ LST_PLAYERS ] );
    CUIXmlInit::InitFont(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":list_item:text" ), 0,
        m_itemInfo.text_color, m_itemInfo.text_font );
    CUIXmlInit::InitEditBox(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":edit_gs_filter" ), 0,
        &m_edit_gs_filter );
    m_fEditPos[ 0 ] = m_edit_gs_filter.GetWndPos().y;
    m_fEditPos[ 1 ] = xml_doc.ReadAttribFlt( buf, 0, "y2" );
    CUIXmlInit::InitFrameLine(
        xml_doc,
        strconcat( sizeof( buf ), buf, path, ":cap_server_properties" ), 0,
        &m_header2[ 0 ] );
    CUIXmlInit::InitFrameLine(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":cap_players_list" ), 0,
        &m_header2[ 1 ] );
    CUIXmlInit::InitFrameLine(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":cap_frags" ), 0,
        &m_header2[ 2 ] );
    CUIXmlInit::InitFrameLine(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":cap_death" ), 0,
        &m_header2[ 3 ] );

    m_itemInfo.size.icon = xml_doc.ReadAttribFlt(
        strconcat( sizeof( buf ), buf, path, ":sizes" ), 0, "icon" );
    ...
    for ( int i = 0; i < LST_COLUMN_COUNT; i++ ) {
        CUIXmlInit::Init3tButton(
            xml_doc, strconcat( sizeof( buf ), buf, path, ":header" ), 0,
            &m_header[ i ] );
        CUIXmlInit::InitFrameLine(
            xml_doc, strconcat( sizeof( buf ), buf, path, ":header_frames" ), 0,
            &m_header_frames[ i ] );
    }
...
void CServerList::ConnectToSelected() {
    ...
    //	{
    //		if (m_connect_cb)
    //			m_connect_cb(ece_unique_nick_not_registred,
    //"mp_gp_unique_nick_not_registred"); 		return;
    //	}
    //	if (!xr_strcmp(tmp_profile->unique_nick(), "@expired"))
    //	{
    //		if (m_connect_cb)
    //			m_connect_cb(ece_unique_nick_expired,
    //"mp_gp_unique_nick_has_expired"); 		return;
    //	}
    //}

    // CUIListItemServer* item =
    // smart_cast<CUIListItemServer*>(m_list[LST_SERVER].GetSelectedItem());
    // if(!item)
    //	return;
    ...
    // item->GetInfo()->info.icons.user_pass)
    //{
    //	m_message_box->m_pMessageBox->SetUserPasswordMode
    //(item->GetInfo()->info.icons.user_pass);
    //	m_message_box->m_pMessageBox->SetPasswordMode
    //(item->GetInfo()->info.icons.pass); 	m_message_box->ShowDialog(true);
    // }
    // else
    //{
    //	xr_string command;
...
void CServerList::UpdateServerInList( ServerInfo* pServerInfo, int index ) {
    ...
    for ( int i = 0; i < sz; i++ ) {
        CUIListItemServer* pItem = static_cast< CUIListItemServer* >(
            m_list[ LST_SERVER ].GetItemByIDX( i ) );
...
void CServerList::RefreshList_internal() {
    ...
    // u32 NumServersFound				=
    // browser().GetServersCount(); g_gs_browser
    // = m_GSBrowser; m_tmp_srv_lst.resize (NumServersFound);
    ...
    if ( 0 == xr_strcmp( m_sort_func, "server_name" ) )
        std::sort( m_tmp_srv_lst.begin(), m_tmp_srv_lst.end(),
                   sort_by_ServerName );
    ...
    // for (u32 i=0; i<NumServersFound; i++)
    //{
    //	ServerInfo NewServerInfo; 	browser().GetServerInfoByIndex
    //(&NewServerInfo, m_tmp_srv_lst[i]);
...
void CServerList::SrvInfo2LstSrvInfo( const ServerInfo* pServerInfo ) {
    ...
    // xr_string address				=
    // pServerInfo->m_HostName; char
    // port[8]; address							+=
    // "/port="; address
    // += itoa(pServerInfo->m_Port, port, 10); m_itemInfo.info.address
    // = address.c_str(); m_itemInfo.info.map				=
    // pServerInfo->m_SessionName; m_itemInfo.info.game			=
    // GameTypeToString( (EGameIDs)pServerInfo->m_GameType, true);
    // m_itemInfo.info.players.printf	("%d/%d",
    // pServerInfo->m_ServerNumPlayers, pServerInfo->m_ServerMaxPlayers);
    ...
    // m_itemInfo.info.icons.punkbuster= false;//	=
    // pServerInfo->m_bPunkBuster; m_itemInfo.info.icons.user_pass	=
...
bool CServerList::sort_by_Players( int p1, int p2 ) {
    ...
    // return (g_bSort_Ascending) ? (info1.m_ServerNumPlayers <
    // info2.m_ServerNumPlayers) : (info1.m_ServerNumPlayers >
    // info2.m_ServerNumPlayers);
    return p1 < p2;
}
...
bool CServerList::sort_by_Ping( int p1, int p2 ) {
    ...
    // return (g_bSort_Ascending) ? (info1.m_Ping < info2.m_Ping) :
    // (info1.m_Ping > info2.m_Ping);
    return p1 < p2;
}
