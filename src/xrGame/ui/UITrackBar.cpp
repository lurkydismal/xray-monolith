...
void CUITrackBar::InitTrackBar( Fvector2 pos, Fvector2 size ) {
    ...
    item_width =
        CUITextureMaster::GetTextureWidth( "ui_inGame2_opt_slider_box_e" );
    item_height =
        CUITextureMaster::GetTextureHeight( "ui_inGame2_opt_slider_box_e" );

    item_width *= UI().get_current_kx();

    m_pSlider->InitButton(
        Fvector2().set( 0.0f, 0.0f ) /*(size.y - item_height)/2.0f)*/,
...
void CUITrackBar::UpdatePosRelativeToMouse() {
    ...
    __fval = ( __fmax - __fmin ) * ( fpos - btn_width / 2 ) /
...
void CUITrackBar::UpdatePos() {
    ...
    if ( m_b_is_float )
        R_ASSERT2(
            m_f_val >= m_f_min && m_f_val <= m_f_max,
            "CUITrackBar::UpdatePos() - m_val >= m_min && m_val <= m_max" );
    else
        R_ASSERT2(
            m_i_val >= m_i_min && m_i_val <= m_i_max,
            "CUITrackBar::UpdatePos() - m_val >= m_min && m_val <= m_max" );
