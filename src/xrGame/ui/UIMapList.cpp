...
void CUIMapList::StartDedicatedServer() {
    ...
    GetFullPathName( ModuleFileName, sizeof( g_sLaunchWorkingFolder ),
                     g_sLaunchWorkingFolder, &ModuleName );
...
EGameIDs CUIMapList::GetCurGameType() {
    ...
    if ( combo_ms ) {
        ...
        if ( 0 == xr_strcmp( text, CStringTable().translate( get_token_name(
                                       g_GameModes, eGameIDDeathmatch ) ) ) )
            return eGameIDDeathmatch;
        else if ( 0 == xr_strcmp( text,
                                  CStringTable().translate( get_token_name(
                                      g_GameModes, eGameIDTeamDeathmatch ) ) ) )
            return eGameIDTeamDeathmatch;
        else if ( 0 ==
                  xr_strcmp( text, CStringTable().translate( get_token_name(
                                       g_GameModes, eGameIDArtefactHunt ) ) ) )
            return eGameIDArtefactHunt;
        else if ( 0 ==
                  xr_strcmp( text,
                             CStringTable().translate( get_token_name(
                                 g_GameModes, eGameIDCaptureTheArtefact ) ) ) )
        ...
        if ( 0 == xr_strcmp(
                      text, get_token_name( g_GameModes, eGameIDDeathmatch ) ) )
            return eGameIDDeathmatch;
        else if ( 0 ==
                  xr_strcmp( text, get_token_name( g_GameModes,
            ...
            return eGameIDArtefactHunt;
                           ...
                           text, get_token_name( g_GameModes,
...
const char* CUIMapList::GetCommandLine( LPCSTR player_name ) {
    ...
    if ( player_name == NULL || 0 == xr_strlen( player_name ) ) {
        ...
        if ( xr_strlen( player_name2 ) == 0 ) {
            xr_strcpy( player_name2, xr_strlen( Core.UserName )
                                         ? Core.UserName
                                         : Core.CompName );
        }
...
void CUIMapList::SaveMapList() {
    ...
    for ( u32 idx = 0; idx < m_pList2->GetSize(); ++idx ) {
        ...
        xr_sprintf( map_name, "sv_addmap %s/ver=%s", M.map_name.c_str(),
                    M.map_ver.c_str() );
...
void CUIMapList::AddWeather( const shared_str& WeatherType,
                             const shared_str& WeatherTime,
                             u32 _id ) {
...
void CUIMapList::InitFromXml( CUIXml& xml_doc, const char* path ) {
    ...
    CUIXmlInit::InitFrameLine(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":header_1" ), 0,
        m_pLbl1 );
    CUIXmlInit::InitFrameLine(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":header_2" ), 0,
        m_pLbl2 );
    CUIXmlInit::InitFrameWindow(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":frame_1" ), 0,
        m_pFrame1 );
    CUIXmlInit::InitFrameWindow(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":frame_2" ), 0,
        m_pFrame2 );
    CUIXmlInit::InitListBox( xml_doc,
                             strconcat( sizeof( buf ), buf, path, ":list_1" ),
                             0, m_pList1 );
    CUIXmlInit::InitListBox( xml_doc,
                             strconcat( sizeof( buf ), buf, path, ":list_2" ),
                             0, m_pList2 );
    CUIXmlInit::Init3tButton(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":btn_left" ), 0,
        m_pBtnLeft );
    CUIXmlInit::Init3tButton(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":btn_right" ), 0,
        m_pBtnRight );
    CUIXmlInit::Init3tButton( xml_doc,
                              strconcat( sizeof( buf ), buf, path, ":btn_up" ),
                              0, m_pBtnUp );
    CUIXmlInit::Init3tButton(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":btn_down" ), 0,
        m_pBtnDown );
}
...
void CUIMapList::UpdateMapList( EGameIDs GameType ) {
    ...
    for ( u32 i = 0; i < cnt; ++i ) {
        CUIListBoxItem* itm = m_pList1->AddTextItem(
            CStringTable().translate( M.m_map_names[ i ].map_name ).c_str() );
