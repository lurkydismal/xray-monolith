...
void CUIKeyBinding::InitFromXml( CUIXml& xml_doc, LPCSTR path ) {
    ...
    CUIXmlInit::InitScrollView(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":scroll_view" ), 0,
        m_scroll_wnd );

    CUIXmlInit::InitFrameWindow(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":frame" ), 0, &m_frame );
    ...
    CUIXmlInit::InitFrameLine(
        ...
        &m_header[ 1 ] );
    CUIXmlInit::InitFrameLine(
        xml_doc, strconcat( sizeof( buf ), buf, path, ":header_3" ), 0,
        &m_header[ 2 ] );
...
void CUIKeyBinding::FillUpList( CUIXml& xml_doc_ui, LPCSTR path_ui ) {
    ...
    for ( int i = 0; i < groupsCount; ++i ) {
        ...
        CUIXmlInit::InitStatic(
            xml_doc_ui,
            strconcat( sizeof( buf ), buf, path_ui, ":scroll_view:item_group" ),
            0, pItem );
        ...
        for ( int j = 0; j < commandsCount; ++j ) {
            ...
            CUIXmlInit::InitStatic( xml_doc_ui,
                                    strconcat( sizeof( buf ), buf, path_ui,
                                               ":scroll_view:item_key" ),
                                    0, pItem );
            ...
            pEditKB->InitKeyBind(
                Fvector2().set( item_pos, 0.0f ),
                Fvector2().set( item_width, pItem->GetWndSize().y ) );
        ...
        if ( action_name ) {
            if ( IsActionExist( action_name, xml_doc ) )
                ...
                if ( first ) {
                    ...
                    pItem->SetWndSize(
                        Fvector2().set( m_scroll_wnd->GetWndSize().x, 20.0f ) );
                    pItem->SetText(
                        "NEXT ITEMS NOT DESCRIBED IN COMMAND DESC LIST" );
                    first = false;
