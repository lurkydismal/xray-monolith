...
CSE_ALifeTraderAbstract* ch_info_get_from_id( u16 id ) {
    if ( ai().get_alife() && ai().get_game_graph() ) {
        return smart_cast< CSE_ALifeTraderAbstract* >(
            ai().alife().objects().object( id ) );
    } else {
        return smart_cast< CSE_ALifeTraderAbstract* >(
            Level().Server->game->get_entity_from_eid( id ) );
    }
}
...
void CUICharacterInfo::InitCharacterInfo( Fvector2 pos,
                                          Fvector2 size,
                                          CUIXml* xml_doc ) {
    ...
    /*	Init_IconInfoItem( *xml_doc, "rank_icon",           eRankIcon     );
       ...
       ); Init_IconInfoItem( *xml_doc, "commumity_big_icon_over",
    ...
    */
    ...
    if ( xml_doc->NavigateToNode( "icon:deadbody", 0 ) ) {
        m_deadbody_color = CUIXmlInit::GetColor( *xml_doc, "icon:deadbody", 0,
                                                 m_deadbody_color );
    }
...
void CUICharacterInfo::Init_StrInfoItem( CUIXml& xml_doc,
                                         LPCSTR item_str,
                                         UIItemType type ) {
...
void CUICharacterInfo::Init_IconInfoItem( CUIXml& xml_doc,
                                          LPCSTR item_str,
                                          UIItemType type ) {
    if ( xml_doc.NavigateToNode( item_str, 0 ) ) {
        CUIStatic* pItem = m_icons[ type ] = xr_new< CUIStatic >();
...
void CUICharacterInfo::InitCharacter( u16 id ) {
    ...
    if ( m_icons[ eName ] ) {
        m_icons[ eName ]->TextItemControl()->SetTextST(
            T->m_character_name.c_str() );
    }
    if ( m_icons[ eRank ] ) {
        m_icons[ eRank ]->TextItemControl()->SetTextST(
            GetRankAsText( chInfo.Rank().value() ) );
    }
    if ( m_icons[ eCommunity ] ) {
        m_icons[ eCommunity ]->TextItemControl()->SetTextST(
            chInfo.Community().id().c_str() );
    }
    ...
    /*
            ...
            {
                    if ( m_icons[eCommunityIcon   ] ) { m_icons[eCommunityIcon
       ]->InitTexture( community1 ); } if ( m_icons[eCommunityBigIcon] ) {
       m_icons[eCommunityBigIcon]->InitTexture( community2 ); } return;
            }
            ...
            if ( CUICharacterInfo::get_actor_community( &our_comm, &enemy ) )
            {
                    ...
                    {
                            xr_strcpy( community1, sizeof(community1),
       our_comm.c_str() ); xr_strcat( community1, sizeof(community1), "_icon" );

                            xr_strcpy( community2, sizeof(community2),
       our_comm.c_str() ); xr_strcat( community2, sizeof(community2), "_wide" );

                            if ( m_icons[eCommunityIcon   ] ) {
       m_icons[eCommunityIcon   ]->InitTexture( community1 ); } if (
       m_icons[eCommunityBigIcon] ) { m_icons[eCommunityBigIcon]->InitTexture(
       community2 ); } return;
                    }
            }

            if ( m_icons[eCommunityIcon   ]     ) {
       m_icons[eCommunityIcon]->Show( false ); } if ( m_icons[eCommunityBigIcon]
       ) { m_icons[eCommunityBigIcon]->Show( false ); } if (
       m_icons[eCommunityIconOver   ] ) { m_icons[eCommunityIconOver]->Show(
       false ); } if ( m_icons[eCommunityBigIconOver] ) {
       m_icons[eCommunityBigIconOver]->Show( false ); }
    */
}

void CUICharacterInfo::InitCharacterMP( LPCSTR player_name,
                                        LPCSTR player_icon ) {
    ClearInfo();
...
void CUICharacterInfo::SetRelation( ALife::ERelationType relation,
    ...
    m_icons[ eRelation ]->TextItemControl()->SetTextColor(
        GetRelationColor( relation ) );
    m_icons[ eRelation ]->TextItemControl()->SetTextST(
        GetGoodwillAsText( goodwill ) );
}
...
void CUICharacterInfo::Update() {
    ...
    if ( hasOwner() && ( m_bForceUpdate || ( Device.dwFrame % 50 == 0 ) ) ) {
        ...
        CSE_ALifeTraderAbstract* T =
            detail::object_exists_in_alife_registry( m_ownerID )
        ...
        if ( m_icons[ eIcon ] ) {
            CSE_ALifeCreatureAbstract* pCreature =
                smart_cast< CSE_ALifeCreatureAbstract* >( T );
            if ( pCreature && !pCreature->g_Alive() ) {
                m_icons[ eIcon ]->SetTextureColor(
                    color_argb( 255, 255, 160, 160 ) );
            }
        }
    }
}
...
// ------- static ---------
bool CUICharacterInfo::get_actor_community( shared_str* our,
                                            shared_str* enemy ) {
    VERIFY( our && enemy );
    our->_set( NULL );
    enemy->_set( NULL );
    ...
    LPCSTR vs_teams =
        pSettings->r_string( "actor_communities", actor_team.c_str() );
