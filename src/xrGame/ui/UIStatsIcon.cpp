...
void CUIStatsIcon::InitTexInfo() {
    ...
    for ( int i = RANK_0; i <= RANK_4; i++ ) {
        ...
        m_tex_info[ i ][ 0 ].rect =
            CUITextureMaster::GetTextureRect( rank_tex );
    ...
    if ( pSettings->line_exist( artefact_name, "icons_texture" ) ) {
        LPCSTR icons_texture =
            pSettings->r_string( artefact_name, "icons_texture" );
        m_tex_info[ ARTEFACT ][ 0 ].sh =
    ...
    m_tex_info[ ARTEFACT ][ 0 ].rect.set(
        fXPos * INV_GRID_WIDTH, fYPos * INV_GRID_HEIGHT,
    ...
    // death
    m_tex_info[ DEATH ][ 0 ].sh->create( "hud\\default",
                                         "ui\\ui_mp_icon_kill" );
