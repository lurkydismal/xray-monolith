...
CUITextWnd* CUIGameLog::AddLogMessage( LPCSTR msg ) {
    ...
    pItem->SetColorAnimation( "ui_main_msgs_short", LA_ONLYALPHA | LA_TEXTCOLOR,
                              5000.0f );
    ForceUpdate();
    return pItem;
}
...
CUIPdaMsgListItem* CUIGameLog::AddPdaMessage() {
    ...
    pItem->InitPdaMsgListItem(
        Fvector2().set( GetDesiredChildWidth(), 10.0f ) );
...
void CUIGameLog::Update() {
    ...
    for ( ; it != it_e; ++it ) {
        CUILightAnimColorConroller* pItem =
            smart_cast< CUILightAnimColorConroller* >( *it );
    ...
    for ( it = m_pad->GetChildWndList().begin(); it != it_e; ++it ) {
        ...
        if ( !( visible_rect.in( r.x1, r.y1 ) &&
                visible_rect.in( r.x2, r.y1 ) &&
                visible_rect.in( r.x1, r.y2 ) &&
