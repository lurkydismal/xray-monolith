...
CUITabControl::~CUITabControl() {
    xr_delete( m_arrows ); // detaches the arrow widgets while this parent is
                           // still valid
    RemoveAll();
}
...
bool CUITabControl::AddItem( LPCSTR pItemName,
                             LPCSTR pTexName,
                             Fvector2 pos,
                             Fvector2 size ) {
...
bool CUITabControl::InsertItem( CUITabButton* pButton, u32 at ) {
    ...
    // Seam geometry (pitch = width - overlap + margin) is authored: overlap is
    // the drawn end cap (from the art), the leftover pitch is the strip margin,
    // fixed once from the XML rest positions before any scroll. Seed from the
    // first two non-parked non-dynamic tabs (index-agnostic, so a parked
    // zero-width stub anywhere in the strip can't defeat it); every later strip
    // tab inherits it; dynamic tabs arrive pre-seeded (AddTab copies the
    // reference tab's). Non-dynamic tabs are only ever appended, so `pButton`
    // is the second such tab when the count first reaches 2. The strip is
    // assumed to be single-overlap packed from layout x 0 -- laying it out that
    // way is the caller's responsibility. Each tab then owns its shape.
    if ( !pButton->m_dynamic && pButton->GetWndSize().x > 0.0f ) {
        ...
        if ( strip_n == 2 ) {
            // Split the authored pitch: overlap is the drawn end cap (from the
            // art); the leftover is the strip margin. Deriving overlap as width
            // - pitch would fold the gap in (that value is overlap - margin).
            ...
            pButton->SetOverlap(
                first->Overlap() ); // inherit the strip overlap
    }
...
void CUITabControl::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    // Scroll-arrow clicks (handled before the accelerator gate so they work
    // regardless of mode)
...
void CUITabControl::OnStaticFocusReceive( CUIWindow* pWnd ) {
    GetMessageTarget()->SendMessage( this, WINDOW_FOCUS_RECEIVED,
                                     static_cast< void* >( pWnd ) );
}

void CUITabControl::OnStaticFocusLost( CUIWindow* pWnd ) {
    GetMessageTarget()->SendMessage( this, WINDOW_FOCUS_LOST,
                                     static_cast< void* >( pWnd ) );
}

void CUITabControl::OnTabChange( const shared_str& sCur,
                                 const shared_str& sPrev ) {
...
CUITabButton* CUITabControl::GetButtonById( const shared_str& id ) {
    TABS_VECTOR::const_iterator it =
        std::find( m_TabsArr.begin(), m_TabsArr.end(), id );
    if ( it != m_TabsArr.end() )
        return *it;
    else
        return NULL;
