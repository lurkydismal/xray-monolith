...
// Tutorial Item
//-----------------------------------------------------------------------------
CUISequenceVideoItem::CUISequenceVideoItem( CUISequencer* owner )
    : CUISequenceItem( owner ) {
    // m_texture				= NULL;
    m_flags.set( etiPlaying | etiNeedStart | etiDelayed | etiBackVisible,
                 FALSE );
    m_delay = 0.f;
    m_wnd = NULL;
    m_wnd_bg = NULL;
...
void CUISequenceVideoItem::Load( CUIXml* xml, int idx ) {
    ...
    bool bFullScreen =
        ( 1 == xml->ReadAttribInt( "video_wnd", 0, "fullscreen", 0 ) );
    if ( !bFullScreen ) {
...
void CUISequenceVideoItem::Update() {
    ...
    u32 sync_tm = ( 0 == m_sound._handle() )
                      ...
                      : ( m_sound._feedback() ? m_sound._feedback()->play_time()
                                              : m_sync_time );
    m_sync_time = sync_tm;
    // processing A&V

    if ( m_texture->HasTexture() ) {
        BOOL is_playing = m_sound._handle() ? !!m_sound._feedback()
                                            : m_texture->video_IsPlaying();
        if ( is_playing ) {
            m_texture->video_Sync( m_sync_time );
        } else {
            // sync start
            if ( m_flags.test( etiNeedStart ) ) {
                m_sound.play_at_pos( NULL, Fvector().set( 0.0f, 0.f, 0.0f ),
                                     sm_Intro );
...
void CUISequenceVideoItem::OnRender() {
    if ( !m_texture->HasTexture() && m_wnd->GetShader() &&
         m_wnd->GetShader()->inited() ) {
...
bool CUISequenceVideoItem::Stop( bool bForce ) {
    ...
    m_wnd->Show( false );
    if ( Device.dwTimeContinual >= m_time_start &&
         m_wnd->GetParent() == m_owner->MainWnd() )
