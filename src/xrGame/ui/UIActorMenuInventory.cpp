...
void CUIActorMenu::SendEvent_Item2Slot( PIItem pItem,
                                        u16 recipient,
                                        u16 slot_id ) {
    ...
    NET_Packet P;
    CGameObject::u_EventGen( P, GEG_PLAYER_ITEM2SLOT,
                             pItem->object().H_Parent()->ID() );
...
void CUIActorMenu::SendEvent_Item2Belt( PIItem pItem, u16 recipient ) {
    ...
    NET_Packet P;
    CGameObject::u_EventGen( P, GEG_PLAYER_ITEM2BELT,
                             pItem->object().H_Parent()->ID() );
    P.w_u16( pItem->object().ID() );
    CGameObject::u_EventSend( P );
...
void CUIActorMenu::SendEvent_Item2Ruck( PIItem pItem, u16 recipient ) {
    ...
    NET_Packet P;
    CGameObject::u_EventGen( P, GEG_PLAYER_ITEM2RUCK,
                             pItem->object().H_Parent()->ID() );
    P.w_u16( pItem->object().ID() );
    CGameObject::u_EventSend( P );
...
bool FindItemInList( CUIDragDropListEx* lst,
                     PIItem pItem,
                     CUICellItem*& ci_res ) {
    ...
    for ( u32 i = 0; i < count; ++i ) {
        ...
        for ( u32 j = 0; j < ci->ChildsCount(); ++j ) {
            CUIInventoryCellItem* ici =
                smart_cast< CUIInventoryCellItem* >( ci->Child( j ) );
            if ( ici->object() == pItem ) {
                ci_res = ici;
...
void CUIActorMenu::OnInventoryAction( PIItem pItem, u16 action_type ) {
    CUIDragDropListEx* all_lists[] = {
        m_pInventoryBeltList, m_pInventoryBagList, m_pTradeActorBagList,
    ...
    switch ( action_type ) {
        ...
        case GE_OWNERSHIP_TAKE: {
            ...
            CUICellItem* ci = NULL;
            if ( GetMenuMode() == mmDeadBodySearch &&
                 FindItemInList( m_pDeadBodyBagList, pItem, ci ) )
                break;
                ...
                if ( lst_to_add ) {
                    ...
                    if ( 0 == xr_strcmp( m_sort_kinds[ mode ], "s_all" ) ) {
                        ...
                        for ( int i = 0; i < kinds; i++ ) {
                            ...
                            if ( pItem->m_kind != NULL &&
                                 pItem->m_kind.equal( kind ) ) {
                                CUICellItem* itm = create_cell_item( pItem );
                                lst_to_add->SetItem( itm );
                            }
                        }
...
#ifndef MASTER_GOLD
                    Msg( "all ok. item [%d] removed from list",
                         pItem->object_id() );
#endif // #ifndef MASTER_GOLD
                    break;
                }
...
void CUIActorMenu::AttachAddon( PIItem item_to_upgrade ) {
    ...
    if ( OnClient() ) {
        NET_Packet P;
        CGameObject::u_EventGen( P, GE_ADDON_ATTACH,
                                 item_to_upgrade->object().ID() );
...
void CUIActorMenu::DetachAddon( LPCSTR addon_name, PIItem itm ) {
    ...
    if ( OnClient() ) {
        NET_Packet P;
        if ( itm == NULL )
            CGameObject::u_EventGen( P, GE_ADDON_DETACH,
...
void CUIActorMenu::InitInventoryContents( CUIDragDropListEx* pBagList ) {
    ...
    for ( u8 i = 1; i <= m_slot_count; ++i ) {
        if ( m_pInvList[ i ] )
            ...
            if ( i != BOLT_SLOT && i != PDA_SLOT &&
                 !m_pActorInvOwner->inventory().SlotIsPersistent( i ) )
                InitCellForSlot( i );
        }
    }

    curr_list = m_pInventoryBeltList;
    TIItemContainer::iterator itb =
        m_pActorInvOwner->inventory().m_belt.begin();
    ...
    std::sort( ruck_list.begin(), ruck_list.end(),
               InventoryUtilities::GreaterRoomInRuck );
    ...
    for ( ; itb != ite; ++itb ) {
        CMPPlayersBag* bag =
            smart_cast< CMPPlayersBag* >( &( *itb )->object() );
        if ( bag )
            continue;
...
bool CUIActorMenu::TryActiveSlot( CUICellItem* itm ) {
    ...
    if ( slot == GRENADE_SLOT ) {
        ...
        if ( prev_iitem && ( prev_iitem->object().cNameSect() !=
                             iitem->object().cNameSect() ) ) {
...
bool CUIActorMenu::ToSlotScript( CScriptGameObject* GO,
                                 bool force_place,
                                 u16 slot_id ) {
    CInventoryItem* iitem =
        smart_cast< CInventoryItem* >( GO->object().dcast_CObject() );
    ...
    for ( WINDOW_LIST_it it = child_list.begin(); child_list.end() != it;
          ++it ) {
...
bool CUIActorMenu::ToSlot( CUICellItem* itm, bool force_place, u16 slot_id ) {
    ...
    if ( m_pActorInvOwner->inventory().CanPutInSlot( iitem, slot_id ) ) {
        ...
        bool result = ( !b_own_item ) ||
                      m_pActorInvOwner->inventory().Slot( slot_id, iitem );
        VERIFY( result );

        CUICellItem* i =
            old_owner->RemoveItem( itm, ( old_owner == new_owner ) );
        ...
        if ( m_pActorInvOwner->inventory().SlotIsPersistent( slot_id ) &&
             slot_id != DETECTOR_SLOT )
            return false;

        if ( slot_id == INV_SLOT_2 ) {
            if ( m_pActorInvOwner->inventory().CanPutInSlot( iitem,
                                                             INV_SLOT_3 ) &&
                 iitem->BaseSlot() != KNIFE_SLOT )
                return ToSlot( itm, force_place, INV_SLOT_3 );
            if ( m_pActorInvOwner->inventory().CanPutInSlot( iitem,
                                                             KNIFE_SLOT ) &&
                 iitem->BaseSlot() != INV_SLOT_3 )
        ...
        if ( CUIDragDropListEx::m_drag_item &&
             CUIDragDropListEx::m_drag_item->BackList() )
        ...
        if ( slot_list != GetListByType( iActorBag ) ) {
            ...
            // Alundaio: Since the player's inventory is being used as a slot we
            // need to search for cell with matching m_pData
            CUICellContainer* c = slot_list->GetContainer();
            xrCriticalSectionGuard guard( c->csUi );
            CUIWindow::WINDOW_LIST child_list = c->GetChildWndList();

            for ( WINDOW_LIST_it it = child_list.begin();
                  child_list.end() != it; ++it ) {
                CUICellItem* i = ( CUICellItem* )( *it );
                PIItem pitm = ( PIItem )i->m_pData;
                if ( pitm == _iitem ) {
                    if ( ToBag( i, false ) ) {
        ...
        if ( b_own_item && slot_id == DETECTOR_SLOT ) {
            if ( ToSlot( itm, false, slot_id ) ) {
                ...
                if ( det )
                    det->ToggleDevice( g_player_hud->attached_item( 0 ) !=
                                       NULL );
                return true;
            }
            return false;
...
bool CUIActorMenu::ToBag( CUICellItem* itm, bool b_use_cursor_pos ) {
    ...
    if ( m_pActorInvOwner->inventory().CanPutInRuck( iitem ) ||
         ( b_already && ( new_owner != old_owner ) ) ) {
        bool result =
            b_already ||
            ( !b_own_item || m_pActorInvOwner->inventory().Ruck( iitem ) );
        VERIFY( result );
...
bool CUIActorMenu::ToBeltScript( CScriptGameObject* GO,
                                 bool b_use_cursor_pos ) {
    CInventoryItem* iitem =
        smart_cast< CInventoryItem* >( GO->object().dcast_CObject() );
...
bool CUIActorMenu::ToBelt( CUICellItem* itm, bool b_use_cursor_pos ) {
    ...
    if ( m_pActorInvOwner->inventory().CanPutInBelt( iitem ) ) {
        ...
        bool result =
            ( !b_own_item ) || m_pActorInvOwner->inventory().Belt( iitem );
        VERIFY( result );
        CUICellItem* i =
            old_owner->RemoveItem( itm, ( old_owner == new_owner ) );
        ...
        Ivector2 belt_cell_pos =
            belt_list->PickCell( GetUICursor().GetCursorPosition() );
        ...
        //		VERIFY
        //(slot_cell && ((PIItem)slot_cell->m_pData)==_iitem);
...
bool CUIActorMenu::ToQuickSlot( CUICellItem* itm ) {
    ...
    // Alundaio: Fix deep recursion if placing icon greater then col/row set in
    // actor_menu.xml
    ...
    u8 slot_idx =
        u8( m_pQuickSlot->PickCell( GetUICursor().GetCursorPosition() ).x );
    if ( slot_idx == 255 )
        return false;
    ...
    m_pQuickSlot->SetItem( create_cell_item( iitem ),
                           GetUICursor().GetCursorPosition() );
    xr_strcpy( ACTOR_DEFS::g_quick_use_slots[ slot_idx ],
               iitem->m_section_id.c_str() );
    return true;
}

bool CUIActorMenu::OnItemDropped( PIItem itm,
                                  CUIDragDropListEx* new_owner,
                                  CUIDragDropListEx* old_owner ) {
    CUICellItem* _citem =
        ( new_owner->ItemsCount() == 1 ) ? new_owner->GetItemIdx( 0 ) : NULL;
...
void CUIActorMenu::ActivatePropertiesBox() {
    ...
    if ( !( m_currMenuMode == mmInventory ||
            m_currMenuMode == mmDeadBodySearch || m_currMenuMode == mmUpgrade ||
...
void CUIActorMenu::PropertiesBoxForSlots( PIItem item, bool& b_show ) {
    ...
    // Флаг-признак для невлючения пункта контекстного меню: Dreess Outfit, если
    // костюм уже надет
    bool bAlreadyDressed = false;
    u16 cur_slot = item->BaseSlot();

    if ( !pOutfit && !pHelmet && !pBackpack && cur_slot != NO_ACTIVE_SLOT &&
         !inv.SlotIsPersistent( cur_slot ) &&
         m_pActorInvOwner->inventory().ItemFromSlot( cur_slot ) != item
         /*&& inv.CanPutInSlot(item, cur_slot)*/ ) {
        m_UIPropertiesBox->AddItem( "st_move_to_slot", NULL,
                                    INVENTORY_TO_SLOT_ACTION );
        b_show = true;
    }
    if ( item->Belt() && inv.CanPutInBelt( item ) ) {
        m_UIPropertiesBox->AddItem( "st_move_on_belt", NULL,
            ...
            if ( !pHelmet ) {
                if ( !pBackpack ) {
                    if ( m_currMenuMode == mmDeadBodySearch )
                        m_UIPropertiesBox->AddItem( "st_move_to_bag", NULL,
                                                    INVENTORY_TO_BAG_ACTION );
                    else
                        m_UIPropertiesBox->AddItem( "st_unequip", NULL,
                                                    INVENTORY_TO_BAG_ACTION );
                } else
                    m_UIPropertiesBox->AddItem( "st_unequip_backpack", NULL,
                                                INVENTORY_TO_BAG_ACTION );
            } else
                m_UIPropertiesBox->AddItem( "st_undress_helmet", NULL,
                                            INVENTORY_TO_BAG_ACTION );
        } else
            m_UIPropertiesBox->AddItem( "st_undress_outfit", NULL,
                                        INVENTORY_TO_BAG_ACTION );
    ...
    if ( pOutfit && !bAlreadyDressed ) {
        m_UIPropertiesBox->AddItem( "st_dress_outfit", NULL,
                                    INVENTORY_TO_SLOT_ACTION );
        b_show = true;
    }

    CCustomOutfit* outfit_in_slot = m_pActorInvOwner->GetOutfit();
    if ( pHelmet && !bAlreadyDressed &&
         ( !outfit_in_slot || outfit_in_slot->bIsHelmetAvaliable ) ) {
        m_UIPropertiesBox->AddItem( "st_dress_helmet", NULL,
                                    INVENTORY_TO_SLOT_ACTION );
        b_show = true;
    }

    if ( pBackpack && !bAlreadyDressed &&
         ( !outfit_in_slot || outfit_in_slot->bIsBackpackAvaliable ) ) {
        m_UIPropertiesBox->AddItem( "st_equip_backpack", NULL,
                                    INVENTORY_TO_SLOT_ACTION );
        b_show = true;
    }
}

void CUIActorMenu::PropertiesBoxForWeapon( CUICellItem* cell_item,
                                           PIItem item,
                                           bool& b_show ) {
    ...
    if ( pWeapon->GrenadeLauncherAttachable() ) {
        if ( pWeapon->IsGrenadeLauncherAttached() ) {
            m_UIPropertiesBox->AddItem(
                "st_detach_gl", NULL, INVENTORY_DETACH_GRENADE_LAUNCHER_ADDON );
            b_show = true;
        } else {
        }
    }
    if ( pWeapon->ScopeAttachable() ) {
        if ( pWeapon->IsScopeAttached() ) {
            m_UIPropertiesBox->AddItem( "st_detach_scope", NULL,
    ...
    if ( pWeapon->SilencerAttachable() ) {
        if ( pWeapon->IsSilencerAttached() ) {
            m_UIPropertiesBox->AddItem( "st_detach_silencer", NULL,
    ...
    if ( smart_cast< CWeaponMagazined* >( pWeapon ) && IsGameTypeSingle() ) {
        ...
        if ( !b ) {
            for ( u32 i = 0; i < cell_item->ChildsCount(); ++i ) {
                CWeaponMagazined* weap_mag = smart_cast< CWeaponMagazined* >(
                    ( CWeapon* )cell_item->Child( i )->m_pData );
        ...
        if ( b ) {
            m_UIPropertiesBox->AddItem( "st_unload_magazine", NULL,
...
void CUIActorMenu::PropertiesBoxForAddon( PIItem item, bool& b_show ) {
    ...
    CGrenadeLauncher* pGrenadeLauncher =
        smart_cast< CGrenadeLauncher* >( item );
    CInventory* inv = &m_pActorInvOwner->inventory();
    ...
    if ( pScope ) {
        if ( item_in_slot_2 && item_in_slot_2->CanAttach( pScope ) ) {
            shared_str str =
                CStringTable().translate( "st_attach_scope_to_pistol" );
            str.printf( "%s %s", str.c_str(), item_in_slot_2->m_name.c_str() );
            m_UIPropertiesBox->AddItem( str.c_str(), ( void* )item_in_slot_2,
        ...
        if ( item_in_slot_3 && item_in_slot_3->CanAttach( pScope ) ) {
            ...
            m_UIPropertiesBox->AddItem( str.c_str(), ( void* )item_in_slot_3,
                                        INVENTORY_ATTACH_ADDON );
            //			m_UIPropertiesBox->AddItem(
    ...
    if ( pSilencer ) {
        if ( item_in_slot_2 && item_in_slot_2->CanAttach( pSilencer ) ) {
            shared_str str =
                CStringTable().translate( "st_attach_silencer_to_pistol" );
            str.printf( "%s %s", str.c_str(), item_in_slot_2->m_name.c_str() );
            m_UIPropertiesBox->AddItem( str.c_str(), ( void* )item_in_slot_2,
                                        INVENTORY_ATTACH_ADDON );
            //			m_UIPropertiesBox->AddItem(
    ...
    if ( pGrenadeLauncher ) {
        if ( item_in_slot_2 && item_in_slot_2->CanAttach( pGrenadeLauncher ) ) {
            shared_str str =
                CStringTable().translate( "st_attach_gl_to_rifle" );
            str.printf( "%s %s", str.c_str(), item_in_slot_2->m_name.c_str() );
            m_UIPropertiesBox->AddItem( str.c_str(), ( void* )item_in_slot_2,
                                        INVENTORY_ATTACH_ADDON );
            //			m_UIPropertiesBox->AddItem(
...
void CUIActorMenu::PropertiesBoxForUsing( PIItem item, bool& b_show ) {
    ...
    act_str = READ_IF_EXISTS( pSettings, r_string, section_name,
                              "default_use_text", 0 );
    if ( act_str ) {
        ...
        if ( pMedkit || pAntirad ) {
            ...
            if ( !xr_strcmp( section_name, "vodka" ) ||
                 !( xr_strcmp( section_name, "energy_drink" ) ) ) {
                act_str = "st_drink";
            } else if ( !xr_strcmp( section_name, "bread" ) ||
                        !xr_strcmp( section_name, "kolbasa" ) ||
    ...
    LPCSTR functor_name = READ_IF_EXISTS( pSettings, r_string, GO->cNameSect(),
                                          "use1_functor", 0 );
    if ( functor_name ) {
        ...
        if ( ai().script_engine().functor( functor_name, funct1 ) ) {
            ...
            if ( act_str ) {
                m_UIPropertiesBox->AddItem( act_str, NULL,
    ...
    functor_name = READ_IF_EXISTS( pSettings, r_string, GO->cNameSect(),
                                   "use2_functor", 0 );
    if ( functor_name ) {
        ::luabind::functor< LPCSTR > funct1;
        if ( ai().script_engine().functor( functor_name, funct1 ) ) {
...
void CUIActorMenu::PropertiesBoxForDrop( CUICellItem* cell_item,
                                         PIItem item,
                                         bool& b_show ) {
    if ( !item->IsQuestItem() ) {
        ...
        if ( cell_item->ChildsCount() ) {
            m_UIPropertiesBox->AddItem( "st_drop_all", ( void* )33,
...
void CUIActorMenu::PropertiesBoxForDonate( PIItem item, bool& b_show ) {
    if ( !item->IsQuestItem() ) {
        m_UIPropertiesBox->AddItem( "st_donate", NULL,
...
void CUIActorMenu::ProcessPropertiesBoxClicked( CUIWindow* w, void* d ) {
    PIItem item = CurrentIItem();
    CUICellItem* cell_item = CurrentItem();
    if ( !m_UIPropertiesBox->GetClickedItem() || !item || !cell_item ||
         !cell_item->OwnerList() ) {
        return;
    }
    ...
    switch ( m_UIPropertiesBox->GetClickedItem()->GetTAG() ) {
        ...
        case INVENTORY_EAT2_ACTION: {
            CGameObject* GO = smart_cast< CGameObject* >( item );
            LPCSTR functor_name =
                READ_IF_EXISTS( pSettings, r_string, GO->cNameSect(),
                                "use1_action_functor", 0 );
            if ( functor_name ) {
        ...
        case INVENTORY_ATTACH_ADDON: {
            PIItem item =
                CurrentIItem(); // temporary storing because of AttachAddon is
                                // setting curiitem to NULL
            AttachAddon(
                ( PIItem )( m_UIPropertiesBox->GetClickedItem()->GetData() ) );
            if ( m_currMenuMode == mmDeadBodySearch )
        ...
        case INVENTORY_UNLOAD_MAGAZINE: {
            ...
            for ( u32 i = 0; i < cell_item->ChildsCount(); ++i ) {
                CUICellItem* child_itm = cell_item->Child( i );
                CWeaponMagazined* child_weap_mag =
                    smart_cast< CWeaponMagazined* >(
                        ( CWeapon* )child_itm->m_pData );
                if ( child_weap_mag ) {
