...
const ButtonListDialog::NamedButton& ButtonListDialog::GetButton(
    int i ) const {
    return buttons[ i ];
}
...
bool ButtonListDialog::OnKeyboardAction( int dik, EUIMessages keyboardAction ) {
    ...
    if ( WINDOW_KEY_PRESSED == keyboardAction ) {
        ...
        if ( dik >= DIK_1 && dik <= DIK_1 - 1 + btnCount &&
             btnCount <= 9 ) // handle 1..9 keys only
        {
...
void ButtonListDialog::SendMessage( CUIWindow* wnd,
                                    s16 msg,
                                    void* data /*= nullptr */ ) {
...
void ChangeWeatherDialog::InitChangeWeather( CUIXml& xmlDoc ) {
    ...
    CUIXmlInit::InitStatic( xmlDoc, "change_weather:background", 0,
                            Background );
    CUIXmlInit::Init3tButton( xmlDoc, "change_weather:btn_cancel", 0,
                              CancelButton );
    ...
    for ( u32 i = 0; i < weathers.size(); i++ ) {
        xr_sprintf( path, "change_weather:btn_%s",
                    gameWeathers[ i ].Name.c_str() );
        CUIXmlInit::Init3tButton( xmlDoc, path, 0, GetButton( i ).Button );
        xr_sprintf( path, "change_weather:txt_%s",
                    gameWeathers[ i ].Name.c_str() );
...
void ChangeWeatherDialog::OnButtonClick( int i ) {
    string1024 command;
    xr_sprintf( command, "cl_votestart changeweather %s %s",
                *weathers[ i ].Name, *weathers[ i ].Time );
...
void ChangeGameTypeDialog::InitChangeGameType( CUIXml& xmlDoc ) {
    ...
    CUIXmlInit::InitStatic( xmlDoc, "change_gametype:background", 0,
                            Background );
    CUIXmlInit::Init3tButton( xmlDoc, "change_gametype:btn_cancel", 0,
                              CancelButton );
...
void ChangeGameTypeDialog::OnButtonClick( int i ) {
    string1024 command;
    xr_sprintf( command, "cl_votestart changegametype %s",
                gameTypes[ i ].c_str() );
    Console->Execute( command );
    HideDialog();
}
