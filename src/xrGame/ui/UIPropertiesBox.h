...
public:
    ...
    bool AddItem_script_tag( LPCSTR str, u32 tag_value ) {
        return AddItem( str, NULL, tag_value );
    };
    ...
    CUIListBoxItem* GetItemByIDX( int idx ) {
        return m_UIListWnd.GetItemByIDX( idx );
    }

    const xr_map< CUIListBoxItem*, CUIPropertiesBox* >& GetItemSubmenus()
        const {
        return m_item_submenus;
    }
    ...
    // Hover-expanding submenus, Lua-fed. Creates and owns a child box under
    // `label`, returns a borrowed pointer for Lua to populate (see AddSubmenu
    // in the .cpp).
