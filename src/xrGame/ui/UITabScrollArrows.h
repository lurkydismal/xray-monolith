...
// Scroll arrow for the tab strip: a tab-style button whose face is built from
// two mirrored halves of the strip tabs' own cap art.
...
public:
    ...
    float Width( int side ) const {
        return m_arrow[ side ] ? m_arrow[ side ]->GetWndSize().x : 0.0f;
    }
