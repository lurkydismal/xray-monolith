...
void CUILines::ParseText( bool force ) {
    if ( !force &&
         ( !uFlags.test( flComplexMode ) || !uFlags.test( flNeedReparse ) ) )
        return;
...
#define UBUFFER_SIZE 100
        ...
        if ( ( vsz > 1 ) && ( !bNewLines ) ) {
            ...
            for ( int i = 0; i < vsz; i++ ) {
                ...
                VERIFY( pszText );
                u16 nMarkers = m_pFont->SplitByWidth( aMarkers, UBUFFER_SIZE,
                                                      fTargetWidth, pszText );
        ...
        for ( u32 sbl_idx = 0; sbl_idx < sbl_cnt; ++sbl_idx ) {
            ...
            for ( u32 idx = 0; idx < sub_len; ++idx ) {
                ...
                if ( bOver || b_last_ch ) {
                    ...
                    strncpy_s( buff, sizeof( buff ),
                               sbl.m_text.c_str() + curr_w_pos,
                               idx - curr_w_pos + 1 );
...
LPCSTR GetElipsisText( CGameFont* pFont,
                       float width,
                       LPCSTR source_text,
                       LPSTR buff,
                       int buff_len ) {
    ...
    if ( !uFlags.is( flComplexMode ) ) {
        ...
        if ( uFlags.test( flPasswordMode ) ) {
            ...
            if ( uFlags.test( flEllipsis ) ) {
                ...
                LPCSTR str = GetElipsisText( m_pFont, m_wndSize.x,
                                             m_text.c_str(), p, buff_len );
...
u32 CUILines::GetColorFromText( const xr_string& str ) const {
    ...
    for ( CUIXmlInit::ColorDefs::const_iterator it =
              CUIXmlInit::GetColorDefs()->begin();
...
CUILine* CUILines::ParseTextToColoredLine( const xr_string& str ) {
    ...
    do {
        ...
        line->AddSubLine( entry,
                          subst_alpha( color, color_get_A( GetTextColor() ) ) );
    } while ( tmp.size() > 0 );

    return line;
}

void CUILines::CutFirstColoredTextEntry( xr_string& entry,
                                         u32& color,
