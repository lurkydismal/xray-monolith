...
void CUIStatsPlayerInfo::SetInfo( game_PlayerState* pInfo ) {
    ...
    if ( Level().CurrentViewEntity() &&
         Level().CurrentViewEntity()->ID() == pInfo->GameID )
...
void CUIStatsPlayerInfo::Update() {
    ...
    for ( u32 i = 0; i < m_fields.size(); i++ )
        m_fields[ i ]->TextItemControl()->SetText(
            GetInfoByID( *field_info[ i ].name ) );
...
void CUIStatsPlayerInfo::AddField( float len,
    ...
    if ( m_fields.empty() ) {
        ...
        wnd->SetWndPos(
            Fvector2().set( m_fields.back()->GetWndRect().right, 0.0f ) );
        wnd->SetWndSize( Fvector2().set( len, this->GetHeight() ) );
...
const char* CUIStatsPlayerInfo::GetInfoByID( const char* id ) {
    ...
    if ( 0 == xr_strcmp( id, "name" ) )
        ...
        if ( 0 == team )
            xr_sprintf( ans, "ui_hud_status_green_0%d",
                        ( int )m_pPlayerInfo->rank + 1 );
        else
            xr_sprintf( ans, "ui_hud_status_blue_0%d",
                        ( int )m_pPlayerInfo->rank + 1 );
        ...
        if ( m_pPlayerInfo->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) )
            ...
            game_cl_ArtefactHunt* pGameAHunt =
                smart_cast< game_cl_ArtefactHunt* >( &( Game() ) );
            R_ASSERT( pGameAHunt );
