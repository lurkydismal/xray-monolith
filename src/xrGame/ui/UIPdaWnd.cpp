...
CUIPdaWnd::CUIPdaWnd() {
    ...
    m_cursor_box.set( 117.f, 39.f, UI_BASE_WIDTH - 121.f,
                      UI_BASE_HEIGHT - 37.f );
...
void CUIPdaWnd::Init() {
    ...
    /*
            ...
            CUIXmlInit::InitAnimatedStatic(uiXml, "anim_static", 0,
       m_anim_static);
    */
...
void CUIPdaWnd::RebuildExtraTabs() {
    ...
    if ( ai().script_engine().functor( "pda_dynamic_tabs.build_extra_tabs",
                                       build_tabs ) )
...
bool CUIPdaWnd::OnMouseAction( float x, float y, EUIMessages mouse_action ) {
    switch ( mouse_action ) {
        ...
        case WINDOW_RBUTTON_UP: {
            CPda* pda =
                smart_cast< CPda* >( Actor()->inventory().ActiveItem() );
            if ( pda ) {
                if ( pda->IsPending() )
                    return true;
...
void CUIPdaWnd::MouseMovement( float x, float y ) {
    ...
    g_player_hud->m_bone_callback_params[ r_finger0 ]->m_target.set(
        y * .15f, y * -.05f, ( x * -.15f ) + ( buttonpressed ? .002f : 0.f ) );
    g_player_hud->m_bone_callback_params[ r_finger01 ]->m_target.set(
        0.f, 0.f, ( x * -.25f ) + ( buttonpressed ? .01f : 0.f ) );
    g_player_hud->m_bone_callback_params[ r_finger02 ]->m_target.set(
        0.f, 0.f, ( x * .75f ) + ( buttonpressed ? .025f : 0.f ) );
}
...
void CUIPdaWnd::Show( bool status ) {
    ...
    if ( status ) {
        ...
        if ( m_sActiveSection == NULL ||
             strcmp( m_sActiveSection.c_str(), "" ) == 0 ) {
        ...
        CurrentGameUI()->UIMainIngameWnd->SetFlashIconState_(
            CUIMainIngameWnd::efiPdaTask, false );
        if ( m_pActiveDialog ) {
            m_pActiveDialog->Show( false );
...
bool CUIPdaWnd::OnKeyboardAction( int dik, EUIMessages keyboard_action ) {
    if ( IsShown() && ( keyboard_action == WINDOW_KEY_PRESSED ||
                        keyboard_action == WINDOW_KEY_RELEASED ) ) {
        if ( !psActorFlags.test( AF_3D_PDA ) &&
             keyboard_action == WINDOW_KEY_PRESSED ) {
            EGameActions action = get_binded_action( dik );

            if ( action == kQUIT || action == kINVENTORY ||
                 action == kACTIVE_JOBS ) {
                HideDialog();
                return action == kQUIT;
            }
        ...
        return pda->Action(
            ...
            keyboard_action == WINDOW_KEY_PRESSED ? CMD_START : CMD_STOP );
    }
}
