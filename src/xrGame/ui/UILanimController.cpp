...
void CUIColorAnimConrollerContainer::ColorAnimationSetTextureColor(
    u32 color,
    bool only_alpha ) {
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( TO )
            TO->SetTextureColor(
                ( only_alpha ) ? subst_alpha( TO->GetTextureColor(), color )
                               : color );
    }
}

void CUIColorAnimConrollerContainer::ColorAnimationSetTextColor(
    u32 color,
    bool only_alpha ) {
    xrCriticalSectionGuard guard( csUi );
    WINDOW_LIST::iterator it = m_ChildWndList.begin();
