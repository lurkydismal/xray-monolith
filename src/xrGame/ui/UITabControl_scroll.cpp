...
// A zero-width tab is "parked": an invisible off-strip dispatch target excluded
// from strip geometry and scrolling.
...
bool CUITabControl::AddTab( LPCSTR id, LPCSTR caption, LPCSTR after_id ) {
    ...
    if ( !first_tab ) {
        Msg( "! [CUITabControl] AddTab(%s): control has no tabs to derive "
             "geometry from",
             id );
        return false;
    }
    ...
    if ( after_id && xr_strlen( after_id ) ) {
        ...
        for ( ; i < m_TabsArr.size(); ++i ) {
            if ( m_TabsArr[ i ]->m_btn_id == anchor &&
                 !TabParked( m_TabsArr[ i ] ) ) {
                at = i + 1;
                break;
            }
        }
        if ( i == m_TabsArr.size() )
            Msg( "! [CUITabControl] AddTab(%s): anchor tab [%s] not found, "
                 "appending",
                 id, after_id );
    }
...
void CUITabControl::RecalcScroll() {
    // Two independent left-to-right layouts. content_w spans ALL strip tabs
    // (what must fit); strip_w spans the STATIC tabs alone (the viewport): the
    // legacy PDA layout ships a wrong control width, so the vanilla static tabs
    // define how wide the visible strip is, and the dynamic (AddTab) tabs
    // overflow it and scroll. GetWidth() is only a floor.
    m_content_w = 0.0f;
    m_strip_w = GetWidth();
    ...
    m_view_left = m_arrows->Width( CUITabScrollArrows::eLeft ) -
                  ( first_tab ? Tuck( first_tab ) : 0.0f );
    m_view_right = m_strip_w - m_arrows->Width( CUITabScrollArrows::eRight );

    m_arrows->Layout( m_view_right,
                      first_tab ? first_tab->GetWndPos().y : 0.0f );
...
float CUITabControl::MaxScroll() const {
    ...
    const float max_scroll =
        m_content_w + m_view_left - m_view_right - Tuck( first );
    ...
    for ( u32 i = 0; i < m_TabsArr.size(); ++i ) {
        if ( m_TabsArr[ i ]->m_btn_id == id ) {
            ...
            if ( layout_x - scroll < 0.0f )
                ...
                float need = layout_x + m_view_left + tab_w - m_view_right -
                             Tuck( m_TabsArr[ i ] );
                if ( scroll < need )
                    scroll = need;
            }
...
// Slide a tab's hit parallelogram (BL,BR,TR,TL) inside the viewport walls
// left/right, gauged on its bottom edge (BL,BR); the top vertices follow, not a
// vertical clamp: each slanted side moves as a unit
static void SlidePolyIntoBand( Fvector2* poly, float left, float right ) {
    if ( poly[ 0 ].x < left ) {
        const float d = left - poly[ 0 ].x;
        poly[ 0 ].x += d;
        poly[ 3 ].x += d;
    } else if ( poly[ 0 ].x > right ) {
        const float d = right - poly[ 0 ].x;
        poly[ 0 ].x += d;
        poly[ 3 ].x += d;
    }
    if ( poly[ 1 ].x > right ) {
        const float d = right - poly[ 1 ].x;
        poly[ 1 ].x += d;
        poly[ 2 ].x += d;
    } else if ( poly[ 1 ].x < left ) {
        const float d = left - poly[ 1 ].x;
        poly[ 1 ].x += d;
        poly[ 2 ].x += d;
    }
}
...
bool CUITabControl::OnMouseAction( float x,
                                   float y,
