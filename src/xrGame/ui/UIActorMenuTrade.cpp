...
void CUIActorMenu::InitPartnerInventoryContents() {
    ...
    m_trade_partner_inventory_state =
        m_pPartnerInvOwner->inventory().ModifyFrame();
}
...
bool CUIActorMenu::ToActorTrade( CUICellItem* itm, bool b_use_cursor_pos ) {
    ...
    //	if(m_pActorInvOwner->inventory().CanPutInRuck(iitem))
    {
        ...
        bool result = ( old_owner_type != iActorBag )
...
bool CUIActorMenu::ToPartnerTrade( CUICellItem* itm, bool b_use_cursor_pos ) {
    ...
    if ( !m_pPartnerInvOwner->AllowItemToTrade( iitem, pl ) ) {
        /// R_ASSERT2( 0, make_string( "Partner can`t cell item (%s)",
        /// iitem->NameItem() ) );
...
bool CUIActorMenu::ToPartnerTradeBag( CUICellItem* itm,
...
u32 CUIActorMenu::CalcItemsPrice( CUIDragDropListEx* pList,
                                  CTrade* pTrade,
                                  bool bBuying ) {
    u32 res = 0;
    for ( u32 i = 0; i < pList->ItemsCount(); ++i ) {
...
bool CUIActorMenu::CanMoveToPartner( PIItem pItem ) {
    ...
    bool has_max_uses =
        pItem->cast_eatable_item() && pItem->cast_eatable_item()->GetMaxUses();
    if ( !has_max_uses &&
         ( pItem->GetCondition() <
           m_pPartnerInvOwner->trade_parameters().buy_item_condition_factor ) )
        return false;
    ...
    float partner_inv_weight =
        m_pPartnerInvOwner->inventory().CalcTotalWeight();
...
void CUIActorMenu::UpdateActor() {
    ...
    InventoryUtilities::UpdateWeightStr( *m_ActorWeight, *m_ActorWeightMax,
                                         m_pActorInvOwner );
    ...
    pos.x =
        m_ActorWeightMax->GetWndPos().x - m_ActorWeight->GetWndSize().x - 5.0f;
...
void CUIActorMenu::UpdatePrices() {
    ...
    UpdatePartnerBag();
    u32 actor_price =
        CalcItemsPrice( m_pTradeActorList, m_partner_trade, true );
    u32 partner_price =
        CalcItemsPrice( m_pTradePartnerList, m_partner_trade, false );
    ...
    pos.x = m_ActorTradeWeightMax->GetWndPos().x -
            m_ActorTradePrice->GetWndSize().x - 5.0f;
    ...
    pos.x = m_PartnerTradeWeightMax->GetWndPos().x -
            m_PartnerTradePrice->GetWndSize().x - 5.0f;
...
void CUIActorMenu::OnBtnPerformTradeBuy( CUIWindow* w, void* d ) {
    ...
    int actor_price =
        0; //(int)CalcItemsPrice( m_pTradeActorList,   m_partner_trade, true  );
    int partner_price =
        ( int )CalcItemsPrice( m_pTradePartnerList, m_partner_trade, false );

    int delta_price = actor_price - partner_price;
    actor_money += delta_price;
    partner_money -= delta_price;

    if ( ( actor_money >= 0 ) /*&& ( partner_money >= 0 )*/ &&
         ( actor_price >= 0 || partner_price > 0 ) ) {
...
void CUIActorMenu::OnBtnPerformTradeSell( CUIWindow* w, void* d ) {
    ...
    int partner_money = ( int )m_pPartnerInvOwner->get_money();
    int actor_price =
        ( int )CalcItemsPrice( m_pTradeActorList, m_partner_trade, true );
    int partner_price =
        0; //(int)CalcItemsPrice( m_pTradePartnerList, m_partner_trade, false );
...
void CUIActorMenu::TransferItems( CUIDragDropListEx* pSellList,
                                  CUIDragDropListEx* pBuyList,
                                  CTrade* pTrade,
                                  bool bBuying ) {
    while ( pSellList->ItemsCount() ) {
        CUICellItem* cell_item =
            pSellList->RemoveItem( pSellList->GetItemIdx( 0 ), false );
        ...
        if ( bBuying ) {
            SInvItemPlace pl;
            pl.type = eItemPlaceRuck;
            if ( pTrade->pThis.inv_owner->CInventoryOwner::AllowItemToTrade(
                     item, pl ) ) {
    ...
    pTrade->pThis.inv_owner->set_money( pTrade->pThis.inv_owner->get_money(),
                                        true );
    pTrade->pPartner.inv_owner->set_money(
        pTrade->pPartner.inv_owner->get_money(), true );
}
...
void CUIActorMenu::DonateCurrentItem( CUICellItem* cell_item ) {
    ...
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_DonateCurrentItem", funct ) ) {
        if ( funct( m_pPartnerInvOwner->cast_game_object()->lua_game_object(),
                    item->object().lua_game_object() ) == false )
            return;
    }
    //-Alundaio
