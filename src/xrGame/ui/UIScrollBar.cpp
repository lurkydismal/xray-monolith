...
void CUIScrollBar::InitScrollBar( Fvector2 pos,
                                  float length,
                                  bool bIsHorizontal,
                                  LPCSTR profile ) {
    string256 _path;
    ...
    float height = xml_doc.ReadAttribFlt(
        profile, 0, ( bIsHorizontal ) ? "height" : "height_v" );
    R_ASSERT( height > 0 );
    ...
    if ( m_bIsHorizontal ) {
        ...
        m_IncButton->SetWndPos(
            Fvector2().set( length - m_IncButton->GetWidth(), 0.0f ) );
        ...
        CUIXmlInit::Init3tButton( xml_doc, _path, 0, m_IncButton );
        m_IncButton->SetWndPos(
            Fvector2().set( 0.0f, length - m_IncButton->GetHeight() ) );
...
void CUIScrollBar::SetWidth( float width ) {
    ...
    if ( m_bIsHorizontal ) {
        float work_area =
            float( width ) - m_DecButton->GetWidth() - m_IncButton->GetWidth();
...
void CUIScrollBar::SetHeight( float height ) {
    ...
    if ( !m_bIsHorizontal ) {
        float work_area = float( height ) - m_DecButton->GetHeight() -
                          m_IncButton->GetHeight();
        m_ScrollWorkArea = work_area < 0.f ? 0 : int( work_area );
    }
    UpdateScrollBar();
}
    ...
    if ( IsShown() ) {
        ...
        if ( IsRelevant() ) {
            if ( m_bIsHorizontal ) {
                ...
                // set pos
                int pos = PosViewFromScroll( iFloor( m_ScrollBox->GetWidth() ),
                                             iFloor( GetHeight() ) );
                ...
                // set pos
                int pos = PosViewFromScroll( iFloor( m_ScrollBox->GetHeight() ),
                                             iFloor( GetWidth() ) );
                m_ScrollBox->SetWndPos( Fvector2().set(
...
bool CUIScrollBar::OnKeyboardHold( int dik ) {
    if ( dik == MOUSE_1 &&
         ( last_hold_time + m_hold_delay ) < Device.dwTimeContinual ) // 100
    {
        if ( OnMouseDownEx() ) {
...
void CUIScrollBar::ClampByViewRect() {
    if ( m_bIsHorizontal ) {
        if ( m_ScrollBox->GetWndRect().left <= m_DecButton->GetWidth() )
            m_ScrollBox->SetWndPos( Fvector2().set(
                m_DecButton->GetWidth(), m_ScrollBox->GetWndRect().top ) );
        else if ( m_ScrollBox->GetWndRect().right >=
                  m_IncButton->GetWndPos().x )
            m_ScrollBox->SetWndPos( Fvector2().set(
...
void CUIScrollBar::SetPosScrollFromView( float view_pos,
                                         float view_size,
                                         float view_offs ) {
...
int CUIScrollBar::PosViewFromScroll( int view_size, int view_offs ) {
    ...
    int scroll_size = ScrollSize();
    return scroll_size ? ( m_iScrollPos * work_size + scroll_size * view_offs -
                           m_iMinPos * work_size ) /
                             scroll_size
                       : 0;
}
...
void CUIScrollBar::Draw() {
    if ( m_bIsHorizontal ) {
        float size =
            GetWidth() - m_DecButton->GetWidth() - m_IncButton->GetWidth();

        m_FrameBackground->SetWndSize( Fvector2().set( size, GetHeight() ) );
        m_FrameBackground->SetWndPos(
            Fvector2().set( m_DecButton->GetWidth(), 0.0f ) );
    } else {
        float size =
        ...
        m_FrameBackground->SetWndPos(
            Fvector2().set( 0.0f, m_DecButton->GetHeight() ) );
    }
    inherited::Draw();
}
