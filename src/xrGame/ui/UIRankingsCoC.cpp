...
void CUIRankingsCoC::init_from_xml( CUIXml& xml, u8 index, bool bUnique ) {
    string128 tmp;
    xr_sprintf( tmp, sizeof( tmp ), "%s",
                bUnique ? "coc_ranking_itm_actor" : "coc_ranking_itm" );
...
bool CUIRankingsCoC::ParentHasMe() {
    WINDOW_LIST::const_iterator it =
        std::find( m_parent->Items().begin(), m_parent->Items().end(), this );
