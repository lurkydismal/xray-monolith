...
void draw_rect( Frect& r, u32 color ) {
    ...
    UIRender->StartPrimitive( 5, IUIRender::ptLineStrip,
...
CUIWindow::~CUIWindow() {
    ...
    if ( !bOK )
        Msg( "CUIWindow::~CUIWindow.[%d] cannot find window in list",
             m_dbg_id );
#endif
}
...
void CUIWindow::Update() {
    ...
    CUIDialogWnd* TIR =
        CurrentGameUI() ? CurrentGameUI()->TopInputReceiver() : nullptr;
...
void CUIWindow::AttachChild( CUIWindow* pChild ) {
    ...
    if ( IsChild( pChild ) ) {
        Msg( "!CUIWindow::AttachChild, %s child element is already attached to "
             "%s",
             pChild->WindowName_script(), WindowName_script() );
        return;
    }
...
void CUIWindow::DetachChild( CUIWindow* pChild ) {
...

    {
        ...
        WINDOW_LIST_it it =
            std::find( m_ChildWndList.begin(), m_ChildWndList.end(), pChild );
    ...
    if ( pChild->IsAutoDelete() )
        if ( std::find( m_ChildWndToDelete.begin(), m_ChildWndToDelete.end(),
                        pChild ) == m_ChildWndToDelete.end() )
...
bool CUIWindow::OnMouseAction( float x, float y, EUIMessages mouse_action ) {
    ...
    if ( m_pMouseCapturer ) {
        m_pMouseCapturer->OnMouseAction(
            cursor_pos.x - m_pMouseCapturer->GetWndRect().left,
    ...
    for ( ; it != m_ChildWndList.rend(); ++it ) {
        ...
        if ( wndRect.in( cursor_pos ) &&
             w->HitClipPass( GetUICursor().GetCursorPosition() ) ) {
            if ( w->IsEnabled() ) {
                if ( w->OnMouseAction( cursor_pos.x - w->GetWndRect().left,
                                       cursor_pos.y - w->GetWndRect().top,
                                       mouse_action ) )
                    return true;
            }
...
void CUIWindow::SetKeyboardCapture( CUIWindow* pChildWindow,
                                    bool capture_status ) {
    if ( NULL != GetParent() )
    ...
    if ( capture_status ) {
        ...
        if ( NULL != m_pKeyboardCapturer )
            m_pKeyboardCapturer->SendMessage( this,
...
bool CUIWindow::IsChild( CUIWindow* pPossibleChild ) const {
    ...
    WINDOW_LIST::const_iterator it = std::find(
        m_ChildWndList.begin(), m_ChildWndList.end(), pPossibleChild );
...
void CUIWindow::SetParent( CUIWindow* pNewParent ) {
    ...
    if ( m_pParentWnd && m_pParentWnd->IsChild( this ) ) {
        Msg( "!CUIWindow::SetParent, %s element is already attached to %s",
             WindowName_script(), m_pParentWnd->WindowName_script() );
        return;
    }
...
{
    return ( a.x1 < b.x1 ) && ( a.x2 > b.x2 ) && ( a.y1 < b.y1 ) &&
           ( a.y2 > b.y2 );
}

bool fit_in_rect( CUIWindow* w,
                  Frect const& vis_rect,
                  float border,
                  float dx16pos ) // this = hint wnd
{
    ...
    Frect rect;
    rect.set( -border, -border, w->GetWidth() - 2.0f * border,
              w->GetHeight() - 2.0f * border );
    ...
    float yn =
        rect.top - vis_rect.height() + rect.height() - border + cursor_height;
    if ( !is_in( vis_rect, rect ) ) {
        rect.sub( 0.0f, yn );
    }
    if ( !is_in( vis_rect, rect ) ) {
                   ...
                   float dx16pos ) // this = hint wnd
{
    ...
    bool const cursor_is_left_of_center =
        cursor_x_in_vis_rect < ( vis_rect.width() / 2 ) - cursor_size;
    ...
    bool const cursor_is_top_of_center =
        cursor_y_in_vis_rect < ( vis_rect.height() / 2 ) - cursor_size;
    ...
    if ( rect.top < vis_rect.top ) {
        // hint window sticks out of parent at the top, correct that by moving
        // it down
    ...
    if ( rect.bottom > vis_rect.bottom ) {
        // hint window sticks out of parent at the bottom, correct that by
        // moving it up
        float const y_correction = vis_rect.bottom - rect.bottom;
        rect.add( 0.0f, y_correction );
    }
    // TODO: #1 if hint window taller than parent, consider increasing width by
    // const step and the looping the logic until hint_window_width >=
    // vis_rect.width
