...
void CUIOptionsManagerScript::SendMessage2Group( const char* group,
                                                 const char* message ) {
...
LPCSTR CUIOptionsManagerScript::GetRendererName( int index ) {
    for ( int i = 0; vid_quality_token[ i ].name; i++ ) {
        if ( i == index )
            return vid_quality_token[ i ].name;
    }
    return "";
}
...
void CUIOptionsManagerScript::script_register( lua_State* L ) {
    module( L )[ class_< CUIOptionsManagerScript >( "COptionsManager" )
                     .def( constructor<>() )
                     .def( "SaveBackupValues",
                           &CUIOptionsManagerScript::SaveBackupValues )
                     .def( "SetCurrentValues",
                           &CUIOptionsManagerScript::SetCurrentValues )
                     .def( "SaveValues", &CUIOptionsManagerScript::SaveValues )
                     .def( "UndoGroup", &CUIOptionsManagerScript::UndoGroup )
                     .def( "OptionsPostAccept",
                           &CUIOptionsManagerScript::OptionsPostAccept )
                     .def( "SendMessage2Group",
                           &CUIOptionsManagerScript::SendMessage2Group )
                     .def( "NeedSystemRestart",
                           &CUIOptionsManagerScript::NeedSystemRestart )
                     .def( "NeedVidRestart",
                           &CUIOptionsManagerScript::NeedVidRestart )
                     .def( "GetRendererName",
                           &CUIOptionsManagerScript::GetRendererName ) ];
}
