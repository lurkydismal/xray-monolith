...
void CallFunction( shared_str const& func ) {
    ...
    bool functor_exists =
        ai().script_engine().functor( func.c_str(), functor_to_call );
    THROW3( functor_exists,
            "Cannot find script function described in tutorial item ",
            func.c_str() );
...
void CUISequenceItem::Load( CUIXml* xml, int idx ) {
    ...
    for ( j = 0; j < f_num; ++j )
        m_start_lua_functions[ j ] =
            xml->Read( xml->GetLocalRoot(), "function_on_start", j, NULL );

    f_num = xml->GetNodesNum( xml->GetLocalRoot(), "function_on_stop" );
    m_stop_lua_functions.resize( f_num );
    for ( j = 0; j < f_num; ++j )
        m_stop_lua_functions[ j ] =
            xml->Read( xml->GetLocalRoot(), "function_on_stop", j, NULL );

    m_check_lua_function =
        xml->Read( xml->GetLocalRoot(), "function_check_start", 0, NULL );
    m_onframe_lua_function =
        xml->Read( xml->GetLocalRoot(), "function_on_frame", 0, NULL );
...
bool CUISequenceItem::AllowKey( int dik ) {
    xr_vector< int >::iterator it =
        std::find( m_disabled_actions.begin(), m_disabled_actions.end(),
...
void CUISequenceItem::Start() {
    ...
    if ( m_onframe_lua_function.size() ) {
        bool functor_exists = ai().script_engine().functor(
            m_onframe_lua_function.c_str(), m_onframe_functor );
        THROW3( functor_exists,
                "Cannot find script function described in tutorial item ",
...
void CUISequencer::Start( LPCSTR tutor_name ) {
    ...
    if ( m_flags.test( etsNeedPauseOn ) &&
         !m_flags.test( etsStoredPauseState ) ) {
        Device.Pause( TRUE, TRUE, TRUE, "tutorial_start" );
        bShowPauseString = FALSE;
    }

    if ( m_flags.test( etsNeedPauseOff ) &&
         m_flags.test( etsStoredPauseState ) )
...
CUISequenceItem* CUISequencer::GetNextItem() {
    ...
    while ( m_sequencer_items.size() ) {
        ...
        bool functor_exists =
            ai().script_engine().functor( f.c_str(), functor_to_call );
        THROW3( functor_exists,
                "Cannot find script function described in tutorial item ",
                f.c_str() );
...
void CUISequencer::IR_OnKeyboardPress( int dik ) {
    ...
    if ( binded && CurrentGameUI() ) {
        ...
        if ( CurrentGameUI()->TopInputReceiver() &&
             CurrentGameUI()->TopInputReceiver()->IsShown() ) {
            CurrentGameUI()->TopInputReceiver()->OnKeyboardAction(
                dik, WINDOW_KEY_PRESSED );
            return;
        }
