...
CUIButton::CUIButton() {
    ...
    TextItemControl()->SetTextAlignment(
        CGameFont::alCenter ); // this will create class instance for m_pLines
...
void CUIButton::DrawTexture() {
    ...
    if ( m_bTextureEnable && GetShader() && GetShader()->inited() ) {
        ...
        if ( GetStretchTexture() )
            ...
            m_UIStaticItem.SetSize(
                Fvector2().set( m_UIStaticItem.GetTextureRect().width(),
...
bool is_in2( const Frect& b1, const Frect& b2 ) {
    return ( b1.x1 < b2.x1 ) && ( b1.x2 > b2.x2 ) && ( b1.y1 < b2.y1 ) &&
           ( b1.y2 > b2.y2 );
}
...
bool CUIButton::IsAccelerator( int iAccel ) const {
    ...
    if ( !res ) {
        res = ( ( m_uAccelerator[ 2 ] != -1 )
                    ? is_binded( ( EGameActions )GetAccelerator( 2 ), iAccel )
                    : false ) ||
              ( ( m_uAccelerator[ 3 ] != -1 )
                    ? is_binded( ( EGameActions )GetAccelerator( 3 ), iAccel )
                    : false );
    }
    return res;
}
