...
Frect CUIXmlInit::GetFRect( CUIXml& xml_doc, LPCSTR path, int index ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
               path, xml_doc.m_xml_file_name );
    Frect rect;
    rect.set( 0, 0, 0, 0 );
...
bool CUIXmlInit::InitWindow( CUIXml& xml_doc,
                             LPCSTR path,
                             int index,
                             CUIWindow* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
...
bool CUIXmlInit::InitFrameWindow( CUIXml& xml_doc,
                                  LPCSTR path,
                                  int index,
                                  CUIFrameWindow* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
...
bool CUIXmlInit::InitOptionsItem( CUIXml& xml_doc,
                                  LPCSTR path,
                                  int index,
                                  CUIOptionsItem* pWnd ) {
    string256 buf;
    ...
    if ( xml_doc.NavigateToNode( buf, index ) ) {
        ...
        if ( depends ) {
            ...
            if ( 0 == stricmp( depends, "vid" ) )
                ...
                Msg( "! unknown param [%s] in optionsItem [%s]", depends,
                     entry.c_str() );
...
bool CUIXmlInit::InitStatic( CUIXml& xml_doc,
                             LPCSTR path,
                             int index,
                             CUIStatic* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
    ...
    string256 buf;
    InitText( xml_doc, strconcat( sizeof( buf ), buf, path, ":text" ), index,
              pWnd );
    InitTexture( xml_doc, path, index, pWnd );
    InitTextureOffset( xml_doc, path, index, pWnd );

    bool cover_flag =
        xml_doc.ReadAttribInt( path, index, "cover", 0 ) ? true : false;
    ...
    float heading_angle =
        xml_doc.ReadAttribFlt( path, index, "heading_angle", 0.0f );
    ...
    bool bComplexMode =
        xml_doc.ReadAttribInt( path, index, "complex_mode", 0 ) ? true : false;
    if ( bComplexMode )
...
bool CUIXmlInit::InitTextWnd( CUIXml& xml_doc,
                              LPCSTR path,
                              int index,
                              CUITextWnd* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
    ...
    R_ASSERT3( NULL == xml_doc.NavigateToNode( buf, index ),
               xml_doc.m_xml_file_name, buf );

    R_ASSERT( pWnd->GetChildNum() == 0 );
    return true;
}

bool CUIXmlInit::InitCheck( CUIXml& xml_doc,
                            LPCSTR path,
                            int index,
                            CUICheckButton* pWnd ) {
...
bool CUIXmlInit::InitSpin( CUIXml& xml_doc,
                           LPCSTR path,
                           int index,
                           CUICustomSpin* pWnd ) {
    InitWindow( xml_doc, path, index, pWnd );
...
bool CUIXmlInit::InitText( CUIXml& xml_doc,
                           LPCSTR path,
                           int index,
                           CUIStatic* pWnd ) {
...
bool CUIXmlInit::InitText( CUIXml& xml_doc,
    ...
    pLines->SetTextComplexMode(
        xml_doc.ReadAttribInt( path, index, "complex_mode", 0 ) ? true
                                                                : false );
...
bool CUIXmlInit::Init3tButton( CUIXml& xml_doc,
                               LPCSTR path,
                               int index,
                               CUI3tButton* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
    ...
    pWnd->m_frameline_mode = ( CUI3tButton::EFrameMode )xml_doc.ReadAttribInt(
        path, index, "frame_mode", 0 );

    pWnd->vertical =
        ( xml_doc.ReadAttribInt( path, index, "vertical", 0 ) == 1 ) ? true
                                                                     : false;
...
bool CUIXmlInit::InitTabButtonMP( CUIXml& xml_doc,
                                  LPCSTR path,
                                  int index,
                                  CUITabButtonMP* pWnd ) {
    ...
    if ( xml_doc.NavigateToNode( buff, index ) ) {
        pWnd->m_text_ident_cursor_over.x =
            xml_doc.ReadAttribFlt( buff, index, "over_x", 0 );
        pWnd->m_text_ident_cursor_over.y =
            xml_doc.ReadAttribFlt( buff, index, "over_y", 0 );

        pWnd->m_text_ident_normal.x =
            xml_doc.ReadAttribFlt( buff, index, "normal_x", 0 );
        pWnd->m_text_ident_normal.y =
            xml_doc.ReadAttribFlt( buff, index, "normal_y", 0 );
    }
...
bool CUIXmlInit::InitSound( CUIXml& xml_doc,
                            LPCSTR path,
                            int index,
                            CUI3tButton* pWnd ) {
    string256 sound_h;
...
bool CUIXmlInit::InitDragDropListEx( CUIXml& xml_doc,
                                     LPCSTR path,
                                     int index,
                                     CUIDragDropListEx* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
               path, xml_doc.m_xml_file_name );
    ...
    if ( tmp != 0 ) {
        xr_string vc_vert_align =
            xml_doc.ReadAttrib( path, index, "vc_vert_align", "" );
        pWnd->SetCellsVertAlignment( vc_vert_align );
        xr_string vc_horiz_align =
            xml_doc.ReadAttrib( path, index, "vc_horiz_align", "" );
...
bool CUIXmlInit::InitProgressBar( CUIXml& xml_doc,
                                  LPCSTR path,
                                  int index,
                                  CUIProgressBar* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
...
bool CUIXmlInit::InitProgressShape( CUIXml& xml_doc,
                                    LPCSTR path,
                                    int index,
                                    CUIProgressShape* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
               path, xml_doc.m_xml_file_name );
    ...
    if ( xml_doc.NavigateToNode(
             strconcat( sizeof( _path ), _path, path, ":back" ), index ) ) {
        R_ASSERT2( 0, "unused <back> node in progress shape " );
    }

    if ( xml_doc.NavigateToNode(
             strconcat( sizeof( _path ), _path, path, ":front" ), index ) ) {
        R_ASSERT2( 0, "unused <front> node in progress shape " );
    }
    //    InitStatic(xml_doc, strconcat(sizeof(_path),_path, path, ":front"),
    //    index, pWnd->m_pTexture);

    pWnd->m_sectorCount =
        xml_doc.ReadAttribInt( path, index, "sector_count", 8 );
    pWnd->m_bClockwise =
        xml_doc.ReadAttribInt( path, index, "clockwise" ) ? true : false;

    pWnd->m_blend = ( xml_doc.ReadAttribInt( path, index, "blend", 1 ) == 1 )
                        ? true
                        : false;
    pWnd->m_angle_begin =
        xml_doc.ReadAttribFlt( path, index, "begin_angle", 0.0f );
    pWnd->m_angle_end =
        xml_doc.ReadAttribFlt( path, index, "end_angle", PI_MUL_2 );

    return true;
}

void CUIXmlInit::InitAutoStaticGroup( CUIXml& xml_doc,
                                      LPCSTR path,
                                      int index,
                                      CUIWindow* pParentWnd ) {
    ...
    while ( node ) {
        ...
        if ( 0 == stricmp( node_name, "auto_static" ) ) {
            ...
            InitFrameLine( xml_doc, "auto_frameline", cnt_frameline,
                           pUIFrameline );
            ...
            InitFrameWindow( xml_doc, "auto_framewindow", cnt_framewindow,
                             pUIFramewindow );
    ...
    /*
            ...
            {
                    pUIStatic						=
       xr_new<CUIStatic>(); InitStatic
       (xml_doc, "auto_static", i, pUIStatic); xr_sprintf
       (sname,"auto_static_%d", i); pUIStatic->SetWindowName		(sname);
                    pUIStatic->SetAutoDelete		(true);
                    pParentWnd->AttachChild			(pUIStatic);
                    pUIStatic						= NULL;
            }
...
void CUIXmlInit::InitAutoFrameLineGroup( CUIXml& xml_doc,
                                         LPCSTR path,
                                         int index,
                                         CUIWindow* pParentWnd ) {
...
bool CUIXmlInit::InitFont( CUIXml& xml_doc,
                           LPCSTR path,
                           int index,
                           u32& color,
                           CGameFont*& pFnt ) {
...
bool CUIXmlInit::InitTabControl( CUIXml& xml_doc,
                                 LPCSTR path,
                                 int index,
                                 CUITabControl* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
    ...
    for ( int i = 0; i < tabsCount; ++i ) {
        newButton =
            radio ? xr_new< CUIRadioButton >() : xr_new< CUITabButton >();
...
bool CUIXmlInit::InitFrameLine( CUIXml& xml_doc,
                                LPCSTR path,
                                int index,
                                CUIFrameLineWnd* pWnd ) {
    R_ASSERT3( xml_doc.NavigateToNode( path, index ), "XML node not found",
               path );

    string256 buf;

    bool stretch_flag =
        xml_doc.ReadAttribInt( path, index, "stretch" ) ? true : false;
...
bool CUIXmlInit::InitCustomEdit( CUIXml& xml_doc,
                                 LPCSTR path,
                                 int index,
                                 CUICustomEdit* pWnd ) {
    InitStatic( xml_doc, path, index, pWnd );
    ...
    bool num_only =
        ( xml_doc.ReadAttribInt( path, index, "num_only", 0 ) == 1 );
    bool read_only =
        ( xml_doc.ReadAttribInt( path, index, "read_only", 0 ) == 1 );
    bool file_name_mode =
        ( xml_doc.ReadAttribInt( path, index, "file_name_mode", 0 ) == 1 );
...
bool CUIXmlInit::InitEditBox( CUIXml& xml_doc,
                              LPCSTR path,
                              int index,
                              CUIEditBox* pWnd ) {
...
bool CUIXmlInit::InitAnimatedStatic( CUIXml& xml_doc,
                                     const char* path,
                                     int index,
                                     CUIAnimatedStatic* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
               path, xml_doc.m_xml_file_name );
    ...
    u32 framesCount =
        static_cast< u32 >( xml_doc.ReadAttribInt( path, index, "frames", 0 ) );
    u32 animDuration = static_cast< u32 >(
        xml_doc.ReadAttribInt( path, index, "duration", 0 ) );
    u32 animCols = static_cast< u32 >(
        xml_doc.ReadAttribInt( path, index, "columns", 0 ) );
...
bool CUIXmlInit::InitSleepStatic( CUIXml& xml_doc,
                                  const char* path,
                                  int index,
                                  CUISleepStatic* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
...
bool CUIXmlInit::InitTexture( CUIXml& xml_doc,
                              LPCSTR path,
                              int index,
                              ITextureOwner* pWnd ) {
    string256 buf;
...
bool CUIXmlInit::InitTextureOffset( CUIXml& xml_doc,
                                    LPCSTR path,
                                    int index,
                                    CUIStatic* pWnd ) {
    ...
    if ( 0 == xr_strcmp( path, "" ) )
        ...
        strconcat( sizeof( textureOffset ), textureOffset, path,
                   ":texture_offset" );
...
bool CUIXmlInit::InitMultiTexture( CUIXml& xml_doc,
                                   LPCSTR path,
                                   int index,
                                   CUI3tButton* pWnd ) {
    string256 buff;
    ...
    if ( texture.size() ) {
        if ( pWnd->m_background ) {
            ...
            pWnd->m_back_frameline->Get( S_Enabled )
                ->SetHorizontal( !( pWnd->vertical ) );
    ...
    if ( texture.size() ) {
        if ( pWnd->m_background ) {
            ...
            pWnd->m_back_frameline->Get( S_Touched )
                ->SetHorizontal( !( pWnd->vertical ) );
        } else if ( pWnd->m_back_framewindow ) {
    ...
    if ( texture.size() ) {
        if ( pWnd->m_background ) {
            ...
            pWnd->m_back_frameline->Get( S_Disabled )
                ->SetHorizontal( !( pWnd->vertical ) );
        } else if ( pWnd->m_back_framewindow ) {
    ...
    if ( texture.size() ) {
        if ( pWnd->m_background ) {
            ...
            pWnd->m_back_frameline->Get( S_Highlighted )
                ->SetHorizontal( !( pWnd->vertical ) );
        } else if ( pWnd->m_back_framewindow ) {
            pWnd->m_back_framewindow->InitState( S_Highlighted,
                                                 texture.c_str() );
        }
        success = true;
    }
...
bool CUIXmlInit::InitAlignment( CUIXml& xml_doc,
                                const char* path,
                                int index,
                                float& x,
                                float& y,
                                CUIWindow* pWnd ) {
    xr_string wnd_alignment =
        xml_doc.ReadAttrib( path, index, "alignment", "" );
...
bool CUIXmlInit::InitScrollView( CUIXml& xml_doc,
                                 LPCSTR path,
                                 int index,
                                 CUIScrollView* pWnd ) {
    R_ASSERT4( xml_doc.NavigateToNode( path, index ), "XML node not found",
    ...
    pWnd->SetRightIndention(
        xml_doc.ReadAttribFlt( path, index, "right_ident", 0.0f ) );
    pWnd->SetLeftIndention(
        xml_doc.ReadAttribFlt( path, index, "left_ident", 0.0f ) );
    pWnd->SetUpIndention(
        xml_doc.ReadAttribFlt( path, index, "top_indent", 0.0f ) );
    pWnd->SetDownIndention(
        xml_doc.ReadAttribFlt( path, index, "bottom_indent", 0.0f ) );

    float vi = xml_doc.ReadAttribFlt( path, index, "vert_interval", 0.0f );
    pWnd->m_vertInterval = ( vi );

    bool bInverseDir =
        ( 1 == xml_doc.ReadAttribInt( path, index, "inverse_dir", 0 ) );
    pWnd->m_flags.set( CUIScrollView::eInverseDir, bInverseDir );

    pWnd->SetScrollBarProfile(
        xml_doc.ReadAttrib( path, index, "scroll_profile", "default" ) );

    pWnd->InitScrollView();

    bool bVertFlip =
        ( 1 == xml_doc.ReadAttribInt( path, index, "flip_vert", 0 ) );
    pWnd->SetVertFlip( bVertFlip );

    bool b =
        ( 1 == xml_doc.ReadAttribInt( path, index, "always_show_scroll", 1 ) );
...
bool CUIXmlInit::InitListBox( CUIXml& xml_doc,
                              LPCSTR path,
                              int index,
                              CUIListBox* pWnd ) {
    ...
    pWnd->bComplexMode =
        xml_doc.ReadAttribInt( "properties_box:list", 0, "complex_mode", 0 );
    pWnd->bLineWrap =
        xml_doc.ReadAttribInt( "properties_box:list", 0, "line_wrap", 1 );

    return true;
}

bool CUIXmlInit::InitTrackBar( CUIXml& xml_doc,
                               LPCSTR path,
                               int index,
                               CUITrackBar* pWnd ) {
...
bool CUIXmlInit::InitComboBox( CUIXml& xml_doc,
                               LPCSTR path,
                               int index,
                               CUIComboBox* pWnd ) {
    u32 color;
    ...
    pWnd->SetListLength(
        xml_doc.ReadAttribInt( path, index, "list_length", 4 ) );
...
u32 CUIXmlInit::GetColor( CUIXml& xml_doc,
                          LPCSTR path,
                          int index,
                          u32 def_clr ) {
