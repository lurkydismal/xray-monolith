...
void CUIMMShniaga::InitShniaga( CUIXml& xml_doc, LPCSTR path ) {
    ...
    if ( !g_pGameLevel || !g_pGameLevel->bReady ) {
        if ( !*g_last_saved_game ||
             !CSavedGameWrapper::valid_saved_game( g_last_saved_game ) )
...
void CUIMMShniaga::CreateList( xr_vector< CUITextWnd* >& lst,
                               CUIXml& xml_doc,
                               LPCSTR path ) {
    CGameFont* pF;
    u32 color;
    ...
    for ( int i = 0; i < nodes_num; ++i ) {
        ...
        st->SetWndSize(
            Fvector2().set( m_view->GetDesiredChildWidth(), button_height ) );
        st->SetFont( pF );
        ...
        //.		st->SetTextOffset			(0,
        //(button_height-font_height)/2.0f);
...
void CUIMMShniaga::SetPage( enum_page_id page_id,
                            LPCSTR xml_file,
                            LPCSTR xml_path ) {
...
void CUIMMShniaga::Update() {
    if ( m_start_time > Device.dwTimeContinual - m_run_time ) {
        ...
        pos.y = this->pos( m_origin, m_destination,
                           Device.dwTimeContinual - m_start_time );
...
void CUIMMShniaga::ProcessEvent( EVENT ev ) {
    switch ( ev ) {
        case E_Begin: {
            ...
            m_destination =
                m_selected->GetWndPos().y - m_magnifier->GetWndPos().y;
            m_destination += m_offset;
            m_run_time = u32( ( log( 1 + abs( m_origin - m_destination ) ) /
                                log( GetHeight() ) ) *
                              300 );
            if ( m_run_time < 100 )
                m_run_time = 100;
