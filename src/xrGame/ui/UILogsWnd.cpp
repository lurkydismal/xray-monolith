...
extern u64 generate_time( u32 years,
                          u32 months,
                          u32 days,
                          u32 hours,
                          u32 minutes,
                          u32 seconds,
                          u32 milliseconds = 0 );
extern void split_time( u64 time,
                        u32& years,
                        u32& months,
                        u32& days,
                        u32& hours,
...
void CUILogsWnd::Init() {
    ...
    //	m_background		= UIHelper::CreateFrameLine( m_uiXml,
    //"background", this );
    m_background = UIHelper::CreateFrameWindow( m_uiXml, "background", this );
    m_center_background =
        UIHelper::CreateFrameWindow( m_uiXml, "center_background", this );
    ...
    //	m_center_background	= UIHelper::CreateStatic( m_uiXml,
    //"center_background", this );
    m_center_caption =
        UIHelper::CreateTextWnd( m_uiXml, "center_caption", this );

    string256 buf;
    xr_strcpy( buf, sizeof( buf ), m_center_caption->GetText() );
    xr_strcat( buf, sizeof( buf ),
               CStringTable().translate( "ui_logs_center_caption" ).c_str() );
    ...
    //	m_logs_list->m_sort_function = fastdelegate::MakeDelegate( this,
    //&CUIRankingWnd::SortingLessFunction );
    ...
    m_prev_period =
        UIHelper::Create3tButton( m_uiXml, "btn_prev_period", this );
    m_next_period =
        UIHelper::Create3tButton( m_uiXml, "btn_next_period", this );
    ...
    AddCallback(
        m_filter_news, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUILogsWnd::UpdateChecks ) );
    AddCallback(
        m_filter_talk, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUILogsWnd::UpdateChecks ) );
    AddCallback(
        m_prev_period, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUILogsWnd::PrevPeriod ) );
    AddCallback(
        m_next_period, BUTTON_CLICKED,
        CUIWndCallback::void_function( this, &CUILogsWnd::NextPeriod ) );
...
void CUILogsWnd::ReLoadNews() {
    ...
    LPCSTR date_str = InventoryUtilities::GetDateAsString(
                          m_selected_period, InventoryUtilities::edpDateToDay )
                          .c_str();
    m_period->SetText( date_str );
    Fvector2 pos = m_period_caption->GetWndPos();
    pos.x = m_period->GetWndPos().x - m_period_caption->GetWidth() -
            m_prev_period->GetWidth() - 5.0f;
    ...
    for ( u32 idx = 0; ib != ie; ++ib, ++idx ) {
        ...
        if ( gn.receive_time < m_selected_period ||
             end_period < gn.receive_time ) {
            add = false;
        }
    ...
    if ( !m_list->Empty() ) {
        ...
        m_items_cache.insert( m_items_cache.end(), m_list->Items().begin(),
                              m_list->Items().end() );
        m_list->Items().clear();

        std::for_each( m_items_cache.begin(), m_items_cache.end(),
                       itemToCache );
    }
    PerformWork();
}
...
ALife::_TIME_ID CUILogsWnd::GetShiftPeriod( ALife::_TIME_ID datetime,
                                            int shift_day ) {
