...
void CUIMpTradeWnd::OnBtnPistolSilencerClicked( CUIWindow* w, void* d ) {
    ...
    if ( IsAddonAttached( pitem, at_silencer ) ) {
        ...
        SBuyItemInfo* addon_item =
            CreateItem( addon_name, SBuyItemInfo::e_undefined, false );
...
void CUIMpTradeWnd::OnBtnRifleAmmo2Clicked( CUIWindow* w, void* d ) {
    ...
    CWeaponMagazinedWGrenade* wpn =
        smart_cast< CWeaponMagazinedWGrenade* >( ii );
    if ( !wpn )
        return;
...
bool CUIMpTradeWnd::TryToAttachItemAsAddon( SBuyItemInfo* itm,
    ...
    if ( itm_parent ) {
        ...
        for ( u32 i = 0; i < 2; ++i ) {
            ...
            CUICellItem* ci =
                ( _list->ItemsCount() ) ? _list->GetItemIdx( 0 ) : NULL;
            if ( !ci )
                return false;
...
void CUIMpTradeWnd::SellItemAddons( SBuyItemInfo* sell_itm,
                                    item_addon_type addon_type ) {
    ...
    if ( IsAddonAttached( sell_itm, addon_type ) ) {
        ...
        u32 _item_cost =
            m_item_mngr->GetItemCost( detached_addon->m_name_sect, GetRank() );
        SetMoneyAmount( GetMoneyAmount() + _item_cost );
        DestroyItem( detached_addon );

        if ( addon_type == at_glauncher ) {
            CWeaponMagazinedWGrenade* wpn2 =
                smart_cast< CWeaponMagazinedWGrenade* >( item_ );
            VERIFY( wpn2 );

            for ( u32 ammo_idx = 0; ammo_idx < wpn2->m_ammoTypes2.size();
                  ++ammo_idx ) {
                const shared_str& ammo_name = wpn2->m_ammoTypes2[ ammo_idx ];
                SBuyItemInfo* ammo = NULL;

                while ( ( ammo = FindItem(
                              ammo_name, SBuyItemInfo::e_bought ) ) != NULL ) {
...
bool CUIMpTradeWnd::IsAddonAttached( SBuyItemInfo* itm, item_addon_type at ) {
    ...
    switch ( at ) {
        ...
        case at_glauncher: {
            b_res = ( w->GrenadeLauncherAttachable() &&
                      w->IsGrenadeLauncherAttached() );
        } break;
    };
    return b_res;
}
...
SBuyItemInfo* CUIMpTradeWnd::DetachAddon( SBuyItemInfo* itm,
                                          item_addon_type at ) {
    ...
    SBuyItemInfo* detached_addon =
        CreateItem( addon_name_sect, SBuyItemInfo::e_own, false );
    return detached_addon;
}

shared_str CUIMpTradeWnd::GetAddonNameSect( SBuyItemInfo* itm,
                                            item_addon_type at ) {
    CInventoryItem* item_ = ( CInventoryItem* )itm->m_cell_item->m_pData;
    CWeapon* w = smart_cast< CWeapon* >( item_ );
...
CUIMpTradeWnd::item_addon_type CUIMpTradeWnd::GetItemType(
