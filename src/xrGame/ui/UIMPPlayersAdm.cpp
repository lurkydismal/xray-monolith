...
xr_token g_ban_times[] = {
    { "ui_mp_am_10_minutes", 600 },    { "ui_mp_am_30_minutes", 1800 },
...
void CUIMpPlayersAdm::Init( CUIXml& xml_doc ) {
    ...
    CUIXmlInit::InitListBox( xml_doc, "players_adm:players_list", 0,
                             m_pPlayersList );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:refresh_button", 0,
                              m_pRefreshBtn );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:screen_all_button", 0,
                              m_pScreenAllBtn );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:config_all_button", 0,
                              m_pConfigAllBtn );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:max_ping_limit_button", 0,
                              m_pPingLimitBtn );
    CUIXmlInit::InitTrackBar( xml_doc, "players_adm:max_ping_limit_track", 0,
                              m_pPingLimitTrack );
    CUIXmlInit::InitTextWnd( xml_doc, "players_adm:max_ping_limit_text", 0,
                             m_pPingLimitText );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:screen_player_button", 0,
                              m_pScreenPlayerBtn );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:config_player_button", 0,
                              m_pConfigPlayerBtn );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:kick_player_button", 0,
                              m_pKickPlayerBtn );
    CUIXmlInit::Init3tButton( xml_doc, "players_adm:ban_player_button", 0,
                              m_pBanPlayerBtn );
    CUIXmlInit::InitComboBox( xml_doc, "players_adm:ban_player_combo", 0,
                              m_pBanPlayerCombo );
    // CUIXmlInit::InitTrackBar(xml_doc, "players_adm:ban_time_track", 0,
    // m_pBanTimeTrack); CUIXmlInit::InitTextWnd(xml_doc,
    // "players_adm:ban_time_text", 0, m_pBanTimeText);
    RefreshPlayersList();
    int min, max;
    g_sv_adm_menu_ping_limit =
        iCeil( Console->GetInteger( "sv_max_ping_limit", min, max ) / 10.0f );
    ...
    for ( ; b != Game().players.end(); b++ ) {
        //		if(b->first!=Game().local_svdpnid)
        {
            ...
            xr_sprintf( tmp_string, "%s, id:%u, ip:%s, ping:%u",
                        b->second->getName(), b->first.value(),
...
void CUIMpPlayersAdm::RefreshPlayersList() {
    ...
    tmp_game->RequestPlayersInfo( xr_delegate< void( u32 const ) >(
        this, &CUIMpPlayersAdm::FillPlayersList ) );
}
...
void CUIMpPlayersAdm::SetMaxPingLimitText() {
    int ping_limit = m_pPingLimitTrack->GetIValue();
    string512 tmp_string;
    xr_sprintf( tmp_string, "%s %d",
                CStringTable().translate( "ui_mp_am_ping_limit" ).c_str(),
                ping_limit * 10 );
