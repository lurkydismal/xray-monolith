...
CUICustomEdit::CUICustomEdit() {
    m_editor_control =
        xr_new< text_editor::line_edit_control >( ( u32 )EDIT_BUF_SIZE );
...
void CUICustomEdit::Register_callbacks() {
    ec().assign_callback( DIK_ESCAPE, text_editor::ks_free,
                          Callback( this, &CUICustomEdit::press_escape ) );
    ec().assign_callback( DIK_RETURN, text_editor::ks_free,
                          Callback( this, &CUICustomEdit::press_commit ) );
    ec().assign_callback( DIK_NUMPADENTER, text_editor::ks_free,
                          Callback( this, &CUICustomEdit::press_commit ) );
    ec().assign_callback( DIK_GRAVE, text_editor::ks_free,
                          Callback( this, &CUICustomEdit::nothing ) );
    ec().assign_callback( DIK_TAB, text_editor::ks_free,
                          Callback( this, &CUICustomEdit::press_tab ) );
}

void CUICustomEdit::Init( u32 max_char_count,
                          bool number_only_mode,
                          bool read_mode,
                          bool fn_mode ) {
    if ( read_mode ) {
        ...
        if ( number_only_mode ) {
            m_editor_control->init( max_char_count,
                                    text_editor::im_number_only );
        } else if ( fn_mode ) {
            m_editor_control->init( max_char_count,
                                    text_editor::im_file_name_mode );
        } else {
            m_editor_control->init( max_char_count );
        }
...
bool CUICustomEdit::OnMouseAction( float x,
                                   float y,
...
bool CUICustomEdit::OnKeyboardAction( int dik, EUIMessages keyboard_action ) {
    ...
    if ( dik == mouse_button_2_key[ 0 ] ||
         dik == mouse_button_2_key[ 1 ] ) // Mouse 1 + 2
    {
...
void CUICustomEdit::Draw() {
    ...
    {
        out.x = pos.x + 0.0f + TextItemControl()->m_TextOffset.x +
                TextItemControl()->GetIndentByAlign();
        out.y = pos.y + 2.0f + TextItemControl()->m_TextOffset.y +
                TextItemControl()->GetVIndentByAlign();
