...
void CUIInventoryUpgradeWnd::LoadCellStates( LPCSTR state_str,
                                             LPCSTR texture_name,
    ...
    SetCellState( SelectCellState( state_str ), texture_name, texture_name2,
                  color );
}

UIUpgrade::ViewState CUIInventoryUpgradeWnd::SelectCellState(
    LPCSTR state_str ) {
    ...
    VERIFY2( 0, make_string( "Such UI upgrade state (%s) does not exist !",
                             state_str ) );
    return UIUpgrade::STATE_UNKNOWN;
}

void CUIInventoryUpgradeWnd::SetCellState( UIUpgrade::ViewState state,
                                           LPCSTR texture_name,
                                           LPCSTR texture_name2,
                                           u32 color ) {
...
void CUIInventoryUpgradeWnd::LoadSchemes( CUIXml& uiXml ) {
    ...
    t_cell_item.x2 =
        t_cell_item.x1 + uiXml.ReadAttribFlt( "cell_item", 0, "width" ) *
                             ( UI().is_widescreen() ? 0.8f : 1.0f );
    t_cell_item.y2 =
        t_cell_item.y1 + uiXml.ReadAttribFlt( "cell_item", 0, "height" );
