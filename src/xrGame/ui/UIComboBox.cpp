...
void CUIComboBox::InitComboBox( Fvector2 pos, float width ) {
    ...
    m_frameLine.InitIB( Fvector2().set( 0, 0 ),
                        Fvector2().set( width, CB_HEIGHT ) );

    m_frameLine.InitState(
        S_Enabled, "ui_inGame2_combobox_linetext" ); // horizontal by default
    ...
    float item_height =
        CUITextureMaster::GetTextureHeight( "ui_inGame2_combobox_line_b" );

    m_list_box.SetWndPos( Fvector2().set( lb_text_offset, 0.0f ) );
    m_list_box.SetWndSize(
        Fvector2().set( width - lb_text_offset, item_height * m_iListHeight ) );
    m_list_box.InitScrollView();
    m_list_box.SetTextColor( m_textColor[ 0 ] );
    m_list_box.SetSelectionTexture( "ui_inGame2_combobox_line" );
    m_list_box.SetItemHeight(
        CUITextureMaster::GetTextureHeight( "ui_inGame2_combobox_line_b" ) );
    // frame(texture) for list
    m_list_frame.InitTexture( "ui_inGame2_combobox" );
    m_list_frame.SetWndSize(
        Fvector2().set( width, m_list_box.GetItemHeight() * m_iListHeight ) );
...
void CUIComboBox::disable_id( int id ) {
    if ( m_disabled.end() ==
         std::find( m_disabled.begin(), m_disabled.end(), id ) )
...
void CUIComboBox::SetCurrentOptValue() {
    ...
    while ( tok->name ) {
        if ( m_disabled.end() ==
             std::find( m_disabled.begin(), m_disabled.end(), tok->id ) ) {
