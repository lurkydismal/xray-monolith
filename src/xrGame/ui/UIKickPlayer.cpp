...
void CUIKickPlayer::Init_internal( CUIXml& xml_doc ) {
    ...
    //	CUIXmlInit::InitFrameWindow	(xml_doc, "kick_ban:list_back",
    //0, lst_back);
    ...
    CUIXmlInit::InitSpin( xml_doc, "kick_ban:spin_ban_time", 0,
                          m_spin_ban_sec );
    CUIXmlInit::InitStatic( xml_doc, "kick_ban:ban_time_lbl", 0,
                            m_ban_sec_label );
}
...
void CUIKickPlayer::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    if ( LIST_ITEM_SELECT == msg && pWnd == m_ui_players_list ) {
        CUIListBoxItem* itm =
            smart_cast< CUIListBoxItem* >( m_ui_players_list->GetSelected() );
    ...
    if ( item ) {
        ...
        switch ( mode ) {
            case MODE_KICK:
                ...
                break;
            case MODE_BAN: {
                xr_sprintf( command, "cl_votestart ban %s %d", item->GetText(),
                            m_spin_ban_sec->Value() );
            } break;
        }
...
void CUIKickPlayer::Update() {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( m_selected_item_text.size() &&
             !stricmp( pI->getName(), m_selected_item_text.c_str() ) )
            bHasSelected = true;
