...
SLuaWpnParams::SLuaWpnParams() {
    bool functor_exists;
    functor_exists =
        ai().script_engine().functor( "ui_wpn_params.GetRPM", m_functorRPM );
    VERIFY( functor_exists );
    functor_exists = ai().script_engine().functor( "ui_wpn_params.GetDamage",
                                                   m_functorDamage );
    VERIFY( functor_exists );
    functor_exists = ai().script_engine().functor( "ui_wpn_params.GetDamageMP",
                                                   m_functorDamageMP );
    VERIFY( functor_exists );
    functor_exists = ai().script_engine().functor( "ui_wpn_params.GetHandling",
                                                   m_functorHandling );
    VERIFY( functor_exists );
    functor_exists = ai().script_engine().functor( "ui_wpn_params.GetAccuracy",
                                                   m_functorAccuracy );
    VERIFY( functor_exists );
}
...
void CUIWpnParams::InitFromXml( CUIXml& xml_doc ) {
    ...
    CUIXmlInit::InitStatic( xml_doc, "wpn_params:static_accuracy", 0,
                            &m_icon_acc );
    CUIXmlInit::InitStatic( xml_doc, "wpn_params:static_damage", 0,
                            &m_icon_dam );
    CUIXmlInit::InitStatic( xml_doc, "wpn_params:static_handling", 0,
                            &m_icon_han );
    CUIXmlInit::InitStatic( xml_doc, "wpn_params:static_rpm", 0, &m_icon_rpm );

    CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_accuracy", 0,
                             &m_textAccuracy );
    CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_damage", 0,
                             &m_textDamage );
    CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_handling", 0,
                             &m_textHandling );
    ...
    if ( IsGameTypeSingle() ) {
        CUIXmlInit::InitStatic( xml_doc, "wpn_params:static_ammo", 0,
                                &m_stAmmo );
        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_ammo_count", 0,
                                 &m_textAmmoCount );
        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_ammo_count2", 0,
                                 &m_textAmmoCount2 );
        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_ammo_types", 0,
                                 &m_textAmmoTypes );
        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_ammo_used_type", 0,
                                 &m_textAmmoUsedType );
        // CUIXmlInit::InitStatic			(xml_doc,
        // "wpn_params:static_ammo_type1",	0, &m_stAmmoType1);
        // CUIXmlInit::InitStatic			(xml_doc,
        // "wpn_params:static_ammo_type2",	0, &m_stAmmoType2);

        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_accuracy_inc", 0,
                                 &m_textAccuracy_inc );
        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_damage_inc", 0,
                                 &m_textDamage_inc );
        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_handling_inc", 0,
                                 &m_textHandling_inc );
        CUIXmlInit::InitTextWnd( xml_doc, "wpn_params:cap_rpm_inc", 0,
                                 &m_textRPM_inc );

        // Выводим иконки для любого кол-ва типов патронов, описанных в XML
        // --#SM+#--
        ...
        {
            iCnt++;
            xr_sprintf( str, sizeof( str ), "wpn_params:static_ammo_type%d",
                        iCnt );
...
void CUIWpnParams::SetInfo( CInventoryItem* slot_wpn,
                            CInventoryItem& cur_wpn ) {
    if ( !g_lua_wpn_params ) {
    ...
    float cur_rpm =
        iFloor( g_lua_wpn_params->m_functorRPM( cur_section, str_upgrades ) *
                53.0f ) /
        53.0f;
    float cur_accur = iFloor( g_lua_wpn_params->m_functorAccuracy(
                                  cur_section, str_upgrades ) *
                              53.0f ) /
                      53.0f;
    float cur_hand = iFloor( g_lua_wpn_params->m_functorHandling(
                                 cur_section, str_upgrades ) *
                             53.0f ) /
                     53.0f;
    float cur_damage = ( GameID() == eGameIDSingle )
                           ? iFloor( g_lua_wpn_params->m_functorDamage(
                                         cur_section, str_upgrades ) *
                                     53.0f ) /
                                 53.0f
                           : iFloor( g_lua_wpn_params->m_functorDamageMP(
                                         cur_section, str_upgrades ) *
                                     53.0f ) /
                                 53.0f;
    ...
    if ( slot_wpn && ( slot_wpn != &cur_wpn ) ) {
        ...
        slot_rpm = iFloor( g_lua_wpn_params->m_functorRPM( slot_section,
                                                           str_upgrades ) *
                           53.0f ) /
                   53.0f;
        slot_accur = iFloor( g_lua_wpn_params->m_functorAccuracy(
                                 slot_section, str_upgrades ) *
                             53.0f ) /
                     53.0f;
        slot_hand = iFloor( g_lua_wpn_params->m_functorHandling(
                                slot_section, str_upgrades ) *
                            53.0f ) /
                    53.0f;
        ...
        xr_sprintf( str_value, sizeof( str_value ),
                    adj_value >= 0 ? "+%.1f%%" : "%.1f%%", adj_value );
    ...
    if ( IsGameTypeSingle() ) {
        ...
        std::copy_if( ammo_types.begin(), ammo_types.end(),
                      std::back_inserter( good_ammo ),
                      []( const shared_str& ammo_type ) {
                          return ammo_type != NULL &&
                                 !strstr( ammo_type.c_str(), "_bad" ) &&
                                 !strstr( ammo_type.c_str(), "_verybad" );
                      } );
        ...
        for ( u8 i = 0; i < m_vecStAmmoTypes.size(); i++ ) {
            ...
            if ( i >= good_ammo.size() ) {
                ...
                if ( pSettings->line_exist( good_ammo[ i ].c_str(),
                    ...
                    LPCSTR icons_texture = pSettings->r_string(
                        good_ammo[ i ].c_str(), "icons_texture" );
                    StAmmoType.SetShader(
                ...
                tex_rect.x1 = float(
                    pSettings->r_u32( good_ammo[ i ].c_str(), "inv_grid_x" ) *
                    INV_GRID_WIDTH );
                tex_rect.y1 = float(
                    pSettings->r_u32( good_ammo[ i ].c_str(), "inv_grid_y" ) *
                    INV_GRID_HEIGHT );
                tex_rect.x2 = float( pSettings->r_u32( good_ammo[ i ].c_str(),
                                                       "inv_grid_width" ) *
                                     INV_GRID_WIDTH );
                tex_rect.y2 = float( pSettings->r_u32( good_ammo[ i ].c_str(),
                                                       "inv_grid_height" ) *
                                     INV_GRID_HEIGHT );
            ...
            StAmmoType.SetWndSize( Fvector2().set(
                ( tex_rect.x2 - tex_rect.x1 ) * UI().get_current_kx(),
...
bool CUIWpnParams::Check( const shared_str& wpn_section ) {
    if ( pSettings->line_exist( wpn_section, "fire_dispersion_base" ) ) {
        ...
        if ( 0 == xr_strcmp( pSettings->r_string( wpn_section, "class" ),
                             "WP_KNIFE" ) )
            return false;
...
void CUIConditionParams::SetInfo( CInventoryItem const* slot_item,
                                  CInventoryItem const& cur_item ) {
