...
void CUI3tButton::SetWidth( float width ) {
    ...
    if ( m_background ) {
        ...
        m_back_framewindow->SetWidth( width );
    }
}
...
void CUI3tButton::SetHeight( float height ) {
    ...
    if ( m_background ) {
        ...
        m_back_framewindow->SetHeight( height );
    }
}
...
void CUI3tButton::Update() {
    ...
    if ( m_bTextureEnable ) {
        if ( !m_bIsEnabled ) {
            if ( m_background ) {
                ...
                m_back_framewindow->SetCurrentState( S_Disabled );
            }
            ...
            if ( m_background ) {
                ...
                m_back_framewindow->SetCurrentState( S_Touched );
            }
            ...
            if ( m_background ) {
                ...
                m_back_framewindow->SetCurrentState( S_Highlighted );
            }
        } else {
            if ( m_background ) {
                ...
                m_back_framewindow->SetCurrentState( S_Enabled );
            }
        }
    }
    ...
    if ( !m_bIsEnabled ) {
        textColor = m_bUseTextColor[ S_Disabled ] ? m_dwTextColor[ S_Disabled ]
                                                  : m_dwTextColor[ S_Enabled ];
    } else if ( CUIButton::BUTTON_PUSHED == GetButtonState() ) {
        textColor = m_bUseTextColor[ S_Touched ] ? m_dwTextColor[ S_Touched ]
                                                 : m_dwTextColor[ S_Enabled ];
    } else if ( m_bCursorOverWindow ) {
        textColor = m_bUseTextColor[ S_Highlighted ]
                        ? m_dwTextColor[ S_Highlighted ]
                        : m_dwTextColor[ S_Enabled ];
    } else {
