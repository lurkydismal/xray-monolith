...
void CUITalkWnd::InitTalkDialog() {
    ...
    m_pOthersDialogManager =
        smart_cast< CPhraseDialogManager* >( m_pOthersInvOwner );

    // имена собеседников
    UITalkDialogWnd->UICharacterInfoLeft.InitCharacter(
        m_pOurInvOwner->object_id() );
    UITalkDialogWnd->UICharacterInfoRight.InitCharacter(
        m_pOthersInvOwner->object_id() );

    //.	UITalkDialogWnd->UIDialogFrame.UITitleText.SetText
    //(m_pOthersInvOwner->Name()); .
    //UITalkDialogWnd->UIOurPhrasesFrame.UITitleText.SetText
    //(m_pOurInvOwner->Name());
    ...
    UITalkDialogWnd->mechanic_mode =
        m_pOthersInvOwner->SpecificCharacter().upgrade_mechanic();
    UITalkDialogWnd->SetOsoznanieMode( m_pOthersInvOwner->NeedOsoznanieMode() );
    UITalkDialogWnd->Show();
    UITalkDialogWnd->UpdateButtonsLayout( b_disable_break,
...
void CUITalkWnd::InitOthersStartDialog() {
    ...
    if ( !m_pOthersDialogManager->AvailableDialogs().empty() ) {
        ...
        m_pOthersDialogManager->InitDialog( m_pOurDialogManager,
                                            m_pCurrentDialog );

        // сказать фразу
        CStringTable stbl;
        AddAnswer( m_pCurrentDialog->GetPhraseText( "0" ),
                   m_pOthersInvOwner->Name() );
...
void CUITalkWnd::UpdateQuestions() {
    ...
    if ( !m_pCurrentDialog ) {
        ...
        for ( u32 i = 0; i < m_pOurDialogManager->AvailableDialogs().size();
              ++i ) {
            const DIALOG_SHARED_PTR& phrase_dialog =
                m_pOurDialogManager->AvailableDialogs()[ i ];
            bool bfinalizer =
                ( phrase_dialog->GetPhrase( "0" ) )->IsFinalizer();

            AddQuestion( phrase_dialog->DialogCaption(),
                         phrase_dialog->GetDialogID(), i, bfinalizer );
        }
    } else {
        if ( m_pCurrentDialog->IsWeSpeaking( m_pOurDialogManager ) ) {
            // если в списке допустимых фраз только одна фраза пустышка, то
            // просто сказать (игрок сам не производит никаких действий)
            if ( !m_pCurrentDialog->PhraseList().empty() &&
                 m_pCurrentDialog->allIsDummy() ) {
                CPhrase* phrase = m_pCurrentDialog->PhraseList()[ Random.randI(
                    m_pCurrentDialog->PhraseList().size() ) ];
            ...
            if ( m_pCurrentDialog && !m_pCurrentDialog->allIsDummy() ) {
                int number = 0;
                for ( PHRASE_VECTOR::const_iterator it =
                          m_pCurrentDialog->PhraseList().begin();
                      it != m_pCurrentDialog->PhraseList().end();
                      ++it, ++number ) {
                    CPhrase* phrase = *it;
                    AddQuestion(
                        m_pCurrentDialog->GetPhraseText( phrase->GetID() ),
                        phrase->GetID(), number, phrase->IsFinalizer() );
                }
            } else
                UpdateQuestions();
        }
...
void CUITalkWnd::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    if ( pWnd == UITalkDialogWnd && msg == TALK_DIALOG_TRADE_BUTTON_CLICKED ) {
        SwitchToTrade();
    } else if ( pWnd == UITalkDialogWnd &&
                msg == TALK_DIALOG_UPGRADE_BUTTON_CLICKED ) {
        SwitchToUpgrade();
    } else if ( pWnd == UITalkDialogWnd &&
                msg == TALK_DIALOG_QUESTION_CLICKED ) {
        AskQuestion();
    }
...
void UpdateCameraDirection( CGameObject* pTo ) {
    ...
    if ( angle_difference( cam->yaw, -h ) > 0.2 )
        cam->yaw = angle_inertion_var( cam->yaw, -h, 0.15f, 0.2f, PI_DIV_6,
                                       Device.fTimeDelta );

    if ( angle_difference( cam->pitch, -p ) > 0.2 )
        cam->pitch = angle_inertion_var( cam->pitch, -p, 0.15f, 0.2f, PI_DIV_6,
                                         Device.fTimeDelta );
}
...
void CUITalkWnd::AskQuestion() {
    ...
    if ( TopicMode() ) {
        if ( ( UITalkDialogWnd->m_ClickedQuestionID == "" ) ||
             ( !m_pOurDialogManager->HaveAvailableDialog(
                 UITalkDialogWnd->m_ClickedQuestionID ) ) ) {
            string128 s;
            xr_sprintf( s,
                        "ID = [%s] of selected question is out of range of "
                        "available dialogs ",
                        UITalkDialogWnd->m_ClickedQuestionID.c_str() );
            VERIFY2( FALSE, s );
        }

        m_pCurrentDialog = m_pOurDialogManager->GetDialogByID(
            UITalkDialogWnd->m_ClickedQuestionID );

        m_pOurDialogManager->InitDialog( m_pOthersDialogManager,
                                         m_pCurrentDialog );
        phrase_id = "0";
    } else {
...
void CUITalkWnd::SayPhrase( const shared_str& phrase_id ) {
    AddAnswer( m_pCurrentDialog->GetPhraseText( phrase_id ),
               m_pOurInvOwner->Name() );
...
void CUITalkWnd::AddQuestion( const shared_str& text,
                              const shared_str& value,
                              int number,
                              bool b_finalizer ) {
    if ( text.size() == 0 )
        return;

    UITalkDialogWnd->AddQuestion( CStringTable().translate( text ).c_str(),
                                  value.c_str(), number, b_finalizer );
}
...
void CUITalkWnd::AddAnswer( const shared_str& text, LPCSTR SpeakerName ) {
    ...
    UITalkDialogWnd->AddAnswer( SpeakerName, *CStringTable().translate( text ),
                                i_am );
}

void CUITalkWnd::SwitchToTrade() {
    if ( m_pOurInvOwner->IsTradeEnabled() &&
         m_pOthersInvOwner->IsTradeEnabled() ) {
...
void CUITalkWnd::SwitchToUpgrade() {
    // if ( m_pOurInvOwner->IsInvUpgradeEnabled() &&
    // m_pOthersInvOwner->IsInvUpgradeEnabled() )
    {
        CUIGameSP* pGameSP = smart_cast< CUIGameSP* >( CurrentGameUI() );
        if ( pGameSP ) {
            /*			if ( pGameSP->MainInputReceiver() )
...
void CUITalkWnd::PlaySnd( LPCSTR text ) {
    ...
    if ( FS.exist( "$game_sounds$", fn ) ) {
        ...
        if ( !m_pActor->OnDialogSoundHandlerStart( m_pOthersInvOwner, fn ) ) {
            CGameObject* pOtherGO =
                smart_cast< CGameObject* >( m_pOthersInvOwner );
            Fvector P = pOtherGO->Position();
            P.y += 1.8f;
...
void CUITalkWnd::AddIconedMessage( LPCSTR caption,
                                   LPCSTR text,
                                   LPCSTR texture_name,
                                   LPCSTR templ_name ) {
