...
private:
    void GatherSpotsUnderCursor( CMapSpot* clicked,
                                 xr_vector< CMapSpot* >& out );
    void AddSpotProperties( CUIPropertiesBox* box,
                            u16 id,
                            const shared_str& level,
                            LPCSTR hint );
    u16 AddRightClickMapProperties( CMapSpot* top, shared_str& out_level );

    struct SPropertyOwner {
        CUIListBoxItem* item;
        u16 id;
        shared_str level;
        shared_str hint;
    ...
    void TagSubmenuOwners( CUIPropertiesBox* box,
    ...
    // DIRTY HACK
    // Never shown; absorbs the items mods re-add when the add-properties
    // callback is replayed at click time to rebuild their per-spot dispatch
    // state (see SendMessage).
