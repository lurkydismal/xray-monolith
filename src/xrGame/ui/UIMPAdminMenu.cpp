...
void CUIMpAdminMenu::Init() {
    ...
    CUIXmlInit::InitTabControl( *xml_doc, "admin_menu:tab_control", 0,
                                m_pTabControl );
    ...
    CUIXmlInit::Init3tButton( *xml_doc, "admin_menu:close_button", 0,
                              m_pClose );

    m_pMessageBoxLogin->InitMessageBox( "message_box_ra_login" );
    m_pMessageBoxLogin->func_on_ok = CUIWndCallback::void_function(
        this, &CUIMpAdminMenu::RemoteAdminLogin );
...
void CUIMpAdminMenu::ShowMessageBox( CUIMessageBox::E_MESSAGEBOX_STYLE style,
                                     LPCSTR reason ) {
    switch ( style ) {
...
void CUIMpAdminMenu::RemoteAdminLogin( CUIWindow*, void* ) {
    ...
    xr_sprintf( tmp_string, "ra login %s %s",
                m_pMessageBoxLogin->m_pMessageBox->GetUserPassword(),
