...
void CUIListItemServer::SetParams( LIST_SRV_ITEM& params ) {
    ...
    cut_string_by_length( m_map->GetFont(), _srv_name, buff, sizeof( buff ),
                          m_server->GetWidth() );
    m_server->SetText( buff );

    LPCSTR _map_name = CStringTable().translate( params.info.map ).c_str();
    cut_string_by_length( m_map->GetFont(), _map_name, buff, sizeof( buff ),
                          m_map->GetWidth() );
    m_map->SetText( buff );

    LPCSTR _game_name = CStringTable().translate( params.info.game ).c_str();
    cut_string_by_length( m_game->GetFont(), _game_name, buff, sizeof( buff ),
                          m_game->GetWidth() );
    m_game->SetText( buff );
...
void CUIListItemServer::CreateConsoleCommand( xr_string& command,
                                              LPCSTR player_name,
                                              LPCSTR player_pass,
                                              LPCSTR server_psw ) {
