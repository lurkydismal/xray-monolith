...
void CUIMpTradeWnd::UpdateMoneyIndicator() {
    if ( m_bIgnoreMoneyAndRank ) {
        ...
        for ( u32 i = _preset_idx_last; i <= _preset_idx_3; ++i ) {
            ...
            u32 clr = ( b_has_enought_money ) ? m_text_color_money_positive
...
void CUIMpTradeWnd::SetMoneyChangeString( int diff ) {
    ...
    u32 clr = ( diff > 0 ) ? m_text_color_money_positive
                           : m_text_color_money_negative;
...
CInventoryItem* CUIMpTradeWnd::CurrentIItem() {
    return ( m_pCurrentCellItem )
               ? ( CInventoryItem* )m_pCurrentCellItem->m_pData
               : NULL;
}
...
void CUIMpTradeWnd::SetCurrentItem( CUICellItem* itm ) {
    ...
    if ( m_pCurrentCellItem ) {
        const shared_str& current_sect_name =
            CurrentIItem()->object().cNameSect();
        string256 str;
        ...
        xr_sprintf( tex_name, "ui_hud_status_%s_0%d", team,
                    1 + get_rank( current_sect_name.c_str() ) );
...
int CUIMpTradeWnd::GetItemPrice( CInventoryItem* itm ) {
    return m_item_mngr->GetItemCost( itm->object().cNameSect(),
                                     g_mp_restrictions.GetRank() );
}

void CUIMpTradeWnd::BindDragDropListEvents( CUIDragDropListEx* lst,
                                            bool bDrag ) {
    lst->m_f_item_drop =
        CUIDragDropListEx::DRAG_CELL_EVENT( this, &CUIMpTradeWnd::OnItemDrop );
    lst->m_f_item_db_click = CUIDragDropListEx::DRAG_CELL_EVENT(
        this, &CUIMpTradeWnd::OnItemDbClick );
    lst->m_f_item_selected = CUIDragDropListEx::DRAG_CELL_EVENT(
        this, &CUIMpTradeWnd::OnItemSelected );
    lst->m_f_item_rbutton_click = CUIDragDropListEx::DRAG_CELL_EVENT(
        this, &CUIMpTradeWnd::OnItemRButtonClick );
    lst->m_f_item_lbutton_click = CUIDragDropListEx::DRAG_CELL_EVENT(
        this, &CUIMpTradeWnd::OnItemLButtonClick );
    if ( bDrag )
        lst->m_f_item_start_drag = CUIDragDropListEx::DRAG_CELL_EVENT(
            this, &CUIMpTradeWnd::OnItemStartDrag );
}
...
bool CUIMpTradeWnd::OnItemLButtonClick( CUICellItem* itm ) {
    ...
    SBuyItemInfo* pitem =
        CreateItem( buy_item_name, SBuyItemInfo::e_undefined, false );
...
bool CUIMpTradeWnd::CanBuyOrSellInList( CUIDragDropListEx* list ) {
    return list == m_list[ e_pistol_ammo ] || list == m_list[ e_rifle_ammo ] ||
           list == m_list[ e_medkit ] || list == m_list[ e_granade ];
}
...
CUIDragDropListEx* CUIMpTradeWnd::GetMatchedListForItem(
    const shared_str& sect_name ) {
    ...
    if ( list_idx == e_pistol_ammo || list_idx == e_rifle_ammo ) {
        CUICellItem* ci = ( m_list[ list_idx - 1 ]->ItemsCount() )
                              ? m_list[ list_idx - 1 ]->GetItemIdx( 0 )
                              : NULL;
        if ( !ci )
...
void CUIMpTradeWnd::GetWeaponIndexByName( const shared_str& sectionName,
                                          u8& grpNum,
                                          u8& idx ) {
...
u32 CUIMpTradeWnd::GetPresetCost( ETradePreset idx ) {
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( _one.addon_names[ 0 ].c_str() )
            _item_cost +=
                m_item_mngr->GetItemCost( _one.addon_names[ 0 ], GetRank() );

        if ( _one.addon_names[ 1 ].c_str() )
            _item_cost +=
                m_item_mngr->GetItemCost( _one.addon_names[ 1 ], GetRank() );

        if ( _one.addon_names[ 2 ].c_str() )
            _item_cost +=
                m_item_mngr->GetItemCost( _one.addon_names[ 2 ], GetRank() );
...
void CUIMpTradeWnd::AddonToSlot( int add_on,
                                 int slot,
                                 bool bRealRepresentationSet ) {
    // own
}

void CUIMpTradeWnd::SectionToSlot( const u8 grpNum,
                                   u8 uIndexInSlot,
                                   bool bRealRepresentationSet ) {
    // own
}
