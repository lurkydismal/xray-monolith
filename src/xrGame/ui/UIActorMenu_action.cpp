...
bool CUIActorMenu::AllowItemDrops( EDDListType from, EDDListType to ) {
    ...
    xr_vector< EDDListType >::iterator it =
        std::find( v.begin(), v.end(), from );
...
public:
    CUITrashIcon() {
        m_icon.SetWndSize(
            Fvector2().set( 29.0f * UI().get_current_kx(), 36.0f ) );
...
bool CUIActorMenu::OnItemDrop( CUICellItem* itm ) {
    ...
    if ( old_owner == new_owner ) {
        // Alundaio: Here we export the action of dragging one inventory item
        // ontop of another!
        ::luabind::functor< bool > funct1;
        if ( ai().script_engine().functor(
                 "actor_menu_inventory.CUIActorMenu_OnItemDropped", funct1 ) ) {
            // If list only has 1 item, get it, otherwise try to get item at
            // current drag position
            CUICellItem* _citem = ( new_owner->ItemsCount() == 1 )
                                      ? new_owner->GetItemIdx( 0 )
                                      : NULL;
            if ( !_citem ) {
                CUICellContainer* c = old_owner->GetContainer();
                Ivector2 c_pos =
                    c->PickCell( old_owner->GetDragItemPosition() );
            ...
            CGameObject* GO2 =
                _iitem ? smart_cast< CGameObject* >( _iitem ) : NULL;
            if ( funct1( GO1 ? GO1->lua_game_object() : ( 0 ),
                         GO2 ? GO2->lua_game_object() : ( 0 ), ( int )t_old,
                         ( int )t_new ) == false )
                return false;
        }
        //-Alundaio
    ...
    // Alundaio: Here we export the action of dragging one inventory item ontop
    // of another!
    ::luabind::functor< bool > funct1;
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_OnItemDropped", funct1 ) ) {
        // If list only has 1 item, get it, otherwise try to get item at current
        // drag position
        CUICellItem* _citem = ( new_owner->ItemsCount() == 1 )
                                  ? new_owner->GetItemIdx( 0 )
                                  : NULL;
...
bool CUIActorMenu::OnItemDbClick( CUICellItem* itm ) {
    ...
    switch ( t_old ) {
        ...
        case iActorBag: {
            ...
            if ( !m_pActorInvOwner->inventory().SlotIsPersistent(
                     iitem_to_place->BaseSlot() ) &&
                 m_pActorInvOwner->inventory().ItemFromSlot(
                     iitem_to_place->BaseSlot() ) == iitem_to_place ) {
                ToBag( itm, false );
...
bool CUIActorMenu::OnItemFocusReceive( CUICellItem* itm ) {
    ...
    ::luabind::functor< bool > funct1;
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_OnItemFocusReceive",
             funct1 ) ) {
...
bool CUIActorMenu::OnItemFocusLost( CUICellItem* itm ) {
    ...
    ::luabind::functor< bool > funct1;
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_OnItemFocusLost", funct1 ) ) {
        PIItem _iitem = ( PIItem )itm->m_pData;
