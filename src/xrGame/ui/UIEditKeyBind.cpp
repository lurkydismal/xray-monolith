...
u32 cut_string_by_length( CGameFont* pFont,
                          LPCSTR src,
                          LPSTR dst,
                          u32 dst_size,
                          float length ) {
...
void CUIEditKeyBind::SetText( const char* text ) {
    if ( !text || 0 == xr_strlen( text ) )
        ...
        cut_string_by_length( TextItemControl()->GetFont(), text, buff,
                              sizeof( buff ), GetWidth() );
...
void CUIEditKeyBind::OnFocusLost() {
    ...
    SetEditMode( false );
    TextItemControl()->SetTextColor( ( subst_alpha(
        TextItemControl()->GetTextColor(), color_get_A( 0xffffffff ) ) ) );
}
...
void CUIEditKeyBind::SetEditMode( bool b ) {
    ...
    if ( b ) {
        SetColorAnimation( "ui_map_area_anim",
                           LA_CYCLIC | LA_ONLYALPHA | LA_TEXTCOLOR );
        TextureOn();
    } else {
...
void CUIEditKeyBind::AssignProps( const shared_str& entry,
                                  const shared_str& group ) {
...
void CUIEditKeyBind::OnMessage( LPCSTR message ) {
    ...
    if ( is_group_not_conflicted( m_action->key_group,
                                  other_action->key_group ) )
        return;
