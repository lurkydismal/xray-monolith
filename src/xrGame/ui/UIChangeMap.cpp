...
void CUIChangeMap::InitChangeMap( CUIXml& xml_doc ) {
    ...
    CUIXmlInit::InitTextWnd( xml_doc, "change_map:map_ver_txt", 0,
                             map_version );
    CUIXmlInit::InitStatic( xml_doc, "change_map:map_pic", 0, map_pic );
    //	CUIXmlInit::InitFrameWindow			(xml_doc,
    //"change_map:list_back", 0, lst_back); 	CUIXmlInit::InitFrameWindow
    //(xml_doc,			"change_map:frame", 0, frame);
...
void CUIChangeMap::OnItemSelect() {
    ...
    const SGameTypeMaps& M =
        gMapListHelper.GetMapListFor( ( EGameIDs )GameID() );
    const shared_str& name = M.m_map_names[ idx ].map_name;
    LPSTR map_ver = NULL;
    STRCONCAT( map_ver, "[",
               M.m_map_names[ idx ].map_ver.c_str()
                   ? M.m_map_names[ idx ].map_ver.c_str()
                   : "unknown",
               "]" );
...
void CUIChangeMap::OnBtnOk() {
    ...
    if ( idx >= 0 && idx < M.m_map_names.size() ) {
        ...
        string512 command;
        xr_sprintf( command, "cl_votestart changemap %s %s", name.c_str(),
                    ver.c_str() );
...
void CUIChangeMap::FillUpList() {
    ...
    for ( u32 i = 0; i < cnt; ++i ) {
        CUIListBoxItem* itm = lst->AddTextItem(
            CStringTable().translate( M.m_map_names[ i ].map_name ).c_str() );
        itm->Enable(
            true ); // m_pExtraContentFilter->IsDataEnabled(M.m_map_names[i].map_name.c_str()));
    }
}
