...
UIProperty::Property_type* UIProperty::get_property() {
    ...
    Property_type* proper =
        ai().alife().inventory_upgrade_manager().get_property( m_property_id );
    VERIFY( proper );
    return proper;
}

bool UIProperty::read_value_from_section( LPCSTR section,
                                          LPCSTR param,
                                          float& result ) {
    result = 0.0f;
    ...
    if ( pSettings->line_exist( section, param ) &&
         *pSettings->r_string( section, param ) ) {
...
bool UIProperty::compute_value( ItemUpgrades_type const& item_upgrades ) {
    ...
    for ( ; ib_upg != ie_upg; ++ib_upg ) {
        Upgrade_type* upgr =
            ai().alife().inventory_upgrade_manager().get_upgrade( *ib_upg );
        VERIFY( upgr );
...
void UIInvUpgPropertiesWnd::init_from_xml( LPCSTR xml_name ) {
    ...
    for ( ; ib != ie; ++ib ) {
        ...
        if ( !ui_property->init_property( property_id ) ) {
            Msg( "! Invalid property <%s> in inventory upgrade manager!",
                 property_id.c_str() );
...
void UIInvUpgPropertiesWnd::set_info( ItemUpgrades_type const& item_upgrades ) {
    ...
    for ( ; ib != ie; ++ib ) {
        ...
        if ( ui_property->compute_value( item_upgrades ) ) {
            ui_property->SetWndPos(
                Fvector2().set( ui_property->GetWndPos().x, new_size.y ) );
