...
void CUIMapDesc::Init() {
    ...
    m_pMapInfo->InitMapInfo( m_pMapInfo->GetWndPos(),
                             m_pMapInfo->GetWndSize() );
    m_pMapInfo->InitMap( Level().name().c_str(), NULL );

    ADD_TEXT_TO_VIEW2( m_pMapInfo->GetLargeDesc(), m_pTextDesc );

    CUIXmlInit::InitStatic( xml_doc, "map_desc:image_frames_1", 0,
                            m_pFrame[ 0 ] );
    CUIXmlInit::InitStatic( xml_doc, "map_desc:image_frames_2", 0,
                            m_pFrame[ 1 ] );
    CUIXmlInit::InitStatic( xml_doc, "map_desc:image_frames_3", 0,
                            m_pFrame[ 2 ] );

    CUIXmlInit::Init3tButton( xml_doc, "map_desc:btn_next", 0, m_pBtnNext );
    CUIXmlInit::Init3tButton( xml_doc, "map_desc:btn_spectator", 0,
                              m_pBtnSpectator );
