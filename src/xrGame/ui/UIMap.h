...
public:
    ...
    virtual Fvector2 ConvertRealToLocal( const Fvector2& src,
                                         bool for_drawing );
    ...
    virtual bool GetPointerTo( const Fvector2& src,
                               float item_radius,
                               Fvector2& pos,
                               float& heading );
    ...
    Fvector2 GetCurrentZoom() const {
        return Fvector2().set( GetWndRect().height() / BoundRect().height(),
                               GetWndRect().width() / BoundRect().width() );
    }
...
protected:
    virtual void Init_internal( const shared_str& name,
                                CInifile& pLtx,
...
class CUILevelMap : public CUICustomMap {
    ...
    float m_label_scale_max; // hide label when global zoom >= this; 0 = always
                             // show
