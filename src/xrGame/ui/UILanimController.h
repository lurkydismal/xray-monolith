...
public:
    ...
    virtual void SetColorAnimation( LPCSTR lanim,
                                    u8 const& flags,
                                    float delay = 0.0f ) = 0;
...
public:
    ...
    void SetColorAnimation( LPCSTR lanim,
                            u8 const& flags,
                            float delay = 0.0f ) {
        if ( lanim && lanim[ 0 ] != 0 )
        ...
        R_ASSERT(
            ( m_lanim_clr.m_lanim == NULL ) ||
            m_lanim_clr.m_lanimFlags.test( LA_TEXTCOLOR | LA_TEXTURECOLOR ) );
    }
    ...
    virtual void RemoveColorAnimation() {
        ...
        // Reset texture color
        ColorAnimationSetTextureColor( color_rgba( 255, 255, 255, 255 ),
                                       false );
    }

    virtual void ResetColorAnimation() {
        m_lanim_clr.m_lanim_start_time =
            Device.dwTimeContinual / 1000.0f +
            m_lanim_clr.m_lanim_delay_time / 1000.0f;
    }
    ...
    void UpdateColorAnimation() {
        ...
        if ( m_lanim_clr.m_lanimFlags.test( LA_CYCLIC ) ||
             t - m_lanim_clr.m_lanim_start_time <
                 m_lanim_clr.m_lanim->Length_sec() ) {
            int frame;
            u32 clr = m_lanim_clr.m_lanim->CalculateRGB(
                t - m_lanim_clr.m_lanim_start_time, frame );
