...
void CUIMapWnd::Init( LPCSTR xml_name, LPCSTR start_from ) {
    ...
    m_map_move_step =
        uiXml.ReadAttribFlt( start_from, 0, "map_move_step", 10.0f );
    ...
    m_scroll_mode =
        ( uiXml.ReadAttribInt( start_from, 0, "scroll_enable", 0 ) == 1 )
            ? true
            : false;
    if ( m_scroll_mode ) {
        float dx, dy, sx, sy;
        ...
        m_UIMainScrollH->InitScrollBar(
            Fvector2().set( r.left + dx, r.bottom - sy ), true );
        m_UIMainScrollH->SetStepSize(
            _max( 1, ( int )( m_UILevelFrame->GetWidth() * 0.1f ) ) );
        m_UIMainScrollH->SetPageSize(
            ( int )m_UILevelFrame->GetWidth() ); // iFloor
        AttachChild( m_UIMainScrollH );
        Register( m_UIMainScrollH );
        AddCallback(
            m_UIMainScrollH, SCROLLBAR_HSCROLL,
            CUIWndCallback::void_function( this, &CUIMapWnd::OnScrollH ) );

        m_UIMainScrollV = xr_new< CUIFixedScrollBar >();
        m_UIMainScrollV->SetAutoDelete( true );
        m_UIMainScrollV->InitScrollBar(
            Fvector2().set( r.right - sx, r.top + dy ), false );
        m_UIMainScrollV->SetStepSize(
            _max( 1, ( int )( m_UILevelFrame->GetHeight() * 0.1f ) ) );
        m_UIMainScrollV->SetPageSize( ( int )m_UILevelFrame->GetHeight() );
        AttachChild( m_UIMainScrollV );
        Register( m_UIMainScrollV );
        AddCallback(
            m_UIMainScrollV, SCROLLBAR_VSCROLL,
            CUIWndCallback::void_function( this, &CUIMapWnd::OnScrollV ) );
    }
    ...
    for ( ; it != m_GameMaps.end(); ++it ) {
        ...
        for ( it2 = it; it2 != m_GameMaps.end(); ++it2 ) {
            ...
            if ( l->GlobalRect().intersected( l2->GlobalRect() ) ) {
                Msg( " --error-incorrect map definition global rect of map "
                     "[%s] intersects with [%s]",
                     *l->MapName(), *l2->MapName() );
            }
        }
        if ( FALSE ==
             l->GlobalRect().intersected( GlobalMap()->BoundRect() ) ) {
            Msg( " --error-incorrect map definition map [%s] places outside "
                 "global map",
                 *l->MapName() );
        }
    }
    ...
    m_UIScratchBox->InitPropertiesBox( Fvector2().set( 0, 0 ),
                                       Fvector2().set( 300, 300 ) );
...
void CUIMapWnd::SetTargetMap( CUICustomMap* m,
    ...
    if ( m == GlobalMap() ) {
        ...
        //		m_tgtCenter
        //= m->ConvertRealToLocalNoTransform(pos, m->BoundRect());
...
void CUIMapWnd::MapLocationRelcase( CMapLocation* ml ) {
    ...
    if ( owner ) {
        ...
        if ( ms &&
             ms->MapLocation() == ml ) // CUITaskItem also can be a HintOwner
...
bool CUIMapWnd::UpdateZoom( bool b_zoom_in, bool b_scroll_wheel ) {
    ...
    if ( b_scroll_wheel &&
         ( pda_map_zoom_in_to_mouse || pda_map_zoom_out_to_mouse ) ) {
...
void CUIMapWnd::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    ...
    if ( pWnd == m_UIPropertiesBox && msg == PROPERTY_CLICKED &&
         m_UIPropertiesBox->GetClickedItem() ) {
        CUIListBoxItem* clicked = m_UIPropertiesBox->GetClickedItem();
        for ( const SPropertyOwner& o : m_property_owners ) {
            if ( o.item == clicked ) {
                // Mods rebuild their dispatch state (stored target id,
                // label->action maps) inside pda.property_box_add_properties;
                // after a merged multi-spot build it reflects the last spot
                // enumerated. Replay it for the clicked item's owner so click
                // handlers see the state a single-spot right-click would have
                // left.
                if ( o.id != u16( 65535 ) ) {
                    m_UIScratchBox->RemoveAll();
                    AddSpotProperties( m_UIScratchBox, o.id, o.level,
                                       o.hint.c_str() );
                    m_UIScratchBox->RemoveAll();
                }
                ::luabind::functor< void > setctx;
                if ( ai().script_engine().functor( "pda.set_map_spot_context",
                                                   setctx ) )
...
static void clean_hint_title( LPCSTR hint, string256& out ) {
    ...
    if ( !hint ||
         0 ==
             xr_strcmp(
                 hint, "no hint" ) ) // map_spots XML default for hintless spots
        return;
    ...
    size_t end = s.find( '\n', start );
    s = s.substr( start,
                  end == xr_string::npos ? xr_string::npos : end - start );
...
void CUIMapWnd::GatherSpotsUnderCursor( CMapSpot* clicked,
                                        xr_vector< CMapSpot* >& out ) {
    ...
    for ( auto it = children.rbegin(); it != children.rend(); ++it ) {
        ...
        if ( std::none_of( out.begin(), out.end(), [ id ]( CMapSpot* o ) {
                 return o->MapLocation()->ObjectID() == id;
             } ) )
            out.push_back( sp );
    }
    if ( out.empty() )
...
void CUIMapWnd::AddSpotProperties( CUIPropertiesBox* box,
                                   u16 id,
                                   const shared_str& level,
                                   LPCSTR hint ) {
    ::luabind::functor< void > funct;
    if ( ai().script_engine().functor( "pda.property_box_add_properties",
                                       funct ) )
        funct( box, id, ( LPCSTR )level.c_str(), hint );
}

void CUIMapWnd::TagSubmenuOwners( CUIPropertiesBox* box,
                                  CUIListBoxItem* owner_item,
                                  u16 id,
                                  const shared_str& level,
                                  const shared_str& hint ) {
...
void CUIMapWnd::ActivatePropertiesBox( const xr_vector< CMapSpot* >& spots ) {
    ...
    for ( CMapSpot* sp : spots ) {
        ...
        CUIListBoxItem* header =
            m_UIPropertiesBox->AddHeader( title[ 0 ] ? title : level.c_str() );
...

    {
        ...
        shared_str pos_hint = ( top && top->MapLocation() )
                                  ? top->MapLocation()->GetHint()
                                  : NULL;
        u32 before = m_UIPropertiesBox->GetItemsCount();
        ...
        for ( u32 i = before; i < after; ++i ) {
            CUIListBoxItem* root_item = m_UIPropertiesBox->GetItemByIDX( i );
            m_property_owners.push_back(
                { root_item, pos_id, pos_level, pos_hint } );
...
u16 CUIMapWnd::AddRightClickMapProperties( CMapSpot* top,
                                           shared_str& out_level ) {
    u16 obj_id = ( top && top->MapLocation() ) ? top->MapLocation()->ObjectID()
                                               : u16( 65535 );
    out_level = ( top && top->MapLocation() )
                    ? top->MapLocation()->GetLevelName()
                    : shared_str( "" );

    // demonized: possibility to click trigger properties box anywhere on the
    // map with right click
    ...
    if ( ai().script_engine().functor( "_G.COnRightClickMap", rcFunct ) ) {
        ...
        if ( gm->hoveredMap ) {
            ...
            lm_real_mouse_pos.x =
                lm_bound_rect.lt.x + lm_adjusted_mouse_pos.x / lm_zoom.x;
            lm_real_mouse_pos.y = lm_bound_rect.height() + lm_bound_rect.lt.y -
                                  lm_adjusted_mouse_pos.y / lm_zoom.x;
            lm_real_mouse_pos.x /= UI().get_current_kx();

            // Get map name
            LPCSTR lm_name = lm->MapName().c_str();

            // Get Y component of real world coordinates - raycast. Only on
            // current level On other levels y will be taken from lvid Get
            // accurate lvid as well
            Fvector lm_real_pos = { lm_real_mouse_pos.x, 0,
                                    lm_real_mouse_pos.y };
            u32 lm_lvid = u32( -1 );
            if ( 0 == xr_strcmp( g_pGameLevel->name().c_str(), lm_name ) ) {
                ...
                Fvector start = { lm_real_mouse_pos.x, lm_bv.max.y,
                                  lm_real_mouse_pos.y };
                Fvector dir = { 0, -1, 0 };
                float range = lm_bv.max.y - lm_bv.min.y;
                Fvector res;
                if ( Level().ObjectSpace.RayPick(
                         start, dir, range, collide::rqtStatic, R, ignore ) ) {
            ...
            while ( gg.valid_vertex_id( current_gvid ) ) {
                ...
                const GameGraph::LEVEL_MAP& levels =
                    ai().game_graph().header().levels();
                GameGraph::LEVEL_MAP::const_iterator I =
                    levels.find( level_id );
                if ( I == levels.end() ) {
                    current_gvid++;
                    continue;
                }
                ...
                if ( 0 == xr_strcmp( level_name, lm_name ) ) {
                    ...
                    if ( d < dist ) {
                        ...
                        if ( lvid_not_set ) {
                            ...
                            /*Fvector3 lvid_pos =
                            ai().level_graph().vertex_position(lm_lvid);
            ...
            table[ "hint" ] = ( top && top->MapLocation() )
                                  ? top->MapLocation()->GetHint()
                                  : NULL;
            table[ "lvid" ] = lm_lvid;
            table[ "gvid" ] = lm_gvid;
...
void CUIMapWnd::UpdateScroll() {
    if ( m_scroll_mode ) {
        ...
        m_UIMainScrollV->SetRange( m_UIMainScrollV->GetMinRange(),
                                   iFloor( GlobalMap()->GetHeight() ) );
        m_UIMainScrollH->SetRange( m_UIMainScrollV->GetMinRange(),
                                   iFloor( GlobalMap()->GetWidth() ) );
...
void CUIMapWnd::SetZoom( float value ) {
    m_currentZoom = value;
    clamp( m_currentZoom, GlobalMap()->GetMinZoom(),
           GlobalMap()->GetMaxZoom() );
}
...
void CUIMapWnd::ShowHintSpot( CMapSpot* spot ) {
    ...
    if ( prev_spot &&
         ( prev_spot->get_location_level() < spot->get_location_level() ) ) {
        m_map_location_hint->SetInfoMSpot( spot );
        m_map_location_hint->SetOwner( spot );
        ShowHint();
        return;
...
void CUIMapWnd::SpotSelected( CUIWindow* w ) {
    ...
    if ( ai().script_engine().functor( "pda.map_spot_selected", funct ) )
        funct( ml->ObjectID(), ml->GetLevelName().c_str(), ml->spot_type );
}
