...
public:
    ...
    LPCSTR			GetHintText			();
    void			SetHintWnd			(UIHint*
    hintWnd) { m_pHint = hintWnd; }; UIHint*			GetHintWnd
    () { return m_pHint; };
    */
    //-Alundaio
    ...
    // Optional convex hit polygon (absolute coords, any vertex count): the
    // cursor must also be inside it to hover or receive mouse actions.
    ...
    IC bool HitClipPass( const Fvector2& abs_pos ) const {
        ...
        for ( u32 i = 0; i < n; ++i ) {
            ...
            const float cr = ( b.x - a.x ) * ( abs_pos.y - a.y ) -
                             ( b.y - a.y ) * ( abs_pos.x - a.x );
            if ( cr != 0.0f ) {
                if ( sign == 0.0f )
                    sign = cr;
...
protected:
    IC void SafeRemoveChild( CUIWindow* child ) {
        xrCriticalSectionGuard guard( csUi );
        WINDOW_LIST_it it =
            std::find( m_ChildWndList.begin(), m_ChildWndList.end(), child );
