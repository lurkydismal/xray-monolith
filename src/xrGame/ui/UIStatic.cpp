...
void CUIStatic::DrawText() {
    if ( m_pTextControl ) {
        if ( !fsimilar( m_pTextControl->m_wndSize.x, m_wndSize.x ) ||
             !fsimilar( m_pTextControl->m_wndSize.y, m_wndSize.y ) ) {
...
void CUIStatic::DrawTexture() {
    if ( m_bTextureEnable && GetShader() && GetShader()->inited() ) {
        ...
        m_UIStaticItem.SetPos( rect.left + m_TextureOffset.x,
                               rect.top + m_TextureOffset.y );

        if ( m_eTextureMode == tmNative ) {
            Frect r = { 0.0f, 0.0f, m_UIStaticItem.GetTextureRect().width(),
            ...
            // tmStretch and tmCover both fill the slot; cover crops the UV in
            // the renderer
            if ( Heading() ) {
            ...
            m_UIStaticItem.SetSize(
                Fvector2().set( rect.width(), rect.height() ) );
        }
...
void CUIStatic::Update() {
    ...
    if ( m_lanim_xform.m_lanim ) {
        ...
        if ( m_lanim_xform.m_lanimFlags.test( LA_CYCLIC ) ||
             t - m_lanim_xform.m_lanim_start_time <
                 m_lanim_xform.m_lanim->Length_sec() ) {
            int frame;
            u32 clr = m_lanim_xform.m_lanim->CalculateRGB(
                t - m_lanim_xform.m_lanim_start_time, frame );
            ...
            Fvector2 _sz;
            _sz.set( m_lanim_xform.m_origSize.x * f_scale,
                     m_lanim_xform.m_origSize.y * f_scale );
            SetWndSize( _sz );
        } else {
    ...
    if ( CursorOverWindow() && m_stat_hint_text.size() &&
         !g_statHint->Owner() &&
...
void CUIStatic::AdjustWidthToText() {
    if ( !m_pTextControl )
        return;
    float _len =
        m_pTextControl->GetFont()->SizeOf_( m_pTextControl->GetText() );
    UI().ClientToScreenScaledWidth( _len );
    SetWidth( _len );
}

void CUIStatic::ColorAnimationSetTextureColor( u32 color, bool only_alpha ) {
    SetTextureColor( ( only_alpha ) ? subst_alpha( GetTextureColor(), color )
                                    : color );
}

void CUIStatic::ColorAnimationSetTextColor( u32 color, bool only_alpha ) {
    TextItemControl()->SetTextColor(
        ( only_alpha ) ? subst_alpha( TextItemControl()->GetTextColor(), color )
                       : color );
}
...
void CUITextWnd::AdjustWidthToText() {
    float _len =
        TextItemControl().GetFont()->SizeOf_( TextItemControl().GetText() );
    UI().ClientToScreenScaledWidth( _len );
    SetWidth( _len );
}

void CUITextWnd::Draw() {
    if ( !fsimilar( TextItemControl().m_wndSize.x, m_wndSize.x ) ||
         !fsimilar( TextItemControl().m_wndSize.y, m_wndSize.y ) ) {
...
void CUITextWnd::ColorAnimationSetTextColor( u32 color, bool only_alpha ) {
    SetTextColor( ( only_alpha ) ? subst_alpha( GetTextColor(), color )
                                 : color );
}
