...
void UITaskListWnd::init_from_xml( CUIXml& xml, LPCSTR path ) {
    ...
    Register( m_bt_close );
    AddCallback(
        m_bt_close, BUTTON_DOWN,
        CUIWndCallback::void_function( this, &UITaskListWnd::OnBtnClose ) );
    ...
    m_list->m_sort_function =
        fastdelegate::MakeDelegate( this, &UITaskListWnd::SortingLessFunction );

    xml.SetLocalRoot( stored_root );
}

bool UITaskListWnd::OnMouseAction( float x,
                                   float y,
...
{
        u32  m_progress_task_count = Level().GameTaskManager().GetTaskCount(
eTaskStateInProgress ); CGameTask* act_task =
Level().GameTaskManager().ActiveTask(); u32 task2_index     =
Level().GameTaskManager().GetTaskIndex( act_task, eTaskStateInProgress );

        string32 buf;
        xr_sprintf( buf, sizeof(buf), "%d / %d", task2_index,
m_progress_task_count ); m_counter->SetText( buf );
}
*/
// -
// -----------------------------------------------------------------------------------------------
...
bool UITaskListWndItem::init_task( CGameTask* task, UITaskListWnd* parent ) {
    ...
    m_name =
        UIHelper::Create3tButton( xml, "second_task_wnd:task_item:name", this );
    //	m_bt_view  = UIHelper::CreateCheck(      xml,
    //"second_task_wnd:task_item:btn_view", this );
    m_st_story = UIHelper::CreateStatic(
        xml, "second_task_wnd:task_item:st_story", this );
    m_bt_focus = UIHelper::Create3tButton(
        xml, "second_task_wnd:task_item:btn_focus", this );

    m_color_states[ stt_activ ] = CUIXmlInit::GetColor(
        xml, "second_task_wnd:task_item:activ", 0, ( u32 )( -1 ) );
    m_color_states[ stt_unread ] = CUIXmlInit::GetColor(
        xml, "second_task_wnd:task_item:unread", 0, ( u32 )( -1 ) );
    m_color_states[ stt_read ] = CUIXmlInit::GetColor(
        xml, "second_task_wnd:task_item:read", 0, ( u32 )( -1 ) );
    update_view();
    return true;
}
...
void UITaskListWndItem::Update() {
    ...
    if ( m_task && m_name->CursorOverWindow() && show_hint_can ) {
        if ( Device.dwTimeGlobal > ( m_name->FocusReceiveTime() + 700 ) ) {
            show_hint = true;
            GetMessageTarget()->SendMessage( this, PDA_TASK_SHOW_HINT,
                                             ( void* )m_task );
            return;
        }
    }
...
void UITaskListWndItem::SendMessage( CUIWindow* pWnd, s16 msg, void* pData ) {
    if ( pWnd == m_bt_focus ) {
        if ( msg == BUTTON_DOWN ) {
            GetMessageTarget()->SendMessage( this, PDA_TASK_SET_TARGET_MAP,
                                             ( void* )m_task );
        }
    }
    /*
    ...
    PDA_TASK_HIDE_MAP_SPOT, (void*)m_task );
    //			Msg( " HIDE task  id = %d", m_task->m_ID );
                    ...
                    {
                            GetMessageTarget()->SendMessage( this,
    PDA_TASK_SHOW_MAP_SPOT, (void*)m_task );
...
bool UITaskListWndItem::OnMouseAction( float x,
                                       float y,
                                       EUIMessages mouse_action ) {
    if ( inherited::OnMouseAction( x, y, mouse_action ) ) {
        // return true;
