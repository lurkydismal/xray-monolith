...
void CUICDkey::assign_callbacks() {
    m_editor_control->assign_callback(
        DIK_V, text_editor::ks_Ctrl,
        Callback( this, &CUICDkey::paste_from_clipboard ) );
    m_editor_control->assign_callback(
        DIK_INSERT, text_editor::ks_Shift,
        Callback( this, &CUICDkey::paste_from_clipboard ) );
}
...
void CUICDkey::paste_from_clipboard() {
    ...
    LPSTR const new_end =
        std::remove_if( &temp[ 0 ], &temp[ 0 ] + xr_strlen( temp ),
                        inappropriate_characters() );
    *new_end = 0;
    temp[ 16 ] = 0;
...
void CUICDkey::Draw() {
    ...
    out.y =
        ( m_wndSize.y - TextItemControl()->m_pFont->CurrentHeight_() ) / 2.0f;
    out.x = TextItemControl()->m_TextOffset.x +
            TextItemControl()->GetIndentByAlign();
    TextItemControl()->m_pFont->SetColor( TextItemControl()->GetTextColor() );
    TextItemControl()->m_pFont->SetAligment(
        TextItemControl()->GetTextAlignment() );
    ...
    if ( m_bInputFocus ) {
        ...
        TextItemControl()->m_pFont->Out( pos.x, pos.y, "%s",
                                         AddHyphens( res ) );
...
void GetPlayerName_FromRegistry( char* name, u32 const name_size ) {
    ...
    {
        name[ 0 ] = 0;
        // Msg( "! Player name registry key (%s) not found !",
        // REGISTRY_VALUE_USERNAME );
        return;
    }
    ...
    // if ( xr_strlen(name) == 0 )
    //{
    //	//Msg( "! Player name in registry is empty! (%s)",
    // REGISTRY_VALUE_USERNAME );
    // }
