...
#define TEMPLATE_SPECIALIZATION                                              \
    template < typename _Graph, typename _DataStorage, typename _Parameters, \
               typename _dist_type, typename _index_type,                    \
               typename _iteration_type >

#define CGenericPathManager                                             \
    CPathManagerGeneric< _Graph, _DataStorage, _Parameters, _dist_type, \
...
TEMPLATE_SPECIALIZATION
IC void CGenericPathManager::setup( const _Graph* _graph,
...
TEMPLATE_SPECIALIZATION
IC _dist_type CGenericPathManager::evaluate( const _index_type& node_index1,
...
TEMPLATE_SPECIALIZATION
IC _dist_type
CGenericPathManager::estimate( const _index_type& vertex_id ) const {
    VERIFY( graph );
    return ( _dist_type( 0 ) );
}
...
TEMPLATE_SPECIALIZATION
IC bool CGenericPathManager::is_goal_reached(
    const _index_type& vertex_id ) const {
    return ( vertex_id == goal_node_index );
}

TEMPLATE_SPECIALIZATION
IC bool CGenericPathManager::is_limit_reached(
...
TEMPLATE_SPECIALIZATION
IC bool CGenericPathManager::is_accessible(
    const _index_type& vertex_id ) const {
    VERIFY( graph );
...
IC bool CGenericPathManager::is_metric_euclidian() const {
    // #pragma todo("Dima to Dima : implement path manager for non-euclidian
    // heuristics")
    return ( true );
}

TEMPLATE_SPECIALIZATION
IC void CGenericPathManager::begin( const _index_type& vertex_id,
...
TEMPLATE_SPECIALIZATION
IC const typename CGenericPathManager::const_iterator&
CGenericPathManager::edge( const_iterator& i ) const {
    return ( i );
}
