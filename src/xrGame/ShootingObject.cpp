...
void CShootingObject::Load( LPCSTR section ) {
    ...
    fModeShotTime = READ_IF_EXISTS( pSettings, r_float, section, "rpm_mode_2",
                                    fOneShotTime );
    ...
    m_air_resistance_factor = READ_IF_EXISTS( pSettings, r_float, section,
                                              "air_resistance_factor", 1.f );
}
...
void CShootingObject::LoadFireParams( LPCSTR section ) {
    ...
    fireDispersionBase =
        deg2rad( pSettings->r_float( section, "fire_dispersion_base" ) );

    // сила выстрела и его мощьность
    s_sHitPower = pSettings->r_string_wb(
        section, "hit_power" ); // читаем строку силы хита пули оружия
    s_sHitPowerCritical =
        pSettings->r_string_wb( section, "hit_power_critical" );
    ...
    fvHitPowerCritical[ egdNovice ] = fvHitPowerCritical[ egdStalker ] =
        fvHitPowerCritical[ egdVeteran ] =
            fvHitPowerCritical[ egdMaster ]; // изначально параметры для других
    ...
    if ( num_game_diff_param > 1 ) // если задан второй параметр хита
    {
        fvHitPower[ egdVeteran ] = ( float )atof(
            _GetItem( *s_sHitPower, 1,
                      buffer ) ); // то вычитываем его для уровня ветерана
    }
    if ( num_game_diff_param > 2 ) // если задан третий параметр хита
    {
        fvHitPower[ egdStalker ] = ( float )atof(
            _GetItem( *s_sHitPower, 2,
                      buffer ) ); // то вычитываем его для уровня сталкера
    }
    if ( num_game_diff_param > 3 ) // если задан четвёртый параметр хита
    {
        fvHitPower[ egdNovice ] = ( float )atof( _GetItem(
            *s_sHitPower, 3, buffer ) ); // то вычитываем его для уровня новичка
    }

    num_game_diff_param =
        _GetItemCount( *s_sHitPowerCritical ); // узнаём колличество параметров
    if ( num_game_diff_param > 1 ) // если задан второй параметр хита
    {
        fvHitPowerCritical[ egdVeteran ] =
...
void CShootingObject::LoadLights( LPCSTR section, LPCSTR prefix ) {
    ...
    if ( m_bLightShotEnabled ) {
        Fvector clr = pSettings->r_fvector3(
            section, strconcat( sizeof( full_name ), full_name, prefix,
                                "light_color" ) );
        light_base_color.set( clr.x, clr.y, clr.z, 1 );
        light_base_range = pSettings->r_float(
            section, strconcat( sizeof( full_name ), full_name, prefix,
                                "light_range" ) );
        ...
        light_var_range = pSettings->r_float(
            section, strconcat( sizeof( full_name ), full_name, prefix,
                                "light_var_range" ) );
        light_lifetime = pSettings->r_float(
            section,
...
void CShootingObject::Light_Render( const Fvector& P ) {
    ...
    light_render->set_color( light_build_color.r * light_scale,
                             light_build_color.g * light_scale,
                             light_build_color.b * light_scale );
    light_render->set_range(
        fmaxf( .1f, ( light_build_range * light_scale ) ) );
...
void CShootingObject::StartParticles( CParticlesObject*& pParticles,
    ...
    pParticles =
        Particles::Details::Create( particles_name, ( BOOL )auto_remove_flag )
            .get();
...
void CShootingObject::OnShellDrop( const Fvector& play_pos,
    ...
    intrusive_ptr< CParticlesObject > pShellParticles =
        Particles::Details::Create( *m_sShellParticles, TRUE );
...
void CShootingObject::StartSmokeParticles( const Fvector& play_pos,
    ...
    StartParticles( pSmokeParticles, *m_sSmokeParticlesCurrent, play_pos,
                    parent_vel, true );
}
...
void CShootingObject::StartFlameParticles() {
    ...
    StopFlameParticles();
    m_pFlameParticles =
        Particles::Details::Create( *m_sFlameParticlesCurrent, FALSE );
...
void CShootingObject::UpdateFlameParticles() {
    ...
    if ( !m_pFlameParticles->IsLooped() && !m_pFlameParticles->IsPlaying() &&
...
void CShootingObject::FireBullet( const Fvector& pos,
    ...
    /*if (m_bUseAimBullet)
    {
            if (ParentMayHaveAimBullet())
            {
                    ...
                    {
                            if ((Device.fTimeGlobal - m_fPredBulletTime) >=
    m_fTimeToAim)
                            {
                                    aim_bullet = true;
                            }
    ...
    Level().BulletManager().AddBullet(
        pos, dir, m_fStartBulletSpeed * cur_silencer_koef.bullet_speed,
        ...
        m_air_resistance_factor, send_hit, aim_bullet, iShotNum );
}
