...
void CAgentManagerPlanner::add_evaluators() {
    add_evaluator( ePropertyOrders,
                   xr_new< CAgentManagerPropertyEvaluatorConst >(
                       false, "property_order" ) );
    add_evaluator( ePropertyItem, xr_new< CAgentManagerPropertyEvaluatorItem >(
                                      &object(), "property_item" ) );
    add_evaluator( ePropertyEnemy,
                   xr_new< CAgentManagerPropertyEvaluatorEnemy >(
                       &object(), "property_enemy" ) );
    add_evaluator( ePropertyDanger,
                   xr_new< CAgentManagerPropertyEvaluatorDanger >(
                       &object(), "property_danger" ) );
}
...
void CAgentManagerPlanner::add_actions() {
    ...
    action =
        xr_new< CAgentManagerActionGatherItems >( &object(), "gather_items" );
    ...
    action = xr_new< CAgentManagerActionReactOnDanger >( &object(),
                                                         "react_on_danger" );
    add_condition( action, ePropertyEnemy, false );
