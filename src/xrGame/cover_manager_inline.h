...
IC bool CCoverManager::inertia( Fvector const& position,
                                float radius,
...
IC const CCoverPoint* CCoverManager::best_cover(
    ...
    if ( last ) {
        if ( position.distance_to_sqr( last->position() ) <
             _sqr( 3 * radius ) ) {
    ...
    typename xr_vector< CCoverPoint* >::const_iterator I = m_nearest.begin();
    typename xr_vector< CCoverPoint* >::const_iterator E = m_nearest.end();
    for ( ; I != E; ++I ) {
...
IC const CCoverPoint* CCoverManager::best_cover(
    const Fvector& position,
    float radius,
    _evaluator_type& evaluator ) const {
    return ( best_cover< _evaluator_type, CCoverManager >( position, radius,
                                                           evaluator, *this ) );
}
