...
void CObjectHandlerPlanner::add_evaluators( CMissile* missile ) {
    ...
    add_evaluator(
        ...
        xr_new< CObjectPropertyEvaluatorMissileHidden >( missile, m_object ) );
    add_evaluator(
        uid( id, eWorldPropertyThrowStarted ),
    ...
    add_evaluator( uid( id, eWorldPropertyThrow ),
                   xr_new< CObjectPropertyEvaluatorMissile >(
                       missile, m_object, CMissile::eThrowEnd ) );
    ...
    //,xr_new<CObjectPropertyEvaluatorConst>(false)); 	add_evaluator
...
void CObjectHandlerPlanner::add_operators( CMissile* missile ) {
    ...
    // show
    action =
        xr_new< CObjectActionShow >( missile, m_object, &m_storage, "show" );
    ...
    // hide
    action =
        xr_new< CObjectActionHide >( missile, m_object, &m_storage, "hide" );
    ...
    // drop
    action =
        xr_new< CObjectActionDrop >( missile, m_object, &m_storage, "drop" );
    add_condition( action, id, eWorldPropertyHidden, false );
    add_effect( action, id, eWorldPropertyDropped, true );
    add_operator( uid( id, eWorldOperatorDrop ), action );

    // idle
    action = xr_new< CObjectActionIdleMissile >( missile, m_object, &m_storage,
                                                 "idle" );
    add_condition( action, id, eWorldPropertyHidden, false );
    ...
    // fire start
    action = xr_new< CObjectActionThrowMissile >( missile, m_object, &m_storage,
                                                  "throw start" );
    add_condition( action, id, eWorldPropertyHidden, false );
    ...
    // fire throw
    action = xr_new< CSObjectActionBase >( missile, m_object, &m_storage,
                                           "throwing" );
    add_condition( action, id, eWorldPropertyHidden, false );
