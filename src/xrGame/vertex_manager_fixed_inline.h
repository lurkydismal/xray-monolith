...
#define CFixedVertexManager                                                \
    CVertexManagerFixed< _path_id_type, _index_type, mask >::CDataStorage< \
...
IC void CFixedVertexManager::init() {
    ...
    if ( !m_current_path_id ) {
        ZeroMemory( m_indexes,
                    ( m_max_node_count ) * sizeof( CGraphIndexVertex ) );
        ++m_current_path_id;
    }
}
...
TEMPLATE_SPECIALIZATION
IC typename CFixedVertexManager::CGraphVertex& CFixedVertexManager::get_node(
...
CFixedVertexManager::create_vertex( CGraphVertex& vertex,
...
TEMPLATE_SPECIALIZATION
IC typename CFixedVertexManager::_path_id_type
CFixedVertexManager::current_path_id() const {
