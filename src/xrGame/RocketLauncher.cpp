...
void CRocketLauncher::SpawnRocket( const shared_str& rocket_section,
                                   CGameObject* parent_rocket_launcher ) {
    if ( OnClient() )
        return;
    ...
    l_tpTemporary->m_tNodeID =
        ( g_dedicated_server )
            ? u32( -1 )
            : parent_rocket_launcher->ai_location().level_vertex_id();
...
void CRocketLauncher::AttachRocket( u16 rocket_id,
                                    CGameObject* parent_rocket_launcher ) {
    CCustomRocket* pRocket =
        smart_cast< CCustomRocket* >( Level().Objects.net_Find( rocket_id ) );
    pRocket->m_pOwner =
        smart_cast< CGameObject* >( parent_rocket_launcher->H_Root() );
...
void CRocketLauncher::DetachRocket( u16 rocket_id, bool bLaunch ) {
    ...
    ROCKETIT It_l = std::find( m_launched_rockets.begin(),
                               m_launched_rockets.end(), pRocket );
...
void CRocketLauncher::LaunchRocket( const Fmatrix& xform,
    ...
    VERIFY2( _valid( xform ),
             "CRocketLauncher::LaunchRocket. Invalid xform argument!" );
