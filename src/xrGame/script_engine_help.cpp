...
xr_string member_to_string( ::luabind::object const& e,
    ...
    if ( e.type() == LUA_TFUNCTION ) {
...

        {
            ...
            detail::stack_pop p2( L, 1 );
            detail::method_rep* m =
                static_cast< detail::method_rep* >( lua_touserdata( L, -1 ) );

            for ( std::vector< detail::overload_rep >::const_iterator i =
                      m->overloads().begin();
                  i != m->overloads().end(); ++i ) {
...
void print_class( lua_State* L, ::luabind::detail::class_rep* crep ) {
    ...
    {
        S = ( crep->get_class_type() !=
              ::luabind::detail::class_rep::cpp_class )
                ? "LUA class "
                : "C++ class ";
        S.append( crep->name() );
        typedef ::luabind::internal_vector<
    ...
    {
        const ::luabind::detail::class_rep::STATIC_CONSTANTS& constants =
            crep->static_constants();
        ::luabind::detail::class_rep::STATIC_CONSTANTS::const_iterator I =
            constants.begin();
        ::luabind::detail::class_rep::STATIC_CONSTANTS::const_iterator E =
            constants.end();
        for ( ; I != E; ++I )
#ifndef USE_NATIVE_LUA_STRINGS
            Msg( "    const %s = %d;", ( *I ).first, ( *I ).second );
#else
            Msg( "    const %s = %d;", getstr( ( *I ).first.m_object ),
                 ( *I ).second );
#endif
        if ( !constants.empty() )
            Msg( "    " );
    ...
    {
#ifndef USE_NATIVE_LUA_STRINGS
        typedef ::luabind::internal_map< const char*,
                                         ::luabind::detail::class_rep::callback,
                                         ::luabind::detail::ltstr >
            PROPERTIES;
#else
    ...
    {
        typedef ::luabind::internal_vector<
            ::luabind::detail::construct_rep::overload_t >
            Constructors;
    ...
    // print class methods
    {
        ...
        table.set();
        for ( ::luabind::object::iterator i = table.begin(); i != table.end();
              ++i ) {
...
void print_free_functions( lua_State* L,
            ...
            if ( lua_getupvalue( L, -1, 2 ) != 0 ) {
                ...
                if ( lua_touserdata( L, -1 ) == ( void* )0x1337 ) {
                    if ( lua_getupvalue( L, -2, 1 ) != 0 ) {
                        ...
                        ++count;
                        rep = static_cast<
                            ::luabind::detail::free_functions::function_rep* >(
                            lua_touserdata( L, -1 ) );
                        std::vector< ::luabind::detail::free_functions::
                                         overload_rep >::const_iterator i =
                            rep->overloads().begin();
                        std::vector< ::luabind::detail::free_functions::
                                         overload_rep >::const_iterator e =
                            rep->overloads().end();
                        for ( ; i != e; ++i ) {
                            ::luabind::internal_string luaS;
                            ( *i ).get_signature( L, luaS );
                            xr_string S( luaS.c_str() );
                            Msg( "    %sfunction %s%s;", indent.c_str(),
                                 rep->name(), process_signature( S ).c_str() );
                        }
                        lua_pop( L, 1 );
                    }
                }
...
void print_help( lua_State* L ) {
    ...
    ::luabind::detail::class_registry::get_registry( L )->iterate_classes(
        L, &print_class );
