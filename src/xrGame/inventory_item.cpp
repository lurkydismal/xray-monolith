...
CInventoryItem::~CInventoryItem() {
    ...
    bool B_GOOD =
        ( !m_pInventory ||
          ( std::find( m_pInventory->m_all.begin(), m_pInventory->m_all.end(),
                       this ) == m_pInventory->m_all.end() ) );
    if ( !B_GOOD ) {
        ...
        Msg( "! ERROR item_id[%d] H_Parent=[%s][%d] [%d]", object().ID(),
             p ? p->cName().c_str() : "none", p ? p->ID() : -1,
             Device.dwFrame );
    }
...
void CInventoryItem::Load( LPCSTR section ) {
    CHitImmunity::LoadImmunities(
        pSettings->r_string( section, "immunities_sect" ), pSettings );

    if ( cast_game_object() ) {
        cast_game_object()->SpatialComponent->spatial.type |=
            STYPE_VISIBLEFORAI;
    }

    m_section_id._set( section );
    m_name =
        CStringTable().translate( pSettings->r_string( section, "inv_name" ) );
    m_nameShort = CStringTable().translate(
    ...
    m_Description = CStringTable().translate(
        pSettings->r_string( section, "description" ) );

    m_flags.set( Fbelt,
                 READ_IF_EXISTS( pSettings, r_bool, section, "belt", FALSE ) );
    m_can_trade =
        READ_IF_EXISTS( pSettings, r_bool, section, "can_trade", TRUE );
    m_flags.set( FCanTake, READ_IF_EXISTS( pSettings, r_bool, section,
                                           "can_take", TRUE ) );
    m_flags.set( FCanTrade, m_can_trade );
    m_flags.set( FIsQuestItem, READ_IF_EXISTS( pSettings, r_bool, section,
                                               "quest_item", FALSE ) );
    ...
    m_highlight_equipped = !!READ_IF_EXISTS( pSettings, r_bool, section,
                                             "highlight_equipped", FALSE );

    if ( BaseSlot() != NO_ACTIVE_SLOT || Belt() ) {
        m_flags.set( FRuckDefault,
                     pSettings->r_bool( section, "default_to_ruck" ) );
        m_flags.set( FAllowSprint,
                     pSettings->r_bool( section, "sprint_allowed" ) );
        m_fControlInertionFactor =
            pSettings->r_float( section, "control_inertion_factor" );
    }
    m_icon_name =
        READ_IF_EXISTS( pSettings, r_string, section, "icon_name", NULL );

    m_fLowestBatteryCharge =
        READ_IF_EXISTS( pSettings, r_float, section, "power_critical", .03f );
}
...
LPCSTR CInventoryItem::NameComplex()
{
        ...
        if( m_flags.test(FUsingCondition) ){
                string32		cond;
                if(GetCondition()<0.33)		xr_strcpy		(cond,
"[poor]"); else if(GetCondition()<0.66)xr_strcpy		(cond,	"[bad]"
); else						xr_strcpy		(cond,
"[good]"); string256		temp; strconcat		(temp,*m_nameComplex,"
",cond)	;
                // xr_sprintf			(temp,"%s
%s",*m_nameComplex,cond); m_nameComplex	= temp;
        }
...
void CInventoryItem::OnEvent( NET_Packet& P, u16 type ) {
    switch ( type ) {
        case GE_ADDON_ATTACH: {
            u16 ItemID;
            P.r_u16( ItemID );
            CInventoryItem* ItemToAttach = smart_cast< CInventoryItem* >(
                Level().Objects.net_Find( ItemID ) );
            if ( !ItemToAttach )
                break;
...
bool CInventoryItem::Detach( const char* item_section_name,
    ...
    if ( b_spawn_item ) {
        ...
        l_tpALifeDynamicObject->m_tNodeID =
            ( g_dedicated_server ) ? u32( -1 )
        ...
        {
            // but it is simpliest way to avoid exception in MP BuyWnd...
            // [Satan]
            if ( object().H_Parent() )
                D->ID_Parent = u16( object().H_Parent()->ID() );
            else
...
BOOL CInventoryItem::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( alife_object ) {
        m_flags.set( Fuseful_for_NPC, alife_object->m_flags.test(
                                          CSE_ALifeObject::flUsefulForAI ) );
    }

    CSE_ALifeInventoryItem* pSE_InventoryItem =
        smart_cast< CSE_ALifeInventoryItem* >( e );
...
void CInventoryItem::net_Destroy() {
    if ( m_pInventory ) {
        VERIFY( std::find( m_pInventory->m_all.begin(),
...
void CInventoryItem::net_Import( NET_Packet& P ) {
    ...
    // Msg("Inventory item [%d][%s] net_Import...", object().ID(),
    // object().cName().c_str());
    u8 NumItems = 0;
    NumItems = P.r_u8();
    if ( !NumItems )
    ...
    //(p->NET_IItem.back().dwTimeStamp>=N.dwTimeStamp)) 		return;
    ...
    if ( !m_activated ) {
#ifdef DEBUG
        Msg( "Activating object [%d] before interpolation starts",
             object().ID() );
#endif // #ifdef DEBUG
        object().processing_activate();
        m_activated = true;
    ...
    N.State.enabled				= num_items.mask &
    CSE_ALifeInventoryItem::inventory_item_state_enabled; if (!(num_items.mask &
    ...
    { N.State.linear_vel.x	= P.r_float_q8(-32.f,32.f); N.State.linear_vel.y
    = P.r_float_q8(-32.f,32.f); N.State.linear_vel.z	=
    P.r_float_q8(-32.f,32.f);
    }
    else
...
void CInventoryItem::net_Import_PH_Params( NET_Packet& P,
                                           net_update_IItem& N,
                                           mask_inv_num_items& num_items ) {
    // N.State.force.set			(0.f,0.f,0.f);
    // N.State.torque.set			(0.f,0.f,0.f);
    ...
    // UI().Font().pFontStat->OutNext("Import
    // N.State.enabled:%i",int(N.State.enabled));
    if ( !( num_items.mask &
            CSE_ALifeInventoryItem::inventory_item_angular_null ) ) {
...
void CInventoryItem::net_Export_PH_Params( NET_Packet& P,
                                           SPHNetState& State,
                                           mask_inv_num_items& num_items ) {
    // UI().Font().pFontStat->OutSet(100.0f,100.0f);
    ...
    // UI().Font().pFontStat->OutNext("Export
    // State.torque:%4.6f",State.torque.magnitude());
    ...
    if ( !( num_items.mask &
            CSE_ALifeInventoryItem::inventory_item_angular_null ) ) {
        /*	clamp
        (State.angular_vel.x,-10.f*PI_MUL_2,10.f*PI_MUL_2); clamp
        (State.angular_vel.y,-10.f*PI_MUL_2,10.f*PI_MUL_2); clamp
        (State.angular_vel.z,-10.f*PI_MUL_2,10.f*PI_MUL_2);*/
...
void CInventoryItem::net_Export( NET_Packet& P ) {
    ...
    // if (m_pPhysicsShell->PPhysicsShellAnimator())		{num_items.mask
    // |= CSE_ALifeObjectPhysic::animated;}
    ...
    num_items.mask			= 0;
    u16						temp =
    object().PHGetSyncItemsNumber(); R_ASSERT				(temp <
    ...
    float					magnitude =
    _sqrt(State.quaternion.magnitude()); if (fis_zero(magnitude)) { magnitude
    = 1; State.quaternion.x	= 0.f; State.quaternion.y	= 0.f;
            State.quaternion.z	= 1.f;
            State.quaternion.w	= 0.f;
    }
    else {
    ...
    (State.angular_vel.z,0.f,10.f*PI_MUL_2);
    ...
    if (!(num_items.mask & CSE_ALifeInventoryItem::inventory_item_linear_null))
    { clamp				(State.linear_vel.x,-32.f,32.f); clamp
    (State.linear_vel.y,-32.f,32.f); clamp
    (State.linear_vel.z,-32.f,32.f);
...
void CInventoryItem::PH_I_CrPr() // actions & operations between two phisic
                                 // prediction steps
{
    /*net_updateData* p					= NetSync();
    //store recalculated data, then we able to restore it after small future
    prediction if (!object().CrPr_IsActivated())	return;
    ////////////////////////////////////
    CPHSynchronize* pSyncObj			= NULL;
    pSyncObj = object().PHGetSyncItem	(0);
    if (!pSyncObj)						return;
    ...
    ///////////////////////////////////////////////
    Fmatrix xformX;
    pSyncObj->cv2obj_Xfrom(p->RecalculatedState.quaternion,
    p->RecalculatedState.position, xformX);

    VERIFY2
    (_valid(xformX),*object().cName()); pSyncObj->cv2obj_Xfrom
    (p->RecalculatedState.quaternion, p->RecalculatedState.position, xformX);

    p->IRecRot.set(xformX);
    p->IRecPos.set(xformX.c);
    VERIFY2
    (_valid(p->IRecPos),*object().cName());*/
};
...
void CInventoryItem::PH_Ch_CrPr() {
    ...
    {
            if (p->CheckState.enabled == false && p->RecalculatedState.enabled
    == true)
            {
                    ///////////////////////////////////////////////////////////////////
...
void CInventoryItem::PH_A_CrPr() {
    if ( m_just_after_spawn ) {
        ...
        VERIFY( K );
        if ( !object().PPhysicsShell() ) {
            Msg( "! ERROR: PhysicsShell is NULL, object [%s][%d]",
                 object().cName().c_str(), object().ID() );
        ...
        object().PPhysicsShell()->GetGlobalTransformDynamic(
            &object().XFORM() );
        K->CalculateBones_Invalidate();
        K->CalculateBones( TRUE );
#if 0
    ...
    ////////////////////////////////////
    Fmatrix xformX;
    pSyncObj->cv2obj_Xfrom(p->PredictedState.quaternion,
    p->PredictedState.position, xformX);

    VERIFY2
    (_valid(xformX),*object().cName()); pSyncObj->cv2obj_Xfrom
    (p->PredictedState.quaternion, p->PredictedState.position, xformX);

    p->IEndRot.set						(xformX);
    p->IEndPos.set						(xformX.c);
    VERIFY2
    (_valid(p->IEndPos),*object().cName());
    /////////////////////////////////////////////////////////////////////////
...
void CInventoryItem::CalculateInterpolationParams()
{
        ...
        if (m_flags.test(FInInterpolation))
        {
                ...
                float factor	= float(CurTime -
p->m_dwIStartTime)/(p->m_dwIEndTime - p->m_dwIStartTime); if (factor > 1.0f)
factor = 1.0f;
                ...
                {
                        P0[k] =
c*(c*(c*p->SCoeff[k][0]+p->SCoeff[k][1])+p->SCoeff[k][2])+p->SCoeff[k][3]; P1[k]
= (c*c*p->SCoeff[k][0]*3+c*p->SCoeff[k][1]*2+p->SCoeff[k][2])/3; // сокрость из
формулы в 3 раза превышает скорость при расчете коэффициентов !!!!
                };
                P0.set(p->IStartPos);
                P1.add(p->IStartPos);
        }
        else
        {
                ...
                {
                        pSyncObj->cv2obj_Xfrom(p->RecalculatedState.previous_quaternion,
p->RecalculatedState.previous_position, xformX0);
                        pSyncObj->cv2obj_Xfrom(p->RecalculatedState.quaternion,
p->RecalculatedState.position, xformX1);
                }
                else
                {
                        pSyncObj->cv2obj_Xfrom(p->LastState.previous_quaternion,
p->LastState.previous_position, xformX0);
                        pSyncObj->cv2obj_Xfrom(p->LastState.quaternion,
p->LastState.position, xformX1);
                };
        ...
        pSyncObj->cv2obj_Xfrom(p->PredictedState.quaternion,
p->PredictedState.position, xformX1); P3.set(xformX1.c);
        /////////////////////////////////////////////////////////////////////////////
        Fvector TotalPath;
        TotalPath.sub(P3, P0);
        ...
        u32		ConstTime = u32((fixed_step -
ph_world->m_frame_time)*1000)+
...
void CInventoryItem::make_Interpolation	()
{
        ...
        if(!object().H_Parent() && object().getVisible() &&
object().m_pPhysicsShell && m_flags.test(FInInterpolation) )
        {
                ...
                if (CurTime >= p->m_dwIEndTime)
                {
                        ...
                        pSyncObj = object().PHGetSyncItem(0);
                        pSyncObj->set_State
(p->PredictedState); Fmatrix xformI; pSyncObj->cv2obj_Xfrom
(p->PredictedState.quaternion, p->PredictedState.position, xformI); VERIFY2
(_valid(object().renderable.xform),*object().cName()); object().XFORM().set
(xformI); VERIFY2
(_valid(object().renderable.xform),*object().cName());
                }
                else
                {
                        VERIFY			(CurTime <= p->m_dwIEndTime);
                        float factor	= float(CurTime -
p->m_dwIStartTime)/(p->m_dwIEndTime - p->m_dwIStartTime); if (factor > 1) factor
= 1.0f; else if (factor < 0) factor = 0;
                        ...
                        {
                                IPos[k] =
c*(c*(c*p->SCoeff[k][0]+p->SCoeff[k][1])+p->SCoeff[k][2])+p->SCoeff[k][3];
                        };

                        VERIFY2
(_valid(IPos),*object().cName()); VERIFY			(factor>=0.f &&
factor<=1.f); IRot.slerp(p->IStartRot, p->IEndRot, factor); VERIFY2
(_valid(IRot),*object().cName()); object().XFORM().rotation(IRot); VERIFY2
(_valid(object().renderable.xform),*object().cName());
...
void CInventoryItem::Interpolate() {
    ...
    if ( !object().H_Parent() && object().getVisible() &&
         object().m_pPhysicsShell && !OnServer() && p->NET_IItem.size() ) {
        SPHNetState newState = p->NET_IItem.front().State;

        if ( p->NET_IItem.size() >= 2 ) {
            float ret_interpolate = interpolate_states(
                p->NET_IItem.front(), p->NET_IItem.back(), newState );
            // Msg("Interpolation factor is %0.4f", ret_interpolate);
            // Msg("Current position is: x = %3.3f, y = %3.3f, z = %3.3f",
            // newState.position.x, newState.position.y, newState.position.z);
            if ( ret_interpolate >= 1.f ) {
                p->NET_IItem.pop_front();
                if ( m_activated ) {
#ifdef DEBUG
                    Msg( "Deactivating object [%d] after interpolation finish",
                         object().ID() );
#endif // #ifdef DEBUG
...
float CInventoryItem::interpolate_states( net_update_IItem const& first,
    ...
    float factor = float( CurTime - last.dwTimeStamp ) /
                   float( last.dwTimeStamp - first.dwTimeStamp );
    ...
    current.position.x =
        ...
        ( factor * ( last.State.position.x - first.State.position.x ) );
    current.position.y =
        first.State.position.y +
        ( factor * ( last.State.position.y - first.State.position.y ) );
    current.position.z =
        first.State.position.z +
        ( factor * ( last.State.position.z - first.State.position.z ) );
    current.previous_position = current.position;

    current.quaternion.slerp( first.State.quaternion, last.State.quaternion,
                              factor );
...
void CInventoryItem::reload( LPCSTR section ) {
    ...
    m_holder_range_modifier = READ_IF_EXISTS( pSettings, r_float, section,
                                              "holder_range_modifier", 1.f );
    m_holder_fov_modifier = READ_IF_EXISTS( pSettings, r_float, section,
                                            "holder_fov_modifier", 1.f );
}
...
const CInventoryItem* CInventoryItem::can_kill(
    const xr_vector< const CGameObject* >& items ) const {
    return ( 0 );
}

CInventoryItem* CInventoryItem::can_make_killing(
...
void CInventoryItem::UpdateXForm() {
    ...
    if ( boneR == -1 )
        return;
    //	if ((HandDependence() == hd1Hand) || (STATE == eReload) ||
    //(!E->g_Alive())) 		boneL = boneR2;
...
void CInventoryItem::OnRender() {
    if ( bDebug && object().Visual() ) {
        ...
        M.c.add( bc );
        Level().debug_renderer().draw_obb( M, bd,
        ...
        //		Level().debug_renderer().draw_obb
        (M,bd,Color); float size = 0.01f; if (!H_Parent())
                        {
                                Level().debug_renderer().draw_aabb
        (Position(), size, size, size, color_rgba(0, 255, 0, 255));
        ...
        Pos1, Pos2, color_rgba(255, 255, 0, 255)); Pos1 = Pos2;
                                };
                        ...
                        if (OnClient() && !H_Parent() && m_bInInterpolation)
                        {
                                ...
                                Level().debug_renderer().draw_aabb
        (IRecPos, size, size, size, color_rgba(255, 0, 255, 255));

                                xformI.rotation(IEndRot);
                                xformI.c.set(IEndPos);
                                Level().debug_renderer().draw_obb
        (xformI,bd,color_rgba(0, 255, 0, 255));
                                ...
                                for (float i=0.1f; i<1.1f; i+= 0.1f)
                                {
                                        ...
                                        {
                                                point1[k] =
        c*(c*(c*SCoeff[k][0]+SCoeff[k][1])+SCoeff[k][2])+SCoeff[k][3];
                                        };
                                        Level().debug_renderer().draw_line(Fidentity,
        point0, point1, color_rgba(0, 0, 255, 255)); point0.set(point1);
                                };
                        };
                        */
    };
...
bool CInventoryItem::CanTrade() const {
    bool res = true;
#pragma todo( \
    "Dima to Andy : why CInventoryItem::CanTrade can be called for the item, which doesn't have owner?" )
    if ( m_pInventory )
...
Frect CInventoryItem::GetKillMsgRect() const {
    ...
    x = READ_IF_EXISTS( pSettings, r_float, m_object->cNameSect(), "kill_msg_x",
                        0.0f );
    y = READ_IF_EXISTS( pSettings, r_float, m_object->cNameSect(), "kill_msg_y",
                        0.0f );
    w = READ_IF_EXISTS( pSettings, r_float, m_object->cNameSect(),
                        "kill_msg_width", 0.0f );
    h = READ_IF_EXISTS( pSettings, r_float, m_object->cNameSect(),
                        "kill_msg_height", 0.0f );
...
Irect CInventoryItem::GetUpgrIconRect() const {
    ...
    x = READ_IF_EXISTS( pSettings, r_u32, m_object->cNameSect(), "upgr_icon_x",
                        0 );
    y = READ_IF_EXISTS( pSettings, r_u32, m_object->cNameSect(), "upgr_icon_y",
                        0 );
    w = READ_IF_EXISTS( pSettings, r_u32, m_object->cNameSect(),
                        "upgr_icon_width", 0 );
    h = READ_IF_EXISTS( pSettings, r_u32, m_object->cNameSect(),
                        "upgr_icon_height", 0 );
...
void CInventoryItem::SetDropManual( BOOL val ) {
    ...
    if ( !IsGameTypeSingle() ) {
        if ( !!m_name ) {
            Msg( "! WARNING: trying to set drop manual flag to item [%d][%s] "
                 "to %d",
                 object_id(), m_name.c_str(), val );
        }
    }
#endif // #ifdef DEBUG
    if ( !IsGameTypeSingle() ) {
