...
void CWeapon::FireTrace( const Fvector& P, const Fvector& D ) {
    ...
    //-------------------------------------------------------------
    bool is_tracer =
        m_bHasTracers && !!l_cartridge.m_flags.test( CCartridge::cfTracer );
    if ( is_tracer && !IsGameTypeSingle() )
        is_tracer = is_tracer /*&& (m_magazine.size() % 3 == 0)*/ &&
                    !IsSilencerAttached();
    ...
    ChangeCondition( -GetWeaponDeterioration() * l_cartridge.param_s.impair *
                     cur_silencer_koef.condition_shot_dec );
    ...
    if ( !IsGameTypeSingle() ) {
        ...
        if ( tmp_actor ) {
            ...
            if ( m_first_bullet_controller.is_bullet_first(
                     state.fVelocity ) ) {
        ...
        if ( tmp_mp_game->get_reward_generator() )
            tmp_mp_game->get_reward_generator()->OnWeapon_Fire(
                H_Parent()->ID(), ID() );
    }
    ...
    for ( int i = 0; i < l_cartridge.param_s.buckShot; ++i ) {
        FireBullet( P, D, fire_disp, l_cartridge, H_Parent()->ID(), ID(),
                    SendHit, iAmmoElapsed );
    }
    ...
    g_pGamePersistent->GrassBendersAddShot(
        cast_game_object()->ID(), ShotPos, D, 3.0f, 20.0f,
        ps_ssfx_int_grass_params_2.z, ps_ssfx_int_grass_params_2.w );
...
void CWeapon::FireBullet( const Fvector& pos,
                          ...
                          bool send_hit,
                          int iShotNum ) {
    CShootingObject::FireBullet( pos, shot_dir, fire_disp, cartridge, parent_id,
                                 weapon_id, send_hit, iShotNum );
...
void CWeapon::StartFlameParticles2() {
    CShootingObject::StartParticles( m_pFlameParticles2, *m_sFlameParticles2,
                                     get_LastFP2() );
}
