...
public:
    ...
    virtual void OnItemRuck( CInventoryItem* inventory_item,
                             const SInvItemPlace& previous_place );
    virtual void OnItemBelt( CInventoryItem* inventory_item,
                             const SInvItemPlace& previous_place );
...
protected:
    ...
    bool m_bOutBorder;
    // сохраняет счетчик объектов в feel_touch, для которых необходимо обновлять
    // размер колижена с актером
...
public:
    ...
    //////////////////////////////////////////////////////////////////////////
public:
    void g_cl_CheckControls( u32 mstate_wf,
...
public:
    ...
    virtual BOOL net_Relevant(); //	{ return getSVU() | getLocal(); };
...
protected:
    ...
    net_update NET_Last;
    BOOL NET_WasInterpolating; // previous update was by interpolation or by
                               // extrapolation
    ...
    Fvector IPosS, IPosH,
        IPosL; // положение актера после интерполяции Бизье, Эрмита, линейной
...
public:
    ...
    virtual void PH_I_CrPr(); // actions & operations after correction before
                              // prediction steps
    virtual void PH_A_CrPr(); // actions & operations after phisic
                              // correction-prediction steps
    ...
    virtual void RenderIndicator( Fvector dpos,
                                  float r1,
                                  float r2,
                                  const ui_shader& IndShader );
    virtual void RenderText( LPCSTR Text,
                             Fvector dpos,
                             float* pdup,
                             u32 color );
...
private: // IPhysicsShellHolder
    virtual void _BCL HideAllWeapons( bool v ) {
        SetWeaponHideState( INV_STATE_BLOCK_ALL, v );
    }
...
protected:
    virtual CEntityConditionSimple* create_entity_condition(
...
private:
    ...
    BOOL CanPickItem( const CFrustum& frustum,
                      const Fvector& from,
                      CObject* item );
