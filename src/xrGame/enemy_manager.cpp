...
bool CEnemyManager::useful( const CEntityAlive* entity_alive ) const {
    ...
    if ( ( entity_alive->SpatialComponent->spatial.type &
           STYPE_VISIBLEFORAI ) != STYPE_VISIBLEFORAI )
        return ( false );

    if ( ( m_object->ID() == entity_alive->ID() ) ||
         !m_object->is_relation_enemy( entity_alive ) )
        return ( false );

    if ( !ai().get_level_graph() ||
         !ai().level_graph().valid_vertex_id(
             entity_alive->ai_location().level_vertex_id() ) )
        return ( false );

    if ( m_object->human_being() && !entity_alive->human_being() &&
         !expedient( entity_alive ) &&
         ( evaluate( entity_alive ) >= m_ignore_monster_threshold ) &&
         ( m_object->Position().distance_to( entity_alive->Position() ) >=
           m_max_ignore_distance ) )
        return ( false );

    // Disable caching if time is negative for testing
    if ( enemy_manager_useful_cache_time < 0 )
        return ( m_useful_callback
                     ? m_useful_callback( m_object->lua_game_object(),
                                          entity_alive->lua_game_object() )
                     : true );
    ...
    bool result = ( m_useful_callback
                        ? m_useful_callback( m_object->lua_game_object(),
                                             entity_alive->lua_game_object() )
                        : true );

    // Add id based jitter so that next updates will be spread between frames
    // for different entities
    int jitter =
        ( entity_alive->ID() % 97 + 1 ) * ( entity_alive->ID() & 1 ? -1 : 1 );
    u32 next_time =
        current_time + _max( 0, enemy_manager_useful_cache_time + jitter );
    cache.result = result;
...
float CEnemyManager::evaluate( const CEntityAlive* object ) const {
    ...
    bool wounded =
        stalker ? stalker->wounded( &m_object->movement().restrictions() )
                : false;
    if ( wounded ) {
        if ( m_stalker && m_stalker->agent_manager().enemy().assigned_wounded(
                              object, m_stalker ) )
            return ( 0.f );

        float distance =
            m_object->Position().distance_to_sqr( object->Position() );
        return ( distance );
    }
    ...
    if ( object->ID() == m_object->memory().hit().last_hit_object_id() ) {
        ...
        // but WON'T ignore a guy actively fighting them at 5m just because they
        // got shot!
        if ( hit_dist < 30.f )
            penalty -= 5.f;

        // For medium/long range, give a standard 100m aggro advantage
        // so they still react to snipers if they aren't busy with a close
        // target.
        else
            penalty -= 100.f;
    }
    ...
    return ( penalty + distance / 100.f
             // + ai().ef_storage().m_pfVictoryProbability->ffGetValue() / 100.f
             // //SkyKi: Removed to stop AI from locking onto heavily armed
             // targets (like the player) across the map
    );
#else  // USE_EVALUATOR
    float distance = m_object->Position().distance_to_sqr( object->Position() );
...
void CEnemyManager::reload( LPCSTR section ) {
    m_ignore_monster_threshold = READ_IF_EXISTS(
        pSettings, r_float, section, "ignore_monster_threshold", 1.f );
    m_max_ignore_distance = READ_IF_EXISTS( pSettings, r_float, section,
                                            "max_ignore_distance", 0.f );
    m_last_enemy_time = 0;
    m_last_enemy = 0;
    m_last_enemy_change = 0;
...
void CEnemyManager::remove_links( CObject* object ) {
    ...
    OBJECTS::iterator I = std::find( m_objects.begin(), m_objects.end(),
                                     ( CEntityAlive* )object );
    if ( I != m_objects.end() )
...
void CEnemyManager::ignore_monster_threshold(
...
void CEnemyManager::restore_ignore_monster_threshold() {
    m_ignore_monster_threshold =
        READ_IF_EXISTS( pSettings, r_float, *m_object->cNameSect(),
                        "ignore_monster_threshold", 1.f );
}
...
void CEnemyManager::max_ignore_monster_distance(
...
bool CEnemyManager::change_from_wounded( const CEntityAlive* current,
                                         const CEntityAlive* previous ) const {
    const CAI_Stalker* current_stalker =
        smart_cast< const CAI_Stalker* >( current );
    if ( !current_stalker )
        return ( false );
    ...
    const CAI_Stalker* previous_stalker =
        smart_cast< const CAI_Stalker* >( previous );
    if ( !previous_stalker )
        return ( false );
...
IC bool CEnemyManager::enemy_inertia(
    const CEntityAlive* previous_enemy ) const {
    if ( smart_cast< CActor const* >( m_selected ) )
        return ( Device.dwTimeGlobal <=
                 ( m_last_enemy_change + ENEMY_INERTIA_TIME_TO_ACTOR ) );

    if ( previous_enemy && smart_cast< CActor const* >( previous_enemy ) )
        return ( Device.dwTimeGlobal <=
                 ( m_last_enemy_change + ENEMY_INERTIA_TIME_FROM_ACTOR ) );

    return ( Device.dwTimeGlobal <=
             ( m_last_enemy_change + ENEMY_INERTIA_TIME_TO_SOMEBODY ) );
}
...
void CEnemyManager::on_enemy_change( const CEntityAlive* previous_enemy ) {
    ...
    if ( !m_object->memory().visual().visible_now( previous_enemy ) &&
...
bool CEnemyManager::need_update( const bool& only_wounded ) const {
    ...
    if ( last_hit_time && ( last_hit_time > m_last_enemy_change ) ) {
        ALife::_OBJECT_ID enemy_id =
            m_object->memory().hit().last_hit_object_id();
        VERIFY( enemy_id != ALife::_OBJECT_ID( -1 ) );
        CObject const* enemy = Level().Objects.net_Find( enemy_id );
        VERIFY( enemy );
        CEntityAlive const* alive_enemy =
            smart_cast< CEntityAlive const* >( enemy );
...
void CEnemyManager::try_change_enemy() {
    ...
    if ( selected() != previous_selected ) {
        ...
        if ( ai().script_engine().functor( "_G.CAI_Stalker__OnEnemySelected",
                                           funct ) )
            funct( m_object->lua_game_object(),
                   selected() ? selected()->lua_game_object() : nullptr );
    }
}
