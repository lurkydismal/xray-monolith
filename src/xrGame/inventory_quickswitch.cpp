...
public:
    ...
    void operator()( PIItem const& item ) {
        ...
        if ( ( tmp_cost + ( tmp_ammo_count * ammo_to_cost_map_koef ) ) >
             ( m_best_fit_cost +
               ( m_best_fit_ammo_elapsed * ammo_to_cost_map_koef ) ) ) {
            m_best_fit = item;
            m_best_fit_cost = tmp_weapon->Cost();
            m_best_fit_ammo_elapsed = tmp_weapon->GetAmmoElapsed();
            return;
...
PIItem CInventory::GetNextItemInActiveSlot( u8 const priority_value,
    ...
    priority_group& tmp_prior_group =
        GetPriorityGroup( priority_value, m_iActiveSlot );
    next_weapon_searcher tmp_predicate( tmp_prior_group, best_fit,
                                        m_next_items_exceptions, ignore_ammo );
...
bool CInventory::ActivateNextItemInActiveSlot() {
    ...
    if ( current_item ) {
        ...
        R_ASSERT( res );
        current_item->object().u_EventGen(
            P, GEG_PLAYER_ITEM2RUCK, current_item->object().H_Parent()->ID() );
    ...
    R_ASSERT( res );
    new_item->object().u_EventGen( P, GEG_PLAYER_ITEM2SLOT,
                                   new_item->object().H_Parent()->ID() );
    P.w_u16( new_item->object().ID() );
    P.w_u16( m_iActiveSlot );
    new_item->object().u_EventSend( P );

    // activate
    new_item->object().u_EventGen( P, GEG_PLAYER_ACTIVATE_SLOT,
                                   new_item->object().H_Parent()->ID() );
    P.w_u16( m_iActiveSlot );
    new_item->object().u_EventSend( P );
...
priority_group& CInventory::GetPriorityGroup( u8 const priority_value,
                                              u16 slot ) {
...
void CInventory::InitPriorityGroupsForQSwitch() {
    STATIC_CHECK( epg_groups_count == CInventory::qs_priorities_count,
                  groups_count_problem );
    for ( int i = epg_pistols; i < epg_groups_count; ++i ) {
        m_groups[ i ].init_group( teamdata_section, groups_names[ i ] );
    }

    m_slot2_priorities[ g_slot2_pistol_switch_priority ] =
        &m_groups[ epg_pistols ];
    m_slot2_priorities[ g_slot2_shotgun_switch_priority ] =
        &m_groups[ epg_shotgun ];
    m_slot2_priorities[ g_slot2_assault_switch_priority ] =
        &m_groups[ epg_assault ];
    m_slot2_priorities[ g_slot2_sniper_switch_priority ] =
        &m_groups[ epg_sniper_rifels ];
    m_slot2_priorities[ g_slot2_heavy_switch_priority ] =
        &m_groups[ epg_heavy_weapons ];

    m_slot3_priorities[ g_slot3_pistol_switch_priority ] =
        &m_groups[ epg_assault ]; //&m_groups[epg_pistols];
    m_slot3_priorities[ g_slot3_shotgun_switch_priority ] =
        &m_groups[ epg_shotgun ];
    m_slot3_priorities[ g_slot3_assault_switch_priority ] =
        &m_groups[ epg_assault ];
    m_slot3_priorities[ g_slot3_sniper_switch_priority ] =
        &m_groups[ epg_sniper_rifels ];
    m_slot3_priorities[ g_slot3_heavy_switch_priority ] =
        &m_groups[ epg_heavy_weapons ];
}

priority_group::priority_group() {}

void priority_group::init_group( shared_str const& game_section,
                                 shared_str const& line ) {
...
PIItem CInventory::GetNextActiveGrenade() {
    ...
    if ( count_types > 1 ) {
        ...
        xr_vector< shared_str >::const_iterator E = types_sect_grn.end();
        for ( ; I != E; ++I ) {
            if ( !xr_strcmp( ActiveItem()->cast_game_object()->cNameSect(),
                             *I ) ) // åñëè ñîâïàäàþò
                break;
            curr_num++;
        }
        ...
        shared_str sect_next_grn =
            types_sect_grn[ next_num ]; // ñåêöèÿ ñëåäóùåé ãðàíàòû
        ...
        for ( ; it != it_e; ++it ) {
            CGrenade* pGrenade = smart_cast< CGrenade* >( *it );
            if ( pGrenade &&
                 !xr_strcmp( pGrenade->cNameSect(), sect_next_grn ) )
                return *it;
        }
    }
