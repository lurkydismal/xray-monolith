...
void CDialogHolder::StopMenu( CUIDialogWnd* pDialog ) {
    ...
    if ( TopInputReceiver() == pDialog ) {
        ...
        {
            bool b =
                !!m_input_receivers.back().m_flags.test( recvItem::eCrosshair );
            psHUD_Flags.set( HUD_CROSSHAIR_RT, b );
            b = !!m_input_receivers.back().m_flags.test(
...
void CDialogHolder::AddDialogToRender( CUIWindow* pDialog ) {
    ...
    bool bAdd = ( m_dialogsToRender_new.end() ==
                  std::find( m_dialogsToRender_new.begin(),
                             m_dialogsToRender_new.end(), itm ) );
    if ( !bAdd )
        return;

    bAdd = ( m_dialogsToRender.end() == std::find( m_dialogsToRender.begin(),
...
void CDialogHolder::RemoveDialogToRender( CUIWindow* pDialog ) {
    ...
    auto remove_from_list = [ & ]( xr_vector< dlgItem >& list ) {
        ...
        if ( it != list.end() ) {
            ...
            // Otherwise, we MUST nullify the pointer to prevent dangling
            // access.
            if ( m_b_in_update || m_b_in_render )
                ( *it ).wnd =
                    nullptr; // Crucial: stop OnFrame from touching this memory
            else
                list.erase( it );
...
void CDialogHolder::SetMainInputReceiver( CUIDialogWnd* ir,
                                          bool _find_remove ) {
    if ( !_find_remove && TopInputReceiver() == ir )
        return;
    ...
    if ( !ir || _find_remove ) {
        ...
        if ( !ir )
            ...
            for ( int currentIndex = m_input_receivers.size() - 1;
                ...
                if ( nextIndex < m_input_receivers.size() ) {
                    m_input_receivers[ nextIndex ].m_flags.set(
                        recvItem::eCrosshair,
                        m_input_receivers[ currentIndex ].m_flags.test(
                            recvItem::eCrosshair ) );
                    m_input_receivers[ nextIndex ].m_flags.set(
                        recvItem::eIndicators,
                        m_input_receivers[ currentIndex ].m_flags.test(
                ...
                m_input_receivers.erase( m_input_receivers.begin() +
                                         currentIndex );
                break;
            }
        }
...
void CDialogHolder::StartDialog( CUIDialogWnd* pDialog,
                                 bool bDoHideIndicators ) {
...
void CDialogHolder::OnFrame() {
    ...
    if ( !m_dialogsToRender_new.empty() ) {
        m_dialogsToRender.insert( m_dialogsToRender.end(),
                                  m_dialogsToRender_new.begin(),
                                  m_dialogsToRender_new.end() );
    ...
    m_dialogsToRender.erase(
        std::remove_if( m_dialogsToRender.begin(), m_dialogsToRender.end(),
                        eraseFunc ),
        m_dialogsToRender.end() );
}
...
bool CDialogHolder::IR_UIOnMouseWheel( int direction ) {
    ...
    TIR->OnMouseAction(
        pos.x, pos.y,
        ( direction > 0 ) ? WINDOW_MOUSE_WHEEL_UP : WINDOW_MOUSE_WHEEL_DOWN );
    return true;
}
