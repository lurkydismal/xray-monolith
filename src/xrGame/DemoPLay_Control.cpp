...
demoplay_control::demoplay_control() {
    m_onround_start = fastdelegate::MakeDelegate(
        this, &demoplay_control::on_round_start_impl );
    m_on_kill =
        fastdelegate::MakeDelegate( this, &demoplay_control::on_kill_impl );
    m_on_die =
        fastdelegate::MakeDelegate( this, &demoplay_control::on_die_impl );
    m_on_artefactdelivering = fastdelegate::MakeDelegate(
        this, &demoplay_control::on_artefactdelivering_impl );
    m_on_artefactcapturing = fastdelegate::MakeDelegate(
        this, &demoplay_control::on_artefactcapturing_impl );
    m_on_artefactloosing = fastdelegate::MakeDelegate(
        this, &demoplay_control::on_artefactloosing_impl );
    m_current_mode = not_active;
}

demoplay_control::~demoplay_control() {}

void demoplay_control::pause_on( EAction const action,
...
bool demoplay_control::rewind_until( EAction const action,
                                     shared_str const& param,
                                     user_callback_t ucb ) {
    if ( m_current_mode != not_active ) {
        Msg( "! ERROR: already active." );
...
void demoplay_control::activate_filer( EAction const action,
                                       shared_str const& param ) {
...
void demoplay_control::deactivate_filter() {
    ...
    switch ( m_current_action ) {
        case on_round_start: {
            tmp_msg_filter->remove_filter( M_GAMEMESSAGE,
...
void __stdcall demoplay_control::on_round_start_impl( u32 message,
                                                      u32 subtype,
                                                      NET_Packet& packet ) {
    process_action();
}

void __stdcall demoplay_control::on_kill_impl( u32 message,
                                               u32 subtype,
                                               NET_Packet& packet ) {
    u16 msg_type;
...
void __stdcall demoplay_control::on_die_impl( u32 message,
                                              u32 subtype,
                                              NET_Packet& packet ) {
    u16 msg_type;
    packet.r_begin( msg_type );
...
void __stdcall demoplay_control::on_artefactdelivering_impl(
    u32 message,
    u32 subtype,
    NET_Packet& packet ) {
    u16 msg_type;
...
void __stdcall demoplay_control::on_artefactcapturing_impl(
    u32 message,
    u32 subtype,
    NET_Packet& packet ) {
    u16 msg_type;
    ...
    if ( current_game_type == eGameIDCaptureTheArtefact ) {
        ...
        game_cl_GameState::PLAYERS_MAP_CIT playerIt =
            Game().players.find( clientId );
...
void __stdcall demoplay_control::on_artefactloosing_impl( u32 message,
                                                          u32 subtype,
                                                          NET_Packet& packet ) {
    u16 msg_type;
    packet.r_begin( msg_type );
