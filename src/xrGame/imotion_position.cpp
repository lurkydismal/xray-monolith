...
static void interactive_motion_diag( LPCSTR message,
                                     const CBlend& b,
                                     CPhysicsShell* s,
                                     float time_left ) {
#ifdef DEBUG
    ...
    IKinematicsAnimated* KA =
        smart_cast< IKinematicsAnimated* >( s->PKinematics() );
    VERIFY( KA );
    CPhysicsShellHolder* O = smart_cast< CPhysicsShellHolder* >(
        s->get_ElementByStoreOrder( 0 )->PhysicsRefObject() );
    VERIFY( O );
    LPCSTR motion_name = KA->LL_MotionDefName_dbg( m ).first;
    Msg( "death anims - interactive_motion:- %s, motion: %s, blend time %f , "
         "total blend time %f , time left: %f , obj: %s, model:  %s ",
         message, motion_name, b.timeCurrent, b.timeTotal, time_left,
...
static void get_depth( bool& do_colide,
                       bool bo1,
                       dContact& c,
                       SGameMtl* /*material_1*/,
                       SGameMtl* /*material_2*/ ) {
...
void imotion_position::state_start() {
    ...
    IKinematicsAnimated* KA =
        smart_cast< IKinematicsAnimated* >( shell->PKinematics() );
    VERIFY( KA );
...
*/
    ...
    if ( !is_enabled() )
        return;
    CPhysicsShellHolder* obj = static_cast< CPhysicsShellHolder* >(
        shell->get_ElementByStoreOrder( 0 )->PhysicsRefObject() );
    VERIFY( obj );
...
static void dbg_draw_state_end( CPhysicsShell* shell ) {
    ...
    if ( dbg_imotion_draw_velocity ) {
        DBG_OpenCashedDraw();
        shell->dbg_draw_velocity( dbg_imotion_draw_velocity_scale,
...
void imotion_position::state_end() {
    ...
    shell->AnimToVelocityState( end_delta, default_l_limit * 10,
                                default_w_limit * 10 );
#ifdef DEBUG
...
void collide_anim_dbg_draw( CPhysicsShell* shell, float dt ) {
    VERIFY( shell );
    if ( dbg_imotion_draw_velocity ) {
        shell->AnimToVelocityState( dt, default_l_limit * 10,
                                    default_w_limit * 10 );
        DBG_OpenCashedDraw();
        shell->dbg_draw_velocity( dbg_imotion_draw_velocity_scale,
    ...
    if ( dbg_imotion_draw_skeleton ) {
        DBG_OpenCashedDraw();
        CPhysicsShellHolder* sh = static_cast< CPhysicsShellHolder* >(
            shell->get_ElementByStoreOrder( 0 )->PhysicsRefObject() );
        DBG_PhysBones( *sh );
        DBG_ClosedCashedDraw( 50000 );
    }
...
float imotion_position::collide_animation( float dt, IKinematicsAnimated& k ) {
...
#endif
    shell->ToAnimBonesPositions( shell_motion_has_history ? mh_not_clear
                                                          : mh_unspecified );
    depth = 0;
#ifdef DEBUG
    if ( dbg_imotion_collide_debug )
    ...
    if ( dbg_imotion_collide_debug )
        interactive_motion_diagnostic(
            make_string( " collide_animation: deppth= %f", depth ).c_str() );
#endif
    return dt;
}
...
void imotion_position::collide_not_move( IKinematicsAnimated& KA ) {
    ...
    buffer_vector< sblend_save > saved_blends(
        _alloca( sv_blends_num * sizeof( sblend_save ) ), sv_blends_num );
    ...
    for ( u32 i = 0; i < iterations; ++i ) {
        ...
        if ( !!flags.test( fl_switch_dm_toragdoll ) ) {
            ...
            shell->ToAnimBonesPositions(
                shell_motion_has_history ? mh_not_clear : mh_unspecified );
            ...
            if ( dbg_imotion_collide_debug ) {
                ...
                shell->CollideAll();
                interactive_motion_diagnostic(
                    make_string( " move (to ragdoll): deppth= %f", depth )
                        .c_str() );
                DBG_OpenCashedDraw();
                shell->dbg_draw_geometry( 0.02,
...
float imotion_position::motion_collide( float dt, IKinematicsAnimated& KA ) {
    ...
    if ( time_to_end < ( max_collide_timedelta + end_delta ) ) {
        interactive_motion_diagnostic(
            make_string( "motion_collide 0: stoped: time out, time delta %f",
                         dt )
                .c_str() );
    ...
    if ( depth > depth_resolve ) {
        ...
        if ( dbg_imotion_collide_debug ) {
            ...
            // collided0: deppth= %f", depth ).c_str() );
            interactive_motion_diagnostic(
                make_string( "motion_collide 1: stoped: colide: %s, depth %f",
                             collide_diag().c_str(), depth )
                    .c_str() );
            DBG_OpenCashedDraw();
            shell->dbg_draw_geometry( 0.02, D3DCOLOR_ARGB( 255, 0, 255, 0 ) );
            DBG_ClosedCashedDraw( 50000 );
        ...
        if ( dbg_imotion_collide_debug ) {
            interactive_motion_diagnostic(
                make_string( " motion_collide collided1: deppth= %f", depth )
                    .c_str() );
            DBG_OpenCashedDraw();
        ...
        if ( depth > depth0 ) {
            ...
            if ( dbg_imotion_collide_debug ) {
                interactive_motion_diagnostic(
                    make_string( " motion_collide collided2: deppth= %f",
                                 depth )
                        .c_str() );
                DBG_OpenCashedDraw();
                shell->dbg_draw_geometry( 0.02,
            ...
            if ( depth > depth_resolve ) {
                interactive_motion_diagnostic(
                    make_string(
                        "motion_collide 2: stoped: colide: %s, depth %f",
                        collide_diag().c_str(), depth )
                        .c_str() );
                flags.set( fl_switch_dm_toragdoll, TRUE );
            }
        ...
        force_calculate_bones( KA );
        shell->ToAnimBonesPositions(
            shell_motion_has_history ? mh_clear : mh_unspecified );
        ...
        if ( dbg_imotion_collide_debug ) {
            ...
            shell->CollideAll();
            interactive_motion_diagnostic(
                make_string( " motion_collide restore: %f ", depth ).c_str() );
            DBG_OpenCashedDraw();
...
////////////////////////////////////////////////////////////////////////////////////
bool imotion_position::tracks_update::operator()( float dt,
...
void imotion_position::set_root_callback() {
    ...
    VERIFY( !bi.callback() );
    bi.set_callback( bctCustom, rootbone_callback, this,
                     true ); // root may be not "0" !
}
...
void imotion_position::rootbone_callback( CBoneInstance* BI ) {
    ...
    R_ASSERT2( _valid( BI->mTransform ),
               "imotion_position::rootbone_callback" );
}
