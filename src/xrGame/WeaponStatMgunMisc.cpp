...
void CWeaponStatMgun::OverrideRangeFOV( const CGameObject* npc, float& range ) {
    /*
        ...
        Max max_view_distance of danger mental state is 114m after all
       calculation in object_visible_distance() - update_range_fov(). It also
       affects other place that use update_range_fov(). It makes
       gunner:see(target) return false in LUA. This overrider allows changing
       start_range in update_range_fov() effectively changing visibility range.
        For now it's used for machine gunners only. It allow gunners to engage
       actor from further away.
    */
    if ( m_on_range_fov_callback.size() == 0 )
        return;
    ::luabind::functor< ::luabind::object > lua_function;
    if ( ai().script_engine().functor( m_on_range_fov_callback.c_str(),
                                       lua_function ) == false )
        return;
    ::luabind::object lua_var =
        ::luabind::newtable( ai().script_engine().lua() );
    lua_var[ "range" ] = range;
    ::luabind::object lua_res =
        lua_function( lua_game_object(), npc->lua_game_object(), lua_var );
...
BOOL CWeaponStatMgun::SStmAnimWeapon::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( A ) {
        ...
        if ( ini->section_exist( anim_weapon ) ) {
            ...
            for ( int i = 0; i < eStmAnimWeapon_size; i++ ) {
                LPCSTR str = READ_IF_EXISTS( ini, r_string, anim_weapon,
                                             anim_name[ i ], nullptr );
                string128 tmp;
                if ( str && strlen( str ) ) {
                    for ( int k = 0, n = _GetItemCount( str ); k < n; ++k ) {
                        ...
                        m_anims[ i ].push_back(
                            A->ID_Cycle( _GetItem( str, k, tmp ) ) );
                    }
                }
            }
            {
                m_magazine_hide_bid =
                    ini->line_exist( anim_weapon, "magazine_hide_bid" )
                        ? K->LL_BoneID( ini->r_string( anim_weapon,
                                                       "magazine_hide_bid" ) )
                        : BI_NONE;
                m_magazine_hide_anm.clear();
                LPCSTR str = READ_IF_EXISTS( ini, r_string, anim_weapon,
                                             "magazine_hide_anm", nullptr );
                string128 tmp;
                if ( str && strlen( str ) ) {
                    for ( int k = 0, n = _GetItemCount( str ); k < n; ++k ) {
                        memset( tmp, 0, sizeof( tmp ) );
                        m_magazine_hide_anm.push_back(
                            A->ID_Cycle( _GetItem( str, k, tmp ) ) );
                    }
                }
            }
    ...
    if ( ini->section_exist( "animation_hand" ) ) {
        m_hand_bid =
            ini->line_exist( "animation_hand", "hand_bid" )
                ? K->LL_BoneID( ini->r_string( "animation_hand", "hand_bid" ) )
                : BI_NONE;
        m_hand_pos = READ_IF_EXISTS( ini, r_fvector3, "animation_hand",
                                     "hand_pos", Fvector().set( 0, 0, 0 ) );
        m_hand_anims[ eStmAnimWeapon_idle ]._set( READ_IF_EXISTS(
            ini, r_string, "animation_hand", "hand_idle", "" ) );
    }
    return TRUE;
}
...
void CWeaponStatMgun::SStmAnimWeapon::Play( u8 anim ) {
    ...
    m_stm->Visual()->dcast_PKinematicsAnimated()->PlayCycle(
        m_current_mid, TRUE, AnimationCallback, m_stm );
...
bool CWeaponStatMgun::SStmAnimWeapon::HandGetVisualName() {
    ...
    if ( g_player_hud == nullptr || g_player_hud->section_name().size() == 0 )
        return false;
    LPCSTR str = READ_IF_EXISTS( pSettings, r_string,
                                 g_player_hud->section_name().c_str(), "visual",
                                 nullptr );
...
void CWeaponStatMgun::SStmAnimWeapon::UpdateMagazineVisibility() {
    ...
    R_ASSERT( K );
    bool visibility =
        std::find( m_magazine_hide_anm.begin(), m_magazine_hide_anm.end(),
                   m_current_mid ) == m_magazine_hide_anm.end();
