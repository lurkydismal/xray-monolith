...
void CTorch::Load( LPCSTR section ) {
    ...
    m_light_section = READ_IF_EXISTS( pSettings, r_string, section,
                                      "light_section", "torch_definition" );
    if ( pSettings->line_exist( section, "snd_turn_on" ) )
        m_sounds.LoadSound( section, "snd_turn_on", "sndTurnOn", false,
                            SOUND_TYPE_ITEM_USING );
    if ( pSettings->line_exist( section, "snd_turn_off" ) )
        m_sounds.LoadSound( section, "snd_turn_off", "sndTurnOff", false,
                            SOUND_TYPE_ITEM_USING );

    m_torch_offset = READ_IF_EXISTS( pSettings, r_fvector3, section,
                                     "torch_offset", TORCH_OFFSET );
    m_omni_offset = READ_IF_EXISTS( pSettings, r_fvector3, section,
                                    "omni_offset", OMNI_OFFSET );
    m_torch_inertion_speed_max =
        READ_IF_EXISTS( pSettings, r_float, section, "torch_inertion_speed_max",
                        TORCH_INERTION_SPEED_MAX );
    m_torch_inertion_speed_min =
        READ_IF_EXISTS( pSettings, r_float, section, "torch_inertion_speed_min",
                        TORCH_INERTION_SPEED_MIN );
    m_torch_inertion_clamp =
        READ_IF_EXISTS( pSettings, r_float, section, "torch_inertion_clamp",
                        TORCH_INERTION_CLAMP );

    m_bUseInertion =
        READ_IF_EXISTS( pSettings, r_bool, section, "torch_inertion", true );
}
    ...
    if ( pActor ) {
        if ( light_on && !m_switched_on ) {
            if ( m_sounds.FindSoundItem( "SndTurnOn", false ) )
                m_sounds.PlaySound( "SndTurnOn", pActor->Position(), NULL,
                                    !!pActor->HUDview() );
        } else if ( !light_on && m_switched_on ) {
            if ( m_sounds.FindSoundItem( "SndTurnOff", false ) )
                m_sounds.PlaySound( "SndTurnOff", pActor->Position(), NULL,
                                    !!pActor->HUDview() );
        }
    }
...
void CTorch::LoadLightParams() {
    ...
    "Section not found in torch user data! Check 'light_section' field in
    config");*/
    def_lanim =
        pSettings->r_string( light_definition.c_str(), "color_animator" );
    lanim = LALib.FindItem( def_lanim );
    guid_bone = K->LL_BoneID(
        pSettings->r_string( light_definition.c_str(), "guide_bone" ) );
    VERIFY( guid_bone != BI_NONE );

    Fcolor clr = pSettings->r_fcolor( light_definition.c_str(),
                                      ( b_r2 ) ? "color_r2" : "color" );
    def_clr = clr;
    fBrightness = clr.intensity();
    float range = pSettings->r_float( light_definition.c_str(),
                                      ( b_r2 ) ? "range_r2" : "range" );
    light_render->set_color( clr );
    light_render->set_range( range );

    Fcolor clr_o = pSettings->r_fcolor(
        light_definition.c_str(), ( b_r2 ) ? "omni_color_r2" : "omni_color" );
    float range_o = pSettings->r_float(
        light_definition.c_str(), ( b_r2 ) ? "omni_range_r2" : "omni_range" );
    light_omni->set_color( clr_o );
    light_omni->set_range( range_o );

    light_render->set_cone( deg2rad(
        pSettings->r_float( light_definition.c_str(), "spot_angle" ) ) );
    light_render->set_texture( READ_IF_EXISTS( pSettings, r_string,
                                               light_definition.c_str(),
                                               "spot_texture", ( 0 ) ) );

    glow_render->set_texture(
        pSettings->r_string( light_definition.c_str(), "glow_texture" ) );
    glow_render->set_color( clr );
    glow_render->set_radius(
        pSettings->r_float( light_definition.c_str(), "glow_radius" ) );

    light_render->set_volumetric( !!READ_IF_EXISTS(
        pSettings, r_bool, light_definition.c_str(), "volumetric", 0 ) );
    light_render->set_volumetric_quality(
        READ_IF_EXISTS( pSettings, r_float, light_definition.c_str(),
                        "volumetric_quality", 1.f ) );
    light_render->set_volumetric_intensity(
        READ_IF_EXISTS( pSettings, r_float, light_definition.c_str(),
                        "volumetric_intensity", 1.f ) );
    light_render->set_volumetric_distance(
        READ_IF_EXISTS( pSettings, r_float, light_definition.c_str(),
                        "volumetric_distance", 1.f ) );

    light_render->set_type( ( IRender_Light::LT )( READ_IF_EXISTS(
        pSettings, r_u8, light_definition.c_str(), "type", 2 ) ) );
    light_omni->set_type( ( IRender_Light::LT )( READ_IF_EXISTS(
        pSettings, r_u8, light_definition.c_str(), "omni_type", 1 ) ) );
...
void CTorch::Update() {
        ...
        if ( actor ) {
            ...
            if ( actor->active_cam() == eacLookAt ) {
                m_prev_hp.x = angle_inertion_var(
                    m_prev_hp.x, -actor->cam_Active()->yaw,
                    m_torch_inertion_speed_min, m_torch_inertion_speed_max,
                    m_torch_inertion_clamp, Device.fTimeDelta );
                m_prev_hp.y = angle_inertion_var(
                    m_prev_hp.y, -actor->cam_Active()->pitch,
                    m_torch_inertion_speed_min, m_torch_inertion_speed_max,
                    m_torch_inertion_clamp, Device.fTimeDelta );
            } else {
                m_prev_hp.x = angle_inertion_var(
                    m_prev_hp.x, -actor->cam_FirstEye()->yaw,
                    m_torch_inertion_speed_min, m_torch_inertion_speed_max,
                    m_torch_inertion_clamp, Device.fTimeDelta );
                m_prev_hp.y = angle_inertion_var(
                    m_prev_hp.y, -actor->cam_FirstEye()->pitch,
                    m_torch_inertion_speed_min, m_torch_inertion_speed_max,
                    m_torch_inertion_clamp, Device.fTimeDelta );
            }
            ...
            if ( !m_bUseInertion && actor->HUDview() ) {
                ...
                light_render->set_rotation( actorcam->Direction(),
                                            actorcam->Right(), eps );
                light_omni->set_rotation( actorcam->Direction(),
                                          actorcam->Right(), eps );
            ...
            float eps = r_optimize_torch
                            ? clampr< float >( dist * 0.002f, 0.01f, 0.12f )
                            : 0.001f;
    ...
    Fcolor fclr;
    fclr.set( ( float )color_get_B( clr ), ( float )color_get_G( clr ),
              ( float )color_get_R( clr ), 1.f );
...
void CTorch::SetLanim( LPCSTR name,
                       bool bFlicker,
                       int flickerChance,
                       float flickerDelay,
                       float framerate ) {
