...
public:
    ...
    virtual bool init( CTelekinesis* tele,
...
private:
    ...
    float m_fKeepRadius;
    float m_fThrowPower;
    boolean m_bHeightFixed; // whether object will be hard-fixed at tele_heigh
                            // or can go above (true - default)
...
public:
    ...
    virtual CTelekineticObject* activate( CPhysicsShellHolder* obj,
                                          float strength,
                                          float height,
                                          u32 max_time_keep,
                                          bool rot = true );
    virtual CTelekineticObject* alloc_tele_object() {
        if ( m_iTelekinesisType == 1 ) {
            return static_cast< CTelekineticObject* >(
                xr_new< CTeleTrampolinObject >() );
        } else {
            return static_cast< CTelekineticObject* >(
                xr_new< CTeleWhirlwindObject >() );
        }
    }
