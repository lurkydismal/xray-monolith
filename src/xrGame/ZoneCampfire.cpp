...
CZoneCampfire::CZoneCampfire()
    : m_pDisabledParticles( NULL ),
      m_pEnablingParticles( NULL ),
      m_turned_on( true ),
      m_turn_time( 0 ) {
    //.	g_zone = this;
}
...
void CZoneCampfire::PlayIdleParticles( bool bIdleLight ) {
    if ( m_turn_time == 0 ||
         m_turn_time - Device.dwTimeGlobal < ( OVL_TIME - 2000 ) ) {
...
void CZoneCampfire::StopIdleParticles( bool bIdleLight ) {
    if ( m_turn_time == 0 ||
         m_turn_time - Device.dwTimeGlobal < ( OVL_TIME - 500 ) )
...
void CZoneCampfire::UpdateWorkload( u32 dt ) {
    ...
    if ( m_turn_time > Device.dwTimeGlobal ) {
        float k =
            float( m_turn_time - Device.dwTimeGlobal ) / float( OVL_TIME );
        ...
        if ( m_pIdleLight && m_pIdleLight->get_active() ) {
            ...
            float range =
                m_fIdleLightRange + 0.25f * ::Random.randF( -1.f, 1.f );
            range *= k;
