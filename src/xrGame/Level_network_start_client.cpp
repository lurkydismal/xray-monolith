...
bool CLevel::net_start_client1() {
    ...
    //	xr_strcpy
    //(name_of_server,*m_caClientOptions);
...
bool CLevel::net_start_client3() {
    ...
    if ( connected_to_server ) {
        ...
        if ( psNET_direct_connect ) // single
        {
            ...
            level_name = name().c_str(); // Server->level_name
                                         // (server_options).c_str();
        ...
        } else // multiplayer
        {
            ...
            rescan_mp_archives(); // because if we are using
                                  // psNET_direct_connect, we not download
                                  // map...
        }
        ...
        if ( level_id == -1 ) {
            ...
            Msg( "! Level (name:%s), (version:%s), not found, try to download "
                 "from:%s",
...
#ifdef DEBUG
        Msg( "--- net_start_client3: level_id [%d], level_name[%s], "
             "level_version[%s]",
             level_id, level_name, level_ver );
#endif // #ifdef DEBUG
        map_data.m_name = level_name;
        map_data.m_map_version = level_ver;
        map_data.m_map_download_url = download_url;
...
bool CLevel::net_start_client4() {
    ...
    if ( connected_to_server ) {
        ...
        create_physics_world( !!psDeviceFlags.test( mtPhysics ), &ObjectSpace,
                              &Objects, &Device );

        R_ASSERT( physics_world() );

        m_ph_commander_physics_worldstep = xr_new< CPHCommander >();
        physics_world()->set_update_callback(
            m_ph_commander_physics_worldstep );

        physics_world()->set_default_contact_shotmark( ContactShotMark );
        physics_world()->set_default_character_contact_shotmark(
            CharacterContactShotMark );
