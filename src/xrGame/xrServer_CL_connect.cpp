...
void xrServer::Prepare_connect_spawn(
    CSE_Abstract* E,
    ...
    if ( pSettings->section_exist( E->s_name.c_str() ) &&
         pSettings->line_exist( E->s_name.c_str(), "visual" ) )
        spawn.ltx_visual = pSettings->r_string( E->s_name.c_str(), "visual" );
    prepared.push_back( std::move( spawn ) );
}

void xrServer::Perform_connect_spawn( const PreparedClientSpawn& prepared,
                                      xrClientData* CL,
                                      NET_Packet& P ) {
...
void xrServer::SendConnectionData( IClient* _CL ) {
    ...
    const bool prepare_local_resources =
        !g_dedicated_server && CL == GetServerClient();
#if 0
	try
	{
#endif
    if ( prepare_local_resources )
        for ( PreparedClientSpawn& spawn : prepared ) {
            spawn.resource_batch =
                executor.BeginBatch( executor.CurrentGeneration() );
            auto prepare_resources = [ &spawn, level_path ]() {
                if ( spawn.actual_visual.size() )
                    ::Render->model_CollectTextures(
                        spawn.actual_visual.c_str(), level_path.c_str(),
                        spawn.textures );
                if ( spawn.ltx_visual.size() &&
                     spawn.ltx_visual != spawn.actual_visual )
                    ::Render->model_CollectTextures( spawn.ltx_visual.c_str(),
                                                     level_path.c_str(),
                                                     spawn.textures );
                std::sort( spawn.textures.begin(), spawn.textures.end() );
                spawn.textures.erase(
                    std::unique( spawn.textures.begin(), spawn.textures.end() ),
                    spawn.textures.end() );
                for ( const xr_string& texture : spawn.textures )
                    Device.m_pRender->ResourcesPrefetchCreateTexture(
                        texture.c_str(), level_path.c_str() );
            };
            if ( spawn.resource_batch.Valid() )
                executor.Submit( spawn.resource_batch,
                                 NativeLoadPriority::Spawn,
                                 std::move( prepare_resources ) );
            else
                prepare_resources();
        }
    ...
    for ( const PreparedClientSpawn& spawn : prepared ) {
        ...
        if ( prepare_local_resources )
            Level().RegisterPreparedClientSpawnResource(
                spawn.id, spawn.parent_id, spawn.section, spawn.actual_visual,
                spawn.ltx_visual, level_path.c_str() );
#endif
        model_count +=
            spawn.actual_visual.size() || spawn.ltx_visual.size() ? 1u : 0u;
        texture_count += static_cast< u32 >( spawn.textures.size() );
        order_hash = crc32( &spawn.id, sizeof( spawn.id ), order_hash );
        order_hash =
            crc32( &spawn.parent_id, sizeof( spawn.parent_id ), order_hash );
        order_hash = crc32( spawn.section.c_str(),
                            xr_strlen( spawn.section.c_str() ), order_hash );
        Perform_connect_spawn( spawn, CL, P );
    }
    Msg( "* [client-spawn] prepared=%u models=%u textures=%u order_hash=%08x",
         static_cast< u32 >( prepared.size() ), model_count, texture_count,
         order_hash );
#if 0
	}
	catch (...)
...
void xrServer::OnCL_Connected( IClient* _CL ) {
    ...
    // csPlayers.Enter					();	//sychronized by
    // a parent call
    Export_game_type( CL );
    Perform_game_export();
...
void xrServer::SendConnectResult( IClient* CL,
                                  u8 res,
                                  u8 res1,
...
// this method response for client validation on connect state
// (CLevel::net_start_client2) the first validation is CDKEY, then gamedata
// checksum (NeedToCheckClient_BuildVersion), then banned or not... WARNING ! if
// you will change this method see M_AUTH_CHALLENGE event handler
...
void xrServer::OnBuildVersionRespond( IClient* CL, NET_Packet& P ) {
    ...
    if ( _our != _him ) {
        ...
        if ( CL->flags.bLocal || bAccessUser ) {
            ...
            SendConnectResult( CL, 0, ecr_password_verification_failed,
                               res_check );
        }
    }
};
