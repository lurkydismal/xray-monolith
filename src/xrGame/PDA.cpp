...
void CPda::Load( LPCSTR section ) {
    ...
    m_functor_str =
        READ_IF_EXISTS( pSettings, r_string, section, "play_function", "" );
    m_fDisplayBrightnessPowerSaving = READ_IF_EXISTS(
        pSettings, r_float, section, "power_saving_brightness", .6f );
    m_fPowerSavingCharge = READ_IF_EXISTS( pSettings, r_float, section,
                                           "power_saving_charge", .15f );
    m_joystick_bone = READ_IF_EXISTS( pSettings, r_string, section,
                                      "joystick_bone", nullptr );
    ...
    m_screen_on_delay =
        READ_IF_EXISTS( pSettings, r_float, section, "screen_on_delay", 0.f );
    m_screen_off_delay =
        READ_IF_EXISTS( pSettings, r_float, section, "screen_off_delay", 0.f );
    m_thumb_rot[ 0 ] =
        READ_IF_EXISTS( pSettings, r_float, section, "thumb_rot_x", 0.f );
    m_thumb_rot[ 1 ] =
        READ_IF_EXISTS( pSettings, r_float, section, "thumb_rot_y", 0.f );
    m_nearwall_zoomed_range = READ_IF_EXISTS( pSettings, r_float, section,
                                              "nearwall_zoomed_range", .1f );
}
...
void CPda::OnStateSwitch( u32 S, u32 oldState ) {
    ...
    switch ( S ) {
        case eShowing: {
            ...
            m_sounds.PlaySound(
                hasEnoughBatteryPower() ? "sndShow" : "sndShowEmpty",
                Position(), H_Root(), !!GetHUDmode(), false );
            PlayHUDMotion(
                !m_bNoticedEmptyBattery ? "anm_show" : "anm_show_empty", FALSE,
                this, GetState(), 1.f, 0.f, false );
        ...
        case eHiding: {
            if ( oldState != eHiding ) {
                m_sounds.PlaySound(
                    hasEnoughBatteryPower() ? "sndHide" : "sndHideEmpty",
                    Position(), H_Root(), !!GetHUDmode(), false );
                PlayHUDMotion(
                    !m_bNoticedEmptyBattery ? "anm_hide" : "anm_hide_empty",
                    TRUE, this, GetState() );
                SetPending( TRUE );
                m_bZoomed = false;
                ...
                if ( joystick != BI_NONE )
                    HudItemData()
                        ->m_model->LL_GetBoneInstance( joystick )
                        .reset_callback();
        ...
        case eIdle: {
            ...
            if ( joystick != BI_NONE ) {
                CBoneInstance* bi =
                    &HudItemData()->m_model->LL_GetBoneInstance( joystick );
                if ( bi )
        ...
        case eEmptyBattery: {
            SetPending( TRUE );
            m_sounds.PlaySound( "sndEmptyBattery", Position(), H_Root(),
                                !!GetHUDmode(), false );
...
void CPda::JoystickCallback( CBoneInstance* B ) {
    ...
    if ( !fsimilar( target_press, press, .0001f ) ) {
        ...
        if ( prev_press == 0.f && press < 0.f )
            Pda->m_sounds.PlaySound( "sndButtonPress", B->mTransform.c,
                                     Pda->H_Root(), !!Pda->GetHUDmode() );
        else if ( prev_press < -.001f && press >= -.001f )
            Pda->m_sounds.PlaySound( "sndButtonRelease", B->mTransform.c,
                                     Pda->H_Root(), !!Pda->GetHUDmode() );
    } else
        press = target_press;
...
void CPda::UpdateCL() {
    ...
    if ( pda->IsShown() ) {
        ...
        if ( !enoughBatteryPower || state == eHidden ) {
            ...
            // Force update PDA UI if it's disabled (no input) and check for
            // deferred enable or zoom in.
            if ( !pda->IsEnabled() ) {
                pda->Update();
                if ( m_eDeferredEnable == eEnable ||
            ...
            if ( IsUsingCondition() && condition < m_fPowerSavingCharge ) {
                if ( !m_bPowerSaving ) {
                    ...
                    if ( ai().script_engine().functor( "pda.on_low_battery",
                                                       funct ) )
                        funct();
                    m_bPowerSaving = true;
                }
        ...
        if ( !IsMainMenuActive() && state != eHiding && state != eHidden &&
            ...
            m_bNoticedEmptyBattery = false;
            if ( m_eDeferredEnable ==
                 eEnable ) // Don't disable input if it was enabled before
...
void CPda::shedule_Update( u32 dt ) {
    ...
    if ( IsOn() && Level().CurrentEntity() &&
         Level().CurrentEntity()->ID() == H_Parent()->ID() ) {
...
bool CPda::Action( u16 cmd, u32 flags ) {
    ...
    switch ( cmd ) {
        ...
        case kWPN_FUNC: {
            if ( flags & CMD_START && !IsPending() &&
                 !psActorFlags.test( AF_SIMPLE_PDA ) ) {
                pda->Enable( !pda->IsEnabled() );
                if ( m_bZoomed && !pda->IsEnabled() )
                    m_bZoomed = false;
                if ( pda->IsEnabled() )
                    Actor()->StopSprint();
                return true;
...
void CPda::UpdateHudAdditional( Fmatrix& trans ) {
    ...
    if ( pActor->cam_freelook == eflEnabled ||
         pActor->cam_freelook == eflEnabling ||
         g_player_hud->script_anim_part != u8( -1 ) ) {
        CUIPdaWnd* pda = &CurrentGameUI()->GetPdaMenu();
    ...
    // Remove pending state before weapon has fully moved to the new position to
    // remove some delay
    if ( curr_offs.similar( m_hud_offset[ 0 ], .02f ) &&
         curr_rot.similar( m_hud_offset[ 1 ], .02f ) ) {
        if ( ( m_bZoomed == true || m_LastMBZoom == true ) && IsPending() )
            SetPending( FALSE );
    ...
    float fStrafeMinAngle =
        _lerp( hi->m_measures.m_strafe_offset[ 3 ][ 0 ].y,
               hi->m_measures.m_strafe_offset[ 3 ][ 1 ].y, m_fZoomfactor );

    // Высчитываем мксимальный наклон от поворота камеры
    float fCamLimitBlend =
        _lerp( hi->m_measures.m_strafe_offset[ 3 ][ 0 ].x,
               hi->m_measures.m_strafe_offset[ 3 ][ 1 ].x, m_fZoomfactor );
    ...
    if ( abs( fYMag ) >
        ...
        if ( m_fLR_CameraFactor < 0.0f ) {
            m_fLR_CameraFactor +=
                fStepPerUpd * ( m_bZoomed ? 1.0f : fCamReturnSpeedMod );
    ...
    float fInertiaSpeedMod = _lerp(
        ...
        hi->m_measures.m_inertion_params.m_tendto_speed_aim, m_fZoomfactor );
    ...
    float fInertiaMinAngle = _lerp(
        ...
        hi->m_measures.m_inertion_params.m_min_angle_aim, m_fZoomfactor );
...
void CPda::feel_touch_new( CObject* O ) {
    if ( CInventoryOwner* pNewContactInvOwner =
             smart_cast< CInventoryOwner* >( O ) ) {
...
void CPda::feel_touch_delete( CObject* O ) {
    if ( !H_Parent() )
        return;
    if ( CInventoryOwner* pLostContactInvOwner =
             smart_cast< CInventoryOwner* >( O ) ) {
        CInventoryOwner* pOwner = smart_cast< CInventoryOwner* >( H_Parent() );
        VERIFY( pOwner );
...
bool CPda::feel_touch_contact( CObject* O ) {
    ...
    if ( entity_alive && entity_alive->cast_base_monster() ) {
        return true;
    } else if ( CInventoryOwner* pInvOwner =
                    smart_cast< CInventoryOwner* >( O ) ) {
...
void CPda::OnH_A_Chield() {
    ...
    if ( H_Parent()->ID() == m_idOriginalOwner ) {
        ...
        if ( m_sFullName.empty() ) {
            ...
            m_sFullName += " ";
            m_sFullName +=
                ( smart_cast< CInventoryOwner* >( H_Parent() ) )->Name();
        }
    };
...
void CPda::OnH_B_Independent( bool just_before_destroy ) {
    ...
    m_sounds.PlaySound( hasEnoughBatteryPower() ? "sndHide" : "sndHideEmpty",
                        Position(), H_Root(), !!GetHUDmode(), false );
...
void CPda::PlayScriptFunction() {
    if ( xr_strcmp( m_functor_str, "" ) ) {
        ...
        R_ASSERT(
            ai().script_engine().functor( m_functor_str.c_str(), m_functor ) );
        m_functor();
    }
}
