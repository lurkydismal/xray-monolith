...
IC CSpaceRestrictionBridge::CSpaceRestrictionBridge(
    CSpaceRestrictionBase* object ) {
    VERIFY( object );
    m_object = object;
}
...
template < typename T >
IC u32 CSpaceRestrictionBridge::accessible_nearest( T restriction,
                                                    const Fvector& position,
                                                    Fvector& result,
                                                    bool out_restriction ) {
#pragma todo( \
    "Dima to Dima : _Warning : this place can be optimized in case of a slowdown" )
    VERIFY( initialized() );
    VERIFY( !restriction->border().empty() );
    VERIFY( !restriction
                 ->accessible_neighbour_border( restriction, out_restriction )
                 .empty() );

    float min_dist_sqr = flt_max;
    u32 selected = u32( -1 );
    typename xr_vector< u32 >::const_iterator I =
        restriction->accessible_neighbour_border( restriction, out_restriction )
            .begin();
    typename xr_vector< u32 >::const_iterator E =
        restriction->accessible_neighbour_border( restriction, out_restriction )
            .end();
    for ( ; I != E; ++I ) {
        VERIFY2( ai().level_graph().valid_vertex_id( *I ),
                 make_string( "%d", *I ) );
        float distance_sqr =
            ai().level_graph().vertex_position( *I ).distance_to_sqr(
                position );
    ...
    VERIFY2( ai().level_graph().valid_vertex_id( selected ),
             make_string( "vertex_id[%d], object[%s], position[%f][%f][%f]",
                          selected, *name(), VPUSH( position ) ) );
...

    {
        ...
        for ( ; I != E; ++I ) {
            ...
            if ( restriction->inside( current, !out_restriction ) !=
                 out_restriction )
                continue;

            float distance_sqr =
                ai().level_graph().vertex_position( current ).distance_to_sqr(
                    position );
            if ( distance_sqr < min_dist_sqr ) {
                min_dist_sqr = distance_sqr;
...

    {
        ...
        for ( u32 i = 0; i < 5; ++i ) {
            ...
            switch ( i ) {
                case 0:
                    current.P.set( center.x + offset, center.y,
                                   center.z + offset );
                    break;
                case 1:
                    current.P.set( center.x + offset, center.y,
            ...
            if ( i < 4 )
                current.P.y = ai().level_graph().vertex_plane_y(
                    selected, current.P.x, current.P.z );

            VERIFY( ai().level_graph().inside( selected, current.P ) );
            VERIFY( restriction->inside( selected, !out_restriction ) ==
                    out_restriction );
...
template < typename T >
IC const xr_vector< u32 >& CSpaceRestrictionBridge::accessible_neighbour_border(
    T& restriction,
    bool out_restriction ) {
    return (
        object().accessible_neighbour_border( restriction, out_restriction ) );
}
