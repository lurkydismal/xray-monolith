...
void CWeaponStatMgun::UpdateFire() {
    ...
    if ( m_overheat_enabled ) {
        ...
        if ( m_overheat_value >= 100.f ) {
            if ( !p_overheat ) {
                p_overheat = Particles::Details::Create(
                    m_overheat_particles.c_str(), FALSE );
                Fmatrix pos;
                pos.set( get_ParticlesXFORM() );
                pos.c.set( get_CurrentFirePoint() );
                p_overheat->SetXFORM( pos );
...
void CWeaponStatMgun::OnShot() {
    ...
    CCartridge& l_cartridge =
        ( m_magazine.size() > 0 ) ? m_magazine.back() : m_DefaultCartridge;
    FireBullet( m_fire_pos, m_fire_dir, GetFireDispersion( true ), l_cartridge,
                owner->ID(), ID(), true, iMagazineSize - iAmmoElapsed );
    ++m_iShotNum;
...
#else
    bool b_hud_mode =
        ( Level().CurrentEntity() == smart_cast< CObject* >( Owner() ) );
...
void CWeaponStatMgun::OnShot( SStmBarrel& B ) {
    ...
    CCartridge& l_cartridge =
        ( m_magazine.size() > 0 ) ? m_magazine.back() : m_DefaultCartridge;
    FireBullet( B.m_fire_pos, B.m_fire_dir, GetFireDispersion( true ),
                l_cartridge, owner->ID(), ID(), true,
                iMagazineSize - B.iAmmoElapsed );
...
bool CWeaponStatMgun::BarrelAllowFire( SStmBarrel& B ) {
    /*
            ...
            This ensure that all barrels have their equad shares of rounds being
       shot. Stop when having shot enough. Ignore if infinite ammo or setting
       ammo elapsed.
    */
    if ( IsUnlimitedAmmo() )
        return true;
    ...
    if ( B.iAmmoElapsed > 0 )
        return true;
    /* If ammo elapsed matches, stop this barrel from firing as intended. If
     * doesn't, there is issue but let it fires. Not a big deal. */
    return ( iAmmoElapsed == BarrelAmmoElapsed() ) ? false : true;
}

void CWeaponStatMgun::BarrelAmmoElapsedCorrecting() {
    /* Call after setting ammo elapsed for the gun. Correct new ammo elapsed for
     * each barrel. */
    if ( m_barrels.size() == 0 )
        return;
...
void CWeaponStatMgun::AddShotEffector() {
    if ( OwnerActor() ) {
#ifdef STATIONARYMGUN_NEW
        if ( Camera() && ( Camera()->tag == eCamFirst ) &&
             m_shot_effector.size() && xr_strcmp( m_shot_effector, "nil" ) ) {
        ...
        CCameraShotEffector* S = smart_cast< CCameraShotEffector* >(
            OwnerActor()->Cameras().GetCamEffector( eCEShot ) );
...
float CWeaponStatMgun::GetFireDispersion( bool with_cartridge,
                                          bool for_crosshair ) {
    if ( !with_cartridge )
...
float CWeaponStatMgun::GetFireDispersion( float cartridge_k,
                                          bool for_crosshair ) {
    float fire_disp = GetBaseDispersion( cartridge_k );
    if ( OwnerActor() ) {
        fire_disp +=
            ( OwnerActor()->GetWeaponAccuracyStm() * fireDispersionOwnerScale );
    } else if ( Owner() ) {
        fire_disp += ( Owner()->cast_stalker()->GetWeaponAccuracy() *
                       fireDispersionOwnerScale );
    }
    return fire_disp;
}
...
void CWeaponStatMgun::switch2_Fire() {
    if ( m_magazine.size() == 0 ) {
        Fmatrix xfm = Fmatrix().mul_43(
            XFORM(),
            Visual()->dcast_PKinematics()->LL_GetTransform( m_rotate_x_bone ) );
...
void CWeaponStatMgun::switch2_Reload() {
    ...
    if ( m_next_ammoType_on_reload.valid() &&
         ( GetAmmoCount_forType(
               m_ammoTypes[ m_next_ammoType_on_reload.get() ] ) == 0 ) ) {
        m_next_ammoType_on_reload.reset();
        return;
    }
...
bool CWeaponStatMgun::IsReloadConsume() {
    if ( m_reload_consume_callback && strlen( m_reload_consume_callback ) ) {
        ...
        if ( ai().script_engine().functor( m_reload_consume_callback,
                                           lua_function ) ) {
            if ( lua_function(
                     lua_game_object(), this,
                     ( Owner() ) ? Owner()->lua_game_object() : nullptr ) )
                return true;
            else
                return false;
        }
...
CInventory* CWeaponStatMgun::GetInventory() {
    return ( Owner() && Owner()->cast_inventory_owner() )
               ? &Owner()->cast_inventory_owner()->inventory()
               : nullptr;
}
...
void CWeaponStatMgun::ReloadMagazine() {
    ...
    if ( !m_bLockType && !m_magazine.empty() &&
         ( !m_pCurrentAmmo ||
           xr_strcmp( m_pCurrentAmmo->cNameSect(),
...
void CWeaponStatMgun::UnloadMagazine( bool spawn_ammo ) {
    ...
    if ( spawn_ammo && inv ) {
        ...
        for ( l_it = l_ammo.begin(); l_ammo.end() != l_it; ++l_it ) {
            CWeaponAmmo* l_pA =
                smart_cast< CWeaponAmmo* >( inv->GetAny( l_it->first ) );
            if ( l_pA ) {
...
u16 CWeaponStatMgun::AddCartridge( u16 cnt ) {
    if ( iAmmoElapsed >= iMagazineSize )
        return 0;
    m_pCurrentAmmo = smart_cast< CWeaponAmmo* >(
        GetInventory()->GetAny( m_ammoTypes[ m_ammoType ].c_str() ) );
    if ( m_DefaultCartridge.m_LocalAmmoType != m_ammoType ) {
        m_DefaultCartridge.Load( m_ammoTypes[ m_ammoType ].c_str(),
                                 m_ammoType );
    }
...
int CWeaponStatMgun::GetAmmoCount( u8 ammo_type ) {
    return ( GetInventory() && ( ammo_type < m_ammoTypes.size() ) )
               ? GetAmmoCount_forType( m_ammoTypes[ ammo_type ] )
               : 0;
}
