...
void rewarding_event_handlers::OnWeapon_Fire( u16 sender,
                                              u16 sender_weapon_id ) {
    for ( handlers_store_t::iterator i = m_events_store.begin(),
                                     ie = m_events_store.end();
          i != ie; ++i ) {
...
void rewarding_event_handlers::OnBullet_Fire( u16 sender,
                                              u16 sender_weapon_id,
    ...
    for ( handlers_store_t::iterator i = m_events_store.begin(),
                                     ie = m_events_store.end();
          i != ie; ++i ) {
        if ( i->second->OnBullet_Fire( sender, sender_weapon_id, position,
                                       direction ) ) {
            m_reward_action( i->first );
            break;
        }
    ...
    if ( m_null_hanlder )
        m_null_hanlder->OnBullet_Fire( sender, sender_weapon_id, position,
                                       direction );
}

void rewarding_event_handlers::OnBullet_Hit( CObject const* hitter,
...
void rewarding_event_handlers::OnPlayerTakeArtefact(
...
void rewarding_event_handlers::OnPlayerDropArtefact(
    game_PlayerState const* ps ) {
    for ( handlers_store_t::iterator i = m_events_store.begin(),
                                     ie = m_events_store.end();
          i != ie; ++i ) {
...
void rewarding_event_handlers::OnPlayerBringArtefact(
    game_PlayerState const* ps ) {
    for ( handlers_store_t::iterator i = m_events_store.begin(),
                                     ie = m_events_store.end();
          i != ie; ++i ) {
...
void rewarding_event_handlers::OnPlayerKilled(
    ...
    for ( handlers_store_t::iterator i = m_events_store.begin(),
                                     ie = m_events_store.end();
          i != ie; ++i ) {
        if ( i->second->OnPlayerKilled( killer_id, target_id, weapon_id,
                                        kill_type ) ) {
            m_reward_action( i->first );
            break;
        }
    ...
    if ( m_null_hanlder )
        m_null_hanlder->OnPlayerKilled( killer_id, target_id, weapon_id,
                                        kill_type );
}
...
void rewarding_event_handlers::set_null_handler(
    reward_event_handler* new_handler ) {
    VERIFY( new_handler );
