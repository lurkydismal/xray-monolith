...
#define TEMPLATE_SPECIALIZATION                                               \
    template <                                                                \
        typename _dist_type, typename _priority_queue,                        \
        typename _vertex_manager, typename _vertex_allocator,                 \
        bool euclidian_heuristics, typename _data_storage_base,               \
        template < typename _T > class _vertex,                               \
        template < typename _1,                                               \
                   typename _2 > class _builder_allocator_constructor,        \
        template < typename _1, typename _2, typename _3,                     \
                   template < typename _1, typename _2 > class _4 > class     \
        _manager_builder_allocator_constructor,                               \
        template <                                                            \
            typename _algorithm, typename _manager, typename _builder,        \
            typename _allocator, template < typename _T > class _vertex,      \
            template < typename _1,                                           \
                       typename _2 > class _builder_allocator_constructor,    \
            template < typename _1, typename _2, typename _3,                 \
                       template < typename _1, typename _2 > class _4 > class \
            _manager_builder_allocator_constructor > class                    \
        _data_storage_constructor,                                            \
        typename _iteration_type >

#define CSDijkstra                                                          \
    CDijkstra< _dist_type, _priority_queue, _vertex_manager,                \
               _vertex_allocator, euclidian_heuristics, _data_storage_base, \
               _vertex, _builder_allocator_constructor,                     \
               _manager_builder_allocator_constructor,                      \
...
IC void CSDijkstra::initialize( _PathManager& path_manager ) {
    ...
    // create a node
    CGraphVertex& start =
        data_storage().create_vertex( path_manager.start_node() );
...
IC bool CSDijkstra::step( _PathManager& path_manager ) {
    ...
    for ( ; i != e; ++i ) {
        ...
        if ( data_storage().is_visited( neighbour_index ) ) {
            ...
            CGraphVertex& neighbour =
                data_storage().get_node( neighbour_index );
            // check if this node is in the opened list
            if ( data_storage().is_opened( neighbour ) ) {
                // compute 'g' for the node
                _dist_type f =
                    best.f() +
                    path_manager.evaluate( best.index(), neighbour_index, i );
            ...
            CGraphVertex& neighbour =
                data_storage().create_vertex( neighbour_index );
            // fill the corresponding node parameters
            neighbour.f() = best.f() + path_manager.evaluate(
                                           best.index(), neighbour_index, i );
...
IC bool CSDijkstra::find( _PathManager& path_manager ) {
    ...
    for ( _iteration_type i = _iteration_type( 0 );
          !data_storage().is_opened_empty(); ++i ) {
