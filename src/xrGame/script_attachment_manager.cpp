...
static void update_visbox_attachment( IKinematics* k ) {
    script_attachment* att =
        static_cast< script_attachment* >( k->GetUpdateCallbackParam() );
    if ( !att )
        return;
    ...
    if ( k->NeedUCalc() ) {
        for ( auto& pair : *att->GetAttachments() ) {
            ...
            if ( child_att->GetType() != att->GetType() )
                continue;
    ...
    render.draw_obb( box, radius, D3DCOLOR_XRGB( 150, 0, 150 ),
                     att->GetType() == eSA_HUD );
#endif
}
...
script_attachment* script_attachment::AddAttachment( LPCSTR name,
                                                     LPCSTR model_name ) {
...
void script_attachment::spatial_move() {
    if ( !SpatialComponent->spatial.node_ptr )
        return;
    renderable.xform.transform_tiny( SpatialComponent->spatial.sphere.P,
                                     renderable.visual->getVisData().sphere.P );
    Fvector& scale = m_attachment_offset[ 2 ];
    SpatialComponent->spatial.sphere.R =
        renderable.visual->getVisData().sphere.R *
        std::max( scale.x, std::max( scale.y, scale.z ) );
...
void script_attachment::renderable_Render( IDSGraphManager* DM ) {
    ...
    if ( m_script_light ) {
        ...
        if ( m_kinematics->LL_BoneCount() > m_script_light_bone )
            ...
            light_bone =
                m_kinematics->LL_GetTransform( m_kinematics->LL_GetBoneRoot() );
...
void script_attachment::Render( IKinematics* model,
                                Fmatrix* mat,
                                IDSGraphManager* DM ) {
    if ( !model || ( m_bone_callbacks[ 0 ] &&
                     m_bone_callbacks[ 0 ]->m_bone_id != BI_NONE ) )
...
void script_attachment::Update() {
    if ( m_last_upd_frame == Device.dwFrame )
        return;
        ...
        for ( auto& pair : m_bone_callbacks ) {
            ...
            if ( cb->m_func ) {
                cb->m_mat.set( ( *( cb->m_func ) )(
                    m_kinematics->LL_GetBoneInstance( cb->m_attachment_bone_id )
                        .mTransformHidden, // Transform without bone callback
                                           // modifier
            ...
            if ( m_parent_object ) {
                if ( GetType() == eSA_HUD ) {
                    ...
                    if ( itm ) {
                        if ( bone >=
                             itm->HudItemData()->m_model->LL_BoneCount() )
                            target = Fidentity;
                        else
                            target =
                                itm->HudItemData()->m_model->LL_GetTransform(
                                    bone );
                    ...
                    if ( act ) {
                        if ( bone >= g_player_hud->m_model->dcast_PKinematics()
                                         ->LL_BoneCount() )
                            target = Fidentity;
                        else
                            target =
                                ( bone < 21 )
                                    ? g_player_hud->m_model_2
                                          ->dcast_PKinematics()
                                          ->LL_GetTransform( bone )
                                    : g_player_hud->m_model->dcast_PKinematics()
                                          ->LL_GetTransform( bone );
                ...
                if ( bone >= m_parent_object->Visual()
                                 ->dcast_PKinematics()
                                 ->LL_BoneCount() )
                    target = Fidentity;
                else
                    target = m_parent_object->Visual()
                                 ->dcast_PKinematics()
                                 ->LL_GetTransform( bone );
            ...
            if ( m_parent_attachment ) {
                if ( bone >= m_parent_attachment->m_kinematics->LL_BoneCount() )
                    ...
                    target = m_parent_attachment->m_kinematics->LL_GetTransform(
                        bone );
            }
        }
    }
...
::luabind::object script_attachment::GetParent() {
    ...
    table[ "object" ] =
        m_parent_object ? m_parent_object->lua_game_object() : nullptr;
    table[ "attachment" ] = m_parent_attachment ? m_parent_attachment : nullptr;
    table[ "level" ] = m_parent_level;
    return table;
}

script_attachment* script_attachment::AddChild( LPCSTR name,
                                                script_attachment* att ) {
    R_ASSERT( att );
...
void script_attachment::IterateAttachments(
...
u32 script_attachment::motion_length( const MotionID& M,
                                      const CMotionDef*& md,
                                      float speed ) {
    IKinematicsAnimated* k = renderable.visual->dcast_PKinematicsAnimated();
...
void script_attachment::SetBoneVisible( u16 bone_id,
                                        bool bVisibility,
                                        bool bRecursive ) {
...
void script_attachment::SetParentBone( LPCSTR bone ) {
    if ( m_parent_object ) {
        m_parent_bone = m_parent_object->lua_game_object()->bone_id(
            bone, m_type == eSA_HUD );
        return;
    }
...
Fmatrix script_attachment::bone_transform( u16 bone_id ) {
    if ( bone_id == BI_NONE || bone_id >= m_kinematics->LL_BoneCount() ) {
        if ( Core.isDebug() && print_bone_warnings ) {
            Msg( "![bone_position] Incorrect bone_id provided for %s (%s), "
                 "fallback to root bone",
                 GetName(), GetModelScript() );
...
u16 script_attachment::bone_parent( u16 bone_id ) {
    if ( bone_id == m_kinematics->LL_GetBoneRoot() ||
         bone_id >= m_kinematics->LL_BoneCount() )
        return BI_NONE;
...
void script_attachment::SetType( u16 type ) {
    ...
    if ( pVisual )
        pVisual->MarkIgnoreOptimization( m_type == eSA_HUD ||
                                         m_type == eSA_CamAttached );
}
...
void script_attachment::LoadModel( LPCSTR model_name, bool keep_bc ) {
    ...
    // Bone Callbacks
    if ( keep_bc && m_bone_callbacks.size() &&
         count_prev <= m_kinematics->LL_BoneCount() ) {
        for ( auto& pair : m_bone_callbacks ) {
            m_kinematics->LL_GetBoneInstance( pair.first )
                .set_callback( bctCustom, ScriptAttachmentBoneCallback,
                               pair.second, pair.second->m_overwrite );
        }
...
const ::luabind::object& script_attachment::GetUserdata() const {
    if ( !m_userdata ) {
        const_cast< ::luabind::object* >( m_userdata ) =
            xr_new< ::luabind::object >();
...
void script_attachment::SetUserdata( ::luabind::object obj ) {
    ...
    if ( obj.type() != LUA_TTABLE ) {
        Msg( "![Script Attachment]: Trying to set userdata to wrong type! "
             "(Allowed types: table, nil)" );
        return;
    }
...
void script_attachment::SetScriptUI( LPCSTR ui_func ) {
    if ( m_script_ui_func != nullptr &&
         0 == xr_strcmp( m_script_ui_func, ui_func ) )
        return;
    ...
    if ( ai().script_engine().functor( ui_func, funct ) ) {
        ...
        if ( pScriptWnd ) {
            ...
            Msg( "![Script Attachment]: Failed to load script UI [%s]!",
                 ui_func );
    } else
        Msg( "![Script Attachment]: Script UI functor [%s] does not exist!",
             ui_func );
}
...
void script_attachment::ScriptAttachmentBoneCallback( CBoneInstance* B ) {
    script_attachment_bone_cb* params =
        static_cast< script_attachment_bone_cb* >( B->callback_param() );
    bool bVisible =
        params->m_attachment->GetBoneVisible( params->m_attachment_bone_id );
    Fmatrix& target = bVisible ? B->mTransform : B->mTransformHidden;
    target = params->m_mat;
}

void script_attachment::SetBoneCallback( u16 bone_id,
                                         LPCSTR parent_bone,
                                         bool overwrite ) {
    if ( m_parent_object ) {
        SetBoneCallback( bone_id,
                         m_parent_object->lua_game_object()->bone_id(
                             parent_bone, m_type == eSA_HUD ),
                         overwrite );
        return;
    }

    if ( m_parent_attachment ) {
        SetBoneCallback( bone_id, m_parent_attachment->bone_id( parent_bone ),
                         overwrite );
        return;
    }
...
void script_attachment::SetBoneCallback( LPCSTR bone,
                                         LPCSTR parent_bone,
                                         bool overwrite ) {
    if ( m_parent_object ) {
        SetBoneCallback( bone_id( bone ),
                         m_parent_object->lua_game_object()->bone_id(
                             parent_bone, m_type == eSA_HUD ),
                         overwrite );
        return;
    ...
    if ( m_parent_attachment ) {
        SetBoneCallback( bone_id( bone ),
                         m_parent_attachment->bone_id( parent_bone ),
                         overwrite );
        return;
    }
...
void script_attachment::SetBoneCallback( u16 bone_id,
    ...
    m_bone_callbacks[ bone_id ] = xr_new< script_attachment_bone_cb >(
        parent_bone, this, bone_id, overwrite );
    m_kinematics->LL_GetBoneInstance( bone_id ).set_callback(
        bctCustom, ScriptAttachmentBoneCallback, m_bone_callbacks[ bone_id ],
        overwrite );
}

void script_attachment::SetBoneCallback(
    u16 bone_id,
    ...
    m_bone_callbacks[ bone_id ] =
        xr_new< script_attachment_bone_cb >( func, this, bone_id, overwrite );
    CBoneInstance& bInst = m_kinematics->LL_GetBoneInstance( bone_id );
    bInst.set_callback( bctCustom, ScriptAttachmentBoneCallback,
                        m_bone_callbacks[ bone_id ], overwrite );
    ( m_bone_callbacks[ bone_id ]->m_mat )
        .set( GetBoneVisible( bone_id ) ? bInst.mTransformHidden
                                        : bInst.mTransform );
}
...
::luabind::object script_attachment::GetShaders() {
    ...
    xr_vector< IRenderVisual* >* children_invisible =
        renderable.visual->get_children_invisible();
...
void script_attachment::SetShaderTexture( int id,
                                          LPCSTR shader,
                                          LPCSTR texture ) {
    if ( !renderable.visual )
        return;
