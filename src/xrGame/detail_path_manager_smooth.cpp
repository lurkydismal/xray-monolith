...
IC bool coincide_directions( const Fvector2& start_circle_center,
    ...
    if ( fis_zero( start_cross_product ) ) {
        Fvector2 circle_tangent_point_direction =
            Fvector2().sub( dest_tangent_point, dest_circle_center );
        Fvector2 start_tangent_dest_tangent_direction =
            Fvector2().sub( dest_tangent_point, start_tangent_point );
        float cp1 = start_tangent_dest_tangent_direction.crossproduct(
            circle_tangent_point_direction );
        return ( dest_cross_product * cp1 >= 0.f );
    }

    Fvector2 circle_tangent_point_direction =
        Fvector2().sub( start_tangent_point, start_circle_center );
    Fvector2 start_tangent_dest_tangent_direction =
        Fvector2().sub( dest_tangent_point, start_tangent_point );
    float cp1 = start_tangent_dest_tangent_direction.crossproduct(
        circle_tangent_point_direction );
...
bool CDetailPathManager::compute_tangent(
    ...
    if ( start_cp * dest_cp >= 0.f ) {
        ...
        if ( start_circle.center.similar( dest_circle.center, EPS_S ) ) {
            if ( fsimilar( start_circle.radius, dest_circle.radius, EPS_S ) ) {
                ...
                adjust_point( start_circle.center, dest_yaw,
                              start_circle.radius, tangents[ 0 ].point );
                if ( start_cp >= 0.f )
                    assign_angle( tangents[ 0 ].angle, start_yaw, dest_yaw,
                                  true, direction_type );
                else
                    assign_angle( tangents[ 0 ].angle, start_yaw, dest_yaw,
                                  false, direction_type );
            ...
            if ( ( r_diff_abs > distance ) &&
                 !fsimilar( r_diff_abs, distance, EPS_S ) )
                return ( false );
        ...
        if ( ( start_circle.radius + dest_circle.radius > distance ) &&
             !fsimilar( start_circle.radius + dest_circle.radius, distance,
                        EPS_S ) )
            return ( false );
    ...
    adjust_point( start_circle.center, yaw1 + alpha, start_circle.radius,
                  tangents[ 0 ].point );
    adjust_point( dest_circle.center, yaw2 + alpha, dest_circle.radius,
                  tangents[ 1 ].point );

    if ( coincide_directions( start_circle.center, tangents[ 0 ].point,
                              start_cp, dest_circle.center, tangents[ 1 ].point,
                              dest_cp ) ) {
        assign_angle(
            tangents[ 0 ].angle, start_yaw,
            yaw1 + alpha < PI_MUL_2 ? yaw1 + alpha : yaw1 + alpha - PI_MUL_2,
            start_cp >= 0, direction_type );
        assign_angle(
            tangents[ 1 ].angle, dest_yaw,
            yaw2 + alpha < PI_MUL_2 ? yaw2 + alpha : yaw2 + alpha - PI_MUL_2,
    ...
    adjust_point( dest_circle.center, yaw2 - alpha, dest_circle.radius,
                  tangents[ 1 ].point );
    assign_angle( tangents[ 0 ].angle, start_yaw,
                  yaw1 - alpha >= 0.f ? yaw1 - alpha : yaw1 - alpha + PI_MUL_2,
                  start_cp >= 0, direction_type );
    assign_angle( tangents[ 1 ].angle, dest_yaw,
                  yaw2 - alpha >= 0.f ? yaw2 - alpha : yaw2 - alpha + PI_MUL_2,
                  dest_cp >= 0, direction_type, false );
...
bool CDetailPathManager::build_circle_trajectory(
    ...
    if ( position.radius * _abs( position.angle ) <= min_dist ) {
        ...
        if ( vertex_id || ( !path->empty() && !path->back().position.similar(
                                                  t.position, EPS_S ) ) ) {
    ...
    if ( fis_zero( position.angular_velocity ) )
        ...
        int m = _min(
            iFloor( _abs( angle ) / position.angular_velocity * 10.f + 1.5f ),
            iFloor( position.radius * _abs( angle ) / min_dist + 1.5f ) );
#ifdef DEBUG
        if ( m >= 10000 ) {
            Msg( "! [position.radius=%f],[angle=%f],[m=%d]", position.radius,
                 angle, m );
            VERIFY( m < 10000 );
        }
#endif
    ...
    for ( int i = 0; i <= n + k; ++i ) {
        VERIFY( t.velocity != u32( -1 ) );
        t.position.x = -sin_apb( sina, cosa, sini, cosi ) * position.radius +
                       position.center.x;
        t.position.z = cos_apb( sina, cosa, sini, cosi ) * position.radius +
                       position.center.y;

        curr_vertex_id = ai().level_graph().check_position_in_direction(
            curr_vertex_id, curr_pos, t.position );
...
bool CDetailPathManager::build_line_trajectory(
    ...
    return (
        path ? ai().level_graph().create_straight_path< false >(
             ...
             : ai().level_graph().valid_vertex_id(
                   ai().level_graph().check_position_in_direction(
                       vertex_id, start.point, dest.point ) ) );
}
...
bool CDetailPathManager::build_trajectory(
    ...
    float straight_velocity = _abs( velocity( velocity2 ).linear_velocity );
    {
        for ( u32 i = 0; i < tangent_count; ++i ) {
            ...
            dist[ i ].time =
                ...
                tangents[ i ][ 0 ].point.distance_to(
                    tangents[ i ][ 1 ].point ) *
                    ( fis_zero( straight_velocity ) ? 0
                                                    : 1.f / straight_velocity );
        }
    }
...

    {
        for ( u32 i = 0, j = path ? path->size() : 0; i < tangent_count; ++i ) {
            ...
            if ( build_trajectory( start, dest, path, velocity1, velocity2,
                                   velocity3 ) ) {
                time = dist[ i ].time;
                return ( true );
            } else if ( path )
...
bool CDetailPathManager::compute_trajectory(
    ...
    for ( u32 i = 0; i < 2; ++i )
        for ( u32 j = 0; j < 2; ++j )
            if ( compute_tangent(
                     start, start_circles[ i ], dest, dest_circles[ j ],
                     tangent_points[ tangent_count ], direction_type ) ) {
                if ( !ai().level_graph().valid_vertex_position(
                         ai().level_graph().v3d(
                             tangent_points[ tangent_count ][ 0 ].point ) ) )
                    continue;

                if ( !ai().level_graph().valid_vertex_position(
                         ai().level_graph().v3d(
                             tangent_points[ tangent_count ][ 1 ].point ) ) )
                    continue;
...
bool CDetailPathManager::compute_path(
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( is_negative( start.linear_velocity ) ) {
            ...
            direction_type =
                EDirectionType( direction_type | eDirectionTypeFN );
        ...
        for ( ; i != e; ++i ) {
            ...
            if ( is_negative( dest.linear_velocity ) )
                direction_type =
                    EDirectionType( direction_type | eDirectionTypeSN );

            if ( direction_type & eDirectionTypeFN )
                dest.direction.mul( -1.f );

            m_temp_path.clear();
            if ( compute_trajectory( start, dest,
                                     m_tpTravelLine ? &m_temp_path : 0, time,
                                     ( *I ).index, real_straight_line_index,
                                     ( *i ).index, direction_type ) ) {
                if ( !m_try_min_time || ( time < min_time ) ) {
                    min_time = time;
                    if ( m_tpTravelLine ) {
                        m_tpTravelLine->resize( size );
                        m_tpTravelLine->insert( m_tpTravelLine->end(),
                                                m_temp_path.begin(),
                                                m_temp_path.end() );
...
void CDetailPathManager::validate_vertex_position(
    STrajectoryPoint& point ) const {
    if ( ai().level_graph().valid_vertex_position(
             ai().level_graph().v3d( point.position ) ) &&
    ...
    ai().level_graph().nearest(
        position, ai().level_graph().v3d( point.position ), contour );
...
bool CDetailPathManager::init_build( const xr_vector< u32 >& level_path,
    ...
    m_corrected_dest_position.y = ai().level_graph().vertex_plane_y(
        dest.vertex_id, dest.position.x, dest.position.y );
...
bool CDetailPathManager::fill_key_points( const xr_vector< u32 >& level_path,
                                          u32 intermediate_index,
                                          STrajectoryPoint& start,
                                          STrajectoryPoint& dest ) {
    ...
    for ( int _i = 0, i = 0, n = ( int )level_path.size() - 1, j = n, m = j;
          _i < n; ) {
        if ( !ai().level_graph().check_vertex_in_direction(
                 start_point.vertex_id, start_point.position,
                 level_path[ j ] ) ) {
            m = j;
            j = ( i + j ) / 2;
        } else {
        ...
        if ( i >= m - 1 ) {
            ...
            start_point.position = ai().level_graph().v2d(
                ai().level_graph().vertex_position( start_point.vertex_id ) );
            j = m = n;
        }
    }
    return ( true );
...
IC CDetailPathManager::STravelPoint
CDetailPathManager::compute_better_key_point( const STravelPoint& point0,
    ...
    do {
        ...
        //.
        if ( !ai().level_graph().valid_vertex_position(
                 ai().level_graph().v3d( direction21 ) ) )
            return ( point1 );

        if ( !reverse_order )
            vertex_id = ai().level_graph().check_position_in_direction(
                point0.vertex_id, point0.position, direction21 );
        else
            vertex_id = ai().level_graph().check_position_in_direction(
                point2.vertex_id, point2.position, direction21 );
        if ( ai().level_graph().valid_vertex_id( vertex_id ) ) {
            VERIFY( ai().level_graph().inside( vertex_id, direction21 ) );
            u32 test_vertex_id;
            if ( !reverse_order )
                test_vertex_id = ai().level_graph().check_position_in_direction(
                    vertex_id, direction21, point2.position );
            else
                test_vertex_id = ai().level_graph().check_position_in_direction(
                    vertex_id, direction21, point0.position );
...
IC bool CDetailPathManager::better_key_point( const STravelPoint& point0,
                                              const STravelPoint& point2,
...
void CDetailPathManager::build_path_via_key_points(
    ...
    for ( B != E ? ++I : I; I != E; ++I ) {
        VERIFY(
            ai().level_graph().inside( ( *I ).vertex_id, ( *I ).position ) );
        ...
        bool succeed =
            compute_path( s, d, &m_path, m_start_params,
                          last_point ? finish_params : m_start_params,
        ...
        if ( !m_path.empty() ) {
            if ( is_negative(
                     velocity( m_path.back().velocity ).linear_velocity ) )
                s.direction.mul( -1.f );
        }
    ...
    if ( ( B == E ) &&
         !compute_path( s, dest, &m_path, m_start_params, finish_params,
                        straight_line_index, straight_line_index_negative ) ) {
        m_path.clear();
        return;
    }
...
void CDetailPathManager::postprocess_key_points(
    ...
    if ( m_key_points[ m_key_points.size() - 2 ].position.similar(
             m_key_points[ m_key_points.size() - 1 ].position, EPS_S ) )
        m_key_points.pop_back();

    for ( int i = 1, n = ( int )m_key_points.size() - 1; i < n; ++i ) {
        STravelPoint key_point0 =
            compute_better_key_point( m_key_points[ i - 1 ], m_key_points[ i ],
                                      m_key_points[ i + 1 ], false );
        STravelPoint key_point1 =
            compute_better_key_point( m_key_points[ i + 1 ], m_key_points[ i ],
                                      m_key_points[ i - 1 ], true );
        {
            u32 vertex_id = ai().level_graph().check_position_in_direction(
                m_key_points[ i - 1 ].vertex_id, m_key_points[ i - 1 ].position,
                key_point0.position );
            if ( !ai().level_graph().valid_vertex_id( vertex_id ) ) {
                vertex_id = vertex_id;
            }
        ...
        {
            u32 vertex_id = ai().level_graph().check_position_in_direction(
                key_point0.vertex_id, key_point0.position,
        ...
        {
            u32 vertex_id = ai().level_graph().check_position_in_direction(
                key_point1.vertex_id, key_point1.position,
                m_key_points[ i + 1 ].position );
            if ( !ai().level_graph().valid_vertex_id( vertex_id ) ) {
                vertex_id = vertex_id;
            }
        }
        if ( better_key_point( m_key_points[ i - 1 ], m_key_points[ i + 1 ],
                               key_point0, key_point1 ) )
            m_key_points[ i ] = key_point0;
        else
            m_key_points[ i ] = key_point1;
        VERIFY( !m_key_points[ i ].position.similar(
            m_key_points[ i - 1 ].position, EPS_S ) );
    }
}
...
void CDetailPathManager::add_patrol_point() {
    ...
    if ( ( m_path.size() > 1 ) && m_state_patrol_path &&
         !fis_zero( extrapolate_length() ) ) {
        STravelPathPoint t;
        Fvector v;
        v.sub( m_path.back().position,
               m_path[ m_last_patrol_point - 1 ].position );
        v.y = 0.f;
        ...
        ai().level_graph().create_straight_path< false >(
            ...
            ai().level_graph().v2d( t.position ), m_path, m_path.back(), false,
            false );
    }
}
...
void CDetailPathManager::build_smooth_path( const xr_vector< u32 >& level_path,
    ...
    if ( m_restricted_object ) {
        ...
        start_pos.y = ai().level_graph().vertex_plane_y(
            start.vertex_id, start_pos.x, start_pos.z );
        ...
        if ( !( ( alvi && asp ) || ( !asp && !alvi ) ) ) {
            Msg( "! vertex [%d], position [%f][%f][%f], alvi[%c], asp[%c]",
                 start.vertex_id, VPUSH( start_pos ), alvi ? '+' : '-',
                 asp ? '+' : '-' );
        }
        VERIFY3( ( alvi && asp ) || ( !asp && !alvi ) ||
                     show_restrictions( m_restricted_object ),
                 "Invalid restrictions (see log for details) for object ",
    ...
    postprocess_key_points( level_path, intermediate_index, start, dest,
                            finish_params, straight_line_index,
