...
bool CScriptGameObject::is_body_turning() const {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ...
            make_string(
                "CGameObject [%s]: cannot access class member is_turning!",
    ...
    if ( !stalker )
        return ( !fis_zero( angle_difference(
            monster->movement().body_orientation().target.yaw,
            monster->movement().body_orientation().current.yaw ) ) );
    else
        return ( !fis_zero( angle_difference(
                     stalker->movement().head_orientation().target.yaw,
                     stalker->movement().head_orientation().current.yaw ) ) ||
                 !fis_zero( angle_difference(
                     monster->movement().body_orientation().target.yaw,
                     monster->movement().body_orientation().current.yaw ) ) );
}
...
u32 CScriptGameObject::add_sound( LPCSTR prefix,
                                  u32 max_count,
                                  ESoundTypes type,
                                  u32 priority,
                                  u32 mask,
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CSoundPlayer [%s]: cannot access class member add!",
                         object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    } else
        return ( monster->sound().add( prefix, max_count, type, priority, mask,
                                       internal_type, bone_name ) );
}

u32 CScriptGameObject::add_combat_sound( LPCSTR prefix,
                                         u32 max_count,
                                         ESoundTypes type,
                                         u32 priority,
                                         u32 mask,
    ...
    if ( !stalker ) {
        ...
        return ( stalker->sound().add(
            prefix, max_count, type, priority, mask, internal_type, bone_name,
...
u32 CScriptGameObject::add_sound( LPCSTR prefix,
                                  ...
                                  u32 mask,
                                  u32 internal_type ) {
    return ( add_sound( prefix, max_count, type, priority, mask, internal_type,
                        "bip01_head" ) );
}
...
void CScriptGameObject::set_sound_mask( u32 sound_mask ) {
    ...
    if ( !monster )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CSoundPlayer [%s]: cannot access class member set_sound_mask!",
                object().cNameSect().c_str() )
                .c_str() );
    else {
        CEntityAlive* entity_alive = smart_cast< CEntityAlive* >( monster );
        if ( entity_alive ) {
            VERIFY2( entity_alive->g_Alive(),
                     "Stalkers talk after death??? Say why??" );
        }
...
void CScriptGameObject::play_sound( u32 internal_type ) {
    ...
    if ( !monster )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CSoundPlayer [%s]: cannot access class member play!",
                         object().cNameSect().c_str() )
                .c_str() );
    else
...
bool CScriptGameObject::wounded() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member wounded!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }
...
void CScriptGameObject::set_enable_movement_collision( bool value ) {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CAI_Stalker : cannot set_enable_movement_collision for non "
            "CAI_Stalker objects" );
        return;
    }
...
CSightParams CScriptGameObject::sight_params() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member sight_params!",
                object().cNameSect().c_str() )
                .c_str() );
    ...
    result.m_object = action.object_to_look()
                          ? action.object_to_look()->lua_game_object()
                          : 0;
...
// AVO: directly set entity health instead of going throuhg normal health
// property which operates on delta
...
// TEST_OBJECT_CLASS(CScriptGameObject::IsPhysicsShellHolder,
// CPhysicsShellHolder) TEST_OBJECT_CLASS(CScriptGameObject::IsGrenade,
// CGrenade) TEST_OBJECT_CLASS(CScriptGameObject::IsBottleItem, CBottleItem)
...
SPECIFIC_CAST( CScriptGameObject::cast_WeaponMagazinedWGrenade,
