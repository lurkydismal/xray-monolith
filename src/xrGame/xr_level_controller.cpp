...
_keyboard keyboards[] = {
    ...
    // RavenAscendant: additional 100 entries, will let the bind console command
    // bind actions to values that won't get triggered by keyboard input but can
    // be triggered from the level.press_action functions
    { "kCUSTOM0", 1000 },
    { "kCUSTOM1", 1001 },
    { "kCUSTOM2", 1002 },
    { "kCUSTOM3", 1003 },
...
EGameActions get_binded_action( int _dik ) {
    for ( int idx = 0; idx < bindings_count; ++idx ) {
        ...
        bool b_is_group_matching = is_group_matching(
            binding->m_action->key_group, g_current_keygroup );

        if ( !b_is_group_matching )
            continue;

        if ( binding->m_keyboard[ 0 ] &&
             binding->m_keyboard[ 0 ]->dik == _dik && b_is_group_matching )
            return binding->m_action->id;

        if ( binding->m_keyboard[ 1 ] &&
             binding->m_keyboard[ 1 ]->dik == _dik && b_is_group_matching )
            return binding->m_action->id;
    }
    return kNOTBINDED;
}
...
void GetActionAllBinding( LPCSTR _action, char* dst_buff, int dst_buff_sz ) {
    ...
    if ( NULL == pbinding->m_keyboard[ 0 ] &&
         NULL == pbinding->m_keyboard[ 1 ] ) {
        xr_sprintf( dst_buff, dst_buff_sz, "%s",
                    CStringTable().translate( "st_key_notbinded" ).c_str() );
    } else
        xr_sprintf( dst_buff, dst_buff_sz, "%s%s%s", prim[ 0 ] ? prim : "",
                    ( sec[ 0 ] && prim[ 0 ] ) ? " , " : "",
                    sec[ 0 ] ? sec : "" );
}
        ...
        *key = 0;
...

        {
            for ( int idx = 0; idx < bindings_count; ++idx ) {
                ...
                bool b_conflict = !is_group_not_conflicted(
                    binding->m_action->key_group,
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        for ( int idx = 0; idx < bindings_count; ++idx ) {
            ...
            xr_sprintf( buff, "[%s] primary is[%s] secondary is[%s]",
                        pbinding->m_action->action_name,
                        ( pbinding->m_keyboard[ 0 ] )
                            ? pbinding->m_keyboard[ 0 ]->key_local_name.c_str()
                            : "NULL",
                        ( pbinding->m_keyboard[ 1 ] )
                            ? pbinding->m_keyboard[ 1 ]->key_local_name.c_str()
                            : "NULL" );
            Log( buff );
        }
        Log( "- --- Bind list end   ---" );
