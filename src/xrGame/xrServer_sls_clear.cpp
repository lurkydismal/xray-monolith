...
void xrServer::Perform_destroy( CSE_Abstract* object, u32 mode ) {
    ...
    if ( object->ID_Parent != 0xffff ) {
        ...
        // if (name)
        // 	Msg("Attempt to destroy object %s with parent %d", name,
        // object->ID_Parent);
    ...
    while ( !object->children.empty() ) {
        CSE_Abstract* child =
            game->get_entity_from_eid( object->children.back() );
        R_ASSERT2( child, make_string( "child registered but not found [%d]",
    ...
    //	Msg						("SLS-CLEAR : DESTROY
    //[%s][%s]",object->name(),object->name_replace());
...
void xrServer::SLS_Clear() {
    ...
    while ( !entities.empty() ) {
        ...
        if ( !found ) // R_ASSERT(found);
        {
            ...
            for ( ; I != E; ++I ) {
                if ( I->second )
                    Msg( "! ERROR: can't destroy object [%d][%s] with parent "
                         "[%d]",
                         I->second->ID,
                         I->second->s_name.size() ? I->second->s_name.c_str()
                                                  : "unknown",
                         I->second->ID_Parent );
                else
                    Msg( "! ERROR: can't destroy entity [%d][?] with parent[?]",
                         I->first );
            }
