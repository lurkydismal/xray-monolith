...
void CBastArtefact::ObjectContactCallback( bool& /**do_colide/**/,
                                           bool bo1,
                                           dContact& c,
                                           SGameMtl* /*material_1*/,
                                           SGameMtl* /*material_2*/ ) {
    ...
    CBastArtefact* pBastArtefact =
        l_pUD1 ? smart_cast< CBastArtefact* >( l_pUD1->ph_ref_object ) : NULL;
    if ( !pBastArtefact )
        pBastArtefact =
            l_pUD2 ? smart_cast< CBastArtefact* >( l_pUD2->ph_ref_object )
                   : NULL;
    if ( !pBastArtefact )
        return;
    ...
    pEntityAlive =
        l_pUD1 ? smart_cast< CEntityAlive* >( l_pUD1->ph_ref_object ) : NULL;
    if ( !pEntityAlive )
        pEntityAlive =
            l_pUD2 ? smart_cast< CEntityAlive* >( l_pUD2->ph_ref_object )
                   : NULL;
...
void CBastArtefact::UpdateCLChild() {
    // Log						("--- A -
    // CBastArtefact",*cName()); Log
    // ("--- A - CBastArtefact",renderable.xform);
    ...
    if ( getVisible() && m_pPhysicsShell ) {
        ...
        if ( m_AttakingEntity ) {
            if ( m_AttakingEntity->g_Alive() && m_fEnergy > m_fStrikeImpulse ) {
                ...
                m_pPhysicsShell->applyImpulse(
                    dir, m_fStrikeImpulse * Device.fTimeDelta *
        ...
        if ( m_fEnergy > 0 &&
             ::Random.randF( 0.f, 1.0f ) <
                 ( m_fEnergy / ( m_fStrikeImpulse * 100.f ) ) ) {
            intrusive_ptr< CParticlesObject > pStaticPG =
                Particles::Details::Create( *m_sParticleName, TRUE );
            Fmatrix pos;
            pos.set( XFORM() );
            Fvector vel;
...
void CBastArtefact::feel_touch_delete( CObject* O ) {
    ...
    if ( pEntityAlive ) {
        m_AliveList.erase(
            std::find( m_AliveList.begin(), m_AliveList.end(), pEntityAlive ) );
    }
}
