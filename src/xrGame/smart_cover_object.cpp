...
BOOL object::net_Spawn( CSE_Abstract* server_entity ) {
    CSE_SmartCover* smart_cover =
        smart_cast< CSE_SmartCover* >( server_entity );
    VERIFY( smart_cover );

    if ( !smart_cover->m_description.size() )
        Msg( "! smart cover %s has no description",
             smart_cover->name_replace() );
    ...
    if ( ai().get_alife() && smart_cover->m_description.size() )
        m_cover = ai().cover_manager().add_smart_cover(
            smart_cover->m_description.c_str(), *this,
...
void object::OnRender() {
    ...
    for ( ; I != E; ++I ) {
        ...
        Fvector up = XFORM().j;
        dbg_draw_frustum( loophole->fov() * 180.f / PI, loophole->range(), 1.f,
...
bool object::inside( Fvector const& position ) const {
    ...
    for ( ; i != e; ++i ) {
        switch ( ( *i ).type ) {
            case 0: {
                if ( ( *i ).data.sphere.P.distance_to( position ) <=
                     ( *i ).data.sphere.R )
                    return ( true );
