...
#ifdef EXPLOSIVE_CHANGE
    m_on_grenade_explode_callback._set( READ_IF_EXISTS(
        pSettings, r_string, section, "on_grenade_explode", "" ) );
    m_contact.enabled = !!READ_IF_EXISTS( pSettings, r_bool, section,
...
void CGrenade::Hit( SHit* pHDS ) {
    if ( ALife::eHitTypeExplosion == pHDS->hit_type &&
         m_grenade_detonation_threshold_hit < pHDS->damage() &&
    ...
    switch ( state ) {
        case eThrowEnd: {
            if ( m_thrown ) {
                ...
                if ( Local() ) {
#ifndef MASTER_GOLD
                    Msg( "Destroying local grenade[%d][%d]", ID(),
                         Device.dwFrame );
...
bool CGrenade::Useful() const {
    bool res = ( /* !m_throw && */ m_dwDestroyTime == 0xffffffff &&
                 CExplosive::Useful() &&
...
void CGrenade::PutNextToSlot() {
    ...
    if ( m_pInventory ) {
        ...
        Msg( "! PutNextToSlot : m_pInventory = NULL [%d][%d]", ID(),
             Device.dwFrame );

    if ( smart_cast< CInventoryOwner* >( H_Parent() ) && m_pInventory ) {
        CGrenade* pNext =
            smart_cast< CGrenade* >( m_pInventory->Same( this, true ) );
        if ( !pNext )
            pNext = smart_cast< CGrenade* >(
                m_pInventory->SameSlot( GRENADE_SLOT, this, true ) );

        VERIFY( pNext != this );

        if ( pNext ) {
            ::luabind::functor< CScriptGameObject* > funct;
            if ( ai().script_engine().functor( "_g.CMissile__PutNextToSlot",
                                               funct ) ) {
                CScriptGameObject* obj = funct( pNext->lua_game_object() );
                if ( !obj || !smart_cast< CGrenade* >( &obj->object() ) )
                    return;
            ...
            if ( m_pInventory->Slot( pNext->BaseSlot(), pNext ) ) {
                pNext->u_EventGen( P, GEG_PLAYER_ITEM2SLOT,
                                   pNext->H_Parent()->ID() );
                P.w_u16( pNext->ID() );
...
void CGrenade::DeactivateItem() {
    ...
    if ( !GetTmpPreDestroy() && Local() &&
         ( GetState() == eThrowStart || GetState() == eReady ||
           GetState() == eThrow ) ) {
        if ( m_fake_missile ) {
            CGrenade* pGrenade = smart_cast< CGrenade* >( m_fake_missile );
            if ( pGrenade ) {
                if ( m_pInventory->GetOwner() ) {
                    CActor* pActor =
                        smart_cast< CActor* >( m_pInventory->GetOwner() );
                    if ( pActor ) {
                        if ( !pActor->g_Alive() ) {
                            m_constpower = false;
                            m_fThrowForce = 0;
...
bool CGrenade::GetBriefInfo( II_BriefInfo& info ) {
    ...
    u32 ThisGrenadeCount =
        m_pInventory->dwfGetSameItemCount( cNameSect().c_str(), true );
...
void CGrenade::Contact( const Fvector& pos,
                        const Fvector& vel,
                        const Fvector& nor,
                        const LPCSTR mtl ) {
...
void CGrenade::GrenadeContactCallback( bool& do_colide,
                                       bool bo1,
                                       dContact& c,
                                       SGameMtl* material_1,
                                       SGameMtl* material_2 ) {
    ...
    CGrenade* gnd =
        gd1 ? smart_cast< CGrenade* >( gd1->ph_ref_object ) : nullptr;
    if ( !gnd ) {
    ...
    if ( gnd == nullptr || gnd->m_contact.enabled != true ||
         gnd->m_contact.contact == true )
        return;
...
void CGrenade::OnBeforeExplosion() {
    ...
    if ( m_on_grenade_explode_callback.size() ) {
        ...
        if ( ai().script_engine().functor(
                 m_on_grenade_explode_callback.c_str(), lua_function ) ) {
            Fvector pos;
            Fvector vel;
            Fvector nor;
            LPCSTR mtl = "";
