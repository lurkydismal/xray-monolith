...
struct CrosshairSettings {
    ...
    CrosshairSettings( Flags32 _flags,
                       string32 _shader,
                       string32 _texture,
...
struct TargetCrosshair {
    ...
    TargetCrosshair( CrosshairSettings& settings )
        : settings( settings ), pos( Fvector() ), opacity( 1.f ) {};
    ~TargetCrosshair() {};
...
struct CrosshairPair {
    ...
    CrosshairPair( CrosshairSettings& settings_near,
                   CrosshairSettings& settings_far )
    ...
    void RenderAimLine( Fvector va, Fvector vb ) const;
};
