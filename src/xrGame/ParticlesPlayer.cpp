...
CParticlesPlayer::SParticlesInfo* CParticlesPlayer::SBoneInfo::FindParticles(
    const shared_str& ps_name ) {
    for ( ParticlesInfoListIt it = particles.begin(); it != particles.end();
          it++ )
...
void CParticlesPlayer::LoadParticles( IKinematics* K ) {
    ...
    if ( ini && ini->section_exist( "particle_bones" ) ) {
        ...
        for ( CInifile::SectCIt I = data.Data.begin(); I != data.Data.end();
            ...
            VERIFY3( index != BI_NONE, "Particles bone not found",
                     *item.first );
            if ( index != BI_NONE ) {
                Fvector offs;
    ...
    if ( m_Bones.empty() ) {
        bone_mask = u64( 1 ) << u64( 0 );
        m_Bones.push_back(
            SBoneInfo( K->LL_GetBoneRoot(), Fvector().set( 0, 0, 0 ) ) );
    }
}
...
void CParticlesPlayer::net_DestroyParticles() {
    ...
    for ( BoneInfoVecIt b_it = m_Bones.begin(); b_it != m_Bones.end();
          b_it++ ) {
        SBoneInfo& b_info = *b_it;

        for ( ParticlesInfoListIt p_it = b_info.particles.begin();
              p_it != b_info.particles.end(); p_it++ ) {
...
CParticlesPlayer::SBoneInfo* CParticlesPlayer::get_nearest_bone_info(
    IKinematics* K,
    u16 bone_index ) {
    u16 play_bone = bone_index;
    while ( ( BI_NONE != play_bone ) &&
            !( bone_mask & ( u64( 1 ) << u64( play_bone ) ) ) ) {
        play_bone = K->LL_GetData( play_bone ).GetParentID();
    }
    return get_bone_info( play_bone );
}

void CParticlesPlayer::StartParticles( const shared_str& particles_name,
                                       u16 bone_num,
                                       const Fvector& dir,
                                       u16 sender_id,
                                       int life_time,
    ...
    StartParticles( particles_name, bone_num, xform, sender_id, life_time,
                    auto_stop );
}
...
void CParticlesPlayer::StartParticles( const shared_str& particles_name,
    ...
    SBoneInfo* pBoneInfo = get_nearest_bone_info(
        smart_cast< IKinematics* >( object->Visual() ), bone_num );
    if ( !pBoneInfo )
        return;

    SParticlesInfo& particles_info =
        *pBoneInfo->AppendParticles( object, particles_name );
    ...
    Fmatrix m;
    m.setHPB( particles_info.angles.x, particles_info.angles.y,
              particles_info.angles.z );
...
void CParticlesPlayer::StartParticles( const shared_str& ps_name,
                                       const Fmatrix& xform,
                                       u16 sender_id,
                                       int life_time,
                                       bool auto_stop ) {
    ...
    for ( BoneInfoVecIt it = m_Bones.begin(); it != m_Bones.end(); it++ ) {
        SParticlesInfo& particles_info =
            *it->AppendParticles( object, ps_name );
        particles_info.sender_id = sender_id;
...
void CParticlesPlayer::StopParticles( u16 sender_id,
                                      u16 bone_id,
                                      bool bDestroy ) {
    if ( BI_NONE == bone_id ) {
        for ( BoneInfoVecIt it = m_Bones.begin(); it != m_Bones.end(); it++ )
...
void CParticlesPlayer::StopParticles( const shared_str& ps_name,
                                      u16 bone_id,
                                      bool bDestroy ) {
    if ( BI_NONE == bone_id ) {
        for ( BoneInfoVecIt it = m_Bones.begin(); it != m_Bones.end(); it++ )
...
void CParticlesPlayer::AutoStopParticles( const shared_str& ps_name,
                                          u16 bone_id,
                                          u32 life_time ) {
    if ( BI_NONE == bone_id ) {
        for ( BoneInfoVecIt it = m_Bones.begin(); it != m_Bones.end(); it++ ) {
...
void CParticlesPlayer::UpdateParticles() {
    ...
    for ( BoneInfoVecIt b_it = m_Bones.begin(); b_it != m_Bones.end();
        ...
        const auto RI = std::remove_if(
            b_info.particles.begin(), b_info.particles.end(),
            []( const SParticlesInfo& pi ) { return pi.ps == nullptr; } );
        b_info.particles.erase( RI, b_info.particles.end() );
    }
}

void CParticlesPlayer::GetBonePos( CObject* pObject,
                                   u16 bone_id,
...
void CParticlesPlayer::MakeXFORM( CObject* pObject,
                                  u16 bone_id,
                                  const Fvector& dir,
                                  const Fvector& offset,
                                  Fmatrix& result ) {
