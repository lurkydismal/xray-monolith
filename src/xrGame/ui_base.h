...
class ui_core : public CDeviceResetNotifier {
    ...
    // Clip-frustum stack: base is the screen frustum, tighter convex polys push
    // on top (twin of m_Scissors).
...
public:
    ...
    IC float ClientToScreenScaledX( float left ) const {
        return left * m_current_scale->x;
    };

    IC float ClientToScreenScaledY( float top ) const {
    ...
    const C2DFrustum& ActiveClipFrustum() const {
        return m_clip_stack.empty() ? ScreenFrustum() : *m_clip_stack.top();
    }
