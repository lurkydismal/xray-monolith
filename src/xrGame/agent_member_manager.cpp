...
void CAgentMemberManager::add( CEntity* member ) {
    ...
    iterator I = std::find_if( m_members.begin(), m_members.end(),
                               CMemberPredicate( stalker ) );
...
void CAgentMemberManager::remove( CEntity* member ) {
    ...
    // Force rebuild m_combat_members
    m_actuality = false;
...
void CAgentMemberManager::remove_links( CObject* object ) {
    ...
    for ( ; I != E; ++I ) {
        if ( ( *I )->grenade_reaction().m_grenade ) {
            const CGameObject* explosive = smart_cast< const CGameObject* >(
                ( *I )->grenade_reaction().m_grenade );
            VERIFY( explosive );
            if ( explosive->ID() == object->ID() )
                ( *I )->grenade_reaction().clear();
            else {
                CGrenade const* grenade =
                    smart_cast< CGrenade const* >( explosive );
                if ( grenade && grenade->CurrentParentID() == object->ID() )
                    ( *I )->grenade_reaction().clear();
            }
        }

        if ( ( *I )->grenade_reaction().m_game_object &&
             ( ( *I )->grenade_reaction().m_game_object->ID() ==
               object->ID() ) )
            ( *I )->grenade_reaction().clear();

        if ( ( *I )->member_death_reaction().m_member &&
             ( ( *I )->member_death_reaction().m_member->ID() ==
               object->ID() ) )
...
void CAgentMemberManager::unregister_in_combat( const CAI_Stalker* object ) {
    //	if (!object->group_behaviour()) {
    //		VERIFY
    //(!registered_in_combat(object)); 		return;
    //	}
    ...
    m_actuality =
        m_actuality &&
        ( ( m_combat_mask & ( squad_mask_type( -1 ) ^ m ) ) == m_combat_mask );
    m_combat_mask &= squad_mask_type( -1 ) ^ m;
}

bool CAgentMemberManager::registered_in_combat(
...
CAgentMemberManager::squad_mask_type
CAgentMemberManager::non_combat_members_mask() const {
...
bool CAgentMemberManager::can_detour() const {
    ...
    return ( !in_detour_count ||
             ( in_detour_count < ( members().size() / 2 ) ) );
}
...
MemorySpace::squad_mask_type CAgentMemberManager::mask(
    const ALife::_OBJECT_ID& object_id ) const {
    const_iterator I = std::find_if( members().begin(), members().end(),
                                     CMemberPredicate2( object_id ) );
    VERIFY( I != members().end() );
    return ( MemorySpace::squad_mask_type( 1 ) << ( I - members().begin() ) );
}

CMemberOrder* CAgentMemberManager::get_member(
    const ALife::_OBJECT_ID& object_id ) {
    iterator I = std::find_if( members().begin(), members().end(),
                               CMemberPredicate2( object_id ) );
    if ( I == members().end() )
        return ( 0 );
...
bool CAgentMemberManager::can_throw_grenade( const Fvector& location ) const {
    ...
    MEMBER_STORAGE::const_iterator E = members().end();
    for ( ; I != E; ++I ) {
        if ( ( *I )->object().Position().distance_to_sqr( location ) <=
             member_danger_radius_sqr )
            return ( false );

        if ( !( *I )->cover() )
            continue;

        if ( ( *I )->cover()->m_position.distance_to_sqr( location ) <=
             cover_danger_radius_sqr )
            return ( false );
    }
