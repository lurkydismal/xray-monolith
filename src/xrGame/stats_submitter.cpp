...
stats_submitter::stats_submitter( CGameSpy_Full* fullgs )
    ...
    // m_fullgs_obj				= fullgs;
    // m_atlas_obj					=
    // fullgs->GetGameSpyATLAS(); VERIFY(m_atlas_obj);
    m_last_operation_profile = NULL;
    // m_atlas_report				= NULL;
    m_last_best_scores = NULL;
    // ZeroMemory					(m_atlas_connection_id,
    // sizeof(m_atlas_connection_id));
}

stats_submitter::~stats_submitter() {}

void stats_submitter::reward_with_award( enum_awards_t award_id,
                                         u32 const count,
...
void stats_submitter::set_best_scores( all_best_scores_t const* scores,
                                       gamespy_gp::profile const* profile,
                                       store_operation_cb opcb ) {
    if ( !opcb ) {
        m_last_operation_cb.bind( this, &stats_submitter::onlylog_operation );
...
void __stdcall stats_submitter::onlylog_operation( bool const result,
...
bool stats_submitter::prepare_report() {
    ...
    // SCResult tmp_res = m_atlas_obj->CreateReport(ATLAS_RULE_SET_VERSION, 1,
    // 0, &m_atlas_report); VERIFY2((tmp_res == SCResult_NO_ERROR) &&
    // (m_atlas_report), "failed to create atlas report"); if ((tmp_res !=
    // SCResult_NO_ERROR) || (!m_atlas_report)) 	return false;
...
void stats_submitter::quick_reward_with_award(
...
void stats_submitter::quick_set_best_scores(
    all_best_scores_t const* scores,
    gamespy_gp::profile const* profile ) {
    // VERIFY(scores);
    // profile_store*		tmp_prof_store = MainMenu()->GetProfileStore();
    // VERIFY(tmp_prof_store);
    // best_scores_store*	tmp_bs_store =
    // tmp_prof_store->get_best_scores_store(); VERIFY(tmp_bs_store);

    // all_best_scores_t&	tmp_best_scores =
    // tmp_bs_store->get_player_best_scores();
    //
...
void stats_submitter::save_file( gamespy_gp::profile const* profile ) {
    // profile_store*		tmp_prof_store = MainMenu()->GetProfileStore();
    // VERIFY(tmp_prof_store);
    // awards_store*		tmp_awards_store =
    // tmp_prof_store->get_awards_store(); VERIFY(tmp_awards_store);
    // best_scores_store*	tmp_bs_store =
    // tmp_prof_store->get_best_scores_store(); VERIFY(tmp_prof_store);

    // all_awards_t&		tmp_awards =
    // tmp_awards_store->get_player_awards(); all_best_scores_t&
    // tmp_best_scores = tmp_bs_store->get_player_best_scores();
    //
    ...
    //_time32				(&tmp_time);
    //
    // ltx_to_write.w_s32(profile_data_section, profile_id_line,
    // profile->m_profile_id); ltx_to_write.w_u32(profile_data_section,
    // profile_last_submit_time, static_cast<u32>(tmp_time));

    // IWriter* tmp_writer = FS.w_open("$app_data_root$",
    // profile_store_file_name); m_ltx_file.sign_and_save(*tmp_writer);
    // FS.w_close(tmp_writer);
}

void stats_submitter::fill_private_key(
    crypto::xr_dsa::private_key_t& priv_key_dest ) {
