...
IC float CLevelGraph::distance( const Fvector& position,
                                const Fvector& point0,
                                const Fvector& point1 ) const {
    Fvector c, V;
...
IC void CLevelGraph::project_point( const Fplane& plane,
                                    Fvector& point ) const {
...
IC float CLevelGraph::distance( const CLevelGraph::CVertex* _node,
                                u32 vertex_id1 ) const {
    return ( distance( _node, vertex( vertex_id1 ) ) );
}

IC float CLevelGraph::distance( u32 vertex_id0,
                                const CLevelGraph::CVertex* _node ) const {
    return ( distance( vertex( vertex_id0 ), _node ) );
}

IC float CLevelGraph::distance( const CLevelGraph::CVertex* node0,
                                const CLevelGraph::CVertex* node1 ) const {
    return ( vertex_position( node0 ).distance_to( vertex_position( node1 ) ) );
}

IC float CLevelGraph::distance( const Fvector& position,
                                const u32 vertex_id ) const {
    return ( distance( position, vertex( vertex_id ) ) );
}

IC float CLevelGraph::distance( const u32 vertex_id,
...
IC float CLevelGraph::distance( const CPosition& position,
                                const u32 vertex_id ) const {
    return ( distance( vertex_position( position ), vertex_id ) );
}
...
IC bool CLevelGraph::similar( const Fvector& tPoint0,
                              const Fvector& tPoint1 ) const {
    return ( ( _abs( tPoint0.x - tPoint1.x ) < EPS_L ) &&
             ( _abs( tPoint0.z - tPoint1.z ) < EPS_L ) );
}

IC bool CLevelGraph::inside( const Fvector& tPoint,
                             const CLevelGraph::SContour& tContour ) const {
    return ( ( tContour.v1.x - EPS_L <= tPoint.x ) &&
             ( tContour.v1.z - EPS_L <= tPoint.z ) &&
             ( tContour.v3.x + EPS_L >= tPoint.x ) &&
             ( tContour.v3.z + EPS_L >= tPoint.z ) );
}

IC void CLevelGraph::intersect( SSegment& tSegment,
                                const SContour& tContour0,
                                const SContour& tContour1 ) const {
    bool bFound = false;
...
IC float CLevelGraph::nearest( Fvector& Dest,
                               const Fvector& P,
                               const Fvector& A,
...
IC void CLevelGraph::contour( CLevelGraph::SContour& _contour,
                              u32 vertex_id ) const {
    contour( _contour, vertex( vertex_id ) );
}

IC void CLevelGraph::contour( CLevelGraph::SContour& _contour,
                              const CLevelGraph::CVertex* vertex ) const {
    ...
    _contour.v1.set( vertex_position.x - st, vertex_position.y,
                     vertex_position.z - st );
    _contour.v2.set( vertex_position.x + st, vertex_position.y,
                     vertex_position.z - st );
    _contour.v3.set( vertex_position.x + st, vertex_position.y,
                     vertex_position.z + st );
    _contour.v4.set( vertex_position.x - st, vertex_position.y,
                     vertex_position.z + st );
...
IC float CLevelGraph::nearest( Fvector& destination,
                               const Fvector& position,
                               const CLevelGraph::SContour& contour ) const {
...
IC bool CLevelGraph::intersect( Fvector& dst,
                                const Fvector& v1,
                                const Fvector& v2,
                                const Fvector& v3,
    ...
    if ( LevelGraph::eLineIntersectionIntersect !=
         intersect( v1.x, v1.z, v2.x, v2.z, v3.x, v3.z, v4.x, v4.z, &T.x,
                    &T.z ) )
        return ( false );
...
IC float CLevelGraph::square( float a1, float b1, float fAlpha ) const {
    ...
    return ( fAlpha * fAlpha * fAlpha * a * a / 6 +
             fAlpha * fAlpha * a * b / 2 + fAlpha * b * b / 2 );
}
...
IC float CLevelGraph::compute_square( float fAngle,
                                      float fAngleOfView,
                                      float b1,
                                      float b0,
                                      float b3,
...
IC float CLevelGraph::compute_high_square(
    float fAngle,
    float fAngleOfView,
    const CLevelGraph::CVertex* vertex ) const {
    return ( compute_square( fAngle, fAngleOfView,
...
IC float CLevelGraph::compute_low_square(
    float fAngle,
    float fAngleOfView,
    const CLevelGraph::CVertex* vertex ) const {
    return ( compute_square( fAngle, fAngleOfView,
...
IC float CLevelGraph::compute_high_square( float fAngle,
                                           float fAngleOfView,
                                           u32 dwNodeID ) const {
    return ( compute_high_square( fAngle, fAngleOfView, vertex( dwNodeID ) ) );
}

IC float CLevelGraph::compute_low_square( float fAngle,
                                          float fAngleOfView,
                                          u32 dwNodeID ) const {
    return ( compute_low_square( fAngle, fAngleOfView, vertex( dwNodeID ) ) );
}

IC float CLevelGraph::vertex_high_cover(
    const CLevelGraph::CVertex* vertex ) const {
    float _cover = 0.f;
    _cover += square( NORMALIZE_NODE_COVER_HIGH( vertex, 0 ),
                      NORMALIZE_NODE_COVER_HIGH( vertex, 1 ) );
    _cover += square( NORMALIZE_NODE_COVER_HIGH( vertex, 1 ),
                      NORMALIZE_NODE_COVER_HIGH( vertex, 2 ) );
    _cover += square( NORMALIZE_NODE_COVER_HIGH( vertex, 2 ),
                      NORMALIZE_NODE_COVER_HIGH( vertex, 3 ) );
    _cover += square( NORMALIZE_NODE_COVER_HIGH( vertex, 3 ),
                      NORMALIZE_NODE_COVER_HIGH( vertex, 0 ) );
    return ( _cover );
}

IC float CLevelGraph::vertex_low_cover(
    const CLevelGraph::CVertex* vertex ) const {
    float _cover = 0.f;
    _cover += square( NORMALIZE_NODE_COVER_LOW( vertex, 0 ),
                      NORMALIZE_NODE_COVER_LOW( vertex, 1 ) );
    _cover += square( NORMALIZE_NODE_COVER_LOW( vertex, 1 ),
                      NORMALIZE_NODE_COVER_LOW( vertex, 2 ) );
    _cover += square( NORMALIZE_NODE_COVER_LOW( vertex, 2 ),
                      NORMALIZE_NODE_COVER_LOW( vertex, 3 ) );
    _cover += square( NORMALIZE_NODE_COVER_LOW( vertex, 3 ),
                      NORMALIZE_NODE_COVER_LOW( vertex, 0 ) );
    return ( _cover );
}
...
IC float CLevelGraph::high_cover_in_direction(
    float angle,
    const CLevelGraph::CVertex* vertex ) const {
    return ( cover_in_direction( angle, NORMALIZE_NODE_COVER_HIGH( vertex, 0 ),
                                 NORMALIZE_NODE_COVER_HIGH( vertex, 1 ),
                                 NORMALIZE_NODE_COVER_HIGH( vertex, 2 ),
                                 NORMALIZE_NODE_COVER_HIGH( vertex, 3 ) ) );
}

IC float CLevelGraph::low_cover_in_direction(
    float angle,
    const CLevelGraph::CVertex* vertex ) const {
    return ( cover_in_direction( angle, NORMALIZE_NODE_COVER_LOW( vertex, 0 ),
                                 NORMALIZE_NODE_COVER_LOW( vertex, 1 ),
                                 NORMALIZE_NODE_COVER_LOW( vertex, 2 ),
                                 NORMALIZE_NODE_COVER_LOW( vertex, 3 ) ) );
}
...
IC float CLevelGraph::high_cover_in_direction( float angle,
                                               u32 vertex_id ) const {
    return ( high_cover_in_direction( angle, vertex( vertex_id ) ) );
}

IC float CLevelGraph::low_cover_in_direction( float angle,
                                              u32 vertex_id ) const {
...
IC u32 CLevelGraph::check_position_in_direction(
    u32 start_vertex_id,
    const Fvector2& start_position,
    const Fvector2& finish_position ) const {
    if ( inside( start_vertex_id, finish_position ) )
        return ( start_vertex_id );
    return ( check_position_in_direction_slow( start_vertex_id, start_position,
                                               finish_position ) );
}

IC bool CLevelGraph::check_vertex_in_direction( u32 start_vertex_id,
                                                const Fvector2& start_position,
                                                u32 finish_vertex_id ) const {
    if ( start_vertex_id == finish_vertex_id )
        return ( true );
    return ( check_vertex_in_direction_slow( start_vertex_id, start_position,
                                             finish_vertex_id ) );
}
...
IC u32 CLevelGraph::check_position_in_direction(
    ...
    return ( check_position_in_direction(
        start_vertex_id, Fvector2().set( start_position.x, start_position.z ),
...
float CLevelGraph::vertex_high_cover_angle(
    u32 vertex_id,
    float inc_angle,
    _predicate compare_predicate ) const {
...
template < class _predicate >
float CLevelGraph::vertex_low_cover_angle(
    u32 vertex_id,
    float inc_angle,
    _predicate compare_predicate ) const {
    float best_angle = 0.f;
