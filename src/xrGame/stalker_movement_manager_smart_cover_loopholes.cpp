...
float stalker_movement_manager_smart_cover::enter_path(
    ...
    for ( ; i != e; ++i ) {
        ...
        loophole_path( cover, ( *i )->id(), target_loophole_id,
                       m_temp_loophole_path );
        ...
        smart_cover::loophole const& loophole =
            this->loophole( cover, loophole_id );
        float new_value =
            cover.fov_position( loophole ).distance_to( position );
        new_value += ai().graph_engine()
    ...
    if ( result )
        result->insert( result->begin(),
                        smart_cover::transform_vertex( "", true ) );
...
void stalker_movement_manager_smart_cover::build_enter_path() {
    ...
    shared_str target_loophole_id = smart_cover::transform_vertex(
        m_target.cover_loophole() ? m_target.cover_loophole()->id() : "",
        false );
    Fvector const& position = object().Position();
    enter_path( &m_path, position, object().ai_location().level_vertex_id(),
                target_cover, target_loophole_id );

    if ( m_path.size() > 1 ) {
        m_current_transition =
            &action( target_cover, m_path[ 0 ], m_path[ 1 ] );
...
bool stalker_movement_manager_smart_cover::test_pick(
    Fvector source,
    ...
    struct parameters {
        ...
        inline parameters( float& range, CAI_Stalker& object )
            : m_range( &range ), m_object( &object ) {}
    }; // struct parameters

    struct test_pick {
        static BOOL callback( collide::rq_result& result, LPVOID user_data ) {
            parameters* const param = ( parameters* )user_data;
            if ( param->m_object->feel_vision_mtl_transp(
                     result.O, result.element ) < 1.f ) {
    ...
    collide::ray_defs ray_defs( source, direction, distance, CDB::OPT_CULL,
                                collide::rqtStatic );
    Level().ObjectSpace.RayQuery( m_ray_query_storage, ray_defs,
                                  &test_pick::callback, &params, NULL, NULL );
    return ( range == distance );
}

stalker_movement_manager_smart_cover::transition_action const&
stalker_movement_manager_smart_cover::nearest_action(
    smart_cover::cover const& cover,
    ...
    edge_type const* edge =
        cover.description()->transitions().edge( loophole_id0, loophole_id1 );
    VERIFY( edge );
    ...
    for ( ; i != e; ++i ) {
        ...
        for ( ; I != E; ++I ) {
            ...
            float const distance_sqr =
                action_position.distance_to_sqr( position );
            ...
            if ( ( *I )->has_animation() ) {
                ...
                float const y = ai().level_graph().vertex_plane_y(
                    vertex_id, action_position.x, action_position.z );
            ...
            if ( target_body_state &&
                 ( ( *I )->body_state() != *target_body_state ) &&
    ...
    VERIFY2(
        ...
        make_string(
            ...
            cover.id().c_str(), cover.description()->table_id().c_str(),
            loophole_id0.c_str(), loophole_id1.c_str(),
            !target_body_state
                ? ""
                : ( *target_body_state == eBodyStateStand
                        ? "stand"
                        : ( *target_body_state == eBodyStateCrouch
                                ? "crouch"
                                : "invalid!" ) ),
            VPUSH( position ) ) );
    return ( *result );
...
stalker_movement_manager_smart_cover::transition_action const&
stalker_movement_manager_smart_cover::action(
    smart_cover::cover const& cover,
    shared_str const& loophole_id0,
    ...
    ActionsList::const_iterator i =
        std::find_if( actions.begin(), actions.end(), &applicable::predicate );
...
void stalker_movement_manager_smart_cover::build_exit_path() {
    ...
    for ( ; I != E; ++I ) {
        ...
        loophole_path( cur_cover, cur_loophole.id(), exitable_loophole_id,
                       m_temp_loophole_path );
        VERIFY( !m_temp_loophole_path.empty() );

        float new_value = ai().graph_engine()
                              .m_string_algorithm->data_storage()
                              .get_best()
                              .g();
        float exit_edge =
            cur_cover.description()
                ->transitions()
                .edge( exitable_loophole_id,
        ...
        nearest_action( cur_cover, exitable_loophole_id,
                        smart_cover::transform_vertex( "", false ),
                        target_position, exit_position, exit_vertex_id,
                        &m_target.m_body_state );

        new_value += exit_path_weight( exit_vertex_id, exit_position,
                                       targe_vertex_id, target_position );
    ...
    if ( m_path.size() > 1 ) {
        m_current_transition =
            &action( *m_current.cover(), m_path[ 0 ], m_path[ 1 ] );
        m_current_transition_animation = &m_current_transition->animation();
    } else {
        m_current_transition = 0;
        m_current_transition_animation = 0;
...
void stalker_movement_manager_smart_cover::build_exit_path_to_cover() {
    ...
    Fvector target_position;
    target_cover.object().XFORM().transform_tiny(
        target_position, target_loophole.fov_position() );
    ...
    for ( ; I != E; ++I ) {
        ...
        shared_str const& exitable_loophole_id = ( *I )->id();
        loophole_path( current_cover, current_loophole.id(),
                       exitable_loophole_id, m_temp_loophole_path );
        VERIFY( !m_temp_loophole_path.empty() );
        ...
        smart_cover::transitions::action const& current_action = nearest_action(
            current_cover, exitable_loophole_id,
            smart_cover::transform_vertex( "", false ), target_position,
            exit_position, exit_vertex_id, &exit_body_state );

        buffer_vector< shared_str > temp(
            _alloca( sizeof( u32 ) * m_temp_loophole_path.size() ),
            m_temp_loophole_path.size(), m_temp_loophole_path.begin(),
...
void stalker_movement_manager_smart_cover::actualize_path() {
    ...
    shared_str current_loophole_id = smart_cover::transform_vertex(
        m_current.cover_loophole() ? m_current.cover_loophole()->id() : "",
        true );
    ...
    loophole_path( *m_current.cover(), current_loophole_id, target_loophole_id,
                   m_path );
...
void stalker_movement_manager_smart_cover::try_actualize_path() {
    ...
    shared_str target_loophole_id =
        smart_cover::transform_vertex( ( m_target.cover() == m_current.cover() )
                                           ? m_target.cover_loophole()->id()
                                           : "",
                                       false );
...
loophole const&
stalker_movement_manager_smart_cover::nearest_enterable_loophole() {
    VERIFY( !m_current.cover() );
    VERIFY( !m_current.cover_loophole() );
    VERIFY( m_target.cover() );
    VERIFY( m_target.cover_loophole() );
    ...
    VERIFY( m_path[ 0 ]._get() ==
            smart_cover::transform_vertex( "", true )._get() );
...
shared_str const& stalker_movement_manager_smart_cover::next_loophole_id() {
    VERIFY2(
        ...
        make_string(
            ...
            m_current.cover() ? m_current.cover_loophole()->id().c_str()
                              : "<no loophole>",
            m_target.cover() ? m_target.cover()->id().c_str() : "<world>",
            m_target.cover() ? m_target.cover_loophole()->id().c_str()
                             : "<no loophole>",
            m_path.size() ) );
...
void stalker_movement_manager_smart_cover::go_next_loophole() {
    ...
    if ( m_path[ 0 ]._get() ==
        ...
        VERIFY( !m_current.cover() );
#ifdef DEBUG
        Msg( "setting up cover (direct from target): %s (%s)",
             m_target.cover_id().c_str(), m_enter_cover_id.c_str() );
#endif // #ifdef DEBUG
...
loophole const& stalker_movement_manager_smart_cover::loophole(
    ...
    Loopholes::const_iterator i =
        std::find_if( loopholes.begin(), loopholes.end(),
                      loophole_id_predicate( loophole_id ) );

    VERIFY2( i != loopholes.end(),
             make_string( "loophole [%s] not present in smart_cover [%s]",
                          loophole_id.c_str(), cover.id().c_str() ) );
    return ( **i );
}
...
void stalker_movement_manager_smart_cover::lookout_min_time(
    float const& value ) {
...
void stalker_movement_manager_smart_cover::lookout_max_time(
    float const& value ) {
    animation_selector().planner().lookout_max_time( value );
}

void stalker_movement_manager_smart_cover::
    start_non_animated_loophole_change() {
