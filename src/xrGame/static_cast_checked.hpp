...
struct value {
    ...
    inline static void check( source_type* source ) {
        VERIFY( smart_cast< destination_type >( source ) ==
...
inline destination_type static_cast_checked( source_type const& source ) {
    typedef object_type_traits::remove_pointer< source_type >::type
        pointerless_type;
    typedef object_type_traits::remove_reference< pointerless_type >::type
        pure_source_type;

    debug::detail::static_cast_checked::helper< source_type const&,
                                                destination_type >::
        check< is_polymorphic< pure_source_type >::result >( source );
