...
public:
    ...
    public:
        ...
        IC void remove_item( LPCSTR text ) {
            ITEM_STORAGE_VEC_IT it = std::remove_if(
                m_data.begin(), m_data.end(), remove_text_pred( text ) );
        ...
        IC void process( T& process_pred ) {
            for ( ITEM_STORAGE_VEC_IT it = m_data.begin(); it != m_data.end();
                  ++it ) {
                process_pred( *it );
            }
        }
    ...
    struct SInfoItem {
        ...
        SInfoItem( LPCSTR str, u32 col, u32 i )
            : text( str ), color( col ), id( i ) {}
    };
...
#define SHIFT_POS_DEFAULT Fvector().set( 0.f, 2.f, 0.f )
    ...
    struct SLevelItem {
        ...
        enum { ePoint = u32( 0 ), eLine, eBox } ptype;
    ...
    public:
        ...
        void add_item( const Fvector& pos1,
                       const Fvector& pos2,
                       u32 color,
                       u32 id = u32( -1 ) );
        void add_item( const Fvector& pos,
                       float radius,
                       u32 color,
                       u32 id = u32( -1 ) );
        void draw_info();
