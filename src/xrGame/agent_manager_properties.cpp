...
CAgentManagerPropertyEvaluatorItem::_value_type
CAgentManagerPropertyEvaluatorItem::evaluate() {
...
CAgentManagerPropertyEvaluatorEnemy::_value_type
CAgentManagerPropertyEvaluatorEnemy::evaluate() {
    CAgentMemberManager::iterator I =
        m_object->member().combat_members().begin();
    CAgentMemberManager::iterator E = m_object->member().combat_members().end();
    for ( ; I != E; ++I ) {
        VERIFY( *I );
        if ( ( *I ) && ( *I )->object().memoryPtr() &&
             ( *I )->object().memory().enemy().selected() )
            return ( true );
    }
    return ( false );
}
...
CAgentManagerPropertyEvaluatorDanger::_value_type
CAgentManagerPropertyEvaluatorDanger::evaluate() {
    CAgentMemberManager::iterator I = m_object->member().members().begin();
    CAgentMemberManager::iterator E = m_object->member().members().end();
    for ( ; I != E; ++I ) {
        VERIFY( *I );
