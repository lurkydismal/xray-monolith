...
#define TEMPLATE_SPECIALIZATION                            \
    template < typename _Graph, typename _VertexEvaluator, \
               typename _vertex_id_type, typename _index_type >

#define CPathManagerTemplate                                         \
    CAbstractPathManager< _Graph, _VertexEvaluator, _vertex_id_type, \
                          _index_type >
...
TEMPLATE_SPECIALIZATION
IC void CPathManagerTemplate::build_path(
    const _vertex_id_type start_vertex_id,
    const _vertex_id_type dest_vertex_id ) {
    VERIFY( m_graph && m_evaluator &&
            m_graph->valid_vertex_id( start_vertex_id ) &&
            m_graph->valid_vertex_id( dest_vertex_id ) );

    if ( ( m_failed_start_vertex_id == start_vertex_id ) &&
    ...
    m_failed = !ai().graph_engine().search(
        *m_graph, start_vertex_id, dest_vertex_id, &m_path, *m_evaluator );
    after_search();
    m_current_index = _index_type( -1 );
    m_intermediate_index = _index_type( -1 );
    m_actuality = !failed();
...
TEMPLATE_SPECIALIZATION
IC const typename CPathManagerTemplate::PATH& CPathManagerTemplate::path()
    const {
    return ( m_path );
}
...
TEMPLATE_SPECIALIZATION
IC void CPathManagerTemplate::set_dest_vertex(
...
TEMPLATE_SPECIALIZATION
IC void CPathManagerTemplate::before_search(
    const _vertex_id_type start_vertex_id,
    const _vertex_id_type dest_vertex_id ) {}
...
TEMPLATE_SPECIALIZATION
IC bool CPathManagerTemplate::check_vertex(
