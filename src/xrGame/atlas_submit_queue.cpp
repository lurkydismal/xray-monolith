...
atlas_submit_queue::atlas_submit_queue(
    gamespy_profile::stats_submitter* stats_submitter )
...
void atlas_submit_queue::submit_all() {
    ...
    // * tmp_curr_prof	= tmp_lmngr->get_current_profile(); if (!tmp_curr_prof
    // || !tmp_curr_prof->online())
    //{
...
void atlas_submit_queue::submit_reward(
    ...
    // tmp_task.m_awards_count		= tmp_iter->second.m_count;	//+1 was
    // on quick_reward_with_award m_reward_tasks.push_back	(tmp_task);
    //
    // update();
}
...
void atlas_submit_queue::submit_best_results() {
    ...
    // submit_task	tmp_task;
    // tmp_task.m_data_type				=
    // submit_task::edt_best_scores_ptr; m_reward_tasks.push_back(tmp_task);
    // m_stats_submitter->quick_set_best_scores(&m_best_results_to_submit,
    // tmp_curr_prof); update();
}
...
void atlas_submit_queue::update() {
    ...
    // gamespy_gp::login_manager* tmp_lmngr		=
    // MainMenu()->GetLoginMngr(); VERIFY(tmp_lmngr); gamespy_gp::profile const
    // * tmp_curr_prof	= tmp_lmngr->get_current_profile();
    // VERIFY(tmp_curr_prof);
    // VERIFY2(tmp_curr_prof->online(), "ATLAS submit can be only in online
    // profile mode");
    //
    ...
    //	);
    // } else if (m_reward_tasks.front().m_data_type ==
    // submit_task::edt_best_scores_ptr)
    //{
    //	do_atlas_best_results(tmp_curr_prof, &m_best_results_to_submit);
    // } else if (m_reward_tasks.front().m_data_type ==
    // submit_task::edt_submit_all)
    //{
...
void atlas_submit_queue::do_atlas_reward(
    ...
    m_stats_submitter->reward_with_award( award_id, count, profile,
                                          m_atlas_submitted );
}
...
void atlas_submit_queue::do_atlas_submit_all(
    gamespy_gp::profile const* profile ) {
    VERIFY( m_stats_submitter );
    VERIFY( !m_atlas_in_process );

    // gamespy_profile::profile_store* tmp_store	=
    // MainMenu()->GetProfileStore(); VERIFY(tmp_store);
    //
    // m_atlas_in_process = true;
...
void __stdcall atlas_submit_queue::atlas_submitted( bool result,
