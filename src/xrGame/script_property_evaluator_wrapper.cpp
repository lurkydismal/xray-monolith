...
bool CScriptPropertyEvaluatorWrapper::evaluate() {
    try {
        ...
        ai().script_engine().script_log(
            ...
            "SCRIPT RUNTIME ERROR : evaluator [%s] returns value with not a %s "
            "type!",
            m_evaluator_name, exception.info()->name() );
#else
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "SCRIPT RUNTIME ERROR : evaluator "
                                         "returns value with not a %s type!",
                                         exception.info()->name() );
#endif
    }
#endif
        ...
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "SCRIPT RUNTIME ERROR : evaluator [%s] returns value with not a "
            "bool type!",
            m_evaluator_name );
    }
    return ( false );
