...
void CMapSpot::Load( CUIXml* xml, LPCSTR path ) {
    ...
    if ( xml->NavigateToNode( str ) ) {
        ...
        if ( !Heading() ) {
            m_border_static->SetWidth( m_border_static->GetWidth() *
                                       UI().get_current_kx() );
            if ( !UI().is_widescreen() ) {
                m_border_static->SetHeight( m_border_static->GetHeight() *
                                            UI().get_current_kx() );
            } else {
...
void CMiniMapSpot::Load( CUIXml* xml, LPCSTR path ) {
    ...
    if ( n ) {
        ...
        if ( strchr( texture, '\\' ) ) {
            ...
            float width =
                xml->ReadAttribFlt( buf, 0, "width", base_rect.width() );
            float height =
                xml->ReadAttribFlt( buf, 0, "height", base_rect.height() );
...
CUIStaticOrig* CComplexMapSpot::CreateStaticOrig( CUIXml& xml,
                                                  LPCSTR ui_path ) {
...
void CComplexMapSpot::Load( CUIXml* xml, LPCSTR path ) // complex_spot_template
{
    ...
    if ( !UI().is_widescreen() ) {
        m_top_icon->SetHeight( m_top_icon->GetHeight() *
                               UI().get_current_kx() );
    }
...
void CComplexMapSpot::Update() {
        ...
        if ( Level().GetGameTime() > m_timer_finish ) {
            ...
            if ( !m_infinity_time ) {
                ...
                m_timer->TextItemControl()->SetText(
                    GetTimeAsString( dt, InventoryUtilities::etpTimeToMinutes,
                                     ':', false )
                        .c_str() );
            }
        }
    ...
    if ( MapLocation()->SpotEnabled() ) {
        m_timer->Show( ( m_timer->GetWndSize().x > 5.0f ) &&
                       ( Level().GetGameTime() < m_timer_finish ) );
    }
}
