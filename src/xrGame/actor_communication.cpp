...
void CActor::UpdateAvailableDialogs( CPhraseDialogManager* partner ) {
    ...
    CInventoryOwner* pInvOwnerPartner =
        smart_cast< CInventoryOwner* >( partner );
    VERIFY( pInvOwnerPartner );

    for ( u32 i = 0;
          i < pInvOwnerPartner->CharacterInfo().ActorDialogs().size(); i++ )
        AddAvailableDialog(
            pInvOwnerPartner->CharacterInfo().ActorDialogs()[ i ], partner );
...
void CActor::RunTalkDialog( CInventoryOwner* talk_partner,
    ...
    if ( talk_partner->OfferTalk( this ) ) {
        ...
        smart_cast< CUIGameSP* >( CurrentGameUI() )
            ->StartTalk( talk_partner->bDisableBreakDialog );
    }
}
...
bool CActor::OnDialogSoundHandlerStart( CInventoryOwner* inv_owner,
                                        LPCSTR phrase ) {
