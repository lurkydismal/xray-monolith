...
void S2DVert::rotate_pt( const Fvector2& pivot,
                         const float cosA,
                         const float sinA,
                         const float kx ) {
    Fvector2 t = pt;
...
void C2DFrustum::CreateFromRect( const Frect& rect ) {
    m_rect.set( float( rect.x1 ), float( rect.y1 ), float( rect.x2 ),
                float( rect.y2 ) );
    m_force_clip = false;
    planes.resize( 4 );
...
sPoly2D* C2DFrustum::ClipPoly( sPoly2D& S, sPoly2D& D ) const {
    ...
    for ( u32 i = 0; i < planes.size(); i++ ) {
        ...
        // here we end up with complete polygon in 'dest' which is inside plane
        // #i
        if ( dest->size() < 3 )
            return 0;
    }
...
void ui_core::OnDeviceReset() {
    ...
    m_2DFrustum.CreateFromRect( Frect().set(
        0.0f, 0.0f, float( Device.dwWidth ), float( Device.dwHeight ) ) );
}

void ui_core::ClientToScreenScaled( Fvector2& dest,
                                    float left,
                                    float top ) const {
...
void ui_core::ClientToScreenScaled( Fvector2& src_and_dest ) const {
    if ( m_currentPointType != IUIRender::pttLIT )
        src_and_dest.set( ClientToScreenScaledX( src_and_dest.x ),
                          ClientToScreenScaledY( src_and_dest.y ) );
}
...
void ui_core::PushScissor( const Frect& r_tgt, bool overlapped ) {
    ...
    if ( !( result.x1 >= 0 && result.y1 >= 0 && result.x2 <= UI_BASE_WIDTH &&
            result.y2 <= UI_BASE_HEIGHT ) ) {
        Msg( "! r_tgt [%.3f][%.3f][%.3f][%.3f]", r_tgt.x1, r_tgt.y1, r_tgt.x2,
             r_tgt.y2 );
        Msg( "! result [%.3f][%.3f][%.3f][%.3f]", result.x1, result.y1,
             result.x2, result.y2 );
        VERIFY( result.x1 >= 0 && result.y1 >= 0 &&
                result.x2 <= UI_BASE_WIDTH && result.y2 <= UI_BASE_HEIGHT );
    }
...
void ui_core::pp_start() {
    ...
    m_pp_scale_.set(
        ...
        float( ::Render->getTarget()->get_height() ) /
            float( UI_BASE_HEIGHT ) );
    m_2DFrustumPP.CreateFromRect(
        Frect().set( 0.0f, 0.0f, float( ::Render->getTarget()->get_width() ),
                     float( ::Render->getTarget()->get_height() ) ) );
...
bool ui_core::is_widescreen() {
    return ( float( Device.dwWidth ) / float( Device.dwHeight ) ) >
...
u8 ui_core::screenmode() {
    if ( ( float( Device.dwWidth ) / float( Device.dwHeight ) ) > 1.8f )
        return u8( 2 );
    else if ( ( float( Device.dwWidth ) / float( Device.dwHeight ) ) >
              ( UI_BASE_WIDTH / UI_BASE_HEIGHT + 0.01f ) )
        return u8( 1 );
    return u8( 0 );
}
