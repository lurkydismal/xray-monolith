...
BOOL CProjector::net_Spawn( CSE_Abstract* DC ) {
    ...
    CSE_ALifeObjectProjector* slight =
        smart_cast< CSE_ALifeObjectProjector* >( e );
    R_ASSERT( slight );
    ...
    lanim = LALib.FindItem(
        pUserData->r_string( "projector_definition", "color_animator" ) );
    guid_bone = K->LL_BoneID(
        pUserData->r_string( "projector_definition", "guide_bone" ) );
    VERIFY( guid_bone != BI_NONE );
    bone_x.id = K->LL_BoneID(
        pUserData->r_string( "projector_definition", "rotation_bone_x" ) );
    VERIFY( bone_x.id != BI_NONE );
    bone_y.id = K->LL_BoneID(
        pUserData->r_string( "projector_definition", "rotation_bone_y" ) );
    VERIFY( bone_y.id != BI_NONE );
    Fcolor clr = pUserData->r_fcolor( "projector_definition", "color" );
    fBrightness = clr.intensity();
    light_render->set_color( clr );
    light_render->set_range(
        pUserData->r_float( "projector_definition", "range" ) );
    light_render->set_cone(
        deg2rad( pUserData->r_float( "projector_definition", "spot_angle" ) ) );
    light_render->set_texture(
        pUserData->r_string( "projector_definition", "spot_texture" ) );

    glow_render->set_texture(
        pUserData->r_string( "projector_definition", "glow_texture" ) );
    glow_render->set_color( clr );
    glow_render->set_radius(
        pUserData->r_float( "projector_definition", "glow_radius" ) );
#endif
    ...
    //////////////////////////////////////////////////////////////////////////
    CBoneInstance& b_x =
        smart_cast< IKinematics* >( Visual() )->LL_GetBoneInstance( bone_x.id );
    b_x.set_callback( bctCustom, BoneCallbackX, this );

    CBoneInstance& b_y =
        smart_cast< IKinematics* >( Visual() )->LL_GetBoneInstance( bone_y.id );
...
void CProjector::TurnOff() {
    ...
    smart_cast< IKinematics* >( Visual() )
        ->LL_SetBoneVisible( guid_bone, FALSE, TRUE );
}
#endif
...
void CProjector::UpdateCL() {
    ...
    if ( light_render->get_active() ) {
        ...
        CBoneInstance& BI = smart_cast< IKinematics* >( Visual() )
                                ->LL_GetBoneInstance( guid_bone );
        Fmatrix M;
    ...
    angle_lerp( _current.pitch, _target.pitch, bone_y.velocity,
                Device.fTimeDelta );
#endif
}
...
bool CProjector::bfAssignWatch( CScriptEntityAction* tpEntityAction ) {
    ...
    bone_y.velocity = ( fis_zero( time, EPS_L ) ? l_tWatchAction.vel_bone_y
                                                : delta_pitch / time );

    return false ==
           ( l_tWatchAction.m_bCompleted =
                 ( ( delta_yaw < EPS_L ) && ( delta_pitch < EPS_L ) ) );
}
...
bool CProjector::bfAssignObject( CScriptEntityAction* tpEntityAction ) {
    ...
    if ( l_tObjectAction.m_tGoalType == MonsterSpace::eObjectActionTurnOn )
        TurnOn();
    else if ( l_tObjectAction.m_tGoalType ==
              MonsterSpace::eObjectActionTurnOff )
        TurnOff();
...
void CProjector::CreateSkeleton( CSE_Abstract* P ) {
    ...
    ApplySpawnIniToPhysicShell( &P->spawn_ini(), m_pPhysicsShell,
                                fixed_bones.size() > 0 );
