...
// some conventional value used as evaluative param (there is no code
// restriction on mass)
//////////////////////////////////////////////////////////////////////////////////
static const float SQUARE_PARTICLE_EFFECT_DIST =
    PARTICLE_EFFECT_DIST * PARTICLE_EFFECT_DIST;
static const float SQUARE_SOUND_EFFECT_DIST =
    SOUND_EFFECT_DIST * SOUND_EFFECT_DIST;
...
public:
    CPHParticlesPlayCall( const dContactGeom& contact,
                          bool invert_n,
                          LPCSTR psn ) {
        ps_name = psn;
        c = contact;
...
class CPHLiquidParticlesPlayCall : public CPHParticlesPlayCall,
                                   public CPHReqComparerV {
    u32 remove_time;
    bool b_called;

public:
    CPHLiquidParticlesPlayCall( const dContactGeom& contact,
                                bool invert_n,
                                LPCSTR psn )
...
class CPHLiquidParticlesCondition : public CPHCondition,
                                    public CPHReqComparerV {
private:
    virtual bool compare( const CPHReqComparerV* v ) const {
        return v->compare( this );
...
public:
    CPHFindLiquidParticlesComparer( const Fvector& position )
        : m_position( position ) {}
...
//(const	CPHReqComparerV* v)					const
//{return v->compare(this);} 	virtual bool			compare
//(const	CPHOnesConditionSelfCmpTrue* v)		const	{return true;}
//
// };
...
static void play_object( dxGeomUserData* data,
...
IC bool play_liquid_particle_criteria( dxGeomUserData& data, float vel_cret ) {
    ...
    bool controller = !!data.ph_object &&
                      data.ph_object->CastType() == CPHObject::tpCharacter;
...
template < class Pars >
void play_particles( float vel_cret,
                     dxGeomUserData* data,
                     const dContactGeom* c,
    ...
    bool play_liquid =
        liquid && play_liquid_particle_criteria< Pars >( *data, vel_cret );
    ...
    if ( play_not_liquid )
        ...
        if ( !Level().ph_commander().has_call( &find, &find ) )
            Level().ph_commander().add_call(
                ...
                xr_new< CPHLiquidParticlesPlayCall >( *c, b_invert_normal,
                                                      ps_name ) );
    }
}
...
ContactCallbackFun* CharacterContactShotMark =
    &TContactShotMark< CharacterEffectPars >;
