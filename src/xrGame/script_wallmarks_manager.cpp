...
ScriptWallmarksArray::ScriptWallmarksArray( LPCSTR section ) {
    ...
    R_ASSERT2( pSettings->section_exist( section ),
               make_string( "[ScriptWallmarksManager] Can't find section '%s'",
                            m_section ) );
    LPCSTR wallmarks_string =
        READ_IF_EXISTS( pSettings, r_string, m_section, "wallmarks", nullptr );
    R_ASSERT2(
        wallmarks_string,
        make_string(
            "[ScriptWallmarksManager] Can't find 'wallmarks' in section '%s'",
            m_section ) );
...
void ScriptWallmarksManager::PlaceWallmark( Fvector dir,
                                            Fvector start_pos,
                                            float trace_dist,
                                            float wallmark_size,
                                            LPCSTR section,
                                            ...
                                            float ttl ) {
    PlaceWallmark( dir, start_pos, trace_dist, wallmark_size, section,
                   ignore_obj, ttl, true );
}
...
void ScriptWallmarksManager::PlaceWallmark( Fvector dir,
    ...
    BOOL reach_wall = Level().ObjectSpace.RayPick(
    ...
    if ( reach_wall ) {
        ...
        if ( !pMaterial->Flags.is( SGameMtl::flSuppressWallmarks ) ) {
            ...
            ::Render->add_StaticWallmark( FindSection( section ), end_point,
                                          wallmark_size, pTri, pVerts, ttl,
                                          true, rotation );
        }
    }
}

void ScriptWallmarksManager::PlaceSkeletonWallmark( CScriptGameObject* obj,
                                                    LPCSTR section,
                                                    Fvector start,
                                                    Fvector dir,
                                                    float size,
    ...
    Render->add_SkeletonWallmark(
        &object->renderable.xform, object->Visual()->dcast_PKinematics(),
        FindSection( section ), start, dir, size, ttl, true );
}
