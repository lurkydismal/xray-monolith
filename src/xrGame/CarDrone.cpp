...
void CCarDrone::Load( LPCSTR section ) {
    m_control_ele_force = READ_IF_EXISTS( pSettings, r_float, section,
                                          "control_ele_force", 0.0F );
    m_control_pit_force = READ_IF_EXISTS( pSettings, r_float, section,
                                          "control_pit_force", 0.0F );
    m_control_rol_force = READ_IF_EXISTS( pSettings, r_float, section,
                                          "control_rol_force", 0.0F );
    m_control_yaw_force = deg2rad( READ_IF_EXISTS(
        pSettings, r_float, section, "control_yaw_force", 0.0F ) );

    m_damping_velocity =
        READ_IF_EXISTS( pSettings, r_float, section, "damping_velocity", 0.0F );
    m_damping_rotation.set( READ_IF_EXISTS( pSettings, r_fvector3, section,
                                            "damping_rotation",
                                            m_damping_rotation ) );
...

    {
        ...
        R_ASSERT3( m_rotor_bones.size(), __FUNCTION__ " No m_rotor_bones",
                   car->cNameSect_str() );
    }
    {
        m_drive_bones.clear();
        ...
        R_ASSERT3( m_drive_bones.size(), __FUNCTION__ " No m_drive_bones",
                   car->cNameSect_str() );
    }
...
void CCarDrone::PhDataUpdate( float step ) {
    ...
    if ( car->b_engine_on && m_drive_bones.size() ) {
        ...
        {
            float force =
                mass * physics_world()->Gravity() / m_drive_bones.size();
        ...
        if ( m_damping_velocity > EPS_L ) {
            if ( m_control_ele == eControlEle_NA &&
                ...
                if ( cur_velocity.magnitude() > DAMPING_ESP_VEL ) {
                    float force = mass * __min( cur_velocity.magnitude(),
                                                m_damping_velocity );
                    new_velocity.invert( cur_velocity )
                        .normalize_safe()
                        .mul( force );
                    car->m_pPhysicsShell->applyForce(
        ...
        /* Rotation damping. */
        {
            ...
            bool apply = false;
            if ( m_damping_rotation.x > EPS_L &&
                 _abs( cur_angular.x ) > DAMPING_ESP_ANG ) {
                new_angular.x =
                    mass *
                    __min( _abs( cur_angular.x ), m_damping_rotation.x ) *
                    ( cur_angular.x < 0 ? 1 : -1 );
                apply = true;
            }
            if ( m_damping_rotation.y > EPS_L &&
                 _abs( cur_angular.y ) > DAMPING_ESP_ANG &&
                 m_control_yaw == eControlYaw_NA ) {
                new_angular.y =
                    mass *
                    __min( _abs( cur_angular.y ), m_damping_rotation.y ) *
                    ( cur_angular.y < 0 ? 1 : -1 );
                apply = true;
            }
            if ( m_damping_rotation.z > EPS_L &&
                 _abs( cur_angular.z ) > DAMPING_ESP_ANG ) {
                new_angular.z =
                    mass *
                    __min( _abs( cur_angular.z ), m_damping_rotation.z ) *
                    ( cur_angular.z < 0 ? 1 : -1 );
                apply = true;
            }
            if ( apply ) {
                car->m_pPhysicsShell->applyTorque( new_angular.x, new_angular.y,
                                                   new_angular.z );
            }
        }
        ...
        switch ( m_control_ele ) {
            case eControlEle_UP:
            case eControlEle_DW: {
                float force = mass * GetPowerEfficiency() *
                              m_control_ele_force / m_drive_bones.size();
        ...
        switch ( m_control_pit ) {
            ...
            case eControlPit_BS: {
                float force = mass * GetPowerEfficiency() *
                              m_control_pit_force / m_drive_bones.size();
        ...
        switch ( m_control_rol ) {
            ...
            case eControlRol_LS: {
                float force = mass * GetPowerEfficiency() *
                              m_control_rol_force / m_drive_bones.size();
        ...
        for ( auto& I : m_rotor_bones ) {
            if ( I.force != true ) {
                ...
                I.J->SetForceAndVelocity(
                    m_rotor_force,
                    m_rotor_speed * PI_MUL_2 * ( I.clock ? 1 : -1 ), 1 );
            }
        }
    } else {
        for ( auto& I : m_rotor_bones ) {
