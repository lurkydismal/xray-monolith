...
void player_state_avenger::OnPlayerKilled(
    ...
    struct need_revenge : xray::noncopyable {
        ...
        bool operator()( shared_str const& killer,
            ...
            if ( ( killer == m_killer_name ) &&
                ...
                if ( std::find( m_my_team_players.begin(),
    ...
    need_revenge tmp_predicate(
        team_players_store,
        sizeof( team_players_store ) / sizeof( shared_str ) );
    tmp_predicate.m_killer_name = tmp_victim->getName();

    player_spawn_times_t::const_iterator tmp_spawn_time =
        m_player_spawns.find( tmp_predicate.m_killer_name );
    ...
    if ( m_owner->get_kills_store().fetch_kills( tmp_predicate,
                                                 tmp_fake_buffer ) > 0 ) {
        ++m_aveng_count;
    }
}

void player_state_avenger::feel_my_team_players(
    game_PlayerState const* of_player,
...
void player_state_avenger::OnPlayerSpawned( game_PlayerState const* ps ) {
    ...
    if ( tmp_local_player && m_owner->is_enemies( tmp_local_player, ps ) ) {
        m_player_spawns.insert( std::make_pair( shared_str( ps->getName() ),
                                                Device.dwTimeGlobal ) );
    }
}
} // namespace award_system
