...
void CSoundMemoryManager::reload( LPCSTR section ) {
    m_max_sound_count =
        READ_IF_EXISTS( pSettings, r_s32, section, "DynamicSoundsCount", 1 );
    m_min_sound_threshold =
        READ_IF_EXISTS( pSettings, r_float, section, "sound_threshold", 0.05f );
    m_self_sound_factor =
        READ_IF_EXISTS( pSettings, r_float, section, "self_sound_factor", 0.f );
    m_sound_decrease_quant =
        READ_IF_EXISTS( pSettings, r_u32, section, "self_decrease_quant", 250 );
    m_decrease_factor = READ_IF_EXISTS( pSettings, r_float, section,
                                        "self_decrease_factor", .95f );

    LPCSTR sound_perceive_section = READ_IF_EXISTS(
        pSettings, r_string, section, "sound_perceive_section", section );
    m_weapon_factor = READ_IF_EXISTS( pSettings, r_float,
                                      sound_perceive_section, "weapon", 10.f );
    m_item_factor = READ_IF_EXISTS( pSettings, r_float, sound_perceive_section,
                                    "item", 1.f );
    m_npc_factor = READ_IF_EXISTS( pSettings, r_float, sound_perceive_section,
                                   "npc", 1.f );
    m_anomaly_factor = READ_IF_EXISTS( pSettings, r_float,
                                       sound_perceive_section, "anomaly", 1.f );
    m_world_factor = READ_IF_EXISTS( pSettings, r_float, sound_perceive_section,
                                     "world", 1.f );
}
...
IC void CSoundMemoryManager::update_sound_threshold() {
    ...
    m_sound_threshold = _max(
        m_self_sound_factor * m_sound_threshold *
...
IC u32
CSoundMemoryManager::priority( const MemorySpace::CSoundObject& sound ) const {
    u32 priority = u32( -1 );
    ...
    for ( ; I != E; ++I )
        if ( ( ( *I ).second < priority ) &&
             ( ( *I ).first & sound.m_sound_type ) == ( *I ).first )
            priority = ( *I ).second;
    return ( priority );
}

void CSoundMemoryManager::enable( const CObject* object, bool enable ) {
    xr_vector< CSoundObject >::iterator J =
        std::find( m_sounds->begin(), m_sounds->end(), object_id( object ) );
    if ( J == m_sounds->end() )
        return;
    ( *J ).m_enabled = enable;
}
...
void CSoundMemoryManager::feel_sound_new( CObject* object,
                                          int sound_type,
...
#ifndef MASTER_GOLD
    if ( object && smart_cast< CActor* >( object ) &&
    ...
    VERIFY( self );
#ifndef SILENCE
    Msg( "%s (%d) - sound type %x from %s at %d in (%.2f,%.2f,%.2f) with power "
         "%.2f",
         *self->cName(), Device.dwTimeGlobal, sound_type,
         object ? *object->cName() : "world", Device.dwTimeGlobal, position.x,
    ...
    if ( sound_power >= m_sound_threshold ) {
        ...
        if ( !m_stalker || !m_stalker->memory().enemy().selected() )
            ...
            if ( object ) {
                //				bool		is_shooting =
                //is_sound_type(sound_type,SOUND_TYPE_WEAPON_SHOOTING); 				bool
                //is_colliding =
                //is_sound_type(sound_type,SOUND_TYPE_WORLD_OBJECT_COLLIDING);
                //				bool		very_close =
                //m_stalker->Position().distance_to_sqr(object->Position()) <=
                //COMBAT_SOUND_PERCEIVE_RADIUS_SQR; 				if (is_shooting ||
                //is_colliding || very_close)
                add( object, sound_type, position, sound_power );
            }
        }
    }
...
void CSoundMemoryManager::add( const CSoundObject& sound_object,
                               bool check_for_existance ) {
    if ( check_for_existance ) {
        if ( m_sounds->end() !=
             std::find( m_sounds->begin(), m_sounds->end(),
                        object_id( sound_object.m_object ) ) )
            return;
    }
    ...
    if ( m_max_sound_count <= m_sounds->size() ) {
        xr_vector< CSoundObject >::iterator I =
            std::min_element( m_sounds->begin(), m_sounds->end(),
...
void CSoundMemoryManager::add( const CObject* object,
                               int sound_type,
                               const Fvector& position,
                               float sound_power ) {
    ...
    if ( object && object->H_Parent() &&
         ( object->H_Parent()->ID() == m_object->ID() ) )
        return;
#endif

#ifndef SAVE_NON_ALIVE_OBJECT_SOUNDS
    // we do not want to save sounds from the non-alive objects (?!)
    if ( object && !m_object->memory().enemy().selected() &&
         !smart_cast< const CEntityAlive* >( object ) )
        return;
#endif
    ...
    if ( object && object->H_Parent() &&
         ...
         ( me_entity_alive->tfGetRelationType(
               smart_cast< const CEntityAlive* >( object->H_Parent() ) ) ==
               ALife::eRelationTypeFriend ||
           me_entity_alive->tfGetRelationType(
               smart_cast< const CEntityAlive* >( object->H_Parent() ) ) ==
               ALife::eRelationTypeNeutral ) )
        return;

    const CEntityAlive* who_entity_alive =
        smart_cast< const CEntityAlive* >( object );
    // we do not want to save sounds from the teammates and neutrals
    if ( who_entity_alive && me_entity_alive &&
         ( me_entity_alive->tfGetRelationType( who_entity_alive ) ==
               ALife::eRelationTypeFriend ||
           me_entity_alive->tfGetRelationType( who_entity_alive ) ==
               ALife::eRelationTypeNeutral ) )
        return;
    ...
    if ( who_entity_alive &&
         !m_object->memory().enemy().is_useful( who_entity_alive ) )
        return;
    ...
    bool heavy_sound = ( ( sound_type & SOUND_TYPE_WEAPON_SHOOTING ) ==
                             SOUND_TYPE_WEAPON_SHOOTING ||
                         ( sound_type & SOUND_TYPE_WEAPON_BULLET_HIT ) ==
                             SOUND_TYPE_WEAPON_BULLET_HIT ||
                         ( sound_type & SOUND_TYPE_WEAPON_EMPTY_CLICKING ) ==
    ...
    if ( heavy_sound && sound_power >= 4.f ) {
        if ( CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( m_object ) ) {
            if ( who_entity_alive && who_entity_alive->g_Alive() &&
                 !m_object->memory().visual().visible_now(
                     who_entity_alive ) ) {
                stalker->movement().set_mental_state( eMentalStateDanger );
                CNotYetVisibleObject new_object;
                new_object.m_object =
                    smart_cast< const CGameObject* >( object );
                new_object.m_value = 1.0f;
                new_object.m_prev_time = Device.dwTimeGlobal - 1;
                new_object.m_update_time = Device.dwTimeGlobal;
                m_object->memory().visual().add_not_yet_visible_object(
                    new_object );
                m_object->memory().visual().add_visible_object(
                    object, Device.fTimeDelta );
    ...
    if ( m_sounds->end() == J ) {
        ...
        sound_object.fill(
            game_object, self, ESoundTypes( sound_type ), sound_power,
            !m_stalker
                ? squad_mask_type( -1 )
                : m_stalker->agent_manager().member().mask( m_stalker ) );
        if ( !game_object )
        ...
        ( *J ).fill( game_object, self, ESoundTypes( sound_type ), sound_power,
                     ( !m_stalker ? ( *J ).m_squad_mask.get()
                                  : ( ( *J ).m_squad_mask.get() |
                                      m_stalker->agent_manager().member().mask(
                                          m_stalker ) ) ) );
        if ( !game_object )
...
void CSoundMemoryManager::update() {
    ...
    if ( m_sounds )
        m_sounds->erase( std::remove_if( m_sounds->begin(), m_sounds->end(),
...
void CSoundMemoryManager::remove_links( CObject* object ) {
    VERIFY( m_sounds );
    SOUNDS::iterator I = std::find_if( m_sounds->begin(), m_sounds->end(),
                                       CSoundObjectPredicate( object ) );
    if ( I != m_sounds->end() )
        m_sounds->erase( I );
...
void CSoundMemoryManager::save( NET_Packet& packet ) const {
    ...
    for ( ; I != E; ++I ) {
        packet.w_u16( ( *I ).m_object ? ( *I ).m_object->ID()
                                      : ALife::_OBJECT_ID( -1 ) );
        // object params
       ...
       // self params
        ...
        packet.w_u32( ( Device.dwTimeGlobal > ( *I ).m_level_time )
                          ? ( Device.dwTimeGlobal - ( *I ).m_level_time )
                          : 0 );
#endif // USE_LAST_LEVEL_TIME
#ifdef USE_LEVEL_TIME
        packet.w_u32( ( Device.dwTimeGlobal > ( *I ).m_level_time )
                          ? ( Device.dwTimeGlobal - ( *I ).m_last_level_time )
                          : 0 );
#endif // USE_LAST_LEVEL_TIME
#ifdef USE_FIRST_LEVEL_TIME
        packet.w_u32( ( Device.dwTimeGlobal >= ( *I ).m_level_time )
                          ? ( Device.dwTimeGlobal - ( *I ).m_first_level_time )
                          : 0 );
...
void CSoundMemoryManager::load( IReader& packet ) {
    ...
    for ( int i = 0; i < count; ++i ) {
        ...
        if ( delayed_object.m_object_id != ALife::_OBJECT_ID( -1 ) )
            object.m_object = smart_cast< CGameObject* >(
                Level().Objects.net_Find( delayed_object.m_object_id ) );
        else
            object.m_object = 0;
        ...
        if ( object.m_object ||
             ( delayed_object.m_object_id == ALife::_OBJECT_ID( -1 ) ) ) {
            add( object, true );
            continue;
        }
        ...
        const CClientSpawnManager::CSpawnCallback* spawn_callback =
            Level().client_spawn_manager().callback( delayed_object.m_object_id,
                                                     m_object->ID() );
        if ( !spawn_callback || !spawn_callback->m_object_callback )
            if ( !g_dedicated_server )
                Level().client_spawn_manager().add( delayed_object.m_object_id,
                                                    m_object->ID(), callback );
#ifdef DEBUG
            else {
                if ( spawn_callback && spawn_callback->m_object_callback ) {
...
void CSoundMemoryManager::on_requested_spawn( CObject* object ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( m_object->g_Alive() ) {
            ( *I ).m_sound_object.m_object =
                smart_cast< CGameObject* >( object );
