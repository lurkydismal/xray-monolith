...
bool CLevel::Load_GameSpecific_After() {
    ...
    if ( FS.exist( fn_game, "$level$", "level.ps_static" ) ) {
        ...
        for ( IReader* OBJ = F->open_chunk_iterator( chunk ); OBJ;
            ...
            if ( ( g_pGamePersistent->m_game_params.m_e_game_type &
                   EGameIDs( gametype_usage ) ) ||
                 ( ver == 0 ) ) {
                auto pStaticParticles =
                    Particles::Details::Create( ref_name, FALSE, false );
    ...
    if ( !g_dedicated_server ) {
        ...
        if ( FS.exist( fn_game, "$level$", "level.snd_env" ) ) {
            ...
            // demonized: reset sound environment if the map doesn't have it, so
            // that the next map won't be using environment of the previous one
        ...
        if ( pSettings->section_exist( "sounds_random" ) ) {
            ...
            for ( CInifile::SectCIt I = S.Data.begin(); S.Data.end() != I;
                  ++I ) {
                Sounds_Random.push_back( ref_sound() );
                Sound->create( Sounds_Random.back(), *I->first, st_Effect,
                               sg_SourceType );
            }
