...
void read_level_resource( LPCSTR canonical_level_path,
                          LPCSTR file_name,
                          xr_vector< u8 >& bytes ) {
    const xr_string path =
        level_resource_path( canonical_level_path, file_name );
...
bool CLevel::Load_GameSpecific_Before() {
    ...
    if ( GamePersistent().GameType() == eGameIDSingle && !ai().get_alife() &&
         FS.exist( fn_game, "$level$", "level.ai" ) && !net_Hosts.empty() )
...
void CLevel::BeginGameSpecificPrepare( LPCSTR canonical_level_path ) {
    ...
    if ( !level_path.empty() && level_path.back() != '\\' &&
         level_path.back() != '/' )
        level_path += '\\';
    if ( m_game_specific_prepare ) {
        R_ASSERT3( !stricmp( m_game_specific_prepare->level_path.c_str(),
                             level_path.c_str() ),
                   "Level prepare target changed", level_path.c_str() );
        return;
    }

    level_game_specific_prepare* prepared =
        xr_new< level_game_specific_prepare >();
    ...
    prepared->environment_batch =
        executor.BeginBatch( executor.CurrentGeneration() );
    m_game_specific_prepare = prepared;
    auto submit = [ prepared, &executor ]( NativeLoadPriority priority,
                                           auto&& work ) {
        if ( prepared->batch.Valid() )
            executor.Submit( prepared->batch, priority,
                             std::forward< decltype( work ) >( work ) );
        else
            prepared->fallback_tasks.run(
                std::forward< decltype( work ) >( work ) );
    };
    auto submit_environment = [ prepared, &executor ]( auto&& work ) {
        if ( prepared->environment_batch.Valid() )
            executor.Submit( prepared->environment_batch,
                             NativeLoadPriority::Environment,
                             std::forward< decltype( work ) >( work ) );
        else
            prepared->fallback_environment_tasks.run(
                std::forward< decltype( work ) >( work ) );
    };

    submit_environment( [ prepared ]() {
        CEnvironment::PrepareLevelModifiers( prepared->level_path.c_str(),
                                             prepared->environment_modifiers );
    } );
    ...
    if ( pSettings->section_exist( "sounds_random" ) ) {
        ...
        for ( CInifile::SectCIt it = section.Data.begin();
              it != section.Data.end(); ++it )
            prepared->random_sounds.push_back( it->first );
    }

    submit( NativeLoadPriority::Environment, [ prepared ]() {
        const xr_string file_name = level_resource_path(
            prepared->level_path.c_str(), "level.ps_static" );
        ...
        Fmatrix transform;
        u32 ver = 0;
        for ( IReader* OBJ = F->open_chunk_iterator( chunk ); OBJ;
              OBJ = F->open_chunk_iterator( chunk, OBJ ) ) {
            if ( chunk == 0 ) {
            ...
            if ( ( prepared->game_type & u32( gametype_usage ) ) ||
                 ( ver == 0 ) ) {
    ...
    submit( NativeLoadPriority::Environment, [ this, prepared ]() {
        if ( !g_dedicated_server )
            m_level_sound_manager->Prepare( prepared->level_path.c_str(),
                                            prepared->sounds );
    } );
    ...
    submit( NativeLoadPriority::Environment, [ prepared ]() {
        if ( g_dedicated_server )
            return;
        const xr_string environment = level_resource_path(
            prepared->level_path.c_str(), "level.snd_env" );
        prepared->has_sound_environment = FS.exist( environment.c_str() );
        if ( prepared->has_sound_environment )
            read_level_resource( prepared->level_path.c_str(), "level.snd_env",
                                 prepared->sound_environment );
        const xr_string occlusion =
            level_resource_path( prepared->level_path.c_str(), "level.som" );
        prepared->has_sound_occlusion = FS.exist( occlusion.c_str() );
        if ( prepared->has_sound_occlusion )
            read_level_resource( prepared->level_path.c_str(), "level.som",
                                 prepared->sound_occlusion );
    } );
}
...
bool CLevel::Load_Prepared_Environment() {
    ...
    if ( !prepared.environment_committed ) {
        ...
        g_pGamePersistent->Environment().CommitLevelModifiers(
            prepared.environment_modifiers );
...
bool CLevel::Load_GameSpecific_After() {
    ...
    for ( const level_prepared_particle& particle : prepared.particles ) {
        auto instance =
            Particles::Details::Create( particle.name.c_str(), FALSE, false );
    ...
    if ( !g_dedicated_server ) {
        ...
        if ( prepared.has_sound_environment ) {
            IReader reader( prepared.sound_environment.data(),
                            prepared.sound_environment.size() );
            ::Sound->set_geometry_env( &reader );
        } else
            ::Sound->set_geometry_env( nullptr );
        if ( prepared.has_sound_occlusion ) {
            IReader reader( prepared.sound_occlusion.data(),
                            prepared.sound_occlusion.size() );
        ...
        for ( const shared_str& name : random_sounds ) {
            ...
            Sound->create( Sounds_Random.back(), name.c_str(), st_Effect,
                           sg_SourceType );
        }
        ...
        if ( pLevel->section_exist( "level_scripts" ) &&
             pLevel->line_exist( "level_scripts", "script" ) )
...
void CLevel::Load_GameSpecific_CFORM( CDB::TRI* tris, u32 count ) {
    ...
    int max_ID = 0;
    int max_static_ID = 0;
    for ( GameMtlIt I = GMLib.FirstMaterial(); GMLib.LastMaterial() != I;
          ++I, ++index ) {
    ...
    // Msg("* Material remapping ID: [Max:%d,
    // StaticMax:%d]",max_ID,max_static_ID);
    ...
    if ( static_mtl_count < 128 ) {
        ...
        for ( ; I != E; ++I ) {
            ID_INDEX_PAIRS::iterator i = std::find(
                translator.begin(), translator.end(), ( u16 )( *I ).material );
            if ( i != translator.end() ) {
                ( *I ).material = ( *i ).m_index;
                SGameMtl* mtl = GMLib.GetMaterialByIdx( ( *i ).m_index );
                ( *I ).suppress_shadows =
                    mtl->Flags.is( SGameMtl::flSuppressShadows );
                ( *I ).suppress_wm =
                    mtl->Flags.is( SGameMtl::flSuppressWallmarks );
                continue;
            }

            Debug.fatal( DEBUG_INFO, "Game material '%d' not found",
                         ( *I ).material );
        }
        return;
    }
    ...
    std::sort( translator.begin(), translator.end() );
    {
        ...
        CDB::TRI* E = tris + count;
        for ( ; I != E; ++I ) {
            ID_INDEX_PAIRS::iterator i = std::lower_bound(
                translator.begin(), translator.end(), ( u16 )( *I ).material );
            if ( ( i != translator.end() ) &&
                 ( ( *i ).m_id == ( *I ).material ) ) {
                ( *I ).material = ( *i ).m_index;
