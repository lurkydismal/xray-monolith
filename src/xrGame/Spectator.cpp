...
CSpectator::CSpectator() : CGameObject() {
    ...
    cameras[ eacFreeFly ] =
        xr_new< CSpectrCameraFirstEye >( m_fTimeDelta, this, 0 );
...
void CSpectator::UpdateCL() {
    ...
    if ( g_pGameLevel->CurrentViewEntity() == this ) {
        if ( eacFreeFly != cam_active ) {
            ...
            if ( P && ( P->team >= 0 ) &&
                 ( P->team <
                   ( int )Level().seniority_holder().teams().size() ) ) {
                const CTeamHierarchyHolder& T =
                    Level().seniority_holder().team( P->team );
    ...
    switch ( cmd ) {
        ...
        case kCAM_3:
            if ( cam_active == eacFreeFly && SelectNextPlayerToLook( false ) )
                cam_Set( eacFreeLook );
            break;
            // case kCAM_4:	cam_Set			(eacFreeFly);
            // m_pActorToLookAt = NULL;	break;
        case kWPN_FIRE: {
        ...
        case kWPN_ZOOM: {
            ...
            EActorCameras new_camera =
                EActorCameras( ( cam_active + 1 ) % eacMaxCam );
            ...
            if ( !PS->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) ) {
                ...
                while ( !found ) {
                    ...
                    if ( new_camera == ( eacMaxCam - 1 ) )
                        break;
                    new_camera =
                        EActorCameras( ( new_camera + 1 ) % eacMaxCam );
                }
                if ( !found )
                    break;
            };
...
void CSpectator::IR_OnKeyboardHold( int cmd ) {
    ...
    if ( ( cam_active == eacFreeFly ) || ( cam_active == eacFreeLook ) ) {
        ...
        if ( cam_active != eacFreeFly ||
             ( pMPGame->Is_Spectator_Camera_Allowed( eacFreeFly ) ||
               ( PS && PS->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) ) ) )
            XFORM().c.add( vmove );
    }
}
...
void CSpectator::cam_Update( CActor* A ) {
    if ( A ) {
        ...
        if ( Device.Paused() ) {
            Device.fTimeDelta =
                m_fTimeDelta; // fake, to update cam (problem with fov)
...
BOOL CSpectator::net_Spawn( CSE_Abstract* DC ) {
    ...
    cameras[ cam_active ]->Set( -E->o_Angle.y, -E->o_Angle.x,
                                tmp_roll ); // set's camera orientation
...
bool CSpectator::SelectNextPlayerToLook( bool const search_next ) {
    ...
    for ( ; it != ite; ++it ) {
        ...
        if ( !ps ||
             ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) /*|| (ps==PS)*/ )
            continue;
        ...
        if ( !A )
            continue;
        if ( m_last_player_name.size() &&
             ( m_last_player_name == ps->getName() ) ) {
    ...
    if ( PPCount > 0 ) {
        ...
        game_PlayerState* tmp_state =
            Game().GetPlayerByGameID( m_pActorToLookAt->ID() );
        if ( tmp_state )
...
void CSpectator::net_Relcase( CObject* O ) {
    ...
    if ( m_pActorToLookAt !=
         Level().CurrentEntity() ) // new spectator was spawned
    {
        m_pActorToLookAt = NULL;
        return;
    }
    ...
    if ( cam_active != eacFreeFly ) {
        SelectNextPlayerToLook( false );
        if ( m_pActorToLookAt ==
             O ) // selected to look at player that will be destroyed
        {
            m_pActorToLookAt = NULL;
        }
    }
