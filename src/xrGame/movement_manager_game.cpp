...
void CMovementManager::show_game_path_info() {
    ...
    Fvector temp = ai().game_graph()
                       .vertex( object().ai_location().game_vertex_id() )
                       ->level_point();
    Msg( "! CURRENT game point position : [%f][%f][%f]", VPUSH( temp ) );
    const GameGraph::CVertex* vertex =
        ai().game_graph().vertex( game_dest_vertex_id() );
    Msg( "! TARGET LEVEL : %s",
         *ai().game_graph().header().level( vertex->level_id() ).name() );
    temp = vertex->level_point();
    Msg( "! TARGET  game point position : [%f][%f][%f]", VPUSH( temp ) );
    const u8* target_vertex_type =
        ai().game_graph().vertex( game_dest_vertex_id() )->vertex_type();
    Msg( "! Target point mask [%d][%d][%d][%d]", target_vertex_type[ 0 ],
         target_vertex_type[ 1 ], target_vertex_type[ 2 ],
    ...
    for ( ; I != E; ++I )
        Msg( "!   [%d][%d][%d][%d]", ( *I ).tMask[ 0 ], ( *I ).tMask[ 1 ],
             ( *I ).tMask[ 2 ], ( *I ).tMask[ 3 ] );
}
...
void CMovementManager::process_game_path() {
    ...
    switch ( m_path_state ) {
        case ePathStateSelectGameVertex: {
            ...
            game_selector().select_location(
                object().ai_location().game_vertex_id(),
                game_path().m_dest_vertex_id );
            if ( ( current == game_path().m_dest_vertex_id ) &&
                 game_selector().used() ) {
        ...
        case ePathStateBuildGamePath: {
            game_path().build_path( object().ai_location().game_vertex_id(),
                                    game_dest_vertex_id() );

            if ( game_path().failed() ) {
                ::luabind::functor< bool > funct;
                if ( ai().script_engine().functor(
                         "_G.CAI_Stalker__PathBuildFailCallback", funct ) ) {
        ...
        case ePathStateContinueGamePath: {
            ...
            if ( ai().game_graph()
                     ...
                     ->level_id() !=
                 ai().game_graph()
                     .vertex( game_path().intermediate_vertex_id() )
                     ->level_id() ) {
        ...
        case ePathStateBuildLevelPath: {
            ...
            if ( !ai().level_graph().valid_vertex_id( dest_level_vertex_id ) ) {
                Msg( "! "
                     "[CMovementManager::process_game_path::"
                     "ePathStateBuildLevelPath] cannot build path, invalid "
                     "dest_level_vertex_id %lu, object %s",
                     dest_level_vertex_id, object().cName().c_str() );
                break;
            }
            ...
            if ( !ai().level_graph().valid_vertex_id( dest_level_vertex_id ) ) {
                Msg( "! "
                     "[CMovementManager::process_game_path::"
                     "ePathStateBuildLevelPath] cannot build path after "
                     "accessible_nearest check, invalid dest_level_vertex_id "
                     "%lu, object %s",
                     dest_level_vertex_id, object().cName().c_str() );
                break;
            }
            ...
            level_path_builder().setup(
                object().ai_location().level_vertex_id(), dest_level_vertex_id,
                true, &temp );
        ...
        case ePathStateBuildDetailPath: {
            ...
            detail().set_start_direction(
                Fvector().setHP( -m_body.current.yaw, 0 ) );
            detail().set_dest_position( ai().level_graph().vertex_position(
                level_path().intermediate_vertex_id() ) );

            detail_path_builder().setup( level_path().path(),
        ...
        case ePathStatePathVerification: {
            if ( !game_selector().actual(
                ...
                m_path_state = ePathStateBuildLevelPath;
            else if ( detail().completed( object().Position(),
