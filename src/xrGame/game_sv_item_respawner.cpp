...
bool item_respawn_manager::search_by_id_predicate::operator()(
    spawn_item const& left,
    u16 right ) const {
...
void item_respawn_manager::load_respawn_items(shared_str const section)
{
        ...
        CInifile::Sect resp_sect	=
level_ini_file->r_section(section.c_str());
        ...
        for (sect_iter i = resp_sect.Data.begin(); i != ie; ++i)
        {
                ...
                m_respawns.insert(std::make_pair(i->first, spawn_item(temp_int *
1000)));
        }
}
...
m_respawns.find(shared_str(item->name_replace())); if (temp_iter !=
m_respawns.end())
        {
                if (!temp_iter->second.item_object)
                {
                        NET_Packet clone_store;
                        CSE_Abstract* temp_entity =
F_entity_Create(item->s_name.c_str()); item->Spawn_Write(clone_store, false);
...
CSE_Abstract* item_respawn_manager::make_respawn_entity(
    shared_str const& section_name,
    u8 addons,
    u16 count_of_ammo ) {
    R_ASSERT( m_server );
    ...
    CSE_ALifeItemWeapon* pWeapon =
        smart_cast< CSE_ALifeItemWeapon* >( temp_entity );
...
bool item_respawn_manager::parse_string( char const* str,
                                         u32 str_size,
                                         section_item& result ) {
...
u32 item_respawn_manager::load_section_items( CInifile& ini,
    ...
    while ( ini.line_exist( section_name, item_name ) ) {
        ...
        if ( !parse_string( item_value, xr_strlen( item_value ),
                            temp_sect_item ) ) {
            Msg( "! WARNING: failed to parse item [%s] in section [%s]",
...
item_respawn_manager::load_respawn_section( shared_str const& section_name ) {
    ...
    std::pair< respawn_section_iter, bool > insert_res =
        m_respawn_sections_cache.insert(
            std::make_pair( section_name, tmp_sect_items ) );
...
void item_respawn_manager::add_new_rpoint( shared_str profile_sect,
                                           RPoint const& point ) {
    respawn_section_iter tmp_resp_sect =
        m_respawn_sections_cache.find( profile_sect );

    if ( tmp_resp_sect == m_respawn_sections_cache.end() ) {
        tmp_resp_sect = load_respawn_section( profile_sect );
        if ( tmp_resp_sect == m_respawn_sections_cache.end() ) {
#ifndef MASTER_GOLD
            Msg( "! ERROR: not found section %s in respawn_items.ltx",
                 profile_sect.c_str() );
#endif // #ifndef MASTER_GOLD
            return;
        }
    ...
    for ( section_items_iter iter_rsect = tmp_resp_sect->second->begin();
        ...
        new_item.item_object =
            make_respawn_entity( iter_rsect->section_name, iter_rsect->addons,
...
#ifndef MASTER_GOLD
            Msg( "! ERROR: failed to create entity [%s] with addons [%d]",
                 iter_rsect->section_name, iter_rsect->addons );
#endif // #ifndef MASTER_GOLD
        }
    }
}
...
void item_respawn_manager::check_to_delete( u16 item_id ) {
    respawn_iter temp_iter =
        std::find_if( m_respawns.begin(), m_respawns.end(),
                      [ item_id ]( const spawn_item& left ) {
                          return search_by_id_predicate{}( left, item_id );
                      } );
...
void item_respawn_manager::clear_level_items() {
    ...
    for ( xr_set< u16 >::iterator i = level_items_respawn.begin(); i != ie;
          ++i ) {
...
void item_respawn_manager::respawn_level_items() {
    ...
    if ( FS.exist( fn_spawn, "$level$", "level_rs.spawn" ) ) {
        ...
        NET_Packet P;
        u32 S_id;
        for ( IReader* S = SP->open_chunk_iterator( S_id ); S;
              S = SP->open_chunk_iterator( S_id, S ) ) {
            P.B.count = S->length();
