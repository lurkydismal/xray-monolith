...
void game_cl_TeamDeathmatch::net_import_state( NET_Packet& P ) {
    bool teamsEqual =
        ( !teams.empty() ) ? ( teams[ 0 ].score == teams[ 1 ].score ) : false;
    u16 old_phase = Phase();
...
void game_cl_TeamDeathmatch::TranslateGameMessage( u32 msg, NET_Packet& P ) {
    CStringTable st;
    string512 Text;
    //	LPSTR	Color_Teams[3]	= {"%c[255,255,255,255]", "%c[255,64,255,64]",
    //"%c[255,64,64,255]"};
    ...
    switch ( msg ) {
        case GAME_EVENT_PLAYER_JOIN_TEAM: // tdm
        {
            ...
            xr_sprintf( Text, "%s%s %s%s %s%s",
                        "", // no color
                        PlayerName, Color_Main, *st.translate( "mp_joined" ),
...
void game_cl_TeamDeathmatch::GetMapEntities(
...
void game_cl_TeamDeathmatch::SetCurrentBuyMenu() {
    ...
    if ( !pCurBuyMenu ) {
        ...
        if ( team_index == 1 ) {
            LoadTeamDefaultPresetItems( GetTeamMenu( team_index ), pCurBuyMenu,
                                        &PresetItemsTeam1 );
...
void game_cl_TeamDeathmatch::shedule_Update( u32 dt ) {
    ...
    if ( m_game_ui->m_pUITeamSelectWnd &&
         m_game_ui->m_pUITeamSelectWnd->IsShown() && !CanCallTeamSelectMenu() )
        m_game_ui->m_pUITeamSelectWnd->HideDialog();
    //.		m_game_ui->StartStopMenu(m_game_ui->m_pUITeamSelectWnd,true);
    //---------------------------------------------------------
    ...
    switch ( m_phase ) {
        case GAME_PHASE_TEAM1_SCORES: {
            xr_sprintf( msg, st.translate( "mp_team_wins" ).c_str(),
                        CTeamInfo::GetTeam_name( 1 ) );
        ...
        case GAME_PHASE_INPROGRESS: {
            if ( local_player && !local_player->IsSkip() ) {
                ...
                if ( local_player->testFlag( GAME_PLAYER_FLAG_ONBASE ) &&
                    ...
                    m_bBuyEnabled = true;
                } else if ( !local_player->testFlag(
                                GAME_PLAYER_FLAG_ONBASE ) &&
                            !local_player->testFlag(
                                GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ) {
                    m_bBuyEnabled = false;
                } else if ( local_player->testFlag(
    ...
    if ( local_player && ( local_player == lookat_player ) &&
        ...
        for ( ; it != players.end(); ++it ) {
            ...
            if ( /*m_bFriendlyNames &&*/ m_bShowPlayersNames ) {
                ...
                pActor->RenderText( ps->getName(), IPos, &dup,
                                    PLAYER_NAME_COLOR );
            }
...
BOOL game_cl_TeamDeathmatch::CanCallTeamSelectMenu() {
    ...
    m_game_ui->m_pUITeamSelectWnd->SetCurTeam(
        ModifyTeam( local_player->team ) );
    return TRUE;
};
    ...
    if ( local_player ) {
        ...
        for ( ; it != players.end(); ++it ) {
            ...
            if ( !pObject || !smart_cast< CActor* >( pObject ) )
                continue;
            if ( IsEnemy( ps ) ) {
                if ( Level().MapManager().HasMapLocation( FRIEND_LOCATION,
                                                          id ) ) {
                    Level().MapManager().RemoveMapLocation( FRIEND_LOCATION,
                                                            id );
                };
                continue;
            };
...
bool game_cl_TeamDeathmatch::IsPlayerInTeam( game_PlayerState* ps,
                                             ETeam team ) {
