...
void CInfoPortion::load_shared	(LPCSTR)
{
        ...
        if(item_data==NULL)
        {
                Msg("! attempt to use non-existent INFOPORTION [%s]",
m_InfoId.c_str()); return;
        }
        ...
        //loading from XML
        XML_NODE* pNode			=
pXML->NavigateToNode(id_to_index::tag_name, item_data->pos_in_file); THROW3
(pNode, "info_portion id=", *item_data->id);
        ...
        {
                shared_str info_id		= pXML->Read(pNode, "disable",
i,""); info_data()->m_DisableInfo.push_back(info_id);
        }
        ...
        for(i=0; i<articles_num; ++i)
        {
                LPCSTR article_str_id = pXML->Read(pNode, "article_disable", i,
NULL); THROW(article_str_id);
...
void   CInfoPortion::InitXmlIdToIndex()
{
        ...
        if(!id_to_index::file_str)
                id_to_index::file_str = pSettings->r_string("info_portions",
"files");
}
*/
void _destroy_item_data_vector_cont( T_VECTOR* vec ) {
    ...
    for ( ; it != it_e; ++it ) {
        xr_vector< CUIXml* >::iterator it_f =
            std::find( _tmp.begin(), _tmp.end(), ( *it )._xml );
        if ( it_f == _tmp.end() )
