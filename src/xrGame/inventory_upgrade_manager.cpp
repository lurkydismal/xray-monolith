...
Root* Manager::add_root( shared_str const& root_id ) {
    if ( get_root( root_id ) ) {
        VERIFY2(
            0, make_string(
                   "Try add the existent upgrade_root for inventory item <%s>!",
                   root_id.c_str() ) );
    }
...
Upgrade* Manager::add_upgrade( shared_str const& upgrade_id,
                               Group& parent_group ) {
    if ( get_upgrade( upgrade_id ) ) {
        VERIFY2( 0,
                 make_string(
                     "Try add the existent upgrade (%s), in group <%s>. Such "
                     "upgrade is in group <%s> already!",
...
Group* Manager::add_group( shared_str const& group_id,
                           UpgradeBase& parent_upgrade ) {
...
Property* Manager::add_property( shared_str const& property_id ) {
    if ( get_property( property_id ) ) {
        VERIFY2( 0, make_string( "Try add the existent upgrade property <%s>!",
                                 property_id.c_str() ) );
    }
...
bool Manager::item_upgrades_exist( shared_str const& item_id ) {
    VERIFY2(
        pSettings->section_exist( item_id ),
        make_string( "Inventory item [%s] does not exist!", item_id.c_str() ) );
    if ( !pSettings->line_exist( item_id, "upgrades" ) ||
         !pSettings->r_string( item_id, "upgrades" ) ) {
        return false;
    }
    if ( !pSettings->line_exist( item_id, "upgrade_scheme" ) ||
         !pSettings->r_string( item_id, "upgrade_scheme" ) ) {
        return false;
    }
...
void Manager::load_all_inventory() {
    ...
    VERIFY2( pSettings->section_exist( items_section ), make_string( "Section
[%s] does not exist !", items_section ) ); VERIFY2( pSettings->line_count(
items_section ),    make_string( "Section [%s] is empty !",       items_section
) );
    ...
    CInifile::Sect&		inv_section = pSettings->r_section(
items_section ); CInifile::SectIt_	ib = inv_section.Data.begin();
...
//		if ( !item_upgrades_exist( root_id ) ) continue;
    ...
    */
    ...
    // Alundaio: No longer the need to define upgradable sections in
    // [upgraded_inventory]
    ...
    for ( ; i != e; ++i ) {
        if ( !pSettings->line_exist( ( *i ).Name, "upgrades" ) ||
             !pSettings->r_string( ( *i ).Name, "upgrades" ) )
            continue;

        if ( !pSettings->line_exist( ( *i ).Name, "upgrade_scheme" ) ||
             !pSettings->r_string( ( *i ).Name, "upgrade_scheme" ) )
            continue;
...
void Manager::test_all_upgrades( CInventoryItem& item ) {
    ...
    VERIFY2( root_p,
             make_string( "Upgrades for item <%s> (id = %d) does not exist!",
                          item.m_section_id.c_str(), item.object_id() ) );
    root_p->test_all_upgrades( item );

    if ( g_upgrades_log == 1 ) {
        Msg( "- Checking all upgrades of item <%s> (id = %d) is successful.",
             root_p->id_str(), item.object_id() );
    }
}

#endif // DEBUG

Upgrade* Manager::upgrade_verify( shared_str const& item_section,
                                  shared_str const& upgrade_id ) {
    Root* root_p = get_root( item_section );
    VERIFY2( root_p, make_string( "Upgrades of item <%s> don`t exist!",
                                  item_section.c_str() ) );

    Upgrade* upgrade_p = get_upgrade( upgrade_id );
    VERIFY2( upgrade_p,
             make_string( "Upgrade <%s> in item <%s> does not exist!",
                          upgrade_id.c_str(), item_section.c_str() ) );

    VERIFY2( root_p->contain_upgrade( upgrade_id ),
             make_string( "Inventory item <%s> not contain upgrade <%s> !",
                          item_section.c_str(), upgrade_id.c_str() ) );

    return ( upgrade_p );
}

bool Manager::make_known_upgrade( CInventoryItem& item,
                                  shared_str const& upgrade_id ) {
...
bool Manager::make_known_upgrade( const shared_str& upgrade_id ) {
    Upgrade* upgrade_p = get_upgrade( upgrade_id );
    VERIFY2( upgrade_p, make_string( "Upgrade <%s> does not exist!",
                                     upgrade_id.c_str() ) );
    return ( upgrade_p->make_known() );
}

bool Manager::is_known_upgrade( CInventoryItem& item,
                                shared_str const& upgrade_id ) {
...
/*
bool Manager::is_disabled_upgrade( CInventoryItem& item, shared_str const&
upgrade_id )
{
...
bool Manager::upgrade_install( CInventoryItem& item,
                               shared_str const& upgrade_id,
                               bool loading ) {
    ...
    if ( res == result_ok ) {
        ...
        if ( item.install_upgrade( upgrade->section() ) ) {
            ...
            if ( g_upgrades_log == 1 ) {
                Msg( "# Upgrade <%s> of inventory item [%s] (id = %d) is "
                     "installed.",
                     upgrade_id.c_str(), item.m_section_id.c_str(),
                     item.object_id() );
            }
            return true;
        } else {
            FATAL( make_string( "! Upgrade <%s> of item [%s] (id = %d) is "
                                "EMPTY or FAILED !",
                                upgrade_id.c_str(), item.m_section_id.c_str(),
                                item.object_id() )
                       .c_str() );
    ...
    if ( g_upgrades_log == 1 ) {
        Msg( "- Upgrade <%s> of inventory item [%s] (id = %d) can`t be "
             "installed. Error = %d",
...
void Manager::init_install( CInventoryItem& item ) {
    ...
    if ( pSettings->line_exist( item.m_section_id, "installed_upgrades" ) ) {
        ...
        LPCSTR installed_upgrades_str =
            pSettings->r_string( item.m_section_id, "installed_upgrades" );
        if ( installed_upgrades_str ) {
            u32 const buffer_size =
                ( xr_strlen( installed_upgrades_str ) + 1 ) * sizeof( char );
            PSTR temp = ( PSTR )_alloca( buffer_size );

            for ( int n = _GetItemCount( installed_upgrades_str ), i = 0; i < n;
                  ++i ) {
                upgrade_install(
                    item,
                    _GetItem( installed_upgrades_str, i, temp, buffer_size ),
                    true );
            }
        }
    } // if exist
...
LPCSTR Manager::get_upgrade_by_index( CInventoryItem& item,
                                      Ivector2 const& index ) {
    ...
    VERIFY2( upgrade,
             make_string( "! Upgrade with index <%d,%d> in inventory item [%s] "
                          "does not exist!",
...
bool Manager::compute_range( LPCSTR parameter, float& low, float& high ) {
    ...
    Roots_type::iterator ie = m_roots.end();
    for ( ; ib != ie; ++ib ) {
        compute_range_section( ( ( *ib ).second )->id_str(), parameter, low,
                               high );
    }
    ...
    for ( ; uib != uie; ++uib ) {
        compute_range_section( ( ( *uib ).second )->section(), parameter, low,
                               high );
    }
...
void Manager::compute_range_section( LPCSTR section,
                                     LPCSTR parameter,
                                     float& low,
                                     float& high ) {
    if ( !pSettings->line_exist( section, parameter ) ||
         !*pSettings->r_string( section, parameter ) ) {
        return;
    }
...
void Manager::highlight_hierarchy( CInventoryItem& item,
                                   shared_str const& upgrade_id ) {
    Root* root_p = get_root( item.m_section_id );
    if ( root_p ) {
