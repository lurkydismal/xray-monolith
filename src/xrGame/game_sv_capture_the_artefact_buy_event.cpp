...
void game_sv_CaptureTheArtefact::OnPlayerBuyFinished( ClientID id_who,
                                                      NET_Packet& P ) {
...
void game_sv_CaptureTheArtefact::OnPlayerCloseBuyMenu(
    xrClientData const* pclient ) {
    R_ASSERT2( pclient, "bad client closed buy menu" );
    TBuyMenuPlayerStates::iterator temp_iter =
        m_buyMenuPlayerStates.find( pclient );
    if ( temp_iter != m_buyMenuPlayerStates.end() ) {
        game_PlayerState* ps = temp_iter->first->ps;
        if ( !ps )
            return;
        VERIFY2( !ps->testFlag( GAME_PLAYER_FLAG_SPECTATOR ),
                 "spectator can't send OnPlayerCloseBuyMenu messages" );
...
void game_sv_CaptureTheArtefact::OnPlayerOpenBuyMenu(
    xrClientData const* pclient ) {
    R_ASSERT( pclient->ps );
    if ( !pclient->ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) )
        return;
    // R_ASSERT2(pclient->ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD), "only
    // dead players can open buy menu in async mode");
#ifdef DEBUG
    Msg( "--- Player [%s] opens buy menu", pclient->ps->getName() );
#endif // #ifdef DEBUG
    m_buyMenuPlayerStates.insert(
        std::make_pair( pclient, buyMenuPlayerOpensBuyMenu ) );
}

bool game_sv_CaptureTheArtefact::CheckIfPlayerInBuyMenu(
    xrClientData const* pclient ) {
    TBuyMenuPlayerStates::const_iterator temp_iter =
        m_buyMenuPlayerStates.find( pclient );
    if ( temp_iter != m_buyMenuPlayerStates.end() ) {
...
void game_sv_CaptureTheArtefact::SetReadyToSpawnPlayer(
    xrClientData const* pclient ) {
    R_ASSERT( pclient->ps );
    // if (!pclient->ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD))
    //	return;
...
#endif // #ifdef DEBUG
    m_buyMenuPlayerStates.insert(
        std::make_pair( pclient, buyMenuPlayerReadyToSpawn ) );
}
