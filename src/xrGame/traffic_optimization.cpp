...
void init_ppmd_trained_stream( ppmd_trained_stream*& dest ) {
    ...
    R_ASSERT2( FS.exist( file_name ),
               "can't find configs\\mp\\ppmd_updates.mdl" );
...
void init_lzo( u8*& dest_wm,
               u8*& wm_buffer,
               lzo_dictionary_buffer& dest_dict ) {
    lzo_initialize();
    ...
    R_ASSERT2( FS.exist( file_name ),
               "can't find configs\\mp\\lzo_updates.dic" );
    IReader* reader = FS.r_open( file_name );
    u32 buffer_size = reader->length();
    u8* buffer = ( u8* )xr_malloc( buffer_size );
    reader->r( buffer, buffer_size );
