...
public:
    ...
    xr_map< u16, prepared_client_spawn_resource >
        prepared_client_spawn_resources;
    void RegisterPreparedClientSpawnResource( u16 id,
                                              u16 parent_id,
...
public:
    ...
    virtual void Send( NET_Packet& P,
                       u32 dwFlags = DPNSEND_GUARANTEED,
                       u32 dwTimeout = 0 );
    void g_cl_Spawn( LPCSTR name,
                     u8 rp,
                     u16 flags,
                     Fvector pos );     // only ask server
    ...
    static void
    LuaGCDebug(); // GC that will called from Device via Device.LuaGCDebug
    ...
    // возвращает время для энвайронмента в милисекундах относительно начала
    // игры
    ALife::_TIME_ID GetEnvironmentGameTime();
    // игровое время в отформатированном виде
    void GetGameDateTime( u32& year,
                          u32& month,
                          u32& day,
                          u32& hours,
                          u32& mins,
    ...
    virtual void SetEnvironmentGameTimeFactor( u64 const& GameTime,
