...
void SBinocVisibleObj::create_default( u32 color ) {
    ...
    m_rb.SetTextureRect(
        Frect().set( 32 - RECT_SIZE, 32 - RECT_SIZE, 32, 32 ) );
...
void SBinocVisibleObj::Update() {
    ...
    if ( m_flags.is( flTargetLocked ) ) {
        ...
        cur_rect.lt.x +=
            ( mn.x - cur_rect.lt.x ) * m_upd_speed * Device.fTimeDelta;
        cur_rect.lt.y +=
            ( mn.y - cur_rect.lt.y ) * m_upd_speed * Device.fTimeDelta;
        cur_rect.rb.x +=
            ( mx.x - cur_rect.rb.x ) * m_upd_speed * Device.fTimeDelta;
        cur_rect.rb.y +=
            ( mx.y - cur_rect.rb.y ) * m_upd_speed * Device.fTimeDelta;
        if ( mn.similar( cur_rect.lt, 2.f ) &&
             mx.similar( cur_rect.rb, 2.f ) ) {
            // target locked
            ...
            if ( pActor ) {
                ...
                CInventoryOwner* our_inv_owner =
                    smart_cast< CInventoryOwner* >( pActor );
                CInventoryOwner* others_inv_owner =
                    smart_cast< CInventoryOwner* >( m_object );
                CBaseMonster* monster = smart_cast< CBaseMonster* >( m_object );

                if ( our_inv_owner && others_inv_owner && !monster ) {
                    if ( IsGameTypeSingle() ) {
                        switch ( RELATION_REGISTRY().GetRelationType(
                            others_inv_owner, our_inv_owner ) ) {
                        ...
                        CEntityAlive* our_ealive =
                            smart_cast< CEntityAlive* >( pActor );
                        CEntityAlive* others_ealive =
                            smart_cast< CEntityAlive* >( m_object );
...
void CBinocularsVision::Update() {
    ...
    if ( !pActor )
        return;
    //-----------------------------------------------------
    const CVisualMemoryManager::VISIBLES& vVisibles =
        pActor->memory().visual().objects();
    ...
    for ( ; v_it != vVisibles.end(); ++v_it ) {
        ...
        if ( !pActor->memory().visual().visible_right_now(
        ...
        Fvector opos = Fvector( object_->Position() );
        if ( 0 > opos.sub( Actor()->Position() )
                     .dotproduct( Device.vCameraDirection ) )
            continue;

        FindVisObjByObject f( object_ );
        VIS_OBJECTS_IT found;
        found =
            std::find_if( m_active_objects.begin(), m_active_objects.end(), f );
        ...
        if ( found != m_active_objects.end() ) {
            ...
            m_sounds.PlaySound( "found_snd", Fvector().set( 0, 0, 0 ), NULL,
                                true );
        }
    }
    ...
    while (m_active_objects.size() &&
    m_active_objects.back()->m_flags.test(flVisObjNotValid))
    {
    ...
    */
    ...
    m_active_objects.erase(
        remove_if( m_active_objects.begin(), m_active_objects.end(),
    ...
    for ( ; it != m_active_objects.end(); ++it ) {
        ...
        if ( bLocked != visObj->m_flags.test( flTargetLocked ) )
            m_sounds.PlaySound( "catch_snd", Fvector().set( 0, 0, 0 ), NULL,
                                true );
    }
}
...
void CBinocularsVision::Load( const shared_str& section ) {
    ...
    m_sounds.LoadSound( section.c_str(), "found_snd", "found_snd", false,
                        SOUND_TYPE_NO_SOUND );
    m_sounds.LoadSound( section.c_str(), "catch_snd", "catch_snd", false,
                        SOUND_TYPE_NO_SOUND );
}

void CBinocularsVision::remove_links( CObject* object ) {
    VIS_OBJECTS::iterator I =
        std::find_if( m_active_objects.begin(), m_active_objects.end(),
