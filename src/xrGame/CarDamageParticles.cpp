...
void read_bones( IKinematics* K, LPCSTR S, xr_vector< u16 >& bones ) {
    ...
    for ( int i = 0; i < count; ++i ) {
        ...
        xr_vector< u16 >::iterator iter =
            std::find( bones.begin(), bones.end(), bone_id );
...
void CCarDamageParticles::Init( CCar* car ) {
    ...
    if ( ini->section_exist( "damage_particles" ) ) {
        m_car_damage_particles1 =
            ini->r_string( "damage_particles", "car_damage_particles1" );
        m_car_damage_particles2 =
            ini->r_string( "damage_particles", "car_damage_particles2" );
        m_wheels_damage_particles1 =
            ini->r_string( "damage_particles", "wheels_damage_particles1" );
        m_wheels_damage_particles2 =
            ini->r_string( "damage_particles", "wheels_damage_particles2" );

        read_bones( K, ini->r_string( "damage_particles", "particle_bones1" ),
                    bones1 );
        read_bones( K, ini->r_string( "damage_particles", "particle_bones2" ),
                    bones2 );
    }
}
...
void CCarDamageParticles::Play1( CCar* car ) {
    if ( *m_car_damage_particles1 ) {
        ...
        for ( ; e != i; ++i )
            car->StartParticles( m_car_damage_particles1, *i,
                                 Fvector().set( 0, 1, 0 ), car->ID() );
    }
}
...
void CCarDamageParticles::Play2( CCar* car ) {
    ...
    if ( *m_car_damage_particles2 ) {
        ...
        for ( ; e != i; ++i )
            car->StartParticles( m_car_damage_particles2, *i,
                                 Fvector().set( 0, 1, 0 ), car->ID() );
    }
}
...
void CCarDamageParticles::PlayWheel1( CCar* car, u16 bone_id ) {
    ...
    if ( *m_wheels_damage_particles1 )
        car->StartParticles( m_wheels_damage_particles1, bone_id,
                             Fvector().set( 0, 1, 0 ), car->ID() );
}
...
void CCarDamageParticles::PlayWheel2( CCar* car, u16 bone_id ) {
    ...
    if ( *m_wheels_damage_particles2 )
        car->StartParticles( m_wheels_damage_particles2, bone_id,
                             Fvector().set( 0, 1, 0 ), car->ID() );
}
