...
void CUIGameCTA::Init( int stage ) {
    if ( stage == 0 ) {
        m_round_result_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_round_result", Window );
        m_pressbuy_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_pressbuy", Window );
        m_pressjump_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_pressjump", Window );
        m_spectator_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_spectator", Window );
        m_spectrmode_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_spetatormode", Window );
        m_warm_up_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_warm_up", Window );
        m_time_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_timelimit", Window );
        m_demo_play_caption =
            UIHelper::CreateTextWnd( *MsgConfig, "mp_demo_play", Window );
        ...
        CUIXmlInit::InitTextWnd( uiXml, "reinforcement", 0,
                                 m_pReinforcementInidcator );
...
void CUIGameCTA::SetClGame( game_cl_GameState* g ) {
    ...
    m_pBuySpawnMsgBox->func_on_ok = CUIWndCallback::void_function(
        m_game, &game_cl_CaptureTheArtefact::OnBuySpawn );
}
...
void CUIGameCTA::UpdateBuyMenu( shared_str const& teamSection,
                                shared_str const& costSection ) {
    if ( m_pCurBuyMenu ) {
...
void CUIGameCTA::UpdateSkinMenu( shared_str const& teamSection ) {
    ...
    m_pCurSkinMenu = xr_new< CUISkinSelectorWnd >(
        m_teamSectionForSkinMenu.c_str(),
...
struct AmmoSearcherPredicate {
    ...
    bool operator()( PIItem const& item ) {
        ...
        if ( ( temp_ammo->m_boxCurr + additional_ammo_count ) <
             temp_ammo->m_boxSize )
            return false;
...
void TryToDefuseGrenadeLauncher( CWeaponMagazinedWGrenade const* weapon,
    ...
    VERIFY2( ammo_section.size(),
             make_string( "grenade ammo type of [%s] hasn't section name",
    ...
    TIItemContainer::const_iterator temp_iter =
        std::find_if( all_items.begin(), all_items.end(), ammo_completitor );
...
void TryToDefuseWeapon( CWeapon const* weapon,
    ...
    CWeaponMagazinedWGrenade const* tmp_gl_weapon =
        smart_cast< CWeaponMagazinedWGrenade const* >( weapon );
    ...
    VERIFY2( ammo_section.size(),
             make_string( "ammo type of [%s] hasn't section name",
                          weapon->cName().c_str() )
                 .c_str() );
    if ( !ammo_section.size() )
        return;
...
void CUIGameCTA::AdditionalAmmoInserter(
    aditional_ammo_t::value_type const& sect_name ) {
...
void CUIGameCTA::SetPlayerDefItemsToBuyMenu() {
    ...
    m_pCurBuyMenu->GetWeaponIndexByName( "mp_wpn_knife", KnifeSlot,
                                         KnifeIndex );
    //---------------------------------------------------------
    ...
    for ( ; It != Et; ++It ) {
        ...
        if ( PIT.ItemID == KnifeIndex )
            continue;
        m_pCurBuyMenu->ItemToSlot(
            m_pCurBuyMenu->GetWeaponNameByIndex( 0, PIT.ItemID ), PIT.SlotID );
    };
    //---------------------------------------------------------
...
void CUIGameCTA::SetPlayerItemsToBuyMenu() {
    ...
    if ( actor && !ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ) {
        ...
        aditional_ammo_t add_ammo(
            _alloca( sizeof( aditional_ammo_t::value_type ) *
                     ( max_addammo_count * 2 ) ),
            max_addammo_count * 2 );
...
void CUIGameCTA::GetPurchaseItems( BuyMenuItemsCollection& dest,
                                   s32& moneyDif ) {
    R_ASSERT( m_game );
    R_ASSERT( m_pCurBuyMenu );
    preset_items const* tmpPresItems =
        &( m_pCurBuyMenu->GetPreset( _preset_idx_last ) );
    if ( tmpPresItems->size() == 0 ) {
        tmpPresItems = &( m_pCurBuyMenu->GetPreset(
            _preset_idx_default ) ); //_preset_idx_origin));
    }
    preset_items::const_iterator pie = tmpPresItems->end();
    for ( preset_items::const_iterator pi = tmpPresItems->begin(); pi != pie;
          ++pi ) {
        u8 addon;
        u8 itemId;
    ...
    if ( m_game->local_player &&
         m_game->local_player->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ) {
        u8 KnifeSlot, KnifeIndex;
        m_pCurBuyMenu->GetWeaponIndexByName( "mp_wpn_knife", KnifeSlot,
                                             KnifeIndex );
    ...
    moneyDif = m_pCurBuyMenu->GetPresetCost( _preset_idx_origin ) -
               m_pCurBuyMenu->GetPresetCost( _preset_idx_last );
}

CUIGameCTA::BuyMenuItemPair CUIGameCTA::GetBuyMenuItem(
    shared_str const& itemSectionName ) {
    VERIFY( m_pCurBuyMenu );
    u8 groupId;
    u8 itemId;
    // we just use addon variable as temp storage
...
void CUIGameCTA::ShowSkinMenu( s8 currentSkin ) {
    ...
    if ( !m_pCurSkinMenu ) {
        ...
        Msg( "Warning: current skin window not initialized while trying to "
             "show it" );
#endif
        return;
    }
...
void CUIGameCTA::SetScore( s32 max_score,
                           s32 greenTeamScore,
                           s32 blueTeamScore ) {
    string32 str;
...
void CUIGameCTA::ShowBuySpawn( s32 spawn_cost ) {
    ...
    xr_sprintf( pay_frm_str, pay_frm_size, format_str,
        ...
        case DIK_CAPSLOCK: {
            if ( m_game ) {
                if ( m_game->Get_ShowPlayerNamesEnabled() )
                    m_game->Set_ShowPlayerNames(
                        !m_game->Get_ShowPlayerNames() );
                else
