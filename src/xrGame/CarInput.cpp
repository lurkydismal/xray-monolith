...
bool CCar::bfAssignMovement( CScriptEntityAction* tpEntityAction ) {
    ...
    vfProcessInputKey(
        kFWD, !!( l_tInput & CScriptMovementAction::eInputKeyForward ) );
    // While governed, throttle/brake/steer are owned by
    // SetTargetSpeed/SetSteer; the action only holds the engine on and the
    // drive gear engaged, so its back/steer keys must not reassert.
    if ( !m_speed_governed ) {
        vfProcessInputKey(
            kBACK, !!( l_tInput & CScriptMovementAction::eInputKeyBack ) );
        vfProcessInputKey(
            kL_STRAFE, !!( l_tInput & CScriptMovementAction::eInputKeyLeft ) );
        vfProcessInputKey(
            kR_STRAFE, !!( l_tInput & CScriptMovementAction::eInputKeyRight ) );
    }
    vfProcessInputKey(
        kACCEL, !!( l_tInput & CScriptMovementAction::eInputKeyShiftUp ) );
    vfProcessInputKey(
        kCROUCH, !!( l_tInput & CScriptMovementAction::eInputKeyShiftDown ) );
    vfProcessInputKey(
        kJUMP, !!( l_tInput & CScriptMovementAction::eInputKeyBreaks ) );
    ...
    // m_current_rpm =
    // _abs(tpEntityAction->m_tMovementAction.m_fSpeed*m_current_gear_ratio);
...
bool CCar::bfAssignObject( CScriptEntityAction* tpEntityAction ) {
    ...
    if ( l_tObjectAction.m_bCompleted ||
         !xr_strlen( l_tObjectAction.m_caBoneName ) )
        return ( ( l_tObjectAction.m_bCompleted = true ) == false );

    s16 l_sBoneID = smart_cast< IKinematics* >( Visual() )
                        ->LL_BoneID( l_tObjectAction.m_caBoneName );
...
void CCar::OnKeyboardPress( int cmd ) {
    ...
    if ( m_on_key_board_callback && strlen( m_on_key_board_callback ) ) {
        ...
        if ( ai().script_engine().functor( m_on_key_board_callback,
                                           lua_function ) ) {
    ...
    switch ( cmd ) {
        ...
        case kWPN_FUNC:
            if ( HasWeapon() )
                m_car_weapon->Action( CCarWeapon::eWpnActivate,
                                      !m_car_weapon->IsActive() );
            break;
            break;
        case kTORCH:
...
void CCar::OnKeyboardHold( int cmd ) {
    ...
    switch ( cmd ) {
        ...
        case kRIGHT:
            ...
            /*
                    case kFWD:
                            if (ectFree==active_camera->tag)
               active_camera->Move(kUP); else
               m_vCamDeltaHP.y += active_camera->rot_speed.y*Device.fTimeDelta;
                            break;
                    case kBACK:
                            if (ectFree==active_camera->tag)
               active_camera->Move(kDOWN); else
               m_vCamDeltaHP.y -= active_camera->rot_speed.y*Device.fTimeDelta;
                            break;
                    case kL_STRAFE:
                            if (ectFree==active_camera->tag)
               active_camera->Move(kLEFT); else
               m_vCamDeltaHP.x -= active_camera->rot_speed.x*Device.fTimeDelta;
                            break;
                    case kR_STRAFE:
                            if (ectFree==active_camera->tag)
               active_camera->Move(kRIGHT); else
               m_vCamDeltaHP.x += active_camera->rot_speed.x*Device.fTimeDelta;
                            break;
            */
    }
...
bool CCar::isObjectVisible( CScriptGameObject* O_ ) {
    if ( m_memory ) {
        ...
        if ( !O_ ) {
            Msg( "Attempt to call CCar::isObjectVisible method wihth passed "
                 "NULL parameter" );
            return false;
        }
        CObject* O = &O_->object();
        ...
        BOOL res =
            Level().ObjectSpace.RayTest( from_point, dir_to_object, ray_length,
                                         collide::rqtStatic, NULL, NULL );
        return ( 0 == res );
    }
}
