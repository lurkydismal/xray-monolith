...
void dump_collision_hit( CPHMovementControl* movement_control ) {
    ...
    Msg( "ai unit: %s hited by collision; power: %f, spawn frame %d, current "
         "frame %d ",
         obj->cName().c_str(), movement_control->gcontact_HealthLost,
         obj->spawn_time(), Device.dwFrame );
...
#endif
void CMovementManager::apply_collision_hit(
    CPHMovementControl* movement_control ) {
    VERIFY( movement_control );
    if ( object().g_Alive() &&
         !fsimilar( 0.f, movement_control->gcontact_HealthLost ) ) {
#ifdef DEBUG
        dump_collision_hit( movement_control );
#endif
        const ICollisionDamageInfo* di =
            movement_control->CollisionDamageInfo();
        VERIFY( di );
        Fvector dir;
        di->HitDir( dir );

        SHit HDS = SHit( movement_control->gcontact_HealthLost, dir,
                         di->DamageInitiator(), movement_control->ContactBone(),
...
Fvector CMovementManager::path_position( const float& velocity,
    ...
    // обновить detail().m_current_travel_point в соответствие с текущей
    // позицией
    while ( current_travel_point < detail().path().size() - 2 ) {
        float pos_dist_to_cur_point = dest_position.distance_to(
            detail().path()[ current_travel_point ].position );
        float pos_dist_to_next_point = dest_position.distance_to(
            detail().path()[ current_travel_point + 1 ].position );
        float cur_point_dist_to_next_point =
            detail().path()[ current_travel_point ].position.distance_to(
                detail().path()[ current_travel_point + 1 ].position );

        if ( ( pos_dist_to_cur_point > cur_point_dist_to_next_point ) &&
             ( pos_dist_to_cur_point > pos_dist_to_next_point ) ) {
...
Fvector CMovementManager::path_position( const float& time_to_check ) {
    ...
    return ( path_position( old_desirable_speed(), object().Position(),
                            time_to_check, current_travel_point, dist,
...
void CMovementManager::move_along_path( CPHMovementControl* movement_control,
    ...
    if ( !move_along_path() ) {
        ...
        if ( movement_control->IsCharacterEnabled() ) {
            movement_control->Calculate( detail().path(), 0.f,
    ...
    float dist_to_target;
    u32 current_travel_point = detail().m_current_travel_point;
    dest_position = path_position( old_desirable_speed(), object().Position(),
                                   time_delta, current_travel_point, dist,
                                   dist_to_target, dir_to_target );
...
#pragma todo( "Dima to ? : is this correct?" )
    ...
    if ( m_nearest_objects_query_pos.distance_to_sqr( dest_position ) >
    ...
    if ( DBG_PH_MOVE_CONDITIONS(
             ...
             !ph_dbg_draw_mask.test(
                 phDbgAlwaysUseAiPhMove ) && ) !( m_nearest_objects
                                                      .empty() ) ) {
        //  физ. объект

        if ( DBG_PH_MOVE_CONDITIONS( !ph_dbg_draw_mask.test(
                 phDbgNeverUseAiPhMove ) && ) !movement_control
                 ->TryPosition( dest_position ) ) {
            movement_control->GetPosition( dest_position );
            movement_control->Calculate( detail().path(), desirable_speed,
                                         detail().m_current_travel_point,
                                         precision );
    ...
    /*
            ...
            movement_control->Calculate
    (detail().path(), desirable_speed, detail().m_current_travel_point,
