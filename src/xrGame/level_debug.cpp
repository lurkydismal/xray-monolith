...
CLevelDebug::CLevelDebug()
    : m_p_texttree( xr_new< debug::text_tree >() ), m_texttree_offs( 0 ) {}
    ...
    if ( !smart_cast< CBaseMonster* >( Level().CurrentEntity() ) ) {
        ...
        if ( debug_actor_view && actor_view ) {
            debug::draw_text_tree(
                *actor_view, 2, x_start, y_start, m_texttree_offs, column_size,
                80, D3DCOLOR_XRGB( 0, 255, 0 ), D3DCOLOR_XRGB( 255, 255, 0 ) );
        }
        return;
    }

    if ( m_p_texttree->find_node( "General" ) ) {
        debug::draw_text_tree( *m_p_texttree->find_node( "General" ), 2,
                               x_start, y_start, m_texttree_offs, column_size,
                               80, D3DCOLOR_XRGB( 0, 255, 0 ),
                               D3DCOLOR_XRGB( 255, 255, 0 ) );
    }

    if ( m_p_texttree->find_node( "Brain" ) ) {
        debug::draw_text_tree(
            *m_p_texttree->find_node( "Brain" ), 2, x_start * 2 + column_size,
            y_start, m_texttree_offs, column_size, 80,
            D3DCOLOR_XRGB( 0, 255, 0 ), D3DCOLOR_XRGB( 255, 255, 0 ) );
    }

    if ( m_p_texttree->find_node( "Controllers" ) ) {
        debug::draw_text_tree( *m_p_texttree->find_node( "Controllers" ), 2,
                               x_start * 3 + column_size * 2, y_start,
...
CLevelDebug::CObjectInfo& CLevelDebug::object_info( CObject* obj,
                                                    LPCSTR class_name ) {
...
CLevelDebug::CTextInfo& CLevelDebug::text( void* class_ptr,
                                           LPCSTR class_name ) {
...
CLevelDebug::CLevelInfo& CLevelDebug::level_info( void* class_ptr,
                                                  LPCSTR class_name ) {
    SKey key( class_ptr, class_name );
...
void CLevelDebug::free_mem() {
    // free object info
    for ( OBJECT_INFO_MAP_IT it_obj = m_objects_info.begin();
          it_obj != m_objects_info.end(); ++it_obj ) {
        for ( CLASS_INFO_MAP_IT it_class = it_obj->second.begin();
              it_class != it_obj->second.end(); ++it_class ) {
            xr_delete( it_class->second );
        }
    }

    // free text info
    for ( TEXT_INFO_MAP_IT it = m_text_info.begin(); it != m_text_info.end();
          ++it ) {
        xr_delete( it->second );
    }

    // free text info
    for ( LEVEL_INFO_MAP_IT it = m_level_info.begin(); it != m_level_info.end();
          ++it ) {
        xr_delete( it->second );
    }
}
...
void CLevelDebug::draw_object_info() {
    ...
    for ( OBJECT_INFO_MAP_IT it = m_objects_info.begin();
          it != m_objects_info.end(); ++it ) {
        // если объект невалидный - удалить информацию
        if ( !it->first || it->first->getDestroy() ) {
            for ( CLASS_INFO_MAP_IT it_class = it->second.begin();
                  it_class != it->second.end(); ++it_class ) {
                xr_delete( it_class->second );
            }
        ...
        for ( CLASS_INFO_MAP_IT class_it = it->second.begin();
              class_it != it->second.end(); ++class_it ) {
            // get up on 2 meters
            ...
            if ( v_res.z < 0 || v_res.w < 0 )
                continue;
            if ( v_res.x < -1.f || v_res.x > 1.f || v_res.y < -1.f ||
                 v_res.y > 1.f )
                continue;
            ...
            float y =
                ( 1.f - v_res.y ) / 2.f * ( Device.dwHeight ) - delta_height;
            float start_y = y;
...
void CLevelDebug::CTextInfo::add_item( LPCSTR text,
                                       float x,
                                       float y,
                                       u32 color,
                                       u32 id ) {
...
void CLevelDebug::CLevelInfo::add_item( const Fvector& pos,
                                        u32 color,
                                        u32 id ) {
    inherited::add_item( SLevelItem( pos, color, id ) );
}

void CLevelDebug::CLevelInfo::add_item( const Fvector& pos1,
                                        const Fvector& pos2,
                                        u32 color,
                                        u32 id ) {
...
struct DrawLevelPredicate {
    void operator()( CLevelDebug::SLevelItem s ) {
        if ( s.ptype == CLevelDebug::SLevelItem::ePoint ) {
            Level().debug_renderer().draw_aabb( s.position1, 0.35f, 0.35f,
                                                0.35f, s.color );
            ...
            Level().debug_renderer().draw_line( Fidentity, s.position1, upV,
                                                s.color );

        } else if ( s.ptype == CLevelDebug::SLevelItem::eLine ) {
            Level().debug_renderer().draw_line( Fidentity, s.position1,
                                                s.position2, s.color );
        } else if ( s.ptype == CLevelDebug::SLevelItem::eBox ) {
            Level().debug_renderer().draw_aabb( s.position1, s.radius, s.radius,
                                                s.radius, s.color );
        }
    }
};
