...
#define TEMPLATE_SPECIALIZATION                              \
    template < typename _action_type, typename _object_type, \
               typename _action_id_type >

#define CSSetupManager \
    CSetupManager< _action_type, _object_type, _action_id_type >
...
TEMPLATE_SPECIALIZATION
IC _action_type& CSSetupManager::action(
    const _action_id_type& action_id ) const {
    typename setup_actions::const_iterator I = std::find_if(
        actions().begin(), actions().end(), setup_pred( action_id ) );
...
TEMPLATE_SPECIALIZATION
IC void CSSetupManager::add_action( const _action_id_type& action_id,
                                    _action_type* action ) {
    m_actuality = false;
    VERIFY( action );
    VERIFY( std::find_if( actions().begin(), actions().end(),
                          setup_pred( action_id ) ) == actions().end() );
        ...
        for ( ; I != E; ++I ) {
            ...
            if ( m_total_weight > m_random ) {
                if ( std::find_if( actions().begin(), actions().end(),
                                   setup_pred( m_current_action_id ) ) !=
                     actions().end() )
...
TEMPLATE_SPECIALIZATION
IC const typename CSSetupManager::setup_actions& CSSetupManager::actions()
    const {
    return ( m_actions );
}
