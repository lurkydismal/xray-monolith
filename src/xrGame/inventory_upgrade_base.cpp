...
void UpgradeBase::construct( const shared_str& upgrade_id,
                             Manager& manager_r ) {
...
void UpgradeBase::add_dependent_groups( LPCSTR groups_str,
                                        Manager& manager_r ) {
    ...
    for ( int n = _GetItemCount( groups_str ), i = 0; i < n; ++i ) {
        Group* group_p = manager_r.add_group(
            _GetItem( groups_str, i, temp, buffer_size ), *this );

        if ( std::find( m_depended_groups.begin(), m_depended_groups.end(),
                        group_p ) == m_depended_groups.end() ) {
...
UpgradeStateResult UpgradeBase::can_install( CInventoryItem& item,
                                             bool loading ) {
    if ( !m_known && !loading ) {
        if ( g_upgrades_log == 1 ) {
            Msg( "- Upgrade <%s> (id = %d) is in mode <unknown>.", id_str(),
                 item.object_id() );
        }
    ...
    if ( item.has_upgrade( m_id ) ) {
        if ( g_upgrades_log == 1 ) {
            Msg( "- Upgrade <%s> (id = %d) is installed already.", id_str(),
                 item.object_id() );
        }
        /*if ( loading )
        {
                FATAL( make_string( "Loading item: Upgrade <%s> (id = %d) is
        installed already.", id_str(), item.object_id() ).c_str() );
        }*/
