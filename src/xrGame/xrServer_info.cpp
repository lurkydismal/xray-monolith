...
server_info_uploader& xrServer::GetServerInfoUploader() {
    ...
    info_uploaders_t::iterator tmp_iter =
        std::find_if( m_info_uploaders.begin(), m_info_uploaders.end(),
                      free_info_searcher() );
...
void xrServer::SendServerInfoToClient(
    ClientID const& new_client ) // WARNING ! this function is thread unsafe !!!
{
    if ( IsGameTypeSingle() ) {
    ...
    svinfo_upload_complete_cb upload_compl_cb( this,
                                               &xrServer::SendConfigFinished );
    server_info_uploader& tmp_uploader = GetServerInfoUploader();
    tmp_uploader.start_upload_info( m_server_logo, m_server_rules, new_client,
                                    upload_compl_cb );
}
...
server_info_uploader::server_info_uploader(
    file_transfer::server_site* file_transfers )
...
void server_info_uploader::terminate_upload() {
    R_ASSERT( is_active() );
    m_file_transfers->stop_transfer_file(
        std::make_pair( m_to_client, m_from_client ) );
    m_state = eUploadNotActive;
    execute_complete_cb();
}
...
void server_info_uploader::start_upload_info(
    ...
    buffer_vector< mutable_buffer_t > tmp_bufvec(
        _alloca( sizeof( mutable_buffer_t ) * 2 ), 2 );

    tmp_bufvec.push_back( std::make_pair(
        static_cast< u8* >( svlogo->pointer() ), svlogo->length() ) );

    tmp_bufvec.push_back( std::make_pair(
        static_cast< u8* >( svrules->pointer() ), svrules->length() ) );

    m_to_client = toclient;

    m_file_transfers->start_transfer_file( tmp_bufvec, m_to_client,
                                           m_from_client, sndcb, 0 );
...
void __stdcall server_info_uploader::upload_server_info_callback(
    ...
    switch ( status ) {
...
#ifdef DEBUG
            Msg( "* uploaded %d from %d bytes of server logo to client [%d]",
                 uploaded, total, m_to_client.value() );
#endif
            return;
        } break;
        ...
        case file_transfer::sending_rejected_by_peer: {
            Msg( "* upload server logo terminated by peer [%d]",
                 m_to_client.value() );
        } break;
        case file_transfer::sending_complete: {
            Msg( "* upload server info to client [%d] complete !",
                 m_to_client.value() );
        } break;
    };
    m_state = eUploadNotActive;
