...
void game_cl_Deathmatch::OnBuyMenu_Ok() {
    ...
    const preset_items* _p = &( pCurBuyMenu->GetPreset(
        _preset_idx_last ) ); //_preset_idx_last : _preset_idx_origin);
    if ( _p->size() == 0 ) {
    ...
    for ( u32 i = 0; i < ItemsCount; ++i ) {
        ...
        for ( u32 idx = 0; idx < _pitem.count; ++idx ) {
            u8 SlotID = 0;
            u8 ItemID = 0;
            pCurBuyMenu->GetWeaponIndexByName( _pitem.sect_name, SlotID,
                                               ItemID );
    ...
    if ( local_player &&
        ...
        pCurBuyMenu->GetWeaponIndexByName( "mp_wpn_knife", KnifeSlot,
                                           KnifeIndex );
...
void game_cl_Deathmatch::OnBuyMenu_DefaultItems() {
    ...
    /*	PRESET_ITEMS_it It = PlayerDefItems.begin();
            ...
            for ( ; It != Et; ++It)
            {
                    ...
                    pCurBuyMenu->SectionToSlot(u8((ItemID&0xff00)>>0x08),
       u8(ItemID&0x00ff), false);
            };
    */ //---------------------------------------------------------
...
void game_cl_Deathmatch::SetBuyMenuItems( PRESET_ITEMS* pItems,
    ...
    //---------------------------------------------------------
    CActor* pCurActor =
        smart_cast< CActor* >( Level().Objects.net_Find( P->GameID ) );
    if ( pCurActor ) {
        ...
        for ( ; ISlot <= ESlot; ++ISlot ) {
            ...
            if ( !pItem )
                continue;
            // if (pItem->IsInvalid() || pItem->object().CLS_ID ==
            // CLSID_OBJECT_W_KNIFE)	continue;
            if ( pItem->IsInvalid() ||
                 smart_cast< CWeaponKnife* >( &pItem->object() ) )
                continue;
            if ( !pItem->CanTrade() ) {
                continue;
            }

            if ( pSettings->line_exist( GetBaseCostSect(),
        ...
        TIItemContainer::const_iterator IBelt =
            pCurActor->inventory().m_belt.begin();
        TIItemContainer::const_iterator EBelt =
            pCurActor->inventory().m_belt.end();
        ...
        TIItemContainer::const_iterator ERuck =
            pCurActor->inventory().m_ruck.end();
        ...
        for ( ; It != Et; ++It ) {
            ...
            if ( PIT.ItemID == KnifeIndex )
                continue;
            pCurBuyMenu->ItemToSlot(
                pCurBuyMenu->GetWeaponNameByIndex( 0, PIT.ItemID ),
                PIT.SlotID );
        };
        //---------------------------------------------------------
    };
...
void game_cl_Deathmatch::CheckItem( PIItem pItem,
    ...
    u8 SlotID, ItemID;
    pCurBuyMenu->GetWeaponIndexByName( *pItem->object().cNameSect(), SlotID,
                                       ItemID );
    ...
    //-----------------------------------------------------
    PRESET_ITEMS_it PresetItemIt =
        std::find( pPresetItems->begin(), pPresetItems->end(), BigID );
    if ( OnlyPreset ) {
    ...
    if ( SlotID == INV_SLOT_2 ) {
        PRESET_ITEMS_it DefPistolIt =
            std::find( PlayerDefItems.begin(), PlayerDefItems.end(), BigID );
        if ( DefPistolIt != PlayerDefItems.end() &&
             PresetItemIt == pPresetItems->end() )
            return;
    }
    ...
    if ( pWeapon ) {
        if ( pWeapon->ScopeAttachable() ) {
            pCurBuyMenu->GetWeaponIndexByName( *pWeapon->GetScopeName(), SlotID,
                                               ItemID );
            if ( SlotID != 0xff && ItemID != 0xff ) {
                if ( pWeapon->IsScopeAttached() ) {
                    if ( ( DesiredAddons &
                        ...
                        pCurBuyMenu->AddonToSlot(
                            CSE_ALifeItemWeapon::eWeaponAddonScope,
                            pWeapon->BaseSlot(), true );
                } else {
                    if ( DesiredAddons &
                         CSE_ALifeItemWeapon::eWeaponAddonScope )
                        pCurBuyMenu->AddonToSlot(
                            CSE_ALifeItemWeapon::eWeaponAddonScope,
                            pWeapon->BaseSlot(), false );
                }
            }
        };

        if ( pWeapon->GrenadeLauncherAttachable() ) {
            pCurBuyMenu->GetWeaponIndexByName(
                *pWeapon->GetGrenadeLauncherName(), SlotID, ItemID );
            if ( SlotID != 0xff && ItemID != 0xff ) {
        ...
        if ( pWeapon->SilencerAttachable() ) {
            pCurBuyMenu->GetWeaponIndexByName( *pWeapon->GetSilencerName(),
                                               SlotID, ItemID );
            if ( SlotID != 0xff && ItemID != 0xff ) {
...
void game_cl_Deathmatch::LoadDefItemsForRank( IBuyWnd* pBuyMenu ) {
    ...
    for ( ; It != Et; ++It ) {
        ...
        if ( PIT.ItemID == KnifeIndex )
            continue;
        pCurBuyMenu->ItemToSlot(
            pCurBuyMenu->GetWeaponNameByIndex( 0, PIT.ItemID ), PIT.SlotID );
    };
    //---------------------------------------------------------
...
void game_cl_Deathmatch::AdditionalAmmoInserter(
