...
IC bool CCoverManager::edge_vertex( u32 index ) const {
    ...
    return ( ( !ai().level_graph().valid_vertex_id( v->link( 0 ) ) &&
               ( v->high_cover( 0 ) < MIN_COVER_VALUE ) ) ||
             ( !ai().level_graph().valid_vertex_id( v->link( 1 ) ) &&
               ( v->high_cover( 1 ) < MIN_COVER_VALUE ) ) ||
             ( !ai().level_graph().valid_vertex_id( v->link( 2 ) ) &&
               ( v->high_cover( 2 ) < MIN_COVER_VALUE ) ) ||
             ( !ai().level_graph().valid_vertex_id( v->link( 3 ) ) &&
               ( v->high_cover( 3 ) < MIN_COVER_VALUE ) ) ||
             ( !ai().level_graph().valid_vertex_id( v->link( 0 ) ) &&
               ( v->low_cover( 0 ) < MIN_COVER_VALUE ) ) ||
             ( !ai().level_graph().valid_vertex_id( v->link( 1 ) ) &&
               ( v->low_cover( 1 ) < MIN_COVER_VALUE ) ) ||
             ( !ai().level_graph().valid_vertex_id( v->link( 2 ) ) &&
               ( v->low_cover( 2 ) < MIN_COVER_VALUE ) ) ||
             ( !ai().level_graph().valid_vertex_id( v->link( 3 ) ) &&
               ( v->low_cover( 3 ) < MIN_COVER_VALUE ) ) );
}

IC bool CCoverManager::cover( CLevelGraph::CVertex* v,
                              u32 index0,
                              u32 index1 ) const {
    return (
        ai().level_graph().valid_vertex_id( v->link( index0 ) ) &&
        ai().level_graph().valid_vertex_id(
            ai().level_graph().vertex( v->link( index0 ) )->link( index1 ) ) &&
        m_temp[ ai().level_graph()
                    .vertex( v->link( index0 ) )
                    ->link( index1 ) ] );
}

IC bool CCoverManager::critical_point( CLevelGraph::CVertex* v,
                                       u32 index,
                                       u32 index0,
                                       u32 index1 ) const {
...
void CCoverManager::compute_static_cover() {
    ...
    m_covers = xr_new< CPointQuadTree >(
        ...
        ai().level_graph().header().cell_size() * .5f, 8 * 65536, 4 * 65536 );
    ...
    xr_parallel_for( 0u, LevelVertexCount, [ & ]( u32 range ) {
        const CLevelGraph::CVertex& vertex = *Graph.vertex( range );
    ...
    for ( u32 i = 0; i < LevelVertexCount; ++i )
        if ( m_temp[ i ] && critical_cover( i ) )
            m_covers->insert(
                xr_new< CCoverPoint >( ai().level_graph().vertex_position(
                                           ai().level_graph().vertex( i ) ),
                                       i ) );
...
void CCoverManager::remove_nearby_covers(
    smart_cover::cover const& cover,
    smart_cover::object const& object ) const {
    m_nearest.clear();
    ...
    for ( ; I != E; ++I ) {
        ...
        m_nearest.erase( std::remove_if( m_nearest.begin(), m_nearest.end(),
...
CCoverManager::Cover const* CCoverManager::add_smart_cover(
    ...
    Cover* smart_cover = xr_new< Cover >(
        object, m_smart_covers_storage->description( table_name ),
        is_combat_cover, can_fire, loopholes );
...
void CCoverManager::actualize_smart_covers() const {
    std::sort( m_smart_covers.begin(), m_smart_covers.end(),
               id_predicate_less() );

    m_smart_covers_actual = true;
}

CCoverManager::Cover* CCoverManager::smart_cover(
    shared_str const& cover_id ) const {
    if ( !m_smart_covers_actual )
        actualize_smart_covers();

    SmartCovers::iterator found =
        std::lower_bound( m_smart_covers.begin(), m_smart_covers.end(),
                          cover_id, id_predicate_less() );

    VERIFY2( ( ( found != m_smart_covers.end() ) &&
               ( ( *found )->id()._get() == cover_id._get() ) ),
