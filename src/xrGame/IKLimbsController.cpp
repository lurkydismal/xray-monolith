...
void CIKLimbsController::Create( CGameObject* O ) {
    ...
    if ( already_has_callbacks )
        std::swap( *( O->visual_callbacks().begin() ),
                   *( O->visual_callbacks().end() - 1 ) );
...
void CIKLimbsController::LimbSetup() {
    ...
    IKinematicsAnimated* skeleton_animated =
        m_object->Visual()->dcast_PKinematicsAnimated();

    _bone_chains.back().Create( ( u16 )_bone_chains.size() - 1,
                                skeleton_animated, true );
}

void CIKLimbsController::LimbCalculate( SCalculateData& cd ) {
    cd.do_collide =
        m_legs_blend && !cd.m_limb->KinematicsAnimated()
                             ->LL_GetMotionDef( m_legs_blend->motionID )
                             ->marks.empty(); // m_legs_blend->;
...
float CIKLimbsController::LegLengthShiftLimit(
    float current_shift,
    const SCalculateData cd[ max_size ] ) {
    float shift_down = -phInfinity;
    const u16 sz = ( u16 )_bone_chains.size();
    for ( u16 j = 0; sz > j; ++j )
        if ( cd[ j ].state.foot_step ) {
            float s_down =
                cd[ j ].m_limb->ObjShiftDown( current_shift, cd[ j ] );
...
float CIKLimbsController::StaticObjectShift(
    const SCalculateData cd[ max_size ] ) {
    ...
    if ( shift_down > 0.f )
        ...
        shift = shift_up;
    VERIFY( _valid( shift ) );
    _object_shift.set_taget(
        shift, _abs( current_shift - shift ) / static_shift_object_speed );
    return shift;
}
...
bool CIKLimbsController::PredictObjectShift(
    const SCalculateData cd[ max_size ] ) {
    ...
    for ( u16 j = 0; sz > j; ++j )
        if ( !cd[ j ].state.foot_step ) {
            ...
            } else if ( current_shift < -doun_shift_to_correct &&
                        time < predict_time_shift_up ) {
...
void CIKLimbsController::ObjectShift( float static_shift,
                                      const SCalculateData cd[ max_size ] ) {
    u16 cnt_in_step = 0;
    const u16 sz = ( u16 )_bone_chains.size();
    for ( u16 j = 0; sz > j; ++j )
...
void CIKLimbsController::Calculate() {
    ...
    if ( ph_dbg_draw_mask1.test( phDbgDrawIKSHiftObject ) )
        _object_shift.dbg_draw( obj, _pose_extrapolation,
                                Fvector().set( 0, 2.5f, 0 ) );
#endif

    SCalculateData cd[ max_size ];

    xr_vector< CIKLimb >::iterator i, b = _bone_chains.begin(),
                                      e = _bone_chains.end();
    for ( i = b; e != i; ++i ) {
    ...
    root_bi.set_callback( root_bi.callback_type(), 0, root_bi.callback_param(),
                          TRUE );
    ...
    for ( u16 j = 0; sz > j; ++j ) {
        ...
        if ( ph_dbg_draw_mask1.test( phDbgDrawIKPredict ) ) {
            ...
            DBG_DrawLine(
                toe,
                Fvector().add( toe,
                               Fvector().set( 0, -_object_shift.shift(), 0 ) ),
    ...
    root_bi.set_callback( root_bi.callback_type(), sv_root_cb,
                          root_bi.callback_param(), sv_root_cb_ovwr );
}
...
void CIKLimbsController::Destroy( CGameObject* O ) {
    ...
    xr_vector< CIKLimb >::iterator i = _bone_chains.begin(),
                                   e = _bone_chains.end();
    for ( ; e != i; ++i )
        i->Destroy();
    _bone_chains.clear();
        ...
        if ( CPhysicsShellHolder* Sh = O->cast_physics_shell_holder() ) {
            if ( CIKLimbsController* ik = Sh->character_ik_controller() ) {
                if ( !Sh->m_pPhysicsShell ) {
                    Fvector ce;
                    O->Center( ce );
                    if ( Render->ViewBase.testSphere_dirty( ce,
                                                            O->Radius() ) ) {
                        if ( Device.vCameraPosition.distance_to_sqr(
                                 O->Position() ) > 3000.f ) {
                            // if (Device.dwFrame < ik->optimize_frame)
                            IKinematics* K = O->Visual()->dcast_PKinematics();
                            u16 root = K->LL_GetBoneRoot();
                            CBoneInstance& root_bi =
                                K->LL_GetBoneInstance( root );
                            root_bi.reset_callback();
                            return;
                            // ik->optimize_frame = Device.dwFrame +
                            // Random.randI(8);
                        }
...
void CIKLimbsController::PlayLegs( CBlend* b ) {
    ...
    anim_set_name =
        skeleton_animated->LL_MotionDefName_dbg( b->motionID ).second;

    CMotionDef& MD = *skeleton_animated->LL_GetMotionDef( b->motionID );
    if ( MD.marks.empty() && ( MD.flags & esmUseFootSteps ) )
        Msg( "! No foot stseps for animation: animation name: %s, animation "
             "set: %s ",
             anim_name, anim_set_name );
#endif
}
