...
shared_str CSpaceRestrictionManager::base_in_restrictions(
    ALife::_OBJECT_ID id ) {
...
shared_str CSpaceRestrictionManager::base_out_restrictions(
    ALife::_OBJECT_ID id ) {
    CLIENT_RESTRICTIONS::iterator I = m_clients->find( id );
    VERIFY( m_clients->end() != I );
...
IC void CSpaceRestrictionManager::collect_garbage() {
    ...
    for ( ; I != E; ) {
        if ( !( *I ).second->intrusive_ref_count() &&
             ( Device.dwTimeGlobal >=
               ( *I ).second->m_last_time_dec + time_to_delete ) ) {
            J = I;
            ++I;
            xr_delete( ( *J ).second );
...
void CSpaceRestrictionManager::restrict( ALife::_OBJECT_ID id,
                                         shared_str out_restrictors,
    ...
    difference_restrictions( _default_out_restrictions,
                             merged_in_restrictions );
    difference_restrictions( _default_in_restrictions,
                             merged_out_restrictions );
    ...
    CLIENT_RESTRICTIONS::iterator I = m_clients->find( id );
    VERIFY2(
        ( m_clients->end() == I ) || !( *I ).second.m_restriction ||
            !( *I ).second.m_restriction->applied(),
        "Restriction cannot be changed since its border is still applied!" );
    ( *m_clients )[ id ].m_restriction =
        restriction( merged_out_restrictions, merged_in_restrictions );
...
bool CSpaceRestrictionManager::accessible( ALife::_OBJECT_ID id,
...
CSpaceRestrictionManager::CRestrictionPtr CSpaceRestrictionManager::restriction(
    ...
    SPACE_RESTRICTIONS::const_iterator I =
        m_space_restrictions.find( space_restrictions );
...
u32 CSpaceRestrictionManager::accessible_nearest( ALife::_OBJECT_ID id,
...
bool CSpaceRestrictionManager::restriction_presented(
    shared_str restrictions,
    shared_str restriction ) const {
    string4096 m_temp;
...
IC void CSpaceRestrictionManager::join_restrictions( shared_str& restrictions,
                                                     shared_str update ) {
    string4096 m_temp1;
    string4096 m_temp2;
    xr_strcpy( m_temp2, *restrictions );
    for ( u32 i = 0, n = _GetItemCount( *update ), count = xr_strlen( m_temp2 );
          i < n; ++i )
        if ( !restriction_presented( m_temp2,
                                     _GetItem( *update, i, m_temp1 ) ) ) {
            if ( count )
                xr_strcat( m_temp2, "," );
...
IC void CSpaceRestrictionManager::difference_restrictions(
    shared_str& restrictions,
    shared_str update ) {
    string4096 m_temp1;
    string4096 m_temp2;
    ...
    for ( u32 i = 0, n = _GetItemCount( *restrictions ), count = 0; i < n; ++i )
        if ( !restriction_presented( update,
                                     _GetItem( *restrictions, i, m_temp1 ) ) ) {
            if ( count )
                xr_strcat( m_temp2, "," );
            xr_strcat( m_temp2, m_temp1 );
            ++count;
...
void CSpaceRestrictionManager::add_restrictions(
    ALife::_OBJECT_ID id,
    shared_str add_out_restrictions,
    ...
    shared_str new_out_restrictions =
        client_restriction.m_base_out_restrictions;
...
void CSpaceRestrictionManager::remove_restrictions(
    ALife::_OBJECT_ID id,
    shared_str remove_out_restrictions,
...
void CSpaceRestrictionManager::change_restrictions(
    ALife::_OBJECT_ID id,
    shared_str add_out_restrictions,
    shared_str add_in_restrictions,
    shared_str remove_out_restrictions,
...
void CSpaceRestrictionManager::on_default_restrictions_changed() {
    ...
    for ( ; I != E; ++I )
        restrict( ( *I ).first, ( *I ).second.m_base_out_restrictions,
                  ( *I ).second.m_base_in_restrictions );
}
