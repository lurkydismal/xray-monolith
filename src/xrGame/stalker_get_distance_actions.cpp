...
CStalkerActionRunToCover::CStalkerActionRunToCover( CAI_Stalker* object,
                                                    LPCSTR action_name )
...
void CStalkerActionRunToCover::initialize() {
    ...
    object().m_ce_close->setup(
        position, 0.f, object().Position().distance_to( position ), 10.f );
    ...
    if ( object().movement().restrictions().accessible(
...
void CStalkerActionRunToCover::execute() {
    ...
    if ( !object().memory().visual().visible_now(
        ...
        object().sight().setup( CSightAction(
            object().memory().enemy().selected(), true, true, false ) );
        fire();
    }
    ...
    m_storage->set_property( StalkerDecisionSpace::eWorldPropertyInCover,
                             true );
}
...
CStalkerActionWaitInCover::CStalkerActionWaitInCover( CAI_Stalker* object,
                                                      LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionWaitInCover::initialize() {
    ...
    if ( object().movement().body_state() == eBodyStateStand )
        object().movement().set_body_state(
            ::Random.randI( 2 ) ? eBodyStateCrouch : eBodyStateStand );
    else
...
void CStalkerActionWaitInCover::execute() {
    ...
    if ( !completed() && !object().memory().visual().visible_now(
                             object().memory().enemy().selected() ) )
        return;
