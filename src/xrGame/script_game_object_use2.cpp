...
void CScriptGameObject::force_visibility_state( int state ) {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CAI_Bloodsucker [%s]: cannot access class member "
...
void CScriptGameObject::set_override_animation( pcstr anim_name ) {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "object is not of CBaseMonster class!" );
        return;
    }
...
void CScriptGameObject::set_override_animation( u32 AnimType, u32 AnimIndex ) {
    ...
    monster->anim().set_override_animation_script( ( EMotionAnim )AnimType,
                                                   AnimIndex );
}
...
void CScriptGameObject::set_manual_invisibility( bool val ) {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CAI_Bloodsucker [%s]: cannot access class member "
                         "set_manual_invisible!",
                         object().cNameSect().c_str() )
                .c_str() );
        return;
    }
    val ? monster->set_manual_control( true )
        : monster->set_manual_control( false );
}

void CScriptGameObject::bloodsucker_drag_jump( CScriptGameObject* e,
                                               LPCSTR e_str,
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CScriptGameObject : cannot process drag, anim, jump for "
            "CAI_Bloodsucker!" );
        return;
    }
...
void CScriptGameObject::set_enemy( CScriptGameObject* e ) {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Bloodsucker [%s]: cannot access class member set_enemy!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
CScriptSoundInfo CScriptGameObject::GetSoundInfo() {
    ...
    if ( l_tpMonster ) {
        if ( l_tpMonster->SoundMemory.IsRememberSound() ) {
            ...
            ret_val.set(
                ( pO && !pO->getDestroy() ) ? pO->lua_game_object() : 0,
                bDangerous, se.position, se.power, int( se.time ) );
        }
    } else {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
...
CScriptMonsterHitInfo CScriptGameObject::GetMonsterHitInfo() {
    ...
    if ( l_tpMonster ) {
        if ( l_tpMonster->HitMemory.is_hit() ) {
            CGameObject* pO = smart_cast< CGameObject* >(
                l_tpMonster->HitMemory.get_last_hit_object() );
            ret_val.set(
                ( pO && !pO->getDestroy() ) ? pO->lua_game_object() : 0,
                l_tpMonster->HitMemory.get_last_hit_dir(),
...
void CScriptGameObject::set_home( LPCSTR name,
                                  float r_min,
                                  float r_max,
                                  bool aggressive,
                                  float r_mid ) {
...
void CScriptGameObject::set_home( u32 lv_ID,
                                  float r_min,
                                  float r_max,
                                  bool aggressive,
                                  float r_mid ) {
...
bool CScriptGameObject::fake_death_fall_down() {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CZombie [%s]: cannot access class member "
