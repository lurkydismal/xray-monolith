...
void player_state_marksman::OnPlayerKilled(
    ...
    if ( !m_owner->is_item_in_group( weapon_gid,
    ...
    u16 victim_weapon_gid =
        m_owner->get_active_weapon_of_player( victim_player );
    if ( !m_owner->is_item_in_group( victim_weapon_gid,
                                     ammunition_group::gid_sniper_rifels ) )
        return;
    ...
    if ( killer_actor && victim_actor ) {
        float kill_dist =
            killer_actor->Position().distance_to( victim_actor->Position() );
