...
class clientdata_proxy : xray::noncopyable {
private:
...
public:
    ...
    void make_screenshot( ClientID const& admin_id,
                          ClientID const& cheater_id );
    void make_config_dump( ClientID const& admin_id,
                           ClientID const& cheater_id );

    bool is_active();

    void __stdcall download_screenshot_callback(
        file_transfer::receiving_status_t status,
        u32 downloaded,
        u32 total );
    void __stdcall download_config_callback(
        file_transfer::receiving_status_t status,
        u32 downloaded,
        u32 total );
