...
bool disk_file_reader::make_data_packet( NET_Packet& packet, u32 chunk_size ) {
    u32 size_to_write =
        ( static_cast< u32 >( m_reader->elapsed() ) >= chunk_size )
            ? chunk_size
            : m_reader->elapsed();
...
memory_reader::memory_reader( u8* data_ptr, u32 data_size ) {
    m_reader = xr_new< IReader >( static_cast< void* >( data_ptr ),
...
buffers_vector_reader::buffers_vector_reader(
    buffer_vector< mutable_buffer_t >* buffers )
...
void buffers_vector_reader::read_from_current_buf( NET_Packet& dest,
                                                   u32 read_size ) {
...
bool buffers_vector_reader::make_data_packet( NET_Packet& packet,
                                              u32 chunk_size ) {
    u32 buffer_size = m_buffers.front().second;
    ...
    do {
        ...
        if ( ( chunk_size <= rest_size ) &&
             ( chunk_size > buff_size_header ) ) {
...
memory_writer_reader::memory_writer_reader( CMemoryWriter* src_writer,
                                            u32 const max_size )
...
bool memory_writer_reader::make_data_packet( NET_Packet& packet,
                                             u32 chunk_size ) {
    u32 elapsed = m_writer_as_src->size() - m_writer_pointer;
    // in case of very fast client (can be server client),
    // server can send all data stored in memory writer, while new data is not
    // received ready_to_send method will return true because tell() < size().
...
void filetransfer_node::calculate_chunk_size( u32 peak_throughput,
                                              u32 current_throughput ) {
    if ( ( Device.dwTimeGlobal - m_last_chunksize_update_time ) < 1000 )
        return;

    if ( m_last_peak_throughput <
         peak_throughput ) // peak throughput is increasing, so we can increase
                           // upload size :)
    {
        m_chunk_size += data_min_chunk_size;
#ifdef MP_LOGGING
        Msg( "* peak throughout is not reached - increasing upload rate : "
             "(m_chunk_size: %d)",
             m_chunk_size );
#endif
...
#ifdef MP_LOGGING
        Msg( "* peak throughout is reached, (current_throughput: %d), "
             "(peak_throughput: %d), (m_chunk_size: %d)",
