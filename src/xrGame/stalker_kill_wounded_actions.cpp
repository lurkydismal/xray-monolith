...
CStalkerActionReachWounded::CStalkerActionReachWounded( CAI_Stalker* object,
                                                        LPCSTR action_name )
...
void CStalkerActionReachWounded::initialize() {
    ...
    object().set_goal( eObjectActionIdle, weapon_to_kill( &object() ),
                       MIN_QUEUE, MAX_QUEUE, MIN_INTERVAL, MAX_INTERVAL );
}
...
void CStalkerActionReachWounded::execute() {
    ...
    if ( object().Position().distance_to_sqr(
             mem_object.m_object_params.m_position ) < _sqr( 3.f ) ) {
        object().movement().set_movement_type( eMovementTypeStand );
        return;
    }
...
CStalkerActionAimWounded::CStalkerActionAimWounded( CAI_Stalker* object,
                                                    LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionAimWounded::initialize() {
    ...
    object().CObjectHandler::set_goal( eObjectActionAimReady1,
                                       weapon_to_kill( &object() ), MIN_QUEUE,
                                       MAX_QUEUE, MIN_INTERVAL, MAX_INTERVAL );
...
CStalkerActionPrepareWounded::CStalkerActionPrepareWounded( CAI_Stalker* object,
                                                            LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionPrepareWounded::execute() {
    ...
    if ( object().agent_manager().enemy().wounded_processor( enemy ) !=
         object().ID() )
        return;
...
CStalkerActionKillWounded::CStalkerActionKillWounded( CAI_Stalker* object,
                                                      LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionKillWounded::execute() {
    ...
    const CEntityAlive* enemy = object().memory().enemy().selected();
    object().sight().setup( CSightAction( enemy, true ) );
    object().set_goal( eObjectActionFire1, weapon_to_kill( &object() ),
                       MIN_QUEUE, MAX_QUEUE, MIN_INTERVAL, MAX_INTERVAL );
    ...
    if ( object().memory().visual().visible_now( enemy ) &&
         object().can_kill_enemy() && !object().can_kill_member() )
        return;
    ...
    HS.boneID = smart_cast< IKinematics* >(
                    ( const_cast< CEntityAlive* >( enemy ) )->Visual() )
                    ->LL_GetBoneRoot();
...
CStalkerActionPauseAfterKill::CStalkerActionPauseAfterKill( CAI_Stalker* object,
                                                            LPCSTR action_name )
    : inherited( object, action_name ) {}
...
void CStalkerActionPauseAfterKill::initialize() {
    ...
    object().CObjectHandler::set_goal( eObjectActionAimReady1,
                                       ...
                                       MAX_QUEUE, MIN_INTERVAL, MAX_INTERVAL );
    object().sight().setup(
        CSightAction( SightManager::eSightTypeCurrentDirection, true ) );
}
