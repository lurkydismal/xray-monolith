...
BOOL CEffectorBobbing::ProcessCam( SCamEffectorInfo& info ) {
    ...
    if ( !fsimilar( fReminderFactor, 0 ) ) {
        ...
        float _sinA =
            _abs( _sin( ST ) * A ) * fReminderFactor * g_head_bob_factor;
...
// demonized: First Person Death (Cam Effector, can be used in scripts any time
// to set custom position and direction)
...
void CFPCamEffector::ema( Fvector& current,
                          Fvector& target,
                          unsigned int steps ) {
    float smoothing_alpha = 2.0 / ( steps + 1 );
    float delta = Device.dwTimeDelta;

    if ( fis_zero( current.x ) && fis_zero( current.y ) &&
         fis_zero( current.z ) ) {
        current.x = target.x;
        current.y = target.y;
        current.z = target.z;
    ...
    current.x =
        current.x + std::min( 1.f, smoothing_alpha * ( delta / steps ) ) *
                        ( target.x - current.x );
    current.y =
        current.y + std::min( 1.f, smoothing_alpha * ( delta / steps ) ) *
                        ( target.y - current.y );
    current.z =
        current.z + std::min( 1.f, smoothing_alpha * ( delta / steps ) ) *
                        ( target.z - current.z );
}
...
BOOL CFPCamEffector::ProcessCam( SCamEffectorInfo& info ) {
    ...
    Fmatrix temp;
    temp.identity()
        .setHPB( m_HPB.x, m_HPB.y, m_HPB.z )
        .translate_over( m_Position );
