...
void SHeliMovementState::Load( LPCSTR section ) {
    float angularSpeedPitch =
        pSettings->r_float( section, "path_angular_sp_pitch" );
    AngSP = angularSpeedPitch;
    float angularSpeedHeading =
        pSettings->r_float( section, "path_angular_sp_heading" );
...
float SHeliMovementState::GetAngSpeedHeading( float speed ) {
    if ( isAdnAcc == 0 )
        ...
        return AngSH /
               ( 2 * HeadingSpB - AngSH + speed * ( HeadingSpB - AngSH ) / 2 );
...
void SHeliMovementState::getPathAltitude( Fvector& point,
                                          float base_altitude ) {
    ...
    Level().ObjectSpace.RayPick( point, down_dir, boundSize.y + 1.0f,
                                 collide::rqtStatic, cR, NULL );
...
void SHeliMovementState::goPatrolByPatrolPath( LPCSTR path_name,
                                               int start_idx ) {
    if ( need_to_del_path && currPatrolPath ) {
        CPatrolPath* tmp = const_cast< CPatrolPath* >( currPatrolPath );
        xr_delete( tmp );
    ...
    currPatrolPath =
        ai().patrol_paths().safe_path( patrol_path_name, false, true );
    need_to_del_path = false;
...
void SHeliMovementState::goByRoundPath( Fvector center_,
                                        float radius_,
                                        bool clockwise_ ) {
    if ( type == eMovRoundPath )
    ...
    if ( r_verify > radius_ ) {
#ifndef MASTER_GOLD
        Msg( "! Helicopter: cannot build round path R=%f. Min R=%f", radius_,
             r_verify );
    ...
    for ( ; it != it_e; ++it, ++pt_idx ) {
        ...
        CPatrolPoint pt = CPatrolPoint(
            ( CLevelGraph* )0, ( CGameLevelCrossTable* )0, ( CGameGraph* )0, pp,
            ( *it ).point, u32( -1 ), 0, pt_name );
    ...
    float stop_path =
        curLinearSpeed * stop_t - LinearAcc_bk * stop_t * stop_t / 2.0f;
...
void SHeliMovementState::SetPointFlags( u32 idx, u32 new_flags ) {
    ...
    CPatrolPoint* pt_new = xr_new< CPatrolPoint >(
        ( CLevelGraph* )0, ( CGameLevelCrossTable* )0, ( CGameGraph* )0,
        currPatrolPath, pt_curr->position(), u32( -1 ), new_flags,
        pt_curr->name() );
...
float SHeliMovementState::GetSpeedInDestPoint() {
    if ( need_to_del_path && currPatrolVertex &&
         currPatrolVertex->data().flags() )
        return 0.0f;
    else
...
void CHelicopter::OnRender() {
    /*
            ...
            CPatrolPath::const_vertex_iterator b =
       m_movement.currPatrolPath->vertices().begin();
            CPatrolPath::const_vertex_iterator e =
       m_movement.currPatrolPath->vertices().end(); for ( ; b != e; ++b) {
                    Fvector p = (*b).second->data().position();
                    Level().debug_renderer().draw_aabb
       (p,0.1f,0.1f,0.1f,D3DCOLOR_XRGB(0,255,0));
            }
    */
    /*
