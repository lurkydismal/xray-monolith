...
// PhraseDialogManager
void CAI_PhraseDialogManager::ReceivePhrase(
...
void CAI_PhraseDialogManager::AnswerPhrase( DIALOG_SHARED_PTR& phrase_dialog ) {
    ...
    THROW( pInvOwner );
    CGameObject* pOthersGO =
        smart_cast< CGameObject* >( phrase_dialog->OurPartner( this ) );
    THROW( pOthersGO );
    ...
    if ( !phrase_dialog->IsFinished() ) {
        CHARACTER_GOODWILL attitude =
            RELATION_REGISTRY().GetAttitude( pOthersIO, pInvOwner );
        ...
        for ( u32 i = 0; i < phrase_dialog->PhraseList().size(); ++i ) {
            phrase_goodwill =
                phrase_dialog->PhraseList()[ phrase_num ]->GoodwillLevel();
        ...
        shared_str phrase_id =
            phrase_dialog->PhraseList()[ phrase_num ]->GetID();

        CUIGameSP* pGameSP = smart_cast< CUIGameSP* >( CurrentGameUI() );
        pGameSP->TalkMenu->AddAnswer( phrase_dialog->GetPhraseText( phrase_id ),
                                      pInvOwner->Name() );
...
void CAI_PhraseDialogManager::SetDefaultStartDialog(
...
void CAI_PhraseDialogManager::UpdateAvailableDialogs(
