...
base::base( CGameObject* object,
    ...
    animation_movement_controller const* controller =
        m_object.animation_movement();
    if ( !controller )
...
void base::aim_at_position( Fvector const& bone_position,
    ...
    VERIFY2( _valid( bone_position ),
             make_string( "[%f][%f][%f]", VPUSH( bone_position ) ) );
    VERIFY2( _valid( object_position ),
             make_string( "[%f][%f][%f]", VPUSH( object_position ) ) );
    VERIFY2( _valid( object_direction ),
             make_string( "[%f][%f][%f]", VPUSH( object_direction ) ) );
    VERIFY2( _valid( m_target ),
             make_string( "[%f][%f][%f]", VPUSH( m_target ) ) );
    ...
    Fvector const current_point =
        Fvector().mad( object_position, object_direction, offset );
    ...
    VERIFY2( _valid( plane ),
             make_string( "[%f][%f][%f] [%f][%f][%f] [%f][%f][%f] %f",
                          VPUSH( circle_center ), VPUSH( direction_target ),
                          VPUSH( plane.n ), plane.d ) );
    Fvector projection;
    plane.project( projection, current_point );
    VERIFY( _valid( projection ) );

    Fvector projection2circle_center =
        Fvector().sub( projection, circle_center );
    ...
    Fvector const center2projection_direction =
        projection2circle_center.normalize();
    ...
    Fmatrix transform0;
    {
        ...
        if ( !fis_zero( sin_alpha ) ) {
            float cos_alpha = clampr(
                current_direction.dotproduct( target_direction ), -1.f, 1.f );
            transform0.rotation( cross_product.div( sin_alpha ),
                                 atan2f( sin_alpha, cos_alpha ) );
            VERIFY( _valid( transform0 ) );
        } else {
            float const dot_product = clampr(
                current_direction.dotproduct( target_direction ), -1.f, 1.f );
            if ( fsimilar( _abs( dot_product ), 0.f ) )
                ...
                cross_product.crossproduct( current_direction,
                                            direction_target );
                float const sin_alpha2 =
                    clampr( cross_product.magnitude(), -1.f, 1.f );
                if ( !fis_zero( sin_alpha2 ) ) {
                    transform0.rotation( cross_product.div( sin_alpha2 ),
                                         dot_product > 0.f ? 0.f : PI );
                    VERIFY( _valid( transform0 ) );
                } else {
                    transform0.rotation( Fvector().set( 0.f, 0.f, 1.f ),
                                         dot_product > 0.f ? 0.f : PI );
                    VERIFY( _valid( transform0 ) );
                }
            }
    ...
    Fmatrix transform1;
    {
        ...
        if ( !fis_zero( sin_alpha ) ) {
            float const cos_alpha =
                clampr( old_direction.dotproduct( new_direction ), -1.f, 1.f );
