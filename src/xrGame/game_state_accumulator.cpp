...
void game_state_accumulator::init_player( game_PlayerState* local_player ) {
    ...
    game_cl_Deathmatch* tmp_dm_game =
        smart_cast< game_cl_Deathmatch* >( &Game() );
    if ( tmp_dm_game ) {
        tmp_trade_wnd =
            smart_cast< CUIMpTradeWnd* >( tmp_dm_game->GetBuyWnd() );
    } else {
...
void game_state_accumulator::OnWeapon_Fire( u16 sender, u16 sender_weapon_id ) {
    for ( accumulative_values_collection_t::iterator
              i = m_accumulative_values.begin(),
...
void game_state_accumulator::OnBullet_Fire( u16 sender,
    ...
    for ( accumulative_values_collection_t::iterator
              ...
              ie = m_accumulative_values.end();
          i != ie; ++i ) {
        i->second->OnBullet_Fire( sender, sender_weapon_id, position,
                                  direction );
    }
}

void game_state_accumulator::OnBullet_Hit( CObject const* hitter,
    ...
    m_hits.add_hit( hitter->cName(), victim->cName(), get_object_id( weapon ),
                    bone, bullet_fly_dist );
    for ( accumulative_values_collection_t::iterator
              i = m_accumulative_values.begin(),
              ie = m_accumulative_values.end();
          i != ie; ++i ) {
...
void game_state_accumulator::OnPlayerTakeArtefact(
...
void game_state_accumulator::OnPlayerDropArtefact(
    game_PlayerState const* ps ) {
    for ( accumulative_values_collection_t::iterator
              i = m_accumulative_values.begin(),
              ie = m_accumulative_values.end();
...
void game_state_accumulator::OnPlayerBringArtefact(
    game_PlayerState const* ps ) {
    for ( accumulative_values_collection_t::iterator
              i = m_accumulative_values.begin(),
              ie = m_accumulative_values.end();
...
void game_state_accumulator::OnPlayerKilled(
    ...
    if ( killer_obj && victim_obj ) {
        m_kills.add_kill( killer_obj->cName(), victim_obj->cName(),
                          get_object_id( weapon_id ), kill_type.first,
                          kill_type.second );
    }
    ...
    struct hit_fetcher {
        bool operator()( shared_str const& hitter,
            ...
            if ( !m_owner->is_item_in_group(
                     hit.m_weapon_id, ammunition_group::gid_sniper_rifels ) )
                return false;
...
bool game_state_accumulator::check_accumulative_value(
    ...
    float right_arg ) {
    accumulative_values_collection_t::iterator tmp_iter =
        m_accumulative_values.find( param_id );
    VERIFY2( tmp_iter != m_accumulative_values.end(),
             make_string( "accumulative parameter %d not found", param_id )
                 .c_str() );
...
bool game_state_accumulator::is_item_in_group(
    u16 item_id,
    ammunition_group::enum_group_id gid ) {
    return m_amm_groups.is_item_in_group( item_id, gid );
}

bool game_state_accumulator::is_bone_in_group( u16 bone_id,
                                               bone_group::enum_group_id gid ) {
...
u16 game_state_accumulator::get_active_weapon_of_player(
    ...
    CInventoryItem const* tmp_inv_item =
        tmp_active_slot != NO_ACTIVE_SLOT
...
bool game_state_accumulator::is_enemies(
    game_PlayerState const* left_player,
