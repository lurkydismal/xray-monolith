...
IC void CGameVertexPathManager::setup( const _Graph* _graph,
...
TEMPLATE_SPECIALIZATION
IC bool CGameVertexPathManager::is_accessible(
    ...
    for ( ; I != E; ++I )
        if ( graph->mask( ( *I ).tMask,
                          graph->vertex( vertex_id )->vertex_type() ) )
            return ( true );
