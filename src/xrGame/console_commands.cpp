...
#define CrosshairBaseCommands( crosshair, suffix )                             \
    CMD3( CCC_Mask, Concat2( g_crosshair_, suffix ), &crosshair.flags,         \
          CROSSHAIR_SHOW );                                                    \
    CMD3( CCC_Mask, Concat3( g_crosshair_, suffix, _recon ), &crosshair.flags, \
          CROSSHAIR_RECON );                                                   \
    CMD4( CCC_Float, Concat3( g_crosshair_, suffix, _recon_max_opacity ),      \
          &crosshair.recon_max_opacity, 0.f, 1.f );                            \
    CMD3( CCC_Mask, Concat3( g_crosshair_, suffix, _use_shader ),              \
          &crosshair.flags, CROSSHAIR_USE_SHADER );                            \
    CMD3( CCC_String, Concat3( g_crosshair_, suffix, _shader ),                \
          crosshair.shader, 32 );                                              \
    CMD3( CCC_String, Concat3( g_crosshair_, suffix, _texture ),               \
          crosshair.texture, 32 );                                             \
    CMD4( CCC_Float, Concat3( g_crosshair_, suffix, _size ), &crosshair.size,  \
          1.f, 64.f );                                                         \
    CMD4( CCC_Float, Concat3( g_crosshair_, suffix, _depth ),                  \
          &crosshair.depth, 0.f, 300.f );                                      \
    CMD2( CCC_Color, Concat3( g_crosshair_, suffix, _color ),                  \
          &crosshair.color );

#define CrosshairDistanceCommands( crosshair, suffix )                     \
    CMD3( CCC_Mask, Concat3( g_crosshair_, suffix, _distance_lerp ),       \
          &crosshair.flags, CROSSHAIR_DISTANCE_LERP );                     \
    CMD4( CCC_Float, Concat3( g_crosshair_, suffix, _distance_lerp_rate ), \
          &crosshair.distance_lerp_rate, 1.f, 100.f );

#define CrosshairOpacityCommands( crosshair, suffix )                       \
    CMD4( CCC_Float, Concat3( g_crosshair_, suffix, _occluded_opacity ),    \
          &crosshair.occluded_opacity, 0.f, 1.f );                          \
    CMD4( CCC_Float, Concat3( g_crosshair_, suffix, _occlusion_fade_rate ), \
          &crosshair.occlusion_fade_rate, 1.f, 100.f );

#define CrosshairLineCommands( crosshair, suffix )                            \
    CMD3( CCC_Mask, Concat3( g_crosshair_, suffix, _line ), &crosshair.flags, \
          CROSSHAIR_LINE );
...
xr_token dead_body_collision_tokens[] = { { "full", 2 },
                                          { "actor_only", 1 },
                                          { "off", 0 },
                                          { 0, 0 } };
...
static void full_memory_stats( bool assert = true ) {
    ...
    int _eco_strings = ( int )g_pStringContainer->stat_economy(
        _eco_strings_count, _eco_strings_unique_count );
    ...
    Msg( "* [x-ray]: process heap[%u K], game lua[%d K], render[%d K]",
         _process_heap / 1024, _game_lua / 1024, _render / 1024 );
#endif // SEVERAL_ALLOCATORS

    Msg( "* [x-ray]: shared strings: memory[%ld K], count[%lu], unique[%lu]",
         _eco_strings / 1024, _eco_strings_count, _eco_strings_unique_count );
    ...
    CInifile::GetCacheStats( DLTX_files_cached, DLTX_total_bytes,
                             DLTX_section_count );
    Msg( "* [x-ray]: DLTX Cache: Files Cached: %zu, Sections Total %zu, Usage: "
         "%.2f MB",
         DLTX_files_cached, DLTX_section_count,
         ( double )DLTX_total_bytes / 1024 / 1024 );
...
#ifdef FS_DEBUG
    Msg( "* [x-ray]: file mapping: memory[%d K], count[%d]",
         g_file_mapped_memory / 1024, g_file_mapped_count );
    dump_file_mappings();
#endif // DEBUG
}
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        if ( g_mem_stats_async_in_progress.exchange(
                 true, std::memory_order_acq_rel ) ) {
            Msg( "* [x-ray]: stat_memory_async is already running" );
            return;
        }

        thread_spawn( &mem_stats_async_thread, "stat_memory_async", 0,
                      nullptr );
...
public:
    ...
    virtual void Execute( LPCSTR args ) { shared_string_dump(); }
};
...
public:
    CCC_GameDifficulty( LPCSTR N )
        : CCC_Token( N,
                     ( u32* )&g_SingleGameDifficulty,
                     difficulty_type_token ) {};
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( g_pGameLevel && Level().game ) {
            ...
            game_cl_Single* game =
                smart_cast< game_cl_Single* >( Level().game );
            VERIFY( game );
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        if ( !ai().get_level_graph() )
            ...
            if ( ( -1 != id1 ) && ( -1 != id2 ) )
                if ( _max( id1, id2 ) >
                     ( int )ai().game_graph().header().vertex_count() - 1 )
                    Msg( "! there are only %d vertexes!",
                         ai().game_graph().header().vertex_count() );
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        if ( ( GameID() == eGameIDSingle ) && ai().get_alife() ) {
            game_sv_Single* tpGame =
                smart_cast< game_sv_Single* >( Level().Server->game );
            VERIFY( tpGame );
            int id1 = 0;
            sscanf( args, "%d", &id1 );
            if ( id1 < 1 )
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( !( iss >> arg1 ) ) {
            Msg( "not enough parameters. e.g. demo_record_return_ctrl_inputs "
                 "[filename] [boundary]" );
            return;
        }
        ...
        auto pDemoRecord =
            xr_new< CDemoRecord >( fn, &pDemoRecords, FALSE, life_time, TRUE );
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( 0 == g_pGameLevel ) {
            ...
            g_pGameLevel->Cameras().AddCamEffector(
                xr_new< CDemoPlay >( fn, 1.0f, loops ) );
        }
    }
};
...
public:
    CCC_FPDDirectionOffset( LPCSTR N )
        : CCC_Vector3( N,
                       &d,
                       Fvector().set( -FLT_MAX, -FLT_MAX, -FLT_MAX ),
                       Fvector().set( FLT_MAX, FLT_MAX, FLT_MAX ) ) {};
...
public:
    CCC_FPDPositionOffset( LPCSTR N )
        : CCC_Vector3( N,
                       &d,
                       Fvector().set( -FLT_MAX, -FLT_MAX, -FLT_MAX ),
                       Fvector().set( FLT_MAX, FLT_MAX, FLT_MAX ) ) {};
...
void get_files_list( xr_vector< shared_str >& files,
                     LPCSTR dir,
                     LPCSTR file_ext,
    ...
    // Tooltips call this every frame while typing. Avoid forcing a full
    // file-system rescan each frame by caching results for a short period.
    constexpr u32 kCacheLifetimeMs = 10000;

    const bool same_request = ( cache.dir == dir ) && ( cache.ext == file_ext );
    const bool cache_valid =
        !force_rescan && same_request &&
        ( Device.dwTimeGlobal >= cache.updated_at ) &&
    ...
    for ( ; itb != ite; ++itb ) {
        ...
        string_path fn;
        strncpy_s( fn, sizeof( fn ), fn_ext,
                   xr_strlen( fn_ext ) - len_str_ext );
        files.push_back( fn );
    }
...
#ifdef DEBUG
        Msg( "Game save overhead  : %f milliseconds",
             timer.GetElapsed_sec() * 1000.f );
#endif
        StaticDrawableWrapper* _s =
            CurrentGameUI()->AddCustomStatic( "game_saved", true );
        LPSTR save_name;
        STRCONCAT( save_name,
                   CStringTable().translate( "st_game_saved" ).c_str(), ": ",
                   S );
...
#ifdef DEBUG
        Msg( "Screenshot overhead : %f milliseconds",
             timer.GetElapsed_sec() * 1000.f );
#endif
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( !CSavedGameWrapper::valid_saved_game( saved_game ) ) {
            Msg( "! Cannot load saved game %s, version mismatch or saved game "
                 "is corrupted",
                 saved_game );
            return;
        }
        ...
        game_sv_Single			*game =
        smart_cast<game_sv_Single*>(Level().Server->game); R_ASSERT
        (game); game->restart_simulator	(saved_game);
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( !*g_last_saved_game ) {
            Msg( "! cannot load last saved game since it hasn't been "
                 "specified" );
            return;
        }
        ...
        if ( !valid_saved_game_name( g_last_saved_game ) ) {
            Msg( "! Cannot load saved game %s, invalid file name",
                 g_last_saved_game );
            return;
        }
        ...
        STRCONCAT( command, "start server(", g_last_saved_game,
                   "/single/alife/load)" );
        Console->Execute( command );
    }
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        const GameGraph::SLevel* level =
            ai().game_graph().header().level( args, true );
        if ( !level ) {
...
public:
    ...
    virtual void Info( TInfo& I ) {
        if ( strstr( cName, "script_debug_break" ) == cName )
            ...
            xr_strcpy( I,
                       "restarts script debugger or start if no script "
                       "debugger presents" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        for ( ; I != E; ++I ) {
            CSE_ALifeCreatureAbstract* obj =
                smart_cast< CSE_ALifeCreatureAbstract* >( I->second );
            if ( obj ) {
...
struct CCC_JumpToLevel : public IConsole_Command {
    ...
    virtual void Execute( LPCSTR level ) {
        ...
        GameGraph::LEVEL_MAP::const_iterator I =
            ai().game_graph().header().levels().begin();
        GameGraph::LEVEL_MAP::const_iterator E =
            ai().game_graph().header().levels().end();
        for ( ; I != E; ++I )
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        ...
        GameGraph::LEVEL_MAP::const_iterator itb =
            ai().game_graph().header().levels().begin();
        GameGraph::LEVEL_MAP::const_iterator ite =
            ai().game_graph().header().levels().end();
        for ( ; itb != ite; ++itb ) {
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        if ( !xr_strlen( args ) ) {
            ...
            FS.rescan_pathes();
            // run script
            if ( ai().script_engine().script_process(
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        if ( !xr_strlen( args ) )
            ...
            xr_sprintf( S, "%s\n", args );
            int l_iErrorCode =
                luaL_loadbuffer( ai().script_engine().lua(), S, xr_strlen( S ),
                                 "@console_command" );
            if ( !l_iErrorCode ) {
                l_iErrorCode = lua_pcall( ai().script_engine().lua(), 0, 0, 0 );
                if ( l_iErrorCode ) {
                    ai().script_engine().print_output(
                        ai().script_engine().lua(), *m_script_name,
                        l_iErrorCode );
            ...
            ai().script_engine().print_output( ai().script_engine().lua(),
                                               *m_script_name, l_iErrorCode );
        }
    } // void	Execute

    virtual void Status( TStatus& S ) {
        xr_strcpy(
            S, "<script_name.function()> (Specify script and function name!)" );
    }
...
public:
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        TStatus str;
        xr_sprintf( str, sizeof( str ), "%3.3f  (current)  [0.001 - 1000.0]",
                    Device.time_factor() );
        tips.push_back( str );
        IConsole_Command::fill_tips( tips, mode );
    }
...
public:
    CCC_DiscordStatus( LPCSTR N, Flags32* V, u32 M ) : CCC_Mask( N, V, M ) {};
...
struct CCC_StartTimeSingle : public IConsole_Command {
    ...
    virtual void Execute( LPCSTR args ) {
        u32 year = 1, month = 1, day = 1, hours = 0, mins = 0, secs = 0,
            milisecs = 0;
        sscanf( args, "%d.%d.%d %d:%d:%d.%d", &year, &month, &day, &hours,
                &mins, &secs, &milisecs );
        year = _max( year, 1 );
        month = _max( month, 1 );
        day = _max( day, 1 );
        g_qwStartGameTime =
            generate_time( year, month, day, hours, mins, secs, milisecs );
    ...
    virtual void Status( TStatus& S ) {
        ...
        split_time( g_qwStartGameTime, year, month, day, hours, mins, secs,
                    milisecs );
        xr_sprintf( S, "%d.%d.%d %d:%d:%d.%d", year, month, day, hours, mins,
                    secs, milisecs );
    }
};

struct CCC_TimeFactorSingle : public CCC_Float {
    CCC_TimeFactorSingle( LPCSTR N,
                          float* V,
                          float _min = 0.f,
                          float _max = 1.f )
        : CCC_Float( N, V, _min, _max ) {};
...
#define CMD_RADIOGROUPMASK2( p1, p2, p3, p4, p5, p6 )         \
    {                                                         \
        ...
        static CCC_RadioGroupMask2 x##CCC_RadioGroupMask2(    \
            &x##CCC_RadioMask1, &x##CCC_RadioMask2 );         \
    }

struct CCC_DbgBullets : public CCC_Integer {
    CCC_DbgBullets( LPCSTR N, int* V, int _min = 0, int _max = 999 )
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( itm ) {
            ...
            if ( active_item && active_item->object().cNameSect() == ssss )
                CAttachableItem::m_dbgItem =
                    active_item->cast_attachable_item();
        }

        if ( CAttachableItem::m_dbgItem )
            Msg( "CCC_TuneAttachableItem switched to ON for [%s]", args );
        else
            Msg( "CCC_TuneAttachableItem cannot find attached item [%s]",
                 args );
    }

    virtual void Info( TInfo& I ) {
        xr_sprintf( I,
                    "allows to change bind rotation and position offsets for "
                    "attached item, <section_name> given as arguments" );
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR arguments ) {
        ...
        if ( !FS.exist( arguments ) && !FS.exist( fn, "$level$", name ) &&
             !FS.exist( fn, "$game_meshes$", name ) ) {
        ...
        if ( !kinematics ) {
            ...
            Msg( "! Invalid visual type \"%s\" (not a IKinematics)",
                 arguments );
            return;
        }
...
public:
    CCC_Net_SV_GuaranteedPacketMode( LPCSTR N,
                                     int* V,
                                     int _min = 0,
                                     int _max = 2 )
        : CCC_Integer( N, V, _min, _max ), value_blin( V ) {};
...
public:
    CCC_DbgVar( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
...
public:
    CCC_DLTXCache( LPCSTR N ) : CCC_Integer( N, &dltx_use_cache, 0, 1 ) {};
...
public:
    CCC_MovePathQueryPosThreshold( LPCSTR N )
        : CCC_Float( N,
                     &movement_manager_move_along_path_query_pos_threshold,
                     0.f,
                     2.f ) {};
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        movement_manager_move_along_path_query_pos_threshold =
            std::atof( args );
        movement_manager_move_along_path_query_pos_threshold_sqr =
            movement_manager_move_along_path_query_pos_threshold *
            movement_manager_move_along_path_query_pos_threshold;
    }
};
...
public:
    ...
    virtual void Execute( LPCSTR args ) {
        ...
        if ( Level().ObjectSpace.RayPick(
                 Device.vCameraPosition, Device.vCameraDirection, 1000.f,
                 collide::rqtBoth, l_rq, Level().CurrentControlEntity() ) ) {
            int count = 1;
            string256 string;
            string[ 0 ] = 0;
            ...
            for ( int i = 0; i < count; ++i ) {
                ...
                Fvector::generate_orthonormal_basis_normalized( pos.k, pos.j,
                                                                pos.i );
                pos.c.set( Fvector( Device.vCameraPosition )
                               .add( Fvector( Device.vCameraDirection )
                                         .mul( l_rq.range ) ) );
    ...
    virtual void fill_tips( vecTips& tips, u32 mode ) {
        particles_systems::library_interface const& library =
            GamePersistent()
                .Environment()
                .m_pRender->particles_systems_library();
        for ( auto pi = library.vec_all_particles().begin();
              pi != library.vec_all_particles().end(); ++pi )
...
void CCC_RegisterCommands() {
    ...
    CMD1( CCC_ALifeSwitchDistance,
          "al_switch_distance" );                      // set switch distance
    ...
    CMD1( CCC_ALifeObjectsPerUpdate,
          "al_objects_per_update" ); // set process time
    ...
    CMD3( CCC_Mask, "ai_obstacles_avoiding_static", &psAI_Flags,
          aiObstaclesAvoidingStatic );
    CMD3( CCC_Mask, "ai_use_smart_covers", &psAI_Flags, aiUseSmartCovers );
    CMD3( CCC_Mask, "ai_use_smart_covers_animation_slots", &psAI_Flags,
          ( u32 )aiUseSmartCoversAnimationSlot );
    ...
    // demonized: GC step that is used for repeated calls on second thread while
    // frame is rendering, limit to small values
    CMD4( CCC_Integer, "lua_parallel_gcstep", &psLua_ParallelGCStep, 1, 100 );
    CMD4( CCC_Integer, "lua_parallel_gc_call_amount",
          &psLua_ParallelGC_CallAmount, 1, 50 );
    ...
    CMD3( CCC_Mask, "ai_draw_game_graph_stalkers", &psAI_Flags,
          aiDrawGameGraphStalkers );
    CMD3( CCC_Mask, "ai_draw_game_graph_objects", &psAI_Flags,
          aiDrawGameGraphObjects );
    CMD3( CCC_Mask, "ai_draw_game_graph_real_pos", &psAI_Flags,
          aiDrawGameGraphRealPos );

    CMD3( CCC_Mask, "ai_nil_object_access", &psAI_Flags, aiNilObjectAccess );

    CMD3( CCC_Mask, "ai_draw_visibility_rays", &psAI_Flags,
          aiDrawVisibilityRays );
    ...
    // side_sensitivity_threshold	= 0.2f; float
    // anim_channel_factor			= 3.f;

    CMD4( CCC_Float, "hit_anims_power", &ghit_anims_params.power_factor, 0.0f,
          100.0f );
    CMD4( CCC_Float, "hit_anims_rotational_power",
          &ghit_anims_params.rotational_power_factor, 0.0f, 100.0f );
    CMD4( CCC_Float, "hit_anims_side_sensitivity_threshold",
          &ghit_anims_params.side_sensitivity_threshold, 0.0f, 10.0f );
    CMD4( CCC_Float, "hit_anims_channel_factor",
          &ghit_anims_params.anim_channel_factor, 0.0f, 100.0f );
    // float	block_blend					= 0.1f;
    // float	reduce_blend				= 0.5f;
    // float	reduce_power_factor			= 0.5f;
    CMD4( CCC_Float, "hit_anims_block_blend", &ghit_anims_params.block_blend,
          0.f, 1.f );
    CMD4( CCC_Float, "hit_anims_reduce_blend", &ghit_anims_params.reduce_blend,
          0.f, 1.f );
    CMD4( CCC_Float, "hit_anims_reduce_blend_factor",
    ...
    CMD3( CCC_Mask, "debug_on_frame_gather_stats", &psAI_Flags,
          aiDebugOnFrameAllocs );
    CMD4( CCC_Float, "debug_on_frame_gather_stats_frequency",
          &debug_on_frame_gather_stats_frequency, 0.f, 1.f );
    ...
    CMD4( CCC_Integer, "ai_dbg_inactive_time", &g_AI_inactive_time, 0,
          1000000 );
    ...
    CMD4( CCC_FloatBlock, "ph_break_common_factor",
          &ph_console::phBreakCommonFactor, 0.f, 1000000000.f );
    CMD4( CCC_FloatBlock, "ph_rigid_break_weapon_factor",
          &ph_console::phRigidBreakWeaponFactor, 0.f, 1000000000.f );
    CMD4( CCC_Integer, "ph_tri_clear_disable_count",
          &ph_console::ph_tri_clear_disable_count, 0, 255 );
    CMD4( CCC_FloatBlock, "ph_tri_query_ex_aabb_rate",
          &ph_console::ph_tri_query_ex_aabb_rate, 1.01f, 3.f );
    ...
    CMD4( CCC_Float, "g_legs_spine_offset_y", &legs_spine_offset_y, -1.0f,
          1.0f );
    ...
    CMD4( CCC_Integer, "g_legs_attach_to_camera", &legs_attach_to_camera, 0,
          1 );
    CMD4( CCC_Integer, "g_legs_render_attachments_shadow",
          &legs_render_attachments_shadow, 0, 1 );

    CMD4( CCC_Integer, "r__actor_shadow_in_demo_record",
          &r__actor_shadow_in_demo_record, 0, 1 );

    CMD4( CCC_Integer, "g_enemy_manager_useful_cache_time",
          &enemy_manager_useful_cache_time, -1, 500 );

    CMD4( CCC_Integer, "ai_enemy_inertia_time_to_somebody",
          ( int* )&ENEMY_INERTIA_TIME_TO_SOMEBODY, 0, 120000 );
    CMD4( CCC_Integer, "ai_enemy_inertia_time_to_actor",
          ( int* )&ENEMY_INERTIA_TIME_TO_ACTOR, 0, 120000 );
    CMD4( CCC_Integer, "ai_enemy_inertia_time_from_actor",
          ( int* )&ENEMY_INERTIA_TIME_FROM_ACTOR, 0, 120000 );
    CMD4( CCC_Integer, "ai_search_inertia_time",
          ( int* )&ENEMY_INERTIA_TIME_SEARCH, 0, 120000 );

    CMD4( CCC_Integer, "ai_hold_position_inertia_base",
          &g_ai_hold_position_inertia_base, 0, 120000 );
    CMD4( CCC_Integer, "ai_hold_position_inertia_random",
          &g_ai_hold_position_inertia_random, 0, 120000 );
    CMD4( CCC_Integer, "ai_grenade_throw_delay_base",
          &g_ai_grenade_throw_delay_base, 0, 10000 );
    CMD4( CCC_Integer, "ai_grenade_throw_delay_step",
          &g_ai_grenade_throw_delay_step, 0, 10000 );

    CMD4( CCC_Integer, "ai_aim_inertia_time", ( int* )&g_ai_aim_inertia_time, 0,
          10000 );
    CMD4( CCC_Integer, "ai_aim_queue_inertia_time",
          ( int* )&g_ai_aim_queue_inertia_time, 0, 10000 );
    // Danger perception multipliers: 1.0 = original engine behaviour, 0.0 =
    // disabled, 5.0 = 5x urgency
    extern float g_ai_danger_ricochet_mult;
    ...
    CMD4( CCC_Float, "ai_danger_ricochet_mult", &g_ai_danger_ricochet_mult, 0.f,
          5.f );
    CMD4( CCC_Float, "ai_danger_attack_sound_mult",
          &g_ai_danger_attack_sound_mult, 0.f, 5.f );
    CMD4( CCC_Float, "ai_danger_entity_attacked_mult",
          &g_ai_danger_entity_attacked_mult, 0.f, 5.f );
    CMD4( CCC_Float, "ai_danger_entity_death_mult",
          &g_ai_danger_entity_death_mult, 0.f, 5.f );
    CMD4( CCC_Float, "ai_danger_corpse_mult", &g_ai_danger_corpse_mult, 0.f,
          5.f );
    CMD4( CCC_Float, "ai_danger_attacked_mult", &g_ai_danger_attacked_mult, 0.f,
          5.f );
    CMD4( CCC_Float, "ai_danger_grenade_mult", &g_ai_danger_grenade_mult, 0.f,
          5.f );
    CMD4( CCC_Float, "ai_danger_enemy_sound_mult",
          &g_ai_danger_enemy_sound_mult, 0.f, 5.f );
    ...
    CMD4( CCC_Float, "ai_vision_speed_boost", &g_ai_vision_speed_boost, 0.1f,
          10.0f );
    CMD4( CCC_Float, "ai_reload_threshold", &g_ai_reload_threshold, 0.01f,
          1.0f );
    ...
    // 0 = fire only while enemy visible; default 10000ms; max 20000ms (20s is
    // already extreme)
    CMD4( CCC_Integer, "ai_fire_make_sense_interval",
          ( int* )&g_ai_fire_make_sense_interval, 0, 20000 );
    // extra metres past pick_distance where suppression fire still makes sense;
    // default 2.5m
    CMD4( CCC_Float, "ai_fire_range_extension", &g_ai_fire_range_extension, 0.f,
          10.f );
    // max vertical gap between NPC and enemy for fire to make sense;
    // default 2.0m (~2 floors max)
    CMD4( CCC_Float, "ai_fire_max_height_diff", &g_ai_fire_max_height_diff, 0.f,
          8.f );
    ...
    // cover danger zone radius/duration when NPC is shot in cover; default 3.0m
    // / 120s
    CMD4( CCC_Float, "ai_cover_danger_radius", &g_ai_cover_danger_radius, 0.f,
          15.f );
    CMD4( CCC_Integer, "ai_cover_danger_time", ( int* )&g_ai_cover_danger_time,
          0, 300000 );
    // cover danger zone when NPC abandons cover to flank; default 5.0m / 120s
    CMD4( CCC_Float, "ai_cover_detour_radius", &g_ai_cover_detour_radius, 0.f,
          15.f );
    CMD4( CCC_Integer, "ai_cover_detour_time", ( int* )&g_ai_cover_detour_time,
          0, 300000 );
    // cover danger zone on unknown threat (grenade, sound); default 5.0m / 120s
    CMD4( CCC_Float, "ai_cover_unknown_radius", &g_ai_cover_unknown_radius, 0.f,
          15.f );
    CMD4( CCC_Integer, "ai_cover_unknown_time",
          ( int* )&g_ai_cover_unknown_time, 0, 300000 );
    ...
    CMD4( CCC_Float, "ai_close_move_distance", &g_ai_close_move_distance, 0.f,
          6.f );
    ...
    CMD4( CCC_Float, "ai_cover_search_near_radius",
          &g_ai_cover_search_near_radius, 1.f, 30.f );
    CMD4( CCC_Float, "ai_cover_search_far_radius",
          &g_ai_cover_search_far_radius, 1.f, 80.f );
    // min 3.0 = MIN_SUITABLE_ENEMY_DISTANCE; below that the cover evaluator
    // gets degenerate inputs
    CMD4( CCC_Float, "ai_cover_pistol_max_dist", &g_ai_cover_pistol_max_dist,
          3.f, 50.f );
    CMD4( CCC_Float, "ai_cover_shotgun_max_dist", &g_ai_cover_shotgun_max_dist,
          3.f, 20.f );
    // 0.0 = use engine default min (3m); max 80m covers any realistic sniper
    // scenario
    CMD4( CCC_Float, "ai_cover_sniper_min_dist", &g_ai_cover_sniper_min_dist,
          0.f, 80.f );
    CMD4( CCC_Float, "ai_cover_default_max_dist", &g_ai_cover_default_max_dist,
          3.f, 50.f );
    // 0 = randomize every LookOut init; default 5000ms; max 12000ms (~12s is
    // very cautious)
    CMD4( CCC_Integer, "ai_crouch_look_out_delta",
          ( int* )&g_ai_crouch_look_out_delta, 0, 12000 );
    // 0 = leave immediately; default 30000ms (30s); max 120000ms (2min)
    CMD4( CCC_Integer, "ai_wait_in_smart_cover_time",
          ( int* )&g_ai_wait_in_smart_cover_time, 0, 120000 );

    CMD3( CCC_Mask, "g_firepos", &psActorFlags, AF_FIREPOS );
    CMD3( CCC_Mask, "g_firepos_zoom", &psActorFlags, AF_FIREPOS_ZOOM );
    CMD3( CCC_Mask, "g_firedir_third_person", &psActorFlags,
          AF_FIREDIR_THIRD_PERSON );
    ...
    CMD3( CCC_Mask, "g_crosshair_show_always", &psCrosshair_Flags,
          CROSSHAIR_SHOW_ALWAYS );
    CMD3( CCC_Mask, "g_crosshair_independent", &psCrosshair_Flags,
          CROSSHAIR_INDEPENDENT );
    ...
    CMD4( CCC_Integer, "g_decouple_horz_recoil", &g_decouple_horz_recoil, 0,
          1 );
    CMD4( CCC_Integer, "g_launcher_dynamic_range_zoom",
          &g_launcher_dynamic_range_zoom, 0, 1 );
    CMD4( CCC_Integer, "g_use_non_linear_inertia", &g_use_non_linear_inertia, 0,
          1 );

    extern XRPHYSICS_API BOOL g_clamp_actor_camera_collision;
    CMD4( CCC_Integer, "g_clamp_actor_camera_collision",
          &g_clamp_actor_camera_collision, 0, 1 );
    ...
    CMD3( CCC_Mask, "dbg_draw_actor_alive", &dbg_net_Draw_Flags,
          dbg_draw_actor_alive );
    CMD3( CCC_Mask, "dbg_draw_actor_dead", &dbg_net_Draw_Flags,
          dbg_draw_actor_dead );
    CMD3( CCC_Mask, "dbg_draw_customzone", &dbg_net_Draw_Flags,
          dbg_draw_customzone );
    CMD3( CCC_Mask, "dbg_draw_teamzone", &dbg_net_Draw_Flags,
          dbg_draw_teamzone );
    CMD3( CCC_Mask, "dbg_draw_invitem", &dbg_net_Draw_Flags, dbg_draw_invitem );
    CMD3( CCC_Mask, "dbg_draw_actor_phys", &dbg_net_Draw_Flags,
          dbg_draw_actor_phys );
    CMD3( CCC_Mask, "dbg_draw_customdetector", &dbg_net_Draw_Flags,
          dbg_draw_customdetector );
    CMD3( CCC_Mask, "dbg_destroy", &dbg_net_Draw_Flags, dbg_destroy );
    CMD3( CCC_Mask, "dbg_draw_autopickupbox", &dbg_net_Draw_Flags,
          dbg_draw_autopickupbox );
    CMD3( CCC_Mask, "dbg_draw_rp", &dbg_net_Draw_Flags, dbg_draw_rp );
    CMD3( CCC_Mask, "dbg_draw_climbable", &dbg_net_Draw_Flags,
          dbg_draw_climbable );
    CMD3( CCC_Mask, "dbg_draw_skeleton", &dbg_net_Draw_Flags,
          dbg_draw_skeleton );

    CMD3( CCC_Mask, "dbg_draw_ph_contacts", &ph_dbg_draw_mask,
          phDbgDrawContacts );
    CMD3( CCC_Mask, "dbg_draw_ph_enabled_aabbs", &ph_dbg_draw_mask,
          phDbgDrawEnabledAABBS );
    CMD3( CCC_Mask, "dbg_draw_ph_intersected_tries", &ph_dbg_draw_mask,
          phDBgDrawIntersectedTries );
    CMD3( CCC_Mask, "dbg_draw_ph_saved_tries", &ph_dbg_draw_mask,
          phDbgDrawSavedTries );
    CMD3( CCC_Mask, "dbg_draw_ph_tri_trace", &ph_dbg_draw_mask,
          phDbgDrawTriTrace );
    CMD3( CCC_Mask, "dbg_draw_ph_positive_tries", &ph_dbg_draw_mask,
          phDBgDrawPositiveTries );
    CMD3( CCC_Mask, "dbg_draw_ph_negative_tries", &ph_dbg_draw_mask,
          phDBgDrawNegativeTries );
    CMD3( CCC_Mask, "dbg_draw_ph_tri_test_aabb", &ph_dbg_draw_mask,
          phDbgDrawTriTestAABB );
    CMD3( CCC_Mask, "dbg_draw_ph_tries_changes_sign", &ph_dbg_draw_mask,
          phDBgDrawTriesChangesSign );
    CMD3( CCC_Mask, "dbg_draw_ph_tri_point", &ph_dbg_draw_mask,
          phDbgDrawTriPoint );
    CMD3( CCC_Mask, "dbg_draw_ph_explosion_position", &ph_dbg_draw_mask,
          phDbgDrawExplosionPos );
    CMD3( CCC_Mask, "dbg_draw_ph_statistics", &ph_dbg_draw_mask,
          phDbgDrawObjectStatistics );
    CMD3( CCC_Mask, "dbg_draw_ph_mass_centres", &ph_dbg_draw_mask,
          phDbgDrawMassCenters );
    CMD3( CCC_Mask, "dbg_draw_ph_death_boxes", &ph_dbg_draw_mask,
          phDbgDrawDeathActivationBox );
    CMD3( CCC_Mask, "dbg_draw_ph_hit_app_pos", &ph_dbg_draw_mask,
          phHitApplicationPoints );
    CMD3( CCC_Mask, "dbg_draw_ph_cashed_tries_stats", &ph_dbg_draw_mask,
          phDbgDrawCashedTriesStat );
    CMD3( CCC_Mask, "dbg_draw_ph_car_dynamics", &ph_dbg_draw_mask,
          phDbgDrawCarDynamics );
    CMD3( CCC_Mask, "dbg_draw_ph_car_plots", &ph_dbg_draw_mask,
          phDbgDrawCarPlots );
    CMD3( CCC_Mask, "dbg_ph_ladder", &ph_dbg_draw_mask, phDbgLadder );
    CMD3( CCC_Mask, "dbg_draw_ph_explosions", &ph_dbg_draw_mask,
          phDbgDrawExplosions );
    CMD3( CCC_Mask, "dbg_draw_car_plots_all_trans", &ph_dbg_draw_mask,
          phDbgDrawCarAllTrnsm );
    CMD3( CCC_Mask, "dbg_draw_ph_zbuffer_disable", &ph_dbg_draw_mask,
          phDbgDrawZDisable );
    CMD3( CCC_Mask, "dbg_ph_obj_collision_damage", &ph_dbg_draw_mask,
          phDbgDispObjCollisionDammage );
    CMD_RADIOGROUPMASK2( "dbg_ph_ai_always_phmove", &ph_dbg_draw_mask,
                         phDbgAlwaysUseAiPhMove, "dbg_ph_ai_never_phmove",
    ...
    CMD3( CCC_Mask, "dbg_ph_character_control", &ph_dbg_draw_mask,
          phDbgCharacterControl );
    CMD3( CCC_Mask, "dbg_draw_ph_ray_motions", &ph_dbg_draw_mask,
          phDbgDrawRayMotions );
    CMD4( CCC_Float, "dbg_ph_vel_collid_damage_to_display",
          &dbg_vel_collid_damage_to_display, 0.f, 1000.f );
    CMD4( CCC_DbgBullets, "dbg_draw_bullet_hit", &g_bDrawBulletHit, 0, 1 );
    CMD4( CCC_Integer, "dbg_draw_fb_crosshair", &g_bDrawFirstBulletCrosshair, 0,
          1 );
    CMD1( CCC_DbgPhTrackObj, "dbg_track_obj" );
    CMD3( CCC_Mask, "dbg_ph_actor_restriction", &ph_dbg_draw_mask1,
          ph_m1_DbgActorRestriction );
    CMD3( CCC_Mask, "dbg_draw_ph_hit_anims", &ph_dbg_draw_mask1,
          phDbgHitAnims );
    CMD3( CCC_Mask, "dbg_draw_ph_ik_limits", &ph_dbg_draw_mask1,
          phDbgDrawIKLimits );
    CMD3( CCC_Mask, "dbg_draw_ph_ik_predict", &ph_dbg_draw_mask1,
          phDbgDrawIKPredict );
    CMD3( CCC_Mask, "dbg_draw_ph_ik_collision", &ph_dbg_draw_mask1,
          phDbgDrawIKCollision );
    CMD3( CCC_Mask, "dbg_draw_ph_ik_shift_object", &ph_dbg_draw_mask1,
          phDbgDrawIKSHiftObject );
    CMD3( CCC_Mask, "dbg_draw_ph_ik_blending", &ph_dbg_draw_mask1,
          phDbgDrawIKBlending );
    ...
    CMD4( CCC_Integer, "dbg_draw_character_bones", &dbg_draw_character_bones,
          FALSE, TRUE );
    CMD4( CCC_Integer, "dbg_draw_character_physics",
          &dbg_draw_character_physics, FALSE, TRUE );
    CMD4( CCC_Integer, "dbg_draw_character_binds", &dbg_draw_character_binds,
          FALSE, TRUE );
    CMD4( CCC_Integer, "dbg_draw_character_physics_pones",
          &dbg_draw_character_physics_pones, FALSE, TRUE );

    CMD4( CCC_Integer, "ik_cam_shift", &ik_cam_shift, FALSE, TRUE );

    extern float ik_cam_shift_tolerance;
    CMD4( CCC_Float, "ik_cam_shift_tolerance", &ik_cam_shift_tolerance, 0.f,
          2.f );
    ...
    CMD4(CCC_Integer,	"ik_allign_free_foot"
    ,&ik_allign_free_foot,	0,	1); CMD4(CCC_Integer,
    "ik_local_blending"				,&ik_local_blending,	0,
    1); CMD4(CCC_Integer,	"ik_blend_free_foot"
    ,&ik_blend_free_foot,	0,	1); CMD4(CCC_Integer,
    "ik_collide_blend"				,&ik_collide_blend,	0,
    1);
    */
    extern BOOL dbg_draw_ragdoll_spawn;
    CMD4( CCC_Integer, "dbg_draw_ragdoll_spawn", &dbg_draw_ragdoll_spawn, FALSE,
          TRUE );
    extern BOOL debug_step_info;
    extern BOOL debug_step_info_load;
    CMD4( CCC_Integer, "debug_step_info", &debug_step_info, FALSE, TRUE );
    CMD4( CCC_Integer, "debug_step_info_load", &debug_step_info_load, FALSE,
          TRUE );
    extern BOOL debug_character_material_load;
    CMD4( CCC_Integer, "debug_character_material_load",
          &debug_character_material_load, FALSE, TRUE );
    extern XRPHYSICS_API BOOL dbg_draw_camera_collision;
    CMD4( CCC_Integer, "dbg_draw_camera_collision", &dbg_draw_camera_collision,
          FALSE, TRUE );
    ...
    CMD4( CCC_FloatBlock, "camera_collision_character_shift_z",
          &camera_collision_character_shift_z, 0.f, 1.f );
    CMD4( CCC_FloatBlock, "camera_collision_character_skin_depth",
          &camera_collision_character_skin_depth, 0.f, 1.f );
    extern BOOL dbg_draw_animation_movement_controller;
    CMD4( CCC_Integer, "dbg_draw_animation_movement_controller",
          &dbg_draw_animation_movement_controller, FALSE, TRUE );
    ...
    */
    ...
    CMD3( CCC_Mask, "dbg_track_obj_blends_bp_0", &dbg_track_obj_flags,
          dbg_track_obj_blends_bp_0 );
    CMD3( CCC_Mask, "dbg_track_obj_blends_bp_1", &dbg_track_obj_flags,
          dbg_track_obj_blends_bp_1 );
    CMD3( CCC_Mask, "dbg_track_obj_blends_bp_2", &dbg_track_obj_flags,
          dbg_track_obj_blends_bp_2 );
    CMD3( CCC_Mask, "dbg_track_obj_blends_bp_3", &dbg_track_obj_flags,
          dbg_track_obj_blends_bp_3 );
    CMD3( CCC_Mask, "dbg_track_obj_blends_motion_name", &dbg_track_obj_flags,
          dbg_track_obj_blends_motion_name );
    CMD3( CCC_Mask, "dbg_track_obj_blends_time", &dbg_track_obj_flags,
          dbg_track_obj_blends_time );

    CMD3( CCC_Mask, "dbg_track_obj_blends_ammount", &dbg_track_obj_flags,
          dbg_track_obj_blends_ammount );
    CMD3( CCC_Mask, "dbg_track_obj_blends_mix_params", &dbg_track_obj_flags,
          dbg_track_obj_blends_mix_params );
    CMD3( CCC_Mask, "dbg_track_obj_blends_flags", &dbg_track_obj_flags,
          dbg_track_obj_blends_flags );
    CMD3( CCC_Mask, "dbg_track_obj_blends_state", &dbg_track_obj_flags,
          dbg_track_obj_blends_state );
    CMD3( CCC_Mask, "dbg_track_obj_blends_dump", &dbg_track_obj_flags,
          dbg_track_obj_blends_dump );

    CMD1( CCC_DbgVar, "dbg_var" );

    extern float dbg_text_height_scale;
    CMD4( CCC_FloatBlock, "dbg_text_height_scale", &dbg_text_height_scale, 0.2f,
          5.f );
#endif
    ...
    CMD4( CCC_Integer, "string_table_error_msg",
          &CStringTable::m_bWriteErrorsToLog, 0, 1 );

    CMD3( CCC_Mask, "cl_dynamiccrosshair", &psHUD_Flags,
          HUD_CROSSHAIR_DYNAMIC );
    CMD1( CCC_MainMenu, "main_menu" );

#ifndef MASTER_GOLD
    CMD1( CCC_StartTimeSingle, "start_time_single" );
    CMD4( CCC_TimeFactorSingle, "time_factor_single", &g_fTimeFactor, 0.f,
          1000.0f );
#endif // MASTER_GOLD
    ...
    CMD3( CCC_Mask, "ai_use_torch_dynamic_lights", &g_uCommonFlags,
          flAiUseTorchDynamicLights );
    ...
    CMD4( CCC_Integer, "death_anim_velocity", &b_death_anim_velocity, FALSE,
          TRUE );
    CMD4( CCC_Integer, "dbg_imotion_draw_velocity", &dbg_imotion_draw_velocity,
          FALSE, TRUE );
    CMD4( CCC_Integer, "dbg_imotion_collide_debug", &dbg_imotion_collide_debug,
          FALSE, TRUE );

    CMD4( CCC_Integer, "dbg_imotion_draw_skeleton", &dbg_imotion_draw_skeleton,
          FALSE, TRUE );
    CMD4( CCC_Float, "dbg_imotion_draw_velocity_scale",
          &dbg_imotion_draw_velocity_scale, 0.0001f, 100.0f );

    CMD4( CCC_Integer, "show_wnd_rect_all", &g_show_wnd_rect2, 0, 1 );
    CMD4( CCC_Integer, "dbg_show_ani_info", &g_ShowAnimationInfo, 0, 1 );
    CMD4( CCC_Integer, "dbg_dump_physics_step",
          &ph_console::g_bDebugDumpPhysicsStep, 0, 1 );
    ...
    CMD4( CCC_Integer, "dbg_bones_snd_player", &dbg_moving_bones_snd_player,
          FALSE, TRUE );
#endif
    ...
    //	CMD4(CCC_Integer,	"hud_old_style",
    //&g_old_style_ui_hud, 0, 1);

#ifdef DEBUG
    CMD4( CCC_Float, "ai_smart_cover_animation_speed_factor",
          &g_smart_cover_animation_speed_factor, .1f, 10.f );
    CMD4( CCC_Float, "air_resistance_epsilon", &air_resistance_epsilon, .0f,
          1.f );
#endif // #ifdef DEBUG
    ...
    CMD4( CCC_Integer, "ai_die_in_anomaly", &g_ai_die_in_anomaly, 0,
          1 ); // Alundaio

    CMD4( CCC_Integer, "binoculars_dynamic_zoom_check",
          &binoculars_dynamic_zoom_check, 0, 1 ); // VodoXleb

    CMD4( CCC_Integer, "pseudogiant_can_damage_objects_on_stomp",
          &pseudogiantCanDamageObjects, 0, 1 );

    CMD4( CCC_Integer, "pseudogiant_dodge_stomp_while_falling",
          &pseudogiantDodgeWhileFalling, 0, 1 ); // Verdatim

    CMD4( CCC_Integer, "telekinetic_objects_include_corpses",
          &g_telekinetic_objects_include_corpses, 0, 1 ); // Tosox

    CMD4( CCC_Integer, "allow_weapon_control_inertion_factor",
          &g_allow_weapon_control_inertion_factor, 0, 1 ); // momopate
    CMD4( CCC_Integer, "allow_outfit_control_inertion_factor",
          &g_allow_outfit_control_inertion_factor, 0, 1 );
    ...
    CMD4( CCC_Integer, "smooth_ads_transition", &g_smooth_ads_transition, 0,
          1 );
    CMD4( CCC_Integer, "allow_silencer_hide_tracer",
          &g_allow_silencer_hide_tracer, 0, 1 );
    ...
    CMD4( CCC_Integer, "mt_update_weapon_sounds", &mt_UpdateWeaponSounds, 0,
          1 );
    ...
    CMD4( CCC_Integer, "spawn_antifreeze_debug", &spawn_antifreeze_debug, 0,
          1 );
    ...
    CMD4( CCC_Integer, "r__optimize_calculate_bones",
          &r_optimize_calculate_bones, 0, 1 );
    CMD4( CCC_Integer, "r__optimize_torch", &r_optimize_torch, 0, 1 );
    CMD4( CCC_Integer, "hud_frequent_updates", &hud_frequent_updates, 0, 1 );

    CMD4( CCC_Integer, "g_progressive_stamina_cost", &progressiveStaminaCost, 0,
          1 );
    CMD4( CCC_Integer, "g_npcs_look_at_actor", &NPCsLookAtActor, 0, 1 );
    CMD4( CCC_Float, "g_npcs_look_at_actor_min_distance",
          &NPCsLookAtActorMinDistance, 1.f, 8.f );
    CMD4( CCC_Integer, "g_interrupt_fire_on_aim_toggle",
          &interruptFireOnAimToggle, 0, 1 );

    CMD4( CCC_Integer, "g_restriction_rebuild_frames",
          &RESTRICTION_REBUILD_SMOOTH_FRAMES, 5, 60 );
    ...
    // CMD4(CCC_Integer,	"use_new_ballistics",	&g_use_new_ballistics,
    // 0, 1);
    ...
    CMD4( CCC_Integer, "ai_aim_use_smooth_aim", &g_ai_aim_use_smooth_aim, 0,
          1 );
    ...
    *g_last_saved_game = 0;
    ...
    CMD3( CCC_Mask, "g_multi_item_pickup", &psActorFlags,
          AF_MULTI_ITEM_PICKUP );

    CMD3( CCC_Token, "g_dead_body_collision", &g_dead_body_collision,
          dead_body_collision_tokens );
    ...
    CMD4( CCC_Integer, "use_english_text_for_missing_translations",
          &use_english_text_for_missing_translations, 0, 1 );
    ...
    CMD4( CCC_Integer, "first_person_death_position_smoothing",
          &firstPersonDeathPositionSmoothing, 1, 30 );
    CMD4( CCC_Integer, "first_person_death_direction_smoothing",
          &firstPersonDeathDirectionSmoothing, 1, 60 );
    CMD4( CCC_Float, "first_person_death_near_plane_offset",
          &viewportNearOffset, -.1f, .5f );
    CMD4( CCC_Float, "first_person_death_head_scale",
          &firstPersonDeathHeadScale, 1.f, 10.f );
    ...
    // PDA commands
    CMD4( CCC_Integer, "pda_map_zoom_in_to_mouse", &pda_map_zoom_in_to_mouse, 0,
          1 );
    CMD4( CCC_Integer, "pda_map_zoom_out_to_mouse", &pda_map_zoom_out_to_mouse,
          0, 1 );
    CMD4( CCC_Integer, "pda_show_map_labels", &pda_show_map_labels, 0, 1 );

    // Mouse Wheel
    CMD4( CCC_Integer, "mouse_wheel_change_weapon", &mouseWheelChangeWeapon, 0,
          1 );
    CMD4( CCC_Integer, "mouse_wheel_invert_change_weapon",
          &mouseWheelInvertChangeWeapons, 0, 1 );
    ...
    // Print warnings when using bone_position and bone_direction functions and
    // encounter invalid bones
    CMD4( CCC_Integer, "print_bone_warnings", &print_bone_warnings, 0, 1 );

    // Print DLTX warnings when "override section which doesn't exist", also
    // prints cache hit for each file
    ...
    CMD4( CCC_Integer, "hanging_lamp_ignore_match_configuration",
          &alifeObjectHangingLampIgnoreMatchConfiguration, 0, 1 );

    // Ignore "Specified story object is already in the Story registry!: error
    CMD4( CCC_Integer, "duplicate_story_id_crash", &duplicate_story_id_crash, 0,
          1 );

    // Poltergeists spawn corpses on death
    CMD4( CCC_Integer, "poltergeist_spawn_corpse_on_death",
          &poltergeist_spawn_corpse_on_death, 0, 1 );

    // New zoom delta algorithm
    CMD4( CCC_Integer, "new_zoom_delta_algorithm", &useNewZoomDeltaAlgorithm, 0,
          1 );
    ...
    // When switching to UBGL the weapon will remember what mode you switched
    // from and will put you back in that mode. For example: You were aiming
    // down with a canted sight when you switched to UBGL. When you switch back
    // it will put you back into Canted sight aim and not the scope.
    CMD4( CCC_Integer, "use_separate_ubgl_keybind", &useSeparateUBGLKeybind, 0,
          1 );
    CMD4( CCC_Integer, "aimmode_remember", &g_aimmode_remember, 0, 1 );

    // Allows freelook during reload animations
    CMD4( CCC_Integer, "freelook_while_reloading", &g_freelook_while_reloading,
          0, 1 );
    ...
    // Wallmark distances
    // CMD4(CCC_Float, "g_wallmark_range_static", &wallmark_range_static, 0.f,
    // 1000.f); CMD4(CCC_Float, "g_wallmark_range_skeleton",
    // &wallmark_range_skeleton, 0.f, 1000.f);

    CMD1( CCC_MovePathQueryPosThreshold,
          "movement_manager_move_along_path_query_pos_threshold" );

    CMD1( CCC_Particle_TEST, "g_ps_test" );
    CMD4( CCC_Integer, "show_actor_body", &showActorBody, 0, 2 );
    CMD4( CCC_Integer, "disable_actor_body_rotation_delay",
          &disableActorBodyRotationDelay, 0, 1 );
}
