...
IC CGameGraph::CGameGraph( LPCSTR file_name, u32 current_version ) {
    ...
    R_ASSERT2( header().version() == XRAI_CURRENT_VERSION,
    ...
    m_cross_tables =
        ( u32* )( ( ( CLevelPoint* )temp ) + header().death_point_count() );
...
IC float CGameGraph::distance( const _GRAPH_ID tGraphID0,
                               const _GRAPH_ID tGraphID1 ) const {
    const_iterator i, e;
    begin( tGraphID0, i, e );
    for ( ; i != e; ++i )
...
IC void CGameGraph::begin( u32 const vertex_id,
                           const_iterator& start,
                           const_iterator& end ) const {
    end = ( start = ( const CEdge* )( ( BYTE* )m_nodes +
                                      vertex( _GRAPH_ID( vertex_id ) )
                                          ->edge_offset() ) ) +
          vertex( _GRAPH_ID( vertex_id ) )->edge_count();
}

IC const CGameGraph::_GRAPH_ID& CGameGraph::value( u32 const vertex_id,
...
IC GameGraph::_LEVEL_ID GameGraph::CHeader::level_count() const {
    VERIFY( m_levels.size() <
            ( u32( 1 ) << ( 8 * sizeof( GameGraph::_LEVEL_ID ) ) ) );
...
IC const GameGraph::SLevel& GameGraph::CHeader::level(
    const _LEVEL_ID& id ) const {
    LEVEL_MAP::const_iterator I = levels().find( id );
    if ( I == levels().end() ) {
        R_ASSERT2(
            false,
            make_string( "there is no specified level in the game graph : %d",
                         id ) );
    }
    return ( ( *I ).second );
}
...
#else
    R_ASSERT3( false, "There is no specified level in the game graph!",
               level_name );
    NODEFAULT;
#endif
}

IC const GameGraph::SLevel* GameGraph::CHeader::level( LPCSTR level_name,
                                                       bool ) const {
    LEVEL_MAP::const_iterator I = levels().begin();
    LEVEL_MAP::const_iterator E = levels().end();
    for ( ; I != E; ++I )
...
IC void CGameGraph::begin_spawn( u32 const vertex_id,
                                 const_spawn_iterator& start,
                                 const_spawn_iterator& end ) const {
    const CVertex* object = vertex( vertex_id );
    start = ( const_spawn_iterator )( ( u8* )m_nodes +
                                      object->death_point_offset() );
...
IC GameGraph::_GRAPH_ID CGameGraph::vertex_id(
    const CGameGraph::CVertex* vertex ) const {
...
IC void CGameGraph::set_current_level( u32 const level_id ) {
    ...
    for ( ; I != E; ++I ) {
        if ( level_id != ( *I ).first ) {
            current_cross_table =
                ( u32* )( ( u8* )current_cross_table + *current_cross_table );
            continue;
        }

        m_current_level_cross_table = xr_new< CGameLevelCrossTable >(
            current_cross_table + 1, *current_cross_table );
        break;
    }
