...
void CPostprocessAnimator::Load( LPCSTR name ) {
    ...
    if ( ext ) {
        if ( !xr_strcmp( ext, POSTPROCESS_FILE_EXTENSION ) ) {
            ...
            FATAL(
                "ERROR: Can't support files with many animations set. "
                "Incorrect file." );
    }
...
BOOL CPostprocessAnimator::Process( SPPInfo& PPInfo ) {
    ...
    if ( m_bStop )
        ...
        m_factor +=
            m_factor_speed * Device.fTimeDelta * ( m_dest_factor - m_factor );
...
void CPostprocessAnimator::Create() {
    ...
    m_Params[ 0 ] = xr_new< CPostProcessColor >(
        &m_EffectorParams.color_base ); // base color
    VERIFY( m_Params[ 0 ] );
    m_Params[ 1 ] =
        xr_new< CPostProcessColor >( &m_EffectorParams.color_add ); // add color
    VERIFY( m_Params[ 1 ] );
    m_Params[ 2 ] = xr_new< CPostProcessColor >(
        &m_EffectorParams.color_gray ); // gray color
    VERIFY( m_Params[ 2 ] );
    m_Params[ 3 ] =
        xr_new< CPostProcessValue >( &m_EffectorParams.gray ); // gray value
    VERIFY( m_Params[ 3 ] );
    m_Params[ 4 ] =
        xr_new< CPostProcessValue >( &m_EffectorParams.blur ); // blur value
    VERIFY( m_Params[ 4 ] );
    m_Params[ 5 ] = xr_new< CPostProcessValue >(
        &m_EffectorParams.duality.h ); // duality horizontal
    VERIFY( m_Params[ 5 ] );
    m_Params[ 6 ] = xr_new< CPostProcessValue >(
        &m_EffectorParams.duality.v ); // duality vertical
    VERIFY( m_Params[ 6 ] );
    m_Params[ 7 ] = xr_new< CPostProcessValue >(
        &m_EffectorParams.noise.intensity ); // noise intensity
    VERIFY( m_Params[ 7 ] );
    m_Params[ 8 ] = xr_new< CPostProcessValue >(
        &m_EffectorParams.noise.grain ); // noise granularity
    VERIFY( m_Params[ 8 ] );
    m_Params[ 9 ] =
        xr_new< CPostProcessValue >( &m_EffectorParams.noise.fps ); // noise fps
    VERIFY( m_Params[ 9 ] );
    m_Params[ 10 ] =
        xr_new< CPostProcessValue >( &m_EffectorParams.cm_influence );
    VERIFY( m_Params[ 10 ] );
}
...
void CPostprocessAnimator::ResetParam( pp_params param ) {
    ...
    switch ( param ) {
        case pp_base_color:
            ...
            break;
        case pp_add_color:
            m_Params[ 1 ] = xr_new< CPostProcessColor >(
                &m_EffectorParams.color_add ); // add color
            break;
        case pp_gray_color:
            m_Params[ 2 ] = xr_new< CPostProcessColor >(
                &m_EffectorParams.color_gray ); // gray color
            break;
        case pp_gray_value:
            m_Params[ 3 ] = xr_new< CPostProcessValue >(
                &m_EffectorParams.gray ); // gray value
            break;
        case pp_blur:
            m_Params[ 4 ] = xr_new< CPostProcessValue >(
                &m_EffectorParams.blur ); // blur value
            break;
        case pp_dual_h:
            m_Params[ 5 ] = xr_new< CPostProcessValue >(
                &m_EffectorParams.duality.h ); // duality horizontal
        ...
        case pp_noise_g:
            ...
            break;
        case pp_noise_f:
            m_Params[ 9 ] = xr_new< CPostProcessValue >(
                &m_EffectorParams.noise.fps ); // noise fps
            break;
...
CPostprocessAnimatorControlled::CPostprocessAnimatorControlled(
    CEffectorController* c )
    : m_controller( c ) {
    m_controller->SetPP( this );
    SetFactorFunc(
        xr_make_delegate( m_controller, &CEffectorController::GetFactor ) );
}
