...
static const float s_fLandingTime1 = 0.1f; // через сколько снять флаг Landing1
                                           // (т.е. включить следующую анимацию)
static const float s_fLandingTime2 = 0.3f; // через сколько снять флаг Landing2
                                           // (т.е. включить следующую анимацию)
static const float s_fJumpTime = 0.3f;
static const float s_fJumpGroundTime =
    0.1f; // для снятия флажка Jump если на земле
const float s_fFallTime = 0.2f;

IC static void generate_orthonormal_basis1( const Fvector& dir,
                                            Fvector& updir,
                                            Fvector& right ) {
...
void CActor::g_cl_ValidateMState( float dt, u32 mstate_wf ) {
    ...
    if ( ( ( mstate_wf & mcLLookout ) && ( mstate_wf & mcRLookout ) ) ||
         ( ( mstate_real & mcLLookout ) && ( mstate_real & mcRLookout ) ) ) {
    ...
    if ( character_physics_support()->movement()->gcontact_Was ) {
        if ( mstate_real & mcFall ) {
            if ( character_physics_support()->movement()->GetContactSpeed() >
                 4.f ) {
                if ( fis_zero( character_physics_support()
                                   ->movement()
                                   ->gcontact_HealthLost ) ) {
            ...
            if ( ai().script_engine().functor( "_G.CActor_on_land", on_land ) )
                on_land( character_physics_support()
                             ->movement()
                             ->GetContactSpeed() );
        }
    ...
    if ( ( ( character_physics_support()->movement()->GetVelocityActual() <
             0.2f ) &&
           ( !( mstate_real & ( mcFall | mcJump | mcLanding | mcLanding2 ) ) ||
             ( !( mstate_real & mcClimb ) &&
               character_physics_support()->movement()->Environment() ==
                   CPHMovementControl::peAtWall ) ) ) ||
         character_physics_support()->movement()->bSleep ) {
        mstate_real &= ~mcAnyMove;
    }
    if ( character_physics_support()->movement()->Environment() ==
             CPHMovementControl::peOnGround ||
         character_physics_support()->movement()->Environment() ==
             CPHMovementControl::peAtWall ) {
        // если на земле гарантированно снимать флажок Jump
        if ( ( ( s_fJumpTime - m_fJumpTime ) > s_fJumpGroundTime ) &&
             ( mstate_real & mcJump ) ) {
            mstate_real &= ~mcJump;
            m_fJumpTime = s_fJumpTime;
        }
    ...
    if ( mstate_wf != mstate_real ) {
        if ( ( mstate_real & mcCrouch ) &&
             ( ( 0 == ( mstate_wf & mcCrouch ) ) || mstate_real & mcClimb ) ) {
    ...
    if ( !CanAccelerate() &&
         isActorAccelerated( mstate_real, IsZoomAimingMode() ) ) {
        mstate_real ^= mcAccel;
    };
    ...
    if ( this == Level().CurrentControlEntity() ) {
        ...
        if ( bOnClimbNow != bOnClimbOld ) {
            SetWeaponHideState( /*INV_STATE_LADDER*/ INV_STATE_BLOCK_ALL,
                                bOnClimbNow );
        };
    };
};

void CActor::g_cl_CheckControls( u32 mstate_wf,
    ...
    CPHMovementControl::EEnvironment curr_env =
        character_physics_support()->movement()->Environment();
    if ( curr_env == CPHMovementControl::peOnGround ||
        ...
        if ( ( 0 == ( mstate_real & mcCrouch ) ) && ( mstate_wf & mcCrouch ) ) {
            if ( mstate_real & mcClimb ) {
                ...
                if ( isActorAccelerated( mstate_wf, IsZoomAimingMode() ) )
                    Crouched = character_physics_support()
                                   ->movement()
                                   ->ActivateBoxDynamic( 1 );
                else
                    Crouched = character_physics_support()
                                   ->movement()
                                   ->ActivateBoxDynamic( 2 );
        ...
        if ( mstate_real & mcAnyMove ) {
            ...
            if ( scale > EPS ) {
                ...
                if ( mstate_real & ( mcLStrafe | mcRStrafe ) &&
                     !( mstate_real & mcCrouch ) ) {
                    if ( mstate_real & mcSprint )
    ...
    if ( IsGameTypeSingle() && cam_eff_factor > EPS ) {
        ...
        if ( state_anm ) {
            ...
            CActor* control_entity = static_cast_checked< CActor* >(
                Level().CurrentControlEntity() );
            if ( control_entity ) {
                R_ASSERT2( control_entity, "current control entity is NULL" );
                CEffectorCam* ec =
                    control_entity->Cameras().GetCamEffector( eCEActorMoving );
                if ( NULL == ec ) {
                    string_path eff_name;
                    xr_sprintf( eff_name, sizeof( eff_name ), "%s.anm",
                                state_anm );
                    string_path ce_path;
                    string_path anm_name;
                    strconcat( sizeof( anm_name ), anm_name,
                               "camera_effects\\actor_move\\", eff_name );
                    if ( FS.exist( ce_path, "$game_anims$", anm_name ) ) {
                        CAnimatorCamLerpEffectorConst* e =
                            xr_new< CAnimatorCamLerpEffectorConst >();
...
void CActor::g_Orientate( u32 mstate_rl, float dt ) {
    static float fwd_l_strafe_yaw =
        deg2rad( pSettings->r_float( ACTOR_ANIM_SECT, "fwd_l_strafe_yaw" ) );
    static float back_l_strafe_yaw =
        deg2rad( pSettings->r_float( ACTOR_ANIM_SECT, "back_l_strafe_yaw" ) );
    static float fwd_r_strafe_yaw =
        deg2rad( pSettings->r_float( ACTOR_ANIM_SECT, "fwd_r_strafe_yaw" ) );
    static float back_r_strafe_yaw =
        deg2rad( pSettings->r_float( ACTOR_ANIM_SECT, "back_r_strafe_yaw" ) );
    static float l_strafe_yaw =
        deg2rad( pSettings->r_float( ACTOR_ANIM_SECT, "l_strafe_yaw" ) );
    static float r_strafe_yaw =
        deg2rad( pSettings->r_float( ACTOR_ANIM_SECT, "r_strafe_yaw" ) );
    ...
    if ( cam_active == eacFirstEye && g_player_hud &&
         m_legs_controller.is_active() ) {
        scale_yaw_offset = 0.15f;
    }

    angle_lerp( r_model_yaw_delta, calc_yaw * scale_yaw_offset,
                PI_MUL_4 * scale_yaw_offset, dt );
    ...
    if ( mstate_rl & mcLookout ) {
        tgt_roll = ( mstate_rl & mcLLookout ) ? -ACTOR_LLOOKOUT_ANGLE
                                              : ACTOR_RLOOKOUT_ANGLE;
    ...
    if ( !fsimilar( tgt_roll, r_torso_tgt_roll, EPS ) ) {
        r_torso_tgt_roll =
            angle_inertion_var( r_torso_tgt_roll, tgt_roll, 0.f,
                                CurrentHeight * PI_MUL_2, PI_DIV_2, dt );
...
// ****************************** Update actor orientation according to camera
// orientation
void CActor::g_cl_Orientate( u32 mstate_rl, float dt ) {
    ...
    if ( eacFreeLook != cam_active ) {
        if ( cam_freelook == eflDisabled ) {
            ...
            r_torso.yaw = angle_lerp( cam_Active()->GetWorldYaw(),
                                      -old_torso_yaw, freelook_cam_control );
            float old_pitch = cam_Active()->GetWorldPitch();
            float new_pitch =
                old_pitch > 0.f ? old_pitch * .6f : old_pitch * .8f;
            r_torso.pitch =
                angle_lerp( old_pitch, new_pitch, freelook_cam_control );
        }
    } else {
...
void CActor::g_sv_Orientate( u32 /**mstate_rl/**/, float /**dt/**/ ) {
    ...
    CWeaponMagazined* pWM = smart_cast< CWeaponMagazined* >(
        inventory().GetActiveSlot() != NO_ACTIVE_SLOT
            ...
            : NULL );
    if ( pWM &&
         pWM->GetCurrentFireMode() == 1 /* && eacFirstEye != cam_active*/ ) {
        Fvector dangle = weapon_recoil_last_delta();
...
bool CActor::CanSprint() {
    ...
    if ( ( mstate_real & mcLStrafe || mstate_real & mcRStrafe ) &&
         !( mstate_real & mcFwd ) )
        can_Sprint = false;
...
bool CActor::CanJump() {
    bool can_Jump = !conditions().IsCantSprint() &&
                    !character_physics_support()->movement()->PHCapture() &&
                    ( ( mstate_real & mcJump ) == 0 ) &&
                    ( m_fJumpTime <= 0.f ) && !m_bJumpKeyPressed &&
                    !IsZoomAimingMode();
...
float CActor::get_additional_weight() const {
    ...
    CBackpack* pBackpack =
        smart_cast< CBackpack* >( inventory().ItemFromSlot( BACKPACK_SLOT ) );
    if ( pBackpack )
