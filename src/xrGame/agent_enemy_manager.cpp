...
struct CEnemyFiller {
    ...
    IC void operator()( const CEntityAlive* enemy ) const {
        typename ENEMIES::iterator I =
            std::find( m_enemies->begin(), m_enemies->end(), enemy );
        if ( I == m_enemies->end() ) {
...
struct remove_wounded_predicate {
    IC bool operator()( const CMemberEnemy& enemy ) const {
        const CAI_Stalker* stalker =
            smart_cast< const CAI_Stalker* >( enemy.m_object );
        if ( !stalker )
            return ( false );
...
void CAgentEnemyManager::fill_enemies() {
    ...
    {
        typename CAgentMemberManager::iterator I =
            object().member().combat_members().begin();
        typename CAgentMemberManager::iterator E =
        ...
        for ( ; I != E; ++I ) {
            ...
            if ( ( *I )->object().getDestroy() )
                continue;
            ( *I )->probability( 1.f );
            ( *I )->object().memory().fill_enemies( CEnemyFiller(
                &m_enemies, object().member().mask( &( *I )->object() ) ) );
        }
    }
    ...
    m_is_any_wounded = false;
    {
        ...
        typename ENEMIES::iterator I = enemies().begin();
        typename ENEMIES::iterator E = enemies().end();
        for ( ; I != E; ++I ) {
            if ( m_only_wounded_left ) {
                const CAI_Stalker* stalker =
                    smart_cast< const CAI_Stalker* >( ( *I ).m_object );
            ...
            memory.object_information( ( *I ).m_object, ( *I ).m_level_time,
                                       ( *I ).m_enemy_position );
        }
    }
...
float CAgentEnemyManager::evaluate( const CEntityAlive* object0,
                                    const CEntityAlive* object1 ) const {
...
void CAgentEnemyManager::exchange_enemies( CMemberOrder& member0,
                                           CMemberOrder& member1 ) {
...
void CAgentEnemyManager::compute_enemy_danger() {
    {
        typename ENEMIES::iterator I = m_enemies.begin();
        typename ENEMIES::iterator E = m_enemies.end();
        for ( ; I != E; ++I ) {
            float best = -1.f;
            CAgentMemberManager::const_iterator i =
                object().member().combat_members().begin();
            CAgentMemberManager::const_iterator e =
                object().member().combat_members().end();
            for ( ; i != e; ++i ) {
...
void CAgentEnemyManager::permutate_enemies() {
    ...
    do {
        ...
        for ( ; I != E; ++I ) {
            ...
            float best = ( *I )->object().Position().distance_to(
                m_enemies[ ( *I )->selected_enemy() ].m_object->Position() );
            bool found = false;
            ...
            for ( ; i != e; ++i ) {
                if ( ( *I )->selected_enemy() == *i )
                    continue;
                float my_distance = ( *I )->object().Position().distance_to(
                    m_enemies[ *i ].m_object->Position() );
                if ( my_distance < best ) {
                    // check if we can exchange enemies
                    squad_mask_type J = m_enemies[ *i ].m_distribute_mask.get(),
                                    K;
                    // iterating on members, whose current enemy is the new one
                    for ( ; J; J &= J - 1 ) {
                        K = ( J & ( J - 1 ) ) ^ J;
                        typename CAgentMemberManager::iterator j =
                            object().member().member( K );
                        typename xr_vector< u32 >::iterator ii = std::find(
                            ( *j )->enemies().begin(), ( *j )->enemies().end(),
                            ( *I )->selected_enemy() );
                        ...
                        float member_distance =
                            ( *j )->object().Position().distance_to(
                                m_enemies[ *i ].m_object->Position() );
                        if ( member_distance <= my_distance )
                            continue;

                        // check if our effectiveness is near the same
                        float my_to_his = evaluate(
                            &( *I )->object(),
                            m_enemies[ ( *j )->selected_enemy() ].m_object );
                        float his_to_my = evaluate(
                            &( *j )->object(),
                            m_enemies[ ( *I )->selected_enemy() ].m_object );
                        if ( !fsimilar( my_to_his, ( *j )->probability() ) ||
                             !fsimilar( his_to_my, ( *I )->probability() ) )
                            continue;
...
IC void CAgentEnemyManager::setup_mask(
    ...
    typename xr_vector< T >::iterator I =
        std::find( objects.begin(), objects.end(), enemy.m_object->ID() );
    if ( I != objects.end() ) {
        ( *I ).m_squad_mask.assign( ( *I ).m_squad_mask.get() |
...
void CAgentEnemyManager::assign_enemy_masks() {
    {
        ...
        typename ENEMIES::iterator E = m_enemies.end();
        for ( ; I != E; ++I ) {
            CAgentMemberManager::MEMBER_STORAGE::const_iterator i =
                object().member().combat_members().begin();
            CAgentMemberManager::MEMBER_STORAGE::const_iterator e =
                object().member().combat_members().end();
            for ( ; i != e; ++i )
                ( *i )->object().memory().make_object_visible_somewhen(
                    ( *I ).m_object );
        }
    }

    squad_mask_type non_combat_members =
        object().member().non_combat_members_mask();
...
void CAgentEnemyManager::assign_wounded() {
    ...
    WOUNDED_ENEMY* previous_wounded = ( WOUNDED_ENEMY* )_alloca(
        previous_wounded_count * sizeof( WOUNDED_ENEMY ) );
    ...
    squad_mask_type assigned = 0;
    {
        ...
        for ( ; I != E; ++I ) {
            typename ENEMIES::iterator J =
                std::find( m_enemies.begin(), m_enemies.end(), ( *I ).first );
            if ( J == m_enemies.end() )
                continue;

            CMemberOrder* member_order =
                object().member().get_member( ( *I ).second.first );
            if ( !member_order )
                continue;

            squad_mask_type mask =
                object().member().mask( ( *I ).second.first );
            if ( !( object().member().combat_mask() & mask ) )
                continue;

            typename CAgentMemberManager::iterator i =
                object().member().member( mask );
            if ( ( *I ).first->Position().distance_to_sqr(
                     ( *i )->object().Position() ) >
                 _sqr( wounded_enemy_reached_distance ) )
                continue;

            if ( wounded_processor( ( *J ).m_object ) !=
                 ALife::_OBJECT_ID( -1 ) )
                continue;
    ...
    {
        VERIFY( combat_member_count ==
                object().member().combat_members().size() );
    }
    ...
    while ( population( assigned ) < combat_member_count ) {
        ...
        for ( int i = 0; i < 2; ++i ) {
            ...
            typename ENEMIES::iterator E = m_enemies.end();
            for ( ; I != E; ++I ) {
                if ( population( ( *I ).m_distribute_mask.get() ) >
                     population_level )
                    continue;
                ...
                for ( ; J; J &= J - 1 ) {
                    ...
                    typename CAgentMemberManager::iterator i =
                        object().member().member( K );
                    float distance_sqr =
                        ( *i )->object().Position().distance_to_sqr(
                            ( *I ).m_object->Position() );
        ...
        if ( !enemy ) {
            ...
            Msg( "wounded enemies(%d):", m_enemies.size() );
            {
                ...
                for ( ; I != E; ++I )
                    Msg( "  [%s][0x%08x][0x%08x][%.2f]",
                         *( *I ).m_object->cName(), ( *I ).m_mask.get(),
                         ( *I ).m_distribute_mask.get(), ( *I ).m_probability );
            }
            Msg( "combat members(%d):",
                 object().member().combat_members().size() );
            {
                typedef CAgentMemberManager::MEMBER_STORAGE::const_iterator
                    const_iterator;
                const_iterator I = object().member().combat_members().begin();
                const_iterator E = object().member().combat_members().end();
                for ( ; I != E; ++I )
                    Msg( "  [%s][0x%08x][0x%08x]", *( *I )->object().cName(),
                         object().member().mask( &( *I )->object() ),
                         ( *I )->selected_enemy() );
            }
        }
        ...
        // since the last enemy went offline and he knows nothing about the
        // second one so, in this case we just need to stop iterating since nest
        // procedure (setup_enemy_masks) will make the second enemy known for
        // the soldier and on its update he will select it
        if ( !enemy )
            return;

        //		Msg
        //("wounded enemy [%s] is assigned to member
...
struct wounded_predicate {
    ...
    IC bool operator()(
        const CAgentEnemyManager::WOUNDED_ENEMY& wounded_enemy ) const {
...
void CAgentEnemyManager::remove_links( CObject* object ) {
    m_wounded.erase( std::remove_if( m_wounded.begin(), m_wounded.end(),
...
ALife::_OBJECT_ID CAgentEnemyManager::wounded_processor(
...
void CAgentEnemyManager::wounded_processor(
    const CEntityAlive* object,
    const ALife::_OBJECT_ID& wounded_processor_id ) {
    VERIFY( std::find_if( m_wounded.begin(), m_wounded.end(),
                          find_wounded_predicate( object ) ) ==
            m_wounded.end() );
    m_wounded.push_back( std::make_pair(
        object, std::make_pair( wounded_processor_id, false ) ) );
}

void CAgentEnemyManager::wounded_processed( const CEntityAlive* object,
                                            bool value ) {
    VERIFY( value );
    typename WOUNDED_ENEMIES::iterator I = std::find_if(
        m_wounded.begin(), m_wounded.end(), find_wounded_predicate( object ) );
    if ( I == m_wounded.end() )
        return;
...
bool CAgentEnemyManager::wounded_processed( const CEntityAlive* object ) const {
    WOUNDED_ENEMIES::const_iterator I = std::find_if(
        m_wounded.begin(), m_wounded.end(), find_wounded_predicate( object ) );
    if ( I == m_wounded.end() )
        return ( false );
...
bool CAgentEnemyManager::assigned_wounded( const CEntityAlive* wounded,
                                           const CAI_Stalker* member ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        return ( !!( *I ).m_distribute_mask.test(
...
bool CAgentEnemyManager::useful_enemy( const CEntityAlive* enemy,
                                       const CAI_Stalker* member ) const {
    ...
    return (
        !!( *I ).m_distribute_mask.test( object().member().mask( member ) ) );
}
