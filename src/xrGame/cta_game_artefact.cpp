...
void CtaGameArtefact::UpdateCLChild() {
    ...
    if ( m_game &&
         m_artefact_rpoint->similar( XFORM().c, m_game->GetBaseRadius() ) &&
         PPhysicsShell() ) {
...
(Position());
        ...
        num_items.mask			= 0;
        num_items.num_items		= 1;// always to synchronize .
...
= 1.f/magnitude;
        ...
        if (!(num_items.mask &
CSE_ALifeInventoryItem::inventory_item_angular_null)) { clamp
(State.angular_vel.x,0.f,10.f*PI_MUL_2); clamp
...
(State.angular_vel.z,0.f,10.f*PI_MUL_2);
        ...
        if (!(num_items.mask &
CSE_ALifeInventoryItem::inventory_item_linear_null)) { clamp
(State.linear_vel.x,-32.f,32.f); clamp
