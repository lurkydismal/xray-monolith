...
void CInventoryOwner::Load( LPCSTR section ) {
    if ( pSettings->line_exist( section, "inv_max_weight" ) )
        m_inventory->SetMaxWeight(
            pSettings->r_float( section, "inv_max_weight" ) );

    if ( pSettings->line_exist( section, "need_osoznanie_mode" ) ) {
        m_need_osoznanie_mode =
            pSettings->r_bool( section, "need_osoznanie_mode" );
    } else {
        m_need_osoznanie_mode = FALSE;
    }
}
...
BOOL CInventoryOwner::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( IsGameTypeSingle() ) {
        ...
        CAI_PhraseDialogManager* dialog_manager =
            smart_cast< CAI_PhraseDialogManager* >( this );
        if ( dialog_manager && !dialog_manager->GetStartDialog().size() ) {
            dialog_manager->SetStartDialog( CharacterInfo().StartDialog() );
            dialog_manager->SetDefaultStartDialog(
                CharacterInfo().StartDialog() );
        }
...
bool CInventoryOwner::OfferTalk( CInventoryOwner* talk_partner ) {
    ...
    CEntityAlive* pPartnerEntityAlive =
        smart_cast< CEntityAlive* >( talk_partner );
...
void CInventoryOwner::OnItemTake( CInventoryItem* inventory_item ) {
    ...
    VERIFY( object );
    object->callback( GameObject::eOnItemTake )(
        inventory_item->object().lua_game_object() );
...
void CInventoryOwner::spawn_supplies() {
    ...
    if ( use_bolts() )
        Level().spawn_item( "bolt", game_object->Position(),
                            game_object->ai_location().level_vertex_id(),
                            game_object->ID() );
...
Level().spawn_item("device_pda", game_object->Position(),
game_object->ai_location().level_vertex_id(), game_object->ID(), true);
    CSE_ALifeItemPDA					*pda =
smart_cast<CSE_ALifeItemPDA*>(abstract); R_ASSERT(pda); pda->m_original_owner =
(u16) game_object->ID(); NET_Packet
P; abstract->Spawn_Write(P, TRUE); Level().Send(P, net_flags(TRUE));
...
void CInventoryOwner::SetCommunity( CHARACTER_COMMUNITY_INDEX new_community ) {
    ...
    if ( EA->g_Alive() ) {
        EA->ChangeTeam( CharacterInfo().Community().team(), EA->g_Squad(),
                        EA->g_Group() );
    }

    CSE_ALifeTraderAbstract* trader =
        smart_cast< CSE_ALifeTraderAbstract* >( e_entity );
    if ( !trader )
        return;
...
void CInventoryOwner::OnItemDrop( CInventoryItem* inventory_item,
    ...
    CGameObject* object = smart_cast< CGameObject* >( this );
    VERIFY( object );
    object->callback( GameObject::eOnItemDrop )(
        inventory_item->object().lua_game_object() );
...
void CInventoryOwner::OnItemBelt( CInventoryItem* inventory_item,
                                  const SInvItemPlace& previous_place ) {
    /* avo: script callback */
#ifdef EXTENDED_ITEM_CALLBACKS
    CGameObject* object = smart_cast< CGameObject* >( this );
    VERIFY( object );
    object->callback( GameObject::eItemToBelt )(
        inventory_item->object().lua_game_object() );
#endif
    /* avo: end */
}

void CInventoryOwner::OnItemRuck( CInventoryItem* inventory_item,
                                  const SInvItemPlace& previous_place ) {
    /* avo: script callback */
#ifdef EXTENDED_ITEM_CALLBACKS
    CGameObject* object = smart_cast< CGameObject* >( this );
    VERIFY( object );
    object->callback( GameObject::eItemToRuck )(
        inventory_item->object().lua_game_object() );
#endif
    /* avo: end */
...
void CInventoryOwner::OnItemSlot( CInventoryItem* inventory_item,
                                  const SInvItemPlace& previous_place ) {
    /* avo: script callback */
#ifdef EXTENDED_ITEM_CALLBACKS
    CGameObject* object = smart_cast< CGameObject* >( this );
    VERIFY( object );
    object->callback( GameObject::eItemToSlot )(
        inventory_item->object().lua_game_object() );
#endif
    /* avo: end */
...
CCustomOutfit* CInventoryOwner::GetOutfit() const {
    return smart_cast< CCustomOutfit* >(
...
LPCSTR CInventoryOwner::trade_section() const {
    ...
    VERIFY( game_object );
    return ( READ_IF_EXISTS( pSettings, r_string, game_object->cNameSect(),
                             "trade_section", "trade" ) );
}
...
bool CInventoryOwner::AllowItemToTrade( CInventoryItem const* item,
                                        const SInvItemPlace& place ) const {
    return ( trade_parameters().enabled( CTradeParameters::action_sell( 0 ),
