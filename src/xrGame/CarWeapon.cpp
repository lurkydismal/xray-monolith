...
CCarWeapon::CCarWeapon( CPhysicsShellHolder* obj ) {
    ...
    m_min_gun_speed =
        pUserData->r_float( "mounted_weapon_definition", "min_gun_speed" );
    m_max_gun_speed =
        pUserData->r_float( "mounted_weapon_definition", "max_gun_speed" );
    CBoneData& bdX =
        K->LL_GetData( m_rotate_x_bone ); // VERIFY(bdX.IK_data.type==jtJoint);
    m_lim_x_rot.set( bdX.IK_data.limits[ 0 ].limit.x,
                     bdX.IK_data.limits[ 0 ].limit.y );
    CBoneData& bdY =
        K->LL_GetData( m_rotate_y_bone ); // VERIFY(bdY.IK_data.type==jtJoint);
...
void CCarWeapon::Load( LPCSTR section ) {
    ...
    HUD_SOUND_ITEM::LoadSound( section, "snd_shoot", m_sndShot,
...
void CCarWeapon::SetBoneCallbacks() {
    ...
    CBoneInstance& biX = smart_cast< IKinematics* >( m_object->Visual() )
                             ->LL_GetBoneInstance( m_rotate_x_bone );
    biX.set_callback( bctCustom, BoneCallbackX, this );
    CBoneInstance& biY = smart_cast< IKinematics* >( m_object->Visual() )
...
void CCarWeapon::UpdateBarrelDir() {
    ...
    m_cur_x_rot = angle_inertion_var( m_cur_x_rot, m_tgt_x_rot, m_min_gun_speed,
                                      m_max_gun_speed, PI, Device.fTimeDelta );
    m_cur_y_rot = angle_inertion_var( m_cur_y_rot, m_tgt_y_rot, m_min_gun_speed,
                                      m_max_gun_speed, PI, Device.fTimeDelta );
...
void CCarWeapon::OnShot() {
    ...
    FireBullet( m_fire_pos, m_fire_dir, fireDispersionBase, *m_Ammo,
                holder->Engaged() ? 0 : m_object->ID(), m_object->ID(),
...
void CCarWeapon::Action( u16 id, u32 flags ) {
    switch ( id ) {
        ...
        case eWpnToDefaultDir: {
            SetParam( eWpnDesiredDir,
                      Fvector2().set( m_bind_y_rot, m_bind_x_rot ) );
        } break;
    }
}
