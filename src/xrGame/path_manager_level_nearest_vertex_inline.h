...
#define CNearestVertexPathManager                                             \
    CPathManager< CLevelGraph, _DataStorage,                                  \
                  SNearestVertex< _dist_type, _index_type, _iteration_type >, \
...
TEMPLATE_SPECIALIZATION
IC void CNearestVertexPathManager::setup( const _Graph* _graph,
...
TEMPLATE_SPECIALIZATION
IC bool CNearestVertexPathManager::is_goal_reached(
    const _index_type& node_index ) {
    VERIFY( path );
    best_node = graph->vertex( node_index );

    float current_distance = m_target_position.distance_to_xz_sqr(
        graph->vertex_position( best_node ) );
...
TEMPLATE_SPECIALIZATION
IC _dist_type
CNearestVertexPathManager::evaluate( const _index_type& node_index1,
...
TEMPLATE_SPECIALIZATION
IC _dist_type
CNearestVertexPathManager::estimate( const _index_type& node_index ) const {
    VERIFY( graph );
    return ( _dist_type( 0 ) );
}

TEMPLATE_SPECIALIZATION
IC bool CNearestVertexPathManager::is_accessible(
...
TEMPLATE_SPECIALIZATION
IC bool CNearestVertexPathManager::is_limit_reached(
