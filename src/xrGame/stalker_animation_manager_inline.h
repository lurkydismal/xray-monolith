...
IC const CStalkerAnimationManager::SCRIPT_ANIMATIONS&
CStalkerAnimationManager::script_animations() const {
...
IC bool CStalkerAnimationManager::non_script_need_update() const {
    return ( m_global_selector || m_global_callback || m_global.need_update() ||
             m_head.need_update() || m_torso.need_update() ||
             m_legs.need_update() );
}
...
IC CStalkerAnimationManager::AnimationSelector const&
CStalkerAnimationManager::global_selector() const {
    return ( m_global_selector );
}

IC void CStalkerAnimationManager::global_selector(
    AnimationSelector const& selector ) {
    m_global_selector = selector;
}

IC CStalkerAnimationManager::AnimationCallback const&
CStalkerAnimationManager::global_callback() const {
    return ( m_global_callback );
}

IC void CStalkerAnimationManager::global_callback(
    AnimationCallback const& callback ) {
    m_global_callback = callback;
}

IC CStalkerAnimationManager::AnimationModifier const&
CStalkerAnimationManager::global_modifier() const {
    return ( m_global_modifier );
}

IC void CStalkerAnimationManager::global_modifier(
    AnimationModifier const& modifier ) {
