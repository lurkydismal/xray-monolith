...
CSE_ALifeInventoryItem *tpALifeInventoryItem2) const
        {
                if (tpALifeInventoryItem1->m_dwCost ==
tpALifeInventoryItem2->m_dwCost) if (tpALifeInventoryItem1->m_tPreviousParentID
== m_tParentID) if (tpALifeInventoryItem2->m_tPreviousParentID == m_tParentID)
                                        return
(tpALifeInventoryItem1->base()->ID < tpALifeInventoryItem2->base()->ID); else
                                        return				(true);
                        else
                                if (tpALifeInventoryItem2->m_tPreviousParentID
== m_tParentID) return				(false); else return
(tpALifeInventoryItem1->base()->ID < tpALifeInventoryItem2->base()->ID); else
                        return
(tpALifeInventoryItem1->m_dwCost > tpALifeInventoryItem2->m_dwCost);
        }
...
CALifeCommunicationManager::CALifeCommunicationManager( xrServer* server,
                                                        LPCSTR section )
...
("%s[%d]",tpALifeHumanAbstract->name_replace(),tpALifeHumanAbstract->m_dwMoney);
        OBJECT_IT			I =
tpALifeHumanAbstract->children.begin(); OBJECT_IT			E =
tpALifeHumanAbstract->children.end(); for ( ; I != E; ++I) Msg
...
(m_temp_item_vector.begin(),m_temp_item_vector.end(),CSortByOwnerPredicate(tpALifeHumanAbstract->ID));
        switch (i) {
                case 0 : {
                        j	=
tpALifeHumanAbstract->brain().objects().choose_food
(&tpBlockedItems); break;
                }
                case 1 : {
                        j	=
tpALifeHumanAbstract->brain().objects().choose_weapon
...
(eWeaponPriorityTypeGrenade,&tpBlockedItems); break;
                ...
                case 5 : {
                        j	=
tpALifeHumanAbstract->brain().objects().choose_medikit
(&tpBlockedItems); break;
                }
                case 6 : {
                        j	=
tpALifeHumanAbstract->brain().objects().choose_detector	(&tpBlockedItems);
                        break;
                }
                case 7 : {
                        j	=
tpALifeHumanAbstract->brain().objects().choose_equipment
(&tpBlockedItems); break;
                }
                default :
NODEFAULT;
...
(ALife::_OBJECT_ID*)alloca(sizeof(ALife::_OBJECT_ID)*iItemCount); std::copy
...
*tpALifeHumanAbstract, ITEM_P_VECTOR &tpItemVector)
{
        ...
        for ( ; I != E; ++I)
                if
(std::find(l_tpChildren.begin(),l_tpChildren.end(),(*I)->base()->ID) !=
...
*tpALifeHumanAbstract, ITEM_P_VECTOR &tpItemVector)
{
...
#endif
        {
                ...
                OBJECT_IT		i =
tpALifeHumanAbstract->children.begin(); #endif for ( ; I != E; ++I) { #ifndef
FAST_OWNERSHIP
...
objects().object(*I); l_tpALifeDynamicObject->ID_Parent = 0xffff; graph().attach
...
INT_VECTOR &tpSums)
{
        ...
        for (int i=0, n=tpTrader.size(); i<n; ++i) {
                ...
                for (;l_iStackPointer;) {
                        ...
                        POP_STACK
(i1,i2,iCurrentSum,m_tpStack1,l_iStackPointer);
...
#else
                                INT_IT I =
std::find(tpSums.begin(),tpSums.end(),iCurrentSum); if (tpSums.end() == I) {
                                        tpSums.push_back(iCurrentSum);
                                        if (tpSums.size() >=
SUM_COUNT_THRESHOLD) { sort		(tpSums.begin(),tpSums.end()); return;
                                        }
                                }
#endif
                                continue;
...
iStartI, int iStackPointer)
{
        for (int j=iStartI, n=tpTrader.size(); j<n; ++j) {
                ...
                PUSH_STACK (j+1,n-1,0,tpStack,iStackPointer); tpIndexes.resize
(j + 2);
                ...
                for (;iStackPointer;) {
                        ...
                        POP_STACK
(i1,i2,iCurrentSum,tpStack,iStackPointer);
...
iSum2, int iMoney2, int iBalance)
{
        ...
        int						l_iStartI1 = 0,
l_iStackPointer1 = 0, l_iStartI2 = 0, l_iStackPointer2 = 0; for (;;) {
                l_tpIndexes1.clear	();
...
(!bfCheckForInventoryCapacity(tpALifeHumanAbstract1,tpTrader1,l_tpIndexes1,tpALifeHumanAbstract2,tpTrader2,l_tpIndexes2))
                        ...
                        if (psAI_Flags.test(aiALife)) {
                                S1				+=
xr_sprintf(S1,"%s -> ",tpALifeHumanAbstract1->name_replace());

                                if (iSum1) {
                                        for (int i=0, n=l_tpIndexes1.size(); i<n
;++i) S1		+= xr_sprintf(S1,"%3d",l_tpIndexes1[i]);
                                }
                        }
#endif
                        if (iSum1 < iBalance + iSum2) {
                                ...
                                if (psAI_Flags.test(aiALife)) {
                                        S1			+=
xr_sprintf(S1," + $%d",iBalance + iSum2 - iSum1);
                                }
#endif
                                R_ASSERT
(int(tpALifeHumanAbstract1->m_dwMoney) >= iBalance + iSum2 - iSum1);
                                tpALifeHumanAbstract1->m_dwMoney -= iBalance +
iSum2 - iSum1; tpALifeHumanAbstract2->m_dwMoney += iBalance + iSum2 - iSum1;
                        }
...
xr_sprintf(S1,"\n%s -> ",tpALifeHumanAbstract2->name_replace());
                                ...
                                if (iSum2) {
                                        for (int i=0, n=l_tpIndexes2.size(); i<n
;++i) S1		+= xr_sprintf(S1,"%3d",l_tpIndexes2[i]);
                                }
                        }
#endif
                        ...
                        if (iSum1 > iBalance + iSum2) {
                                ...
                                if (psAI_Flags.test(aiALife)) {
                                        S1			+=
xr_sprintf(S1," + $%d",iSum1 - iBalance - iSum2);
                                }
#endif
                                R_ASSERT
(int(tpALifeHumanAbstract2->m_dwMoney) >= iSum1 - iBalance - iSum2);
                                tpALifeHumanAbstract1->m_dwMoney += iSum1 -
iBalance - iSum2; tpALifeHumanAbstract2->m_dwMoney -= iSum1 - iBalance - iSum2;
                        }
...
money");
                ...
                vfFillTraderVector
(tpALifeHumanAbstract1,iItemCount1,m_tpTrader1); vfFillTraderVector
(tpALifeHumanAbstract2,iItemCount2,m_tpTrader2);
                ...
                if (psAI_Flags.test(aiALife)) {
                        {
                                ...
                                S1				+=
xr_sprintf(S1,"%s [%5d]:
",tpALifeHumanAbstract1->name_replace(),tpALifeHumanAbstract1->m_dwMoney); for
(int i=0, n=m_tpTrader1.size(); i<n; ++i) S1			+=
...
("%s",S);
                        }
                        {
                                ...
                                S1				+=
xr_sprintf(S1,"%s [%5d]:
",tpALifeHumanAbstract2->name_replace(),tpALifeHumanAbstract2->m_dwMoney); for
(int i=0, n=m_tpTrader2.size(); i<n; ++i) S1			+=
                ...
                if (psAI_Flags.test(aiALife)) {
                        {
                                ...
                                char			*S1 = S;
                                S1				+=
xr_sprintf(S1,"%s : ",tpALifeHumanAbstract1->name_replace()); INT_IT
I = m_tpSums1.begin(); INT_IT			E = m_tpSums1.end(); for ( ; I
...
("%s",S);
                        }
                        {
                                ...
                                char			*S1 = S;
                                S1				+=
xr_sprintf(S1,"%s : ",tpALifeHumanAbstract2->name_replace()); INT_IT
I = m_tpSums2.begin(); INT_IT			E = m_tpSums2.end(); for ( ; I
!= E; ++I) S1			+= xr_sprintf(S1,"%6d",*I); Msg
("%s",S);
                        }
                }
                ...
                if (iBalance < 0)
                        return
(bfCheckForTrade(tpALifeHumanAbstract1, m_tpTrader1, m_tpSums1,
tpALifeHumanAbstract1->m_dwMoney, tpALifeHumanAbstract2, m_tpTrader2, m_tpSums2,
tpALifeHumanAbstract2->m_dwMoney, _abs(iBalance))); else return
(bfCheckForTrade(tpALifeHumanAbstract2, m_tpTrader2, m_tpSums2,
tpALifeHumanAbstract2->m_dwMoney, tpALifeHumanAbstract1, m_tpTrader1, m_tpSums1,
tpALifeHumanAbstract1->m_dwMoney, iBalance));
}
...
{
//	VERIFY
(tpALifeHumanAbstract1->check_inventory_consistency());
//	VERIFY
(tpALifeHumanAbstract2->check_inventory_consistency());
...
#ifdef DEBUG
                if (psAI_Flags.test(aiALife)) {
                        Msg			("There is no money and valuable
items to trade");
                }
#endif
                return;
...
!(tpALifeHumanAbstract1->brain().m_dwTotalMoney +
tpALifeHumanAbstract2->brain().m_dwTotalMoney)) break; int
l_iItemCount1 = 0, l_iItemCount2 = 0; switch (k) { case 0 : {
...
l_iItemCount2,tpALifeHumanAbstract2->children.end(),*I); if
(tpALifeHumanAbstract2->children.end() != J) { if
...
item parent"); m_tpBlockedItems1.push_back(*I);
                ...
                if (m_tpBlockedItems1.empty() && m_tpBlockedItems2.empty()) {
                        if (l_iItemCount1)
                                vfAssignItemParents
(tpALifeHumanAbstract1,l_iItemCount1); if (l_iItemCount2) vfAssignItemParents
(tpALifeHumanAbstract2,l_iItemCount2); if (l_iItemCount1 + l_iItemCount2) {
                                ITEM_P_IT			I =
...
l_iItemCount2 = tpALifeHumanAbstract2->children.size();
        ...
        vfAttachOwnerItems
(tpALifeHumanAbstract1,m_temp_item_vector,m_tpItems1); vfAttachOwnerItems
(tpALifeHumanAbstract2,m_temp_item_vector,m_tpItems2);
...
ifComputeBalance(tpALifeHumanAbstract2,m_tpItems1))) { vfRestoreItems
(tpALifeHumanAbstract1,m_tpItems1); vfRestoreItems
(tpALifeHumanAbstract2,m_tpItems2);
        }
...
(tpALifeHumanAbstract1->check_inventory_consistency());
//	VERIFY
(tpALifeHumanAbstract2->check_inventory_consistency()); #else else {
...
smart_cast<CSE_ALifeGroupAbstract*>(tpALifeHumanAbstract); if
(l_tpALifeAbstractGroup) { OBJECT_IT
I = l_tpALifeAbstractGroup->m_tpMembers.begin(); OBJECT_IT
...
#endif
        CSE_ALifeItemPDA
*original_pda = 0; tpALifeHumanAbstract->brain().m_dwTotalMoney	=
tpALifeHumanAbstract->m_dwMoney;
        {
                OBJECT_IT I = tpALifeHumanAbstract->children.begin(); OBJECT_IT
...
l_dwItemCost = tpALifeTrader->dwfGetItemCost(l_tpALifeInventoryItem);
                        tpALifeHumanAbstract->brain().m_dwTotalMoney +=
l_dwItemCost; tpALifeTrader->m_dwMoney		-= l_dwItemCost;
                }
