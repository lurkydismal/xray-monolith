...
class CActionPlanner : public CProblemSolver< GraphEngineSpace::CWorldProperty,
...
public:
    using CProblemSolver = CProblemSolver< GraphEngineSpace::CWorldProperty,
                                           GraphEngineSpace::CWorldState,
                                           _world_operator,
                                           _condition_evaluator,
                                           u32,
...
public:
    ...
    // Called at the action-switch decision point in update()
    // (action_planner_inline.h), only when a switch is proposed (current !=
    // solution front). Default allows every switch; a derived planner may veto
    // to keep the current action executing. Base behavior = vanilla for every
    // planner.
    virtual bool allow_action_switch(
    ...
    IC void add_condition( _world_operator* action,
                           _condition_type condition_id,
                           _value_type condition_value );
    IC void add_effect( _world_operator* action,
                        _condition_type condition_id,
                        _value_type condition_value );
