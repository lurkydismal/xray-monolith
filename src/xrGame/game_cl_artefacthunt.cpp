...
game_cl_ArtefactHunt::game_cl_ArtefactHunt() {
    ...
    LoadSndMessages();
    //---------------------------------
    m_iSpawn_Cost = READ_IF_EXISTS( pSettings, r_s32, "artefacthunt_gamedata",
                                    "spawn_cost", -10000 );
}
...
void game_cl_ArtefactHunt::Init() {
    ...
    /*	string_path	fn_game;
            if (FS.exist(fn_game, "$level$", "level.game"))
            {
                    ...
                    if (0!=(O = F->open_chunk	(RPOINT_CHUNK)))
                    {
                            ...
                            {
                                    RPoint R; u8
       RP_team; u8						RP_type; u16
       RP_GameType;

                                    O->r_fvector3			(R.P);
                                    O->r_fvector3			(R.A);
                                    RP_team
       = O->r_u8	();	VERIFY(RP_team>=0 && RP_team<4); RP_type
       = O->r_u8	(); RP_GameType				= O->r_u16
       ();
                                    //u16 res =
                                    //O->r_u8	();

                                    if (RP_GameType != GAME_ANY && RP_GameType
       != GAME_ARTEFACTHUNT)
                                    {
                                            continue;
                                    };
                                    ...
                                    case rptTeamBaseParticle:
                                            {
                                                    ...
                                                    xr_sprintf(ParticleStr,
       "teambase_particle_%d", RP_team); if
       (pSettings->line_exist("artefacthunt_gamedata", ParticleStr))
                                                    {
                                                            Fmatrix
       transform; transform.identity(); transform.setXYZ(R.A);
                                                            transform.translate_over(R.P);
                                                            CParticlesObject*
       pStaticParticles			=
       Particles::Details::Create(pSettings->r_string("artefacthunt_gamedata",
       ParticleStr),FALSE,false).get(); pStaticParticles->UpdateParent
       (transform,zero_vel); pStaticParticles->Play			();
                                                            Level().m_StaticParticles.push_back
       (pStaticParticles);
                                                    };
                                            }break;
                                    };
    ...
    //-------------------------------------------------------
    if ( pSettings->line_exist( "artefacthunt_gamedata",
                                "artefact_spawn_effect" ) )
        m_Eff_Af_Spawn = pSettings->r_string( "artefacthunt_gamedata",
                                              "artefact_spawn_effect" );
    if ( pSettings->line_exist( "artefacthunt_gamedata",
                                "artefact_disappear_effect" ) )
        m_Eff_Af_Disappear = pSettings->r_string( "artefacthunt_gamedata",
                                                  "artefact_disappear_effect" );
};
...
void game_cl_ArtefactHunt::TranslateGameMessage( u32 msg, NET_Packet& P ) {
    ...
    switch ( msg ) {
        ...
        case GAME_EVENT_ARTEFACT_TAKEN: // ahunt
        {
            ...
            xr_sprintf( tmp, "%s%s", "%s%s %s",
                        *st.translate( "mp_has_tak_art" ) );

            xr_sprintf( Text, tmp, CTeamInfo::GetTeam_color_tag( int( Team ) ),
                        pPlayer->getName(), Color_Main, Color_Artefact );
            ...
            if ( Game().local_player->GameID == PlayerID )
                PlaySndMessage( ID_AF_TEAM1_TAKE +
                                ModifyTeam( Game().local_player->team ) );
        ...
        case GAME_EVENT_ARTEFACT_ONBASE: // ahunt
        {
            ...
            xr_sprintf( Text, tmp, CTeamInfo::GetTeam_color_tag( int( Team ) ),
                        CTeamInfo::GetTeam_name( int( Team ) ), Color_Main );
            if ( CurrentGameUI() )
                CurrentGameUI()->CommonMessageOut( Text );
...
void game_cl_ArtefactHunt::GetMapEntities(
    xr_vector< SZoneMapEntityData >& dst ) {
    ...
    if ( pParent && pParent->ID() == artefactBearerID &&
         GetPlayerByGameID( artefactBearerID ) ) {
        ...
        ( ps->team == local_team ) ? D.color = color_friend_with_artefact
                                   : D.color = color_enemy_with_artefact;
                ...
                if ( smart_cast< CActor* >( Level().CurrentControlEntity() ) ) {
                    ...
                    if ( m_bBuyEnabled ) {
                        if ( !( pCurBuyMenu && pCurBuyMenu->IsShown() ) &&
                            ...
                            xr_sprintf( msg, *st.translate( "mp_press_to_buy" ),
                                        "B" );
                            if ( m_game_ui )
                    ...
                    if ( m_bTeamSelected && m_bSkinSelected ) {
                        if ( iReinforcementTime != 0 ) {
                            if ( !m_game_ui->m_pBuySpawnMsgBox->IsShown() &&
                                 ( local_player->money_for_round +
                                   m_iSpawn_Cost ) >= 0 ) {
                                if ( m_game_ui )
                                    m_game_ui->SetPressJumpMsgCaption(
                                        "mp_press_jump2pay_spaw" );
                            } else {
                                if ( m_game_ui )
                                    m_game_ui->SetPressJumpMsgCaption( NULL );
                            }
                        ...
                        if ( !m_bTeamSelected )
                            if ( m_game_ui )
                                m_game_ui->SetPressJumpMsgCaption(
                                    "mp_press_jump2select_team" );
                            else if ( !m_bSkinSelected )
                                if ( m_game_ui )
                                    m_game_ui->SetPressJumpMsgCaption(
                                        "mp_press_jump2select_skin" );
                    }
                };
            }
            ...
            if ( local_player ) {
                ...
                if ( dReinforcementTime > 0 && Level().CurrentViewEntity() &&
                    ...
                    if ( s32( CurTime ) > dReinforcementTime )
                        ...
                        dTime = iCeil( float( dReinforcementTime - CurTime ) /
                                       1000 );

                    string128 _buff;
                    m_game_ui->m_pReinforcementInidcator->SetText(
                        itoa( dTime, _buff, 10 ) );
                } else
...
void game_cl_ArtefactHunt::SetScore() {
    ...
    if ( Level().CurrentViewEntity() && m_game_ui ) {
        game_PlayerState* ps =
            GetPlayerByGameID( Level().CurrentViewEntity()->ID() );
...
BOOL game_cl_ArtefactHunt::CanCallBuyMenu() {
    ...
    if ( m_game_ui->m_pUITeamSelectWnd &&
...
void game_cl_ArtefactHunt::UpdateMapLocations() {
    ...
    if ( local_player ) {
        if ( !artefactID ) {
            ...
            if ( !artefactBearerID ) {
                if ( !Level().MapManager().HasMapLocation( ARTEFACT_NEUTRAL,
                                                           artefactID ) ) {
                    Level().MapManager().RemoveMapLocationByObjectID(
                        artefactID );
                    ( Level().MapManager().AddMapLocation( ARTEFACT_NEUTRAL,
                                                           artefactID ) )
                        ->EnablePointer();
                };
            } else {
                if ( teamInPossession == local_player->team ) {
                    if ( !Level().MapManager().HasMapLocation( ARTEFACT_FRIEND,
                                                               artefactID ) ) {
                        Level().MapManager().RemoveMapLocationByObjectID(
                            artefactID );
                        ( Level().MapManager().AddMapLocation( ARTEFACT_FRIEND,
                                                               artefactID ) )
                            ->EnablePointer();
                    }
                } else {
                    if ( !Level().MapManager().HasMapLocation( ARTEFACT_ENEMY,
                                                               artefactID ) ) {
                        Level().MapManager().RemoveMapLocationByObjectID(
                            artefactID );
                    };
                    ...
                    CActor* pActor =
                    smart_cast<CActor*>(Level().Objects.net_Find(artefactBearerID));
                    if (pActor)
                    {
                            CCustomOutfit* pOutfit			=
                    pActor->GetOutfit(); if (pOutfit && pOutfit->CLS_ID ==
                    CLSID_EQUIPMENT_SCIENTIFIC)
                            {
                                    if (!pActor->AnyAction())
                                    {
                                            if
                    (Level().MapManager().HasMapLocation(ARTEFACT_ENEMY,
                    artefactID))
                                            {
                    ...
                    if ( !Level().MapManager().HasMapLocation( ARTEFACT_ENEMY,
                                                               artefactID ) ) {
                        ( Level().MapManager().AddMapLocation( ARTEFACT_ENEMY,
                                                               artefactID ) )
                            ->EnablePointer();
                    }
                }
...
bool game_cl_ArtefactHunt::NeedToSendReady_Spectator( int key,
                                                      game_PlayerState* ps ) {
    CStringTable st;
    ...
    if ( ( Phase() == GAME_PHASE_INPROGRESS ) && ( kJUMP == key ) &&
         m_cl_dwWarmUp_Time ) {
        return res;
    }
    ...
    if ( ( GAME_PHASE_INPROGRESS == Phase() ) && ( kJUMP == key ) &&
        ...
        xr_sprintf( BuySpawnText, *st.translate( "mp_press_yes2pay" ),
                    abs( local_player->money_for_round ),
                    abs( m_iSpawn_Cost ) );
...
void game_cl_ArtefactHunt::LoadSndMessages() {
    ...
    LoadSndMessage( "ahunt_snd_messages", "team1_artefact_on_base",
                    ID_AF_TEAM1_ONBASE );
    LoadSndMessage( "ahunt_snd_messages", "team2_artefact_on_base",
                    ID_AF_TEAM2_ONBASE );
    LoadSndMessage( "ahunt_snd_messages", "team1_artefact_on_base_r",
                    ID_AF_TEAM1_ONBASE_R );
    LoadSndMessage( "ahunt_snd_messages", "team2_artefact_on_base_r",
                    ID_AF_TEAM2_ONBASE_R );
    LoadSndMessage( "ahunt_snd_messages", "team1_artefact_on_base_enemy",
                    ID_AF_TEAM1_ONBASE_ENEMY );
    LoadSndMessage( "ahunt_snd_messages", "team2_artefact_on_base_enemy",
                    ID_AF_TEAM2_ONBASE_ENEMY );

    LoadSndMessage( "ahunt_snd_messages", "team1_artefact_take",
                    ID_AF_TEAM1_TAKE );
    LoadSndMessage( "ahunt_snd_messages", "team2_artefact_take",
                    ID_AF_TEAM2_TAKE );
    LoadSndMessage( "ahunt_snd_messages", "team1_artefact_take_r",
                    ID_AF_TEAM1_TAKE_R );
    LoadSndMessage( "ahunt_snd_messages", "team2_artefact_take_r",
                    ID_AF_TEAM2_TAKE_R );
    LoadSndMessage( "ahunt_snd_messages", "team1_artefact_take_enemy",
                    ID_AF_TEAM1_TAKE_ENEMY );
    LoadSndMessage( "ahunt_snd_messages", "team2_artefact_take_enemy",
...
void game_cl_ArtefactHunt::OnSellItemsFromRuck() {
    if ( !local_player ||
        ...
        return;
    CActor* pCurActor = smart_cast< CActor* >(
        Level().Objects.net_Find( local_player->GameID ) );
    if ( !pCurActor )
        return;

    TIItemContainer::const_iterator IRuck =
        pCurActor->inventory().m_ruck.begin();
