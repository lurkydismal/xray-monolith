...
UITeamHeader::UITeamHeader( UITeamState const* const parent )
    : m_parent( parent ), m_team_header_root( NULL ) {}
...
void UITeamHeader::Update() {
    ...
    for ( FieldsStatics::iterator i = m_field_fillers.begin(); i != ie; ++i ) {
        ...
        VERIFY2( fieldStatic,
                 make_string( "field %s not initialized", i->first.c_str() )
                     .c_str() );
        STRING_VALUE temp_text = m_translated_strings[ i->first ];
        buffer_vector< char >::size_type new_size =
            temp_text.size() + 16; // i hope STRING_VALUE has size() method :)
        buffer_vector< char > new_string( _alloca( new_size ), new_size );
        xr_sprintf( new_string.begin(), new_size, "%s: %d", temp_text.c_str(),
                    temp_value );
...
void UITeamHeader::InitColumnsStatics( CUIXml& uiXml ) {
    ...
    for ( int i = 0; i < tempNumber; ++i ) {
        ...
        if ( !tempColumnNode )
            break;
        LPCSTR tempColumnName = uiXml.ReadAttrib(
            tempColumnNode, "name", "column_not_set_in_name_attribute" );
        ...
        m_columns_statics.insert(
            std::make_pair( shared_str( tempColumnName ), tempColumn ) );
    }
}
...
void UITeamHeader::InitFieldsStatics( CUIXml& uiXml ) {
    ...
    for ( int i = 0; i < tempNumber; ++i ) {
        ...
        if ( !tempFieldNode )
            break;
        LPCSTR tempFieldName = uiXml.ReadAttrib(
            tempFieldNode, "name", "field_not_set_in_name_attribute" );
        ...
        m_translated_strings.insert( std::make_pair(
            shared_str( tempFieldName ), st.translate( tempFieldName ) ) );
        m_field_fillers.insert(
            std::make_pair( shared_str( tempFieldName ), tempField ) );
    }
}
