...
struct FindLocationBySpotID {
    ...
    FindLocationBySpotID( const shared_str& s, u16 id )
        : spot_id( s ), object_id( id ) {}
...
CMapLocation* CMapManager::AddMapLocation( const shared_str& spot_type,
                                           u16 id ) {
    ...
    if ( IsGameTypeSingle() && g_actor )
        Actor()->callback( GameObject::eMapLocationAdded )( spot_type.c_str(),
                                                            id );
...
CMapLocation* CMapManager::AddRelationLocation( CInventoryOwner* pInvOwner ) {
    ...
    CInventoryOwner* pActor =
        smart_cast< CInventoryOwner* >( Level().CurrentViewEntity() );
    ...
    CMapLocation* l = xr_new< CRelationMapLocation >(
        sname, pInvOwner->object_id(), pActor->object_id() );
...
void CMapManager::RemoveRelationLocation( CInventoryOwner* pInvOwner ) {
    for ( int t = ALife::eRelationTypeFriend; t < ALife::eRelationTypeLast;
          ++t ) {
        ALife::ERelationType tt = ( ALife::ERelationType )t;
        Level().MapManager().RemoveMapLocation(
            RELATION_REGISTRY().GetSpotName( tt ), pInvOwner->object_id() );
    }
    Level().MapManager().RemoveMapLocation( "deadbody_location",
                                            pInvOwner->object_id() );
}
...
void CMapManager::RemoveMapLocation( const shared_str& spot_type, u16 id ) {
    ...
    Locations_it it =
        std::find_if( Locations().begin(), Locations().end(), key );
    if ( it != Locations().end() ) {
        if ( IsGameTypeSingle() )
...
void CMapManager::RemoveAllMapLocationsById( u16 id ) {
    for ( Locations_it it = Locations().begin(); it != Locations().end(); ) {
        if ( it->object_id == id ) {
            if ( IsGameTypeSingle() )
                Level().GameTaskManager().MapLocationRelcase(
                    ( *it ).location );
            Destroy( ( *it ).location );
...
void CMapManager::RemoveMapLocationByObjectID(
    u16 id ) // call on destroy object
{
...
bool CMapManager::GetMapLocationsForObject( u16 id,
...
CMapLocation* CMapManager::GetMapLocation( const shared_str& spot_type,
                                           u16 id ) {
    FindLocationBySpotID key( spot_type, id );
    Locations_it it =
        std::find_if( Locations().begin(), Locations().end(), key );
...
void CMapManager::GetMapLocations( const shared_str& spot_type,
                                   u16 id,
                                   xr_vector< CMapLocation* >& res ) {
    FindLocationBySpotID key( spot_type, id );
    Locations_it it =
...
void CMapManager::Dump() {
    ...
    Locations_it it_e = Locations().end();
    for ( ; it != it_e; ++it ) {
        Msg( "spot_type=[%s] object_id=[%d]", *( ( *it ).spot_type ),
             ( *it ).object_id );
        ( *it ).location->Dump();
    }
