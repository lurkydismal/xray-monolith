...
LPSTR remove_version_option( LPCSTR opt_str,
                             LPSTR new_opt_str,
                             u32 new_opt_str_size ) {
    LPCSTR temp_substr = strstr( opt_str, map_ver_string );
    if ( !temp_substr ) {
        xr_strcpy( new_opt_str, new_opt_str_size, opt_str );
        return new_opt_str;
    }
    strncpy_s( new_opt_str, new_opt_str_size, opt_str,
               static_cast< size_t >( temp_substr - opt_str - 1 ) );
...
static bool SimmulateNetworkLag() {
    ...
    if ( !max_lag_time || ( max_lag_time <= Device.dwTimeGlobal ) ) {
        ...
        max_lag_time =
            Device.dwTimeGlobal + tmp_random.randI( lag_simmulator_min_ping,
                                                    lag_simmulator_max_ping );
        return false;
    }
    return true;
}
    ...
    for ( NET_Packet* P = net_msg_Retreive(); P; P = net_msg_Retreive() ) {
        ...
        switch ( m_type ) {
            case M_SPAWN: {
                ...
                {
                    Msg( "! Unconventional M_SPAWN received : map_data[%s] | "
                         "bReady[%s] | deny_m_spawn[%s]",
            ...
            case M_EVENT_PACK: {
                /*if (!game_configured)
                {
                        Msg("! WARNING: ignoring game event [%d] - game not
                configured...", m_type); break;
                }*/
                NET_Packet tmpP;
                xrSRWLockGuard g( prefetch_lock );
                while ( !P->r_eof() ) {
            ...
            case M_UPDATE_OBJECTS: {
                ...
                if ( ( Level().timeServer() + pStat.getPing() ) <
                     P->timeReceive ) {
                    dTime = pStat.getPing();
                } else
                    dTime =
                        Level().timeServer() - P->timeReceive + pStat.getPing();
            ...
            case M_CL_UPDATE: {
                ...
                u32 Ping = P->r_u32();
                CGameObject* O =
                    smart_cast< CGameObject* >( Objects.net_Find( ID ) );
                if ( 0 == O )
                    break;
                O->net_Import( *P );
                //---------------------------------------------------
                ...
                O->CrPr_SetActivationStep( u32( physics_world()->StepsNum() ) -
                                           NumSteps );
            ...
            case M_MOVE_PLAYERS: {
                ...
                // [08.11.07] Alexander Maniluk: added new message handler for
                // moving artefacts.
            case M_MOVE_ARTEFACTS: {
                ...
                for ( u8 i = 0; i < Count; ++i ) {
                    ...
                    P->r_vec3( NewPos );
                    CArtefact* OArtefact =
                        smart_cast< CArtefact* >( Objects.net_Find( ID ) );
                    if ( !OArtefact )
                        break;
            ...
            case M_CHANGE_LEVEL_GAME: {
                ...
                if ( OnClient() ) {
                    ...
                    xr_sprintf( NewServerOptions, "%s/%s/%s%s",
                                LevelName.c_str(), GameType.c_str(),
                                map_ver_string, LevelVersion.c_str() );

                    if ( m_SO ) {
                        string4096 additional_options;
                        xr_strcat( NewServerOptions, sizeof( NewServerOptions ),
                                   remove_version_option(
                                       m_SO, additional_options,
                                       sizeof( additional_options ) ) );
                    }
            ...
            case M_STATISTIC_UPDATE_RESPOND: // deprecated, see
                                             // xrServer::OnMessage
            {
