...
struct CDangerPredicate {
    ...
    IC CDangerPredicate( const CObject* object ) { m_object = object; }
...
void CDangerManager::update() {
    ...
    m_objects.erase(
        std::remove_if( m_objects.begin(), m_objects.end(),
                        CRemoveByTimePredicate( time_line(), this ) ),
        m_objects.end() );
    ...
    for ( ; I != E; ++I ) {
        //		Msg					("%6d : Danger :
        //[%d][%d]",(*I).time(),(*I).type(),(*I).perceive_type());
...
void CDangerManager::remove_links( const CObject* object ) {
    ...
    IGNORED::iterator I =
        std::find( m_ignored.begin(), m_ignored.end(), object->ID() );
    if ( I != m_ignored.end() )
...
bool CDangerManager::useful( const CDangerObject& object ) const {
    if ( object.object() && !object.dependent_object() ) {
        IGNORED::const_iterator I = std::find(
            m_ignored.begin(), m_ignored.end(), object.object()->ID() );
        if ( I != m_ignored.end() )
            return ( false );
    }
...
void CDangerManager::add( const CVisibleObject& object ) {
    ...
    const CEntityAlive* obj =
        smart_cast< const CEntityAlive* >( object.m_object );
    if ( obj && !obj->g_Alive() &&
         ( obj->killer_id() != ALife::_OBJECT_ID( -1 ) ) ) {
        add( CDangerObject( obj, obj->Position(), object.m_level_time,
                            CDangerObject::eDangerTypeFreshEntityCorpse,
...
void CDangerManager::add( const CSoundObject& object ) {
    ...
    if ( ( object.m_sound_type & SOUND_TYPE_BULLET_HIT ) ==
        ...
        add( CDangerObject( obj, object.m_object_params.m_position,
                            object.m_level_time,
...
void CDangerManager::add( const CHitObject& object ) {
    ...
    const CEntityAlive* obj =
        smart_cast< const CEntityAlive* >( object.m_object );
    add( CDangerObject( obj, obj->Position(), object.m_level_time,
                        CDangerObject::eDangerTypeAttacked,
...
void CDangerManager::add( const CDangerObject& object ) {
    ...
    if ( m_object->memory().enemy().selected() &&
         object.object() ) // && !object.object()->g_Alive())
        ignore( object.object() );

    if ( !is_useful( object ) )
        return;

    OBJECTS::iterator I = std::find_if( m_objects.begin(), m_objects.end(),
                                        CFindPredicate( object ) );
    if ( I != m_objects.end() ) {
        *I = object;
        return;
    }
