...
template <
    ...
    template < typename _algorithm,
               ...
               template < typename _1,
                          ...
                          template < typename _1, typename _2 > class _4 > class
               _manager_builder_allocator_constructor =
                   CManagerBuilderAllocatorConstructor > class
    _data_storage_constructor = CDataStorageConstructor,
...
public:
    ...
    struct _Vertex : public _vertex< T1 > {
        ...
        IC _dist_type& f() { return ( _f ); }

        IC const _dist_type& f() const { return ( _f ); }

        IC T1*& back() { return ( _back ); }
    };

    typedef _data_storage_constructor< _priority_queue,
                                       _vertex_manager,
                                       _data_storage_base,
                                       _vertex_allocator,
                                       _Vertex,
