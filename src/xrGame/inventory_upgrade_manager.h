...
public:
    typedef associative_vector< shared_str, Property*, shared_str_predicate >
        Properties_type;

private:
    typedef associative_vector< shared_str, Root*, shared_str_predicate >
        Roots_type;
    typedef associative_vector< shared_str, Group*, shared_str_predicate >
        Groups_type;
    typedef associative_vector< shared_str, Upgrade*, shared_str_predicate >
        Upgrades_type;
...
public:
    ...
    bool make_known_upgrade( CInventoryItem& item,
    ...
    void highlight_hierarchy( CInventoryItem& item,
                              shared_str const& upgrade_id );
...
private:
    ...
    Upgrade* upgrade_verify( shared_str const& item_section,
                             shared_str const& upgrade_id );
    static bool item_upgrades_exist( shared_str const& item_id );

    void compute_range_section( LPCSTR section,
                                LPCSTR parameter,
                                float& low,
                                float& high );
