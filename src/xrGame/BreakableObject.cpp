...
void CBreakableObject::Load( LPCSTR section ) {
    ...
    m_damage_threshold =
        pSettings->r_float( section, "collision_break_threthhold" );
...
BOOL CBreakableObject::net_Spawn( CSE_Abstract* DC ) {
    ...
    CSE_ALifeObjectBreakable* obj =
        smart_cast< CSE_ALifeObjectBreakable* >( e );
    R_ASSERT( obj );
...
void CBreakableObject::shedule_Update( u32 dt ) {
    ...
    if ( m_pPhysicsShell && !bRemoved &&
         Device.dwTimeGlobal - m_break_time > m_remove_time )
        SendDestroy();
}
    ...
    if ( m_pPhysicsShell ) {
        if ( pHDS->hit_type == ALife::eHitTypeExplosion ) {
            ...
            if ( ( pHDS->impulse > EPS ) && ( BI_NONE != pHDS->bone() ) )
                m_pPhysicsShell->applyImpulseTrace( pHDS->p_in_bone_space,
                                                    pHDS->dir, pHDS->impulse,
                                                    pHDS->bone() );
        }
    }
}
...
void CBreakableObject::CreateUnbroken() {
    m_pUnbrokenObject =
        P_BuildStaticGeomShell( ( this ), BreakableObjectCollisionCallback );
}
...
void CBreakableObject::CreateBroken() {
    ...
    m_Shell->SetMaxAABBRadius(
        _max( _max( b.m_halfsize.x, b.m_halfsize.y ), b.m_halfsize.z ) *
        2.f ); //+2.f
}
...
void CBreakableObject::Break() {
    ...
    for ( u16 i = 0; i < el_num; i++ ) {
        Fvector pos, dir;
        pos.set( Random.randF( -0.3f, 0.3f ), Random.randF( -0.3f, 0.3f ),
                 Random.randF( -0.3f, 0.3f ) );
        dir.set( Random.randF( -0.3f, 0.3f ), Random.randF( -0.3f, 0.3f ),
                 Random.randF( -0.3f, 0.3f ) );
        dir.normalize();
        m_pPhysicsShell->get_ElementByStoreOrder( i )->applyImpulseTrace(
            pos, dir, Random.randF( 0.5f, 3.f ), 0 );
    }
...
void CBreakableObject::CollisionHit( u16 source_id,
                                     u16 bone_id,
                                     float c_damage,
                                     const Fvector& dir,
                                     Fvector& pos ) {
...
void CBreakableObject::ApplyExplosion( const Fvector& dir, float impulse ) {
    ...
    for ( u16 i = 0; i < el_num; i++ ) {
        ...
        CPhysicsElement* element =
            m_pPhysicsShell->get_ElementByStoreOrder( i );
