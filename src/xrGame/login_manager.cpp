...
void login_manager::login_offline( char const* nick,
                                   login_operation_cb logincb ) {
...
void __stdcall login_manager::only_log_login( profile const* res_profile,
    ...
    Msg( "* GameSpy login operation success ! Hello %s !",
         res_profile->m_unique_nick.c_str() );
}
...
void __cdecl login_manager::setunick_cb( GPConnection* connection,
    ...
    // GPRegisterUniqueNickResponseArg* tmp_res =
    // static_cast<GPRegisterUniqueNickResponseArg*>(arg); VERIFY(my_inst);
    // VERIFY(tmp_res);
...
void __cdecl login_manager::login_cb( GPConnection* connection,
    ...
    // GPResult tmp_lticket_res =
    // my_inst->m_gamespy_gp->GetLoginTicket(tmp_ticket_dest);
...
void login_manager::save_password_to_registry( char const* password ) {
    ...
    // WriteRegistry_BinaryValue(REGISTRY_VALUE_USERPASSWORD, buffer,
    // buffer_size);
}
