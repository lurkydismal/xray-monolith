...
void player_team_win_score::save_round_scores() {
    ...
    switch ( Game().Type() ) {
        case eGameIDCaptureTheArtefact: {
            game_cl_CaptureTheArtefact* tmp_game =
                smart_cast< game_cl_CaptureTheArtefact* >( Level().game );
        ...
        case eGameIDTeamDeathmatch: {
            game_cl_TeamDeathmatch* tmp_game =
                smart_cast< game_cl_TeamDeathmatch* >( Level().game );
            m_green_team_score = tmp_game->GetGreenTeamScore();
            m_blue_team_score = tmp_game->GetBlueTeamScore();
            if ( tmp_local_player->team > 0 ) {
                m_player_team = static_cast< u8 >(
                    tmp_game->ModifyTeam( tmp_local_player->team ) );
            }
        } break;
        case eGameIDDeathmatch: {
            game_cl_Deathmatch* tmp_game =
                smart_cast< game_cl_Deathmatch* >( Level().game );
            if ( !xr_strcmp( tmp_local_player->getName(),
                             tmp_game->WinnerName ) ) {
    ...
    if ( static_cast< ETeam >( m_player_team ) == etGreenTeam ) {
        m_win_score =
            ( m_green_team_score > m_blue_team_score ) ? m_green_team_score : 0;
    } else if ( static_cast< ETeam >( m_player_team ) == etBlueTeam ) {
        m_win_score =
            ( m_blue_team_score > m_green_team_score ) ? m_blue_team_score : 0;
    }
}

player_enemy_team_score::player_enemy_team_score(
    game_state_accumulator* owner )
    : inherited( owner ) {
...
player_runtime_win_score::player_runtime_win_score(
    game_state_accumulator* owner )
    : inherited( owner ) {}
...
void player_runtime_win_score::OnPlayerBringArtefact(
    game_PlayerState const* ps ) {
    save_round_scores();
}

player_runtime_enemy_team_score::player_runtime_enemy_team_score(
    game_state_accumulator* owner )
    : inherited( owner ) {}

void player_runtime_enemy_team_score::OnPlayerBringArtefact(
    game_PlayerState const* ps ) {
    save_round_scores();
}
} // namespace award_system
