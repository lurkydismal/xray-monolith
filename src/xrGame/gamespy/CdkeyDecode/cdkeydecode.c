...
#define MAX_ENCODED_KEY \
    ( ( ( MAX_EXTRA_DATA_LENGTH + KEY_LENGTH + CHECK_LENGTH ) * 8 + 4 ) / 5 )
#include <string.h>
...
int DecodeKeyData( const char* key, unsigned char* extradata ) {
    ...
    unsigned char
        keyandcheck[ MAX_EXTRA_DATA_LENGTH + KEY_LENGTH + CHECK_LENGTH ];
    int keybytes;
    int extradatalen;
    int i;
    ...
    if ( !CleanForBase32( cleankey, key, MAX_ENCODED_KEY + 1 ) )
        return 0;
    keybytes = ConvertFromBase32( ( char* )keyandcheck, cleankey,
                                  ( int )strlen( cleankey ) );
    if ( keybytes <= 0 )
    ...
    for ( i = 0; i < extradatalen; i++ )
        extradata[ i ] =
            keyandcheck[ i ] ^ keyandcheck[ extradatalen + ( i % KEY_LENGTH ) ];
    return extradatalen;
}

static unsigned short CreateCheck( unsigned char* key,
                                   int keylen,
                                   unsigned short cskey ) {
    int i;
...
int VerifyClientCheck( const char* key, unsigned short cskey ) {
    ...
    return ( correctcheck ==
             *( unsigned short* )( keyandcheck + extradatalen + KEY_LENGTH ) )
               ? 1
               : 0;
}
