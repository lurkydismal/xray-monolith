...
void reward_manager::load_rewards() {
    ...
    STRCONCAT( section_name, section_name_prefix,
...
void reward_manager::load_reward_item( CInifile& reward_config,
    ...
    tmp_descriptor->m_texture_name =
        reward_config.r_string( section, "ingame_texture" );
    tmp_descriptor->m_color_animation =
        reward_config.r_string( section, "color_animation" );
    tmp_descriptor->m_width =
        reward_config.r_u32( section, "ingame_texture_width" );
    tmp_descriptor->m_height =
        reward_config.r_u32( section, "ingame_texture_height" );
    ...
    tmp_descriptor->m_process_time =
        reward_config.r_u32( section, "reward_time" );

#ifdef DEBUG
    award_name_searcher tmp_award_searcher;
    tmp_award_searcher.m_award_name = tmp_descriptor->m_award_name;
    VERIFY2( std::find_if( m_rewards_map.begin(), m_rewards_map.end(),
...
void reward_manager::process_reward( u32 const award_id ) {
    ...
    tmp_ui_mp_game->AddAchivment(
        tmp_iter->second->m_texture_name, tmp_iter->second->m_color_animation,
