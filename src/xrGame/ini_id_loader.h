...
// ITEM_DATA		-	структура с полями id и index типа T_ID и
// T_INDEX, 					обязательно имеет конструктор с
// параметрами (T_INDEX index, T_ID id,
// LPCSTR r1, ..., LPCSTR rN) 					N = ITEM_REC_NUM
// - число доп. параметров в ITEM_DATA

// T_INIT		-	класс где определена статическая InitIdToIndex
//					функция инициализации section_name и
// line_name

#define TEMPLATE_SPECIALIZATION                                     \
    template < u32 ITEM_REC_NUM, typename ITEM_DATA, typename T_ID, \
               typename T_INDEX, typename T_INIT >
#define CSINI_IdToIndex \
...
protected:
    ...
    static void LoadItemData( u32, LPCSTR ) {
        STATIC_CHECK(
            false,
            Specialization_for_LoadItemData_in_CIni_IdToIndex_not_found );
        NODEFAULT;
    }
    ...
    static void LoadItemData< 0 >( u32 count, LPCSTR cfgRecord ) {
        for ( u32 k = 0; k < count; k += 1 ) {
            ...
            ITEM_DATA item_data( T_INDEX( m_pItemDataVector->size() ),
                                 T_ID( id_str ) );
...
public:
    ...
    static const T_INDEX IdToIndex( const T_ID& str_id,
                                    T_INDEX default_index = T_INDEX( -1 ),
                                    bool no_assert = false ) {
        const ITEM_DATA* item = GetById( str_id, no_assert );
        return item ? item->index : default_index;
    }

    static const T_ID IndexToId( T_INDEX index,
                                 T_ID default_id = NULL,
                                 bool no_assert = false ) {
...
TEMPLATE_SPECIALIZATION
const typename ITEM_DATA* CSINI_IdToIndex::GetById( const T_ID& str_id,
                                                    bool no_assert ) {
...
TEMPLATE_SPECIALIZATION
const typename ITEM_DATA* CSINI_IdToIndex::GetByIndex( T_INDEX index,
                                                       bool no_assert ) {
    if ( ( size_t )index >= m_pItemDataVector->size() ) {
        if ( !no_assert )
            Debug.fatal( DEBUG_INFO,
                         "item by index not found in section %s, line %s",
