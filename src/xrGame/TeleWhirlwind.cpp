...
CTelekineticObject* CTeleWhirlwind::activate( CPhysicsShellHolder* obj,
                                              float strength,
                                              float height,
                                              u32 max_time_keep,
                                              bool rot ) {
    if ( inherited::activate( obj, strength, height, max_time_keep, rot ) ) {
        ...
        if ( smart_cast< CTeleWhirlwindObject* >( o ) ) {
            smart_cast< CTeleWhirlwindObject* >( o )->set_throw_power(
                m_fThrowPower );
        } else if ( smart_cast< CTeleTrampolinObject* >( o ) ) {
            smart_cast< CTeleTrampolinObject* >( o )->set_throw_power(
                m_fThrowPower );
        }
        return o;
    } else
...
void CTeleWhirlwind::Load( LPCSTR section ) {
    ...
    m_pTearingSound.create(
        pSettings->r_string( section, "body_tearing_sound" ), st_Effect,
        sg_SourceType );
    m_sTearingParticles =
        shared_str( pSettings->r_string( section, "tearing_particles" ) );
...
void CTeleWhirlwind::clear_notrelevant() {
    ...
    objects.erase(
        std::remove_if( objects.begin(), objects.end(), &RemovePred ),
        objects.end() );
}
...
bool CTeleWhirlwindObject::init( CTelekinesis* tele,
...
void CTeleWhirlwindObject::release() {
    if ( !object || object->getDestroy() || !object->m_pPhysicsShell ||
...
void CTeleWhirlwindObject::raise( float step ) {
    ...
    for ( u16 element = 0; element < element_number; ++element ) {
        ...
        Fvector force;
        force.set( dir );
        if ( predict_mag > mag && predict_vel.dotproduct( dir ) > 0.f &&
             predict_v > predict_v_eps ) {
            Fvector motion_dir;
        ...
        E->applyForce(
            force.x, force.y + get_object()->EffectiveGravity() * E->getMass(),
            force.z );
    }
    Fvector dist;
    dist.sub( center, maxE->mass_Center() );
    if ( dist.magnitude() < m_pTelekinesis->GetKeepRadius() &&
         m_bDestroyable ) {
...
bool CTeleTrampolinObject::init( CTelekinesis* tele,
                                 CPhysicsShellHolder* obj,
                                 float s,
                                 float h,
                                 u32 ttk,
...
void CTeleTrampolinObject::raise( float step ) {
    ...
    float angularVelocity =
        clampF( timeRaisePassed * rotateSpeed, 0.f, rotateSpeed );
