...
#define TEMPLATE_SPECIALIZATION                                 \
    template < typename _data_type, typename _edge_weight_type, \
               typename _vertex_id_type, typename _edge_data_type >

#define CAbstractGraph                                              \
    CGraphAbstract< _data_type, _edge_weight_type, _vertex_id_type, \
                    _edge_data_type >
...
TEMPLATE_SPECIALIZATION
IC void CAbstractGraph::add_vertex( const _data_type& data,
                                    const _vertex_id_type& vertex_id ) {
    VERIFY( !vertex( vertex_id ) );
    m_vertices.insert( std::make_pair(
        vertex_id, xr_new< CVertex >( data, vertex_id, &m_edge_count ) ) );
}
...
TEMPLATE_SPECIALIZATION
IC void CAbstractGraph::add_edge( const _vertex_id_type& vertex_id0,
...
TEMPLATE_SPECIALIZATION
IC void CAbstractGraph::remove_edge( const _vertex_id_type& vertex_id0,
                                     const _vertex_id_type& vertex_id1 ) {
...
TEMPLATE_SPECIALIZATION
IC const typename CAbstractGraph::CVertex* CAbstractGraph::vertex(
    const _vertex_id_type& vertex_id ) const {
...
TEMPLATE_SPECIALIZATION
IC typename CAbstractGraph::CVertex* CAbstractGraph::vertex(
    const _vertex_id_type& vertex_id ) {
    vertex_iterator I = m_vertices.find( vertex_id );
    if ( m_vertices.end() == I )
        return ( 0 );
...
TEMPLATE_SPECIALIZATION
IC const _edge_weight_type
CAbstractGraph::get_edge_weight( const _vertex_id_type vertex_index0,
...
TEMPLATE_SPECIALIZATION
IC bool CAbstractGraph::is_accessible(
    const _vertex_id_type vertex_index ) const {
    return ( true );
}

TEMPLATE_SPECIALIZATION
IC typename CAbstractGraph::_vertex_id_type const& CAbstractGraph::value(
    _vertex_id_type const& vertex_index,
    const_iterator i ) const {
    return ( ( *i ).vertex_id() );
}

TEMPLATE_SPECIALIZATION
IC void CAbstractGraph::begin( const CVertex* vertex,
                               const_iterator& b,
...
TEMPLATE_SPECIALIZATION
IC void CAbstractGraph::begin( _vertex_id_type const& vertex_index,
                               const_iterator& b,
                               const_iterator& e ) const {
...
IC void CAbstractGraph::load( IReader& stream ) {
    ...
    for ( chunk1 = chunk0->open_chunk_iterator( id ); chunk1;
          chunk1 = chunk0->open_chunk_iterator( id, chunk1 ) ) {
