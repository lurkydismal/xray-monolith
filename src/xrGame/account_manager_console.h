...
public:
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy( I,
                   "Creates GameSpy account:gs_create_account <nick> "
                   "<unique_nick> <email> <password>" );
    }
}; // CCC_CreateGameSpyAccount

class CCC_GapySpyListProfiles : public IConsole_Command {
public:
    CCC_GapySpyListProfiles( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
    ...
    virtual void Info( TInfo& I ) {
        xr_strcpy(
            I, "Lists account profiles : gs_list_profiles <email> <password>" );
    }
}; // CCC_GapySpyListProfiles

class CCC_GameSpyLogin : public IConsole_Command {
public:
    CCC_GameSpyLogin( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
...
public:
    CCC_GameSpyLogout( LPCSTR N ) : IConsole_Command( N ) {
...
public:
    CCC_GameSpyPrintProfile( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
...
public:
    CCC_GameSpySuggestUNicks( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
...
public:
    CCC_GameSpyRegisterUniqueNick( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
...
public:
    CCC_GameSpyDeleteProfile( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = true;
    };
...
public:
    CCC_GameSpyProfile( LPCSTR N ) : IConsole_Command( N ) {
        bEmptyArgsHandled = false;
    };
