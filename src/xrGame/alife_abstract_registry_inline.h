...
#define CSALifeAbstractRegistry \
    CALifeAbstractRegistry< _index_type, _data_type >
...
TEMPLATE_SPECIALIZATION
IC const typename CSALifeAbstractRegistry::OBJECT_REGISTRY&
CSALifeAbstractRegistry::objects() const {
    return ( m_objects );
}

TEMPLATE_SPECIALIZATION
IC void CSALifeAbstractRegistry::add( const _index_type& index,
                                      _data_type& data,
                                      bool no_assert ) {
    const_iterator I = objects().find( index );
    if ( I != objects().end() ) {
        THROW2( no_assert,
                "Specified object has been already found in the specified "
                "registry!" );
        return;
    }
...
TEMPLATE_SPECIALIZATION
IC void CSALifeAbstractRegistry::remove( const _index_type& index,
                                         bool no_assert ) {
    iterator I = m_objects.find( index );
    if ( I == objects().end() ) {
        THROW2(
            no_assert,
            "Specified object hasn't been found in the specified registry!" );
        return;
    }
    m_objects.erase( I );
}
...
TEMPLATE_SPECIALIZATION
IC _data_type* CSALifeAbstractRegistry::object( const _index_type& index,
                                                bool no_assert ) {
    iterator I = m_objects.find( index );
    if ( I == objects().end() ) {
        THROW2(
