...
#define TEMPLATE_SPECIALIZATION                                          \
    template < typename _operator_condition, typename _operator,         \
               typename _condition_state, typename _condition_evaluator, \
               typename _operator_id_type, bool _reverse_search,         \
               typename _operator_ptr, typename _condition_evaluator_ptr >

#define CProblemSolverAbstract                                                \
    CProblemSolver< _operator_condition, _operator, _condition_state,         \
...
IC bool CProblemSolverAbstract::actual() const {
    ...
    typename xr_vector< COperatorCondition >::const_iterator I =
        current_state().conditions().begin();
    typename xr_vector< COperatorCondition >::const_iterator E =
        current_state().conditions().end();
    ...
    for ( ; I != E; ++I ) {
        if ( ( *i ).first < ( *I ).condition() )
            i = std::lower_bound( i, e, ( *I ).condition(),
                                  evaluators().value_comp() );
        VERIFY( i != e );
...
TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::add_operator( const _edge_type& operator_id,
                                              _operator_ptr _operator ) {
    typename OPERATOR_VECTOR::iterator I =
        std::lower_bound( m_operators.begin(), m_operators.end(), operator_id );
    THROW( ( I == m_operators.end() ) ||
           ( ( *I ).m_operator_id != operator_id ) );
#ifdef DEBUG
...
#ifdef DEBUG
TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::validate_properties(
    const CState& conditions ) const {
    ...
    for ( ; I != E; ++I ) {
        if ( evaluators().find( ( *I ).condition() ) == evaluators().end() ) {
            Msg( "! cannot find corresponding evaluator to the property with "
                 "id %d",
                 ( *I ).condition() );
            THROW( evaluators().find( ( *I ).condition() ) !=
                   evaluators().end() );
        }
    }
}
...
TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::remove_operator(
    const _edge_type& operator_id ) {
    typename OPERATOR_VECTOR::iterator I =
        std::lower_bound( m_operators.begin(), m_operators.end(), operator_id );
...
TEMPLATE_SPECIALIZATION
IC const typename CProblemSolverAbstract::OPERATOR_VECTOR&
CProblemSolverAbstract::operators() const {
    return ( m_operators );
}
...
TEMPLATE_SPECIALIZATION
IC const typename CProblemSolverAbstract::CState&
CProblemSolverAbstract::current_state() const {
    return ( m_current_state );
}

TEMPLATE_SPECIALIZATION
IC const typename CProblemSolverAbstract::CState&
CProblemSolverAbstract::target_state() const {
    return ( m_target_state );
}

TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::add_evaluator(
    const _condition_type& condition_id,
...
TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::remove_evaluator(
    const _condition_type& condition_id ) {
...
TEMPLATE_SPECIALIZATION
IC const typename CProblemSolverAbstract::EVALUATORS&
CProblemSolverAbstract::evaluators() const {
    return ( m_evaluators );
}
...
IC void CProblemSolverAbstract::evaluate_condition(
    ...
    m_current_state.add_condition(
        I, COperatorCondition( condition_id,
                               evaluator( condition_id )->evaluate() ) );
...
CProblemSolverAbstract::get_edge_weight( const _index_type& vertex_index0,
...
TEMPLATE_SPECIALIZATION
IC bool CProblemSolverAbstract::is_accessible(
    const _index_type& vertex_index ) const {
    return ( m_applied );
}

TEMPLATE_SPECIALIZATION
IC const typename CProblemSolverAbstract::_index_type&
CProblemSolverAbstract::value( const _index_type& vertex_index,
                               const_iterator& i,
                               bool reverse_search ) const {
    if ( reverse_search ) {
        if ( ( *i ).m_operator->applicable_reverse(
                 ( *i ).m_operator->effects(), ( *i ).m_operator->conditions(),
                 vertex_index ) )
            m_applied = ( *i ).m_operator->apply_reverse(
                vertex_index, ( *i ).m_operator->effects(), m_temp,
        ...
        if ( ( *i ).m_operator->applicable( vertex_index, current_state(),
                                            ( *i ).m_operator->conditions(),
                                            *this ) ) {
            ( *i ).m_operator->apply( vertex_index,
                                      ( *i ).m_operator->effects(), m_temp,
                                      m_current_state, *this );
            m_applied = true;
        } else
            m_applied = false;
    }
...
TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::begin( const _index_type& vertex_index,
                                       const_iterator& b,
...
TEMPLATE_SPECIALIZATION
IC bool CProblemSolverAbstract::is_goal_reached(
    const _index_type& vertex_index ) const {
    return ( is_goal_reached_impl< reverse_search >( vertex_index ) );
}

TEMPLATE_SPECIALIZATION
IC bool CProblemSolverAbstract::is_goal_reached_impl(
    const _index_type& vertex_index ) const {
    STATIC_CHECK( !reverse_search,
                  This_function_cannot_be_used_in_the_REVERSE_search );
    typename xr_vector< COperatorCondition >::const_iterator I =
        vertex_index.conditions().begin();
    typename xr_vector< COperatorCondition >::const_iterator E =
        vertex_index.conditions().end();
    typename xr_vector< COperatorCondition >::const_iterator i =
        target_state().conditions().begin();
    typename xr_vector< COperatorCondition >::const_iterator e =
        target_state().conditions().end();
    typename xr_vector< COperatorCondition >::const_iterator II =
        current_state().conditions().begin();
    typename xr_vector< COperatorCondition >::const_iterator EE =
        current_state().conditions().end();
    for ( ; ( i != e ) && ( I != E ); ) {
...
TEMPLATE_SPECIALIZATION
IC bool CProblemSolverAbstract::is_goal_reached_impl(
    const _index_type& vertex_index,
    bool ) const {
    STATIC_CHECK( reverse_search,
                  This_function_cannot_be_used_in_the_STRAIGHT_search );
    typename xr_vector< COperatorCondition >::const_iterator I =
        m_current_state.conditions().begin();
    typename xr_vector< COperatorCondition >::const_iterator E =
        m_current_state.conditions().end();
    typename xr_vector< COperatorCondition >::const_iterator i =
        vertex_index.conditions().begin();
    typename xr_vector< COperatorCondition >::const_iterator e =
        vertex_index.conditions().end();
...
TEMPLATE_SPECIALIZATION
IC typename CProblemSolverAbstract::_operator_ptr
CProblemSolverAbstract::get_operator( const _edge_type& operator_id ) {
    typename OPERATOR_VECTOR::iterator I =
        std::lower_bound( m_operators.begin(), m_operators.end(), operator_id );
...
IC void CProblemSolverAbstract::solve() {
    ...
    m_failed = !ai().graph_engine().search(
        *this, reverse_search ? target_state() : current_state(),
...
TEMPLATE_SPECIALIZATION
IC typename CProblemSolverAbstract::_edge_value_type
CProblemSolverAbstract::estimate_edge_weight(
    const _index_type& condition ) const {
    return ( helper< reverse_search >::estimate_edge_weight_impl( *this,
                                                                  condition ) );
}

TEMPLATE_SPECIALIZATION
IC typename CProblemSolverAbstract::_edge_value_type
CProblemSolverAbstract::estimate_edge_weight_impl(
    const _index_type& condition ) const {
    STATIC_CHECK( !reverse_search,
                  This_function_cannot_be_used_in_the_REVERSE_search );
...
TEMPLATE_SPECIALIZATION
IC typename CProblemSolverAbstract::_edge_value_type
CProblemSolverAbstract::estimate_edge_weight_impl( const _index_type& condition,
                                                   bool ) const {
    STATIC_CHECK( reverse_search,
                  This_function_cannot_be_used_in_the_STRAIGHT_search );
    _edge_value_type result = 0;
