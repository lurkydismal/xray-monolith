...
TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::add_operator(const _edge_type& operator_id, _operator_ptr __operator)
{
...
#ifdef DEBUG
	validate_properties			(__operator->conditions());
	validate_properties			(__operator->effects());
#endif
	m_actuality = false;
	m_operators.insert(I, SOperator(operator_id, __operator));
}
