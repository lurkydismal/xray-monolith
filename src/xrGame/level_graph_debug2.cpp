...
void CLevelGraph::draw_nodes() {
    ...
    //	float				x,z;
    //	unpack_xz			(Local,x,z);
    //	F->Out				(0.f,0.55f,"%3d,%4d,%3d -> %d",
    // iFloor(x),iFloor(Local.y()),iFloor(z),u32(ID));
...
void CLevelGraph::draw_restrictions() {
    ...
    for ( ; I != E; ++I ) {
        ...
        for ( ; i != e; ++i ) {
            ...
            temp.y += .1f;
            Level().debug_renderer().draw_aabb( temp, .05f, .05f, .05f,
                                                D3DCOLOR_XRGB( r, g, b ) );
        }

#ifdef USE_FREE_IN_RESTRICTIONS
        CSpaceRestriction::FREE_IN_RESTRICTIONS::const_iterator II =
            ( *I ).second->m_free_in_restrictions.begin();
        ...
        for ( ; II != EE; ++II ) {
            xr_vector< u32 >::const_iterator i =
                ( *II ).m_restriction->border().begin();
            xr_vector< u32 >::const_iterator e =
                ( *II ).m_restriction->border().end();
            for ( ; i != e; ++i ) {
                Fvector temp = ai().level_graph().vertex_position( *i );
                temp.y += .1f;
                Level().debug_renderer().draw_aabb(
                    temp, .05f, .05f, .05f, D3DCOLOR_XRGB( 255, 0, 0 ) );
            }
            {
                xr_vector< u32 >::const_iterator i =
                    ( *II ).m_restriction->border().begin();
                ...
                for ( ; i != e; ++i ) {
                    ...
                    temp.y += .1f;
                    Level().debug_renderer().draw_aabb(
                        temp, .05f, .05f, .05f, D3DCOLOR_XRGB( 0, 255, 0 ) );
                }
            }
        }
#endif
...
void CLevelGraph::draw_covers() {
    ...
    ai().cover_manager().covers().nearest( Device.vCameraPosition, 5.f,
                                           nearest );
    ...
    for ( ; I != E; ++I ) {
        ...
        for ( u32 i = 0, j = 0; i < 36; ++i ) {
            float value =
                high_cover_in_direction( float( 10 * i ) / 180.f * PI, v );
            ...
            value = compute_high_square( float( 10 * i ) / 180.f * PI, PI / 2.f,
                                         v );
            if ( value > best_value ) {
                best_value = value;
                j = i;
        ...
        float value =
            high_cover_in_direction( float( 10 * j ) / 180.f * PI, v );
        ...
        // low
        {
            ...
            for ( u32 i = 0, j = 0; i < 36; ++i ) {
                float value =
                    low_cover_in_direction( float( 10 * i ) / 180.f * PI, v );
                direction.setHP( float( 10 * i ) / 180.f * PI, 0 );
                direction.normalize();
                direction.mul( value * half_size );
                direction.add( position );
                ...
                Level().debug_renderer().draw_line(
                    ...
                    D3DCOLOR_XRGB( 0, 0, 255 ) );
                value = compute_low_square( float( 10 * i ) / 180.f * PI,
                                            PI / 2.f, v );
                if ( value > best_value ) {
                    best_value = value;
                    j = i;
            ...
            direction.set(
                position.x, position.y,
                position.z + half_size * float( v->low_cover( 1 ) ) / 15.f );
            Level().debug_renderer().draw_line( Fidentity, position, direction,
                                                D3DCOLOR_XRGB( 255, 0, 0 ) );
            ...
            direction.set(
                position.x, position.y,
                position.z - half_size * float( v->low_cover( 3 ) ) / 15.f );
            Level().debug_renderer().draw_line( Fidentity, position, direction,
                                                D3DCOLOR_XRGB( 255, 0, 0 ) );

            float value =
                low_cover_in_direction( float( 10 * j ) / 180.f * PI, v );
            direction.setHP( float( 10 * j ) / 180.f * PI, 0 );
            direction.normalize();
            direction.mul( value * half_size );
            direction.add( position );
        ...
        if ( tpCustomMonster ) {
            ...
            if ( !tpCustomMonster->movement().detail().path().empty() ) {
                Fvector temp = tpCustomMonster->movement()
                                   ...
                                   .position;
                Level().debug_renderer().draw_aabb(
                    temp, 1.f, 1.f, 1.f, D3DCOLOR_XRGB( 0, 0, 255 ) );
            }
        }

        smart_cover::object* smart_cover =
            smart_cast< smart_cover::object* >( _O );
        if ( smart_cover ) {
...
void CLevelGraph::draw_debug_node() {
    if ( g_bDebugNode ) {
        ...
        if ( ai().level_graph().valid_vertex_id( g_dwDebugNodeSource ) ) {
            ...
            Level().debug_renderer().draw_aabb( pos_src, 0.35f, 0.35f, 0.35f,
                                                D3DCOLOR_XRGB( 0, 0, 255 ) );
            Level().debug_renderer().draw_line( Fidentity, pos_src, pos_dest,
                                                D3DCOLOR_XRGB( 0, 0, 255 ) );
        }
