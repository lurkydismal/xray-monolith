...
private:
    ITEM_TABLE* m_pTable;
    LPCSTR table_sect;
    // ширина таблицы, если -1 то таблица делается квадратной (ширина равна
    // высоте)
    int table_width;
    ...
    T_ITEM convert( LPCSTR ) {
        STATIC_CHECK( false,
                      Specialization_for_convert_in_CIni_Table_not_found );
        NODEFAULT;
    }
...
typename CSIni_Table::ITEM_TABLE& CSIni_Table::table() {
    ...
    std::size_t cur_table_width =
        ( table_width == -1 ) ? table_size : ( std::size_t )table_width;
    ...
    R_ASSERT3( table_ini.Data.size() == table_size,
               "wrong size for table in section", table_sect );

    for ( CInifile::SectCIt i = table_ini.Data.begin();
          table_ini.Data.end() != i; ++i ) {
        T_INI_LOADER::index_type cur_index = T_INI_LOADER::IdToIndex(
            ( *i ).first, type_max( T_INI_LOADER::index_type ) );

        if ( type_max( T_INI_LOADER::index_type ) == cur_index )
            Debug.fatal( DEBUG_INFO, "wrong community %s in section [%s]",
                         ( *i ).first, table_sect );

        ( *m_pTable )[ cur_index ].resize( cur_table_width );
        for ( std::size_t j = 0; j < cur_table_width; j++ ) {
            ( *m_pTable )[ cur_index ][ j ] = convert< typename T_ITEM >(
                _GetItem( *( *i ).second, ( int )j, buffer ) );
        }
    }
