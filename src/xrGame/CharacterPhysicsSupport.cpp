...
// void  NodynamicsCollide( bool& do_colide, bool bo1, dContact& c, SGameMtl *
...
CCharacterPhysicsSupport::CCharacterPhysicsSupport( EType atype,
                                                    CEntityAlive* aentity )
    ...
    switch ( atype ) {
        case etActor:
            m_PhysicMovementControl->AllocateCharacterObject(
                CPHMovementControl::actor );
            m_PhysicMovementControl->SetRestrictionType( rtActor );
            break;
        case etStalker:
            m_PhysicMovementControl->AllocateCharacterObject(
                CPHMovementControl::ai );
...
void CCharacterPhysicsSupport::in_Load( LPCSTR section ) {
    ...
    if ( pSettings->line_exist( section, "bonce_damage_factor" ) )
        m_BonceDamageFactor =
            pSettings->r_float( section, "bonce_damage_factor_for_objects" );
    else
        m_BonceDamageFactor = pSettings->r_float(
            "collision_damage", "bonce_damage_factor_for_objects" );
    CPHDestroyable::Load( section );
}

void CCharacterPhysicsSupport::run_interactive( CBlend* B ) {
    VERIFY( !m_interactive_animation );
    m_interactive_animation =
        xr_new< interactive_animation >( &m_EntityAlife, B );
}
...
void CCharacterPhysicsSupport::update_interactive_anims() {
    ...
    if ( b && !m_interactive_animation &&
         stalker->animation().global().callback_on_collision() )
        run_interactive( b );
    if ( m_interactive_animation &&
         !m_interactive_animation->update( m_EntityAlife.XFORM() ) )
...
void CCharacterPhysicsSupport::in_NetSpawn( CSE_Abstract* e ) {
    ...
    if ( !m_EntityAlife.g_Alive() ) {
        if ( m_eType == etStalker ) {
            // pK->LL_GetData( 0
            // ).shape.flags.set(SBoneShape::sfVisibilityIgnore,TRUE);
            // pK->LL_GetData( pK->LL_BoneID("bip01")
            // ).shape.flags.set(SBoneShape::sfVisibilityIgnore,TRUE);
            ka->PlayCycle( "waunded_1_idle_0" );
        } else
            ka->PlayCycle( "death_init" );
    } else if ( !m_EntityAlife.animation_movement_controlled() )
        ka->PlayCycle( "death_init" ); /// непонятно зачем это вообще надо
                                       /// запускать этот хак нужен, потому что
                                       /// некоторым монстрам анимация после
                                       /// спона, может быть вообще не назначена
    ...
    if ( Type() == etStalker ) {
        m_hit_animations.SetupHitMotions(
            *smart_cast< IKinematicsAnimated* >( m_EntityAlife.Visual() ) );
    }
    anim_mov_state.init();

    anim_mov_state.active = m_EntityAlife.animation_movement_controlled();
    CInifile* ini = m_EntityAlife.spawn_ini();
    if ( ini && ini->section_exist( "physics" ) &&
         ini->line_exist( "physics", "controller_can_be_moved_by_player" ) )
        m_PhysicMovementControl->SetActorMovable(
            !!ini->r_bool( "physics", "controller_can_be_moved_by_player" ) );
}
...
bool HACK_TERRIBLE_DONOT_COLLIDE_ON_SPAWN( CEntityAlive& ea ) {
    if ( pSettings->line_exist( ea.cNameSect().c_str(),
                                "hack_terrible_donot_collide_on_spawn" ) &&
         pSettings->r_bool( ea.cNameSect().c_str(),
                            "hack_terrible_donot_collide_on_spawn" ) )
        return true;
    return false;
}
...
void CCharacterPhysicsSupport::SpawnInitPhysics( CSE_Abstract* e ) {
    if ( m_EntityAlife.g_Alive() ) {
        ...
        if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
             stricmp( PH_DBG_ObjectTrackName(), *m_EntityAlife.cName() ) ==
                 0 ) {
            Msg( "CCharacterPhysicsSupport::SpawnInitPhysics obj %s before "
                 "collision correction %f,%f,%f",
                 PH_DBG_ObjectTrackName(), m_EntityAlife.Position().x,
        ...
        if ( ph_dbg_draw_mask1.test( ph_m1_DbgTrackObject ) &&
             stricmp( PH_DBG_ObjectTrackName(), *m_EntityAlife.cName() ) ==
                 0 ) {
            Msg( "CCharacterPhysicsSupport::SpawnInitPhysics obj %s after "
                 "collision correction %f,%f,%f",
                 PH_DBG_ObjectTrackName(), m_EntityAlife.Position().x,
                 m_EntityAlife.Position().y, m_EntityAlife.Position().z );
        }
...
#ifdef DEBUG
        Msg( "! saved bones %d , current bones %d, object :%s",
             saved_bones.size(), m_EntityAlife.PHGetSyncItemsNumber(),
...
void CCharacterPhysicsSupport::SpawnCharacterCreate() {
    if ( HACK_TERRIBLE_DONOT_COLLIDE_ON_SPAWN(
             m_EntityAlife ) ) //|| m_EntityAlife.animation_movement_controlled(
                               //)
        return;
    CreateCharacterSafe();
    // if( m_eType != etStalker )
...
void CCharacterPhysicsSupport::in_shedule_Update( u32 DT ) {
    ...
    else if ( m_pPhysicsShell && m_pPhysicsShell->isFullActive() &&
              !m_pPhysicsShell->isEnabled() )
...
void CCharacterPhysicsSupport::KillHit( SHit& H ) {
    ...
    // IKinematicsAnimated * KA = m_EntityAlife.Visual(
    // )->dcast_PKinematicsAnimated	(); VERIFY( KA );
    // KA->SetUpdateTracksCalback( &tracks_disable_update );

    m_character_shell_control.TestForWounded(
        m_EntityAlife.XFORM(), m_EntityAlife.Visual()->dcast_PKinematics() );
    Fmatrix prev_pose;
    ...
    if ( holder &&
         ( holder->wounded() || holder->movement().current_params().cover() ) )
        m = MotionID();
...
void CCharacterPhysicsSupport::in_Hit( SHit& H, bool is_killing ) {
    ...
    if ( m_flags.test( fl_block_hit ) ) {
        VERIFY2( !m_EntityAlife.g_Alive(),
                 make_string( "entity [%s][%d] is dead", m_EntityAlife.Name(),
                              m_EntityAlife.ID() )
                     .c_str() );
    ...
    if ( m_EntityAlife.g_Alive() && is_killing &&
         H.type() == ALife::eHitTypeExplosion && H.damage() > 70.f )
    ...
    if ( m_flags.test( fl_use_hit_anims ) && Type() != etBitting &&
         !m_flags.test( fl_death_anim_on ) )
    //&& Type() == etStalker
    {
        m_hit_animations.PlayHitMotion( H.direction(), H.bone_space_position(),
                                        H.bone(), m_EntityAlife );
    }

    if ( !( m_pPhysicsShell && m_pPhysicsShell->isActive() ) ) {
        if ( !is_killing && m_EntityAlife.g_Alive() )
            m_PhysicMovementControl->ApplyHit( H.direction(), H.phys_impulse(),
                                               H.type() );
    } else {
#ifdef DEBUG
        if ( is_killing && death_anim_debug &&
             !is_imotion( m_interactive_motion ) ) {
            Msg( "death anim: applied fatal impulse dir: (%f,%f,%f), value: "
                 "(%f) ",
                 H.dir.x, H.dir.y, H.dir.z, H.impulse );
        }
#endif
        m_pPhysicsShell->applyHit( H.bone_space_position(), H.direction(),
                                   H.phys_impulse(), H.bone(), H.type() );
    }
}
...
void dbg_draw_geoms( xr_vector< CODEGeom* >& m_weapon_geoms ) {
    xr_vector< CODEGeom* >::iterator ii = m_weapon_geoms.begin(),
                                     ee = m_weapon_geoms.end();
    for ( ; ii != ee; ++ii ) {
        CODEGeom* g = ( *ii );
...
void CCharacterPhysicsSupport::in_UpdateCL() {
    ...
    if ( dbg_draw_character_physics && m_pPhysicsShell )
        m_pPhysicsShell->dbg_draw_geometry( 0.2f,
    ...
    if ( m_pPhysicsShell ) {
        VERIFY( m_pPhysicsShell->isFullActive() );
        m_pPhysicsShell->SetRagDoll(); // Теперь шела относиться к классу
                                       // объектов cbClassRagDoll

        if ( !is_imotion(
                 m_interactive_motion ) ) //! m_flags.test(fl_use_death_motion)
        ...
        m_character_shell_control.UpdateFrictionAndJointResistanse(
            m_pPhysicsShell );
    }
    // else if ( !m_EntityAlife.g_Alive( ) &&
    // !m_EntityAlife.use_simplified_visual( ) )
    //{
    // ActivateShell( NULL );
    // m_PhysicMovementControl->DestroyCharacter( );
    // }
        ...
        float ssa = Device.CalcSSADynamic(
            m_EntityAlife.SpatialComponent->spatial.sphere.P,
            m_EntityAlife.SpatialComponent->spatial.sphere.R );
        if ( ssa > IK_CALC_SSA ) {
            if ( view_frust.testSphere_dirty(
                     m_EntityAlife.SpatialComponent->spatial.sphere.P,
                     m_EntityAlife.SpatialComponent->spatial.sphere.R ) ||
    ...
    if ( Type() == etStalker && ph_dbg_draw_mask1.test( phDbgHitAnims ) ) {
        ...
        /*
                        IKinematicsAnimated	*K =
           smart_cast<IKinematicsAnimated*>(m_EntityAlife.Visual()); u16 hb =
           K->LL_BoneID("bip01_head"); u16 pb = K->LL_GetBoneRoot(); u16 nb =
           K->LL_BoneID("bip01_neck"); u16 eb = K->LL_BoneID("eye_right");
...
void CCharacterPhysicsSupport::CreateSkeleton( CPhysicsShell*& pShell ) {
    ...
    disable_params.Load(
        smart_cast< IKinematics* >( m_EntityAlife.Visual() )->LL_UserData() );
    pShell->set_DisableParams( disable_params );

    pShell->Build();

#ifdef DEBUG
    Msg( "shell for %s[%d] created in %f ms", *m_EntityAlife.cName(),
         m_EntityAlife.ID(), t.GetElapsed_sec() * 1000.f );
#endif
}
...
bool CCharacterPhysicsSupport::CollisionCorrectObjPos(
    const Fvector& start_from,
    bool character_create /*=false*/ ) {
    ...
    if ( character_create )
        ...
        if ( m_pPhysicsShell ) {
            ...
            box.setb( Fvector().sub( c, m_EntityAlife.Position() ),
                      Fvector( sz ).mul( 0.5f ) );
    ...
    bool not_collide_characters =
        !DoCharacterShellCollide() && !character_create;
    ...
    bool ret = ActivateShapeCharacterPhysicsSupport(
        activation_res, vbox, activation_pos, mXFORM, not_collide_characters,
        set_rotation, &m_EntityAlife );
    //////////////////
...
{
        VERIFY( &shell );
        CPhysicsElement * physics_root_element = shell.get_ElementByStoreOrder(
0 ); VERIFY( physics_root_element );
        ...
        CODEGeom	*physics_root_bone_geom =
physics_root_element->geometry( 0 ); VERIFY( physics_root_bone_geom );
        ...
        //buffer_vector<u32>	anim_bones_bind_positions(
_alloca(anim_bones_number*sizeof(u32)),
        //
anim_bones_number
        //
        ...
        const Fmatrix physics_root_to_anim_root_bind_transformation
                = Fmatrix().mul_43( Fmatrix().invert( anim_bones_bind_positions[
physics_root_bone_id ] ), anim_bones_bind_positions[ animation_root_bone_id ] );

        const Fmatrix &physics_root_bone_anim_transform = K->LL_GetTransform(
physics_root_bone_id );

        const Fmatrix physics_root_bone_corrected_pos = Fmatrix().mul_43(
physics_root_bone_anim_transform , physics_root_to_anim_root_bind_transformation
                                                                                                                                          );

        physics_root_element->SetTransform( Fmatrix().mul_43( shell.mXFORM,
physics_root_bone_corrected_pos ) );

        //physics_root_element->TransformPosition( Fmatrix().mul_43(
Fmatrix().invert( K->LL_GetTransform( animation_root_bone_id ) ),
physics_root_bone_corrected_pos ) );
}
*/
...
void CCharacterPhysicsSupport::ActivateShell( CObject* who ) {
    if ( !_valid( m_EntityAlife.Position() ) ) {
        Debug.fatal(
            DEBUG_INFO,
            "CCharacterPhysicsSupport::ActivateShell Fatal error, invalid "
            "position for m_EntityAlife %s, id %d, x %.5f, y %.5f, z %.5f",
            m_EntityAlife.cNameSect().c_str(), m_EntityAlife.ID(),
            m_EntityAlife.Position().x, m_EntityAlife.Position().y,
...
// }
bool CCharacterPhysicsSupport::has_shell_collision_place(
    const CPhysicsShellHolder* obj ) const {
    return m_active_item_obj && obj == m_active_item_obj;
}

void CCharacterPhysicsSupport::on_child_shell_activate(
...
void CCharacterPhysicsSupport::RemoveActiveWeaponCollision() {
    ...
    Fmatrix m0;
    ( *ii )->get_xform( m0 );
    CPhysicsElement* root =
        m_active_item_obj->PPhysicsShell()->get_ElementByStoreOrder( 0 );
    ...
    // m_active_item_obj->PPhysicsShell()->dbg_draw_geometry( 0.2f,
    // D3DCOLOR_XRGB( 255, 0, 100 ) );

    Fvector a_vel, l_vel;
    const Fvector& mc = root->mass_Center();
    // dBodyGetPointVel( m_weapon_attach_bone->get_body(),mc.x, mc.y, mc.z,
    // cast_fp(l_vel) );
...
void CCharacterPhysicsSupport::bone_chain_disable( u16 bone,
                                                   u16 r_bone,
                                                   IKinematics& K ) {
    VERIFY( &K );
    u16 bid = bone;
    ...
    while ( bid != r_bone && bid != K.LL_GetBoneRoot() ) {
        ...
        if ( K.LL_GetBoneInstance( bid ).callback() !=
             anim_bone_fix::callback ) {
...
void CCharacterPhysicsSupport::AddActiveWeaponCollision() {
    ...
    CInventoryOwner* inv_owner =
        smart_cast< CInventoryOwner* >( &m_EntityAlife );
    VERIFY( inv_owner );
    ...
    CPhysicsShell* weapon_shell = P_build_Shell(
        &active_weapon_item->object(), true, ( BONE_P_MAP* )( 0 ), true );

    VERIFY( m_pPhysicsShell );
    CPhysicsElement* weapon_attach_bone =
        m_pPhysicsShell->get_PhysicsParrentElement( ( u16 )br );

    bone_chain_disable( ( u16 )br, weapon_attach_bone->m_SelfID,
                        *m_pPhysicsShell->PKinematics() );
    if ( bl != br && bl != -1 ) {
        CPhysicsElement* p =
            m_pPhysicsShell->get_PhysicsParrentElement( ( u16 )bl );
        VERIFY( p );
        bone_chain_disable( ( u16 )bl, p->m_SelfID,
                            *m_pPhysicsShell->PKinematics() );
    }
    if ( br2 != bl && br2 != br && br2 != -1 ) {
        CPhysicsElement* p =
            m_pPhysicsShell->get_PhysicsParrentElement( ( u16 )br2 );
        VERIFY( p );
        bone_chain_disable( ( u16 )br2, weapon_attach_bone->m_SelfID,
                            *m_pPhysicsShell->PKinematics() );
    }

    CPhysicsElement* weapon_element =
        weapon_shell->get_ElementByStoreOrder( 0 );
...
void CCharacterPhysicsSupport::CreateShell( CObject* who,
                                            Fvector& dp,
                                            Fvector& velocity ) {
    xr_delete( m_collision_activating_delay );
    xr_delete( m_interactive_animation );
    ...
    if ( IsGameTypeSingle() ) {
        m_pPhysicsShell->SetPrefereExactIntegration(); // use exact integration
                                                       // for ragdolls in single
        m_pPhysicsShell->SetDeadBody();
    } else
...
void CCharacterPhysicsSupport::EndActivateFreeShell(
...
#ifdef DEBUG
    if ( death_anim_debug ) {
        Msg( "death anim: ragdoll velocity picked from char controller "
             "=(%f,%f,%f), velocity applied to ragdoll =(%f,%f,%f)  ",
    ...
    // if( false &&  anim_mov_ctrl && anim_mov_blend && anim_mov_blend->blend !=
    // CBlend::eFREE_SLOT &&  anim_mov_blend->timeCurrent +
    // Device.fTimeDelta*anim_mov_blend->speed <
...
void CCharacterPhysicsSupport::set_collision_hit_callback(
    ICollisionHitCallback* cc ) {
    xr_delete( m_collision_hit_callback );
...
void CCharacterPhysicsSupport::on_create_anim_mov_ctrl() {
    ...
    // anim_mov_state.character_exist =
