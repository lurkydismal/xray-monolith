...
void first_bullet_controller::load( shared_str const& section ) {
    m_use_first_bullet =
        pSettings->r_bool( section, "use_first_bullet" ) ? true : false;
    if ( !m_use_first_bullet )
        return;
    m_fire_dispertion =
        pSettings->r_float( section, "first_bullet_dispertion" );
    m_shot_timeout = pSettings->r_u32( section, "first_bullet_timeout" );
    m_actor_velocity_limit =
        pSettings->r_float( section, "first_bullet_velocity_limit" );
}

bool first_bullet_controller::is_bullet_first(
    float actor_linear_velocity ) const {
    R_ASSERT2( !IsGameTypeSingle(),
               "first bullet shot can't be in single game mode" );
    if ( !m_use_first_bullet )
        return false;
