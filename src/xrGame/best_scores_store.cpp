...
void best_scores_store::init_field_names() {
    for ( int i = 0; i < bst_score_types_count; ++i ) {
        m_field_names_store[ i ] = ATLAS_GET_STAT_NAME( get_best_score_id_stat(
            static_cast< enum_best_score_type >( i ) ) );
    }
}
...
void best_scores_store::reset_scores() {
    ...
    for ( int bsidx = 0; bsidx < bst_score_types_count; ++bsidx ) {
        m_result_scores.insert(
            std::make_pair( enum_best_score_type( bsidx ), 0 ) );
    };
}
...
void best_scores_store::load_best_scores_from_ltx( CInifile& ini ) {
    for ( int i = 0; i != bst_score_types_count; ++i ) {
        ...
        m_ltx_result_scores.insert(
            std::make_pair( bstype, ini.r_u32( get_best_score_name( bstype ),
                                               best_score_value_line ) ) );
    }
}

void best_scores_store::merge_sake_to_ltx_best_scores() {
    for ( all_best_scores_t::iterator i = m_ltx_result_scores.begin(),
                                      ie = m_ltx_result_scores.end();
          i != ie; ++i ) {
        all_best_scores_t::const_iterator tmp_bi =
...
bool best_scores_store::is_sake_equal_to_file() const {
    ...
    for ( all_best_scores_t::const_iterator i = m_ltx_result_scores.begin(),
                                            ie = m_ltx_result_scores.end();
          i != ie; ++i ) {
        all_best_scores_t::const_iterator tmp_iter =
            m_result_scores.find( i->first );
...
//		my_inst->m_scores_operation_cb		(true,
//"mp_load_best_scores_complete");
//	}
//	my_inst->m_scores_operation_cb.clear	();
// }

// void best_scores_store::process_scores_out_response(SAKEGetMyRecordsOutput*
// tmp_out, int const out_fields_count)
//{
//	VERIFY(tmp_out->mNumRecords <= 1);
////one raw 	if (tmp_out->mNumRecords == 0)
//	{
//		for (int i = 0; i < bst_score_types_count; ++i)
//		{
//			m_result_scores.insert(
