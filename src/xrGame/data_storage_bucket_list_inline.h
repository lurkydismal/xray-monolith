...
#define TEMPLATE_SPECIALIZATION                                  \
    template < typename _path_id_type, typename _bucket_id_type, \
               u32 bucket_count, bool clear_buckets >            \
    template < typename _data_storage,                           \
               template < typename _T > class _vertex >

#define CBucketList                                                       \
    CDataStorageBucketList< _path_id_type, _bucket_id_type, bucket_count, \
                            clear_buckets >::CDataStorage< _data_storage, \
                                                           _vertex >
...
IC bool CBucketList::is_opened_empty() {
    ...
    if ( !m_buckets[ m_min_bucket_id ] ) {
        if ( !clear_buckets )
            for ( ++m_min_bucket_id;
                  ( m_min_bucket_id < bucket_count ) &&
                  ( !m_buckets[ m_min_bucket_id ] ||
                    ( m_buckets[ m_min_bucket_id ]->m_path_id !=
                      current_path_id() ) ||
                    ( m_buckets[ m_min_bucket_id ]->m_bucket_id !=
                      m_min_bucket_id ) );
                  ++m_min_bucket_id )
                ;
        else
...
IC u32 CBucketList::compute_bucket_id( CGraphVertex& vertex ) const {
    ...
    if ( vertex.f() <= m_min_bucket_value )
        return ( 0 );
    return ( u32( bucket_count * ( vertex.f() - m_min_bucket_value ) /
                  ( m_max_bucket_value - m_min_bucket_value ) ) );
}
...
IC void CBucketList::add_to_bucket( CGraphVertex& vertex, u32 m_bucket_id ) {
    ...
    if ( !i || ( !clear_buckets && ( ( i->m_path_id != current_path_id() ) ||
                                     ( i->m_bucket_id != m_bucket_id ) ) ) ) {
...
TEMPLATE_SPECIALIZATION
IC void CBucketList::decrease_opened( CGraphVertex& vertex,
                                      const _dist_type value ) {
    VERIFY( !is_opened_empty() );
...
IC void CBucketList::remove_best_opened() {
    VERIFY( !is_opened_empty() );
    verify_buckets();
    VERIFY( m_buckets[ m_min_bucket_id ] &&
            is_visited( m_buckets[ m_min_bucket_id ]->index() ) );
