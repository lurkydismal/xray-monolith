...
bool moving_objects::collided_static( const Fvector& position,
                                      const float& radius ) {
...
bool moving_objects::collided_static( moving_object* object,
                                      const Fvector& dest_position ) {
    float radius =
        object->radius() + ai().level_graph().header().cell_size() * .5f;
    ...
    for ( u32 i = 0; i < step_count; ++i ) {
        ...
        if ( collided_static( object->predict_position( i * step_to_check ),
                              radius ) )
            return ( true );
    }
...
void moving_objects::fill_all_static( moving_object* object,
                                      const Fvector& dest_position ) {
    float radius =
        object->radius() + ai().level_graph().header().cell_size() * .5f;
    float linear_velocity =
    ...
    for ( u32 i = 0; i < step_count; ++i ) {
        ...
        fill_static( object->static_query(),
                     object->predict_position( i * step_to_check ), radius );
    }
}
...
void moving_objects::query_action_static( moving_object* object ) {
    query_action_static( object, object->position(),
                         object->predict_position( time_to_check ) );
}
