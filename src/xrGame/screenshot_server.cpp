...
clientdata_proxy::~clientdata_proxy() {
    if ( is_active() ) {
        ...
        if ( m_ft_server->is_transfer_active( m_admin_id, m_chearer_id ) )
            m_ft_server->stop_transfer_file(
                std::make_pair( m_admin_id, m_chearer_id ) );
    }
}

void clientdata_proxy::make_screenshot( ClientID const& admin_id,
    ...
    if ( m_ft_server->is_receiving_active( cheater_id ) ) {
        Msg( "! Receiving from client [%u] already active, please try later",
             cheater_id.value() );
        return;
    }
    ...
    ssr_packet.w_u16(
        u16( Random.randI( 2 ) ) ); // food for thought for crackers :)
    ssr_packet.w_u16( u16( Random.randI( 2 ) ) );
    ssr_packet.w_u16( u16( Random.randI( 2 ) ) );
    ssr_packet.w_u8( u8( Random.randI( 2 ) ) );

    Level().Server->SecureSendTo( tmp_cheater, ssr_packet,
                                  net_flags( TRUE, TRUE ) );

    file_transfer::receiving_state_callback_t receiving_cb =
        fastdelegate::MakeDelegate(
            this, &clientdata_proxy::download_screenshot_callback );
    if ( my_proxy_mem_file.size() )
        my_proxy_mem_file.clear();
    m_first_receive = true;
    m_receiver = m_ft_server->start_receive_file( my_proxy_mem_file,
                                                  m_chearer_id, receiving_cb );
}

void clientdata_proxy::make_config_dump( ClientID const& admin_id,
                                         ClientID const& cheater_id ) {
    m_admin_id = admin_id;
    m_chearer_id = cheater_id;
    xrClientData* tmp_cheater = static_cast< xrClientData* >(
    ...
    file_transfer::receiving_state_callback_t receiving_cb =
        fastdelegate::MakeDelegate(
            this, &clientdata_proxy::download_config_callback );
    if ( my_proxy_mem_file.size() )
        my_proxy_mem_file.clear();
    m_first_receive = true;
    m_receiver = m_ft_server->start_receive_file( my_proxy_mem_file,
...
void clientdata_proxy::notify_admin( clientdata_event_t event_for_admin,
                                     char const* reason ) {
    NET_Packet ssr_packet;
    ssr_packet.w_begin( M_GAMEMESSAGE );
    ssr_packet.w_u32( GAME_EVENT_MAKE_DATA );
    ...
    if ( ( event_for_admin == e_screenshot_response ) ||
        ...
        ssr_packet.w_stringZ( reason ? reason
...
void clientdata_proxy::save_proxy_screenshot() {
    ...
    STRCONCAT(
        dest_file_name,
        clgame->make_file_name( m_cheater_name.c_str(), screenshot_fn ), "_",
    ...
    clgame->decompress_and_save_screenshot(
        screenshot_fn, my_proxy_mem_file.pointer(), my_proxy_mem_file.size(),
...
void clientdata_proxy::download_screenshot_callback(
    file_transfer::receiving_status_t status,
    u32 downloaded,
    u32 total ) {
    switch ( status ) {
        case file_transfer::receiving_data: {
            Msg( "* downloaded %d from %d bytes of screenshot from client [%d]",
                 downloaded, total, m_chearer_id );
            if ( m_first_receive ) {
                ...
                m_ft_server->start_transfer_file(
                    my_proxy_mem_file, total, m_admin_id, m_chearer_id,
                    sending_cb, m_receiver->get_user_param() );
...
void clientdata_proxy::download_config_callback(
    file_transfer::receiving_status_t status,
    u32 downloaded,
    u32 total ) {
    switch ( status ) {
        case file_transfer::receiving_data: {
            Msg( "* downloaded %d from %d bytes of config from client [%d]",
                 downloaded, total, m_chearer_id );
            if ( m_first_receive ) {
...
void clientdata_proxy::upload_file_callback(
    file_transfer::sending_status_t status,
    u32 uploaded,
    u32 total ) {
    switch ( status ) {
        case file_transfer::sending_data: {
            Msg( "* uploaded %d from %d bytes to client [%d]", uploaded, total,
                 m_admin_id );
        } break;
