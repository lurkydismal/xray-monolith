...
void CSpaceRestrictionBridge::change_implementation(
...
u32 CSpaceRestrictionBridge::accessible_nearest( const Fvector& position,
                                                 Fvector& result,
                                                 bool out_restriction ) {
    return (
        accessible_nearest( m_object, position, result, out_restriction ) );
}
...
bool CSpaceRestrictionBridge::inside( u32 level_vertex_id,
                                      bool partially_inside ) {
...
bool CSpaceRestrictionBridge::on_border( const Fvector& position ) const {
    ...
    xr_vector< u32 >::const_iterator I =
        std::lower_bound( object().border().begin(), object().border().end(),
                          pos.xz(), CFindByXZ_predicate() );

    if ( ( I == E ) ||
         ( ai().level_graph().vertex( *I )->position().xz() != pos.xz() ) )
        return ( false );
...
bool CSpaceRestrictionBridge::out_of_border( const Fvector& position ) {
    ...
    temp.R = EPS_L;
    temp.P = position;
    temp.P.y = ai().level_graph().vertex_plane_y( level_vertex_id, position.x,
                                                  position.z );
    return ( !inside( temp ) );
