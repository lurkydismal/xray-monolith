...
GameEvent* GameEventQueue::CreateSafe( NET_Packet& P,
                                       u16 type,
                                       u32 time,
                                       ClientID clientID ) {
...
#ifdef DEBUG
            Msg( "--- Ignoring event type[%d] time[%d] clientID[0x%08x]", type,
                 time, clientID );
#endif // #ifdef DEBUG
            return NULL;
        }
...
GameEvent* GameEventQueue::Create( NET_Packet& P,
                                   u16 type,
                                   u32 time,
                                   ClientID clientID ) {
    GameEvent* ge = 0;
...
u32 GameEventQueue::EraseEvents( event_predicate to_del ) {
    ...
    eq_iterator need_to_erase =
        std::find_if( ready.begin(), ready.end(), to_del );
    while ( need_to_erase != ready.end() ) {
        ...
        //-----
#ifdef DEBUG
        Msg( "! GameEventQueue::EraseEvents - destroying event type[%d], "
             "sender[0x%08x]",
             ( *need_to_erase )->type, ( *need_to_erase )->sender );
#endif
