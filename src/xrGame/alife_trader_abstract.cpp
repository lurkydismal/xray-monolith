...
void CSE_ALifeTraderAbstract::spawn_supplies() {
    CSE_ALifeDynamicObject* dynamic_object =
        smart_cast< CSE_ALifeDynamicObject* >( this );
    VERIFY( dynamic_object );
    CSE_Abstract* abstract = dynamic_object->alife().spawn_item(
        "device_pda", base()->o_Position, dynamic_object->m_tNodeID,
        dynamic_object->m_tGraphID, base()->ID );
...
#pragma warning( disable : 4238 )
            CInifile ini( &IReader( ( void* )( *dynamic_object->m_ini_string ),
...
void CSE_ALifeDynamicObject::attach(
    ...
    R_ASSERT2( std::find( children.begin(), children.end(),
                          tpALifeInventoryItem->base()->ID ) == children.end(),
...
void CSE_ALifeDynamicObject::detach(
    ...
    CSE_ALifeDynamicObject* l_tpALifeDynamicObject1 =
        smart_cast< CSE_ALifeDynamicObject* >( tpALifeInventoryItem );
    ...
    ALife::OBJECT_IT i = std::find( children.begin(), children.end(),
                                    tpALifeInventoryItem->base()->ID );
    R_ASSERT2( children.end() != i,
               "Can't detach an item which is not on my own" );
    children.erase_fast( i );
}

void add_online_impl( CSE_ALifeDynamicObject* object,
    ...
    for ( ; I != E; ++I ) {
        ...
        // R_ASSERT2				(l_tpALifeInventoryItem,"Non
        // inventory item object has parent?!");
        ...
        Msg( "[LSS][%d] Going online [%d][%s][%d] with parent [%d][%s] on '%s'",
             ...
             l_tpALifeInventoryItem->base()->ID, object->ID,
...
void add_offline_impl( CSE_ALifeDynamicObject* object,
    ...
    for ( u32 i = 0, n = saved_children.size(); i < n; ++i ) {
        ...
        // R_ASSERT(child);
        if ( !child ) {
            Msg( "[TR] can't switch child [%d] offline, it's null",
                 saved_children[ i ] );
            continue;
        }
        ...
        Msg(
            ...
            object->ID, object->name_replace(), "*SERVER*" );
#endif
        ...
        object->alife().graph().attach( *object, inventory_item,
                                        child->m_tGraphID, true );
    }
