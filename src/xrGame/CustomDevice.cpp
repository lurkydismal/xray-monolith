...
bool CCustomDevice::CheckCompatibilityInt( CHudItem* itm,
                                           u16* slot_to_activate ) {
    if ( itm == nullptr )
        return true;

    CInventoryItem& iitm = itm->item();
    u32 slot = iitm.BaseSlot();
    bool bres = ( slot == INV_SLOT_2 || slot == KNIFE_SLOT ||
                  slot == BOLT_SLOT ); // || iitm.AllowDevice();
    if ( !bres && slot_to_activate ) {
        *slot_to_activate = NO_ACTIVE_SLOT;
        ...
        if ( m_pInventory->ItemFromSlot( INV_SLOT_3 ) &&
             m_pInventory->ItemFromSlot( INV_SLOT_3 )->BaseSlot() !=
                 INV_SLOT_3 )
            *slot_to_activate = INV_SLOT_3;

        if ( m_pInventory->ItemFromSlot( INV_SLOT_2 ) &&
             m_pInventory->ItemFromSlot( INV_SLOT_2 )->BaseSlot() !=
                 INV_SLOT_3 )
    ...
    if ( bres ) {
        ...
        if ( W )
            bres = bres && ( W->GetState() != CHUDState::eBore ) &&
...
void CCustomDevice::OnStateSwitch( u32 S, u32 oldState ) {
    ...
    switch ( S ) {
        case eShowing: {
            ...
            if ( !IsUsingCondition() ||
                 ( m_CustomDeviceEnabled && IsUsingCondition() &&
                   GetCondition() >= m_fLowestBatteryCharge ) )
                TurnDeviceInternal( true );

            m_sounds.PlaySound( "sndShow", Fvector().set( 0, 0, 0 ), this, true,
                                false );
            ...
            m_bZoomed&& HudAnimationExist( "anm_zoom_show" )
                ? PlayHUDMotion( "anm_zoom_show", FALSE, this, GetState(), 1.f,
                                 0.f, false )
                : PlayHUDMotion( m_bFastAnimMode ? "anm_show_fast" : "anm_show",
                                 FALSE, this, GetState(), 1.f, 0.f, false );
            SetPending( TRUE );
        } break;
        case eHiding: {
            if ( oldState != eHiding ) {
                m_sounds.PlaySound( "sndHide", Fvector().set( 0, 0, 0 ), this,
                                    true, false );

                m_fZoomfactor > .5f &&
                        ( oldState == eIdleZoom || oldState == eIdleZoomIn ||
                          oldState == eIdleZoomOut || oldState == eShowing ) &&
                        HudAnimationExist( m_bFastAnimMode
                                               ? "anm_zoom_hide_fast"
                                               : "anm_zoom_hide" )
                    ? PlayHUDMotion( m_bFastAnimMode ? "anm_zoom_hide_fast"
                                                     : "anm_zoom_hide",
                                     TRUE, this, GetState() )
                    : PlayHUDMotion(
                          m_bFastAnimMode ? "anm_hide_fast" : "anm_hide", TRUE,
                          this, GetState() );
                SetPending( TRUE );
        ...
        case eIdleZoomIn: {
            ...
            if ( HudAnimationExist( m_bOldZoom ? "anm_zoom_in"
                                               : "anm_zoom" ) ) {
                PlayHUDMotion( m_bOldZoom ? "anm_zoom_in" : "anm_zoom", TRUE,
                               this, GetState() );
            } else {
        ...
        case eIdleZoomOut: {
            m_bZoomed = false;
            if ( HudAnimationExist( m_bOldZoom ? "anm_zoom_out"
                                               : "anm_zoom" ) ) {
                PlayHUDMotion( m_bOldZoom ? "anm_zoom_out" : "anm_zoom", TRUE,
                               this, GetState() );
            } else {
                SwitchState( eIdle );
            }
    ...
    switch ( state ) {
        case eShowing: {
            if ( m_bCanBeZoomed ) {
                ...
                if ( i0 ) {
                    CWeapon* wpn =
                        smart_cast< CWeapon* >( i0->m_parent_hud_item );
                    if ( wpn && wpn->IsZoomed() ) {
                        SwitchState( eIdleZoom );
                        return;
                    }
...
void CCustomDevice::Load( LPCSTR section ) {
    ...
    m_bOldZoom = pSettings->line_exist( hud_sect, "anm_zoom_in" ) &&
                 pSettings->line_exist( hud_sect, "anm_zoom_out" );
}
...
bool CCustomDevice::IsWorking() {
    return m_bWorking && H_Parent() &&
           H_Parent() == Level().CurrentViewEntity();
}
    ...
    if ( i0 && IsAttachedToHUD() ) {
        if ( bClimb ) {
            ...
            if ( msl && m_bThrowAnm ) {
                ...
                if ( ( state == CMissile::eThrowStart ||
                       state == CMissile::eReady ) &&
                     GetState() == eIdle )
                    SwitchState( eIdleThrowStart );
                else if ( state == CMissile::eThrow &&
                          GetState() == eIdleThrow )
                    SwitchState( eIdleThrowEnd );
                else if ( state == CMissile::eHiding &&
                          ( GetState() == eIdleThrowStart ||
                            GetState() == eIdleThrow ) )
                    SwitchState( eIdle );
            } else {
                ...
                if ( wpn ) {
                    ...
                    if ( ( wpn->IsZoomed() && !m_bCanBeZoomed ) ||
                         state == CWeapon::eReload ||
                        ...
                        if ( ( GetState() == eIdle ||
                               GetState() == eIdleZoomOut ||
                               GetState() == eShowing ) &&
                             !m_current_motion.equal( "anm_zoom_show" ) )
                            SwitchState( eIdleZoomIn );
                    } else if ( GetState() == eIdleZoom ||
                                GetState() == eIdleZoomIn ) {
                        SwitchState( eIdleZoomOut );
                    } else if ( m_bNeedActivation && !bClimb &&
                                CheckCompatibilityInt( i0->m_parent_hud_item,
                                                       0 ) ) {
                        ShowDevice( true );
                    }
                }
        ...
        if ( bClimb ) {
            ...
            if ( m_bNeedActivation &&
                 ( !i0 || ( i0 && CheckCompatibilityInt( i0->m_parent_hud_item,
                                                         0 ) ) ) )
                ShowDevice( true );
        }
...
void CCustomDevice::UpdateCL() {
    ...
    if ( IsUsingCondition() ) {
        if ( m_bWorking && ( !m_CustomDeviceEnabled ||
                             GetCondition() < m_fLowestBatteryCharge ) )
            TurnDeviceInternal( false );
        else if ( m_CustomDeviceEnabled && !m_bWorking &&
                  ( GetState() == eIdle ||
                    m_CustomDeviceEnabled && GetState() == eIdleZoom ) &&
                  GetCondition() >= m_fLowestBatteryCharge )
            TurnDeviceInternal( true );
    }
