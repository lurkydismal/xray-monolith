...
#define CSingleLinkedList                                                \
    CDataStorageSingleLinkedList< sorted >::CDataStorage< _data_storage, \
                                                          _vertex >

TEMPLATE_SPECIALIZATION
IC CSingleLinkedList::CDataStorage( const u32 vertex_count,
...
TEMPLATE_SPECIALIZATION
IC void CSingleLinkedList::decrease_opened( CGraphVertex& vertex,
...
TEMPLATE_SPECIALIZATION
IC typename CSingleLinkedList::CGraphVertex& CSingleLinkedList::get_best()
    const {
    VERIFY( !is_opened_empty() );
    ...
    for ( CGraphVertex *i = m_list_head, *best_prev = 0;
          i->next() != m_list_tail; i = i->next() )
        if ( i->next()->f() < fmin ) {
            fmin = i->next()->f();
            best_prev = i;
        }
