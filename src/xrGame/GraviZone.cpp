...
void CBaseGraviZone::Load( LPCSTR section ) {
    ...
    m_fThrowInImpulse =
        pSettings->r_float( section, "throw_in_impulse" ); // 800.f;
    m_fThrowInImpulseAlive =
        pSettings->r_float( section, "throw_in_impulse_alive" ); // 800.f;
    m_fThrowInAtten = pSettings->r_float( section, "throw_in_atten" );
    m_fBlowoutRadiusPercent =
        pSettings->r_float( section, "blowout_radius_percent" ); // 0.3f;
    ...
    if ( pSettings->line_exist( section, "tele_particles_big" ) )
        m_sTeleParticlesBig =
            pSettings->r_string( section, "tele_particles_big" );
    else
        m_sTeleParticlesBig = NULL;

    if ( pSettings->line_exist( section, "tele_particles_small" ) )
        m_sTeleParticlesSmall =
            pSettings->r_string( section, "tele_particles_small" );
    else
    ...
    if ( !result ) {
        if ( m_dwTeleTime > m_dwTimeToTele ) {
            for ( OBJECT_INFO_VEC_IT it = m_ObjectInfoMap.begin();
                ...
                CPhysicsShellHolder* GO =
                    smart_cast< CPhysicsShellHolder* >( ( *it ).object );

                if ( GO && GO->PPhysicsShell() &&
                     Telekinesis().is_active_object( GO ) ) {
        ...
        if ( m_dwTeleTime > m_dwTimeToTele + m_dwTelePause ) {
            ...
            for ( OBJECT_INFO_VEC_IT it = m_ObjectInfoMap.begin();
                ...
                if ( GO && GO->PPhysicsShell() &&
                     !Telekinesis().is_active_object( GO ) ) {
                    Telekinesis().activate( GO, 0.1f, m_fTeleHeight,
                                            m_dwTimeToTele );
...
bool CBaseGraviZone::CheckAffectField( CPhysicsShellHolder* GO,
...
void CBaseGraviZone::Affect( SZoneObjectInfo* O ) {
    ...
    if ( CheckAffectField( GO, dist_to_radius ) && CanApplyPhisImpulse ) {
        ...
        if ( m_dwBlowoutExplosionTime < ( u32 )m_iPreviousStateTime ||
            ...
            AffectPull( GO, throw_in_dir,
                        BlowoutRadiusPercent( GO ) * Radius() );
            return;
        }
...
void CBaseGraviZone::AffectPull( CPhysicsShellHolder* GO,
                                 const Fvector& throw_in_dir,
                                 float dist ) {
...
void CBaseGraviZone::AffectPullAlife( CEntityAlive* EA,
                                      const Fvector& throw_in_dir,
                                      float dist ) {
    float rel_power = RelativePower( dist, Radius() );
    float throw_power = m_fThrowInImpulseAlive * rel_power * rel_power *
...
void CBaseGraviZone::AffectPullDead( CPhysicsShellHolder* GO,
                                     const Fvector& throw_in_dir,
                                     float dist ) {
    GO->PPhysicsShell()->applyImpulse(
        throw_in_dir, dist * m_fThrowInImpulse * GO->GetMass() / 100.f );
}

void CBaseGraviZone::AffectThrow( SZoneObjectInfo* O,
                                  CPhysicsShellHolder* GO,
                                  const Fvector& throw_in_dir,
                                  float dist ) {
    ...
    float power = Power(
        dist, Radius() ); // Power(GO->Position().distance_to(zone_center));
    float impulse = m_fHitImpulseScale * power * GO->GetMass();

    if ( power > 0.01f ) {
        position_in_bone_space.set( 0.f, 0.f, 0.f );
        CreateHit( GO->ID(), ID(), throw_in_dir, power, 0,
                   position_in_bone_space, impulse, m_eHitTypeBlowout );
        PlayHitParticles( GO );
    }
}
