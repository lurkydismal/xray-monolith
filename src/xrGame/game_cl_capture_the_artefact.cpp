...
void game_cl_CaptureTheArtefact::SetGameUI(
    CUIGameCustom* already_created_ui ) {
    ...
    switch ( Phase() ) {
        case GAME_PHASE_INPROGRESS: {
            if ( m_game_ui ) {
                if ( local_player && !local_player->IsSkip() ) {
                    if ( !m_bReadMapDesc && Level().CurrentEntity() ) {
                        m_bReadMapDesc =
                            m_game_ui->ShowServerInfo() ? TRUE : FALSE;
                        GetActiveVoting();
                    }

                    UpdateMoneyIndicator();

                    if ( ( local_player->testFlag(
                             GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ) &&
                         ( static_cast< ETeam >( local_player->team ) !=
                           etSpectatorsTeam ) ) {
                        /*if (!sendedSpawnMe)
                                SpawnMe();*/

                        if ( ( local_player->money_for_round + spawn_cost +
                               buy_amount ) >= 0 ) {
                ...
                if ( InWarmUp() ) {
                    ...
                    m_game_ui->SetReinforcementTimes( m_curReinforcementTime,
                                                      m_maxReinforcementTime );
                }
            ...
            /*if (Level().CurrentControlEntity()){
                            CGameObject* GO =
            smart_cast<CGameObject*>(Level().CurrentControlEntity());
                            Msg("---I'm ready (ID = %d) sending player ready
            packet !!!", GO->ID()); NET_Packet			P;
...
void game_cl_CaptureTheArtefact::TranslateGameMessage( u32 msg,
                                                       NET_Packet& P ) {
    CStringTable st;
    string1024 Text;
    // string512 tmp;
    //	LPSTR	Color_Teams[3]		= {"%c[255,255,255,255]",
    ...
    switch ( msg ) {
        ...
        case GAME_EVENT_ARTEFACT_TAKEN: {
            ...
            VERIFY2( playerIt != players.end(),
                     make_string( "player (ClientID = 0x%08x) that took the "
                                  "artefact not found on client site",
                                  clientId.value() )
                         .c_str() );

            game_PlayerState const* ps = playerIt->second;
            VERIFY2( ps, make_string(
                             "player state (ClientID = 0x%08x) not initialized",
                             clientId.value() )
                             .c_str() );
            ...
            if ( ps->team == artefactOwnerTeam ) {
                ...
                xr_sprintf( Text, "%s%s %s%s",
                            CTeamInfo::GetTeam_color_tag(
                                ModifyTeam( artefactOwnerTeam ) + 1 ),
                            ps->getName(), Color_Main,
                ...
                xr_sprintf( Text, "%s%s", Color_Main,
                            *st.translate( "mp_you_captured_artefact" ) );
        ...
        case GAME_EVENT_ARTEFACT_DROPPED: // ahunt
        {
            ...
            if ( artefactOwnerTeam == static_cast< u8 >( etGreenTeam ) ) {
                if ( local_player && !local_player->IsSkip() )
                    Level().MapManager().RemoveMapLocationByObjectID(
                        greenArtefactOwner );
                greenArtefactOwner = 0;
            } else {
                if ( local_player && !local_player->IsSkip() )
            ...
            if ( ps ) {
                xr_sprintf(
                    ...
                    CTeamInfo::GetTeam_color_tag( ModifyTeam( ps->team ) + 1 ),
                    ps->getName(), Color_Main,
                    st.translate( "mp_has_dropped_artefact" )
                        .c_str() ); // need translate
        ...
        case GAME_EVENT_ARTEFACT_ONBASE: {
            ...
            if ( !local_player ) // can be NULL, because not actor or spectator
                                 // spawned yet...
                return;
...
void game_cl_CaptureTheArtefact::PlayCapturedTheArtefact(
    game_PlayerState const* capturer ) {
...
void game_cl_CaptureTheArtefact::PlayReturnedTheArtefact(
    game_PlayerState const* returnerer ) {
...
void game_cl_CaptureTheArtefact::PlayDeliveredTheArtefact(
    game_PlayerState const* deliverer ) {
...
void game_cl_CaptureTheArtefact::LoadSndMessages() {
    LoadSndMessage( "cta_snd_messages", "team1_artefact_on_base",
                    ID_AF_TEAM1_ONBASE );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_on_base",
                    ID_AF_TEAM2_ONBASE );
    LoadSndMessage( "cta_snd_messages", "team1_artefact_on_base_r",
                    ID_AF_TEAM1_ONBASE_R );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_on_base_r",
                    ID_AF_TEAM2_ONBASE_R );
    LoadSndMessage( "cta_snd_messages", "team1_artefact_on_base_enemy",
                    ID_AF_TEAM1_ONBASE_ENEMY );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_on_base_enemy",
                    ID_AF_TEAM2_ONBASE_ENEMY );

    LoadSndMessage( "cta_snd_messages", "team1_artefact_returned",
                    ID_AF_TEAM1_RETURNED );
    LoadSndMessage( "cta_snd_messages", "team1_artefact_returned_r",
                    ID_AF_TEAM1_RETURNED_R );
    LoadSndMessage( "cta_snd_messages", "team1_artefact_returned_enemy",
                    ID_AF_TEAM1_RETURNED_ENEMY );

    LoadSndMessage( "cta_snd_messages", "team2_artefact_returned",
                    ID_AF_TEAM2_RETURNED );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_returned_r",
                    ID_AF_TEAM2_RETURNED_R );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_returned_enemy",
                    ID_AF_TEAM2_RETURNED_ENEMY );

    LoadSndMessage( "cta_snd_messages", "team1_artefact_take",
                    ID_AF_TEAM1_TAKE );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_take",
                    ID_AF_TEAM2_TAKE );
    LoadSndMessage( "cta_snd_messages", "team1_artefact_take_r",
                    ID_AF_TEAM1_TAKE_R );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_take_r",
                    ID_AF_TEAM2_TAKE_R );
    LoadSndMessage( "cta_snd_messages", "team1_artefact_take_enemy",
                    ID_AF_TEAM1_TAKE_ENEMY );
    LoadSndMessage( "cta_snd_messages", "team2_artefact_take_enemy",
...
CUIGameCustom* game_cl_CaptureTheArtefact::createGameUI() {
    ...
    m_game_ui = smart_cast< CUIGameCTA* >(
        NEW_INSTANCE( CLSID_GAME_UI_CAPTURETHEARTEFACT ) );
...
const shared_str& game_cl_CaptureTheArtefact::GetLocalPlayerTeamSection()
    const {
    VERIFY2( TeamList.size() > local_player->team,
             make_string( "local_player has not valid team number: %d",
                          local_player->team )
                 .c_str() );
...
void game_cl_CaptureTheArtefact::OnGameMenuRespond_ChangeSkin( NET_Packet& P ) {
    ...
    Msg( "* player [%s][%d] changed skin to %d", local_player->getName(),
         local_player->GameID, local_player->skin );
...
void game_cl_CaptureTheArtefact::OnGameMenuRespond_ChangeTeam( NET_Packet& P ) {
    ...
    VERIFY( local_player );
    Msg( "* player [%s][%d] changed team to %d", local_player->getName(),
         local_player->GameID, local_player->team );
...
void game_cl_CaptureTheArtefact::UpdateMapLocations() {
    ...
    for ( PLAYERS_MAP_IT tempPlayerIt = players.begin(); tempPlayerIt != ie;
          ++tempPlayerIt ) {
    ...
    if ( greenArtefact && blueArtefact ) {
        ...
        if ( local_player->team == etGreenTeam ) {
            tempLocation = Level().MapManager().AddMapLocation(
                FREE_ARTEFACT_FRIEND, greenArtefact );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
            tempLocation = Level().MapManager().AddMapLocation(
                ARTEFACT_NEUTRAL, blueArtefact );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
        } else {
            tempLocation = Level().MapManager().AddMapLocation(
                ARTEFACT_NEUTRAL, greenArtefact );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
            tempLocation = Level().MapManager().AddMapLocation(
                FREE_ARTEFACT_FRIEND, blueArtefact );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
        }
    }
    ...
    if ( local_player->team == static_cast< u8 >( etGreenTeam ) ) {
        if ( greenArtefactOwner ) {
            Level().MapManager().RemoveMapLocationByObjectID( greenArtefact );
            tempLocation = Level().MapManager().AddMapLocation(
                ARTEFACT_ENEMY, greenArtefactOwner );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
        }
        if ( blueArtefactOwner ) {
            Level().MapManager().RemoveMapLocationByObjectID( blueArtefact );
            tempLocation = Level().MapManager().AddMapLocation(
                ARTEFACT_FRIEND, blueArtefactOwner );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
        }
        ...
        if ( blueArtefactOwner ) {
            Level().MapManager().RemoveMapLocationByObjectID( blueArtefact );
            tempLocation = Level().MapManager().AddMapLocation(
                ARTEFACT_ENEMY, blueArtefactOwner );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
        }
        if ( greenArtefactOwner ) {
            Level().MapManager().RemoveMapLocationByObjectID( greenArtefact );
            tempLocation = Level().MapManager().AddMapLocation(
                ARTEFACT_FRIEND, greenArtefactOwner );
            VERIFY( tempLocation );
            tempLocation->EnablePointer();
        }
    }
...
void game_cl_CaptureTheArtefact::OnSpawn( CObject* pObj ) {
    ...
    if ( pArtefact ) {
        Level()
            .MapManager()
            .AddMapLocation( ARTEFACT_NEUTRAL, pObj->ID() )
            ->EnablePointer();
    ...
    if ( smart_cast< CWeapon* >( pObj ) ) {
        if ( pObj->H_Parent() ) {
            ...
            if ( ps ) {
                m_WeaponUsageStatistic->OnWeaponBought(
                    ps, pObj->cNameSect().c_str() );
            }
        }
    }
}

void game_cl_CaptureTheArtefact::SetInvinciblePlayer( u16 const gameId,
                                                      bool const invincible ) {
...
void game_cl_CaptureTheArtefact::OnPlayerFlagsChanged( game_PlayerState* ps ) {
    ...
    SetInvinciblePlayer( ps->GameID,
                         ps->testFlag( GAME_PLAYER_FLAG_INVINCIBLE ) );
}

void game_cl_CaptureTheArtefact::OnNewPlayerConnected(
    ClientID const& newClient ) {
    if ( m_game_ui ) {
...
bool game_cl_CaptureTheArtefact::OnKeyboardPress( int key ) {
    ...
    if ( ( Phase() == GAME_PHASE_INPROGRESS ) && ( m_game_ui ) &&
...
#ifndef MASTER_GOLD
    Msg( "---CanBeReady = true: [%s][%d]", local_player->getName(),
         local_player->GameID );
#endif // #ifndef MASTER_GOLD
    return true;
}

bool game_cl_CaptureTheArtefact::NeedToSendReady_Actor( int key,
                                                        game_PlayerState* ps ) {
    return inherited::NeedToSendReady_Actor( key, ps );
}

bool game_cl_CaptureTheArtefact::NeedToSendReady_Spectator(
    int key,
    game_PlayerState* ps ) {
    CStringTable st;
    bool res = inherited::NeedToSendReady_Spectator( key, ps );
    u32 gphase = Phase();
    if ( ( gphase == GAME_PHASE_INPROGRESS ) && ( key == kJUMP ) &&
        ...
        if ( ( ( local_player->money_for_round + spawn_cost + buy_amount ) >=
               0 ) &&
             ( static_cast< ETeam >( local_player->team ) !=
               etSpectatorsTeam ) ) {
...
void game_cl_CaptureTheArtefact::OnVoteStart( NET_Packet& P ) {
    ...
    static const char* scans_format = "%s %s %s %s %s";
    ...
    Msg( "---Received vote begin message: (command: %s), (player: %s)", command,
         player );
#endif
    ...
    int args_count =
        sscanf_s( command + cmd_len, scans_format, args[ 0 ], psize + 1,
                  args[ 1 ], psize + 1, args[ 2 ], psize + 1, args[ 3 ],
                  psize + 1, args[ 4 ], psize + 1 );
    if ( args_count < 0 )
        args_count = 0;
    ...
    VERIFY( t_vote_str );
    u32 fin_str_size =
        xr_strlen( t_vote_str ) + vstr_size + xr_strlen( player ) + 1;
    char* fin_str = static_cast< char* >( _alloca( fin_str_size ) );

#ifdef CLIENT_CTA_LOG
    Msg( "---Making finally string: (t_vote_str: %s), (vstr: %s), (player: %s)",
         t_vote_str, vstr, player );
#endif
...
void game_cl_CaptureTheArtefact::UpdateWarmupTime( u32 current_time ) {
    if ( !m_inWarmup )
        return;
    u32 remain_seconds =
        m_captions_manager.SetWarmupTime( m_currentWarmupTime, current_time );
    if ( remain_seconds != 0 ) {
...
void game_cl_CaptureTheArtefact::UpdateTimeLimit( u32 current_time ) {
    m_captions_manager.SetTimeLimit( m_start_time + m_s32TimeLimit,
                                     current_time );
}
...
void game_cl_CaptureTheArtefact::UpdateVotingTime( u32 current_time ) {
    if ( IsVotingEnabled() && IsVotingActive() &&
        ...
        xr_sprintf( VoteTimeResStr, st.translate( "mp_timeleft" ).c_str(),
                    MinitsLeft, SecsLeft, float( NumAgreed ) / players.size() );
        if ( m_game_ui )
...
u16 game_cl_CaptureTheArtefact::GetGreenArtefactOwnerID() const {
    R_ASSERT2( haveGotUpdate,
               "synchronization problem: not received client update, while try "
               "to get values" );
...
void game_cl_CaptureTheArtefact::OnRender() {
    ...
    if ( local_player && ( local_player == lookat_player ) &&
        ...
        for ( ; it != players.end(); ++it ) {
            ...
            if ( ps->testFlag( GAME_PLAYER_FLAG_INVINCIBLE ) && !ps->IsSkip() &&
                 ( ps->team != etSpectatorsTeam ) && ( ps != local_player ) ) {
                // pActor->RenderIndicator(IPos, pTS->Indicator_r1,
                // pTS->Indicator_r2, pTS->InvincibleShader);
            }
