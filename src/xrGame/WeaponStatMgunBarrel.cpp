...
void SStmBarrel::Load( LPCSTR section ) {
    ...
    m_bLightShotEnabled =
        !READ_IF_EXISTS( pSettings, r_bool, section, "light_disabled", false );
    LoadLights( section, "" );
...
BOOL SStmBarrel::net_Spawn( CSE_Abstract* DC ) {
    ...
    m_fire_bid = ini->line_exist( Name(), "fire_bone" )
                     ? K->LL_BoneID( ini->r_string( Name(), "fire_bone" ) )
                     : BI_NONE;
    m_drop_bid = ini->line_exist( Name(), "drop_bone" )
                     ? K->LL_BoneID( ini->r_string( Name(), "drop_bone" ) )
                     : BI_NONE;
    return TRUE;
}
...
void SStmBarrel::UpdateEx() {
    VERIFY3( m_fire_bid != BI_NONE, "Fire bone incorrect.",
             m_stm->cNameSect_str() );
    IKinematics* K = m_stm->Visual()->dcast_PKinematics();
...
void SStmBarrel::StartParticles( CParticlesObject*& pParticles,
...
void SStmBarrel::UpdateParticles( CParticlesObject*& pParticles,
                                  const Fvector& pos,
                                  const Fvector& vel ) {
    if ( !pParticles )
        return;
...
void SStmBarrel::StartFlameParticles() {
    ...
    StopFlameParticles();
    m_pFlameParticles =
        Particles::Details::Create( m_sFlameParticles.c_str(), FALSE );
    UpdateFlameParticles();
...
void SStmBarrel::UpdateFlameParticles() {
    if ( m_pFlameParticles == nullptr )
        return;
    m_pFlameParticles->SetXFORM(
        Fmatrix().set( m_fire_xfm ).translate_over( m_fire_pos ) );
...
void SStmBarrel::StartSmokeParticles( const Fvector& play_pos,
                                      const Fvector& parent_vel ) {
    if ( 0 == m_sSmokeParticles.size() )
        return;
    CParticlesObject* pSmokeParticles = nullptr;
    StartParticles( pSmokeParticles, m_sSmokeParticles.c_str(), play_pos,
                    parent_vel, true );
}
...
void SStmBarrel::OnShellDrop( const Fvector& play_pos,
                              const Fvector& parent_vel ) {
    if ( m_sShellParticles.size() == 0 )
        return;
    intrusive_ptr< CParticlesObject > pShellParticles =
        Particles::Details::Create( m_sShellParticles.c_str(), TRUE );
    pShellParticles->UpdateParent(
        Fmatrix().set( m_fire_xfm ).translate_over( play_pos ), parent_vel );
