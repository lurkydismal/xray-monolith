...
void xrServer::PerformSecretKeysSyncAck( xrClientData* xrCL, NET_Packet& P ) {
    ...
    VERIFY2( new_seed == xrCL->m_last_key_sync_request_seed,
             "cracker detected !" );
    secure_messaging::generate_key( xrCL->m_last_key_sync_request_seed,
                                    xrCL->m_secret_key );
}

void xrServer::SecureSendTo( xrClientData* xrCL,
                             NET_Packet& P,
                             u32 dwFlags,
                             u32 dwTimeout ) {
    VERIFY( xrCL );
    ...
    u32 checksum = secure_messaging::encrypt(
        enc_packet.B.data + sizeof( u16 ), enc_packet.B.count - sizeof( u16 ),
        xrCL->m_secret_key );
...
void xrServer::OnSecureMessage( NET_Packet& P, xrClientData* xrClSender ) {
    ...
    u32 dbg_encrypt_checksum = secure_messaging::encrypt(
        dbg_tmp_buff, sizeof( dbg_tmp_buff ), xrClSender->m_secret_key );
    u32 dbg_decrypt_checksum = secure_messaging::decrypt(
        dbg_tmp_buff, sizeof( dbg_tmp_buff ), xrClSender->m_secret_key );
    VERIFY( dbg_encrypt_checksum == dbg_decrypt_checksum );
#endif
    NET_Packet dec_packet;
    dec_packet.B.count = P.B.count - sizeof( u16 ) -
                         sizeof( u32 ); // - r_begin - crypt_check_sum
    P.r( dec_packet.B.data, dec_packet.B.count );
    u32 checksum = secure_messaging::decrypt(
        dec_packet.B.data, dec_packet.B.count, xrClSender->m_secret_key );
    u32 real_checksum = 0;
