...
CPatternFunction::CPatternFunction( LPCSTR caFileName, CEF_Storage* storage )
    : CBaseFunction( storage ) {
    m_dwPatternCount = 0;
    m_dwVariableCount = 0;
...
void CPatternFunction::vfLoadEF( LPCSTR caFileName ) {
    ...
    if ( EFC_VERSION != m_tEFHeader.dwBuilderVersion ) {
        FS.r_close( F );
        Msg( "! Evaluation function (%s) : Not supported version of the "
             "Evaluation Function Contructor",
             caPath );
        R_ASSERT( false );
        return;
    }
    ...
    for ( u32 i = 0; i < m_dwVariableCount; ++i ) {
        ...
        if ( i )
            m_dwaAtomicIndexes[ i ] =
                m_dwaAtomicIndexes[ i - 1 ] + m_dwaAtomicFeatureRange[ i - 1 ];
    }
    ...
    for ( u32 i = 0; i < m_dwPatternCount; ++i ) {
        ...
        F->r( &( m_tpPatterns[ i ].dwCardinality ),
              sizeof( m_tpPatterns[ i ].dwCardinality ) );
        m_tpPatterns[ i ].dwaVariableIndexes =
            xr_alloc< u32 >( m_tpPatterns[ i ].dwCardinality );
        F->r( m_tpPatterns[ i ].dwaVariableIndexes,
              m_tpPatterns[ i ].dwCardinality * sizeof( u32 ) );
        u32 m_dwComplexity = 1;
        for ( int j = 0; j < ( int )m_tpPatterns[ i ].dwCardinality; ++j )
            m_dwComplexity *=
                m_dwaAtomicFeatureRange[ m_tpPatterns[ i ]
                                             .dwaVariableIndexes[ j ] ];
    ...
    // Msg			("* Evaluation function \"%s\" is successfully
    // loaded",m_caName);
}
...
float CPatternFunction::ffEvaluate() {
    ...
    for ( u32 i = 0; i < m_dwPatternCount; ++i )
        fResult +=
            m_faParameters[ dwfGetPatternIndex( m_dwaVariableValues, i ) ];
    return ( fResult );
}
...
float CPatternFunction::ffGetValue() {
    for ( u32 i = 0; i < m_dwVariableCount; ++i )
        m_dwaVariableValues[ i ] =
            ef_storage()
                .m_fpaBaseFunctions[ m_dwaVariableTypes[ i ] ]
                ->dwfGetDiscreteValue( m_dwaAtomicFeatureRange[ i ] );
    ...
    if ( psAI_Flags.test( aiFuncs ) ) {
        ...
        for ( i = 0; i < m_dwVariableCount; ++i )
            j += xr_sprintf( caString + j, sizeof( caString ) - j, " %3d",
                             m_dwaVariableValues[ i ] + 1 );
