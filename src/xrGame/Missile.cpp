...
void CMissile::Load( LPCSTR section ) {
    ...
    if ( pSettings->line_exist( section, "snd_throw" ) )
        m_sounds.LoadSound( section, "snd_throw", "sndThrow", false,
                            SOUND_TYPE_ITEM_HIDING );

    if ( pSettings->line_exist( section, "snd_checkout" ) )
        m_sounds.LoadSound( section, "snd_checkout", "sndCheckout", false,
...
void CMissile::spawn_fake_missile() {
    ...
    if ( !getDestroy() ) {
        ...
        CSE_ALifeObject* alife_object =
            smart_cast< CSE_ALifeObject* >( object );
        VERIFY( alife_object );
...
void CMissile::UpdateCL() {
    ...
    if ( pActor && !pActor->AnyMove() &&
        ...
        if ( hud_adj_mode == 0 && g_player_hud->script_anim_part == u8( -1 ) &&
             GetState() == eIdle &&
    ...
    if ( GetState() == eReady ) {
        if ( m_throw ) {
            ...
            if ( actor ) {
                m_fThrowForce +=
                    ( m_fForceGrowSpeed * Device.dwTimeDelta ) * .001f;
...
void CMissile::State( u32 state, u32 old_state ) {
    switch ( state ) {
        case eShowing: {
            ...
            SetPending( TRUE );
            PlayHUDMotion( "anm_show", FALSE, this, GetState(), 1.f, 0.f,
                           false );

            if ( m_sounds.FindSoundItem( "sndShow", false ) )
                m_sounds.PlaySound( "sndShow", H_Root()->Position(), H_Root(),
                                    !!GetHUDmode() );
        } break;
        case eIdle: {
            SetPending( FALSE );
            ...
            if ( H_Parent() ) {
                if ( old_state != eHiding ) {
                    ...
                    if ( m_sounds.FindSoundItem( "sndHide", false ) )
                        m_sounds.PlaySound( "sndHide", H_Root()->Position(),
                                            H_Root(), !!GetHUDmode() );
                }
            }
        } break;
        ...
        case eThrowEnd: {
            ...
            /*	case eBore:
                            {
                                    PlaySound
               (sndPlaying,Position()); PlayHUDMotion		("anm_bore",
               TRUE, this, GetState()); } break;
            */
    }
}
...
void CMissile::UpdateXForm() {
    if ( Device.dwFrame != dwXF_Frame ) {
        ...
        const CInventoryOwner* parent =
            smart_cast< const CInventoryOwner* >( E );
...
void CMissile::Throw() {
    ...
    if ( inventory_owner->use_default_throw_force() )
        m_fake_missile->m_fThrowForce =
            m_constpower ? m_fConstForce : m_fThrowForce;
    else
...
void CMissile::OnEvent( NET_Packet& P, u16 type ) {
    ...
    switch ( type ) {
        case GE_OWNERSHIP_TAKE: {
            P.r_u16( id );
            CMissile* missile =
                smart_cast< CMissile* >( Level().Objects.net_Find( id ) );
...
bool CMissile::Action( u16 cmd, u32 flags ) {
    ...
    switch ( cmd ) {
        ...
        case kWPN_ZOOM: {
            ...
            } else if ( GetState() == eReady || GetState() == eThrowStart ||
                        GetState() == eIdle ) {
                m_throw = true;
                if ( GetState() == eReady )
                    SwitchState( eThrow );
    ...
    if ( Device.dwFrame != dwFP_Frame ) {
        ...
        if ( GetHUDmode() && !IsHidden() ) {
            ...
            /*
                                    ...
                                    CKinematics* V			=
               smart_cast<CKinematics*>(GetHUD()->Visual()); VERIFY
               (V); V->CalculateBones		();

                                    // fire point&direction
                                    Fmatrix& parent			=
               GetHUD()->Transform	(); m_throw_direction.set
               (parent.k);
            */
        } else {
            // 3rd person
...
void CMissile::activate_physic_shell() {
    if ( !smart_cast< CMissile* >( H_Parent() ) ) {
        ...
        if ( m_pPhysicsShell && m_pPhysicsShell->isActive() &&
            ...
            m_pPhysicsShell->set_CallbackData(
                smart_cast< CPhysicsShellHolder* >( H_Root() ) );
        }
        return;
    }
    ...
    Fvector a_vel;
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( H_Root() );
    ...
    if ( entity_alive && entity_alive->character_physics_support() ) {
        Fvector parent_vel;
        entity_alive->character_physics_support()
            ->movement()
            ->GetCharacterVelocity( parent_vel );
    ...
    m_pPhysicsShell->add_ObjectContactCallback( ExitContactCallback );
    m_pPhysicsShell->set_CallbackData(
        smart_cast< CPhysicsShellHolder* >( entity_alive ) );
    ...
    if ( m_fThrowForce != 0.f && !m_sCheckoutBones.empty() ) {
        ...
        for ( const auto& boneName : m_sCheckoutBones ) {
            ...
            if ( bone_id != BI_NONE &&
                 kinematics->LL_GetBoneVisible( bone_id ) )
...
void CMissile::net_Relcase( CObject* O ) {
    ...
    if ( PPhysicsShell() && PPhysicsShell()->isActive() ) {
        if ( O ==
             smart_cast< CObject* >( ( CPhysicsShellHolder* )PPhysicsShell()
                                         ->get_CallbackData() ) ) {
            PPhysicsShell()->remove_ObjectContactCallback(
                ExitContactCallback );
...
bool CMissile::render_item_ui_query() {
    ...
    return b_is_active_item && ( GetState() == eReady ) && !m_throw &&
...
void CMissile::ExitContactCallback( bool& do_colide,
                                    bool bo1,
                                    dContact& c,
                                    SGameMtl* /*material_1*/,
                                    SGameMtl* /*material_2*/ ) {
