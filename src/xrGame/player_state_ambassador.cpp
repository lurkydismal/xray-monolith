...
player_state_ambassador::player_state_ambassador(
...
void player_state_ambassador::OnWeapon_Fire( u16 sender,
...
void player_state_ambassador::OnPlayerTakeArtefact(
...
void player_state_ambassador::OnPlayerDropArtefact(
    game_PlayerState const* ps ) {
    game_PlayerState* tmp_local_player = m_owner->get_local_player();
    if ( !tmp_local_player ) {
        ++m_art_drop_count;
...
void player_state_ambassador::OnPlayerBringArtefact(
    game_PlayerState const* ps ) {
    m_art_drop_count = 0;
