...
void SBullet::Init( const Fvector& position,
    ...
    if ( !cartridge.param_s.tracer_silenced &&
         g_allow_silencer_hide_tracer ) // momopate: Make it optional
    {
        ...
        if ( g_obj ) {
            ...
            if ( weapon && weapon->IsSilencerAttached() )
                flags.allow_tracer =
                    ( weapon->GetSilencedTracers() ); // momopate: customize per
                                                      // weapon
        }
    }
    ...
    flags.magnetic_beam =
        !!cartridge.m_flags.test( CCartridge::cfMagneticBeam );
...
void populateBulletTable( ::luabind::object& table,
...
CBulletManager::CBulletManager()
    : pure_relcase( &CBulletManager::net_Relcase )
...
void CBulletManager::Load() {
    ...
    m_fTracerLengthMax =
        pSettings->r_float( bullet_manager_sect, "tracer_length_max" );
    m_fTracerLengthMin =
        pSettings->r_float( bullet_manager_sect, "tracer_length_min" );

    m_fGravityConst =
        pSettings->r_float( bullet_manager_sect, "gravity_const" );
    m_fAirResistanceK =
        pSettings->r_float( bullet_manager_sect, "air_resistance_k" );

    m_fMinBulletSpeed =
        pSettings->r_float( bullet_manager_sect, "min_bullet_speed" );
    m_fCollisionEnergyMin =
        pSettings->r_float( bullet_manager_sect, "collision_energy_min" );
    m_fCollisionEnergyMax =
        pSettings->r_float( bullet_manager_sect, "collision_energy_max" );

    m_fHPMaxDist =
        pSettings->r_float( bullet_manager_sect, "hit_probability_max_dist" );

    if ( pSettings->line_exist( bullet_manager_sect,
                                "bullet_velocity_time_factor" ) ) {
        g_bullet_time_factor = pSettings->r_float(
            bullet_manager_sect, "bullet_velocity_time_factor" );
    }

    LPCSTR whine_sounds =
        pSettings->r_string( bullet_manager_sect, "whine_sounds" );
    ...
    LPCSTR explode_particles =
        pSettings->r_string( bullet_manager_sect, "explode_particles" );
...
void CBulletManager::PlayExplodePS( const Fmatrix& xf ) {
    ...
    shared_str const& ps_name =
        m_ExplodeParticles[ Random.randI( 0, m_ExplodeParticles.size() ) ];
    intrusive_ptr< CParticlesObject > const ps =
        Particles::Details::Create( *ps_name, TRUE );
    ps->UpdateParent( xf, zero_vel );
    GamePersistent().ps_needtoplay.push_back( ps );
}

void CBulletManager::PlayWhineSound( SBullet* bullet,
                                     CObject* object,
                                     const Fvector& pos ) {
    ...
    bullet->m_whine_snd =
        m_WhineSounds[ Random.randI( 0, m_WhineSounds.size() ) ];
...
void CBulletManager::AddBullet(
    ...
    bullet.Init( position, direction, starting_speed, power,
                 /*power_critical,*/ impulse, sender_id, sendersweapon_id,
                 e_hit_type, maximum_distance, cartridge, air_resistance_factor,
                 SendHit, iShotNum );
    ...
    if ( !IsGameTypeSingle() ) {
        ...
        if ( tmp_cl_game->get_reward_generator() )
            tmp_cl_game->get_reward_generator()->OnBullet_Fire(
                sender_id, sendersweapon_id, position, direction );
    }
}
...
void CBulletManager::UpdateWorkload() {
    ...
    for ( u16 j = u16( e - i ); i != e; ++i, --j ) {
        if ( process_bullet( rq_storage, *i,
                             u32( time_delta * g_bullet_time_factor ) ) )
            continue;

        VERIFY( j > 0 );
        RegisterEvent( EVENT_REMOVE, FALSE, &*i, Fvector().set( 0, 0, 0 ),
                       dummy, j - 1 );
    }
}

static Fvector parabolic_velocity( Fvector const& start_velocity,
...
static Fvector trajectory_velocity( Fvector const& start_velocity,
                                    Fvector const& gravity,
                                    float const air_resistance,
                                    float const time ) {
    float const parabolic_time =
        _max( 0.f, 2.f / air_resistance - air_resistance_epsilon );
    float const fall_down_time = time - parabolic_time;
    //	float const fake_velocity	= start_velocity*2.f;
    if ( fall_down_time < 0.f ) {
        Fvector const xz_velocity =
            Fvector().set( start_velocity.x, 0.f, start_velocity.z );
        ...
        if ( !fis_zero( xz_velocity.square_magnitude() ) ) {
            return ( parabolic_velocity( start_velocity, gravity,
                                         air_resistance, time ) );
        }
    ...
    Fvector parabolic_velocity = ::parabolic_velocity(
        start_velocity, gravity, air_resistance, parabolic_time );

    VERIFY(
        !fis_zero( air_resistance_epsilon ) ||
        fis_zero( _sqr( parabolic_velocity.x ) + _sqr( parabolic_velocity.z ),
                  EPS_L ) );
    return ( parabolic_velocity.mad( gravity, fall_down_time ) );
}

static Fvector parabolic_position( Fvector const& start_position,
    ...
    return ( Fvector()
                 ...
                 .mad( Fvector( start_velocity ).mul( -air_resistance ),
                       sqr_t_div_2 )
...
static Fvector trajectory_position( Fvector const& start_position,
    ...
    // g_use_new_ballistics ? Fvector(base_gravity).mul(_sqr(factor)) :
    // base_gravity;
    ...
    // g_use_new_ballistics ? Fvector(base_start_velocity).mul( factor ) :
    // base_start_velocity;
    ...
    if ( fall_down_time < 0.f ) {
        Fvector const xz_velocity =
            Fvector().set( start_velocity.x, 0.f, start_velocity.z );
        if ( !fis_zero( xz_velocity.square_magnitude() ) )
            return ( parabolic_position( start_position, start_velocity,
                                         gravity, air_resistance, time ) );

        return ( Fvector( start_position )
                     .mad( start_velocity, time )
                     .mad( gravity, _sqr( time ) * .5f ) );
    }

    Fvector const parabolic_position =
        ::parabolic_position( start_position, start_velocity, gravity,
                              air_resistance, parabolic_time );
...
inline static float trajectory_max_error_time( float const t0,
                                               float const t1 ) {
    return ( ( t1 + t0 ) * .5f );
...
static float trajectory_pick_error( float const low,
                                    float const high,
    ...
    Fvector const start =
        trajectory_position( position, velocity, gravity, air_resistance, low );
    Fvector const target = trajectory_position( position, velocity, gravity,
                                                air_resistance, high );
    Fvector const max_error = trajectory_position(
        position, velocity, gravity, air_resistance, max_error_time );
    ...
    float cosine_alpha = _max(
        -1.f, _min( start_to_max_error.dotproduct( start_to_target ), 1.f ) );
    float sine_alpha = _sqrt( 1.f - _sqr( cosine_alpha ) );
    return ( magnitude * sine_alpha );
}

static float trajectory_select_pick_gravity( SBullet& bullet,
                                             float start_low,
                                             float const high,
                                             Fvector const& gravity,
                                             float const air_resistance ) {
    ...
    float time = ( _sqrt( _sqr( fall_down_velocity_magnitude ) +
                          2.f * max_test_distance * positive_gravity ) -
    ...
    VERIFY2(
        result <= high,
        make_string( "result[%f], high[%f], start_low[%f], air_resistance[%f]",
                     result, high, start_low, air_resistance ) );
    return ( result );
}

static float trajectory_select_pick_parabolic( SBullet& bullet,
    ...
    float const max_test_distance = bullet.max_dist - bullet.fly_dist;
    Fvector const start =
        trajectory_position( bullet.start_position, bullet.start_velocity,
                             gravity, air_resistance, start_low );
    float const start_high = high;
    float low = start_low;
    float check_time = high;
    while ( !fsimilar( low, high ) ) {
        Fvector const intermediate = trajectory_position(
            bullet.start_position, bullet.start_velocity, gravity,
            air_resistance, start_low + ( check_time - start_low ) * .5f );
        Fvector const target =
            trajectory_position( bullet.start_position, bullet.start_velocity,
                                 gravity, air_resistance, check_time );
        float const distance = start.distance_to( intermediate ) +
                               intermediate.distance_to( target );
...
static bool trajectory_select_pick_ranges( float& result,
                                           SBullet& bullet,
                                           float const low,
                                           float const high,
                                           Fvector const& gravity,
    ...
    if ( air_resistance * ( low + air_resistance_epsilon ) >= 1.f ) {
        result = trajectory_select_pick_gravity( bullet, low, high, gravity,
                                                 air_resistance );
        return ( true );
    }

    if ( air_resistance * ( high + air_resistance_epsilon ) < 1.f ) {
        result = trajectory_select_pick_parabolic( bullet, low, high, gravity,
                                                   air_resistance );
        return ( false );
    }
    ...
    if ( !fsimilar( fall_down_time, low ) ) {
        result = trajectory_select_pick_parabolic( bullet, low, fall_down_time,
                                                   gravity, air_resistance );
        return ( false );
    }

    result = trajectory_select_pick_gravity( bullet, fall_down_time, high,
                                             gravity, air_resistance );
    return ( false );
}

static float trajectory_select_pick_time( SBullet& bullet,
                                          float const start_low,
                                          float high,
                                          Fvector const& gravity,
                                          float const air_resistance ) {
    VERIFY2( start_low < high,
             make_string( "start_low[%f] high[%f]", start_low, high ) );
    float const start_high = high;
    if ( trajectory_select_pick_ranges( high, bullet, start_low, high, gravity,
                                        air_resistance ) ) {
        if ( high <= start_high )
            return ( high );
    ...
    while ( !fsimilar( low, high ) ) {
        float distance = trajectory_pick_error(
            start_low, check_time, bullet.start_position, bullet.start_velocity,
            gravity, air_resistance );
    ...
    VERIFY2( low <= start_high,
             make_string( "low[%f], high[%f]", low, start_high ) );
    return ( low );
}

void CBulletManager::add_bullet_point( Fvector const& start_position,
...
#ifdef DEBUG
    Fvector const temp = trajectory_position(
        start_position, start_velocity, gravity, air_resistance, current_time );
...
static void update_bullet_parabolic( SBullet& bullet,
                                     bullet_test_callback_data& data,
                                     Fvector const& gravity,
                                     float const air_resistance ) {
    Fvector xz_projection =
        Fvector( data.collide_position ).sub( bullet.start_position );
    xz_projection.y = 0;
    float const xz_range = xz_projection.magnitude();
    Fvector const xz_velocity =
        Fvector().set( bullet.start_velocity.x, 0.f, bullet.start_velocity.z );
    ...
    data.collide_position =
        trajectory_position( bullet.start_position, bullet.start_velocity,
                             gravity, air_resistance, data.collide_time );
    Fvector const new_velocity = trajectory_velocity(
        bullet.start_velocity, gravity, air_resistance, data.collide_time );
    bullet.speed = new_velocity.magnitude();
    bullet.dir = Fvector( new_velocity ).normalize_safe();
}

static void update_bullet_gravitation( SBullet& bullet,
                                       bullet_test_callback_data& data,
                                       Fvector const& gravity,
                                       float const air_resistance,
                                       float const fall_down_time ) {
    Fvector const fall_down_position =
        trajectory_position( bullet.start_position, bullet.start_velocity,
                             gravity, air_resistance, fall_down_time );
    Fvector const fall_down_velocity = trajectory_velocity(
        bullet.start_velocity, gravity, air_resistance, fall_down_time );
    VERIFY(
        !fis_zero( air_resistance_epsilon ) ||
        fis_zero( _sqr( fall_down_velocity.x ) + _sqr( fall_down_velocity.z ),
                  EPS_L ) );
    float const fall_down_velocity_magnitude = fall_down_velocity.magnitude();

    Fvector xz_projection =
        Fvector( data.collide_position ).sub( fall_down_position );
    xz_projection.y = 0;
    float const xz_range = xz_projection.magnitude();
    Fvector const xz_velocity =
        Fvector().set( fall_down_velocity.x, 0.f, fall_down_velocity.z );
    ...
    if ( !fis_zero( xz_velocity.magnitude() ) ) {
        ...
        //		VERIFY
        //(data.collide_time <= data.high_time); 		VERIFY
        //(data.collide_time >= bullet.life_time); 		VERIFY
        //(data.collide_time <= bullet.life_time + Device.fTimeGlobal);
        clamp( data.collide_time, bullet.life_time, data.high_time );
    } else {
        ...
        float const distance =
            fall_down_position.distance_to( data.collide_position );
        data.collide_time =
            fall_down_time + ( _sqrt( _sqr( fall_down_velocity_magnitude ) +
                                      2.f * distance * positive_gravity ) -
                               fall_down_velocity_magnitude ) /
                                 positive_gravity;
        VERIFY( data.collide_time >= 0.f );
...
static void update_bullet( SBullet& bullet,
                           bullet_test_callback_data& data,
                           Fvector const& gravity,
                           float const air_resistance ) {
    if ( air_resistance * ( bullet.life_time + air_resistance_epsilon ) >=
         1.f ) {
        update_bullet_gravitation(
            bullet, data, gravity, air_resistance,
            _max( 0.f, 1.f / air_resistance - air_resistance_epsilon ) );
        return;
    }
...
BOOL CBulletManager::firetrace_callback( collide::rq_result& result,
                                         LPVOID params ) {
    ...
    collide_position =
        Fvector().mad( bullet.bullet_pos, bullet.dir, result.range );
    ...
    // статический объект
    if ( !result.O ) {
        CDB::TRI const& triangle =
            *( Level().ObjectSpace.GetStaticTris() + result.element );
        bullet_manager.RegisterEvent( EVENT_HIT, FALSE, &bullet,
                                      collide_position, result,
                                      triangle.material );
        return ( FALSE );
    }

    // динамический объект
    VERIFY( !( result.O->ID() == bullet.parent_id &&
               bullet.fly_dist < parent_ignore_distance ) );
    IKinematics* const kinematics =
        smart_cast< IKinematics* >( result.O->Visual() );
    if ( !kinematics )
        return ( FALSE );

#ifdef CBULLETMANAGER_EX
    if ( result.O->GetBulletCheckVisual() &&
         !bullet_manager.ValidateHitDynamicVisualMesh(
             bullet, result, collide_position, data.max_distance ) ) {
        /* Ignore this hit and allow the bullet to continue flying. */
        return TRUE;
    }
#endif

    CBoneData const& bone_data =
        kinematics->LL_GetData( ( u16 )result.element );
    bullet_manager.RegisterEvent( EVENT_HIT, TRUE, &bullet, collide_position,
                                  result, bone_data.game_mtl_idx );
    return ( FALSE );
}

bool CBulletManager::trajectory_check_error( Fvector& previous_position,
    ...
    collide::ray_defs RD( start, start_to_target, distance, CDB::OPT_FULL_TEST,
                          collide::rqtBoth );
...
static bool try_update_bullet( SBullet& bullet,
                               Fvector const& gravity,
                               float const air_resistance,
                               float const time ) {
    ...
    Fvector const new_velocity = trajectory_velocity(
        bullet.start_velocity, gravity, air_resistance, bullet.life_time );
    bullet.speed = new_velocity.magnitude();
    if ( fis_zero( bullet.speed ) )
        return ( false );
    ...
    if ( ai().script_engine().functor( "_G.CBulletOnUpdate", funct ) ) {
        ::luabind::object table =
            ::luabind::newtable( ai().script_engine().lua() );
        populateBulletTable(
            table, bullet.bullet_pos, bullet.dir, bullet.speed, bullet.fly_dist,
            bullet.catridgeSection, bullet.bulletId, bullet.weapon_id,
            bullet.parent_id, 65535, NULL, bullet.life_time, -1 );
        funct( table );
...
bool CBulletManager::process_bullet( collide::rq_results& storage,
                                     SBullet& bullet,
    ...
    //	Msg
    //("process_bullet0: low[%f], high[%f]", low, high);
    ...
    for ( ;; ) {
        for ( ;; ) {
            ...
            float time = trajectory_select_pick_time( bullet, low, high,
                                                      gravity, air_resistance );
            if ( time == low )
                return ( false );

            float safe_time = time;
            VERIFY2( safe_time <= high, make_string( "safe_time[%f], high[%f]",
                                                     safe_time, high ) );
            if ( !trajectory_check_error( previous_position, storage, bullet,
                                          low, time, gravity,
                                          air_resistance ) ) {
                VERIFY2(
                    safe_time >= time,
                    make_string( "safe_time[%f], time[%f]", safe_time, time ) );
                VERIFY2(
                    safe_time <= high,
                    make_string( "safe_time[%f], high[%f]", safe_time, high ) );
                //				clamp
                //(safe_time, time, high);
                high = high - safe_time + time;
                VERIFY2( low <= high,
                         make_string( "start_low[%f] high[%f]", low, high ) );
...
float SqrDistancePointToSegment( const Fvector& pt,
                                 const Fvector& orig,
                                 const Fvector& dir ) {
    Fvector diff;
    diff.sub( pt, orig );
...
void CBulletManager::Render() {
    ...
    if ( g_bDrawBulletHit && !m_bullet_points.empty() ) {
        ...
        for ( ; i != e; i += 2 ) {
            ...
            renderer.draw_line( Fidentity, *i, *( i + 1 ),
    ...
    if ( g_bDrawBulletHit ) {
        ...
        for ( int i = 0; i < 3; ++i )
            for ( it = g_hit[ i ].begin(); it != g_hit[ i ].end(); ++it ) {
                Level().debug_renderer().draw_aabb( *it, 0.01f, 0.01f, 0.01f,
                                                    C[ i ] );
            }
    }
#endif
    ...
    UIRender->StartPrimitive( bullet_num * 12, IUIRender::ptTriList,
                              IUIRender::pttLIT );
    ...
    for ( SBullet& bullet : m_Bullets ) {
        ...
        Fvector const tracer =
            Fvector().sub( bullet.bullet_pos, bullet.tracer_start_position );
        float length = tracer.magnitude();
        Fvector const tracer_direction =
            length >= EPS_L ? Fvector( tracer ).mul( 1.f / length )
                            : Fvector().set( 0.f, 0.f, 1.f );

        if ( length < m_fTracerLengthMin ) {
            // momopate: Optionally cap tracers to their minimum length instead
        ...
        float dist2segSqr = SqrDistancePointToSegment(
            Device.vCameraPosition, bullet.bullet_pos, tracer );
        //---------------------------------------------
        ...
        if ( Device.vCameraPosition.distance_to_sqr( bullet.bullet_pos ) <
             ( length * length ) ) {
            length =
                Device.vCameraPosition.distance_to( bullet.bullet_pos ) - 0.3f;
        }
        ...
        tracers.Render( bullet.bullet_pos, center, tracer_direction, length,
                        width, bullet.m_u8ColorID, bullet.speed, bActor );
    }
...
void CBulletManager::CommitEvents() // @ the start of frame
{
        ...
        switch ( E.Type ) {
            case EVENT_HIT: {
                // demonized: bullet on impact callback
                {
                    xrCriticalSectionGuard guard( &m_Lock );
                    ::luabind::functor< void > funct;
                    if ( ai().script_engine().functor( "_G.CBulletOnImpact",
                                                       funct ) ) {
                        ::luabind::object table =
                            ::luabind::newtable( ai().script_engine().lua() );
                        populateBulletTable(
                            table,
                            !fis_zero( end_point.x ) &&
                                    !fis_zero( end_point.y ) &&
                                    !fis_zero( end_point.z )
                                ? end_point
                                : bullet->bullet_pos,
                            bullet->dir, bullet->speed,
                            bullet->fly_dist + E.R.range,
                            bullet->catridgeSection, bullet->bulletId,
                            bullet->weapon_id, bullet->parent_id,
                            E.dynamic && E.R.O ? E.R.O->ID() : 65535,
                            mt ? mt->m_Name.c_str() : NULL, bullet->life_time,
                            E.R.element );
                        funct( table );
                    }
                }
            ...
            case EVENT_REMOVE: {
                ...
                xrCriticalSectionGuard guard( &m_Lock );
                ::luabind::functor< void > funct;
                if ( ai().script_engine().functor( "_G.CBulletOnRemove",
                                                   funct ) ) {
                    ::luabind::object table =
                        ::luabind::newtable( ai().script_engine().lua() );
                    populateBulletTable(
                        table,
                        !fis_zero( end_point.x ) && !fis_zero( end_point.y ) &&
                                !fis_zero( end_point.z )
                            ? end_point
                            : bullet->bullet_pos,
    ...
    if ( g_mt_config.test( mtBullets ) ) {
        Device.seqParallel.push_back(
            xr_make_delegate( this, &CBulletManager::UpdateWorkload ) );
    } else {
        UpdateWorkload();
    }
}

void CBulletManager::RegisterEvent( EventType Type,
                                    BOOL _dynamic,
                                    SBullet* bullet,
    ...
    switch ( Type ) {
        case EVENT_HIT: {
            ...
            ObjectHit( &E.hit_result, bullet, end_point, R, tgt_material,
                       E.normal );
...
#ifdef CBULLETMANAGER_EX
bool CBulletManager::ValidateHitDynamicVisualMesh( SBullet& B,
                                                   collide::rq_result& R,
                                                   Fvector& collide_position,
                                                   float range ) {
    IKinematics::pick_result res;
    if ( R.O->Visual()->dcast_PKinematics()->PickBone( R.O->XFORM(), res, range,
                                                       B.bullet_pos, B.dir,
                                                       ( u16 )R.element ) ) {
