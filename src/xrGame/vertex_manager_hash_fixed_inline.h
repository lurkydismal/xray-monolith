...
#define TEMPLATE_SPECIALIZATION                                             \
    template < typename _path_id_type, typename _index_type, u32 hash_size, \
               u32 fix_size >                                               \
...
#define CHashFixedVertexManager                                                \
    CVertexManagerHashFixed< _path_id_type, _index_type, hash_size,            \
                             fix_size >::CDataStorage< _vertex, _index_vertex, \
                                                       _data_storage >
...
IC void CHashFixedVertexManager::init() {
    ...
    //	ZeroMemory
    //(m_hash,(hash_size)*sizeof(CGraphIndexVertex*)); 	ZeroMemory
    //(m_vertices,(fix_size)*sizeof(CGraphIndexVertex));
}
...
TEMPLATE_SPECIALIZATION
IC typename CHashFixedVertexManager::_path_id_type
CHashFixedVertexManager::current_path_id() const {
...
TEMPLATE_SPECIALIZATION
IC u32
CHashFixedVertexManager::hash_index( const _index_type& vertex_id ) const {
    return ( hash_fixed_vertex_manager::to_u32( vertex_id ) % hash_size );
}

TEMPLATE_SPECIALIZATION
IC bool CHashFixedVertexManager::is_visited(
    const _index_type& vertex_id ) const {
    u32 index = hash_index( vertex_id );
    CGraphIndexVertex* vertex = m_hash[ index ];
    if ( !vertex || ( vertex->m_path_id != current_path_id() ) ||
         ( vertex->m_hash != index ) )
        return ( false );
...
CHashFixedVertexManager::get_node( const _index_type& vertex_id ) const {
...
TEMPLATE_SPECIALIZATION
IC typename CHashFixedVertexManager::CGraphVertex&
CHashFixedVertexManager::create_vertex( CGraphVertex& vertex,
    ...
    if ( index_vertex->m_prev ) {
        ...
        if ( m_hash[ index_vertex->m_hash ] &&
             ( m_hash[ index_vertex->m_hash ]->m_path_id !=
               current_path_id() ) )
    ...
    if ( !_vertex || ( _vertex->m_path_id != current_path_id() ) ||
         ( _vertex->m_hash != index ) )
        _vertex = 0;
