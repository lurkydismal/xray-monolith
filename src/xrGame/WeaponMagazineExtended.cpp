...
void CWeaponMagazinedWGrenade::switch2_StartAim() {
    ...
    if ( IsGrenadeLauncherAttached() ) {
        if ( m_bGrenadeMode ) {
            if ( iAmmoElapsed == 0 &&
                 HudAnimationExist( "anm_idle_aim_start_g_empty" ) )
                PlayHUDMotion( "anm_idle_aim_start_g_empty", TRUE, this,
                               GetState() );
            else
                PlayHUDMotion( "anm_idle_aim_start_g", TRUE, this, GetState() );
        } else {
            if ( iAmmoElapsed == 0 &&
                 HudAnimationExist( "anm_idle_aim_start_w_gl_empty" ) )
                PlayHUDMotion( "anm_idle_aim_start_w_gl_empty", TRUE, this,
                               GetState() );
            else
                PlayHUDMotion( "anm_idle_aim_start_w_gl", TRUE, this,
                               GetState() );
        }
    } else
...
void CWeaponMagazinedWGrenade::switch2_EndAim() {
    ...
    if ( IsGrenadeLauncherAttached() ) {
        if ( m_bGrenadeMode ) {
            if ( iAmmoElapsed == 0 &&
                 HudAnimationExist( "anm_idle_aim_end_g_empty" ) )
                PlayHUDMotion( "anm_idle_aim_end_g_empty", TRUE, this,
                               GetState() );
            else
                PlayHUDMotion( "anm_idle_aim_end_g", TRUE, this, GetState() );
        } else {
            if ( iAmmoElapsed == 0 &&
                 HudAnimationExist( "anm_idle_aim_end_w_gl_empty" ) )
                PlayHUDMotion( "anm_idle_aim_end_w_gl_empty", TRUE, this,
                               GetState() );
            else
                PlayHUDMotion( "anm_idle_aim_end_w_gl", TRUE, this,
                               GetState() );
        }
    } else
