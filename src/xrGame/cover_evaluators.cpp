...
bool CCoverEvaluatorBase::inertia( Fvector const& position, float radius ) {
    ...
    bool time_criteria =
        ( Device.dwTimeGlobal < m_last_update + m_inertia_time );
    ...
    if ( !m_stalker->get_current_loophole() ||
         !cover->is_position_in_danger_fov( *m_stalker->get_current_loophole(),
                                            position ) )
        return false;
...
void CCoverEvaluatorBase::evaluate( CCoverPoint const* cover_point,
                                    float weight ) {
    ...
    smart_cover::cover const* tmp =
        static_cast< smart_cover::cover const* >( cover_point );
...
void CCoverEvaluatorCloseToEnemy::evaluate_cover(
    const CCoverPoint* cover_point,
    float weight ) {
...
void CCoverEvaluatorCloseToEnemy::evaluate_smart_cover(
    smart_cover::cover const* smart_cover,
    float const& weight ) {}
...
void CCoverEvaluatorFarFromEnemy::evaluate_cover(
    const CCoverPoint* cover_point,
    float weight ) {
    float enemy_distance =
        m_enemy_position.distance_to( cover_point->position() );
...
void CCoverEvaluatorFarFromEnemy::evaluate_smart_cover(
    smart_cover::cover const* smart_cover,
    float const& weight ) {}
...
bool CCoverEvaluatorBest::threat_on_the_way(
    Fvector const& cover_position ) const {
    Fvector const start_to_cover =
        Fvector().sub( cover_position, m_start_position );
    float const start_to_cover_magnitude = start_to_cover.magnitude();
    if ( start_to_cover_magnitude < EPS_L )
        return ( false );

    Fvector const start_to_threat =
        Fvector().sub( m_enemy_position, m_start_position );
    Fvector const start_to_cover_direction =
        Fvector( start_to_cover ).normalize();
    float const projection =
        start_to_cover_direction.dotproduct( start_to_threat );
...
void CCoverEvaluatorBest::evaluate_cover( const CCoverPoint* cover_point,
                                          float weight ) {
    if ( fis_zero( weight ) )
        return;
    ...
    float high_cover_value = ai().level_graph().high_cover_in_direction(
        y, cover_point->level_vertex_id() );
    float low_cover_value = ai().level_graph().low_cover_in_direction(
        y, cover_point->level_vertex_id() );
    float cover_value = _min( high_cover_value, low_cover_value );
    float value = cover_value;
    if ( ai().level_graph().neighbour_in_direction(
             direction, cover_point->level_vertex_id() ) )
        value += 10.f;

    if ( m_stalker &&
         m_stalker->agent_manager().member().members().size() > 1 ) {
        for ( auto& it : m_stalker->agent_manager().member().members() ) {
            CAI_Stalker* teammate = &it->object();
            if ( teammate->ID() == m_stalker->ID() )
                continue;

            // 1.5m squared is 2.25f. Huge penalty if a teammate is already
            // hiding here.
            if ( teammate->Position().distance_to_sqr(
                     cover_point->position() ) < 2.25f ) {
                value += 1000.f;
            }
        }
    }
...
void CCoverEvaluatorBest::evaluate_smart_cover(
    ...
    smart_cover::loophole* best_loophole = smart_cover->best_loophole(
        m_enemy_position, value, false,
        m_stalker->movement().current_params().cover() == smart_cover );
    if ( !best_loophole )
        return;
    ...
    //	Msg
    //("Loophole is NULL CCoverEvaluatorBest::evaluate_cover");
#endif // #if 1
}
...
void CCoverEvaluatorAngle::initialize( const Fvector& start_position,
    ...
    for ( float alpha = 0.f, step = PI_MUL_2 / 360.f; alpha < PI_MUL_2;
          alpha += step ) {
        float high_value = ai().level_graph().compute_high_square(
            alpha, PI_DIV_2, m_level_vertex_id );
        float low_value = ai().level_graph().compute_low_square(
            alpha, PI_DIV_2, m_level_vertex_id );
...
void CCoverEvaluatorAngle::evaluate_cover( const CCoverPoint* cover_point,
                                           float weight ) {
    float enemy_distance =
        m_enemy_position.distance_to( cover_point->position() );
...
void CCoverEvaluatorAngle::evaluate_smart_cover(
    smart_cover::cover const* smart_cover,
    float const& weight ) {}
...
void CCoverEvaluatorSafe::evaluate_cover( const CCoverPoint* cover_point,
                                          float weight ) {
    if ( m_start_position.distance_to( cover_point->position() ) <=
         m_min_distance )
        return;

    float high_cover_value =
        ai().level_graph().vertex_high_cover( cover_point->level_vertex_id() );
    float low_cover_value =
        ai().level_graph().vertex_low_cover( cover_point->level_vertex_id() );
    float cover_value = _min( high_cover_value, low_cover_value );
    if ( cover_value >= m_best_value )
        return;
...
void CCoverEvaluatorSafe::evaluate_smart_cover(
    smart_cover::cover const* smart_cover,
    float const& weight ) {}
...
void CCoverEvaluatorAmbush::setup( const Fvector& my_position,
    ...
    m_actuality =
        m_actuality && m_enemy_position.similar( enemy_position, 5.f );
    m_enemy_position = enemy_position;

    m_actuality =
        m_actuality && fsimilar( m_min_enemy_distance, min_enemy_distance );
...
void CCoverEvaluatorAmbush::evaluate_cover( const CCoverPoint* cover_point,
    ...
    direction.getHP( y, p );
    y = angle_normalize( y );
    float high_cover_from_enemy = ai().level_graph().high_cover_in_direction(
        y, cover_point->level_vertex_id() );
    float low_cover_from_enemy = ai().level_graph().low_cover_in_direction(
        y, cover_point->level_vertex_id() );
    float cover_from_enemy =
        _min( high_cover_from_enemy, low_cover_from_enemy );

    direction.sub( m_my_position, cover_point->position() );
    direction.getHP( y, p );
    y = angle_normalize( y );
    float high_cover_from_myself = ai().level_graph().high_cover_in_direction(
        y, cover_point->level_vertex_id() );
    float low_cover_from_myself = ai().level_graph().low_cover_in_direction(
        y, cover_point->level_vertex_id() );
    float cover_from_myself =
        _min( high_cover_from_myself, low_cover_from_myself );
...
void CCoverEvaluatorAmbush::evaluate_smart_cover(
    smart_cover::cover const* smart_cover,
    float const& weight ) {}
