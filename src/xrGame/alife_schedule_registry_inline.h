...
IC void CALifeScheduleRegistry::objects_per_update(
    const u32& objects_per_update ) {
...
IC void CALifeScheduleRegistry::update() {
    ...
    if ( psAI_Flags.test( aiALife ) ) {
        //		Msg
        //("[LSS][SU][%d : %d]",count, objects().size());
    }
#endif
}

IC CSE_ALifeSchedulable* CALifeScheduleRegistry::object(
    const ALife::_OBJECT_ID& id,
    bool no_assert ) const {
    _const_iterator I = objects().find( id );
    if ( I == objects().end() ) {
        THROW2( no_assert,
                "The spesified object hasn't been found in the schedule "
                "registry!" );
        return ( 0 );
    }
    return ( ( *I ).second );
