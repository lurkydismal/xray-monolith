...
template < typename T >
IC const CPatrolPath::CVertex* CPatrolPath::point( const Fvector& position,
    ...
    for ( ; I != E; ++I ) {
        if ( !evaluator( ( *I ).second->data().position() ) )
            continue;
        float distance =
            ( *I ).second->data().position().distance_to_sqr( position );
...
IC const CPatrolPath::CVertex* CPatrolPath::point(
