...
Fvector& global_hit_position( Fvector& gp, CEntityAlive& ea, const SHit& H ) {
    ...
    VERIFY( K );
    K->LL_GetTransform( H.bone() )
        .transform_tiny( gp, H.bone_space_position() );
    ea.XFORM().transform_tiny( gp );
    return gp;
}

type_motion::edirection type_motion::dir( CEntityAlive& ea,
                                          const SHit& H,
                                          float& angle ) {
...
bool is_bone_head( IKinematics& K, u16 bone ) {
    ...
    return ( bone != BI_NONE ) && neck_bone == bone ||
           find_in_parents( head_bone, bone, K );
}

void type_motion_diagnostic( LPCSTR message,
    ...
    Msg( "death anims: %s, dir: %s, motion: %s,  obj: %s, model: %s, bone: %s ",
         message, motion_dirs[ dr ].name, motion_name, ea.cName().c_str(),
         ea.cNameVisual().c_str(), bone_name );
...
class type_motion0 : public type_motion {
    bool predicate( CEntityAlive& ea,
                    const SHit& H,
                    MotionID& m,
                    float& angle ) const {
        m = MotionID();
        ...
        const Fvector stalker_velocity_dir =
            Fvector().mul( stalker_velocity, 1.f / stalker_speed );

        const Fvector dir_to_actor =
            Fvector()
                .sub( H.initiator()->Position(), ea.Position() )
                .normalize_safe();
        ...
        Fvector p;
        if ( Fvector()
                 .sub( H.initiator()->Position(),
                       global_hit_position( p, ea, H ) )
                 .magnitude() > 30.f )
            return false;

        m = motion( front );
        type_motion_diagnostic(
            " type_motion0: 1. = Инерционное движение вперед от попадания в "
            "голову ",
            front, ea, H, m );
        return true;
    }
...
class type_motion2 : public type_motion {
    bool predicate( CEntityAlive& ea,
        ...
        CWeaponShotgun* s =
            smart_cast< CWeaponShotgun* >( static_cast< CGameObject* >( O ) );
        if ( !s ) {
            CWeaponAutomaticShotgun* s = smart_cast< CWeaponAutomaticShotgun* >(
                static_cast< CGameObject* >( O ) );
            if ( !s )
                return false;
        }
        Fvector p;
        ...
        m = motion( dr );
        type_motion_diagnostic( " type_motion2: 3.	Шотган ", dr, ea, H,
                                m );
        return true;
    }
};
...
class type_motion3 : public type_motion {
    bool predicate( CEntityAlive& ea,
        ...
        if ( is_bone_head( *K, H.bone() ) ) {
            ...
            m = motion( dr );
            type_motion_diagnostic(
                " type_motion3: 4.	Хедшот (по вероятности), кроме 5 (4)",
                dr, ea, H, m );
            return true;
        }
        return false;
...
class type_motion4 : public type_motion {
    bool predicate( CEntityAlive& ea,
        ...
        if ( is_snipper( H.weaponID ) ) {
            edirection dr = dir( ea, H, angle );
            m = motion( dr );
            type_motion_diagnostic( " type_motion4: 5.	Снайперка в голову", dr,
                                    ea, H, m );
            return true;
        }
        return false;
...
class type_motion5 : public type_motion {
    bool predicate( CEntityAlive& ea,
        ...
        if ( is_snipper( H.weaponID ) && !is_bone_head( *K, H.bone() ) ) {
            edirection dr = dir( ea, H, angle );
            m = motion( dr );
            type_motion_diagnostic( "type_motion5: 6.	Снайперка в тело", dr,
                                    ea, H, m );
            return true;
        }
        return false;
...
class type_motion6 : public type_motion {
    bool predicate( CEntityAlive& ea,
        ...
        if ( smart_cast< CExplosive* >( O ) != 0 ) {
            edirection dr = dir( ea, H, angle );
            m = motion( dr );
            type_motion_diagnostic( "type_motion6: 7. Гранта - осколок", dr, ea,
                                    H, m );
            return true;
        }
...
void death_anims::setup( IKinematicsAnimated* k,
                         LPCSTR section,
                         CInifile const* ini ) {
    clear();
    //	if( !ini )
    ...
    anims[ 0 ] =
        xr_new< type_motion0 >()->setup( k, ini, section, "kill_enertion" );
    // 1.	Инерционное движение вперед от попадания в голову
    anims[ 1 ] = xr_new< type_motion1 >()->setup(
        k, ini, section, "kill_burst" ); // 2.	Изрешетить пулями
    anims[ 2 ] = xr_new< type_motion2 >()->setup(
        k, ini, section, "kill_shortgun" ); // 3.	Шотган

    anims[ 6 ] =
        xr_new< type_motion3 >()->setup( k, ini, section, "kill_headshot" );
    // 4.	Хедшот (по вероятности), кроме 5 (4)
    anims[ 4 ] = xr_new< type_motion4 >()->setup(
        k, ini, section,
        "kill_sniper_headshot" ); // 5.	Снайперка в голову.
    anims[ 5 ] = xr_new< type_motion5 >()->setup(
        k, ini, section, "kill_sniper_body" ); // 6.	Снайперка в тело.
    anims[ 3 ] = xr_new< type_motion6 >()->setup(
        k, ini, section, "kill_grenade" ); // 7.	Гранта
    if ( ini->line_exist( section, "random_death_animations" ) )
        rnd_anims.setup( k,
                         ini->r_string( section, "random_death_animations" ) );
}
