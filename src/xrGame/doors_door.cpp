...
static bool valid( door_state const state ) {
    return ( state == doors::door_state_open ) ||
           ( state == doors::door_state_closed );
}
...
void door::change_state( actor* initiator ) {
    ...
    m_object.callback( GameObject::eUseObject )(
        ...
        ( CScriptGameObject* )( initiator ? initiator->lua_game_object()
                                          : 0 ) );
#ifdef DEBUG
    if ( g_debug_doors )
        Msg( "door[%s] started to change its state to [%s]",
...
void door::change_state( actor* const initiator,
...
#ifdef DEBUG
        if ( g_debug_doors )
            Msg( "door[%s] added initiator[%s] to keep door %s",
                 m_object.cName().c_str(), initiator->get_name(),
                 m_target_state == door_state_open ? "open" : "closed" );
#endif // #ifdef DEBUG
    ...
    actors_type::iterator const found =
        std::find( m_initiators.begin(), m_initiators.end(), initiator );
    VERIFY2( found != m_initiators.end(),
             make_string( "cannot find initiator %s", initiator->get_name() ) );
    if ( found != m_initiators.end() ) {
#ifdef DEBUG
        if ( g_debug_doors )
            Msg( "door[%s] removed initiator[%s] to keep door %s",
                 m_object.cName().c_str(), initiator->get_name(),
                 m_target_state == door_state_open ? "open" : "closed" );
#endif // #ifdef DEBUG
       //		if ( !xr_strcmp( "sim_default_duty_28212",
...
void door::change_state( actor* const initiator,
    ...
    change_state(
        initiator, state,
        state == door_state_open ? door_state_closed : door_state_open );
}
...
bool door::check_initiator( actor const* const initiator ) const {
    return std::find( m_initiators.begin(), m_initiators.end(), initiator ) !=
           m_initiators.end();
}
#endif // #ifdef DEBUG
