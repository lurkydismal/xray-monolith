...
struct CVertexManagerHashFixed {
    ...
    struct VertexManager {
        ...
        struct _vertex : public T1< T2 > {
            ...
            IC const _index_type& index() const { return ( _index ); }

            IC _index_type& index() { return ( _index ); }

            IC bool& opened() { return ( _opened ); }

            IC bool opened() const { return ( _opened ); }
        };
    };

    template < template < typename _T > class _vertex = CEmptyClassTemplate,
               template < typename _T1, typename _T2 > class _index_vertex =
                   CEmptyClassTemplate2,
               typename _data_storage = CBuilderAllocatorConstructor >
    class CDataStorage : public _data_storage::template CDataStorage<
...
#pragma pack( push, 1 )
        ...
        struct SGraphIndexVertex
            : public _index_vertex< CGraphVertex,
                                    SGraphIndexVertex< _path_id_type > > {
...
#pragma pack( pop )
    ...
    public:
        ...
        IC CGraphVertex& create_vertex( CGraphVertex& vertex,
