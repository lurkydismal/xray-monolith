...
static void FromAxisAngle( Fmatrix& self,
                           const Fvector& rkAxis,
                           float fRadians ) {
...
static Fvector GetColumn( Fmatrix& self, const u32& index ) {
    switch ( index ) {
        case 0:
            return ( Fvector().set( self._11, self._21, self._31 ) );
        case 1:
            return ( Fvector().set( self._12, self._22, self._32 ) );
        case 2:
            return ( Fvector().set( self._13, self._23, self._33 ) );
        default:
            NODEFAULT;
    }
#ifdef DEBUG
...
static float Volume( const float* afAngle, void* pvUserData ) {
    ...
    float fVolume =
        ( kMax.x - kMin.x ) * ( kMax.y - kMin.y ) * ( kMax.z - kMin.z );
    return fVolume;
}

//----------------------------------------------------------------------------
static void MinimalBoxForAngles( int iQuantity,
                                 const Fvector* akPoint,
                                 float afAngle[ 3 ],
                                 MagicBox3& rkBox ) {
    float fCos0 = _cos( afAngle[ 0 ] );
    float fSin0 = _sin( afAngle[ 0 ] );
...
MagicBox3 MagicMinBox( int iQuantity, const Fvector* akPoint ) {
    ...
    MinimizeND< 3 > kMinimizer( Volume, iMaxLevel, iMaxBracket, iMaxIterations,
                                &kPA );
