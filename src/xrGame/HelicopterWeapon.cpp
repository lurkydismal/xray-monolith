...
void CHelicopter::OnShot() {
    ...
    if ( m_enemy.bUseFireTrail ) {
        ...
        static float fire_trace_width =
            pSettings->r_float( *cNameSect(), "fire_trace_width" );
    ...
    FireBullet( fire_pos, fire_dir, fireDispersionBase, m_CurrentAmmo, ID(),
                ID(), OnServer(), ::Random.randI( 0, 30 ) );
    ...
    if ( isOnAttack() ) {
        if ( m_allow_fire ) {
            ...
            if ( between( d, m_min_rocket_dist, m_max_rocket_dist ) &&
                 ( Device.dwTimeGlobal - m_last_rocket_attack >
...
void CHelicopter::UpdateMGunDir() {
    ...
    if ( ( angle_difference( m_cur_rot.x, m_tgt_rot.x ) >
           deg2rad( m_barrel_dir_tolerance ) ) ||
         ( angle_difference( m_cur_rot.y, m_tgt_rot.y ) >
           deg2rad( m_barrel_dir_tolerance ) ) )
        m_allow_fire = FALSE;
}
...
void CHelicopter::startRocket( u16 idx ) {
    if ( ( getRocketCount() >= 1 ) && m_use_rocket_on_attack ) {
        ...
        Fmatrix rocketXFORM;
        ( idx == 1 ) ? rocketXFORM = m_left_rocket_bone_xform
