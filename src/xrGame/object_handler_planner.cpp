...
IC ObjectHandlerSpace::EWorldProperties CObjectHandlerPlanner::object_property(
    ...
    switch ( object_action ) {
        ...
        case MonsterSpace::eObjectActionFireNoReload:
            return ( ObjectHandlerSpace::eWorldPropertyFiringNoReload1 );
...
void CObjectHandlerPlanner::set_goal( MonsterSpace::EObjectAction object_action,
    ...
    if ( game_object && ( eWorldPropertyNoItemsIdle != goal ) ) {
        ...
        if ( weapon && ( goal == eWorldPropertyIdleStrap ) &&
             !weapon->can_be_strapped() )
    ...
    if ( m_use_log ) {
        Msg( "%6d : Active item %s", Device.dwTimeGlobal,
             object().inventory().ActiveItem()
                 ...
                 : "no active items" );
        Msg( "%6d : Goal %s", Device.dwTimeGlobal,
             property2string( condition_id ) );
    }
#endif
    CState condition;
    ...
    if ( ( m_min_queue_size != min_queue_size ) ||
        ...
        if ( m_max_queue_size == m_min_queue_size )
            ...
            m_queue_size =
                _max( 1, ::Random.randI( m_min_queue_size, m_max_queue_size ) );

        if ( m_max_queue_interval == m_min_queue_interval )
            m_queue_interval = m_min_queue_interval;
        else
            m_queue_interval =
                ::Random.randI( m_min_queue_interval, m_max_queue_interval );
...
LPCSTR CObjectHandlerPlanner::action2string( const _action_id_type& id ) {
    ...
    if ( action_object_id( id ) != 0xffff )
        if ( Level().Objects.net_Find( action_object_id( id ) ) )
            xr_strcpy(
                m_temp_string,
                *Level().Objects.net_Find( action_object_id( id ) )->cName() );
        else
...
#pragma todo( "Dima to Dima : safe, but not optimal!" )
    for ( ;; ) {
        EVALUATORS::iterator I =
            m_evaluators.lower_bound( uid( object->ID(), 0 ) );
...
#pragma todo( "Dima to Dima : safe, but not optimal!" )
    for ( ;; ) {
        OPERATOR_VECTOR::iterator I = std::lower_bound(
            m_operators.begin(), m_operators.end(), uid( object->ID(), 0 ) );
...
void CObjectHandlerPlanner::setup( CAI_Stalker* object ) {
    ...
    add_evaluator( u32( eWorldPropertyNoItems ),
                   xr_new< CObjectPropertyEvaluatorNoItems >( m_object ) );
    add_evaluator( u32( eWorldPropertyNoItemsIdle ),
                   xr_new< CObjectPropertyEvaluatorConst >( false ) );
    action = xr_new< CSObjectActionBase >( m_object, m_object, &m_storage,
                                           "no items idle" );
...
void CObjectHandlerPlanner::remove_item( CInventoryItem* inventory_item ) {
    ...
    if ( action_object_id( target_state().conditions().back().condition() ) ==
         inventory_item->object().ID() ) {
        init_storage();
        set_goal( MonsterSpace::eObjectActionIdle, 0, 0, 0, 0, 0 );
    }
...
#ifdef LOG_ACTION
    if ( ( psAI_Flags.test( aiGOAPObject ) && !m_use_log ) ||
         ( !psAI_Flags.test( aiGOAPObject ) && m_use_log ) )
        set_use_log( !!psAI_Flags.test( aiGOAPObject ) );
#endif
    inherited::update();
}

CObjectHandlerPlanner::_condition_type CObjectHandlerPlanner::uid(
    const u32 id0,
    const u32 id1 ) const {
    VERIFY( !( ( id0 << 16 ) & id1 ) );
    return ( ( id0 << 16 ) | id1 );
