...
BOOL CBulletManager::test_callback( const collide::ray_defs& rd,
                                    CObject* object,
                                    LPVOID params ) {
    if ( !object )
        return TRUE;
    ...
    if ( entity && entity->g_Alive() &&
        ...
        if ( ( NULL != cform ) && ( cftObject == cform->Type() ) ) {
            ...
            if ( actor && IsGameTypeSingle() /**/ || stalker /**/ ) {
                ...
                if ( Fsphere::rpNone != S.intersect_full( bullet->bullet_pos,
                                                          bullet->dir,
                                                          dist ) ) {
                    // да попали, найдем кто стрелял
                    bool play_whine = true;
                    CObject* initiator =
                        Level().Objects.net_Find( bullet->parent_id );
                    if ( actor ) {
                        // попали в актера
                        float hpf = 1.f;
...
#else
                        float game_difficulty_hit_probability =
                            actor->HitProbability();
                        CAI_Stalker* stalker =
                            smart_cast< CAI_Stalker* >( initiator );
                        if ( stalker )
                            hpf = stalker->SpecificCharacter()
                                      .hit_probability_factor();

                        float dist_factor = 1.f;
                        CObject* weapon_object =
                            Level().Objects.net_Find( bullet->weapon_id );
                        if ( weapon_object ) {
                            CWeapon* weapon =
                                smart_cast< CWeapon* >( weapon_object );
                            if ( weapon ) {
                                game_difficulty_hit_probability =
                                    weapon->hit_probability();
                                float fly_dist = bullet->fly_dist + dist;
                                dist_factor = _min(
                                    1.f,
                                    fly_dist /
                                        Level().BulletManager().m_fHPMaxDist );
                            }
                        }

                        ahp = dist_factor * game_difficulty_hit_probability +
                              ( 1.f - dist_factor ) * 1.f;
#endif
#else
                        CAI_Stalker* i_stalker =
                            smart_cast< CAI_Stalker* >( initiator );
                        // если стрелял сталкер, учитываем -
                        // hit_probability_factor сталкерa иначе - 1.0
                        if ( i_stalker ) {
                            hpf = i_stalker->SpecificCharacter()
                                      .hit_probability_factor();
                            float fly_dist = bullet->fly_dist + dist;
                            float dist_factor = _min(
                                1.f, fly_dist /
                                         Level().BulletManager().m_fHPMaxDist );
                            ahp = dist_factor * actor->HitProbability() +
                                  ( 1.f - dist_factor ) * 1.f;
                        }
#endif
                        if ( Random.randF( 0.f, 1.f ) > ( ahp * hpf ) ) {
                            ...
                            if ( cform->_RayQuery(
                                     rd,
                                     Level().BulletManager().m_rq_results ) ) {
                                bRes = TRUE;        // hit actor
                    ...
                    if ( play_whine ) {
                        ...
                        Level().BulletManager().PlayWhineSound( bullet,
                                                                initiator, pt );
                    }
                } else {
...
void CBulletManager::FireShotmark( SBullet* bullet,
                                   const Fvector& vDir,
                                   const Fvector& vEnd,
                                   collide::rq_result& R,
                                   u16 target_material,
    ...
    SGameMtlPair* mtl_pair =
        GMLib.GetMaterialPair( bullet->bullet_material_idx, target_material );
    ...
    if ( R.O ) {
        ...
        if ( mtl_pair && !mtl_pair->m_pCollideMarks->empty() && ShowMark ) {
            ...
            if ( !g_dedicated_server )
                ::Render->add_SkeletonWallmark(
                    &R.O->renderable.xform, PKinematics( R.O->Visual() ),
                    &*mtl_pair->m_pCollideMarks, p, bullet->dir,
        ...
        if ( mtl_pair && !mtl_pair->m_pCollideMarks->empty() && ShowMark ) {
            // добавить отметку на материале
            ::Render->add_StaticWallmark( &*mtl_pair->m_pCollideMarks, vEnd,
    ...
    ref_sound* pSound = ( !mtl_pair || mtl_pair->CollideSounds.empty() )
                            ? NULL
                            : &mtl_pair->CollideSounds[::Random.randI(
                                  0, mtl_pair->CollideSounds.size() ) ];
    ...
    if ( ( ps_name && ShowMark ) || ( bullet->flags.explosive && bStatic ) ) {
        VERIFY2( ( particle_dir.x * particle_dir.x +
                   particle_dir.y * particle_dir.y +
                   particle_dir.z * particle_dir.z ) > flt_zero,
...
void CBulletManager::DynamicObjectHit( CBulletManager::_event& E ) {
    ...
    if ( V ) {
        VERIFY3( V->LL_GetBoneVisible( u16( E.R.element ) ),
                 *E.R.O->cNameVisual(),
                 V->LL_BoneName_dbg( u16( E.R.element ) ) );
        Fmatrix& m_bone =
            ( V->LL_GetBoneInstance( u16( E.R.element ) ) ).mTransform;
        Fmatrix m_inv_bone;
    ...
    if ( E.bullet.flags.allow_sendhit && !E.Repeated ) {
        ...
        if ( GameID() != eGameIDSingle && E.bullet.flags.allow_sendhit &&
             smart_cast< CActor* >( E.R.O ) &&
             Game().m_WeaponUsageStatistic->CollectData() ) {
            CActor* pActor = smart_cast< CActor* >( E.R.O );
            if ( pActor ) // && pActor->g_Alive())
            {
                Game().m_WeaponUsageStatistic->OnBullet_Hit(
                    &E.bullet, E.R.O->ID(), ( s16 )E.R.element, E.point );
                AddStatistic = true;
            };
        };

        SHit Hit =
            SHit( ( E.bullet.parent_id == 0
                        ? hit_param.power * hit_modifier
                        : hit_param.power ), // Make sure only damage dealt by
                                             // actor is modified
                  original_dir, NULL, u16( E.R.element ),
                  position_in_bone_space, hit_param.impulse, E.bullet.hit_type,
        ...
        Hit.GenHeader(
            u16( ( AddStatistic ) ? GE_HIT_STATISTIC : GE_HIT ) & 0xffff,
            E.R.O->ID() );
...
extern void random_dir( Fvector& tgt_dir,
                        const Fvector& src_dir,
                        float dispersion );

bool CBulletManager::ObjectHit( SBullet_Hit* hit_res,
                                SBullet* bullet,
    ...
    if ( ai().script_engine().functor( "_g.CBulletManager__ObjectHit", funct ) )
        funct( bullet->section,
               smart_cast< CGameObject* >( R.O )
                   ? smart_cast< CGameObject* >( R.O )->lua_game_object()
                   : nullptr,
               end_point, bullet->dir,
               *GMLib.GetMaterialByIdx( target_material )->m_Name,
               bullet->speed, bullet->weapon_id );
    ...
    if ( R.O ) {
        ...
        Fvector* pVerts = Level().ObjectSpace.GetStaticVerts();
        CDB::TRI* pTri = Level().ObjectSpace.GetStaticTris() + R.element;
        hit_normal.mknormal( pVerts[ pTri->verts[ 0 ] ],
                             pVerts[ pTri->verts[ 1 ] ],
                             pVerts[ pTri->verts[ 2 ] ] );
        ...
        if ( DOT( hit_normal, bullet->dir ) < 0 ) {
            ...
            bullet->begin_density.mad( bullet->bullet_pos, bullet->dir,
                                       R.range );
        } else {
    ...
    *hit_res = bullet->hit_param; // default param
    ...
    if ( fsimilar( mtl_ap,
                   0.0f ) ) // Если материал полностью простреливаемый (кусты)
    {
#ifdef DEBUG
        bullet_state = 2;
#endif
    ...
    if ( ( f < ricoshet_factor ) &&
        ...
        // уменьшение скорости полета в зависимости от угла падения пули (чем
        // прямее угол, тем больше потеря)
        bullet->flags.allow_ricochet = 0;
        float scale = 1.0f - _abs( bullet->dir.dotproduct( hit_normal ) ) *
                                 m_fCollisionEnergyMin;
        clamp( scale, 0.0f, m_fCollisionEnergyMax );
        speed_scale = scale;

        // вычисление рикошета, делается немного фейком, т.к. пуля остается в
        // точке столкновения и сразу выходит из RayQuery()
