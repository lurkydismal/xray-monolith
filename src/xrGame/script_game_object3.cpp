...
const CCoverPoint* CScriptGameObject::best_cover( const Fvector& position,
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ...
            make_string(
                "CGameObject [%s]: cannot access class member best_cover!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
    stalker->m_ce_best->setup( enemy_position, min_enemy_distance,
                               max_enemy_distance, 0.f );
    const CCoverPoint* point = ai().cover_manager().best_cover(
        position, radius, *stalker->m_ce_best );
    return ( point );
}

const CCoverPoint* CScriptGameObject::safe_cover( const Fvector& position,
                                                  float radius,
                                                  float min_distance ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker ) {
    ...
    const CCoverPoint* point = ai().cover_manager().best_cover(
        position, radius, *stalker->m_ce_safe );
    return ( point );
}

const xr_vector< MemorySpace::CVisibleObject >&
CScriptGameObject::memory_visible_objects() const {
    CCustomMonster* monster = smart_cast< CCustomMonster* >( &object() );
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CGameObject [%s]: cannot access class member "
...
const xr_vector< MemorySpace::CSoundObject >&
CScriptGameObject::memory_sound_objects() const {
    CCustomMonster* monster = smart_cast< CCustomMonster* >( &object() );
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
...
const xr_vector< MemorySpace::CHitObject >&
CScriptGameObject::memory_hit_objects() const {
    CCustomMonster* monster = smart_cast< CCustomMonster* >( &object() );
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::ChangeTeam( u8 team, u8 squad, u8 group ) {
    ...
    if ( !custom_monster )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CCustomMonster[%s]: cannot access class member ChangeTeam!",
                object().cNameSect().c_str() )
                .c_str() );
    else
...
void CScriptGameObject::set_view_distance_factor( float value ) {
    ...
    if ( !custom_monster )
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CCustomMonster : cannot access class "
                                         "member set_view_distance_factor!" );
    else
...
void CScriptGameObject::set_health_restore_boost( float value ) {
    ...
    if ( !entity_alive )
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CEntityAlive : cannot access class "
                                         "member set_health_restore_boost!" );
    else
        entity_alive->conditions().set_health_restore_boost( value );
}

float CScriptGameObject::GetObjectVisibleDistance(
    const CScriptGameObject* obj ) {
    if ( obj == nullptr ) {
        ai().script_engine().script_log( ScriptStorage::eLuaMessageTypeError,
                                         "CGameObject: [%s] wrong parameters.",
    ...
    if ( custom_monster == nullptr ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CGameObject: [%s] cannot access class member CCustomMonster.",
            object().cNameSect_str() );
    }
    float distance = 0.0F;
    return custom_monster->visual_memory()->object_visible_distance(
        &obj->object(), distance );
}
...
CScriptGameObject* CScriptGameObject::GetEnemy() const {
    CCustomMonster* l_tpCustomMonster =
        smart_cast< CCustomMonster* >( &object() );
    if ( l_tpCustomMonster && l_tpCustomMonster->g_Alive() ) {
        if ( l_tpCustomMonster->GetCurrentEnemy() &&
             !l_tpCustomMonster->GetCurrentEnemy()->getDestroy() )
        ...
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CScriptGameObject [%s]: cannot access class member GetEnemy!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
CScriptGameObject* CScriptGameObject::GetCorpse() const {
    ...
    if ( l_tpCustomMonster )
        if ( l_tpCustomMonster->GetCurrentCorpse() &&
             !l_tpCustomMonster->GetCurrentCorpse()->getDestroy() )
        ...
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CScriptGameObject [%s]: cannot access class member GetCorpse!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
bool CScriptGameObject::CheckTypeVisibility( const char* section_name ) {
    ...
    if ( l_tpCustomMonster )
        ...
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CScriptGameObject [%s]: cannot access class member "
                         "CheckTypeVisibility!",
                         object().cNameSect().c_str() )
                .c_str() );
        return ( false );
...
void CScriptGameObject::deadbody_closed( bool status ) {
    CInventoryOwner* inventoryOwner =
...
CScriptGameObject* CScriptGameObject::GetFood() const {
    ...
    if ( !l_tpStalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member GetFood!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
    CGameObject* food =
        l_tpStalker->GetFood() ? &l_tpStalker->GetFood()->object() : 0;
...
CScriptGameObject* CScriptGameObject::GetMedikit() const {
    ...
    CGameObject* medikit =
        l_tpStalker->GetMedikit() ? &l_tpStalker->GetMedikit()->object() : 0;
...
LPCSTR CScriptGameObject::GetPatrolPathName() {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker ) {
        CScriptEntity* script_monster =
            smart_cast< CScriptEntity* >( &object() );
        if ( !script_monster ) {
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                make_string( "CGameObject [%s]: cannot access class member "
...
void CScriptGameObject::add_animation( LPCSTR animation,
                                       bool hand_usage,
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CGameObject [%s]: cannot access class member add_animation!",
                object().cNameSect().c_str() )
                .c_str() );
        return;
    }
    ...
    if ( stalker->movement().current_params().cover() ) {
        ai().script_engine().script_log(
            ...
            "Cannot add animation [%s]: object [%s] is in smart_cover!",
            animation, stalker->cName().c_str() );
    }

    if ( stalker->animation().global_selector() ) {
        ai().script_engine().script_log(
            eLuaMessageTypeError,
            "Cannot add animation [%s]: global selector is set for object "
            "[%s], in_smart_cover returned [%s]!",
            animation, stalker->cName().c_str(),
...
void CScriptGameObject::add_animation( LPCSTR animation,
    ...
    stalker->animation().add_script_animation( animation, hand_usage, position,
                                               rotation, local_animation );
}
...
void CScriptGameObject::set_patrol_path(
    LPCSTR path_name,
    const PatrolPathManager::EPatrolStartType patrol_start_type,
    const PatrolPathManager::EPatrolRouteType patrol_route_type,
    bool random ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member movement!",
                object().cNameSect().c_str() )
                .c_str() );
    else
        stalker->movement().patrol().set_path( path_name, patrol_start_type,
                                               patrol_route_type, random );
}
    ...
    if ( !stalker )
        ...
        if ( !ai().level_graph().valid_vertex_id( level_vertex_id ) ) {
            ...
            ai().script_engine().script_log(
                ...
                stalker->brain()
                    .CStalkerPlanner::current_action()
                    .m_action_name );
#endif
            return;
        }
        if ( !stalker->movement().restrictions().accessible(
                 level_vertex_id ) ) {
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                "! you are trying to setup destination for the stalker %s, "
                "which is not accessible by its restrictors in[%s] out[%s]",
                stalker->cName().c_str(),
                Level()
                    .space_restriction_manager()
                    .in_restrictions( stalker->ID() )
                    .c_str(),
                Level()
                    .space_restriction_manager()
                    .out_restrictions( stalker->ID() )
                    .c_str() );
            return;
        }
...
void CScriptGameObject::set_dest_game_vertex_id(
...
void CScriptGameObject::set_movement_selection_type(
...
CHARACTER_RANK_VALUE CScriptGameObject::GetRank() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member GetRank!",
                object().cNameSect().c_str() )
                .c_str() );
...
void CScriptGameObject::best_cover_invalidate() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CGameObject: [%s] best_cover_invalidate() called on non-stalker "
            "object",
            object().cNameSect_str() );
        return;
    }
...
LPCSTR CScriptGameObject::GetCurrentSmartCoverName() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CGameObject: [%s] get_current_smart_cover_name() called on "
            "non-stalker object",
            object().cNameSect_str() );
        return ( "" );
    }
...
LPCSTR CScriptGameObject::GetCurrentLoopholeId() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CGameObject: [%s] get_current_loophole_id() called on non-stalker "
            "object",
            object().cNameSect_str() );
        return ( "" );
    }
...
void CScriptGameObject::set_desired_position(
    const Fvector* desired_position ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
        ...
        THROW2(
            desired_position || stalker->movement().restrictions().accessible(
                                    *desired_position ),
            *stalker->cName() );
...
void CScriptGameObject::set_desired_direction(
    const Fvector* desired_direction ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
        ...
        if ( fsimilar( desired_direction->magnitude(), 0.f ) )
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                "CAI_Stalker : [%s] set_desired_direction - you passed zero "
                "direction!",
                stalker->cName().c_str() );
        else {
            if ( !fsimilar( desired_direction->magnitude(), 1.f ) )
                ai().script_engine().script_log(
                    ScriptStorage::eLuaMessageTypeError,
                    "CAI_Stalker : [%s] set_desired_direction - you passed "
...
void CScriptGameObject::set_body_state( EBodyState body_state ) {
    THROW( ( body_state == eBodyStateStand ) ||
           ( body_state == eBodyStateCrouch ) );
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::set_detail_path_type(
...
MonsterSpace::EBodyState CScriptGameObject::body_state() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member body_state!",
                object().cNameSect().c_str() )
                .c_str() );
...
MonsterSpace::EMovementType CScriptGameObject::movement_type() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member movement_type!",
                object().cNameSect().c_str() )
                .c_str() );
...
MonsterSpace::EMentalState CScriptGameObject::mental_state() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member mental_state!",
                object().cNameSect().c_str() )
                .c_str() );
...
MovementManager::EPathType CScriptGameObject::path_type() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member path_type!",
                object().cNameSect().c_str() )
                .c_str() );
...
void CScriptGameObject::set_sight( SightManager::ESightType sight_type,
                                   Fvector* vector3d,
                                   u32 dwLookOverDelay ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker )
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CSightManager [%s]: cannot access class member set_sight!",
                object().cNameSect().c_str() )
                .c_str() );
    else {
        if ( ( sight_type == SightManager::eSightTypeDirection ) && vector3d &&
             ( _abs( vector3d->magnitude() - 1.f ) > .01f ) ) {
            VERIFY2( false, make_string(
                                "non-normalized direction passed [%f][%f][%f]",
                                VPUSH( *vector3d ) ) );
            vector3d->normalize();
        }
...
void CScriptGameObject::set_sight( CScriptGameObject* object_to_look,
    ...
    if ( !stalker )
        ...
        stalker->sight().setup( CSightAction(
            &object_to_look->object(), torso_look, fire_object, no_pitch ) );
}

void CScriptGameObject::set_sight( const CMemoryInfo* memory_object,
                                   bool torso_look ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker )
        ai().script_engine().script_log(
...
u32 CScriptGameObject::GetInventoryObjectCount() const {
    CInventoryOwner* l_tpInventoryOwner =
        smart_cast< CInventoryOwner* >( &object() );
    if ( l_tpInventoryOwner )
        ...
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CScriptGameObject [%s]: cannot access class member obj_count!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
CScriptGameObject* CScriptGameObject::GetActiveItem() {
    ...
    if ( l_tpInventoryOwner )
        if ( l_tpInventoryOwner->inventory().ActiveItem() )
            return ( l_tpInventoryOwner->inventory()
                         .ActiveItem()
                         ->object()
                         .lua_game_object() );
        else
...
CScriptGameObject* CScriptGameObject::GetObjectByName( LPCSTR caObjectName ) {
    ...
    if ( l_tpInventoryOwner ) {
        CInventoryItem* l_tpInventoryItem =
            l_tpInventoryOwner->inventory().GetItemFromInventory(
                caObjectName );
        CGameObject* l_tpGameObject =
            smart_cast< CGameObject* >( l_tpInventoryItem );
        if ( !l_tpGameObject )
            return ( 0 );
        else
        ...
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CScriptGameObject [%s]: cannot access class member object!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
CScriptGameObject* CScriptGameObject::GetObjectByIndex( int iIndex ) {
    ...
    if ( l_tpInventoryOwner ) {
        CInventoryItem* l_tpInventoryItem =
            l_tpInventoryOwner->inventory().tpfGetObjectByIndex( iIndex );
        CGameObject* l_tpGameObject =
            smart_cast< CGameObject* >( l_tpInventoryItem );
        if ( !l_tpGameObject )
            return ( 0 );
...
CScriptGameObject* CScriptGameObject::GetObjectById( u16 id ) const {
    ...
    if ( l_tpInventoryOwner ) {
        CInventoryItem* l_tpInventoryItem =
            l_tpInventoryOwner->inventory().GetItemFromInventory( id );
        CGameObject* l_tpGameObject =
            smart_cast< CGameObject* >( l_tpInventoryItem );
        if ( !l_tpGameObject )
            return ( 0 );
        ...
        for ( ; I != E; ++I ) {
            if ( *I == id ) {
                CGameObject* GO = smart_cast< CGameObject* >(
                    Level().Objects.net_Find( *I ) );
                if ( GO )
                    return ( GO->lua_game_object() );
                return ( 0 );
            }
        ...
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CScriptGameObject [%s]: cannot access class member object_id!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
void CScriptGameObject::set_color_animator( LPCSTR name,
                                            bool bFlicker,
                                            int flickerChance,
                                            float flickerDelay,
                                            int framerate ) {
    ...
    if ( torch ) {
        torch->SetLanim( name, bFlicker, flickerChance, flickerDelay,
                         framerate );
        return;
    }
    ...
    if ( flashlight )
        flashlight->SetLanim( name, bFlicker, flickerChance, flickerDelay,
                              framerate );
}
...
bool CScriptGameObject::weapon_strapped() const {
    ...
    //	Msg					( "[%6d][%s] weapon_strapped =
    //%s", Device.dwTimeGlobal, stalker->cName().c_str(), result ? "true" :
    //"false" );
    return ( result );
}
...
void CScriptGameObject::info_add( LPCSTR text ) {
    ...
    DBG()
        ...
        .add_item( text, D3DCOLOR_XRGB( 255, 0, 0 ), 0 );
#endif
}
...
void CScriptGameObject::make_object_visible_somewhen(
    ...
    CEntityAlive* entity_alive =
        smart_cast< CEntityAlive* >( &object->object() );
    if ( !entity_alive ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::sell_condition( CScriptIniFile* ini_file,
                                        LPCSTR section ) {
    ...
    inventory_owner->trade_parameters().process(
        CTradeParameters::action_sell( 0 ), *ini_file, section );
}

void CScriptGameObject::sell_condition( float friend_factor,
                                        float enemy_factor ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &object() );
    if ( !inventory_owner ) {
...
void CScriptGameObject::buy_condition( CScriptIniFile* ini_file,
                                       LPCSTR section ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &object() );
    if ( !inventory_owner ) {
    ...
    inventory_owner->trade_parameters().process(
        CTradeParameters::action_buy( 0 ), *ini_file, section );
}

void CScriptGameObject::buy_condition( float friend_factor,
                                       float enemy_factor ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &object() );
    if ( !inventory_owner ) {
...
void CScriptGameObject::show_condition( CScriptIniFile* ini_file,
                                        LPCSTR section ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &object() );
    if ( !inventory_owner ) {
    ...
    inventory_owner->trade_parameters().process(
        CTradeParameters::action_show( 0 ), *ini_file, section );
}

void CScriptGameObject::buy_supplies( CScriptIniFile* ini_file,
                                      LPCSTR section ) {
    CInventoryOwner* inventory_owner =
        smart_cast< CInventoryOwner* >( &object() );
    if ( !inventory_owner ) {
...
void sell_condition( CScriptIniFile* ini_file, LPCSTR section ) {
    default_trade_parameters().process( CTradeParameters::action_sell( 0 ),
                                        *ini_file, section );
}
...
void buy_condition( CScriptIniFile* ini_file, LPCSTR section ) {
    default_trade_parameters().process( CTradeParameters::action_buy( 0 ),
                                        *ini_file, section );
}
...
void show_condition( CScriptIniFile* ini_file, LPCSTR section ) {
    default_trade_parameters().process( CTradeParameters::action_show( 0 ),
                                        *ini_file, section );
}
...
LPCSTR CScriptGameObject::sound_prefix() const {
    ...
    if ( !custom_monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CCustomMonster [%s]: cannot access class member sound_prefix!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( 0 );
    }
...
bool CScriptGameObject::is_weapon_going_to_be_strapped(
    CScriptGameObject const* object ) const {
    if ( !object ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CGameObject [%s]: cannot access class member "
                "is_weapon_going_to_be_strapped (object passed is null)!",
                this->object().cNameSect().c_str() )
                .c_str() );
        return false;
    }

    CAI_Stalker const* stalker =
        smart_cast< CAI_Stalker const* >( &this->object() );
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CGameObject [%s]: cannot access class member "
...
void CScriptGameObject::SetArtefactImmunity( ALife::EHitType hit_type,
                                             float value ) {
    CArtefact* artefact = smart_cast< CArtefact* >( &object() );
    THROW( artefact );
...
void CScriptGameObject::AttachVehicle( CScriptGameObject* veh, bool bForce ) {
    ...
    if ( actor ) {
        ...
        if ( vehicle )
            ...
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                "CGameObject : cannot be cast to CHolderCustom!" );
    }
...
u32 CScriptGameObject::PlayHudMotion( LPCSTR M,
                                      bool bMixIn,
                                      u32 state,
                                      float speed,
                                      float end ) {
...
bool CScriptGameObject::WeaponInGrenadeMode() {
    CWeaponMagazinedWGrenade* wpn =
        smart_cast< CWeaponMagazinedWGrenade* >( &object() );
    if ( !wpn )
        return false;
...
void CScriptGameObject::SetBoneVisible( LPCSTR bone_name,
                                        bool bVisibility,
                                        bool bRecursive,
                                        bool bHud ) {
    ...
    if ( restr ) {
        ...
        if ( typ != RestrictionSpace::eRestrictorTypeNone )
            Level().space_restriction_manager().register_restrictor(
                restr, RestrictionSpace::ERestrictorTypes( typ ) );
    }
}
...
void CScriptGameObject::ForceSetRestrictionType( u8 typ ) {
    ...
    RestrictionSpace::ERestrictorTypes new_type =
        RestrictionSpace::ERestrictorTypes( typ );
    switch ( new_type ) {
        ...
        default:
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                make_string( "CGameObject [%s]: invalid restrictor type [%u]!",
                             object().cNameSect().c_str(), typ )
                    .c_str() );
            return;
    }

    RestrictionSpace::ERestrictorTypes old_type =
        RestrictionSpace::ERestrictorTypes( restr->m_space_restrictor_type );
    ...
    if ( new_type != RestrictionSpace::eRestrictorTypeNone )
        Level().space_restriction_manager().register_restrictor( restr,
                                                                 new_type );
}
...
void CScriptGameObject::InvalidateRestrictions() {
    ...
    if ( !monster ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CGameObject [%s]: cannot invalidate restrictions "
                         "(not a CCustomMonster)!",
                         object().cNameSect().c_str() )
                .c_str() );
        return;
    }
...
// demonized: add getters and setters for pathfinding for npcs around anomalies
// and damage for npcs
bool CScriptGameObject::get_enable_anomalies_pathfinding() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CGameObject [%s]: cannot access class member "
                         "m_enable_anomalies_pathfinding!",
                         object().cNameSect().c_str() )
                .c_str() );
        return false;
    }
