...
CStalkerCombatPlanner::CStalkerCombatPlanner( CAI_Stalker* object,
                                              LPCSTR action_name )
...
void CStalkerCombatPlanner::on_best_cover_changed(
    const CCoverPoint* new_cover,
    const CCoverPoint* old_cover ) {
    CScriptActionPlanner::m_storage.set_property( eWorldPropertyInCover,
                                                  false );
    CScriptActionPlanner::m_storage.set_property( eWorldPropertyLookedOut,
                                                  false );
    CScriptActionPlanner::m_storage.set_property( eWorldPropertyPositionHolded,
                                                  false );
    CScriptActionPlanner::m_storage.set_property( eWorldPropertyEnemyDetoured,
                                                  false );
}

void CStalkerCombatPlanner::setup( CAI_Stalker* object,
    ...
    CScriptActionPlanner::m_storage.set_property(
        eWorldPropertyUseCrouchToLookOut, true );
    CScriptActionPlanner::m_storage.set_property( eWorldPropertyKilledWounded,
                                                  false );
    CScriptActionPlanner::m_storage.set_property(
        eWorldPropertyStartedToThrowGrenade, false );

    this->object().brain().CStalkerPlanner::m_storage.set_property(
        eWorldPropertyCriticallyWounded, false );
...
void CStalkerCombatPlanner::update() {
    ...
    const u32 previous_action_id =
        was_initialized ? current_action_id() : u32( -1 );

    inherited::update();

    if ( was_initialized && initialized() &&
         current_action_id() != previous_action_id ) {
        ::luabind::functor< void > funct;
        if ( ai().script_engine().functor(
                 "_G.CAI_Stalker__OnCombatActionChanged", funct ) )
            funct( object().lua_game_object(), previous_action_id,
                   current_action_id() );
    }
...
bool CStalkerCombatPlanner::allow_action_switch(
    const _action_id_type& from_action_id,
    const _action_id_type& to_action_id ) {
    // Veto gate, fired BEFORE the planner swaps actions
    // (npc_on_combat_action_changed fires after an allowed swap). A Lua
    // subscriber may return false to keep the current action (e.g. suppress the
    // take_cover <-> look_out oscillation); no subscriber = vanilla. Re-asked
    // each brain update while the proposed switch stays denied. A denied swap
    // produces no transition, so the changed-callback stays silent for it.
    luabind::functor< bool > funct;
    if ( ai().script_engine().functor(
             "_G.CAI_Stalker__AllowCombatActionSwitch", funct ) )
        return ( funct( object().lua_game_object(), ( u32 )from_action_id,
                        ( u32 )to_action_id ) );
...
void CStalkerCombatPlanner::initialize() {
    ...
    if ( !m_loaded ) {
        ...
        CScriptActionPlanner::m_storage.set_property( eWorldPropertyLookedOut,
                                                      false );
        CScriptActionPlanner::m_storage.set_property(
            eWorldPropertyPositionHolded, false );
        CScriptActionPlanner::m_storage.set_property(
            eWorldPropertyEnemyDetoured, false );
        CScriptActionPlanner::m_storage.set_property(
            eWorldPropertyUseSuddenness, true );
        CScriptActionPlanner::m_storage.set_property(
            eWorldPropertyKilledWounded, false );

        object().brain().CStalkerPlanner::m_storage.set_property(
            eWorldPropertyCriticallyWounded, false );
    }
    ...
    // we must clear path, since it can be built using eMentalStateFree
    // velocities and our new path may not be ready
    ...
    if ( !m_loaded && object().memory().enemy().selected() ) {
        CVisualMemoryManager* visual_memory_manager =
            object().memory().enemy().selected()->visual_memory();
...

    {
        if ( !object().agent_manager().member().combat_members().empty() )
            CScriptActionPlanner::m_storage.set_property(
                eWorldPropertyUseSuddenness, false );
    }
    ...
    if ( m_object->memory().visual().visible_now(
             m_object->memory().enemy().selected() ) ) {
        if ( m_object->memory().enemy().selected()->human_being() )
            if ( object().agent_manager().member().can_cry_noninfo_phrase() )
                if ( object().agent_manager().member().members().size() > 1 )
                    if ( !CScriptActionPlanner::m_storage.property(
                             eWorldPropertyUseSuddenness ) )
...
void CStalkerCombatPlanner::finalize() {
    ...
    //	object().sound().remove_active_sounds
    //(eStalkerSoundMaskNoDanger);

    if ( object().inventory().ItemFromSlot( INV_SLOT_2 ) ) {
        CWeaponMagazined* temp = smart_cast< CWeaponMagazined* >(
            object().inventory().ItemFromSlot( INV_SLOT_2 ) );
        if ( object().inventory().ActiveItem() && temp &&
             ( object().inventory().ActiveItem()->object().ID() ==
               temp->ID() ) )
            object().set_goal(
                eObjectActionIdle,
                object().inventory().ItemFromSlot( INV_SLOT_2 ) );
...
void CStalkerCombatPlanner::add_evaluators() {
    add_evaluator( eWorldPropertyPureEnemy,
                   xr_new< CStalkerPropertyEvaluatorEnemies >(
                       m_object, "is_there_enemies", 0 ) );
    add_evaluator( eWorldPropertyEnemy,
                   xr_new< CStalkerPropertyEvaluatorEnemies >(
                       m_object, "is_there_enemies_delayed",
                       POST_COMBAT_WAIT_INTERVAL, &m_last_wounded ) );
    add_evaluator(
        eWorldPropertySeeEnemy,
        xr_new< CStalkerPropertyEvaluatorSeeEnemy >( m_object, "see enemy" ) );
    add_evaluator( eWorldPropertyEnemySeeMe,
                   xr_new< CStalkerPropertyEvaluatorEnemySeeMe >(
                       m_object, "enemy see me" ) );
    add_evaluator( eWorldPropertyItemToKill,
                   xr_new< CStalkerPropertyEvaluatorItemToKill >(
                       m_object, "item to kill" ) );
    add_evaluator( eWorldPropertyItemCanKill,
                   xr_new< CStalkerPropertyEvaluatorItemCanKill >(
                       m_object, "item can kill" ) );
    add_evaluator( eWorldPropertyFoundItemToKill,
                   xr_new< CStalkerPropertyEvaluatorFoundItemToKill >(
                       m_object, "found item to kill" ) );
    add_evaluator( eWorldPropertyFoundAmmo,
                   xr_new< CStalkerPropertyEvaluatorFoundAmmo >(
                       m_object, "found ammo" ) );
    add_evaluator( eWorldPropertyReadyToKill,
                   xr_new< CStalkerPropertyEvaluatorReadyToKill >(
                       m_object, "ready to kill" ) );
    add_evaluator( eWorldPropertyReadyToDetour,
                   xr_new< CStalkerPropertyEvaluatorReadyToDetour >(
                       m_object, "ready to detour" ) );
    add_evaluator(
        eWorldPropertyPanic,
        xr_new< CStalkerPropertyEvaluatorPanic >( m_object, "panic" ) );
    add_evaluator( eWorldPropertyDangerGrenade,
                   xr_new< CStalkerPropertyEvaluatorGrenadeToExplode >(
                       m_object, "is there grenade to explode" ) );
    add_evaluator( eWorldPropertyEnemyWounded,
                   xr_new< CStalkerPropertyEvaluatorEnemyWounded >(
                       m_object, "is enemy wounded" ) );
    add_evaluator( eWorldPropertyPlayerOnThePath,
                   xr_new< CStalkerPropertyEvaluatorPlayerOnThePath >(
                       m_object, "player on the path" ) );
    add_evaluator( eWorldPropertyEnemyCriticallyWounded,
                   xr_new< CStalkerPropertyEvaluatorEnemyCriticallyWounded >(
                       m_object, "enemy_critically_wounded" ) );
    add_evaluator( eWorldPropertyTooFarToKillEnemy,
                   xr_new< CStalkerPropertyEvaluatorTooFarToKillEnemy >(
                       m_object, "too far to kill" ) );

    add_evaluator( eWorldPropertyInCover,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyInCover, true,
                       true, "in cover" ) );
    add_evaluator( eWorldPropertyLookedOut,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyLookedOut, true,
                       true, "looked out" ) );
    add_evaluator( eWorldPropertyPositionHolded,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyPositionHolded,
                       true, true, "position holded" ) );
    add_evaluator( eWorldPropertyEnemyDetoured,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyEnemyDetoured,
                       true, true, "enemy detoured" ) );
    add_evaluator( eWorldPropertyUseSuddenness,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       ( CPropertyStorage* )0, eWorldPropertyUseSuddenness,
                       true, true, "use suddenness" ) );
    add_evaluator( eWorldPropertyCriticallyWounded,
                   xr_new< CStalkerPropertyEvaluatorMember >(
                       &object().brain().CStalkerPlanner::m_storage,
    ...
    add_evaluator( eWorldPropertyShouldThrowGrenade,
                   xr_new< CStalkerPropertyEvaluatorShouldThrowGrenade >(
                       m_object, "should throw grenade" ) );
    add_evaluator( eWorldPropertyLowCover,
                   xr_new< CStalkerPropertyEvaluatorLowCover >(
                       m_object, "using low cover" ) );

    add_evaluator( eWorldPropertyInSmartCover,
                   xr_new< smart_cover::evaluators::in_cover_evaluator >(
                       m_object, "in smart cover" ) );
}
...
void CStalkerCombatPlanner::add_actions() {
    ...
    action =
        xr_new< CStalkerActionGetItemToKill >( m_object, "get_item_to_kill" );
    ...
    action = xr_new< CStalkerActionMakeItemKilling >( m_object,
                                                      "make_item_killing" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    ...
    action = xr_new< CStalkerActionRetreatFromEnemy >( m_object,
                                                       "retreat_from_enemy" );
    add_effect( action, eWorldPropertyPureEnemy, false );
    add_operator( eWorldOperatorRetreatFromEnemy, action );

    action = xr_new< CStalkerActionGetReadyToKill >( true, m_object,
                                                     "get_ready_to_kill" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    ...
    action = xr_new< CStalkerActionGetReadyToKill >( false, m_object,
                                                     "get_ready_to_detour" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    add_condition( action, eWorldPropertyDangerGrenade, false );
    add_condition( action, eWorldPropertyUseSuddenness, false );
    ...
    add_condition( action, eWorldPropertyLowCover, false );
    //	add_condition
    //(action,eWorldPropertyShouldThrowGrenade,false);
    ...
    add_condition( action, eWorldPropertyPlayerOnThePath, false );
    //	add_condition
    //(action,eWorldPropertyTooFarToKillEnemy,false);
    ...
    {
        CStalkerSearchPlanner* action =
            xr_new< CStalkerSearchPlanner >( m_object, "search enemy planner" );
        add_condition( action, eWorldPropertyCriticallyWounded, false );
        add_condition( action, eWorldPropertyDangerGrenade, false );
        add_condition( action, eWorldPropertyUseSuddenness, false );
        add_condition( action, eWorldPropertyReadyToKill, true );
    ...
    action =
        xr_new< CStalkerActionKillEnemy >( m_object, "kill_if_not_visible" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    add_condition( action, eWorldPropertyDangerGrenade, false );
    add_condition( action, eWorldPropertyUseSuddenness, false );
    add_condition( action, eWorldPropertyReadyToKill, true );
    ...
    action = xr_new< CStalkerActionKillEnemy >( m_object,
                                                "kill_if_critically_wounded" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    add_condition( action, eWorldPropertyDangerGrenade, false );
    add_condition( action, eWorldPropertyUseSuddenness, false );
    add_condition( action, eWorldPropertyReadyToKill, true );
    ...
    action =
        xr_new< CStalkerActionPostCombatWait >( m_object, "post_combat_wait" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    add_condition( action, eWorldPropertyDangerGrenade, false );
    ...
    action = xr_new< CStalkerActionHideFromGrenade >( m_object,
                                                      "hide from grenade" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    ...
    action = xr_new< CStalkerActionKillEnemyIfPlayerOnThePath >(
        m_object, "kill enemy, if player is on my path" );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    add_condition( action, eWorldPropertyUseSuddenness, false );
    ...
    {
        CStalkerKillWoundedPlanner* planner =
            xr_new< CStalkerKillWoundedPlanner >( m_object,
                                                  "kill wounded enemy" );
    ...
    {
        CStalkerGetDistancePlanner* planner =
            xr_new< CStalkerGetDistancePlanner >( m_object, "get distance" );
        add_condition( planner, eWorldPropertyCriticallyWounded, false );
        add_condition( planner, eWorldPropertyDangerGrenade, false );
    ...
    {
        stalker_low_cover_planner* planner =
            xr_new< stalker_low_cover_planner >( m_object, "use low cover" );
        add_condition( planner, eWorldPropertyCriticallyWounded, false );
        add_condition( planner, eWorldPropertyDangerGrenade, false );
        add_condition( planner, eWorldPropertyUseSuddenness, false );
    ...
    action =
        xr_new< CStalkerCombatActionSmartCover >( m_object, "smart_cover" );
    add_condition( action, eWorldPropertyPureEnemy, true );
    add_condition( action, eWorldPropertyPanic, false );
    add_condition( action, eWorldPropertyCriticallyWounded, false );
    add_condition( action, eWorldPropertyDangerGrenade, false );
