...
void CEntityAlive::LoadBloodyWallmarks( LPCSTR section ) {
    ...
    for ( int k = 0; k < cnt; ++k )
        ( *m_pBloodMarksVector )
            ->AppendMark( _GetItem( wallmarks_name, k, tmp ) );
...
void CEntityAlive::reload( LPCSTR section ) {
    ...
    m_ef_detector_type = READ_IF_EXISTS( pSettings, r_u32, section,
...
BOOL CEntityAlive::net_Spawn( CSE_Abstract* DC ) {
    ...
    CEntityCondition::WOUND_VECTOR::const_iterator it =
        conditions().wounds().begin();
    CEntityCondition::WOUND_VECTOR::const_iterator it_e =
        conditions().wounds().end();
    for ( ; it != it_e; ++it ) {
        CWound* pWound = *it;
...
void CEntityAlive::HitImpulse( float /**amount/**/,
                               Fvector& /**vWorldDir/**/,
                               Fvector& /**vLocalDir/**/ ) {
    //	float Q					=
...
void CEntityAlive::Hit( SHit* pHDS ) {
    ...
    //-------------------------------------------------------------------
    CDamageManager::HitScale( HDS.boneID, conditions().hit_bone_scale(),
                              conditions().wound_bone_scale(),
                              pHDS->aim_bullet );
    ...
    if ( pWound && !pWound->GetDestroy() ) {
        if ( ALife::eHitTypeBurn == HDS.hit_type ||
             ALife::eHitTypeLightBurn == HDS.hit_type )
            StartFireParticles( pWound );
    }
    ...
    if ( HDS.hit_type != ALife::eHitTypeTelepatic &&
         ...
         HDS.hit_type != ALife::eHitTypeLightBurn &&
         HDS.hit_type != ALife::eHitTypeChemicalBurn ) {
        // äîáàâèòü êðîâü íà ñòåíû
        if ( !use_simplified_visual() )
            BloodyWallmarks( HDS.damage(), HDS.dir, HDS.bone(),
                             HDS.p_in_bone_space );
    }
    ...
    if ( g_Alive() && IsGameTypeSingle() ) {
        ...
        if ( EA && EA->g_Alive() && EA->ID() != ID() ) {
            RELATION_REGISTRY().FightRegister(
                EA->ID(), ID(), this->tfGetRelationType( EA ), HDS.damage() );
...
void CEntityAlive::Die( CObject* who ) {
    if ( IsGameTypeSingle() )
        RELATION_REGISTRY().Action( smart_cast< CEntityAlive* >( who ), this,
                                    RELATION_REGISTRY::KILL );
    inherited::Die( who );

    const CGameObject* who_object = smart_cast< const CGameObject* >( who );
    callback( GameObject::eDeath )(
        lua_game_object(), who_object ? who_object->lua_game_object() : 0 );
...
void CEntityAlive::BloodyWallmarks( float P,
                                    const Fvector& dir,
                                    s16 element,
    ...
    if ( V ) {
        Fmatrix& m_bone =
            ( V->LL_GetBoneInstance( u16( element ) ) ).mTransform;
    ...
    PlaceBloodWallmark( dir, start_pos, m_fBloodMarkDistance, wallmark_size,
                        &**m_pBloodMarksVector );
}
...
void CEntityAlive::PlaceBloodWallmark( const Fvector& dir,
    ...
    bool bSurfaceReached = Level().ObjectSpace.RayPick(
        start_pos, dir, trace_dist, collide::rqtBoth, result,
        r_blood_decals_on_objects ? nullptr : this );
    ...
    if ( r_blood_decals_on_objects && result.O ) {
        ...
        if ( pMaterial->Flags.is( SGameMtl::flBloodmark ) )
            ::Render->add_SkeletonWallmark( &result.O->renderable.xform, pK,
                                            pwallmarks_vector, end_point, dir,
                                            wallmark_size );
    }
    ...
    if ( pMaterial->Flags.is( SGameMtl::flBloodmark ) ) {
        ...
        // ref_shader wallmarkShader =
        // wallmarks_vector[::Random.randI(wallmarks_vector.size())];
...
void CEntityAlive::StartFireParticles( CWound* pWound ) {
    if ( pWound->TypeSize( ALife::eHitTypeBurn ) > m_fStartBurnWoundSize ) {
        ...
        u16 particle_bone =
            CParticlesPlayer::GetNearestBone( V, pWound->GetBoneNum() );
        VERIFY( particle_bone < 64 || BI_NONE == particle_bone );

        pWound->SetParticleBoneNum( particle_bone );
        pWound->SetParticleName( ( *m_pFireParticlesVector )[::Random.randI(
            0, m_pFireParticlesVector->size() ) ] );

        if ( BI_NONE != particle_bone ) {
            CParticlesPlayer::StartParticles(
                pWound->GetParticleName(), pWound->GetParticleBoneNum(),
                Fvector().set( 0, 1, 0 ), ID(),
                u32( float( m_dwMinBurnTime ) * ::Random.randF( 0.5f, 1.5f ) ),
                false );
        } else {
            CParticlesPlayer::StartParticles(
                pWound->GetParticleName(), Fvector().set( 0, 1, 0 ), ID(),
                u32( float( m_dwMinBurnTime ) * ::Random.randF( 0.5f, 1.5f ) ),
                false );
        }
    }
...
void CEntityAlive::UpdateFireParticles() {
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( !pWound->GetParticleName() ||
             pWound->GetParticleBoneNum() == BI_NONE )
            continue;

        float burn_size = pWound->TypeSize( ALife::eHitTypeBurn );

        if ( ( burn_size > 0 &&
               ( burn_size < m_fStopBurnWoundSize || !g_Alive() ) ) ) {
            CParticlesPlayer::AutoStopParticles(
                pWound->GetParticleName(), pWound->GetParticleBoneNum(),
                u32( float( m_dwMinBurnTime ) *
                     ::Random.randF( 0.5f, 1.5f ) ) );
        }
    }
}

ALife::ERelationType CEntityAlive::tfGetRelationType(
    const CEntityAlive* tpEntityAlive ) const {
    int relation = MONSTER_COMMUNITY::relation(
        this->monster_community->index(),
...
bool CEntityAlive::is_relation_enemy(
    const CEntityAlive* tpEntityAlive ) const {
    return (
        ( tfGetRelationType( tpEntityAlive ) == ALife::eRelationTypeEnemy ) ||
        ( tfGetRelationType( tpEntityAlive ) ==
          ALife::eRelationTypeWorstEnemy ) );
}

void CEntityAlive::UpdateBloodDrops() {
    static float m_fBloodDropTimeMax =
        pSettings->r_float( BLOOD_MARKS_SECT, "blood_drop_time_max" );
    static float m_fBloodDropTimeMin =
        pSettings->r_float( BLOOD_MARKS_SECT, "blood_drop_time_min" );
    ...
    for ( ; it != it_e; ++it ) {
        ...
        if ( pWound->m_fDropTime < Device.fTimeGlobal ) {
            ...
            pWound->m_fDropTime =
                Device.fTimeGlobal +
                ( m_fBloodDropTimeMax -
                  ( m_fBloodDropTimeMax - m_fBloodDropTimeMin ) * size_k ) *
                    Random.randF( 0.8f, 1.2f );
            VERIFY( m_pBloodDropsVector );
            Fvector pos;
            Fvector pos_distort;
            pos_distort.random_dir();
            pos_distort.mul( 0.15f );
            CParticlesPlayer::GetBonePos( this, pWound->GetBoneNum(),
                                          Fvector().set( 0, 0, 0 ), pos );
            pos.add( pos_distort );
            PlaceBloodWallmark( Fvector().set( 0.f, -1.f, 0.f ), pos,
                                m_fBloodMarkDistance, m_fBloodDropSize,
                                &**m_pBloodDropsVector );
        }
    }
}
...
CEntityConditionSimple* CEntityAlive::create_entity_condition(
...
DLL_Pure* CEntityAlive::_construct() {
    ...
    if ( character_physics_support() )
        m_material_manager = xr_new< CMaterialManager >(
...
struct element_predicate {
    inline bool operator()( CCF_Skeleton::SElement const& element,
                            u16 element_id ) const {
        return element.elem_id < element_id;
    }
}; // struct element_predicate

struct sort_surface_area_predicate {
    inline bool operator()( std::pair< u16, float > const& left,
                            std::pair< u16, float > const& right ) const {
...
void CEntityAlive::fill_hit_bone_surface_areas() const {
    ...
    for ( u16 i = 0, n = kinematics->LL_BoneCount(); i < n; ++i ) {
        ...
        switch ( shape.type ) {
            case SBoneShape::stBox: {
                ...
                surface_area =
                    2.f * ( half_size.x * ( half_size.y + half_size.z ) +
            ...
            case SBoneShape::stCylinder: {
                surface_area =
                    2.f * PI * shape.cylinder.m_radius *
                    ( shape.cylinder.m_radius + shape.cylinder.m_height );
                break;
            }
            default:
                NODEFAULT;
    ...
    std::sort( m_hit_bone_surface_areas.begin(), m_hit_bone_surface_areas.end(),
               sort_surface_area_predicate() );
...
Fvector CEntityAlive::get_new_local_point_on_mesh( u16& bone_id ) const {
    ...
    hit_bone_surface_areas_type::const_iterator i =
        m_hit_bone_surface_areas.begin();
    hit_bone_surface_areas_type::const_iterator const e =
        m_hit_bone_surface_areas.end();
    for ( ; i != e; ++i ) {
    ...
    VERIFY2( hit_bones_surface_area > 0.f,
             make_string( "m_hit_bone_surface_areas[%d]",
                          m_hit_bone_surface_areas.size() ) );
    float const selected_area =
        m_hit_bones_random.randF( hit_bones_surface_area );
    ...
    if ( i == e ) {
        Msg( "![CEntityAlive::get_new_local_point_on_mesh] ERROR, iterated "
             "through all m_hit_bone_surface_areas[%d] but accumulator %.5f < "
             "selected_area %.5f",
             m_hit_bone_surface_areas.size(), accumulator, selected_area );
        return inherited::get_new_local_point_on_mesh( bone_id );
    }
    ...
    switch ( shape.type ) {
        case SBoneShape::stBox: {
            ...
            Fvector random = { ( random_value & 1 ) ? -1.f : 1.f,
                               ::Random.randF( 2.f ) - 1.f,
                               ::Random.randF( 2.f ) - 1.f };
            random.normalize();
            if ( random_value < 2 )
        ...
        case SBoneShape::stCylinder: {
            float const total_square =
                ( shape.cylinder.m_height +
                  shape.cylinder.m_radius ); // *2*PI*c_cylinder.m_radius
            ...
            Fmatrix rotation =
                Fmatrix().rotation( shape.cylinder.m_direction, normal );
            ...
            if ( random_value < shape.cylinder.m_height ) {
                ...
                float const normalized_value =
                    random_value - shape.cylinder.m_height;
                height = shape.cylinder.m_height / 2.f *
                         ( ( normalized_value < shape.cylinder.m_radius / 2.f )
                               ? -1.f
                               : 1.f );
                radius = height > 0.f
                             ? normalized_value - shape.cylinder.m_radius / 2.f
...
Fvector CEntityAlive::get_last_local_point_on_mesh( Fvector const& last_point,
                                                    u16 bone_id ) const {
    if ( bone_id == u16( -1 ) )
