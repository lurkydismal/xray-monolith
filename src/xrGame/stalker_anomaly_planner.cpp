...
CStalkerAnomalyPlanner::CStalkerAnomalyPlanner( CAI_Stalker* object,
                                                LPCSTR action_name )
    : inherited( object, action_name ) {}

CStalkerAnomalyPlanner::~CStalkerAnomalyPlanner() {}

void CStalkerAnomalyPlanner::setup( CAI_Stalker* object,
                                    CPropertyStorage* storage ) {
    inherited::setup( object, storage );

    CScriptActionPlanner::m_storage.set_property( eWorldPropertyAnomaly,
                                                  false );
...
void CStalkerAnomalyPlanner::add_evaluators() {
    add_evaluator( eWorldPropertyInsideAnomaly,
                   xr_new< CStalkerPropertyEvaluatorInsideAnomaly >(
                       m_object, "inside anomaly" ) );
    add_evaluator( eWorldPropertyAnomaly,
                   xr_new< CStalkerPropertyEvaluatorAnomaly >(
                       m_object, "undetected anomaly" ) );
}

void CStalkerAnomalyPlanner::add_actions() {
    CStalkerActionBase* action;

    action = xr_new< CStalkerActionGetOutOfAnomaly >( m_object,
                                                      "get_out_of_anomaly" );
    ...
    action =
        xr_new< CStalkerActionDetectAnomaly >( m_object, "detect_anomaly" );
