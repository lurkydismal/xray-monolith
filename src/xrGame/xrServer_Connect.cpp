...
LPCSTR xrServer::get_map_download_url( LPCSTR level_name,
    ...
    if ( !level_ini ) {
        if ( !IsGameTypeSingle() )
            Msg( "! Warning: level [%s][%s] has not header ltx", level_name,
                 level_version );
...
xrServer::EConnect xrServer::Connect( shared_str& session_name,
    ...
    string1024 options;
    R_ASSERT2( xr_strlen( session_name ) <= sizeof( options ),
               "session_name too BIIIGGG!!!" );
    xr_strcpy( options, strchr( *session_name, '/' ) + 1 );

    // Parse game type
    string1024 type;
    R_ASSERT2( xr_strlen( options ) <= sizeof( type ),
               "session_name too BIIIGGG!!!" );
    ...
    xr_strcpy( game_descr.map_name,
               game->level_name( session_name.c_str() ).c_str() );
    xr_strcpy( game_descr.map_version,
               game_sv_GameState::parse_level_version( session_name.c_str() )
                   .c_str() );
    xr_strcpy(
        game_descr.download_url,
...
void xrServer::AttachNewClient( IClient* CL ) {
    ...
    if ( psNET_direct_connect ) // single_game
    {
        SV_Client = CL;
        CL->flags.bLocal = 1;
        SendTo_LL( SV_Client->ID, &msgConfig, sizeof( msgConfig ),
                   net_flags( TRUE, TRUE, TRUE, TRUE ) );
    } else {
...
void xrServer::ProcessClientDigest( xrClientData* xrCL, NET_Packet* P ) {
    ...
    if ( server_game->IsPlayerBanned( xrCL->m_cdkey_digest.c_str(),
                                      admin_name ) ) {
        R_ASSERT2( tmp_client != GetServerClient(),
                   "can't disconnect server client" );
        Msg( "--- Client [%s] tried to connect - rejecting connection (he is "
             "banned by %s) ...",
             tmp_client->m_cAddress.to_string().c_str(),
             admin_name.size() ? admin_name.c_str() : "Server" );
        LPCSTR message_to_user;
        if ( admin_name.size() ) {
            STRCONCAT( message_to_user, "mp_you_have_been_banned_by ",
                       admin_name.c_str() );
        } else {
            message_to_user = "";
        }
        SendConnectResult( tmp_client, 0, ecr_have_been_banned,
                           message_to_user );
        return;
    }
    GetPooledState( xrCL );
