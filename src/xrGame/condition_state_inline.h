...
TEMPLATE_SPECIALIZATION
IC const xr_vector< typename CConditionStateAbstract::COperatorCondition >&
CConditionStateAbstract::conditions() const {
    return ( m_conditions );
}

TEMPLATE_SPECIALIZATION
IC void CConditionStateAbstract::add_condition_back(
    const COperatorCondition& condition ) {
    THROW( m_conditions.empty() ||
           ( m_conditions.back().condition() < condition.condition() ) );
...
IC void CConditionStateAbstract::add_condition(
    const COperatorCondition& condition ) {
    xr_vector< COperatorCondition >::iterator I =
        std::lower_bound( m_conditions.begin(), m_conditions.end(), condition );
    THROW( ( I == m_conditions.end() ) ||
...
TEMPLATE_SPECIALIZATION
IC void CConditionStateAbstract::remove_condition(
    const typename COperatorCondition::_condition_type& condition ) {
    xr_vector< COperatorCondition >::iterator I = std::lower_bound(
        m_conditions.begin(), m_conditions.end(),
        COperatorCondition( condition, COperatorCondition::_value_type( 0 ) ) );
...
IC void CConditionStateAbstract::add_condition(
    ...
    const COperatorCondition& condition ) {
    m_conditions.insert( m_conditions.begin() + ( J - m_conditions.begin() ),
                         condition );
    m_hash ^= condition.hash_value();
}
...
TEMPLATE_SPECIALIZATION
IC u8 CConditionStateAbstract::weight(
    const CConditionState& condition ) const {
    u8 result = 0;
...
TEMPLATE_SPECIALIZATION
IC bool CConditionStateAbstract::operator<(
    const CConditionState& condition ) const {
    xr_vector< COperatorCondition >::const_iterator I = conditions().begin();
    xr_vector< COperatorCondition >::const_iterator E = conditions().end();
    xr_vector< COperatorCondition >::const_iterator i =
...
TEMPLATE_SPECIALIZATION
IC CConditionState< _world_property >& CConditionStateAbstract::operator-=(
    const CConditionState& condition ) {
    m_hash = 0;
...
TEMPLATE_SPECIALIZATION
IC bool CConditionStateAbstract::includes(
    const CConditionState& condition ) const {
    xr_vector< COperatorCondition >::const_iterator I = conditions().begin();
    xr_vector< COperatorCondition >::const_iterator E = conditions().end();
    xr_vector< COperatorCondition >::const_iterator i =
...
CConditionStateAbstract::property(
    const typename CConditionStateAbstract::COperatorCondition::_condition_type&
        condition ) const {
