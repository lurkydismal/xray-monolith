...
struct CMergeInOutPredicate {
    ...
    IC bool operator()( u32 level_vertex_id ) const {
        if ( !m_out || !m_in )
            return ( false );
        return ( a ? m_in->inside( level_vertex_id, false )
                   : !m_out->inside( level_vertex_id, true ) );
    }
};
...
struct CRemoveMergedFreeInRestrictions {
    ...
    IC CRemoveMergedFreeInRestrictions(
        CSpaceRestriction::RESTRICTIONS& restrictions ) {
        m_restrictions = &restrictions;
    }

    IC bool operator()( const CSpaceRestriction::CFreeInRestriction&
                            free_in_restriction ) const {
        return ( std::find( m_restrictions->begin(), m_restrictions->end(),
                            free_in_restriction.m_restriction ) !=
                 m_restrictions->end() );
    }
};
...
bool CSpaceRestriction::accessible( u32 level_vertex_id, float radius ) {
    ...
    return ( ( m_out_space_restriction ? m_out_space_restriction->inside(
                                             level_vertex_id, false, radius )
                                       : true ) &&
             ( m_in_space_restriction ? !m_in_space_restriction->inside(
                                            level_vertex_id, true, radius )
                                      : true ) );
}

IC bool CSpaceRestriction::intersects(
    SpaceRestrictionHolder::CBaseRestrictionPtr bridge0,
    ...
    if ( !bridge0->border().empty() &&
         bridge1->inside( bridge0->border().front(), true ) )
        return ( true );

    m_temp.resize( bridge0->border().size() + bridge1->border().size() );
    xr_vector< u32 >::iterator J = std::set_intersection(
        bridge0->border().begin(), bridge0->border().end(),
...
void CSpaceRestriction::merge_in_out_restrictions() {
    ...
    m_border.erase(
        std::remove_if( m_border.begin(), m_border.end(),
                        CMergeInOutPredicate< true >(
                            m_out_space_restriction, m_in_space_restriction ) ),
        m_border.end() );
    ...
    if ( m_in_space_restriction ) {
        ...
        temp_border.erase(
            std::remove_if(
                temp_border.begin(), temp_border.end(),
                CMergeInOutPredicate< false >( m_out_space_restriction,
                                               m_in_space_restriction ) ),
            temp_border.end() );
        m_border.insert( m_border.end(), temp_border.begin(),
                         temp_border.end() );
    }

    std::sort( m_border.begin(), m_border.end() );
    m_border.erase( std::unique( m_border.begin(), m_border.end() ),
                    m_border.end() );
    STOP_PROFILE;
}

CSpaceRestriction::CBaseRestrictionPtr CSpaceRestriction::merge(
    CBaseRestrictionPtr bridge,
    ...
    for ( ; I != E; ++I )
        temp = strconcat(
            strlen( S ), S, *temp, ",",
            *( *I )->name() ); // SizeOf to strlen because S is LPSTR pointer
...
void CSpaceRestriction::merge_free_in_retrictions() {
    ...
    for ( u32 i = 0, n = _GetItemCount( *m_in_restrictions ); i < n; ++i ) {
        SpaceRestrictionHolder::CBaseRestrictionPtr bridge =
            m_space_restriction_manager->restriction(
                shared_str( _GetItem( *m_in_restrictions, i, temp ) ) );
...
void CSpaceRestriction::initialize() {
    ...
    m_out_space_restriction =
        m_space_restriction_manager->restriction( m_out_restrictions );
    m_in_space_restriction =
        m_space_restriction_manager->restriction( m_in_restrictions );
    ...
    if ( ( m_out_space_restriction &&
           !m_out_space_restriction->initialized() ) ||
...
u32 CSpaceRestriction::accessible_nearest( const Fvector& position,
                                           Fvector& result ) {
    if ( m_out_space_restriction )
        return ( m_out_space_restriction->accessible_nearest( this, position,
                                                              result, true ) );

    VERIFY( m_in_space_restriction );
    return ( m_in_space_restriction->accessible_nearest(
        m_in_space_restriction, position, result, false ) );
}

bool CSpaceRestriction::affect(
    SpaceRestrictionHolder::CBaseRestrictionPtr bridge,
    const Fsphere& sphere ) const {
    ...
    // return
    // (start_position.distance_to(position) <= radius + dependent_distance);
}
...
bool CSpaceRestriction::affect(
    ...
    return ( affect( bridge, start_vertex_id, 0.f ) ||
             affect( bridge, dest_vertex_id, 0.f ) );
}
