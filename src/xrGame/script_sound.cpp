...
#ifdef DEBUG
    THROW3( !m_sound._feedback(),
            "playing sound is not completed, but is destroying",
...
void CScriptSound::PlayAtPos( CScriptGameObject* object,
    ...
    m_sound.play_at_pos( ( object ) ? &object->object() : NULL, position, flags,
                         delay );
}

void CScriptSound::PlayNoFeedback( CScriptGameObject* object,
                                   u32 flags /*!< Looping */,
                                   float delay /*!< Delay */,
                                   Fvector pos,
                                   float vol,
                                   float freq ) {
    THROW3( m_sound._handle(), "There is no sound", *m_caSoundToPlay );
    m_sound.play_no_feedback( ( object ) ? &object->object() : NULL, flags,
                              delay, &pos, &vol, &freq );
}
