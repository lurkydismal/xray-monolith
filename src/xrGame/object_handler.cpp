...
void CObjectHandler::reinit( CAI_Stalker* object ) {
    ...
    IKinematics* kinematics =
        smart_cast< IKinematics* >( planner().m_object->Visual() );
    m_r_hand = kinematics->LL_BoneID( pSettings->r_string(
        *planner().m_object->cNameSect(), "weapon_bone0" ) );
    m_l_finger1 = kinematics->LL_BoneID( pSettings->r_string(
        *planner().m_object->cNameSect(), "weapon_bone1" ) );
    m_r_finger2 = kinematics->LL_BoneID( pSettings->r_string(
        *planner().m_object->cNameSect(), "weapon_bone2" ) );
...
BOOL CObjectHandler::net_Spawn( CSE_Abstract* DC ) {
    ...
    CSE_ALifeTraderAbstract* trader =
        smart_cast< CSE_ALifeTraderAbstract* >( abstract );
    VERIFY( trader );

    m_infinite_ammo = !!trader->m_trader_flags.test(
        CSE_ALifeTraderAbstract::eTraderFlagInfiniteAmmo );
    return ( TRUE );
}
...
void CObjectHandler::OnItemTake( CInventoryItem* inventory_item ) {
    ...
    if ( inventory_item->useful_for_NPC() &&
         ( inventory_item->object().cNameSect() == m_item_to_spawn ) ) {
...
void CObjectHandler::OnItemDrop( CInventoryItem* inventory_item,
    ...
    if ( g_ai_unlimited_ammo && m_infinite_ammo &&
         planner().object().g_Alive() && !inventory_item->useful_for_NPC() ) {
        CWeaponAmmo* weapon_ammo = smart_cast< CWeaponAmmo* >( inventory_item );
        if ( weapon_ammo ) {
            Level().spawn_item(
                *weapon_ammo->cNameSect(), planner().object().Position(),
                planner().object().ai_location().level_vertex_id(),
                planner().object().ID() );
...
void CObjectHandler::set_goal( MonsterSpace::EObjectAction object_action,
                               CGameObject* game_object,
                               u32 min_queue_size,
                               u32 max_queue_size,
                               u32 min_queue_interval,
                               u32 max_queue_interval ) {
    planner().set_goal( object_action, game_object, min_queue_size,
                        max_queue_size, min_queue_interval,
                        max_queue_interval );
}
...
void CObjectHandler::set_goal( MonsterSpace::EObjectAction object_action,
                               ...
                               u32 min_queue_interval,
                               u32 max_queue_interval ) {
    set_goal( object_action, inventory_item ? &inventory_item->object() : 0,
...
void CObjectHandler::weapon_bones( int& b0, int& b1, int& b2 ) const {
    ...
    if ( !weapon || !planner().m_storage.property(
    ...
    THROW3( weapon->can_be_strapped(), "Cannot strap weapon",
            *weapon->cName() );
...
void CObjectHandler::actualize_strap_mode( CWeapon* weapon ) const {
    ...
    if ( !planner().m_storage.property(
             ObjectHandlerSpace::eWorldPropertyStrapped ) ) {
        //		Msg						(
        //"[%6d][%s] actualizing: weapon_strapped = false", Device.dwTimeGlobal,
    ...
    THROW3( weapon->can_be_strapped(), "Cannot strap weapon",
            *weapon->cName() );
    //	Msg							( "[%6d][%s]
    //actualizing: weapon_strapped = true", Device.dwTimeGlobal,
    //planner().object().cName().c_str() );
    weapon->strapped_mode( true );
}
...
bool CObjectHandler::weapon_strapped( CWeapon* weapon ) const {
    ...
    if ( !weapon->can_be_strapped() ) {
        //		Msg						(
        //"[%6d][%s] weapon_strapped = false0", Device.dwTimeGlobal,
        //planner().object().cName().c_str() );
        return ( false );
    }

    bool const almost_strapped =
        planner().current_action_state_id() ==
        ObjectHandlerSpace::eWorldOperatorStrapping2Idle;
    if ( almost_strapped ||
         ( planner().current_action_state_id() ==
           ObjectHandlerSpace::eWorldOperatorStrapping ) ||
         ( planner().current_action_state_id() ==
           ObjectHandlerSpace::eWorldOperatorUnstrapping2Idle ) ||
         ( planner().current_action_state_id() ==
           ObjectHandlerSpace::eWorldOperatorUnstrapping ) ) {
        //		Msg						(
        //"[%6d][%s] weapon_strapped = %s1", Device.dwTimeGlobal,
        //planner().object().cName().c_str(), almost_strapped &&
        //!planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle)
        //? "true" : "false" );

        if ( !almost_strapped ||
             planner().m_storage.property(
                 ObjectHandlerSpace::eWorldPropertyStrapped2Idle ) )
            return ( false );
    }
...
bool CObjectHandler::weapon_unstrapped() const {
    ...
    if ( !weapon ) {
        //		Msg						(
        //"[%6d][%s] no active item!!(%d)(%s)", Device.dwTimeGlobal,
        //planner().object().cName().c_str(), inventory().GetActiveSlot(),
        //inventory().ItemFromSlot(3) ?
...
bool CObjectHandler::weapon_unstrapped( CWeapon* weapon ) const {
    ...
    if ( !weapon->can_be_strapped() ) {
        //		Msg						(
        //"[%6d][%s] weapon_unstrapped = true0", Device.dwTimeGlobal,
        //planner().object().cName().c_str() );
        return ( true );
    }
    ...
    if ( almost_unstrapped ||
         ...
         ( planner().current_action_state_id() ==
           ObjectHandlerSpace::eWorldOperatorUnstrapping ) ) {
        //		Msg						(
        //"[%6d][%s] weapon_unstrapped = %s1", Device.dwTimeGlobal,
    ...
    VERIFY( ( planner().current_action_state_id() !=
              ObjectHandlerSpace::eWorldOperatorStrapped ) ||
            weapon->strapped_mode() );

    return ( !weapon->strapped_mode() );
}

IC void CObjectHandler::switch_torch( CInventoryItem* inventory_item,
                                      bool value ) {
...
void CObjectHandler::set_inertia( const CWeapon& weapon,
                                  const u32& action_id,
                                  const u32& aim_time ) const {
...
void CObjectHandler::aim_time( const CWeapon& weapon,
                               const u32& aim_time ) const {
    ...
    set_inertia( weapon, ObjectHandlerSpace::eWorldOperatorAimingReady1,
                 aim_time );
    set_inertia( weapon, ObjectHandlerSpace::eWorldOperatorAimingReady2,
                 aim_time );
    set_inertia( weapon, ObjectHandlerSpace::eWorldOperatorAimForceFull1,
                 aim_time );
    set_inertia( weapon, ObjectHandlerSpace::eWorldOperatorAimForceFull2,
                 aim_time );
}

u32 CObjectHandler::aim_time( const CWeapon& weapon ) const {
    _condition_type operator_id =
        planner().uid( weapon.ID(), ObjectHandlerSpace::eWorldOperatorAim1 );
    typedef CObjectHandlerPlanner::CSActionBase operator_type;
...
bool CObjectHandler::is_weapon_going_to_be_strapped(
    CGameObject const* object ) const {
    CObjectHandlerPlanner const& planner = this->planner();
    typedef xr_vector< typename GraphEngineSpace::CWorldProperty >
        properties_type;
    CWorldProperty const strap_property(
        planner.uid( object->ID(),
    ...
    properties_type::const_iterator const found =
        std::lower_bound( properties.begin(), properties_end, strap_property );
