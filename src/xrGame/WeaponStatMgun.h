...
/*
If you want CWeaponStatMgun to be an item too - putting in inventory, cost,
weight, icon, condition, etc, replace CPhysicsShellHolder with CInventoryItem or
CInventoryItemObject and fix all the problems coming along. Check
...
struct SStmBarrel {
    ...
    void UpdateParticles( CParticlesObject*& pParticles,
                          const Fvector& pos,
                          const Fvector& vel = zero_vel );
    ...
    void StartSmokeParticles( const Fvector& play_pos,
...
private:
    ...
    float m_tgt_x_rot, m_tgt_y_rot, m_cur_x_rot, m_cur_y_rot, m_bind_x_rot,
        m_bind_y_rot;
...
protected:
    ...
    void ClampRotationHorz( float& tgt_val,
                            const float& cur_val,
                            const float& lim_min,
                            const float& lim_max );
...
public:
    ...
    virtual void SetNextAmmoTypeOnReload(
        u8 ammo_type = SStmNextAmmoTypeOnReload::undefined );
