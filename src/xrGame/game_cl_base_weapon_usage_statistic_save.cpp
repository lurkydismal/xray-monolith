...
void WeaponUsageStatistic::WriteLtx( CInifile& ini ) {
    ...
    ini.w_u32( sect, "dwTotalPlayersAliveTime_0_sec",
               m_dwTotalPlayersAliveTime[ 0 ] / 1000 );
    ini.w_u32( sect, "dwTotalPlayersAliveTime_1_sec",
               m_dwTotalPlayersAliveTime[ 1 ] / 1000 );
    ini.w_u32( sect, "dwTotalPlayersAliveTime_2_sec",
               m_dwTotalPlayersAliveTime[ 2 ] / 1000 );

    ini.w_u32( sect, "dwTotalPlayersMoneyRound_0",
               m_dwTotalPlayersMoneyRound[ 0 ] );
    ini.w_u32( sect, "dwTotalPlayersMoneyRound_1",
               m_dwTotalPlayersMoneyRound[ 1 ] );
    ini.w_u32( sect, "dwTotalPlayersMoneyRound_2",
               m_dwTotalPlayersMoneyRound[ 2 ] );
...
void WeaponUsageStatistic::SaveData() {
    ...
    switch ( GameID() ) {
        case eGameIDDeathmatch:
            xr_sprintf( GameType, "dm" );
            break;
        case eGameIDTeamDeathmatch:
            xr_sprintf( GameType, "tdm" );
            break;
        case eGameIDArtefactHunt:
            xr_sprintf( GameType, "ah" );
            break;
        case eGameIDCaptureTheArtefact:
            xr_sprintf( GameType, "cta" );
            break;
        default:
            return;
            break;
    ...
    xr_sprintf( mFileName, "(%s)_(%s)_%02d.%02d.%02d_%02d.%02d.%02d.wus",
                *( Level().name() ), GameType, Time.wMonth, Time.wDay,
...
void Weapon_Statistic::add_hit( HitData const& hit ) {
    ...
    for ( HITS_VEC::reverse_iterator i = m_Hits.rbegin(), ie = m_Hits.rend();
          i != ie; ++i ) {
        HitData& tmp_hit = *i;
...
void HitData::WriteLtx( CInifile& ini, LPCSTR sect, LPCSTR prefix ) {
    ...
    ini.w_fvector3( sect, strconcat( sizeof( buff ), buff, prefix, "pos_0" ),
                    Pos0 );
    ini.w_fvector3( sect, strconcat( sizeof( buff ), buff, prefix, "pos_1" ),
                    Pos1 );

    ini.w_u16( sect, strconcat( sizeof( buff ), buff, prefix, "BoneID" ),
               BoneID );

    ini.w_bool( sect, strconcat( sizeof( buff ), buff, prefix, "Deadly" ),
                Deadly );

    ini.w_string( sect, strconcat( sizeof( buff ), buff, prefix, "TargetName" ),
                  TargetName.c_str() );

    ini.w_string( sect, strconcat( sizeof( buff ), buff, prefix, "BoneName" ),
                  BoneName.c_str() );
};
