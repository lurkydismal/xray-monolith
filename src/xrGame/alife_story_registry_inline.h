...
IC void CALifeStoryRegistry::remove( ALife::_STORY_ID id, bool no_assert ) {
    ...
    if ( I == m_objects.end() ) {
        if ( !no_assert ) {
            Msg( "Cannot find story object with id [%d] in the Story registry!",
                 id );
            THROW( false );
        }
        return;
...
IC const CALifeStoryRegistry::STORY_REGISTRY& CALifeStoryRegistry::objects()
    const {
    return ( m_objects );
}

IC CSE_ALifeDynamicObject* CALifeStoryRegistry::object( ALife::_STORY_ID id,
