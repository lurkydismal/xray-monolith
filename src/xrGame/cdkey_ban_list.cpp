...
void cdkey_ban_list::load() {
    ...
    for ( CInifile::Root::iterator i = banlist.begin(), ie = banlist.end();
          i != ie; ++i ) {
        banned_client* tmp_client = xr_new< banned_client >();
        if ( tmp_client->load( &bl_ini, ( *i ).Name ) ) {
            m_ban_list.push_back( tmp_client );
        } else {
            Msg( "! ERROR: load [%s] ban item section",
                 ( *i ).Name.size() > 0 ? ( *i ).Name.c_str() : "" );
...
void cdkey_ban_list::save() {
    ...
    for ( ban_list_t::iterator i = m_ban_list.begin(), ie = m_ban_list.end();
          i != ie; ++i ) {
...
bool cdkey_ban_list::is_player_banned( char const* hexstr_digest,
                                       shared_str& buy_who ) {
    if ( !hexstr_digest )
        return false;
    ...
    for ( ban_list_t::iterator i = m_ban_list.begin(), ie = m_ban_list.end();
        ...
        if ( !xr_strcmp( ( *i )->client_hexstr_digest, hexstr_digest ) ) {
            Msg( "* found banned client [%s] by admin [%s]", hexstr_digest,
                 ( *i )->admin_name.size() ? ( *i )->admin_name.c_str()
                                           : "Server" );
...
void cdkey_ban_list::ban_player( xrClientData const* player_data,
                                 s32 end_time_sec,
                                 xrClientData const* admin ) {
    banned_client* tmp_client = xr_new< banned_client >();
    ...
    if ( !player_data->m_cdkey_digest.size() ) {
        Msg( "! ERROR: Can't ban client without unique digest, try to ban by "
             "IP address." );
        xr_delete( tmp_client );
        return;
    }
    ...
    tmp_client->ban_end_time =
        tmp_client->ban_start_time + static_cast< u32 >( end_time_sec );
    if ( admin ) {
...
void cdkey_ban_list::ban_player_ll( char const* hexstr_digest,
                                    s32 end_time_sec,
                                    xrClientData const* admin ) {
    banned_client* tmp_client = xr_new< banned_client >();
...
char const* print_time( time_t const& src_time, string64& dest_time ) {
    ...
    xr_sprintf( dest_time, sizeof( dest_time ), "%02d.%02d.%d_%02d:%02d:%02d",
                tmp_tm->tm_mday, tmp_tm->tm_mon + 1, tmp_tm->tm_year + 1900,
...
void cdkey_ban_list::print_ban_list( char const* filter_string ) {
    ...
    for ( ban_list_t::iterator i = m_ban_list.begin(), ie = m_ban_list.end();
        ...
        xr_sprintf( tmp_string,
                    "- (player index : %d), (ip : %s), (name : %s), (end time "
                    ": %s), (hex digest : %s);",
                    index, ( *i )->client_ip_addr.to_string().c_str(),
                    ( *i )->client_name.c_str(),
...
time_t get_time_from_string( LPCSTR str_time ) {
    tm tmp_time;
    int res_t = sscanf( str_time, "%02d.%02d.%d_%02d:%02d:%02d",
                        &tmp_time.tm_mday, &tmp_time.tm_mon, &tmp_time.tm_year,
...
bool cdkey_ban_list::banned_client::load( CInifile* ini,
                                          shared_str const& name_sect ) {
...
void cdkey_ban_list::banned_client::save( CInifile* ini,
                                          char const* name_sect ) {
    ini->w_string( name_sect, CLIENT_HEX_DIGEST_KEY,
                   client_hexstr_digest.c_str() );
    string64 stor_string;
    ini->w_string( name_sect, CLIENT_BAN_START_TIME_KEY,
                   print_time( ban_start_time, stor_string ) );
    ini->w_string( name_sect, CLIENT_BAN_END_TIME_KEY,
                   print_time( ban_end_time, stor_string ) );
    ini->w_string( name_sect, CLIENT_NAME_KEY, client_name.c_str() );
    ini->w_string( name_sect, CLIENT_IP_KEY,
                   client_ip_addr.to_string().c_str() );
    ini->w_string( name_sect, ADMIN_NAME_KEY, admin_name.c_str() );
    ini->w_string( name_sect, ADMIN_IP_KEY, admin_ip_addr.to_string().c_str() );
    ini->w_string( name_sect, ADMIN_HEX_DIGEST_KEY,
                   admin_hexstr_digest.c_str() );
}
...
void cdkey_ban_list::erase_expired_ban_items() {
    ...
    ban_list_t::iterator new_end_iter =
        std::remove_if( m_ban_list.begin(), m_ban_list.end(), tmp_predicate );
