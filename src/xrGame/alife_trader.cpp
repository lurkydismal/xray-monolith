...
u32 CSE_ALifeTrader::dwfGetItemCost(
...
#pragma todo( "Dima to Dima : correct price for non-artefact objects" )
    CSE_ALifeItemArtefact* l_tpALifeItemArtefact =
        smart_cast< CSE_ALifeItemArtefact* >( tpALifeInventoryItem );
    if ( !l_tpALifeItemArtefact )
        return ( tpALifeInventoryItem->m_dwCost );

    u32 l_dwPurchasedCount = 0;
#pragma todo( \
    "Dima to Dima : optimize this cycle by keeping additional data structure with bought items" )
    {
        ALife::OBJECT_IT i = children.begin();
        ALife::OBJECT_IT e = children.end();
        for ( ; i != e; ++i )
            if ( !xr_strcmp( ai().alife().objects().object( *i )->s_name,
                             l_tpALifeItemArtefact->s_name ) )
                ++l_dwPurchasedCount;
    }
    return ( tpALifeInventoryItem->m_dwCost );
}
...
void CSE_ALifeTrader::add_offline(
