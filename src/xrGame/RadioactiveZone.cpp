...
void CRadioactiveZone::Affect( SZoneObjectInfo* O ) {
    ...
    while ( O->f_time_affected + one < tg ) {
        CreateHit( O->object->ID(), ID(), dir, send_power, BI_NONE,
                   Fvector().set( 0.0f, 0.0f, 0.0f ), impulse,
                   m_eHitTypeBlowout );
#ifdef DEBUG
        //		if(bDebug)
        /*		Msg			(	"Zone[%s]-hit->[%s]
           Power=%3.3f Frame=%d Time=%3.3f", cName().c_str(),
...
void CRadioactiveZone::feel_touch_new( CObject* O ) {
    ...
    if ( GameID() != eGameIDSingle ) {
        if ( smart_cast< CActor* >( O ) ) {
            CreateHit( O->ID(), ID(), Fvector().set( 0, 0, 0 ), 0.0f, BI_NONE,
                       Fvector().set( 0, 0, 0 ), 0.0f,
    ...
    if ( IsEnabled() && GameID() != eGameIDSingle ) {
        ...
        for ( it = m_ObjectInfoMap.begin(); m_ObjectInfoMap.end() != it;
            ...
            if ( !( *it ).object->getDestroy() &&
                ...
                float power =
                    Power( dist, nearest_shape_radius( &( *it ) ) ) * dt / 1000;
