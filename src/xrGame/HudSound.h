    ...
    ICF void set_position( const Fvector& pos ) {
        if ( m_activeSnd ) {
            if ( m_activeSnd->snd._feedback() &&
                 !m_activeSnd->snd._feedback()->is_2D() )
...
public:
    ...
    shared_str
        m_alias; // Alundaio: For use when it's part of a layered Collection
    xr_vector< HUD_SOUND_ITEM > m_sound_items; // Alundaio: made public

    HUD_SOUND_ITEM* FindSoundItem(
        LPCSTR alias,
        bool b_assert ); // AVO: made public to check if sound is loaded
...
public:
    ...
    void LoadSound( LPCSTR section,
                    ...
                    bool exclusive = false,
                    int type = sg_SourceType );
    void LoadSound( CInifile const* ini,
                    LPCSTR section,
                    LPCSTR line,
                    LPCSTR alias,
                    bool exclusive = false,
