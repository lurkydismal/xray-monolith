...
stalker_movement_params& stalker_movement_params::operator=(
    stalker_movement_params const& rhs ) {
    ...
    m_desired_direction =
        rhs.m_desired_direction ? &m_desired_direction_impl : 0;
    ...
    m_cover_fire_position =
        rhs.m_cover_fire_position ? &m_cover_fire_position_impl : 0;
...
bool stalker_movement_params::equal_to_target(
    stalker_movement_params const& target ) const {
    ...
    if ( !m_cover_fire_position_impl.similar(
             target.m_cover_fire_position_impl ) )
        return ( false );
...
void stalker_movement_params::actualize_loophole() const {
    if ( m_selected_loophole_actual ) {
        if ( !m_cover || !m_cover_selected_loophole ||
             m_cover->description()->loophole(
                 m_cover_selected_loophole->id() ) ) {
            if ( m_last_selection_time + time_before_selection >
                 Device.dwTimeGlobal )
                return;
        }
    }
    ...
    m_cover_selected_loophole = m_cover->best_loophole(
        position, value, true, m_manager->current_params().cover() == m_cover );
}
...
LPCSTR stalker_movement_params::cover_loophole_id() const {
    ...
    VERIFY(
        ...
        m_cover->description()->loophole( m_cover_selected_loophole->id() ) );
