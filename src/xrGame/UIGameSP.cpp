...
bool CUIGameSP::IR_UIOnKeyboardPress( int dik ) {
    ...
    CInventoryOwner* pInvOwner =
        smart_cast< CInventoryOwner* >( Level().CurrentEntity() );
    if ( !pInvOwner )
        return false;
        ...
        case kSCORES:
            if ( !pActor->inventory_disabled() ) {
                ...
                if ( t1 && t1->m_Description.c_str() ) {
                    StaticDrawableWrapper* sm2 =
                        AddCustomStatic( "secondary_task", true );
                    sm2->m_static->TextItemControl()->SetTextST(
                        t1->m_Description.c_str() );
                }
            }
            break;
    }
...
void CUIGameSP::StartTrade( CInventoryOwner* pActorInv,
    ...
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_OnMode_Trade", funct1 ) ) {
...
void CUIGameSP::StartUpgrade( CInventoryOwner* pActorInv,
                              CInventoryOwner* pMech ) {
    //.	if( MainInputReceiver() )	return;

    //---- before upgrade mode ---------------------------
    ::luabind::functor< bool > funct1;
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_OnMode_Upgrade", funct1 ) ) {
...
void CUIGameSP::StartCarBody( CInventoryOwner* pActorInv,
                              CInventoryOwner* pOtherOwner ) // Deadbody search
{
    if ( TopInputReceiver() )
        return;
    ...
    ::luabind::functor< bool > funct1;
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_OnMode_DeadBodySearch",
             funct1 ) ) {
        CGameObject* GO = smart_cast< CGameObject* >( pOtherOwner );
        if ( funct1( GO->lua_game_object() ) )
            return;
...
void CUIGameSP::StartCarBody( CInventoryOwner* pActorInv,
                              CInventoryBox* pBox ) // Deadbody search
{
    if ( TopInputReceiver() )
        return;
