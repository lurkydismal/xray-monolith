...
void configs_dumper::shedule_Update( u32 dt ) {
    ...
    R_ASSERT( ( thread_result != WAIT_ABANDONED ) &&
              ( thread_result != WAIT_FAILED ) );
    R_ASSERT( m_state == ds_active );
    if ( thread_result == WAIT_OBJECT_0 ) {
        m_complete_cb( m_buffer_for_compress, m_buffer_for_compress_size,
                       m_dump_result.size() );
        m_state = ds_not_active;
...
static active_objects_t::size_type get_active_objects(
    active_objects_t& dest ) {
    CActorMP const* tmp_actor =
        smart_cast< CActorMP const* >( Level().CurrentControlEntity() );
    ...
    for ( u16 i = KNIFE_SLOT; i <= GRENADE_SLOT; ++i ) {
        ...
        CInventoryItem const* tmp_inv_item =
            tmp_actor->inventory().ItemFromSlot( i );
        if ( !tmp_inv_item )
            continue;

        CWeapon const* tmp_weapon =
            smart_cast< CWeapon const* >( tmp_inv_item );
        if ( tmp_weapon ) {
            ...
            if ( tmp_weapon->m_magazine.size() ) {
                ...
                IAnticheatDumpable const* tmp_cartridge =
                    &tmp_weapon->m_magazine[ 0 ];
                if ( !tmp_cartridge )
                    continue;

                ExistDumpPredicate tmp_predicate;
                tmp_predicate.section_name =
                    tmp_cartridge->GetAnticheatSectionName();
                if ( std::find_if( dest.begin(), dest.end(), tmp_predicate ) ==
                     dest.end() ) {
...
void configs_dumper::write_configs() {
    ...
    active_objects_t active_objects(
        ...
        max_active_objects );
    active_objects_t::size_type aobjs_count =
        get_active_objects( active_objects );
    string16 tmp_strbuff;
    for ( active_objects_t::size_type i = 0; i < aobjs_count; ++i ) {
        xr_sprintf( tmp_strbuff, "%d", i + 1 );
        m_active_params.dump( active_objects[ i ], tmp_strbuff,
                              active_params_dumper );
    }
...
void configs_dumper::sign_configs() {
    ...
    STRCONCAT( tmp_player_name, "\"",
               tmp_cl_game->local_player ? tmp_cl_game->local_player->getName()
    ...
    LPCSTR add_str = NULL;
    STRCONCAT( add_str, tmp_player_name, tmp_cdkey_digest,
    ...
    if ( m_yield_cb ) {
        tmp_dsign = m_dump_signer.sign_mt( m_dump_result.pointer(),
                                           m_dump_result.size(), m_yield_cb );
    } else {
        tmp_dsign =
            m_dump_signer.sign( m_dump_result.pointer(), m_dump_result.size() );
    }
...
void configs_dumper::compress_configs() {
    ...
    m_buffer_for_compress_size = ppmd_compress_mt(
        m_buffer_for_compress, m_buffer_for_compress_capacity,
        m_dump_result.pointer(), m_dump_result.size(), ts_cb );
}
...
void configs_dumper::dumper_thread( void* my_ptr ) {
    ...
    DWORD wait_result =
        WaitForSingleObject( this_ptr->m_make_start_event, INFINITE );
...
void configs_dumper::realloc_compress_buffer( u32 need_size ) {
    if ( m_buffer_for_compress &&
         ( need_size <= m_buffer_for_compress_capacity ) )
        return;
#ifdef DEBUG
    ...
    void* new_buffer =
        xr_realloc( m_buffer_for_compress, m_buffer_for_compress_capacity );
...
void configs_dumper::timer_end() {
    Msg( "* %s : %u ms", m_timer_comment.c_str(),
         m_debug_timer.GetElapsed_ms() );
}
#endif // #ifdef DEBUG
