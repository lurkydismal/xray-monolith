...
template < typename _operator_condition,
...
private:
    typedef CProblemSolver< _operator_condition,
                            _condition_state,
                            _operator,
...
public:
    ...
    struct SOperator {
        ...
        IC SOperator( const _operator_id_type& operator_id,
    ...
    typedef associative_vector< _condition_type, _condition_evaluator_ptr >
        EVALUATORS;
...
private:
    ...
    template <>
    IC bool is_goal_reached_impl< true >(
    ...
    IC _edge_value_type
    estimate_edge_weight_impl( const _index_type& vertex_index ) const;
    IC _edge_value_type
    estimate_edge_weight_impl( const _index_type& vertex_index, bool ) const;

private:
    template < bool >
    struct helper {
        static IC _edge_value_type
        estimate_edge_weight_impl( self_type const& self,
...
public:
    ...
    // graph interface
    IC _edge_value_type get_edge_weight( const _index_type& vertex_index0,
                                         const _index_type& vertex_index1,
                                         const const_iterator& i ) const;
    IC bool is_accessible( const _index_type& vertex_index ) const;
    IC const _index_type& value( const _index_type& vertex_index,
                                 const_iterator& i,
                                 bool reverse_search ) const;
    IC void begin( const _index_type& vertex_index,
                   const_iterator& b,
                   const_iterator& e ) const;
    IC bool is_goal_reached( const _index_type& vertex_index ) const;
    IC _edge_value_type
    estimate_edge_weight( const _index_type& vertex_index ) const;

    // operator interface
    IC virtual void add_operator( const _edge_type& operator_id,
                                  _operator_ptr _operator );
    ...
    // evaluator interface
    IC virtual void add_evaluator( const _condition_type& condition_id,
                                   _condition_evaluator_ptr evaluator );
    IC virtual void remove_evaluator( const _condition_type& condition_id );
    IC _condition_evaluator_ptr
    evaluator( const _condition_type& condition_id ) const;
    IC const EVALUATORS& evaluators() const;
    IC void evaluate_condition(
