...
CRestrictedObjectObstacle::CRestrictedObjectObstacle(
    CCustomMonster* object,
    const obstacles_query& static_query,
    const obstacles_query& dynamic_query )
    : inherited( object ),
      m_static_query( static_query ),
      m_dynamic_query( dynamic_query ) {}

void CRestrictedObjectObstacle::apply( const obstacles_query& query,
                                       const u32& start_vertex_id ) const {
...
void CRestrictedObjectObstacle::add_border( u32 start_vertex_id,
                                            float radius ) const {
...
void CRestrictedObjectObstacle::add_border(
    const Fvector& start_position,
    const Fvector& dest_position ) const {
