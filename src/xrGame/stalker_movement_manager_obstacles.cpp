...
CRestrictedObject*
stalker_movement_manager_obstacles::create_restricted_object() {
    m_restricted_object = xr_new< CRestrictedObjectObstacle >(
        &object(), m_static_obstacles.active_query(),
...
bool stalker_movement_manager_obstacles::apply_border(
    const obstacles_query& query ) {
    ...
    restricted_object().CRestrictedObject::add_border( start_vertex_id,
                                                       dest_vertex_id );
...
void stalker_movement_manager_obstacles::remove_border(
    const obstacles_query& query ) {
...
bool stalker_movement_manager_obstacles::can_build_restricted_path(
    const obstacles_query& query ) {
    ...
    m_failed_to_build_path = !ai().graph_engine().search(
        ai().level_graph(), object().ai_location().level_vertex_id(),
        level_path().dest_vertex_id(), &m_temp_path,
...
void stalker_movement_manager_obstacles::move_along_path_impl(
...
#endif // MASTER_GOLD
    {
        ...
        if ( !m_dynamic_obstacles.movement_enabled() ) {
            ...
            inherited::move_along_path( movement_control, dest_position,
                                        time_delta );
...
void stalker_movement_manager_obstacles::move_along_path(
    ...
    if ( !ai().doors().actualize_doors_state( *m_doors_actor,
                                              old_desirable_speed() ) ) {
        //		Msg
        //( "%6d stalker %s waits for the some door to be open/closed",
        //Device.dwTimeGlobal, object().cName().c_str() );
        float desirable_speed = old_desirable_speed();
        set_desirable_speed( 0.f );
    ...
    //	Msg								( "%6d
    //stalker %s is going", Device.dwTimeGlobal, object().cName().c_str() );
...
static float get_distance( Fvector const& a_first,
    ...
    switch ( ai().level_graph().intersect(
        a_first.x, a_first.z, a_second.x, a_second.z, b_first.x, b_first.z,
        b_second.x, b_second.z, &intersection.x, &intersection.y ) ) {
        case LevelGraph::eLineIntersectionEqual:
            return 0.f;

        case LevelGraph::eLineIntersectionCollinear: {
            Fvector2 const as_af =
                Fvector2().sub( Fvector2().set( a_second.x, a_second.z ),
                                Fvector2().set( a_first.x, a_first.z ) );
            Fvector2 const bf_af =
                Fvector2().sub( Fvector2().set( b_first.x, b_first.z ),
                                Fvector2().set( a_first.x, a_first.z ) );
            ...
            float const distance =
                _sqrt( _sqr( bf_af_magnitude ) + _sqr( signed_distance ) );
        ...
        case LevelGraph::eLineIntersectionIntersect:
            return intersection.distance_to(
                Fvector2().set( a_first.x, a_first.z ) );
...
float stalker_movement_manager_obstacles::is_going_through(
    ...
    for ( ; i != e; ++i ) {
        float const distance =
            get_distance( ( *( i - 1 ) ).position, ( *i ).position,
                          start_position, stop_position, .35f );
        min_distance =
            distance > -1.f ? std::min( min_distance, distance ) : min_distance;
