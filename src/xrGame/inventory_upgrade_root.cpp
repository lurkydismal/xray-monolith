...
void Root::construct( const shared_str& root_id, Manager& manager_r ) {
    ...
    LPCSTR upgrade_scheme_str =
        pSettings->r_string( root_id, "upgrade_scheme" );
    VERIFY2( upgrade_scheme_str,
             make_string( "In inventory item <%s> `upgrade_scheme` is empty!",
                          root_id.c_str() ) );
...
void Root::add_upgrade( Upgrade* upgr ) {
    ...
    VERIFY2( verify_scheme_index( scheme_index ),
             make_string( "in upgrade <%s> for item <%s> scheme index [%d, %d] "
                          "is duplicated !",
                          upgr->id_str(), id_str(), scheme_index.x,
                          scheme_index.y ) );
...
void Root::test_all_upgrades( CInventoryItem& item ) {
    ...
    for ( ; ib != ie; ++ib ) {
        ...
        if ( g_upgrades_log == 1 ) {
            Msg( "# Checking upgrade <%s> (id = %d) is successful: %s ",
                 ( *ib )->section(), item.object_id(), res ? "OK" : "FAILED" );
        }
    }
}
