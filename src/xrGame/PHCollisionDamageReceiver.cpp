...
void CPHCollisionDamageReceiver::Init() {
    ...
    if ( ini->section_exist( "collision_damage" ) ) {
        ...
        for ( CInifile::SectCIt I = data.Data.begin(); I != data.Data.end();
            ...
            // R_ASSERT3(og, "collision damage bone has no physics collision",
            // *item.first);
            if ( og )
...
void CPHCollisionDamageReceiver::CollisionHit( u16 source_id,
                                               u16 bone_id,
                                               float power,
                                               const Fvector& dir,
                                               Fvector& pos ) {
...
void CPHCollisionDamageReceiver::Clear() {
    ...
    // xr_map<u16,float>::iterator
    // i=m_controled_bones.begin(),e=m_controled_bones.end(); for(;e!=i;++i)
    //{
