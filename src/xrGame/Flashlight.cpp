...
BOOL CFlashlight::net_Spawn( CSE_Abstract* DC ) {
    ...
    Fcolor clr =
        pSettings->r_fcolor( m_light_section, ( b_r2 ) ? "color_r2" : "color" );
    def_color = clr;
    fBrightness = clr.intensity();
    float range =
        pSettings->r_float( m_light_section, ( b_r2 ) ? "range_r2" : "range" );
    light_render->set_color( clr );
    light_render->set_range( range );
    light_render->set_hud_mode( READ_IF_EXISTS(
        pSettings, r_bool, m_light_section, "hud_mode", false ) );

    Fcolor clr_o = pSettings->r_fcolor(
        m_light_section, ( b_r2 ) ? "omni_color_r2" : "omni_color" );
    float range_o = pSettings->r_float(
        m_light_section, ( b_r2 ) ? "omni_range_r2" : "omni_range" );
    light_omni->set_color( clr_o );
    light_omni->set_range( range_o );
    light_omni->set_hud_mode( READ_IF_EXISTS(
        pSettings, r_bool, m_light_section, "hud_mode", false ) );

    light_render->set_cone(
        deg2rad( pSettings->r_float( m_light_section, "spot_angle" ) ) );
    light_render->set_texture( READ_IF_EXISTS(
        pSettings, r_string, m_light_section, "spot_texture", ( 0 ) ) );

    glow_render->set_texture(
        pSettings->r_string( m_light_section, "glow_texture" ) );
    glow_render->set_color( clr );
    glow_render->set_radius(
        pSettings->r_float( m_light_section, "glow_radius" ) );

    light_render->set_volumetric( !!READ_IF_EXISTS(
        pSettings, r_bool, m_light_section, "volumetric", 0 ) );
    light_render->set_volumetric_quality( READ_IF_EXISTS(
        pSettings, r_float, m_light_section, "volumetric_quality", 1.f ) );
    light_render->set_volumetric_intensity( READ_IF_EXISTS(
        pSettings, r_float, m_light_section, "volumetric_intensity", 1.f ) );
    light_render->set_volumetric_distance( READ_IF_EXISTS(
        pSettings, r_float, m_light_section, "volumetric_distance", 1.f ) );
    light_render->set_type( ( IRender_Light::LT )( READ_IF_EXISTS(
        pSettings, r_u8, m_light_section, "type", 2 ) ) );
    light_omni->set_type( ( IRender_Light::LT )( READ_IF_EXISTS(
        pSettings, r_u8, m_light_section, "omni_type", 1 ) ) );
...
void CFlashlight::Load( LPCSTR section ) {
    ...
    light_trace_bone =
        READ_IF_EXISTS( pSettings, r_string, section, "light_trace_bone", "" );
...
void CFlashlight::UpdateCL() {
    ...
    if ( Actor()->cam_freelook != eflDisabled ) {
        dir.setHP( -angle_normalize_signed( Actor()->old_torso_yaw ),
                   dir.getP() > 0.f ? dir.getP() * .6f : dir.getP() * .8f );
        dir.lerp( dep.m_FireParticlesXForm.k, dir,
                  Actor()->freelook_cam_control );
    }
...
void CFlashlight::SetLanim( LPCSTR name,
                            bool bFlicker,
                            int flickerChance,
                            float flickerDelay,
                            float framerate ) {
...
void CFlashlight::OnStateSwitch( u32 S, u32 oldState ) {
    if ( S == eShowing ) {
        ...
        if ( HudAnimationExist( "anm_zoom_show" ) && need_zoom ) {
            PlayHUDMotion( "anm_zoom_show", FALSE, this, GetState(), 1.f, 0.f,
                           false );
            m_bZoomed = true;
        } else
