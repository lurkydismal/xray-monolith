...
CStalkerPropertyEvaluatorDangers::CStalkerPropertyEvaluatorDangers(
    CAI_Stalker* object,
...
CStalkerPropertyEvaluatorDangerInDirection::
    CStalkerPropertyEvaluatorDangerInDirection( CAI_Stalker* object,
                                                LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorDangerWithGrenade::
    CStalkerPropertyEvaluatorDangerWithGrenade( CAI_Stalker* object,
                                                LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorDangerWithGrenade::evaluate() {
    ...
    return ( CDangerObject::eDangerTypeGrenade ==
...
CStalkerPropertyEvaluatorDangerUnknownCoverActual::
    CStalkerPropertyEvaluatorDangerUnknownCoverActual( CAI_Stalker* object,
                                                       LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorDangerUnknownCoverActual::evaluate() {
    ...
    if ( !property( StalkerDecisionSpace::eWorldPropertyCoverReached ) &&
         object().movement().path_completed() )
        m_cover_selection_position = object().Position();

    if ( object().m_ce_best->selected() &&
    ...
    for ( ;; ) {
        ...
        point = ai().cover_manager().best_cover(
            m_cover_selection_position, 10.f, *object().m_ce_best,
            CStalkerMovementRestrictor( m_object, true, false ) );
        if ( !point ) {
            object().m_ce_best->setup( position, 10.f, 170.f, 10.f );
            point = ai().cover_manager().best_cover(
                m_cover_selection_position, 30.f, *object().m_ce_best,
                CStalkerMovementRestrictor( m_object, true, false ) );
        }
        ...
        if ( last_cover && point &&
             ( point->position().distance_to_sqr( last_cover->position() ) <=
               1.f ) ) {
            point = last_cover;
            result = true;
            break;
...
CStalkerPropertyEvaluatorDangerGrenadeExploded::
    CStalkerPropertyEvaluatorDangerGrenadeExploded( CAI_Stalker* object,
                                                    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
_value_type CStalkerPropertyEvaluatorDangerGrenadeExploded::evaluate() {
    ...
    if ( CDangerObject::eDangerTypeGrenade !=
         m_object->memory().danger().selected()->type() )
        return ( false );
...
CStalkerPropertyEvaluatorGrenadeToExplode::
    CStalkerPropertyEvaluatorGrenadeToExplode( CAI_Stalker* object,
                                               LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
...
CStalkerPropertyEvaluatorEnemyWounded::CStalkerPropertyEvaluatorEnemyWounded(
    CAI_Stalker* object,
    LPCSTR evaluator_name )
    : inherited( object ? object->lua_game_object() : 0, evaluator_name ) {}
