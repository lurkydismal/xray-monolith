...
void CSpaceRestrictionHolder::clear() {
    ...
    // т.к. они используют ref_counter. Если чистить простые, то в IntrusivePtr
    // остаются битые указатели что приводит к порче памяти

    for ( auto& [ _, SharedPtr ] : m_restrictions ) {
        if ( CSpaceRestrictionComposition* Obj =
                 smart_cast< CSpaceRestrictionComposition* >(
                     SharedPtr->m_object ) ) {
...
shared_str CSpaceRestrictionHolder::normalize_string(
    shared_str space_restrictors ) {
    u32 n = xr_strlen( space_restrictors );
    if ( !n )
        return ( "" );

    // 1. parse the string, copying to temp buffer with leading zeroes, storing
    // pointers in vector
    LPSTR* strings =
        ( LPSTR* )_alloca( MAX_RESTRICTION_PER_TYPE_COUNT * sizeof( LPSTR ) );
    ...
    for ( ; *I; ++I, ++i ) {
        ...
        *i = 0;
        VERIFY( u32( string_current - strings ) <
                MAX_RESTRICTION_PER_TYPE_COUNT );
        *string_current = j;
        ++string_current;
        j = i + 1;
    }
...
SpaceRestrictionHolder::CBaseRestrictionPtr
CSpaceRestrictionHolder::restriction( shared_str space_restrictors ) {
    ...
    CSpaceRestrictionBridge* bridge =
        xr_new< CSpaceRestrictionBridge >( composition );
...
void CSpaceRestrictionHolder::register_restrictor(
    ...
    if ( restrictor_type != RestrictionSpace::eDefaultRestrictorTypeNone ) {
        ...
        else if ( restrictor_type ==
                  RestrictionSpace::eDefaultRestrictorTypeIn )
        ...
        if ( xr_strlen( *temp ) && xr_strlen( space_restrictors ) )
            strconcat( sizeof( m_temp_string ), m_temp_string, **temp, ",",
                       *space_restrictors );
        else
            strconcat( sizeof( m_temp_string ), m_temp_string, **temp,
                       *space_restrictors );
    ...
    if ( I == m_restrictions.end() ) {
        CSpaceRestrictionBridge* bridge =
            xr_new< CSpaceRestrictionBridge >( shape );
        m_restrictions.insert( std::make_pair( space_restrictors, bridge ) );
        return;
    }
...
bool try_remove_string( shared_str& search_string,
                        const shared_str& string_to_search ) {
    bool found = false;
    string256 temp;
    string4096 temp1;
    *temp1 = 0;
    for ( int i = 0, j = 0, n = _GetItemCount( *search_string ); i < n;
          ++i, ++j ) {
        if ( xr_strcmp( string_to_search,
                        _GetItem( *search_string, i, temp ) ) ) {
            if ( j )
                xr_strcat( temp1, "," );
...
void CSpaceRestrictionHolder::unregister_restrictor(
    CSpaceRestrictor* space_restrictor ) {
    ...
    CSpaceRestrictionBase* composition =
        xr_new< CSpaceRestrictionComposition >( this, restrictor_id );
...
IC void CSpaceRestrictionHolder::collect_garbage() {
    ...
    for ( ; I != E; ) {
        if ( !( *I ).second->shape() && !( *I ).second->intrusive_ref_count() &&
             ( Device.dwTimeGlobal >=
