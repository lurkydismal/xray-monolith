...
static void on_door_destroy( doors_type& doors, door& door ) {
    doors_type::iterator const found =
        std::find( doors.begin(), doors.end(), &door );
    if ( found != doors.end() )
...
bool actor::need_update() const {
    ...
    // Using && can leave a single queue "orphaned", preventing initiator
    // release.
...
public:
    ...
    inline bool operator()( door* const door ) const {
        ...
        float distance_to_open_state =
            m_movement->is_going_through( matrix, open, m_danger_distance );
        if ( distance_to_open_state != -1.f )
            return false;

        float distance_to_closed_state =
            m_movement->is_going_through( matrix, closed, m_danger_distance );
        ...
        matrix.c = temp;
        float distance_to_diagonal =
            m_movement->is_going_through( matrix, diagonal, m_danger_distance );
...
void actor::process_doors( float const average_speed,
    ...
    processed_doors.erase(
        std::remove_if( processed_doors.begin(), processed_doors.end(),
                        passed_doors_predicate( movement, *this, stop_state,
                                                danger_distance ) ),
        processed_doors.end() );
    ...
    processed_doors.insert( processed_doors.end(), new_doors.begin(),
                            new_doors.end() );
    std::inplace_merge( processed_doors.begin(),
                        processed_doors.begin() + processed_count,
                        processed_doors.end() );
}
...
bool actor::add_new_door( float const average_speed,
                          door* const door,
...
#ifdef DEBUG
        if ( g_debug_doors )
            Msg( "actor[%s] is waiting for the locked door[%s]", get_name(),
                 door->get_name() );
#endif // #ifdef DEBUG
        return false;
    }
    ...
    if ( door->is_blocked( state ) ) {
        doors_type::iterator const i =
            std::find( locked_doors.begin(), locked_doors.end(), door );
        if ( i == locked_doors.end() ) {
#ifdef DEBUG
            if ( g_debug_doors )
                Msg( "actor[%s] is waiting for the door[%s] blocked by %s",
                     get_name(), door->get_name(),
                     door->get_initiators_ids().c_str() );
#endif // #ifdef DEBUG
            return false;
        }

        VERIFY( std::find( processed_doors.begin(), processed_doors.end(),
                           door ) == processed_doors.end() );
    ...
    if ( std::find( processed_doors.begin(), processed_doors.end(), door ) !=
         processed_doors.end() )
        return true;
    ...
    float distance_to_open_state =
        movement.is_going_through( matrix, open, danger_distance );
    distance_to_open_state =
        distance_to_open_state == -1.f ? flt_max : distance_to_open_state;

    float distance_to_closed_state =
        movement.is_going_through( matrix, closed, danger_distance );
    distance_to_closed_state =
        distance_to_closed_state == -1.f ? flt_max : distance_to_closed_state;

    Fvector temp;
    matrix.transform_tiny( temp, closed );
    matrix.c = temp;
    float distance_to_diagonal =
        movement.is_going_through( matrix, diagonal, danger_distance );
    distance_to_diagonal =
        distance_to_diagonal == -1.f ? flt_max : distance_to_diagonal;

    float const min_distance = std::min(
        distance_to_diagonal,
...
bool actor::update_doors( doors_type const& detected_doors,
    ...
    temp_doors_type new_doors_to_open(
        _alloca( detected_doors_count * sizeof( doors_type::value_type ) ),
        detected_doors_count );
    temp_doors_type new_doors_to_close(
        _alloca( detected_doors_count * sizeof( doors_type::value_type ) ),
        detected_doors_count );

    float const check_distance =
        average_speed * g_door_open_time + g_door_length;
    ...
    for ( ; i != e; ++i ) {
        ...
        float const distance_to_open = movement.is_going_through(
            matrix, ( *i )->get_vector( door_state_open ), check_distance );
        float const distance_to_closed = movement.is_going_through(
            matrix, ( *i )->get_vector( door_state_closed ), check_distance );
        if ( distance_to_open >= 0.f ) {
            if ( distance_to_closed >= 0.f )
                if ( distance_to_open < distance_to_closed ) {
                    if ( !add_new_door( average_speed, *i, m_open_doors,
                                        m_closed_doors, new_doors_to_open,
                                        door_state_open ) )
                        return false;
                } else {
                    if ( !add_new_door( average_speed, *i, m_closed_doors,
                                        m_open_doors, new_doors_to_close,
                                        door_state_closed ) )
                        return false;
                }
            else {
        ...
        if ( !add_new_door( average_speed, *i, m_open_doors, m_closed_doors,
                            new_doors_to_open, door_state_open ) )
            return false;
    }

    process_doors( average_speed, m_open_doors, new_doors_to_open,
                   door_state_open, door_state_closed );
    process_doors( average_speed, m_closed_doors, new_doors_to_close,
...
void actor::render() const {
    ...
    for ( ; i != e; ++i ) {
        ...
        renderer.draw_line( Fidentity, matrix.c, temp,
