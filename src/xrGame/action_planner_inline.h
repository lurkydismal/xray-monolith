...
#define TEMPLATE_SPECIALIZATION                                         \
    template < typename _object_type, bool _reverse_search,             \
               typename _world_operator, typename _condition_evaluator, \
               typename _world_operator_ptr,                            \
               typename _condition_evaluator_ptr >

#define CPlanner                                                    \
    CActionPlanner< _object_type, _reverse_search, _world_operator, \
                    _condition_evaluator, _world_operator_ptr,      \
                    _condition_evaluator_ptr >

TEMPLATE_SPECIALIZATION
IC CPlanner::CActionPlanner()
    : m_initialized( false ), m_loaded( false ), m_solving( false ) {
#ifdef LOG_ACTION
    m_use_log = false;
#endif
}
...
void CPlanner::update() {
    ...
    if ( m_use_log ) {
        if ( m_solution_changed ) {
            ...
            Msg( "%6d : Solution for object %s [%d vertices searched]",
                 Device.dwTimeGlobal, object_name(),
                 ai().graph_engine()
                     .solver_algorithm()
                     .data_storage()
                     .get_visited_node_count() );
    ...
    if ( m_failed ) {
        ...
        show();
        Msg( "!ERROR: there is no action sequence, which can transfer current "
             "world state to the target one action[%s] object[%s]",
    ...
    if ( bDbgAct && m_failed && current_action().m_action_name )
        Msg( "!ERROR: there is no action sequence, which can transfer current "
             "world state to the target one. action[%s]",
             current_action().m_action_name );
#endif
    ...
    // Alundaio: More detailed logging for executing action; Knowing the last
    // executing action before a crash can be very useful for debugging
    if ( bDbgAct == true )
...
TEMPLATE_SPECIALIZATION
IC typename CPlanner::COperator& CPlanner::action(
    const _action_id_type& action_id ) {
    return ( *get_operator( action_id ) );
}

TEMPLATE_SPECIALIZATION
IC typename CPlanner::CConditionEvaluator& CPlanner::evaluator(
    const _condition_type& evaluator_id ) {
...
IC typename CPlanner::_action_id_type CPlanner::current_action_id() const {
    VERIFY2( initialized(),
             make_string( "ERROR: action by id [%d] not initialized!",
                          m_current_action_id ) );
...
TEMPLATE_SPECIALIZATION
IC void CPlanner::add_condition( _world_operator* action,
                                 _condition_type condition_id,
                                 _value_type condition_value ) {
#ifdef DEBUG
    VERIFY2( !m_solving, make_string( "do not change preconditions during "
                                      "planner update, object %s, id[%d]",
...
TEMPLATE_SPECIALIZATION
IC void CPlanner::add_effect( _world_operator* action,
                              _condition_type condition_id,
                              _value_type condition_value ) {
#ifdef DEBUG
    VERIFY2(
...
LPCSTR CPlanner::property2string( const _condition_type& property_id ) {
    return ( evaluator( property_id )
                 .m_evaluator_name ); // itoa(property_id,m_temp_string,10));
}
...
TEMPLATE_SPECIALIZATION
IC void CPlanner::add_operator( const _edge_type& operator_id,
...
IC void CPlanner::show_current_world_state() {
    ...
    for ( ; I != E; ++I ) {
        ...
        char temp = '?';
        if ( ( J != current_state().conditions().end() ) &&
             ( ( *J ).condition() == ( *I ).first ) ) {
            temp = ( *J ).value() ? '+' : '-';
            Msg( "%5c : [%d][%s]", temp, ( *I ).first,
...
IC void CPlanner::show_target_world_state() {
    ...
    for ( ; I != E; ++I ) {
        xr_vector< COperatorCondition >::const_iterator J =
            std::lower_bound( target_state().conditions().begin(),
                              target_state().conditions().end(),
                              CWorldProperty( ( *I ).first, false ) );
        char temp = '?';
        if ( ( J != target_state().conditions().end() ) &&
             ( ( *J ).condition() == ( *I ).first ) ) {
            temp = ( *J ).value() ? '+' : '-';
            Msg( "%5c : [%d][%s]", temp, ( *I ).first,
                 property2string( ( *I ).first ) );
...
IC void CPlanner::show( LPCSTR offset ) {
    ...
    strconcat( sizeof( temp ), temp, offset, "    " );
    {
        ...
        EVALUATORS::const_iterator E = evaluators().end();
        for ( ; I != E; ++I )
            Msg( "%sevaluator   [%d][%s]", offset, ( *I ).first,
                 property2string( ( *I ).first ) );
    }
    {
        ...
        for ( ; I != E; ++I ) {
...

            {
                ...
                for ( ; i != e; ++i )
                    Msg( "%s	condition [%d][%s] = %s", offset,
                         ( *i ).condition(),
                         property2string( ( *i ).condition() ),
                         ( *i ).value() ? "TRUE" : "FALSE" );
            }
            {
                xr_vector< COperatorCondition >::const_iterator i =
