...
void awards_store::reset_awards() {
    ...
    for ( int aidx = 0; aidx < at_awards_count; ++aidx ) {
        m_awards_result.insert( std::make_pair(
            enum_awards_t( aidx ), award_data( u16( 0 ), u32( 0 ) ) ) );
    };
}
...
void awards_store::merge_sake_to_ltx_awards() {
    for ( all_awards_t::iterator i = m_ltx_awards_result.begin(),
                                 ie = m_ltx_awards_result.end();
          i != ie; ++i ) {
        all_awards_t::const_iterator tmp_awi = m_awards_result.find( i->first );
        if ( tmp_awi != m_awards_result.end() ) {
            u16 tmp_count =
                std::max( i->second.m_count, tmp_awi->second.m_count );
            u32 tmp_rdate = std::max( i->second.m_last_reward_date,
                                      tmp_awi->second.m_last_reward_date );
...
bool awards_store::is_sake_equal_to_file() const {
    ...
    for ( all_awards_t::const_iterator i = m_ltx_awards_result.begin(),
                                       ie = m_ltx_awards_result.end();
          i != ie; ++i ) {
        all_awards_t::const_iterator tmp_iter =
...
void awards_store::process_award( SAKEField* award_params ) {
    // enum_awards_t awid			=
    // get_award_by_stat_name(award_params[ap_award_id].mName); VERIFY(awid !=
    // at_awards_count);

    // int rdate_statid_real		= get_award_reward_date_stat(awid);
    // int rdate_statid_from_table =
    // ATLAS_GET_STAT(award_params[ap_award_rdate].mName);
    // VERIFY(rdate_statid_real == rdate_statid_from_table);

    // u16	awards_count			=
    // award_params[ap_award_id].mValue.mShort; u32 award_rdate
    // = award_params[ap_award_rdate].mValue.mInt; m_awards_result.insert
    // (std::make_pair(awid, award_data(awards_count, award_rdate)));
}

void awards_store::process_aw_out_response( SAKEGetMyRecordsOutput* tmp_out,
                                            int const out_fields_count ) {
    // VERIFY(tmp_out->mNumRecords <= 1);
    // //one raw if (tmp_out->mNumRecords == 0) 	return;
