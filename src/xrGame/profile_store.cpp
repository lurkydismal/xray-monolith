...
profile_store::profile_store( CGameSpy_Full* fullgs_obj )
    : m_dsigned_reader( stats_submitter::p_number,
...
void profile_store::set_current_profile( int profileId,
                                         char const* loginTicket ) {
    VERIFY( m_sake_obj );
    // m_sake_obj->SetProfile(profileId, loginTicket);
}

void profile_store::load_current_profile(
    store_operation_cb progress_indicator_cb,
    ...
    // MainMenu()->GetLoginMngr(); R_ASSERT(tmp_lmngr); gamespy_gp::profile
    // const * tmp_curr_prof	= tmp_lmngr->get_current_profile(); if
    // (!tmp_curr_prof)
    //{
...
void profile_store::load_profile( store_operation_cb progress_indicator_cb ) {
    ...
    if ( tmp_reader ) {
        ...
        if ( tmp_length ) {
            m_valid_ltx = m_dsigned_reader.load_and_verify(
                static_cast< u8* >( tmp_reader->pointer() ), tmp_length );
    ...
    if ( m_valid_ltx ) {
        ...
        // gamespy_gp::login_manager*	tmp_lmngr		=
        // MainMenu()->GetLoginMngr(); R_ASSERT(tmp_lmngr); gamespy_gp::profile
        // const * tmp_curr_prof	= tmp_lmngr->get_current_profile();
    ...
    merge_fields( m_best_scores_store->get_field_names(),
...
void profile_store::loaded_fields( bool const result, char const* err_descr ) {
    ...
    if ( m_valid_ltx ) {
        ...
        m_best_scores_store->load_best_scores_from_ltx(
            m_dsigned_reader.get_ltx() );
...
void __stdcall profile_store::onlylog_operation( bool const result,
                                                 char const* descr ) {
    if ( !result ) {
...
void __stdcall profile_store::onlylog_completion( bool const result,
...
void profile_store::check_sake_actuality() {
    if ( !m_awards_store->is_sake_equal_to_file() ||
        ...
        __time32_t last_submit_time =
            static_cast< __time32_t >( m_dsigned_reader.get_ltx().r_u32(
                profile_data_section, profile_last_submit_time ) );
        if ( ( current_time - last_submit_time ) >= actuality_update_time ) {
            // atlas_submit_queue* tmp_submit_queue =
            // MainMenu()->GetSubmitQueue(); VERIFY(tmp_submit_queue);
