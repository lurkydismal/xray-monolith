...
CObjectActionCommand::CObjectActionCommand( CInventoryItem* item,
                                            CAI_Stalker* owner,
...
CObjectActionShow::CObjectActionShow( CInventoryItem* item,
                                      CAI_Stalker* owner,
                                      CPropertyStorage* storage,
                                      LPCSTR action_name )
...
void CObjectActionShow::initialize() {
    ...
    CInventoryItem* slot_item =
        object().inventory().ItemFromSlot( m_item->BaseSlot() );
    if ( slot_item == m_item )
        return;
...
void CObjectActionShow::execute() {
    ...
    if ( object().inventory().ActiveItem() &&
         ( object().inventory().ActiveItem() == m_item ) )
        return;

    CHudItem* hud_item =
        smart_cast< CHudItem* >( object().inventory().ActiveItem() );
    if ( !hud_item ) {
        object().inventory().Slot( m_item->BaseSlot(), m_item );
...
CObjectActionHide::CObjectActionHide( CInventoryItem* item,
                                      CAI_Stalker* owner,
                                      CPropertyStorage* storage,
                                      LPCSTR action_name )
    : inherited( item, owner, storage, action_name ) {}
...
static bool try_advance_ammo( CWeapon const& weapon ) {
    ...
    for ( u8 i = 0; i < u8( weapon.m_ammoTypes.size() ); ++i ) {
        ...
        for ( TIItemContainer::iterator l_it = inventory.m_belt.begin();
              inventory.m_belt.end() != l_it; ++l_it ) {
        ...
        for ( TIItemContainer::iterator l_it = inventory.m_ruck.begin();
              inventory.m_ruck.end() != l_it; ++l_it ) {
            CWeaponAmmo* l_pAmmo = smart_cast< CWeaponAmmo* >( *l_it );
            if ( l_pAmmo && !xr_strcmp( l_pAmmo->cNameSect(), l_ammoType ) ) {
                if ( l_pAmmo->m_boxCurr < l_pAmmo->m_boxSize ) {
                    l_pAmmo->m_boxCurr = l_pAmmo->m_boxSize;
...
CObjectActionReload::CObjectActionReload( CInventoryItem* item,
                                          CAI_Stalker* owner,
                                          CPropertyStorage* storage,
...
void CObjectActionReload::initialize() {
    ...
    VERIFY( object().inventory().ActiveItem()->object().ID() ==
            m_item->object().ID() );
...
CObjectActionFire::CObjectActionFire( CInventoryItem* item,
                                      CAI_Stalker* owner,
                                      CPropertyStorage* storage,
                                      _condition_type type,
                                      LPCSTR action_name )
...
CObjectActionFireNoReload::CObjectActionFireNoReload( CInventoryItem* item,
                                                      CAI_Stalker* owner,
                                                      CPropertyStorage* storage,
                                                      _condition_type type,
                                                      LPCSTR action_name )
...
CObjectActionStrapping::CObjectActionStrapping( CInventoryItem* item,
                                                CAI_Stalker* owner,
                                                CPropertyStorage* storage,
                                                LPCSTR action_name )
    : inherited( item, owner, storage, action_name ) {
    ...
    if ( m_callback_removed ) {
        VERIFY( !object().animation().torso().callback(
            CStalkerAnimationPair::CALLBACK_ID(
                this, &CObjectActionStrapping::on_animation_end ) ) );
        return;
    }
...
void CObjectActionStrapping::initialize() {
    ...
    m_storage->set_property( ObjectHandlerSpace::eWorldPropertyStrapped2Idle,
                             true );
...
CObjectActionStrappingToIdle::~CObjectActionStrappingToIdle() {
    if ( m_callback_removed ) {
        VERIFY( !object().animation().torso().callback(
            CStalkerAnimationPair::CALLBACK_ID(
                this, &CObjectActionStrappingToIdle::on_animation_end ) ) );
        return;
    }
...
CObjectActionUnstrapping::CObjectActionUnstrapping( CInventoryItem* item,
                                                    CAI_Stalker* owner,
                                                    CPropertyStorage* storage,
                                                    LPCSTR action_name )
    : inherited( item, owner, storage, action_name ) {
...
CObjectActionUnstrapping::~CObjectActionUnstrapping() {
    if ( m_callback_removed ) {
        VERIFY( !object().animation().torso().callback(
            CStalkerAnimationPair::CALLBACK_ID(
                this, &CObjectActionUnstrapping::on_animation_end ) ) );
        return;
    }
...
void CObjectActionUnstrapping::on_animation_end() {
    ...
    m_storage->set_property( ObjectHandlerSpace::eWorldPropertyStrapped,
                             false );
...
CObjectActionUnstrappingToIdle::~CObjectActionUnstrappingToIdle() {
    if ( m_callback_removed ) {
        VERIFY( !object().animation().torso().callback(
            CStalkerAnimationPair::CALLBACK_ID(
                this, &CObjectActionUnstrappingToIdle::on_animation_end ) ) );
        return;
    }
...
CObjectActionQueueWait::CObjectActionQueueWait( CInventoryItem* item,
                                                CAI_Stalker* owner,
                                                CPropertyStorage* storage,
                                                _condition_type type,
                                                LPCSTR action_name )
...
void CObjectActionQueueWait::finalize() {
    ...
    // VERIFY						(m_item);
    // VERIFY
    // (object().inventory().ActiveItem()); VERIFY
...
CObjectActionSwitch::CObjectActionSwitch( CInventoryItem* item,
                                          CAI_Stalker* owner,
                                          CPropertyStorage* storage,
                                          _condition_type type,
                                          LPCSTR action_name )
...
CObjectActionDrop::CObjectActionDrop( CInventoryItem* item,
                                      CAI_Stalker* owner,
                                      CPropertyStorage* storage,
                                      LPCSTR action_name )
    : inherited( item, owner, storage, action_name ) {}
...
void CObjectActionDrop::initialize() {
    inherited::initialize();
    if ( !m_item || !m_item->object().H_Parent() ||
         ( m_object->ID() != m_item->object().H_Parent()->ID() ) )
        return;
...
CObjectActionAim::CObjectActionAim( CInventoryItem* item,
                                    CAI_Stalker* owner,
                                    CPropertyStorage* storage,
...
CObjectActionIdle::CObjectActionIdle( CInventoryItem* item,
                                      CAI_Stalker* owner,
                                      CPropertyStorage* storage,
                                      LPCSTR action_name )
    : inherited( item, owner, storage, action_name ) {}
...
void CObjectActionIdle::initialize() {
    ...
    if ( m_storage->property( ObjectHandlerSpace::eWorldPropertyUseEnough ) )
        object().CObjectHandler::set_goal( MonsterSpace::eObjectActionActivate,
                                           object().inventory().ActiveItem() );
    m_storage->set_property( ObjectHandlerSpace::eWorldPropertyUseEnough,
                             false );
}
...
CObjectActionIdleMissile::CObjectActionIdleMissile( CInventoryItem* item,
                                                    CAI_Stalker* owner,
                                                    CPropertyStorage* storage,
                                                    LPCSTR action_name )
    : inherited( item, owner, storage, action_name ) {}
...
void CObjectActionIdleMissile::initialize() {
    ...
    m_storage->set_property(
        object().planner().uid(
            m_item->object().ID(),
            ObjectHandlerSpace::eWorldPropertyThrowStarted ),
        false );
    m_storage->set_property(
        object().planner().uid( m_item->object().ID(),
                                ObjectHandlerSpace::eWorldPropertyThrowIdle ),
        false );
    m_storage->set_property(
        object().planner().uid( m_item->object().ID(),
                                ObjectHandlerSpace::eWorldPropertyFiring1 ),
        false );
}
...
CObjectActionThrowMissile::CObjectActionThrowMissile( CInventoryItem* item,
                                                      CAI_Stalker* owner,
                                                      CPropertyStorage* storage,
                                                      LPCSTR action_name )
    : inherited( item, owner, storage, action_name ) {}
...
void CObjectActionThrowMissile::initialize() {
    ...
    if ( distance > 45 ) {
        set_inertia_time( g_ai_grenade_throw_delay_base +
                          g_ai_grenade_throw_delay_step * 3 );
        return;
    }

    if ( distance > 30 ) {
        set_inertia_time( g_ai_grenade_throw_delay_base +
                          g_ai_grenade_throw_delay_step * 2 );
        return;
    }

    if ( distance > 15 ) {
        set_inertia_time( g_ai_grenade_throw_delay_base +
                          g_ai_grenade_throw_delay_step );
        return;
    }
