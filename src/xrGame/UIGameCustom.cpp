...
void CUIGameCustom::OnFrame() {
    ...
    for ( auto item : CustomStatics )
        item->Update();
    auto comparer = []( const StaticDrawableWrapper* s1,
                        const StaticDrawableWrapper* s2 ) {
...
StaticDrawableWrapper* CUIGameCustom::AddCustomStatic( const char* id,
                                                       bool singleInstance ) {
    if ( singleInstance ) {
        auto it = std::find_if( CustomStatics.begin(), CustomStatics.end(),
                                predicate_find_stat( id ) );
...
void CUIGameCustom::HideActorMenu() {
    ...
    if ( ai().script_engine().functor(
             "actor_menu_inventory.CUIActorMenu_OnHideActorMenu", funct1 ) )
        funct1();
    //-------------------------------
}
...
void CMapListHelper::LoadMapInfo( const char* cfgName,
                                  const xr_string& levelName,
                                  const char* levelVer /*= "1.0"*/ ) {
    CInifile levelCfg( cfgName );
    shared_str shLevelName =
        levelName.substr( 0, levelName.find( '\\' ) ).c_str();
    ...
    if ( levelCfg.section_exist( "map_usage" ) ) {
        ...
        for ( const CInifile::Item& kv :
              levelCfg.r_section( "map_usage" ).Data ) {
            ...
            if ( !suitableLevels ) {
                ...
                lastItem.m_game_type_id =
                    ParseStringToGameType( gameType.c_str() );
            ...
            if ( std::find( levelNames.begin(), levelNames.end(), levelDesc ) !=
                 levelNames.end() ) {
                Msg( "! duplicate map found [%s] [%s]", shLevelName.c_str(),
                     shLevelVer.c_str() );
            } else {
#ifndef MASTER_GOLD
                Msg( "added map [%s] [%s]", shLevelName.c_str(),
                     shLevelVer.c_str() );
#endif
...
void CMapListHelper::Load() {
    ...
    for ( CLocatorAPI::archive& arch : FS.m_archives ) {
        ...
        const char* levelVersion =
            arch.header->r_string( "header", "level_ver" );
...
const SGameTypeMaps& CMapListHelper::GetMapListFor(
    const shared_str& gameType ) {
    if ( m_storage.size() == 0 )
        Load();
