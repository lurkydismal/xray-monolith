...
CEntityConditionSimple* CEntity::create_entity_condition(
...
// s16 element,Fvector position_in_object_space, float impulse, ALife::EHitType
// hit_type)
...
#pragma todo( "Jim to Dima: no specific figures or comments needed" )
    ...
    m_dwBodyRemoveTime = READ_IF_EXISTS( pSettings, r_u32, section,
                                         "body_remove_time", BODY_REMOVE_TIME );
    //////////////////////////////////////
}
...
BOOL CEntity::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( E ) {
        ...
        // R_ASSERT2(!((E->get_killer_id() != ALife::_OBJECT_ID(-1)) &&
        // g_Alive()),
        ...
        if ( ( m_killer_id != ALife::_OBJECT_ID( -1 ) ) && g_Alive() ) {
            Msg( "!server entity [%s][%d] has a killer [%d] and is not dead",
    ...
    if ( !E ) {
        ...
        R_ASSERT2(
            C || T || H,
            "Invalid entity (no inheritance from CSE_CreatureAbstract, "
            "CSE_ALifeItemCar and CSE_ALifeTrader and CSE_ALifeHelicopter)!" );
        ...
        CSE_ALifeMonsterBase* monster =
            smart_cast< CSE_ALifeMonsterBase* >( E );
        if ( monster ) {
            MONSTER_COMMUNITY monster_community;
            monster_community.set(
                pSettings->r_string( *cNameSect(), "species" ) );
    ...
    if ( ini ) {
        if ( ini->section_exist( "damage_section" ) &&
            ...
            CDamageManager::reload(
                pSettings->r_string( "damage_section", "damage" ), ini );
...
void CEntity::KillEntity(
    u16 whoID,
    BOOL bypass_actor_check /*AVO: added for actor_before_death callback*/ ) {
    // AVO: allow scripts to process actor condition and prevent actor's death
    // or kill him if desired. IMPORTANT: if you wish to kill actor you need to
    // call db.actor:kill(level:object_by_id(whoID), true) in actor_before_death
    // callback, to ensure all objects are properly destroyed
    ...
    if ( whoID != ID() ) {
        ...
        if ( m_killer_id != ALife::_OBJECT_ID( -1 ) ) {
            Msg( "! Entity [%s][%s] already has killer with id %d, but new "
                 "killer id arrived - %d",
                 *cNameSect(), *cName(), m_killer_id, whoID );

            CObject* old_killer = Level().Objects.net_Find( m_killer_id );
            Msg( "! Old killer is %s",
                 old_killer ? *old_killer->cName() : "unknown" );

            CObject* new_killer = Level().Objects.net_Find( whoID );
            Msg( "! New killer is %s",
                 new_killer ? *new_killer->cName() : "unknown" );
...
void CEntity::set_death_time() {
    m_level_death_time = Device.dwTimeGlobal;
    m_game_death_time = ai().get_alife()
                            ? ai().alife().time_manager().game_time()
                            : Level().GetGameTime();
}

bool CEntity::IsFocused() const {
    return ( smart_cast< const CEntity* >( g_pGameLevel->CurrentEntity() ) ==
             this );
}

bool CEntity::IsMyCamera() const {
    return ( smart_cast< const CEntity* >(
                 g_pGameLevel->CurrentViewEntity() ) == this );
}
...
float CEntity::GetHotness() {
    if ( heat_vision_cooldown && ( AlreadyDie() || !g_Alive() ) )
        return 1.0f - ( float )clampU( Device.dwTimeGlobal - m_level_death_time,
                                       0, heat_vision_cooldown_time ) /
                          heat_vision_cooldown_time;
    return 1.0f;
}
...
float CEntity::GetGlowing() {
    ...
    if ( invOwner ) {
        auto weapon =
            smart_cast< CWeapon* >( invOwner->inventory().ActiveItem() );
        if ( weapon ) {
