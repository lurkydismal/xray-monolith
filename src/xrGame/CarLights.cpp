...
void SCarLight::ParseDefinitions( LPCSTR section ) {
    ...
    IKinematics* pKinematics =
        smart_cast< IKinematics* >( m_holder->PCar()->Visual() );
    ...
    // lanim					=
    // LALib.FindItem(ini->r_string(section,"animator"));
}
...
void SCarLight::TurnOff() {
    ...
    light_render->set_active( false );
    smart_cast< IKinematics* >( m_holder->PCar()->Visual() )
        ->LL_SetBoneVisible( bone_id, FALSE, TRUE );
}
...
void SCarLight::Update() {
    ...
    CBoneInstance& BI = smart_cast< IKinematics* >( pcar->Visual() )
...
void CCarLights::ParseDefinitions() {
    CInifile* ini =
        smart_cast< IKinematics* >( m_pcar->Visual() )->LL_UserData();
