...
IC bool b_is_blending( const CBlend* current_blend, const CBlend* b ) {
    return current_blend &&
           ...
           current_blend != b && b->blendAmount < b->blendPower - EPS_L;
}
...
void ik_anim_state::update( IKinematicsAnimated* K, const CBlend* b, u16 i ) {
    ...
    if ( b_is_blending( current_blend, b ) ) {
        ...
        bool is_cur_step = ( m_def_cur.marks.size() > i ) &&
                           blend_in( *current_blend, m_def_cur.marks[ i ] );
        is_idle =
            !!( m_def_new.flags & esmIdle ) && !!( m_def_cur.flags & esmIdle );
        bool any_idle =
            ( m_def_cur.flags & esmIdle ) || ( m_def_new.flags & esmIdle );
...
bool ik_anim_state::time_step_begin( IKinematicsAnimated* K,
                                     const CBlend& B,
                                     u16 limb_id,
                                     float& time ) {
    time = 0;
