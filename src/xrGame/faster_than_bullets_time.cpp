...
faster_than_bullets_time::faster_than_bullets_time(
...
struct last_hits_fetcher {
    ...
    last_hits_fetcher( shared_str const& killer_name,
                       shared_str const& victim_name )
...
void faster_than_bullets_time::OnPlayerKilled(
    ...
    last_hits_fetcher tmp_predicate( victim_obj->cName(),
                                     tmp_local_player->getName() );
    buffer_vector< hits_store::bullet_hit > tmp_hits_result( NULL, 0 );

    if ( m_owner->get_hits_store().fetch_hits( tmp_predicate,
                                               tmp_hits_result ) ) {
