...
void rat_state_free_active::execute() {
    ...
    if ( object().switch_if_enemy() &&
         ( !object().switch_if_porsuit() || !object().switch_if_home() ) ) {
...
void rat_state_attack_range::execute() {
    ...
    if ( !object().m_attack_rebuild && !object().can_stand_in_position() ||
         object().switch_if_position() || object().switch_if_diff() ) {
        object().m_state_manager->push_state( CAI_Rat::aiRatNoWay );
        return;
    }
...
void rat_state_attack_melee::execute() {
    ...
    if ( squad && ( ( squad->GetLeader() != object().cast_entity() &&
    ...
    if ( object().switch_if_enemy() ) {
        ...
        if ( object().get_if_dw_time() ) {
            if ( object().get_if_tp_entity() ) {
                object().m_state_manager->push_state(
                    CAI_Rat::aiRatAttackMelee );
                return;
            }
...
void rat_state_return_home::execute() {
    ...
    if ( !object().switch_if_enemy() || object().switch_if_home() ||
         !object().switch_if_alife() ) {
        object().m_state_manager->pop_state();
        return;
    }
    object().set_home_pos();
...
void rat_state_no_way::execute() {
    ...
    if ( !object().m_walk_on_way ) {
        ...
        object().set_movement_type( !!object().m_bWayCanAdjustSpeed,
                                    !!object().m_bWayStraightForward );
