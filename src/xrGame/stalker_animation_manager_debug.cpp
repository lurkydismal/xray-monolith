...
// IC	bool shared_str_predicate	(const shared_str &_1, const shared_str
// &_2)
//{
...
struct animation_stats {
    ...
    IC animation_stats( const shared_str& visual_id,
...
typedef xr_map< ANIMATION_ID, animation_stats, animation_id_predicate >
    ANIMATION_STATS;
...
struct blend_id_predicate {
    ...
    template < typename T >
    IC bool less( const std::pair< T, T >& _1,
                  const std::pair< T, T >& _2 ) const {
...
void show_animations() {
    ...
    const ANIMATION_STATS_PAIR** const animations =
        ( ANIMATION_STATS_PAIR const** )_alloca(
            animation_count * sizeof( ANIMATION_STATS_PAIR* ) );
    ...
    struct predicate {
        static IC bool frame_count( const ANIMATION_STATS_PAIR* const& _1,
                                    const ANIMATION_STATS_PAIR* const& _2 ) {
    ...
    for ( i = animations; i != e; ++i )
        Msg( "%6d %6d %-32s %s", ( *i )->second.m_frame_count,
             ( *i )->second.m_start_count, *( *i )->first.first,
             *( *i )->first.second );
}

void show_blends() {
    u32 blend_count = g_blend_stats.size();
    const BLEND_STATS_PAIR** const blends = ( BLEND_STATS_PAIR const** )_alloca(
        blend_count * sizeof( BLEND_STATS_PAIR* ) );
    ...
    struct predicate {
        static IC bool blend_count( const BLEND_STATS_PAIR* const& _1,
                                    const BLEND_STATS_PAIR* const& _2 ) {
    ...
    Msg( "       animation_set1                                  animation1    "
         "count     animation2                                  "
         "animation_set2" );
    for ( i = blends; i != e; ++i )
        Msg( "%-32s %32s ->[%6d]-> %-32s %32s", *( *i )->first.second.second,
             *( *i )->first.second.first, ( *i )->second,
...
void add_animation( const shared_str& animation_id,
                    const shared_str& animation_set_id,
    ...
    g_animation_stats.insert( std::make_pair(
        query, animation_stats( visual_id, 1, just_started ? 1 : 0 ) ) );
}

void add_blend( const shared_str& animation_id,
                const shared_str& animation_set_id,
                const shared_str& visual_id,
    ...
    BLEND_ID query =
        std::make_pair( std::make_pair( animation_id, animation_set_id ),
                        std::make_pair( shared_str( blend_id->first ),
...
void add_animation_stats( const shared_str& animation_id,
                          const shared_str& animation_set_id,
                          const shared_str& visual_id,
                          const std::pair< LPCSTR, LPCSTR >* blend_id,
                          bool just_started ) {
...
void CStalkerAnimationManager::add_animation_stats(
    ...
    const BLEND_ID* blend_id,
    bool just_started ) {
    ::add_animation_stats( animation_id.first, animation_id.second,
                           *object().Visual()->getDebugName(), blend_id,
                           just_started );
}
...
void CStalkerAnimationManager::add_animation_stats() {
    ...
    if ( script().animation() ) {
        add_animation_stats(
            m_skeleton_animated->LL_MotionDefName_dbg( script().animation() ),
            script().blend_id( m_skeleton_animated, blend ),
    ...
    if ( global().animation() ) {
        add_animation_stats(
            m_skeleton_animated->LL_MotionDefName_dbg( global().animation() ),
            global().blend_id( m_skeleton_animated, blend ),
    ...
    add_animation_stats(
        m_skeleton_animated->LL_MotionDefName_dbg( torso().animation() ),
        torso().blend_id( m_skeleton_animated, blend ),
        torso().m_just_started );
    add_animation_stats(
        m_skeleton_animated->LL_MotionDefName_dbg( legs().animation() ),
