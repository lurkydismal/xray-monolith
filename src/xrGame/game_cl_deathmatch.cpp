...
void game_cl_Deathmatch::Init() {
    ...
    if ( pSettings->line_exist( "deathmatch_gamedata", "actor_spawn_effect" ) )
        Actor_Spawn_Effect =
            pSettings->r_string( "deathmatch_gamedata", "actor_spawn_effect" );
}
...
void game_cl_Deathmatch::net_import_state( NET_Packet& P ) {
    ...
    switch ( Phase() ) {
        case GAME_PHASE_PLAYER_SCORES: {
            ...
            if ( NeedSndMessage && local_player &&
                 !xr_strcmp( WinnerName, local_player->getName() ) ) {
...
void game_cl_Deathmatch::net_import_update( NET_Packet& P ) {
    ...
    if ( pCurBuyMenu && local_player ) {
        if ( local_player->rank != pCurBuyMenu->GetRank() &&
             !pCurBuyMenu->IsIgnoreMoneyAndRank() ) {
...
IBuyWnd* game_cl_Deathmatch::InitBuyMenu( const shared_str& BasePriceSection,
                                          s16 Team ) {
    if ( Team == -1 ) {
...
CUISkinSelectorWnd* game_cl_Deathmatch::InitSkinMenu( s16 Team ) {
    ...
    CUISkinSelectorWnd* pMenu = xr_new< CUISkinSelectorWnd >(
        ( char* )pTeamSect->caSection.c_str(), Team );
    return pMenu;
};
...
BOOL game_cl_Deathmatch::CanCallInventoryMenu() {
    if ( Phase() != GAME_PHASE_INPROGRESS )
        return false;
    if ( Level().CurrentEntity() &&
         !smart_cast< CActor* >( Level().CurrentEntity() ) ) {
        return FALSE;
    }
    if ( pCurSkinMenu && pCurSkinMenu->IsShown() ) {
        return FALSE;
...
void game_cl_Deathmatch::SetCurrentBuyMenu() {
    ...
    if ( !pCurBuyMenu ) {
        ...
        LoadTeamDefaultPresetItems( GetTeamMenu( 0 ), pCurBuyMenu,
                                    &PresetItemsTeam0 );
...
bool game_cl_Deathmatch::CanBeReady() {
    ...
    if ( pCurBuyMenu ) {
        ...
        Passed =
            ( _p.size() == 0 )
                ? 1
                : ( s32( pCurBuyMenu->GetPresetCost( _preset_idx_last ) ) <=
...
string16 places[] = { "1st",  "2nd",  "3rd",  "4th",  "5th",  "6th",  "7th",
                      "8th",  "9th",  "10th", "11th", "12th", "13th", "15th",
...
void game_cl_Deathmatch::shedule_Update( u32 dt ) {
    ...
    switch ( Phase() ) {
            ...
            if ( local_player && !local_player->IsSkip() ) {
                if ( m_bFirstRun ) {
                    ...
                    if ( !Level().IsDemoPlayStarted() &&
                        ...
                        VERIFY( m_game_ui );
                        m_bFirstRun =
                            m_game_ui->ShowServerInfo() ? FALSE : TRUE;
                    }
                ...
                if ( m_cl_dwWarmUp_Time > Level().timeServer() ) {
                    ...
                    if ( TimeRemains > 10000 )
                        strconcat( sizeof( tmpStr ), tmpStr,
                                   *st.translate( "mp_time2start" ), " ", S );
                    else {
                        if ( TimeRemains < 1000 )
                            strconcat( sizeof( tmpStr ), tmpStr,
                                       *st.translate( "mp_go" ), "" );
                            ...
                            if ( dwLastTimeRemains != dwCurTimeRemains ) {
                                if ( dwCurTimeRemains > 0 &&
                                     dwCurTimeRemains <= 5 )
                                    PlaySndMessage( ID_COUNTDOWN_1 +
                                                    dwCurTimeRemains - 1 );
                            }
                            dwLastTimeRemains = dwCurTimeRemains;
                            _itoa( dwCurTimeRemains, S, 10 );
                            strconcat( sizeof( tmpStr ), tmpStr,
                                       *st.translate( "mp_ready" ), "...", S );
                        }
                    };
                ...
                if ( Level().CurrentControlEntity() &&
                     smart_cast< CSpectator* >(
                         Level().CurrentControlEntity() ) &&
                     ( CurrentGameUI()->GameIndicatorsShown() ) ) {
                    CSpectator* pSpectator = smart_cast< CSpectator* >(
                        Level().CurrentControlEntity() );
                    if ( pSpectator ) {
                ...
                if ( IsVotingEnabled() && IsVotingActive() &&
                    ...
                    xr_sprintf( VoteTimeResStr,
                                st.translate( "mp_timeleft" ).c_str(),
                                MinitsLeft, SecsLeft,
                ...
                if ( local_player->testFlag(
                    ...
                    ConvertTime2String( &S, Rest );
                    string128 FullS;
                    xr_sprintf( FullS, "%s : %s",
                                *st.translate( "mp_time2respawn" ), S );
        ...
        case GAME_PHASE_PLAYER_SCORES: {
            ...
            string128 resstring;
            xr_sprintf( resstring, st.translate( "mp_player_wins" ).c_str(),
                        WinnerName );
    ...
    // cur_game_state!=GAME_PHASE_INPROGRESS)
    //{
...
bool game_cl_Deathmatch::OnKeyboardPress( int key ) {
    ...
    if ( kINVENTORY == key ) {
        if ( Level().CurrentControlEntity() &&
             smart_cast< CActor* >( Level().CurrentControlEntity() ) ) {
            if ( m_game_ui ) {
    ...
    /*
            if( kMAP == key)
            {
                    if (m_game_ui)
                    {
                            if (m_game_ui->m_pPdaMenu &&
       m_game_ui->m_pPdaMenu->IsShown())
...
void game_cl_Deathmatch::OnVoteStart( NET_Packet& P ) {
    ...
    if ( m_game_ui ) {
        ...
        sscanf( Command, "%s %s %s %s %s %s", CmdName, CmdParams[ 0 ],
                CmdParams[ 1 ], CmdParams[ 2 ], CmdParams[ 3 ],
                CmdParams[ 4 ] );
        ...
        if ( !xr_strcmp( CmdName, "restart" ) ) {
            ...
            xr_sprintf( NewCmd, "%s %s", *st.translate( "mp_kick" ),
                        CmdParams[ 0 ] );
            ...
            xr_sprintf( NewCmd, "%s %s", *st.translate( "mp_ban" ),
                        CmdParams[ 0 ] );
            for ( int i = 1; i < MAX_VOTE_PARAMS; i++ ) {
                if ( xr_strlen( CmdParams[ i ] ) ) {
                    xr_strcat( NewCmd, " " );
            ...
            xr_sprintf( NewCmd, "%s %s", *st.translate( "mp_change_map" ),
                        *st.translate( CmdParams[ 0 ] ) );
        } else if ( !xr_strcmp( CmdName, "changeweather" ) ) {
            xr_sprintf( NewCmd, "%s %s", *st.translate( "mp_change_weather" ),
                        *st.translate( CmdParams[ 0 ] ) );
        }

        string1024 VoteStr;
        xr_sprintf( VoteStr, *st.translate( "mp_voting_started" ), NewCmd,
                    Player );
...
void game_cl_Deathmatch::GetMapEntities(
...
void game_cl_Deathmatch::OnRender() {
    ...
    if ( m_bDamageBlockIndicators && local_player &&
        ...
        for ( ; it != players.end(); ++it ) {
            ...
            VERIFY( pActor );
            pActor->RenderIndicator( pTS->IndicatorPos, pTS->Indicator_r1,
...
IC bool DM_Compare_Players( game_PlayerState* p1, game_PlayerState* p2 ) {
    if ( p1->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) &&
         !p2->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) )
        return false;
    if ( !p1->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) &&
         p2->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) )
        return true;
...
void game_cl_Deathmatch::PlayParticleEffect( LPCSTR EffName, Fvector& pos ) {
    ...
    intrusive_ptr< CParticlesObject > ps =
        Particles::Details::Create( EffName, TRUE );
...
void game_cl_Deathmatch::OnGameRoundStarted() {
    ...
    if ( local_player ) {
        ...
        if ( pCurBuyMenu && pCurPresetItems ) {
            LoadTeamDefaultPresetItems( GetTeamMenu( local_player->team ),
                                        pCurBuyMenu, pCurPresetItems );
...
void game_cl_Deathmatch::LoadPlayerDefItems( const char* TeamName,
                                             IBuyWnd* pBuyMenu ) {
    if ( !local_player )
        return;
...
void game_cl_Deathmatch::OnPlayerFlagsChanged( game_PlayerState* ps ) {
    ...
    pActor->conditions().SetCanBeHarmedState(
...
void game_cl_Deathmatch::UpdateMapLocations() {
    ...
    if ( local_player ) {
        if ( !Level().MapManager().HasMapLocation( SELF_LOCATION,
                                                   local_player->GameID ) ) {
            ( Level().MapManager().AddMapLocation( SELF_LOCATION,
                                                   local_player->GameID ) )
                ->EnablePointer();
        }
    }
...
bool game_cl_Deathmatch::IsPlayerInTeam( game_PlayerState* ps, ETeam team ) {
    if ( ps->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) &&
         ( team == etSpectatorsTeam ) ) {
        return true;
    } else if ( !ps->testFlag( GAME_PLAYER_FLAG_SPECTATOR ) &&
                ( team == etGreenTeam ) ) {
        return true;
    }
    return false;
