...
bool CHelicopter::isObjectVisible( CObject* O ) {
    ...
    BOOL res = Level().ObjectSpace.RayTest(
        from_point, dir_to_object, ray_length, collide::rqtStatic, NULL, NULL );
...
void CHelicopter::UpdateHeliParticles() {
    ...
    if ( m_light_render->get_active() ) {
        ...
        if ( m_lanim ) {
            int frame;
            u32 clr = m_lanim->CalculateBGR(
                Device.fTimeGlobal, frame ); // тючтЁр•рхЄ т ЇюЁьрЄх BGR
            Fcolor fclr;
...
float CHelicopter::GetRealAltitude() {
    ...
    Level().ObjectSpace.RayPick( XFORM().c, down_dir, 1000.0f,
...
void CHelicopter::Hit( SHit* pHDS ) {
    ...
    if ( pHDS->who && ( smart_cast< CActor* >( pHDS->who ) ||
                        smart_cast< CAI_Stalker* >( pHDS->who ) ||
                        smart_cast< CCustomZone* >( pHDS->who ) ) ) {
        callback( GameObject::eHelicopterOnHit )(
            pHDS->damage(), pHDS->impulse, pHDS->hit_type, pHDS->who->ID() );
    }
...
void CollisionCallbackDead( bool& do_colide,
                            bool bo1,
                            dContact& c,
                            SGameMtl* material_1,
                            SGameMtl* material_2 ) {
    ...
    CHelicopter* l_this =
        bo1 ? smart_cast< CHelicopter* >(
                  PHRetrieveGeomUserData( c.geom.g1 )->ph_ref_object )
            : smart_cast< CHelicopter* >(
                  PHRetrieveGeomUserData( c.geom.g2 )->ph_ref_object );
...
void CHelicopter::DieHelicopter() {
    ...
    m_brokenSound.create( pSettings->r_string( *cNameSect(), "broken_snd" ),
    ...
    if ( true /*!PPhysicsShell()*/ ) {
        ...
        u16 bone_id;
        for ( u32 i = 0, n = _GetItemCount( *m_death_bones_to_hide ); i < n;
              ++i ) {
    ...
    if ( Device.dwTimeGlobal != PositionStack.front().dwTime )
        lin_vel.div( ( Device.dwTimeGlobal - PositionStack.front().dwTime ) /
                     1000.0f );
...
void CHelicopter::UseFireTrail( bool val ) {
    ...
    if ( val ) {
        fireDispersionBase =
            pSettings->r_float( *cNameSect(), "fire_dispersion_null" );
        fireDispersionBase = deg2rad( fireDispersionBase );
    } else {
        fireDispersionBase =
            pSettings->r_float( *cNameSect(), "fire_dispersion_base" );
        fireDispersionBase = deg2rad( fireDispersionBase );
    }
}
...
void SHeliBodyState::Load( LPCSTR section ) {
    ...
    model_angSpeedPitch =
        pSettings->r_float( section, "model_angular_sp_pitch" );
...
float t_xx( float V0, float V1, float a0, float a1, float d, float fSign ) {
    return ( V1 + _sqrt( V1 * V1 - ( a1 / ( a1 - a0 ) ) *
                                       ( V1 * V1 - V0 * V0 - 2 * a0 * d ) ) *
                      fSign ) /
           a1;
}
