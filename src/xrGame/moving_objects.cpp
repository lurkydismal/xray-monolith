...
void moving_objects::on_level_load() {
    xr_delete( m_tree );
    m_tree = xr_new< TREE >( ai().level_graph().header().box(),
...
void moving_objects::register_object( moving_object* moving_object ) {
    ...
    VERIFY2( m_objects.find( moving_object ) == m_objects.end(),
             make_string( "moving object %s is registers twice",
                          *moving_object->id() ) );
...
void moving_objects::unregister_object( moving_object* moving_object ) {
    ...
    VERIFY2( m_objects.find( moving_object ) != m_objects.end(),
             make_string(
                 "moving object %s is not yet registered or unregisters twice",
                 *moving_object->id() ) );
...
#ifdef DEBUG
    VERIFY2( m_objects.find( moving_object ) != m_objects.end(),
             make_string( "moving object %s is not yet registered",
                          *moving_object->id() ) );
#endif
