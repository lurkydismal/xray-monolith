...
#define TRANSLATE_TYPE( val )                                  \
    EEffectorPPType( val ? u32( u64( typeid( this ).name() ) ) \
                         : u32( u64( this ) & u32( -1 ) ) )
...
CPPEffectorCustom::CPPEffectorCustom( const SPPInfo& ppi,
                                      bool one_instance,
                                      bool destroy_from_engine )
    : inherited( TRANSLATE_TYPE( one_instance ),
                 flt_max,
                 destroy_from_engine ) {
    m_state = ppi;
    m_factor = 0.f;
...
CPPEffectorControlled::CPPEffectorControlled( CPPEffectorController* controller,
                                              const SPPInfo& ppi,
                                              bool one_instance,
                                              bool destroy_from_engine )
