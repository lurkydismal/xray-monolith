...
private:
    ...
    IC Fvector& get_local_vector( u16 bone,
                                  Fvector& v,
                                  const local_vector& lv ) const;
...
public:
    bool GetFootStepMatrix( ik_goal_matrix& m,
    ...
    void SetFootGeom( ik_foot_geom& fg,
                      const Fmatrix& ref_bone,
                      const Fmatrix& object_matrix ) const;
    void Collide( SIKCollideData& cld,
...
private:
    ik_goal_matrix::e_collide_state CollideFoot( float angle,
                                                 float& out_angle,
    ...
    ik_goal_matrix::e_collide_state rotate( Fmatrix& xm,
                                            const Fplane& p,
                                            const Fvector& normal,
                                            const Fvector& global_point,
                                            bool collide ) const;

    IC bool make_shift( Fmatrix& xm,
