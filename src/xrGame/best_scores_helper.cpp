...
void best_scores_helper::write_max() {
    ...
    m_max_knife_kills_in_row =
        std::max( m_max_knife_kills_in_row, m_knife_kills_in_row );
    m_max_backstab_kills_in_row =
        std::max( m_max_backstab_kills_in_row, m_backstab_kills_in_row );
    m_max_headshots_kills_in_row =
        std::max( m_max_headshots_kills_in_row, m_headshots_kills_in_row );
    m_max_eyeshots_kills_in_row =
        std::max( m_max_eyeshots_kills_in_row, m_eyeshots_kills_in_row );
    m_max_bleed_kills_in_row =
        std::max( m_max_bleed_kills_in_row, m_bleed_kills_in_row );
    m_max_explosive_kills_in_row =
        std::max( m_max_explosive_kills_in_row, m_explosive_kills_in_row );
}

void best_scores_helper::fill_best_results(
    gamespy_profile::all_best_scores_t& dest_br ) {
    using namespace gamespy_profile;
    write_max();
    dest_br.clear();
    dest_br.insert( std::make_pair( bst_kills_in_row, m_max_kills_in_row ) );
    dest_br.insert(
        std::make_pair( bst_kinife_kills_in_row, m_max_knife_kills_in_row ) );
    dest_br.insert(
        std::make_pair( bst_backstabs_in_row, m_max_backstab_kills_in_row ) );
    dest_br.insert(
        std::make_pair( bst_head_shots_in_row, m_max_headshots_kills_in_row ) );
    dest_br.insert(
        std::make_pair( bst_eye_kills_in_row, m_max_eyeshots_kills_in_row ) );
    dest_br.insert(
        std::make_pair( bst_bleed_kills_in_row, m_max_bleed_kills_in_row ) );
    dest_br.insert( std::make_pair( bst_explosive_kills_in_row,
                                    m_max_explosive_kills_in_row ) );
}
...
bool best_scores_helper::OnPlayerKilled(
    ...
    if ( m_player_state_accum->is_item_in_group(
             kill_weapon_id, ammunition_group::gid_hand_grenades ) ) {
