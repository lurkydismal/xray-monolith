...
void UITeamPanels::InitAllFrames( shared_str const& frame_node ) {
    int number_of_items =
        uiXml.GetNodesNum( uiXml.GetLocalRoot(), frame_node.c_str() );
    for ( int i = 0; i < number_of_items; ++i ) {
        XML_NODE* tempFrameNode = uiXml.NavigateToNode( frame_node.c_str(), i );
        if ( !tempFrameNode )
            break;
        LPCSTR frame_class = uiXml.ReadAttrib( tempFrameNode, "class",
                                               "class_of_frame_not_defined" );
        if ( !xr_strcmp( frame_class, "frame_line" ) ) {
            CUIFrameLineWnd* temp_frame_line = xr_new< CUIFrameLineWnd >();
            CUIXmlInit::InitFrameLine( uiXml, frame_node.c_str(), i,
                                       temp_frame_line );
...
void UITeamPanels::InitAllTeams( shared_str const& team_node ) {
    int numberOfTeams =
        uiXml.GetNodesNum( uiXml.GetLocalRoot(), team_node.c_str() );
    for ( int i = 0; i < numberOfTeams; ++i ) {
        XML_NODE* tempTeamNode = uiXml.NavigateToNode( team_node.c_str(), i );
        if ( !tempTeamNode )
            break;
        LPCSTR tempTeamName = uiXml.ReadAttrib(
            tempTeamNode, "tname", "team_not_set_in_tname_xml_attribute" );
        UITeamState* tempTeamPanel =
            panelsFactory.CreateTeamPanel( tempTeamName, this );
        VERIFY2(
            tempTeamPanel,
            make_string( "failed to create team panel \"%s\"", tempTeamName )
                .c_str() );
        tempTeamPanel->Init( uiXml, team_node.c_str(), i );
        tempTeamPanel->SetAutoDelete( true );
        AttachChild( tempTeamPanel );
        myPanels.insert(
            std::make_pair( shared_str( tempTeamName ), tempTeamPanel ) );
    }
}
...
void UITeamPanels::UpdateExistingPlayers() {
    ...
    for ( game_cl_GameState::PLAYERS_MAP_IT i = Game().players.begin(); i != ie;
          ++i ) {
