...
void CStalkerAnimationPair::synchronize(
    ...
    CMotionDef* motion1 =
        skeleton_animated->LL_GetMotionDef( stalker_animation.animation() );
    VERIFY( motion1 );
...
#endif
{
    ...
    for ( u16 i = 0; i < MAX_PARTS; ++i ) {
        ...
        CBlend* blend = 0;
        if ( !m_blend ) {
            blend = skeleton_animated->LL_PlayCycle(
                i, animation(), mix_animations ? TRUE : FALSE, callback,
                m_object );
            ...
            if ( use_animation_movement_control ||
                 this->use_animation_movement_control( skeleton_animated,
                                                       animation() ) ) {
                m_object->create_anim_mov_ctrl( blend, m_target_matrix,
                                                local_animation );
            } else {
            ...
            skeleton_animated->LL_PlayCycle(
                i, animation(), mix_animations ? TRUE : FALSE, 0, 0 );
    }
    // DBG_OpenCashedDraw();
    // DBG_DrawBones( *m_object );
    // DBG_ClosedCashedDraw( 50000 );
...
#ifndef USE_HEAD_BONE_PART_FAKE
void CStalkerAnimationPair::play( IKinematicsAnimated* skeleton_animated,
                                  PlayCallback callback,
                                  const bool& use_animation_movement_control,
                                  const bool& local_animation,
...
#endif
{
    ...
    if ( !global_animation() ) {
        ...
        if ( m_step_dependence && continue_interrupted_animation ) {
            ...
            if ( m_step_dependence && m_blend )
                pos = fmod( m_blend->timeCurrent, m_blend->timeTotal ) /
                      m_blend->timeTotal;
        }
        // DBG_OpenCashedDraw();
        // DBG_DrawBones( *m_object );
        // DBG_ClosedCashedDraw( 50000 );
        m_blend = skeleton_animated->PlayCycle( animation(), TRUE, callback,
                                                m_object );
...
#ifndef USE_HEAD_BONE_PART_FAKE
        play_global_animation( skeleton_animated, callback,
                               use_animation_movement_control, local_animation,
                               mix_animations );
#else
        play_global_animation( skeleton_animated, callback, bone_part,
                               use_animation_movement_control, local_animation,
                               mix_animations );
#endif
    m_actual = true;
    ...
    if ( psAI_Flags.is( aiAnimation ) ) {
        ...
        Msg( "%6d [%s][%s][%s][%d][%c][%c][%c][%f][%f][%f]",
             Device.dwTimeGlobal, m_object_name, m_animation_type_name, name,
             motion->bone_or_part,
             ( !( motion->flags & esmStopAtEnd ) ) ? '+' : '-',
             use_animation_movement_control ? '+' : '-',
             local_animation ? '+' : '-',
             VPUSH( ( m_target_matrix ? m_target_matrix->c
                                      : m_object->XFORM().c ) ) );
    }
#endif
}
...
std::pair< LPCSTR, LPCSTR >* CStalkerAnimationPair::blend_id(
    ...
    // const BlendSVec			&blends =
    // skeleton_animated->blend_cycle(bone_part_id);
    const u32 part_blends_num =
        skeleton_animated->LL_PartBlendsCount( bone_part_id );
...
void CStalkerAnimationPair::select_animation(
    const ANIM_VECTOR& array,
    const ANIMATION_WEIGHTS* weights ) {
    if ( !weights ) {
...
MotionID CStalkerAnimationPair::select( const ANIM_VECTOR& array,
                                        const ANIMATION_WEIGHTS* weights ) {
    VERIFY( !array.empty() );
...
void CStalkerAnimationPair::on_animation_end() {
    ...
    Callbacks callbacks(
        ...
        callback_count, m_callbacks.begin(), m_callbacks.end() );
...
void CStalkerAnimationPair::target_matrix( Fvector const& position,
