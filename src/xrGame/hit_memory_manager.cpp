...
const CHitObject* CHitMemoryManager::hit( const CEntityAlive* object ) const {
    VERIFY( m_hits );
    HITS::const_iterator I = std::find_if( m_hits->begin(), m_hits->end(),
                                           CHitObjectPredicate( object ) );
    if ( m_hits->end() != I )
        return ( &*I );
...
#endif
    m_max_hit_count =
        READ_IF_EXISTS( pSettings, r_s32, section, "DynamicHitCount", 1 );
}

void CHitMemoryManager::add( float amount,
                             const Fvector& vLocalDir,
                             const CObject* who,
                             s16 element ) {
    PROF_EVENT( "HitMemory::add" );
#ifndef MASTER_GOLD
    if ( who && smart_cast< CActor const* >( who ) &&
    ...
    object().callback( GameObject::eHit )(
        m_object->lua_game_object(), amount, vLocalDir,
    ...
    if ( !entity_alive || ( m_object->tfGetRelationType( entity_alive ) ==
                            ALife::eRelationTypeFriend ) )
        return;

    HITS::iterator J =
        std::find( m_hits->begin(), m_hits->end(), object_id( who ) );
    if ( m_hits->end() == J ) {
        CHitObject hit_object;

        hit_object.fill( entity_alive, m_object,
                         !m_stalker ? squad_mask_type( -1 )
        ...
        if ( m_max_hit_count <= m_hits->size() ) {
            HITS::iterator I =
                std::min_element( m_hits->begin(), m_hits->end(),
                                  SLevelTimePredicate< CEntityAlive >() );
            VERIFY( m_hits->end() != I );
            *I = hit_object;
        } else
...
void CHitMemoryManager::add( const CHitObject& _hit_object ) {
#ifndef MASTER_GOLD
    if ( _hit_object.m_object &&
         smart_cast< CActor const* >( _hit_object.m_object ) &&
         psAI_Flags.test( aiIgnoreActor ) )
        return;
#endif // MASTER_GOLD
    ...
    HITS::iterator J =
        std::find( m_hits->begin(), m_hits->end(), object_id( entity_alive ) );
    if ( m_hits->end() == J ) {
        if ( m_max_hit_count <= m_hits->size() ) {
            HITS::iterator I =
                std::min_element( m_hits->begin(), m_hits->end(),
        ...
        hit_object.m_squad_mask.assign( hit_object.m_squad_mask.get() |
                                        ( *J ).m_squad_mask.get() );
        *J = hit_object;
    }
}
...
struct CRemoveOfflinePredicate {
    bool operator()( const CHitObject& object ) const {
        ...
        return ( !object.m_object || !!object.m_object->getDestroy() ||
...
void CHitMemoryManager::enable( const CObject* object, bool enable ) {
    HITS::iterator J =
        std::find( m_hits->begin(), m_hits->end(), object_id( object ) );
    if ( J == m_hits->end() )
        return;
...
void CHitMemoryManager::remove_links( CObject* object ) {
    ...
    VERIFY( m_hits );
    HITS::iterator I = std::find_if( m_hits->begin(), m_hits->end(),
                                     CHitObjectPredicate( object ) );
    if ( I != m_hits->end() )
        m_hits->erase( I );
...
void CHitMemoryManager::load( IReader& packet ) {
    ...
    for ( int i = 0; i < count; ++i ) {
        ...
        object.m_object = smart_cast< CEntityAlive* >(
...
void CHitMemoryManager::on_requested_spawn( CObject* object ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        if ( m_object->g_Alive() ) {
            ( *I ).m_hit_object.m_object =
                smart_cast< CEntityAlive* >( object );
