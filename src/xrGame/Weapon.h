...
public:
    ...
    virtual void HUD_VisualBulletUpdate( bool force = false,
                                         int force_idx = -1 );
    ...
    virtual void SendHiddenItem(); // same as OnHiddenItem but for client...
...
public:
    ...
    virtual const CInventoryItem* can_kill(
...
protected:
    ...
    void InterpolateOffset( Fvector& current,
...
protected:
    ...
    float misfireStartCondition; // èçíîøåííîñòü, ïðè êîòîðîé ïîÿâëÿåòñÿ øàíñ
                                 // îñå÷êè
    float misfireEndCondition; // èçíîøåíîñòü ïðè êîòîðîé øàíñ îñå÷êè ñòàíîâèòñÿ
                               // êîíñòàíòíûì
...
public:
    ...
    // Setters
    void SetMisfireStartCondition( float val ) { misfireStartCondition = val; };

    void SetMisfireEndCondition( float val ) { misfireEndCondition = val; };
...
public:
    ...
    // Setters
    virtual void Set_PDM_Base( float val ) { m_pdm.m_fPDM_disp_base = val; };
...
protected:
    ...
    mutable u32
        m_BriefInfo_CalcFrame; // êàäð íà êîòîðîì ïðîñ÷èòàëè êîë-âî ïàòðîíîâ
...
protected:
    // This is because when scope is attached we can't ask scope for these
    // params therefore we should hold them by ourself :-((
...
public:
    float m_fLR_ShootingFactor; // Фактор горизонтального сдвига худа при
                                // стрельбе [-1; +1]
    float m_fUD_ShootingFactor; // Фактор вертикального сдвига худа при стрельбе
                                // [-1; +1]
    float m_fBACKW_ShootingFactor; // Фактор сдвига худа в сторону лица при
                                   // стрельбе [0; +1]
public:
