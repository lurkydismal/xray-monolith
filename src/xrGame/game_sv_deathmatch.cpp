...
void game_sv_Deathmatch::Create( shared_str& options ) {
    ...
    LoadTeams();
    m_not_free_ammo_str = READ_IF_EXISTS(
        pSettings, r_string, "deathmatch_gamedata", "not_free_ammo", "" );
...
void game_sv_Deathmatch::OnPlayerKillPlayer( game_PlayerState* ps_killer,
...
void game_sv_Deathmatch::Processing_Victim( game_PlayerState* pVictim,
...
KILL_RES game_sv_Deathmatch::GetKillResult( game_PlayerState* pKiller,
...
bool game_sv_Deathmatch::OnKillResult( KILL_RES KillResult,
                                       game_PlayerState* pKiller,
                                       game_PlayerState* pVictim ) {
    if ( !pKiller || !pVictim )
        return false;
...
void game_sv_Deathmatch::OnGiveBonus( KILL_RES KillResult,
                                      game_PlayerState* pKiller,
                                      game_PlayerState* pVictim,
                                      KILL_TYPE KillType,
                                      SPECIAL_KILL_TYPE SpecialKillType,
            ...
            if ( m_bSpectatorMode ) {
                ...
                if ( GameDM ) {
                    ...
                    if ( pObject && smart_cast< CActor* >( pObject ) ) {
                        string1024 Text;
                        xr_sprintf( Text, "Following %s",
                                    pObject->cName().c_str() );

                        GameDM->SetSpectrModeMsgCaption( Text );
                    } else
                        GameDM->SetSpectrModeMsgCaption(
...
bool game_sv_Deathmatch::checkForRoundStart() {
    if ( !Level().m_bGameConfigStarted ) // in case of starting server stage
                                         // (net_start 1..6) we can't do restart
                                         // ....
        return false;

    if ( m_bFastRestart || ( AllPlayers_Ready() ||
                             (
#ifdef DEBUG
                                 !g_sv_Wait_For_Players_Ready &&
#endif
                                 ( ( ( Level().timeServer() - StartTime() ) ) >
                                   u32( g_sv_Pending_Wait_Time ) ) ) ) ) {
...
bool game_sv_Deathmatch::checkForTimeLimit() {
    if ( m_dwWarmUp_CurTime != 0 || m_bInWarmUp )
        return false;
    if ( GetTimeLimit() && ( ( Level().timeServer() - StartTime() ) ) >
...
void game_sv_Deathmatch::SM_SwitchOnPlayer( CObject* pNewObject ) {
    ...
    if ( pActor ) {
        ...
        /*
                        CHudItem* pHudItem =
           smart_cast<CHudItem*>(pActor->inventory().ActiveItem()); if
           (pHudItem)
                        {
    ...
    switch ( m_phase ) {
        ...
        case GAME_PHASE_INPROGRESS: {
            xrClientData* xrCData =
                ( xrClientData* )m_server->ID_to_client( id );
            game_PlayerState* ps = get_id( id );
            if ( ps->IsSkip() )
                break;
            ...
            xrClientData* xrSCData =
                ( xrClientData* )m_server->GetServerClient();
...
void game_sv_Deathmatch::assign_RP( CSE_Abstract* E,
    ...
    R_ASSERT2( *std::max_element( m_vFreeRPoints[ Team ].begin(),
                                  m_vFreeRPoints[ Team ].end() ) < rp.size(),
    ...
    u32 NewPointID =
        ( HalfList )
            ? ( tmpPoints.size() - HalfList + ::Random.randI( HalfList ) )
            : 0;
    VERIFY2( NewPointID < tmpPoints.size(), "problem with random rpoints" );

    m_dwLastRPoints[ Team ] =
        m_vFreeRPoints[ Team ][ tmpPoints[ NewPointID ].PointID ];
    m_vFreeRPoints[ Team ].erase( m_vFreeRPoints[ Team ].begin() +
                                  tmpPoints[ NewPointID ].PointID );
...
void game_sv_Deathmatch::CheckItem( game_PlayerState* ps,
                                    PIItem pItem,
    ...
    if ( m_strWeaponsData->GetItemIdx( pItem->object().cNameSect() ) ==
         u32( -1 ) )
        return;
    //-------------------------------------------
    bool found = false;
    for ( u32 it = 0; it < pItemsDesired->size(); it++ ) {
        ...
        if ( ( ItemID & 0x00ff ) != u16( m_strWeaponsData->GetItemIdx(
        ...
        if ( pWeapon ) {
            ...
            u8 NewAddons =
                u8( ( ItemID & 0xff00 ) >> 0x08 ) /*u8(ItemID&0x00ff)>>0x05*/;
            if ( ExactMatch ) {
            ...
            if ( OldAddons != NewAddons ) {
                CSE_ALifeItemWeapon* pSWeapon =
                    smart_cast< CSE_ALifeItemWeapon* >(
                        get_entity_from_eid( pWeapon->ID() ) );
                if ( pSWeapon ) {
...
void game_sv_Deathmatch::OnPlayerBuyFinished( ClientID id_who, NET_Packet& P ) {
    ...
    VERIFY2( ps, make_string( "player state not found (ClientID = 0x%08x)",
                              id_who.value() )
                     .c_str() );
    CSE_ALifeCreatureActor* e_Actor = smart_cast< CSE_ALifeCreatureActor* >(
        get_entity_from_eid( ps->GameID ) );
    VERIFY2( e_Actor || ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ),
             make_string( "server entity of actor not found (GameID = 0x%08x)",
                          ps->GameID )
                 .c_str() );
...
//		Msg("------------- Player wants %d", ItemID);
    ...
    CSE_ALifeCreatureActor*		e_Actor	=
smart_cast<CSE_ALifeCreatureActor*>(get_entity_from_eid	(ps->GameID)); CActor*
pActor = smart_cast<CActor*>(Level().Objects.net_Find	(ps->GameID)); if
(pActor)
    {
            PIItem pItem = NULL;
            ...
            //проверяем пояс
            TIItemContainer::const_iterator	IBelt =
pActor->inventory().m_belt.begin(); TIItemContainer::const_iterator	EBelt =
pActor->inventory().m_belt.end();
...
ExactMatch);
            ...
            //проверяем ruck
            TIItemContainer::const_iterator	IRuck =
pActor->inventory().m_ruck.begin(); TIItemContainer::const_iterator	ERuck =
pActor->inventory().m_ruck.end();
...
ExactMatch);
            ...
            //проверяем слоты
            TISlotArr::const_iterator	ISlot =
pActor->inventory().m_slots.begin(); TISlotArr::const_iterator	ESlot =
pActor->inventory().m_slots.end();
...
void game_sv_Deathmatch::SpawnWeaponsForActor( CSE_Abstract* pE,
                                               game_PlayerState* ps ) {
...
#ifdef DEBUG
        Msg( "--- Server: spawning item [%d] for actor [%s]", ItemID,
             ps->getName() );
#endif // #ifdef DEBUG
...
void game_sv_Deathmatch::LoadSkinsForTeam( const shared_str& caSection,
...
void game_sv_Deathmatch::LoadDefItemsForTeam( const shared_str& caSection,
...
void game_sv_Deathmatch::SetSkin( CSE_Abstract* E, u16 Team, u16 ID ) {
    ...
    if ( !TeamList.empty() && TeamList.size() > Team &&
...
void game_sv_Deathmatch::OnPlayerHitPlayer_Case( game_PlayerState* ps_hitter,
...
void game_sv_Deathmatch::OnPlayerHitPlayer( u16 id_hitter,
                                            u16 id_hitted,
                                            NET_Packet& P ) {
...
void game_sv_Deathmatch::LoadTeams() {
    ...
    if ( !pSettings->section_exist( m_sBaseWeaponCostSection ) ) {
        R_ASSERT2(
            0, "No section for base weapon cost for this type of the Game!" );
        return;
    };
...
s32 game_sv_Deathmatch::GetMoneyAmount( const shared_str& caSection,
...
void game_sv_Deathmatch::OnDestroyObject( u16 eid_who ) {
    ...
    if ( entity && entity->owner->ps->GameID == eid_who ) {
        ...
        if ( Phase() == GAME_PHASE_INPROGRESS ) {
            CSE_ALifeCreatureActor* A =
                smart_cast< CSE_ALifeCreatureActor* >( entity );
            if ( A ) {
...
void game_sv_Deathmatch::OnTeamScore( u32 Team, bool Minor ) {
    struct team_score_money_adder {
        ...
        void operator()( IClient* client ) {
            ...
            if ( ps->team == s16( Team ) )
                m_owner->Player_AddMoney(
                    ps, ( ( Minor ) ? pTeam->m_iM_RoundWin_Minor
                                    : pTeam->m_iM_RoundWin ) );
            else
                m_owner->Player_AddMoney(
                    ps, ( ( Minor ) ? pTeam->m_iM_RoundLoose_Minor
...
void game_sv_Deathmatch::OnPlayerChangeSkin( ClientID id_who, s8 skin ) {
    ...
    if ( skin == -1 ) {
        ps_who->skin = u8(
            ::Random.randI( ( int )TeamList[ ps_who->team ].aSkins.size() ) );
    }
...
void game_sv_Deathmatch::LoadAnomalySets() {
    ...
    for ( u32 i = 0; i < 20; i++ ) {
        ...
        xr_strcpy( AnomaliesNames,
                   Level().pLevel->r_string( ASetBaseName, SetName ) );
    ...
    if ( Level().pLevel->line_exist( ASetBaseName, "permanent" ) ) {
        xr_strcpy( AnomaliesNames,
                   Level().pLevel->r_string( ASetBaseName, "permanent" ) );
        u32 count = _GetItemCount( AnomaliesNames );
        for ( u32 j = 0; j < count; j++ ) {
            _GetItem( AnomaliesNames, j, AnomalyName );
...
void game_sv_Deathmatch::Send_EventPack_for_AnomalySet( u32 AnomalySet,
                                                        u8 Event ) {
...
void game_sv_Deathmatch::StartAnomalies( int AnomalySet ) {
    ...
    if ( m_dwLastAnomalySetID < m_AnomalySetsList.size() ) {
        Send_EventPack_for_AnomalySet(
            m_dwLastAnomalySetID, CCustomZone::eZoneStateDisabled ); // Disable
    };
    ///////////////////////////////////////////////////
    ...
    if ( IsAnomaliesEnabled() )
        Send_EventPack_for_AnomalySet( m_dwLastAnomalySetID,
                                       CCustomZone::eZoneStateIdle ); // Idle
    ...
    if ( A ) {
        ...
        if ( W ) {
            ...
            for ( u32 it = 0; it < C.size(); ++it ) {
                ...
                if ( 0 == Et )
                    continue;
                CSE_ALifeItemWeapon* T =
                    smart_cast< CSE_ALifeItemWeapon* >( Et );
                if ( 0 == T )
                    continue;
                if ( slot == T->get_slot() ) {
                    if ( bForced ) {
        ...
        CSE_ALifeItemGrenade* pIGrenade =
            smart_cast< CSE_ALifeItemGrenade* >( e_what );
        if ( pIGrenade ) {
            // Grenade
            return TRUE;
        };

        CSE_ALifeItemCustomOutfit* pOutfit =
            smart_cast< CSE_ALifeItemCustomOutfit* >( e_what );
        if ( pOutfit ) {
            ...
            if ( e_what->ID_Parent == 0xffff ) {
                ...
                if ( !e_what->children.empty() ) {
                    ...
                    while ( !e_what->children.empty() ) {
                        CSE_Abstract* e_child_item =
                            get_entity_from_eid( e_what->children.back() );
                        if ( e_child_item ) {
                            if ( !OnTouch( eid_who, e_child_item->ID,
                                           FALSE ) ) {
                                NET_Packet P;
                                ...
                                m_server->Process_event_reject(
                                    P, m_server->GetServerClient()->ID, 0,
                                    e_what->ID, e_child_item->ID );
                                continue;
                            }
                        }
                        ...
                        EventPack.w( &PacketReject.B.data,
                                     PacketReject.B.count );
                ...
                if ( pKiller ) {
                    if ( g_sv_dm_bPDAHunt ) {
                        Player_AddBonusMoney(
                            pKiller,
                            READ_IF_EXISTS( pSettings, r_s32, "mp_bonus_money",
                                            "pda_taken", 0 ),
                            SKT_PDA );
                    };
                };
...
void game_sv_Deathmatch::OnDetach( u16 eid_who, u16 eid_what ) {
    ...
    CSE_ActorMP* actor =
        e_parent ? smart_cast< CSE_ActorMP* >( e_parent ) : NULL;
    ...
    if ( e_entity->m_tClassID == CLSID_OBJECT_PLAYERS_BAG && actor ) {
        ...
        for ( ; tr_it != tr_it_e; ++tr_it ) {
            m_server->Perform_transfer( PacketReject, PacketTake, *tr_it,
                                        e_parent, e_entity );
            EventPack.w_u8( u8( PacketReject.B.count ) );
...
void game_sv_Deathmatch::check_Player_for_Invincibility(
    game_PlayerState* ps ) {
    if ( !ps )
        return;
    u32 CurTime = Device.dwTimeGlobal;

    if ( ( ps->RespawnTime + GetDMBLimit() * 1000 < CurTime ) &&
         ps->testFlag( GAME_PLAYER_FLAG_INVINCIBLE ) ) {
...
bool game_sv_Deathmatch::check_for_Anomalies() {
    ...
    if ( m_dwLastAnomalySetID < 1000 ) {
        if ( GetAnomaliesTime() == 0 )
            return false;
        if ( m_dwLastAnomalyStartTime + GetAnomaliesTime() * 60000 >
             Level().timeServer() )
            return false;
    };
    StartAnomalies();
...
BOOL game_sv_Deathmatch::Is_Anomaly_InLists( CSE_Abstract* E ) {
    if ( !E )
        return FALSE;
    return TRUE;
    /*CSE_ALifeCustomZone* pCustomZone	=	smart_cast<CSE_ALifeCustomZone*>
    (E); if (pCustomZone)
    {
            if (pCustomZone->m_owner_id != 0xffffffff) return TRUE;
    }

    ANOMALIES_it It = std::find(m_AnomaliesPermanent.begin(),
    m_AnomaliesPermanent.end(),E->name_replace()); if (It !=
    m_AnomaliesPermanent.end())
    {
            return TRUE;
    };
    ...
    for (u32 j=0; j<m_AnomalySetsList.size(); j++)
    {
            ...
            ANOMALIES_it It = std::find(Anomalies->begin(),
    Anomalies->end(),E->name_replace()); if (It != Anomalies->end())
            {
                    return TRUE;
            };
    };
...
void game_sv_Deathmatch::OnPostCreate( u16 eid_who ) {
    ...
    CSE_ALifeCustomZone* pCustomZone =
        smart_cast< CSE_ALifeCustomZone* >( pEntity );
    if ( !pCustomZone || pCustomZone->m_owner_id != u32( -1 ) )
        return;

    for ( u32 j = 0; j < m_AnomalySetsList.size(); j++ ) {
        ANOMALIES* Anomalies = &( m_AnomalySetsList[ j ] );
        ANOMALIES_it It = std::find( Anomalies->begin(), Anomalies->end(),
                                     pCustomZone->name_replace() );
        if ( It != Anomalies->end() ) {
            ...
            P.w_u8(
                u8( CCustomZone::eZoneStateDisabled ) ); // eZoneStateDisabled
            u_EventSend( P );
            //-----------------------------------------------------------------------------
            return;
        };
    ...
    /*
    ANOMALIES_it It = std::find(m_AnomaliesPermanent.begin(),
    m_AnomaliesPermanent.end(),pCustomZone->name_replace()); if (It ==
    m_AnomaliesPermanent.end())
    {
            Msg("! Anomaly Not Found in any Set : %s",
    pCustomZone->name_replace());

            NET_Packet P;
            u_EventGen		(P,GE_ZONE_STATE_CHANGE,eid_who);
            P.w_u8			(u8(CCustomZone::eZoneStateDisabled));
    //eZoneStateDisabled u_EventSend(P);
    };*/
};
...
void game_sv_Deathmatch::Send_Anomaly_States( ClientID id_who ) {
    ...
    for ( u32 j = 0; j < m_AnomalyIDSetsList.size(); j++ ) {
        u8 AnomalyState = u8( ( m_dwLastAnomalySetID == j )
...
void game_sv_Deathmatch::ReadOptions( shared_str& options ) {
    ...
    //-------------------------------
    g_sv_dm_dwForceRespawn =
        get_option_i( *options, "frcrspwn", g_sv_dm_dwForceRespawn );
    g_sv_dm_dwFragLimit =
        get_option_i( *options, "fraglimit", g_sv_dm_dwFragLimit );
    g_sv_dm_dwTimeLimit =
        get_option_i( *options, "timelimit", g_sv_dm_dwTimeLimit ); // in (min)
    g_sv_dm_dwDamageBlockTime = get_option_i(
        *options, "dmgblock", g_sv_dm_dwDamageBlockTime ); // in (sec)
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
void game_sv_Deathmatch::OnRender() {
    ...
    !m_graph_engine->search(*m_level_graph,m_level_graph->vertex(u32(-1),v0),m_level_graph->vertex(u32(-1),v1),&xPath,GraphEngineSpace::CBaseParameters());
                    ...
                    for ( ; I != E; ++I) {
                            ...
                            {
                                    Fvector p0 =
    m_level_graph->vertex_position(*I); Fvector p1 =
    m_level_graph->vertex_position(*(I+1));
...
void game_sv_Deathmatch::on_death( CSE_Abstract* e_dest, CSE_Abstract* e_src ) {
    CSE_ALifeCreatureActor* pVictim =
        smart_cast< CSE_ALifeCreatureActor* >( e_dest );
    if ( !pVictim )
        return;
...
void game_sv_Deathmatch::WriteGameState( CInifile& ini,
                                         LPCSTR sect,
                                         bool bRoundResult ) {
...
void game_sv_Deathmatch::FillDeathActorRejectItems(
    CSE_ActorMP* actor,
    xr_vector< CSE_Abstract* >& to_reject ) {
    R_ASSERT( actor );
    CActor* pActor =
    ...
    if ( active_slot != NO_ACTIVE_SLOT ) {
        ...
        if ( !server_item ) {
#ifndef MASTER_GOLD
            Msg( "! ERROR: server entity is NULL, object_id = [%d]",
                 item->object_id() );
#endif // #ifndef MASTER_GOLD
            return;
        }
