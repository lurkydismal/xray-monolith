...
void character_hit_animation_controller::SetupHitMotions(
    IKinematicsAnimated& ca ) {
    // IKinematicsAnimated* ca =
...
*/
    ...
    base_bone = smart_cast< IKinematics* >( &ca )->LL_BoneID(
        "bip01_spine1" ); // bip01_spine1
...
IC void play_cycle( IKinematicsAnimated* CA,
                    const MotionID& m,
                    u8 channel,
                    CBlend*& blend_block,
                    float base_power ) {
    ...
    if ( !B ) {
        Msg( "! [character_hit_animations.cpp] play_cycle failed to play "
             "motion [m.slot %d: m.idx %d]",
             m.slot, m.idx );
        return;
    }
    B->blendAmount = power;
...
void character_hit_animation_controller::PlayHitMotion(
    ...
    if ( ph_dbg_draw_mask1.test( phDbgHitAnims ) ) {
        DBG_OpenCashedDraw();
        DBG_DrawLine( m.c, Fvector().sub( m.c, Fvector().mul( dir, 1.5 ) ),
    ...
    float rotational_ammount = hit_point.magnitude() * g_params.power_factor *
                               g_params.rotational_power_factor;
    //_abs(torqu.x)
...
bool character_hit_animation_controller::IsEffected( u16 bi,
                                                     IKinematics& ca ) const {
...
void character_hit_animation_controller::GetBaseMatrix(
    Fmatrix& m,
    CEntityAlive& ea ) const {
