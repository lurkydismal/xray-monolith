...
bool CLevel::PrepareToPlayDemo( shared_str const& file_name ) {
    ...
    if ( !m_reader ) {
        Msg( "ERROR: failed to open file [%s] to play demo...",
             file_name.c_str() );
        return false;
    }
    if ( !LoadDemoHeader() ) {
...
void CLevel::RestartPlayDemo() {
    ...
    if ( IsDemoPlayStarted() ) {
        ...
        // After remove_objects() invokation there where left a serveral (20)
        // UpdateCLs so:
#ifdef DEBUG
        VERIFY( g_pGameLevel );
...
bool CLevel::LoadPacket( NET_Packet& dest_packet, u32 global_time_delta ) {
    ...
    if ( map_data.m_sended_map_name_request
             ...
             : ( tmp_hdr.m_time_global_delta < global_time_delta ) ) {
        R_ASSERT2( tmp_hdr.m_packet_size < NET_PacketSizeLimit,
                   "bad demo packet" );
...
void CLevel::SpawnDemoSpectator() {
    ...
    CSE_Spectator* specentity =
        smart_cast< CSE_Spectator* >( tmp_sv_game->spawn_begin( "spectator" ) );
    R_ASSERT( specentity );
    R_ASSERT2( mp_cl_game->local_player, "player not spawned" );
    // mp_cl_game->local_player		= mp_cl_game->createPlayerState();
    // xr_strcpy
    // (mp_cl_game->local_player->name, "demo_spectator");
...
void CLevel::SetDemoPlayPos(float const pos)
{
        ...
        if (cur_pos >= pos)
        {
                Msg("! demo play position must be greater than current
position"); return;
        }

        u32 old_file_pos = m_reader->tell();

        u32				file_pos = u32(float(m_reader->length())
* pos); if (file_pos <= old_file_pos)
        {
...
void __stdcall CLevel::MSpawnsCatchCallback( u32 message,
                                             u32 subtype,
                                             NET_Packet& packet ) {
