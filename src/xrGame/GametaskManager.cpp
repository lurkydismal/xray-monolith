...
struct FindTaskByID {
    ...
    FindTaskByID( const shared_str& s, bool search_only_inprocess )
        : id( s ), b_only_inprocess( search_only_inprocess ) {}

    bool operator()( const SGameTaskKey& key ) {
        if ( b_only_inprocess )
            return ( id == key.task_id &&
                     key.game_task->GetTaskState() == eTaskStateInProgress );
        else
            return ( id == key.task_id );
    }
};
...
CGameTask* CGameTaskManager::HasGameTask( const shared_str& id,
                                          bool only_inprocess ) {
    FindTaskByID key( id, only_inprocess );
    vGameTasks_it it =
        std::find_if( GetGameTasks().begin(), GetGameTasks().end(), key );
...
CGameTask* CGameTaskManager::GiveGameTaskToActor( CGameTask* t,
                                                  u32 timeToComplete,
                                                  bool bCheckExisting,
                                                  u32 timer_ttl ) {
    ...
    if ( /* bCheckExisting &&*/ HasGameTask( t->m_ID, true ) ) {
        ...
        VERIFY2( 0, make_string( "give_task : Task [%s] already inprocess!",
                                 t->m_ID.c_str() ) );
        return NULL;
    }
    ...
    std::stable_sort( GetGameTasks().begin(), GetGameTasks().end(),
                      task_prio_pred );
...
void CGameTaskManager::UpdateTasks() {
...

    {
        ...
        Tasks tasks( _alloca( task_count * sizeof( SGameTaskKey ) ), task_count,
                     GetGameTasks().begin(), GetGameTasks().end() );
        ...
        for ( ; I != E; ++I ) {
            ...
            if ( ( state == eTaskStateFail ) ||
                 ( state == eTaskStateCompleted ) )
                SetTaskState( t, state );
        }
    }
...
void CGameTaskManager::RPC_UpdateTaskName() {
    ...
    if ( tr )
        snprintf( discord_gameinfo.task_name, 128,
                  xr_ToUTF8( *CStringTable().translate( tr->m_Title ) ) );
}
...
CGameTask* CGameTaskManager::HasGameTask( const CMapLocation* ml,
                                          bool only_inprocess ) {
...
CGameTask* CGameTaskManager::IterateGet( CGameTask* t,
                                         ETaskState state,
                                         bool bForward ) {
...
const char* sTaskStates[] = { "eTaskStateFail", "TaskStateInProgress",
                              "TaskStateCompleted", "TaskStateDummy" };
...
void CGameTaskManager::DumpTasks() {
    ...
    for ( ; it != it_e; ++it ) {
        ...
        Msg( " ID=[%s] state=[%s] prio=[%d] ", gt->m_ID.c_str(),
             sTaskStates[ gt->GetTaskState() ], gt->m_priority );
    }
}
