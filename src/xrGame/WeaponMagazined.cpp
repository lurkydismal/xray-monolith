...
CWeaponMagazined::CWeaponMagazined( ESoundTypes eSoundType ) : CWeapon() {
    ...
    m_eSoundReloadEmpty =
        ESoundTypes( SOUND_TYPE_WEAPON_RECHARGING | eSoundType );
...
CWeaponMagazined::~CWeaponMagazined() {
    ...
    Device.remove_from_seq_parallel(
        xr_make_delegate( this, &CWeaponMagazined::UpdateSoundsPositions ) );
}
...
bool CWeaponMagazined::WeaponSoundExist( LPCSTR section, LPCSTR sound_name ) {
    LPCSTR str;
    bool sec_exist = process_if_exists_set( section, sound_name,
                                            &CInifile::r_string, str, true );
    if ( sec_exist )
        return true;
    else {
#ifdef DEBUG
        Msg( "~ [WARNING] ------ Sound [%s] does not exist in [%s]", sound_name,
             section );
#endif
        return false;
    }
...
void CWeaponMagazined::Load( LPCSTR section ) {
    ...
    if ( WeaponSoundExist( section, "snd_shoot_actor" ) )
        m_sounds.LoadSound( section, "snd_shoot_actor", "sndShotActor", false,
                            m_eSoundShot );
    //-Alundaio
    // Cyclic fire sounds
    if ( WeaponSoundExist( section, "snd_shoot_actor_first" ) )
        m_sounds.LoadSound( section, "snd_shoot_actor_first",
                            "sndShotActorFirst", false, m_eSoundShot );

    // misfire shot
    if ( WeaponSoundExist( section, "snd_shot_misfire" ) )
        m_sounds.LoadSound( section, "snd_shot_misfire", "sndShotMisfire",
                            false, m_eSoundShot );
    if ( WeaponSoundExist( section, "snd_shot_misfire_actor" ) )
        m_sounds.LoadSound( section, "snd_shot_misfire_actor",
                            "sndShotMisfireActor", false, m_eSoundShot );

    m_sounds.LoadSound( section, "snd_empty", "sndEmptyClick", true,
                        m_eSoundEmptyClick );
    m_sounds.LoadSound( section, "snd_reload", "sndReload", true,
                        m_eSoundReload );

    if ( WeaponSoundExist( section, "snd_reload_empty" ) )
        m_sounds.LoadSound( section, "snd_reload_empty", "sndReloadEmpty", true,
                            m_eSoundReloadEmpty );
    if ( WeaponSoundExist( section, "snd_reload_misfire" ) )
        m_sounds.LoadSound( section, "snd_reload_misfire", "sndReloadMisfire",
                            true, m_eSoundReloadMisfire );
    if ( WeaponSoundExist( section, "snd_switch_mode" ) )
        m_sounds.LoadSound( section, "snd_switch_mode", "sndSwitchMode", true,
                            m_eSoundEmptyClick );
    if ( WeaponSoundExist( section, "snd_misfire" ) )
        m_sounds.LoadSound( section, "snd_misfire", "sndClickMisfire", true,
                            m_eSoundEmptyClick );

    if ( WeaponSoundExist( section, "snd_reload_actor" ) )
        m_sounds.LoadSound( section, "snd_reload_actor", "sndReloadActor", true,
                            m_eSoundReload );
    if ( WeaponSoundExist( section, "snd_reload_empty_actor" ) )
        m_sounds.LoadSound( section, "snd_reload_empty_actor",
                            "sndReloadEmptyActor", true, m_eSoundReloadEmpty );
    if ( WeaponSoundExist( section, "snd_reload_misfire_actor" ) )
        m_sounds.LoadSound( section, "snd_reload_misfire_actor",
                            "sndReloadMisfireActor", true,
                            m_eSoundReloadMisfire );
    if ( WeaponSoundExist( section, "snd_empty_actor" ) )
        m_sounds.LoadSound( section, "snd_empty_actor", "sndEmptyClickActor",
                            true, m_eSoundEmptyClick );
    if ( WeaponSoundExist( section, "snd_misfire_actor" ) )
        m_sounds.LoadSound( section, "snd_misfire_actor",
                            "sndClickMisfireActor", true, m_eSoundEmptyClick );

    if ( WeaponSoundExist( section, "snd_draw_actor" ) )
        m_sounds.LoadSound( section, "snd_draw_actor", "sndShowActor", true,
                            m_eSoundShow );
    if ( WeaponSoundExist( section, "snd_holster_actor" ) )
        m_sounds.LoadSound( section, "snd_holster_actor", "sndHideActor", true,
                            m_eSoundHide );
    ...
    if ( m_eSilencerStatus == ALife::eAddonAttachable ||
         m_eSilencerStatus == ALife::eAddonPermanent ) {
        if ( pSettings->line_exist( section, "silencer_flame_particles" ) )
            m_sSilencerFlameParticles =
                pSettings->r_string( section, "silencer_flame_particles" );
        if ( pSettings->line_exist( section, "silencer_smoke_particles" ) )
            m_sSilencerSmokeParticles =
                pSettings->r_string( section, "silencer_smoke_particles" );

        // Alundaio: LAYERED_SND_SHOOT Silencer
        m_sounds.LoadSound( section, "snd_silncer_shot", "sndSilencerShot",
                            false, m_eSoundShot );
        if ( WeaponSoundExist( section, "snd_silncer_shot_actor" ) )
            m_sounds.LoadSound( section, "snd_silncer_shot_actor",
                                "sndSilencerShotActor", false, m_eSoundShot );
        //-Alundaio

        // Cyclic fire sounds w/ silencer
        if ( WeaponSoundExist( section, "snd_silncer_shoot_actor_first" ) )
            m_sounds.LoadSound( section, "snd_silncer_shoot_actor_first",
                                "sndSilencerShotActorFirst", false,
                                m_eSoundShot );

        // misfire shot
        if ( WeaponSoundExist( section, "snd_silncer_shot_misfire" ) )
            m_sounds.LoadSound( section, "snd_silncer_shot_misfire",
                                "sndSilencerShotMisfire", false, m_eSoundShot );
        if ( WeaponSoundExist( section, "snd_silncer_shot_misfire_actor" ) )
            m_sounds.LoadSound( section, "snd_silncer_shot_misfire_actor",
                                "sndSilencerShotMisfireActor", false,
                                m_eSoundShot );
    }

    m_iBaseDispersionedBulletsCount = READ_IF_EXISTS(
        pSettings, r_u8, section, "base_dispersioned_bullets_count", 0 );
    m_fBaseDispersionedBulletsSpeed = READ_IF_EXISTS(
        pSettings, r_float, section, "base_dispersioned_bullets_speed",
        m_fStartBulletSpeed );
    ...
    if ( pSettings->line_exist( section, "fire_modes" ) ) {
        ...
        m_iPrefferedFireMode = READ_IF_EXISTS( pSettings, r_s16, section,
                                               "preffered_fire_mode", -1 );
    } else {
    ...
    empty_click_layer = READ_IF_EXISTS( pSettings, r_string, *hud_sect,
                                        "empty_click_anm", nullptr );

    if ( empty_click_layer ) {
        empty_click_speed = READ_IF_EXISTS( pSettings, r_float, *hud_sect,
                                            "empty_click_anm_speed", 1.f );
        empty_click_power = READ_IF_EXISTS( pSettings, r_float, *hud_sect,
                                            "empty_click_anm_power", 1.f );
    }
...
void CWeaponMagazined::FireStart() {
    if ( !IsMisfire() ) {
        ...
        if ( object )
            object->callback( GameObject::eOnWeaponJammed )(
                object->lua_game_object(), this->lua_game_object() );
#endif
        //-Alundaio

        if ( smart_cast< CActor* >( this->H_Parent() ) &&
             ( Level().CurrentViewEntity() == H_Parent() ) )
...
void CWeaponMagazined::FireEnd() {
    ...
    // Alundaio: Removed auto-reload since it's widely asked by just about
    // everyone who is a gun whore Lander: Reinstated as a cvar
    if ( g_auto_reload ) {
...
bool CWeaponMagazined::TryReload() {
    if ( m_pInventory ) {
        if ( IsGameTypeSingle() && ParentIsActor() ) {
            ...
            Actor()->callback( GameObject::eWeaponNoAmmoAvailable )(
                lua_game_object(), AC );
        }
        ...
        m_pCurrentAmmo = smart_cast< CWeaponAmmo* >(
            m_pInventory->GetAny( m_ammoTypes[ m_ammoType ].c_str() ) );
        ...
        if ( iAmmoElapsed == 0 )
            for ( u8 i = 0; i < u8( m_ammoTypes.size() ); ++i ) {
                m_pCurrentAmmo = smart_cast< CWeaponAmmo* >(
                    m_pInventory->GetAny( m_ammoTypes[ i ].c_str() ) );
                if ( m_pCurrentAmmo ) {
...
void CWeaponMagazined::OnMagazineEmpty() {
    ...
    if ( IsGameTypeSingle() && ParentIsActor() ) {
        int AC = GetSuitableAmmoTotal();
        Actor()->callback( GameObject::eOnWeaponMagazineEmpty )(
            lua_game_object(), AC );
    }
#endif
    if ( GetState() == eIdle ) {
...
int CWeaponMagazined::CheckAmmoBeforeReload( u8& v_ammoType ) {
    ...
    CWeaponAmmo* ammo =
        smart_cast< CWeaponAmmo* >( m_pInventory->GetAny( tmp_sect_name ) );

    if ( !ammo && !m_bLockType ) {
        for ( u8 i = 0; i < u8( m_ammoTypes.size() ); ++i ) {
            // проверить патроны всех подходящих типов
            ammo = smart_cast< CWeaponAmmo* >(
                m_pInventory->GetAny( m_ammoTypes[ i ].c_str() ) );
            if ( ammo ) {
                v_ammoType = i;
                break;
...
void CWeaponMagazined::UnloadMagazine( bool spawn_ammo ) {
    ...
    for ( l_it = l_ammo.begin(); l_ammo.end() != l_it; ++l_it ) {
        if ( m_pInventory ) {
            CWeaponAmmo* l_pA = smart_cast< CWeaponAmmo* >(
                m_pInventory->GetAny( l_it->first ) );
            if ( l_pA ) {
                u16 l_free = l_pA->m_boxSize - l_pA->m_boxCurr;
                l_pA->m_boxCurr =
                    l_pA->m_boxCurr +
                    ( l_free < l_it->second ? l_free : l_it->second );
                l_it->second =
                    l_it->second -
                    ( l_free < l_it->second ? l_free : l_it->second );
            }
...
void CWeaponMagazined::ReloadMagazine() {
    ...
    {
        CWeaponMagazinedWGrenade* wg =
            smart_cast< CWeaponMagazinedWGrenade* >( this );
        if ( wg && wg->IsGrenadeLauncherAttached() &&
             wg->GetGrenadeLauncherMode() ) {
            wg->UnloadRocket();
        }
...
void CWeaponMagazined::UpdateSounds() {
    ...
    if ( g_bootComplete && mt_UpdateWeaponSounds &&
         dwUpdateSounds_Frame != 0 ) {
        // Force update of fire dependencies and then put into second thread,
        // fixes flickering limbs
        get_LastFP();
        Device.seqParallel.push_back( xr_make_delegate(
            this, &CWeaponMagazined::UpdateSoundsPositions ) );
    } else {
...
// demonized: check if cycle_down is enabled and shot num below max possible
// burst. Adds support for arbitrary burst shot at rpm_mode_2 with cycling down
// to rpm after maxBurstAmount
bool CWeaponMagazined::cycleDownCheck() {
    s8 maxBurstAmount = 0;
    for ( const auto& fm : m_aFireModes ) {
        maxBurstAmount = std::max( maxBurstAmount, fm );
    }
    return bCycleDown && maxBurstAmount > 1 &&
           m_iShotNum < ( maxBurstAmount - 1 );
}
...
void CWeaponMagazined::state_Fire( float dt ) {
    if ( iAmmoElapsed > 0 ) {
        ...
        if ( NULL == io->inventory().ActiveItem() ) {
            ...
            StopShooting();
            return;
            // Alundaio: This is not supposed to happen but it does. GSC was
            // aware but why no return here? Known to cause crash on game load
            // if npc immediatly enters combat.
        }
        ...
        while ( !m_magazine.empty() && fShotTimeCounter < 0 &&
                ( IsWorking() || m_bFireSingleShot ) &&
                ( m_iQueueSize < 0 || m_iShotNum < m_iQueueSize ) ) {
            m_bFireSingleShot = false;

            // Alundaio: Use fModeShotTime instead of fOneShotTime if current
            // fire mode is 2-shot burst Alundaio: Cycle down RPM after two
            // shots; used for Abakan/AN-94
    ...
    if ( fShotTimeCounter < 0 ) {
        /*
                ...
                {
                Msg("stop shooting w=[%s] magsize=[%d] sshot=[%s] qsize=[%d]
           shotnum=[%d]", IsWorking()?"true":"false", m_magazine.size(),
...
void CWeaponMagazined::PlaySoundShot() {
    if ( ParentIsActor() ) {
        if ( bMisfire ) {
            ...
            strconcat( sizeof( sndNameMisfire ), sndNameMisfire,
                       m_sSndShotCurrent.c_str(), "MisfireActor" );
            if ( m_sounds.FindSoundItem( sndNameMisfire, false ) ) {
                m_sounds.PlaySound( sndNameMisfire, get_LastFP(), H_Root(),
                                    !!GetHUDmode(), false, ( u8 )-1 );
                return;
            }
        }

        string128 sndNameFirst;
        strconcat( sizeof( sndNameFirst ), sndNameFirst,
                   m_sSndShotCurrent.c_str(), "ActorFirst" );
        if ( m_iShotNum == 1 &&
             m_sounds.FindSoundItem( sndNameFirst, false ) ) {
            m_sounds.PlaySound( sndNameFirst, get_LastFP(), H_Root(),
                                !!GetHUDmode(), false, ( u8 )-1 );
            return;
        }

        string128 sndName;
        strconcat( sizeof( sndName ), sndName, m_sSndShotCurrent.c_str(),
                   "Actor" );
        if ( m_sounds.FindSoundItem( sndName, false ) ) {
            m_sounds.PlaySound( sndName, get_LastFP(), H_Root(), !!GetHUDmode(),
                                false, ( u8 )-1 );
            return;
        }
    }
    ...
    if ( bMisfire ) {
        string128 sndNameMisfire;
        strconcat( sizeof( sndNameMisfire ), sndNameMisfire,
                   m_sSndShotCurrent.c_str(), "Misfire" );
        if ( m_sounds.FindSoundItem( sndNameMisfire, false ) ) {
            m_sounds.PlaySound( sndNameMisfire, get_LastFP(), H_Root(),
                                !!GetHUDmode(), false, ( u8 )-1 );
            return;
    ...
    m_sounds.PlaySound( m_sSndShotCurrent.c_str(), get_LastFP(), H_Root(),
                        !!GetHUDmode(), false, ( u8 )-1 );
}
...
void CWeaponMagazined::OnShot() {
    ...
    CGameObject* object = smart_cast< CGameObject* >( H_Parent() );
    if ( object )
        object->callback( GameObject::eOnWeaponFired )(
            object->lua_game_object(), this->lua_game_object(), iAmmoElapsed );
#endif
...
void CWeaponMagazined::OnAnimationEnd( u32 state ) {
    switch ( state ) {
        case eReload: {
            ...
            // demonized: If wpn fire button is held, initiate auto firing when
            // reload is done
            if ( Actor() && H_Parent() == Actor() &&
                 pInput->iGetAsyncKeyState( get_action_dik( kWPN_FIRE ) ) )
                m_pendingShot = true;
            SwitchState( eIdle );
...
void CWeaponMagazined::UpdateFireMode() {
    m_iCurFireMode =
        ( m_iCurFireMode + ( m_nextFireMode ? 1 : -1 ) + m_aFireModes.size() ) %
        m_aFireModes.size();
...
void CWeaponMagazined::switch2_Fire() {
    ...
    if ( ii != io->inventory().ActiveItem() ) {
        Msg( "WARNING: Not an active item, item %s, owner %s, active item %s",
             *cName(), *H_Parent()->cName(),
             io->inventory().ActiveItem()
                 ? *io->inventory().ActiveItem()->object().cName()
                 : "no_active_item" );
        return;
    }
#ifdef DEBUG
...
void CWeaponMagazined::PlayReloadSound() {
    if ( m_sounds_enabled ) {
        if ( bMisfire && iAmmoElapsed ) {
            if ( ParentIsActor() ) {
                if ( m_sounds.FindSoundItem( "sndReloadMisfireActor",
                                             false ) ) {
            ...
            // TODO: make sure correct sound is loaded in
            // CWeaponMagazined::Load(LPCSTR section)
            ...
            if ( iAmmoElapsed == 0 ) {
                if ( ParentIsActor() ) {
                    if ( m_sounds.FindSoundItem( "sndReloadEmptyActor",
                                                 false ) ) {
...
void CWeaponMagazined::switch2_Hiding() {
    ...
    if ( m_sounds_enabled ) {
        if ( ParentIsActor() &&
             m_sounds.FindSoundItem( "sndHideActor", false ) )
            m_sounds.PlaySound( "sndHideActor", get_LastFP(), H_Root(),
                                !!GetHUDmode(), false, ( u8 )-1 );
        else
...
void CWeaponMagazined::switch2_Showing() {
    ...
    if ( m_sounds_enabled ) {
        if ( ParentIsActor() &&
             m_sounds.FindSoundItem( "sndShowActor", false ) )
...
bool CWeaponMagazined::PlayAnimCrouchIdleMoving() {
    if ( iAmmoElapsed == 0 ) {
        HudAnimationExist( "anm_idle_moving_crouch_empty" )
            ? PlayHUDMotion( "anm_idle_moving_crouch_empty", TRUE, NULL,
                             GetState() )
        : HudAnimationExist( "anm_idle_moving_empty" )
            ? PlayHUDMotion( "anm_idle_moving_empty", TRUE, NULL, GetState(),
                             .7f )
...
bool CWeaponMagazined::CanAttach( PIItem pIItem ) {
    ...
    CGrenadeLauncher* pGrenadeLauncher =
        smart_cast< CGrenadeLauncher* >( pIItem );
																		  ...
																		  (m_scopes[cur_scope]->m_sScopeName == pIItem->object().cNameSect())*/)
	{
        ...
        for ( ; it != m_scopes.end(); it++ ) {
            if ( m_modular_attachments ) {
                ...
                if ( pSettings->r_string( ( *it ), "scope_name" ) ==
        ...
        return false;
                ...
                ( m_flagsAddOnState &
                  CSE_ALifeItemWeapon::eWeaponAddonSilencer ) == 0 &&
                ( m_sSilencerName == pIItem->object().cNameSect() ) )
        return true;
    else if ( pGrenadeLauncher &&
              m_eGrenadeLauncherStatus == ALife::eAddonAttachable &&
              ( m_flagsAddOnState &
                CSE_ALifeItemWeapon::eWeaponAddonGrenadeLauncher ) == 0 &&
        ...
        for ( ; it != m_scopes.end(); it++ ) {
            if ( m_modular_attachments ) {
                ...
                LPCSTR iter_scope_name =
                    pSettings->r_string( ( *it ), "scope_name" );
...
bool CWeaponMagazined::DetachScope( const char* item_section_name,
                                    bool b_spawn_item ) {
    bool detached = false;
    SCOPES_VECTOR_IT it = m_scopes.begin();
    for ( ; it != m_scopes.end(); it++ ) {
...
void CWeaponMagazined::InitAddons() {
    if ( IsScopeAttached() ) {
        ...
        if ( m_eScopeStatus == ALife::eAddonAttachable ) {
            scope_tex_name = READ_IF_EXISTS(
                pSettings, r_string, GetScopeName(), "scope_texture", NULL );
            m_zoom_params.m_fScopeZoomFactor =
                pSettings->r_float( GetScopeName(), "scope_zoom_factor" );
            m_zoom_params.m_sUseZoomPostprocess = READ_IF_EXISTS(
                pSettings, r_string, GetScopeName(), "scope_nightvision", 0 );
            ...
            m_zoom_params.m_sUseBinocularVision =
                READ_IF_EXISTS( pSettings, r_string, GetScopeName(),
                                "scope_alive_detector", 0 );
            m_zoom_params.m_fZoomStepCount = READ_IF_EXISTS(
                pSettings, r_float, GetScopeName(), "zoom_step_count", 0 );
...
void CWeaponMagazined::LoadSilencerKoeffs() {
    if ( m_eSilencerStatus == ALife::eAddonAttachable ) {
        ...
        m_silencer_koef.hit_power = READ_IF_EXISTS(
            pSettings, r_float, sect, "bullet_hit_power_k", 1.0f );
        m_silencer_koef.hit_impulse = READ_IF_EXISTS(
            pSettings, r_float, sect, "bullet_hit_impulse_k", 1.0f );
        m_silencer_koef.bullet_speed =
            READ_IF_EXISTS( pSettings, r_float, sect, "bullet_speed_k", 1.0f );
        m_silencer_koef.fire_dispersion = READ_IF_EXISTS(
            pSettings, r_float, sect, "fire_dispersion_base_k", 1.0f );
        m_silencer_koef.cam_dispersion = READ_IF_EXISTS(
            pSettings, r_float, sect, "cam_dispersion_k", 1.0f );
        m_silencer_koef.cam_disper_inc = READ_IF_EXISTS(
            pSettings, r_float, sect, "cam_dispersion_inc_k", 1.0f );
        m_silencer_koef.pdm_base =
            READ_IF_EXISTS( pSettings, r_float, sect, "PDM_disp_base_k", 1.0f );
        m_silencer_koef.pdm_accel = READ_IF_EXISTS( pSettings, r_float, sect,
                                                    "PDM_disp_accel_k", 1.0f );
        m_silencer_koef.pdm_vel =
            READ_IF_EXISTS( pSettings, r_float, sect, "PDM_disp_vel_k", 1.0f );
        m_silencer_koef.crosshair_inertion = READ_IF_EXISTS(
            pSettings, r_float, sect, "crosshair_inertion_k", 1.0f );
        m_silencer_koef.zoom_rotate_time = READ_IF_EXISTS(
            pSettings, r_float, sect, "zoom_rotate_time_k", 1.0f );
        m_silencer_koef.condition_shot_dec = READ_IF_EXISTS(
            pSettings, r_float, sect, "condition_shot_dec_k", 1.0f );
    }
    ...
    if ( m_eScopeStatus == ALife::eAddonAttachable &&
        ...
        if ( !sect )
            Debug.fatal( DEBUG_INFO,
                         "!CWeaponMagazined::LoadScopeKoeffs ERROR: "
                         "GetScopeName for `%s` returns null, check scope_name "
                         "in ltx, m_scopes.size() %d, m_cur_scope %d",
                         cNameSect_str(), m_scopes.size(), m_cur_scope );
        m_scope_koef.cam_dispersion = READ_IF_EXISTS(
            pSettings, r_float, sect, "cam_dispersion_k", 1.0f );
        m_scope_koef.cam_disper_inc = READ_IF_EXISTS(
            pSettings, r_float, sect, "cam_dispersion_inc_k", 1.0f );
        m_scope_koef.pdm_base =
            READ_IF_EXISTS( pSettings, r_float, sect, "PDM_disp_base_k", 1.0f );
        m_scope_koef.pdm_accel = READ_IF_EXISTS( pSettings, r_float, sect,
                                                 "PDM_disp_accel_k", 1.0f );
        m_scope_koef.pdm_vel =
            READ_IF_EXISTS( pSettings, r_float, sect, "PDM_disp_vel_k", 1.0f );
        m_scope_koef.crosshair_inertion = READ_IF_EXISTS(
            pSettings, r_float, sect, "crosshair_inertion_k", 1.0f );
        m_scope_koef.zoom_rotate_time = READ_IF_EXISTS(
            pSettings, r_float, sect, "zoom_rotate_time_k", 1.0f );
    }
...
void CWeaponMagazined::PlayAnimShow() {
    ...
    iAmmoElapsed == 0 && HudAnimationExist( "anm_show_empty" )
        ? PlayHUDMotion( "anm_show_empty", FALSE, this, GetState(), 1.f, 0.f,
                         false )
...
void CWeaponMagazined::PlayAnimShoot() {
    ...
    if ( iAmmoElapsed > 1 || !HudAnimationExist( "anm_shot_l" ) ) {
        if ( !IsZoomed() || !HudAnimationExist( "anm_shots_aim" ) )
            PlayHUDMotion( "anm_shots", TRUE, this, GetState(), 1.f, 0.f,
                           false );
        else
            PlayHUDMotion( "anm_shots_aim", TRUE, this, GetState(), 1.f, 0.f,
                           false );
    } else {
        if ( !IsZoomed() || !HudAnimationExist( "anm_shots_aim_l" ) )
            PlayHUDMotion( "anm_shot_l", TRUE, this, GetState(), 1.f, 0.f,
                           false );
        else
            PlayHUDMotion( "anm_shots_aim_l", TRUE, this, GetState(), 1.f, 0.f,
                           false );
    }
}
...
void CWeaponMagazined::OnMotionMark( u32 state, const motion_marks& M ) {
    ...
    shared_str reloadMarkName = READ_IF_EXISTS(
        pSettings, r_string, cNameSect(), "motion_mark_reload", "" );
    if ( state == eReload &&
         ( reloadMarkName.size() == 0 || reloadMarkName == M.name ) ) {
        if ( bClearJamOnly ) {
            bMisfire = false;
            bClearJamOnly = false;
            m_needReload = false; // Verdatim, fix for anm_reload_misfire with
                                  // motion marks causing reloads
            return;
        }

        if ( bHasBulletsToHide &&
             xr_strcmp( M.name.c_str(), "lmg_reload" ) == 0 ) {
            ...
            last_hide_bullet =
                ae >= bullet_cnt ? bullet_cnt : bullet_cnt - ae - 1;
            ...
            // Verdatim, fix for lmg with non-(lmg_reload) motion marks causing
            // belts to disappear
...
void CWeaponMagazined::OnZoomIn() {
    ...
    CGameObject* object = smart_cast< CGameObject* >( H_Parent() );
    if ( object )
        object->callback( GameObject::eOnWeaponZoomIn )(
            object->lua_game_object(), this->lua_game_object() );
#endif
    //-Alundaio
    ...
    if ( pActor ) {
        CEffectorZoomInertion* S = smart_cast< CEffectorZoomInertion* >(
            pActor->Cameras().GetCamEffector( eCEZoom ) );
        if ( !S ) {
            S = ( CEffectorZoomInertion* )pActor->Cameras().AddCamEffector(
                xr_new< CEffectorZoomInertion >() );
            S->Init( this );
        };
...
void CWeaponMagazined::OnZoomOut() {
    ...
    CGameObject* object = smart_cast< CGameObject* >( H_Parent() );
    if ( object )
        object->callback( GameObject::eOnWeaponZoomOut )(
            object->lua_game_object(), this->lua_game_object() );
#endif
    //-Alundaio
...
bool CWeaponMagazined::GetBriefInfo( II_BriefInfo& info ) {
    ...
    if ( ae != 0 && m_magazine.size() != 0 ) {
        LPCSTR ammo_type =
            m_ammoTypes[ m_magazine.back().m_LocalAmmoType ].c_str();
        info.name = CStringTable().translate(
            pSettings->r_string( ammo_type, "inv_name_short" ) );
        info.icon = ammo_type;
    } else {
...
bool CWeaponMagazined::install_upgrade_impl( LPCSTR section, bool test ) {
    ...
    LPCSTR str;
    // fire_modes = 1, 2, -1
    bool result2 = process_if_exists_set( section, "fire_modes",
    ...
    result |= process_if_exists_set( section, "base_dispersioned_bullets_count",
                                     &CInifile::r_s32,
                                     m_iBaseDispersionedBulletsCount, test );
    result |= process_if_exists_set( section, "base_dispersioned_bullets_speed",
                                     &CInifile::r_float,
                                     m_fBaseDispersionedBulletsSpeed, test );

    // sounds (name of the sound, volume (0.0 - 1.0), delay (sec))
    result2 = process_if_exists_set( section, "snd_draw", &CInifile::r_string,
                                     str, test );
    if ( result2 && !test ) {
    ...
    result2 = process_if_exists_set( section, "snd_holster",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
    ...
    result2 = process_if_exists_set( section, "snd_shoot", &CInifile::r_string,
                                     str, test );
    if ( result2 && !test ) {
    ...
    result2 = process_if_exists_set( section, "snd_empty", &CInifile::r_string,
                                     str, test );
    if ( result2 && !test ) {
        m_sounds.LoadSound( section, "snd_empty", "sndEmptyClick", false,
                            m_eSoundEmptyClick );
    }
    result |= result2;

    result2 = process_if_exists_set( section, "snd_reload", &CInifile::r_string,
                                     str, test );
    if ( result2 && !test ) {
        m_sounds.LoadSound( section, "snd_reload", "sndReload", true,
                            m_eSoundReload );
    ...
    result2 = process_if_exists_set( section, "snd_reload_empty",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        m_sounds.LoadSound( section, "snd_reload_empty", "sndReloadEmpty", true,
                            m_eSoundReloadEmpty );
    ...
    if ( m_eSilencerStatus == ALife::eAddonAttachable ||
         m_eSilencerStatus == ALife::eAddonPermanent ) {
        result |= process_if_exists_set( section, "silencer_flame_particles",
                                         &CInifile::r_string,
                                         m_sSilencerFlameParticles, test );
        result |= process_if_exists_set( section, "silencer_smoke_particles",
                                         &CInifile::r_string,
                                         m_sSilencerSmokeParticles, test );

        result2 = process_if_exists_set( section, "snd_silncer_shot",
                                         &CInifile::r_string, str, test );
        if ( result2 && !test ) {
            m_sounds.LoadSound( section, "snd_silncer_shot", "sndSilencerShot",
                                false, m_eSoundShot );
    ...
    // fov for zoom mode
    result |=
        process_if_exists( section, "scope_zoom_factor", &CInifile::r_float,
                           m_zoom_params.m_fBaseZoomFactor, test );

    // demonized: additional weapon upgrade fields
    float tempFloat = 0.0;
    result2 = process_if_exists_set( section, "rpm_mode_2", &CInifile::r_float,
                                     tempFloat, test );
    if ( result2 && !test ) {
    ...
    result2 = process_if_exists_set( section, "cycle_down", &CInifile::r_string,
                                     str, test );
    if ( result2 && !test ) {
    ...
    result2 = process_if_exists_set( section, "scope_texture",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
    ...
    result2 = process_if_exists_set( section, "scope_texture_alt",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
    ...
    result2 = process_if_exists_set( section, "fire_point", &CInifile::r_string,
                                     str, test );
    if ( result2 && !test ) {
        ...
        if ( pointsCount == 3 ) {
            ...
            Msg( "! CWeaponMagazined::install_upgrade_impl ERROR: Invalid "
                 "fire_point count in section [%s], expected 3, got %d",
                 section, pointsCount );
        }
    }
    result |= result2;

    result2 = process_if_exists_set( section, "fire_point2",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        int pointsCount = _GetItemCount( str );
        if ( pointsCount == 3 ) {
            ...
            Msg( "! CWeaponMagazined::install_upgrade_impl ERROR: Invalid "
                 "fire_point2 count in section [%s], expected 3, got %d",
                 section, pointsCount );
        }
    }
    result |= result2;

    result2 = process_if_exists_set( section, "fire_point_silencer",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        int pointsCount = _GetItemCount( str );
        if ( pointsCount == 3 ) {
            ...
            Msg(
                "! CWeaponMagazined::install_upgrade_impl ERROR: Invalid "
                "fire_point_silencer count in section [%s], expected 3, got %d",
                section, pointsCount );
        }
    }
    result |= result2;

    result2 = process_if_exists_set( section, "hud_fire_point",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        int pointsCount = _GetItemCount( str );
        if ( pointsCount == 3 ) {
            for ( int i = 0; i < pointsCount; ++i ) {
                string16 sItem;
                _GetItem( str, i, sItem );
                HudItemData()->m_measures.m_fire_point_offset[ i ] =
                    std::atof( sItem );
            }
        } else {
            Msg( "! CWeaponMagazined::install_upgrade_impl ERROR: Invalid "
                 "hud_fire_point count in section [%s], expected 3, got %d",
                 section, pointsCount );
        }
    }
    result |= result2;

    result2 = process_if_exists_set( section, "hud_fire_point2",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        int pointsCount = _GetItemCount( str );
        if ( pointsCount == 3 ) {
            for ( int i = 0; i < pointsCount; ++i ) {
                string16 sItem;
                _GetItem( str, i, sItem );
                HudItemData()->m_measures.m_fire_point2_offset[ i ] =
                    std::atof( sItem );
            }
        } else {
            Msg( "! CWeaponMagazined::install_upgrade_impl ERROR: Invalid "
                 "hud_fire_point2 count in section [%s], expected 3, got %d",
                 section, pointsCount );
        }
    }
    result |= result2;

    result2 = process_if_exists_set( section, "hud_fire_point_silencer",
                                     &CInifile::r_string, str, test );
    if ( result2 && !test ) {
        int pointsCount = _GetItemCount( str );
        if ( pointsCount == 3 ) {
            for ( int i = 0; i < pointsCount; ++i ) {
                string16 sItem;
                _GetItem( str, i, sItem );
                HudItemData()->m_measures.m_fire_point_silencer[ i ] =
                    std::atof( sItem );
            }
        } else {
            Msg( "! CWeaponMagazined::install_upgrade_impl ERROR: Invalid "
                 "hud_fire_point_silencer count in section [%s], expected 3, "
                 "got %d",
                 section, pointsCount );
        }
    }
...
// текущая дисперсия (в радианах) оружия с учетом используемого патрона и
// недисперсионных пуль
float CWeaponMagazined::GetFireDispersion( float cartridge_k,
                                           bool for_crosshair ) {
...
void CWeaponMagazined::FireBullet( const Fvector& pos,
    ...
    inherited::FireBullet( pos, shot_dir, fire_disp, cartridge, parent_id,
                           weapon_id, send_hit, GetAmmoElapsed() );
}
