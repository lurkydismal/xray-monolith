...
void CALifeSurgeManager::spawn_new_spawns() {
    ...
    for ( ; I != E; ++I ) {
        const CALifeSpawnRegistry::SPAWN_GRAPH::CVertex* const vertex_ptr =
            spawns().spawns().vertex( *I );
        if ( !vertex_ptr )
            continue;
        CSE_ALifeDynamicObject *object,
            *spawn = smart_cast< CSE_ALifeDynamicObject* >(
        ...
        if ( psAI_Flags.test( aiALife ) )
            Msg( "LSS : SURGE : SPAWN : [%s],[%s], level %s, time %f ms",
                 *spawn->s_name, spawn->name_replace(),
                 *ai().game_graph()
                      .header()
                      .level( ai().game_graph()
                                  .vertex( spawn->m_tGraphID )
                                  ->level_id() )
                      .name(),
