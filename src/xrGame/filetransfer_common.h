...
u32 const data_max_chunk_size = 4096; // 4Kb on update ~	80
                                      // Kb/sec
u32 const data_min_chunk_size =
    128; //					2.5		Kb/sec
...
enum ft_command_t // command byte to M_FILE_TRANSFER message ...
{
    ...
    receive_rejected =
        0x02 // this command send by dest site, if he doesn't want file..
};

typedef xr_delegate< void( sending_status_t, u32, u32 ) >
    sending_state_callback_t;
typedef xr_delegate< void( receiving_status_t, u32, u32 ) >
    receiving_state_callback_t;
