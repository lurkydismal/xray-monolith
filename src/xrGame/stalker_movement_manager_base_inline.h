...
IC const MonsterSpace::SBoneRotation&
stalker_movement_manager_base::head_orientation() const {
    return ( m_head );
}

IC void stalker_movement_manager_base::set_head_orientation(
    const MonsterSpace::SBoneRotation& orientation ) {
    m_head = orientation;
}

IC void stalker_movement_manager_base::set_desired_direction(
...
IC bool stalker_movement_manager_base::turn_in_place() const {
    return ( !path_completed() && fis_zero( speed() ) &&
             ( angle_difference( body_orientation().current.yaw,
                                 body_orientation().target.yaw ) > EPS_L ) );
}

IC void stalker_movement_manager_base::set_body_state( EBodyState body_state ) {
#ifdef DEBUG
    THROW2( ( body_state != eBodyStateCrouch ) ||
                ( m_target.m_mental_state != eMentalStateFree ),
...
IC EBodyState stalker_movement_manager_base::body_state_combat_override(
    EWorldOperators wo,
    EBodyState body_state ) {
    ::luabind::functor< EBodyState > lua_function;
    if ( ai().script_engine().functor( "_G.CAI_Stalker__CombatSetBodyState",
                                       lua_function ) ) {
        body_state =
            lua_function( m_object->lua_game_object(), wo, body_state );
    }
    return body_state;
}
#endif

IC void stalker_movement_manager_base::set_movement_type(
    EMovementType movement_type ) {
    m_target.m_movement_type = movement_type;
}

IC void stalker_movement_manager_base::set_mental_state(
    EMentalState mental_state ) {
    THROW( ( m_target.m_body_state != eBodyStateCrouch ) ||
           ( mental_state != eMentalStateFree ) );
    m_target.m_mental_state = mental_state;
    // #pragma todo("Dima to Dima: this is correct, commented just because of
...
IC void stalker_movement_manager_base::set_detail_path_type(
    EDetailPathType detail_path_type ) {
    m_target.m_detail_path_type = detail_path_type;
}

IC MonsterSpace::EBodyState const& stalker_movement_manager_base::body_state()
    const {
    return ( m_current.m_body_state );
}

IC MonsterSpace::EBodyState const&
stalker_movement_manager_base::target_body_state() const {
    return ( m_target.m_body_state );
}

IC MonsterSpace::EMovementType const&
stalker_movement_manager_base::movement_type() const {
    return ( m_current.m_movement_type );
}

IC MonsterSpace::EMentalState const&
stalker_movement_manager_base::mental_state() const {
    return ( m_current.m_mental_state );
}

IC MonsterSpace::EMentalState const&
stalker_movement_manager_base::target_mental_state() const {
    return ( m_target.m_mental_state );
}

IC MovementManager::EPathType const& stalker_movement_manager_base::path_type()
    const {
    return ( m_current.m_path_type );
}

IC DetailPathManager::EDetailPathType const&
stalker_movement_manager_base::detail_path_type() const {
...
IC MonsterSpace::EMovementType const&
stalker_movement_manager_base::target_movement_type() const {
...
IC stalker_movement_params const&
stalker_movement_manager_base::current_params() const {
    return ( m_current );
}
