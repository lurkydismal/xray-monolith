...
BOOL CSpaceRestrictor::net_Spawn( CSE_Abstract* data ) {
    ...
    CSE_ALifeSpaceRestrictor* se_shape =
        smart_cast< CSE_ALifeSpaceRestrictor* >( abstract );
    R_ASSERT( se_shape );
    ...
    // Alundaio: zone visible for ai if g_ai_die_in_anomaly = 1 unless it's
    // Radioactive Zone or Campfire
    CCustomZone* zone = smart_cast< CCustomZone* >( this );
    // if (g_ai_die_in_anomaly == 0 || !zone ||
    // smart_cast<CRadioactiveZone*>(zone) || smart_cast<CZoneCampfire*>(zone))
    if ( !zone || smart_cast< CRadioactiveZone* >( zone ) ||
         smart_cast< CZoneCampfire* >( zone ) )
        SpatialComponent->spatial.type &= ~STYPE_VISIBLEFORAI;

    setEnabled( FALSE );
    setVisible( FALSE );

    if ( !ai().get_level_graph() || ( RestrictionSpace::ERestrictorTypes(
                                          se_shape->m_space_restrictor_type ) ==
                                      RestrictionSpace::eRestrictorTypeNone ) )
        return ( TRUE );

    Level().space_restriction_manager().register_restrictor(
        this, RestrictionSpace::ERestrictorTypes(
                  se_shape->m_space_restrictor_type ) );
...
void CSpaceRestrictor::net_Destroy() {
    ...
    if ( RestrictionSpace::ERestrictorTypes( m_space_restrictor_type ) ==
         RestrictionSpace::eRestrictorTypeNone )
        return;
...
void CSpaceRestrictor::OnRender() {
    ...
    xr_vector< CCF_Shape::shape_def >& l_shapes =
        ( ( CCF_Shape* )CFORM() )->Shapes();
    ...
    for ( l_pShape = l_shapes.begin(); l_shapes.end() != l_pShape;
          ++l_pShape ) {
    ...
    if ( Level().CurrentViewEntity()->Position().distance_to( XFORM().c ) <
         100.0f ) {
        // DRAW name
