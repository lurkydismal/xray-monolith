...
void CScriptGameObject::set_smart_cover_target_idle() {
    ...
    if ( !stalker->g_Alive() ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CAI_Stalker : do not call smart_cover_setup_idle_target when "
            "stalker is dead!" );
        return;
    }
...
void CScriptGameObject::set_smart_cover_target_lookout() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CAI_Stalker [%s]: cannot access class member "
                         "smart_cover_setup_lookout_target!",
                         object().cNameSect().c_str() )
                .c_str() );
        return;
    }
    ...
    if ( !stalker->g_Alive() ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CAI_Stalker : do not call smart_cover_setup_lookout_target when "
            "stalker is dead!" );
        return;
    }
...
void CScriptGameObject::set_smart_cover_target_fire() {
    ...
    if ( !stalker->g_Alive() ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CAI_Stalker : do not call smart_cover_setup_fire_target when "
            "stalker is dead!" );
        return;
    }
...
void CScriptGameObject::set_smart_cover_target_fire_no_lookout() {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CAI_Stalker [%s]: cannot access class member "
                         "smart_cover_setup_fire_no_lookout_target!",
                         object().cNameSect().c_str() )
                .c_str() );
        return;
    }
    ...
    if ( !stalker->g_Alive() ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CAI_Stalker : do not call set_smart_cover_target_fire_no_lookout "
            "when stalker is dead!" );
        return;
    }
...
void CScriptGameObject::set_smart_cover_target_default( bool value ) {
    ...
    if ( !stalker->g_Alive() ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            "CAI_Stalker : do not call set_smart_cover_target_default when "
            "stalker is dead!" );
        return;
    }
...
void CScriptGameObject::set_smart_cover_target(
    CScriptGameObject* enemy_object ) {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
    ...
    stalker->movement().target_params().cover_fire_object(
        &enemy_object->object() );
}

bool CScriptGameObject::in_loophole_fov( LPCSTR cover_id,
                                         LPCSTR loophole_id,
    ...
    return (
        stalker->movement().in_fov( cover_id, loophole_id, object_position ) );
}

bool CScriptGameObject::in_current_loophole_fov(
    Fvector object_position ) const {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker ) {
        ai().script_engine().script_log(
...
bool CScriptGameObject::in_loophole_range( LPCSTR cover_id,
                                           LPCSTR loophole_id,
                                           Fvector object_position ) const {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string( "CAI_Stalker [%s]: cannot access class member "
                         "object_in_loophole_range!",
                         object().cNameSect().c_str() )
                .c_str() );
        return ( false );
    }

    return ( stalker->movement().in_range( cover_id, loophole_id,
                                           object_position ) );
}

bool CScriptGameObject::in_current_loophole_range(
    Fvector object_position ) const {
    CAI_Stalker* stalker = smart_cast< CAI_Stalker* >( &object() );
    if ( !stalker ) {
        ai().script_engine().script_log(
...
float const CScriptGameObject::idle_min_time() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member idle_min_time!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( flt_max );
    }
...
float const CScriptGameObject::idle_max_time() const {
    ...
    if ( !stalker ) {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
            make_string(
                "CAI_Stalker [%s]: cannot access class member idle_max_time!",
                object().cNameSect().c_str() )
                .c_str() );
        return ( flt_max );
    }
...
bool CScriptGameObject::movement_target_reached() {
    ...
    return ( stalker->movement().current_params().equal_to_target(
        stalker->movement().target_params() ) );
}
