...
public:
    statistic_sync_quard( xrCriticalSection& mutex ) : m_mutex( mutex ) {
        m_mutex.Enter();
    }
...
u32 const HitData::net_packet_size =
    ( 3 * 2 * sizeof( float ) + 1 + 2 + 1 + 1 );

void HitData::net_save( NET_Packet* P,
                        victims_table const& vt,
                        bone_table const& bt ) {
    P->w_vec3( Pos0 );
    P->w_vec3( Pos1 );
...
void Weapon_Statistic::net_save( NET_Packet* P,
    ...
    class CompleteFilter {
        ...
        CompleteFilter& operator=( CompleteFilter& copy ) {
            return *this;
        }; // C4512
    public:
        CompleteFilter( u32& hits_count_result,
                        NET_Packet* P,
                        victims_table const& vt,
                        bone_table const& bt )
        ...
        bool operator()( HitData& hit ) {
            if ( hit.Completed ) {
                if ( NET_PacketSizeLimit - packet_to_write->w_tell() <
                     HitData::net_packet_size )
                    return false;
    ...
    m_Hits.erase( std::remove_if( m_Hits.begin(), m_Hits.end(), tmp_filter ),
                  m_Hits.end() );
...
u8 victims_table::get_id_by_name( shared_str const& player_name ) const {
    u8 index = 0;
    for ( victims_table_t::const_iterator i = m_data.begin(), ie = m_data.end();
          i != ie; ++i ) {
        if ( *i == player_name )
            return index;
        ++index;
...
bool victims_table::add_name( shared_str const& player_name ) {
    ...
    victims_table_t::const_iterator tmp_iter =
        std::find( m_data.begin(), m_data.end(), player_name );
...
shared_str bone_table::get_name_by_id( s16 id ) const {
    bone_table_t::const_iterator tmp_iter =
        std::find_if( m_data.begin(), m_data.end(), bone_id_searcher( id ) );
    if ( tmp_iter == m_data.end() )
        return shared_str();
...
bool bone_table::add_bone( shared_str const& bone_name, s16 bone_id ) {
    ...
    public:
        bone_name_searcher( shared_str const& name_to_search )
            : name( name_to_search ) {}
    ...
    if ( tmp_iter != m_data.end() )
        return false;
    tmp_iter = std::find_if( m_data.begin(), m_data.end(),
                             bone_id_searcher( bone_id ) );
    if ( tmp_iter != m_data.end() )
        return false;
...
void bone_table::net_save( NET_Packet* P ) {
    ...
    for ( bone_table_t::const_iterator i = m_data.begin(), ie = m_data.end();
          i != ie; ++i ) {
        P->w_stringZ( i->first );
        P->w_s16( i->second );
    }
...
u32 Player_Statistic::create_victims_table( victims_table& victims_table ) {
    ...
    for ( u32 wi = 0, weapons_size = aWeaponStats.size(); wi < weapons_size;
          ++wi ) {
        Weapon_Statistic& WS = aWeaponStats[ wi ];
        for ( u32 hi = 0, hits_count = WS.m_Hits.size(); hi < hits_count;
              ++hi ) {
...
void Player_Statistic::net_save( NET_Packet* P ) {
    ...
    victims_table::victims_table_t vt_storage(
        _alloca( sizeof( victims_table::victims_table_t::value_type ) * 255 ),
        255 );
    ...
    if ( NET_PacketSizeLimit - P->w_tell() <
         tables_size + Weapon_Statistic::net_packet_size )
        return;
...
void WeaponUsageStatistic::Clear() {
    ...
    ZeroMemory( m_dwTotalPlayersAliveTime,
                sizeof( m_dwTotalPlayersAliveTime ) );
    ZeroMemory( m_dwTotalPlayersMoneyRound,
                sizeof( m_dwTotalPlayersMoneyRound ) );
...
bool WeaponUsageStatistic::GetPlayer( LPCSTR PlayerName,
                                      PLAYERS_STATS_it& pPlayerI ) {
    statistic_sync_quard syncg( m_mutex );
    pPlayerI = std::find( aPlayersStatistic.begin(), aPlayersStatistic.end(),
                          PlayerName );
    if ( pPlayerI == aPlayersStatistic.end() ||
         !( ( *pPlayerI ) == PlayerName ) )
        return false;
    return true;
}
...
WEAPON_STATS_it Player_Statistic::FindPlayersWeapon( LPCSTR WeaponName ) {
    R_ASSERT( WeaponName );
    WEAPON_STATS_it pWeaponI =
        std::find( aWeaponStats.begin(), aWeaponStats.end(), WeaponName );
...
bool WeaponUsageStatistic::FindBullet( u32 BulletID, ABULLETS_it& Bullet_It ) {
    Bullet_It =
        std::find( ActiveBullets.begin(), ActiveBullets.end(), BulletID );
...
void WeaponUsageStatistic::RemoveBullet( ABULLETS_it& Bullet_it ) {
    statistic_sync_quard syncg( m_mutex );
    if ( !Bullet_it->Removed ||
         Bullet_it->HitRefCount != Bullet_it->HitResponds )
        return;
    //-------------------------------------------------------------
    PLAYERS_STATS_it PlayerIt = FindPlayer( *( Bullet_it->FirerName ) );
    WEAPON_STATS_it WeaponIt =
        PlayerIt->FindPlayersWeapon( *( Bullet_it->WeaponName ) );
    HITS_VEC_it HitIt;
...
void WeaponUsageStatistic::OnWeaponBought( game_PlayerState* ps,
                                           LPCSTR WeaponName ) {
    statistic_sync_quard syncg( m_mutex );
    if ( !CollectData() )
        return;
...
void WeaponUsageStatistic::OnBullet_Fire( SBullet* pBullet,
    ...
    WEAPON_STATS_it WeaponIt =
        PlayerIt->FindPlayersWeapon( *object_weapon->cNameSect() );
    WeaponIt->m_dwRoundsFired =
        ( ++WeaponIt->m_dwBulletsFired ) / cartridge.param_s.buckShot;
    WeaponIt->m_dwBulletsFired_d++;
    //-----------------------------------------------------------------------------------
    ActiveBullets.push_back( BulletData(
        object_parent->cName(), object_weapon->cNameSect(), pBullet ) );

    //	Msg("! OnBullet Fire ID[%d]", pBullet->m_dwID);
}

void WeaponUsageStatistic::OnBullet_Hit( SBullet* pBullet,
                                         u16 TargetID,
                                         s16 element,
                                         Fvector HitLocation ) {
    statistic_sync_quard syncg( m_mutex );
    ...
    WEAPON_STATS_it WeaponIt =
        PlayerIt->FindPlayersWeapon( *( BulletIt->WeaponName ) );
    if ( !BulletIt->HitRefCount++ ) {
        ...
        NewHit.BoneName = smart_cast< IKinematics* >( pTarget->Visual() )
                              ->LL_BoneName_dbg( element );
        NewHit.count = 1;
        //---------------------------
...
void WeaponUsageStatistic::OnBullet_Check_Request( SHit* pHDS ) {
    ...
    BChA_it pSenderI =
        std::find( m_Requests.begin(), m_Requests.end(), SenderID );
    ...
    ( *pSenderI )
        .Requests.push_back( Bullet_Check_Request( BulletID, BoneID ) );
...
void WeaponUsageStatistic::OnBullet_Check_Result( bool Result ) {
    ...
    if ( m_dwLastRequestSenderID ) {
        BChA_it pSenderI = std::find( m_Requests.begin(), m_Requests.end(),
                                      m_dwLastRequestSenderID );
        if ( pSenderI != m_Requests.end() &&
             ( *pSenderI ) == m_dwLastRequestSenderID ) {
    ...
    for ( u32 i = 0; i < m_Requests.size(); i++ ) {
        ...
        while ( j < BChA_Request.Requests.size() ) {
            ...
            if ( !curBChR.Processed ) {
                ...
                //				HitChecksResponded++;
                //-----------------------------------------------------
                *( BChA_Request.Requests.begin() + j ) =
                    BChA_Request.Requests.back();
...
void WeaponUsageStatistic::On_Check_Respond( NET_Packet* P ) {
    ...
    for ( i = 0; i < NumTrue; i++ ) {
        ...
        if ( WeaponIt->FindHit( BulletID, HitIt ) ) {
            ...
            if ( pObj )
                HData.BoneName = smart_cast< IKinematics* >( pObj->Visual() )
                                     ->LL_BoneName_dbg( BoneID );
        }
        //---------------------------------------------------------------
        RemoveBullet( BulletIt );
    }
...
void WeaponUsageStatistic::OnPlayerAddMoney( game_PlayerState* ps,
                                             s32 MoneyAmount ) {
    if ( !CollectData() )
        return;
    if ( !ps || MoneyAmount <= 0 )
...
void WeaponUsageStatistic::OnExplosionKill( game_PlayerState* ps,
                                            const SHit& hit ) {
    if ( !CollectData() )
        return;
    if ( !ps )
...
void WeaponUsageStatistic::OnBleedKill( game_PlayerState* killer_ps,
                                        game_PlayerState* victim_ps,
                                        u16 weapon_id ) {
    statistic_sync_quard syncg( m_mutex );
    if ( !killer_ps || !victim_ps )
        return;
    Player_Statistic& PlayerStatKiller =
        *( FindPlayer( killer_ps->getName() ) );
        ...
        void operator()( IClient* client ) {
            ...
            if ( ps && !ps->testFlag( GAME_PLAYER_FLAG_VERY_VERY_DEAD ) ) {
                if ( xr_strlen( ps->getName() ) ) {
                    Player_Statistic& pstat =
                        *( owner.FindPlayer( ps->getName() ) );
                    u8 team = owner.ConvertToTeamIndex( ps->team );
                    u32 time_dif =
                        Device.dwTimeGlobal - pstat.last_alive_update_time;
...
void WeaponUsageStatistic::Update() {
    if ( !CollectData() )
        return;
    SVUpdateAliveTimes(); // update client alive time and servers total alive
                          // times
    if ( !OnServer() )
        return;
...
void WeaponUsageStatistic::OnUpdateRespond( NET_Packet* P,
                                            shared_str const& sender_digest,
                                            u32 sender_pid ) {
    if ( !P )
        return;
