...
const DIALOG_SHARED_PTR& CPhraseDialogManager::GetDialogByID(
    const shared_str& dialog_id ) const {
...
bool CPhraseDialogManager::HaveAvailableDialog(
    const shared_str& dialog_id ) const {
    DIALOG_VECTOR::const_iterator it = m_AvailableDialogs.begin();
    DIALOG_VECTOR::const_iterator it_e = m_AvailableDialogs.end();
...
void CPhraseDialogManager::InitDialog( CPhraseDialogManager* dialog_partner,
...
void CPhraseDialogManager::AddDialog( DIALOG_SHARED_PTR& phrase_dialog ) {
    DIALOG_VECTOR_IT it = std::find( m_ActiveDialogs.begin(),
                                     m_ActiveDialogs.end(), phrase_dialog );
...
static bool dialog_priority( DIALOG_SHARED_PTR dialog1,
                             DIALOG_SHARED_PTR dialog2 ) {
...
void CPhraseDialogManager::UpdateAvailableDialogs(
    CPhraseDialogManager* partner ) {
    std::sort( m_AvailableDialogs.begin(), m_AvailableDialogs.end(),
               dialog_priority );
}

bool CPhraseDialogManager::AddAvailableDialog( shared_str dialog_id,
                                               CPhraseDialogManager* partner ) {
    //	PHRASE_DIALOG_INDEX dialog_index =  CPhraseDialog::IdToIndex(dialog_id);
    if ( std::find( m_CheckedDialogs.begin(), m_CheckedDialogs.end(),
                    dialog_id ) != m_CheckedDialogs.end() )
        return false;
    ...
    const CGameObject* pSpeakerGO2 =
        smart_cast< const CGameObject* >( partner );
    VERIFY( pSpeakerGO2 );

    bool predicate_result =
        phrase_dialog->Precondition( pSpeakerGO1, pSpeakerGO2 );
    if ( predicate_result )
