...
LPCSTR clearBOM( LPCSTR s ) {
    if ( xr_strlen( s ) >= 3 ) {
        if ( s[ 0 ] == ( char )0xEF && s[ 1 ] == ( char )0xBB &&
             s[ 2 ] == ( char )0xBF ) {
            LPCSTR new_s = s + 3;
            return new_s;
        }
...
void XMLLuaCallback( CXml& m_xml, LPCSTR xml_string ) {
    ...
    if ( ai().script_engine().functor( "_G.COnXmlRead", funct ) ) {
        ...
        // Msg("XMLLuaCallback, xml %s, contents %s", m_xml.m_xml_file_name,
        // res);
...
bool CScriptXmlInit::NavigateToNode_ByAttribute( LPCSTR tag_name,
                                                 LPCSTR attrib_name,
                                                 LPCSTR attrib_value ) {
    XML_NODE* node = m_xml.NavigateToNodeWithAttribute( tag_name, attrib_name,
                                                        attrib_value );
    if ( node ) {
        m_xml.SetLocalRoot( node );
        return true;
...
bool CScriptXmlInit::NavigateToNode_ByPath( LPCSTR path,
                                            int index,
                                            LPCSTR tag_name,
                                            LPCSTR attrib,
                                            LPCSTR attrib_value_pattern ) {
    XML_NODE* node = m_xml.SearchForAttribute( path, index, tag_name, attrib,
...
CUIFrameLineWnd* CScriptXmlInit::InitFrameLine( LPCSTR path,
                                                CUIWindow* parent ) {
...
CUIScrollView* CScriptXmlInit::InitScrollView( LPCSTR path,
                                               CUIWindow* parent ) {
...
CUIProgressBar* CScriptXmlInit::InitProgressBar( LPCSTR path,
                                                 CUIWindow* parent ) {
...
void CScriptXmlInit::script_register( lua_State* L ) {
    module(
        L )[ class_< CScriptXmlInit >( "CScriptXmlInit" )
                 ...
                 .def( "NavigateToNode_ByAttribute",
                       &CScriptXmlInit::NavigateToNode_ByAttribute )
                 .def( "NavigateToNode_ByPath",
                       &CScriptXmlInit::NavigateToNode_ByPath )
