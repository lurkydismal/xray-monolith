...
bool CItemManager::useful( const CGameObject* object ) const {
    ...
    const CInventoryItem* inventory_item =
        gameObject ? gameObject->cast_inventory_item() : NULL;
    ...
    if ( m_stalker && !m_stalker->memory().enemy().selected() ) {
        if ( !m_object->movement().restrictions().accessible(
                 object->Position() ) )
            return ( false );
    ...
    if ( !ai().level_graph().inside( object->ai_location().level_vertex_id(),
...
float CItemManager::do_evaluate( const CGameObject* object ) const {
    VERIFY3( m_object->movement().restrictions().accessible(
                 object->ai_location().level_vertex_id() ),
             *m_object->cName(), *object->cName() );
...
void CItemManager::update() {
    ...
    VERIFY3( !selected() || m_object->movement().restrictions().accessible(
                                selected()->ai_location().level_vertex_id() ),
             *m_object->cName(),
...
void CItemManager::remove_links( CObject* object ) {
    ...
    OBJECTS::iterator I =
        std::find( m_objects.begin(), m_objects.end(), ( CGameObject* )object );
    if ( I != m_objects.end() )
        m_objects.erase( I );
...
void CItemManager::on_restrictions_change() {
    ...
    if ( !m_object->movement().restrictions().accessible(
             m_selected->ai_location().level_vertex_id() ) ) {
        m_selected = 0;
        return;
    }
