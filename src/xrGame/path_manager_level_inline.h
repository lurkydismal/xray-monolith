...
#define TEMPLATE_SPECIALIZATION                            \
    template < typename _DataStorage, typename _dist_type, \
               typename _index_type, typename _iteration_type >

#define CLevelPathManager                                                      \
    CPathManager< CLevelGraph, _DataStorage,                                   \
...
TEMPLATE_SPECIALIZATION
IC void CLevelPathManager::setup( const _Graph* _graph,
    ...
    inherited::setup( _graph, _data_storage, _path, _start_node_index,
                      _goal_node_index, parameters );
    m_distance_xz = graph->header().cell_size();
    m_sqr_distance_xz = _sqr( graph->header().cell_size() );
    //		square_size_y			=
    //_sqr((float)(graph->header().factor_y()/32767.0)); 		size_y
...
TEMPLATE_SPECIALIZATION
IC _dist_type
CLevelPathManager::evaluate( const _index_type& node_index1,
    ...
    //		return
    //(_sqrt(square_size_y*(float)_sqr(y2 - y1) + m_sqr_distance_xz));
    return ( m_distance_xz ); // + (y2 - y1)*size_y);
}

TEMPLATE_SPECIALIZATION
IC _dist_type
CLevelPathManager::estimate( const _index_type& node_index ) const {
    VERIFY( graph );
    //		return
    //(_sqrt((float)(m_sqr_distance_xz*float(_sqr(x3 - x1) + _sqr(z3 - z1)) +
    ...
    //x1); 		int						z = _abs(z3 - z1);
    //		return
    //(m_distance_xz*_dist_type(_min(x,z)*3 + 2*_abs(x - z)));
}
...
TEMPLATE_SPECIALIZATION
IC bool CLevelPathManager::is_limit_reached(
    const _iteration_type iteration_count ) const {
    VERIFY( data_storage );
...
TEMPLATE_SPECIALIZATION
IC void CLevelPathManager::begin( const _index_type& vertex_id,
                                  _Graph::const_iterator& begin,
                                  _Graph::const_iterator& end ) {
    graph->begin( best_node, begin, end );
}

TEMPLATE_SPECIALIZATION
IC const _index_type
CLevelPathManager::get_value( _Graph::const_iterator& i ) const {
