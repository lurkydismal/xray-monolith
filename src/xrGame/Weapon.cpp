...
float CWeapon::SDS_Radius( bool alt ) {
    ...
    if ( zoomFlags.test( SDS ) ) {
        if ( 0 != ( m_flagsAddOnState &
                    CSE_ALifeItemWeapon::eWeaponAddonScope ) &&
             m_scopes.size() ) {
            scope_tex_name =
                alt ? m_secondary_scope_tex_name : m_primary_scope_tex_name;
        } else {
            scope_tex_name =
                alt ? m_secondary_scope_tex_name : m_primary_scope_tex_name;

            // demonized: ugly hack to fix stuck scope texture on old scopes
            // system
            if ( !READ_IF_EXISTS( pSettings, r_string, cNameSect(),
                                  "scope_texture", NULL ) ) {
                scope_tex_name = NULL;
            }
        }
...
void CWeapon::UpdateXForm() {
    ...
    // this ugly case is possible in case of a CustomMonster, not a Stalker, nor
    // an Actor
    ...
    if ( ( HandDependence() == hd1Hand ) || ( GetState() == eReload ) ||
         ( !E->g_Alive() ) )
        boneL = boneR2;
...
void updateCurrentScope() {
    ...
    CInventoryOwner* pGameObject =
        smart_cast< CInventoryOwner* >( Level().Objects.net_Find( 0 ) );
    if ( pGameObject ) {
        if ( pGameObject->inventory().ActiveItem() ) {
            CWeapon* weapon =
                smart_cast< CWeapon* >( pGameObject->inventory().ActiveItem() );
            if ( weapon ) {
...
void CWeapon::UpdateZoomParams() {
    //////////
    m_zoom_params.m_fMinBaseZoomFactor = READ_IF_EXISTS(
        pSettings, r_float, cNameSect(), "min_scope_zoom_factor", 200.0f );

    float zoom_multiple = 1.0f;
    if ( zoomFlags.test( SDS_ZOOM ) &&
         ( SDS_Radius( m_zoomtype == 1 ) > 0.0 ) ) {
    ...
    if ( m_zoomtype == 2 ) // GL
    {
        m_zoom_params.m_bUseDynamicZoom =
            ...
            READ_IF_EXISTS( pSettings, r_bool, cNameSect(),
                            "scope_dynamic_zoom_gl", false );
        m_zoom_params.m_fScopeZoomFactor =
            g_player_hud->m_adjust_mode
                ? g_player_hud->m_adjust_zoom_factor[ 1 ]
    ...
    } else if ( m_zoomtype == 1 ) // Alt
    {
        m_zoom_params.m_bUseDynamicZoom =
            ...
            READ_IF_EXISTS( pSettings, r_bool, cNameSect(),
                            "scope_dynamic_zoom_alt", false );
        m_zoom_params.m_fScopeZoomFactor =
            ( g_player_hud->m_adjust_mode
                  ? g_player_hud->m_adjust_zoom_factor[ 2 ]
                  : READ_IF_EXISTS( pSettings, r_float, cNameSect(),
                                    "scope_zoom_factor_alt", 0 ) ) /
            ( READ_IF_EXISTS( pSettings, r_string, cNameSect(),
                              "scope_texture_alt", NULL ) &&
                      zoomFlags.test( SDS_ZOOM ) && ( SDS_Radius( true ) > 0.0 )
                  ? zoom_multiple
                  : 1 );
        m_zoom_params.m_fZoomStepCount = 0;
    } else // Main Sight
    {
        ...
        if ( g_player_hud->m_adjust_mode ) {
            m_zoom_params.m_fScopeZoomFactor =
                g_player_hud->m_adjust_zoom_factor[ 0 ] / zoom_multiple;
            ...
            if ( m_modular_attachments ) {
                m_zoom_params.m_bUseDynamicZoom =
                    READ_IF_EXISTS( pSettings, r_bool, GetScopeName(),
                                    "scope_dynamic_zoom", false );
                m_zoom_params.m_fMinBaseZoomFactor =
                    READ_IF_EXISTS( pSettings, r_float, GetScopeName(),
                                    "min_scope_zoom_factor", 200.0f );
                stepCount = READ_IF_EXISTS( pSettings, r_float, GetScopeName(),
                                            "zoom_step_count", 0 );
            }
        } else {
            m_zoom_params.m_fScopeZoomFactor =
                m_zoom_params.m_fBaseZoomFactor / zoom_multiple;
        }
        if ( stepCount == 0 )
            stepCount = READ_IF_EXISTS( pSettings, r_float, cNameSect(),
                                        "zoom_step_count", 0 );
    ...
    if ( IsZoomed() ) {
        ...
        if ( m_zoomtype == 0 && zoomFlags.test( SDS_SPEED ) &&
             ( scope_radius > 0.0 ) ) {
...
void CWeapon::UpdateUIScope() {
    ...
    if ( m_zoomtype == 0 ) {
        if ( 0 != ( m_flagsAddOnState &
            ...
            if ( !m_primary_scope_tex_name || m_modular_attachments ) {
                m_primary_scope_tex_name =
                    pSettings->r_string( GetScopeName(), "scope_texture" );
            }
            scope_tex_name = m_primary_scope_tex_name;
        } else {
            if ( !m_primary_scope_tex_name ) {
                m_primary_scope_tex_name = READ_IF_EXISTS(
                    pSettings, r_string, cNameSect(), "scope_texture", NULL );
            }
            scope_tex_name = m_primary_scope_tex_name;
        ...
        if ( !m_secondary_scope_tex_name ) {
            m_secondary_scope_tex_name = READ_IF_EXISTS(
                pSettings, r_string, cNameSect(), "scope_texture_alt", NULL );
        }
    ...
    if ( !g_dedicated_server ) {
        ...
        if ( !scope_tex_name || scope_tex_name.equal( "none" ) ||
            ...
            CUIXmlInit::InitWindow( *pWpnScopeXml, scope_tex_name.c_str(), 0,
                                    m_UIScope );
        }
    }
    UpdateZoomParams();
...
void CWeapon::SwitchZoomType() {
    if ( !useSeparateUBGLKeybind ) {
        if ( m_zoomtype == 0 &&
             ( m_altAimPos || g_player_hud->m_adjust_mode ||
               ( m_modular_attachments && IsScopeAttached() &&
                 READ_IF_EXISTS( pSettings, r_bool, GetScopeName(),
...
void CWeapon::SetZoomType( u8 new_zoom_type ) {
    ...
    if ( ai().script_engine().functor( "_G.CWeapon_OnSwitchZoomType",
                                       funct ) ) {
...
float CWeapon::GetTargetHudFov() {
    ...
    float x = IsZoomed() ? easeOutQuart( m_zoom_params.m_fZoomRotationFactor )
                         : easeInQuart( m_zoom_params.m_fZoomRotationFactor );
    float factor = hud_fov_aim_factor > 0
                       ? hud_fov_aim_factor * hud_fov_aim_multiplier
                       : 1; // ltx hud_fov_aim_factor
    base = ( base * factor ) * x + base * ( 1 - x );

    /*
    if (m_zoom_params.m_fBaseZoomFactor == 0.f)
    {
            base = _lerp(base, base * (g_ironsights_factor / 1.7f),
    m_zoom_params.m_fZoomRotationFactor); clamp(base, 0.1f, 1.f);
    }
    */
...
void CWeapon::Load( LPCSTR section ) {
    ...
    if ( pSettings->line_exist( section, "flame_particles_2" ) )
        m_sFlameParticles2 =
            pSettings->r_string( section, "flame_particles_2" );
    ...
    cam_recoil.DispersionFrac = _abs( READ_IF_EXISTS(
        pSettings, r_float, section, "cam_dispersion_frac", 0.7f ) );
    ...
    if ( pSettings->line_exist( section, "zoom_cam_relax_speed" ) ) {
        zoom_cam_recoil.RelaxSpeed = _abs(
            deg2rad( pSettings->r_float( section, "zoom_cam_relax_speed" ) ) );
    ...
    if ( pSettings->line_exist( section, "zoom_cam_relax_speed_ai" ) ) {
        zoom_cam_recoil.RelaxSpeed_AI = _abs( deg2rad(
            pSettings->r_float( section, "zoom_cam_relax_speed_ai" ) ) );
    ...
    if ( pSettings->line_exist( section, "zoom_cam_max_angle" ) ) {
        zoom_cam_recoil.MaxAngleVert = _abs(
            deg2rad( pSettings->r_float( section, "zoom_cam_max_angle" ) ) );
    ...
    if ( pSettings->line_exist( section, "zoom_cam_max_angle_horz" ) ) {
        zoom_cam_recoil.MaxAngleHorz = _abs( deg2rad(
            pSettings->r_float( section, "zoom_cam_max_angle_horz" ) ) );
    ...
    if ( pSettings->line_exist( section, "zoom_cam_step_angle_horz" ) ) {
        zoom_cam_recoil.StepAngleHorz = deg2rad(
            pSettings->r_float( section, "zoom_cam_step_angle_horz" ) );
    }
    ...
    m_pdm.m_fPDM_disp_vel_factor =
        pSettings->r_float( section, "PDM_disp_vel_factor" );
    m_pdm.m_fPDM_disp_accel_factor =
        pSettings->r_float( section, "PDM_disp_accel_factor" );
    m_pdm.m_fPDM_disp_crouch = pSettings->r_float( section, "PDM_disp_crouch" );
    m_pdm.m_fPDM_disp_crouch_no_acc =
        pSettings->r_float( section, "PDM_disp_crouch_no_acc" );
    m_pdm.m_fPDM_disp_buckShot =
        READ_IF_EXISTS( pSettings, r_float, section, "PDM_disp_buckshot", 1.f );
    m_crosshair_inertion = READ_IF_EXISTS( pSettings, r_float, section,
                                           "crosshair_inertion", 5.91f );
    m_first_bullet_controller.load( section );
    fireDispersionConditionFactor =
        pSettings->r_float( section, "fire_dispersion_condition_factor" );
    ...
    misfireEndCondition = READ_IF_EXISTS( pSettings, r_float, section,
                                          "misfire_end_condition", 0.f );
    misfireStartProbability = READ_IF_EXISTS( pSettings, r_float, section,
                                              "misfire_start_prob", 0.f );
    misfireEndProbability = pSettings->r_float( section, "misfire_end_prob" );
    conditionDecreasePerShot =
        pSettings->r_float( section, "condition_shot_dec" );
    conditionDecreasePerQueueShot =
        READ_IF_EXISTS( pSettings, r_float, section, "condition_queue_shot_dec",
                        conditionDecreasePerShot );
    ...
    if ( pSettings->line_exist( section, "fire_point_silencer" ) )
        vLoadedFirePointSilencer =
            pSettings->r_fvector3( section, "fire_point_silencer" );
    else
        vLoadedFirePointSilencer = vLoadedFirePoint;

    // hands
    eHandDependence =
        EHandDependence( pSettings->r_s32( section, "hand_dependence" ) );
    ...
    m_eScopeStatus = ( ALife::EWeaponAddonStatus )pSettings->r_s32(
        section, "scope_status" );
    m_eSilencerStatus = ( ALife::EWeaponAddonStatus )pSettings->r_s32(
        section, "silencer_status" );
    m_eGrenadeLauncherStatus = ( ALife::EWeaponAddonStatus )pSettings->r_s32(
        section, "grenade_launcher_status" );

    m_altAimPos =
        READ_IF_EXISTS( pSettings, r_bool, section, "use_alt_aim_hud", false );

    m_zoom_params.m_bZoomEnabled =
        !!pSettings->r_bool( section, "zoom_enabled" );
    m_zoom_params.m_fZoomRotateTime =
        pSettings->r_float( section, "zoom_rotate_time" );
    m_fZoomRotateModifier = READ_IF_EXISTS( pSettings, r_float, section,
                                            "zoom_rotate_modifier", 1 );
    m_zoom_params.m_fBaseZoomFactor = READ_IF_EXISTS(
        pSettings, r_float, cNameSect(), "scope_zoom_factor", 0 );
    m_modular_attachments = READ_IF_EXISTS( pSettings, r_bool, section,
                                            "modular_attachments", false );

    if ( m_eScopeStatus == ALife::eAddonAttachable ) {
        if ( m_modular_attachments ) {
            LPCSTR scope_group =
                pSettings->r_string( section, "modular_scope_group" );
        ...
        shared_str scope_tex_name = READ_IF_EXISTS(
            pSettings, r_string, cNameSect(), "scope_texture", NULL );
        ...
        if ( !!scope_tex_name && !scope_tex_name.equal( "none" ) &&
            ...
            if ( !g_dedicated_server ) {
                ...
                m_scope_tex_name = scope_tex_name;
                CUIXmlInit::InitWindow( *pWpnScopeXml, scope_tex_name.c_str(),
                                        0, m_UIScope );
            }
        }
    }
    ...
    if ( m_eGrenadeLauncherStatus == ALife::eAddonAttachable ) {
        m_sGrenadeLauncherName =
            pSettings->r_string( section, "grenade_launcher_name" );
    ...
    if ( pSettings->line_exist( section, "weapon_remove_time" ) )
        m_dwWeaponRemoveTime =
            pSettings->r_u32( section, "weapon_remove_time" );
    else
    ...
    m_zoom_params.m_fSecondVPFovFactor =
        READ_IF_EXISTS( pSettings, r_float, section, "scope_lense_fov", 0.0f );
    m_zoom_params.m_bHideCrosshairInZoom = true;

    if ( pSettings->line_exist( hud_sect, "zoom_hide_crosshair" ) )
        m_zoom_params.m_bHideCrosshairInZoom =
            !!pSettings->r_bool( hud_sect, "zoom_hide_crosshair" );

    Fvector def_dof;
    def_dof.set( -1, -1, -1 );
    m_zoom_params.m_ZoomDof =
        READ_IF_EXISTS( pSettings, r_fvector3, section, "zoom_dof",
                        Fvector().set( -1, -1, -1 ) );
    m_zoom_params.m_bZoomDofEnabled =
        !def_dof.similar( m_zoom_params.m_ZoomDof );

    m_zoom_params.m_ReloadDof =
        READ_IF_EXISTS( pSettings, r_fvector4, section, "reload_dof",
                        Fvector4().set( -1, -1, -1, -1 ) );

    // Swartz: empty reload
    m_zoom_params.m_ReloadEmptyDof =
        READ_IF_EXISTS( pSettings, r_fvector4, section, "reload_empty_dof",
                        Fvector4().set( -1, -1, -1, -1 ) );
    //-Swartz

    m_bHasTracers =
        !!READ_IF_EXISTS( pSettings, r_bool, section, "tracers", true );
    m_u8TracerColorID = READ_IF_EXISTS( pSettings, r_u8, section,
                                        "tracers_color_ID", u8( -1 ) );

    // momopate
    m_bSilencedTracers =
        READ_IF_EXISTS( pSettings, r_bool, section, "silenced_tracers", false );

    string256 temp;
    for ( int i = egdNovice; i < egdCount; ++i ) {
        strconcat( sizeof( temp ), temp, "hit_probability_",
                   get_token_name( difficulty_type_token, i ) );
        m_hit_probability[ i ] =
            READ_IF_EXISTS( pSettings, r_float, section, temp, 1.f );
    }

    m_zoom_params.m_bUseDynamicZoom = READ_IF_EXISTS(
        pSettings, r_bool, section, "scope_dynamic_zoom", FALSE );
    m_zoom_params.m_sUseZoomPostprocess = 0;
    m_zoom_params.m_sUseBinocularVision = 0;

    // Added by Axel, to enable optional condition use on any item
    m_flags.set( FUsingCondition, READ_IF_EXISTS( pSettings, r_bool, section,
                                                  "use_condition", TRUE ) );

    m_APk = READ_IF_EXISTS( pSettings, r_float, section, "ap_modifier", 1.0f );

    m_bCanBeLowered =
        READ_IF_EXISTS( pSettings, r_bool, section, "can_be_lowered", false );

    m_fSafeModeRotateTime = READ_IF_EXISTS( pSettings, r_float, section,
                                            "weapon_lower_speed", 1.f );
    hud_fov_aim_multiplier = READ_IF_EXISTS( pSettings, r_float, section,
                                             "hud_fov_aim_factor", 1.f );

    UpdateUIScope();

    // Rezy safemode blend anms
    m_safemode_anm[ 0 ].name = READ_IF_EXISTS( pSettings, r_string, *hud_sect,
                                               "safemode_anm", nullptr );
    m_safemode_anm[ 1 ].name = READ_IF_EXISTS( pSettings, r_string, *hud_sect,
                                               "safemode_anm2", nullptr );
    m_safemode_anm[ 0 ].speed = READ_IF_EXISTS( pSettings, r_float, *hud_sect,
                                                "safemode_anm_speed", 1.f );
    m_safemode_anm[ 1 ].speed = READ_IF_EXISTS( pSettings, r_float, *hud_sect,
                                                "safemode_anm_speed2", 1.f );
    m_safemode_anm[ 0 ].power = READ_IF_EXISTS( pSettings, r_float, *hud_sect,
                                                "safemode_anm_power", 1.f );
    m_safemode_anm[ 1 ].power = READ_IF_EXISTS( pSettings, r_float, *hud_sect,
                                                "safemode_anm_power2", 1.f );

    m_shoot_shake_mat.identity();

    m_playFullShotAnim =
        READ_IF_EXISTS( pSettings, r_bool, *hud_sect, "rpm_anim_fix", true );
    ...
    m_nearwall_zoomed_range = READ_IF_EXISTS( pSettings, r_float, section,
                                              "nearwall_zoomed_range", 0.04f );
...
void CWeapon::LoadFireParams( LPCSTR section ) {
    cam_recoil.Dispersion =
        deg2rad( pSettings->r_float( section, "cam_dispersion" ) );
    cam_recoil.DispersionInc = 0.0f;

    if ( pSettings->line_exist( section, "cam_dispersion_inc" ) ) {
        cam_recoil.DispersionInc =
            deg2rad( pSettings->r_float( section, "cam_dispersion_inc" ) );
    }
    ...
    if ( pSettings->line_exist( section, "zoom_cam_dispersion" ) ) {
        zoom_cam_recoil.Dispersion =
            deg2rad( pSettings->r_float( section, "zoom_cam_dispersion" ) );
    }
    if ( pSettings->line_exist( section, "zoom_cam_dispersion_inc" ) ) {
        zoom_cam_recoil.DispersionInc =
            deg2rad( pSettings->r_float( section, "zoom_cam_dispersion_inc" ) );
    }

    CShootingObject::LoadFireParams( section );
};

void GetZoomData( const float scope_factor,
                  const float zoom_step_count,
                  const float min_zoom_setting,
                  float& delta,
                  float& min_zoom_factor );
void newGetZoomDelta( const float scope_factor,
                      float& delta,
                      const float min_zoom_factor,
                      float steps );
extern BOOL useNewZoomDeltaAlgorithm;

void NewGetZoomData( const float scope_factor,
                     const float zoom_step_count,
                     float& delta,
                     float& min_zoom_factor,
                     float zoom,
    ...
    float loc_min_zoom_factor =
        ( ( atan( tan( def_fov * ( 0.5 * PI / 180 ) ) / g_ironsights_factor ) /
            ( 0.5 * PI / 180 ) ) /
          0.75f ) *
        ( scope_radius > 0.0 ? scope_scrollpower : 1 );
    ...
    // Msg("min zoom factor %f, min zoom %f, loc min zoom factor %f,
    // g_ironsights_factor %f, scope_radius %f, scope_scrollpower %f,
    // zoom_step_count %f, n_zoom_step_count %f, steps %f, delta %f",
    // min_zoom_factor, min_zoom, loc_min_zoom_factor, g_ironsights_factor,
    // scope_radius, scope_scrollpower, zoom_step_count, n_zoom_step_count,
    // steps, delta);
}
...
BOOL CWeapon::net_Spawn( CSE_Abstract* DC ) {
    if ( m_zoom_params.m_bUseDynamicZoom ) {
        ...
        if ( zoomFlags.test( NEW_ZOOM ) ) {
            NewGetZoomData( m_zoom_params.m_fScopeZoomFactor * power,
                            m_zoom_params.m_fZoomStepCount, delta,
                            min_zoom_factor, GetZoomFactor() * power,
                            m_zoom_params.m_fMinBaseZoomFactor );
        } else {
            GetZoomData( m_zoom_params.m_fScopeZoomFactor * power,
                         m_zoom_params.m_fZoomStepCount,
                         m_zoom_params.m_fMinBaseZoomFactor, delta,
                         min_zoom_factor );
        }
    ...
    if ( m_modular_attachments && m_cur_scope == 0 &&
    ...
    if ( !m_ammoTypes[ m_ammoType ].c_str() ) {
        Msg( "![%s] ERROR: CWeapon::net_Spawn: m_ammoTypes[m_ammoType] is "
             "invalid, m_ammoTypes.size %d, m_ammoType %d",
             Name(), m_ammoTypes.size(), m_ammoType );
        m_ammoType = 0;
    }
...
void CWeapon::SendHiddenItem() {
    if ( !CHudItem::object().getDestroy() && m_pInventory ) {
        ...
        NET_Packet P;
        CHudItem::object().u_EventGen( P, GE_WPN_STATE_CHANGE,
                                       CHudItem::object().ID() );
        P.w_u8( u8( eHiding ) );
        ...
        CHudItem::object().u_EventSend( P,
                                        net_flags( TRUE, TRUE, FALSE, TRUE ) );
        SetPending( TRUE );
    }
}
...
void CWeapon::UpdateCL() {
    ...
    if ( ( GetNextState() == GetState() ) && IsGameTypeSingle() &&
         H_Parent() == Level().CurrentEntity() ) {
        CActor* pActor = smart_cast< CActor* >( H_Parent() );
        if ( pActor && !pActor->AnyMove() &&
             this == pActor->inventory().ActiveItem() ) {
            if ( hud_adj_mode == 0 &&
    ...
    if ( m_zoom_params.m_pNight_vision && !need_renderable() ) {
        if ( !m_zoom_params.m_pNight_vision->IsActive() ) {
            ...
            m_zoom_params.m_pNight_vision->Start(
                m_zoom_params.m_sUseZoomPostprocess, pA, false );
        }
...
bool CWeapon::need_renderable() {
    return !Device.m_SecondViewport.IsSVPFrame() &&
           !( IsZoomed() && ZoomTexture() && !IsRotatingToZoom() );
}
...
bool CWeapon::Action( u16 cmd, u32 flags ) {
    ...
    switch ( cmd ) {
                    ...
                    if ( flags & CMD_START ) {
                        if ( !IsZoomed() ) {
                            if ( !IsPending() ) {
                                ...
                                if ( GetState() != eAimStart &&
                                     HudAnimationExist( "anm_idle_aim_start" ) )
                                    SwitchState( eAimStart );
                                else if ( interruptFireOnAimToggle &&
                                          GetState() != eIdle )
                                    SwitchState( eIdle );
                            ...
                            if ( GetState() != eAimEnd &&
                                 HudAnimationExist( "anm_idle_aim_end" ) )
                                SwitchState( eAimEnd );
        ...
        case kSAFEMODE: {
            if ( pActor && flags & CMD_START && !IsPending() &&
                 m_bCanBeLowered ) {
                ...
                if ( !new_state && m_safemode_anm[ 1 ].name )
                    PlayBlendAnm( m_safemode_anm[ 1 ].name,
                                  m_safemode_anm[ 1 ].speed,
                                  m_safemode_anm[ 1 ].power, false );
                else if ( m_safemode_anm[ 0 ].name )
                    PlayBlendAnm( m_safemode_anm[ 0 ].name,
                                  m_safemode_anm[ 0 ].speed,
                                  m_safemode_anm[ 0 ].power, false );
            }
            return true;
        }
        case kCUSTOM21:
...
bool CWeapon::SwitchAmmoType( u32 flags ) {
    ...
    do {
        ...
        b2 =
            unlimited_ammo()
                ? false
                : ( !m_pInventory->GetAny( m_ammoTypes[ l_newType ].c_str() ) );
    } while ( b1 && b2 );
...
void CWeapon::SpawnAmmo( u32 boxCurr, LPCSTR ammoSect, u32 ParentID ) {
...

    {
        ...
        while ( boxCurr ) {
            l_pA->a_elapsed =
                ( u16 )( boxCurr > l_pA->m_boxSize ? l_pA->m_boxSize
                                                   : boxCurr );
            NET_Packet P;
...
float CWeapon::GetConditionMisfireProbability() const {
    ...
    if ( GetCondition() > misfireStartCondition )
        ...
        result =
            ...
            ( ( misfireStartCondition -
                GetCondition() ) * // condition goes from 1.f to 0.f
              ( misfireEndProbability -
                misfireStartProbability ) / // probability goes from 0.f to 1.f
    ...
    if ( !smart_cast< CActor* >( H_Parent() ) ) {
        ...
        if ( ai().script_engine().functor(
                 "xr_weapon_jam.GetConditionMisfireProbability", funct ) ) {
            auto gobj = smart_cast< CGameObject* >( H_Parent() );
            result = funct( lua_game_object(),
                            gobj ? gobj->lua_game_object() : nullptr, result );
        }
    }
...
bool CWeapon::IsSilencerAttached() const {
    return ( ALife::eAddonAttachable == m_eSilencerStatus &&
             0 != ( m_flagsAddOnState &
                    CSE_ALifeItemWeapon::eWeaponAddonSilencer ) ) ||
...
void CWeapon::UpdateHUDAddonsVisibility() {
    ...
    if ( m_modular_attachments ) {
        ...
        if ( ScopeAttachable() ) {
            HudItemData()->set_bone_visible( wpn_scoped_hide,
                                             !IsScopeAttached(), TRUE );
            HudItemData()->set_bone_visible( wpn_scoped_unhide,
                                             IsScopeAttached(), TRUE );
        }
    } else {
        if ( ScopeAttachable() ) {
    ...
    if ( GrenadeLauncherAttachable() ) {
        HudItemData()->set_bone_visible( wpn_grenade_launcher,
...
void CWeapon::UpdateAddonsVisibility() {
    ...
    if ( m_eScopeStatus == ALife::eAddonDisabled && bone_id != BI_NONE &&
        ...
        pWeaponVisual->LL_SetBoneVisible( bone_id, FALSE, TRUE );
        //		Log("scope", pWeaponVisual->LL_GetBoneVisible
        //(bone_id));
    }
    ...
    if ( m_eSilencerStatus == ALife::eAddonDisabled && bone_id != BI_NONE &&
         pWeaponVisual->LL_GetBoneVisible( bone_id ) ) {
        pWeaponVisual->LL_SetBoneVisible( bone_id, FALSE, TRUE );
        //		Log("silencer", pWeaponVisual->LL_GetBoneVisible
        //(bone_id));
    }
    ...
    if ( m_eGrenadeLauncherStatus == ALife::eAddonDisabled &&
         bone_id != BI_NONE && pWeaponVisual->LL_GetBoneVisible( bone_id ) ) {
        pWeaponVisual->LL_SetBoneVisible( bone_id, FALSE, TRUE );
        //		Log("gl", pWeaponVisual->LL_GetBoneVisible
        //(bone_id));
    }
...
void CWeapon::OnZoomIn() {
    ...
    // Msg("m_fRTZoomFactor %f, scope_scrollpower %f", m_fRTZoomFactor,
    // scope_scrollpower);

    if ( m_zoom_params.m_bUseDynamicZoom )
        SetZoomFactor( scope_radius > 0.0 ? m_fRTZoomFactor / scope_scrollpower
                                          : m_fRTZoomFactor );
    else
    ...
    if ( m_zoom_params.m_sUseBinocularVision.size() && IsScopeAttached() &&
        ...
        m_zoom_params.m_pVision =
            xr_new< CBinocularsVision >( m_zoom_params.m_sUseBinocularVision );
...
void CWeapon::OnZoomOut() {
    m_zoom_params.m_bIsZoomModeNow = false;
    if ( m_zoom_params.m_bUseDynamicZoom ) {
        m_fRTZoomFactor = scope_radius > 0.0
                              ? GetZoomFactor() * scope_scrollpower
                              : GetZoomFactor(); // store current
    }
...
void CWeapon::SwitchState( u32 S ) {
...
#ifndef MASTER_GOLD
    if ( bDebug ) {
        Msg( "---Server is going to send GE_WPN_STATE_CHANGE to [%d], "
             "weapon_section[%s], parent[%s]",
             S, cNameSect().c_str(),
    ...
    SetNextState( S );
    if ( CHudItem::object().Local() && !CHudItem::object().getDestroy() &&
...
void CWeapon::reload( LPCSTR section ) {
    ...
    if ( m_eScopeStatus == ALife::eAddonAttachable && m_scopes.size() ) {
        m_addon_holder_range_modifier =
            READ_IF_EXISTS( pSettings, r_float, GetScopeName(),
                            "holder_range_modifier", m_holder_range_modifier );
    ...
    m_ef_main_weapon_type = READ_IF_EXISTS( pSettings, r_u32, section,
                                            "ef_main_weapon_type", u32( -1 ) );
    m_ef_weapon_type = READ_IF_EXISTS( pSettings, r_u32, section,
...
CInventoryItem* CWeapon::can_kill( CInventory* inventory ) const {
    if ( const_cast< CWeapon* >( this )->unlimited_ammo() || GetAmmoElapsed() ||
         m_ammoTypes.empty() )
    ...
    for ( ; I != E; ++I ) {
        ...
        xr_vector< shared_str >::const_iterator i =
            std::find( m_ammoTypes.begin(), m_ammoTypes.end(),
...
const CInventoryItem* CWeapon::can_kill(
    const xr_vector< const CGameObject* >& items ) const {
    if ( const_cast< CWeapon* >( this )->unlimited_ammo() ||
         m_ammoTypes.empty() )
        return ( this );
    ...
    for ( ; I != E; ++I ) {
        const CInventoryItem* inventory_item =
            smart_cast< const CInventoryItem* >( *I );
        if ( !inventory_item )
            continue;
...
bool CWeapon::ready_to_kill() const {
    // Alundaio
    const CInventoryOwner* io =
        smart_cast< const CInventoryOwner* >( H_Parent() );
    if ( !io )
        return false;

    if ( io->inventory().ActiveItem() == NULL ||
         io->inventory().ActiveItem()->object().ID() != ID() )
        return false;
    //-Alundaio
    return ( !IsMisfire() &&
             ( ( GetState() == eIdle ) || ( GetState() == eFire ) ||
               ( GetState() == eFire2 ) ) &&
             GetAmmoElapsed() );
}

void CWeapon::InterpolateOffset( Fvector& current,
...
void CWeapon::UpdateHudAdditional( Fmatrix& trans ) {
    ...
    //============= Поворот ствола во время аима =============//
    {
        ...
        if ( ( idx == 1 || idx == 3 ) && m_modular_attachments ) {
            ...
            if ( idx == 0 ) {
                curr_offs =
                    g_player_hud->m_adjust_offset[ 0 ][ 5 ]; // pos,normal2
                curr_rot =
            ...
            if ( idx == 0 ) {
                curr_offs =
                    hi->m_measures.m_hands_offset[ 0 ][ 5 ]; // pos,normal2
                curr_rot =
        ...
        if ( idx == 4 || last_idx == 4 )
            ...
            factor = Device.fTimeDelta / ( m_zoom_params.m_fZoomRotateTime *
                                           cur_silencer_koef.zoom_rotate_time *
                                           cur_scope_koef.zoom_rotate_time *
        ...
        // Remove pending state before weapon has fully moved to the new
        // position to remove some delay
        if ( curr_offs.similar( m_hud_offset[ 0 ], .02f ) &&
             curr_rot.similar( m_hud_offset[ 1 ], .02f ) ) {
            if ( ( idx == 4 || last_idx == 4 ) && IsPending() )
                SetPending( FALSE );
            last_idx = idx;
        }
    ...
    if ( hi->m_measures.m_shooting_params.bShootShake ) {
        // Параметры сдвига
        float fShootingReturnSpeedMod =
            _lerp( hi->m_measures.m_shooting_params.m_ret_speed,
                   hi->m_measures.m_shooting_params.m_ret_speed_aim,
                   m_zoom_params.m_fZoomRotationFactor );

        float fShootingBackwOffset =
            _lerp( hi->m_measures.m_shooting_params.m_shot_offset_BACKW.x,
        ...
        // Плавное затухание сдвига от стрельбы (основное, но без линейной
        // никогда не опустит до полного 0.0f)
        m_fLR_ShootingFactor *=
            clampr( 1.f - fAvgTimeDelta * fShootingReturnSpeedMod, 0.0f, 1.0f );
        m_fUD_ShootingFactor *=
            clampr( 1.f - fAvgTimeDelta * fShootingReturnSpeedMod, 0.0f, 1.0f );
        m_fBACKW_ShootingFactor *=
            clampr( 1.f - fAvgTimeDelta * fShootingReturnSpeedMod, 0.0f, 1.0f );

        // Минимальное линейное затухание сдвига от стрельбы при покое
        // (горизонталь)
        {
        ...
        {
            float fLR_lim =
                ( m_fLR_ShootingFactor < 0.0f ? vShOffsets.x : vShOffsets.y );
            float fUD_lim =
                ( m_fUD_ShootingFactor < 0.0f ? vShOffsets.z : vShOffsets.w );
    ...
    float fStepPerUpd =
        fAvgTimeDelta / fStrafeMaxTime; // Величина изменение фактора поворота

    // Добавляем боковой наклон от движения камеры
    float fCamReturnSpeedMod = 1.5f;
    // Восколько ускоряем нормализацию наклона, полученного от движения камеры
    // (только от бедра)

    // Высчитываем минимальную скорость поворота камеры для начала инерции
    float fStrafeMinAngle = _lerp( hi->m_measures.m_strafe_offset[ 3 ][ 0 ].y,
                                   hi->m_measures.m_strafe_offset[ 3 ][ 1 ].y,
                                   m_zoom_params.m_fZoomRotationFactor );

    // Высчитываем мксимальный наклон от поворота камеры
    float fCamLimitBlend = _lerp( hi->m_measures.m_strafe_offset[ 3 ][ 0 ].x,
                                  hi->m_measures.m_strafe_offset[ 3 ][ 1 ].x,
                                  m_zoom_params.m_fZoomRotationFactor );

    // Считаем стрейф от поворота камеры
    if ( abs( fYMag ) >
         ( m_fLR_CameraFactor == 0.0f ? fStrafeMinAngle : 0.0f ) ) {
        ...
        if ( m_fLR_CameraFactor < 0.0f ) {
            m_fLR_CameraFactor +=
                fStepPerUpd * ( bForAim ? 1.0f : fCamReturnSpeedMod );
    ...
    // Восколько быстро меняем направление направление наклона, если оно в
    // другую сторону от текущего
    u32 iMovingState = pActor->MovingState();
    if ( ( iMovingState & mcLStrafe ) != 0 ) {
        // Движемся влево
        float fVal =
            ( m_fLR_MovingFactor > 0.f ? fStepPerUpd * fChangeDirSpeedMod
                                       : fStepPerUpd );
        ...
        if ( m_fLR_MovingFactor < 0.0f ) {
            if ( g_use_non_linear_inertia ) {
                m_fLR_MovingFactor +=
                    fStepPerUpd * ( 0.1f - 2.f * m_fLR_MovingFactor );
            } else {
    ...
    {
        bool bEnabled =
            ( hi->m_measures.m_strafe_offset[ 2 ][ _idx ].x != 0.0f );
        if ( !bEnabled )
            continue;
    ...
    //============= Инерция оружия =============//
    // Параметры инерции
    float fInertiaSpeedMod =
        _lerp( hi->m_measures.m_inertion_params.m_tendto_speed,
               hi->m_measures.m_inertion_params.m_tendto_speed_aim,
               m_zoom_params.m_fZoomRotationFactor );

    float fInertiaReturnSpeedMod =
        _lerp( hi->m_measures.m_inertion_params.m_tendto_ret_speed,
               hi->m_measures.m_inertion_params.m_tendto_ret_speed_aim,
               m_zoom_params.m_fZoomRotationFactor );

    float fInertiaMinAngle =
        _lerp( hi->m_measures.m_inertion_params.m_min_angle,
    ...
    bool bIsInertionPresent =
        m_fLR_InertiaFactor != 0.0f || m_fUD_InertiaFactor != 0.0f;
    if ( abs( fYMag ) > fInertiaMinAngle || bIsInertionPresent ) {
        ...
        m_fLR_InertiaFactor -=
            ( fYMag * fAvgTimeDelta * fSpeed ); // Горизонталь (м.б. > |1.0|)
    }
    ...
    if ( abs( fPMag ) > fInertiaMinAngle || bIsInertionPresent ) {
        ...
        m_fUD_InertiaFactor -=
            ( fPMag * fAvgTimeDelta * fSpeed ); // Вертикаль (м.б. > |1.0|)
    }

    clamp( m_fLR_InertiaFactor, -1.0f, 1.0f );
    clamp( m_fUD_InertiaFactor, -1.0f, 1.0f );

    // Плавное затухание инерции (основное, но без линейной никогда не опустит
    // инерцию до полного 0.0f)
    m_fLR_InertiaFactor *=
        clampr( 1.f - fAvgTimeDelta * fInertiaReturnSpeedMod, 0.0f, 1.0f );
    m_fUD_InertiaFactor *=
        clampr( 1.f - fAvgTimeDelta * fInertiaReturnSpeedMod, 0.0f, 1.0f );

    // Минимальное линейное затухание инерции при покое (горизонталь)
    if ( fYMag == 0.0f ) {
        float fRetSpeedMod = ( fYMag == 0.0f ? 1.0f : 0.75f ) *
                             ( fInertiaReturnSpeedMod * 0.075f );
        if ( m_fLR_InertiaFactor < 0.0f ) {
            if ( g_use_non_linear_inertia ) {
                m_fLR_InertiaFactor += ( 0.3f - m_fLR_InertiaFactor ) *
                                       fAvgTimeDelta * fRetSpeedMod;
            } else {
    ...
    if ( fPMag == 0.0f ) {
        float fRetSpeedMod = ( fPMag == 0.0f ? 1.0f : 0.75f ) *
                             ( fInertiaReturnSpeedMod * 0.075f );
        if ( m_fUD_InertiaFactor < 0.0f ) {
            if ( g_use_non_linear_inertia ) {
                m_fUD_InertiaFactor += ( 0.3f - m_fUD_InertiaFactor ) *
                                       fAvgTimeDelta * fRetSpeedMod;
            } else {
    ...
    Fvector curr_offs;
    curr_offs = { fLR_lim * -1.f * m_fLR_InertiaFactor,
                  fUD_lim * m_fUD_InertiaFactor, 0.0f };
...
void CWeapon::AddHUDShootingEffect() {
    ...
    if ( hi != nullptr ) {
        if ( !hi->m_measures.m_shooting_params.bShootShake )
            return;
        fPowerMin = clampr( hi->m_measures.m_shooting_params.m_min_LRUD_power,
                            0.0f, 0.99f );
    }
    ...
    m_fLR_ShootingFactor +=
        ( m_fLR_ShootingFactor >= 0.0f ? fPowerMin : -fPowerMin );

    m_fUD_ShootingFactor = ::Random.randF( -fPowerRnd, fPowerRnd );
    m_fUD_ShootingFactor +=
        ( m_fUD_ShootingFactor >= 0.0f ? fPowerMin : -fPowerMin );
}
...
bool CWeapon::IsNecessaryItem( const shared_str& item_sect ) {
    return ( std::find( m_ammoTypes.begin(), m_ammoTypes.end(), item_sect ) !=
             m_ammoTypes.end() );
}
...
bool CWeapon::render_item_ui_query() {
    ...
    bool res = b_is_active_item && IsZoomed() && ZoomHideCrosshair() &&
               ZoomTexture() && !IsRotatingToZoom();
    return res;
}
    ...
    if ( IsGameTypeSingle() ) {
        if ( m_pInventory ) {
            return inventory_owner().unlimited_ammo() &&
                   m_DefaultCartridge.m_flags.test(
                       CCartridge::cfCanBeUnlimited );
        } else
            return false;
    }
...
float CWeapon::GetMagazineWeight(
    const decltype( CWeapon::m_magazine )& mag ) const {
    float res = 0;
    const char* last_type = nullptr;
    float last_ammo_weight = 0;
    for ( auto& c : mag ) {
        // Usually ammos in mag have same type, use this fact to improve
        // performance
...
void CWeapon::debug_draw_firedeps() {
    ...
    if ( hud_adj_mode == 5 || hud_adj_mode == 6 || hud_adj_mode == 7 ) {
        ...
        if ( hud_adj_mode == 5 )
            render.draw_aabb( get_LastFP(), 0.005f, 0.005f, 0.005f,
                              D3DCOLOR_XRGB( 255, 0, 0 ) );

        if ( hud_adj_mode == 6 )
            render.draw_aabb( get_LastFP2(), 0.005f, 0.005f, 0.005f,
                              D3DCOLOR_XRGB( 0, 0, 255 ) );

        if ( hud_adj_mode == 7 )
            render.draw_aabb( get_LastSP(), 0.005f, 0.005f, 0.005f,
...
const float& CWeapon::hit_probability() const {
    VERIFY( ( g_SingleGameDifficulty >= egdNovice ) &&
            ( g_SingleGameDifficulty <= egdMaster ) );
...
void CWeapon::OnStateSwitch( u32 S, u32 oldState ) {
    ...
    // NPC reload edges for Lua (npc_on_weapon_reload_start / _stop). Actor
    // reloads already surface through the actor binder callback
    // (actor_on_weapon_reload); a stalker's reload was observable only by
    // polling get_state() == eReload per tick. Fired here because every weapon
    // type converges on CWeapon::OnStateSwitch - the tri-state shotgun path
    // skips CWeaponMagazined's override for S == eReload. Best-effort stop
    // edge: a weapon detached mid-reload (death, drop) loses its stalker parent
    // and fires nothing - get_state() stays the authoritative read.
    ...
    if ( GetState() == eReload ) {
        if ( iAmmoElapsed == 0 ) // Swartz: re-written to use reload empty DOF
        {
            if ( H_Parent() == Level().CurrentEntity() &&
                 !fsimilar( m_zoom_params.m_ReloadEmptyDof.w, -1.0f ) ) {
                CActor* current_actor = smart_cast< CActor* >( H_Parent() );
                if ( current_actor )
                    current_actor->Cameras().AddCamEffector(
                        xr_new< CEffectorDOF >(
                            m_zoom_params.m_ReloadEmptyDof ) );
            }
        } else {
            if ( H_Parent() == Level().CurrentEntity() &&
                 !fsimilar( m_zoom_params.m_ReloadDof.w, -1.0f ) ) {
                CActor* current_actor = smart_cast< CActor* >( H_Parent() );
                if ( current_actor )
                    current_actor->Cameras().AddCamEffector(
                        xr_new< CEffectorDOF >( m_zoom_params.m_ReloadDof ) );
            }
        }
    }

    if ( ParentIsActor() &&
         smart_cast< CActor* >( H_Parent() )->inventory().ActiveItem() ==
             this &&
         GetState() != eIdle )
        Actor()->set_safemode( false );
}
...
float CWeapon::GetMinScopeZoomFactor() const {
    ...
    if ( zoomFlags.test( NEW_ZOOM ) ) {
        NewGetZoomData( m_zoom_params.m_fScopeZoomFactor * power,
                        m_zoom_params.m_fZoomStepCount, delta, min_zoom_factor,
                        GetZoomFactor() * power,
                        m_zoom_params.m_fMinBaseZoomFactor );
    } else {
        GetZoomData( m_zoom_params.m_fScopeZoomFactor * power,
...
u32 CWeapon::Cost() const {
    ...
    if ( iAmmoElapsed ) {
        float w =
            pSettings->r_float( m_ammoTypes[ m_ammoType ].c_str(), "cost" );
        float bs =
            pSettings->r_float( m_ammoTypes[ m_ammoType ].c_str(), "box_size" );
...
void CWeapon::UpdateSecondVP() {
    if ( !( ParentIsActor() && ( m_pInventory != NULL ) &&
    ...
    Device.m_SecondViewport.SetSVPActive(
        ...
        IsSecondVPZoomPresent() &&
        m_zoom_params.m_fZoomRotationFactor > 0.05f );
}
...
Fmatrix CWeapon::RayTransform() {
    ...
    if ( GetHUDmode() ) {
        ...
        if ( psActorFlags.test( AF_FIREDIR_THIRD_PERSON ) ) {
            // If firedir is enabled, override the barrel orientation with the
            // HUD equivalent
...
// v2v3v4: fix ctd when zooming into about to be destroyed object with detector
// scopes
