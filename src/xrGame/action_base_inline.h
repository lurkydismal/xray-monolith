...
IC void CBaseAction::debug_log( const EActionStates state_state ) const {
    switch ( state_state ) {
        case eActionStateConstructed: {
            Msg( "[%6d] action %s is constructed", Device.dwTimeGlobal,
                 m_action_name );
            break;
        }
        case eActionStateSetup: {
            Msg( "[%6d] action %s is setup", Device.dwTimeGlobal,
                 m_action_name );
            break;
        }
        case eActionStateInitialized: {
            Msg( "[%6d] action %s is initialized", Device.dwTimeGlobal,
                 m_action_name );
            break;
        }
        case eActionStateExecuted: {
            Msg( "[%6d] action %s is executed", Device.dwTimeGlobal,
                 m_action_name );
            break;
        }
        case eActionStateFinalized: {
            Msg( "[%6d] action %s is finalized", Device.dwTimeGlobal,
                 m_action_name );
            break;
        }
        default:
...
TEMPLATE_SPECIALIZATION
IC void CBaseAction::set_property( const _condition_type& condition_id,
...
TEMPLATE_SPECIALIZATION
IC const typename CBaseAction::_value_type& CBaseAction::property(
