...
public:
    void load_raw( const CLevelGraph* level_graph,
                   const CGameLevelCrossTable* cross,
                   const CGameGraph* game_graph,
                   IReader& stream );
    IC const CPatrolPath* path( shared_str patrol_name,
                                bool no_assert = false ) const;
    const CPatrolPath* safe_path( shared_str patrol_name,
                                  bool no_assert = false,
                                  bool on_level = false ) const;
