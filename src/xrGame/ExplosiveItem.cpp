...
//						действием различных хитов
//(канистры, 						балоны с газом и т.д.)
////////////////////////////////////////////////////////////////////////////
...
void CExplosiveItem::Load( LPCSTR section ) {
    ...
    CDelayedActionFuse::Initialize(
        pSettings->r_float( section, "time_to_explode" ),
...
void CExplosiveItem::StartTimerEffects() {
    CParticlesPlayer::StartParticles(
        pSettings->r_string( *cNameSect(), "set_timer_particles" ),
...
void CExplosiveItem::shedule_Update( u32 dt ) {
    ...
    if ( CDelayedActionFuse::isActive() &&
         CDelayedActionFuse::Update( GetCondition() ) ) {
        Fvector normal;
        FindNormal( normal );
        CExplosive::GenExplodeEvent( Position(), normal );
        CParticlesPlayer::StopParticles( ID(), BI_NONE, true );
        ::luabind::functor< void > funct;
        if ( ai().script_engine().functor( "_G.CExplosiveItem__OnExplode",
                                           funct ) ) {
...
void CExplosiveItem::ActivateExplosionBox( const Fvector& size,
                                           Fvector& in_out_pos ) {
