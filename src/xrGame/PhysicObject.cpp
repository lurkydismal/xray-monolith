...
BOOL CPhysicObject::net_Spawn( CSE_Abstract* DC ) {
    ...
    if ( !PPhysicsShell()->isBreakable() && !CScriptBinder::object() &&
         !CPHSkeleton::IsRemoving() )
        SheduleUnregister();
...
void CPhysicObject::create_collision_model() {
    ...
    if ( ini && ini->section_exist( "collide" ) &&
         ini->line_exist( "collide", "mesh" ) &&
         ini->r_bool( "collide", "mesh" ) ) {
    ...
    switch(m_type) {
            ...
            case epotFixedChain:
            case epotFreeChain :
            case epotSkeleton  :	collidable.model =
    xr_new<CCF_Skeleton>(this);	break;
...
void CPhysicObject::play_bones_sound() {
    if ( !bones_snd_player ) {
        Msg( "! no sound loaded for obj: %s, model :%s - can not play",
             cName().c_str(), cNameVisual().c_str() );
        return;
    }
...
static void door_ignore( bool& do_collide,
                         bool bo1,
                         dContact& c,
                         SGameMtl* /*material_1*/,
                         SGameMtl* /*material_2*/ ) {
...
#ifdef DEBUG
            VERIFY2( ( !!PKinematicsAnimated
                             ->LL_MotionID( visual->startup_animation.c_str() )
                             .valid() ),
                     ( make_string( " animation %s not faund ",
                                    visual->startup_animation.c_str() ) +
                       dbg_object_base_dump_string( this ) )
                         .c_str() );
#endif
            m_anim_blend = m_anim_script_callback.play_cycle(
                PKinematicsAnimated, visual->startup_animation );
        }
...
#ifdef DEBUG
    if ( !b )
        Msg( " ! can not control anim - model is not animated name[%s] "
             "sect[%s] visual[%s]",
             name, sect, visual );
#endif
    return !!b;
...
void CPhysicObject::run_anim_forward() {
    if ( !check_blend( m_anim_blend, cName().c_str(), cNameSect().c_str(),
                       cNameVisual().c_str() ) )
        return;
...
#ifdef DEBUG
        Msg(
            " ! can not set blend time %f - it must be in range 0 - "
            "%f(timeTotal) obj: %s, model: %s, anim: %s",
            time, m_anim_blend->timeTotal, cName().c_str(),
            cNameVisual().c_str(),
...
void CPhysicObject::CreateSkeleton( CSE_ALifeObjectPhysic* po ) {
    ...
    m_pPhysicsShell = P_build_Shell(
        this, !po->_flags.test( CSE_PHSkeleton::flActive ), fixed_bones );
    ApplySpawnIniToPhysicShell( &po->spawn_ini(), m_pPhysicsShell,
                                fixed_bones[ 0 ] != '\0' );
    ApplySpawnIniToPhysicShell(
        smart_cast< IKinematics* >( Visual() )->LL_UserData(), m_pPhysicsShell,
        fixed_bones[ 0 ] != '\0' );
}
...
#ifdef CPHYSICOBJECT_CHANGE
    m_physic_contact_callback = READ_IF_EXISTS(
        pSettings, r_string, cNameSect_str(), "on_physic_contact", NULL );
    if ( m_physic_contact_callback && strlen( m_physic_contact_callback ) ) {
        if ( ai().script_engine().functor( m_physic_contact_callback,
                                           m_physic_contact_function ) ==
             false ) {
            m_physic_contact_callback = NULL;
        }
    }
...
void CPhysicObject::UpdateCL() {
    ...
    if ( m_pPhysicsShell->PPhysicsShellAnimator() ) {
        m_pPhysicsShell->AnimatorOnFrame( !!Render->ViewBase.testSphere_dirty(
            SpatialComponent->spatial.sphere.P,
...
void CPhysicObject::AddElement( CPhysicsElement* root_e, int id ) {
    ...
    if ( !( m_type == epotFreeChain && root_e == 0 ) ) {
        CPhysicsJoint* J =
            P_create_Joint( CPhysicsJoint::full_control, root_e, E );
...
void CPhysicObject::CreateBody( CSE_ALifeObjectPhysic* po ) {
    ...
    switch ( m_type ) {
        case epotBox: {
            m_pPhysicsShell = P_build_SimpleShell(
                this, m_mass,
                !po->_flags.test( CSE_ALifeObjectPhysic::flActive ) );
        } break;
        case epotFixedChain:
        case epotFreeChain: {
...
void CPhysicObject::InitServerObject( CSE_Abstract* D ) {
    ...
    CSE_ALifeObjectPhysic* l_tpALifePhysicObject =
        smart_cast< CSE_ALifeObjectPhysic* >( D );
...
//////////////////////////////////////////////////////////////////////////
/*
DEFINE_MAP_PRED	(LPCSTR,	CPhysicsJoint*,	JOINT_P_MAP,	JOINT_P_PAIR_IT,
pred_str);
...
bool CPhysicObject::is_ai_obstacle() const {
    return !!( READ_IF_EXISTS( pSettings, r_bool, cNameSect(), "is_ai_obstacle",
                               true ) );
}
...
void CPhysicObject::net_Export_PH_Params( NET_Packet& P,
                                          SPHNetState& State,
                                          mask_num_items& num_items ) {
    ...
    if ( !( num_items.mask &
            CSE_ALifeObjectPhysic::inventory_item_angular_null ) ) {
        /*	clamp
    ...
    if ( !( num_items.mask &
            CSE_ALifeObjectPhysic::inventory_item_linear_null ) ) {
...
void CPhysicObject::net_Import_PH_Params( NET_Packet& P,
                                          net_update_PItem& N,
                                          mask_num_items& num_items ) {
...
void CPhysicObject::PH_I_CrPr() // actions & operations between two phisic
                                // prediction steps
{};
...
void CPhysicObject::Interpolate() {
    ...
    if ( !this->H_Parent() && this->getVisible() && this->m_pPhysicsShell &&
         !OnServer() && p->NET_IItem.size() ) {
...
float CPhysicObject::interpolate_states( net_update_PItem const& first,
...
bool CPhysicObject::get_door_vectors( Fvector& closed, Fvector& open ) const {
    ...
    // Fvector door_dir_bone; start_bone_pos.transform_dir(door_dir_bone,
    // box.m_rotate.i );
    ...
    if ( dbg_draw_doors ) {
        ...
        DBG_DrawLine( pos, Fvector().add( pos, open ),
                      D3DCOLOR_XRGB( 0, 255, 0 ) );
        DBG_DrawLine( pos, Fvector().add( pos, closed ),
                      D3DCOLOR_XRGB( 255, 0, 0 ) );

        DBG_DrawLine( pos1, Fvector().add( pos1, det_vector ),
...
#ifdef CPHYSICOBJECT_CHANGE
void CPhysicObject::PhysicContactCallback( bool& do_colide,
                                           bool bo1,
                                           dContact& c,
                                           SGameMtl* material_1,
                                           SGameMtl* material_2 ) {
    ...
    CGameObject* obj =
        ( gd1 ) ? smart_cast< CGameObject* >( gd1->ph_ref_object ) : NULL;
    CGameObject* who =
        ( gd2 ) ? smart_cast< CGameObject* >( gd2->ph_ref_object ) : NULL;
...
bool CPhysicObject::PhysicContactFunction( CGameObject* who ) {
    if ( m_physic_contact_callback ) {
        return m_physic_contact_function( cast_game_object()->lua_game_object(),
                                          who->lua_game_object() );
    }
    return true;
}
