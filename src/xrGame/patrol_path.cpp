...
CPatrolPath& CPatrolPath::load_raw( const CLevelGraph* level_graph,
    ...
    for ( u32 i = 0; i < vertex_count; ++i ) {
        add_vertex( CPatrolPoint( this ).load_raw( level_graph, cross,
                                                   game_graph, stream ),
                    i );
    }
...
CPatrolPath& CPatrolPath::load_from_config( CInifile* ini_paths,
                                            LPCSTR patrol_name ) {
    R_ASSERT3( ini_paths->line_exist( patrol_name, "points" ),
               "Missing key 'points' in patrol path", patrol_name );
    xr_string points_csv = ini_paths->r_string( patrol_name, "points" );
    xr_vector< xr_string > points =
        points_csv.SplitStringMulti( ",", false, true );
    ...
    for ( int idx = 0; idx < points.size(); idx++ ) {
        ...
        add_vertex( CPatrolPoint( this ).load_from_config(
                        ini_paths, patrol_name, point_name ),
                    idx );
    ...
    for ( int idx = 0; idx < points.size(); idx++ ) {
        ...
        xr_string links_csv =
            ini_paths->r_string( patrol_name, links_csv_key.c_str() );
        xr_vector< xr_string > links =
            links_csv.SplitStringMulti( ",", false, true );

        for ( xr_string link : links ) {
            // Link current point to target points
            std::pair< u32, float > link_info =
                parse_point_link( patrol_name, link, vertex_ids_by_name );
            add_edge( idx, link_info.first, link_info.second );
            Msg( "[PP] Linked %d to %d with a probability of %f",
                 link_info.first, idx, link_info.second );
        }
    }
...
std::pair< u32, float > CPatrolPath::parse_point_link(
    LPCSTR patrol_name,
    xr_string link,
    xr_map< shared_str, u32 > vertex_ids_by_name ) {
    ...
    R_ASSERT4( matched, "Bad format for patrol path link", patrol_name,
               link.c_str() );
    ...
    R_ASSERT4( I != vertex_ids_by_name.end(),
               "Patrol point link target does not exist", patrol_name,
               target.c_str() );
