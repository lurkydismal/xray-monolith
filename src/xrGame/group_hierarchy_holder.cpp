...
//Maksimchuk 	Description : Group hierarchy holder
////////////////////////////////////////////////////////////////////////////
...
void CGroupHierarchyHolder::register_in_group( CEntity* member ) {
    VERIFY( member );
    MEMBER_REGISTRY::iterator I =
        std::find( m_members.begin(), m_members.end(), member );
    VERIFY3( I == m_members.end(),
             "Specified group member has already been found",
             *member->cName() );
...
void CGroupHierarchyHolder::unregister_in_group( CEntity* member ) {
    ...
    MEMBER_REGISTRY::iterator I =
        std::find( m_members.begin(), m_members.end(), member );
    VERIFY3( I != m_members.end(), "Specified group member cannot be found",
             *member->cName() );
    m_members.erase( I );
}
