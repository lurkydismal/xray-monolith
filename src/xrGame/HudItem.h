...
#define TENDTO_SPEED_RET \
    5.0f // Модификатор силы отката инерции (больше - быстрее)
#define TENDTO_SPEED_RET_AIM 5.0f // (Для прицеливания)
#define INERT_MIN_ANGLE \
    0.0f // Минимальная сила наклона, необходимая для старта инерции
...
enum ENearWallMode { NW_OFF = 0, NW_FOV, NW_POS, NW_MAX };

enum ENearWallTrace { NT_CAM = 0, NT_ITEM, NT_MAX };
...
protected:
    ...
    float
        m_fLR_CameraFactor; // Фактор бокового наклона худа при ходьбе [-1; +1]
    float m_fLR_MovingFactor; // Фактор бокового наклона худа при движении
                              // камеры [-1; +1]
    float m_fLR_InertiaFactor; // Фактор горизонтальной инерции худа при
                               // движении камеры [-1; +1]
    float m_fUD_InertiaFactor; // Фактор вертикальной инерции худа при движении
                               // камеры [-1; +1]
...
public:
    ...
    virtual void PlaySound( LPCSTR alias,
    ...
    u32 PlayHUDMotion( shared_str M,
                       BOOL bMixIn,
                       CHudItem* W,
                       u32 state,
                       float speed = 1.f,
                       float end = 0.f,
                       bool bMixIn2 = true );
    u32 PlayHUDMotion_noCB( const shared_str& M,
                            BOOL bMixIn,
                            float speed = 1.f,
                            bool bMixIn2 = true );
    ...
    IC void SetHudInertionEnabled( BOOL B ) {
...
public:
    ...
    bool has_object() const { return m_object; }
...
public:
    ...
    IC HUD_CALLBACK_VECTOR& visual_callbacks() { return ( m_visual_callback ); }
#endif
};
