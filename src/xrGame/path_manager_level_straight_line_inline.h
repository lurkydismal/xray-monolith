...
#define CLevelStraightLinePathManager                                    \
    CPathManager<                                                        \
        ...
        SStraightLineParams< _dist_type, _index_type, _iteration_type >, \
...
IC void CLevelStraightLinePathManager::create_path( T& vertex ) {
    ...
    _dist_type fCumulativeDistance = 0, fLastDirectDistance = 0,
               fDirectDistance;
    ...
    for ( ++I; I != E; ++I ) {
        u32 vertex_id = graph->check_position_in_direction(
            dwNode, tPosition, graph->vertex_position( *I ) );
        if ( graph->valid_vertex_id( vertex_id ) )
            fDirectDistance =
                tPosition.distance_to( graph->vertex_position( *I ) );
        else
        ...
        if ( fCumulativeDistance + fLastDirectDistance >=
    ...
    u32 vertex_id = graph->check_position_in_direction(
        dwNode, tPosition, m_parameters->m_dest_point );
    if ( graph->valid_vertex_id( vertex_id ) )
