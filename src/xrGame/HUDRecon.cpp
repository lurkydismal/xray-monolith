                ...
                if ( EA && EA->g_Alive() && EA->cast_base_monster() ) {
                    ...
                    if ( our_inv_owner && others_inv_owner ) {
                        ...
                        if ( fuzzyShowInfo > 0.5f ) {
                            CStringTable strtbl;
                            line1 = strtbl.translate( others_inv_owner->Name() )
                                        .c_str();
                            line2 = strtbl
                                        .translate(
                                            others_inv_owner->CharacterInfo()
                                                .Community()
                                                .id() )
                                        .c_str();
                        }
                ...
                if ( EA && ( EA->GetfHealth() > 0 ) ) {
                    if ( pCurEnt && GameID() & eGameIDSingle ) {
                        if ( GameID() & eGameIDDeathmatch )
                            ...
                            if ( EA->g_Team() != pCurEnt->g_Team() )
                                color = C_ON_ENEMY;
                            else
                                color = C_ON_FRIEND;
                        };
                        if ( dist >= recon_mindist && dist <= recon_maxdist ) {
                            float ddist = ( dist - recon_mindist ) /
                                          ( recon_maxdist - recon_mindist );
                            float dspeed =
                                recon_minspeed +
                                ( recon_maxspeed - recon_minspeed ) * ddist;
                            fuzzyShowInfo += Device.fTimeDelta / dspeed;
                        } else {
                            if ( dist < recon_mindist )
                                fuzzyShowInfo +=
                                    recon_minspeed * Device.fTimeDelta;
                            else
                                fuzzyShowInfo = 0;
                        };
...
void CHUDRecon::Render() const {
    ...
    if ( psHUD_Flags.test( HUD_INFO ) ) {
        CStringTable strtbl;
        F->SetColor( subst_alpha(
            color, u8( iFloor( 255.f * ( fuzzyShowInfo - 0.5f ) * 2.f ) ) ) );
