...
#define CGameManagerTemplate                                         \
    CBasePathManager< CGameGraph, _VertexEvaluator, _vertex_id_type, \
                      _index_type >
...
IC bool CGameManagerTemplate::actual() const {
    return ( inherited::actual(
        m_object->object().ai_location().game_vertex_id(), dest_vertex_id() ) );
}
...
IC bool CGameManagerTemplate::completed() const {
    if ( path().empty() ||
         ( m_intermediate_index >= ( _vertex_id_type )path().size() - 1 ) )
