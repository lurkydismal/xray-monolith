...
#ifdef DEBUG
    //	Msg			("[LSS] spawning object
    //[%d][%d][%s][%s]",ID,ID_Parent,name(),name_replace());
#endif
}
...
void CSE_ALifeDynamicObject::on_register() {
    ...
    if ( ai().script_engine().functor( "_G.CSE_ALifeDynamicObject_on_register",
                                       funct ) )
        funct( ( u16 )ID );
}
...
void CSE_ALifeDynamicObject::on_unregister() {
    ...
    if ( ai().script_engine().functor(
             "_G.CSE_ALifeDynamicObject_on_unregister", funct ) )
        funct( ( u16 )ID );
...
bool CSE_ALifeDynamicObject::synchronize_location() {
    ...
    if ( !ai().level_graph().valid_vertex_position( o_Position ) ||
         ai().level_graph().inside( ai().level_graph().vertex( m_tNodeID ),
                                    o_Position ) )
        return ( true );

    u32 const new_vertex_id =
    ...
    GameGraph::_GRAPH_ID tGraphID =
        ai().cross_table().vertex( m_tNodeID ).game_vertex_id();
    if ( tGraphID != m_tGraphID ) {
        if ( !m_bOnline ) {
            ...
            VERIFY( ai().game_graph().vertex( tGraphID )->level_id() ==
                    alife().graph().level().level_id() );
            m_tGraphID = tGraphID;
        }
    }
...
void CSE_ALifeDynamicObject::try_switch_online() {
    CSE_ALifeSchedulable* schedulable =
        smart_cast< CSE_ALifeSchedulable* >( this );
    ...
    if ( alife().graph().actor()->o_Position.distance_to( o_Position ) >
         alife().online_distance() ) {
        on_failed_switch_online();
        return;
    }
...
void CSE_ALifeInventoryBox::add_online( const bool& update_registries ) {
    ...
    ClientID clientID;
    clientID.set( object->alife().server().GetServerClient()
                      ? object->alife().server().GetServerClient()->ID.value()
                      : 0 );
    ...
    for ( ; I != E; ++I ) {
        ...
        R_ASSERT2( l_tpALifeInventoryItem,
                   "Non inventory item object has parent?!" );
        l_tpALifeInventoryItem->base()->s_flags.or_( M_SPAWN_UPDATE );
        ...
        Msg( "[LSS][%d] Going online [%d][%s][%d] with parent [%d][%s] on '%s'",
             ...
             l_tpALifeInventoryItem->base()->ID, ID, name_replace(),
             "*SERVER*" );
#endif
        ...
        object->alife().server().Process_spawn(
            tNetPacket, clientID, FALSE, l_tpALifeInventoryItem->base() );
        l_tpALifeDynamicObject->s_flags.and_( u16( -1 ) ^ M_SPAWN_UPDATE );
...
void CSE_ALifeInventoryBox::add_offline(
    ...
    for ( u32 i = 0, n = saved_children.size(); i < n; ++i ) {
        ...
        // R_ASSERT(child);
        if ( !child ) {
            Msg( "[DO] can't switch child [%d] offline, it's null",
                 saved_children[ i ] );
            continue;
        }
        ...
        Msg(
            ...
            Device.dwFrame, Device.dwTimeGlobal,
            inventory_item->base()->name_replace(), inventory_item->base()->ID,
            ID, name_replace(), "*SERVER*" );
#endif

        ALife::_OBJECT_ID item_id = inventory_item->base()->ID;
        inventory_item->base()->ID =
            object->alife().server().PerformIDgen( item_id );
        ...
        //		object->alife().graph().attach
        //(*object,inventory_item,child->m_tGraphID,true);
        alife().graph().remove( child, child->m_tGraphID );
        children.push_back( child->ID );
        child->ID_Parent = ID;
    }

    CSE_ALifeDynamicObjectVisual::add_offline( saved_children,
                                               update_registries );
}
...
void CSE_ALifeDynamicObject::clear_client_data() {
    ...
    if ( !client_data.empty() )
        Msg( "CSE_ALifeDynamicObject::switch_offline: client_data is cleared "
             "for [%d][%s]",
             ID, name_replace() );
#endif // DEBUG
