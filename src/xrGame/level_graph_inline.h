...
ICF u32 CLevelGraph::vertex( const CVertex* vertex_p ) const {
    VERIFY( ( vertex_p >= m_nodes ) &&
            valid_vertex_id( u32( vertex_p - m_nodes ) ) );
...
IC void CLevelGraph::unpack_xz( const CLevelGraph::CPosition& vertex_position,
                                u32& x,
                                u32& z ) const {
...
template < typename T >
IC void CLevelGraph::unpack_xz( const CLevelGraph::CVertex& vertex,
                                T& x,
                                T& z ) const {
...
ICF const Fvector CLevelGraph::vertex_position(
    ...
    dest_position.y =
        ( float( source_position.y() ) / 65535 ) * header().factor_y() +
        header().box().min.y;
...
IC const CLevelGraph::CPosition& CLevelGraph::vertex_position(
    ...
    VERIFY( iFloor( ( source_position.z - header().box().min.z ) /
                        header().cell_size() +
                    .5f ) < ( int )m_row_length );
    int pxz = iFloor( ( source_position.x - header().box().min.x ) /
                          header().cell_size() +
                      .5f ) *
                  m_row_length +
              iFloor( ( source_position.z - header().box().min.z ) /
                          header().cell_size() +
                      .5f );
    int py = iFloor( 65535.f * ( source_position.y - header().box().min.y ) /
                         header().factor_y() +
                     EPS_S );
...
IC const Fvector
CLevelGraph::vertex_position( const CLevelGraph::CVertex& vertex ) const {
    Fvector position;
...
IC bool CLevelGraph::inside(
    ...
    const CLevelGraph::CPosition& vertex_position ) const {
    return ( vertex_position.xz() == vertex.position().xz() );
}

IC bool CLevelGraph::inside( const CLevelGraph::CVertex& vertex,
                             const Fvector& position ) const {
...
IC bool CLevelGraph::inside( const CVertex* vertex,
                             const Fvector& position ) const {
...
IC bool CLevelGraph::inside( const u32 vertex_id,
                             const Fvector& position ) const {
...
IC bool CLevelGraph::inside( const CLevelGraph::CVertex& vertex,
    ...
    return ( inside( vertex, _vertex_position ) &&
             ( _abs( vertex_position( vertex ).y -
                     vertex_position( _vertex_position ).y ) <= epsilon ) );
}
...
IC bool CLevelGraph::inside( const u32 vertex_id,
    ...
    int pxz =
        iFloor( ( position.x - header().box().min.x ) / header().cell_size() +
                .5f ) *
            m_row_length +
        iFloor( ( position.y - header().box().min.z ) / header().cell_size() +
                .5f );
    VERIFY( pxz < ( 1 << MAX_NODE_BIT_COUNT ) - 1 );
...
IC float CLevelGraph::vertex_plane_y( const CLevelGraph::CVertex& vertex,
                                      const float X,
                                      const float Z ) const {
...
IC float CLevelGraph::vertex_plane_y(
...
IC float CLevelGraph::vertex_plane_y( const u32 vertex_id,
                                      const float X,
                                      const float Z ) const {
...
ICF bool CLevelGraph::CVertex::operator<(
    const CLevelGraph::CVertex& vertex ) const {
...
IC void CLevelGraph::begin( const CVertex& vertex,
...
IC void CLevelGraph::begin( const CVertex* _vertex,
                            const_iterator& _begin,
                            const_iterator& end ) const {
    begin( *_vertex, _begin, end );
}

IC void CLevelGraph::begin( const u32 vertex_id,
                            const_iterator& _begin,
                            const_iterator& end ) const {
...
IC void CLevelGraph::set_invalid_vertex( u32& vertex_id,
...
IC const u32
CLevelGraph::vertex_id( const CLevelGraph::CVertex* vertex ) const {
...
IC bool CLevelGraph::create_straight_path( u32 start_vertex_id,
                                           const Fvector2& start_point,
                                           const Fvector2& finish_point,
                                           xr_vector< T >& tpaOutputPoints,
                                           const T& example,
                                           bool bAddFirstPoint,
    ...
    if ( bAddFirstPoint ) {
        ...
        if ( bAssignY )
            pos3d.y =
                vertex_plane_y( start_vertex_id, start_point.x, start_point.y );
    ...
    if ( vertex( start_vertex_id )->position().xz() == dest_xz ) {
        ...
        if ( bAssignY )
            tIntersectPoint.y = vertex_plane_y(
                vertex( cur_vertex_id ), tIntersectPoint.x, tIntersectPoint.z );
    ...
    for ( ;; ) {
        ...
        for ( ; I != E; ++I ) {
            ...
            if ( ( next_vertex_id == prev_vertex_id ) ||
                 !valid_vertex_id( next_vertex_id ) )
                continue;
            ...
            if ( box.pick_exact( start, dir ) ) {
...
#endif
                    intersect_no_check(
                        start_point.x, start_point.y, finish_point.x,
                        finish_point.y, next1.x, next1.y, next2.x, next2.y,
                ...
                clamp( tIntersectPoint.x, _min( next1.x, next2.x ),
                       _max( next1.x, next2.x ) );
                clamp( tIntersectPoint.z, _min( next1.y, next2.y ),
                       _max( next1.y, next2.y ) );
                if ( bAssignY )
                    tIntersectPoint.y =
                        vertex_plane_y( vertex( cur_vertex_id ),
                                        tIntersectPoint.x, tIntersectPoint.z );
                path_node.set_position( tIntersectPoint );
                ...
                if ( dest_xz == v->position().xz() /**box.contains(dest)/**/ ) {
                    ...
                    if ( bAssignY )
                        tIntersectPoint.y = vertex_plane_y(
                            vertex( cur_vertex_id ), tIntersectPoint.x,
                            tIntersectPoint.z );
                    path_node.set_position( tIntersectPoint );
                    path_node.set_vertex_id( next_vertex_id );
                    tpaOutputPoints.push_back( path_node );
                ...
                if ( tpaOutputPoints.size() > 100000 ) {
                    Msg( "CLevelGraph::create_straight_path : Loop became "
                         "infinite (%d,[%f][%f][%f],[%f][%f][%f])",
                         start_vertex_id, VPUSH( v3d( start_point ) ),
                         VPUSH( v3d( finish_point ) ) );
                    xrLogger::FlushLog();
                    R_ASSERT2( false,
                               "Loop became infinite :-( call Dima and SAVE "
                               "YOUR LOG!" );
                }
#endif
                break;
...
IC bool CLevelGraph::valid_vertex_position( const Fvector& position ) const {
    ...
    if ( ( position.x < box->min.x - header().cell_size() * .5f ) ||
         ( position.x > box->max.x + header().cell_size() * .5f ) ||
         ( position.z < box->min.z - header().cell_size() * .5f ) ||
         ( position.z > box->max.z + header().cell_size() * .5f ) )
        return ( false );

    if ( !( iFloor( ( position.z - box->min.z ) / header().cell_size() + .5f ) <
            ( int )m_row_length ) )
        return ( false );

    if ( !( iFloor( ( position.x - box->min.x ) / header().cell_size() + .5f ) <
            ( int )m_column_length ) )
        return ( false );

    return ( ( vertex_position( position ).xz() <
               ( 1 << MAX_NODE_BIT_COUNT ) - 1 ) );
}
...
template < typename P >
IC void CLevelGraph::iterate_vertices( const Fvector& min_position,
                                       const Fvector& max_position,
                                       const P& predicate ) const {
    CVertex *I, *E;

    if ( valid_vertex_position( min_position ) )
        I = std::lower_bound( m_nodes, m_nodes + header().vertex_count(),
                              vertex_position( min_position ).xz(),
                              &vertex::predicate2 );
    else
        I = m_nodes;

    if ( valid_vertex_position( max_position ) ) {
        E = std::upper_bound( m_nodes, m_nodes + header().vertex_count(),
                              vertex_position( max_position ).xz(),
                              &vertex::predicate );
