...
CALifeMonsterDetailPathManager::CALifeMonsterDetailPathManager(
    object_type* object ) {
    VERIFY( object );
    m_object = object;
...
void CALifeMonsterDetailPathManager::target(
    ...
    VERIFY( ( ai().game_graph().vertex( game_vertex_id )->level_id() !=
              ai().alife().graph().level().level_id() ) ||
            ( ai().cross_table().vertex( level_vertex_id ).game_vertex_id() ==
              game_vertex_id ) );
    VERIFY( ( ai().game_graph().vertex( game_vertex_id )->level_id() !=
              ai().alife().graph().level().level_id() ) ||
...
void CALifeMonsterDetailPathManager::target(
    const GameGraph::_GRAPH_ID& game_vertex_id ) {
    VERIFY( ai().game_graph().valid_vertex_id( game_vertex_id ) );
    target( game_vertex_id,
            ai().game_graph().vertex( game_vertex_id )->level_vertex_id(),
            ai().game_graph().vertex( game_vertex_id )->level_point() );
}

void CALifeMonsterDetailPathManager::target(
    const CALifeSmartTerrainTask& task ) {
    if ( &task == NULL ) {
        return;
    }
...
void CALifeMonsterDetailPathManager::update() {
    ...
    update( time_delta );
    // we advisedly "lost" time we need to process a query to avoid some
    // undesirable effects
...
void CALifeMonsterDetailPathManager::actualize() {
    ...
    bool failed = !ai().graph_engine().search(
        ai().game_graph(), object().get_object().m_tGraphID,
        m_destination.m_game_vertex_id, &m_path, temp );

#ifdef DEBUG
    if ( failed ) {
        Msg( "! %s couldn't build game path from",
             object().get_object().name_replace() );
        {
            ...
            Msg( "! game_graph_mask -> [ %d, %d, %d, %d]",
                 vertex->vertex_type()[ 0 ], vertex->vertex_type()[ 1 ],
...
void CALifeMonsterDetailPathManager::update(
    const ALife::_TIME_ID& time_delta ) {
...
void CALifeMonsterDetailPathManager::setup_current_speed() {
    if ( ai().game_graph()
             .vertex( object().get_object().m_tGraphID )
...
void CALifeMonsterDetailPathManager::follow_path(
    const ALife::_TIME_ID& time_delta ) {
    VERIFY( !completed() );
    VERIFY( !failed() );
    VERIFY( actual() );
    ...
    if ( m_path.size() == 1 ) {
        VERIFY( object().get_object().m_tGraphID ==
    ...
    for ( ; m_path.size() > 1; ) {
        ...
        float update_distance =
            ( last_time_delta /
              ai().alife().time_manager().normal_time_factor() ) *
            speed();
        ...
        last_time_delta = update_distance *
                          ai().alife().time_manager().normal_time_factor() /
                          speed();
        ...
        m_path.pop_back();
        //		Msg
        //("%6d %s changes graph point from %d to
        ...
        object().get_object().alife().graph().change(
            &object().get_object(), object().get_object().m_tGraphID,
...
Fvector CALifeMonsterDetailPathManager::draw_level_position() const {
    ...
    const GameGraph::CVertex* current =
        ai().game_graph().vertex( object().get_object().m_tGraphID );
    const GameGraph::CVertex* next =
        ai().game_graph().vertex( m_path[ path_size - 2 ] );
