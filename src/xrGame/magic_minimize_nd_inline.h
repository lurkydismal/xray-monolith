...
TEMPLATE_SPECIALIZATION
_MinimizeND::MinimizeND( Function oF,
                         int iMaxLevel,
                         int iMaxBracket,
                         int iMaxIterations,
                         void* pvUserData )
...
TEMPLATE_SPECIALIZATION
void _MinimizeND::GetMinimum( const float* afT0,
                              const float* afT1,
...
TEMPLATE_SPECIALIZATION
void _MinimizeND::ComputeDomain( const float* afT0,
                                 const float* afT1,
                                 float& rfL0,
                                 float& rfL1 ) {
    rfL0 = -flt_max;
...
float _MinimizeND::LineFunction( float fT, void* pvUserData ) {
    ...
    for ( int i = 0; i < m_iDimensions; i++ ) {
        pkThis->m_afLineArg[ i ] =
            pkThis->m_afTCurr[ i ] + fT * pkThis->m_afDCurr[ i ];
    }
