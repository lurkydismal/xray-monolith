...
void CScriptActionPlannerWrapper::setup_static( CScriptActionPlanner* planner,
...
#ifdef LOG_ACTION
    if ( ( psAI_Flags.test( aiGOAPScript ) && !m_use_log ) ||
         ( !psAI_Flags.test( aiGOAPScript ) && m_use_log ) )
        set_use_log( !!psAI_Flags.test( aiGOAPScript ) );
#endif
...
void CScriptActionPlannerWrapper::update_static(
    CScriptActionPlanner* planner ) {
