...
UIPlayerItem::UIPlayerItem( ETeam team,
                            ClientID const& clientId,
                            UITeamState* tstate,
                            UITeamPanels* tpanels ) {
    VERIFY( tstate );
    m_teamState = tstate;
...
void UIPlayerItem::Init( CUIXml& uiXml, LPCSTR playerNode, int index ) {
    ...
    VERIFY2( m_player_node_root,
             "player item in team xml node not initialized" );
...
s32 UIPlayerItem::CalculateCheckPoints( game_PlayerState const* ps ) const {
    return ( ps->m_iRivalKills + ( ps->af_count * 3 ) -
             ( ps->m_iTeamKills * 2 ) );
}
...
void UIPlayerItem::InitTextParams( CUIXml& uiXml ) {
    ...
    int temp_number =
        uiXml.GetNodesNum( m_player_node_root, TEXTPARAM_NODE_NAME );
    for ( int i = 0; i < temp_number; ++i ) {
        XML_NODE* text_param_node =
            uiXml.NavigateToNode( TEXTPARAM_NODE_NAME, i );
        if ( !text_param_node )
            break;
        LPCSTR param_name = uiXml.ReadAttrib(
            text_param_node, "name", "param_name_not_set_in_name_attribute" );
        ...
        m_text_params.insert(
            std::make_pair( shared_str( param_name ), temp_static ) );
    }
}
...
void UIPlayerItem::InitIconParams( CUIXml& uiXml ) {
    VERIFY( m_player_node_root );
    int temp_number =
        uiXml.GetNodesNum( m_player_node_root, ICONPARAM_NODE_NAME );
    for ( int i = 0; i < temp_number; ++i ) {
        XML_NODE* icon_param_node =
            uiXml.NavigateToNode( ICONPARAM_NODE_NAME, i );
        if ( !icon_param_node )
            break;
        LPCSTR param_name = uiXml.ReadAttrib(
            icon_param_node, "name", "param_name_not_set_in_name_attribute" );
        CUIStatsIcon* temp_static = xr_new< CUIStatsIcon >();
        VERIFY( temp_static );
        this->AttachChild( temp_static );
        temp_static->SetAutoDelete( true );
        CUIXmlInit::InitStatic( uiXml, ICONPARAM_NODE_NAME, i, temp_static );
        m_icon_params.insert(
            std::make_pair( shared_str( param_name ), temp_static ) );
    }
}
...
void UIPlayerItem::UpdateIconParams( game_PlayerState const* ps ) {
    ...
    for ( TMapStrToUIStatic::iterator i = m_icon_params.begin(); i != ie;
          ++i ) {
        VERIFY( i->second );
...
void UIPlayerItem::GetTextParamValue( game_PlayerState const* ps,
    ...
    if ( param_name.equal( "mp_name" ) ) {
        ...
        xr_sprintf( dest.begin(), dest.size(), "%d",
                    ps->m_iRivalKills - ps->m_iSelfKills );
        ...
        if ( ps->testFlag( GAME_PLAYER_FLAG_READY ) )
            xr_strcpy( dest.begin(), dest.size(),
                       st.translate( "st_mp_ready" ).c_str() );
...
void UIPlayerItem::GetIconParamValue( game_PlayerState const* ps,
    ...
    if ( param_name.equal( "rank" ) ) {
        if ( ETeam( cl_game->ModifyTeam( ps->team ) ) == etGreenTeam ) {
            xr_sprintf( dest.begin(), dest.size(), "ui_hud_status_green_0%d",
                        ps->rank + 1 );
        } else if ( ETeam( cl_game->ModifyTeam( ps->team ) ) == etBlueTeam ) {
            xr_sprintf( dest.begin(), dest.size(), "ui_hud_status_blue_0%d",
                        ps->rank + 1 );
        }
        ...
        if ( cl_game->Type() == eGameIDCaptureTheArtefact ) {
            game_cl_CaptureTheArtefact* cta_cl_game =
                static_cast< game_cl_CaptureTheArtefact* >( cl_game );
            ...
            game_cl_ArtefactHunt* ahunt_cl_game =
                static_cast< game_cl_ArtefactHunt* >( cl_game );
        ...
        VERIFY2( false,
                 make_string( "unknown icon parameter: %s", param_name.c_str() )
                     .c_str() );
    }
}
