...
void character_shell_control::Load( LPCSTR section ) {
    skel_airr_ang_factor =
        pSettings->r_float( section, "ph_skeleton_airr_ang_factor" );
    skel_airr_lin_factor =
        pSettings->r_float( section, "ph_skeleton_airr_lin_factor" );
    hinge_force_factor1 =
        pSettings->r_float( section, "ph_skeleton_hinger_factor1" );
    skel_ddelay = pSettings->r_float( section, "ph_skeleton_ddelay" );
    skel_remain_time = skel_ddelay;
    skel_fatal_impulse_factor =
        pSettings->r_float( section, "ph_skel_fatal_impulse_factor" );
    skeleton_skin_ddelay =
        pSettings->r_float( section, "ph_skeleton_skin_ddelay" );
    skeleton_skin_remain_time = skeleton_skin_ddelay;
    // gray_wolf>Читаем из ltx параметры для поддержки изменяющегося трения у
    // персонажей во время смерти gray_wolf<
    skeleton_skin_friction_start =
        pSettings->r_float( section, "ph_skeleton_skin_friction_start" );
    skeleton_skin_friction_end =
        pSettings->r_float( section, "ph_skeleton_skin_friction_end" );
    character_have_wounded_state =
        pSettings->r_bool( section, "ph_character_have_wounded_state" );
    ...
    pelvis_factor_low_pose_detect =
        pSettings->r_float( section, "ph_pelvis_factor_low_pose_detect" );
    if ( pSettings->line_exist( section, "ph_skel_shot_up_factor" ) )
        m_shot_up_factor =
            pSettings->r_float( section, "ph_skel_shot_up_factor" );
    if ( pSettings->line_exist( section, "ph_after_death_velocity_factor" ) )
        m_after_death_velocity_factor =
            pSettings->r_float( section, "ph_after_death_velocity_factor" );
}

void character_shell_control::set_kill_hit( SHit& H ) const {
    Fvector& dir = H.dir;
    if ( !fis_zero( m_shot_up_factor ) &&
         H.type() != ALife::eHitTypeExplosion ) {
...
void character_shell_control::set_fatal_impulse( SHit& H ) const {
    if ( !m_was_wounded ) {
        H.impulse *= ( H.type() == ALife::eHitTypeExplosion
                           ? 1.f
                           : skel_fatal_impulse_factor );
    }
}

void OnCharacterContactInDeath( bool& do_colide,
                                bool bo1,
                                dContact& c,
                                SGameMtl* /*material_1*/,
                                SGameMtl* /*material_2*/ ) {
    ...
    if ( bo1 ) {
        l_character_physic_support =
            ( character_shell_control* )PHRetrieveGeomUserData( c.geom.g1 )
                ->callback_data;
    } else {
        l_character_physic_support =
            ( character_shell_control* )PHRetrieveGeomUserData( c.geom.g2 )
                ->callback_data;
    }
...
void character_shell_control::set_start_shell_params(
    CPhysicsShell* sh ) const {
...
void character_shell_control::apply_start_velocity_factor(
    CObject* who,
...
void character_shell_control::TestForWounded( const Fmatrix& xform,
                                              IKinematics* CKA ) {
    m_was_wounded = false;
    ...
    CKA->CalculateBones();
    CBoneInstance CBI =
        CKA->LL_GetBoneInstance( CKA->LL_BoneID( "bip01_pelvis" ) );
    Fmatrix position_matrix;
    position_matrix.mul( xform, CBI.mTransform );

    xrXRC xrc;
    xrc.ray_options( 0 );
    xrc.ray_query( Level().ObjectSpace.GetStaticModel(), position_matrix.c,
...
#ifdef DEBUG
    if ( death_anim_debug ) {
        Msg( "death anim: test for wounded %s ",
             m_was_wounded ? "true" : "false" );
    }
#endif
};
...
void character_shell_control::UpdateFrictionAndJointResistanse(
    CPhysicsShell* sh ) {
    // Преобразование skel_ddelay из кадров в секунды и линейное нарастание
    // сопротивления в джоинтах со временем от момента смерти
