...
void game_cl_mp::AddSoundMessage( LPCSTR sound_name,
                                  u32 const sound_priority,
                                  u32 const soundID ) {
    m_pSndMessages.push_back( xr_new< SND_Message >() );
...
void game_cl_mp::PlaySndMessage( u32 ID ) {
    ...
    SNDMESSAGES_it it = std::find_if( m_pSndMessages.begin(),
                                      m_pSndMessages.end(), tmp_predicate );
    ...
    //	if (Level().timeServer()<pSndMgs->pSound._handle()->length_ms() +
    // pSndMgs->LastStarted) return;
    ...
    for ( u32 i = 0; i < m_pSndMessagesInPlay.size(); i++ ) {
        ...
        if ( pSndMsgIP->priority == SndMsg->priority ) {
            if ( Level().timeServer_Async() >
                 pSndMsgIP->LastStarted +
                     iFloor( pSndMsgIP->pSound.get_length_sec() * 1000.0f ) )
                continue;

            u32 Delay = pSndMsgIP->LastStarted +
                        iFloor( pSndMsgIP->pSound.get_length_sec() * 1000.0f ) -
    ...
    SndMsg->pSound.play_at_pos( NULL, Fvector().set( 0, 0, 0 ), sm_2D,
                                float( MaxDelay ) / 1000.0f );
