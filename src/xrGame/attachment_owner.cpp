...
void CAttachmentOwner::reload( LPCSTR section ) {
    ...
    LPCSTR attached_sections =
        pSettings->r_string( section, "attachable_items" );
    ...
    for ( u32 i = 0; i < item_count; ++i )
        m_attach_item_sections[ i ] =
            _GetItem( attached_sections, i, current_item_section );
}
...
void __stdcall AttachmentCallback( IKinematics* tpKinematics ) {
    CGameObject* game_object = smart_cast< CGameObject* >(
        static_cast< CObject* >( tpKinematics->GetUpdateCallbackParam() ) );
    VERIFY( game_object );

    CAttachmentOwner* attachment_owner =
        smart_cast< CAttachmentOwner* >( game_object );
    VERIFY( attachment_owner );

    IKinematics* kinematics =
        smart_cast< IKinematics* >( game_object->Visual() );

    xr_vector< CAttachableItem* >::const_iterator I =
        attachment_owner->attached_objects().begin();
    xr_vector< CAttachableItem* >::const_iterator E =
        attachment_owner->attached_objects().end();
    for ( ; I != E; ++I ) {
        ( *I )->item().object().XFORM().mul_43(
            kinematics->LL_GetBoneInstance( ( *I )->bone_id() ).mTransform,
            ( *I )->offset() );
...
void CAttachmentOwner::attach( CInventoryItem* inventory_item ) {
    ...
    for ( ; I != E; ++I ) {
        ...
        //		VERIFY
        //((*I)->ID() != inventory_item->object().ID());
    }

    if ( can_attach( inventory_item ) ) {
        CAttachableItem* attachable_item =
            smart_cast< CAttachableItem* >( inventory_item );
...
bool CAttachmentOwner::can_attach(
    const CInventoryItem* inventory_item ) const {
...
CAttachableItem* CAttachmentOwner::attachedItem( shared_str& section ) const {
    ...
    xr_vector< CAttachableItem* >::const_iterator E = m_attached_objects.end();
    for ( ; I != E; ++I )
        if ( !xr_strcmp( ( *I )->item().object().cNameSect(), section ) &&
             !( *I )->item().IsInvalid() )
            return ( *I );
