...
struct victim_raw_kill {
    ...
    bool operator()( shared_str const& killer,
        ...
        if ( ( killer == m_killer ) && ( victim == m_victim ) &&
...
void harvest_time::OnPlayerKilled(
    ...
    victim_raw_kill tmp_predicate( tmp_local_player->getName(),
                                   victim_obj->cName(), m_spawn_time );

    buffer_vector< kills_store::kill > tmp_fake_buffer( NULL, 0 );

    m_harvest_count = m_owner->get_kills_store().fetch_kills( tmp_predicate,
                                                              tmp_fake_buffer );
}
