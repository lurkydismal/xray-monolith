...
void CParticlesAsync::Start() {
...

    {
        ...
        for ( intrusive_ptr< CPS_Instance >& particle :
              g_pGamePersistent->ps_active ) {
            if ( particle->m_bDead )
                continue;
...
void CParticlesAsync::UpdateParticle(
    intrusive_ptr< CPS_Instance > particle ) const {
    u32 dt = Device.dwTimeGlobal - particle->dwLastTime;
    IParticleCustom* V =
        smart_cast< IParticleCustom* >( particle->renderable.visual );
