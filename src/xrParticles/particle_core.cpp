...
pDomain::pDomain( PDomainEnum dtype,
                  float a0,
                  float a1,
                  float a2,
                  float a3,
    ...
    switch ( type ) {
        ...
        case PDDisc: {
            ...
            p2 = pVector( a3, a4,
                          a5 ); // Normal (not used in Within and Generate)
            p2.normalize_safe();
...
BOOL pDomain::Within( const pVector& pos ) const {
    switch ( type ) {
        ...
        case PDBlob: {
            pVector x( pos - p1 );
            // return exp(-0.5 * xSq * Sqr(oneOverSigma)) * ONEOVERSQRT2PI *
            // oneOverSigma;
    ...
    switch ( type ) {
        ...
        case PDSphere:
            ...
            if ( radius1 == radius2 )
                ...
                pos =
                    p1 + pos * ( radius2 + drand48() * ( radius1 - radius2 ) );
            break;
        case PDCylinder:
        case PDCone: {
        ...
        case PDDisc: {
            float theta =
                drand48() * 2.0f * float( M_PI ); // Angle around normal
