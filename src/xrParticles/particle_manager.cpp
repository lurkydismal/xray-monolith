...
CParticleManager::SharedParticleEffect CParticleManager::GetEffectPtr(
    int effect_id ) {
...
CParticleManager::SharedParticleActions CParticleManager::GetActionListPtr(
    int a_list_num ) {
...
int CParticleManager::CreateEffect( u32 max_particles ) {
    ...
    auto EffectResultPair = m_effect_map.emplace(
        effectId, xr_new< ParticleEffect >( max_particles ) );
    R_ASSERT2( EffectResultPair.second,
               "Can't create particle effect with global counter" );
...
int CParticleManager::CreateActionList() {
    ...
    auto ActionResultPair =
        m_alist_map.emplace( actionId, xr_new< ParticleActions >() );
    R_ASSERT2( ActionResultPair.second,
               "Can't create particle action with global counter" );
...
void CParticleManager::PlayEffect( int effect_id, int alist_id ) {
    ...
    for ( PAVecIt it = particleAction->begin(); it != particleAction->end();
          ++it ) {
        VERIFY( ( *it ) );
        switch ( ( *it )->type ) {
            case PASourceID:
                static_cast< PASource* >( *it )->m_Flags.set(
                    PASource::flSilent, FALSE );
                break;
            case PAExplosionID:
...
void CParticleManager::StopEffect( int effect_id,
                                   int alist_id,
                                   BOOL deffered ) {
    // Execute the specified action list.
    SharedParticleActions particleAction = GetActionListPtr( alist_id );
        ...
        switch ( ( *it )->type ) {
            case PASourceID:
                static_cast< PASource* >( *it )->parent_vel =
                    pVector( vel.x, vel.y, vel.z ) *
                    static_cast< PASource* >( *it )->parent_motion;
                break;
        }
    }
}
...
void CParticleManager::SetCallback( int effect_id,
                                    OnBirthParticleCB b,
                                    OnDeadParticleCB d,
                                    void* owner,
                                    u32 param ) {
...
void CParticleManager::GetParticles( int effect_id,
                                     Particle*& particles,
                                     u32& cnt ) {
    SharedParticleEffect pe = GetEffectPtr( effect_id );
    if ( !pe ) {
