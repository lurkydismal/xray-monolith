...
void PAPI::PAAvoid::Execute( ParticleEffect* effect,
    ...
    switch ( position.type ) {
        ...
        case PDRectangle: {
            ...
            float det =
                1 / ( wz * u.x * v.y - wz * u.y * v.x - u.z * wx * v.y -
                      u.x * v.z * wy + v.z * wx * u.y + u.z * v.x * wy );

            pVector s1( ( v.y * wz - v.z * wy ), ( v.z * wx - v.x * wz ),
                        ( v.x * wy - v.y * wx ) );
            s1 *= det;
            pVector s2( ( u.y * wz - u.z * wy ), ( u.z * wx - u.x * wz ),
                        ( u.x * wy - u.y * wx ) );
            s2 *= -det;
...
void PABounce::Execute( ParticleEffect* effect,
                        const float dt,
                        float& tm_max ) {
    switch ( position.type ) {
        case PDTriangle: {
            ...
            for ( u32 i = 0; i < effect->p_count; i++ ) {
                ...
                if ( position.Within( pnext ) ) {
                    ...
                    if ( pinside ) {
                        ...
                        // surface boundary. Reverse normal component of
                        // velocity, and apply friction (if Vt >= cutoff) and
                        // resilience.
...
void PACopyVertexB::Execute( ParticleEffect* effect,
                             const float dt,
                             float& tm_max ) {
    u32 i;
...
// Dampen velocities
void PADamping::Execute( ParticleEffect* effect,
                         const float dt,
                         float& tm_max ) {
...
void PAExplosion::Execute( ParticleEffect* effect,
                           const float dt,
                           float& tm_max ) {
    ...
    for ( u32 i = 0; i < effect->p_count; i++ ) {
        ...
        m.vel +=
            dir * ( Gd * magdt / ( ( dist + EPS ) * ( distSqr + epsilon ) ) );
    }
...
void PAFollow::Execute( ParticleEffect* effect,
                        const float dt,
                        float& tm_max ) {
    float magdt = magnitude * dt;
...
void PAGravity::Execute( ParticleEffect* effect,
                         const float dt,
                         float& tm_max ) {
...
void PAScatter::Execute( ParticleEffect* effect,
                         const float dt,
                         float& tm_max ) {
    float magdt = magnitude * dt;
    float max_radiusSqr = max_radius * max_radius;
...
void PAKillOld::Execute( ParticleEffect* effect,
                         const float dt,
                         float& tm_max ) {
    ...
    if ( max_radiusSqr < P_MAXFLOAT ) {
        for ( u32 i = 0; i < effect->p_count; i++ ) {
            ...
            if ( rSqr < max_radiusSqr )
                // Step velocity with acceleration
                m.vel +=
                    into * ( magdt / ( _sqrt( rSqr ) + ( rSqr + epsilon ) ) );
        }
    } else {
...
void PAOrbitPoint::Execute( ParticleEffect* effect,
                            const float dt,
                            float& tm_max ) {
    float magdt = magnitude * dt;
    float max_radiusSqr = max_radius * max_radius;
    ...
    if ( max_radiusSqr < P_MAXFLOAT ) {
        for ( u32 i = 0; i < effect->p_count; i++ ) {
            ...
            if ( rSqr < max_radiusSqr )
                m.vel +=
                    dir * ( magdt / ( _sqrt( rSqr ) + ( rSqr + epsilon ) ) );
        }
    } else {
...
void PARandomAccel::Execute( ParticleEffect* effect,
                             const float dt,
                             float& tm_max ) {
    for ( u32 i = 0; i < effect->p_count; i++ ) {
        Particle& m = effect->particles[ i ];
...
void PARandomDisplace::Execute( ParticleEffect* effect,
                                const float dt,
                                float& tm_max ) {
    for ( u32 i = 0; i < effect->p_count; i++ ) {
        Particle& m = effect->particles[ i ];
...
void PARandomVelocity::Execute( ParticleEffect* effect,
                                const float dt,
                                float& tm_max ) {
    for ( u32 i = 0; i < effect->p_count; i++ ) {
        Particle& m = effect->particles[ i ];
...
void PARestore::Execute( ParticleEffect* effect,
                         const float dt,
                         float& tm_max ) {
    if ( time_left <= 0 ) {
        for ( u32 i = 0; i < effect->p_count; i++ ) {
        ...
        for ( u32 i = 0; i < effect->p_count; i++ ) {
            ...
            // _pconstrain(m.pos.x, m.vel.x, m.posB.x, 0., timeLeft, &a, &b,
            // &c);
            ...
            float b =
                ( -2 * t * m.vel.x + 3 * m.posB.x - 3 * m.pos.x ) * tSqrInv2dt;
            float a =
                ( t * m.vel.x - m.posB.x - m.posB.x + m.pos.x + m.pos.x ) *
                tCubInv3dtSqr;
            ...
            a = ( t * m.vel.y - m.posB.y - m.posB.y + m.pos.y + m.pos.y ) *
                tCubInv3dtSqr;
            ...
            a = ( t * m.vel.z - m.posB.z - m.posB.z + m.pos.z + m.pos.z ) *
                tCubInv3dtSqr;
...
void PASinkVelocity::Execute( ParticleEffect* effect,
                              const float dt,
                              float& tm_max ) {
    // Must traverse list in reverse order so Remove will work
    for ( int i = effect->p_count - 1; i >= 0; i-- ) {
...
void PASource::Execute( ParticleEffect* effect,
                        const float dt,
                        float& tm_max ) {
    ...
    if ( m_Flags.is( u32( flVertexB_tracks ) ) ) {
        for ( int i = 0; i < rate; i++ ) {
            ...
            effect->Add( pos, pos, siz, rt, vel,
                         color_argb_f( alpha, col.x, col.y, col.z ), ag );
        }
    } else {
        for ( int i = 0; i < rate; i++ ) {
            position.Generate( pos );
            ...
            effect->Add( pos, posB, siz, rt, vel,
                         color_argb_f( alpha, col.x, col.y, col.z ), ag );
        }
    }
}
...
void PAVortex::Execute( ParticleEffect* effect,
                        const float dt,
                        float& tm_max ) {
    float magdt = magnitude * dt;
    float max_radiusSqr = max_radius * max_radius;
...
void PATurbulenceExecuteStream( LPVOID lpvParams ) {
    ...
    for ( u32 i = 0; i < count; i++ ) {
        ...
        _tmp =
            _mm_unpacklo_ps( _vmo, _vmo ); // _tmp = yy | yy | xx + zz | xx + zz
        ...
        _tmp =
            _mm_unpacklo_ps( _vmn, _vmn ); // _tmp = yy | yy | xx + zz | xx + zz
        _tmp = _mm_movehl_ps( _tmp, _tmp ); // _tmp = yy | yy | yy | yy
        ...
        _vmo = _mm_shuffle_ps(
            _vmo, _vmo,
            _MM_SHUFFLE( 0, 0, 0, 0 ) ); // _vmo = scale | scale | scale | scale
...
void PATurbulence::Execute( ParticleEffect* effect,
                            const float dt,
                            float& tm_max ) {
#ifdef _GPA_ENABLED
...
void PATurbulence::Execute( ParticleEffect* effect,
    ...
    for ( u32 i = 0; i < effect->p_count; i++ ) {
        ...
        float d = fractalsum3( pV, frequency, octaves );
        D.x =
            ( fractalsum3( vX, frequency, octaves ) - d ) * ( float )magnitude;
        D.y =
            ( fractalsum3( vY, frequency, octaves ) - d ) * ( float )magnitude;
        D.z =
            ( fractalsum3( vZ, frequency, octaves ) - d ) * ( float )magnitude;
