...
public:
    ...
    // IC CMotionDef*				LL_GetMotionDef	(MotionID
    // id){return m_Motions[id.slot].motions.motion_def(id.idx);} IC CMotion*
    // LL_GetRootMotion(MotionID id){return
    // &m_Motions[id.slot].bone_motions[iRoot]->at(id.idx);} IC CMotion*
    // LL_GetMotion	(MotionID id, u16 bone_id){return
    ...
    virtual void LL_BuldBoneMatrixDequatize( const CBoneData* bd,
                                             u8 channel_mask,
                                             SKeyTable& keys ) = 0;
    virtual void LL_BoneMatrixBuild( u16 bone_id,
                                     CBoneInstance& bi,
                                     const Fmatrix* parent,
                                     const SKeyTable& keys ) = 0;
    ...
    //	virtual CBlendInstance&				LL_GetBlendInstance
    //(u16 bone_id) = 0;

    // Main functionality
    virtual void UpdateTracks() = 0; // Update motions
    virtual void LL_UpdateTracks( float dt,
                                  bool b_force,
                                  bool leave_blends ) = 0; // Update motions
    // void						DestroyCycle	(CBlend
    // &B);
    ...
    virtual CBlend* PlayCycle( LPCSTR N,
                               BOOL bMixIn = TRUE,
                               PlayCallback Callback = 0,
                               LPVOID CallbackParam = 0,
                               u8 channel = 0 ) = 0;
    virtual CBlend* PlayCycle( MotionID M,
                               BOOL bMixIn = TRUE,
                               PlayCallback Callback = 0,
                               LPVOID CallbackParam = 0,
                               u8 channel = 0 ) = 0;
    virtual CBlend* PlayCycle( u16 partition,
                               MotionID M,
                               BOOL bMixIn = TRUE,
                               PlayCallback Callback = 0,
                               LPVOID CallbackParam = 0,
    ...
    // #ifdef DEBUG
    //	virtual	const BlendSVec			&blend_cycle	(const u32
    //&bone_part_id) const = 0; #endif //	DEBUG
};
