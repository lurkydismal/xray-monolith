...
enum { sq_DEFAULT, sq_NOVIRT, sq_LIGHT, sq_HIGH, sq_forcedword = u32( -1 ) };
...
typedef resptr_core< CSound_UserData, resptr_base< CSound_UserData > >
    CSound_UserDataPtr;
...
public:
    ...
    CSound_emitter*
        feedback; //!< Pointer to emitter, automaticaly clears on emitter-stop
    esound_type s_type;
...
typedef resptr_core< ref_sound_data, resptr_base< ref_sound_data > >
    ref_sound_data_ptr;
...
public:
    ...
    IC void clone( const ref_sound& from,
                   esound_type sound_type,
                   int game_type );

    IC void destroy();

    IC void play( CObject* O, u32 flags = 0, float delay = 0.f );
    IC void play_at_pos( CObject* O,
                         const Fvector& pos,
                         u32 flags = 0,
                         float delay = 0.f );
    IC void play_no_feedback( CObject* O,
                              u32 flags = 0,
                              float delay = 0.f,
                              Fvector* pos = 0,
                              float* vol = 0,
...
public:
    CSound_params() : set( false ) {
        ...
        pitch_variation = 0.02 * Random.randF( -1.f, 1.f ) *
                          soundSmoothingParams::pitchVariationPower;
    }
...
public:
    ...
    IC virtual void update_position( const Fvector& newPosition ) {
        // If the position has been set already, start getting a moving average
        // of the velocity.
        if ( set ) {
            prevVelocity.set( accVelocity );
            curVelocity.sub( newPosition, position );

            // accVelocity.set(curVelocity.mul(alpha).add(prevVelocity.mul(1.f -
            // alpha)));
        } else {
            set = true;
        }
    ...
    IC virtual void update_velocity( const float dt ) {
        ...
        accVelocity.x = soundSmoothingParams::getSmoothedValue(
            curVelocity.x * p / dt, accVelocity.x, a );
        accVelocity.y = soundSmoothingParams::getSmoothedValue(
            curVelocity.y * p / dt, accVelocity.y, a );
        accVelocity.z = soundSmoothingParams::getSmoothedValue(
            curVelocity.z * p / dt, accVelocity.z, a );
...
protected:
    ...
    virtual void _create_data( ref_sound_data& S,
                               LPCSTR fName,
                               esound_type sound_type,
                               int game_type ) = 0;
...
public:
    ...
    virtual void create( ref_sound& S,
                         LPCSTR fName,
                         esound_type sound_type,
                         int game_type ) = 0;
    virtual void attach_tail( ref_sound& S, LPCSTR fName ) = 0;
    virtual void clone( ref_sound& S,
                        const ref_sound& from,
                        esound_type sound_type,
                        int game_type ) = 0;
    ...
    virtual void play( ref_sound& S,
                       CObject* O,
                       u32 flags = 0,
                       float delay = 0.f ) = 0;
    virtual void play_at_pos( ref_sound& S,
                              CObject* O,
                              const Fvector& pos,
                              u32 flags = 0,
                              float delay = 0.f ) = 0;
    virtual void play_no_feedback( ref_sound& S,
                                   CObject* O,
                                   u32 flags = 0,
                                   float delay = 0.f,
                                   Fvector* pos = 0,
    ...
    virtual void update( const Fvector& P,
                         const Fvector& D,
                         const Fvector& N ) = 0;
    ...
    virtual void source_prefetch_prepare(
        const xr_vector< xr_string >& sources ) = 0;
    ...
    virtual void set_environment_size( CSound_environment* src_env,
                                       CSound_environment** dst_env ) = 0;
#endif
};
...
IC ref_sound_data::ref_sound_data( LPCSTR fName,
                                   esound_type sound_type,
                                   int game_type ) {
...
IC void ref_sound::create( LPCSTR name,
                           esound_type sound_type,
                           int game_type ) {
...
IC void ref_sound::clone( const ref_sound& from,
                          esound_type sound_type,
                          int game_type ) {
    VERIFY( !::Sound->i_locked() );
...
IC void ref_sound::play_at_pos( CObject* O,
                                const Fvector& pos,
                                u32 flags,
                                float d ) {
    VERIFY( !::Sound->i_locked() );
...
IC void ref_sound::play_no_feedback( CObject* O,
                                     u32 flags,
                                     float d,
                                     Fvector* pos,
                                     float* vol,
