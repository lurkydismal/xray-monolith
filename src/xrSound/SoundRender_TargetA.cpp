...
BOOL CSoundRender_TargetA::_initialize() {
    ...
    if ( AL_NO_ERROR == error ) {
        ...
        Msg( "! sound: OpenAL: Can't create source. Error: %s.",
             ( LPCSTR )alGetString( error ) );
        return FALSE;
    }
}
...
void CSoundRender_TargetA::start( CSoundRender_Emitter* E ) {
    ...
    // Calc storage
    buf_block =
        sdef_target_block * E->source()->m_wformat.nAvgBytesPerSec / 1000;
...
void CSoundRender_TargetA::render() {
    ...
    if ( Slot != u32( -1 ) && !m_pEmitter->bIntro ) {
        A_CHK( alSource3i( pSource, AL_AUXILIARY_SEND_FILTER, Slot, 0,
                           AL_FILTER_NULL ) );
    }
    // demonized: explicitly disable effects by sending sounds to null slot, ie.
    // not sending
    else {
...
void CSoundRender_TargetA::update() {
    ...
    if ( processed > 0 ) {
        ...
        // processed == 0
        // check play status -- if stopped then queue is not being filled fast
        // enough
        ALint state;
        A_CHK( alGetSourcei( pSource, AL_SOURCE_STATE, &state ) );
        if ( state != AL_PLAYING ) {
...
void CSoundRender_TargetA::fill_parameters() {
    ...
    VERIFY2( m_pEmitter, SE->source()->file_name() );
    A_CHK( alSourcef( pSource, AL_MAX_DISTANCE,
                      m_pEmitter->p_source.max_distance ) );
    ...
    VERIFY2( m_pEmitter, SE->source()->file_name() );
    A_CHK( alSource3f( pSource, AL_VELOCITY, m_pEmitter->p_source.velocity.x,
                       m_pEmitter->p_source.velocity.y,
    ...
    if ( !fsimilar( cache_pitch, _pitch * psSpeedOfSound ) ) {
        ...
        if ( !m_pEmitter->iPaused &&
             ( m_pEmitter->m_current_state ==
                   CSoundRender_Emitter::stStarting ||
               m_pEmitter->m_current_state == CSoundRender_Emitter::stPlaying ||
               m_pEmitter->m_current_state ==
                   CSoundRender_Emitter::stSimulating ) )
            m_pEmitter->fTimeToStop =
                SoundRender->fTimer_Value +
                ( ( m_pEmitter->get_length_sec() -
                    ( SoundRender->fTimer_Value - m_pEmitter->fTimeStarted ) ) /
                  cache_pitch );
...
void CSoundRender_TargetA::fill_block( ALuint BufferID ) {
    R_ASSERT( m_pEmitter );
    ALuint format = ( m_pEmitter->source()->m_wformat.nChannels == 1 )
                        ? AL_FORMAT_MONO16
                        : AL_FORMAT_STEREO16;
    if ( format == AL_FORMAT_MONO16 ) {
        m_pEmitter->fill_block( &g_target_temp_data.front(),
                                g_target_temp_data.size() );
        A_CHK( alBufferData( BufferID, format, &g_target_temp_data.front(),
                             g_target_temp_data.size(),
                             m_pEmitter->source()->m_wformat.nSamplesPerSec ) );
    } else {
        m_pEmitter->fill_block( &g_target_temp_data_16.front(),
                                g_target_temp_data_16.size() );
        A_CHK( alBufferData( BufferID, format, &g_target_temp_data_16.front(),
                             g_target_temp_data_16.size(),
                             m_pEmitter->source()->m_wformat.nSamplesPerSec ) );
    }
}
