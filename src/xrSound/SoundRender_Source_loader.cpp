...
bool CSoundRender_Source::LoadWave( LPCSTR pName ) {
    ...
    m_wformat.nAvgBytesPerSec =
        m_wformat.nSamplesPerSec * m_wformat.nBlockAlign;

    s64 pcm_total = ov_pcm_total( &ovf, -1 );
    dwBytesTotal = u32( pcm_total * m_wformat.nBlockAlign );
    fTimeTotal = s_f_def_source_footer +
                 dwBytesTotal / float( m_wformat.nAvgBytesPerSec );
    ...
    R_ASSERT3( ( m_fMaxAIDist >= 0.1f ) && ( m_fMaxDist >= 0.1f ),
               "Invalid max distance.", pname.c_str() );
