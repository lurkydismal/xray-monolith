...
bool ReadOggPage( const u8* data,
                  size_t size,
                  size_t offset,
                  ogg_page& page,
                  size_t& next ) {
    if ( offset > size || size - offset < OggPageHeaderSize ||
         memcmp( data + offset, "OggS", 4 ) || data[ offset + 4 ] )
        return false;
...
bool ReadSingleStreamPcmTotal( const u8* data,
                               size_t size,
                               const OggVorbis_File& vorbis_file,
                               vorbis_info& info,
                               s64& pcm_total ) {
    ...
    const size_t tail_begin =
        size > OggMaximumPageSize ? size - OggMaximumPageSize : 0;
    s64 final_pcm = -1;
    for ( size_t tail = size - OggPageHeaderSize;; --tail ) {
        if ( data[ tail ] == 'O' ) {
            ogg_page page{};
            size_t next = 0;
            if ( ReadOggPage( data, size, tail, page, next ) && next == size &&
                 ogg_page_eos( &page ) ) {
                if ( ogg_page_serialno( &page ) != serial )
                    return false;
...
int ov_seek_func( void* datasource, s64 offset, int whence ) {
    switch ( whence ) {
        ...
        case SEEK_END:
            ( ( IReader* )datasource )
                ->seek( ( int )offset + ( ( IReader* )datasource )->length() );
            break;
    }
    return 0;
}
...
bool CSoundRender_Source::prepare( LPCSTR path,
    ...
    OggVorbis_File ovf;
    ov_callbacks ovc = { ov_read_func, ov_seek_func, ov_close_func,
                         ov_tell_func };
    ...
    // Headers and comments do not require libvorbisfile's full seekable-open
    // pass.
    const int open_result = ov_test_callbacks( wave, &ovf, NULL, 0, ovc );
    if ( open_result ) {
        FS.r_close( wave );
        error = make_string( "Invalid OGG stream (%d): %s", open_result, path )
                    .c_str();
        return false;
    }
    ...
    s64 pcm_total = 0;
    if ( !ReadSingleStreamPcmTotal( wave_data, wave_size, ovf, *ovi,
                                    pcm_total ) ) {
        // Chained, multiplexed and unusual streams retain the original full
        // parser.
        const int finish_result = ov_test_open( &ovf );
        if ( finish_result ) {
            FS.r_close( wave );
            error = make_string( "Invalid OGG stream (%d): %s", finish_result,
                                 path )
                        .c_str();
            return false;
        }
    ...
    prepared.format.nBlockAlign =
        prepared.format.wBitsPerSample / 8 * prepared.format.nChannels;
    prepared.format.nAvgBytesPerSec =
        prepared.format.nSamplesPerSec * prepared.format.nBlockAlign;

    prepared.bytes_total = u32( pcm_total * prepared.format.nBlockAlign );
    prepared.time_total =
        s_f_def_source_footer +
        prepared.bytes_total / float( prepared.format.nAvgBytesPerSec );

    vorbis_comment* ovm = ov_comment( &ovf, -1 );
    if ( ovm && ovm->comments && ovm->user_comments[ 0 ] &&
         ovm->comment_lengths[ 0 ] >= sizeof( u32 ) ) {
...
bool CSoundRender_Source::LoadWave( LPCSTR path ) {
    PreparedSoundSource prepared;
    xr_string error;
    R_ASSERT3( prepare( path, prepared, error ), "Can't prepare sound source",
               error.c_str() );
...
void CSoundRender_Source::load_prepared( LPCSTR name,
                                         const PreparedSoundSource& prepared ) {
    fname = name;
