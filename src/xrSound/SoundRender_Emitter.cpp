...
void CSoundRender_Emitter::Event_Propagade() {
    fTimeToPropagade += ::Random.randF( s_f_def_event_pulse - 0.030f,
...
u32 CSoundRender_Emitter::play_time() {
    if ( m_current_state == stPlaying || m_current_state == stPlayingLooped ||
...
void CSoundRender_Emitter::set_cursor( u32 p ) {
    ...
    if ( owner_data._get() && owner_data->fn_attached[ 0 ].size() ) {
        ...
        if ( m_stream_cursor >= m_cur_handle_cursor + bt ) {
            SoundRender->i_destroy_source(
                ( CSoundRender_Source* )owner_data->handle );
            owner_data->handle = SoundRender->i_create_source(
                owner_data->fn_attached[ 0 ].c_str() );
