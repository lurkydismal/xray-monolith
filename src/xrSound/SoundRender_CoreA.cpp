...
int CSoundRender_CoreA::load_reverb( ALuint effect_,
    ...
    A_CHK( alEffectf( effect_, AL_EAXREVERB_DECAY_HFRATIO,
                      reverb->flDecayHFRatio ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_DECAY_LFRATIO,
                      reverb->flDecayLFRatio ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_REFLECTIONS_GAIN,
                      reverb->flReflectionsGain ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_REFLECTIONS_DELAY,
                      reverb->flReflectionsDelay ) );
    A_CHK( alEffectfv( effect_, AL_EAXREVERB_REFLECTIONS_PAN,
                       reverb->flReflectionsPan ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_LATE_REVERB_GAIN,
                      reverb->flLateReverbGain ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_LATE_REVERB_DELAY,
                      reverb->flLateReverbDelay ) );
    A_CHK( alEffectfv( effect_, AL_EAXREVERB_LATE_REVERB_PAN,
    ...
    A_CHK( alEffectf( effect_, AL_EAXREVERB_MODULATION_TIME,
                      reverb->flModulationTime ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_MODULATION_DEPTH,
                      reverb->flModulationDepth ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_AIR_ABSORPTION_GAINHF,
                      reverb->flAirAbsorptionGainHF ) );
    A_CHK(
        alEffectf( effect_, AL_EAXREVERB_HFREFERENCE, reverb->flHFReference ) );
    A_CHK(
        alEffectf( effect_, AL_EAXREVERB_LFREFERENCE, reverb->flLFReference ) );
    A_CHK( alEffectf( effect_, AL_EAXREVERB_ROOM_ROLLOFF_FACTOR,
                      reverb->flRoomRolloffFactor ) );
    A_CHK( alEffecti( effect_, AL_EAXREVERB_DECAY_HFLIMIT,
...
void CSoundRender_CoreA::commit() {
    ...
    A_CHK( alAuxiliaryEffectSloti( slot, AL_EFFECTSLOT_AUXILIARY_SEND_AUTO,
                                   false ) );
...
void CSoundRender_CoreA::set_listener( const CSoundRender_Environment& env ) {
    ...
    if ( reverb_overwrite ) {
        ...
        A_CHK(
            alEffectf( effect, AL_EAXREVERB_DECAY_TIME, psReverbDecayTime ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_DECAY_HFRATIO,
                          psReverbDecayHFRatio ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_DECAY_LFRATIO,
                          psReverbDecayLFRatio ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_REFLECTIONS_GAIN,
                          psReverbReflectionsGain ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_REFLECTIONS_DELAY,
                          psReverbReflectionsDelay ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_LATE_REVERB_GAIN,
                          psReverbLateReverbGain ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_LATE_REVERB_DELAY,
                          psReverbLateReverbDelay ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_ECHO_TIME, psReverbEchoTime ) );
        A_CHK(
            alEffectf( effect, AL_EAXREVERB_ECHO_DEPTH, psReverbEchoDepth ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_MODULATION_TIME,
                          psReverbModulationTime ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_MODULATION_DEPTH,
                          psReverbModulationDepth ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_AIR_ABSORPTION_GAINHF,
                          psReverbAirAbsorptionGainHF ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_HFREFERENCE,
                          psReverbHFReference ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_LFREFERENCE,
                          psReverbLFReference ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_ROOM_ROLLOFF_FACTOR,
                          psReverbRoomRolloffFactor ) );
        A_CHK( alEffecti( effect, AL_EAXREVERB_DECAY_HFLIMIT,
                          psReverbDecayHFLimit ) );
    }
    ////
    else {
        A_CHK( alEffectf( effect, AL_EAXREVERB_DIFFUSION,
                          env.EnvironmentDiffusion ) );
        ...
        A_CHK(
            alEffectf( effect, AL_EAXREVERB_DECAY_HFRATIO, env.DecayHFRatio ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_REFLECTIONS_GAIN,
                          env.Reflections ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_REFLECTIONS_DELAY,
                          env.ReflectionsDelay ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_LATE_REVERB_GAIN, env.Reverb ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_AIR_ABSORPTION_GAINHF,
                          env.AirAbsorptionHF ) );
        A_CHK( alEffectf( effect, AL_EAXREVERB_ROOM_ROLLOFF_FACTOR,
                          env.RoomRolloffFactor ) );

        if ( env.version == sndenv_ver_extended ) {
            A_CHK( alEffectf( effect, AL_EAXREVERB_GAINLF, env.RoomLF ) );
            // A_CHK(alEffectf(effect, AL_EAXREVERB_REFLECTIONS_PAN,
            // *env.ReflectionsPan)); A_CHK(alEffectf(effect,
            // AL_EAXREVERB_LATE_REVERB_PAN, *env.ReverbPan));
            A_CHK( alEffecti( effect, AL_EAXREVERB_DECAY_HFLIMIT,
                              env.DecayHFLimit ) );
            A_CHK( alEffectf( effect, AL_EAXREVERB_ECHO_TIME, env.EchoTime ) );
            A_CHK(
                alEffectf( effect, AL_EAXREVERB_ECHO_DEPTH, env.EchoDepth ) );
            A_CHK( alEffectf( effect, AL_EAXREVERB_LATE_REVERB_DELAY,
                              env.ReverbDelay ) );
            A_CHK( alEffectf( effect, AL_EAXREVERB_DECAY_LFRATIO,
                              env.DecayLFRatio ) );
...
void CSoundRender_CoreA::get_listener( CSoundRender_Environment& env ) {
    ...
    A_CHK(
        alGetEffectf( effect, AL_EAXREVERB_DECAY_HFRATIO, &env.DecayHFRatio ) );
    A_CHK(
        alGetEffectf( effect, AL_EAXREVERB_DECAY_LFRATIO, &env.DecayLFRatio ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_REFLECTIONS_GAIN,
                         &env.Reflections ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_REFLECTIONS_DELAY,
                         &env.ReflectionsDelay ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_LATE_REVERB_GAIN, &env.Reverb ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_REFLECTIONS_PAN,
                         env.ReflectionsPan ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_LATE_REVERB_DELAY,
                         &env.ReverbDelay ) );
    ...
    A_CHK(
        alGetEffectf( effect, AL_EAXREVERB_LATE_REVERB_PAN, env.ReverbPan ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_MODULATION_TIME,
                         &env.ModulationTime ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_MODULATION_DEPTH,
                         &env.ModulationDepth ) );
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_AIR_ABSORPTION_GAINHF,
                         &env.AirAbsorptionHF ) );
    ...
    A_CHK( alGetEffectf( effect, AL_EAXREVERB_ROOM_ROLLOFF_FACTOR,
                         &env.RoomRolloffFactor ) );
    A_CHK( alEffecti( effect, AL_EAXREVERB_DECAY_HFLIMIT,
                      ( ALint )env.DecayHFLimit ) );
}
...
void CSoundRender_CoreA::refresh_devices() {
    ...
    Msg( "SOUND: OpenAL: Device list refreshed, %d devices found",
         pDeviceList->GetNumDevices() );

    if ( pDeviceList->GetNumDevices() == 0 )
        return;

    const ALDeviceDesc* pDeviceDesc =
        pDeviceList->GetDeviceDescByName( snd_device_name.c_str() );
    if ( !pDeviceDesc ) {
        Msg( "! SOUND: Current device not found after refresh: %s. Switching "
             "to default",
             snd_device_name.c_str() );
...
void CSoundRender_CoreA::default_device_changed() {
    ...
    if ( _stricmp( snd_device_name.c_str(),
                   pDeviceList->GetDefaultDeviceName() ) == 0 ) {
...
void CSoundRender_CoreA::switch_device( LPCSTR device_name ) {
    ...
    const ALDeviceDesc* pDeviceDesc =
        pDeviceList->GetDeviceDescByName( device_name );
    if ( !pDeviceDesc ) {
    ...
    Msg( "SOUND: Attempting to open device: name='%s', name_al='%s'",
         deviceDesc.name, deviceDesc.name_al );
    ...
    if ( !newContext ) {
        Msg( "! SOUND: Failed to create context for device: %s",
             deviceDesc.name );
    ...
    for ( u32 it = 0; it < s_targets.size(); it++ ) {
        CSoundRender_TargetA* AlTarget =
            ( CSoundRender_TargetA* )s_targets[ it ];
        AlTarget->_destroy();
    }
...
void CSoundRender_CoreA::_initialize( int stage ) {
    ...
    const ALDeviceDesc* pDeviceDesc =
        pDeviceList->GetDeviceDescByName( snd_device_name.c_str() );
    if ( !pDeviceDesc ) {
        Msg( "! SOUND: Saved device not found: %s. Falling back to default.",
             snd_device_name.c_str() );
        snd_device_name = "";
        pDeviceList->SelectBestDevice();
        pDeviceDesc =
    ...
    R_ASSERT2( pDeviceDesc, make_string( "Sound device not found: %s",
                                         snd_device_name.c_str() ) );
    const ALDeviceDesc& deviceDesc = *pDeviceDesc;
    // OpenAL device
...
void CSoundRender_CoreA::update_listener( const Fvector& P,
                                          const Fvector& D,
                                          const Fvector& N,
                                          float dt ) {
    ...
    Listener.accVelocity.x = soundSmoothingParams::getSmoothedValue(
        Listener.curVelocity.x * p / dt, Listener.accVelocity.x, a );
    Listener.accVelocity.y = soundSmoothingParams::getSmoothedValue(
        Listener.curVelocity.y * p / dt, Listener.accVelocity.y, a );
    Listener.accVelocity.z = soundSmoothingParams::getSmoothedValue(
        Listener.curVelocity.z * p / dt, Listener.accVelocity.z, a );

    Listener.prevVelocity.set( Listener.accVelocity );

    // Msg("listener sound update delta %.3f, velocity %.3f, %.3f, %.3f, power
    // %.1f, alpha %.3f", dt, Listener.prevVelocity.x, Listener.prevVelocity.y,
    // Listener.prevVelocity.z, soundSmoothingParams::power,
    // soundSmoothingParams::alpha);
    ...
    A_CHK( alListener3f( AL_POSITION, Listener.position.x, Listener.position.y,
                         -Listener.position.z ) );
    A_CHK( alListener3f( AL_VELOCITY, Listener.prevVelocity.x,
                         Listener.prevVelocity.y, -Listener.prevVelocity.z ) );
