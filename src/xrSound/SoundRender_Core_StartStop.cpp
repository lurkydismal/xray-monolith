...
void CSoundRender_Core::i_start( CSoundRender_Emitter* E ) {
    ...
    if ( E->target->get_emitter() ) {
        float dist = SoundRender->listener_position().distance_to(
            E->target->get_emitter()->p_source.position );
        if ( dist > E->target->get_emitter()->p_source.max_distance &&
             !E->target->get_emitter()->b2D )
...
void CSoundRender_Core::i_stop( CSoundRender_Emitter* E ) {
    // Msg					("- %10s : %3d[%1.4f] :
    // %s","i_stop",E->dbg_ID,E->priority(),E->source->fname);
    R_ASSERT( E );
...
void CSoundRender_Core::i_rewind( CSoundRender_Emitter* E ) {
    // Msg					("- %10s : %3d[%1.4f] :
    // %s","i_rewind",E->dbg_ID,E->priority(),E->source->fname);
    R_ASSERT( E );
    R_ASSERT( E == E->target->get_emitter() );
    E->target->rewind();
}
