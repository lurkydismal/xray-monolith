...
void CSoundRender_Emitter::fill_data( u8* _dest, u32 offset, u32 size ) {
    /*
            Msg				("stream: %10s -
    %d",*source->fname,size); CopyMemory
    (_dest,&source->m_buffer.front()+offset,size); return;
    //*/
    /*
            Memory.mem_fill	(_dest,0,size);	// debug only
            //	Msg			("stream: %10s -
    %d",*source->fname,size); int				dummy;
            ov_pcm_seek
    (source->ovf,(psSoundFreq==sf_22K)?offset:offset/2);
    //	ov_pcm_seek		(source->ovf,0);
            ...
            {
                    int ret		=
    ov_read(source->ovf,dest,left,0,2,1,&dummy);
    //		Msg			("Part: %d - %d",left,ret);
                    ...
                    break;
                            ...
                            switch (ret){
                            case OV_HOLE:		Msg("OV_HOLE");
    continue; break; case OV_EBADLINK:	Msg("OV_EBADLINK"); continue; break;
                            }
                            break;
                    }
            }
...
void CSoundRender_Emitter::fill_block( void* ptr, u32 size ) {
    // Msg			("stream: %10s - [%X]:%d, p=%d,
    // t=%d",*source->fname,ptr,size,position,source->dwBytesTotal);
    ...
    if ( ( get_cursor( true ) + size ) > dwBytesTotal ) {
        ...
        switch ( m_current_state ) {
            ...
            case stPlayingLooped: {
                ...
                do {
                    ...
                    fill_data( dest + hw_position, get_cursor( true ),
                               sz_write );
        ...
        u32 bt_handle =
            ( ( CSoundRender_Source* )owner_data->handle )->dwBytesTotal;
