...
int CSoundRender_Core::pause_emitters( bool val ) {
    ...
    for ( u32 it = 0; it < s_emitters.size(); it++ )
        ( ( CSoundRender_Emitter* )s_emitters[ it ] )
            ->pause( val, val ? m_iPauseCounter : m_iPauseCounter + 1 );
...
void CSoundRender_Core::set_geometry_som( IReader* I ) {
    ...
    while ( !geom->eof() ) {
        SOM_poly P;
        geom->r( &P, sizeof( P ) );
        ETOOLS::collector_add_face_pd( CL, P.v1, P.v2, P.v3, *( u32* )&P.occ,
                                       0.01f );
        if ( P.b2sided )
            ETOOLS::collector_add_face_pd( CL, P.v3, P.v2, P.v1,
                                           *( u32* )&P.occ, 0.01f );
    }
    ...
    geom_SOM->build( CL.getV(), int( CL.getVS() ), CL.getT(),
                     int( CL.getTS() ) );
#endif
...
void CSoundRender_Core::set_geometry_env( IReader* I ) {
    ...
    for ( u32 it = 0; it < H.facecount; it++ ) {
        CDB::TRI* T = tris + it;
        u16 id_front = ( u16 )( ( T->dummy & 0x0000ffff ) >> 0 ); //	front
                                                                  //face
        u16 id_back = ( u16 )( ( T->dummy & 0xffff0000 ) >> 16 ); //	back
                                                                  //face
...
void CSoundRender_Core::create( ref_sound& S,
                                const char* fName,
...
void CSoundRender_Core::attach_tail( ref_sound& S, const char* fName ) {
...
#ifdef DEBUG
        Msg( "! 2 file already in queue [%s][%s]",
             S._p->fn_attached[ 0 ].c_str(), S._p->fn_attached[ 1 ].c_str() );
#endif // #ifdef DEBUG
        return;
    }
    ...
    if ( S._feedback() )
        ( ( CSoundRender_Emitter* )S._feedback() )->fTimeToStop +=
            s->length_sec();

    SoundRender->i_destroy_source( s );
}

void CSoundRender_Core::clone( ref_sound& S,
    ...
    S._p->g_type =
        ( game_type == sg_SourceType ) ? S._p->handle->game_type() : game_type;
    S._p->s_type = sound_type;
}

void CSoundRender_Core::play( ref_sound& S,
                              CObject* O,
                              u32 flags,
                              float delay ) {
...
void CSoundRender_Core::play_no_feedback( ref_sound& S,
                                          CObject* O,
                                          u32 flags,
                                          float delay,
                                          Fvector* pos,
...
void CSoundRender_Core::play_at_pos( ref_sound& S,
                                     CObject* O,
                                     const Fvector& pos,
                                     u32 flags,
                                     float delay ) {
...
void CSoundRender_Core::_create_data( ref_sound_data& S,
                                      LPCSTR fName,
                                      esound_type sound_type,
                                      int game_type ) {
    string_path fn;
    ...
    S.g_type =
        ( game_type == sg_SourceType ) ? S.handle->game_type() : game_type;
    S.s_type = sound_type;
    S.feedback = 0;
    S.g_object = 0;
    S.g_userdata = 0;
...
CSoundRender_Environment* CSoundRender_Core::get_environment(
    const Fvector& P ) {
    PROF_EVENT();
    ...
    if ( bUserEnvironment ) {
        ...
        if ( geom_ENV ) {
            ...
            if ( geom_DB.r_count() ) {
                ...
                Fvector tri_norm;
                tri_norm.mknormal( V[ T->verts[ 0 ] ], V[ T->verts[ 1 ] ],
                                   V[ T->verts[ 2 ] ] );
                float dot = dir.dotproduct( tri_norm );
                if ( dot < 0 ) {
                    u16 id_front = ( u16 )( ( T->dummy & 0x0000ffff ) >>
                                            0 ); //	front face
                    return s_environment->Get( id_front );
                } else {
                    u16 id_back = ( u16 )( ( T->dummy & 0xffff0000 ) >>
                                           16 ); //	back face
...
void CSoundRender_Core::update_listener( const Fvector& P,
                                         const Fvector& D,
                                         const Fvector& N,
                                         float dt ) {}
    ...
    if ( obj ) {
        for ( u32 eit = 0; eit < s_emitters.size(); eit++ ) {
            if ( s_emitters[ eit ] && s_emitters[ eit ]->owner_data &&
                 ( obj == s_emitters[ eit ]->owner_data->g_object ) )
