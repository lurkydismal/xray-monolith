...
// -----------------------------------------------------------------------
IC bool IntersectRaySphere( const Fvector& rO,
                            const Fvector& rV,
                            const Fvector& sO,
                            float sR ) {
    Fvector Q;
...
IC bool TestRayTri( const Fvector& C,
                    const Fvector& D,
                    Fvector** p,
                    float& u,
                    float& v,
...
//--------------------------------
IC bool TestRayTri2( const Fvector& C,
                     const Fvector& D,
                     Fvector* p,
                     float& range ) {
    Fvector edge1, edge2, tvec, pvec, qvec;
    ...
    qvec.crossproduct( tvec, edge1 ); // prepare to test V parameter
    range =
        edge2.dotproduct( qvec ) * inv_det; // calculate t, ray intersects plane
    if ( u < 0.0f || u > 1.0f )
        return false;
...
IC bool TestBBoxTri( const Fmatrix33& A,
    ...
    float R =
        _abs( extA.x * A0dN ) + _abs( extA.y * A1dN ) + _abs( extA.z * A2dN );
...
//----------------------------------------------------------------------------
IC float MgcSqrDistance( const Fvector& rkPoint,
                         const Fvector& orig,
                         const Fvector& e0,
                         const Fvector& e1 ) {
    Fvector kDiff;
...
IC EST_Result TestSphereTri( const Fvector& sphereOrigin,
                             float sphereRadius,
                             const Fvector& orig,
                             const Fvector& e0,
                             const Fvector& e1 ) {
...
//----------------------------------------------------------------------------
IC bool TestRayOBB( const Fvector3& origin,
    ...
    fADdU[ 0 ] = _abs( fDdU[ 0 ] );
    if ( fADdU[ 0 ] > rkBox.m_halfsize[ 0 ] &&
         fDdU[ 0 ] * fWdU[ 0 ] >= ( float )0.0 )
        return false;
    ...
    fADdU[ 1 ] = _abs( fDdU[ 1 ] );
    if ( fADdU[ 1 ] > rkBox.m_halfsize[ 1 ] &&
         fDdU[ 1 ] * fWdU[ 1 ] >= ( float )0.0 )
        return false;
    ...
    fADdU[ 2 ] = _abs( fDdU[ 2 ] );
    if ( fADdU[ 2 ] > rkBox.m_halfsize[ 2 ] &&
         fDdU[ 2 ] * fWdU[ 2 ] >= ( float )0.0 )
        return false;
    ...
    fRhs =
        rkBox.m_halfsize[ 1 ] * fAWdU[ 2 ] + rkBox.m_halfsize[ 2 ] * fAWdU[ 1 ];
    if ( fAWxDdU[ 0 ] > fRhs )
        return false;

    fAWxDdU[ 1 ] = _abs( kWxD.dotproduct( rkBox.m_rotate.j ) );
    fRhs =
        rkBox.m_halfsize[ 0 ] * fAWdU[ 2 ] + rkBox.m_halfsize[ 2 ] * fAWdU[ 0 ];
    if ( fAWxDdU[ 1 ] > fRhs )
        return false;

    fAWxDdU[ 2 ] = _abs( kWxD.dotproduct( rkBox.m_rotate.k ) );
    fRhs =
        rkBox.m_halfsize[ 0 ] * fAWdU[ 1 ] + rkBox.m_halfsize[ 1 ] * fAWdU[ 0 ];
    if ( fAWxDdU[ 2 ] > fRhs )
        return false;
