...
CSoundRender_Source* CSoundRender_Core::i_create_source( LPCSTR name ) {
    ...
    SoundPrefetchJob* job =
        found == m_source_prefetch_by_id.end() ? nullptr : found->second;
    if ( !job || _stricmp( job->path.c_str(), path.c_str() ) ) {
        // Level-local and dynamically discovered sounds retain the original
        // synchronous path.
...
#endif
        finish_source_prepare( *job, std::move( prepared ),
                               std::move( error ) );
        lock.lock();
...
void CSoundRender_Core::build_source_prefetch_manifest() {
    ...
    for ( const FS_File& file : files ) {
        ...
        if ( m_source_prefetch_by_id.find( id ) !=
             m_source_prefetch_by_id.end() )
            continue;
    ...
    std::sort(
        m_source_prefetch_order.begin(), m_source_prefetch_order.end(),
...
void CSoundRender_Core::source_prefetch_worker() {
    ...
    for ( ;; ) {
        ...
        {
            std::lock_guard< std::mutex > lock( m_source_prefetch_mutex );
            if ( m_source_prefetch_pause || m_source_prefetch_shutdown ||
                 m_source_prefetch_failure )
                break;
            while ( m_source_prefetch_cursor <
                    m_source_prefetch_order.size() ) {
                SoundPrefetchJob* candidate =
                    m_source_prefetch_order[ m_source_prefetch_cursor++ ];
        ...
        if ( succeeded ) {
            std::unique_lock< std::mutex > lock( m_source_prefetch_mutex );
            m_source_prefetch_changed.wait_for(
                lock,
                std::chrono::milliseconds( m_source_prefetch_idle_ms.load(
                    std::memory_order_relaxed ) ),
                [ this ]() {
                    return m_source_prefetch_pause ||
...

    {
        ...
        if ( !m_source_prefetch_failure &&
             source_prefetch_remaining_locked() == 0 )
            m_source_prefetch_completion_pending = true;
    }
    m_source_prefetch_changed.notify_all();
}

void CSoundRender_Core::finish_source_prepare( SoundPrefetchJob& job,
                                               PreparedSoundSource&& prepared,
                                               xr_string&& error ) {
    {
        std::lock_guard< std::mutex > lock( m_source_prefetch_mutex );
...
CSoundRender_Source* CSoundRender_Core::commit_source_locked(
    SoundPrefetchJob& job ) {
...
u64 CSoundRender_Core::source_prefetch_hash_locked() const {
    ...
    for ( const SoundPrefetchJob* job : m_source_prefetch_jobs ) {
        ...
        append( &job->prepared.bytes_total,
                sizeof( job->prepared.bytes_total ) );
        append( &job->prepared.base_volume,
                sizeof( job->prepared.base_volume ) );
        append( &job->prepared.min_distance,
                sizeof( job->prepared.min_distance ) );
        append( &job->prepared.max_distance,
                sizeof( job->prepared.max_distance ) );
        append( &job->prepared.max_ai_distance,
                sizeof( job->prepared.max_ai_distance ) );
...
void CSoundRender_Core::source_prefetch_start() {
    ...
    m_source_prefetch_idle_ms.store(
        m_source_prefetch_started_at ? RuntimeSoundIdleMs : StartupSoundIdleMs,
        std::memory_order_relaxed );
    ...
    if ( m_source_prefetch_thread.joinable() ) {
        ...
        m_source_prefetch_running =
            true; // Reserve restart while the completed worker is reaped.
        lock.unlock();
        completed.join();
        lock.lock();
        m_source_prefetch_running = false;
	...
	{
#endif
    m_source_prefetch_thread =
        std::thread( &CSoundRender_Core::source_prefetch_worker, this );
#if 0
	}
	catch (...)
	{
...
#endif
    Msg( "* [SOUND PREFETCH] started: remaining=%u",
         source_prefetch_remaining_locked() );
}
...
void CSoundRender_Core::source_prefetch_pause() {
    ...
    if ( m_source_prefetch_enabled )
        Msg(
            "* [SOUND PREFETCH] paused: prepared=%u, promoted=%u, remaining=%u",
            m_source_prefetch_prepared, m_source_prefetch_promoted,
            source_prefetch_remaining_locked() );
}

void CSoundRender_Core::source_prefetch_prepare(
    const xr_vector< xr_string >& sources ) {
    ...
    timer.Start();
    xr_parallel_for(
        0u, static_cast< u32 >( sources.size() ), [ & ]( u32 index ) {
            string256 id;
    ...
    Msg( "* [SOUND PREFETCH] requested sources prepared: requested=%u time=%u "
         "ms",
...
void CSoundRender_Core::source_prefetch_poll() {
    ...
    xr_string failure;
    {
        ...
        } else if ( m_source_prefetch_completion_pending &&
                    !m_source_prefetch_completion_logged ) {
            m_source_prefetch_completion_pending = false;
            m_source_prefetch_completion_logged = true;
            Msg( "* [SOUND PREFETCH] complete: prepared=%u, promoted=%u, "
                 "demand_wait=%u ms, "
                 "duration=%u ms, metadata_hash=%016llx",
                 m_source_prefetch_prepared, m_source_prefetch_promoted,
                 m_source_prefetch_waited_ms,
                 GetTickCount() - m_source_prefetch_started_at,
                 source_prefetch_hash_locked() );
        }
    }
    if ( !failure.empty() ) {
        Msg( "! [SOUND PREFETCH] failed: %s (%s)", failed_path.c_str(),
             failure.c_str() );
