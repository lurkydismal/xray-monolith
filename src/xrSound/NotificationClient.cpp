...
CNotificationClient::CNotificationClient() : m_bComInitialized( false ) {
    Start();
}
...
inline bool CNotificationClient::Start() {
    ...
    // RPC_E_CHANGED_MODE means COM already initialized in different mode - this
    // is OK
...
inline STDMETHODIMP_( HRESULT __stdcall )
    CNotificationClient::OnDeviceAdded( LPCWSTR pwstrDeviceId ) {
...
inline STDMETHODIMP_( HRESULT __stdcall )
    CNotificationClient::OnDefaultDeviceChanged( EDataFlow flow,
                                                 ERole role,
...
inline STDMETHODIMP_( HRESULT __stdcall )
    CNotificationClient::OnDeviceRemoved( LPCWSTR pwstrDeviceId ) {
    SoundRenderA->bPendingDeviceListRefresh = TRUE;
    return S_OK;
}

inline STDMETHODIMP_( HRESULT __stdcall )
    CNotificationClient::OnDeviceStateChanged( LPCWSTR pwstrDeviceId,
                                               DWORD dwNewState ) {
    SoundRenderA->bPendingDeviceListRefresh = TRUE;
    return S_OK;
}
...
inline STDMETHODIMP_( HRESULT __stdcall )
    CNotificationClient::OnPropertyValueChanged( LPCWSTR pwstrDeviceId,
