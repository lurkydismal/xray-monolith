...
inline u32 calc_cursor( const float& fTimeStarted,
                        float& fTime,
                        const float& fTimeTotal,
                        const WAVEFORMATEX& wfx ) {
    if ( fTime < fTimeStarted )
        fTime = fTimeStarted; // Андрюха посоветовал, ассерт что ниже вылетел из
                              // за паузы как то хитро
    ...
    u32 curr_sample_num =
        iFloor( ( fTime - fTimeStarted ) * wfx.nSamplesPerSec );
...
void CSoundRender_Emitter::update( float dt ) {
    ...
    VERIFY2( !!( owner_data ) ||
                 ( !( owner_data ) && ( m_current_state == stStopped ) ),
             "owner" );
    ...
    // demonized: add preplay update, calculate delay based on distance and
    // speed of sound Adaptation for user experience For distances < 30m
    // (configurable via cvar), no delay Then gradually ramp up to delay
    // calculated by math Starting at 90m, full delay kicks in Don't apply delay
    // to already delayed sounds, they are most likely already handled in
    // respective parts of code and scripts
    if ( need_preplay_update && m_current_state < stPlaying &&
         starting_delay == 0.f ) {
        if ( _valid( p_source.position ) &&
             !p_source.position.similar( Fvector().set( 0.f, 0.f, 0.f ) ) &&
             owner_data && source() && source()->channels_num() == 1 ) {
            // Smooth Ramp Using Hermite(Smootherstep)
            static auto CalculateSmoothSoundDelay =
                []( float distance, float speedOfSound, float rampStart,
                    float rampRange ) {
                    ...
                    // Ramp factor from 0 to 1 between rampStart and rampStart +
                    // rampRange
                    float t = std::clamp( ( distance - rampStart ) / rampRange,
                                          0.0f, 1.0f );
            ...
            auto delay = CalculateSmoothSoundDelay(
                p_source.position.distance_to(
                    SoundRender->listener_position() ),
                speedOfSound,
            ...
            if ( delay > 0.f ) {
                if ( m_current_state == stStarting ||
                     m_current_state == stStartingLooped ) {
                    starting_delay = delay;
                    m_current_state = m_current_state == stStarting
                                          ? stStartingDelayed
                ...
                /*Msg("CSoundRender_Emitter::update, file %s, state %s,
                need_preplay_update, distance %.2f, old delay %.2f, delay %.2f",
                        owner_data && source() && source()->file_name() ?
        ...
        case stSimulating:
            ...
            if ( fTime >= fTimeToStop ) {
                ...
                if ( update_culling( dt ) ) {
                    ...
                    /*
                                                    u32 ptr
                       = calc_cursor(	fTimeStarted, fTime, get_length_sec(),
                                                                                                                                            source()->m_wformat);
                                                    set_cursor
                       (ptr);
                    */
...
BOOL CSoundRender_Emitter::update_culling( float dt ) {
    ...
    if ( b2D ) {
        ...
        float fade_scale =
            bStopping || ( p_source.base_volume * p_source.volume *
                               ( owner_data->s_type == st_Effect
                                     ? psSoundVEffects * psSoundVFactor
                                     : psSoundVMusic * psSoundVMusicFactor ) <
                           psSoundCull )
        ...
        float occ = ( owner_data->g_type == SOUND_TYPE_WORLD_AMBIENT )
                        ? 1.0f
                        : SoundRender->get_occlusion( p_source.position, .2f,
                                                      occluder );
    ...
    // Update smoothing
    // LostAlphaRus in
    smooth_volume = ( p_source.base_volume * volume_att *
                      ( owner_data->s_type == st_Effect
                            ? psSoundVEffects * psSoundVFactor
                            : psSoundVMusic * psSoundVMusicFactor ) *
