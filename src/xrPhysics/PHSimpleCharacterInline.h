void CPHSimpleCharacter::UpdateStaticDamage( dContact* c,
                                             SGameMtl* tri_material,
                                             bool bo1 ) {
...
void CPHSimpleCharacter::UpdateDynamicDamage( dContact* c,
                                              u16 obj_material_idx,
                                              dBodyID b,
                                              bool bo1 ) {
    ...
    dVector3 Pc = { vel[ 0 ] * m_mass + obj_vel[ 0 ] * m.mass,
                    vel[ 1 ] * m_mass + obj_vel[ 1 ] * m.mass,
    ...
    dReal accepted_energy =
        Kself * m_collision_damage_factor + Kobj * object_damage_factor - KK;
    // DeltaK=m1*m2*(v1-v2)^2/(2*(m1+m2))
    if ( accepted_energy > 0.f ) {
        SGameMtl* obj_material =
            GMLibrary().GetMaterialByIdx( obj_material_idx );
        c_vel = dSqrt( accepted_energy / m_mass * 2.f ) *
                obj_material->fBounceDamageFactor;
    } else
        c_vel = 0.f;
#ifdef DEBUG
    ...
    if ( c_vel > m_collision_damage_info.m_contact_velocity ) {
        ...
        __try {
            ...
            if ( !obj->ObjectGetDestroy() ) {
                ...
                m_collision_damage_info.m_dmc_signum = bo1 ? 1.f : -1.f;
                m_collision_damage_info.m_dmc_type =
                    SCollisionDamageInfo::ctObject;
                m_collision_damage_info.m_damege_contact = *c;
                m_collision_damage_info.m_hit_callback =
                    obj->ObjectGetCollisionHitCallback();
                m_collision_damage_info.m_obj_id = obj->ObjectID();
            }
#ifdef _MSC_VER
        } __except ( EXCEPTION_EXECUTE_HANDLER ) {
            // Stale geometry owner pointer. Ignore this collision source
            // safely.
        }
#endif
    }
...
IC void CPHSimpleCharacter::foot_material_update( u16 contact_material_idx,
    ...
    const SGameMtl* contact_material =
        GMLibrary().GetMaterialByIdx( contact_material_idx );
