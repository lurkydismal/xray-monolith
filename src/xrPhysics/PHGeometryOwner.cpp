...
void CPHGeometryOwner::get_mc_kinematics( IKinematics* K,
                                          Fvector& mc,
                                          float& mass ) {
    mc.set( 0.f, 0.f, 0.f );
    mass = 0.f;
...
void CPHGeometryOwner::add_Cylinder( const Fcylinder& V ) {
    m_geoms.push_back(
        smart_cast< CODEGeom* >( xr_new< CCylinderGeom >( V ) ) );
}
...
void CPHGeometryOwner::set_ObjectContactCallback(
...
void CPHGeometryOwner::add_ObjectContactCallback(
    ObjectContactCallbackFun* callback ) {
    if ( !object_contact_callback ) {
        object_contact_callback = callback;
    }
...
void CPHGeometryOwner::remove_ObjectContactCallback(
    ObjectContactCallbackFun* callback ) {
...
void CPHGeometryOwner::get_Extensions( const Fvector& axis,
                                       float center_prg,
                                       float& lo_ext,
                                       float& hi_ext ) const {
...
CODEGeom* CPHGeometryOwner::GeomByBoneID( u16 bone_id ) {
    GEOM_I g =
        std::find_if( m_geoms.begin(), m_geoms.end(), SFindPred( bone_id ) );
    if ( g != m_geoms.end() ) {
        return *g;
    } else {
        return NULL;
