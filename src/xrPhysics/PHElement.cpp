...
void CPHElement::build() {
    ...
    VERIFY_BOUNDARIES2( m_mass_center, phBoundaries, PhysicsRefObject(),
                        "m_mass_center" );

    dBodySetPosition( m_body, m_mass_center.x, m_mass_center.y,
                      m_mass_center.z );
...
void CPHElement::calc_it_fract_data_use_density( const Fvector& mc,
                                                 float density ) {
...
dMass CPHElement::recursive_mass_summ( u16 start_geom,
                                       FRACTURE_I cur_fracture ) {
    dMass end_mass;
    dMassSetZero( &end_mass );
    GEOM_I i_geom = m_geoms.begin() + start_geom,
           e = m_geoms.begin() + cur_fracture->m_start_geom_num;
    for ( ; i_geom != e; ++i_geom )
    ...
    if ( m_fratures_holder->m_fractures.end() != cur_fracture )
        cur_fracture->SetMassParts(
            m_mass, recursive_mass_summ( start_geom, cur_fracture ) );
    return end_mass;
}
...
void CPHElement::SetTransform( const Fmatrix& m0,
                               motion_history_state history_state ) {
#ifdef DEBUG
    VERIFY2( _valid( m0 ), make_string( "invalid_form_in_set_transform" ) +
                               ( PhysicsRefObject()->dump( full_capped ) ) );
    VERIFY2( valid_pos( m0.c ),
             dbg_valide_pos_string( m0.c, PhysicsRefObject(),
                                    "invalid_form_in_set_transform" ) );
#endif
    Fvector mc;
    CPHGeometryOwner::get_mc_vs_transform( mc, m0 );
    VERIFY_BOUNDARIES2( mc, phBoundaries, PhysicsRefObject(),
                        "mass	center	in set transform" );
    ...
    if ( history_state != mh_not_clear )
        CPHGeometryOwner::clear_motion_history( mh_unspecified ==
                                                history_state );
}
...
void CPHElement::SetGlobalPositionDynamic( const Fvector& position ) {
    ...
    VERIFY_BOUNDARIES2( position, phBoundaries, PhysicsRefObject(),
                        "SetGlobalPosition argument " );
...
void CPHElement::TransformPosition( const Fmatrix& form,
    ...
    PHDynamicData::DMXPStoFMX( dBodyGetRotation( m_body ),
                               dBodyGetPosition( m_body ), bm );
    Fmatrix new_bm;
    new_bm.mul( form, bm );
    dMatrix3 dBM;
    PHDynamicData::FMXtoDMX( new_bm, dBM );
    dBodySetRotation( m_body, dBM );
    VERIFY_BOUNDARIES2( new_bm.c, phBoundaries, PhysicsRefObject(),
                        "TransformPosition dest pos" );
...
void CPHElement::SetBoneCallback() {
    IKinematics* K = m_shell->PKinematics();
    VERIFY( K );
    K->LL_GetBoneInstance( m_SelfID )
        .set_callback( bctPhysics, m_shell->GetBonesCallback(),
                       cast_PhysicsElement( this ) );
}
...
void CPHElement::Activate( const Fmatrix& transform,
...
void CPHElement::Activate( const Fmatrix& m0,
                           float dt01,
                           const Fmatrix& m2,
                           bool disable ) {
    Fvector lvel, avel;
...
void CPHElement::PhTune( dReal step ) {
    ...
    VERIFY_BOUNDARIES2( cast_fv( dBodyGetPosition( m_body ) ), phBoundaries,
                        PhysicsRefObject(), "PhTune body position" );
}
...
void CPHElement::PhDataUpdate( dReal step ) {
    ...
    // VERIFY_BOUNDARIES2(cast_fv(dBodyGetPosition(m_body)),phBoundaries,PhysicsRefObject(),"PhDataUpdate
    // begin, body position");
#ifdef DEBUG
    if ( debug_output().ph_dbg_draw_mask().test( phDbgDrawMassCenters ) ) {
        debug_output().DBG_DrawPoint( cast_fv( dBodyGetPosition( m_body ) ),
                                      0.03f, D3DCOLOR_XRGB( 255, 0, 0 ) );
    }
#endif
    ...
    if ( !dV_valid( angular_velocity ) ) {
        Msg( "angular vel %f,%f,%f", angular_velocity[ 0 ],
             angular_velocity[ 1 ], angular_velocity[ 2 ] );
        Msg( "linear vel %f,%f,%f", linear_velocity[ 0 ], linear_velocity[ 1 ],
             linear_velocity[ 2 ] );
        Msg( "position  %f,%f,%f", dBodyGetPosition( m_body )[ 0 ],
             dBodyGetPosition( m_body )[ 1 ], dBodyGetPosition( m_body )[ 2 ] );
        Msg( "quaternion  %f,%f,%f,%f", dBodyGetQuaternion( m_body )[ 0 ],
             dBodyGetQuaternion( m_body )[ 1 ],
             dBodyGetQuaternion( m_body )[ 2 ],
             dBodyGetQuaternion( m_body )[ 3 ] );
        Msg( "matrix" );
        Msg( "x  %f,%f,%f", dBodyGetRotation( m_body )[ 0 ],
             dBodyGetRotation( m_body )[ 4 ], dBodyGetRotation( m_body )[ 8 ] );
        Msg( "y  %f,%f,%f", dBodyGetRotation( m_body )[ 1 ],
             dBodyGetRotation( m_body )[ 5 ], dBodyGetRotation( m_body )[ 9 ] );
        Msg( "z  %f,%f,%f", dBodyGetRotation( m_body )[ 2 ],
             dBodyGetRotation( m_body )[ 6 ],
             dBodyGetRotation( m_body )[ 10 ] );
    ...
    dBodySetLinearVel( m_body, linear_velocity[ 0 ] / m_l_scale,
                       linear_velocity[ 1 ] / m_l_scale,
                       linear_velocity[ 2 ] / m_l_scale );
    dBodySetAngularVel( m_body, angular_velocity[ 0 ] / m_w_scale,
                        angular_velocity[ 1 ] / m_w_scale,
                        angular_velocity[ 2 ] / m_w_scale );

    ///////////////////scale changes values directly so get base values after
    /// it/////////////////////////
    /////////////////////////////base
    /// values////////////////////////////////////////////////////////////
    ...
    VERIFY2( dV_valid( dBodyGetQuaternion( m_body ) ),
             "invalid body rotation" );
    /*
            if(!valid_pos(cast_fv(dBodyGetPosition(m_body)),phBoundaries))
       //hack {
       //hack Fvector	pos;
       //hack m_body_interpolation.GetPosition(pos,0);
       //hack dBodySetPosition(m_body,pos.x,pos.y,pos.z);
       //hack }
       //hack
    */
    VERIFY_BOUNDARIES2( cast_fv( dBodyGetPosition( m_body ) ), phBoundaries,
                        PhysicsRefObject(), "PhDataUpdate end, body position" );
    UpdateInterpolation();
    ...
    if ( !fis_zero( k_w ) )
        dBodyAddTorque( m_body, -angular_velocity[ 0 ] * k_w,
...
void CPHElement::UnFreeze() {
    if ( !m_body )
        return;
    if ( m_flags.test(
             flWasEnabledBeforeFreeze ) /*was_enabled_before_freeze*/ )
        dBodyEnable( m_body );
}

bool dbg_draw_ph_force_apply = false;

void CPHElement::applyImpulseVsMC( const Fvector& pos,
                                   const Fvector& dir,
                                   float val ) {
    ...
    if ( dbg_draw_ph_force_apply ) {
        ...
        debug_output().DBG_DrawLine( draw_pos, Fvector().add( draw_pos, dir ),
                                     D3DCOLOR_XRGB( 255, 0, 0 ) );
        debug_output().DBG_ClosedCashedDraw( 50000 );
    }
#endif
    dBodyAddForceAtRelPos( m_body, impulse.x, impulse.y, impulse.z, pos.x,
                           pos.y, pos.z );
    BodyCutForce( m_body, m_l_limit, m_w_limit );
    ////////////////////////////////////////////////////////////////////////
}

void CPHElement::applyImpulseVsGF( const Fvector& pos,
                                   const Fvector& dir,
                                   float val ) {
    ...
    if ( dbg_draw_ph_force_apply ) {
        debug_output().DBG_OpenCashedDraw();
        debug_output().DBG_DrawLine( pos, Fvector().add( pos, dir ),
                                     D3DCOLOR_XRGB( 255, 05, 0 ) );
        debug_output().DBG_ClosedCashedDraw( 50000 );
    }
#endif
    /////////////////////////////////////////////////////////////////////////
    ...
    impulse.set( dir );
    impulse.mul( val );
    dBodyAddForceAtPos( m_body, impulse.x, impulse.y, impulse.z, pos.x, pos.y,
                        pos.z );
    BodyCutForce( m_body, m_l_limit, m_w_limit );
    VERIFY( dBodyStateValide( m_body ) );
    ////////////////////////////////////////////////////////////////////////
...
void CPHElement::applyImpulseTrace( const Fvector& pos,
                                    const Fvector& dir,
                                    float val,
                                    u16 id ) {
    VERIFY( _valid( pos ) && _valid( dir ) && _valid( val ) );
    ...
    if ( debug_output().ph_dbg_draw_mask().test( phHitApplicationPoints ) ) {
        ...
        dMULTIPLY0_331( cast_fp( dbg_position ), dBodyGetRotation( m_body ),
                        cast_fp( body_pos ) );
        dbg_position.add( cast_fv( dBodyGetPosition( m_body ) ) );
        debug_output().DBG_DrawPoint( dbg_position, 0.01f,
        ...
        debug_output().DBG_DrawLine(
            dbg_position,
            Fvector().add( dbg_position, Fvector().mul( dir, 0.4f ) ),
...
void CPHElement::StataticRootBonesCallBack( CBoneInstance* B ) {
    ...
    // VERIFY2(fsimilar(DET(B->mTransform),1.f,DET_CHECK_EPS),"Bones callback
    // resive 0 matrix");
    ...
    if ( m_flags.test( flActivating ) ) {
        ...
        // VERIFY2(fsimilar(DET(B->mTransform),1.f,DET_CHECK_EPS),"Bones
        // callback returns 0 matrix");
        VERIFY_RMATRIX( B->mTransform );
        VERIFY( valid_pos( B->mTransform.c, phBoundaries ) );
        // return;
    }
    ...
    VERIFY_RMATRIX( B->mTransform );
    VERIFY( valid_pos( B->mTransform.c, phBoundaries ) );
    // if( !m_shell->is_active() && !m_flags.test(flUpdate)/*!bUpdate*/ )
    // return;
...
bool CPHElement::AnimToVel( float dt, float l_limit, float a_limit ) {
    ...
    VERIFY( m_shell->PKinematics() );
    // CBoneInstance *BI =
    // &m_shell->PKinematics()->LL_GetBoneInstance(m_SelfID);
    //
...
void CPHElement::ToBonePos( const Fmatrix& BoneTransform,
                            motion_history_state history_state ) {
...
void CPHElement::SetBoneCallbackOverwrite( bool v ) {
    ...
    VERIFY( m_shell->PKinematics() );
    m_shell->PKinematics()
        ->LL_GetBoneInstance( m_SelfID )
        .set_callback_overwrite( v );
}
...
void CPHElement::BonesCallBack( CBoneInstance* B ) {
    ...
    // receive 0 matrix");
    VERIFY_RMATRIX( B->mTransform );
    VERIFY_BOUNDARIES2( B->mTransform.c, phBoundaries, PhysicsRefObject(),
                        "BonesCallBack incoming bone position" );
...
void CPHElement::set_LinearVel( const Fvector& velocity ) {
    ...
    if ( velocity.magnitude() > m_l_limit )
        Msg(
            " CPHElement::set_LinearVel set velocity magnitude is too large %f",
            velocity.magnitude() );
#endif
...
void CPHElement::set_AngularVel( const Fvector& velocity ) {
    ...
    if ( velocity.magnitude() > m_w_limit )
        Msg(
            "CPHElement::set_AngularVel set velocity magnitude is too large %f",
            velocity.magnitude() );
#endif
...
void CPHElement::applyForce( float x,
                             float y,
                             float z ) // called anywhere ph state influent
{
...
{
    applyForce( dir.x * val / fixed_step, dir.y * val / fixed_step,
                dir.z * val / fixed_step );
}
...
void CPHElement::add_Mass( const SBoneShape& shape,
    ...
    switch ( shape.type ) {
        case SBoneShape::stBox: {
            dMassSetBox( &m, 1.f, shape.box.m_halfsize.x * 2.f,
                         shape.box.m_halfsize.y * 2.f,
            ...
            dMassRotate( &m, DMatx );
            dMassTranslate( &m, shape.box.m_translate.x - mass_center.x,
                            shape.box.m_translate.y - mass_center.y,
        ...
        case SBoneShape::stSphere: {
            ...
            dMassAdjust( &m, mass );
            dMassTranslate( &m, shape.sphere.P.x - mass_center.x,
                            shape.sphere.P.y - mass_center.y,
        ...
        case SBoneShape::stCylinder: {
            ...
            dMassSetCylinder( &m, 1.f, 2, shape.cylinder.m_radius,
                              shape.cylinder.m_height );
            dMassAdjust( &m, mass );
            dMatrix3 DMatx;
            Fmatrix33 m33;
    ...
    dMassTranslate( &m_mass, m_mass_center.x, m_mass_center.y,
                    m_mass_center.z );
    if ( m_fratures_holder ) {
        m_fratures_holder->DistributeAdditionalMass( u16( m_geoms.size() - 1 ),
                                                     m );
    }
    if ( fracture ) {
...
void CPHElement::calculate_it_data_use_density( const Fvector& mc,
                                                float density ) {
    dMassSetZero( &m_mass );
...
void CPHElement::set_DynamicScales( float l_scale /* =default_l_scale */,
                                    float w_scale /* =default_w_scale */ ) {
    m_l_scale = l_scale;
    m_w_scale = w_scale;
}
...
void CPHElement::get_Extensions( const Fvector& axis,
                                 float center_prg,
                                 float& lo_ext,
                                 float& hi_ext ) const {
...
void CPHElement::ReAdjustMassPositions( const Fmatrix& shift_pivot,
                                        float density ) {
...
void CPHElement::cv2bone_Xfrom( const Fquaternion& q,
                                const Fvector& pos,
                                Fmatrix& xform ) {
...
void CPHElement::cv2obj_Xfrom( const Fquaternion& q,
                               const Fvector& pos,
                               Fmatrix& xform ) {
...
void CPHElement::CutVelocity( float l_limit, float a_limit ) {
    ...
    if ( blimitl || blimita ) {
        ...
        dBodySetLinearVel( m_body, limitedl[ 0 ], limitedl[ 1 ],
                           limitedl[ 2 ] );
        dBodySetAngularVel( m_body, limiteda[ 0 ], limiteda[ 1 ],
                            limiteda[ 2 ] );
    }
}
...
void CPHElement::SetScale( float scale ) {
    ...
    for ( int i = 0; i < numberOfGeoms(); ++i ) {
        ...
        dGeomSetPosition( geom->geom(), scaled_pos.x, scaled_pos.y,
                          scaled_pos.z );
    }
}
...
void CPHElement::dbg_draw_velocity( float scale, u32 color ) {
    ...
    debug_output().DBG_DrawLine(
        bone.c, Fvector().add( bone.c, cast_fv( res ).mul( scale ) ), color );
    // m_shell->PKinematics()->LL_GetTransform()
}

static void dBodyGetPointForce( dBodyID b,
                                dReal px,
                                dReal py,
                                dReal pz,
                                dVector3 result ) {
...
void CPHElement::dbg_draw_geometry(
    float scale,
    u32 color,
    Flags32 flags /*= Flags32().assign( 0 )*/ ) const {
