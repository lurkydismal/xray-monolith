...
SDisableUpdateState& SDisableUpdateState::operator&=(
    SDisableUpdateState& lstate ) {
...
CBaseDisableData::CBaseDisableData()
    : m_disabled( false ), m_last_frame_updated( u16( -1 ) ) {
...
void CBaseDisableData::Disabling() {
    ...
    if ( m_count ==
         0 ) // ph_world->disable_count==dis_frames//m_count==m_frames
    {
        UpdateL2();
    ...
    if ( dBodyIsEnabled( body ) ) {
        ReEnable();
        if ( !m_disabled &&
             ( ph_world->disable_count !=
               m_count % worldDisablingParams.objects_params.L2frames ) ) {
...
void CPHDisablingBase::UpdateValues( const Fvector& new_pos,
                                     const Fvector& new_vel ) {
    if ( m_count < m_frames ) {
        float velocity_param = m_mean_velocity.Update( new_pos );
        float acceleration_param = m_mean_acceleration.Update( new_vel );
        CheckState( m_stateL1, velocity_param * m_frames,
                    acceleration_param * m_frames );
    } else {
...
#endif
    CPHDisablingBase::UpdateValues( *( Fvector* )position,
                                    *( Fvector* )velocity );
    // float			velocity_param		=	m_mean_velocity
    // .Update(* (Fvector*) position)		; float
    // acceleration_param	=	m_mean_acceleration	.Update(*
    // (Fvector*) velocity)		; CheckState
    // (m_stateL1,velocity_param*m_frames,acceleration_param*m_frames)
    // ;
}

void CPHDisablingTranslational::set_DisableParams(
    const SAllDDOParams& params ) {
...
void CPHDisablingRotational::UpdateL1() {
    ...
    // float			velocity_param		=	m_mean_velocity
    // .Update	(			 vrotation	)	; float
    // acceleration_param	=	m_mean_acceleration	.Update	(*
    // (Fvector*) velocity	)	;

    // CheckState
    // (m_stateL1,velocity_param,acceleration_param)		;
}
