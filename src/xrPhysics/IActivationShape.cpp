...
void ActivateShapeExplosive( IPhysicsShellHolder* self_obj,
                             const Fvector& size,
                             Fvector& out_size,
                             Fvector& in_out_pos ) {
...
void ActivateShapePhysShellHolder( IPhysicsShellHolder* obj,
    ...
    if ( obj->ObjectPPhysicsShell() ) {
        activation_shape.collide_bits() =
            obj->ObjectPPhysicsShell()->collide_bits();
        activation_shape.collide_class_bits() =
            obj->ObjectPPhysicsShell()->collide_class_bits();
    }
    ...
    if ( !valid_pos( out_pos, phBoundaries ) ) {
        Msg( "not valid position	%f,%f,%f", out_pos.x, out_pos.y,
             out_pos.z );
