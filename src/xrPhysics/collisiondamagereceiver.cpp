...
void DamageReceiverCollisionCallback( bool& do_colide,
                                      bool bo1,
                                      dContact& c,
                                      SGameMtl* material_1,
                                      SGameMtl* material_2 ) {
    ...
    dxGeomUserData* ud_self = bo1 ? retrieveGeomUserData( c.geom.g1 )
                                  : retrieveGeomUserData( c.geom.g2 );
    dxGeomUserData* ud_damager = bo1 ? retrieveGeomUserData( c.geom.g2 )
    ...
    // CPHCollisionDamageReceiver	*dr	=
    // static_cast<CPhysicsShellHolder*>( o_self )->PHCollisionDamageReceiver();
    ICollisionDamageReceiver* dr =
        o_self ? ( o_self )->ObjectPhCollisionDamageReceiver() : nullptr;
    ...
    float dfs =
        ( material_self->fBounceDamageFactor + damager_material_factor );
    if ( fis_zero( dfs ) )
        return;
    Fvector dir;
    ...
    pos.sub( *( Fvector* )c.geom.pos,
             *( Fvector* )dGeomGetPosition( bo1 ? c.geom.g1 : c.geom.g2 ) );
    // it is not true pos in bone space

    if ( dr )
        dr->CollisionHit(
            source_id, ud_self->bone_id,
            E_NL( b1, b2, c.geom.normal ) * damager_material_factor / dfs, dir,
            pos );
}

void BreakableObjectCollisionCallback( bool& /**do_colide/**/,
                                       bool bo1,
                                       dContact& c,
                                       SGameMtl* /*material_1*/,
                                       SGameMtl* /*material_2*/ ) {
    ...
    if ( bo1 ) {
        if ( !usr_data_1->ph_ref_object )
            return;
        damag_receiver =
            usr_data_1->ph_ref_object->ObjectPhCollisionDamageReceiver();
        ...
        if ( !usr_data_2->ph_ref_object )
            return;
        damag_receiver =
            usr_data_2->ph_ref_object->ObjectPhCollisionDamageReceiver();
    ...
    /*
            ...
            VERIFY( usr_data_1 );
            VERIFY( usr_data_2 );
            this_object1 = smart_cast<CBreakableObject*>(
       usr_data_1->ph_ref_object ); this_object2 =
       smart_cast<CBreakableObject*>( usr_data_2->ph_ref_object ); if(
                    usr_data_1&&
    ...
    Fvector dir = Fvector().set( -c.geom.normal[ 0 ] * norm_sign,
                                 -c.geom.normal[ 1 ] * norm_sign,
                                 -c.geom.normal[ 2 ] * norm_sign );
    Fvector pos =
        Fvector().set( c.geom.pos[ 0 ], c.geom.pos[ 1 ], c.geom.pos[ 2 ] );
