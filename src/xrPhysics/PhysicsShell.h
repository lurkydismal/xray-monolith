...
// ABSTRACT:
class CPhysicsBase;
extern XRPHYSICS_API void get_box( const CPhysicsBase* shell,
                                   const Fmatrix& form,
                                   Fvector& sz,
                                   Fvector& c );
...
public:
    virtual void Activate( const Fmatrix& m0,
                           float dt01,
                           const Fmatrix& m2,
                           bool disable = false ) = 0;
    virtual void Activate( const Fmatrix& transform,
    ...
    virtual void SetAirResistance( float linear = default_k_l,
                                   float angular = default_k_w ) = 0;
    virtual void GetAirResistance( float& linear, float& angular ) = 0;
    virtual void set_DynamicLimits( float l_limit = default_l_limit,
                                    float w_limit = default_w_limit ) = 0;
    virtual void set_DynamicScales( float l_scale = default_l_scale,
                                    float w_scale = default_w_scale ) = 0;
    virtual void set_ContactCallback( ContactCallbackFun* callback ) = 0;
    virtual void set_ObjectContactCallback(
        ObjectContactCallbackFun* callback ) = 0;
    virtual void SetAnimated( bool v ) = 0;
    virtual void add_ObjectContactCallback(
        ObjectContactCallbackFun* callback ) = 0;
    virtual void remove_ObjectContactCallback(
        ObjectContactCallbackFun* callback ) = 0;
    ...
    //	virtual		void			get_LinearVel
    //(Fvector& velocity) const
    //= 0; 	virtual		void			get_AngularVel
    //(Fvector& velocity)	const
    //= 0;
    ...
    virtual void TransformPosition( const Fmatrix& form,
                                    motion_history_state history_state ) = 0;
    ...
    virtual void dbg_draw_geometry(
        float scale,
        u32 color,
        Flags32 flags = Flags32().assign( 0 ) ) const = 0;
#endif
    virtual ~CPhysicsBase() {};
};

// ABSTRACT:
// Element is fully Rigid and consists of one or more forms, such as sphere,
// box, cylinder, etc.
class XRPHYSICS_API CPhysicsElement : public CPhysicsBase,
...
public:
    ...
    virtual void add_Mass( const SBoneShape& shape,
    ...
    virtual void applyImpulseVsMC( const Fvector& pos,
                                   const Fvector& dir,
                                   float val ) = 0;
    virtual void applyImpulseVsGF( const Fvector& pos,
                                   const Fvector& dir,
                                   float val ) = 0;
    virtual void applyImpulseTrace( const Fvector& pos,
                                    const Fvector& dir,
                                    float val,
                                    const u16 id ) = 0;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
XRPHYSICS_API float NonElasticCollisionEnergy( CPhysicsElement* e1,
                                               CPhysicsElement* e2,
                                               const Fvector& norm );
...
public:
    ...
    virtual void SetAxisSDfactors( float spring_factor,
                                   float damping_factor,
                                   int axis_num ) = 0;
    virtual void SetJointSDfactors( float spring_factor,
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    virtual void SetAxisDir( const Fvector& orientation,
                             const int axis_num ) = 0;
    virtual void SetAxisDirVsFirstElement( const Fvector& orientation,
                                           const int axis_num ) = 0;
    virtual void SetAxisDirVsSecondElement( const Fvector& orientation,
                                            const int axis_num ) = 0;

    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    virtual void SetAnchor( const float x, const float y, const float z ) = 0;
    virtual void SetAnchorVsFirstElement( const float x,
                                          const float y,
                                          const float z ) = 0;
    virtual void SetAnchorVsSecondElement( const float x,
                                           const float y,
                                           const float z ) = 0;
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    virtual void SetAxisDir( const float x,
                             const float y,
                             const float z,
                             const int axis_num ) = 0;
    virtual void SetAxisDirVsFirstElement( const float x,
                                           const float y,
                                           const float z,
                                           const int axis_num ) = 0;
    virtual void SetAxisDirVsSecondElement( const float x,
                                            const float y,
                                            const float z,
                                            const int axis_num ) = 0;

    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    virtual void SetLimits( const float low,
                            const float high,
                            const int axis_num ) = 0;
    virtual void SetLimitsVsFirstElement( const float low,
                                          const float high,
                                          const int axis_num ) = 0;
    virtual void SetLimitsVsSecondElement( const float low,
                                           const float high,
                                           const int axis_num ) = 0;
    ...
    virtual bool isBreakable() = 0;
    virtual void SetForceAndVelocity( const float force,
                                      const float velocity = 0.f,
                                      const int axis_num = -1 ) = 0;
    virtual void GetMaxForceAndVelocity( float& force,
                                         float& velocity,
                                         int axis_num ) = 0;
    ...
    //()
    //=0;
    virtual void GetAxisSDfactors( float& spring_factor,
                                   float& damping_factor,
                                   int axis_num ) = 0;
    virtual void GetJointSDfactors( float& spring_factor,
                                    float& damping_factor ) = 0;
    virtual void GetLimits( float& lo_limit,
                            float& hi_limit,
                            int axis_num ) = 0;
...
public:
    ...
    virtual void applyImpulseTrace( const Fvector& pos,
                                    ...
                                    float val,
                                    const u16 id ) = 0;
    virtual void applyHit( const Fvector& pos,
                           const Fvector& dir,
                           float val,
                           const u16 id,
    ...
    virtual CPhysicsElement* NearestToPoint(
        const Fvector& point,
        NearestToPointCallback* cb = 0 ) = 0;
    virtual void build_FromKinematics( IKinematics* K,
                                       BONE_P_MAP* p_geting_map = NULL ) = 0;
    virtual void preBuild_FromKinematics( IKinematics* K,
                                          BONE_P_MAP* p_geting_map = NULL ) = 0;
    ...
    virtual bool AnimToVelocityState( float dt,
                                      float l_limit,
                                      float a_limit ) = 0;
    ...
    // build_FromKinematics		in returns elements  & joint pointers
    // according bone IDs; 	DECLARE_SCRIPT_REGISTER_FUNCTION
};

struct dContact;
struct SGameMtl;
XRPHYSICS_API void StaticEnvironmentCB( bool& do_colide,
                                        bool bo1,
                                        dContact& c,
                                        SGameMtl* material_1,
                                        SGameMtl* material_2 );
...
XRPHYSICS_API CPhysicsJoint* P_create_Joint( CPhysicsJoint::enumType type,
...
XRPHYSICS_API CPhysicsShell* P_build_Shell( IPhysicsShellHolder* obj,
                                            bool not_active_state,
                                            LPCSTR fixed_bones );
XRPHYSICS_API CPhysicsShell* P_build_Shell( IPhysicsShellHolder* obj,
                                            bool not_active_state,
                                            U16Vec& fixed_bones );
...
XRPHYSICS_API CPhysicsShell* P_build_SimpleShell( IPhysicsShellHolder* obj,
                                                  float mass,
                                                  bool not_active_state );
XRPHYSICS_API void ApplySpawnIniToPhysicShell( CInifile const* ini,
                                               CPhysicsShell* physics_shell,
                                               bool fixed );
void fix_bones( LPCSTR fixed_bones, CPhysicsShell* shell );

extern "C" XRPHYSICS_API void __stdcall destroy_physics_shell(
    CPhysicsShell*& p );

extern "C" XRPHYSICS_API bool __stdcall can_create_phys_shell(
    string1024& reason,
    IPhysicsShellHolder& O );
