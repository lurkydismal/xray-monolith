...
void CPHObject::activate() {
    R_ASSERT2( dSpacedGeom(),
               "trying to activate destroyed or not created object!" );
...
void CPHObject::Collide() {
    if ( m_flags.test( fl_ray_motions ) ) {
        ...
        for ( ; E != I; I++ ) {
            ...
            g_SpatialSpacePhysic->q_ray( ph_world->r_spatial, 0, STYPE_PHYSIC,
                                         *from, dir, magnitude );
            ...
            if ( debug_output().ph_dbg_draw_mask().test(
                ...
                debug_output().DBG_DrawLine(
                    *from,
                    Fvector().add( *from, Fvector().mul( dir, magnitude ) ),
            ...
            for ( ; i != e; ++i ) {
                ...
                dGeomRayMotionsSet( motion_ray, ( const dReal* )from,
                                    ( const dReal* )&dir, magnitude );
    ...
    if ( CPHCollideValidator::DoCollideStatic( *this ) &&
         !m_flags.test( fl_collision_disable ) )
...
void CPHObject::CollideDynamics() {
    g_SpatialSpacePhysic->q_box( ph_world->r_spatial, 0, STYPE_PHYSIC,
    ...
    for ( ; i != e; ++i ) {
        ...
        if ( obj2 == this || !obj2->m_flags.test( st_dirty ) ||
             m_flags.test( fl_collision_disable ) ||
             obj2->m_flags.test( fl_collision_disable ) )
            continue;
        ...
        if ( obj2->m_flags.test( is_deadbody ) ) {
            ...
            if ( g_dead_body_collision == 2 ||
                 ( g_dead_body_collision == 1 && achar &&
                   achar->CastActorCharacter() ) ) {
                if ( obj2->collide_class_bits().test(
                         CPHCollideValidator::cbNCClassCharacter ) )
                    obj2->collide_class_bits().set(
                        CPHCollideValidator::cbNCClassCharacter, FALSE );
            } else {
                if ( !obj2->collide_class_bits().test(
                         CPHCollideValidator::cbNCClassCharacter ) )
                    obj2->collide_class_bits().set(
                        CPHCollideValidator::cbNCClassCharacter, TRUE );
            }
        }
...
void CPHObject::step_prediction( float time ) {
    // general idea:
    // perform normal step by time as local as possible for this object then
    // return world to the pervious state
}
...
// it is still not a true step for object because it collide the object only not
