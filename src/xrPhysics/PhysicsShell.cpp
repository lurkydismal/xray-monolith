...
CPhysicsJoint* P_create_Joint( CPhysicsJoint::enumType type,
...
CPhysicsShell* __stdcall P_build_Shell( IPhysicsShellHolder* obj,
...
void fix_bones( LPCSTR fixed_bones, CPhysicsShell* shell ) {
    ...
    for ( int i = 0; i < count; ++i ) {
        ...
        R_ASSERT2(
            BI_NONE != fixed_bone_id,
            make_string( "wrong fixed bone [%s] for object with visual [%s]",
...
CPhysicsShell* P_build_Shell( IPhysicsShellHolder* obj,
                              bool not_active_state,
                              BONE_P_MAP* p_bone_map,
                              LPCSTR fixed_bones ) {
    ...
    if ( fixed_bones ) {
        ...
        for ( int i = 0; i < count; ++i ) {
            ...
            R_ASSERT2( BI_NONE != fixed_bone_id,
                       make_string(
                           "wrong fixed bone [%s] for object with visual [%s]",
                           fixed_bone, pKinematics->getDebugName().c_str() )
                           .c_str() );
...
CPhysicsShell* P_build_Shell( IPhysicsShellHolder* obj,
    ...
    if ( fixed_bones ) {
        // IKinematics* K		=
        // smart_cast<IKinematics*>(obj->ObjectVisual());
...
CPhysicsShell* P_build_SimpleShell( IPhysicsShellHolder* obj,
                                    float mass,
                                    bool not_active_state ) {
    CPhysicsShell* pPhysicsShell = P_create_Shell();
#ifdef DEBUG
    pPhysicsShell->dbg_obj = ( obj );
#endif
    // Fobb obb; obj->ObjectVisual()->getVisData().box.get_CD( obb.m_translate,
    // obb.m_halfsize );
    VERIFY( obj );
    ...
    if ( !obj->has_parent_object() )
        pPhysicsShell->Activate( obj->ObjectXFORM(), 0, obj->ObjectXFORM(),
                                 not_active_state );
    return pPhysicsShell;
}

void ApplySpawnIniToPhysicShell( CInifile const* ini,
...
#pragma todo( "not ignore static if non realy fixed! " )
        fix_bones( ini->r_string( "physics_common", "fixed_bones" ),
                   physics_shell );
    }
    if ( ini->section_exist( "collide" ) ) {
        if ( ( ini->line_exist( "collide", "ignore_static" ) && fixed ) ||
             ( ini->line_exist( "collide", "ignore_static" ) &&
        ...
        // If need, then show here that it is needed to ignore collisions with
        // "animated_object"
...
void get_box( const CPhysicsBase* shell,
              const Fmatrix& form,
              Fvector& sz,
              Fvector& c ) {
...
bool bone_has_pysics( IKinematics& K, u16 bone_id ) {
    ...
    return K.LL_GetBoneVisible( bone_id ) &&
           shape_is_physic( K.GetBoneData( bone_id ).get_shape() );
}
...
void phys_shell_verify_model( IKinematics& K ) {
    ...
    VERIFY2( has_physics_collision_shapes( K ),
             make_string( "Can not create physics shell for model %s because "
                          "it has no physics collision shapes set",
...
void phys_shell_verify_object_model( IPhysicsShellHolder& O ) {
    ...
    // VERIFY2( V, make_string( "Can not create physics shell for object %s it
    // has no model", O.ObjectName() )/*+ make_string("\n object dump: \n") +
    ...
    VERIFY2( K, make_string( "Can not create physics shell for object %s, "
                             "model %s is not skeleton",
    ...
    VERIFY2(_valid( O.ObjectXFORM() ),
	        ...
	        /*+ make_string("\n object dump: \n") + dbg_object_full_dump_string( &O )*/);
#ifdef DEBUG
    VERIFY2( valid_pos( O.ObjectXFORM().c ),
             dbg_valide_pos_string( O.ObjectXFORM().c, &O,
                                    "create physics shell" ) );
#endif
}

bool __stdcall can_create_phys_shell( string1024& reason,
                                      IPhysicsShellHolder& O ) {
    xr_strcpy( reason, "ok" );
    bool result = true;
    IKinematics* K = O.ObjectKinematics();
    if ( !K ) {
        xr_strcpy( reason, make_string( "Can not create physics shell for "
                                        "object %s, model %s is not skeleton",
                                        O.ObjectName(), O.ObjectNameVisual() )
                               .c_str() );
        return false;
    }
    if ( !has_physics_collision_shapes( *K ) ) {
        xr_strcpy( reason,
                   make_string( "Can not create physics shell for object %s, "
                                "model %s has no physics collision shapes set",
                                O.ObjectName(), O.ObjectNameVisual() )
                       .c_str() );
        return false;
    }
...
#ifdef DEBUG
        xr_strcpy( reason, dbg_valide_pos_string( O.ObjectXFORM().c, &O,
                                                  "create physics shell" )
                               .c_str() );
#else
        xr_strcpy(
            reason,
            make_string( "create physics shell: object position is not valid" )
                .c_str() );
#endif
        return false;
    }
...
float NonElasticCollisionEnergy( CPhysicsElement* e1,
                                 CPhysicsElement* e2,
                                 const Fvector& norm ) // norm - from 2 to 1
{
    VERIFY( e1 );
    VERIFY( e2 );
...
void StaticEnvironmentCB( bool& do_colide,
                          bool bo1,
                          dContact& c,
                          SGameMtl* material_1,
                          SGameMtl* material_2 ) {
    ...
    if ( bo1 ) {
        ( ( CPHIsland* )( retrieveGeomUserData( c.geom.g1 )->callback_data ) )
            ->DActiveIsland()
            ->ConnectJoint( contact_joint );
        dJointAttach( contact_joint, dGeomGetBody( c.geom.g1 ), 0 );
    } else {
        ( ( CPHIsland* )( retrieveGeomUserData( c.geom.g2 )->callback_data ) )
            ->DActiveIsland()
            ->ConnectJoint( contact_joint );
