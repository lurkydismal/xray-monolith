...
protected:
    ...
    ObjectContactCallbackFun*
        object_contact_callback; //->to shell ??		//st
public:
    ///
...
public:
    void set_ContactCallback( ContactCallbackFun* callback ); // aux (may not
                                                              // be)
    void set_ObjectContactCallback(
        ObjectContactCallbackFun*
            callback ); // called anywhere ph state influent
    void add_ObjectContactCallback(
        ObjectContactCallbackFun*
            callback ); // called anywhere ph state influent
    void remove_ObjectContactCallback(
        ObjectContactCallbackFun*
            callback ); // called anywhere ph state influent
...
void t_get_extensions( const xr_vector< geometry_type* >& geoms,
                       const Fvector& axis,
                       float center_prg,
                       float& lo_ext,
                       float& hi_ext ) {
    lo_ext = dInfinity;
    hi_ext = -dInfinity;
    typename xr_vector< geometry_type* >::const_iterator i = geoms.begin(),
                                                         e = geoms.end();
    for ( ; i != e; ++i ) {
