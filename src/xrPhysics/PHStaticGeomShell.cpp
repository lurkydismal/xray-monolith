...
void CPHStaticGeomShell::get_spatial_params() {
    Fvector AABB;
    spatialParsFromDGeom( dSpacedGeometry(), SpatialComponent->spatial.sphere.P,
                          AABB, SpatialComponent->spatial.sphere.R );
}
...
void P_BuildStaticGeomShell( CPHStaticGeomShell* pUnbrokenObject,
...
IPHStaticGeomShell* P_BuildStaticGeomShell(
    IPhysicsShellHolder* obj,
    ObjectContactCallbackFun* object_contact_callback ) {
    Fobb b;
    ...
    CPHStaticGeomShell* pUnbrokenObject =
        P_BuildStaticGeomShell( obj, object_contact_callback, b );

    // IKinematics* K=smart_cast<IKinematics*>(V); VERIFY(K);
    K->CalculateBones( TRUE );
    for ( u16 k = 0; k < K->LL_BoneCount(); k++ ) {
        K->LL_GetBoneInstance( k ).set_callback(
            bctPhysics, cb, K->LL_GetBoneInstance( k ).callback_param(), TRUE );
...
void DestroyStaticGeomShell( IPHStaticGeomShell*& UnbrokenObject ) {
    if ( !UnbrokenObject )
        return;
    CPHStaticGeomShell* gs =
        static_cast< CPHStaticGeomShell* >( UnbrokenObject );
    gs->Deactivate();
    xr_delete( gs );
    UnbrokenObject = 0;
}
...
IPHStaticGeomShell* P_BuildLeaderGeomShell( IClimableObject* obj,
                                            ObjectContactCallbackFun* callback,
                                            const Fobb& b ) {
    CPHLeaderGeomShell* pStaticShell = xr_new< CPHLeaderGeomShell >( obj );
    P_BuildStaticGeomShell( smart_cast< CPHStaticGeomShell* >( pStaticShell ),
                            smart_cast< IPhysicsShellHolder* >( obj ), 0, b );
