...
//); 				void DBG_ObjBeforePhDataUpdate	( CPHObject *obj ); 				void
//DBG_ObjAfterStep			( CPHObject *obj ); 				void
//DBG_ObjBeforeStep			( CPHObject *obj ); 				void
//DBG_ObjeAfterPhTune		( CPHObject *obj ); 				void DBG_ObjBeforePhTune
//( CPHObject *obj ); 				void DBG_ObjAfterCollision		( CPHObject *obj
...
void __stdcall create_physics_world( bool mt,
                                     CObjectSpace* os,
                                     CObjectList* lo,
...
CObjectSpace* __stdcall create_object_space() {
    // CFileReader* fr =
    // xr_new<CFileReader>("D:/STALKER/resources/gamedata/levels/stohe_selo/level.cform");
...
void CPHWorld::SetStep( float s ) {
    fixed_step = s;
    world_cfm = CFM( SPRING_S( base_cfm, base_erp, base_fixed_step ),
                     DAMPING( base_cfm, base_erp ) );
    world_erp = ERP( SPRING_S( base_cfm, base_erp, base_fixed_step ),
                     DAMPING( base_cfm, base_erp ) );
...
void CPHWorld::Create( bool mt,
    ...
    m_level_objects = lo;
    m_device = dv;
    // if (psDeviceFlags.test(mtPhysics))	Device.seqFrameMT.Add
    // (this,REG_PRIORITY_HIGH); else
    ...
    // m_commander
    // =xr_new<CPHCommander>(); dVector3 extensions={2048,256,2048};
    /*
    Fbox	level_box		=
    Level().ObjectSpace.GetBoundingVolume(); Fvector level_size,level_center;
    level_box				.	getsize		(level_size);
    level_box				.	getcenter	(level_center);
    dVector3 extensions		=	{ level_size.x ,256.f,level_size.z};
    dVector3 center			=
    {level_center.x,0.f,level_center.z};
    */
...
void CPHWorld::OnFrame() {
    ...
    // Msg
    // ("------------- physics: %d / %d",u32(Device.dwFrame),u32(m_steps_num));
...
void CPHWorld::Step() {
    ...
    if ( disable_count == 0 ) {
        ...
        for ( i_object = m_recently_disabled_objects.begin();
              m_recently_disabled_objects.end() != i_object; ) {
    ...
    for ( i_update_object = m_update_objects.begin();
          m_update_objects.end() != i_update_object; ) {
...
#endif
    dJointGroupEmpty(
        ContactGroup ); // this is to be called after PhDataUpdate!!!-the order
                        // is critical!!!
...
u32 CPHWorld::CalcNumSteps( u32 dTime ) {
    if ( dTime < m_frame_time * 1000 )
        return 0;
    u32 res = iCeil( ( float( dTime ) - m_frame_time * 1000 ) /
                     ( fixed_step * 1000 ) );
...
void CPHWorld::FrameStep( dReal step ) {
    ...
    if ( debug_output().ph_dbg_draw_mask().test( phDbgDrawObjectStatistics ) ) {
        ...
        b_processing = true;
        debug_output().DBG_OutText( "phys steps per frame %2.1f",
                                    dbg_iterations );
        b_processing = false;
    }
#endif
