...
void CPHShell::applyHit( const Fvector& pos,
                         const Fvector& dir,
                         float val,
                         const u16 id,
...
void CPHShell::ExplosionHit( const Fvector& pos,
                             const Fvector& dir,
                             float val,
                             const u16 id ) {
    if ( !isActive() )
    ...
    for ( ; i != e; i++ ) {
        ...
        for ( u16 j = 0; j < gn; ++j ) {
            ...
            element->applyImpulseTrace(
                r_pos, r_dir, g_impulse,
                element->CPHGeometryOwner::Geom( j )->bone_id() );
        }
    }
}
