...
struct TPHCharacterRestrictor : public SPHCharacterRestrictor {
    ...
    void Create( CPHCharacter* ch, dVector3 sizes ) {
        dGeomUserDataSetObjectContactCallback( m_restrictor,
                                               RestrictorCallBack );
    }

    static void RestrictorCallBack( bool& do_colide,
                                    bool bo1,
                                    dContact& c,
                                    SGameMtl* material_1,
                                    SGameMtl* material_2 ) {
        ...
        if ( !( o1 && o2 ) )
            return;
        if ( o1->CastType() != CPHObject::tpCharacter ||
             o2->CastType() != CPHObject::tpCharacter )
            return;
