...
void CPHIsland::Step( dReal step ) {
    if ( !m_flags.is_active() )
        return;
    // dWorldStepFast1	(DWorld(),	fixed_step,
    // phIterations/*+Random.randI(0,phIterationCycle)*/);
    if ( ( m_flags.is_exact_integration_forced() &&
           nj < max_joint_allowed_for_forced_exact_integration ) ||
         ( m_flags.is_exact_integration_prefeared() &&
           nj < max_joint_allowed_for_exeact_integration ) )
...
void CPHIsland::Enable() {
    ...
    for ( dxBody* body = DWorld()->firstbody; body;
          body = ( dxBody* )body->next )
