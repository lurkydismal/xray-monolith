...
public:
    ...
    // force overrides the joint budget that gates the prefere flag (capped, see
    // CPHIsland::Step)
    ...
    IC BOOL is_exact_integration_forced() {
        return flags.test( flForceExactIntegration << shift_to_variable );
    }
    ...
    IC void unmerge() {
        flags.flags = ( ( flags.flags & mask_static ) << shift_to_variable ) |
                      ( flags.flags & mask_static );
    }
};
...
public:
    ...
    IC void Merge( CPHIsland* island ) {
        ...
        *( second_island->m_bodies_tail ) = first_island->firstbody;
        ...
        VERIFY(
            !( ( !( first_island->nj ) ) && ( first_island->firstjoint ) ) );
    ...
    IC void AddBody( dxBody* body ) {
        VERIFY2( m_nj == nj && m_nb == nb && m_flags.is_active(),
                 "can not remove/add during processing phase" );
