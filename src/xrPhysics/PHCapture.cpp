...
///////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
IPHCapture* phcapture_create( CPHCharacter* ch,
...
void CPHCapture::PullingUpdate() {
    if ( !m_taget_element->isActive() ||
         inl_ph_world().Device().dwTimeGlobal - m_time_start >
             m_capture_time ) {
        Release();
        return;
    }
    ...
    m_character->PhysicsRefObject()->ObjectXFORM().transform_tiny(
        capture_bone_position );
    ...
    if ( dist < m_capture_distance ) {
        ...
        CreateBody();
        dBodySetPosition( m_body, capture_bone_position.x,
                          capture_bone_position.y, capture_bone_position.z );
        ...
        dJointSetBallAnchor( m_joint, capture_bone_position.x,
                             capture_bone_position.y, capture_bone_position.z );
        ...
        if ( dir.x > EPS ) {
            if ( dir.y > EPS ) {
                ...
                dJointSetAMotorAxis( m_ajoint, 2, 2, -dir.y / mag, dir.x / mag,
                                     0.f );
            } else if ( dir.z > EPS ) {
                float mag = dir.x * dir.x + dir.z * dir.z;
                dJointSetAMotorAxis( m_ajoint, 2, 2, -dir.z / mag, 0.f,
                                     dir.x / mag );
            } else {
            ...
            if ( dir.y > EPS ) {
                if ( dir.z > EPS ) {
                    ...
                    dJointSetAMotorAxis( m_ajoint, 2, 2, 0.f, -dir.z / mag,
                                         dir.y / mag );
                } else {
...
void CPHCapture::CapturedUpdate() {
    ...
    if ( !m_taget_element->isActive() ||
         dDOT( m_joint_feedback.f2, m_joint_feedback.f2 ) >
    ...
    // m_back_force=m_back_force*0.999f+ ((mag<m_capture_force/5.f) ? mag :
    // (m_capture_force/5.f))*0.001f;
    //
    if ( b_character_feedback && mag > m_capture_force / 2.2f ) {
        float f = mag / ( m_capture_force / 15.f );
        m_character->ApplyForce( m_joint_feedback.f1[ 0 ] / f,
                                 m_joint_feedback.f1[ 1 ] / f,
                                 m_joint_feedback.f1[ 2 ] / f );
    }
    ...
    m_character->PhysicsRefObject()->ObjectXFORM().transform_tiny(
        capture_bone_position );
    dBodySetPosition( m_body, capture_bone_position.x, capture_bone_position.y,
                      capture_bone_position.z );
}
...
void CPHCapture::object_contactCallbackFun( bool& do_colide,
                                            bool bo1,
                                            dContact& c,
                                            SGameMtl* /*material_1*/,
                                            SGameMtl* /*material_2*/ ) {
    ...
    if ( capturer ) {
        ...
        if ( capture ) {
            if ( capture->m_taget_element->PhysicsRefObject() ==
                 l_pUD2->ph_ref_object ) {
                do_colide = false;
    ...
    if ( capturer ) {
        CPHCapture* capture =
            static_cast< CPHCapture* >( capturer->PHCapture() );
        if ( capture ) {
            if ( capture->m_taget_element->PhysicsRefObject() ==
                 l_pUD1->ph_ref_object ) {
                do_colide = false;
                capture->m_taget_element->Enable();
                if ( capture->e_state == CPHCapture::cstReleased )
                    capture->ReleaseInCallBack();
