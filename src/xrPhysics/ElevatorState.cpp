...
void CElevatorState::PhTune( float step ) {
    ...
    if ( ( !g_actor_allow_ladder &&
           m_character->RestrictionType() == rtActor ) ||
         m_character->RestrictionType() == rtMonsterMedium ) {
...
#ifdef DEBUG
const char* dbg_state[] = { "clbNone",       "clbNearUp",       "clbNearDown",
                            "clbClimbingUp", "clbClimbingDown", "clbDepart",
                            "clbNoLadder" };
#endif
...
void CElevatorState::UpdateStNone() {
    ...
    if ( m_ladder->BeforeLadder( m_character ) &&
        ...
        Fvector temp;
        float d_to_lower = m_ladder->DDLowerP( m_character, temp ),
              d_to_upper = m_ladder->DDUpperP( m_character, temp );
...
void CElevatorState::UpdateStNearUp() {
    ...
    if ( m_ladder->InTouch( m_character ) &&
         ...
         // ClimbDirection()<0.f&&
         m_ladder->DDToPlain( m_character, d ) >
             m_character->FootRadius() / 3.f &&
...
void CElevatorState::UpdateStNearDown() {
    ...
    if ( m_ladder->InTouch( m_character ) &&
         dXZDotNormalized( d, m_character->CamDir() ) > look_angle_cosine &&
         d.dotproduct( m_character->ControlAccel() ) > 0.f &&
         ClimbDirection() > 0.f && m_ladder->BeforeLadder( m_character ) )
        SwitchState( clbClimbingUp );
    if ( dist - m_character->FootRadius() > out_dist )
        SwitchState( ( clbNoLadder ) );
}
...
void CElevatorState::UpdateStClimbingDown() {
    ...
    if ( !fis_zero( to_ax ) && !fis_zero( control_a ) &&
         abs( -ca.dotproduct( Fvector( m_ladder->Norm() ).normalize() ) ) <
             M_SQRT1_2 )
        SwitchState( clbDepart );
    if ( m_ladder->AxDistToLowerP( m_character ) - m_character->FootRadius() <
         stop_climbing_dist )
        SwitchState( clbNearDown );
    ...
    if ( vel.y > EPS_S ) {
        m_character->ApplyForce(
            0.f, -m_character->Mass() * ph_world->Gravity(), 0.f );
    }
...
void CElevatorState::UpdateClimbingCommon( const Fvector& d_to_ax,
                                           float to_ax,
                                           const Fvector& control_accel,
                                           float ca ) {
    VERIFY( m_ladder && m_character );
    if ( to_ax - m_character->FootRadius() > out_dist )
        SwitchState( ( clbNoLadder ) );
...
#endif
        m_character->ApplyForce( d_to_ax,
                                 m_character->Mass() * ph_world->Gravity() ); //
    }
}
...
bool CElevatorState::GetControlDir( Fvector& dir ) {
    ...
    switch ( m_state ) {
        ...
        case clbNearUp:
            ...
            if ( dXZDotNormalized( d, m_character->CamDir() ) >
                     look_angle_cosine &&
                 !fis_zero( dist, EPS_L ) &&
...
CElevatorState::SEnertionState
    CElevatorState::m_etable[ clbNoState ][ clbNoState ] = {
        //						clbNone
        //clbNearUp		clbNearDown		clbClimbingUp
        ...
        { { 0, 0 },
          ...
          { 0, 0 },
          { 0, 0 },
          { depart_dist, depart_time } }, // clbDepart
        /*clbNoLadder		*/
        { { 0, 0 },
          { 0, 0 },
          { 0, 0 },
...
bool CElevatorState::StateSwitchInertion( Estate new_state ) {
    ...
    if ( m_etable[ m_state ][ new_state ].dist < p.magnitude() ||
         m_etable[ m_state ][ new_state ].time <
             inl_ph_world().Device().dwTimeGlobal - m_start_time )
        return true;
    else
        return false;
}
