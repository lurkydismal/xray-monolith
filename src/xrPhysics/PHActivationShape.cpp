...
#include "stdafx.h"
// #include "Level.h"
#include "../3rd_party/ode/ode/src/util.h"
#include "PHWorld.h"
...
static	const float dynamic_erp				= 1.f /
1000.f;//static_erp;//
*/
...
static void ActivateTestDepthCallback( bool& do_colide,
                                       bool bo1,
                                       dContact& c,
                                       SGameMtl* material_1,
                                       SGameMtl* material_2 ) {
    if ( !do_colide || material_1->Flags.test( SGameMtl::flPassable ) ||
    ...
    /*
       ...
       (CPHActivationShape*)(PHRetrieveGeomUserData( bo1 ? c.geom.g1 : c.geom.g2
       )->callback_data) ) ); VERIFY( self );

            if( cl_statics )
            {
                    c.surface.soft_cfm=static_cfm;
                    c.surface.soft_erp=static_erp;
                    dJointID contact_joint	= dJointCreateContactSpecial( 0,
       ContactGroup, &c ); self->DActiveIsland()->ConnectJoint(contact_joint);
                    dJointAttach(contact_joint, dGeomGetBody(c.geom.g1),
       dGeomGetBody(c.geom.g2)); do_colide = false; return;
            }

            c.surface.soft_cfm=dynamic_cfm;
            c.surface.soft_erp=dynamic_erp;

            dxGeomUserData* data_oposite =  retrieveGeomUserData( bo1 ?
       c.geom.g2 : c.geom.g1 );
            ...
            int max_contacts;
            if(
       !obj1->DActiveIsland()->CanMerge(obj2->DActiveIsland(),max_contacts ) )
                    return;
            if( max_contacts < 1 )
                    return;

            dJointID contact_joint	= dJointCreateContactSpecial( 0,
       ContactGroup, &c ); obj1->DActiveIsland()->ConnectJoint(contact_joint);
            dJointAttach			(contact_joint,
...
void StaticEnvironment( bool& do_colide,
                        bool bo1,
                        dContact& c,
                        SGameMtl* material_1,
                        SGameMtl* material_2 ) {
    ...
    if ( bo1 ) {
        ( ( CPHActivationShape* )( retrieveGeomUserData( c.geom.g1 )
                                       ->callback_data ) )
            ->DActiveIsland()
            ->ConnectJoint( contact_joint );
        dJointAttach( contact_joint, dGeomGetBody( c.geom.g1 ), 0 );
    } else {
        ( ( CPHActivationShape* )( retrieveGeomUserData( c.geom.g2 )
                                       ->callback_data ) )
            ->DActiveIsland()
            ->ConnectJoint( contact_joint );
...
void GetMaxDepthCallback( bool& do_colide,
                          bool bo1,
                          dContact& c,
                          SGameMtl* material_1,
                          SGameMtl* material_2 ) {
...
void CPHActivationShape::Create( const Fvector start_pos,
...
bool CPHActivationShape::Activate( const Fvector need_size,
                                   u16 steps,
    ...
    float limit_l_vel = _max( _max( need_size.x, need_size.y ), need_size.z ) /
                        fnum_it * fnum_steps_r / fixed_step;
    ...
    CHECK_POS( Position(), "pos after RestoreVelocityState(temp_state);",
               true );
    if ( !un_freeze_later )
        ph_world->UnFreeze();
#ifdef DEBUG
...
void CPHActivationShape::get_spatial_params() {
    spatialParsFromDGeom( m_geom, SpatialComponent->spatial.sphere.P, AABB,
                          SpatialComponent->spatial.sphere.R );
}

void CPHActivationShape::InitContact( dContact* c,
                                      bool& do_collide,
                                      u16,
                                      u16 ) {}
