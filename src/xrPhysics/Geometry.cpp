...
static void computeFinalTx( dGeomID geom_transform,
                            dReal* final_pos,
                            dReal* final_R ) {
    R_ASSERT2( dGeomGetClass( geom_transform ) == dGeomTransformClass,
               "is not a geom transform" );
...
void GetBoxExtensions( dGeomID box,
                       const dReal* axis,
                       const dReal* pos,
                       const dReal* rot,
                       float center_prg,
...
void GetCylinderExtensions( dGeomID cyl,
                            const dReal* axis,
                            const dReal* pos,
                            const dReal* rot,
                            float center_prg,
                            dReal* lo_ext,
                            dReal* hi_ext ) {
    R_ASSERT2( dGeomGetClass( cyl ) == dCylinderClassUser,
               "is not a cylinder" );
    dReal radius, length;
...
void GetSphereExtensions( dGeomID sphere,
                          const dReal* axis,
                          const dReal* pos,
                          float center_prg,
                          dReal* lo_ext,
...
void TransformedGeometryExtensionLocalParams( dGeomID geom_transform,
                                              const dReal* axis,
                                              float center_prg,
                                              dReal* local_axis,
...
void CODEGeom::add_self_mass( dMass& m,
...
void CODEGeom::get_local_form_bt( Fmatrix& form ) {
    PHDynamicData::DMXPStoFMX( dGeomGetRotation( geom() ),
                               dGeomGetPosition( geom() ), form );
}

void CODEGeom::get_global_center_bt( Fvector& center ) {
    center.set( *( ( const Fvector* )dGeomGetPosition( m_geom_transform ) ) );
    dVector3 add;
    dMULTIPLY0_331( add, dGeomGetRotation( m_geom_transform ),
...
{
        dMULTIPLY0_331
((dReal*)(&form.c),dGeomGetRotation(m_geom_transform),dGeomGetPosition(geom()));
        form.c.add(*((const Fvector*)dGeomGetPosition(m_geom_transform)));
        dMULTIPLY3_333
((dReal*)(&form),dGeomGetRotation(m_geom_transform),dGeomGetRotation(geom()));
...
void CODEGeom::clear_motion_history( bool set_unspecified ) {
    ...
    get_xform( m );
    if ( Fvector()
             .sub( m.c, cast_fv( dGeomGetUserData( geom() )->last_pos ) )
             .magnitude() > EPS )
        Msg( "! WRONG THING" );
#endif
}
...
void CODEGeom::get_final_tx_bt( const dReal*& p,
                                const dReal*& R,
                                dReal* bufV,
                                dReal* bufM ) const {
    VERIFY( m_geom_transform );
...
void CODEGeom::get_final_tx( dGeomID g,
                             const dReal*& p,
                             const dReal*& R,
                             dReal* bufV,
                             dReal* bufM ) {
...
void CBoxGeom::get_Extensions( const Fvector& axis,
                               float center_prg,
                               float& lo_ext,
                               float& hi_ext ) const {
    VERIFY( m_geom_transform );
    ...
    dGeomID g = geometry_bt();
    get_final_tx_bt( pos, rot, p, r );
    GetBoxExtensions( g, cast_fp( axis ), pos, rot, center_prg, &lo_ext,
                      &hi_ext );
}
...
void CBoxGeom::get_max_area_dir_bt( Fvector& dir ) {
    ...
    dReal S1 = length[ 0 ] * length[ 1 ], S2 = length[ 0 ] * length[ 2 ],
          S3 = length[ 1 ] * length[ 2 ];
...
dGeomID CBoxGeom::create() {
    return dCreateBox( 0, m_box.m_halfsize.x * 2.f, m_box.m_halfsize.y * 2.f,
...
void CBoxGeom::set_size( const Fvector& half_size ) {
    ...
    VERIFY( geom() );
    dGeomBoxSetLengths( geom(), m_box.m_halfsize.x * 2.f,
                        m_box.m_halfsize.y * 2.f, m_box.m_halfsize.z * 2.f );
}
...
void CBoxGeom::set_build_position( const Fvector& ref_point ) {
    ...
    dGeomSetPosition( geom(), local_position[ 0 ], local_position[ 1 ],
                      local_position[ 2 ] );
    dMatrix3 R;
...
void CSphereGeom::get_Extensions( const Fvector& axis,
                                  float center_prg,
                                  float& lo_ext,
                                  float& hi_ext ) const {
    VERIFY( m_geom_transform );
    ...
    dGeomID g = geometry_bt();
    get_final_tx_bt( pos, rot, p, r );
    GetSphereExtensions( g, cast_fp( axis ), pos, center_prg, &lo_ext,
                         &hi_ext );
}
...
void CSphereGeom::set_build_position( const Fvector& ref_point ) {
    inherited::set_build_position( ref_point );
    dVector3 local_position = { m_sphere.P.x - ref_point.x,
...
float CCylinderGeom::volume() {
    return M_PI * m_cylinder.m_radius * m_cylinder.m_radius *
           m_cylinder.m_height;
}
...
void CCylinderGeom::get_Extensions( const Fvector& axis,
                                    float center_prg,
                                    float& lo_ext,
                                    float& hi_ext ) const {
    VERIFY( m_geom_transform );
    ...
    dGeomID g = geometry_bt();
    get_final_tx_bt( pos, rot, p, r );
    GetCylinderExtensions( g, cast_fp( axis ), pos, rot, center_prg, &lo_ext,
                           &hi_ext );
}
...
void CCylinderGeom::set_build_position( const Fvector& ref_point ) {
    inherited::set_build_position( ref_point );
    dVector3 local_position = { m_cylinder.m_center.x - ref_point.x,
