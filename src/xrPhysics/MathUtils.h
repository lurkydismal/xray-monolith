...
IC void dVectorDeviation( const float* vector3_from,
                          const float* vector3_to,
                          float* vector_dev ) {
    dVectorSub( vector_dev, vector3_to, vector3_from );
}

IC void dVectorDeviationAdd( const float* vector3_from,
                             const float* vector3_to,
                             float* vector_dev ) {
...
IC void dMatrixSmallDeviation( const float* matrix33_from,
                               const float* matrix33_to,
                               float* vector_dev ) {
...
IC void dMatrixSmallDeviationAdd( const float* matrix33_from,
                                  const float* matrix33_to,
                                  float* vector_dev ) {
    vector_dev[ 0 ] += matrix33_from[ 10 ] - matrix33_to[ 10 ];
    vector_dev[ 1 ] += matrix33_from[ 2 ] - matrix33_to[ 2 ];
...
IC void twoq_2w( const Fquaternion& q1,
                 const Fquaternion& q2,
                 float dt,
                 Fvector& w ) {
    //
...
IC void prg_pos_on_axis( const Fvector& in_ax_p,
                         const Fvector& in_ax_d,
                         Fvector& in_out_pos ) {
...
IC float prg_pos_on_plane( const Fvector& in_norm,
                           float d,
...
IC void prg_on_normal( const Fvector& in_norm,
...
IC void TransferenceToThrowVel( Fvector& in_transference_out_vel,
                                float time,
                                float gravity_accel ) {
...
IC u8 TransferenceAndThrowVelToTgA( const Fvector& transference,
                                    float throw_vel,
                                    float gravity_accel,
                                    Fvector2& tgA,
                                    float& s ) {
    float sqx =
        transference.x * transference.x + transference.z * transference.z;
    float sqv = throw_vel * throw_vel;
    float sqD4 = 1.f - gravity_accel / ( sqv * sqv ) *
                           ( 2.f * transference.y * sqv + gravity_accel * sqx );
    if ( sqD4 < 0.f )
        return 0;
    s = _sqrt( sqx );
...
IC u8 TransferenceAndThrowVelToTgA( const Fvector& transference,
                                    ...
                                    Fvector2& tgA ) {
    float s;
    return TransferenceAndThrowVelToTgA( transference, throw_vel, gravity_accel,
                                         tgA, s );
}

IC u8 TransferenceAndThrowVelToThrowDir( const Fvector& transference,
                                         float throw_vel,
                                         float gravity_accel,
                                         Fvector throw_dir[ 2 ] ) {
    ...
    Fvector2 tgA;
    float s;
    u8 ret = TransferenceAndThrowVelToTgA( transference, throw_vel,
                                           gravity_accel, tgA, s );
    switch ( ret ) {
        case 0:
            return 0;
...
#define SORT( x, on_x1, on_x2, on_x3, y, on_y1, on_y2, on_y3, z, on_z1, on_z2, \
              on_z3 )                                                          \
    if ( x < y ) {                                                             \
        if ( x < z ) {                                                         \
            if ( y < z ) {                                                     \
