...
public:
    ...
    // (const Fvector &pos); virtual		void
    // SetDesiredPosition					(const Fvector&
    // pos)
    // {m_vDesiredPosition.set(pos)	;} virtual		void
    // GetDesiredPosition					(Fvector& dpos)
    // {dpos.set(m_vDesiredPosition)	;}
    virtual void ValidateWalkOn();
    //	virtual		void		BringToDesired
    //(float time,float velocity,float force=1.f)
    //;
