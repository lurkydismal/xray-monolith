...
template < class Pars >
void TTestDepthCallback( bool& do_colide,
                         bool bo1,
                         dContact& c,
                         SGameMtl* material_1,
                         SGameMtl* material_2 ) {
    ...
    if ( do_colide && !material_1->Flags.test( SGameMtl::flPassable ) &&
        ...
        if ( test_depth > Pars::depth_to_use_force ) {
            ...
            if ( b1 )
                dBodyAddForce( b1, c.geom.normal[ 0 ] * force,
                               c.geom.normal[ 1 ] * force,
                               c.geom.normal[ 2 ] * force );
            if ( b2 )
                dBodyAddForce( b2, -c.geom.normal[ 0 ] * force,
                               -c.geom.normal[ 1 ] * force,
                               -c.geom.normal[ 2 ] * force );
...
ObjectContactCallbackFun* TestDepthCallback =
    &TTestDepthCallback< STestCallbackPars >;
ObjectContactCallbackFun* TestFootDepthCallback =
    &TTestDepthCallback< STestFootCallbackPars >;
...
public:
    ...
    bool VelocityLimit() {
        ...
        if ( dV_valid( linear_velocity ) ) {
            ...
            dBodySetLinearVel( m_body, vlinear_velocity.x, vlinear_velocity.y,
                               vlinear_velocity.z );
            return ret;
        } else {
            dBodySetLinearVel( m_body, m_safe_velocity[ 0 ],
                               m_safe_velocity[ 1 ], m_safe_velocity[ 2 ] );
            return true;
        }
    }
    ...
    virtual void PhDataUpdate( dReal step ) {
        ...
        for ( int i = 0; i < num; i++ ) {
            ...
            if ( dJointGetType( joint ) == dJointTypeContact ) {
                ...
                save_max( m_max_force_self,
                          _sqrt( dDOT( self_force, self_force ) ) );
                save_max( m_max_torque_self,
                          _sqrt( dDOT( self_torque, self_torque ) ) );
                save_max( m_max_force_self_y, _abs( self_force[ 1 ] ) );
                save_max( m_max_force_self_sd,
                          _sqrt( self_force[ 0 ] * self_force[ 0 ] +
                                 self_force[ 2 ] * self_force[ 2 ] ) );
                if ( other_body ) {
                    dVector3 shoulder;
                    ...
                    save_max( m_max_force_others,
                              _sqrt( dDOT( othrers_force, othrers_force ) ) );
                    if ( !fis_zero( shoulder_lenght ) )
                        save_max(
                            m_max_torque_others,
                            _sqrt( dDOT( othrers_torque, othrers_torque ) ) /
                                shoulder_lenght );
                }
            }
        }
...
/////////////////////////////////////////////////////////////////////////////////////
bool ActivateBoxDynamic( IPHMovementControl* mov_control,
    ...
    // void		CPHMovementControl::		SetFootCallBack
    // (ObjectContactCallbackFun* callback)
    //{
    //	VERIFY(m_character);
    ...
    float pass = character_exist
                     ? _abs( mov_control->Box().getradius() -
    ...
    mov_control->actor_calculate( Fvector().set( 0, 0, 0 ),
                                  Fvector().set( 1, 0, 0 ), 0, 0, 0, 0 );

    CVelocityLimiter vl( mov_control->character()->get_body(), max_vel,
                         max_vel );
    ...
    for ( int m = 0; 30 > m; ++m ) {
        ...
        mov_control->character()->ApplyForce(
            0, ph_world->Gravity() * mov_control->character()->Mass(), 0 );
        max_depth = 0.f;
        ph_world->Step();
    ...
    mov_control->character()->SetStaticContactCallBack(
        ph_world->default_character_contact_shotmark() );
    vl.Deactivate();
