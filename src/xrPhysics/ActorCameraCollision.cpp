...
static void cammera_shell_collide_callback_common( bool& do_collide,
                                                   bool bo1,
                                                   dContact& c,
                                                   SGameMtl* material_1,
                                                   SGameMtl* material_2 ) {
    ...
    dxGeomUserData* my_data =
        retrieveGeomUserData( bo1 ? c.geom.g1 : c.geom.g2 );
    dxGeomUserData* oposite_data =
        retrieveGeomUserData( bo1 ? c.geom.g2 : c.geom.g1 );
...
static void cammera_shell_collide_callback( bool& do_collide,
                                            bool bo1,
                                            dContact& c,
                                            SGameMtl* material_1,
                                            SGameMtl* material_2 ) {
    ...
    if ( oposite_data && oposite_data->ph_object &&
         oposite_data->ph_object->CastType() == CPHObject::tpCharacter )
        c.surface.soft_cfm = soft_cfm_for_controllers;

    cammera_shell_collide_callback_common( do_collide, bo1, c, material_1,
                                           material_2 );
}

static void cammera_shell_character_collide_callback( bool& do_collide,
                                                      bool bo1,
                                                      dContact& c,
                                                      SGameMtl* material_1,
                                                      SGameMtl* material_2 ) {
    ...
    if ( !oposite_data->ph_ref_object ||
         !( oposite_data->ph_ref_object->IsStalker() ) )
        return;
...
static void get_viewport_geom( Fvector& box,
                               Fmatrix& form,
                               const CCameraBase& camera,
                               float _viewport_near ) {
    box.z = _viewport_near / 2.f;
    tviewport_size( inl_ph_world().Device(), _viewport_near, camera, box.x,
                    box.y );
    form.identity();
...
CPhysicsShell* create_camera_shell( IPhysicsShellHolder* actor ) {
    VERIFY( actor );
    // CGameObject	*actor = smart_cast<CGameObject	*>(
    // Level().CurrentEntity() ); VERIFY( Level().CurrentEntity() );
    CPhysicsShell* shell =
        P_build_SimpleShell( actor, actor_camera_hudge_mass, true );
    CPhysicsElement* roote = shell->get_ElementByStoreOrder( 0 );
    // Fobb obb; obb.m_halfsize.set(0.5f,0.5f,0.5f);
    // obb.m_rotate.identity();obb.m_translate.set(0,0,0);
    Fcylinder cyl;
    ...
    cyl.m_radius = 0.5f;
    // roote->add_Box(obb);
    CODEGeom* character_test_geom =
        smart_cast< CODEGeom* >( xr_new< CCylinderGeom >( cyl ) );
    character_test_geom->build(
        Fvector().set( 0, 0, 0 ) ); // roote->mass_Center()
    ...
    character_test_geom->set_obj_contact_cb(
        cammera_shell_character_collide_callback );
...
void get_camera_box( Fvector& box_size,
                     Fmatrix& xform,
                     const CCameraBase& camera,
                     float _viewport_near ) {
    get_viewport_geom( box_size, xform, camera, _viewport_near );
    box_size.add( Fvector().set( camera_collision_sckin_depth,
                                 camera_collision_sckin_depth,
                                 camera_collision_sckin_depth ) );
}

void get_old_camera_box( Fvector& old_box_size,
                         Fmatrix& old_form,
                         const CPhysicsElement* roote,
                         const CBoxGeom* box ) {
    roote->GetGlobalTransformDynamic( &old_form );
    box->get_size( old_box_size );
}

void set_camera_collision( const Fvector& box_size,
                           const Fmatrix& xform,
    ...
    // HIGH-FOV DOORWAY-SNAG FIX (gated by g_clamp_actor_camera_collision,
    // default off): box_size is sized from FOV-driven viewport-near-plane
    // geometry (see get_viewport_geom
    // + tviewport_size). At FOV ≥ ~85 the resulting world-collision box gets
    // large enough to catch on doorframes/corridors, halting actor movement for
    // ~1s while up to 100 correction steps push the actor back. When enabled,
    // clamp the world-box dimensions to the values they would have had at FOV
    // ~75. Only the world-collision box geometry is clamped; the
    // character-vs-character cylinder below still uses the original box_size.x
    // for its radius, so NPC/mutant interactions are unchanged.
    if ( g_clamp_actor_camera_collision ) {
        const float kCamCollisionMaxY =
            0.23f; // 0.3 * tan(75/2 deg) ≈ FOV 75 vertical
        const float kCamCollisionMaxX =
            0.10f; // generous clamp; widescreen x is naturally small
        if ( bs.y > kCamCollisionMaxY )
            bs.y = kCamCollisionMaxY;
        if ( bs.x > kCamCollisionMaxX )
            bs.x = kCamCollisionMaxX;
    }
    ...
    // CBoxGeom* character_collision_geom = smart_cast<CBoxGeom*>(
    // roote->geometry( 1 ) );
    CCylinderGeom* character_collision_geom =
        smart_cast< CCylinderGeom* >( roote->geometry( 1 ) );
    VERIFY( character_collision_geom );
    const Fvector character_collision_box_size = Fvector().add(
        box_size, Fvector().set( camera_collision_character_skin_depth,
    ...
    Fvector shift_fv =
        Fvector().mul( xform.k, camera_collision_character_shift_z );
    shift_fv.y = 0;
    ...
    character_collision_geom_local_xform.c.set(
        Fvector()
            .mul( character_collision_geom_local_xform.j,
                  -camera_collision_character_gl_shift_y )
            .add( Fvector().set( shift_fv ) ) );
    // character_collision_geom_local_xform.c.y =-0.8f;
    character_collision_geom->set_local_form_bt(
        character_collision_geom_local_xform );
    roote->SetTransform( xform, mh_clear );
}

void do_collide_and_move( const Fmatrix& xform,
                          IPhysicsShellHolder* l_actor,
                          CPhysicsShell* shell,
                          CPhysicsElement* roote ) {
    ///////////////////////////////////////////////////////////////////
    VERIFY( ph_world );
...
bool do_collide_not_move( const Fmatrix& xform,
                          IPhysicsShellHolder* l_actor,
                          CPhysicsShell* shell,
                          CPhysicsElement* roote ) {
    ///////////////////////////////////////////////////////////////////
...
bool test_camera_box( const Fvector& box_size,
                      const Fmatrix& xform,
                      IPhysicsShellHolder* l_actor ) {
    // CPhysicsShellHolder* l_actor = smart_cast<CPhysicsShellHolder*>(
    // Level().CurrentEntity() );
    VERIFY( l_actor );
...
void collide_camera( CCameraBase& camera,
                     float _viewport_near,
                     IPhysicsShellHolder* l_actor ) {
    // CPhysicsShellHolder* l_actor = smart_cast<CPhysicsShellHolder*>(
    // Level().CurrentEntity() );
    ...
    Fmatrix old_form;
    get_old_camera_box( old_box_size, old_form, roote, box );
    if ( clamp_change( old_form, xform, EPS, EPS, EPS, EPS ) &&
         Fvector().sub( old_box_size, box_size ).magnitude() < EPS )
        return;
    set_camera_collision( box_size, xform, roote, box );
#ifdef DEBUG
    if ( dbg_draw_camera_collision ) {
