...
#include "../3rd_party/ode/include/ode/mass.h"
class CPHFracture;
class CPHElement;

DEFINE_VECTOR( dJointFeedback, CFEEDBACK_STORAGE, CFEEDBACK_I )

IC void sub_diapasones( u16& from1,
                        u16& to1,
                        const u16& from0,
                        const u16& to0 );
...
public:
    IC void sub_diapasone( const CShellSplitInfo& sub ) {
        sub_diapasones( m_start_el_num, m_end_el_num, sub.m_start_el_num,
                        sub.m_end_el_num );
        sub_diapasones( m_start_jt_num, m_end_jt_num, sub.m_start_jt_num,
                        sub.m_end_jt_num );
    }
...
class CPHFracture : public CShellSplitInfo {
    ...
    dMass m_firstM;
    dMass m_secondM;
    // when breaked m_pos_in_element-additional force m_break_force-additional
    // torque -x additional torque-y add_torque_z - additional torque z
    float m_break_force;
...
public:
    ...
    void PhTune( dBodyID body ); // set feedback for joints called from PhTune
                                 // of ShellSplitterHolder
    bool PhDataUpdate( CPHElement* element );
    // collect joints and external impacts in fractures Update which set
