...
// budget for islands that FORCE exact integration (script opt-in); ceiling
// keeps a force-flagged island merged into a large pile from stepping the
// O(n^3) solver
...
float world_cfm = CFM( SPRING_S( base_cfm, base_erp, base_fixed_step ),
                       DAMPING( base_cfm, base_erp ) );
float world_erp = ERP( SPRING_S( base_cfm, base_erp, base_fixed_step ),
                       DAMPING( base_cfm, base_erp ) );
...
/////////////////////////////////////////////////////////////////////////////
IC void add_contact_body_effector( dBodyID body,
                                   const dContact& c,
                                   SGameMtl* material ) {
    CPHContactBodyEffector* effector =
        ( CPHContactBodyEffector* )dBodyGetData( body );
    if ( effector )
...
IC static int CollideIntoGroup( dGeomID o1,
    ...
    for ( i = 0; i < n; ++i ) {
        ...
        float damping =
            material_2->fPHDamping * material_1->fPHDamping * world_damping;
...
#pragma warning( disable : 4245 )
            if ( material_1->Flags.test( SGameMtl::flSlowDown ) &&
                 !( !usr_data_2 || usr_data_2->pushing_neg ||
...
#pragma warning( pop )
            {
                ...
                if ( material_1->Flags.test( SGameMtl::flLiquid ) ) {
                    ...
                    if ( !usr_data_2 || !usr_data_2->ph_object ||
                         !usr_data_2->ph_object->IsRayMotion() ) {
                        add_contact_body_effector( body, c, material_1 );
                    }
                }
            }
...
#pragma warning( disable : 4245 )
            if ( material_2->Flags.test( SGameMtl::flSlowDown ) &&
                 !( !usr_data_1 || usr_data_1->pushing_neg ||
...
#pragma warning( pop )
            {
                ...
                if ( material_2->Flags.test( SGameMtl::flLiquid ) ) {
                    ...
                    if ( !usr_data_1 || !usr_data_1->ph_object ||
                         !usr_data_1->ph_object->IsRayMotion() ) {
                        add_contact_body_effector( body, c, material_2 );
                    }
                }
            }
        ...
        if ( flags_1.test( SGameMtl::flBounceable ) &&
             flags_2.test( SGameMtl::flBounceable ) ) {
            surface.mode |= dContactBounce;
            surface.bounce_vel = _max( material_1->fPHBounceStartVelocity,
                                       material_2->fPHBounceStartVelocity );
            surface.bounce =
                _min( material_1->fPHBouncing, material_2->fPHBouncing );
        }
        /////////////////////////////////////////////////////////////////////////////////////////////////
        if ( usr_data_2 && usr_data_2->object_callbacks ) {
            usr_data_2->object_callbacks->Call( do_collide, false, c,
                                                material_1, material_2 );
        }

        if ( usr_data_1 && usr_data_1->object_callbacks ) {
            usr_data_1->object_callbacks->Call( do_collide, true, c, material_1,
                                                material_2 );
        }
        ...
        if ( usr_data_2 ) {
            ...
            if ( usr_data_2->ph_object ) {
                usr_data_2->ph_object->InitContact(
                    &c, do_collide, material_idx_1, material_idx_2 );
            }
        }
        ///////////////////////////////////////////////////////////////////////////////////////
        if ( usr_data_1 ) {
            ...
            if ( usr_data_1->ph_object ) {
                usr_data_1->ph_object->InitContact(
                    &c, do_collide, material_idx_1, material_idx_2 );
            }
        }
        ...
        if ( do_collide && collided_contacts < MAX_CONTACTS ) {
            ...
            dJointAttach( contact_joint, dGeomGetBody( g1 ),
                          dGeomGetBody( g2 ) );
        }
    }
...
void NearCallback( CPHObject* obj1, CPHObject* obj2, dGeomID o1, dGeomID o2 ) {
    ...
    if ( !island1->CanMerge( island2, MAX_CONTACTS ) )
        return;
    if ( CollideIntoGroup( o1, o2, ContactGroup, island1, MAX_CONTACTS ) !=
         0 ) {
...
void CollideStatic( dGeomID o2, CPHObject* obj2 ) {
    ...
    CPHIsland* island2 = obj2->DActiveIsland();
    CollideIntoGroup( ph_world->GetMeshGeom(), o2, ContactGroup, island2,
                      island2->MaxJoints() );
}
...
//result velocity
dVector3
vel={start_vel[0]+add_vel[0],start_vel[1]+add_vel[1],start_vel[2]+add_vel[2]};
...
//sinus of the angle between cutted velocity and adding velociity (sinus
theorem) dReal sinus2=sinus1/linear_limit*start_speed; dReal
sinus2_2=sinus2*sinus2; if(sinus2_2>1.f)sinus2_2=1.f;
...
void dBodyAngAccelFromTorqu( const dBodyID body,
                             dReal* ang_accel,
                             const dReal* torque ) {
    dMass m;
    dMatrix3 invI;
...
void BodyCutForce( dBodyID body, float l_limit, float w_limit ) {
    ...
    if ( force_mag > force_limit ) {
        dBodySetForce( body, force[ 0 ] / force_mag * force_limit,
...
// body - static case
float E_NlS( dBodyID body,
             const dReal* norm,
             float norm_sign ) // if body c.geom.g1 norm_sign + else -
{
...
float E_NLD( dBodyID b1, dBodyID b2, const dReal* norm ) // norm - from 2 to 1
{
    ...
    dVector3 impuls1 = { vel1[ 0 ] * m1.mass, vel1[ 1 ] * m1.mass,
                         vel1[ 2 ] * m1.mass };
    dVector3 impuls2 = { vel2[ 0 ] * m2.mass, vel2[ 1 ] * m2.mass,
                         vel2[ 2 ] * m2.mass };

    dVector3 c_mas_impuls = { impuls1[ 0 ] + impuls2[ 0 ],
                              impuls1[ 1 ] + impuls2[ 1 ],
                              impuls1[ 2 ] + impuls2[ 2 ] };
    dReal cmass = m1.mass + m2.mass;
    dVector3 c_mass_vel = { c_mas_impuls[ 0 ] / cmass,
                            c_mas_impuls[ 1 ] / cmass,
                            c_mas_impuls[ 2 ] / cmass };

    dReal c_mass_vel_prg = dDOT( c_mass_vel, norm );

    dReal kin_energy_start =
        vel_pr1 * vel_pr1 * m1.mass / 2.f + vel_pr2 * vel_pr2 * m2.mass / 2.f;
...
void ApplyGravityAccel( dBodyID body, const dReal* accel ) {
    dMass m;
    dBodyGetMass( body, &m );
    dBodyAddForce( body, accel[ 0 ] * m.mass, accel[ 1 ] * m.mass,
                   accel[ 2 ] * m.mass );
}
