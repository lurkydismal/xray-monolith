...
private:
    ...
    IC void
    UpdateInterpolation() // interpolation called from ph update visual influent
    {
...
public:
    ...
    virtual void set_ObjectContactCallback(
        ObjectContactCallbackFun*
            callback ); // called anywhere ph state influent
...
private:
    ...
    void calculate_it_data_use_density( const Fvector& mc,
                                        float density ); // aux
...
public:  //
    ...
    // void						BoneGlPos
    // (Fmatrix &m, const CBoneInstance* B)const;
    ...
    void ToBonePos( const Fmatrix& BoneTransform,
                    motion_history_state history_state );
    ...
    void _BCL
    BonesCallBack( CBoneInstance* B ); // called from updateCL visual influent
    ...
    virtual void SetAirResistance(
        dReal linear = default_k_l,
        dReal angular = default_k_w ) // aux (may not be)
    {
        //
        k_w = angular; //
        k_l = linear;  //
    ...
    virtual void set_DynamicLimits(
        float l_limit = default_l_limit,
        float w_limit = default_w_limit ); // aux (may not be)
    virtual void set_DynamicScales(
        float l_scale = default_l_scale,
        float w_scale = default_w_scale ); // aux (may not be)
    virtual void Fix();
    ...
    virtual void set_LinearVel(
        const Fvector& velocity ); // called anywhere ph state influent
    virtual void set_AngularVel(
        const Fvector& velocity ); // called anywhere ph state influent
    ...
    virtual void cv2obj_Xfrom( const Fquaternion& q,
                               const Fvector& pos,
                               Fmatrix& xform ); //
    virtual void cv2bone_Xfrom( const Fquaternion& q,
                                const Fvector& pos,
                                Fmatrix& xform ); //
    virtual void _BCL
    InterpolateGlobalTransform( Fmatrix* m ); // called UpdateCL vis influent
    ...
    virtual void Activate(
        bool disable = false,
        bool not_set_bone_callbacks = false ); // some isues not to be aux
    ...
    void CreateSimulBase(); // create body & cpace
                            // //aux
    void ReInitDynamics( const Fmatrix& shift_pivot,
                         float density ); // set body & geom positions
    void PresetActive();                  //
    void build();                         // aux
