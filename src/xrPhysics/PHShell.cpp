...
void CPHShell::MassAddBox( float mass, const Fvector& full_size ) {
    dMass m;
    dMassSetBox(
        &m, mass, full_size.x, full_size.y,
        full_size.z ); // mass = m_Shell->getMass()/100.f, full_size (1,1,1)
...
void CPHShell::applyImpulseTrace( const Fvector& pos,
                                  const Fvector& dir,
                                  float val ) {
    if ( !isActive() )
        return;
...
void CPHShell::applyImpulseTrace( const Fvector& pos,
    ...
    ( ( CPhysicsElement* )instance.callback_param() )
...
CPhysicsElement* CPHShell::get_Element( u16 bone_id ) {
    if ( m_pKinematics && isActive() ) {
        ...
        if ( instance.callback() == BonesCallback ||
             instance.callback() == StataticRootBonesCallBack ) {
            return ( instance.callback_type() == bctPhysics )
                       ? ( CPhysicsElement* )instance.callback_param()
                       : NULL;
        }
    }
...
void CPHShell::update_root_transforms() {
    ...
    // Fmatrix anim_root_transform = PKinematics()->LL_GetBindTransform(
    // anim_root );
}

void CPHShell::BonesCallback( CBoneInstance* B ) {
    /// CPHElement*  E           = smart_cast<CPHElement*>
    /// (static_cast<CPhysicsBase*>(B->Callback_Param));
...
void CPHShell::remove_ObjectContactCallback(
...
void CPHShell::TransformPosition( const Fmatrix& form,
    ...
    ELEMENT_I i = elements.begin(), e = elements.end();
    for ( ; i != e; i++ )
        ( *i )->TransformPosition( form, history_state );
}
...
void CPHShell::build_FromKinematics( IKinematics* K,
                                     BONE_P_MAP* p_geting_map ) {
    VERIFY( K );
    ...
    AddElementRecursive( 0, m_pKinematics->LL_GetBoneRoot(), Fidentity, 0,
                         &vis_check );
    // R_ASSERT2((*elements.begin())->numberOfGeoms(),"No physics shapes was
    // assigned for model or no shapes in main root bone!!!");
    // SetCallbacks(BonesCallback);
    if ( m_spliter_holder->isEmpty() )
        ClearBreakInfo();
}

void CPHShell::preBuild_FromKinematics( IKinematics* K,
                                        BONE_P_MAP* p_geting_map ) {
    VERIFY( K );
    phys_shell_verify_model( *K );
    m_pKinematics = K;
...
ICF bool no_physics_shape( const SBoneShape& shape ) {
    return shape.type == SBoneShape::stNone ||
           shape.flags.test( SBoneShape::sfNoPhysics );
}
...
void CPHShell::AddElementRecursive( CPhysicsElement* root_e,
                                    u16 id,
                                    Fmatrix global_parent,
                                    u16 element_number,
                                    bool* vis_check ) {
    ...
    if ( no_visible ) {
        // for (vecBonesIt it=bone_data.children.begin();
        // bone_data.children.end() != it; ++it) AddElementRecursive
        ...
        for ( u16 i = 0; i < num_children; ++i )
            AddElementRecursive( root_e, bone_data.GetChild( i ).GetSelfID(),
                                 fm_position, element_number, &lvis_check );
    ...
    bool element_added =
        false; // set true when if elemen created and added by this call
    ...
    if ( !no_physics_shape( bone_data.get_shape() ) || !root_e ) //
    {
        if ( joint_data.type == jtRigid && root_e ) //
        {
            ...
            if ( breakable ) {
                ...
                R_ASSERT2( id < 64,
                           "ower 64 bones in breacable are not supported" );
                ...
                root_e->add_Mass( bone_data.get_shape(), vs_root_position,
                                  bone_data.get_center_of_mass(),
        ...
        } else //
        {
            ...
            if ( !no_physics_shape( bone_data.get_shape() ) ) {
                ...
                E->setMassMC( bone_data.get_mass(),
                              bone_data.get_center_of_mass() );
            }
            ...
            if ( root_e ) {
                ...
                if ( J ) {
                    ...
                    if ( breakable ) {
                        setEndJointSplitter();
                        J->SetBreakable( joint_data.break_force,
                                         joint_data.break_torque );
                    }
                }
            }
            if ( m_spliter_holder ) {
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    // for (vecBonesIt it=bone_data.children.begin(); bone_data.children.end()
    // != it; ++it)
    //   AddElementRecursive
    //   (E,(*it)->GetSelfID(),fm_position,element_number,arg_check);
    // IBoneData &ibone_data = bone_data;
    u16 num_children = bone_data.GetNumChildren();
    for ( u16 i = 0; i < num_children; ++i )
        AddElementRecursive( E, bone_data.GetChild( i ).GetSelfID(),
                             fm_position, element_number, arg_check );
    /////////////////////////////////////////////////////////////////////////////////////
    if ( breakable ) {
        if ( joint_data.type == jtRigid ) {
            ...
            fracture.m_end_el_num =
                u16( elements.size() ); // just after this el = current+1
    ...
    VERIFY3( bbb, dbg_obj->ObjectNameVisual(),
             "has breaking parts with no vertexes or size less than 1mm" ); //
#endif
}
...
void CPHShell::ResetCallbacksRecursive( u16 id, u16 element, Flags64& mask ) {
    ...
    if ( mask.is( 1ui64 << ( u64 )id ) ) {
        if ( ( no_physics_shape( bone_data.get_shape() ) ||
               joint_data.type == jtRigid ) &&
             element != u16( -1 ) ) {
            B.set_callback( bctPhysics, 0,
                            cast_PhysicsElement( elements[ element ] ) );
        } else {
            element++;
            R_ASSERT2( element < elements.size(), "Out of elements!!" );
            // if(elements.size()==element)  return;
            B.set_callback( bctPhysics, BonesCallback,
                            cast_PhysicsElement( elements[ element ] ) );
    ...
    // for (vecBonesIt it=bone_data.children.begin();
    // it!=bone_data.children.end(); ++it)
    //   ResetCallbacksRecursive((*it)->GetSelfID(),element,mask);
    // IBoneData &ibone_data = bone_data;
    u16 num_children = bone_data.GetNumChildren();
    for ( u16 i = 0; i < num_children; ++i )
        ResetCallbacksRecursive( bone_data.GetChild( i ).GetSelfID(), element,
                                 mask );
}
...
void CPHShell::EnabledCallbacks( BOOL val ) {
    if ( val ) {
        ...
        for ( ; i != e; ++i ) {
            CBoneInstance& B =
                m_pKinematics->LL_GetBoneInstance( ( *i )->m_SelfID );
            B.set_callback_overwrite( TRUE );
        }
    } else
        ZeroCallbacks();
        ...
        void operator()( u16 id ) {
            ...
            if ( !bi.callback() || bi.callback_type() != bctPhysics ) {
                ...
                if ( root_e && K.LL_GetBoneVisible( id ) )
                    bi.set_callback( bctPhysics, 0,
                                     cast_PhysicsElement( root_e ) );
            }
        }
    };
...
void CPHShell::set_DynamicScales( float l_scale /* =default_l_scale */,
...
void CPHShell::setEndElementSplitter() {
    if ( !elements.back()->FracturesHolder() )
        // adding fracture for element supposed before adding splitter. Need
        // only one splitter for an element
        AddSplitter( CPHShellSplitter::splElement, u16( elements.size() - 1 ),
                     u16( joints.size() - 1 ) );
}

void CPHShell::setElementSplitter( u16 element_number, u16 splitter_position ) {
    AddSplitter( CPHShellSplitter::splElement, element_number,
                 element_number - 1, splitter_position );
}

void CPHShell::AddSplitter( CPHShellSplitter::EType type,
                            u16 element,
                            u16 joint ) {
    if ( !m_spliter_holder )
        m_spliter_holder = xr_new< CPHShellSplitterHolder >( this );
...
void CPHShell::setEndJointSplitter() {
    if ( !joints.back()->JointDestroyInfo() )
        // setting joint breacable supposed before adding splitter. Need only
        // one splitter for a joint
        AddSplitter( CPHShellSplitter::splJoint, u16( elements.size() - 1 ),
                     u16( joints.size() - 1 ) );
}
...
void CPHShell::PlaceBindToElForms() {
    Flags64 mask;
    mask.assign( m_pKinematics->LL_GetBonesVisible() );
    PlaceBindToElFormsRecursive( Fidentity, m_pKinematics->LL_GetBoneRoot(), 0,
                                 mask );
}
...
void CPHShell::PlaceBindToElFormsRecursive( Fmatrix parent,
                                            u16 id,
                                            u16 element,
                                            Flags64& mask ) {
    ...
    if ( mask.is( 1ui64 << ( u64 )id ) ) {
        if ( no_physics_shape( bone_data.shape ) ||
             joint_data.type == jtRigid && element != u16( -1 ) ) {
        } else {
            element++;
            R_ASSERT2( element < elements.size(), "Out of elements!!" );
            // if(elements.size()==element)  return;
    ...
    for ( u16 i = 0; i < num_children; ++i )
        PlaceBindToElFormsRecursive(
            mXFORM, ibone_data.GetChild( i ).GetSelfID(), element, mask );
}
...
void CPHShell::BonesBindCalculateRecursive( Fmatrix parent, u16 id ) {
    ...
    u16 num_children = ibone_data.GetNumChildren();
    for ( u16 i = 0; i < num_children; ++i )
        BonesBindCalculateRecursive( bone_instance.mTransform,
                                     ibone_data.GetChild( i ).GetSelfID() );
}
...
void CPHShell::RegisterToCLGroup( CGID g ) {
    CPHCollideValidator::RegisterObjToGroup(
        g, *static_cast< CPHObject* >( this ) );
}
...
void CPHShell::CreateShellAnimator( CInifile const* ini, LPCSTR section ) {
    ...
    m_pPhysicsShellAnimatorC =
        xr_new< CPhysicsShellAnimator >( this, ini, section );
...
void CPHShell::get_Extensions( const Fvector& axis,
                               float center_prg,
                               float& lo_ext,
                               float& hi_ext ) const {
...
void CPHShell::ToAnimBonesPositions( motion_history_state history_state ) {
    ...
    for ( ; i != e; ++i )
        ( *i )->ToBonePos(
            &PKinematics()->LL_GetBoneInstance( ( *i )->m_SelfID ),
            history_state );
}
...
void CPHShell::dbg_draw_geometry(
    float scale,
    u32 color,
