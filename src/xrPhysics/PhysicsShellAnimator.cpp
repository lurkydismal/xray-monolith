...
CPhysicsShellAnimator::CPhysicsShellAnimator( CPhysicsShell* _pPhysicsShell,
                                              CInifile const* ini,
                                              LPCSTR section )
    : m_pPhysicsShell( _pPhysicsShell ) {
    VERIFY( ini->section_exist( section ) );
    IPhysicsShellHolder* obj =
        ( *( _pPhysicsShell->Elements().begin() ) )->PhysicsRefObject();
    ...
    if ( all_bones )
        for ( xr_vector< CPHElement* >::iterator i =
                  m_pPhysicsShell->Elements().begin();
              i != m_pPhysicsShell->Elements().end(); i++ )
            CreateJoint( *i );

    if ( ini->line_exist( section, "leave_joints" ) &&
         xr_strcmp( ini->r_string( section, "leave_joints" ), "all" ) == 0 )
        return;
...
CPhysicsShellAnimator::~CPhysicsShellAnimator() {
    for ( xr_vector< CPhysicsShellAnimatorBoneData >::iterator i =
              m_bones_data.begin();
          i != m_bones_data.end(); i++ ) {
        ( ( CPHShell* )( m_pPhysicsShell ) )
            ->Island()
            .DActiveIsland()
            ->RemoveJoint( i->m_anim_fixed_dJointID );
...
void CPhysicsShellAnimator::CreateJoints( LPCSTR controled ) {
    IPhysicsShellHolder* obj =
        ( *( m_pPhysicsShell->Elements().begin() ) )->PhysicsRefObject();
    ...
    for ( u16 i = 0; nb > i; ++i ) {
        ...
        VERIFY2( bid != BI_NONE,
                 make_string( "shell_animation - controled bone %s not found! "
                              "object: %s, model: %s",
                              n, obj->ObjectName(), obj->ObjectNameVisual() ) );
        CPHElement* e =
            smart_cast< CPHElement* >( m_pPhysicsShell->get_Element( bid ) );
        VERIFY2( e,
                 make_string( "shell_animation - controled bone %s has no "
                              "physics collision! object: %s, model: %s",
                              n, obj->ObjectName(), obj->ObjectNameVisual() ) );
        CreateJoint( e );
    }
}
...
void CPhysicsShellAnimator::OnFrame( bool calculate_bones ) {
    ...
    for ( xr_vector< CPhysicsShellAnimatorBoneData >::iterator i =
        ...
        CBoneInstance& B = m_pPhysicsShell->PKinematics()->LL_GetBoneInstance(
            i->m_element->m_SelfID );
...
#pragma todo( "reset callback?" )
        ...
        Fvector mc;
        i->m_element->CPHGeometryOwner::get_mc_vs_transform(
            mc, target_obj_posFmatrixS );

        Fvector4 ODEVect;
        ODEVect.set( mc.x, mc.y, mc.z );

        dJointSetFixedQuaternionPos( i->m_anim_fixed_dJointID,
                                     target_obj_quat_dQuaternionS, &ODEVect.x );
    }
