...
ICF void CalculateInitTriangle( CDB::TRI* XTri,
                                Triangle& triangle,
                                const Fvector* V_array ) {
    // VERIFY(g_pGameLevel);
    // const Fvector* V_array=inl_ph_world().ObjectSpace().GetStaticVerts();
    const float* VRT[ 3 ] = { ( dReal* )&V_array[ XTri->verts[ 0 ] ],
...
ICF void CalculateTriangle( CDB::TRI* XTri,
                            const float* pos,
                            Triangle& triangle,
                            const Fvector* V_array ) {
...
inline bool TriContainPoint( const dReal* v0,
                             const dReal* v1,
                             const dReal* v2,
...
ICF bool TriContainPoint( const dReal* v0,
                          const dReal* v1,
                          const dReal* v2,
                          const dReal* triAx,
                          const dReal* triSideAx0,
    ...
    dVector3 triSideAx2 = { v0[ 0 ] - v2[ 0 ], v0[ 1 ] - v2[ 1 ],
                            v0[ 2 ] - v2[ 2 ] };
    return TriContainPoint( v0, v1, v2, triSideAx0, triSideAx1, triSideAx2,
                            triAx, pos, c );
}

ICF bool TriContainPoint( Triangle* T,
                          const float* pos,
                          u16& c,
                          const Fvector* V_array ) {
    // TriContainPoint(const dReal* v0,const dReal* v1,const dReal* v2,const
    // dReal* triAx,const dReal* triSideAx0,const dReal* triSideAx1, const
    // dReal* pos) VERIFY( g_pGameLevel ); const Fvector*
    // V_array=inl_ph_world().ObjectSpace().GetStaticVerts();
    CDB::TRI* XTri = T->T;
    const float* VRT[ 3 ] = { ( dReal* )&V_array[ XTri->verts[ 0 ] ],
                              ( dReal* )&V_array[ XTri->verts[ 1 ] ],
                              ( dReal* )&V_array[ XTri->verts[ 2 ] ] };
    return TriContainPoint( VRT[ 0 ], VRT[ 1 ], VRT[ 2 ], T->norm, T->side0,
                            T->side1, pos, c );
}

enum ETriDist { tdBehind, tdPlane, tdSide, tdVert };

IC float DistToFragmenton( const dReal* point,
                           const dReal* pt1,
                           const dReal* pt2,
                           dReal* p,
                           dReal* to_point,
    ...
    dVector3 V = { pt2[ 0 ] - pt1[ 0 ], pt2[ 1 ] - pt1[ 1 ],
                   pt2[ 2 ] - pt1[ 2 ] };
    dVector3 L = { pt1[ 0 ] - point[ 0 ], pt1[ 1 ] - point[ 1 ],
                   pt1[ 2 ] - point[ 2 ] };
    ...
    dVector3 Pc = { pt1[ 0 ] + t * V[ 0 ], pt1[ 1 ] + t * V[ 1 ],
                    pt1[ 2 ] + t * V[ 2 ] };
    dVectorSet( p, Pc );
    dVector3 Dc = { point[ 0 ] - Pc[ 0 ], point[ 1 ] - Pc[ 1 ],
                    point[ 2 ] - Pc[ 2 ] };
    dVectorSet( to_point, Dc );
    return dSqrt( dDOT( Dc, Dc ) );
}

ICF float DistToTri( Triangle* T,
                     const float* pos,
                     float* dir,
                     float* p,
                     ETriDist& c,
