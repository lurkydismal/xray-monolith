...
#ifdef DEBUG
enum EDumpType { base, poses, vis_geom, props, full, full_capped };
#endif
...
public:
    ...
    // virtual	void
    // SetWeaponHideState					( u16 State,
    // bool bSet )=0;
    virtual void _BCL HideAllWeapons(
        bool v ) = 0; //(SetWeaponHideState(INV_STATE_BLOCK_ALL,true))
    virtual void _BCL MovementCollisionEnable( bool enable ) = 0;
    virtual CPHSoundPlayer* _BCL ObjectPhSoundPlayer() = 0;
    virtual ICollisionDamageReceiver* _BCL
    ObjectPhCollisionDamageReceiver() = 0;
