// Do NOT build this file seperately. It is included in dTriList.cpp
// automatically.
...
int dCollideBP( const dxGeom* o1,
                const dxGeom* o2,
                int flags,
                dContactGeom* contact,
                int skip ); // ODE internal function

// #define CONTACT(Ptr, Stride) ((dContactGeom*) (((byte*)Ptr) + (Stride)))
// #define SURFACE(Ptr, Stride) ((dSurfaceParameters*) (((byte*)Ptr) +
// (Stride-sizeof(dSurfaceParameters))))

int dcTriListCollider::CollideBox( dxGeom* Box,
                                   int Flags,
    ...
    AABB.x =
        ( dFabs( BoxSides[ 0 ] * R[ 0 ] ) + dFabs( BoxSides[ 1 ] * R[ 1 ] ) +
          dFabs( BoxSides[ 2 ] * R[ 2 ] ) ) /
            2.f +
        10.f * EPS_L;
    AABB.y =
        ( dFabs( BoxSides[ 0 ] * R[ 4 ] ) + dFabs( BoxSides[ 1 ] * R[ 5 ] ) +
          dFabs( BoxSides[ 2 ] * R[ 6 ] ) ) /
            2.f +
        10.f * EPS_L;
    AABB.z =
        ( dFabs( BoxSides[ 0 ] * R[ 8 ] ) + dFabs( BoxSides[ 1 ] * R[ 9 ] ) +
          dFabs( BoxSides[ 2 ] * R[ 10 ] ) ) /
            2.f +
        10.f * EPS_L;
    ...
    BoxTri bt( *this );
    return dSortTriPrimitiveCollide( bt, Box, Geometry, Flags, Contacts, Stride,
                                     AABB );
}

int dcTriListCollider::CollideCylinder( dxGeom* Cylinder,
                                        int Flags,
                                        dContactGeom* Contacts,
                                        int Stride ) {
    Fvector AABB;
    ...
    CylTri ct( *this );
    return dSortTriPrimitiveCollide( ct, Cylinder, Geometry, Flags, Contacts,
                                     Stride, AABB );
}
...
///////////////////////////////////////////////////////////////////////////
int dcTriListCollider::CollideSphere( dxGeom* Sphere,
                                      int Flags,
                                      dContactGeom* Contacts,
                                      int Stride ) {
    ...
    AABB.z += dFabs( velocity[ 2 ] ) * 0.04f;
    SphereTri st( *this );
    return dSortTriPrimitiveCollide( st, Sphere, Geometry, Flags, Contacts,
                                     Stride, AABB );
}
