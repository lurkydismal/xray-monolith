...
IC float dcTriListCollider::dBoxProj( dxGeom* box, const dReal* normal ) {
    ...
    return dFabs( dDOT14( normal, R + 0 ) * hside[ 0 ] ) +
           dFabs( dDOT14( normal, R + 1 ) * hside[ 1 ] ) +
           dFabs( dDOT14( normal, R + 2 ) * hside[ 2 ] );
}

IC void dcTriListCollider::CrossProjLine( const dReal* pt1,
                                          const dReal* vc1,
                                          const dReal* pt2,
                                          const dReal* vc2,
                                          dReal* proj ) {
    dVector3 ac = { pt1[ 0 ] - pt2[ 0 ], pt1[ 1 ] - pt2[ 1 ],
                    pt1[ 2 ] - pt2[ 2 ] };
    dReal factor = ( dDOT( vc2, vc2 ) * dDOT44( vc1, vc1 ) -
                     dDOT14( vc2, vc1 ) * dDOT14( vc2, vc1 ) );
    if ( factor == 0.f ) {
        proj[ 0 ] = dInfinity;
        // proj[1]=dInfinity;
        // proj[2]=dInfinity;
    ...
    dReal t = ( dDOT( ac, vc2 ) * dDOT41( vc1, vc2 ) -
                dDOT41( vc1, ac ) * dDOT( vc2, vc2 ) ) /
              factor;
...
IC void dcTriListCollider::CrossProjLine1( const dReal* pt1,
                                           const dReal* vc1,
                                           const dReal* pt2,
                                           const dReal* vc2,
                                           dReal* proj ) {
    dVector3 ac = { pt1[ 0 ] - pt2[ 0 ], pt1[ 1 ] - pt2[ 1 ],
                    pt1[ 2 ] - pt2[ 2 ] };
    dReal factor = ( dDOT44( vc2, vc2 ) * dDOT( vc1, vc1 ) -
                     dDOT41( vc2, vc1 ) * dDOT41( vc2, vc1 ) );
    if ( factor == 0.f ) {
        proj[ 0 ] = dInfinity;
        // proj[1]=dInfinity;
        // proj[2]=dInfinity;
    ...
    dReal t = ( dDOT14( ac, vc2 ) * dDOT14( vc1, vc2 ) -
                dDOT( vc1, ac ) * dDOT44( vc2, vc2 ) ) /
              factor;
...
IC bool dcTriListCollider::CrossProjLine14( const dReal* pt1,
                                            const dReal* vc1,
                                            const dReal* pt2,
                                            const dReal* vc2,
                                            dReal hside,
...
// is point in Box
IC bool dcTriListCollider::IsPtInBx( const dReal* Pt,
                                     const dReal* BxP,
                                     const dReal* BxEx,
                                     const dReal* BxR ) {
    dVector3 BxPR, PtR;
...
inline dReal PointBoxTest( const dReal* Pt,
                           const dReal* BxP,
                           const dReal* BxEx,
                           const dReal* R,
                           dReal* norm ) {
...
IC dReal dcTriListCollider::FragmentonBoxTest( const dReal* Pt1,
                                               const dReal* Pt2,
                                               const dReal* BxP,
                                               const dReal* BxEx,
                                               const dReal* R,
                                               dReal* norm,
                                               dReal* pos ) {
    dVector3 fragmentonAx = { Pt2[ 0 ] - Pt1[ 0 ], Pt2[ 1 ] - Pt1[ 1 ],
                              Pt2[ 2 ] - Pt1[ 2 ] };
    dReal BxExPr;
    accurate_normalize( fragmentonAx );
    dReal BxPPr = dDOT( fragmentonAx, BxP );
    BxExPr = dFabs( dDOT14( fragmentonAx, R + 0 ) * BxEx[ 0 ] ) +
             dFabs( dDOT14( fragmentonAx, R + 1 ) * BxEx[ 1 ] ) +
             dFabs( dDOT14( fragmentonAx, R + 2 ) * BxEx[ 2 ] );

    if ( ( dDOT( fragmentonAx, Pt1 ) - BxPPr - BxExPr / 2.f ) *
                 ( dDOT( fragmentonAx, Pt2 ) - BxPPr - BxExPr / 2.f ) >
             0.f &&
    ...
    BxExPr = dFabs( dDOT14( crossAx0, R + 0 ) * BxEx[ 0 ] ) +
    ...
    BxExPr = dFabs( dDOT14( crossAx1, R + 0 ) * BxEx[ 0 ] ) +
    ...
    BxExPr = dFabs( dDOT14( crossAx2, R + 0 ) * BxEx[ 0 ] ) +
...
/*
inline dReal FragmentonBoxTest1(const dReal* Pt1,const dReal* Pt2,const dReal*
BxP,const dReal* BxEx,const dReal* R,dReal* norm){
