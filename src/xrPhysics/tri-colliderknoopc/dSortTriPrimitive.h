...
IC bool negative_tri_set_ignored_by_positive_tri( const Triangle& neg_tri,
                                                  const Triangle& pos_tri,
    ...
    return ( common0 && common1 && common2 ) ||
           !common0 && !( dDOT( neg_tri.norm,
                                ( dReal* )&V_array[ pos_tri.T->verts[ 0 ] ] ) >
                          neg_tri.pos ) ||
           !common1 && !( dDOT( neg_tri.norm,
                                ( dReal* )&V_array[ pos_tri.T->verts[ 1 ] ] ) >
                          neg_tri.pos ) ||
           !common2 && !( dDOT( neg_tri.norm,
                                ( dReal* )&V_array[ pos_tri.T->verts[ 2 ] ] ) >
                          neg_tri.pos );
}

int SetBackTrajectoryCnt( const dReal* p,
...
template < class T >
IC int dcTriListCollider::dSortTriPrimitiveCollide( T primitive,
                                                    dxGeom* o1,
                                                    dxGeom* o2,
                                                    int flags,
    ...
    if ( no_last_pos || !last_box.contains( box ) ) {
        ...
        XRC.box_options( 0 );
        // VERIFY( g_pGameLevel );
        XRC.box_query( inl_ph_world().ObjectSpace().GetStaticModel(),
                       cast_fv( p ), aabb );
    ...
    if ( debug_output().ph_dbg_draw_mask().test( phDbgDrawTriTestAABB ) )
        debug_output().DBG_DrawAABB( cast_fv( p ), AABB,
    ...
    if ( *pushing_neg ) {
        ...
        const dReal* neg_vertices[ 3 ] = {
            cast_fp( V_array[ neg_tri.T->verts[ 0 ] ] ),
            cast_fp( V_array[ neg_tri.T->verts[ 1 ] ] ),
            cast_fp( V_array[ neg_tri.T->verts[ 2 ] ] ) };
        neg_tri_contains_point = TriContainPoint(
            neg_vertices[ 0 ], neg_vertices[ 1 ], neg_vertices[ 2 ],
        ...
        if ( debug_output().ph_dbg_draw_mask().test( phDbgDrawSavedTries ) )
            debug_output().DBG_DrawTri( neg_tri.T, V_array,
...
#ifdef DEBUG
        if ( debug_output().ph_dbg_draw_mask().test( phDbgDrawSavedTries ) )
            debug_output().DBG_DrawTri( b_neg_tri.T, V_array,
                                        D3DCOLOR_XRGB( 0, 0, 255 ) );
#endif
    }
    ...
    for ( I = B; I != E; ++I ) {
        ...
        const Point vertices[ 3 ] = {
            Point( ( dReal* )&V_array[ T->verts[ 0 ] ] ),
            Point( ( dReal* )&V_array[ T->verts[ 1 ] ] ),
        ...
        if ( debug_output().ph_dbg_draw_mask().test(
            ...
            debug_output().DBG_DrawTri( T, V_array,
        ...
        if ( tri.dist < 0.f ) {
            ...
            if ( ( !( last_pos_dist < 0.f ) ) || b_pushing )
                if ( __aabb_tri( Point( p ), Point( ( float* )&AABB ),
                                 vertices ) ) {
#ifdef DEBUG
                    if ( debug_output().ph_dbg_draw_mask().test(
                             phDBgDrawTriesChangesSign ) )
                        debug_output().DBG_DrawTri(
                            T, V_array, D3DCOLOR_XRGB( 0, 255, 0 ) );
#endif
                    SGameMtl* material =
                        GMLibrary().GetMaterialByIdx( T->material );
                    VERIFY( material );
                    bool b_passable =
                        !!material->Flags.test( SGameMtl::flPassable );
                    bool contain_pos = TriContainPoint(
                        vertices[ 0 ], vertices[ 1 ], vertices[ 2 ], tri.norm,
                        tri.side0, tri.side1, p );
                    bool b_pased = false;
                        ...
                        if ( !no_last_pos && !b_passable ) {
                            ...
                            if ( debug_output().ph_dbg_draw_mask().test(
                                ...
                                debug_output().DBG_DrawLine(
                                    cast_fv( last_pos ), cast_fv( p ),
                                    D3DCOLOR_XRGB( 255, 0, 255 ) );
#endif
                            dVector3 tri_point;
                            PlanePoint( tri, last_pos, p, last_pos_dist,
                                        tri_point );
#ifdef DEBUG
                            if ( debug_output().ph_dbg_draw_mask().test(
                                     phDbgDrawTriPoint ) )
                                debug_output().DBG_DrawPoint(
                                    cast_fv( tri_point ), 0.01f,
                                    D3DCOLOR_XRGB( 255, 0, 255 ) );
#endif
                            ...
                            intersect =
                                ...
                                TriContainPoint(
                                    ...
                                    tri.norm, tri.side0, tri.side1, tri_point );
                            b_pased = intersect && !was_intersect;
                            gb_pased = b_pased || gb_pased;
#ifdef DEBUG
                            if ( b_pased &&
                                 debug_output().ph_dbg_draw_mask().test(
                                     phDbgDrawTriPoint ) ) {
                            ...
                            if ( contain_pos &&
                                 primitive.Proj( o1, tri.norm ) > -tri.dist )
                                intersect = true;
                        }
                    } else {
                        intersect = true;
            ...
            if ( !b_pushing && ( !intersect || no_last_pos ) )
                ret += primitive.Collide(
                    vertices[ 0 ], vertices[ 1 ], vertices[ 2 ], &tri, o1, o2,
                    3, CONTACT( contact, ret * skip ), skip );
            if ( no_last_pos )
    ...
    if ( intersect ) {
        if ( neg_depth < dInfinity ) {
            ...
            if ( no_last_pos )
                for ( i = pos_tries.begin(); pos_tries.end() != i; ++i ) {
                    ...
                    if ( TriContainPoint(
                             ...
                             ( dReal* )&V_array[ i->T->verts[ 2 ] ], i->norm,
                             i->side0, i->side1, p ) )
                        if ( negative_tri_set_ignored_by_positive_tri(
                                 neg_tri, *i, V_array ) ) {
                            include = false;
                            break;
                        }
            ...
            if ( include ) {
                ...
                // neg_tri_contains_point =
                // TriContainPoint(neg_vertices[0],neg_vertices[1],neg_vertices[2],
                // neg_tri.norm,neg_tri.side0, neg_tri.side1,p);
                if ( neg_tri_contains_point ) {
                    int bret = primitive.CollidePlain(
                        neg_tri.side0, neg_tri.side1, neg_tri.norm, neg_tri.T,
                        neg_tri.dist, o1, o2, flags, CONTACT( contact, 0 ),
                        skip );
                    *pushing_neg = !!bret;
                    if ( *pushing_neg )
                        ret = bret;
                } else
                    ret = SetBackTrajectoryCnt( p, last_pos, neg_tri, o1, o2,
                                                CONTACT( contact, 0 ) );
            }
        }
    ...
    if ( b_neg_depth < dInfinity ) {
        ...
        if ( no_last_pos )
            for ( i = pos_tries.begin(); pos_tries.end() != i; ++i ) {
                ...
                if ( negative_tri_set_ignored_by_positive_tri( b_neg_tri, *i,
                                                               V_array )
        ...
        if ( include ) {
            ...
            if ( ret < flags - 10 )
                bret = primitive.CollidePlain(
                    b_neg_tri.side0, b_neg_tri.side1, b_neg_tri.norm,
                    b_neg_tri.T, b_neg_tri.dist, o1, o2, flags,
