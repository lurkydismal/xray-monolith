...
public:
    ...
    //! Returns MIN(x, y, z);
    // IC	float		Min()
    // const		{ return MIN(x, MIN(y, z));
    // }
    //! Returns MAX(x, y, z);
    // IC	float		Max()
    // const		{ return MAX(x, MAX(y, z));
    // }
    //! TO BE DOCUMENTED
    // IC	Point&		Min(const Point& p)
    // { x = MIN(x, p.x); y = MIN(y, p.y); z = MIN(z, p.z);	return *this;
    // }
    //! TO BE DOCUMENTED
    // IC	Point&		Max(const Point& p)
    // { x = MAX(x, p.x); y = MAX(y, p.y); z = MAX(z, p.z);	return *this;
    // }
...
//! TO BE DOCUMENTED
ICF bool planeBoxOverlap_slow( const Point& normal,
                               const float d,
                               const Point& maxbox ) {
    Point vmin, vmax;
    if ( normal[ 0 ] > 0.0f ) {
...
ICF bool planeBoxOverlap( const Point& normal,
                          const float d,
                          const Point& maxbox ) {
    float norm_box_proj = _abs( maxbox.x * normal.x ) +
                          _abs( maxbox.y * normal.y ) +
                          _abs( maxbox.z * normal.z );
...
IC bool aabb_tri_aabb( Point center, Point extents, const Point* mLeafVerts ) {
    ...
    // ### could be precomputed since we use the same leaf triangle several
    // times
...
IC bool __aabb_tri( Point center, Point extents, const Point* mLeafVerts ) {
    ...
    // if(bClass3)
    {
        ...
        // - fabs precomputed (half less work, possible since extents are always
        // >0)
