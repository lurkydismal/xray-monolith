...
int dcTriListCollider::dSortedTriBox( const dReal* triSideAx0,
    ...
    dReal sidePr = dFabs( dDOT14( triAx, R + 0 ) * hside[ 0 ] ) +
...
#undef BAR
        ...
        // going along the two sides with the smallest projected length.

        //(@slipch) it is not perfectly right for triangle collision
        // because it need to check if additional points are in the triangle but
        // it seems cause no problem

#define FOO( i, j, op )                            \
    CONTACT( contact, i* skip )->pos[ 0 ] =        \
        pos[ 0 ] op 2.f * hside[ j ] * R[ 0 + j ]; \
    CONTACT( contact, i* skip )->pos[ 1 ] =        \
        pos[ 1 ] op 2.f * hside[ j ] * R[ 4 + j ]; \
    CONTACT( contact, i* skip )->pos[ 2 ] =        \
        pos[ 2 ] op 2.f * hside[ j ] * R[ 8 + j ];
...
/*
bool test_cross_side( dReal* outAx, dReal& outDepth, dReal *pos, dReal&
outSignum, int &code, u8 c,  const dReal* R, const dReal* hside, const dReal* p,
const dReal* triSideAx, const dReal* vax,   const dReal* vox  )
{
        ...
        if( depth_ax>depth_ox ){
                if(depth_ax>0.f){
                        if(depth_ax*1.05f<outDepth)
                                                {
                                                        dReal sgn=dist_ax<0.f ?
-1.f : 1.f; dReal sgn1=sgn*dDOT14(axis,R+ix1)<0.f ? -1.f : 1.f; dReal
sgn2=sgn*dDOT14(axis,R+ix2)<0.f ? -1.f : 1.f; dVector3 crpos; for(int
ii=0;ii<3;++ii)
                                                                crpos[ii]=p[ii]+R[ii*4+ix1]*hside[ix1]*sgn1+R[ii*4+ix2]*hside[ix2]*sgn2;

                                                        if(
dcTriListCollider::CrossProjLine14( vax, triSideAx, crpos,R+i, hside[i], pos ) )
                                                        {
                                                        outDepth=depth_ax;
                                                        outSignum=sgn;
                                                        outAx[0]=axis[0];
...
int dcTriListCollider::dTriBox( const dReal* v0,
                                const dReal* v1,
                                const dReal* v2,
                                Triangle* T,
                                dxGeom* o1,
    ...
    dVector3 triSideAx2 = { v0[ 0 ] - v2[ 0 ], v0[ 1 ] - v2[ 1 ],
                            v0[ 2 ] - v2[ 2 ] };
...
#define TEST( ax, ox, c )                                                     \
    for ( i = 0; i < 3; ++i ) {                                               \
        ...
        int ix1 = ( i + 1 ) % 3;                                              \
        int ix2 = ( i + 2 ) % 3;                                              \
        sidePr = dFabs( dDOT14( axis, R + ix1 ) * hside[ ix1 ] ) +            \
                 dFabs( dDOT14( axis, R + ix2 ) * hside[ ix2 ] );             \
                                                                              \
        dist##ax = dDOT( v##ax, axis ) - dDOT( p, axis );                     \
        dist##ox = dDOT( v##ox, axis ) - dDOT( p, axis );                     \
        ...
        if ( depth##ax > depth##ox ) {                                        \
            if ( depth##ax > 0.f ) {                                          \
                if ( depth##ax * 1.05f < outDepth ) {                         \
                    ...
                    dReal sgn1 =                                              \
                        sgn * dDOT14( axis, R + ix1 ) < 0.f ? -1.f : 1.f;     \
                    dReal sgn2 =                                              \
                        sgn * dDOT14( axis, R + ix2 ) < 0.f ? -1.f : 1.f;     \
                    for ( int ii = 0; ii < 3; ++ii )                          \
                        crpos[ ii ] =                                         \
                            p[ ii ] +                                         \
                            R[ ii * 4 + ix1 ] * hside[ ix1 ] * sgn1 +         \
                            R[ ii * 4 + ix2 ] * hside[ ix2 ] * sgn2;          \
                    if ( CrossProjLine14( v##ax, triSideAx##ax, crpos, R + i, \
                                          hside[ i ], pos ) ) {               \
                        outDepth = depth##ax;                                 \
                        signum = sgn;                                         \
                        outAx[ 0 ] = axis[ 0 ];                               \
    ...
    dVector3 crpos;
    // #define TEST(ax,ox,c)
    // test_cross_side( dReal* outAx, dReal& outDepth, dReal *pos, dReal&
    // outSignum, u8 &code, u8 c,  const dReal* R, const dReal* hside, const
    // dReal* p,  const dReal* triSideAx, const dReal* vax,   const dReal* vox )
    // if( test_cross_side
    // (outAx,outDepth,pos,signum,code,10,R,hside,p,triSideAx0,v0,v2) )
    //  return 0;
    TEST( 0, 2, 10 )
    TEST( 1, 0, 13 )
    TEST( 2, 1, 16 )
