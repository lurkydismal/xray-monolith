...
bool dcTriListCollider::circleLineIntersection( const dReal* cn,
                                                const dReal* cp,
                                                dReal r,
                                                const dReal* lv,
                                                const dReal* lp,
    ...
    if ( D < 0.f ) {
        ...
        dVector3 O1 = { lp[ 0 ] + lv[ 0 ] * t1, lp[ 1 ] + lv[ 1 ] * t1,
                        lp[ 2 ] + lv[ 2 ] * t1 };
        dVector3 O2 = { lp[ 0 ] + lv[ 0 ] * t2, lp[ 1 ] + lv[ 1 ] * t2,
                        lp[ 2 ] + lv[ 2 ] * t2 };
...
int dcTriListCollider::dSortedTriCyl( const dReal* triSideAx0,
    ...
    if ( code == 0 ) {
        ...
        if ( dFabs( Q2 ) > M_SQRT1_2 ) {
            ...
            CONTACT( contact, ret * skip )->pos[ 0 ] =
                contact->pos[ 0 ] -
                2.f * ( A2 > 0 ? hlz * R[ 1 ] : -hlz * R[ 1 ] );
            CONTACT( contact, ret * skip )->pos[ 1 ] =
                contact->pos[ 1 ] -
                2.f * ( A2 > 0 ? hlz * R[ 5 ] : -hlz * R[ 5 ] );
            CONTACT( contact, ret * skip )->pos[ 2 ] =
                contact->pos[ 2 ] -
                2.f * ( A2 > 0 ? hlz * R[ 9 ] : -hlz * R[ 9 ] );
...
IC bool dcTriListCollider::cylinderCrossesLine( const dReal* p,
                                                const dReal* R,
                                                dReal hlz,
                                                const dReal* v0,
                                                const dReal* v1,
...
int dcTriListCollider::dTriCyl( const dReal* v0,
                                const dReal* v1,
                                const dReal* v2,
                                Triangle* T,
                                dxGeom* o1,
    ...
    dVector3 triSideAx0 = {
        T->side0[ 0 ], T->side0[ 1 ],
        T->side0[ 2 ] }; //{v1[0]-v0[0],v1[1]-v0[1],v1[2]-v0[2]};
    dVector3 triSideAx1 = {
        T->side1[ 0 ], T->side1[ 1 ],
        T->side1[ 2 ] }; //{v2[0]-v1[0],v2[1]-v1[1],v2[2]-v1[2]};
    ...
    //////////////////////////////////////////////////////////////////////////////
    // cylinder axis - one of the triangle vertexes touches cylinder's flat
    // surface
    //////////////////////////////////////////////////////////////////////////////
...
#undef TEST
    ...
    dVector3 tpos, pos;
    if ( code > 3 )
        outDepth = pointDepth; // deepest vertex axis used if its depth less
                               // than outDepth
    // else{
    ...
    /////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////
    /// crosses between triangle sides and cylinder
    /// axis//////////////////////////
    /////////////////////////////////////////////////////////////////////////////
#define TEST( ax, nx, ox, c )                                             \
    if ( cylinderCrossesLine( p, R + 1, hlz, v##ax, v##nx, triSideAx##ax, \
                              tpos ) ) {                                  \
...
#define TEST( ax, nx, ox, c )                                                  \
    {                                                                          \
        ...
        cen[ 2 ] = p[ 2 ] - sign * R[ 9 ] * hlz;                               \
                                                                               \
        cs = circleLineIntersection( R + 1, cen, radius, triSideAx##ax, v##ax, \
                                     -sign, tpos );                            \
                                                                               \
        axis[ 0 ] = tpos[ 0 ] - cen[ 0 ];                                      \
        axis[ 1 ] = tpos[ 1 ] - cen[ 1 ];                                      \
...
#undef TEST
    ...
    if ( code == 0 ) {
        ...
        if ( dFabs( Q2 ) > M_SQRT1_2 ) {
            ...
            if ( CONTACT( contact, ret * skip )->depth > 0.f )
                if ( dDOT( cross0, CONTACT( contact, ret * skip )->pos ) - ds0 >
                         0.f &&
                     dDOT( cross1, CONTACT( contact, ret * skip )->pos ) - ds1 >
                         0.f &&
                     dDOT( cross2, CONTACT( contact, ret * skip )->pos ) - ds2 >
                         0.f )
                    ++ret;
            ...
            CONTACT( contact, ret * skip )->pos[ 0 ] =
                contact->pos[ 0 ] -
                2.f * ( Q2 > 0 ? hlz * R[ 1 ] : -hlz * R[ 1 ] );
            CONTACT( contact, ret * skip )->pos[ 1 ] =
                contact->pos[ 1 ] -
                2.f * ( Q2 > 0 ? hlz * R[ 5 ] : -hlz * R[ 5 ] );
            CONTACT( contact, ret * skip )->pos[ 2 ] =
                contact->pos[ 2 ] -
                2.f * ( Q2 > 0 ? hlz * R[ 9 ] : -hlz * R[ 9 ] );
            CONTACT( contact, ret * skip )->depth =
