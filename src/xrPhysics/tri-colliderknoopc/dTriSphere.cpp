...
inline dReal dcTriListCollider::FragmentonSphereTest( const dReal* center,
                                                      const dReal radius,
                                                      const dReal* pt1,
                                                      const dReal* pt2,
                                                      dReal* norm ) {
    dVector3 direction = { pt2[ 0 ] - pt1[ 0 ], pt2[ 1 ] - pt1[ 1 ],
                           pt2[ 2 ] - pt1[ 2 ] };
...
///////////////////////////////////////////////////////////////////////////////
IC bool dcTriListCollider::FragmentonSphereTest( const dReal* center,
                                                 const dReal radius,
                                                 const dReal* pt1,
                                                 const dReal* pt2,
                                                 dReal* norm,
                                                 dReal& depth ) {
    dVector3 V = { pt2[ 0 ] - pt1[ 0 ], pt2[ 1 ] - pt1[ 1 ],
                   pt2[ 2 ] - pt1[ 2 ] };
    dVector3 L = { pt1[ 0 ] - center[ 0 ], pt1[ 1 ] - center[ 1 ],
                   pt1[ 2 ] - center[ 2 ] };
    ...
    if ( t < 0.f || t > 1.f )
        return false;
    dVector3 Pc = { pt1[ 0 ] + t * V[ 0 ], pt1[ 1 ] + t * V[ 1 ],
                    pt1[ 2 ] + t * V[ 2 ] };
    dVector3 Dc = { center[ 0 ] - Pc[ 0 ], center[ 1 ] - Pc[ 1 ],
                    center[ 2 ] - Pc[ 2 ] };
...
IC bool dcTriListCollider::PointSphereTest( const dReal* center,
                                            const dReal radius,
                                            const dReal* pt,
                                            dReal* norm,
                                            dReal& depth ) {
...
/////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
int dcTriListCollider::dSortedTriSphere( const dReal* /**v1/**/,
                                         const dReal* /**v2/**/,
                                         const dReal* triAx,
                                         CDB::TRI* T,
                                         dReal dist,
    ...
    dVector3 ContactPos = { SphereCenter[ 0 ] - triAx[ 0 ] * SphereRadius,
                            SphereCenter[ 1 ] - triAx[ 1 ] * SphereRadius,
    ...
    if ( ContactDepth >= 0 ) {
        ...
        Contacts->g2 = Sphere;
        ( ( dxGeomUserData* )dGeomGetData( Sphere ) )->tri_material =
            T->material;
...
int dcTriListCollider::dTriSphere( const dReal* v0,
                                   const dReal* v1,
                                   const dReal* v2,
                                   Triangle* T,
                                   dxGeom* Sphere,
    ...
    if ( TriContainPoint( v0, v1, v2, triAx, triSideAx0, triSideAx1, pos ) ) {
        ...
        // dVector3 ContactPos={pos[0]-triAx[0]* radius,pos[1]-triAx[1]*
        // radius,pos[2]-triAx[2]* radius};
    } else {
        // VERIFY( g_pGameLevel );
        CDB::TRI* T_array = inl_ph_world().ObjectSpace().GetStaticTris();
        flags8& gl_state = gl_cl_tries_state[ I - B ];
        if ( gl_state.test( fl_engaged_s0 ) || gl_state.test( fl_engaged_s1 ) ||
             gl_state.test( fl_engaged_s2 ) )
            return 0;
        if ( FragmentonSphereTest( pos, radius, v0, v1, ContactNormal,
                                   Depth ) ) {
            SideToGlClTriState( T->T->verts[ 0 ], T->T->verts[ 1 ], T_array );
        } else if ( FragmentonSphereTest( pos, radius, v1, v2, ContactNormal,
                                          Depth ) ) {
            SideToGlClTriState( T->T->verts[ 1 ], T->T->verts[ 2 ], T_array );
        } else if ( FragmentonSphereTest( pos, radius, v2, v0, ContactNormal,
                                          Depth ) ) {
            SideToGlClTriState( T->T->verts[ 2 ], T->T->verts[ 0 ], T_array );
        } else {
            if ( gl_state.test( fl_engaged_v0 ) ||
                 gl_state.test( fl_engaged_v1 ) ||
                 gl_state.test( fl_engaged_v2 ) )
                return 0;
            if ( PointSphereTest( pos, radius, v0, ContactNormal, Depth ) ) {
                VxToGlClTriState( T->T->verts[ 0 ], T_array );
            } else if ( PointSphereTest( pos, radius, v1, ContactNormal,
                                         Depth ) ) {
                VxToGlClTriState( T->T->verts[ 1 ], T_array );
            } else if ( PointSphereTest( pos, radius, v2, ContactNormal,
                                         Depth ) ) {
