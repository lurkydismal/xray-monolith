...
void CPHContactBodyEffector::Init( dBodyID body,
...
void CPHContactBodyEffector::Merge( const dContact& contact,
                                    SGameMtl* material ) {
    m_recip_flotation =
        _max( 1.f - material->fFlotationFactor, m_recip_flotation );
...
void CPHContactBodyEffector::Apply() {
    ...
    if ( !fis_zero( l_air ) ) {
        dVector3 force = { -linear_velocity[ 0 ] * l_air,
                           -linear_velocity[ 1 ] * l_air,
                           -linear_velocity[ 2 ] * l_air, 0.f };
        ...
        if ( !m_material->Flags.is( SGameMtl::flPassable ) ) {
            ...
            dReal prg =
                1.f *
                dDOT( force,
                      norm ); //+dDOT(linear_velocity,norm)*m.mass/fixed_step
            force[ 0 ] -= prg * norm[ 0 ];
            force[ 1 ] -= prg * norm[ 1 ];
            force[ 2 ] -= prg * norm[ 2 ];
        }
