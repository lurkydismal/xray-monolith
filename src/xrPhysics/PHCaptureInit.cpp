...
///////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
bool can_capture( CPHCharacter* a_character,
    ...
    if ( !a_taget_object || !a_taget_object->ObjectPPhysicsShell() ||
         ...
         a_taget_object->IsInventoryItem() || !a_character ||
    ...
    // IKinematics* p_kinematics = smart_cast<IKinematics*>(
    // a_character->PhysicsRefObject()->ObjectVisual( ) );
    IKinematics* p_kinematics =
        a_character->PhysicsRefObject()->ObjectKinematics();
    VERIFY( p_kinematics );
...

{
    ...
    m_taget_element = m_taget_object->ObjectPPhysicsShell()->NearestToPoint(
        m_capture_bone->mTransform.c, cb );
    if ( !m_taget_element )
        return;
    Init();
}
...
void CPHCapture::Init() {
    ...
    // IKinematics* p_kinematics = smart_cast<IKinematics*>(
    // m_character->PhysicsRefObject()->ObjectVisual( ) );
    IKinematics* p_kinematics =
        m_character->PhysicsRefObject()->ObjectKinematics();
    VERIFY( p_kinematics );
    CInifile* ini = p_kinematics->LL_UserData();
    VERIFY( ini );
    ...
    ( m_character->PhysicsRefObject() )
        ->ObjectXFORM()
        .transform_tiny( capture_bone_position );
    ...
    float max_pull_force = ini->r_float( "capture", "pull_force" ); // pull
                                                                    // force
    m_pull_force = pool_force_factor * ph_world->Gravity() *
                   m_taget_element->PhysicsShell()->getMass();
    if ( m_pull_force > max_pull_force )
        m_pull_force = max_pull_force;
    float pulling_vel_scale = ini->r_float( "capture", "velocity_scale" ); //

    m_taget_element->set_DynamicLimits( default_l_limit * pulling_vel_scale,
                                        default_w_limit * pulling_vel_scale );
...
void CPHCapture::Release() {
    ...
    if ( e_state == cstPulling && m_taget_element &&
         !m_taget_object->ObjectGetDestroy() &&
