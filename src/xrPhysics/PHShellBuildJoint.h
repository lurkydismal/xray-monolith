...
IC void SetJointLimit( CPhysicsJoint& J,
                       const IBoneData& bone_data,
                       u8 limit_num,
                       u8 axis_num ) {
...
IC bool IsFreeRLimit( const IBoneData& bone_data, u8 limit_num ) {
    // const SJointIKData& joint_data	=	bone_data.get_IK_data();
    // const SJointLimit&	limit		=
    // joint_data.limits[limit_num];
    float lo = bone_data.lo_limit( limit_num ); // limit.x;
    float hi = bone_data.hi_limit( limit_num ); // limit.y;
    return !( hi - lo < M_PI * 2.f );
}

IC void SetJointRLimit( CPhysicsJoint& J,
                        const IBoneData& bone_data,
                        u8 limit_num,
                        u8 axis_num ) {
...
IC CPhysicsJoint* CtreateHinge( const IBoneData& bone_data,
                                u8 limit_num,
                                CPhysicsElement* root_e,
                                CPhysicsElement* E ) {
    u8 axis_num = 0;
...
IC CPhysicsJoint* CtreateFullControl( const IBoneData& bone_data,
                                      u8 limit_num[ 3 ],
                                      CPhysicsElement* root_e,
                                      CPhysicsElement* E ) {
    const SJointIKData& joint_data = bone_data.get_IK_data();
...
IC CPhysicsJoint* BuildWheelJoint( const IBoneData& bone_data,
                                   CPhysicsElement* root_e,
                                   CPhysicsElement* E ) {
    const SJointIKData& joint_data = bone_data.get_IK_data();
...
IC CPhysicsJoint* BuildSliderJoint( const IBoneData& bone_data,
                                    CPhysicsElement* root_e,
                                    CPhysicsElement* E ) {
    const SJointIKData& joint_data = bone_data.get_IK_data();
    ...
    J->SetLimits( joint_data.limits[ 0 ].limit.x,
                  joint_data.limits[ 0 ].limit.y, 0 );
...
IC CPhysicsJoint* BuildBallJoint( const IBoneData& bone_data,
                                  CPhysicsElement* root_e,
                                  CPhysicsElement* E ) {
    const SJointIKData& joint_data = bone_data.get_IK_data();
...
IC CPhysicsJoint* BuildGenericJoint( const IBoneData& bone_data,
                                     CPhysicsElement* root_e,
                                     CPhysicsElement* E ) {
    const SJointIKData& joint_data = bone_data.get_IK_data();

    bool eqx = !!fsimilar( joint_data.limits[ 0 ].limit.x,
                           joint_data.limits[ 0 ].limit.y ),
         eqy = !!fsimilar( joint_data.limits[ 1 ].limit.x,
                           joint_data.limits[ 1 ].limit.y ),
         eqz = !!fsimilar( joint_data.limits[ 2 ].limit.x,
                           joint_data.limits[ 2 ].limit.y );
...
IC CPhysicsJoint* BuildJoint( const IBoneData& bone_data,
                              CPhysicsElement* root_e,
                              CPhysicsElement* E ) {
    CPhysicsJoint* J = 0;
    const SJointIKData& joint_data = bone_data.get_IK_data();
