...
shell_root CPHShellSplitterHolder::SplitJoint( u16 aspl ) {
    ...
    u16 end_element =
        m_pShell->joints[ start_joint ]->JointDestroyInfo()->m_end_element;
    u16 end_joint =
        m_pShell->joints[ start_joint ]->JointDestroyInfo()->m_end_joint;

    shell_root ret =
        mk_pair( new_shell, ( m_pShell->joints[ start_joint ] )->BoneID() );
...
void CPHShellSplitterHolder::PassEndSplitters( const CShellSplitInfo& spl_inf,
                                               CPHShell* dest,
                                               u16 jt_add_shift,
                                               u16 el_add_shift ) {
    ...
    ELEMENT_I i_elem = source_elements.begin(),
              e_elem = source_elements.begin() + spl_inf.m_start_el_num;
    u16 shift_e = spl_inf.m_end_el_num - spl_inf.m_start_el_num;
    u16 shift_j = spl_inf.m_end_jt_num - spl_inf.m_start_jt_num;

    R_ASSERT2( source_elements.size() >= spl_inf.m_start_el_num &&
                   source_elements.size() >= spl_inf.m_end_el_num,
               "wrong spl_inf" );

    for ( ; i_elem != e_elem;
          ++i_elem ) // until start elem in both joint or elem split fractures
                     ...
                     // if grater then end elem in moving diapason
    {
        ...
        if ( !fracturesHolder )
            continue;
        FRACTURE_I f_i = fracturesHolder->m_fractures.begin(),
                   f_e = fracturesHolder->m_fractures.end();
        for ( ; f_i != f_e; ++f_i ) {
    ...
    if ( u16( -1 ) != spl_inf.m_start_jt_num ) {
        R_ASSERT2( source_joints.size() >= spl_inf.m_start_jt_num &&
                       source_joints.size() >= spl_inf.m_end_jt_num,
                   "wrong spl_inf" );
        e_joint = source_joints.begin() + spl_inf.m_start_jt_num;
        for ( ; i_joint != e_joint; i_joint++ ) {
            CPHJointDestroyInfo* jointDestroyInfo =
                ( *i_joint )->JointDestroyInfo();
            if ( !jointDestroyInfo )
                continue;
    ...
    for ( ; i_dest_elem != e_dest_elem; ++i_dest_elem ) {
        CPHFracturesHolder* fracturesHolder =
            ( *i_dest_elem )->FracturesHolder();
        if ( !fracturesHolder )
            continue;
        FRACTURE_I f_i = fracturesHolder->m_fractures.begin(),
                   f_e = fracturesHolder->m_fractures.end();
    ...
    dest_holder->m_splitters.insert( dest_holder->m_splitters.end(), i_from,
                                     i_to );
...
shell_root CPHShellSplitterHolder::ElementSingleSplit(
    const element_fracture& split_elem,
    ...
    // it is not right for multiple joints attached to the unsplited part
    // becource all these need to be reattached
    if ( start_joint != end_joint ) {
        JOINT_STORAGE& joints = m_pShell->joints;
        JOINT_I i = joints.begin() + start_joint,
                e = joints.begin() + end_joint;
        for ( ; i != e; ++i ) {
            CPHJoint* joint = ( *i );
            if ( joint->PFirst_element() == source_element ) {
                ...
                // CPhysicsElement*
                // el1=cast_PhysicsElement(split_elem.first),*el2=joint->PSecond_element();
                VERIFY( smart_cast< CPHElement* >( joint->PSecond_element() ) );

                CPHElement *el1 = ( split_elem.first ),
                           *el2 = static_cast< CPHElement* >(
                ...
                dVectorSet( const_cast< dReal* >( dBodyGetPosition( body1 ) ),
                            safe_pos1 );
                dVectorSet( const_cast< dReal* >( dBodyGetPosition( body2 ) ),
                            safe_pos2 );

                dQuaternionSet(
                    const_cast< dReal* >( dBodyGetQuaternion( body1 ) ),
                    safe_q1 );
                dQuaternionSet(
                    const_cast< dReal* >( dBodyGetQuaternion( body2 ) ),
                    safe_q2 );

                dBodySetPosition( body1, safe_pos1[ 0 ], safe_pos1[ 1 ],
                                  safe_pos1[ 2 ] );
                dBodySetPosition( body2, safe_pos2[ 0 ], safe_pos2[ 1 ],
                                  safe_pos2[ 2 ] );
    ...
    // the last new shell will have all splitted old elements end joints and one
    // new element reattached to old joint
    // m_splitters.erase(m_splitters.begin()+aspl);
    // now aspl points to the next splitter
    if ( ( split_elem.first )
             ->FracturesHolder() ) // if this element can be splitted add a
                                   // splitter for it
        new_shell_last_desc->AddSplitter( CPHShellSplitter::splElement, 0,
                                          u16( -1 ) ); //
    ...
    m_pShell->PassEndElements( split_elem.second.m_start_el_num,
                               split_elem.second.m_end_el_num,
                               new_shell_last_desc );

    m_pShell->PassEndJoints( split_elem.second.m_start_jt_num,
                             split_elem.second.m_end_jt_num,
                             new_shell_last_desc );
...
void CPHShellSplitterHolder::SplitElement( u16 aspl,
    ...
    for ( ; i != e; ++i ) {
        ...
        CPhysicsElement* ee =
            out_shels.back().first->get_ElementByStoreOrder( 0 );
        VERIFY( ee );
    ...
    if ( !E->FracturesHolder() )
        m_splitters.erase(
            spl_i ); // delete splitter if the element no longer have fractures
    else
...
void CPHShellSplitterHolder::SplitProcess( PHSHELL_PAIR_VECTOR& out_shels ) {
    ...
    // this based on that all childs in the bone hierarchy was added after their
    // parrent
...
void CPHShellSplitterHolder::InitNewShell( CPHShell* shell ) {
    ...
    if ( m_pShell->IsGroupObject() )
        CPHCollideValidator::RegisterObjToGroup(
            m_pShell->collide_bits(), *static_cast< CPHObject* >( shell ) );
}
    ...
    for ( ; i != e; ++i ) {
        switch ( i->m_type ) {
            case CPHShellSplitter::splElement: {
                CPHElement* element = m_pShell->elements[ i->m_element ];
                dBodyID body =
                    element->get_body(); //! element->EnabledStateOnStep()
                if ( !dBodyIsEnabled( body ) )
                    return; //
                i->m_breaked =
                    ( element->FracturesHolder()->PhDataUpdate( element ) ) ||
                    i->m_breaked;
                break;
            }
            case CPHShellSplitter::splJoint: {
...
void CPHShellSplitterHolder::AddSplitter( CPHShellSplitter::EType type,
                                          u16 element,
                                          u16 joint ) {
...
void CPHShellSplitterHolder::AddSplitter( CPHShellSplitter::EType type,
                                          ...
                                          u16 joint,
                                          u16 position ) {
    m_splitters.insert( m_splitters.begin() + position,
                        CPHShellSplitter( type, element, joint ) );
}

CPHShellSplitter::CPHShellSplitter( CPHShellSplitter::EType type,
                                    u16 element,
                                    u16 joint ) {
    m_breaked = false;
    m_type = type;
