...
bool test_sides( const Fvector& center,
                 const Fvector& side_dir,
                 const Fvector& fv_dir,
                 const Fvector& box,
                 int tri_id ) {
    Triangle tri;
    CalculateInitTriangle(
        inl_ph_world().ObjectSpace().GetStaticTris() + tri_id, tri,
    ...
    Fvector* verts = inl_ph_world().ObjectSpace().GetStaticVerts();
    {
        ...
        if ( sg_fvn * fvn * box.z + sg_sdn * sdn * box.x +
                 _abs( tri.norm[ 1 ] ) * box.y >
             _abs( dist ) )
            return false;
    }
    {
    ...
    for ( u8 i = 0; 3 > i; ++i ) {
        ...
        if ( sgn( ov_prg - c_prg ) != sg_dist ) {
            if ( _abs( fv_dir.dotproduct( crs ) ) * box.z +
                     _abs( side_dir.dotproduct( crs ) ) * box.x <
                 sg_dist * dist )
                return false;
        }
    }
...
void CPHSimpleCharacter::TestPathCallback( bool& do_colide,
                                           bool bo1,
                                           dContact& c,
                                           SGameMtl* /*material_1*/,
                                           SGameMtl* /*material_2*/ ) {
    ...
    if ( bo1 ) {
        ch = static_cast< CPHSimpleCharacter* >(
            retrieveGeomUserData( c.geom.g1 )->ph_object );
    } else {
        ch = static_cast< CPHSimpleCharacter* >(
            retrieveGeomUserData( c.geom.g2 )->ph_object );
    }
    VERIFY( ch );
...
void CPHSimpleCharacter::Destroy() {
    ...
    R_ASSERT2( !ph_world->Processing(),
               "can not deactivate physics character shell during physics "
               "processing!!!" ); // if(ph_world)
    R_ASSERT2(
        !ph_world->IsFreezed(),
        "can not deactivate physics character when ph world is freezed!!!" );
...
void CPHSimpleCharacter::ApplyImpulse( const Fvector& dir, dReal P ) {
    ...
    dBodySetForce( m_body, m_ext_imulse.x * P / fixed_step,
                   m_ext_imulse.y * P / fixed_step,
...
void CPHSimpleCharacter::PhDataUpdate( dReal /**step/**/ ) {
    ...
    dReal linear_velocity_mag =
        _sqrt( dDOT( linear_velocity, linear_velocity ) );
    ...
    if ( !fis_zero( l_air ) )
        dBodyAddForce( m_body, -linear_velocity[ 0 ] * l_air,
    ...
    VERIFY2( dV_valid( dBodyGetPosition( m_body ) ),
             "WRONG BODYSTATE IN PhDataUpdate" );
    if ( PhOutOfBoundaries( cast_fv( dBodyGetPosition( m_body ) ) ) )
        Disable();
    VERIFY_BOUNDARIES( cast_fv( dBodyGetPosition( m_body ) ), phBoundaries,
                       PhysicsRefObject() );
...
void CPHSimpleCharacter::PhTune( dReal step ) {
    ...
    if ( debug_output().ph_dbg_draw_mask().test( phDbgCharacterControl ) ) {
        if ( b_air_contact_state )
            debug_output().DBG_DrawPoint( cast_fv( dBodyGetPosition( m_body ) ),
                                          m_radius,
                                          D3DCOLOR_XRGB( 255, 0, 0 ) );
    }
#endif
    const bool b_good_graund =
        b_valide_ground_contact && m_ground_contact_normal[ 1 ] > M_SQRT1_2;
    ...
    if ( ( ud->pushing_neg || ud->pushing_b_neg ) && !b_death_pos ) {
        ...
        // #ifdef DEBUG
        //		Msg("death pos
        //%f2.2,%f2.2,%f2.2",ud->last_pos[0],ud->last_pos[1],ud->last_pos[2]);
        // #endif
        Fvector pos;
        ...
        if ( !ud->pushing_b_neg ) {
            // Fvector
            // movement;movement.sub(cast_fv(dGeomGetPosition(m_wheel)),cast_fv(ud->last_pos));
    ...
    if ( !b_on_ground && !m_elevator_state.ClimbingState() ) {
        ...
        if ( dDOT( dif, dif ) > LOSE_CONTROL_DISTANCE * LOSE_CONTROL_DISTANCE &&
             dFabs( dif[ 1 ] ) > 0.1 ) {
            b_lose_control = true;
    ...
    if ( b_jump ) {
        b_lose_control = true;
        b_depart_control = true;
        dBodySetLinearVel( m_body, m_jump_accel.x, m_jump_accel.y,
                           m_jump_accel.z ); // vel[1]+
    ...
    b_lose_ground = !( b_good_graund || m_elevator_state.ClimbingState() ) ||
                    b_lose_control;
    ...
    if ( is_control ) {
        ...
        dBodyAddForce( m_body, m_control_force[ 0 ], m_control_force[ 1 ],
                       m_control_force[ 2 ] ); //+2.f*9.8f*70.f
        if ( !b_lose_control || b_clamb_jump ) //)&&!b_external_impulse
            dBodyAddForce(
                m_body,
                -sidedir[ 0 ] * vProj * ( 500.f + 200.f * b_clamb_jump ) *
                    m_friction_factor,
                -m.mass * ( 50.f ) *
                    ( !b_lose_control &&
                      !( is_contact || ( b_any_contacts ) ) ), //&&!b_climb
                -sidedir[ 2 ] * vProj * ( 500.f + 200.f * b_clamb_jump ) *
                    m_friction_factor );
#ifdef DEBUG
        if ( debug_output().ph_dbg_draw_mask().test( phDbgCharacterControl ) ) {
            ...
            debug_output().DBG_DrawLine(
                ...
                Fvector().add( cast_fv( dBodyGetPosition( m_body ) ),
                               Fvector().mul( cast_fv( sidedir ), 1.f ) ),
                D3DCOLOR_XRGB( 0, 0, 255 ) );
            debug_output().DBG_DrawLine(
                cast_fv( dBodyGetPosition( m_body ) ),
                Fvector().add(
                    cast_fv( dBodyGetPosition( m_body ) ),
                    Fvector().mul( cast_fv( m_control_force ), 1.f / 1000.f ) ),
                D3DCOLOR_XRGB( 0, 0, 255 ) );
            debug_output().DBG_DrawLine(
                ...
                Fvector().add(
                    Fvector().add( cast_fv( dBodyGetPosition( m_body ) ),
                                   dipsp ),
                    Fvector().mul( cast_fv( dBodyGetForce( m_body ) ),
                                   1.f / 1000.f ) ),
                D3DCOLOR_XRGB( 255, 0, 0 ) );
        }
#endif
    ...
    if ( b_jumping ) {
        ...
        dReal proj =
            m_acceleration.x * chVel[ 0 ] + m_acceleration.z * chVel[ 2 ];
        ...
        dReal amag = _sqrt( m_acceleration.x * m_acceleration.x +
                            m_acceleration.z * m_acceleration.z );
        if ( amag > 0.f ) {
            if ( dif[ 0 ] * m_acceleration.x / amag +
                     dif[ 2 ] * m_acceleration.z / amag <
                 0.3f ) {
...
bool CPHSimpleCharacter::ValidateWalkOnObject() {
    ...
    if ( b_clamb_jump ) {
        ...
        if ( //! b_valide_wall_contact||
             //(dDOT(dif,dif)>CLAMB_DISTANCE*CLAMB_DISTANCE))	//
                          ...
                          // dDOT(m_wall_contact_normal,m_ground_contact_normal)<.9f
    {
        ...
        if ( ( ( m_wall_contact_position[ 0 ] -
                 m_ground_contact_position[ 0 ] ) *
                   m_control_force[ 0 ] +
               ( m_wall_contact_position[ 2 ] -
                 m_ground_contact_position[ 2 ] ) *
                   m_control_force[ 2 ] ) > 0.05f &&
             m_wall_contact_position[ 1 ] - m_ground_contact_position[ 1 ] >
                 0.01f )
            b_clamb_jump = true;
    }

    if ( b_valide_wall_contact && ( m_contact_count > 1 ) && b_clamb_jump )
        if ( dFabs( ( m_wall_contact_position[ 0 ] -
                      m_ground_contact_position[ 0 ] ) + //*m_control_force[0]
                    ( m_wall_contact_position[ 2 ] -
                      m_ground_contact_position[ 2 ] ) ) >
                 0.05f && // 0.01f//*m_control_force[2]
             m_wall_contact_position[ 1 ] - m_ground_contact_position[ 1 ] >
                 0.01f )
...
bool CPHSimpleCharacter::ValidateWalkOnMesh() {
    ...
    for ( CDB::RESULT* Res = R_begin; Res != R_end; ++Res ) {
        ...
        Point vertices[ 3 ] = { Point( ( dReal* )&Res->verts[ 0 ] ),
                                Point( ( dReal* )&Res->verts[ 1 ] ),
                                Point( ( dReal* )&Res->verts[ 2 ] ) };
        if ( __aabb_tri( Point( ( float* )&center_forbid ),
                         Point( ( float* )&AABB_forbid ), vertices ) ) {
            if ( test_sides( center_forbid, sd_dir, accel, obb_fb, Res->id ) ) {
#ifdef DEBUG
                if ( debug_output().ph_dbg_draw_mask().test(
                         phDbgCharacterControl ) ) {
                    debug_output().DBG_DrawTri( Res,
                                                D3DCOLOR_XRGB( 255, 0, 0 ) );
                }
#endif
                b_side_contact = true;
    ...
    for ( CDB::RESULT* Res = R_begin; Res != R_end; ++Res ) {
        ...
        Point vertices[ 3 ] = { Point( ( dReal* )&Res->verts[ 0 ] ),
                                Point( ( dReal* )&Res->verts[ 1 ] ),
                                Point( ( dReal* )&Res->verts[ 2 ] ) };
        if ( __aabb_tri( Point( ( float* )&center ), Point( ( float* )&AABB ),
                         vertices ) ) {
...
void CPHSimpleCharacter::ApplyAcceleration() {
    ...
    if ( m_elevator_state.Active() ) {
        ...
        if ( m_elevator_state.ClimbingState() ) {
            ...
            if ( m_elevator_state.GetControlDir(
                     *( Fvector* )m_control_force ) ) {
                dVectorMul( m_control_force,
                            m_friction_factor * m.mass * pull_force * 2.f );
                return;
            }
        }
    }
    ...
    if ( !m_elevator_state.ClimbingState() && b_clamb_jump ) {
        ...
        m_control_force[ 0 ] = m_control_force[ 0 ] * accel[ 0 ] >= 0.f
                                   ? m_control_force[ 0 ]
                                   : -m_control_force[ 0 ];
        m_control_force[ 2 ] = m_control_force[ 2 ] * accel[ 2 ] >= 0.f
                                   ? m_control_force[ 2 ]
                                   : -m_control_force[ 2 ];
    }
...
void CPHSimpleCharacter::GetPosition( Fvector& vpos ) {
    if ( !b_exist ) {
        vpos.set( m_safe_position[ 0 ], m_safe_position[ 1 ] - m_radius,
                  m_safe_position[ 2 ] );
    } else {
...
void CPHSimpleCharacter::GetVelocity( Fvector& vvel ) const {
    if ( !b_exist ) {
        vvel.set( m_safe_velocity[ 0 ], m_safe_velocity[ 1 ],
                  m_safe_velocity[ 2 ] );
        return;
    }
...
void CPHSimpleCharacter::SetPhysicsRefObject(
    IPhysicsShellHolder* ref_object ) {
...
void CPHSimpleCharacter::SafeAndLimitVelocity() {
    ...
    if ( dV_valid( linear_velocity ) ) {
        ...
        if ( b_external_impulse ) {
            ...
            float ll_limit =
                m_ext_imulse.dotproduct( cast_fv( linear_velocity ) ) * 10.f /
                fixed_step;
            if ( sq_mag > EPS_L ) {
                Fvector acc;
                acc.set(
                    Fvector().mul( m_acceleration, 1.f / _sqrt( sq_mag ) ) );
                Fvector vll;
        ...
        if ( mag > l_limit ) {
            ...
            if ( !fis_zero( l_limit ) ) {
                ...
                if ( b_lose_ground && linear_velocity[ 1 ] < 0.f &&
                     linear_velocity[ 1 ] > -default_l_limit )
                    dBodySetLinearVel( m_body, linear_velocity[ 0 ] / f,
                                       linear_velocity[ 1 ],
                                       linear_velocity[ 2 ] / f ); /// f
                else
                ...
                if ( is_control && !b_lose_control ) {
                    dBodySetPosition( m_body,
                                      m_safe_position[ 0 ] +
                                          linear_velocity[ 0 ] * fixed_step,
                                      m_safe_position[ 1 ] +
                                          linear_velocity[ 1 ] * fixed_step,
                                      m_safe_position[ 2 ] +
                                          linear_velocity[ 2 ] * fixed_step );
                    VERIFY_BOUNDARIES( cast_fv( dBodyGetPosition( m_body ) ),
        ...
        dBodySetLinearVel( m_body, m_safe_velocity[ 0 ], m_safe_velocity[ 1 ],
                           m_safe_velocity[ 2 ] );
    }
...
void CPHSimpleCharacter::SetObjectContactCallback(
...
void CPHSimpleCharacter::AddObjectContactCallback(
    ObjectContactCallbackFun* callback ) {
    VERIFY( b_exist );
...
void CPHSimpleCharacter::RemoveObjectContactCallback(
    ObjectContactCallbackFun* callback ) {
...
void CPHSimpleCharacter::SetWheelContactCallback(
    ObjectContactCallbackFun* callback ) {
    VERIFY( b_exist );
    dGeomUserDataSetObjectContactCallback( m_wheel, callback );
}

void CPHSimpleCharacter::SetStaticContactCallBack(
...
u16 CPHSimpleCharacter::RetriveContactBone() {
    ...
    collide::ray_defs Q(
        m_collision_damage_info.HitPos(), dir, m_radius,
    ...
    if ( inl_ph_world().ObjectSpace().RayQuery(
             RQR, m_phys_ref_object->ObjectCollisionModel(), Q ) ) {
        ...
        // IKinematics*
        // K=smart_cast<IKinematics*>(m_phys_ref_object->ObjectVisual());
        ...
        pos_in_object.sub( m_collision_damage_info.HitPos(),
                           m_phys_ref_object->ObjectPosition() );
        // vector from object center to contact position currently in global
        // frame
        Fmatrix object_form;
        object_form.set( m_phys_ref_object->ObjectXFORM() );
        object_form.transpose();
        object_form.transform_dir( pos_in_object );
        // project pos_in_object on object axes now it is position of contact in
        // object frame
...
void CPHSimpleCharacter::InitContact( dContact* c,
    ...
    bool bo1 = ( g1 == m_wheel ) || g1 == m_cap_transform ||
               g1 == m_shell_transform || g1 == m_hat_transform;

    // SGameMtl*
    // tri_material=GMLibrary().GetMaterialByIdx((u16)c->surface.mode);
    ...
    u16 foot_material_idx =
        ( ( dxGeomUserData* )dGeomGetData( m_wheel ) )->tri_material;
    ...
    if ( g1 == m_hat_transform || g2 == m_hat_transform ) {
        b_side_contact = true;
        MulSprDmp( c->surface.soft_cfm, c->surface.soft_erp, spring_rate,
                   dumping_rate );
        c->surface.mu = 0.00f;
    }
    ...
    if ( object ) {
        ...
        if ( g1 == m_wheel || g2 == m_wheel ) {
            dxGeomUserData* ud = bo1 ? retrieveGeomUserData( c->geom.g2 )
                                     : retrieveGeomUserData( c->geom.g1 );
    ...
    if ( bo1 ) {
        if ( normal[ 1 ] > m_ground_contact_normal[ 1 ] ||
             !b_valide_ground_contact ) //
        {
            dVectorSet( m_ground_contact_normal, normal );
            dVectorSet( m_ground_contact_position, pos );
            b_valide_ground_contact = true;
        }
        if ( dXZDot( normal, cast_fp( m_acceleration ) ) <
                 dXZDot( m_wall_contact_normal, cast_fp( m_acceleration ) ) ||
    ...
    float soft_param =
        dumping_rate +
        normal[ 1 ] *
            ( 1.f - dumping_rate ); //=(1.f-normal[1])*dumping_rate +normal[1]
    if ( is_control ) {
...
void CPHSimpleCharacter::FootProcess( dContact* c, bool& do_collide, bool bo ) {
    ...
    if ( m_elevator_state.ClimbingState() || !is_control || !b_clamb_jump ||
         b_was_side_contact || b_side_contact || b_jumping || b_jump ) {
...
u16 CPHSimpleCharacter::DamageInitiatorID() const {
    ...
    if ( m_collision_damage_info.m_obj_id != u16( -1 ) ) {
        CObject* obj = inl_ph_world().LevelObjects().net_Find(
            m_collision_damage_info.m_obj_id );
...
void CPHSimpleCharacter::SCollisionDamageInfo::HitDir( Fvector& dir ) const {
    dir.set( m_damege_contact.geom.normal[ 0 ] * m_dmc_signum,
             m_damege_contact.geom.normal[ 1 ] * m_dmc_signum,
...
//=static_cast<IPhysicsShellHolder*>(Level().Objects.net_Find(m_obj_id));
...
void CPHSimpleCharacter::TestRestrictorContactCallbackFun(
    ...
    if ( obj_data->ph_object->CastType() != tpCharacter )
        return;
    CPHActorCharacter* actor_character =
        ( static_cast< CPHCharacter* >( obj_data->ph_object ) )
            ->CastActorCharacter();
    if ( !actor_character )
        return;
    CPHSimpleCharacter* ch_this = static_cast< CPHSimpleCharacter* >(
        retrieveGeomUserData( g_this )->ph_object );
    VERIFY( ch_this );
...
IC bool valide_res( u16& res_material_idx, const collide::rq_result& R ) {
    if ( !R.O ) {
        CDB::TRI* tri =
            inl_ph_world().ObjectSpace().GetStaticTris() + R.element;
        VERIFY( tri );
...
bool PickMaterial( u16& res_material_idx,
                   const Fvector& pos_,
                   const Fvector& dir_,
                   float range_,
    ...
    while ( inl_ph_world().ObjectSpace().RayPick(
        pos, dir, range, collide::rqtBoth, R, ignore_object ) ) {
...
void CPHSimpleCharacter::update_last_material() {
    ...
    if ( m_last_picked_material != GAMEMTL_NONE_IDX &&
         pos.similar( m_last_environment_update, material_update_tolerance ) ) {
    ...
    if ( PickMaterial( new_material, pos, Fvector().set( 0, -1, 0 ),
                       material_pick_dist + material_pick_upset,
                       smart_cast< CObject* >( PhysicsRefObject() ) ) ) {
...
void CPHSimpleCharacter::Collide() {
    ...
    inherited::Collide();
    if ( injuriousMaterialIDX == GAMEMTL_NONE_IDX &&
         ( *p_lastMaterialIDX ) != GAMEMTL_NONE_IDX &&
         GMLibrary()
             .GetMaterialByIdx( *p_lastMaterialIDX )
             ->Flags.test( SGameMtl::flInjurious ) )
