...
// #endif
CPHActorCharacter::CPHActorCharacter( bool single_game )
    : b_single_game( single_game ) {
    SetRestrictionType( rtActor );

    // std::fill(m_restrictors_index,m_restrictors_index+CPHCharacter::rtNone,end(m_restrictors));
    // m_restrictors_index[CPHCharacter::rtStalker]
    // =begin(m_restrictors)+0;
    // m_restrictors_index[CPHCharacter::rtMonsterMedium]
    // =begin(m_restrictors)+1;
...
void SPHCharacterRestrictor::Create( CPHCharacter* ch, dVector3 sizes ) {
    ...
    switch ( m_type ) {
        case rtStalker:
            static_cast< CPHActorCharacter::stalker_restrictor* >( this )
                ->Create( ch, sizes );
            break;
        case rtStalkerSmall:
            static_cast< CPHActorCharacter::stalker_small_restrictor* >( this )
                ->Create( ch, sizes );
            break;
        case rtMonsterMedium:
            static_cast< CPHActorCharacter::medium_monster_restrictor* >( this )
                ->Create( ch, sizes );
            break;
        default:
            NODEFAULT;
...
void SPHCharacterRestrictor::SetPhysicsRefObject(
...
void CPHActorCharacter::SetAcceleration( Fvector accel ) {
    ...
    if ( !cur_a.similar( input_a, 0.05f ) ||
         !fis_zero( input_mug - cur_mug, 0.5f ) )
...
void CPHActorCharacter::Jump( const Fvector& accel ) {
    ...
    if ( CanJump() ) {
        ...
        if ( m_elevator_state.ClimbingState() ) {
            ...
            m_jump_accel.set( vel[ 0 ] * JUMP_INCREASE_VELOCITY_RATE +
                                  m_acceleration.x / amag * 0.2f,
                              jump_up_velocity,
                              vel[ 2 ] * JUMP_INCREASE_VELOCITY_RATE +
                                  m_acceleration.z / amag * 0.2f );
        }
        Enable();
    }
}

void CPHActorCharacter::SetObjectContactCallback(
...
void CPHActorCharacter::InitContact( dContact* c,
    ...
    RESTRICTOR_I r =
        std::find_if( begin( m_restrictors ), end( m_restrictors ), fp );
    ...
    if ( ( material_1 &&
           material_1->Flags.test( SGameMtl::flActorObstacle ) ) ||
    ...
    if ( b_single_game ) {
        if ( b_restrictor ) {
            ...
            inherited::InitContact( c, do_collide, material_idx_1,
                                    material_idx_2 );
        if ( b_restrictor && do_collide &&
             !( b1 ? static_cast< CPHCharacter* >(
                         retrieveGeomUserData( c->geom.g2 )->ph_object )
                         ->ActorMovable()
                   : static_cast< CPHCharacter* >(
        ...
        if ( D1 && D2 ) {
            ...
            if ( A1 && A2 && A1->IsActor() && A2->IsActor() ) {
                do_collide = do_collide && !b_restrictor &&
                             ( A1->ObjectPPhysicsShell() == 0 ) ==
                                 ( A2->ObjectPPhysicsShell() == 0 );
                c->surface.mu = 1.f;
            }
        }
...
void CPHActorCharacter::ChooseRestrictionType( ERestrictionType my_type,
                                               float my_depth,
                                               CPHCharacter* ch ) {
    if ( my_type != rtStalker || ( ch->RestrictionType() != rtStalker &&
                                   ch->RestrictionType() != rtStalkerSmall ) )
        return;
...
#ifdef DEBUG
                if ( debug_output().ph_dbg_draw_mask1().test(
