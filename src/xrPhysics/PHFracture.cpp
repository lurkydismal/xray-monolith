...
element_fracture CPHFracturesHolder::SplitFromEnd( CPHElement* element,
                                                   u16 fracture ) {
    ...
    const CBoneInstance& new_bi =
        pKinematics->LL_GetBoneInstance( new_element->m_SelfID );
    const CBoneInstance& old_bi =
        pKinematics->LL_GetBoneInstance( element->m_SelfID );
    ...
    element_fracture ret =
        mk_pair( new_element, ( CShellSplitInfo )( *fract_i ) );

    if ( m_fractures.size() - fracture > 0 ) {
        if ( new_element->m_fratures_holder ==
             NULL ) // create fractures holder if it was not created before
        {
...
void CPHFracturesHolder::PassEndFractures( u16 from, CPHElement* dest ) {
    FRACTURE_I i = m_fractures.begin(), i_from = m_fractures.begin() + from,
               e = m_fractures.end();
    ...
    for ( ; i != i_from;
          ++i ) // correct end geoms for fractures leaved in source
    {
    ...
    // these to be passed
    for ( ; i != e;
          i++ ) // itterate antil a fracture where geom num > end geom num
    {
        u16& cur_end_geom = i->m_end_geom_num;
    ...
    if ( i_from + 1 != i_to ) // insure it!!
    {
        ...
        dest_fract_holder->m_fractures.insert(
            dest_fract_holder->m_fractures.end(), i_from + 1, i_to );
...
void CPHFracturesHolder::SplitProcess( CPHElement* element,
                                       ELEMENT_PAIR_VECTOR& new_elements ) {
...
void CPHFracturesHolder::PhTune( dBodyID body ) {
    // iterate through all body's joints and set joints feedbacks where is not
    // already set contact feedbacks stored in global storage - ContactFeedBacks
    // wich cleared on each step breacable joints already has their feedbacks,
    // feedbacks for rest noncontact joints stored in m_feedbacks in runtime in
    // this function and and killed by destructor
...
void CPHFracturesHolder::AddImpact( const Fvector& force,
                                    const Fvector& point,
                                    u16 id ) {
...
void CPHFracturesHolder::DistributeAdditionalMass( u16 geom_num,
                                                   const dMass& m ) {
    FRACTURE_I f_i = m_fractures.begin(), f_e = m_fractures.end();
    for ( ; f_i != f_e; ++f_i ) {
        R_ASSERT2( u16( -1 ) != f_i->m_start_geom_num,
                   "fracture does not initialized!" );
    ...
    for ( ; f_i != f_e; ++f_i ) {
        ...
        if ( start_geom > f_i->m_start_geom_num ) {
            if ( end_geom <= f_i->m_end_geom_num )
                f_i->MassSubFromSecond(
                    second_mass ); // tag fracture is in current
            else {
                R_ASSERT2( start_geom >= f_i->m_end_geom_num,
                           "Odd fracture!!!" );
                f_i->MassSubFromFirst(
                    second_mass ); // tag fracture is ouside current
            }
        } else {
            if ( end_geom >= f_i->m_end_geom_num )
                f_i->MassSubFromFirst(
                    first_mass ); // current fracture is in tag
            else {
                R_ASSERT2( end_geom <= f_i->m_start_geom_num,
                           "Odd fracture!!!" );
                f_i->MassSubFromFirst(
                    second_mass ); // tag fracture is ouside current
            }
...
bool CPHFracture::Update( CPHElement* element ) {
    ...
    Fvector second_part_force, first_part_force, second_part_torque,
        first_part_torque;
    ...
    const Fvector& body_global_pos =
        *( const Fvector* )dBodyGetPosition( body );
    ...
    // both parts have eqiual start angular vel same as have body so we ignore
    // it
    ...
    dMULTIPLY0_331( ( float* )&break_torque, glInvI,
                    ( float* )&first_part_torque );
    ...
    // break_torque.sub(vtemp);
#ifdef DBG_BREAK
    float btm_dbg = break_torque.magnitude() * ph_console::phBreakCommonFactor /
                    torque_factor;
#endif
    if ( break_torque.magnitude() * ph_console::phBreakCommonFactor >
         m_break_torque * torque_factor ) {
    ...
    break_force.mul(
        1.f / element->getMass() ); // element->getMass()//body->mass.mass
...
#ifdef DBG_BREAK
    Msg( "bone_id %d break_torque - %f(max %f) break_force %f (max %f) breaked "
         "%d",
         m_bone_id, btm_dbg, m_break_torque, bfm, m_break_force, m_breaked );
#endif
    return m_breaked;
}
