...
bool CPHAICharacter::TryPosition( Fvector pos, bool exact_state ) {
    ...
    if ( fis_zero( disp_mag ) ||
         fis_zero( inl_ph_world().Device().fTimeDelta ) )
        return true;
    ...
    SetPosition( pos_new );
    m_last_move.sub( pos_new, current_pos )
        .mul( 1.f / inl_ph_world().Device().fTimeDelta );
...
void CPHAICharacter::InitContact( dContact* c,
    ...
    if ( D1 && D2 && D1->ph_object && D2->ph_object &&
         D1->ph_object->CastType() == tpCharacter &&
