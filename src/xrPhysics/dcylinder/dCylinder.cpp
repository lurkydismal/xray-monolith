...
/////////////////////////////////////////////////////////////////////////////////////////////////
inline bool circleIntersection( const dReal* n1,
                                const dReal* cp1,
                                dReal r1,
                                const dReal* n2,
                                const dReal* cp2,
    ...
    dVector3 lp = { p1 * n1[ 0 ] + p2 * n2[ 0 ], p1 * n1[ 4 ] + p2 * n2[ 4 ],
                    p1 * n1[ 8 ] + p2 * n2[ 8 ] };
    dVector3 n;
    dCROSS144( n, =, n1, n2 );
    dVector3 LC1 = { lp[ 0 ] - cp1[ 0 ], lp[ 1 ] - cp1[ 1 ],
                     lp[ 2 ] - cp1[ 2 ] };
    dVector3 LC2 = { lp[ 0 ] - cp2[ 0 ], lp[ 1 ] - cp2[ 1 ],
                     lp[ 2 ] - cp2[ 2 ] };
    ...
    dVector3 O1 = { lp[ 0 ] + n[ 0 ] * t1, lp[ 1 ] + n[ 1 ] * t1,
                    lp[ 2 ] + n[ 2 ] * t1 };
    dVector3 O2 = { lp[ 0 ] + n[ 0 ] * t2, lp[ 1 ] + n[ 1 ] * t2,
                    lp[ 2 ] + n[ 2 ] * t2 };

    dVector3 O3 = { lp[ 0 ] + n[ 0 ] * t3, lp[ 1 ] + n[ 1 ] * t3,
                    lp[ 2 ] + n[ 2 ] * t3 };
    dVector3 O4 = { lp[ 0 ] + n[ 0 ] * t4, lp[ 1 ] + n[ 1 ] * t4,
                    lp[ 2 ] + n[ 2 ] * t4 };
...
void lineClosestApproach( const dVector3 pa,
                          const dVector3 ua,
                          const dVector3 pb,
                          const dVector3 ub,
                          dReal* alpha,
...
extern "C" int dCylBox( const dVector3 p1,
                        const dMatrix3 R1,
                        const dReal radius,
                        const dReal lz,
                        const dVector3 p2,
    ...
    dReal B1, B2, B3, R11, R12, R13, R21, R22, R23, R31, R32, R33, Q11, Q12,
        Q13, Q21, Q22, Q23, Q31, Q32, Q33, s, s2, l, sQ21, sQ22, sQ23;
    int i, invert_normal;
...
#define TEST( expr1, expr2, norm, cc )     \
    ...
    *code = 0;
    ...
    // there is two ways to compute sQ: sQ21=dSqrt(1.f-Q21*Q21); or
    // sQ21=dSqrt(Q23*Q23+Q22*Q22); if we did not need Q23 and Q22 the first way
    // might be used to quiken the routine but then it need to check if
    // Q21<=1.f, becouse it may slightly exeed 1.f.
...
#define TEST( expr1, expr2, n1, n2, n3, cc ) \
    ...
    // separating axis is a normal to the cylinder axis passing across the
    // nearest box vertex
    ...
    dVector3 tAx, Ax, pb;
    {
        ...
        // building axis which is normal to cylinder ax to the nearest box
        // vertex
    ...
    TEST( p[ 0 ] * Ax[ 0 ] + p[ 1 ] * Ax[ 1 ] + p[ 2 ] * Ax[ 2 ],
          ( radius + boxProj ), Ax[ 0 ], Ax[ 1 ], Ax[ 2 ], 4 );

    // next three test used to handle collisions between cylinder circles and
    // box ages
    ...
    // making perpendicular to tAx lying in the plane which is normal to the
    // cylinder axis it is tangent in the point where projection of tAx on
    // cylinder's ring intersect edge circle
    ...
    TEST( p[ 0 ] * Ax[ 0 ] + p[ 1 ] * Ax[ 1 ] + p[ 2 ] * Ax[ 2 ],
          ( _sin * radius + _cos * hlz + boxProj ), Ax[ 0 ], Ax[ 1 ], Ax[ 2 ],
          5 );
...
#define TEST( expr1, expr2, n1, n2, n3, cc )                          \
    ...
    TEST( pp[ 0 ] * R31 - pp[ 2 ] * R11, ( radius + B2 * Q23 + B3 * Q22 ), R31,
          0, -R11, 8 );
    TEST( pp[ 0 ] * R32 - pp[ 2 ] * R12, ( radius + B1 * Q23 + B3 * Q21 ), R32,
          0, -R12, 9 );
    TEST( pp[ 0 ] * R33 - pp[ 2 ] * R13, ( radius + B1 * Q22 + B2 * Q21 ), R33,
          0, -R13, 10 );
    ...
    if ( *code == 0 ) {
        ...
        if ( *code < 4 ) {
            ...
            if ( sQ2 < M_SQRT1_2 ) {
                ...
                CONTACT( contact, ret * skip )->pos[ 0 ] =
                    center[ 0 ] + A1 * R1[ 0 ] + A3 * R1[ 2 ];
                CONTACT( contact, ret * skip )->pos[ 1 ] =
                    center[ 1 ] + A1 * R1[ 4 ] + A3 * R1[ 6 ];
                CONTACT( contact, ret * skip )->pos[ 2 ] =
                    center[ 2 ] + A1 * R1[ 8 ] + A3 * R1[ 10 ];
                CONTACT( contact, ret * skip )->depth =
                    centerDepth + Q1 * A1 + Q3 * A3;
...
extern "C" int dCylCyl( const dVector3 p1,
                        const dMatrix3 R1,
                        const dReal radius1,
                        const dReal lz1,
                        const dVector3 p2,
...
#define TEST( expr1, expr2, n1, n2, n3, cc ) \
    ...
    dNormalize3( Ax );
    TEST( p[ 0 ] * Ax[ 0 ] + p[ 1 ] * Ax[ 1 ] + p[ 2 ] * Ax[ 2 ],
          radius1 + radius2, Ax[ 0 ], Ax[ 1 ], Ax[ 2 ], 6 );

    {
        dReal sign, factor;

        // making ax which is perpendicular to cyl1 ax passing across cyl2
        // position// (project p on cyl1 flat surface )
        for ( i = 0; i < 3; ++i )
            pb[ i ] = p2[ i ];
        // cos1 = dDOT14(p,R1+0);
        // cos3 = dDOT14(p,R1+2) ;
    ...
    TEST( p[ 0 ] * Ax[ 0 ] + p[ 1 ] * Ax[ 1 ] + p[ 2 ] * Ax[ 2 ],
          radius1 + _cos * hlz2 + _sin * radius2, Ax[ 0 ], Ax[ 1 ], Ax[ 2 ],
          3 );

    {
        ...
        // making ax which is perpendicular to cyl2 ax passing across cyl1
        // position// (project p on cyl2 flat surface ) cos1 = dDOT14(p,R2+0);
        // cos3 = dDOT14(p,R2+2) ;
    ...
    TEST( p[ 0 ] * Ax[ 0 ] + p[ 1 ] * Ax[ 1 ] + p[ 2 ] * Ax[ 2 ],
          radius2 + _cos * hlz1 + _sin * radius1, Ax[ 0 ], Ax[ 1 ], Ax[ 2 ],
          4 );
    ...
    dVector3 point;
    {
        ...
        TEST( p[ 0 ] * Ax[ 0 ] + p[ 1 ] * Ax[ 1 ] + p[ 2 ] * Ax[ 2 ],
              cyl1Pr + cyl2Pr, Ax[ 0 ], Ax[ 1 ], Ax[ 2 ], 5 );
    }
...
#undef TEST
    ...
    *depth = -s;
    ...
    // @@@ temporary: make deepest point on the "other" cylinder the contact
    // point.
    ...
    if ( *code == 0 ) {
        ...
        CONTACT( contact, ret * skip )->pos[ 0 ] =
            center[ 0 ] + A1 * R2[ 0 ] + A3 * R2[ 2 ];
        CONTACT( contact, ret * skip )->pos[ 1 ] =
            center[ 1 ] + A1 * R2[ 4 ] + A3 * R2[ 6 ];
        CONTACT( contact, ret * skip )->pos[ 2 ] =
            center[ 2 ] + A1 * R2[ 8 ] + A3 * R2[ 10 ];
        CONTACT( contact, ret * skip )->depth =
            centerDepth + ( Q1 * A1 ) + ( Q3 * A3 );
        ...
        CONTACT( contact, ret * skip )->pos[ 2 ] =
            center[ 2 ] + A1 * R1[ 8 ] + A3 * R1[ 10 ];
        CONTACT( contact, ret * skip )->depth =
            centerDepth + dFabs( Q1 * A1 ) + dFabs( Q3 * A3 );
...
#define TEST( expr1, expr2, n1, n2, n3, cc ) \
    ...
    cos3 = dDOT14( Ax, R + 2 );
    _sin = dSqrt( cos1 * cos1 + cos3 * cos3 );
    TEST( dDOT( p, Ax ), sphereRadius + cylRadius * _sin + hl * _cos, Ax[ 0 ],
          Ax[ 1 ], Ax[ 2 ], 14 );
...
int dCollideCylB( dxGeom* o1,
    ...
    int num = dCylBox( dGeomGetPosition( o1 ), dGeomGetRotation( o1 ),
                       cylRadius, cylLength, dGeomGetPosition( o2 ),
                       dGeomGetRotation( o2 ), boxSides, normal, &depth, &code,
...
int dCollideCylCyl( dxGeom* o1,
    ...
    int num = dCylCyl( dGeomGetPosition( o1 ), dGeomGetRotation( o1 ),
                       cylRadius1, cylLength1, dGeomGetPosition( o2 ),
                       dGeomGetRotation( o2 ), cylRadius2, cylLength2, normal,
...
int dCollideCylPlane( dxGeom* o1,
                      dxGeom* o2,
                      int flags,
                      dContactGeom* contact,
                      int skip ) {
    ...
    if ( dFabs( Q2 ) > M_SQRT1_2 ) {
        ...
        CONTACT( contact, ret * skip )->pos[ 2 ] =
            pos[ 2 ] + 2.f * A3 * R[ 10 ];
        CONTACT( contact, ret * skip )->depth =
            outDepth - dFabs( Q3 * 2.f * A3 );

        if ( CONTACT( contact, ret * skip )->depth > 0.f )
            ++ret;
    } else {
        CONTACT( contact, ret * skip )->pos[ 0 ] =
            pos[ 0 ] + 2.f * ( A2 > 0 ? hlz * R[ 1 ] : -hlz * R[ 1 ] );
        CONTACT( contact, ret * skip )->pos[ 1 ] =
            pos[ 1 ] + 2.f * ( A2 > 0 ? hlz * R[ 5 ] : -hlz * R[ 5 ] );
        CONTACT( contact, ret * skip )->pos[ 2 ] =
            pos[ 2 ] + 2.f * ( A2 > 0 ? hlz * R[ 9 ] : -hlz * R[ 9 ] );
        CONTACT( contact, ret * skip )->depth =
            outDepth - dFabs( Q2 * 2.f * A2 );
    ...
    if ( !inside_cyl && C < 0 ) {
        ...
        if ( k < 0 ) {
            ...
            if ( k >= -lz2 && k <= lz2 ) {
                contact->normal[ 0 ] =
                    nsign *
                    ( contact->pos[ 0 ] - ( p[ 0 ] + k * R[ 0 * 4 + 1 ] ) );
                contact->normal[ 1 ] =
                    nsign *
                    ( contact->pos[ 1 ] - ( p[ 1 ] + k * R[ 1 * 4 + 1 ] ) );
                contact->normal[ 2 ] =
                    nsign *
                    ( contact->pos[ 2 ] - ( p[ 2 ] + k * R[ 2 * 4 + 1 ] ) );
...
static void dCylinderAABB( dxGeom* geom, dReal aabb[ 6 ] ) {
    ...
    dReal xrange = REAL( 0.5 ) * dFabs( R[ 1 ] * lz ) +
                   ( dSqrt( R[ 0 ] * R[ 0 ] + R[ 2 ] * R[ 2 ] ) * radius );

    dReal yrange = REAL( 0.5 ) * dFabs( R[ 5 ] * lz ) +
                   ( dSqrt( R[ 4 ] * R[ 4 ] + R[ 6 ] * R[ 6 ] ) * radius );

    dReal zrange = REAL( 0.5 ) * dFabs( R[ 9 ] * lz ) +
                   ( dSqrt( R[ 8 ] * R[ 8 ] + R[ 10 ] * R[ 10 ] ) * radius );
...
void dGeomCylinderSetParams( dGeomID g, dReal radius, dReal length ) {
    VERIFY2( g && dGeomGetClass( g ) == dCylinderClassUser,
             "argument not a cylinder" );
