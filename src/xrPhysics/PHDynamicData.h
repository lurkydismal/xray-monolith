...
#if !defined( \
    AFX_PHDynamicData_H__ACC01646_B581_4639_B78C_30311432021B__INCLUDED_ )
...
public:
    ...
    bool SetChild( unsigned int ChildNum,
                   unsigned int numOfchilds,
                   dBodyID body );
    void SetAsZero();
    ...
    void GetTGeomWorldMX( Fmatrix& aTransform ) {
        ...
        DMXPStoFMX( dGeomGetRotation( dGeomTransformGetGeom( transform ) ), P0,
                    Transform );

        dVectorSet( ( dReal* )&Translate,
                    dGeomGetPosition( dGeomTransformGetGeom( transform ) ) );
    ...
    static inline void DMXPStoFMX( const dReal* R,
                                   const dReal* pos,
                                   Fmatrix& aTransform ) {
