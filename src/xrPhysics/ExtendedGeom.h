...
#include "../3rd_party/ode/include/ode/collision.h"
...
public:
    ...
    static void RemoveCallback( CObjectContactCallback*& callbacks,
                                ObjectContactCallbackFun* c ) {
        if ( !callbacks )
            return;
        VERIFY( c );
        ...
        if ( c == callbacks->callback ) {
            ...
            for ( CObjectContactCallback *i = callbacks->next, *p = callbacks;
                  i; ) {
                VERIFY( p->callback );
                VERIFY( i->callback );
                VERIFY( i );
...
XRPHYSICS_API void get_user_data( dxGeomUserData*& gd1,
                                  dxGeomUserData*& gd2,
                                  bool bo1,
                                  const dContactGeom& geom );
...
IC void dGeomUserDataSetPhysicsRefObject( dxGeom* geom,
                                          IPhysicsShellHolder* phRefObject ) {
    ( dGeomGetUserData( geom ) )->ph_ref_object = phRefObject;
}

IC void dGeomUserDataSetContactCallback( dxGeom* geom,
                                         ContactCallbackFun* callback ) {
    ( dGeomGetUserData( geom ) )->callback = callback;
}

IC void dGeomUserDataSetObjectContactCallback(
    dxGeom* geom,
    ...
    if ( obj_callback )
        ( dGeomGetUserData( geom ) )->object_callbacks =
            xr_new< CObjectContactCallback >( obj_callback );
}

IC void dGeomUserDataAddObjectContactCallback(
    dxGeom* geom,
    ObjectContactCallbackFun* obj_callback ) {
...
IC void dGeomUserDataRemoveObjectContactCallback(
    dxGeom* geom,
    ObjectContactCallbackFun* obj_callback ) {
    CObjectContactCallback::RemoveCallback(
        ( dGeomGetUserData( geom ) )->object_callbacks, ( obj_callback ) );
}

// XRPHYSICS_API bool dGeomUserDataHasCallback(dxGeom*
// geom,ObjectContactCallbackFun	*obj_callback);
