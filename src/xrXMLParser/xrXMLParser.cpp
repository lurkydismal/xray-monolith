...
extern bool XMLLuaCallback( CXml& m_xml,
                            LPCSTR xml_string,
                            xr_string& transformed );

XRXMLPARSER_API CXml::CXml() : m_root( NULL ), m_pLocalRoot( NULL ) {}
            ...
            if ( _GetItem( str, 1, inc_name, '"' ) ) {
                ...
                if ( strstr( inc_name, "*.xml" ) ) {
                    ...
                    for ( FS_FileSet::iterator it = fset.begin();
                        ...
                        if ( file_name == strstr( file_name, "ui\\" ) ) {
                            shared_str fn = xml->correct_file_name(
                                "ui", strchr( file_name, '\\' ) + 1 );
                            string_path buff;
                            strconcat( sizeof( buff ), buff, "ui\\",
                                       fn.c_str() );
                ...
                if ( !strstr( inc_name, "*.xml" ) ) {
                    ...
                    if ( !I ) {
                        string1024 str;
                        xr_sprintf( str,
                                    "XML file[%s] parsing failed. Can't find "
                                    "include file:[%s]",
                                    path, inc_name );
                        R_ASSERT2( false, str );
                    }
...
void CXml::Load( LPCSTR path, LPCSTR xml_filename ) {
    ...
    xr_string transformed;
    const LPCSTR source =
        XMLLuaCallback( *this, ( LPCSTR )W.pointer(), transformed )
            ? transformed.c_str()
            : ( LPCSTR )W.pointer();
...
void CXml::LoadFromString( LPCSTR xml_string ) {
    ...
    if ( m_Doc.Error() ) {
        string1024 str;
        xr_sprintf( str, "XML file:%s value:%s errDescr:%s", m_xml_file_name,
                    m_Doc.Value(), m_Doc.ErrorDesc() );
        R_ASSERT2( false, str );
    }
...
XML_NODE* CXml::NavigateToNode( XML_NODE* start_node,
                                LPCSTR path,
                                int node_index ) {
    R_ASSERT3( start_node && path, "NavigateToNode failed in XML file ",
               m_xml_file_name );
    XML_NODE* node = NULL;
...
XML_NODE* CXml::NavigateToNode( LPCSTR path, int node_index ) {
    return NavigateToNode( GetLocalRoot() ? GetLocalRoot() : GetRoot(), path,
                           node_index );
}

XML_NODE* CXml::NavigateToNodeWithAttribute( LPCSTR tag_name,
                                             LPCSTR attrib_name,
                                             LPCSTR attrib_value ) {
...
LPCSTR CXml::Read( XML_NODE* start_node,
                   LPCSTR path,
                   int index,
                   LPCSTR default_str_val ) {
...
int CXml::ReadInt( XML_NODE* start_node,
                   LPCSTR path,
                   int index,
                   int default_int_val ) {
...
float CXml::ReadFlt( XML_NODE* start_node,
                     LPCSTR path,
                     int index,
                     float default_flt_val ) {
    LPCSTR result_str = Read( start_node, path, index, NULL );
    if ( result_str == NULL )
        return default_flt_val;
...
LPCSTR CXml::ReadAttrib( XML_NODE* start_node,
                         LPCSTR path,
                         int index,
                         LPCSTR attrib,
                         LPCSTR default_str_val ) {
...
LPCSTR CXml::ReadAttrib( LPCSTR path,
                         int index,
                         LPCSTR attrib,
                         LPCSTR default_str_val ) {
    XML_NODE* node = NavigateToNode( path, index );
...
LPCSTR CXml::ReadAttrib( XML_NODE* node,
                         LPCSTR attrib,
                         LPCSTR default_str_val ) {
    if ( node == NULL )
        return default_str_val;
        ...
        /*
                        ...
                        //не сможем запомнить строку и return вернет левый
           указатель shared_str result_str;
        */
...
int CXml::ReadAttribInt( LPCSTR path,
                         int index,
                         LPCSTR attrib,
                         int default_int_val ) {
...
int CXml::ReadAttribInt( XML_NODE* start_node,
                         LPCSTR path,
                         int index,
                         LPCSTR attrib,
                         int default_int_val ) {
...
float CXml::ReadAttribFlt( LPCSTR path,
                           int index,
                           LPCSTR attrib,
                           float default_flt_val ) {
    LPCSTR result_str = ReadAttrib( path, index, attrib, NULL );
...
float CXml::ReadAttribFlt( XML_NODE* start_node,
                           LPCSTR path,
                           int index,
                           LPCSTR attrib,
                           float default_flt_val ) {
...
float CXml::ReadAttribFlt( XML_NODE* node,
                           LPCSTR attrib,
                           float default_flt_val ) {
    LPCSTR result_str = ReadAttrib( node, attrib, NULL );
...
XML_NODE* CXml::SearchForAttribute( LPCSTR path,
                                    int index,
                                    LPCSTR tag_name,
                                    LPCSTR attrib,
                                    LPCSTR attrib_value_pattern ) {
    XML_NODE* start_node = NavigateToNode( path, index );
    XML_NODE* result = SearchForAttribute( start_node, tag_name, attrib,
                                           attrib_value_pattern );
    return result;
}
...
XML_NODE* CXml::SearchForAttribute( XML_NODE* start_node,
    ...
    while ( start_node ) {
        ...
        if ( el ) {
            ...
            if ( attribStr &&
                 0 == xr_strcmp( attribStr, attrib_value_pattern ) &&
        ...
        newEl =
            SearchForAttribute( newEl, tag_name, attrib, attrib_value_pattern );
        if ( newEl )
            return newEl;
...
LPCSTR CXml::CheckUniqueAttrib( XML_NODE* start_node,
    ...
    for ( int i = 0; i < tags_num; i++ ) {
        LPCSTR attrib =
            ReadAttrib( start_node, tag_name, i, attrib_name, NULL );
