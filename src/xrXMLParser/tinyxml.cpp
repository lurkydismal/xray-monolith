...
const TiXmlElement* TiXmlNode::FirstChildElement( const char* _value ) const {
    ...
    for ( node = FirstChild( _value ); node;
...
const TiXmlElement* TiXmlNode::NextSiblingElement( const char* _value ) const {
    ...
    for ( node = NextSibling( _value ); node;
          node = node->NextSibling( _value ) ) {
        if ( node->ToElement() )
            return node->ToElement();
    }
...
#else
    assert(
        !Find( addMe->Name() ) ); // Shouldn't be multiply adding to the set.
#endif
...
const TiXmlAttribute* TiXmlAttributeSet::Find( const xr_string& name ) const {
    for ( const TiXmlAttribute* node = sentinel.next; node != &sentinel;
          node = node->next ) {
        if ( node->name == name )
            return node;
    }
