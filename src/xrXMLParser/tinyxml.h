...
public:
    ...
    */
    ...
    /**	The world does not agree on whether white space should be kept or
            ...
            are provided to set whether or not TinyXml will condense all white
       space into a single space or not. The default is to condense. Note
       changing this value is not thread safe.
    */
...
protected:
    ...
    */
    static const char* ReadName( const char* p,
                                 TIXML_STRING* name,
    ...
    */
    ...
    static const char* GetEntity( const char* in,
                                  char* value,
                                  int* length,
                                  TiXmlEncoding encoding );
    ...
    inline static const char* GetChar( const char* p,
                                       char* _value,
                                       int* length,
                                       TiXmlEncoding encoding ) {
        assert( p );
...
public:
    ...
    */
    //	    friend std::istream& operator >> (std::istream& in, TiXmlNode&
    // base);
    ...
    */
    //	    friend std::ostream& operator<< (std::ostream& out, const TiXmlNode&
    // base);
    ...
    */
    ...
    TiXmlNode* IterateChildren( const char* _value,
...
#ifdef TIXML_USE_STL
    const TiXmlNode* IterateChildren( const xr_string& _value,
                                      const TiXmlNode* previous ) const {
...
private:
    ...
    //	TiXmlDocument*	document;	// A pointer back to a document, for
    // error reporting.
    TIXML_STRING name;
    TIXML_STRING value;
    TiXmlAttribute* prev;
...
private:
    ...
    void operator=(
        const TiXmlAttributeSet& ); // not allowed (as TiXmlAttribute)
    ...
    */
    ...
    /** Convenience function for easy access to the text inside an element.
       ...
       TiXmlText child and accessing it directly.
            ...
            returns the character string of the Text node, else null is
       returned.
       ...
       created from this XML:
            ...
            then the value of str would be null. The first child node isn't a
       text node, it is another element. From this XML:
            @verbatim
            <foo>This is <b>text</b></foo>
            @endverbatim
            GetText() will return "This is ".

            WARNING: GetText() accesses a child node - don't become confused
       with the similarly named TiXmlHandle::Text() and TiXmlNode::ToText()
...
public:
    ...
    */
    ...
    /** If an error occurs, Error will be set to true. Also,
            - The ErrorId() will contain the integer identifier of the error
       (not generally useful)
            - The ErrorDesc() method will return the name of the error. (very
       useful)
            - The ErrorRow() and ErrorCol() will return the location of the
       error (if known)
    */
    ...
    */
    ...
    /** SetTabSize() allows the error reporting functions (ErrorRow() and
       ...
       stored when the file is loaded. Very useful for tracking the DOM back in
       to the source file.

            The tab size is required for calculating the location of nodes. If
       not set, the default of 4 is used. The tabsize is set per document.
