...
extern "C" {
    ...
    ( _MSC_VER > 1399 ) && !defined( NVAPI_INTERNAL ) &&                \
...
#define __nvapi_deprecated_datatype( FirstRelease )        \
    __declspec( deprecated(                                \
        "Do not use this data type - it is deprecated in " \
...
//! to others in NvAPI
//!                 These are meant to be opaque types.  Do not assume they
//!                 correspond to indices, HDCs, display indexes or anything
//!                 else.
//!
//!                 Most handles remain valid until a display re-configuration
//!                 (display mode set) or GPU reconfiguration (going into or out
//!                 of SLI modes) occurs.  If NVAPI_HANDLE_INVALIDATED is
//!                 received by an app, it should discard all handles, and
//!                 re-enumerate them.
//! @{
NV_DECLARE_HANDLE( NvDisplayHandle ); //!< Display Device driven by NVIDIA
                                      //!< GPU(s) (an attached display)
NV_DECLARE_HANDLE( NvMonitorHandle ); //!< Monitor handle
NV_DECLARE_HANDLE( NvUnAttachedDisplayHandle ); //!< Unattached Display Device
                                                //!< driven by NVIDIA GPU(s)
NV_DECLARE_HANDLE(
    NvLogicalGpuHandle ); //!< One or more physical GPUs acting in concert (SLI)
NV_DECLARE_HANDLE( NvPhysicalGpuHandle ); //!< A single physical GPU
NV_DECLARE_HANDLE(
    NvEventHandle ); //!< A handle to an event registration instance
NV_DECLARE_HANDLE(
    NvVisualComputingDeviceHandle ); //!< A handle to a Visual Computing Device
...
NV_DECLARE_HANDLE(
    NvTransitionHandle ); //!< A handle to address a single transition request
NV_DECLARE_HANDLE( NvAudioHandle ); //!< NVIDIA HD Audio Device
NV_DECLARE_HANDLE(
    Nv3DVPContextHandle ); //!< A handle for a 3D Vision Pro (3DVP) context
NV_DECLARE_HANDLE(
    Nv3DVPTransceiverHandle ); //!< A handle for a 3DVP RF transceiver
NV_DECLARE_HANDLE(
    Nv3DVPGlassesHandle ); //!< A handle for a pair of 3DVP RF shutter glasses

typedef void*
    StereoHandle; //!< A stereo handle, that corresponds to the device interface
...
#define NV_MAX_HEADS \
    4 //!< Maximum heads, each with NVAPI_DESKTOP_RES resolution
#define NVAPI_MAX_HEADS_PER_GPU 32

#define NV_MAX_HEADS \
    4 //!< Maximum number of heads, each with #NVAPI_DESKTOP_RES resolution
#define NV_MAX_VID_STREAMS \
    4 //!< Maximum number of input video streams, each with a
      //!< #NVAPI_VIDEO_SRC_INFO
#define NV_MAX_VID_PROFILES \
    4 //!< Maximum number of output video profiles supported
...
//!  Maintain per structure specific version define using the MAKE_NVAPI_VERSION
//!  macro. \n Usage: #define NV_GENLOCK_STATUS_VER
//!  MAKE_NVAPI_VERSION(NV_GENLOCK_STATUS, 1)
//!  \ingroup nvapitypes
// =========================================================================================
#define MAKE_NVAPI_VERSION( typeName, ver ) \
...
typedef enum _NvAPI_Status {
    ...
    NVAPI_NVIDIA_DEVICE_NOT_FOUND = -6, //!< No NVIDIA display driver, or NVIDIA
                                        //!< GPU driving a display, was found.
    ...
    NVAPI_HANDLE_INVALIDATED = -10, //!< The handle is no longer valid (likely
                                    //!< due to GPU or display re-configuration)
    ...
    NVAPI_INSTRUMENTATION_DISABLED =
        -13, //!< OpenGL Expert is supported, but driver instrumentation is
             //!< currently disabled
    ...
    NVAPI_EXPECTED_UNATTACHED_DISPLAY_HANDLE =
        -106, //!< Expected an unattached display handle as one of the input
              //!< parameters.
    ...
    NVAPI_DEVICE_SWITCHING_NOT_ALLOWED =
        -117, //!< Inhibit is ON due to one of the flags in
    ...
    NVAPI_UNKNOWN_UNDERSCAN_CONFIG =
        -119, //!< The specified underscan config is from an unknown source
              //!< (e.g. INF)
    ...
    NVAPI_IMPLICIT_SET_GPU_TOPOLOGY_CHANGE_NOT_ALLOWED =
        -128, //!< There is no implicit GPU topology active. Use
              //!< NVAPI_SetHybridMode to change topology.
    ...
    NVAPI_WAS_STILL_DRAWING =
        -131, //!< The previous operation that is transferring information to or
              //!< from this surface is incomplete.
    NVAPI_FILE_NOT_FOUND = -132, //!< The file was not found.
    NVAPI_TOO_MANY_UNIQUE_STATE_OBJECTS =
        -133, //!< There are too many unique instances of a particular type of
              //!< state object.
    NVAPI_INVALID_CALL =
        -134, //!< The method call is invalid. For example, a method's parameter
    ...
    NVAPI_STEREO_NOT_INITIALIZED =
        -140, //!< The Stereo part of NVAPI failed to initialize completely.
              //!< Check if the stereo driver is installed.
    ...
    NVAPI_STEREO_NOT_ENABLED = -144, //!< Stereo is not enabled and the function
                                     //!< needed it to execute completely.
    NVAPI_STEREO_NOT_TURNED_ON =
        -145, //!< Stereo is not turned on and the function needed it to execute
              //!< completely.
    NVAPI_STEREO_INVALID_DEVICE_INTERFACE = -146, //!< Invalid device interface.
    NVAPI_STEREO_PARAMETER_OUT_OF_RANGE =
        -147, //!< Separation percentage or JPEG image capture quality is out of
              //!< [0-100] range.
    NVAPI_STEREO_FRUSTUM_ADJUST_MODE_NOT_SUPPORTED =
        -148, //!< The given frustum adjust mode is not supported.
    NVAPI_TOPO_NOT_POSSIBLE =
        -149, //!< The mosaic topology is not possible given the current state
              //!< of the hardware.
    ...
    NVAPI_PROFILE_REMOVED = -169, //!< The profile passed as parameter has been
    ...
    NVAPI_ID_OUT_OF_RANGE =
        -171, //!< The DisplayId corresponds to a display which is not within
              //!< the normal outputId range.
    NVAPI_DISPLAYCONFIG_VALIDATION_FAILED =
        -172, //!< Display topology is not valid so the driver cannot do a mode
              //!< set on this configuration.
    NVAPI_DPMST_CHANGED =
    ...
    NVAPI_MOSAIC_NOT_ACTIVE = -176, //!< The requested action cannot be
                                    //!< performed without Mosaic being enabled.
    ...
    NVAPI_D3D_DEVICE_LOST =
        -179, //!< D3D device status is D3DERR_DEVICELOST or
              //!< D3DERR_DEVICENOTRESET - the user has to reset the device.
    NVAPI_INVALID_CONFIGURATION = -180, //!< The requested action cannot be
    ...
    NVAPI_EXECUTABLE_PATH_IS_AMBIGUOUS =
        -182, //!< The path provided was too short to determine the correct
              //!< NVDRS_APPLICATION
    ...
    NVAPI_DPMST_DISPLAY_ID_EXPECTED =
        -186, //!< The input display id is not that of a multi stream enabled
              //!< connector or a display device in a multi stream topology
    NVAPI_INVALID_DISPLAY_ID =
        -187, //!< The input display id is not valid or the monitor associated
              //!< to it does not support the current operation
    ...
    NVAPI_GPU_WORKSTATION_FEATURE_INCOMPLETE =
        -192, //!< The requested workstation feature set has incomplete driver
              //!< internal allocation resources
    NVAPI_STEREO_INIT_ACTIVATION_NOT_DONE =
        -193, //!< Call failed because InitActivation was not called.
    NVAPI_SYNC_NOT_ACTIVE = -194, //!< The requested action cannot be performed
                                  //!< without Sync being enabled.
    NVAPI_SYNC_MASTER_NOT_FOUND =
        -195, //!< The requested action cannot be performed without Sync Master
              //!< being enabled.
...
//
//!   DESCRIPTION: This API returns display driver version and driver-branch
//!   string.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \param [out]  pDriverVersion         Contains the driver version after
//! successful return.
//! \param [out]  szBuildBranchString    Contains the driver-branch string after
//! successful return.
//!
//! \retval ::NVAPI_INVALID_ARGUMENT: either pDriverVersion is NULL or enum
//! index too big
...
typedef struct {
    ...
    NvU32 availableDedicatedVideoMemory; //!< Size(in kb) of the available
                                         //!< physical framebuffer for
                                         //!< allocating video memory surfaces.
    NvU32 systemVideoMemory;  //!< Size(in kb) of system memory the driver
                              //!< allocates at load time.
    NvU32 sharedSystemMemory; //!< Size(in kb) of shared system memory that
                              //!< driver is allowed to commit for surfaces
...
typedef struct {
    ...
    NvU32 sharedSystemMemory; //!< Size(in kb) of shared system memory that
                              //!< driver is allowed to commit for surfaces
                              //!< across all allocations.
    NvU32 curAvailableDedicatedVideoMemory; //!< Size(in kb) of the current
                                            //!< available physical framebuffer
                                            //!< for allocating video memory
                                            //!< surfaces.
...
#define NV_DISPLAY_DRIVER_MEMORY_INFO_VER_1 \
...
#define NV_DISPLAY_DRIVER_MEMORY_INFO_VER_2 \
...
//
//!   DESCRIPTION: This function retrieves the available driver memory footprint
...
//! \since Release: 177
//!
//!  \param [in]   hPhysicalGpu  Handle of the physical GPU for which the memory
...
//!  NV_DISPLAY_DRIVER_MEMORY_INFO structure version mismatch.
//!
//!  \ingroup  driverapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetMemoryInfo(
...
//! That GPU may be part of an SLI configuration, or may not be visible to the
//! OS directly.
//!
...
//!
//! \note In drivers older than 105.00, all physical GPU handles get invalidated
//! on a
//!       modeset. So the calling applications need to renum the handles after
//!       every modeset.\n With drivers 105.00 and up, all physical GPU handles
//!       are constant. Physical GPU handles are constant as long as the GPUs
//!       are not physically moved and the SBIOS VGA order is unchanged.
//!
...
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_EnumPhysicalGPUs(
...
//!
//! \param [in]  pDev       The ID3D11Device, ID3D10Device or IDirect3DDevice9
//! to use
//! \param [in]  pResource  The ID3D10Resource, ID3D10Resource or
