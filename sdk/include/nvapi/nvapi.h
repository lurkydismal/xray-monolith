...
|*  NOTICE TO USER: *|
|* *|
|*  This software is subject to NVIDIA ownership rights under U.S. and
international Copyright laws.                                  *|
|* *|
|*  This software and the information contained herein are PROPRIETARY and
CONFIDENTIAL to NVIDIA                                     *|
|*  and are being provided solely under the terms and conditions of an NVIDIA
software license agreement.                             *|
|*  Otherwise, you have no rights to use or access this software in any manner.
*|
|* *|
|*  If not covered by the applicable NVIDIA software license agreement: *|
|*  NVIDIA MAKES NO REPRESENTATION ABOUT THE SUITABILITY OF THIS SOFTWARE FOR
ANY PURPOSE.                                            *|
...
|*  This software is a "commercial item" as that term is defined at 48
C.F.R. 2.101 (OCT 1995),                                       *|
|*  consisting  of "commercial computer  software"  and "commercial computer
software documentation"                                  *|
|*  as such terms are  used in 48 C.F.R. 12.212 (SEPT 1995) and is provided to
the U.S. Government only as a commercial end item.     *|
...
#define NVAPI_API_NOT_INTIALIZED \
    NVAPI_API_NOT_INITIALIZED //!< Fix typo in error code

//!   \ingroup nvapistatus
#define NVAPI_INVALID_USER_PRIVILEDGE \
    NVAPI_INVALID_USER_PRIVILEGE //!< Fix typo in error code
...
//! \since Release: 80
//!
//! \retval  NVAPI_ERROR            An error occurred during the initialization
...
//!
//!
//!  !! This is not thread safe. In a multithreaded environment, calling
//!  NvAPI_Unload       !! \n
//!  !! while another thread is executing another NvAPI_XXX function, results in
//!  !!  \n
//!  !! undefined behaviour and might even cause the application to crash.
//!  Developers       !! \n
//!  !! must make sure that they are not in any other function before calling
//!  NvAPI_Unload. !! \n
//!
//!
//!  Unloading NvAPI library is not supported when the library is in a resource
//!  locked state. Some functions in the NvAPI library initiates an operation or
//!  allocates certain resources and there are corresponding functions
//!  available, to complete the operation or free the allocated resources. All
//!  such function pairs are designed to prevent unloading NvAPI library.
//!
//!  For example, if NvAPI_Unload is called after NvAPI_XXX which locks a
//!  resource, it fails with NVAPI_ERROR. Developers need to call the
//!  corresponding NvAPI_YYY to unlock the resources, before calling
//!  NvAPI_Unload again.
//!
//! \retval ::NVAPI_ERROR            One or more resources are locked and hence
//! cannot unload NVAPI library
...
//!               The contents of the string are human readable.  Do not assume
//!               a fixed
//!                format.
//!
//!
...
//
//! \fn NvAPI_GPU_GetEDID(NvPhysicalGpuHandle hPhysicalGpu, NvU32
//! displayOutputId, NV_EDID *pEDID)
//!  This function returns the EDID data for the specified GPU handle and
//!  connection bit mask. displayOutputId should have exactly 1 bit set to
//!  indicate a single display. See \ref handles.
//!
...
//! \since Release: 85
//!
//! \retval    NVAPI_INVALID_ARGUMENT              pEDID is NULL;
//! displayOutputId has 0 or > 1 bits set
//! \retval    NVAPI_OK                           *pEDID contains valid data.
//! \retval    NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a
//! display was found.
//! \retval    NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a
//! physical GPU handle.
//! \retval    NVAPI_DATA_NOT_FOUND                The requested display does
//! not contain an EDID.
//
///////////////////////////////////////////////////////////////////////////////
...
typedef struct {
    ...
    NvU32 edidId; //!< ID which always returned in a monotonically increasing
                  //!< counter. Across a split-EDID read we need to verify that
                  //!< all calls returned the same edidId. This counter is
                  //!< incremented if we get the updated EDID.
    NvU32 offset; //!< Which 256-byte page of the EDID we want to read. Start at
                  //!< 0. If the read succeeds with edidSize >
                  //!< NV_EDID_DATA_SIZE, call back again with offset+256 until
...
typedef enum _NV_FORMAT {
    NV_FORMAT_UNKNOWN =
        0,             //!< unknown. Driver will choose one as following value.
...
typedef enum _NV_TIMING_OVERRIDE {
    ...
    NV_TIMING_OVERRIDE_AUTO,      //!< the timing the driver will use based the
                                  //!< current policy
    ...
    NV_TIMING_OVERRIDE_NV_PREDEFINED, //!< NV pre-defined timing (basically the
                                      //!< PsF timings)
...
typedef struct tagNV_TIMINGEXT {
    NvU32 flag;   //!< Reserved for NVIDIA hardware-based enhancement, such as
                  //!< double-scan.
    NvU16 rr;     //!< Logical refresh rate to present
    NvU32 rrx1k;  //!< Physical vertical refresh rate in 0.001Hz
    NvU32 aspect; //!< Display aspect ratio Hi(aspect):horizontal-aspect,
                  //!< Low(aspect):vertical-aspect
    NvU16 rep; //!< Bit-wise pixel repetition factor: 0x1:no pixel repetition;
...
//
//! \fn NvAPI_SetView(NvDisplayHandle hNvDisplay, NV_VIEW_TARGET_INFO
//! *pTargetInfo, NV_TARGET_VIEW_MODE targetView)
//!  This function lets the caller modify the target display arrangement of the
//!  selected source display handle in any nView mode. It can also modify or
//!  extend the source display in Dualview mode.
//!  \note Maps the selected source to the associated target Ids.
//!  \note Display PATH with this API is limited to single GPU. DUALVIEW across
//!  GPUs cannot be enabled with this API.
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_DISP_SetDisplayConfig. SUPPORTED OS:  Windows Vista and
//! higher
//!
//!
//! \since Release: 90
...
//! #NVAPI_DEFAULT_HANDLE is not allowed, it has to be a handle enumerated with
//! NvAPI_EnumNVidiaDisplayHandle().
//! \param [in]  pTargetInfo      Pointer to array of NV_VIEW_TARGET_INFO,
//! specifying device properties in this view.
//!                               The first device entry in the array is the
//!                               physical primary. The device entry with the
//!                               lowest source id is the desktop primary.
//! \param [in]  targetCount      Count of target devices specified in
//! pTargetInfo.
...
typedef struct {
    ...
    struct {
        ...
        NvU32 sourceId;   //!< (IN/OUT) Source ID - values will be based on the
                          //!< number of heads exposed per GPU.
        NvU32 bPrimary
            : 1; //!< (OUT) Indicates if this is the GPU's primary view target.
                 //!< This is not the desktop GDI primary. NvAPI_SetView
                 //!< automatically selects the first target in
                 //!< NV_VIEW_TARGET_INFO index 0 as the GPU's primary view.
        NvU32 bInterlaced : 1; //!< (IN/OUT) Indicates if the timing being used
                               //!< on this monitor is interlaced.
        NvU32 bGDIPrimary
            : 1; //!< (IN/OUT) Indicates if this is the desktop GDI primary.
        NvU32 bForceModeSet
            : 1; //!< (IN) Used only on Win7 and higher during a call to
                 //!< NvAPI_SetView(). Turns off optimization & forces OS to set
                 //!< supplied mode.
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_DISP_SetDisplayConfig." ) NVAPI_INTERFACE
    NvAPI_SetView( NvDisplayHandle hNvDisplay,
...
//
//!  \fn NvAPI_SetViewEx(NvDisplayHandle hNvDisplay, NV_DISPLAY_PATH_INFO
//!  *pPathInfo, NV_TARGET_VIEW_MODE displayView) This function lets caller to
//!  modify the display arrangement for selected source display handle in any of
//!  the nview modes. It also allows to modify or extend the source display in
//!  dualview mode.
//!   \note Maps the selected source to the associated target Ids.
//!   \note Display PATH with this API is limited to single GPU. DUALVIEW across
//!   GPUs cannot be enabled with this API.
...
//! \since Release: 95
//!
//! \param [in]  hNvDisplay   NVIDIA Display selection. #NVAPI_DEFAULT_HANDLE is
//! not allowed, it has to be a handle enumerated with
//!                           NvAPI_EnumNVidiaDisplayHandle().
//! \param [in]  pPathInfo    Pointer to array of NV_VIEW_PATH_INFO, specifying
//! device properties in this view.
//!                           The first device entry in the array is the
//!                           physical primary. The device entry with the lowest
//!                           source id is the desktop primary.
...
typedef struct {
    ...
    NvU32 sourceId;   //!< (IN) Values will be based on the number of heads
                      //!< exposed per GPU(0, 1?)
    NvU32 bPrimary
        : 1; //!< (IN/OUT) Indicates if this is the GPU's primary view target.
             //!< This is not the desktop GDI primary. NvAPI_SetViewEx()
             //!< automatically selects the first target in NV_DISPLAY_PATH_INFO
             //!< index 0 as the GPU's primary view.
    NV_GPU_CONNECTOR_TYPE
    connector; //!< (IN) Specify connector type. For TV only.
    ...
    NV_FORMAT colorFormat; //!<      Color format if it needs to be specified.
                           //!<      Not used now.
    ...
    NV_DISPLAY_TV_FORMAT tvFormat; //!< (IN) To choose the last TV format set
                                   //!< this value to NV_DISPLAY_TV_FORMAT_NONE
    ...
    NvU32 bForceModeSet : 1; //!< (IN) Used only on Win7 and higher during a
                             //!< call to NvAPI_SetViewEx(). Turns off
                             //!< optimization & forces OS to set supplied mode.
    NvU32 bFocusDisplay : 1; //!< (IN) If set, this display path should have the
                             //!< focus after the GPU topology change
    NvU32 gpuId
        : 24; //!< (IN) the physical display/target Gpu id which is the owner of
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_DISP_SetDisplayConfig." ) NVAPI_INTERFACE
    NvAPI_SetViewEx( NvDisplayHandle hNvDisplay,
...
typedef struct _NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO_V1 {
    ...
    // Refresh Rate
    NvU32 refreshRate1K; //!< (IN) Non-interlaced Refresh Rate of the mode,
                         //!< multiplied by 1000, 0 = ignored This is the value
                         //!< which driver reports to the OS.
    // Flags
    NvU32 interlaced
        : 1; //!< (IN) Interlaced mode flag, ignored if refreshRate == 0
    NvU32 primary
        : 1; //!< (IN) Declares primary display in clone configuration. This is
             //!< *NOT* GDI Primary. Only one target can be primary per source.
             //!< If no primary is specified, the first target will
             //!< automatically be primary.
#ifdef NV_PAN_AND_SCAN_DEFINED
    NvU32 isPanAndScanTarget
        : 1; //!< Whether on this target Pan and Scan is enabled or has to be
             //!< enabled. Valid only when the target is part of clone topology.
    NvU32 reserved : 29;
#else
    NvU32 reserved : 30;
#endif
    ...
    NV_GPU_CONNECTOR_TYPE
    connector; //!< Specify connector type. For TV only, ignored if tvFormat
               //!< == NV_DISPLAY_TV_FORMAT_NONE
    NV_DISPLAY_TV_FORMAT tvFormat; //!< (IN) to choose the last TV format set
                                   //!< this value to NV_DISPLAY_TV_FORMAT_NONE

    // Backend (raster) timing standard
    NV_TIMING_OVERRIDE timingOverride; //!< Ignored if timingOverride ==
                                       //!< NV_TIMING_OVERRIDE_CURRENT
    NV_TIMING timing; //!< Scan out timing, valid only if timingOverride ==
                      //!< NV_TIMING_OVERRIDE_CUST The value
                      //!< NV_TIMING::NV_TIMINGEXT::rrx1k is obtained from the
                      //!< EDID. The driver may tweak this value for HDTV,
                      //!< stereo, etc., before reporting it to the OS.
} NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO_V1;

//! \ingroup dispcontrol
typedef NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO_V1
...
//! \ingroup dispcontrol
#define NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO_VER \
...
typedef struct _NV_DISPLAYCONFIG_PATH_TARGET_INFO_V1 {
    ...
    NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO*
        details; //!< May be NULL if no advanced settings are required. NULL for
                 //!< Non-NVIDIA Display.
...
typedef struct _NV_DISPLAYCONFIG_PATH_TARGET_INFO_V2 {
    NvU32 displayId; //!< Display ID
    NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO*
        details;    //!< May be NULL if no advanced settings are required
    NvU32 targetId; //!< Windows CCD target ID. Must be present only for
                    //!< non-NVIDIA adapter, for NVIDIA adapter this parameter
                    //!< is ignored.
} NV_DISPLAYCONFIG_PATH_TARGET_INFO_V2;

//! \ingroup dispcontrol
//! As version is not defined for this structure, we will be using version of
...
typedef struct _NV_DISPLAYCONFIG_SOURCE_MODE_INFO_V1 {
    ...
    NV_FORMAT
    colorFormat;          //!< Ignored at present, must be NV_FORMAT_UNKNOWN (0)
    NV_POSITION position; //!< Is all positions are 0 or invalid, displays will
                          //!< be automatically positioned from left to right
                          //!< with GDI Primary at 0,0, and all other displays
                          //!< in the order of the path array.
    NV_DISPLAYCONFIG_SPANNING_ORIENTATION
    spanningOrientation; //!< Spanning is only supported on XP
    NvU32 bGDIPrimary : 1;
    NvU32 bSLIFocus : 1;
...
typedef struct _NV_DISPLAYCONFIG_PATH_INFO_V1 {
    NvU32 version;
    NvU32 reserved_sourceId; //!< This field is reserved. There is ongoing
                             //!< debate if we need this field. Identifies
                             //!< sourceIds used by Windows. If all sourceIds
                             //!< are 0, these will be computed automatically.
    ...
    NV_DISPLAYCONFIG_SOURCE_MODE_INFO*
        sourceModeInfo; //!< May be NULL if mode info is not important
...
typedef struct _NV_DISPLAYCONFIG_PATH_INFO_V2 {
    ...
    void* pOSAdapterID; //!< Used by Non-NVIDIA adapter for pointer to OS
                        //!< Adapter of LUID type, type casted to void *.
...
//! \ingroup dispcontrol
#define NV_DISPLAYCONFIG_PATH_INFO_VER1 \
    MAKE_NVAPI_VERSION( NV_DISPLAYCONFIG_PATH_INFO_V1, 1 )

//! \ingroup dispcontrol
#define NV_DISPLAYCONFIG_PATH_INFO_VER2 \
...
typedef enum _NV_DISPLAYCONFIG_FLAGS {
    ...
    NV_DISPLAYCONFIG_DRIVER_RELOAD_ALLOWED =
        0x00000004, //!< Driver reload is permitted if necessary
    NV_DISPLAYCONFIG_FORCE_MODE_ENUMERATION =
        0x00000008, //!< Refresh OS mode list.
...
//! \fn NvAPI_GetDisplayDriverVersion(NvDisplayHandle hNvDisplay,
//! NV_DISPLAY_DRIVER_VERSION *pVersion) This function returns a struct that
//! describes aspects of the display driver
//!                build.
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_SYS_GetDriverAndBranchVersion. SUPPORTED OS:  Windows XP
//! and higher,  Mac OS X
//!
//!
//! \since Release: 80
...
//! \ingroup driverapi
#define NV_DISPLAY_DRIVER_VERSION_VER \
    MAKE_NVAPI_VERSION( NV_DISPLAY_DRIVER_VERSION, 1 )

//! \ingroup driverapi
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_SYS_GetDriverAndBranchVersion." ) NVAPI_INTERFACE
    NvAPI_GetDisplayDriverVersion( NvDisplayHandle hNvDisplay,
...
//! \retval  NVAPI_ERROR                       OpenGL driver failed to load
//! properly
...
//!   \ingroup oglapi
///////////////////////////////////////////////////////////////////////////////
typedef void ( *NVAPI_OGLEXPERT_CALLBACK )( unsigned int categoryId,
...
//!
//! The array nvGPUHandle will be filled with physical GPU handle values. The
//! returned gpuCount determines how many entries in the array are valid.
//!
//! NOTE: Handles enumerated by this API are only valid for NvAPIs that are
//! tagged as TCC_SUPPORTED
//!       If handle is passed to any other API, it will fail with
...
//!
//!
//! \param [out]   nvGPUHandle      Physical GPU array that will contain all TCC
//! Physical GPUs
//! \param [out]   pGpuCount        count represent the number of valid entries
//! in nvGPUHandle
//!
//!
...
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_EnumTCCPhysicalGPUs(
...
//!
//! Each handle represents one or more GPUs acting in concert as a single
//! graphics device.
//!
//! At least one GPU must be present in the system and running an NVIDIA display
//! driver.
//!
//! The array nvGPUHandle will be filled with logical GPU handle values.  The
//! returned gpuCount determines how many entries in the array are valid.
//!
//! \note All logical GPUs handles get invalidated on a GPU topology change, so
//! the calling
//!       application is required to renum the logical GPU handles to get latest
//!       physical handle mapping after every GPU topology change activated by a
//!       call to NvAPI_SetGpuTopologies().
//!
...
//! \retval NVAPI_INVALID_ARGUMENT         nvGPUHandle or pGpuCount is NULL
//! \retval NVAPI_OK                       One or more handles were returned
//! \retval NVAPI_NVIDIA_DEVICE_NOT_FOUND  No NVIDIA GPU driving a display was
//! found
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_EnumLogicalGPUs(
...
//
//! This function returns an array of physical GPU handles associated with the
//! specified display.
//!
//! At least one GPU must be present in the system and running an NVIDIA display
//! driver.
...
//! returned gpuCount determines how many entries in the array are valid.
//!
//! If the display corresponds to more than one physical GPU, the first GPU
//! returned is the one with the attached active output.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \since Release: 80
//!
//! \retval NVAPI_INVALID_ARGUMENT         hNvDisp is not valid; nvGPUHandle or
//! pGpuCount is NULL
//! \retval NVAPI_OK                       One or more handles were returned
//! \retval NVAPI_NVIDIA_DEVICE_NOT_FOUND  no NVIDIA GPU driving a display was
//! found
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GetPhysicalGPUsFromDisplay(
...
//
//! This function returns a physical GPU handle associated with the specified
//! unattached display. The source GPU is a physical render GPU which renders
//! the frame buffer but may or may not drive the scan out.
//!
//! At least one GPU must be present in the system and running an NVIDIA display
//! driver.
//!
...
//! \since Release: 80
//!
//! \retval NVAPI_INVALID_ARGUMENT         hNvUnAttachedDisp is not valid or
...
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GetPhysicalGPUFromUnAttachedDisplay(
...
//
//! This function returns the logical GPU handle associated with the specified
//! display. At least one GPU must be present in the system and running an
...
//! \since Release: 80
//!
//! \retval NVAPI_INVALID_ARGUMENT         hNvDisp is not valid; pLogicalGPU is
//! NULL
//! \retval NVAPI_OK                       One or more handles were returned
//! \retval NVAPI_NVIDIA_DEVICE_NOT_FOUND  No NVIDIA GPU driving a display was
//! found
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GetLogicalGPUFromDisplay(
    NvDisplayHandle hNvDisp,
...
//
//! This function returns the logical GPU handle associated with specified
//! physical GPU handle. At least one GPU must be present in the system and
...
//! \since Release: 80
//!
//! \retval NVAPI_INVALID_ARGUMENT         hPhysicalGPU is not valid;
//! pLogicalGPU is NULL
//! \retval NVAPI_OK                       One or more handles were returned
//! \retval NVAPI_NVIDIA_DEVICE_NOT_FOUND  No NVIDIA GPU driving a display was
//! found
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GetLogicalGPUFromPhysicalGPU(
...
//
//!  This function returns the physical GPU handles associated with the
//!  specified logical GPU handle. At least one GPU must be present in the
//!  system and running an NVIDIA display driver.
//!
//!  The array hPhysicalGPU will be filled with physical GPU handle values.  The
//!  returned gpuCount determines how many entries in the array are valid.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \since Release: 80
//!
//! \retval NVAPI_INVALID_ARGUMENT             hLogicalGPU is not valid;
//! hPhysicalGPU is NULL
//! \retval NVAPI_OK                           One or more handles were returned
//! \retval NVAPI_NVIDIA_DEVICE_NOT_FOUND      No NVIDIA GPU driving a display
//! was found
//! \retval NVAPI_EXPECTED_LOGICAL_GPU_HANDLE  hLogicalGPU was not a logical GPU
//! handle
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GetPhysicalGPUsFromLogicalGPU(
...
//! \retval ::NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a
//! physical GPU handle
//! \retval ::NVAPI_NOT_SUPPORTED                 API call is not supported on
...
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_GPU_GetAllDisplayIds. SUPPORTED OS:  Windows XP and
//! higher
//!
//!
//! \since Release: 85
//!
//! \retval   NVAPI_INVALID_ARGUMENT              hPhysicalGpu or pOutputsMask
//! is NULL.
//! \retval   NVAPI_OK                           *pOutputsMask contains a set of
//! GPU-output identifiers.
//! \retval   NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a
//! display was found.
//! \retval   NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a
//! physical GPU handle.
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_GPU_GetAllDisplayIds." ) NVAPI_INTERFACE
    NvAPI_GPU_GetAllOutputs( NvPhysicalGpuHandle hPhysicalGpu,
                             NvU32* pOutputsMask );
...
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_GPU_GetConnectedDisplayIds. SUPPORTED OS:  Windows XP and
//! higher
//!
//!
//! \since Release: 80
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_GPU_GetConnectedDisplayIds." ) NVAPI_INTERFACE
    NvAPI_GPU_GetConnectedOutputs( NvPhysicalGpuHandle hPhysicalGpu,
                                   NvU32* pOutputsMask );
...
//
//!   DESCRIPTION: This function is the same as NvAPI_GPU_GetConnectedOutputs()
//!   but returns only the set of GPU-output
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_GPU_GetConnectedDisplayIds." ) NVAPI_INTERFACE
    NvAPI_GPU_GetConnectedSLIOutputs( NvPhysicalGpuHandle hPhysicalGpu,
                                      NvU32* pOutputsMask );
...
//! \addtogroup gpu
//! @{
#define NV_GPU_CONNECTED_IDS_FLAG_UNCACHED \
    NV_BIT( 0 ) //!< Get uncached connected devices
#define NV_GPU_CONNECTED_IDS_FLAG_SLI \
    NV_BIT( 1 ) //!< Get devices such that those can be selected in an SLI
                //!< configuration
#define NV_GPU_CONNECTED_IDS_FLAG_LIDSTATE \
    NV_BIT( 2 ) //!< Get devices such that to reflect the Lid State
#define NV_GPU_CONNECTED_IDS_FLAG_FAKE \
    NV_BIT( 3 ) //!< Get devices that includes the fake connected monitors
#define NV_GPU_CONNECTED_IDS_FLAG_EXCLUDE_MST \
    NV_BIT(                                   \
        4 ) //!< Excludes devices that are part of the multi stream topology.
...
typedef struct _NV_GPU_DISPLAYIDS {
    NvU32 version;
    NV_MONITOR_CONN_TYPE
    connectorType; //!< out: vga, tv, dvi, hdmi and dp. This is reserved for
                   //!< future use and clients should not rely on this
                   //!< information. Instead get the GPU connector type from
                   //!< NvAPI_GPU_GetConnectorInfo/NvAPI_GPU_GetConnectorInfoEx
    NvU32 displayId;     //!< this is a unique identifier for each device
    NvU32 isDynamic : 1; //!< if bit is set then this display is part of MST
                         //!< topology and it's a dynamic
    NvU32 isMultiStreamRootNode
        : 1; //!< if bit is set then this displayID belongs to a multi stream
             //!< enabled connector(root node). Note that when multi stream is
             //!< enabled and a single multi stream capable monitor is connected
             //!< to it, the monitor will share the display id with the
             //!< RootNode. When there is more than one monitor connected in a
             //!< multi stream topology, then the root node will have a separate
             //!< displayId.
    NvU32 isActive
        : 1; //!< if bit is set then this display is being actively driven
    NvU32 isCluster
        : 1; //!< if bit is set then this display is the representative display
    NvU32 isOSVisible
        : 1;         //!< if bit is set, then this display is reported to the OS
...
//
//! \code
//!   DESCRIPTION: Due to space limitation NvAPI_GPU_GetConnectedOutputs can
//!   return maximum 32 devices, but
//!                this is no longer true for DPMST.
//!                NvAPI_GPU_GetConnectedDisplayIds will return all the
//!                connected display devices in the form of displayIds for the
//!                associated hPhysicalGpu. This function can accept set of
//!                flags to request cached, uncached, sli and lid to get the
//!                connected devices. Default value for flags will be cached .
//! HOW TO USE: 1) for each PhysicalGpu, make a call to get the number of
//! connected displayId's
//!                using NvAPI_GPU_GetConnectedDisplayIds by passing the
//!                pDisplayIds as NULL On call success:
//!             2) Allocate memory based on pDisplayIdCount then make a call
//!             NvAPI_GPU_GetConnectedDisplayIds to populate DisplayIds
//! SUPPORTED OS:  Windows XP and higher
//!
//! PARAMETERS:     hPhysicalGpu (IN)  - GPU selection
//!                 flags        (IN)  - One or more defines from
//!                 NV_GPU_CONNECTED_IDS_FLAG_* as valid flags. pDisplayIds
//!                 (IN/OUT) - Pointer to an NV_GPU_DISPLAYIDS struct, each
//!                 entry represents a one displayID and its attributes
//!                 pDisplayIdCount(OUT)- Number of displayId's.
//!
//! RETURN STATUS: NVAPI_INVALID_ARGUMENT: hPhysicalGpu or pDisplayIds or
//! pDisplayIdCount is NULL
//!                NVAPI_OK: *pDisplayIds contains a set of GPU-output
//!                identifiers NVAPI_NVIDIA_DEVICE_NOT_FOUND: no NVIDIA GPU
//!                driving a display was found
//!                NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE: hPhysicalGpu was not a
//!                physical GPU handle
//! \endcode
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
...
//
//!   DESCRIPTION: This API returns display IDs for all possible outputs on the
//!   GPU.
//!                For DPMST connector, it will return display IDs for all the
//!                video sinks in the topology. \n
//! HOW TO USE: 1. The first call should be made to get the all display ID
//! count. To get the display ID count, send in \n
//!                  a) hPhysicalGpu    - a valid GPU handle(enumerated using
//!                  NvAPI_EnumPhysicalGPUs()) as input,      \n b) pDisplayIds
//!                  - NULL, as we just want to get the display ID count. \n c)
//!                  pDisplayIdCount - a valid pointer to NvU32, whose value is
//!                  set to ZERO.                        \n
//!                If all parameters are correct and this call is successful,
//!                this call will return the display ID's count. \n
//!             2. To get the display ID array, make the second call to
//!             NvAPI_GPU_GetAllDisplayIds() with              \n
//!                  a) hPhysicalGpu    - should be same value which was sent in
//!                  first call,                           \n b) pDisplayIds -
//!                  pointer to the display ID array allocated by caller based
//!                  on display ID count,    \n
//!                                       eg. malloc(sizeof(NV_GPU_DISPLAYIDS) *
//!                                       pDisplayIdCount). \n
//!                  c) pDisplayIdCount - a valid pointer to NvU32. This
//!                  indicates for how many display IDs            \n
//!                                       the memory is allocated(pDisplayIds)
//!                                       by the caller. \n
//!                If all parameters are correct and this call is successful,
//!                this call will return the display ID array and actual display
//!                ID count (which was obtained in the first call to
//!                NvAPI_GPU_GetAllDisplayIds). If the input display ID count is
//!                less than the actual display ID count, it will overwrite the
...
//! \param [in,out] DisplayIds           Pointer to an array of
//! NV_GPU_DISPLAYIDS structures, each entry represents one displayID
//!                                      and its attributes.
//! \param [in,out] pDisplayIdCount      As input, this parameter indicates the
//! number of display's id's for which caller has
//!                                      allocated the memory. As output, it
//!                                      will return the actual number of
//!                                      display IDs.
//!
...
//!
//! \retval  NVAPI_INSUFFICIENT_BUFFER  When the input buffer(pDisplayIds) is
//! less than the actual number of display IDs, this API
...
//
//!  This function is similar to NvAPI_GPU_GetConnectedOutputs(), and returns
...
//!  State in the output mask. Thus if you expect the LID close state to be
//!  available in the connection mask use this API.
//!  - If LID is closed then this API will remove the LID panel from the
//!  connected display identifiers.
//!  - If LID is open then this API will reflect the LID panel in the connected
//!  display identifiers.
//!
//! \note This API should be used on notebook systems and on systems where the
//! LID state is required in the connection
//!       output mask. On desktop systems the returned identifiers will match
...
//! \since Release: 95
//!
//! \retval  NVAPI_INVALID_ARGUMENT              hPhysicalGpu or pOutputsMask is
//! NULL
//! \retval  NVAPI_OK                           *pOutputsMask contains a set of
//! GPU-output identifiers
//! \retval  NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a display
//! was found
//! \retval  NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a physical
//! GPU handle
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_GPU_GetConnectedDisplayIds." ) NVAPI_INTERFACE
    NvAPI_GPU_GetConnectedOutputsWithLidState( NvPhysicalGpuHandle hPhysicalGpu,
                                               NvU32* pOutputsMask );
...
//!                of GPU-output identifiers that can be selected in an SLI
//!                configuration. With SLI disabled, this function matches
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_GPU_GetConnectedDisplayIds." ) NVAPI_INTERFACE
    NvAPI_GPU_GetConnectedSLIOutputsWithLidState(
        NvPhysicalGpuHandle hPhysicalGpu,
        NvU32* pOutputsMask );
...
//
//! \fn NvAPI_GPU_GetSystemType(NvPhysicalGpuHandle hPhysicalGpu, NV_SYSTEM_TYPE
//! *pSystemType)
//!  This function identifies whether the GPU is a notebook GPU or a desktop
//!  GPU.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \retval  NVAPI_INVALID_ARGUMENT              hPhysicalGpu or pOutputsMask is
//! NULL
//! \retval  NVAPI_OK                           *pSystemType contains the GPU
//! system type
//! \retval  NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a display
//! was found
//! \retval  NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE: hPhysicalGpu was not a physical
...
//
//!  This function is the same as NvAPI_GPU_GetAllOutputs but returns only the
//!  set of GPU output identifiers that are actively driving display devices.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//! \retval    NVAPI_INVALID_ARGUMENT              hPhysicalGpu or pOutputsMask
//! is NULL.
//! \retval    NVAPI_OK                           *pOutputsMask contains a set
//! of GPU-output identifiers.
//! \retval    NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a
//! display was found.
//! \retval    NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a
//! physical GPU handle.
...
//
//!  Thus function sets the EDID data for the specified GPU handle and
//!  connection bit mask. displayOutputId should have exactly 1 bit set to
//!  indicate a single display. See \ref handles.
...
//!        OS and NVAPI connection status APIs will reflect the newly set or
...
//! \since Release: 100
//!
//! \retval  NVAPI_INVALID_ARGUMENT              pEDID is NULL; displayOutputId
//! has 0 or > 1 bits set
//! \retval  NVAPI_OK                           *pEDID data was applied to the
//! requested displayOutputId.
//! \retval  NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a display
//! was found.
//! \retval  NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE: hPhysicalGpu was not a physical
//! GPU handle.
...
//
//! \fn NvAPI_GPU_GetOutputType(NvPhysicalGpuHandle hPhysicalGpu, NvU32
//! outputId, NV_GPU_OUTPUT_TYPE *pOutputType)
//!  This function returns the output type for a specific physical GPU handle
//!  and outputId (exactly 1 bit set - see \ref handles).
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//!
//! \retval     NVAPI_INVALID_ARGUMENT              hPhysicalGpu, outputId, or
//! pOutputsMask is NULL; or outputId has > 1 bit set
//! \retval     NVAPI_OK                           *pOutputType contains a
//! NvGpuOutputType value
//! \retval     NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a
//! display was found
//! \retval     NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a
//! physical GPU handle
...
//! \since Release: 85
//!
//! \retval  NVAPI_OK                            Combination of outputs in
//! outputsMask are valid (can be active simultaneously).
//! \retval  NVAPI_INVALID_COMBINATION           Combination of outputs in
//! outputsMask are NOT valid.
//! \retval  NVAPI_INVALID_ARGUMENT              hPhysicalGpu or outputsMask
//! does not have at least 2 bits set.
//! \retval  NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a physical
//! GPU handle.
//! \retval  NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a display
//! was found.
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_ValidateOutputCombination(
    NvPhysicalGpuHandle hPhysicalGpu,
    NvU32 outputsMask );
...
//! \param   RevisionId    The internal PCI device-specific revision identifier
//! for the GPU.
//! \param   ExtDeviceId   The external PCI device identifier for the GPU.
//!
//! \retval  NVAPI_INVALID_ARGUMENT              hPhysicalGpu or an argument is
//! NULL
//! \retval  NVAPI_OK                            Arguments are populated with
//! PCI identifiers
//! \retval  NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a display
//! was found
//! \retval  NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a physical
...
//! #NvAPI_Status. If there are return error codes with
//!              specific meaning for this API, they are listed below.
//! \retval      NVAPI_INVALID_ARGUMENT             hPhysicalGpu or pBusType is
//! NULL.
//! \retval      NVAPI_OK                          *pBusType contains bus
//! identifier.
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
...
//! \since Release: 167
//!
//!  \retval  NVAPI_INVALID_ARGUMENT              hPhysicalGpu or pBusId is
//!  NULL.
...
//! \since Release: 167
//!
//!  \retval  NVAPI_INVALID_ARGUMENT              hPhysicalGpu or pBusSlotId is
//!  NULL.
//!  \retval  NVAPI_OK                           *pBusSlotId contains the bus
//!  slot ID.
//!  \retval  NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a
//!  display was found.
//!  \retval  NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a
...
//
//!  This function returns the revision of the video BIOS associated with this
//!  GPU.
//!
//! SUPPORTED OS:  Windows XP and higher,  Mac OS X
//!
...
//! \since Release: 90
//!
//! \retval    NVAPI_INVALID_ARGUMENT               hPhysicalGpu or
//! pBiosRevision is NULL.
//! \retval    NVAPI_OK                            *pBiosRevision contains
//! revision number.
//! \retval    NVAPI_NVIDIA_DEVICE_NOT_FOUND        No NVIDIA GPU driving a
//! display was found.
//! \retval    NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE   hPhysicalGpu was not a
...
//
//!  This function returns the OEM revision of the video BIOS associated with
//!  this GPU.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \since Release: 90
//!
//! \retval    NVAPI_INVALID_ARGUMENT              hPhysicalGpu or pBiosRevision
//! is NULL
//! \retval    NVAPI_OK                           *pBiosRevision contains
//! revision number
//! \retval    NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a
...
//
//!  This function returns the full video BIOS version string in the form of
//!  xx.xx.xx.xx.yy where
...
//! \retval   NVAPI_OK                            szBiosRevision contains
//! version string.
//! \retval   NVAPI_NVIDIA_DEVICE_NOT_FOUND       No NVIDIA GPU driving a
//! display was found.
//! \retval   NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu was not a
...
//
//!  This function returns the current AGP Rate (0 = AGP not present, 1 = 1x, 2
//!  = 2x, etc.).
//!
//! SUPPORTED OS:  Windows XP and higher,  Mac OS X
//!
...
//
//!  This function returns the number of PCIE lanes being used for the PCIE
...
//
//!   This function returns the physical size of framebuffer in KB.  This does
//!   NOT include any system RAM that may be dedicated for use by the GPU.
//!
//! SUPPORTED OS:  Windows XP and higher,  Mac OS X
//!
//!
...
//
//!  This function returns the virtual size of framebuffer in KB.  This includes
//!  the physical RAM plus any system RAM that has been dedicated for use by the
//!  GPU.
//!
//! SUPPORTED OS:  Windows XP and higher,  Mac OS X
//!
...
//
//!   DESCRIPTION: This API Retrieves the Board information (a unique GPU Board
//!   Serial Number) stored in the InfoROM.
//!
...
//! \retval ::NVAPI_OK                     completed request
//! \retval ::NVAPI_ERROR                  miscellaneous error occurred
//! \retval ::NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  handle passed is not a
//! physical GPU handle
//! \retval ::NVAPI_API_NOT_INTIALIZED            NVAPI not initialized
//! \retval ::NVAPI_INVALID_POINTER               pBoardInfo is NULL
//! \retval ::NVAPI_INCOMPATIBLE_STRUCT_VERSION   the version of the INFO struct
//! is not supported
//!
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
...
typedef struct {
    NvU32 version;       //!< Structure version
    NvU32 ClockType : 2; //!< One of NV_GPU_CLOCK_FREQUENCIES_CLOCK_TYPE. Used
                         //!< to specify the type of clock to be returned.
    NvU32 reserved
...
//! \addtogroup gpuclock
//! @{
#define NV_GPU_CLOCK_FREQUENCIES_VER_1 \
...
//
//!   This function retrieves the NV_GPU_CLOCK_FREQUENCIES structure for the
//!   specified physical GPU.
//!
...
//!   clkFreqs.domain[NVAPI_GPU_PUBLIC_CLOCK_MEMORY] holds the info for the
//!   MEMORY domain.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \ingroup gpuclock
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetAllClockFrequencies(
    __in NvPhysicalGpuHandle hPhysicalGPU,
...
typedef struct {
    ...
    struct {
        ...
        NvU32
            flags; //!< - bit 0 indicates if the PCIE limit is GEN1 or GEN2
...
typedef struct {
    ...
    } pstates[ NVAPI_MAX_GPU_PERF_PSTATES ]; //!< Valid index range is 0 to
                                             //!< numVoltages-1
...
//
//! DESCRIPTION:     This API retrieves all performance states (P-States)
//! information. This is the same as
//!                  NvAPI_GPU_GetPstatesInfo(), but supports an input flag for
//!                  various options.
//!
//!                  P-States are GPU active/executing performance capability
//!                  and power consumption states.
//!
//!                  P-States ranges from P0 to P15, with P0 being the highest
//!                  performance/power state, and P15 being the lowest
//!                  performance/power state. Each P-State, if available, maps
//!                  to a performance level. Not all P-States are available on a
//!                  given system. The definitions of each P-State are currently
//!                  as follows: \n
...
//!
//! \deprecated  Do not use this function - it is deprecated in release 304.
//! Instead, use NvAPI_GPU_GetPstates20. SUPPORTED OS:  Windows XP and higher,
//! Mac OS X
//!
//!
//! \param [in]     hPhysicalGPU       GPU selection.
//! \param [out]    pPerfPstatesInfo   P-States information retrieved, as
//! detailed below: \n
//!                  - flags is reserved for future use.
//!                  - numPstates is the number of available P-States
//!                  - numClocks is the number of clock domains supported by
//!                  each P-State
...
//!                        - bit 1 indicates if the Pstate is overclocked or not
//!                        - bit 2 indicates if the Pstate is overclockable or
//!                        not
//!                    - pstates[i].clocks has valid index range from 0 to
//!                    numClocks -1
//!                    - pstates[i].clocks[j].domainId is the public ID of the
//!                    clock domain,
//!                        containing the following info:
//!                      - pstates[i].clocks[j].flags containing the following
//!                      info:
//!                          bit 0 indicates if the clock domain is
//!                          overclockable or not
//!                      - pstates[i].clocks[j].freq is the clock frequency in
//!                      kHz
//!                    - pstates[i].voltages has a valid index range from 0 to
//!                    numVoltages - 1
//!                    - pstates[i].voltages[j].domainId is the ID of the
//!                    voltage domain,
//!                        containing the following info:
//!                      - pstates[i].voltages[j].flags is reserved for future
//!                      use.
//!                      - pstates[i].voltages[j].mvolt is the voltage in mV
//!                  inputFlags(IN)   - This can be used to select various
//!                  options:
//!                    - if bit 0 is set, pPerfPstatesInfo would contain the
//!                    default settings
//!                        instead of the current, possibily overclocked
//!                        settings.
//!                    - if bit 1 is set, pPerfPstatesInfo would contain the
//!                    maximum clock
//!                        frequencies instead of the nominal frequencies.
//!                    - if bit 2 is set, pPerfPstatesInfo would contain the
//!                    minimum clock
//!                        frequencies instead of the nominal frequencies.
//!                    - all other bits must be set to 0.
//!
//! \retval ::NVAPI_OK                            Completed request
//! \retval ::NVAPI_ERROR                         Miscellaneous error occurred
//! \retval ::NVAPI_HANDLE_INVALIDATED            Handle passed has been
//! invalidated (see user guide)
//! \retval ::NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  Handle passed is not a
//! physical GPU handle
//! \retval ::NVAPI_INCOMPATIBLE_STRUCT_VERSION   The version of the
//! NV_GPU_PERF_PSTATES struct is not supported
//!
//! \ingroup gpupstate
///////////////////////////////////////////////////////////////////////////////
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 304. Instead, use "
    "NvAPI_GPU_GetPstates20." ) NVAPI_INTERFACE
    NvAPI_GPU_GetPstatesInfoEx( NvPhysicalGpuHandle hPhysicalGpu,
...
typedef struct {
    ...
    struct {
        //! Min value allowed for parameter delta (in respective units [kHz,
        //! uV])
        NvS32 min;

        //! Max value allowed for parameter delta (in respective units [kHz,
        //! uV])
        NvS32 max;
    } valueRange;
...
typedef struct {
    ...
    union {
        ...
        struct {
            ...
            //! Voltage domain ID and value range in (uV) required for this
            //! clock
...
typedef struct {
    ...
    NV_GPU_PERF_PSTATES20_PARAM_DELTA
    voltDelta_uV; // Current base voltage delta from nominal settings in
                  // [uV]
...
typedef struct {
    ...
    struct {
        ...
        NV_GPU_PSTATE20_BASE_VOLTAGE_ENTRY_V1
        baseVoltages[ NVAPI_MAX_GPU_PSTATE20_BASE_VOLTAGES ];
...
typedef struct _NV_GPU_PERF_PSTATES20_INFO_V2 {
    ...
    //! OV settings - Please refer to NVIDIA over-volting recommendation to
    //! understand impact of this functionality Valid index range is 0 to
    //! numVoltages-1
    struct {
...
#define NV_GPU_PERF_PSTATES20_INFO_VER1 \
...
#define NV_GPU_PERF_PSTATES20_INFO_VER2 \
...
//
//! DESCRIPTION:    This API retrieves all performance states (P-States) 2.0
//! information.
//!
//!                 P-States are GPU active/executing performance capability
//!                 states. They range from P0 to P15, with P0 being the highest
//!                 performance state, and P15 being the lowest performance
//!                 state. Each P-State, if available, maps to a performance
//!                 level. Not all P-States are available on a given system. The
//!                 definition of each P-States are currently as follow:
//!                 - P0/P1 - Maximum 3D performance
//!                 - P2/P3 - Balanced 3D performance-power
//!                 - P8 - Basic HD video playback
//!                 - P10 - DVD playback
...
//!
//! \param [in]   hPhysicalGPU  GPU selection
//! \param [out]  pPstatesInfo  P-States information retrieved, as documented in
//! declaration above
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
...
//! \ingroup gpupstate
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetPstates20(
    __in NvPhysicalGpuHandle hPhysicalGpu,
...
//
//! DESCRIPTION:     This function retrieves the current performance state
//! (P-State).
//!
//! SUPPORTED OS:  Windows XP and higher,  Mac OS X
//!
...
//!
//! \param [in]      hPhysicalGPU     GPU selection
//! \param [out]     pCurrentPstate   The ID of the current P-State of the GPU -
//! see \ref NV_GPU_PERF_PSTATES.
//!
//! \retval    NVAPI_OK                             Completed request
...
//! \retval    NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE   Handle passed is not a
//! physical GPU handle.
//! \retval    NVAPI_NOT_SUPPORTED                  P-States is not supported on
//! this setup.
//!
//! \ingroup   gpupstate
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetCurrentPstate(
    NvPhysicalGpuHandle hPhysicalGpu,
...
typedef struct {
    ...
    struct {
        ...
        NvU32 percentage; //!< Percentage of time where the domain is considered
                          //!< busy in the last 1 second interval
    } utilization[ NVAPI_MAX_GPU_UTILIZATIONS ];
} NV_GPU_DYNAMIC_PSTATES_INFO_EX;

//! \ingroup gpupstate
//! Macro for constructing the version field of NV_GPU_DYNAMIC_PSTATES_INFO_EX
#define NV_GPU_DYNAMIC_PSTATES_INFO_EX_VER \
...
//!                holds the info for the GPU domain. \p There are currently 4
//!                domains for which GPU utilization and dynamic P-State
//!                thresholds can be retrieved:
//!                   graphic engine (GPU), frame buffer (FB), video engine
...
//! \ingroup gpupstate
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetDynamicPstatesInfoEx(
    NvPhysicalGpuHandle hPhysicalGpu,
    NV_GPU_DYNAMIC_PSTATES_INFO_EX* pDynamicPstatesInfoEx );

///////////////////////////////////////////////////////////////////////////////////
//  Thermal API
//  Provides ability to get temperature levels from the various thermal sensors
...
typedef enum {
    ...
    NVAPI_THERMAL_TARGET_GPU =
        1, //!< GPU core temperature requires NvPhysicalGpuHandle
    NVAPI_THERMAL_TARGET_MEMORY =
        2, //!< GPU memory temperature requires NvPhysicalGpuHandle
    NVAPI_THERMAL_TARGET_POWER_SUPPLY =
        4, //!< GPU power supply temperature requires NvPhysicalGpuHandle
    NVAPI_THERMAL_TARGET_BOARD =
        8, //!< GPU board ambient temperature requires NvPhysicalGpuHandle
    NVAPI_THERMAL_TARGET_VCD_BOARD =
        9, //!< Visual Computing Device Board temperature requires
           //!< NvVisualComputingDeviceHandle
    NVAPI_THERMAL_TARGET_VCD_INLET =
        10, //!< Visual Computing Device Inlet temperature requires
            //!< NvVisualComputingDeviceHandle
    NVAPI_THERMAL_TARGET_VCD_OUTLET =
        11, //!< Visual Computing Device Outlet temperature requires
            //!< NvVisualComputingDeviceHandle
...
typedef struct {
    ...
    struct {
        ...
        NvU32 defaultMinTemp; //!< The min default temperature value of the
                              //!< thermal sensor in degree Celsius
        NvU32 defaultMaxTemp; //!< The max default temperature value of the
                              //!< thermal sensor in degree Celsius
        NvU32 currentTemp;    //!< The current temperature value of the thermal
                              //!< sensor in degree Celsius
        NV_THERMAL_TARGET
        target; //!< Thermal sensor targeted @ GPU, memory, chipset,
                //!< powersupply, Visual Computing Device, etc.
...
typedef struct {
    ...
    struct {
        ...
        NvS32 defaultMinTemp; //!< Minimum default temperature value of the
                              //!< thermal sensor in degree Celsius
        NvS32 defaultMaxTemp; //!< Maximum default temperature value of the
                              //!< thermal sensor in degree Celsius
        NvS32 currentTemp; //!< Current temperature value of the thermal sensor
                           //!< in degree Celsius
        NV_THERMAL_TARGET
        target; //!< Thermal sensor targeted - GPU, memory, chipset,
                //!< powersupply, Visual Computing Device, etc
...
//
//!  This function retrieves the thermal information of all thermal sensors or
//!  specific thermal sensor associated with the selected GPU. Thermal sensors
//!  are indexed 0 to NVAPI_MAX_THERMAL_SENSORS_PER_GPU-1.
//!
//!  - To retrieve specific thermal sensor info, set the sensorIndex to the
//!  required thermal sensor index.
//!  - To retrieve info for all sensors, set sensorIndex to
...
//! \retval   NVAPI_HANDLE_INVALIDATED           Handle passed has been
//! invalidated (see user guide).
//! \retval   NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE Handle passed is not a physical
//! GPU handle.
//! \retval   NVAPI_INCOMPATIBLE_STRUCT_VERSION  The version of the INFO struct
//! is not supported.
//! \ingroup gputhermal
...
typedef enum {
    NVAPI_I2C_SPEED_DEFAULT, //!< Set i2cSpeedKhz to I2C_SPEED_DEFAULT if
                             //!< default I2C speed is to be chosen, ie.use the
...
typedef struct {
    ...
    NvU8 bIsDDCPort; //!< This flag indicates either the DDC port (TRUE) or the
                     //!< communication port (FALSE) of the concerned display.
    NvU8 i2cDevAddress; //!< The address of the I2C slave.  The address should
                        //!< be shifted left by one.  For example, the I2C
                        //!< address 0x50, often used for reading EDIDs, would
                        //!< be stored here as 0xA0.  This matches the position
                        //!< within the byte sent by the master, as the last bit
                        //!< is reserved to specify the read or write direction.
    NvU8* pbI2cRegAddress; //!< The I2C target register address.  May be NULL,
                           //!< which indicates no register address should be
                           //!< sent.
    NvU32 regAddrSize; //!< The size in bytes of target register address.  If
                       //!< pbI2cRegAddress is NULL, this field must be 0.
    NvU8* pbData;      //!< The buffer of data which is to be read or written
                       //!< (depending on the command).
    NvU32
        cbSize; //!< The size of the data buffer, pbData, to be read or written.
    NvU32 i2cSpeed; //!< The target speed of the transaction (between 28Kbps to
                    //!< 40Kbps; not guaranteed).
} NV_I2C_INFO_V1;
...
typedef struct {
    ...
    NV_I2C_SPEED i2cSpeedKhz; //!< The target speed of the transaction in (kHz)
...
typedef struct {
    ...
    NV_I2C_SPEED i2cSpeedKhz; //!< The target speed of the transaction in (kHz)
                              //!< (Chosen from the enum NV_I2C_SPEED).
    NvU8 portId; //!< The portid on which device is connected (remember to set
                 //!< bIsPortIdSet if this value is set) Optional for pre-Kepler
...
//!                 The I2C request must be for a DDC port: pI2cInfo->bIsDDCPort
//!                 = 1.
//!
//!                 A data buffer size larger than 16 bytes may be rejected if a
//!                 register address is specified.  In such a case,
//!                 NVAPI_ARGUMENT_EXCEED_MAX_SIZE would be returned.
//!
//!                 If a register address is specified (i.e. regAddrSize is
//!                 positive), then the transaction will be performed in the
//!                 combined format described in the I2C specification.  The
//!                 register address will be written, followed by reading into
//!                 the data buffer.
//!
//! SUPPORTED OS:  Windows XP and higher,  Mac OS X
//!
...
//! \retval   NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  Handle passed is not a
//! physical GPU handle.
//! \retval   NVAPI_INCOMPATIBLE_STRUCT_VERSION   Structure version is not
//! supported.
//! \retval   NVAPI_INVALID_ARGUMENT - argument does not meet specified
//! requirements
//! \retval   NVAPI_ARGUMENT_EXCEED_MAX_SIZE - an argument exceeds the maximum
//!
//! \ingroup i2capi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_I2CRead( NvPhysicalGpuHandle hPhysicalGpu,
...
//!
//!                 If a register address is specified (i.e. regAddrSize is
//!                 positive), then the register address will be written and the
//!                 data buffer will immediately follow without a restart.
//!
//! SUPPORTED OS:  Windows XP and higher,  Mac OS X
//!
//!
...
//! \retval   NVAPI_INCOMPATIBLE_STRUCT_VERSION    Structure version is not
//! supported.
//! \retval   NVAPI_INVALID_ARGUMENT              Argument does not meet
//! specified requirements
//! \retval   NVAPI_ARGUMENT_EXCEED_MAX_SIZE      Argument exceeds the maximum
//!
//! \ingroup i2capi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_I2CWrite( NvPhysicalGpuHandle hPhysicalGpu,
                                NV_I2C_INFO* pI2cInfo );
...
//
//! \fn NvAPI_GPU_WorkstationFeatureSetup(NvPhysicalGpuHandle hPhysicalGpu,
//! NvU32 featureEnableMask, NvU32 featureDisableMask)
//!   DESCRIPTION: This API configures the driver for a set of workstation
//!   features.
...
//!
//!
//! \param [in]   hPhysicalGpu       Physical GPU Handle of the display adapter
//! to be configured. GPU handles may be retrieved
//!                                  using NvAPI_EnumPhysicalGPUs. A value of
//!                                  NULL is permitted and applies the same
//!                                  operation to all GPU handles enumerated by
//!                                  NvAPI_EnumPhysicalGPUs.
//! \param [in]   featureEnableMask  Mask of features the caller requests to
//! enable for use
//! \param [in]   featureDisableMask Mask of features the caller requests to
//! disable
//!
//!                As a general rule, features in the enable and disable masks
//!                are expected to be disjoint, although the disable mask has
//!                precedence and a feature flagged in both masks will be
//!                disabled.
//!
//! \retval ::NVAPI_OK                            configuration request
//! succeeded
//! \retval ::NVAPI_ERROR                         configuration request failed
//! \retval ::NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE  hPhysicalGpu is not a physical
//! GPU handle.
//! \retval ::NVAPI_GPU_WORKSTATION_FEATURE_INCOMPLETE  requested feature set
//! does not have all resources allocated for completeness.
...
//! \ingroup gpu
NVAPI_INTERFACE NvAPI_GPU_WorkstationFeatureSetup(
    __in NvPhysicalGpuHandle hPhysicalGpu,
...
//! \param [out]  pConfiguredFeatureMask  Mask of features requested for use by
//! client drivers
//! \param [out]  pConsistentFeatureMask  Mask of features that have all
...
//! \ingroup gpu
NVAPI_INTERFACE NvAPI_GPU_WorkstationFeatureQuery(
    __in NvPhysicalGpuHandle hPhysicalGpu,
...
//
//! \fn NvAPI_GPU_GetHDCPSupportStatus(NvPhysicalGpuHandle hPhysicalGpu,
//! NV_GPU_GET_HDCP_SUPPORT_STATUS *pGetHDCPSupportStatus) DESCRIPTION: This
//! function returns a GPU's HDCP support status.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
typedef struct {
    NvU32 version; //! Structure version constucted by macro
    ...
    NV_GPU_HDCP_KEY_SOURCE_STATE
    hdcpKeySourceState; //! GPU's HDCP key source state
} NV_GPU_GET_HDCP_SUPPORT_STATUS;

//! Macro for constructing the version for structure
//! NV_GPU_GET_HDCP_SUPPORT_STATUS
...
//!  \ingroup gpu
NVAPI_INTERFACE NvAPI_GPU_GetHDCPSupportStatus(
    NvPhysicalGpuHandle hPhysicalGpu,
...
//
//!   DESCRIPTION: This API retrieves the fan speed tachometer reading for the
//!   specified physical GPU.
//!
//!   HOW TO USE:
//!                 - NvU32 Value = 0;
...
//!
//! \param [in]    hPhysicalGpu   GPU selection.
//! \param [out]   pValue         Pointer to a variable to get the tachometer
//! reading
//!
//! \retval ::NVAPI_OK - completed request
//! \retval ::NVAPI_ERROR - miscellaneous error occurred
...
//! \retval ::NVAPI_HANDLE_INVALIDATED - handle passed has been invalidated (see
//! user guide)
//! \retval ::NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE - handle passed is not a
//! physical GPU handle
//!
...
//! \retval ::NVAPI_EXPECTED_PHYSICAL_GPU_HANDLE The provided GPU handle is not
//! a physical GPU handle.
//! \retval ::NVAPI_INVALID_HANDLE      The provided GPU handle is invalid.
//! \retval ::NVAPI_HANDLE_INVALIDATED  The provided GPU handle is no longer
//! valid.
//! \retval ::NVAPI_INVALID_POINTER     An invalid argument pointer was
//! provided.
...
typedef enum _NV_ECC_CONFIGURATION {
    ...
    NV_ECC_CONFIGURATION_IMMEDIATE, //!< Changes can optionally be made to take
                                    //!< effect immediately
...
typedef struct {
    ...
    NV_ECC_CONFIGURATION configurationOptions; //!< Supported ECC memory feature
...
#define NV_GPU_ECC_STATUS_INFO_VER \
    MAKE_NVAPI_VERSION( NV_GPU_ECC_STATUS_INFO, 1 )

//! \ingroup gpuecc
NVAPI_INTERFACE NvAPI_GPU_GetECCStatusInfo(
    NvPhysicalGpuHandle hPhysicalGpu,
...
//! \ingroup gpuecc
NVAPI_INTERFACE NvAPI_GPU_GetECCErrorInfo(
    NvPhysicalGpuHandle hPhysicalGpu,
...
//!          specific meaning for this API, they are listed below.
//!
//! \retval ::NVAPI_INVALID_USER_PRIVILEGE       - The caller does not have
...
typedef struct {
    NvU32 version; //! Structure version
    NvU32 isEnabled
        : 1; //! Current ECC configuration stored in non-volatile memory
...
#define NV_GPU_ECC_CONFIGURATION_INFO_VER \
    MAKE_NVAPI_VERSION( NV_GPU_ECC_CONFIGURATION_INFO, 1 )

//! \ingroup gpuecc
NVAPI_INTERFACE NvAPI_GPU_GetECCConfigurationInfo(
    NvPhysicalGpuHandle hPhysicalGpu,
...
//! \param [in]      bEnableImmediately   Request that the new setting take
//! effect immediately.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status. If there are return error codes with
//!          specific meaning for this API, they are listed below.
//!
//! \retval ::NVAPI_INVALID_CONFIGURATION  - Possibly SLI is enabled. Disable
//! SLI and retry.
//! \retval ::NVAPI_INVALID_USER_PRIVILEGE - The caller does not have
//! administrative privileges
//!
...
typedef struct {
    ...
    float* blendingTexture; //!< array of floating values building an intensity
                            //!< RGB texture
...
typedef struct {
    ...
    float* blendingTexture; //!< array of floating values building an intensity
                            //!< RGB texture
    float*
        offsetTexture; //!< array of floating values building an offset texture
    NvU32 offsetTexChannels; //!< number of channels per pixel in the offset
                             //!< texture
...
//! \ingroup gpu
#define NV_SCANOUT_INTENSITY_DATA_VER1 \
    MAKE_NVAPI_VERSION( NV_SCANOUT_INTENSITY_DATA_V1, 1 )
#define NV_SCANOUT_INTENSITY_DATA_VER2 \
...
//
//!   DESCRIPTION: This API enables and sets up per-pixel intensity feature on
//!   the specified display.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
//!
//! \param [in]   displayId              combined physical display and GPU
//! identifier of the display to apply the intensity control.
//! \param [in]   scanoutIntensityData   the intensity texture info.
//! \param [out]  pbSticky(OUT)           indicates whether the settings will be
//! kept over a reboot.
//!
//! \retval ::NVAPI_INVALID_ARGUMENT Invalid input parameters.
//! \retval ::NVAPI_API_NOT_INITIALIZED NvAPI not initialized.
//! \retval ::NVAPI_NOT_SUPPORTED Interface not supported by the driver used, or
//! only supported on selected GPUs
//! \retval ::NVAPI_INVALID_ARGUMENT Invalid input data.
//! \retval ::NVAPI_INCOMPATIBLE_STRUCT_VERSION NV_SCANOUT_INTENSITY_DATA
...
//! \ingroup gpu
#define NV_SCANOUT_INTENSITY_STATE_VER \
    MAKE_NVAPI_VERSION( NV_SCANOUT_INTENSITY_STATE_DATA, 1 )

///////////////////////////////////////////////////////////////////////////////
// FUNCTION NAME: NvAPI_GPU_GetScanoutIntensityState
//
//!   DESCRIPTION: This API queries current state of the intensity feature on
//!   the specified display.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
//!
//! \param [in]     displayId                       combined physical display
//! and GPU identifier of the display to query the configuration.
...
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetScanoutIntensityState(
...
typedef struct {
    ...
    NV_GPU_WARPING_VERTICE_FORMAT
    vertexFormat;        //!< format of the input vertices
    int numVertices;     //!< number of the input vertices
    NvSBox* textureRect; //!< rectangle in desktop coordinates describing the
...
//
//!   DESCRIPTION: This API enables and sets up the warping feature on the
//!   specified display.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
...
//! \param [out]   pbSticky                Indicates whether the settings will
//! be kept over a reboot.
//!
//! \retval ::NVAPI_INVALID_ARGUMENT Invalid input parameters.
//! \retval ::NVAPI_API_NOT_INITIALIZED NvAPI not initialized.
//! \retval ::NVAPI_NOT_SUPPORTED Interface not supported by the driver used, or
...
//! \ingroup gpu
#define NV_SCANOUT_WARPING_STATE_VER \
    MAKE_NVAPI_VERSION( NV_SCANOUT_WARPING_STATE_DATA, 1 )

///////////////////////////////////////////////////////////////////////////////
// FUNCTION NAME: NvAPI_GPU_GetScanoutWarpingState
//
//!   DESCRIPTION: This API queries current state of the warping feature on the
//!   specified display.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
//!
//! \param [in]     displayId                      combined physical display and
//! GPU identifier of the display to query the configuration.
...
//
//!   DESCRIPTION: This API queries the desktop and scanout portion of the
//!   specified display.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
...
//! \param [in,out] desktopRect        desktop area of the display in desktop
//! coordinates.
//! \param [in,out] scanoutRect        scanout area of the display relative to
//! desktopRect.
//!
//! \retval ::NVAPI_INVALID_ARGUMENT Invalid input parameters.
//! \retval ::NVAPI_API_NOT_INITIALIZED NvAPI not initialized.
...
typedef struct _NV_SCANOUT_INFORMATION {
    NvU32 version; //!< Structure version, needs to be initialized with
                   //!< NV_SCANOUT_INFORMATION_VER.

    NvSBox
        sourceDesktopRect; //!< Operating system display device rect in desktop
                           //!< coordinates displayId is scanning out from.
    NvSBox sourceViewportRect; //!< Area inside the sourceDesktopRect which is
                               //!< scanned out to the display.
    NvSBox targetViewportRect; //!< Area inside the rect described by
                               //!< targetDisplayWidth/Height sourceViewportRect
                               //!< is scanned out to.
    NvU32 targetDisplayWidth;  //!< Horizontal size of the active resolution
                               //!< scanned out to the display.
    NvU32 targetDisplayHeight; //!< Vertical size of the active resolution
                               //!< scanned out to the display.
    NvU32 cloneImportance;     //!< If targets are cloned views of the
                               //!< sourceDesktopRect the cloned targets have an
    //!< imporantce assigned (0:primary,1 secondary,...).
    NV_ROTATE sourceToTargetRotation; //!< Rotation performed between the
                                      //!< sourceViewportRect and the
                                      //!< targetViewportRect.
} NV_SCANOUT_INFORMATION;

#define NV_SCANOUT_INFORMATION_VER \
...
//! \param [in]     displayId            combined physical display and GPU
//! identifier of the display to query the configuration.
//! \param [in,out] pScanoutInformation  desktop area to displayId mapping
//! information.
//!
//! \return This API can return any of the error codes enumerated in
//! #NvAPI_Status.
...
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetScanoutConfigurationEx(
    __in NvU32 displayId,
...
//
//! DESCRIPTION:   This function retrieves - in NvU32 variable - reasons for the
//! current performance decrease.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//! \param [in]      hPhysicalGPU    (IN)    - GPU for which performance
//! decrease is to be evaluated.
//! \param [out]  pPerfDecrInfo    (OUT)    - Pointer to a NvU32 variable
...
//! \ingroup gpu
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetPerfDecreaseInfo( __in NvPhysicalGpuHandle
                                                   hPhysicalGpu,
...
//
//! \fn NvAPI_GPU_QueryIlluminationSupport(__inout
//! NV_GPU_QUERY_ILLUMINATION_SUPPORT_PARM *pIlluminationSupportInfo)
//! DESCRIPTION:   This function reports if the specified illumination attribute
//! is supported.
//!
//! \note Only a single GPU can manage an given attribute on a given HW element,
//!       regardless of how many are attatched. I.E. only one GPU will be used
//!       to control the brightness of the LED on an SLI bridge, regardless of
//!       how many are physicaly attached. You should enumerate thru the GPUs
//!       with this call to determine which GPU is managing the attribute.
//!
...
//!
//! \param [in]  hPhysicalGpu        Physical GPU handle
//! \param       Attribute           An enumeration value specifying the
//! Illumination attribute to be querried
//! \param [out] pSupported          A boolean indicating if the attribute is
//! supported.
//!
//! \return See \ref nvapistatus for the list of possible return values.
//
...
typedef struct _NV_GPU_QUERY_ILLUMINATION_SUPPORT_PARM_V1 {
    ...
    NvU32 version; //!< Version of this structure
    NvPhysicalGpuHandle
        hPhysicalGpu; //!< The handle of the GPU that you are checking for the
                      //!< specified attribute. note that this is the GPU that
                      //!< is managing the attribute. Only a single GPU can
                      //!< manage an given attribute on a given HW element,
                      //!< regardless of how many are attatched.
                      //!< I.E. only one GPU will be used to control the
                      //!< brightness of the LED on an SLI bridge, regardless of
                      //!< how many are physicaly attached. You enumerate thru
                      //!< the GPUs with this call to determine which GPU is
                      //!< managing the attribute.
    NV_GPU_ILLUMINATION_ATTRIB
    Attribute; //!< An enumeration value specifying the Illumination
...
//! \ingroup gpu
typedef NV_GPU_QUERY_ILLUMINATION_SUPPORT_PARM_V1
...
//! \ingroup gpu
#define NV_GPU_QUERY_ILLUMINATION_SUPPORT_PARM_VER \
...
//! *pIlluminationInfo) DESCRIPTION:   This function reports value of the
//! specified illumination attribute.
//!
//! \note Only a single GPU can manage an given attribute on a given HW element,
//!       regardless of how many are attatched. I.E. only one GPU will be used
//!       to control the brightness of the LED on an SLI bridge, regardless of
//!       how many are physicaly attached. You should enumerate thru the GPUs
//!       with the \ref NvAPI_GPU_QueryIlluminationSupport call to determine
...
//! \param       Attribute           An enumeration value specifying the
//! Illumination attribute to be querried
//! \param [out] Value               A DWORD containing the current value for
//! the specified attribute.
//!                                  This is specified as a percentage of the
//!                                  full range of the attribute (0-100; 0 =
//!                                  off, 100 = full brightness)
//!
//! \return See \ref nvapistatus for the list of possible return values. Return
//! values of special interest are:
//!             NVAPI_INVALID_ARGUMENT The specified attibute is not known to
//!             the driver. NVAPI_NOT_SUPPORTED:   The specified attribute is
//!             not supported on the specified GPU
//
//////////////////////////////////////////////////////////////////////////////
...
typedef struct _NV_GPU_GET_ILLUMINATION_PARM_V1 {
    ...
    NV_GPU_ILLUMINATION_ATTRIB
    Attribute; //!< An enumeration value specifying the Illumination
               //!< attribute to be querried. refer to enum \ref
               //!< NV_GPU_ILLUMINATION_ATTRIB.

    // OUT
    NvU32 Value; //!< A DWORD that will contain the current value of the
                 //!< specified attribute.
                 //! This is specified as a percentage of the full range of the
                 //! attribute (0-100; 0 = off, 100 = full brightness)
...
//! \ingroup gpu
#define NV_GPU_GET_ILLUMINATION_PARM_VER_1 \
...
//! \ingroup gpu
NVAPI_INTERFACE NvAPI_GPU_GetIllumination(
    NV_GPU_GET_ILLUMINATION_PARM* pIlluminationInfo );
...
//! *pIlluminationInfo) DESCRIPTION:   This function sets the value of the
//! specified illumination attribute.
//!
//! \note Only a single GPU can manage an given attribute on a given HW element,
//!       regardless of how many are attatched. I.E. only one GPU will be used
...
//! \param [in]  hPhysicalGpu        Physical GPU handle
//! \param       Attribute           An enumeration value specifying the
//! Illumination attribute to be set
//! \param       Value               The new value for the specified attribute.
//!                                  This should be specified as a percentage of
//!                                  the full range of the attribute (0-100; 0 =
//!                                  off, 100 = full brightness) If a value is
...
//! values of special interest are:
//!             NVAPI_INVALID_ARGUMENT	The specified attibute is not known to
//!             the driver, or the specified value is out of range.
//!             NVAPI_NOT_SUPPORTED     The specified attribute is not supported
...
typedef struct _NV_GPU_SET_ILLUMINATION_PARM_V1 {
    ...
    Attribute;   //!< An enumeration value specifying the Illumination
                 //!< attribute to be querried. refer to enum \ref
                 //!< NV_GPU_ILLUMINATION_ATTRIB.
    NvU32 Value; //!< A DWORD containing the new value for the specified
                 //!< attribute. This should be specified as a percentage of the
                 //!< full range of the attribute (0-100; 0 = off, 100 = full
                 //!< brightness) If a value is specified outside this range,
                 //!< NVAPI_INVALID_ARGUMENT will be returned.
...
//! \ingroup gpu
#define NV_GPU_SET_ILLUMINATION_PARM_VER_1 \
...
//! \ingroup gpu
NVAPI_INTERFACE NvAPI_GPU_SetIllumination(
    NV_GPU_SET_ILLUMINATION_PARM* pIlluminationInfo );
...
//!
//!                Note: Display handles can get invalidated on a modeset, so
//!                the calling applications need to renum the handles after
//!                every modeset.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//!
//! \retval NVAPI_INVALID_ARGUMENT        Either the handle pointer is NULL or
//! enum index too big
//! \retval NVAPI_OK                      Return a valid NvDisplayHandle based
//! on the enum index
...
//
//! This function returns the handle of the NVIDIA unattached display specified
//! by the enum
//!                index (thisEnum). The client should keep enumerating until it
//!                returns error.
//!                Note: Display handles can get invalidated on a modeset, so
...
//!
//! \param [in]  thisEnum                  The index of the NVIDIA display.
//! \param [out] pNvUnAttachedDispHandle   Pointer to the NVIDIA display handle
//! of the unattached display.
//!
//! \retval NVAPI_INVALID_ARGUMENT         Either the handle pointer is NULL or
//! enum index too big
//! \retval NVAPI_OK                       Return a valid NvDisplayHandle based
...
//! \ingroup disphandle
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_EnumNvidiaUnAttachedDisplayHandle(
    NvU32 thisEnum,
...
//
//! This function converts the unattached display handle to an active attached
//! display handle.
//!
//! At least one GPU must be present in the system and running an NVIDIA display
//! driver.
...
//! \retval NVAPI_NVIDIA_DEVICE_NOT_FOUND  No NVIDIA device maps to that display
//! name
//! \ingroup disphandle
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GetAssociatedNvidiaDisplayHandle(
...
//
//!   DESCRIPTION: This function returns the handle of an unattached NVIDIA
//!   display that is
//!                associated with the given display name (such as
//!                "\\DISPLAY1").
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//!
//! \retval ::NVAPI_INVALID_ARGUMENT          Either argument is NULL.
//! \retval ::NVAPI_OK                       *pNvUnAttachedDispHandle is now
//! valid.
//! \retval ::NVAPI_NVIDIA_DEVICE_NOT_FOUND   No NVIDIA device maps to that
//! display name.
//!
//! \ingroup disphandle
///////////////////////////////////////////////////////////////////////////////
...
//
//!  For a given NVIDIA display handle, this function returns a string (such as
//!  "\\.\DISPLAY1") to identify the display.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//
//!  This function returns the display name given, for example, "\\DISPLAY1",
//!  using the unattached NVIDIA display handle
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//!  The new refresh rate can be applied right away in this API call or deferred
//!  to be applied with the next OS modeset. The override is good for only one
//!  modeset (regardless whether it's deferred or immediate).
//!
//!
//! SUPPORTED OS:  Windows XP
//!
...
//! \since Release: 80
//!
//!  \param [in] hNvDisplay    The NVIDIA display handle. It can be
//!  NVAPI_DEFAULT_HANDLE or a handle
//!                           enumerated from NvAPI_EnumNVidiaDisplayHandle().
//!  \param [in] outputsMask  A set of bits that identify all target outputs
//!  which are associated with the NVIDIA
//!                           display handle to apply the refresh rate override.
//!                           When SLI is enabled, the outputsMask only applies
//!                           to the GPU that is driving the display output.
//!  \param [in] refreshRate  The override value. "0.0" means cancel the
//!  override.
//!  \param [in] bSetDeferred
//!              - "0": Apply the refresh rate override immediately in this API
//!              call.\p
...
//! \since Release: 90
//!
//! \param [in]  hNvDisplay  NVIDIA Display selection. It can be
//! NVAPI_DEFAULT_HANDLE or a handle enumerated from
//! NvAPI_EnumNVidiaDisplayHandle().
//! \param [out] outputId    The active display output ID associated with the
//! selected display handle hNvDisplay.
//!                          The outputid will have only one bit set. In the
//!                          case of Clone or Span mode, this will indicate the
//!                          display outputId of the primary display that the
//!                          GPU is driving. See \ref handles.
//!
//! \retval  NVAPI_OK                      Call successful.
//! \retval  NVAPI_NVIDIA_DEVICE_NOT_FOUND No NVIDIA GPU driving a display was
//! found.
//! \retval  NVAPI_EXPECTED_DISPLAY_HANDLE hNvDisplay is not a valid display
//! handle.
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
...
typedef struct {
    ...
    NvU32 isInternalDp
        : 1; //!< If the monitor is driven by an NV Dp transmitter
...
//
//! \fn NvAPI_GetDisplayPortInfo(__in_opt NvDisplayHandle hNvDisplay, __in NvU32
//! outputId, __inout NV_DISPLAY_PORT_INFO *pInfo) DESCRIPTION:     This
//! function returns the current DisplayPort-related information on the
//! specified device (monitor).
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \since Release: 165
//!
//! \param [in]     hvDisplay     NVIDIA Display selection. It can be
//! NVAPI_DEFAULT_HANDLE or a handle enumerated from
//! NvAPI_EnumNVidiaDisplayHandle().
//!                               This parameter is ignored when the outputId is
//!                               a NvAPI displayId.
//! \param [in]     outputId      This can either be the connection bit mask or
//! the NvAPI displayId. When the legacy connection bit mask is passed,
//!                               it should have exactly 1 bit set to indicate a
//!                               single display. If it's "0" then the default
//!                               outputId from
//!                               NvAPI_GetAssociatedDisplayOutputId() will be
//!                               used. See \ref handles.
...
//
//! \fn NvAPI_SetDisplayPort(NvDisplayHandle hNvDisplay, NvU32 outputId,
//! NV_DISPLAY_PORT_CONFIG *pCfg) DESCRIPTION:     This function sets up
//! DisplayPort-related configurations.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \since Release:   165
//!
//! \param [in]       hNvDisplay  NVIDIA display handle. It can be
//! NVAPI_DEFAULT_HANDLE or a handle enumerated from
//!                               NvAPI_EnumNVidiaDisplayHandle().
//! \param [in]       outputId    This display output ID, when it's "0" it means
//! the default outputId generated from the return of
//!                               NvAPI_GetAssociatedDisplayOutputId(). See \ref
//!                               handles.
//! \param [in]       pCfg        The display port config structure. If pCfg is
...
typedef struct {
    ...
    NvU32 isSetDeferred
        : 1; //!< Requires an OS modeset to finalize the setup if set
...
#define NV_DISPLAY_PORT_CONFIG_VER \
    MAKE_NVAPI_VERSION( NV_DISPLAY_PORT_CONFIG, 2 )
//! Macro for constructing the version field of NV_DISPLAY_PORT_CONFIG
#define NV_DISPLAY_PORT_CONFIG_VER_1 \
    MAKE_NVAPI_VERSION( NV_DISPLAY_PORT_CONFIG, 1 )
//! Macro for constructing the version field of NV_DISPLAY_PORT_CONFIG
#define NV_DISPLAY_PORT_CONFIG_VER_2 \
    MAKE_NVAPI_VERSION( NV_DISPLAY_PORT_CONFIG, 2 )
//! @}
...
//
//! \fn NvAPI_GetHDMISupportInfo(__in_opt NvDisplayHandle hNvDisplay, __in NvU32
//! outputId, __inout NV_HDMI_SUPPORT_INFO *pInfo)
//!   This API returns the current infoframe data on the specified
//!   device(monitor).
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
...
//! NVAPI_DEFAULT_HANDLE or a handle enumerated from
//! NvAPI_EnumNVidiaDisplayHandle().
//!                         This parameter is ignored when the outputId is a
//!                         NvAPI displayId.
//! \param [in]  outputId   This can either be the connection bit mask or the
//! NvAPI displayId. When the legacy connection bit mask is passed,
//!                         it should have exactly 1 bit set to indicate a
//!                         single display. If it's "0" then the default
//!                         outputId from NvAPI_GetAssociatedDisplayOutputId()
//!                         will be used. See \ref handles.
...
typedef struct {
    ...
    NvU32 isMonHDMI
        : 1; //!< If the monitor is HDMI (with IEEE's HDMI registry ID)
...
typedef enum {
    NV_INFOFRAME_CMD_GET_DEFAULT =
        0, //!< Returns the fields in the infoframe with values set by the
           //!< manufacturer - NVIDIA/OEM.
    NV_INFOFRAME_CMD_RESET, //!< Sets the fields in the infoframe to auto, and
                            //!< infoframe to the default infoframe for use in a
                            //!< set.
    NV_INFOFRAME_CMD_GET,   //!< Get the current infoframe state.
    NV_INFOFRAME_CMD_SET,   //!< Set the current infoframe state (flushed to the
                            //!< monitor), the values are one time and do not
                            //!< persist.
    NV_INFOFRAME_CMD_GET_OVERRIDE, //!< Get the override infoframe state,
                                   //!< non-override fields will be set to value
                                   //!< = AUTO, overridden fields will have the
                                   //!< current override values.
    NV_INFOFRAME_CMD_SET_OVERRIDE, //!< Set the override infoframe state,
                                   //!< non-override fields will be set to value
                                   //!< = AUTO, other values indicate override;
                                   //!< persist across modeset/reboot
    NV_INFOFRAME_CMD_GET_PROPERTY, //!< get properties associated with infoframe
                                   //!< (each of the infoframe type will have
                                   //!< properties)
...
typedef enum {
    NV_INFOFRAME_PROPERTY_MODE_AUTO =
        0, //!< Driver determines whether to send infoframes.
    ...
    NV_INFOFRAME_PROPERTY_MODE_ALLOW_OVERRIDE, //!< Driver only sends infoframe
                                               //!< when client requests it via
                                               //!< infoframe escape call.
} NV_INFOFRAME_PROPERTY_MODE;

//! Returns whether the current monitor is in blacklist or force this monitor to
//! be in blacklist.
typedef enum {
...
typedef struct {
    ...
    union {
        NV_INFOFRAME_PROPERTY
        property; //!< This is NVIDIA-specific and corresponds to the
                  //!< property cmds and associated infoframe.
...
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Disp_InfoFrameControl(
    __in NvU32 displayId,
...
//!
//! \param [in]     displayId         Monitor Identifier
//! \param [in,out] pColorData        Contains data corresponding to color
//! information
//!
...
NVAPI_INTERFACE NvAPI_Disp_ColorControl( NvU32 displayId,
...
typedef struct {
    ...
    union {
        ...
        NvU32 ceaId
            : 8; //!< The EIA/CEA 861B/D predefined short timing descriptor ID.
...
typedef struct _NV_TIMING_INPUT {
    ...
    NV_TIMING_FLAG
    flag; //!< Flag containing additional info for timing calculation.

    NV_TIMING_OVERRIDE
    type; //!< Timing type(formula) to use for calculating the timing
} NV_TIMING_INPUT;
...
//
//! DESCRIPTION:  This function calculates the timing from the visible
...
//
//! \fn NvAPI_DISP_GetMonitorCapabilities(NvU32 displayId,
//! NV_MONITOR_CAPABILITIES *pMonitorCapabilities) DESCRIPTION:     This API
...
#define NV_MONITOR_CAPABILITIES_VER \
    MAKE_NVAPI_VERSION( NV_MONITOR_CAPABILITIES, 1 )
...
//!
//! \ingroup dispcontrol
NVAPI_INTERFACE NvAPI_DISP_GetMonitorCapabilities(
    __in NvU32 displayId,
...
typedef struct _NV_MONITOR_COLOR_DATA {
    NvU32 version;
    // We are only supporting DP monitors for now. We need to extend this to
    // HDMI panels as well
...
//! \ingroup dispcontrol
#define NV_MONITOR_COLOR_CAPS_VER1 \
...
//
//! DESCRIPTION:    This API returns all the color formats and bit depth values
//! supported by a given DP monitor.
//!
//! USAGE:         Sequence of calls which caller should make to get the
//! information.
//!                1. First call NvAPI_DISP_GetMonitorColorCapabilities() with
//!                pMonitorColorCapabilities as NULL to get the count.
//!                2. Allocate memory for color caps(NV_MONITOR_COLOR_CAPS)
//!                array.
//!                3. Call NvAPI_DISP_GetMonitorColorCapabilities() again with
//!                the pointer to the memory allocated to get all the
//!                   color capabilities.
//!
//!                Note :
//!                1. pColorCapsCount should never be NULL, else the API will
//!                fail with NVAPI_INVALID_ARGUMENT.
//!                2. *pColorCapsCount returned from the API will always be the
//!                actual count in any/every call.
//!                3. Memory size to be allocated should be (*pColorCapsCount *
//!                sizeof(NV_MONITOR_COLOR_CAPS)).
//!                4. If the memory allocated is less than what is required to
//!                return all the timings, this API will return the
//!                   amount of information which can fit in user provided
//!                   buffer and API will return NVAPI_INSUFFICIENT_BUFFER.
//!                5. If the caller specifies a greater value for
//!                *pColorCapsCount in second call to
//!                NvAPI_DISP_GetMonitorColorCapabilities()
//!                   than what was returned from first call, the API will
//!                   return only the actual number of elements in the color
//!                   capabilities array and the extra buffer will remain
//!                   unused.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
//! \param [in]      displayId                  Monitor Identifier
//! \param [in, out] pMonitorColorCapabilities  The monitor color capabilities
//! information
//! \param [in, out] pColorCapsCount            - During input, the number of
//! elements allocated for the pMonitorColorCapabilities pointer
//!                                             - During output, the actual
//!                                             number of color data elements
...
//!            specific meaning for this API, they are listed below.
//!
//! \retval         NVAPI_INSUFFICIENT_BUFFER   The input buffer size is not
//! sufficient to hold the total contents. In this case
//!                                             *pColorCapsCount will hold the
//!                                             required amount of elements.
//! \retval         NVAPI_INVALID_DISPLAY_ID    The input monitor is either not
//! connected or is not a DP panel.
//!
//! \ingroup dispcontrol
//!
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DISP_GetMonitorColorCapabilities(
    __in NvU32 displayId,
...
typedef struct {
    ...
    NV_VIEWPORTF srcPartition; //!< For multimon support, should be set to
                               //!< (0,0,1.0,1.0) for now.
    ...
    NV_TIMING
    timing; //!< Timing used to program TMDS/DAC/LVDS/HDMI/TVEncoder, etc.
    NvU32 hwModeSetOnly
        : 1; //!< If set, it means a hardware modeset without OS update
...
//
//! DESCRIPTION:   This API enumerates the custom timing specified by the enum
//! index.
//!				   The client should keep enumerating until it
//! returns NVAPI_END_ENUMERATION.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! #NvAPI_Status. If there are return error codes with
//!                specific meaning for this API, they are listed below.
//! \retval        NVAPI_INVALID_DISPLAY_ID:   Custom Timing is not supported on
//! the Display, whose display id is passed
//!
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DISP_EnumCustomDisplay(
    __in NvU32 displayId,
    __in NvU32 index,
...
//
//! DESCRIPTION:    This API is used to set up a custom display without saving
//! the configuration on multiple displays.
//!
//! \note
//!  All the members of srcPartition, present in NV_CUSTOM_DISPLAY structure,
//!  should have their range in (0.0,1.0). In clone mode the timings can applied
//!  to both the target monitors but only one target at a time. \n For the
//!  secondary target the applied timings works under the following conditions:
//!  - If the secondary monitor EDID supports the selected timing, OR
//!  - If the selected custom timings can be scaled by the secondary monitor for
//!  the selected source resolution on the primary, OR
//!  - If the selected custom timings matches the existing source resolution on
//!  the primary. Setting up a custom display on non-active but connected
//!  monitors is supported only for Win7 and above.
//!
...
//!
//!
//! \param [in]    pDisplayIds    Array of the target display Dispaly IDs - See
//! \ref handles.
//! \param [in]    count          Total number of the incoming Display IDs and
...
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DISP_TryCustomDisplay( __in_ecount( count )
                                                 NvU32* pDisplayIds,
                                             __in NvU32 count,
                                             __in_ecount( count )
...
//
//! DESCRIPTION:    This function deletes the custom display configuration,
...
//!
//!
//! \param [in]     pDisplayIds    Array of Dispaly IDs on which custom display
//! configuration is to be saved.
//! \param [in]     count          Total number of the incoming Dispaly IDs.
//! This is for the multi-head support.
//!	\param [in]     pCustDisp	   Pointer to the NV_CUSTOM_DISPLAY
//! structure
//!
//! \return        This API can return any of the error codes enumerated in
//! #NvAPI_Status. If there are return error codes with
...
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DISP_DeleteCustomDisplay(
    __in_ecount( count ) NvU32* pDisplayIds,
    __in NvU32 count,
...
//
//! DESCRIPTION:    This function saves the current hardware display
//! configuration on the specified Display IDs as a custom display
//! configuration.
//!                 This function should be called right after
//!                 NvAPI_DISP_TryCustomDisplay() to save the custom display
//!                 from the current hardware context. This function will not do
//!                 anything if the custom display configuration is not tested
//!                 on the hardware.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//!
//!
//! \param [in]     pDisplayIds         Array of Dispaly IDs on which custom
//! display configuration is to be saved.
//! \param [in]     count               Total number of the incoming Dispaly
//! IDs. This is for the multi-head support.
//! \param [in]     isThisOutputIdOnly  If set, the saved custom display will
//! only be applied on the monitor with the same outputId (see \ref handles).
//! \param [in]     isThisMonitorIdOnly If set, the saved custom display will
//! only be applied on the monitor with the same EDID ID or
//!                                     the same TV connector in case of analog
//!                                     TV.
//!
//! \return        This API can return any of the error codes enumerated in
//! #NvAPI_Status. If there are return error codes with
...
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DISP_SaveCustomDisplay( __in_ecount( count )
                                                  NvU32* pDisplayIds,
                                              __in NvU32 count,
...
//
//! DESCRIPTION:    This API is used to restore the display configuration, that
//! was changed by calling NvAPI_DISP_TryCustomDisplay(). This function
//!                 must be called only after a custom display configuration is
//!                 tested on the hardware, using NvAPI_DISP_TryCustomDisplay(),
//!                 otherwise no action is taken. On Vista,
//!                 NvAPI_DISP_RevertCustomDisplayTrial should be called with an
//!                 active display that was affected during the
//!                 NvAPI_DISP_TryCustomDisplay() call, per GPU.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
...
//! \param [in]    count         Total number of incoming Display IDs. For
//! future use only. Currently it is expected to be passed as 1.
//!
//! \return        This API can return any of the error codes enumerated in
//! #NvAPI_Status. If there are return error codes with
//!                specific meaning for this API, they are listed below.
...
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DISP_RevertCustomDisplayTrial( __in_ecount( count )
                                                         NvU32* pDisplayIds,
                                                     __in NvU32 count );
...
//
//! This API lets caller retrieve the target display arrangement for selected
//! source display handle.
//! \note Display PATH with this API is limited to single GPU. DUALVIEW across
//! GPUs will be returned as STANDARD VIEW.
//!       Use NvAPI_SYS_GetDisplayTopologies() to query views across GPUs.
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_DISP_GetDisplayConfig. SUPPORTED OS:  Windows Vista and
//! higher
//!
//!
//! \since Release: 85
...
//!  #NVAPI_DEFAULT_HANDLE or a handle enumerated from
//!                                         NvAPI_EnumNVidiaDisplayHandle().
//!  \param [out]    pTargets               User allocated storage to retrieve
//!  an array of  NV_VIEW_TARGET_INFO. Can be NULL to retrieve
//!                                         the targetCount.
//!  \param [in,out] targetMaskCount        Count of target device mask
//!  specified in pTargetMask.
//!  \param [out]    targetView             Target view selected from
//!  NV_TARGET_VIEW_MODE.
//!
//!  \retval         NVAPI_OK               Completed request
//!  \retval         NVAPI_ERROR            Miscellaneous error occurred
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_DISP_GetDisplayConfig." ) NVAPI_INTERFACE
    NvAPI_GetView( NvDisplayHandle hNvDisplay,
...
//
//! DESCRIPTION:    This API lets caller retrieve the target display arrangement
//! for selected source display handle.
//!                 \note Display PATH with this API is limited to single GPU.
//!                 DUALVIEW across GPUs will be returned as STANDARD VIEW.
//!                       Use NvAPI_SYS_GetDisplayTopologies() to query views
//!                       across GPUs.
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_DISP_GetDisplayConfig. SUPPORTED OS:  Windows Vista and
...
//! #NVAPI_DEFAULT_HANDLE is not allowed, it has to be a handle enumerated with
//!                                  NvAPI_EnumNVidiaDisplayHandle().
//! \param [in,out] pPathInfo        Count field should be set to
//! NVAPI_MAX_DISPLAY_PATH. Can be NULL to retrieve just the pathCount.
//! \param [in,out] pPathCount       Number of elements in array
//! pPathInfo->path.
//! \param [out]    pTargetViewMode  Display view selected from
//! NV_TARGET_VIEW_MODE.
//!
//! \retval         NVAPI_OK                      Completed request
//! \retval         NVAPI_API_NOT_INTIALIZED      NVAPI not initialized
//! \retval         NVAPI_ERROR                   Miscellaneous error occurred
//! \retval         NVAPI_INVALID_ARGUMENT        Invalid input parameter.
//! \retval         NVAPI_EXPECTED_DISPLAY_HANDLE hNvDisplay is not a valid
//! display handle.
//!
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_DISP_GetDisplayConfig." ) NVAPI_INTERFACE
    NvAPI_GetViewEx( NvDisplayHandle hNvDisplay,
                     NV_DISPLAY_PATH_INFO* pPathInfo,
                     NvU32* pPathCount,
...
//
//!  This API lets caller enumerate all the supported NVIDIA display views -
//!  nView and Dualview modes.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//!  #NVAPI_DEFAULT_HANDLE or a handle enumerated from
//!                                         NvAPI_EnumNVidiaDisplayHandle().
//!  \param [out]    pTargetViews           Array of supported views. Can be
//!  NULL to retrieve the pViewCount first.
...
//! retval ::NVAPI_INVALID_ARGUMENT:            One or more args passed in are
//! invalid. retval ::NVAPI_API_NOT_INTIALIZED:          The NvAPI API needs to
//! be initialized first retval ::NVAPI_NO_IMPLEMENTATION:           This
//! entrypoint not available retval ::NVAPI_ERROR: Miscellaneous error occurred
//!
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
...
//!                  configuration.
//!       USAGE:     The caller might have to call this three times to fetch all
//!       the required configuration details as follows:
//!                  First  Pass: Caller should Call
//!                  NvAPI_DISP_GetDisplayConfig() with pathInfo set to NULL to
//!                  fetch pathInfoCount. Second Pass: Allocate memory for
//!                  pathInfo with respect to the number of pathInfoCount(from
//!                  First Pass) to fetch
//!                               targetInfoCount. If sourceModeInfo is needed
//!                               allocate memory or it can be initialized to
//!                               NULL.
//!             Third  Pass(Optional, only required if target information is
//!             required): Allocate memory for targetInfo with respect
//!                               to number of targetInfoCount(from Second
//!                               Pass).
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
//! \param [in,out]  pathInfoCount    Number of elements in pathInfo array,
...
//!            specific meaning for this API, they are listed below.
//!
//! \retval    NVAPI_INVALID_ARGUMENT  -   Invalid input parameter. Following
//! can be the reason for this return value:
//!                                        -# pathInfoCount is NULL.
//!                                        -# *pathInfoCount is 0 and pathInfo
//!                                        is not NULL.
//!                                        -# *pathInfoCount is not 0 and
//!                                        pathInfo is NULL.
//! \retval    NVAPI_DEVICE_BUSY       -   ModeSet has not yet completed. Please
...
NVAPI_INTERFACE NvAPI_DISP_GetDisplayConfig(
    ...
    __out_ecount_full_opt( *pathInfoCount )
...
//!
//!                  If all sourceIds are zero, then NvAPI will pick up
//!                  sourceId's based on the following criteria :
//!                  - If user provides sourceModeInfo then we are trying to
//!                  assign 0th sourceId always to GDIPrimary.
//!                     This is needed since active windows always moves along
//!                     with 0th sourceId.
//!                  - For rest of the paths, we are incrementally assigning the
//!                  sourceId per adapter basis.
//!                  - If user doesn't provide sourceModeInfo then NVAPI just
//!                  picks up some default sourceId's in incremental order. Note
//!                  : NVAPI will not intelligently choose the sourceIDs for any
...
//! \ingroup dispcontrol
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DISP_SetDisplayConfig(
...
#define NV_MOSAIC_TOPO_VALIDITY_MISSING_GPU \
    0x00000001 //!< Not enough SLI GPUs were found to fill the entire
               //! topology. hPhysicalGPU will be 0 for these.
#define NV_MOSAIC_TOPO_VALIDITY_MISSING_DISPLAY \
    0x00000002 //!< Not enough displays were found to fill the entire
               //! topology. displayOutputId will be 0 for these.
#define NV_MOSAIC_TOPO_VALIDITY_MIXED_DISPLAY_TYPES \
    0x00000004 //!< The topoogy is only possible with displays of the same
...
typedef struct {
    ...
    NvU32
        validityMask; //!< 0 means topology is valid with the current hardware.
                      //! If not 0, inspect bits against
                      //! NV_MOSAIC_TOPO_VALIDITY_*.
    NvU32 rowCount;   //!< Number of displays in a row
    NvU32 colCount;   //!< Number of displays in a column

    struct {
        NvPhysicalGpuHandle hPhysicalGPU; //!< Physical GPU to be used in the
                                          //!< topology (0 if GPU missing)
        NvU32 displayOutputId; //!< Connected display target (0 if no display
                               //!< connected)
        NvS32
            overlapX; //!< Pixels of overlap on left of target: (+overlap, -gap)
        NvS32
            overlapY; //!< Pixels of overlap on top of target: (+overlap, -gap)

    } gpuLayout[ NVAPI_MAX_MOSAIC_DISPLAY_ROWS ]
               [ NVAPI_MAX_MOSAIC_DISPLAY_COLUMNS ];

} NV_MOSAIC_TOPO_DETAILS;

//! Macro for constructing te vesion field of NV_MOSAIC_TOPO_DETAILS
#define NVAPI_MOSAIC_TOPO_DETAILS_VER \
    MAKE_NVAPI_VERSION( NV_MOSAIC_TOPO_DETAILS, 1 )

//
//! These values refer to the different types of Mosaic topologies that are
//! possible.  When getting the supported Mosaic topologies, you can specify one
//! of these types to narrow down the returned list to only those that match the
//! given type.
typedef enum {
    ...
    NV_MOSAIC_TOPO_TYPE_PASSIVE_STEREO_SCALED_CLONE, //!< Not supported at this
                                                     //!< time
...
//!
//! Using a "Basic" topology combines multiple monitors to create a single
//! desktop.
//!
//! Using a "Passive" topology combines multiples monitors to create a passive
//! stereo desktop. In passive stereo, two identical topologies combine - one
//! topology is used for the right eye and the other identical //! topology
//! (targeting different displays) is used for the left eye.  \n NOTE:
//! common\inc\nvEscDef.h shadows a couple PASSIVE_STEREO enums.  If this
//!       enum list changes and effects the value of
//!       NV_MOSAIC_TOPO_BEGIN_PASSIVE_STEREO please update the corresponding
//!       value in nvEscDef.h
typedef enum {
    NV_MOSAIC_TOPO_NONE,
    ...
    //
    // The result of using one of these Mosaic topos is that multiple monitors
    // will combine to create a single PASSIVE STEREO desktop.  What this means
    // is that there will be two topos that combine to create the overall
    // desktop. One topo will be used for the left eye, and the other topo (of
    // the same rows x cols), will be used for the right eye.  The difference
    // between the two topos is that different GPUs and displays will be used.
    //
...
//
//! This is a "topology brief" structure.  It tells you what you need to know
//! about a topology at a high level. A list of these is returned when you query
//! for the supported Mosaic information.
//!
...
#define NVAPI_MOSAIC_TOPO_BRIEF_VER \
    MAKE_NVAPI_VERSION( NV_MOSAIC_TOPO_BRIEF, 1 )
...
#define NVAPI_MOSAIC_DISPLAY_SETTING_VER \
    MAKE_NVAPI_VERSION( NV_MOSAIC_DISPLAY_SETTING, 1 )
...
#define NV_MOSAIC_DISPLAY_SETTINGS_MAX \
    40 //!< Set a reasonable maximum number of display settings to support
       //! so arrays are bound.
...
typedef struct {
    ...
    NV_MOSAIC_TOPO_BRIEF
    topoBriefs[ NV_MOSAIC_TOPO_MAX ]; //!< List of supported topologies with
                                      //!< only brief details
    NvU32 displaySettingsCount; //!< Number of display settings in below array
    NV_MOSAIC_DISPLAY_SETTING
    displaySettings[ NV_MOSAIC_DISPLAY_SETTINGS_MAX ]; //!< List of per
                                                       //!< display settings
                                                       //!< possible
...
#define NVAPI_MOSAIC_SUPPORTED_TOPO_INFO_VER \
    MAKE_NVAPI_VERSION( NV_MOSAIC_SUPPORTED_TOPO_INFO, 1 )
...
//
//! This structure defines a group of topologies that work together to create
//! one overall layout.  All of the supported topologies are represented with
//! this structure.
//!
//! For example, a 'Passive Stereo' topology would be represented with this
//! structure, and would have separate topology details for the left and right
//! eyes. The count would be 2.  A 'Basic' topology is also represented by this
//! structure, with a count of 1.
//!
//! The structure is primarily used internally, but is exposed to applications
//! in a read-only fashion because there are some details in it that might be
//! useful (like the number of rows/cols, or connected display information).  A
//! user can get the filled-in structure by calling NvAPI_Mosaic_GetTopoGroup().
//!
//! You can then look at the detailed values within the structure.  There are no
//! entrypoints which take this structure as input (effectively making it
//! read-only).
typedef struct {
    NvU32 version;              //!< Version of this structure
...
#define NVAPI_MOSAIC_TOPO_GROUP_VER \
    MAKE_NVAPI_VERSION( NV_MOSAIC_TOPO_GROUP, 1 )
...
//
//! DESCRIPTION:     This API returns information on the topologies and display
//! resolutions
...
//!                  NvAPI_Mosaic_GetTopoGroup() with one of the Mosaic
//!                  topologies if you need more information about it.
//!
//!     <b>'IN' Notes:</b>  pSupportedTopoInfo->version must be set before
//!     calling this function.
//!                  If the specified version is not supported by this
//!                  implementation, an error will be returned
//!                  (NVAPI_INCOMPATIBLE_STRUCT_VERSION).
//!
//!     <b>'OUT' Notes:</b> Some of the topologies returned might not be valid
//!     for one reason or
//!                  another.  It could be due to mismatched or missing
//!                  displays.  It could also be because the required number of
//!                  GPUs is not found. At a high level, you can see if the
//!                  topology is valid and can be enabled by looking at the
//!                  pSupportedTopoInfo->topoBriefs[xxx].isPossible flag. If
//!                  this is true, the topology can be enabled. If it is false,
//!                  you can find out why it cannot be enabled by getting the
//!                  details of the topology via NvAPI_Mosaic_GetTopoGroup().
//!                  From there, look at the validityMask of the individual
//!                  topologies.  The bits can be tested against the
//!                  NV_MOSAIC_TOPO_VALIDITY_* bits.
//!
//!                  It is possible for this function to return NVAPI_OK with no
//!                  topologies listed in the return structure.  If this is the
//!                  case, it means that the current hardware DOES support
//!                  Mosaic, but with the given configuration no valid
//!                  topologies were found.  This most likely means that SLI was
//!                  not enabled for the hardware. Once enabled, you should see
//!                  valid topologies returned from this function.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//!
//!
//! \param [in,out]  pSupportedTopoInfo  Information about what topologies and
//! display resolutions
//!                                      are supported for Mosaic.
//! \param [in]      type                The type of topologies the caller is
//! interested in
//!                                      getting. See NV_MOSAIC_TOPO_TYPE for
//!                                      possible values.
//!
//! \retval ::NVAPI_OK                          No errors in returning supported
//! topologies.
//! \retval ::NVAPI_NOT_SUPPORTED               Mosaic is not supported with the
//! existing hardware.
//! \retval ::NVAPI_INVALID_ARGUMENT            One or more arguments passed in
//! are invalid.
//! \retval ::NVAPI_API_NOT_INTIALIZED          The NvAPI API needs to be
//! initialized first.
//! \retval ::NVAPI_NO_IMPLEMENTATION           This entrypoint not available.
//! \retval ::NVAPI_INCOMPATIBLE_STRUCT_VERSION The version of the structure
//! passed in is not
...
//! \ingroup mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Mosaic_GetSupportedTopoInfo(
...
//!                  is obviously not enabled), and the overlap values will be
//!                  0.
//!
//!     <b>'IN' Notes:</b>  pTopoGroup->version must be set before calling this
//!     function.
//!                  If the specified version is not supported by this
//!                  implementation, an error will be returned
//!                  (NVAPI_INCOMPATIBLE_STRUCT_VERSION).
...
//!
//! \retval ::NVAPI_OK                          Details were retrieved
//! successfully.
//! \retval ::NVAPI_NOT_SUPPORTED               Mosaic is not supported with the
//! existing hardware.
//! \retval ::NVAPI_INVALID_ARGUMENT            One or more argumentss passed in
//! are invalid.
//! \retval ::NVAPI_API_NOT_INTIALIZED          The NvAPI API needs to be
//! initialized first.
//! \retval ::NVAPI_NO_IMPLEMENTATION           This entrypoint not available.
...
//!                  the given Mosaic topology and display settings are to be
//!                  used.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//!                                   returned from
//!                                   NvAPI_Mosaic_GetSupportedTopoInfo().
//! \param [in]   pDisplaySetting     The display settings for getting the
//! limits.
...
//! \ingroup mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Mosaic_GetOverlapLimits(
...
//
//! DESCRIPTION:     This API sets the Mosaic topology and performs a mode
//! switch
//!                  using the given display settings.
//!
//!                  If NVAPI_OK is returned, the current Mosaic topology was
//!                  set correctly.  Any other status returned means the
...
//! \since Release: 185
//!
//! \param [in]     pTopoBrief        The topology to set. This must be one of
//! the topologies returned from
//!                                   NvAPI_Mosaic_GetSupportedTopoInfo(), and
//!                                   it must have an isPossible value of 1.
//! \param [in]     pDisplaySetting   The per display settings to be used in the
//! Mosaic mode. This must be one of the
//!                                   settings returned from
//!                                   NvAPI_Mosaic_GetSupportedTopoInfo().
//! \param [in]     overlapX          The pixel overlap to use between
//! horizontal displays (use positive a number for
//!                                   overlap, or a negative number to create a
//!                                   gap.) If the overlap is out of bounds for
//!                                   what is possible given the topo and
//!                                   display setting, the overlap will be
//!                                   clamped.
//! \param [in]     overlapY          The pixel overlap to use between vertical
...
//!                                   display setting, the overlap will be
//!                                   clamped.
//! \param [in]     enable            If 1, the topology being set will also be
//! enabled, meaning that the mode set will
//!                                   occur.  \n
//!                                   If 0, you don't want to be in Mosaic mode
//!                                   right now, but want to set the current
//!                                   Mosaic topology so you can enable it later
//!                                   with NvAPI_Mosaic_EnableCurrentTopo().
//!
//! \retval  ::NVAPI_OK                          The Mosaic topology was set.
//! \retval  ::NVAPI_NOT_SUPPORTED               Mosaic is not supported with
//! the existing hardware.
//! \retval  ::NVAPI_INVALID_ARGUMENT            One or more argumentss passed
//! in are invalid.
//! \retval  ::NVAPI_TOPO_NOT_POSSIBLE           The topology passed in is not
//! currently possible.
//! \retval  ::NVAPI_API_NOT_INTIALIZED          The NvAPI API needs to be
//! initialized first.
//! \retval  ::NVAPI_NO_IMPLEMENTATION           This entrypoint not available.
...
//!                                              entrypoint.
//! \retval  ::NVAPI_MODE_CHANGE_FAILED          There was an error changing the
//! display mode.
//! \retval  ::NVAPI_ERROR                       Miscellaneous error occurred.
//!
//! \ingroup mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Mosaic_SetCurrentTopo(
    NV_MOSAIC_TOPO_BRIEF* pTopoBrief,
    NV_MOSAIC_DISPLAY_SETTING* pDisplaySetting,
    NvS32 overlapX,
    NvS32 overlapY,
...
//
//! DESCRIPTION:     This API returns information for the current Mosaic
//! topology.
//!                  This includes topology, display settings, and overlap
//!                  values.
//!
//!                  You can call NvAPI_Mosaic_GetTopoGroup() with the topology
//!                  if you require more information.
//!
//!                  If there isn't a current topology, then pTopoBrief->topo
//!                  will be NV_MOSAIC_TOPO_NONE.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//! \param [out]     pOverlapX        The pixel overlap between horizontal
//! displays
//! \param [out]     pOverlapY        The pixel overlap between vertical
//! displays
//!
...
//! \ingroup mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Mosaic_GetCurrentTopo(
    NV_MOSAIC_TOPO_BRIEF* pTopoBrief,
    NV_MOSAIC_DISPLAY_SETTING* pDisplaySetting,
    NvS32* pOverlapX,
    NvS32* pOverlapY );
...
//!
//!                  An "enable" setting enables the current (previously set)
//!                  Mosaic topology. Note that when the current Mosaic topology
//!                  is retrieved, it must have an isPossible value of 1 or an
//!                  error will occur.
//!
...
//! \since Release: 185
//!
//! \param [in]   enable               1 to enable the current Mosaic topo, 0 to
//! disable it.
//!
//! \retval ::NVAPI_OK                 The Mosaic topo was enabled/disabled.
//! \retval ::NVAPI_NOT_SUPPORTED      Mosaic is not supported with the existing
//! hardware.
//! \retval ::NVAPI_INVALID_ARGUMENT   One or more arguments passed in are
//! invalid.
//! \retval ::NVAPI_TOPO_NOT_POSSIBLE  The current topology is not currently
//! possible.
//! \retval ::NVAPI_MODE_CHANGE_FAILED There was an error changing the display
//! mode.
//! \retval ::NVAPI_ERROR:             Miscellaneous error occurred.
//!
//! \ingroup mosaicapi
...
typedef struct {
    ...
    NvU32 applyWithBezelCorrect
        : 1; //!< When enabling and doing the modeset, do we switch to the
             //!< bezel-corrected resolution
    NvU32 immersiveGaming : 1; //!< Enable as immersive gaming instead of Mosaic
                               //!< SLI (for Quadro-boards only)
    NvU32 baseMosaic : 1;      //!< Enable as Base Mosaic (Panoramic) instead of
                               //!< Mosaic SLI (for NVS and Quadro-boards only)
    NvU32 driverReloadAllowed
        : 1; //!< If necessary, reloading the driver is permitted (for Vista and
             //!< above only). Will not be persisted. Value undefined on get.
    NvU32 acceleratePrimaryDisplay
        : 1; //!< Enable SLI acceleration on the primary display while in
             //!< single-wide mode (For Immersive Gaming only). Will not be
             //!< persisted. Value undefined on get.
    NvU32 reserved : 27; //!< Reserved, must be 0
    NV_MOSAIC_GRID_TOPO_DISPLAY
    displays[ NV_MOSAIC_MAX_DISPLAYS ];        //!< Displays are done as [(row *
                                               //!< columns) + column]
...
//
//! DESCRIPTION:     Sets a new display topology, replacing any existing
//! topologies
...
//!
//!                  To revert to a single display, specify that display as a
//!                  1x1 grid.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
...
//! \param [in]      gridCount          The number of elements in the
//! pGridTopologies array.
//! \param [in]      setTopoFlags       Zero or more of the
...
//! initialized first
//! \retval ::NVAPI_NO_IMPLEMENTATION           This entrypoint not available
//! \retval ::NVAPI_NO_ACTIVE_SLI_TOPOLOGY      No matching GPU topologies could
//! be found.
//! \retval ::NVAPI_TOPO_NOT_POSSIBLE           One or more of the display grids
//! are not valid.
//! \retval ::NVAPI_ERROR                       Miscellaneous error occurred
//! \ingroup mosaicapi
///////////////////////////////////////////////////////////////////////////////
...
//! Do not allow a driver reload. That is, stick with the same master GPU as
//! well as the same SLI configuration.
...
NVAPI_INTERFACE NvAPI_Mosaic_SetDisplayGrids(
...
typedef struct {
    ...
    NvU32 warningFlags; //!< (OUT) Any of the NV_MOSAIC_DISPLAYTOPO_WARNING_*
                        //!< flags.
    ...
    struct {
        ...
        NvU32 errorFlags;   //!< (OUT) Any of the NV_MOSAIC_DISPLAYTOPO_ERROR_*
                            //!< flags.
        NvU32 warningFlags; //!< (OUT) Any of the
                            //!< NV_MOSAIC_DISPLAYTOPO_WARNING_* flags.
...
//! \ingroup mosaicapi
#define NV_MOSAIC_DISPLAY_TOPO_STATUS_VER \
...
//
//! DESCRIPTION:     Determines if a list of grid topologies is valid. It will
//! choose an SLI
//!                  configuration in the same way that
//!                  NvAPI_Mosaic_SetDisplayGrids() does.
//!
//!                  On return, each element in the pTopoStatus array will
//!                  contain any errors or warnings about each grid topology. If
//!                  any error flags are set, then the topology is not valid. If
//!                  any warning flags are set, then the topology is valid, but
//!                  sub-optimal.
//!
//!                  If the ALLOW_INVALID flag is set, then it will continue to
//!                  validate the grids even if no SLI configuration will allow
//!                  all of the grids. In this case, a grid grid with no
//!                  matching GPU topology will have the error flags
//!                  NO_GPU_TOPOLOGY or NOT_SUPPORTED set.
//!
//!                  If the ALLOW_INVALID flag is not set and no matching SLI
//!                  configuration is found, then it will skip the rest of the
//!                  validation and return NVAPI_NO_ACTIVE_SLI_TOPOLOGY.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
//!
...
//! \param [in,out]  pTopoStatus        The array of problems and warnings with
//! each grid topology.
//! \param [in]      gridCount          The number of elements in the
//! pGridTopologies and
//!                                     pTopoStatus arrays.
//!
//!
//! \retval ::NVAPI_OK:                          Capabilities have been
//! returned.
//! \retval ::NVAPI_INVALID_ARGUMENT:            One or more args passed in are
//! invalid.
//! \retval ::NVAPI_API_NOT_INTIALIZED:          The NvAPI API needs to be
//! initialized first
//! \retval ::NVAPI_NO_IMPLEMENTATION:           This entrypoint not available
//! \retval ::NVAPI_NO_ACTIVE_SLI_TOPOLOGY:      No matching GPU topologies
//! could be found.
//! \retval ::NVAPI_ERROR:                       Miscellaneous error occurred
//!
//! \ingroup mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Mosaic_ValidateDisplayGrids(
    __in NvU32 setTopoFlags,
    __in_ecount( gridCount ) NV_MOSAIC_GRID_TOPO* pGridTopologies,
...
//
//! DESCRIPTION:     Determines the set of available display modes for a given
//! grid topology.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
...
//! \param [in,out]  pDisplaySettings      A pointer to an array of display
//! settings to populate,
//!                                        or NULL to find out the total number
//!                                        of available modes.
//! \param [in,out]  pDisplayCount         If pDisplaySettings is not NULL, then
//! pDisplayCount
//!                                        should point to the number of
//!                                        elements in the pDisplaySettings
//!                                        array. On return, it will contain the
//!                                        number of modes that were actually
//!                                        returned. If pDisplaySettings is
//!                                        NULL, then pDisplayCount will receive
//!                                        the total number of modes that are
//!                                        available.
//!
//!
//! \retval ::NVAPI_OK                          Capabilities have been returned.
...
//! \ingroup mosaciapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Mosaic_EnumDisplayModes(
    __in NV_MOSAIC_GRID_TOPO* pGridTopology,
...
//
//! DESCRIPTION:     Enumerates the current active grid topologies. This
//! includes Mosaic, IG, and
//!                  Panoramic topologies, as well as single displays.
//!
//!                  If pGridTopologies is NULL, then pGridCount will be set to
//!                  the number of active grid topologies.
//!
//!                  If pGridTopologies is not NULL, then pGridCount contains
//!                  the maximum number of grid topologies to return. On return,
//!                  pGridCount will be set to the number of grid topologies
//!                  that were returned.
//!
//! \param [out]     pGridTopologies   The list of active grid topologies.
//! \param [in,out]  pGridCount        A pointer to the number of grid
//! topologies returned.
//!
//! \retval ::NVAPI_OK                          Capabilties have been returned.
//! \retval ::NVAPI_END_ENUMERATION             There are no more topologies to
//! return.
//! \retval ::NVAPI_INVALID_ARGUMENT            One or more args passed in are
//! invalid.
//! \retval ::NVAPI_API_NOT_INTIALIZED          The NvAPI API needs to be
...
//! \ingroup mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Mosaic_EnumDisplayGrids(
    __inout_ecount_part_opt( *pGridCount, *pGridCount )
...
//   remain once Phase 2 is complete.  For a small amount of time, the two will
//   co-exist.  As soon as apps (nvapichk, NvAPITestMosaic, and CPL) are updated
//   to use the Phase 2 entrypoints, the code below will be deleted.
//
// DELME_RUSS - DELME_RUSS - DELME_RUSS - DELME_RUSS - DELME_RUSS - DELME_RUSS
// ###########################################################################
//
...
//
// Selected scan out targets can be one per GPU or more than one on the same
// GPU.
//
...
//! Used in NvAPI_GetCurrentMosaicTopology() and
...
typedef struct {
    ...
    struct {
        NvPhysicalGpuHandle
            hPhysicalGPU;      //!< Physical GPU to be used in the topology
        NvU32 displayOutputId; //!< Connected display target
        NvS32 overlapX; //!< Pixels of overlap on the left of target: (+overlap,
                        //!< -gap)
        NvS32 overlapY; //!< Pixels of overlap on the top of target: (+overlap,
                        //!< -gap)
...
typedef struct {
    ...
    NV_MOSAIC_TOPOLOGY
    topos[ NVAPI_MAX_MOSAIC_TOPOS ]; //!< Maximum number of topologies

} NV_MOSAIC_SUPPORTED_TOPOLOGIES;

//! Used in NV_MOSAIC_SUPPORTED_TOPOLOGIES.
#define NVAPI_MOSAIC_SUPPORTED_TOPOLOGIES_VER \
...
//! \since Release: 177
//!
//! \param [out] pMosaicTopos                   An array of valid Mosaic
//! topologies.
//!
//! \retval      NVAPI_OK                       Call succeeded; 1 or more
//! topologies were returned
//! \retval      NVAPI_INVALID_ARGUMENT         One or more arguments are
//! invalid
//! \retval      NVAPI_MIXED_TARGET_TYPES       Mosaic topology is only possible
//! with all targets of the same NV_GPU_OUTPUT_TYPE.
//! \retval      NVAPI_NVIDIA_DEVICE_NOT_FOUND  No NVIDIA GPU driving a display
//! was found
//! \retval      NVAPI_NOT_SUPPORTED            Mosaic is not supported with
//! GPUs on this system.
//! \retval      NVAPI_NO_ACTIVE_SLI_TOPOLOGY   SLI is not enabled, yet needs to
//! be, in order for this function to succeed.
//!
//! \ingroup     mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GetSupportedMosaicTopologies(
    NV_MOSAIC_SUPPORTED_TOPOLOGIES* pMosaicTopos );
...
//! \param [out] pEnabled                       TRUE if returned topology is
//! currently enabled, else FALSE
//!
...
//
//! DESCRIPTION:     This API sets the Mosaic topology, and enables it so that
//! the
...
//! \ingroup     mosaicapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_SetCurrentMosaicTopology(
    NV_MOSAIC_TOPOLOGY* pMosaicTopo );
...
//!
//!                  - If enabled, enumeration of display settings will include
//!                  valid Mosaic resolutions.
//!                  - If disabled, enumeration of display settings will not
//!                  include Mosaic resolutions.
//!
//! SUPPORTED OS:  Windows XP
//!
//!
//! \since Release: 177
//!
//! \param [in]  enable                         TRUE to enable the Mosaic
//! Topology, FALSE to disable it.
//!
//! \retval      NVAPI_OK                       Call succeeded
//! \retval      NVAPI_INVALID_ARGUMENT         One or more arguments are
//! invalid
...
//
//!   DESCRIPTION: This API returns an array of Sync device handles. A Sync
//!   device handle represents a
...
//! \since Release: 313
//!
//! \param [out] nvGSyncHandles-  The caller provides an array of handles, which
//! must contain at least
//!                               NVAPI_MAX_GSYNC_DEVICES elements. The API will
//!                               zero out the entire array and then fill in one
//!                               or more handles. If an error occurs, the array
//!                               is invalid.
//! \param [out] *gsyncCount-     The caller provides the storage space.
//! NvAPI_GSync_EnumSyncDevices
//!                               sets *gsyncCount to indicate how many of the
//!                               elements in the nvGSyncHandles[] array are
//!                               valid. If an error occurs, *gsyncCount will be
//!                               set to zero.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
//!          If there are return error codes with specific meaning for this API,
//!          they are listed below.
//! \retval ::NVAPI_INVALID_ARGUMENT         nvGSyncHandles or gsyncCount is
//! NULL.
//! \retval ::NVAPI_NVIDIA_DEVICE_NOT_FOUND  The queried Graphics system does
//! not have any Sync Device.
//!
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_EnumSyncDevices(
    __out NvGSyncDeviceHandle nvGSyncHandles[ NVAPI_MAX_GSYNC_DEVICES ],
    __out NvU32* gsyncCount );

// GSync boardId values
#define NVAPI_GSYNC_BOARD_ID_P358 \
    856 //!< GSync board ID 0x358, see NV_GSYNC_CAPABILITIES
#define NVAPI_GSYNC_BOARD_ID_P2060 \
    8288 //!< GSync board ID 0x2060, see NV_GSYNC_CAPABILITIES
...
//! \since Release: 313
//!
//! \param [in]    hNvGSyncDevice-        The handle for a Sync device for which
//! the capabilities will be queried.
//! \param [inout] *pNvGSyncCapabilities- The caller provides the storage space.
//! NvAPI_GSync_QueryCapabilities() sets
//!                                       *pNvGSyncCapabilities to the version
//!                                       and capabilities details of the Sync
//!                                       device If an error occurs,
//!                                       *pNvGSyncCapabilities will be set to
//!                                       NULL.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
...
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_QueryCapabilities(
    __in NvGSyncDeviceHandle hNvGSyncDevice,
...
typedef struct _NV_GSYNC_GPU {
    ...
    NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR
    connector; //!< Indicates which connector on the device the GPU is
               //!< connected to.
    NvPhysicalGpuHandle
        hProxyPhysicalGpu; //!< GPU through which hPhysicalGpu is connected to
                           //!< the Sync device (if not directly connected)
...
typedef struct _NV_GSYNC_DISPLAY {
    NvU32 version;   //!< Version of the structure
    NvU32 displayId; //!< display identifier for displays.The GPU to which it is
    ...
    NvU32 reserved : 31;    //!< Should be set to ZERO
    NVAPI_GSYNC_DISPLAY_SYNC_STATE
    syncState; //!< Is this display slave/master
               //!< (Retrieved with topology or set by caller for
               //!< enable/disable sync)
} NV_GSYNC_DISPLAY;
...
//! \since Release: 313
//!
//! \param [in]       hNvGSyncDevice-     The caller provides the handle for a
//! Sync device for which the topology will be queried.
//! \param [in, out]  gsyncGpuCount-      It returns number of GPUs connected to
//! Sync device
//! \param [in, out]  gsyncGPUs-          It returns info about GPUs connected
//! to Sync device
//! \param [in, out]  gsyncDisplayCount-  It returns number of active displays
//! that belongs to Sync device
//! \param [in, out]  gsyncDisplays-      It returns info about all active
//! displays that belongs to Sync device
//!
//! HOW TO USE: 1) make a call to get the number of GPUs connected OR displays
//! synced through Sync device
//!                by passing the gsyncGPUs OR gsyncDisplays as NULL
//!                respectively. Both gsyncGpuCount and gsyncDisplayCount can be
//!                retrieved in same call by passing both gsyncGPUs and
//!                gsyncDisplays as NULL On call success:
//!             2) Allocate memory based on gsyncGpuCount(for gsyncGPUs) and/or
//!             gsyncDisplayCount(for gsyncDisplays) then make a call to
//!             populate gsyncGPUs and/or gsyncDisplays respectively.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
//!          If there are return error codes with specific meaning for this API,
//!          they are listed below.
//! \retval ::NVAPI_INVALID_ARGUMENT               hNvGSyncDevice is NULL.
//! \retval ::NVAPI_NVIDIA_DEVICE_NOT_FOUND        The queried Graphics system
//! does not have any Sync Device.
//! \retval ::NVAPI_INSUFFICIENT_BUFFER            When the actual number of
//! GPUs/displays in the topology exceed the number of elements allocated for
//! SyncGPUs/SyncDisplays respectively.
//!
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
...
//! \since Release: 313
//!
//! \param [in]  gsyncDisplayCount-			The number of displays
//! in gsyncDisplays.
//! \param [in]  pGsyncDisplays-			The caller provides the
//! structure containing all displays that need to be synchronized in the
//! system.
//!											The
//! displays that are not part of pGsyncDisplays, will be un-synchronized.
//! \param [in]  flags-						Reserved for
//! future use.
//!
//!
//! \return  This API can return any of the error codes enumerated in
...
//!          they are listed below.
//!
//! \retval ::NVAPI_INVALID_ARGUMENT			If the display topology
//! or count not valid.
//! \retval ::NVAPI_NVIDIA_DEVICE_NOT_FOUND		The queried Graphics
//! system does not have any Sync Device.
//! \retval ::NVAPI_INVALID_SYNC_TOPOLOGY       1.If any mosaic grid is partial.
//!                                             2.If
//!                                             timing(HVisible/VVisible/refreshRate)
//!                                             applied of any display is
//!                                             different. 3.If There is a
//!                                             across GPU mosaic grid in system
//!                                             and that is not a part of
//!                                             pGsyncDisplays.
//!
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_SetSyncStateSettings(
    __in NvU32 gsyncDisplayCount,
...
typedef struct _NV_GSYNC_DELAY {
    ...
    NvU32 maxLines;  //!< maximum number of lines supported at current display
                     //!< mode to induce delay. Updated by
                     //!< NvAPI_GSync_GetControlParameters(). Read only.
    NvU32 minPixels; //!< minimum number of pixels required at current display
                     //!< mode to induce delay. Updated by
                     //!< NvAPI_GSync_GetControlParameters(). Read only.
} NV_GSYNC_DELAY;
...
//! Used in NvAPI_GSync_GetControlParameters() and
...
typedef struct _NV_GSYNC_CONTROL_PARAMS {
    ...
    NvU32 interval; //!< Number of pulses to wait between framelock signal
                    //!< generation
    ...
    NV_GSYNC_DELAY syncSkew; //!< The time delay between the frame sync signal
                             //!< and the GPUs signal.
...
//
//!   DESCRIPTION: This API queries for sync control parameters as defined in
//!   NV_GSYNC_CONTROL_PARAMS.
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
...
//! \since Release: 313
//!
//! \param [in]    hNvGSyncDevice-   The caller provides the handle of the Sync
//! device for which to get parameters
//! \param [inout] *pGsyncControls-  The caller provides the storage space.
//! NvAPI_GSync_GetControlParameters() populates *pGsyncControls with values.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
//!          If there are return error codes with specific meaning for this API,
...
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_GetControlParameters(
    __in NvGSyncDeviceHandle hNvGSyncDevice,
...
//
//!   DESCRIPTION: This API sets control parameters as defined in
...
//! NV_GSYNC_CONTROL_PARAMS. skew and startDelay will be updated to the applied
//! values.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
...
//! \retval ::NVAPI_NVIDIA_DEVICE_NOT_FOUND   The queried Graphics system does
//! not have any Sync Device.
//! \retval ::NVAPI_SYNC_MASTER_NOT_FOUND     Control Parameters can only be set
//! if there is a Sync Master enabled on the Gsync card.
//!
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_SetControlParameters(
    __in NvGSyncDeviceHandle hNvGSyncDevice,
    __inout NV_GSYNC_CONTROL_PARAMS* pGsyncControls );
...
//
//!   DESCRIPTION: This API adjusts the skew and startDelay to the closest
...
//! \since Release: 319
//!
//! \param [in]  hNvGSyncDevice-   	The caller provides the handle of the
//! Sync device for which to get parameters
//! \param [in]  delayType-   		Specifies whether the delay is syncSkew
//! or startupDelay.
//! \param [inout]  *pGsyncDelay-  	The caller provides NV_GSYNC_DELAY. skew
//! and startDelay will be adjusted and updated to the closest values.
//! \param [out]  *syncSteps-  		This parameter is optional. It returns
//! the sync delay in unit steps. If 0, it means either the
...
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_AdjustSyncDelay(
    __in NvGSyncDeviceHandle hNvGSyncDevice,
...
typedef struct _NV_GSYNC_STATUS {
    ...
    NvU32 bIsStereoSynced; //!< Does the phase of the timing signal from the GPU
                           //!< = the phase of the master sync signal?
...
//
//!   DESCRIPTION: This API queries the sync status of a GPU - timing,
...
//! \param [out] *status-            The caller provides the storage space.
//! NvAPI_GSync_GetSyncStatus() populates *status with
//!                                  values - timing, stereosync and signal
//!                                  availability. On error, *status is set to
//!                                  NULL.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
...
//! SyncTarget is NULL.
//! \retval ::NVAPI_NVIDIA_DEVICE_NOT_FOUND   The queried Graphics system does
//! not have any G-Sync Device.
//!
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_GetSyncStatus(
    __in NvGSyncDeviceHandle hNvGSyncDevice,
    __in NvPhysicalGpuHandle hPhysicalGpu,
...
typedef enum _NVAPI_GSYNC_RJ45_IO {
    ...
    NVAPI_GSYNC_RJ45_UNUSED = 2 //!< This field is used to notify that the
                                //!< framelock is not actually present.
...
typedef struct _NV_GSYNC_STATUS_PARAMS {
    ...
    NVAPI_GSYNC_RJ45_IO
    RJ45_IO[ NVAPI_MAX_RJ45_PER_GSYNC ]; //!< Configured as input / output
    NvU32 RJ45_Ethernet[ NVAPI_MAX_RJ45_PER_GSYNC ]; //!< Connected to ethernet
                                                     //!< hub? [ERRONEOUSLY
                                                     //!< CONNECTED!]
...
#define NV_GSYNC_STATUS_PARAMS_VER \
    MAKE_NVAPI_VERSION( NV_GSYNC_STATUS_PARAMS, 1 )

///////////////////////////////////////////////////////////////////////////////
//
// FUNCTION NAME: NvAPI_GSync_GetStatusParameters
//
//!   DESCRIPTION: This API queries for sync status parameters as defined in
//!   NV_GSYNC_STATUS_PARAMS.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \since Release: 313
//!
//! \param [in]  hNvGSyncDevice   The caller provides the handle of the GSync
//! device for which to get parameters
//! \param [out] *pStatusParams   The caller provides the storage space.
...
//! \retval ::NVAPI_NVIDIA_DEVICE_NOT_FOUND     The queried Graphics system does
//! not have any GSync Device.
//!
//! \ingroup gsyncapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GSync_GetStatusParameters(
...
//
//! DESCRIPTION:    This API binds a resource (surface/texture) so that it can
//! be retrieved
//!                 internally by NVAPI.
//!
//! SUPPORTED OS:  Windows XP and higher
...
//!                                              DWORD dwFlag);
//!   DESCRIPTION: Create a texture that is an alias of a surface registered
//!   with NvAPI.  The
//!                new texture can be bound with IDirect3DDevice9::SetTexture().
//!                Note that the texture must be unbound before drawing to the
//!                surface again. Unless the USE_SUPER flag is passed, MSAA
//!                surfaces will be resolved before being used as a texture.
//!                MSAA depth buffers are resolved with a point filter, and
//!                non-depth MSAA surfaces are resolved with a linear filter.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \param [in]    pDev         The D3D device that owns the objects
//! \param [in]    pSurface     Pointer to a surface that has been registered
//! with NvAPI
//!                             to which a texture alias is to be provided
//! \param [out]   ppTexture    Fill with the texture created
//! \param [in]    dwFlag       NVAPI_ALIAS_SURFACE_FLAG to describe how to
//! handle the texture
//!
//! \retval ::NVAPI_OK                  completed request
...
//! probably dwFlag.
//! \retval ::NVAPI_UNREGISTERED_RESOURCE pSurface has not been registered with
//! NvAPI
...
typedef enum {
    ...
    NVAPI_ALIAS_SURFACE_FLAG_USE_SUPER =
        0x00000001, //!< Use the surface's msaa buffer directly as a texture,
                    //!< rather than resolving. (This is much slower, but
                    //!< potentially has higher quality.)
...
//
//! DESCRIPTION:     This API copies the contents of the source resource to the
//! destination
...
//!                  with NvAPI before being used with
...
//! \param [in]     pSrcRect            Defines the rectangle on the source to
//! copy from.  If NULL, copy from the entire resource.
//! \param [in]     pDestResource       Pointer to the destination resource.
//! \param [in]     pDstRect            Defines the rectangle on the destination
//! to copy to.  If NULL, copy to the entire resource.
//! \param [in]     Filter              Choose a filtering method: D3DTEXF_NONE,
//! D3DTEXF_POINT, D3DTEXF_LINEAR.
//!
//! \retval ::NVAPI_OK                       completed request
//! \retval ::NVAPI_INVALID_POINTER          An invalid pointer was passed as an
//! argument (probably NULL)
//! \retval ::NVAPI_INVALID_ARGUMENT         One of the arguments was invalid
//! \retval ::NVAPI_UNREGISTERED_RESOURCE    a resource was passed in without
//! being registered
//! \retval ::NVAPI_ERROR                    error occurred
//!
//! \ingroup dx
...
//
//! DESCRIPTION:     This API Clears the currently bound render target(s) with
//! the
//!                  given color
//!
//!
...
//!
//! \param [in]     pDevice             The D3D device that owns the objects.
//! \param [in]     dwNumRects          The no of rectangles to clear. If 0,
//! clear the entire surface (clipped to viewport)
//! \param [in]     pRects              Defines the rectangles to clear.  Should
//! be NULL if dwNumRects == 0
...
//!                                            *pStereoInfo);
//! \code
//!   DESCRIPTION: This api specifies the stereo format of a surface, so that
//!   the
//!                surface could be used for stereo video processing or
//!                compositing. In particular, this api could be used to link
//!                the left and right views of a decoded picture.
//!
//! \since Release: 313
//!
//!         INPUT: pDev        - The device on which the stereo surface will be
//!         used
...
typedef struct _NV_DX_VIDEO_STEREO_INFO {
    ...
    NVDX_ObjectHandle
        hSurface; //!< The surface whose stereo format is to be set
    NVDX_ObjectHandle
        hLinkedSurface; //!< The linked surface (must be valid when
                        //!< eFormat==NV_STEREO_VIDEO_FORMAT_TWO_FRAMES_LR)
    NV_STEREO_VIDEO_FORMAT eFormat; //!< Stereo format of the surface
    NvS32 sViewOffset;  //!< Signed offset of each view (positive offset
                        //!< indicating left view is shifted left)
    BOOL bStereoEnable; //!< Whether stereo rendering should be enabled (if
                        //!< FALSE, only left view will be used)
} NV_DX_VIDEO_STEREO_INFO;

//! Macro for constructing the version field of ::NV_DX_VIDEO_STEREO_INFO
#define NV_DX_VIDEO_STEREO_INFO_VER \
    MAKE_NVAPI_VERSION( NV_DX_VIDEO_STEREO_INFO, 1 )

NVAPI_INTERFACE NvAPI_D3D9_VideoSetStereoInfo(
...
//! @}
    ...
    ( defined( __d3d11_h__ ) || defined( __d3d11_1_h__ ) )
///////////////////////////////////////////////////////////////////////////////
//
...
//!                supported on current hardware. List of opcodes is in
//!                nvShaderExtnEnums.h To use Nvidia HLSL extensions the
//!                application must include nvHLSLExtns.h in the hlsl shader
//!                code. See nvHLSLExtns.h for more details on supported
//!                opcodes.
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
...
//!
//! RETURN STATUS:     This API can return any of the error codes enumerated in
//! #NvAPI_Status.
//!                    If there are return error codes with specific meaning for
...
//
//!   DESCRIPTION: This function sets the fake UAV slot that is used by Nvidia
//!   HLSL
//!                shader extensions. All createShader calls made to the driver
//!                after setting this slot would treat writes/reads to this UAV
//!                in a different way. Applications are expected to bind null
//!                UAV to this slot. The same slot is used for all shader
//!                stages. To disable shader extensions the app may set this uav
//!                slot to some value that is bigger than the max allowed slot
//!                index e.g, 128 or 0xFFFFFFFF. To use Nvidia HLSL extensions
//!                the application must include nvHLSLExtns.h in the hlsl shader
//!                code. See nvHLSLExtns.h for more details.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
//! \param [in]        pDev         The device for which to set the extension
//! slot
//!                                 should be a ID3D11Device+ device
...
//
//!   DESCRIPTION: Causes the driver to skip synchronization that is normally
//!   needed when accessing UAVs.
//!                Applications must use this with caution otherwise this might
//!                cause data hazards when multiple draw calls/compute shader
//!                launches are accessing same memory locations
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
//! \param [in]        *pDeviceOrContext     pointer to D3D11 device, or D3D11
//! device context
//!
//! RETURN STATUS:     This API can return any of the error codes enumerated in
//! #NvAPI_Status.
...
//
//!   DESCRIPTION: Re-enables driver synchronization between calls that access
//!   same UAVs
...
//-----------------------------------------------------------------------------
    ...
    defined( __d3d10_1_h__ ) || defined( __d3d11_h__ )
///////////////////////////////////////////////////////////////////////////////
//
// FUNCTION NAME: NvAPI_D3D_SetFPSIndicatorState
//
//!   DESCRIPTION: Display an overlay that tracks the number of times the app
//!   presents per second, or,
...
#endif // if defined(_D3D9_H_) || defined(__d3d10_h__) || defined(__d3d10_1_h__)
       // || defined(__d3d11_h__)
...
//!
//! \param [in]    pDevice               Device to disabled the shader disk
//! cache on
//!
//!
...
typedef enum _NVVIOOWNERTYPE {
    ...
    NVVIOOWNERTYPE_DESKTOP, //!<  Desktop transparent mode owns the device (not
                            //!<  applicable for video input)
} NVVIOOWNERTYPE;
...
//! VIO_DATA.ulOwnerID high-bit is set only if device has been initialized by
//! VIOAPI examined at NvAPI_GetCapabilities|NvAPI_VIO_Open to determine if
//! settings need to be applied from registry or POR state read
#define NVVIO_OWNERID_INITIALIZED 0x80000000

//! VIO_DATA.ulOwnerID next-bit is set only if device is currently in exclusive
//! write access mode from NvAPI_VIO_Open()
...
typedef enum _NVVIOSIGNALFORMAT {
    ...
    NVVIOSIGNALFORMAT_487I_59_94_SMPTE259_NTSC, //!< 01  487i    59.94Hz
                                                //!< (SMPTE259) NTSC
    NVVIOSIGNALFORMAT_576I_50_00_SMPTE259_PAL,  //!< 02  576i    50.00Hz
                                                //!< (SMPTE259) PAL
    ...
    NVVIOSIGNALFORMAT_1080P_23_976_SMPTE274, //!< 13  1080p   23.976Hz
                                             //!< (SMPTE274)
    ...
    NVVIOSIGNALFORMAT_1080PSF_25_00_SMPTE274, //!< 34  1080PsF 25.00Hz
                                              //!< (SMPTE274)
    NVVIOSIGNALFORMAT_1080PSF_29_97_SMPTE274, //!< 35  1080PsF 29.97Hz
                                              //!< (SMPTE274)
    NVVIOSIGNALFORMAT_1080PSF_30_00_SMPTE274, //!< 36  1080PsF 30.00Hz
                                              //!< (SMPTE274)
    NVVIOSIGNALFORMAT_1080PSF_24_00_SMPTE274, //!< 37  1080PsF 24.00Hz
                                              //!< (SMPTE274)
    NVVIOSIGNALFORMAT_1080PSF_23_98_SMPTE274, //!< 38  1080PsF 23.98Hz
                                              //!< (SMPTE274)

    NVVIOSIGNALFORMAT_1080P_50_00_SMPTE274_3G_LEVEL_A, //!< 39  1080P   50.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< A
    NVVIOSIGNALFORMAT_1080P_59_94_SMPTE274_3G_LEVEL_A, //!< 40  1080P   59.94Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< A
    NVVIOSIGNALFORMAT_1080P_60_00_SMPTE274_3G_LEVEL_A, //!< 41  1080P   60.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< A

    NVVIOSIGNALFORMAT_1080P_60_00_SMPTE274_3G_LEVEL_B, //!< 42  1080p   60.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080I_60_00_SMPTE274_3G_LEVEL_B, //!< 43  1080i   60.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048I_60_00_SMPTE372_3G_LEVEL_B, //!< 44  2048i   60.00Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080P_50_00_SMPTE274_3G_LEVEL_B, //!< 45  1080p   50.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080I_50_00_SMPTE274_3G_LEVEL_B, //!< 46  1080i   50.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048I_50_00_SMPTE372_3G_LEVEL_B, //!< 47  2048i   50.00Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080P_30_00_SMPTE274_3G_LEVEL_B, //!< 48  1080p   30.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048P_30_00_SMPTE372_3G_LEVEL_B, //!< 49  2048p   30.00Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080P_25_00_SMPTE274_3G_LEVEL_B, //!< 50  1080p   25.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048P_25_00_SMPTE372_3G_LEVEL_B, //!< 51  2048p   25.00Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080P_24_00_SMPTE274_3G_LEVEL_B, //!< 52  1080p   24.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048P_24_00_SMPTE372_3G_LEVEL_B, //!< 53  2048p   24.00Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080I_48_00_SMPTE274_3G_LEVEL_B, //!< 54  1080i   48.00Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048I_48_00_SMPTE372_3G_LEVEL_B, //!< 55  2048i   48.00Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080P_59_94_SMPTE274_3G_LEVEL_B, //!< 56  1080p   59.94Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080I_59_94_SMPTE274_3G_LEVEL_B, //!< 57  1080i   59.94Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048I_59_94_SMPTE372_3G_LEVEL_B, //!< 58  2048i   59.94Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080P_29_97_SMPTE274_3G_LEVEL_B, //!< 59  1080p   29.97Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048P_29_97_SMPTE372_3G_LEVEL_B, //!< 60  2048p   29.97Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080P_23_98_SMPTE274_3G_LEVEL_B, //!< 61  1080p   29.98Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048P_23_98_SMPTE372_3G_LEVEL_B, //!< 62  2048p   29.98Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_1080I_47_96_SMPTE274_3G_LEVEL_B, //!< 63  1080i   47.96Hz
                                                       //!< (SMPTE274) 3G Level
                                                       //!< B
    NVVIOSIGNALFORMAT_2048I_47_96_SMPTE372_3G_LEVEL_B, //!< 64  2048i   47.96Hz
                                                       //!< (SMPTE372) 3G Level
                                                       //!< B
...
typedef enum _NVVIODATAFORMAT {
    ...
    NVVIODATAFORMAT_R8G8B8_TO_YCRCB444, //!< R8:G8:B8                => YCrCb
                                        //!< (4:4:4)
    NVVIODATAFORMAT_R8G8B8A8_TO_YCRCBA4444,  //!< R8:G8:B8:A8             =>
                                             //!< YCrCbA (4:4:4:4)
    NVVIODATAFORMAT_R8G8B8Z10_TO_YCRCBZ4444, //!< R8:G8:B8:Z10            =>
                                             //!< YCrCbZ (4:4:4:4)
    NVVIODATAFORMAT_R8G8B8_TO_YCRCB422, //!< R8:G8:B8                => YCrCb
                                        //!< (4:2:2)
    NVVIODATAFORMAT_R8G8B8A8_TO_YCRCBA4224,  //!< R8:G8:B8:A8             =>
                                             //!< YCrCbA (4:2:2:4)
    NVVIODATAFORMAT_R8G8B8Z10_TO_YCRCBZ4224, //!< R8:G8:B8:Z10            =>
                                             //!< YCrCbZ (4:2:2:4)
    NVVIODATAFORMAT_X8X8X8_444_PASSTHRU,     //!< R8:G8:B8                => RGB
                                             //!< (4:4:4)
    NVVIODATAFORMAT_X8X8X8A8_4444_PASSTHRU, //!< R8:G8:B8:A8             => RGBA
                                            //!< (4:4:4:4)
    NVVIODATAFORMAT_X8X8X8Z10_4444_PASSTHRU, //!< R8:G8:B8:Z10            =>
                                             //!< RGBZ   (4:4:4:4)
    NVVIODATAFORMAT_X10X10X10_444_PASSTHRU,  //!< Y10:CR10:CB10           =>
                                             //!< YCrCb  (4:4:4)
    NVVIODATAFORMAT_X10X8X8_444_PASSTHRU, //!< Y10:CR8:CB8             => YCrCb
                                          //!< (4:4:4)
    NVVIODATAFORMAT_X10X8X8A10_4444_PASSTHRU, //!< Y10:CR8:CB8:A10         =>
                                              //!< YCrCbA (4:4:4:4)
    NVVIODATAFORMAT_X10X8X8Z10_4444_PASSTHRU, //!< Y10:CR8:CB8:Z10         =>
                                              //!< YCrCbZ (4:4:4:4)
    NVVIODATAFORMAT_DUAL_R8G8B8_TO_DUAL_YCRCB422, //!< R8:G8:B8 + R8:G8:B8 =>
                                                  //!< YCrCb  (4:2:2 + 4:2:2)
    NVVIODATAFORMAT_DUAL_X8X8X8_TO_DUAL_422_PASSTHRU, //!< Y8:CR8:CB8 +
                                                      //!< Y8:CR8:CB8 => YCrCb
                                                      //!< (4:2:2 + 4:2:2)
    NVVIODATAFORMAT_R10G10B10_TO_YCRCB422, //!< R10:G10:B10             => YCrCb
                                           //!< (4:2:2)
    NVVIODATAFORMAT_R10G10B10_TO_YCRCB444, //!< R10:G10:B10             => YCrCb
                                           //!< (4:4:4)
    NVVIODATAFORMAT_X12X12X12_444_PASSTHRU,  //!< X12:X12:X12             => XXX
                                             //!< (4:4:4)
    NVVIODATAFORMAT_X12X12X12_422_PASSTHRU,  //!< X12:X12:X12             => XXX
                                             //!< (4:2:2)
    NVVIODATAFORMAT_Y10CR10CB10_TO_YCRCB422, //!< Y10:CR10:CB10           =>
                                             //!< YCrCb  (4:2:2)
    NVVIODATAFORMAT_Y8CR8CB8_TO_YCRCB422, //!< Y8:CR8:CB8              => YCrCb
                                          //!< (4:2:2)
    NVVIODATAFORMAT_Y10CR8CB8A10_TO_YCRCBA4224, //!< Y10:CR8:CB8:A10         =>
                                                //!< YCrCbA (4:2:2:4)
    NVVIODATAFORMAT_R10G10B10_TO_RGB444,   //!< R10:G10:B10             => RGB
                                           //!< (4:4:4)
    NVVIODATAFORMAT_R12G12B12_TO_YCRCB444, //!< R12:G12:B12             => YCrCb
                                           //!< (4:4:4)
    NVVIODATAFORMAT_R12G12B12_TO_YCRCB422, //!< R12:G12:B12             => YCrCb
                                           //!< (4:2:2)
} NVVIODATAFORMAT;
...
typedef enum _NVVIOOUTPUTAREA {
    ...
    NVVIOOUTPUTAREA_SAFEACTION, //!< Output to centered 90% of video resolution
                                //!< (safe action)
    NVVIOOUTPUTAREA_SAFETITLE,  //!< Output to centered 80% of video resolution
                                //!< (safe title)
} NVVIOOUTPUTAREA;
...
typedef struct _NVVIOCAPS {
    ...
    struct //
    {
        NvU32 majorVersion; //!< Major version. For GVI, majorVersion contains
    ...
    struct {
        NvU32 majorVersion; //!< Major version. In version 2, for both GVI and
                            //!< GVO, majorVersion contains MajorVersion(HIWORD)
                            //!< And MinorVersion(LOWORD)
        NvU32 minorVersion; //!< Minor version. In version 2, for both GVI and
...
typedef struct _NVVIOINPUTSTATUS {
    NVVIOCHANNELSTATUS
    vidIn[ NVAPI_MAX_VIO_JACKS ]
         [ NVAPI_MAX_VIO_CHANNELS_PER_JACK ]; //!< Video input status per
                                              //!< channel within a jack
...
typedef struct _NVVIOGAMMARAMP8 {
    ...
    NvU16 uGreen[ 256 ]; //!< Green channel gamma ramp (8-bit index, 16-bit
                         //!< values)
    NvU16
        uBlue[ 256 ]; //!< Blue channel gamma ramp (8-bit index, 16-bit values)
} NVVIOGAMMARAMP8;

//! Gamma ramp (10-bit index)
typedef struct _NVVIOGAMMARAMP10 {
    NvU16
        uRed[ 1024 ]; //!< Red channel gamma ramp (10-bit index, 16-bit values)
    NvU16 uGreen[ 1024 ]; //!< Green channel gamma ramp (10-bit index, 16-bit
                          //!< values)
    NvU16 uBlue[ 1024 ];  //!< Blue channel gamma ramp (10-bit index, 16-bit
                          //!< values)
} NVVIOGAMMARAMP10;
...
typedef struct _NVVIOCOLORCONVERSION {
    ...
    NvU32 compositeSafe; //!<  compositeSafe constrains luminance range when
...
// Device configuration (fields masks indicating NVVIOCONFIG fields to use for
// NvAPI_VIO_GetConfig/NvAPI_VIO_SetConfig() )
//
...
#define NVVIOCONFIG_SYNCSOURCEENABLE \
    0x00000040                           //!< fields: syncSource and syncEnable
...
#define NVVIOCONFIG_ALLFIELDS                                           \
    ( NVVIOCONFIG_SIGNALFORMAT | NVVIOCONFIG_DATAFORMAT |               \
      NVVIOCONFIG_OUTPUTREGION | NVVIOCONFIG_OUTPUTAREA |               \
      NVVIOCONFIG_COLORCONVERSION | NVVIOCONFIG_GAMMACORRECTION |       \
      NVVIOCONFIG_SYNCSOURCEENABLE | NVVIOCONFIG_SYNCDELAY |            \
      NVVIOCONFIG_COMPOSITESYNCTYPE | NVVIOCONFIG_FRAMELOCKENABLE |     \
      NVVIOCONFIG_422FILTER | NVVIOCONFIG_COMPOSITETERMINATE |          \
      NVVIOCONFIG_DATAINTEGRITYCHECK | NVVIOCONFIG_CSCOVERRIDE |        \
      NVVIOCONFIG_FLIPQUEUELENGTH | NVVIOCONFIG_ANCTIMECODEGENERATION | \
      NVVIOCONFIG_COMPOSITE | NVVIOCONFIG_ALPHAKEYCOMPOSITE |           \
      NVVIOCONFIG_COMPOSITE_Y | NVVIOCONFIG_COMPOSITE_CR |              \
      NVVIOCONFIG_COMPOSITE_CB | NVVIOCONFIG_FULL_COLOR_RANGE |         \
      NVVIOCONFIG_RGB_DATA | NVVIOCONFIG_RESERVED_SDIOUTPUTENABLE |     \
      NVVIOCONFIG_STREAMS | NVVIOCONFIG_ANC_PARITY_COMPUTATION |        \
...
#define NVVIOCONFIG_VALIDFIELDS                                         \
    ( NVVIOCONFIG_SIGNALFORMAT | NVVIOCONFIG_DATAFORMAT |               \
      ...
      NVVIOCONFIG_SYNCSOURCEENABLE | NVVIOCONFIG_SYNCDELAY |            \
      NVVIOCONFIG_COMPOSITESYNCTYPE | NVVIOCONFIG_FRAMELOCKENABLE |     \
      NVVIOCONFIG_RESERVED_SDIOUTPUTENABLE | NVVIOCONFIG_422FILTER |    \
      NVVIOCONFIG_COMPOSITETERMINATE | NVVIOCONFIG_DATAINTEGRITYCHECK | \
      NVVIOCONFIG_CSCOVERRIDE | NVVIOCONFIG_FLIPQUEUELENGTH |           \
      NVVIOCONFIG_ANCTIMECODEGENERATION | NVVIOCONFIG_COMPOSITE |       \
      NVVIOCONFIG_ALPHAKEYCOMPOSITE | NVVIOCONFIG_COMPOSITE_Y |         \
      NVVIOCONFIG_COMPOSITE_CR | NVVIOCONFIG_COMPOSITE_CB |             \
      NVVIOCONFIG_FULL_COLOR_RANGE | NVVIOCONFIG_RGB_DATA |             \
      NVVIOCONFIG_RESERVED_SDIOUTPUTENABLE | NVVIOCONFIG_STREAMS |      \
      NVVIOCONFIG_ANC_PARITY_COMPUTATION | NVVIOCONFIG_ANC_AUDIO_REPEAT )

#define NVVIOCONFIG_DRIVERFIELDS                          \
    ( NVVIOCONFIG_OUTPUTREGION | NVVIOCONFIG_OUTPUTAREA | \
      NVVIOCONFIG_COLORCONVERSION | NVVIOCONFIG_FLIPQUEUELENGTH )

#define NVVIOCONFIG_GAMMAFIELDS ( NVVIOCONFIG_GAMMACORRECTION )

#define NVVIOCONFIG_RMCTRLFIELDS                                        \
    ( NVVIOCONFIG_SIGNALFORMAT | NVVIOCONFIG_DATAFORMAT |               \
      NVVIOCONFIG_SYNCSOURCEENABLE | NVVIOCONFIG_COMPOSITESYNCTYPE |    \
      NVVIOCONFIG_FRAMELOCKENABLE | NVVIOCONFIG_422FILTER |             \
      NVVIOCONFIG_COMPOSITETERMINATE | NVVIOCONFIG_DATAINTEGRITYCHECK | \
      NVVIOCONFIG_COMPOSITE | NVVIOCONFIG_ALPHAKEYCOMPOSITE |           \
      NVVIOCONFIG_COMPOSITE_Y | NVVIOCONFIG_COMPOSITE_CR |              \
...
#define NVVIOCONFIG_ALLOWSDIRUNNING_FIELDS                          \
    ( NVVIOCONFIG_DATAINTEGRITYCHECK | NVVIOCONFIG_SYNCDELAY |      \
      NVVIOCONFIG_CSCOVERRIDE | NVVIOCONFIG_ANCTIMECODEGENERATION | \
      NVVIOCONFIG_COMPOSITE | NVVIOCONFIG_ALPHAKEYCOMPOSITE |       \
      NVVIOCONFIG_COMPOSITE_Y | NVVIOCONFIG_COMPOSITE_CR |          \
      NVVIOCONFIG_COMPOSITE_CB | NVVIOCONFIG_ANC_PARITY_COMPUTATION )

#define NVVIOCONFIG_RMMODESET_FIELDS                               \
    ( NVVIOCONFIG_SIGNALFORMAT | NVVIOCONFIG_DATAFORMAT |          \
      NVVIOCONFIG_SYNCSOURCEENABLE | NVVIOCONFIG_FRAMELOCKENABLE | \
...
typedef struct _NVVIOOUTPUTCONFIG_V1 {
    ...
    NVVIOOUTPUTAREA
    outputArea; //!< Usable resolution for video output (safe area)
    ...
    NvU32 psfSignalFormat; //!< Indicates whether contained format is PSF Signal
                           //!< format
    NvU32 enable422Filter; //!< Enables/Disables 4:2:2 filter
    NvU32 compositeTerminate;       //!< Composite termination
    NvU32 enableDataIntegrityCheck; //!< Enable data integrity check: true -
                                    //!< enable, false - disable
    NvU32 cscOverride; //!< Use provided CSC color matrix to overwrite
    NvU32
        flipQueueLength; //!< Number of buffers used for the internal flipqueue
    ...
    NvU8 reservedData[ 256 ]; //!< Inicates last stored SDI output state TRUE-ON
                              //!< / FALSE-OFF
...
typedef struct _NVVIOOUTPUTCONFIG_V2 {
    ...
    NvU32 cscOverride; //!< Use provided CSC color matrix to overwrite
    NvU32
        flipQueueLength; //!< Number of buffers used for the internal flip queue
    NvU32 enableANCTimeCodeGeneration; //!< Enable SDI ANC time code generation
    NvU32 enableComposite;             //!< Enable composite
    NvU32 enableAlphaKeyComposite;     //!< Enable Alpha key composite
    NVVIOCOMPOSITERANGE compRange;     //!< Composite ranges
    NvU8 reservedData[ 256 ];   //!< Indicates last stored SDI output state
                                //!< TRUE-ON / FALSE-OFF
    NvU32 enableFullColorRange; //!< Flag indicating Full Color Range
    NvU32 enableRGBData;        //!< Indicates data is in RGB format
    NVVIOANCPARITYCOMPUTATION
    ancParityComputation; //!< Enable HW ANC parity bit computation
                          //!< (auto/on/off)
...
typedef struct _NVVIOINPUTCONFIG {
    NvU32 numRawCaptureImages; //!< numRawCaptureImages is the number of frames
                               //!< to keep in the capture queue. must be
                               //!< between NVAPI_GVI_MIN_RAW_CAPTURE_IMAGES and
                               //!< NVAPI_GVI_MAX_RAW_CAPTURE_IMAGES,
    NVVIOSIGNALFORMAT
    signalFormat;     //!< Signal format.
                      //!< Please note that both numRawCaptureImages and
                      //!< signalFormat should be set together.
    ...
    NvU32 bTestMode; //!< This attribute controls the GVI test mode.
                     ...
                     //!< GVI device will generate fake data as quickly as
                     //!< possible.
} NVVIOINPUTCONFIG;
...
typedef struct {
    NvPhysicalGpuHandle
        hPhysicalGpu; //!< Handle to Physical GPU (This could be NULL for GVI
                      //!< device if its not binded)
...
typedef struct _NV_VIO_TOPOLOGY {
    ...
    NVVIOTOPOLOGYTARGET
    vioTarget[ NVAPI_MAX_VIO_DEVICES ]; //!< Array of video I/O targets
...
//!
//!   Description: This API determine the graphics adapter video I/O
//!   capabilities.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \since Release: 190
//!
//! \param [in]  NvVioHandle   The caller provides the SDI device handle as
//! input.
//! \param [out] pAdapterCaps  Pointer to receive capabilities
//!
//! \retval ::NVAPI_OK                           Success
//! \retval ::NVAPI_API_NOT_INTIALIZED           NVAPI Not Initialized
//! \retval ::NVAPI_INVALID_ARGUMENT             Arguments passed to API are not
//! valid
//! \retval ::NVAPI_INCOMPATIBLE_STRUCT_VERSION  NVVIOCAPS struct version used
//! by the app is not compatible
...
//! \since Release: 190
//!
//! \param [in]  NvVioHandle      The caller provides the SDI output device
//! handle as input.
//! \param [in]  vioClass         Class interface (NVVIOCLASS_* value)
//! \param [in]  ownerType        Specify NVVIOOWNERTYPE_APPLICATION or
...
//! \retval ::NVAPI_NOT_SUPPORTED                Video I/O not supported
//! \retval ::NVAPI_ERROR                        NVAPI Random errors
//! \retval ::NVAPI_DEVICE_BUSY                  Access denied for requested
//! access
///////////////////////////////////////////////////////////////////////////////
...
//!
//!   Description: This API closes the graphics adapter for graphics-to-video
//!   operations
...
//! \since Release: 190
//!
//! \param [in]  NvVioHandle   The caller provides the SDI output device handle
//! as input.
...
//! \param [in]  NvVioHandle  The caller provides the SDI device handle as
//! input.
//! \param [out] pWait        Pointer to receive how many milliseconds will
//! lapse
...
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_VIO_SetConfig. SUPPORTED OS:  Windows XP and higher
//!
//!
//! \since Release: 190
//!
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_VIO_SetConfig." ) NVAPI_INTERFACE
...
//!
//! \deprecated  Do not use this function - it is deprecated in release 290.
//! Instead, use NvAPI_VIO_GetConfig. SUPPORTED OS:  Windows XP and higher
//!
//!
//! \since Release: 190
//!
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_VIO_GetConfig." ) NVAPI_INTERFACE
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_VIO_SetConfig." ) NVAPI_INTERFACE
    NvAPI_VIO_SetSyncDelay( NvVioHandle hVioHandle,
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 290. Instead, use "
    "NvAPI_VIO_GetConfig." ) NVAPI_INTERFACE
    NvAPI_VIO_GetSyncDelay( NvVioHandle hVioHandle,
...
typedef struct _NVVIOPCIINFO {
    ...
    NvU32 pciDeviceId; //!< specifies the internal PCI device identifier for the
                       //!< GVI.
    NvU32 pciSubSystemId; //!< specifies the internal PCI subsystem identifier
                          //!< for the GVI.
    NvU32 pciRevisionId;  //!< specifies the internal PCI device-specific
                          //!< revision identifier for the GVI.
    ...
    NVVIOPCILINKWIDTH
    pciLinkWidth; //!< specifies the the negotiated PCIE link width.
    NVVIOPCILINKRATE
    pciLinkRate; //!< specifies the the negotiated PCIE link rate.
} NVVIOPCIINFO_V1;
...
//                  pVioPCIInfo    (OUT)    - PCI information of the attached
//                  SDI capture card.
//
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \since Release: 190
//!
//! \param [in]  NvVioHandle[IN]          The caller provides the SDI device
//! handle as input.
//!
...
//!              This API should be called for NVVIOOWNERTYPE_DESKTOP only and
//!              will not work for OGL applications.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \since Release: 190
//!
//! \param [in]  NvVioHandle[IN]     The caller provides the SDI device handle
//! as input.
//!
//! \retval ::NVAPI_OK                           Success
//! \retval ::NVAPI_API_NOT_INTIALIZED           NVAPI Not Initialized
...
//!
//!   Description: This API checks whether modes are compatible in frame lock
//!   mode.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \since Release: 190
//!
//! \param [in]   NvVioHandle          The caller provides the SDI device handle
//! as input.
...
//! \since Release: 190
//!
//! \param [out]  NvVioHandle                  User passes the pointer of
//! NvVioHandle[] array to get handles to
//!                                            all the connected video I/O
//!                                            devices.
//! \param [out]  vioDeviceCount               User gets total number of VIO
...
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_VIO_EnumDevices(
...
//! \since Release: 190
//!
//! \param [out] pNvVIOTopology     User passes the pointer to NVVIOTOPOLOGY to
//! fetch all valid SDI topologies.
//!
//! \retval ::NVAPI_OK                           Success
//! \retval ::NVAPI_API_NOT_INTIALIZED           NVAPI Not Initialized
//! \retval ::NVAPI_INVALID_ARGUMENT             Arguments passed to API are not
...
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_VIO_EnumSignalFormats(
...
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_VIO_EnumDataFormats(
    NvVioHandle hVioHandle,
    NvU32 enumIndex,
...
//
//! \fn
//! NvAPI_Stereo_CreateConfigurationProfileRegistryKey(NV_STEREO_REGISTRY_PROFILE_TYPE
//! registryProfileType)
//!
//! DESCRIPTION:   Creates new configuration registry key for current
//! application.
//!
//!                If there is no configuration profile prior to the function
//!                call, this API tries to create a new configuration profile
//!                registry key for a given application and fill it with the
//!                default values. If an application already has a configuration
//!                profile registry key, the API does nothing. The name of the
//!                key is automatically set to the name of the executable that
//!                calls this function. Because of this, the executable should
//!                have a distinct and unique name. If the application is using
//!                only one version of DirectX, then the default profile type
//!                will be appropriate. If the application is using more than
//!                one version of DirectX from the same executable, it should
//!                use the appropriate profile type for each configuration
//!                profile.
//!
//! HOW TO USE:    When there is a need for an application to have default
...
//! \since Release: 180
//!
//! \param [in]    registryProfileType  Type of profile the application wants to
//! create. It should be one of the symbolic constants defined in
//!                                     ::NV_STEREO_REGISTRY_PROFILE_TYPE. Any
//!                                     other value will cause function to do
//!                                     nothing and return
//!                                     ::NV_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED.
//!
//! \retval ::NVAPI_OK                                           Key exists in
//! the registry.
//! \retval ::NVAPI_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED   This profile
//! type is not supported.
//! \retval ::NVAPI_STEREO_REGISTRY_ACCESS_FAILED                Access to
//! registry failed.
//! \retval ::NVAPI_API_NOT_INTIALIZED
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED                       Stereo part of
...
typedef enum _NV_StereoRegistryProfileType {
    NVAPI_STEREO_DEFAULT_REGISTRY_PROFILE, //!< Default registry configuration
                                           //!< profile.
    NVAPI_STEREO_DX9_REGISTRY_PROFILE,     //!< Separate registry configuration
                                       //!< profile for a DirectX 9 executable.
    NVAPI_STEREO_DX10_REGISTRY_PROFILE //!< Separate registry configuration
                                       //!< profile for a DirectX 10 executable.
} NV_STEREO_REGISTRY_PROFILE_TYPE;

//! \ingroup stereoapi
NVAPI_INTERFACE NvAPI_Stereo_CreateConfigurationProfileRegistryKey(
...
//!
//!                If an application already has a configuration profile prior
//!                to this function call, the function attempts to remove the
//!                application's configuration profile registry key from the
//!                registry. If there is no configuration profile registry key
//!                prior to the function call, the function does nothing and
...
//! \since Release: 180
//!
//! \param [in]   registryProfileType   Type of profile that the application
//! wants to delete. This should be one of the symbolic
//!                                     constants defined in
//!                                     ::NV_STEREO_REGISTRY_PROFILE_TYPE. Any
//!                                     other value will cause the function to
//!                                     do nothing and return
//!                                     ::NV_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED.
//!
//! \retval ::NVAPI_OK                                           Key does not
//! exist in the registry any more.
//! \retval ::NVAPI_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED   This profile
//! type is not supported.
//! \retval ::NVAPI_STEREO_REGISTRY_ACCESS_FAILED                Access to
//! registry failed.
//! \retval ::NVAPI_API_NOT_INTIALIZED                           NVAPI is not
//! initialized.
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED                       Stereo part of
//! NVAPI is not initialized.
//! \retval ::NVAPI_ERROR
//!
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_DeleteConfigurationProfileRegistryKey(
    NV_STEREO_REGISTRY_PROFILE_TYPE registryProfileType );
...
//
//! \fn
//! NvAPI_Stereo_SetConfigurationProfileValue(NV_STEREO_REGISTRY_PROFILE_TYPE
//! registryProfileType, NV_STEREO_REGISTRY_ID valueRegistryID, void *pValue)
//!
//! DESCRIPTION:   This API sets the given parameter value under the
//! application's registry key.
//!
//!                If the value does not exist under the application's registry
//!                key, the value will be created under the key.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
//! \since Release: 180
//!
//! \param [in]     registryProfileType  The type of profile the application
...
//!                                      nothing and return
//!                                      ::NV_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED.
//! \param [in]     valueRegistryID      ID of the value that is being set. It
//! should be one of the symbolic constants defined in
//!                                      ::NV_STEREO_REGISTRY_PROFILE_TYPE. Any
//!                                      other value will cause function to do
//!                                      nothing and return
//!                                      ::NVAPI_STEREO_REGISTRY_VALUE_NOT_SUPPORTED.
//! \param [in]     pValue               Address of the value that is being set.
//! It should be either address of a DWORD or of a float,
//!                                      dependent on the type of the stereo
//!                                      parameter whose value is being set. The
//!                                      API will then cast that address to
//!                                      DWORD* and write whatever is in those 4
//!                                      bytes as a DWORD to the registry.
//!
//! \retval ::NVAPI_OK                                           Value is
//! written to registry.
//! \retval ::NVAPI_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED   This profile
//! type is not supported.
//! \retval ::NVAPI_STEREO_REGISTRY_VALUE_NOT_SUPPORTED          This value is
//! not supported.
//! \retval ::NVAPI_STEREO_REGISTRY_ACCESS_FAILED                Access to
//! registry failed.
//! \retval ::NVAPI_API_NOT_INTIALIZED                           NVAPI is not
...
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED                       Stereo part of
//! NVAPI is not initialized.
//! \retval ::NVAPI_ERROR                                        Something is
//! wrong (generic error).
//
///////////////////////////////////////////////////////////////////////////////
...
typedef enum _NV_StereoRegistryID {
    ...
    NVAPI_FRUSTUM_ADJUST_MODE_ID, //!< Symbolic constant for frustum adjust mode
                                  //!< registry ID.
} NV_STEREO_REGISTRY_ID;

//! \ingroup stereoapi
NVAPI_INTERFACE NvAPI_Stereo_SetConfigurationProfileValue(
    NV_STEREO_REGISTRY_PROFILE_TYPE registryProfileType,
...
//
//! DESCRIPTION:   This API removes the given value from the application's
//! configuration profile registry key.
//!                If there is no such value, the function does nothing and does
//!                not report an error.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
...
//!                                       nothing and return
//!                                       ::NV_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED.
//! \param [in]     valueRegistryID       ID of the value that is being deleted.
...
//!                                       ::NVAPI_STEREO_REGISTRY_VALUE_NOT_SUPPORTED.
//!
//! \retval ::NVAPI_OK                                           Value does not
//! exist in registry any more.
//! \retval ::NVAPI_STEREO_REGISTRY_PROFILE_TYPE_NOT_SUPPORTED   This profile
//! type is not supported.
//! \retval ::NVAPI_STEREO_REGISTRY_VALUE_NOT_SUPPORTED          This value is
//! not supported.
...
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_DeleteConfigurationProfileValue(
    NV_STEREO_REGISTRY_PROFILE_TYPE registryProfileType,
    NV_STEREO_REGISTRY_ID valueRegistryID );
...
//
//! DESCRIPTION:  This API checks what kind of stereo support is currently
//! supported on a particular display.
//!               If the the display is prohibited from showing stereo (e.g.
//!               secondary in a multi-mon setup), we will return 0 for all
//!               stereo modes (full screen exclusive, automatic windowed,
//!               persistent windowed). Otherwise, we will check which stereo
//!               mode is supported. On 120Hz display, this will be what the
//!               user chooses in control panel. On HDMI 1.4 display, persistent
//!               windowed mode is always assumed to be supported. Note that
//!               this function does not check if the CURRENT RESOLUTION/REFRESH
//!               RATE can support stereo. For HDMI 1.4, it is the app's
//!               responsibility to change the resolution/refresh rate to one
//!               that is 3D compatible. For 120Hz, the driver will ALWAYS force
//!               120Hz anyway.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
...
//! \param [out]    pCaps    Address where the result of the inquiry will be
//! placed.
//!                          *pCaps is defined in NVAPI_STEREO_CAPS.
//! \return       This API can return any of the following error codes
...
//
//! DESCRIPTION:   This API decreases separation for the given device interface
//! (just like the Ctrl+F3 hotkey).
//!
//! WHEN TO USE:   After the stereo handle for device interface is created via
//! successfull call to the appropriate NvAPI_Stereo_CreateHandleFrom()
//! function.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
...
//! \since Release: 180
//!
//! \param [in]   stereoHandle  Stereo handle that corresponds to the device
//! interface.
//!
//! \retval ::NVAPI_OK - Decrease of separation percentage was successfull.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE - Device interface is not
//! valid. Create again, then attach again.
//! \retval ::NVAPI_API_NOT_INTIALIZED - NVAPI not initialized.
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED - Stereo part of NVAPI not
//! initialized.
...
//
//! DESCRIPTION:   This API increases separation for the given device interface
//! (just like the Ctrl+F4 hotkey).
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
...
//! interface.
//!
//! \retval ::NVAPI_OK                               Increase of separation
//! percentage was successfull.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE  Device interface is not
//! valid. Create again, then attach again.
//! \retval ::NVAPI_API_NOT_INTIALIZED               NVAPI not initialized.
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED           Stereo part of NVAPI not
//! initialized.
//! \retval ::NVAPI_ERROR                            Something is wrong (generic
//! error).
//!
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
...
//
//! DESCRIPTION:   This API decreases convergence for the given device interface
//! (just like the Ctrl+F5 hotkey).
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
//! via successfull call to the appropriate NvAPI_Stereo_CreateHandleFrom()
//! function.
...
//
//! DESCRIPTION:   This API increases convergence for given the device interface
//! (just like the Ctrl+F5 hotkey).
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
//! via successfull call to the appropriate NvAPI_Stereo_CreateHandleFrom()
...
//! interface.
//!
//! \retval ::NVAPI_OK                               Increase of convergence was
//! successfull.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE  Device interface is not
//! valid. Create again, then attach again.
//! \retval ::NVAPI_API_NOT_INTIALIZED
...
//
//! \fn NvAPI_Stereo_GetFrustumAdjustMode(StereoHandle stereoHandle,
//! NV_FRUSTUM_ADJUST_MODE *pFrustumAdjustMode) DESCRIPTION:   This API gets the
//! current frustum adjust mode value.
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
//! via successfull call to the appropriate NvAPI_Stereo_CreateHandleFrom()
//! function.
...
//! \since Release: 180
//!
//! \param [in]     stereoHandle         Stereo handle that corresponds to the
//! device interface.
//! \param [out]    pFrustumAdjustMode   Address of the NV_FRUSTUM_ADJUST_MODE
//! type variable to store current frustum value in.
//!
...
//! \ingroup stereoapi
NVAPI_INTERFACE NvAPI_Stereo_GetFrustumAdjustMode(
...
//! \since Release: 180
//!
//! \param [in]     stereoHandle                Stereo handle that corresponds
//! to the device interface.
//! \param [in]     newFrustumAdjustModeValue   New value for frustum adjust
//! mode. It should be one of the symbolic constants defined in
//!                                             ::NV_FRUSTUM_ADJUST_MODE. Any
//!                                             other value will cause function
//!                                             to do nothing and return
//!                                             ::NVAPI_STEREO_FRUSTUM_ADJUST_MODE_NOT_SUPPORTED.
//!
//! \retval ::NVAPI_OK                                         Retrieval of
//! frustum adjust mode was successfull.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE            Device interface
//! is not valid. Create again, then attach again.
//! \retval ::NVAPI_API_NOT_INTIALIZED
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED                     Stereo part of
//! NVAPI not initialized.
//! \retval ::NVAPI_STEREO_FRUSTUM_ADJUST_MODE_NOT_SUPPORTED   Given frustum
//! adjust mode is not supported.
//! \retval ::NVAPI_ERROR
//!
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_SetFrustumAdjustMode(
    StereoHandle stereoHandle,
...
//
//! DESCRIPTION:   This API captures the current stereo image in JPEG stereo
//! format with the given quality.
...
//! \param [in]     stereoHandle  Stereo handle that corresponds to the device
//! interface.
//! \param [in]     quality        Quality of the JPEG image to be captured.
...
//! \retval ::NVAPI_STEREO_PARAMETER_OUT_OF_RANGE   Given quality is out of
//! [0..100] range.
//! \retval ::NVAPI_ERROR
//!
//! \ingroup stereoapi
...
//
//! DESCRIPTION:   This API allows an application to enable stereo viewing,
//! without the need of a GUID/Key pair
//!				   This API cannot be used to enable stereo
//! viewing on 3DTV.
//!
//! HOW TO USE:    Call this function immediately after device creation, then
//! follow with a reset. \n
//!                Very generically:
//!                Create Device->Create Stereo Handle->InitActivation->Reset
//!                Device
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//! \since Release: 302
//!
//! \param [in]   stereoHandle            Stereo handle corresponding to the
//! device interface.
...
NVAPI_INTERFACE NvAPI_Stereo_InitActivation(
...
//
//! DESCRIPTION:   This API allows an application to trigger creation of a
//! stereo desktop,
//!				   in case the creation was stopped on
//! application launch.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
...
//!         If there are return error codes with specific meaning for this API,
//!         they are listed below.
//! \retval ::NVAPI_STEREO_INIT_ACTIVATION_NOT_DONE - Stereo InitActivation not
//! called.
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED - Stereo part of NVAPI not
//! initialized.
//!
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_Trigger_Activation(
    __in StereoHandle hStereoHandle );
...
//
//! DESCRIPTION:   This API captures the current stereo image in PNG stereo
//! format.
//!                Only the last capture call per flip will be effective.
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
...
//! \since Release: 180
//!
//! \param [in]     stereoHandle  Stereo handle corresponding to the device
//! interface.
//!
//! \retval ::NVAPI_OK                               Image captured.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE  Device interface is not
...
//!
//! HOW TO USE:    Use after the stereo handle for the device interface is
//! created via successfull call to the appropriate
//!                NvAPI_Stereo_CreateHandleFrom() function.
//!                After reversed stereo blit control is turned on, blits from
//!                the stereo surface will produce the right-eye image in the
//!                left side of the destination surface and the left-eye image
//!                in the right side of the destination surface.
//!
//!                In DirectX 9, the destination surface must be created as the
//!                render target, and StretchRect must be used. Conditions:
//!                - DstWidth == 2*SrcWidth
//!                - DstHeight == SrcHeight
...
//!                - dstY == 0,
//!                - dstZ == 0,
//!                - SrcBox: left=top=front==0; right==SrcWidth;
//!                bottom==SrcHeight; back==1;
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
...
//!
//!
//! \retval ::NVAPI_OK                                Retrieval of frustum
//! adjust mode was successfull.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE   Device interface is not
//! valid. Create again, then attach again.
//! \retval ::NVAPI_API_NOT_INTIALIZED
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED            Stereo part of NVAPI not
...
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_ReverseStereoBlitControl(
...
//
//! DESCRIPTION:   This API is a Setup notification message that the stereo
//! driver uses to notify the application
//!                when the user changes the stereo driver state.
//!
//!                When the user changes the stereo state (Activated or
//!                Deactivated, separation or conversion) the stereo driver
//!                posts a defined message with the following parameters:
//!
//!                lParam  is the current conversion. (Actual conversion is
//!                *(float*)&lParam )
//!
//!                wParam == MAKEWPARAM(l, h) where
//!                - l == 0 if stereo is deactivated
//!                - l == 1 if stereo is deactivated
//!                - h is the current separation. (Actual separation is
//!                float(h*100.f/0xFFFF)
//!
//!                Call this API with NULL hWnd to prohibit notification.
//!
//! WHEN TO USE:   Use after the stereo handle for device interface is created
//! via successful call to appropriate
//!                NvAPI_Stereo_CreateHandleFrom() function.
//!
//!
//! SUPPORTED OS:  Windows Vista and higher
...
//! \param [in]     stereoHandle  Stereo handle corresponding to the device
//! interface.
//! \param [in]     hWnd          Window HWND that will be notified when the
//! user changes the stereo driver state.
//!                               Actual HWND must be cast to an NvU64.
//! \param [in]     messageID     MessageID of the message that will be posted
//! to hWnd
//!
...
//
//! DESCRIPTION: This API allows the user to create a mono or a stereo swap
//! chain.
//!
//!          NOTE: NvAPI_D3D1x_CreateSwapChain is a wrapper of the method
//!          IDXGIFactory::CreateSwapChain which
//!                additionally notifies the D3D driver of the mode in which
//!                stereo mode the swap chain is to be created.
//!
//! \since Release: 285
//!
//! SUPPORTED OS:  Windows 7 and higher
//!
//!
//! \param [in]     hStereoHandle      Stereo handle that corresponds to the
//! device interface.
//!                                   A pointer to the device that will write 2D
//!                                   images to the swap chain.
//! \param [in]     pDesc             A pointer to the swap-chain description
...
#endif // if defined(__d3d10_h__) || defined(__d3d10_1_h__) ||
       // defined(__d3d11_h__)
...
//! chain.
//!
//!          NOTE: NvAPI_D3D9_CreateSwapChain is a wrapper of the method
//!          IDirect3DDevice9::CreateAdditionalSwapChain which
//!                additionally notifies the D3D driver if the swap chain
...
//!
//!
//! \param [in]      hStereoHandle            Stereo handle that corresponds to
//! the device interface.
//! \param [in, out] pPresentationParameters  A pointer to the swap-chain
//! description (DXGI). This parameter cannot be NULL.
//! \param [out]     ppSwapChain              A pointer to the swap chain
//! created.
//! \param [in]      mode                     The stereo mode for the swap
//! chain.
//!                                           NVAPI_STEREO_SWAPCHAIN_DEFAULT
//!                                           NVAPI_STEREO_SWAPCHAIN_STEREO
//!                                           NVAPI_STEREO_SWAPCHAIN_MONO
//!
//! \retval ::NVAPI_OK                        The swap chain creation was
//! successful
//! \retval ::NVAPI_ERROR                     The operation failed.
//!
//!\ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_D3D9_CreateSwapChain(
    StereoHandle hStereoHandle,
...
typedef struct _NVDRS_SETTING_VALUES {
    ...
    {
        NvU32
            u32DefaultValue; //!< Accessing default DWORD value of this setting.
        NVDRS_BINARY_SETTING
        binaryDefaultValue; //!< Accessing default Binary value of this
                            //!< setting. Must be allocated by caller with
                            //!< valueLength specifying buffer size, or only
                            //!< valueLength will be filled in.
    ...
    {     //!< NOT mixed types.
        ...
        NVDRS_BINARY_SETTING
        binaryValue; //!< All possible Binary values for a setting
        NvAPI_UnicodeString wszValue; //!< Accessing current unicode string
...
typedef struct _NVDRS_SETTING {
    ...
    NVDRS_SETTING_TYPE settingType;         //!< Type of setting value.
    NVDRS_SETTING_LOCATION settingLocation; //!< Describes where the value in
                                            //!< CurrentValue comes from.
    NvU32 isCurrentPredefined; //!< It is different than 0 if the currentValue
                               //!< is a predefined Value, 0 if the currentValue
                               //!< is a user value.
    ...
    union //!< Setting can hold either DWORD or Binary value or string. Not
          //!< mixed types.
    {
        NvU32 u32PredefinedValue; //!< Accessing default DWORD value of this
                                  //!< setting.
        NVDRS_BINARY_SETTING
        binaryPredefinedValue; //!< Accessing default Binary value of this
                               //!< setting. Must be allocated by caller
                               //!< with valueLength specifying buffer size,
    ...
    {
        NvU32
            u32CurrentValue; //!< Accessing current DWORD value of this setting.
        NVDRS_BINARY_SETTING
        binaryCurrentValue; //!< Accessing current Binary value of this
                            //!< setting. Must be allocated by caller with
                            //!< valueLength specifying buffer size, or only
                            //!< valueLength will be filled in.
        NvAPI_UnicodeString wszCurrentValue; //!< Accessing current unicode
                                             //!< string value of this setting.
    };
} NVDRS_SETTING;
...
typedef struct _NVDRS_APPLICATION_V1 {
    ...
    NvAPI_UnicodeString
        userFriendlyName;         //!< UserFriendly name of the Application
    NvAPI_UnicodeString launcher; //!< Indicates the name (if any) of the
                                  //!< launcher that starts the application
...
typedef struct _NVDRS_APPLICATION_V2 {
    ...
    NvAPI_UnicodeString launcher; //!< Indicates the name (if any) of the
                                  //!< launcher that starts the Application
    NvAPI_UnicodeString
        fileInFolder; //!< Select this application only if this file is found.
                      //!< When specifying multiple files, separate them using
                      //!< the ':' character.
...
typedef struct _NVDRS_PROFILE {
    ...
    NVDRS_GPU_SUPPORT
    gpuSupport; //!< This read-only flag indicates the profile support on
                //!< either Quadro, or Geforce, or both.
    NvU32 isPredefined;  //!< Is the Profile user-defined, or predefined
    NvU32 numOfApps;     //!< Total number of applications that belong to this
                         //!< profile. Read-only
    NvU32 numOfSettings; //!< Total number of settings applied for this Profile.
                         //!< Read-only
} NVDRS_PROFILE;
...
//
//!   DESCRIPTION: This API deletes a profile or sets it back to a predefined
//!   value.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \ingroup drsapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DRS_SetCurrentGlobalProfile(
...
//! \ingroup drsapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DRS_GetCurrentGlobalProfile(
    NvDRSSessionHandle hSession,
    NvDRSProfileHandle* phProfile );
...
//
//!   DESCRIPTION: This API gets information about the given profile. User needs
//!   to specify the name of the Profile.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//
//!   DESCRIPTION: Specifies flags for a given profile. Currently only the
//!   NVDRS_GPU_SUPPORT is
//!                used to update the profile. Neither the name, number of
//!                settings or applications or other profile information can be
...
//!                  NVAPI_END_ENUMERATION: index exceeds the total number of
...
//
//!   DESCRIPTION: This API obtains the number of profiles in the current
//!   session object.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \param [in]  hSession       Input to the session handle.
//! \param out]  *numProfiles   Returns count of profiles in the current
//! hSession.
//!
//! \retval ::NVAPI_OK                  SUCCESS
...
//! \param [in]  hSession       Input to the session handle.
//! \param [in]  hProfile       Input profile handle.
//! \param [in]  *pApplication  Input NVDRS_APPLICATION struct with the
//! executable name to be added.
//!
//! \retval ::NVAPI_OK     SUCCESS
//! \retval ::NVAPI_ERROR  For miscellaneous errors.
//!
...
//! \param [in]   hSession  - Input to the session handle.
//! \param [in]   hProfile  - Input profile handle.
//! \param [in]   *pApp     - Input all the information about the application to
//! be removed.
//!
//! \retval ::NVAPI_OK  SUCCESS
//! \retval ::NVAPI_ERROR For miscellaneous errors.
//! \retval ::NVAPI_EXECUTABLE_PATH_IS_AMBIGUOUS If the path provided could
//! refer to two different executables,
...
//
//!   DESCRIPTION: This API gets information about the given application.  The
//!   input application name
//!                must match exactly what the Profile has stored for the
//!                application. This function is better used to retrieve
//!                application information from a previous enumeration.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//! \param [out]  *pApplication  Returns NVDRS_APPLICATION struct with all the
//! attributes.
//!
//! \return  This API can return any of the error codes enumerated in
//! #NvAPI_Status.
//!          If there are return error codes with specific meaning for this API,
//!          they are listed below.
//! \retval ::NVAPI_EXECUTABLE_PATH_IS_AMBIGUOUS   The application name could
//! not
//                                                single out only one
//                                                executable.
//! \retval ::NVAPI_EXECUTABLE_NOT_FOUND           No application with that name
...
//
//!   DESCRIPTION: This API enumerates all the applications in a given profile
//!   from the starting index to the maximum length.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//! \param [in,out]  *appCount        Input maximum length of the passed in
//! arrays. Returns the actual length.
//! \param [out]     *pApplication    Returns NVDRS_APPLICATION struct with all
//! the attributes.
//!
//! \retval ::NVAPI_OK               SUCCESS
//! \retval ::NVAPI_ERROR            For miscellaneous errors.
...
//
//!   DESCRIPTION: This API searches the application and the associated profile
//!   for the given application name.
//!                If a fully qualified path is provided, this function will
//!                always return the profile the driver will apply upon running
//!                the application (on the path provided).
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \param [in]   hSession       Input to the hSession handle
//! \param [in]   appName        Input appName. For best results, provide a
...
//! \retval ::NVAPI_EXECUTABLE_PATH_IS_AMBIGUOUS   If the input appName was not
//! fully qualified, this error might return in the case of multiple matches
//!
//! \ingroup drsapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DRS_FindApplicationByName(
    NvDRSSessionHandle hSession,
    NvAPI_UnicodeString appName,
    NvDRSProfileHandle* phProfile,
    NVDRS_APPLICATION* pApplication );
...
//
//!   DESCRIPTION: This API enumerates all the settings of a given profile from
//!   startIndex to the maximum length.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
...
//! \param [in]      hProfile        Input profile handle.
//! \param [in]      startIndex      Indicates starting index for enumeration.
//! \param [in,out]  *settingsCount  Input max length of the passed in arrays,
...
//
//!   DESCRIPTION: This API enumerates all the Ids of all the settings
//!   recognized by NVAPI.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \param [out]    pSettingIds     User-provided array of length *pMaxCount
//! that NVAPI will fill with IDs.
//! \param [in,out] pMaxCount       Input max length of the passed in array,
//! Returns the actual length.
//!
//! \retval ::NVAPI_OK     SUCCESS
//! \retval ::NVAPI_ERROR  For miscellaneous errors.
//!                  NVAPI_END_ENUMERATION: the provided pMaxCount is not enough
...
//
//!   DESCRIPTION: This API enumerates all available setting values for a given
//!   setting.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
//!
//! \param [in]      settingId          Input settingId.
//! \param [in,out]  maxNumCount        Input max length of the passed in
//! arrays, Returns the actual length.
//! \param [out]     *pSettingValues    Returns all available setting values and
//! its count.
//!
//! \retval ::NVAPI_OK     SUCCESS
//! \retval ::NVAPI_ERROR  For miscellaneous errors.
...
//! \ingroup drsapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DRS_EnumAvailableSettingValues(
    NvU32 settingId,
    NvU32* pMaxNumValues,
...
//
//!   DESCRIPTION: This API gets the binary ID of a setting given the setting
//!   name.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \ingroup drsapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DRS_GetSettingNameFromId(
    NvU32 settingId,
...
//
//!   DESCRIPTION: This API deletes a setting or sets it back to predefined
//!   value.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//
//!   DESCRIPTION: This API restores the whole system to predefined(default)
//!   values.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//
//!   DESCRIPTION: This API restores the given profile to predefined(default)
//!   values.
//!                Any and all user specified modifications will be removed.
//!                If the whole profile was set by the user, the profile will be
//!                removed.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//! \retval ::NVAPI_OK              SUCCESS if the profile is found
//! \retval ::NVAPI_ERROR           For miscellaneous errors.
//! \retval ::NVAPI_PROFILE_REMOVED SUCCESS, and the hProfile is no longer
//! valid.
//! \retval ::NVAPI_ERROR           For miscellaneous errors.
//!
//! \ingroup drsapi
...
//
//!   DESCRIPTION: This API restores the given profile setting to
...
//! \ingroup drsapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_DRS_RestoreProfileDefaultSetting(
    NvDRSSessionHandle hSession,
    NvDRSProfileHandle hProfile,
    NvU32 settingId );
...
//! \retval  NVAPI_INCOMPATIBLE_STRUCT_VERSION   NV_CHIPSET_INFO version not
...
typedef struct {
    NvU32 version; //! Structure version, constructed from the macro
...
//
//! DESCRIPTION:     This API converts a display ID to a Physical GPU handle and
//! output ID.
//!
//! SUPPORTED OS:  Windows XP and higher
//!
...
//
//! \code
//! DESCRIPTION:     This API retrieves the Physical GPU handle of the connected
//! display
//!
//! \since Release: 313
//!
