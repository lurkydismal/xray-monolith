...
//!                If stereo is not enabled, then calls to functions that
//!                require that stereo is enabled have no effect, and will
...
//! \since Release: 180
//!
//! \param [out]     pIsStereoEnabled   Address where the result of the inquiry
//! will be placed.
//!
//! \retval ::NVAPI_OK                       Check was sucessfully completed and
//! result reflects current state of stereo availability.
//! \retval ::NVAPI_API_NOT_INTIALIZED
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED   Stereo part of NVAPI not
//! initialized.
//! \retval ::NVAPI_ERROR
...
//
//! DESCRIPTION:   This API creates a stereo handle that is used in subsequent
//! calls related to a given device interface.
//!                This must be called before any other NvAPI_Stereo_ function
//!                for that handle. Multiple devices can be used at one time
//!                using multiple calls to this function (one per each device).
//!
//! HOW TO USE:    After the Direct3D device is created, create the stereo
//! handle.
//!                On call success:
//!                -# Use all other NvAPI_Stereo_ functions that have stereo
//!                handle as first parameter.
//!                -# After the device interface that corresponds to the the
//!                stereo handle is destroyed, the application should call
//!                NvAPI_DestroyStereoHandle() for that stereo handle.
//!
...
//! \since Release: 180
//!
//! \param [in]     pDevice        Pointer to IUnknown interface that is
//! IDirect3DDevice9* in DX9, ID3D10Device*.
//! \param [out]    pStereoHandle  Pointer to the newly created stereo handle.
//!
//! \retval ::NVAPI_OK                       Stereo handle is created for given
//! device interface.
//! \retval ::NVAPI_INVALID_ARGUMENT         Provided device interface is
//! invalid.
//! \retval ::NVAPI_API_NOT_INTIALIZED
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED   Stereo part of NVAPI not
//! initialized.
...
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_CreateHandleFromIUnknown(
    IUnknown* pDevice,
...
//
//! DESCRIPTION:   This API destroys the stereo handle created with one of the
//! NvAPI_Stereo_CreateHandleFrom() functions.
//!                This should be called after the device corresponding to the
...
//
//! DESCRIPTION:   This API activates stereo for the device interface
//! corresponding to the given stereo handle.
//!                Activating stereo is possible only if stereo was enabled
//!                previously in the registry. If stereo is not activated, then
//!                calls to functions that require that stereo is activated have
//!                no effect, and will return the appropriate error code.
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
//! via successfull call to the appropriate NvAPI_Stereo_CreateHandleFrom()
//! function.
...
//!                If stereo is not activated, then calls to functions that
//!                require that stereo is activated have no effect, and will
//!                return the appropriate error code.
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
//! via successfull call to the appropriate NvAPI_Stereo_CreateHandleFrom()
...
//
//! DESCRIPTION:   This API checks if stereo is activated for the given device
//! interface.
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
//! via successfull call to the appropriate NvAPI_Stereo_CreateHandleFrom()
...
//! \param [in]    stereoHandle  Stereo handle that corresponds to the device
//! interface.
//! \param [in]    pIsStereoOn   Address where result of the inquiry will be
//! placed.
//!
//! \retval ::NVAPI_OK - Check was sucessfully completed and result reflects
...
//! device interface.
//! \param [out]    pSeparationPercentage  Address of @c float type variable to
//! store current separation percentage in.
//!
//! \retval ::NVAPI_OK                                Retrieval of separation
...
//!
//! WHEN TO USE:   After the stereo handle for the device interface is created
//! via successfull call to appropriate NvAPI_Stereo_CreateHandleFrom()
//! function.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
...
//! \since Release: 180
//!
//! \param [in]     stereoHandle             Stereo handle that corresponds to
//! the device interface.
//! \param [in]     newSeparationPercentage  New value for separation
//! percentage.
//!
//! \retval ::NVAPI_OK                               Setting of separation
//! percentage was successfull.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE  Device interface is not
//! valid. Create again, then attach again.
//! \retval ::NVAPI_API_NOT_INTIALIZED               NVAPI not initialized.
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED           Stereo part of NVAPI not
//! initialized.
//! \retval ::NVAPI_STEREO_PARAMETER_OUT_OF_RANGE    Given separation percentage
//! is out of [0..100] range.
//! \retval ::NVAPI_ERROR
//!
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
...
//! \param [in]     stereoHandle   Stereo handle that corresponds to the device
//! interface.
//! \param [out]    pConvergence   Address of @c float type variable to store
//! current convergence value in.
//!
//! \retval ::NVAPI_OK                               Retrieval of convergence
...
//!
//! \retval ::NVAPI_OK                                Setting of convergence
//! value was successfull.
//! \retval ::NVAPI_STEREO_INVALID_DEVICE_INTERFACE   Device interface is not
//! valid. Create again, then attach again.
//! \retval ::NVAPI_API_NOT_INTIALIZED
...
//
//! \fn NvAPI_Stereo_SetActiveEye(StereoHandle hStereoHandle,
//! NV_STEREO_ACTIVE_EYE StereoEye); DESCRIPTION:   This API sets the back
//! buffer to left or right in Direct stereo mode.
//!
//! HOW TO USE:    After the stereo handle for device interface is created via
...
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED - Stereo part of NVAPI not
//! initialized.
//! \retval ::NVAPI_INVALID_ARGUMENT - StereoEye parameter has not allowed
//! value.
...
//!
//!
//! \param [in]    mode       Defines the 3D stereo driver mode: Direct or
//! Automatic
//!
//! \retval ::NVAPI_OK                      Active eye is set.
//! \retval ::NVAPI_API_NOT_INTIALIZED      NVAPI not initialized.
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED  Stereo part of NVAPI not
//! initialized.
//! \retval ::NVAPI_INVALID_ARGUMENT        mode parameter has not allowed
//! value.
//! \retval ::NVAPI_ERROR                   Something is wrong (generic error).
//
///////////////////////////////////////////////////////////////////////////////
...
//
//! DESCRIPTION:   This API returns eye separation as a ratio of <between eye
...
//!
//!
//! \retval ::NVAPI_OK                      Retrieval of frustum adjust mode was
//! successfull.
//! \retval ::NVAPI_API_NOT_INTIALIZED      NVAPI not initialized.
//! \retval ::NVAPI_STEREO_NOT_INITIALIZED  Stereo part of NVAPI not
//! initialized.
...
//
//! \function NvAPI_Stereo_SetSurfaceCreationMode(StereoHandle hStereoHandle,
//! NVAPI_STEREO_SURFACECREATEMODE creationMode)
//! \param [in]   hStereoHandle   Stereo handle that corresponds to the device
//! interface.
//! \param [in]   creationMode    New surface creation mode for this device
//! interface.
//!
//! \since Release: 285
//!
...
//! #NvAPI_Status.
//!              There are no return error codes with specific meaning for this
//!              API.
//!
///////////////////////////////////////////////////////////////////////////////
...
typedef enum _NVAPI_STEREO_SURFACECREATEMODE {
    NVAPI_STEREO_SURFACECREATEMODE_AUTO, //!< Use driver registry profile
                                         //!< settings for surface creation
                                         //!< mode.
    NVAPI_STEREO_SURFACECREATEMODE_FORCESTEREO, //!< Always create stereo
                                                //!< surfaces.
...
//
//! \function NvAPI_Stereo_GetSurfaceCreationMode(StereoHandle hStereoHandle,
//! NVAPI_STEREO_SURFACECREATEMODE* pCreationMode)
//! \param [in]   hStereoHandle   Stereo handle that corresponds to the device
//! interface.
//! \param [out]   pCreationMode   The current creation mode for this device
//! interface.
//!
//! \since Release: 295
//!
...
//! \param [in]  hStereoHandle    Stereo handle that corresponds to the device
//! interface.
//! \param [out] pWasStereoized   Address where result of the inquiry will be
//! placed.
//!
//! SUPPORTED OS:  Windows Vista and higher
//!
//!
//! DESCRIPTION: This API checks if the last draw call was stereoized. It is a
//! very expensive to call and should be used for debugging purpose *only*.
//!
//! WHEN TO USE: After the stereo handle for device interface is created via
//! successful call to appropriate NvAPI_Stereo_CreateHandleFrom function.
//!
...
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_Debug_WasLastDrawStereoized(
    __in StereoHandle hStereoHandle,
...
//!
//!
//! DESCRIPTION: This API defines the stereo profile used by the driver in case
//! the application has no associated profile.
//!
//! WHEN TO USE: To take effect, this API must be called before D3D device is
//! created. Calling once a device has been created will not affect the current
//! device.
//!
...
//!
//! \retval      NVAPI_SUCCESS                               - Default stereo
//! profile name has been copied into szProfileName.
//! \retval      NVAPI_INVALID_ARGUMENT                      - szProfileName ==
//! NULL.
//! \retval      NVAPI_DEFAULT_STEREO_PROFILE_DOES_NOT_EXIST - Default stereo
//! profile does not exist
//!
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_Stereo_SetDefaultProfile(
    __in const char* szProfileName );
///////////////////////////////////////////////////////////////////////////////
//
...
//!
//!              After call cbSizeOut contain 0 if default profile is not set
//!              required buffer size cbSizeOut. To get needed buffer size this
//!              function can be called with szProfileName==0 and cbSizeIn == 0.
//!
//! WHEN TO USE: This API can be called at any time.
//!
//!
//! \param [in]   cbSizeIn             Size of buffer allocated for default
//! stereo profile name.
//! \param [out]  szProfileName        Default stereo profile name.
//! \param [out]  pcbSizeOut           Required buffer size.
//!                     # ==0 - there is no default stereo profile name
//!                     currently set # !=0 - size of buffer required for
//!                     currently set default stereo profile name including
//!                     trailing '0'.
//!
//!
//! \return      This API can return any of the error codes enumerated in
...
//! \retval      NVAPI_SUCCESS                                - Default stereo
//! profile name has been copied into szProfileName.
//! \retval      NVAPI_DEFAULT_STEREO_PROFILE_IS_NOT_DEFINED  - There is no
//! default stereo profile set at this time.
//! \retval      NVAPI_INVALID_ARGUMENT                       - pcbSizeOut == 0
//! or cbSizeIn >= *pcbSizeOut && szProfileName == 0
//! \retval      NVAPI_INSUFFICIENT_BUFFER                    - cbSizeIn <
//! *pcbSizeOut
//!
//! \ingroup stereoapi
///////////////////////////////////////////////////////////////////////////////
