...
/* Called once, when first SOS marker is reached */
{
    ...
    if ( ( long )cinfo->image_height > ( long )JPEG_MAX_DIMENSION ||
        ...
        ERREXIT1( cinfo, JERR_IMAGE_TOO_BIG,
    ...
    for ( ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
         ...
         * jdmaster.c if we are doing full decompression.  The transcoder
         * library doesn't use these values, but the calling application might.
         */
        /* Size in samples */
    ...
    if ( cinfo->comps_in_scan == 1 ) {
        ...
        for ( ci = 0; ci < cinfo->comps_in_scan; ci++ ) {
            ...
            compptr->MCU_sample_width =
                compptr->MCU_width * compptr->DCT_scaled_size;
...
consume_markers( j_decompress_ptr cinfo ) {
    ...
    switch ( val ) {
        ...
        case JPEG_REACHED_EOI: /* Found EOI */
            ...
            if ( inputctl
                     ->inheaders ) { /* Tables-only datastream, apparently */
