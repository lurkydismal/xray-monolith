        ...
        for ( blk_y = 0; blk_y < compptr->height_in_blocks;
            ...
            for ( offset_y = 0; offset_y < compptr->v_samp_factor;
                ...
                for ( blk_x = 0; blk_x * 2 < comp_width; blk_x++ ) {
                    ...
                    /* this unrolled loop doesn't need to know which row it's
                     * on... */
        ...
        for ( dst_blk_y = 0; dst_blk_y < compptr->height_in_blocks;
            ...
            for ( offset_y = 0; offset_y < compptr->v_samp_factor;
                ...
                for ( dst_blk_x = 0; dst_blk_x < compptr->width_in_blocks;
                    ...
                    src_buffer = ( *srcinfo->mem->access_virt_barray )(
                        ( j_common_ptr )srcinfo, src_coef_arrays[ ci ],
                        dst_blk_x, ( JDIMENSION )compptr->h_samp_factor,
                        FALSE );
                    for ( offset_x = 0; offset_x < compptr->h_samp_factor;
                          offset_x++ ) {
                        src_ptr =
                            src_buffer[ offset_x ][ dst_blk_y + offset_y ];
                        dst_ptr =
                            dst_buffer[ offset_y ][ dst_blk_x + offset_x ];
                        for ( i = 0; i < DCTSIZE; i++ )
                            for ( j = 0; j < DCTSIZE; j++ )
                                dst_ptr[ j * DCTSIZE + i ] =
                                    src_ptr[ i * DCTSIZE + j ];
                ...
                for ( dst_blk_x = 0; dst_blk_x < compptr->width_in_blocks;
                    ...
                    for ( offset_x = 0; offset_x < compptr->h_samp_factor;
                        ...
                        if ( dst_blk_x < comp_width ) {
                            ...
                            dst_ptr =
                                dst_buffer[ offset_y ][ comp_width - dst_blk_x -
                                                        offset_x - 1 ];
                            for ( i = 0; i < DCTSIZE; i++ ) {
                                for ( j = 0; j < DCTSIZE; j++ )
                                    dst_ptr[ j * DCTSIZE + i ] =
            ...
            for ( offset_y = 0; offset_y < compptr->v_samp_factor;
                ...
                for ( dst_blk_x = 0; dst_blk_x < compptr->width_in_blocks;
                    ...
                    for ( offset_x = 0; offset_x < compptr->h_samp_factor;
                        ...
                        if ( dst_blk_y < comp_height ) {
                            /* Block is within the mirrorable area. */
                            src_ptr = src_buffer[ offset_x ][ comp_height -
                                                              dst_blk_y -
                                                              offset_y - 1 ];
                            for ( i = 0; i < DCTSIZE; i++ ) {
                                for ( j = 0; j < DCTSIZE; j++ ) {
        ...
        for ( dst_blk_y = 0; dst_blk_y < compptr->height_in_blocks;
            ...
            for ( offset_y = 0; offset_y < compptr->v_samp_factor;
                ...
                if ( dst_blk_y < comp_height ) {
                     ...
                     * vertically. */
                    for ( ; dst_blk_x < compptr->width_in_blocks;
                          dst_blk_x++ ) {
                        dst_ptr = dst_row_ptr[ dst_blk_x ];
                        src_ptr = src_row_ptr[ dst_blk_x ];
                        for ( i = 0; i < DCTSIZE; i += 2 ) {
...
jtransform_request_workspace( j_decompress_ptr srcinfo,
    ...
    if ( info->force_grayscale && srcinfo->jpeg_color_space == JCS_YCbCr &&
    ...
    switch ( info->transform ) {
        ...
        case JXFORM_ROT_180:
             ...
             * Note that we allocate arrays padded out to the next iMCU
             * boundary, so that transform routines need not worry about missing
             * edge blocks.
             */
...
jtransform_adjust_parameters( j_decompress_ptr srcinfo,
    ...
    if ( info->force_grayscale ) {
         ...
         * properly.  Among other things, the target h_samp_factor &
         * v_samp_factor will get set to 1, which typically won't match the
         * source. In fact we do this even if the source is already grayscale;
         * that provides an easy way of coercing a grayscale JPEG with funny
         * sampling factors to the customary 1,1.  (Some decoders fail on other
         * factors.)
         */
...
jcopy_markers_execute( j_decompress_ptr srcinfo,
     ...
     */
    for ( marker = srcinfo->marker_list; marker != NULL;
          marker = marker->next ) {
