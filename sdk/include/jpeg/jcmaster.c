...
#ifdef C_PROGRESSIVE_SUPPORTED
            /* The JPEG spec simply gives the ranges 0..13 for Ah and Al, but
             * that seems wrong: the upper bound ought to depend on data
...
#define MAX_AH_AL 13
            ...
            for ( ci = 0; ci < ncomps; ci++ ) {
                last_bitpos_ptr =
                    &last_bitpos[ scanptr->component_index[ ci ] ][ 0 ];
                if ( Ss != 0 &&
