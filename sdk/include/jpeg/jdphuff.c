...
start_pass_phuff_decoder( j_decompress_ptr cinfo ) {
    ...
    for ( ci = 0; ci < cinfo->comps_in_scan; ci++ ) {
        ...
        if ( !is_DC_band &&
             coef_bit_ptr[ 0 ] < 0 ) /* AC without prior DC scan */
            WARNMS2( cinfo, JWRN_BOGUS_PROGRESSION, cindex, 0 );
        for ( coefi = cinfo->Ss; coefi <= cinfo->Se; coefi++ ) {
            int expected =
                ( coef_bit_ptr[ coefi ] < 0 ) ? 0 : coef_bit_ptr[ coefi ];
...
decode_mcu_AC_refine( j_decompress_ptr cinfo, JBLOCKROW* MCU_data ) {
    ...
    if ( !entropy->pub.insufficient_data ) {
         ...
         * newly nonzero coefficients in the block, because otherwise we'd get
         * confused next time about which coefficients were already nonzero. But
        ...
        if ( EOBRUN == 0 ) {
            for ( ; k <= Se; k++ ) {
                 ...
                 * is 1 if the absolute value of the coefficient must be
                 * increased.
                 */
                do {
        ...
        if ( EOBRUN > 0 ) {
             ...
             * correction bit to each already-nonzero coefficient.  A correction
             * bit is 1 if the absolute value of the coefficient must be
             * increased.
             */
            for ( ; k <= Se; k++ ) {
                thiscoef = *block + jpeg_natural_order[ k ];
