...
/* Allocate a "small" object */
{
    ...
    if ( hdr_ptr == NULL ) {
        ...
        for ( ;; ) {
            hdr_ptr =
                ( small_pool_ptr )jpeg_get_small( cinfo, min_request + slop );
            if ( hdr_ptr != NULL )
                break;
            slop /= 2;
...
/* Allocate a "large" object */
{
    ...
    /* Check for unsatisfiable request (do now to ensure no overflow below) */
    if ( sizeofobject >
         ( size_t )( MAX_ALLOC_CHUNK - SIZEOF( large_pool_hdr ) ) )
...
/* caller intends to modify the accessed area. */
{
    ...
    if ( start_row < ptr->cur_start_row ||
         ...
         * have start_row = 0, so the limiting case applies and we load from 0
         * anyway.
         */
        if ( start_row > ptr->cur_start_row ) {
    ...
    if ( ptr->first_undef_row < end_row ) {
        ...
        if ( ptr->pre_zero ) {
            size_t bytesperrow =
                ( size_t )ptr->samplesperrow * SIZEOF( JSAMPLE );
            undef_row -=
                ptr->cur_start_row; /* make indexes relative to buffer */
            end_row -= ptr->cur_start_row;
            while ( undef_row < end_row ) {
                jzero_far( ( void FAR* )ptr->mem_buffer[ undef_row ],
                           bytesperrow );
                undef_row++;
            }
        } else {
...
free_pool( j_common_ptr cinfo, int pool_id ) {
    ...
    if ( cinfo->err->trace_level > 1 )
        print_mem_stats( cinfo,
                         pool_id ); /* print pool's memory usage statistics */
#endif
    ...
    if ( pool_id == JPOOL_IMAGE ) {
        ...
        for ( sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next ) {
            if ( sptr->b_s_open ) {     /* there may be no backing store */
                ...
                ( *sptr->b_s_info.close_backing_store )( cinfo,
                                                         &sptr->b_s_info );
            }
        }
        ...
        for ( bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next ) {
            if ( bptr->b_s_open ) {     /* there may be no backing store */
                ...
                ( *bptr->b_s_info.close_backing_store )( cinfo,
                                                         &bptr->b_s_info );
            }
        }
