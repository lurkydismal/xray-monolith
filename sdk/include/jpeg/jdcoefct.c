...
decompress_onepass( j_decompress_ptr cinfo, JSAMPIMAGE output_buf ) {
    ...
    for ( yoffset = coef->MCU_vert_offset;
        ...
        for ( MCU_col_num = coef->MCU_ctr; MCU_col_num <= last_MCU_col;
             ...
             * thing. We skip dummy blocks at the right and bottom edges (but
             * blkn gets incremented past them!).  Note the inner loop relies on
             * having allocated the MCU_buffer[] blocks sequentially.
             */
...
decompress_smooth_data( j_decompress_ptr cinfo, JSAMPIMAGE output_buf ) {
    ...
    for ( ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
        ...
        for ( block_row = 0; block_row < block_rows; block_row++ ) {
             ...
             * approach. Initialize all nine here so as to do the right thing on
             * narrow pics.
             */
            ...
            for ( block_num = 0; block_num <= last_block_column; block_num++ ) {
                 ...
                 */
                jcopy_block_row( buffer_ptr, ( JBLOCKROW )workspace,
                                 ( JDIMENSION )1 );
                /* Update DC values */
