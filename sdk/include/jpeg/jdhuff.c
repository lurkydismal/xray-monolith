...
jpeg_make_d_derived_tbl( j_decompress_ptr cinfo,
    ...
    for ( l = 1; l <= 16; l++ ) {
        if ( htbl->bits[ l ] ) {
            ...
            p += htbl->bits[ l ];
            dtbl->maxcode[ l ] =
                huffcode[ p - 1 ]; /* maximum code of length l */
        } else {
    ...
    if ( cinfo->unread_marker == 0 ) { /* cannot advance past a marker */
        while ( bits_left < MIN_GET_BITS ) {
            ...
            if ( c == 0xFF ) {
                 ...
                 * will accept multiple FF's followed by a 0 as meaning a single
                 * FF data byte.  This data pattern is not valid according to
                 * the standard.
                 */
                do {
                    if ( bytes_in_buffer == 0 ) {
                ...
                if ( c == 0 ) {
                     ...
                     * data. Save the marker code for later use. Fine point: it
                     * might appear that we should save the marker into bitread
                     * working state, not straight into permanent state.  But
                     * once we have hit a marker, we cannot need to suspend
                     * within the current MCU, because we will read no more
                     * bytes from the data source.  So it is OK to update
                     * permanent state right away.
                     */
            ...
            if ( entropy->ac_needed[ blkn ] ) {
                ...
                for ( k = 1; k < DCTSIZE2; k++ ) {
                    ...
                    if ( s ) {
                         ...
                         * save us if k >= DCTSIZE2, which could happen if the
                         * data is corrupted.
                         */
