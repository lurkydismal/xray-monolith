...
typedef struct {
     ...
     * i, premultiplied as described above.  Since colormap indexes must fit
     * into JSAMPLEs, the entries of this array will too.
     */
...
/* Must have largest(j=0) >= 0, and largest(j=maxj) >= MAXJSAMPLE */
{
    ...
    return ( int )( ( ( INT32 )( 2 * j + 1 ) * MAXJSAMPLE + maxj ) /
                    ( 2 * maxj ) );
}
...
make_odither_array( j_decompress_ptr cinfo, int ncolors ) {
    ...
    for ( j = 0; j < ODITHER_SIZE; j++ ) {
        for ( k = 0; k < ODITHER_SIZE; k++ ) {
            num = ( ( INT32 )( ODITHER_CELLS - 1 -
                               2 * ( ( int )base_dither_matrix[ j ][ k ] ) ) ) *
                  MAXJSAMPLE;
...
/* General case, no dithering */
{
    ...
    for ( row = 0; row < num_rows; row++ ) {
        ...
        for ( col = width; col > 0; col-- ) {
            ...
            for ( ci = 0; ci < nc; ci++ ) {
                pixcode +=
                    GETJSAMPLE( colorindex[ ci ][ GETJSAMPLE( *ptrin++ ) ] );
            }
    ...
    for ( row = 0; row < num_rows; row++ ) {
        ...
        for ( ci = 0; ci < nc; ci++ ) {
            ...
            for ( col = width; col > 0; col-- ) {
                 ...
                 * pixel. Range-limiting need not be done explicitly, as we have
                 * extended the colorindex table to produce the right answers
                 * for out-of-range inputs.  The maximum dither is +-
                 * MAXJSAMPLE; this sets the required amount of padding.
                 */
                *output_ptr += colorindex_ci[ GETJSAMPLE( *input_ptr ) +
                                              dither[ col_index ] ];
                input_ptr += nc;
                output_ptr++;
    ...
    for ( row = 0; row < num_rows; row++ ) {
        ...
        for ( ci = 0; ci < nc; ci++ ) {
            ...
            if ( cquantize->on_odd_row ) {
                ...
                input_ptr +=
                    ( width - 1 ) * nc; /* so point to rightmost pixel */
                output_ptr += width - 1;
                dir = -1;
                dirnc = -nc;
                ...
                errorptr =
                    cquantize
                        ->fserrors[ ci ]; /* => entry before first column */
            }
            colorindex_ci = cquantize->colorindex[ ci ];
            ...
            for ( col = width; col > 0; col-- ) {
                 ...
                 * adding 8 is correct for either sign of the error value. Note:
                 * errorptr points to *previous* column's array entry.
                 */
                cur = RIGHT_SHIFT( cur + errorptr[ dir ] + 8, 4 );
                /* Form pixel value + error, and range-limit to 0..MAXJSAMPLE.
                 * The maximum error is +- MAXJSAMPLE; this sets the required
                 * size of the range_limit array.
                 */
                ...
                *output_ptr += ( JSAMPLE )pixcode;
                 ...
                 * propagated to the next pixel on the current line, and all the
                 * errors for the next line have been shifted over. We are
             ...
             */
            errorptr[ 0 ] =
                ( FSERROR )bpreverr; /* unload prev err into array */
        }
...
start_pass_1_quant( j_decompress_ptr cinfo, boolean is_pre_scan ) {
    ...
    switch ( cinfo->dither_mode ) {
        ...
        case JDITHER_FS:
            ...
            arraysize =
                ( size_t )( ( cinfo->output_width + 2 ) * SIZEOF( FSERROR ) );
            for ( i = 0; i < cinfo->out_color_components; i++ )
