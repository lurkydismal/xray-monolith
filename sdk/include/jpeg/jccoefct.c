...
typedef struct {
     ...
     * unchanged when a large coefficient buffer is necessary.) In multi-pass
     * modes, this array points to the current MCU's blocks within the virtual
     * arrays.
     */
...
compress_data( j_compress_ptr cinfo, JSAMPIMAGE input_buf ) {
    ...
    for ( yoffset = coef->MCU_vert_offset;
          yoffset < coef->MCU_rows_per_iMCU_row; yoffset++ ) {
        for ( MCU_col_num = coef->mcu_ctr; MCU_col_num <= last_MCU_col;
              MCU_col_num++ ) {
            /* Determine where data comes from in input_buf and do the DCT
             * thing. Each call on forward_DCT processes a horizontal row of DCT
             * blocks as wide as an MCU; we rely on having allocated the
             * MCU_buffer[] blocks sequentially.  Dummy blocks at the right or
             * bottom edge are filled in specially.  The data in them does not
             * matter for image reconstruction, so we fill them with values that
             * will encode to the smallest amount of data, viz: all zeroes in
             * the AC entries, DC entries equal to previous block's DC value.
             * (Thanks to Thomas Kinsman for this idea.)
             */
            blkn = 0;
                ...
                for ( yindex = 0; yindex < compptr->MCU_height; yindex++ ) {
                    if ( coef->iMCU_row_num < last_iMCU_row ||
                        ...
                        if ( blockcnt < compptr->MCU_width ) {
                             ...
                             * image. */
                            jzero_far( ( void FAR* )
                                           coef->MCU_buffer[ blkn + blockcnt ],
                                       ( compptr->MCU_width - blockcnt ) *
                                           SIZEOF( JBLOCK ) );
                            for ( bi = blockcnt; bi < compptr->MCU_width;
                                  bi++ ) {
