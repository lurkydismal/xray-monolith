...
typedef struct {
     ...
     * full-size, ie do not need rescaling.  The corresponding entry of
     * color_buf[] is simply set to point to the input data array, thereby
     * avoiding copying.
     */
...
sep_upsample( j_decompress_ptr cinfo,
    ...
    num_rows =
        ( JDIMENSION )( cinfo->max_v_samp_factor - upsample->next_row_out );
...
h2v1_fancy_upsample( j_decompress_ptr cinfo,
    ...
    for ( inrow = 0; inrow < cinfo->max_v_samp_factor; inrow++ ) {
        ...
        *outptr++ =
            ( JSAMPLE )( ( invalue * 3 + GETJSAMPLE( *inptr ) + 2 ) >> 2 );

        for ( colctr = compptr->downsampled_width - 2; colctr > 0; colctr-- ) {
            /* General case: 3/4 * nearer pixel + 1/4 * further pixel */
            invalue = GETJSAMPLE( *inptr++ ) * 3;
            *outptr++ =
                ( JSAMPLE )( ( invalue + GETJSAMPLE( inptr[ -2 ] ) + 1 ) >> 2 );
            *outptr++ =
                ( JSAMPLE )( ( invalue + GETJSAMPLE( *inptr ) + 2 ) >> 2 );
        }

        /* Special case for last column */
        invalue = GETJSAMPLE( *inptr );
        *outptr++ =
            ( JSAMPLE )( ( invalue * 3 + GETJSAMPLE( inptr[ -1 ] ) + 1 ) >> 2 );
        *outptr++ = ( JSAMPLE )invalue;
    }
}
        ...
        for ( v = 0; v < 2; v++ ) {
            ...
            *outptr++ = ( JSAMPLE )( ( thiscolsum * 3 + nextcolsum + 7 ) >> 4 );
            ...
            for ( colctr = compptr->downsampled_width - 2; colctr > 0;
                ...
                nextcolsum =
                    GETJSAMPLE( *inptr0++ ) * 3 + GETJSAMPLE( *inptr1++ );
                *outptr++ =
...
jinit_upsampler( j_decompress_ptr cinfo ) {
    ...
    for ( ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
         ...
         * samples are to be converted to max_h_samp_factor * max_v_samp_factor
         * pixels.
         */
