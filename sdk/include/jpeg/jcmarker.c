...
/* one DAC marker.  Therefore this routine does its own scan of the table. */
{
    ...
    for ( i = 0; i < NUM_ARITH_TBLS; i++ ) {
        if ( dc_in_use[ i ] ) {
            emit_byte( cinfo, i );
            emit_byte( cinfo, cinfo->arith_dc_L[ i ] +
                                  ( cinfo->arith_dc_U[ i ] << 4 ) );
        }
        if ( ac_in_use[ i ] ) {
...
/* Emit a SOS marker */
{
    ...
    for ( i = 0; i < cinfo->comps_in_scan; i++ ) {
        ...
        if ( cinfo->progressive_mode ) {
             ...
             * all. We emit 0 for unused field(s); this is recommended by the
             * P&M text but does not seem to be specified in the standard.
             */
            if ( cinfo->Ss == 0 ) {
                ta = 0; /* DC scan */
...
write_frame_header( j_compress_ptr cinfo ) {
    ...
    if ( cinfo->arith_code ) {
        ...
        if ( cinfo->progressive_mode )
            ...
            emit_sof( cinfo,
                      M_SOF0 ); /* SOF code for baseline implementation */
        else
            emit_sof( cinfo,
                      M_SOF1 ); /* SOF code for non-baseline Huffman file */
    }
}
