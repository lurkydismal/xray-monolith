...
/* complain about bad command line */
{
    ...
    fprintf( stderr,
             "  -quality N     Compression quality (0..100; 5-95 is useful "
             "range)\n" );
...
#endif
    fprintf( stderr,
             "  -restart N     Set restart interval in rows, or in blocks with "
             "B\n" );
 ...
 */
{
    ...
    for ( argn = 1; argn < argc; argn++ ) {
        ...
        if ( *arg != '-' ) {
            ...
            if ( argn <= last_file_arg_seen ) {
                outfilename =
                    NULL; /* -outfile applies to just one input file */
        ...
        if ( keymatch( arg, "arithmetic", 1 ) ) {
        ...
        } else if ( keymatch( arg, "debug", 1 ) ||
                    keymatch( arg, "verbose", 1 ) ) {
            ...
            /* We postpone actually reading the file in case -quality comes
             * later. */
            ...
            if ( ++argn >= argc ) /* advance to next argument */
                usage();
            samplearg = argv[ argn ];
            /* Must delay setting sample factors until after we have processed
             * any colorspace-determining switches, since jpeg_set_colorspace
             * sets default sampling factors.
             */
    ...
    if ( for_real ) {
        ...
        if ( qtablefile != NULL ) /* process -qtables if it was present */
            if ( !read_quant_tables( cinfo, qtablefile, q_scale_factor,
                                     force_baseline ) )
                usage();
...
int main( int argc, char** argv ) {
    ...
    /* Open the input file. */
    if ( file_index < argc ) {
        if ( ( input_file = fopen( argv[ file_index ], READ_BINARY ) ) ==
             NULL ) {
