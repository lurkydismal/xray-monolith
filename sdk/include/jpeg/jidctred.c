...
#define DEQUANTIZE( coef, quantval ) \
    ...
    for ( ctr = DCTSIZE; ctr > 0; inptr++, quantptr++, wsptr++, ctr-- ) {
        ...
        tmp2 =
            MULTIPLY( z2, FIX_1_847759065 ) + MULTIPLY( z3, -FIX_0_765366865 );
    ...
    for ( ctr = 0; ctr < 4; ctr++ ) {
        ...
        outptr[ 0 ] =
            range_limit[ ( int )DESCALE( tmp10 + tmp2,
        ...
        outptr[ 3 ] =
            range_limit[ ( int )DESCALE( tmp10 - tmp2,
                         ...
                         RANGE_MASK ];
        outptr[ 1 ] =
            range_limit[ ( int )DESCALE( tmp12 + tmp0,
                                         CONST_BITS + PASS1_BITS + 3 + 1 ) &
                         RANGE_MASK ];
        outptr[ 2 ] =
            range_limit[ ( int )DESCALE( tmp12 - tmp0,
...
jpeg_idct_2x2( j_decompress_ptr cinfo,
    ...
    for ( ctr = 0; ctr < 2; ctr++ ) {
...
#ifndef NO_ZERO_ROW_TEST
        if ( wsptr[ 1 ] == 0 && wsptr[ 3 ] == 0 && wsptr[ 5 ] == 0 &&
             wsptr[ 7 ] == 0 ) {
            /* AC terms all zero */
            JSAMPLE dcval = range_limit[ ( int )DESCALE( ( INT32 )wsptr[ 0 ],
                                                         PASS1_BITS + 3 ) &
        ...
        outptr[ 0 ] =
            range_limit[ ( int )DESCALE( tmp10 + tmp0,
