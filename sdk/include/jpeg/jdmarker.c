...
/* Process a DHT marker */
{
    ...
    while ( length > 16 ) {
        ...
        TRACEMS8( cinfo, 2, JTRC_HUFFBITS, bits[ 1 ], bits[ 2 ], bits[ 3 ],
                  bits[ 4 ], bits[ 5 ], bits[ 6 ], bits[ 7 ], bits[ 8 ] );
        TRACEMS8( cinfo, 2, JTRC_HUFFBITS, bits[ 9 ], bits[ 10 ], bits[ 11 ],
                  bits[ 12 ], bits[ 13 ], bits[ 14 ], bits[ 15 ], bits[ 16 ] );
        ...
        MEMCOPY( ( *htblptr )->huffval, huffval,
 ...
 */
{
    ...
    if ( datalen >= APP0_DATA_LEN && GETJOCTET( data[ 0 ] ) == 0x4A &&
        ...
        cinfo->X_density =
            ( GETJOCTET( data[ 8 ] ) << 8 ) + GETJOCTET( data[ 9 ] );
        cinfo->Y_density =
            ( GETJOCTET( data[ 10 ] ) << 8 ) + GETJOCTET( data[ 11 ] );
        /* Check version.
         * Major version must be 1, anything else signals an incompatible
         * change. (We used to treat this as an error, but now it's a nonfatal
         * warning, because some bozo at Hijaak couldn't read the spec.) Minor
         * version should be 0..2, but process anyway if newer.
         */
...
next_marker( j_decompress_ptr cinfo ) {
    ...
    for ( ;; ) {
         ...
         * file. We sync after each discarded byte so that a suspending data
         * source can discard the byte from its buffer.
         */
        while ( c != 0xFF ) {
    ...
    if ( cinfo->marker->discarded_bytes != 0 ) {
        WARNMS2( cinfo, JWRN_EXTRANEOUS_DATA, cinfo->marker->discarded_bytes,
                 c );
        ...
        switch ( cinfo->unread_marker ) {
            ...
            case M_APP15:
                if ( !( *( ( my_marker_ptr )cinfo->marker )
                             ->process_APPn[ cinfo->unread_marker -
                                             ( int )M_APP0 ] )( cinfo ) )
                    return JPEG_SUSPENDED;
                break;

            case M_COM:
                if ( !( *( ( my_marker_ptr )cinfo->marker )->process_COM )(
                         cinfo ) )
                    return JPEG_SUSPENDED;
                break;
            ...
            default: /* must be DHP, EXP, JPGn, or RESn */
                 ...
                 * they are likely to be used to signal incompatible JPEG Part 3
                 * extensions. Once the JPEG 3 version-number marker is well
                 * defined, this code ought to change!
                 */
                ERREXIT1( cinfo, JERR_UNKNOWN_MARKER, cinfo->unread_marker );
                break;
        }
...
jpeg_save_markers( j_decompress_ptr cinfo,
     ...
     */
    maxlength =
        cinfo->mem->max_alloc_chunk - SIZEOF( struct jpeg_marker_struct );
    ...
    if ( length_limit ) {
        ...
        else if ( marker_code == ( int )M_APP14 &&
    ...
    } else if ( marker_code >= ( int )M_APP0 &&
                marker_code <= ( int )M_APP15 ) {
