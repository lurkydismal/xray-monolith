...
static void idct8_1( ogg_int16_t* _y, const ogg_int16_t _x[ 1 ] ) {
    _y[ 0 << 3 ] = _y[ 1 << 3 ] = _y[ 2 << 3 ] = _y[ 3 << 3 ] = _y[ 4 << 3 ] =
        _y[ 5 << 3 ] = _y[ 6 << 3 ] = _y[ 7 << 3 ] =
            ( ogg_int16_t )( OC_C4S4 * _x[ 0 ] >> 16 );
}
...
static void oc_idct8x8_3( ogg_int16_t _y[ 64 ], const ogg_int16_t _x[ 64 ] ) {
    ...
    for ( in = w, out = _y, end = out + 8; out < end; in += 8, out++ )
        idct8_2( out, in );
    /*Adjust for the scale factor.*/
    for ( out = _y, end = out + 64; out < end; out++ )
        *out = ( ogg_int16_t )( *out + 8 >> 4 );
}
...
static void oc_idct8x8_slow( ogg_int16_t _y[ 64 ],
    ...
    ogg_int16_t w[ 64 ];
    /*Transform rows of x into columns of w.*/
    for ( in = _x, out = w, end = out + 8; out < end; in += 8, out++ )
        idct8( out, in );
    /*Transform rows of w into columns of y.*/
    for ( in = w, out = _y, end = out + 8; out < end; in += 8, out++ )
        idct8( out, in );
...
void oc_idct8x8( const oc_theora_state* _state,
                 ogg_int16_t _y[ 64 ],
                 int _last_zzi ) {
