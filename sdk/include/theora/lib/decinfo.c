...
static int oc_info_unpack( oc_pack_buf* _opb, th_info* _info ) {
    ...
    if ( _info->pixel_fmt == TH_PF_RSVD )
        return TH_EBADHEADER;
    val = oc_pack_read( _opb, 3 );
    if ( val != 0 || oc_pack_bytes_left( _opb ) < 0 )
        return TH_EBADHEADER;
    return 0;
}
...
static int oc_dec_headerin( oc_pack_buf* _opb,
                            th_info* _info,
                            th_comment* _tc,
    ...
    if ( !( packtype & 0x80 ) && _info->frame_width > 0 &&
         _tc->vendor != NULL && *_setup != NULL ) {
        return 0;
    }
    /*Check the codec string.*/
    oc_unpack_octets( _opb, buffer, 6 );
    if ( memcmp( buffer, "theora", 6 ) != 0 )
    ...
    switch ( packtype ) {
        ...
        case 0x80: {
            /*This should be the first packet, and we should not already be
               initialized.*/
            if ( !_op->b_o_s || _info->frame_width > 0 )
                return TH_EBADHEADER;
        ...
        case 0x81: {
            ...
            /*We shoud have already decoded the info header, and should not yet
               have decoded the comment header.*/
            if ( _info->frame_width == 0 || _tc->vendor != NULL )
                return TH_EBADHEADER;
        ...
        case 0x82: {
            oc_setup_info* setup;
            if ( _tc == NULL || _setup == NULL )
                return TH_EFAULT;
            /*We should have already decoded the info header and the comment
               header, and should not yet have decoded the setup header.*/
            if ( _info->frame_width == 0 || _tc->vendor == NULL ||
                 *_setup != NULL ) {
                return TH_EBADHEADER;
            }
