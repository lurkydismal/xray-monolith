...
unsigned oc_enc_frag_sad_mmxext( const unsigned char* _src,
    ...
    __asm__ __volatile__(
        ...
        "paddw %%mm2,%%mm0\n\t"
        "movd %%mm0,%[ret]\n\t"
        : [ret] "=a"( ret ), [src] "+%r"( _src ), [ref] "+r"( _ref ),
          [ystride3] "=&r"( ystride3 )
...
#define OC_SAD2_LOOP                                                           \
    "#OC_SAD2_LOOP\n\t" /*We want to compute (%%mm0+%%mm1>>1) on unsigned      \
                          bytes without overflow, but pavgb computes           \
                          (%%mm0+%%mm1+1>>1). The latter is exactly 1 too      \
                          large when the low bit of two corresponding bytes is \
                          only set in one of them. Therefore we pxor the       \
                          operands, pand to mask out the low bits, and psubb   \
                          to correct the output of pavgb.*/                    \
    "movq %%mm0,%%mm6\n\t"                                                     \
...
unsigned oc_enc_frag_sad2_thresh_mmxext( const unsigned char* _src,
    ...
    __asm__ __volatile__(
        ...
        "pcmpeqb %%mm6,%%mm6\n\t"
        "movq (%[src],%[ystride]),%%mm5\n\t"
        "psubb %%mm6,%%mm7\n\t" OC_SAD2_LOOP OC_SAD2_LOOP OC_SAD2_LOOP
            OC_SAD2_TAIL
        : [ret] "=&a"( ret ), [src] "+r"( _src ), [ref1] "+%r"( _ref1 ),
          [ref2] "+r"( _ref2 )
        : [ystride] "r"( ( ptrdiff_t )_ystride ) );
    return ( unsigned )ret;
}
...
#define OC_HADAMARD_8x4 OC_HADAMARD_AB_8x4 OC_HADAMARD_C_8x4
...
#define OC_HADAMARD_C_ABS_ACCUM_B_8x4( _r6, _r7 )                           \
    ...
    "paddsw %%mm7,%%mm1\n\t"                                                \
    "psubw %%mm6,%%mm2\n\t"                                                 \
    "psubw %%mm1,%%mm4\n\t" /*mm7={1}x4 (needed for the horizontal add that \
                              follows)                                      \
...
#define OC_HADAMARD_ABS_ACCUM_8x4( _r6, _r7 ) \
    OC_HADAMARD_AB_8x4 OC_HADAMARD_C_ABS_ACCUM_8x4( _r6, _r7 )
...
#define OC_TRANSPOSE_4x4x2( _off )                                       \
    ...
    "punpckldq %%mm5,%%mm2\n\t" /*mm0 = h0 g0 f0 e0                      \
                                  ...
                                  mm3 = h3 g3 f3 e3*/                    \
    "movq 0x10+" _off                                                    \
    "(%[buf]),%%mm5\n\t" /*Second 4x4 transpose:*/ /*mm4 = a3 a2 a1 a0   \
                                                     mm5 = b3 b2 b1 b0   \
                                                     mm6 = c3 c2 c1 c0   \
                                                     mm7 = d3 d2 d1 d0*/ \
    "movq %%mm6,%%mm0\n\t"                                               \
...
static void oc_int_frag_copy2_mmxext( unsigned char* _dst,
    ...
    __asm__ __volatile__(
        ...
        : [dst_ystride] "r"( ( ptrdiff_t )_dst_ystride ), [src_ystride] "r"(
                                                              ( ptrdiff_t )
                                                                  _src_ystride )
        : "memory" );
}
