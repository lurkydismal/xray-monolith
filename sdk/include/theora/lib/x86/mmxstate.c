...
void oc_state_frag_recon_mmx( const oc_theora_state* _state,
                              ptrdiff_t _fragi,
                              int _pli,
    ...
    if ( mb_mode == OC_MODE_INTRA )
        oc_frag_recon_intra_mmx( dst, ystride, _dct_coeffs );
    else {
        ...
        if ( oc_state_get_mv_offsets( _state, mvoffsets, _pli,
            ...
            oc_frag_recon_inter2_mmx( dst, ref + mvoffsets[ 0 ],
                                      ref + mvoffsets[ 1 ], ystride,
                                      _dct_coeffs );
        } else
            oc_frag_recon_inter_mmx( dst, ref + mvoffsets[ 0 ], ystride,
                                     _dct_coeffs );
    }
}
    ...
    while ( fragi0 < fragi0_end ) {
        ...
        while ( fragi < fragi_end ) {
            if ( frags[ fragi ].coded ) {
                ...
                if ( fragi > fragi0 )
                    OC_LOOP_FILTER_H_MMX( ref, ystride, ll );
                if ( fragi0 > fragi_top )
                    OC_LOOP_FILTER_V_MMX( ref, ystride, ll );
