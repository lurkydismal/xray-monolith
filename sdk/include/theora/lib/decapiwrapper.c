...
static void th_dec_api_clear( th_api_wrapper* _api ) {
    if ( _api->setup )
        th_setup_free( _api->setup );
    if ( _api->decode )
        th_decode_free( _api->decode );
...
static int theora_decode_control( theora_state* _td,
                                  int _req,
                                  void* _buf,
                                  size_t _buf_sz ) {
...
static ogg_int64_t theora_decode_granule_frame( theora_state* _td,
                                                ogg_int64_t _gp ) {
    return th_granule_frame( ( ( th_api_wrapper* )_td->i->codec_setup )->decode,
                             _gp );
}

static double theora_decode_granule_time( theora_state* _td, ogg_int64_t _gp ) {
    return th_granule_time( ( ( th_api_wrapper* )_td->i->codec_setup )->decode,
                            _gp );
}
...
static void th_info2theora_info( theora_info* _ci, const th_info* _info ) {
    ...
    switch ( _info->colorspace ) {
        case TH_CS_ITU_REC_470M:
            _ci->colorspace = OC_CS_ITU_REC_470M;
            break;
        case TH_CS_ITU_REC_470BG:
            _ci->colorspace = OC_CS_ITU_REC_470BG;
            break;
        default:
            _ci->colorspace = OC_CS_UNSPECIFIED;
            break;
    }
    switch ( _info->pixel_fmt ) {
        case TH_PF_420:
            ...
            break;
        case TH_PF_422:
            _ci->pixelformat = OC_PF_422;
            break;
        case TH_PF_444:
            _ci->pixelformat = OC_PF_444;
            break;
        default:
            _ci->pixelformat = OC_PF_RSVD;
    }
...
int theora_decode_header( theora_info* _ci,
                          theora_comment* _cc,
                          ogg_packet* _op ) {
    th_api_wrapper* api;
    th_info info;
    ...
    /*Allocate an API wrapper struct on demand, since it will not also include a
       theora_info struct like the ones that are used in a theora_state
       struct.*/
    if ( api == NULL ) {
        _ci->codec_setup = _ogg_calloc( 1, sizeof( *api ) );
        if ( _ci->codec_setup == NULL )
            return OC_FAULT;
        api = ( th_api_wrapper* )_ci->codec_setup;
...
int theora_decode_packetin( theora_state* _td, ogg_packet* _op ) {
    ...
    ogg_int64_t gp;
    int ret;
    if ( !_td || !_td->i || !_td->i->codec_setup )
        return OC_FAULT;
