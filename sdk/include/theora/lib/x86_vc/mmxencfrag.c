...
#define OC_SAD2_LOOP                                                          \
    __asm { /*We want to compute (mm0+mm1>>1) on unsigned bytes without       \
             overflow, but pavgb computes (mm0+mm1+1>>1). The latter is       \
             exactly 1 too large when the low bit of two corresponding bytes  \
             is only set in one of them. Therefore we pxor the operands, pand \
             to mask out the low bits, and psubb to correct the output of     \
             pavgb.*/ \
  __asm  movq mm6,mm0 \
...
#define OC_HADAMARD_C_ABS_ACCUM_A_8x4( _r6, _r7 )                                  \
    __asm {     /*We use the fact that                                             \
                    (abs(a+b)+abs(a-b))/2=max(abs(a),abs(b))                       \
                   to merge the final butterfly with the abs and the first stage   \
                  of     accumulation.     Thus we can avoid using pabsw, which is \
                  not     available until SSSE3.     Emulating pabsw takes 3                            \
                  instructions, so     the straightforward MMXEXT     implementation                                 \
                  would be (3+3)*8+7=55     instructions (+4 for spilling     registers).                                                                   \
                  Even with pabsw, it     would be (3+1)*8+7=39 instructions (with no                                   \
                  spills).     This     implementation is only 26 (+4 for spilling                                    \
                  registers).*/ \
...
#define RET2 edx
    ...
    OC_HADAMARD_ABS_ACCUM_8x4(0x28,0x38)
        /*Up to this point, everything fit in 16 bits (8 input + 1 for the
           difference + 2*3 for the two 8-point 1-D Hadamards - 1 for the abs -
          1 for the factor of two we dropped + 3 for the vertical accumulation).
    ...
    movq mm7,[0x78+BUF]
        /*The sums produced by OC_HADAMARD_ABS_ACCUM_8x4 each have an extra 4
           added to them, and a factor of two removed; correct the final sum
           here.*/
    lea RET,[RET+RET-32]
    movq mm0,[0x40+BUF]
    cmp RET,RET2
    ...
    pmaddwd mm0,mm7
            /*There isn't much to stick in here to hide the latency this time,
               but the alternative to pmaddwd is
               movq->punpcklwd->punpckhwd->paddd, whose latency is even worse.*/
    sub RET,32
    movq mm4,mm0
    punpckhdq mm0,mm0
    paddd mm4,mm0
...
unsigned oc_enc_frag_satd_thresh_mmxext( const unsigned char* _src,
                                         ...
                                         int _ystride,
                                         unsigned _thresh ) {
    return oc_int_frag_satd_thresh_mmxext( _src, _ystride, _ref, _ystride,
                                           _thresh );
}
...
static void oc_int_frag_copy2_mmxext( unsigned char* _dst,
                                      int _dst_ystride,
...
#define YSTRIDE3 ecx
    ...
    OC_HADAMARD_ABS_ACCUM_8x4(0x68,0x78)
    pmaddwd mm0,mm7
            /*We assume that the DC coefficient is always positive (which is
            true, because the input to the INTRA transform was not a
            difference).*/
    movzx RET,RET_WORD
    add RET2,RET2
    sub RET2,RET
...
void oc_enc_frag_sub_mmx( ogg_int16_t _residue[ 64 ],
                          ...
                          int _ystride ) {
    int i;
    __asm pxor mm7, mm7 for ( i = 4; i-- > 0; ) {
        __asm {
#define SRC edx
#define YSTRIDE esi
#define RESIDUE eax
