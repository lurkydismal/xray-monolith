...
const unsigned char OC_FZIG_ZAG[ 128 ] = {
    ...
    60, 61, 54, 47, 55, 62, 63, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64,
...
const unsigned char OC_DCT_TOKEN_EXTRA_BITS[ TH_NDCT_TOKENS ] = {
    0, 0, 0, 2, 3, 4, 12, 3, 6, 0, 0, 0, 0, 1, 1, 1,
    1, 2, 3, 4, 5, 6, 10, 1, 1, 1, 1, 1, 3, 4, 2, 3 };
...
static void oc_set_chroma_mvs00( oc_mv _cbmvs[ 4 ], const oc_mv _lbmvs[ 4 ] ) {
    int dx;
    int dy;
    dx = _lbmvs[ 0 ][ 0 ] + _lbmvs[ 1 ][ 0 ] + _lbmvs[ 2 ][ 0 ] +
         _lbmvs[ 3 ][ 0 ];
    dy = _lbmvs[ 0 ][ 1 ] + _lbmvs[ 1 ][ 1 ] + _lbmvs[ 2 ][ 1 ] +
         _lbmvs[ 3 ][ 1 ];
...
ogg_uint32_t th_version_number( void ) {
    return ( TH_VERSION_MAJOR << 16 ) + ( TH_VERSION_MINOR << 8 ) +
           TH_VERSION_SUB;
}
