...
void oc_frag_recon_intra_c( unsigned char* _dst,
    ...
    for ( i = 0; i < 8; i++ ) {
        int j;
        for ( j = 0; j < 8; j++ )
            _dst[ j ] = OC_CLAMP255( _residue[ i * 8 + j ] + 128 );
        _dst += _ystride;
    }
}
...
void oc_frag_recon_inter_c( unsigned char* _dst,
    ...
    for ( i = 0; i < 8; i++ ) {
        int j;
        for ( j = 0; j < 8; j++ )
            _dst[ j ] = OC_CLAMP255( _residue[ i * 8 + j ] + _src[ j ] );
        _dst += _ystride;
        _src += _ystride;
    }
}
...
void oc_frag_recon_inter2( const oc_theora_state* _state,
                           ...
                           int _ystride,
                           const ogg_int16_t _residue[ 64 ] ) {
    _state->opt_vtable.frag_recon_inter2( _dst, _src1, _src2, _ystride,
                                          _residue );
}
...
void oc_frag_recon_inter2_c( unsigned char* _dst,
    ...
    for ( i = 0; i < 8; i++ ) {
        int j;
        for ( j = 0; j < 8; j++ )
            _dst[ j ] = OC_CLAMP255( _residue[ i * 8 + j ] +
                                     ( _src1[ j ] + _src2[ j ] >> 1 ) );
        _dst += _ystride;
        _src1 += _ystride;
        _src2 += _ystride;
