...
static void oc_mcenc_find_candidates( oc_enc_ctx* _enc,
                                      oc_mcenc_ctx* _mcenc,
                                      int _accum[ 2 ],
                                      int _mbi,
                                      int _frame ) {
    ...
    if ( embs[ _mbi ].ncneighbors > 0 ) {
        ...
        for ( i = 0; i < embs[ _mbi ].ncneighbors; i++ ) {
            ...
            _mcenc->candidates[ ncandidates ][ 0 ] =
                embs[ nmbi ].analysis_mv[ 0 ][ _frame ][ 0 ];
            _mcenc->candidates[ ncandidates ][ 1 ] =
                embs[ nmbi ].analysis_mv[ 0 ][ _frame ][ 1 ];
            ncandidates++;
        }
    }
    ...
    ncandidates++;
    _mcenc->candidates[ ncandidates ][ 0 ] = OC_CLAMPI(
        -31, embs[ _mbi ].analysis_mv[ 1 ][ _frame ][ 0 ] + _accum[ 0 ], 31 );
    _mcenc->candidates[ ncandidates ][ 1 ] = OC_CLAMPI(
        -31, embs[ _mbi ].analysis_mv[ 1 ][ _frame ][ 1 ] + _accum[ 1 ], 31 );
    ncandidates++;
    ...
    for ( i = 0;; i++ ) {
        _mcenc->candidates[ ncandidates ][ 0 ] = OC_CLAMPI(
            ...
            2 * embs[ _mbi ].analysis_mv[ 1 ][ _frame ][ 0 ] -
                embs[ _mbi ].analysis_mv[ 2 ][ _frame ][ 0 ] + _accum[ 0 ],
            31 );
        _mcenc->candidates[ ncandidates ][ 1 ] = OC_CLAMPI(
            -31,
            2 * embs[ _mbi ].analysis_mv[ 1 ][ _frame ][ 1 ] -
                embs[ _mbi ].analysis_mv[ 2 ][ _frame ][ 1 ] + _accum[ 1 ],
            31 );
        ncandidates++;
        if ( i >= embs[ _mbi ].npneighbors )
            break;
...
static unsigned oc_satd16_halfpel( const oc_enc_ctx* _enc,
                                   const ptrdiff_t* _frag_buf_offs,
                                   const ptrdiff_t _fragis[ 4 ],
                                   int _mvoffset0,
                                   int _mvoffset1,
    ...
    for ( bi = 0; bi < 4; bi++ ) {
        ptrdiff_t frag_offs;
        frag_offs = _frag_buf_offs[ _fragis[ bi ] ];
        err += oc_enc_frag_satd2_thresh(
            _enc, _src + frag_offs, _ref + frag_offs + _mvoffset0,
            _ref + frag_offs + _mvoffset1, _ystride, _best_err - err );
    }
    return err;
}

static unsigned oc_mcenc_ysad_check_mbcandidate_fullpel(
    const oc_enc_ctx* _enc,
    const ptrdiff_t* _frag_buf_offs,
    const ptrdiff_t _fragis[ 4 ],
    int _dx,
    int _dy,
    ...
    for ( bi = 0; bi < 4; bi++ ) {
        ...
        unsigned block_err;
        frag_offs = _frag_buf_offs[ _fragis[ bi ] ];
        block_err = oc_enc_frag_sad( _enc, _src + frag_offs,
                                     _ref + frag_offs + mvoffset, _ystride );
...
static int oc_mcenc_ysatd_check_mbcandidate_fullpel(
    const oc_enc_ctx* _enc,
    const ptrdiff_t* _frag_buf_offs,
    const ptrdiff_t _fragis[ 4 ],
    int _dx,
    ...
    for ( bi = 0; bi < 4; bi++ ) {
        ptrdiff_t frag_offs;
        frag_offs = _frag_buf_offs[ _fragis[ bi ] ];
        err += oc_enc_frag_satd_thresh( _enc, _src + frag_offs,
                                        _ref + frag_offs + mvoffset, _ystride,
                                        UINT_MAX );
    }
    return err;
...
static unsigned oc_mcenc_ysatd_check_bcandidate_fullpel(
    const oc_enc_ctx* _enc,
    ptrdiff_t _frag_offs,
    int _dx,
    int _dy,
    const unsigned char* _src,
    const unsigned char* _ref,
    int _ystride ) {
    return oc_enc_frag_satd_thresh( _enc, _src + _frag_offs,
                                    _ref + _frag_offs + _dx + _dy * _ystride,
                                    _ystride, UINT_MAX );
}
...
void oc_mcenc_search_frame( oc_enc_ctx* _enc,
                            int _accum[ 2 ],
                            int _mbi,
                            int _frame ) {
    ...
    best_err = oc_mcenc_ysad_check_mbcandidate_fullpel(
        _enc, frag_buf_offs, fragis, candx, candy, src, ref, ystride,
        block_err );
    best_vec[ 0 ] = candx;
    best_vec[ 1 ] = candy;
    ...
    if ( best_err > OC_YSAD_THRESH1 ) {
        ...
        for ( ci = 0; ci < ncs; ci++ ) {
            t2 = OC_MAXI(
                t2, embs[ embs[ _mbi ].cneighbors[ ci ] ].error[ _frame ] );
        }
        ...
        for ( ci = 1; ci < mcenc.setb0; ci++ ) {
            ...
            /*If we've already examined this vector, then we would be using it
               if it was better than what we are using.*/
            hitbit = ( ogg_int32_t )1 << candx + 15;
            if ( hit_cache[ candy + 15 ] & hitbit )
                continue;
            hit_cache[ candy + 15 ] |= hitbit;
            err = oc_mcenc_ysad_check_mbcandidate_fullpel(
                _enc, frag_buf_offs, fragis, candx, candy, src, ref, ystride,
                block_err );
            if ( err < best_err ) {
                best_err = err;
        ...
        if ( best_err > t2 ) {
            ...
            for ( ; ci < mcenc.ncandidates; ci++ ) {
                ...
                hit_cache[ candy + 15 ] |= hitbit;
                err = oc_mcenc_ysad_check_mbcandidate_fullpel(
                    _enc, frag_buf_offs, fragis, candx, candy, src, ref,
                    ystride, block_err );
                if ( err < best_err ) {
                    best_err = err;
                    best_vec[ 0 ] = candx;
            ...
            if ( best_err > t2 ) {
                ...
                for ( ;; ) {
                    ...
                    b = OC_DIV16( -best_vec[ 0 ] + 1 ) |
                        OC_DIV16( best_vec[ 0 ] + 1 ) << 1 |
                        OC_DIV16( -best_vec[ 1 ] + 1 ) << 2 |
                        OC_DIV16( best_vec[ 1 ] + 1 ) << 3;
                    ...
                    for ( bi = 0; bi < 4; bi++ ) {
                        if ( best_block_err[ bi ] > t2 ) {
                            /*Square pattern search.
                              ...
                              We continue to check the SAD of all four blocks in
                              the macro block. This gives us two things: 1) We
                              can continue to use the hit_cache to avoid
                              ...
                              avoid extra hit_cache queries, and it might even
                              be a speed win. 2) It gives us a slightly better
                              chance of escaping local minima. We would not be
                              here if we weren't doing a fairly bad job in
                              finding a good vector, and checking these vectors
                              can save us from 100 to several thousand points
                              off our SAD 1 in 15 times.
                            ...
                            for ( ;; ) {
                                ...
                                /*Compose the bit flags for boundary
                                 * conditions.*/
                                b = OC_DIV16( -bestx + 1 ) |
                                    OC_DIV16( bestx + 1 ) << 1 |
                                    OC_DIV16( -besty + 1 ) << 2 |
                                ...
                                for ( sitei = 0; sitei < nsites; sitei++ ) {
                                    ...
                                    hit_cache[ candy + 15 ] |= hitbit;
                                    err =
                                        oc_mcenc_ysad_check_mbcandidate_fullpel(
                                            _enc, frag_buf_offs, fragis, candx,
                                            candy, src, ref, ystride,
                                            block_err );
                                    if ( err < best_err ) {
                                        best_err = err;
                                        best_vec[ 0 ] = candx;
                                ...
                                if ( best_block_vec[ bi ][ 0 ] == bestx &&
                                     best_block_vec[ bi ][ 1 ] == besty ) {
                                    break;
                                }
                            }
                        }
    ...
    candx = best_vec[ 0 ];
    candy = best_vec[ 1 ];
    embs[ _mbi ].satd[ _frame ] = oc_mcenc_ysatd_check_mbcandidate_fullpel(
        _enc, frag_buf_offs, fragis, candx, candy, src, ref, ystride );
    embs[ _mbi ].analysis_mv[ 0 ][ _frame ][ 0 ] =
        ( signed char )( candx << 1 );
    embs[ _mbi ].analysis_mv[ 0 ][ _frame ][ 1 ] =
        ( signed char )( candy << 1 );
    if ( _frame == OC_FRAME_PREV ) {
        for ( bi = 0; bi < 4; bi++ ) {
            candx = best_block_vec[ bi ][ 0 ];
            candy = best_block_vec[ bi ][ 1 ];
            embs[ _mbi ].block_satd[ bi ] =
                oc_mcenc_ysatd_check_bcandidate_fullpel(
                    _enc, frag_buf_offs[ fragis[ bi ] ], candx, candy, src, ref,
                    ystride );
...
static unsigned oc_mcenc_ysatd_halfpel_mbrefine( const oc_enc_ctx* _enc,
    ...
    for ( sitei = 0; sitei < 8; sitei++ ) {
        ...
        /*The following code SHOULD be equivalent to
            oc_state_get_mv_offsets(&_mcenc->enc.state,&mvoffset0,&mvoffset1,
             (_vec[0]<<1)+dx,(_vec[1]<<1)+dy,ref_ystride,0);
          However, it should also be much faster, as it involves no multiplies
          and doesn't have to handle chroma vectors.*/
        xmask = OC_SIGNMASK( ( ( _vec[ 0 ] << 1 ) + dx ) ^ dx );
        ymask = OC_SIGNMASK( ( ( _vec[ 1 ] << 1 ) + dy ) ^ dy );
        mvoffset0 =
            mvoffset_base + ( dx & xmask ) + ( offset_y[ site ] & ymask );
        mvoffset1 =
            mvoffset_base + ( dx & ~xmask ) + ( offset_y[ site ] & ~ymask );
        err = oc_satd16_halfpel( _enc, frag_buf_offs, fragis, mvoffset0,
                                 mvoffset1, src, ref, ystride, _best_err );
        if ( err < _best_err ) {
            _best_err = err;
            best_site = site;
        }
...
void oc_mcenc_refine1mv( oc_enc_ctx* _enc, int _mbi, int _frame ) {
    ...
    embs[ _mbi ].satd[ _frame ] = oc_mcenc_ysatd_halfpel_mbrefine(
        _enc, _mbi, vec, embs[ _mbi ].satd[ _frame ], _frame );
...
static unsigned oc_mcenc_ysatd_halfpel_brefine( const oc_enc_ctx* _enc,
    ...
    for ( sitei = 0; sitei < 8; sitei++ ) {
        ...
        ymask = OC_SIGNMASK( ( ( _vec[ 1 ] << 1 ) + dy ) ^ dy );
        mvoffset0 =
            mvoffset_base + ( dx & xmask ) + ( _offset_y[ site ] & ymask );
        mvoffset1 =
            mvoffset_base + ( dx & ~xmask ) + ( _offset_y[ site ] & ~ymask );
        err = oc_enc_frag_satd2_thresh( _enc, _src, _ref + mvoffset0,
                                        _ref + mvoffset1, _ystride, _best_err );
        if ( err < _best_err ) {
            _best_err = err;
            best_site = site;
        }
...
void oc_mcenc_refine4mv( oc_enc_ctx* _enc, int _mbi ) {
    ...
    for ( bi = 0; bi < 4; bi++ ) {
        ...
        embs[ _mbi ].block_satd[ bi ] = oc_mcenc_ysatd_halfpel_brefine(
            _enc, vec, src + frag_offs, ref + frag_offs, ystride, offset_y,
            embs[ _mbi ].block_satd[ bi ] );
