...
void oc_enc_frag_sub( const oc_enc_ctx* _enc,
                      ogg_int16_t _diff[ 64 ],
...
void oc_enc_frag_sub_c( ogg_int16_t _diff[ 64 ],
    ...
    for ( i = 0; i < 8; i++ ) {
        int j;
        for ( j = 0; j < 8; j++ )
            _diff[ i * 8 + j ] = ( ogg_int16_t )( _src[ j ] - _ref[ j ] );
        _src += _ystride;
        _ref += _ystride;
    }
}

void oc_enc_frag_sub_128( const oc_enc_ctx* _enc,
                          ogg_int16_t _diff[ 64 ],
                          const unsigned char* _src,
                          int _ystride ) {
...
void oc_enc_frag_sub_128_c( ogg_int16_t* _diff,
    ...
    for ( i = 0; i < 8; i++ ) {
        int j;
        for ( j = 0; j < 8; j++ )
            _diff[ i * 8 + j ] = ( ogg_int16_t )( _src[ j ] - 128 );
        _src += _ystride;
    }
}

unsigned oc_enc_frag_sad( const oc_enc_ctx* _enc,
...
unsigned oc_enc_frag_sad_thresh( const oc_enc_ctx* _enc,
                                 ...
                                 int _ystride,
                                 unsigned _thresh ) {
    return ( *_enc->opt_vtable.frag_sad_thresh )( _src, _ref, _ystride,
                                                  _thresh );
}
...
unsigned oc_enc_frag_sad2_thresh_c( const unsigned char* _src,
    ...
    for ( i = 8; i-- > 0; ) {
        int j;
        for ( j = 0; j < 8; j++ )
            sad += abs( _src[ j ] - ( _ref1[ j ] + _ref2[ j ] >> 1 ) );
        if ( sad > _thresh )
            break;
        _src += _ystride;
        _ref1 += _ystride;
...
static void oc_diff_hadamard( ogg_int16_t _buf[ 64 ],
                              const unsigned char* _src,
                              const unsigned char* _ref,
                              int _ystride ) {
    int i;
...
static void oc_diff_hadamard2( ogg_int16_t _buf[ 64 ],
                               const unsigned char* _src,
                               const unsigned char* _ref1,
                               const unsigned char* _ref2,
                               int _ystride ) {
...
static void oc_intra_hadamard( ogg_int16_t _buf[ 64 ],
                               const unsigned char* _src,
                               int _ystride ) {
    int i;
    for ( i = 0; i < 8; i++ ) {
...
unsigned oc_hadamard_sad_thresh( const ogg_int16_t _buf[ 64 ],
                                 unsigned _thresh ) {
    unsigned sad;
    int t0;
    int t1;
...
unsigned oc_enc_frag_satd_thresh( const oc_enc_ctx* _enc,
                                  ...
                                  int _ystride,
                                  unsigned _thresh ) {
    return ( *_enc->opt_vtable.frag_satd_thresh )( _src, _ref, _ystride,
                                                   _thresh );
}
...
unsigned oc_enc_frag_satd2_thresh( const oc_enc_ctx* _enc,
                                   ...
                                   int _ystride,
                                   unsigned _thresh ) {
    return ( *_enc->opt_vtable.frag_satd2_thresh )( _src, _ref1, _ref2,
                                                    _ystride, _thresh );
}
...
unsigned oc_enc_frag_intra_satd_c( const unsigned char* _src, int _ystride ) {
    ...
    return oc_hadamard_sad_thresh( buf, UINT_MAX ) -
           abs( buf[ 0 ] + buf[ 1 ] + buf[ 2 ] + buf[ 3 ] + buf[ 4 ] +
                buf[ 5 ] + buf[ 6 ] + buf[ 7 ] );
}

void oc_enc_frag_copy2( const oc_enc_ctx* _enc,
                        unsigned char* _dst,
...
void oc_enc_frag_recon_inter( const oc_enc_ctx* _enc,
                              unsigned char* _dst,
                              const unsigned char* _src,
                              int _ystride,
                              const ogg_int16_t _residue[ 64 ] ) {
