...
static const unsigned char OC_DCT_TOKEN_MAP_ENTRIES[ TH_NDCT_TOKENS ] = {
    1, 1, 1, 4, 8, 1, 1, 8, 1, 1, 1, 1, 1, 2, 2, 2,
    2, 4, 8, 2, 2, 2, 4, 2, 2, 2, 2, 2, 8, 2, 4, 8 };
...
static long oc_pack_look( oc_pack_buf* _b, int _bits ) {
    ...
    if ( _bits == 0 )
        return 0;
    if ( _bits > available )
        _b->window = window = oc_pack_refill( _b, _bits );
...
static void oc_pack_adv( oc_pack_buf* _b, int _bits ) {
    /*We ignore the special cases for _bits==0 and _bits==32 here, since they
      are never used actually used. OC_HUFF_SLUSH (defined below) would have to
      be at least 27 to actually read 32 bits in a single go, and would require
      a 32 GB lookup table (assuming 8 byte pointers, since 4 byte pointers
      couldn't fit such a table).*/
    _b->window <<= _bits;
    _b->bits -= _bits;
}
...
static size_t oc_huff_node_size( int _nbits ) {
    ...
    if ( _nbits > 0 )
        size += sizeof( oc_huff_node* ) * ( 1 << _nbits );
    return size;
}

static oc_huff_node* oc_huff_node_init( char** _storage,
                                        size_t _size,
                                        int _nbits ) {
    oc_huff_node* ret;
...
static size_t oc_huff_tree_size( const oc_huff_node* _node ) {
    ...
    if ( _node->nbits ) {
        ...
        for ( i = 0; i < nchildren;
              i += 1 << _node->nbits - _node->nodes[ i ]->depth ) {
...
static int oc_huff_tree_unpack( oc_pack_buf* _opb,
    ...
    if ( !bits ) {
        ...
        if ( ret >= 0 ) {
            ...
            ret = oc_huff_tree_unpack( _opb, _binodes + nused,
                                       _nbinodes - nused );
        }
        if ( ret < 0 )
            return ret;
    ...
    /*Read a leaf node:*/
        ...
        if ( oc_pack_bytes_left( _opb ) < 0 )
            return TH_EBADHEADER;
        /*Find out how many internal tokens we translate this external token
         * into.*/
        ntokens = OC_DCT_TOKEN_MAP_ENTRIES[ bits ];
        if ( _nbinodes < 2 * ntokens - 1 )
            return TH_EBADHEADER;
...
static oc_huff_node* oc_huff_tree_copy( const oc_huff_node* _node,
                                        char** _storage ) {
    oc_huff_node* ret;
    ret = oc_huff_node_init( _storage, oc_huff_node_size( _node->nbits ),
                             _node->nbits );
...
static size_t oc_huff_tree_collapse_size( oc_huff_node* _binode, int _depth ) {
    ...
    } while ( occupancy > loccupancy &&
              occupancy >= 1 << OC_MAXI( depth - OC_HUFF_SLUSH, 0 ) );
    depth--;
...
static void oc_huff_node_fill( oc_huff_node** _nodes,
    ...
    if ( _level <= 0 || _binode->nbits == 0 ) {
        ...
        oc_huff_node_fill( _nodes, _binode->nodes[ 0 ], _level, _depth,
                           _storage );
        _nodes += 1 << _level;
        oc_huff_node_fill( _nodes, _binode->nodes[ 1 ], _level, _depth,
                           _storage );
    }
}
...
static oc_huff_node* oc_huff_tree_collapse( oc_huff_node* _binode,
    ...
    depth--;
    if ( depth <= 1 )
        return oc_huff_tree_copy( _binode, _storage );
    size = oc_huff_node_size( depth );
...
int oc_huff_trees_unpack( oc_pack_buf* _opb,
    ...
    for ( i = 0; i < TH_NHUFFMAN_TABLES; i++ ) {
        ...
        ret = oc_huff_tree_unpack( _opb, nodes,
