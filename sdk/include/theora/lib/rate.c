...
static unsigned short OC_ROUGH_TAN_LOOKUP[ 18 ] = {
    0,    358,  722,  1098, 1491, 1910,  2365,  2868,  3437,
    4096, 4881, 5850, 7094, 8784, 11254, 15286, 23230, 46817 };
...
static int oc_warp_alpha( int _alpha ) {
    ...
    d = _alpha * 36 - ( i << 24 );
    return ( int )( ( ( ogg_int64_t )t0 << 32 ) +
                        ( t1 - t0 << 8 ) * ( ogg_int64_t )d >>
                    32 );
}
...
/*Initialize a 2nd order low-pass Bessel filter with the corresponding delay
   and initial value.
  _value is Q24.*/
static void oc_iir_filter_init( oc_iir_filter* _f,
                                int _delay,
                                ogg_int32_t _value ) {
...
static int oc_enc_find_qi_for_target( oc_enc_ctx* _enc,
                                      int _qti,
                                      int _qi_old,
                                      int _qi_min,
                                      ogg_int64_t _log_qtarget ) {
    ...
    best_qdiff =
        best_qdiff + OC_SIGNMASK( best_qdiff ) ^ OC_SIGNMASK( best_qdiff );
    for ( qi = _qi_min + 1; qi < 64; qi++ ) {
        ...
        if ( qdiff < best_qdiff ||
             qdiff == best_qdiff &&
                 abs( qi - _qi_old ) < abs( best_qi - _qi_old ) ) {
            best_qi = qi;
            best_qdiff = qdiff;
        }
    }
...
void oc_enc_calc_lambda( oc_enc_ctx* _enc, int _qti ) {
    ...
    /*Select additional quantizers.
      The R-D optimal block AC quantizer statistics suggest that the
      distribution is roughly Gaussian-like with a slight positive skew. K-means
      clustering on log_qavg to select 3 quantizers produces cluster centers of
      {log_qavg-0.6,log_qavg,log_qavg+0.7}. Experiments confirm these are
    ...
    if ( lq < ( OC_Q57( 56 ) >> 3 ) && !_enc->vp3_compatible ) {
        qi1 = oc_enc_find_qi_for_target( _enc, _qti, OC_MAXI( qi - 1, 0 ), 0,
                                         lq + ( OC_Q57( 7 ) + 5 ) / 10 );
        if ( qi1 != qi )
            _enc->state.qis[ nqis++ ] = qi1;
        qi1 = oc_enc_find_qi_for_target( _enc, _qti, OC_MINI( qi + 1, 63 ), 0,
                                         lq - ( OC_Q57( 6 ) + 5 ) / 10 );
        if ( qi1 != qi && qi1 != _enc->state.qis[ nqis - 1 ] )
            _enc->state.qis[ nqis++ ] = qi1;
    }
...
static void oc_enc_rc_reset( oc_enc_ctx* _enc ) {
    ...
    } else if ( _enc->rc.bits_per_frame < 32 )
        _enc->rc.bits_per_frame = 32;
    _enc->rc.buf_delay = OC_MAXI( _enc->rc.buf_delay, 12 );
    _enc->rc.max = _enc->rc.bits_per_frame * _enc->rc.buf_delay;
    /*Start with a buffer fullness of 50% plus 25% of the amount we plan to
      spend on a single keyframe interval. We can require fully half the bits in
      an interval for a keyframe, so this initial level gives us maximum
      flexibility for over/under-shooting in subsequent frames.*/
    _enc->rc.target =
        ( _enc->rc.max + 1 >> 1 ) +
        ( _enc->rc.bits_per_frame + 2 >> 2 ) *
    ...
    inter_delay =
        ( _enc->rc.twopass ? OC_MAXI( _enc->keyframe_frequency_force, 12 )
                           : _enc->rc.buf_delay ) >>
        1;
    _enc->rc.inter_count = 0;
    /*We clamp the actual inter_delay to a minimum of 10 to work within the
      range of values where later incrementing the delay works as designed. 10
      is not an exact choice, but rather a good working trade-off.*/
    ...
    if ( _enc->state.info.target_bitrate > 0 ) {
        /*The buffer size is set equal to the keyframe interval, clamped to the
          ...
          The 12 frame minimum gives us some chance to distribute bit estimation
           errors.
          The 256 frame maximum means we'll require 8-10 seconds of
          pre-buffering at 24-30 fps, which is not unreasonable.*/
...
void oc_enc_rc_resize( oc_enc_ctx* _enc ) {
    ...
    if ( _enc->state.curframe_num < 0 )
        ...
        /*Update the INTER-frame scale filter delay.
          We jump to it immediately if we've already seen enough frames;
          otherwise it is simply set as the new target.*/
        _enc->rc.inter_delay_target = idt =
            OC_MAXI( _enc->rc.buf_delay >> 1, 10 );
    ...
    if ( _enc->rc.twopass == 2 ) {
        ...
        reset_window = _enc->rc.frame_metrics == NULL &&
                       ( _enc->rc.frames_total[ 0 ] == 0 ||
                         buf_delay < _enc->rc.frames_total[ 0 ] +
                                         _enc->rc.frames_total[ 1 ] +
        ...
        if ( cfm < buf_delay &&
             ( _enc->rc.frame_metrics != NULL || reset_window ) ) {
            oc_frame_metrics* fm;
            int nfm;
            int fmh;
            fm = ( oc_frame_metrics* )_ogg_realloc(
                _enc->rc.frame_metrics,
                buf_delay * sizeof( *_enc->rc.frame_metrics ) );
            if ( fm == NULL ) {
                /*We failed to allocate a finite buffer.*/
                /*If we don't have a valid 2-pass header yet, just return; we'll
                   reset the buffer size when we read the header.*/
                if ( _enc->rc.frames_total[ 0 ] == 0 )
                    return;
                /*Otherwise revert to the largest finite buffer previously set,
                   or to whole-file buffering if we were still using that.*/
                _enc->rc.buf_delay = _enc->rc.frame_metrics != NULL
                                         ? cfm
                                         : _enc->rc.frames_total[ 0 ] +
                                               _enc->rc.frames_total[ 1 ] +
                                               _enc->rc.frames_total[ 2 ];
            ...
            if ( fmh + nfm > cfm ) {
                ...
                memcpy( fm + cfm, fm,
                        OC_MINI( fmh + nfm - cfm, buf_delay - cfm ) *
                            sizeof( *fm ) );
                if ( fmh + nfm > buf_delay )
                    memmove( fm, fm + shift, fmh + nfm - buf_delay );
            }
        }
        ...
        if ( reset_window ) {
            ...
            if ( _enc->rc.twopass_buffer_bytes ) {
                int qti;
                /*We already read the metrics for the first frame in the
                 * window.*/
                *( _enc->rc.frame_metrics ) = *&_enc->rc.cur_metrics;
                ...
                _enc->rc.scale_sum[ qti ] +=
                    oc_bexp_q24( _enc->rc.cur_metrics.log_scale );
                _enc->rc.scale_window_end += _enc->rc.cur_metrics.dup_count + 1;
                if ( _enc->rc.scale_window_end - _enc->rc.scale_window0 <
                     buf_delay ) {
        ...
        /*Otherwise, we could shrink the size of the current window, if
           necessary, but leaving it like it is lets us adapt to the new buffer
           size more gracefully.*/
    }
}
...
static int oc_rc_scale_drop( oc_rc_state* _rc, int _nframes ) {
    if ( _rc->prev_drop_count > 0 || _rc->log_drop_scale > OC_Q57( 0 ) ) {
        ...
        dup_scale = oc_bexp64(
            ( _rc->log_drop_scale + oc_blog64( _rc->prev_drop_count + 1 ) >>
              1 ) +
            OC_Q57( 8 ) );
        if ( dup_scale < _nframes << 8 ) {
            ...
            if ( dup_scalei > 0 )
                _nframes = ( ( _nframes << 8 ) + dup_scalei - 1 ) / dup_scalei;
        } else
            _nframes = !!_nframes;
    }
    return _nframes;
...
int oc_enc_select_qi( oc_enc_ctx* _enc, int _qti, int _clamp ) {
    ...
    switch ( _enc->rc.twopass ) {
        default: {
            ...
            /*Single pass mode: assume only forced keyframes and attempt to
               estimate the drop count for VFR content.*/
            next_key_frame = _qti ? _enc->keyframe_frequency_force -
                                        ( _enc->state.curframe_num -
                                          _enc->state.keyframe_num )
                                  : 0;
            nframes[ 0 ] = ( _enc->rc.buf_delay -
                             OC_MINI( next_key_frame, _enc->rc.buf_delay ) +
                             _enc->keyframe_frequency_force - 1 ) /
                           _enc->keyframe_frequency_force;
            if ( nframes[ 0 ] + _qti > 1 ) {
                nframes[ 0 ]--;
                buf_delay = next_key_frame +
                            nframes[ 0 ] * _enc->keyframe_frequency_force;
            } else
                buf_delay = _enc->rc.buf_delay;
            nframes[ 1 ] = buf_delay - nframes[ 0 ];
            /*Downgrade the delta frame rate to correspond to the recent drop
               count history.*/
        ...
        case 2: {
            ...
            int qti;
            /*Pass 2 mode: we know exactly how much of each frame type there is
               in the current buffer window, and have estimates for the
               scales.*/
            nframes[ 0 ] = _enc->rc.nframes[ 0 ];
            nframes[ 1 ] = _enc->rc.nframes[ 1 ];
            scale_sum[ 0 ] = _enc->rc.scale_sum[ 0 ];
            scale_sum[ 1 ] = _enc->rc.scale_sum[ 1 ];
            /*The window size can be slightly larger than the buffer window for
               VFR content; clamp it down, if appropriate (the excess will all
               be dup frames).*/
            buf_delay =
                OC_MINI( _enc->rc.scale_window_end - _enc->rc.scale_window0,
                         _enc->rc.buf_delay );
            /*If we're approaching the end of the file, add some slack to keep
              us from slamming into a rail. Our rate accuracy goes down, but it
              keeps the result sensible. We position the target where the first
              forced keyframe beyond the end of the file would be (for
              consistency with 1-pass mode).*/
            buf_pad = OC_MINI( _enc->rc.buf_delay,
                               _enc->state.keyframe_num +
                                   _enc->keyframe_frequency_force -
                                   _enc->rc.scale_window0 );
            if ( buf_delay < buf_pad )
                buf_pad -= buf_delay;
            else {
                /*Otherwise, search for the last keyframe in the buffer window
                   and target that.*/
                buf_pad = 0;
                /*TODO: Currently we only do this when using a finite buffer; we
                   could save the position of the last keyframe in the summary
                   data and do it with a whole-file buffer as well, but it isn't
                   likely to make a difference.*/
                if ( _enc->rc.frame_metrics != NULL ) {
                    int fmi;
                    int fm_tail;
                    fm_tail =
                        _enc->rc.frame_metrics_head + _enc->rc.nframe_metrics;
                    if ( fm_tail >= _enc->rc.cframe_metrics )
                        fm_tail -= _enc->rc.cframe_metrics;
                    for ( fmi = fm_tail;; ) {
                        oc_frame_metrics* m;
                        fmi--;
                        if ( fmi < 0 )
                            fmi += _enc->rc.cframe_metrics;
                        /*Stop before we remove the first frame.*/
                        if ( fmi == _enc->rc.frame_metrics_head )
                            break;
                        m = _enc->rc.frame_metrics + fmi;
                        /*If we find a keyframe, remove it and everything past
                         * it.*/
            ...
            /*If we're not using the same frame type as in pass 1 (because
              someone changed the keyframe interval), remove that scale
              estimate. We'll add in a replacement for the correct frame type
              below.*/
            qti = _enc->rc.cur_metrics.frame_type;
            if ( qti != _qti ) {
                nframes[ qti ]--;
            ...
            /*Compute log_scale estimates for each frame type from the pass-1
               scales we measured in the current window.*/
            for ( qti = 0; qti < 2; qti++ ) {
                _enc->rc.log_scale[ qti ] =
                    nframes[ qti ] > 0
                        ? oc_blog64( scale_sum[ qti ] ) -
            ...
            /*If we're not using the same frame type as in pass 1, add a scale
               ...
               filter value.
              This is mostly to ensure we have a valid estimate even when pass 1
              had no frames of this type in the buffer window.
              TODO: We could also plan ahead and figure out how many keyframes
              we'll be forced to add in the current buffer window.*/
            qti = _enc->rc.cur_metrics.frame_type;
            if ( qti != _qti ) {
                ogg_int64_t scale;
                scale =
                    _enc->rc.log_scale[ _qti ] < OC_Q57( 23 )
                        ? oc_bexp64( _enc->rc.log_scale[ _qti ] + OC_Q57( 24 ) )
                        : 0x7FFFFFFFFFFFLL;
                scale *= nframes[ _qti ];
                nframes[ _qti ]++;
                scale += oc_bexp_q24( log_cur_scale >> 33 );
                _enc->rc.log_scale[ _qti ] = oc_blog64( scale ) -
                                             oc_blog64( nframes[ qti ] ) -
                                             OC_Q57( 24 );
            } else
                log_cur_scale = ( ogg_int64_t )_enc->rc.cur_metrics.log_scale
                                << 33;
            ...
            if ( buf_pad > 0 ) {
                ...
                /*Extend the buffer.*/
                buf_delay += buf_pad;
                /*Add virtual delta frames according to the estimated drop
                 * count.*/
                nextra_frames = oc_rc_scale_drop( &_enc->rc, buf_pad );
                /*And blend in the low-pass filtered scale according to how many
                   frames we added.*/
                scale = oc_bexp64( _enc->rc.log_scale[ 1 ] + OC_Q57( 24 ) ) *
                            ( ogg_int64_t )nframes[ 1 ] +
                        oc_bexp_q24( _enc->rc.scalefilter[ 1 ].y[ 0 ] ) *
                            ( ogg_int64_t )nextra_frames;
                nframes[ 1 ] += nextra_frames;
                _enc->rc.log_scale[ 1 ] = oc_blog64( scale ) -
                                          oc_blog64( nframes[ 1 ] ) -
                                          OC_Q57( 24 );
            }
        } break;
    ...
    if ( rate_total <= buf_delay )
        log_qtarget = OC_QUANT_MAX_LOG;
    else {
        ...
        while ( rlo < rhi ) {
            ...
            log_rpow = ( log_rpow + ( _enc->rc.exp[ _qti ] >> 1 ) ) /
                       _enc->rc.exp[ _qti ];
            if ( _qti )
                log_rpow += LOG_KEY_RATIO >> 6;
            else
                log_rpow -= LOG_KEY_RATIO >> 6;
        ...
        log_qtarget = OC_Q57( 2 ) - ( ( oc_blog64( rlo ) - log_scale0 +
                                        ( _enc->rc.exp[ _qti ] >> 1 ) ) /
                                          _enc->rc.exp[ _qti ]
                                      << 6 );
    ...
    if ( _enc->rc.cap_overflow ) {
        ...
        soft_limit = _enc->rc.fullness + _enc->rc.bits_per_frame -
                     ( _enc->rc.max - margin );
        ...
        if ( log_scale0 - log_qexp < log_soft_limit ) {
            ...
            log_qtarget =
                ( ( log_qexp + ( exp0 >> 1 ) ) / exp0 << 6 ) + OC_Q57( 2 );
        }
    }
    ...
    if ( _clamp ) {
        ...
        /*Clamp the target quantizer to within [0.8*Q,1.2*Q], where Q is the
           current quantizer.
          TODO: With user-specified quant matrices, we need to enlarge these
          limits if they don't actually let us change qi values.*/
    ...
    /*The above allocation looks only at the total rate we'll accumulate in the
       next buf_delay frames.
      However, we could bust the budget on the very next frame, so check for
      that here, if we're not using a soft target.*/
    ...
    if ( _enc->state.info.quality == 0 ) {
        ...
        /*Compute the maximum number of bits we can use in the next frame.
          ...
          complexity.*/
        log_hard_limit =
            oc_blog64( _enc->rc.fullness + ( _enc->rc.bits_per_frame >> 1 ) );
    ...
    _enc->rc.rate_bias +=
        oc_bexp64( log_cur_scale + _enc->rc.log_npixels - log_qexp );
...
int oc_enc_update_rc_state( oc_enc_ctx* _enc,
    ...
    /* Drop frames also disabled for now in the case of infinite-buffer
       two-pass mode */
    if ( !_enc->rc.drop_frames ||
         _enc->rc.twopass && _enc->rc.frame_metrics == NULL ) {
        _droppable = 0;
    }
    ...
    if ( _bits <= 0 ) {
        ...
        log_scale =
            OC_MINI( log_bits - _enc->rc.log_npixels + log_qexp, OC_Q57( 16 ) );
    }
        ...
        case 2: {
            ...
            if ( !_trial ) {
                ...
                *&_enc->rc.prev_metrics = *&_enc->rc.cur_metrics;
                ...
                /*Back out the last frame's statistics from the sliding
                 * window.*/
                ...
                /*Update state, so the user doesn't have to keep calling
                   2pass_in after they've fed in all the data when we're using a
                   finite buffer.*/
    ...
    if ( _bits > 0 ) {
        if ( _trial ) {
            ...
            if ( _enc->rc.inter_delay < _enc->rc.inter_delay_target &&
                 _enc->rc.inter_count >= _enc->rc.inter_delay &&
                 _qti == OC_INTER_FRAME ) {
                oc_iir_filter_reinit( &_enc->rc.scalefilter[ 1 ],
                                      ++_enc->rc.inter_delay );
            }
            /*Otherwise update the low-pass scale filter for this frame type,
               regardless of whether or not we dropped this frame.*/
            _enc->rc.log_scale[ _qti ] =
                oc_iir_filter_update( _enc->rc.scalefilter + _qti,
                                      oc_q57_to_q24( log_scale ) )
                << 33;
            ...
            if ( _droppable && _enc->rc.fullness + buf_delta < _bits ) {
                ...
                /*Update a low-pass filter to estimate the "real" frame rate
                  taking drops and duplicates into account. This is only done if
                  the frame is coded, as it needs the final count of dropped
                  frames.*/
                ...
                if ( drop_count > 0x7F )
                    ...
                    drop_count <<= 24;
                _enc->rc.log_drop_scale =
                    oc_blog64( oc_iir_filter_update( &_enc->rc.vfrfilter,
                                                     drop_count ) ) -
                    OC_Q57( 24 );
                /*Initialize the drop count for this frame to the user-requested
                  dup count. It will be increased if we drop more frames.*/
                _enc->rc.prev_drop_count = _enc->dup_count;
            }
        }
        /*Increment the INTER frame count, for filter adaptation purposes.*/
        if ( _enc->rc.inter_count < INT_MAX )
            _enc->rc.inter_count += _qti;
    }
...
static void oc_rc_buffer_val( oc_rc_state* _rc, ogg_int64_t _val, int _bytes ) {
    while ( _bytes-- > 0 ) {
        _rc->twopass_buffer[ _rc->twopass_buffer_bytes++ ] =
            ( unsigned char )( _val & 0xFF );
        _val >>= 8;
    }
}
    ...
    if ( _enc->rc.twopass_buffer_bytes == 0 ) {
        if ( _enc->rc.twopass == 0 ) {
            ...
            oc_rc_buffer_val( &_enc->rc,
                              _enc->rc.cur_metrics.dup_count |
                                  _enc->rc.cur_metrics.frame_type << 31,
                              4 );
...
int oc_enc_rc_2pass_in( oc_enc_ctx* _enc, unsigned char* _buf, size_t _bytes ) {
    ...
    if ( _enc->rc.frames_total[ 0 ] == 0 ) {
        if ( !_buf ) {
            ...
            frames_needed =
                _enc->rc.frame_metrics == NULL ? 1 : _enc->rc.buf_delay;
            return OC_RC_2PASS_HDR_SZ + frames_needed * OC_RC_2PASS_PACKET_SZ -
                   _enc->rc.twopass_buffer_fill;
        }
        consumed = oc_rc_buffer_fill( &_enc->rc, _buf, _bytes, consumed,
                                      OC_RC_2PASS_HDR_SZ );
        if ( _enc->rc.twopass_buffer_fill >= OC_RC_2PASS_HDR_SZ ) {
            ...
            _enc->rc.frames_total[ 0 ] =
                ( ogg_uint32_t )oc_rc_unbuffer_val( &_enc->rc, 4 );
            _enc->rc.frames_total[ 1 ] =
                ( ogg_uint32_t )oc_rc_unbuffer_val( &_enc->rc, 4 );
            _enc->rc.frames_total[ 2 ] =
                ( ogg_uint32_t )oc_rc_unbuffer_val( &_enc->rc, 4 );
            ...
            /*Make sure the file claims to have at least one frame.
              ...
              pass 1. Also make sure the total frame count doesn't overflow an
              integer.*/
            buf_delay = _enc->rc.frames_total[ 0 ] +
                        _enc->rc.frames_total[ 1 ] + _enc->rc.frames_total[ 2 ];
            ...
            _enc->rc.exp[ 0 ] = exp[ 0 ];
            _enc->rc.exp[ 1 ] = exp[ 1 ];
            /*Clear the header data from the buffer to make room for packet
             * data.*/
            _enc->rc.twopass_buffer_fill = 0;
            _enc->rc.twopass_buffer_bytes = 0;
        }
    ...
    if ( _enc->rc.frames_total[ 0 ] != 0 ) {
        ...
        nframes_total = _enc->rc.frames_total[ 0 ] +
                        _enc->rc.frames_total[ 1 ] + _enc->rc.frames_total[ 2 ];
        if ( curframe_num >= nframes_total ) {
            /*We don't want any more data after the last frame, and we don't
               want to allow any more frames to be encoded.*/
            _enc->rc.twopass_buffer_bytes = 0;
        } else if ( _enc->rc.twopass_buffer_bytes == 0 ) {
            if ( _enc->rc.frame_metrics == NULL ) {
                /*We're using a whole-file buffer:*/
                if ( !_buf )
                    return OC_RC_2PASS_PACKET_SZ - _enc->rc.twopass_buffer_fill;
                consumed = oc_rc_buffer_fill( &_enc->rc, _buf, _bytes, consumed,
                                              OC_RC_2PASS_PACKET_SZ );
                if ( _enc->rc.twopass_buffer_fill >= OC_RC_2PASS_PACKET_SZ ) {
                    ...
                    /*"Helpfully" set the dup count back to what it was in
                     * pass 1.*/
                    arg = _enc->rc.cur_metrics.dup_count;
                    th_encode_ctl( _enc, TH_ENCCTL_SET_DUP_COUNT, &arg,
                                   sizeof( arg ) );
                ...
                while ( frames_needed > 0 ) {
                    ...
                    consumed =
                        oc_rc_buffer_fill( &_enc->rc, _buf, _bytes, consumed,
                                           OC_RC_2PASS_PACKET_SZ );
                    if ( _enc->rc.twopass_buffer_fill >=
                         OC_RC_2PASS_PACKET_SZ ) {
                        oc_frame_metrics* m;
                        int fmi;
                        ogg_uint32_t dup_count;
                        ...
                        fmi = _enc->rc.frame_metrics_head +
                              _enc->rc.nframe_metrics++;
                        if ( fmi >= _enc->rc.cframe_metrics )
                            fmi -= _enc->rc.cframe_metrics;
                        m = _enc->rc.frame_metrics + fmi;
                        ...
                        _enc->rc.scale_sum[ qti ] +=
                            oc_bexp_q24( m->log_scale );
                        _enc->rc.scale_window_end += m->dup_count + 1;
                        /*Compute an upper bound on the number of remaining
                           packets needed for the current window.*/
                        frames_needed = OC_CLAMPI(
                            0,
                            _enc->rc.buf_delay - ( _enc->rc.scale_window_end -
                                                   _enc->rc.scale_window0 ),
                            _enc->rc.frames_left[ 0 ] +
                                _enc->rc.frames_left[ 1 ] -
                                _enc->rc.nframes[ 0 ] - _enc->rc.nframes[ 1 ] );
                        /*Clear the buffer for the next frame.*/
                        _enc->rc.twopass_buffer_fill = 0;
                        _enc->rc.twopass_buffer_bytes = 0;
                ...
                /*If we've got all the frames we need, fill in the current
                  metrics. We're ready to go.*/
                if ( frames_needed <= 0 ) {
                    int arg;
                    *&_enc->rc.cur_metrics = *( _enc->rc.frame_metrics +
                                                _enc->rc.frame_metrics_head );
