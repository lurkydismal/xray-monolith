...
struct oc_enc_opt_vtable {
    ...
    void ( *frag_sub )( ogg_int16_t _diff[ 64 ],
...
struct oc_rc_state {
    ...
    /*The total drop count from the previous frame.
      This includes duplicates explicitly requested via the
       TH_ENCCTL_SET_DUP_COUNT API as well as frames we chose to drop
      ourselves.*/
...
int oc_state_flushheader(
    oc_theora_state* _state,
    int* _packet_state,
    oggpack_buffer* _opb,
...
void oc_enc_frag_recon_inter( const oc_enc_ctx* _enc,
                              ...
                              int _ystride,
                              const ogg_int16_t _residue[ 64 ] );
void oc_enc_fdct8x8( const oc_enc_ctx* _enc,
                     ogg_int16_t _y[ 64 ],
