...
static void oc_fdct8( ogg_int16_t _y[ 8 ], const ogg_int16_t* _x ) {
    ...
    /*Stages 3 and 4 are where all the approximation occurs.
      ...
      The (t5!=0) term could be replaced simply by 1, but we want to send 0 to
      0. The fDCT of an all-zeros block will still not be zero, because of the
       biases we added at the very beginning of the process, but it will be
      close enough that it is guaranteed to round to zero.*/
    /*Stage 3:*/
    /*4-5 butterfly.*/
...
void oc_enc_fdct8x8_c( ogg_int16_t _y[ 64 ], const ogg_int16_t _x[ 64 ] ) {
    ...
    for ( in = w, out = _y, end = out + 64; out < end; in++, out += 8 )
        oc_fdct8( out, in );
    /*Transform columns of _y into rows of w.*/
    for ( in = _y, out = w, end = out + 64; out < end; in++, out += 8 )
        oc_fdct8( out, in );
