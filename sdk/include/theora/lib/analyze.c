...
void oc_mode_scheme_chooser_init( oc_mode_scheme_chooser* _chooser ) {
    ...
    for ( si = 1; si < 8; si++ )
        _chooser->mode_ranks[ si ] = OC_MODE_RANKS[ si - 1 ];
}
...
static void oc_mode_scheme_chooser_reset( oc_mode_scheme_chooser* _chooser ) {
    int si;
    memset( _chooser->mode_counts, 0,
            OC_NMODES * sizeof( *_chooser->mode_counts ) );
    /*Scheme 0 starts with 24 bits to store the mode list in.*/
    _chooser->scheme_bits[ 0 ] = 24;
    memset( _chooser->scheme_bits + 1, 0,
            7 * sizeof( *_chooser->scheme_bits ) );
    for ( si = 0; si < 8; si++ ) {
...
static int oc_mode_scheme_chooser_cost( oc_mode_scheme_chooser* _chooser,
    ...
    mode_bits = OC_MODE_BITS[ scheme0 + 1 >> 3 ]
                            [ _chooser->mode_ranks[ scheme0 ][ _mb_mode ] ];
    /*Typical case: If the difference between the best scheme and the next best
       ...
       scheme we use.*/
    if ( _chooser->scheme_bits[ scheme1 ] - best_bits > 6 )
        return mode_bits;
    ...
    best_bits += mode_bits;
    do {
        /*For any scheme except 0, we can just use the bit cost of the mode's
           rank in that scheme.*/
        if ( scheme1 != 0 ) {
            scheme_bits =
                _chooser->scheme_bits[ scheme1 ] +
                OC_MODE_BITS[ scheme1 + 1 >> 3 ]
                            [ _chooser->mode_ranks[ scheme1 ][ _mb_mode ] ];
        } else {
            int ri;
            /*For scheme 0, incrementing the mode count could potentially change
              the mode's rank. Find the index where the mode would be moved to
              in the optimal list, and use its bit cost instead of the one for
              the mode's current position in the list.*/
            /*We don't recompute scheme bits; this is computing opportunity
               cost, not an update.*/
            for ( ri = _chooser->scheme0_ranks[ _mb_mode ];
                  ri > 0 &&
                  _chooser->mode_counts[ _mb_mode ] >=
                      _chooser->mode_counts[ _chooser->scheme0_list[ ri - 1 ] ];
                  ri-- )
                ;
            scheme_bits = _chooser->scheme_bits[ 0 ] + OC_MODE_BITS[ 0 ][ ri ];
        }
        if ( scheme_bits < best_bits )
            best_bits = scheme_bits;
        if ( ++si >= 8 )
            break;
        scheme1 = _chooser->scheme_list[ si ];
    } while ( _chooser->scheme_bits[ scheme1 ] -
                  _chooser->scheme_bits[ scheme0 ] <=
              6 );
...
static void oc_mode_scheme_chooser_update( oc_mode_scheme_chooser* _chooser,
    ...
    for ( si = 0; si < 8; si++ ) {
        _chooser->scheme_bits[ si ] +=
            OC_MODE_BITS[ si + 1 >> 3 ]
                        [ _chooser->mode_ranks[ si ][ _mb_mode ] ];
    }
    ...
    for ( si = 1; si < 8; si++ ) {
        ...
        do {
            ...
            if ( bits0 >= _chooser->scheme_bits[ scheme1 ] )
                break;
...
static void oc_fr_state_flush_sb( oc_fr_state* _fr ) {
    ...
    if ( b_count > 0 ) {
        ...
        if ( b_coded_count >= b_count ) {
            /*This SB was fully coded/uncoded; roll back the partial block
             * flags.*/
...
static void oc_fr_state_advance_block( oc_fr_state* _fr, int _b_coded ) {
    ...
    if ( b_count >= 16 ) {
        ...
        if ( b_coded_count >= 16 ) {
            ...
            if ( b_coded_count > 16 )
                bits += oc_block_run_bits( b_coded_count - 16 );
            sb_partial = 0;
            sb_full = _b_coded;
...
static void oc_enc_pipeline_init( oc_enc_ctx* _enc,
                                  oc_enc_pipeline_state* _pipe ) {
    ...
    for ( pli = 0; pli < 3; pli++ )
        oc_fr_state_init( _pipe->fr + pli );
    for ( pli = 0; pli < 3; pli++ )
        oc_qii_state_init( _pipe->qs + pli );
    ...
    for ( pli = 0; pli < 3; pli++ ) {
        for ( qii = 0; qii < _enc->state.nqis; qii++ ) {
            ...
            for ( qti = 0; qti < 2; qti++ ) {
                ...
                _pipe->enquant[ pli ][ qii ][ qti ] =
                    _enc->enquant_tables[ qi ][ pli ][ qti ];
            }
        }
    }
    ...
    _pipe->loop_filter =
        !oc_state_loop_filter_init( &_enc->state, _pipe->bounding_values );
}
...
static int oc_enc_pipeline_set_stripe( oc_enc_ctx* _enc,
    ...
    if ( notdone )
        sby_end = _sby + mcu_nvsbs;
    vdec = 0;
    for ( pli = 0; pli < 3; pli++ ) {
        fplane = _enc->state.fplanes + pli;
        _pipe->sbi0[ pli ] =
            fplane->sboffset + ( _sby >> vdec ) * fplane->nhsbs;
        ...
        if ( notdone ) {
            _pipe->sbi_end[ pli ] =
                fplane->sboffset + ( sby_end >> vdec ) * fplane->nhsbs;
...
static void oc_enc_pipeline_finish_mcu_plane( oc_enc_ctx* _enc,
                                              ...
                                              int _edelay ) {
    int refi;
    /*Copy over all the uncoded fragments from this plane and advance the
       uncoded fragment list.*/
    _pipe->uncoded_fragis[ _pli ] -= _pipe->nuncoded_fragis[ _pli ];
    oc_state_frag_copy_list( &_enc->state, _pipe->uncoded_fragis[ _pli ],
                             _pipe->nuncoded_fragis[ _pli ], OC_FRAME_SELF,
                             OC_FRAME_PREV, _pli );
    _pipe->nuncoded_fragis[ _pli ] = 0;
    /*Perform DC prediction.*/
    oc_enc_pred_dc_frag_rows( _enc, _pli, _pipe->fragy0[ _pli ],
                              _pipe->fragy_end[ _pli ] );
    /*Finish DC tokenization.*/
    oc_enc_tokenize_dc_frag_list(
        _enc, _pli, _pipe->coded_fragis[ _pli ], _pipe->ncoded_fragis[ _pli ],
    ...
    if ( _pipe->loop_filter ) {
        oc_state_loop_filter_frag_rows( &_enc->state, _pipe->bounding_values,
                                        refi, _pli,
                                        _pipe->fragy0[ _pli ] - _sdelay,
                                        _pipe->fragy_end[ _pli ] - _edelay );
    } else
        _sdelay = _edelay = 0;
...
static int oc_enc_block_transform_quantize( oc_enc_ctx* _enc,
    ...
    ref = _enc->state.ref_frame_data
              [ _enc->state.ref_frame_idx[ OC_FRAME_FOR_MODE( mb_mode ) ] ] +
          frag_offs;
    dst = _enc->state
              .ref_frame_data[ _enc->state.ref_frame_idx[ OC_FRAME_SELF ] ] +
          frag_offs;
    /*Motion compensation:*/
    switch ( mb_mode ) {
        case OC_MODE_INTRA: {
        ...
        default: {
            ...
            nmv_offs = oc_state_get_mv_offsets( &_enc->state, mv_offs, _pli,
                                                ...
                                                frag_mvs[ _fragi ][ 1 ] );
            if ( nmv_offs > 1 ) {
                oc_enc_frag_copy2( _enc, dst, ref + mv_offs[ 0 ],
                                   ref + mv_offs[ 1 ], ystride );
                oc_enc_frag_sub( _enc, data, src, dst, ystride );
            } else
                oc_enc_frag_sub( _enc, data, src, ref + mv_offs[ 0 ], ystride );
        } break;
    }
#if defined( OC_COLLECT_METRICS )
    {
        ...
        switch ( nmv_offs ) {
            case 0:
                satd = oc_enc_frag_intra_satd( _enc, src, ystride );
                break;
            case 1: {
                satd = oc_enc_frag_satd_thresh( _enc, src, ref + mv_offs[ 0 ],
                                                ystride, UINT_MAX );
            } break;
            default: {
                satd = oc_enc_frag_satd_thresh( _enc, src, dst, ystride,
                                                UINT_MAX );
            }
        }
    ...
    s = OC_SIGNMASK( val );
    val += dc_dequant + s ^ s;
    val = ( ( enquant[ 0 ].m * ( ogg_int32_t )val >> 16 ) + val >>
            enquant[ 0 ].l ) -
          s;
    ...
    for ( zzi = 1; zzi < 64; zzi++ ) {
        ...
        if ( v >= d ) {
            ...
            val = ( ( enquant[ zzi ].m * ( ogg_int32_t )val >> 16 ) + val >>
                    enquant[ zzi ].l ) -
                  s;
    ...
    /*Tokenize.*/
    checkpoint = *_stack;
    ac_bits = oc_enc_tokenize_ac( _enc, _pli, _fragi, data, dequant, dct,
                                  nonzero + 1, _stack, qti ? 0 : 3 );
    /*Reconstruct.
      TODO: nonzero may need to be adjusted after tokenization.*/
    if ( nonzero == 0 ) {
        ogg_int16_t p;
        int ci;
        /*We round this dequant product (and not any of the others) because
           there's no iDCT rounding.*/
    ...
    if ( !qti )
        oc_enc_frag_recon_intra( _enc, dst, ystride, data );
    else {
        oc_enc_frag_recon_inter( _enc, dst,
                                 nmv_offs == 1 ? ref + mv_offs[ 0 ] : dst,
                                 ystride, data );
    }
...
#endif
        uncoded_ssd =
            _pipe->skip_ssd[ _pli ][ _fragi - _pipe->froffset[ _pli ] ];
        if ( uncoded_ssd < UINT_MAX ) {
            /*Although the fragment coding overhead determination is accurate,
              it is greedy, using very coarse-grained local information.
              Allowing it to mildly discourage coding turns out to be
              beneficial, but it's not clear that allowing it to encourage
              coding through negative coding overhead deltas is useful. For that
              reason, we disallow negative coding_overheads.*/
            if ( _overhead_bits < 0 )
                _overhead_bits = 0;
            if ( uncoded_ssd <=
                     coded_ssd + ( _overhead_bits + ac_bits ) * _enc->lambda &&
                ...
                oc_enc_tokenlog_rollback( _enc, checkpoint,
                                          ( *_stack ) - checkpoint );
                *_stack = checkpoint;
                frags[ _fragi ].coded = 0;
                return 0;
    ...
    if ( _enc->state.frame_type != OC_INTRA_FRAME ) {
        if ( ncoded > 0 && !mo.dc_flag ) {
            ...
            /*Some individual blocks were worth coding.
              See if that's still true when accounting for mode and MV
              overhead.*/
            cost =
                mo.coded_ac_ssd +
                _enc->lambda * ( mo.ac_bits +
                                 oc_fr_cost4( &fr_checkpoint, _pipe->fr + 0 ) +
                                 _mode_overhead );
            if ( mo.uncoded_ac_ssd <= cost ) {
                /*Taking macroblock overhead into account, it is not worth
                   coding this MB.*/
        ...
        /*Assume that a 1MV with a single coded block is always cheaper than a
          4MV with a single coded block. This may not be strictly true: a 4MV
          computes chroma MVs using (0,0) for skipped blocks, while a 1MV does
          not.*/
        ...
        for ( quadi = 0; quadi < 4; quadi++ )
            for ( bi = 0; bi < 4; bi++ ) {
                ...
                if ( fragi >= 0 ) {
                    oc_token_checkpoint* stackptr;
                    stackptr = stack;
                    if ( oc_enc_block_transform_quantize(
                             _enc, _pipe, _pli, fragi,
                             oc_fr_cost1( _pipe->fr + _pli ), &mo,
                             &stackptr ) ) {
                        coded_fragis[ ncoded_fragis++ ] = fragi;
...
#define OC_MODE_RD_COST( _ssd, _rate, _lambda )                         \
    ( ( _ssd ) >> OC_BIT_SCALE ) +                                      \
        ( ( _rate ) >> OC_BIT_SCALE ) * ( _lambda ) +                   \
        ( ( ( _ssd ) & ( 1 << OC_BIT_SCALE ) - 1 ) +                    \
              ( ( _rate ) & ( 1 << OC_BIT_SCALE ) - 1 ) * ( _lambda ) + \
              ( ( 1 << OC_BIT_SCALE ) >> 1 ) >>                         \
          OC_BIT_SCALE )
...
static unsigned oc_analyze_intra_mb_luma( oc_enc_ctx* _enc,
    ...
    for ( qii = 0; qii < nqis; qii++ ) {
        ...
        rate[ 0 ][ qii ] =
            oc_dct_cost2( ssd[ 0 ] + qii, _enc->state.qis[ qii ], 0, 0, satd ) +
            ( qs[ 0 ][ qii ].bits - _qs->bits << OC_BIT_SCALE );
        cost[ 0 ][ qii ] =
            OC_MODE_RD_COST( ssd[ 0 ][ qii ], rate[ 0 ][ qii ], lambda );
    }
    for ( bi = 1; bi < 4; bi++ ) {
        fragi = sb_maps[ _mbi >> 2 ][ _mbi & 3 ][ bi ];
        frag_offs = frag_buf_offs[ fragi ];
        ...
        for ( qii = 0; qii < nqis; qii++ ) {
            ...
            cur_rate =
                oc_dct_cost2( &cur_ssd, _enc->state.qis[ qii ], 0, 0, satd );
            best_ssd = ssd[ bi - 1 ][ 0 ] + cur_ssd;
            best_rate =
                rate[ bi - 1 ][ 0 ] + cur_rate +
...
static unsigned oc_analyze_intra_chroma_block( oc_enc_ctx* _enc,
    ...
    for ( qii = 0; qii < nqis; qii++ ) {
        ...
        cur_rate =
            oc_dct_cost2( &cur_ssd, _enc->state.qis[ qii ], _pli, 0, satd ) +
...
static void oc_enc_sb_transform_quantize_intra_chroma(
    oc_enc_ctx* _enc,
    oc_enc_pipeline_state* _pipe,
    int _pli,
    int _sbi_start,
    ...
    for ( sbi = _sbi_start; sbi < _sbi_end; sbi++ ) {
        ...
        for ( quadi = 0; quadi < 4; quadi++ )
            for ( bi = 0; bi < 4; bi++ ) {
                ...
                if ( fragi >= 0 ) {
                    oc_token_checkpoint* stackptr;
                    oc_analyze_intra_chroma_block( _enc, _pipe->qs + _pli, _pli,
                                                   fragi );
                    stackptr = stack;
...
void oc_enc_analyze_intra( oc_enc_ctx* _enc, int _recode ) {
    ...
    for ( stripe_sby = 0; notdone; stripe_sby += mcu_nvsbs ) {
        ...
        for ( sbi = pipe.sbi0[ 0 ]; sbi < sbi_end; sbi++ ) {
            ...
            for ( quadi = 0; quadi < 4; quadi++ )
                if ( sb_flags[ sbi ].quad_valid & 1 << quadi ) {
                    unsigned mbi;
                    int mapii;
                    int mapi;
                    int bi;
                    ...
                    mbi = sbi << 2 | quadi;
                    /*Motion estimation:
                      We always do a basic 1MV search for all macroblocks, coded
                      or not, keyframe or not.*/
        ...
        /*Code chroma planes.*/
        for ( pli = 1; pli < 3; pli++ ) {
            oc_enc_sb_transform_quantize_intra_chroma(
                _enc, &pipe, pli, pipe.sbi0[ pli ], pipe.sbi_end[ pli ] );
            oc_enc_pipeline_finish_mcu_plane( _enc, &pipe, pli, notstart,
                                              notdone );
        }
        notstart = 1;
    }
...
static void oc_analyze_mb_mode_luma( oc_enc_ctx* _enc,
                                     oc_mode_choice* _modec,
                                     const oc_fr_state* _fr,
                                     const oc_qii_state* _qs,
                                     const unsigned _frag_satd[ 12 ],
    ...
    *&qs = *_qs;
    ...
    for ( bi = 0; bi < 4; bi++ ) {
        ...
        *( ft + 0 ) = *&fr;
        ...
        best_rate =
            oc_dct_cost2( &best_ssd, _enc->state.qis[ 0 ], 0, _qti, satd ) +
            ( qt[ 0 ].bits - qs.bits << OC_BIT_SCALE );
        best_cost = OC_MODE_RD_COST( ssd + best_ssd,
                                     rate + best_rate + best_overhead, lambda );
        best_fri = 0;
        best_qii = 0;
        for ( qii = 1; qii < nqis; qii++ ) {
            oc_qii_state_advance( qt + qii, &qs, qii );
            cur_rate = oc_dct_cost2( &cur_ssd, _enc->state.qis[ qii ], 0, _qti,
                                     satd ) +
                       ( qt[ qii ].bits - qs.bits << OC_BIT_SCALE );
            cur_cost = OC_MODE_RD_COST(
                ssd + cur_ssd, rate + cur_rate + best_overhead, lambda );
        ...
        if ( _skip_ssd[ bi ] < UINT_MAX && nskipped < 3 ) {
            *( ft + 1 ) = *&fr;
            ...
            cur_cost =
                OC_MODE_RD_COST( ssd + cur_ssd, rate + cur_overhead, lambda );
            if ( cur_cost <= best_cost ) {
                best_ssd = cur_ssd;
                best_rate = 0;
...
static void oc_analyze_mb_mode_chroma( oc_enc_ctx* _enc,
    ...
    ssd = _modec->ssd;
    rate = _modec->rate;
    /*Because (except in 4:4:4 mode) we aren't considering chroma blocks in
       coded order, we assume a constant overhead for coded block and qii
       flags.*/
    ...
    for ( pli = 1; pli < 3; pli++ ) {
        for ( ; bi < nblocks; bi++ ) {
            unsigned best_cost;
            satd = _frag_satd[ bi ];
            best_rate = oc_dct_cost2( &best_ssd, _enc->state.qis[ 0 ], pli,
                                      _qti, satd ) +
                        OC_CHROMA_QII_RATE;
            best_cost =
                OC_MODE_RD_COST( ssd + best_ssd, rate + best_rate, lambda );
            best_qii = 0;
            for ( qii = 1; qii < nqis; qii++ ) {
                cur_rate = oc_dct_cost2( &cur_ssd, _enc->state.qis[ qii ], 0,
                                         _qti, satd ) +
                           OC_CHROMA_QII_RATE;
                cur_cost =
                    OC_MODE_RD_COST( ssd + cur_ssd, rate + cur_rate, lambda );
                if ( cur_cost < best_cost ) {
                    best_cost = cur_cost;
                    best_ssd = cur_ssd;
                    best_rate = cur_rate;
...
static void oc_skip_cost( oc_enc_ctx* _enc,
                          oc_enc_pipeline_state* _pipe,
                          unsigned _mbi,
                          unsigned _ssd[ 12 ] ) {
    ...
    src = _enc->state.ref_frame_data[ OC_FRAME_IO ];
    ref = _enc->state
              .ref_frame_data[ _enc->state.ref_frame_idx[ OC_FRAME_PREV ] ];
    ystride = _enc->state.ref_ystride[ 0 ];
    frags = _enc->state.frags;
    frag_buf_offs = _enc->state.frag_buf_offs;
    sb_map = _enc->state.sb_maps[ _mbi >> 2 ][ _mbi & 3 ];
    dc_dequant =
        _enc->state.dequant_tables[ _enc->state.qis[ 0 ] ][ 0 ][ 1 ][ 0 ];
    for ( bi = 0; bi < 4; bi++ ) {
        fragi = sb_map[ bi ];
        frag_offs = frag_buf_offs[ fragi ];
        oc_enc_frag_sub( _enc, buffer, src + frag_offs, ref + frag_offs,
                         ystride );
        ...
        /*DC is a special case; if there's more than a full-quantizer
           improvement in the effective DC component, always force-code the
           block.*/
        dc_flag = abs( uncoded_dc ) > dc_dequant << 1;
        uncoded_ssd |= -dc_flag;
        _pipe->skip_ssd[ 0 ][ fragi - _pipe->froffset[ 0 ] ] = _ssd[ bi ] =
            uncoded_ssd;
    }
    ...
    for ( pli = 1; pli < 3; pli++ ) {
        ...
        dc_dequant =
            _enc->state.dequant_tables[ _enc->state.qis[ 0 ] ][ pli ][ 1 ][ 0 ];
        for ( ; mapii < map_nidxs; mapii++ ) {
            ...
            dc_flag = abs( uncoded_dc ) > dc_dequant << 1;
            uncoded_ssd |= -dc_flag;
            _pipe->skip_ssd[ pli ][ fragi - _pipe->froffset[ pli ] ] =
...
static void oc_mb_intra_satd( oc_enc_ctx* _enc,
                              unsigned _mbi,
...
static void oc_cost_intra( oc_enc_ctx* _enc,
                           oc_mode_choice* _modec,
                           unsigned _mbi,
                           const oc_fr_state* _fr,
                           const oc_qii_state* _qs,
    ...
    oc_analyze_mb_mode_chroma( _enc, _modec, _fr, _qs, _frag_satd, _skip_ssd,
                               0 );
    _modec->overhead +=
        oc_mode_scheme_chooser_cost( &_enc->chooser, OC_MODE_INTRA )
        << OC_BIT_SCALE;
    oc_mode_set_cost( _modec, _enc->lambda );
}

static void oc_cost_inter( oc_enc_ctx* _enc,
                           oc_mode_choice* _modec,
                           unsigned _mbi,
                           int _mb_mode,
                           const signed char* _mv,
                           const oc_fr_state* _fr,
                           const oc_qii_state* _qs,
                           const unsigned _skip_ssd[ 12 ] ) {
    ...
    if ( oc_state_get_mv_offsets( &_enc->state, mv_offs, 0, dx, dy ) > 1 ) {
        for ( bi = 0; bi < 4; bi++ ) {
            fragi = sb_map[ bi ];
            frag_offs = frag_buf_offs[ fragi ];
            frag_satd[ bi ] = oc_enc_frag_satd2_thresh(
                _enc, src + frag_offs, ref + frag_offs + mv_offs[ 0 ],
                ref + frag_offs + mv_offs[ 1 ], ystride, UINT_MAX );
        }
    } else {
        for ( bi = 0; bi < 4; bi++ ) {
            fragi = sb_map[ bi ];
            frag_offs = frag_buf_offs[ fragi ];
            frag_satd[ bi ] = oc_enc_frag_satd_thresh(
                _enc, src + frag_offs, ref + frag_offs + mv_offs[ 0 ], ystride,
                UINT_MAX );
        }
    }
    mb_map = ( const oc_mb_map_plane* )_enc->state.mb_maps[ _mbi ];
    ...
    if ( oc_state_get_mv_offsets( &_enc->state, mv_offs, 1, dx, dy ) > 1 ) {
        for ( mapii = 4; mapii < map_nidxs; mapii++ ) {
            ...
            fragi = mb_map[ pli ][ bi ];
            frag_offs = frag_buf_offs[ fragi ];
            frag_satd[ mapii ] = oc_enc_frag_satd2_thresh(
                _enc, src + frag_offs, ref + frag_offs + mv_offs[ 0 ],
                ref + frag_offs + mv_offs[ 1 ], ystride, UINT_MAX );
        }
    } else {
        for ( mapii = 4; mapii < map_nidxs; mapii++ ) {
            ...
            fragi = mb_map[ pli ][ bi ];
            frag_offs = frag_buf_offs[ fragi ];
            frag_satd[ mapii ] = oc_enc_frag_satd_thresh(
                _enc, src + frag_offs, ref + frag_offs + mv_offs[ 0 ], ystride,
                UINT_MAX );
        }
    }
    oc_analyze_mb_mode_luma( _enc, _modec, _fr, _qs, frag_satd, _skip_ssd, 1 );
    oc_analyze_mb_mode_chroma( _enc, _modec, _fr, _qs, frag_satd, _skip_ssd,
                               1 );
    _modec->overhead += oc_mode_scheme_chooser_cost( &_enc->chooser, _mb_mode )
                        << OC_BIT_SCALE;
    oc_mode_set_cost( _modec, _enc->lambda );
}

static void oc_cost_inter_nomv( oc_enc_ctx* _enc,
                                oc_mode_choice* _modec,
                                unsigned _mbi,
                                int _mb_mode,
                                const oc_fr_state* _fr,
    ...
    oc_cost_inter( _enc, _modec, _mbi, _mb_mode, OC_MV_ZERO, _fr, _qs,
                   _skip_ssd );
}

static int oc_cost_inter1mv( oc_enc_ctx* _enc,
                             oc_mode_choice* _modec,
                             unsigned _mbi,
                             int _mb_mode,
                             const signed char* _mv,
    ...
    _modec->overhead +=
        OC_MINI( _enc->mv_bits[ 0 ] + bits0, _enc->mv_bits[ 1 ] + 12 ) -
            OC_MINI( _enc->mv_bits[ 0 ], _enc->mv_bits[ 1 ] )
        << OC_BIT_SCALE;
    oc_mode_set_cost( _modec, _enc->lambda );
    return bits0;
}
...
static const unsigned char OC_MB_PHASE[ 4 ][ 4 ] = { { 0, 1, 3, 2 },
                                                     { 0, 3, 1, 2 },
                                                     { 0, 3, 1, 2 },
                                                     { 2, 3, 1, 0 } };

static void oc_cost_inter4mv( oc_enc_ctx* _enc,
                              oc_mode_choice* _modec,
                              unsigned _mbi,
                              oc_mv _mv[ 4 ],
                              const oc_fr_state* _fr,
    ...
    for ( bi = 0; bi < 4; bi++ ) {
        ...
        frag_offs = frag_buf_offs[ fragi ];
        if ( oc_state_get_mv_offsets( &_enc->state, mv_offs, 0, dx, dy ) > 1 ) {
            satd = oc_enc_frag_satd2_thresh(
                _enc, src + frag_offs, ref + frag_offs + mv_offs[ 0 ],
                ref + frag_offs + mv_offs[ 1 ], ystride, UINT_MAX );
        } else {
            satd = oc_enc_frag_satd_thresh( _enc, src + frag_offs,
                                            ref + frag_offs + mv_offs[ 0 ],
                                            ystride, UINT_MAX );
        }
    ...
    for ( bi = 0; bi < 4; bi++ ) {
        if ( _modec->qii[ OC_MB_PHASE[ _mbi & 3 ][ bi ] ] >= nqis ) {
            ...
            bits0 += OC_MV_BITS[ 0 ][ _mv[ bi ][ 0 ] + 31 ] +
                     OC_MV_BITS[ 0 ][ _mv[ bi ][ 1 ] + 31 ];
            bits1 += 12;
        }
    }
    ( *OC_SET_CHROMA_MVS_TABLE[ _enc->state.info.pixel_fmt ] )(
        cbmvs, ( const oc_mv* )lbmvs );
    map_idxs = OC_MB_MAP_IDXS[ _enc->state.info.pixel_fmt ];
    map_nidxs = OC_MB_MAP_NIDXS[ _enc->state.info.pixel_fmt ];
    /*Note: This assumes ref_ystride[1]==ref_ystride[2].*/
    ystride = _enc->state.ref_ystride[ 1 ];
    for ( mapii = 4; mapii < map_nidxs; mapii++ ) {
        ...
        dy = cbmvs[ bi ][ 1 ];
        frag_offs = frag_buf_offs[ fragi ];
        /*TODO: We could save half these calls by re-using the results for the
           Cb and Cr planes; is it worth it?*/
        if ( oc_state_get_mv_offsets( &_enc->state, mv_offs, pli, dx, dy ) >
             1 ) {
            satd = oc_enc_frag_satd2_thresh(
                _enc, src + frag_offs, ref + frag_offs + mv_offs[ 0 ],
                ref + frag_offs + mv_offs[ 1 ], ystride, UINT_MAX );
...
int oc_enc_analyze_inter( oc_enc_ctx* _enc, int _allow_keyframe, int _recode ) {
    ...
    for ( stripe_sby = 0; notdone; stripe_sby += mcu_nvsbs ) {
            ...
            for ( quadi = 0; quadi < 4; quadi++ )
                if ( sb_flags[ sbi ].quad_valid & 1 << quadi ) {
                    ...
                    /*Motion estimation:
                      We always do a basic 1MV search for all macroblocks, coded
                      or not, keyframe or not.*/
                    if ( !_recode && _enc->sp_level < OC_SP_LEVEL_NOMC )
                        oc_mcenc_search( _enc, mbi );
                    dx = dy = 0;
                    /*Find the block choice with the lowest estimated coding
                      cost. If a Cb or Cr block is coded but no Y' block from a
                      macro block then the mode MUST be OC_MODE_INTER_NOMV. This
                      is the default state to which the mode data structure is
                       initialised in encoder and decoder at the start of each
                      frame.*/
                    /*Block coding cost is estimated from correlated SATD
                     * metrics.*/
                    /*At this point, all blocks that are in frame are still
                     * marked coded.*/
                    if ( !_recode ) {
                        memcpy( embs[ mbi ].unref_mv,
                                embs[ mbi ].analysis_mv[ 0 ],
                                sizeof( embs[ mbi ].unref_mv ) );
                        embs[ mbi ].refined = 0;
                    }
                    ...
                    if ( _allow_keyframe ) {
                        oc_cost_intra( _enc, modes + OC_MODE_INTRA, mbi,
                                       pipe.fr + 0, &intra_luma_qs, intra_satd,
                                       OC_NOSKIP );
                        intrabits += modes[ OC_MODE_INTRA ].rate;
                        for ( bi = 0; bi < 4; bi++ ) {
                            oc_qii_state_advance(
                                &intra_luma_qs, &intra_luma_qs,
                    ...
                    oc_cost_inter_nomv( _enc, modes + OC_MODE_INTER_NOMV, mbi,
                                        OC_MODE_INTER_NOMV, pipe.fr + 0,
                                        pipe.qs + 0, skip_ssd );
                    if ( _enc->sp_level < OC_SP_LEVEL_NOMC ) {
                        oc_cost_intra( _enc, modes + OC_MODE_INTRA, mbi,
                                       pipe.fr + 0, pipe.qs + 0, intra_satd,
                                       skip_ssd );
                        mb_mv_bits_0 = oc_cost_inter1mv(
                            _enc, modes + OC_MODE_INTER_MV, mbi,
                            OC_MODE_INTER_MV,
                            embs[ mbi ].unref_mv[ OC_FRAME_PREV ], pipe.fr + 0,
                            pipe.qs + 0, skip_ssd );
                        oc_cost_inter( _enc, modes + OC_MODE_INTER_MV_LAST, mbi,
                                       OC_MODE_INTER_MV_LAST, last_mv,
                                       pipe.fr + 0, pipe.qs + 0, skip_ssd );
                        oc_cost_inter( _enc, modes + OC_MODE_INTER_MV_LAST2,
                                       mbi, OC_MODE_INTER_MV_LAST2, prior_mv,
                                       pipe.fr + 0, pipe.qs + 0, skip_ssd );
                        oc_cost_inter4mv( _enc, modes + OC_MODE_INTER_MV_FOUR,
                                          mbi, embs[ mbi ].block_mv,
                                          pipe.fr + 0, pipe.qs + 0, skip_ssd );
                        oc_cost_inter_nomv( _enc, modes + OC_MODE_GOLDEN_NOMV,
                                            mbi, OC_MODE_GOLDEN_NOMV,
                                            pipe.fr + 0, pipe.qs + 0,
                                            skip_ssd );
                        mb_gmv_bits_0 = oc_cost_inter1mv(
                            _enc, modes + OC_MODE_GOLDEN_MV, mbi,
                            OC_MODE_GOLDEN_MV,
                            embs[ mbi ].unref_mv[ OC_FRAME_GOLD ], pipe.fr + 0,
                            pipe.qs + 0, skip_ssd );
                        /*The explicit MV modes (2,6,7) have not yet gone
                          through halfpel refinement. We choose the explicit MV
                          mode that's already furthest ahead on R-D cost and
                          refine only that one. We have to be careful to
                          remember which ones we've refined so that we don't
                          refine it again if we re-encode this frame.*/
                        inter_mv_pref = _enc->lambda * 3;
                        if ( modes[ OC_MODE_INTER_MV_FOUR ].cost <
                                 modes[ OC_MODE_INTER_MV ].cost &&
                             modes[ OC_MODE_INTER_MV_FOUR ].cost <
                                 modes[ OC_MODE_GOLDEN_MV ].cost ) {
                            if ( !( embs[ mbi ].refined & 0x80 ) ) {
                                oc_mcenc_refine4mv( _enc, mbi );
                                embs[ mbi ].refined |= 0x80;
                            }
                            oc_cost_inter4mv(
                                _enc, modes + OC_MODE_INTER_MV_FOUR, mbi,
                                embs[ mbi ].ref_mv, pipe.fr + 0, pipe.qs + 0,
                                skip_ssd );
                        } else if ( modes[ OC_MODE_GOLDEN_MV ].cost +
                                        inter_mv_pref <
                                    modes[ OC_MODE_INTER_MV ].cost ) {
                        ...
                        mb_mv_bits_0 = oc_cost_inter1mv(
                            ...
                            pipe.fr + 0, pipe.qs + 0, skip_ssd );
                        /*Finally, pick the mode with the cheapest estimated R-D
                         * cost.*/
                        mb_mode = OC_MODE_INTER_NOMV;
                        if ( modes[ OC_MODE_INTRA ].cost <
                             modes[ OC_MODE_INTER_NOMV ].cost ) {
                            mb_mode = OC_MODE_INTRA;
                        }
                        if ( modes[ OC_MODE_INTER_MV_LAST ].cost <
                             modes[ mb_mode ].cost ) {
                            mb_mode = OC_MODE_INTER_MV_LAST;
                        }
                        if ( modes[ OC_MODE_INTER_MV_LAST2 ].cost <
                             modes[ mb_mode ].cost ) {
                            mb_mode = OC_MODE_INTER_MV_LAST2;
                        }
                        if ( modes[ OC_MODE_GOLDEN_NOMV ].cost <
                             modes[ mb_mode ].cost ) {
                            mb_mode = OC_MODE_GOLDEN_NOMV;
                        }
                        if ( modes[ OC_MODE_GOLDEN_MV ].cost <
                             modes[ mb_mode ].cost ) {
                        ...
                        /*We prefer OC_MODE_INTER_MV, but not over LAST and
                         * LAST2.*/
                        if ( mb_mode == OC_MODE_INTER_MV_LAST ||
                             mb_mode == OC_MODE_INTER_MV_LAST2 ) {
                            inter_mv_pref = 0;
                        }
                        if ( modes[ OC_MODE_INTER_MV ].cost <
                             modes[ mb_mode ].cost + inter_mv_pref ) {
                    ...
                    if ( mb_mode != OC_MODE_INTER_MV_FOUR ) {
                        switch ( mb_mode ) {
                            case OC_MODE_INTER_MV: {
                                dx =
                                    embs[ mbi ]
                                        .analysis_mv[ 0 ][ OC_FRAME_PREV ][ 0 ];
                                dy =
                                    embs[ mbi ]
                                        .analysis_mv[ 0 ][ OC_FRAME_PREV ][ 1 ];
                            } break;
                            ...
                            case OC_MODE_GOLDEN_MV: {
                                dx =
                                    embs[ mbi ]
                                        .analysis_mv[ 0 ][ OC_FRAME_GOLD ][ 0 ];
                                dy =
                                    embs[ mbi ]
                                        .analysis_mv[ 0 ][ OC_FRAME_GOLD ][ 1 ];
                            } break;
                        }
                        for ( bi = 0; bi < 4; bi++ ) {
                    ...
                    if ( oc_enc_mb_transform_quantize_luma(
                             _enc, &pipe, mbi,
                        ...
                        switch ( mb_mode ) {
                            case OC_MODE_INTER_MV: {
                                ...
                                /*If we're backing out from 4MV, find the MV
                                   we're actually using.*/
                                if ( orig_mb_mode == OC_MODE_INTER_MV_FOUR ) {
                                    for ( bi = 0;; bi++ ) {
                                        fragi = mb_maps[ mbi ][ 0 ][ bi ];
                                        if ( frags[ fragi ].coded ) {
                                            memcpy( last_mv, frag_mvs[ fragi ],
                                                    sizeof( last_mv ) );
                                            dx = frag_mvs[ fragi ][ 0 ];
                                            dy = frag_mvs[ fragi ][ 1 ];
                                            break;
                                    ...
                                    mb_mv_bits_0 = OC_MV_BITS[ 0 ][ dx + 31 ] +
                                                   OC_MV_BITS[ 0 ][ dy + 31 ];
                                }
                            ...
                            case OC_MODE_INTER_MV_FOUR: {
                                ...
                                for ( bi = 0; bi < 4; bi++ ) {
                                    ...
                                    if ( frags[ fragi ].coded ) {
                                        memcpy( last_mv, frag_mvs[ fragi ],
                                                sizeof( last_mv ) );
                                        memcpy( lbmvs[ bi ], frag_mvs[ fragi ],
                                                sizeof( lbmvs[ bi ] ) );
                                        _enc->mv_bits[ 0 ] +=
                                            OC_MV_BITS[ 0 ]
                                                      [ frag_mvs[ fragi ][ 0 ] +
                                    ...
                                    /*Replace the block MVs for not-coded blocks
                                       with (0,0).*/
                                    else
                                        memset( lbmvs[ bi ], 0,
                                                sizeof( lbmvs[ bi ] ) );
                                }
                                ( *set_chroma_mvs )( cbmvs,
                                                     ( const oc_mv* )lbmvs );
                                for ( mapii = 4; mapii < nmap_idxs; mapii++ ) {
                                    mapi = map_idxs[ mapii ];
                                    pli = mapi >> 2;
                                    ...
                                    frags[ fragi ].mb_mode = mb_mode;
                                    frags[ fragi ].qii =
                                        modes[ OC_MODE_INTER_MV_FOUR ]
                                            .qii[ mapii ];
                                    memcpy( frag_mvs[ fragi ], cbmvs[ bi ],
                                            sizeof( frag_mvs[ fragi ] ) );
                                }
                            } break;
                        }
                        coded_mbis[ ncoded_mbis++ ] = mbi;
                        oc_mode_scheme_chooser_update( &_enc->chooser,
                                                       mb_mode );
                        interbits +=
                            modes[ mb_mode ].rate + modes[ mb_mode ].overhead;
                    } else {
                    ...
                    /*Propagate final MB mode and MVs to the chroma blocks.
                      This has already been done for 4MV mode, since it requires
                      individual block motion vectors.*/
                    if ( mb_mode != OC_MODE_INTER_MV_FOUR ) {
                        for ( mapii = 4; mapii < nmap_idxs; mapii++ ) {
                            mapi = map_idxs[ mapii ];
                            pli = mapi >> 2;
                            ...
                            fragi = mb_maps[ mbi ][ pli ][ bi ];
                            frags[ fragi ].mb_mode = mb_mode;
                            /*If we switched from 4MV mode to INTER_MV mode,
                               then the qii values won't have been chosen with
                               the right MV, but it's probaby not worth
                               re-estimating them.*/
        ...
        /*Code chroma planes.*/
        for ( pli = 1; pli < 3; pli++ ) {
            oc_enc_sb_transform_quantize_chroma(
                _enc, &pipe, pli, pipe.sbi0[ pli ], pipe.sbi_end[ pli ] );
            oc_enc_pipeline_finish_mcu_plane( _enc, &pipe, pli, notstart,
                                              notdone );
        }
    ...
    if ( _allow_keyframe ) {
        ...
        interbits += OC_MINI( _enc->mv_bits[ 0 ], _enc->mv_bits[ 1 ] )
                     << OC_BIT_SCALE;
        interbits += _enc->chooser.scheme_bits[ _enc->chooser.scheme_list[ 0 ] ]
                     << OC_BIT_SCALE;
        if ( interbits > intrabits )
            return 1;
    }
    ...
    /*Compact the coded fragment list.*/
    {
        ...
        for ( pli = 1; pli < 3; pli++ ) {
            memmove(
                _enc->state.coded_fragis + ncoded_fragis,
                _enc->state.coded_fragis + _enc->state.fplanes[ pli ].froffset,
                _enc->state.ncoded_fragis[ pli ] *
                    sizeof( *_enc->state.coded_fragis ) );
    ...
    for ( pli = 0; pli < 3; pli++ ) {
        for ( qti = 0; qti < 2; qti++ ) {
            ...
            for ( bin = 0; bin < OC_SAD_BINS; bin++ ) {
                ...
                while ( ( bin1 < bin + 1 || fragw < OC_ZWEIGHT ) &&
                        bin1 < OC_SAD_BINS - 1 ) {
                    fragw +=
                        OC_MODE_METRICS[ _qi ][ pli ][ qti ][ bin1++ ].fragw;
                }
                while ( bin0 + 1 < bin && bin0 + 1 < bin1 &&
                        fragw - OC_MODE_METRICS[ _qi ][ pli ][ qti ][ bin0 ]
                                    .fragw >=
                            OC_ZWEIGHT ) {
                    fragw -=
                        OC_MODE_METRICS[ _qi ][ pli ][ qti ][ bin0++ ].fragw;
                }
                /*Merge statistics and fit lines.*/
                oc_mode_metrics_merge(
                    &metrics, OC_MODE_METRICS[ _qi ][ pli ][ qti ] + bin0,
                    bin1 - bin0 );
                if ( metrics.fragw > 0 && metrics.satd2 > 0 ) {
                    ...
                    /*Compute the points on these lines corresponding to the
                       actual bin value.*/
                    b = metrics.satdrate / metrics.satd2;
                    a = mrate - b * msatd;
                    rate =
                        ldexp( a + b * ( bin << OC_SAD_SHIFT ), OC_BIT_SCALE );
                    OC_MODE_RD[ _qi ][ pli ][ qti ][ bin ].rate =
                        ( ogg_int16_t )OC_CLAMPI( -32768, ( int )( rate + 0.5 ),
                                                  32767 );
                    b = metrics.satdrmse / metrics.satd2;
                    a = mrmse - b * msatd;
                    rmse =
                        ldexp( a + b * ( bin << OC_SAD_SHIFT ), OC_RMSE_SCALE );
                    OC_MODE_RD[ _qi ][ pli ][ qti ][ bin ].rmse =
                        ( ogg_int16_t )OC_CLAMPI( -32768, ( int )( rmse + 0.5 ),
                                                  32767 );
                }
            }
        }
...
void oc_enc_mode_metrics_collect( oc_enc_ctx* _enc ) {
    static const unsigned char OC_ZZI_HUFF_OFFSET[ 64 ] = {
        0,  16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 32, 32,
        32, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48,
    ...
    for ( pli = 0; pli < 3; pli++ ) {
        ...
        /*Set up token indices and eob run counts.
          We don't bother trying to figure out the real cost of the runs that
          span coefficients; instead we use the costs that were available when
          R-D token optimization was done.*/
        ...
        for ( ; fragii < ncoded_fragis; fragii++ ) {
            ...
            for ( zzi = 0; zzi < 64; ) {
                ...
                if ( eob_token[ zzi ] < OC_NDCT_EOB_TOKEN_MAX ) {
                    ...
                    frag_bits +=
                        _enc->huff_codes[ huffi ][ eob_token[ zzi ] ].nbits +
            ...
            oc_mode_metrics_add(
                OC_MODE_METRICS[ qi ][ pli ][ mb_mode != OC_MODE_INTRA ] + bin,
                fragw, satd, frag_bits << OC_BIT_SCALE,
                sqrt( frag_ssd[ fragi ] ) );
        }
    }
...
void oc_enc_mode_metrics_dump( oc_enc_ctx* _enc ) {
    ...
    fprintf( stdout,
             ...
             "/*The number of extra bits of precision at which to store RMSE "
             "metrics.\n"
