...
static int oc_make_dct_token_full( int _zzi, int _zzj, int _val, int* _eb ) {
    ...
    if ( zero_run > 0 ) {
        int adj;
        /*Implement a minor restriction on stack 1 so that we know during DC
           fixups that extending a dctrun token from stack 1 will never
           overflow.*/
        adj = _zzi != 1;
        if ( _val < 2 && zero_run < 17 + adj ) {
            ...
            if ( zero_run < 9 )
                token = OC_DCT_SHORT_ZRL_TOKEN;
            else
                token = OC_DCT_ZRL_TOKEN;
            eb = zero_run - 1;
        }
    } else if ( _val < 3 ) {
...
static const unsigned char OC_ZZI_HUFF_OFFSET[ 64 ] = {
    0,  16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 32, 32, 32, 48, 48, 48, 48,
    48, 48, 48, 48, 48, 48, 48, 48, 48, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64,
    64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64 };

static int oc_token_bits( oc_enc_ctx* _enc, int _huffi, int _zzi, int _token ) {
    return _enc->huff_codes[ _huffi + OC_ZZI_HUFF_OFFSET[ _zzi ] ][ _token ]
               .nbits +
           OC_DCT_TOKEN_EXTRA_BITS[ _token ];
}

static void oc_enc_tokenlog_checkpoint( oc_enc_ctx* _enc,
                                        oc_token_checkpoint* _cp,
                                        int _pli,
                                        int _zzi ) {
    _cp->pli = _pli;
    _cp->zzi = _zzi;
...
void oc_enc_tokenlog_rollback( oc_enc_ctx* _enc,
                               const oc_token_checkpoint* _stack,
                               int _n ) {
    int i;
    for ( i = _n; i-- > 0; ) {
        int pli;
...
int oc_enc_tokenize_ac( oc_enc_ctx* _enc,
                        int _pli,
                        ptrdiff_t _fragi,
                        ogg_int16_t* _qdct,
                        const ogg_uint16_t* _dequant,
                        const ogg_int16_t* _dct,
                        int _zzi,
    ...
    for ( zzi = OC_MINI( _zzi, 63 ); zzi > 0; zzi-- ) {
        ...
        if ( qc <= 1 ) {
            ...
            if ( !qc ) {
                /*Skip runs that are already quantized to zeros.
                  If we considered each zero coefficient in turn, we might
                   theoretically find a better way to partition long zero runs
                  (e.g., a run of > 17 zeros followed by a 1 might be better
                  coded as a short zero run followed by a combo token, rather
                  than the longer zero token followed by a 1 value token), but
                  zeros are so common that this becomes very computationally
                  expensive (quadratic instead of linear in the number of
                  coefficients), for a marginal gain.*/
                while ( zzi > 1 && !_qdct[ zzi - 1 ] )
                    zzi--;
                /*The distortion of coefficients originally quantized to zero is
                   treated as zero (since we'll never quantize them to anything
                   else).*/
                d2 = 0;
            } else {
                c = c + s ^ s;
            ...
            d2_accum[ zzi ] = sum_d2;
            flush_bits = eob > 0 ? oc_token_bits( _enc, huffi, zzi,
                                                  oc_make_eob_token( eob ) )
                                 : 0;
            /*We reserve 1 spot for combo run tokens that start in the 1st AC
               stack to ensure they can be extended to include the DC
               coefficient if necessary; this greatly simplifies stack-rewriting
               later on.*/
            dc_reserve = zzi + 62 >> 6;
            ...
            for ( ;; ) {
                if ( nzflags >> zzj & 1 ) {
                    ...
                    bits =
                        flush_bits + oc_token_bits( _enc, huffi, zzi, token );
                    ...
                    if ( nzeros < 16 + dc_reserve ) {
                        ...
                        if ( val <= 2 ) {
                            ...
                            if ( nzeros < 6 ) {
                                ...
                                eb = ( -val_s << cat + 2 ) + nzeros - 6 -
                                     ( cat << 2 );
                            }
                            e = ( _dct[ OC_FZIG_ZAG[ zzj ] ] + val_s ^ val_s ) -
                                _dequant[ zzj ];
                            d2 =
                                e * ( ogg_int32_t )e + sum_d2 - d2_accum[ zzj ];
                            bits = flush_bits +
                                   oc_token_bits( _enc, huffi, zzi, token );
                            cost =
                                d2 + lambda * bits + tokens[ zzk ][ tk ].cost;
                            if ( cost <= best_cost ) {
                                best_next = next;
                                best_token = token;
                                best_eb = eb;
                    ...
                    /*zzj can't be coded as a zero, so stop trying to extend the
                     * run.*/
                    if ( !( zflags >> zzj & 1 ) )
                        break;
                }
                /*We could try to consider _all_ potentially non-zero
                   coefficients, but if we already found a bunch of them not
                   worth coding, it's fairly unlikely they would now be worth
                   coding from this position; skipping them saves a lot of
                   work.*/
                zzj = ( tokens[ zzj ][ 0 ].next >> 1 ) -
                          ( tokens[ zzj ][ 0 ].qc != 0 ) &
                      63;
                if ( zzj == 0 ) {
                    /*We made it all the way to the end of the block; try an EOB
                     * token.*/
                    if ( eob < 4095 ) {
                        bits = oc_token_bits( _enc, huffi, zzi,
                                              oc_make_eob_token( eob + 1 ) ) -
                               flush_bits;
                    } else
                        bits = oc_token_bits( _enc, huffi, zzi,
                                              OC_DCT_EOB1_TOKEN );
                    cost = sum_d2 + bits * lambda;
                    /*If the best route so far is still a pure zero run to the
                      end of the block, force coding it as an EOB. Even if it's
                      not optimal for this block, it has a good chance of
                       getting combined with an EOB token from subsequent
                      blocks, saving bits overall.*/
                    if ( cost <= best_cost || best_token <= OC_DCT_ZRL_TOKEN &&
                                                  zzi + best_eb == 63 ) {
                        best_next = 0;
                        /*This token is just a marker; in reality we may not
                           emit any tokens, but update eob_run[] instead.*/
            ...
            if ( qc <= 2 ) {
                ...
                best_bits =
                    flush_bits + oc_token_bits( _enc, huffi, zzi, best_token );
    ...
    do {
        ...
        if ( tokens[ zzi ][ ti ].token < OC_NDCT_EOB_TOKEN_MAX ) {
            ...
            eob_run[ zzi ] = eob;
            /*We don't include the actual EOB cost for this block in the return
              value. It will be paid for by the fragment that terminates the EOB
              run.*/
            bits -= tokens[ zzi ][ ti ].bits;
            zzi = _zzi;
            break;
        ...
        oc_enc_token_log( _enc, _pli, zzi, tokens[ zzi ][ ti ].token,
                          tokens[ zzi ][ ti ].eb );
        next = tokens[ zzi ][ ti ].next;
        qc = tokens[ zzi ][ ti ].qc;
        zzj = ( next >> 1 ) - 1 & 63;
        /*TODO: It may be worth saving the dequantized coefficient in the
           trellis above; we had to compute it to measure the error anyway.*/
        _qdct[ dct_fzig_zag[ zzj ] ] =
            ( ogg_int16_t )( qc * ( int )_dequant[ zzj ] );
        zzi = next >> 1;
        ti = next & 1;
    } while ( zzi );
    *_stack = stack;
        ...
        if ( fragy == 0 ) {
            ...
            for ( fragx = 0; fragx < nhfrags; fragx++, fragi++ ) {
                if ( frags[ fragi ].coded ) {
                    ...
                    frag_dc[ fragi ] =
                        ( ogg_int16_t )( frags[ fragi ].dc - pred_last[ ref ] );
            ...
            for ( fragx = 0; fragx < nhfrags; fragx++, fragi++ ) {
                ...
                if ( frags[ fragi ].coded ) {
                    ...
                    frag_dc[ fragi ] =
                        ( ogg_int16_t )( frags[ fragi ].dc - pred );
                    pred_last[ ref ] = frags[ fragi ].dc;
                    l_ref = ref;
                } else
                    l_ref = -1;
...
void oc_enc_tokenize_dc_frag_list( oc_enc_ctx* _enc,
                                   int _pli,
                                   const ptrdiff_t* _coded_fragis,
                                   ptrdiff_t _ncoded_fragis,
                                   int _prev_ndct_tokens1,
                                   int _prev_eob_run1 ) {
    ...
    if ( eob_run1 > 0 )
        oc_enc_eob_log( _enc, _pli, 1, eob_run1 );
    ...
    for ( fragii = 0; fragii < _ncoded_fragis; fragii++ ) {
        ...
        if ( val ) {
            ...
            /*Zero DC value; that means the entry in stack 1 might need to be
              coded from stack 0. This requires a stack 1 fixup.*/
            if ( neobs1 > 0 ) {
                ...
                /*No active EOB run in stack 1, so we can't extend one in stack
                  0. Flush it if we've got it.*/
                if ( eob_run0 > 0 ) {
                    token = oc_make_eob_token_full( eob_run0, &eb );
                    dct_tokens0[ ti0 ] = ( unsigned char )token;
                    extra_bits0[ ti0 ] = ( ogg_uint16_t )eb;
                ...
                /*Stack 1 token is one of: a pure zero run token, a single
                  ...
                  A zero run token is expanded and moved to token stack 0, and
                  the stack 1 entry dropped. A single coefficient value may be
                  transformed into combo token that is moved to stack 0, or if
                  it cannot be combined, it is left alone and a single length-1
                  zero run is emitted in stack 0. A combo token is extended and
                  moved to stack 0. During AC coding, we restrict the run
                  lengths on combo tokens for stack 1 to guarantee we can extend
                  them.*/
                switch ( token1 ) {
                    ...
                    case OC_MINUS_ONE_TOKEN: {
                        ...
                        extra_bits0[ ti0 ] =
                            ( ogg_uint16_t )( token1 - OC_ONE_TOKEN );
                        ti0++;
                        /*Don't write the AC coefficient back out.*/
                    }
                        continue;
                    ...
                    case OC_MINUS_TWO_TOKEN: {
                        ...
                        extra_bits0[ ti0 ] =
                            ( ogg_uint16_t )( token1 - OC_TWO_TOKEN << 1 );
                        ti0++;
                        /*Don't write the AC coefficient back out.*/
                    }
                        continue;
                ...
                /*We can't merge tokens, write a short zero run and keep
                 * going.*/
                dct_tokens0[ ti0 ] = OC_DCT_SHORT_ZRL_TOKEN;
                extra_bits0[ ti0 ] = 0;
                ti0++;
        ...
        if ( !neobs1 ) {
            ...
            neobs1--;
            /*If we have more than 4095 EOBs outstanding in stack1, flush the
             * run.*/
...
void oc_enc_tokenize_finish( oc_enc_ctx* _enc ) {
    ...
    for ( pli = 0; pli < 3; pli++ )
        for ( zzi = 0; zzi < 64; zzi++ ) {
            ...
            if ( eob_run > 0 )
                oc_enc_eob_log( _enc, pli, zzi, eob_run );
        }
    ...
    for ( zzi = 0; zzi < 64; zzi++ )
        for ( pli = 0; pli < 3; pli++ ) {
            ...
            if ( old_tok1 >= OC_NDCT_EOB_TOKEN_MAX )
                continue;
            ...
            /*We can't possibly combine these into one run.
              It might be possible to split them more optimally, but we'll just
              leave them as-is.*/
            if ( run_count >= 4096 )
                continue;
