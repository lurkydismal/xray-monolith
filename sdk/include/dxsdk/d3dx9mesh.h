...
// Mesh options - lower 3 bytes only, upper byte used by _D3DXMESHOPT option
// flags
enum _D3DXMESH {
    D3DXMESH_32BIT = 0x001, // If set, then use 32 bit indices, if not set use
                            // 16 bit indices.
    D3DXMESH_DONOTCLIP = 0x002,    // Use D3DUSAGE_DONOTCLIP for VB & IB.
    D3DXMESH_POINTS = 0x004,       // Use D3DUSAGE_POINTS for VB & IB.
    D3DXMESH_RTPATCHES = 0x008,    // Use D3DUSAGE_RTPATCHES for VB & IB.
    D3DXMESH_NPATCHES = 0x4000,    // Use D3DUSAGE_NPATCHES for VB & IB.
    D3DXMESH_VB_SYSTEMMEM = 0x010, // Use D3DPOOL_SYSTEMMEM for VB. Overrides
                                   // D3DXMESH_MANAGEDVERTEXBUFFER
    D3DXMESH_VB_MANAGED = 0x020,   // Use D3DPOOL_MANAGED for VB.
    D3DXMESH_VB_WRITEONLY = 0x040, // Use D3DUSAGE_WRITEONLY for VB.
    D3DXMESH_VB_DYNAMIC = 0x080,   // Use D3DUSAGE_DYNAMIC for VB.
    D3DXMESH_VB_SOFTWAREPROCESSING =
        0x8000,                    // Use D3DUSAGE_SOFTWAREPROCESSING for VB.
    D3DXMESH_IB_SYSTEMMEM = 0x100, // Use D3DPOOL_SYSTEMMEM for IB. Overrides
                                   // D3DXMESH_MANAGEDINDEXBUFFER
    ...
    D3DXMESH_VB_SHARE = 0x1000, // Valid for Clone* calls only, forces cloned
                                // mesh/pmesh to share vertex buffer
    ...
    D3DXMESH_SOFTWAREPROCESSING = 0x18000, // D3DXMESH_VB_SOFTWAREPROCESSING |
                                           // D3DXMESH_IB_SOFTWAREPROCESSING
...
typedef enum _D3DXCLEANTYPE {
    ...
    // Helper options
    D3DXCLEAN_SKINNING = D3DXCLEAN_BACKFACING, // Bowtie cleaning modifies
                                               // geometry and breaks skinning
...
// These options are only valid for UVAtlasCreate and UVAtlasPartition, we may
// add more for UVAtlasPack if necessary D3DXUVATLAS_DEFAULT - Meshes with more
// than 25k faces go through fast, meshes with fewer than 25k faces go through
// quality D3DXUVATLAS_GEODESIC_FAST - Uses approximations to improve charting
// speed at the cost of added stretch or more charts.
// D3DXUVATLAS_GEODESIC_QUALITY - Provides better quality charts, but requires
// more time and memory than fast.
...
typedef enum _D3DXEFFECTDEFAULTTYPE {
    ...
    D3DXEDT_FLOATS = 0x2, // pValue points to an array of floats - number of
                          // floats is NumBytes / sizeof(float)
...
enum _D3DXWELDEPSILONSFLAGS {
    ...
    D3DXWELDEPSILONS_WELDPARTIALMATCHES =
        0x2, // if a given vertex component is within epsilon, modify partial
             // matched vertices so that both components identical AND if all
             // components "equal" remove one of the vertices
    D3DXWELDEPSILONS_DONOTREMOVEVERTICES =
        0x4, // instructs weld to only allow modifications to vertices and not
             // removal ONLY valid if D3DXWELDEPSILONS_WELDPARTIALMATCHES is set
             // useful to modify vertices to be equal, but not allow vertices to
             // be removed

    D3DXWELDEPSILONS_DONOTSPLIT =
        0x8, // instructs weld to specify the D3DXMESHOPT_DONOTSPLIT flag when
             // doing an Optimize(ATTR_SORT) if this flag is not set, all
             // vertices that are in separate attribute groups will remain split
             // and not welded.  Setting this flag can slow down software vertex
             // processing

};

typedef struct _D3DXWELDEPSILONS {
    FLOAT Position; // NOTE: This does NOT replace the epsilon in
                    // GenerateAdjacency in general, it should be the same value
                    // or greater than the one passed to GeneratedAdjacency
    FLOAT BlendWeights;
    FLOAT Normal;
    FLOAT PSize;
    FLOAT Specular;
...
DECLARE_INTERFACE_( ID3DXBaseMesh, IUnknown ) {
    ...
    STDMETHOD( GetDeclaration )(
        THIS_ D3DVERTEXELEMENT9 Declaration[ MAX_FVF_DECL_SIZE ] ) PURE;
    ...
    STDMETHOD( CloneMeshFVF )( THIS_ DWORD Options, DWORD FVF,
                               ...
                               LPD3DXMESH * ppCloneMesh ) PURE;
    STDMETHOD( CloneMesh )(
        THIS_ DWORD Options, CONST D3DVERTEXELEMENT9 * pDeclaration,
        LPDIRECT3DDEVICE9 pD3DDevice, LPD3DXMESH * ppCloneMesh ) PURE;
    ...
    STDMETHOD( GetAttributeTable )( THIS_ D3DXATTRIBUTERANGE * pAttribTable,
                                    DWORD * pAttribTableSize ) PURE;

    STDMETHOD( ConvertPointRepsToAdjacency )( THIS_ CONST DWORD * pPRep,
                                              DWORD * pAdjacency ) PURE;
    STDMETHOD( ConvertAdjacencyToPointReps )( THIS_ CONST DWORD * pAdjacency,
                                              DWORD * pPRep ) PURE;
...
DECLARE_INTERFACE_( ID3DXMesh, ID3DXBaseMesh ) {
    ...
    STDMETHOD( SetAttributeTable )(
        THIS_ CONST D3DXATTRIBUTERANGE * pAttribTable, DWORD cAttribTableSize )
        PURE;
};
...
DECLARE_INTERFACE_( ID3DXPMesh, ID3DXBaseMesh ) {
    ...
    STDMETHOD( ClonePMeshFVF )( THIS_ DWORD Options, DWORD FVF,
                                LPDIRECT3DDEVICE9 pD3DDevice,
                                LPD3DXPMESH * ppCloneMesh ) PURE;
    STDMETHOD( ClonePMesh )(
        THIS_ DWORD Options, CONST D3DVERTEXELEMENT9 * pDeclaration,
        LPDIRECT3DDEVICE9 pD3DDevice, LPD3DXPMESH * ppCloneMesh ) PURE;
    ...
    STDMETHOD( Save )( THIS_ IStream * pStream, CONST D3DXMATERIAL * pMaterials,
                       CONST D3DXEFFECTINSTANCE * pEffectInstances,
                       DWORD NumMaterials ) PURE;
    ...
    STDMETHOD( TrimByFaces )( THIS_ DWORD NewFacesMin, DWORD NewFacesMax,
                              DWORD * rgiFaceRemap, DWORD * rgiVertRemap ) PURE;
    STDMETHOD( TrimByVertices )( THIS_ DWORD NewVerticesMin,
                                 DWORD NewVerticesMax, DWORD * rgiFaceRemap,
                                 DWORD * rgiVertRemap ) PURE;

    STDMETHOD( GetAdjacency )( THIS_ DWORD * pAdjacency ) PURE;

    //  Used to generate the immediate "ancestor" for each vertex when it is
    //  removed by a vsplit.  Allows generation of geomorphs
    //     Vertex buffer must be equal to or greater than the maximum number of
    //     vertices in the pmesh
...
DECLARE_INTERFACE_( ID3DXSPMesh, IUnknown ) {
    ...
    STDMETHOD( CloneMeshFVF )( THIS_ DWORD Options, DWORD FVF,
                               LPDIRECT3DDEVICE9 pD3DDevice,
                               DWORD * pAdjacencyOut, DWORD * pVertexRemapOut,
                               LPD3DXMESH * ppCloneMesh ) PURE;
    STDMETHOD( CloneMesh )(
        THIS_ DWORD Options, CONST D3DVERTEXELEMENT9 * pDeclaration,
        LPDIRECT3DDEVICE9 pD3DDevice, DWORD * pAdjacencyOut,
        DWORD * pVertexRemapOut, LPD3DXMESH * ppCloneMesh ) PURE;
    STDMETHOD( ClonePMeshFVF )( THIS_ DWORD Options, DWORD FVF,
                                LPDIRECT3DDEVICE9 pD3DDevice,
                                DWORD * pVertexRemapOut, FLOAT * pErrorsByFace,
                                LPD3DXPMESH * ppCloneMesh ) PURE;
    STDMETHOD( ClonePMesh )(
        THIS_ DWORD Options, CONST D3DVERTEXELEMENT9 * pDeclaration,
        LPDIRECT3DDEVICE9 pD3DDevice, DWORD * pVertexRemapOut,
        FLOAT * pErrorsbyFace, LPD3DXPMESH * ppCloneMesh ) PURE;
    STDMETHOD( ReduceFaces )( THIS_ DWORD Faces ) PURE;
    STDMETHOD( ReduceVertices )( THIS_ DWORD Vertices ) PURE;
    STDMETHOD_( DWORD, GetMaxFaces )( THIS ) PURE;
    STDMETHOD_( DWORD, GetMaxVertices )( THIS ) PURE;
    STDMETHOD( GetVertexAttributeWeights )(
        THIS_ LPD3DXATTRIBUTEWEIGHTS pVertexAttributeWeights ) PURE;
...
// ID3DXMesh::Optimize options - upper byte only, lower 3 bytes used from
// _D3DXMESH option flags
enum _D3DXMESHOPT {
    ...
    D3DXMESHOPT_DONOTSPLIT =
        0x20000000, // do not split vertices shared between attribute groups
                    // when attribute sorting
    D3DXMESHOPT_DEVICEINDEPENDENT =
        0x00400000, // Only affects VCache.  uses a static known good cache size
                    // for all cards

    // D3DXMESHOPT_SHAREVB has been removed, please use D3DXMESH_VB_SHARE
    // instead
...
DECLARE_INTERFACE_( ID3DXPatchMesh, IUnknown ) {
    ...
    // This function returns the size of the tessellated mesh given a
    // tessellation level. This assumes uniform tessellation. For adaptive
    // tessellation the Adaptive parameter must be set to TRUE and
    // TessellationLevel should be the max tessellation. This will result in the
    // max mesh size necessary for adaptive tessellation.
    STDMETHOD( GetTessSize )( THIS_ FLOAT fTessLevel, DWORD Adaptive,
                              DWORD * NumTriangles, DWORD * NumVertices ) PURE;

    // GenerateAdjacency determines which patches are adjacent with provided
    // tolerance this information is used internally to optimize tessellation
    STDMETHOD( GenerateAdjacency )( THIS_ FLOAT Tolerance ) PURE;

    // CloneMesh Creates a new patchmesh with the specified decl, and converts
    // the vertex buffer to the new decl. Entries in the new decl which are new
    // are set to 0. If the current mesh has adjacency, the new mesh will also
    // have adjacency
    STDMETHOD( CloneMesh )( THIS_ DWORD Options,
                            CONST D3DVERTEXELEMENT9 * pDecl,
                            LPD3DXPATCHMESH * pMesh ) PURE;

    // Optimizes the patchmesh for efficient tessellation. This function is
    // designed to perform one time optimization for patch meshes that need to
    // be tessellated repeatedly by calling the Tessellate() method. The
    // optimization performed is independent of the actual tessellation level
    // used. Currently Flags is unused. If vertices are changed, Optimize must
    // be called again
    STDMETHOD( Optimize )( THIS_ DWORD flags ) PURE;

    // gets and sets displacement parameters
    // displacement maps can only be 2D textures MIP-MAPPING is ignored for non
    // adapative tessellation
    STDMETHOD( SetDisplaceParam )(
        THIS_ LPDIRECT3DBASETEXTURE9 Texture, D3DTEXTUREFILTERTYPE MinFilter,
        D3DTEXTUREFILTERTYPE MagFilter, D3DTEXTUREFILTERTYPE MipFilter,
        D3DTEXTUREADDRESS Wrap, DWORD dwLODBias ) PURE;

    STDMETHOD( GetDisplaceParam )(
        THIS_ LPDIRECT3DBASETEXTURE9 * Texture,
        D3DTEXTUREFILTERTYPE * MinFilter, D3DTEXTUREFILTERTYPE * MagFilter,
        D3DTEXTUREFILTERTYPE * MipFilter, D3DTEXTUREADDRESS * Wrap,
        DWORD * dwLODBias ) PURE;

    // Performs the uniform tessellation based on the tessellation level.
    // This function will perform more efficiently if the patch mesh has been
    // optimized using the Optimize() call.
    STDMETHOD( Tessellate )( THIS_ FLOAT fTessLevel, LPD3DXMESH pMesh ) PURE;

    // Performs adaptive tessellation based on the Z based adaptive tessellation
    // criterion. pTrans specifies a 4D vector that is dotted with the vertices
    // to get the per vertex adaptive tessellation amount. Each edge is
    // tessellated to the average of the criterion at the 2 vertices it
    // connects. MaxTessLevel specifies the upper limit for adaptive
    // tesselation. This function will perform more efficiently if the patch
    // mesh has been optimized using the Optimize() call.
    STDMETHOD( TessellateAdaptive )( THIS_ CONST D3DXVECTOR4 * pTrans,
                                     DWORD dwMaxTessLevel, DWORD dwMinTessLevel,
...
DECLARE_INTERFACE_( ID3DXSkinInfo, IUnknown ) {
    ...
    STDMETHOD( SetBoneVertexInfluence )(
        THIS_ DWORD boneNum, DWORD influenceNum, float weight ) PURE;
    STDMETHOD_( DWORD, GetNumBoneInfluences )( THIS_ DWORD bone ) PURE;
    STDMETHOD( GetBoneInfluence )( THIS_ DWORD bone, DWORD * vertices,
                                   FLOAT * weights ) PURE;
    STDMETHOD( GetBoneVertexInfluence )( THIS_ DWORD boneNum,
                                         DWORD influenceNum, float* pWeight,
                                         DWORD* pVertexNum ) PURE;
    STDMETHOD( GetMaxVertexInfluences )( THIS_ DWORD * maxVertexInfluences )
        PURE;
    STDMETHOD_( DWORD, GetNumBones )( THIS ) PURE;
    STDMETHOD( FindBoneVertexInfluenceIndex )(
        THIS_ DWORD boneNum, DWORD vertexNum, DWORD * pInfluenceIndex ) PURE;

    // This gets the max face influences based on a triangle mesh with the
    // specified index buffer
    STDMETHOD( GetMaxFaceInfluences )( THIS_ LPDIRECT3DINDEXBUFFER9 pIB,
                                       DWORD NumFaces,
                                       DWORD * maxFaceInfluences ) PURE;

    // Set min bone influence. Bone influences that are smaller than this are
    // ignored
    ...
    // Bone names are returned by D3DXLoadSkinMeshFromXof. They are not used by
    // any other method of this object
    STDMETHOD( SetBoneName )( THIS_ DWORD Bone, LPCSTR pName )
        PURE; // pName is copied to an internal string buffer
    STDMETHOD_( LPCSTR, GetBoneName )( THIS_ DWORD Bone )
        PURE; // A pointer to an internal string buffer is returned. Do not free
              // this.

    // Bone offset matrices are returned by D3DXLoadSkinMeshFromXof. They are
    // not used by any other method of this object
    STDMETHOD( SetBoneOffsetMatrix )( THIS_ DWORD Bone,
    ...
    STDMETHOD_( LPD3DXMATRIX, GetBoneOffsetMatrix )( THIS_ DWORD Bone )
        PURE; // A pointer to an internal matrix is returned. Do not free this.

    // Clone a skin info object
    STDMETHOD( Clone )( THIS_ LPD3DXSKININFO * ppSkinInfo ) PURE;

    // Update bone influence information to match vertices after they are
    // reordered. This should be called if the target vertex buffer has been
    // reordered externally.
    STDMETHOD( Remap )( THIS_ DWORD NumVertices, DWORD * pVertexRemap ) PURE;

    // These methods enable the modification of the vertex layout of the
    // vertices that will be skinned
    STDMETHOD( SetFVF )( THIS_ DWORD FVF ) PURE;
    STDMETHOD( SetDeclaration )( THIS_ CONST D3DVERTEXELEMENT9 * pDeclaration )
        PURE;
    STDMETHOD_( DWORD, GetFVF )( THIS ) PURE;
    STDMETHOD( GetDeclaration )(
        THIS_ D3DVERTEXELEMENT9 Declaration[ MAX_FVF_DECL_SIZE ] ) PURE;
    ...
    STDMETHOD( UpdateSkinnedMesh )(
        THIS_ CONST D3DXMATRIX * pBoneTransforms,
        CONST D3DXMATRIX * pBoneInvTransposeTransforms, LPCVOID pVerticesSrc,
        PVOID pVerticesDst ) PURE;

    // Takes a mesh and returns a new mesh with per vertex blend weights and a
    // bone combination table that describes which bones affect which subsets of
    // the mesh
    STDMETHOD( ConvertToBlendedMesh )(
        THIS_ LPD3DXMESH pMesh, DWORD Options, CONST DWORD * pAdjacencyIn,
        LPDWORD pAdjacencyOut, DWORD * pFaceRemap, LPD3DXBUFFER * ppVertexRemap,
        DWORD * pMaxFaceInfl, DWORD * pNumBoneCombinations,
        LPD3DXBUFFER * ppBoneCombinationTable, LPD3DXMESH * ppMesh ) PURE;

    // Takes a mesh and returns a new mesh with per vertex blend weights and
    // indices and a bone combination table that describes which bones palettes
    // affect which subsets of the mesh
    STDMETHOD( ConvertToIndexedBlendedMesh )(
        THIS_ LPD3DXMESH pMesh, DWORD Options, DWORD paletteSize,
        CONST DWORD * pAdjacencyIn, LPDWORD pAdjacencyOut, DWORD * pFaceRemap,
        LPD3DXBUFFER * ppVertexRemap, DWORD * pMaxVertexInfl,
        DWORD * pNumBoneCombinations, LPD3DXBUFFER * ppBoneCombinationTable,
...
HRESULT WINAPI
D3DXCreateSPMesh( LPD3DXMESH pMesh,
...
HRESULT WINAPI
D3DXGeneratePMesh( LPD3DXMESH pMesh,
                   CONST DWORD* pAdjacency,
                   CONST D3DXATTRIBUTEWEIGHTS* pVertexAttributeWeights,
                   CONST FLOAT* pVertexWeights,
                   DWORD MinValue,
...
HRESULT WINAPI
D3DXSimplifyMesh( LPD3DXMESH pMesh,
                  CONST DWORD* pAdjacency,
                  CONST D3DXATTRIBUTEWEIGHTS* pVertexAttributeWeights,
                  CONST FLOAT* pVertexWeights,
                  DWORD MinValue,
...
// Creates a skin info object based on the number of vertices, number of bones,
// and a declaration describing the vertex layout of the target vertices The
// bone names and initial bone transforms are not filled in the skin info object
// by this method.
...
// Creates a skin info object based on the number of vertices, number of bones,
// and a FVF describing the vertex layout of the target vertices The bone names
// and initial bone transforms are not filled in the skin info object by this
// method.
...
// This similar to D3DXLoadMeshFromXof, except also returns skinning info if
// present in the file If skinning info is not present, ppSkinInfo will be NULL
...
// The inverse of D3DXConvertTo{Indexed}BlendedMesh() functions. It figures out
// the skinning info from the mesh and the bone combination table and populates
// a skin info object with that data. The bone names and initial bone transforms
// are not filled in the skin info object by this method. This works with either
// a non-indexed or indexed blended mesh. It examines the FVF or declarator of
// the mesh to determine what type it is.
...
HRESULT WINAPI D3DXTessellateNPatches(
    ...
    CONST DWORD* pAdjacencyIn,
    FLOAT NumSegs,
    BOOL QuadraticInterpNormals, // if false use linear intrep for normals, if
                                 // true use quadratic
    LPD3DXMESH* ppMeshOut,
...
HRESULT WINAPI
D3DXRectPatchSize( CONST FLOAT* pfNumSegs, // segments for each edge (4)
...
HRESULT WINAPI
D3DXTriPatchSize( CONST FLOAT* pfNumSegs, // segments for each edge (3)
                  DWORD* pdwTriangles,    // output number of triangles
                  DWORD* pdwVertices );   // output number of vertices
...
HRESULT WINAPI
D3DXIntersect( LPD3DXBASEMESH pMesh,
               ...
               LPD3DXBUFFER* ppAllHits, // Array of D3DXINTERSECTINFOs for all
                                        // hits (not just closest)
...
HRESULT WINAPI D3DXIntersectSubset(
    ...
    FLOAT* pV,               // Barycentric Hit Coordinates
    FLOAT* pDist,            // Ray-Intersection Parameter Distance
    LPD3DXBUFFER* ppAllHits, // Array of D3DXINTERSECTINFOs for all hits (not
                             // just closest)
    DWORD* pCountOfHits );   // Number of entries in AllHits array
...
BOOL WINAPI
D3DXIntersectTri( CONST D3DXVECTOR3* p0,      // Triangle vertex 0 position
...
// and places the results in the TANGENT[TangentIndex] specified in the meshes'
// DECL puts the binorm in BINORM[BinormIndex] also specified in the decl.
//
...
//
//============================================================================
typedef HRESULT( WINAPI* LPD3DXUVATLASCB )( FLOAT fPercentDone,
...
//  pdwFalseEdgeAdjacency - a pointer to an array with 3 DWORDS per face,
//  indicating
//                          at each face, whether an edge is a false edge or not
//                          (using the same ordering as the adjacency data
//                          structure). If this is NULL, then it is assumed that
//                          there are no false edges. If not NULL, then a
//                          non-false edge is indicated by -1 and a false edge
//                          is indicated by any other value (it is not required,
//                          but it may be useful for the caller to use the
//                          original adjacency value). This allows you to
//                          parameterize a mesh of quads, and the edges down the
//                          middle of each quad will not be cut when
...
//
//  pStatusCallback - Since the atlas creation process can be very CPU
//  intensive,
//                    this allows the programmer to specify a function to be
//                    called periodically, similarly to how it is done in the
//                    PRT simulation engine.
...
//              mesh.
//  ppFacePartitioning - A pointer to a location to store a pointer for an
//  array,
//                       one DWORD per face, giving the final partitioning
//                       created by the atlasing algorithm.
//  ppVertexRemapArray - A pointer to a location to store a pointer for an
//  array,
...
//                   maximum number of charts was too low, this gives the
//                   minimum
...
// This has the same exact arguments as Create, except that it does not perform
// the final packing step. This method allows one to get a partitioning out, and
// possibly modify it before sending it to be repacked. Note that if you change
// the partitioning, you'll also need to calculate new texture coordinates for
// any faces that have switched charts.
//
...
// |_____|
//
// In order to make this equivalent to a disc, we would need to add a cut, and
...
// ppIMTData        - Where to store the buffer holding the IMT data
HRESULT WINAPI
D3DXComputeIMTFromSignal( LPD3DXMESH pMesh,
                          DWORD dwTextureIndex,
                          UINT uSignalDimension,
...
// ppIMTData        - Where to store the buffer holding the IMT data
HRESULT WINAPI
D3DXComputeIMTFromPerTexelSignal( LPD3DXMESH pMesh,
                                  DWORD dwTextureIndex,
                                  FLOAT* pfTexelSignal,
                                  UINT uWidth,
                                  UINT uHeight,
...
HRESULT WINAPI
D3DXConvertMeshSubsetToSingleStrip( LPD3DXBASEMESH MeshIn,
                                    DWORD AttribId,
                                    DWORD IBOptions,
...
HRESULT WINAPI
D3DXConvertMeshSubsetToStrips( LPD3DXBASEMESH MeshIn,
                               DWORD AttribId,
                               DWORD IBOptions,
                               LPDIRECT3DINDEXBUFFER9* ppIndexBuffer,
                               DWORD* pNumIndices,
...
//      The number stored for a given element is where in the new ordering
//        the vertex will have come from.  See ID3DXMesh::Optimize for more
//        info.
//
//============================================================================
...
typedef struct _D3DXSHMATERIAL {
    D3DCOLORVALUE Diffuse; // Diffuse albedo of the surface.  (Ignored if object
                           // is a Mirror)
    BOOL bMirror;  // Must be set to FALSE.  bMirror == TRUE not currently
                   // supported
    BOOL bSubSurf; // true if the object does subsurface scattering - can't do
                   // this and be a mirror
...
DECLARE_INTERFACE_( ID3DXPRTBuffer, IUnknown ) {
    ...
    STDMETHOD( LockBuffer )( THIS_ UINT Start, UINT NumSamples,
                             FLOAT * *ppData ) PURE;
    ...
    // extracts NumCoefficients coefficients into mesh - only applicable on
    // single channel buffers, otherwise just lockbuffer and copy data.  With
    // SHPRT data NumCoefficients should be Order^2
    STDMETHOD( ExtractToMesh )( THIS_ UINT NumCoefficients, D3DDECLUSAGE Usage,
                                UINT UsageIndexStart, LPD3DXMESH pScene ) PURE;
};
...
DECLARE_INTERFACE_( ID3DXPRTCompBuffer, IUnknown ) {
    ...
    STDMETHOD( ExtractPCA )( THIS_ UINT StartPCA, UINT NumExtract,
    ...
    STDMETHOD( ExtractToMesh )( THIS_ UINT NumPCA, D3DDECLUSAGE Usage,
                                UINT UsageIndexStart, LPD3DXMESH pScene ) PURE;
};
...
DECLARE_INTERFACE_( ID3DXTextureGutterHelper, IUnknown ) {
    ...
    STDMETHOD( ApplyGuttersFloat )( THIS_ FLOAT * pDataIn, UINT NumCoeffs,
    ...
    //
    // Usage would generally be D3DDECLUSAGE_TEXCOORD  and UsageIndex other than
    // zero
    STDMETHOD( ResampleTex )( THIS_ LPDIRECT3DTEXTURE9 pTextureIn,
                              LPD3DXMESH pMeshIn, D3DDECLUSAGE Usage,
                              UINT UsageIndex, LPDIRECT3DTEXTURE9 pTextureOut );
...
DECLARE_INTERFACE_( ID3DXPRTEngine, IUnknown ) {
    ...
    // bSetAlbedo sets albedo from material if TRUE - which clobbers per
    // texel/vertex
    //  albedo that might have been set before.  FALSE won't clobber.
    // fLengthScale is used for subsurface scattering - scene is mapped into a
    // 1mm unit cube
    //  and scaled by this amount
    STDMETHOD( SetMeshMaterials )( THIS_ CONST D3DXSHMATERIAL * *ppMaterials,
                                   UINT NumMeshes, UINT NumChannels,
                                   BOOL bSetAlbedo, FLOAT fLengthScale ) PURE;

    // setting albedo per-vertex or per-texel over rides the albedos stored per
    // mesh but it does not over ride any other settings

    // sets an albedo to be used per vertex - the albedo is represented as a
    // float pDataIn input pointer (pointint to albedo of 1st sample)
    // NumChannels 1 implies "grayscale" materials, set this to 3 to enable
    //  color bleeding effects
    // Stride - stride in bytes to get to next samples albedo
    STDMETHOD( SetPerVertexAlbedo )( THIS_ CONST VOID * pDataIn,
                                     UINT NumChannels, UINT Stride ) PURE;

    // represents the albedo per-texel instead of per-vertex (even if per-vertex
    // PRT is used) pAlbedoTexture - texture that stores the albedo (dimension
    // arbitrary) NumChannels 1 implies "grayscale" materials, set this to 3 to
    // enable
    //  color bleeding effects
    // pGH - optional gutter helper, otherwise one is constructed in computation
    // routines and
    ...
    STDMETHOD( GetVertexAlbedo )( THIS_ D3DXCOLOR * pVertColors, UINT NumVerts )
        PURE;

    // If pixel PRT is being computed normals default to ones that are
    // interpolated from the vertex normals.  This specifies a texture that
    // stores an object space normal map instead (must use a texture format that
    // can represent signed values) pNormalTexture - normal map, must be same
    // dimensions as PRTBuffers, signed
    STDMETHOD( SetPerTexelNormal )( THIS_ LPDIRECT3DTEXTURE9 pNormalTexture )
        PURE;
    ...
    // NumChannels 1 implies "grayscale" materials, set this to 3 to enable
    //  color bleeding effects
    STDMETHOD( ExtractPerVertexAlbedo )(
        THIS_ LPD3DXMESH pMesh, D3DDECLUSAGE Usage, UINT NumChannels ) PURE;

    // Resamples the input buffer into the output buffer
    // can be used to move between per-vertex and per-texel buffers.  This can
    // also be used to convert single channel buffers to 3-channel buffers and
    // vice-versa.
    STDMETHOD( ResampleBuffer )( THIS_ LPD3DXPRTBUFFER pBufferIn,
                                 LPD3DXPRTBUFFER pBufferOut ) PURE;

    // Returns the scene mesh - including modifications from adaptive spatial
    // sampling The returned mesh only has positions, normals and texture
    // coordinates (if defined) pD3DDevice - d3d device that will be used to
    // allocate the mesh pFaceRemap - each face has a pointer back to the face
    // on the original mesh that it comes from
    //  if the face hasn't been subdivided this will be an identity mapping
    // pVertRemap - each vertex contains 3 vertices that this is a linear
    // combination of pVertWeights - weights for each of above indices (sum
    // to 1.0f) ppMesh - mesh that will be allocated and filled
    STDMETHOD( GetAdaptedMesh )(
        THIS_ LPDIRECT3DDEVICE9 pD3DDevice, UINT * pFaceRemap,
        UINT * pVertRemap, FLOAT * pfVertWeights, LPD3DXMESH * ppMesh ) PURE;

    // Number of vertices currently allocated (includes new vertices from
    // adaptive sampling)
    ...
    // Sets the Minimum/Maximum intersection distances, this can be used to
    // control maximum distance that objects can shadow/reflect light, and help
    // with "bad" art that might have near features that you don't want to
    // shadow.  This does not apply for GPU simulations.
    //  fMin - minimum intersection distance, must be positive and less than
    //  fMax fMax - maximum intersection distance, if 0.0f use the previous
    //  value, otherwise
    ...
    STDMETHOD( RobustMeshRefine )( THIS_ FLOAT MinEdgeLength, UINT MaxSubdiv )
        PURE;

    // This sets to sampling information used by the simulator.  Adaptive
    // sampling parameters are currently ignored. NumRays - number of rays to
    // shoot per sample UseSphere - if TRUE uses spherical samples, otherwise
    // samples over
    //  the hemisphere.  Should only be used with GPU and Vol computations
    // UseCosine - if TRUE uses a cosine weighting - not used for Vol
    // computations
    ...
    STDMETHOD( SetSamplingInfo )( THIS_ UINT NumRays, BOOL UseSphere,
                                  BOOL UseCosine, BOOL Adaptive,
    ...
    //  will default to 4
    // pDataOut - PRT buffer that is generated.  Can be single channel.
    STDMETHOD( ComputeDirectLightingSHAdaptive )(
        THIS_ UINT SHOrder, FLOAT AdaptiveThresh, FLOAT MinEdgeLength,
        UINT MaxSubdiv, LPD3DXPRTBUFFER pDataOut ) PURE;
    ...
    // ZAngleBias - scaled by one minus cosine of angle with light (offset in
    // depth) pDataOut - PRT buffer that is filled in.  Can be single channel
    STDMETHOD( ComputeDirectLightingSHGPU )(
        THIS_ LPDIRECT3DDEVICE9 pD3DDevice, UINT Flags, UINT SHOrder,
        FLOAT ZBias, FLOAT ZAngleBias, LPD3DXPRTBUFFER pDataOut ) PURE;
    ...
    STDMETHOD( ComputeSSAdaptive )( THIS_ LPD3DXPRTBUFFER pDataIn,
                                    FLOAT AdaptiveThresh, FLOAT MinEdgeLength,
                                    UINT MaxSubdiv, LPD3DXPRTBUFFER pDataOut,
                                    LPD3DXPRTBUFFER pDataTotal ) PURE;
    ...
    // pDataOut - PRT buffer that is generated
    // pDataTotal - [optional] can be used to keep a running sum
    STDMETHOD( ComputeBounceAdaptive )(
        THIS_ LPD3DXPRTBUFFER pDataIn, FLOAT AdaptiveThresh,
        FLOAT MinEdgeLength, UINT MaxSubdiv, LPD3DXPRTBUFFER pDataOut,
        LPD3DXPRTBUFFER pDataTotal ) PURE;
    ...
    STDMETHOD( ComputeVolumeSamplesDirectSH )(
        THIS_ UINT SHOrderIn, UINT SHOrderOut, UINT NumVolSamples,
        CONST D3DXVECTOR3 * pSampleLocs, LPD3DXPRTBUFFER pDataOut ) PURE;
    ...
    // pSampleLocs    - position of sample locations
    // pDataOut       - PRT Buffer that will store output results
    STDMETHOD( ComputeVolumeSamples )(
        THIS_ LPD3DXPRTBUFFER pSurfDataIn, UINT SHOrder, UINT NumVolSamples,
        CONST D3DXVECTOR3 * pSampleLocs, LPD3DXPRTBUFFER pDataOut ) PURE;
    ...
    // pDataOut     - PRT Buffer that will store output results
    STDMETHOD( ComputeSurfSamplesDirectSH )(
        THIS_ UINT SHOrder, UINT NumSamples, CONST D3DXVECTOR3 * pSampleLocs,
        CONST D3DXVECTOR3 * pSampleNorms, LPD3DXPRTBUFFER pDataOut ) PURE;

    // given the solution for PRT or light maps, computes transfer vector at
    // arbitrary position/normal pairs in space
    //
    ...
    STDMETHOD( ComputeSurfSamplesBounce )(
        THIS_ LPD3DXPRTBUFFER pSurfDataIn, UINT NumSamples,
        CONST D3DXVECTOR3 * pSampleLocs, CONST D3DXVECTOR3 * pSampleNorms,
        LPD3DXPRTBUFFER pDataOut, LPD3DXPRTBUFFER pDataTotal ) PURE;

    // Frees temporary data structures that can be created for subsurface
    // scattering this data is freed when the PRTComputeEngine is freed and is
    // lazily created
    STDMETHOD( FreeSSData )( THIS ) PURE;

    // Frees temporary data structures that can be created for bounce
    // simulations this data is freed when the PRTComputeEngine is freed and is
    // lazily created
    STDMETHOD( FreeBounceData )( THIS ) PURE;

    // This computes the Local Deformable PRT (LDPRT) coefficients relative to
    // the per sample normals that minimize error in a least squares sense with
    // respect to the input PRT data set.  These coefficients can be used with
    // skinned/transformed normals to model global effects with dynamic objects.
    // Shading normals can optionally be solved for - these normals (along with
    // the LDPRT coefficients) can more accurately represent the PRT signal. The
    // coefficients are for zonal harmonics oriented in the normal/shading
    // normal direction.
    //
    ...
    //             these normals.  This array must be the same size as the
    //             number of samples in pDataIn
    // pDataOut - Output buffer (SHOrder zonal harmonic coefficients per channel
    // per sample)
    STDMETHOD( ComputeLDPRTCoeffs )( THIS_ LPD3DXPRTBUFFER pDataIn,
                                     UINT SHOrder, D3DXVECTOR3 * pNormOut,
                                     LPD3DXPRTBUFFER pDataOut ) PURE;
    ...
    STDMETHOD( ScaleMeshChunk )( THIS_ UINT uMeshChunk, FLOAT fScale,
                                 LPD3DXPRTBUFFER pDataOut ) PURE;

    // mutliplies each PRT vector by the albedo - can be used if you want to
    // have the albedo burned into the dataset, often better not to do this.  If
    // this is not done the user must mutliply the albedo themselves when
    // rendering - just multiply the albedo times the result of the PRT dot
    // product. If pDataOut is a texture simulation result and there is an
    // albedo texture it must be represented at the same resolution as the
    // simulation buffer.  You can use LoadSurfaceFromSurface and set a new
    // albedo texture if this is an issue - but must be careful about how the
    // gutters are handled.
    //
    ...
    STDMETHOD( SetCallBack )( THIS_ LPD3DXSHPRTSIMCB pCB, FLOAT Frequency,
                              LPVOID lpUserContext ) PURE;

    // Returns TRUE if the ray intersects the mesh, FALSE if it does not.  This
    // function takes into account settings from SetMinMaxIntersection.  If the
    // closest intersection is not needed this function is more efficient
    // compared to the ClosestRayIntersection method. pRayPos - origin of ray
    // pRayDir - normalized ray direction (normalization required for SetMinMax
    // to be meaningful)

    STDMETHOD_( BOOL, ShadowRayIntersects )( THIS_ CONST D3DXVECTOR3 * pRayPos,
                                             CONST D3DXVECTOR3 * pRayDir ) PURE;

    // Returns TRUE if the ray intersects the mesh, FALSE if it does not.  If
    // there is an intersection the closest face that was intersected and its
    // first two barycentric coordinates are returned.  This function takes into
    // account settings from SetMinMaxIntersection. This is a slower function
    // compared to ShadowRayIntersects and should only be used where needed. The
    // third vertices barycentric coordinates will be 1 - pU - pV. pRayPos     -
    // origin of ray pRayDir     - normalized ray direction (normalization
    // required for SetMinMax to be meaningful) pFaceIndex  - Closest face that
    // intersects.  This index is based on stacking the pBlockerMesh
    ...
    STDMETHOD_( BOOL, ClosestRayIntersects )(
...
//  Parameters:
//    D3DXSHCOMPRESSQUALITYTYPE
//      Quality of compression - low is faster (computes PCA per voronoi
//      cluster) high is slower but better quality (clusters based on distance
//      to affine subspace)
//    NumClusters
...
//  transform each sub mesh or its texture coordinates.  If no DECL is given it
//  will generate a union of all of the DECL's of the sub meshes, promoting
//  channels and types if neccesary.  It will create an AttributeTable if
//  possible, one can call OptimizeMesh with attribute sort and compacting
//  enabled to ensure this.
//
//  Parameters:
//    ppMeshes
...
//   ppIBData
//      LPD3DXBUFFER holds raw index buffer that will contain the resulting
//      split faces. Format determined by bIBIs32Bit.  Allocated by function
//   pIBDataLength
...
//      16-bit is assumed)
//   ppFaceRemap
//      LPD3DXBUFFER mapping of each face in ppIBData to original faces.  Length
//      is *pIBDataLength/3.  Optional paramter, allocated in function
//   ppVertData
//      LPD3DXBUFFER contains new vertex data structure.  Size of
//      pVertDataLength
//   pVertDataLength
...
HRESULT WINAPI
D3DXSHPRTCompSplitMeshSC( UINT* pClusterIDs,
                          UINT NumVertices,
                          UINT NumClusters,
                          UINT* pSuperClusterIDs,
