...
// D3DX10 and D3DX9 math look the same. You can include either one into your
// project. We are intentionally using the header define from D3DX9 math to
...
#define D3DX_16F_EPSILON \
    4.8875809e-4f                  // smallest such that 1.0 + epsilon != 1.0
...
public:
    ...
    D3DXVECTOR3_16F( CONST D3DXFLOAT16& x,
                     CONST D3DXFLOAT16& y,
                     CONST D3DXFLOAT16& z );
...
public:
    ...
    D3DXVECTOR4_16F( CONST D3DXFLOAT16& x,
                     CONST D3DXFLOAT16& y,
                     CONST D3DXFLOAT16& z,
                     CONST D3DXFLOAT16& w );
...
D3DXMATRIX* WINAPI
D3DXMatrixTransformation( D3DXMATRIX* pOut,
...
// Build 2D transformation matrix in XY plane.  NULL arguments are treated as
// identity. Mout = Msc-1 * Msr-1 * Ms * Msr * Msc * Mrc-1 * Mr * Mrc * Mt
D3DXMATRIX* WINAPI
D3DXMatrixTransformation2D( D3DXMATRIX* pOut,
...
D3DXMATRIX* WINAPI
D3DXMatrixAffineTransformation( D3DXMATRIX* pOut,
                                FLOAT Scaling,
                                CONST D3DXVECTOR3* pRotationCenter,
                                CONST D3DXQUATERNION* pRotation,
                                CONST D3DXVECTOR3* pTranslation );

// Build 2D affine transformation matrix in XY plane.  NULL arguments are
// treated as identity. Mout = Ms * Mrc-1 * Mr * Mrc * Mt
D3DXMATRIX* WINAPI
D3DXMatrixAffineTransformation2D( D3DXMATRIX* pOut,
                                  FLOAT Scaling,
                                  CONST D3DXVECTOR2* pRotationCenter,
                                  FLOAT Rotation,
                                  CONST D3DXVECTOR2* pTranslation );
...
D3DXMATRIX* WINAPI
D3DXMatrixOrthoRH( D3DXMATRIX* pOut, FLOAT w, FLOAT h, FLOAT zn, FLOAT zf );
...
// Transform an array of planes by a matrix.  The vectors (a,b,c) must be
// normal. M should be the inverse transpose of the transformation desired.
...
//  coefficients.  In general the product of two SH functions of order O
//  generates and SH function of order 2*O - 1, but we truncate the result. This
//  means that the product commutes (f*g == g*f) but doesn't associate (f*(g*h)
//  != (f*g)*h.
//
//  Parameters:
//   pOut
...
__out_ecount( 4 ) FLOAT* WINAPI
    D3DXSHMultiply2( __out_ecount( 4 ) FLOAT* pOut,
                     __in_ecount( 4 ) CONST FLOAT* pF,
                     __in_ecount( 4 ) CONST FLOAT* pG );
__out_ecount( 9 ) FLOAT* WINAPI
    D3DXSHMultiply3( __out_ecount( 9 ) FLOAT* pOut,
                     __in_ecount( 9 ) CONST FLOAT* pF,
                     __in_ecount( 9 ) CONST FLOAT* pG );
__out_ecount( 16 ) FLOAT* WINAPI
    D3DXSHMultiply4( __out_ecount( 16 ) FLOAT* pOut,
                     __in_ecount( 16 ) CONST FLOAT* pF,
                     __in_ecount( 16 ) CONST FLOAT* pG );
__out_ecount( 25 ) FLOAT* WINAPI
    D3DXSHMultiply5( __out_ecount( 25 ) FLOAT* pOut,
                     __in_ecount( 25 ) CONST FLOAT* pF,
                     __in_ecount( 25 ) CONST FLOAT* pG );
__out_ecount( 36 ) FLOAT* WINAPI
    D3DXSHMultiply6( __out_ecount( 36 ) FLOAT* pOut,
                     __in_ecount( 36 ) CONST FLOAT* pF,
                     __in_ecount( 36 ) CONST FLOAT* pG );
...
//  unit the resulting exit radiance of a point directly under the light
//  oriented in the cone direction on a diffuse object with an albedo of 1 would
//  be 1.0. This will compute 3 spectral samples, pROut has to be specified,
//  while pGout and pBout are optional.
//
//  Parameters:
//   Order
//      Order of the SH evaluation, generates Order^2 coefs, degree is Order-1
