 ...
 * CoCreateInstance be used to create DirectPlay8 address objects.
 *
 * HRESULT WINAPI DirectPlay8AddressCreate( const GUID * pcIID, void
...
DECLARE_INTERFACE_( IDirectPlay8Address, IUnknown ) {
    ...
    STDMETHOD( GetUserData )( THIS_ void* pvUserData, PDWORD pdwBufferSize )
        PURE;
    STDMETHOD( SetSP )( THIS_ const GUID* const pguidSP ) PURE;
    STDMETHOD( SetUserData )( THIS_ const void* const pvUserData,
                              const DWORD dwDataSize ) PURE;
    ...
    STDMETHOD( GetComponentByName )( THIS_ const WCHAR* const pwszName,
                                     ...
                                     PDWORD pdwDataType ) PURE;
    STDMETHOD( GetComponentByIndex )(
        THIS_ const DWORD dwComponentID, WCHAR* pwszName, PDWORD pdwNameLen,
        void* pvBuffer, PDWORD pdwBufferSize, PDWORD pdwDataType ) PURE;
    STDMETHOD( AddComponent )(
        THIS_ const WCHAR* const pwszName, const void* const lpvData,
        const DWORD dwDataSize, const DWORD dwDataType ) PURE;
    STDMETHOD( GetDevice )( THIS_ GUID* ) PURE;
    STDMETHOD( SetDevice )( THIS_ const GUID* const ) PURE;
    STDMETHOD( BuildFromDPADDRESS )( THIS_ LPVOID pvAddress, DWORD dwDataSize )
        PURE;
};
...
DECLARE_INTERFACE_( IDirectPlay8AddressIP, IUnknown ) {
    ...
    STDMETHOD( BuildAddress )( THIS_ const WCHAR* const wszAddress,
                               const USHORT usPort ) PURE;

    // Builds a local TCP Address
    STDMETHOD( BuildLocalAddress )( THIS_ const GUID* const pguidAdapter,
                                    const USHORT usPort ) PURE;
    ...
    STDMETHOD( GetLocalAddress )( THIS_ GUID * pguidAdapter, USHORT * pusPort )
        PURE;

    // Gets the remote address
    STDMETHOD( GetAddress )( THIS_ WCHAR * wszAddress, PDWORD pdwAddressLength,
                             USHORT * psPort ) PURE;
};
...
#define IDirectPlay8Address_GetUserData( p, a, b ) \
    ( p )->lpVtbl->GetUserData( p, a, b )
#define IDirectPlay8Address_SetSP( p, a ) ( p )->lpVtbl->SetSP( p, a )
#define IDirectPlay8Address_SetUserData( p, a, b ) \
    ( p )->lpVtbl->SetUserData( p, a, b )
#define IDirectPlay8Address_GetNumComponents( p, a ) \
    ( p )->lpVtbl->GetNumComponents( p, a )
#define IDirectPlay8Address_GetComponentByName( p, a, b, c, d ) \
    ( p )->lpVtbl->GetComponentByName( p, a, b, c, d )
#define IDirectPlay8Address_GetComponentByIndex( p, a, b, c, d, e, f ) \
...
#define IDirectPlay8Address_BuildFromDirectPlay4Address( p, a, b ) \
    ( p )->lpVtbl->BuildFromDirectPlay4Address( p, a, b )

#define IDirectPlay8AddressIP_QueryInterface( p, a, b ) \
    ( p )->lpVtbl->QueryInterface( p, a, b )
#define IDirectPlay8AddressIP_AddRef( p ) ( p )->lpVtbl->AddRef( p )
#define IDirectPlay8AddressIP_Release( p ) ( p )->lpVtbl->Release( p )
#define IDirectPlay8AddressIP_BuildFromSockAddr( p, a ) \
    ( p )->lpVtbl->BuildFromSockAddr( p, a )
#define IDirectPlay8AddressIP_BuildAddress( p, a, b ) \
    ( p )->lpVtbl->BuildAddress( p, a, b )
#define IDirectPlay8AddressIP_BuildLocalAddress( p, a, b ) \
    ( p )->lpVtbl->BuildLocalAddress( p, a, b )
#define IDirectPlay8AddressIP_GetSockAddress( p, a, b ) \
    ( p )->lpVtbl->GetSockAddress( p, a, b )
#define IDirectPlay8AddressIP_GetLocalAddress( p, a, b ) \
    ( p )->lpVtbl->GetLocalAddress( p, a, b )
#define IDirectPlay8AddressIP_GetAddress( p, a, b, c ) \
    ( p )->lpVtbl->GetAddress( p, a, b, c )
...
#define IDirectPlay8AddressIP_BuildFromSockAddr( p, a ) \
    ( p )->BuildFromSockAddr( a )
#define IDirectPlay8AddressIP_BuildAddress( p, a, b ) \
    ( p )->BuildAddress( a, b )
#define IDirectPlay8AddressIP_BuildLocalAddress( p, a, b ) \
    ( p )->BuildLocalAddress( a, b )
#define IDirectPlay8AddressIP_GetSockAddress( p, a, b ) \
    ( p )->GetSockAddress( a, b )
#define IDirectPlay8AddressIP_GetLocalAddress( p, a, b ) \
    ( p )->GetLocalAddress( a, b )
#define IDirectPlay8AddressIP_GetAddress( p, a, b, c ) \
    ( p )->GetAddress( a, b, c )
