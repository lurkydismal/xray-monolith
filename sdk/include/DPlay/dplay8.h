...
#define DPNCANCEL_PLAYER_SENDS_PRIORITY_HIGH \
    ( DPNCANCEL_PLAYER_SENDS | 0x00010000 )
#define DPNCANCEL_PLAYER_SENDS_PRIORITY_NORMAL \
    ( DPNCANCEL_PLAYER_SENDS | 0x00020000 )
#define DPNCANCEL_PLAYER_SENDS_PRIORITY_LOW \
    ( DPNCANCEL_PLAYER_SENDS | 0x00040000 )
...
typedef struct _DPN_CAPS {
    ...
    DWORD
    dwTimeoutUntilKeepAlive; // ms of inactivity before a keep alive is sent
} DPN_CAPS, *PDPN_CAPS;
...
typedef struct _DPN_CAPS_EX {
    ...
    DWORD
    dwTimeoutUntilKeepAlive; // ms of inactivity before a keep alive is sent
    DWORD dwMaxRecvMsgSize;  // maximum size in bytes of message that can be
                             // received
    ...
    DWORD
    dwDropThresholdRate;  // percentage of dropped packets before throttling
    DWORD dwThrottleRate; // percentage amount to reduce send window when
                          // throttling
    DWORD dwNumHardDisconnectSends;  // number of hard disconnect frames to send
                                     // when close immediate flag is specified
    DWORD dwMaxHardDisconnectPeriod; // maximum period between hard disconnect
                                     // sends
...
typedef struct _DPN_SP_CAPS {
    ...
    DWORD dwSystemBufferSize; // amount of buffering to do in addition to posted
                              // receive buffers
...
typedef struct _DPNMSG_CONNECT_COMPLETE {
    ...
    DWORD
    dwApplicationReplyDataSize; // Size (in bytes) of pvApplicationReplyData
...
typedef struct _DPNMSG_ENUM_HOSTS_QUERY {
    DWORD dwSize; // Size of this structure.
    IDirectPlay8Address*
        pAddressSender; // Address of client who sent the request
    IDirectPlay8Address*
        pAddressDevice;          // Address of device request was received on
...
typedef struct _DPNMSG_ENUM_HOSTS_RESPONSE {
    ...
    const DPN_APPLICATION_DESC*
        pApplicationDescription; // Application description for the session
...
typedef struct _DPNMSG_INDICATE_CONNECT {
    ...
    IDirectPlay8Address*
        pAddressDevice; // Address of device receiving connect attempt
...
typedef struct _DPNMSG_SEND_COMPLETE {
    ...
    DPN_BUFFER_DESC* pBuffers;    // Pointer to array of buffers sent, if
                                  // DirectPlay did not make a copy
...
typedef struct _DPNMSG_NAT_RESOLVER_QUERY {
    DWORD dwSize; // Size of this structure.
    IDirectPlay8Address*
        pAddressSender; // Address of client that sent the query
    IDirectPlay8Address*
        pAddressDevice;    // Address of device on which query was received
 ...
 * CoCreateInstance be used to create DirectPlay8 objects.
 *
 * extern HRESULT WINAPI DirectPlay8Create( const CLSID * pcIID, void
 * **ppvInterface, IUnknown *pUnknown );
 *
 */
...
DECLARE_INTERFACE_( IDirectPlay8Client, IUnknown ) {
    ...
    STDMETHOD( Initialize )( THIS_ PVOID const pvUserContext,
                             const PFNDPNMESSAGEHANDLER pfn,
                             const DWORD dwFlags ) PURE;
    STDMETHOD( EnumServiceProviders )(
        THIS_ const GUID* const pguidServiceProvider,
        const GUID* const pguidApplication,
        DPN_SERVICE_PROVIDER_INFO* const pSPInfoBuffer,
        PDWORD const pcbEnumData, PDWORD const pcReturned, const DWORD dwFlags )
        PURE;
    STDMETHOD( EnumHosts )(
        THIS_ PDPN_APPLICATION_DESC const pApplicationDesc,
        IDirectPlay8Address* const pAddrHost,
        IDirectPlay8Address* const pDeviceInfo, PVOID const pUserEnumData,
        const DWORD dwUserEnumDataSize, const DWORD dwEnumCount,
        const DWORD dwRetryInterval, const DWORD dwTimeOut,
        PVOID const pvUserContext, DPNHANDLE* const pAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( CancelAsyncOperation )( THIS_ const DPNHANDLE hAsyncHandle,
                                       const DWORD dwFlags ) PURE;
    STDMETHOD( Connect )(
        THIS_ const DPN_APPLICATION_DESC* const pdnAppDesc,
        ...
        const void* const pvUserConnectData, const DWORD dwUserConnectDataSize,
        void* const pvAsyncContext, DPNHANDLE* const phAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( Send )( THIS_ const DPN_BUFFER_DESC* const prgBufferDesc,
                       const DWORD cBufferDesc, const DWORD dwTimeOut,
                       void* const pvAsyncContext,
                       DPNHANDLE* const phAsyncHandle, const DWORD dwFlags )
        PURE;
    STDMETHOD( GetSendQueueInfo )( THIS_ DWORD* const pdwNumMsgs,
                                   DWORD* const pdwNumBytes,
                                   const DWORD dwFlags ) PURE;
    STDMETHOD( GetApplicationDesc )(
        THIS_ DPN_APPLICATION_DESC* const pAppDescBuffer,
        DWORD* const pcbDataSize, const DWORD dwFlags ) PURE;
    STDMETHOD( SetClientInfo )(
        THIS_ const DPN_PLAYER_INFO* const pdpnPlayerInfo,
        PVOID const pvAsyncContext, DPNHANDLE* const phAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( GetServerInfo )( THIS_ DPN_PLAYER_INFO* const pdpnPlayerInfo,
                                DWORD* const pdwSize, const DWORD dwFlags )
        PURE;
    STDMETHOD( GetServerAddress )( THIS_ IDirectPlay8Address * * const pAddress,
                                   const DWORD dwFlags ) PURE;
    STDMETHOD( Close )( THIS_ const DWORD dwFlags ) PURE;
    STDMETHOD( ReturnBuffer )( THIS_ const DPNHANDLE hBufferHandle,
                               const DWORD dwFlags ) PURE;
    STDMETHOD( GetCaps )( THIS_ DPN_CAPS* const pdpCaps, const DWORD dwFlags )
        PURE;
    STDMETHOD( SetCaps )( THIS_ const DPN_CAPS* const pdpCaps,
                          const DWORD dwFlags ) PURE;
    STDMETHOD( SetSPCaps )( THIS_ const GUID* const pguidSP,
                            const DPN_SP_CAPS* const pdpspCaps,
                            const DWORD dwFlags ) PURE;
    STDMETHOD( GetSPCaps )( THIS_ const GUID* const pguidSP,
                            DPN_SP_CAPS* const pdpspCaps, const DWORD dwFlags )
        PURE;
    STDMETHOD( GetConnectionInfo )(
        THIS_ DPN_CONNECTION_INFO* const pdpConnectionInfo,
        const DWORD dwFlags ) PURE;
    STDMETHOD( RegisterLobby )(
        THIS_ const DPNHANDLE dpnHandle,
        struct IDirectPlay8LobbiedApplication* const pIDP8LobbiedApplication,
        const DWORD dwFlags ) PURE;
};
...
DECLARE_INTERFACE_( IDirectPlay8Server, IUnknown ) {
    ...
    STDMETHOD( CancelAsyncOperation )( THIS_ const DPNHANDLE hAsyncHandle,
                                       const DWORD dwFlags ) PURE;
    STDMETHOD( GetSendQueueInfo )(
        THIS_ const DPNID dpnid, DWORD* const pdwNumMsgs,
        DWORD* const pdwNumBytes, const DWORD dwFlags ) PURE;
    STDMETHOD( GetApplicationDesc )(
        THIS_ DPN_APPLICATION_DESC* const pAppDescBuffer,
        DWORD* const pcbDataSize, const DWORD dwFlags ) PURE;
    STDMETHOD( SetServerInfo )(
        THIS_ const DPN_PLAYER_INFO* const pdpnPlayerInfo,
        PVOID const pvAsyncContext, DPNHANDLE* const phAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( GetClientInfo )(
        THIS_ const DPNID dpnid, DPN_PLAYER_INFO* const pdpnPlayerInfo,
        DWORD* const pdwSize, const DWORD dwFlags ) PURE;
    STDMETHOD( GetClientAddress )( THIS_ const DPNID dpnid,
                                   IDirectPlay8Address** const pAddress,
                                   const DWORD dwFlags ) PURE;
    STDMETHOD( GetLocalHostAddresses )(
        THIS_ IDirectPlay8Address * * const prgpAddress, DWORD* const pcAddress,
        const DWORD dwFlags ) PURE;
    STDMETHOD( SetApplicationDesc )(
        THIS_ const DPN_APPLICATION_DESC* const pad, const DWORD dwFlags ) PURE;
    STDMETHOD( Host )( THIS_ const DPN_APPLICATION_DESC* const pdnAppDesc,
                       IDirectPlay8Address** const prgpDeviceInfo,
                       const DWORD cDeviceInfo,
                       const DPN_SECURITY_DESC* const pdnSecurity,
                       const DPN_SECURITY_CREDENTIALS* const pdnCredentials,
                       void* const pvPlayerContext, const DWORD dwFlags ) PURE;
    STDMETHOD( SendTo )(
        THIS_ const DPNID dpnid, const DPN_BUFFER_DESC* const prgBufferDesc,
        const DWORD cBufferDesc, const DWORD dwTimeOut,
        void* const pvAsyncContext, DPNHANDLE* const phAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( CreateGroup )(
        THIS_ const DPN_GROUP_INFO* const pdpnGroupInfo,
        void* const pvGroupContext, void* const pvAsyncContext,
        DPNHANDLE* const phAsyncHandle, const DWORD dwFlags ) PURE;
    STDMETHOD( DestroyGroup )(
        THIS_ const DPNID idGroup, PVOID const pvAsyncContext,
        DPNHANDLE* const phAsyncHandle, const DWORD dwFlags ) PURE;
    STDMETHOD( AddPlayerToGroup )(
        THIS_ const DPNID idGroup, const DPNID idClient,
        PVOID const pvAsyncContext, DPNHANDLE* const phAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( RemovePlayerFromGroup )(
        THIS_ const DPNID idGroup, const DPNID idClient,
        PVOID const pvAsyncContext, DPNHANDLE* const phAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( SetGroupInfo )(
        THIS_ const DPNID dpnid, DPN_GROUP_INFO* const pdpnGroupInfo,
        PVOID const pvAsyncContext, DPNHANDLE* const phAsyncHandle,
        const DWORD dwFlags ) PURE;
    STDMETHOD( GetGroupInfo )( THIS_ const DPNID dpnid,
                               DPN_GROUP_INFO* const pdpnGroupInfo,
                               DWORD* const pdwSize, const DWORD dwFlags ) PURE;
    STDMETHOD( EnumPlayersAndGroups )(
        THIS_ DPNID* const prgdpnid, DWORD* const pcdpnid, const DWORD dwFlags )
        PURE;
    STDMETHOD( EnumGroupMembers )( THIS_ const DPNID dpnid,
                                   DPNID* const prgdpnid, DWORD* const pcdpnid,
                                   const DWORD dwFlags ) PURE;
    STDMETHOD( Close )( THIS_ const DWORD dwFlags ) PURE;
    STDMETHOD( DestroyClient )(
        THIS_ const DPNID dpnidClient, const void* const pvDestroyData,
        const DWORD dwDestroyDataSize, const DWORD dwFlags ) PURE;
    STDMETHOD( ReturnBuffer )( THIS_ const DPNHANDLE hBufferHandle,
                               const DWORD dwFlags ) PURE;
    STDMETHOD( GetPlayerContext )( THIS_ const DPNID dpnid,
                                   PVOID* const ppvPlayerContext,
                                   const DWORD dwFlags ) PURE;
    STDMETHOD( GetGroupContext )( THIS_ const DPNID dpnid,
    ...
    STDMETHOD( GetSPCaps )( THIS_ const GUID* const pguidSP,
        ...
        PURE;
    STDMETHOD( GetConnectionInfo )(
        THIS_ const DPNID dpnid, DPN_CONNECTION_INFO* const pdpConnectionInfo,
        const DWORD dwFlags ) PURE;
    STDMETHOD( RegisterLobby )(
        THIS_ const DPNHANDLE dpnHandle,
        struct IDirectPlay8LobbiedApplication* const pIDP8LobbiedApplication,
...
DECLARE_INTERFACE_( IDirectPlay8Peer, IUnknown ) {
    ...
    STDMETHOD( EnumGroupMembers )( THIS_ const DPNID dpnid,
                                   DPNID* const prgdpnid, DWORD* const pcdpnid,
                                   const DWORD dwFlags ) PURE;
    STDMETHOD( SetPeerInfo )( THIS_ const DPN_PLAYER_INFO* const pdpnPlayerInfo,
                              PVOID const pvAsyncContext,
                              DPNHANDLE* const phAsyncHandle,
                              const DWORD dwFlags ) PURE;
    STDMETHOD( GetPeerInfo )( THIS_ const DPNID dpnid,
                              DPN_PLAYER_INFO* const pdpnPlayerInfo,
                              DWORD* const pdwSize, const DWORD dwFlags ) PURE;
    STDMETHOD( GetPeerAddress )( THIS_ const DPNID dpnid,
    ...
    STDMETHOD( RegisterLobby )(
        ...
        struct IDirectPlay8LobbiedApplication* const pIDP8LobbiedApplication,
        const DWORD dwFlags ) PURE;
    STDMETHOD( TerminateSession )( THIS_ void* const pvTerminateData,
...
DECLARE_INTERFACE_( IDirectPlay8ThreadPool, IUnknown ) {
    ...
    STDMETHOD( Initialize )( THIS_ PVOID const pvUserContext,
                             ...
                             const DWORD dwFlags ) PURE;
    STDMETHOD( Close )( THIS_ const DWORD dwFlags ) PURE;
    STDMETHOD( GetThreadCount )( THIS_ const DWORD dwProcessorNum,
                                 DWORD* const pdwNumThreads,
                                 const DWORD dwFlags ) PURE;
    STDMETHOD( SetThreadCount )( THIS_ const DWORD dwProcessorNum,
                                 const DWORD dwNumThreads, const DWORD dwFlags )
        PURE;
    STDMETHOD( DoWork )( THIS_ const DWORD dwAllowedTimeSlice,
                         const DWORD dwFlags ) PURE;
};
...
DECLARE_INTERFACE_( IDirectPlay8NATResolver, IUnknown ) {
    ...
    STDMETHOD( Initialize )( THIS_ PVOID const pvUserContext,
                             const PFNDPNMESSAGEHANDLER pfn,
                             const DWORD dwFlags ) PURE;
    STDMETHOD( Start )( THIS_ IDirectPlay8Address * * const ppDevices,
                        const DWORD dwNumDevices, const DWORD dwFlags ) PURE;
    STDMETHOD( Close )( THIS_ const DWORD dwFlags ) PURE;
    STDMETHOD( EnumDevices )(
        THIS_ DPN_SERVICE_PROVIDER_INFO* const pSPInfoBuffer,
        PDWORD const pdwBufferSize, PDWORD const pdwNumDevices,
        const DWORD dwFlags ) PURE;
    STDMETHOD( GetAddresses )( THIS_ IDirectPlay8Address * * const ppAddresses,
                               DWORD* const pdwNumAddresses,
                               const DWORD dwFlags ) PURE;
};
...
#define IDirectPlay8Client_QueryInterface( p, a, b ) \
    ( p )->lpVtbl->QueryInterface( p, a, b )
#define IDirectPlay8Client_AddRef( p ) ( p )->lpVtbl->AddRef( p )
#define IDirectPlay8Client_Release( p ) ( p )->lpVtbl->Release( p )
#define IDirectPlay8Client_Initialize( p, a, b, c ) \
    ( p )->lpVtbl->Initialize( p, a, b, c )
#define IDirectPlay8Client_EnumServiceProviders( p, a, b, c, d, e, f ) \
    ( p )->lpVtbl->EnumServiceProviders( p, a, b, c, d, e, f )
#define IDirectPlay8Client_EnumHosts( p, a, b, c, d, e, f, g, h, i, j, k ) \
    ( p )->lpVtbl->EnumHosts( p, a, b, c, d, e, f, g, h, i, j, k )
#define IDirectPlay8Client_CancelAsyncOperation( p, a, b ) \
    ( p )->lpVtbl->CancelAsyncOperation( p, a, b )
#define IDirectPlay8Client_Connect( p, a, b, c, d, e, f, g, h, i, j ) \
    ( p )->lpVtbl->Connect( p, a, b, c, d, e, f, g, h, i, j )
#define IDirectPlay8Client_Send( p, a, b, c, d, e, f ) \
    ( p )->lpVtbl->Send( p, a, b, c, d, e, f )
#define IDirectPlay8Client_GetSendQueueInfo( p, a, b, c ) \
    ( p )->lpVtbl->GetSendQueueInfo( p, a, b, c )
#define IDirectPlay8Client_GetApplicationDesc( p, a, b, c ) \
    ( p )->lpVtbl->GetApplicationDesc( p, a, b, c )
#define IDirectPlay8Client_SetClientInfo( p, a, b, c, d ) \
    ( p )->lpVtbl->SetClientInfo( p, a, b, c, d )
#define IDirectPlay8Client_GetServerInfo( p, a, b, c ) \
    ( p )->lpVtbl->GetServerInfo( p, a, b, c )
#define IDirectPlay8Client_GetServerAddress( p, a, b ) \
    ( p )->lpVtbl->GetServerAddress( p, a, b )
#define IDirectPlay8Client_Close( p, a ) ( p )->lpVtbl->Close( p, a )
#define IDirectPlay8Client_ReturnBuffer( p, a, b ) \
...
#define IDirectPlay8Client_SetSPCaps( p, a, b, c ) \
    ( p )->lpVtbl->SetSPCaps( p, a, b, c )
#define IDirectPlay8Client_GetSPCaps( p, a, b, c ) \
    ( p )->lpVtbl->GetSPCaps( p, a, b, c )
#define IDirectPlay8Client_GetConnectionInfo( p, a, b ) \
    ( p )->lpVtbl->GetConnectionInfo( p, a, b )
#define IDirectPlay8Client_RegisterLobby( p, a, b, c ) \
    ( p )->lpVtbl->RegisterLobby( p, a, b, c )

#define IDirectPlay8Server_QueryInterface( p, a, b ) \
    ( p )->lpVtbl->QueryInterface( p, a, b )
#define IDirectPlay8Server_AddRef( p ) ( p )->lpVtbl->AddRef( p )
#define IDirectPlay8Server_Release( p ) ( p )->lpVtbl->Release( p )
#define IDirectPlay8Server_Initialize( p, a, b, c ) \
    ( p )->lpVtbl->Initialize( p, a, b, c )
#define IDirectPlay8Server_EnumServiceProviders( p, a, b, c, d, e, f ) \
    ( p )->lpVtbl->EnumServiceProviders( p, a, b, c, d, e, f )
#define IDirectPlay8Server_CancelAsyncOperation( p, a, b ) \
    ( p )->lpVtbl->CancelAsyncOperation( p, a, b )
#define IDirectPlay8Server_GetSendQueueInfo( p, a, b, c, d ) \
    ( p )->lpVtbl->GetSendQueueInfo( p, a, b, c, d )
#define IDirectPlay8Server_GetApplicationDesc( p, a, b, c ) \
    ( p )->lpVtbl->GetApplicationDesc( p, a, b, c )
#define IDirectPlay8Server_SetServerInfo( p, a, b, c, d ) \
    ( p )->lpVtbl->SetServerInfo( p, a, b, c, d )
#define IDirectPlay8Server_GetClientInfo( p, a, b, c, d ) \
    ( p )->lpVtbl->GetClientInfo( p, a, b, c, d )
#define IDirectPlay8Server_GetClientAddress( p, a, b, c ) \
    ( p )->lpVtbl->GetClientAddress( p, a, b, c )
#define IDirectPlay8Server_GetLocalHostAddresses( p, a, b, c ) \
    ( p )->lpVtbl->GetLocalHostAddresses( p, a, b, c )
#define IDirectPlay8Server_SetApplicationDesc( p, a, b ) \
    ( p )->lpVtbl->SetApplicationDesc( p, a, b )
#define IDirectPlay8Server_Host( p, a, b, c, d, e, f, g ) \
    ( p )->lpVtbl->Host( p, a, b, c, d, e, f, g )
#define IDirectPlay8Server_SendTo( p, a, b, c, d, e, f, g ) \
    ( p )->lpVtbl->SendTo( p, a, b, c, d, e, f, g )
#define IDirectPlay8Server_CreateGroup( p, a, b, c, d, e ) \
    ( p )->lpVtbl->CreateGroup( p, a, b, c, d, e )
#define IDirectPlay8Server_DestroyGroup( p, a, b, c, d ) \
    ( p )->lpVtbl->DestroyGroup( p, a, b, c, d )
#define IDirectPlay8Server_AddPlayerToGroup( p, a, b, c, d, e ) \
    ( p )->lpVtbl->AddPlayerToGroup( p, a, b, c, d, e )
#define IDirectPlay8Server_RemovePlayerFromGroup( p, a, b, c, d, e ) \
    ( p )->lpVtbl->RemovePlayerFromGroup( p, a, b, c, d, e )
#define IDirectPlay8Server_SetGroupInfo( p, a, b, c, d, e ) \
    ( p )->lpVtbl->SetGroupInfo( p, a, b, c, d, e )
#define IDirectPlay8Server_GetGroupInfo( p, a, b, c, d ) \
    ( p )->lpVtbl->GetGroupInfo( p, a, b, c, d )
#define IDirectPlay8Server_EnumPlayersAndGroups( p, a, b, c ) \
    ( p )->lpVtbl->EnumPlayersAndGroups( p, a, b, c )
#define IDirectPlay8Server_EnumGroupMembers( p, a, b, c, d ) \
    ( p )->lpVtbl->EnumGroupMembers( p, a, b, c, d )
#define IDirectPlay8Server_Close( p, a ) ( p )->lpVtbl->Close( p, a )
#define IDirectPlay8Server_DestroyClient( p, a, b, c, d ) \
    ( p )->lpVtbl->DestroyClient( p, a, b, c, d )
#define IDirectPlay8Server_ReturnBuffer( p, a, b ) \
    ( p )->lpVtbl->ReturnBuffer( p, a, b )
#define IDirectPlay8Server_GetPlayerContext( p, a, b, c ) \
    ( p )->lpVtbl->GetPlayerContext( p, a, b, c )
#define IDirectPlay8Server_GetGroupContext( p, a, b, c ) \
...
#define IDirectPlay8Server_SetSPCaps( p, a, b, c ) \
    ( p )->lpVtbl->SetSPCaps( p, a, b, c )
#define IDirectPlay8Server_GetSPCaps( p, a, b, c ) \
    ( p )->lpVtbl->GetSPCaps( p, a, b, c )
#define IDirectPlay8Server_GetConnectionInfo( p, a, b, c ) \
    ( p )->lpVtbl->GetConnectionInfo( p, a, b, c )
#define IDirectPlay8Server_RegisterLobby( p, a, b, c ) \
    ( p )->lpVtbl->RegisterLobby( p, a, b, c )
...
#define IDirectPlay8Peer_Initialize( p, a, b, c ) \
    ( p )->lpVtbl->Initialize( p, a, b, c )
#define IDirectPlay8Peer_EnumServiceProviders( p, a, b, c, d, e, f ) \
    ( p )->lpVtbl->EnumServiceProviders( p, a, b, c, d, e, f )
...
#define IDirectPlay8Peer_SendTo( p, a, b, c, d, e, f, g ) \
    ( p )->lpVtbl->SendTo( p, a, b, c, d, e, f, g )
...
#define IDirectPlay8ThreadPool_QueryInterface( p, a, b ) \
    ( p )->lpVtbl->QueryInterface( p, a, b )
#define IDirectPlay8ThreadPool_AddRef( p ) ( p )->lpVtbl->AddRef( p )
#define IDirectPlay8ThreadPool_Release( p ) ( p )->lpVtbl->Release( p )
#define IDirectPlay8ThreadPool_Initialize( p, a, b, c ) \
    ( p )->lpVtbl->Initialize( p, a, b, c )
#define IDirectPlay8ThreadPool_Close( p, a ) ( p )->lpVtbl->Close( p, a )
#define IDirectPlay8ThreadPool_GetThreadCount( p, a, b, c ) \
    ( p )->lpVtbl->GetThreadCount( p, a, b, c )
#define IDirectPlay8ThreadPool_SetThreadCount( p, a, b, c ) \
    ( p )->lpVtbl->SetThreadCount( p, a, b, c )
#define IDirectPlay8ThreadPool_DoWork( p, a, b ) \
    ( p )->lpVtbl->DoWork( p, a, b )

#define IDirectPlay8NATResolver_QueryInterface( p, a, b ) \
    ( p )->lpVtbl->QueryInterface( p, a, b )
#define IDirectPlay8NATResolver_AddRef( p ) ( p )->lpVtbl->AddRef( p )
#define IDirectPlay8NATResolver_Release( p ) ( p )->lpVtbl->Release( p )
#define IDirectPlay8NATResolver_Initialize( p, a, b, c ) \
    ( p )->lpVtbl->Initialize( p, a, b, c )
#define IDirectPlay8NATResolver_Start( p, a, b, c ) \
    ( p )->lpVtbl->Start( p, a, b, c )
#define IDirectPlay8NATResolver_Close( p, a ) ( p )->lpVtbl->Close( p, a )
#define IDirectPlay8NATResolver_EnumDevices( p, a, b, c, d ) \
    ( p )->lpVtbl->EnumDevices( p, a, b, c, d )
#define IDirectPlay8NATResolver_GetAddresses( p, a, b, c ) \
...
#define IDirectPlay8Client_EnumServiceProviders( p, a, b, c, d, e, f ) \
    ( p )->EnumServiceProviders( a, b, c, d, e, f )
#define IDirectPlay8Client_EnumHosts( p, a, b, c, d, e, f, g, h, i, j, k ) \
    ( p )->EnumHosts( a, b, c, d, e, f, g, h, i, j, k )
#define IDirectPlay8Client_CancelAsyncOperation( p, a, b ) \
    ( p )->CancelAsyncOperation( a, b )
#define IDirectPlay8Client_Connect( p, a, b, c, d, e, f, g, h, i, j ) \
    ( p )->Connect( a, b, c, d, e, f, g, h, i, j )
#define IDirectPlay8Client_Send( p, a, b, c, d, e, f ) \
    ( p )->Send( a, b, c, d, e, f )
#define IDirectPlay8Client_GetSendQueueInfo( p, a, b, c ) \
    ( p )->GetSendQueueInfo( a, b, c )
#define IDirectPlay8Client_GetApplicationDesc( p, a, b, c ) \
    ( p )->GetApplicationDesc( a, b, c )
#define IDirectPlay8Client_SetClientInfo( p, a, b, c, d ) \
    ( p )->SetClientInfo( a, b, c, d )
#define IDirectPlay8Client_GetServerInfo( p, a, b, c ) \
    ( p )->GetServerInfo( a, b, c )
#define IDirectPlay8Client_GetServerAddress( p, a, b ) \
    ( p )->GetServerAddress( a, b )
...
#define IDirectPlay8ThreadPool_GetThreadCount( p, a, b, c ) \
    ( p )->GetThreadCount( a, b, c )
#define IDirectPlay8ThreadPool_SetThreadCount( p, a, b, c ) \
    ( p )->SetThreadCount( a, b, c )
...
#define MAKE_DPNHRESULT( code ) \
    MAKE_HRESULT( 1, _DPN_FACILITY_CODE, ( code + _DPNHRESULT_BASE ) )

#define DPN_OK S_OK

#define DPNSUCCESS_EQUAL \
    MAKE_HRESULT( 0, _DPN_FACILITY_CODE, ( 0x5 + _DPNHRESULT_BASE ) )
#define DPNSUCCESS_NOPLAYERSINGROUP      \
    MAKE_HRESULT( 0, _DPN_FACILITY_CODE, \
                  ( 0x8 + _DPNHRESULT_BASE ) ) // added for DirectX 9
#define DPNSUCCESS_NOTEQUAL \
    MAKE_HRESULT( 0, _DPN_FACILITY_CODE, ( 0x0A + _DPNHRESULT_BASE ) )
#define DPNSUCCESS_PENDING \
    MAKE_HRESULT( 0, _DPN_FACILITY_CODE, ( 0x0e + _DPNHRESULT_BASE ) )
