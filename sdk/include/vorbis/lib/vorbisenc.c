...
static void vorbis_encode_floor_setup(
    vorbis_info* vi,
    double s,
    int block,
    ...
    /* books */
    {
        ...
        for ( i = 0; i <= maxbook; i++ )
            ci->book_param[ ci->books++ ] =
                ( static_codebook* )books[ x[ is ] ][ i ];
    }
...
static void vorbis_encode_global_psych_setup( vorbis_info* vi,
                                              double s,
    ...
    for ( i = 0; i < 4; i++ ) {
        g->preecho_thresh[ i ] = in[ is ].preecho_thresh[ i ] * ( 1. - ds ) +
                                 in[ is + 1 ].preecho_thresh[ i ] * ds;
        g->postecho_thresh[ i ] = in[ is ].postecho_thresh[ i ] * ( 1. - ds ) +
                                  in[ is + 1 ].postecho_thresh[ i ] * ds;
    }
...
static void vorbis_encode_global_stereo( vorbis_info* vi,
    ...
    if ( p ) {
        memcpy( g->coupling_prepointamp, p[ is ].pre,
                sizeof( *p[ is ].pre ) * PACKETBLOBS );
        memcpy( g->coupling_postpointamp, p[ is ].post,
                sizeof( *p[ is ].post ) * PACKETBLOBS );

        if ( hi->managed ) {
            /* interpolate the kHz threshholds */
            for ( i = 0; i < PACKETBLOBS; i++ ) {
                float kHz =
                    p[ is ].kHz[ i ] * ( 1. - ds ) + p[ is + 1 ].kHz[ i ] * ds;
                g->coupling_pointlimit[ 0 ][ i ] =
                    kHz * 1000. / vi->rate * ci->blocksizes[ 0 ];
                g->coupling_pointlimit[ 1 ][ i ] =
                    kHz * 1000. / vi->rate * ci->blocksizes[ 1 ];
                g->coupling_pkHz[ i ] = kHz;

                kHz = p[ is ].lowpasskHz[ i ] * ( 1. - ds ) +
                      p[ is + 1 ].lowpasskHz[ i ] * ds;
                g->sliding_lowpass[ 0 ][ i ] =
                    kHz * 1000. / vi->rate * ci->blocksizes[ 0 ];
                g->sliding_lowpass[ 1 ][ i ] =
                    kHz * 1000. / vi->rate * ci->blocksizes[ 1 ];
            }
        } else {
            float kHz = p[ is ].kHz[ PACKETBLOBS / 2 ] * ( 1. - ds ) +
                        p[ is + 1 ].kHz[ PACKETBLOBS / 2 ] * ds;
            for ( i = 0; i < PACKETBLOBS; i++ ) {
                g->coupling_pointlimit[ 0 ][ i ] =
                    kHz * 1000. / vi->rate * ci->blocksizes[ 0 ];
                g->coupling_pointlimit[ 1 ][ i ] =
            ...
            kHz = p[ is ].lowpasskHz[ PACKETBLOBS / 2 ] * ( 1. - ds ) +
...
static void vorbis_encode_psyset_setup( vorbis_info* vi,
                                        double s,
                                        const int* nn_start,
...
static void vorbis_encode_tonemask_setup( vorbis_info* vi,
    ...
    p->tone_masteratt[ 0 ] =
        att[ is ].att[ 0 ] * ( 1. - ds ) + att[ is + 1 ].att[ 0 ] * ds;
    p->tone_masteratt[ 1 ] =
        att[ is ].att[ 1 ] * ( 1. - ds ) + att[ is + 1 ].att[ 1 ] * ds;
    p->tone_masteratt[ 2 ] =
        att[ is ].att[ 2 ] * ( 1. - ds ) + att[ is + 1 ].att[ 2 ] * ds;
    p->tone_centerboost =
        att[ is ].boost * ( 1. - ds ) + att[ is + 1 ].boost * ds;
    ...
    for ( i = 0; i < P_BANDS; i++ )
        p->toneatt[ i ] =
            in[ is ].block[ i ] * ( 1. - ds ) + in[ is + 1 ].block[ i ] * ds;
    return;
}
...
static void vorbis_encode_compand_setup( vorbis_info* vi,
    ...
    for ( i = 0; i < NOISE_COMPAND_LEVELS; i++ )
        p->noisecompand[ i ] =
            in[ is ].data[ i ] * ( 1. - ds ) + in[ is + 1 ].data[ i ] * ds;
    return;
}
...
static void vorbis_encode_noisebias_setup( vorbis_info* vi,
    ...
    for ( j = 0; j < P_NOISECURVES; j++ )
        for ( i = 0; i < P_BANDS; i++ )
            p->noiseoff[ j ][ i ] = in[ is ].data[ j ][ i ] * ( 1. - ds ) +
                                    in[ is + 1 ].data[ j ][ i ] * ds;
...
static void vorbis_encode_residue_setup( vorbis_info* vi,
    ...
    /* fill in all the books */
    {
        ...
        if ( ci->hi.managed ) {
            ...
            ci->book_param[ r->groupbook ] =
                ( static_codebook* )res->book_aux_managed;

            for ( i = 0; i < r->partitions; i++ ) {
                for ( k = 0; k < 3; k++ ) {
                    if ( res->books_base_managed->books[ i ][ k ] ) {
                        int bookid = book_dup_or_new(
                            ci, res->books_base_managed->books[ i ][ k ] );
                        r->booklist[ booklist++ ] = bookid;
                        ci->book_param[ bookid ] =
                            ( static_codebook* )
                                res->books_base_managed->books[ i ][ k ];
                    }
                }
            }
            ...
            for ( i = 0; i < r->partitions; i++ ) {
                for ( k = 0; k < 3; k++ ) {
                    if ( res->books_base->books[ i ][ k ] ) {
                        int bookid = book_dup_or_new(
                            ci, res->books_base->books[ i ][ k ] );
                        r->booklist[ booklist++ ] = bookid;
                        ci->book_param[ bookid ] =
                            ( static_codebook* )
                                res->books_base->books[ i ][ k ];
                    }
                }
            }
        }
    ...
    /* lowpass setup/pointlimit */
    {
        ...
        if ( ci->residue_type[ block ] == 2 )
            r->end = ( int )( ( freq / nyq * blocksize * 2 ) / r->grouping +
                              .9 ) * /* round up only if we're well past */
                     r->grouping;
        else
            r->end = ( int )( ( freq / nyq * blocksize ) / r->grouping +
                              .9 ) * /* round up only if we're well past */
                     r->grouping;
    }
}
...
static void vorbis_encode_map_n_res_setup(
    vorbis_info* vi,
    double s,
    const vorbis_mapping_template* maps ) {
    codec_setup_info* ci = vi->codec_setup;
    ...
    while ( setup_list[ i ] ) {
        if ( setup_list[ i ]->coupling_restriction == -1 ||
            ...
            if ( srate >= setup_list[ i ]->samplerate_min_restriction &&
                ...
                const double* map =
                    ( q_or_bitrate ? setup_list[ i ]->rate_mapping
...
int vorbis_encode_setup_init( vorbis_info* vi ) {
    ...
    vorbis_encode_blocksize_setup( vi, hi->base_setting, setup->blocksize_short,
    ...
    vorbis_encode_floor_setup( vi, hi->short_setting, 0, setup->floor_books,
                               setup->floor_params,
                               setup->floor_short_mapping );
    if ( !singleblock )
        vorbis_encode_floor_setup( vi, hi->long_setting, 1, setup->floor_books,
                                   setup->floor_params,
                                   setup->floor_long_mapping );

    /* setup of [mostly] short block detection and stereo*/
    vorbis_encode_global_psych_setup(
        vi, hi->trigger_setting, setup->global_params, setup->global_mapping );
    ...
    vorbis_encode_psyset_setup( vi, hi->short_setting,
                                ...
                                setup->psy_noise_normal_thresh, 0 );
    vorbis_encode_psyset_setup( vi, hi->short_setting,
                                setup->psy_noise_normal_start[ 0 ],
                                setup->psy_noise_normal_partition[ 0 ],
                                setup->psy_noise_normal_thresh, 1 );
    if ( !singleblock ) {
        vorbis_encode_psyset_setup( vi, hi->long_setting,
                                    setup->psy_noise_normal_start[ 1 ],
                                    setup->psy_noise_normal_partition[ 1 ],
                                    setup->psy_noise_normal_thresh, 2 );
        vorbis_encode_psyset_setup( vi, hi->long_setting,
                                    setup->psy_noise_normal_start[ 1 ],
                                    setup->psy_noise_normal_partition[ 1 ],
                                    setup->psy_noise_normal_thresh, 3 );
    }

    /* tone masking setup */
    vorbis_encode_tonemask_setup(
        vi, hi->block[ i0 ].tone_mask_setting, 0, setup->psy_tone_masteratt,
        setup->psy_tone_0dB, setup->psy_tone_adj_impulse );
    vorbis_encode_tonemask_setup(
        vi, hi->block[ 1 ].tone_mask_setting, 1, setup->psy_tone_masteratt,
        setup->psy_tone_0dB, setup->psy_tone_adj_other );
    if ( !singleblock ) {
        vorbis_encode_tonemask_setup(
            vi, hi->block[ 2 ].tone_mask_setting, 2, setup->psy_tone_masteratt,
            setup->psy_tone_0dB, setup->psy_tone_adj_other );
        vorbis_encode_tonemask_setup(
            vi, hi->block[ 3 ].tone_mask_setting, 3, setup->psy_tone_masteratt,
            setup->psy_tone_0dB, setup->psy_tone_adj_long );
    }
    ...
    vorbis_encode_compand_setup( vi, hi->block[ 1 ].noise_compand_setting, 1,
                                 ...
                                 setup->psy_noise_compand_short_mapping );
    if ( !singleblock ) {
        vorbis_encode_compand_setup( vi, hi->block[ 2 ].noise_compand_setting,
                                     2, setup->psy_noise_compand,
                                     setup->psy_noise_compand_long_mapping );
        vorbis_encode_compand_setup( vi, hi->block[ 3 ].noise_compand_setting,
                                     3, setup->psy_noise_compand,
                                     setup->psy_noise_compand_long_mapping );
    }
    ...
    vorbis_encode_noisebias_setup(
        ...
        setup->psy_noise_bias_impulse, setup->psy_noiseguards,
        ( i0 == 0 ? hi->impulse_noisetune : 0. ) );
    vorbis_encode_noisebias_setup(
        vi, hi->block[ 1 ].noise_bias_setting, 1, setup->psy_noise_dBsuppress,
        setup->psy_noise_bias_padding, setup->psy_noiseguards, 0. );
    if ( !singleblock ) {
...
static int vorbis_encode_setup_setting( vorbis_info* vi,
    ...
    hi->lowpass_kHz = setup->psy_lowpass[ is ] * ( 1. - ds ) +
                      setup->psy_lowpass[ is + 1 ] * ds;
...
int vorbis_encode_init( vorbis_info* vi,
                        ...
                        long nominal_bitrate,
                        long min_bitrate ) {
    int ret = vorbis_encode_setup_managed( vi, channels, rate, max_bitrate,
                                           nominal_bitrate, min_bitrate );
    if ( ret ) {
        vorbis_info_clear( vi );
        return ( ret );
        ...
        switch ( number ) {
            ...
            case OV_ECTL_RATEMANAGE_AVG: {
                ...
                if ( ai == NULL ) {
                    ...
                    hi->bitrate_av =
                        ( ai->bitrate_av_lo + ai->bitrate_av_hi ) * .5;
                }
            }
                return ( 0 );
            /* now deprecated *****************/
            ...
            case OV_ECTL_RATEMANAGE2_SET: {
                ...
                if ( ai == NULL ) {
                    ...
                    hi->bitrate_reservoir_bias =
                        ai->bitrate_limit_reservoir_bias;
                }
            }
                return 0;
