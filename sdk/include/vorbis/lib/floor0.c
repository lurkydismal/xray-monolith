...
static vorbis_info_floor* floor0_unpack( vorbis_info* vi,
                                         oggpack_buffer* opb ) {
...
static void floor0_map_lazy_init( vorbis_block* vb,
    ...
    if ( !look->linearmap[ vb->W ] ) {
        ...
        /* the mapping from a linear scale to a smaller bark scale is
           ...
           accurate */
        look->linearmap[ W ] =
            _ogg_malloc( ( n + 1 ) * sizeof( **look->linearmap ) );
        for ( j = 0; j < n; j++ ) {
    ...
    if ( ampraw > 0 ) { /* also handles the -1 out of data case */
        ...
        if ( booknum != -1 && booknum < info->numbooks ) { /* be paranoid */
            ...
            /* the additional b->dim is a guard against any possible stack
               ...
               vector */
            float* lsp = _vorbis_block_alloc(
                vb, sizeof( *lsp ) * ( look->m + b->dim + 1 ) );

            for ( j = 0; j < look->m; j += b->dim )
                if ( vorbis_book_decodev_set( b, lsp + j, &vb->opb, b->dim ) ==
                     -1 )
                    goto eop;
            for ( j = 0; j < look->m; ) {
                for ( k = 0; k < b->dim; k++, j++ )
...
static int floor0_inverse2( vorbis_block* vb,
    ...
    if ( memo ) {
        ...
        vorbis_lsp_to_curve( out, look->linearmap[ vb->W ], look->n[ vb->W ],
                             look->ln, lsp, look->m, amp,
                             ( float )info->ampdB );
        return ( 1 );
    }
...
/* export hooks */
const vorbis_func_floor floor0_exportbundle = { NULL,
                                                &floor0_unpack,
                                                &floor0_look,
                                                &floor0_free_info,
                                                &floor0_free_look,
