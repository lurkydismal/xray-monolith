...
static void floor1_pack( vorbis_info_floor* i, oggpack_buffer* opb ) {
    ...
    for ( j = 0; j < info->partitions; j++ ) {
        oggpack_write( opb, info->partitionclass[ j ],
...
static vorbis_info_floor* floor1_unpack( vorbis_info* vi,
    ...
    for ( j = 0; j < info->partitions; j++ ) {
        info->partitionclass[ j ] =
            oggpack_read( opb, 4 ); /* only 0 to 15 legal */
    ...
    for ( j = 0; j < maxclass + 1; j++ ) {
        ...
        for ( k = 0; k < ( 1 << info->class_subs[ j ] ); k++ ) {
            ...
            if ( info->class_subbook[ j ][ k ] < -1 ||
                 info->class_subbook[ j ][ k ] >= ci->books )
                goto err_out;
        }
    }
...
static int accumulate_fit( const float* flr,
    ...
    long xa = 0, ya = 0, x2a = 0, y2a = 0, xya = 0, na = 0, xb = 0, yb = 0,
         x2b = 0, y2b = 0, xyb = 0, nb = 0;
    ...
    if ( posts == 0 ) {
        ...
        for ( i = 0; i < posts - 1; i++ )
            nonzero += accumulate_fit(
                logmask, logmdct, look->sorted_index[ i ],
                look->sorted_index[ i + 1 ], fits + i, n, info );
    }
    ...
    if ( nonzero ) {
        ...
        for ( i = 2; i < posts; i++ ) {
            ...
            if ( memo[ ln ] != hn ) {
                ...
                {
                    /* A note: we want to bound/minimize *local*, not global,
                     * error */
                    ...
                    if ( inspect_error( lx, hx, ly, hy, logmask, logmdct,
                                        info ) ) {
                        ...
                        int hy0 = -200;
                        int hy1 = -200;
                        int ret0 = fit_line( fits + lsortpos,
                                             sortpos - lsortpos, &ly0, &ly1 );
                        int ret1 = fit_line( fits + sortpos, hsortpos - sortpos,
                                             &hy0, &hy1 );
...
int* floor1_interpolate_fit( vorbis_block* vb,
    ...
    if ( A && B ) {
        ...
        for ( i = 0; i < posts; i++ ) {
            output[ i ] = ( ( 65536 - del ) * ( A[ i ] & 0x7fff ) +
                            del * ( B[ i ] & 0x7fff ) + 32768 ) >>
                          16;
    ...
    if ( post ) {
        ...
        for ( i = 2; i < posts; i++ ) {
            ...
            if ( ( post[ i ] & 0x8000 ) || ( predicted == post[ i ] ) ) {
                post[ i ] = predicted | 0x8000; /* in case there was roundoff
            ...
            if ( csubbits ) {
                ...
                for ( k = 0; k < csub; k++ ) {
                    ...
                    if ( booknum < 0 ) {
                        ...
                        maxval[ k ] =
                            sbooks[ info->class_subbook[ class ][ k ] ]
                                ->entries;
                    }
                }
                for ( k = 0; k < cdim; k++ ) {
                ...
                look->phrasebits += vorbis_book_encode(
                    books + info->class_book[ class ], cval, opb );

#ifdef TRAIN_FLOOR1
                {
                    FILE* of;
                    char buffer[ 80 ];
                    sprintf( buffer, "line_%dx%ld_class%d.vqd", vb->pcmend / 2,
                             posts - 2, class );
                    of = fopen( buffer, "a" );
                    fprintf( of, "%d\n", cval );
                    fclose( of );
...
static void* floor1_inverse1( vorbis_block* vb, vorbis_look_floor* in ) {
    ...
    if ( oggpack_read( &vb->opb, 1 ) == 1 ) {
        int* fit_value =
            _vorbis_block_alloc( vb, ( look->posts ) * sizeof( *fit_value ) );
        ...
        for ( i = 0, j = 2; i < info->partitions; i++ ) {
            ...
            if ( csubbits ) {
                cval = vorbis_book_decode( books + info->class_book[ class ],
                                           &vb->opb );
            ...
            for ( k = 0; k < cdim; k++ ) {
                ...
                cval >>= csubbits;
                if ( book >= 0 ) {
                    if ( ( fit_value[ j + k ] = vorbis_book_decode(
                               books + book, &vb->opb ) ) == -1 )
                        goto eop;
                } else {
                    fit_value[ j + k ] = 0;
