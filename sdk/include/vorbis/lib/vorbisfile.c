...
static ogg_int64_t _get_next_page( OggVorbis_File* vf,
                                   ogg_page* og,
...
static void _add_serialno( ogg_page* og, long** serialno_list, int* n ) {
    ...
    if ( *serialno_list ) {
        *serialno_list =
            _ogg_realloc( *serialno_list, sizeof( **serialno_list ) * ( *n ) );
    } else {
    ...
    while ( offset == -1 ) {
        ...
        while ( vf->offset < end ) {
            ...
            if ( ret < 0 ) {
                ...
                if ( !_lookup_serialno( ret_serialno, serial_list,
                                        serial_n ) ) {
...
static int _fetch_headers( OggVorbis_File* vf,
    ...
    while ( ogg_page_bos( og_ptr ) ) {
        if ( serialno_list ) {
            if ( _lookup_page_serialno( og_ptr, *serialno_list,
                                        *serialno_n ) ) {
                /* a dupe serialnumber in an initial header packet set ==
                 * invalid stream */
                if ( *serialno_list )
        ...
        /* get next page */
        {
            ...
            /* if this page also belongs to our vorbis stream, submit it and
             * break */
            if ( vf->ready_state == STREAMSET &&
...
static ogg_int64_t _initial_pcmoffset( OggVorbis_File* vf, vorbis_info* vi ) {
    ...
    while ( 1 ) {
        ...
        if ( _get_next_page( vf, &og, -1 ) < 0 )
            break; /* should not be possible unless the file is
                      truncated/mangled */
    ...
    */
    ...
    if ( _lookup_serialno( endserial, currentno_list, currentnos ) ) {
        ...
        while ( endserial != serialno ) {
            endserial = serialno;
            vf->offset = _get_prev_page_serial( vf, currentno_list, currentnos,
                                                &endserial, &endgran );
        }
        ...
        vf->pcmlengths =
            _ogg_malloc( vf->links * 2 * sizeof( *vf->pcmlengths ) );
        ...
        while ( searched < endsearched ) {
            ...
            if ( last == OV_EREAD )
                return ( OV_EREAD );
            if ( last < 0 ||
                 !_lookup_page_serialno( &og, currentno_list, currentnos ) ) {
                endsearched = bisect;
                if ( last >= 0 )
                    next = last;
            } else {
        ...
        /* for the time being, fetch end PCM offset the simple way */
        {
            ...
            while ( testserial != serialno ) {
                testserial = serialno;
                vf->offset = _get_prev_page_serial(
                    vf, currentno_list, currentnos, &testserial, &searchgran );
            }
        }
        ...
        ret = _fetch_headers( vf, &vi, &vc, &next_serialno_list,
                              &next_serialnos, NULL );
        if ( ret )
            return ( ret );
        ...
        ret = _bisect_forward_serialno( vf, next, vf->offset, end, endgran,
                                        endserial, next_serialno_list,
                                        next_serialnos, m + 1 );
        if ( ret )
            return ( ret );
...
static int _open_seekable2( OggVorbis_File* vf ) {
    ...
    end = _get_prev_page_serial( vf, vf->serialnos + 2, vf->serialnos[ 1 ],
                                 &endserial, &endgran );
    if ( end < 0 )
        return ( end );

    /* now determine bitstream structure recursively */
    if ( _bisect_forward_serialno( vf, 0, dataoffset, vf->offset, endgran,
                                   endserial, vf->serialnos + 2,
                                   vf->serialnos[ 1 ], 0 ) < 0 )
        return ( OV_EREAD );
            ...
            while ( 1 ) {
                ...
                if ( result > 0 ) {
                    ...
                    if ( !vorbis_synthesis(
                        ...
                        {
                            int oldsamples =
                                vorbis_synthesis_pcmout( &vf->vd, NULL );
                        ...
                        if ( granulepos != -1 && !op_ptr->e_o_s ) {
                            ...
                            /* this packet has a pcm_offset on it (the last
                               packet completed on a page carries the offset)
                               After processing (above), we know the pcm
                               position of the *last* sample ready to be
                               returned. Find the offset of the *first*

                               As an aside, this trick is inaccurate if we begin
                               reading anew right at the last page; the
                               end-of-stream granulepos declares the last frame
                               in the stream, and the last packet of the last
                               page may be a partial frame. So, we need a
                               previous granulepos from an in-sequence page to
                               have a reference point.  Thus the !op_ptr->e_o_s
                               clause above */
                ...
                if ( vf->ready_state == INITSET ) {
                    if ( vf->current_serialno != ogg_page_serialno( &og ) ) {
                        /* two possibilities:
                           ...
                           2) another stream is multiplexed into this logical
                           section */
            ...
            if ( vf->ready_state < STREAMSET ) {
                if ( vf->seekable ) {
                    ...
                    /* match the serialno to bitstream section.  We use this
                       rather than offset positions to avoid problems near
                    ...
                    int ret =
                        _fetch_headers( vf, vf->vi, vf->vc, NULL, NULL, &og );
                    if ( ret )
                        return ( ret );
...
static int _ov_open1( void* f,
                      OggVorbis_File* vf,
                      char* initial,
                      long ibytes,
                      ov_callbacks callbacks ) {
    int offsettest =
        ( ( f && callbacks.seek_func ) ? callbacks.seek_func( f, 0, SEEK_CUR )
                                       : -1 );
    ...
    if ( ( ret = _fetch_headers( vf, vf->vi, vf->vc, &serialno_list,
                                 &serialno_list_size, NULL ) ) < 0 ) {
        vf->datasource = NULL;
        ov_clear( vf );
    } else {
        ...
        vf->serialnos =
            _ogg_calloc( serialno_list_size + 2, sizeof( *vf->serialnos ) );
        vf->serialnos[ 0 ] = vf->current_serialno;
        vf->serialnos[ 1 ] = serialno_list_size;
        memcpy( vf->serialnos + 2, serialno_list,
                serialno_list_size * sizeof( *vf->serialnos ) );
...
long ov_bitrate( OggVorbis_File* vf, int i ) {
    ...
    if ( i < 0 ) {
        ...
        if ( vf->seekable ) {
            ...
            return ( rint( ( vf->offsets[ i + 1 ] - vf->dataoffsets[ i ] ) * 8 /
                           ov_time_total( vf, i ) ) );
        } else {
    ...
    */

    {
        ...
        ogg_stream_init( &work_os,
                         vf->current_serialno ); /* get the memory ready */
                ...
                if ( result > 0 ) {
                    if ( vf->vi[ vf->current_link ].codec_setup ) {
                        thisblock = vorbis_packet_blocksize(
                            vf->vi + vf->current_link, &op );
                        if ( thisblock < 0 ) {
                            ogg_stream_packetout( &vf->os, NULL );
                            thisblock = 0;
                        } else {
                            /* We can't get a guaranteed correct pcm position
                               out of the last page in a stream because it might
                               have a 'short' granpos, which can only be
                               detected in the presence of a preceeding page.
                               However, if the last page is also the first page,
                               the granpos rules of a first page take
                               precedence.  Not only that, but for first==last,
                               the EOS page must be treated as if its a normal
                               first page for the stream to open/play. */
                        ...
                        if ( op.granulepos != -1 ) {
                            ...
                            ogg_int64_t granulepos =
                                op.granulepos - vf->pcmlengths[ link * 2 ];
                            if ( granulepos < 0 )
                                granulepos = 0;
    ...
    /* new search algorithm by HB (Nicholas Vinen) */
    {
        ...
        while ( begin < end ) {
            ...
            if ( end - begin < CHUNKSIZE ) {
                ...
                bisect = begin +
                         ( ogg_int64_t )( ( double )( target - begintime ) *
                                          ( end - begin ) /
                                          ( endtime - begintime ) ) -
                         CHUNKSIZE;
                if ( bisect <= begin )
            ...
            while ( begin < end ) {
                ...
                if ( result < 0 ) {
                    ...
                    if ( granulepos < target ) {
                        best =
                            result; /* raw offset of packet with granulepos */
                        ...
                        if ( bisect <= begin + 1 )
                            ...
                            if ( end == vf->offset ) { /* we're pretty close -
                                                          we'd be stuck in */
                                end = result;
                                bisect -=
                                    CHUNKSIZE; /* an endless loop otherwise. */
                                if ( bisect <= begin )
                                    bisect = begin + 1;
                                result = _seek_helper( vf, bisect );
                                if ( result )
           ...
           raw_seek, don't keep packets preceeding granulepos. */
        {
            ...
            while ( 1 ) {
                ...
                if ( op.granulepos != -1 ) {
                    vf->pcm_offset =
                        op.granulepos - vf->pcmlengths[ vf->current_link * 2 ];
                    if ( vf->pcm_offset < 0 )
                        vf->pcm_offset = 0;
                    vf->pcm_offset += total;
                    break;
...
int ov_pcm_seek( OggVorbis_File* vf, ogg_int64_t pos ) {
    ...
    while ( 1 ) {
        ...
        if ( ret > 0 ) {
            thisblock =
                vorbis_packet_blocksize( vf->vi + vf->current_link, &op );
            if ( thisblock < 0 ) {
                ogg_stream_packetout( &vf->os, NULL );
            ...
            if ( vf->pcm_offset +
                     ( ( thisblock + vorbis_info_blocksize( vf->vi, 1 ) ) >>
                       2 ) >=
                 pos )
                break;
...
double ov_time_tell( OggVorbis_File* vf ) {
    ...
    return ( ( double )time_total +
             ( double )( vf->pcm_offset - pcm_total ) / vf->vi[ link ].rate );
}
...
long ov_read( OggVorbis_File* vf,
              ...
              int sgned,
              int* bitstream ) {
    return ov_read_filter( vf, buffer, length, bigendianp, word, sgned,
                           bitstream, NULL, NULL );
}
...
static int _ov_initprime( OggVorbis_File* vf ) {
    ...
    while ( 1 ) {
        if ( vf->ready_state == INITSET )
            if ( vorbis_synthesis_pcmout( vd, NULL ) )
                break;
...
static void _ov_getlap( OggVorbis_File* vf,
                        vorbis_info* vi,
                        vorbis_dsp_state* vd,
                        float** lappcm,
                        int lapsize ) {
    ...
    while ( lapcount < lapsize ) {
        ...
        if ( samples ) {
            ...
            for ( i = 0; i < vi->channels; i++ )
                memcpy( lappcm[ i ] + lapcount, pcm[ i ],
                        sizeof( **pcm ) * samples );
            lapcount += samples;
            ...
            /* suck in another packet */
            int ret =
                _fetch_and_process_packet( vf, NULL, 1, 0 ); /* do *not* span */
            if ( ret == OV_EOF )
                break;
        }
    }
...
static int _ov_64_seek_lap( OggVorbis_File* vf,
                            ogg_int64_t pos,
...
static int _ov_d_seek_lap( OggVorbis_File* vf,
                           double pos,
