...
void vorbis_lsp_to_curve( float* curve,
    ...
    while ( i < n ) {
        ...
        q = frexp( p + q, &qexp );
        q = vorbis_fromdBlook( amp * vorbis_invsqlook( q ) *
...
void vorbis_lsp_to_curve( float* curve,
    ...
    for ( i = 0; i < m; i++ )
        ilsp[ i ] = vorbis_coslook_i( lsp[ i ] / M_PI * 65536.f + .5f );
...
static int Laguerre_With_Deflation( float* a, int ord, float* r ) {
    ...
    for ( m = ord; m > 0; m-- ) {
        ...
        while ( 1 ) {
            ...
            if ( denom < 0 )
                return ( -1 ); /* complex root!  The LPC generator handed us a
                                  bad filter */
