...
static int _ve_amp( envelope_lookup* ve,
    ...
    {
        float temp = vec[ 0 ] * vec[ 0 ] + .7 * vec[ 1 ] * vec[ 1 ] +
                     .2 * vec[ 2 ] * vec[ 2 ];
...
long _ve_envelope_search( vorbis_dsp_state* v ) {
...

    {
        ...
        long testW = centerW + ci->blocksizes[ v->W ] / 4 +
                     ci->blocksizes[ 1 ] / 2 + ci->blocksizes[ 0 ] / 4;

        j = ve->cursor;

        while ( j < ve->current -
                        ( ve->searchstep ) ) { /* account for postecho
...
void _ve_envelope_shift( envelope_lookup* e, long shift ) {
    int smallsize =
        e->current / e->searchstep + VE_POST; /* adjust for placing marks
                                                 ahead of ve->current */
    int smallshift = shift / e->searchstep;

    memmove( e->mark, e->mark + smallshift,
             ( smallsize - smallshift ) * sizeof( *e->mark ) );
