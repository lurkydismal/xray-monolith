...
float vorbis_coslook( float a ) {
    ...
    return COS_LOOKUP[ i ] +
           ( d - i ) * ( COS_LOOKUP[ i + 1 ] - COS_LOOKUP[ i ] );
}

/* interpolated 1./sqrt(p) where .5 <= p < 1. */
float vorbis_invsqlook( float a ) {
    double d =
        a * ( 2.f * ( float )INVSQ_LOOKUP_SZ ) - ( float )INVSQ_LOOKUP_SZ;
    int i = vorbis_ftoi( d - .5f );
    return INVSQ_LOOKUP[ i ] +
           ( d - i ) * ( INVSQ_LOOKUP[ i + 1 ] - INVSQ_LOOKUP[ i ] );
}
...
long vorbis_invsqlook_i( long a, long e ) {
    ...
    e += 32;
    if ( e & 1 )
        val = ( val * 5792 ) >> 13; /* multiply val by 1/sqrt(2) */
    e = ( e >> 1 ) - 8;
