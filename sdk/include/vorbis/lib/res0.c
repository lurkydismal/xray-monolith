            ...
            for ( j = 0; j < look->parts; j++ ) {
                ...
                for ( k = 0; k < 8; k++ )
                    if ( look->training_data[ k ][ j ] ) {
                        ...
                        /* long and short into the same bucket by current
                         * convention */
...
void res0_pack( vorbis_info_residue* vr, oggpack_buffer* opb ) {
    ...
    oggpack_write( opb, info->grouping - 1,
                   24 ); /* residue vectors to group and
                           code with a partitioned book */
    oggpack_write( opb, info->partitions - 1,
                   6 );                       /* possible partition choices */
    ...
    for ( j = 0; j < info->partitions; j++ ) {
        if ( ilog( info->secondstages[ j ] ) > 3 ) {
            ...
            oggpack_write( opb, info->secondstages[ j ],
                           4 ); /* trailing zero */
...
vorbis_look_residue* res0_look( vorbis_dsp_state* vd,
    ...
    for ( j = 0; j < look->parts; j++ ) {
        ...
        if ( stages ) {
            if ( stages > maxstage )
                maxstage = stages;
            look->partbooks[ j ] =
                _ogg_calloc( stages, sizeof( *look->partbooks[ j ] ) );
            for ( k = 0; k < stages; k++ )
                if ( info->secondstages[ j ] & ( 1 << k ) ) {
                    look->partbooks[ j ][ k ] =
                        ci->fullbooks + info->booklist[ acc++ ];
#ifdef TRAIN_RES
                    look->training_data[ k ][ j ] =
    ...
    look->decodemap =
        _ogg_malloc( look->partvals * sizeof( *look->decodemap ) );
    for ( j = 0; j < look->partvals; j++ ) {
        long val = j;
        long mult = look->partvals / look->parts;
        look->decodemap[ j ] =
            _ogg_malloc( dim * sizeof( *look->decodemap[ j ] ) );
        for ( k = 0; k < dim; k++ ) {
            long deco = val / mult;
            val -= deco * mult;
            mult /= look->parts;
...
static long** _01class( vorbis_block* vb,
    ...
    for ( i = 0; i < ch; i++ ) {
        partword[ i ] = _vorbis_block_alloc(
            vb, n / samples_per_partition * sizeof( *partword[ i ] ) );
        memset( partword[ i ], 0,
                n / samples_per_partition * sizeof( *partword[ i ] ) );
    }
    ...
    for ( i = 0; i < partvals; i++ ) {
        ...
        for ( j = 0; j < ch; j++ ) {
            ...
            for ( k = 0; k < samples_per_partition; k++ ) {
                if ( fabs( in[ j ][ offset + k ] ) > max )
                    max = fabs( in[ j ][ offset + k ] );
...
static long** _2class( vorbis_block* vb,
    ...
    memset( partword[ 0 ], 0,
            n * ch / samples_per_partition * sizeof( *partword[ 0 ] ) );
...
static int _01forward(
    ...
    int ch,
    long** partword,
    int ( *encode )( oggpack_buffer*, float*, int, codebook*, long* ) ) {
    long i, j, k, s;
    vorbis_look_residue0* look = ( vorbis_look_residue0* )vl;
    vorbis_info_residue0* info = look->info;
        ...
        for ( i = 0; i < partvals; ) {
            ...
            if ( s == 0 ) {
                for ( j = 0; j < ch; j++ ) {
                    ...
                    if ( val < look->phrasebook->entries )
                        look->phrasebits +=
                            vorbis_book_encode( look->phrasebook, val, opb );
#if 0 /*def TRAIN_RES*/
          else
            fprintf(stderr,"!");
#endif
            ...
            for ( k = 0; k < partitions_per_word && i < partvals; k++, i++ ) {
                ...
                for ( j = 0; j < ch; j++ ) {
                    ...
                    if ( info->secondstages[ partword[ j ][ i ] ] &
                         ( 1 << s ) ) {
                        codebook* statebook =
                            look->partbooks[ partword[ j ][ i ] ][ s ];
                        if ( statebook ) {
                            int ret;
...
#ifdef TRAIN_RES
                            accumulator =
                                look->training_data[ s ][ partword[ j ][ i ] ];
                            {
                                int l;
                            ...
                            ret = encode( opb, in[ j ] + offset,
                                          samples_per_partition, statebook,
                                          accumulator );
...
static int _01inverse(
    ...
    if ( n > 0 ) {
        ...
        for ( j = 0; j < ch; j++ )
            partword[ j ] =
                _vorbis_block_alloc( vb, partwords * sizeof( *partword[ j ] ) );
        ...
        for ( s = 0; s < look->stages; s++ ) {
            ...
            for ( i = 0, l = 0; i < partvals; l++ ) {
                if ( s == 0 ) {
                    ...
                    for ( j = 0; j < ch; j++ ) {
                        int temp =
                            vorbis_book_decode( look->phrasebook, &vb->opb );
                ...
                for ( k = 0; k < partitions_per_word && i < partvals; k++, i++ )
                    for ( j = 0; j < ch; j++ ) {
                        ...
                        if ( info->secondstages[ partword[ j ][ l ][ k ] ] &
                             ( 1 << s ) ) {
                            codebook* stagebook =
                                look->partbooks[ partword[ j ][ l ][ k ] ][ s ];
                            if ( stagebook ) {
                                if ( decodepart( stagebook, in[ j ] + offset,
                                                 &vb->opb,
...
int res2_inverse( vorbis_block* vb,
    ...
    if ( n > 0 ) {
        ...
        int partwords =
            ( partvals + partitions_per_word - 1 ) / partitions_per_word;
        int** partword =
            _vorbis_block_alloc( vb, partwords * sizeof( *partword ) );
        ...
        for ( s = 0; s < look->stages; s++ ) {
            for ( i = 0, l = 0; i < partvals; l++ ) {
                ...
                /* now we decode residual values for the partitions */
                for ( k = 0; k < partitions_per_word && i < partvals; k++, i++ )
                    if ( info->secondstages[ partword[ l ][ k ] ] &
                         ( 1 << s ) ) {
                        codebook* stagebook =
...
const vorbis_func_residue residue0_exportbundle = {
    NULL, &res0_unpack, &res0_look,   &res0_free_info, &res0_free_look,
    NULL, NULL,         &res0_inverse };

const vorbis_func_residue residue1_exportbundle = {
    &res0_pack,      &res0_unpack, &res0_look,    &res0_free_info,
    &res0_free_look, &res1_class,  &res1_forward, &res1_inverse };

const vorbis_func_residue residue2_exportbundle = {
    &res0_pack,      &res0_unpack, &res0_look,    &res0_free_info,
    &res0_free_look, &res2_class,  &res2_forward, &res2_inverse };
