...
int vorbis_bitrate_addblock( vorbis_block* vb ) {
    ...
    long min_target_bits =
        ( vb->W ? bm->min_bitsper * bm->short_per_long : bm->min_bitsper );
    long max_target_bits =
        ( vb->W ? bm->max_bitsper * bm->short_per_long : bm->max_bitsper );
    ...
    if ( !bm->managed ) {
        ...
        if ( bm->vb )
            return ( -1 ); /* one has been submitted without
                              being claimed */
        bm->vb = vb;
        return ( 0 );
    }
    ...
    if ( bm->avg_bitsper > 0 ) {
        ...
        long avg_target_bits =
            ( vb->W ? bm->avg_bitsper * bm->short_per_long : bm->avg_bitsper );
        ...
        if ( bm->avg_reservoir + ( this_bits - avg_target_bits ) >
             desired_fill ) {
    ...
    if ( bm->max_bitsper > 0 ) {
        ...
        if ( this_bits > max_target_bits ) {
            while ( bm->minmax_reservoir + ( this_bits - max_target_bits ) >
                    bi->reservoir_bits ) {
                choice--;
                if ( choice < 0 )
                    break;
    ...
    /* now we have the final packet and the final packet size.  Update
     * statistics */
    ...
    if ( bm->min_bitsper > 0 || bm->max_bitsper > 0 ) {
        if ( max_target_bits > 0 && this_bits > max_target_bits ) {
            ...
            /* inbetween; we want to take reservoir toward but not past
             * desired_fill */
