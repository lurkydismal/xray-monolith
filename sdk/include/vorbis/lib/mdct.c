...
void mdct_init( mdct_lookup* lookup, int n ) {
    ...
    for ( i = 0; i < n / 4; i++ ) {
        ...
        T[ n2 + i * 2 ] =
            FLOAT_CONV( cos( ( M_PI / ( 2 * n ) ) * ( 2 * i + 1 ) ) );
        T[ n2 + i * 2 + 1 ] =
            FLOAT_CONV( sin( ( M_PI / ( 2 * n ) ) * ( 2 * i + 1 ) ) );
    }
    for ( i = 0; i < n / 8; i++ ) {
        T[ n + i * 2 ] = FLOAT_CONV( cos( ( M_PI / n ) * ( 4 * i + 2 ) ) * .5 );
        T[ n + i * 2 + 1 ] =
            FLOAT_CONV( -sin( ( M_PI / n ) * ( 4 * i + 2 ) ) * .5 );
    }
...
STIN void mdct_butterflies( mdct_lookup* init, DATA_TYPE* x, int points ) {
    ...
    for ( i = 1; --stages > 0; i++ ) {
        for ( j = 0; j < ( 1 << i ); j++ )
            mdct_butterfly_generic( T, x + ( points >> i ) * j, points >> i,
                                    4 << i );
    }
...
void mdct_forward( mdct_lookup* init, DATA_TYPE* in, DATA_TYPE* out ) {
    ...
    for ( i = 0; i < n4; i++ ) {
        x0--;
        out[ i ] =
            MULT_NORM( ( w[ 0 ] * T[ 0 ] + w[ 1 ] * T[ 1 ] ) * init->scale );
        x0[ 0 ] =
            MULT_NORM( ( w[ 0 ] * T[ 1 ] - w[ 1 ] * T[ 0 ] ) * init->scale );
        w += 2;
        T += 2;
    }
}
