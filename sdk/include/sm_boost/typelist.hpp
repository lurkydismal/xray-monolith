...
struct typelist {
    ...
    template < typename T >
    static constexpr size_t index_of( type_identity< T > ) noexcept {
        return index_of_impl< T >( 0, typelist< Ts... >() );
    }

    template < typename T >
    static constexpr bool contains( type_identity< T > type ) noexcept {
...
private:
    ...
    static constexpr size_t index_of_impl( const size_t index,
                                           typelist< H, Hs... > ) noexcept {
        return std::is_same_v< T, H >
                   ? index
                   : index_of_impl< T >( index + 1, typelist< Hs... >() );
    }
...
struct ts_reverse< typelist< T, Ts... > > {
    using type =
        ts_append_t< typename ts_reverse< typelist< Ts... > >::type, T >;
};
...
struct ts_erase< typelist< H, Hs... >, T > {
    ...
    using type = std::
        conditional_t< std::is_same_v< T, H >, next, ts_prepend_t< H, next > >;
};
...
struct ts_applier< typelist< Ts... > > {
    ...
    static void apply( Handler&& handler ) {
        [[maybe_unused]]
        const int fold[] = { ( handler( type_identity< Ts >() ), 0 )..., 0 };
    }
};
