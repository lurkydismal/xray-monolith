...
// TODO: not sure if PQ application/normalization is correct, it's applied to
// channels, but technically should be applied to luminance (per channel
...
// Other
// HDR Analysis Shaders ::
// https://github.com/EndlesslyFlowering/ReShade_HDR_shaders/tree/master
// SpecialK HDR 		::
// https://github.com/SpecialKO/SpecialK/tree/12b1dda5b1b952e0fb77a1e810b9734f3ee30f81/resource/shaders/HDR
// DirectXTK HDR 		::
// https://github.com/Microsoft/DirectXTK/wiki/Using-HDR-rendering Xbox HDR
// ::
// https://github.com/microsoft/Xbox-GDK-Samples/blob/main/Kits/ATGTK/HDR/HDRCommon.hlsli#L199
// Rec.709 -> Rec.2020  ::
...
// see
// https://www.colour-science.org:8010/apps/rgb_colourspace_transformation_matrix?input-colourspace=ITU-R+BT.709&output-colourspace=P3-D65&chromatic-adaptation-transform=Bradford&formatter=str&decimals=8
...
// see:
// https://www.colour-science.org:8010/apps/rgb_colourspace_transformation_matrix?input-colourspace=ITU-R+BT.709&output-colourspace=ITU-R+BT.2020&chromatic-adaptation-transform=Bradford&formatter=str&decimals=8
...
// see:
// https://www.colour-science.org:8010/apps/rgb_colourspace_transformation_matrix?input-colourspace=P3-D65&output-colourspace=ITU-R+BT.709&chromatic-adaptation-transform=Bradford&formatter=str&decimals=8
...
// see:
// https://www.colour-science.org:8010/apps/rgb_colourspace_transformation_matrix?input-colourspace=P3-D65&output-colourspace=ITU-R+BT.2020&chromatic-adaptation-transform=Bradford&formatter=str&decimals=8
...
// see:
// https://www.colour-science.org:8010/apps/rgb_colourspace_transformation_matrix?input-colourspace=ITU-R+BT.2020&output-colourspace=ITU-R+BT.709&chromatic-adaptation-transform=Bradford&formatter=str&decimals=8
...
// see:
// https://www.colour-science.org:8010/apps/rgb_colourspace_transformation_matrix?input-colourspace=ITU-R+BT.2020&output-colourspace=DCI-P3&chromatic-adaptation-transform=Bradford&formatter=str&decimals=8
...
// NOTE: the luminance weight vector is the y-component (2nd row) of the CIE XYZ
// matrix for RGB -> CIE XYZ
...
float3 HDR10_LinearToLogC( float3 x ) {
    return ( x > LogC_cut ) ? ( LogC_c * log10( LogC_a * x + LogC_b ) + LogC_d )
                            : ( LogC_e * x + LogC_f );
}

float3 HDR10_LogCToLinear( float3 x ) {
    return ( x > LogC_e * LogC_cut + LogC_f )
               ? ( ( pow( 10.0, ( x - LogC_d ) / LogC_c ) - LogC_b ) / LogC_a )
               : ( ( x - LogC_f ) / LogC_e );
}
...
float3 HDR10_Tonemap_ACES_Hill( float3 color ) {
    static const float3x3 input_mtx = { { 0.59719, 0.35458, 0.04823 },
                                        { 0.07600, 0.90834, 0.01566 },
                                        { 0.02840, 0.13383, 0.83777 } };

    static const float3x3 output_mtx = { { 1.60475, -0.53108, -0.07367 },
...
float3 HDR10_Tonemap_Uchimura( float3 x,
                               float P,
                               float a,
                               float m,
                               float l,
...
float3 HDR10_Tonemap_Uncharted2( float3 x,
                                 float a,
                                 float b,
                                 float c,
                                 float d,
...
float3 HDR10_Tonemap_Uncharted2( float3 color ) {
    ...
    float3 white_scale =
        1.0 / HDR10_Tonemap_Uncharted2( w.xxx, a, b, c, d, e, f, w );
...
float3 HDR10_Tonemap_AgX_DefaultContrastApprox( float3 x ) {
    // original polynomial:
    // (15.5 * x^6) + (-40.14 * x^5) + (31.96 * x^4) + (-6.868 * x^3) + (0.4298
    // * x^2) + (0.1191 * x^1) + (-0.00232 * x^0)

    // improved polynomial:
    // NOTE: see https://www.shadertoy.com/view/mdcSDH comment by FordPerfect
    // (15.122061 * x^6) + (-38.901506 * x^5) + (30.376821 * x^4) + (-5.9293431
    // * x^3) + (0.2078625 * x^2) + (0.12410293 * x^1) exact endpoints plus the
    // error is better

    // Horner evaluation (fewer multiplies, all FMAs, better numerical
    // stability):
...
// NOTE: see https://www.shadertoy.com/view/cd3XWr
float3 HDR10_Tonemap_AgX_Look( float3 color,
                               float3 slope,
                               float3 power,
                               float saturation ) {
...
// TODO: https://iolite-engine.com/blog_posts/minimal_agx_implementation says
// that AgX expects input color space
//       to be sRGB/Rec.709. I'm not sure if this is because of the default
//       luminance calculation or the input/output transform matrices I adjusted
//       the luma calculation, no idea if/how to adjust the matrices
// NOTE: see https://www.shadertoy.com/view/cd3XWr
float3 HDR10_Tonemap_AgX_Input( float3 color ) {
    ...
    // log2 encoding
    // NOTE: see
    // https://github.com/ampas/aces-core/blob/518c27f577e99cdecfddf2ebcfaa53444b1f9343/transforms/ctl/utilities/ACESutil.Lin_to_Log2_param.ctl
...
float3 HDR10_TransformColorspace_ToTarget( float3 color ) {
    if ( HDR10_USE_COLORSPACE_P3D65 ) {
        return HDR10_ApplyColorspaceTransform(
            color, HDR10_CSTransform_Rec709_To_P3D65 );

    } else if ( HDR10_USE_COLORSPACE_REC2020 ) {
        return HDR10_ApplyColorspaceTransform(
            color, HDR10_CSTransform_Rec709_To_Rec2020 );
    }
...
float3 HDR10_TransformColorspace_ToDisplay( float3 color ) {
    if ( HDR10_USE_COLORSPACE_P3D65 ) {
        return HDR10_ApplyColorspaceTransform(
            color, HDR10_CSTransform_P3D65_To_Rec2020 );
...
float3 HDR10_ApplyColorGrading_Rec709( float3 color ) {
    ...
    color = ( color - HDR10_CONTRAST_MIDDLE_GRAY ) * HDR10_CONTRAST +
            HDR10_CONTRAST_MIDDLE_GRAY;
...
float3 HDR10_ToDisplay_World( float3 color ) {
    ...
    // color is [0, inf) in 2.2 gamma sRGB (because we disabled all tonemapping
    // in the code and the game seems to render in 2.2 gamma sRGB) we want
    // linear (sRGB/Rec.709 colorspace, but without non-linear OETF applied)
    // NOTE: sRGB/Rec.709 are equivalent color spaces, their OETFs (gamma curves
    // basically) aren't, but when linearized they are equivalent (identical
    ...
    // ST2084 curve operates on normalized luminance (unitless, but 1.0 from
    // tonemapper -> HDR_WHITEPOINT_NITS (cd/m2), normalized by ST2084 peak
    // cd/m2)
...
// Convert from sRGB to HDR10
// only applies the colorspace transform and nits scaling for UI elements (no
// tonemapping)
float3 HDR10_ToDisplay_UI( float3 color, float nits_scalar, float alpha ) {
    ...
    // linear (sRGB/Rec.709 colorspace, but without non-linear OETF applied)
    // NOTE: sRGB/Rec.709 are equivalent color spaces, their OETFs (gamma curves
    // basically) aren't, but when linearized they are equivalent
    color = HDR10_sRGBToLinear( color );

    // Apply UI saturation. This is a workaround for UI blending in non-linear
    // PQ colorspace Correct:   output = PQ(alpha * color) Incorrect: output =
    // alpha * PQ(color) at alpha = 1, the color is correct at alpha = 0, the
    // color is the most incorrect
